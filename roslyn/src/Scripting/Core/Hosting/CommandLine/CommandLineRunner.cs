...
namespace Microsoft.CodeAnalysis.Scripting.Hosting
{
    internal sealed class CommandLineRunner
    {
        ...
        private int RunScript(
            ...
            var script = Script.CreateInitialScript<int>(
                _scriptCompiler,
                code,
                options,
                globals.GetType(),
            ...
            try
            {
                return script.RunAsync(globals, cancellationToken)
                    .GetAwaiter()
                    .GetResult().ReturnValue;
            }
            catch (CompilationErrorException e)
            {
                _compiler.ReportDiagnostics(
                    e.Diagnostics,
                    _console.Error,
                    errorLogger,
                    compilation: null
        ...
        private static ScriptOptions UpdateOptions(
            ...
            return options.RemoveImportsAndReferences()
                .WithMetadataResolver(
                    currentMetadataResolver.WithRelativePathResolver(
                        currentMetadataResolver.PathResolver.WithBaseDirectory(newWorkingDirectory)
                            .WithSearchPaths(newReferenceSearchPaths)
                    )
                )
                .WithSourceResolver(
                    currentSourceResolver.WithBaseDirectory(newWorkingDirectory)
        ...
        private void DisplayDiagnostics(ImmutableArray<Diagnostic> diagnostics)
        {
            ...
            var ordered = diagnostics.OrderBy(
                (d1, d2) =>
                {
