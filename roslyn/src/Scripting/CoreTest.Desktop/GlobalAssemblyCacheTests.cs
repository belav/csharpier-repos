    ...
    public class GlobalAssemblyCacheTests
    {
        ...
        public void GetAssemblyIdentities()
        {
            ...
            foreach (var name in names)
            {
                ...
                Assert.True(
                    name.GetDisplayName().Contains("PublicKeyToken=b77a5c561934e089"),
            ...
            names = gac.GetAssemblyIdentities("nonexistingassemblyname" + Guid.NewGuid().ToString())
                .ToArray();
            Assert.Equal(0, names.Length);
        }
        ...
        {
            var names = ClrGlobalAssemblyCache
                .GetAssemblyObjects(
            ...
            var gacLocationsUpper = GlobalAssemblyCacheLocation.RootLocations.Select(location =>
                location.ToUpper()
            );
            foreach (var name in names)
            {
                ...
                Assert.True(
                    gacLocationsUpper.Any(gac =>
                        location.StartsWith(gac, StringComparison.OrdinalIgnoreCase)
                    ),
                    "Path within some GAC root"
                );
