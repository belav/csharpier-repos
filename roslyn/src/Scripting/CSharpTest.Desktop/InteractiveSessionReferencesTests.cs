...
namespace Microsoft.CodeAnalysis.CSharp.Scripting.Test
{
    ...
    {
        private static readonly CSharpCompilationOptions s_signedDll = new CSharpCompilationOptions(
            OutputKind.DynamicallyLinkedLibrary,
            cryptoPublicKey: TestResources.TestKeys.PublicKey_ce65828c82a341f2
        );
...
Process.GetCurrentProcess()"
        ...
        {
            var s = CSharpScript.RunAsync("using System;")
            ...
            ScriptingTestHelpers.AssertCompilationError(
                ...
                Diagnostic(ErrorCode.ERR_AmbigMember, "Version")
                    .WithArguments("System.Version", "System.Environment.Version")
            );
...
";
            ...
            ScriptingTestHelpers.AssertCompilationError(
                () => CSharpScript.EvaluateAsync(source, options),
...
",
            ...
            var options2 = options1.AddReferences(
...
",
            ...
            var options3 = options2.AddReferences(
...
"
            ...
            DiagnosticExtensions.VerifyEmitDiagnostics(
                script2.GetCompilation(),
                // (2,6): error CS0029: Cannot implicitly convert type 'C [{c2.Path}]' to 'C [{c1.Path}]'
                Diagnostic(ErrorCode.ERR_NoImplicitConv, "c2")
                    .WithArguments($"C [{c2.Path}]", $"C [{c1.Path}]")
            );
        }
        ...
        public void AssemblyResolution()
        {
            ...
            var s1 = s0.ContinueWith<Type>(
                "System.Type.GetType(x.GetType().AssemblyQualifiedName, true)"
            );
...
"
        ...
        public async Task MissingRefrencesAutoResolution()
        {
            ...
            // includes corlib, host type assembly by default:
            AssertEx.Equal(
                new[] { typeof(object).GetTypeInfo().Assembly.Location, "PortableLib" },
                c0.ExternalReferences.SelectAsArray(m => m.Display)
            );
            ...
            AssertEx.Equal(
                ...
                c0.GetBoundReferenceManager()
                    .GetReferencedAssemblies()
                    .Select(a => a.Value.Identity.GetDisplayName())
            );
        }
        ...
        {
            var lib = CreateCSharpCompilation(
                "public class C { public int X = 1, Y = 2; }",
                new[] { Net451.mscorlib },
                "HostLib"
            );
...
",
            ...
            var libBaseRefAndSymbol = c4.GetBoundReferenceManager()
                .GetReferencedAssemblies()
                .ToArray()[3];
            Assert.Equal(
                fileBase1.Path,
...
",
                new MetadataReference[] { Net451.mscorlib, libBase1.ToMetadataReference() },
                lib1Name
            );
...
",
            ...
            try
            {
                ...
                when (fileLoadEx.InnerException is InteractiveAssemblyLoaderException)
            {
                exceptionSeen = true;
            }
...
",
                new MetadataReference[] { Net451.mscorlib, libBase2.ToMetadataReference() },
                lib2Name
            );
...
";
            ...
            var s0 = await CSharpScript.RunAsync(
                "C c;",
                ScriptOptions.Default.WithReferences(libFile.Path)
            );
