    ...
    public class SourceTextTests
    {
        ...
        public void ChecksumAlgorithm_Default()
        {
            ...
            Assert.Equal(
                ...
                SourceText.From(bytes, bytes.Length).ChecksumAlgorithm
            );
        ...
        public void ChecksumAlgorithm1(SourceHashAlgorithm algorithm)
        {
            Assert.Equal(
                algorithm,
                SourceText.From(HelloWorld, checksumAlgorithm: algorithm).ChecksumAlgorithm
            );

            var bytes = s_unicode.GetBytes(HelloWorld);
            Assert.Equal(
                algorithm,
                SourceText.From(bytes, bytes.Length, checksumAlgorithm: algorithm).ChecksumAlgorithm
            );

            var stream = new MemoryStream(bytes);
            Assert.Equal(
                algorithm,
                SourceText.From(stream, checksumAlgorithm: algorithm).ChecksumAlgorithm
            );
        }
        ...
        public void ChecksumAndBOM()
        {
            ...
            var checksumNoBOM = ImmutableArray.Create<byte>(
                0xa,
                0xa,
                0x9f,
                0x2a,
            ...
            var checksumBOM = ImmutableArray.Create<byte>(
                0xb2,
                0x19,
                0x0,
                0x9b,
            ...
            var expectedContentHash = ImmutableArray.Create<byte>(
                0xa9,
                0x94,
                0xb1,
                0x1f,
            ...
            verifyChecksumAndContentHash(
                SourceText.From(HelloWorld, encodingNoBOM, checksumAlgorithm),
                checksumNoBOM,
                expectedContentHash
            );
            verifyChecksumAndContentHash(
                SourceText.From(HelloWorld, encodingBOM, checksumAlgorithm),
                checksumBOM,
                expectedContentHash
            );

            var bytesNoBOM = "Hello, World!"u8.ToArray();
            var bytesBOM = new byte[]
            {
                0xef,
                0xbb,
                0xbf,
            ...
            verifyChecksumAndContentHash(
                SourceText.From(bytesNoBOM, bytesNoBOM.Length, null, checksumAlgorithm),
                checksumNoBOM,
                expectedContentHash
            );
            verifyChecksumAndContentHash(
            ...
            verifyChecksumAndContentHash(
                SourceText.From(bytesNoBOM, bytesNoBOM.Length, encodingBOM, checksumAlgorithm),
                checksumNoBOM,
                expectedContentHash
            );
            ...
            verifyChecksumAndContentHash(
                SourceText.From(bytesBOM, bytesBOM.Length, null, checksumAlgorithm),
                checksumBOM,
                expectedContentHash
            );
            verifyChecksumAndContentHash(
                SourceText.From(bytesBOM, bytesBOM.Length, encodingNoBOM, checksumAlgorithm),
                checksumBOM,
                expectedContentHash
            );
            verifyChecksumAndContentHash(
            ...
            verifyChecksumAndContentHash(
                SourceText.From(streamNoBOM, null, checksumAlgorithm),
                checksumNoBOM,
                expectedContentHash
            );
            verifyChecksumAndContentHash(
            ...
            verifyChecksumAndContentHash(
                SourceText.From(streamBOM, null, checksumAlgorithm),
                checksumBOM,
                expectedContentHash
            );
            verifyChecksumAndContentHash(
                SourceText.From(streamBOM, encodingNoBOM, checksumAlgorithm),
                checksumBOM,
                expectedContentHash
            );
            verifyChecksumAndContentHash(
            ...
            verifyChecksumAndContentHash(
                FromLargeTextWriter(HelloWorld, encodingBOM, checksumAlgorithm),
                checksumBOM,
                expectedContentHash
            );
            ...
            verifyChecksumAndContentHash(
                fromChanges(SourceText.From(HelloWorld, encodingBOM, checksumAlgorithm)),
                checksumBOM,
                expectedContentHash
            );
            ...
            verifyChecksumAndContentHash(
                fromChanges(SourceText.From(streamNoBOM, encodingNoBOM, checksumAlgorithm)),
                checksumNoBOM,
                expectedContentHash
            );
            verifyChecksumAndContentHash(
                fromChanges(SourceText.From(streamNoBOM, encodingBOM, checksumAlgorithm)),
                checksumBOM,
                expectedContentHash
            );
            ...
            verifyChecksumAndContentHash(
                fromChanges(SourceText.From(streamBOM, encodingNoBOM, checksumAlgorithm)),
                checksumBOM,
                expectedContentHash
            );
            verifyChecksumAndContentHash(
                fromChanges(SourceText.From(streamBOM, encodingBOM, checksumAlgorithm)),
                checksumBOM,
                expectedContentHash
            );
            ...
            static void verifyChecksumAndContentHash(
                SourceText text,
                ImmutableArray<byte> expectedChecksum,
                ImmutableArray<byte> expectedContentHash
            )
            {
        ...
        private static SourceText FromLargeTextWriter(
            string source,
            Encoding encoding,
        ...
        public void ContentEquals()
        {
            ...
            Assert.True(
                SourceText
                    .From(HelloWorld, s_utf8)
                    .ContentEquals(SourceText.From(HelloWorld, s_utf8))
            );

            var e1 = EncodedStringText.Create(
                new MemoryStream(s_unicode.GetBytes(HelloWorld)),
                s_unicode
            );
            var e2 = EncodedStringText.Create(
                new MemoryStream(s_utf8.GetBytes(HelloWorld)),
                s_utf8
            );
        ...
        public void ContentEqualsLarge()
        {
            ...
            foreach (var sourceText1 in allSourceTexts)
            {
                foreach (var sourceText2 in allSourceTexts)
                {
                    ...
                    Assert.True(
                        sourceText1.GetContentHash().SequenceEqual(sourceText2.GetContentHash())
                    );
                }
            }
        }
            ...
            foreach (var encoding1 in encodings)
            {
                ...
                foreach (var encoding2 in encodings)
                {
                    ...
                    if (encoding1 == Encoding.ASCII && encoding2 == Encoding.ASCII)
                        Assert.True(
                            sourceText1.GetChecksum().SequenceEqual(sourceText2.GetChecksum())
                        );
                }
            }
        }
        ...
        public void IsBinary()
        {
            ...
            Assert.False(
                SourceText.IsBinary(encoding.GetString(new byte[] { 0x81, 0x8D, 0x8F, 0x90, 0x9D }))
            );
            // Unicode string: äëïöüû
            Assert.False(
                SourceText.IsBinary("abc def baz aeiouy \u00E4\u00EB\u00EF\u00F6\u00FC\u00FB")
            );
            Assert.True(
                SourceText.IsBinary(encoding.GetString(TestMetadata.ResourcesNet451.System))
            );
        }
        ...
        private static void TestTryReadByteOrderMark(
        ...
        public void TryReadByteOrderMark()
        {
            ...
            TestTryReadByteOrderMark(
                expectedEncoding: null,
                expectedPreambleLength: 0,
                data: new byte[] { 0xef, 0xbb }
            );
            TestTryReadByteOrderMark(
                expectedEncoding: null,
                expectedPreambleLength: 0,
                data: new byte[] { 0xef, 0xBB, 0xBF },
                validLength: 2
            );
            TestTryReadByteOrderMark(
                expectedEncoding: Encoding.UTF8,
                expectedPreambleLength: 3,
                data: new byte[] { 0xef, 0xBB, 0xBF }
            );
            ...
            TestTryReadByteOrderMark(
                expectedEncoding: Encoding.Unicode,
                expectedPreambleLength: 2,
            ...
            TestTryReadByteOrderMark(
                expectedEncoding: Encoding.BigEndianUnicode,
                expectedPreambleLength: 2,
                data: new byte[] { 0xfe, 0xff }
            );
        }
