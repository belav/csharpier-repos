...
namespace Microsoft.CodeAnalysis.UnitTests.Diagnostics
{
    public partial class DiagnosticLocalizationTests
    {
        ...
        public void TestDiagnosticLocalization()
        {
            ...
            var defaultCultureResourceSet = resourceManager.GetResourceSet(
            ...
            var localizableTitle = new LocalizableResourceString(
                nameOfResource1,
                resourceManager,
                typeof(CustomResourceManager)
            );
            var localizableMessageFormat = new LocalizableResourceString(
                nameOfResource2,
                resourceManager,
                typeof(CustomResourceManager)
            );
            var localizableDescription = new LocalizableResourceString(
                nameOfResource3,
                resourceManager,
                typeof(CustomResourceManager)
            );
            ...
            Assert.Equal<string>(
                fixedTitle,
                descriptor.Title.ToString(CustomResourceManager.DefaultCulture)
            );
            Assert.Equal<string>(
                fixedMessageFormat,
                descriptor.MessageFormat.ToString(CustomResourceManager.DefaultCulture)
            );
            Assert.Equal<string>(
                fixedDescription,
                descriptor.Description.ToString(CustomResourceManager.DefaultCulture)
            );
            ...
            Assert.Equal(
                fixedMessageFormat,
                localizableDiagnostic.GetMessage(CustomResourceManager.DefaultCulture)
            );
            Assert.Equal(
                fixedDescription,
                localizableDiagnostic
            ...
            Assert.Equal(
                localizedTitle,
                localizableDiagnostic.Descriptor.Title.ToString(arCulture)
            );
            Assert.Equal(localizedMessageFormat, localizableDiagnostic.GetMessage(arCulture));
            Assert.Equal(
                localizedDescription,
                localizableDiagnostic.Descriptor.Description.ToString(arCulture)
            );
            ...
            Assert.Equal(
                expected,
                localizableResource.ToString(CustomResourceManager.DefaultCulture)
            );
        ...
        private class CustomResourceManager : ResourceManager
        {
            ...
            public void VerifyResourceValue(
                string resourceName,
                string cultureName,
        ...
        private static void TestDescriptorIsExceptionSafeCore(DiagnosticDescriptor descriptor)
        {
            ...
            Action<
                ...
                Diagnostic,
                CancellationToken
            > onAnalyzerException = (ex, a, diag, ct) => exceptionDiagnostics.Add(diag);
        ...
        private static DiagnosticDescriptor GetDescriptorWithLocalizableResourceStringsThatThrow()
        {
            ...
            var localizableTitle = new LocalizableResourceString(
                "NonExistentTitleResourceName",
                resourceManager,
                typeof(CustomResourceManager)
            );
            var localizableMessage = new LocalizableResourceString(
                "NonExistentMessageResourceName",
                resourceManager,
                typeof(CustomResourceManager)
            );
            var localizableDescription = new LocalizableResourceString(
                "NonExistentDescriptionResourceName",
                resourceManager,
                typeof(CustomResourceManager)
            );
