...
namespace Microsoft.CodeAnalysis.UnitTests
{
    public class VersionHelperTests
    {
        ...
        public void TimeBased()
        {
            ...
            var version = VersionHelper.GenerateVersionFromPatternAndCurrentTime(
                now,
                new Version(3, 2, 65535, 65535)
            );
            Assert.Equal(3, version.Major);
            Assert.Equal(2, version.Minor);
            ...
            version = VersionHelper.GenerateVersionFromPatternAndCurrentTime(
                now,
                new Version(1, 2, 3, 65535)
            );
        ...
        public void ParseBad()
        {
            ...
            Assert.Equal(expected, version);
            Assert.False(
                VersionHelper.TryParseAssemblyVersion("", allowWildcard: true, version: out version)
            );
            Assert.Equal(expected, version);
            Assert.False(
                VersionHelper.TryParseAssemblyVersion(
        ...
        public void ParsePartial()
        {
            ...
            Assert.False(
                VersionHelper.TryParse(
                    "1.1.18446744073709551617999999999999999999999999900001.1",
                    out version
                )
            );
            ...
            Assert.False(
                VersionHelper.TryParse(
                    "1.1.18446744073709551617999999999999999999999999900001garbage.1",
                    out version
                )
            );
            ...
            Assert.False(
                VersionHelper.TryParse(
                    "1.1.18446744073709551617999999999999999999999999900001.23garbage",
                    out version
                )
            );
