...
namespace Microsoft.CodeAnalysis.UnitTests
{
    ...
    public sealed class DefaultAnalyzerAssemblyLoaderTests : TestBase
    {
        ...
        private void Run(
            Action<DefaultAnalyzerAssemblyLoader, AssemblyLoadTestFixture> action,
        ...
        )
        {
            ...
            var method = util.GetType()
                .GetMethod("Exec", BindingFlags.Public | BindingFlags.Instance)!;
            method.Invoke(
                util,
                new object[] { action.Method.DeclaringType!.FullName!, action.Method.Name }
            );
        ...
        public void LoadWithDependency()
        {
            Run(
                ...
                ) =>
                {
                    ...
                    var analyzerMainReference = new AnalyzerFileReference(
                        analyzerMainFile.Path,
                        loader
                    );
            ...
            Run(
                ...
                ) =>
                {
                    ...
                    Assert.Throws<FileNotFoundException>(
                        () => loader.LoadFromPath(testFixture.Beta.Path)
                    );
                }
            );
        }
        ...
        public void AssemblyLoading_DependencyInDifferentDirectory()
        {
            Run(
                ...
                ) =>
                {
                    ...
                    var deltaFile = temp.CreateDirectory()
                        .CreateFile("Delta.dll")
                        .CopyContentFrom(testFixture.Delta1.Path);
                    var gammaFile = temp.CreateDirectory()
                        .CreateFile("Gamma.dll")
                        .CopyContentFrom(testFixture.Gamma.Path);
...
",
                    ...
                    VerifyDependencyAssemblies(loader, deltaFile.Path, gammaFile.Path);
                }
            );
        }
        ...
        public void AssemblyLoading_DependencyInDifferentDirectory2()
        {
            Run(
                ...
                ) =>
                {
                    ...
                    var deltaFile1 = temp.CreateDirectory()
                        .CreateFile("Delta.dll")
                        .CopyContentFrom(testFixture.Delta1.Path);
...
",
                    ...
                    VerifyDependencyAssemblies(loader, deltaFile2.Path, gammaFile.Path);
                }
            );
        }
            ...
            Run(
                ...
                ) =>
                {
                    ...
                    var epsilonFile = tempDir
                        ...
                        .CopyContentFrom(testFixture.Epsilon.Path);
                    ...
                    VerifyDependencyAssemblies(loader, delta1File.Path, epsilonFile.Path);
...
",
        ...
        public void AssemblyLoading_MultipleVersions_MultipleLoaders()
        {
            Run(
                static (
                    DefaultAnalyzerAssemblyLoader loader1,
                    AssemblyLoadTestFixture testFixture
                ) =>
                {
                    StringBuilder sb = new StringBuilder();
        ...
        public void AssemblyLoading_UnifyToHighest()
        {
            Run(
                ...
                ) =>
                {
                    ...
                    var epsilon = loader.LoadFromPath(
                        testFixture.EpsilonReferencingPublicSigned.Path
                    );
                    var e = epsilon.CreateInstance("Epsilon.E")!;
                    e.GetType().GetMethod("Write")!.Invoke(e, new object[] { sb, "Test E" });
            ...
            Run(
                ...
                ) =>
                {
                    ...
                    loader.AddDependencyLocation(
                        testFixture.AnalyzerReferencesSystemCollectionsImmutable1.Path
                    );
        ...
        public void AssemblyLoading_AnalyzerReferencesSystemCollectionsImmutable_02()
        {
            Run(
                ...
                ) =>
                {
                    ...
                    loader.AddDependencyLocation(testFixture.UserSystemCollectionsImmutable.Path);
                    loader.AddDependencyLocation(
                        testFixture.AnalyzerReferencesSystemCollectionsImmutable2.Path
                    );
        ...
        public void AssemblyLoading_NativeDependency()
        {
            Run(
                ...
                ) =>
                {
                    ...
                    Assembly analyzerAssembly = loader.LoadFromPath(
                        testFixture.AnalyzerWithNativeDependency.Path
                    );
                    var analyzer = analyzerAssembly.CreateInstance("Class1")!;
                    var result = analyzer
                        .GetType()
                        .GetMethod("GetFileAttributes")!
                        .Invoke(analyzer, new[] { testFixture.AnalyzerWithNativeDependency.Path });
...
",
        ...
        public void AssemblyLoadingInNonDefaultContext_AnalyzerReferencesSystemCollectionsImmutable()
        {
            ...
            _ = compilerContext.LoadFromAssemblyPath(
                testFixture.UserSystemCollectionsImmutable.Path
            );
            _ = compilerContext.LoadFromAssemblyPath(
...
",
        ...
        private void AssemblyLoadingInNonDefaultContextHelper2(StringBuilder sb)
        {
            ...
            Assembly analyzerAssembly = loader.LoadFromPath(
                testFixture.AnalyzerReferencesDelta1.Path
            );
            var analyzer = analyzerAssembly.CreateInstance("Analyzer")!;
            analyzer.GetType().GetMethod("Method")!.Invoke(analyzer, new object[] { sb });
        }
