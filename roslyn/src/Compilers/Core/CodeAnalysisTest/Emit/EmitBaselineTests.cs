...
namespace Microsoft.CodeAnalysis.UnitTests.Emit
{
    public class EmitBaselineTests
    {
        ...
        {
            var debugInfoProvider = new Func<
            ...
            var mdBytesHandle = GCHandle.Alloc(
                mdBytes.DangerousGetUnderlyingArray(),
                GCHandleType.Pinned
            );
            var mdModule = ModuleMetadata.CreateFromMetadata(
                mdBytesHandle.AddrOfPinnedObject(),
                mdBytes.Length
            );
