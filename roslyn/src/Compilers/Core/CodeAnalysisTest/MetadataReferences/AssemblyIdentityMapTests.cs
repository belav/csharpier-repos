...
namespace Microsoft.CodeAnalysis.UnitTests
{
    public class AssemblyIdentityMapTests
    {
        ...
        public void Map()
        {
            ...
            Assert.True(
                map.TryGetValue(new AssemblyIdentity("a", new Version(1, 0, 0, 0)), out value)
            );
            Assert.Equal(10, value);

            Assert.True(map.Contains(new AssemblyIdentity("a", new Version(1, 1, 0, 0))));
            Assert.True(
                map.TryGetValue(new AssemblyIdentity("a", new Version(1, 1, 0, 0)), out value)
            );
            Assert.Equal(15, value);

            Assert.True(
                map.Contains(
                    new AssemblyIdentity("a", new Version(1, 0, 0, 0)),
            ...
            Assert.False(
                map.Contains(
                    new AssemblyIdentity("a", new Version(1, 1, 0, 0)),
                    allowHigherVersion: false
                )
            );
            Assert.False(
            ...
            Assert.False(
                map.Contains(
                    new AssemblyIdentity("b", new Version(1, 1, 0, 0)),
                    allowHigherVersion: true
                )
            );
            Assert.False(
                map.Contains(
            ...
            Assert.True(
                map.TryGetValue(new AssemblyIdentity("b", new Version(1, 0, 0, 0)), out value)
            );
            Assert.Equal(10, value);
        }
    }
