    ...
    public class AssemblyIdentityComparerTests : AssemblyIdentityTestBase
    {
        ...
        )
        {
            ...
            )
            {
                ...
                {
                    using (
                        var policyStream = new FileStream(
                            policyPath,
                            FileMode.Open,
                            FileAccess.Read
                        )
                ...
                var fusionResult1 = FusionAssemblyIdentityComparer.CompareAssemblyIdentity(
                    displayName1,
                    displayName2,
                    ignoreVersion,
                    policy: fusionPolicy,
                ...
                Assert.Equal(
                    fusionUnificationApplied ?? unificationApplied,
                    fusionUnificationApplied1
                );

                AssemblyIdentity id1,
                    id2;
                AssemblyIdentityParts parts1,
                    parts2;

                Assert.True(
                    AssemblyIdentity.TryParseDisplayName(displayName1, out id1, out parts1)
                );
                Assert.Equal(partial, !AssemblyIdentity.IsFullName(parts1));

                Assert.True(
                    AssemblyIdentity.TryParseDisplayName(displayName2, out id2, out parts2)
                );
                Assert.True(AssemblyIdentity.IsFullName(parts2), "Expected full name");

                bool unificationApplied1;
                var actual1 = comparer.Compare(
                    null,
                    displayName1,
                    id2,
                    out unificationApplied1,
                    ignoreVersion
                );
                ...
                if (!partial && id1 != null)
                {
                    ...
                    var actual2 = comparer.Compare(
                        id1,
                        null,
                        id2,
                        out unificationApplied2,
                        ignoreVersion
                    );
