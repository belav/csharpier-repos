...
";
        ...
        public void FromBytes_SmallSpan()
        {
            ...
            var text = EmbeddedText.FromBytes(
                "pathToSmall",
                new ArraySegment<byte>(paddedBytes, 1, bytes.Length)
            );
        ...
        public void FromTextReader_Small()
        {
            ...
            var actual = SourceText.From(
                new StringReader(SmallSource),
                SmallSource.Length,
                Encoding.UTF8,
                SourceHashAlgorithm.Sha1
            );
        ...
        public void FromSource_Precomputed()
        {
            ...
            foreach (bool useStream in new[] { true, false })
            {
                ...
                Assert.Equal(SourceHashAlgorithm.Sha1, text.ChecksumAlgorithm);
                AssertEx.Equal(
                    SourceText.CalculateChecksum(bytes, 0, bytes.Length, SourceHashAlgorithm.Sha1),
                    source.GetChecksum()
                );
                AssertEx.Equal(source.GetChecksum(), text.Checksum);
                AssertEx.Equal(BitConverter.GetBytes(bytes.Length), text.Blob.Take(4));
        ...
        {
            var source = EncodedStringText.Create(
                new MemoryStream(new byte[] { 0xA9, 0x0D, 0x0A }),
                canBeEmbedded: true
            );
            var text = EmbeddedText.FromSource("pathToLarge", source);
