    ...
    public class AnalyzerFileReferenceTests : TestBase
    {
        ...
        public void AnalyzerFileReference_Errors()
        {
            Assert.Throws<ArgumentNullException>(
                "fullPath",
                () => new AnalyzerFileReference(null!, s_analyzerLoader)
            );
            Assert.Throws<ArgumentNullException>(
                "assemblyLoader",
                () => new AnalyzerFileReference(TempRoot.Root, null!)
            );

            // path must be absolute
            Assert.Throws<ArgumentException>(
                "fullPath",
                () => new AnalyzerFileReference("a.dll", s_analyzerLoader)
            );
        }
        ...
        public void DisplayAndId_BadPath()
        {
            ...
            var refBadPath = new AnalyzerFileReference(
                PathUtilities.CombinePathsUnchecked(TempRoot.Root, "\0<>|*.xyz"),
                loader
            );
        ...
        public void Equality()
        {
            ...
            var loader1 = new TestAnalyzerAssemblyLoader(
                loadFromPath: _ => throw new InvalidOperationException()
            );
            var loader2 = new TestAnalyzerAssemblyLoader(
                loadFromPath: _ => throw new InvalidOperationException()
            );
        ...
        {
            AnalyzerFileReference reference = CreateAnalyzerFileReference(
                Path.Combine(TempRoot.Root, "random.dll")
            );
        ...
        public void TestLoadErrors3()
        {
            ...
            var alphaDll = directory.CreateFile("Alpha.dll")
        ...
        {
            var directory = Temp.CreateDirectory();
            var textFile = directory.CreateFile("Goo.txt")
                .WriteAllText("I am the very model of a modern major general.");
        ...
        public void ValidAnalyzerReference_Id()
        {
            ...
            AssemblyIdentity.TryParseDisplayName(
                "Alpha, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null",
        ...
        {
            var directory = Temp.CreateDirectory();
            var analyzerDll = directory.CreateFile("Alpha.dll")
                .WriteAllBytes(TestResources.AnalyzerTests.FaultyAnalyzer);
        ...
        [ConditionalFact(
            typeof(CoreClrOnly),
            Reason = "Can't load a framework targeting generator, which these are in desktop"
        )]
...
{{
        ...
        public void TestLoadedAnalyzerOrderIsDeterministic()
        {
            ...
            var csharpAnalyzers = reference.GetAnalyzers(LanguageNames.CSharp)
            ...
            var vbAnalyzers = reference.GetAnalyzers(LanguageNames.VisualBasic)
                .Select(a => a.GetType().FullName)
                .ToArray();
            ...
            var allAnalyzers = reference.GetAnalyzersForAllLanguages()
                .Select(a => a.GetType().FullName)
                .ToArray();
        ...
        public void TestLoadedGeneratorOrderIsDeterministic()
        {
            ...
            var csharpGenerators = reference.GetGenerators(LanguageNames.CSharp)
                .Select(a => a.GetType().FullName)
                .ToArray();
            ...
            var vbGenerators = reference.GetGenerators(LanguageNames.VisualBasic)
                .Select(a => a.GetType().FullName)
                .ToArray();
            ...
            var allGenerators = reference.GetGeneratorsForAllLanguages()
