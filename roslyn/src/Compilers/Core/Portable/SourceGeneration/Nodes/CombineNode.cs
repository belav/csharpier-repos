        ...
        : IIncrementalGeneratorNode<(TInput1, TInput2)>
    {
        ...
        )
        {
            ...
            if (input1Table.IsCached && input2Table.IsCached && previousTable is not null)
            {
                ...
                {
                    return RecordStepsForCachedTable(
                        graphState,
                        previousTable,
                        input1Table,
                        input2Table
            ...
            var builder = graphState.CreateTableBuilder(
                previousTable,
                _name,
                _comparer,
                totalEntryItemCount
            ...
            foreach (var entry1 in input1Table)
            {
                ...
                var stepInputs = builder.TrackIncrementalSteps
        ...
        private NodeStateTable<(TInput1, TInput2)> RecordStepsForCachedTable(
            DriverStateTable.Builder graphState,
            NodeStateTable<(TInput1, TInput2)> previousTable,
            NodeStateTable<TInput1> input1Table,
            NodeStateTable<TInput2> input2Table
        )
        {
            ...
            {
                var stepInputs = ImmutableArray.Create(
                    (entry.Step!, entry.OutputIndex),
                    (input2Step!, 0)
                );
        ...
        ) => new CombineNode<TInput1, TInput2>(_input1, _input2, comparer, _name);

        public IIncrementalGeneratorNode<(TInput1, TInput2)> WithTrackingName(string name) =>
            new CombineNode<TInput1, TInput2>(_input1, _input2, _comparer, name);
