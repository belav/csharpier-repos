...
namespace Microsoft.CodeAnalysis.Emit
{
    internal abstract class DefinitionMap
    {
        ...
        protected DefinitionMap(IEnumerable<SemanticEdit> edits, EmitBaseline baseline)
        {
            ...
            _methodInstrumentations = edits
                ...
                .ToImmutableDictionary(
                    edit => (IMethodSymbolInternal)GetISymbolInternalOrNull(edit.NewSymbol!)!,
        ...
        public MethodDefinitionHandle GetPreviousMethodHandle(
            ...
            out IMethodSymbolInternal? peMethod
        )
        {
            ...
            {
                peMethod = (IMethodSymbolInternal?)
                    PreviousSourceToMetadataSymbolMatcher
                        .MapDefinition(oldMethodDef)
                        ?.GetInternalSymbol();
        ...
        protected abstract ImmutableArray<EncLocalInfo> GetLocalSlotMapFromMetadata(
            StandaloneSignatureHandle handle,
            EditAndContinueMethodDebugInformation debugInfo
        );
        protected abstract ITypeSymbolInternal? TryGetStateMachineType(
            MethodDefinitionHandle methodHandle
        );
        protected abstract IMethodSymbolInternal GetMethodSymbol(
            MethodDefinitionHandle methodHandle
        );
        ...
        )
        {
            ...
            IReadOnlyDictionary<
                (int syntaxOffset, AwaitDebugId debugId),
                StateMachineState
            ...
            else
            {
                ...
                {
                    firstUnusedIncreasingStateMachineState =
                        debugInfo.StateMachineStates.Max(s => s.StateNumber) + 1;
                    firstUnusedDecreasingStateMachineState =
        ...
        internal MethodInstrumentation GetMethodBodyInstrumentations(
            ...
            _methodInstrumentations.TryGetValue(method, out var instrumentation)
                ? instrumentation
        ...
        )
        {
            map = debugInfos.IsDefault
                ? null
                : debugInfos.ToDictionary(
                    entry => (entry.SyntaxOffset, entry.AwaitId),
                    entry => entry.StateNumber
                );
        }
        ...
        )
        {
            ...
            IEnumerable<IMethodSymbolInternal> recurse(
                ImmutableArray<ISymbolInternal> synthesizedMembers,
                bool inSpecificDisplayClass
            )
            {
                foreach (var synthesizedMember in synthesizedMembers)
                {
                    ...
                    if (
                        TryParseDisplayClassOrLambdaName(
                            memberName,
                            out int suffixIndex,
                            out char idSeparator,
                            out bool isDisplayClass,
                    ...
                    )
                    {
                        ...
                        if (hasDebugIds)
                        {
                            if (
                                !CommonGeneratedNames.TryParseDebugIds(
                                    suffixSpan,
                                    idSeparator,
                                    isMethodIdOptional: inSpecificDisplayClass,
                                    out parsedMethodId,
                                    out parsedEntityId
                                )
                            )
                        ...
                        if (isDisplayClass)
                        {
                            ...
                            var displayClassMembers =
                                (synthesizedMemberMap != null)
                                    ? synthesizedMemberMap[displayClass]
                                    : displayClass.GetMembers();

                            foreach (
                                var displayClassMember in recurse(
                                    displayClassMembers,
                                    inSpecificDisplayClass: hasDebugIds
                                )
                            )
                            {
        ...
        )
        {
            ...
            try
            {
                provider = Baseline.DebugInformationProvider(
                    MetadataTokens.MethodDefinitionHandle(methodRowId)
                );
            }
            catch (Exception e) when (e is InvalidDataException or IOException)
            {
            ...
            return getDeletedSynthesizedClosureMethods(
                peMethod.ContainingType.GetMembers(),
                synthesizedMemberMap: null,
                debugId,
                provider.Lambdas
            );
            ...
            )
            {
                ...
                foreach (
                    var method in GetSynthesizedClosureMethods(
                        synthesizedSiblings,
                        synthesizedMemberMap,
                        methodId,
                        lambdaIdSet
