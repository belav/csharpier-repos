...
namespace Microsoft.DiaSymReader
{
    internal static class SymUnmanagedWriterFactory
    {
        ...
        /// <remarks>
        /// Tries to load the implementation of the PDB writer from
        // Microsoft.DiaSymReader.Native.{platform}.dll library first.
        /// It searches for this library in the directory Microsoft.DiaSymReader.dll is loaded from,
        /// the application directory, the %WinDir%\System32 directory, and user directories in the DLL
        // search path, in this order.
        /// If not found in the above locations and <see
        // cref="SymUnmanagedWriterCreationOptions.UseAlternativeLoadPath"/> option is specified
        /// the directory specified by MICROSOFT_DIASYMREADER_NATIVE_ALT_LOAD_PATH environment variable is
        // also searched.
        /// If the Microsoft.DiaSymReader.Native.{platform}.dll library can't be found and <see
        // cref="SymUnmanagedWriterCreationOptions.UseComRegistry"/>
        /// option is specified checks if the PDB reader is available from a globally registered COM object.
        // This COM object is provided
        /// by .NET Framework and has limited functionality (features like determinism and source link are
        // not supported).
        /// </remarks>
        /// <exception cref="ArgumentNullException"><paramref name="metadataProvider"/>is null.</exception>
        /// <exception cref="DllNotFoundException">The SymWriter implementation is not available or failed
        // to load.</exception>
        /// <exception cref="SymUnmanagedWriterException">Error creating the PDB writer. See inner exception
        // for root cause.</exception>
        ...
        )
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // SymWriter only uses it for filling CodeView debug directory data when asked for them, but we
                    // never do.
