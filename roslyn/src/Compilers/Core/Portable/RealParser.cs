...
namespace Microsoft.CodeAnalysis
{
    ...
    internal static class RealParser
    {
            ...
            )
            {
                ...
                else if (normalExponent < this.MinBinaryExponent)
                {
                    ...
                    int denormalMantissaShift =
                        normalMantissaShift + normalExponent + this.ExponentBias - 1;
                ...
                ulong shiftedExponent =
                    ((ulong)(exponent + this.ExponentBias)) << DenormalMantissaBits;
                Debug.Assert((shiftedExponent & DenormalMantissaMask) == 0);
                Debug.Assert((mantissa & ~DenormalMantissaMask) == 0);
                Debug.Assert(
                    (shiftedExponent & ~(((1UL << this.ExponentBits) - 1) << DenormalMantissaBits))
                        == 0
        ...
        /// </summary>
        private static Status ConvertDecimalToFloatingPointBits(
        ...
        )
        {
            ...
            BigInteger integerValue = AccumulateDecimalDigitsIntoBigInteger(
                data,
                integerFirstIndex,
                integerLastIndex
            );
            ...
            uint integerBitsOfPrecision = CountSignificantBits(
                integerValue,
                out integerValueAsBytes
            );
            ...
            uint fractionalDenominatorExponent =
                data.Exponent < 0
            ...
            BigInteger fractionalNumerator = AccumulateDecimalDigitsIntoBigInteger(
                data,
            ...
            uint fractionalExponent =
                fractionalNumerator < fractionalDenominator ? fractionalShift + 1 : fractionalShift;

            ShiftLeft(ref fractionalNumerator, remainingBitsOfPrecisionRequired);
            BigInteger fractionalRemainder;
            BigInteger bigFractionalMantissa = BigInteger.DivRem(
                fractionalNumerator,
                fractionalDenominator,
                out fractionalRemainder
            );
            ...
            return type.AssembleFloatingPointValue(
                completeMantissa,
                finalExponent,
                hasZeroTail,
                out result
        ...
        private static BigInteger AccumulateDecimalDigitsIntoBigInteger(
            DecimalFloatingPointString data,
