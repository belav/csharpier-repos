    ...
    internal static class UICultureUtilities
    {
        ...
        )
        {
            ...
            try
            {
                ...
                var currentThreadGetter =
                    typeInfo.GetDeclaredProperty(currentThreadName)?.GetMethod;
                if (
                ...
                var currentUICultureSetter =
                    typeInfo.GetDeclaredProperty(currentUICultureName)?.SetMethod;
                if (
                    (object?)currentUICultureSetter == null
                    || currentUICultureSetter.IsStatic
                    || currentUICultureSetter.ContainsGenericParameters
                ...
                setter = culture =>
                {
                    currentUICultureSetter.Invoke(
                        currentThreadGetter.Invoke(null, null),
                        new[] { culture }
                    );
                };
                return true;
