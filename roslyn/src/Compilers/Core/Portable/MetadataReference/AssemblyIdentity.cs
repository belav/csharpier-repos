...
namespace Microsoft.CodeAnalysis
{
    ...
    public sealed partial class AssemblyIdentity : IEquatable<AssemblyIdentity>
    {
        ...
        )
        {
            if (!IsValid(contentType))
            {
                throw new ArgumentOutOfRangeException(
                    nameof(contentType),
                    CodeAnalysisResources.InvalidContentType
                );
            }
            ...
            if (!IsValidName(name))
            {
                throw new ArgumentException(
                    string.Format(CodeAnalysisResources.InvalidAssemblyName, name),
                    nameof(name)
                );
            }
            ...
            {
                throw new ArgumentException(
                    CodeAnalysisResources.WinRTIdentityCantBeRetargetable,
                    nameof(isRetargetable)
                );
            }
        ...
        {
            return value >= AssemblyContentType.Default
                && value <= AssemblyContentType.WindowsRuntime;
        }
        ...
        {
            const AssemblyIdentityParts nvc =
        ...
        public bool Equals(AssemblyIdentity? obj)
        {
            return !ReferenceEquals(obj, null)
                && (
                    _lazyHashCode == 0
                    || obj._lazyHashCode == 0
                    || _lazyHashCode == obj._lazyHashCode
                )
        ...
        {
            return x.IsRetargetable == y.IsRetargetable
        ...
        internal static AssemblyIdentity FromAssemblyDefinition(AssemblyName name)
        {
            ...
            ImmutableArray<byte> publicKey =
                (publicKeyBytes != null)
                    ? ImmutableArray.Create(publicKeyBytes)
