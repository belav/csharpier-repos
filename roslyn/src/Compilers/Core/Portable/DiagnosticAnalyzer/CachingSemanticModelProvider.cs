...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    ...
    /// Clients using this provider are responsible for maintaining the lifetime of the entries in this
    // cache,
    /// and should invoke <see cref="ClearCache(SyntaxTree, Compilation)"/> and <see
    // cref="ClearCache(Compilation)"/> to clear entries when appropriate.
    /// For example, <see cref="CompilationWithAnalyzers"/> uses this provider to ensure that semantic
    // model instances
    /// are shared between the compiler and analyzers for improved analyzer execution performance. The
    // underlying
    /// <see cref="AnalyzerDriver"/> executing analyzers clears per-tree entries in the cache whenever a
    // <see cref="CompilationUnitCompletedEvent"/>
    /// has been processed, indicating all relevant analyzers have executed on the corresponding syntax
    // tree for the event.
    /// Similarly, it clears the entire compilation wide cache whenever a <see
    // cref="CompilationCompletedEvent"/> has been processed,
    ...
    internal sealed class CachingSemanticModelProvider : SemanticModelProvider
    {
        ...
        private sealed class PerCompilationProvider
        {
            ...
            // We only care about caching semantic models for internal callers, which use the default
