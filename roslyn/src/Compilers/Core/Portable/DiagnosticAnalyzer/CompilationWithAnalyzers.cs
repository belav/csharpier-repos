    ...
    public class CompilationWithAnalyzers
    {
        ...
        )
        {
            ...
            compilation = compilation.WithOptions(
                compilation.Options
                    .WithReportSuppressedDiagnostics(analysisOptions.ReportSuppressedDiagnostics)
            )
        ...
        )
        {
            ...
            try
            {
                ...
                // Force compilation diagnostics and wait for analyzer execution to complete.
                var compDiags = compilation.GetDiagnostics(cancellationToken);
                var analyzerDiags = await driver.GetDiagnosticsAsync(compilation)
                    .ConfigureAwait(false);
            ...
            if (_compilation.EventQueue?.Count > 0)
            {
                ...
                try
                {
                    ...
                    > getCompilationEvents = (eventQueue, additionalFiles) =>
                        dequeueGeneratedCompilationEvents(eventQueue, additionalFiles);
                    var additionalFiles =
                        _analysisOptions.Options?.AdditionalFiles
                        ?? ImmutableArray<AdditionalText>.Empty;
                    await _analysisState.OnCompilationEventsGeneratedAsync(
            ...
            try
            {
                ...
                if (eventQueue.Count > 0 || _analysisState.HasPendingSyntaxAnalysis(analysisScope))
                {
                    try
                    {
                        ...
                        await driver.AttachQueueAndProcessAllEventsAsync(
                            eventQueue,
                            analysisScope,
                            _analysisState,
                            cancellationToken: cancellationToken
        ...
        )
        {
            ...
            try
            {
                driver = await GetAnalyzerDriverAsync(cancellationToken).ConfigureAwait(false);
                cancellationToken.ThrowIfCancellationRequested();
                return await _analysisState.GetOrComputeAnalyzerActionCountsAsync(
                    analyzer,
                    driver,
                    cancellationToken
                )
