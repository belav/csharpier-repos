    ...
    internal partial class AnalyzerExecutor
    {
        ...
        )
        {
            ...
            using var _ = PooledDelegates.GetPooledFunction(
                (d, ct, arg) => arg.self.IsSupportedDiagnostic(arg.analyzer, d, ct),
                (self: this, analyzer),
        ...
        {
            if (
                !_analyzerManager.TryStartExecuteSymbolEndActions(
                    symbolEndActions,
                    analyzer,
                    symbolDeclaredEvent
                )
            ...
            where TLanguageKindEnum : struct
        {
            ...
            var tuples = nodeActionsByKind.Select(kvp =>
                KeyValuePairUtil.Create(kvp.Key, kvp.Value.ToImmutableAndFree())
            );
            ...
            where TLanguageKindEnum : struct
        {
            ...
            var diagReporter = GetAddSemanticDiagnostic(
                model.SyntaxTree,
                spanForContainingTopmostNodeForAnalysis,
                analyzer,
                cancellationToken
            );
            ...
            ExecuteSyntaxNodeActions(
                ...
                filterSpan,
                isGeneratedCode,
                hasCodeBlockStartOrSymbolStartActions,
                cancellationToken
            );
            diagReporter.Free();
        }
            ...
            where TLanguageKindEnum : struct
        {
            ...
            foreach (var node in nodesToAnalyze)
            {
                ...
                {
                    Debug.Assert(
                        !actionsForKind.IsEmpty,
                        $"Unexpected empty action collection in {nameof(nodeActionsByKind)}"
                    );
        ...
        > GetOperationActionsByKind(IEnumerable<OperationAnalyzerAction> operationActions)
        {
            ...
            var tuples = operationActionsByKind.Select(kvp =>
                KeyValuePairUtil.Create(kvp.Key, kvp.Value.ToImmutableAndFree())
            );
            var map = ImmutableSegmentedDictionary.CreateRange(tuples);
        ...
        )
        {
            ...
            var diagReporter = GetAddSemanticDiagnostic(
                model.SyntaxTree,
                spanForContainingOperationBlock,
                analyzer,
                cancellationToken
            );
            ...
            ExecuteOperationActions(
                ...
                filterSpan,
                isGeneratedCode,
                hasOperationBlockStartOrSymbolStartActions,
                cancellationToken
            );
            diagReporter.Free();
        }
        ...
        )
        {
            ...
            foreach (var operation in operationsToAnalyze)
            {
                ...
                if (operationActionsByKind.TryGetValue(operation.Kind, out var actionsForKind))
                {
                    Debug.Assert(
                        !actionsForKind.IsEmpty,
                        $"Unexpected empty action collection in {nameof(operationActionsByKind)}"
                    );
        ...
        )
        {
            ...
            if (gate != null)
            {
                lock (gate)
                {
                    ExecuteAndCatchIfThrows_NoLock(
                        analyzer,
                        analyze,
                        argument,
                        contextInfo,
        ...
        )
        {
            try
            {
                ...
                when (HandleAnalyzerException(analyzer, ex, info)
        ...
        private bool HandleAnalyzerException(
            DiagnosticAnalyzer analyzer,
            Exception ex,
            in AnalysisContextInfo? info
        )
        {
            if (!this.Compilation.CatchAnalyzerExceptions)
            {
                ...
                Environment.FailFast(
                    CreateAnalyzerExceptionDiagnostic(analyzer, ex, info).ToString()
                );
                return false;
            }
        ...
        )
        {
            ...
            catch (Exception ex)
            {
                return string.Format(
                    CodeAnalysisResources.CompilerAnalyzerThrows,
                    analyzerName,
                    ex.GetType().ToString(),
                    ex.Message,
        ...
        private bool IsSupportedDiagnostic(
            DiagnosticAnalyzer analyzer,
        ...
        private bool IsAnalyzerSuppressedForSymbol(
            DiagnosticAnalyzer analyzer,
            ISymbol symbol,
            CancellationToken cancellationToken
        )
