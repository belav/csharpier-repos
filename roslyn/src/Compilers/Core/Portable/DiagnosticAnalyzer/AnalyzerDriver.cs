...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    ...
    internal abstract partial class AnalyzerDriver : IDisposable
    {
        ...
        )
        {
            ...
            foreach (var tree in analysisScope.SyntaxTrees)
            {
                ...
                try
                {
                    ...
                    analysisState?.TryMarkSyntaxAnalysisCompleteForUnprocessedAnalyzers(
                        file,
                        analysisScope,
                        processedAnalyzers!
                    );
        ...
        )
        {
            ...
            {
                await analysisState
                    .OnCompilationEventProcessedAsync(
                        compilationEvent,
                        processedAnalyzers,
            ...
            )
            {
                if (
                    ...
                    && AnalyzerExecutor.TryExecuteSymbolEndActionsForContainer(
                        ...
                        analysisState,
                        IsGeneratedCodeSymbol(containerSymbol),
        ...
        )
        {
            ...
            try
            {
                ...
                if (
                    analysisState != null
                    && !analysisState.TryMarkSymbolCompleteForUnprocessedAnalyzers(
                        symbol,
                        analysisScope,
                        processedAnalyzers!
                    )
        ...
        )
        {
            ...
            if (!analysisScope.ShouldAnalyze(symbol) || symbolEndActions.IsEmpty)
            {
                ...
                if (
                    analysisState != null
                    && !analysisState.TryMarkSymbolEndAnalysisComplete(
                        symbol,
                        analysisScope.Analyzers
                    )
                )
            ...
            try
            {
                foreach (var groupedActions in symbolEndActions.GroupBy(a => a.Analyzer))
                {
                    ...
                    if (
                        !AnalyzerExecutor.TryMarkSymbolEndAnalysisComplete(
                            symbol,
                            analyzer,
                            analysisState
                        )
        ...
        )
        {
            ...
            try
            {
                ...
                if (
                    analysisState != null
                    && !analysisState.TryMarkEventCompleteForUnprocessedAnalyzers(
                        completedEvent,
                        analysisScope,
                        processedAnalyzers!
                    )
        ...
        )
        {
            ...
            foreach (var analyzer in analyzers)
            {
                ...
                var isConcurrent = await analyzerManager
                    .IsConcurrentAnalyzerAsync(analyzer, analyzerExecutor)
                    .ConfigureAwait(false);
                if (!isConcurrent)
                {
        ...
        )
        {
            ...
            foreach (var analyzer in analyzers)
            {
                ...
                var generatedCodeAnalysisFlags = await analyzerManager
                    .GetGeneratedCodeAnalysisFlagsAsync(analyzer, analyzerExecutor)
                    .ConfigureAwait(false);
        ...
        )
        {
            ...
            var analyzerActions = await AnalyzerManager
                .GetAnalyzerActionsAsync(analyzer, executor)
                .ConfigureAwait(false);
            if (analyzerActions.IsEmpty)
            {
                return AnalyzerActionCounts.Empty;
            ...
            )
            {
                ...
                for (var i = 0; i < declaringReferences.Length; i++)
                {
                    ...
                    if (isInGeneratedCode && DoNotAnalyzeGeneratedCode)
                    {
                        if (
                            analysisState != null
                            && !analysisState.TryMarkDeclarationComplete(
                                symbol,
                                i,
                                analysisScope.Analyzers
                            )
