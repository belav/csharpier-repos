    ...
    internal partial class AnalyzerManager
    {
        private sealed class AnalyzerExecutionContext
        {
                        ...
                        _lazyCompilationScopeTask = Task.Run(
                            () =>
                            {
                                ...
                                analyzerExecutor.ExecuteCompilationStartActions(
                                    sessionScope.GetAnalyzerActions(_analyzer).CompilationStartActions,
                ...
                HashSet<ISymbol>? getDependentSymbols()
                {
                    ...
                    void processMembers(IEnumerable<ISymbol> members)
                    {
                        foreach (var member in members)
                        {
                            ...
                            if (member.Kind != symbol.Kind && member is INamedTypeSymbol typeMember)
                            {
            ...
            )
            {
                ...
                {
                    var handler = new EventHandler<Exception>(
                        (sender, ex) =>
                        {
