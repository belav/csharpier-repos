...
internal static partial class EncodingExtensions
{
    ...
    private static readonly Encoding s_encodingUtf8 = new UTF8Encoding(
    ...
    private static readonly Encoding s_encodingUtf32_BE_BOM = new UTF32Encoding(
        bigEndian: true,
        byteOrderMark: true
    );
    private static readonly Encoding s_encodingUtf32_LE = new UTF32Encoding(
        bigEndian: false,
        byteOrderMark: false
    );
    private static readonly Encoding s_encodingUnicode_BE = new UnicodeEncoding(
        bigEndian: true,
        byteOrderMark: false
    );
    private static readonly Encoding s_encodingUnicode_LE = new UnicodeEncoding(
        bigEndian: false,
        byteOrderMark: false
    );
    ...
    public static bool TryGetEncodingKind(this Encoding encoding, out TextEncodingKind kind)
    {
        switch (encoding.CodePage)
        {
            case 1200:
                ...
                kind =
                    (encoding.Equals(Encoding.Unicode) || HasPreamble(encoding))
                        ? TextEncodingKind.EncodingUnicode_LE_BOM
                        : TextEncodingKind.EncodingUnicode_LE;
                return true;

            case 1201:
                Debug.Assert(HasPreamble(Encoding.BigEndianUnicode));
                kind =
                    (encoding.Equals(Encoding.BigEndianUnicode) || HasPreamble(encoding))
                        ? TextEncodingKind.EncodingUnicode_BE_BOM
                        : TextEncodingKind.EncodingUnicode_BE;
                return true;

            case 12000:
                Debug.Assert(HasPreamble(Encoding.UTF32));
                kind =
                    (encoding.Equals(Encoding.UTF32) || HasPreamble(encoding))
                        ? TextEncodingKind.EncodingUtf32_LE_BOM
                        : TextEncodingKind.EncodingUtf32_LE;
                return true;
            ...
            case 65001:
                ...
                kind =
                    (encoding.Equals(Encoding.UTF8) || HasPreamble(encoding))
                        ? TextEncodingKind.EncodingUtf8_BOM
                        : TextEncodingKind.EncodingUtf8;
                return true;
