...
namespace Microsoft.CodeAnalysis
{
    ...
    {
        internal static DeclarationInfo GetDeclarationInfo(
            SemanticModel model,
            SyntaxNode node,
            bool getSymbol,
        ...
        )
        {
            return GetDeclarationInfo(
                ...
                node,
                getSymbol,
                SpecializedCollections.SingletonEnumerable(executableCodeBlock),
                cancellationToken
            );
        }
        ...
        )
        {
            return GetDeclarationInfo(
                ...
                node,
                getSymbol,
                executableCodeBlocks.AsEnumerable(),
                cancellationToken
            );
        }
        ...
        )
        {
            ...
            // However, we are only interested in the namespace symbol scoped to the compilation's source
            // assembly.
            if (
            ...
            )
            {
                ...
                var assemblyScopedNamespaceSymbol =
                    namespaceSymbol.ConstituentNamespaces.FirstOrDefault(ns =>
                        ns.ContainingAssembly == assemblyToScope
                    );
