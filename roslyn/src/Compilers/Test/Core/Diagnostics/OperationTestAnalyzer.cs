...
namespace Microsoft.CodeAnalysis.UnitTests.Diagnostics
{
                ...
                (operationBlockContext) =>
                {
                    if (operationBlockContext.Compilation.Language != "Stumble")
                    {
                        operationBlockContext.RegisterOperationAction(
                            ...
                            {
                                if (
                                    operationContext.ContainingSymbol.Name.StartsWith("Funky")
                                    && operationContext.Compilation.Language != "Mumble"
                                )
                                {
                                    operationContext.ReportDiagnostic(
                                        Diagnostic.Create(
    ...
    public class BigForTestAnalyzer : DiagnosticAnalyzer
    {
        ...
        private void AnalyzeOperation(OperationAnalysisContext operationContext)
        {
            ...
            if (loop.LoopKind == LoopKind.For)
            {
                ...
                if (forCondition.Kind == OperationKind.Binary)
                {
                    ...
                    if (
                        conditionRight.ConstantValue.HasValue
                    ...
                    )
                    {
                        ...
                        if (forLoop.Before.Length == 1)
                        {
                            ...
                            if (
                                setup.Kind == OperationKind.ExpressionStatement
                                && ((IExpressionStatementOperation)setup).Operation.Kind
                                    == OperationKind.SimpleAssignment
                            )
                            {
                                ISimpleAssignmentOperation setupAssignment =
                                    (ISimpleAssignmentOperation)
                                        ((IExpressionStatementOperation)setup).Operation;
                                if (
                                    setupAssignment.Target.Kind == OperationKind.LocalReference
                                    && ((ILocalReferenceOperation)setupAssignment.Target).Local
                                        == testVariable
                                    && setupAssignment.Value.ConstantValue.HasValue
                                    && setupAssignment.Value.Type.SpecialType
                                        == SpecialType.System_Int32
                                )
                                {
                                    // Setup is known to be an assignment of a constant to the local used in the test.

                                    int initialValue = (int)
                                        setupAssignment.Value.ConstantValue.Value;
                                    ...
                                    if (forLoop.AtLoopBottom.Length == 1)
                                    {
                                        ...
                                        if (advance.Kind == OperationKind.ExpressionStatement)
                                        {
                                            ...
                                            GetOperationKindAndValue(
                                                testVariable,
                                                advanceExpression,
                                                out advanceOperationCode,
                                                out advanceIncrementOpt
                                            );

                                            if (
                                                advanceIncrementOpt.HasValue
                                                && advanceOperationCode.HasValue
                                            )
                                            {
                                                var incrementValue = (int)advanceIncrementOpt.Value;
                                                if (
                                                    advanceOperationCode.Value
                                                ...
                                                {
                                                    int iterationCount =
                                                        (testValue - initialValue) / incrementValue;
        ...
        private void GetOperationKindAndValue(
            ILocalSymbol testVariable,
            IOperation advanceExpression,
            out BinaryOperatorKind? advanceOperationCode,
            out Optional<object> advanceIncrementOpt
        )
        {
            ...
            if (advanceExpression.Kind == OperationKind.SimpleAssignment)
            {
                ...
                )
                {
                    ...
                    if (
                        advanceOperation.OperatorMethod == null
                        && advanceOperation.LeftOperand.Kind == OperationKind.LocalReference
                        && ((ILocalReferenceOperation)advanceOperation.LeftOperand).Local
                            == testVariable
                        && advanceOperation.RightOperand.ConstantValue.HasValue
                        && advanceOperation.RightOperand.Type.SpecialType
                            == SpecialType.System_Int32
                    )
                    {
    ...
    public class SwitchTestAnalyzer : DiagnosticAnalyzer
    {
        ...
        public sealed override void Initialize(AnalysisContext context)
        {
            context.RegisterOperationAction(
                (operationContext) =>
                {
                        ...
                        foreach (ICaseClauseOperation clause in switchCase.Clauses)
                        {
                            switch (clause.CaseKind)
                            {
                                case CaseKind.SingleValue:
                                {
                                    ...
                                    if (
                                        singleValueExpression != null
                                        && singleValueExpression.ConstantValue.HasValue
                                        && singleValueExpression.Type.SpecialType
                                            == SpecialType.System_Int32
                                    )
                                    {
                                        int singleValue = (int)
                                            singleValueExpression.ConstantValue.Value;
                                ...
                                {
                                    hasNonDefault = true;
                                    IRangeCaseClauseOperation rangeClause =
                                        (IRangeCaseClauseOperation)clause;
                                    IOperation rangeMinExpression = rangeClause.MinimumValue;
                                    IOperation rangeMaxExpression = rangeClause.MaximumValue;
                                    if (
                                        rangeMinExpression != null
                                        && rangeMinExpression.ConstantValue.HasValue
                                        && rangeMinExpression.Type.SpecialType
                                            == SpecialType.System_Int32
                                        && rangeMaxExpression != null
                                        && rangeMaxExpression.ConstantValue.HasValue
                                        && rangeMaxExpression.Type.SpecialType
                                            == SpecialType.System_Int32
                                    )
                                    {
                                        int rangeMinValue = (int)
                                            rangeMinExpression.ConstantValue.Value;
                                        int rangeMaxValue = (int)
                                            rangeMaxExpression.ConstantValue.Value;
                                        caseValueCount += IncludeClause(
                                            rangeMinValue,
                                            rangeMaxValue,
                                            ref minCaseValue,
                                ...
                                {
                                    hasNonDefault = true;
                                    IRelationalCaseClauseOperation relationalClause =
                                        (IRelationalCaseClauseOperation)clause;
                                    IOperation relationalValueExpression = relationalClause.Value;
                                    if (
                                        relationalValueExpression != null
                                        && relationalValueExpression.ConstantValue.HasValue
                                        && relationalValueExpression.Type.SpecialType
                                            == SpecialType.System_Int32
                                    )
                                    {
                                        int rangeMinValue = int.MaxValue;
                                        int rangeMaxValue = int.MinValue;
                                        int relationalValue = (int)
                                            relationalValueExpression.ConstantValue.Value;
                    ...
                    if (
                        caseValueCount == 0 && !hasDefault
                        || caseValueCount != 0 && span / caseValueCount > 100
                    )
                    {
                        Report(
                            operationContext,
                            switchOperation.Value.Syntax,
                            SparseSwitchDescriptor
                        );
                    }
                    if (!hasDefault)
        ...
        private static int IncludeClause(
            int clauseMinValue,
            int clauseMaxValue,
            ref long minCaseValue,
            ref long maxCaseValue
        )
        {
            ...
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    ...
                    foreach (IArgumentOperation argument in invocation.Arguments)
                    {
                        ...
                        TestAscendingArgument(
                            operationContext,
                            argument.Value,
                            ref priorArgumentValue
                        );
        ...
        public sealed override void Initialize(AnalysisContext context)
        {
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    ...
                    if (
                        literal.Type.SpecialType == SpecialType.System_Int32
    ...
    public class VariableDeclarationTestAnalyzer : DiagnosticAnalyzer
    {
        ...
        public sealed override void Initialize(AnalysisContext context)
        {
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    var declarationStatement = (IVariableDeclarationGroupOperation)
                        operationContext.Operation;
                    if (declarationStatement.GetDeclaredVariables().Count() > 3)
                    {
                        Report(
                            operationContext,
                            declarationStatement.Syntax,
                            TooManyLocalVarDeclarationsDescriptor
                        );
                    }
                    ...
                    )
                    {
                        ...
                        )
                        {
                            Report(
                                operationContext,
                                decl.Symbol.DeclaringSyntaxReferences.Single().GetSyntax(),
                                LocalVarInitializedDeclarationDescriptor
                            );
                        }
                    }
                ...
                (operationContext) =>
                {
                    switch (operationContext.Operation.Kind)
                    {
                        ...
                        case OperationKind.SwitchCase:
                            ...
                            if (
                                !switchSection.HasErrors(
                                    operationContext.Compilation,
                                    operationContext.CancellationToken
                                )
                                && switchSection.Clauses.Length > 1
                            )
                            {
                                Report(
                                    operationContext,
                                    switchSection.Syntax,
                                    MultipleCaseClausesDescriptor
                                );
                            }
                            break;
    ...
    public class ExplicitVsImplicitInstanceAnalyzer : DiagnosticAnalyzer
    {
        ...
        public sealed override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics =>
            ImmutableArray.Create(ImplicitInstanceDescriptor, ExplicitInstanceDescriptor);
        ...
        public sealed override void Initialize(AnalysisContext context)
        {
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    IInstanceReferenceOperation instanceReference = (IInstanceReferenceOperation)
                        operationContext.Operation;
                    operationContext.ReportDiagnostic(
                        Diagnostic.Create(
    ...
    public class MemberReferenceAnalyzer : DiagnosticAnalyzer
    {
        ...
        public sealed override void Initialize(AnalysisContext context)
        {
            ...
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    IEventAssignmentOperation eventAssignment = (IEventAssignmentOperation)
                        operationContext.Operation;
                    operationContext.ReportDiagnostic(
                        Diagnostic.Create(
                    ...
                    if (
                        eventAssignment.EventReference.Kind == OperationKind.Invalid
                        || eventAssignment.HasErrors(
                            operationContext.Compilation,
                            operationContext.CancellationToken
                        )
                    )
    ...
    public class ParamsArrayTestAnalyzer : DiagnosticAnalyzer
    {
        ...
        public sealed override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics =>
            ImmutableArray.Create(LongParamsDescriptor, InvalidConstructorDescriptor);
        ...
        public sealed override void Initialize(AnalysisContext context)
        {
            ...
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    IObjectCreationOperation creation = (IObjectCreationOperation)
                        operationContext.Operation;
    ...
    public class EqualsValueTestAnalyzer : DiagnosticAnalyzer
    {
        ...
        public sealed override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics =>
            ImmutableArray.Create(EqualsValueDescriptor);
        ...
        public sealed override void Initialize(AnalysisContext context)
        {
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    IFieldInitializerOperation equalsValue = (IFieldInitializerOperation)
                        operationContext.Operation;
                    if (equalsValue.InitializedFields[0].Name.StartsWith("F"))
                    {
                        operationContext.ReportDiagnostic(
                            Diagnostic.Create(
                                EqualsValueDescriptor,
                                equalsValue.Syntax.GetLocation()
                            )
                        );
                    }
                },
            ...
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    IParameterInitializerOperation equalsValue = (IParameterInitializerOperation)
                        operationContext.Operation;
            ...
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    ...
                    operationContext.ReportDiagnostic(
                        Diagnostic.Create(
                            AddressOfDescriptor,
                            addressOfOperation.Syntax.GetLocation()
                        )
                    );

                    if (
                        addressOfOperation.Reference.Kind == OperationKind.Invalid
                        && addressOfOperation.HasErrors(
                            operationContext.Compilation,
                            operationContext.CancellationToken
                        )
                    )
                    {
                        operationContext.ReportDiagnostic(
                            Diagnostic.Create(
                                InvalidAddressOfReferenceDescriptor,
                                addressOfOperation.Reference.Syntax.GetLocation()
                            )
                        );
                    }
                },
    ...
    public class LabelOperationsTestAnalyzer : DiagnosticAnalyzer
    {
        ...
        public sealed override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics =>
            ImmutableArray.Create(LabelDescriptor, GotoDescriptor);
    ...
    public class UnaryAndBinaryOperationsTestAnalyzer : DiagnosticAnalyzer
    {
        ...
        public sealed override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics =>
            ImmutableArray.Create(
                OperatorAddMethodDescriptor,
                OperatorMinusMethodDescriptor,
                DoubleMultiplyDescriptor,
                BooleanNotDescriptor
            );
            ...
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    ...
                    if (
                        ...
                        && binary.OperatorMethod.Name.Contains("Addition")
                    )
                    {
                        operationContext.ReportDiagnostic(
                            Diagnostic.Create(
                    ...
                    if (
                        ...
                        && binary.Type.SpecialType == SpecialType.System_Double
                    )
                    {
                        operationContext.ReportDiagnostic(
            ...
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    ...
                    if (
                        ...
                        && unary.OperatorMethod.Name.Contains("UnaryNegation")
                    )
                    {
                        operationContext.ReportDiagnostic(
                            Diagnostic.Create(
            ...
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    ...
                    if (binary.OperatorMethod != null)
                    {
                        operationContext.ReportDiagnostic(
                            Diagnostic.Create(
                                BinaryUserDefinedOperatorDescriptor,
                                binary.Syntax.GetLocation(),
    ...
    public class NullOperationSyntaxTestAnalyzer : DiagnosticAnalyzer
    {
        ...
        public sealed override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics
        {
            ...
            {
                return ImmutableArray.Create(
                    NullOperationSyntaxDescriptor,
                    ParamsArrayOperationDescriptor
                );
            }
        }
    ...
    public class ConditionalAccessOperationTestAnalyzer : DiagnosticAnalyzer
    {
        ...
        public sealed override void Initialize(AnalysisContext context)
        {
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    IConditionalAccessOperation conditionalAccess = (IConditionalAccessOperation)
                        operationContext.Operation;
                    if (
                        conditionalAccess.WhenNotNull != null && conditionalAccess.Operation != null
                    )
                    {
                        operationContext.ReportDiagnostic(
                            Diagnostic.Create(
            ...
            context.RegisterOperationAction(
                (operationContext) =>
                {
                    IConditionalAccessInstanceOperation conditionalAccessInstance =
                        (IConditionalAccessInstanceOperation)operationContext.Operation;
                    operationContext.ReportDiagnostic(
                        Diagnostic.Create(
                            ConditionalAccessInstanceOperationDescriptor,
                            conditionalAccessInstance.Syntax.GetLocation()
                        )
                    );
                },
                        ...
                        )
                        {
                            ...
                            operationContext.ReportDiagnostic(
                                Diagnostic.Create(
                                    ...
                                    forLoop.LimitValue.Syntax.GetLocation()
                                )
                            );
                        }
                    }
                        ...
                        (operationContext) =>
                        {
                            operationContext.ReportDiagnostic(
                                Diagnostic.Create(
                                    ...
                                    operationContext.Operation.Syntax.GetLocation(),
                                    "Operation",
                                    "CompilationStart within Analysis"
                                )
                            );
                        },
                        OperationKind.Literal
