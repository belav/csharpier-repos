        ...
        : TestDiagnosticAnalyzer<TLanguageKindEnum> where TLanguageKindEnum : struct
    {
        ...
        public class Entry
        {
            ...
            )
            {
                ...
                SyntaxKind =
                    node == null
                        ? default(TLanguageKindEnum)
                        : (TLanguageKindEnum)(object)(ushort)node.RawKind;
                SymbolKind = symbol == null ? (SymbolKind?)null : symbol.Kind;
                MethodKind =
                    symbol is IMethodSymbol
                        ? ((IMethodSymbol)symbol).MethodKind
                        : (MethodKind?)null;
            ...
            public override string ToString()
            {
                return CallerName
                    + "("
                    + string.Join(", ", SymbolKind, MethodKind, SyntaxKind)
                    + ")";
            }
        }
        ...
        private static readonly Regex s_omittedSyntaxKindRegex = new Regex(
            @"None|Trivia|Token|Keyword|List|Xml|Cref|Compilation|Namespace|Class|Struct|Enum|Interface|Delegate|Field|Property|Indexer|Event|Operator|Constructor|Access|Incomplete|Attribute|Filter|InterpolatedString"
        );
        ...
        public void VerifyAnalyzeSymbolCalledForAllSymbolKinds()
        {
            ...
            var actualSymbolKinds = _callLog
                .Where(a => FilterByAbstractName(a, "Symbol"))
                .Where(e => e.SymbolKind.HasValue)
                .Select(e => e.SymbolKind.Value)
                .Distinct();
        ...
        )
        {
            ...
            Assert.True(
                ...
                "AllInOne test contains ignored SyntaxKinds: "
                    + string.Join(", ", expectedMissingSyntaxKinds)
            );
            actualSyntaxKinds.UnionWith(savedSyntaxKindsPatterns);
            AssertIsSuperset(expectedSyntaxKinds, actualSyntaxKinds);
        }

        protected virtual bool IsOnCodeBlockSupported(
        ...
        )
        {
            ...
            {
                expectedArguments = expectedArguments.Where(
                    a => !symbolKindsWithNoCodeBlocks.Contains(a.SymbolKind)
                );
            }

            expectedArguments = expectedArguments.Where(
                a => IsOnCodeBlockSupported(a.SymbolKind, a.MethodKind, a.ReturnsVoid)
            );

            var actualOnCodeBlockStartedArguments = _callLog
                .Where(a => FilterByAbstractName(a, "CodeBlockStart"))
                .Select(
                    e =>
                        new
                        {
            ...
            var actualOnCodeBlockEndedArguments = _callLog
                .Where(a => FilterByAbstractName(a, "CodeBlock"))
                .Select(
                    e =>
                        new
                        {
        ...
        {
            var missingElements = expectedSubset
                .GroupJoin(
                    actualSuperset,
                    e => e,
                    a => a,
