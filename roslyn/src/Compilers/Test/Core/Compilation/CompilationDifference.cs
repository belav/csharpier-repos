...
namespace Microsoft.CodeAnalysis.Test.Utilities
{
    public sealed class CompilationDifference
    {
        ...
        {
            get { return EmitResult.Baseline; }
        }
        ...
        )
        {
            ...
            if (!methodToken.IsNil)
            {
                ...
                sequencePointMarkers = ILValidation.GetSequencePointMarkers(
                    XElement.Parse(actualPdb)
                );
        ...
        internal string GetMethodIL(string qualifiedMethodName)
        {
            return ILBuilderVisualizer.ILBuilderToString(
                this.TestData.GetMethodData(qualifiedMethodName).ILBuilder,
                ToLocalInfo
            );
        }
        ...
        private static ILVisualizer.LocalInfo ToLocalInfo(Cci.ILocalDefinition local)
        {
            ...
            if (signature == null)
            {
                return new ILVisualizer.LocalInfo(
                    local.Name,
                    local.Type,
                    local.IsPinned,
                    local.IsReference
            ...
            else
            {
                ...
                var typeName =
                    (signature.Length == 1) ? GetTypeName((SignatureTypeCode)signature[0]) : null;
        ...
        public void VerifyUpdatedMethods(params string[] expectedMethodTokens)
        {
            AssertEx.Equal(
                ...
                EmitResult.UpdatedMethods.Select(
                    methodHandle => $"0x{MetadataTokens.GetToken(methodHandle):X8}"
                )
            );
        }
    }
