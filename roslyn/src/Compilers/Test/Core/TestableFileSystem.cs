...
{
    public delegate Stream OpenFileExFunc(
        string filePath,
        FileMode mode,
        FileAccess access,
        FileShare share,
    ...
    public sealed class TestableFileSystem : ICommonCompilerFileSystem
    {
        ...
        public OpenFileFunc OpenFileFunc { get; private set; } =
            delegate
            {
                throw new InvalidOperationException();
            };
        public OpenFileExFunc OpenFileExFunc { get; private set; } =
            (string _, FileMode _, FileAccess _, FileShare _, int _, FileOptions _, out string _) =>
                throw new InvalidOperationException();
        public Func<string, bool> FileExistsFunc { get; private set; } =
            delegate
            {
                throw new InvalidOperationException();
            };
        ...
        public static TestableFileSystem CreateForFiles(
            params (string FilePath, TestableFile TestableFile)[] files
        )
        {
            var map = files.ToDictionary(x => x.FilePath, x => x.TestableFile);
