...
namespace Roslyn.Test.Utilities
{
    internal static class MetadataReaderUtils
    {
        ...
        {
            return reader.AssemblyReferences.Select(
                    handle => reader.GetAssemblyReference(handle).Name
                )
                .ToArray();
        }
        ...
        {
            return reader.TypeDefinitions.Select(handle => reader.GetTypeDefinition(handle).Name)
                .ToArray();
        }
        ...
        {
            return reader.TypeReferences.Select(handle => reader.GetTypeReference(handle).Name)
                .ToArray();
        }

        public static StringHandle[] GetEventDefNames(this MetadataReader reader)
        {
            return reader.EventDefinitions.Select(handle => reader.GetEventDefinition(handle).Name)
                .ToArray();
        }

        public static StringHandle[] GetFieldDefNames(this MetadataReader reader)
        {
            return reader.FieldDefinitions.Select(handle => reader.GetFieldDefinition(handle).Name)
                .ToArray();
        }

        public static StringHandle[] GetMethodDefNames(this MetadataReader reader)
        {
            return reader.MethodDefinitions.Select(
                    handle => reader.GetMethodDefinition(handle).Name
                )
                .ToArray();
        }
        ...
        {
            return reader.MemberReferences.Select(handle => reader.GetMemberReference(handle).Name)
                .ToArray();
        }
        ...
        public static StringHandle[] GetParameterDefNames(this MetadataReader reader)
        {
            return reader.GetParameters()
                .Select(handle => reader.GetParameter(handle).Name)
                .ToArray();
        }

        public static StringHandle[] GetPropertyDefNames(this MetadataReader reader)
        {
            return reader.PropertyDefinitions.Select(
                    handle => reader.GetPropertyDefinition(handle).Name
                )
                .ToArray();
        }
        ...
        private static ImmutableArray<T> ReadArray<T>(
            this MetadataReader reader,
        ...
        )
        {
            ...
            switch (token.Kind)
            {
                ...
                case HandleKind.MethodDefinition:
                    parent = reader.GetMethodDefinition((MethodDefinitionHandle)token)
                        .GetDeclaringType();
                    break;
                default:
                    throw ExceptionUtilities.UnexpectedValue(token.Kind);
        ...
        {
            return (
                from handle in reader.CustomDebugInformation
        ...
        {
            return reader.TypeReferences.Select(t => reader.GetTypeReference(t))
                .Select(
                    t =>
            ...
            switch (handle.Kind)
            {
                ...
                case HandleKind.MethodDefinition:
                {
                    ...
                    var decoder = new SignatureDecoder<string, object>(
                        ConstantSignatureVisualizer.Instance,
                        reader,
                        genericContext: null
                    );
        ...
        private sealed class ConstantSignatureVisualizer : ISignatureTypeProvider<string, object>
        {
            public static readonly ConstantSignatureVisualizer Instance =
                new ConstantSignatureVisualizer();
        ...
        internal static void VerifyPEMetadata(
            string pePath,
            string[] types,
            string[] methods,
            string[] attributes
        )
        {
            ...
            using (var refPeReader = new PEReader(peStream))
            {
                ...
                AssertEx.SetEqual(
                    metadataReader.TypeDefinitions.Select(t => metadataReader.Dump(t)),
                    types
                );
                AssertEx.SetEqual(
                    metadataReader.MethodDefinitions.Select(t => metadataReader.Dump(t)),
                    methods
                );

                AssertEx.SetEqual(
                    metadataReader.CustomAttributes.Select(
