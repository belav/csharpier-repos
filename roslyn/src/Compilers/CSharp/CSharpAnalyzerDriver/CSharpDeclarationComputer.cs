...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal class CSharpDeclarationComputer : DeclarationComputer
    {
        ...
        )
        {
            ...
            switch (node.Kind())
            {
                    ...
                    if (associatedSymbol is IMethodSymbol ctor)
                    {
                        ...
                        Debug.Assert(
                            ctor.MethodKind == MethodKind.Constructor
                                && recordDeclaration.ParameterList is object
                        );
                ...
                case SyntaxKind.EnumMemberDeclaration:
                {
                    ...
                    var attributes = GetAttributes(t.AttributeLists);
                    var codeBlocks = SpecializedCollections.SingletonEnumerable(t.EqualsValue)
                        .Concat(attributes);
                    builder.Add(
                ...
                case SyntaxKind.PropertyDeclaration:
                {
                    ...
                    var attributes = GetAttributes(t.AttributeLists);
                    var codeBlocks = SpecializedCollections.SingletonEnumerable(t.Initializer)
                        .Concat(attributes);
                    builder.Add(
                        GetDeclarationInfo(model, node, getSymbol, codeBlocks, cancellationToken)
                    );
        ...
        private static IEnumerable<SyntaxNode> GetParameterInitializersAndAttributes(
            ...
            SpecializedCollections.SingletonEnumerable(parameter.Default)
