...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal class CSharpDeclarationComputer : DeclarationComputer
    {
        ...
        )
        {
            ...
            switch (node.Kind())
            {
                    ...
                    if (associatedSymbol is IMethodSymbol ctor)
                    {
                        ...
                        Debug.Assert(
                            ctor.MethodKind == MethodKind.Constructor
                                && typeDeclaration.ParameterList is object
                        );

                        var codeBlocks = GetParameterListInitializersAndAttributes(
                            typeDeclaration.ParameterList
                        );

                        if (
                            typeDeclaration.BaseList?.Types.FirstOrDefault()
                ...
                case SyntaxKind.InterfaceDeclaration:
                {
                    ...
                    builder.Add(
                        GetDeclarationInfo(model, node, getSymbol, attributes, cancellationToken)
                    );
                    return;
                }
                ...
                case SyntaxKind.EnumMemberDeclaration:
                {
                    ...
                    var attributes = GetAttributes(t.AttributeLists);
                    var codeBlocks = SpecializedCollections
                        .SingletonEnumerable(t.EqualsValue)
                        .Concat(attributes);
                    builder.Add(
                        GetDeclarationInfo(model, node, getSymbol, codeBlocks, cancellationToken)
                    );
                    return;
                }
                ...
                case SyntaxKind.FieldDeclaration:
                {
                    ...
                    {
                        var codeBlocks = SpecializedCollections
                            .SingletonEnumerable(decl.Initializer)
                            .Concat(attributes);
                        builder.Add(
                            GetDeclarationInfo(
                                model,
                ...
                case SyntaxKind.PropertyDeclaration:
                {
                    ...
                    var attributes = GetAttributes(t.AttributeLists);
                    var codeBlocks = SpecializedCollections
                        .SingletonEnumerable(t.Initializer)
                        .Concat(attributes);
                    builder.Add(
                        GetDeclarationInfo(model, node, getSymbol, codeBlocks, cancellationToken)
                    );
                ...
                case SyntaxKind.InitAccessorDeclaration:
                {
                    ...
                    builder.Add(
                        GetDeclarationInfo(model, node, getSymbol, blocks, cancellationToken)
                    );
                    blocks.Free();
        ...
        private static IEnumerable<SyntaxNode> GetParameterListInitializersAndAttributes(
        ...
        private static IEnumerable<SyntaxNode> GetParameterInitializersAndAttributes(
            ...
            SpecializedCollections
                .SingletonEnumerable(parameter.Default)
                .Concat(GetAttributes(parameter.AttributeLists));

        private static IEnumerable<SyntaxNode> GetTypeParameterListAttributes(
