...
namespace Microsoft.CodeAnalysis.CSharp.Syntax.InternalSyntax
{
    ...
    internal sealed partial class LanguageParser : SyntaxParser
    {
        ...
        internal TNode ParseWithStackGuard<TNode>(
            ...
            Func<LanguageParser, TNode> createEmptyNodeFunc
        )
                    ...
                    switch (this.CurrentToken.Kind)
                    {
                        ...
                        case SyntaxKind.OpenBracketToken:
                            if (this.IsPossibleGlobalAttributeDeclaration())
                            {
                                ...
                                var attribute = this.TryParseAttributeDeclaration(
                                    inExpressionContext: parentKind == SyntaxKind.CompilationUnit
                                );
                                if (attribute != null)
                                {
                                    // incomplete members must be processed before we add any nodes to the body:
        ...
        private UsingDirectiveSyntax ParseUsingDirective()
        {
            ...
            )
            {
                ...
                unsafeToken = AddTrailingSkippedSyntax(
                    unsafeToken,
                    AddError(this.EatToken(), ErrorCode.ERR_BadStaticAfterUnsafe)
                );
            }
            ...
            var isAliasToFunctionPointer =
                alias != null && this.CurrentToken.Kind == SyntaxKind.DelegateKeyword;
            if (!isAliasToFunctionPointer && IsPossibleNamespaceMemberDeclaration())
            {
                ...
                type = WithAdditionalDiagnostics(
        ...
        private AttributeListSyntax? TryParseAttributeDeclaration(bool inExpressionContext)
        {
            ...
            )
            {
                return @this.SkipBadSeparatedListTokensWithExpectedKind(
                    ...
                    list,
                    static p =>
                        p.CurrentToken.Kind != SyntaxKind.CommaToken && !p.IsPossibleAttribute(),
        ...
        private bool ShouldContextualKeywordBeTreatedAsModifier(bool parsingStatementNotDeclaration)
        {
            ...
            if (!parsingStatementNotDeclaration)
            {
                ...
                if (
                    IsTypeModifierOrTypeKeyword(currentTokenKind)
...
#nullable disable
        ...
        private BaseListSyntax ParseBaseList()
        {
            ...
            while (true)
            {
                ...
                else if (this.CurrentToken.Kind == SyntaxKind.CommaToken || this.IsPossibleType())
                {
                    ...
                    skipBadBaseListTokens(ref colon, list, SyntaxKind.CommaToken)
            ...
            PostSkipAction skipBadBaseListTokens(
            ...
            else
            {
                ...
                while (true)
                {
                    ...
                    )
                    {
                        ...
                        skipBadTypeParameterConstraintTokens(bounds, SyntaxKind.CommaToken)
                        == PostSkipAction.Abort
                    )
                    {
                        break;
        ...
        private TypeParameterConstraintSyntax ParseTypeParameterConstraint()
        {
            ...
            {
                SyntaxKind.NewKeyword => _syntaxFactory.ConstructorConstraint(
                    newKeyword: this.EatToken(),
                    this.EatToken(SyntaxKind.OpenParenToken),
                    this.EatToken(SyntaxKind.CloseParenToken)
                ),

                SyntaxKind.StructKeyword => _syntaxFactory.ClassOrStructConstraint(
                ...
                SyntaxKind.ClassKeyword => _syntaxFactory.ClassOrStructConstraint(
                    SyntaxKind.ClassConstraint,
                    classOrStructKeyword: this.EatToken(),
                    this.TryEatToken(SyntaxKind.QuestionToken)
                ),

                SyntaxKind.DefaultKeyword => _syntaxFactory.DefaultConstraint(
                    defaultKeyword: this.EatToken()
                ),

                SyntaxKind.EnumKeyword => _syntaxFactory.TypeConstraint(
                ...
                SyntaxKind.DelegateKeyword => PeekToken(1).Kind == SyntaxKind.AsteriskToken
        ...
        )
        {
            return this.SkipBadListTokensWithErrorCode(
                ...
                list,
                static p =>
                    p.CurrentToken.Kind != SyntaxKind.CloseBraceToken && !p.IsPossibleAccessor(),
        ...
        )
        {
            ...
            )
            {
                return @this.SkipBadSeparatedListTokensWithExpectedKind(
                    ...
                    list,
                    static p =>
                        p.CurrentToken.Kind != SyntaxKind.CommaToken && !p.IsPossibleParameter(),
                    static (p, closeKind) => p.CurrentToken.Kind == closeKind,
                    expectedKind,
                    closeKind
                );
...
#nullable disable
        ...
        )
        {
            ...
            return _syntaxFactory.FieldDeclaration(
                ...
                _syntaxFactory.VariableDeclaration(
                    type,
                    this.ParseFieldDeclarationVariableDeclarators(
                        type,
                        VariableFlags.Fixed,
                        parentKind
                    )
        ...
        {
            var variables = this.ParseFieldDeclarationVariableDeclarators(
                type,
        ...
        )
        {
            ...
            isDefinitelyTypeArgumentList =
                isDefinitelyTypeArgumentList
                || this.CurrentToken.Kind is SyntaxKind.CloseParenToken;
            if (isDefinitelyTypeArgumentList)
            {
                result = ScanTypeFlags.GenericTypeOrMethod;
            }
...
#nullable disable
        ...
        )
        {
            return this.SkipBadSeparatedListTokensWithExpectedKind(
                ...
                list,
                static p =>
                    p.CurrentToken.Kind != SyntaxKind.CommaToken && !p.IsPossibleExpression(),
...
#nullable disable
        ...
        private SyntaxList<AttributeListSyntax> ParseStatementAttributeDeclarations()
        {
            ...
            var attributes = isCollectionExpression
                ? default
        ...
        private BlockSyntax ParsePossiblyAttributedBlock() =>
            ParseBlock(this.ParseAttributeDeclarations(inExpressionContext: false));
        ...
        {
            Debug.Assert(
                this.CurrentToken.Kind is SyntaxKind.SwitchKeyword or SyntaxKind.CaseKeyword
            );

            parseSwitchHeader(
                out var switchKeyword,
                out var openParen,
                out var expression,
                out var closeParen,
                out var openBrace
        ...
        )
        {
            ...
            )
            {
                ...
                using var originalAfterWhenTrue = this.GetDisposableResetPoint(
                    resetOnDispose: false
                );
            ...
            static bool containsTernaryCollectionToReinterpret(ExpressionSyntax expression)
            {
                ...
                while (stack.Count > 0)
                {
                    ...
                    if (
                        current is ConditionalExpressionSyntax conditionalExpression
                        && conditionalExpression.WhenTrue.GetFirstToken().Kind
                            == SyntaxKind.OpenBracketToken
                    )
                    {
                        stack.Free();
        ...
        )
        {
            ...
            return node switch
            {
                PatternSyntax pattern => _syntaxFactory.IsPatternExpression(
                    leftOperand,
                    opToken,
                    pattern
                ),
                TypeSyntax type => _syntaxFactory.BinaryExpression(
        ...
        {
            Debug.Assert(
                this.CurrentToken.Kind is SyntaxKind.DotToken or SyntaxKind.OpenBracketToken
            );
            ExpressionSyntax expr = this.CurrentToken.Kind switch
            {
                SyntaxKind.DotToken => _syntaxFactory.MemberBindingExpression(
                    this.EatToken(),
                    this.ParseSimpleName(NameOptions.InExpression)
                ),
                SyntaxKind.OpenBracketToken => _syntaxFactory.ElementBindingExpression(
                    this.ParseBracketedArgumentList()
                ),
        ...
        private bool ScanCast(bool forPattern = false)
        {
            ...
            switch (type)
            {
                ...
                case ScanTypeFlags.AliasQualifiedName:
                    ...
                    return !forPattern
                        || this.CurrentToken.Kind switch
                        {
                            ...
                            or SyntaxKind.DotDotToken => true,
                ...
                case ScanTypeFlags.TupleType:
                    ...
                    return this.CurrentToken.Kind == SyntaxKind.OpenBracketToken
                        || CanFollowCast(this.CurrentToken.Kind);
        ...
        private CollectionExpressionSyntax ParseCollectionExpression()
        {
            ...
            static PostSkipAction skipBadCollectionElementTokens(
                LanguageParser @this,
                ref SyntaxToken openBracket,
                SeparatedSyntaxListBuilder<CollectionElementSyntax> list,
                SyntaxKind expectedKind,
                SyntaxKind closeKind
            )
            {
...
#nullable disable
        ...
        private InitializerExpressionSyntax ParseObjectOrCollectionInitializer()
        {
            ...
            var kind = isObjectInitializer(initializers)
            ...
            static bool isObjectInitializer(SeparatedSyntaxList<ExpressionSyntax> initializers)
            {
                ...
                {
                    if (
                        initializers[i] is AssignmentExpressionSyntax
                        {
                            Kind: SyntaxKind.SimpleAssignmentExpression,
                            Left.Kind: SyntaxKind.IdentifierName
                                or SyntaxKind.ImplicitElementAccess,
                        }
                    )
                    {
                        return true;
        ...
        private OrderByClauseSyntax ParseOrderByClause()
        {
            ...
            while (this.CurrentToken.Kind == SyntaxKind.CommaToken)
            {
                ...
                else if (this.CurrentToken.Kind == SyntaxKind.CommaToken)
                {
                    ...
                    skipBadOrderingListTokens(list, SyntaxKind.CommaToken) == PostSkipAction.Abort
                )
                {
                    break;
                }
