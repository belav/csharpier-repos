...
namespace Microsoft.CodeAnalysis.CSharp.Syntax.InternalSyntax
{
    internal partial class LanguageParser
    {
        private ExpressionSyntax ParseInterpolatedStringToken()
        {
            ...
            // the holes and builds a tree for the whole thing (resulting in an
            ...
            //
            // (4) On an incremental change, we widen the invalidated region to include any enclosing
            // interpolated
            ...
            // compute the positions of the interpolations in the original string literal, if there was an error
            // or not,
            ...
            rescanInterpolation(
                out var kind,
                out var error,
                out var openQuoteRange,
                interpolations,
                out var closeQuoteRange
            );

            // Only bother trying to do dedentation if we have a multiline literal without errors.  There's no
            // point
            // trying in the presence of errors as we may not even be able to determine what the dedentation
            // should be.
            var needsDedentation =
                kind == Lexer.InterpolatedStringKind.MultiLineRaw && error == null;
            ...
            if (error != null)
            {
                // Errors are positioned relative to the start of the token that was lexed.  Specifically relative
                // to
                // the starting `$` or `@`.  However, when placed on a node like this, it will be relative to the
                // node's
            ...
            void rescanInterpolation(
                out Lexer.InterpolatedStringKind kind,
            ...
            {
                using var tempLexer = new Lexer(
                    SourceText.From(originalText),
                    this.Options,
            ...
            )
            {
                ...
                var indentationWhitespace = needsDedentation
                    ? getIndentationWhitespace(originalTextSpan)
                    : default;
                ...
                for (var i = 0; i < interpolations.Count; i++)
                {
                    ...
                    builder.Add(
                        makeContent(
                            ...
                            isLast: false,
                            originalTextSpan[
                                currentContentStart..interpolation.OpenBraceRange.Start
                            ]
                        )
                    );

                    // Now parse the interpolation itself.
                    var interpolationNode = ParseInterpolation(
                        this.Options,
                        originalText,
                        interpolation,
                        kind
                    ...
                    var indentationError = getInterpolationIndentationError(
                        indentationWhitespace,
                        interpolation
                    );
                    if (indentationError != null)
                        interpolationNode = interpolationNode.WithDiagnosticsGreen(
                ...
                builder.Add(
                    makeContent(
                        indentationWhitespace,
                        content,
                        isFirst: interpolations.Count == 0,
                        isLast: true,
                        originalTextSpan[currentContentStart..closeQuoteRange.Start]
                    )
                );

                CodeAnalysis.Syntax.InternalSyntax.SyntaxList<InterpolatedStringContentSyntax> result =
                    builder;
                _pool.Free(builder);
                content.Free();
                return result;
            ...
            ReadOnlySpan<char> getIndentationWhitespace(ReadOnlySpan<char> originalTextSpan)
            {
                ...
                // A multi-line raw interpolation without errors always ends with a new-line, some number of spaces,
                // and
            ...
            )
            {
                ...
                // If we're not dedenting then just make a standard interpolated text token.  Also, we can
                // short-circuit
                ...
                if (!needsDedentation || indentationWhitespace.IsEmpty)
                    return SyntaxFactory.InterpolatedStringText(
                        MakeInterpolatedStringTextToken(kind, text.ToString())
                    );

                content.Clear();
                var currentIndex = 0;

                // If we're not processing the first content chunk, then we must be processing a chunk that came
                // after
                ...
                if (!isFirst)
                    currentIndex = ConsumeRemainingContentThroughNewLine(
                        content,
                        text,
                        currentIndex
                    );
                ...
                while (currentIndex < text.Length)
                {
                    ...
                    if (indentationError == null)
                    {
                        ...
                        if (!currentLineWhitespace.StartsWith(indentationWhitespace))
                        {
                            ...
                            var isBlankLine =
                                (currentIndex == text.Length && isLast)
                                || (
                                    currentIndex < text.Length
                                    && SyntaxFacts.IsNewLine(text[currentIndex])
                                );
                            var isLegalBlankLine =
                                isBlankLine
                                && indentationWhitespace.StartsWith(currentLineWhitespace);
                            if (!isLegalBlankLine)
                            {
                    ...
                    // Skip the leading whitespace that matches the terminator line and add any text after that to our
                    // content.
                    currentIndex = Math.Min(
                        currentIndex,
                        lineStartPosition + indentationWhitespace.Length
                    );
                    currentIndex = ConsumeRemainingContentThroughNewLine(
                        content,
                        text,
                ...
                // if we ran into any errors, don't give this item any special value.  It just has the value of our
                // actual text.
            ...
            //          """
            //
            // as initial whitespace in text will already be checked in makeContent.  This is only for the case
            // where
            ...
            )
            {
                if (needsDedentation && !indentationWhitespace.IsEmpty)
                {
                    ...
                    if (
                        openBracePosition > 0
                        && SyntaxFacts.IsNewLine(originalText[openBracePosition - 1])
                    )
        ...
        private static int ConsumeRemainingContentThroughNewLine(
            StringBuilder content,
            ReadOnlySpan<char> text,
            int currentIndex
        )
        {
            ...
            while (currentIndex < text.Length)
            {
                ...
                currentIndex += SlidingTextWindow.GetNewLineWidth(
                    ch,
                    currentIndex + 1 < text.Length ? text[currentIndex + 1] : '\0'
                );
                break;
            }
        ...
        {
            // Grab the text from after the { all the way to the start of the } (or the start of the : if
            // present). This
            // will be used to parse out the expression of the interpolation.
            //
            // The parsing of the open brace, close brace and colon is specially handled in ParseInterpolation
            // below.
            var followingRange = interpolation.HasColon
                ? interpolation.ColonRange
                : interpolation.CloseBraceRange;
            var expressionText = text[interpolation.OpenBraceRange.End..followingRange.Start];

            using var tempLexer = new Lexer(
                SourceText.From(expressionText),
                options,
                allowPreprocessorDirectives: false,
        ...
        )
        {
            ...
            (
                ...
                InterpolationAlignmentClauseSyntax? alignment
            ) getExpressionAndAlignment()
            {
                ...
                if (interpolation.HasColon)
                {
                    var format = SyntaxFactory.InterpolationFormatClause(
                        SyntaxFactory.Token(
                            ...
                            text[interpolation.ColonRange],
                            trailing: null
                        ),
                        MakeInterpolatedStringTextToken(
                            kind,
                            text[interpolation.ColonRange.End..interpolation.CloseBraceRange.Start]
                        )
                    );
        ...
        /// </summary>
        /// <param name="text">The text for the full string literal, including the quotes and
        // contents</param>
        ...
        {
            // with a raw string, we don't do any interpretation of the content.  Note: removal of indentation
            // is
            // handled already in splitContent
            if (
                kind
                is Lexer.InterpolatedStringKind.SingleLineRaw
        ...
        private static DiagnosticInfo[] MoveDiagnostics(DiagnosticInfo[] infos, int offset)
        {
            ...
            {
                // This cast should always be safe.  We are only moving diagnostics produced on syntax nodes and
                // tokens.
