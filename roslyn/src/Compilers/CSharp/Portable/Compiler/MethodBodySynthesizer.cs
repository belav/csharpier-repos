...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal static class MethodBodySynthesizer
    {
        ...
        )
        {
            ...
            foreach (var field in synthesizedFields.FieldSymbols)
            {
                ...
                var targetSubmissionIndex =
                    targetScriptType.DeclaringCompilation.GetSubmissionSlotIndex();
        ...
        )
        {
            ...
            // value
            BoundParameter parameterAccess = new BoundParameter(syntax, accessor.Parameters[0]);
            ...
            if (isAddMethod)
            {
                ...
                // }
                BoundStatement returnStatement = BoundReturnStatement.Synthesized(
                    syntax,
                    RefKind.None,
                    processHandlerCall
                );
            ...
            else
            {
                ...
                //     return;
                // }
                BoundStatement callStatement = new BoundExpressionStatement(
                    syntax,
                    processHandlerCall
                );
                BoundStatement returnStatement = new BoundReturnStatement(
                    syntax,
                    RefKind.None,
                    expressionOpt: null
                );
            ...
            if ((object)baseTypeFinalize != null)
            {
                BoundStatement baseFinalizeCall = new BoundExpressionStatement(
                    ...
                    BoundCall.Synthesized(
                        syntax,
                        new BoundBaseReference(syntax, method.ContainingType)
                        {
                            WasCompilerGenerated = true
                        },
                        baseTypeFinalize
                ...
                foreach (Symbol member in baseType.GetMembers(WellKnownMemberNames.DestructorName))
                {
                    if (member.Kind == SymbolKind.Method)
                    {
                        ...
                        if (
                            ...
                            && // NOTE: the native compiler doesn't check this, so it broken IL.
