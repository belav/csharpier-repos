    ...
    internal abstract class PENamedTypeSymbol : NamedTypeSymbol
    {
        ...
        private NamedTypeSymbol MakeDeclaredBaseType()
        {
            if (!_flags.IsInterface())
            {
                try
                {
                    ...
                    if (!token.IsNil)
                    {
                        return (NamedTypeSymbol)new MetadataDecoder(moduleSymbol, this)
                ...
                if (interfaceImpls.Count > 0)
                {
                    ...
                    foreach (var interfaceImpl in interfaceImpls)
                    {
                        EntityHandle interfaceHandle =
                            moduleSymbol.Module.MetadataReader
                ...
                try
                {
                    foreach (var fieldDef in module.GetFieldsOfTypeOrThrow(_handle))
                    {
                        ...
                        if ((fieldFlags & FieldAttributes.Static) == 0)
                        {
                            ...
                            TypeSymbol type = decoder.DecodeFieldSignature(
                                fieldDef,
                                out customModifiers
                            );
        ...
        private void LoadMembers()
        {
            ...
            if (_lazyMembersInDeclarationOrder.IsDefault)
            {
                ...
                {
                    ArrayBuilder<PEFieldSymbol> fieldMembers = ArrayBuilder<PEFieldSymbol>
                        .GetInstance();
        ...
        private ImmutableArray<NamedTypeSymbol> MakeAcyclicInterfaces()
        {
            ...
            return declaredInterfaces.SelectAsArray(
                t => BaseTypeAnalysis.TypeDependsOn(t, this) ? CyclicInheritanceError(this, t) : t
            );
        }
        ...
        )
        {
            ...
            Debug.Assert(
                found
                    || !module.ShouldImportMethod(methodDef, this.ContainingPEModule.ImportOptions)
            );
            return method;
        }
        ...
        internal string DefaultMemberName
        {
            get
            {
                ...
                if (uncommon.lazyDefaultMemberName == null)
                {
                    ...
                    this.ContainingPEModule.Module
        ...
        internal override ImmutableArray<string> GetAppliedConditionalSymbols()
        {
            ...
            {
                ImmutableArray<string> conditionalSymbols = this.ContainingPEModule.Module
