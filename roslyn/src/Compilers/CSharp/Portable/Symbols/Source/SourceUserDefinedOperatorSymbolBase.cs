...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
        ...
        : SourceOrdinaryMethodOrUserDefinedOperatorSymbol
    {
        ...
        private const TypeCompareKind ComparisonForUserDefinedOperators =
...
#nullable disable
            ...
            )
        {
            ...
            if (
                this.ContainingType.IsInterface
        ...
        protected static DeclarationModifiers MakeDeclarationModifiers(
            MethodKind methodKind,
            bool inInterface,
        ...
        )
        {
            ...
            var defaultAccess =
                inInterface && !isExplicitInterfaceImplementation
            ...
            var result = ModifierUtils.MakeAndCheckNonTypeMemberModifiers(
                ...
                isForInterfaceMember: inInterface,
                syntax.Modifiers,
                defaultAccess,
                allowedModifiers,
                location,
                diagnostics,
                modifierErrors: out _
            );
            ...
            if (inInterface)
            {
                ...
                {
                    if (
                        (result & DeclarationModifiers.Sealed) != 0
                        && (result & (DeclarationModifiers.Abstract | DeclarationModifiers.Virtual))
                            != 0
                    )
                    {
                        diagnostics.Add(
                            ...
                            location,
                            ModifierUtils.ConvertSingleModifierToSyntaxText(
                                DeclarationModifiers.Sealed
                            )
                        );
            ...
            static void reportModifierIfPresent(
                ...
                DeclarationModifiers errorModifier,
                Location location,
                BindingDiagnosticBag diagnostics,
            ...
            if (_explicitInterfaceType is object)
            {
                ...
                switch (syntaxReferenceOpt.GetSyntax())
                {
                    case OperatorDeclarationSyntax operatorDeclaration:
                        interfaceMethodName = OperatorFacts.OperatorNameFromDeclaration(
                            operatorDeclaration
                        );
...
#nullable disable
        ...
        private void CheckUserDefinedConversionSignature(BindingDiagnosticBag diagnostics)
        {
            ...
            if (source0.IsInterfaceType() || target0.IsInterfaceType())
            {
                ...
                diagnostics.Add(
                    ErrorCode.ERR_ConversionWithInterface,
                    this.GetFirstLocation(),
                    this
                );
                return;
            ...
            if (different.IsClassType() && !same.IsTypeParameter())
            {
                ...
                )
                {
                    ...
                    diagnostics.Add(
                        ErrorCode.ERR_ConversionWithBase,
                        this.GetFirstLocation(),
                        this
                    );
                }
                else if (
                    different.IsDerivedFrom(
                        same,
                        ComparisonForUserDefinedOperators,
                        useSiteInfo: ref useSiteInfo
                    )
                )
                {
                    ...
                    diagnostics.Add(
                        ErrorCode.ERR_ConversionWithDerived,
                        this.GetFirstLocation(),
                        this
                    );
                }
        ...
        private bool MatchesContainingType(TypeSymbol type)
        {
            return IsContainingType(type)
                || ((IsAbstract || IsVirtual) && IsSelfConstrainedTypeParameter(type));
        }
        ...
        public static bool IsSelfConstrainedTypeParameter(
            TypeSymbol type,
        ...
        private void CheckShiftSignature(BindingDiagnosticBag diagnostics)
        {
            ...
            )
            {
                ...
                Binder.CheckFeatureAvailability(
                    location.SourceTree,
                    MessageID.IDS_FeatureRelaxedShiftOperator,
                    diagnostics,
                    location
                );
            }
        ...
        {
            if (
                !IsSelfConstrainedTypeParameter(this.GetParameterType(0).StrippedType())
                && !IsSelfConstrainedTypeParameter(this.GetParameterType(1).StrippedType())
            )
            {
                diagnostics.Add(
                    ErrorCode.ERR_BadAbstractEqualityOperatorSignature,
                    this.GetFirstLocation(),
                    this.ContainingType
                );
            }
