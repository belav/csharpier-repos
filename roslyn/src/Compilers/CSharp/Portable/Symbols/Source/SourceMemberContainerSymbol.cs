    ...
    internal abstract partial class SourceMemberContainerTypeSymbol : NamedTypeSymbol
    {
        ...
        > GetEarlyAttributeDecodingMembersDictionary()
        {
            if (_lazyEarlyAttributeDecodingMembersDictionary == null)
            {
                ...
                else
                {
                    ...
                    membersByName = membersAndInitializers.NonTypeMembers.WhereAsArray(
                            s =>
                                !s.IsIndexer()
                                && (
                                    !s.IsAccessor()
        ...
        internal void AssertMemberExposure(Symbol member, bool forDiagnostics = false)
        {
            ...
            if (
                ...
                || Volatile.Read(ref _lazyMembersAndInitializers)?.NonTypeMembers.Contains(
                    m => m == (object)member
                ) == true
            )
            {
        ...
        private sealed class DeclaredMembersAndInitializersBuilder
        {
            ...
            {
                this.NonTypeMembers = type.AddOrWrapTupleMembers(
                    this.NonTypeMembers.ToImmutableAndFree()
                );
            }
        }
        ...
        protected virtual MembersAndInitializers? BuildMembersAndInitializers(
            BindingDiagnosticBag diagnostics
        )
        {
            ...
            foreach (var decl in this.declaration.Declarations)
            {
                ...
                switch (syntax.Kind())
                {
                    ...
                    case SyntaxKind.NamespaceDeclaration:
                        ...
                        AddNonTypeMembers(
                            builder,
                            ((NamespaceDeclarationSyntax)syntax).Members,
                            diagnostics
                        );
                        break;
            ...
            foreach (var name in memberNames)
            {
                ...
                foreach (SourceOrdinaryMethodSymbol method in methodsBySignature.Values)
                {
                    ...
                    if (method.IsPartialImplementation && method.OtherPartOfPartial is null)
                    {
                        ...
                        method.OtherPartOfPartial is MethodSymbol otherPart
            ...
            foreach (var s in members)
            {
                ...
                if (!(m is null))
                {
                    if (m.MethodKind == MethodKind.Constructor && m.ParameterCount == 0)
                    {
                        ...
                        {
                            diagnostics.Add(
                                ErrorCode.ERR_StructsCantContainDefaultConstructor,
                                m.Locations[0]
                            );
                        }
                    }
        ...
        )
        {
            ...
            foreach (var initializers in builder.InstanceInitializers)
            {
                foreach (FieldOrPropertyInitializer initializer in initializers)
                {
                    ...
                    diagnostics.Add(
                        ErrorCode.ERR_FieldInitializerInStruct,
                        (initializer.FieldOpt.AssociatedSymbol ?? initializer.FieldOpt).Locations[
                            0
                        ],
                        this
                    );
        ...
        )
        {
            ...
            ImmutableArray<PropertySymbol> addProperties(
                ImmutableArray<ParameterSymbol> recordParameters
            )
            {
                var existingOrAddedMembers = ArrayBuilder<PropertySymbol>.GetInstance(
