...
#nullable disable
        ...
        private bool IsKnownAssemblyAttribute(CSharpAttributeData attribute)
        {
            ...
            if (
                attribute.IsTargetAttribute(AttributeDescription.AssemblyTitleAttribute)
...
#nullable disable
        ...
        )
        {
            ...
            bool emitCompilationRelaxationsAttribute =
                !isBuildingNetModule
                && !this.Modules.Any(static m => m.HasAssemblyCompilationRelaxationsAttribute);
        ...
        )
        {
            ...
            else if (attribute.IsTargetAttribute(AttributeDescription.TypeLibVersionAttribute))
            {
                ...
                attribute.IsTargetAttribute(AttributeDescription.ComCompatibleVersionAttribute)
            )
            {
                ValidateIntegralAttributeNonNegativeArguments(
                    attribute,
            ...
            {
                string guidString = attribute.DecodeGuidAttribute(
                    arguments.AttributeSyntaxOpt,
                    diagnostics
                );
                arguments.GetOrCreateData<CommonAssemblyWellKnownAttributeData>().GuidAttribute =
                    guidString;
            }
            else if (attribute.IsTargetAttribute(AttributeDescription.ImportedFromTypeLibAttribute))
            {
                ...
                attribute.IsTargetAttribute(AttributeDescription.PrimaryInteropAssemblyAttribute)
            )
            {
                ...
                attribute.IsTargetAttribute(AttributeDescription.CompilationRelaxationsAttribute)
            )
            {
                arguments
                    .GetOrCreateData<CommonAssemblyWellKnownAttributeData>()
            ...
            else if (attribute.IsTargetAttribute(AttributeDescription.ReferenceAssemblyAttribute))
            {
                ...
                attribute.IsTargetAttribute(AttributeDescription.RuntimeCompatibilityAttribute)
            )
            {
            ...
            else if (attribute.IsTargetAttribute(AttributeDescription.DebuggableAttribute))
            {
                ...
                !isFromNetModule
                && attribute.IsTargetAttribute(AttributeDescription.TypeForwardedToAttribute)
            )
            {
            ...
            for (int i = 0; i < argCount; i++)
            {
                ...
                if (arg < 0)
                {
                    ...
                    diagnostics.Add(
                        ...
                        attribute.GetAttributeArgumentLocation(i),
        ...
        )
        {
            if (_unusedFieldWarnings.IsDefault)
            {
                ...
                foreach (FieldSymbol field in _unassignedFieldsMap.Keys) // Not mutating, so no snapshot required.
                {
                    ...
                    if (
                        containingType.HasStructLayoutAttribute
                        || containingType.HasInlineArrayAttribute(out _)
                    )
                    {
                        continue;
                    }
                    ...
                    )
                    {
                        if (unread)
                        {
                            diagnostics.Add(
                                ...
                                associatedPropertyOrEvent.GetFirstLocationOrNone(),
        ...
        internal sealed override ObsoleteAttributeData? ObsoleteAttributeData
        {
            get
            {
                ...
                if (
                    ...
                    && lazySourceAttributesBag.IsDecodedWellKnownAttributeDataComputed
                )
                {
                    var data = (CommonAssemblyWellKnownAttributeData)
                ...
                if (
                    lazyNetModuleAttributesBag != null
                    && lazyNetModuleAttributesBag.IsDecodedWellKnownAttributeDataComputed
                )
                {
                    var data = (CommonAssemblyWellKnownAttributeData)
