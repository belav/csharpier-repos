...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    internal sealed partial class SourceNamespaceSymbol : NamespaceSymbol
    {
        ...
        )
        {
            ...
            {
                if (
                    singleDeclaration.HasExternAliases
                    || singleDeclaration.HasGlobalUsings
                    || singleDeclaration.HasUsings
                )
                {
        ...
        private Dictionary<string, ImmutableArray<NamedTypeSymbol>> GetNameToTypeMembersMap()
        {
            if (_nameToTypeMembersMap == null)
            {
                ...
                Interlocked.CompareExchange(
                    ...
                    GetTypesFromMemberMap(GetNameToMembersMap()),
                    null
                );
            }
        ...
        )
        {
            ...
            foreach (var name in result.Keys)
            {
                ...
                foreach (var symbol in result[name])
                {
                    ...
                    if ((object)other != null)
                    {
                        switch (nts, other)
                        {
                            case (SourceNamedTypeSymbol left, SourceNamedTypeSymbol right)
                                when isFileLocalTypeInSeparateFileFrom(left, right)
                                    || isFileLocalTypeInSeparateFileFrom(right, left):
                                // no error
                                break;
                            case
                            (SourceNamedTypeSymbol { IsFileLocal: true }, _)
                            or
                            (_, SourceNamedTypeSymbol { IsFileLocal: true }):
                                diagnostics.Add(
                                ...
                                break;
                            case (
                                SourceNamedTypeSymbol { IsPartial: true },
                                SourceNamedTypeSymbol { IsPartial: true }
                            ):
                                diagnostics.Add(
            ...
            static bool isFileLocalTypeInSeparateFileFrom(
                SourceNamedTypeSymbol possibleFileLocalType,
                SourceNamedTypeSymbol otherSymbol
            )
            {
        ...
        private readonly struct NameToSymbolMapBuilder
        {
            ...
            {
                var result = new Dictionary<String, ImmutableArray<NamespaceOrTypeSymbol>>(
                    _dictionary.Count,
                    StringOrdinalComparer.Instance
                );
