...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    ...
    internal sealed class SourceModuleSymbol : NonMissingModuleSymbol, IAttributeTargetSymbol
    {
            ...
            get
            {
                if (_locations.IsDefault)
                {
                    ImmutableInterlocked.InterlockedInitialize(
                        ref _locations,
                        DeclaringCompilation.MergedRootDeclaration.Declarations.SelectAsArray(d =>
                            (Location)d.Location
                        )
                    );
                }
        ...
        )
        {
            ...
            {
                arguments
                    .GetOrCreateData<ModuleWellKnownAttributeData>()
                    .ExperimentalAttributeData = attribute.DecodeExperimentalAttribute();
            }
        }
...
#nullable enable
        ...
        )
        {
            ...
            {
                var version = ImmutableArray.Create(
                    new TypedConstant(
                ...
                AddSynthesizedAttribute(
                    ref attributes,
                    moduleBuilder.SynthesizeRefSafetyRulesAttribute(version)
                );
            }
        ...
        internal override bool UseUpdatedEscapeRules
        {
            get
            {
                if (_lazyUseUpdatedEscapeRules == ThreeState.Unknown)
                {
                    var compilation = _assemblySymbol.DeclaringCompilation;
                    bool value =
                        compilation.IsFeatureEnabled(MessageID.IDS_FeatureRefFields)
        ...
        internal sealed override ObsoleteAttributeData? ObsoleteAttributeData
        {
            get
            {
                ...
                if (attributesBag != null && attributesBag.IsDecodedWellKnownAttributeDataComputed)
                {
                    var decodedData = (ModuleWellKnownAttributeData)
