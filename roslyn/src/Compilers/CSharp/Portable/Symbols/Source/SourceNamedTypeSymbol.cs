...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
            ...
            IAttributeTargetSymbol
    {
            ...
            : base(containingSymbol, declaration, diagnostics, tupleData)
        {
            ...
            _typeParameterInfo =
                declaration.Arity == 0 ? TypeParameterInfo.Empty : new TypeParameterInfo();
        }
        ...
        )
        {
            ...
            foreach (var syntaxRef in this.SyntaxReferences)
            {
                ...
                foreach (var tp in tpl.Parameters)
                {
                    if (tp.VarianceKeyword.Kind() != SyntaxKind.None)
                    {
                        if (!isInterfaceOrDelegate)
                        {
                            diagnostics.Add(
                                ...
                                tp.VarianceKeyword.GetLocation()
                            );
                        }
                        else
                        {
                    ...
                    if (name == null)
                    {
                        ...
                        for (int j = 0; j < i; j++)
                        {
                            if (name == typeParameterNames[j])
                            {
                                ...
                                diagnostics.Add(
                                    ErrorCode.ERR_DuplicateTypeParameter,
                                    location,
                                    name
                                );
                                goto next;
                    ...
                    parameterBuilder.Add(
                        new TypeParameterBuilder(syntaxTree.GetReference(tp), this, location)
                    );
                    i++;
                }
            }
            ...
            var parameters = parameterBuilders2.Select(
                (builders, i) => builders[0].MakeSymbol(i, builders, diagnostics)
            );
        ...
        private ImmutableArray<
            ImmutableArray<TypeWithAnnotations>
        > MakeTypeParameterConstraintTypes(BindingDiagnosticBag diagnostics)
        {
            ...
            if (arity > 0)
            {
                bool skipPartialDeclarationsWithoutConstraintClauses =
                ...
                foreach (var decl in declaration.Declarations)
                {
                    ...
                    var constraintClauses = GetConstraintClauses(
                        (CSharpSyntaxNode)syntaxRef.GetSyntax(),
                ...
                results = MergeConstraintTypesForPartialDeclarations(
                    results,
                    otherPartialClauses,
                    diagnostics
                );
        ...
        private ImmutableArray<TypeParameterConstraintKind> MakeTypeParameterConstraintKinds()
        {
            ...
            if (arity > 0)
            {
                ...
                results = ConstraintsHelper.AdjustConstraintKindsBasedOnConstraintTypes(
                    typeParameters,
                    results
                );
        ...
        /// </summary>
        private ImmutableArray<TypeParameterConstraintClause> MergeConstraintTypesForPartialDeclarations(
        ...
        )
        {
            ...
            for (int i = 0; i < arity; i++)
            {
                ...
                bool report =
                    (
                        GetTypeParameterConstraintKind(i)
                ...
                )
                {
                    if (
                        !mergeConstraints(
                            ...
                            ref originalConstraintTypesMap,
                ...
                if (report)
                {
                    ...
                    diagnostics.Add(
                        ErrorCode.ERR_PartialWrongConstraints,
                        GetFirstLocation(),
                        this,
                        typeParameters[i]
                    );
            ...
            )
            {
                ...
                {
                    TypeWithAnnotations constraintType1 =
                        mergedConstraintTypes?[index1] ?? originalConstraintTypes[index1];
                    int index2;
                    ...
                    )
                    {
                        ...
                        mergedConstraintTypes[index1] = constraintType1.MergeEquivalentTypes(
                            constraintType2,
                            VarianceKind.None
                        );
                    }
                ...
                if (constraint.Constraints != mergedKind)
                {
                    Debug.Assert(
                        (
                            constraint.Constraints
                            & (
                                TypeParameterConstraintKind.AllNonNullableKinds
                    ...
                    Debug.Assert(
                        (constraint.Constraints & TypeParameterConstraintKind.AllReferenceTypeKinds)
            ...
            )
            {
                ...
                )
                {
                    ...
                    TypeParameterConstraintKind clause1Constraints =
                        mergedKind & TypeParameterConstraintKind.AllReferenceTypeKinds;
                    TypeParameterConstraintKind clause2Constraints =
                        clause.Constraints & TypeParameterConstraintKind.AllReferenceTypeKinds;
        ...
        internal ImmutableArray<SyntaxList<AttributeListSyntax>> GetAttributeDeclarations(
            QuickAttributes? quickAttributes = null
        )
        {
            ...
            if (quickAttributes != null)
            {
                ...
                {
                    if (
                        decl is RootSingleNamespaceDeclaration rootNamespaceDecl
        ...
        )
        {
            ...
            )
            {
                ...
                if (!attributeData.HasErrors)
                {
                    ...
                    arguments.GetOrCreateData<TypeEarlyWellKnownAttributeData>().InlineArrayLength =
                        length > 0 ? length : -1;
            ...
            )
            {
                ...
                {
                    Debug.Assert(
                        attributeData.CommonConstructorArguments[0].Kind == TypedConstantKind.Type
                    );
                    TypeSymbol? builderType =
                        attributeData.CommonConstructorArguments[0].ValueInternal as TypeSymbol;
                    string? methodName = attributeData.GetConstructorArgument<string>(
                        1,
                        SpecialType.System_String
                    );
                    var data = new CollectionBuilderAttributeData(builderType, methodName);
                    arguments.GetOrCreateData<TypeEarlyWellKnownAttributeData>().CollectionBuilder =
                        data;
...
#nullable enable
        ...
        )
        {
            ...
            {
                arguments.GetOrCreateData<TypeWellKnownAttributeData>().HasDefaultMemberAttribute =
                    true;
            }
            ...
            {
                arguments.GetOrCreateData<TypeWellKnownAttributeData>().GuidString =
            ...
            {
                arguments.GetOrCreateData<TypeWellKnownAttributeData>().HasSpecialNameAttribute =
                    true;
            }
            else if (attribute.IsTargetAttribute(this, AttributeDescription.SerializableAttribute))
            {
                arguments.GetOrCreateData<TypeWellKnownAttributeData>().HasSerializableAttribute =
                    true;
            }
            else if (
            ...
            else if (attribute.IsTargetAttribute(this, AttributeDescription.StructLayoutAttribute))
            {
                ...
                >(
                    ref arguments,
                    this.DefaultMarshallingCharSet,
                    defaultAutoLayoutSize: 0,
                    messageProvider: MessageProvider.Instance
                );
            }
            else if (
                attribute.IsTargetAttribute(
            ...
            )
            {
                ...
                attribute.IsTargetAttribute(this, AttributeDescription.CollectionBuilderAttribute)
            )
            {
                var builderType =
                    attribute.CommonConstructorArguments[0].ValueInternal as TypeSymbol;
                if (!IsValidCollectionBuilderType(builderType))
                {
                    diagnostics.Add(
                        ErrorCode.ERR_CollectionBuilderAttributeInvalidType,
                        arguments.AttributeSyntaxOpt.Name.Location
                    );
                }
                ...
                string? methodName = attribute.CommonConstructorArguments[1].DecodeValue<string>(
                    SpecialType.System_String
                );
            ...
            {
                int length = attribute.CommonConstructorArguments[0].DecodeValue<int>(
                    SpecialType.System_Int32
                );
                ...
                {
                    diagnostics.Add(
                        ErrorCode.ERR_InvalidInlineArrayLength,
            ...
            else
            {
                ...
                {
                    attribute.DecodeSecurityAttribute<TypeWellKnownAttributeData>(
                        this,
                        compilation,
                        ref arguments
                    );
        ...
        internal static bool IsValidCollectionBuilderType(
            [NotNullWhen(true)] TypeSymbol? builderType
        )
        {
            return builderType
                is NamedTypeSymbol
...
#nullable disable
        ...
        private void CheckPresenceOfTypeIdentifierAttribute()
        {
            ...
            ImmutableArray<SyntaxList<AttributeListSyntax>> attributeLists =
                GetAttributeDeclarations(QuickAttributes.TypeIdentifier);
        ...
        )
        {
            ...
            )
            {
                ...
                {
                    arguments.GetOrCreateData<TypeWellKnownAttributeData>().ComImportCoClass =
                        coClassType;
                }
            }
        }
...
#nullable disable
        ...
        public sealed override bool AreLocalsZeroed
        {
            get
            {
                ...
                return data?.HasSkipLocalsInitAttribute != true
                    && (ContainingType?.AreLocalsZeroed ?? ContainingModule.AreLocalsZeroed);
            }
        }
        ...
        )
        {
            ...
            if (this.IsComImport)
            {
                ...
                {
                    int index = boundAttributes.IndexOfAttribute(
                        this,
                        AttributeDescription.ComImportAttribute
                    );
                    diagnostics.Add(
                        ErrorCode.ERR_ComImportWithoutUuidAttribute,
                        allAttributeSyntaxNodes[index].Name.Location
                    );
                }
                            ...
                            foreach (var singleInitializer in initializerGroup)
                            {
                                if (!singleInitializer.FieldOpt.IsMetadataConstant)
                                {
                                    ...
                                    diagnostics.Add(
                                        ...
                                        singleInitializer.Syntax.GetLocation(),
                                        this.Name
                                    );
                                }
                            }
            ...
            else if ((object)this.ComImportCoClass != null)
            {
                ...
                diagnostics.Add(
                    ...
                    allAttributeSyntaxNodes[index].Location,
                    this.Name
                );
            }
            ...
            if (data != null && data.HasDefaultMemberAttribute && this.Indexers.Any())
            {
                ...
                diagnostics.Add(
                    ErrorCode.ERR_DefaultMemberOnIndexedType,
                    allAttributeSyntaxNodes[index].Name.Location
                );
            }
        ...
        )
        {
            ...
            if (this.IsRefLikeType)
            {
                AddSynthesizedAttribute(
                    ref attributes,
                    moduleBuilder.SynthesizeIsByRefLikeAttribute(this)
                );
            ...
            if (
                moduleBuilder.EncSymbolChanges?.IsReplaced(((ISymbolInternal)this).GetISymbol())
                == true
            )
            {
...
#nullable enable
        ...
        {
            get { return this.declaration.Declarations.Any(static d => d.IsSimpleProgram); }
        }
        ...
        protected override void AfterMembersCompletedChecks(BindingDiagnosticBag diagnostics)
        {
            ...
            if (TypeKind == TypeKind.Struct && HasInlineArrayAttribute(out _))
            {
                ...
                )
                {
                    ...
                    if (
                        elementField.IsRequired
                        || elementField.IsReadOnly
                        || elementField.IsVolatile
                        || elementField.IsFixedSizeBuffer
                    )
                    {
                        diagnostics.Add(
                            ErrorCode.ERR_InlineArrayUnsupportedElementFieldModifier,
                            elementField.TryGetFirstLocation() ?? GetFirstLocation()
                        );
                    ...
                    {
                        if (
                            indexer.Parameters is [{ Type: { } type }]
                            && (
                                type.SpecialType == SpecialType.System_Int32
                                || type.Equals(
                                    index ??= DeclaringCompilation.GetWellKnownType(
                                        WellKnownType.System_Index
                                    ),
                                    TypeCompareKind.AllIgnoreOptions
                                )
                                || type.Equals(
                                    range ??= DeclaringCompilation.GetWellKnownType(
                        ...
                        {
                            diagnostics.Add(
                                ErrorCode.WRN_InlineArrayIndexerNotUsed,
                    ...
                    foreach (
                        var slice in GetMembers(WellKnownMemberNames.SliceMethodName)
                            .OfType<MethodSymbol>()
                    )
                    {
                        ...
                        {
                            diagnostics.Add(
                                ErrorCode.WRN_InlineArraySliceNotUsed,
                                slice.TryGetFirstLocation() ?? GetFirstLocation()
                            );
                            break;
                        }
                    }
                    ...
                    bool fieldSupported = TypeSymbol.IsInlineArrayElementFieldSupported(
                        elementField
                    );
                    if (fieldSupported)
                    {
                        foreach (
                            var conversion in GetMembers()
                                .OfType<SourceUserDefinedConversionSymbol>()
                        )
                        {
                            ...
                            if (
                                ...
                                && conversion.Parameters[0].Type.Equals(
                                    this,
                                    TypeCompareKind.AllIgnoreOptions
                                )
                                && (
                                    ...
                                    || returnTypeOriginalDefinition.Equals(
                                        readOnlySpan ??= DeclaringCompilation.GetWellKnownType(
                            ...
                            )
                            {
                                diagnostics.Add(
                                    ...
                                    conversion.TryGetFirstLocation() ?? GetFirstLocation()
                                );
                            }
                        }
                    }
                    ...
                    if (
                        ...
                        && (
                            !fieldSupported
                            || elementType.Type.IsPointerOrFunctionPointer()
                            || elementType.IsRestrictedType()
                        )
                    )
                    {
                        diagnostics.Add(
                            ErrorCode.WRN_InlineArrayNotSupportedByLanguage,
                            elementField.TryGetFirstLocation() ?? GetFirstLocation()
                        );
                    }
                }
