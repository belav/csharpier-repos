...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    internal static class ModifierUtils
    {
        ...
        )
        {
            ...
            var readonlyToken = modifiers.FirstOrDefault(
                static t => t.Kind() == SyntaxKind.ReadOnlyKeyword
            );
            if (
        ...
        )
        {
            ...
            while (errorModifiers != DeclarationModifiers.None)
            {
                ...
                switch (oneError)
                {
                    ...
                    case DeclarationModifiers.Virtual:
                        ...
                        diagnostics.Add(
                            ErrorCode.ERR_StaticNotVirtual,
                            errorLocation,
                            ModifierUtils.ConvertSingleModifierToSyntaxText(oneError)
                        );
                        break;
            ...
            modifierErrors |=
                checkFeature(
                    DeclarationModifiers.PrivateProtected,
        ...
        internal static void CheckScopedModifierAvailability(
        ...
        private static void ReportPartialError(
            Location errorLocation,
            BindingDiagnosticBag diagnostics,
            SyntaxTokenList? modifierTokens
        )
        ...
        )
        {
            if ((modifiers & defaultInterfaceImplementationModifiers) != 0)
            {
                LanguageVersion availableVersion = (
                    (CSharpParseOptions)errorLocation.SourceTree.Options
                ).LanguageVersion;
                ...
                )
                {
                    ...
                    if (
                        ...
                        && (
                            modifiers
                            & defaultInterfaceImplementationModifiers
                            & (DeclarationModifiers.Abstract | DeclarationModifiers.Virtual)
                        ) != 0
                    )
                    {
                        diagnostics.Add(
                            ErrorCode.ERR_BadMemberFlag,
                            errorLocation,
                            ConvertSingleModifierToSyntaxText(DeclarationModifiers.Sealed)
                        );
        ...
        internal static void ReportUnsupportedModifiersForLanguageVersion(
            DeclarationModifiers modifiers,
            DeclarationModifiers unsupportedModifiers,
            Location errorLocation,
            BindingDiagnosticBag diagnostics,
            LanguageVersion availableVersion,
            LanguageVersion requiredVersion
        ...
        )
        {
            ...
            {
                if (
                    (mods & DeclarationModifiers.Partial) == 0
        ...
        )
        {
            ...
            for (int i = 0; i < modifiers.Count; i++)
            {
                ...
                ReportDuplicateModifiers(modifier, one, result, ref seenNoDuplicates, diagnostics);

                if (one == DeclarationModifiers.Partial)
                {
                    var messageId = isForTypeDeclaration
                    ...
                    var isPartialAsyncMethod =
                        isOrdinaryMethod
                        && i == modifiers.Count - 2
                        && modifiers[i + 1].ContextualKind() is SyntaxKind.AsyncKeyword;
        ...
        )
        {
            ...
            if ((modifiers & DeclarationModifiers.Required) != 0)
            {
                var useSiteInfo = new CompoundUseSiteInfo<AssemblySymbol>(
                    futureDestination: diagnostics,
                    assemblyBeingBuilt: symbol.ContainingAssembly
                );
                ...
                {
                    case FieldSymbol
                        when !symbol.IsAsRestrictive(symbol.ContainingType, ref useSiteInfo):
                    case PropertySymbol { SetMethod: { } method }
                        when !method.IsAsRestrictive(symbol.ContainingType, ref useSiteInfo):
                    ...
                    case FieldSymbol when (modifiers & DeclarationModifiers.ReadOnly) != 0:
                        ...
                        diagnostics.Add(
                            ErrorCode.ERR_RequiredMemberMustBeSettable,
                            errorLocation,
                            symbol
                        );
                        reportedError = true;
