            ...
            : AnonymousTypeOrDelegateTemplateSymbol
        {
            ...
            ) : base(manager, Location.None) // Location is not needed since NameAndIndex is set explicitly below.
            {
                ...
                TypeParameters = createTypeParameters(
                    this,
                    parameterCount,
                    returnsVoid: voidReturnTypeOpt is { }
                );
                    ...
                    for (int i = 0; i < parameterCount; i++)
                    {
                        parameters.Add(
                            (
                                ...
                                refKinds.IsNull ? RefKind.None : refKinds[i],
                ...
                static ImmutableArray<TypeParameterSymbol> createTypeParameters(
                    AnonymousDelegateTemplateSymbol containingType,
                    int parameterCount,
                    bool returnsVoid
                )
                {
                    ...
                    {
                        typeParameters.Add(
                            new AnonymousTypeManager.AnonymousTypeParameterSymbol(
                                containingType,
                                i,
                                "T" + (i + 1)
                            )
            ...
            ) : base(manager, typeDescr.Location)
            {
                ...
                else
                {
                    ...
                    {
                        typeParameters.Add(
                            new AnonymousTypeParameterSymbol(this, i, "T" + (i + 1))
                        );
                    }
                    TypeParameters = typeParameters.ToImmutableAndFree();
                    typeMap = new TypeMap(
                        typeParametersToSubstitute,
                        TypeParameters,
                        allowAlpha: true
                    );
                }
                ...
                )
                {
                    ...
                    for (int i = 0; i < parameterCount; i++)
                    {
                        var field = fields[i];
                        parameters.Add(
                            (typeMap.SubstituteType(field.Type), field.RefKind, field.Scope)
                        );
                    }
