...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    internal abstract partial class NamedTypeSymbol
    {
        ...
        )
        {
            ...
            Debug.Assert(
                elementNames.IsDefault || elementTypesWithAnnotations.Length == elementNames.Length
            );
            ...
            NamedTypeSymbol underlyingType = getTupleUnderlyingType(
            ...
            if (
                numElements >= ValueTupleRestPosition
                && diagnostics != null
                && !underlyingType.IsErrorType()
            )
            {
                ...
                _ = GetWellKnownMemberInType(
                    underlyingType.OriginalDefinition,
                    wellKnownTupleRest,
                    diagnostics,
                    syntax
                );
            ...
            if (
                ...
                && ((SourceModuleSymbol)compilation.SourceModule).AnyReferencedAssembliesAreLinked
            )
            {
            ...
            var locations = locationOpt is null
                ...
                : ImmutableArray.Create(locationOpt);
            var constructedType = CreateTuple(
                underlyingType,
                elementNames,
                errorPositions,
                elementLocations,
            ...
            if (shouldCheckConstraints && diagnostics != null)
            {
                ...
                constructedType.CheckConstraints(
                    ...
                    syntax,
                    elementLocations,
                    nullabilityDiagnosticsOpt: includeNullability ? diagnostics : null
                );
            }
            ...
            )
            {
                ...
                NamedTypeSymbol firstTupleType = compilation.GetWellKnownType(
                ...
                if (chainLength > 1)
                {
                    chainedTupleType = compilation.GetWellKnownType(
        ...
        )
        {
            ...
            return tupleCompatibleType.WithElementNames(
                elementNames,
                elementLocations,
                errorPositions,
                locations
        ...
        internal NamedTypeSymbol WithTupleDataFrom(NamedTypeSymbol original)
        {
            if (
                !IsTupleType
                || (original._lazyTupleData == null && this._lazyTupleData == null)
            ...
            return WithElementNames(
                ...
                original.TupleElementLocations,
                original.TupleErrorPositions,
                original.Locations
            );
        }

        internal NamedTypeSymbol? TupleUnderlyingType =>
            this._lazyTupleData != null
                ? this.TupleData!.TupleUnderlyingType
        ...
        internal NamedTypeSymbol WithElementTypes(
            ImmutableArray<TypeWithAnnotations> newElementTypes
        )
        {
            ...
            return CreateTuple(
                ...
                elementLocations: TupleElementLocations,
                errorPositions: TupleErrorPositions,
                locations: Locations
            );
        }
        ...
        )
        {
            ...
            Debug.Assert(
                newElementNames.IsDefault
                    || this.TupleElementTypesWithAnnotations.Length == newElementNames.Length
            );
            return WithTupleData(
                new TupleExtraData(
                    this.TupleUnderlyingType!,
                    newElementNames,
                    newElementLocations,
                    errorPositions,
                    locations
        ...
        private NamedTypeSymbol WithTupleData(TupleExtraData newData)
        {
            ...
            if (this.IsDefinition)
            {
                ...
                return this.ConstructCore(this.GetTypeParametersAsTypeArguments(), unbound: false)
                    .WithTupleData(newData);
            }
        ...
        {
            Debug.Assert(
                chainedTupleTypeOpt is null == elementTypes.Length < ValueTupleRestPosition
            );
            ...
            NamedTypeSymbol currentSymbol = firstTupleType.Construct(
                ImmutableArray.Create(
                    elementTypes,
                    (chainLength - 1) * (ValueTupleRestPosition - 1),
                    remainder
                )
            );
        ...
        private static void ReportUseSiteAndObsoleteDiagnostics(
            CSharpSyntaxNode? syntax,
            BindingDiagnosticBag diagnostics,
            NamedTypeSymbol firstTupleType
        )
        {
            ...
            Binder.ReportDiagnosticsIfObsoleteInternal(
                diagnostics,
                firstTupleType,
                syntax,
                firstTupleType.ContainingType,
                BinderFlags.None
            );
        }
        ...
        /// </summary>
        internal static void VerifyTupleTypePresent(
            int cardinality,
            CSharpSyntaxNode? syntax,
            CSharpCompilation compilation,
            BindingDiagnosticBag diagnostics
        )
        {
            ...
            if (chainLength > 1)
            {
                NamedTypeSymbol chainedTupleType = compilation.GetWellKnownType(
                    GetTupleType(ValueTupleRestPosition)
                );
                ReportUseSiteAndObsoleteDiagnostics(syntax, diagnostics, chainedTupleType);
            }
        ...
        internal static void ReportTupleNamesMismatchesIfAny(
            TypeSymbol destination,
            BoundTupleLiteral literal,
            BindingDiagnosticBag diagnostics
        )
        {
            ...
            for (int i = 0; i < sourceLength; i++)
            {
                ...
                if (
                    sourceName != null
                    && !wasInferred
                    && (allMissing || string.CompareOrdinal(destinationNames[i], sourceName) != 0)
                )
                {
                    diagnostics.Add(
                        ErrorCode.WRN_TupleLiteralNameMismatch,
                        literal.Arguments[i].Syntax.Parent!.Location,
                        sourceName,
                        destination
                    );
                }
        ...
        {
            new[] { WellKnownMember.System_ValueTuple_T1__Item1 },
            new[]
            {
        ...
        /// </summary>
        internal static Symbol? GetWellKnownMemberInType(
            NamedTypeSymbol type,
            WellKnownMember relativeMember,
            BindingDiagnosticBag diagnostics,
            SyntaxNode? syntax
        )
        {
            ...
            static Symbol? GetWellKnownMemberInType(
                NamedTypeSymbol type,
                WellKnownMember relativeMember
            )
            {
                Debug.Assert(
                    relativeMember >= WellKnownMember.System_ValueTuple_T1__Item1
        ...
        )
        {
            ...
            var fieldDefinitionsToIndexMap = IsDefinition
                ? new SmallDictionary<FieldSymbol, int>(ReferenceEqualityComparer.Instance)
                : null;
            ...
            while (true)
            {
                foreach (Symbol member in currentMembers)
                {
                    switch (member.Kind)
                    {
                        case SymbolKind.Field:
                            ...
                            int tupleFieldIndex = currentFieldsForElements.IndexOf(
                                underlyingField,
                            ...
                            else if (tupleFieldIndex >= 0)
                            {
                                ...
                                {
                                    tupleFieldIndex +=
                                        (ValueTupleRestPosition - 1) * currentNestingLevel;
                                }
                                else
                                {
                                    replacedFields?.Add(field);
                                ...
                                var providedName = elementNames.IsDefault
                                    ? null
                                    : elementNames[tupleFieldIndex];
                                ImmutableArray<Location> locations = getElementLocations(
                                    in elementLocations,
                                    tupleFieldIndex
                                );
                                ...
                                {
                                    Debug.Assert(
                                        fieldSymbol.Name == defaultName,
                                        "top level underlying field must match default name"
                                    );
                                    if (IsDefinition)
                                    {
                                ...
                                if (
                                    defaultImplicitlyDeclared && !string.IsNullOrEmpty(providedName)
                                )
                                {
                                    var errorPositions = TupleErrorPositions;
                                    var isError = errorPositions.IsDefault
                                        ? false
                                        : errorPositions[tupleFieldIndex];

                                    // The name given doesn't match the default name Item8, etc.
                                    // Add a virtual field with the given name
                                    members.Add(
                                        new TupleVirtualElementFieldSymbol(
                                            this,
                                            fieldSymbol,
                                            providedName,
                                            tupleFieldIndex,
                ...
                {
                    Debug.Assert(
                        (object)oldUnderlying.OriginalDefinition
                            == currentValueTuple.OriginalDefinition
                    );
                }
            }
            ...
            for (int i = 0; i < elementsMatchedByFields.Count; i++)
            {
                if (!elementsMatchedByFields[i])
                {
                    ...
                    // Add a field with default name. It should be present regardless.
                    TupleErrorFieldSymbol defaultTupleField = new TupleErrorFieldSymbol(
                        this,
                        defaultName,
                        i,
            ...
            static NamedTypeSymbol getNestedTupleUnderlyingType(
                NamedTypeSymbol topLevelUnderlyingType,
                int depth
            )
            {
            ...
            )
            {
                ...
                for (int i = 0; i < fieldsPerType; i++)
                {
                    ...
                    fieldsForElements.Add(
                        (FieldSymbol?)GetWellKnownMemberInType(members, wellKnownTupleField)
                    );
                }
            }
            ...
            static ImmutableArray<Location> getElementLocations(
                in ImmutableArray<Location?> elementLocations,
                int tupleFieldIndex
            )
            {
        ...
        private TypeSymbol MergeTupleNames(NamedTypeSymbol other, NamedTypeSymbol mergedType)
        {
            ...
            else
            {
                ...
                mergedNames = names1.ZipAsArray(
                    names2,
                    (n1, n2) => string.CompareOrdinal(n1, n2) == 0 ? n1 : null
                )!;
        ...
        internal sealed class TupleExtraData
        {
            ...
            internal bool EqualsIgnoringTupleUnderlyingType(TupleExtraData? other)
            {
                if (
                    other is null
                    && this.ElementNames.IsDefault
                    && this.ElementLocations.IsDefault
                    && this.ErrorPositions.IsDefault
                )
                {
                    return true;
                }
            ...
            public ImmutableArray<TypeWithAnnotations> TupleElementTypesWithAnnotations(
            ...
            )
            {
                ...
                static ImmutableArray<TypeWithAnnotations> collectTupleElementTypesWithAnnotations(
                    NamedTypeSymbol tuple
                )
                {
                    ImmutableArray<TypeWithAnnotations> elementTypes;
            ...
            internal SmallDictionary<FieldSymbol, int> GetFieldDefinitionsToIndexMap(
                NamedTypeSymbol tuple
            )
            {
                        ...
                        for (int i = members.Length - 1; i >= 0; i--)
                        {
                            ...
                            switch (member.Kind)
                            {
                                ...
                                case SymbolKind.Event:
                                    ...
                                    {
                                        Debug.Assert(
                                            (object)underlyingAssociatedField.ContainingSymbol
                                                == TupleUnderlyingType
                                        );
                                        Debug.Assert(
                                            TupleUnderlyingType
                                                .GetMembers(underlyingAssociatedField.Name)
                                                .IndexOf(underlyingAssociatedField) < 0
                                        );
                                        map.Add(
                                            underlyingAssociatedField.OriginalDefinition,
            ...
            ) where TMember : Symbol
            {
                ...
                {
                    if (
                        UnderlyingDefinitionToMemberMap.TryGetValue(
