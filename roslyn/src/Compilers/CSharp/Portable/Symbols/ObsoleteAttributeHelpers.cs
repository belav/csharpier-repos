...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    ...
    internal static class ObsoleteAttributeHelpers
    {
        ...
        /// </returns>
        private static ThreeState GetObsoleteContextState(
            Symbol symbol,
            bool forceComplete,
        ...
        internal static ObsoleteDiagnosticKind GetObsoleteDiagnosticKind(
            Symbol symbol,
        ...
        )
        {
            switch (symbol.ObsoleteKind)
            {
                case ObsoleteAttributeKind.None:
                    if (
                        symbol.ContainingModule.ObsoleteKind is ObsoleteAttributeKind.Experimental
                        || symbol.ContainingAssembly.ObsoleteKind
                    ...
                    )
                    {
                        return getDiagnosticKind(
                            containingMember,
                            forceComplete,
                            getStateFromSymbol: static (symbol) => symbol.ExperimentalState
                        );
                    }

                    if (
                        symbol.ContainingModule.ObsoleteKind is ObsoleteAttributeKind.Uninitialized
                        || symbol.ContainingAssembly.ObsoleteKind
            ...
            return getDiagnosticKind(
                containingMember,
                forceComplete,
                getStateFromSymbol: static (symbol) => symbol.ObsoleteState
            );

            static ObsoleteDiagnosticKind getDiagnosticKind(
                Symbol containingMember,
                bool forceComplete,
                Func<Symbol, ThreeState> getStateFromSymbol
            )
            {
                switch (
                    GetObsoleteContextState(containingMember, forceComplete, getStateFromSymbol)
                )
                {
            ...
            static DiagnosticInfo createObsoleteDiagnostic(Symbol symbol, BinderFlags location)
            {
                ...
                if (data.Kind == ObsoleteAttributeKind.Experimental)
                {
                    ...
                    return new CustomObsoleteDiagnosticInfo(
                        ...
                        (int)ErrorCode.WRN_Experimental,
                        data,
                        new FormattedSymbol(symbol, SymbolDisplayFormat.CSharpErrorMessageFormat)
                    );
                }
        ...
        {
            return (ErrorCode)diagnosticInfo.Code
                is (
                    ErrorCode.WRN_Experimental
