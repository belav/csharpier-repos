...
namespace Microsoft.CodeAnalysis.CSharp.Syntax
{
    internal static class SyntaxNodeRemover
    {
        ...
        private class SyntaxRemover : CSharpSyntaxRewriter
        {
            ...
            {
                return node.FullSpan.IntersectsWith(_searchSpan)
            ...
            private void AddTrivia(SyntaxToken token, SyntaxNode node)
            {
                ...
                else if ((_options & SyntaxRemoveOptions.KeepEndOfLine) != 0)
                {
                    ...
                    var eol =
                        GetEndOfLine(token.LeadingTrivia) ?? GetEndOfLine(token.TrailingTrivia);
            ...
            private void AddTrivia(SyntaxNode node, SyntaxToken token)
            {
                ...
                else if ((_options & SyntaxRemoveOptions.KeepEndOfLine) != 0)
                {
                    ...
                    var eol =
                        GetEndOfLine(node.GetTrailingTrivia())
                ...
                if (node.ContainsDirectives)
                {
                    ...
                    foreach (var directive in directivesInSpan)
                    {
                        ...
                        else if (HasRelatedDirectives(directive))
                        {
                            ...
                            var balanced = relatedDirectives.All(rd =>
                                rd.FullSpan.OverlapsWith(span)
                            );
                            ...
                            if (!balanced)
                            {
                                ...
                                foreach (
                                    var unbalancedDirective in relatedDirectives.Where(rd =>
                                        rd.FullSpan.OverlapsWith(span)
                                    )
                                )
                                {
                        ...
                        {
                            AddResidualTrivia(
                                SyntaxFactory.TriviaList(directive.ParentTrivia),
