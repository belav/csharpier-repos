...
#pragma warning restore
        ...
        {
            return SyntaxReplacer
                .InsertTriviaInList(this, originalTrivia, newTrivia, insertBefore)
                .AsRootOfNewTreeWithOptionsFrom(this.SyntaxTree);
        }
        ...
        internal string Dump()
        {
            ...
            static TreeDumperNode makeTree(SyntaxNodeOrToken nodeOrToken)
            {
                ...
                if (nodeOrToken.AsNode(out var node) && node is not IdentifierNameSyntax)
                {
                    return new TreeDumperNode(
                        kind,
                        null,
                        node.ChildNodesAndTokens().Select(makeTree)
                    );
                }
