    ...
    internal abstract partial class BoundNode
    {
        ...
        internal string DumpSource()
        {
            ...
            appendSourceCore(
                this,
                indent: 0,
                tempIdentifiers: new Dictionary<SynthesizedLocal, int>()
            );
            return builder.ToString();

            void appendSourceCore(
                BoundNode node,
                int indent,
                ...
                switch (node)
                {
                    case BoundTryStatement tryStatement:
                    {
                        ...
                        if (catchBlocks != null)
                        {
                            foreach (var catchBlock in catchBlocks)
                            {
                                ...
                                {
                                    if (
                                        catchBlock.ExceptionFilterPrologueOpt is
                                        { } exceptionFilterPrologue
                                    )
                                    {
                                        appendLine("");
                                        appendLine("{");
                    ...
                    case BoundBlock block:
                    {
                        ...
                        foreach (var local in block.Locals)
                        {
                            ...
                            {
                                appendLine(
                                    $"{local.TypeWithAnnotations.ToDisplayString()} {name(synthesized)};"
                                );
                            }
                            else
                            {
