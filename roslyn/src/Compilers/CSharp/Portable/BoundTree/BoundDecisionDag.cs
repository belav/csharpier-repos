    ...
    internal partial class BoundDecisionDag
    {
        ...
        public ImmutableArray<BoundDecisionDagNode> TopologicallySortedNodes
        {
            get
            {
                ...
                {
                    // We use an iterative topological sort to avoid overflowing the compiler's runtime stack for a
                    ...
                    // Since these nodes were constructed by an isomorphic mapping from a known acyclic graph, it cannot
                    // be cyclic
        ...
        )
        {
            ...
            // a node's successors before the node, the replacement should always be in the cache when we need
            // it.
            ...
            // Loop backwards through the topologically sorted nodes to translate them, so that we always visit
            // a node after its successors
        ...
        /// <summary>
        /// A trivial node replacement function for use with <see cref="Rewrite(Func{BoundDecisionDagNode,
        // IReadOnlyDictionary{BoundDecisionDagNode, BoundDecisionDagNode}, BoundDecisionDagNode})"/>.
        /// </summary>
        ...
        /// <summary>
        /// Given a decision dag and a constant-valued input, produce a simplified decision dag that has
        // removed all the
        /// tests that are unnecessary due to that constant value. This simplification affects flow analysis
        // (reachability
        ...
        public BoundDecisionDag SimplifyDecisionDagIfConstantInput(BoundExpression input)
        {
            ...
            else
            {
                ...
                )
                {
                    ...
                    {
                        // This is the key to the optimization. The result of a top-level test might be known if the input
                        // is constant.
