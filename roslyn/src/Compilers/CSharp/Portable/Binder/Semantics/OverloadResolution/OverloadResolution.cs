            ...
            for (int f = 0; f < results.Count; ++f)
            {
                ...
                if (result.Result.IsValid && member.RequiresInstanceReceiver() == requireStatic)
                {
                    results[f] = new MemberResolutionResult<TMember>(
                        ...
                        MemberAnalysisResult.StaticInstanceMismatch()
                    );
                }
            }
        }
...
#nullable disable
            ...
            for (int f = 0; f < results.Count; ++f)
            {
                ...
                if (!returnsMatch)
                {
                    results[f] = new MemberResolutionResult<TMember>(
                        result.Member,
                        result.LeastOverriddenMember,
                        MemberAnalysisResult.WrongReturnType()
                    );
                }
                else if (method.RefKind != returnRefKind)
                {
                    results[f] = new MemberResolutionResult<TMember>(
                        result.Member,
                        result.LeastOverriddenMember,
                        MemberAnalysisResult.WrongRefKind()
                    );
                }
            }
        }
        ...
        > PartitionMembersByContainingType<TMember>(
            ArrayBuilder<TMember> members
        ) where TMember : Symbol
        {
        ...
        private void AddConstructorToCandidateSet(
            ...
            if (
                ...
                results.Add(
                    new MemberResolutionResult<MethodSymbol>(constructor, constructor, result)
                );
            }
        }
            ...
            for (int f = 0; f < results.Count; ++f)
            {
                ...
                if (
                    ...
                    results[f] = new MemberResolutionResult<TMember>(
                        result.Member,
                        result.LeastOverriddenMember,
                        MemberAnalysisResult.InaccessibleTypeArgument()
                    );
                }
            }
        }
            ...
            for (int f = 0; f < results.Count; ++f)
            {
                ...
                if (
                    ...
                    results[f] = new MemberResolutionResult<TMember>(
                        result.Member,
                        result.LeastOverriddenMember,
                        MemberAnalysisResult.LessDerived()
                    );
                }
            }
        }
        ...
        ) where TMember : Symbol
        {
            for (int f = 0; f < results.Count; ++f)
            {
                ...
                if (
                    ...
                    && currentType.AllInterfacesWithDefinitionUseSiteDiagnostics(ref useSiteInfo)
        ...
        ) where TMember : Symbol
        {
            ...
            if (
                ...
                return PreferValOverInParameters(
                    arguments,
                    m1,
        ...
        private static BetterResult MoreSpecificType(
            ArrayBuilder<TypeSymbol> t1,
            ArrayBuilder<TypeSymbol> t2,
            ref CompoundUseSiteInfo<AssemblySymbol> useSiteInfo
        ) {
        ...
        private bool ExpressionMatchExactly(
            ...
            if (
                ...
                if (lambda.Symbol.IsAsync)
                {
                    ...
                    {
                        y =
                            ((NamedTypeSymbol)y).TypeArgumentsWithAnnotationsNoUseSiteDiagnostics[
                                0
                            ].Type;
                    }
                    else
        ...
        private static RefKind GetEffectiveParameterRefKind(
            ...
            if (
                !isMethodGroupConversion && argRefKind == RefKind.None && paramRefKind == RefKind.In
            ) {
                return RefKind.None;
            }
        ...
        private ImmutableArray<TypeWithAnnotations> InferMethodTypeArguments(
            ...
            if (arguments.IsExtensionMethodInvocation)
            {
                var inferredFromFirstArgument =
                    MethodTypeInferrer.InferTypeArgumentsFromFirstArgument(
                        _binder.Conversions,
                        method,
                        args,
                        useSiteInfo: ref useSiteInfo
        ...
        private MemberAnalysisResult IsApplicable(
            ...
            if (badArguments != null)
            {
                result = MemberAnalysisResult.BadArgumentConversions(
                    argsToParameters,
                    badArguments.ToImmutableAndFree(),
                    conversionsArray
                );
            }
            else
        ...
        private Conversion CheckArgumentForApplicability(
            ...
            if (
                (object)argType != null && Conversions.HasIdentityConversion(argType, parameterType)
            ) {
                return Conversion.Identity;
            }
            else
