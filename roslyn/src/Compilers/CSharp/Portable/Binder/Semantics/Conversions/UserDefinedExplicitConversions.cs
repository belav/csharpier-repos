...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal partial class ConversionsBase
    {
        ...
        )
        {
            ...
            ComputeApplicableUserDefinedExplicitConversionSet(
                sourceExpression,
                source,
                target,
                isChecked: isChecked,
            ...
            TypeSymbol sx = MostSpecificSourceTypeForExplicitUserDefinedConversion(
                u,
                sourceExpression,
                source,
                ref useSiteInfo
            ...
            TypeSymbol tx = MostSpecificTargetTypeForExplicitUserDefinedConversion(
                u,
                target,
                ref useSiteInfo
            );
        ...
        )
        {
            ...
            AddTypesParticipatingInUserDefinedConversion(
                d,
                source,
        ...
        )
        {
            ...
            foreach (TypeSymbol declaringType in d)
            {
                ...
                {
                    ImmutableArray<NamedTypeSymbol> interfaceTypes =
                    ...
                    {
                        lookedInInterfaces ??= new HashSet<NamedTypeSymbol>(
                            Symbols.SymbolEqualityComparer.AllIgnoreOptions
                        ); // Equivalent to has identity conversion check
                        ...
                        foreach (var interfaceType in interfaceTypes)
                        {
                            if (lookedInInterfaces.Add(interfaceType))
                            {
                                addCandidatesFromType(
                                    constrainedToTypeOpt: typeParameter,
                                    interfaceType,
                                    sourceExpression,
                                    source,
                                    target,
        ...
        )
        {
            ...
            {
                ImmutableArray<MethodSymbol> operators2 = declaringType.GetOperators(
                    WellKnownMemberNames.ExplicitConversionName
                );
            ...
            foreach (MethodSymbol op in candidates)
            {
                ...
                Conversion fromConversion = EncompassingExplicitConversion(
                    sourceExpression,
                    source,
                    convertsFrom,
                    ref useSiteInfo
                );
                Conversion toConversion = EncompassingExplicitConversion(
                    convertsTo,
                    target,
                    ref useSiteInfo
                );
                ...
                if (
                    !fromConversion.Exists
                    && (object)source != null
                    && source.IsNullableType()
                    && EncompassingExplicitConversion(
                        source.GetNullableUnderlyingType(),
                        convertsFrom,
                        ref useSiteInfo
                    ).Exists
                )
                ...
                {
                    toConversion = ClassifyBuiltInConversion(
                        convertsTo,
                        target,
                        isChecked: isChecked,
                        ref useSiteInfo
                ...
                if (fromConversion.Exists && toConversion.Exists)
                {
                    ...
                    )
                    {
                        ...
                        TypeSymbol nullableTo = convertsTo.IsValidNullableTypeArgument()
                        ...
                        Conversion liftedToConversion = EncompassingExplicitConversion(
                            nullableTo,
                            target,
                            ref useSiteInfo
                        );
                        ...
                        u.Add(
                            UserDefinedConversionAnalysis.Lifted(
                                constrainedToTypeOpt,
                                op,
                    ...
                    else
                    {
                        ...
                        if (target.IsNullableType() && convertsTo.IsValidNullableTypeArgument())
                        {
                            ...
                            toConversion = EncompassingExplicitConversion(
                                convertsTo,
                                target,
                                ref useSiteInfo
                            );
                        ...
                        u.Add(
                            UserDefinedConversionAnalysis.Normal(
                                constrainedToTypeOpt,
                                op,
                                fromConversion,
                                toConversion,
        ...
        )
        {
            ...
            if ((object)source != null)
            {
                ...
                System.Func<UserDefinedConversionAnalysis, bool> isValid = conv =>
                    IsEncompassedBy(
                        sourceExpression,
                        source,
                        conv.FromType,
                ...
                if (u.Any(isValid))
                {
                    var result = MostEncompassedType(
                        u,
                        isValid,
                        conv => conv.FromType,
                        ref inLambdaUseSiteInfo
        ...
        )
        {
            ...
            CompoundUseSiteInfo<AssemblySymbol> inLambdaUseSiteInfo = useSiteInfo;
            System.Func<UserDefinedConversionAnalysis, bool> isValid = conv =>
                IsEncompassedBy(conv.ToType, target, ref inLambdaUseSiteInfo);
            if (u.Any(isValid))
            {
                var result = MostEncompassingType(
                    u,
                    isValid,
                    conv => conv.ToType,
                    ref inLambdaUseSiteInfo
        ...
        private Conversion EncompassingExplicitConversion(
