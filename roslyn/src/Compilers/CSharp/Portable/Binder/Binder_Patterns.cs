...
namespace Microsoft.CodeAnalysis.CSharp
{
    partial class Binder
    {
        ...
        )
        {
            ...
            {
                receiverPlaceholder = new BoundSlicePatternReceiverPlaceholder(
                    node,
                    GetValEscape(inputType, inputValEscape),
                    inputType
                )
                {
                ...
                pattern = BindPattern(
                    node.Pattern,
                    sliceType,
                    GetValEscape(sliceType, inputValEscape),
                    permitDesignations,
                    hasErrors,
                    diagnostics
                );
            ...
            foreach (PatternSyntax pattern in subpatterns)
            {
                ...
                else
                {
                    boundPattern = BindPattern(
                        pattern,
                        elementType,
                        GetValEscape(elementType, inputValEscape),
                        permitDesignations,
                        hasErrors,
                        diagnostics
                    );
        ...
        )
        {
            var success = BindLengthAndIndexerForListPattern(
                node,
                inputType,
                inputValEscape: CallingMethodScope,
        ...
        )
        {
            ...
            receiverPlaceholder = new BoundListPatternReceiverPlaceholder(
                node,
                GetValEscape(inputType, inputValEscape),
                inputType
            )
                ...
                if (convertedExpression.Kind == BoundKind.Conversion)
                {
                    ...
                    if (
                        ...
                        && (
                            ...
                            || !convertedExpression.ConstantValue.IsNull
                        )
                    )
                    {
