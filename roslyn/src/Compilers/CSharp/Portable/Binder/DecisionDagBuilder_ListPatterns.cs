                    ...
                    new Tests.One(
                        list.HasSlice
                            ? new BoundDagRelationalTest(
                                ...
                                ConstantValue.Create(subpatterns.Length - 1),
                                lengthTemp
                            )
                            : new BoundDagValueTest(
                                syntax,
                                ConstantValue.Create(subpatterns.Length),
                                lengthTemp
                            )
                    )
                );
                ...
                foreach (BoundPattern subpattern in subpatterns)
                {
                    if (subpattern is BoundSlicePattern slice)
                    {
                        ...
                        if (slice.Pattern is BoundPattern slicePattern)
                        {
                            ...
                            var sliceEvaluation = new BoundDagSliceEvaluation(
                                slicePattern.Syntax,
                                slicePattern.InputType,
                                lengthTemp,
                                startIndex: startIndex,
                                endIndex: index,
                                slice.IndexerAccess,
                                slice.ReceiverPlaceholder,
                                slice.ArgumentPlaceholder,
                                input
                            );

                            tests.Add(new Tests.One(sliceEvaluation));
                            var sliceTemp = new BoundDagTemp(
                                slicePattern.Syntax,
                                slicePattern.InputType,
                                sliceEvaluation
                            );
                    ...
                    var indexEvaluation = new BoundDagIndexerEvaluation(
                        subpattern.Syntax,
                        subpattern.InputType,
                        lengthTemp,
                        index++,
                        list.IndexerAccess,
                        list.ReceiverPlaceholder,
                        list.ArgumentPlaceholder,
                        input
                    );

                    tests.Add(new Tests.One(indexEvaluation));
                    var indexTemp = new BoundDagTemp(
                        subpattern.Syntax,
                        subpattern.InputType,
                        indexEvaluation
                    );
