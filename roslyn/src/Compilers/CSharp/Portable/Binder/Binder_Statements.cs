...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal partial class Binder
    {
        ...
        private void CheckRequiredLangVersionForAsyncIteratorMethods(
            BindingDiagnosticBag diagnostics
        )
        {
        ...
        )
        {
            ...
            TypeWithAnnotations declType = BindTypeOrVarKeyword(
                typeSyntax.SkipScoped(out _).SkipRef(out _),
                diagnostics,
                out isVar,
                out alias
            );
        ...
        )
        {
            ...
            if (
                localSymbol.Scope == DeclarationScope.ValueScoped
            ...
            if (initializerOpt != null)
            {
                if (UseUpdatedEscapeRules && localSymbol.Scope != DeclarationScope.Unscoped)
                {
                    ...
                    Debug.Assert(
                        localSymbol.RefKind == RefKind.None
                            || localSymbol.RefEscapeScope
                                >= GetRefEscape(initializerOpt, LocalScopeDepth)
                    );

                    if (declTypeOpt.Type.IsRefLikeType)
                    {
                        initializerOpt = ValidateEscape(
                            initializerOpt,
                            localSymbol.ValEscapeScope,
                            isByRef: false,
                            diagnostics
                        );
                    }
                ...
                if (verifyEscapeSafety)
                {
                    if (isRef)
                    {
                        ...
                        if (leftEscape < rightEscape)
                        {
                            ...
                            {
                                (Binder.ReturnOnlyScope, false)
                                    => ErrorCode.ERR_RefAssignReturnOnly,
...
#nullable disable
                ...
                if (conversion.Kind == ConversionKind.Identity)
                {
                    ...
                    if (expr.Kind == BoundKind.AssignmentOperator)
                    {
                        ...
                        if (
                            ...
                            && assignment.Right.ConstantValue.Discriminator
        ...
        )
        {
            ...
            else if (IsInAsyncMethod())
            {
                ...
                (object)retType != null && (refKind != RefKind.None) != (sigRefKind != RefKind.None)
            )
            {
                var errorCode =
                    refKind != RefKind.None
        ...
        )
        {
            ...
            )
            {
                ...
                expression = bodyBinder.ValidateEscape(
                    expression,
                    Binder.ReturnOnlyScope,
                    refKind != RefKind.None,
                    diagnostics
        ...
        )
        {
            ...
            return GetBinder(compilationUnit)
                .BindSimpleProgramCompilationUnit(compilationUnit, simpleProgram, diagnostics);
        }
        ...
        private BoundNode BindRecordConstructorBody(
        ...
        )
        {
            ...
            if (
                recordDecl.PrimaryConstructorBaseTypeIfClass
            ...
            {
                initializer = bodyBinder.BindImplicitConstructorInitializer(
                    recordDecl,
                    diagnostics
                );
            }
        ...
        )
        {
            ...
            if (!thisInitializer && hasRecordPrimaryConstructor())
            {
                if (
