    ...
    internal partial class Binder
    {
        ...
        )
        {
            ...
            BoundExpression left = BindValue(
                node.Left,
                diagnostics,
                GetBinaryAssignmentKind(node.Kind())
            );
            ...
            if (left.Kind == BoundKind.EventAccess)
            {
                ...
                switch (kindOperator)
                {
                    ...
                    // fall-through for other operators, if RHS is dynamic we produce dynamic operation, otherwise we'll
                    // report an error ...
                }
            }
            ...
            {
                if (
                    IsLegalDynamicOperand(right)
                    && IsLegalDynamicOperand(left)
                    && kind != BinaryOperatorKind.UnsignedRightShift
                )
                {
            ...
            // We want to have the converted (bound) lambda in the bound tree, not the unconverted unbound
            // lambda.

            LookupResultKind resultKind;
            ImmutableArray<MethodSymbol> originalUserDefinedOperators;
            BinaryOperatorAnalysisResult best = this.BinaryOperatorOverloadResolution(
                kind,
                isChecked: CheckOverflowAtRuntime,
                left,
                right,
            ...
            CheckConstraintLanguageVersionAndRuntimeSupportForOperator(
                ...
                isUnsignedRightShift: bestSignature.Kind.Operator()
            ...
            BoundExpression rightConverted = CreateConversion(
                right,
            ...
            BoundExpression finalConversion = GenerateConversionForAssignment(
                leftType,
                finalPlaceholder,
                diagnostics,
            ...
            {
                Debug.Assert(
                    finalConversion.HasErrors || (object)finalConversion == finalPlaceholder
                );
            ...
            var leftPlaceholder = new BoundValuePlaceholder(
                left.Syntax,
                leftType
            ).MakeCompilerGenerated();
        ...
        {
            Debug.Assert(
                opKind == BinaryOperatorKind.Addition || opKind == BinaryOperatorKind.Subtraction
            );
            ...
            BoundExpression argument = CreateConversion(
                right,
                argumentConversion,
                delegateType,
                diagnostics
            ...
            if ((object)method == null)
            {
                ...
                if (!eventSymbol.OriginalDefinition.IsFromCompilation(this.Compilation))
                {
                    ...
                    Error(
                        ...
                        node,
                        delegateType,
                        SourceEventSymbol.GetAccessorName(eventSymbol.Name, isAddition)
                    );
                }
            }
            else
            {
                ...
                if (
                    !this.IsAccessible(
                        method,
                        ref useSiteInfo,
                        this.GetAccessThroughType(receiverOpt)
                    )
                )
                {
                    // CONSIDER: depending on the accessibility (e.g. if it's private), dev10 might just report the
                    // whole event bogus.
        ...
        )
        {
            ...
            {
                // We need to make sure left is either implicitly convertible to Boolean or has user defined truth
                // operator.
                ...
                if (
                    !IsValidDynamicCondition(
                        left,
                        isNegative: kind == BinaryOperatorKind.LogicalAnd,
                        diagnostics,
                        userDefinedOperator: out userDefinedOperator
                    )
                )
                {
                    // Dev11 reports ERR_MustHaveOpTF. The error was shared between this case and user-defined binary
                    // Boolean operators.
                    // We report two distinct more specific error messages.
                    Error(
                        diagnostics,
                        ErrorCode.ERR_InvalidDynamicCondition,
                        node.Left,
                        left.Type,
        ...
        )
        {
            ...
            while (syntaxNodes.Count > 0)
            {
                ...
                BoundExpression right = BindValue(
                    syntaxNode.Right,
                    diagnostics,
                    BindValueKind.RValue
                );
                BoundExpression boundOp = BindSimpleBinaryOperator(
                    syntaxNode,
                    diagnostics,
                    left,
                    right,
                    leaveUnconvertedIfInterpolatedString: true
                );
                result = boundOp;
            }
        ...
        private BoundExpression BindSimpleBinaryOperator(
            ...
            BoundExpression left,
            BoundExpression right,
            bool leaveUnconvertedIfInterpolatedString
        )
        {
            BinaryOperatorKind kind = SyntaxKindToBinaryOperatorKind(node.Kind());
            ...
            if (left.HasAnyErrors || right.HasAnyErrors)
            {
                ...
                return new BoundBinaryOperator(
                    ...
                    left,
                    right,
                    GetBinaryOperatorErrorType(kind, diagnostics, node),
                    true
                );
            }
            ...
            if (isEquality && leftNull && rightNull)
            {
                return new BoundLiteral(
                    node,
                    ConstantValue.Create(kind == BinaryOperatorKind.Equal),
                    GetSpecialType(SpecialType.System_Boolean, diagnostics, node)
                );
            }

            if (
                IsTupleBinaryOperation(left, right)
                && (kind == BinaryOperatorKind.Equal || kind == BinaryOperatorKind.NotEqual)
            )
            {
            ...
            if (
                leaveUnconvertedIfInterpolatedString
            ...
            )
            {
                ...
                var stringConstant = FoldBinaryOperator(
                    node,
            ...
            switch (node.Kind())
            {
                ...
                case SyntaxKind.GreaterThanExpression:
                case SyntaxKind.GreaterThanOrEqualExpression:
                    // Function pointer comparisons are defined on `void*` with implicit conversions to `void*` on both
                    // sides. So if this is a
                    // pointer comparison operation, and the underlying types of the left and right are both function
                    // pointers, then we need to
                    // warn about them because of JIT recompilation. If either side is explicitly cast to void*, that
                    // side's type will be void*,
                    // not delegate*, and we won't warn.
                    if (
                        (resultOperatorKind & BinaryOperatorKind.Pointer)
                            == BinaryOperatorKind.Pointer
                        && leftType?.TypeKind == TypeKind.FunctionPointer
                        && rightType?.TypeKind == TypeKind.FunctionPointer
                    )
                    {
                        // Comparison of function pointers might yield an unexpected result, since pointers to the same
                        // function may be distinct.
                        Error(
                            diagnostics,
            ...
            if (foundOperator)
            {
                ...
                CheckConstraintLanguageVersionAndRuntimeSupportForOperator(
                    node,
                    signature.Method,
                    isUnsignedRightShift: resultOperatorKind.Operator()
                        == BinaryOperatorKind.UnsignedRightShift,
            ...
            if (
                foundOperator
                && (resultOperatorKind.OperandTypes() != BinaryOperatorKind.NullableNull)
            )
            {
        ...
        {
            bool foundOperator;
            best = this.BinaryOperatorOverloadResolution(
                kind,
                isChecked: CheckOverflowAtRuntime,
                left,
                right,
            ...
            else
            {
                ...
                bool isObjectEquality =
                    signature.Kind == BinaryOperatorKind.ObjectEqual
                    || signature.Kind == BinaryOperatorKind.ObjectNotEqual;
                ...
                else
                {
                    ...
                    foundOperator =
                        !isObjectEquality
                        || BuiltInOperators.IsValidObjectEquality(
                            Conversions,
                            leftType,
                            leftNull,
                            leftDefault,
...
#nullable enable
        private BoundExpression RebindSimpleBinaryOperatorAsConverted(
            BoundBinaryOperator unconvertedBinaryOperator,
            BindingDiagnosticBag diagnostics
        )
        {
            if (
                TryBindUnconvertedBinaryOperatorToDefaultInterpolatedStringHandler(
                    unconvertedBinaryOperator,
                    diagnostics,
                    out var convertedBinaryOperator
                )
            )
            {
            ...
            BoundExpression doRebind(BindingDiagnosticBag diagnostics, BoundBinaryOperator? current)
            {
                ...
                Debug.Assert(
                    stack.Count > 0 && stack.Peek().Left is BoundUnconvertedInterpolatedString
                );
...
#nullable disable
        ...
        )
        {
            ...
            {
                // We'll have reported an error for not being able to target-type `default` so we can avoid a
        ...
        private void ReportAssignmentOperatorError(
            AssignmentExpressionSyntax node,
            BinaryOperatorKind kind,
            BindingDiagnosticBag diagnostics,
            BoundExpression left,
        ...
        {
            if (
                IsTypelessExpressionAllowedInBinaryOperator(kind, left, right)
                && node.OperatorToken.RawKind
                    is (int)SyntaxKind.PlusEqualsToken
                        or (int)SyntaxKind.MinusEqualsToken
                && (object)left.Type != null
                && left.Type.TypeKind == TypeKind.Delegate
            )
            {
        ...
        )
        {
            ...
            switch (left.Kind, right.Kind)
            {
                ...
                case (BoundKind.DefaultLiteral, _) when right.Type is TypeParameterSymbol:
                    ...
                    Error(
                        diagnostics,
                        ErrorCode.ERR_AmbigBinaryOpsOnUnconstrainedDefault,
                        node,
                        operatorToken.Text,
                        right.Type
                    );
...
#nullable enable
            ...
            switch (resultKind)
            {
                ...
                case LookupResultKind.OverloadResolutionFailure
                    when operatorToken.Kind() is SyntaxKind.PlusToken
                        && isReadOnlySpanOfByte(left.Type)
                        && isReadOnlySpanOfByte(right.Type):
...
#nullable disable
        ...
        )
        {
            ...
            while (true)
            {
                ...
                if (
                    childAsBinary == null
                    || (
                        childAsBinary.Kind() != SyntaxKind.LogicalOrExpression
                        && childAsBinary.Kind() != SyntaxKind.LogicalAndExpression
                    )
                )
                {
                    break;
        ...
        )
        {
            ...
            Debug.Assert(
                kind == BinaryOperatorKind.LogicalAnd || kind == BinaryOperatorKind.LogicalOr
            );
            ...
            if (
                ...
                && (object)right.Type != null
                && right.Type.SpecialType == SpecialType.System_Boolean
            )
            {
                var constantValue = FoldBinaryOperator(
                    node,
            ...
            if (left.HasAnyErrors || right.HasAnyErrors)
            {
                ...
                return new BoundBinaryOperator(
                    ...
                    left,
                    right,
                    type: GetBinaryOperatorErrorType(kind, diagnostics, node),
                    hasErrors: true
                );
            }
            ...
            ImmutableArray<MethodSymbol> originalUserDefinedOperators;
            var best = this.BinaryOperatorOverloadResolution(
                kind,
                isChecked: CheckOverflowAtRuntime,
                left,
                right,
            ...
            else
            {
                ...
                bool bothBool =
                    signature.LeftType.SpecialType == SpecialType.System_Boolean
                ...
                if (!bothBool && !signature.Kind.IsUserDefined())
                {
                    ...
                    bothBool
                    || IsValidUserDefinedConditionalLogicalOperator(
                        node,
                        signature,
                        diagnostics,
                        out trueOperator,
                ...
                {
                    var resultLeft = CreateConversion(
                        left,
                        best.LeftConversion,
                        signature.LeftType,
                        diagnostics
                    );
                    var resultRight = CreateConversion(
                        right,
                        best.RightConversion,
                        signature.RightType,
                        diagnostics
        ...
        private bool IsValidDynamicCondition(
            BoundExpression left,
            bool isNegative,
            BindingDiagnosticBag diagnostics,
            out MethodSymbol userDefinedOperator
        )
        {
            ...
            var implicitConversion = Conversions.ClassifyImplicitConversionFromExpression(
                left,
                booleanType,
                ref useSiteInfo
            );
            ...
            if (implicitConversion.Exists)
            {
                ...
                {
                    var operandPlaceholder = new BoundValuePlaceholder(
                        left.Syntax,
                        left.Type
                    ).MakeCompilerGenerated();
                    CreateConversion(
        ...
        )
        {
            ...
            bool typeMatchesContainer =
                TypeSymbol.Equals(
                    signature.ReturnType.StrippedType(),
                    t,
                    TypeCompareKind.ConsiderEverything2
                )
                || (
        ...
        private bool HasApplicableBooleanOperator(
            NamedTypeSymbol containingType,
            string name,
            TypeSymbol argumentType,
            ref CompoundUseSiteInfo<AssemblySymbol> useSiteInfo,
            out MethodSymbol @operator
        )
        {
            for (
        ...
        private TypeSymbol GetBinaryOperatorErrorType(
            BinaryOperatorKind kind,
            BindingDiagnosticBag diagnostics,
            CSharpSyntaxNode node
        )
        ...
        private void ReportObsoleteAndFeatureAvailabilityDiagnostics(
        ...
        )
        {
            if ((object)operatorMethod != null)
            {
                ...
                if (
                    operatorMethod.ContainingType.IsInterface
        ...
        private bool IsTypelessExpressionAllowedInBinaryOperator(
            BinaryOperatorKind kind,
            BoundExpression left,
            BoundExpression right
        )
        {
            // The default literal is only allowed with equality operators and both operands cannot be typeless
            // at the same time.
            // Note: we only need to restrict expressions that can be converted to *any* type, in which case the
            // resolution could always succeed.
        ...
        )
        {
            ...
            {
                return type.SpecialType == SpecialType.System_UIntPtr && type.IsNativeIntegerType;
            }
        }

        private static object FoldDecimalBinaryOperators(
            BinaryOperatorKind kind,
            ConstantValue valueLeft,
            ConstantValue valueRight
        )
        {
        ...
        private static object FoldNativeIntegerOverflowingBinaryOperator(
            BinaryOperatorKind kind,
            ConstantValue valueLeft,
            ConstantValue valueRight
        )
        ...
        private static object FoldUncheckedIntegralBinaryOperator(
            BinaryOperatorKind kind,
            ConstantValue valueLeft,
            ConstantValue valueRight
        )
        ...
        private static object FoldCheckedIntegralBinaryOperator(
            BinaryOperatorKind kind,
            ConstantValue valueLeft,
            ConstantValue valueRight
        )
        ...
        internal static TypeSymbol GetEnumType(
            BinaryOperatorKind kind,
            BoundExpression left,
            BoundExpression right
        )
...
#nullable enable
        ...
        )
        {
            ...
            TypeSymbol operandType =
                (operandSpecialType == underlyingType.SpecialType)
                    ? underlyingType
            ...
            if (
                resultTypeSymbol.SpecialType != SpecialType.System_Boolean
                && constantValue != null
                && !constantValue.IsBad
            )
            {
                TypeSymbol resultType =
                    kind == BinaryOperatorKind.EnumSubtraction ? underlyingType : enumType;
        ...
        )
        {
            ...
            //
            // Although remainder and division always overflow at runtime with arguments
            // int.MinValue/long.MinValue and -1
        ...
        /// </summary>
        /// <param name="kind">The operator kind.  Nothing will happen if it is not a lifted equality
        // operator.</param>
        /// <param name="left">The left-hand operand of the operation (possibly wrapped in a
        // conversion).</param>
        /// <param name="right">The right-hand operand of the operation (possibly wrapped in a
        // conversion).</param>
        /// <returns>
        /// If the operator represents lifted equality, then constant value true if both arguments have
        // constant
        /// value null, constant value false if exactly one argument has constant value null, and null
        // otherwise.
        /// If the operator represents lifted inequality, then constant value false if both arguments have
        // constant
        /// value null, constant value true if exactly one argument has constant value null, and null
        // otherwise.
        /// </returns>
        /// <remarks>
        ...
        /// </remarks>
        private static ConstantValue? TryFoldingNullableEquality(
            BinaryOperatorKind kind,
            BoundExpression left,
            BoundExpression right
        )
        ...
        // Some binary operators on constants never overflow, regardless of whether the context is checked
        // or not.
        private static object? FoldNeverOverflowBinaryOperators(
            BinaryOperatorKind kind,
            ConstantValue valueLeft,
            ConstantValue valueRight
        )
        ...
        /// <summary>
        /// Returns ConstantValue.Bad if, and only if, the resulting string length exceeds <see
        // cref="int.MaxValue"/>.
        /// </summary>
        private static ConstantValue? FoldStringConcatenation(
            BinaryOperatorKind kind,
            ConstantValue valueLeft,
            ConstantValue valueRight
        )
...
#nullable disable
        ...
        )
        {
            ...
            BoundExpression operand = BindToNaturalType(
                BindValue(operandSyntax, diagnostics, BindValueKind.IncrementDecrement),
                diagnostics
            );
            ...
            var resultPlaceholder = new BoundValuePlaceholder(
                node,
                signature.ReturnType
            ).MakeCompilerGenerated();

            BoundExpression resultConversion = GenerateConversionForAssignment(
                operandType,
                resultPlaceholder,
                diagnostics,
...
#nullable enable
        ...
        /// </summary>
        private bool CheckConstraintLanguageVersionAndRuntimeSupportForOperator(
            SyntaxNode node,
            MethodSymbol? methodOpt,
            bool isUnsignedRightShift,
        ...
        )
        {
            ...
            if (methodOpt?.ContainingType?.IsInterface == true && methodOpt.IsStatic)
            {
                if (methodOpt.IsAbstract || methodOpt.IsVirtual)
                {
                    ...
                    methodOpt.Name
                    is WellKnownMemberNames.EqualityOperatorName
            ...
            {
                Debug.Assert(
                    (methodOpt.Name == WellKnownMemberNames.UnsignedRightShiftOperatorName)
...
#nullable disable
        ...
        )
        {
            ...
            switch (expr.Kind)
            {
                ...
                default:
                    ...
                    {
                        Debug.Assert(
                            node.Operand.SkipParens().GetLastToken().Kind()
        ...
        {
            BoundExpression operand = BindToNaturalType(
                BindValue(node.Operand, diagnostics, GetUnaryAssignmentKind(node.Kind())),
                diagnostics
            );
        ...
        {
            BoundExpression operand = BindToNaturalType(
                BindValue(node.Operand, diagnostics, BindValueKind.AddressOf),
                diagnostics
            );
            ...
            Debug.Assert(
                ...
                "BindValue should have caught a null operand type"
            );
            ...
            {
                if (
                    IsMoveableVariable(operand, accessedLocalOrParameterOpt: out _)
        ...
        /// </summary>
        internal bool IsMoveableVariable(
            BoundExpression expr,
            out Symbol accessedLocalOrParameterOpt
        )
        {
                ...
                switch (exprKind)
                {
                    ...
                    case BoundKind.EventAccess:
                    {
                        ...
                        else
                        {
                            ...
                            if (
                                !eventAccess.IsUsableAsField
                                || eventAccess.EventSymbol.IsWindowsRuntimeEvent
                            )
                            {
                                return true;
                            }
                        ...
                        bool receiverIsLValue = CheckValueKind(
                            receiver.Syntax,
                            receiver,
                    ...
                    case BoundKind.InlineArrayAccess:
                    {
                        ...
                        if (
                            elementAccess.GetItemOrSliceHelper
        ...
        )
        {
            ...
            return constant
                ?? BindUnaryOperatorCore(node, node.OperatorToken.Text, operand, diagnostics);
        }

        private void ReportSuppressionIfNeeded(
            BoundExpression expr,
            BindingDiagnosticBag diagnostics
        )
        {
...
#nullable enable
        private BoundExpression BindUnaryOperatorCore(
            CSharpSyntaxNode node,
            string operatorText,
            BoundExpression operand,
            BindingDiagnosticBag diagnostics
        )
        {
            ...
            var resultConstant = FoldUnaryOperator(
                node,
                resultOperatorKind,
                resultOperand,
                resultType,
        ...
        )
        {
            ...
            var constantValue = FoldUnaryOperator(
                syntax,
                newKind,
                operand,
                upconvertType,
            ...
            if (constantValue != null && !constantValue.IsBad)
            {
                ...
                var binder =
                    kind.Operator() == UnaryOperatorKind.BitwiseComplement
                        ? this.WithCheckedOrUncheckedRegion(@checked: false)
                        : this;
                return binder.FoldConstantNumericConversion(
                    syntax,
                    constantValue,
                    underlyingType,
                    diagnostics
...
#nullable disable
        ...
        )
        {
            ...
            // SPEC: When a decimal-integer-literal with the value 9223372036854775808 and no
            // integer-type-suffix
            ...
            if (
                node.Operand != operand.Syntax
                || operand.Syntax.Kind() != SyntaxKind.NumericLiteralExpression
            )
            {
                return null;
            }
            ...
            if (token.Value is uint)
            {
                ...
                if (
                    token.Text.Contains("u")
                    || token.Text.Contains("U")
                    || token.Text.Contains("l")
                    || token.Text.Contains("L")
                )
                {
                    return null;
                ...
                return new BoundLiteral(
                    node,
                    ConstantValue.Create((int)-2147483648),
            ...
            else if (token.Value is ulong)
            {
                ...
                return new BoundLiteral(
                    node,
                    ConstantValue.Create(-9223372036854775808),
                    GetSpecialType(SpecialType.System_Int64, diagnostics, node)
                );
            }
        ...
        private bool IsOperandErrors(
            CSharpSyntaxNode node,
            ref BoundExpression operand,
            BindingDiagnosticBag diagnostics
        )
        {
            switch (operand.Kind)
                    ...
                    if ((object)operand.Type == null && !operand.IsLiteralNull())
                    {
                        if (!operand.HasAnyErrors)
                        {
                            ...
                            Error(
                                ...
                                node,
                                SyntaxFacts.GetText(SyntaxKind.IsKeyword),
                                operand.Display
                            );
                        }
        ...
        private bool IsOperatorErrors(
            CSharpSyntaxNode node,
            TypeSymbol operandType,
            BoundTypeExpression typeExpression,
            BindingDiagnosticBag diagnostics
        )
        {
            ...
            if (
                (object)operandType != null && operandType.IsPointerOrFunctionPointer()
        ...
        private BoundExpression BindIsOperator(
            BinaryExpressionSyntax node,
            BindingDiagnosticBag diagnostics
        )
        {
            ...
            if (
                !tryBindAsType(
                    node.Right,
                    diagnostics,
                    out BindingDiagnosticBag isTypeDiagnostics,
            ...
            )
            {
                ...
                var convertedExpression = BindExpressionForPattern(
                    operand.Type,
                    node.Right,
                    ref hasErrors,
                    isPatternDiagnostics,
                    out var constantValueOpt,
                ...
                if (wasExpression)
                {
                    ...
                    var boundConstantPattern = new BoundConstantPattern(
                        ...
                        operand.Type,
                        convertedExpression.Type ?? operand.Type,
                        hasErrors
                    )
...
#pragma warning restore format
            ...
            if (
                operandHasErrors
                || IsOperatorErrors(node, operand.Type, typeExpression, diagnostics)
            )
            {
            ...
            {
                diagnostics.Add(
                    ErrorCode.WRN_IsTypeNamedUnderscore,
                    node.Right.Location,
                    typeExpression.AliasOpt ?? (Symbol)targetType
                );
            }
            ...
            if (
                operand.ConstantValueOpt == ConstantValue.Null
            ...
            )
            {
                ...
                Conversion conv = Conversions.ClassifyConversionFromExpression(
                    operand,
                    targetType,
                    isChecked: CheckOverflowAtRuntime,
                    ref useSiteInfo
            ...
            Conversion conversion = Conversions.ClassifyBuiltInConversion(
                operandType,
                targetType,
                isChecked: CheckOverflowAtRuntime,
                ref useSiteInfo
            ...
            {
                bindAsTypeDiagnostics = BindingDiagnosticBag.GetInstance(
                    withDiagnostics: true,
                    withDependencies: diagnostics.AccumulatesDependencies
                );
                TypeWithAnnotations targetTypeWithAnnotations = BindType(
                    possibleType,
                    bindAsTypeDiagnostics,
                ...
                return !(
                    targetType?.IsErrorType() == true
                    && bindAsTypeDiagnostics.HasAnyResolvedErrors()
                );
            }
        }
        ...
        )
        {
            ...
            ConstantValue constantValue = GetIsOperatorConstantResult(
                operandType,
                targetType,
                conversionKind,
        ...
        ///  - <see cref="ConstantValue.Bad"/> - compiler doesn't support the type check, i.e. cannot
        // perform it, even at runtime
        ...
        )
        {
            ...
            // SPEC:    1)      If T is a reference type, the result is true if D and T are the same type, if D
            // is a reference type and
            // SPEC:        an implicit reference conversion from D to T exists, or if D is a value type and a
            // boxing conversion from D to T exists.
            // SPEC:    2)      If T is a nullable type, the result is true if D is the underlying type of T.
            // SPEC:    3)      If T is a non-nullable value type, the result is true if D and T are the same
            // type.
            ...
            // NOTE:    However, we are interested in computing the compile time constant value for the
            // expression.
            // NOTE:    Even though BoundIsOperator and BoundAsOperator will always have no ConstantValue
            // NOTE:    (they are non-constant expressions according to Section 7.19 of the specification),
            // NOTE:    we want to perform constant analysis of is/as expressions during binding to generate
            // warnings
            // NOTE:    (always true/false/null) and during rewriting for optimized codegen.
            // NOTE:
            // NOTE:    Because the heuristic presented here is used to change codegen, it must be conservative.
            // It is acceptable
            // NOTE:    for us to fail to report a warning in cases where humans could logically deduce that the
            // operator will
            // NOTE:    always return false. It is not acceptable to inaccurately warn that the operator will
            // always return false
            // NOTE:    if there are cases where it might succeed.
            // NOTE:
            // NOTE:    These same heuristics are also used in pattern-matching to determine if an expression of
            // the form
            // NOTE:    `e is T x` is permitted. It is an error if `e` cannot be of type `T` according to this
            // method
            // NOTE:    returning ConstantValue.False.
            // NOTE:    The heuristics are also used to determine if a `case T1 x1:` is subsumed by
            // NOTE:    some previous `case T2 x2:` in a switch statement. For that purpose operandType is T1,
            // targetType is T2,
            // NOTE:    and operandCouldBeNull is false; the former subsumes the latter if this method returns
            // ConstantValue.True.
            // NOTE:    Since the heuristic is now used to produce errors in pattern-matching, making it more
            // accurate in the
            ...
            switch (conversionKind)
            {
                ...
                case ConversionKind.ImplicitTuple:
                    ...
                    //
                    // If we have "x is T" and the conversion from x to T is numeric or enum then the result must be
                    // false.
                    //
                    // If we have "null is T" then obviously that must be false.
                    //
                    // If we have "1 is long" then that must be false. (If we have "1 is int" then it is an identity
                    // conversion,
                ...
                case ConversionKind.ExplicitNullable:
                    ...
                    if (
                        Conversions.HasIdentityConversion(
                            operandType.GetNullableUnderlyingType(),
                            targetType
                        )
                    )
                    {
        ...
        private BoundExpression BindAsOperator(
            BinaryExpressionSyntax node,
            BindingDiagnosticBag diagnostics
        )
        {
            ...
            else if (!targetType.IsReferenceType && !targetType.IsNullableType())
            {
                ...
                if (targetTypeKind == TypeKind.TypeParameter)
                {
                    ...
                    targetTypeKind == TypeKind.Pointer
                    || targetTypeKind == TypeKind.FunctionPointer
                )
                {
                    Error(diagnostics, ErrorCode.ERR_PointerInAsOrIs, node);
                }
            ...
            if (operand.IsLiteralNull())
            {
                ...
                operandPlaceholder = new BoundValuePlaceholder(
                    operand.Syntax,
                    operand.Type
                ).MakeCompilerGenerated();
            ...
            diagnostics.Add(node, useSiteInfo);
            bool hasErrors = ReportAsOperatorConversionDiagnostics(
                node,
                diagnostics,
                this.Compilation,
                operandType,
        ...
        {
            // SPEC:    In an operation of the form E as T, E must be an expression and T must be a reference
            // type,
            ...
            // SPEC VIOLATION:  See
            // Microsoft.CodeAnalysis.CSharp.UnitTests.SyntaxBinderTests.TestAsOperator_SpecErrorCase() test for an
            // example.

            // SPEC VIOLATION:  The specification also unintentionally allows the case where requirement 2
            // above:
            ...
            switch (conversionKind)
            {
                ...
                default:
                    ...
                    // and the targetType are closed types OR operandType is void type, otherwise we need a runtime
                    // check
                    if (
                        !operandType.ContainsTypeParameter() && !targetType.ContainsTypeParameter()
        ...
        )
        {
            ...
            ConstantValue constantValue = GetAsOperatorConstantResult(
                operandType,
                targetType,
                conversionKind,
                operandConstantValue
        ...
        )
        {
            ...
            // NOTE:    (they are non-constant expressions according to Section 7.19 of the specification),
            // NOTE:    we want to perform constant analysis of is/as expressions during binding to generate
            // warnings (always true/false/null)
            // NOTE:    and during rewriting for optimized codegen.

            ConstantValue isOperatorConstantResult = GetIsOperatorConstantResult(
                operandType,
                targetType,
                conversionKind,
                operandConstantValue
        ...
        private BoundExpression BindNullCoalescingOperator(
            BinaryExpressionSyntax node,
            BindingDiagnosticBag diagnostics
        )
        {
            ...
            // SPEC: between the types of the operands. In order of preference, the type of a ?? b is A0, A, or
            // B,
            ...
            // SPEC: The left hand side must be either the null literal or it must have a type. Lambdas and
            // method groups do not have a type,
            ...
            {
                return GenerateNullCoalescingBadBinaryOpsError(
                    node,
                    leftOperand,
                    rightOperand,
                    diagnostics
            ...
            // SPEC: Otherwise, if A exists and is a non-nullable value type, a compile-time error occurs. First
            // we check for the pre-C# 8.0
            ...
            {
                // Prior to C# 8.0, the spec said that the left type must be either a reference type or a nullable
                // value type. This was relaxed
                // with C# 8.0, so if the feature is not enabled then issue a diagnostic and return
                if (!optLeftType.IsValueType)
                {
                    CheckFeatureAvailability(
                        node,
                        MessageID.IDS_FeatureUnconstrainedTypeParameterInNullCoalescingOperator,
                        diagnostics
                    );
                }
                else
            ...
            //
            // Note that there is no runtime dynamic dispatch since comparison with null is not a dynamic
            // operation.
            CompoundUseSiteInfo<AssemblySymbol> useSiteInfo = GetNewCompoundUseSiteInfo(
                diagnostics
            );
            ...
            // SPEC:    Otherwise, if A exists and is a nullable type and an implicit conversion exists from b
            // to A0,
            ...
            if (isLeftNullable)
            {
                var rightConversion = Conversions.ClassifyImplicitConversionFromExpression(
                    rightOperand,
                    optLeftType0,
                    ref useSiteInfo
                );
                if (rightConversion.Exists)
                {
                    ...
                    // Note: we use an identity conversion for LHS and let lowering get 'a0' from 'a' with
                    // GetValueOrDefault
                    return new BoundNullCoalescingOperator(
                        node,
                        leftOperand,
            ...
            // SPEC:    Otherwise, if A exists and an implicit conversion exists from b to A, the result type is
            // A.
            ...
            // SPEC VIOLATION:  Native compiler violates the specification here and implements this part based
            // on
            ...
            if ((object)optRightType != null)
            {
                ...
                if (isLeftNullable)
                {
                    ...
                    // We just store the second conversion in the bound node and insert the first conversion during
                    // rewriting
                    // the null coalescing operator. See method LocalRewriter.GetConvertedLeftForNullCoalescingOperator.

                    leftConversionClassification = Conversions.ClassifyImplicitConversionFromType(
                        optLeftType0,
                        optRightType,
                        ref useSiteInfo
                    );
        ...
        private BoundExpression BindNullCoalescingAssignmentOperator(
            AssignmentExpressionSyntax node,
            BindingDiagnosticBag diagnostics
        )
        {
            ...
            // Given a ??= b, the type of a is A, the type of B is b, and if A is a nullable value type, the
            // underlying
            ...
            // If A0 exists and B is implicitly convertible to A0, then the result type of this expression is
            // A0, except if B is dynamic.
            // This differs from most assignments such that you cannot directly replace a with (a ??= b).
            // The exception for dynamic is called out in the spec, it's the same behavior that ?? has with
            // respect to dynamic.
            ...
            // If an implicit conversion exists from B to A, we store that conversion. At runtime, a is first
            // evaluated. If
            // a is not null, b is not evaluated. If a is null, b is evaluated and converted to type A, and is
            // stored in a.
            // Reset useSiteDiagnostics because they could have been used populated incorrectly from attempting
            // to bind
        ...
        ///
        /// The v2.0 specification states that the types of the second and third operands T and S of a
        ...
        )
        {
            ...
            if (!isRef)
            {
                ...
                {
                    diagnostics.Add(
                        ErrorCode.ERR_RefConditionalNeedsTwoRefs,
                        whenFalse.GetFirstToken().GetLocation()
                    );
                }
                ...
                if (whenTrueRefKind == RefKind.Ref)
                {
                    diagnostics.Add(
                        ErrorCode.ERR_RefConditionalNeedsTwoRefs,
                        whenTrue.GetFirstToken().GetLocation()
                    );
                }
            }
            else
...
#nullable enable
        ...
        )
        {
            ...
            TypeSymbol? bestType = BestTypeInferrer.InferBestTypeForConditionalOperator(
                trueExpr,
                falseExpr,
                this.Conversions,
            ...
            if (bestType is null)
            {
                ErrorCode noCommonTypeError = hadMultipleCandidates
...
#nullable disable
        ...
        {
            BoundExpression condition = BindBooleanExpression(node.Condition, diagnostics);
            BoundExpression trueExpr = BindValue(
                whenTrue,
                diagnostics,
                BindValueKind.RValue | BindValueKind.RefersToLocation
            );
            BoundExpression falseExpr = BindValue(
                whenFalse,
                diagnostics,
                BindValueKind.RValue | BindValueKind.RefersToLocation
            );
            ...
            if (!Conversions.HasIdentityConversion(trueType, falseType))
            {
                if (!hasErrors)
                    diagnostics.Add(
                        ErrorCode.ERR_RefConditionalDifferentTypes,
                        falseExpr.Syntax.Location,
                        trueType
                    );
            ...
            else
            {
                ...
                type = BestTypeInferrer.InferBestTypeForConditionalOperator(
                    trueExpr,
                    falseExpr,
                    this.Conversions,
                    hadMultipleCandidates: out _,
                    ref useSiteInfo
                );
                diagnostics.Add(node, useSiteInfo);
    ...
    partial class RefSafetyAnalysis
    {
        ...
        )
        {
            ...
            {
                // ask the one with narrower escape, for the wider - hopefully the errors will make the violation
                // easier to fix.
                if (whenTrueEscape < whenFalseEscape)
                    CheckValEscape(
                        falseExpr.Syntax,
    ...
    partial class Binder
    {
        ...
        /// </summary>
        private static ConstantValue FoldConditionalOperator(
