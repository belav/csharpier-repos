    ...
    partial class Binder
    {
        internal abstract class IdentifierUsedAsValueFinder
        {
            ...
            protected bool CheckIdentifiersInNode(CSharpSyntaxNode? node, Binder binder)
            {
                ...
                var nodesOfInterest = node.DescendantNodesAndSelf(
                    descendIntoChildren: childrenNeedChecking,
                    descendIntoTrivia: false
                );

                foreach (var n in nodesOfInterest)
                {
                    Binder enclosingBinder = getEnclosingBinderForNode(
                        contextNode: node,
                ...
                static Binder getEnclosingBinderForNode(
            ...
            {
                UnboundLambda unboundLambda = enclosingBinder.AnalyzeAnonymousFunction(
                    lambdaSyntax,
                    BindingDiagnosticBag.Discarded
                );
                var lambdaBodyBinder = CreateLambdaBodyBinder(enclosingBinder, unboundLambda);
                return CheckIdentifiersInNode(
                    lambdaSyntax.Body,
                    lambdaBodyBinder.GetBinder(lambdaSyntax.Body) ?? lambdaBodyBinder
                );
            }

            private static ExecutableCodeBinder CreateLambdaBodyBinder(
                Binder enclosingBinder,
                UnboundLambda unboundLambda
            )
            {
                unboundLambda.HasExplicitReturnType(
                    out RefKind refKind,
                ...
                return new ExecutableCodeBinder(
                    ...
                    lambdaSymbol,
                    unboundLambda.GetWithParametersBinder(lambdaSymbol, enclosingBinder)
                );
            }
            ...
            private bool CheckIdentifier(Binder enclosingBinder, IdentifierNameSyntax id)
            {
                ...
                enclosingBinder.LookupIdentifier(
                    _lookupResult,
                    id,
                    SyntaxFacts.IsInvoked(id),
                    ref useSiteInfo
                );
            ...
            protected abstract bool CheckAndClearLookupResult(
                Binder enclosingBinder,
            ...
            )
            {
                ...
                switch (id.Parent)
                {
                    ...
                    case QualifiedNameSyntax qualifiedName when qualifiedName.Left == id:
                        ...
                        invoked = false;
                        break;
                    case FromClauseSyntax { Parent: QueryExpressionSyntax query } fromClause
                        when query.FromClause == fromClause && fromClause.Expression == id:
                ...
                return memberAccessNode is not null
                    && enclosingBinder.IsPotentialColorColorReceiver(id, type);
            }

            /// <summary>
            /// Follows the logic of <see cref="Binder.BindInstanceMemberAccess"/> and <see
            // cref="Binder.BindMemberOfType"/>
            /// </summary>
            ...
            )
            {
                ...
                var useSiteInfo = CompoundUseSiteInfo<AssemblySymbol>.Discarded;
                enclosingBinder.LookupInstanceMember(
                    lookupResult,
                    type,
                    leftIsBaseReference: false,
                    memberName,
                    targetMemberArity,
                    invoked,
                    ref useSiteInfo
                ...
                if (lookupResult.IsMultiViable)
                {
                    ...
                    Symbol symbol = enclosingBinder.GetSymbolOrMethodOrPropertyGroup(
                        lookupResult,
                        memberAccessNode,
                        memberName,
                        targetMemberArity,
                    ...
                    if ((object)symbol == null)
                    {
                        ...
                        lookupResult.Clear();
                        enclosingBinder.CheckWhatCandidatesWeHave(
                            members,
                            type,
                            memberName,
                            targetMemberArity,
            ...
            private bool CheckQuery(QueryExpressionSyntax query, Binder enclosingBinder)
            {
                ...
                {
                    (QueryTranslationState state, _) =
                        enclosingBinder.MakeInitialQueryTranslationState(
                            query,
                            BindingDiagnosticBag.Discarded
                        );
                    ...
                    )
                    {
                        ...
                        // is translated into
                        //     from x in ( from ... ) ...
                        enclosingBinder.PrepareQueryTranslationStateForContinuation(
                            state,
                            continuation,
                            BindingDiagnosticBag.Discarded
                        );
            ...
            private bool ReduceQuery(Binder enclosingBinder, QueryTranslationState state)
            {
                ...
                switch (topClause.Kind())
                {
                    ...
                    case SyntaxKind.OrderByClause:
                        return ReduceOrderBy(
                            enclosingBinder,
                            (OrderByClauseSyntax)topClause,
                            state
                        );
            ...
            {
                if (
                    CheckIdentifiersInNode(join.InExpression, enclosingBinder)
                    && MakeQueryUnboundLambda(
                        enclosingBinder,
                        state.RangeVariableMap(),
                        state.rangeVariable,
                ...
                )
                {
                    ...
                    )
                    {
                        if (join.Into != null)
                        {
                            ...
                            state.AddRangeVariable(
                                enclosingBinder,
                                join.Into.Identifier,
                                BindingDiagnosticBag.Discarded
                            );
                        }
            ...
            private bool MakeQueryUnboundLambda(
                Binder enclosingBinder,
            ...
            )
            {
                ...
                return CheckIdentifiersInNode(
                    expression,
                    lambdaBodyBinder.GetRequiredBinder(expression)
                );
            }
        }
    }
