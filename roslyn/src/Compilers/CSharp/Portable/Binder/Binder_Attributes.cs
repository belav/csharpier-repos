...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal partial class Binder
    {
        ...
        )
        {
            ...
            if (!attributeTypeForBinding.IsErrorType())
            {
                attributeConstructor = BindAttributeConstructor(
                    node,
                    attributeTypeForBinding,
        ...
        )
        {
            ...
            {
                constructorArguments = GetRewrittenAttributeConstructorArguments(
                    ...
                    boundAttribute.ConstructorArgumentsToParamsOpt,
                    diagnostics,
                    ref hasErrors
                );
            }
        ...
        )
        {
            ...
            Debug.Assert(
                !hasNamedCtorArguments || constructorArgumentNamesOpt.Length == argumentsCount
            );
            ...
            for (int i = 0; i < parameterCount; i++)
            {
                ...
                else if (argsConsumedCount < argumentsCount)
                {
                    ...
                    else
                    {
                        ...
                        reorderedArgument = getMatchingNamedOrOptionalConstructorArgument(
                            out matchingArgumentIndex,
                            constructorArgsArray,
                            parameter,
                            argumentsCount,
                            ref argsConsumedCount,
                            syntax,
                            argumentsToParams,
                            diagnostics
                        );
                ...
                {
                    reorderedArgument = GetDefaultValueArgument(
                        parameter,
                        syntax,
                        argumentsToParams,
                        argumentsCount,
            ...
            constructorArgumentsSourceIndices =
                sourceIndices != null
                    ? sourceIndices.AsImmutableOrNull()
            ...
            )
            {
                ...
                {
                    return GetDefaultValueArgument(
                        parameter,
                        syntax,
                        argumentsToParams,
                        argumentsCount,
        ...
        private TypedConstant GetDefaultValueArgument(
            ParameterSymbol parameter,
            AttributeSyntax syntax,
            ImmutableArray<int> argumentsToParams,
            int argumentsCount,
        ...
        )
        {
            ...
            ConstantValue? defaultConstantValue = parameter.IsOptional
            ...
            else if (!IsEarlyAttributeBinder && parameter.IsCallerFilePath)
            {
                ...
                && (object)((ContextualAttributeBinder)this).AttributedMember != null
            )
            {
                parameterType = GetSpecialType(SpecialType.System_String, diagnostics, syntax);
                kind = TypedConstantKind.Primitive;
            ...
            // We shouldn't be checking CallerArgumentExpression
                ...
                && !parameter.IsCallerMemberName
                && getCallerArgumentArgumentIndex(parameter, argumentsToParams) is int argumentIndex
                && argumentIndex > -1
            ...
            static int getCallerArgumentArgumentIndex(
                ParameterSymbol parameter,
                ImmutableArray<int> argumentsToParams
            )
            {
            ...
            if (!constructorArgumentNamesOpt.IsDefault)
            {
                ...
                if (argIndex >= 0)
                {
                    ...
                    return new TypedConstant(
                        parameter.Type,
                        ImmutableArray.Create(constructorArgsArray[argIndex])
                    );
                }
            }
