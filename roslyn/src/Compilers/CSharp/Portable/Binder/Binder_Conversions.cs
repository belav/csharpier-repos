    ...
    internal partial class Binder
    {
        ...
        )
        {
            ...
            if (collectionTypeKind == CollectionExpressionTypeKind.CollectionInitializer)
            {
                ...
                (
                    collectionTypeKind == CollectionExpressionTypeKind.ListInterface
                    && isListInterfaceThatRequiresList(targetType)
                ) || elements.Any(e => e is BoundCollectionExpressionSpreadElement)
            ) // https://github.com/dotnet/roslyn/issues/68785: Avoid intermediate List<T> if all spread elements have Length property.
            {
                Debug.Assert(elementType is { });

                var implicitReceiverType = GetWellKnownType(
        ...
        )
        {
            ...
            if (collectionTypeKind == CollectionExpressionTypeKind.CollectionInitializer)
            {
                BoundObjectOrCollectionValuePlaceholder? implicitReceiver =
                    new BoundObjectOrCollectionValuePlaceholder(
                        node.Syntax,
                        isNewInstance: true,
                        targetType
                    )
            ...
            else if (collectionTypeKind != CollectionExpressionTypeKind.None && elementType is { })
            {
                var elements = node.Elements;
                var useSiteInfo = GetNewCompoundUseSiteInfo(diagnostics);
                foreach (var element in elements)
