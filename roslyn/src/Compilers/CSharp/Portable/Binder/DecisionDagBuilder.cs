...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal sealed partial class DecisionDagBuilder
    {
        ...
        )
        {
            ...
            else
            {
                ...
                if (
                    ValueSetFactory
                        .ForInput(input)
                        ?.Related(BinaryOperatorKind.Equal, constant.ConstantValue)
                        .IsEmpty == true
                )
                {
                    ...
                    Debug.Assert(
                        input.Source is BoundDagPropertyEvaluation { IsLengthOrCount: true }
                    );
        ...
        )
        {
            ...
            if (!recursive.Properties.IsDefault)
            {
                ...
                foreach (var subpattern in recursive.Properties)
                {
                    ...
                    if (
                        !tryMakeTestsForSubpatternMember(
                            subpattern.Member,
                            ref currentInput,
                            subpattern.IsLengthOrCount
                        )
                    )
                    {
            ...
            bool tryMakeTestsForSubpatternMember(
        ...
        private BoundDecisionDag MakeBoundDecisionDag(
            SyntaxNode syntax,
            ref TemporaryArray<StateForCase> cases
        )
        {
        ...
        )
        {
            ...
            var initialState = uniquifyState(
                new FrozenArrayBuilder<StateForCase>(rewrittenCases),
        ...
        )
        {
            ...
            (whenTrueValues, whenFalseValues, bool whenTruePossible, bool whenFalsePossible) =
                SplitValues(state.RemainingValues, test);
        ...
        private static (
            ...
            bool truePossible,
            bool falsePossible
        ) SplitValues(ImmutableDictionary<BoundDagTemp, IValueSet> values, BoundDagTest test)
        {
            switch (test)
            {
        ...
        private static (BoundDagTemp input, BoundDagTemp lengthTemp, int index) GetCanonicalInput(
            BoundDagIndexerEvaluation e
        )
        {
            int index = e.Index;
            BoundDagTemp input = e.Input;
                    ...
                    case var (s1, s2) when s1 == s2:
                        if (conditions != null)
                        {
                            ...
                            relationEffect = new Tests.One(
                                new BoundDagAssignmentEvaluation(
                                    syntax,
                                    target: other.Input,
                                    input: test.Input
                                )
                    ...
                    case (BoundDagIndexerEvaluation s1, BoundDagIndexerEvaluation s2):
                        ...
                        )
                        {
                            ...
                            if (s1Index < 0 != s2Index < 0)
                            {
                                ...
                                var lengthValues =
                                    (IValueSet<int>)state.RemainingValues[s1LengthTemp];
                                ...
                                int lengthValue =
                                    s1Index < 0 ? s2Index - s1Index : s1Index - s2Index;
                                ...
                                if (
                                    !_forLowering
                                    && lengthValues.Any(BinaryOperatorKind.Equal, lengthValue)
                                )
                                {
                                    // Otherwise, we add a test to make the result conditional on the length value.
                                    (conditions ??= ArrayBuilder<Tests>.GetInstance()).Add(
                                        new Tests.One(
                                            new BoundDagValueTest(
                                                syntax,
                                                ConstantValue.Create(lengthValue),
                                                s1LengthTemp
                                            )
                                        )
                                    );
        ...
        {
            ConstantValue result = Binder.ExpressionOfTypeMatchesPatternType(
                _conversions,
                expressionType,
                patternType,
                ref useSiteInfo,
            ...
            {
                while (
                    expressionType is ArrayTypeSymbol array1
        ...
        private sealed class DecisionDag
        {
            ...
            internal string Dump()
            {
                ...
                {
                    return (e == null) ? 0
                        : tempIdentifierMap.TryGetValue(e, out int value) ? value
                        : tempIdentifierMap[e] = ++nextTempNumber;
                }
        ...
        private readonly struct FrozenArrayBuilder<T>
        {
            ...
            public FrozenArrayBuilder(ArrayBuilder<T> arrayBuilder)
            {
                ...
                if (
                    arrayBuilder.Capacity >= ArrayBuilder<T>.PooledArrayLengthLimitExclusive
            ...
            public void Free() => _arrayBuilder.Free();
        ...
        private sealed class DagState
        {
            ...
            /// called on this.
            /// </summary>
            public static DagState GetInstance(
                FrozenArrayBuilder<StateForCase> cases,
                ImmutableDictionary<BoundDagTemp, IValueSet> remainingValues
            )
            {
        ...
        private abstract class Tests
        {
            ...
            public sealed class One : Tests
            {
                ...
                )
                {
                    ...
                    if (
                        other is BoundDagEvaluation
                        || !builder.CheckInputRelation(
                            syntax,
                            state,
                            test,
                            other,
                    ...
                    )
                    {
                        return decisionImpliesTrueOther
                                ? OrSequence.Create(
                                    AndSequence.Create(relationCondition, relationEffect),
                                    other
                                )
                            ...
                            : AndSequence.Create(
                                OrSequence.Create(Not.Create(relationCondition), relationEffect),
                                other
                            );
                    }
                }
                ...
                public override Tests RewriteNestedLengthTests()
                {
                    ...
                    if (test.Input.Source is BoundDagPropertyEvaluation { IsLengthOrCount: true } e)
                    {
                        if (e.Syntax.IsKind(SyntaxKind.ListPattern))
                        {
                            ...
                            {
                                Debug.Assert(
                                    t.Value.Discriminator == ConstantValueTypeDiscriminator.Int32
                                );
                    ...
                    )
                    {
                        ...
                        var possibleValues = fac.Related(
                            BinaryOperatorKind.LessThanOrEqual,
                            int.MaxValue - offset
                        );
            ...
            public sealed class Not : Tests
            {
                ...
                public override Tests RewriteNestedLengthTests() =>
                    Create(Negated.RewriteNestedLengthTests());
