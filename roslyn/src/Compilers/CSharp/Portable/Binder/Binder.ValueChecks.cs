            ...
            if (refSafeToEscape > escapeTo)
            {
                ...
                Debug.Assert(
                    parameterSymbol.RefKind == RefKind.None
                        || isRefScoped
                        || refSafeToEscape == Binder.ReturnOnlyScope
                );
                var inUnsafeRegion = this.InUnsafeRegion;
...
#nullable disable
        ...
        /// </summary>
        internal BoundExpression ValidateEscape(
            BoundExpression expr,
            uint escapeTo,
            bool isByRef,
            BindingDiagnosticBag diagnostics
        ...
        )
        {
            ...
            switch (expr.Kind)
            {
                ...
                case BoundKind.InterpolatedStringArgumentPlaceholder:
                    if (
                        ((BoundInterpolatedStringArgumentPlaceholder)expr).ValSafeToEscape
                        > escapeTo
                    )
                    {
                        Error(
