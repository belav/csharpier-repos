...
#nullable disable
        ...
        )
        {
            ...
            switch (expr.Kind)
            {
                ...
                case BoundKind.RangeVariable:
                    ...
                    Error(
                        diagnostics,
                        GetRangeLvalueError(valueKind),
                        node,
                        queryref.RangeVariableSymbol.Name
                    );
                    return false;
        ...
        private static bool CheckLocalRefEscape(
            SyntaxNode node,
            BoundLocal local,
            uint escapeTo,
            bool checkingReceiver,
        ...
        private static bool CheckParameterRefEscape(
            SyntaxNode node,
            BoundParameter parameter,
            uint escapeTo,
            bool checkingReceiver,
        ...
        private static bool CheckFieldRefEscape(
            SyntaxNode node,
            BoundFieldAccess fieldAccess,
            uint escapeFrom,
            uint escapeTo,
        ...
        private static bool CheckFieldLikeEventRefEscape(
            SyntaxNode node,
            ...
            else
            {
                ...
                else if (RequiresVariable(valueKind))
                {
                    if (eventSymbol.IsWindowsRuntimeEvent && valueKind != BindValueKind.Assignable)
                    {
                        ...
                        ErrorCode errorCode =
                            valueKind == BindValueKind.RefOrOut
                                ? ErrorCode.ERR_WinRtEventPassedByRef
        ...
        )
        {
            ...
            if (!argsOpt.IsDefault)
            {
                ...
                for (var argIndex = 0; argIndex < argsOpt.Length; argIndex++)
                {
                    ...
                    RefKind effectiveRefKind = GetEffectiveRefKindAndMarkMatchedInParameter(
                        argIndex,
                        argRefKindsOpt,
                        parameters,
                        argsToParamsOpt,
                        ref inParametersMatchedWithArgs
                    );
            ...
            ParameterSymbol unmatchedInParameter = TryGetunmatchedInParameterAndFreeMatchedArgs(
                parameters,
                ref inParametersMatchedWithArgs
            );
        ...
        )
        {
            ...
            if (unmatchedInParameter != null && isRefEscape)
            {
                ...
                Error(
                    diagnostics,
                    GetStandardCallEscapeError(checkingReceiver),
                    syntax,
                    symbol,
                    parameterName
                );
            ...
            if (!argsOpt.IsDefault)
            {
                ...
                if (argList != null)
                {
                    ...
                    for (var argIndex = 0; argIndex < argListArgs.Length; argIndex++)
                    {
                        ...
                        var refKind = argListRefKindsOpt.IsDefault
                            ? RefKind.None
                            : argListRefKindsOpt[argIndex];
        ...
        )
        {
            ...
            )
            {
                ...
                if (parameters[paramIndex].RefKind == RefKind.In)
                {
                    ...
                    inParametersMatchedWithArgs =
                        inParametersMatchedWithArgs
                        ?? ArrayBuilder<bool>.GetInstance(parameters.Length, fillWithValue: false);
                ...
                case BoundKind.ThisReference:
                    ...
                    if (escapeTo == Binder.ExternalScope)
                    {
                        Error(
                            ...
                            node,
                            ThisParameterSymbol.SymbolName
                        );
                        return false;
                    }
        ...
        /// Additionally, the method emits diagnostics (possibly more than one, recursively) that would help identify the cause for the failure.
        /// </summary>
        internal static bool CheckValEscape(
            SyntaxNode node,
            BoundExpression expr,
            uint escapeFrom,
            uint escapeTo,
        ...
        private static bool CheckTupleValEscape(
            ImmutableArray<BoundExpression> elements,
            uint escapeFrom,
            uint escapeTo,
            BindingDiagnosticBag diagnostics
        ...
        private static bool CheckValEscapeOfObjectInitializer(
            BoundObjectInitializerExpression initExpr,
            uint escapeFrom,
            uint escapeTo,
            BindingDiagnosticBag diagnostics
        ...
        private static bool CheckValEscape(
            ImmutableArray<BoundExpression> expressions,
            uint escapeFrom,
            uint escapeTo,
            BindingDiagnosticBag diagnostics
        ...
        private static bool CheckInterpolatedStringHandlerConversionEscape(
