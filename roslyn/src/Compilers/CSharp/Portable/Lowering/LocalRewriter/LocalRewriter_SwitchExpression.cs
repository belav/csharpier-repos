        ...
        private sealed class SwitchExpressionLocalRewriter : BaseSwitchLocalRewriter
        {
            ...
            private BoundExpression LowerSwitchExpression(BoundConvertedSwitchExpression node)
            {
                ...
                var produceDetailedSequencePoints =
                    ...
                    && _localRewriter._compilation.Options.OptimizationLevel
                ...
                if (produceDetailedSequencePoints)
                {
                    ...
                    result.Add(
                        new BoundSavePreviousSequencePoint(
                            syntax,
                            restorePointForEnclosingStatement
                        )
                    );
                    ...
                    result.Add(
                        new BoundStepThroughSequencePoint(node.Syntax, span: spanForSwitchBody)
                    );
                    result.Add(
                        new BoundSavePreviousSequencePoint(syntax, restorePointForSwitchBody)
                    );
                }
                ...
                foreach (BoundSwitchExpressionArm arm in node.SwitchArms)
                {
                    ...
                    if (GenerateInstrumentation)
                        loweredValue =
                            this._localRewriter.Instrumenter.InstrumentSwitchExpressionArmExpression(
                                arm.Value,
                                loweredValue,
                                _factory
                            );
                    ...
                    sectionBuilder.Add(
                        _factory.Assignment(_factory.Local(resultTemp), loweredValue)
                    );
                ...
                if (defaultLabel is not null)
                {
                    ...
                    if (produceDetailedSequencePoints)
                        result.Add(
                            new BoundRestorePreviousSequencePoint(
                                node.Syntax,
                                restorePointForSwitchBody
                            )
                        );
            ...
            private static BoundStatement ConstructThrowSwitchExpressionExceptionHelperCall(
            ...
            )
            {
                ...
                var throwSwitchExpressionExceptionMethod =
            ...
            private static BoundStatement ConstructThrowSwitchExpressionExceptionParameterlessHelperCall(
                SyntheticBoundNodeFactory factory
            )
            {
                Debug.Assert(factory.ModuleBuilderOpt is not null);
                ...
                Debug.Assert(diagnostics is not null);
                var throwSwitchExpressionExceptionMethod =
                    module.EnsureThrowSwitchExpressionExceptionParameterlessExists(
                        diagnosticSyntax,
                        factory,
                        diagnostics
                    );
                var call = factory.Call(receiver: null, throwSwitchExpressionExceptionMethod);
                return factory.HiddenSequencePoint(factory.ExpressionStatement(call));
            }

            private static BoundStatement ConstructThrowInvalidOperationExceptionHelperCall(
                SyntheticBoundNodeFactory factory
            )
            {
                Debug.Assert(factory.ModuleBuilderOpt is not null);
                ...
                var diagnostics = factory.Diagnostics.DiagnosticBag;
                Debug.Assert(diagnostics is not null);
                var throwMethod = module.EnsureThrowInvalidOperationExceptionExists(
                    diagnosticSyntax,
                    factory,
                    diagnostics
                );
                var call = factory.Call(receiver: null, throwMethod);
                return factory.HiddenSequencePoint(factory.ExpressionStatement(call));
            }
        }
    }
