        ...
        )
        {
            ...
            switch (strategy)
            {
                case PatternIndexOffsetLoweringStrategy.SubtractFromLength:
                    ...
                    if (
                        makeOffsetInput.ConstantValue is null
            ...
            )
            {
                ...
                && arguments[0]
                    is {
                        ...
                        ConstantValue.Value: int _ and >= 0
                    } index
                && arguments[1]
                    is {
                        Type.SpecialType: SpecialType.System_Boolean,
                        ConstantValue.Value: bool fromEnd
                    }
            )
            {
                if (fromEnd)
        ...
        private BoundExpression VisitRangePatternIndexerAccess(BoundImplicitIndexerAccess node)
        {
            ...
            if (rangeArg is BoundRangeExpression rangeExpr)
            {
                ...
                if (startExpr.ConstantValue?.Int32Value == 0)
                {
                    ...
                    startExpr.ConstantValue is { Int32Value: var startConst }
                    && endExpr.ConstantValue is { Int32Value: var endConst }
                )
                {
