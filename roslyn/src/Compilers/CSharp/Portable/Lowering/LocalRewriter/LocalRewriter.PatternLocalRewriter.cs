        ...
        private abstract class PatternLocalRewriter
        {
            ...
            public sealed class DagTempAllocator
            {
                ...
                public BoundExpression GetTemp(BoundDagTemp dagTemp)
                {
                    ...
                    {
                        var kind = _generateSequencePoints
                            ? SynthesizedLocalKind.SwitchCasePatternMatching
                            : SynthesizedLocalKind.LoweringTemp;
                        LocalSymbol temp = _factory.SynthesizedLocal(
                            dagTemp.Type,
                            syntax: _node,
                            kind: kind
                        );
            ...
            protected BoundExpression LowerEvaluation(BoundDagEvaluation evaluation)
            {
                ...
                switch (evaluation)
                {
                    case BoundDagFieldEvaluation f:
                    {
                        ...
                        BoundExpression access = _localRewriter.MakeFieldAccess(
                            f.Syntax,
                            input,
                            field,
                            null,
                    ...
                    case BoundDagPropertyEvaluation p:
                    {
                        ...
                        return _factory.AssignmentExpression(
                            output,
                            _localRewriter.MakePropertyAccess(
                                _factory.Syntax,
                                input,
                                property,
                                LookupResultKind.Viable,
                    ...
                    case BoundDagDeconstructEvaluation d:
                    {
                        ...
                        for (int i = extensionExtra; i < method.ParameterCount; i++)
                        {
                            ...
                            var outputTemp = new BoundDagTemp(
                                d.Syntax,
                                parameter.Type,
                                d,
                                i - extensionExtra
                        ...
                        return _factory.Call(
                            receiver,
                            method,
                            refKindBuilder.ToImmutableAndFree(),
                        ...
                        if (conversion.Exists)
                        {
                            if (
                                ...
                                && inputType
                                    ...
                                    .Equals(output.Type, TypeCompareKind.AllIgnoreOptions)
                                && _localRewriter.TryGetNullableMethod(
                                    t.Syntax,
                                    inputType,
                    ...
                    case BoundDagIndexEvaluation e:
                    {
                        ...
                        Debug.Assert(
                            e.Property.GetMethod.Parameters[0].Type.SpecialType
                        ...
                        return _factory.AssignmentExpression(
                            output,
                            _factory.Indexer(input, e.Property, _factory.Literal(e.Index))
                        );
                    }
                    ...
                    case BoundDagIndexerEvaluation e:
                    {
                        ...
                        placeholderValues.Add(
                            ...
                            makeUnloweredIndexArgument(e.Index)
                        );
                        indexerAccess = PlaceholderReplacer.Replace(
                            placeholderValues,
                            indexerAccess
                        );
                ...
                BoundExpression makeUnloweredIndexArgument(int index)
                {
                    ...
                    var ctor = (MethodSymbol)
                        _factory.WellKnownMember(WellKnownMember.System_Index__ctor);
                    ...
                    if (index < 0)
                    {
                        return new BoundFromEndIndexExpression(
                            ...
                            methodOpt: ctor,
                            _factory.WellKnownType(WellKnownType.System_Index)
                        );
                    }
                ...
                BoundExpression makeUnloweredRangeArgument(BoundDagSliceEvaluation e)
                {
                    ...
                    var rangeCtor = (MethodSymbol)
                        _factory.WellKnownMember(WellKnownMember.System_Range__ctor);
                    return new BoundRangeExpression(
                        e.Syntax,
                        makeUnloweredIndexArgument(e.StartIndex),
                        end,
                        methodOpt: rangeCtor,
                        _factory.WellKnownType(WellKnownType.System_Range)
                    );
                }
            }
            ...
            )
            {
                ...
                if (rewrittenExpr.Type.IsPointerOrFunctionPointer())
                {
                    ...
                    return _localRewriter.MakeBinaryOperator(
                        ...
                        _factory.Convert(
                            operandType,
                            new BoundLiteral(syntax, ConstantValue.Null, objectType)
                        ),
            ...
            protected BoundExpression MakeValueTest(
                SyntaxNode syntax,
            ...
            protected BoundExpression MakeRelationalTest(
                SyntaxNode syntax,
                BoundExpression input,
                BinaryOperatorKind operatorKind,
                ConstantValue value
            ...
            private BoundExpression MakeSpanStringTest(BoundExpression input, ConstantValue value)
            {
                ...
                return _factory.Call(
                    ...
                    sequenceEqual,
                    input,
                    _factory.Call(null, asSpan, _factory.StringLiteral(value))
                );
            }
            ...
            )
            {
                ...
                if (
                    ...
                    && (
                        conv.IsIdentity
                        || conv.Kind == ConversionKind.ImplicitReference
                        || conv.IsBoxing
                    )
                ...
                )
                {
                    ...
                    sideEffect = _factory.AssignmentExpression(
                        output,
                        _factory.Convert(baseType, input)
                    );
            ...
            )
            {
                ...
                bool anyWhenClause = decisionDag.TopologicallySortedNodes.Any(static node =>
                    node is BoundWhenDecisionDagNode { WhenExpression: { ConstantValueOpt: null } }
                );

                var inputDagTemp = BoundDagTemp.ForOriginalInput(loweredInput);
                if (
                    (
                        loweredInput.Kind == BoundKind.Local
                ...
                foreach (BoundDecisionDagNode node in decisionDag.TopologicallySortedNodes)
                {
                    if (node is BoundWhenDecisionDagNode w)
                    {
                        ...
                        foreach (BoundPatternBinding binding in w.Bindings)
                        {
                            ...
                            {
                                Debug.Assert(
                                    l.LocalSymbol.DeclarationKind
                                        == LocalDeclarationKind.PatternVariable
                                );
                                _ = _tempAllocator.TrySetTemp(
            ...
            )
            {
                ...
                var originalInput = BoundDagTemp.ForOriginalInput(
                    loweredInput.Syntax,
                    loweredInput.Type
                );
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    var fieldFetchEvaluation = new BoundDagFieldEvaluation(
                        expr.Syntax,
                        field,
                        originalInput
                    );
                ...
                savedInputExpression = loweredInput.Update(
                    loweredInput.Constructor,
                    arguments: newArguments.ToImmutableAndFree(),
                    loweredInput.ArgumentNamesOpt,
                    loweredInput.ArgumentRefKindsOpt,
                    loweredInput.Expanded,
                    loweredInput.ArgsToParamsOpt,
                    loweredInput.DefaultArguments,
                    loweredInput.ConstantValueOpt,
                    loweredInput.InitializerExpressionOpt,
                    loweredInput.Type
                );
                ...
                static BoundDecisionDagNode makeReplacement(
                ...
                )
                {
                    switch (node)
                    {
                        case BoundEvaluationDecisionDagNode evalNode:
                            if (
                                evalNode.Evaluation is BoundDagFieldEvaluation eval
