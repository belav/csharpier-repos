...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    /// </summary>
    internal sealed class AsyncExceptionHandlerRewriter
        : BoundTreeRewriterWithStackGuardWithoutRecursionOnTheLeftOfBinaryOperator
    {
        ...
        )
        {
            ...
            var rewriter = new AsyncExceptionHandlerRewriter(
                containingSymbol,
                containingType,
                factory,
                analysis
        ...
        public override BoundNode VisitTryStatement(BoundTryStatement node)
        {
            ...
            var pendingExceptionLocal = new SynthesizedLocal(
                _F.CurrentFunction,
                TypeWithAnnotations.Create(exceptionType),
                SynthesizedLocalKind.TryAwaitPendingException,
                tryStatementSyntax
            );
            var finallyLabel = _F.GenerateLabel("finallyLabel");
            var pendingBranchVar = new SynthesizedLocal(
                _F.CurrentFunction,
                TypeWithAnnotations.Create(_F.SpecialType(SpecialType.System_Int32)),
                SynthesizedLocalKind.TryAwaitPendingBranch,
                tryStatementSyntax
            );
            ...
            statements.Add(
                _F.Assignment(
                    _F.Local(pendingExceptionLocal),
                    _F.Default(pendingExceptionLocal.Type)
                )
            );
            locals.Add(pendingBranchVar);
            statements.Add(
                _F.Assignment(_F.Local(pendingBranchVar), _F.Default(pendingBranchVar.Type))
            );
                ...
                else
                {
                    ...
                    else
                    {
                        unpendReturn = _F.Block(
                            _F.Assignment(_F.Local(returnValue), pendingValue),
                            _F.Goto(returnLabel)
                        );
                    }
                }
        ...
        {
            Debug.Assert(
                node.Label == _currentAwaitFinallyFrame.ProxyLabelIfNeeded(node.Label),
                "conditional leave?"
            );
        ...
        public override BoundNode VisitReturnStatement(BoundReturnStatement node)
        {
            ...
            {
                return _F.Block(
                    _F.Assignment(_F.Local(returnValue), returnExpr),
                    _F.Goto(returnLabel)
                );
            }
            else
        ...
        private BoundStatement UnpendException(LocalSymbol pendingExceptionLocal)
        {
            ...
            return _F.Block(
                ...
                objInit,
                _F.If(_F.ObjectNotEqual(_F.Local(obj), _F.Null(obj.Type)), rethrow)
            );
        }
        ...
        private BoundStatement Rethrow(LocalSymbol obj)
        {
            ...
            var exceptionDispatchInfoCapture = _F.WellKnownMethod(
                WellKnownMember.System_Runtime_ExceptionServices_ExceptionDispatchInfo__Capture,
                isOptional: true
            );
            var exceptionDispatchInfoThrow = _F.WellKnownMethod(
                WellKnownMember.System_Runtime_ExceptionServices_ExceptionDispatchInfo__Throw,
                isOptional: true
            );
            ...
            if (exceptionDispatchInfoCapture != null && exceptionDispatchInfoThrow != null)
            {
                ...
                var assignment = _F.Assignment(_F.Local(ex), _F.As(_F.Local(obj), ex.Type));
        ...
        private BoundStatement RewriteFinalizedRegion(BoundTryStatement node)
        {
            ...
            if (currentAwaitCatchFrame != null)
            {
                ...
                {
                    handlers.Add(
                        _F.SwitchSection(i + 1, _F.Block(handlersList[i], _F.Goto(handledLabel)))
                    );
                }
        ...
        public override BoundNode VisitCatchBlock(BoundCatchBlock node)
        {
            ...
            var storePending = _F.AssignmentExpression(
                ...
                _F.Convert(currentAwaitCatchFrame.pendingCaughtException.Type, _F.Local(catchTemp))
            );
            ...
            else
            {
                ...
                catchAndPend = node.Update(
                    ...
                    body: _F.Block(_F.HiddenSequencePoint(), setPendingCatchNum),
                    isSynthesizedAsyncCatchAll: node.IsSynthesizedAsyncCatchAll
                );
            }
            ...
            if (filterOpt == null)
            {
                ...
                if (sourceOpt != null)
                {
                    BoundExpression assignSource = AssignCatchSource(
                        (BoundExpression)this.Visit(sourceOpt),
                        currentAwaitCatchFrame
                    );
            ...
            var handler = _F.Block(handlerLocals, handlerStatements.ToImmutableAndFree());
        ...
        private AwaitFinallyFrame PushFrame(BoundTryStatement statement)
        {
            var newFrame = new AwaitFinallyFrame(
                ...
                _analysis.Labels(statement),
                (StatementSyntax)statement.Syntax
            );
        ...
        private sealed class AwaitFinallyFrame
        {
            ...
            )
            {
                ...
                Debug.Assert(
                    statementSyntax.Kind() == SyntaxKind.TryStatement
                        || (
                            statementSyntax.Kind() == SyntaxKind.UsingStatement
                            && ((UsingStatementSyntax)statementSyntax).AwaitKeyword != default
                        )
                        || (
                            statementSyntax.Kind() == SyntaxKind.ForEachStatement
                            && ((CommonForEachStatementSyntax)statementSyntax).AwaitKeyword
                                != default
                        )
                        || (
                            statementSyntax.Kind() == SyntaxKind.ForEachVariableStatement
                            && ((CommonForEachStatementSyntax)statementSyntax).AwaitKeyword
                                != default
                        )
                        || (
                            statementSyntax.Kind() == SyntaxKind.LocalDeclarationStatement
                            && ((LocalDeclarationStatementSyntax)statementSyntax).AwaitKeyword
                                != default
                        )
                );
        ...
        private sealed class AwaitCatchFrame
        {
            ...
            public void HoistLocal(LocalSymbol local, SyntheticBoundNodeFactory F)
            {
                ...
                var newLocal = F.SynthesizedLocal(
                    local.Type,
                    pendingCatch.SyntaxOpt,
