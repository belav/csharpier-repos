    ...
    internal struct AsyncMethodBuilderMemberCollection
    {
        ...
        )
        {
            ...
            if (method.IsAsyncEffectivelyReturningTask(F.Compilation))
            {
                ...
                bool useMethodLevelBuilder = method.HasAsyncMethodBuilderAttribute(
            ...
            if (method.IsAsyncEffectivelyReturningGenericTask(F.Compilation))
            {
                var returnType = (NamedTypeSymbol)method.ReturnType;
                var resultType =
                    returnType.TypeArgumentsWithAnnotationsNoUseSiteDiagnostics.Single().Type;
        ...
        private static NamedTypeSymbol ValidateBuilderType(
            SyntheticBoundNodeFactory F,
            object builderAttributeArgument,
            Accessibility desiredAccessibility,
            bool isGeneric,
            bool forMethodLevelBuilder = false
        )
        {
