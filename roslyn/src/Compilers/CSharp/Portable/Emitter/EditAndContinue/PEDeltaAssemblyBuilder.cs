    ...
    internal sealed class PEDeltaAssemblyBuilder : PEAssemblyBuilderBase, IPEDeltaAssemblyBuilder
    {
        ...
        )
        {
            ...
            GetAnonymousTypeMapFromMetadata(
                ...
                metadataDecoder,
                out var metadataAnonymousTypes,
                out var metadataAnonymousDelegatesWithFixedTypes
            );
            var metadataAnonymousDelegates = GetAnonymousDelegateMapFromMetadata(
                originalMetadata.MetadataReader,
                metadataDecoder
            );
            var metadataSymbols = new EmitBaseline.MetadataSymbols(
                metadataAnonymousTypes,
                metadataAnonymousDelegates,
                metadataAnonymousDelegatesWithFixedTypes,
                metadataDecoder,
        ...
        )
        {
            ...
            const string AnonymousTypeNameWithoutModulePrefix =
                AnonymousTypeOrDelegateNamePrefix + "Type";
            const string AnonymousDelegateNameWithoutModulePrefix =
                AnonymousTypeOrDelegateNamePrefix + "Delegate";
            ...
            foreach (var handle in reader.TypeDefinitions)
            {
                ...
                if (name.StartsWith(AnonymousTypeNameWithoutModulePrefix, StringComparison.Ordinal))
                {
                    if (
                        int.TryParse(
                            name.Substring(AnonymousTypeNameWithoutModulePrefix.Length),
                            NumberStyles.None,
                    ...
                    name.StartsWith(
                        AnonymousDelegateNameWithoutModulePrefix,
                ...
                )
                {
                    if (
                        int.TryParse(
                            name.Substring(AnonymousDelegateNameWithoutModulePrefix.Length),
                            NumberStyles.None,
                            CultureInfo.InvariantCulture,
                            out int index
                        )
        ...
        > GetAnonymousDelegateMapFromMetadata(
            MetadataReader reader,
            MetadataDecoder metadataDecoder
        )
        {
        ...
        public IReadOnlyDictionary<string, AnonymousTypeValue> GetAnonymousDelegatesWithFixedTypes()
        {
            var anonymousDelegates =
                this.Compilation.AnonymousTypeManager.GetAnonymousDelegatesWithFixedTypes();
            // Should contain all entries in previous generation.
            Debug.Assert(
                _previousGeneration.AnonymousDelegatesWithFixedTypes.All(
                    p => anonymousDelegates.ContainsKey(p.Key)
                )
            );
            return anonymousDelegates;
