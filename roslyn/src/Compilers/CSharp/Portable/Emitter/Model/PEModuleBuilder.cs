...
#nullable enable
        ...
        /// </remarks>
        internal MethodSymbol EnsureThrowIfNullFunctionExists(
        ...
        )
        {
            ...
            var throwIfNullAdapter = privateImplClass.GetMethod(
            ...
            if (throwIfNullAdapter is null)
            {
                ...
                privateImplClass.TryAddSynthesizedMethod(
                    new SynthesizedThrowMethod(
                        sourceModule,
                        privateImplClass,
                        returnType,
                        paramNameType
                ...
                var actuallyAddedThrowMethod = (SynthesizedThrowMethod)
                    privateImplClass
                        .GetMethod(PrivateImplementationDetails.SynthesizedThrowFunctionName)!
                        .GetInternalSymbol()!;
                privateImplClass.TryAddSynthesizedMethod(
                    new SynthesizedThrowIfNullMethod(
                        sourceModule,
                        privateImplClass,
                        actuallyAddedThrowMethod,
                        returnType,
                        argumentType,
                        paramNameType
                    ).GetCciAdapter()
                );
                throwIfNullAdapter = privateImplClass.GetMethod(
                    PrivateImplementationDetails.SynthesizedThrowIfNullFunctionName
                )!;
            }

            var internalSymbol = (SynthesizedThrowIfNullMethod)
                throwIfNullAdapter.GetInternalSymbol()!;
        ...
        {
            var privateImplClass = GetPrivateImplClass(syntaxNode, diagnostics);
            var throwSwitchExpressionAdapter = privateImplClass.GetMethod(
            ...
            // use add-then-get pattern to ensure the symbol exists, and then ensure we use the single "canonical" instance added by whichever thread won the race.
            privateImplClass.TryAddSynthesizedMethod(
                new SynthesizedThrowSwitchExpressionExceptionMethod(
                    sourceModule,
                    privateImplClass,
                    returnType,
                    unmatchedValueType
        ...
        )
        {
            ...
            // use add-then-get pattern to ensure the symbol exists, and then ensure we use the single "canonical" instance added by whichever thread won the race.
            privateImplClass.TryAddSynthesizedMethod(
                new SynthesizedParameterlessThrowMethod(
                    sourceModule,
                    privateImplClass,
                    returnType,
                    PrivateImplementationDetails.SynthesizedThrowSwitchExpressionExceptionParameterlessFunctionName,
        ...
        {
            var privateImplClass = GetPrivateImplClass(syntaxNode, diagnostics);
            var throwAdapter = privateImplClass.GetMethod(
