        ...
        private sealed class NintValueSetFactory : IValueSetFactory<int>, IValueSetFactory
        {
            ...
            IValueSet IValueSetFactory.Random(int expectedSize, Random random)
            {
                return new NintValueSet(
                    ...
                    values: (IValueSet<int>)
                        NumericValueSetFactory<int, IntTC>.Instance.Random(expectedSize, random),
                    hasLarge: random.NextDouble() < 0.25
                );
            }

            ConstantValue IValueSetFactory.RandomValue(Random random) =>
                ConstantValue.CreateNativeInt(default(IntTC).Random(random));
