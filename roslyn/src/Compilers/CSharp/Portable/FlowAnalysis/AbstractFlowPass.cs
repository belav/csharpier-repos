...
namespace Microsoft.CodeAnalysis.CSharp
{
            ...
            >.AbstractLocalFunctionState
    {
        ...
        protected struct SavedPending
        {
            ...
            public SavedPending(
                ArrayBuilder<PendingBranch> pendingBranches,
        ...
        public override BoundNode VisitDelegateCreationExpression(
            ...
            if (methodGroup != null)
            {
                if ((object)node.MethodOpt != null && node.MethodOpt.RequiresInstanceReceiver)
                {
                    ...
                    {
                        if (
                            methodGroup == this.firstInRegion
                            && this.regionPlace == RegionPlace.Before
                        ) {
                            EnterRegion();
                        }
        ...
        public override BoundNode VisitConversion(BoundConversion node)
        {
            if (node.ConversionKind == ConversionKind.MethodGroup)
            {
                if (
                    ...
                    {
                        if (
                            node.Operand == this.firstInRegion
                            && this.regionPlace == RegionPlace.Before
                        ) {
                            EnterRegion();
                        }
        ...
        private void VisitArrayInitializationInternal(
            ...
            foreach (var child in node.Initializers)
            {
                ...
                {
                    VisitArrayInitializationInternal(
                        arrayCreation,
                        (BoundArrayInitialization)child
                    );
                }
                else
                {
