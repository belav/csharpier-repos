...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal sealed partial class SyntaxAndDeclarationManager : CommonSyntaxAndDeclarationManager
    {
        internal sealed class State
        {
            ...
            /// <summary>
            /// Mapping from a syntax tree to the last fully computed member names for each the types (in
            // lexical order)
            /// for this file.  Specifically, the key of the collection is the tree the data is cached for.  The
            // <see
            /// cref="OneOrMany"/> is a compact array of items, each of which corresponds to the prior
            // type-declaration
            /// in the tree that contributed members (in lexical order).  Each item in that compact array is
            // then the
            /// member names for that particular type declaration.
            /// </summary>
            /// <remarks>
            /// Member names often don't change for most edits, so being able to reuse the same set from the
            // last time
            /// things were computed saves on a lot of memory churn producing the new set, then GC'ing the last
            // set
            /// (esp. for very large types). The value is stored as a <see cref="OneOrMany"/> as the most common
            // case
            /// for most files is a single type declaration.
            /// <para/>
            /// We store this with weak references so that we can obtain this optimization in the common case
            // where the
            /// old declaration is still around rooting the old names.  If the decls are gone though, the names
            // are subject
