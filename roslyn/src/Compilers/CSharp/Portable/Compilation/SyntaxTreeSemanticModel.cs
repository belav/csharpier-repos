...
#nullable disable
        ...
        )
        {
            ...
            switch (node)
            {
                ...
                case TypeDeclarationSyntax
                {
                    ...
                    when TryGetSynthesizedPrimaryConstructor(typeDeclaration)
                ...
                if ((object)type != null)
                {
                    ...
                    else if (SyntaxFacts.IsInTypeOnlyContext(type))
                    {
                        ...
                        if ((object)result == null || result.Kind == SymbolKind.ErrorType)
                        {
                            ...
                            var variableDecl =
                                type.ModifyingScopedOrRefTypeOrSelf().Parent
        ...
        internal PublicSemanticModel CreateSpeculativeAttributeSemanticModel(
            int position,
            ...
            if (memberDecl != null)
            {
                ...
                switch (memberDecl.Kind())
                {
                    ...
                    case SyntaxKind.RecordDeclaration:
                    {
                        var typeDecl = (TypeDeclarationSyntax)memberDecl;
                        return
                            typeDecl.ParameterList is object
        ...
        )
        {
            ...
            var symbol = GetDeclaredNamespace(
                container,
        ...
        )
        {
            switch (name.Kind())
            {
                ...
                case SyntaxKind.IdentifierName:
                    return (NamespaceSymbol)GetDeclaredMember(
                        container,
                        declarationSpan,
                        isKnownToBeANamespace: true,
                ...
                case SyntaxKind.QualifiedName:
                    ...
                    var left =
                        GetDeclaredNamespace(container, declarationSpan, qn.Left)
        ...
        )
        {
            ...
            if (isKnownToBeANamespace)
            {
                ...
                Debug.Assert(
                    name is not null,
                    "Should only be looking for a known namespace by name."
                );
                Debug.Assert(
                    namespaces is [NamespaceSymbol],
                    "Namespace declarations of the same name are expected to appear as a single merged symbol."
                );

                if (name != null && namespaces is [NamespaceSymbol knownNamespace])
                {
                    Debug.Assert(
                        knownNamespace.HasLocationContainedWithin(
                            SyntaxTree,
                            declarationSpan,
                            out _
                        ),
                        "Namespace symbols should include all syntax declaration locations."
                    );
            ...
            foreach (var symbol in collection)
            {
                ...
                if ((object)namedType != null && namedType.IsImplicitClass)
                {
                    ...
                    var result = GetDeclaredMember(
                        namedType,
                        declarationSpan,
                        isKnownToBeANamespace,
                        name
                    );
                ...
                if (
                    symbol.HasLocationContainedWithin(
                        this.SyntaxTree,
                        declarationSpan,
                        out var wasZeroWidthMatch
                    )
                )
                {
        ...
        )
        {
            ...
            for (; binder != null; binder = binder.Next)
            {
                ...
                if (!usingAliases.IsDefault)
                {
                    ...
                    {
                        if (
                            alias.Alias.GetFirstLocation().SourceSpan
                            == declarationSyntax.Alias.Name.Span
                        )
                        {
            ...
            for (; binder != null; binder = binder.Next)
            {
                ...
                if (!externAliases.IsDefault)
                {
                    foreach (var alias in externAliases)
                    {
                        if (
                            alias.Alias.GetFirstLocation().SourceSpan
                            == declarationSyntax.Identifier.Span
                        )
                        {
                            return alias.Alias.GetPublicSymbol();
                        }
        ...
        )
        {
            switch (declaredNode)
            {
                ...
                case TypeDeclarationSyntax typeDeclaration
                    when TryGetSynthesizedPrimaryConstructor(typeDeclaration)
                        is SynthesizedPrimaryConstructor ctor:
                    if (
                    ...
                    )
                    {
                        switch (declaredSymbol.Kind)
                        {
                            ...
                            case SymbolKind.NamedType:
                                ...
                                return (node) =>
                                    node != typeDeclaration.ParameterList
                                    && !(
                ...
                {
                    Parent: BaseListSyntax { Parent: TypeDeclarationSyntax typeDeclaration }
                } baseType
                    when typeDeclaration.PrimaryConstructorBaseTypeIfClass == declaredNode
                        && TryGetSynthesizedPrimaryConstructor(typeDeclaration)
                            is SynthesizedPrimaryConstructor ctor:
