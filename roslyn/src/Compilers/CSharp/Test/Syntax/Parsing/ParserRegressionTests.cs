...
class Program
{
    ...
    partial abstract struct S {}
            ...
            comp.VerifyDiagnostics(
                ...
                // (5,5): error CS0267: The 'partial' modifier can only appear immediately before 'class', 'record',
                // 'struct', 'interface', or a method return type.
                //     partial abstract class A {}
                ...
                // (10,5): error CS0267: The 'partial' modifier can only appear immediately before 'class',
                // 'record', 'struct', 'interface', or a method return type.
                //     partial abstract struct S {}
                Diagnostic(ErrorCode.ERR_PartialMisplaced, "partial").WithLocation(10, 5),
                // (11,5): error CS0267: The 'partial' modifier can only appear immediately before 'class',
                // 'record', 'struct', 'interface', or a method return type.
                //     partial abstract struct S {}
                ...
                Diagnostic(ErrorCode.ERR_DuplicateNameInClass, "")
                    ...
                    .WithLocation(8, 13),
                // (8,5): error CS0246: The type or namespace name 'partial' could not be found (are you missing a
                // using directive or an assembly reference?)
                //     partial partial class B {}
                Diagnostic(ErrorCode.ERR_SingleTypeNameNotFound, "partial")
                    .WithArguments("partial")
                    .WithLocation(8, 5),
                // (7,5): error CS0246: The type or namespace name 'partial' could not be found (are you missing a
                // using directive or an assembly reference?)
                //     partial partial class B {}
                Diagnostic(ErrorCode.ERR_SingleTypeNameNotFound, "partial")
                    .WithArguments("partial")
        ...
        {
            var test =
                @"///   \u2750\uDFC1  = </   @goto   </  ascending abstract  + (  descending __arglist  + descending   @if   <?   @switch  + global @long  + @orderby   \u23DC\u6D71\u5070\u9350   ++  into _\u6105\uE331\u27D0   #  join [  + break   @extern   [   @char   <<  partial |  + remove + do   @else  + @typeof   @private  + 
";
        ...
        {
            var test =
                @"internal int TYPES()        {             break  retVal =  @while ; __reftype             CLASS c = dynamic   descending  CLASS( % ; on             IF xx = module   _4䓞  |=              \u14DB\u0849   <![CDATA[  c =>  @default  $             retVal @assembly  += c void .Member &  -= ; @typeof 
";
            var tree = SyntaxFactory.ParseSyntaxTree(test);
        }
        ...
        {
            var test =
                @"/// </summary>        /// <returns></returns>         else  int OPERATOR @uint  $ )        { -              static ? operator  :: ]  @readonly  = @on   async  int? , [ return ] { 1 ! ,  @property  &  3 !   @case  %   partial   += ;/*[] bug*/ // YES []            int % ] endregion  var  =   ]]>   @for  |=   @struct , 3, lock  4 @on  %  5 goto  } @stackalloc  } /*,;*/            int %=  i = @fixed   ?> int << a base  <= 1] default ; 
...
";
        ...
        {
            var test =
                @"/// </summary>        /// <returns></returns>        internal  do  OPERATOR || )        {            int?[] a = new int? \u14DB\u0849 [5] { 1, 2, 3, 4,  @using  } /= /*[] bug*/ // YES []            int[] var = { 1, 2, 3, 4, 5 } $ /*,;*/            int i =  ; int)a[1];/*[]*/            i = i  <<=   @__arglist  - i @sbyte  *  @extern  / i % i ++   %  i  ||   @checked  ^ i; 
 /*+ - * / % & | ^*/
";
            var tree = SyntaxFactory.ParseSyntaxTree(test);
        }
        ...
        {
            var test =
                @"int?[] ///  a   = new int? .  @string ] { 1,  typeof  $  3, 4, 5 } static ; @partial /*[] bug*/ // YES []            int[] var = { 1,  else , 3 </  4 |  5 };/*,;*/            int i = (int)a @in [ __refvalue ] [ /*[]*/             @readonly  = i + i - abstract   @typevar  * i /  @this  % i & ,  i | i ^ unchecked  i; in /*+ - * / % & | ^*/            bool b = true & false +  | true ^ false readonly ;/*& | ^*/             @unsafe  = !b;/*!*/            i = ~i;/*~i*/            b = i < (i - 
  1 @for ) && (i + 1) > i;/*< && >*/
";
            var tree = SyntaxFactory.ParseSyntaxTree(test);
        }
        ...
        {
            var test =
                @"/// </summary>        /// <returns></returns>         const   by  TYPES ascending  / ) $         { @let             int @byte   @by   |  0 
 ; 
";
            var tree = SyntaxFactory.ParseSyntaxTree(test);
        }
...
class A
{
    ...
    public int
            ...
            ParseAndValidate(
                ...
                //     #warning hello
                Diagnostic(ErrorCode.WRN_WarningDirective, "hello")
                    .WithArguments("hello")
                    .WithLocation(4, 14),
                // (6,1): error CS1519: Invalid token '}' in class, record, struct, or interface member declaration
                // }
        ...
        [WorkItem(
            217398,
            "https://devdiv.visualstudio.com/DefaultCollection/DevDiv/_workitems?id=217398"
        )]
        public void LexerTooManyBadTokens()
        {
            ...
            Assert.Equal(
                "(1,1201): error CS1056: Unexpected character '\\u003C'",
                actualErrors.ElementAt(200).ToString(EnsureEnglishUICulture.PreferredOrNull)
            );
            Assert.Equal(
                "(1,1207): error CS1056: Unexpected character '\\u003E\\u003E\\u003E\\u003E'",
                actualErrors.ElementAt(201).ToString(EnsureEnglishUICulture.PreferredOrNull)
            );
        }
        ...
        public void LexerTooManyBadTokens_LongUnicode()
        {
            ...
            Assert.Equal("202", actualErrors.Count().ToString());
            Assert.Equal(
                "(1,2001): error CS1056: Unexpected character '\\U0000003C'",
                actualErrors.ElementAt(200).ToString(EnsureEnglishUICulture.PreferredOrNull)
            );
            Assert.Equal(
                "(1,2011): error CS1056: Unexpected character '\\u003E\\u003E\\u003E\\u003E'",
                actualErrors.ElementAt(201).ToString(EnsureEnglishUICulture.PreferredOrNull)
            );
        }
