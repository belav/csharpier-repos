...
"
        ...
        public void TestAncestors()
        {
            var text = "a + (b - (c * (d / e)))";
            var expression = SyntaxFactory.ParseExpression(text);
            var e = expression.DescendantNodes()
        ...
        public void TestFirstAncestorOrSelf()
        {
            ...
            var firstParens = e.FirstAncestorOrSelf<ExpressionSyntax>(
                n => n.Kind() == SyntaxKind.ParenthesizedExpression
            );
        ...
        public void TestDescendantNodes()
        {
            ...
            nodes = statement.DescendantNodes(n => n is StatementSyntax, descendIntoTrivia: true)
                .ToList();
            Assert.Equal(2, nodes.Count);
            Assert.Equal(SyntaxKind.IfDirectiveTrivia, nodes[0].Kind());
            Assert.Equal(SyntaxKind.TrueLiteralExpression, nodes[1].Kind());
            ...
            nodes = statement.DescendantNodes(statement.FullSpan, n => n is StatementSyntax)
                .ToList();
            Assert.Equal(1, nodes.Count);
            Assert.Equal(SyntaxKind.TrueLiteralExpression, nodes[0].Kind());
        ...
        public void TestDescendantNodesAndSelf()
        {
            ...
            nodes = statement.DescendantNodesAndSelf(statement.FullSpan, descendIntoTrivia: true)
                .ToList();
            Assert.Equal(4, nodes.Count);
            Assert.Equal(SyntaxKind.ReturnStatement, nodes[0].Kind());
            Assert.Equal(SyntaxKind.IfDirectiveTrivia, nodes[1].Kind());
            ...
            nodes = statement.DescendantNodesAndSelf(statement.FullSpan, n => n is StatementSyntax)
                .ToList();
            Assert.Equal(2, nodes.Count);
            Assert.Equal(SyntaxKind.ReturnStatement, nodes[0].Kind());
            Assert.Equal(SyntaxKind.TrueLiteralExpression, nodes[1].Kind());
        ...
        public void TestDescendantNodesAndTokensAndSelf()
        {
            ...
            nodesAndTokens = statement.DescendantNodesAndTokensAndSelf(descendIntoTrivia: true)
                .ToList();
...
"
        ...
        public void TestReplaceTriviaDeep()
        {
            ...
            var deepTrivia = expr.GetDirectives()
                .SelectMany(
                    d => d.DescendantTrivia().Where(tr => tr.Kind() == SyntaxKind.WhitespaceTrivia)
                )
                .ToList();
...
";
        ...
        public void TestSyntaxTriviaListEquals()
        {
            ...
            var emptyToken = SyntaxFactory.MissingToken(SyntaxKind.IdentifierToken)
            ...
            emptyToken = emptyToken.WithLeadingTrivia(emptyTriviaList)
                ...
                @"class Goo
{
    void Bar()
    {
        ...
        public void SyntaxNodeContains()
        {
            var text = "a + (b - (c * (d / e)))";
            var expression = SyntaxFactory.ParseExpression(text);
            var a = expression.DescendantNodes()
                .OfType<IdentifierNameSyntax>()
