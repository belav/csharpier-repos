...
{
    public class SyntaxAwareGeneratorTests : CSharpTestBase
    {
        [Fact]
...
";
            ...
            var testGenerator = new CallbackGenerator(
                ...
                onExecute: (e) =>
                {
                    syntaxReceiver = e.SyntaxReceiver;
                    contextReceiver = e.SyntaxContextReceiver;
                }
            );
...
";
            ...
            Assert.Equal(
                testReceiver.VisitedNodes[0].SemanticModel.SyntaxTree,
                testReceiver.VisitedNodes[0].Node.SyntaxTree
            );
        }
...
";
            ...
            var testGenerator = new CallbackGenerator(
                onInit: (i) =>
                    i.RegisterForSyntaxNotifications(() => new TestSyntaxReceiver(++invocations)),
                onExecute: (e) => receiver = e.SyntaxReceiver
            );
...
";
            ...
            var testGenerator = new CallbackGenerator(
                onInit: (i) =>
                    i.RegisterForSyntaxNotifications((SyntaxReceiverCreator)(() => null!)),
                onExecute: (e) =>
                {
                    syntaxRx = e.SyntaxReceiver;
                    syntaxContextRx = e.SyntaxContextReceiver;
                }
            );
...
";
            ...
            var testGenerator = new CallbackGenerator(
                onInit: (i) =>
                    i.RegisterForSyntaxNotifications((SyntaxContextReceiverCreator)(() => null!)),
                onExecute: (e) =>
                {
                    syntaxRx = e.SyntaxReceiver;
                    syntaxContextRx = e.SyntaxContextReceiver;
...
";
            ...
            var testGenerator = new PipelineCallbackGenerator(
                context =>
                {
                    var source = context.SyntaxProvider.CreateSyntaxProvider(
                        (c, _) => c is FieldDeclarationSyntax fds,
                        (c, _) =>
                            ((FieldDeclarationSyntax)c.Node).Declaration.Variables[
                                0
...
";
            ...
            compilation = compilation.AddSyntaxTrees(
                CSharpSyntaxTree.ParseText(source2, parseOptions)
            );
            driver = driver.RunGenerators(compilation);
...
class E 
{
    string fieldC = null; 
                ...
                context =>
                {
                    var source = context.SyntaxProvider.CreateSyntaxProvider(
                        (c, _) =>
                        {
                            ...
                            {
                                syntaxFieldsCalledFor.Add(
                                    fds.Declaration.Variables[0].Identifier.ValueText
                                );
                                return true;
                            }
                            return false;
            ...
            compilation = compilation
                .ReplaceSyntaxTree(firstTree, dummyTree)
                ...
                context =>
                {
                    var source = context.SyntaxProvider.CreateSyntaxProvider(
                        ...
                        (c, _) =>
                        {
                            ...
                            return ((FieldDeclarationSyntax)c.Node).Declaration.Variables[
                                0
                            ].Identifier.ValueText;
                        }
                    );
...
";
            ...
            Assert.Equal(
                new[] { "fieldA", "fieldB", "fieldC", "fieldA", "fieldB", "fieldC" },
                syntaxCalledFor
            );
            ...
            driver = driver.RunGenerators(compilation);
            Assert.Equal(
                new[] { "fieldD", "fieldE", "fieldF", "fieldD", "fieldE", "fieldF" },
                syntaxCalledFor
            );
...
";
            ...
            Assert.Equal(new[] { "fieldA", "fieldB", "fieldC", }, syntaxCalledFor);
            Assert.Equal(
                new[] { "Output1_fieldA", "Output1_fieldB", "Output1_fieldC" },
                output1CalledFor
            );
            Assert.Equal(
                new[] { "Output2_fieldA", "Output2_fieldB", "Output2_fieldC" },
                output2CalledFor
            );
        }
...
";
                ...
                ctx =>
                {
                    ctx.RegisterSourceOutput(
                        ctx.SyntaxProvider.CreateSyntaxProvider(
                            (s, _) =>
                            {
                                if (s is AssignmentExpressionSyntax)
                                    throw exception;
                                return true;
                            },
                            (c, _) => c.Node
            ...
            outputDiagnostics.Verify(
                Diagnostic("CS" + (int)ErrorCode.WRN_GeneratorFailedDuringGeneration)
                    .WithArguments("PipelineCallbackGenerator", "Exception", "Test Exception")
                    .WithLocation(1, 1)
            );
        }
...
";
            ...
            GeneratorDriver driver = CSharpGeneratorDriver.Create(
                new[]
                {
                    ...
                    new IncrementalGeneratorWrapper(testGenerator2)
                },
                parseOptions: parseOptions
            );
            driver = driver.RunGeneratorsAndUpdateCompilation(
...
";
            ...
            var generator = new IncrementalGeneratorWrapper(
                new PipelineCallbackGenerator(
                    (ctx) =>
                    {
                        var step1 = ctx.SyntaxProvider.CreateSyntaxProvider(
                            (c, ct) =>
                            {
                                filterCalled++;
                                if (c is AssignmentExpressionSyntax)
                                    cts.Cancel();
                                return true;
                            },
                            (a, _) => a
            ...
            var generator = new IncrementalGeneratorWrapper(
                new PipelineCallbackGenerator(
                    (ctx) =>
                    {
                        ...
                        var step2 = ctx.SyntaxProvider.CreateSyntaxProvider(
                            (c, ct) =>
                            {
                                return true;
                            },
