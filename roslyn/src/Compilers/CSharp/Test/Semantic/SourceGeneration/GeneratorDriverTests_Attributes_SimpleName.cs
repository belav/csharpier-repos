...
";
        ...
        Assert.Collection(
            ...
            step =>
            {
                ...
                Assert.False(
                    step.Outputs.Any(
                        o => o.Value is ClassDeclarationSyntax { Identifier.ValueText: "D" }
                    )
                );
            }
        );
...
";
        ...
        Assert.Collection(
            ...
            step =>
            {
                Assert.False(
                    step.Outputs.Any(
                        o => o.Value is ClassDeclarationSyntax { Identifier.ValueText: "C" }
                    )
                );
                Assert.True(
                    step.Outputs.Last().Value
...
";
        ...
        driver = driver.RunGenerators(
            compilation.RemoveSyntaxTrees(compilation.SyntaxTrees.Last())
        );
        runResult = driver.GetRunResult().Results[0];
...
";
        ...
        driver = driver.RunGenerators(
            compilation.RemoveSyntaxTrees(compilation.SyntaxTrees.First())
        );
        runResult = driver.GetRunResult().Results[0];
        ...
        Assert.False(
            runResult.TrackedSteps.ContainsKey("compilationUnitAndGlobalAliases_ForAttribute")
        );
    }
