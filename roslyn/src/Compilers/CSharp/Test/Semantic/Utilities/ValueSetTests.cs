    ...
    public class ValueSetTests
    {
        ...
        public void TestUnion_01()
        {
            for (int i = 0; i < 100; i++)
            {
                ...
                if ((i1 + 1) >= i2)
                    continue;
                IValueSet<int> values1 = ForInt.Related(LessThanOrEqual, i1)
                    .Union(ForInt.Related(GreaterThanOrEqual, i2));
                Assert.Equal($"[{int.MinValue}..{i1}],[{i2}..{int.MaxValue}]", values1.ToString());
                IValueSet<int> values2 = ForInt.Related(GreaterThanOrEqual, i2)
