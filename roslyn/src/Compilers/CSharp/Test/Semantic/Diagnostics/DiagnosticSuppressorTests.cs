...
class C
{
    ...
    private readonly int f;
            ...
            compilation.VerifyDiagnostics(
                ...
                //     private readonly int f;
                Diagnostic(ErrorCode.WRN_UnreferencedField, "f", isSuppressed: false)
                    .WithArguments("C.f")
                    .WithLocation(5, 26)
            );
            ...
            VerifySuppressedDiagnostics(
                ...
                // (5,26): warning CS0169: The field 'C.f' is never used
                //     private readonly int f;
                Diagnostic("CS0169", "f", isSuppressed: true)
                    .WithArguments("C.f")
                    .WithLocation(5, 26)
            );
        }
                    ...
                    foreach (DiagnosticSeverity effectiveSeverity in severities)
                    {
                        var diagnostic = Diagnostic("ID1000", "class C { }", isSuppressed: true)
                            ...
                            .WithEffectiveSeverity(
                                configurable ? effectiveSeverity : defaultSeverity
                            );
                        ...
                        VerifyAnalyzerDiagnostics(
                            compilation,
                            analyzersWithoutSuppressor,
                            diagnosticNoSuppressor
                        );
...
class C { }";
            ...
            var suppressor = new DiagnosticSuppressor_UnsupportedSuppressionReported(
                analyzer.Descriptor.Id,
            ...
            EventHandler<FirstChanceExceptionEventArgs> firstChanceException = (sender, e) =>
            {
                if (e.Exception is ArgumentException && e.Exception.Message == exceptionMessage)
                {
                    exceptions.Add(e.Exception);
                }
            };
...
class C { }";
            ...
            var suppressor = new DiagnosticSuppressor_InvalidDiagnosticSuppressionReported(
                analyzer.Descriptor.Id,
...
class C { }";
            ...
            var suppressor = new DiagnosticSuppressor_NonReportedDiagnosticCannotBeSuppressed(
                analyzer.Descriptor.Id,
...
-----";
        ...
        public void TestProgrammaticSuppressionInfo_DiagnosticSuppressor()
        {
            ...
            var suppressionInfo = diagnostics.Select(d => d.ProgrammaticSuppressionInfo)
                .Single()
                .Suppressions.Single();
            Assert.Equal(suppressionId, suppressionInfo.Id);
            Assert.Equal(
                suppressor.SuppressionDescriptor.Justification,
                suppressionInfo.Justification
            );
            ...
            Assert.Equal(
                suppressor.SuppressionDescriptor.Justification,
                orderedSuppressions[0].Justification
            );
            Assert.Equal(suppressionId2, orderedSuppressions[1].Id);
            Assert.Equal(
                suppressor2.SuppressionDescriptor.Justification,
                orderedSuppressions[1].Justification
            );
        }
