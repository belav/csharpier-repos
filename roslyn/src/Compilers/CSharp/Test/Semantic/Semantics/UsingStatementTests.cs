...
";
            ...
            var lookupSymbol = model
                .LookupSymbols(memberAccessExpression.SpanStart, name: declaredLocal.Name)
                .Single();
...
";
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (6,16): error CS1674: 'method group': type used in a using statement must be implicitly
...
class C2
{
    static void Main()
    {
        using (C1 c1 = new C1())
        {
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (11,16): error CS1674: 'C1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (C1 c1 = new C1())
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "C1 c1 = new C1()")
                        .WithArguments("C1")
                        .WithLocation(11, 16)
                );
        }
...
class C2
{
    static void Main()
    {
        using (S1 s = new S1())
        {
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (5,17): error CS0111: Type 'S1' already defines a member called 'Dispose' with the same parameter
                    // types
                    //     public void Dispose() { }
                    Diagnostic(ErrorCode.ERR_MemberAlreadyExists, "Dispose")
                        .WithArguments("Dispose", "S1")
                        .WithLocation(5, 17),
                    // (12,16): error CS0121: The call is ambiguous between the following methods or properties:
                    // 'S1.Dispose()' and 'S1.Dispose()'
                    //         using (S1 c = new S1())
                    Diagnostic(ErrorCode.ERR_AmbigCall, "S1 s = new S1()")
                        .WithArguments("S1.Dispose()", "S1.Dispose()")
                        .WithLocation(12, 16),
                    // (12,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (S1 c = new S1())
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    ...
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
                        .WithLocation(12, 16),
                    // (16,16): error CS0121: The call is ambiguous between the following methods or properties:
                    // 'S1.Dispose()' and 'S1.Dispose()'
                    //         using (c1b) { }
                    Diagnostic(ErrorCode.ERR_AmbigCall, "s1b")
                        .WithArguments("S1.Dispose()", "S1.Dispose()")
                        .WithLocation(16, 16),
                    // (16,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (c1b) { }
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "s1b")
                        .WithArguments("S1")
                        .WithLocation(16, 16)
                );
        }
...
class C2
{
    static void Main()
    {
        ...
        using (s1b) { }
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (5,19): error CS0111: Type 'S1' already defines a member called 'Dispose' with the same parameter
                    // types
                    //     internal void Dispose() { }
                    Diagnostic(ErrorCode.ERR_MemberAlreadyExists, "Dispose")
                        .WithArguments("Dispose", "S1")
...
class C3
{
    static void Main()
    {
        ...
        using (s1b) { }
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (15,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (S1 s = new S1())
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
                        .WithLocation(15, 16),
                    // (19,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (s1b) { }
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "s1b")
                        .WithArguments("S1")
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    ...
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
                        .WithLocation(16, 16),
                    // (20,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (s1b) { }
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "s1b")
                        .WithArguments("S1")
...
class C2
{
    static void Main()
    {
        using (S1 s = new S1())
        {
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (11,16): error CS0176: Member 'S1.Dispose()' cannot be accessed with an instance reference;
                    // qualify it with a type name instead
                    //         using (S1 c = new S1())
                    Diagnostic(ErrorCode.ERR_ObjectProhibited, "S1 s = new S1()")
                        .WithArguments("S1.Dispose()")
                        .WithLocation(11, 16),
                    // (11,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (S1 c = new S1())
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
...
class C4
{
    static void Main()
    {
        using (S1 s = new S1())
        {
            ...
            // Extension methods should just be ignored, rather than rejected after-the-fact. So there should be
            // no error about ambiguities
            // Tracked by https://github.com/dotnet/roslyn/issues/32767

            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (20,16): error CS0121: The call is ambiguous between the following methods or properties:
                    // 'C2.Dispose(S1)' and 'C3.Dispose(S1)'
                    //         using (S1 c = new S1())
                    Diagnostic(ErrorCode.ERR_AmbigCall, "S1 s = new S1()")
                        .WithArguments("C2.Dispose(S1)", "C3.Dispose(S1)")
                        .WithLocation(20, 16),
                    // (20,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (S1 c = new S1())
...
namespace N4
{
    ...
    namespace N5
    {
        ...
        namespace N6
        {
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (37,20): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //             using (S1 s = new S1()) // error 1
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
                        .WithLocation(37, 20),
                    // (50,20): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //             using (S1 s = new S1()) // error 2
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
                        .WithLocation(50, 20),
                    // (63,20): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //             using (S1 s = new S1()) // error 3
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
                        .WithLocation(63, 20),
                    // (77,20): error CS0121: The call is ambiguous between the following methods or properties:
                    // 'N1.C2.Dispose(S1)' and 'N3.C4.Dispose(S1)'
                    //             using (S1 s = new S1())  // error 4
                    Diagnostic(ErrorCode.ERR_AmbigCall, "S1 s = new S1()")
                        .WithArguments("N1.C2.Dispose(S1)", "N3.C4.Dispose(S1)")
                        .WithLocation(77, 20),
                    // (77,20): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //             using (S1 s = new S1())  // error 4
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
                        .WithLocation(77, 20),
                    // (92,24): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //                 using (S1 s = new S1())  // error 5
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
                        .WithLocation(92, 24),
                    // (105,28): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //                     using (S1 s = new S1())  // error 6
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
...
class C4
{
    static void Main()
    {
        ...
        using (s2b) { }
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (22,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (S1 s = new S1())
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
                        .WithLocation(22, 16),
                    // (26,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (s1b) { }
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "s1b")
                        .WithArguments("S1")
                        .WithLocation(26, 16),
                    // (28,16): error CS1674: 'S2': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (S2 s = new S2())
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S2 s = new S2()")
                        .WithArguments("S2")
                        .WithLocation(28, 16),
                    // (32,16): error CS1674: 'S2': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (s2b) { }
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "s2b")
                        .WithArguments("S2")
                        .WithLocation(32, 16)
                );
        }
...
class C4
{
    static void Main()
    {
       using (S1 s = new S1())
       {
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (21,15): error CS0121: The call is ambiguous between the following methods or properties:
                    // 'C2.Dispose(S1, int)' and 'C3.Dispose(S1, int)'
                    //        using (S1 s = new S1())
                    Diagnostic(ErrorCode.ERR_AmbigCall, "S1 s = new S1()")
                        .WithArguments("C2.Dispose(S1, int)", "C3.Dispose(S1, int)")
                        .WithLocation(21, 15),
                    // (21,15): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //        using (S1 s = new S1())
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    ...
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
                        .WithLocation(15, 15),
                    // (19,15): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //        using (s1b) { }
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "s1b")
                        .WithArguments("S1")
...
class C3
{
    static void Main()
    {
       using (S1 s = new S1())
       {
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (19,15): warning CS0280: 'S1' does not implement the 'disposable' pattern. 'S1.Dispose()' has the
                    // wrong signature.
                    //        using (S1 s = new S1())
                    Diagnostic(ErrorCode.WRN_PatternBadSignature, "S1 s = new S1()")
                        .WithArguments("S1", "disposable", "S1.Dispose()")
                        .WithLocation(19, 15),
                    // (19,15): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //        using (S1 s = new S1())
...
class C3
{
    static void Main()
    {
       using (S1 s = new S1())
       {
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (16,15): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //        using (S1 s = new S1())
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
       ...
       using (S1 s = new S1())
       {
            ...
            var compilation = CreateCompilation(source)
                .VerifyDiagnostics(
                    ...
                    //        using (S1 s = new S1())
                    Diagnostic(ErrorCode.ERR_RefReadonlyLocalCause, "S1 s = new S1()")
                        .WithArguments("s", "using variable")
                        .WithLocation(15, 15)
                );
        }
...
class C2
{
    static void Main()
    {
        ...
        using (c1b) { }
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (11,16): warning CS0280: 'S1' does not implement the 'disposable' pattern. 'S1.Dispose()' has the
                    // wrong signature.
                    //         using (S1 c = new S1())
                    Diagnostic(ErrorCode.WRN_PatternBadSignature, "S1 c = new S1()")
                        .WithArguments("S1", "disposable", "S1.Dispose()")
                        .WithLocation(11, 16),
                    // (11,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (S1 c = new S1())
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 c = new S1()")
                        .WithArguments("S1")
                        .WithLocation(11, 16),
                    // (15,16): warning CS0280: 'S1' does not implement the 'disposable' pattern. 'S1.Dispose()' has the
                    // wrong signature.
                    //         using (c1b) { }
                    Diagnostic(ErrorCode.WRN_PatternBadSignature, "c1b")
                        .WithArguments("S1", "disposable", "S1.Dispose()")
                        .WithLocation(15, 16),
                    // (15,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (c1b) { }
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "c1b")
                        .WithArguments("S1")
                        .WithLocation(15, 16)
                );
        }
        ...
        using (c1b) { }
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    ...
                    //         using (S1 c = new S1())
                    Diagnostic(ErrorCode.ERR_BadAccess, "S1 c = new S1()")
                        .WithArguments("S1.Dispose()")
                        .WithLocation(11, 16),
                    // (11,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    ...
                    //         using (c1b) { }
                    Diagnostic(ErrorCode.ERR_BadAccess, "c1b")
                        .WithArguments("S1.Dispose()")
                        .WithLocation(15, 16),
                    // (15,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
...
";
            CreateCompilation(source)
                .VerifyDiagnostics(
                    ...
                    // qualify it with a type name instead
                    //         using (S1 c1 = new S1())
                    Diagnostic(ErrorCode.ERR_ObjectProhibited, "S1 c1 = new S1()")
                        .WithArguments("S1.Dispose()")
                        .WithLocation(11, 16),
                    // (11,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (S1 c1 = new S1())
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 c1 = new S1()")
                        .WithArguments("S1")
                        .WithLocation(11, 16)
                );
        }
...
class C2
{
    static void Main()
    {
        using (S1 c = new S1())
        {
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (11,16): error CS0411: The type arguments for method 'S1.Dispose<T>()' cannot be inferred from
                    // the usage. Try specifying the type arguments explicitly.
                    //         using (S1 c = new S1())
                    Diagnostic(ErrorCode.ERR_CantInferMethTypeArgs, "S1 c = new S1()")
                        .WithArguments("S1.Dispose<T>()")
                        .WithLocation(11, 16),
                    // (11,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    //         using (S1 c = new S1())
    ...
    static async Task Main()
    {
        await using (S1 c = new S1())
        {
            ...
            var compilation = CreateCompilationWithTasksExtensions(
                ...
                .VerifyDiagnostics(
                    // (16,22): error CS4012: Parameters or locals of type 'S1' cannot be declared in async methods or
                    // async lambda expressions.
                    //         await using (S1 c = new S1())
                    Diagnostic(ErrorCode.ERR_BadSpecialByRefLocal, "S1")
                        .WithArguments("S1")
                        .WithLocation(16, 22)
                );
        }
...
";
            ...
            CreateCompilation(source, parseOptions: TestOptions.Regular7_3)
                .VerifyDiagnostics(
                    // (11,16): error CS8370: Feature 'pattern-based disposal' is not available in C# 7.3. Please use
                    // language version 8.0 or greater.
                    //         using (S1 s = new S1())
                    Diagnostic(ErrorCode.ERR_FeatureNotAvailableInVersion7_3, "S1 s = new S1()")
...
";
            CreateCompilation(source, parseOptions: TestOptions.Regular7_3)
                .VerifyDiagnostics(
                    // (11,16): error CS1674: 'S1': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable' or implement a suitable 'Dispose' method.
                    //         using (S1 s = new S1())
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "S1 s = new S1()")
                        .WithArguments("S1")
                        .WithLocation(11, 16)
...
";
            ...
            compilation.VerifyDiagnostics(
                ...
                Diagnostic(
                    ...
                    @"var a = new StreamWriter(""""), b = new StreamReader("""")"
                )
            );
...
";
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    ...
                    // (11,13): warning CS0728: Possibly incorrect assignment to local 'i' which is the argument to a
                    ...
                    // (12,21): warning CS0728: Possibly incorrect assignment to local 'i' which is the argument to a
                    // using or lock statement. The Dispose call or unlocking will happen on the original value of the
                    // local.
                    Diagnostic(ErrorCode.WRN_AssignmentToLockOrDispose, "i").WithArguments("i"),
                    // (13,22): warning CS0728: Possibly incorrect assignment to local 'i' which is the argument to a
                    // using or lock statement. The Dispose call or unlocking will happen on the original value of the
                    // local.
                    Diagnostic(ErrorCode.WRN_AssignmentToLockOrDispose, "i").WithArguments("i")
                );
...
";
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    ...
                    Diagnostic(ErrorCode.ERR_IllegalRefParam, "out"),
                    // (10,13): warning CS0728: Possibly incorrect assignment to local 'i' which is the argument to a
                    // using or lock statement. The Dispose call or unlocking will happen on the original value of the
                    // local.
                    Diagnostic(ErrorCode.WRN_AssignmentToLockOrDispose, "i").WithArguments("i"),
                    // (11,21): warning CS0728: Possibly incorrect assignment to local 'i' which is the argument to a
                    // using or lock statement. The Dispose call or unlocking will happen on the original value of the
                    // local.
                    Diagnostic(ErrorCode.WRN_AssignmentToLockOrDispose, "i").WithArguments("i"),
                    // (12,22): warning CS0728: Possibly incorrect assignment to local 'i' which is the argument to a
                    // using or lock statement. The Dispose call or unlocking will happen on the original value of the
                    // local.
                    Diagnostic(ErrorCode.WRN_AssignmentToLockOrDispose, "i").WithArguments("i")
                );
...
" + _managedStruct;
            ...
            var compilation = CreateCompilation(source);
            var symbols = VerifyDeclaredSymbolForUsingStatements(
                compilation,
                1,
                "mnObj1",
                "mnObj2"
...
class C
{
        ...
        where T4 : T2
    {
        ...
        using (t4) { }
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    // (16,16): error CS1674: 'T0': type used in a using statement must be implicitly convertible to
                    // 'System.IDisposable'.
                    Diagnostic(ErrorCode.ERR_NoConvToIDisp, "t0")
                        .WithArguments("T0")
                        .WithLocation(16, 16)
                );
        }
        ...
        using (var v = null) ;
            ...
            CreateEmptyCompilation(source)
                .VerifyDiagnostics(
                    ...
                    //         using (var v = null) ;
                    Diagnostic(ErrorCode.ERR_ImplicitlyTypedVariableAssignedBadValue, "v = null")
                        .WithArguments("<null>")
                        .WithLocation(11, 20),
        ...
        using (null);
            ...
            CreateEmptyCompilation(source)
                .VerifyDiagnostics(
                    ...
                    //         using (null);
                    Diagnostic(ErrorCode.ERR_PredefinedTypeNotFound, "using")
        ...
        using (var v = v) { }
            ...
            CreateCompilation(source)
                .VerifyDiagnostics(
                    ...
                    //         using (var v = v) { }
                    Diagnostic(ErrorCode.ERR_VariableUsedBeforeDeclaration, "v")
                        .WithArguments("v")
                        .WithLocation(6, 24)
                );
        }
...
";
        ...
        private IEnumerable<ILocalSymbol> VerifyDeclaredSymbolForUsingStatements(
        ...
        private SymbolInfo VerifySymbolInfoForUsingStatements(
            CSharpCompilation compilation,
            ISymbol symbol,
            int index = 1
        )
        ...
        private ISymbol VerifyLookUpSymbolForUsingStatements(
            CSharpCompilation compilation,
            ISymbol symbol,
            int index = 1
        )
        {
            ...
            var actualSymbol = model
                .LookupSymbols(usingStatements[index - 1].SpanStart, name: symbol.Name)
                .Single();
