    ...
    public class PatternMatchingTestBase : CSharpTestBase
    {
        ...
        )
        {
            return tree.GetRoot()
                ...
                .Where(
                    p =>
                        p.Parent.Kind() == SyntaxKind.DeclarationPattern
                        || p.Parent.Kind() == SyntaxKind.VarPattern
                );
        }
        ...
        )
        {
            ...
            foreach (var reference in references)
            {
                ...
                Assert.Same(
                    symbol,
                    model
                        .LookupSymbols(reference.SpanStart, name: designation.Identifier.ValueText)
                        .Single()
                );
                Assert.True(
                    model
                        .LookupNames(reference.SpanStart)
                        .Contains(designation.Identifier.ValueText)
                );
            }
        }

        private static void AssertTypeInfo(
            SemanticModel model,
        ...
        )
        {
            ...
            Assert.NotEqual(
                symbol,
                model
                    .LookupSymbols(declarator.SpanStart, name: declarator.Identifier.ValueText)
                    .Single()
            );
            Assert.True(
                model.LookupNames(declarator.SpanStart).Contains(declarator.Identifier.ValueText)
            );
        }
        ...
        )
        {
            ...
            Assert.Same(
                symbol,
                model
                    .LookupSymbols(reference.SpanStart, name: reference.Identifier.ValueText)
                    .Single()
            );
            Assert.True(
                model.LookupNames(reference.SpanStart).Contains(reference.Identifier.ValueText)
            );
        }
        ...
        protected static void VerifyNotInScope(SemanticModel model, IdentifierNameSyntax reference)
        {
            ...
            Assert.False(
                model.LookupSymbols(reference.SpanStart, name: reference.Identifier.ValueText).Any()
            );
            Assert.False(
                model.LookupNames(reference.SpanStart).Contains(reference.Identifier.ValueText)
            );
        ...
        )
        {
            ...
            var declarator = designation
                .Ancestors()
                .OfType<VariableDeclaratorSyntax>()
                .FirstOrDefault();
            var inFieldDeclaratorArgumentlist =
                declarator != null
        ...
        )
        {
            foreach (var reference in references)
            {
                ...
                Assert.DoesNotContain(
                    reference.Identifier.ValueText,
                    model.LookupNames(reference.SpanStart)
                );
...
";
        ...
        protected static void VerifyDecisionDagDump<T>(
            Compilation comp,
        ...
        ) where T : CSharpSyntaxNode
        {
            ...
            {
                SwitchStatementSyntax n
                    => (
                        (BoundSwitchStatement)
                            binder.BindStatement(n, BindingDiagnosticBag.Discarded)
