    ...
    static void Main()
    {
        ...
        a = (object x, [A] object _) => { };
            ...
            static void verifyAttributes(CSharpCompilation comp)
            {
                ...
                var lambdas = exprs
                    .Select(e => (IMethodSymbol)model.GetSymbolInfo(e).Symbol)
                    .ToArray();
                Assert.Equal(2, lambdas.Length);
                Assert.Equal(
                    new[] { "AAttribute" },
                    getParameterAttributes(lambdas[0].Parameters[0])
                );
                ...
                Assert.Equal(
                    new[] { "AAttribute" },
                    getParameterAttributes(lambdas[1].Parameters[1])
                );
            }

            static ImmutableArray<string> getParameterAttributes(IParameterSymbol parameter) =>
                parameter.GetAttributes().SelectAsArray(a => a.ToString());
        }
