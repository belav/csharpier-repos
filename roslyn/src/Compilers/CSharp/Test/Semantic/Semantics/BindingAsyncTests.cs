...
class C
{
    public static void Main()
    {
        ...
        Action<Task> f3 = async (t) => { await t; };
            ...
            var compilation = CreateCompilationWithMscorlib45(new SyntaxTree[] { tree })
                .VerifyDiagnostics();
            ...
            Assert.True(
                parens.Count() == 2,
                "Expect exactly two parenthesized lambda expressions in the syntax tree."
            );
...
class C
{
    async extern ~C();
            ...
            CreateCompilationWithMscorlib45(source)
                .VerifyDiagnostics(
                    Diagnostic(ErrorCode.ERR_BadMemberFlag, "C").WithArguments("async"),
                    Diagnostic(ErrorCode.WRN_ExternMethodNoImplementation, "C")
                        .WithArguments("C.~C()")
                );
        }
            ...
            CreateCompilationWithMscorlib45(source)
                .VerifyDiagnostics(
                    ...
                    Diagnostic(ErrorCode.ERR_TaskRetNoObjectRequired, "return")
                        .WithArguments("C.F1()", "System.Threading.Tasks.Task")
                        .WithLocation(14, 9),
...
await M();
";
            CreateCompilationWithTasksExtensions(source)
                .VerifyDiagnostics(
                    // (6,12): error CS4016: Since this is an async method, the return expression must be of type 'int' rather than 'ValueTask<int>'
                    //     return new ValueTask<int>();
                    Diagnostic(ErrorCode.ERR_BadAsyncReturnExpression, "new ValueTask<int>()")
                        .WithArguments("int", "System.Threading.Tasks.ValueTask<int>")
                        .WithLocation(6, 12)
                );
        }
    ...
    async static Task M4(delegate*<void>[] i) { await Task.Yield(); } // 4
            ...
            CreateCompilationWithMscorlib45(source, null, TestOptions.UnsafeReleaseDll)
                .VerifyDiagnostics(
                    ...
                    //     unsafe async static Task M1(int*[] i) { await Task.Yield(); } // 1
                    Diagnostic(ErrorCode.ERR_AwaitInUnsafeContext, "await Task.Yield()")
                        .WithLocation(6, 45),
        ...
        finally
        {
            ...
            CreateCompilationWithMscorlib45(source, options: TestOptions.UnsafeReleaseDll)
                .VerifyDiagnostics(
                    ...
                    //     static Task M2()
                    Diagnostic(ErrorCode.ERR_ReturnExpected, "M2")
                        .WithArguments("Test.M2()")
                        .WithLocation(6, 17)
                );
        }
...
interface IInterface
{
    async void F();
            ...
            CreateCompilationWithMscorlib45(source, parseOptions: TestOptions.Regular7)
                .VerifyDiagnostics(
        ...
        await Task.Factory.StartNew(() => { });
            ...
            CreateCompilationWithMscorlib45(source)
                .VerifyDiagnostics(
                    ...
                    //         var tr = new TypedReference();
                    Diagnostic(ErrorCode.WRN_UnreferencedVarAssg, "tr")
                        .WithArguments("tr")
                        .WithLocation(9, 13)
                );
        }
...
#pragma warning restore CS1998
        ...
        [
            Theory,
            CombinatorialData,
            WorkItem(64964, "https://github.com/dotnet/roslyn/issues/64964")
        ]
...
""";
            ...
            var entryPoint = SynthesizedSimpleProgramEntryPointSymbol.GetSimpleProgramEntryPoint(
                comp
            );
            Assert.Equal(
                voidReturn
...
""";
            ...
            Assert.Equal(
                voidReturn ? "System.Void" : "System.Int32",
                entryPoint.ReturnType.ToTestDisplayString()
            );
        }
