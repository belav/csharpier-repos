...
public class Test2
{
    public void M()
    {
        ...
        var h = new Test<System.Delegate, D1>();
                ...
                //         var d = new Test<System.MulticastDelegate, System.Delegate>();
                Diagnostic(ErrorCode.ERR_GenericConstraintNotSatisfiedRefType, "System.Delegate")
                    .WithArguments("Test<T, U>", "System.MulticastDelegate", "U", "System.Delegate")
                    .WithLocation(14, 52),
                ...
                //         var g = new Test<D1, System.Delegate>();
                Diagnostic(ErrorCode.ERR_GenericConstraintNotSatisfiedRefType, "System.Delegate")
                    .WithArguments("Test<T, U>", "D1", "U", "System.Delegate")
                    .WithLocation(18, 30)
            );
        }
    ...
    public void M()
    {
        ...
        var h = new Test<System.MulticastDelegate, D1>();
                ...
                Diagnostic(
                    ...
                    .WithArguments("Test<T, U>", "D1", "U", "System.MulticastDelegate")
                    .WithLocation(18, 30)
            );
        }
...
public class Test2
{
    public void M<U, W>() where U : unmanaged
    {
        ...
        {
            CreateCompilation("public class Test<T> where T : unmanaged, new() {}")
                .VerifyDiagnostics(
        ...
        {
            CreateCompilation("public class Test<T> where T : System.Exception, unmanaged { }")
                .VerifyDiagnostics(
        ...
        public void UnmanagedConstraint_OtherValidTypes_Before()
        {
            Action<ModuleSymbol> validator = module =>
            {
                ...
                AssertEx.Equal(
                    new string[] { "Enum", "IDisposable" },
                    typeParameter.ConstraintTypes().Select(type => type.Name)
                );
            };
        ...
        {
            CreateCompilation("public class Test<T, U> where T : U, unmanaged { }")
                .VerifyDiagnostics(
        ...
        {
            CreateCompilation("public class Test<T, U> where T : unmanaged, U { }")
                .VerifyDiagnostics();
            CreateCompilation(
                "public class Test<T, U> where U: class where T : unmanaged, U, System.IDisposable { }"
            )
                .VerifyDiagnostics();
        }
...
class Test
{
    static void Main()
    {
        ...
        System.Console.WriteLine(local.a);
                ...
                references: new[] { reference, reference1 },
...
class Test
{
    ...
    void N()
    {
        ...
        public void UnmanagedConstraints_PointerOperations_SimpleTypes(
            string arg,
            string type,
            int size
        )
...
unsafe class Test
{
    ...
    static void Main()
    {
        M("
                ...
                expectedOutput: string.Join(Environment.NewLine, type, size)
            ).VerifyIL(
                "Test.M<T>",
                @"
{
...
unsafe public class Test
{
    ...
    public void N()
    {
        var result = M<int>();
            ...
            var value =
                (
                    (VariableDeclaratorSyntax)tree.FindNodeOrTokenByKind(
    ...
    void User()
    {
        ...
        IsNew<Wrapper<string>.S>();
            ...
            CreateCompilation(code).VerifyDiagnostics(
                ...
                Diagnostic(
                    ErrorCode.ERR_GenericConstraintNotSatisfiedValType,
                    "IsEnum<Wrapper<string>.S>"
                )
                    .WithArguments("Test.IsEnum<T>()", "System.Enum", "T", "Wrapper<string>.S")
                    .WithLocation(38, 9)
            );
        }
                ...
                symbolValidator: module =>
                {
                    Assert.True(
                        module.ContainingAssembly
                            ...
                            .TypeParameters.Single().HasUnmanagedTypeConstraint
                    );
                    Assert.True(
                        module.ContainingAssembly
...
class C
{
    unsafe void M<T>(T* a) where T : unmanaged
    {
        ...
        M(p);
            ...
            Assert.Equal(
                declaredMethod.TypeParameters.Single().GetPublicSymbol(),
    ...
    unsafe void N()
    {
        ...
        M(p);
            ...
            Assert.Equal(
                ...
                inferredMethod.TypeArguments.Single().SpecialType
            );
        }
...
unsafe class C
{
    ...
    void Test()
    {
        ...
        UnmanagedWithInterface(&a);         // fail (does not match interface)
                ...
                Diagnostic(
                    ErrorCode.ERR_GenericConstraintNotSatisfiedValType,
                    "UnmanagedWithInterface"
                )
                    .WithArguments(
                        "C.UnmanagedWithInterface<T>(T*)",
                        "System.IDisposable",
                        "T",
                        "int"
                    )
...
";
            CreateCompilation(code, options: TestOptions.UnsafeReleaseDll).VerifyDiagnostics(
                ...
                //         M2(&myStruct);
                Diagnostic(ErrorCode.ERR_ManagedAddr, "&myStruct")
...
";
            CreateCompilation(code, options: TestOptions.UnsafeReleaseDll).VerifyDiagnostics(
                ...
                //         MyStruct<InnerStruct<int>> myStruct = default;
                Diagnostic(ErrorCode.ERR_GenericConstraintNotSatisfiedValType, "InnerStruct<int>")
                    .WithArguments("MyStruct<T>", "System.IDisposable", "T", "InnerStruct<int>")
                    .WithLocation(16, 18)
            );
...
";
            CreateCompilation(code, options: TestOptions.UnsafeReleaseDll).VerifyDiagnostics(
                ...
                //     public unsafe void M2<V>(MyStruct<V>* ms) { }
                Diagnostic(ErrorCode.ERR_ManagedAddr, "ms")
...
";
            ...
            compilation.VerifyDiagnostics();
            Assert.False(
                compilation.GetMember<NamedTypeSymbol>("MyStruct").IsManagedTypeNoUseSiteDiagnostics
            );
            Assert.False(
                compilation.GetMember<NamedTypeSymbol>(
                    "YourStruct"
...
public struct Z
{
    public X<Z> field;
            ...
            Assert.False(
                compilation.GetMember<NamedTypeSymbol>("X").IsManagedTypeNoUseSiteDiagnostics
            );
            Assert.False(
