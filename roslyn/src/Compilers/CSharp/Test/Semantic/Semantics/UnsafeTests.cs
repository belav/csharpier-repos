...
";
            ...
            var fieldTypes = Enumerable.Range(0, 8)
...
";
            ...
            Assert.False(
                globalNamespace.GetMember<NamedTypeSymbol>("R")
                    .GetMember<NamedTypeSymbol>("E").IsManagedTypeNoUseSiteDiagnostics
            );
        }
...
";
            ...
            Assert.False(
                globalNamespace.GetMember<NamedTypeSymbol>("Q")
                    .GetMember<NamedTypeSymbol>("S").IsManagedTypeNoUseSiteDiagnostics
            );
            Assert.False(
                globalNamespace.GetMember<NamedTypeSymbol>("R")
...
";
            ...
            compilation.VerifyDiagnostics(
                ...
                //         var badPtr = &bad;
                Diagnostic(ErrorCode.ERR_ManagedAddr, "&bad")
                ...
                //         var badPtr2 = &bad2;
                Diagnostic(ErrorCode.ERR_ManagedAddr, "&bad2")
...
";
            CreateCompilation(text, options: TestOptions.UnsafeReleaseDll)
                .VerifyDiagnostics(
                    ...
                    //         var p3 = &anon; //CS0208
                    Diagnostic(ErrorCode.ERR_ManagedAddr, "&anon")
...
";
            ...
            var structMethod1 = structType.GetMembers("M")
                .OfType<MethodSymbol>()
                .Single(m => m.ParameterCount == 0);
            var structMethod2 = structType.GetMembers("M")
                .OfType<MethodSymbol>()
...
";
            ...
            var methodSymbol = compilation.GlobalNamespace.GetMember<NamedTypeSymbol>("C")
            ...
            foreach (
                ...
                else
                {
                    ...
                    Assert.Equal(
                        ...
                        ((IMethodSymbol)summary.Symbol).MethodKind
                    );
                }
...
";
            ...
            var declarators = tree.GetCompilationUnitRoot()
                ...
                .Reverse()
                .ToArray();
            var declaredSymbols = declarators.Select(
                    syntax => (ILocalSymbol)model.GetDeclaredSymbol(syntax)
                )
                .ToArray();
...
";
            ...
            var initializerSummaries = declarators.Select(
                    syntax => model.GetSemanticInfoSummary(syntax.Initializer.Value)
                )
                .ToArray();
            ...
            var arraySymbol = compilation.GlobalNamespace.GetMember<TypeSymbol>("C")
            ...
            var accessSymbolInfos = dereferences.Select(
