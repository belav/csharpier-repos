...
class C
{
    ...
    string this[string s] => /*<bind>*/M(s)/*</bind>*/;
            ...
            Assert.Equal(
                "System.Int32 C.M(System.Int32 i)",
                semanticInfo.CandidateSymbols[0].ToTestDisplayString()
            );
            Assert.Equal(
                "System.Int64 C.M(System.Int64 l)",
                semanticInfo.CandidateSymbols[1].ToTestDisplayString()
            );
...
class C
{
    int this[int i] => /*<bind>*/i/*</bind>*/;
            ...
            var accessor = Assert.IsType<SourcePropertyAccessorSymbol>(
...
class C
{
    ...
    public static explicit operator C(int i) => M(/*<bind>*/i/*</bind>*/);
        ...
        [
            WorkItem(1065375, "http://vstfdevdiv:8080/DevDiv2/DevDiv/_workitems/edit/1065375"),
            WorkItem(313, "CodePlex")
        ]
        [Fact]
...
"
    ...
    => P1;"
            ...
            Assert.Equal(
                "System.Int32 C.P1 { get; set; }",
                model.GetSymbolInfo(node2).Symbol.ToTestDisplayString()
            );
        }
...
"
        ...
        => P1;"
            ...
            Assert.Equal(
                1,
                tree.GetRoot().DescendantNodes().OfType<ArrowExpressionClauseSyntax>().Count()
            );
        }
...
"
            ...
            comp.VerifyDiagnostics(
                ...
                //         int Bar() { return 0; } => 0;
                Diagnostic(ErrorCode.WRN_UnreferencedLocalFunction, "Bar")
                    .WithArguments("Bar")
                    .WithLocation(6, 13)
            );
        }
...
"
            ...
            for (int i = 0; i < nodes.Length; i++)
            {
                ...
                Assert.Equal(
                    $"System.Int32 y{i}",
                    model.LookupSymbols(nodes[i].SpanStart, name: $"y{i}")
                        .Single()
                        .ToTestDisplayString()
                );
            }
