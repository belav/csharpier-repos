...
Console.WriteLine($""""""{span}"""""");";
        ...
        comp.VerifyDiagnostics(
            ...
            // Console.WriteLine($"""{span}""");
            Diagnostic(ErrorCode.ERR_FeatureInPreview, @"$""""""{span}""""""")
    ...
    public void TargetTypedInterpolationHoles_Errors(string expression)
    {
        ...
        if (expression.Contains('+'))
        {
            comp.VerifyDiagnostics(
                ...
                Diagnostic(ErrorCode.ERR_FeatureInPreview, @"$""""""{(null, default)}""""""")
                    .WithArguments("raw string literals")
                    .WithLocation(1, 26),
                ...
                // System.Console.WriteLine($"""{(null, default)}""" + $"""{new()}""");
                Diagnostic(ErrorCode.ERR_FeatureInPreview, @"$""""""{new()}""""""")
                    .WithArguments("raw string literals")
                    .WithLocation(1, 53),
        ...
        else
        {
            comp.VerifyDiagnostics(
                // (1,26): error CS8652: The feature 'raw string literals' is currently in Preview and *unsupported*. To use Preview features, use the 'preview' language version.
                // System.Console.WriteLine($"""{(null, default)}{new()}""");
                Diagnostic(ErrorCode.ERR_FeatureInPreview, @"$""""""{(null, default)}{new()}""""""")
                    .WithArguments("raw string literals")
                    .WithLocation(1, 26),
                // (1,31): error CS1503: Argument 1: cannot convert from '(<null>, default)' to 'object'
                // System.Console.WriteLine($"""{(null, default)}{new()}""");
...
";
        ...
        if (expression.Contains('+'))
        {
            comp.VerifyDiagnostics(
                ...
                // C.M(() => $"""{new S { Field = """Field""" }}""" + $"""
                Diagnostic(ErrorCode.ERR_FeatureInPreview, @"""""""Field""""""")
                    .WithArguments("raw string literals")
                    .WithLocation(3, 32),
...
";
        ...
        if (expression.Contains('+'))
        {
            comp.VerifyEmitDiagnostics(
                ...
                //         param = $"""{1}""" + $"""{2}""";
                Diagnostic(ErrorCode.ERR_FeatureInPreview, @"$""""""{1}""""""")
                    .WithArguments("raw string literals")
                    .WithLocation(5, 17),
                ...
                //         param = $"""{1}""" + $"""{2}""";
                Diagnostic(ErrorCode.ERR_FeatureInPreview, @"$""""""{2}""""""")
                    .WithArguments("raw string literals")
                    .WithLocation(5, 30)
            );
        }
