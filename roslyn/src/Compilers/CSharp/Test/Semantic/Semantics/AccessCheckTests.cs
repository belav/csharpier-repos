...
"
            ...
            c.VerifyDiagnostics(
                ...
                //             static private int n2_priv;
                Diagnostic(ErrorCode.WRN_UnreferencedFieldAssg, "n2_priv")
                    .WithArguments("C.N1.N2.n2_priv")
            );
        }
    ...
    public void m() {
        ...
        int f = D.d_pub;
            ...
            c.VerifyDiagnostics(
                ...
                //         int b = C.c_int;
                Diagnostic(ErrorCode.ERR_BadAccess, "c_int")
                    .WithArguments("C.c_int")
                    .WithLocation(6, 19),
                // (7,19): error CS0122: 'C.c_pro' is inaccessible due to its protection level
                //         int c = C.c_pro;
                Diagnostic(ErrorCode.ERR_BadAccess, "c_pro")
                    .WithArguments("C.c_pro")
                    .WithLocation(7, 19),
                // (8,19): error CS0122: 'C.c_intpro' is inaccessible due to its protection level
                //         int d = C.c_intpro;
                Diagnostic(ErrorCode.ERR_BadAccess, "c_intpro")
                    .WithArguments("C.c_intpro")
                    .WithLocation(8, 19),
                // (9,19): error CS0122: 'C.c_priv' is inaccessible due to its protection level
                //         int e = C.c_priv;
                Diagnostic(ErrorCode.ERR_BadAccess, "c_priv")
                    .WithArguments("C.c_priv")
                    .WithLocation(9, 19),
...
"
            ...
            INamedTypeSymbol classADerived =
                globalNS.GetMembers("ADerived").Single() as INamedTypeSymbol;
            INamedTypeSymbol classADerived2 =
                globalNS.GetMembers("ADerived2").Single() as INamedTypeSymbol;
            ...
            Assert.False(
                Symbol.IsSymbolAccessible(kinreturnfuncptrType.GetSymbol(), classB.GetSymbol())
            );
            Assert.False(compilation.IsSymbolAccessibleWithin(kinreturnfuncptrType, classB));
            Assert.False(
                Symbol.IsSymbolAccessible(kinparamfuncptr1Type.GetSymbol(), classB.GetSymbol())
            );
            Assert.False(compilation.IsSymbolAccessibleWithin(kinparamfuncptr1Type, classB));
            Assert.False(
                Symbol.IsSymbolAccessible(kinparamfuncptr2Type.GetSymbol(), classB.GetSymbol())
            );
            ...
            Assert.True(
                Symbol.IsSymbolAccessible(
                    protField.GetSymbol(),
                    classA.GetSymbol(),
                    classADerived.GetSymbol()
                )
            );
            ...
            Assert.True(
                compilation.IsSymbolAccessibleWithin(protField, classADerived, classADerived)
            );
            Assert.False(
                Symbol.IsSymbolAccessible(
                    protField.GetSymbol(),
                    classADerived.GetSymbol(),
                    classADerived2.GetSymbol()
                )
            );
            Assert.False(
                compilation.IsSymbolAccessibleWithin(protField, classADerived, classADerived2)
            );
            ...
            Assert.False(
                Symbol.IsSymbolAccessible(karrayType.GetSymbol(), sourceAssem.GetSymbol())
            );
            ...
            Assert.True(
                Symbol.IsSymbolAccessible(afuncptrType.GetSymbol(), sourceAssem.GetSymbol())
            );
            ...
            Assert.False(
                Symbol.IsSymbolAccessible(kinreturnfuncptrType.GetSymbol(), sourceAssem.GetSymbol())
            );
            Assert.False(compilation.IsSymbolAccessibleWithin(kinreturnfuncptrType, sourceAssem));
            Assert.False(
                Symbol.IsSymbolAccessible(kinparamfuncptr1Type.GetSymbol(), sourceAssem.GetSymbol())
            );
            Assert.False(compilation.IsSymbolAccessibleWithin(kinparamfuncptr1Type, sourceAssem));
            Assert.False(
                Symbol.IsSymbolAccessible(kinparamfuncptr2Type.GetSymbol(), sourceAssem.GetSymbol())
            );
            ...
            Assert.True(
                Symbol.IsSymbolAccessible(unknownType.GetSymbol(), sourceAssem.GetSymbol())
            );
            Assert.True(compilation.IsSymbolAccessibleWithin(unknownType, sourceAssem));
            Assert.True(
                Symbol.IsSymbolAccessible(mscorlibAssem.GetSymbol(), sourceAssem.GetSymbol())
            );
...
"
            ...
            IAliasSymbol SomeAlias = model.GetDeclaredSymbol(
                tree.GetRoot()
                    .DescendantNodes()
                    .OfType<UsingDirectiveSyntax>()
                    .Where(u => u.Alias != null)
            ...
            IFieldSymbol Outer_Inner_Field =
                Outer_Inner.GetMembers("Field").Single() as IFieldSymbol;
            ...
            IFieldSymbol Outer_ProtectedInternal =
                Outer.GetMembers("ProtectedInternal").Single() as IFieldSymbol;
            IFieldSymbol Outer_PrivateProtected =
                Outer.GetMembers("PrivateProtected").Single() as IFieldSymbol;
            ...
            Assert.True(
                compilation1.IsSymbolAccessibleWithin(Outer_ProtectedInternal, Derived, Outer)
            );
            Assert.False(
                compilation1.IsSymbolAccessibleWithin(Outer_PrivateProtected, Derived, Outer)
            );
...
"
            ...
            INamedTypeSymbol InOtherCompilation =
                c2.GlobalNamespace.GetMember("InOtherCompilation") as INamedTypeSymbol;
...
public class A
{
  internal class B
  {
    protected B(C o) {}
                ...
                assemblyName: "AccessCheckCrossAssemblyParameterProtectedMethod2"
            );
...
public class A
{
  internal class B
  {
    protected B(C o) {}
                ...
                new MetadataReference[] { MetadataReference.CreateFromImage(other) },
                assemblyName: "AccessCheckCrossAssemblyParameterProtectedMethod2"
            );
            Assert.Empty(c.GetDiagnostics());
        }
        ...
        {
            var assembly1Compilation = CreateCSharpCompilation(
                "Assembly1",
                @"public class InstancePropertyContainer
{
    internal protected int PropIntProProSet { get { return 5; } protected set { } }
            ...
            var assembly2Compilation = CreateCSharpCompilation(
                "Assembly2",
...
";
            ...
            var referenceA = MetadataReference.CreateFromImage(
                compilationVerifier.EmittedAssemblyData
            );
...
";
            ...
            var referenceB = MetadataReference.CreateFromImage(
                compilationVerifier.EmittedAssemblyData
            );
...
";
            var compilationC = CreateCompilation(
                sourceC,
                assemblyName: "C",
