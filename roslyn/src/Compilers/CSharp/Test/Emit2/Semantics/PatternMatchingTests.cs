...
";
            ...
            compilation.VerifyDiagnostics(
                ...
                //             case typeof(string):
                Diagnostic(ErrorCode.ERR_ConstantValueOfTypeExpected, "typeof(string)")
...
";
            ...
            compilation.VerifyDiagnostics(
                ...
                //             case _:
                Diagnostic(ErrorCode.ERR_ConstantValueOfTypeExpected, "_")
                    .WithArguments("int")
                    .WithLocation(12, 18)
            );
        }
    ...
    private static object M() => null;
            ...
            var compilation = CreateCompilation(program)
                .VerifyDiagnostics(
                    ...
                    //             case true ? x3 : 4:
                    Diagnostic(ErrorCode.ERR_ConstantValueOfTypeExpected, "true ? x3 : 4")
                        .WithArguments("int")
                        .WithLocation(9, 18),
...
class Program
{
    ...
    static bool F2(Span<char> span) => span is $""{n}"";
            ...
            comp.VerifyEmitDiagnostics(
                ...
                //     static bool F1(ReadOnlySpan<char> span) => span is $"{123}";
                Diagnostic(ErrorCode.ERR_ConstantValueOfTypeExpected, @"$""{123}""")
                    .WithArguments("System.ReadOnlySpan<char>")
                    .WithLocation(5, 56),
                // (6,48): error CS9133: A constant value of type 'Span<char>' is expected
                //     static bool F2(Span<char> span) => span is $"{n}";
                Diagnostic(ErrorCode.ERR_ConstantValueOfTypeExpected, @"$""{n}""")
                    .WithArguments("System.Span<char>")
                    .WithLocation(6, 48)
            );
        }
...
class Program
{
    ...
    static bool F2(Span<char> span, bool b) => span is (b ? """" : ""123"");
            ...
            comp.VerifyEmitDiagnostics(
                ...
                //     static bool F1(ReadOnlySpan<char> span, bool b) => span is (b ? "" : "ABC");
                Diagnostic(ErrorCode.ERR_ConstantValueOfTypeExpected, @"b ? """" : ""ABC""")
                    .WithArguments("System.ReadOnlySpan<char>")
                    .WithLocation(4, 65),
                // (5,57): error CS9133: A constant value of type 'Span<char>' is expected
                //     static bool F2(Span<char> span, bool b) => span is (b ? "" : "123");
                Diagnostic(ErrorCode.ERR_ConstantValueOfTypeExpected, @"b ? """" : ""123""")
                    .WithArguments("System.Span<char>")
                    .WithLocation(5, 57)
            );
        }
    ...
    static bool M(A a) => a switch { ""implicitA"" => true, _ => false };
            ...
            CreateCompilationWithSpanAndMemoryExtensions(
                ...
                .VerifyDiagnostics(
                    ...
                    //     static bool M(A a) => a switch { "implicitA" => true, _ => false };
                    Diagnostic(ErrorCode.ERR_ConstantValueOfTypeExpected, @"""implicitA""")
                        .WithArguments("A")
                        .WithLocation(8, 38)
                );
        }
    ...
    static bool M(A a) => a switch { (A)""castedA"" => true, _ => false };
            ...
            CreateCompilationWithSpanAndMemoryExtensions(
                ...
                .VerifyDiagnostics(
                    // (8,38): error CS9133: A constant value of type 'A' is expected
                    //     static bool M(A a) => a switch { (A)"castedA" => true, _ => false };
                    Diagnostic(ErrorCode.ERR_ConstantValueOfTypeExpected, @"(A)""castedA""")
                        .WithArguments("A")
                        .WithLocation(8, 38)
                );
        }
