...
class Test<T> where T : unmanaged
{
            ...
            CreateCompilation(text, options: TestOptions.DebugDll.WithOutputKind(outputKind))
                .VerifyDiagnostics(
...
class Test<T> where T : unmanaged
{
            ...
            switch (accessibility)
            {
                case Accessibility.Internal:
                {
                    ...
                    Assert.Equal(
                        ...
                        isUnmanagedTypeAttributes[0].AttributeClass.ToDisplayString()
                    );
                    Assert.Equal(
                        AttributeDescription.CodeAnalysisEmbeddedAttribute.FullName,
                        isUnmanagedTypeAttributes[1].AttributeClass.ToDisplayString()
                    );
                    break;
                }
        ...
        {
            var isUnmanagedAttributeTypeName = WellKnownTypes.GetMetadataName(
