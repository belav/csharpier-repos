...
namespace N
{
    ...
    [Experimental] public enum E { A }
            ...
            comp1.VerifyDiagnostics(
                ...
                //             new B();
                Diagnostic(ErrorCode.WRN_WindowsExperimental, "B")
                    .WithArguments("N.A<T>.B")
                    .WithLocation(11, 17),
                // (12,13): warning CS8305: 'N.D<int>' is for evaluation purposes only and is subject to change or removal in future updates.
                //             D<int> d = null;
                Diagnostic(ErrorCode.WRN_WindowsExperimental, "D<int>")
                    .WithArguments("N.D<int>")
                    .WithLocation(12, 13)
            );
    ...
    static void F(A a)
    {
        ...
        (new A.B()).ToString();
            ...
            var comp = CreateCompilationWithMscorlib40AndSystemCore(
                new[]
                {
                    Parse(DeprecatedAttributeSource),
                    Parse(ExperimentalAttributeSource),
                    Parse(source),
                }
            );
            comp.VerifyDiagnostics(
                ...
                //     static void F(A a)
                Diagnostic(ErrorCode.WRN_WindowsExperimental, "A")
                    .WithArguments("A")
                    .WithLocation(20, 19),
                ...
                //         a.F2();
                Diagnostic(ErrorCode.ERR_DeprecatedSymbolStr, "a.F2()")
                    .WithArguments("A.F2()", "")
                    .WithLocation(24, 9),
                // (25,9): warning CS0618: 'A.F3()' is obsolete: ''
                //         a.F3();
                Diagnostic(ErrorCode.WRN_DeprecatedSymbolStr, "a.F3()")
                    .WithArguments("A.F3()", "")
                    .WithLocation(25, 9),
                // (26,9): error CS0619: 'A.F4()' is obsolete: ''
                //         a.F4();
                Diagnostic(ErrorCode.ERR_DeprecatedSymbolStr, "a.F4()")
                    .WithArguments("A.F4()", "")
                    .WithLocation(26, 9),
                ...
                //         (new A.B()).ToString();
                Diagnostic(ErrorCode.WRN_WindowsExperimental, "A.B")
                    .WithArguments("A.B")
                    .WithLocation(27, 14)
            );
        }
...
#pragma warning disable 8321 // Unreferenced local function
        ...
        void local3()
        {
            ...
            var comp = CreateCompilationWithMscorlib40AndSystemCore(
                new[] { DeprecatedAttributeSource, ExperimentalAttributeSource, source },
                parseOptions: TestOptions.Regular9
            );
            comp.VerifyDiagnostics(
                // (7,9): warning CS0618: 'local1()' is obsolete: ''
                //         local1(); // 1
                Diagnostic(ErrorCode.WRN_DeprecatedSymbolStr, "local1()")
                    .WithArguments("local1()", "")
                    .WithLocation(7, 9),
                // (8,9): error CS0619: 'local2()' is obsolete: ''
                //         local2(); // 2
                Diagnostic(ErrorCode.ERR_DeprecatedSymbolStr, "local2()")
...
class C
{
    ...
    static object FB() => new B();
            ...
            var comp = CreateCompilationWithMscorlib40AndSystemCore(
                new[] { Parse(ExperimentalAttributeSource), Parse(source) }
            );
            comp.VerifyDiagnostics(
...
class B
{
    A F() => null;
            ...
            comp.VerifyDiagnostics(
                ...
                // [MyAttribute]
                Diagnostic(ErrorCode.WRN_DeprecatedSymbol, "MyAttribute")
...
class B
{
    A F() => null;
            ...
            comp.VerifyDiagnostics(
                ...
                // [MyAttribute]
                Diagnostic(ErrorCode.WRN_WindowsExperimental, "MyAttribute")
                    .WithArguments("MyAttribute")
                    .WithLocation(8, 2),
...
class C
{
            ...
            comp.VerifyDiagnostics(
                ...
                // [A]
                Diagnostic(ErrorCode.WRN_WindowsExperimental, "A")
                    .WithArguments("AAttribute")
                    .WithLocation(9, 2),
                // (4,2): warning CS0612: 'BAttribute' is obsolete
                // [B]
                Diagnostic(ErrorCode.WRN_DeprecatedSymbol, "B")
                    .WithArguments("BAttribute")
                    .WithLocation(4, 2),
                ...
                @"class C
{
    ...
    static void Main()
    {
        ...
        F(default(EP));
            ...
            var comp2 = CreateCompilationWithMscorlib40AndSystemCore(
                source2,
            ...
            comp2.VerifyDiagnostics(
                ...
                //         F(default(SA));
                Diagnostic(ErrorCode.WRN_DeprecatedSymbolStr, "SA")
                    .WithArguments("SA", "DA")
                    .WithLocation(6, 19),
                // (7,19): error CS0619: 'SB' is obsolete: 'DB'
                //         F(default(SB));
                Diagnostic(ErrorCode.ERR_DeprecatedSymbolStr, "SB")
                    .WithArguments("SB", "DB")
                    .WithLocation(7, 19),
                // (8,19): warning CS0618: 'SC' is obsolete: 'OC'
                //         F(default(SC));
                Diagnostic(ErrorCode.WRN_DeprecatedSymbolStr, "SC")
                    .WithArguments("SC", "OC")
                    .WithLocation(8, 19),
                // (9,19): warning CS0618: 'SD' is obsolete: 'DC'
                //         F(default(SD));
                Diagnostic(ErrorCode.WRN_DeprecatedSymbolStr, "SD")
                    .WithArguments("SD", "DC")
                    .WithLocation(9, 19),
                // (10,19): error CS0619: 'SE' is obsolete: 'DD'
                //         F(default(SE));
                Diagnostic(ErrorCode.ERR_DeprecatedSymbolStr, "SE")
                    .WithArguments("SE", "DD")
                    .WithLocation(10, 19),
                // (11,19): error CS0619: 'SF' is obsolete: 'OF'
                //         F(default(SF));
                Diagnostic(ErrorCode.ERR_DeprecatedSymbolStr, "SF")
                    .WithArguments("SF", "OF")
                    .WithLocation(11, 19),
                // (12,19): warning CS0618: 'IG' is obsolete: 'DG'
                //         F(default(IG));
                Diagnostic(ErrorCode.WRN_DeprecatedSymbolStr, "IG")
                    .WithArguments("IG", "DG")
                    .WithLocation(12, 19),
                // (13,19): warning CS0618: 'IH' is obsolete: 'DH'
                //         F(default(IH));
                Diagnostic(ErrorCode.WRN_DeprecatedSymbolStr, "IH")
                    .WithArguments("IH", "DH")
                    .WithLocation(13, 19),
                // (14,19): warning CS0618: 'II' is obsolete: 'DI'
                //         F(default(II));
                Diagnostic(ErrorCode.WRN_DeprecatedSymbolStr, "II")
                    .WithArguments("II", "DI")
                    .WithLocation(14, 19),
                // (15,19): error CS0619: 'IJ' is obsolete: 'DJ'
                //         F(default(IJ));
                Diagnostic(ErrorCode.ERR_DeprecatedSymbolStr, "IJ")
                    .WithArguments("IJ", "DJ")
                    .WithLocation(15, 19),
                // (16,19): error CS0619: 'IK' is obsolete: 'DK'
                //         F(default(IK));
                Diagnostic(ErrorCode.ERR_DeprecatedSymbolStr, "IK")
                    .WithArguments("IK", "DK")
                    .WithLocation(16, 19),
                // (17,19): warning CS0618: 'EM' is obsolete: 'OM'
                //         F(default(EM));
                Diagnostic(ErrorCode.WRN_DeprecatedSymbolStr, "EM")
                    .WithArguments("EM", "OM")
                    .WithLocation(17, 19),
                // (18,19): error CS0619: 'EN' is obsolete: 'ON'
                //         F(default(EN));
                Diagnostic(ErrorCode.ERR_DeprecatedSymbolStr, "EN")
                    .WithArguments("EN", "ON")
                    .WithLocation(18, 19),
                // (19,19): warning CS0618: 'EO' is obsolete: 'DO'
                //         F(default(EO));
                Diagnostic(ErrorCode.WRN_DeprecatedSymbolStr, "EO")
                    .WithArguments("EO", "DO")
                    .WithLocation(19, 19),
                // (20,19): error CS0619: 'EP' is obsolete: 'DP'
                //         F(default(EP));
                Diagnostic(ErrorCode.ERR_DeprecatedSymbolStr, "EP")
                    .WithArguments("EP", "DP")
                    .WithLocation(20, 19)
            );
        }
