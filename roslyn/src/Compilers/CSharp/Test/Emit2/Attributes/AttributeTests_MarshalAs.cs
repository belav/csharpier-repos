...
";
            CreateCompilation(source, options: TestOptions.ReleaseDll.WithWarningLevel(7))
                .VerifyDiagnostics();

            CreateCompilation(source, options: TestOptions.ReleaseDll.WithWarningLevel(8))
                .VerifyDiagnostics(
                    // (6,6): warning CS9125: Attribute parameter 'SizeConst' must be specified.
                    //     [MarshalAs(UnmanagedType.ByValArray)]
                    Diagnostic(
...
";
            ...
            var cciBlobs = new Dictionary<string, byte[]>
            {
                {
                    "SafeArray5",
                    new byte[] { 0x1d, 0x09, 0x75 }.Append(
                        e.GetBytes(
