    ...
    void M(int i)
    {
        ...
        public void TestSuppression_CompilerSyntaxBindingError_AndSuppressibleWarning()
        {
            ...
            compilation.VerifyDiagnostics(
                ...
                //                         new MyClass().MyPrivateMethod();
                Diagnostic(ErrorCode.ERR_BadAccess, "MyPrivateMethod")
                    .WithArguments("MyClass.MyPrivateMethod(int)")
                    .WithLocation(17, 39)
            );
        ...
        public void TestSuppression_CompilerSyntaxDeclarationError_AndSuppressibleWarning()
        {
            ...
            compilation.VerifyDiagnostics(
                ...
                //                     public extern abstract void MyFaultyMethod()
                Diagnostic(ErrorCode.ERR_AbstractAndExtern, "MyFaultyMethod")
                    .WithArguments("MyAbstractClass.MyFaultyMethod()")
                    .WithLocation(16, 49)
            );
...
-----";
        ...
        [
            Theory,
            CombinatorialData,
            WorkItem("https://devdiv.visualstudio.com/DevDiv/_workitems/edit/1819603")
        ]
        public async Task TestSuppressionAcrossCallsIntoCompilationWithAnalyzers(
            bool withSuppressor
        )
        {
            string source = @"class C1 { }";
            ...
            var suppressor = new DiagnosticSuppressorForMultipleIds(
                analyzer1.Descriptor.Id,
                analyzer2.Descriptor.Id
            );
            var analyzersAndSuppressors = new DiagnosticAnalyzer[]
            {
                analyzer1,
            ...
            var compilationWithAnalyzers = new CompilationWithAnalyzers(
                compilation,
                analyzersAndSuppressors.ToImmutableArray(),
                options
            );
            var tree = compilation.SyntaxTrees[0];
            var semanticModel = compilation.GetSemanticModel(tree);
            var analyzers = withSuppressor
                ? ImmutableArray.Create<DiagnosticAnalyzer>(analyzer1, suppressor)
                : ImmutableArray.Create<DiagnosticAnalyzer>(analyzer1);
            var diagnostics1 = await compilationWithAnalyzers.GetAnalyzerSemanticDiagnosticsAsync(
                semanticModel,
                filterSpan: null,
                analyzers,
            ...
            analyzers = withSuppressor
                ? ImmutableArray.Create<DiagnosticAnalyzer>(analyzer2, suppressor)
                : ImmutableArray.Create<DiagnosticAnalyzer>(analyzer2);
            var diagnostics2 = await compilationWithAnalyzers.GetAnalyzerSemanticDiagnosticsAsync(
                semanticModel,
                filterSpan: null,
                analyzers,
                CancellationToken.None
