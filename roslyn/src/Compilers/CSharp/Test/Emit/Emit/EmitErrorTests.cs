...
";
            ...
            compilation1.VerifyDiagnostics(
                ...
                //     public static S Goo(S p = 42) { return p; }
                Diagnostic(ErrorCode.ERR_NoConversionForDefaultParam, "p")
                    .WithArguments("int", "S")
                    .WithLocation(9, 27)
            );
...
";
        ...
        public void ModuleNameMismatch()
        {
            ...
            var netModule = CreateCompilation(
                moduleSource,
            ...
            var compilationOK = CreateCompilation(
                source,
                new MetadataReference[]
                {
                    moduleMetadata.GetReference(filePath: @"R:\A\B\ModuleNameMismatch.netmodule")
                }
            );

            CompileAndVerify(compilationOK);

            var compilationError = CreateCompilation(
                source,
                new MetadataReference[]
                {
                    moduleMetadata.GetReference(filePath: @"R:\A\B\ModuleNameMismatch.mod")
                }
            );
            ...
            compilationError.VerifyDiagnostics(
                ...
                Diagnostic(ErrorCode.ERR_NetModuleNameMismatch)
                    .WithArguments("ModuleNameMismatch.netmodule", "ModuleNameMismatch.mod")
            );
        }
...
"
            ...
            warnOpts.Add(
                MessageProvider.Instance.GetIdForErrorCode((int)ErrorCode.WRN_UnreferencedVarAssg),
                ReportDiagnostic.Suppress
            );
            var compilation1 = CreateCompilation(
                builder.ToString(),
                null,
                TestOptions.DebugDll.WithSpecificDiagnosticOptions(warnOpts)
            );
