...
class C
{
    static object obj = null!;
        ...
        )
        {
            ...
            {
                var text = sourceLines[expectedSpanResult.StartLine]
                    .Substring(expectedSpanResult.StartColumn);
                Assert.True(
                    text.StartsWith(expectedSpanResult.TextStart),
                    $"Text doesn't start with {expectedSpanResult.TextStart}. Text is: {text}"
                );
        ...
        )
        {
            ...
            var actual =
                ...
                let hashAlg = (hashAlgGuid == sha1) ? " (SHA1)"
