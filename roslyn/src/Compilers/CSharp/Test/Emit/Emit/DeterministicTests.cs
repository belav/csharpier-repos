...
namespace Microsoft.CodeAnalysis.CSharp.UnitTests.Emit
{
    ...
    public class DeterministicTests : EmitMetadataTestBase
    {
        ...
        private (ImmutableArray<byte> pe, ImmutableArray<byte> pdb) EmitDeterministic(
            string source,
            Platform platform,
        ...
        )
        {
            ...
            var compilation = CreateEmptyCompilation(
                ...
                references: new[] { MscorlibRef, SystemCoreRef, CSharpRef },
                options: options
            );
            ...
            var pdbStream =
                (pdbFormat == DebugInformationFormat.Embedded) ? null : new MemoryStream();

            return (
                pe: compilation.EmitToArray(
                    EmitOptions.Default.WithDebugInformationFormat(pdbFormat),
                    pdbStream: pdbStream
                ),
        ...
        public void BanVersionWildcards()
        {
            ...
            var resultDeterministic = compilationDeterministic.Emit(
                Stream.Null,
                pdbStream: Stream.Null
            );
            var resultNonDeterministic = compilationNonDeterministic.Emit(
                Stream.Null,
                pdbStream: Stream.Null
            );
                ...
                @"class Program
{
    public static void Main(string[] args) {}
        ...
        public void PlatformChangeTimestamp()
        {
            ...
            Assert.NotEqual(
                peReader1.PEHeaders.CoffHeader.TimeDateStamp,
                ...
                @"class Program
{
    ...
    CHANGE
            ...
            var mvid2 = CompiledGuid(
                source.Replace("CHANGE", "private void M() { }"),
                "X1",
                TestOptions.DebugDll,
                emitRefAssembly
            );
...
namespace N
{
    ...
    class Program
    {
        ...
        public void CompareAllBytesEmitted_Release(DebugInformationFormat pdbFormat)
        {
            ...
            var result3 = EmitDeterministic(
                CompareAllBytesEmitted_Source,
                Platform.X64,
                pdbFormat,
                optimize: true
            );
            var result4 = EmitDeterministic(
                CompareAllBytesEmitted_Source,
                Platform.X64,
                pdbFormat,
                optimize: true
            ...
            var result5 = EmitDeterministic(
                CompareAllBytesEmitted_Source,
                Platform.Arm64,
                pdbFormat,
                optimize: true
            );
            var result6 = EmitDeterministic(
                CompareAllBytesEmitted_Source,
                Platform.Arm64,
                pdbFormat,
                optimize: true
...
";
            ...
            static IEnumerable<string> getNamesOfForwardedTypes(AssemblySymbol assembly)
            {
                return getForwardedTypes(assembly)
                    .Select(t => t.ToDisplayString(SymbolDisplayFormat.QualifiedNameArityFormat));
            }
        ...
        [ConditionalFact(
            typeof(ClrOnly),
            Reason = "Static execution is runtime defined and this tests Clr behavior only"
        )]
...
";
            ...
            for (int i = 0; i < max; i++)
            {
                int j = (i + 7) % max;
                sourceBuilder.Add(
                    source.Replace("NUMBER1", i.ToString()).Replace("NUMBER2", j.ToString())
                );
            }
