...
"
        ...
        [
            ConditionalTheory(
                typeof(ClrOnly),
                Reason = "https://github.com/mono/mono/issues/10834"
            ),
            CombinatorialData
        ]
...
public class C
{
    ...
    public static void M<T>("
                + modifier
                + @" T value) where T : struct, IEnumerable<int>
    {
        foreach (var x in value) { }
    }
}
...
""";
            var libUser = CreateCompilation(
                libUser_cs,
                references: new[] { libOrig.EmitToImageReference() }
            );
