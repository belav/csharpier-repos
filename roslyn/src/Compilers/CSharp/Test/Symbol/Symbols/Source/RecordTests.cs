    ...
    public C(int a, int b) // overload
    {
            ...
            foreach (MethodSymbol ctor in ctors)
            {
                ...
                else
                {
                    ...
                    Assert.True(
                        c.Equals(ctor.Parameters[0].Type, TypeCompareKind.ConsiderEverything)
                    );
                }
            }
        }
...
"
            ...
            comp.VerifyDiagnostics(
                ...
                //     public bool Equals(C c) => throw null;
                Diagnostic(ErrorCode.ERR_NotOverridableAPIInRecord, "Equals")
                    .WithArguments("C.Equals(C)")
                    .WithLocation(4, 17),
...
"
            ...
            comp.VerifyDiagnostics(
                ...
                //     public int Equals(object o) => throw null;
                Diagnostic(ErrorCode.WRN_NewOrOverrideExpected, "Equals")
                    .WithArguments("C.Equals(object)", "object.Equals(object)")
                    .WithLocation(4, 16),
...
";
            ...
            Assert.Equal(
                new[] { "C..ctor(System.Int32 X, System.Int32 Y)", "C..ctor(C original)" },
                comp.GetTypeByMetadataName("C")!.Constructors.Select(m => m.ToTestDisplayString())
            );
        }
...
data struct S2(int X, int Y);";
            ...
            comp.VerifyDiagnostics(
                ...
                // data class C1 { }
                Diagnostic(ErrorCode.ERR_SingleTypeNameNotFound, "data")
                    .WithArguments("data")
                    .WithLocation(2, 1),
...
enum G : C { }";
            ...
            comp.VerifyDiagnostics(
                ...
                // interface E : C { }
                Diagnostic(ErrorCode.ERR_NonInterfaceInInterfaceList, "C")
                    .WithArguments("C")
                    .WithLocation(6, 15),
...
";
            ...
            comp2.VerifyDiagnostics(
                ...
                // record E : A { }
                Diagnostic(ErrorCode.ERR_OverrideNotExpected, "E")
                    .WithArguments("E.EqualityContract")
                    .WithLocation(3, 8),
                // (3,8): error CS0115: 'E.Equals(A?)': no suitable method found to override
                // record E : A { }
                Diagnostic(ErrorCode.ERR_OverrideNotExpected, "E")
                    .WithArguments("E.Equals(A?)")
                    .WithLocation(3, 8),
                // (3,8): error CS0115: 'E.PrintMembers(StringBuilder)': no suitable method found to override
                // record E : A { }
                Diagnostic(ErrorCode.ERR_OverrideNotExpected, "E")
                    .WithArguments("E.PrintMembers(System.Text.StringBuilder)")
                    .WithLocation(3, 8),
                // (3,8): error CS8867: No accessible copy constructor found in base type 'A'.
                // record E : A { }
                Diagnostic(ErrorCode.ERR_NoCopyConstructorInBaseType, "E")
                    .WithArguments("A")
                    .WithLocation(3, 8),
                // (3,12): error CS8864: Records may only inherit from object or another record
                // record E : A { }
...
class C
{
    public static void Main()
    {
        ...
        r4 = r4 with { };
            ...
            Assert.True(clone2.OverriddenMethod.Equals(clone, TypeCompareKind.ConsiderEverything));
            Assert.Equal(
                "R R2." + WellKnownMemberNames.CloneMethodName + "()",
            ...
            Assert.Equal(
                "R R3." + WellKnownMemberNames.CloneMethodName + "()",
            ...
            Assert.Equal(
                "R R4." + WellKnownMemberNames.CloneMethodName + "()",
            ...
            Assert.Equal(
                "R R5." + WellKnownMemberNames.CloneMethodName + "()",
...
";
            CompileAndVerify(src1 + src2, expectedOutput: "C { X = 11, Y = 22 }")
                .VerifyDiagnostics();
            CompileAndVerify(new[] { src1, src2 }, expectedOutput: "C { X = 11, Y = 22 }")
                .VerifyDiagnostics();
        }
...
";
            CompileAndVerify(src1 + src2, expectedOutput: "C { Y = 22, X = 11 }")
                .VerifyDiagnostics();
            CompileAndVerify(new[] { src1, src2 }, expectedOutput: "C { Y = 22, X = 11 }")
                .VerifyDiagnostics();
        }
...
";
            CompileAndVerify(src1 + src2, expectedOutput: "C { X = 11, U = 44, Y = 22 }")
                .VerifyDiagnostics();
            CompileAndVerify(new[] { src1, src2 }, expectedOutput: "C { X = 11, U = 44, Y = 22 }")
                .VerifyDiagnostics();
        }
...
";
            CompileAndVerify(src1 + src2, expectedOutput: "C { Y = 22, X = 11, U = 44 }")
                .VerifyDiagnostics();
            CompileAndVerify(new[] { src1, src2 }, expectedOutput: "C { Y = 22, X = 11, U = 44 }")
                .VerifyDiagnostics();
        }
