...
namespace Microsoft.CodeAnalysis.CSharp.UnitTests.Symbols.Metadata.PE
{
    public class TypeForwarders : CSharpTestBase
    {
        ...
        private void TestTypeForwarderHelper(AssemblySymbol[] assemblies)
        {
            ...
            Assert.True(
                module1.TypeRefHandleToTypeMap.Values.Contains((TypeSymbol)base4.OriginalDefinition)
            );
            Assert.True(
                module1.TypeRefHandleToTypeMap.Values.Contains((TypeSymbol)base6.OriginalDefinition)
            );

            Assert.Equal(base1, assembly2.CachedTypeByEmittedName(base1.ToTestDisplayString()));
            Assert.Equal(
                base4.OriginalDefinition,
...
";
            ...
            compilation.VerifyDiagnostics(
                ...
                Diagnostic(ErrorCode.ERR_SingleTypeNameNotFoundFwd, "Base")
                    .WithArguments(
                        "Base",
                        "pe3, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null"
                    ),
...
";
            ...
            compilation.VerifyDiagnostics(
                ...
                //         us.Goo();
                Diagnostic(ErrorCode.ERR_CycleInTypeForwarder, "us.Goo")
                    .WithArguments("Cycle", "pe2")
            );
        }
...
";
            ...
            Assert.True(
                actualNamespaces.SetEquals(expectedNamespaces, EqualityComparer<string>.Default)
            );
        }
...
";
            ...
            CheckForwarderEmit(
                ...
                source2,
                "NS.Forwarded",
                "NS.Forwarded+Internal",
                "NS.Forwarded+Protected",
                "NS.Forwarded+ProtectedInternal"
            );
        }
        ...
        )
        {
            ...
            Action<ModuleSymbol> metadataValidator = module =>
            {
                ...
                if (module is PEModuleSymbol)
                {
                    Assert.Equal(
                        0,
                        assembly.GetAttributes(AttributeDescription.TypeForwardedToAttribute)
                            .Count()
                    );
                }
            ...
            )
            {
                ...
                Assert.Equal(
                    forwardedTypeFullNames.Length,
                    metadataReader.GetTableRowCount(TableIndex.ExportedType)
                );

                int i = 0;
                foreach (var exportedType in metadataReader.ExportedTypes)
                {
                    ValidateExportedTypeRow(
                        exportedType,
                        metadataReader,
                        forwardedTypeFullNames[i]
                    );
                    i++;
                }
            }
...
{}";
            ...
            var forwardedTypesCompilation = CreateCompilation(
                forwardedTypes,
                options: TestOptions.ReleaseDll,
...
[assembly: System.Runtime.CompilerServices.TypeForwardedToAttribute(typeof(CF1))]
            ...
            var modCompilation = CreateCompilation(
                mod,
                references: new[] { new CSharpCompilationReference(forwardedTypesCompilation) },
                options: TestOptions.ReleaseModule
            );
...
";
            ...
            token = appModule.GetTypeRef(
                appModule.GetAssemblyRef("mscorlib"),
                "System.Runtime.CompilerServices",
                "AssemblyAttributesGoHereM"
            );
            Assert.True(token.IsNil); //could the type ref be located? If not then the attribute's not there.
        ...
        private void CheckForwarderEmit2(
            string source0,
            string source1,
            string source2,
            params string[] forwardedTypeFullNames
        )
        {
            ...
            var asm0 = ModuleMetadata.CreateFromImage(
                CompileAndVerify(comp0, verify: Verification.Skipped).EmittedAssemblyData
            );
            ...
            Action<ModuleSymbol> metadataValidator = module =>
            {
                ...
                Assert.Equal(
                    0,
                    assembly.GetAttributes(AttributeDescription.TypeForwardedToAttribute).Count()
                );
        ...
        private static void ValidateExportedTypeRow(
        ...
        )
        {
            ...
            if (expectedFullName.Contains('+'))
            {
                Assert.Equal(
                    (TypeAttributes)0,
                    exportedTypeRow.Attributes & TypeAttributesMissing.Forwarder
                );
                Assert.Equal(0, exportedTypeRow.GetTypeDefinitionId());
                Assert.Equal(
                    expectedType.Split('+').Last(),
                    reader.GetString(exportedTypeRow.Name)
                ); //Only the actual type name.
            ...
            {
                Assert.Equal(
                    TypeAttributes.NotPublic | TypeAttributesMissing.Forwarder,
...
",
                new[] { new CSharpCompilationReference(cA_v1) },
                options: TestOptions.ReleaseDll,
                assemblyName: "B"
            );
...
",
                new[] { new CSharpCompilationReference(cC_v1) },
                options: TestOptions.ReleaseDll,
                assemblyName: "A"
            );
...
",
            ...
            var cA_v3 = CreateCompilation(
                @"",
                new[] { cD_ImageRef, new CSharpCompilationReference(cC_v1) },
                options: TestOptions.ReleaseDll,
                assemblyName: "A"
            );
...
[assembly: System.Runtime.CompilerServices.TypeForwardedTo(typeof(MyNamespace.MyClass))]";
            var compB2 = CreateCompilation(
                sourceB2,
                references: new[] { refBImpl },
                assemblyName: "B"
            );
