...
namespace Microsoft.CodeAnalysis.CSharp.UnitTests.Symbols.Metadata.PE
{
    public class LoadingMethods : CSharpTestBase
    {
        ...
        public void TestExplicitImplementationConstructed()
        {
            ...
            var classMethod = (MethodSymbol)@class.GetMembers("IGeneric<System.Int32>.Method")
                .Last(); //this assumes decl order
            Assert.Equal(
        ...
        public void TestExplicitImplementationInterfaceCycleSuccess()
        {
            ...
            var cyclicInterface = (NamedTypeSymbol)globalNamespace.GetTypeMembers("ImplementsSelf")
                .Single();
            Assert.Equal(TypeKind.Interface, cyclicInterface.TypeKind);

            var implementedInterface = (NamedTypeSymbol)globalNamespace.GetTypeMembers("I1")
                .Single();
            Assert.Equal(TypeKind.Interface, implementedInterface.TypeKind);

            var interface2Method = (MethodSymbol)implementedInterface.GetMembers("Method1")
                .Single();

            var @class = (NamedTypeSymbol)globalNamespace.GetTypeMembers("InterfaceCycleSuccess")
                .Single();
            Assert.Equal(TypeKind.Class, @class.TypeKind);
        ...
        public void TestExplicitImplementationInterfaceCycleFailure()
        {
            ...
            var @class = (NamedTypeSymbol)globalNamespace.GetTypeMembers("InterfaceCycleFailure")
                .Single();
            Assert.Equal(TypeKind.Class, @class.TypeKind);
            Assert.True(@class.Interfaces().Contains(cyclicInterface));
