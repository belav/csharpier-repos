...
class C : IB, IC
{
    ...
    public object this[string index]
    {
        ...
        set { }
            ...
            var typeAProperties = type.GetMembers()
            ...
            var bridgeMethods = sourceType.GetSynthesizedExplicitImplementations(
                CancellationToken.None
            );
            Assert.Equal(2, bridgeMethods.Length);
            Assert.True(
                bridgeMethods
                    .Select(GetPairForSynthesizedExplicitImplementation)
                    .SetEquals(
                        new[]
                        {
                            new KeyValuePair<string, string>(
                                "System.Object IC.this[System.String index].get",
                                "System.Object B.this[System.String index].get"
                            ),
                            new KeyValuePair<string, string>(
                                "void IC.this[System.String index].set",
                                "void B.this[System.String index].set"
                            ),
                        }
                    )
            );
            ...
            bridgeMethods = sourceType.GetSynthesizedExplicitImplementations(
                CancellationToken.None
            );
            ...
            Assert.True(
                bridgeMethods
                    ...
                    .SetEquals(
                        ...
                        {
                            new KeyValuePair<string, string>(
                                "System.Object IB.this[System.String index].get",
                                "System.Object C.this[System.String index].get"
                            ),
                            new KeyValuePair<string, string>(
                                "System.Object IC.this[System.String index].get",
                                "System.Object C.this[System.String index].get"
                            ...
                            new KeyValuePair<string, string>(
                                "void IC.this[System.String index].set",
                                "void C.this[System.String index].set"
                            ),
                        }
                    )
            );
        ...
        )
        {
            ...
            if (hasSet)
            {
                ...
                CheckParameters(
                    setter.Parameters,
                    expectedParameterTypes.Concat(new[] { expectedType }).ToArray()
                );
            }
            else
            {
...
";
            ...
            var synthesizedExplicitImplementations = @class.GetSynthesizedExplicitImplementations(
            ...
            Assert.True(
                interface2Getter == interface1GetterImpl ^ interface2Getter == interface2GetterImpl
            );
        }
            ...
            CompileWithCustomILSource(
                ...
                compilation =>
                {
                    ...
                    var synthesizedExplicitImplementations =
                        @class.GetSynthesizedExplicitImplementations(default(CancellationToken));
                    Assert.Equal(2, synthesizedExplicitImplementations.Length);
...
";
            ...
            Assert.Equal(
                baseIndexer,
                derivedIndexer.OverriddenOrHiddenMembers.HiddenMembers.Single()
            );
        }
            ...
            Func<bool, Action<ModuleSymbol>> validator = isFromSource =>
                module =>
                {
                    ...
                    var classEventImpl = @class
                        .GetMembers()
                        .Where(
                            m => m.GetExplicitInterfaceImplementations().Contains(interfaceEvent)
                        )
                        .Single();
                    var classPropertyImpl = @class
                        .GetMembers()
                        .Where(
                            m => m.GetExplicitInterfaceImplementations().Contains(interfaceProperty)
                        )
                        .Single();
                    var classIndexerImpl = @class
                        .GetMembers()
                        .Where(
                            m => m.GetExplicitInterfaceImplementations().Contains(interfaceIndexer)
                        )
                        .Single();
...
public class Derived : Base
{
    ...
    void Method()
    {
        int x = base[1];
            ...
            var indexerAccessSyntax = GetElementAccessExpressions(tree.GetCompilationUnitRoot())
                .Single();
...
";
            var compilation = CreateCompilation(
                source,
                new[] { TestReferences.SymbolsTests.Indexers }
            );
            ...
            compilation.VerifyDiagnostics(
                ...
                Diagnostic(ErrorCode.ERR_BindToBogusProp2, "r[ref x]")
                    .WithArguments(
                        "RefIndexer.this[ref int]",
                        "RefIndexer.get_Item(ref int)",
                        "RefIndexer.set_Item(ref int, int)"
                    ),
...
";
            ...
            compilation.VerifyDiagnostics(
                ...
                Diagnostic(ErrorCode.ERR_UnimplementedInterfaceMember, "IRefIndexer")
                    .WithArguments("Test", "IRefIndexer.get_Item(ref int)"),
                // (2,7): error CS0535: 'Test' does not implement interface member 'IRefIndexer.set_Item(ref int, int)'
                Diagnostic(ErrorCode.ERR_UnimplementedInterfaceMember, "IRefIndexer")
                    .WithArguments("Test", "IRefIndexer.set_Item(ref int, int)")
            );
        }
...
";
            ...
            CreateCompilation(
                ...
                .VerifyDiagnostics(
                    ...
                    //     const string Constant2 = A.Constant2;
                    Diagnostic(ErrorCode.ERR_FeatureNotAvailableInVersion7, "Constant2")
                        .WithArguments("default interface implementation", "8.0")
                        .WithLocation(13, 18),
...
";
            ...
            Assert.True(
                indexer
                    .GetAttributes()
                    .Single()
                    .IsTargetAttribute(indexer, AttributeDescription.IndexerNameAttribute)
            );
                ...
                @"public class C
{
    ...
    void Method()
    {
        ...
        x = this[new int[1]];
            ...
            CheckOverloadResolutionResults(
                tree,
                model,
...
";
            ...
            var derivedIndexerGroup = derivedIndexers.Concat(
            ...
            foreach (var syntax in receiverSyntaxes)
            {
                ...
                {
                    Assert.True(
                        indexerGroup.SetEquals(
                            baseIndexerGroup.GetPublicSymbols(),
                ...
                else if (type.Equals(derivedType))
                {
                    Assert.True(
                        indexerGroup.SetEquals(
                            derivedIndexerGroup.GetPublicSymbols(),
                            EqualityComparer<IPropertySymbol>.Default
                        )
                    );
                }
...
";
            ...
            var publicIndexer = indexers.Single(
                indexer => indexer.DeclaredAccessibility == Accessibility.Public
            );
            var protectedIndexer = indexers.Single(
                indexer => indexer.DeclaredAccessibility == Accessibility.Protected
            );
            var privateIndexer = indexers.Single(
                indexer => indexer.DeclaredAccessibility == Accessibility.Private
            );

            var receiverSyntaxes = elementAccessSyntaxes
                .Select(access => access.Expression)
                .ToArray();
            ...
            Assert.True(
                model
                    .GetIndexerGroup(receiverSyntaxes[0])
                    .SetEquals(
                        ImmutableArray
                            .Create<PropertySymbol>(publicIndexer, protectedIndexer, privateIndexer)
                            .GetPublicSymbols(),
                        EqualityComparer<IPropertySymbol>.Default
                    )
            );
            ...
            Assert.True(
                model
                    .GetIndexerGroup(receiverSyntaxes[1])
                    .SetEquals(
                        ImmutableArray
                            .Create<PropertySymbol>(publicIndexer, protectedIndexer)
                            .GetPublicSymbols(),
                        EqualityComparer<IPropertySymbol>.Default
                    )
            );
            ...
            Assert.True(
                model
                    .GetIndexerGroup(receiverSyntaxes[2])
                    .SetEquals(
...
";
            ...
            var unsubstitutedIndexer = unsubstitutedType.GetMember<SourcePropertySymbol>(
            ...
            var substitutedType = unsubstitutedType.Construct(
                comp.GetSpecialType(SpecialType.System_Int32)
            );
            var substitutedIndexer = substitutedType.GetMember<SubstitutedPropertySymbol>(
                WellKnownMemberNames.Indexer
            );
