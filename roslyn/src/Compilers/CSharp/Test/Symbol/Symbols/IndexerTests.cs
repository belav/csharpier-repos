            ...
            Func<bool, Action<ModuleSymbol>> validator = isFromSource =>
                module =>
                {
                    ...
                    var classPropertyImpl = @class
                        .GetMembers()
                        .Where(m =>
                            m.GetExplicitInterfaceImplementations().Contains(interfaceProperty)
                        )
                        .Single();
                    var classIndexerImpl = @class
                        .GetMembers()
                        .Where(m =>
                            m.GetExplicitInterfaceImplementations().Contains(interfaceIndexer)
                        )
                        .Single();
...
";
            ...
            var publicIndexer = indexers.Single(indexer =>
                indexer.DeclaredAccessibility == Accessibility.Public
            );
            var protectedIndexer = indexers.Single(indexer =>
                indexer.DeclaredAccessibility == Accessibility.Protected
            );
            var privateIndexer = indexers.Single(indexer =>
    ...
    object this[object o] { get { return null; } }
        ...
        public void DefaultSyntaxValueReentrancy_01()
        {
            ...
            compilation.VerifyDiagnostics(
                ...
                //     public A(int x, A a = new A()[1]) { }
                Diagnostic(ErrorCode.ERR_DefaultValueMustBeConstant, "new A()[1]")
                    .WithArguments("a")
                    .WithLocation(8, 27)
            );
        }
