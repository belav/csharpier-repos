...
class D : C
{
    internal override void M1(string s1) // 1
    {
            ...
            var comp = CreateCompilation(
                new[] { text1, AllowNullAttributeDefinition },
                parseOptions: TestOptions.RegularWithExtendedPartialMethods
            );
            comp.VerifyDiagnostics(
...
partial class D
{
    ...
    public override partial string ToString() => ""hello"";
            ...
            Assert.Equal(
                ...
                method.OverriddenMethod.ToTestDisplayString()
            );
...
";
            var comp = CreateCompilation(
                new[] { text, AllowNullAttributeDefinition },
                parseOptions: TestOptions.RegularWithExtendedPartialMethods
            );
            comp.VerifyDiagnostics(
...
";
            ...
            comp.VerifyDiagnostics(
                ...
                //     public partial void M([AllowNull] string s1);
                Diagnostic(ErrorCode.ERR_DuplicateAttribute, "AllowNull")
                    .WithArguments("AllowNull")
                    .WithLocation(13, 28),
                // (20,9): warning CS8602: Dereference of a possibly null reference.
                //         s1.ToString(); // 2
                Diagnostic(ErrorCode.WRN_NullReferenceReceiver, "s1").WithLocation(20, 9)
...
";
            ...
            Assert.Equal(
                ...
                GetAttributeNames(definitionPart.GetReturnTypeAttributes())
            );
            ...
            Assert.Equal(
                expectedAttributeNames,
                GetAttributeNames(implementationPart.GetReturnTypeAttributes())
            );
            ...
            void validator(ModuleSymbol module)
            {
                ...
                Assert.Equal(
                    expectedAttributeNames,
                    GetAttributeNames(method.GetReturnTypeAttributes())
                );
            }
        }
    ...
    public override partial int[] M2() => new int[0];
            ...
            comp.VerifyDiagnostics(
                ...
                Diagnostic(ErrorCode.ERR_UnimplementedAbstractMethod, "Derived")
                    .WithArguments("Derived", "Base.M1(int[*])")
                    .WithLocation(2, 22),
...
partial class C
{
    ...
    partial void M2() { }
            ...
            void verifyPublicAPI(IMethodSymbol defSymbol)
            {
                ...
                Assert.Equal(
                    implSymbol.ToTestDisplayString(includeNonNullable: false),
                    defSymbol.ToTestDisplayString(includeNonNullable: false)
                );
            }
        }
    }
