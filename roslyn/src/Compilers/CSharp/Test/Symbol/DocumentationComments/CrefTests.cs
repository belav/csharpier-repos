...
";
            ...
            var expectedSymbol = compilation.GlobalNamespace.GetMember<NamedTypeSymbol>("Outer")
                .GetMember<MethodSymbol>("M");
            var actualSymbol = GetReferencedSymbol(crefSyntax, compilation);
            Assert.Equal(expectedSymbol, actualSymbol);
        }
...
";
            ...
            foreach (var crefSyntax in GetCrefSyntaxes(compilation))
            {
                Assert.Equal(
                    ...
                    GetReferencedSymbol(crefSyntax, compilation).Kind
                );
            }
        }
...
";
            ...
            foreach (var crefSyntax in GetCrefSyntaxes(compilation))
            {
                ...
                string arguments = text.Contains("C()")
                    ? "C()"
                    : text.Contains("C") ? "C" : text.Contains("D()") ? "D()" : "D";
                Assert.Null(
                    GetReferencedSymbol(
                        crefSyntax,
                        compilation,
...
";
            ...
            Symbol actualWinner;
            var actualCandidates = GetReferencedSymbols(
                crefSyntax,
                compilation,
                out actualWinner,
...
";
            ...
            var expectedSymbol = compilation.GlobalNamespace.GetMembers("B")
...
";
            ...
            var actualTypeParameters = actualSymbol.GetMemberTypeArgumentsNoUseSiteDiagnostics()
                .Cast<CrefTypeParameterSymbol>()
                .ToArray();
            ...
            Assert.Equal(
                actualTypeParameters[0].GetHashCode(),
                actualTypeParameters[1].GetHashCode()
            );
            ...
            Assert.NotEqual(
                actualTypeParameters[0].GetHashCode(),
                actualTypeParameters[2].GetHashCode()
            );
#endif
        }
...
";
            ...
            var expectedOriginalDefinitionSymbol =
                compilation.GlobalNamespace.GetMember<NamedTypeSymbol>("A")
                    .GetMember<NamedTypeSymbol>("B")
                    .GetMember<MethodSymbol>("M");
            var actualSymbol = GetReferencedSymbol(crefSyntax, compilation);
            Assert.Equal(expectedOriginalDefinitionSymbol, actualSymbol.OriginalDefinition);

            var expectedOriginalParameterTypes = expectedOriginalDefinitionSymbol.Parameters.Select(
                    p => p.Type
                )
                .Cast<TypeParameterSymbol>();
            var actualParameterTypes = actualSymbol.GetParameters()
                .Select(p => p.Type)
                .Cast<TypeParameterSymbol>();

            AssertEx.Equal(
                expectedOriginalParameterTypes.Select(t => t.Ordinal),
                actualParameterTypes.Select(t => t.Ordinal)
            );
            AssertEx.None(
                expectedOriginalParameterTypes.Zip(actualParameterTypes, object.Equals),
                x => x
            );
        }
...
";
            ...
            Assert.Equal(
                actualWinner.ContainingType.GetMembers(actualWinner.Name)
...
";
            ...
            Assert.Equal(
                actualParameterType,
                actualWinner.ContainingType.ContainingType.TypeArguments().Single()
            );
...
";
            ...
            Symbol actualSymbol;
            var symbols = GetReferencedSymbols(
                crefSyntax,
                compilation,
                out actualSymbol,
...
";
            ...
            var expectedSymbol = compilation.GlobalNamespace.GetMember<NamedTypeSymbol>("Base")
...
";
            var lib1 = CreateCompilationWithMscorlib40AndDocumentationComments(
                libSource,
                assemblyName: "Lib1"
            );
            var lib2 = CreateCompilationWithMscorlib40AndDocumentationComments(
                libSource,
                assemblyName: "Lib2"
            );
            ...
            var compilation = CreateCompilationWithMscorlib40AndDocumentationComments(
                source,
                new[] { new CSharpCompilationReference(lib1), new CSharpCompilationReference(lib2) }
            );
            var crefSyntax = GetCrefSyntaxes(compilation).Single();
            ...
            Assert.Equal(2, actualCandidates.Length);
            AssertEx.SetEqual(
                actualCandidates.Select(sym => sym.ContainingAssembly.Name),
                "Lib1",
                "Lib2"
            );
            ...
            AssertEx.SetEqual(
                info.CandidateSymbols.Select(sym => sym.ContainingAssembly.Name),
                "Lib1",
                "Lib2"
            );
        }
...
";
            var lib = CreateCompilationWithMscorlib40AndDocumentationComments(
                libSource,
                assemblyName: "Lib"
            );
...
";
            ...
            var actualMethodSymbol2 =
                model.GetSymbolInfo(((NameMemberCrefSyntax)crefSyntax.Member).Name).Symbol;
...
";
            ...
            ITypeParameterSymbol actualJ;
            {
                ...
                var actualTypeArgument =
                    model.GetSymbolInfo(left.TypeArgumentList.Arguments.Single()).Symbol;
            ...
            ITypeParameterSymbol actualK;
            {
                ...
                var actualTypeArgument =
                    model.GetSymbolInfo(right.TypeArgumentList.Arguments.Single()).Symbol;
            ...
            ITypeParameterSymbol actualL;
            {
                ...
                var actualParameterTypes = nameMemberSyntax.Parameters.Parameters.Select(
                        syntax => model.GetSymbolInfo(syntax.Type).Symbol
                    )
                    .ToArray();
                ...
                Assert.Equal(
                    typeA.Construct(typeInt).GetMember<INamedTypeSymbol>("B").Construct(actualK),
                    actualParameterTypes[5]
                );
            }
        }
...
";
            ...
            var expectedParameterType =
                compilation.GetSpecialType(SpecialType.System_Int32).ISymbol;
            var actualParameterType =
                model.GetSymbolInfo(crefSyntax.Parameters.Parameters.Single().Type).Symbol;
...
";
            ...
            var expectedOperator =
                typeC.GetMember<MethodSymbol>(WellKnownMemberNames.UnaryPlusOperatorName).ISymbol;
...
";
            var compilation = CreateCompilationWithMscorlib40AndDocumentationComments(source);
            var model = compilation.GetSemanticModel(compilation.SyntaxTrees.Single());
            var crefSyntax = (ConversionOperatorMemberCrefSyntax)GetCrefSyntaxes(compilation)
                .Single();

            var typeC = compilation.GlobalNamespace.GetMember<NamedTypeSymbol>("C");

            var expectedOperator =
                typeC.GetMember<MethodSymbol>(WellKnownMemberNames.ImplicitConversionName).ISymbol;
            var actualOperator = model.GetSymbolInfo(crefSyntax).Symbol;
            Assert.Equal(expectedOperator, actualOperator);
...
";
            ...
            Assert.Equal(
                ...
                ((IMethodSymbol)info.CandidateSymbols[0]).MethodKind
            );
        }
...
";
            ...
            AssertEx.SetEqual(
                model.LookupSymbols(position).Select(SymbolExtensions.ToTestDisplayString),
            ...
            // Consider inaccessible symbols, as in Dev11
            Assert.Equal(
                typeInner.GetPublicSymbol(),
                model.LookupSymbols(position, typeOuter.GetPublicSymbol(), typeInner.Name).Single()
            );
        }
...
";
            ...
            CreateCompilationWithMscorlib40AndDocumentationComments(source)
                .VerifyDiagnostics(
                    ...
                    // (3,22): warning CS1658: Identifier expected. See also error CS1001.
                    // /// Error <see cref="2"/>
                    Diagnostic(ErrorCode.WRN_ErrorOverride, "2")
                        .WithArguments("Identifier expected", "1001"),
                    ...
                    // /// Error <see cref="3A"/>
                    Diagnostic(ErrorCode.WRN_ErrorOverride, "3")
                        .WithArguments("Identifier expected", "1001"),
...
";
            ...
            CreateCompilationWithMscorlib40AndDocumentationComments(source)
                .VerifyDiagnostics(
                    ...
                    // /// Error <see cref="G{T}.M&lt;3&gt;"/>
                    Diagnostic(ErrorCode.WRN_BadXMLRefSyntax, "G{T}.M&lt;")
                        .WithArguments("G{T}.M&lt;3&gt;"),
...
";
            CreateCompilationWithMscorlib40AndDocumentationComments(source)
                .VerifyDiagnostics(
                    ...
                    // /// <typeparam name="T"/> -- warning
                    Diagnostic(ErrorCode.WRN_DuplicateTypeParamTag, @"name=""T""")
                        .WithArguments("T"),
                    // (7,20): warning CS1710: XML comment has a duplicate typeparam tag for 'U'
                    //     /// <typeparam name="U"/> -- warning
                    Diagnostic(ErrorCode.WRN_DuplicateTypeParamTag, @"name=""U""")
                        .WithArguments("U"),
...
";
            CreateCompilationWithMscorlib40AndDocumentationComments(source)
                .VerifyDiagnostics(
                    ...
                    //     public void Method() { }
                    Diagnostic(ErrorCode.WRN_MissingXMLComment, "Method")
                    ...
                    //     public int Property { get; set; }
                    Diagnostic(ErrorCode.WRN_MissingXMLComment, "Property")
                    ...
                    //     public delegate void Delegate();
                    Diagnostic(ErrorCode.WRN_MissingXMLComment, "Delegate")
...
";
            ...
            var tree1 = Parse(
                source1,
                options: TestOptions.Regular.WithDocumentationMode(DocumentationMode.Diagnose)
                    .WithLanguageVersion(LanguageVersion.Latest)
            );
            var tree2 = Parse(
                source2,
                options: TestOptions.Regular.WithDocumentationMode(DocumentationMode.None)
                    .WithLanguageVersion(LanguageVersion.Latest)
            );

            // This scenario does not exist in dev11, but the diagnostics seem reasonable.
            CreateCompilation(new[] { tree1, tree2 })
                .VerifyDiagnostics(
                    // (5,22): warning CS1591: Missing XML comment for publicly visible type or member 'D'
                    // public partial class D { }
                    Diagnostic(ErrorCode.WRN_MissingXMLComment, "D")
                        .WithArguments("D")
                        .WithLocation(5, 22),
                    // (7,22): warning CS1591: Missing XML comment for publicly visible type or member 'E'
                    // public partial class E { }
                    Diagnostic(ErrorCode.WRN_MissingXMLComment, "E")
                        .WithArguments("E")
                        .WithLocation(7, 22)
                );
        }
...
";
            ...
            CreateCompilationWithMscorlib40AndDocumentationComments(source)
                .VerifyDiagnostics(
                    ...
                    // /// <see cref="M(Q[])"/>
                    Diagnostic(ErrorCode.WRN_BadXMLRefParamType, "Q[]")
                        .WithArguments("Q[]", "M(Q[])"),
            ...
            CreateCompilationWithMscorlib40AndDocumentationComments(source)
                .VerifyDiagnostics(
                    ...
                    Diagnostic(ErrorCode.WRN_BadXMLRefReturnType, "Q")
                        .WithArguments("Q", "explicit operator Q"),
                    ...
                    Diagnostic(ErrorCode.WRN_BadXMLRefReturnType, "C{Q}")
                        .WithArguments("C{Q}", "explicit operator C{Q}"),
...
";
            ...
            Assert.Equal(
                ...
                crefSyntax.Ancestors().First(n => n.IsStructuredTrivia).ParentTrivia.Token.Kind()
            );
...
";
            ...
            Assert.Equal(
                "A<A<T>> A<T>.B<U>.op_Explicit(A<T>.B<U> b)",
                symbols[4].ToTestDisplayString()
            );
        }
...
";
            ...
            Assert.Equal(
                "void A<T>.B<U>.M<System.Int32>(A<System.Int32> a)",
                symbols[2].ToTestDisplayString()
            );
            Assert.Equal(
                "void A<T>.B<U>.M<V>(A<System.Int32> a)",
                symbols[3].ToTestDisplayString()
            );
            Assert.Equal(
                "A<System.Int32> A<T>.B<U>.op_Explicit(A<T>.B<U> b)",
                symbols[4].ToTestDisplayString()
            );
        }
...
";
            ...
            var overridingMethod = compilation.GlobalNamespace.GetMember<INamedTypeSymbol>(
                    "Derived"
                )
...
";
            ...
            var overridingMethod = compilation.GlobalNamespace.GetMember<NamedTypeSymbol>("Middle")
                .GetMember<MethodSymbol>("M");
...
";
            ...
            var compilation = (Compilation)CreateCompilationWithMscorlib40AndSystemCore(
                source,
                parseOptions: TestOptions.RegularWithDocumentationComments
            );
            compilation.VerifyDiagnostics(
            ...
            var global = compilation.GlobalNamespace;
            var derivedM1 = global.GetMember<INamedTypeSymbol>("Derived")
                .GetMember<IMethodSymbol>("M1");
...
";
            ...
            var constructor = compilation.GlobalNamespace.GetMember<INamedTypeSymbol>("G")
                .InstanceConstructors.Single();
...
";
            ...
            var outerCtor = compilation.GlobalNamespace.GetMember<INamedTypeSymbol>("Outer")
                .InstanceConstructors.Single();
...
";
            ...
            compilation.VerifyDiagnostics(
                ...
                // /// <see cref="operator q"/>
                Diagnostic(ErrorCode.WRN_ErrorOverride, "q")
                    .WithArguments("Overloadable operator expected", "1037")
            );
...
";
            ...
            Assert.Equal(
                ...
                model.GetSymbolInfo(crefSyntaxes[0]).Symbol
            );
            Assert.Equal(
                compilation.GetSpecialType(SpecialType.System_Int64),
                model.GetSymbolInfo(crefSyntaxes[1]).Symbol
            );
        }
...
";
            ...
            Assert.Equal(
                "System.IntPtr System.IntPtr.op_Explicit(System.Void* value)",
                model.GetSymbolInfo(cref).Symbol.ToTestDisplayString()
            );
        }
...
";
            ...
            var expectedSymbol = compilation.GlobalNamespace.GetMember<INamedTypeSymbol>("Outer")
...
";
            ...
            var parameterTypeContainingType = parameterType.DescendantNodes()
...
";
            ...
            var delegateConstructor = compilation.GlobalNamespace.GetMember<INamespaceSymbol>(
                    "System"
                )
                .GetMembers("Action")
...
";
            ...
            var expectedSymbolOriginalDefinition =
                compilation.GlobalNamespace.GetMember<INamedTypeSymbol>("C")
                    .InstanceConstructors.Single();
...
";
            ...
            var crefSyntax = GetCrefSyntaxes(compilation).Single();
            var typeParameterSyntax = crefSyntax.DescendantNodes()
            ...
            Assert.IsType<CrefTypeParameterSymbol>(
                ((CSharp.Symbols.PublicModel.Symbol)typeParameterSymbol).UnderlyingSymbol
            );

            var members = model.LookupSymbols(
                typeParameterSyntax.SpanStart,
                (ITypeSymbol)typeParameterSymbol
            );
...
";
            ...
            var crefSyntax = GetCrefSyntaxes(compilation).Single();
            var methodNameSyntax = crefSyntax.DescendantNodes()
                .OfType<IdentifierNameSyntax>()
                .Last();
            ...
            var members = model.LookupSymbols(
                methodNameSyntax.SpanStart,
                ((IMethodSymbol)methodSymbol).ReturnType
            );
            Assert.Equal(0, members.Length);
        }
...
";
            ...
            var expectedReturnTypeSymbol = compilation.GlobalNamespace.GetMember<INamedTypeSymbol>(
                    "X"
                )
            ...
            var expectedCrefSymbol = expectedReturnTypeSymbol.GetMember<IMethodSymbol>(
                WellKnownMemberNames.ImplicitConversionName
            );
...
";
            ...
            var parameterTypeSyntax =
                ((NameMemberCrefSyntax)crefSyntax).Parameters.Parameters[0].Type;
...
";
            ...
            var parameterTypeSyntax =
                crefSyntax.DescendantNodes().OfType<CrefParameterSyntax>().Single().Type;
...
";
            ...
            var crefSyntax = GetCrefSyntaxes(compilation).Single();
            var parameterSyntax = crefSyntax.DescendantNodes()
                .OfType<CrefParameterSyntax>()
                .Single()
                .DescendantNodes()
                .OfType<IdentifierNameSyntax>()
                .Single();
...
"
            ...
            Assert.Equal(
                "System.Action<System.Int32> P.b",
...
"
            ...
            var nonCrefPosition =
                tree.GetRoot()
                    .DescendantTrivia()
                    .Single(
                        t => t.IsKind(SyntaxKind.SingleLineDocumentationCommentTrivia)
                    ).SpanStart;

            var accessor =
                compilation.GlobalNamespace.GetMember<INamedTypeSymbol>("Outer")
                    .GetMember<INamedTypeSymbol>("Inner")
                    .GetMember<IPropertySymbol>("P").GetMethod;
            var inheritedType = compilation.GlobalNamespace.GetMember<INamedTypeSymbol>("Base")
            ...
            var crefInfo = model.GetSpeculativeSymbolInfo(
                crefPosition,
                accessorName,
                SpeculativeBindingOption.BindAsExpression
            );
            var returnInfo = model.GetSpeculativeSymbolInfo(
                crefReturnTypePosition,
                accessorName,
                SpeculativeBindingOption.BindAsExpression
            );
            var paramInfo = model.GetSpeculativeSymbolInfo(
                crefParameterTypePosition,
                accessorName,
                SpeculativeBindingOption.BindAsExpression
            );
            var nonCrefInfo = model.GetSpeculativeSymbolInfo(
                nonCrefPosition,
                accessorName,
                SpeculativeBindingOption.BindAsExpression
            );
            ...
            crefInfo = model.GetSpeculativeSymbolInfo(
                crefPosition,
                inheritedTypeName,
                SpeculativeBindingOption.BindAsExpression
            );
            returnInfo = model.GetSpeculativeSymbolInfo(
                crefReturnTypePosition,
                inheritedTypeName,
                SpeculativeBindingOption.BindAsExpression
            );
            paramInfo = model.GetSpeculativeSymbolInfo(
                crefParameterTypePosition,
                inheritedTypeName,
                SpeculativeBindingOption.BindAsExpression
            );
            nonCrefInfo = model.GetSpeculativeSymbolInfo(
                nonCrefPosition,
                inheritedTypeName,
                SpeculativeBindingOption.BindAsExpression
            );
...
";
            ...
            comp.VerifyDiagnostics(
                ...
                // /// <see cref='IDerived.P'/>
                Diagnostic(ErrorCode.WRN_BadXMLRef, "IDerived.P")
                    .WithArguments("P")
                    .WithLocation(11, 16)
            );
...
";
            ...
            comp.VerifyDiagnostics(
                ...
                // /// <see cref='I.ToString'/>
                Diagnostic(ErrorCode.WRN_BadXMLRef, "I.ToString")
...
";
            ...
            compilation.VerifyDiagnostics(
                ...
                // /// <see cref="C{}" />
                Diagnostic(ErrorCode.WRN_BadXMLRefSyntax, "C{}")
                    .WithArguments("C{}")
                    .WithLocation(3, 16),
            ...
            Assert.Equal(
                compilation.GlobalNamespace.GetMember<INamedTypeSymbol>("C"),
                actualSymbol1.OriginalDefinition
            );
            Assert.Equal(
                TypeKind.Error,
                ((INamedTypeSymbol)actualSymbol1).TypeArguments.Single().TypeKind
            );
        }
...
";
            ...
            Assert.Equal(
                compilation.GlobalNamespace.GetMember<INamedTypeSymbol>("Cat"),
                model.GetSymbolInfo(cref).Symbol
            );
        }
        ...
        private static ImmutableArray<Symbol> GetReferencedSymbols(
            CrefSyntax crefSyntax,
            CSharpCompilation compilation,
