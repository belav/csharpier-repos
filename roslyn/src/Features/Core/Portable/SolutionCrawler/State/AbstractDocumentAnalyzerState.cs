...
{
    internal abstract class AbstractDocumentAnalyzerState<T>
        : AbstractAnalyzerState<DocumentId, Document, T>
    {
        protected abstract string StateName { get; }

        protected override DocumentId GetCacheKey(Document value) => value.Id;

        protected override Solution GetSolution(Document value) => value.Project.Solution;

        protected override bool ShouldCache(Document value) => value.IsOpen();
