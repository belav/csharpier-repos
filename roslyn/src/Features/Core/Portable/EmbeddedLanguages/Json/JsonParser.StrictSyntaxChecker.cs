...
namespace Microsoft.CodeAnalysis.Features.EmbeddedLanguages.Json
{
    ...
    internal partial struct JsonParser
    {
        ...
        {
            public static EmbeddedDiagnostic? CheckRootSyntax(
                JsonCompilationUnit node,
                JsonOptions options
            )
            {
            ...
            private static EmbeddedDiagnostic? CheckToken(JsonToken token, bool allowComments) =>
                CheckTrivia(token.LeadingTrivia, allowComments)
                ?? CheckTrivia(token.TrailingTrivia, allowComments);

            private static EmbeddedDiagnostic? CheckTrivia(
                ImmutableArray<JsonTrivia> triviaList,
                bool allowComments
            )
            {
            ...
            private static EmbeddedDiagnostic? CheckObject(
                JsonObjectNode node,
            ...
            )
            {
                ...
                if (
                    !allowTrailingComma
                    && node.Sequence.NodesAndTokens.Length != 0
                    && node.Sequence.NodesAndTokens.Length % 2 == 0
                )
                    return new EmbeddedDiagnostic(
            ...
            private static EmbeddedDiagnostic? CheckArray(
                JsonArrayNode node,
                bool allowTrailingComma
            ...
            private static EmbeddedDiagnostic? CheckProperty(
...
$",
            ...
            private static EmbeddedDiagnostic? InvalidLiteral(JsonToken literalToken) =>
                new(
                    string.Format(
                        FeaturesResources._0_literal_not_allowed,
                        literalToken.VirtualChars.CreateString()
                    ),
                    literalToken.GetSpan()
                );
