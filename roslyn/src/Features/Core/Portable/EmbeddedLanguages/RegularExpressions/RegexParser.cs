...
namespace Microsoft.CodeAnalysis.EmbeddedLanguages.RegularExpressions
{
    ...
    internal partial struct RegexParser
    {
        ...
        private RegexTree ParseTree()
        {
            ...
            // are encountered.
            var expression = this.ParseAlternatingSequences(
                consumeCloseParen: true,
                isConditional: false
            );
        ...
        )
        {
            ...
            while (_currentToken.Kind == RegexKind.BarToken)
            {
                ...
                if (isConditional && builder.Count >= 3)
                {
                    ...
                    barToken = barToken.AddDiagnosticIfNone(
        ...
        )
        {
            return _currentToken.Kind switch
            {
                ...
                RegexKind.OpenBraceToken
                    => ParsePossibleUnexpectedNumericQuantifier(lastExpression),
                RegexKind.AsteriskToken
                or RegexKind.PlusToken
                or RegexKind.QuestionToken
        ...
        private readonly bool HasCapture(int value) => _captureNumbersToSpan.ContainsKey(value);

        private readonly bool HasCapture(string value) => _captureNamesToSpan.ContainsKey(value);
        ...
        {
            var currentOptions = _options;
            var result = this.ParseAlternatingSequences(
                consumeCloseParen: false,
                isConditional: true
            );
            _options = currentOptions;
