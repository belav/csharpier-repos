...
namespace Microsoft.CodeAnalysis.EditAndContinue
{
    ...
    internal sealed class EditAndContinueWorkspaceService : IEditAndContinueWorkspaceService
    {
        ...
        public async ValueTask StartDebuggingSessionAsync(
            Solution solution,
        ...
        )
        {
            ...
            var newSession = new DebuggingSession(
                ...
                _debuggingSessionTelemetry,
                _editSessionTelemetry
            );
            var previousSession = Interlocked.CompareExchange(
                ref _debuggingSession,
                newSession,
                null
            );
            Contract.ThrowIfFalse(
                previousSession == null,
                "New debugging session can't be started until the existing one has ended."
            );
        }
        ...
        internal static bool SupportsEditAndContinue(DocumentState documentState) =>
            ...
            && documentState.SupportsSyntaxTree
            && (
        ...
        )
        {
            try
            {
                ...
                var (mvid, _) = await debuggingSession
                ...
                var oldProject =
                    oldDocument?.Project
                    ?? debuggingSession.LastCommittedSolution.GetProject(project.Id);
                if (oldProject == null)
                {
            ...
            foreach (var documentId in documentIds)
            {
                ...
                )
                {
                    var document = await solution
                        ...
                        .ConfigureAwait(false);
                    var (baseDocument, _) = await lastCommittedSolution
                        .GetDocumentAndStateAsync(documentId, document, cancellationToken)
                        .ConfigureAwait(false);
                    if (baseDocument != null && document != null)
                    {
                        ...
                        {
                            spans.Add(
                                documentBaseActiveStatements.SelectAsArray(s => (s.Span, s.Flags))
                            );
                            continue;
                        }
                        ...
                        {
                            spans.Add(
                                analysis.ActiveStatements.SelectAsArray(s => (s.Span, s.Flags))
                            );
                            continue;
                        }
                    }
        ...
        private static void ReportTelemetry(DebuggingSessionTelemetry.Data data)
        {
            ...
            _ = Task.Run(
                () => LogDebuggingSessionTelemetry(data, Logger.Log, LogAggregator.GetNextId)
            );
        }

        // internal for testing
        internal static void LogDebuggingSessionTelemetry(
        ...
        internal readonly struct TestAccessor
        {
            ...
            internal void SetOutputProvider(Func<Project, CompilationOutputs> value) =>
                _service._compilationOutputsProvider = value;
            internal void SetReportTelemetry(Action<DebuggingSessionTelemetry.Data> value) =>
                _service._reportTelemetry = value;
        }
    }
}
