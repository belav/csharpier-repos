...
namespace Microsoft.CodeAnalysis.EditAndContinue
{
    ...
    internal sealed class CommittedSolution
    {
        ...
        public bool HasNoChanges(Solution solution) => _solution == solution;

        public Project? GetProject(ProjectId id) => _solution.GetProject(id);

        public Project GetRequiredProject(ProjectId id) => _solution.GetRequiredProject(id);
        ...
        /// </summary>
        public async Task<(Document? Document, DocumentState State)> GetDocumentAndStateAsync(
        ...
        private async ValueTask<(
            Optional<SourceText?> matchingSourceText,
            bool? hasDocument
        ...
        )
        {
            ...
            var maybePdbHasDocument = TryReadSourceFileChecksumFromPdb(
                document,
                out var requiredChecksum,
                out var checksumAlgorithm
            );

            var maybeMatchingSourceText =
                (maybePdbHasDocument == true)
                    ? await TryGetMatchingSourceTextAsync(
                            sourceText,
                            document.FilePath,
                            currentDocument,
                            _debuggingSession.SourceTextProvider,
                            requiredChecksum,
                            checksumAlgorithm,
                            cancellationToken
                        )
        ...
        )
        {
            ...
            return await Task.Run(
                    () =>
                        TryGetPdbMatchingSourceTextFromDisk(
                            filePath,
                            sourceText.Encoding,
                            requiredChecksum,
                            checksumAlgorithm
        ...
        internal static async Task<
            IEnumerable<KeyValuePair<DocumentId, DocumentState>>
        ...
        )
        {
            var projectTasks = documentsByProject.Select(async projectDocumentStates =>
            {
                ...
                var documentTasks = documentStates.Select(async documentState =>
                {
                    ...
                    if (documentState.SupportsEditAndContinue())
                    {
                        ...
                        if (
                            TryReadSourceFileChecksumFromPdb(
                                debugInfoReader,
                                sourceFilePath,
                                out var requiredChecksum,
                                out var checksumAlgorithm
                            ...
                            && await TryGetMatchingSourceTextAsync(
                                ...
                                .ConfigureAwait(false)
                                is { HasValue: true, Value: not null }
                        )
                        {
            ...
            return documentIdArrays
                .SelectMany(ids => ids.WhereNotNull())
                .Select(id => KeyValuePairUtil.Create(id, DocumentState.MatchesBuildOutput));
        }
            ...
            try
            {
                ...
                if (debugInfoReaderProvider == null)
                {
                    EditAndContinueService.Log.Write(
                        ...
                        compilationOutputs.AssemblyDisplayPath
                    );
                }
        ...
        private static bool IsMatchingSourceText(
            SourceText sourceText,
            ImmutableArray<byte> requiredChecksum,
            SourceHashAlgorithm checksumAlgorithm
        ) =>
            checksumAlgorithm == sourceText.ChecksumAlgorithm
            && sourceText.GetChecksum().SequenceEqual(requiredChecksum);
        ...
        private bool? TryReadSourceFileChecksumFromPdb(
            Document document,
            out ImmutableArray<byte> requiredChecksum,
        ...
        )
        {
            ...
            return TryReadSourceFileChecksumFromPdb(
                debugInfoReader,
                document.FilePath,
                out requiredChecksum,
                out checksumAlgorithm
            );
        }
        ...
        )
        {
            ...
            {
                if (
                    !debugInfoReader.TryGetDocumentChecksum(
                        sourceFilePath,
                        out checksum,
                        out var algorithmId
                    )
