...
namespace Microsoft.CodeAnalysis.EditAndContinue
{
    internal abstract class AbstractEditAndContinueAnalyzer : IEditAndContinueAnalyzer
    {
        ...
        private static TraceLog Log => EditAndContinueService.AnalysisLog;
        ...
        /// </remarks>
        internal abstract bool TryFindMemberDeclaration(
            SyntaxNode? root,
            SyntaxNode node,
            TextSpan activeSpan,
            out OneOrMany<SyntaxNode> declarations
        );
        ...
        /// </summary>
        protected abstract LambdaBody? FindEnclosingLambdaBody(
            SyntaxNode encompassingAncestor,
            SyntaxNode node
        );

        protected abstract Match<SyntaxNode> ComputeTopLevelMatch(
            SyntaxNode oldCompilationUnit,
            SyntaxNode newCompilationUnit
        );
        protected abstract BidirectionalMap<SyntaxNode>? ComputeParameterMap(
        ...
        /// </returns>
        protected abstract bool TryGetActiveSpan(
            SyntaxNode node,
            int statementPart,
            int minLength,
            out TextSpan span
        ...
        protected abstract IEnumerable<(
            ...
            int statementPart
        )> EnumerateNearStatements(SyntaxNode statement);
        ...
        /// </remarks>
        protected abstract bool AreEquivalentActiveStatements(
        ...
        private bool AreEquivalent(SyntaxToken oldToken, SyntaxToken newToken) =>
            oldToken.RawKind == newToken.RawKind
            && oldToken.Span.Length == newToken.Span.Length
        ...
        /// </summary>
        protected abstract IEnumerable<SyntaxNode> GetTopLevelTypeDeclarations(
        ...
        /// </summary>
        protected abstract IEnumerable<SyntaxNode> GetVariableUseSites(
        ...
        protected abstract bool AreHandledEventsEqual(
        ...
        internal string GetDisplayKindAndName(
            ISymbol symbol,
        ...
        )
        {
            ...
            var format = fullyQualify
                ? s_fullyQualifiedMemberDisplayFormat
                : s_unqualifiedMemberDisplayFormat;

            return (
                symbol is IParameterSymbol { ContainingType: not { TypeKind: TypeKind.Delegate } }
            )
                ? string.Format(
        ...
        internal string GetDisplayName(SyntaxNode node, EditKind editKind = EditKind.Update) =>
            TryGetDisplayName(node, editKind)
        ...
        internal virtual string GetDisplayName(IEventSymbol symbol) => FeaturesResources.event_;
        ...
        internal virtual string GetDisplayName(IMethodSymbol symbol) =>
            symbol.MethodKind switch
            {
                ...
                MethodKind.EventAdd or MethodKind.EventRaise or MethodKind.EventRemove =>
                    FeaturesResources.event_accessor,
                MethodKind.BuiltinOperator
                or MethodKind.UserDefinedOperator
                or MethodKind.Conversion => FeaturesResources.operator_,
        ...
        /// </summary>
        protected virtual string GetBodyDisplayName(
            SyntaxNode node,
            EditKind editKind = EditKind.Update
        )
        {
            var current = node.Parent;
        ...
        protected virtual string GetSuspensionPointDisplayName(
            SyntaxNode node,
            EditKind editKind
        ...
        internal abstract void ReportTopLevelSyntacticRudeEdits(
        ...
        internal abstract bool HasUnsupportedOperation(
            IEnumerable<SyntaxNode> newNodes,
            [NotNullWhen(true)] out SyntaxNode? unsupportedNode,
            out RudeEditKind rudeEdit
        );

        private bool ReportUnsupportedOperations(
        ...
        )
        {
            if (
                HasUnsupportedOperation(
                    body.GetDescendantNodes(IsNotLambda),
                    out var unsupportedNode,
                    out var rudeEdit
                )
            )
            {
        ...
        internal abstract bool TryGetLambdaBodies(
            SyntaxNode node,
            [NotNullWhen(true)] out LambdaBody? body1,
            out LambdaBody? body2
        );
        ...
        )
        {
            ...
            try
            {
                ...
                var syntaxError = syntaxDiagnostics.FirstOrDefault(d =>
                ...
                if (!hasChanges)
                {
                    ...
                    return DocumentAnalysisResults.Unchanged(
                        newDocument.Id,
                        filePath,
                        analysisStopwatch.Elapsed
                    );
                ...
                var capabilities = new EditAndContinueCapabilitiesGrantor(
                    await lazyCapabilities.GetValueAsync(cancellationToken).ConfigureAwait(false)
                );
                ...
                if (!capabilities.Grant(EditAndContinueCapabilities.Baseline))
                {
                    return DocumentAnalysisResults.SyntaxErrors(
                        ...
                        filePath,
                        ImmutableArray.Create(
                            new RudeEditDiagnostic(RudeEditKind.NotSupportedByRuntime, default)
                        ),
                        syntaxError: null,
                        analysisStopwatch.Elapsed,
                        hasChanges
                ...
                AnalyzeTrivia(topMatch, editMap, triviaEdits, lineEdits, cancellationToken);
                ...
                var newExceptionRegions = ImmutableArray.CreateBuilder<
                    ImmutableArray<SourceFileSpan>
                >(oldActiveStatements.Length);
                ...
                AnalyzeUnchangedActiveMemberBodies(
                    diagnostics,
                    syntacticEdits.Match,
                    newText,
                    oldActiveStatements,
                    newActiveStatementSpans,
                    newActiveStatements,
                ...
                var diagnostic =
                    (e is OutOfMemoryException)
                        ? new RudeEditDiagnostic(
                            ...
                            span: default,
                            arguments: [newDocument.FilePath]
                        )
                        : new RudeEditDiagnostic(
                            RudeEditKind.InternalError,
                            span: default,
                            arguments: [newDocument.FilePath, e.ToString()]
                        );
                    ...
                    try
                    {
                        ...
                        lineText = text.ToString(
                            TextSpan.FromBounds(
                                ...
                                Math.Min(diagnostic.Span.Start + 120, line.End)
                            )
                        );
                    }
                    catch
        ...
        private void ReportTopLevelSyntacticRudeEdits(
            ArrayBuilder<RudeEditDiagnostic> diagnostics,
            EditScript<SyntaxNode> syntacticEdits,
            Dictionary<SyntaxNode, EditKind> editMap
        )
        {
        ...
        internal Dictionary<SyntaxNode, EditKind> BuildEditMap(EditScript<SyntaxNode> editScript)
        {
            ...
            if (
                (
                    IsCompilationUnitWithGlobalStatements(editScript.Match.OldRoot)
        ...
        )
        {
            Debug.Assert(!newActiveStatementSpans.IsDefault);
            Debug.Assert(
                newActiveStatementSpans.IsEmpty
                    || oldActiveStatements.Length == newActiveStatementSpans.Length
            );
            ...
            for (var i = 0; i < newActiveStatements.Count; i++)
            {
                if (newActiveStatements[i] == null)
                {
                    ...
                    if (
                        node != null
                        && TryFindMemberDeclaration(
                            topMatch.OldRoot,
                            node,
                            oldStatementSpan,
                            out var oldMemberDeclarations
                        )
                    )
                    {
                        foreach (var oldMember in oldMemberDeclarations)
                        {
                            ...
                            if (
                                TryGetTrackedStatement(
                                    ...
                                    newText,
                                    newBody,
                                    out var trackedStatement,
                                    out var trackedStatementPart
                                )
                            )
                            {
                                ...
                                //   [|public int <<F = Expr()>>;|]
                                var adjustedOldStatementStart = oldBody.ContainsActiveStatementSpan(
                                    oldStatementSpan
                                )
                                    ? oldStatementSpan.Start
                                    : oldBody.Envelope.Start;

                                // The tracking span might have been moved outside of lambda.
                                // It is not an error to move the statement - we just ignore it.
                                var oldEnclosingLambdaBody = FindEnclosingLambdaBody(
                                    oldBody.EncompassingAncestor,
                                    oldMember.FindToken(adjustedOldStatementStart).Parent!
                                );
                                var newEnclosingLambdaBody = FindEnclosingLambdaBody(
                            ...
                            if (newStatement == null)
                            {
                                ...
                                oldBody.FindStatementAndPartner(
                                    oldStatementSpan,
                                    newBody,
                                    out newStatement,
                                    out statementPart
                            ...
                            {
                                var ancestors = GetExceptionHandlingAncestors(
                                    newStatement,
                                    newBody.EncompassingAncestor,
                                    oldActiveStatements[i].Statement.IsNonLeaf
                                );
                                newExceptionRegions[i] = GetExceptionRegions(
                                    ancestors,
                                    newStatement.SyntaxTree,
                                    cancellationToken
                                ).Spans;
                            }
                            ...
                            var newStatementSpan = FindClosestActiveSpan(
                                newStatement,
                                statementPart
                            );

                            newActiveStatements[i] = GetActiveStatementWithSpan(
                                oldActiveStatements[i],
                                newBody.SyntaxTree,
                                newStatementSpan,
                                diagnostics,
                                cancellationToken
        ...
        internal readonly struct ActiveNode(
            ...
            SyntaxNode oldNode,
            LambdaBody? enclosingLambdaBody,
            int statementPart,
            SyntaxNode? newTrackedNode
        )
        {
        ...
        internal readonly struct LambdaInfo
        {
            ...
            public bool HasActiveStatement => ActiveNodeIndices != null;
        ...
        )
        {
            ...
            Debug.Assert(
                oldActiveStatements.IsEmpty
                    || oldActiveStatements.Length == newActiveStatements.Count
            );
            ...
            try
            {
                ...
                if (newMemberBody == null)
                {
                    ...
                    foreach (var activeStatementIndex in activeStatementIndices)
                    {
                        ...
                        var newSpan = GetDeletedDeclarationActiveSpan(
                            topMatch.Matches,
                            oldDeclaration
                        );
                        newActiveStatements[activeStatementIndex] = GetActiveStatementWithSpan(
                            oldActiveStatements[activeStatementIndex],
                ...
                else
                {
                    foreach (var activeStatementIndex in activeStatementIndices)
                    {
                        ...
                        var oldStatementSpan = oldActiveStatements[
                            activeStatementIndex
                        ].UnmappedSpan;

                        var oldStatementSyntax = oldMemberBody.FindStatement(
                            oldStatementSpan,
                            out var statementPart
                        );
                        Contract.ThrowIfNull(oldStatementSyntax);

                        var oldEnclosingLambdaBody = FindEnclosingLambdaBody(
                            oldMemberBody.EncompassingAncestor,
                            oldStatementSyntax
                        );
                        if (oldEnclosingLambdaBody != null)
                        {
                            ...
                            if (
                                !lazyActiveOrMatchedLambdas.TryGetValue(
                        ...
                        if (
                            TryGetTrackedStatement(
                                ...
                                newText,
                                newMemberBody,
                                out var newStatementSyntax,
                                out var _
                            )
                        )
                        {
                            var newEnclosingLambdaBody = FindEnclosingLambdaBody(
                                newMemberBody.EncompassingAncestor,
                                newStatementSyntax
                            );
                            ...
                            // It is not an error to move the statement - we just ignore it.
                            if (
                                oldEnclosingLambdaBody == newEnclosingLambdaBody
                ...
                var activeNodesInBody = activeNodes
                    .Where(n => n.EnclosingLambdaBody == null)
                    .ToArray();

                var bodyMap = ComputeMatch(oldMemberBody, newMemberBody, activeNodesInBody);
                var map = IncludeLambdaBodyMaps(
                    bodyMap,
                    activeNodes,
                    ref lazyActiveOrMatchedLambdas
                );

                var oldStateMachineInfo =
                    oldMemberBody?.GetStateMachineInfo() ?? StateMachineInfo.None;
                var newStateMachineInfo =
                    newMemberBody?.GetStateMachineInfo() ?? StateMachineInfo.None;

                ReportStateMachineBodyUpdateRudeEdits(
                    diagnosticContext,
                    bodyMap,
                    oldStateMachineInfo,
                    newStateMachineInfo,
                    hasActiveStatement: activeNodesInBody.Length != 0,
                    cancellationToken
                );
                ...
                foreach (var activeNode in activeNodes)
                {
                    ...
                    var isPartiallyExecuted =
                        (
                            oldActiveStatements[activeStatementIndex].Statement.Flags
                    ...
                    if (match != null)
                    {
                        ...
                        hasMatching = oldBody.TryMatchActiveStatement(
                            newBody,
                            oldStatementSyntax,
                            ref statementPart,
                        ...
                        if (!hasMatching)
                        {
                            ...
                            hasMatching = match.Value.Forward.TryGetValue(
                                oldStatementSyntax,
                                out newStatementSyntax
                            );
                        }
            ...
            catch (Exception e) when (FatalError.ReportAndCatchUnlessCanceled(e, cancellationToken))
            {
                ...
                else
                {
                    diagnosticContext.Report(
                        ...
                        cancellationToken,
                        arguments: new[] { newMember.Name, e.ToString() }
                    );
                }
            }
        }

        private static bool TryGetTrackedStatement(
            ImmutableArray<LinePositionSpan> activeStatementSpans,
            int index,
            SourceText text,
            MemberBody body,
            [NotNullWhen(true)] out SyntaxNode? trackedStatement,
            out int trackedStatementPart
        )
        {
        ...
        private ActiveStatement GetActiveStatementWithSpan(
            UnmappedActiveStatement oldStatement,
            SyntaxTree newTree,
            TextSpan newSpan,
            ArrayBuilder<RudeEditDiagnostic> diagnostics,
            CancellationToken cancellationToken
            ...
            )
            {
                ...
                diagnostics.Add(
                    new RudeEditDiagnostic(
                        ...
                        arguments: new[]
                        {
                            string.Format(FeaturesResources._0_directive, LineDirectiveKeyword),
                        }
                    )
                );
            }
        ...
        )
        {
            if (newStatementSyntax == null)
            {
                ...
                newStatementSyntax = newEncompassingAncestor
            ...
            {
                var edits = new MapBasedLongestCommonSubsequence<SyntaxNode>(forwardMap).GetEdits(
                    oldAncestors,
                    newAncestors
                );
                ...
                if (diagnostics.Count == 0)
                {
                    ...
                    newExceptionRegions[ordinal] = GetExceptionRegions(
                        newAncestors,
                        newStatementSyntax.SyntaxTree,
                        cancellationToken
                    ).Spans;
                }
            }
        ...
        )
        {
            ...
            while (true)
            {
                foreach (var (oldNode, newNode) in currentBodyMatch.Forward)
                {
                    ...
                    {
                        lambdaBodyMatches ??= ArrayBuilder<(
                            BidirectionalMap<SyntaxNode>,
                            SyntaxNode?
                        )>.GetInstance();
                        lazyActiveOrMatchedLambdas ??= new Dictionary<LambdaBody, LambdaInfo>();
                        ...
                        {
                            lambdaBodyMatches.Add(
                                (
                                    ComputeMatch(
                                        oldLambdaBody1,
                                        newLambdaBody1,
                                        memberBodyActiveNodes,
                                        lazyActiveOrMatchedLambdas
                                    ),
                                    oldNode
                                )
        ...
        )
        {
            ...
            if (activeOrMatchedLambdas.TryGetValue(oldLambdaBody, out var info))
            {
                ...
                activeNodesInLambdaBody =
                    info.ActiveNodeIndices?.Select(i => memberBodyActiveNodes[i])
                    ?? Array.Empty<ActiveNode>();
            }
            else
            {
            ...
            var lambdaBodyMatch = ComputeMatch(
                oldLambdaBody,
                newLambdaBody,
                activeNodesInLambdaBody
            );
        ...
        private static BidirectionalMap<SyntaxNode> ComputeMatch(
            DeclarationBody? oldBody,
            DeclarationBody? newBody,
            IEnumerable<ActiveNode> activeNodes
        ) =>
        ...
        )
        {
            ...
            if (
                hasActiveStatement
                && oldStateMachineInfo.IsStateMachine != newStateMachineInfo.IsStateMachine
            )
            {
                diagnosticContext.Report(
        ...
        private static List<KeyValuePair<SyntaxNode, SyntaxNode>>? GetMatchingActiveNodes(
            IEnumerable<ActiveNode> activeNodes
        )
        {
            ...
            foreach (var activeNode in activeNodes)
            {
                if (activeNode.NewTrackedNode != null)
                {
                    ...
                    lazyKnownMatches.Add(
                        KeyValuePairUtil.Create(activeNode.OldNode, activeNode.NewTrackedNode)
                    );
                }
            }
        ...
        public ActiveStatementExceptionRegions GetExceptionRegions(
            SyntaxNode root,
        ...
        private ActiveStatementExceptionRegions GetExceptionRegions(
            List<SyntaxNode> exceptionHandlingAncestors,
            SyntaxTree tree,
            CancellationToken cancellationToken
        )
        {
            ...
            {
                return new ActiveStatementExceptionRegions(
                    ImmutableArray<SourceFileSpan>.Empty,
            ...
            {
                var span = GetExceptionHandlingRegion(
                    exceptionHandlingAncestors[i],
                    out var coversAllChildren
                );
        ...
        )
        {
            ...
            while (true)
            {
                ...
                var oldParentLambdaBody = FindEnclosingLambdaBody(
                    oldEncompassingAncestor,
                    oldLambda
                );
                ...
                if (
                    lambdaInfos.TryGetValue(oldParentLambdaBody, out var lambdaInfo)
                    && lambdaInfo.Match != null
                )
                {
        ...
        private TextSpan FindClosestActiveSpan(SyntaxNode statement, int statementPart)
        {
            if (
                TryGetActiveSpan(
                    statement,
                    statementPart,
                    minLength: statement.Span.Length,
                    out var span
                )
            )
            {
        ...
        internal TextSpan GetDeletedNodeActiveSpan(
            IReadOnlyDictionary<SyntaxNode, SyntaxNode> forwardMap,
            SyntaxNode deletedNode
        )
        {
        ...
        internal TextSpan GetDeletedDeclarationActiveSpan(
            IReadOnlyDictionary<SyntaxNode, SyntaxNode> forwardMap,
            SyntaxNode deletedDeclaration
        )
        {
            ...
            var hasAncestor = TryGetMatchingAncestor(
                forwardMap,
                deletedDeclaration,
                out var newAncestor
            );
        ...
        internal TextSpan GetDeletedNodeDiagnosticSpan(
            IReadOnlyDictionary<SyntaxNode, SyntaxNode> forwardMap,
            SyntaxNode deletedNode
        )
        {
        ...
        private static bool TryGetMatchingAncestor(
            IReadOnlyDictionary<SyntaxNode, SyntaxNode> forwardMap,
            SyntaxNode? oldNode,
            [NotNullWhen(true)] out SyntaxNode? newAncestor
        )
        {
            while (oldNode != null)
            {
        ...
        protected static bool HasParentEdit(
            IReadOnlyDictionary<SyntaxNode, EditKind> editMap,
            Edit<SyntaxNode> edit
        )
        {
        ...
        protected void AddAroundActiveStatementRudeDiagnostic(
            ArrayBuilder<RudeEditDiagnostic> diagnostics,
            SyntaxNode? oldNode,
            SyntaxNode? newNode,
            TextSpan newActiveStatementSpan
        )
        {
            if (oldNode == null)
            {
        ...
        protected void AddRudeUpdateAroundActiveStatement(
            ArrayBuilder<RudeEditDiagnostic> diagnostics,
            SyntaxNode newNode
        )
        {
        ...
        protected void AddRudeInsertAroundActiveStatement(
            ArrayBuilder<RudeEditDiagnostic> diagnostics,
            SyntaxNode newNode
        )
        {
        ...
        protected void AddRudeDeleteAroundActiveStatement(
            ArrayBuilder<RudeEditDiagnostic> diagnostics,
            SyntaxNode oldNode,
            TextSpan newActiveStatementSpan
        )
            ...
            where TSyntaxNode : SyntaxNode
        {
            ...
            if (oldNodes != null)
            {
                matchCount = MatchNodes(
                    oldNodes,
                    newNodes,
                    diagnostics: null,
                    reverseMap,
                    comparer: areEquivalent
                );
        ...
        private void ReportRudeEditsAndInserts(
            ...
            where TSyntaxNode : SyntaxNode
        {
            ...
            for (var newIndex = 0; newIndex < newNodes.Count; newIndex++)
            {
                ...
                if (reverseMap == null)
                {
                    ...
                    reverseMap.TryGetValue(newNode, out var oldPartner)
                    && comparer((TSyntaxNode)oldPartner, (TSyntaxNode)newNode)
                )
                {
        ...
        private static int IndexOfEquivalent<TSyntaxNode>(
            SyntaxNode newNode,
            List<SyntaxNode?> oldNodes,
            int startIndex,
            Func<TSyntaxNode, TSyntaxNode, bool> comparer
        ...
        private static List<SyntaxNode?>? GetAncestors(
            SyntaxNode? root,
            SyntaxNode node,
            Func<SyntaxNode, bool> nodeSelector
        )
        ...
        )
        {
            ...
            foreach (var (oldNode, newNode) in topMatch.Matches)
            {
                ...
                var currentSegment = (
                    ...
                    oldStartLine: 0,
                    delta: 0,
                    firstOldToken: default(SyntaxToken),
                    firstNewToken: default(SyntaxToken)
                );
                ...
                bool IsCurrentSegmentBreakpointSpanMappable()
                {
                    ...
                    if (
                        !TryGetEnclosingBreakpointSpan(oldToken, out var oldBreakpointSpan)
                ...
                while (true)
                {
                    ...
                    if (
                        oldHasToken != newHasToken
                        || oldHasToken
                            && !AreEquivalent(oldTokensEnum.Current, newTokensEnum.Current)
                    )
                    {
                    ...
                    if (
                        currentSegment.path != oldMappedSpan.Path
                        || currentSegment.delta != lineDelta
                    )
                    {
                        // end of segment:
                        if (currentSegment.path != null)
                        {
                            ...
                            if (
                                ...
                                && string.Equals(lastOldFilePath, oldMappedSpan.Path)
                            )
                            {
                                requiresUpdate = true;
                                break;
                        ...
                        currentSegment = (
                            ...
                            oldStartLine,
                            lineDelta,
                            oldTokensEnum.Current,
                            newTokensEnum.Current
                        );
                    }
            ...
            foreach (var segment in segments)
            {
                if (segment.filePath != currentDocumentPath)
                {
                    ...
                    if (documentLineEdits.Count > 0)
                    {
                        lineEdits.Add(
                            new SequencePointUpdates(
                                currentDocumentPath,
                                documentLineEdits.ToImmutableAndClear()
                            )
                        );
                    }
                    ...
                    && segment.delta != previousLineDelta
                )
                {
                ...
                if (documentLineEdits.Count > 0 && segment.oldStartLine > previousOldEndLine + 1)
                {
                    ...
                    documentLineEdits.Add(
                        new SourceLineUpdate(previousOldEndLine + 1, previousOldEndLine + 1)
                    );
                    previousLineDelta = 0;
                }
                ...
                {
                    documentLineEdits.Add(
                        new SourceLineUpdate(
                            segment.oldStartLine,
                            segment.oldStartLine + segment.delta
                        )
                    );
                }
            ...
            {
                lineEdits.Add(
                    new SequencePointUpdates(currentDocumentPath, documentLineEdits.ToImmutable())
                );
            }
        }
        ...
        protected static bool ParameterTypesEquivalent(
            ImmutableArray<IParameterSymbol> oldParameters,
            ImmutableArray<IParameterSymbol> newParameters,
            bool exact
        ) => oldParameters.SequenceEqual(newParameters, exact, ParameterTypesEquivalent);

        protected static bool CustomModifiersEquivalent(
            CustomModifier oldModifier,
            CustomModifier newModifier,
            bool exact
        ) =>
        ...
        protected static bool CustomModifiersEquivalent(
            ...
            ImmutableArray<CustomModifier> newModifiers,
            bool exact
        ) => oldModifiers.SequenceEqual(newModifiers, exact, CustomModifiersEquivalent);

        protected static bool ReturnTypesEquivalent(
            IMethodSymbol oldMethod,
            IMethodSymbol newMethod,
            bool exact
        ) =>
            ...
            && // modreq emitted on the return type
            CustomModifiersEquivalent(
                oldMethod.ReturnTypeCustomModifiers,
                newMethod.ReturnTypeCustomModifiers,
                exact
            )
            && CustomModifiersEquivalent(
                oldMethod.RefCustomModifiers,
                newMethod.RefCustomModifiers,
                exact
            )
        ...
        protected static bool ReturnTypesEquivalent(
            ...
            && CustomModifiersEquivalent(
                ...
                newProperty.TypeCustomModifiers,
                exact
            )
            && CustomModifiersEquivalent(
                oldProperty.RefCustomModifiers,
                newProperty.RefCustomModifiers,
                exact
            )
            && TypesEquivalent(oldProperty.Type, newProperty.Type, exact);

        protected static bool ReturnTypesEquivalent(
            IEventSymbol oldEvent,
            IEventSymbol newEvent,
            bool exact
        ) => TypesEquivalent(oldEvent.Type, newEvent.Type, exact);

        protected static bool ReturnTypesEquivalent(
            IFieldSymbol oldField,
            IFieldSymbol newField,
            bool exact
        ) =>
            CustomModifiersEquivalent(
                oldField.RefCustomModifiers,
                newField.RefCustomModifiers,
                exact
        ...
        protected static bool TypesEquivalent(
            ITypeSymbol? oldType,
            ITypeSymbol? newType,
            bool exact
        ) =>
        ...
        protected static bool TypeParameterConstraintsEquivalent(
            ...
            ITypeParameterSymbol newParameter,
            bool exact
        ) =>
        ...
        protected static bool TypeParametersEquivalent(
            ImmutableArray<ITypeParameterSymbol> oldParameters,
            ImmutableArray<ITypeParameterSymbol> newParameters,
            bool exact
        ) => oldParameters.SequenceEqual(newParameters, exact, TypeParameterConstraintsEquivalent);

        protected static bool BaseTypesEquivalent(
        ...
        protected static bool MemberOrDelegateSignaturesEquivalent(
            ISymbol? oldMember,
            ISymbol? newMember,
            bool exact = false
        )
            ...
            switch (oldMember.Kind)
            {
                case SymbolKind.Field:
                    return ReturnTypesEquivalent(
                        (IFieldSymbol)oldMember,
                        (IFieldSymbol)newMember,
                        exact
                    );

                case SymbolKind.Event:
                    return ReturnTypesEquivalent(
                        (IEventSymbol)oldMember,
                        (IEventSymbol)newMember,
                        exact
                    );
                ...
                case SymbolKind.Property:
                    ...
                    return ParameterTypesEquivalent(
                            oldProperty.Parameters,
                            newProperty.Parameters,
                            exact
        ...
        protected sealed class SymbolInfoCache(PooledDictionary<ISymbol, SymbolKey> symbolKeyCache)
        {
            public SymbolKey GetKey(ISymbol symbol, CancellationToken cancellationToken) =>
                symbolKeyCache.GetOrAdd(
                    symbol,
                    static (symbol, cancellationToken) =>
        ...
        )
        {
            ...
            try
            {
                ...
                foreach (var edit in editScript.Edits)
                {
                    ...
                    Debug.Assert(
                        edit.OldNode is null
                            || edit.NewNode is null
                            || IsNamespaceDeclaration(edit.OldNode)
                    ...
                    var symbolEdits =
                        oldModel != null && IsNamespaceDeclaration(edit.OldNode ?? edit.NewNode!)
                            ? OneOrMany.Create(
                                GetNamespaceSymbolEdits(oldModel, newModel, cancellationToken)
                            )
                            : GetSymbolEdits(
                                edit.Kind,
                                edit.OldNode,
                    ...
                    foreach (var symbolEdit in symbolEdits)
                    {
                        ...
                        if (syntacticEditKind == EditKind.Move)
                        {
                            ...
                            var oldSymbolInNewCompilation = symbolCache
                                .GetKey(oldSymbol, cancellationToken)
                                .Resolve(newCompilation, ignoreAssemblyKey: true, cancellationToken)
                                .Symbol;
                            var newSymbolInOldCompilation = symbolCache
                                .GetKey(newSymbol, cancellationToken)
                                .Resolve(oldCompilation, ignoreAssemblyKey: true, cancellationToken)
                                .Symbol;

                            if (
                                oldSymbolInNewCompilation == null
                                || newSymbolInOldCompilation == null
                            )
                            {
                                if (
                                    TypesEquivalent(
                                    ...
                                    && !SymbolsEquivalent(
                                        oldSymbol.ContainingNamespace,
                                        newSymbol.ContainingNamespace
                                    )
                                )
                                {
                                    ...
                                    diagnostics.Add(
                                        new RudeEditDiagnostic(
                                            ...
                                            new[]
                                            {
                                                GetDisplayName(newTypeDeclaration),
                                                oldSymbol.ContainingNamespace.ToDisplayString(),
                                                newSymbol.ContainingNamespace.ToDisplayString(),
                                            }
                                        )
                                    );
                                }
                        ...
                        var (oldDeclaration, newDeclaration) = GetSymbolDeclarationNodes(
                            oldSymbol,
                            newSymbol,
                            edit.OldNode,
                            edit.NewNode
                        ...
                        if (syntacticEditKind == EditKind.Update)
                        {
                            if (
                                ...
                                || oldDeclaration == null
                                || oldDeclaration != null
                                    && oldDeclaration.SyntaxTree != oldModel?.SyntaxTree
                            )
                            {
                                ...
                                || newDeclaration == null
                                || newDeclaration != null
                                    && newDeclaration.SyntaxTree != newModel.SyntaxTree
                            )
                            {
                        ...
                        if (!inBreakState)
                        {
                            ...
                            if (
                                containingType != null
                                && (syntacticEditKind != EditKind.Delete || newSymbol == null)
                            )
                            {
                                var containingTypeSymbolKey = symbolCache.GetKey(
                                    containingType,
                                    cancellationToken
                                );
                                ...
                                if (
                                    ...
                                    && newContainingType != null
                                    && IsReloadable(oldContainingType)
                                )
                                {
                                    if (processedSymbols.Add(newContainingType))
                                    {
                                        ...
                                        {
                                            semanticEdits.Add(
                                                SemanticEditInfo.CreateReplace(
                                                    containingTypeSymbolKey,
                                                    IsPartialTypeEdit(
                                                        oldContainingType,
                                                        newContainingType,
                            ...
                            if (
                                syntacticEditKind != EditKind.Delete
                                && oldSymbol is INamedTypeSymbol oldType
                                && newSymbol is INamedTypeSymbol newType
                                && IsReloadable(oldType)
                            )
                            {
                                if (symbol == newType || processedSymbols.Add(newType))
                                {
                                    ...
                                    else if (oldType.Arity != newType.Arity)
                                    {
                                        ...
                                        diagnosticContext.Report(
                                            RudeEditKind.ChangingTypeParameters,
                                            cancellationToken
                                        );
                                    }
                                    else if (
                                    ...
                                    {
                                        var typeKey = symbolCache.GetKey(
                                            newType,
                                            cancellationToken
                                        );
                                        semanticEdits.Add(
                        ...
                        switch (syntacticEditKind)
                        {
                            case EditKind.Delete:
                            {
                                ...
                                if (newSymbol != null)
                                {
                                    ...
                                    if (
                                        oldSymbol.ContainingType.IsRecord
                                        && newSymbol.ContainingType.IsRecord
                                        && newSymbol is IPropertySymbol newProperty
                                        && IsPrimaryConstructorParameterMatchingSymbol(
                                            newSymbol,
                                            cancellationToken
                                        )
                                    )
                                    {
                                        AnalyzeRecordPropertyReplacement(
                                            (IPropertySymbol)oldSymbol,
                                            newProperty,
                                            isDeleteEdit: true
                                        );
                                ...
                                var diagnosticSpan = GetDeletedNodeDiagnosticSpan(
                                ...
                                var oldContainingType = oldSymbol.ContainingType;
                                var containingTypeKey = symbolCache.GetKey(
                                    oldContainingType,
                                    cancellationToken
                                );
                                var newContainingType = (INamedTypeSymbol?)
                                    containingTypeKey
                                        .Resolve(
                                            newCompilation,
                                            ignoreAssemblyKey: true,
                                ...
                                if (IsDeclarationWithInitializer(oldDeclaration))
                                {
                                    DeferConstructorEdit(
                                        ...
                                        syntaxMap,
                                        oldSymbol.IsStatic,
                                        isMemberWithDeletedInitializer: true
                                    );
                                }
                                ...
                                if (SymbolPresenceAffectsSynthesizedRecordMembers(oldSymbol))
                                {
                                    ...
                                    var newMatchingSymbol = newContainingType
                                ...
                                if (oldContainingType.IsRecord)
                                {
                                    if (IsPrimaryConstructor(oldSymbol, cancellationToken))
                                    {
                                        ...
                                        AddDeconstructorEdits(
                                            semanticEdits,
                                            oldPrimaryConstructor,
                                            otherConstructor: null,
                                            containingTypeKey,
                                            oldCompilation,
                                            newCompilation,
                                        ...
                                        oldSymbol is IParameterSymbol oldParameter
                                        && IsPrimaryConstructor(
                                            oldParameter.ContainingSymbol,
                                            cancellationToken
                                        )
                                    )
                                    {
                                        AddSynthesizedMemberEditsForRecordParameterChange(
                                            semanticEdits,
                                            oldParameter,
                                            newContainingType,
                                            containingTypeKey,
                                ...
                                AddDeleteEditsForMemberAndAccessors(
                                    semanticEdits,
                                    oldSymbol.PartialAsImplementation(),
                                    containingTypeKey,
                                    cancellationToken
                                );
                                continue;
                            ...
                            case EditKind.Insert:
                                {
                                    ...
                                    if (oldSymbol != null)
                                    {
                                        ...
                                        if (
                                            oldSymbol
                                                is IPropertySymbol
                                                {
                                                    ...
                                                    GetMethod.IsImplicitlyDeclared: true,
                                                    SetMethod.IsImplicitlyDeclared: true
                                                } oldRecordProperty
                                            && IsPrimaryConstructorParameterMatchingSymbol(
                                                oldSymbol,
                                                cancellationToken
                                        ...
                                        ReportTypeLayoutUpdateRudeEdits(
                                            diagnosticContext,
                                            newSymbol,
                                            cancellationToken
                                        );
                                    ...
                                    if (newContainingType != null && !IsGlobalMain(newSymbol))
                                    {
                                        ...
                                        var hasAssociatedSymbolInsert =
                                            GetAssociatedMember(newSymbol)
                                                is { } newAssociatedMember
                                            && HasEdit(
                                                editMap,
                                        ...
                                        if (!hasAssociatedSymbolInsert && IsMember(newSymbol))
                                        {
                                            ReportInsertedMemberSymbolRudeEdits(
                                                ...
                                                newSymbol,
                                                newDeclaration,
                                                insertingIntoExistingContainingType: oldContainingType
                                                    != null
                                            );
                                        }
                                        ...
                                        if (oldContainingType == null)
                                        {
                                            ...
                                            if (
                                                ...
                                                && newSymbol is ITypeParameterSymbol
                                            )
                                            {
                                                diagnosticContext.Report(
                                                    RudeEditKind.Insert,
                                        ...
                                        // (PrintMembers print all properties and fields, Equals and GHC compare all data members, etc.)
                                        if (
                                            SymbolPresenceAffectsSynthesizedRecordMembers(newSymbol)
                                        )
                                        {
                                            AddSynthesizedRecordMethodUpdatesForPropertyChange(
                                                semanticEdits,
                                        ...
                                        if (
                                            newSymbol is IParameterSymbol newParameter
                                        ...
                                        if (
                                            hasAssociatedSymbolInsert
                                            || newSymbol is IParameterSymbol or ITypeParameterSymbol
                                        )
                                        {
                                            continue;
                                        }
                                    ...
                                    else
                                    {
                                        ...
                                        if (IsMember(newSymbol))
                                        {
                                            ReportInsertedMemberSymbolRudeEdits(
                                                ...
                                                newSymbol,
                                                newDeclaration,
                                                insertingIntoExistingContainingType: false
                                            );
                                        }
                                    }
                                    ...
                                    var isConstructorWithMemberInitializers =
                                    ...
                                    )
                                    {
                                        ...
                                        DeferConstructorEdit(
                                            ...
                                            syntaxMap,
                                            newSymbol.IsStatic,
                                            isMemberWithDeletedInitializer: false
                                        );
                            ...
                            case EditKind.Reorder:
                                ...
                                if (
                                    oldSymbol is IParameterSymbol
                                    && !IsMemberOrDelegateReplaced(
                                        oldSymbol.ContainingSymbol,
                                        newSymbol.ContainingSymbol
                                    )
                                    && !capabilities.Grant(
                        ...
                        void AnalyzeRecordPropertyReplacement(
                            IPropertySymbol oldProperty,
                            IPropertySymbol newProperty,
                            bool isDeleteEdit
                        )
                        {
                            ...
                            var (customProperty, synthesizedProperty) = isDeleteEdit
                            ...
                            DeferConstructorEdit(
                                oldProperty.ContainingType,
                                newProperty.ContainingType,
                                newDeclaration: null,
                                syntaxMap,
                            ...
                            if (customProperty.SetMethod == null)
                            {
                                ...
                                if (isDeleteEdit)
                                {
                                    AddInsertEditsForMemberAndAccessors(
                                        semanticEdits,
                                        synthesizedProperty.SetMethod,
                                        cancellationToken
                                    );
                                }
                                else
                        ...
                        void ReportDeletedMemberActiveStatementsRudeEdits()
                        {
                            ...
                            var activeStatementIndices = oldBody.GetOverlappingActiveStatements(
                                oldActiveStatements
                            );
                            ...
                            foreach (var index in activeStatementIndices)
                            {
                                ...
                                {
                                    newActiveStatementSpan ??= GetDeletedDeclarationActiveSpan(
                                        editScript.Match.Matches,
                                        oldDeclaration
                                    );
                                    newActiveStatements[index] = GetActiveStatementWithSpan(
                                        oldActiveStatements[index],
                                        newTree,
                                        newActiveStatementSpan.Value,
                                        diagnostics,
                                        cancellationToken
                                    );
                        ...
                        if (editKind == SemanticEditKind.Update)
                        {
                            ...
                            var replaceMember =
                                IsMemberOrDelegate(oldSymbol)
                                && IsMemberOrDelegateReplaced(oldSymbol, newSymbol);

                            if (replaceMember && oldSymbol.Name == newSymbol.Name)
                            {
                                var signatureRudeEdit = GetSignatureChangeRudeEdit(
                                    oldSymbol,
                                    newSymbol,
                                    capabilities
                                );
                            ...
                            var oldBody =
                                (oldDeclaration != null)
                                    ? TryGetDeclarationBody(oldDeclaration, oldSymbol)
                                    : null;
                            if (!skipBodyAnalysis)
                            {
                                var newBody =
                                    (newDeclaration != null)
                                        ? TryGetDeclarationBody(newDeclaration, newSymbol)
                                        : null;
                                if (oldBody != null || newBody != null)
                                {
                            ...
                            var isConstructorWithMemberInitializers =
                                IsConstructorWithMemberInitializers(newSymbol, cancellationToken);
                            var isOldDeclarationWithInitializer =
                                oldDeclaration != null
                                && IsDeclarationWithInitializer(oldDeclaration);
                            var isNewDeclarationWithInitializer =
                                newDeclaration != null
                                && IsDeclarationWithInitializer(newDeclaration);

                            if (
                                isConstructorWithMemberInitializers
                                || isOldDeclarationWithInitializer
                                || isNewDeclarationWithInitializer
                            )
                            {
                                DeferConstructorEdit(
                                    oldSymbol.ContainingType,
                                    ...
                                    syntaxMap,
                                    newSymbol.IsStatic,
                                    isMemberWithDeletedInitializer: isOldDeclarationWithInitializer
                                        && !isNewDeclarationWithInitializer
                                );
                            ...
                            if (replaceMember)
                            {
                                ...
                                )
                                {
                                    ...
                                    var containingSymbolKey = symbolCache.GetKey(
                                        oldSymbol.ContainingType,
                                        cancellationToken
                                    );
                                    AddMemberSignatureOrNameChangeEdits(
                                        semanticEdits,
                                        oldSymbol.PartialAsImplementation(),
                                        newSymbol.PartialAsImplementation(),
                                        containingSymbolKey,
                                        cancellationToken
                                    );
                                }
                            ...
                            if (
                                !hasAttributeChange
                                && newSymbol
                                    is INamedTypeSymbol { IsGenericType: false }
                                        or // changes in type parameter attributes and constraints need type update
                                        IPropertySymbol { IsIndexer: false }
                                        or // changes in parameter attributes need indexer update
                                        IFieldSymbol
                                        or IEventSymbol
                            )
                            {
                        ...
                        var partialType =
                            ...
                            : IsPartialTypeEdit(oldSymbol, newSymbol, oldTree, newTree) ? symbolKey
                            : (SymbolKey?)null;

                        semanticEdits.Add(
                            editKind switch
                            {
                                SemanticEditKind.Update => SemanticEditInfo.CreateUpdate(
                                    symbolKey,
                                    syntaxMap,
                                    syntaxMapTree: (syntaxMap != null) ? newModel.SyntaxTree : null,
                                    partialType
                                ),
                                SemanticEditKind.Insert => SemanticEditInfo.CreateInsert(
                                    symbolKey,
                                    partialType
                                ),
                                SemanticEditKind.Replace => SemanticEditInfo.CreateReplace(
                                    symbolKey,
                                    partialType
                                ),
                    ...
                    foreach (var edit in triviaSymbolEdits)
                    {
                        ...
                        )
                        {
                            if (processedSymbols.Add(newContainingType))
                            {
                                ...
                                {
                                    var oldContainingTypeKey = SymbolKey.Create(
                                        oldContainingType,
                                        cancellationToken
                                    );
                                    semanticEdits.Add(
                                        SemanticEditInfo.CreateReplace(
                                            oldContainingTypeKey,
                                            IsPartialTypeEdit(
                                                oldContainingType,
                                                newContainingType,
                                                oldTree,
                        ...
                        Contract.ThrowIfNull(newBody);
                        Debug.Assert(
                            IsConstructorWithMemberInitializers(oldSymbol, cancellationToken)
                                == IsConstructorWithMemberInitializers(newSymbol, cancellationToken)
                        );
                        Debug.Assert(
                            IsDeclarationWithInitializer(oldDeclaration)
                        ...
                        syntaxMap = isActiveMember
                            ? CreateSyntaxMapForEquivalentNodes(oldBody, newBody)
                            : null;

                        var isConstructorWithMemberInitializers =
                            IsConstructorWithMemberInitializers(newSymbol, cancellationToken);
                        var isDeclarationWithInitializer = IsDeclarationWithInitializer(
                            newDeclaration
                        );
                ...
                bool PreprocessSymbolEdit(ref ISymbol? oldSymbol, ref ISymbol? newSymbol)
                {
                    ...
                    oldSymbol ??= Resolve(
                        newSymbol!,
                        symbolCache.GetKey(newSymbol!, cancellationToken),
                        oldCompilation,
                        cancellationToken
                    );
                    newSymbol ??= Resolve(
                        oldSymbol!,
                        symbolCache.GetKey(oldSymbol!, cancellationToken),
                        newCompilation,
                        cancellationToken
                    );

                    static ISymbol? Resolve(
                        ISymbol symbol,
                        SymbolKey symbolKey,
                    ...
                    )
                    {
                        ...
                        return
                            symbol is IMethodSymbol { PartialDefinitionPart: not null }
                            && result is IMethodSymbol { IsPartialDefinition: true }
                            ? null
                            : result;
                    }
                ...
                )
                {
                    ...
                    {
                        constructorEdits.Add(
                            newType,
                            constructorEdit = new MemberInitializationUpdates(oldType)
                        );
                    }

                    if (
                        newDeclaration != null
                        && !constructorEdit.ChangedDeclarations.ContainsKey(newDeclaration)
                    )
                    {
            ...
            (SyntaxNode? oldDeclaration, SyntaxNode? newDeclaration) GetSymbolDeclarationNodes(
                ISymbol? oldSymbol,
                ISymbol? newSymbol,
                SyntaxNode? oldNode,
                SyntaxNode? newNode
                ...
                (
                    oldDeclaration: (
                        oldSymbol != null
                        && GetSingleSymbolDeclarationSyntax(oldSymbol, cancellationToken)
                            is { } oldDeclaration
                    )
                        ? oldDeclaration
                        : oldNode,
                    newDeclaration: (
                        newSymbol != null
                        && GetSingleSymbolDeclarationSyntax(newSymbol, cancellationToken)
                            is { } newDeclaration
                    )
                        ? newDeclaration
                        : newNode
        ...
        protected static ISymbol? GetSemanticallyMatchingNewSymbol(
            ISymbol? oldSymbol,
            ISymbol? newSymbol,
            SemanticModel newModel,
            SymbolInfoCache symbolCache,
            ...
            && symbolCache
                .GetKey(oldSymbol, cancellationToken)
                .Resolve(newModel.Compilation, ignoreAssemblyKey: true, cancellationToken)
                .Symbol
        ...
        protected static void AddMemberUpdate(
            ref TemporaryArray<(ISymbol?, ISymbol?, EditKind)> result,
            ISymbol? oldSymbol,
            ISymbol? newSymbol,
            ISymbol? newSemanticallyMatchingSymbol
        )
        {
        ...
        )
        {
            ...
            var synthesizedProperty = GetPropertySynthesizedForRecordPrimaryConstructorParameter(
                parameterSymbol
            );
            if (synthesizedProperty != null)
            {
                var otherMembersOfParameterName = otherContainingType.GetMembers(
                ...
                if (otherMembersOfParameterName.Any(static m => m is IPropertySymbol))
                {
                    ...
                    AddUpdateEditsForMemberAndAccessors(
                        semanticEdits,
                        synthesizedProperty,
                        cancellationToken
                    );
                ...
                else if (isParameterDelete)
                {
                    ...
                    AddDeleteEditsForMemberAndAccessors(
                        semanticEdits,
                        synthesizedProperty,
                        deletedSymbolContainer: containingTypeKey,
                        cancellationToken
                    );
                }
                else
        ...
        )
        {
            ...
            void AddEdits(IMethodSymbol? constructor, Compilation otherCompilation, bool isDelete)
            {
                if (
                    ...
                    && constructor.GetMatchingDeconstructor()
                        is { IsImplicitlyDeclared: true } deconstructor
                )
                {
                    if (
                        SymbolKey
                            .Create(deconstructor, cancellationToken)
                            .Resolve(otherCompilation, ignoreAssemblyKey: true, cancellationToken)
                            .Symbol != null
                    )
                    {
        ...
        private static bool AllowsDeletion(ISymbol symbol)
        {
            ...
            if (
                ...
                is not { IsVirtual: false, IsAbstract: false, IsOverride: false }
            )
                return false;
            ...
            return symbol
                is IParameterSymbol
                    ...
                    or IEventSymbol
                    {
                        AddMethod.IsImplicitlyDeclared: false,
                        RemoveMethod.IsImplicitlyDeclared: false
                    };
        }
        ...
        )
        {
            ...
            void AddDelete(ISymbol? symbol)
            {
                ...
                var partialType = symbol is IMethodSymbol { PartialDefinitionPart: not null }
                    ? SymbolKey.Create(symbol.ContainingType, cancellationToken)
                    : (SymbolKey?)null;
                semanticEdits.Add(
                    SemanticEditInfo.CreateDelete(
                        SymbolKey.Create(symbol, cancellationToken),
        ...
        )
        {
            ...
            var partialType = newSymbol is IMethodSymbol { PartialDefinitionPart: not null }
                ? SymbolKey.Create(newSymbol.ContainingType, cancellationToken)
                : (SymbolKey?)null;
            semanticEdits.Add(
                SemanticEditInfo.CreateInsert(
        ...
        )
        {
            ...
            switch (oldSymbol)
            {
                ...
                case IEventSymbol oldEventSymbol:
                    ...
                    semanticEdits.Add(
                        SemanticEditInfo.CreateUpdate(
                            SymbolKey.Create(oldSymbol, cancellationToken),
                            syntaxMap: null,
                            syntaxMapTree: null,
                            partialType: null
                        )
        ...
        )
        {
            ...
            using var _2 = PooledDictionary<
                ...
                ArrayBuilder<int>
            >.GetInstance(out var deletedTypes);
            ...
            {
                var oldType = (INamedTypeSymbol)GetRequiredDeclaredSymbol(
                    oldModel,
                    oldTypeDeclaration,
                    cancellationToken
                );
                ...
                var newType = SymbolKey
                    .Create(oldType, cancellationToken)
                    .Resolve(newModel.Compilation, ignoreAssemblyKey: true, cancellationToken)
                    .Symbol;
                if (newType == null)
                {
            ...
            {
                var newType = (INamedTypeSymbol)GetRequiredDeclaredSymbol(
                    newModel,
                    newTypeDeclaration,
                    cancellationToken
                );
                ...
                var oldType = SymbolKey
                    .Create(newType, cancellationToken)
                    .Resolve(oldModel.Compilation, ignoreAssemblyKey: true, cancellationToken)
                    .Symbol;
                if (oldType == null)
                {
                    // Check if a type with the same name and arity was also removed. If so treat it as a move.
                    if (
                        deletedTypes.TryGetValue(
                            (newType.Name, newType.Arity),
                            out var deletedTypeIndices
                        )
                        && deletedTypeIndices.Count > 0
                    )
                    {
                        var deletedTypeIndex = deletedTypeIndices.Last();
                        deletedTypeIndices.RemoveLast();

                        builder[deletedTypeIndex] = (
                            builder[deletedTypeIndex].oldSymbol,
                            newType,
                            EditKind.Move
                        );
                    }
            ...
            while (current != null)
            {
                foreach (var attributeData in current.GetAttributes())
                {
                    ...
                    if (
                        ...
                        {
                            Name: CreateNewOnMetadataUpdateAttributeName,
                            ContainingNamespace:
                            {
        ...
        private sealed class SemanticEditInfoComparer : IEqualityComparer<SemanticEditInfo>
        {
            ...
            private static readonly IEqualityComparer<SymbolKey> s_symbolKeyComparer =
            ...
            public int GetHashCode([DisallowNull] SemanticEditInfo obj) => obj.Symbol.GetHashCode();
        }
        ...
        )
        {
            ...
            _ =
                ...
                || oldBody != null
                    && ReportUnsupportedOperations(diagnosticContext, oldBody, cancellationToken);
            ...
            if (
                !oldStateMachineInfo.IsStateMachine
            ...
            )
            {
                ...
                var rudeEdit = newStateMachineInfo.IsAsync
        ...
        )
        {
            ...
            {
                rudeEdit =
                    (oldSymbol.Kind == SymbolKind.Field || newSymbol.Kind == SymbolKind.Field)
            ...
            else if (oldSymbol.Name != newSymbol.Name)
            {
                if (oldSymbol is IParameterSymbol && newSymbol is IParameterSymbol)
                {
                    ...
                    oldSymbol is IMethodSymbol oldMethod
                ...
                )
                {
                    ...
                    else if (oldMethod.MethodKind == MethodKind.Conversion)
                    {
                        ...
                        oldMethod.MethodKind == MethodKind.ExplicitInterfaceImplementation
                        || newMethod.MethodKind == MethodKind.ExplicitInterfaceImplementation
                    )
                    {
                ...
                {
                    if (
                        !oldProperty.ExplicitInterfaceImplementations.IsEmpty
                        || !newProperty.ExplicitInterfaceImplementations.IsEmpty
                    )
                    {
                        // Can't change from explicit to implicit interface implementation, or one interface to another
                        rudeEdit = RudeEditKind.Renamed;
                ...
                {
                    if (
                        !oldEvent.ExplicitInterfaceImplementations.IsEmpty
                        || !newEvent.ExplicitInterfaceImplementations.IsEmpty
                    )
                    {
                        // Can't change from explicit to implicit interface implementation, or one interface to another
                        rudeEdit = RudeEditKind.Renamed;
            ...
            if (
                oldSymbol.IsStatic != newSymbol.IsStatic
            ...
            {
                if (
                    oldField.IsConst != newField.IsConst
                ...
                if (
                    oldField.IsConst
                    && newField.IsConst
                    && !Equals(oldField.ConstantValue, newField.ConstantValue)
                    && TypesEquivalent(
                        oldField.ContainingType.EnumUnderlyingType,
                        newField.ContainingType.EnumUnderlyingType,
                        exact: false
                    )
                )
                {
                ...
                if (!IsMemberOrDelegateReplaced(oldField, newField))
                {
                    ...
                    hasGeneratedAttributeChange |= !ReturnTypesEquivalent(
                        oldField,
                        newField,
                        exact: true
                    );
            ...
            else if (oldSymbol is IMethodSymbol oldMethod && newSymbol is IMethodSymbol newMethod)
            {
                ...
                if (
                    oldMethod
                        is {
                            ...
                            AssociatedSymbol: IPropertySymbol oldProperty
                        }
                    && !oldProperty.IsAutoProperty()
                    && newMethod
                        is {
                            MethodKind: MethodKind.PropertyGet,
                            AssociatedSymbol: IPropertySymbol newProperty
                        }
                    && newProperty.IsAutoProperty()
                ...
                if (
                    oldMethod.MethodKind == MethodKind.DeclareMethod
                    || newMethod.MethodKind == MethodKind.DeclareMethod
                )
                {
                    ...
                    if (oldImportData != null && newImportData != null)
                    {
                        ...
                        Debug.Assert(
                            oldImportData.BestFitMapping == newImportData.BestFitMapping
                                || oldImportData.CallingConvention
                                    == newImportData.CallingConvention
                                || oldImportData.ExactSpelling == newImportData.ExactSpelling
                                || oldImportData.SetLastError == newImportData.SetLastError
                                || oldImportData.ThrowOnUnmappableCharacter
                                    == newImportData.ThrowOnUnmappableCharacter
                        );
                ...
                )
                {
                    ...
                    hasGeneratedReturnTypeAttributeChange |= !ReturnTypesEquivalent(
                        oldMethod,
                        newMethod,
                        exact: true
                    );
            ...
            {
                if (oldType.TypeKind != newType.TypeKind || oldType.IsRecord != newType.IsRecord) // TODO: https://github.com/dotnet/roslyn/issues/51874
                {
                    rudeEdit = RudeEditKind.TypeKindUpdate;
                }
                else if (
                    oldType.IsRefLikeType != newType.IsRefLikeType
                ...
                if (rudeEdit == RudeEditKind.None)
                {
                    AnalyzeBaseTypes(
                        ...
                        newType,
                        ref rudeEdit,
                        ref hasGeneratedAttributeChange
                    );
            ...
            else if (oldSymbol is IEventSymbol oldEvent && newSymbol is IEventSymbol newEvent)
            {
                ...
                if (
                    oldEvent.AddMethod != null
                        && newEvent.AddMethod != null
                        && oldEvent.AddMethod.IsReadOnly != newEvent.AddMethod.IsReadOnly
                    || oldEvent.RemoveMethod != null
                        && newEvent.RemoveMethod != null
            ...
            )
            {
                ...
                {
                    if (
                        IsExtensionMethodThisParameter(oldParameter)
                            != IsExtensionMethodThisParameter(newParameter)
                        || GeneratesParameterAttribute(oldParameter.RefKind)
                            != GeneratesParameterAttribute(newParameter.RefKind)
                    ...
                    if (
                        ...
                        || oldParameter.HasExplicitDefaultValue
                            && !Equals(
                                oldParameter.ExplicitDefaultValue,
                                newParameter.ExplicitDefaultValue
                            )
                    )
                    {
                        rudeEdit = RudeEditKind.InitializerUpdate;
                ...
                oldSymbol is ITypeParameterSymbol oldTypeParameter
                && newSymbol is ITypeParameterSymbol newTypeParameter
            )
            {
        ...
        )
        {
            if (oldType.EnumUnderlyingType != null && newType.EnumUnderlyingType != null)
            {
                if (
                    !TypesEquivalent(
                        oldType.EnumUnderlyingType,
                        newType.EnumUnderlyingType,
                        exact: true
                    )
                )
                {
        ...
        private static RudeEditKind GetSignatureChangeRudeEdit(
            ISymbol oldMember,
            ISymbol newMember,
            EditAndContinueCapabilitiesGrantor capabilities
        )
        {
            ...
            if (IsGlobalMain(oldMember))
            {
                ...
                Debug.Assert(
                    ParameterTypesEquivalent(
                        oldMember.GetParameters(),
                        newMember.GetParameters(),
                        exact: true
                    )
                );
            ...
            if (
                !CanRenameOrChangeSignature(oldMember, newMember, capabilities)
                && oldMember
                    is not IMethodSymbol
                    {
                        AssociatedSymbol.Kind: SymbolKind.Property or SymbolKind.Event
                    }
            )
            {
                ...
                else
                {
                    rudeEdit =
                        (oldParameter.Variance != newParameter.Variance)
                            ...
                            : RudeEditKind.ChangingConstraints;
                }
            }
        }
        ...
        )
        {
            ...
            ReportUpdatedSymbolDeclarationRudeEdits(
                diagnosticContext,
                capabilities,
                out var hasGeneratedAttributeChange,
                out var hasGeneratedReturnTypeAttributeChange,
                cancellationToken
            );
            ...
            ReportCustomAttributeRudeEdits(
                diagnosticContext,
                capabilities,
                out var hasSymbolAttributeChange,
                out var hasReturnTypeAttributeChange,
                cancellationToken
            );
            ...
            {
                AddSemanticEditsOriginatingFromParameterUpdate(
                    semanticEdits,
                    oldParameter,
                    newParameter,
                    diagnosticContext.NewModel.Compilation,
                    cancellationToken
                );
                ...
                if (
                    (hasSymbolAttributeChange || oldParameter.Name != newParameter.Name)
                    && newParameter.ContainingType
                        is INamedTypeSymbol
                        {
            ...
            if (
                hasReturnTypeAttributeChange
                && newSymbol is INamedTypeSymbol { TypeKind: TypeKind.Delegate } newDelegateType
            )
            {
        ...
        )
        {
            ...
            if (
                oldContainingMember.ContainingType.IsRecord
            ...
            )
            {
                ...
                if (
                    ...
                    && newIsPrimary
                    && (
                        oldParameterSymbol.Name != newParameterSymbol.Name
                        || !ParameterTypesEquivalent(
                            oldParameterSymbol,
                            newParameterSymbol,
                            exact: false
                ...
                )
                {
                    ...
                    var containingSymbolKey = SymbolKey.Create(
                        oldContainingMember.ContainingSymbol,
                        cancellationToken
                    );
                    ...
                    var oldSynthesizedDeconstructor =
                        oldPrimaryConstructor.GetMatchingDeconstructor();
                    var newSynthesizedDeconstructor =
                        newPrimaryConstructor.GetMatchingDeconstructor();
                    Contract.ThrowIfNull(oldSynthesizedDeconstructor);
                    Contract.ThrowIfNull(newSynthesizedDeconstructor);

                    AddMemberSignatureOrNameChangeEdits(
                        semanticEdits,
                        oldSynthesizedDeconstructor,
                        newSynthesizedDeconstructor,
                        containingSymbolKey,
                        cancellationToken
                    );
                    ...
                    AddSynthesizedRecordMethodUpdatesForPropertyChange(
                        semanticEdits,
                        newCompilation,
                        newContainingMember.ContainingType,
                        cancellationToken
                    );
                }
            }
        ...
        private static void AddDelegateMethodEdit(
            ArrayBuilder<SemanticEditInfo> semanticEdits,
        ...
        )
        {
            ...
            if (beginInvokeMethod != null)
            {
                semanticEdits.Add(
                    SemanticEditInfo.CreateUpdate(
                        SymbolKey.Create(beginInvokeMethod, cancellationToken),
                        syntaxMap: null,
                        syntaxMapTree: null,
                        partialType: null
                    )
        ...
        )
        {
            ...
            hasAttributeChange = ReportCustomAttributeRudeEdits(
                diagnosticContext,
                oldSymbol.GetAttributes(),
                newSymbol.GetAttributes(),
                capabilities,
                cancellationToken
            );
            ...
            {
                hasReturnTypeAttributeChange |= ReportCustomAttributeRudeEdits(
                    diagnosticContext,
                    oldMethod.GetReturnTypeAttributes(),
                    newMethod.GetReturnTypeAttributes(),
                    capabilities,
                    cancellationToken
                );
            }
                ...
                oldSymbol
                    is INamedTypeSymbol { DelegateInvokeMethod: not null and var oldInvokeMethod }
                && newSymbol
                    is INamedTypeSymbol { DelegateInvokeMethod: not null and var newInvokeMethod }
            )
            {
                hasReturnTypeAttributeChange |= ReportCustomAttributeRudeEdits(
                    diagnosticContext,
                    oldInvokeMethod.GetReturnTypeAttributes(),
                    newInvokeMethod.GetReturnTypeAttributes(),
                    capabilities,
                    cancellationToken
                );
            }
        ...
        )
        {
            ...
            {
                var rudeEdit =
                    oldSymbol.ContainingSymbol.Kind == SymbolKind.Method
            ...
            foreach (var attributeData in changedAttributes)
            {
                ...
                {
                    diagnosticContext.Report(
                        RudeEditKind.ChangingAttribute,
                        cancellationToken,
                        arguments: new[] { attributeData.AttributeClass.Name }
                    );
            ...
            static void FindChangedAttributes(
                ImmutableArray<AttributeData>? oldAttributes,
                ImmutableArray<AttributeData> newAttributes,
                ArrayBuilder<AttributeData> changedAttributes
            )
            {
            ...
            static AttributeData? FindMatch(
        ...
        /// </summary>
        private static bool CanRenameOrChangeSignature(
            ISymbol oldSymbol,
            ISymbol newSymbol,
            EditAndContinueCapabilitiesGrantor capabilities
        ) =>
        ...
        private static bool CanAddNewMemberToExistingType(
            ISymbol newSymbol,
            EditAndContinueCapabilitiesGrantor capabilities
        )
        {
        ...
        private static EditAndContinueCapabilities GetRequiredAddMethodCapabilities(
            ISymbol symbol
        ) =>
        ...
        private static EditAndContinueCapabilities GetRequiredAddFieldCapabilities(
            ISymbol symbol
        ) =>
            (
                symbol.IsStatic
        ...
        private static bool CanUpdateMemberBody(
            ISymbol oldSymbol,
            EditAndContinueCapabilitiesGrantor capabilities
        )
        {
        ...
        private static IEnumerable<ISymbol> GetRecordUpdatedSynthesizedMethods(
            Compilation compilation,
        ...
        )
        {
            ...
            var result = record
                .GetMembers(WellKnownMemberNames.PrintMembersMethodName)
                .FirstOrDefault(
                    static (m, compilation) =>
                        m is IMethodSymbol { IsImplicitlyDeclared: true } method
                        && HasPrintMembersSignature(method, compilation),
                    compilation
                );
            ...
            result = record
                ...
                .FirstOrDefault(static m =>
                    m is IMethodSymbol { IsImplicitlyDeclared: true } method
                    && HasIEquatableEqualsSignature(method)
                );
            if (result is not null)
            {
                yield return result;
            ...
            result = record
                .GetMembers(WellKnownMemberNames.ObjectGetHashCode)
                .FirstOrDefault(static m =>
                    m is IMethodSymbol { IsImplicitlyDeclared: true } method
                    && HasGetHashCodeSignature(method)
                );
            if (result is not null)
            {
                yield return result;
            ...
            {
                result = record.InstanceConstructors.SingleOrDefault(m =>
                    m.IsImplicitlyDeclared && m.IsCopyConstructor()
                );
                if (result is not null)
                {
                    yield return result;
        ...
        )
        {
            ...
            public ISymbol? OldSymbol => oldSymbol;
            ...
            private SyntaxNode GetDiagnosticNode(
                out int distance,
                CancellationToken cancellationToken
            )
            {
                ...
                if (newDiagnosticSymbol == null)
                {
                    ...
                    newDiagnosticSymbol = TryGetNewContainer(
                        oldSymbol,
                        ref distance,
                        cancellationToken
                    );
                    ...
                    {
                        var oldContainerDeclaration = analyzer.GetSymbolDeclarationSyntax(
                            oldSymbol.ContainingSymbol,
                            topMatch.OldRoot.SyntaxTree,
                            cancellationToken
                        );
                        if (
                            oldContainerDeclaration != null
                            && topMatch.TryGetNewNode(
                                oldContainerDeclaration,
                                out var newContainerDeclaration
                            )
                        )
                        {
                ...
                while (newDiagnosticSymbol != null)
                {
                    ...
                    if (
                        newDiagnosticSymbol.DeclaringSyntaxReferences.Length > 0
                        && !newDiagnosticSymbol.IsImplicitlyDeclared
                    )
                    {
                        var node = analyzer.GetSymbolDeclarationSyntax(
                            newDiagnosticSymbol,
                            newModel.SyntaxTree,
                            cancellationToken
                        );
                    ...
                    if (
                        newDiagnosticSymbol.Kind
                        is not (SymbolKind.Parameter or SymbolKind.TypeParameter)
                    )
                    {
                        distance++;
                    }
            ...
            private ISymbol? TryGetNewContainer(
                ISymbol oldSymbol,
                ref int distance,
                CancellationToken cancellationToken
            )
            {
                ...
                while (
                    oldContainer is not null and not INamespaceSymbol { IsGlobalNamespace: true }
                )
                {
            ...
            public void Report(
                ...
                Report(
                    ...
                    span: (locationNode.SyntaxTree == newModel.SyntaxTree)
                        ? locationNode.Span
                        : analyzer.GetDiagnosticSpan(
                            GetDiagnosticNode(out var distance, cancellationToken),
                            distance > 0 ? EditKind.Delete : EditKind.Update
                        ),
                    arguments
                );
            ...
            )
            {
                ...
                span ??= diagnosticSpan.IsEmpty
                    ? analyzer.GetDiagnosticSpan(
                        node,
                        (distance > 0 || kind == RudeEditKind.ChangeImplicitMainReturnType)
                            ? EditKind.Delete
                            : EditKind.Update
                    )
                    : diagnosticSpan;
                    ...
                    new RudeEditDiagnostic(
                        ...
                        arguments
                            ?? kind switch
                            {
                                ...
                                or RudeEditKind.SwitchBetweenLambdaAndLocalFunction
                                or RudeEditKind.AccessorKindUpdate
                                or RudeEditKind.InsertConstructorToTypeWithInitializersWithLambdas =>
                                    Array.Empty<string>(),

                                RudeEditKind.ChangingReloadableTypeNotSupportedByRuntime => new[]
                                {
                                    CreateNewOnMetadataUpdateAttributeName,
                                },

                                RudeEditKind.Renamed => new[]
                                {
                                ...
                                _ => new[]
                                {
                                    ...
                                    : distance > 0
                                        ? analyzer.GetDisplayKindAndName(
                                            symbol,
                                            fullyQualify: distance > 1
                                        )
        ...
        )
        {
            switch (newSymbol.Kind)
            {
                ...
                case SymbolKind.Event:
                    if (
                        HasBackingField((IEventSymbol)newSymbol)
                        && HasExplicitOrSequentialLayout(
                            newSymbol.ContainingType,
                            diagnosticContext.NewModel
                        )
                ...
                case SymbolKind.Parameter:
                    ...
                    if (
                        HasBackingField((IParameterSymbol)newSymbol, cancellationToken)
                        && HasExplicitOrSequentialLayout(
                            newSymbol.ContainingType,
                            diagnosticContext.NewModel
                        )
        ...
        private bool HasBackingField(
            ...
            && parameter
                .ContainingType.GetMembers($"<{parameter.Name}>P")
...
#nullable enable
        ...
        )
        {
            ...
            var layoutAttribute = model.Compilation.GetTypeByMetadataName(
            ...
            foreach (var attribute in attributes)
            {
                ...
                if (
                    attribute.AttributeClass.Equals(layoutAttribute)
        ...
        )
        {
            ...
            foreach (var (newType, updatesInCurrentDocument) in updatedTypes)
            {
                ...
                var isPartialEdit = IsPartialTypeEdit(
                    oldType,
                    newType,
                    oldSyntaxTree,
                    newSyntaxTree
                ...
                var aggregateSyntaxMap = CreateAggregateSyntaxMap(
                    topMatch.ReverseMatches,
                ...
                foreach (
                    var newCtor in isStatic
                        ? newType.StaticConstructors
                        : newType.InstanceConstructors
                )
                {
                    if (
                        newType.TypeKind != oldType.TypeKind
                    ...
                    if (!newCtor.IsImplicitlyDeclared)
                    {
                        ...
                        if (topMatch.TryGetOldNode(newDeclaration, out oldDeclaration))
                        {
                            ...
                            oldCtor = (IMethodSymbol)GetRequiredDeclaredSymbol(
                                oldModel,
                                oldDeclaration,
                                cancellationToken
                            );
                            ...
                            hasSignatureChanges = !MemberOrDelegateSignaturesEquivalent(
                                oldCtor,
                                newCtor,
                                exact: false
                            );
                        ...
                        {
                            var resolution = newCtorKey.Resolve(
                                oldCompilation,
                                ignoreAssemblyKey: true,
                                cancellationToken
                            );
                            ...
                            Debug.Assert(
                                oldCtor == null
                                    || MemberOrDelegateSignaturesEquivalent(
                                        oldCtor,
                                        newCtor,
                                        exact: false
                                    )
                    ...
                    else
                    {
                        ...
                        if (
                            !anyInitializerUpdatesInCurrentDocument
                            && oldCtor is { IsImplicitlyDeclared: true }
                        )
                        {
                            continue;
                        }
                    ...
                    if (newDeclaration?.SyntaxTree == newSyntaxTree)
                    {
                        unsupportedOperationReported |=
                            ...
                            || oldDeclaration != null
                                && TryGetDeclarationBody(oldDeclaration, oldCtor) is { } oldBody
                                && ReportUnsupportedOperations(
                                    diagnosticContext,
                                    oldBody,
                                    cancellationToken
                    ...
                    {
                        if (
                            !memberInitializerContainingLambdaReported
                            && AnyMemberInitializerBody(
                                oldType,
                                ContainsLambda,
                                isStatic,
                                cancellationToken
                    ...
                    if (newType.IsRecord)
                    {
                        var oldCtorIsPrimary =
                            oldCtor != null && IsPrimaryConstructor(oldCtor, cancellationToken);
                        var newCtorIsPrimary = IsPrimaryConstructor(newCtor, cancellationToken);

                        if (
                            hasSignatureChanges && oldCtorIsPrimary && newCtorIsPrimary
                        ...
                        )
                        {
                            ...
                            AddDeconstructorEdits(
                                ...
                                oldCompilation,
                                newModel.Compilation,
                                isParameterDelete: newCtorIsPrimary,
                                cancellationToken
                            );
                ...
                if (
                    !isStatic
                    && oldType.TypeKind == TypeKind.Class
                    && newType.TypeKind == TypeKind.Class
                )
                {
                    // Adding the first instance constructor with parameters suppresses synthesized default constructor.
                    if (
                        oldType.HasSynthesizedDefaultConstructor()
                        && !newType.HasSynthesizedDefaultConstructor()
                    )
                    {
                        semanticEdits.Add(
                            SemanticEditInfo.CreateDelete(
                                SymbolKey.Create(
                                    oldType.InstanceConstructors.Single(c => c.Parameters is []),
                                    cancellationToken
                                ),
        ...
        /// </summary>
        private bool AnyMemberInitializerBody(
            INamedTypeSymbol type,
            Func<MemberBody, bool> predicate,
            bool isStatic,
            CancellationToken cancellationToken
        )
        {
            ...
            foreach (var member in type.GetMembers())
            {
                ...
                ) // skip generated fields (e.g. VB auto-property backing fields)
                {
                    var syntax = GetSymbolDeclarationSyntax(member, cancellationToken);
                    if (
                        IsDeclarationWithInitializer(syntax)
                        && TryGetDeclarationBody(syntax, member) is { } body
                        && predicate(body)
                    )
                    {
                        return true;
        ...
        private static IMethodSymbol? TryGetParameterlessConstructor(
            INamedTypeSymbol type,
            bool isStatic
        )
        {
        ...
        private static bool IsPartialTypeEdit(
            ISymbol? oldSymbol,
            ISymbol? newSymbol,
        ...
        )
        {
            ...
            static bool IsPartialTypeEdit(ISymbol? symbol, SyntaxTree tree) =>
                ...
                && symbol.DeclaringSyntaxReferences.Any(IsNotInDocument, tree);
        ...
        )
        {
            ...
            if (activeOrMatchedLambdas != null)
            {
                ...
                foreach (var (oldLambdaBody, newLambdaInfo) in activeOrMatchedLambdas)
                {
                    ...
                    var oldLambdaSymbol = isNestedFunction
                        ? GetLambdaExpressionSymbol(oldModel, oldLambda, cancellationToken)
                        : null;
                    var newLambdaSymbol = isNestedFunction
                        ? GetLambdaExpressionSymbol(newModel, newLambda, cancellationToken)
                        : null;
                    ...
                    if (!oldLambdaBody.IsSyntaxEquivalentTo(newLambdaBody))
                    {
                        ...
                        if (
                            (IsGenericLocalFunction(oldLambda) || IsGenericLocalFunction(newLambda))
                            && !capabilities.Grant(EditAndContinueCapabilities.GenericUpdateMethod)
                        )
                        {
                            diagnosticContext.Report(
                ...
                foreach (var (oldQueryClause, newQueryClause) in map.Forward)
                {
                    ...
                    if (
                        !QueryClauseLambdasTypeEquivalent(
                            oldModel,
                            oldQueryClause,
                            newModel,
                            newQueryClause,
                ...
                {
                    foreach (
                        var newQueryClause in from clause in lazyNewErroneousClauses
            ...
            var oldPrimaryConstructor =
                oldDeclaration != null && IsPrimaryConstructorDeclaration(oldDeclaration)
                    ? (IMethodSymbol)oldMember
                    : GetPrimaryConstructor(oldMember.ContainingType, cancellationToken);

            var newPrimaryConstructor =
                newDeclaration != null && IsPrimaryConstructorDeclaration(newDeclaration)
            ...
            var typeLayoutDisallowsNewCaptures =
                (newPrimaryConstructor != null || oldPrimaryConstructor != null)
                && HasExplicitOrSequentialLayout(newMember.ContainingType, newModel);

            // The primary constructor if its parameters are lifted into fields when accessed from this member, otherwise null.
            var oldLiftingPrimaryConstructor =
                oldMember != oldPrimaryConstructor
                && oldDeclaration != null
                && !IsDeclarationWithInitializer(oldDeclaration)
                    ? oldPrimaryConstructor
                    : null;
            var newLiftingPrimaryConstructor =
                newMember != newPrimaryConstructor
                && newDeclaration != null
                && !IsDeclarationWithInitializer(newDeclaration)
            ...
            using var _1 = ArrayBuilder<int>.GetInstance(
                newInLambdaCaptures.Length,
                fillWithValue: 0,
                out var reverseCapturesMap
            );

            // { new capture index -> new closure scope or null for "this" }
            using var _2 = ArrayBuilder<SyntaxNode?>.GetInstance(
                newInLambdaCaptures.Length,
                fillWithValue: null,
                out var newCapturesToClosureScopes
            );
            ...
            using var _3 = ArrayBuilder<SyntaxNode?>.GetInstance(
                oldInLambdaCaptures.Length,
                fillWithValue: null,
                out var oldCapturesToClosureScopes
            );
            ...
            using var _11 = PooledDictionary<VariableCapture, int>.GetInstance(
                out var oldCapturesIndex
            );
            using var _12 = PooledDictionary<VariableCapture, int>.GetInstance(
            ...
            if (activeOrMatchedLambdas != null)
            {
                ...
                foreach (var (oldLambdaBody, newLambdaInfo) in activeOrMatchedLambdas)
                {
                    ...
                    var accessedOldCaptures = GetAccessedCaptures(
                        oldLambdaBody,
                        oldModel,
                        oldInLambdaCaptures,
                        oldCapturesIndex,
                    ...
                    var accessedNewCaptures = GetAccessedCaptures(
                        newLambdaBody,
                        newModel,
                        newInLambdaCaptures,
                        newCapturesIndex,
                    ...
                    for (
                        var newCaptureIndex = 0;
                        newCaptureIndex < newInLambdaCaptures.Length;
                        newCaptureIndex++
                    )
                    {
                        ...
                        if (newAccessed != oldAccessed)
                        {
                            ...
                            else if (newAccessed)
                            {
                                ...
                                foreach (
                                    var useSite in GetVariableUseSites(
                                        newLambdaBody.GetExpressionsAndStatements(),
                                        newCapture.Symbol,
                                        newModel,
                                        cancellationToken
                                    )
            ...
            foreach (
                var (oldLambda, oldLambdaBody1, oldLambdaBody2) in GetLambdaBodies(oldMemberBody)
            )
            {
            ...
            foreach (
                var (newLambda, newLambdaBody1, newLambdaBody2) in GetLambdaBodies(newMemberBody)
            )
            {
                if (!map.Reverse.ContainsKey(newLambda))
                {
                    if (!CanAddNewLambda(newLambda, newLambdaBody1, newLambdaBody2))
                    {
                        diagnostics.Add(
                            new RudeEditDiagnostic(
                                RudeEditKind.InsertNotSupportedByRuntime,
                                GetDiagnosticSpan(newLambda, EditKind.Insert),
                                newLambda,
                                new string[] { GetDisplayName(newLambda, EditKind.Insert) }
                            )
                        );
                    }
            ...
            bool CanAddNewLambda(
                SyntaxNode newLambda,
                LambdaBody newLambdaBody1,
                LambdaBody? newLambdaBody2
            )
            {
                ...
                var inGenericLocalContext =
                    newMemberBody != null
                    && InGenericLocalContext(newLambda, newMemberBody.RootNodes);
        ...
        private IEnumerable<(
            SyntaxNode lambda,
            LambdaBody lambdaBody1,
            LambdaBody? lambdaBody2
        ...
        private readonly record struct VariableCapture(VariableCaptureKind Kind, ISymbol Symbol)
        {
            ...
            public bool Equals(VariableCapture other) =>
                Kind == other.Kind
                && (Kind == VariableCaptureKind.This || ReferenceEquals(Symbol, other.Symbol));

            public override int GetHashCode() =>
                Hash.Combine(
                    (int)Kind,
                    (Kind == VariableCaptureKind.This) ? 0 : Symbol.GetHashCode()
                );

            public static VariableCapture Create(
                ISymbol variable,
                IMethodSymbol? liftingPrimaryConstructor
            ) => new(GetCaptureKind(variable, liftingPrimaryConstructor), variable);
        }

        private static VariableCaptureKind GetCaptureKind(
            ISymbol variable,
            IMethodSymbol? liftingPrimaryConstructor
        ) =>
            variable is IParameterSymbol parameter
            && (parameter.IsThis || parameter.ContainingSymbol == liftingPrimaryConstructor)
                ? VariableCaptureKind.This
                : VariableCaptureKind.LocalOrParameter;
        ...
        )
        {
            ...
            foreach (var (lambda, lambdaBody1, lambdaBody2) in GetLambdaBodies(memberBody))
            {
                ...
                void AddCaptures(LambdaBody lambdaBody)
                {
                    ...
                    if (!captures.IsEmpty)
                    {
                        ...
                        {
                            var variableCapture = VariableCapture.Create(
                                capture,
            ...
            {
                primaryParametersCapturedViaThis = memberBody
                    .GetCapturedVariables(model)
                    .SelectAsArray(
                        predicate: (capture, liftingPrimaryConstructor) =>
                            capture.ContainingSymbol == liftingPrimaryConstructor,
        ...
        )
        {
            ...
            {
                if (
                    oldLiftingPrimaryConstructor == null
                    || !IsCapturedPrimaryParameterCapturedInType(
                        newCapture,
                        oldLiftingPrimaryConstructor.ContainingType
                    )
                )
                {
                    diagnostics.Add(
            ...
            {
                if (
                    newLiftingPrimaryConstructor == null
                    || !IsCapturedPrimaryParameterCapturedInType(
                        oldCapture,
                        newLiftingPrimaryConstructor.ContainingType
                    )
                )
                {
                    diagnostics.Add(
            ...
            static bool IsCapturedPrimaryParameterCapturedInType(
                IParameterSymbol capture,
                INamedTypeSymbol otherType
            )
            {
            ...
            static string GetLayoutKindDisplay(IParameterSymbol parameter) =>
                (parameter.ContainingType.TypeKind == TypeKind.Struct)
                    ? FeaturesResources.struct_
        ...
        )
        {
            ...
            var accessedCaptures = GetAccessedCaptures(
                lambdaBody,
                model,
                captures,
                capturesIndex,
            ...
            for (var i = 0; i < captures.Length; i++)
            {
                ...
                if (accessedCaptures[i])
                {
                    if (firstAccessedCaptureIndex == -1)
                    {
                        ...
                        newCapturesToClosureScopes[firstAccessedCaptureIndex]
                        != newCapturesToClosureScopes[i]
                    )
                    {
                        ...
                        if (isInsert)
                        {
                            if (capture.IsThis)
                            {
                                errorSpan = GetDiagnosticSpan(
                                    lambdaBody.GetLambda(),
                                    EditKind.Insert
                                );
                            }
                            else
                            {
                                errorSpan = GetVariableUseSites(
                                        lambdaBody.GetExpressionsAndStatements(),
                                        capture.Symbol,
                                        model,
                                        cancellationToken
                                    )
                        ...
                        {
                            errorSpan = GetSymbolLocationSpan(
                                newCaptures[reverseCapturesMap.IndexOf(i)].Symbol,
                                cancellationToken
                            );
                        ...
                        diagnostics.Add(
                            new RudeEditDiagnostic(
                                ...
                                new[]
                                {
                                    GetDisplayName(lambdaBody.GetLambda()),
                                    captures[firstAccessedCaptureIndex].Name,
                                    capture.Name,
                                }
                            )
                        );
        ...
        )
        {
            ...
            foreach (var expressionOrStatement in lambdaBody.GetExpressionsAndStatements())
            {
                ...
                void MarkVariables(ImmutableArray<ISymbol> variables)
                {
                    ...
                    {
                        if (
                            capturesIndex.TryGetValue(
                                VariableCapture.Create(variable, liftingPrimaryConstructor),
                                out var newCaptureIndex
                            )
                        )
                        {
        ...
        /// </summary>
        protected abstract SyntaxNode? GetSymbolDeclarationSyntax(
            ISymbol symbol,
            Func<ImmutableArray<SyntaxReference>, SyntaxReference?> selector,
            CancellationToken cancellationToken
        );

        protected SyntaxNode GetSymbolDeclarationSyntax(
            ISymbol symbol,
            CancellationToken cancellationToken
        ) =>
            GetSymbolDeclarationSyntax(
                symbol,
                selector: System.Linq.ImmutableArrayExtensions.First,
                cancellationToken
            )!;

        protected SyntaxNode? GetSingleSymbolDeclarationSyntax(
            ISymbol symbol,
            CancellationToken cancellationToken
        ) =>
            GetSymbolDeclarationSyntax(
                symbol,
                selector: refs => refs is [var single] ? single : null,
                cancellationToken
            )!;
        ...
        protected SyntaxNode? GetSymbolDeclarationSyntax(
            ...
            GetSymbolDeclarationSyntax(
                symbol,
                syntaxRefs => syntaxRefs.FirstOrDefault(r => r.SyntaxTree == tree),
                cancellationToken
            );

        protected abstract ISymbol? GetDeclaredSymbol(
            SemanticModel model,
        ...
        private TextSpan GetSymbolLocationSpan(ISymbol symbol, CancellationToken cancellationToken)
        {
            ...
            return IsGlobalMain(symbol)
                    ? GetDiagnosticSpan(
                        GetSymbolDeclarationSyntax(symbol, cancellationToken),
                        EditKind.Update
                    )
                : symbol is IParameterSymbol && IsGlobalMain(symbol.ContainingSymbol)
                    ? GetDiagnosticSpan(
        ...
        )
        {
            ...
            var lambda = parameter.ContainingSymbol
                is IMethodSymbol
                {
                    MethodKind: MethodKind.LambdaMethod or MethodKind.LocalFunction
                } containingLambda
                ? GetSymbolDeclarationSyntax(containingLambda, cancellationToken)
                : null;
            ...
            if (
                parameter is
                {
                    ...
                    ContainingSymbol: IMethodSymbol { AssociatedSymbol: { } associatedSymbol }
                }
            )
            {
            ...
            return new CapturedParameterKey(
                ...
                GetSymbolDeclarationSyntax(parameter, cancellationToken),
                lambda
            );
        }
        ...
        )
        {
            if (parameterKey.ContainingLambda == null)
            {
                ...
                if (
                    parameterKey.Syntax == null
                    || parameterMap?.TryGetValue(parameterKey.Syntax, out mappedParameter) == true
                )
                {
                ...
                bodyMap.TryGetValue(
                    parameterKey.ContainingLambda,
                    out var mappedContainingLambdaSyntax
                )
            )
            {
                ...
                {
                    mappedParameterKey = parameterKey with
                    {
                        Syntax = mappedParameter,
                        ContainingLambda = mappedContainingLambdaSyntax,
                    };
                    return true;
                }
            }
        ...
        private readonly record struct CapturedParameterKey(
            ...
            SyntaxNode? ContainingLambda = null
        );
        ...
        )
        {
            ...
            if (oldDeclaration != null && newDeclaration != null)
            {
                ...
                )
                {
                    ...
                    parameterMap =
                        (parameterMap != null)
                            ? parameterMap.Value.With(primaryParameterMap.Value)
                            : primaryParameterMap;
                }
            }
            ...
            using var _2 = PooledDictionary<CapturedParameterKey, int>.GetInstance(
            ...
            for (var oldCaptureIndex = 0; oldCaptureIndex < oldCaptures.Length; oldCaptureIndex++)
            {
                ...
                {
                    oldParameterCaptures.Add(
                        GetParameterKey(oldParameterCapture, cancellationToken),
                        oldCaptureIndex
                    );
                }
                else
                {
                    oldLocalCaptures.Add(
                        GetSymbolDeclarationSyntax(oldCapture.Symbol, cancellationToken),
                        oldCaptureIndex
                    );
                }
            }
            ...
            for (var newCaptureIndex = 0; newCaptureIndex < newCaptures.Length; newCaptureIndex++)
            {
                ...
                if (newCapture.Symbol is IParameterSymbol newParameterCapture)
                {
                    ...
                    if (
                        !TryMapParameter(
                            newParameterKey,
                            parameterMap?.Reverse,
                            bodyMap.Reverse,
                            out var oldParameterKey
                ...
                else
                {
                    ...
                    if (
                        !bodyMap.Reverse.TryGetValue(newCaptureSyntax, out var mappedOldSyntax)
            ...
            if (oldParameterCaptures.Count > 0)
            {
                ...
                foreach (
                    var (
                        (oldParameterKind, oldParameterSyntax, oldContainingLambdaSyntax),
                        oldCaptureIndex
                    ) in oldParameterCaptures
                )
                {
                    diagnostics.Add(
                        new RudeEditDiagnostic(
                            ...
                            oldParameterKind switch
                            {
                                ...
                                parameterMap?.Forward.TryGetValue(
                                    oldParameterSyntax!,
                                    out var newParameterSyntax
                                ) == true
                                    ? GetDiagnosticSpan(newParameterSyntax, EditKind.Update)
                                : bodyMap.Forward.TryGetValue(
                                    oldParameterSyntax!,
                                    out newParameterSyntax
                                )
                                    ? GetDiagnosticSpan(newParameterSyntax, EditKind.Update)
                                ...
                                && bodyMap.Forward.TryGetValue(
                                    ...
                                    ? GetDiagnosticSpan(newContainingLambdaSyntax, EditKind.Update)
                                : GetSymbolLocationSpan(newMember, cancellationToken),
        ...
        )
        {
            ...
            else if (!ReturnTypesEquivalent(oldLambdaSymbol, newLambdaSymbol, exact: false))
            {
                ...
                !TypeParametersEquivalent(
                    oldLambdaSymbol.TypeParameters,
                    newLambdaSymbol.TypeParameters,
                    exact: false
                )
                || !oldLambdaSymbol.TypeParameters.SequenceEqual(
                    newLambdaSymbol.TypeParameters,
            ...
            {
                ReportCustomAttributeRudeEdits(
                    diagnosticContext.WithSymbols(
                        oldLambdaSymbol.Parameters[i],
                        newLambdaSymbol.Parameters[i]
                    ),
                    capabilities,
                    out _,
                    out _,
            ...
            for (var i = 0; i < oldLambdaSymbol.TypeParameters.Length; i++)
            {
                ReportCustomAttributeRudeEdits(
                    diagnosticContext.WithSymbols(
                        oldLambdaSymbol.TypeParameters[i],
                        newLambdaSymbol.TypeParameters[i]
                    ),
                    capabilities,
                    out _,
                    out _,
        ...
        private static bool AreEquivalentClosureScopes(
            SyntaxNode? oldScope,
            SyntaxNode? newScope,
            IReadOnlyDictionary<SyntaxNode, SyntaxNode> reverseMap
        )
        ...
        )
        {
            ...
            {
                { IsIterator: true } and { IsAsync: true } =>
            ...
            if (oldCompilation.GetTypeByMetadataName(stateMachineAttributeQualifiedName) == null)
            {
                diagnosticContext.Report(
                    ...
                    cancellationToken,
                    arguments: new[] { stateMachineAttributeQualifiedName }
                );
            }
        }
        ...
        private static SyntaxNode? FindPartner(
            OneOrMany<SyntaxNode> rootNodes,
            OneOrMany<SyntaxNode> otherRootNodes,
        ...
        internal static SyntaxNode FindPartner(
            SyntaxNode root,
            SyntaxNode otherRoot,
            SyntaxNode otherNode
        )
        {
            ...
            while (otherNode != originalLeftNode)
            {
                ...
                var leftChild = ChildThatContainsPosition(
                    otherNode,
                    leftPosition,
                    out var childIndex
                );
        ...
        )
        {
            ...
            Debug.Assert(
                !self.FullSpan.Contains(position),
                "Position is valid. How could we not find a child?"
            );
            throw new ArgumentOutOfRangeException(nameof(position));
        }

        internal static void FindLeafNodeAndPartner(
            SyntaxNode leftRoot,
            int leftPosition,
        ...
        private sealed class TypedConstantComparer : IEqualityComparer<TypedConstant>
        {
            ...
            public bool Equals(TypedConstant x, TypedConstant y) =>
                ...
                && x.Kind switch
                {
                    TypedConstantKind.Array => x.Values.IsDefault
                        || x.Values.SequenceEqual(y.Values, Instance),
                    TypedConstantKind.Type => TypesEquivalent(
                        (ITypeSymbol?)x.Value,
                        (ITypeSymbol?)y.Value,
                        exact: false
                    ),
            ...
            public int GetHashCode(TypedConstant obj) => obj.GetHashCode();
        }

        private sealed class NamedArgumentComparer
            : IEqualityComparer<KeyValuePair<string, TypedConstant>>
        {
            public static readonly NamedArgumentComparer Instance = new();

            public bool Equals(
                KeyValuePair<string, TypedConstant> x,
                KeyValuePair<string, TypedConstant> y
            ) => x.Key.Equals(y.Key) && TypedConstantComparer.Instance.Equals(x.Value, y.Value);

            public int GetHashCode(KeyValuePair<string, TypedConstant> obj) => obj.GetHashCode();
        }
        ...
        public IMethodSymbol? GetPrimaryConstructor(
        ...
        public bool IsPrimaryConstructor(ISymbol symbol, CancellationToken cancellationToken) =>
            ...
            && IsPrimaryConstructorDeclaration(
                GetSymbolDeclarationSyntax(symbol, cancellationToken)
            );
        ...
        /// </summary>
        public bool IsPrimaryConstructorParameterMatchingSymbol(
            ISymbol symbol,
            CancellationToken cancellationToken
        ) =>
            ...
            && primaryCtor.Parameters.Any(
                static (parameter, name) => parameter.Name == name,
                symbol.Name
            );
        ...
        /// </summary>
        public IMethodSymbol? GetEncompassingPrimaryConstructor(
            SyntaxNode declaration,
            ISymbol symbol,
            CancellationToken cancellationToken
        ) =>
            ...
            : IsDeclarationWithInitializer(declaration)
                ? symbol.ContainingType.InstanceConstructors.FirstOrDefault(
                    IsPrimaryConstructor,
                    cancellationToken
                )
            : null;

        private static IPropertySymbol? GetPropertySynthesizedForRecordPrimaryConstructorParameter(
            IParameterSymbol parameter
        ) =>
            (IPropertySymbol?)
                parameter
                    .ContainingType.GetMembers(parameter.Name)
                    .FirstOrDefault(static m =>
                        m
                            is IPropertySymbol
                            {
        ...
        private static bool SymbolPresenceAffectsSynthesizedRecordMembers(ISymbol symbol) =>
            symbol
                is { IsStatic: false, ContainingType.IsRecord: true }
                    and (
                        IPropertySymbol
                        {
                            GetMethod.IsImplicitlyDeclared: false,
                            SetMethod: null or { IsImplicitlyDeclared: false }
                        }
                        or IFieldSymbol
                    );
        ...
        /// </summary>
        private bool DeleteEditImpliesInsertEdit(
            ISymbol oldSymbol,
            ISymbol newSymbol,
            Compilation oldCompilation,
            CancellationToken cancellationToken
        )
        {
            ...
            if (
                newSymbol
                    is IMethodSymbol
                    {
                        ...
                        ContainingType.IsRecord: true,
                        ReturnsVoid: true,
                        Name: WellKnownMemberNames.DeconstructMethodName
                    } method
                && GetPrimaryConstructor(newSymbol.ContainingType, cancellationToken)
                    is { } newPrimaryConstructor
                && method.HasDeconstructorSignature(newPrimaryConstructor)
            )
            {
                var oldConstructor = SymbolKey
                    .Create(newPrimaryConstructor, cancellationToken)
                    .Resolve(oldCompilation, ignoreAssemblyKey: true, cancellationToken)
                    .Symbol;
            ...
            if (
                newSymbol is IPropertySymbol { IsStatic: false, ContainingType.IsRecord: true }
                && GetPrimaryConstructor(newSymbol.ContainingType, cancellationToken)
                    ?.Parameters.FirstOrDefault(
                        static (parameter, name) => parameter.Name == name,
                        newSymbol.Name
                    )
                    is { } newPrimaryParameter
            )
            {
                var oldParameter = SymbolKey
                    .Create(newPrimaryParameter, cancellationToken)
                    .Resolve(oldCompilation, ignoreAssemblyKey: true, cancellationToken)
                    .Symbol;
                ...
                return !newPrimaryParameter.IsSynthesized()
                    && (
                        oldParameter == null
                        || oldParameter.IsSynthesized()
                        || !IsPrimaryConstructor(oldParameter.ContainingSymbol, cancellationToken)
                        || oldProperty.IsSynthesizedAutoProperty()
                    );
            }
            ...
            if (newSymbol is IMethodSymbol { AssociatedSymbol: IPropertySymbol { } newProperty })
            {
                ...
                return DeleteEditImpliesInsertEdit(
                    oldProperty,
                    newProperty,
                    oldCompilation,
                    cancellationToken
        ...
        private static bool HasPrintMembersSignature(
        ...
        internal readonly struct TestAccessor(
            AbstractEditAndContinueAnalyzer abstractEditAndContinueAnalyzer
        )
        {
            private readonly AbstractEditAndContinueAnalyzer _abstractEditAndContinueAnalyzer =
                abstractEditAndContinueAnalyzer;

            internal void ReportTopLevelSyntacticRudeEdits(
                ArrayBuilder<RudeEditDiagnostic> diagnostics,
                EditScript<SyntaxNode> syntacticEdits,
                Dictionary<SyntaxNode, EditKind> editMap
            ) =>
                _abstractEditAndContinueAnalyzer.ReportTopLevelSyntacticRudeEdits(
                    diagnostics,
                    syntacticEdits,
                    editMap
                );
            ...
            {
                return _abstractEditAndContinueAnalyzer.IncludeLambdaBodyMaps(
                    bodyMatch,
                    memberBodyActiveNodes,
                    ref lazyActiveOrMatchedLambdas
                );
