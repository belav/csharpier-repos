...
namespace Microsoft.CodeAnalysis.EditAndContinue
{
    internal abstract class AbstractEditAndContinueAnalyzer : IEditAndContinueAnalyzer
    {
        ...
        /// </summary>
        internal abstract SyntaxNode FindPartner(
            SyntaxNode leftRoot,
            SyntaxNode rightRoot,
            SyntaxNode leftNode
        );

        internal abstract SyntaxNode? FindPartnerInMemberInitializer(
        ...
        protected abstract void ReportLocalFunctionsDeclarationRudeEdits(
        ...
        internal abstract void ReportTypeDeclarationInsertDeleteRudeEdits(
            ArrayBuilder<RudeEditDiagnostic> diagnostics,
        ...
        )
        {
            ...
            try
            {
                ...
                if (oldDocument != null)
                {
                    oldTree = await oldDocument.GetSyntaxTreeAsync(cancellationToken)
                        .ConfigureAwait(false);
                ...
                var newTree = await newDocument.GetSyntaxTreeAsync(cancellationToken)
                    .ConfigureAwait(false);
                ...
                var hasSyntaxError = syntaxDiagnostics.Any(
        ...
        internal static Dictionary<SyntaxNode, EditKind> BuildEditMap(
        ...
        )
        {
            ...
            for (var i = 0; i < newActiveStatements.Count; i++)
            {
                if (newActiveStatements[i] == null)
                {
                    ...
                    var trackedSpan = newActiveStatementSpans.IsEmpty
                        ? default
                        : newActiveStatementSpans[i];
                    if (trackedSpan.Length != 0 && newMember.Span.Contains(trackedSpan))
                    {
                        var trackedStatement = FindStatement(
                            newBody,
                            trackedSpan,
                    ...
                    if (diagnostics.Count == 0)
                    {
                        var ancestors = GetExceptionHandlingAncestors(
                            newStatement,
                            oldActiveStatements[i].IsNonLeaf
                        );
        ...
        )
        {
            ...
            var hasActiveStatement = TryGetOverlappingActiveStatements(
                oldText,
                oldDeclaration.Span,
                oldActiveStatements,
                out var start,
            ...
            try
            {
                ...
                for (var i = 0; i < activeNodes.Length; i++)
                {
                    ...
                    var trackedSpan = newActiveStatementSpans.IsEmpty
                        ? (TextSpan?)null
                        : newActiveStatementSpans[ordinal];
                    if (trackedSpan.HasValue)
                    {
                        // The tracking span might have been deleted or moved outside of the member span.
                        // It is not an error to move the statement - we just ignore it.
                        if (
                            trackedSpan.Value.Length != 0
                            && newDeclaration.Span.Contains(trackedSpan.Value)
                        )
                        {
                            var newStatementSyntax = FindStatement(
                                newBody,
                                trackedSpan.Value,
                                out var part
                            );
                ...
                for (var i = 0; i < activeNodes.Length; i++)
                {
                    ...
                    var isPartiallyExecuted =
                        (
                            oldActiveStatements[ordinal].Flags
        ...
        public ImmutableArray<LinePositionSpan> GetExceptionRegions(
            SourceText text,
            SyntaxNode syntaxRoot,
            LinePositionSpan activeStatementSpan,
            bool isNonLeaf,
            out bool isCovered
        )
        {
        ...
        private ImmutableArray<LinePositionSpan> GetExceptionRegions(
        ...
        )
        {
            ...
            while (
                i < statements.Length
                && !declarationSpan.OverlapsWith(lines.GetTextSpanSafe(statements[i].Span))
            )
            {
                i++;
            }
        ...
        )
        {
            ...
            foreach (var (oldNode, newNode) in topMatch.Matches)
            {
                ...
                if (editMap.ContainsKey(newNode))
                {
                    ...
                    Debug.Assert(
                        editMap[newNode] == EditKind.Update || editMap[newNode] == EditKind.Insert
                    );
                    continue;
                }
                ...
                while (true)
                {
                    ...
                    if (isFirstToken)
                    {
                        ...
                        firstTokenLineChange =
                            (lineDelta != 0)
                                ? new SourceLineUpdate(oldPosition.Line, newPosition.Line)
                                : default;
                    }
        ...
        protected static readonly SymbolEquivalenceComparer s_assemblyEqualityComparer =
            new(
        ...
        {
            return oldParameters.SequenceEqual(
                    newParameters,
        ...
        )
        {
            ...
            var oldModel =
                (oldDocument != null)
                    ? await oldDocument.GetRequiredSemanticModelAsync(cancellationToken)
                          .ConfigureAwait(false)
                    : null;
            var newModel = await newDocument.GetRequiredSemanticModelAsync(cancellationToken)
                .ConfigureAwait(false);
            var oldCompilation =
                oldModel?.Compilation
                ?? await oldProject.GetRequiredCompilationAsync(cancellationToken)
                    .ConfigureAwait(false);
            ...
            try
            {
                ...
                for (var i = 0; i < editScript.Edits.Length; i++)
                {
                    ...
                    switch (edit.Kind)
                    {
                        ...
                        case EditKind.Delete:

                            {
                                ...
                                oldSymbol = GetSymbolForEdit(
                                    oldModel,
                                    edit.OldNode,
                                    edit.Kind,
                                    editMap,
                                    out var oldIsAmbiguous,
                                    cancellationToken
                                );
                                if (
                                    oldSymbol == null
                                    || oldIsAmbiguous
                                    || !processedSymbols.Add(oldSymbol)
                                )
                                {
                                ...
                                )
                                {
                                    ...
                                    if (
                                        newSymbol.DeclaredAccessibility
                                        != oldSymbol.DeclaredAccessibility
                                    )
                                    {
                                        ReportDeletedMemberRudeEdit(
                                            diagnostics,
                                ...
                                else
                                {
                                    ...
                                    var oldContainingSymbol =
                                        (oldSymbol as IMethodSymbol)?.AssociatedSymbol
                                        ?? oldSymbol.ContainingType;
                        ...
                        case EditKind.Insert:

                            {
                                ...
                                syntaxMap = null;
                                oldSymbol = null;
                                newSymbol = GetSymbolForEdit(
                                    newModel,
                                    edit.NewNode,
                                    edit.Kind,
                                    editMap,
                                    out var newIsAmbiguous,
                                    cancellationToken
                                );
                                if (
                                    newSymbol == null
                                    || newIsAmbiguous
                                    || !processedSymbols.Add(newSymbol)
                                )
                                {
                                ...
                                if (oldSymbol != null)
                                {
                                    ...
                                    if (oldSymbol.IsImplicitlyDeclared)
                                    {
                                        ...
                                        )
                                        {
                                            diagnostics.Add(
                                                new RudeEditDiagnostic(
                                                    ...
                                                    arguments: new[]
                                                    {
                                                        GetDisplayName(edit.NewNode, edit.Kind)
                                                    }
                                                )
                                            );
                                            continue;
                                        ...
                                        {
                                            var oldNode = GetSymbolDeclarationSyntax(
                                                oldSymbol.DeclaringSyntaxReferences[0],
                                                cancellationToken
                                            );
                                    ...
                                    )
                                    {
                                        ...
                                        {
                                            TypeKind: not (TypeKind.Delegate or TypeKind.Enum)
                                        } newTypeSymbol
                                    )
                                    {
                                        ...
                                        ReportTypeDeclarationInsertDeleteRudeEdits(
                                            diagnostics,
                                            oldTypeSymbol,
                                            newTypeSymbol,
                                            edit.NewNode,
                                    ...
                                    )
                                    {
                                        ...
                                        if (
                                            IsConstructorWithMemberInitializers(newNode)
                                ...
                                else if (newSymbol.ContainingType != null)
                                {
                                    ...
                                    var newAssociatedMemberDeclaration =
                                        TryGetAssociatedMemberDeclaration(edit.NewNode);
                                    if (
                                        newAssociatedMemberDeclaration != null
                                        && HasEdit(
                                            editMap,
                                ...
                                var isConstructorWithMemberInitializers =
                                    IsConstructorWithMemberInitializers(edit.NewNode);
                                if (
                    ...
                    if (editKind == SemanticEditKind.Update)
                    {
                        ...
                        if (
                            newSymbol is IPropertySymbol
                            {
                                GetMethod: var propertyGetter and not null
                            }
                        )
                        {
                ...
                foreach (var (oldNode, newNode) in triviaEdits)
                {
                    ...
                    var newSymbol = GetSymbolForEdit(
                        newModel,
                        newNode,
                        EditKind.Update,
                        editMap,
                    ...
                    var oldSymbol = GetSymbolForEdit(
                        oldModel,
                        oldNode,
                        EditKind.Update,
                        editMap,
                    ...
                    var isActiveMember =
                        TryGetOverlappingActiveStatements(
                            oldText,
                            oldNode.Span,
                            oldActiveStatements,
                            out var start,
                    ...
                    var syntaxMap = isActiveMember
                        ? CreateSyntaxMapForEquivalentNodes(oldNode, newNode)
                        : null;

                    // only trivia changed:
                    Contract.ThrowIfFalse(
                        IsConstructorWithMemberInitializers(oldNode)
                            == IsConstructorWithMemberInitializers(newNode)
                    );
                    Contract.ThrowIfFalse(
                        IsDeclarationWithInitializer(oldNode)
...
#nullable enable
        ...
        {
            return newNode =>
                newRoot.FullSpan.Contains(newNode.SpanStart)
                    ? FindPartner(newRoot, oldRoot, newNode)
                    : null;
        }
            ...
            foreach (var (newType, updatesInCurrentDocument) in updatedTypes)
            {
                ...
                )
                {
                    ...
                    if (!newCtor.IsImplicitlyDeclared)
                    {
                        ...
                        var newDeclaration = GetSymbolDeclarationSyntax(
        ...
        )
        {
            ...
            {
                if (
                    TryGetLambdaBodies(oldLambda, out var oldLambdaBody1, out var oldLambdaBody2)
                    && !map.Forward.ContainsKey(oldLambda)
                )
                {
                    ReportMultiScopeCaptures(
                        oldLambdaBody1,
        ...
        )
        {
            ...
            if (
                containingLambda?.MethodKind == MethodKind.LambdaMethod
        ...
        internal readonly struct TestAccessor
        {
            ...
            {
                _abstractEditAndContinueAnalyzer.AnalyzeUnchangedDocument(
                    oldActiveStatements,
                    newText,
                    newRoot,
                    newActiveStatements,
            ...
            {
                _abstractEditAndContinueAnalyzer.AnalyzeTrivia(
                    oldSource,
                    newSource,
                    topMatch,
                    editMap,
