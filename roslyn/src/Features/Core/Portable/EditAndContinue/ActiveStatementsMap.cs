    ...
    internal sealed class ActiveStatementsMap
    {
        ...
        public static readonly Comparer<ActiveStatement> Comparer =
            Comparer<ActiveStatement>.Create(
                (x, y) => x.FileSpan.Start.CompareTo(y.FileSpan.Start)
            );

        private static readonly Comparer<(ManagedActiveStatementDebugInfo, SourceFileSpan, int)> s_infoSpanComparer =
            Comparer<(ManagedActiveStatementDebugInfo, SourceFileSpan span, int)>.Create(
                (x, y) => x.span.Start.CompareTo(y.span.Start)
            );
        ...
        )
        {
            using var _1 = PooledDictionary<
                string,
                ArrayBuilder<(ManagedActiveStatementDebugInfo info, SourceFileSpan span, int ordinal)>
            >.GetInstance(out var updatedSpansByDocumentPath);
            ...
            foreach (var debugInfo in debugInfos)
            {
                ...
                {
                    updatedSpansByDocumentPath.Add(
                        documentName,
                        documentInfos =
                            ArrayBuilder<(ManagedActiveStatementDebugInfo, SourceFileSpan, int)>.GetInstance()
                    );
                }

                documentInfos.Add(
                    (debugInfo, new SourceFileSpan(documentName, baseSpan), ordinal++)
                );
            }
        ...
        )
        {
            ...
            if (remapping.TryGetValue(instructionId.Method, out var regionsInMethod))
            {
                ...
                {
                    if (
                        region.Span.Span.Contains(activeSpan)
                        && activeStatementInfo.DocumentName == region.Span.Path
                    )
                    {
        ...
        public bool IsEmpty => InstructionMap.IsEmpty();

        internal async ValueTask<
            ImmutableArray<UnmappedActiveStatement>
        > GetOldActiveStatementsAsync(
            IEditAndContinueAnalyzer analyzer,
        ...
        )
        {
            ...
            return ImmutableInterlocked.GetOrAdd(
                ...
                oldSyntaxTree,
                oldSyntaxTree =>
                    CalculateOldActiveStatementsAndExceptionRegions(
                        analyzer,
                        oldSyntaxTree,
                        oldText,
                        oldRoot,
        ...
        )
        {
            ...
            void AddStatement(LinePositionSpan unmappedLineSpan, ActiveStatement activeStatement)
            {
                ...
                if (
                    TryGetTextSpan(oldText.Lines, unmappedLineSpan, out var unmappedSpan)
                    && mappedStatements.Add(activeStatement)
                )
                {
                    var exceptionRegions = analyzer.GetExceptionRegions(
                        oldRoot,
                        unmappedSpan,
                    ...
                    builder.Add(
                        new UnmappedActiveStatement(unmappedSpan, activeStatement, exceptionRegions)
                    );
                }
            }
            ...
            foreach (var lineMapping in oldTree.GetLineMappings(cancellationToken))
            {
                ...
                var targetPath = mappedSection.HasMappedPath
                    ? mappedSection.Path
                    : oldTree.FilePath;
                ...
                if (DocumentPathMap.TryGetValue(targetPath, out var activeStatementsInMappedFile))
                {
                    ...
                    for (var i = range.Start.Value; i < range.End.Value; i++)
                    {
                        ...
                        var unmappedLineSpan = ReverseMapLinePositionSpan(
                            unmappedSection,
                            mappedSection.Span,
            ...
            Debug.Assert(
                builder.IsSorted(
                    Comparer<UnmappedActiveStatement>.Create(
                        (x, y) => x.UnmappedSpan.Start.CompareTo(y.UnmappedSpan.End)
                    )
                )
            );
        ...
        private static LinePositionSpan ReverseMapLinePositionSpan(
            LinePositionSpan unmappedSection,
            LinePositionSpan mappedSection,
        ...
        )
        {
            ...
            var unmappedStartColumn =
                (mappedSpan.Start.Line == mappedSection.Start.Line)
                    ? unmappedSection.Start.Character + mappedSpan.Start.Character
                      - mappedSection.Start.Character
                    : mappedSpan.Start.Character;

            var unmappedEndColumn =
                (mappedSpan.End.Line == mappedSection.Start.Line)
                    ? unmappedSection.Start.Character + mappedSpan.End.Character
                      - mappedSection.Start.Character
                    : mappedSpan.End.Character;

            return new(
                new(unmappedStartLine, unmappedStartColumn),
                new(unmappedEndLine, unmappedEndColumn)
            );
        }
