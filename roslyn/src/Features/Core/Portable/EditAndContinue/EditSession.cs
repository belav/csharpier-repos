    ...
    internal sealed class EditSession
    {
        ...
        > GetBaseActiveExceptionRegionsAsync(Solution solution, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var baseActiveStatements = await BaseActiveStatements
                ...
                foreach (var activeStatement in instructionMap.Values)
                {
                    ...
                    var primaryDocument = await solution
                        .GetDocumentAsync(
                    ...
                    if (baseDocument != null)
                    {
                        ...
                        var sourceText = await baseDocument
                            .GetTextAsync(cancellationToken)
                            .ConfigureAwait(false);
                        var syntaxRoot = await baseDocument
        ...
        )
        {
            ...
            var analyses = await Analyses
                .GetDocumentAnalysesAsync(oldProject, builder, cancellationToken)
                .ConfigureAwait(false);
        ...
        )
        {
            try
            {
                ...
                foreach (var project in projects)
                {
                    ...
                    var (mvid, mvidReadError) = await DebuggingSession
                        .GetProjectModuleIdAsync(project, cancellationToken)
                        .ConfigureAwait(false);
        ...
        )
        {
            try
            {
                ...
                foreach (var newProject in solution.Projects)
                {
                    ...
                    var (mvid, mvidReadError) = await DebuggingSession
                        .GetProjectModuleIdAsync(newProject, cancellationToken)
                        .ConfigureAwait(false);
                    if (mvidReadError != null)
                    {
                    ...
                    var oldCompilation = await oldProject
                        ...
                        .ConfigureAwait(false);
                    var newCompilation = await newProject
                        .GetCompilationAsync(cancellationToken)
                        .ConfigureAwait(false);
                    ...
                    var lineEdits = await projectChanges.LineChanges
                        .SelectAsArrayAsync(
                            async (lineChange, cancellationToken) =>
                            {
                                var document = await newProject
                                    .GetDocumentAsync(
