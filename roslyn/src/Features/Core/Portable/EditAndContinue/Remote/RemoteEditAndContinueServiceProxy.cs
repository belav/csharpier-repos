    ...
    internal readonly struct RemoteEditAndContinueServiceProxy : IActiveStatementSpanProvider
    {
              ...
              IRemoteEditAndContinueService.ICallback
        {
            ...
            public ValueTask<ImmutableArray<TextSpan>> GetSpansAsync(
                ...
                (
                    (SolutionActiveStatementSpanProviderCallback)GetCallback(callbackId)
                ).GetSpansAsync(documentId, cancellationToken);
        ...
        private sealed class DocumentActiveStatementSpanProviderCallback
        {
            ...
            public DocumentActiveStatementSpanProviderCallback(
                DocumentActiveStatementSpanProvider documentProvider
        ...
        private sealed class SolutionActiveStatementSpanProviderCallback
        {
            ...
            public SolutionActiveStatementSpanProviderCallback(
                SolutionActiveStatementSpanProvider solutionProvider
        ...
        private sealed class EditSessionCallback
        {
            ...
            )
            {
                try
                {
                    ...
                    when (FatalError.ReportAndCatchUnlessCanceled(e, cancellationToken))
                {
                    return new ManagedEditAndContinueAvailability(
                        ManagedEditAndContinueAvailabilityStatus.InternalError,
                        e.Message
                    );
                }
            }
        ...
        public async ValueTask<IDisposable?> StartDebuggingSessionAsync(
            Solution solution,
                ...
                var documentsToReanalyzeOpt = await client
                    .TryInvokeAsync<IRemoteEditAndContinueService, ImmutableArray<DocumentId>>(
                        (service, cancallationToken) =>
                            service.BreakStateEnteredAsync(cancellationToken),
                        cancellationToken
                    )
                    .ConfigureAwait(false);
        ...
        )
        {
            ...
            else
            {
                var documentsToReanalyzeOpt = await client
                    .TryInvokeAsync<IRemoteEditAndContinueService, ImmutableArray<DocumentId>>(
                        (service, cancallationToken) =>
                            service.EndDebuggingSessionAsync(cancellationToken),
                        cancellationToken
                    )
                    .ConfigureAwait(false);
        ...
        )
        {
            ...
            {
                result.Add(
                    await data.ToDiagnosticAsync(document.Project, cancellationToken)
                        .ConfigureAwait(false)
                );
            }
        ...
        public async ValueTask<(ManagedModuleUpdates updates, ImmutableArray<DiagnosticData> diagnostics, ImmutableArray<(DocumentId DocumentId, ImmutableArray<RudeEditDiagnostic> Diagnostics)>)> EmitSolutionUpdateAsync(
            Solution solution,
            SolutionActiveStatementSpanProvider activeStatementSpanProvider,
            IDiagnosticAnalyzerService diagnosticService,
            EditAndContinueDiagnosticUpdateSource diagnosticUpdateSource,
        ...
        )
        {
            ...
            {
                var result = await client
                    .TryInvokeAsync<IRemoteEditAndContinueService, EmitSolutionUpdateResults.Data>(
                        solution,
                        (service, solutionInfo, callbackId, cancellationToken) =>
        ...
        )
        {
            ...
            else
            {
                var documentsToReanalyzeOpt = await client
                    .TryInvokeAsync<IRemoteEditAndContinueService, ImmutableArray<DocumentId>>(
                        (service, cancallationToken) =>
                            service.CommitSolutionUpdateAsync(cancellationToken),
                        cancellationToken
                    )
                    .ConfigureAwait(false);
            ...
            await client
                .TryInvokeAsync<IRemoteEditAndContinueService>(
                    (service, cancellationToken) =>
                        service.DiscardSolutionUpdateAsync(cancellationToken),
                    cancellationToken
                )
                .ConfigureAwait(false);
        }
