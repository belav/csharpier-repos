    ...
    internal abstract class AbstractNavigationBarItemService : INavigationBarItemService
    {
        ...
        )
        {
            ...
            if (referencesInCurrentFile.Length > 0)
            {
                ...
                var navigationLocationSpan =
                    symbol.Locations.FirstOrDefault(loc => loc.SourceTree == tree)?.SourceSpan
                    ?? referencesInCurrentFile.First().Span;

                var spans = referencesInCurrentFile.SelectAsArray(r => computeFullSpan(r));
                return new SymbolItemLocation(
                    (spans, navigationLocationSpan),
                    otherDocumentInfo: null
                );
            }
            else
            {
                ...
                var navigationLocation =
                    symbol.Locations.FirstOrDefault(loc =>
                        loc.SourceTree != null && loc.SourceTree != tree
                    )
                    ?? Location.Create(
                ...
                return new SymbolItemLocation(
                    inDocumentInfo: null,
                    (documentId, navigationLocation.SourceSpan)
                );
            }
        }
        ...
        )
        {
            return GetSymbolLocation(
                ...
                symbol,
                tree,
                r => r.GetSyntax().FullSpan,
                symbolDeclarationService.GetDeclarations(symbol)
            );
        }
    }
}
