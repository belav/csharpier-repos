...
namespace Microsoft.CodeAnalysis.QuickInfo
{
    internal abstract partial class CommonSemanticQuickInfoProvider : CommonQuickInfoProvider
    {
        ...
        )
        {
            ...
            var (model, tokenInformation) = await BindTokenAsync(document, token, cancellationToken)
                .ConfigureAwait(false);
        ...
        )
        {
            ...
            var candidateResults =
                new List<(DocumentId docId, SemanticModel model, TokenInformation tokenInformation)>
                {
            ...
            var bestBinding =
                candidateResults.FirstOrNull(c => HasNoErrors(c.tokenInformation.Symbols))
            ...
            var supportedPlatforms = new SupportedPlatformData(
                invalidProjects,
                candidateProjects,
                document.Project.Solution.Workspace
            );
        ...
        private static bool HasNoErrors(ImmutableArray<ISymbol> symbols) =>
            symbols.Length > 0 && !ErrorVisitor.ContainsError(symbols.FirstOrDefault());
        ...
        )
        {
            ...
            return QuickInfoUtilities.CreateQuickInfoItemAsync(
                ...
                tokenInformation.NullableFlowState,
                cancellationToken
            );
        }
        ...
        protected virtual NullableFlowState GetNullabilityAnalysis(
            Workspace workspace,
            SemanticModel semanticModel,
            ISymbol symbol,
            SyntaxNode node,
        ...
        )
        {
            ...
            var semanticModel = await document
                ...
                .ConfigureAwait(false);
            var enclosingType = semanticModel.GetEnclosingNamedType(
                token.SpanStart,
                cancellationToken
            );

            var symbols = GetSymbolsFromToken(
                token,
                document.Project.Solution.Workspace,
                semanticModel,
                cancellationToken
            ...
            if (symbols.Any())
            {
                ...
                {
                    nullableFlowState = GetNullabilityAnalysis(
                        document.Project.Solution.Workspace,
                        semanticModel,
                        firstSymbol,
                        bindableParent,
            ...
            if (syntaxFacts.IsOperator(token))
            {
                ...
                if (IsOk(typeInfo.Type))
                {
                    return (
                        semanticModel,
                        new TokenInformation(ImmutableArray.Create<ISymbol>(typeInfo.Type))
                    );
                }
            }
        ...
        private ImmutableArray<ISymbol> GetSymbolsFromToken(
            SyntaxToken token,
            Workspace workspace,
            SemanticModel semanticModel,
            CancellationToken cancellationToken
        )
        {
            ...
            return semanticModel
                .GetSemanticInfo(token, workspace, cancellationToken)
