...
namespace Microsoft.CodeAnalysis.IntroduceUsingStatement
{
        ...
        where TTryStatementSyntax : TStatementSyntax
    {
        ...
        protected abstract SyntaxList<TStatementSyntax> GetSurroundingStatements(
        ...
        protected abstract (
            SyntaxList<TStatementSyntax> tryStatements,
            SyntaxList<TStatementSyntax> finallyStatements
        ) GetTryFinallyStatements(TTryStatementSyntax tryStatement);

        protected abstract TStatementSyntax CreateUsingStatement(
            TLocalDeclarationSyntax declarationStatement,
            SyntaxList<TStatementSyntax> statementsToSurround
        );
        protected abstract bool TryCreateUsingLocalDeclaration(
            ParseOptions options,
            TLocalDeclarationSyntax declarationStatement,
            [NotNullWhen(true)] out TLocalDeclarationSyntax? usingDeclarationStatement
        );
        ...
        private async Task<(
            TLocalDeclarationSyntax? declaration,
            string? variableName
        )> FindDisposableLocalDeclarationAsync(
            Document document,
            TextSpan selection,
            CancellationToken cancellationToken
        )
        {
            var declarationSyntax = await document
                .TryGetRelevantNodeAsync<TLocalDeclarationSyntax>(selection, cancellationToken)
                .ConfigureAwait(false);
            if (
                declarationSyntax is null
                || !CanRefactorToContainBlockStatements(declarationSyntax.GetRequiredParent())
            )
                return default;
            ...
            var operation =
                semanticModel.GetOperation(declarationSyntax, cancellationToken)
                as IVariableDeclarationGroupOperation;
        ...
        /// </summary>
        private static bool IsLegalUsingStatementType(
        ...
        )
        {
                ...
                var newParent = WithStatements(
                    ...
                    new SyntaxList<TStatementSyntax>(
                        surroundingStatements
                            ...
                            .Concat(surroundingStatements.Skip(declarationStatementIndex + 2))
                    )
                ); // +2 to skip the decl statement and the try statement
            ...
            else
            {
                var statementsToSurround = GetStatementsToSurround(
                    ...
                    semanticModel,
                    syntaxFacts,
                    out var consumedLastSurroundingStatement,
                    cancellationToken
                );
                ...
                if (
                    ...
                    && this.TryCreateUsingLocalDeclaration(
                        root.SyntaxTree.Options,
                        declarationStatement,
                        out var usingDeclarationStatement
                    )
                )
                {
                    return document.WithSyntaxRoot(
                        ...
                        var newParent = WithStatements(
                            ...
                            new SyntaxList<TStatementSyntax>(
                                surroundingStatements
                                    ...
                                    .Concat(usingStatement)
                                    .Concat(
                                        surroundingStatements.Skip(
                                            declarationStatementIndex
                                                + 1
                                                + statementsToSurround.Count
                                        )
                                    )
                            )
                        );
        ...
        )
        {
            ...
            if (
                syntaxFacts.GetIdentifierOfSimpleName(name).ValueText != nameof(IDisposable.Dispose)
            )
                return false;
            ...
            if (
                syntaxFacts.GetIdentifierOfIdentifierName(accessedExpression).ValueText
                != variableName
            )
                return false;
        ...
        )
        {
            ...
            var declarationStatementIndex = surroundingStatements.IndexOf(declarationStatement);
            var lastUsageStatementIndex = surroundingStatements.IndexOf(
                lastUsageStatement,
                declarationStatementIndex + 1
            );

            return new SyntaxList<TStatementSyntax>(
                surroundingStatements
        ...
        )
        {
            ...
            var statementsFromDeclarationToEnd = declarationSyntax
                .GetRequiredParent()
                .ChildNodesAndTokens()
            ...
            )
            {
                ...
                var declaredVariables = semanticModel.GetAllDeclaredSymbols(
                    currentStatement,
                    cancellationToken
                );
            ...
            foreach (var localSymbol in localVariables)
            {
                ...
                // usage.
                endOfUsingStatementIndex = Math.Max(
                    endOfUsingStatementIndex,
                    lastVariableUsageIndex[localSymbol]
                );
            }
