    ...
    >
    {
        private partial class State
        {
            ...
            )
            {
                ...
                if (
                    !await state
                        .TryInitializeAsync(interfaceNode, cancellationToken)
            ...
            )
            {
                ...
                var existingMembers = TypeToGenerateIn
            ...
            {
                if (
                    !_service.TryInitializeExplicitInterfaceState(
                        _document,
                        propertyDeclaration,
                        cancellationToken,
            ...
            {
                if (
                    !_service.TryInitializeIdentifierNameState(
                        _document,
                        simpleName,
                        cancellationToken,
                        out var identifierToken,
                ...
                IsInInContext = semanticFacts.IsInInContext(
                    semanticModel,
                    SimpleNameOrMemberAccessExpressionOpt,
                    cancellationToken
                );
                IsInOutContext = semanticFacts.IsInOutContext(
                    semanticModel,
                    SimpleNameOrMemberAccessExpressionOpt,
                    cancellationToken
                );
                ...
                IsOnlyWrittenTo = semanticFacts.IsOnlyWrittenTo(
                    semanticModel,
                    SimpleNameOrMemberAccessExpressionOpt,
                    cancellationToken
                );
                ...
                ContainingMethod = FindContainingMethodSymbol(
                    IdentifierToken.SpanStart,
                    semanticModel,
                    cancellationToken
                );
            ...
            )
            {
                ...
                {
                    var assignmentStatement = simpleName
                        .Ancestors()
                        .FirstOrDefault(syntaxFacts.IsSimpleAssignmentStatement);
                    if (assignmentStatement != null)
                    {
                        ...
                        if (left == simpleName)
                        {
                            ...
                            var previousAssignedSymbol = TryGetAssignedSymbol(
                                symbolKind,
                                children,
                                statementindex - 1,
                                cancellationToken
                            );
                            var nextAssignedSymbol = TryGetAssignedSymbol(
                                symbolKind,
                                children,
                                statementindex + 1,
                                cancellationToken
                            ...
                            if (symbolKind == SymbolKind.Field)
                            {
                                OfferReadOnlyFieldFirst =
                                    FieldIsReadOnly(previousAssignedSymbol)
            ...
            private static IMethodSymbol FindContainingMethodSymbol(
                int position,
            ...
            private bool TryDetermineFieldType(CancellationToken cancellationToken)
            {
                ...
                var mapping = capturedMethodTypeParameters.ToDictionary(
                    tp => tp,
