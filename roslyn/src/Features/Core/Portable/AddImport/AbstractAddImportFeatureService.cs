...
namespace Microsoft.CodeAnalysis.AddImport
{
        ...
        where TSimpleNameSyntax : SyntaxNode
    {
        ...
        protected abstract ISet<INamespaceSymbol> GetImportNamespacesInScope(
                ...
                using (Logger.LogBlock(FunctionId.Refactoring_AddImport, cancellationToken))
                {
                    if (!cancellationToken.IsCancellationRequested)
                    {
                        if (
                            CanAddImport(
                                node,
                                options.CleanupOptions.AddImportOptions.AllowInHiddenRegions,
                                cancellationToken
                            )
                        )
                        {
        ...
        )
        {
            ...
            var projectToAssembly = new ConcurrentDictionary<Project, AsyncLazy<IAssemblySymbol>>(
                concurrencyLevel: 2,
                capacity: project.Solution.ProjectIds.Count
            );
            var referenceToCompilation = new ConcurrentDictionary<
                PortableExecutableReference,
                Compilation
            >(
                concurrencyLevel: 2,
                capacity: project.Solution.Projects.Sum(p => p.MetadataReferences.Count)
            );
        ...
        )
        {
            AddRange(
                allSymbolReferences,
                await finder
                    .FindInAllSymbolsInStartingProjectAsync(exact, cancellationToken)
                    .ConfigureAwait(false)
            );
        }
            ...
            foreach (var unreferencedProject in viableUnreferencedProjects)
            {
                ...
                findTasks.Add(
                    ProcessReferencesAsync(
                        ...
                        maxResults,
                        linkedTokenSource,
                        finder.FindInSourceSymbolsInProjectAsync(
                            projectToAssembly,
                            unreferencedProject,
                            exact,
                ...
                if (compilation.GetAssemblyOrModuleSymbol(reference) is IAssemblySymbol assembly)
                {
                    findTasks.Add(
                        ProcessReferencesAsync(
                            ...
                            maxResults,
                            linkedTokenSource,
                            finder.FindInMetadataSymbolsAsync(
                                assembly,
                                referenceProject,
                                reference,
                                exact,
        ...
        private static ImmutableArray<(
            Project,
            PortableExecutableReference
        )> GetUnreferencedMetadataReferences(
            Project project,
        ...
        )
        {
            var compilationService =
                project.Services.GetRequiredService<ICompilationFactoryService>();
        ...
        private static HashSet<Project> GetViableUnreferencedProjects(Project project)
        {
            ...
            var projectsThatTransitivelyDependOnThisProject =
                dependencyGraph.GetProjectsThatTransitivelyDependOnThisProject(project.Id);

            viableProjects.RemoveAll(
                projectsThatTransitivelyDependOnThisProject.Select(solution.GetRequiredProject)
            );

            // We also aren't interested in any projects we're already directly referencing.
            viableProjects.RemoveAll(
                project.ProjectReferences.Select(r => solution.GetRequiredProject(r.ProjectId))
            );
        ...
        private static void AddRange(
            ConcurrentQueue<Reference> allSymbolReferences,
            ImmutableArray<SymbolReference> proposedReferences
        )
        {
        ...
        public async Task<
            ImmutableArray<(Diagnostic Diagnostic, ImmutableArray<AddImportFixData> Fixes)>
        ...
        )
        {
            ...
            using var _ = ArrayBuilder<(Diagnostic, ImmutableArray<AddImportFixData>)>.GetInstance(
                out var result
            );
        ...
        private async Task<
            ImmutableArray<AddImportFixData>
        > GetUniqueFixesAsyncInCurrentProcessAsync(
            Document document,
            TextSpan span,
            ImmutableArray<string> diagnosticIds,
            ISymbolSearchService symbolSearchService,
        ...
        )
        {
            ...
            var getFixesForDiagnosticsTasks = diagnostics
                ...
                .Select(diagnosticsForSourceSpan =>
                    GetFixesForDiagnosticsAsync(
                        document,
        ...
        private static CodeAction? TryCreateCodeAction(
            Document document,
            ...
            fixData.Kind switch
            {
                AddImportFixKind.ProjectSymbol => new ProjectSymbolReferenceCodeAction(
                    document,
                    fixData
                ),
                AddImportFixKind.MetadataSymbol => new MetadataSymbolReferenceCodeAction(
                    document,
                    fixData
                ),
                AddImportFixKind.ReferenceAssemblySymbol => new AssemblyReferenceCodeAction(
                    document,
                    fixData
                ),
                AddImportFixKind.PackageSymbol =>
                    ParentInstallPackageCodeAction.TryCreateCodeAction(
                        document,
                        new InstallPackageData(
                            fixData.PackageSource,
                            fixData.PackageName,
                            fixData.PackageVersionOpt,
                            fixData.TextChanges
                        ),
                        installerService
                    ),
        ...
        private static ITypeSymbol? GetAwaitInfo(
            SemanticModel semanticModel,
            ISyntaxFacts syntaxFactsService,
            SyntaxNode node
        )
        ...
        private static ITypeSymbol? GetCollectionExpressionType(
            SemanticModel semanticModel,
            ISyntaxFacts syntaxFactsService,
            SyntaxNode node
        )
        ...
        protected static bool AncestorOrSelfIsAwaitExpression(
            ISyntaxFacts syntaxFactsService,
            SyntaxNode node
        ) => FirstAwaitExpressionAncestor(syntaxFactsService, node) != null;

        private static SyntaxNode? FirstAwaitExpressionAncestor(
            ISyntaxFacts syntaxFactsService,
            SyntaxNode node
        ) =>
            node.FirstAncestorOrSelf<SyntaxNode, ISyntaxFacts>(
                (n, syntaxFactsService) => syntaxFactsService.IsAwaitExpression(n),
                syntaxFactsService
            );

        private static SyntaxNode? FirstForeachCollectionExpressionAncestor(
            ISyntaxFacts syntaxFactsService,
            SyntaxNode node
        ) =>
            node.FirstAncestorOrSelf<SyntaxNode, ISyntaxFacts>(
                (n, syntaxFactsService) => syntaxFactsService.IsExpressionOfForeach(n),
                syntaxFactsService
            );
    }
}
