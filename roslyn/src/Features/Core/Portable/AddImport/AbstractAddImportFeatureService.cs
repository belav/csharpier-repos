...
namespace Microsoft.CodeAnalysis.AddImport
{
          ...
          IEqualityComparer<PortableExecutableReference> where TSimpleNameSyntax : SyntaxNode
    {
        ...
        {
            return project.Solution.Workspace.Kind
                is WorkspaceKind.Host
                    or WorkspaceKind.RemoteWorkspace
                    or WorkspaceKind.RemoteTemporaryWorkspace;
        }
        ...
        private static HashSet<Project> GetViableUnreferencedProjects(Project project)
        {
            ...
            viableProjects.RemoveAll(
                projectsThatTransitivelyDependOnThisProject.Select(
                    id => solution.GetRequiredProject(id)
                )
            );
        ...
        )
        {
            ...
            var fixesForDiagnosticBuilder =
                ArrayBuilder<(Diagnostic, ImmutableArray<AddImportFixData>)>.GetInstance();
