    ...
    internal abstract partial class AbstractAddImportFeatureService<TSimpleNameSyntax>
    {
        private class InstallPackageAndAddImportCodeAction : AddImportCodeAction
        {
            ...
            > ComputePreviewOperationsAsync(CancellationToken cancellationToken)
            {
                ...
                result.AddRange(
                    await solutionChangeAction.GetPreviewOperationsAsync(cancellationToken)
            ...
            )
            {
                ...
                var oldText = await OriginalDocument.GetTextAsync(cancellationToken)
                    .ConfigureAwait(false);
                var newText = await updatedDocument.GetTextAsync(cancellationToken)
                    .ConfigureAwait(false);
        ...
        private class InstallPackageAndAddImportOperation : CodeActionOperation
        {
            ...
            )
            {
                ...
                {
                    if (
                        _installPackageOperation.TryApply(
                            workspace,
                            progressTracker,
                            cancellationToken
                        )
                    ...
                    var rolledBackSolution = workspace.CurrentSolution
