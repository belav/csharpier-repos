            ...
            IDocumentHighlightsService
    {
        ...
        )
        {
            var semanticModel = await document
                ...
                .ConfigureAwait(false);
            var result = TryGetEmbeddedLanguageHighlights(
                document,
                semanticModel,
                position,
                options,
        ...
        )
        {
            ...
            var embeddedHighlightsServices = this.GetServices(
                semanticModel,
                token,
                cancellationToken
            );
        ...
        )
        {
            ...
            if (ShouldConsiderSymbol(symbol))
            {
                ...
                var options = FindReferencesSearchOptions.GetFeatureOptionsForStartingSymbol(
                    symbol
                ) with
                {
                    Explicit = false,
        ...
        )
        {
            ...
            if (symbol.IsConstructor())
            {
                references = references.WhereAsArray(r =>
                    r.Definition.OriginalDefinition.Equals(symbol.OriginalDefinition)
                );
            }
        ...
        )
        {
                            ...
                            if (document == null)
                            {
                                Debug.Assert(
                                    solution.WorkspaceKind
                                        is WorkspaceKind.Interactive
                                            or WorkspaceKind.MiscellaneousFiles
                                );
                                continue;
                            }
            ...
            {
                using var _2 = ArrayBuilder<HighlightSpan>.GetInstance(
                    kvp.Value.Count,
                    out var spans
                );
