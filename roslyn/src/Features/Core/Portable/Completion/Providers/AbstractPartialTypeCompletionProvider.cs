...
{
    partial internal abstract class AbstractPartialTypeCompletionProvider<TSyntaxContext>
        ...
        where TSyntaxContext : SyntaxContext
    {
            ...
            try
            {
                ...
                if (node != null)
                {
                    var semanticModel = await document
                        .ReuseExistingSpeculativeModelAsync(node, cancellationToken)
                        .ConfigureAwait(false);
                    if (
        ...
        {
            var (displayText, suffix, insertionText) = GetDisplayAndSuffixAndInsertionText(
                symbol,
                context
            );
        ...
        protected abstract SyntaxNode? GetPartialTypeSyntaxNode(
            SyntaxTree tree,
            int position,
        ...
        ) GetDisplayAndSuffixAndInsertionText(INamedTypeSymbol symbol, TSyntaxContext context);

        protected virtual IEnumerable<INamedTypeSymbol>? LookupCandidateSymbols(
            TSyntaxContext context,
        ...
        )
        {
            ...
            return semanticModel
                .LookupNamespacesAndTypes(context.Position, containingSymbol)
                .OfType<INamedTypeSymbol>()
                .Where(
                    symbol =>
                        declaredSymbol.TypeKind == symbol.TypeKind
        ...
        private static bool InSameProject(INamedTypeSymbol symbol, Compilation compilation) =>
            symbol.DeclaringSyntaxReferences.Any(
                static (r, compilation) => compilation.SyntaxTrees.Contains(r.SyntaxTree),
                compilation
            );
        ...
        private static bool NotNewDeclaredMember(INamedTypeSymbol symbol, TSyntaxContext context)
        {
            return symbol.DeclaringSyntaxReferences
                ...
                .Any(
                    node =>
                        !(
                            node.SyntaxTree == context.SyntaxTree
