...
namespace Microsoft.CodeAnalysis.Completion.Providers
{
            ...
            INotifyCommittingItemCompletionProvider
    {
        ...
        public Task NotifyCommittingItemAsync(
            Document document,
            CompletionItem item,
            char? commitKey,
            CancellationToken cancellationToken
        ...
        public override async Task ProvideCompletionsAsync(CompletionContext completionContext)
        {
            ...
            if (!ShouldProvideCompletion(completionContext, syntaxContext))
            {
                ...
                if (
                    options.UpdateImportCompletionCacheInBackground
                    && !options.ForceExpandedCompletionIndexCreation
                )
        ...
        private static async Task<SyntaxContext> CreateContextAsync(
            Document document,
            int position,
            CancellationToken cancellationToken
        )
        ...
        private static HashSet<string> GetNamespacesInScope(
            SyntaxContext syntaxContext,
            CancellationToken cancellationToken
        )
        {
        ...
        private static ImmutableArray<string> GetImportedNamespaces(
        ...
        )
        {
            ...
            foreach (var scope in scopes)
            {
                foreach (var import in scope.Imports)
                {
                    ...
                    {
                        usingsBuilder.Add(
                            @namespace.ToDisplayString(SymbolDisplayFormats.NameFormat)
                        );
                    }
                }
            }
        ...
        private async Task<bool> IsInImportsDirectiveAsync(
            Document document,
            int position,
            CancellationToken cancellationToken
        )
        ...
        )
        {
            ...
            return completionOptions?.CanAddImportStatement != false
                && document
                    .Project.Solution.Services.GetRequiredService<IDocumentSupportsFeatureService>()
                    .SupportsRefactorings(document);
        }
        ...
        private static SyntaxNode CreateImport(Document document, string namespaceName)
        {
            ...
            return syntaxGenerator
                .NamespaceImportDeclaration(namespaceName)
        ...
        internal override Task<CompletionDescription> GetDescriptionWorkerAsync(
            ...
            ImportCompletionItem.GetCompletionDescriptionAsync(
                document,
                item,
                displayOptions,
                cancellationToken
