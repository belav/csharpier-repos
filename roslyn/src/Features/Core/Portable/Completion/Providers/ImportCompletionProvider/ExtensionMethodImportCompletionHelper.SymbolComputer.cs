...
namespace Microsoft.CodeAnalysis.Completion.Providers
{
    internal static partial class ExtensionMethodImportCompletionHelper
    {
        private class SymbolComputer
        {
            ...
            public static async ValueTask UpdateCacheAsync(
                Project project,
                CancellationToken cancellationToken
            )
            {
            ...
            )
            {
                try
                {
                    ...
                    foreach (
                        var peReference in GetAllRelevantPeReferences(_originatingDocument.Project)
                    )
                    {
                        tasks.Add(
                            Task.Run(
                ...
                finally
                {
                    ...
                    if (!forceCacheCreation)
                        GetCacheService(_originatingDocument.Project)
                            .WorkQueue.AddWork(_originatingDocument.Project);
                }
            }
            ...
            private static ImmutableArray<PortableExecutableReference> GetAllRelevantPeReferences(
                Project project
            ) =>
            ...
            )
            {
                ...
                if (forceCacheCreation)
                {
                    cacheEntry = await GetUpToDateCacheEntryAsync(
                            project,
                            _cacheService,
                            cancellationToken
                        )
            ...
            )
            {
                ...
                if (forceCacheCreation)
                {
                    symbolInfo = await SymbolTreeInfo
                        .GetInfoForMetadataReferenceAsync(
                            _originatingDocument.Project.Solution,
                            peReference,
                            checksum: null,
                            cancellationToken
                        )
                ...
                {
                    var cachedInfoTask =
                        SymbolTreeInfo.TryGetCachedInfoForMetadataReferenceIgnoreChecksumAsync(
                            peReference,
                            cancellationToken
                        );
                ...
                foreach (var (declaredReceiverType, methodSymbols) in matchingMethodSymbols)
                {
                    ...
                    foreach (
                        var methodInOriginatingCompilation in methodSymbols
                            .Select(s =>
                                SymbolFinder
                                    .FindSimilarSymbols(s, _originatingSemanticModel.Compilation)
                                    .FirstOrDefault()
                            )
                            .WhereNotNull()
                    )
            ...
            )
            {
                ...
                foreach (var (fullyQualifiedContainerName, methodInfo) in extensionMethodFilter)
                {
                    ...
                    var qualifiedNamespaceName =
                        indexOfLastDot > 0
                            ? fullyQualifiedContainerName[..indexOfLastDot]
                            : string.Empty;
            ...
            private MultiDictionary<
                string,
                (string methodName, string receiverTypeName)
            > CreateAggregatedFilter(ExtensionMethodImportCompletionCacheEntry syntaxIndex)
            {
