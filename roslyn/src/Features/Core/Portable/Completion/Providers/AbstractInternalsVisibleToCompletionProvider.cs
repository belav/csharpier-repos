...
namespace Microsoft.CodeAnalysis.Completion.Providers
{
    internal abstract class AbstractInternalsVisibleToCompletionProvider : LSPCompletionProvider
    {
        ...
        protected abstract bool ShouldTriggerAfterQuotes(
            SourceText text,
            int insertedCharacterPosition
        );
        ...
        public override async Task ProvideCompletionsAsync(CompletionContext context)
        {
            try
            {
                ...
                )
                {
                    ...
                    var attributeSyntaxNode = GetAttributeSyntaxNodeOfToken(
                        syntaxFactsService,
                        token
                    );
        ...
        private static async Task<bool> CheckTypeInfoOfAttributeAsync(
            Document document,
        ...
        )
        {
            var semanticModel = await document
                .ReuseExistingSpeculativeModelAsync(attributeNode, cancellationToken)
                .ConfigureAwait(false);
            ...
            foreach (var project in context.Document.Project.Solution.Projects)
            {
                ...
                var completionItem = CommonCompletionItem.Create(
                    ...
                    properties: ImmutableArray.Create(
                        new KeyValuePair<string, string>(ProjectGuidKey, projectGuid)
                    )
                );
            ...
            foreach (var document in project.Documents)
            {
                ...
                foreach (var attribute in assemblyScopedAttributes)
                {
                    ...
                    )
                    {
                        ...
                        {
                            resultBuilder ??= ImmutableHashSet.CreateBuilder<string>(
        ...
        )
        {
            ...
            var publicKey = await GetPublicKeyOfProjectAsync(project, cancellationToken)
                .ConfigureAwait(false);
