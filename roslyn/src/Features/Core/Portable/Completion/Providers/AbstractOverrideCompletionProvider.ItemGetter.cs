    ...
    internal abstract partial class AbstractOverrideCompletionProvider
    {
        private partial class ItemGetter
        {
            ...
            public async Task<ImmutableArray<CompletionItem>> GetItemsAsync()
            {
                ...
                var startToken = _provider.FindStartingToken(
                    _syntaxTree,
                    _position,
                    _cancellationToken
                );
                ...
                var semanticModel = await _document
                    .ReuseExistingSpeculativeModelAsync(startToken.Parent, _cancellationToken)
                    .ConfigureAwait(false);
                if (
                    !_provider.TryDetermineReturnType(
                        ...
                        _cancellationToken,
                        out var returnType,
                        out var tokenAfterReturnType
                    )
                    || !_provider.TryDetermineModifiers(
                        tokenAfterReturnType,
                        _text,
                        _startLineNumber,
                        out var seenAccessibility,
                        out var modifiers
                    )
                    || !TryDetermineOverridableMembers(
                        semanticModel,
                        startToken,
                        seenAccessibility,
            ...
            )
            {
                ...
                if (seenAccessibility != Accessibility.NotApplicable)
                    result = result.WhereAsArray(m =>
                        MatchesAccessibility(m.DeclaredAccessibility, seenAccessibility)
                    );

                overridableMembers = result;
                return overridableMembers.Length > 0;

                static bool MatchesAccessibility(
                ...
                )
                {
                    ...
                    return seenAccessibility switch
                    {
                        ...
                        Accessibility.Private => declaredAccessibility
                            is Accessibility.Private
                                or Accessibility.ProtectedAndInternal,
                        // `protected`, `private protected`, `protected internal`
                        Accessibility.Protected => declaredAccessibility
                            is Accessibility.Protected
                                or Accessibility.ProtectedAndInternal
                                or Accessibility.ProtectedOrInternal,
                        // `internal`, `protected internal`
                        Accessibility.Internal => declaredAccessibility
