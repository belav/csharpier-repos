...
namespace Microsoft.CodeAnalysis.Completion.Providers
{
        ...
        : LSPCompletionProvider
    {
        ...
        )
        {
            ...
            var globalOptions =
                document.Project.Solution.Workspace.Services.GetService<ILegacyGlobalOptionsWorkspaceService>();
            var fallbackOptions =
        ...
        )
        {
            ...
            var annotatedRoot = tree.GetRoot(cancellationToken)
                .ReplaceToken(token, token.WithAdditionalAnnotations(_otherAnnotation));
            ...
            var memberContainingDocumentCleanupOptions = await document
            ...
            return await Formatter
                .FormatAsync(document, _annotation, formattingOptions, cancellationToken)
                .ConfigureAwait(false);
        }
        ...
        )
        {
            ...
            var symbols = await SymbolCompletionItem
                .GetSymbolsAsync(completionItem, document, cancellationToken)
                .ConfigureAwait(false);
            ...
            {
                memberContainingDocument = await codeGenService
                    .AddMethodAsync(
                        context,
                        containingType,
        ...
        {
            memberContainingDocument = await Simplifier
                .ReduceAsync(
                    memberContainingDocument,
                ...
                .ConfigureAwait(false);
            memberContainingDocument = await Formatter
                .FormatAsync(
                    memberContainingDocument,
        ...
        )
        {
            ...
            dismemberedDocument = await Formatter
                .FormatAsync(
                    dismemberedDocument,
                    Formatter.Annotation,
                    cleanupOptions.FormattingOptions,
        ...
        private static readonly ImmutableArray<CharacterSetModificationRule> s_filterRules =
            ImmutableArray.Create(
        ...
        protected static CompletionItemRules GetRules() => s_defaultRules;
        ...
        internal override Task<CompletionDescription> GetDescriptionWorkerAsync(
            ...
            MemberInsertionCompletionItem.GetDescriptionAsync(
                item,
                document,
                displayOptions,
                cancellationToken
