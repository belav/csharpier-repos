    ...
    internal static class CompletionProvidersLogger
    {
        ...
        internal static void LogTypeImportCompletionTicksDataPoint(int count)
        {
            s_histogramLogAggregator.IncreaseCount(
                (int)ActionInfo.TypeImportCompletionTicks,
                count
            );
        ...
        internal static void LogTypeImportCompletionItemCountDataPoint(int count) =>
            s_statisticLogAggregator.AddDataPoint(
                (int)ActionInfo.TypeImportCompletionItemCount,
                count
            );

        internal static void LogTypeImportCompletionReferenceCountDataPoint(int count) =>
            s_statisticLogAggregator.AddDataPoint(
                (int)ActionInfo.TypeImportCompletionReferenceCount,
                count
            );
        ...
        internal static void LogTargetTypeCompletionTicksDataPoint(int count)
        {
            ...
            s_histogramLogAggregator.IncreaseCount(
                (int)ActionInfo.TargetTypeCompletionTicks,
                count
            );
        }

        internal static void LogExtensionMethodCompletionTicksDataPoint(
            int total,
            int getSymbols,
            int createItems,
            bool isRemote
        ...
        {
            s_histogramLogAggregator.IncreaseCount(
                (int)ActionInfo.ExtensionMethodCompletionTicks,
                total
            );
            s_statisticLogAggregator.AddDataPoint(
                (int)ActionInfo.ExtensionMethodCompletionTicks,
            ...
            {
                s_statisticLogAggregator.AddDataPoint(
                    (int)ActionInfo.ExtensionMethodCompletionRemoteTicks,
                    (total - getSymbols - createItems)
                );
            }

            s_statisticLogAggregator.AddDataPoint(
                (int)ActionInfo.ExtensionMethodCompletionGetSymbolsTicks,
                getSymbols
            );
            s_statisticLogAggregator.AddDataPoint(
                (int)ActionInfo.ExtensionMethodCompletionCreateItemsTicks,
                createItems
            );
        }
        ...
        internal static void LogExtensionMethodCompletionMethodsProvidedDataPoint(int count) =>
            s_statisticLogAggregator.AddDataPoint(
                (int)ActionInfo.ExtensionMethodCompletionMethodsProvided,
                count
            );

        internal static void LogCommitOfExtensionMethodImportCompletionItem() =>
            s_logAggregator.IncreaseCount(
                (int)ActionInfo.CommitsOfExtensionMethodImportCompletionItem
            );

        internal static void LogExtensionMethodCompletionPartialResultCount() =>
            s_logAggregator.IncreaseCount(
                (int)ActionInfo.ExtensionMethodCompletionPartialResultCount
            );
