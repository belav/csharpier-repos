    ...
    public abstract partial class CompletionService : ILanguageService
    {
        ...
        )
        {
            ...
            if (
                ...
                && SupportsTriggerOnDeletion(options)
            )
            {
        ...
        )
        {
            ...
            var description = await extensionManager
                .PerformFunctionAsync(
                    provider,
                    () =>
                        provider.GetDescriptionAsync(
                            document,
                            item,
                            options,
                            displayOptions,
        ...
        )
        {
            ...
            if (provider != null)
            {
                ...
                var change = await extensionManager
                    .PerformFunctionAsync(
                        provider,
                        () =>
                            provider.GetChangeAsync(
                                document,
                                item,
                                commitCharacter,
                                cancellationToken
        ...
        private static readonly ObjectPool<List<MatchResult>> s_listOfMatchResultPool = new(
            factory: () => new()
        );
...
#pragma warning restore RS0030 // Do not used banned APIs
        ...
        /// </summary>
        internal void LoadImportedProviders() => _providerManager.LoadProviders();
