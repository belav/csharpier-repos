...
namespace Microsoft.CodeAnalysis.NavigateTo
{
    using CachedIndexMap = ConcurrentDictionary<
        (
            IChecksummedPersistentStorageService service,
    ...
    internal abstract partial class AbstractNavigateToSearchService
    {
        ...
        )
        {
            ...
            using var _2 = GetPooledHashSet(
                ...
                out var highPriorityGroups
            );
            using var _3 = GetPooledHashSet(
                groups.Where(g => !highPriorityGroups.Contains(g)),
                out var lowPriorityGroups
            );
            ...
            async Task ProcessProjectGroupAsync(IGrouping<ProjectKey, DocumentKey> group)
            {
                ...
                using var _1 = GetPooledHashSet(
                    group.Where(priorityDocumentKeysSet.Contains),
                    out var highPriDocs
                );
                using var _2 = GetPooledHashSet(
                    group.Where(d => !highPriDocs.Contains(d)),
                    out var lowPriDocs
                );
