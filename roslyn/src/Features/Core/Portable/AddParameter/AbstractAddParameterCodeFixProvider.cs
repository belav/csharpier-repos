...
namespace Microsoft.CodeAnalysis.AddParameter
{
        ...
        where TObjectCreationExpressionSyntax : SyntaxNode
    {
        ...
        protected abstract ITypeSymbol GetArgumentType(
            ...
            if (node is TInvocationExpressionSyntax invocationExpression)
            {
                ...
                var arguments =
                    (SeparatedSyntaxList<TArgumentSyntax>)
                        syntaxFacts.GetArgumentsOfInvocationExpression(invocationExpression);
        ...
        )
        {
            if (node is TObjectCreationExpressionSyntax objectCreation)
            {
                ...
                var symbol = semanticModel
                    .GetSymbolInfo(typeNode, cancellationToken)
                    .GetAnySymbol();
                ...
                var arguments =
                    (SeparatedSyntaxList<TArgumentSyntax>)
                        syntaxFacts.GetArgumentsOfObjectCreationExpression(objectCreation);
        ...
        )
        {
            ...
            {
                using var _ = ArrayBuilder<CodeAction>.GetInstance(
                    codeFixData.Length,
                    out var builder
                );
        ...
        {
            using var _ = ArrayBuilder<CodeFixData>.GetInstance(
        ...
        private async Task<(ITypeSymbol, RefKind)> GetArgumentTypeAndRefKindAsync(
            Document invocationDocument,
            TArgumentSyntax argument,
            CancellationToken cancellationToken
        )
        ...
        )
        {
            ...
            else
            {
                ...
                var semanticFacts =
                    invocationDocument.GetRequiredLanguageService<ISemanticFactsService>();
                argumentName = semanticFacts.GenerateNameForExpression(
                    semanticModel,
                    expression,
                    capitalize: containingType.IsRecord,
                    cancellationToken: cancellationToken
                );
