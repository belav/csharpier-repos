        ...
        ) : CodeAction
        {
            ...
            )
            {
                ...
                var codeGenerationService =
                    languageServices.GetRequiredService<ICodeGenerationService>();
                ...
                var useThrowExpressions = await _service
                    .PrefersThrowExpressionAsync(_document, _fallbackOptions, cancellationToken)
                    .ConfigureAwait(false);

                for (
                    var i = _state.DelegatedConstructor.Parameters.Length;
                    i < _state.Parameters.Length;
                    i++
                )
                {
                ...
                var result = await codeGenerationService
                    .AddMethodAsync(
