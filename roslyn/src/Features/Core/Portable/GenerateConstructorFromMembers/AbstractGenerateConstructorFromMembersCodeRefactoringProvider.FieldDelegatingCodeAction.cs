    ...
    internal abstract partial class AbstractGenerateConstructorFromMembersCodeRefactoringProvider
    {
        ...
        ) : CodeAction
        {
            ...
            )
            {
                ...
                var parameterToExistingFieldMap = ImmutableDictionary.CreateBuilder<
                    string,
                    ISymbol
                >();
                for (var i = 0; i < _state.Parameters.Length; i++)
                    parameterToExistingFieldMap[_state.Parameters[i].Name] = _state.SelectedMembers[
                        i
                    ];
                ...
                var preferThrowExpression = await _service
