...
namespace Microsoft.CodeAnalysis.ImplementInterface
{
    internal abstract partial class AbstractImplementInterfaceService
    {
        ...
        private static ImmutableArray<string> s_disposedValueNameParts = ImmutableArray.Create(
            "disposed",
            "value"
        );
        ...
        private static IMethodSymbol? TryGetIDisposableDispose(Compilation compilation)
        {
            ...
            if (idisposable?.TypeKind == TypeKind.Interface)
            {
                ...
                {
                    if (
                        member is IMethodSymbol disposeMethod
        ...
        private static bool ShouldImplementDisposePattern(State state, bool explicitly)
        {
            ...
            return state.ClassOrStructType.FindImplementationForInterfaceMember(disposeMethod)
                == null;
        }
            ...
            : ImplementInterfaceCodeAction
        {
            ...
            )
            {
                ...
                var firstGeneratedMember = rootWithCoreMembers
                    .GetAnnotatedNodes(CodeGenerator.Annotation)
                    .First();
                ...
                var options = await document
                    .GetCodeGenerationOptionsAsync(Options.FallbackOptions, cancellationToken)
                    .ConfigureAwait(false);
            ...
            )
            {
                ...
                var lastMemberWithComments = lastGeneratedMember.WithPrependedLeadingTrivia(
                    finalizerComments
                        .Insert(0, generator.CarriageReturnLineFeed)
            ...
            )
            {
                ...
                return (
                    ImmutableArray.Create<ISymbol>(disposeImplMethod, disposeInterfaceMethod),
                    finalizer
                );
            }
            ...
            )
            {
                ...
                // }
                var ifDisposingStatement = g.IfStatement(
                    g.IdentifierName(DisposingName),
                    Array.Empty<SyntaxNode>()
                );
                ifDisposingStatement = Service
                ...
                // disposedValue = true
                var disposedValueEqualsTrueStatement = AddComments(
                    g,
                    ...
                    g.AssignmentStatement(
                        g.IdentifierName(disposedValueField.Name),
            ...
            )
            {
                ...
                var explicitInterfaceImplementations =
                    Explicitly || !Service.CanImplementImplicitly
                        ? ImmutableArray.Create(disposeMethod)
                        : default;
            ...
            )
            {
                ...
                if (
                    ...
                    >(CodeStyleOptions2.AccessibilityModifiersRequired, out var value)
                )
                {
                    return value.Value;
                }
                ...
                var fallbackFormattingOptions = await (
                    (OptionsProvider<SyntaxFormattingOptions>)Options.FallbackOptions
                )
            ...
            )
            {
                ...
                var requireAccessiblity = await GetAccessibilityModifiersRequiredAsync(
                        document,
                        cancellationToken
                    )
                    .ConfigureAwait(false);
