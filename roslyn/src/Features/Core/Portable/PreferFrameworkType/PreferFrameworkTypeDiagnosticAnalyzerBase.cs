...
namespace Microsoft.CodeAnalysis.PreferFrameworkType
{
        ...
        where TPredefinedTypeSyntax : TExpressionSyntax
    {
        ...
        protected void AnalyzeNode(SyntaxNodeAnalysisContext context)
        {
            ...
            if (
                semanticModel.GetSymbolInfo(predefinedTypeNode, context.CancellationToken).Symbol
                is not ITypeSymbol
            )
            {
                return;
        ...
        )
        {
            ...
            var option = isMemberAccessOrCref
                ? GetOptionForMemberAccessContext
                : GetOptionForDeclarationContext;
        ...
        private static bool OptionSettingPrefersFrameworkType(
            CodeStyleOption2<bool> optionValue,
