...
namespace Microsoft.CodeAnalysis.ReplaceMethodWithProperty
{
    [
        ExportCodeRefactoringProvider(
            ...
            Name = PredefinedCodeRefactoringProviderNames.ReplaceMethodWithProperty
        ),
        Shared
    ]
    internal class ReplaceMethodWithPropertyCodeRefactoringProvider : CodeRefactoringProvider
    {
        ...
        public override async Task ComputeRefactoringsAsync(CodeRefactoringContext context)
        {
            ...
            var methodDeclaration = await service
                .GetMethodDeclarationAsync(context)
        ...
        private static bool HasPrefix(string text, string prefix) =>
            ...
            && !char.IsLower(text[prefix.Length]);
        ...
        )
        {
            ...
            var getMethodReferences = await SymbolFinder
                .FindReferencesAsync(getMethod, originalSolution, cancellationToken)
                .ConfigureAwait(false);
            ...
            var getReferencesByDocument = getMethodReferences
                .SelectMany(r => r.Locations)
                .ToLookup(loc => loc.Document);
            var setReferencesByDocument = setMethodReferences
                .SelectMany(r => r.Locations)
                .ToLookup(loc => loc.Document);
        ...
        {
            var allReferenceDocuments = getReferencesByDocument
                .Concat(setReferencesByDocument)
                .Select(g => g.Key)
                .Distinct();
        ...
        )
        {
            ...
            var service =
                originalDocument.GetRequiredLanguageService<IReplaceMethodWithPropertyService>();
        ...
        )
        {
            if (getReferences != null)
            {
                ...
                foreach (
                    var referenceLocation in getReferences.Distinct(
        ...
        )
        {
            if (setReferences != null)
            {
                ...
                // Note Use DistinctBy (.Net6) once available.
                foreach (
                    var referenceLocation in setReferences.Distinct(
                        LinkedFileReferenceLocationEqualityComparer.Instance
                    )
                )
                {
        ...
        )
        {
            ...
            var documentIds = getDefinitionsByDocumentId
                .Keys.Concat(setDefinitionsByDocumentId.Keys)
                .Distinct();
        ...
        )
        {
            ...
            var semanticModel = await updatedDocument
                .GetRequiredSemanticModelAsync(cancellationToken)
                .ConfigureAwait(false);
            var syntaxTree = await updatedDocument
            ...
            foreach (var originalSetMethod in originalSetDefinitions)
            {
                ...
                var setMethod = GetSymbolInCurrentCompilation(
                    compilation,
                    originalSetMethod,
                    cancellationToken
                );
                ...
                var setMethodDocument = updatedSolution.GetDocument(
                    setMethodDeclaration?.SyntaxTree
                );
        ...
        )
        {
            ...
            foreach (var originalDefinition in originalDefinitions)
            {
                ...
                var getMethod = GetSymbolInCurrentCompilation(
                    compilation,
                    originalDefinition,
                    cancellationToken
                );
        ...
        private static async Task<
            MultiDictionary<DocumentId, IMethodSymbol>
