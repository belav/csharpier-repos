        ...
        where TExpressionSyntax : SyntaxNode
    {
        ...
        private (CodeAction? fieldAction, CodeAction? propertyAction) AddAllParameterInitializationActions(
            ...
            var properties = parameters.SelectAsArray(
                p => (ISymbol)CreateProperty(p, options, rules)
            );
        ...
        private async Task<Document> AddAllSymbolInitializationsAsync(
            ...
            for (var i = 0; i < parameters.Length; i++)
            {
                ...
                var currentParameter = (IParameterSymbol?)parameter.GetSymbolKey(cancellationToken)
