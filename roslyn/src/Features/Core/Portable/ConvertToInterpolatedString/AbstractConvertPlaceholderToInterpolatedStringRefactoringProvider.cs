...
namespace Microsoft.CodeAnalysis.ConvertToInterpolatedString
{
        ...
        where TInterpolationSyntax : SyntaxNode
    {
        ...
        private static readonly ImmutableArray<(string typeName, ImmutableArray<string> methods)> s_compositeFormattedMethods =
            ImmutableArray.Create(
                (
                    typeof(Console).FullName!,
                    ImmutableArray.Create(nameof(Console.Write), nameof(Console.WriteLine))
                ),
                (
                    typeof(Debug).FullName!,
                    ImmutableArray.Create(nameof(Debug.WriteLine), nameof(Debug.Print))
                ),
                (
                    typeof(Trace).FullName!,
                    ImmutableArray.Create(
                        nameof(Trace.TraceError),
                        nameof(Trace.TraceWarning),
                        nameof(Trace.TraceInformation)
                    )
                ),
                (
                    typeof(TraceSource).FullName!,
                    ImmutableArray.Create(nameof(TraceSource.TraceInformation))
                )
            );
        ...
        public override async Task ComputeRefactoringsAsync(CodeRefactoringContext context)
        {
            ...
            var stringInvocationMethods = CollectMethods(
                stringType,
                ImmutableArray.Create(nameof(string.Format))
            );
            var compositeFormattedInvocationMethods = s_compositeFormattedMethods
                .SelectMany(
                    pair =>
                        CollectMethods(
                            semanticModel.Compilation.GetTypeByMetadataName(pair.typeName),
                            pair.methods
                        )
                )
                .ToImmutableArray();

            var allInvocationMethods = stringInvocationMethods.AddRange(
                compositeFormattedInvocationMethods
            );
            ...
            var (invocationSyntax, invocationSymbol) = await TryFindInvocationAsync(
                    textSpan,
                    document,
                    semanticModel,
                    allInvocationMethods,
                    syntaxFactsService,
            ...
            if (
                !IsArgumentListCorrect(
                    syntaxFactsService.GetArgumentsOfInvocationExpression(invocationSyntax),
                    invocationSymbol,
                    allInvocationMethods,
                    semanticModel,
                    syntaxFactsService,
            ...
            static ImmutableArray<IMethodSymbol> CollectMethods(
        ...
        )
        {
            ...
            var possibleInvocations = await document
            ...
            var invocation = possibleInvocations.FirstOrDefault(
                invocation =>
                    IsValidPlaceholderToInterpolatedString(
                        invocation,
                        syntaxFactsService,
                        semanticModel,
                        applicableMethods,
            ...
            var argumentList = await document
                .TryGetRelevantNodeAsync<TArgumentListExpressionSyntax>(span, cancellationToken)
                .ConfigureAwait(false);
        ...
        )
        {
            ...
            )
            {
                ...
                return IsArgumentListNotPassingArrayToParams(
                    syntaxFactsService.GetExpressionOfArgument(
                        GetParamsArgument(arguments, syntaxFactsService)
                    ),
                    invocationSymbol,
                    formatMethods,
                    semanticModel,
        ...
        private static SyntaxNode GetParamsArgument(
            SeparatedSyntaxList<TArgumentSyntax> arguments,
            ISyntaxFactsService syntaxFactsService
        ) =>
        ...
        )
        {
            ...
            {
                var argumentExpression = syntaxFacts.GetExpressionOfArgument(
                    GetArgument(arguments, i, syntaxFacts)
                );
                var convertedType =
                    argumentExpression == null
                        ? null
                        : semanticModel.GetTypeInfo(argumentExpression).ConvertedType;
        ...
        )
        {
            return interpolatedString.ReplaceNodes(
                ...
                (oldNode, newNode) =>
                {
                    ...
                    {
                        if (
                            syntaxFactsService.GetExpressionOfInterpolation(interpolation)
        ...
        )
        {
            if (expression != null)
            {
                var formatMethodsAcceptingParamsArray = formatMethods.Where(
                    x =>
                        x.Parameters.Length > 1 && x.Parameters[1].Type.Kind == SymbolKind.ArrayType
                );
                if (formatMethodsAcceptingParamsArray.Contains(invocationSymbol))
                {
                    return semanticModel.GetTypeInfo(expression, cancellationToken).Type?.Kind
        ...
        {
            public ConvertToInterpolatedStringCodeAction(
