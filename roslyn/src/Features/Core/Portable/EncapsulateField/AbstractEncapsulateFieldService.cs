    ...
    internal abstract partial class AbstractEncapsulateFieldService : ILanguageService
    {
        ...
        private async Task<Solution> EncapsulateFieldsInCurrentProcessAsync(
            ...
            foreach (var field in fields)
            {
                ...
                if (
                    !(
                        field.GetSymbolKey(cancellationToken)
        ...
        private async Task<Solution> EncapsulateFieldAsync(
            ...
            foreach (var linkedDocumentId in document.GetLinkedDocumentIds())
            {
                ...
                var linkedRoot = await linkedDocument.GetSyntaxRootAsync(cancellationToken)
                    .ConfigureAwait(false);
            ...
            var newRoot = await document.GetSyntaxRootAsync(cancellationToken)
                .ConfigureAwait(false);
            var newDeclaration = newRoot.GetAnnotatedNodes<SyntaxNode>(declarationAnnotation)
                .First();
            field =
