...
namespace Microsoft.CodeAnalysis.EncapsulateField
{
    internal abstract partial class AbstractEncapsulateFieldService : ILanguageService
    {
        ...
        protected abstract Task<ImmutableArray<IFieldSymbol>> GetFieldsAsync(
            Document document,
            TextSpan span,
            CancellationToken cancellationToken
        );

        public async Task<EncapsulateFieldResult> EncapsulateFieldsInSpanAsync(
            Document document,
            TextSpan span,
        ...
        )
        {
            ...
            return new EncapsulateFieldResult(
                ...
                firstField.GetGlyph(),
                c =>
                    EncapsulateFieldsAsync(document, fields, fallbackOptions, useDefaultBehavior, c)
            );
        }

        public async Task<ImmutableArray<CodeAction>> GetEncapsulateFieldCodeActionsAsync(
            Document document,
            TextSpan span,
            CleanCodeGenerationOptionsProvider fallbackOptions,
            CancellationToken cancellationToken
        ...
        )
        {
            ...
            using (Logger.LogBlock(FunctionId.Renamer_FindRenameLocationsAsync, cancellationToken))
            {
                ...
                if (client != null)
                {
                    var fieldSymbolKeys = fields.SelectAsArray(f =>
                        SymbolKey.CreateString(f, cancellationToken)
                    );
                    ...
                    var result = await client
                        ...
                        >(
                            ...
                            callbackTarget: new RemoteOptionsProvider<CleanCodeGenerationOptions>(
                                solution.Services,
                                fallbackOptions
                            ),
                            cancellationToken
                    ...
                    return await RemoteUtilities
                        .UpdateSolutionAsync(solution, result.Value, cancellationToken)
                        .ConfigureAwait(false);
                }
            }
        ...
        private async Task<Solution> EncapsulateFieldsInCurrentProcessAsync(
            Document document,
            ImmutableArray<IFieldSymbol> fields,
            CleanCodeGenerationOptionsProvider fallbackOptions,
            bool updateReferences,
        ...
        )
        {
                ...
                fieldDeclaration
                    ...
                    .ReplaceNode(
                        ...
                        fieldDeclaration
                            ...
                            .WithAdditionalAnnotations(declarationAnnotation)
                    )
            );
            ...
            foreach (var linkedDocumentId in document.GetLinkedDocumentIds())
            {
                ...
                var updatedRoot = linkedRoot.ReplaceNode(
                    linkedFieldNode,
                    linkedFieldNode.WithAdditionalAnnotations(declarationAnnotation)
                );
            ...
            {
                var linkedDocument = solution.GetDocument(linkedDocumentId);
                var linkedDocumentFormattingOptions = await linkedDocument
                    .GetSyntaxFormattingOptionsAsync(fallbackOptions, cancellationToken)
                    .ConfigureAwait(false);
                var updatedLinkedRoot = await RewriteFieldNameAndAccessibilityAsync(
                    ...
                    .ConfigureAwait(false);
                var updatedLinkedDocument = await Formatter
                    .FormatAsync(
                        linkedDocument.WithSyntaxRoot(updatedLinkedRoot),
                        Formatter.Annotation,
                        linkedDocumentFormattingOptions,
                        cancellationToken
                    )
                    .ConfigureAwait(false);
            ...
            var newDeclaration = newRoot
                .GetAnnotatedNodes<SyntaxNode>(declarationAnnotation)
                .First();
            field =
                semanticModel.GetDeclaredSymbol(newDeclaration, cancellationToken) as IFieldSymbol;
            ...
            documentWithProperty = await Simplifier
                .ReduceAsync(documentWithProperty, simplifierOptions, cancellationToken)
                .ConfigureAwait(false);
        ...
        )
        {
            ...
            if (field.IsReadOnly)
            {
                ...
                return await RenameAsync(
                        ...
                        generatedPropertyName,
                        (documentId, span) =>
                            !IntersectsWithAny(documentId, span, constructorLocations),
                        fallbackOptions,
                        cancellationToken
                    )
                    .ConfigureAwait(false);
        ...
        )
        {
            ...
            var initialLocations = await Renamer
        ...
        private static bool IntersectsWithAny(
            ...
            TextSpan span,
            ISet<(DocumentId documentId, TextSpan span)> constructorLocations
        )
        {
            foreach (var constructor in constructorLocations)
            {
                if (constructor.documentId == documentId && span.IntersectsWith(constructor.span))
                {
                    return true;
                }
            }
        ...
        private ISet<(DocumentId documentId, TextSpan span)> GetConstructorLocations(
            Solution solution,
        ...
        internal abstract IEnumerable<SyntaxNode> GetConstructorNodes(
            INamedTypeSymbol containingType
        );
            ...
            var propertySymbol = annotation.AddAnnotationToSymbol(
                CodeGenerationSymbolFactory.CreatePropertySymbol(
                    ...
                    modifiers: new DeclarationModifiers(
                        ...
                        isUnsafe: field.RequiresUnsafeModifier()
                    ),
        ...
        protected static IMethodSymbol CreateSet(
            string originalFieldName,
            IFieldSymbol field,
        ...
        protected static IMethodSymbol CreateGet(
            string originalFieldName,
            IFieldSymbol field,
            SyntaxGenerator factory
        )
