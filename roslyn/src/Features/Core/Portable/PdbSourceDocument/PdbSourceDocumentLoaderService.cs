...
namespace Microsoft.CodeAnalysis.PdbSourceDocument
{
    ...
    ) : IPdbSourceDocumentLoaderService
    {
        ...
        public async Task<SourceFileInfo?> LoadSourceDocumentAsync(
            string tempFilePath,
        ...
        )
        {
            ...
            return TryGetEmbeddedSourceFile(tempFilePath, sourceDocument, encoding, telemetry)
                ...
                ?? await TryGetSourceLinkFileAsync(
                        sourceDocument,
                        encoding,
                        telemetry,
                        useExtendedTimeout,
        ...
        private SourceFileInfo? TryGetEmbeddedSourceFile(
            string tempFilePath,
            SourceDocument sourceDocument,
            Encoding encoding,
            TelemetryMessage telemetry
        )
        {
            ...
            if (
                ...
                && LoadSourceFile(
                    ...
                    sourceDocument,
                    encoding,
                    FeaturesResources.embedded,
                    ignoreChecksum: false,
                    fromRemoteLocation: false
                )
                    is { } existing
            )
            {
                ...
                _logger?.Log(
                    FeaturesResources._0_found_in_embedded_PDB_cached_source_file,
            ...
            var stream = new MemoryStream(
                embeddedTextBytes,
                sizeof(int),
                embeddedTextBytes.Length - sizeof(int)
            );
            ...
            if (stream is not null)
            {
                ...
                using (stream)
                {
                    ...
                    {
                        _logger?.Log(
                            FeaturesResources._0_found_in_embedded_PDB_but_could_not_write_file_1,
                            sourceDocument.FilePath,
                            ex.Message
                        );
                        return null;
                ...
                if (result is not null)
                {
                    telemetry.SetSourceFileSource("embedded");
                    _logger?.Log(
                        FeaturesResources._0_found_in_embedded_PDB,
                        sourceDocument.FilePath
                    );
                }
                else
                {
                    _logger?.Log(
                        FeaturesResources._0_found_in_embedded_PDB_but_checksum_failed,
                        sourceDocument.FilePath
                    );
                }
            ...
            if (winner == sourceFileTask)
            {
                ...
                if (sourceFile is not null)
                {
                    ...
                    var result = LoadSourceFile(
                        ...
                        "SourceLink",
                        ignoreChecksum: true,
                        fromRemoteLocation: true
                    );
                    if (result is not null)
                    {
                        telemetry.SetSourceFileSource("sourcelink");
                        _logger?.Log(
                            FeaturesResources._0_found_via_SourceLink,
                            sourceDocument.FilePath
                        );
                    }
                    else
                    {
                        _logger?.Log(
                            FeaturesResources._0_found_via_SourceLink_but_couldnt_read_file,
                            sourceDocument.FilePath
                        );
                    }
        ...
        )
        {
            if (File.Exists(sourceDocument.FilePath))
            {
                ...
                if (result is not null)
                {
                    ...
                    _logger?.Log(
                        FeaturesResources._0_found_in_original_location,
                        sourceDocument.FilePath
                    );
                }
                else
                {
                    _logger?.Log(
                        FeaturesResources._0_found_in_original_location_but_checksum_failed,
                        sourceDocument.FilePath
                    );
                }
        ...
        private static SourceFileInfo? LoadSourceFile(
            string filePath,
            SourceDocument sourceDocument,
            Encoding encoding,
        ...
        )
        {
            return IOUtilities.PerformIO(() =>
            {
                ...
                var sourceText = SourceText.From(
                    stream,
                    encoding,
                    sourceDocument.ChecksumAlgorithm,
