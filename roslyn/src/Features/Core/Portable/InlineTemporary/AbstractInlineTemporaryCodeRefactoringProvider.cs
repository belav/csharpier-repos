...
namespace Microsoft.CodeAnalysis.InlineTemporary
{
        ...
        where TVariableDeclaratorSyntax : SyntaxNode
    {
        protected static async Task<
            ImmutableArray<TIdentifierNameSyntax>
        ...
        )
        {
            ...
            if (local != null)
            {
                ...
                var options = FindReferencesSearchOptions.Default with
                {
                    Cascade = false,
                };
                ...
                var findReferencesResult = await SymbolFinder
                    ...
                    .ConfigureAwait(false);
                var referencedSymbol = findReferencesResult.SingleOrDefault(r =>
                    Equals(r.Definition, local)
                );
