...
namespace Microsoft.CodeAnalysis.GenerateType
{
    ...
    >
    {
        protected abstract bool IsConversionImplicit(
        ...
        private partial class Editor
        {
            ...
            private TargetProjectChangeInLanguage _targetProjectChangeInLanguage =
                TargetProjectChangeInLanguage.NoChange;
            ...
            public async Task<ImmutableArray<CodeActionOperation>> GetOperationsAsync()
            {
                ...
                if (!_fromDialog)
                {
                    ...
                    {
                        return await GetGenerateIntoTypeOperationsAsync(namedType)
                            .ConfigureAwait(false);
                    }
                }
                else
                {
                    var namedType = await GenerateNamedTypeAsync(_generateTypeOptionsResult)
                        .ConfigureAwait(false);
                    ...
                    )
                    {
                        ...
                        _targetLanguageService = _generateTypeOptionsResult
                            .Project
                            .Services
            ...
            private string GetNamespaceToGenerateInto()
            {
                ...
                var rootNamespace = _service
                    .GetRootNamespace(_semanticDocument.SemanticModel.Compilation.Options)
                    .Trim();
                if (!string.IsNullOrWhiteSpace(rootNamespace))
                {
                    if (
                        namespaceToGenerateInto == rootNamespace
                        || namespaceToGenerateInto.StartsWith(
                            rootNamespace + ".",
            ...
            )
            {
                ...
                var rootNamespace = targetLanguageService
                    .GetRootNamespace(targetProject.CompilationOptions)
                    .Trim();
                if (!string.IsNullOrWhiteSpace(rootNamespace))
                {
                    var rootNamespaceLength = CheckIfRootNamespacePresentInNamespace(
            ...
            )
            {
                ...
                var newSolution = projectToBeUpdated
                    .Solution
                ...
                var newSemanticModel = await newDocument
                    .GetSemanticModelAsync(_cancellationToken)
                    .ConfigureAwait(false);
                var enclosingNamespace = newSemanticModel.GetEnclosingNamespace(
                    0,
                    _cancellationToken
                );
                ...
                var adjustedContainer =
                    isDialog ? folders
                    : _state.SimpleName != _state.NameOrMemberAccessExpression ? containers.ToList()
                ...
                if (newDocument.Project.Language == _semanticDocument.Document.Project.Language)
                {
                    var formattingService =
                        newDocument.GetLanguageService<INewDocumentFormattingService>();
                    if (formattingService is not null)
                    {
                        var cleanupOptions = await codeGenResult
                            .GetCodeCleanupOptionsAsync(_fallbackOptions, _cancellationToken)
                            .ConfigureAwait(false);
            ...
            )
            {
                ...
                var updatedSolution = projectToBeUpdated
                    .Solution
                    .AddDocument(
                ...
                updatedSolution = AddProjectReference(
                    projectToBeUpdated,
                    triggeringProject,
                    updatedSolution
                );
            ...
            private static Solution AddProjectReference(
                Project projectToBeUpdated,
                Project triggeringProject,
            ...
            {
                var enclosingNamespace = _semanticDocument
                    .SemanticModel
                ...
                var codeGenResult = await CodeGenerator
                    .AddNamedTypeDeclarationAsync(
                        new CodeGenerationSolutionContext(
                            solution,
                            new CodeGenerationContext(
                                afterThisLocation: _semanticDocument
                                    .SyntaxTree
                ...
                return ImmutableArray.Create<CodeActionOperation>(
                    new ApplyChangesOperation(codeGenResult.Project.Solution)
                );
            }
            ...
            )
            {
                ...
                {
                    enclosingNamespaceGeneratedTypeToAddAndLocation = await _service
                ...
                {
                    enclosingNamespaceGeneratedTypeToAddAndLocation = await _targetLanguageService
                        .GetOrGenerateEnclosingNamespaceSymbolAsync(
                            namedType,
                            containers,
                            generateTypeOptionsResult.ExistingDocument,
            ...
            )
            {
                ...
                else
                {
                    ...
                    if (
                        (
                            _semanticDocument.Project == _generateTypeOptionsResult.Project
                            && _semanticDocument.Project.Language == LanguageNames.CSharp
                        )
                        || (
                            _targetProjectChangeInLanguage == TargetProjectChangeInLanguage.NoChange
                            && _generateTypeOptionsResult.Project.Language == LanguageNames.CSharp
                    ...
                    if (
                        (
                            _semanticDocument.Project == _generateTypeOptionsResult.Project
                            && _semanticDocument.Project.Language == LanguageNames.VisualBasic
                        )
                        || (
                            _semanticDocument.Project != _generateTypeOptionsResult.Project
                            && _targetProjectChangeInLanguage
            ...
            private ITypeSymbol FixType(ITypeSymbol typeSymbol)
            {
            ...
            )
            {
                ...
                if (_state.BaseTypeOrInterfaceOpt != null)
                {
                    ...
                    var members =
                        from t in _state.BaseTypeOrInterfaceOpt.GetBaseTypesAndThis()
