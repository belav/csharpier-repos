...
namespace Microsoft.CodeAnalysis.CodeRefactorings
{
    ...
    ) : ICodeRefactoringService
    {
        ...
        private ImmutableDictionary<
            ...
            FixAllProviderInfo?
        > _fixAllProviderMap = ImmutableDictionary<
            CodeRefactoringProvider,
            FixAllProviderInfo?
        >.Empty;
        ...
        private ConcatImmutableArray<CodeRefactoringProvider> GetProviders(TextDocument document)
        {
            ...
            {
                allRefactorings = ProjectCodeRefactoringProvider.FilterExtensions(
                    document,
                    lazyProviders.Value,
                    GetExtensionInfo
                );
            ...
            static ImmutableArray<CodeRefactoringProvider> GetProjectRefactorings(
            ...
            static ProjectCodeRefactoringProvider.ExtensionInfo GetExtensionInfo(
                ExportCodeRefactoringProviderAttribute attribute
        ...
        )
        {
            using (
                TelemetryLogging.LogBlockTimeAggregated(
                    FunctionId.CodeRefactoring_Summary,
                    ...
                    tasks.Add(
                        Task.Run(
                            async () =>
                            {
                                ...
                                using (
                                    TelemetryLogging.LogBlockTime(
                                        FunctionId.CodeRefactoring_Delay,
                                        logMessage,
                                        CodeRefactoringTelemetryDelay
                                    )
                                )
                                {
                                    return await GetRefactoringFromProviderAsync(
                                            document,
                                            state,
                                            provider,
                                            providerMetadata,
