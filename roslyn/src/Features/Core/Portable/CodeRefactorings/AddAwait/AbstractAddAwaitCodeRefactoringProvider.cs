...
namespace Microsoft.CodeAnalysis.CodeRefactorings.AddAwait
{
        ...
        : CodeRefactoringProvider where TExpressionSyntax : SyntaxNode
    {
        ...
        public sealed override async Task ComputeRefactoringsAsync(CodeRefactoringContext context)
        {
            ...
            var expressions = await context.GetRelevantNodesAsync<TExpressionSyntax>()
                .ConfigureAwait(false);
        ...
        )
        {
            if (syntaxFacts.IsExpressionOfInvocationExpression(invocation.Parent))
            {
                ...
                if (
                    !model.GetTypeInfo(invocation.GetRequiredParent().GetRequiredParent())
                        .Type.IsErrorType()
                )
                    return false;
            }
        ...
        )
        {
            ...
            var awaitExpression = generator.AddParentheses(generator.AwaitExpression(withoutTrivia))
