...
namespace Microsoft.CodeAnalysis.AddMissingImports
{
        ...
        : IAddMissingImportsFeatureService
    {
        ...
        public async Task<Document> AddMissingImportsAsync(
            Document document,
            TextSpan textSpan,
            AddMissingImportsOptions options,
            IProgress<CodeAnalysisProgress> progressTracker,
            CancellationToken cancellationToken
        )
        {
            var analysisResult = await AnalyzeAsync(document, textSpan, options, cancellationToken)
        ...
        )
        {
            ...
            var unambiguousFixes = await addImportFeatureService
                .GetUniqueFixesAsync(
                    document,
                    textSpan,
            ...
            var usableFixes = unambiguousFixes.WhereAsArray(fixData =>
                DoesNotAddReference(fixData, document.Project.Id)
            );
        ...
        )
        {
            ...
            var getChangesTasks = codeActions.Select(action =>
            ...
            return await CleanUpNewLinesAsync(
                    newDocument,
                    insertSpans,
                    formattingOptions,
                    cancellationToken
        ...
        private async Task<Document> CleanUpNewLinesAsync(
            Document document,
        ...
        )
        {
            ...
            var operations = await codeAction
                .GetOperationsAsync(document.Project.Solution, progressTracker, cancellationToken)
                .ConfigureAwait(false);
            ...
            else
            {
                newSolution = await codeAction
                    .GetRequiredChangedSolutionAsync(progressTracker, cancellationToken)
                    .ConfigureAwait(false);
            }
