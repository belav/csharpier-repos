...
{
    internal abstract class AbstractAddMissingImportsFeatureService
        : IAddMissingImportsFeatureService
    {
        ...
        )
        {
            if (analysisResult.CanAddMissingImports)
            {
                ...
                var newDocument = await ApplyFixesAsync(
                    document,
                    analysisResult.AddImportFixData,
                    cancellationToken
                )
                    .ConfigureAwait(false);
                return newDocument;
        ...
        )
        {
            ...
            {
                return new AddMissingImportsAnalysisResult(ImmutableArray<AddImportFixData>.Empty);
            }

            // Find fixes for the diagnostic where there is only a single fix.
            var unambiguousFixes = await GetUnambiguousFixesAsync(
                document,
                diagnostics,
                cancellationToken
            )
            ...
            var usableFixes = unambiguousFixes.WhereAsArray(
                fixData => DoesNotAddReference(fixData, document.Project.Id)
            );
        ...
        )
        {
            return (
                ...
                && (
                    fixData.PortableExecutableReferenceProjectId is null
                    || fixData.PortableExecutableReferenceProjectId == currentProjectId
                )
        ...
        private static async Task<ImmutableArray<AddImportFixData>> GetUnambiguousFixesAsync(
            Document document,
        ...
        )
        {
            ...
            var symbolSearchService = solution.Workspace.Services
            ...
            var getFixesForDiagnosticsTasks = diagnostics.GroupBy(
                diagnostic => diagnostic.Location.SourceSpan
            )
                .Select(
                    diagnosticsForSourceSpan =>
                        addImportService.GetFixesForDiagnosticsAsync(
                            document,
                            diagnosticsForSourceSpan.Key,
                            diagnosticsForSourceSpan.AsImmutable(),
                            maxResultsPerDiagnostic: 2,
                            symbolSearchService,
        ...
        private static async Task<Document> ApplyFixesAsync(
            Document document,
        ...
        )
        {
            ...
            var textDiffingService = solution.Workspace.Services
            ...
            var insertSpans = allTextChanges.GroupBy(change => change.Span)
                .Select(
                    changes =>
                        new TextSpan(
                            changes.Key.Start,
                            changes.Sum(change => change.NewText!.Length)
                        )
                );
            ...
            return await CleanUpNewLinesAsync(newDocument, insertSpans, cancellationToken)
                .ConfigureAwait(false);
        }
        ...
        )
        {
            ...
            {
                newDocument = await CleanUpNewLinesAsync(
                    newDocument,
                    insertSpan,
                    languageFormatter,
                    options,
        ...
        )
        {
            var root = await document.GetRequiredSyntaxRootAsync(cancellationToken)
                .ConfigureAwait(false);
            var text = await document.GetTextAsync(cancellationToken).ConfigureAwait(false);
            var optionService = document.Project.Solution.Workspace.Services
            ...
            var textChanges = languageFormatter.Format(
                root,
                new[] { insertSpan },
        ...
        private sealed class CleanUpNewLinesFormatter : AbstractFormattingRule
        {
            ...
            public CleanUpNewLinesFormatter(SourceText text) => _text = text;
