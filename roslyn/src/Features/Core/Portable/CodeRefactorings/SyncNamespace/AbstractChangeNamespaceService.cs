...
namespace Microsoft.CodeAnalysis.ChangeNamespace
{
    ...
    {
        public abstract Task<bool> CanChangeNamespaceAsync(
            Document document,
            SyntaxNode container,
            CancellationToken cancellationToken
        );

        public abstract Task<Solution> ChangeNamespaceAsync(
            Document document,
            SyntaxNode container,
        ...
        public abstract Task<Solution?> TryChangeTopLevelNamespacesAsync(
            Document document,
            string targetNamespace,
            CancellationToken cancellationToken
        );
        ...
        public abstract bool TryGetReplacementReferenceSyntax(
            ...
            [NotNullWhen(returnValue: true)] out SyntaxNode? old,
            [NotNullWhen(returnValue: true)] out SyntaxNode? @new
        );
    }
        ...
        where TMemberDeclarationSyntax : SyntaxNode
    {
        ...
        protected abstract SyntaxList<TMemberDeclarationSyntax> GetMemberDeclarationsInContainer(
            SyntaxNode compilationUnitOrNamespaceDecl
        );

        protected abstract Task<SyntaxNode?> TryGetApplicableContainerFromSpanAsync(
            Document document,
            TextSpan span,
            CancellationToken cancellationToken
        );
        ...
        public override async Task<bool> CanChangeNamespaceAsync(
            Document document,
            SyntaxNode container,
            CancellationToken cancellationToken
        )
        {
            ...
            var applicableContainers = await GetValidContainersFromAllLinkedDocumentsAsync(
                document,
                container,
                cancellationToken
            )
        ...
        )
        {
            ...
            var originalNamespaceDeclarations = await GetTopLevelNamespacesAsync(
                document,
                cancellationToken
            )
                .ConfigureAwait(false);
            ...
            var semanticModel = await document.GetRequiredSemanticModelAsync(cancellationToken)
                .ConfigureAwait(false);
            var originalNamespaceName = semanticModel.GetRequiredDeclaredSymbol(
                originalNamespaceDeclarations.First(),
                cancellationToken
            )
                .ToDisplayString();
            ...
            {
                var namespaceName = semanticModel.GetRequiredDeclaredSymbol(
                    originalNamespaceDeclarations[i],
                    cancellationToken
                )
                    .ToDisplayString();
                ...
                syntaxRoot = await document.GetRequiredSyntaxRootAsync(cancellationToken)
                    .ConfigureAwait(false);
                ...
                var namespaces = await GetTopLevelNamespacesAsync(document, cancellationToken)
                    .ConfigureAwait(false);
                Debug.Assert(namespaces.Length == originalNamespaceDeclarations.Length);

                var namespaceToRename = namespaces[i];
                solution = await ChangeNamespaceAsync(
                    document,
                    namespaceToRename,
                    targetNamespace,
                    cancellationToken
            ...
            return solution;
            static async Task<ImmutableArray<SyntaxNode>> GetTopLevelNamespacesAsync(
                Document document,
                CancellationToken cancellationToken
            )
            {
                ...
                return syntaxRoot.DescendantNodes(n => !syntaxFacts.IsDeclaration(n))
        ...
        )
        {
            ...
            if (
                ...
                || (
                    ...
                    && !targetNamespace.Split(s_dotSeparator).All(syntaxFacts.IsValidIdentifier)
                )
            )
            {
            ...
            var containersFromAllDocuments = await GetValidContainersFromAllLinkedDocumentsAsync(
                document,
                container,
                cancellationToken
            )
            ...
            var annotatedSolution = await AnnotateContainersAsync(
                solution,
            ...
            var solutionAfterFirstMerge = await MergeDiffAsync(
                solution,
                solutionAfterNamespaceChange,
                cancellationToken
            )
        ...
        )
        {
            ...
            using var containersDisposer = ArrayBuilder<(DocumentId, SyntaxNode)>
                .GetInstance(ids.Length, out var containers);
            using var spanForContainersDisposer = PooledHashSet<TextSpan>
                .GetInstance(out var spanForContainers);

            foreach (var document in documents)
            {
                var container = await TryGetApplicableContainerFromSpanAsync(
                    document,
                    span,
                    cancellationToken
                )
        ...
        protected static async Task<Solution> AnnotateContainersAsync(
            Solution solution,
            ImmutableArray<(DocumentId, SyntaxNode)> containers,
            CancellationToken cancellationToken
        )
        {
            ...
            {
                var documentEditor = await solutionEditor.GetDocumentEditorAsync(
                    id,
                    cancellationToken
                )
                    .ConfigureAwait(false);
                documentEditor.ReplaceNode(
                    container,
                    container.WithAdditionalAnnotations(ContainerAnnotation)
                );
            }
        ...
        )
        {
            ...
            foreach (var memberDecl in memberDecls)
            {
                ...
                if (
                    memberSymbol is ITypeSymbol typeSymbol
        ...
        protected static bool IsSupportedLinkedDocument(
            Document document,
            out ImmutableArray<DocumentId> allDocumentIds
        )
        {
            var solution = document.Project.Solution;
            ...
            if (
                linkedDocumentIds.Any(
                    id =>
                        !PathUtilities.PathsEqual(
                            solution.GetRequiredDocument(id).Project.FilePath!,
        ...
        private static ImmutableArray<SyntaxNode> CreateImports(
            Document document,
            ImmutableArray<string> names,
            bool withFormatterAnnotation
        )
        {
            var generator = SyntaxGenerator.GetGenerator(document);
            using var builderDisposer = ArrayBuilder<SyntaxNode>
                .GetInstance(names.Length, out var builder);
        ...
        private static SyntaxNode CreateImport(
        ...
        )
        {
            ...
            var declaredSymbols = await GetDeclaredSymbolsInContainerAsync(
                document,
                container,
                cancellationToken
            )
            ...
            foreach (var refLocation in refLocations.SelectMany(locs => locs))
            {
                ...
                else
                {
                    ...
                    Debug.Assert(
                        !PathUtilities.PathsEqual(refLocation.Document.FilePath, document.FilePath)
                    );
            ...
            var solutionWithFixedReferences = await MergeDocumentChangesAsync(
        ...
        private static async Task<Solution> MergeDocumentChangesAsync(
            Solution originalSolution,
            Document[] changedDocuments,
            CancellationToken cancellationToken
        )
        {
            foreach (var document in changedDocuments)
            {
                originalSolution = originalSolution.WithDocumentSyntaxRoot(
                    document.Id,
                    await document.GetRequiredSyntaxRootAsync(cancellationToken)
                        .ConfigureAwait(false)
                );
            }
        ...
        {
            public LocationForAffectedSymbol(
                ReferenceLocation location,
                bool isReferenceToExtensionMethod
            )
            {
                ReferenceLocation = location;
        ...
        private static async Task<
            ImmutableArray<LocationForAffectedSymbol>
        > FindReferenceLocationsForSymbolAsync(
            Document document,
            ISymbol symbol,
            CancellationToken cancellationToken
        )
        {
            ...
            var referencedSymbols = await FindReferencesAsync(symbol, document, cancellationToken)
                .ConfigureAwait(false);
            builder.AddRange(
                referencedSymbols.Where(refSymbol => refSymbol.Definition.Equals(symbol))
            ...
            if (symbol is INamedTypeSymbol typeSymbol && typeSymbol.MightContainExtensionMethods)
            {
                foreach (var methodSymbol in typeSymbol.GetMembers().OfType<IMethodSymbol>())
                {
                    ...
                    {
                        var referencedMethodSymbols = await FindReferencesAsync(
                            methodSymbol,
                            document,
                            cancellationToken
                        )
                            .ConfigureAwait(false);
                        builder.AddRange(
                            referencedMethodSymbols.SelectMany(refSymbol => refSymbol.Locations)
                                .Select(
                                    location =>
                                        new LocationForAffectedSymbol(
                                            location,
        ...
        )
        {
            ...
            else
            {
                ...
                containersToAddImports = ImmutableArray.Create(
                    await document.GetRequiredSyntaxRootAsync(cancellationToken)
                        .ConfigureAwait(false)
                );
            }
            ...
            var namesToImport = GetAllNamespaceImportsForDeclaringDocument(
                oldNamespace,
                newNamespace
            );
            ...
            var root = await documentWithAddedImports.GetRequiredSyntaxRootAsync(cancellationToken)
                .ConfigureAwait(false);

            root = ChangeNamespaceDeclaration(
                (TCompilationUnitSyntax)root,
                oldNamespaceParts,
                newNamespaceParts
            )
            ...
            root = Formatter.Format(
                ...
                documentWithAddedImports.Project.Solution.Workspace,
                optionSet,
                cancellationToken
            );
            ...
            return await Simplifier.ReduceAsync(formattedDocument, optionSet, cancellationToken)
                .ConfigureAwait(false);
        }
        ...
        )
        {
            ...
            var optionSet = await documentWithRefFixed.GetOptionsAsync(cancellationToken)
                .ConfigureAwait(false);
            var placeSystemNamespaceFirst = optionSet.GetOption(
                GenerationOptions.PlaceSystemNamespaceFirst,
                documentWithRefFixed.Project.Language
            );
            var allowInHiddenRegions = document.CanAddImportsInHiddenRegions();
        ...
        )
        {
            ...
            foreach (var refLoc in refLocations)
            {
                ...
                var refNode = root.FindNode(
                    refLoc.ReferenceLocation.Location.SourceSpan,
                ...
                {
                    if (
                        abstractChangeNamespaceService.TryGetReplacementReferenceSyntax(
                            refNode,
                            newNamespaceParts,
                            syntaxFacts,
                            out var oldNode,
                    ...
                    )
                    {
                        editor.ReplaceNode(
                            oldNode,
                            newNode.WithAdditionalAnnotations(Simplifier.Annotation)
                        );
                    }
                }
            ...
            root = await fixedDocument.GetRequiredSyntaxRootAsync(cancellationToken)
                .ConfigureAwait(false);
            var result = (
                fixedDocument,
        ...
        )
        {
            ...
            var changeDocuments = await Task.WhenAll(
                documentsToProcess.Select(
                    doc =>
                        RemoveUnnecessaryImportsWorker(
                            doc,
            ...
            return await MergeDocumentChangesAsync(solution, changeDocuments, cancellationToken)
                .ConfigureAwait(false);
            ...
            {
                var removeImportService =
                    doc.GetRequiredLanguageService<IRemoveUnnecessaryImportsService>();
        ...
        )
        {
            ...
            foreach (var container in containers)
            {
                ...
                var contextLocation =
                    container is TNamespaceDeclarationSyntax
                ...
                root = addImportService.AddImports(
                    compilation,
                    root,
                    contextLocation,
                    imports,
        ...
        private static async Task<Solution> MergeDiffAsync(
        ...
        )
        {
            var diffMergingSession = new LinkedFileDiffMergingSession(
                oldSolution,
                newSolution,
                newSolution.GetChanges(oldSolution)
            );
            var mergeResult = await diffMergingSession.MergeDiffsAsync(
