        ...
        where TMemberDeclarationSyntax : SyntaxNode
    {
        ...
        )
        {
            ...
            var originalNamespaceName = semanticModel
                .GetRequiredDeclaredSymbol(originalNamespaceDeclarations.First(), cancellationToken)
                .ToDisplayString();
            ...
            for (var i = 0; i < originalNamespaceDeclarations.Length; i++)
            {
                var namespaceName = semanticModel
                    .GetRequiredDeclaredSymbol(originalNamespaceDeclarations[i], cancellationToken)
                    .ToDisplayString();
        ...
        )
        {
            ...
            foreach (var (id, container) in containers)
            {
                var documentEditor = await solutionEditor
                    .GetDocumentEditorAsync(id, cancellationToken)
                    .ConfigureAwait(false);
        ...
        )
        {
            ...
            root = Formatter.Format(
                ...
                services,
                documentOptions.FormattingOptions,
                cancellationToken
            );
        ...
        )
        {
            ...
            foreach (var refLoc in refLocations)
            {
                ...
                var addImportsOptions = await document
                    .GetAddImportPlacementOptionsAsync(fallbackOptions, cancellationToken)
                    .ConfigureAwait(false);
        ...
        )
        {
            ...
            )
            {
                ...
                var formattingOptions = await doc.GetSyntaxFormattingOptionsAsync(
                        fallbackOptions,
                        token
                    )
                    .ConfigureAwait(false);

                return await removeImportService
