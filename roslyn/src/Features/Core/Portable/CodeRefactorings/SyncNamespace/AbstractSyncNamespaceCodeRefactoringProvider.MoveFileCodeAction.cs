...
namespace Microsoft.CodeAnalysis.CodeRefactorings.SyncNamespace
{
    ...
    {
        private sealed class MoveFileCodeAction(State state, ImmutableArray<string> newFolders)
            : CodeAction
        {
            ...
            public override string Title =>
                _newfolders.Length > 0
                    ? string.Format(
                        FeaturesResources.Move_file_to_0,
                        string.Join(PathUtilities.DirectorySeparatorStr, _newfolders)
                    )
            ...
            public static ImmutableArray<MoveFileCodeAction> Create(State state)
            {
                ...
                Debug.Assert(
                    parts.IsEmpty
                        || parts.Any(static s => s.IndexOfAny(Path.GetInvalidPathChars()) < 0)
                );
