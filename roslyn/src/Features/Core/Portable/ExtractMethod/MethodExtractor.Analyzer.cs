...
namespace Microsoft.CodeAnalysis.ExtractMethod
{
    ...
    >
    {
        protected abstract partial class Analyzer
        {
            ...
            )
            {
                ...
                else
                {
                    ...
                    var parameters = MarkVariableInfoToUseAsReturnValueIfPossible(
                        GetMethodParameters(variableInfoMap)
                    );
                    var variableToUseAsReturnValue = parameters.FirstOrDefault(v =>
                        v.UseAsReturnValue
                    );
                    var returnType =
            ...
            private static ImmutableArray<VariableInfo> GetMethodParameters(
                Dictionary<ISymbol, VariableInfo> variableInfoMap
            )
            {
            ...
            )
            {
                ...
                foreach (var pair in symbolMap.Where(p => p.Key.Kind == SymbolKind.Field))
                {
                    ...
                    if (
                        tokens.All(t =>
                            !semanticFacts.IsWrittenTo(semanticModel, t.Parent, CancellationToken)
                        )
                    )
                        continue;
