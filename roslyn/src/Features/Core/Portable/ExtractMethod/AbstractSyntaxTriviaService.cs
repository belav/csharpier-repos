...
namespace Microsoft.CodeAnalysis.ExtractMethod
{
    internal abstract partial class AbstractSyntaxTriviaService : ISyntaxTriviaService
    {
        ...
        private static Dictionary<SyntaxToken, SyntaxToken> CreateOldToNewTokensMap(
            ...
            map[tokens[TriviaLocation.BeforeBeginningOfSpan]] = token.WithTrailingTrivia(emptyList)
            ...
            map[tokens[TriviaLocation.AfterBeginningOfSpan]] = token.WithLeadingTrivia(emptyList)
            ...
            map[tokens[TriviaLocation.BeforeEndOfSpan]] = token.WithTrailingTrivia(emptyList)
            ...
            map[tokens[TriviaLocation.AfterEndOfSpan]] = token.WithLeadingTrivia(emptyList)
                .WithAdditionalAnnotations(annotations[TriviaLocation.AfterEndOfSpan]);
        ...
        private static Dictionary<TriviaLocation, IEnumerable<SyntaxTrivia>> GetTriviaAtEdges(
            ...
            var triviaAtBeginning = SplitTrivia(
                tokens[TriviaLocation.BeforeBeginningOfSpan],
                tokens[TriviaLocation.AfterBeginningOfSpan],
                t => t.FullSpan.End <= textSpan.Start
            );
            var triviaAtEnd = SplitTrivia(
                tokens[TriviaLocation.BeforeEndOfSpan],
                tokens[TriviaLocation.AfterEndOfSpan],
                t => t.FullSpan.Start < textSpan.End
            );
