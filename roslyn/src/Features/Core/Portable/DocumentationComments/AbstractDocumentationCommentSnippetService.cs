...
namespace Microsoft.CodeAnalysis.DocumentationComments
{
        ...
        where TMemberNode : SyntaxNode
    {
        ...
        protected abstract List<string> GetDocumentationCommentStubLines(
            TMemberNode member,
        ...
        )
        {
            ...
            var lines = addIndentation
                ? GetDocumentationCommentLines(
                    ...
                    out _,
                    out var caretOffset,
                    out var spanToReplaceLength
                )
                : GetDocumentationCommentLinesNoIndentation(
                    token,
                    text,
                    options,
                    out caretOffset,
                    out spanToReplaceLength
                );
        ...
        private List<string>? GetDocumentationCommentLines(
            ...
            out int caretOffset,
            out int spanToReplaceLength
        )
        {
            indentText = null;

            var lines = GetDocumentationStubLines(
                token,
                text,
                options,
                out caretOffset,
                out spanToReplaceLength,
                out var existingCommentText
            );
            if (lines is null)
            {
                return lines;
        ...
        private List<string>? GetDocumentationCommentLinesNoIndentation(
            SyntaxToken token,
            SourceText text,
            in DocumentationCommentOptions options,
            out int caretOffset,
        ...
        private List<string>? GetDocumentationStubLines(
            SyntaxToken token,
            SourceText text,
            in DocumentationCommentOptions options,
            out int caretOffset,
            out int spanToReplaceLength,
            out string? existingCommentText
        )
        {
            caretOffset = 0;
            ...
            if (
                ...
                || !IsSingleExteriorTrivia(documentationComment, out existingCommentText)
            )
            {
                return null;
            }
        ...
        )
        {
            ...
            var newLine = options.NewLine;
            var lines = GetDocumentationCommentLines(
                token,
                text,
                options,
                out var indentText,
        ...
        )
        {
            ...
            var nextLineStartsWithDocComment =
                ...
                && text
                    .Lines[currentLine.LineNumber + 1]
                    .ToString()
                    .Trim()
