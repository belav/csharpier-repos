...
namespace Microsoft.CodeAnalysis.ConvertNumericLiteral
{
        ...
        : CodeRefactoringProvider where TNumericLiteralExpression : SyntaxNode
    {
        ...
        public sealed override async Task ComputeRefactoringsAsync(CodeRefactoringContext context)
        {
            ...
            var (prefix, number, suffix) = GetNumericLiteralParts(
                numericText,
                hexPrefix,
                binaryPrefix
            );
            ...
            if (kind != NumericKind.Decimal)
            {
                RegisterRefactoringWithResult(
                    value.ToString(),
                    FeaturesResources.Convert_to_decimal
                );
            }
            ...
            {
                RegisterRefactoringWithResult(
                    hexPrefix + value.ToString("X"),
                    FeaturesResources.Convert_to_hex
                );
            }
        ...
        private static Task<Document> ReplaceTokenAsync(
            Document document,
            SyntaxNode root,
