    ...
    internal partial class DiagnosticIncrementalAnalyzer
    {
        private static class InMemoryStorage
        {
            ...
            private static readonly ConcurrentDictionary<
                DiagnosticAnalyzer,
                ConcurrentDictionary<(object key, string stateKey), CacheEntry>
            > s_map = new(concurrencyLevel: 2, capacity: 10);
            ...
            )
            {
                ...
                var analyzerMap = s_map.GetOrAdd(
                    analyzer,
                    _ =>
                        new ConcurrentDictionary<(object key, string stateKey), CacheEntry>(
                            concurrencyLevel: 2,
                            capacity: 10
                        )
                );
