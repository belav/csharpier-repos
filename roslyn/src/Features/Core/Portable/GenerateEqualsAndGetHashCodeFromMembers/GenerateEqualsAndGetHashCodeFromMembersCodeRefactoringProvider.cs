...
namespace Microsoft.CodeAnalysis.GenerateEqualsAndGetHashCodeFromMembers
{
        ...
        : AbstractGenerateFromMembersCodeRefactoringProvider
    {
        ...
        public GenerateEqualsAndGetHashCodeFromMembersCodeRefactoringProvider(
            IPickMembersService? pickMembersService
        ...
        )
        {
            ...
            if (
                !helpers.IsOnTypeHeader(root, textSpan.Start, out var typeDeclaration)
        ...
        private static void GetExistingMemberInfo(
            INamedTypeSymbol containingType,
            out bool hasEquals,
                ...
                if (info != null && info.SelectedMembers.All(IsReadableInstanceFieldOrProperty))
                {
                    ...
                    )
                    {
                        ...
                        var root = await document
                            .GetRequiredSyntaxRootAsync(cancellationToken)
                            .ConfigureAwait(false);
                        var typeDeclaration = syntaxFacts.GetContainingTypeDeclaration(
                            root,
                            textSpan.Start
                        );
        ...
        )
        {
            ...
            {
                var globalOptions =
                    document.Project.Solution.Workspace.Services.GetRequiredService<ILegacyGlobalOptionsWorkspaceService>();
                var value =
                    globalOptions.GetGenerateEqualsAndGetHashCodeFromMembersImplementIEquatable(
                        document.Project.Language
                    );

                var displayName = equatableTypeOpt.ToDisplayString(
                ...
                pickMembersOptions.Add(
                    new PickMembersOption(
            ...
            return new GenerateEqualsAndGetHashCodeWithDialogCodeAction(
                ...
                containingType,
                members,
                pickMembersOptions.ToImmutable(),
                fallbackOptions,
                generateEquals,
                generateGetHashCode
            );
