...
namespace Microsoft.CodeAnalysis.ReplacePropertyWithMethods
{
    [
        ExportCodeRefactoringProvider(
            ...
            Name = PredefinedCodeRefactoringProviderNames.ReplacePropertyWithMethods
        ),
        Shared
    ]
            ...
            IEqualityComparer<(IPropertySymbol property, ReferenceLocation location)>
    {
        ...
        public override async Task ComputeRefactoringsAsync(CodeRefactoringContext context)
        {
            ...
            var propertyDeclaration = await service
                .GetPropertyDeclarationAsync(context)
        ...
        )
        {
            ...
            var propertyReferences = await SymbolFinder
                .FindReferencesAsync(propertySymbol, originalSolution, cancellationToken)
                .ConfigureAwait(false);
        ...
        private static ImmutableDictionary<
            IPropertySymbol,
            IFieldSymbol?
        > CreateDefinitionToBackingFieldMap(IEnumerable<ReferencedSymbol> propertyReferences)
        {
            var definitionToBackingField = ImmutableDictionary.CreateBuilder<
                IPropertySymbol,
                IFieldSymbol?
...
#pragma warning disable IDE0060 // Remove unused parameter - Method not completely implemented.
        private static string? GetDefinitionIssues(
...
#pragma warning restore IDE0060 // Remove unused parameter
        ...
        private async Task<Solution> UpdateReferencesAsync(
            ...
            ILookup<
                Document,
                (IPropertySymbol property, ReferenceLocation location)
            > referencesByDocument,
        ...
        )
        {
            ...
            var service =
                originalDocument.GetRequiredLanguageService<IReplacePropertyWithMethodsService>();
            ...
            if (references != null)
            {
                ...
                foreach (var (property, referenceLocation) in references.Distinct(this))
                {
                    ...
                    if (referenceLocation.IsImplicit || !syntaxFacts.IsIdentifierName(parent))
                    {
                        ...
                        editor.ReplaceNode(
                            parent,
                            parent.WithAdditionalAnnotations(
        ...
        private static async Task<
            MultiDictionary<DocumentId, IPropertySymbol>
        > GetDefinitionsByDocumentIdAsync(
            Solution originalSolution,
        ...
        )
        {
            ...
            var service =
                updatedDocument.GetRequiredLanguageService<IReplacePropertyWithMethodsService>();
            ...
            foreach (var (property, declaration) in currentDefinitions)
            {
                ...
                {
                    members = members
                        .Select(editor.Generator.AsInterfaceMember)
                        .WhereNotNull()
                        .ToImmutableArray();
                }
        ...
        private static async Task<
            ImmutableArray<(IPropertySymbol property, SyntaxNode declaration)>
        > GetCurrentPropertiesAsync(
            Solution updatedSolution,
        ...
        )
        {
            ...
            foreach (var originalDefinition in originalDefinitions)
            {
                ...
                var property = GetSymbolInCurrentCompilation(
                    compilation,
                    originalDefinition,
                    cancellationToken
                );
                var declaration = await GetPropertyDeclarationAsync(property, cancellationToken)
                    .ConfigureAwait(false);

                if (
                    property != null
                    && declaration != null
                    && updatedSolution.GetDocument(declaration.SyntaxTree)?.Id == documentId
                )
        ...
        private static TSymbol? GetSymbolInCurrentCompilation<TSymbol>(
            Compilation compilation,
            ...
            where TSymbol : class, ISymbol
        {
            return originalDefinition
                    ...
                    .GetAnySymbol() as TSymbol;
        }

        public bool Equals(
            (IPropertySymbol property, ReferenceLocation location) x,
            (IPropertySymbol property, ReferenceLocation location) y
        )
        {
