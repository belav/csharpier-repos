...
namespace Microsoft.CodeAnalysis.ConvertAutoPropertyToFullProperty
{
    ...
    {
        internal abstract Task<string> GetFieldNameAsync(
            Document document,
        ...
        {
            var containingProperty =
                await context.TryGetRelevantNodeAsync<TPropertyDeclarationNode>()
                    .ConfigureAwait(false);
        ...
        )
        {
            ...
            var fieldName = await GetFieldNameAsync(document, propertySymbol, cancellationToken)
                .ConfigureAwait(false);
