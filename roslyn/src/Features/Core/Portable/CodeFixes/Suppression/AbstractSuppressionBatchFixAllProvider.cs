    ...
    internal abstract class AbstractSuppressionBatchFixAllProvider : FixAllProvider
    {
        ...
        public override async Task<CodeAction?> GetFixAsync(FixAllContext fixAllContext)
        {
            if (fixAllContext.Document != null)
            {
                ...
                return await GetFixAsync(documentsAndDiagnosticsToFixMap, fixAllContext)
            ...
            else
            {
                ...
                return await GetFixAsync(projectsAndDiagnosticsToFixMap, fixAllContext)
                    .ConfigureAwait(false);
            }
        }
        ...
        )
        {
            ...
            if (documentsAndDiagnosticsToFixMap?.Any() == true)
            {
                ...
                {
                    var functionId =
                        FunctionId.CodeFixes_FixAllOccurrencesComputation_Document_Merge;
                    using (
                        Logger.LogBlock(
                            functionId,
        ...
        private async Task<
            ImmutableArray<(Diagnostic diagnostic, CodeAction action)>
                ...
                fixerTasks.Add(
                    Task.Run(
                        () =>
                        {
                            ...
                            return fixAllState.CodeFixProvider.RegisterCodeFixesAsync(context)
                                ?? Task.CompletedTask;
                        },
                        cancellationToken
                    )
        ...
        )
        {
            ...
            if (projectsAndDiagnosticsToFixMap != null && projectsAndDiagnosticsToFixMap.Any())
            {
                ...
                {
                    var functionId =
                        FunctionId.CodeFixes_FixAllOccurrencesComputation_Project_Merge;
                    using (Logger.LogBlock(functionId, cancellationToken))
                    {
                        ...
                        return await TryGetMergedFixAsync(result, fixAllState, cancellationToken)
                            .ConfigureAwait(false);
                    }
                }
            }
        ...
        public virtual string GetFixAllTitle(FixAllState fixAllState) =>
            FixAllContextHelper.GetDefaultFixAllTitle(
                fixAllState.Scope,
                fixAllState.DiagnosticIds,
        ...
        private static async Task<
            IReadOnlyDictionary<DocumentId, ConcurrentBag<(CodeAction, Document)>>
        > GetDocumentIdToChangedDocumentsAsync(
            Solution oldSolution,
            ImmutableArray<(Diagnostic diagnostic, CodeAction action)> diagnosticsAndCodeActions,
            CancellationToken cancellationToken
        )
        {
            ...
            {
                getChangedDocumentsTasks.Add(
                    GetChangedDocumentsAsync(
                        oldSolution,
                        documentIdToChangedDocuments,
                        action,
                        cancellationToken
        ...
        private static async Task<
            IReadOnlyDictionary<DocumentId, SourceText>
        > GetDocumentIdToFinalTextAsync(
            Solution oldSolution,
            IReadOnlyDictionary<
                DocumentId,
                ConcurrentBag<(CodeAction, Document)>
        ...
        )
        {
            ...
            {
                getFinalDocumentTasks.Add(
                    GetFinalDocumentTextAsync(
                        oldSolution,
                        codeActionToDiagnosticLocation,
                        documentIdToFinalText,
                        changedDocuments,
                        cancellationToken
                    )
                );
        ...
        )
        {
            ...
            if (orderedDocuments.Length == 1)
            {
                ...
                var finalText = await document.GetTextAsync(cancellationToken)
                    .ConfigureAwait(false);
            ...
            foreach (var (_, currentDocument) in orderedDocuments)
            {
                ...
                await merger.TryMergeChangesAsync(currentDocument, cancellationToken)
                    .ConfigureAwait(false);
            }

            // WithChanges requires a ordered list of TextChanges without any overlap.
            var newText = await merger.GetFinalMergedTextAsync(cancellationToken)
                .ConfigureAwait(false);
        ...
        )
        {
            ...
            var documentIdsWithChanges = solutionChanges.GetProjectChanges()
            ...
            foreach (var documentId in documentIdsWithChanges)
            {
                ...
                documentIdToChangedDocuments.GetOrAdd(documentId, s_getValue)
                    .Add((codeAction, changedDocument));
            }
        }
    }
}
