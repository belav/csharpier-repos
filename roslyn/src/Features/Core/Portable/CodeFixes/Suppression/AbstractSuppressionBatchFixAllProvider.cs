...
namespace Microsoft.CodeAnalysis.CodeFixes.Suppression
{
    ...
    internal abstract class AbstractSuppressionBatchFixAllProvider : FixAllProvider
    {
        public override async Task<CodeAction?> GetFixAsync(FixAllContext fixAllContext)
        {
            ...
            {
                var documentsAndDiagnosticsToFixMap = await fixAllContext
            ...
            {
                var projectsAndDiagnosticsToFixMap = await fixAllContext
        ...
        )
        {
            ...
            if (documentsAndDiagnosticsToFixMap?.Any() == true)
            {
                ...
                progressTracker.Report(
                    CodeAnalysisProgress.Description(fixAllContext.GetDefaultFixAllTitle())
                );
                ...
                fixerTasks.Add(
                    Task.Run(
                        () =>
                        {
                            ...
                            return fixAllState.Provider.RegisterCodeFixesAsync(context)
                                ?? Task.CompletedTask;
                        },
                        cancellationToken
                    )
        ...
        )
        {
            ...
            {
                var title = FixAllHelper.GetDefaultFixAllTitle(
                    fixAllState.Scope,
                    title: fixAllState.DiagnosticIds.First(),
        ...
        )
        {
            ...
            var orderedDocuments = changedDocuments
                .OrderBy(t => codeActionToDiagnosticLocation[t.action])
            ...
            foreach (var (_, currentDocument) in orderedDocuments)
            {
                ...
                await merger
                    .TryMergeChangesAsync(currentDocument, cancellationToken)
                    .ConfigureAwait(false);
            }
        ...
        )
        {
            ...
            foreach (var documentId in documentIdsWithChanges)
            {
                ...
                documentIdToChangedDocuments
                    .GetOrAdd(documentId, s_getValue)
