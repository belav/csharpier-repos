        ...
        private static class PragmaBatchFixHelpers
        {
                ...
                return CodeAction.Create(
                    ...
                    createChangedDocument: ct =>
                        BatchPragmaFixesAsync(
                            ...
                            pragmaActions,
                            pragmaDiagnostics,
                            fixAllState.CodeActionOptionsProvider,
                            cancellationToken
                        ),
            ...
            )
            {
                ...
                for (var i = 0; i < pragmaActions.Length; i++)
                {
                    ...
                    if (
                        !currentDiagnosticSpans.TryGetValue(
                            diagnostic,
                    ...
                    var newSuppressionFixes = await suppressionFixProvider
                        .GetFixesAsync(
                            currentDocument,
                    ...
                    if (newSuppressionFix != null)
                    {
                        var newPragmaAction =
                            ...
                            ?? newSuppressionFix
                                .Action.NestedCodeActions.OfType<IPragmaBasedCodeAction>()
                                .SingleOrDefault();
                        if (newPragmaAction != null)
                        {
                            ...
                            var currentText = await currentDocument
                                ...
                                .ConfigureAwait(false);
                            var orderedChanges = startTokenChanges
                                .Concat(endTokenChanges)
                                .OrderBy(change => change.Span)
                                .Distinct();
            ...
            {
                var newDocument = await pragmaAction
            ...
            private static void UpdateDiagnosticSpans(
                ImmutableArray<Diagnostic> diagnostics,
            ...
            )
            {
                ...
                static bool IsFollowingSpan(TextSpan span, TextChange textChange) =>
                    span.Start >= textChange.Span.End;
                static bool IsEnclosingSpan(TextSpan span, TextChange textChange) =>
                    span.Contains(textChange.Span);
