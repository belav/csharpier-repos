        ...
        internal abstract partial class RemoveSuppressionCodeAction
        {
            ...
            private class PragmaRemoveAction : RemoveSuppressionCodeAction, IPragmaBasedCodeAction
            {
                ...
                public override SyntaxTree SyntaxTreeToModify =>
                    _suppressionTargetInfo.StartToken.SyntaxTree;
                ...
                )
                {
                    ...
                    int indexOfLeadingPragmaDisableToRemove = -1,
                        indexOfTrailingPragmaEnableToRemove = -1;
                    if (
                        CanRemovePragmaTrivia(
                            _suppressionTargetInfo.StartToken,
                            _diagnostic,
                            Fixer,
                            isStartToken: true,
                            indexOfTriviaToRemove: out indexOfLeadingPragmaDisableToRemove
                        )
                        && CanRemovePragmaTrivia(
                ...
                private static SyntaxTriviaList GetTriviaListForSuppression(
                    SyntaxToken token,
                    bool isStartToken,
                    AbstractSuppressionCodeFixProvider fixer
                )
                ...
                )
                {
                    ...
                    bool shouldIncludeTrivia(SyntaxTrivia t) =>
                        isStartToken
                            ? t.FullSpan.End <= diagnosticSpan.Start
                            : t.FullSpan.Start >= diagnosticSpan.End;
                    ...
                    {
                        if (
                            fixer.IsAnyPragmaDirectiveForId(
                                trivia,
                                diagnostic.Id,
                ...
                private SyntaxToken GetNewTokenWithModifiedPragma(
                    SyntaxToken token,
                    TextSpan currentDiagnosticSpan,
                    bool add,
                    bool toggle,
                ...
                )
                {
                    ...
                    var trivia =
                        _suppressionTargetInfo.StartToken.LeadingTrivia.ElementAt(indexOfPragma);
                    ...
                    var dummyDiagnosticWithLocationToCheck = Diagnostic.Create(
                        _diagnostic.Descriptor,
                        locationToCheck
                    );
                    var effectiveDiagnostic = CompilationWithAnalyzers
