...
namespace Microsoft.CodeAnalysis.CodeFixes.Configuration
{
    ...
    internal sealed partial class ConfigurationUpdater
    {
        ...
        )
        {
            ...
            if (!codeStyleOptionValues.IsEmpty)
            {
                return ConfigureCodeStyleOptionsAsync(
                    codeStyleOptionValues.Select(
                        t => (t.optionName, t.currentOptionValue, t.isPerLanguage)
                    ),
        ...
        internal static ImmutableArray<(
            OptionKey optionKey,
            ICodeStyleOption codeStyleOptionValue,
            IEditorConfigStorageLocation2 location,
            bool isPerLanguage
        )> GetCodeStyleOptionsForDiagnostic(Diagnostic diagnostic, Project project)
        {
            if (
                ...
                foreach (var option in options.OrderBy(option => option.Name))
                {
                    ...
                    if (editorConfigLocation != null)
                    {
                        var optionKey = new OptionKey(
                            option,
                            option.IsPerLanguage ? project.Language : null
                        );
