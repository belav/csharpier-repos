...
namespace Microsoft.CodeAnalysis.ImplementAbstractClass
{
    ...
    )
    {
        ...
        )
        {
            ...
            {
                IMethodSymbol method => GenerateMethod(
                    compilation,
                    method,
                    throughMember,
                    modifiers,
                ...
                IEventSymbol @event => GenerateEvent(
                    @event,
                    throughMember,
                    accessibility,
                    modifiers
        ...
        public ImmutableArray<(ISymbol symbol, bool canDelegateAllMembers)> GetDelegatableMembers(
            CancellationToken cancellationToken
        )
        {
            ...
            using var _ = ArrayBuilder<(ISymbol symbol, bool canDelegateAllMembers)>.GetInstance(
                out var result
            );
            ...
            foreach (var member in members)
            {
                ...
                var accessibleCount = allUnimplementedMembers.Count(m =>
                    m.IsAccessibleWithin(ClassType, throughType: memberType)
                );
