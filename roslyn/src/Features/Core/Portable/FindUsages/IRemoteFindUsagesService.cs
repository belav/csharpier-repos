...
namespace Microsoft.CodeAnalysis.FindUsages
{
    ...
    internal sealed class FindUsagesServerCallback(Solution solution, IFindUsagesContext context)
    {
        ...
        )
        {
            try
            {
                ...
                var rehydrated = await definition
                ...
                await _context
                    .OnDefinitionFoundAsync(rehydrated, cancellationToken)
                    .ConfigureAwait(false);
            }
            catch (Exception ex)
        ...
        )
        {
            try
            {
                var rehydrated = await reference
                    .RehydrateAsync(
                        _solution,
                    ...
                    .ConfigureAwait(false);
                await _context
                    .OnReferenceFoundAsync(rehydrated, cancellationToken)
                    .ConfigureAwait(false);
            }
            catch (Exception ex)
                when (FatalError.ReportAndPropagateUnlessCanceled(ex, cancellationToken))
    ...
    )
    {
        ...
        public async ValueTask<DefinitionItem.DefaultDefinitionItem> RehydrateAsync(
            Solution solution,
            CancellationToken cancellationToken
        )
        {
            var sourceSpans = await SourceSpans
                .SelectAsArrayAsync(
                    static (ss, solution, cancellationToken) =>
                        ss.RehydrateAsync(solution, cancellationToken),
                    solution,
                    cancellationToken
                )
                .ConfigureAwait(false);
