...
namespace Microsoft.CodeAnalysis.SymbolSearch
{
    ...
    internal class SymbolSearchUpdateEngine
    {
        ...
        private sealed class Updater
        {
            ...
            public Updater(
                SymbolSearchUpdateEngine service,
                ISymbolSearchLogService logService,
                string source,
                string localSettingsDirectory
            )
            {
                ...
                _cacheDirectoryInfo = new DirectoryInfo(
                    Path.Combine(
                        localSettingsDirectory,
                        "PackageCache",
                        string.Format(
            ...
            internal async ValueTask UpdateInBackgroundAsync(CancellationToken cancellationToken)
            {
                ...
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    {
                        await LogInfoAsync("Update canceled. Ending update loop", cancellationToken)
                            .ConfigureAwait(false);
                        return;
                    }
                }
            ...
            )
            {
                ...
                try
                {
                    ...
                    when (_service._reportAndSwallowExceptionUnlessCanceled(e, cancellationToken))
                {
            ...
            {
                await LogInfoAsync("Cleaning cache directory", cancellationToken)
                    .ConfigureAwait(false);

                // (intentionally not wrapped in IOUtilities.  If this throws we want to restart).
                if (!_service._ioService.Exists(_cacheDirectoryInfo))
                {
                    await LogInfoAsync("Creating cache directory", cancellationToken)
                        .ConfigureAwait(false);

                    // (intentionally not wrapped in IOUtilities.  If this throws we want to restart).
                    _service._ioService.Create(_cacheDirectoryInfo);
                    await LogInfoAsync("Cache directory created", cancellationToken)
                        .ConfigureAwait(false);
                }
            ...
            )
            {
                ...
                try
                {
                    await CreateAndSetInMemoryDatabaseAsync(bytes, cancellationToken)
                        .ConfigureAwait(false);
                }
                catch (Exception e)
                    when (_service._reportAndSwallowExceptionUnlessCanceled(e, cancellationToken))
                ...
                await WriteDatabaseFileAsync(databaseFileInfo, bytes, cancellationToken)
                    .ConfigureAwait(false);
            ...
            private async Task WriteDatabaseFileAsync(
                FileInfo databaseFileInfo,
                byte[] bytes,
                CancellationToken cancellationToken
            )
            ...
            )
            {
                ...
                await LogInfoAsync("Reading in local database", cancellationToken)
                    .ConfigureAwait(false);
                // (intentionally not wrapped in IOUtilities.  If this throws we want to restart).
                ...
                var serverPath = Invariant(
                    $"Elfie_V{AddReferenceDatabaseTextFileFormatVersion}/{database.DatabaseVersion}_Patch.xml"
                );
            ...
            )
            {
                ...
                if (upToDate)
                {
                    await LogInfoAsync("Local version is up to date", cancellationToken)
                        .ConfigureAwait(false);
                ...
                if (tooOld)
                {
                    await LogInfoAsync("Local version too old", cancellationToken)
                        .ConfigureAwait(false);
                    return null;
                }
                ...
                await WriteDatabaseFileAsync(databaseFileInfo, finalBytes, cancellationToken)
                    .ConfigureAwait(false);

                return _service._delayService.UpdateSucceededDelay;
            }

            private static void ParsePatchElement(
                XElement patchElement,
                out bool upToDate,
                out bool tooOld,
                out byte[] patchBytes
            )
            {
                ...
                var value = (upToDate ? 1 : 0) + (tooOld ? 1 : 0) + (hasPatchBytes ? 1 : 0);
                if (value != 1)
                {
                    throw new FormatException(
                        $"Patch format invalid. {nameof(upToDate)}={upToDate} {nameof(tooOld)}={tooOld} {nameof(hasPatchBytes)}={hasPatchBytes}"
                    );
                }
            }
            ...
            )
            {
                ...
                await LogInfoAsync("Creating database from bytes completed", cancellationToken)
                    .ConfigureAwait(false);
                return result;
            }
            ...
            )
            {
                ...
                while (true)
                {
                    ...
                    var (element, delay) = await TryDownloadFileAsync(client, cancellationToken)
                        .ConfigureAwait(false);
                    if (element == null)
                    {
                        await LogInfoAsync(
            ...
            )
            {
                ...
                await LogInfoAsync("Read file completed. Client returned data", cancellationToken)
                    .ConfigureAwait(false);
                await LogInfoAsync("Converting data to XElement", cancellationToken)
                    .ConfigureAwait(false);
                ...
                try
                {
                    ...
                    await LogInfoAsync("Converting data to XElement completed", cancellationToken)
                        .ConfigureAwait(false);
                    ...
                    when (_service._reportAndSwallowExceptionUnlessCanceled(e, cancellationToken))
                {
                    ...
                    await LogInfoAsync($"Unable to parse file as XElement", cancellationToken)
                        .ConfigureAwait(false);
                    return (element: null, _service._delayService.CatastrophicFailureDelay);
                }
            }

            private async Task RepeatIOAsync(
                ...
                for (var i = 0; i < repeat; i++)
                {
                    ...
                    try
                    {
                        ...
                        when (IOUtilities.IsNormalIOException(e)
                            || _service._reportAndSwallowExceptionUnlessCanceled(
                                e,
                                cancellationToken
                            )
                        )
            ...
            )
            {
                ...
                if (contentsAttribute == null)
                {
                    _service._reportAndSwallowExceptionUnlessCanceled(
                        new FormatException(
                            $"Database element invalid. Missing '{ContentAttributeName}' attribute"
                        ),
                ...
                if (checksumAttribute != null)
                {
                    ...
                    if (!StringComparer.Ordinal.Equals(expectedChecksum, actualChecksum))
                    {
                        _service._reportAndSwallowExceptionUnlessCanceled(
                            new FormatException(
                                $"Checksum mismatch: expected != actual. {expectedChecksum} != {actualChecksum}"
                            ),
                            CancellationToken.None
                        );
