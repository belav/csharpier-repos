...
namespace Microsoft.CodeAnalysis.SymbolSearch
{
    ...
    internal partial class SymbolSearchUpdateEngine
    {
        ...
        private static void LogInfo(string text) => Log(text);
        ...
        )
        {
            ...
            private readonly DirectoryInfo _cacheDirectoryInfo = new DirectoryInfo(
                Path.Combine(
                    localSettingsDirectory,
                    "PackageCache",
                    string.Format(Invariant($"Format{AddReferenceDatabaseTextFileFormatVersion}"))
                )
            );
            ...
            )
            {
                ...
                using var client = _service._fileDownloaderFactory
            ...
            )
            {
                ...
                var contentBytes = await Updater
                    .ConvertContentAttributeAsync(contentsAttribute, cancellationToken)
                    .ConfigureAwait(false);
            ...
            private static async Task<byte[]> ConvertContentAttributeAsync(
