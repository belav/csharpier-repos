...
namespace Microsoft.CodeAnalysis.ExternalAccess.UnitTesting.SolutionCrawler
{
    internal partial class UnitTestingSolutionCrawlerRegistrationService
    {
        internal sealed partial class UnitTestingWorkCoordinator : IUnitTestingWorkCoordinator
        {
            ...
            )
            {
                ...
                var activeFileBackOffTimeSpan =
                    UnitTestingSolutionCrawlerTimeSpan.ActiveFileWorkerBackOff;
                var allFilesWorkerBackOffTimeSpan =
                    UnitTestingSolutionCrawlerTimeSpan.AllFilesWorkerBackOff;
                var entireProjectWorkerBackOffTimeSpan =
                    UnitTestingSolutionCrawlerTimeSpan.EntireProjectWorkerBackOff;
                ...
                var semanticBackOffTimeSpan =
                    UnitTestingSolutionCrawlerTimeSpan.SemanticChangeBackOff;
                var projectBackOffTimeSpan =
                    UnitTestingSolutionCrawlerTimeSpan.ProjectPropagationBackOff;

                _semanticChangeProcessor = new UnitTestingSemanticChangeProcessor(
                    listener,
                    _registration,
            ...
            public void Reanalyze(
                ...
                UnitTestingReanalyzeScope scope
#if false // Not used in unit testing crawling
                ...
                switch (args.Kind)
                {
                    ...
                    case WorkspaceChangeKind.ProjectRemoved:
                        ...
                        EnqueueFullProjectEvent(
                            args.OldSolution,
                            args.ProjectId,
                            UnitTestingInvocationReasons.DocumentRemoved,
                            eventName
                        );
                        break;
                    ...
                    case WorkspaceChangeKind.AnalyzerConfigDocumentReloaded:
                        ...
                        EnqueueFullProjectEvent(
                            args.NewSolution,
                            args.ProjectId,
                            UnitTestingInvocationReasons.AdditionalDocumentChanged,
                            eventName
                        );
                        break;
            ...
            private void EnqueueSolutionChangedEvent(
            ...
            )
            {
                _eventProcessingQueue.ScheduleTask(
                    ...
                    async () =>
                    {
                        ...
                        {
                            await EnqueueFullProjectWorkItemAsync(
                                    addedProject,
                                    UnitTestingInvocationReasons.DocumentAdded
                                )
                        ...
                        {
                            await EnqueueWorkItemAsync(projectChanges)
            ...
            private void EnqueueFullSolutionEvent(
                Solution solution,
            ...
            private void EnqueueProjectChangedEvent(
                Solution oldSolution,
                Solution newSolution,
                ProjectId projectId,
                string eventName
            )
            {
                _eventProcessingQueue.ScheduleTask(
                    ...
                    async () =>
                    {
                        ...
                        await EnqueueWorkItemAsync(newProject.GetChanges(oldProject))
                            .ConfigureAwait(false);
                    },
                    _shutdownToken
                );
            ...
            private void EnqueueFullProjectEvent(
                Solution solution,
                ProjectId projectId,
                UnitTestingInvocationReasons invocationReasons,
                string eventName
            ...
            private void EnqueueFullDocumentEvent(
                Solution solution,
            ...
            )
            {
                _eventProcessingQueue.ScheduleTask(
                    ...
                    () =>
                    {
                        ...
                        return EnqueueDocumentWorkItemAsync(
                            project,
                            documentId,
                            document: null,
                            invocationReasons
            ...
            private void EnqueueDocumentChangedEvent(
                Solution oldSolution,
                Solution newSolution,
                DocumentId documentId,
                string eventName
                ...
                _eventProcessingQueue.ScheduleTask(
                    ...
                    async () =>
                    {
                        ...
                        await EnqueueChangedDocumentWorkItemAsync(
                                ...
                                newProject.GetRequiredDocument(documentId)
                            )
                            .ConfigureAwait(false);
                        ...
                        if (
                            _solutionCrawlerOptionsService?.EnableDiagnosticsInSourceGeneratedFiles
                            == true
                        )
                        {
                            ...
                            foreach (
                                var (oldDocumentId, _) in oldProjectSourceGeneratedDocumentsById
                            )
                            {
                                if (
                                    !newProjectSourceGeneratedDocumentsById.ContainsKey(
                                        oldDocumentId
                                    )
                                )
                                {
                            ...
                            foreach (
                                var (
                                    newDocumentId,
                                    newDocument
                                ) in newProjectSourceGeneratedDocumentsById
                            )
                            {
                                if (
                                    !oldProjectSourceGeneratedDocumentsById.TryGetValue(
                                        newDocumentId,
                                        out var oldDocument
                                    )
                                )
            ...
            private async Task EnqueueDocumentWorkItemAsync(
                Project project,
                DocumentId documentId,
            ...
            )
            {
                ...
                var isLowPriority =
                    ...
                    && await priorityService
                        .IsLowPriorityAsync(sourceDocument, _shutdownToken)
                        .ConfigureAwait(false);
            ...
            private async Task EnqueueFullProjectWorkItemAsync(
                Project project,
                UnitTestingInvocationReasons invocationReasons
            )
            {
            ...
            )
            {
                ...
                )
                {
                    projectConfigurationChange = projectConfigurationChange.With(
                        UnitTestingInvocationReasons.ProjectConfigurationChanged
                    );
                }
            ...
            private async Task EnqueueChangedDocumentWorkItemAsync(
                Document oldDocument,
                Document newDocument
            )
            {
                var differenceService =
                    newDocument.GetLanguageService<IUnitTestingDocumentDifferenceService>();
                ...
                if (differenceService == null)
                {
                    ...
                    await EnqueueDocumentWorkItemAsync(
                            ...
                            newDocument.Id,
                            newDocument,
                            UnitTestingInvocationReasons.DocumentChanged
                        )
                        .ConfigureAwait(false);
                }
                else
                {
...
#else
                    var differenceResult = differenceService.GetDifference(
                        oldDocument,
                        newDocument,
                        _shutdownToken
                    );
                    ...
                    if (differenceResult != null)
                        await EnqueueDocumentWorkItemAsync(
                                ...
                                newDocument.Id,
                                newDocument,
                                differenceResult.ChangeType,
                                differenceResult.ChangedMember
                            )
                            .ConfigureAwait(false);
                }
            }
            ...
            internal readonly struct TestAccessor
            {
                ...
                internal void WaitUntilCompletion(
                    ImmutableArray<IUnitTestingIncrementalAnalyzer> workers
                )
                {
        ...
        internal readonly struct UnitTestingReanalyzeScope
        {
            ...
            public UnitTestingReanalyzeScope(
            ...
            public bool HasMultipleDocuments =>
                _solutionId != null || _projectOrDocumentIds?.Count > 1;
            ...
            public IEnumerable<(Project project, DocumentId documentId)> GetDocumentIds(
                Solution solution
            )
            {
                if (_solutionId != null && solution.Id != _solutionId)
