...
namespace Microsoft.CodeAnalysis.CodeRefactorings.PullMemberUp
{
    internal static class MembersPuller
    {
        ...
        )
        {
            ...
            if (
                result.PullUpOperationNeedsToDoExtraChanges
            ...
            var title = selectedMembers.IsSingle()
                ? string.Format(
                    ...
                    selectedMembers.Single().Name,
                    result.Destination.Name
                )
                : string.Format(
                    FeaturesResources.Pull_selected_members_up_to_0,
                    result.Destination.Name
                );
            ...
            return SolutionChangeAction.Create(
                title,
                cancellationToken =>
                    PullMembersUpAsync(document, result, fallbackOptions, cancellationToken),
                title
            );
        }
        ...
        )
        {
            ...
            var destinationWithMembersAdded = codeGenerationService.AddMembers(
            ...
            foreach (var analysisResult in pullMemberUpOptions.MemberAnalysisResults)
            {
                foreach (var declaration in symbolToDeclarationsMap[analysisResult.Member])
                {
                    ...
                    {
                        if (
                            analysisResult.ChangeOriginalToNonStatic
                            || analysisResult.ChangeOriginalToPublic
                        )
                        {
        ...
        )
        {
            ...
            if (declaration.Equals(eventDeclaration) && !isEventHasExplicitAddOrRemoveMethod)
            {
                ...
                var eventGenerationInfo = info.WithContext(
                    new CodeGenerationContext(null, null, generateMethodBodies: false)
                );
        ...
        )
        {
            ...
            var info = options.GenerationOptions.GetInfo(
                context,
                destinationEditor.OriginalDocument.Project
            );
            ...
            var addImportsService =
                destinationEditor.OriginalDocument.GetRequiredLanguageService<IAddImportsService>();
        ...
        private static SyntaxNode RemoveLeadingTriviaBeforeFirstMember(
            SyntaxNode root,
            ISyntaxFactsService syntaxFacts
        )
        {
        ...
        private static SyntaxNode AddLeadingTriviaBeforeFirstMember(
            SyntaxNode root,
            ISyntaxFactsService syntaxFacts,
        ...
        private static ImmutableArray<SyntaxNode> GetImports(
            SyntaxNode start,
            ISyntaxFactsService syntaxFacts
        )
        {
        ...
        private static ISymbol MakeAbstractVersion(ISymbol member)
        {
            ...
            else if (member is IPropertySymbol propertySymbol)
            {
                return CodeGenerationSymbolFactory.CreatePropertySymbol(
                    ...
                    setMethod: propertySymbol.SetMethod
                );
            }
            else if (member is IEventSymbol eventSymbol)
            {
        ...
        private static async Task<
            ImmutableDictionary<ISymbol, ImmutableArray<SyntaxNode>>
        ...
        /// </summary>
        private static bool IsSelectedMemberDeclarationAlreadyInDestination(
        ...
        )
        {
            ...
            {
                return IsSelectedMemberDeclarationAlreadyInDestinationInterface(
                    selectedMember,
                    destination
                );
            }
            ...
            {
                return IsSelectedMemberDeclarationAlreadyInDestinationClass(
                    selectedMember,
                    destination
                );
            }
        ...
        private static bool IsSelectedMemberDeclarationAlreadyInDestinationClass(
            ISymbol selectedMember,
            INamedTypeSymbol destination
        )
        {
            ...
            else
            {
                ...
                for (
                    var symbol = selectedMember;
                    symbol != null;
                    symbol = symbol.GetOverriddenMember()
                )
