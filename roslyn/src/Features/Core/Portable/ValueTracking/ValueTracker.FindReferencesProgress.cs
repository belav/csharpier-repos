...
namespace Microsoft.CodeAnalysis.ValueTracking
{
    ...
    {
        private class FindReferencesProgress(OperationCollector valueTrackingProgressCollector)
                ...
                IStreamingProgressTracker
        {
            ...
            )
            {
                ...
                else if (symbol is IPropertySymbol { IsIndexer: true } propertySymbol)
                {
                    ...
                    {
                        foreach (
                            var argument in syntaxFacts.GetArgumentsOfArgumentList(argumentList)
                        )
                        {
                            var argumentOperation = semanticModel.GetOperation(
                                argument,
                                cancellationToken
                            );
                            ...
                            {
                                await _operationCollector
                                    .VisitAsync(argumentOperation, cancellationToken)
                                    .ConfigureAwait(false);
                            }
                        }
                    }
                    ...
                    if (expression is not null)
                    {
                        ...
                        var expressionOperation = semanticModel.GetOperation(
                            expression,
                            cancellationToken
                        );
                        ...
                        {
                            await _operationCollector
                                .VisitAsync(expressionOperation, cancellationToken)
                                .ConfigureAwait(false);
                        }
                    }
                }
