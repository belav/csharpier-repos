...
namespace Microsoft.CodeAnalysis.ConvertTupleToStruct
{
        ...
        where TNamespaceDeclarationSyntax : SyntaxNode
    {
            ...
            )
            {
                ...
                if (
                    !capturedTypeParameters.Any(
                        static tp => tp.TypeParameterKind == TypeParameterKind.Method
                    )
                )
                {
                    ...
                    if (
                        !capturedTypeParameters.Any(
                            static tp => tp.TypeParameterKind == TypeParameterKind.Type
                        )
                    )
                    {
        ...
        private SyntaxNodeOrTokenList ConvertArguments(
            ...
            new(
                list.Select(
                    v => ConvertArgumentOrToken(generator, parameterNamingRule, isRecord, v)
                )
            );
