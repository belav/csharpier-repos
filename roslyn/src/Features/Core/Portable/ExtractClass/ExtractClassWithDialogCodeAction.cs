...
namespace Microsoft.CodeAnalysis.ExtractClass
{
    ...
    ) : CodeActionWithOptions
    {
        ...
        private readonly CodeActionPriority _priority = selectedMembers.IsEmpty
        ...
        public override object? GetOptions(CancellationToken cancellationToken)
        {
            var extractClassService =
                _service
                ?? _document.Project.Solution.Services.GetRequiredService<IExtractClassOptionsService>();
        ...
        )
        {
            ...
            var namespaceService =
                _document.GetRequiredLanguageService<AbstractExtractInterfaceService>();
            ...
            var documentWithTypeDeclaration = solutionWithUpdatedOriginalType.GetRequiredDocument(
                updatedDocument.Id
            );
        ...
        {
            using var _1 = ArrayBuilder<(ISymbol member, bool makeAbstract)>.GetInstance(
            ...
            foreach (var (documentId, symbols) in symbolMapping.DocumentIdsToSymbolMap)
            {
                ...
                var root = await document
                    ...
                    .ConfigureAwait(false);
                using var _3 = ArrayBuilder<ExtractClassMemberAnalysisResult>.GetInstance(
                ...
                var memberAnalysisForDocumentSymbols = remainingResults.Where(analysis =>
                    symbols.Contains(analysis.Member)
                );

                foreach (var memberAnalysis in memberAnalysisForDocumentSymbols)
                {
                    var annotation = symbolMapping.SymbolToDeclarationAnnotationMap[
                        memberAnalysis.Member
                    ];
            ...
            var pullMemberUpOptions = PullMembersUpOptionsBuilder.BuildPullMembersUpOptions(
                newType,
                pullMembersBuilder.ToImmutable()
            );
            var updatedOriginalDocument = solution.GetRequiredDocument(_document.Id);

            return await MembersPuller
                .PullMembersUpAsync(
        ...
        private static async Task<INamedTypeSymbol> GetNewTypeSymbolAsync(
            Document document,
        ...
        )
        {
            ...
            return (INamedTypeSymbol)
                semanticModel.GetRequiredDeclaredSymbol(declarationNode, cancellationToken);
        }
