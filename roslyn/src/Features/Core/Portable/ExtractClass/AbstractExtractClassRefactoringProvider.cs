...
namespace Microsoft.CodeAnalysis.ExtractClass
{
    ...
    ) : CodeRefactoringProvider
    {
        ...
        public override async Task ComputeRefactoringsAsync(CodeRefactoringContext context)
        {
            ...
            var optionsService =
                _optionsService ?? solution.Services.GetService<IExtractClassOptionsService>();
            if (optionsService is null)
            {
                return;
            }

            var (action, hasBaseType) = await TryGetMemberActionAsync(context, optionsService)
            ...
            {
                action = await TryGetClassActionAsync(context, optionsService)
                    .ConfigureAwait(false);
            }
        ...
        private async Task<(
            ExtractClassWithDialogCodeAction? action,
            bool hasBaseType
        ...
        )
        {
            ...
            var containingTypeDeclarationNode = selectedMemberNodes
                .First()
                .FirstAncestorOrSelf<SyntaxNode>(syntaxFacts.IsTypeDeclaration);
            ...
            if (
                selectedMemberNodes.Any(m =>
                    m.FirstAncestorOrSelf<SyntaxNode>(syntaxFacts.IsTypeDeclaration)
        ...
        )
        {
            ...
            if (
                ...
                is not INamedTypeSymbol selectedType
            )
            {
                return null;
            }
