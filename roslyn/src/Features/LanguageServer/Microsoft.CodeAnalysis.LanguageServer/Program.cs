...
)
{
    ...
    using var loggerFactory = LoggerFactory.Create(builder =>
    {
        ...
        builder.AddProvider(
            new LspLogMessageLoggerProvider(
    ...
    using var exportProvider = await ExportProviderBuilder.CreateExportProviderAsync(
        serverConfiguration.ExtensionAssemblyPaths,
        loggerFactory
    );
    ...
    exportProvider
        .GetExportedValue<ServerConfigurationFactory>()
        .InitializeConfiguration(serverConfiguration);

    // Initialize the fault handler if it's available
    var telemetryReporter = exportProvider
        .GetExports<ITelemetryReporter>()
        .SingleOrDefault()
        ?.Value;
    RoslynLogger.Initialize(
        telemetryReporter,
    ...
    StarredCompletionAssemblyHelper.InitializeInstance(
        serverConfiguration.StarredCompletionsPath,
        loggerFactory,
...
static CliRootCommand CreateCommandLineParser()
{
    ...
    {
        Description =
            "Telemetry level, Defaults to 'off'. Example values: 'all', 'crash', 'error', or 'off'.",
        Required = false,
    };
    ...
    rootCommand.SetAction(
        (parseResult, cancellationToken) =>
        {
            ...
            var extensionAssemblyPaths =
                parseResult.GetValue(extensionAssemblyPathsOption) ?? Array.Empty<string>();
