...
internal sealed class RestoreHandler(DotnetCliHelper dotnetCliHelper)
    : ILspServiceRequestHandler<RestoreParams, RestorePartialResult[]>
{
    ...
    public async Task<RestorePartialResult[]> HandleRequestAsync(
        RestoreParams request,
    ...
    )
    {
        ...
        progress.Report(
            new RestorePartialResult(
                LanguageServerResources.Restore,
                LanguageServerResources.Restore_started
            )
        );
        ...
        if (restorePaths.IsEmpty)
        {
            progress.Report(
                new RestorePartialResult(
                    LanguageServerResources.Restore,
                    LanguageServerResources.Nothing_found_to_restore
                )
            );
        ...
        progress.Report(
            new RestorePartialResult(
                LanguageServerResources.Restore,
                $"{LanguageServerResources.Restore_complete}{Environment.NewLine}"
            )
        );
        return progress.GetValues() ?? [];
    }
    ...
    private async Task RestoreAsync(
        ...
        BufferedProgress<RestorePartialResult> progress,
        CancellationToken cancellationToken
    )
    {
        foreach (var path in pathsToRestore)
        {
            ...
            var stageName = string.Format(
                LanguageServerResources.Restoring_0,
                Path.GetFileName(path)
            );
            ReportProgress(
                progress,
                stageName,
                string.Format(LanguageServerResources.Running_dotnet_restore_on_0, path)
            );

            var process = dotnetCliHelper.Run(
                arguments,
                workingDirectory,
            ...
            process.OutputDataReceived += (sender, args) =>
                ReportProgress(progress, stageName, args.Data);
            ...
            if (process.ExitCode != 0)
            {
                ReportProgress(
                    progress,
                    stageName,
                    string.Format(LanguageServerResources.Failed_to_run_restore_on_0, path)
                );
            }
        }
    ...
    private static ImmutableArray<string> GetRestorePaths(
        RestoreParams request,
        Solution solution,
        RequestContext context
    )
    {
        ...
        context.TraceInformation(
            $"Found {projects.Length} restorable projects from {solution.Projects.Count()} projects in solution"
        );
        return projects;
    }
}
