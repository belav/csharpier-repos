    ...
    internal sealed class ExperimentalCapabilitiesProvider : ICapabilitiesProvider
    {
        ...
        )
        {
            _completionProviders = completionProviders
                .Where(lz =>
                    lz.Metadata.Language is LanguageNames.CSharp or LanguageNames.VisualBasic
                )
                .ToImmutableArray();
        }
        ...
        public ServerCapabilities GetCapabilities(ClientCapabilities clientCapabilities)
        {
            ...
            var capabilities = supportsVsExtensions
                ? GetVSServerCapabilities()
                : new VSInternalServerCapabilities();

            var commitCharacters =
                AbstractLspCompletionResultCreationService.DefaultCommitCharactersArray;
