    ...
    internal static class CodeActionHelpers
    {
        ...
        )
        {
            ...
            if (suggestedAction is UnifiedSuggestedActionWithNestedActions unifiedSuggestedActions)
            {
                foreach (var actionSet in unifiedSuggestedActions.NestedActionSets)
                {
                    foreach (var action in actionSet.Actions)
                    {
                        ...
                        if (
                            action.OriginalCodeAction
                            is not AbstractConfigurationActionWithNestedActions
                        )
                        {
                            builder.AddRange(
                                GenerateCodeActions(request, action, codeActionKind, currentTitle)
                            );
                        }
                    }
                }
            ...
            else
            {
                ...
                if (
                    suggestedAction is UnifiedCodeFixSuggestedAction unifiedCodeFixSuggestedAction
                    && unifiedCodeFixSuggestedAction.FixAllFlavors is not null
                )
                {
                    var fixAllFlavors = unifiedCodeFixSuggestedAction.FixAllFlavors.Actions
        ...
        private static LSP.Diagnostic[]? GetApplicableDiagnostics(
        ...
        )
        {
            ...
            foreach (var set in actionSets)
            {
                foreach (var suggestedAction in set.Actions)
                {
                    ...
                    {
                        codeActions.Add(
                            GetFixAllActionsFromActionSet(suggestedAction, fixAllScope)
                        );
                    }
                }
            }
        ...
        private static CodeAction GetFixAllActionsFromActionSet(
            IUnifiedSuggestedAction suggestedAction,
            string? fixAllScope
        )
        {
            ...
            if (
                suggestedAction
                is not UnifiedCodeFixSuggestedAction
                {
                    FixAllFlavors: not null
                } unifiedCodeFixSuggestedAction
            )
            {
                return codeAction;
            }
            ...
            var fixAllFlavor = unifiedCodeFixSuggestedAction.FixAllFlavors.Actions
                .OfType<UnifiedFixAllCodeFixSuggestedAction>()
                .Where(action => action.FixAllState.Scope.ToString() == fixAllScope)
                .First();
            return new FixAllCodeAction(
                string.Format(FeaturesResources.Fix_All_0, codeAction.Title),
                fixAllFlavor.FixAllState,
