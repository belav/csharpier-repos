...
namespace Microsoft.CodeAnalysis.LanguageServer.Handler
{
    ...
    internal class CodeActionResolveHandler : IRequestHandler<LSP.VSCodeAction, LSP.VSCodeAction>
    {
        ...
        )
        {
            ...
            var operations = await codeActionToResolve.GetOperationsAsync(cancellationToken)
                .ConfigureAwait(false);
            ...
            ) where T : TextDocument
            {
                foreach (var docId in changedDocuments)
                {
                    ...
                    var oldText = await oldTextDoc.GetTextAsync(cancellationToken)
                        .ConfigureAwait(false);
                    ...
                    {
                        var newText = await newTextDoc.GetTextAsync(cancellationToken)
                            .ConfigureAwait(false);
