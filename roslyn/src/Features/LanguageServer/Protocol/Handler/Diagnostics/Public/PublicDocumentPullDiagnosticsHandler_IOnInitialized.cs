...
// DocumentDiagnosticReport (aka changed / unchanged) followed
        ...
        IOnInitialized
{
    ...
    {
        // Dynamically register a non-local document diagnostic source if Full solution background analysis
        // is enabled
        // for analyzer execution. This diagnostic source reports diagnostics in open documents that are
        // reported
        // when analyzing other documents or at compilation end.
        if (
            clientCapabilities?.TextDocument?.Diagnostic?.DynamicRegistration is true
            && IsFsaEnabled()
        )
        {
        ...
        bool IsFsaEnabled()
        {
            ...
            {
                if (
                    GlobalOptions.GetBackgroundAnalysisScope(language)
