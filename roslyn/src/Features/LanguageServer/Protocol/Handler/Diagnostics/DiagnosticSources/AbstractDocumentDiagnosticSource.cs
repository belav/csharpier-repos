    ...
    where TDocument : TextDocument
{
    private static readonly ImmutableArray<string> s_todoCommentCustomTags = ImmutableArray.Create(
        PullDiagnosticConstants.TaskItemCustomTag
    );
    ...
    {
        var taskListItems = IncludeTaskListItems
            ? await this.GetTaskListDiagnosticsAsync(cancellationToken).ConfigureAwait(false)
            : ImmutableArray<DiagnosticData>.Empty;
        var diagnostics = IncludeStandardDiagnostics
            ? await this.GetDiagnosticsWorkerAsync(
    ...
    private async Task<ImmutableArray<DiagnosticData>> GetTaskListDiagnosticsAsync(
        CancellationToken cancellationToken
    )
    {
        ...
        var tokenList = document.Project.Solution.Options.GetOption(
