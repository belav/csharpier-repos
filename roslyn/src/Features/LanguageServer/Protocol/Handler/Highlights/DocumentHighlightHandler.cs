...
namespace Microsoft.CodeAnalysis.LanguageServer.Handler
{
    ...
    internal class DocumentHighlightsHandler
        : ILspServiceDocumentRequestHandler<TextDocumentPositionParams, DocumentHighlight[]?>
    {
        ...
        public async Task<DocumentHighlight[]?> HandleRequestAsync(
            TextDocumentPositionParams request,
        ...
        )
        {
            ...
            var referenceHighlights = await GetReferenceHighlightsAsync(
                    document,
                    text,
                    position,
                    cancellationToken
        ...
        private async Task<ImmutableArray<DocumentHighlight>> GetKeywordHighlightsAsync(
            Document document,
            SourceText text,
            int position,
            CancellationToken cancellationToken
        )
        {
            ...
            return keywordSpans.SelectAsArray(
                highlight =>
        ...
        private async Task<ImmutableArray<DocumentHighlight>> GetReferenceHighlightsAsync(
            Document document,
            SourceText text,
            int position,
            CancellationToken cancellationToken
        )
        {
            ...
            var highlights = await documentHighlightService
            ...
            if (!highlights.IsDefaultOrEmpty)
            {
                ...
                return highlightsForDocument.HighlightSpans.SelectAsArray(
                    h =>
                        new DocumentHighlight
                        {
