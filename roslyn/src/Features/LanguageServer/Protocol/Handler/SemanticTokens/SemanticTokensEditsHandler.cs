...
namespace Microsoft.CodeAnalysis.LanguageServer.Handler.SemanticTokens
{
          ...
          >
    {
        ...
        )
        {
            ...
            var edits = LongestCommonSemanticTokensSubsequence.GetEdits(
                oldGroupedSemanticTokens,
                ...
                switch (edit.Kind)
                {
                    case EditKind.Insert:
                        ...
                        Contract.ThrowIfTrue(
                            editKindWithoutInsert == SemanticTokenEditKind.Insert,
                            "There cannot be two inserts at the same position."
                        );
                        Contract.ThrowIfTrue(
                            editKindWithoutInsert == SemanticTokenEditKind.Update,
                            "There cannot be an insert and update at the same position."
                        );
                        indexToEditKinds[edit.NewIndex] =
                            editKindWithoutInsert == SemanticTokenEditKind.None
                                ? SemanticTokenEditKind.Insert
                                : SemanticTokenEditKind.Update;
                        break;
                    case EditKind.Delete:
                        indexToEditKinds.TryGetValue(edit.OldIndex, out var editKindWithoutDelete);
                        Contract.ThrowIfTrue(
                            editKindWithoutDelete == SemanticTokenEditKind.Delete,
                            "There cannot be two deletions at the same position."
                        );
                        Contract.ThrowIfTrue(
                            editKindWithoutDelete == SemanticTokenEditKind.Update,
                            "There cannot be a deletion and update at the same position."
                        );
                        indexToEditKinds[edit.OldIndex] =
                            editKindWithoutDelete == SemanticTokenEditKind.None
        ...
        )
        {
            ...
            )
            {
                ...
                else
                {
                    Contract.ThrowIfFalse(
                        initialEditKind == SemanticTokenEditKind.Delete,
                        "Expected initialEditKind to be SemanticTokenEditKind.Delete."
                    );
            ...
            )
            {
                ...
                semanticTokensEdits.Add(
                    GenerateEdit(
                        start: editStartPosition,
                        deleteCount: deleteCount,
                        data: tokensToInsert.ToArray()
                    )
                );
                return endEditIndex;
            }
        ...
        private static SemanticToken[] ConvertToGroupedSemanticTokens(int[] tokens)
        {
            Contract.ThrowIfTrue(
                tokens.Length % 5 != 0,
                $"Tokens length should be divisible by 5. Actual length: {tokens.Length}"
            );
        ...
        internal static LSP.SemanticTokensEdit GenerateEdit(
            int start,
            int deleteCount,
            int[] data
            ...
            : LongestCommonSubsequence<SemanticToken[]>
        {
            ...
            protected override bool ItemsEqual(
                SemanticToken[] oldSemanticTokens,
                int oldIndex,
                SemanticToken[] newSemanticTokens,
                int newIndex
            ...
            public static IEnumerable<SequenceEdit> GetEdits(
                ...
                s_instance.GetEdits(
                    oldSemanticTokens,
                    oldSemanticTokens.Length,
                    newSemanticTokens,
                    newSemanticTokens.Length
                );
        }
