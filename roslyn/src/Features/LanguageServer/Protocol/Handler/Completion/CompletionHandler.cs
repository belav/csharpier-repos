    ...
    internal class CompletionHandler : IRequestHandler<LSP.CompletionParams, LSP.CompletionList?>
    {
        ...
        )
        {
            _csharpTriggerCharacters = completionProviders.Where(
                ...
                .SelectMany(lz => GetTriggerCharacters(lz.Value))
                .ToImmutableHashSet();
            _vbTriggerCharacters = completionProviders.Where(
                lz => lz.Metadata.Language == LanguageNames.VisualBasic
            )
                .SelectMany(lz => GetTriggerCharacters(lz.Value))
                .ToImmutableHashSet();
        ...
        )
        {
            ...
            var list = await completionService.GetCompletionsAsync(
                document,
                position,
                completionTrigger,
            ...
            var lspVSClientCapability =
                context.ClientCapabilities.HasVisualStudioLspCapability() == true;
            ...
            var featureFlagService = context.Solution.Workspace.Services
            ...
            var returnTextEdits =
                ...
                || completionOptions.GetOption(
                    CompletionOptions.ForceRoslynLSPCompletionExperiment,
            ...
            )
            {
                if (useVSCompletionItem)
                {
                    var vsCompletionItem = await CreateCompletionItemAsync<LSP.VSCompletionItem>(
                        ...
                        .ConfigureAwait(false);
                    vsCompletionItem.Icon = new ImageElement(
                        item.Tags.GetFirstGlyph().GetImageId()
                    );
            ...
            ) where TCompletionItem : LSP.CompletionItem, new()
            {
                ...
                var completionItem = new TCompletionItem
                {
                    ...
                    Data = new CompletionResolveData { ResultId = resultId, },
                    Preselect =
                        item.Rules.SelectionBehavior
            ...
            )
            {
                ...
                var commitCharacterSet = commitCharacters.Select(c => c.ToString())
                    .ToImmutableArray();
        ...
        )
        {
            ...
            var documentOptions = await document.GetOptionsAsync(cancellationToken)
                .ConfigureAwait(false);
            var completionOptions = documentOptions.WithChangedOption(
        ...
        private static LSP.CompletionItemKind GetCompletionKind(ImmutableArray<string> tags)
        {
            ...
            {
                if (
                    ProtocolConversions.RoslynTagToCompletionItemKind
        ...
        internal readonly struct TestAccessor
        {
            ...
            public CompletionListCache GetCache() => _completionHandler._completionListCache;
        }
    }
}
