...
namespace Microsoft.CodeAnalysis.LanguageServer.Handler
{
        ...
        : ILspServiceDocumentRequestHandler<LSP.CompletionParams, LSP.CompletionList?>
    {
        ...
        )
        {
            ...
            var capabilityHelper = new CompletionCapabilityHelper(
            ...
            if (
                request.Context?.TriggerKind
                    is not LSP.CompletionTriggerKind.TriggerForIncompleteCompletions
        ...
        )
        {
            ...
            if (
                !capabilityHelper.SupportVSInternalClientCapabilities
                && position < completionList.Span.End
            )
            {
                var defaultSpan = new TextSpan(
                    completionList.Span.Start,
                    length: position - completionList.Span.Start
                );
                completionList = completionList.WithSpan(defaultSpan);
            }

            var completionListMaxSize = _globalOptions.GetOption(
                LspOptionsStorage.MaxCompletionListSize
            );
            var (filteredCompletionList, isIncomplete) = FilterCompletionList(
                completionList,
        ...
        )
        {
            ...
            var resultId = completionListCache.UpdateCache(
                new CompletionListCache.CacheEntry(request.TextDocument, completionList)
            );
        ...
        {
            if (
                completionListMaxSize < 0
                || completionListMaxSize >= completionList.ItemsList.Count
            )
        ...
        )
        {
            ...
            {
                var updateImportCompletionCacheInBackground =
                    options.ShowItemsFromUnimportedNamespaces is true;
                options = options with
                {
                    ShowNewSnippetExperienceUserOption = false,
                    UpdateImportCompletionCacheInBackground =
                        updateImportCompletionCacheInBackground,
                };
            }
