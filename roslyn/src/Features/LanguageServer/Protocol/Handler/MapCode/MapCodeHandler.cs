    ...
    )
    {
        ...
        if (request.Updates is not null)
        {
            throw new NotImplementedException(
                "mapCode Request failed: additional workspace 'Update' is currently not supported"
            );
        }
        ...
        if (
            context.GetRequiredClientCapabilities().Workspace?.WorkspaceEdit?.DocumentChanges
            is true
        )
        {
        ...
        else
        {
            return new WorkspaceEdit
            {
                Changes = uriToEditsMap.ToDictionary(
                    kvp => ProtocolConversions.GetDocumentFilePathFromUri(kvp.Key),
                    kvp => kvp.Value
                ),
            };
        }
        ...
        {
            var textDocument =
                codeMapping.TextDocument
                ?? throw new ArgumentException(
                    $"mapCode sub-request failed: MapCodeMapping.TextDocument not expected to be null."
                );

            if (context.Solution.GetDocument(textDocument) is not Document document)
                throw new ArgumentException(
                    $"mapCode sub-request for {textDocument.Uri} failed: can't find this document in the workspace."
                );
            ...
            var textEdits = textChanges
                .Value.Select(change => ProtocolConversions.TextChangeToTextEdit(change, oldText))
                .ToArray();
        ...
        async Task<
            ImmutableArray<(Document, TextSpan)>
        > ConvertFocusLocationsToDocumentAndSpansAsync(
            Document document,
        ...
        )
        {
            ...
            foreach (var locationsOfSamePriority in focusLocations)
            {
                foreach (var location in locationsOfSamePriority)
                {
                    ...
                    builder.Add(
                        (document, ProtocolConversions.RangeToTextSpan(location.Range, focusText))
                    );
                }
            }
