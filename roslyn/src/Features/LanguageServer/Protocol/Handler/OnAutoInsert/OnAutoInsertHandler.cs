...
namespace Microsoft.CodeAnalysis.LanguageServer.Handler
{
          ...
          >
    {
        ...
        )
        {
            ...
            var postReturnEdit = await service
                .GetTextChangeAfterReturnAsync(context, documentOptions, cancellationToken)
                .ConfigureAwait(false);
        ...
        private async Task<(IBraceCompletionService Service, BraceCompletionContext Context)?> GetBraceCompletionContextAsync(
            int caretLocation,
            Document document,
            CancellationToken cancellationToken
        )
        {
            ...
            {
                var context = await service
                    .GetCompletedBraceContextAsync(document, caretLocation, cancellationToken)
                    .ConfigureAwait(false);
                if (context != null)
                {
