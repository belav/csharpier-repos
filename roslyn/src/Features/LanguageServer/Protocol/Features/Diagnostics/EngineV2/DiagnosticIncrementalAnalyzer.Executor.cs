    ...
    internal partial class DiagnosticIncrementalAnalyzer
    {
        ...
        )
        {
            ...
            )
            {
                try
                {
                    var diagnostics = await executor
                        .ComputeDiagnosticsAsync(analyzer, cancellationToken)
                    ...
                    Logger.Log(
                        functionId,
                        (t, d, a, ds) =>
                            $"{GetDocumentLogMessage(t, d, a)}, {string.Join(Environment.NewLine, ds)}",
                        title,
                        document,
                        analyzer,
                        diagnostics
            ...
            )
            {
                try
                {
                    ...
                    var version = await GetDiagnosticVersionAsync(project, cancellationToken)
                        .ConfigureAwait(false);
                    var existingData = await ProjectAnalysisData
                        .CreateAsync(project, stateSets, avoidLoadingData, cancellationToken)
                        .ConfigureAwait(false);
                    ...
                    if (
                        ...
                        && !CompilationHasOpenFileOnlyAnalyzers(
                            compilationWithAnalyzers,
                            ideOptions.CleanupOptions?.SimplifierOptions
                        )
                    )
                    {
                        return existingData;
                    ...
                    var fullAnalysisEnabled = GlobalOptions.IsFullSolutionAnalysisEnabled(
                        project.Language,
                    ...
                    if (!fullAnalysisEnabled)
                    {
                        ...
                        foreach (
                            var openDocumentId in project.Solution.Workspace.GetOpenDocumentIds()
                        )
                        {
                            if (
                                openDocumentId.ProjectId == project.Id
                                || projectDependencyGraph.DoesProjectTransitivelyDependOnProject(
                                    openDocumentId.ProjectId,
                                    project.Id
                                )
                            )
                            {
                        ...
                        var results = ImmutableDictionary<
                            DiagnosticAnalyzer,
        ...
        )
        {
            ...
            Logger.Log(
                FunctionId.Diagnostics_ProjectDiagnostic,
                p => $"Failed to Load Successfully ({p.FilePath ?? p.Name})",
                project
            );
        ...
        )
        {
            try
            {
                ...
                if (compilationWithAnalyzers?.Analyzers.Length > 0)
                {
                    ...
                    var resultMap = await _diagnosticAnalyzerRunner
                        .AnalyzeProjectAsync(
                            project,
                            compilationWithAnalyzers,
                            forcedAnalysis,
        ...
        )
        {
            try
            {
                ...
                var ideAnalyzers = stateSets
                    .Select(s => s.Analyzer)
                    .Where(a => a is ProjectDiagnosticAnalyzer or DocumentDiagnosticAnalyzer)
                ...
                )
                {
                    ...
                    var compilationWithReducedAnalyzers =
                        (analyzersToRun.Length == 0)
                            ? null
                            : await DocumentAnalysisExecutor
        ...
        )
        {
            try
            {
                ...
                (result, var failedDocuments) =
                    await UpdateWithDocumentLoadAndGeneratorFailuresAsync(
                            result,
                            project,
                            version,
                            cancellationToken
                ...
                foreach (var analyzer in ideAnalyzers)
                {
                    ...
                    result = result.SetItem(
                        analyzer,
                        DiagnosticAnalysisResult.CreateFromBuilder(builder)
                    );
                }
        ...
        private static async Task<(
            ...
            ImmutableHashSet<Document>? failedDocuments
        ...
        )
        {
            ...
            results = results.SetItem(
                ...
                DiagnosticAnalysisResult.Create(
                    ...
                    semanticLocalMap: ImmutableDictionary<
                        ...
                        ImmutableArray<DiagnosticData>
                    >.Empty,
                    nonLocalMap: ImmutableDictionary<
                        DocumentId,
                        ImmutableArray<DiagnosticData>
                    >.Empty,
            ...
            var generatorDiagnostics = await InProcOrRemoteHostAnalyzerRunner
        ...
        )
        {
            foreach (var (analyzer, telemetryInfo) in telemetry)
            {
                var isTelemetryCollectionAllowed =
                    DiagnosticAnalyzerInfoCache.IsTelemetryCollectionAllowed(analyzer);
                _telemetry.UpdateAnalyzerActionsTelemetry(
                    analyzer,
                    telemetryInfo,
