    ...
    internal partial class DiagnosticIncrementalAnalyzer
    {
        ...
        )
        {
            try
            {
                ...
                foreach (var stateSet in stateSets)
                {
                    ...
                    {
                        PersistAndRaiseDiagnosticsIfNeeded(
                            existingData.Value,
                            stateSet.Analyzer,
                            activeFileState
                        );
        ...
        )
        {
            ...
            if (!compilerFullSolutionAnalysisEnabled)
            {
                ...
                stateSets = stateSets.WhereAsArray(
                    s => !s.Analyzer.IsCompilerAnalyzer() || !s.FromBuild(project.Id)
                );
            }
            ...
            return stateSets.WhereAsArray(
                s => IsCandidateForFullSolutionAnalysis(s.Analyzer, project)
            );
        }
        ...
        )
        {
            ...
            AnalyzerService.RaiseBulkDiagnosticsUpdated(async raiseEvents =>
            {
                foreach (var stateSet in stateSets)
                {
                    ...
                    {
                        // add new diagnostics
                        await RaiseProjectDiagnosticsCreatedAsync(
                                project,
                                stateSet,
                                oldAnalysisResult,
                                newAnalysisResult,
        ...
        )
        {
            RaiseDocumentDiagnosticsIfNeeded(
                ...
                oldItems,
                newItems,
                AnalyzerService.RaiseDiagnosticsUpdated,
                forceUpdate: false
            );
        }
        ...
        private async Task RaiseProjectDiagnosticsCreatedAsync(
            Project project,
            StateSet stateSet,
