...
namespace Microsoft.CodeAnalysis.Diagnostics.EngineV2
{
    internal partial class DiagnosticIncrementalAnalyzer
    {
        ...
        public Task AnalyzeNonSourceDocumentAsync(
        ...
        )
        {
            try
            {
                ...
                var version = await GetDiagnosticVersionAsync(document.Project, cancellationToken)
                    .ConfigureAwait(false);
                var backgroundAnalysisScope = GlobalOptions.GetBackgroundAnalysisScope(
                    document.Project.Language
                );
                var compilerDiagnosticsScope = GlobalOptions.GetBackgroundCompilerAnalysisScope(
                    document.Project.Language
                );
                ...
                var isVisibleDocument = _documentTrackingService
                    .GetVisibleDocuments()
                ...
                )>.GetInstance(out var nonCachedAnalyzersAndStates);
                using var argsBuilder = TemporaryArray<DiagnosticsUpdatedArgs>.Empty;
                foreach (var stateSet in stateSets)
                {
                    var (activeFileState, existingData) = TryGetCachedDocumentAnalysisData(
                        document,
                        stateSet,
                        kind,
                        version,
                        ...
                        isVisibleDocument,
                        isOpenDocument,
                        isGeneratedRazorDocument,
                        cancellationToken,
                        out var isAnalyzerSuppressed
                    );
                ...
                if (nonCachedAnalyzersAndStates.Count > 0)
                {
                    var analysisScope = new DocumentAnalysisScope(
                        document,
                        span: null,
                        nonCachedAnalyzersAndStates.SelectAsArray(s => s.analyzer),
                        kind
                    );
            ...
            void PersistAndAddDiagnosticsArgsIfNeeded(
                ref TemporaryArray<DiagnosticsUpdatedArgs> builder,
                DocumentAnalysisData result,
                DiagnosticAnalyzer analyzer,
                ActiveFileState state
            )
            {
                ...
                {
                    AddDocumentDiagnosticsArgsIfNeeded(
                        ref builder,
                        document,
                        analyzer,
                        kind,
        ...
        {
            await AnalyzeProjectAsync(project, forceAnalyzerRun: false, cancellationToken)
                .ConfigureAwait(false);
        }

        public Task ForceAnalyzeProjectAsync(
            Project project,
            CancellationToken cancellationToken
            ...
            try
            {
                ...
                var activeAnalyzers = stateSets
                    ...
                    .Where(a =>
                        (
                            forceAnalyzerRun
                            || DocumentAnalysisExecutor.IsAnalyzerEnabledForProject(
                                a,
                                project,
                                GlobalOptions
                            )
                ...
                {
                    compilationWithAnalyzers = await DocumentAnalysisExecutor
                ...
                foreach (var stateSet in stateSets)
                {
                    ...
                    {
                        await state
                            .SaveToInMemoryStorageAsync(project, analyzerResult)
                            .ConfigureAwait(false);
                ...
                if (analyzedStateSetsBuilder.Count > 0)
                {
                    ...
                    RaiseProjectDiagnosticsIfNeeded(
                        project,
                        analyzedStateSetsBuilder.ToImmutable(),
                        oldResult,
                        result.Result
                    );
                }
        ...
        public Task NonSourceDocumentOpenAsync(
            TextDocument document,
            CancellationToken cancellationToken
        ) => TextDocumentOpenAsync(document, cancellationToken);
        ...
        public Task NonSourceDocumentCloseAsync(
            TextDocument document,
            CancellationToken cancellationToken
        ) => TextDocumentCloseAsync(document, cancellationToken);
            ...
            )
            {
                ...
                foreach (var stateSet in stateSets)
                    documentHadDiagnostics |= await stateSet
                        .OnDocumentClosedAsync(document, GlobalOptions)
                        .ConfigureAwait(false);
        ...
        private void RaiseDiagnosticsRemovedIfRequiredForClosedOrResetDocument(
            TextDocument document,
        ...
        )
        {
            ...
            // This is done for Perf to reduce raising events unnecessarily.
            var _ = GlobalOptions.IsFullSolutionAnalysisEnabled(
                document.Project.Language,
                out var compilerFullAnalysisEnabled,
                out var analyzersFullAnalysisEnabled
            );
            if (compilerFullAnalysisEnabled && analyzersFullAnalysisEnabled)
                return;

            var removeDiagnosticsOnDocumentClose = GlobalOptions.GetOption(
        ...
        public async Task ActiveDocumentSwitchedAsync(
            TextDocument document,
            CancellationToken cancellationToken
        )
        {
        ...
        public Task RemoveDocumentAsync(DocumentId documentId, CancellationToken cancellationToken)
        {
            using (
                Logger.LogBlock(
                    FunctionId.Diagnostics_RemoveDocument,
                    GetRemoveLogMessage,
                    documentId,
        ...
        public Task RemoveProjectAsync(ProjectId projectId, CancellationToken cancellation)
        {
            using (
                Logger.LogBlock(
                    FunctionId.Diagnostics_RemoveProject,
                    GetRemoveLogMessage,
                    projectId,
                    CancellationToken.None
                )
        ...
        )
        {
            ...
            // until next build.
            _ = GlobalOptions.IsFullSolutionAnalysisEnabled(
                project.Language,
            ...
            if (!compilerFullSolutionAnalysisEnabled)
            {
                ...
                stateSets = stateSets.WhereAsArray(s =>
                    !s.Analyzer.IsCompilerAnalyzer() || !s.FromBuild(project.Id)
                );
            }
            ...
            return stateSets.WhereAsArray(s =>
                IsCandidateForFullSolutionAnalysis(s.Analyzer, project)
            );
        }
        ...
        )
        {
            ...
            return descriptors.Any(
                static (d, arg) =>
                    d.GetEffectiveSeverity(
                        arg.CompilationOptions,
                        arg.analyzerConfigOptions?.AnalyzerOptions,
        ...
        )
        {
            ...
            AnalyzerService.RaiseBulkDiagnosticsUpdated(async raiseEvents =>
            {
                ...
                foreach (var stateSet in stateSets)
                {
                    ...
                    {
                        // add new diagnostics
                        argsBuilder.AddRange(
                            await CreateProjectDiagnosticsCreatedArgsAsync(
                                    project,
                                    stateSet,
                                    oldAnalysisResult,
                                    newAnalysisResult,
        ...
        private async Task<
            ImmutableArray<DiagnosticsUpdatedArgs>
        > CreateProjectDiagnosticsCreatedArgsAsync(
            Project project,
            StateSet stateSet,
        ...
        )
        {
            ...
            AddDiagnosticsCreatedArgs(
                ref argsBuilder.AsRef(),
                project,
                stateSet.Analyzer,
                newAnalysisResult.GetOtherDiagnostics()
            );

            return argsBuilder.ToImmutableAndClear();
        }

        private void AddProjectDiagnosticsRemovedArgs(
            ref TemporaryArray<DiagnosticsUpdatedArgs> builder,
            StateSet stateSet,
            ProjectId projectId,
