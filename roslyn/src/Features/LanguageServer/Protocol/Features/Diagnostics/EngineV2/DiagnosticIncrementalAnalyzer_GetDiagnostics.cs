...
namespace Microsoft.CodeAnalysis.Diagnostics.EngineV2
{
    internal partial class DiagnosticIncrementalAnalyzer
    {
        ...
        )
        {
            ...
            var (documentId, projectId) =
                (argsId.ProjectOrDocumentId is DocumentId docId)
                    ? (docId, docId.ProjectId)
                    : (null, (ProjectId)argsId.ProjectOrDocumentId);
            return new IdeCachedDiagnosticGetter(
                this,
                solution,
                projectId,
                documentId,
                includeSuppressedDiagnostics
            ).GetSpecificDiagnosticsAsync(argsId.Analyzer, argsId.Kind, cancellationToken);
        }
        ...
        private abstract class DiagnosticGetter
        {
            ...
            protected ImmutableArray<DiagnosticData> GetDiagnosticData() =>
                (_lazyDataBuilder != null)
                    ? _lazyDataBuilder.ToImmutableArray()
                    : ImmutableArray<DiagnosticData>.Empty;

            protected abstract Task AppendDiagnosticsAsync(
                Project project,
            ...
            protected void AppendDiagnostics(ImmutableArray<DiagnosticData> items)
            {
                ...
                if (_lazyDataBuilder == null)
                {
                    Interlocked.CompareExchange(
                        ref _lazyDataBuilder,
                        ImmutableArray.CreateBuilder<DiagnosticData>(),
                        null
                    );
                }
                ...
                {
                    _lazyDataBuilder.AddRange(
                        items
                            .Where(ShouldIncludeSuppressedDiagnostic)
        ...
        private sealed class IdeCachedDiagnosticGetter : DiagnosticGetter
        {
            ...
            > GetProjectStateDiagnosticsAsync(
                StateSet stateSet,
                Project project,
                DocumentId? documentId,
                AnalysisKind kind,
            ...
            )
            {
                ...
                if (documentId != null)
                {
                    ...
                    var document = await project.Solution
                        .GetDocumentAsync(
                            documentId,
                            includeSourceGenerated: project.Solution.Workspace.Services
                    ...
                    var result = await state
                        .GetAnalysisDataAsync(document, avoidLoadingData: false, cancellationToken)
                        .ConfigureAwait(false);
                    return result.GetDocumentDiagnostics(documentId, kind);
                }

                Contract.ThrowIfFalse(kind == AnalysisKind.NonLocal);
                var nonLocalResult = await state
        ...
        private sealed class IdeLatestDiagnosticGetter : DiagnosticGetter
        {
            ...
            )
            {
                ...
                var stateSets = StateManager
                    .GetOrCreateStateSets(project)
                ...
                foreach (var stateSet in stateSets)
                {
                    ...
                    {
                        AppendDiagnostics(
                            analysisResult.GetDocumentDiagnostics(documentId, AnalysisKind.Syntax)
                        );
                        AppendDiagnostics(
                            analysisResult.GetDocumentDiagnostics(documentId, AnalysisKind.Semantic)
                        );
                        AppendDiagnostics(
                            analysisResult.GetDocumentDiagnostics(documentId, AnalysisKind.NonLocal)
                        );
                    }
