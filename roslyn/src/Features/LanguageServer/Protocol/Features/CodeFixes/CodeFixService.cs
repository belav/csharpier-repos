...
namespace Microsoft.CodeAnalysis.CodeFixes
{
    ...
    internal partial class CodeFixService : ICodeFixService
    {
        ...
        private readonly ImmutableDictionary<
            string,
            ImmutableArray<Lazy<CodeFixProvider, CodeChangeProviderMetadata>>
        > _fixersPerLanguageMap;
        ...
        private ImmutableDictionary<object, FixAllProviderInfo?> _fixAllProviderMap =
            ImmutableDictionary<object, FixAllProviderInfo?>.Empty;
        private ImmutableDictionary<
            CodeFixProvider,
            CodeChangeProviderMetadata?
        > _fixerToMetadataMap = ImmutableDictionary<
            CodeFixProvider,
            CodeChangeProviderMetadata?
        >.Empty;
        ...
        )
        {
            ...
            if (
                !(
                    ...
                    is CodeActionRequestPriority.Default
                        or CodeActionRequestPriority.Low
                )
            )
                return null;
        ...
        )
        {
            using var _ = TelemetryLogging.LogBlockTimeAggregated(
                FunctionId.CodeFix_Summary,
                $"Pri{priorityProvider.Priority.GetPriorityInt()}.{nameof(GetMostSevereFixAsync)}"
            );
            ...
            using (
                TelemetryLogging.LogBlockTimeAggregated(
                    FunctionId.CodeFix_Summary,
                    $"Pri{priorityProvider.Priority.GetPriorityInt()}.{nameof(GetMostSevereFixAsync)}.{nameof(_diagnosticService.GetDiagnosticsForSpanAsync)}"
                )
            )
            {
                (allDiagnostics, upToDate) = await _diagnosticService
                    .TryGetDiagnosticsForSpanAsync(
                        document,
                        range,
                        GetShouldIncludeDiagnosticPredicate(document, priorityProvider),
            ...
            var errorFixTask = Task.Run(
                () => GetFirstFixAsync(spanToErrorDiagnostics, cancellationToken),
                cancellationToken
            );
            var otherFixTask = Task.Run(
                () => GetFirstFixAsync(spanToOtherDiagnostics, linkedToken),
                linkedToken
            );
            ...
            var collection =
                await errorFixTask.ConfigureAwait(false)
            ...
            {
                await foreach (
                    var collection in StreamFixesAsync(
                            document,
                            spanToDiagnostics,
        ...
        )
        {
            ...
            var includeSuppressionFixes =
                priorityProvider.Priority is null or CodeActionRequestPriority.Lowest;
            ...
            using (
                TelemetryLogging.LogBlockTimeAggregated(
                    FunctionId.CodeFix_Summary,
                    $"Pri{priorityProvider.Priority.GetPriorityInt()}.{nameof(_diagnosticService.GetDiagnosticsForSpanAsync)}"
                )
            )
            {
                diagnostics = await _diagnosticService
                    .GetDiagnosticsForSpanAsync(
                        ...
                        includeSuppressedDiagnostics: includeSuppressionFixes,
                        priorityProvider,
                        addOperationScope,
                        DiagnosticKind.All,
                        isExplicit: true,
            ...
            if (
                document.Project.Solution.WorkspaceKind != WorkspaceKind.Interactive
            ...
            )
            {
                ...
                using var _2 = PooledHashSet<string>.GetInstance(
                    out var registeredConfigurationFixTitles
                );
                foreach (var (span, diagnosticList) in spanToDiagnostics)
                {
                    await foreach (
                        var codeFixCollection in StreamConfigurationFixesAsync(
                                document,
                                span,
                                diagnosticList,
        ...
        )
        {
            ...
            using (
                TelemetryLogging.LogBlockTimeAggregated(
                    FunctionId.CodeFix_Summary,
                    $"{nameof(GetDocumentFixAllForIdInSpanAsync)}.{nameof(_diagnosticService.GetDiagnosticsForSpanAsync)}"
                )
            )
            {
                diagnostics = await _diagnosticService
            ...
            await foreach (
                var collection in StreamFixesAsync(
                        ...
                        fallbackOptions,
                        addOperationScope: static _ => null,
                        cancellationToken
                    )
                    .ConfigureAwait(false)
            )
            {
                if (
                    collection.FixAllState is not null
                    && collection.SupportedScopes.Contains(FixAllScope.Document)
                )
                {
        ...
        public Task<TDocument> ApplyCodeFixesForSpecificDiagnosticIdAsync<TDocument>(
            TDocument document,
            string diagnosticId,
        ...
        private bool TryGetWorkspaceFixersMap(
            TextDocument document,
            [NotNullWhen(true)]
                out ImmutableDictionary<DiagnosticId, ImmutableArray<CodeFixProvider>>? fixerMap
        )
        {
            ...
            if (
                !_lazyWorkspaceFixersMap.TryGetValue(
                    document.Project.Language,
                    out var lazyFixerMap
                )
            )
            {
            ...
            {
                var filteredFixers = ProjectCodeFixProvider.FilterExtensions(
                    document,
                    fixers,
                    GetExtensionInfo
                );
        ...
        )
        {
            ...
            {
                var fixersPriorityByLanguageMap = GetFixerPriorityPerLanguageMap(
        ...
        )
        {
            ...
            using var _1 = PooledDictionary<
                CodeFixProvider,
                List<(TextSpan range, List<DiagnosticData> diagnostics)>
            >.GetInstance(out var fixerToRangesAndDiagnostics);
            ...
            foreach (var (range, diagnostics) in spanToDiagnostics)
            {
                ...
                foreach (var diagnosticId in diagnostics.Select(d => d.Id).Distinct())
                {
                    ...
                    if (
                        hasAnySharedFixer
                        && fixerMap!.TryGetValue(diagnosticId, out var workspaceFixers)
                    )
                    {
                        if (isInteractive)
                        {
            ...
            try
            {
                foreach (var fixer in allFixers)
                {
                    ...
                    foreach (var (span, diagnostics) in fixerToRangesAndDiagnostics[fixer])
                    {
                        ...
                        using var _ = TelemetryLogging.LogBlockTime(
                            FunctionId.CodeFix_Delay,
                            logMessage,
                            CodeFixTelemetryDelay
                        );
            ...
            if (fixAllProviderInfo != null && textDocument is Document document)
            {
                ...
                var diagnosticProvider = fixAllForInSpan
                    ...
                    : (FixAllContext.DiagnosticProvider)
                        new FixAllDiagnosticProvider(_diagnosticService, diagnosticIds);
        ...
        private ImmutableDictionary<
            LanguageKind,
            Lazy<ImmutableDictionary<DiagnosticId, ImmutableArray<CodeFixProvider>>>
        > GetFixerPerLanguageMap(SolutionServices services)
        {
            ...
            foreach (var (diagnosticId, lazyFixers) in _fixersPerLanguageMap)
            {
                ...
                >(
                    () =>
                    {
                        ...
                        return mutableMap.ToImmutableDictionary(
                            kvp => kvp.Key,
        ...
        )
        {
            ...
            var configurationFixerMap = ImmutableDictionary.CreateBuilder<
                LanguageKind,
                Lazy<ImmutableArray<IConfigurationFixProvider>>
            >();
        ...
        private ImmutableDictionary<
            LanguageKind,
            Lazy<ImmutableDictionary<CodeFixProvider, int>>
        > GetFixerPriorityPerLanguageMap(SolutionServices services)
        {
        ...
        private ImmutableDictionary<DiagnosticId, ImmutableArray<CodeFixProvider>> GetProjectFixers(
            TextDocument document
        )
        {
            // TODO (https://github.com/dotnet/roslyn/issues/4932): Don't restrict CodeFixes in Interactive
        ...
        private static ProjectCodeFixProvider.ExtensionInfo GetExtensionInfo(
        ...
        private sealed class FixerComparer : IComparer<CodeFixProvider>
        {
            ...
            {
                _fixerToIndex = allFixers
                    .Select((fixer, index) => (fixer, index))
                    .ToDictionary(t => t.fixer, t => t.index);
            ...
            public int Compare([AllowNull] CodeFixProvider x, [AllowNull] CodeFixProvider y)
            {
                ...
                if (
                    _priorityMap.TryGetValue(x, out var xOrder)
