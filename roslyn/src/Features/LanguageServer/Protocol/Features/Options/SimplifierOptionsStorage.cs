...
internal static class SimplifierOptionsStorage
{
    public static ValueTask<SimplifierOptions> GetSimplifierOptionsAsync(
        ...
        document.GetSimplifierOptionsAsync(
            globalOptions.GetSimplifierOptions(document.Project.LanguageServices),
            cancellationToken
        );
    ...
    public static SimplifierOptions.CommonOptions GetCommonSimplifierOptions(
        this IGlobalOptionService globalOptions,
        string language
    ) =>
        new()
        {
            QualifyFieldAccess = globalOptions.GetOption(
                CodeStyleOptions2.QualifyFieldAccess,
                language
            ),
            QualifyPropertyAccess = globalOptions.GetOption(
                CodeStyleOptions2.QualifyPropertyAccess,
                language
            ),
            QualifyMethodAccess = globalOptions.GetOption(
                CodeStyleOptions2.QualifyMethodAccess,
                language
            ),
            QualifyEventAccess = globalOptions.GetOption(
                CodeStyleOptions2.QualifyEventAccess,
                language
            ),
            PreferPredefinedTypeKeywordInMemberAccess = globalOptions.GetOption(
                CodeStyleOptions2.PreferIntrinsicPredefinedTypeKeywordInMemberAccess,
                language
            ),
            PreferPredefinedTypeKeywordInDeclaration = globalOptions.GetOption(
