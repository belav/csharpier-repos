...
internal class LspWorkspaceManager : IDocumentChangeTracker, ILspService
{
    ...
    public async Task<Solution?> TryGetHostLspSolutionAsync(CancellationToken cancellationToken)
    {
        ...
        var (hostWorkspaceSolution, isForked) = updatedSolutions.FirstOrDefault(
            lspSolution => lspSolution.Solution.Workspace.Kind == _hostWorkspaceKind
        );
    ...
    /// This is always called serially in the <see cref="RequestExecutionQueue"/> when creating the <see cref="RequestContext"/>.
    /// </summary>
    public async Task<Document?> GetLspDocumentAsync(
    ...
    )
    {
        ...
        var miscDocument = _trackedDocuments.ContainsKey(uri)
            ? _lspMiscellaneousFilesWorkspace?.AddMiscellaneousDocument(
                uri,
                _trackedDocuments[uri],
                _logger
            )
            : null;
        return miscDocument;
    ...
    /// </summary>
    private async Task<ImmutableArray<(Solution Solution, bool IsForked)>> GetLspSolutionsAsync(
        CancellationToken cancellationToken
    )
    {
        ...
        registeredWorkspaces = registeredWorkspaces
            ...
            .Concat(
                registeredWorkspaces.Where(workspace => workspace is LspMiscellaneousFilesWorkspace)
            )
            .ToImmutableArray();
        ...
        foreach (var workspace in registeredWorkspaces)
        {
            ...
            var lspSolution = await GetLspSolutionForWorkspaceAsync(
        ...
        )
        {
            ...
            if (
                _cachedLspSolutions.TryGetValue(
            ...
            )
            {
                ...
                _cachedLspSolutions[workspaceCurrentSolution.Workspace] = (
    ...
    )
    {
        foreach (var (uriInWorkspace, documentsForUri) in documentsInWorkspace)
        {
            ...
            var isTextEquivalent = await AreChecksumsEqualAsync(
                    firstDocument,
                    _trackedDocuments[uriInWorkspace],
                    cancellationToken
                )
                .ConfigureAwait(false);
            ...
            if (!isTextEquivalent)
            {
                _logger.TraceWarning(
                    $"Text for {uriInWorkspace} did not match document text {firstDocument.Id} in workspace's {firstDocument.Project.Solution.Workspace.Kind} current solution"
                );
                return false;
            }
        }
    ...
    private static async Task<bool> AreChecksumsEqualAsync(
        Document document,
        SourceText lspText,
        CancellationToken cancellationToken
    )
