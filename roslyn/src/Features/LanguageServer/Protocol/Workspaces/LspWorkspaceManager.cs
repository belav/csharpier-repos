...
internal sealed class LspWorkspaceManager : IDocumentChangeTracker, ILspService
{
    ...
    private readonly Dictionary<
        Workspace,
        (int? forkedFromVersion, Solution solution)
    > _cachedLspSolutions = new();
    ...
    /// <para/> Access to this is guaranteed to be serial by the <see cref="RequestExecutionQueue{RequestContextType}"/>
    /// </summary>
    private ImmutableDictionary<Uri, (SourceText Text, string LanguageId)> _trackedDocuments =
        ImmutableDictionary<Uri, (SourceText, string)>.Empty;
    ...
    private static async ValueTask ApplyChangeToMutatingWorkspaceAsync(
        Workspace workspace,
        Uri uri,
        Func<ILspWorkspace, DocumentId, ValueTask> change
    )
    {
    ...
    )
    {
        ...
        Contract.ThrowIfTrue(
            ...
            $"didOpen received for {uri} which is already open."
        );
    ...
    public async ValueTask StopTrackingAsync(Uri uri, CancellationToken cancellationToken)
    {
        ...
        Contract.ThrowIfFalse(
            _trackedDocuments.ContainsKey(uri),
            $"didClose received for {uri} which is not open."
        );
    ...
    public void UpdateTrackedDocument(Uri uri, SourceText newSourceText)
    {
        ...
        Contract.ThrowIfFalse(
            _trackedDocuments.ContainsKey(uri),
            $"didChange received for {uri} which is not open."
        );
    ...
    public ImmutableDictionary<Uri, (SourceText Text, string LanguageId)> GetTrackedLspText() =>
        _trackedDocuments;
    ...
    /// </summary>
    public async Task<(Workspace?, Solution?)> GetLspSolutionInfoAsync(
        CancellationToken cancellationToken
    )
    {
        ...
        var (hostWorkspace, hostWorkspaceSolution, isForked) = updatedSolutions.FirstOrDefault(
            lspSolution => lspSolution.Solution.WorkspaceKind == WorkspaceKind.Host
        );
    ...
    /// This is always called serially in the <see cref="RequestExecutionQueue{RequestContextType}"/> when creating the <see cref="RequestContext"/>.
    /// </summary>
    public async Task<(Workspace?, Solution?, Document?)> GetLspDocumentInfoAsync(
    ...
    )
    {
        ...
        foreach (var (workspace, lspSolution, isForked) in lspSolutions)
        {
            ...
            if (documents.Any())
            {
                ...
                _requestTelemetryLogger.UpdateFindDocumentTelemetryData(
                    success: true,
                    workspaceKind
                );
                _requestTelemetryLogger.UpdateUsedForkedSolutionCounter(isForked);
        ...
        var searchedWorkspaceKinds = string.Join(
            ";",
            lspSolutions.SelectAsArray(lspSolution => lspSolution.Solution.Workspace.Kind)
        );
        _logger.LogInformation(
            $"Could not find '{textDocumentIdentifier.Uri}'.  Searched {searchedWorkspaceKinds}"
        );
        _requestTelemetryLogger.UpdateFindDocumentTelemetryData(
            success: false,
            workspaceKind: null
    ...
    /// </summary>
    private async Task<
        ImmutableArray<(Workspace workspace, Solution Solution, bool IsForked)>
    > GetLspSolutionsAsync(CancellationToken cancellationToken)
    {
        ...
        async Task<(Solution Solution, bool IsForked)> GetLspSolutionForWorkspaceAsync(
            Workspace workspace,
            CancellationToken cancellationToken
        )
        {
            ...
            var documentsInWorkspace = GetDocumentsForUris(
                _trackedDocuments.Keys.ToImmutableArray(),
                workspaceCurrentSolution
            );
            if (
                await DoesAllTextMatchWorkspaceSolutionAsync(
            ...
            if (
                ...
                && cachedSolution.forkedFromVersion == workspaceCurrentSolution.WorkspaceVersion
            )
            ...
            foreach (var (uri, workspaceDocuments) in documentsInWorkspace)
                lspSolution = lspSolution.WithDocumentText(
                    workspaceDocuments.Select(d => d.Id),
                    _trackedDocuments[uri].Text
                );

            // Remember this forked solution and the workspace version it was forked from.
            _cachedLspSolutions[workspace] = (
                workspaceCurrentSolution.WorkspaceVersion,
                lspSolution
            );
        ...
        async ValueTask TryOpenAndEditDocumentsInMutatingWorkspaceAsync(Workspace workspace)
        {
            foreach (var (uri, (sourceText, _)) in _trackedDocuments)
            {
                await ApplyChangeToMutatingWorkspaceAsync(
                        workspace,
                        uri,
                        async (mutatingWorkspace, documentId) =>
                        {
                            ...
                            await mutatingWorkspace
                                .UpdateTextIfPresentAsync(documentId, sourceText, cancellationToken)
                                .ConfigureAwait(false);
                        }
                    )
                    .ConfigureAwait(false);
    ...
    private async Task<bool> DoesAllTextMatchWorkspaceSolutionAsync(
        ImmutableDictionary<Uri, ImmutableArray<Document>> documentsInWorkspace,
        CancellationToken cancellationToken
    )
    {
        foreach (var (uriInWorkspace, documentsForUri) in documentsInWorkspace)
        {
            ...
            var isTextEquivalent = await AreChecksumsEqualAsync(
                    firstDocument,
                    _trackedDocuments[uriInWorkspace].Text,
                    cancellationToken
                )
                .ConfigureAwait(false);
    ...
    private static async ValueTask<bool> AreChecksumsEqualAsync(
        Document document,
        SourceText lspText,
        CancellationToken cancellationToken
    )
    {
        ...
        return lspText
            .GetContentHash()
            .AsSpan()
            .SequenceEqual(documentText.GetContentHash().AsSpan());
    }
    ...
    /// </summary>
    private static ImmutableDictionary<Uri, ImmutableArray<Document>> GetDocumentsForUris(
        ImmutableArray<Uri> trackedDocuments,
        Solution workspaceCurrentSolution
    )
    {
        using var _ = PooledDictionary<Uri, ImmutableArray<Document>>.GetInstance(
