    ...
    : AbstractLanguageServerProtocolTests(testOutputHelper)
{
    ...
    private Task<TestLspServer> CreateTestLspServerAsync(
        ...
        CreateTestLspServerAsync(
            markup,
            mutatingLspWorkspace,
            extraExportedTypes: [typeof(TestWorkspaceNavigateToSearchHostService)]
        );

    private Task<TestLspServer> CreateVisualBasicTestLspServerAsync(
        string markup,
        bool mutatingLspWorkspace
    ) =>
    ...
    protected sealed class TestWorkspaceNavigateToSearchHostService()
        : IWorkspaceNavigateToSearcherHostService
    {
        public ValueTask<bool> IsFullyLoadedAsync(CancellationToken cancellationToken) => new(true);
    }
}
