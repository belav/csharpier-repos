            ...
            @"class A
{
    ...
    <Project Language=""C#"" Name=""CSProj1"" CommonReferences=""true"" FilePath=""C:\CSProj1.csproj"">
        ...
        var (workspace, _, document) = await testLspServer
            .GetManager()
            .GetLspDocumentInfoAsync(
                new LSP.TextDocumentIdentifier { Uri = expectedDocumentUri },
                CancellationToken.None
            );
...
#pragma warning restore
        ...
        var (workspace, _, fileDocument) = await testLspServer
            .GetManager()
            .GetLspDocumentInfoAsync(
                new LSP.TextDocumentIdentifier { Uri = fileDocumentUri },
                CancellationToken.None
            );
        ...
        var (gitWorkspace, _, gitDocument) = await testLspServer
            .GetManager()
            .GetLspDocumentInfoAsync(
                new LSP.TextDocumentIdentifier { Uri = gitDocumentUri },
                CancellationToken.None
            );
