    ...
    public IEnumerable<SnippetInfo> GetSnippetsIfAvailable()
    {
        ...
        if (!File.Exists(snippetsFile))
        {
            throw new InvalidOperationException(
                $"Could not find test snippets file at {snippetsFile}"
            );
        }
        ...
        var snippetInfos = snippets.Value.Select(
            s => new SnippetInfo(s.Shortcut, s.Title, s.Title, snippetsFile)
        );
        return snippetInfos;
    }
