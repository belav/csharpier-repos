            ...
            @"class A
{
    void M()
    {
        ...
        var miscWorkspaceText = await GetMiscellaneousDocument(testLspServer)!
        ...
        {
            Range = new() { Start = new(0, 6), End = new(0, 7) },
            Uri = looseFileUri,
        };
        ...
        miscWorkspaceText = await GetMiscellaneousDocument(testLspServer)!
            .GetTextAsync(CancellationToken.None);
            ...
            @"class A
{
    void M()
    {
        ...
        var fileInWorkspaceUri = ProtocolConversions.CreateAbsoluteUri(
            testLspServer.GetCurrentSolution().Projects.Single().Documents.Single().FilePath!
        );
    ...
    void M()
    {
        ...
        var (miscWorkspace, _, miscDocument) = await testLspServer
            ...
            .GetLspDocumentInfoAsync(
                new LSP.TextDocumentIdentifier { Uri = looseFileUri },
        ...
        Assert.Contains(
            documentPath,
            testLspServer.GetCurrentSolution().Projects.Single().Documents.Select(d => d.FilePath)
        );
...
#pragma warning restore
        ...
        await testLspServer
            .OpenDocumentAsync(looseFileUri, source, languageId: "csharp")
    ...
    )
    {
        var (lspWorkspace, _, _) = await testLspServer
            .GetManager()
            .GetLspDocumentInfoAsync(
                new LSP.TextDocumentIdentifier { Uri = fileUri },
                CancellationToken.None
            );
        Assert.Equal(
            testLspServer.GetManagerAccessor().GetLspMiscellaneousFilesWorkspace(),
            lspWorkspace
        );
    }
