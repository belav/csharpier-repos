...
{
    public VSTypeScriptHandlerTests(ITestOutputHelper testOutputHelper)
    ...
    <Project Language=""TypeScript"" CommonReferences=""true"" AssemblyName=""TypeScriptProj"">
        ...
        await using var testLspServer = await CreateTsTestLspServerAsync(workspaceXml);
        var document = testLspServer.GetCurrentSolution().Projects.Single().Documents.Single();
        var simplifierOptions = testLspServer.TestWorkspace.GlobalOptions.GetSimplifierOptions(
    ...
    private async Task<TestLspServer> CreateTsTestLspServerAsync(
        string workspaceXml,
    ...
    )
    {
        ...
        var testWorkspace = CreateWorkspace(
            options,
