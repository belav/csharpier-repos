...
public class LspWorkspaceManagerTests : AbstractLanguageServerProtocolTests
{
    ...
    public async Task TestUsesLspTextOnOpenCloseAsync()
    {
        ...
        var lspDocument = await GetLspDocumentAsync(documentUri, testLspServer)
        ...
        var closedDocument = await GetLspDocumentAsync(documentUri, testLspServer)
            .ConfigureAwait(false);
    ...
    public async Task TestLspUsesWorkspaceInstanceOnChangesAsync()
    {
        ...
        var firstDocumentWithChange = await GetLspDocumentAsync(firstDocumentUri, testLspServer)
            .ConfigureAwait(false);
        var secondDocumentUnchanged = await GetLspDocumentAsync(secondDocumentUri, testLspServer)
            .ConfigureAwait(false);
    ...
    public async Task TestLspHasClosedDocumentChangesAsync()
    {
        ...
        var openedDocument = await GetLspDocumentAsync(firstDocumentUri, testLspServer)
            .ConfigureAwait(false);
        ...
        secondDocument = await GetLspDocumentAsync(secondDocumentUri, testLspServer)
            .ConfigureAwait(false);
    ...
    public async Task TestLspHasProjectChangesAsync()
    {
        ...
        openedDocument = await GetLspDocumentAsync(documentUri, testLspServer)
            .ConfigureAwait(false);
        AssertEx.NotNull(openedDocument);
        Assert.Equal(
            markup,
    ...
    public async Task TestLspFindsNewDocumentAsync()
    {
        ...
        var lspDocument = await GetLspDocumentAsync(newDocumentUri, testLspServer)
            .ConfigureAwait(false);
        AssertEx.NotNull(lspDocument);
    ...
    public async Task TestLspTransfersDocumentToNewWorkspaceAsync()
    {
        ...
        var miscDocument = await GetLspDocumentAsync(newDocumentUri, testLspServer)
            .ConfigureAwait(false);
        AssertEx.NotNull(miscDocument);
        Assert.Equal(
            testLspServer.GetManagerAccessor().GetLspMiscellaneousFilesWorkspace(),
            miscDocument.Project.Solution.Workspace
        );
        Assert.Equal(
            "LSP text",
        ...
        miscDocument = await GetLspDocumentAsync(newDocumentUri, testLspServer)
            .ConfigureAwait(false);
        AssertEx.NotNull(miscDocument);
        ...
        var document = await GetLspDocumentAsync(newDocumentUri, testLspServer)
            .ConfigureAwait(false);
    ...
    <Project Language=""C#"" CommonReferences=""true"" AssemblyName=""SecondWorkspaceProject"">
        ...
        var firstDocument = await GetLspDocumentAsync(firstWorkspaceDocumentUri, testLspServer)
            .ConfigureAwait(false);
        ...
        var secondDocument = await GetLspDocumentAsync(secondWorkspaceDocumentUri, testLspServer)
            .ConfigureAwait(false);
        AssertEx.NotNull(secondDocument);
        ...
        var changedFirstDocument = await GetLspDocumentAsync(
                firstWorkspaceDocumentUri,
                testLspServer
            )
            .ConfigureAwait(false);
        ...
        var unchangedSecondDocument = await GetLspDocumentAsync(
    ...
    <Project Language=""C#"" CommonReferences=""true"" AssemblyName=""SecondWorkspaceProject"">
        ...
        var secondDocumentChangedProject = await GetLspDocumentAsync(
                secondWorkspaceDocumentUri,
                testLspServer
            )
            .ConfigureAwait(false);
        ...
        Assert.Equal(
            firstDocument,
            await GetLspDocumentAsync(firstWorkspaceDocumentUri, testLspServer)
                .ConfigureAwait(false)
        );
    }
    ...
    <Project Language=""C#"" CommonReferences=""true"" AssemblyName=""CSProj1"">
        ...
        using var testLspServerOne = await TestLspServer.CreateAsync(
            testWorkspace,
            clientCapabilities: new(),
            WellKnownLspServerKinds.AlwaysActiveVSLspServer
        );
        using var testLspServerTwo = await TestLspServer.CreateAsync(
            testWorkspace,
            clientCapabilities: new(),
            WellKnownLspServerKinds.AlwaysActiveVSLspServer
        );
        ...
        var documentServerTwo = await GetLspDocumentAsync(documentUri, testLspServerTwo)
            .ConfigureAwait(false);
        ...
        documentServerOne = await GetLspDocumentAsync(documentUri, testLspServerOne)
            .ConfigureAwait(false);
        AssertEx.NotNull(documentServerOne);
        Assert.Equal(newAssemblyName, documentServerOne.Project.AssemblyName);
        documentServerTwo = await GetLspDocumentAsync(documentUri, testLspServerTwo)
            .ConfigureAwait(false);
        AssertEx.NotNull(documentServerTwo);
    ...
    private static Task<Document?> GetLspDocumentAsync(Uri uri, TestLspServer testLspServer)
    {
        return testLspServer
            .GetManager()
            .GetLspDocumentAsync(CreateTextDocumentIdentifier(uri), CancellationToken.None);
    }
