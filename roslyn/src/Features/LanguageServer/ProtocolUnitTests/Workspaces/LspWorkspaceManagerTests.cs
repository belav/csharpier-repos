...
{
    public LspWorkspaceManagerTests(ITestOutputHelper testOutputHelper)
    ...
    public async Task TestUsesLspTextOnOpenCloseAsync(bool mutatingLspWorkspace)
    {
        ...
        var (_, lspDocument) = await GetLspWorkspaceAndDocumentAsync(documentUri, testLspServer)
            .ConfigureAwait(false);
        AssertEx.NotNull(lspDocument);
        Assert.Equal(
            "LSP text",
            (await lspDocument.GetTextAsync(CancellationToken.None)).ToString()
        );
        ...
        (_, lspDocument) = await GetLspWorkspaceAndDocumentAsync(documentUri, testLspServer)
            .ConfigureAwait(false);
        AssertEx.NotNull(lspDocument);
        Assert.Equal(
            "More textLSP text",
        ...
        var (_, closedDocument) = await GetLspWorkspaceAndDocumentAsync(documentUri, testLspServer)
            .ConfigureAwait(false);
    ...
    public async Task TestLspUsesWorkspaceInstanceOnChangesAsync(bool mutatingLspWorkspace)
    {
        ...
        var markupTwo = "Two";
        await using var testLspServer = await CreateTestLspServerAsync(
            new string[] { markupOne, markupTwo },
            mutatingLspWorkspace
        );
        ...
        var secondDocument = await OpenDocumentAndVerifyLspTextAsync(
            secondDocumentUri,
            testLspServer,
            markupTwo
        );
        var firstDocumentInitialVersion = await firstDocument.GetSyntaxVersionAsync(
            CancellationToken.None
        );
        var secondDocumentInitialVersion = await secondDocument.GetSyntaxVersionAsync(
            CancellationToken.None
        );
        ...
        if (mutatingLspWorkspace)
        {
            ...
            Assert.Same(
                testLspServer.TestWorkspace.CurrentSolution.GetDocument(firstDocument.Id)?.State,
                firstDocument?.State
            );
        }
        else
        ...
        await testLspServer.TestWorkspace.ChangeDocumentAsync(
            firstDocument.Id,
            SourceText.From(
                $"Some more text{markupOne}",
                System.Text.Encoding.UTF8,
                SourceHashAlgorithms.Default
            )
        );

        var (_, firstDocumentWithChange) = await GetLspWorkspaceAndDocumentAsync(
                firstDocumentUri,
                testLspServer
            )
            .ConfigureAwait(false);
        var (_, secondDocumentUnchanged) = await GetLspWorkspaceAndDocumentAsync(
                secondDocumentUri,
                testLspServer
            )
            .ConfigureAwait(false);
        ...
        Assert.NotEqual(
            firstDocumentInitialVersion,
            await firstDocumentWithChange.GetSyntaxVersionAsync(CancellationToken.None)
        );
        Assert.Equal(
            $"Some more text{markupOne}",
            (await firstDocumentWithChange.GetTextAsync(CancellationToken.None)).ToString()
        );

        // Verify that the document that we did not change still has the same version.
        Assert.Equal(
            secondDocumentInitialVersion,
            await secondDocumentUnchanged.GetSyntaxVersionAsync(CancellationToken.None)
        );

        // Verify the LSP documents are the same instance as the workspaces documents.
        Assert.Equal(
            testLspServer.TestWorkspace.CurrentSolution.GetDocument(firstDocumentWithChange.Id),
            firstDocumentWithChange
        );
        Assert.Equal(
            testLspServer.TestWorkspace.CurrentSolution.GetDocument(secondDocumentUnchanged.Id),
            secondDocumentUnchanged
        );
    }
    ...
    public async Task TestLspHasClosedDocumentChangesAsync(bool mutatingLspWorkspace)
    {
        ...
        var (_, openedDocument) = await GetLspWorkspaceAndDocumentAsync(
                firstDocumentUri,
                testLspServer
            )
            .ConfigureAwait(false);
        ...
        Assert.Equal(
            "LSP text",
            (await openedDocument.GetTextAsync(CancellationToken.None)).ToString()
        );

        // Verify that the LSP solution has the workspace text in the closed document.
        (_, secondDocument) = await GetLspWorkspaceAndDocumentAsync(
                secondDocumentUri,
                testLspServer
            )
            .ConfigureAwait(false);
    ...
    public async Task TestLspHasProjectChangesAsync(bool mutatingLspWorkspace)
    {
        ...
        var openedDocument = await OpenDocumentAndVerifyLspTextAsync(
            documentUri,
            testLspServer,
            markup
        );
        ...
        var newProject = testLspServer.TestWorkspace.CurrentSolution.Projects
            .First()
            .WithAssemblyName("NewCSProj1");
        ...
        (_, openedDocument) = await GetLspWorkspaceAndDocumentAsync(documentUri, testLspServer)
            .ConfigureAwait(false);
        AssertEx.NotNull(openedDocument);
        Assert.Equal(
            markup,
    ...
    public async Task TestLspFindsNewDocumentAsync(bool mutatingLspWorkspace)
    {
        ...
        var newDocumentId = DocumentId.CreateNewId(
            testLspServer.TestWorkspace.CurrentSolution.ProjectIds[0]
        );
        var newSolution = testLspServer.TestWorkspace.CurrentSolution.AddDocument(
            newDocumentId,
            "NewDoc.cs",
            SourceText.From("New Doc", System.Text.Encoding.UTF8, SourceHashAlgorithms.Default),
            filePath: @"C:\NewDoc.cs"
        );
    ...
    public async Task TestLspTransfersDocumentToNewWorkspaceAsync(bool mutatingLspWorkspace)
    {
        ...
        var newDocumentInfo = DocumentInfo.Create(
            ...
            loader: new TestTextLoader("New Doc")
        );
        ...
        var (miscWorkspace, miscDocument) = await GetLspWorkspaceAndDocumentAsync(
                newDocumentUri,
                testLspServer
            )
            .ConfigureAwait(false);
        ...
        Assert.Equal(
            "LSP text",
            (await miscDocument.GetTextAsync(CancellationToken.None)).ToString()
        );
        ...
        (_, miscDocument) = await GetLspWorkspaceAndDocumentAsync(newDocumentUri, testLspServer)
            .ConfigureAwait(false);
        AssertEx.NotNull(miscDocument);
        ...
        var (documentWorkspace, document) = await GetLspWorkspaceAndDocumentAsync(
                newDocumentUri,
                testLspServer
            )
            .ConfigureAwait(false);
    ...
    <Project Language=""C#"" CommonReferences=""true"" AssemblyName=""SecondWorkspaceProject"">
        ...
        var (_, hostSolution) = await GetLspHostWorkspaceAndSolutionAsync(testLspServer)
            .ConfigureAwait(false);
    ...
    <Project Language=""C#"" CommonReferences=""true"" AssemblyName=""FirstWorkspaceProject"">
        ...
        var workspaceRegistrationService =
            exportProvider.GetExport<LspWorkspaceRegistrationService>();

        // Verify the workspace is registered.
        Assert.True(IsWorkspaceRegistered(testLspServer.TestWorkspace, testLspServer));

        // Verify there is not workspace matching the host workspace kind.
        var (_, solution) = await GetLspHostWorkspaceAndSolutionAsync(testLspServer)
            .ConfigureAwait(false);
    ...
    <Project Language=""C#"" CommonReferences=""true"" AssemblyName=""SecondWorkspaceProject"">
        ...
        using var testWorkspaceTwo = CreateWorkspace(
            options: null,
        ...
        var firstWorkspaceDocumentUri = ProtocolConversions.CreateAbsoluteUri(
            @"C:\FirstWorkspace.cs"
        );
        var secondWorkspaceDocumentUri = ProtocolConversions.CreateAbsoluteUri(
            @"C:\SecondWorkspace.cs"
        );
        ...
        var (firstWorkspace, firstDocument) = await GetLspWorkspaceAndDocumentAsync(
        ...
        var (secondWorkspace, secondDocument) = await GetLspWorkspaceAndDocumentAsync(
                secondWorkspaceDocumentUri,
                testLspServer
            )
            .ConfigureAwait(false);
        AssertEx.NotNull(secondDocument);
        ...
        var (_, changedFirstDocument) = await GetLspWorkspaceAndDocumentAsync(
                firstWorkspaceDocumentUri,
                testLspServer
            )
            .ConfigureAwait(false);
        AssertEx.NotNull(changedFirstDocument);
        var changedFirstDocumentText = await changedFirstDocument.GetTextAsync(
            CancellationToken.None
        );
        ...
        var (_, unchangedSecondDocument) = await GetLspWorkspaceAndDocumentAsync(
                secondWorkspaceDocumentUri,
                testLspServer
            )
            .ConfigureAwait(false);
    ...
    <Project Language=""C#"" CommonReferences=""true"" AssemblyName=""SecondWorkspaceProject"">
        ...
        var newProjectWorkspaceTwo = testWorkspaceTwo.CurrentSolution.Projects
            .First()
            .WithAssemblyName("NewCSProj1");
        await testWorkspaceTwo.ChangeProjectAsync(
            newProjectWorkspaceTwo.Id,
            newProjectWorkspaceTwo.Solution
        );

        // The second document should have an updated project assembly name.
        var (_, secondDocumentChangedProject) = await GetLspWorkspaceAndDocumentAsync(
                secondWorkspaceDocumentUri,
                testLspServer
            )
            .ConfigureAwait(false);
    ...
    <Project Language=""C#"" CommonReferences=""true"" AssemblyName=""CSProj1"">
        ...
        using var testWorkspace = CreateWorkspace(
            options: null,
            workspaceKind: null,
            mutatingLspWorkspace
        );
        ...
        await using var testLspServerOne = await TestLspServer.CreateAsync(
            testWorkspace,
            new InitializationOptions(),
            TestOutputLspLogger
        );
        await using var testLspServerTwo = await TestLspServer.CreateAsync(
            testWorkspace,
            new InitializationOptions(),
            TestOutputLspLogger
        );
        ...
        var documentServerOne = await OpenDocumentAndVerifyLspTextAsync(
            documentUri,
            testLspServerOne,
            "Server one text"
        );

        var (_, documentServerTwo) = await GetLspWorkspaceAndDocumentAsync(
                documentUri,
                testLspServerTwo
            )
            .ConfigureAwait(false);
        ...
        if (mutatingLspWorkspace)
        {
            ...
            Assert.Equal(
                "Server one text",
                (await documentServerTwo.GetTextAsync(CancellationToken.None)).ToString()
            );
        }
        else
        {
        ...
        var newProject = testWorkspace.CurrentSolution.Projects
            .First()
        ...
        (_, documentServerOne) = await GetLspWorkspaceAndDocumentAsync(
                documentUri,
                testLspServerOne
            )
            .ConfigureAwait(false);
        ...
        (_, documentServerTwo) = await GetLspWorkspaceAndDocumentAsync(
                documentUri,
                testLspServerTwo
            )
            .ConfigureAwait(false);
    ...
    public async Task TestLspDocumentPreferredOverProjectSystemDocumentAddInMutatingWorkspace()
    {
        ...
        var (workspace1, document1) = await GetLspWorkspaceAndDocumentAsync(
                documentUri,
                testLspServer
            )
            .ConfigureAwait(false);
        ...
        await testLspServer.TestWorkspace.ChangeSolutionAsync(
            testLspServer.TestWorkspace.CurrentSolution.Projects
                .Single()
        ...
        Assert.NotEqual(
            ...
            (
                await testLspServer.TestWorkspace.CurrentSolution.Projects
                    .Single()
                    .Documents.Single()
                    .GetTextAsync()
            ).ToString()
    ...
    public async Task TestLspDocumentPreferredOverProjectSystemDocumentChangeInMutatingWorkspace()
    {
        ...
        var documentUri = testLspServer.TestWorkspace.CurrentSolution.Projects
            .Single()
            .Documents.Single()
            .GetURI();
        await testLspServer.OpenDocumentAsync(documentUri, "Text");

        var (workspace, document) = await GetLspWorkspaceAndDocumentAsync(
                documentUri,
                testLspServer
            )
            .ConfigureAwait(false);
        ...
        await testLspServer.TestWorkspace.ChangeSolutionAsync(
            testLspServer.TestWorkspace.CurrentSolution.WithDocumentText(
                document.Id,
                SourceText.From("New Disk Contents")
            )
        );
        ...
        (workspace, document) = await GetLspWorkspaceAndDocumentAsync(documentUri, testLspServer)
            .ConfigureAwait(false);
    ...
    public async Task TestLspDocumentChangedInMutatingWorkspaceIncrementallyParses()
    {
        ...
        var initialContents =
            "namespace N { class C1 { void X() { } } class C2 { void Y() { } } class C3 { void Z() { } } }";
        var documentUri = testLspServer.TestWorkspace.CurrentSolution.Projects
            .Single()
            .Documents.Single()
            .GetURI();
        ...
        var (workspace, originalDocument) = await GetLspWorkspaceAndDocumentAsync(
                documentUri,
                testLspServer
            )
            .ConfigureAwait(false);
        ...
        var originalRoot = await originalDocument.GetRequiredSyntaxRootAsync(
            CancellationToken.None
        );
        ...
        await testLspServer.ReplaceTextAsync(
            ...
            (
                ProtocolConversions.TextSpanToRange(
                    new TextSpan(initialContents.IndexOf("C3"), 1),
                    originalSourceText
                ),
                "D"
            )
        ...
        var (_, newDocument) = await GetLspWorkspaceAndDocumentAsync(documentUri, testLspServer)
            .ConfigureAwait(false);
        ...
        Assert.Equal(
            "namespace N { class C1 { void X() { } } class C2 { void Y() { } } class D3 { void Z() { } } }",
        ...
        var oldClassDeclarations = originalRoot
            .DescendantNodes()
            .Where(n => syntaxFacts.IsClassDeclaration(n))
            .ToImmutableArray();
        var newClassDeclarations = newRoot
            .DescendantNodes()
            .Where(n => syntaxFacts.IsClassDeclaration(n))
            .ToImmutableArray();
        ...
        var oldMethodDeclarations = originalRoot
            .DescendantNodes()
            .Where(n => syntaxFacts.IsMethodLevelMember(n))
            .ToImmutableArray();
        var newMethodDeclarations = newRoot
            .DescendantNodes()
            .Where(n => syntaxFacts.IsMethodLevelMember(n))
            .ToImmutableArray();
    ...
    private static async Task<Document> OpenDocumentAndVerifyLspTextAsync(
        Uri documentUri,
        TestLspServer testLspServer,
        string openText = "LSP text"
    )
    {
    ...
    private static async Task<(
        Workspace? workspace,
        Document? document
    )> GetLspWorkspaceAndDocumentAsync(Uri uri, TestLspServer testLspServer)
    {
        var (workspace, _, document) = await testLspServer
            .GetManager()
            .GetLspDocumentInfoAsync(CreateTextDocumentIdentifier(uri), CancellationToken.None)
            .ConfigureAwait(false);
