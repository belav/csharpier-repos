...
namespace Microsoft.CodeAnalysis.LanguageServer.UnitTests.CodeActions
{
    ...
    {
        public CodeActionResolveTests(ITestOutputHelper testOutputHelper)
...
class {|caret:ABC|}
{
                ...
                new InitializationOptions
                {
                    ClientCapabilities = new ClientCapabilities()
                    {
                        Workspace = new WorkspaceClientCapabilities
                        {
                            ...
                            {
                                ResourceOperations = new ResourceOperationKind[]
                                {
            ...
            var expectedCodeAction = CodeActionsTests.CreateCodeAction(
                ...
                edit: GenerateRenameFileEdit(
                    new List<(Uri, Uri)> { (documentUriBefore, documentUriAfter) }
                )
            );
                                    ...
                                    @"class BCD
{
        ...
        private static WorkspaceEdit GenerateRenameFileEdit(
            IList<(Uri oldUri, Uri newUri)> renameLocations
        ) =>
            new()
            {
                DocumentChanges = renameLocations
                    .Select(
                        locations =>
                            new SumType<TextDocumentEdit, CreateFile, RenameFile, DeleteFile>(
                                new RenameFile()
                                {
                                    OldUri = locations.oldUri,
                                    NewUri = locations.newUri
                                }
                            )
                    )
                    .ToArray()
