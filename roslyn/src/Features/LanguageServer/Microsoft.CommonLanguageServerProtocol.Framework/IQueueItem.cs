...
/// </summary>
/// <typeparam name="TRequestContext">The type of the request context to be passed along to the
// handler.</typeparam>
public interface IQueueItem<TRequestContext>
{
    ...
    /// </summary>
    /// <param name="requestContext">the context created by <see
    // cref="CreateRequestContextAsync(CancellationToken)"/></param>
    ...
    /// running the actual request in <see cref="StartRequestAsync(TRequestContext?,
    // CancellationToken)"/>
    ...
    /// <summary>
    /// Indicates that this request may mutate the server state, so that the queue may handle its
    // execution appropriatly.
    /// </summary>
