...
namespace Microsoft.CodeAnalysis.Editor.CSharp.UnitTests.CodeActions.SyncNamespace
{
    public abstract class CSharpSyncNamespaceTestsBase : AbstractCodeActionTest
    {
        ...
        protected static string ProjectRootPath =>
            PathUtilities.IsUnixLikePlatform ? @"/ProjectA/" : @"C:\ProjectA\";
        ...
        )
        {
            ...
            using (var workspace = CreateWorkspaceFromOptions(initialMarkup, testOptions))
            {
                ...
                {
                    var expectedFolderPaths = expectedFolders.Select(f =>
                        string.Join(PathUtilities.DirectorySeparatorStr, f)
                    );
                    ...
                    foreach (var result in allResults)
                    {
                        ...
                        var newDocument = GetDocumentToVerify(
                            expectedChangedDocumentId: null,
                            oldSolution,
                            newSolution
                        );
                        actualFolderPaths.Add(
                            string.Join(PathUtilities.DirectorySeparatorStr, newDocument.Folders)
                        );
                    }

                    Assert.True(
                        expectedFolderPaths.Count() == actualFolderPaths.Count,
                ...
                else
                {
                    ...
                    if (actions.Length > 0)
                    {
                        var renameFileAction = actions.Any(action =>
                            action is not CodeAction.SolutionChangeAction
                        );
                        Assert.False(
                            renameFileAction,
                            "Move File to match namespace code action was not expected, but shows up."
                        );
                    }
                }
            }
        ...
        )
        {
            ...
            using (var workspace = CreateWorkspaceFromOptions(initialMarkUp, testOptions))
            {
                ...
                if (expectedSourceOriginal != null)
                {
                    ...
                    Assert.True(
                        changedDocumentIds.Contains(originalDocumentId),
                ...
                else
                {
                    ...
                    if (actions.Length > 0)
                    {
                        var hasChangeNamespaceAction = actions.Any(action =>
                            action is CodeAction.SolutionChangeAction
                        );
                        Assert.False(
                            hasChangeNamespaceAction,
                            "Change namespace to match folder action was not expected, but shows up."
                        );
                    }
                }
            }
