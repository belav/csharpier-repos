    ...
    public abstract class CSharpSyncNamespaceTestsBase : AbstractCodeActionTest
    {
        ...
        )
        {
            ...
            using (var workspace = CreateWorkspaceFromOptions(initialMarkup, testOptions))
            {
                ...
                {
                    var expectedFolderPaths = expectedFolders.Select(
                        f => string.Join(PathUtilities.DirectorySeparatorStr, f)
                    );
                ...
                else
                {
                    ...
                    if (actions.Length > 0)
                    {
                        var renameFileAction = actions.Any(
                            action => action is not CodeAction.SolutionChangeAction
                        );
                        Assert.False(
                            renameFileAction,
        ...
        )
        {
            ...
            using (var workspace = CreateWorkspaceFromOptions(initialMarkUp, testOptions))
            {
                ...
                else
                {
                    ...
                    if (actions.Length > 0)
                    {
                        var hasChangeNamespaceAction = actions.Any(
                            action => action is CodeAction.SolutionChangeAction
                        );
                        Assert.False(
