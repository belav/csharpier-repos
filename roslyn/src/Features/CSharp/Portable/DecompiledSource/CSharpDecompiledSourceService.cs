...
namespace Microsoft.CodeAnalysis.CSharp.DecompiledSource
{
    ...
    internal class CSharpDecompiledSourceService : IDecompiledSourceService
    {
        ...
        )
        {
            ...
            var decompiledDocument = decompilationService.PerformDecompilation(
                document,
                fullName,
                symbolCompilation,
                metadataReference,
            ...
            document = await AddAssemblyInfoRegionAsync(
                    document,
                    symbol,
            ...
            return await FormatDocumentAsync(document, formattingOptions, cancellationToken)
                .ConfigureAwait(false);
        }
        ...
        private static string GetFullReflectionName(INamedTypeSymbol? containingType)
        {
            ...
            for (
                ...
                symbol =
                        (INamespaceOrTypeSymbol?)symbol.ContainingType ?? symbol.ContainingNamespace
            )
            {
