...
namespace Microsoft.CodeAnalysis.CSharp.Completion.Providers.DeclarationName
{
    ...
    internal sealed partial class DeclarationNameRecommender : IDeclarationNameRecommender
    {
        ...
        )
        {
            ...
            if (
                type is IErrorTypeSymbol { TypeArguments: [var typeArgument] }
        ...
        private bool LooksLikeWellKnownCollectionType(Compilation compilation, string name)
        {
            ...
            var systemCollections = system
                ?.GetMembers(nameof(System.Collections))
                .OfType<INamespaceSymbol>()
                .FirstOrDefault();
                                    ...
                                    result.Add(
                                        (
                                            ...
                                            NameDeclarationInfo.GetGlyph(
                                                NameDeclarationInfo.GetSymbolKind(kind),
                                                declarationInfo.DeclaredAccessibility
                                            )
                                        )
                                    );
        ...
        )
        {
            ...
            )
            {
                ...
                {
                    MethodDeclarationSyntax method => namedType
                        .GetMembers(method.Identifier.ValueText)
                        .OfType<IMethodSymbol>()
                        .ToImmutableArray(),
                    ConstructorDeclarationSyntax constructor => namedType
