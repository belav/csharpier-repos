    ...
    internal static partial class ConvertProgramTransform
    {
        ...
        )
        {
            ...
            if (root.IsTopLevelProgram())
            {
                ...
                var programType = compilation.GetBestTypeByMetadataName(
                ...
                if (programType != null)
                {
                    ...
                    )
                    {
                        ...
                        var newRoot = root.RemoveNodes(
                            root.Members.OfType<GlobalStatementSyntax>().Skip(1),
        ...
        )
        {
            ...
            var hasExistingPart = programType.DeclaringSyntaxReferences.Any(
                d => d.GetSyntax(cancellationToken) is TypeDeclarationSyntax
            );
            ...
            method = (MethodDeclarationSyntax)
                generator.WithAccessibility(
                    method,
                    useDeclaredAccessibity
                        ? mainMethod.DeclaredAccessibility
            ...
            if (
                ...
                && method.ParameterList.Parameters[0].Type is ArrayTypeSyntax arrayType
            )
            ...
            return FixupComments(
                generator
                    .ClassDeclaration(
                        WellKnownMemberNames.TopLevelStatementsEntryPointTypeName,
                        accessibility: useDeclaredAccessibity
                            ? programType.DeclaredAccessibility
                            : Accessibility.NotApplicable,
                        modifiers: hasExistingPart
        ...
        private static ImmutableArray<StatementSyntax> GenerateProgramMainStatements(
            ...
            out SyntaxTriviaList triviaToMove
        )
        {
            ...
            foreach (var globalStatement in root.Members.OfType<GlobalStatementSyntax>())
            {
                ...
                if (first)
                {
                    ...
                    while (
                        triviaToMove
                            is [.., { RawKind: (int)SyntaxKind.SingleLineCommentTrivia }, { RawKind: (int)SyntaxKind.EndOfLineTrivia }]
                    )
                        triviaToMove = TriviaList(triviaToMove.Take(triviaToMove.Count - 2));

                    var commentsToPreserve = TriviaList(
                        statement.GetLeadingTrivia().Skip(triviaToMove.Count)
                    );
            ...
            where TSyntaxNode : SyntaxNode
        {
            ...
            while (
                commentIndex < leadingTrivia.Count
                && leadingTrivia[commentIndex].Kind() is SyntaxKind.EndOfLineTrivia
            )
                leadingTrivia = leadingTrivia.RemoveAt(commentIndex);
