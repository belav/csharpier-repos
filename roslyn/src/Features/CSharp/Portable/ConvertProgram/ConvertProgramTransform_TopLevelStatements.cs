...
namespace Microsoft.CodeAnalysis.CSharp.ConvertProgram
{
    ...
    internal static partial class ConvertProgramTransform
    {
        ...
        )
        {
            ...
            document = await ConvertFileScopedNamespaceAsync(
                    document,
                    cleanupOptions,
                    cancellationToken
                )
        ...
        )
        {
            ...
            return
                root.Members.OfType<FileScopedNamespaceDeclarationSyntax>().FirstOrDefault()
                    is { } fileScopedNamespace
                ? await ConvertNamespaceTransform
                    .ConvertFileScopedNamespaceAsync(
                        document,
                        fileScopedNamespace,
                        (CSharpSyntaxFormattingOptions)cleanupOptions.FormattingOptions,
                        cancellationToken
                    )
                    .ConfigureAwait(false)
                : document;
        ...
        )
        {
            ...
            var documentWithImportsAdded = document.WithSyntaxRoot(
            ...
            return await removeImportsService
                .RemoveUnnecessaryImportsAsync(
                    documentWithImportsAdded,
                    n => n.HasAnnotation(annotation),
        ...
        private static void AddUsingDirectives(
            ...
            ArrayBuilder<UsingDirectiveSyntax> directives
        )
        {
        ...
        )
        {
            ...
            if (namespaceDeclaration != null && namespaceDeclaration.Members.Count >= 2)
            {
            ...
            else if (namespaceDeclaration != null)
            {
                ...
                var fileBanner =
                    root.GetFirstToken() == namespaceDeclaration.GetFirstToken()
                ...
                editor.ReplaceNode(root, root.ReplaceNode(namespaceDeclaration, globalStatements));
            }
            else
            {
        ...
        )
        {
            ...
            if (statements.Count > 0)
            {
                statements[0] = statements[0]
                    .WithPrependedLeadingTrivia(typeDeclaration.GetLeadingTrivia());
            ...
            using var _1 = ArrayBuilder<GlobalStatementSyntax>.GetInstance(
                out var globalStatements
            );
            foreach (var statement in statements)
                globalStatements.Add(
                    GlobalStatement(statement).WithAdditionalAnnotations(Formatter.Annotation)
                );
