...
namespace Microsoft.CodeAnalysis.CSharp.ExtractInterface
{
    ...
    internal sealed class CSharpExtractInterfaceService : AbstractExtractInterfaceService
    {
        ...
        )
        {
            ...
            var typeDeclarationNode = await document
                .TryGetRelevantNodeAsync<TypeDeclarationSyntax>(span, cancellationToken)
            ...
            // of the focused node.
            if (
                typeDeclarationNode == null
                && typeDiscoveryRule == TypeDiscoveryRule.TypeDeclaration
            )
            {
                var relevantNode = await document
                    .TryGetRelevantNodeAsync<SyntaxNode>(span, cancellationToken)
                    .ConfigureAwait(false);
        ...
        internal override bool ShouldIncludeAccessibilityModifier(SyntaxNode typeNode)
        {
            ...
            return typeDeclaration.Modifiers.Any(m =>
                SyntaxFacts.IsAccessibilityModifier(m.Kind())
            );
        }
