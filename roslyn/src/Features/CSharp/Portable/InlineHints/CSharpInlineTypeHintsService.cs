    ...
    internal sealed class CSharpInlineTypeHintsService : AbstractInlineTypeHintsService
    {
        ...
        )
        {
            ...
            if (forLambdaParameterTypes || displayAllOverride)
            {
                if (node is ParameterSyntax { Type: null } parameterNode)
                {
                    ...
                    {
                        return
                            parameterNode.Parent?.Parent?.Kind()
                            ...
                            ? new TypeHint(
                                ...
                                textChange: new TextChange(
                                    span,
                                    parameter.Type.ToDisplayString(s_minimalTypeStyle) + " "
                                ),
                                trailingSpace: true
                            )
                            : new TypeHint(
                ...
                if (node is ImplicitObjectCreationExpressionSyntax implicitNew)
                {
                    ...
                    if (IsValidType(type))
                    {
                        ...
                        return new(
                            type,
                            span,
                            new TextChange(span, " " + type.ToDisplayString(s_minimalTypeStyle)),
                            leadingSpace: true
                        );
                    }
                }
        ...
        )
        {
            ...
            return new TypeHint(
                type,
                span,
                new TextChange(displayAllSpan.Span, type.ToDisplayString(s_minimalTypeStyle)),
