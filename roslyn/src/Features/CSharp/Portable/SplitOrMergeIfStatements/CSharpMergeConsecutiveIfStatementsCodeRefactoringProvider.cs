...
namespace Microsoft.CodeAnalysis.CSharp.SplitOrMergeIfStatements
{
    [
        ExportCodeRefactoringProvider(
            ...
            Name = PredefinedCodeRefactoringProviderNames.MergeConsecutiveIfStatements
        ),
        Shared
    ]
        ...
        : AbstractMergeConsecutiveIfStatementsCodeRefactoringProvider
    {
        ...
        )
        {
            ...
            {
                // Cases:
                // 1. Position is at a child token of an if statement with no selection (e.g. 'if' keyword, a
                // parenthesis)
                // 2. Selection around the 'if' keyword
                // 3. Selection around the header - from 'if' keyword to the end of the condition
                // 4. Selection around the whole if statement *excluding* its else clause - from 'if' keyword to the
            ...
            )
            {
                ...
                // 6. Selection around the header including the 'else' keyword - from 'else' keyword to the end of
                // the condition
