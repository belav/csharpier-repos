...
namespace Microsoft.CodeAnalysis.CSharp.ConvertToRecord
{
    internal static class ConvertToRecordEngine
    {
        ...
        )
        {
            ...
            var defaults = positionalParameterInfos.SelectAsArray(
            ...
            var lineFormattingOptions = await document
                .GetLineFormattingOptionsAsync(fallbackOptions: null, cancellationToken)
                .ConfigureAwait(false);
            ...
            if (baseList != null)
            {
                ...
                if (iEquatable != null)
                {
                    var iEquatableItem = typeList.FirstOrDefault(
                        baseItem =>
                            iEquatable.Equals(
                            ...
                            var newInitializer = (InitializerExpressionSyntax)
                                generator.RemoveNodes(
                                    ...
                                    expressionIndices
                                        .Where(i => i != -1)
                                        .Select(
                                            i => updatedObjectCreation.Initializer!.Expressions[i]
                                        )
                                );
                                ...
                                .Select(
                                    (node, index) =>
                                    {
                                        if (node is XmlTextSyntax text)
                                        {
                                            ...
                                            var tokens = text.TextTokens.SelectAsArray(
                                                token =>
