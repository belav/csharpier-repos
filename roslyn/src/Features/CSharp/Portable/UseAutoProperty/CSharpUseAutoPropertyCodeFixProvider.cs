...
namespace Microsoft.CodeAnalysis.CSharp.UseAutoProperty
{
    [
        ExportCodeFixProvider(
            ...
            Name = PredefinedCodeFixProviderNames.UseAutoProperty
        ),
        Shared
    ]
        ...
        >
    {
        ...
        )
        {
            ...
            var updatedProperty = propertyDeclaration
                .WithAccessorList(UpdateAccessorList(propertyDeclaration.AccessorList))
            ...
            if (NeedsSetter(compilation, propertyDeclaration, isWrittenOutsideOfConstructor))
            {
                ...
                var modifiers = SyntaxFactory.TokenList(
                    updatedProperty.Modifiers.Where(token =>
            ...
            var fieldInitializer = await GetFieldInitializerAsync(fieldSymbol, cancellationToken)
                .ConfigureAwait(false);
            if (fieldInitializer != null)
            {
                updatedProperty = updatedProperty
                    .WithInitializer(SyntaxFactory.EqualsValueClause(fieldInitializer))
                    .WithSemicolonToken(SyntaxFactory.Token(SyntaxKind.SemicolonToken));
            }
        ...
        private static bool NeedsSetter(
            Compilation compilation,
            PropertyDeclarationSyntax propertyDeclaration,
            bool isWrittenOutsideOfConstructor
        )
        ...
        private static AccessorListSyntax UpdateAccessorList(AccessorListSyntax accessorList)
        {
            ...
            return accessorList.WithAccessors(
                SyntaxFactory.List(GetAccessors(accessorList.Accessors))
            );
        }
