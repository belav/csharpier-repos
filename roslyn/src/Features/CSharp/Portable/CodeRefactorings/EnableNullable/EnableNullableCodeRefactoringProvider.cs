...
namespace Microsoft.CodeAnalysis.CSharp.CodeRefactorings.EnableNullable
{
    ...
    internal partial class EnableNullableCodeRefactoringProvider : CodeRefactoringProvider
    {
        private static readonly Func<
            ...
            bool
        > s_isNullableDirectiveTriviaPredicate = directive =>
            directive.IsKind(SyntaxKind.NullableDirectiveTrivia);
        ...
        public override async Task ComputeRefactoringsAsync(CodeRefactoringContext context)
        {
            ...
            if (
                token.Kind()
                    is not (
                        ...
                        or SyntaxKind.RestoreKeyword
                        or SyntaxKind.DisableKeyword
                        or SyntaxKind.NullableKeyword
            ...
            context.RegisterRefactoring(
                new CustomCodeAction(
                    (purpose, progress, cancellationToken) =>
        ...
        private static async Task<Solution> EnableNullableReferenceTypesAsync(
            Project project,
            CodeActionPurpose purpose,
            CodeActionOptionsProvider fallbackOptions,
            IProgress<CodeAnalysisProgress> _,
        ...
        private static async Task<SyntaxNode> EnableNullableReferenceTypesAsync(
            Document document,
            CodeActionOptionsProvider fallbackOptions,
            CancellationToken cancellationToken
        )
        {
            ...
            return await DisableNullableReferenceTypesInExistingDocumentIfNecessaryAsync(
                    document,
                    root,
                    firstToken,
                    fallbackOptions,
        ...
        private static (SyntaxNode root, SyntaxToken firstToken) RewriteExistingDirectives(
            SyntaxNode root,
        ...
        )
        {
            ...
            for (
                var directive = firstDirective;
                directive is not null;
                directive = directive.GetNextDirective(s_isNullableDirectiveTriviaPredicate)
            )
            {
        ...
        )
        {
            ...
            {
                var nullableDisableTrivia = SyntaxFactory.Trivia(
                ...
                return root.ReplaceToken(
                    ...
                    firstToken.WithLeadingTrivia(
                        existingTriviaList.InsertRange(
                            insertionIndex,
                            new[] { nullableDisableTrivia, newLine, newLine }
                        )
                    )
                );
            }
            else if (
                leadingDirective.SettingToken.IsKind(SyntaxKind.RestoreKeyword)
        ...
        private static SyntaxToken GetFirstTokenOfInterest(SyntaxNode root)
        {
            ...
            )
            {
                ...
                var nextToken = firstToken
                    .Parent.GetLastToken(includeDirectives: true)
        ...
        private static bool HasLeadingNullableDirective(
            SyntaxNode root,
            [NotNullWhen(true)] out NullableDirectiveTriviaSyntax? leadingNullableDirective
        )
        {
            // A leading nullable directive is a '#nullable' directive which precedes any conditional directives ('#if')
            // or code (non-trivia).
            var firstRelevantDirective = root.GetFirstDirective(static directive =>
                directive.Kind()
                    is SyntaxKind.NullableDirectiveTrivia
                        or SyntaxKind.IfDirectiveTrivia
            );
            if (
                firstRelevantDirective is NullableDirectiveTriviaSyntax nullableDirective
                && nullableDirective.TargetToken.IsKind(SyntaxKind.None)
            )
            {
                var firstSemanticToken = root.GetFirstToken();
                if (
                    firstSemanticToken.IsKind(SyntaxKind.None)
        ...
        private sealed class CustomCodeAction(
            ...
            : CodeAction.SolutionChangeAction(
                ...
                (progress, cancellationToken) =>
                    createChangedSolution(CodeActionPurpose.Apply, progress, cancellationToken),
