...
namespace Microsoft.CodeAnalysis.CSharp.CodeRefactorings.UseRecursivePatterns
{
    ...
    [
        ExportCodeRefactoringProvider(
            ...
            Name = PredefinedCodeRefactoringProviderNames.UseRecursivePatterns
        ),
        Shared
    ]
    internal sealed class UseRecursivePatternsCodeRefactoringProvider
        : SyntaxEditorBasedCodeRefactoringProvider
    {
        private static readonly PatternSyntax s_trueConstantPattern = ConstantPattern(
            LiteralExpression(TrueLiteralExpression)
        );
        private static readonly PatternSyntax s_falseConstantPattern = ConstantPattern(
        ...
        private static Func<SyntaxNode, SyntaxNode>? GetReplacementFunc(
            SyntaxNode? node,
            SemanticModel model
        ) =>
            node switch
        ...
        private static Func<SyntaxNode, SyntaxNode>? CombineLogicalAndOperands(
            BinaryExpressionSyntax logicalAnd,
            SemanticModel model
        )
        {
            if (
                TryDetermineReceiver(logicalAnd.Left, model)
                    is not var (leftReceiver, leftTarget, leftFlipped)
                || TryDetermineReceiver(logicalAnd.Right, model)
                    is not var (rightReceiver, rightTarget, rightFlipped)
            )
            {
                return null;
            }
            ...
            if (
                leftTarget.Parent is IsPatternExpressionSyntax isPatternExpression
                && TryFindVariableDesignation(isPatternExpression.Pattern, rightReceiver, model)
                    is var (containingPattern, rightNamesOpt)
            )
            {
                ...
                return root =>
                {
                    ...
                    var rewrittenPattern = RewriteContainingPattern(
                        containingPattern,
                        rightPattern,
                        rightNamesOpt
                    );
                    var replacement = isPatternExpression.ReplaceNode(
                        containingPattern,
                        rewrittenPattern
                    );
                    return root.ReplaceNode(
                        logicalAnd,
                        AdjustBinaryExpressionOperands(logicalAnd, replacement)
                    );
                };
            }

            if (
                TryGetCommonReceiver(leftReceiver, rightReceiver, model)
                is var (commonReceiver, leftNames, rightNames)
            )
            {
                return root =>
                {
                    var leftSubpattern = CreateSubpattern(
                        leftNames,
                        CreatePattern(leftReceiver, leftTarget, leftFlipped)
                    );
                    var rightSubpattern = CreateSubpattern(
                        rightNames,
                        CreatePattern(rightReceiver, rightTarget, rightFlipped)
                    );
                    var replacement = IsPatternExpression(
                        commonReceiver,
                        RecursivePattern(leftSubpattern, rightSubpattern)
                    );
                    return root.ReplaceNode(
                        logicalAnd,
                        AdjustBinaryExpressionOperands(logicalAnd, replacement)
            ...
            static SyntaxNode AdjustBinaryExpressionOperands(
                BinaryExpressionSyntax logicalAnd,
        ...
        private static Func<SyntaxNode, SyntaxNode>? CombineWhenClauseCondition(
            PatternSyntax switchPattern,
            ExpressionSyntax condition,
            SemanticModel model
        )
        {
            if (
                TryDetermineReceiver(condition, model, inWhenClause: true)
                    is not var (receiver, target, flipped)
                || TryFindVariableDesignation(switchPattern, receiver, model)
                    is not var (containingPattern, namesOpt)
            )
            {
                return null;
            }
        ...
        )
        {
            ...
            )
            {
                ...
                return (containingPattern, generatedPattern) switch
                {
                    ...
                    (
                        ...
                        RecursivePatternSyntax { Designation: null } recursive
                    ...
                    (
                        ...
                        RecursivePatternSyntax { Type: null, Designation: null } other
                    )
                        when recursive.PositionalPatternClause is null
                            || other.PositionalPatternClause is null => recursive
            ...
            static PatternSyntax AddSubpattern(
                PatternSyntax containingPattern,
            ...
            )
            {
                return containingPattern switch
                {
                    ...
                    DeclarationPatternSyntax p => RecursivePattern(
                        p.Type,
                        subpattern,
                        p.Designation
                    ),
            ...
            static PropertyPatternClauseSyntax? Concat(
                PropertyPatternClauseSyntax? left,
                PropertyPatternClauseSyntax? right
            )
            {
        ...
        private static PatternSyntax CreatePattern(
            ExpressionSyntax originalReceiver,
        ...
        )
        {
            return target switch
            {
                ...
                TypeSyntax type when originalReceiver.IsParentKind(IsExpression) => TypePattern(
                    type
                ),
        ...
        )
        {
            ...
            if (
                GetInnermostReceiver(rightReceiver, names, model)
            ...
            if (
                designation
                is not { Parent: PatternSyntax(not SyntaxKind.ListPattern) containingPattern }
            )
                return null;
            ...
            Debug.Assert(
                containingPattern.Kind()
                    is SyntaxKind.VarPattern
        ...
        )
        {
            return node switch
            {
                ...
                BinaryExpressionSyntax(
                    EqualsExpression
                        or NotEqualsExpression
                        or GreaterThanExpression
                        or GreaterThanOrEqualExpression
                        or LessThanOrEqualExpression
                        or LessThanExpression
                ) expr => TryDetermineConstant(expr, model),
                ...
                //
                BinaryExpressionSyntax(LogicalAndExpression) expr => TryDetermineReceiver(
            ...
            static (
                ...
                bool Flipped
            )? TryDetermineConstant(BinaryExpressionSyntax node, SemanticModel model)
            {
        ...
        private static SubpatternSyntax CreateSubpattern(
            ImmutableArray<IdentifierNameSyntax> names,
        ...
        )
        {
            ...
            if (
                names.Length > 1
                && names[0].SyntaxTree.Options.LanguageVersion() >= LanguageVersion.CSharp10
            )
            {
                ...
                return SyntaxFactory.Subpattern(
                    ExpressionColon(expression, Token(ColonToken)),
                    pattern
                );
            }
            else
        ...
        private static RecursivePatternSyntax RecursivePattern(
            ...
            SyntaxFactory.RecursivePattern(
                type,
                positionalPatternClause: null,
                PropertyPatternClause(SingletonSeparatedList(subpattern)),
                designation
            );
        ...
        /// </summary>
        private static (
            ExpressionSyntax CommonReceiver,
            ImmutableArray<IdentifierNameSyntax> LeftNames,
            ImmutableArray<IdentifierNameSyntax> RightNames
        )? TryGetCommonReceiver(ExpressionSyntax left, ExpressionSyntax right, SemanticModel model)
        {
            ...
            if (
                !TryGetInnermostReceiver(left, leftNames, out var leftReceiver, model)
                || !TryGetInnermostReceiver(right, rightNames, out var rightReceiver, model)
                || !AreEquivalent(leftReceiver, rightReceiver)
            ) // We must have a common starting point to proceed.
            {
                return null;
            }
            ...
            if (lastName is not null)
            {
                ...
                commonReceiver = GetInnermostReceiver(
                    left,
                    lastName,
                    static (identifierName, lastName) => identifierName != lastName
                );
            }
            ...
            return (
                ...
                leftNames.ToImmutable(),
                rightNames.ToImmutable()
            );

            static bool TryGetInnermostReceiver(
                ExpressionSyntax node,
            ...
            static IdentifierNameSyntax? SkipCommonNames(
                ArrayBuilder<IdentifierNameSyntax> leftNames,
                ArrayBuilder<IdentifierNameSyntax> rightNames
            )
            {
                ...
                for (
                    leftIndex = leftNames.Count - 1, rightIndex = rightNames.Count - 1;
        ...
        )
        {
            ...
            {
                return model.GetSymbolInfo(name).Symbol
                    is {
                        IsStatic: false,
        ...
        )
        {
            ...
            ExpressionSyntax? GetInnermostReceiver(ExpressionSyntax node)
            {
                switch (node)
                {
                    case IdentifierNameSyntax name when canConvertToSubpattern(name, arg):
                        builder?.Add(name);
