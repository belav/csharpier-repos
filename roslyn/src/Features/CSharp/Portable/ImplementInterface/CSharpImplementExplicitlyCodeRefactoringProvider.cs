...
namespace Microsoft.CodeAnalysis.CSharp.ImplementInterface
{
        ...
        : AbstractChangeImplementationCodeRefactoringProvider
    {
        ...
        )
        {
            ...
            var findRefsOptions = FindReferencesSearchOptions.Default with
            {
                Cascade = false,
            };
            var references = await SymbolFinder
                .FindReferencesAsync(implMember, solution, findRefsOptions, cancellationToken)
        ...
        {
            if (
                implMember is IMethodSymbol { Parameters: var implParameters }
                && interfaceMember is IMethodSymbol { Parameters: var interfaceParameters }
            )
            {
                for (
                    int i = 0, n = Math.Max(implParameters.Length, interfaceParameters.Length);
                    i < n;
                    i++
                )
                {
        ...
        private static bool DefaultValueMatches(
            IParameterSymbol parameterSymbol1,
            IParameterSymbol parameterSymbol2
        )
        {
            if (
                parameterSymbol1.HasExplicitDefaultValue != parameterSymbol2.HasExplicitDefaultValue
            )
                return false;

            if (parameterSymbol1.HasExplicitDefaultValue)
                return Equals(
                    parameterSymbol1.ExplicitDefaultValue,
                    parameterSymbol2.ExplicitDefaultValue
                );
