...
namespace Microsoft.CodeAnalysis.CSharp.SignatureHelp
{
    ...
    {
        private static readonly Func<TupleExpressionSyntax, SyntaxToken> s_getOpenToken = e =>
            e.OpenParenToken;
        private static readonly Func<TupleExpressionSyntax, SyntaxToken> s_getCloseToken = e =>
            e.CloseParenToken;
        ...
        )
        {
            ...
            if (
                GetOuterMostParenthesizedExpressionInSpan(
                    root,
                    position,
                    syntaxFacts,
                    currentSpan,
        ...
        public override Boolean IsRetriggerCharacter(Char ch) => ch == ')';

        public override Boolean IsTriggerCharacter(Char ch) => ch is '(' or ',';
        ...
        private SignatureHelpItems? CreateItems(
            int position,
            SyntaxNode root,
            ISyntaxFactsService syntaxFacts,
        ...
        {
            var prefixParts = SpecializedCollections
                .SingletonEnumerable(
                    new SymbolDisplayPart(SymbolDisplayPartKind.Punctuation, null, "(")
                )
                .ToTaggedText();
            var suffixParts = SpecializedCollections
                .SingletonEnumerable(
                    new SymbolDisplayPart(SymbolDisplayPartKind.Punctuation, null, ")")
                )
                .ToTaggedText();
        ...
        private static IEnumerable<SignatureHelpParameter> ConvertTupleMembers(
            INamedTypeSymbol tupleType,
            SemanticModel semanticModel,
            int position
        )
        {
            ...
            foreach (var element in tupleType.TupleElements)
            {
                ...
                if (!string.IsNullOrEmpty(elementName))
                {
                    ...
                    typeParts.Add(
                        new SymbolDisplayPart(SymbolDisplayPartKind.PropertyName, null, elementName)
                    );
                }
