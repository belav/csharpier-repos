...
namespace Microsoft.CodeAnalysis.CSharp.SignatureHelp
{
    ...
    [
        ExportSignatureHelpProvider(
            "PrimaryConstructorBaseTypeSignatureHelpProvider",
        ...
        : AbstractCSharpSignatureHelpProvider
    {
        ...
        private bool TryGetBaseTypeSyntax(
            ...
            out PrimaryConstructorBaseTypeSyntax expression
        )
        {
            if (
        ...
        private bool IsTriggerToken(SyntaxToken token) =>
            SignatureHelpUtilities.IsTriggerParenOrComma<PrimaryConstructorBaseTypeSyntax>(
                token,
                IsTriggerCharacter
            );
        ...
        )
        {
            ...
            if (
                root is null
                || syntaxFacts is null
                || !TryGetBaseTypeSyntax(
                    root,
                    position,
                    syntaxFacts,
                    triggerInfo.TriggerReason,
                    cancellationToken,
                    out var baseTypeSyntax
                )
            )
            {
                return null;
            ...
            var baseType =
                semanticModel.GetTypeInfo(baseTypeSyntax.Type, cancellationToken).Type
                as INamedTypeSymbol;
