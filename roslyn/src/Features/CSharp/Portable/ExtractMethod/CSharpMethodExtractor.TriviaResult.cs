    ...
    internal partial class CSharpMethodExtractor
    {
        private class CSharpTriviaResult : TriviaResult
        {
            ...
            )
            {
                ...
                return (node, location, annotation) =>
                    AnnotationResolver(node, location, annotation, callsite, method);
            }
            ...
            protected override TriviaResolver GetTriviaResolver(SyntaxNode method)
            {
                ...
                return (location, tokenPair, triviaMap) =>
                    TriviaResolver(location, tokenPair, triviaMap, method);
            }
            ...
            )
            {
                ...
                return location switch
                {
                    ...
                    TriviaLocation.AfterEndOfSpan => callsite
            ...
            )
            {
                ...
                return location switch
                {
                    TriviaLocation.BeforeBeginningOfSpan => FilterBeforeBeginningOfSpan(
                        tokenPair,
                        list
                    ),
                    TriviaLocation.AfterEndOfSpan => FilterTriviaList(
                        list.Concat(tokenPair.NextToken.LeadingTrivia)
                    ),
                    TriviaLocation.AfterBeginningOfSpan => FilterTriviaList(
            ...
            ) GetResolverElements(SyntaxNode method)
            {
            ...
            {
                if (
                    tokenPair.PreviousToken.RawKind
                    is ((int)SyntaxKind.OpenBraceToken)
                        or ((int)SyntaxKind.SemicolonToken)
                )
                {
