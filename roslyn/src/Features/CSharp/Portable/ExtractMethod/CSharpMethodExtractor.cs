    ...
    partial internal class CSharpMethodExtractor : MethodExtractor
    {
        ...
        )
        {
            ...
            if (LocalFunction)
            {
                ...
                var localFunctionNode = basePosition.GetAncestor<LocalFunctionStatementSyntax>(
                    node =>
                        (
                            node.Body != null
                            && node.Body.Span.Contains(OriginalSelectionResult.OriginalSpan)
                        )
                        || (
                ...
                {
                    return await InsertionPoint
                        .CreateAsync(document, localFunctionNode, cancellationToken)
                        .ConfigureAwait(false);
                }
            }
            ...
            if (LocalFunction && memberNode is BasePropertyDeclarationSyntax propertyDeclaration)
            {
                ...
                {
                    return await InsertionPoint
                        .CreateAsync(document, accessorNode, cancellationToken)
                        .ConfigureAwait(false);
                }
            }
            ...
            if (memberNode is GlobalStatementSyntax globalStatement)
            {
                ...
                {
                    return await InsertionPoint
                        .CreateAsync(document, globalStatement.Parent, cancellationToken)
                        .ConfigureAwait(false);
                }
                ...
                return await InsertionPoint
                    .CreateAsync(document, globalStatement.Statement, cancellationToken)
                    .ConfigureAwait(false);
            }

            return await InsertionPoint
                .CreateAsync(document, memberNode, cancellationToken)
                .ConfigureAwait(false);
        }
        ...
        protected override ImmutableArray<AbstractFormattingRule> GetCustomFormattingRules(
            Document document
