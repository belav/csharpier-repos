    ...
    : AbstractConvertStringProvider<InterpolatedStringExpressionSyntax>
{
    public static readonly IConvertStringProvider Instance =
        new ConvertInterpolatedStringToRawStringProvider();
    ...
    private static VirtualCharSequence ConvertToVirtualChars(
        InterpolatedStringTextSyntax textSyntax
    )
    {
    ...
    private static VirtualCharSequence TryConvertToVirtualChars(
        InterpolatedStringTextSyntax textSyntax
    ...
    )
    {
        ...
        if (
            stringExpression.StringStartToken.Kind()
        ...
        foreach (var content in stringExpression.Contents)
        {
            if (content is InterpolationSyntax interpolation)
            {
                ...
                {
                    var characters = TryConvertToVirtualChars(
            ...
            else if (content is InterpolatedStringTextSyntax interpolatedStringText)
            {
                ...
                if (canBeSingleLine)
                {
                    ...
                    if (characters.Any(static ch => IsCSharpNewLine(ch)))
                    {
                        ...
                        interpolatedStringText == firstContent
                    ...
                    )
                    {
                        ...
                        interpolatedStringText == lastContent
        ...
        {
            var converted = GetInitialMultiLineRawInterpolatedString(
                stringExpression,
                formattingOptions
            );
    ...
    )
    {
        ...
        if (token.SpanStart == tokenLine.Start)
        {
            ...
            newNode = newNode.WithLeadingTrivia(
                newNode.GetLeadingTrivia().Add(Whitespace(indentation))
            );
            return newNode;
        }
        else
        ...
        )
        {
            ...
            var (interpolationInteriorSpans, restrictedSpans) = GetInterpolationSpans(
                stringExpression,
                cancellationToken
            );
            ...
            for (int i = 1, n = text.Lines.Count; i < n; i++)
            {
                ...
                if (interpolationInteriorSpans.Any(s => s.Contains(line.Start)))
                {
                    ...
                    var positionIndentation = GetIndentationStringForPosition(
                        text,
                        formattingOptions,
                    ...
                    builder.Append(
                        text.ToString(
                            TextSpan.FromBounds(firstNonWhitespacePos, line.EndIncludingLineBreak)
                        )
                    );
                }
                else
    ...
    )
    {
        ...
        if (
            interpolationLine.LineNumber > 1
        ...
        )
        {
            ...
            return GetIndentationStringForPosition(
                text,
                formattingOptions,
    ...
    ) GetDelimiters(InterpolatedStringExpressionSyntax stringExpression)
    {
    ...
    private static (int longestQuoteSequence, int longestBraceSequence) GetLongestSequences(
        InterpolatedStringExpressionSyntax stringExpression
    )
    {
        ...
        foreach (var content in stringExpression.Contents)
        {
            if (content is InterpolatedStringTextSyntax stringText)
            {
                ...
                longestQuoteSequence = Math.Max(
                    longestQuoteSequence,
                    GetLongestQuoteSequence(characters)
                );
                longestBraceSequence = Math.Max(
                    longestBraceSequence,
                    GetLongestBraceSequence(characters)
                );
            }
        }
    ...
    )
    {
        ...
        foreach (var content in stringExpression.Contents)
        {
            if (content is InterpolationSyntax interpolation)
            {
                contents.Add(
                    interpolation
                        .WithOpenBraceToken(
                            UpdateToken(interpolation.OpenBraceToken, openBraceString)
                        )
                        .WithFormatClause(RewriteFormatClause(interpolation.FormatClause))
                        .WithCloseBraceToken(
                            UpdateToken(interpolation.CloseBraceToken, closeBraceString)
                        )
                );
            }
            else if (content is InterpolatedStringTextSyntax stringText)
            {
                var characters = ConvertToVirtualChars(stringText);
                contents.Add(
                    stringText.WithTextToken(
                        UpdateToken(stringText.TextToken, characters.CreateString())
                    )
                );
            }
        }
        ...
        static InterpolationFormatClauseSyntax? RewriteFormatClause(
        ...
        )
        {
            ...
            return formatClause.WithFormatStringToken(
                UpdateToken(formatClause.FormatStringToken, characters.CreateString())
            );
        }
    }

    private static string GetIndentationStringForToken(
        SourceText text,
        SyntaxFormattingOptions options,
        SyntaxToken token
    ) => GetIndentationStringForPosition(text, options, token.SpanStart);

    private static string GetIndentationStringForPosition(
        SourceText text,
        SyntaxFormattingOptions options,
        int position
    )
    ...
    private static (
        ...
        TextSpanIntervalTree restrictedSpans
    ) GetInterpolationSpans(
        InterpolatedStringExpressionSyntax stringExpression,
        CancellationToken cancellationToken
    )
    {
        ...
        foreach (var content in stringExpression.Contents)
        {
            if (content is InterpolationSyntax interpolation)
            {
                interpolationInteriorSpans.AddIntervalInPlace(
                    TextSpan.FromBounds(
                        interpolation.OpenBraceToken.Span.End,
                        interpolation.CloseBraceToken.Span.Start
                    )
                );
                ...
                foreach (
                    var descendant in interpolation.DescendantNodes().OfType<ExpressionSyntax>()
                )
                {
                    if (
                        descendant
                            is LiteralExpressionSyntax(kind: SyntaxKind.StringLiteralExpression)
                    ...
                    )
                    {
                        ...
                        if (startLine != text.Lines.GetLineFromPosition(descendantSpan.End))
                        {
                            ...
                            var start =
                                startLine.GetFirstNonWhitespacePosition() == descendantSpan.Start
                                    ? startLine.Start
                                    : descendantSpan.Start;
                            restrictedSpans.AddIntervalInPlace(
                                TextSpan.FromBounds(start, descendantSpan.End)
                            );
                        }
                    }
                }
    ...
    )
    {
        ...
        var commonWhitespacePrefix = ComputeCommonWhitespacePrefix(
            lines,
                ...
                if (line.GetFirstNonWhitespacePosition() is int pos)
                {
                    ...
                    if (currentLineLeadingWhitespace.StartsWith(commonWhitespacePrefix))
                    {
                        builder.Append(
                            text.ToString(
                                TextSpan.FromBounds(
                                    line.Start + commonWhitespacePrefix.Length,
    ...
    )
    {
        ...
        for (int i = 1, n = lines.Count - 1; i < n; i++)
        {
            ...
            if (
                interpolationInteriorSpans.Any(s => s.Contains(line.Start))
    ...
    private static string ComputeCommonWhitespacePrefix(
        string? leadingWhitespace1,
        string leadingWhitespace2
    )
    {
        ...
        while (
            current < length
            && SyntaxFacts.IsWhitespace(leadingWhitespace1[current])
            && leadingWhitespace1[current] == leadingWhitespace2[current]
        )
            current++;
    ...
    public static SyntaxToken UpdateToken(
        SyntaxToken token,
        string text,
        string valueText = "",
