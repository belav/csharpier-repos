...
[
    ExportCodeRefactoringProvider(
        ...
        Name = PredefinedCodeRefactoringProviderNames.ConvertPrimaryToRegularConstructor
    ),
    Shared
]
    ...
    : CodeRefactoringProvider
{
    public override async Task ComputeRefactoringsAsync(CodeRefactoringContext context)
    {
        ...
        var triggerSpan = TextSpan.FromBounds(
            typeDeclaration.SpanStart,
            typeDeclaration.ParameterList.FullSpan.End
        );
    ...
    )
    {
        ...
        var baseType = typeDeclaration.BaseList?.Types
            ...
            ? type
            : null;
        var methodTargetingAttributes = typeDeclaration.AttributeLists
        ...
        async Task<
            MultiDictionary<IParameterSymbol, IdentifierNameSyntax>
        > GetParameterReferencesAsync()
        {
            ...
            foreach (var parameter in parameters)
            {
                var references = await SymbolFinder
                    .FindReferencesAsync(parameter, solution, documentsToSearch, cancellationToken)
                    .ConfigureAwait(false);
                foreach (var reference in references)
                {
                    ...
                    foreach (
                        var referenceLocation in reference.Locations
        ...
        {
            using var _1 = PooledDictionary<Location, IFieldSymbol>.GetInstance(
                out var locationToField
            );
            using var _2 = PooledDictionary<IParameterSymbol, IFieldSymbol>.GetInstance(
                out var result
            );
            ...
            {
                if (
                    parameter.Locations is [var location, ..]
                    && locationToField.TryGetValue(location, out var existingField)
                )
                {
                    var baseFieldName = fieldNameRule.NamingStyle
                    ...
                    var synthesizedField = CodeGenerationSymbolFactory.CreateFieldSymbol(
                        ...
                        modifiers: isWrittenTo
                            ...
                            : existingField.GetSymbolModifiers().WithIsReadOnly(true),
                        name: fieldName
                    );
        ...
        async Task<
            ImmutableHashSet<(ISymbol fieldOrProperty, EqualsValueClauseSyntax initializer)>
        > GetExistingAssignedFieldsOrPropertiesAsync()
        {
            ...
            foreach (var (parameter, references) in parameterReferences)
            {
                foreach (var reference in references)
                {
                    ...
                    if (
                        initializer.Parent
                        is not PropertyDeclarationSyntax
            ...
            foreach (var grouping in initializers.GroupBy(kvp => kvp.Value.SyntaxTree))
            {
                ...
                var semanticModel = await GetSemanticModelAsync(
                        solution.GetRequiredDocument(syntaxTree)
                    )
                    .ConfigureAwait(false);

                foreach (var initializer in grouping)
                {
                    var fieldOrProperty = semanticModel.GetRequiredDeclaredSymbol(
                        initializer.GetRequiredParent(),
                        cancellationToken
                    );
        ...
        void AddConstructorDeclaration()
        {
            mainDocumentEditor.ReplaceNode(
                ...
                (current, _) =>
                {
                    ...
                    var firstConstructorIndex = currentTypeDeclaration.Members
                        .IndexOf(m =>
                    ...
                    var lastFieldOrProperty = currentTypeDeclaration.Members
                        .LastIndexOf(m => m is FieldDeclarationSyntax);
                    if (lastFieldOrProperty < 0)
                        lastFieldOrProperty = currentTypeDeclaration.Members
                ...
                foreach (var grouping in references.GroupBy(r => r.SyntaxTree))
                {
                    ...
                    var editor = await solutionEditor
                        .GetDocumentEditorAsync(
                            solution.GetDocumentId(syntaxTree),
                            cancellationToken
                        )
                        .ConfigureAwait(false);
                    ...
                    foreach (var identifierName in grouping)
                    {
                        ...
                        if (xmlElement is { Name.LocalName.ValueText: "paramref" })
                        {
                            var seeTag = xmlElement
                                .ReplaceToken(
                                    ...
                                    Identifier("see").WithTriviaFrom(xmlElement.Name.LocalName)
                                )
                                .WithAttributes(
                        ...
                        else
                        {
                            editor.ReplaceNode(
                                identifierName,
                                fieldName.WithTriviaFrom(identifierName)
                            );
                        }
                    }
                }
        ...
        void FixParameterAndBaseArgumentListIndentation()
        {
            ...
            var rewrittenParameterList = AddElementIndentation(
                typeDeclaration,
                constructor,
            ...
            where TListSyntax : SyntaxNode
        {
            ...
            if (
                constructorLeadingWhitespace.Length > typeLeadingWhitespace.Length
        ...
        static string GetLeadingWhitespace(SyntaxNode node) =>
            node.GetLeadingTrivia() is [.., (kind: SyntaxKind.WhitespaceTrivia) whitespace]
                ? whitespace.ToString()
                : "";
        ...
        ConstructorDeclarationSyntax CreateConstructorDeclaration()
        {
            ...
            foreach (var parameter in parameters)
            {
                ...
                var assignment = AssignmentExpression(
                    ...
                    left,
                    parameter.Name.ToIdentifierName()
                );
            ...
            )
            {
                var left = MemberAccessExpression(
                        SyntaxKind.SimpleMemberAccessExpression,
                        ThisExpression(),
                        fieldOrProperty.Name.ToIdentifierName()
                    )
        ...
        TNode RewriteNestedReferences<TNode>(TNode parent)
            where TNode : SyntaxNode
        {
            return parent.ReplaceNodes(
                parent
                    .DescendantNodes()
                    .Where(n => n is MemberAccessExpressionSyntax or QualifiedNameSyntax),
                (node, _) =>
                {
                    if (
                        node is MemberAccessExpressionSyntax memberAccessExpression
                        && namedType.Equals(
                    ...
                    )
                    {
                        ...
                        node is QualifiedNameSyntax qualifiedName
