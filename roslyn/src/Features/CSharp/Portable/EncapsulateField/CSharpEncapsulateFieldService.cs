    ...
    internal class CSharpEncapsulateFieldService : AbstractEncapsulateFieldService
    {
        ...
        )
        {
            ...
            var updatedDeclarator = declarator
                ...
                .WithAdditionalAnnotations(tempAnnotation);

            root = root.ReplaceNode(declarator, updatedDeclarator);
            document = document.WithSyntaxRoot(root);

            var declaration =
                root.GetAnnotatedNodes<SyntaxNode>(tempAnnotation).First().Parent
            ...
            if (declaration.Variables.Count == 1)
            {
                ...
                if (makePrivate)
                {
                    var modifiers = SpecializedCollections
                        .SingletonEnumerable(SyntaxFactory.Token(SyntaxKind.PrivateKeyword))
                        .Concat(
            ...
            else if (declaration.Variables.Count > 1 && makePrivate)
            {
                ...
                declarator = root.GetAnnotatedNodes<VariableDeclaratorSyntax>(tempAnnotation)
                    .First();
                declaration = declarator.Parent as VariableDeclarationSyntax;

                var field =
                    semanticModel.GetDeclaredSymbol(declarator, cancellationToken) as IFieldSymbol;
                ...
                var withField = await codeGenService
                    .AddFieldAsync(
        ...
        protected override async Task<ImmutableArray<IFieldSymbol>> GetFieldsAsync(
            Document document,
            TextSpan span,
            CancellationToken cancellationToken
        )
        {
            ...
            else
            {
                ...
                declarators = declarations.SelectMany(d =>
                    d.Variables.Where(v => v.Span.IntersectsWith(span))
                );
            }
        ...
        )
        {
            ...
            if (field.DeclaredAccessibility == Accessibility.Private || IsNew(field))
            {
                ...
                return (
                    field.Name,
                    MakeUnique(GeneratePropertyName(field.Name), field.ContainingType)
                );
            }
            else
            {
                ...
                if (newPropertyName == field.Name)
                {
                    ...
                    return (
                        MakeUnique(GenerateFieldName(field.Name), field.ContainingType),
                        newPropertyName
                    );
                }
        ...
        private static bool IsNew(IFieldSymbol field) =>
            field.DeclaringSyntaxReferences.Any(static d =>
                d.GetSyntax()
        ...
        {
            var containingTypeMemberNames = containingType
                .GetAccessibleMembersInThisAndBaseTypes<ISymbol>(containingType)
                .Select(m => m.Name);
        ...
        internal override IEnumerable<SyntaxNode> GetConstructorNodes(
            ...
            containingType.Constructors.SelectMany(c =>
                c.DeclaringSyntaxReferences.Select(d => d.GetSyntax())
            );
    }
}
