...
namespace Microsoft.CodeAnalysis.CSharp.EncapsulateField
{
    ...
    internal class CSharpEncapsulateFieldService : AbstractEncapsulateFieldService
    {
            ...
            if (declaration.Variables.Count == 1)
            {
                ...
                if (makePrivate)
                {
                    ...
                    root = root.ReplaceNode(
                        fieldSyntax,
                        fieldSyntax.WithModifiers(SyntaxFactory.TokenList(modifiers))
        ...
        {
            var containingTypeMemberNames =
                containingType.GetAccessibleMembersInThisAndBaseTypes<ISymbol>(containingType)
                    .Select(m => m.Name);
