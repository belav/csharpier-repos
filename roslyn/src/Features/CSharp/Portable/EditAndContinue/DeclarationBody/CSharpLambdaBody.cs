...
internal sealed class CSharpLambdaBody(SyntaxNode node) : LambdaBody
{
    ...
    public override OneOrMany<SyntaxNode> RootNodes => OneOrMany.Create(node);

    public override SyntaxNode EncompassingAncestor => node;
    ...
    public override LambdaBody? TryGetPartnerLambdaBody(SyntaxNode newLambda) =>
        LambdaUtilities.TryGetCorrespondingLambdaBody(node, newLambda) is { } newNode
            ? new CSharpLambdaBody(newNode)
            : null;

    public override IEnumerable<SyntaxNode> GetExpressionsAndStatements() =>
        SpecializedCollections.SingletonEnumerable(node);

    public override SyntaxNode GetLambda() => LambdaUtilities.GetLambda(node);
