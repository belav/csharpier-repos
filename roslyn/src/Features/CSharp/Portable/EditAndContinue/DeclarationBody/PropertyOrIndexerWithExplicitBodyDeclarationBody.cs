...
) : PropertyOrIndexerAccessorDeclarationBody
{
    public ArrowExpressionClauseSyntax Body =>
        (propertyOrIndexer is PropertyDeclarationSyntax property)
            ? property.ExpressionBody!
            : ((IndexerDeclarationSyntax)propertyOrIndexer).ExpressionBody!;

    public ExpressionSyntax BodyExpression => Body.Expression;

    public override SyntaxNode? ExplicitBody => BodyExpression;

    public override SyntaxNode? HeaderActiveStatement => null;

    public override TextSpan HeaderActiveStatementSpan => default;

    public override SyntaxNode? MatchRoot => Body;
