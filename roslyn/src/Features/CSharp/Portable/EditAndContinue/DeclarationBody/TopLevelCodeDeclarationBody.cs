...
internal sealed class TopLevelCodeDeclarationBody(CompilationUnitSyntax unit) : MemberBody
{
    ...
    public override ImmutableArray<ISymbol> GetCapturedVariables(SemanticModel model) =>
        model
            .AnalyzeDataFlow(
                ...
                GlobalStatements.Last().Statement
            )!
            .Captured;
    ...
    public override SyntaxNode EncompassingAncestor => unit;
    ...
    {
        var isAsync = GlobalStatements.Any(
            static s => SyntaxUtilities.GetSuspensionPoints(s).Any()
        );
        return new StateMachineInfo(
            IsAsync: isAsync,
            IsIterator: false,
            HasSuspensionPoints: isAsync
        );
    }
    ...
    public override Match<SyntaxNode>? ComputeSingleRootMatch(
        ...
        CSharpEditAndContinueAnalyzer.ComputeBodyMatch(
            Unit,
            ((TopLevelCodeDeclarationBody)newBody).Unit,
            knownMatches
        );
