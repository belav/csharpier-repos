...
namespace Microsoft.CodeAnalysis.CSharp.BraceMatching
{
    ...
    internal class BlockCommentBraceMatcher : IBraceMatcher
    {
        ...
        )
        {
            ...
            if (
                trivia.Kind() is SyntaxKind.MultiLineCommentTrivia
                && trivia.ToString() is ['/', '*', .., '*', '/']
            )
            {
                return new BraceMatchingResult(
                    new TextSpan(trivia.SpanStart, "/*".Length),
                    TextSpan.FromBounds(trivia.Span.End - "*/".Length, trivia.Span.End)
                );
            }
            else if (trivia.Kind() is SyntaxKind.MultiLineDocumentationCommentTrivia)
            {
                ...
                var endBrace = TextSpan.FromBounds(
