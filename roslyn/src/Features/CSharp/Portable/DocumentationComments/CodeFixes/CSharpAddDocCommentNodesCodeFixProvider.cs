...
namespace Microsoft.CodeAnalysis.DiagnosticComments.CodeFixes
{
    [
        ExportCodeFixProvider(
            ...
            Name = PredefinedCodeFixProviderNames.AddDocCommentNodes
        ),
        Shared
    ]
          ...
          >
    {
        ...
        {
            var docCommentNodes = leadingTrivia.Where(
                f => f.IsKind(SyntaxKind.SingleLineDocumentationCommentTrivia)
            );

            foreach (var node in docCommentNodes)
            {
                var nodeStructure = node.GetStructure();
                var descendentXmlElements = nodeStructure.DescendantNodes()
        ...
        {
            var parameterList = (ParameterListSyntax)member.DescendantNodes(
        ...
        )
        {
            ...
            var newDocCommentNode = SyntaxFactory.DocumentationComment(
            ...
            {
                return elementNode.WithTrailingTrivia(
                    SyntaxFactory.ParseTrailingTrivia(Environment.NewLine)
                );
            }
