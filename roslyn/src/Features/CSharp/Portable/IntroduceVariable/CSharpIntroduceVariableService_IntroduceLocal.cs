...
namespace Microsoft.CodeAnalysis.CSharp.IntroduceVariable
{
    internal partial class CSharpIntroduceVariableService
    {
        ...
        )
        {
            var containerToGenerateInto = expression.Ancestors()
                .FirstOrDefault(
                    s =>
                        s is BlockSyntax
                        || s is ArrowExpressionClauseSyntax
                        || s is LambdaExpressionSyntax
                );
            ...
            var text = await document.Document.GetTextAsync(cancellationToken)
        ...
        )
        {
            ...
            newBody = newBody.WithOpenBraceToken(
        ...
        private static SyntaxNode WithBlockBody(SyntaxNode node, BlockSyntax body)
        {
            switch (node)
            {
                case BasePropertyDeclarationSyntax baseProperty:
                    ...
                    return baseProperty.TryWithExpressionBody(null)
                ...
                case BaseMethodDeclarationSyntax baseMethod:
                    return baseMethod.WithExpressionBody(null)
                        .WithBody(body)
                        .WithSemicolonToken(SyntaxFactory.Token(SyntaxKind.None))
                        .WithTriviaFrom(baseMethod);
                case LocalFunctionStatementSyntax localFunction:
                    return localFunction.WithExpressionBody(null)
                        .WithBody(body)
                        .WithSemicolonToken(SyntaxFactory.Token(SyntaxKind.None))
        ...
        )
        {
            ...
            var localFunctions = innermostCommonBlock.DescendantNodes()
                .Where(
                    node =>
            ...
            var localFunctionCalls = innermostCommonBlock.DescendantNodes()
                .Where(
                    node =>
