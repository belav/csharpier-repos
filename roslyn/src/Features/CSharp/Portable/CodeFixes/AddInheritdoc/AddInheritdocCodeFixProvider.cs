...
namespace Microsoft.CodeAnalysis.CSharp.CodeFixes.AddInheritdoc
{
    [
        ExportCodeFixProvider(
            ...
            Name = PredefinedCodeFixProviderNames.AddInheritdoc
        ),
        Shared
    ]
    internal sealed class AddInheritdocCodeFixProvider : SyntaxEditorBasedCodeFixProvider
    {
        ...
        public override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            foreach (var diagnostic in context.Diagnostics)
            {
                ...
                if (
                    node.Kind()
                    is not SyntaxKind.MethodDeclaration
                        and not SyntaxKind.PropertyDeclaration
                ...
                if (
                    ...
                    && node.Parent?.Parent?.IsKind(SyntaxKind.EventFieldDeclaration) == false
                )
                {
                    continue;
                }
                ...
                {
                    if (symbol.IsOverride || symbol.ImplicitInterfaceImplementations().Any())
                    {
                        context.RegisterCodeFix(
                            new MyCodeAction(
                                FeaturesResources.Explicitly_inherit_documentation,
        ...
        )
        {
            ...
            foreach (var diagnostic in diagnostics)
            {
                ...
                if (
                    node.IsKind(SyntaxKind.VariableDeclarator)
                    && !(node = node.Parent?.Parent).IsKind(SyntaxKind.EventFieldDeclaration)
                )
                {
                    continue;
                }
                ...
                ).GetOption(FormattingOptions2.NewLine);
                ...
                var xmlSpaceAfterTripleSlash = Token(
                    leading: TriviaList(DocumentationCommentExterior("///")),
                ...
                var indentation = sourceText.GetLeadingWhitespaceOfLineAtPosition(
                    node.FullSpan.Start
                );
