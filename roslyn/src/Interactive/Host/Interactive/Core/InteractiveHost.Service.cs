...
namespace Microsoft.CodeAnalysis.Interactive
{
    internal partial class InteractiveHost
    {
        ...
        {
            private static readonly ManualResetEventSlim s_clientExited = new ManualResetEventSlim(
                false
            );
            ...
            private static readonly InteractiveHostPlatformInfo s_currentPlatformInfo =
                InteractiveHostPlatformInfo.GetCurrentPlatformInfo();
            ...
            private sealed class ServiceState : IDisposable
            {
                ...
                public void Dispose() => MetadataFileProvider.Dispose();
            }
                ...
                var referenceResolver = new RuntimeMetadataReferenceResolver(
                    ...
                    (path, properties) =>
                        new ShadowCopyReference(
                            GetServiceState().MetadataFileProvider,
                            path,
                            properties
                        )
                );
            ...
            )
            {
                ...
                var jsonRpc = CreateRpc(
                    serverStream,
                    incomingCallTarget: new Service(invokeOnMainThread)
                );
            ...
            )
            {
                ...
                lock (_lastTaskGuard)
                {
                    _lastTask = InitializeContextAsync(
                        _lastTask,
                        completionSource,
            ...
            private async Task<EvaluationState> AddReferenceAsync(
                Task<EvaluationState> lastTask,
                ...
                try
                {
                    ...
                    if (!resolvedReferences.IsDefaultOrEmpty)
                    {
                        state = state.WithOptions(
                            state.ScriptOptions.AddReferences(resolvedReferences)
                        );
                        success = true;
                    }
                    else
            ...
            private async Task<EvaluationState> ExecuteAsync(
                TaskCompletionSource<RemoteExecutionResult> completionSource,
                Task<EvaluationState> lastTask,
                string text
            )
            {
                ...
                try
                {
                    Script<object>? script = TryCompile(
                        state.ScriptState?.Script,
                        text,
                        null,
                        state.ScriptOptions
                    );
            ...
            private EvaluationState CompleteExecution(
            ...
            )
            {
                ...
                var referencePathsChanged = !newReferencePaths.SequenceEqual(
                    metadataResolver.SearchPaths
                );
                ...
                {
                    newOptions = newOptions.WithSourceResolver(
                        CreateSourceReferenceResolver(newSourcePaths, newWorkingDirectory)
                    );
                }
                    ...
                    if (File.Exists(initializationFilePath))
                    {
                        ...
                        if (args.Errors.Length == 0)
                        {
                            ...
                            var rspMetadataResolver = state
                                .MetadataReferenceResolver.WithSearchPaths(referencePaths)
                            ...
                            var rspState = new EvaluationState(
                                state.ScriptState,
                                state
                                    .ScriptOptions.WithFilePath(initializationScriptPath)
            ...
            )
            {
                ...
                var fullPath = ResolveRelativePath(
                    ...
                    state.SourceSearchPaths,
                    displayPath: false
                );
                if (fullPath != null)
                {
                    var newScriptState = await TryExecuteFileAsync(state, fullPath)
                        .ConfigureAwait(false);
                    if (newScriptState != null)
                    {
                        return CompleteExecution(
                            state.WithScriptState(newScriptState),
                            completionSource,
                            success: newScriptState.Exception == null
                        );
                    }
                }
            ...
            )
            {
                ...
                return await ExecuteOnUIThreadAsync(script, state.ScriptState, displayResult: false)
                    .ConfigureAwait(false);
            }
            ...
            {
                var directories = attemptedFilePaths
            ...
            private Task<ScriptState<object>> ExecuteOnUIThreadAsync(
            ...
            )
            {
                ...
                {
                    output.WriteLine(
                        formatter.Format(diagnostic, output.FormatProvider as CultureInfo)
                    );
                }
