...
namespace Roslyn.VisualStudio.NewIntegrationTests.InProcess
{
    ...
    internal partial class InheritanceMarginInProcess
    {
        ...
        {
            var optionService = await GetComponentModelServiceAsync<IGlobalOptionService>(
                cancellationToken
            );
            var showInheritanceMargin = optionService.GetOption(
                FeatureOnOffOptions.ShowInheritanceMargin,
                languageName
            );
            var combinedWithIndicatorMargin = optionService.GetOption(
                FeatureOnOffOptions.InheritanceMarginCombinedWithIndicatorMargin
            );
            var showGlobalUsings = optionService.GetOption(
                FeatureOnOffOptions.InheritanceMarginIncludeGlobalImports,
                languageName
            );

            if (showInheritanceMargin != true)
            {
                optionService.SetGlobalOption(
                    new OptionKey(FeatureOnOffOptions.ShowInheritanceMargin, languageName),
                    true
                );
            }
            ...
            if (combinedWithIndicatorMargin)
            {
                ...
                optionService.SetGlobalOption(
                    new OptionKey(FeatureOnOffOptions.InheritanceMarginCombinedWithIndicatorMargin),
                    false
                );
            }
        }

        public async Task DisableOptionsAsync(
            string languageName,
            CancellationToken cancellationToken
        )
        {
        ...
        private async Task EnsureGlyphsAppearAsync(
            Func<CancellationToken, Task> makeChangeFunc,
            int expectedGlyphsNumberInMargin,
            CancellationToken cancellationToken
        )
        {
        ...
        public Task EnableOptionsAndEnsureGlyphsAppearAsync(
            string languageName,
            int expectedGlyphsNumberInMargin,
            CancellationToken cancellationToken
        ) =>
            EnsureGlyphsAppearAsync(
                cts => EnableOptionsAsync(languageName, cts),
                expectedGlyphsNumberInMargin,
                cancellationToken
            );

        public Task SetTextAndEnsureGlyphsAppearAsync(
            string text,
            int expectedGlyphsNumberInMargin,
            CancellationToken cancellationToken
        ) =>
            EnsureGlyphsAppearAsync(
                cts => TestServices.Editor.SetTextAsync(text, cts),
                expectedGlyphsNumberInMargin,
                cancellationToken
            );
        ...
        private async Task<IWpfTextViewMargin> GetTextViewMarginAsync(
            CancellationToken cancellationToken
        )
        {
            await WaitForApplicationIdleAsync(cancellationToken);
            ...
            var vsTextView = await vsTextManager.GetActiveViewAsync(
                JoinableTaskFactory,
                cancellationToken
            );
            var testViewHost = await vsTextView.GetTextViewHostAsync(
                JoinableTaskFactory,
                cancellationToken
            );
        ...
        private async Task<Point> GetCenterOfGlyphOnScreenAsync(
