...
namespace Roslyn.VisualStudio.NewIntegrationTests.InProcess
{
    ...
    {
        private static readonly Func<string, string, bool> s_equals = (expected, actual) =>
            actual.Equals(expected);
        private static readonly Func<string, string, bool> s_contains = (expected, actual) =>
            actual.Contains(expected);
        private static readonly Func<string, string, bool> s_endsWith = (expected, actual) =>
        ...
        private async Task<IInteractiveWindow> GetInteractiveWindowAsync(
            CancellationToken cancellationToken
        )
        {
            await JoinableTaskFactory.SwitchToMainThreadAsync(cancellationToken);

            var provider = await GetComponentModelServiceAsync<CSharpVsInteractiveWindowProvider>(
                cancellationToken
            );
        ...
        public async Task<string> GetLastReplInputAsync(CancellationToken cancellationToken)
        {
            ...
            var lastReplInputWithReplSubmissionText =
                (firstNewLineIndex <= 0) ? replText : replText[..firstNewLineIndex];
        ...
        public async Task WaitForReplOutputAsync(
            string outputText,
            CancellationToken cancellationToken
        ) =>
            await WaitForPredicateAsync(
                GetReplTextAsync,
                outputText + Environment.NewLine + ReplPromptText,
                s_endsWith,
                "end with",
                cancellationToken
            );

        public async Task WaitForLastReplOutputAsync(
            string outputText,
            CancellationToken cancellationToken
        ) =>
            await WaitForPredicateAsync(
                GetLastReplOutputAsync,
                outputText,
                s_equals,
                "is",
                cancellationToken
        ...
        public async Task WaitForLastReplOutputContainsAsync(
            string outputText,
            CancellationToken cancellationToken
        ) =>
            await WaitForPredicateAsync(
        ...
        public async Task WaitForLastReplInputAsync(
            string outputText,
            CancellationToken cancellationToken
        ) =>
            await WaitForPredicateAsync(
                GetLastReplInputAsync,
                outputText,
                s_equals,
                "is",
                cancellationToken
        ...
        public async Task WaitForLastReplInputContainsAsync(
            string outputText,
            CancellationToken cancellationToken
        ) =>
            await WaitForPredicateAsync(
        ...
        public async Task VerifyTagsAsync<TTag>(
            int expectedCount,
            CancellationToken cancellationToken
        )
            where TTag : ITag
        {
            ...
            if (expectedCount != actualCount)
            {
                ...
                throw new Exception(
                    $"Failed to verify '{typeof(TTag)}' tags. Expected count: {expectedCount}, Actual count: {actualCount}. All tags: {tagsTypesString}"
                );
            }
        }

        private static async Task WaitForPredicateAsync(
            Func<CancellationToken, Task<string>> getValue,
