...
{
    [TestService]
    partial internal class PreviewChangesDialogInProcess
    {
        /// <summary>
        ...
        {
            await DialogHelpers.FindDialogByNameAsync(
                JoinableTaskFactory,
                await TestServices.Shell.GetMainWindowAsync(cancellationToken),
                expectedTitle,
                isOpen: true,
                cancellationToken
            );
        ...
        public async Task VerifyClosedAsync(
        ...
        public async Task ClickApplyAndWaitForFeatureAsync(
            string expectedTitle,
            string featureName,
            CancellationToken cancellationToken
        )
        {
            await DialogHelpers.PressButtonWithNameFromDialogWithNameAsync(
                JoinableTaskFactory,
                await TestServices.Shell.GetMainWindowAsync(cancellationToken),
                expectedTitle,
                "Apply",
