...
namespace Roslyn.VisualStudio.NewIntegrationTests.CSharp
{
    ...
    public class CSharpIntelliSense : AbstractEditorTest
    {
        ...
        public override async Task InitializeAsync()
        {
            ...
            await TestServices.StateReset.DisableResponsiveCompletion(
        ...
        public async Task AtNamespaceLevel(bool showCompletionInArgumentLists)
        {
            ...
            Assert.Contains(
                "using",
                (
                    await TestServices.Editor.GetCompletionItemsAsync(
                        HangMitigatingCancellationToken
                    )
...
class C
{
    public T Goo<T>() { }$$
        ...
        [IdeTheory, CombinatorialData]
        public async Task VerifyCompletionListMembersOnStaticTypesAndCompleteThem(
    ...
    public static void Navigate(int i){ }
        ...
        public async Task CtrlAltSpace(bool showCompletionInArgumentLists)
        {
            ...
            await TestServices.Input.SendAsync(
                new InputKey(
                    ...
                    ImmutableArray.Create(VirtualKeyCode.CONTROL, VirtualKeyCode.MENU)
                ),
                HangMitigatingCancellationToken
            );
        ...
        public async Task NavigatingWithDownKey(bool showCompletionInArgumentLists)
        {
            ...
            Assert.Equal(
                "c",
                (
                    await TestServices.Editor.GetCurrentCompletionItemAsync(
                        HangMitigatingCancellationToken
                    )
                ).DisplayText
            );
...
class Class1
{
    void Main(string[] args)
    {
        $$
            ...
            var currentSignature = await TestServices.Editor.GetCurrentSignatureAsync(
                HangMitigatingCancellationToken
            );
        ...
        public async Task CompletionUsesTrackingPointsInTheFaceOfAutomaticBraceCompletion(
            bool showCompletionInArgumentLists
        )
        {
            await SetUpEditorAsync(
...
class Class1
{
    int P { get { $$} }
        ...
        {
            var visibleColumns = await TestServices.Editor.GetVisibleColumnCountAsync(
                HangMitigatingCancellationToken
            );
...
{{
    ...
    {{
        ...
        {variableName} = $$
            ...
            Assert.True(
                await TestServices.Editor.IsCaretOnScreenAsync(HangMitigatingCancellationToken)
            );
            Assert.True(
                await TestServices.Editor.GetCaretColumnAsync(HangMitigatingCancellationToken)
                    > visibleColumns,
                "This test is inconclusive if the view didn't need to move to keep the caret on screen."
            );
        }
