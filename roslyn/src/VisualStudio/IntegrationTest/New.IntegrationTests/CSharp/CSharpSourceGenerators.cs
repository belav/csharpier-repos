...
internal static class Program
{
    public static void Main()
    {
        ...
        [CombinatorialData]
        public async Task FindReferencesForFileWithDefinitionInSourceGeneratedFile(
...
internal static class Program
{
    public static void Main()
    {
        Console.WriteLine("
            ...
            var results = (
                await TestServices.FindReferencesWindow.GetContentsAsync(
                    HangMitigatingCancellationToken
                )
            )
                .OrderBy(r => r.GetLine())
            ...
            Assert.Collection(
                ...
                new Action<ITableEntryHandle2>[]
                {
                    ...
                    {
                        Assert.Equal(
                            expected: "/// <summary><see cref=\"HelloWorld\" /> is a simple class to fetch the classic message.</summary>",
                    ...
                    {
                        Assert.Equal(
                            expected: "internal class HelloWorld",
                            actual: reference.GetText()
                        );
...
internal static class Program
{
    public static void Main()
    {
        Console.WriteLine("
            ...
            var referenceInGeneratedFile = results.Single(
                r => r.GetText()?.Contains("<summary>") ?? false
            );
