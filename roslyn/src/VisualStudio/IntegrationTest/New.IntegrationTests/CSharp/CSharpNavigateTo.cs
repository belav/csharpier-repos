...
",
            ...
            await TestServices.Input.SendToNavigateToAsync(
                new InputKey[] { "FirstMethod", VirtualKeyCode.RETURN },
                HangMitigatingCancellationToken
            );
            ...
            Assert.Equal(
                "FirstMethod",
                await TestServices.Editor.GetSelectedTextAsync(HangMitigatingCancellationToken)
            );
            ...
            var isAllInOneSearch = await TestServices.Shell.ShowNavigateToDialogAsync(
                HangMitigatingCancellationToken
            );
            await TestServices.Input.SendToNavigateToAsync(
                new InputKey[] { "FirstClass", VirtualKeyCode.RETURN },
                HangMitigatingCancellationToken
            );
            await TestServices.Workarounds.WaitForNavigationAsync(HangMitigatingCancellationToken);
            Assert.Equal(
            ...
            if (isAllInOneSearch)
            {
                await telemetry.VerifyFiredAsync(
                    new[] { "vs/ide/vbcs/navigateto/search", "vs/ide/search/completed" },
                    HangMitigatingCancellationToken
                );
            }
            else
            {
                await telemetry.VerifyFiredAsync(
                    new[] { "vs/ide/vbcs/navigateto/search", "vs/platform/goto/launch" },
                    HangMitigatingCancellationToken
                );
            }
        }
