    ...
    public class CSharpReferenceTests
    {
        ...
        public async Task AddRemoveProjectAndMetadataReference_CPS()
        {
            ...
            var project1 = await CreateCSharpCPSProjectAsync(
                environment,
                "project1",
                commandLineArguments: @"/out:c:\project1.dll"
            );
            var project2 = await CreateCSharpCPSProjectAsync(
                environment,
                "project2",
                commandLineArguments: @"/out:c:\project2.dll"
            );
            var project3 = await CreateCSharpCPSProjectAsync(
                environment,
                "project3",
                commandLineArguments: @"/out:c:\project3.dll"
            );
            ...
            {
                return environment
                    .Workspace.CurrentSolution.GetProject(project3.Id)
                    .ProjectReferences;
            }
            ...
            Assert.True(
                GetProject3MetadataReferences().Any(mr => mr.FilePath == metadaRefFilePath)
            );
        ...
        public async Task RemoveProjectConvertsProjectReferencesBack()
        {
            ...
            Assert.Single(
                environment.Workspace.CurrentSolution.GetProject(project2.Id).MetadataReferences
            );
        ...
        public async Task AddingMetadataReferenceToProjectThatCannotCompileInTheIdeKeepsMetadataReference()
        {
            ...
            var project2 = await CreateNonCompilableProjectAsync(
                environment,
                "project2",
                @"C:\project2.fsproj",
                targetPath: @"c:\project2.dll"
            );

            project1.AddMetadataReference(
                project2.BinOutputPath,
