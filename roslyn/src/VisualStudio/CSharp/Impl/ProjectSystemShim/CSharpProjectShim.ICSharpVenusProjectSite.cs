...
namespace Microsoft.VisualStudio.LanguageServices.CSharp.ProjectSystemShim
{
    internal partial class CSharpProjectShim : ICSharpVenusProjectSite
    {
        ...
        )
        {
            ...
            var projectReferencesToRemove = ProjectSystemProject
                .GetProjectReferences()
                .Where(p => p.ProjectId == projectSite.ProjectSystemProject.Id)
                .ToList();

            if (projectReferencesToRemove.Count == 0)
            {
                throw new ArgumentException(
                    $"The project {nameof(project)} is not currently referenced by this project."
                );
            }

            foreach (var projectReferenceToRemove in projectReferencesToRemove)
            {
                ProjectSystemProject.RemoveProjectReference(
                    new ProjectReference(projectSite.ProjectSystemProject.Id)
                );
            }
        }

        public void OnDiskFileUpdated(
            string filename,
            ref System.Runtime.InteropServices.ComTypes.FILETIME pFT
        ...
        )
        {
            ...
            {
                var existingProjectReference = ProjectSystemProject
                    .GetProjectReferences()
                    .Single(p => p.ProjectId == projectSite.ProjectSystemProject.Id);

                ProjectSystemProject.RemoveProjectReference(existingProjectReference);
                ProjectSystemProject.AddProjectReference(
                    new ProjectReference(
                        existingProjectReference.ProjectId,
                        ImmutableArray.Create(currentAliases),
                        existingProjectReference.EmbedInteropTypes
                    )
                );
            }
        }

        public void AddReferenceToCodeDirectoryEx(
            string assemblyFileName,
        ...
        )
        {
            ...
            ProjectSystemProject.AddProjectReference(
                new ProjectReference(
                    projectSite.ProjectSystemProject.Id,
        ...
        private static CSharpProjectShim GetProjectSite(ICSharpProjectRoot project)
        {
            ...
            {
                throw new ArgumentException(
                    $"{project} was not properly sited with the language service.",
                    nameof(project)
                );
            }
