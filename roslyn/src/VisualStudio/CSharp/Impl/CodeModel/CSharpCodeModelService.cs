...
namespace Microsoft.VisualStudio.LanguageServices.CSharp.CodeModel
{
    internal partial class CSharpCodeModelService : AbstractCodeModelService
    {
        ...
        public override IEnumerable<SyntaxNode> GetAttributeNodes(SyntaxNode parent)
        {
            ...
            else if (parent is VariableDeclaratorSyntax || parent is VariableDeclarationSyntax)
            {
        ...
        public override EnvDTE.vsCMTypeRef GetTypeKindForCodeTypeRef(ITypeSymbol typeSymbol)
        {
            ...
            else if (typeSymbol.TypeKind == TypeKind.Pointer)
            {
                ...
                typeSymbol.TypeKind == TypeKind.Dynamic || typeSymbol.TypeKind == TypeKind.Unknown
            )
            {
        ...
        public override SyntaxNode GetNodeWithName(SyntaxNode node)
        {
            var kind = node.Kind();
            if (
                kind == SyntaxKind.OperatorDeclaration
        ...
        private EnvDTE.vsCMAccess GetDefaultAccessibility(SyntaxNode node)
        {
            ...
            if (node is BaseTypeDeclarationSyntax || node is DelegateDeclarationSyntax)
            {
            ...
            if (node is AccessorDeclarationSyntax || node is ArrowExpressionClauseSyntax)
            {
        ...
        public override SyntaxNode SetAccess(SyntaxNode node, EnvDTE.vsCMAccess newAccess)
        {
            ...
            if (
                member.Parent.Kind() == SyntaxKind.InterfaceDeclaration
                || member.Parent.Kind() == SyntaxKind.EnumDeclaration
            )
            {
                if (
                    newAccess == EnvDTE.vsCMAccess.vsCMAccessDefault
            ...
            if (member is BaseTypeDeclarationSyntax || member is EnumDeclarationSyntax)
            {
                if (
        ...
        {
            var docCommentTrivia = memberDeclaration.GetLeadingTrivia()
                .Reverse()
        ...
        public override string GetDocComment(SyntaxNode node)
        {
            ...
            var lines = documentationComment.ToString()
        ...
        public override SyntaxNode SetDocComment(SyntaxNode node, string value)
        {
            ...
            if (
                !(xmlDocument.FirstNode is XElement docElement)
        ...
        )
        {
            ...
            var attribute = (AttributeSyntax)argument.Ancestors()
                .First(n => n.Kind() == SyntaxKind.Attribute);

            attributeNode = attribute;
            index = attribute.ArgumentList.Arguments
        ...
        public override SyntaxNode GetEffectiveParentForAttribute(SyntaxNode node)
        {
            ...
            {
                return node.GetAncestor<BaseFieldDeclarationSyntax>()
        ...
        )
        {
            ...
            {
                namespaceNode =
                    usingDirective.Parent.Kind() == SyntaxKind.CompilationUnit
                        ? null
        ...
        )
        {
            ...
            if (
                kind == EnvDTE.vsCMFunction.vsCMFunctionConstructor
        ...
        {
            Debug.Assert(
                variableNode is FieldDeclarationSyntax
            ...
            {
                if (
                    kind != EnvDTE80.vsCMConstKind.vsCMConstKindConst
        ...
        {
            Debug.Assert(
                memberNode is MethodDeclarationSyntax
                ...
                else
                {
                    ...
                    )
                    {
                        ...
                        foreach (var accessor in property.AccessorList.Accessors)
                        {
                            ...
                            var updatedAccessor = accessor.WithBody(null)
                                .WithSemicolonToken(
                                    SyntaxFactory.Token(
                                        SyntaxTriviaList.Create(SyntaxFactory.ElasticMarker),
                                        SyntaxKind.SemicolonToken,
                ...
                else
                {
                    ...
                    )
                    {
                        ...
                        foreach (var accessor in property.AccessorList.Accessors)
                        {
                            ...
                            var updatedAccessor = accessor.WithSemicolonToken(default)
                                .WithBody(newBody);
                            updatedAccessors.Add(updatedAccessor);
                        }
        ...
        {
            Debug.Assert(
                memberNode is BaseMethodDeclarationSyntax
                    || memberNode is BasePropertyDeclarationSyntax
                    || memberNode is EventFieldDeclarationSyntax
            );
        ...
        {
            Debug.Assert(
                variableNode is MemberDeclarationSyntax || variableNode is VariableDeclaratorSyntax
            );
        ...
        {
            Debug.Assert(
                eventNode is EventFieldDeclarationSyntax || eventNode is EventDeclarationSyntax
            );
        ...
        {
            Debug.Assert(
                memberNode is MemberDeclarationSyntax || memberNode is VariableDeclaratorSyntax
            );
        ...
        {
            Debug.Assert(node is MemberDeclarationSyntax || node is ParameterSyntax);
        ...
        )
        {
            ...
            return semanticModel.GetSpeculativeTypeInfo(
                position,
                parsedTypeName,
        ...
        )
        {
            ...
            else if (container is ParameterSyntax parameter)
            {
                ...
                container is VariableDeclaratorSyntax || container is VariableDeclarationSyntax
            )
            {
        ...
        )
        {
            ...
            {
                var parameterList = indexer.ParameterList.Parameters
                    .Insert(index, (ParameterSyntax)parameter);
            ...
            {
                var parameterList = delegateDeclaration.ParameterList.Parameters
                    .Insert(index, (ParameterSyntax)parameter);
        ...
        {
            if (
                !methodDeclaration.IsParentKind(
                    SyntaxKind.ClassDeclaration,
                    out ClassDeclarationSyntax classDecl
