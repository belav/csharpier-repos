        ...
        protected override Task InitializeServiceForOpenedDocumentAsync(Document document)
        {
            ...
            lock (_gate)
            {
                if (!_processedProjects.Contains(document.Project.Id))
                {
                    ...
                    _typeTask = _typeTask.ContinueWith(
                        _ => PopulateTypeImportCompletionCacheAsync(this.Workspace, documentId),
                        TaskScheduler.Default
                    );
                    _extensionMethodTask = _extensionMethodTask.ContinueWith(
                        _ =>
                            PopulateExtensionMethodImportCompletionCacheAsync(
                                this.Workspace,
                                documentId
                            ),
                        TaskScheduler.Default
            ...
            static async Task PopulateTypeImportCompletionCacheAsync(
                Workspace workspace,
            ...
            )
            {
                ...
                await service
                    .WarmUpCacheAsync(partialDocument.Project, CancellationToken.None)
            ...
            static async Task PopulateExtensionMethodImportCompletionCacheAsync(
                Workspace workspace,
                DocumentId documentId,
                CancellationToken cancellationToken = default
            )
            {
                ...
                await ExtensionMethodImportCompletionHelper
                    .WarmUpCacheAsync(partialDocument, cancellationToken)
                    .ConfigureAwait(false);

                // Then try to update the cache with full semantic
                await ExtensionMethodImportCompletionHelper
                    .WarmUpCacheAsync(document, cancellationToken)
                    .ConfigureAwait(false);
            }
        }
    }
