    ...
    internal static class RazorLanguageServiceClientFactory
    {
        ...
        )
        {
            ...
            var client = await clientFactory
                .TryGetRemoteHostClientAsync(cancellationToken)
        ...
        private static string GetServiceName(Workspace workspace)
        {
            if (s_serviceNameDoNotAccessDirectly == null)
            {
                ...
                {
                    x64 = workspace
                        .Services.GetService<IExperimentationService>()
                        .IsExperimentEnabled(WellKnownExperimentNames.RoslynOOP64bit);
                }

                Interlocked.CompareExchange(
                    ref s_serviceNameDoNotAccessDirectly,
                    x64 ? "razorLanguageService64" : "razorLanguageService",
                    null
                );
            }
        ...
        public static readonly Option<bool> OOP64Bit = new Option<bool>(
            ...
            storageLocations: new LocalUserProfileStorageLocation(
                InternalFeatureOnOffOptions.LocalRegistryPath + nameof(OOP64Bit)
            )
        );
