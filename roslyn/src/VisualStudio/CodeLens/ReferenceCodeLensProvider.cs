...
namespace Microsoft.VisualStudio.LanguageServices.CodeLens
{
    ...
    internal class ReferenceCodeLensProvider : IAsyncCodeLensDataPointProvider, IDisposable
    {
        ...
        private readonly Dictionary<
            Guid,
            (string version, HashSet<DataPoint> dataPoints)
        > _dataPoints = new();
        ...
        private async Task PollForUpdatesAsync()
        {
            while (true)
            {
                ...
                var projectVersions = await _lazyCodeLensCallbackService.Value.InvokeAsync<
        ...
        private class DataPoint : IAsyncCodeLensDataPoint, IDisposable
        {
            private static readonly List<CodeLensDetailHeaderDescriptor> s_header =
                new List<CodeLensDetailHeaderDescriptor>()
                {
            ...
            public async Task<CodeLensDataPointDescriptor?> GetDataAsync(
                ...
                var referenceCountString =
                    $"{referenceCount.Count}{(referenceCount.IsCapped ? "+" : string.Empty)}";
            ...
            public async Task<CodeLensDetailsDescriptor> GetDetailsAsync(
                ...
                var referenceLocationDescriptors =
                    await _callbackService.InvokeAsync<(string projectVersion, ImmutableArray<ReferenceLocationDescriptor> references)?>(
                            _owner,
                ...
                {
                    var newCount = new ReferenceCount(
                        referenceLocationDescriptors.Value.references.Length,
                        isCapped: false,
                        version: referenceLocationDescriptors.Value.projectVersion
                    );
                    if (newCount != _calculatedReferenceCount)
                    {
                        _calculatedReferenceCount = newCount;
                        await InvalidatedAsync.InvokeAsync(this, EventArgs.Empty)
                            .ConfigureAwait(false);
                    }
                }

                var entries = referenceLocationDescriptors?.references.Select(
