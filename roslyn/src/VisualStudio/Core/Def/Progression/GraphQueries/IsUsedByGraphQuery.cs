...
namespace Microsoft.VisualStudio.LanguageServices.Implementation.Progression
{
    internal sealed class IsUsedByGraphQuery : IGraphQuery
    {
                ...
                foreach (var node in context.InputNodes)
                {
                    ...
                    foreach (var reference in references)
                    {
                        ...
                        var allLocations = referencedSymbol
                            .Locations.Concat(reference.Locations.Select(r => r.Location))
                            .Where(l => l != null && l.IsInSource);

                        foreach (var location in allLocations)
                        {
                            var locationNode = GetLocationNode(
                                location,
                                context,
                                projectId,
                                cancellationToken
                            ...
                            if (locationNode != null)
                                graphBuilder.AddLink(
                                    node,
                                    CodeLinkCategories.SourceReferences,
                                    locationNode,
                                    cancellationToken
                                );
                        }
        ...
        private static GraphNode? GetLocationNode(
            Location location,
            IGraphContext context,
            ProjectId projectId,
            CancellationToken cancellationToken
        )
        {
            ...
            var lineText = location
                .SourceTree.GetText(cancellationToken)
