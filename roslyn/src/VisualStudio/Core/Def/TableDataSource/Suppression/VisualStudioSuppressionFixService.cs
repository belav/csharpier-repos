    ...
    internal sealed class VisualStudioSuppressionFixService : IVisualStudioSuppressionFixService
    {
        ...
        )
        {
            try
            {
                ...
                var languageServices = new HashSet<HostLanguageServices>(
                    projectDiagnosticsToFixMap
                        .Select(p => p.Key.LanguageServices)
                        .Concat(
                            documentDiagnosticsToFixMap.Select(
                                kvp => kvp.Key.Project.LanguageServices
                            )
                        )
                );
                ...
                foreach (var languageService in languageServices)
                {
                    ...
                    if (showPreviewChangesDialog)
                    {
                        newSolution = FixAllGetFixesService.PreviewChanges(
                            ...
                            languageOpt: languageServices?.Count == 1
        ...
        )
        {
            ...
            // Local functions
            static bool IsDocumentDiagnostic(DiagnosticData d) =>
                d.DataLocation != null && d.HasTextSpan;
        }
        ...
        )
        {
            ...
            foreach (var (projectId, diagnostics) in builder)
            {
                ...
                if (filterStaleDiagnostics)
                {
                    ...
                    // Filter out stale diagnostics in error list.
                    projectDiagnosticsToFix = diagnostics.Where(
                        d =>
