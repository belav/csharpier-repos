...
namespace Microsoft.VisualStudio.LanguageServices.Storage
{
    ...
    internal class CloudCachePersistentStorage : AbstractPersistentStorage
    {
        ...
        public sealed override Task<bool> ChecksumMatchesAsync(
            string name,
            Checksum checksum,
        ...
        protected sealed override Task<bool> ChecksumMatchesAsync(
            ...
            ChecksumMatchesAsync(
                name,
                checksum,
                GetContainerKey(projectKey, project),
                cancellationToken
            );
        ...
        protected sealed override Task<bool> ChecksumMatchesAsync(
            ...
            ChecksumMatchesAsync(
                name,
                checksum,
                GetContainerKey(documentKey, document),
                cancellationToken
            );

        private async Task<bool> ChecksumMatchesAsync(
            string name,
            Checksum checksum,
            CacheContainerKey? containerKey,
            CancellationToken cancellationToken
        )
        {
            ...
            return await _cacheService
                .CheckExistsAsync(
                    new CacheItemKey(containerKey.Value, name) { Version = bytes.Object },
                    cancellationToken
                )
                .ConfigureAwait(false);
        }

        public sealed override Task<Stream?> ReadStreamAsync(
            string name,
            Checksum? checksum,
            CancellationToken cancellationToken
        ...
        )
        {
            ...
            if (checksum == null)
            {
                return await ReadStreamAsync(
                        new CacheItemKey(containerKey.Value, name),
                        cancellationToken
                    )
                    .ConfigureAwait(false);
            }
        ...
        )
        {
            ...
            var result = await _cacheService
                .TryGetItemAsync(key, pipe.Writer, cancellationToken)
                .ConfigureAwait(false);
            if (!result)
                return null;
            ...
            // allocations.
            return await pipe.Reader
                .AsPrebufferedStreamAsync(cancellationToken)
                .ConfigureAwait(false);
        }

        public sealed override Task<bool> WriteStreamAsync(
            string name,
            Stream stream,
            Checksum? checksum,
            CancellationToken cancellationToken
        ...
        {
            await _cacheService
                .SetItemAsync(key, PipeReader.Create(stream), shareable: false, cancellationToken)
                .ConfigureAwait(false);
            return true;
        }
    }
