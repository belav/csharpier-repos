...
namespace Microsoft.VisualStudio.LanguageServices.Implementation.Snippets
{
          ...
          IVsExpansionClient
    {
        ...
        public abstract int GetExpansionFunction(
        ...
        public int FormatSpan(IVsTextLines pBuffer, VsTextSpan[] tsInSurfaceBuffer)
        {
            ...
            if (isFullSnippetFormat)
            {
                ...
                var currentStartPosition =
                    snippetTrackingSpan.GetStartPoint(SubjectBuffer.CurrentSnapshot).Position;
        ...
        )
        {
            ...
            var triggerSpan = SubjectBuffer.CurrentSnapshot
                .GetSpan(
        ...
        )
        {
            ...
            var symbols = ThreadingContext.JoinableTaskFactory
                .Run(
                    () =>
        ...
        private void OnModelUpdated(object sender, ModelUpdatedEventsArgs e)
        {
            ...
            var compilation = ThreadingContext.JoinableTaskFactory
            ...
            var newSymbol = newSymbolKey.Resolve(
                compilation,
        ...
        public void MoveToSpecificMethod(IMethodSymbol method, CancellationToken cancellationToken)
        {
            ...
            var semanticModel = document.GetRequiredSemanticModelAsync(cancellationToken)
                .AsTask()
                .WaitAndGetResult(cancellationToken);
            var position = SubjectBuffer.CurrentSnapshot
        ...
        )
        {
            ...
            var documentWithImports = this.SubjectBuffer.CurrentSnapshot
                .GetOpenDocumentInCurrentContextWithChanges();
            ...
            var documentOptions = documentWithImports.GetOptionsAsync(cancellationToken)
                .WaitAndGetResult(cancellationToken);
        ...
        private void AddReferences(Project originalProject, XElement snippetNode)
        {
            ...
            var existingReferenceNames = originalProject.MetadataReferences
        ...
        )
        {
            ...
            snippetFunctionName = xmlFunctionNode.text
                .Substring(0, xmlFunctionNode.text.IndexOf('('));

            var paramStart = xmlFunctionNode.text.IndexOf('(') + 1;
            var paramLength =
                xmlFunctionNode.text.LastIndexOf(')') - xmlFunctionNode.text.IndexOf('(') - 1;
