            ...
            try
            {
                ...
                foreach (var path in Directory.EnumerateFiles(logPath, "*.log"))
                {
                    try
                    {
                        ...
                        if (
                            !name.Contains("-" + ServiceDescriptors.ServiceNameTopLevelPrefix)
                            ...
                            && !name.Contains(
                                "-" + RemoteServiceName.UnitTestingAnalysisServiceName
                            )
                            && !name.Contains(
                                "-" + RemoteServiceName.LiveUnitTestingBuildServiceName
                            )
                            && !name.Contains(
                                "-" + RemoteServiceName.UnitTestingSourceLookupServiceName
                            )
