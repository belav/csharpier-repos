...
namespace Microsoft.VisualStudio.LanguageServices.Implementation.Library.ObjectBrowser
{
    internal abstract class AbstractListItemFactory
    {
        ...
        protected MemberListItem CreateInheritedMemberListItem(
            ISymbol memberSymbol,
            ProjectId projectId,
            bool hidden
        ) => CreateMemberListItem(memberSymbol, projectId, hidden, isInherited: true);

        protected MemberListItem CreateSimpleMemberListItem(
            ISymbol memberSymbol,
            ProjectId projectId,
            bool hidden
        ) => CreateMemberListItem(memberSymbol, projectId, hidden, isInherited: false);

        private MemberListItem CreateMemberListItem(
            ISymbol memberSymbol,
            ProjectId projectId,
            bool hidden,
            bool isInherited
        ...
        protected TypeListItem CreateSimpleTypeListItem(
        ...
        protected TypeListItem CreateFullyQualifiedTypeListItem(
            INamedTypeSymbol namedTypeSymbol,
            ProjectId projectId,
            bool hidden
        )
        ...
        private ImmutableArray<ObjectListItem> GetBaseTypeListItems(
            INamedTypeSymbol namedTypeSymbol,
            Compilation compilation,
            ProjectId projectId
        )
        {
            ...
            return CreateListItemsFromSymbols(
                symbolBuilder.ToImmutable(),
                compilation,
                projectId,
        ...
        {
            Debug.Assert(parentListItem != null);
            Debug.Assert(parentListItem is TypeListItem or ProjectListItem);
            Debug.Assert(compilation != null);
            ...
            if (parentListItem is ProjectListItem)
            {
                builder.Add(
                    new FolderListItem(
                        ...
                        ServicesVSResources.Project_References
                    )
                );
            }
            ...
            if (parentListItem is TypeListItem parentTypeItem)
            {
                ...
                {
                    builder.Add(
                        new FolderListItem(parentListItem.ProjectId, ServicesVSResources.Base_Types)
                    );
                }
            }
        ...
        )
        {
            ...
            if (fullyQualified)
            {
                AddListItemsFromSymbols(
                    immediateMembers,
                    compilation,
                    projectId,
                    CreateFullyQualifiedMemberListItem,
                    builder
                );
            }
            else
        ...
        public void CollectNamespaceListItems(
        ...
        {
            Debug.Assert(parentListItem != null);
            Debug.Assert(parentListItem is ProjectListItem or ReferenceListItem);
            Debug.Assert(compilation != null);
        ...
        {
            public bool Equals(
                Tuple<ProjectId, IAssemblySymbol> x,
                Tuple<ProjectId, IAssemblySymbol> y
        ...
        public ImmutableHashSet<Tuple<ProjectId, IAssemblySymbol>> GetAssemblySet(
            Solution solution,
            string languageName,
        ...
        )
        {
            ...
            foreach (var projectId in solution.ProjectIds)
            {
                ...
                if (compilation != null && compilation.Assembly != null)
                {
            ...
            foreach (var projectId in projectIds)
            {
                ...
                if (Helpers.IsObjectBrowser(listFlags))
                {
                    ...
                    foreach (var reference in project.MetadataReferences)
                    {
                        ...
                        {
                            var assemblyIdentity = visitedAssemblies.GetOrAdd(
                                portableExecutableReference.FilePath,
                                filePath => AssemblyIdentityUtils.TryGetAssemblyIdentity(filePath)
                            );
                            if (
        ...
        )
        {
            Debug.Assert(parentListItem != null);
            Debug.Assert(
                parentListItem is NamespaceListItem or ProjectListItem or ReferenceListItem
            );
            Debug.Assert(compilation != null);
        ...
        public void CollectTypeListItems(
            IAssemblySymbol assemblySymbol,
            Compilation compilation,
            ProjectId projectId,
            ImmutableArray<ObjectListItem>.Builder builder,
        ...
        )
        {
            ...
            {
                var namespaceSymbol = stack.Pop();
                var typeListItems = GetTypeListItems(
                    namespaceSymbol,
                    compilation,
                    projectId,
                    searchString,
        ...
        public void CollectMemberListItems(
            IAssemblySymbol assemblySymbol,
            Compilation compilation,
            ProjectId projectId,
            ImmutableArray<ObjectListItem>.Builder builder,
        ...
        )
        {
            ...
            while (namespaceStack.Count > 0)
            {
                ...
                {
                    var memberListItems = GetMemberListItems(
                        type,
                        compilation,
                        projectId,
                        fullyQualified: true
