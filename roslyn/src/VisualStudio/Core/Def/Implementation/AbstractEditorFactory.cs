...
namespace Microsoft.VisualStudio.LanguageServices.Implementation
{
          ...
          IVsEditorFactoryNotify
    {
            ...
            switch (physicalView)
            {
                case "Form":
                    ...
                    try
                    {
                        ...
                        var designer = designerService.CreateDesigner(
                            _oleServiceProvider,
                            designerLoader
                        );
        ...
        private static string GetWinFormsLoaderName(IVsHierarchy vsHierarchy)
        {
            ...
            const string NewLoaderName =
                "Microsoft.VisualStudio.Design.Core.Serialization.CodeDom.VSCodeDomDesignerLoader";
            ...
            if (
                !vsHierarchy.TryGetTargetFrameworkMoniker(
        ...
        protected virtual Task<Document> OrganizeUsingsCreatedFromTemplateAsync(
            Document document,
        ...
        )
        {
            ...
            var forkedSolution = solution.AddDocument(
                DocumentInfo.Create(
                    documentId,
                    filePath,
            ...
            if (!string.IsNullOrEmpty(fileHeaderTemplate))
            {
                var documentWithFileHeader = ThreadHelper.JoinableTaskFactory.Run(
                    () =>
                    {
                        var newLineText = documentOptions.GetOption(
                            FormattingOptions.NewLine,
                            rootToFormat.Language
                        );
