...
namespace Microsoft.VisualStudio.LanguageServices.Implementation.Progression
{
    internal sealed class SearchGraphQuery : IGraphQuery
    {
        ...
        public SearchGraphQuery(string searchPattern) => _searchPattern = searchPattern;
        ...
        )
        {
            ...
            if (cacheService != null)
            {
                ...
                {
                    var results = await FindNavigableSourceSymbolsAsync(project, cancellationToken)
                        .ConfigureAwait(false);
                    ...
                    foreach (var symbol in results)
                    {
                        ...
                        {
                            await AddLinkedNodeForMemberAsync(project, symbol, graphBuilder)
                                .ConfigureAwait(false);
                        }
                    }
                }
        ...
        )
        {
            ...
            if (namedType.ContainingType != null)
            {
                var parentTypeNode = await AddLinkedNodeForTypeAsync(
                    ...
                    .ConfigureAwait(false);
                var typeNode = await graphBuilder
                    .AddNodeAsync(namedType, relatedNode: parentTypeNode)
                    .ConfigureAwait(false);
            ...
            else
            {
                ...
                var typeNode = await graphBuilder
                    .AddNodeAsync(namedType, contextProject: project, contextDocument: null)
                    .ConfigureAwait(false);
        ...
        )
        {
            ...
            var parentTypeNode = await AddLinkedNodeForTypeAsync(
                ...
                .ConfigureAwait(false);
            var memberNode = await graphBuilder
                .AddNodeAsync(symbol, relatedNode: parentTypeNode)
                .ConfigureAwait(false);
        ...
        )
        {
            ...
            try
            {
                declarations = await DeclarationFinder
