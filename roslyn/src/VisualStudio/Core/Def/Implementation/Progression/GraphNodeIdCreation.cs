...
namespace Microsoft.VisualStudio.LanguageServices.Implementation.Progression
{
    ...
    internal static class GraphNodeIdCreation
    {
        ...
        internal static async Task<GraphNodeId> GetIdForNamespaceAsync(
        ...
        private static async Task<IEnumerable<GraphNodeId>> GetPartialsForNamespaceAndTypeAsync(
            ITypeSymbol symbol,
        ...
        )
        {
            ...
            if (includeNamespace)
            {
                assembly = await GetAssemblyFullPathAsync(symbol, solution, cancellationToken)
                    .ConfigureAwait(false);
            }
            ...
            if (symbol.TypeKind == TypeKind.TypeParameter)
            {
                ...
                if (typeParameter.TypeParameterKind == TypeParameterKind.Type)
                {
                    if (includeNamespace && !typeParameter.ContainingNamespace.IsGlobalNamespace)
                    {
                        ...
                        {
                            items.Add(
                                GraphNodeId.GetPartial(CodeGraphNodeIdName.Assembly, assembly)
                            );
                        }
                        ...
                        items.Add(
                            GraphNodeId.GetPartial(
                                ...
                                typeParameter.ContainingNamespace.ToDisplayString()
                            )
                        );
                    }
                ...
                items.Add(
                    GraphNodeId.GetPartial(
                        ...
                        ((ITypeParameterSymbol)symbol).Ordinal.ToString()
                    )
                );
            }
            else
            {
                ...
                )
                {
                    items.Add(
                        GraphNodeId.GetPartial(
                            CodeGraphNodeIdName.Namespace,
                            underlyingType.ContainingNamespace.ToDisplayString()
                        )
                    );
                }
        ...
        public static async Task<GraphNodeId> GetIdForMemberAsync(
            ISymbol member,
            Solution solution,
            CancellationToken cancellationToken
        )
        {
            ...
            if (parameters.Any() || member.GetArity() > 0)
            {
                ...
                if (parameters.Any())
                {
                    ...
                    foreach (var p in parameters)
                    {
                        ...
                        {
                            nodes.Add(
                                GraphNodeId.GetPartial(CodeGraphNodeIdName.ParamKind, ParamKind.Ref)
                            );
                        }
                    ...
                    )
                    {
                        ...
                        returnTypePartial.Add(
                            GraphNodeId.GetPartial(
                                CodeGraphNodeIdName.ParamKind,
                                Microsoft.VisualStudio.GraphModel.CodeSchema.ParamKind.Return
                            )
                        );
            ...
            {
                partials.Add(
                    GraphNodeId.GetPartial(CodeGraphNodeIdName.Member, member.MetadataName)
                );
            }
        ...
        private static async Task<Uri> GetAssemblyFullPathAsync(
            ISymbol symbol,
            Solution solution,
            CancellationToken cancellationToken
        )
        ...
        internal static async Task<GraphNodeId> GetIdForAssemblyAsync(
        ...
        )
        {
            ...
            {
                var currentNode = await reference.GetSyntaxAsync(cancellationToken)
                    .ConfigureAwait(false);
