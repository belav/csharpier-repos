        ...
        */
        ...
        public ProjectId GetOrCreateProjectIdForPath(string filePath, string projectDisplayName)
        {
            ...
            return _workspace.CurrentSolution.Projects
                    .FirstOrDefault(p => p.FilePath == filePath)?.Id
        ...
        internal bool TryGetProjectByBinPath(string filePath, out AbstractProject project)
        {
            var projectsWithBinPath = _workspace.CurrentSolution.Projects
                .Where(
                    p =>
                        string
                            .Equals(p.OutputFilePath, filePath, StringComparison.OrdinalIgnoreCase)
                )
                .ToList();
        ...
        public void AddProject(AbstractProject project)
        {
            if (_projectFactory != null)
            {
                ...
                project.VisualStudioProject = this.ThreadingContext.JoinableTaskFactory
                    .Run(
                        () =>
