    ...
    internal class VisualStudioProjectOptionsProcessor : IDisposable
    {
        ...
        private void UpdateProjectOptions_NoLock()
        {
            ...
            if (_ruleSetFile?.Target.Value.FilePath != effectiveRuleSetPath)
            {
                ...
                {
                    _ruleSetFile =
                        _workspaceServices.GetRequiredService<VisualStudioRuleSetManager>()
            ...
            var compilationOptions =
                _commandLineArgumentsForCommandLine.CompilationOptions.WithConcurrentBuild(
                        concurrent: false
                    )
                    ...
                    .WithStrongNameProvider(
                        new DesktopStrongNameProvider(
                            _commandLineArgumentsForCommandLine.KeyFileSearchPaths.WhereNotNull()
                                .ToImmutableArray()
                        )
                    );
            ...
            compilationOptions = ComputeCompilationOptionsWithHostValues(
                compilationOptions,
                this._ruleSetFile?.Target.Value
            );
