...
{
    [
        ExportWorkspaceServiceFactory(typeof(IFrameworkAssemblyPathResolver), ServiceLayer.Host),
        Shared
    ]
    internal sealed class VisualStudioFrameworkAssemblyPathResolverFactory
        : IWorkspaceServiceFactory
    {
        ...
        public IWorkspaceService CreateService(HostWorkspaceServices workspaceServices) =>
            new Service(
                _threadingContext,
                workspaceServices.Workspace as VisualStudioWorkspace,
                _serviceProvider
            );
              ...
              IFrameworkAssemblyPathResolver
        {
            ...
            private bool CanResolveType(Assembly assembly, string? fullyQualifiedTypeName)
            {
                ...
                for (var i = 0; i < 5; i++)
                {
                    var name = i == 0 ? fullyQualifiedTypeName : fullyQualifiedTypeName + "`" + i;
            ...
            private Assembly? ResolveAssembly(ProjectId projectId, string assemblyName)
            {
                ...
                if (
                    hierarchy == null
                    || !hierarchy.TryGetProperty(
                        (__VSHPROPID)__VSHPROPID4.VSHPROPID_TargetFrameworkMoniker,
