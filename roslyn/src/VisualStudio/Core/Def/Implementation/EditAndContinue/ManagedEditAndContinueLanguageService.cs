...
namespace Microsoft.VisualStudio.LanguageServices.EditAndContinue
{
    ...
    internal sealed class ManagedEditAndContinueLanguageService
        : IManagedEditAndContinueLanguageService
    {
        ...
        )
        {
            ...
            _activeStatementTrackingService = workspace.Services
        ...
        public async Task EnterBreakStateAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                await _proxy.BreakStateEnteredAsync(_diagnosticService, cancellationToken)
                    .ConfigureAwait(false);
            }
            catch (Exception e) when (FatalError.ReportAndCatchUnlessCanceled(e, cancellationToken))
            {
            ...
            await _activeStatementTrackingService.StartTrackingAsync(solution, cancellationToken)
                .ConfigureAwait(false);
        }
        ...
        private SolutionActiveStatementSpanProvider GetActiveStatementSpanProvider(
            Solution solution
        ) =>
            new(
                (documentId, cancellationToken) =>
                    _activeStatementTrackingService.GetSpansAsync(
        ...
        )
        {
            try
            {
                ...
                return await _proxy.HasChangesAsync(
                    solution,
        ...
        )
        {
            try
            {
                var solution = _proxy.Workspace.CurrentSolution;
                var activeStatementSpanProvider = GetActiveStatementSpanProvider(solution);
                var (updates, _, _) = await _proxy.EmitSolutionUpdateAsync(
                    solution,
                    activeStatementSpanProvider,
                    _diagnosticService,
                    _diagnosticUpdateSource,
        ...
        )
        {
            try
            {
                ...
                var activeStatementSpanProvider = new SolutionActiveStatementSpanProvider(
                    async (documentId, cancellationToken) =>
                    {
                        var document = solution.GetRequiredDocument(documentId);
                        return await _activeStatementTrackingService.GetSpansAsync(
                            document,
                            cancellationToken
                        )
                            .ConfigureAwait(false);
                ...
                var span = await _proxy.GetCurrentActiveStatementPositionAsync(
                    solution,
                    activeStatementSpanProvider,
                    instruction,
                    cancellationToken
        ...
        )
        {
            ...
            {
                var solution = _proxy.Workspace.CurrentSolution;
                return await _proxy.IsActiveStatementInExceptionRegionAsync(
                    solution,
                    instruction,
                    cancellationToken
                )
