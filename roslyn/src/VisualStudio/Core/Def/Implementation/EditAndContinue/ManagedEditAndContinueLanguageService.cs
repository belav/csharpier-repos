...
namespace Microsoft.VisualStudio.LanguageServices.EditAndContinue
{
        ...
        : IManagedEditAndContinueLanguageService
    {
        ...
        public async Task EnterBreakStateAsync(CancellationToken cancellationToken)
        {
            ...
            {
                await _proxy
                    .BreakStateEnteredAsync(_diagnosticService, cancellationToken)
                    .ConfigureAwait(false);
            }
            catch (Exception e) when (FatalError.ReportAndCatchUnlessCanceled(e, cancellationToken))
            {
            ...
            await _activeStatementTrackingService
                .StartTrackingAsync(solution, cancellationToken)
                .ConfigureAwait(false);
        }
            ...
            try
            {
                ...
                var activeStatementSpanProvider = new SolutionActiveStatementSpanProvider(
                    async (documentId, cancellationToken) =>
                    {
                        ...
                        return await _activeStatementTrackingService
                            .GetSpansAsync(document, cancellationToken)
                            .ConfigureAwait(false);
                    }
                );

                var span = await _proxy
