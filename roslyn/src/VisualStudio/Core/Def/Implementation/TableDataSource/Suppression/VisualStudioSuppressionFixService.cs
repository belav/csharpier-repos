    ...
    internal sealed class VisualStudioSuppressionFixService : IVisualStudioSuppressionFixService
    {
        ...
        private bool ApplySuppressionFix(
            ...
            void computeDiagnosticsAndFix(IWaitContext context)
            {
                ...
                languages = new HashSet<string>(
                    projectDiagnosticsToFixMap.Select(p => p.Key.Language)
                        .Concat(documentDiagnosticsToFixMap.Select(kvp => kvp.Key.Project.Language))
                );
            ...
            Task.Run(
                () =>
                {
                    var reanalyzeDocuments = diagnosticsToFix.Where(d => d.DocumentId != null)
        ...
        > GetDocumentDiagnosticsToFixAsync(
            ...
            var finalBuilder = ImmutableDictionary.CreateBuilder<
                ...
                ImmutableArray<Diagnostic>
            >();
            var latestDocumentDiagnosticsMapOpt = filterStaleDiagnostics
            ...
            foreach (var group in builder.GroupBy(kvp => kvp.Key.ProjectId))
            {
                ...
                {
                    var uniqueDiagnosticIds = group.SelectMany(kvp => kvp.Value.Select(d => d.Id))
                        .ToImmutableHashSet();
                ...
                foreach (var documentDiagnostics in group)
                {
                    ...
                    {
                        if (
                            !latestDocumentDiagnosticsMapOpt.TryGetValue(
                                document.Id,
        ...
        > GetProjectDiagnosticsToFixAsync(
            ...
            var finalBuilder = ImmutableDictionary.CreateBuilder<
                ...
                ImmutableArray<Diagnostic>
            >();
            var latestDiagnosticsToFixOpt = filterStaleDiagnostics
            ...
            foreach (var (projectId, diagnostics) in builder)
            {
                ...
                if (filterStaleDiagnostics)
                {
                    ...
                    latestDiagnosticsToFixOpt.AddRange(
