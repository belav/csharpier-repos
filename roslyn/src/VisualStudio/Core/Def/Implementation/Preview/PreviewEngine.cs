...
namespace Microsoft.VisualStudio.LanguageServices.Implementation.Preview
{
    internal class PreviewEngine : ForegroundThreadAffinitizedObject, IVsPreviewChangesEngine
    {
        ...
        public int GetRootChangesList(out object ppIUnknownPreviewChangesList)
        {
            ...
            _topLevelChange.Children =
                builder.Count == 0 ? ChangeList.Empty : new ChangeList(builder.ToArray());
            ppIUnknownPreviewChangesList =
                _topLevelChange.Children.Changes.Length == 0
                    ? new ChangeList(new[] { new NoChange(this) })
                    : new ChangeList(new[] { _topLevelChange });
        ...
        private void AppendFileChanges(
            ...
            var orderedChangedDocuments = changedDocuments.GroupBy(d => d.ProjectId)
        ...
        public void SetTextView(object textView)
        {
            if (_updater == null)
            {
                _updater = new PreviewUpdater(
                    ThreadingContext,
                    EnsureTextViewIsInitialized(textView)
                );
            }
        }
        ...
        private static void EditBufferToInitialize(IVsTextView adapter)
        {
            ...
            try
            {
                ...
                Marshal.ThrowExceptionForHR(
                    lines.GetLengthOfLine(piLineIndex, out var piLineLength)
                );
            ...
            internal override void GetDisplayData(VSTREEDISPLAYDATA[] pData) =>
                pData[0].Image =
                    pData[0].SelectedImage =
                        (ushort)StandardGlyphGroup.GlyphInformation;
