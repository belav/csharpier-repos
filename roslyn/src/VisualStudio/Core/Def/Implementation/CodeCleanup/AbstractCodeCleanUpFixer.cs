    ...
    internal abstract class AbstractCodeCleanUpFixer : ICodeCleanUpFixer
    {
        ...
        )
        {
            ...
            else if (hierarchy.GetCanonicalName(itemId, out var path) == 0)
            {
                ...
                else
                {
                    ...
                    return await FixDocumentAsync(solution.GetRequiredDocument(documentId), context)
                        .ConfigureAwait(true);
                }
            }
        ...
        )
        {
            ...
            return await codeCleanupService
                .CleanupAsync(document, enabledDiagnostics, progressTracker, cancellationToken)
                .ConfigureAwait(false);
        }
    }
}
