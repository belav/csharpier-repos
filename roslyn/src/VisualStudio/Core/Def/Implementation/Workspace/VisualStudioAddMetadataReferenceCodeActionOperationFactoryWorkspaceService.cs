...
namespace Microsoft.VisualStudio.LanguageServices.Implementation
{
    [
        ExportWorkspaceService(
            typeof(IAddMetadataReferenceCodeActionOperationFactoryWorkspaceService),
            ServiceLayer.Host
        ),
        Shared
    ]
    internal sealed class VisualStudioAddMetadataReferenceCodeActionOperationFactoryWorkspaceService
        : IAddMetadataReferenceCodeActionOperationFactoryWorkspaceService
    {
            ...
            : Microsoft.CodeAnalysis.CodeActions.CodeActionOperation
        {
                    ...
                    )
                    {
                        ...
                        if (
                            ErrorHandler.Succeeded(
                                uiHierarchy.QueryStatusCommand(
                        ...
                        )
                        {
                            if ((((OLECMDF)command[0].cmdf) & OLECMDF.OLECMDF_ENABLED) != 0)
                            {
                                uiHierarchy.ExecCommand(
                                    (uint)VSConstants.VSITEMID.Root,
                                    VSConstants.VSStd2K,
                                    (uint)VSConstants.VSStd2KCmdID.ADDREFERENCE,
                                    0,
                                    IntPtr.Zero,
                                    IntPtr.Zero
                                );
            ...
            public override string Title =>
                string.Format(
                    ServicesVSResources.Add_a_reference_to_0,
                    _assemblyIdentity.GetDisplayName()
                );
        }
    }
