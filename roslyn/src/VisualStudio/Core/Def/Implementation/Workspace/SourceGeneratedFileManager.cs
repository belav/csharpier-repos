            ...
            )
            {
                ...
                if (!_openFiles.TryGetValue(moniker, out var openFile))
                {
                    ...
                    _threadingContext.JoinableTaskFactory
                        .Run(() => openFile.UpdateBufferContentsAsync(CancellationToken.None));

                    // Update the RDT flags to ensure the file can't be saved or appears in any MRUs as it's a temporary generated file name.
                    var cookie = ((IVsRunningDocumentTable4)_runningDocumentTable)
        ...
        private class OpenSourceGeneratedFile : ForegroundThreadAffinitizedObject, IDisposable
        {
            ...
            public async Task UpdateBufferContentsAsync(CancellationToken cancellationToken)
            {
                ...
                else
                {
                    ...
                    {
                        windowFrameMessageToShow = string
                            .Format(
                                    ...
                                    a =>
                                        a.GetGenerators(project.Language)
                                            .Any(
                                                g =>
                                                    g.GetType()
                                                        .Assembly.Equals(_generatorType.Assembly)
                                            )
                                )
                        )
                        {
            ...
            internal void SetWindowFrame(IVsWindowFrame windowFrame, string generatedSourceHintName)
            {
                ...
                windowFrame.SetProperty(
                    ...
                    generatedSourceHintName
                        + " "
                        + string
                            .Format(ServicesVSResources.generated_by_0_suffix, GeneratorDisplayName)
                );
            ...
            private void EnsureWindowFrameInfoBarUpdated()
            {
                ...
                var infoBarFactory = (IVsInfoBarUIFactory)_fileManager._serviceProvider
                    .GetService(typeof(SVsInfoBarUIFactory));
            ...
            public void NavigateToSpan(TextSpan sourceSpan, CancellationToken cancellationToken)
            {
                ...
                _fileManager._visualStudioDocumentNavigationService
                    .NavigateTo(
                        _textBuffer,
