    ...
    internal sealed class RemoteCodeLensReferencesService : ICodeLensReferencesService
    {
        ...
        )
        {
            ...
            )
            {
                ...
                return await FixUpDescriptorsAsync(solution, descriptors.Value, cancellationToken)
                    .ConfigureAwait(false);
            }
        }
        ...
        public async Task<string?> GetFullyQualifiedNameAsync(
            Solution solution,
            DocumentId documentId,
            SyntaxNode? syntaxNode,
            CancellationToken cancellationToken
        )
        {
            using (Logger.LogBlock(FunctionId.CodeLens_GetFullyQualifiedName, cancellationToken))
            {
                ...
                {
                    var result = await client
                        .TryInvokeAsync<IRemoteCodeLensReferencesService, string>(
                            solution,
                            (service, solutionInfo, cancellationToken) =>
        ...
        )
        {
            ...
            foreach (var descriptor in descriptors)
            {
                ...
                var referenceExcerpt = await excerpter
                    .TryExcerptAsync(document, span, ExcerptMode.SingleLine, cancellationToken)
                    .ConfigureAwait(false);
                var tooltipExcerpt = await excerpter
                    .TryExcerptAsync(document, span, ExcerptMode.Tooltip, cancellationToken)
                    .ConfigureAwait(false);

                var (text, start, length) = GetReferenceInfo(referenceExcerpt, descriptor);
                var (before1, before2, after1, after2) = GetReferenceTexts(
                    referenceExcerpt,
                    tooltipExcerpt,
                    descriptor
                );
                ...
                list.Add(
                    new ReferenceLocationDescriptor(
        ...
        private static async Task<ImmutableArray<ReferenceLocationDescriptor>?> FindReferenceLocationsWorkerAsync(
            Solution solution,
            DocumentId documentId,
            SyntaxNode syntaxNode,
            CancellationToken cancellationToken
