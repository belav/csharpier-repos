...
namespace Microsoft.VisualStudio.LanguageServices.Implementation.ProjectSystem.Legacy
{
    ...
    internal abstract partial class AbstractLegacyProject : ForegroundThreadAffinitizedObject
    {
            ...
            : base(threadingContext, assertIsForeground: true)
        {
            ...
            ProjectSystemProject = threadingContext.JoinableTaskFactory.Run(
                () =>
        ...
        public string GetOutputFileName() => ProjectSystemProject.CompilationOutputAssemblyFilePath;
        ...
        protected void AddFile(string filename, string linkMetadata, SourceCodeKind sourceCodeKind)
        {
            // We have tests that assert that XOML files should not get added; this was similar
            // behavior to how ASP.NET projects would add .aspx files even though we ultimately ignored
            // them. XOML support is planned to go away for Dev16, but for now leave the logic there.
            ...
            if (!string.IsNullOrEmpty(linkMetadata))
            {
                ...
                folders = linkFolderPath
                    .Split(PathSeparatorCharacters, StringSplitOptions.RemoveEmptyEntries)
                    .ToImmutableArray();
            }
            else if (!string.IsNullOrEmpty(ProjectSystemProject.FilePath))
            {
                ...
                folders = ImmutableArray.Create(
                    relativePathParts,
                    start: 0,
                    length: relativePathParts.Length - 1
                );
            }
        ...
        protected void RefreshBinOutputPath()
        {
            ...
            if (!PathUtilities.IsAbsolute(outputDirectory))
            {
                ...
                outputDirectory = FileUtilities.ResolveRelativePath(
                    outputDirectory,
                    Path.GetDirectoryName(ProjectSystemProject.FilePath)
                );
            }
            ...
            ProjectSystemProject.OutputFilePath = FileUtilities.NormalizeAbsolutePath(
                Path.Combine(outputDirectory, targetFileName)
            );
        ...
        private ImmutableArray<string> GetFolderNamesForDocument(uint documentItemID)
        {
            ...
            if (
                documentItemID != (uint)VSConstants.VSITEMID.Nil
            ...
            )
            {
                ...
                if (
                    parentID
                    is not ((uint)VSConstants.VSITEMID.Nil)
                        and not ((uint)VSConstants.VSITEMID.Root)
                )
                {
        ...
        private static void ComputeFolderNames(
            uint folderItemID,
            List<string> names,
            IVsHierarchy hierarchy
        )
        {
            if (
                hierarchy.GetProperty(folderItemID, (int)VsHierarchyPropID.Name, out var nameObj)
                == VSConstants.S_OK
            )
            {
        ...
        private static bool TryGetBoolPropertyValue(
            IVsHierarchy hierarchy,
            string propertyName,
