            ...
            : AbstractTableDataSourceFindUsagesContext
        {
            ...
            )
            {
                ...
                if (definition.DisplayIfNoReferences)
                    await AddDeclarationEntriesAsync(
                            definition,
            ...
            private bool HasDeclarationEntries(DefinitionItem definition)
            {
                lock (Gate)
                {
                    return EntriesWhenNotGroupingByDefinition.Any(e =>
                        e.DefinitionBucket.DefinitionItem == definition
                    );
                }
            }
            ...
            )
            {
                ...
                await CreateMissingReferenceEntriesIfNecessaryAsync(cancellationToken)
                    .ConfigureAwait(false);
                await CreateNoResultsFoundEntryIfNecessaryAsync(cancellationToken)
                    .ConfigureAwait(false);
            }
            ...
            {
                await CreateMissingReferenceEntriesIfNecessaryAsync(
                        whenGroupingByDefinition: true,
                        cancellationToken
                    )
                    .ConfigureAwait(false);
                await CreateMissingReferenceEntriesIfNecessaryAsync(
                        whenGroupingByDefinition: false,
                        cancellationToken
                    )
                    .ConfigureAwait(false);
            }
