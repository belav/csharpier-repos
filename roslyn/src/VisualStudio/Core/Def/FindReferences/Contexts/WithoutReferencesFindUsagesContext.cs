    ...
    internal partial class StreamingFindUsagesPresenter
    {
            ...
            : AbstractTableDataSourceFindUsagesContext
        {
            ...
            private async Task CreateNoResultsFoundEntryIfNecessaryAsync()
            {
                ...
                var entry = await SimpleMessageEntry
                    .CreateAsync(definitionBucket, navigationBucket: null, message)
                    .ConfigureAwait(false);

                lock (Gate)
                {
                    EntriesWhenGroupingByDefinition = EntriesWhenGroupingByDefinition.Add(entry);
                    EntriesWhenNotGroupingByDefinition = EntriesWhenNotGroupingByDefinition.Add(
                        entry
                    );
            ...
            )
            {
                ...
                if (definition.SourceSpans.Length == 1)
                {
                    ...
                    var entry = await TryCreateEntryAsync(
                            definitionBucket,
                            definition,
                            cancellationToken
                        )
                ...
                else if (definition.SourceSpans.Length == 0)
                {
                    ...
                    entries.Add(
                        new MetadataDefinitionItemEntry(
                            this,
                            definitionBucket,
