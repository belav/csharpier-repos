...
namespace Microsoft.VisualStudio.LanguageServices.Implementation.Extensions
{
    internal static class VisualStudioWorkspaceImplExtensions
    {
        ...
        private static readonly ConditionalWeakTable<
            IVsHierarchy,
            Dictionary<uint, IImageHandle>
        > s_hierarchyToItemIdToImageHandle = new();

        private static bool TryGetImageListAndIndex(
        ...
        )
        {
            var itemIdToImageHandle = s_hierarchyToItemIdToImageHandle.GetValue(
                hierarchy,
                static _ => new Dictionary<uint, IImageHandle>()
            );

            // Get the actual image moniker that the vs hierarchy is using to in solution explorer.
            var imageMoniker = imageService.GetImageMonikerForHierarchyItem(
                hierarchy,
                itemId,
            ...
            var uiObject = imageService.GetImage(
                imageHandle.Moniker,
                new ImageAttributes
                {
        ...
        public static bool TryGetImageListAndIndex(
            this VisualStudioWorkspaceImpl workspace,
            IVsImageService2 imageService,
            DocumentId id,
            out IntPtr imageList,
            out ushort index
        )
        {
            ...
            if (hierarchy != null && !RoslynString.IsNullOrEmpty(document?.FilePath))
            {
                ...
                return TryGetImageListAndIndex(
                    hierarchy,
                    imageService,
                    itemId,
                    out imageList,
