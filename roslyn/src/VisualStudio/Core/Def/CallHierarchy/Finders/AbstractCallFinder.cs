...
namespace Microsoft.CodeAnalysis.Editor.Implementation.CallHierarchy.Finders
{
    internal abstract class AbstractCallFinder
    {
        ...
        private static IImmutableSet<Document> IncludeDocuments(
            CallHierarchySearchScope scope,
            Project project
        )
        {
            ...
            )
            {
                var documentTrackingService =
                    project.Solution.Services.GetRequiredService<IDocumentTrackingService>();
                    ...
                    else
                    {
                        ...
                        var item = await Provider
                            .CreateItemAsync(
                                ...
                                callingProject,
                                caller.Locations.ToImmutableArray(),
                                cancellationToken
                            )
                            .ConfigureAwait(false);
