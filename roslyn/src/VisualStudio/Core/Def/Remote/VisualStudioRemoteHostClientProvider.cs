    ...
    internal sealed class VisualStudioRemoteHostClientProvider : IRemoteHostClientProvider
    {
        ...
        internal sealed class Factory : IWorkspaceServiceFactory
        {
            ...
            public IWorkspaceService CreateService(HostWorkspaceServices workspaceServices)
            {
                ...
                if (
                    !_globalOptions.GetOption(RemoteHostOptionsStorage.OOP64Bit)
                    || workspaceServices.Workspace
                        is not (VisualStudioWorkspace or PreviewWorkspace)
                    ||
                    ...
                    workspaceServices.SolutionServices.WorkspaceServices.HostServices
                        != _vsWorkspace.Services.HostServices
                    || workspaceServices
                ...
                lock (_gate)
                {
                    ...
                    if (_cachedVSInstance is null)
                        _cachedVSInstance = new VisualStudioRemoteHostClientProvider(
                            _vsWorkspace.Services.SolutionServices,
                            _globalOptions,
        ...
        private async Task<RemoteHostClient?> CreateHostClientAsync()
        {
            try
            {
                var brokeredServiceContainer = await _brokeredServiceContainer
                    .GetValueAsync()
                ...
                _ = brokeredServiceContainer.Proffer(
                    ...
                    (_, _, _, _) =>
                        ValueTaskFactory.FromResult<object?>(new SolutionAssetProvider(Services))
                );
