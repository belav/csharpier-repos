...
namespace Microsoft.VisualStudio.LanguageServices.DocumentOutline
{
    ...
    internal sealed partial class DocumentOutlineViewModel
    {
        ...
        )
        {
            ...
            var manualResponse = await languageServiceBroker
                .RequestAsync(
        ...
        )
        {
            ...
            var documentSymbols =
                token.ToObject<RoslynDocumentSymbol[]>() ?? Array.Empty<RoslynDocumentSymbol>();
            ...
            DocumentSymbolData NestDescendantSymbols(
                ImmutableArray<RoslynDocumentSymbol> allSymbols,
                int start,
                out int newStart
            )
            {
            ...
            SnapshotSpan GetSymbolRangeSpan(Range symbolRange)
            {
                var originalStartPosition =
                    textSnapshot.GetLineFromLineNumber(symbolRange.Start.Line).Start.Position
                    + symbolRange.Start.Character;
                var originalEndPosition =
                    textSnapshot.GetLineFromLineNumber(symbolRange.End.Line).Start.Position
