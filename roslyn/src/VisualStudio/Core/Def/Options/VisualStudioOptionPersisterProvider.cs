...
namespace Microsoft.VisualStudio.LanguageServices.Options
{
    ...
    internal sealed class VisualStudioOptionPersisterProvider : IOptionPersisterProvider
    {
        ...
        )
        {
            ...
            _lazyPersister = new Threading.AsyncLazy<IOptionPersister>(
                () => CreatePersisterAsync(threadingContext.DisposalToken),
        ...
        private async Task<IOptionPersister> CreatePersisterAsync(
            CancellationToken cancellationToken
        )
        {
            ...
            var settingsManager = await GetFreeThreadedServiceAsync<
                SVsSettingsPersistenceManager,
                ISettingsManager
            >()
                .ConfigureAwait(false);
            Assumes.Present(settingsManager);
            var localRegistry = await GetFreeThreadedServiceAsync<SLocalRegistry, ILocalRegistry4>()
                .ConfigureAwait(false);
            Assumes.Present(localRegistry);
            var featureFlags = await GetFreeThreadedServiceAsync<SVsFeatureFlags, IVsFeatureFlags>()
                .ConfigureAwait(false);
        ...
        /// </summary>
        private async ValueTask<I?> GetFreeThreadedServiceAsync<T, I>()
            where I : class
        {
            try
            {
