...
namespace Microsoft.VisualStudio.LanguageServices.Packaging
{
            ...
            IVsSearchProviderCallback
    {
        ...
        private readonly AsyncBatchingWorkQueue<(
            ...
            ProjectId? changedProject
        )> _workQueue;

        private readonly ConcurrentDictionary<
            ProjectId,
            ProjectState
        > _projectToInstalledPackageAndVersion = new();
            ...
            try
            {
                if (_packageSourceProvider != null)
                    return _packageSourceProvider
                        .Value.GetSources(includeUnOfficial: true, includeDisabled: false)
        ...
        )
        {
            ...
            try
            {
                return await this.PerformNuGetProjectServiceWorkAsync(
                        async (nugetService, cancellationToken) =>
                        {
                            // explicitly switch to BG thread to do the installation as nuget installer APIs are free threaded.
                            await TaskScheduler.Default;

                            var installedPackagesMap = await GetInstalledPackagesMapAsync(
                                    nugetService,
                                    projectGuid,
                                    cancellationToken
                                )
                            ...
                            installedPackagesMap = await GetInstalledPackagesMapAsync(
                                ...
                                .ConfigureAwait(false);
                            var installedVersion = installedPackagesMap.TryGetValue(
                                packageName,
                                out var result
                            )
                                ? result
            ...
            catch (OperationCanceledException)
            {
                await UpdateStatusBarAsync(
                        dte,
                        ServicesVSResources.Package_install_canceled,
                        cancellationToken
                    )
                    .ConfigureAwait(false);
                return false;
        ...
        )
        {
                ...
                return await this.PerformNuGetProjectServiceWorkAsync(
                        async (nugetService, cancellationToken) =>
                        {
                            ...
                            var installedPackagesMap = await GetInstalledPackagesMapAsync(
                                ...
                                .ConfigureAwait(false);
                            if (
                                !installedPackagesMap.TryGetValue(
                                    packageName,
            ...
            catch (OperationCanceledException)
            {
                await UpdateStatusBarAsync(
                        dte,
                        ServicesVSResources.Package_uninstall_canceled,
                        cancellationToken
                    )
                    .ConfigureAwait(false);
                return false;
        ...
        private void OnWorkspaceChanged(object sender, WorkspaceChangeEventArgs e)
        {
            ...
            Contract.ThrowIfNull(
                _workQueue,
                "We should only register for events after having create the WorkQueue"
            );
            _workQueue.AddWork((solutionChanged, changedProject));
        }

        private ValueTask ProcessWorkQueueAsync(
            ImmutableSegmentedList<(bool solutionChanged, ProjectId? changedProject)> workQueue,
            CancellationToken cancellationToken
        )
        {
            ThisCanBeCalledOnAnyThread();
            ...
            Contract.ThrowIfNull(
                _workQueue,
                "How could we be processing a workqueue change without a workqueue?"
            );
        ...
        )
        {
            ...
            await PerformNuGetProjectServiceWorkAsync<object>(
                    async (nugetService, cancellationToken) =>
                    {
                        // Figure out the entire set of projects to process.
                        using var _ = PooledHashSet<ProjectId>.GetInstance(
                            out var projectsToProcess
                        );
        ...
        private async Task<T?> PerformNuGetProjectServiceWorkAsync<T>(
            Func<INuGetProjectService, CancellationToken, ValueTask<T?>> doWorkAsync,
            CancellationToken cancellationToken
        )
        {
            ...
            var nugetService = await serviceBroker
                .GetProxyAsync<INuGetProjectService>(
        ...
        private static async Task<ImmutableDictionary<string, string>> GetInstalledPackagesMapAsync(
            INuGetProjectService nugetService,
            Guid projectGuid,
            CancellationToken cancellationToken
        )
        {
            var installedPackagesResult = await nugetService
                .GetInstalledPackagesAsync(projectGuid, cancellationToken)
                .ConfigureAwait(false);
        ...
        {
            ThisCanBeCalledOnAnyThread();
            return _projectToInstalledPackageAndVersion.TryGetValue(
                    projectId,
        ...
        public ImmutableArray<string> GetInstalledVersions(string packageName)
        {
            ...
            var versionsAndSplits = installedVersions
                .Select(v => new { Version = v, Split = v.Split('.') })
                .ToList();
        ...
        public ImmutableArray<Project> GetProjectsWithInstalledPackage(
            Solution solution,
            string packageName,
            string version
        )
        ...
        public bool CanShowManagePackagesDialog() => TryGetOrLoadNuGetPackageManager(out _);
        ...
        {
            public SearchQuery(string packageName) => this.SearchString = packageName;
