...
namespace Microsoft.VisualStudio.LanguageServices.Implementation.CodeCleanup
{
    ...
    internal abstract partial class AbstractCodeCleanUpFixer : ICodeCleanUpFixer
    {
        ...
        public Task<bool> FixAsync(ICodeCleanUpScope scope, ICodeCleanUpExecutionContext context) =>
            scope switch
            {
                TextBufferCodeCleanUpScope textBufferScope => FixTextBufferAsync(
                    textBufferScope,
                    context
                ),
                IVsHierarchyCodeCleanupScope hierarchyContentScope => FixHierarchyContentAsync(
                    hierarchyContentScope,
                    context
                ),
        ...
        )
        {
            ...
            if (hierarchy == null)
            {
                return await FixSolutionAsync(_workspace.CurrentSolution, context)
                    .ConfigureAwait(true);
            }
            ...
            if (projectId is null)
            {
                ...
                if (
                    !hierarchyToProjectMap.TryGetProjectId(
        ...
        private Task<bool> FixDocumentAsync(
            Document document,
            ...
            )
            {
                ...
                if (workspaceStatusService != null)
                {
                    await workspaceStatusService
                        .WaitUntilFullyLoadedAsync(context.OperationContext.UserCancellationToken)
                        .ConfigureAwait(true);
                }
            }
        ...
        )
        {
            ...
            enabledDiagnostics = AdjustDiagnosticOptions(
                enabledDiagnostics,
                enabledFixIds.IsFixIdEnabled
            );
