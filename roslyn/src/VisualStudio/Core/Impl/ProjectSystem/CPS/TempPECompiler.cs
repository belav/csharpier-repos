...
namespace Microsoft.VisualStudio.LanguageServices.Implementation.ProjectSystem.CPS
{
    ...
    internal class TempPECompiler : ITempPECompiler
    {
        ...
        public TempPECompiler(VisualStudioWorkspace workspace) => _workspace = workspace;
        ...
        public async Task<bool> CompileAsync(
            ...
            if (filesToInclude == null || filesToInclude.Count == 0)
            {
                throw new ArgumentException(
                    nameof(filesToInclude),
                    "Must specify some files to compile."
                );
            }
            if (outputFileName == null)
            {
                throw new ArgumentException(
                    ...
                    "Must specify an output file name."
                );
            }
            ...
            foreach (var document in project.Documents)
            {
                ...
                {
                    syntaxTrees.Add(
                        await document.GetRequiredSyntaxTreeAsync(cancellationToken)
                            .ConfigureAwait(false)
                    );
                }
            ...
            using var file = FileUtilities.CreateFileStreamChecked(
                File.Create,
                outputFileName,
                nameof(outputFileName)
            );
