...
namespace Microsoft.VisualStudio.LanguageServices.Implementation.ProjectSystem.CPS
{
    ...
    partial internal class CPSProjectFactory : IWorkspaceProjectContextFactory
    {
        ...
        {
            var data = new IntegrationTestEvaluationData(
                projectFilePath ?? "",
                projectFilePath ?? "",
                assemblyName ?? "",
                binOutputPath ?? "",
            ...
            return CreateProjectContextAsync(
                projectGuid,
                projectUniqueName,
                languageName,
                data,
        ...
        internal sealed class IntegrationTestEvaluationData : EvaluationData
        {
            ...
            public IntegrationTestEvaluationData(
                string projectFilePath,
                string targetPath,
                string assemblyName,
        ...
        )
        {
            ...
            {
                binOutputPath = data.GetRequiredPropertyAbsolutePathValue(
        ...
        private static string? GetIntermediateAssemblyPath(
            EvaluationData data,
            string projectFilePath
        )
        {
            ...
            if (!PathUtilities.IsAbsolute(path))
            {
                throw new InvalidProjectDataException(
                    itemName,
                    values[0],
                    $"Item group '{itemName}' is required to specify an absolute path or a path relative to the directory containing the project: '{values[0]}'."
                );
            }
