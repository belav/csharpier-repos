...
public class VisualStudioOptionStorageTests
{
    ...
    public void OptionValidation()
    {
        ...
        var perLanguageOptionsDefinedInIncorrectAssembly =
            from info in infos
            where
                info.Value.Option.IsPerLanguage
                && info.Value.ContainingAssemblyLanguage is "CSharp" or "VisualBasic"
            select info;
        ...
        var languageSpecificOptionsHaveIncorrectPrefix =
            ...
            where
                ...
                ) != info.Value.ContainingAssemblyLanguage is "CSharp"
                || info.Key.StartsWith(
                    OptionDefinition.VisualBasicConfigNamePrefix,
                    StringComparison.Ordinal
                ) != info.Value.ContainingAssemblyLanguage is "VisualBasic"
            select (info);
