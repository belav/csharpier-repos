    ...
    internal sealed class SerializationValidator
    {
        ...
        internal sealed class ChecksumObjectCollection<T> : IEnumerable<T>
        {
            ...
            )
            {
                ...
                Children = ImmutableArray.CreateRange(
                    collection.Select(c => validator.GetValueAsync<T>(c).Result)
                );
            }
        ...
        {
            var data = await AssetStorage
                .GetTestAccessor()
                .GetRequiredAssetAsync(checksum, CancellationToken.None)
        ...
        {
            var solutionInfo = await new AssetProvider(this)
                .CreateSolutionInfoAsync(scope.SolutionChecksum, CancellationToken.None)
                .ConfigureAwait(false);
        ...
        internal async Task VerifyAssetAsync(SolutionStateChecksums solutionObject)
        {
            await VerifyAssetSerializationAsync<SolutionInfo.SolutionAttributes>(
                    ...
                    (v, k, s) => new SolutionAsset(v.Checksum, v)
                )
                .ConfigureAwait(false);

            foreach (var (projectChecksum, projectId) in solutionObject.Projects)
            {
                var projectObject = await GetValueAsync<ProjectStateChecksums>(projectChecksum)
                    .ConfigureAwait(false);
        ...
        private static void AssertChecksumCollectionEqual<TId>(
            ChecksumsAndIds<TId> collection1,
            ChecksumsAndIds<TId> collection2
        )
        {
        ...
        private static void AssertChecksumCollectionEqual(
            ...
            ChecksumCollection collection2
        )
        {
        ...
        )
        {
            ...
            AssertChecksumCollectionEqual(
                solutionObject1.AnalyzerReferences,
                solutionObject2.AnalyzerReferences
            );
            Assert.Equal(
                solutionObject1.FrozenSourceGeneratedDocumentIdentity,
                solutionObject2.FrozenSourceGeneratedDocumentIdentity
            );
            Assert.Equal(
                solutionObject1.FrozenSourceGeneratedDocumentText,
                solutionObject2.FrozenSourceGeneratedDocumentText
            );

            ProjectStatesEqual(
                ToProjectObjects(solutionObject1.Projects.Checksums),
                ToProjectObjects(solutionObject2.Projects.Checksums)
            );
        }

        private void ProjectStateEqual(
        ...
        )
        {
            ...
            AssertChecksumCollectionEqual(
                projectObjects1.ProjectReferences,
                projectObjects2.ProjectReferences
            );
            AssertChecksumCollectionEqual(
                projectObjects1.MetadataReferences,
                projectObjects2.MetadataReferences
            );
            AssertChecksumCollectionEqual(
                projectObjects1.AnalyzerReferences,
                projectObjects2.AnalyzerReferences
            );
            AssertChecksumCollectionEqual(projectObjects1.Documents, projectObjects2.Documents);
            AssertChecksumCollectionEqual(
                projectObjects1.AdditionalDocuments,
                projectObjects2.AdditionalDocuments
            );
            AssertChecksumCollectionEqual(
                projectObjects1.AnalyzerConfigDocuments,
                projectObjects2.AnalyzerConfigDocuments
            );

            DocumentStatesEqual(
                ToDocumentObjects(projectObjects1.Documents.Checksums),
                ToDocumentObjects(projectObjects2.Documents.Checksums)
            );
            DocumentStatesEqual(
                ToDocumentObjects(projectObjects1.AdditionalDocuments.Checksums),
                ToDocumentObjects(projectObjects2.AdditionalDocuments.Checksums)
            );
            DocumentStatesEqual(
                ToDocumentObjects(projectObjects1.AnalyzerConfigDocuments.Checksums),
                ToDocumentObjects(projectObjects2.AnalyzerConfigDocuments.Checksums)
            );
        }

        private static void DocumentStateEqual(
            DocumentStateChecksums documentObjects1,
            DocumentStateChecksums documentObjects2
        )
        {
        ...
        private void ProjectStatesEqual(
        ...
        private static void DocumentStatesEqual(
            ChecksumObjectCollection<DocumentStateChecksums> documentObjects1,
            ChecksumObjectCollection<DocumentStateChecksums> documentObjects2
        )
        {
        ...
        )
        {
            ...
            await VerifyCollectionInService(
                    ToDocumentObjects(projectObject.Documents.Checksums),
            ...
            await VerifyCollectionInService(
                    ToDocumentObjects(projectObject.AdditionalDocuments.Checksums),
        ...
        )
        {
            ...
            {
                await VerifyChecksumInServiceAsync(checksum, expectedItemKind)
                    .ConfigureAwait(false);
            }
        }
        ...
        internal static void SynchronizationObjectEqual<T>(
            ...
            ChecksumEqual(
                ...
                checksumObject2.Checksum,
                checksumObject2.Kind
            );
        ...
        internal static void ChecksumEqual(
            Checksum checksum1,
