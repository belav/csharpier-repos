...
namespace Microsoft.CodeAnalysis.Remote.UnitTests
{
    ...
    public class SnapshotSerializationTests
    {
        private static Workspace CreateWorkspace(Type[] additionalParts = null) =>
            new AdhocWorkspace(
                FeaturesTestCompositions.Features.AddParts(additionalParts)
        ...
        internal static Solution CreateFullSolution(Workspace workspace)
        {
            ...
            var languages = ImmutableHashSet.Create(
                LanguageNames.CSharp,
                LanguageNames.VisualBasic
            );
            var solutionOptions = solution.Workspace.Services.GetRequiredService<IOptionService>()
            ...
            solution =
                document2.Project.Solution.GetRequiredProject(project1.Id)
                    .AddProjectReference(
                    ...
                    .AddAnalyzerReference(
                        new AnalyzerFileReference(
                            Path.Combine(TempRoot.Root, "path1"),
                            new TestAnalyzerAssemblyLoader()
                        )
                    )
                    .AddAdditionalDocument(
                        "Additional",
                        SourceText.From("hello"),
                        ImmutableArray.Create("test"),
                        @".\Add"
                    ).Project.Solution;

            return solution.WithAnalyzerReferences(
                    new[]
                    {
                        new AnalyzerFileReference(
                            Path.Combine(TempRoot.Root, "path2"),
                            new TestAnalyzerAssemblyLoader()
                        )
                    }
                )
        ...
        public async Task CreateSolutionSnapshotId_Empty()
        {
            ...
            var solutionSyncObject = await validator.AssetStorage.GetAssetAsync(
                    scope.SolutionInfo.ScopeId,
                    checksum,
                    CancellationToken.None
                )
                .ConfigureAwait(false);
            ...
            var solutionObject = await validator.GetValueAsync<SolutionStateChecksums>(checksum)
                .ConfigureAwait(false);
            await validator.VerifyChecksumInServiceAsync(
                    solutionObject.Attributes,
                    WellKnownSynchronizationKind.SolutionAttributes
                )
                .ConfigureAwait(false);
            await validator.VerifyChecksumInServiceAsync(
                    solutionObject.Options,
                    WellKnownSynchronizationKind.OptionSet
                )
                .ConfigureAwait(false);

            var projectsSyncObject = await validator.AssetStorage.GetAssetAsync(
                    scope.SolutionInfo.ScopeId,
                    solutionObject.Projects.Checksum,
                    CancellationToken.None
                )
                .ConfigureAwait(false);
            await validator.VerifySynchronizationObjectInServiceAsync(projectsSyncObject)
                .ConfigureAwait(false);
        ...
        public async Task CreateSolutionSnapshotId_Empty_Serialization()
        {
            ...
            using var scope = await validator.AssetStorage.StoreAssetsAsync(
                ...
                .ConfigureAwait(false);
            await validator.VerifySolutionStateSerializationAsync(
                    solution,
        ...
        public async Task CreateSolutionSnapshotId_Project()
        {
            ...
            var projectSyncObject = await validator.AssetStorage.GetAssetAsync(
                    scope.SolutionInfo.ScopeId,
                    solutionObject.Projects.Checksum,
                    CancellationToken.None
                )
                .ConfigureAwait(false);
            await validator.VerifySynchronizationObjectInServiceAsync(projectSyncObject)
                .ConfigureAwait(false);

            Assert.Equal(1, solutionObject.Projects.Count);
            await validator.VerifySnapshotInServiceAsync(
                    validator.ToProjectObjects(solutionObject.Projects)[0],
                    0,
                    0,
                    0,
                    0,
        ...
        public async Task CreateSolutionSnapshotId_Project_Serialization()
        {
            ...
            using var snapshot = await validator.AssetStorage.StoreAssetsAsync(
                    project.Solution,
                    CancellationToken.None
                )
                .ConfigureAwait(false);
            await validator.VerifySolutionStateSerializationAsync(
                    project.Solution,
                    snapshot.SolutionInfo.SolutionChecksum
                )
                .ConfigureAwait(false);
        }
        ...
        public async Task CreateSolutionSnapshotId()
        {
            ...
            using var scope = await validator.AssetStorage.StoreAssetsAsync(
                ...
                .ConfigureAwait(false);
            var syncObject = await validator.AssetStorage.GetAssetAsync(
                    scope.SolutionInfo.ScopeId,
                    scope.SolutionInfo.SolutionChecksum,
                    CancellationToken.None
                )
        ...
        public async Task CreateSolutionSnapshotId_Serialization()
        {
            ...
            using var workspace = CreateWorkspace();
            var solution = workspace.CurrentSolution;
            var document = solution.AddProject("Project", "Project.dll", LanguageNames.CSharp)
                .AddDocument("Document", SourceText.From(code));
        ...
        public async Task CreateSolutionSnapshotId_Full()
        {
            ...
            var firstProjectChecksum = await solution.GetProject(solution.ProjectIds[0])
                .State.GetChecksumAsync(CancellationToken.None);
            var secondProjectChecksum = await solution.GetProject(solution.ProjectIds[1])
                .State.GetChecksumAsync(CancellationToken.None);
            ...
            await validator.VerifySnapshotInServiceAsync(
                    projects.Where(p => p.Checksum == firstProjectChecksum).First(),
                    1,
                    1,
                    1,
                    1,
                ...
                .ConfigureAwait(false);
            await validator.VerifySnapshotInServiceAsync(
                    projects.Where(p => p.Checksum == secondProjectChecksum).First(),
                    1,
                    0,
                    0,
                    0,
        ...
        public async Task CreateSolutionSnapshotId_Duplicate()
        {
            ...
            using (
                var scope1 = await validator.AssetStorage.StoreAssetsAsync(
                        solution,
                        CancellationToken.None
                    )
                    .ConfigureAwait(false)
            ...
            using (
                var scope2 = await validator.AssetStorage.StoreAssetsAsync(
                        solution,
                        CancellationToken.None
                    )
                    .ConfigureAwait(false)
        ...
        public void MetadataReference_RoundTrip_Test()
        {
            ...
            var assetFromFile = new SolutionAsset(
        ...
        public async Task Workspace_RoundTrip_Test()
        {
            ...
            var solutionObject1 = await validator.GetValueAsync<SolutionStateChecksums>(
                    scope1.SolutionInfo.SolutionChecksum
                )
                .ConfigureAwait(false);

            // create new snapshot from recovered solution
            using var scope2 = await validator.AssetStorage.StoreAssetsAsync(
                    recovered,
                    CancellationToken.None
                )
                .ConfigureAwait(false);
            ...
            var solutionObject2 = await validator.GetValueAsync<SolutionStateChecksums>(
                    scope2.SolutionInfo.SolutionChecksum
                )
                .ConfigureAwait(false);
            ...
            using var scope3 = await validator.AssetStorage.StoreAssetsAsync(
                    roundtrip,
                    CancellationToken.None
                )
                .ConfigureAwait(false);
            // verify asset created by rount trip solution is good
            var solutionObject3 = await validator.GetValueAsync<SolutionStateChecksums>(
                    scope3.SolutionInfo.SolutionChecksum
                )
                .ConfigureAwait(false);
        ...
        public async Task OptionSet_Serialization_CustomValue()
        {
            ...
            var newQualifyFieldAccessValue = new CodeStyleOption2<bool>(
                false,
                NotificationOption2.Error
            );
            var newQualifyMethodAccessValue = new CodeStyleOption2<bool>(
                true,
                NotificationOption2.Warning
            );
            var newVarWhenTypeIsApparentValue = new CodeStyleOption2<bool>(
                false,
            ...
            workspace.TryApplyChanges(
                workspace.CurrentSolution.WithOptions(
                    workspace.Options.WithChangedOption(
                            ...
                            LanguageNames.CSharp,
                            newQualifyFieldAccessValue
                        )
                        .WithChangedOption(
                            CodeStyleOptions2.QualifyMethodAccess,
                            LanguageNames.VisualBasic,
                            newQualifyMethodAccessValue
                        )
                        .WithChangedOption(
                            CSharpCodeStyleOptions.VarWhenTypeIsApparent,
                            newVarWhenTypeIsApparentValue
                        )
                        .WithChangedOption(
                            CodeStyleOptions2.PreferIntrinsicPredefinedTypeKeywordInMemberAccess,
                            LanguageNames.VisualBasic,
                            newPreferIntrinsicPredefinedTypeKeywordInMemberAccessValue
                        )
                )
            );
            ...
            {
                var actualQualifyFieldAccessValue = options.GetOption(
                    CodeStyleOptions2.QualifyFieldAccess,
                    LanguageNames.CSharp
                );
                Assert.Equal(newQualifyFieldAccessValue, actualQualifyFieldAccessValue);

                var actualQualifyMethodAccessValue = options.GetOption(
                    CodeStyleOptions2.QualifyMethodAccess,
                    LanguageNames.VisualBasic
                );
                Assert.Equal(newQualifyMethodAccessValue, actualQualifyMethodAccessValue);

                var actualVarWhenTypeIsApparentValue = options.GetOption(
                    CSharpCodeStyleOptions.VarWhenTypeIsApparent
                );
                Assert.Equal(newVarWhenTypeIsApparentValue, actualVarWhenTypeIsApparentValue);

                var actualPreferIntrinsicPredefinedTypeKeywordInMemberAccessValue =
                    options.GetOption(
                        CodeStyleOptions2.PreferIntrinsicPredefinedTypeKeywordInMemberAccess,
                        LanguageNames.VisualBasic
                    );
                Assert.Equal(
                    newPreferIntrinsicPredefinedTypeKeywordInMemberAccessValue,
                    actualPreferIntrinsicPredefinedTypeKeywordInMemberAccessValue
                );
            }
        }
        ...
        public void Missing_Analyzer_Serialization_Test()
        {
            ...
            var reference = new AnalyzerFileReference(
                Path.Combine(TempRoot.Root, "missing_reference"),
                new MissingAnalyzerLoader()
            );
        ...
        public void RoundTrip_Analyzer_Serialization_Test()
        {
            ...
            var reference = new AnalyzerFileReference(
                location,
                new MockShadowCopyAnalyzerAssemblyLoader(
        ...
        public async Task SnapshotWithIdenticalAnalyzerFiles()
        {
            ...
            var file1 = dir.CreateFile("analyzer1.dll")
                .WriteAllBytes(TestResources.AnalyzerTests.FaultyAnalyzer);
            var file2 = dir.CreateFile("analyzer2.dll")
                .WriteAllBytes(TestResources.AnalyzerTests.FaultyAnalyzer);

            var analyzer1 = new AnalyzerFileReference(
                file1.Path,
                TestAnalyzerAssemblyLoader.LoadNotImplemented
            );
            var analyzer2 = new AnalyzerFileReference(
                file2.Path,
                TestAnalyzerAssemblyLoader.LoadNotImplemented
            );
        ...
        public async Task EmptyAssetChecksumTest()
        {
            var document = CreateWorkspace()
                .CurrentSolution.AddProject("empty", "empty", LanguageNames.CSharp)
                .AddDocument("empty", SourceText.From(""));
            var serializer =
                document.Project.Solution.Workspace.Services.GetService<ISerializerService>();

            var source = serializer.CreateChecksum(
                await document.GetTextAsync().ConfigureAwait(false),
                CancellationToken.None
            );
            var metadata = serializer.CreateChecksum(
                new MissingMetadataReference(),
                CancellationToken.None
            );
            var analyzer = serializer.CreateChecksum(
                new AnalyzerFileReference(
                    Path.Combine(TempRoot.Root, "missing"),
                    new MissingAnalyzerLoader()
                ),
                CancellationToken.None
            );
      ...
      <summary>Supports all classes in the .NET Framework class hierarchy and provides low-level services to derived classes. This is the ultimate base class of all classes in the .NET Framework; it is the root of the type hierarchy.To browse the .NET Framework source code for this type, see the Reference Source.</summary>
            ...
            var solution =
                workspace.CurrentSolution.AddProject("Project", "Project.dll", LanguageNames.CSharp)
                    .AddMetadataReference(
                        MetadataReference.CreateFromFile(tempCorlib.Path)
                    ).Solution;
            ...
            var compilation = await recovered.Projects.First()
        ...
        public void TestEncodingSerialization()
        {
            ...
            using (var stream = SerializableBytes.CreateWritableStream())
            {
                ...
                var newText = serializer.Deserialize<SourceText>(
                    sourceText.GetWellKnownSynchronizationKind(),
                    objectReader,
                    CancellationToken.None
                );
        ...
        {
            var csharpOptions = CSharp.CSharpCompilation.Create("dummy")
                .Options.WithNullableContextOptions(NullableContextOptions.Warnings)
                .WithMetadataImportOptions(MetadataImportOptions.All);
            var vbOptions = VisualBasic.VisualBasicCompilation.Create("dummy")
            ...
            void VerifyOptions(CompilationOptions originalOptions)
            {
                ...
                stream.Position = 0;
                using var objectReader = ObjectReader.TryGetReader(stream);
                var recoveredOptions = serializer.Deserialize<CompilationOptions>(
                    originalOptions.GetWellKnownSynchronizationKind(),
                    objectReader,
                    CancellationToken.None
                );
        ...
        )
        {
            ...
            using var recoveredScope = await validator.AssetStorage.StoreAssetsAsync(
                    recoveredSolution,
                    CancellationToken.None
                )
                .ConfigureAwait(false);
        ...
        private static SolutionAsset CloneAsset(ISerializerService serializer, SolutionAsset asset)
        {
            ...
            var recovered = serializer.Deserialize<object>(
                asset.Kind,
                reader,
                CancellationToken.None
            );
            var assetFromStorage = new SolutionAsset(
                serializer.CreateChecksum(recovered, CancellationToken.None),
                recovered
            );
