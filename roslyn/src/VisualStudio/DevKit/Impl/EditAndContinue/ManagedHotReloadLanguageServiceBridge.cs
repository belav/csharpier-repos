        ...
        IExportedBrokeredService
{
    ...
    private const string MonikerName =
        BrokeredServiceDescriptors.LanguageServerComponentNamespace
        + "."
        + BrokeredServiceDescriptors.LanguageServerComponentName
        + "."
        + ServiceName;

    public static readonly ServiceJsonRpcDescriptor ServiceDescriptor =
        BrokeredServiceDescriptors.CreateServerServiceDescriptor(ServiceName, new(ServiceVersion));
    ...
    public ManagedHotReloadLanguageServiceBridge(
        InternalContracts.IManagedHotReloadLanguageService service
    ) => _service = service;

    ServiceRpcDescriptor IExportedBrokeredService.Descriptor => ServiceDescriptor;
