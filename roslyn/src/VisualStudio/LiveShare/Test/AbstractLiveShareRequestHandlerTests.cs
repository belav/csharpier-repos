...
namespace Microsoft.VisualStudio.LanguageServices.LiveShare.UnitTests
{
    public abstract class AbstractLiveShareRequestHandlerTests : AbstractLanguageServerProtocolTests
    {
        private static readonly TestComposition s_composition = LiveShareTestCompositions
            .Features.AddParts(typeof(MockDocumentNavigationServiceFactory))
            .AddParts(typeof(TestWorkspaceRegistrationService))
            .AddParts(typeof(TestWorkspaceConfigurationService));

        protected AbstractLiveShareRequestHandlerTests(ITestOutputHelper testOutputHelper)
        ...
        private class MockHostProtocolConverter : IHostProtocolConverter
        {
            ...
            public MockHostProtocolConverter(Func<Uri, Uri> uriConversionFunction) =>
                _uriConversionFunction = uriConversionFunction;
        ...
        protected static async Task<ResponseType> TestHandleAsync<RequestType, ResponseType>(
            Solution solution,
            RequestType request,
            string methodName
        )
        {
            var requestContext = new RequestContext<Solution>(
                ...
                JObject.FromObject(new ClientCapabilities())
            );
            return await GetHandler<RequestType, ResponseType>(solution, methodName)
                .HandleAsync(request, requestContext, CancellationToken.None);
        }
        ...
        protected static ILspRequestHandler<RequestType, ResponseType, Solution> GetHandler<
            RequestType,
            ResponseType
        >(Solution solution, string methodName)
        {
            ...
            return (ILspRequestHandler<RequestType, ResponseType, Solution>)
            ...
            {
                var attribute = (ExportLspRequestHandlerAttribute)
                    Attribute.GetCustomAttribute(
                        handler.GetType(),
                        typeof(ExportLspRequestHandlerAttribute)
                    );
