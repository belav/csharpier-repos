    ...
    public class ReferencedModulesTests : ExpressionCompilerTestBase
    {
        ...
        public void UnreferencedAssemblies()
        {
            var (identityMscorlib, moduleMscorlib) = (
                MscorlibRef.GetAssemblyIdentity(),
                MscorlibRef.ToModuleInstance()
            );
            var (identityIntrinsic, moduleIntrinsic) = (
                ExpressionCompilerTestHelpers.IntrinsicAssemblyReference.GetAssemblyIdentity(),
                ExpressionCompilerTestHelpers.IntrinsicAssemblyReference.ToModuleInstance()
            );
            var (identityA1, moduleA1, refA1) = Compile(
                "A1",
                "public class A1 { static void M() { } }"
            );
            var (identityA2, moduleA2, refA2) = Compile(
                "A2",
                "public class A2 { static void M() { } }"
            );
            var (identityB1, moduleB1, refB1) = Compile(
                "B1",
                "public class B1 : A1 { static void M() { } }",
                refA1
            );
            var (identityB2, moduleB2, refB2) = Compile(
                "B2",
                "public class B2 : A1 { static void M() { } }",
                refA1
            );
            var (identityC, moduleC, refC) = Compile(
                "C",
                "public class C1 : B2 { static void M() { } } public class C2 : A2 { }",
                refA1,
                refA2,
                refB2
            );
            ...
            )
            {
                ...
                var context = CreateMethodContext(
                    new AppDomain(),
                    ImmutableArray
                        .Create(moduleMscorlib, moduleA2, moduleB1, moduleB2, moduleC)
                ...
                context = CreateMethodContext(
                    new AppDomain(),
                    ImmutableArray
                        .Create(moduleMscorlib, moduleA1, moduleA2, moduleB1, moduleB2, moduleC)
                        .SelectAsArray(m => m.MetadataBlock),
                    stateB2
                );
...
IL_0005:  ret
                ...
                VerifyResolutionRequests(
                    context,
                    (identityB2, identityB2, 1),
                    (identityA1, identityA1, 1),
                    (identityA2, identityA2, 1)
                );

                // Other EvaluationContext.CreateMethodContext overload.
                // A1.M with all assemblies.
                var allBlocks = ImmutableArray
                    .Create(moduleMscorlib, moduleA1, moduleA2, moduleB1, moduleB2, moduleC)
                    .SelectAsArray(m => m.MetadataBlock);
...
IL_0005:  ret
        ...
        {
            var publicKeyA = ImmutableArray.CreateRange(
                new byte[]
                {
                    0x00,
                    0x24,
            ...
            var (identityA3, moduleA3, refA3) = Compile(
                new AssemblyIdentity(
                    "a",
                    new Version(3, 3, 3, 3),
                    publicKeyOrToken: publicKeyA,
            ...
            var (identityB1, moduleB1, refB1) = Compile(
                ...
                "public class B : A { static void M() { } }",
            ...
            )
            {
                ...
                var context = CreateMethodContext(
                    new AppDomain(),
                    ImmutableArray
                        .Create(moduleMscorlib, moduleA2, moduleB1)
                        .SelectAsArray(m => m.MetadataBlock),
                    stateB
                );
                string error;
...
IL_0005:  ret
                ...
                context = CreateMethodContext(
                    new AppDomain(),
                    ImmutableArray
                        .Create(moduleMscorlib, moduleA3, moduleB1)
                        .SelectAsArray(m => m.MetadataBlock),
                    stateB
                );
                testData = new CompilationTestData();
...
IL_0005:  ret
                ...
                context = CreateMethodContext(
                    new AppDomain(),
                    ImmutableArray
                        .Create(moduleMscorlib, moduleA1, moduleB1)
                        .SelectAsArray(m => m.MetadataBlock),
                    stateB
                );
                testData = new CompilationTestData();
                context.CompileExpression("new B()", out error, testData);
                Assert.Equal(
                    "error CS1705: Assembly 'B' with identity 'B, Version=1.1.1.1, Culture=neutral, PublicKeyToken=null' uses 'A, Version=2.2.2.2, Culture=neutral, PublicKeyToken=1f8a32457d187bf3' which has a higher version than referenced assembly 'A' with identity 'A, Version=1.1.1.1, Culture=neutral, PublicKeyToken=1f8a32457d187bf3'",
                    error
                );
                ...
                context = CreateMethodContext(
                    new AppDomain(),
                    ImmutableArray
                        .Create(moduleMscorlib, moduleA1, moduleA3, moduleA2, moduleB1)
                        .SelectAsArray(m => m.MetadataBlock),
                    stateB
                );
                testData = new CompilationTestData();
...
IL_0005:  ret
                ...
                context = CreateMethodContext(
                    new AppDomain(),
                    ImmutableArray
                        .Create(moduleMscorlib, moduleA3, moduleA1, moduleA3, moduleA1, moduleB1)
                        .SelectAsArray(m => m.MetadataBlock),
                    stateB
                );
                testData = new CompilationTestData();
...
IL_0005:  ret
        ...
        {
            var publicKeyOther = ImmutableArray.CreateRange(
                new byte[]
                {
                    0x00,
                    0x24,
            ...
            var (identityOther, moduleOther, refOther) = Compile(
                new AssemblyIdentity(
                    identityMscorlib.Name,
                    new Version(1, 1, 1, 1),
                    publicKeyOrToken: publicKeyOther,
                    hasPublicKey: true
                ),
                "class Other { }",
                options,
            ...
            var (identityA, moduleA, refA) = Compile(
            ...
            var (identityB, moduleB, refB) = Compile(
                new AssemblyIdentity("B", new Version(1, 1, 1, 1)),
                "public class B : A { static void M() { } }",
                TestOptions.DebugDll,
                refA,
            ...
            {
                var stateB = GetContextState(runtime, "B.M");
                var (moduleVersionId, symReader, methodToken, localSignatureToken, ilOffset) =
                    GetContextState(runtime, "B.M");

                var context = CreateMethodContext(
                    new AppDomain(),
                    ImmutableArray
                        .Create(moduleMscorlib, moduleOther, moduleA, moduleB)
                        .SelectAsArray(m => m.MetadataBlock),
                    stateB
                );
                string error;
...
IL_0005:  ret
                ...
                context = CreateMethodContext(
                    new AppDomain(),
                    ImmutableArray
                        .Create(moduleB, moduleA, moduleOther, moduleMscorlib)
                        .SelectAsArray(m => m.MetadataBlock),
                    stateB
                );
                testData = new CompilationTestData();
...
IL_0005:  ret
        ...
        public void ReuseCompilation()
        {
            ...
            var (identityA2, moduleA2, refA2) = Compile(
                "A2",
                "public class A2 { static void M() { } } public class A3 { static void M() { } }"
            );
            var (identityB1, moduleB1, refB1) = Compile(
                "B1",
                "public class B1 : A1 { static void M() { } } public class B2 { static void M() { } }",
                refA1
            );
                ...
                @"class C
{
    ...
    static void M()
    {
            ...
            var publicKeyB = ImmutableArray.CreateRange(
                new byte[]
                {
                    0x00,
                    0x24,
            ...
            WithRuntimeInstance(
                ...
                runtime =>
                {
                    ...
                    Assert.Equal(
                        identityAS2,
                        context
                            .Compilation.GlobalNamespace.GetMembers("A")
                    ...
                    Assert.Equal(
                        identityBS2,
                        context
                            .Compilation.GlobalNamespace.GetMembers("B")
                            .OfType<NamedTypeSymbol>()
                            .Single()
                            .ContainingAssembly.Identity
                    );
...
IL_0005:  ret
        ...
        ) Compile(string assemblyName, string source, params MetadataReference[] references)
        {
        ...
        )
        {
            ...
            using (var runtime = new RuntimeInstance(modules, DebugInformationFormat.Pdb))
            {
                ...
                var actualReferences = blocks.MakeAssemblyReferences(
                    moduleVersionId,
                    CompilationExtensions.IdentityComparer,
                    MakeAssemblyReferencesKind.DirectReferencesOnly,
                    out referencesBySimpleName
                );
                ...
                actualReferences = blocks.MakeAssemblyReferences(
                    moduleVersionId,
                    CompilationExtensions.IdentityComparer,
                    MakeAssemblyReferencesKind.AllReferences,
                    out referencesBySimpleName
...
public class B
{
    static void M()
    {
        var x = new A();
            ...
            var compilationB = CreateCompilationWithMscorlib40AndSystemCore(
                sourceB,
                options: TestOptions.DebugDll,
                references: new[] { moduleA.GetReference() }
            );
            ...
            contextFactory = CreateMethodContextFactory(
                moduleVersionId,
                symReader,
                methodToken,
                localSignatureToken
            ...
            Assert.Equal(
                $"error CS0121: {string.Format(CSharpResources.ERR_AmbigCall, "N.E.F(A)", "N.E.F(A)")}",
                errorMessage
            );
...
IL_0005:  ret
            ...
            Assert.Equal(
                ((MethodSymbol)methodData.Method).ReturnType.ContainingAssembly.ToDisplayString(),
                ...
                @"public class B : A
{
            ...
            var referenceC = AssemblyMetadata
                .CreateFromImage(TestResources.ExpressionCompiler.Empty)
                .GetReference();
                ...
                @"class C
{
    static void Main()
    {
        ...
        var o = (System.Collections.ObjectModel.ReadOnlyDictionary<object, object>)null;
            ...
            var contractReferences = ImmutableArray.Create(
                systemConsoleRef,
                SystemRuntimePP7Ref,
                systemObjectModelRef
            );
            var runtimeReferences = ImmutableArray.Create(
                systemConsoleRef,
                MscorlibFacadeRef,
            ...
            compilation.VerifyDiagnostics(
                ...
                //         var t = typeof(System.Console);
                Diagnostic(ErrorCode.ERR_SameFullNameAggAgg, "Console")
                    .WithArguments(
                        "System.Console, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null",
                        "System.Console",
                ...
                //         var o = (System.Collections.ObjectModel.ReadOnlyDictionary<object, object>)null;
                Diagnostic(ErrorCode.ERR_SameFullNameAggAgg, "ReadOnlyDictionary<object, object>")
                    .WithArguments(
                        "System.ObjectModel, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null",
...
#pragma warning restore 0436
            ...
            var compCorLib = CreateEmptyCompilation(
                sourceCorLib,
            ...
            ExpressionCompilerTestHelpers.EmitCorLibWithAssemblyReferences(
                compCorLib,
                null,
                (moduleBuilder, emitOptions) =>
                    new PEAssemblyBuilderWithAdditionalReferences(
                        moduleBuilder,
                        emitOptions,
                        objectType.GetCciAdapter()
                    ),
                out peBytes,
                out pdbBytes
            );
            ...
            using (var reader = new PEReader(peBytes))
            {
                ...
                var moduleInstance = ModuleInstance.Create(
                    metadata,
                    metadataReader.GetModuleVersionIdOrThrow()
                );
                    ...
                    @"class C
{
    static void M()
    {
                ...
                var comp = CreateEmptyCompilation(
                    source,
                    options: TestOptions.DebugDll,
                    references: new[] { refLib, AssemblyMetadata.Create(module).GetReference() },
                    parseOptions: parseOptions
                );
  ...
  IL_0005:  ret
                    ...
                    testData = new CompilationTestData();
                    context.CompileExpression("1", out error, testData);
                    Assert.Equal(
                    ...
                    testData = new CompilationTestData();
                    context.CompileExpression("(new object()).F()", out error, testData);
                    Assert.Equal(
                        "error CS0570: 'object.F()' is not supported by the language",
                        error
                    );

                    // Invalid expression: type forwarded to missing referenced assembly.
                    testData = new CompilationTestData();
                    context.CompileExpression("new Private2()", out error, testData);
                    Assert.Equal(
                        "error CS0246: The type or namespace name 'Private2' could not be found (are you missing a using directive or an assembly reference?)",
                        error
                    );
                }
            }
...
#pragma warning restore 0436
            ...
            using (var reader = new PEReader(peBytes))
            {
                ...
                using (var runtime = RuntimeInstance.Create(new[] { moduleInstance }))
                {
                    ...
                    context.CompileExpression("new N.Private()", out error, testData);
                    Assert.Equal(
                        "error CS0246: The type or namespace name 'N' could not be found (are you missing a using directive or an assembly reference?)",
                        error
                    );
                }
            }
                ...
                @"class C
{
  ...
  IL_000b:  ret
                ...
                IAssemblyReference
        {
            ...
            internal PEAssemblyBuilderWithAdditionalReferences(
            ...
            Version IAssemblyReference.AssemblyVersionPattern =>
                ((IAssemblyReference)_builder).AssemblyVersionPattern;
        }
    }
}
