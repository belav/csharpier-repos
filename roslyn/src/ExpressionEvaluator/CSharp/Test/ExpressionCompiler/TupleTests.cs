...
namespace System
{
    public struct ValueTuple<T1, T2>
    {
        ...
        public ValueTuple(T1 item1, T2 item2) => (Item1, Item2) = (item1, item2);
            ...
            var corlibWithoutVT = CreateEmptyCompilation(
                new[]
                {
                    Parse(String.Format(versionTemplate, "1") + corlib_cs, options: parseOptions),
                },
                assemblyName: "corlib"
            );
            ...
            var corlibWithVT = CreateEmptyCompilation(
                new[]
                {
                    Parse(
                        String.Format(versionTemplate, "2") + corlib_cs + valuetuple_cs,
...
";
            var app = CreateEmptyCompilation(
                source + valuetuple_cs,
                references: new[] { corlibWithoutVTRef },
            ...
            var runtime = CreateRuntimeInstance(
                new[] { app.ToModuleInstance(), corlibWithVT.ToModuleInstance() }
            );
            ...
            )
            {
                ...
                AssertEx.SetEqual(
                    new[]
                    {
                        ...
                        "ValueTuple`2, System, AssemblyReference:"
                            + appRef // ValueTuple comes from app, not corlib
                        ,
                    },
                ...
                @"class C
{
    static void M()
    {
        var x = (1, 2, 3, 4, 5, 6, 7, 8);
            ...
            var comp = CreateCompilationWithMscorlib40(
                source,
                new[] { SystemRuntimeFacadeRef, ValueTupleRef },
                options: TestOptions.DebugDll
            );
            WithRuntimeInstance(
                comp,
                new[] { MscorlibRef, SystemCoreRef, SystemRuntimeFacadeRef, ValueTupleRef },
                runtime =>
                {
                    var context = CreateMethodContext(runtime, "C.M");
                    var testData = new CompilationTestData();
    ...
    static void M()
    {
            ...
            WithRuntimeInstance(
                ...
                runtime =>
                {
                    var context = CreateMethodContext(runtime, "C.M");
                    // (int A, (int, int D) B)[] t;
                    var aliasElementNames = new ReadOnlyCollection<string>(
                        new[] { "A", "B", null, "D" }
                    );
                    var alias = new Alias(
