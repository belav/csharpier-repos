                ...
                runtime =>
                {
                    ...
                    foreach (var displayClassType in displayClassTypes)
                    {
                        ...
                        Assert.Equal(
                            ...
                            GeneratedNames.GetKind(displayClassName)
                        );
                        foreach (
                            var displayClassMethod in displayClassType.GetMembers()
                                .OfType<MethodSymbol>()
                                .Where(
                                    m =>
...
public class C
{
        ...
        where T : IEnumerable<U>
    {
        ...
        )
        {
            ...
            var iteratorMethod = originalType.GetMembers("M")
                .OfType<MethodSymbol>()
                .Single(isDesiredOverload);

            var stateMachineType = originalType.GetMembers()
