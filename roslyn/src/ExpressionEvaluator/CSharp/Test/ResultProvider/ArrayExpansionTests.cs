...
namespace Microsoft.CodeAnalysis.CSharp.ExpressionEvaluator.UnitTests
{
    public class ArrayExpansionTests : CSharpResultProviderTestBase
    {
        ...
        public void ZeroLengthArray()
        {
            ...
            var evalResult = FormatResult(rootExpr, value);
            Verify(evalResult, EvalResult(rootExpr, "{object[0]}", "object[]", rootExpr));
        ...
        public void MultiDimensionalArray()
        {
            ...
            Verify(
                ...
                EvalResult(
                    ...
                    "4",
                    "int",
                    "(new int[,] { { 1, 2 }, { 3, 4 }, { 5, 6 } })[1, 1]"
                ),
                EvalResult(
                    "[2, 0]",
                    "5",
                    "int",
                    "(new int[,] { { 1, 2 }, { 3, 4 }, { 5, 6 } })[2, 0]"
                ),
                EvalResult(
                    "[2, 1]",
                    "6",
        ...
        public void ZeroLengthMultiDimensionalArray()
        {
            ...
            var evalResult = FormatResult(rootExpr, value);
            Verify(evalResult, EvalResult(rootExpr, "{int[2, 3, 0]}", "int[,,]", rootExpr));
            ...
            Verify(evalResult, EvalResult(rootExpr, "{int[2, 0, 3]}", "int[,,]", rootExpr));
            Verify(GetChildren(evalResult));

            rootExpr = "new int[0, 2, 3]";
            value = CreateDkmClrValue(new int[0, 2, 3]);
            evalResult = FormatResult(rootExpr, value);
            Verify(evalResult, EvalResult(rootExpr, "{int[0, 2, 3]}", "int[,,]", rootExpr));
            Verify(GetChildren(evalResult));

            rootExpr = "new int[0, 0, 0]";
            value = CreateDkmClrValue(new int[0, 0, 0]);
            evalResult = FormatResult(rootExpr, value);
            Verify(evalResult, EvalResult(rootExpr, "{int[0, 0, 0]}", "int[,,]", rootExpr));
            Verify(GetChildren(evalResult));
        }
        ...
        public void NullArray()
        {
            ...
            var evalResult = FormatResult(
                rootExpr,
                CreateDkmClrValue(new int[][,,] { null, new int[2, 3, 4] })
            );
            Verify(
                evalResult,
                EvalResult(
            ...
            Verify(
                ...
                EvalResult(
                    ...
                    "null",
                    "int[,,]",
                    "(new int[][,,] { null, new int[2, 3, 4] })[0]"
                ),
                EvalResult(
                    "[1]",
                    "{int[2, 3, 4]}",
                    "int[,,]",
                    "(new int[][,,] { null, new int[2, 3, 4] })[1]",
                    DkmEvaluationResultFlags.Expandable
                )
            );
        }
                ...
                @"class C
{
    ...
    System.Collections.IEnumerable G = new char[] { '2' };
        ...
        public void NonZeroLowerBounds()
        {
            ...
            var array = (int[,])System.Array
                .CreateInstance(typeof(int), new[] { 2, 3 }, new[] { 3, 4 });
            array[3, 4] = 1;
            array[3, 5] = 2;
            array[3, 6] = 3;
            array[4, 4] = 4;
        ...
        {
            var array = (int[,])System.Array
                .CreateInstance(typeof(int), new[] { 2, 1 }, new[] { -3, 4 });
            array[-3, 4] = 1;
            array[-2, 4] = 2;
            var value = CreateDkmClrValue(array);
            var inspectionContext = CreateDkmInspectionContext(radix: 16);
            ...
            Verify(
                ...
                EvalResult(
                    "a",
                    "{int[0xfffffffd..0xfffffffe, 0x00000004..0x00000004]}",
                    "int[,]",
                    "a",
                    DkmEvaluationResultFlags.Expandable
                )
        ...
        public void LazyExpansion()
        {
            ...
            for (int i = 0; i < evalResults.Length; i++)
            {
                ...
                var indices1 = string
                    .Format("{0}, {1}, {2}", offset / 1000000, (offset % 1000000) / 1000, 0);
                var indices2 = string
                    .Format(
                        "{0}, {1}, {2}",
                        (offset + 1) / 1000000,
                        ((offset + 1) % 1000000) / 1000,
                        1
                    );
                Verify(
                    items,
                    EvalResult(
                        string.Format("[{0}]", indices1),
                        "0",
                        "byte",
                        string.Format("{0}[{1}]", parenthesizedExpr, indices1)
                    ),
                    EvalResult(
                        string.Format("[{0}]", indices2),
                        "0",
                        "byte",
                        string.Format("{0}[{1}]", parenthesizedExpr, indices2)
                    )
                );
            }
        }
