...
namespace Microsoft.CodeAnalysis.CSharp.ExpressionEvaluator
{
    internal sealed class EEMethodBinder : Binder
    {
            ...
            : base(next, next.Flags | BinderFlags.InEEMethodBinder)
        {
            ...
            //   1) method is the EEMethodSymbol that we're going to synthesize and hand to the debugger to
            // evaluate.
            //   2) containingMethod is the method that we are conceptually in, e.g. the method containing the
            //      lambda that is on top of the call stack.  Any type parameters will have been replaced with
            // the
            //      corresponding type parameters from (1) and its containing type.
            //   3) method.SubstitutedSourceMethod is the method that we are actually in, e.g. a lambda method
            // in
            //      a display class.  Any type parameters will have been replaced with the corresponding type
            // parameters
            //      from (1).
            // So why do we need all these methods?
            //   1) gives us the parameters that we need to actually bind to (it's no good to bind to the
            // symbols
            //      owned by (2) or (3)).  Also, it happens to contain (3), so we don't need to pass (3)
            // explicitly.
            //   2) is where we want to pretend we're binding expressions, so we make it the containing symbol
            // of
            //      this binder.
            //   3) is where we'll pretend to be for the purposes of looking up parameters by name.  However,
            // any
            ...
            // Note that we never expect this InMethodBinder to find candidate symbols which may be file-local,
            // and therefore pass 'associatedFileIdentifier: null' to the BuckStopsHereBinder.
            _sourceBinder = new InMethodBinder(
                substitutedSourceMethod,
                new BuckStopsHereBinder(
                    next.Compilation,
                    associatedFileIdentifier: null
                ).WithAdditionalFlags(BinderFlags.InEEMethodBinder)
            );
        }
