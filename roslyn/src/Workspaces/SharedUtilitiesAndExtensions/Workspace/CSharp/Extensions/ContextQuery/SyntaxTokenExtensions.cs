...
namespace Microsoft.CodeAnalysis.CSharp.Extensions.ContextQuery
{
    internal static partial class SyntaxTokenExtensions
    {
        ...
        {
            return token.Kind() is SyntaxKind.UsingKeyword or SyntaxKind.ExternKeyword;
        }
        ...
        public static bool IsUsingKeywordInUsingDirective(this SyntaxToken token)
        {
            if (token.IsKind(SyntaxKind.UsingKeyword))
            {
                ...
                if (usingDirective != null && usingDirective.UsingKeyword == token)
                {
                    return true;
                }
            }
                ...
                case SyntaxKind.CloseBraceToken:
                    if (token.Parent.IsKind(SyntaxKind.Block))
                    {
                        if (token.Parent.Parent is StatementSyntax)
                        {
                            ...
                            // do {}
                            if (
                                token.Parent.Parent.Kind()
                                is not SyntaxKind.TryStatement
        ...
        public static bool IsBeginningOfGlobalStatementContext(this SyntaxToken token)
        {
            ...
            if (token.Kind() == SyntaxKind.CloseBraceToken)
            {
                ...
                if (
                    ...
                    && memberDeclaration.GetLastToken(includeZeroWidth: true) == token
            ...
            if (token.Kind() == SyntaxKind.SemicolonToken)
            {
                ...
                if (
                    ...
                    && globalStatement.GetLastToken(includeZeroWidth: true) == token
                )
                    return true;
                ...
                if (
                    token.Parent is FileScopedNamespaceDeclarationSyntax namespaceDeclaration
                    && namespaceDeclaration.SemicolonToken == token
                )
                    return false;
                ...
                {
                    if (
                        compUnit.Usings.Count > 0
                        && compUnit.Usings.Last().GetLastToken(includeZeroWidth: true) == token
                    )
                    {
                        return true;
                    }
                    ...
                    if (
                        compUnit.Externs.Count > 0
                        && compUnit.Externs.Last().GetLastToken(includeZeroWidth: true) == token
                    )
                    {
                        return true;
                    }
            ...
            if (token.Kind() == SyntaxKind.CloseBracketToken)
            {
                ...
                {
                    if (
                        compUnit.AttributeLists.Count > 0
                        && compUnit.AttributeLists.Last().GetLastToken(includeZeroWidth: true)
                            == token
                    )
                        return true;
                }
                ...
                if (token.Parent.IsKind(SyntaxKind.AttributeList))
                {
                    ...
                    if (
                        ...
                        && container.Parent is CompilationUnitSyntax
                    )
                        return true;
                }
            }
        ...
        public static bool IsPreProcessorExpressionContext(this SyntaxToken targetToken)
        {
            ...
            // #if
            // #elif
            if (targetToken.Kind() is SyntaxKind.IfKeyword or SyntaxKind.ElifKeyword)
            {
                return true;
            }
            ...
            // ( |
            if (
                targetToken.Kind() == SyntaxKind.OpenParenToken
        ...
        public static bool IsSwitchLabelContext(this SyntaxToken targetToken)
        {
            ...
            if (
                targetToken.Kind() == SyntaxKind.OpenBraceToken
            ...
            if (targetToken.Kind() is SyntaxKind.SemicolonToken or SyntaxKind.CloseBraceToken)
            {
        ...
        public static bool IsXmlCrefParameterModifierContext(this SyntaxToken targetToken)
        {
            return targetToken.Kind() is SyntaxKind.CommaToken or SyntaxKind.OpenParenToken
                && targetToken.Parent
                    is (
                        kind: SyntaxKind.CrefBracketedParameterList
                            or SyntaxKind.CrefParameterList
                    );
        }
        ...
        )
        {
            ...
            // Goo(bar: |
            if (
                targetToken.Kind() == SyntaxKind.ColonToken
            ...
            if (targetToken.Kind() is SyntaxKind.OpenParenToken or SyntaxKind.CommaToken)
            {
                ...
                {
                    if (
                        targetToken.Parent?.Parent?.Kind()
        ...
        {
            if (
                targetToken.Kind() == SyntaxKind.OperatorKeyword
                && targetToken
        ...
        public static bool IsUnsafeContext(this SyntaxToken targetToken)
        {
            return targetToken
                    ...
                    .Any(s => s.IsKind(SyntaxKind.UnsafeStatement))
                || targetToken
                    .GetAncestors<MemberDeclarationSyntax>()
                    .Any(m =>
                        m.GetModifiers().Any(SyntaxKind.UnsafeKeyword)
                        || targetToken
                            .GetAncestors<LocalFunctionStatementSyntax>()
                            .Any(f => f.GetModifiers().Any(SyntaxKind.UnsafeKeyword))
                    )
                || targetToken
                    .GetAncestors<UsingDirectiveSyntax>()
        ...
        public static bool IsAnyAccessorDeclarationContext(
            this SyntaxToken targetToken,
            int position,
        ...
        )
        {
            return targetToken.IsAccessorDeclarationContext<EventDeclarationSyntax>(position, kind)
                || targetToken.IsAccessorDeclarationContext<PropertyDeclarationSyntax>(
                    position,
                    kind
                )
                || targetToken.IsAccessorDeclarationContext<IndexerDeclarationSyntax>(
                    position,
                    kind
                );
        }

        public static bool IsAccessorDeclarationContext<TMemberNode>(
            this SyntaxToken targetToken,
            int position,
            SyntaxKind kind = SyntaxKind.None
        )
        ...
        private static bool IsAccessorDeclarationContextWorker(ref SyntaxToken targetToken)
        {
            ...
            // int Goo { set; |
            if (
                targetToken.Kind() == SyntaxKind.SemicolonToken
        ...
        public static bool IsTypeParameterVarianceContext(this SyntaxToken targetToken)
        {
            ...
            // delegate X D<[Bar]|
            if (
                targetToken.Kind() == SyntaxKind.LessThanToken
        ...
        public static bool IsNumericTypeContext(
