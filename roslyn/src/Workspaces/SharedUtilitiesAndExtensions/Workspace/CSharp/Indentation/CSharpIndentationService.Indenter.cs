...
namespace Microsoft.CodeAnalysis.CSharp.Indentation
{
    internal partial class CSharpIndentationService
    {
        protected override bool ShouldUseTokenIndenter(
            Indenter indenter,
            out SyntaxToken syntaxToken
        ) =>
            ShouldUseSmartTokenFormatterInsteadOfIndenter(
                indenter.Rules,
                indenter.Root,
                indenter.LineToBeIndented,
                indenter.Options,
                out syntaxToken
            );
        ...
        )
        {
            var rules = ImmutableArray
                .Create(baseIndentationRule)
                .AddRange(CSharpSyntaxFormatting.Instance.GetDefaultFormattingRules());
        ...
        )
        {
            ...
            if (token.IsKind(SyntaxKind.MultiLineRawStringLiteralToken))
            {
                ...
                for (
                    var currentLineNumber = indenter.LineToBeIndented.LineNumber - 1;
                    currentLineNumber >= startLine.LineNumber + 1;
                    currentLineNumber--
                )
                {
            ...
            )
            {
                ...
                {
                    var endLine = sourceText
                        .Lines
                        .GetLineFromPosition(interpolatedExpression.StringEndToken.Span.End);
                    ...
                    // If possible, indent to match the indentation of the previous non-whitespace line contained in the
                    // same raw string. Otherwise, indent to match the ending line of the raw string.
                    var startLine = sourceText
                        .Lines
                        .GetLineFromPosition(interpolatedExpression.StringStartToken.SpanStart);
                    for (
                        var currentLineNumber = indenter.LineToBeIndented.LineNumber - 1;
                        currentLineNumber >= startLine.LineNumber + 1;
                        currentLineNumber--
                    )
                    {
                        ...
                        if (
                            !indenter
                                .Root
                                .FindToken(currentLine.Start, findInsideTrivia: true)
                                .IsKind(SyntaxKind.InterpolatedStringTextToken)
                        )
                        {
            ...
            if (
                ...
                || token.Kind()
                    is SyntaxKind.InterpolatedVerbatimStringStartToken
                        or SyntaxKind.InterpolatedStringTextToken
                || (
            ...
            var alignmentTokenIndentation = indenter
                .Finder
        ...
        private static IndentationResult GetIndentationFromCommaSeparatedList(
            Indenter indenter,
            SyntaxToken token
        ) =>
            token.Parent switch
            {
                BaseArgumentListSyntax argument => GetIndentationFromCommaSeparatedList(
                    indenter,
                    argument.Arguments,
                    token
                ),
                BaseParameterListSyntax parameter => GetIndentationFromCommaSeparatedList(
                    indenter,
                    parameter.Parameters,
                    token
                ),
                TypeArgumentListSyntax typeArgument => GetIndentationFromCommaSeparatedList(
                    indenter,
                    typeArgument.Arguments,
                    token
                ),
                TypeParameterListSyntax typeParameter => GetIndentationFromCommaSeparatedList(
                    indenter,
                    typeParameter.Parameters,
                    token
                ),
                EnumDeclarationSyntax enumDeclaration => GetIndentationFromCommaSeparatedList(
                    indenter,
            ...
            where T : SyntaxNode
        {
            ...
            for (var i = (index - 1) / 2; i >= 0; i--)
            {
                ...
                {
                    return indenter.GetIndentationOfLine(
                        sourceText.Lines.GetLineFromPosition(firstToken.SpanStart)
                    );
                }
            }
        ...
        )
        {
            ...
            return indenter.GetIndentationOfToken(
