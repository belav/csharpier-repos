        ...
        private class TypeInferrer : AbstractTypeInferrer
        {
            ...
            )
            {
                ...
                return parentTypes
                    ...
                    .OfType<INamedTypeSymbol>()
                    .Where(namedType =>
                        namedType.IsTupleType && index < namedType.TupleElements.Length
                    )
                    .Select(tupleType => new TypeInferenceInfo(
                    ...
                    var filteredMethods = instantiatedMethods
                        .Where(m =>
                            invocationTypes.Any(t =>
                                Compilation.ClassifyConversion(m.ReturnType, t).IsImplicit
                            )
                        )
                        .ToList();
            ...
            )
            {
                ...
                foreach (var parameterList in parameterizedSymbols)
                {
                    ...
                    {
                        var minParameterCount = parameterList.Count(p =>
                            !p.IsParams && !p.IsOptional
                        );
            ...
            )
            {
                ...
                return rightTypes.Select(x => new TypeInferenceInfo(
                    MakeNullable(x.InferredType, this.Compilation)
                ));
            ...
            )
            {
                ...
                return variableTypes.Select(v => new TypeInferenceInfo(
                    enumerableType.Construct(v.InferredType)
                ));
            }
            ...
            )
            {
                if (initializerExpression.IsKind(SyntaxKind.ComplexElementInitializerExpression))
                {
                    ...
                    var addMethodParameterTypes = addMethodSymbols
                        ...
                        .Select(a => new TypeInferenceInfo(
            ...
            )
            {
                ...
                var declaration = yieldStatement.FirstAncestorOrSelf<SyntaxNode>(n =>
                    n.IsReturnableConstruct()
                );
            ...
            )
            {
                ...
                var ancestor = returnStatement.FirstAncestorOrSelf<SyntaxNode>(n =>
                    n.IsReturnableConstruct()
                );
