...
namespace Microsoft.CodeAnalysis.LanguageService
{
    internal abstract partial class AbstractSemanticFactsService : ISemanticFacts
    {
        ...
        )
        {
            var container =
                ...
                ?? location
                    .AncestorsAndSelf()
                    .FirstOrDefault(a =>
                        BlockFacts.IsExecutableBlock(a)
                        || SyntaxFacts.IsParameterList(a)
                        || SyntaxFacts.IsMethodBody(a)
                    );

            var candidates = GetCollidableSymbols(
                semanticModel,
                location,
                container,
                cancellationToken
            ...
            return GenerateUniqueName(
                baseName,
                filteredCandidates.Select(s => s.Name).Concat(usedNames)
            );
        }
        ...
        /// </summary>
        protected virtual IEnumerable<ISymbol> GetCollidableSymbols(
            SemanticModel semanticModel,
            SyntaxNode location,
            ...
            semanticModel
                .LookupSymbols(location.SpanStart)
...
#nullable enable
        ...
        protected static IMethodSymbol? FindDisposeMethod(
        ...
        )
        {
            ...
            IMethodSymbol? GetDisposeMethod(string typeName, string methodName)
            {
                ...
                return disposableType
                    ...
                    .FirstOrDefault(m => m.Parameters.Length == 0 && m.Name == methodName);
            }
        }
...
#nullable disable
        ...
        public bool ExposesAnonymousFunctionParameterNames =>
            SemanticFacts.ExposesAnonymousFunctionParameterNames;
        ...
        public SymbolInfo GetCollectionInitializerSymbolInfo(
            ...
            SemanticFacts.GetCollectionInitializerSymbolInfo(
                semanticModel,
                node,
                cancellationToken
            );
        ...
        public IParameterSymbol FindParameterForAttributeArgument(
            SemanticModel semanticModel,
            SyntaxNode argumentNode,
            bool allowUncertainCandidates,
            bool allowParams,
        ...
        public ISymbol FindFieldOrPropertyForArgument(
            SemanticModel semanticModel,
            SyntaxNode argumentNode,
            CancellationToken cancellationToken
        ) =>
            SemanticFacts.FindFieldOrPropertyForArgument(
                semanticModel,
                argumentNode,
                cancellationToken
            );

        public ISymbol FindFieldOrPropertyForAttributeArgument(
            SemanticModel semanticModel,
            SyntaxNode argumentNode,
            CancellationToken cancellationToken
        ) =>
            SemanticFacts.FindFieldOrPropertyForAttributeArgument(
                semanticModel,
                argumentNode,
                cancellationToken
            );
        ...
        public ImmutableArray<IMethodSymbol> GetLocalFunctionSymbols(
            Compilation compilation,
            ISymbol symbol,
            CancellationToken cancellationToken
