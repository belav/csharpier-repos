...
namespace Microsoft.CodeAnalysis.AddImport
{
    ...
    {
        public static (TRootSyntax root, bool addBlankLine) MoveTrivia<
            TRootSyntax,
            ...
            where TImportDirectiveSyntax : SyntaxNode
        {
            ...
            else
            {
                ...
                var originalFirstUsingLineEnding =
                    originalFirstUsingTrailingTrivia.Any()
                    && syntaxFacts.IsEndOfLineTrivia(originalFirstUsingTrailingTrivia[^1])
                ...
                if (originalLastUsingCurrentIndex != newImports.Count - 1)
                {
                    ...
                    // And move it to the new using.
                    newImports[^1] = newImports[^1]
                        .WithTrailingTrivia(originalLastUsing.GetTrailingTrivia());
                    newImports[originalLastUsingCurrentIndex] =
                        originalLastUsing.WithoutTrailingTrivia();
                }
                ...
                for (var i = 0; i < newImports.Count; i++)
                {
                    ...
                    if (!trailingTrivia.Any() || !syntaxFacts.IsEndOfLineTrivia(trailingTrivia[^1]))
                    {
                        newImports[i] = newImports[i]
                            .WithAppendedTrailingTrivia(originalFirstUsingLineEnding);
                    }
                }
            }
