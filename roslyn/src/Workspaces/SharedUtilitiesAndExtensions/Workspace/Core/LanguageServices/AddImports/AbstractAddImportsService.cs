...
namespace Microsoft.CodeAnalysis.AddImports
{
        ...
        where TExternSyntax : SyntaxNode
    {
        ...
        )
        {
            ...
            var usingDirectives = filteredImports.OfType<TUsingOrAliasSyntax>()
                .Where(IsSimpleUsing)
                .ToArray();
            var staticUsingDirectives = filteredImports.OfType<TUsingOrAliasSyntax>()
                .Where(IsStaticUsing)
                .ToArray();
            var aliasDirectives = filteredImports.OfType<TUsingOrAliasSyntax>()
                .Where(IsAlias)
                .ToArray();
