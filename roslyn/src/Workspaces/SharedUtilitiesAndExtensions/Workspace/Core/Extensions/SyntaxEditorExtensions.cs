...
namespace Microsoft.CodeAnalysis.Shared.Extensions
{
    internal static class SyntaxEditorExtensions
    {
            ...
            where TNode : SyntaxNode
        {
            IEnumerable<(
                TType instance,
                (TNode semanticNode, IEnumerable<TNode> additionalNodes) nodes
            )> originalNodePairs = originalNodes.Select(n => (n, selector(n)));
            ...
            var nodesBySemanticBoundary = originalNodePairs.GroupBy(pair =>
                getSemanticBoundary(syntaxFacts, pair.nodes.semanticNode)
            );
            ...
            document = document.WithSyntaxRoot(
                originalRoot.TrackNodes(
                    originalNodePairs.SelectMany(pair =>
                        pair.nodes.additionalNodes.Concat(pair.nodes.semanticNode)
                    )
                )
            );
