    ...
    internal static class SyntaxEditorExtensions
    {
        ...
        ) where TNode : SyntaxNode
        {
            return ApplyExpressionLevelSemanticEditsAsync(
                ...
                (semanticModel, currentRoot, _, node) =>
                    updateRoot(semanticModel, currentRoot, node),
                cancellationToken
            );
        }
        ...
        {
            IEnumerable<(TType instance, (TNode semanticNode, IEnumerable<TNode> additionalNodes) nodes)> originalNodePairs =
                originalNodes.Select(n => (n, selector(n)));
            ...
            var nodesBySemanticBoundary = originalNodePairs.GroupBy(
                pair => getSemanticBoundary(syntaxFacts, pair.nodes.semanticNode)
            );
            var nodesToVerify = nodesBySemanticBoundary.Where(group => group.Skip(1).Any())
                .Flatten()
                .ToSet();
            ...
            document = document.WithSyntaxRoot(
                originalRoot.TrackNodes(
                    originalNodePairs.SelectMany(
                        pair => pair.nodes.additionalNodes.Concat(pair.nodes.semanticNode)
                    )
                )
            );
            ...
            foreach (var nodePair in originalNodePairs)
            {
                ...
                {
                    var replacementRoot = updateRoot(
                        semanticModel,
                        currentRoot,
                        instance,
                        currentNode
        ...
        )
        {
            ...
            return node.FirstAncestorOrSelf<SyntaxNode, ISyntaxFactsService>(
                (n, syntaxFacts) =>
                    syntaxFacts.IsExecutableStatement(n)
