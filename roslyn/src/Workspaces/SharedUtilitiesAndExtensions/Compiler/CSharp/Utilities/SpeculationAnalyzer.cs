...
namespace Microsoft.CodeAnalysis.CSharp.Utilities
{
          ...
          >
    {
        ...
        protected override SyntaxNode GetSemanticRootForSpeculation(ExpressionSyntax expression)
        {
            ...
            var parentNodeToSpeculate = expression.AncestorsAndSelf(ascendOutOfTrivia: false)
            ...
            Debug.Assert(
                speculativeSemanticModel != null
                    ...
                    || this.SemanticRootOfOriginalExpression.GetAncestors()
                        .Any(
                            node =>
                                node.IsKind(SyntaxKind.UnknownAccessorDeclaration)
        ...
        public bool ReplacementChangesSemanticsOfUnchangedLambda(
            ...
            var originalIdentifierNodes = originalLambdaBody.DescendantNodes()
            ...
            var replacedIdentifierNodes = replacedLambdaBody.DescendantNodes()
                .OfType<IdentifierNameSyntax>()
                .Where(node => paramNames.Contains(node.Identifier.ValueText));
        ...
        private bool HaveSameParameterType(
            ...
            var originalParamType =
                this.OriginalSemanticModel.GetDeclaredSymbol(originalParam).Type;
            var replacedParamType =
                this.SpeculativeSemanticModel.GetDeclaredSymbol(replacedParam).Type;
        ...
        protected override bool ReplacementChangesSemanticsForNodeLanguageSpecific(
            ...
            else if (currentOriginalNode is AssignmentExpressionSyntax assignment)
            {
                ...
                currentOriginalNode is SelectOrGroupClauseSyntax
                || currentOriginalNode is OrderingSyntax
            ) {
            ...
            else if (currentOriginalNode.Kind() == SyntaxKind.VariableDeclarator)
            {
                ...
                currentOriginalNode.IsKind(
                    SyntaxKind.ConditionalExpression,
                    out ConditionalExpressionSyntax originalExpression
                )
            ) {
                ...
                currentOriginalNode.IsKind(
                    ...
                    out CaseSwitchLabelSyntax originalCaseSwitchLabel
                )
            ) {
                ...
                var originalCaseType =
                    this.OriginalSemanticModel.GetTypeInfo(
                ...
                var oldSwitchStatement =
                    (SwitchStatementSyntax)originalCaseSwitchLabel.Parent.Parent;
                ...
                currentOriginalNode.IsKind(
                    ...
                    out SwitchStatementSyntax originalSwitchStatement
                )
                ...
                if (
                    previousReplacedExpression.WalkDownParentheses()
                        .IsKind(
                ...
                var originalSwitchLabels = originalSwitchStatement.Sections.SelectMany(
                        section => section.Labels
                    )
                    .ToArray();
                var newSwitchLabels = newSwitchStatement.Sections.SelectMany(
                        section => section.Labels
                    )
                    .ToArray();
                ...
                currentOriginalNode.IsKind(
                    ...
                    out SwitchExpressionSyntax originalSwitchExpression
                )
                ...
                currentOriginalNode.IsKind(
                    ...
                    out IfStatementSyntax originalIfStatement
                )
            ) {
            ...
            {
                return !TypesAreCompatible(
                    (ImplicitArrayCreationExpressionSyntax)currentOriginalNode,
                    (ImplicitArrayCreationExpressionSyntax)currentReplacedNode
                );
            }
            else if (
                currentOriginalNode
        ...
        private bool ReplacementBreaksConstructorInitializer(
            ...
            var originalSymbol =
                this.OriginalSemanticModel.GetSymbolInfo(ctorInitializer, CancellationToken).Symbol;
            var newSymbol =
        ...
        private bool ReplacementBreaksCollectionInitializerAddMethod(
            ...
            var originalSymbol =
                this.OriginalSemanticModel.GetCollectionInitializerSymbolInfo(
                    originalInitializer,
                    CancellationToken
                ).Symbol;
            var newSymbol =
                this.SpeculativeSemanticModel.GetCollectionInitializerSymbolInfo(
                    newInitializer,
                    CancellationToken
                ).Symbol;
            return !SymbolsAreCompatible(originalSymbol, newSymbol);
        ...
        private bool ReplacementBreaksIsOrAsExpression(
            ...
            var originalConvertedType =
                this.OriginalSemanticModel.GetTypeInfo(originalIsOrAsExpression.Right).Type;
            var newConvertedType =
                this.SpeculativeSemanticModel.GetTypeInfo(newIsOrAsExpression.Right).Type;
        ...
        protected override bool ReplacementIntroducesErrorType(
        ...
        private static bool ConditionalExpressionConversionsAreAllowed(
            ...
            ((CSharpParseOptions)originalExpression.SyntaxTree.Options).LanguageVersion
