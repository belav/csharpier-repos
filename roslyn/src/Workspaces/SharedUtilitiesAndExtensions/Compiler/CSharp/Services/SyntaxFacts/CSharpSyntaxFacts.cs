...
namespace Microsoft.CodeAnalysis.CSharp.LanguageService
{
    internal class CSharpSyntaxFacts : ISyntaxFacts
    {
        ...
        public bool SupportsCollectionExpressionNaturalType(ParseOptions options) => false;

        public SyntaxToken ParseToken(string text) => SyntaxFactory.ParseToken(text);
        ...
        public bool IsOperator(SyntaxToken token)
        {
            ...
            return (
                    ...
                    && (
                        ...
                        || token.Parent is PostfixUnaryExpressionSyntax
                        || token.Parent is OperatorDeclarationSyntax
                    )
                )
                || (
                    SyntaxFacts.IsBinaryExpression(kind)
                    && (
                        token.Parent
                ...
                || (
                    SyntaxFacts.IsAssignmentExpressionOperatorToken(kind)
        ...
        public bool IsAttributeName(SyntaxNode node) => SyntaxFacts.IsAttributeName(node);
        ...
        public SyntaxNode? GetParameterList(SyntaxNode node) => node.GetParameterList();
        ...
        public void GetPartsOfUsingAliasDirective(
            SyntaxNode node,
            out SyntaxToken globalKeyword,
        ...
        public bool IsStatement([NotNullWhen(true)] SyntaxNode? node) => node is StatementSyntax;
        ...
        public bool AreStatementsInSameContainer(
        ...
        )
        {
            ...
            if (
                IsGlobalStatement(firstStatement.Parent)
        ...
        public bool IsMethodBody([NotNullWhen(true)] SyntaxNode? node)
        {
            ...
            {
                return node.Parent is BaseMethodDeclarationSyntax or AccessorDeclarationSyntax;
            }
        ...
        public bool IsQueryKeyword(SyntaxToken token)
        {
            switch (token.Kind())
            {
                ...
                case SyntaxKind.IntoKeyword:
                    return token.Parent
                        is
                        (kind: SyntaxKind.JoinIntoClause or SyntaxKind.QueryContinuation);
                default:
                    return false;
            }
        }

        public bool IsPredefinedType(SyntaxToken token) => TryGetPredefinedType(token, out _);
        ...
        public bool IsPredefinedType(SyntaxNode? node, PredefinedType type) =>
            node is PredefinedTypeSyntax predefinedType
            && IsPredefinedType(predefinedType.Keyword, type);
        ...
        public bool IsPredefinedOperator(SyntaxToken token) =>
            TryGetPredefinedOperator(token, out var actualOperator)
            && actualOperator != PredefinedOperator.None;
        ...
        public string GetText(int kind) => SyntaxFacts.GetText((SyntaxKind)kind);

        public bool IsIdentifierStartCharacter(char c) => SyntaxFacts.IsIdentifierStartCharacter(c);

        public bool IsIdentifierPartCharacter(char c) => SyntaxFacts.IsIdentifierPartCharacter(c);

        public bool IsIdentifierEscapeCharacter(char c) => c == '@';
        ...
        public bool IsValidIdentifier(string identifier)
        {
            ...
            return this.IsIdentifier(token)
                ...
                && token.ToString().Length == identifier.Length;
        }
        ...
        public bool IsStartOfUnicodeEscapeSequence(char c) => c == '\\';
        ...
        public bool IsPointerMemberAccessExpression([NotNullWhen(true)] SyntaxNode? node) =>
            (node as MemberAccessExpressionSyntax)?.Kind()
        ...
        public SyntaxNode? GetExpressionOfMemberAccessExpression(
            SyntaxNode node,
            bool allowImplicitTarget
        ) => ((MemberAccessExpressionSyntax)node).Expression;

        public void GetPartsOfElementAccessExpression(
            SyntaxNode node,
        ...
        public bool IsInStaticContext(SyntaxNode node) => node.IsInStaticContext();
        ...
        public RefKind GetRefKindOfArgument(SyntaxNode node) => ((ArgumentSyntax)node).GetRefKind();
        ...
        public bool IsUnsafeContext(SyntaxNode node) => node.IsUnsafeContext();

        public SyntaxNode GetNameOfAttribute(SyntaxNode node) => ((AttributeSyntax)node).Name;
            ...
            )
            {
                ...
                else if (identifier.Parent.IsParentKind(SyntaxKind.ObjectInitializerExpression))
                {
                    ...
                    if (objectInitializer.Parent is BaseObjectCreationExpressionSyntax)
                    {
                        ...
                        objectInitializer?.Parent is AssignmentExpressionSyntax
        ...
        public SyntaxNode? GetContainingMemberDeclaration(
            SyntaxNode root,
            int position,
        ...
        private static string? GetName(SyntaxNode node, DisplayNameOptions options)
        {
            ...
            switch (node.Kind())
            {
                ...
                case SyntaxKind.QualifiedName:
                    ...
                    return GetName(qualified.Left, options)
                        + dotToken
                        + GetName(qualified.Right, options);
            }
                ...
                else
                {
                    ...
                    if (nameToken != default)
                    {
                        ...
                        if ((options & DisplayNameOptions.IncludeTypeParameters) != 0)
                        {
                            ...
                            var builder = pooled.Builder;
                            builder.Append(name);
                            AppendTypeParameterList(
                                builder,
                                memberDeclaration.GetTypeParameterList()
                            );
        ...
        public TextSpan GetInactiveRegionSpanAroundPosition(
            SyntaxTree syntaxTree,
            int position,
            CancellationToken cancellationToken
        )
        ...
        public SyntaxList<SyntaxNode> GetContentsOfInterpolatedString(
            SyntaxNode interpolatedString
        ) => ((InterpolatedStringExpressionSyntax)interpolatedString).Contents;

        public bool IsVerbatimStringLiteral(SyntaxToken token) => token.IsVerbatimStringLiteral();

        public bool IsNumericLiteral(SyntaxToken token) =>
            token.Kind() == SyntaxKind.NumericLiteralToken;

        public SeparatedSyntaxList<SyntaxNode> GetArgumentsOfObjectCreationExpression(
            SyntaxNode objectCreationExpression
        ) =>
            ((BaseObjectCreationExpressionSyntax)objectCreationExpression).ArgumentList
                is { } argumentList
                ? GetArgumentsOfArgumentList(argumentList)
                : default;

        public SeparatedSyntaxList<SyntaxNode> GetArgumentsOfArgumentList(
            SyntaxNode argumentList
        ) => ((BaseArgumentListSyntax)argumentList).Arguments;

        public SeparatedSyntaxList<SyntaxNode> GetArgumentsOfAttributeArgumentList(
            SyntaxNode argumentList
        ) => ((AttributeArgumentListSyntax)argumentList).Arguments;

        public bool IsRegularComment(SyntaxTrivia trivia) => trivia.IsRegularComment();

        public bool IsDocumentationComment(SyntaxTrivia trivia) => trivia.IsDocComment();

        public bool IsElastic(SyntaxTrivia trivia) => trivia.IsElastic();
        ...
        public bool IsAnyAssignmentStatement([NotNullWhen(true)] SyntaxNode? node) => false;
        ...
        public bool IsExpressionOfForeach([NotNullWhen(true)] SyntaxNode? node) =>
            node?.Parent is ForEachStatementSyntax foreachStatement
        ...
        public void GetPartsOfTupleExpression<TArgumentSyntax>(
            SyntaxNode node,
            ...
            where TArgumentSyntax : SyntaxNode
        {
            ...
            arguments =
                (SeparatedSyntaxList<TArgumentSyntax>)
                    (SeparatedSyntaxList<SyntaxNode>)tupleExpression.Arguments;
        ...
        public SyntaxTokenList GetModifiers(SyntaxNode? node) => node.GetModifiers();
        ...
        public void GetPartsOfCastExpression(
            SyntaxNode node,
            out SyntaxNode type,
            out SyntaxNode expression
        )
        ...
        public void GetPartsOfAnyIsTypeExpression(
            SyntaxNode node,
            out SyntaxNode expression,
            out SyntaxNode type
        )
        ...
        public void GetPartsOfIsPatternExpression(
            SyntaxNode node,
            out SyntaxNode left,
        ...
        public bool IsAnyPattern([NotNullWhen(true)] SyntaxNode? node) => node is PatternSyntax;

        public SyntaxNode GetExpressionOfConstantPattern(SyntaxNode node) =>
            ((ConstantPatternSyntax)node).Expression;

        public void GetPartsOfDeclarationPattern(
            SyntaxNode node,
            out SyntaxNode type,
        ...
        public void GetPartsOfRecursivePattern(
            SyntaxNode node,
            out SyntaxNode? type,
        ...
        public bool SupportsIsNotTypeExpression(ParseOptions options) => false;
        ...
        public void GetPartsOfParenthesizedPattern(
            SyntaxNode node,
            out SyntaxToken openParen,
        ...
        public void GetPartsOfBinaryPattern(
            SyntaxNode node,
            out SyntaxNode left,
            out SyntaxToken operatorToken,
            out SyntaxNode right
        ...
        public void GetPartsOfUnaryPattern(
            SyntaxNode node,
            out SyntaxToken operatorToken,
            out SyntaxNode pattern
        )
        ...
        public void GetPartsOfRelationalPattern(
            SyntaxNode node,
            out SyntaxToken operatorToken,
            out SyntaxNode expression
        )
        ...
        public SyntaxNode GetTypeOfTypePattern(SyntaxNode node) => ((TypePatternSyntax)node).Type;
        ...
        public bool IsNamedMemberInitializer([NotNullWhen(true)] SyntaxNode? node) =>
            node
                ...
                (SyntaxKind.SimpleAssignmentExpression) { Left: IdentifierNameSyntax };

        public bool IsElementAccessInitializer([NotNullWhen(true)] SyntaxNode? node) =>
            node
                is AssignmentExpressionSyntax
                (SyntaxKind.SimpleAssignmentExpression) { Left: ImplicitElementAccessSyntax };
        ...
        public void GetPartsOfArgumentList(
            SyntaxNode node,
        ...
        public void GetPartsOfBaseObjectCreationExpression(
            SyntaxNode node,
            out SyntaxNode? argumentList,
            out SyntaxNode? initializer
        )
        {
        ...
        public void GetPartsOfBinaryExpression(
            SyntaxNode node,
            out SyntaxNode left,
            out SyntaxToken operatorToken,
            out SyntaxNode right
        ...
        public void GetPartsOfCompilationUnit(
            SyntaxNode node,
            out SyntaxList<SyntaxNode> imports,
            out SyntaxList<SyntaxNode> attributeLists,
            out SyntaxList<SyntaxNode> members
        )
        {
        ...
        public void GetPartsOfConditionalExpression(
            SyntaxNode node,
        ...
        public void GetPartsOfGenericName(
            SyntaxNode node,
            out SyntaxToken identifier,
            out SeparatedSyntaxList<SyntaxNode> typeArguments
        )
        {
        ...
        public void GetPartsOfInterpolationExpression(
            SyntaxNode node,
        ...
        public void GetPartsOfInvocationExpression(
            SyntaxNode node,
            out SyntaxNode expression,
            out SyntaxNode? argumentList
        )
        ...
        public void GetPartsOfMemberAccessExpression(
            SyntaxNode node,
            out SyntaxNode expression,
            out SyntaxToken operatorToken,
            out SyntaxNode name
        ...
        public void GetPartsOfBaseNamespaceDeclaration(
            SyntaxNode node,
            out SyntaxNode name,
            out SyntaxList<SyntaxNode> imports,
            out SyntaxList<SyntaxNode> members
        ...
        public void GetPartsOfNamedMemberInitializer(
            SyntaxNode node,
            out SyntaxNode identifier,
            out SyntaxNode expression
        )
        {
        ...
        public void GetPartsOfObjectCreationExpression(
            SyntaxNode node,
            out SyntaxNode type,
            out SyntaxNode? argumentList,
            out SyntaxNode? initializer
        ...
        public void GetPartsOfParameter(
            SyntaxNode node,
            out SyntaxToken identifier,
            out SyntaxNode? @default
        )
        {
        ...
        public void GetPartsOfPrefixUnaryExpression(
            SyntaxNode node,
            out SyntaxToken operatorToken,
            out SyntaxNode operand
        )
        {
        ...
        public void GetPartsOfQualifiedName(
            SyntaxNode node,
            out SyntaxNode left,
            out SyntaxToken operatorToken,
            out SyntaxNode right
        ...
        public SeparatedSyntaxList<SyntaxNode> GetInitializersOfObjectMemberInitializer(
            SyntaxNode node
        ) =>
            node is InitializerExpressionSyntax(SyntaxKind.ObjectInitializerExpression) initExpr
                ? initExpr.Expressions
                : default;

        public SeparatedSyntaxList<SyntaxNode> GetExpressionsOfObjectCollectionInitializer(
            SyntaxNode node
        ) =>
            node is InitializerExpressionSyntax(SyntaxKind.CollectionInitializerExpression) initExpr
                ? initExpr.Expressions
                : default;
