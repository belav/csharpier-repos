...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal sealed partial class CSharpSemanticFacts : ISemanticFacts
    {
        ...
        )
        {
            ...
            if (
                oldNode is not BaseMethodDeclarationSyntax oldMethod
                || newNode is not BaseMethodDeclarationSyntax newMethod
                || oldMethod.Body == null
            )
            {
            ...
            return model.TryGetSpeculativeSemanticModelForMethodBody(
                oldMethod.Body.OpenBraceToken.Span.End,
                newMethod,
        ...
        )
        {
            foreach (var @namespace in namespaces)
            {
                ...
                AppendAliasNames(
                    @namespace.Members.OfType<BaseNamespaceDeclarationSyntax>(),
                    builder,
                    cancellationToken
                );
            }
        ...
        public SymbolInfo GetCollectionInitializerSymbolInfo(
            ...
            semanticModel.GetCollectionInitializerSymbolInfo(
        ...
        )
        {
            if (node is AssignmentExpressionSyntax assignment && assignment.IsDeconstruction())
            {
                ...
                FlattenDeconstructionMethods(
                    semanticModel.GetDeconstructionInfo(assignment),
                    ref builder.AsRef()
                );
        ...
        )
        {
            if (node is ForEachVariableStatementSyntax @foreach)
            {
                using var builder = TemporaryArray<IMethodSymbol>.Empty;
                FlattenDeconstructionMethods(
                    semanticModel.GetDeconstructionInfo(@foreach),
                    ref builder.AsRef()
                );
                return builder.ToImmutableAndClear();
            }
        ...
        public bool IsPartial(INamedTypeSymbol typeSymbol, CancellationToken cancellationToken)
        {
            ...
            foreach (var syntaxRef in syntaxRefs)
            {
                ...
                if (
                    node is BaseTypeDeclarationSyntax { Modifiers: { } modifiers }
        ...
        )
        {
            switch (memberDeclaration)
            {
                case FieldDeclarationSyntax field:
                    return field.Declaration.Variables.Select(v =>
                        semanticModel.GetRequiredDeclaredSymbol(v, cancellationToken)
                    );

                case EventFieldDeclarationSyntax eventField:
                    return eventField.Declaration.Variables.Select(v =>
                        semanticModel.GetRequiredDeclaredSymbol(v, cancellationToken)
                    );
        ...
        )
        {
            ...
            static ImmutableArray<ISymbol> GetCallingConventionSymbols(
                SemanticModel model,
                FunctionPointerUnmanagedCallingConventionSyntax syntax
            )
            {
            ...
            switch (node)
            {
                case OrderByClauseSyntax orderByClauseSyntax:
                        ...
                        if (index >= 0 && (index + 1) < orderByClauseSyntax.Orderings.Count)
                        {
                            ...
                            if (ordering.AscendingOrDescendingKeyword.Kind() == SyntaxKind.None)
                                return semanticModel
                                    .GetSymbolInfo(ordering, cancellationToken)
                                    .GetBestOrAllSymbols();
                        }
                    }
                    else if (
                    ...
                    )
                    {
                        ...
                        return semanticModel
                            .GetSymbolInfo(orderByClauseSyntax.Orderings[0], cancellationToken)
                            .GetBestOrAllSymbols();
                    }

                    return default;
                case QueryClauseSyntax queryClauseSyntax:
                    var queryInfo = semanticModel.GetQueryClauseInfo(
                        queryClauseSyntax,
                        cancellationToken
                    );
                    ...
                    if (hasCastInfo && hasOperationInfo)
                    {
                        ...
                        return token.IsKind(SyntaxKind.InKeyword)
                            ? queryInfo.CastInfo.GetBestOrAllSymbols()
                            : queryInfo.OperationInfo.GetBestOrAllSymbols();
                    }
                ...
                case IdentifierNameSyntax { Parent: PrimaryConstructorBaseTypeSyntax baseType }:
                    return semanticModel
                        .GetSymbolInfo(baseType, cancellationToken)
                        .GetBestOrAllSymbols();
            }
        ...
        )
        {
            ...
            foreach (var syntaxReference in symbol.DeclaringSyntaxReferences)
            {
                ...
                foreach (
                    var localFunction in node.DescendantNodes()
