...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal sealed partial class CSharpSemanticFacts : ISemanticFacts
    {
        ...
        )
        {
            ...
            AppendAliasNames(
                root.Members.OfType<NamespaceDeclarationSyntax>(),
                builder,
                cancellationToken
            );
        ...
        )
        {
            foreach (var @namespace in namespaces)
            {
                ...
                AppendAliasNames(
                    @namespace.Members.OfType<NamespaceDeclarationSyntax>(),
                    builder,
                    cancellationToken
                );
            }
        ...
        public bool IsPartial(ITypeSymbol typeSymbol, CancellationToken cancellationToken)
        {
            ...
            return syntaxRefs.Any(
                n =>
                    ((BaseTypeDeclarationSyntax)n.GetSyntax(cancellationToken)).Modifiers
            ...
            switch (memberDeclaration)
            {
                case FieldDeclarationSyntax field:
                    return field.Declaration.Variables
                        .Select(v => semanticModel.GetDeclaredSymbol(v, cancellationToken));
        ...
        public IParameterSymbol FindParameterForArgument(
            ...
            ((ArgumentSyntax)argumentNode)
                .DetermineParameter(semanticModel, allowParams: false, cancellationToken);
        ...
        private static SymbolInfo GetSymbolInfo(
            SemanticModel semanticModel,
            SyntaxNode node,
            SyntaxToken token,
            CancellationToken cancellationToken
                ...
                case OrderByClauseSyntax orderByClauseSyntax:
                    ...
                    )
                    {
                        ...
                        return semanticModel.GetSymbolInfo(
                            orderByClauseSyntax.Orderings[0],
                            cancellationToken
                        );
                    }
