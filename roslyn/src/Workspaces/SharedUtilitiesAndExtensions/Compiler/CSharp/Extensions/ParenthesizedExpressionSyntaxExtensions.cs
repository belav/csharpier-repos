...
namespace Microsoft.CodeAnalysis.CSharp.Extensions
{
    internal static class ParenthesizedExpressionSyntaxExtensions
    {
        ...
        )
        {
            ...
            if (
                expression.IsKind(SyntaxKind.GreaterThanExpression, SyntaxKind.LessThanExpression)
            ...
            if (
                (
                    nodeParent.IsKind(
                        ...
                        out EqualsValueClauseSyntax? equalsValue
                    )
                    && equalsValue.Value == node
                )
                || (
                    nodeParent.IsKind(SyntaxKind.IfStatement, out IfStatementSyntax? ifStatement)
                    && ifStatement.Condition == node
                )
                || (
                    nodeParent.IsKind(
                ...
                || (
                    nodeParent.IsKind(
                        ...
                        out YieldStatementSyntax? yieldStatement
                    )
                ...
                || (
                    nodeParent.IsKind(SyntaxKind.DoStatement, out DoStatementSyntax? doStatement)
                    && doStatement.Condition == node
                )
                || (
                    nodeParent.IsKind(SyntaxKind.ForStatement, out ForStatementSyntax? forStatement)
                ...
                || (
                    ...
                    && ((CommonForEachStatementSyntax)node.GetRequiredParent()).Expression == node
                )
                || (
                    nodeParent.IsKind(
                ...
                || (
                    nodeParent.IsKind(
                        ...
                        out CatchFilterClauseSyntax? catchFilter
                    )
            ...
            //   y((x)) -> y(x)
            if (
                nodeParent.IsKind(SyntaxKind.Argument, out ArgumentSyntax? argument)
