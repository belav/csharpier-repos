...
namespace Microsoft.CodeAnalysis.CSharp.Formatting
{
    internal sealed class SpacingFormattingRule : BaseFormattingRule
    {
        ...
        public SpacingFormattingRule() : this(new CachedOptions(null)) { }
        ...
        public override AdjustSpacesOperation? GetAdjustSpacesOperation(
            ...
            if (
                currentToken.IsOpenParenInParameterList()
                && previousKind == SyntaxKind.IdentifierToken
            ) {
            ...
            if (
                currentToken.IsOpenParenInParameterList()
                && previousKind == SyntaxKind.GreaterThanToken
                && previousParentKind == SyntaxKind.TypeParameterList
            ) {
                return AdjustSpacesOperationZeroOrOne(_options.SpacingAfterMethodDeclarationName);
            }
            ...
            if (
                previousToken.Parent.IsKind(SyntaxKind.OperatorDeclaration)
            ...
            if (
                previousToken.IsOpenParenInParameterList()
                && currentToken.IsCloseParenInParameterList()
            ) {
                return AdjustSpacesOperationZeroOrOne(
                    _options.SpaceBetweenEmptyMethodDeclarationParentheses
                );
            }
            ...
            {
                return AdjustSpacesOperationZeroOrOne(
                    _options.SpaceWithinMethodDeclarationParenthesis
                );
            }
            ...
            if (
                previousToken.IsOpenParenInArgumentListOrPositionalPattern()
                && currentToken.IsCloseParenInArgumentListOrPositionalPattern()
            ) {
                return AdjustSpacesOperationZeroOrOne(
                    _options.SpaceBetweenEmptyMethodCallParentheses
                );
            }
            ...
            if (
                currentKind == SyntaxKind.OpenParenToken
                && (
                    previousKind == SyntaxKind.IfKeyword
                    || previousKind == SyntaxKind.WhileKeyword
                    || previousKind == SyntaxKind.SwitchKeyword
                    || previousKind == SyntaxKind.ForKeyword
                    || previousKind == SyntaxKind.ForEachKeyword
                    || previousKind == SyntaxKind.CatchKeyword
                    || previousKind == SyntaxKind.UsingKeyword
                    || previousKind == SyntaxKind.WhenKeyword
                    || previousKind == SyntaxKind.LockKeyword
                    || previousKind == SyntaxKind.FixedKeyword
                )
            ) {
                return AdjustSpacesOperationZeroOrOne(
                    _options.SpaceAfterControlFlowStatementKeyword
                );
            }
            ...
            if (
                (
                    previousParentKind == SyntaxKind.CastExpression
                    && previousKind == SyntaxKind.OpenParenToken
                )
                || (
                    currentParentKind == SyntaxKind.CastExpression
                    && currentKind == SyntaxKind.CloseParenToken
                )
            ) {
            ...
            if (
                previousParentKind == SyntaxKind.ForStatement
            ...
            if (
                (
                    (
                        ...
                        && previousKind == SyntaxKind.OpenBracketToken
                    )
            ...
            if (
                previousKind == SyntaxKind.CloseBracketToken
            ...
            {
                return AdjustSpacesOperationZeroOrOne(
                    _options.SpaceAfterColonInBaseTypeDeclaration
                );
            }
            ...
            {
                return AdjustSpacesOperationZeroOrOne(
                    _options.SpaceBeforeColonInBaseTypeDeclaration
                );
            }
            ...
            if (
                (
                    previousToken.IsCommaInArgumentOrParameterList()
            ...
            if (
                (
                    currentToken.IsCommaInArgumentOrParameterList()
            ...
            if (
                previousToken.IsSemicolonInForStatement()
                && currentKind != SyntaxKind.CloseParenToken
            ) {
                return AdjustSpacesOperationZeroOrOne(_options.SpaceAfterSemicolonsInForStatement);
            }
            ...
            if (
                currentToken.Parent is BinaryExpressionSyntax
                ...
                switch (_options.SpacingAroundBinaryOperator)
                {
                    ...
                    case BinaryOperatorSpacingOptions.Remove:
                        if (
                            currentKind == SyntaxKind.IsKeyword
            ...
            if (previousParentKind == SyntaxKind.FunctionPointerType)
            {
                ...
                if (
                    previousKind == SyntaxKind.AsteriskToken
            ...
            // delegate* unmanaged[
            if (
                previousParentKind == SyntaxKind.FunctionPointerCallingConvention
                && currentParentKind == SyntaxKind.FunctionPointerUnmanagedCallingConventionList
                && currentKind == SyntaxKind.OpenBracketToken
            ) {
            ...
            // delegate*<void
            if (
                previousKind == SyntaxKind.LessThanToken
            ...
            if (
                previousKind == SyntaxKind.InterpolatedStringStartToken
            ...
            if (
                ...
                || (
                    previousKind == SyntaxKind.CloseBraceToken
            ...
            // Index expressions
            if (
                previousKind == SyntaxKind.CaretToken
                && previousParentKind == SyntaxKind.IndexExpression
            ) {
                return CreateAdjustSpacesOperation(0, AdjustSpacesOption.ForceSpaces);
            }
            ...
            if (
                previousKind == SyntaxKind.DotDotToken
            ...
            if (
                currentKind == SyntaxKind.DotDotToken
        ...
        private void SuppressVariableDeclaration(List<SuppressOperation> list, SyntaxNode node)
        {
            if (
                ...
                || node.IsKind(SyntaxKind.EventFieldDeclaration)
        ...
        private static AdjustSpacesOperation AdjustSpacesOperationZeroOrOne(
            bool option,
            AdjustSpacesOption explicitOption = AdjustSpacesOption.ForceSpacesIfOnSingleLine
        ) {
            if (option)
            {
        ...
        private static bool IsFunctionLikeKeywordExpressionKind(SyntaxKind syntaxKind) =>
            (
                syntaxKind == SyntaxKind.TypeOfExpression
                || syntaxKind == SyntaxKind.DefaultExpression
                || syntaxKind == SyntaxKind.SizeOfExpression
            );
        ...
        private static bool IsControlFlowLikeKeywordStatementKind(SyntaxKind syntaxKind)
        {
            return (
                ...
                || syntaxKind == SyntaxKind.ForEachVariableStatement
                || syntaxKind == SyntaxKind.DoStatement
                || syntaxKind == SyntaxKind.CatchDeclaration
                || syntaxKind == SyntaxKind.UsingStatement
                || syntaxKind == SyntaxKind.LockStatement
                || syntaxKind == SyntaxKind.FixedStatement
                || syntaxKind == SyntaxKind.CatchFilterClause
            );
        }
        ...
        private readonly struct CachedOptions : IEquatable<CachedOptions>
        {
            ...
            public bool Equals(CachedOptions other)
            {
                return SpacesIgnoreAroundVariableDeclaration
                    ...
                    && SpaceBeforeSemicolonsInForStatement
                        == other.SpaceBeforeSemicolonsInForStatement
                    && SpaceAfterSemicolonsInForStatement
                        == other.SpaceAfterSemicolonsInForStatement
                    ...
                    && SpaceAfterColonInBaseTypeDeclaration
                        == other.SpaceAfterColonInBaseTypeDeclaration
                    && SpaceBeforeColonInBaseTypeDeclaration
                        == other.SpaceBeforeColonInBaseTypeDeclaration
            ...
            public override int GetHashCode()
            {
                ...
                hashCode =
                    (hashCode << 1) + (SpaceBetweenEmptyMethodDeclarationParentheses ? 1 : 0);
