...
namespace Microsoft.CodeAnalysis.CSharp.Simplification.Simplifiers
{
    ...
    internal static class CastSimplifier
    {
        ...
        )
        {
            ...
            if (
                originalConversionOperation.Parent is IConversionOperation
                {
                    IsImplicit: true,
                    Conversion: { IsUserDefined: true }
                } originalParentImplicitConversion
            )
            {
                ...
                if (
                    !Equals(
                        originalParentImplicitConversion.Conversion.MethodSymbol,
            ...
            )
            {
                ...
                if (
                    ...
                    && isType != null
                    && originalSemanticModel.Compilation.ClassifyConversion(
        ...
        )
        {
            ...
            if (
                rewrittenConditionalOperation.Parent
                    is IConversionOperation { IsImplicit: true } implicitConversion
                && castType.Equals(
                ...
                if (parent.Parent is ConditionalExpressionSyntax conditionalExpression)
                {
                    if (
                        (
                            ...
                            && conditionalExpression.WhenFalse.WalkDownParentheses().Kind()
                                == SyntaxKind.NullLiteralExpression
                        )
                        || (
                            conditionalExpression.WhenFalse == parent
                            && conditionalExpression.WhenTrue.WalkDownParentheses().Kind()
                                == SyntaxKind.NullLiteralExpression
                        )
                    )
                    {
        ...
        )
        {
            var originalMemberSymbol =
                originalSemanticModel.GetSymbolInfo(
                    memberAccessExpression,
                    cancellationToken
                ).Symbol;
            ...
            var rewrittenMemberSymbol =
                rewrittenSemanticModel.GetSymbolInfo(
                    rewrittenMemberAccessExpression,
                    cancellationToken
                ).Symbol;
        ...
        )
        {
            ...
            var rewrittenConvertedType =
                rewrittenSemanticModel.GetTypeInfo(
                    rewrittenExpression,
                    cancellationToken
                ).ConvertedType;
