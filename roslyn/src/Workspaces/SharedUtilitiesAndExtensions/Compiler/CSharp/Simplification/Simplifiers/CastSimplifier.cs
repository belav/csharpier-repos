...
namespace Microsoft.CodeAnalysis.CSharp.Simplification.Simplifiers
{
    ...
    internal static class CastSimplifier
    {
        public static bool IsUnnecessaryCast(
            ...
            cast switch
            {
                CastExpressionSyntax castExpression => IsUnnecessaryCast(
                    castExpression,
                    semanticModel,
                    cancellationToken
                ),
                BinaryExpressionSyntax binaryExpression => IsUnnecessaryAsCast(
                    binaryExpression,
                    semanticModel,
                    cancellationToken
                ),
        ...
        )
        {
            ...
            var castExpressionOperation = semanticModel.GetOperation(
                cast.Expression,
                cancellationToken
            );
            if (
                castExpressionOperation is
                {
            ...
            )
            {
                ...
                // (object)MethodGroup
                if (
                    conversionOperation.Parent
                        is IConversionOperation { Type: { } parentConversionType }
                    && semanticModel
                        .ClassifyConversion(cast.Expression, parentConversionType)
                        .Exists
                )
                {
                    return true;
        ...
        )
        {
            ...
            if (
                leftOrRightChild.Parent is BinaryExpressionSyntax(
            ...
            )
            {
                ...
                {
                    if (
                        leftOrRightChild == binary.Left && IsConstantZero(binary.Right)
        ...
        )
        {
            ...
            if (
                castExpression.WalkUpParentheses().Parent
                    is PrefixUnaryExpressionSyntax(SyntaxKind.BitwiseNotExpression) parent
        ...
        )
        {
            ...
            if (
                isDefaultLiteralCast
                && castNode.WalkUpParentheses().Parent is PatternSyntax or CaseSwitchLabelSyntax
            )
                return false;
            ...
            if (originalOperation is IDelegateCreationOperation originalDelegateCreationOperation)
            {
                return IsDelegateCreationCastSafeToRemove(
                    ...
                    castedExpressionNode,
                    originalSemanticModel,
                    originalDelegateCreationOperation,
                    cancellationToken
                );
            }
        ...
        private static bool CastRemovalCouldCauseSignExtensionWarning(
            ExpressionSyntax castSyntax,
        ...
        )
        {
            ...
            if (
                castSyntax.WalkUpParentheses().GetRequiredParent().Kind()
                is SyntaxKind.BitwiseOrExpression
        ...
        private static bool IsDelegateCreationCastSafeToRemove(
            ...
            ExpressionSyntax castedExpressionNode,
            SemanticModel originalSemanticModel,
            IDelegateCreationOperation originalDelegateCreationOperation,
            CancellationToken cancellationToken
        )
        {
            ...
            var rewrittenOperation = rewrittenSemanticModel.GetOperation(
                rewrittenExpression.WalkDownParentheses(),
                cancellationToken
            );
            if (
                rewrittenOperation is not (IAnonymousFunctionOperation or IMethodReferenceOperation)
            )
                return false;

            if (
                rewrittenOperation.Parent
                is not IDelegateCreationOperation rewrittenDelegateCreationOperation
            )
                return false;
            ...
            return Equals(
                originalDelegateCreationOperation.Type,
                rewrittenDelegateCreationOperation.Type
            );
        }
        ...
        private static bool IsConversionCastSafeToRemove(
            ...
            ExpressionSyntax castedExpressionNode,
            SemanticModel originalSemanticModel,
            IConversionOperation originalConversionOperation,
            CancellationToken cancellationToken
        )
        {
            ...
            if (
                IsExplicitCastThatMustBePreserved(
                    originalSemanticModel,
                    castNode,
                    originalConversion,
                    cancellationToken
            ...
            if (
                ...
                && !originalConvertedType.IsNullable()
            )
                return false;
            ...
            if (
                rewrittenConvertedType is null
                || rewrittenConvertedType.TypeKind == TypeKind.Error
                || !rewrittenConversion.Exists
            )
                return false;

            if (
                CastRemovalWouldCauseUnintendedReferenceComparisonWarning(
                    rewrittenExpression,
                    rewrittenSemanticModel,
                    cancellationToken
                )
            ...
            if (
                originalConversion.IsConstantExpression
            ...
            if (
                originalConversion.IsInterpolatedString && !rewrittenConversion.IsInterpolatedString
            )
                return false;
            ...
            )
            {
                ...
                if (
                    !Equals(
                        originalParentConversion.Conversion.MethodSymbol,
                        rewrittenConversion.MethodSymbol
                    )
                )
                    return false;
            }
            ...
            if (
                IsIdentityFloatingPointCastThatMustBePreserved(
                    castNode,
                    castedExpressionNode,
                    originalSemanticModel,
                    cancellationToken
            ...
            if (
                IsIdentityStructCastThatMustBePreserved(
                    castNode,
                    castedExpressionNode,
                    originalSemanticModel,
                    cancellationToken
            ...
            if (
                castNode.WalkUpParentheses().Parent
            ...
            {
                if (
                    IsComplementaryInvocationAfterCastRemoval(
            ...
            if (
                castNode.WalkUpParentheses().Parent
                    is BinaryExpressionSyntax(SyntaxKind.IsExpression) isExpression
            ...
            if (
                castNode.IsParentKind(SyntaxKind.Interpolation)
            ...
            if (
                originalConversion.IsBoxing
                && rewrittenConversion.IsBoxing
                && originalConvertedType.IsReferenceType
                && rewrittenConvertedType.TypeKind == TypeKind.Dynamic
            )
            {
                return true;
            }
            ...
            //     => a & ~[|(long)|]b;
            if (
                IsRemovableWideningSignedBitwiseNegation(
                    castNode,
                    originalConversionOperation,
                    rewrittenExpression,
                    rewrittenSemanticModel,
        ...
        )
        {
            ...
            var castedSymbol = semanticModel
                .GetSymbolInfo(castedExpressionNode, cancellationToken)
                .GetAnySymbol();
            if (
                castedSymbol
                is not IFieldSymbol
                    and not ILocalSymbol
                    and not IParameterSymbol
                    and not IParameterSymbol { RefKind: RefKind.Ref }
            )
                return false;
        ...
        private static bool IsMultipleImplicitNullableConversion(
            IConversionOperation originalConversionOperation
        )
        {
            // (float?)(int?)2147483647
            ...
            if (
                originalConversionOperation.Parent
                is not IConversionOperation outerOriginalConversionOperation
            )
                return false;
        ...
        )
        {
            ...
            if (
                !IsSignedIntegralOrIntPtrType(originalConversionOperation.Type)
            ...
            if (
                rewrittenOperation
                is not IUnaryOperation { OperatorKind: UnaryOperatorKind.BitwiseNegation }
            )
                return false;
            ...
            if (
                rewrittenOperation.Parent
                is not IConversionOperation rewrittenBitwiseNotConversionOperation
            )
                return false;

            var rewrittenBitwiseNotConversion =
                rewrittenBitwiseNotConversionOperation.GetConversion();
            ...
            if (
                IsNullOrErrorType(originalConvertedType)
        ...
        )
        {
            ...
            if (
                ...
                && parent != originalConditionalExpression.WhenTrue
            )
                return false;

            if (
                originalSemanticModel.GetOperation(castExpression, cancellationToken)
                is not IConversionOperation conversionOperation
            )
                return false;
            ...
            var originalCastExpressionTypeInfo = originalSemanticModel.GetTypeInfo(
                castExpression,
                cancellationToken
            );
            var originalConditionalTypeInfo = originalSemanticModel.GetTypeInfo(
                originalConditionalExpression,
                cancellationToken
            );
            var rewrittenConditionalTypeInfo = rewrittenSemanticModel.GetTypeInfo(
                rewrittenConditionalExpression,
                cancellationToken
            );

            if (
                IsNullOrErrorType(originalCastExpressionTypeInfo)
            ...
            if (
                ...
                is not IConditionalOperation rewrittenConditionalOperation
            )
                return false;

            if (
                castType.Equals(
                    rewrittenConditionalOperation.Type,
                    SymbolEqualityComparer.IncludeNullability
                )
            )
                return true;
            ...
            if (
                rewrittenConditionalOperation.Parent
                    is IConversionOperation conditionalParentConversion
                && conditionalParentConversion.GetConversion().IsImplicit
                && castType.Equals(
                    conditionalParentConversion.Type,
                    SymbolEqualityComparer.IncludeNullability
                )
            )
            {
        ...
        )
        {
            ...
            var parentBinary =
                expression.WalkUpParentheses().GetRequiredParent() as BinaryExpressionSyntax;
            if (
                parentBinary != null
                && parentBinary.Kind()
            ...
            )
            {
                ...
                {
                    if (
                        binaryOperation.LeftOperand.Type?.SpecialType == SpecialType.System_Object
                        && !IsExplicitCast(parentBinary.Left)
                        && !IsConstantNull(binaryOperation.LeftOperand)
                        && ConvertedHasUserDefinedEquals(
                    ...
                    )
                    {
                        ...
                        binaryOperation.RightOperand.Type?.SpecialType == SpecialType.System_Object
        ...
        private static bool ConvertedHasUserDefinedEquals(
        ...
        private static bool IsExplicitCast(SyntaxNode node) =>
            ...
            && expression.WalkDownParentheses().Kind()
        ...
        )
        {
            ...
            if (conversion.IsNullable && castOrAsNode is CastExpressionSyntax castExpression)
            {
                ...
                if (parent.Parent is ConditionalExpressionSyntax conditionalExpression)
                {
                    ...
                    var (castSide, otherSide) =
                        conditionalExpression.WhenTrue == parent
                    ...
                    if (
                        castSideType.IsNullable(out var underlyingType)
                        && Equals(underlyingType, castedExpressionType)
                    )
                    {
                        var otherSideType = semanticModel
                            .GetTypeInfo(otherSide, cancellationToken)
                            .Type;
                        if (
        ...
        )
        {
            ...
            if (!IsFloatingPointType(castedExpressionType) || !IsFloatingPointType(castType))
            {
            ...
            )
            {
                ...
                castNode.Parent is InitializerExpressionSyntax(
        ...
        )
        {
            var operation = semanticModel.GetOperation(
                expression.WalkDownParentheses(),
                cancellationToken
            );
        ...
        {
            for (
                SyntaxNode? currentOld = castNode.WalkUpParentheses().Parent,
                    currentNew = rewrittenExpression.WalkUpParentheses().Parent;
            ...
            )
            {
                ...
                var oldSymbolInfo = originalSemanticModel.GetSymbolInfo(
                    currentOld,
                    cancellationToken
                );
                if (oldSymbolInfo.Symbol != null)
                {
                    ...
                    var newSymbolInfo = rewrittenSemanticModel.GetSymbolInfo(
                        currentNew,
                        cancellationToken
                    );
        ...
        )
        {
            ...
            )
            {
                ...
                if (
                    oldSymbolInfo is IMethodSymbol method
                    && method.MethodKind
                        is not (MethodKind.LocalFunction or MethodKind.LambdaMethod)
        ...
        )
        {
            ...
            )
            {
                Debug.Assert(currentOld.Kind() == currentNew.Kind());
                if (
                    currentOld is CommonForEachStatementSyntax oldForEach
        ...
        )
        {
            ...
            if (originalMemberSymbol.ContainingType.TypeKind == TypeKind.Interface)
            {
                ...
                var implementationMember =
                    rewrittenContainingType.FindImplementationForInterfaceMember(
                ...
                return Equals(implementationMember, rewrittenMemberSymbol)
        ...
        private static bool IsComplementaryDelegateInvoke(
            ISymbol originalMemberSymbol,
            ISymbol rewrittenMemberSymbol
        )
        {
            if (
                originalMemberSymbol
        ...
        )
        {
            ...
            // are passed.
            if (
                memberAccessExpression.GetRequiredParent()
                    is InvocationExpressionSyntax invocationExpression
            ...
            {
                if (
                    originalMemberSymbol is IMethodSymbol originalMethodSymbol
                ...
                )
                {
                    ...
                    for (var i = 0; i < originalParameters.Length; i++)
                    {
                        ...
                        if (
                            originalParameter.Name != rewrittenParameter.Name
                        ...
                        if (
                            originalParameter.HasExplicitDefaultValue
                            && rewrittenParameter.HasExplicitDefaultValue
                            && !Equals(
                                originalParameter.ExplicitDefaultValue,
                                rewrittenParameter.ExplicitDefaultValue
                            )
                            && argumentSyntax == null
                        )
                        {
        ...
        )
        {
            if (castNode.WalkUpParentheses().Parent is InterpolationSyntax)
            {
                ...
                var convertedType = originalConversion.IsIdentity
                    ? originalConvertedType
                    : originalSemanticModel.Compilation.ObjectType;
        ...
        private static (
            SemanticModel? rewrittenSemanticModel,
            ExpressionSyntax? rewrittenExpression
        ) GetSemanticModelWithCastRemoved(
            ExpressionSyntax castNode,
            ExpressionSyntax castedExpressionNode,
            SemanticModel originalSemanticModel,
            CancellationToken cancellationToken
        )
        {
            ...
            if (
                originalSemanticModel.Compilation.LanguageVersion() < LanguageVersion.CSharp9
