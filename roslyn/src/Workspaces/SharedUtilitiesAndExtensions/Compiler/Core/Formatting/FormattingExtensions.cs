...
namespace Microsoft.CodeAnalysis.Formatting
{
    internal static class FormattingExtensions
    {
        ...
        public static IEnumerable<AbstractFormattingRule> Concat(
            this AbstractFormattingRule rule,
        ...
        public static string CreateIndentationString(
        ...
        public static StringBuilder AppendIndentationString(
        ...
        )
        {
            ...
            return sb.Append('\t', repeatCount: numberOfTabs)
                .Append(' ', repeatCount: numberOfSpaces);
        }
        ...
        private static int GetNewIndentationForComments(
            this string line,
            int nonWhitespaceCharIndex,
            bool forceIndentation,
            int indentation,
            int indentationDelta,
            int tabSize
        ...
        )
        {
            ...
            {
                var tokens = node.GetAnnotatedTrivia(SyntaxAnnotation.ElasticAnnotation)
                    .Select(tr => tr.Token)
                    .Distinct();
            ...
            static IEnumerable<TextSpan> EnumerateAnnotatedSpans(
                SyntaxNode node,
                SyntaxAnnotation annotation
            )
            {
                foreach (var nodeOrToken in node.GetAnnotatedNodesAndTokens(annotation))
                {
                    var firstToken = nodeOrToken.IsNode
                        ? nodeOrToken.AsNode()!.GetFirstToken(includeZeroWidth: true)
                        : nodeOrToken.AsToken();
                    var lastToken = nodeOrToken.IsNode
                        ? nodeOrToken.AsNode()!.GetLastToken(includeZeroWidth: true)
                        : nodeOrToken.AsToken();
        ...
        internal static TextSpan GetElasticSpan(SyntaxToken token) => GetSpan(token, token);
        ...
        )
        {
            ...
            if (
                ...
                || headerFacts.IsOnWhileStatementHeader(
                    root,
                    operation.BaseToken.SpanStart,
