...
namespace Microsoft.CodeAnalysis.Formatting
{
    ...
    internal abstract partial class AbstractFormatEngine
    {
        ...
        > s_tokenPairListPool = new(() => new(), trimOnFree: false);
        ...
        public AbstractFormattingResult Format(CancellationToken cancellationToken)
        {
            using (Logger.LogBlock(FunctionId.Formatting_Format, FormatSummary, cancellationToken))
            {
                ...
                ApplyTokenOperations(
                    context,
                    nodeOperations,
                    tokenOperations.Object,
                    cancellationToken
                );
        ...
        protected virtual NodeOperations CreateNodeOperations(CancellationToken cancellationToken)
        {
            ...
            foreach (var node in _commonRoot.DescendantNodesAndSelf(this.SpanToFormat))
            {
                ...
                AddOperations(
                    nodeOperations.SuppressOperation,
                    suppressOperation,
                    node,
                    addSuppressOperation
                );
                AddOperations(
                    nodeOperations.AlignmentOperation,
                    alignmentOperation,
                    node,
                    addAlignTokensOperations
                );
                AddOperations(
                    nodeOperations.AnchorIndentationOperations,
                    anchorIndentationOperations,
                    node,
            ...
            alignmentOperation.Sort(
                static (o1, o2) => o1.BaseToken.Span.CompareTo(o2.BaseToken.Span)
            );
