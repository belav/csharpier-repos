...
namespace Microsoft.CodeAnalysis
{
    internal partial struct SymbolKey
    {
        private sealed class ReducedExtensionMethodSymbolKey : AbstractSymbolKey<IMethodSymbol>
        {
            ...
            protected sealed override SymbolKeyResolution Resolve(
                ...
                IMethodSymbol? contextualMethod,
                out string? failureReason
            )
            {
                var reducedFromResolution = reader.ReadSymbolKey(
                    contextualMethod?.ReducedFrom,
                    out var reducedFromFailureReason
                );
        ...
        private sealed class ConstructedMethodSymbolKey : AbstractSymbolKey<IMethodSymbol>
        {
            ...
            )
            {
                var constructedFrom = reader.ReadSymbolKey(
                    contextualMethod?.ConstructedFrom,
                ...
                using var typeArguments = reader.ReadSymbolKeyArray<IMethodSymbol, ITypeSymbol>(
                    ...
                    getContextualSymbol: static (contextualMethod, i) =>
                        SafeGet(contextualMethod.TypeArguments, i),
                ...
                {
                    failureReason =
                        $"({nameof(ConstructedMethodSymbolKey)} {nameof(typeArguments)} failed -> 'constructedFrom.SymbolCount == 0')";
                    return default;
                }
                ...
                {
                    failureReason =
                        $"({nameof(ConstructedMethodSymbolKey)} {nameof(typeArguments)} failed -> 'typeArguments.IsDefault')";
                    return default;
                }
    ...
    internal partial struct SymbolKey
    {
        private sealed class MethodSymbolKey : AbstractSymbolKey<IMethodSymbol>
        {
            ...
            {
                var returnType = (ITypeSymbol?)
                    reader.ReadSymbolKey(contextualSymbol: method.ReturnType, out _).GetAnySymbol();
                ...
                if (
                    reader.ParameterTypesMatch(
                        ...
                        getContextualType: static (method, i) =>
                            SafeGet(method.Parameters, i)?.Type,
