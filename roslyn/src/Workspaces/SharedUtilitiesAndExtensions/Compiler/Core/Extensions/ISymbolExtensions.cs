...
namespace Microsoft.CodeAnalysis.Shared.Extensions
{
    internal static partial class ISymbolExtensions
    {
        ...
        public static ImmutableArray<ISymbol> ExplicitInterfaceImplementations(
            this ISymbol symbol
        ) =>
            symbol switch
            {
        ...
        )
        {
            if (
                symbol.Kind
                is not SymbolKind.Method
                    and not SymbolKind.Property
                    and not SymbolKind.Event
            )
        ...
        public static ImmutableArray<ISymbol> ImplicitInterfaceImplementations(
            this ISymbol symbol
        ) =>
            symbol
                .ExplicitOrImplicitInterfaceImplementations()
                .Except(symbol.ExplicitInterfaceImplementations())
                .ToImmutableArray();
        ...
        public static ImmutableArray<ITypeParameterSymbol> GetTypeParameters(
            this ISymbol? symbol
        ) =>
            symbol switch
            {
        ...
        )
        {
            ...
            var getAwaiters = potentialGetAwaiters
        ...
        public static bool IsValidGetAwaiter(this IMethodSymbol symbol) =>
            symbol.Name == WellKnownMemberNames.GetAwaiter && VerifyGetAwaiter(symbol);
        ...
        private static bool VerifyGetAwaiter(IMethodSymbol getAwaiter)
        {
            ...
            if (
                !returnType
                    ...
                    .OfType<IPropertySymbol>()
                    .Any(p =>
                        p.Name == WellKnownMemberNames.IsCompleted
            ...
            if (
                !methods.Any(x =>
                    ...
                    && x.ReturnsVoid
                    && x.Parameters is [{ Type.TypeKind: TypeKind.Delegate }]
                )
            )
                return false;
        ...
        private static bool VerifyGetEnumerator(IMethodSymbol getEnumerator)
        {
            ...
            if (
                !members
                    .OfType<IPropertySymbol>()
                    .Any(p =>
                        p.Name == WellKnownMemberNames.CurrentPropertyName && p.GetMethod != null
                    )
            )
            {
                return false;
        ...
        public static bool IsKind<TSymbol>(
            this ISymbol symbol,
            SymbolKind kind,
        ...
        public static bool IsSymbolWithSpecialDiscardName(this ISymbol symbol) =>
            ...
            && (
                ...
                || uint.TryParse(symbol.Name[1..], out _)
                || symbol.Name.All(n => n.Equals('_'))
            );
