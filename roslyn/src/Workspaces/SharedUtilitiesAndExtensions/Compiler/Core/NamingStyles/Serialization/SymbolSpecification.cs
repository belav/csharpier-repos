...
namespace Microsoft.CodeAnalysis.Diagnostics.Analyzers.NamingStyles
{
    ...
    internal sealed class SymbolSpecification(
        ...
        ImmutableArray<SymbolSpecification.ModifierKind> modifiers = default
    ) : IEquatable<SymbolSpecification>
    {
        ...
        )
        {
            ...
            if (
                ...
                && !ApplicableAccessibilityList.Any(
                    static (k, accessibility) => k == accessibility,
                    accessibility
                )
            )
            {
        ...
        public readonly record struct SymbolKindOrTypeKind : ISymbolMatcher
        {
            ...
            {
                // We have two members in TypeKind that point to the same value, Struct and Structure. Because of
                // this,
                // Enum.ToString(), which under the covers uses a binary search, isn't stable which one it will pick
                // and it can
            ...
            )
            {
                var symbolKind = (SymbolKind)
                    Enum.Parse(typeof(SymbolKind), symbolKindElement.Value);
