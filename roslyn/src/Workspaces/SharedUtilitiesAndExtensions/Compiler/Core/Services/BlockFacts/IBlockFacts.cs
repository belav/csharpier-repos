...
{
    /// <summary>
    /// Provides helpers for working across "blocks" of statements in an agnostic fashion across VB and
    // C#.  Both
    /// languages have quirks here that this API attempts to smooth out.  For example, many things in VB
    // are 'blocks'
    /// (like ClassBlocks and MethodBlocks).  However, only a subset of those can have executable
    // statements. Similarly,
    /// C# has actual BlockSyntax nodes (<c>{ ... }</c>), but it can also have sequences of executable
    // statements not
    ...
    internal interface IBlockFacts
    {
        ...
        /// <summary>
        /// A node that can host a list of statements or a single statement. In addition to every
        // "executable block",
