...
namespace Microsoft.CodeAnalysis.EmbeddedLanguages.VirtualChars
{
    internal abstract class AbstractVirtualCharService : IVirtualCharService
    {
            ...
            if (!result.IsDefault)
            {
                ...
                if (result.Length > 0)
                {
                    ...
                    else
                    {
                        Debug.Assert(
                            currentVC.Span.Start == token.SpanStart,
                            "First span should start on the first char of the string."
                        );
                    }
                    ...
                    {
                        Debug.Assert(
                            lastVC.Span.End == token.Span.End - 1,
                    ...
                    {
                        Debug.Assert(
                            lastVC.Span.End == token.Span.End,
                            "Last span has to end right before the end of the string token."
                        );
                    }
                }
        ...
        protected static VirtualCharSequence TryConvertSimpleDoubleQuoteString(
            ...
            for (var index = startIndexInclusive; index < endIndexExclusive;)
            {
                ...
                else
                {
                    ...
                    result.Add(
                        VirtualChar.Create(tokenText[index], new TextSpan(offset + index, 1))
                    );
                    index += 1;
                }
            }
        ...
        protected static bool IsOpenOrCloseBrace(char ch) => ch == '{' || ch == '}';
