...
namespace Microsoft.CodeAnalysis.CSharp.Editing
{
    ...
    internal class CSharpImportAdder : ImportAdderService
    {
        ...
        )
        {
            ...
            {
                // use the symbols containing namespace, and not the potentially less than fully qualified namespace
                // in the full name expression.
        ...
        /// <summary>
        /// Walks the portion of the tree we're adding imports to looking to see if those imports could
        // likely cause
        /// conflicts with existing code.  Note: this is a best-effort basis, and the goal is to catch
        // reasonable
        /// conflicts effectively.  There may be cases that do slip through that we can adjust for in the
        // future.  Those
        /// cases should be assessed to see how reasonable/likely they are.  I.e. if it's just a
        // hypothetical case with
        /// no users being hit, then that's far less important than if we have a reasonable coding pattern
        // that would be
        ...
        private class ConflictWalker : CSharpSyntaxWalker
        {
            ...
            /// <summary>
            /// A mapping containing the simple names and arity of all imported types, mapped to the import that
            // they're
            /// brought in by.
            /// </summary>
            ...
            /// <summary>
            /// A mapping containing the simple names of all imported extension methods, mapped to the import
            // that
            /// they're brought in by.  This doesn't keep track of arity because methods can be called with type
            /// arguments.
            /// </summary>
            /// <remarks>
            /// We could consider adding more information here (for example the min/max number of args that this
            // can be
            /// called with).  That could then be used to check if there could be a conflict. However, that's
            // likely
            ...
            /// <summary>
            /// Track if we're in an anonymous method or not.  If so, because of how the language binds lambdas
            // and
            /// overloads, we'll assume any method access we see inside (instance or otherwise) could end up
            // conflicting
            ...
            public override void VisitSimpleLambdaExpression(SimpleLambdaExpressionSyntax node)
            {
                ...
                //
                // This is legal code where the lambda body is calling the instance method.  However, if we
                // introduce a
                // using that brings in an extension method 'M' on 'int', then the above will become ambiguous.
                // This is
                ...
                //
                // To deal with that, we keep track of if we're in a lambda, and we conservatively assume that a
                // method
            ...
            {
                // Check to see if we have an standalone identifier (or identifier on the left of a dot). If so, if
                // that
                // identifier binds to a type, then we don't want to bring in any imports that would bring in the
                // same
            ...
            public override void VisitMemberAccessExpression(MemberAccessExpressionSyntax node)
            {
                ...
                // Check to see if we have a reference to an extension method.  If so, then pulling in an import
                // could
