    ...
    internal class CSharpRenameConflictLanguageService : AbstractRenameRewriterLanguageService
    {
        ...
        private class RenameRewriter : CSharpSyntaxRewriter
        {
                ...
                : base(visitIntoStructuredTrivia: true)
            {
                ...
                _simplificationService = parameters.Document
                    .Project
                    .Services
                    .GetRequiredService<ISimplificationService>();
                _semanticFactsService = parameters.Document
                    .Project
                    .Services
                ...
                while (expressionOfInvocation != null)
                {
                    switch (expressionOfInvocation.Kind())
                    {
                        ...
                        case SyntaxKind.QualifiedName:
                            identifierToken = ((QualifiedNameSyntax)expressionOfInvocation).Right
                                .Identifier;
                            break;

                        case SyntaxKind.AliasQualifiedName:
                            identifierToken = (
                                (AliasQualifiedNameSyntax)expressionOfInvocation
                            ).Name
                                .Identifier;
                            break;
        ...
        {
            if (
                token.Parent is ExpressionSyntax(SyntaxKind.IdentifierName) expression
        ...
        )
        {
            try
            {
                ...
                )
                {
                    ...
                    {
                        var matchingParameterSymbol = methodSymbol.PartialImplementationPart
                ...
                {
                    var conflictingLocations = renamedSymbol.ContainingType
                        .TypeParameters
        ...
        )
        {
            foreach (var symbol in symbols)
            {
                foreach (var location in symbol.Locations)
                {
                    ...
                    if (
                        location.IsInSource
                        && reverseMappedLocations.TryGetValue(location, out var conflictingLocation)
                    )
                    {
