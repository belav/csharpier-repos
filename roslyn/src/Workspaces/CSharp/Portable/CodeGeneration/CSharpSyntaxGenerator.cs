...
namespace Microsoft.CodeAnalysis.CSharp.CodeGeneration
{
    ...
    internal class CSharpSyntaxGenerator : SyntaxGenerator
    {
        ...
        private static AccessorListSyntax CreateAccessorList(
            AccessorListSyntax accessorListOpt,
            IEnumerable<SyntaxNode> accessorDeclarations
        )
        {
        ...
        {
            if (
                declaration.IsKind(SyntaxKind.MethodDeclaration, out MethodDeclarationSyntax method)
            )
            {
                        ...
                        case SyntaxKind.FieldDeclaration:
                            ...
                            return this.AsInterfaceMember(
                                this.PropertyDeclaration(
                                    this.GetName(f),
                                    this.ClearTrivia(this.GetType(f)),
                                    acc,
                                    modifiers,
                                    getAccessorStatements: null,
                                    setAccessorStatements: null
        ...
        public override SyntaxNode InsertNamespaceImports(
            ...
            PreserveTrivia(
                declaration,
                d => this.InsertNamespaceImportsInternal(d, index, imports)
            );
        ...
        )
        {
            ...
            else
            {
                return declaration switch
                {
                    ...
                    CompilationUnitSyntax compilationUnit
                      => compilationUnit.WithMembers(compilationUnit.Members.AddRange(newMembers)),
                    _ => declaration,
                };
            }
        }
        ...
        private SyntaxNode AsMemberOf(SyntaxNode declaration, SyntaxNode member) =>
            declaration switch
            {
                ...
                TypeDeclarationSyntax typeDeclaration
                  => this.AsClassMember(member, typeDeclaration.Identifier.Text),
        ...
        private static readonly DeclarationModifiers s_localFunctionModifiers =
            DeclarationModifiers.Async | DeclarationModifiers.Static | DeclarationModifiers.Extern;
            ...
            declaration switch
            {
                MethodDeclarationSyntax method
                  => method.WithConstraintClauses(
                      WithTypeConstraints(method.ConstraintClauses, typeParameterName, kinds, types)
                  ),
                TypeDeclarationSyntax type
                ...
                case SyntaxKind.FieldDeclaration:
                    ...
                    if (fd.Declaration.Variables.Count == 1)
                    {
                        return ReplaceWithTrivia(
                            ...
                            fd.Declaration.Variables[0].WithInitializer(eq)
                        );
                    }
                ...
                case SyntaxKind.LocalDeclarationStatement:
                    ...
                    if (ld.Declaration.Variables.Count == 1)
                    {
                        return ReplaceWithTrivia(
                            ...
                            ld.Declaration.Variables[0].WithInitializer(eq)
                        );
                    }
                ...
                case SyntaxKind.VariableDeclaration:
                    ...
                    if (vd.Variables.Count == 1)
                    {
                        return ReplaceWithTrivia(
                            declaration,
                            vd.Variables[0],
                            vd.Variables[0].WithInitializer(eq)
                        );
                    }
        ...
        )
        {
            return SyntaxFactory.AccessorList(
                SyntaxFactory.List(
                    nodes.Select(n => AsAccessor(n, parentKind)).Where(n => n != null)
                )
            );
        }
            ...
            if (root.Span.Contains(declaration.Span))
            {
                ...
                if (fullDecl != declaration && fullDecl.IsKind(newFullDecl.Kind()))
                {
                    ...
                    return ReplaceRange(
                        root,
                        fullDecl,
                        this.SplitAndReplace(fullDecl, index, new[] { newDeclaration })
                    );
                }
        ...
        )
        {
            ...
            if (root.Span.Contains(node.Span))
            {
                ...
                return this.Isolate(
                    ...
                    r => this.RemoveNodeInternal(r, r.GetCurrentNode(node), options)
                );
            }
            else
            {
