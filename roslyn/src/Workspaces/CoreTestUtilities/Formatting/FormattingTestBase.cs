...
#pragma warning restore IDE0060 // Remove unused parameter
        ...
        )
        {
            using (var workspace = new AdhocWorkspace())
            {
                ...
                var formattingOptions =
                    changedOptionSet != null
                        ? formattingService.GetFormattingOptions(
                            changedOptionSet.ToAnalyzerConfigOptions(document.Project.Services),
        ...
        private static async Task AssertFormatAsync(
            SolutionServices services,
            string expected,
            SyntaxNode root,
        ...
        )
        {
            ...
            if (TryAdjustSpans(sourceText, result, resultText, spans, out var adjustedSpans))
            {
                ...
                var idempotentResult = Formatter.GetFormattedTextChanges(
                    resultRoot,
                    adjustedSpans,
                    services,
                    options
        ...
        private static bool TryAdjustSpans(
            SourceText inputText,
            IList<TextChange> changes,
            SourceText outputText,
            ImmutableArray<TextSpan> inputSpans,
            out ImmutableArray<TextSpan> outputSpans
        )
        {
            if (changes.Count == 0)
            {
