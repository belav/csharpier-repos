...
namespace Microsoft.CodeAnalysis.Remote.Testing
{
          ...
          IRemoteHostServiceCallback
    {
        ...
        public RemoteWorkspace GetRemoteWorkspace() => TestData.WorkspaceManager.GetWorkspace();

        /// <summary>
        /// Remote API.
        /// </summary>
        public Task GetAssetsAsync(
            int scopeId,
            Checksum[] checksums,
            string pipeName,
            ...
            RemoteEndPoint.WriteDataToNamedPipeAsync(
                ...
                (writer, data, cancellationToken) =>
                    RemoteHostAssetSerialization.WriteDataAsync(
                        ...
                        _workspaceServices.GetRequiredService<ISerializerService>(),
                        data.scopeId,
                        data.checksums,
                        cancellationToken
                    ),
        ...
        public void RegisterService(
            ...
            Func<Stream, IServiceProvider, ServiceActivationOptions, ServiceBase> serviceCreator
        ...
        public override async Task<RemoteServiceConnection> CreateConnectionAsync(
            RemoteServiceName serviceName,
        ...
        )
        {
            ...
            var serviceStream = await _inprocServices
                .RequestServiceAsync(serviceName)
                .ConfigureAwait(false);
        ...
        private sealed class InProcRemoteServices : IDisposable
        {
            ...
            private readonly Dictionary<
                ServiceMoniker,
                BrokeredServiceBase.IFactory
            > _remoteBrokeredServicesMap = new();
            private readonly Dictionary<
                RemoteServiceName,
                Func<Stream, IServiceProvider, ServiceActivationOptions, ServiceBase>
            > _factoryMap = new();
...
#pragma warning disable CS0618 // Type or member is obsolete
                RegisterService(
                    ...
                    (s, p, o) => new RemoteHostService(s, p)
                );
#pragma warning restore
                ...
                RegisterRemoteBrokeredService(
                    new RemoteAsynchronousOperationListenerService.Factory()
                );
                RegisterRemoteBrokeredService(new RemoteSymbolSearchUpdateService.Factory());
                RegisterRemoteBrokeredService(
                    new RemoteDesignerAttributeDiscoveryService.Factory()
                );
                ...
                RegisterRemoteBrokeredService(
                    new RemoteSemanticClassificationCacheService.Factory()
                );
                ...
                RegisterRemoteBrokeredService(
                    new RemoteConvertTupleToStructCodeRefactoringService.Factory()
                );
                ...
                RegisterRemoteBrokeredService(
                    new RemoteExtensionMethodImportCompletionService.Factory()
                );
                RegisterRemoteBrokeredService(new RemoteDependentTypeFinderService.Factory());
                RegisterRemoteBrokeredService(
                    new RemoteGlobalNotificationDeliveryService.Factory()
                );
            ...
            public Task<Stream> RequestServiceAsync(RemoteServiceName serviceName)
            {
                ...
                return Task.FromResult<Stream>(
                    new WrappedStream(
                        factory(streams.Item1, ServiceProvider, default),
                        streams.Item2
                    )
                );
            }
            ...
            )
            {
                ...
                )
                {
                    ...
                    var serviceConnection = descriptor
                        .WithTraceSource(ServiceProvider.TraceSource)
                ...
                if (
                    _remoteBrokeredServicesMap.TryGetValue(
                        descriptor.Moniker,
