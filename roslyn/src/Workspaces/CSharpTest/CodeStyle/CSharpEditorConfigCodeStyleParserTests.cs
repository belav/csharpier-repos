...
namespace Microsoft.CodeAnalysis.CSharp.UnitTests.CodeStyle
{
    public class CSharpEditorConfigCodeStyleParserTests
    {
        ...
        [InlineData(" none ", BinaryOperatorSpacingOptions.Remove)]
        [InlineData(" before_and_after ", BinaryOperatorSpacingOptions.Single)]
        public void TestParseSpacingAroundBinaryOperator(
            string rawValue,
            BinaryOperatorSpacingOptions parsedValue
        ) =>
            Assert.Equal(
                parsedValue,
        ...
        public void TestParseLabelPositioning(string rawValue, LabelPositionOptions parsedValue) =>
            Assert.Equal(
                parsedValue,
                CSharpFormattingOptions2.ParseEditorConfigLabelPositioning(rawValue)
            );
        ...
        [InlineData(
            "true:warning",
            (int)ExpressionBodyPreference.WhenPossible,
            ReportDiagnostic.Warn
        )]
        [InlineData(
            "when_on_single_line:error",
            (int)ExpressionBodyPreference.WhenOnSingleLine,
        ...
        public void TestParseExpressionBodyPreference(
            string optionString,
            int parsedValue,
        ...
        )
        {
            ...
            var codeStyleOption = CSharpCodeStyleOptions.ParseExpressionBodyPreference(
                optionString,
                defaultValue
            );
        ...
        internal void TestParseUsingDirectivesPlacement(
            string optionString,
        ...
        )
        {
            ...
            severity ??= ReportDiagnostic.Error;
            var codeStyleOption = CSharpCodeStyleOptions.ParseUsingDirectivesPlacement(
                optionString,
                defaultValue
            );
