...
";
            ...
            var compilation1 = GetCompilation(
                source,
                LanguageNames.CSharp,
                references: [.. netstandardReferences]
            );
            ...
            TestRoundTrip(
                GetDeclaredSymbols(compilation1),
                compilation1,
                useSymbolEquivalence: false
            );
            TestRoundTrip(
                GetDeclaredSymbols(compilation1),
                compilation2,
                useSymbolEquivalence: true
            );
            TestRoundTrip(
                GetDeclaredSymbols(compilation2),
                compilation1,
                useSymbolEquivalence: true
            );
            TestRoundTrip(
...
public class C
{
    public Goo<X> G() { }
        ...
        public void TestCrossLanguageEquality1()
        {
            ...
            var symbolKey1 = SymbolKey.Create(
                compilation1.GetSpecialType(SpecialType.System_Int32)
            );
            var symbolKey2 = SymbolKey.Create(
            ...
            Assert.True(
                SymbolKey.GetComparer(ignoreAssemblyKeys: true).Equals(symbolKey1, symbolKey2)
            );
            Assert.True(
                SymbolKey
                    .GetComparer(ignoreCase: true, ignoreAssemblyKeys: true)
                    .Equals(symbolKey1, symbolKey2)
            );
        }
...
";
            ...
            var symbols = methods
                .SelectMany(ms => GetInteriorSymbols(ms, compilation))
                .Where(s => SymbolKey.IsBodyLevelSymbol(s))
                .ToList();
            ...
            var newTree = syntaxTree.WithChangedText(
                text.WithChanges(
            ...
            foreach (var symbol in symbols)
            {
                ...
                Assert.True(
                    resolved.Symbol.Locations[0].SourceSpan.Start
                        > symbol.Locations[0].SourceSpan.Start
                );
            }
        }
        ...
        private static void TestRoundTrip(
            ...
            bool useSymbolEquivalence = false
        )
        {
            foreach (var symbol in symbols)
        ...
        private static void GetDeclaredSymbols(
