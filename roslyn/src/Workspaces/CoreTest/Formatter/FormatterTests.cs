...
public class FormatterTests
{
    ...
    internal class TestFormattingService : IFormattingService
    {
        ...
        public Task<Document> FormatAsync(
            ...
            Task.FromResult(
                document.WithText(
                    SourceText.From(
                        $"Formatted with options: {lineFormattingOptions.ToString().Replace("\r", "\\r").Replace("\n", "\\n")}"
                    )
                )
            );
    }
    ...
    {
        var hostServices = s_composition
            .AddParts([typeof(NoCompilationLanguageService), typeof(TestFormattingService)])
            .GetHostServices();
...
#pragma warning restore RS0030 // Do not used banned APIs
    ...
    [Theory]
    [CombinatorialData]
    public async Task FormatAsync_ForeignLanguageWithFormattingSupport_Options(
        bool passExplicitOptions
    )
    {
        var hostServices = s_composition
            .AddParts([typeof(NoCompilationLanguageService), typeof(TestFormattingService)])
        ...
        var solutionOptions = workspace
            .CurrentSolution.Options.WithChangedOption(
                new OptionKey(FormattingOptions.UseTabs, NoCompilationConstants.LanguageName),
                false
            )
            .WithChangedOption(
                new OptionKey(FormattingOptions.TabSize, NoCompilationConstants.LanguageName),
                1
            )
            .WithChangedOption(
                new OptionKey(
        ...
        document = document
            .Project.Solution.WithOptions(solutionOptions)
...
#pragma warning restore
        ...
        if (passExplicitOptions)
        {
            ...
            AssertEx.Equal(
                @"Formatted with options: LineFormattingOptions { UseTabs = True, TabSize = 5, IndentationSize = 6, NewLine = \r }",
                formattedText.ToString()
            );
        }
        else
    ...
    public async Task PublicOptions()
    {
        ...
        var updatedOptions = OptionsTestHelpers.GetOptionSetWithChangedOptions(
            TestOptionSet.Empty,
        ...
        ValidateCSharpOptions(
            (CSharpSyntaxFormattingOptions)
                (
                    await Formatter.GetFormattingOptionsAsync(
                        csDocument,
                        updatedOptions,
                        CancellationToken.None
                    )
        ...
        var solutionWithUpdatedOptions = workspace.CurrentSolution.WithOptions(
            updatedSolutionOptions
        );
        ...
        static void ValidateCSharpOptions(CSharpSyntaxFormattingOptions formattingOptions)
        {
            ...
            Assert.False(
                formattingOptions.Spacing.HasFlag(SpacePlacement.AfterControlFlowStatementKeyword)
            );
            Assert.False(
                formattingOptions.Spacing.HasFlag(SpacePlacement.AfterSemicolonsInForStatement)
            );
            Assert.False(
                formattingOptions.Spacing.HasFlag(SpacePlacement.AfterColonInBaseTypeDeclaration)
            );
            Assert.False(
                formattingOptions.Spacing.HasFlag(SpacePlacement.BeforeColonInBaseTypeDeclaration)
            );
            Assert.False(formattingOptions.Spacing.HasFlag(SpacePlacement.AfterComma));
            Assert.True(
                formattingOptions.Spacing.HasFlag(SpacePlacement.IgnoreAroundVariableDeclaration)
            );
            Assert.True(
                formattingOptions.Spacing.HasFlag(SpacePlacement.AfterMethodDeclarationName)
            );
            Assert.True(
                formattingOptions.Spacing.HasFlag(
                    SpacePlacement.BetweenEmptyMethodDeclarationParentheses
                )
            );
            Assert.True(
                formattingOptions.Spacing.HasFlag(SpacePlacement.WithinMethodDeclarationParenthesis)
            );
            Assert.True(formattingOptions.Spacing.HasFlag(SpacePlacement.AfterMethodCallName));
            Assert.True(
                formattingOptions.Spacing.HasFlag(SpacePlacement.BetweenEmptyMethodCallParentheses)
            );
            Assert.True(
                formattingOptions.Spacing.HasFlag(SpacePlacement.WithinMethodCallParentheses)
            );
            Assert.True(
                formattingOptions.Spacing.HasFlag(SpacePlacement.WithinExpressionParentheses)
            );
            Assert.True(formattingOptions.Spacing.HasFlag(SpacePlacement.WithinCastParentheses));
            Assert.True(
                formattingOptions.Spacing.HasFlag(SpacePlacement.BeforeSemicolonsInForStatement)
            );
            ...
            Assert.True(
                formattingOptions.Spacing.HasFlag(SpacePlacement.BetweenEmptySquareBrackets)
            );
            ...
            Assert.False(
                formattingOptions.NewLines.HasFlag(
                    NewLinePlacement.BeforeMembersInObjectInitializers
                )
            );
            Assert.False(
                formattingOptions.NewLines.HasFlag(NewLinePlacement.BeforeMembersInAnonymousTypes)
            );
            ...
            Assert.False(
                formattingOptions.NewLines.HasFlag(NewLinePlacement.BeforeOpenBraceInTypes)
            );
            Assert.False(
            ...
            Assert.False(
                formattingOptions.NewLines.HasFlag(NewLinePlacement.BeforeOpenBraceInProperties)
            );
            Assert.False(
                formattingOptions.NewLines.HasFlag(NewLinePlacement.BeforeOpenBraceInMethods)
            );
            Assert.False(
            ...
            Assert.False(
                formattingOptions.NewLines.HasFlag(NewLinePlacement.BeforeOpenBraceInControlBlocks)
            );
            Assert.False(
                formattingOptions.NewLines.HasFlag(NewLinePlacement.BetweenQueryExpressionClauses)
            );
            ...
            Assert.False(
                formattingOptions.Indentation.HasFlag(
