...
namespace Microsoft.CodeAnalysis.UnitTests
{
    public partial class DocumentationCommentIdTests : TestBase
    {
        private static CSharpCompilation CreateCSharpCompilation(string sourceText)
        {
            ...
            return CSharpCompilation.Create("goo.exe")
                .AddReferences(TestMetadata.Net451.mscorlib)
                .AddSyntaxTrees(syntaxTree);
        }

        private static void CheckDeclarationId(
            string expectedId,
        ...
        private static TSymbol CheckDeclarationId<TSymbol>(
            string expectedId,
            Compilation compilation,
        ...
        private static void CheckDeclarationIdExact<TSymbol>(
            string expectedId,
            Compilation compilation,
            Func<TSymbol, bool> test
        ) where TSymbol : ISymbol
        ...
        private static void CheckReferenceId(
            string expectedId,
            INamespaceOrTypeSymbol symbol,
            Compilation compilation
        )
        {
            ...
            var sym = DocumentationCommentId.GetSymbolsForReferenceId(id, compilation)
                .FirstOrDefault();
            Assert.Equal(symbol, sym);
        }
...
"
            ...
            CheckDeclarationId(
                "T:Acme.IProcess",
                compilation.GetTypeByMetadataName("Acme.IProcess"),
                compilation
            );
            CheckDeclarationId(
                "T:Acme.ValueType",
                compilation.GetTypeByMetadataName("Acme.ValueType"),
                compilation
            );
            CheckDeclarationId(
                "T:Acme.Widget",
                compilation.GetTypeByMetadataName("Acme.Widget"),
                compilation
            );
            CheckDeclarationId(
                "T:Acme.Widget.NestedClass",
                compilation.GetTypeByMetadataName("Acme.Widget+NestedClass"),
                compilation
            );
            CheckDeclarationId(
                "T:Acme.Widget.IMenuItem",
                compilation.GetTypeByMetadataName("Acme.Widget+IMenuItem"),
                compilation
            );
            CheckDeclarationId(
                "T:Acme.Widget.Del",
                compilation.GetTypeByMetadataName("Acme.Widget+Del"),
                compilation
            );
            CheckDeclarationId(
                "T:Acme.Widget.Direction",
                compilation.GetTypeByMetadataName("Acme.Widget+Direction"),
                compilation
            );
            CheckDeclarationId(
                "T:Acme.MyList`1",
                compilation.GetTypeByMetadataName("Acme.MyList`1"),
                compilation
            );
            CheckDeclarationId(
                "T:Acme.MyList`1.Helper`2",
                compilation.GetTypeByMetadataName("Acme.MyList`1+Helper`2"),
                compilation
            );
        }
...
"
            ...
            CheckDeclarationId<IMethodSymbol>(
                ...
                compilation,
                s => s.MethodKind == MethodKind.Constructor && s.Parameters.Length == 1
            );
        }
...
"
            ...
            CheckDeclarationId<IMethodSymbol>(
                ...
                s =>
                    ...
                    && s.Parameters[0].Type.Name == "Int32"
            );
            CheckDeclarationId<IMethodSymbol>(
...
"
            ...
            CheckDeclarationId<IMethodSymbol>(
                ...
                compilation,
                m => m.MethodKind == MethodKind.UserDefinedOperator && m.Parameters.Length == 1
            );
        }
...
"
            ...
            CheckDeclarationId<IMethodSymbol>(
                ...
                compilation,
                m => m.MethodKind == MethodKind.UserDefinedOperator && m.Parameters.Length == 2
            );
        }
...
"
            ...
            var ienum = compilation.GetTypeByMetadataName(
            ...
            CheckReferenceId(
                "T:Acme.OuterType`1.InnerType`2:`1",
                innerType.TypeParameters[0],
                compilation
            );
            CheckReferenceId(
                "T:Acme.OuterType`1.InnerType`2:`2",
                innerType.TypeParameters[1],
                compilation
            );
            CheckReferenceId(
            ...
            CheckReferenceId(
                "System.Int32[]",
                compilation.CreateArrayTypeSymbol(intType),
                compilation
            );
            CheckReferenceId(
                "System.Int32*",
