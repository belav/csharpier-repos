...
public class TextLoaderTests
{
    ...
    {
        public new virtual Task<TextAndVersion> LoadTextAndVersionAsync(
            Workspace? workspace,
    ...
    private class LoaderNoOverrideBase : TextLoader
    {
        ...
        // newslot
        public virtual Task<TextAndVersion> LoadTextAndVersionAsync<T>(
            Workspace? workspace,
            DocumentId? documentId,
            CancellationToken cancellationToken
        ) => Task.FromResult((TextAndVersion?)null!);
    ...
    {
        public override Task<TextAndVersion> LoadTextAndVersionAsync<T>(
            Workspace? workspace,
            DocumentId? documentId,
            CancellationToken cancellationToken
        ) => Task.FromResult((TextAndVersion?)null!);
    ...
    {
        public static readonly TextAndVersion Value = TextAndVersion.Create(
            SourceText.From(""),
    ...
    {
        public static new readonly TextAndVersion Value = TextAndVersion.Create(
            SourceText.From(""),
            VersionStamp.Default
        );
    ...
    public async Task OverridesObsolete()
    {
        ...
        Assert.Same(
            LoaderOverridesObsolete.Value,
            await loader.LoadTextAndVersionAsync(
                new LoadTextOptions(SourceHashAlgorithms.Default),
                CancellationToken.None
            )
        );
    ...
    public async Task OverridesObsolete2()
    {
        ...
        Assert.Same(
            LoaderOverridesObsolete2.Value,
            await loader.LoadTextAndVersionAsync(
                new LoadTextOptions(SourceHashAlgorithms.Default),
                CancellationToken.None
            )
