...
namespace Microsoft.CodeAnalysis.UnitTests
{
    ...
    public class SolutionWithSourceGeneratorTests : TestBase
    {
        ...
        )
        {
            ...
            {
                originalCompilation = await project.GetRequiredCompilationAsync(
            ...
            Assert.Equal(
                $"{generatorType.Assembly.GetName().Name}\\{generatorType.FullName}\\Test.generated.cs",
        ...
        public async Task WithReferencesMethodCorrectlyUpdatesWithEqualReferences(TestHost testHost)
        {
            ...
            var analyzerReference1 = new TestGeneratorReferenceWithFilePathEquality(
                CreateGenerator(),
                SharedPath
            );
            var analyzerReference2 = new TestGeneratorReferenceWithFilePathEquality(
                CreateGenerator(),
                SharedPath
            );
            ...
            project = project.WithAnalyzerReferences(
                SpecializedCollections.EmptyEnumerable<AnalyzerReference>()
            );
        ...
        )
        {
            ...
            var generatorReferenceToKeep = new TestGeneratorReference(
                new SingleFileTestGenerator(
                    "// StaticContent",
                    hintName: "generatorReferenceToKeep"
                )
            );
            var analyzerReferenceToAddAndRemove = new TestGeneratorReference(
                new SingleFileTestGenerator2(
                    "// More Static Content",
                    hintName: "analyzerReferenceToAddAndRemove"
                )
            );
            ...
            project = project.WithAnalyzerReferences(
                new[] { generatorReferenceToKeep, analyzerReferenceToAddAndRemove }
            );
        ...
        )
        {
            ...
            var generatorReference2 = new TestGeneratorReference(
                new SingleFileTestGenerator("", hintName: "DuplicateFile"),
                analyzerFilePath: "Z:\\B.dll"
            );
        ...
        public async Task IncrementalSourceGeneratorInvokedCorrectNumberOfTimes()
        {
            using var workspace = CreateWorkspace(
                [typeof(TestCSharpCompilationFactoryServiceWithIncrementalGeneratorTracking)]
            );
            ...
            var generatorDriver = project
                .Solution.State.GetTestAccessor()
        ...
        )
        {
            ...
            project = project
                .Solution.WithDocumentText(documentId, SourceText.From("// Changed Source File"))
                .Projects.Single();
            ...
            static async Task AssertCompilationContainsOneRegularAndOneGeneratedFile(
                Project project,
            ...
            )
            {
                ...
                var regularDocumentSyntaxTree = await project
                ...
                var generatedSyntaxTree = Assert.Single(
                    compilation.SyntaxTrees.Where(t => t != regularDocumentSyntaxTree)
                );
        ...
        )
        {
            ...
            if (assertRightAway)
                await AssertCompilationContainsGeneratedFilesAsync(
                    project,
                    expectedGeneratedContents: Array.Empty<string>()
                );
            ...
            static async Task AssertCompilationContainsGeneratedFilesAsync(
                Project project,
                params string[] expectedGeneratedContents
            )
            {
                var compilation = await project.GetRequiredCompilationAsync(CancellationToken.None);
        ...
        public async Task DocumentIdOfGeneratedDocumentsIsStable(TestHost testHost)
        {
            ...
            var generatedDocumentBeforeChange = Assert.Single(
        ...
        public async Task DocumentIdGuidInDifferentProjectsIsDifferent(TestHost testHost)
        {
            ...
            var solutionWithProjects = AddProjectWithReference(
            ...
            var generatedDocumentsInFirstProject = await solutionWithProjects
                .GetRequiredProject(projectIds[0])
                .GetSourceGeneratedDocumentsAsync();
            var generatedDocumentsInSecondProject = await solutionWithProjects
            ...
            Assert.NotEqual(
                ...
                generatedDocumentsInSecondProject.Single().Id.Id
            );
        ...
        )
        {
            ...
            var compilationWithReference = await solution
                .GetRequiredProject(projectIdWithReference)
                .GetRequiredCompilationAsync(CancellationToken.None);

            var compilationReference = Assert.IsAssignableFrom<CompilationReference>(
                Assert.Single(compilationWithReference.References)
            );

            var compilationWithGenerator = await solution
        ...
        public async Task GetDocumentWithGeneratedTreeReturnsGeneratedDocument(TestHost testHost)
        {
            ...
            var generatedDocument = Assert.IsType<SourceGeneratedDocument>(
        ...
        )
        {
            ...
            project = project
                .Documents.Single(d => d.Name == "RegularDocument.cs")
                .WithFrozenPartialSemantics(CancellationToken.None)
                .Project;
            ...
            var syntaxTree = Assert.Single(
                (await project.GetRequiredCompilationAsync(CancellationToken.None)).SyntaxTrees,
                t => t.FilePath != "RegularDocument.cs"
            );
            var generatedDocument = Assert.IsType<SourceGeneratedDocument>(
                project.GetDocument(syntaxTree)
            );
        ...
        public async Task TreeReusedIfGeneratedFileDoesNotChangeBetweenRuns(TestHost testHost)
        {
            ...
            var generatedTreeBeforeChange = await Assert
                .Single(await project.GetSourceGeneratedDocumentsAsync())
                .GetSyntaxTreeAsync();
            ...
            var generatedTreeAfterChange = await Assert
                .Single(await project.GetSourceGeneratedDocumentsAsync())
                .GetSyntaxTreeAsync();
        ...
        public async Task TreeNotReusedIfParseOptionsChangeChangeBetweenRuns(TestHost testHost)
        {
            ...
            project = project.WithParseOptions(
                project.ParseOptions.WithDocumentationMode(DocumentationMode.Diagnose)
            );
        ...
        )
        {
            ...
            var compilationAfterFirstChange = await project.GetRequiredCompilationAsync(
                CancellationToken.None
            );

            project = await MakeChangesToDocument(project);

            var compilationAfterSecondChange = await project.GetRequiredCompilationAsync(
                CancellationToken.None
            );
            ...
            var cachedStateAfterSecondChange = GetDeclarationManagerCachedStateForUnchangingTrees(
            ...
            {
                var syntaxAndDeclarationsManager = compilation.GetFieldValue(
            ...
            static async Task<Project> MakeChangesToDocument(Project project)
            {
                ...
                var newText = existingText.WithChanges(
        ...
        )
        {
            ...
            workspace.OnSourceGeneratedDocumentOpened(
                differentOpenTextContainer,
                generatedDocument
            );

            generatedDocument = Assert.IsType<SourceGeneratedDocument>(
                differentOpenTextContainer.CurrentText.GetOpenDocumentInCurrentContextWithChanges()
            );
            Assert.Same(
                differentOpenTextContainer.CurrentText,
                await generatedDocument.GetTextAsync()
            );
            ...
            var compilation = await generatedDocument.Project.GetRequiredCompilationAsync(
                CancellationToken.None
            );
        ...
        )
        {
            ...
            var generatedDocument = Assert.Single(
                await workspace.CurrentSolution.Projects.Single().GetSourceGeneratedDocumentsAsync()
            );
            var differentOpenTextContainer = SourceText
                .From("// StaticContent", Encoding.UTF8)
                .Container;
        ...
        )
        {
            ...
            Assert.True(
                workspace.SetCurrentSolution(
                    _ => originalAdditionalFile.Project.Solution,
                    WorkspaceChangeKind.SolutionChanged
                )
            );

            var generatedDocument = Assert.Single(
                await originalAdditionalFile.Project.GetSourceGeneratedDocumentsAsync()
            );
            var differentOpenTextContainer = SourceText.From("// Open Text").Container;
        ...
        )
        {
            ...
            solution = AddEmptyProject(solution)
                .AddProjectReference(new ProjectReference(projectIdWithGenerator))
                .Solution;

            Assert.True(
                workspace.SetCurrentSolution(_ => solution, WorkspaceChangeKind.SolutionChanged)
            );

            var generatedDocument = Assert.Single(
                await workspace
                    .CurrentSolution.GetRequiredProject(projectIdWithGenerator)
                    .GetSourceGeneratedDocumentsAsync()
            );
            var differentOpenTextContainer = SourceText.From("// Open Text").Container;
            ...
            var compilationReference = Assert.Single(
                compilationWithReference.References.OfType<CompilationReference>()
            );
        ...
        )
        {
            ...
            var document = await workspace.CurrentSolution.GetSourceGeneratedDocumentAsync(
        ...
        [Theory, CombinatorialData]
        public async Task FreezingSolutionEnsuresGeneratorsDoNotRun(
            bool forkBeforeFreeze,
            TestHost testHost
        )
        {
        ...
        public async Task ForkAfterFreezeNoLongerRunsGenerators(TestHost testHost)
        {
            ...
            Assert.Equal(
                "// Something else",
                (await document.GetRequiredSyntaxRootAsync(CancellationToken.None)).ToFullString()
            );
        }
        ...
        public async Task LinkedDocumentOfFrozenShouldNotRunSourceGenerator(TestHost testHost)
        {
            ...
            var originalDocument1 = AddEmptyProject(workspace.CurrentSolution, name: "Project1")
                ...
                .AddDocument(
                    "RegularDocument.cs",
                    "// Source File",
                    filePath: "RegularDocument.cs"
                );

            // this is a linked document of document1 above
            var originalDocument2 = AddEmptyProject(
                    originalDocument1.Project.Solution,
                    name: "Project2"
                )
                .AddAnalyzerReference(analyzerReference)
                .AddDocument(
                    originalDocument1.Name,
                    await originalDocument1.GetTextAsync().ConfigureAwait(false),
                    filePath: originalDocument1.FilePath
                );

            var frozenSolution = originalDocument2
                .WithFrozenPartialSemantics(CancellationToken.None)
                .Project.Solution;
            ...
            foreach (var documentIdToTest in documentIdsToTest)
            {
                ...
                Assert.Equal(
                    document.GetLinkedDocumentIds().Single(),
                    documentIdsToTest.Except(new[] { documentIdToTest }).Single()
                );
                document = document.WithText(SourceText.From("// Something else"));
        ...
        public async Task DynamicFilesNotPassedToSourceGenerators(TestHost testHost)
        {
            ...
            project = project
                .Solution.AddDocument(documentInfo)
                .Projects.Single()
                .AddAnalyzerReference(analyzerReference);
