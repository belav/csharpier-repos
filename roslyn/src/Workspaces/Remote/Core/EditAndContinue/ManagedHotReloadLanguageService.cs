...
internal sealed partial class ManagedHotReloadLanguageService : IManagedHotReloadLanguageService
{
    ...
    private async ValueTask<Solution> GetCurrentDesignTimeSolutionAsync(
        CancellationToken cancellationToken
    )
    {
        ...
        var id = await _solutionSnapshotProvider
            .RegisterSolutionSnapshotAsync(cancellationToken)
    ...
    public async ValueTask StartSessionAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            var currentDesignTimeSolution = await GetCurrentDesignTimeSolutionAsync(
                    cancellationToken
                )
                .ConfigureAwait(false);
            ...
            _debuggingSession = await _encService
    ...
    private ValueTask BreakStateOrCapabilitiesChangedAsync(
        bool? inBreakState,
        CancellationToken cancellationToken
    )
    {
        ...
        try
        {
            ...
            _encService.BreakStateOrCapabilitiesChanged(
                _debuggingSession.Value,
                inBreakState,
                out _
            );
        }
    ...
    public ValueTask CommitUpdatesAsync(CancellationToken cancellationToken)
    {
        ...
        {
            Contract.ThrowIfNull(_debuggingSession);
            var committedDesignTimeSolution = Interlocked.Exchange(
                ref _pendingUpdatedDesignTimeSolution,
                null
            );
    ...
    )
    {
        try
        {
            ...
            var newSolution = await GetCurrentDesignTimeSolutionAsync(cancellationToken)
                .ConfigureAwait(false);

            return (sourceFilePath != null)
                ? await EditSession
                    .HasChangesAsync(oldSolution, newSolution, sourceFilePath, cancellationToken)
                    .ConfigureAwait(false)
                : await EditSession
                    .HasChangesAsync(oldSolution, newSolution, cancellationToken)
                    .ConfigureAwait(false);
        }
        catch (Exception e) when (FatalError.ReportAndCatchUnlessCanceled(e, cancellationToken))
        {
    ...
    )
    {
        if (_disabled)
        {
            return new ManagedHotReloadUpdates(
                ...
                ImmutableArray<ManagedHotReloadDiagnostic>.Empty
            );
        }
        ...
        try
        {
            ...
            var designTimeSolution = await GetCurrentDesignTimeSolutionAsync(cancellationToken)
                .ConfigureAwait(false);
            ...
            catch (Exception e) when (FatalError.ReportAndCatchUnlessCanceled(e, cancellationToken))
            {
                ...
                diagnosticData = ImmutableArray.Create(
                    DiagnosticData.Create(designTimeSolution, diagnostic, project: null)
                );
                rudeEdits = ImmutableArray<(
                    DocumentId DocumentId,
                    ImmutableArray<RudeEditDiagnostic> Diagnostics
                )>.Empty;
