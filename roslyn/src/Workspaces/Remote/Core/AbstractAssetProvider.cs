...
internal abstract class AbstractAssetProvider
{
    ...
    )
    {
        ...
        using var _ = ArrayBuilder<ProjectInfo>.GetInstance(
            solutionChecksums.Projects.Length,
            out var projects
        );
        foreach (var (projectChecksum, projectId) in solutionChecksums.Projects)
            projects.AddIfNotNull(
                await CreateProjectInfoAsync(projectId, projectChecksum, cancellationToken)
                    .ConfigureAwait(false)
            );
    ...
    )
    {
        ...
        return ProjectInfo.Create(
            ...
            hostObjectType: null
        ); // TODO: https://github.com/dotnet/roslyn/issues/62804

        async Task<ImmutableArray<DocumentInfo>> CreateDocumentInfosAsync(
            ChecksumsAndIds<DocumentId> checksumsAndIds
        )
        {
            using var _ = ArrayBuilder<DocumentInfo>.GetInstance(
                checksumsAndIds.Length,
                out var documentInfos
            );
            ...
            foreach (var (documentChecksum, documentId) in checksumsAndIds)
            {
                ...
                documentInfos.Add(
                    await CreateDocumentInfoAsync(documentId, documentChecksum, cancellationToken)
                        .ConfigureAwait(false)
                );
            }
    ...
    )
    {
        ...
        var text = await serializableSourceText
            .GetTextAsync(cancellationToken)
            .ConfigureAwait(false);
        var textLoader = TextLoader.From(
            TextAndVersion.Create(text, VersionStamp.Create(), attributes.FilePath)
        );
        ...
        where T : class
    {
        ...
        foreach (var checksum in checksums)
        {
            cancellationToken.ThrowIfCancellationRequested();
            assets.Add(
                await GetAssetAsync<T>(assetHint, checksum, cancellationToken).ConfigureAwait(false)
            );
        }
