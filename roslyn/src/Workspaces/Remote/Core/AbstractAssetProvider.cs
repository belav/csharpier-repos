...
internal abstract class AbstractAssetProvider
{
    ...
    )
    {
        ...
        using var _ = ArrayBuilder<ProjectInfo>.GetInstance(
            solutionChecksums.Projects.Count,
            out var projects
        );
        foreach (var projectChecksum in solutionChecksums.Projects)
            projects.AddIfNotNull(
                await CreateProjectInfoAsync(projectChecksum, cancellationToken)
                    .ConfigureAwait(false)
            );
    ...
    private async Task<ImmutableArray<DocumentInfo>> CreateDocumentInfosAsync(
        ChecksumCollection documentChecksums,
        CancellationToken cancellationToken
    )
    {
        using var _ = ArrayBuilder<DocumentInfo>.GetInstance(
            documentChecksums.Count,
