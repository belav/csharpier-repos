    ...
    internal sealed class JsonRpcConnection : RemoteServiceConnection
    {
        ...
        )
        {
            if (solution != null)
            {
                ...
                if (dataReader != null)
                {
                    return await _serviceEndPoint
                        .InvokeAsync(targetName, argumentsBuilder, dataReader, cancellationToken)
                        .ConfigureAwait(false);
                }
                else
                {
                    return await _serviceEndPoint
                        .InvokeAsync<T>(targetName, argumentsBuilder, cancellationToken)
                        .ConfigureAwait(false);
                }
            }
            ...
            {
                return await _serviceEndPoint
                    .InvokeAsync(targetName, arguments, dataReader, cancellationToken)
                    .ConfigureAwait(false);
            }
            else
            {
                return await _serviceEndPoint
                    .InvokeAsync<T>(targetName, arguments, cancellationToken)
                    .ConfigureAwait(false);
            }
        }
    }
