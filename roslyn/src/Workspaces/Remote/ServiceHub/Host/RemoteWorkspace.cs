...
namespace Microsoft.CodeAnalysis.Remote
{
    ...
    internal sealed partial class RemoteWorkspace : Workspace
    {
        ...
        private readonly Dictionary<
            Checksum,
            ReferenceCountedDisposable<LazySolution>
        > _solutionChecksumToLazySolution = new();
        ...
        )
        {
            ...
            await using var configuredAsyncDisposable = refCountedLazySolution.ConfigureAwait(
                false
            );
            ...
            var newSolution = await refCountedLazySolution.Target.Task
            ...
            if (fromPrimaryBranch)
                (newSolution, _) = await this.TryUpdateWorkspaceCurrentSolutionAsync(
                        workspaceVersion,
                        newSolution,
                        cancellationToken
                    )
            ...
            async ValueTask<ReferenceCountedDisposable<LazySolution>> GetLazySolutionAsync()
            {
                ...
                {
                    if (
                        _solutionChecksumToLazySolution.TryGetValue(
                            solutionChecksum,
                            out var refCountedLazySolution
                        )
                    )
                    {
                    ...
                    var lazySolution = new LazySolution(
                        ...
                        cleanupAsync: async () =>
                        {
                            ...
                            if (
                                _solutionChecksumToLazySolution.TryGetValue(
                                    solutionChecksum,
                                    out var remainingRefCountedLazySolution
                                )
                    ...
                    refCountedLazySolution = new ReferenceCountedDisposable<LazySolution>(
                        lazySolution
                    );
        ...
        private Solution CreateSolutionFromInfoAndOptions(
        ...
        public readonly struct TestAccessor
        {
            ...
            public Solution CreateSolutionFromInfoAndOptions(
                SolutionInfo solutionInfo,
                SerializableOptionSet options
            ) => _remoteWorkspace.CreateSolutionFromInfoAndOptions(solutionInfo, options);

            public ValueTask<(Solution solution, bool updated)> TryUpdateWorkspaceCurrentSolutionAsync(
                Solution newSolution,
                int workspaceVersion
            ) =>
                _remoteWorkspace.TryUpdateWorkspaceCurrentSolutionAsync(
                    workspaceVersion,
                    newSolution,
            ...
            {
                var tuple = await _remoteWorkspace
