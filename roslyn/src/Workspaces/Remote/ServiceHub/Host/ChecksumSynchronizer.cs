...
namespace Microsoft.CodeAnalysis.Remote
{
    internal class ChecksumSynchronizer
    {
        ...
        private async Task SynchronizeProjectAssets_NoLockAsync(
            IEnumerable<Checksum> projectChecksums,
            CancellationToken cancellationToken
        )
        {
            ...
            {
                var projectChecksumObject =
                    await _assetProvider.GetAssetAsync<ProjectStateChecksums>(
                        projectChecksum,
                        cancellationToken
                    )
                        .ConfigureAwait(false);
            ...
            await _assetProvider.SynchronizeAssetsAsync(checksums, cancellationToken)
                .ConfigureAwait(false);
        }
        ...
        private async Task SynchronizeAssets_NoLockAsync(
        ...
        private async Task CollectChecksumChildrenAsync(
        ...
        )
        {
            ...
            {
                var checksumObject = await _assetProvider.GetAssetAsync<ChecksumWithChildren>(
                    checksum,
                    cancellationToken
                )
                    .ConfigureAwait(false);
