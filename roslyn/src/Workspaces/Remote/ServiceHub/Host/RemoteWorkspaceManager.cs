...
namespace Microsoft.CodeAnalysis.Remote
{
    ...
    internal class RemoteWorkspaceManager
    {
        ...
        /// </summary>
        /// <remarks>
        /// The general thinking behind these timings is that we don't want to be too aggressive constantly
        // waking up
        /// and cleaning purging items from the cache.  But we also don't want to wait an excessive amount
        // of time,
        /// allowing it to get too full.
        /// <para>
        /// Also note that the asset cache will not remove items associated with the <see
        /// cref="Workspace.CurrentSolution"/> of the workspace it is created against.  This ensures that
        // the assets
        /// associated with the solution that most closely corresponds to what the user is working with will
        // stay pinned
        /// on the remote side and not get purged just because the user stopped interactive for a while.
        // This ensures
        /// the next sync (which likely overlaps heavily with the current solution) will not force the same
        // assets to be
        /// resent.
        /// </para>
        /// <list type="bullet">
        /// <item>CleanupInterval=30s gives what feels to be a reasonable non-aggressive amount of time to
        // let the cache
        /// do its job, while also making sure several times a minute it is scanned for things that can be
        /// dropped.</item>
        /// <item>PurgeAfter=1m effectively states that an item will be dumped from the cache if not used in
        // the last
        /// minute.  This seems reasonable for keeping around all the parts of the current solutions in use,
        // while
        /// allowing values from the past, or values removed from the solution to not persist too
        // long.</item>
        /// <item>GcAfter=1m means that we'll force some GCs to happen after that amount of time of
        // *non-activity*.  In
        ...
        internal static readonly RemoteWorkspaceManager Default =
            new(workspace => new SolutionAssetCache(
                workspace,
        ...
        /// <summary>
        /// Not ideal that we exposing the workspace solution, while not ensuring it stays alive for other
        // calls using
        /// the same <paramref name="solutionChecksum"/>). However, this is used by Pythia/Razor/UnitTesting
        // which all
