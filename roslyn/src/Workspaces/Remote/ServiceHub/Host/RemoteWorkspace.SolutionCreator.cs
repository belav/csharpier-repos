...
#pragma warning restore
            ...
            public async Task<bool> IsIncrementalUpdateAsync(Checksum newSolutionChecksum)
            {
                var newSolutionChecksums = await _assetProvider
                    .GetAssetAsync<SolutionStateChecksums>(newSolutionChecksum, _cancellationToken)
            ...
            public async Task<Solution> CreateSolutionAsync(Checksum newSolutionChecksum)
            {
                try
                {
                    ...
                    {
                        var newOptions = await _assetProvider
                            .GetAssetAsync<SerializableOptionSet>(
                    ...
                    Contract.ThrowIfFalse(
                        solution.State.FrozenSourceGeneratedDocumentState == null
                    );
                    ...
                    )
                    {
                        ...
                        var sourceText = await serializableSourceText
                            .GetTextAsync(_cancellationToken)
                            .ConfigureAwait(false);
                        solution =
                            solution
                    ...
                    await ValidateChecksumAsync(newSolutionChecksum, solution)
                        .ConfigureAwait(false);
#endif
            ...
            )
            {
                ...
                return await UpdateProjectsAsync(solution, olds.Object, news.Object)
                    .ConfigureAwait(false);
            }
            ...
            )
            {
                ...
                {
                    project = await UpdateProjectInfoAsync(project, newProjectChecksums.Info)
                        .ConfigureAwait(false);
                }
            ...
            )
            {
                var newProjectAttributes = await _assetProvider
                    .GetAssetAsync<ProjectInfo.ProjectAttributes>(infoChecksum, _cancellationToken)
                    .ConfigureAwait(false);
            ...
            )
            {
                ...
                var oldMap = await GetDocumentMapAsync(existingTextDocumentStates, olds.Object)
                    .ConfigureAwait(false);
                var newMap = await GetDocumentMapAsync(_assetProvider, news.Object)
                    .ConfigureAwait(false);
            ...
            )
            {
                ...
                {
                    document = await UpdateDocumentInfoAsync(document, newDocumentChecksums.Info)
                        .ConfigureAwait(false);
                }
            ...
            )
            {
                ...
                var documentChecksums = await assetProvider
                    .GetAssetsAsync<DocumentStateChecksums>(documents, _cancellationToken)
                    .ConfigureAwait(false);
