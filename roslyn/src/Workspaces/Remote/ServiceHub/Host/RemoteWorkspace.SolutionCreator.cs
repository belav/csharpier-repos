...
#pragma warning restore
            ...
            )
            {
                var newSolutionChecksums = await _assetProvider
                    .GetAssetAsync<SolutionStateChecksums>(newSolutionChecksum, cancellationToken)
            ...
            )
            {
                ...
                return await UpdateProjectsAsync(
                        solution,
                        olds.Object,
                        news.Object,
                        cancellationToken
            ...
            {
                var oldMap = await GetProjectMapAsync(solution, oldChecksums, cancellationToken)
                    .ConfigureAwait(false);
                ...
                foreach (var (projectId, newProjectChecksums) in newMap)
                {
                    if (!oldMap.ContainsKey(projectId))
                    {
                        var projectInfo = await _assetProvider
                            .CreateProjectInfoAsync(newProjectChecksums.Checksum, cancellationToken)
                            .ConfigureAwait(false);
                        if (projectInfo == null)
                        {
            ...
            )
            {
                ...
                await _assetProvider
                    .SynchronizeProjectAssetsAsync(pooledObject.Object, cancellationToken)
                    .ConfigureAwait(false);
            }
            ...
            )
            {
                var newProjectAttributes = await _assetProvider
                    .GetAssetAsync<ProjectInfo.ProjectAttributes>(infoChecksum, cancellationToken)
                    .ConfigureAwait(false);
            ...
            )
            {
                ...
                var oldMap = await GetDocumentMapAsync(
                        existingTextDocumentStates,
                        olds.Object,
                        cancellationToken
                    )
                    .ConfigureAwait(false);
            ...
            )
            {
                var newDocumentInfo = await _assetProvider
                    .GetAssetAsync<DocumentInfo.DocumentAttributes>(infoChecksum, cancellationToken)
                    .ConfigureAwait(false);
            ...
            private static async ValueTask<
                Dictionary<DocumentId, DocumentStateChecksums>
            > GetDocumentMapAsync(
            ...
            )
            {
                ...
                var documentChecksums = await assetProvider
                    .GetAssetsAsync<DocumentStateChecksums>(documents, cancellationToken)
                    .ConfigureAwait(false);
                ...
                var infos = await assetProvider
                    .GetAssetsAsync<DocumentInfo.DocumentAttributes>(
                        infoChecksums,
                        cancellationToken
                    )
                    .ConfigureAwait(false);
                ...
                {
                    Debug.Assert(
                        assetProvider.EnsureCacheEntryIfExists(kv.Item2.Info),
                        "Expected the prior call to GetAssetsAsync to obtain all items for this loop."
                    );
            ...
            private static async ValueTask<
                Dictionary<ProjectId, ProjectStateChecksums>
            > GetProjectMapAsync(
                AssetProvider assetProvider,
            ...
            )
            {
                ...
                var projectChecksums = await assetProvider
                    .GetAssetsAsync<ProjectStateChecksums>(projects, cancellationToken)
                    .ConfigureAwait(false);
                ...
                var infos = await assetProvider
                    .GetAssetsAsync<ProjectInfo.ProjectAttributes>(infoChecksums, cancellationToken)
                    .ConfigureAwait(false);
