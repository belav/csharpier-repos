...
#pragma warning restore
            ...
            public async Task<bool> IsIncrementalUpdateAsync(
            ...
            )
            {
                ...
                var newSolutionInfo = await _assetProvider
                    .GetAssetAsync<SolutionInfo.SolutionAttributes>(
                        assetHint: AssetHint.None,
                ...
                return _baseSolution.Id == newSolutionInfo.Id
                ...
                try
                {
                    ...
                    if (oldSolutionChecksums.Attributes != newSolutionChecksums.Attributes)
                    {
                        ...
                        Contract.ThrowIfFalse(
                            solution.Id == newSolutionInfo.Id
                    ...
                    if (
                        newSolutionChecksums.FrozenSourceGeneratedDocumentIdentity != Checksum.Null
                        && newSolutionChecksums.FrozenSourceGeneratedDocumentText != Checksum.Null
                    )
                    {
                        var identity = await _assetProvider
                            .GetAssetAsync<SourceGeneratedDocumentIdentity>(
                                assetHint: AssetHint.None,
                                newSolutionChecksums.FrozenSourceGeneratedDocumentIdentity,
                                cancellationToken
                            )
                            .ConfigureAwait(false);
                        var serializableSourceText = await _assetProvider
                            .GetAssetAsync<SerializableSourceText>(
                                assetHint: AssetHint.None,
                                newSolutionChecksums.FrozenSourceGeneratedDocumentText,
                                cancellationToken
                            )
                            .ConfigureAwait(false);

                        var sourceText = await serializableSourceText
                    ...
                    await ValidateChecksumAsync(newSolutionChecksum, solution, cancellationToken)
                        .ConfigureAwait(false);
#endif
            ...
            private async Task<Solution> UpdateProjectsAsync(
                Solution solution,
                SolutionStateChecksums oldSolutionChecksums,
                SolutionStateChecksums newSolutionChecksums,
                CancellationToken cancellationToken
            )
            {
                ...
                using var _1 = PooledDictionary<ProjectId, ProjectStateChecksums>.GetInstance(
                    out var oldMap
                );
                using var _2 = PooledDictionary<ProjectId, ProjectStateChecksums>.GetInstance(
                    out var newMap
                );
                ...
                // bulk sync assets
                await SynchronizeAssetsAsync(oldMap, newMap, cancellationToken)
                    .ConfigureAwait(false);
                ...
                foreach (var (projectId, newProjectChecksums) in newMap)
                {
                    ...
                    {
                        var projectInfo = await _assetProvider
                            .CreateProjectInfoAsync(
                                projectId,
                ...
                async Task PopulateOldProjectMapAsync()
                {
                    ...
                    {
                        var projectChecksums = await projectState
                ...
                {
                    var projectStateChecksums = await @this
                        ._assetProvider.GetAssetsAsync<ProjectStateChecksums>(
                            assetHint: AssetHint.None,
                            news.Object,
                            cancellationToken
                        )
            ...
            private async ValueTask SynchronizeAssetsAsync(
            ...
            )
            {
                ...
                foreach (var (projectId, projectStateChecksums) in newMap)
                {
                    ...
                    await _assetProvider
                        .SynchronizeProjectAssetsAsync(projectStateChecksums, cancellationToken)
                        .ConfigureAwait(false);
                }
            }

            private async Task<Solution> UpdateProjectAsync(
                Project project,
            ...
            )
            {
                ...
                {
                    project = await UpdateProjectInfoAsync(
                            project,
                            newProjectChecksums.Info,
                            cancellationToken
                        )
                        .ConfigureAwait(false);
                }
                ...
                )
                {
                    project = await UpdateDocumentsAsync(
                            ...
                            static (solution, documents) =>
                                solution.AddAdditionalDocuments(documents),
                            static (solution, documentIds) =>
                                solution.RemoveAdditionalDocuments(documentIds),
                            cancellationToken
                        )
                        .ConfigureAwait(false);
                }
                ...
                )
                {
                    project = await UpdateDocumentsAsync(
                            ...
                            static (solution, documents) =>
                                solution.AddAnalyzerConfigDocuments(documents),
                            static (solution, documentIds) =>
            ...
            )
            {
                ...
                Contract.ThrowIfFalse(
                    project.State.ProjectInfo.Attributes.IsSubmission
                        == newProjectAttributes.IsSubmission
                );
                ...
                {
                    project = project
                        .Solution.WithProjectName(projectId, newProjectAttributes.Name)
                ...
                {
                    project = project
                        .Solution.WithProjectFilePath(projectId, newProjectAttributes.FilePath)
                        .GetProject(projectId)!;
                }
                ...
                {
                    project = project
                        .Solution.WithProjectOutputFilePath(
                            projectId,
                            newProjectAttributes.OutputFilePath
                        )
                        .GetProject(projectId)!;
                ...
                {
                    project = project
                        .Solution.WithProjectOutputRefFilePath(
                            projectId,
                            newProjectAttributes.OutputRefFilePath
                        )
                        .GetProject(projectId)!;
                ...
                {
                    project = project
                        .Solution.WithProjectDefaultNamespace(
                            projectId,
                            newProjectAttributes.DefaultNamespace
                        )
                        .GetProject(projectId)!;
                ...
                {
                    project = project
                        .Solution.WithHasAllInformation(
                            projectId,
                            newProjectAttributes.HasAllInformation
                        )
                        .GetProject(projectId)!;
                ...
                {
                    project = project
                        .Solution.WithRunAnalyzers(projectId, newProjectAttributes.RunAnalyzers)
                        .GetProject(projectId)!;
                }
                ...
                {
                    project = project
                        .Solution.WithProjectChecksumAlgorithm(
                            projectId,
                            newProjectAttributes.ChecksumAlgorithm
                        )
                        .GetProject(projectId)!;
                ...
                where TDocumentState : TextDocumentState
            {
                ...
                using var _1 = PooledDictionary<DocumentId, DocumentStateChecksums>.GetInstance(
                    out var oldMap
                );
                using var _2 = PooledDictionary<DocumentId, DocumentStateChecksums>.GetInstance(
                    out var newMap
                );
                ...
                foreach (var (documentId, newDocumentChecksums) in newMap)
                {
                    ...
                    Contract.ThrowIfTrue(
                        oldDocumentChecksums.Checksum == newDocumentChecksums.Checksum
                    );
                ...
                {
                    project = removeDocuments(project.Solution, lazyDocumentsToRemove.ToImmutable())
                        .GetProject(project.Id)!;
                }
                ...
                async Task PopulateOldDocumentMapAsync()
                {
                    ...
                    {
                        var documentChecksums = await state
                            .GetStateChecksumsAsync(cancellationToken)
                            .ConfigureAwait(false);
                ...
                {
                    var documentStateChecksums = await @this
                        ._assetProvider.GetAssetsAsync<DocumentStateChecksums>(
                            assetHint: project.Id,
                            news.Object,
                            cancellationToken
                        )
            ...
            )
            {
                ...
                {
                    document = await UpdateDocumentInfoAsync(
                            document,
                            newDocumentChecksums.Info,
                            cancellationToken
                        )
                        .ConfigureAwait(false);
                }
                ...
                if (oldDocumentChecksums.Text != newDocumentChecksums.Text)
                {
                    ...
                    {
                        TextDocumentKind.Document => document
                            .Project.Solution.WithDocumentText(document.Id, sourceText)
                            .GetDocument(document.Id)!,
                        TextDocumentKind.AnalyzerConfigDocument => document
                            .Project.Solution.WithAnalyzerConfigDocumentText(
                                document.Id,
                                sourceText
                            )
                            .GetAnalyzerConfigDocument(document.Id)!,
                        TextDocumentKind.AdditionalDocument => document
                            .Project.Solution.WithAdditionalDocumentText(document.Id, sourceText)
            ...
            )
            {
                var newDocumentInfo = await _assetProvider
                    .GetAssetAsync<DocumentInfo.DocumentAttributes>(
                        assetHint: document.Id,
                        infoChecksum,
                        cancellationToken
                    )
                ...
                if (document.State.Attributes.Folders != newDocumentInfo.Folders)
                {
                    ...
                    document = document
                        .Project.Solution.WithDocumentFolders(document.Id, newDocumentInfo.Folders)
                        .GetDocument(document.Id)!;
                }
                ...
                if (document.State.Attributes.SourceCodeKind != newDocumentInfo.SourceCodeKind)
                {
                    ...
                    Contract.ThrowIfFalse(document is Document);
                    document = document
                        .Project.Solution.WithDocumentSourceCodeKind(
                            document.Id,
            ...
            )
            {
                ...
                var solutionInfo = await _assetProvider
                    .CreateSolutionInfoAsync(checksumFromRequest, cancellationToken)
                    .ConfigureAwait(false);
