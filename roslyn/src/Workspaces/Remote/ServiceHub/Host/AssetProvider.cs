...
) : AbstractAssetProvider
{
    ...
    )
    {
        ...
        await this.SynchronizeAssetsAsync(assetHint, checksums, cancellationToken)
    ...
    )
    {
        ...
        using var _ = ArrayBuilder<ValueTuple<Checksum, T>>.GetInstance(
            checksums.Count,
            out var list
        );
    ...
    )
    {
        ...
        using (
            Logger.LogBlock(
                ...
                Checksum.GetProjectChecksumsLogInfo,
                projectChecksums,
                cancellationToken
            )
        )
    ...
    )
    {
        ...
        {
            using var _1 = ArrayBuilder<Checksum>.GetInstance(
                checksums.Count,
                out var missingChecksums
            );
            foreach (var checksum in checksums)
            {
            ...
            var assets = await RequestAssetsAsync(assetHint, checksumsArray, cancellationToken)
                .ConfigureAwait(false);
