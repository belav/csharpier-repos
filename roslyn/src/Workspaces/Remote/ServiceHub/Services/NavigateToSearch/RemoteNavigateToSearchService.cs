...
namespace Microsoft.CodeAnalysis.Remote
{
            ...
            IRemoteNavigateToSearchService
    {
        internal sealed class Factory
            : FactoryBase<IRemoteNavigateToSearchService, IRemoteNavigateToSearchService.ICallback>
        {
        ...
        ) GetCallbacks(RemoteServiceCallbackId callbackId, CancellationToken cancellationToken)
        {
            Func<RoslynNavigateToItem, Task> onItemFound = async i =>
                await _callback
                    .InvokeAsync(
                        (callback, _) => callback.OnResultFoundAsync(callbackId, i),
                        cancellationToken
                    )
                    .ConfigureAwait(false);

            Func<Task> onProjectCompleted = async () =>
                await _callback
                    .InvokeAsync(
                        (callback, _) => callback.OnProjectCompletedAsync(callbackId),
                        cancellationToken
                    )
                    .ConfigureAwait(false);
        ...
        )
        {
            ...
            return RunServiceAsync(
                solutionChecksum,
                solution => ValueTaskFactory.CompletedTask,
                cancellationToken
            );
        }
        ...
        )
        {
            return RunServiceAsync(
                ...
                async solution =>
                {
                    ...
                    var (onItemFound, onProjectCompleted) = GetCallbacks(
                        callbackId,
                        cancellationToken
                    );

                    await AbstractNavigateToSearchService
            ...
            return RunServiceAsync(
                ...
                async solution =>
                {
                    ...
                    var priorityDocuments = priorityDocumentIds.SelectAsArray(d =>
                        solution.GetRequiredDocument(d)
                    );

                    await AbstractNavigateToSearchService
                        .SearchProjectsInCurrentProcessAsync(
                            projects,
                            priorityDocuments,
                            searchPattern,
                            kinds.ToImmutableHashSet(),
        ...
        )
        {
            return RunServiceAsync(
                ...
                async solution =>
                {
                    ...
                    await AbstractNavigateToSearchService
                        .SearchGeneratedDocumentsInCurrentProcessAsync(
                            projects,
                            searchPattern,
                            kinds.ToImmutableHashSet(),
                            onItemFound,
        ...
        )
        {
            return RunServiceAsync(
                async cancellationToken =>
                {
                    ...
                    await AbstractNavigateToSearchService
                        .SearchCachedDocumentsInCurrentProcessAsync(
                            storageService,
                            documentKeys,
