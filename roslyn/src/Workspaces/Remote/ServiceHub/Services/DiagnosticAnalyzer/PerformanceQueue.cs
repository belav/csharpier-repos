    ...
    internal class PerformanceQueue
    {
        ...
        )
        {
            ...
            foreach (var assignedAnalyzerNumber in analyzerSet)
            {
                ...
                aggregatedPerformanceDataPerAnalyzer[reverseMap[assignedAnalyzerNumber]] =
                    GetAverageAndAdjustedStandardDeviation(list);
        ...
        private class Snapshot
        {
            ...
            public void Update(
                IEnumerable<(string analyzerId, TimeSpan timeSpan)> rawData,
                int unitCount
            ...
            )
            {
                ...
                {
                    map[assignedAnalyzerNumber] =
                        (timeSpan.TotalMilliseconds - minSpan.TotalMilliseconds) / fileCount;
                }
            }

            private static IEnumerable<(int assignedAnalyzerNumber, TimeSpan timeSpan)> Convert(
                IEnumerable<(string analyzerId, TimeSpan timeSpan)> rawData
            ) =>
                rawData.Select(
                    kv =>
                        (
                            AnalyzerNumberAssigner.Instance.GetUniqueNumber(kv.analyzerId),
                            kv.timeSpan
                        )
                );
        }
