...
namespace Microsoft.CodeAnalysis.Remote
{
            ...
            IRemoteStackTraceExplorerService
    {
        ...
        {
            protected override IRemoteStackTraceExplorerService CreateService(
            ...
            return RunServiceAsync(
                ...
                async solution =>
                {
                    var result = await StackTraceAnalyzer
                        .AnalyzeAsync(frameString, cancellationToken)
                        .ConfigureAwait(false);
                    if (
                        result.ParsedFrames.Length != 1
                        || result.ParsedFrames[0] is not ParsedStackFrame parsedFrame
                    )
                    {
                        throw new InvalidOperationException();
                    }

                    var definition = await StackTraceExplorerUtilities
                        .GetDefinitionAsync(
                            solution,
                            parsedFrame.Root,
                            symbolPart,
