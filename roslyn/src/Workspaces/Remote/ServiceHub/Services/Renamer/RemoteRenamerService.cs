...
namespace Microsoft.CodeAnalysis.Remote
{
    internal sealed class RemoteRenamerService : BrokeredServiceBase, IRemoteRenamerService
    {
        ...
        {
            protected override IRemoteRenamerService CreateService(
                in ServiceConstructionArguments arguments
            ) => new RemoteRenamerService(arguments);
        }

        public RemoteRenamerService(in ServiceConstructionArguments arguments) : base(arguments) { }
        ...
        )
        {
            return RunServiceAsync(
                async cancellationToken =>
                {
                    ...
                    var result = await RenameLocations
                        .FindLocationsAsync(
                            symbol,
                            solution,
                            options.Rehydrate(),
        ...
        )
        {
            return RunServiceAsync(
                async cancellationToken =>
                {
                    ...
                    var rehydratedSet = await RenameLocations
                        .TryRehydrateAsync(solution, renameLocationSet, cancellationToken)
                        .ConfigureAwait(false);
                    if (rehydratedSet == null)
                        return null;

                    var result = await ConflictResolver
        ...
        private static async Task<ImmutableHashSet<ISymbol>?> GetNonConflictSymbolsAsync(
            Solution solution,
            ImmutableArray<SerializableSymbolAndProjectId> nonConflictSymbolIds,
            CancellationToken cancellationToken
        )
        {
            ...
            {
                var symbol = await id.TryRehydrateAsync(solution, cancellationToken)
                    .ConfigureAwait(false);
                if (symbol != null)
                    builder.Add(symbol);
            }
