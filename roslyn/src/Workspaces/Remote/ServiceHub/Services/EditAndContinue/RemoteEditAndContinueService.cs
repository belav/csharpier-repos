...
namespace Microsoft.CodeAnalysis.EditAndContinue
{
            ...
            IRemoteEditAndContinueService
    {
        ...
        {
            protected override IRemoteEditAndContinueService CreateService(
        ...
        private sealed class ManagedEditAndContinueDebuggerService : IManagedHotReloadService
        {
            ...
            > IManagedHotReloadService.GetActiveStatementsAsync(
            ...
            ValueTask<ManagedHotReloadAvailability> IManagedHotReloadService.GetAvailabilityAsync(
            ...
            ValueTask<ImmutableArray<string>> IManagedHotReloadService.GetCapabilitiesAsync(
                CancellationToken cancellationToken
            ) =>
                _callback.InvokeAsync(
                    (callback, cancellationToken) =>
                        callback.GetCapabilitiesAsync(_callbackId, cancellationToken),
                    cancellationToken
                );

            ValueTask IManagedHotReloadService.PrepareModuleForUpdateAsync(
                Guid moduleVersionId,
                CancellationToken cancellationToken
            ) =>
                _callback.InvokeAsync(
        ...
        private sealed class SourceTextProvider : IPdbMatchingSourceTextProvider
        {
            ...
            public ValueTask<string?> TryGetMatchingSourceTextAsync(
                ...
                _callback.InvokeAsync(
                    (callback, cancellationToken) =>
                        callback.TryGetMatchingSourceTextAsync(
                            _callbackId,
                            filePath,
                            requiredChecksum,
                            checksumAlgorithm,
        ...
        )
        {
            return RunServiceAsync(
                ...
                async solution =>
                {
                    var debuggerService = new ManagedEditAndContinueDebuggerService(
                        _callback,
                        callbackId
                    );
                    var sourceTextProvider = new SourceTextProvider(_callback, callbackId);

                    var sessionId = await GetService()
        ...
        /// Remote API.
        /// </summary>
        public ValueTask<ImmutableArray<DocumentId>> BreakStateOrCapabilitiesChangedAsync(
        ...
        /// Remote API.
        /// </summary>
        public ValueTask<ImmutableArray<DiagnosticData>> GetDocumentDiagnosticsAsync(
            Checksum solutionChecksum,
            RemoteServiceCallbackId callbackId,
            ...
            return RunServiceAsync(
                ...
                async solution =>
                {
                    try
                    {
                        ...
                        var diagnostics = await GetService()
                            .GetDocumentDiagnosticsAsync(
                                document,
                                CreateActiveStatementSpanProvider(callbackId),
                                cancellationToken
                            )
                            .ConfigureAwait(false);
                        return diagnostics.SelectAsArray(
                            diagnostic => DiagnosticData.Create(diagnostic, document)
                        );
                    }
                    catch (Exception ex)
        ...
        private static ImmutableArray<DiagnosticData> GetUnexpectedUpdateError(
            Solution solution,
            Exception e
        )
        {
            var descriptor = EditAndContinueDiagnosticDescriptors.GetDescriptor(
                EditAndContinueErrorCode.CannotApplyChangesUnexpectedError
            );
            var diagnostic = Diagnostic.Create(descriptor, Location.None, new[] { e.Message });
            return ImmutableArray.Create(
                DiagnosticData.Create(solution, diagnostic, project: null)
            );
        }
        ...
        )
        {
            return RunServiceAsync(
                ...
                {
                    var document = await solution
                        .GetRequiredTextDocumentAsync(documentId, cancellationToken)
                        .ConfigureAwait(false);
                    return await GetService()
        ...
        /// Remote API.
        /// </summary>
        public ValueTask SetFileLoggingDirectoryAsync(
            string? logDirectory,
            CancellationToken cancellationToken
        )
        {
