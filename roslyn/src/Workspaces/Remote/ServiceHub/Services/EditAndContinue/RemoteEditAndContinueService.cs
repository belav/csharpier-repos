...
namespace Microsoft.CodeAnalysis.EditAndContinue
{
          ...
          IRemoteEditAndContinueService
    {
        ...
        private sealed class ManagedEditAndContinueDebuggerService
            : IManagedEditAndContinueDebuggerService
        {
            ...
            > IManagedEditAndContinueDebuggerService.GetActiveStatementsAsync(
        ...
        private SolutionActiveStatementSpanProvider CreateSolutionActiveStatementSpanProvider(
            RemoteServiceCallbackId callbackId
        ) =>
            new(
                (documentId, cancellationToken) =>
                    _callback.InvokeAsync(
                        (callback, cancellationToken) =>
                            callback.GetSpansAsync(callbackId, documentId, cancellationToken),
                        cancellationToken
                    )
            );

        private DocumentActiveStatementSpanProvider CreateDocumentActiveStatementSpanProvider(
            RemoteServiceCallbackId callbackId
        ) =>
            new(
                cancellationToken =>
                    _callback.InvokeAsync(
                        (callback, cancellationToken) =>
                            callback.GetSpansAsync(callbackId, cancellationToken),
                        cancellationToken
                    )
            );
                ...
                async cancellationToken =>
                {
                    ...
                    var diagnostics = await GetService()
                        .GetDocumentDiagnosticsAsync(
                            document,
                            CreateDocumentActiveStatementSpanProvider(callbackId),
                            cancellationToken
                        )
                        .ConfigureAwait(false);
            ...
            return RunServiceAsync(
                async cancellationToken =>
                {
                    var solution = await GetSolutionAsync(solutionInfo, cancellationToken)
                        .ConfigureAwait(false);
                    return await GetService()
                        .GetBaseActiveStatementSpansAsync(solution, documentIds, cancellationToken)
                        .ConfigureAwait(false);
                },
                cancellationToken
            );
