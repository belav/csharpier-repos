...
namespace Microsoft.CodeAnalysis.Remote
{
    internal sealed class RemoteFindUsagesService : BrokeredServiceBase, IRemoteFindUsagesService
    {
        internal sealed class Factory
            : FactoryBase<IRemoteFindUsagesService, IRemoteFindUsagesService.ICallback>
        {
            protected override IRemoteFindUsagesService CreateService(
        ...
        )
        {
            return RunServiceAsync(
                async cancellationToken =>
                {
                    ...
                    var context = new RemoteFindUsageContext(
                        _callback,
                        callbackId,
                        cancellationToken
                    );
        ...
        )
        {
            return RunServiceAsync(
                async cancellationToken =>
                {
                    ...
                    await AbstractFindUsagesService
                        .FindImplementationsAsync(symbol, project, context)
                        .ConfigureAwait(false);
                },
                cancellationToken
            );
        ...
        private sealed class RemoteFindUsageContext : IFindUsagesContext, IStreamingProgressTracker
        {
            ...
            public ValueTask AddItemsAsync(int count) =>
                _callback.InvokeAsync(
                    (callback, cancellationToken) => callback.AddItemsAsync(_callbackId, count),
                    CancellationToken
                );

            public ValueTask ItemCompletedAsync() =>
                _callback.InvokeAsync(
                    (callback, cancellationToken) => callback.ItemCompletedAsync(_callbackId),
                    CancellationToken
                );
            ...
            public ValueTask ReportMessageAsync(string message) =>
                _callback.InvokeAsync(
                    (callback, cancellationToken) =>
                        callback.ReportMessageAsync(_callbackId, message),
                    CancellationToken
                );
            ...
            public ValueTask ReportProgressAsync(int current, int maximum) =>
                _callback.InvokeAsync(
                    (callback, cancellationToken) =>
                        callback.ReportProgressAsync(_callbackId, current, maximum),
                    CancellationToken
                );
            ...
            public ValueTask SetSearchTitleAsync(string title) =>
                _callback.InvokeAsync(
                    (callback, cancellationToken) =>
                        callback.SetSearchTitleAsync(_callbackId, title),
                    CancellationToken
                );
            ...
            public ValueTask OnDefinitionFoundAsync(DefinitionItem definition)
            {
                ...
                return _callback.InvokeAsync(
                    (callback, cancellationToken) =>
                        callback.OnDefinitionFoundAsync(_callbackId, dehydratedDefinition),
                    CancellationToken
                );
            }
            ...
            public ValueTask OnReferenceFoundAsync(SourceReferenceItem reference)
            {
                ...
                return _callback.InvokeAsync(
                    (callback, cancellationToken) =>
                        callback.OnReferenceFoundAsync(_callbackId, dehydratedReference),
                    CancellationToken
                );
            }
            #endregion
