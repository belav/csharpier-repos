...
namespace Microsoft.CodeAnalysis.MSBuild.Build
{
    internal class ProjectBuildManager
    {
        ...
        ~ProjectBuildManager()
        {
            if (BatchBuildStarted)
            {
                throw new InvalidOperationException(
                    $"{nameof(ProjectBuildManager)}.{nameof(EndBatchBuild)} not called."
                );
            }
        }
        ...
        )
        {
            ...
            var (project, _) = await LoadProjectAsync(path, cancellationToken)
                .ConfigureAwait(false);
        ...
        public void StartBatchBuild(IDictionary<string, string>? globalProperties = null)
        {
            ...
            var allProperties = s_defaultGlobalProperties
                .RemoveRange(globalProperties.Keys)
                .AddRange(globalProperties);
            ...
            _batchBuildProjectCollection = new MSB.Evaluation.ProjectCollection(
                allProperties,
                loggers,
                MSB.Evaluation.ToolsetDefinitionLocations.Default
            );
            ...
            var buildParameters = new MSB.Execution.BuildParameters(_batchBuildProjectCollection)
            {
                ...
                LogTaskInputs =
                    _msbuildLogger is not null
                    && _msbuildLogger.Verbosity == LoggerVerbosity.Diagnostic,
            };
        ...
        )
        {
            ...
            foreach (var target in targets)
            {
                if (!projectInstance.Targets.ContainsKey(target))
                {
                    log.Add(
                        string.Format(
                            WorkspaceMSBuildBuildHostResources.Project_does_not_contain_0_target,
                            target
                        ),
