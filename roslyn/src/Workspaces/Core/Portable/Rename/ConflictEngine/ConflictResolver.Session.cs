...
namespace Microsoft.CodeAnalysis.Rename.ConflictEngine
{
    internal static partial class ConflictResolver
    {
        ...
        {
            // Set of All Locations that will be renamed (does not include non-reference locations that need to
            ...
            )
            {
                ...
                _documentIdOfRenameSymbolDeclaration = renameLocationSet
                    .Solution.GetRequiredDocument(renameSymbolDeclarationLocation.SourceTree!)
                    .Id;
            ...
            private CodeCleanupOptionsProvider FallbackOptions =>
                _renameLocationSet.FallbackOptions;
            ...
            private readonly struct ConflictLocationInfo
            {
                ...
                {
                    Debug.Assert(
                        location.ComplexifiedTargetSpan.Contains(location.ConflictCheckSpan)
                            || location.Type == RelatedLocationType.UnresolvableConflict
                    );
            ...
            // The method which performs rename, resolves the conflict locations and returns the result of the
            // rename operation
            public async Task<MutableConflictResolution> ResolveConflictsAsync()
            {
                try
                {
                    ...
                    var topologicallySortedProjects = dependencyGraph
                        .GetTopologicallySortedProjects(_cancellationToken)
                        .ToList();

                    // Process rename one project at a time to improve caching and reduce syntax tree serialization.
                    var documentsGroupedByTopologicallySortedProjectId =
                        documentsIdsToBeCheckedForConflict
                            .GroupBy(d => d.ProjectId)
                            .OrderBy(g => topologicallySortedProjects.IndexOf(g.Key));

                    _replacementTextValid = IsIdentifierValid_Worker(
                        baseSolution,
                        _replacementText,
                    ...
                    )
                    {
                        ...
                        //      3rd phase is to expand and simplify all the conflict locations (both reference and
                        // non-reference)
                        // If there are unresolved Conflicts after the 3rd phase then in 4th phase,
                        //      We complexify and resolve locations that were resolvable and for the other locations we
                        // perform the normal token replacement like the first the phase.
                        ...
                        for (var phase = 0; phase < 4; phase++)
                        {
                            ...
                            // 1. If renaming a token which is no conflict then replace the token and make a map of the oldspan
                            // to the newspan
                            // 2. If we encounter a node that has to be expanded( because there was a conflict in previous
                            // phase), we expand it.
                            //    If the node happens to contain a token that needs to be renamed then we annotate it and rename
                            // it after expansion else just expand and proceed
                            // 3. Through the whole process we maintain a map of the oldspan to newspan. In case of expansion &
                            // rename, we map the expanded node and the renamed token
                            conflictResolution.UpdateCurrentSolution(
                            ...
                            if (phase == 0)
                            {
                                Contract.ThrowIfTrue(
                                    conflictLocations.Count != 0,
                                    "We're the first phase, so we should have no conflict locations yet"
                                );
                                ...
                                // If there were no conflicting locations in references, then the first conflict phase has to be
                                // skipped.
                            ...
                            // Note that we need to get the conflictLocations here since we're going to remove some locations
                            // below if phase == 2
                            documentIdsThatGetsAnnotatedAndRenamed = new HashSet<DocumentId>(
                                conflictLocations.Select(l => l.DocumentId)
                            );

                            if (phase == 2)
                            {
                                // After phase 2, if there are still conflicts then remove the conflict locations from being
                                // expanded
                        ...
                        intermediateSolution = await conflictResolution
                            .RemoveAllRenameAnnotationsAsync(
                    ...
                    // This rename could break implicit references of this symbol (e.g. rename MoveNext on a collection
                    // like type in a
...
#if DEBUG
                    await DebugVerifyNoErrorsAsync(
                            conflictResolution,
                            documentsIdsToBeCheckedForConflict
                        )
                        .ConfigureAwait(false);
                    ...
                    when (FatalError.ReportAndPropagateUnlessCanceled(e, ErrorSeverity.Critical))
                {
...
#if DEBUG
            private async Task DebugVerifyNoErrorsAsync(
                MutableConflictResolution conflictResolution,
                IEnumerable<DocumentId> documents
            )
            {
                var documentIdErrorStateLookup = new Dictionary<DocumentId, bool>();

                // we only check for the documentIds we add annotations to, which is a subset of the ones we're
                // going
                ...
                foreach (var documentId in documents)
                {
                    ...
                    documentIdErrorStateLookup.Add(
                        documentId,
                        await originalDoc
                            .HasAnyErrorsAsync(_cancellationToken)
                            .ConfigureAwait(false)
                    );
                }
                ...
                )
                {
                    foreach (var documentId in documents)
                    {
                        ...
                        // errors.
                        if (
                            !documentIdErrorStateLookup[documentId]
                            && _nonConflictSymbolKeys.IsDefault
                        )
                        {
                            await conflictResolution
                ...
                try
                {
                    ...
                    if (!IsRenameValid(conflictResolution, renamedSymbolInNewSolution))
                    {
                        foreach (var documentId in documentIdsForConflictResolution)
                        {
                            ...
                            var nodesOrTokensWithConflictCheckAnnotations =
                                GetNodesOrTokensToCheckForConflicts(syntaxRoot);
                            foreach (
                                var (
                                    syntax,
                                    annotation
                                ) in nodesOrTokensWithConflictCheckAnnotations
                            )
                            {
                    ...
                    foreach (var documentId in documentIdsForConflictResolution)
                    {
                        ...
                        var syntaxFactsService =
                            newDocument.Project.Services.GetRequiredService<ISyntaxFactsService>();
                        ...
                        foreach (
                            var (syntax, annotation) in nodesOrTokensWithConflictCheckAnnotations
                        )
                        {
                            var tokenOrNode = syntax;
                            var conflictAnnotation = annotation;
                            reverseMappedLocations[tokenOrNode.GetLocation()!] =
                            ...
                            var hasConflict = _renameAnnotations.HasAnnotation(
                                tokenOrNode,
                                RenameInvalidIdentifierAnnotation.Instance
                            );
                            if (!hasConflict)
                            {
                                newDocumentSemanticModel ??= await newDocument
                                    .GetRequiredSemanticModelAsync(_cancellationToken)
                                    .ConfigureAwait(false);
                                newReferencedSymbols = GetSymbolsInNewSolution(
                                    newDocument,
                                ...
                                if (!hasConflict && !conflictAnnotation.IsInvocationExpression)
                                    hasConflict = LocalVariableConflictPerLanguage(
                            ...
                            if (!hasConflict)
                            {
                                ...
                                {
                                    // if a complexified location was not a reference location, then it was a resolved conflict of a non
                                    // reference location
                                    if (
                            ...
                            {
                                var baseToken = baseRoot.FindToken(
                                    conflictAnnotation.OriginalSpan.Start,
                                    true
                                );
                                var complexifiedTarget = GetExpansionTargetForLocationPerLanguage(
                                    baseToken,
                                    baseDocument
                                );
                                conflictResolution.AddRelatedLocation(
                    ...
                    // there are more conflicts that cannot be identified by checking if the tokens still reference the
                    // same
                    // symbol. These conflicts are mostly language specific. A good example is a member with the same
                    // name
                    ...
                    if (_documentIdOfRenameSymbolDeclaration.ProjectId == projectId)
                    {
                        ...
                        foreach (
                            var unprocessedDocumentIdWithPotentialDeclarationConflicts in allDocumentIdsInProject.Where(
                                d => !documentIdsForConflictResolution.Contains(d)
                            )
                        )
                        {
                            var newDocument =
            ...
            private async Task<ImmutableHashSet<ISymbol>?> GetNonConflictSymbolsAsync(
            ...
            )
            {
                ...
                var compilation = await currentProject
                    .GetRequiredCompilationAsync(_cancellationToken)
                    .ConfigureAwait(false);
                return ImmutableHashSet.CreateRange(
                    _nonConflictSymbolKeys
                        .Select(s => s.Resolve(compilation).GetAnySymbol())
                        .WhereNotNull()
                );
            }
            ...
            private IEnumerable<(
                ...
                RenameActionAnnotation annotation
                ...
                try
                {
                    ...
                    else if (conflictAnnotation.IsMemberGroupReference)
                    {
                        ...
                        else
                        {
                            ...
                            foreach (
                                var originalReference in conflictAnnotation.RenameDeclarationLocationReferences.Where(
                                    loc => loc.IsSourceLocation
                                )
                            )
                            {
                                var adjustedStartPosition =
                                    conflictResolution.GetAdjustedTokenStartingPosition(
                                        originalReference.TextSpan.Start,
                        ...
                        !conflictAnnotation.IsRenameLocation
                        && conflictAnnotation.IsOriginalTextLocation
                        && conflictAnnotation.RenameDeclarationLocationReferences.Length > 1
                        && newReferencedSymbols.Length == 1
                    )
                    {
                        ...
                        newReferencedSymbols.Length
                        != conflictAnnotation.RenameDeclarationLocationReferences.Length
                    )
                    {
                        // Don't show conflicts for errors in the old solution that now bind in the new solution.
                        if (
                            newReferencedSymbols.Length != 0
                            && conflictAnnotation.RenameDeclarationLocationReferences.Length == 0
                        )
                        {
                            hasConflict = false;
                        }
                    ...
                    else
                    {
                        ...
                        foreach (var symbol in newReferencedSymbols)
                        {
                            ...
                            )
                            {
                                ...
                                var renameDeclarationLocationReference =
                                    conflictAnnotation.RenameDeclarationLocationReferences[
                                        symbolIndex
                                    ];
                                var newAdjustedStartPosition =
                                    conflictResolution.GetAdjustedTokenStartingPosition(
                                        renameDeclarationLocationReference.TextSpan.Start,
                                        renameDeclarationLocationReference.DocumentId
                                    );
                                ...
                                )
                                {
                                    var overridingSymbol = await SymbolFinder
                                        .FindSymbolAtPositionAsync(
                                            solution.GetRequiredDocument(newLocation.SourceTree),
                                            newLocation.SourceSpan.Start,
                                            cancellationToken: _cancellationToken
                                        )
                                        .ConfigureAwait(false);
                                    if (
                                        overridingSymbol != null
                                        && !Equals(renamedSymbolInNewSolution, overridingSymbol)
                                    )
                                    {
            ...
            private ImmutableArray<ISymbol> GetSymbolsInNewSolution(
                Document newDocument,
                SemanticModel newDocumentSemanticModel,
                RenameActionAnnotation conflictAnnotation,
                SyntaxNodeOrToken tokenOrNode
            )
            {
                var newReferencedSymbols = RenameUtilities.GetSymbolsTouchingPosition(
                    tokenOrNode.Span.Start,
                    newDocumentSemanticModel,
                    newDocument.Project.Solution.Services,
                    _cancellationToken
                );
                ...
                if (conflictAnnotation.IsInvocationExpression)
                {
                    ...
                    {
                        var invocationReferencedSymbols =
                            SymbolsForEnclosingInvocationExpressionWorker(
                                (SyntaxNode)tokenOrNode!,
                                newDocumentSemanticModel,
                                _cancellationToken
                            );
            ...
            /// <summary>
            /// The method determines the set of documents that need to be processed for Rename and also
            // determines
            ...
            )> FindDocumentsAndPossibleNameConflictsAsync()
            {
                try
                {
                    ...
                    var allRenamedDocuments = _renameLocationSet
                        .Locations.Select(loc => loc.Location.SourceTree!)
                        .Distinct()
                    ...
                    var documentsFromAffectedProjects =
                        RenameUtilities.GetDocumentsAffectedByRename(
                            symbol,
                            solution,
                            _renameLocationSet.Locations
                        );
                    foreach (
                        var language in documentsFromAffectedProjects
                            .Select(d => d.Project.Language)
                            .Distinct()
                    )
                    {
                        solution
                ...
                try
                {
                    foreach (var document in documents)
                    {
                        ...
                        var info = await SyntaxTreeIndex
                            .GetRequiredIndexAsync(document, _cancellationToken)
                            .ConfigureAwait(false);
                ...
                try
                {
                    foreach (var documentId in documentIdsToRename.ToList())
                    {
                        ...
                        var semanticModel = await document
                            .GetRequiredSemanticModelAsync(_cancellationToken)
                            .ConfigureAwait(false);
                        var originalSyntaxRoot = await semanticModel
                            .SyntaxTree.GetRootAsync(_cancellationToken)
                            .ConfigureAwait(false);
                        ...
                        // Annotate all nodes with a RenameLocation annotations to record old locations & old referenced
                        // symbols.
                        // Also annotate nodes that should get complexified (nodes for rename locations + conflict
                        // locations)
                        ...
                        {
                            // Update the list for the current phase, some files with strings containing the original or
                            // replacement
                        ...
                        else
                        {
                            partiallyRenamedSolution =
                                partiallyRenamedSolution.WithDocumentSyntaxRoot(
                                    documentId,
                                    newRoot,
            ...
            private static bool ShouldIncludeLocation(
                ImmutableArray<RenameLocation> renameLocations,
            ...
            /// <summary>
            /// We try to compute the sub-spans to rename within the given <paramref
            // name="containingLocationForStringOrComment"/>.
            /// If we are renaming within a string, the locations to rename are always within this containing
            // string location
            ...
            /// so we return null and the rename rewriter will perform a complete regex match within comment
            // trivia
            ...
            )
            {
                ...
                {
                    if (
                        !containingLocationForStringOrComment.Contains(
                            renameLocation.Location.SourceSpan
                        )
                    )
                    {
                    ...
                    var offset =
                        renameLocation.Location.SourceSpan.Start
                        - containingLocationForStringOrComment.Start;
