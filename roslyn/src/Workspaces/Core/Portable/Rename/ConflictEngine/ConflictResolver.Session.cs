        ...
        private class Session
        {
            ...
            )
            {
                ...
                _documentIdOfRenameSymbolDeclaration = renameLocationSet.Solution
                    .GetRequiredDocument(renameSymbolDeclarationLocation.SourceTree!)
                    .Id;
            ...
            public async Task<MutableConflictResolution> ResolveConflictsAsync()
            {
                try
                {
                    ...
                    )
                    {
                        ...
                        var cacheService =
                            baseSolution.Services.GetService<IProjectCacheHostService>();
                        using (cacheService?.EnableCaching(documentsByProject.Key))
                        {
                            ...
                            for (var phase = 0; phase < 4; phase++)
                            {
                                ...
                                if (phase == 1)
                                {
                                    _conflictLocations = _conflictLocations
                                        .Concat(
            ...
            )
            {
                try
                {
                    ...
                    foreach (var documentId in documentIdsForConflictResolution)
                    {
                        ...
                        var complexifiedLocationSpanForThisDocument = _conflictLocations
            ...
            private async Task FindDocumentsAndPossibleNameConflictsAsync()
            {
                try
                {
                    ...
                    _topologicallySortedProjects = dependencyGraph
