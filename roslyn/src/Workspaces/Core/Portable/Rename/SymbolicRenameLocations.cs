...
namespace Microsoft.CodeAnalysis.Rename
{
    ...
    internal sealed partial class SymbolicRenameLocations
    {
        ...
        {
            Debug.Assert(
                locations.Distinct().Count() == locations.Length,
        ...
        )
        {
            ...
            using (Logger.LogBlock(FunctionId.Rename_AllRenameLocations, cancellationToken))
            {
                ...
                var overloadsResult = options.RenameOverloads
                    ? await GetOverloadsAsync(symbol, solution, cancellationToken)
                        .ConfigureAwait(false)
                ...
                var renameMethodGroupReferences =
                    options.RenameOverloads || !RenameUtilities.GetOverloadedSymbols(symbol).Any();
                foreach (var result in overloadsResult.Concat(originalSymbolResult))
                {
                    mergedLocations.AddRange(
                        renameMethodGroupReferences
                            ? result.Locations
                            : result.Locations.Where(x =>
                                x.CandidateReason != CandidateReason.MemberGroup
                            )
                    );
        ...
        )
        {
            ...
            foreach (var overloadedSymbol in RenameUtilities.GetOverloadedSymbols(symbol))
                overloadsResult.Add(
                    await AddLocationsReferenceSymbolsAsync(
                            overloadedSymbol,
                            solution,
                            cancellationToken
                        )
        ...
        )
        {
            ...
            var referenceSymbols = await SymbolFinder
            ...
            foreach (var referencedSymbol in referenceSymbols)
            {
                ...
                locations.AddAll(
                    await referencedSymbol
                        .Locations.SelectManyInParallelAsync(
                            (l, c) =>
            ...
            var implicitLocations = referenceSymbols
                .SelectMany(refSym => refSym.Locations)
                .Where(loc => loc.IsImplicit)
                .ToImmutableArray();
            var referencedSymbols = referenceSymbols
                .Select(r => r.Definition)
                .Where(r => !r.Equals(symbol))
                .ToImmutableArray();
