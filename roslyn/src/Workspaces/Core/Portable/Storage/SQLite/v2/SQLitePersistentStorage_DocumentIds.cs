...
namespace Microsoft.CodeAnalysis.SQLite.v2
{
    internal partial class SQLitePersistentStorage
    {
        ...
        private readonly ConcurrentDictionary<
            DocumentId,
            DocumentPrimaryKey
        > _documentIdToPrimaryKeyMap = new();
            ...
            if (!_documentIdToPrimaryKeyMap.TryGetValue(documentKey.Id, out var existingId))
            {
                ...
                var documentFolder =
                    ...
                    && PathUtilities.GetDirectoryName(
                        PathUtilities.GetRelativePath(_solutionDirectory, documentKey.FilePath)
                    )
                ...
                if (
                    TryGetProjectPrimaryKey(connection, documentKey.Project, allowWrite)
                        is not ProjectPrimaryKey projectPrimaryKey
                    || TryGetStringId(connection, documentFolder, allowWrite)
                        is not int documentFolderId
                    || TryGetStringId(connection, documentKey.Name, allowWrite)
                        is not int documentNameId
                )
                {
                    return null;
                }
                ...
                existingId = new DocumentPrimaryKey(
                    projectPrimaryKey,
                    documentFolderId,
                    documentNameId
                );
