    ...
    internal partial class SQLitePersistentStorage
    {
            ...
            where TDatabaseKey : struct
        {
            ...
            )
            {
                ...
                _primaryKeyColumns = primaryKeysArray
                    .ToImmutableArray()
                    .Add((DataNameIdColumnName, SQLiteIntegerType));
                _allColumns = _primaryKeyColumns
                    .Add((ChecksumColumnName, SQLiteBlobType))
                    .Add((DataColumnName, SQLiteBlobType));
                ...
                _insert_or_replace_into_writecache_table_values_0primarykey_1checksum_2data = $"""
                    ...
                    ({string.Join(",", _allColumns.Select(c => c.name))}) values ({string.Join(
                        ",",
                        _allColumns.Select(n => "?")
                    )})
                    """;
            ...
            public Task<bool> ChecksumMatchesAsync(
                ...
                Storage.PerformReadAsync(
                    static t =>
                        t.self.ChecksumMatches(t.key, t.name, t.checksum, t.cancellationToken),
                    (self: this, name, key, checksum, cancellationToken),
                    cancellationToken
                );
            ...
            public Task<Stream?> ReadStreamAsync(
                ...
                Storage.PerformReadAsync(
                    ...
                    (self: this, key, name, checksum, cancellationToken),
                    cancellationToken
                );
            ...
            )
            {
                ...
                if (!Storage._shutdownTokenSource.IsCancellationRequested)
                {
                    ...
                    if (
                        TryGetDatabaseKey(connection, key, allowWrite: false)
                            is TDatabaseKey databaseKey
                        && Storage.TryGetStringId(connection, name, allowWrite: false)
                            is int dataNameId
                    )
                    {
                        try
                ...
                )
                {
                    ...
                    // safety issue here.
                    return TryGetActualRowIdFromDatabase(
                        connection,
                        database,
                        databaseKey,
                        dataNameID,
                        out var writeCacheRowId
                    )
            ...
            )
            {
                ...
                if (!Storage._shutdownTokenSource.IsCancellationRequested)
                {
                    ...
                    if (
                        TryGetDatabaseKey(connection, key, allowWrite: true)
                            is TDatabaseKey databaseKey
                        && Storage.TryGetStringId(connection, dataName, allowWrite: true)
                            is int dataNameId
                    )
                    {
            ...
            )
            {
                ...
                )
                {
                    ...
                    BindPrimaryKey(statement, databaseKey, dataNameId);
                    statement.BindBlobParameter(
                        parameterIndex: _primaryKeyColumns.Length + 1,
                        checksumBytes
                    );
                    statement.BindBlobParameter(
                        parameterIndex: _primaryKeyColumns.Length + 2,
                        dataBytes
                    );
