...
namespace Microsoft.CodeAnalysis.FindSymbols
{
    public static partial class SymbolFinder
    {
        ...
        internal sealed class FindReferencesServerCallback
        {
            ...
            public async ValueTask OnFindInDocumentStartedAsync(
                DocumentId documentId,
                CancellationToken cancellationToken
            )
            {
                var document = await _solution
                ...
                await _progress
                    .OnFindInDocumentStartedAsync(document, cancellationToken)
                    .ConfigureAwait(false);
            }

            public async ValueTask OnFindInDocumentCompletedAsync(
                DocumentId documentId,
                CancellationToken cancellationToken
            )
            {
                var document = await _solution
                    ...
                    .ConfigureAwait(false);
                await _progress
                    .OnFindInDocumentCompletedAsync(document, cancellationToken)
                    .ConfigureAwait(false);
            }
