...
namespace Microsoft.CodeAnalysis.FindSymbols
{
    ...
    /// </param>
    /// <param name="Explicit">
    /// Whether or not this find ref operation was explicitly invoked or not.  If explicit invoked, the
    // find
    ...
    /// When cascading if we should only travel in a consistent direction away from the starting symbol.
    // For
    /// example, starting on a virtual method, this would cascade upwards to implemented interface
    // methods, and
    /// downwards to overridden methods.  However, it would not then travel back down to other
    // implementations of
    /// those interface methods.  This is useful for cases where the client only wants references that
    // could lead to
    /// this symbol actually being called into at runtime.
    ///
    /// There are cases where a client will not want this behavior.  An example of that is 'Rename'.  In
    // rename,
    /// there is a implicit link between members in a hierarchy with the same name (and appropriate
    // signature).  For example, in:
    ///
    /// <code>
    ...
    /// </code>
    ///
    /// If <c>C1.Goo</c> is renamed, this will need to rename <c>C2.Goo</c> as well to keep the code
    // properly
    /// compiling.  So, by default 'Rename' will cascade to all of these so it can appropriately update
    // them.  This
    /// option is the more relevant with knowing if a particular reference would actually result in a
    // call to the
    ...
    )
    {
        ...
        /// <summary>
        /// Returns the appropriate options for a given symbol for the specific 'Find References' feature.
        // This should
        /// not be used for other features (like 'Rename').  For the 'Find References' feature, if the user
        // starts
        /// searching on an accessor, then we want to give results associated with the specific accessor.
        // Otherwise, if
        /// they search on a property, then associate everything with the property.  We also only want to
        // travel an
        /// inheritance hierarchy unidirectionally so that we only see potential references that could
        // actually reach
