...
{
    internal sealed class PropertySymbolReferenceFinder
        : AbstractMethodOrPropertyOrEventSymbolReferenceFinder<IPropertySymbol>
    {
        protected override bool CanFind(IPropertySymbol symbol) => true;
        ...
        private static void CascadeToBackingFields(
            IPropertySymbol symbol,
        ...
        )
        {
            ...
            {
                if (member is IFieldSymbol field && symbol.Equals(field.AssociatedSymbol))
                {
                    result.Add(field);
                }
            }
        ...
        private static void CascadeToPrimaryConstructorParameters(
        ...
        )
        {
            if (
                property is
                {
                    ...
                    ContainingType:
                    { IsRecord: true, DeclaringSyntaxReferences.Length: > 0 } containingType,
                }
            )
            {
                ...
                foreach (var constructor in containingType.Constructors)
                {
                    if (constructor.DeclaringSyntaxReferences.Length > 0)
                    {
                        ...
                        if (containingTypeSyntaxes.Contains(constructorSyntax))
                        {
                            ...
                            {
                                if (
                                    property.Name.Equals(parameter.Name)
                                    && property.Equals(
                                        parameter.GetAssociatedSynthesizedRecordProperty(
                                            cancellationToken
                                        )
                                    )
                                )
        ...
        )
        {
            ...
            var elementAccessDocument = symbol.IsIndexer
                ? await FindDocumentWithExplicitOrImplicitElementAccessExpressionsAsync(
                        project,
                        documents,
                        cancellationToken
                    )
            ...
            return ordinaryDocuments.Concat(
                forEachDocuments,
                elementAccessDocument,
                indexerMemberCrefDocument,
        ...
        )
        {
            ...
            var forEachReferences = IsForEachProperty(symbol)
                ? await FindReferencesInForEachStatementsAsync(symbol, state, cancellationToken)
                    .ConfigureAwait(false)
                : ImmutableArray<FinderLocation>.Empty;

            var indexerReferences = symbol.IsIndexer
                ? await FindIndexerReferencesAsync(symbol, state, options, cancellationToken)
                    .ConfigureAwait(false)
                : ImmutableArray<FinderLocation>.Empty;
        ...
        private static Task<
            ImmutableArray<Document>
        > FindDocumentWithExplicitOrImplicitElementAccessExpressionsAsync(
            Project project,
            IImmutableSet<Document>? documents,
            CancellationToken cancellationToken
        )
        {
            return FindDocumentsWithPredicateAsync(
                project,
                documents,
                static index => index.ContainsExplicitOrImplicitElementAccessExpression,
                cancellationToken
            );
        }
        ...
        )
        {
            return FindDocumentsWithPredicateAsync(
                project,
                documents,
                static index => index.ContainsIndexerMemberCref,
                cancellationToken
            );
        }
        ...
        )
        {
            ...
            foreach (var node in indexerReferenceExpressions)
            {
                ...
                var (matched, candidateReason, indexerReference) =
                    await ComputeIndexerInformationAsync(symbol, state, node, cancellationToken)
                        .ConfigureAwait(false);
                if (!matched)
                    continue;
        ...
        )> ComputeIndexerInformationAsync(
            IPropertySymbol symbol,
            FindReferencesDocumentState state,
            SyntaxNode node,
            CancellationToken cancellationToken
        )
        {
            ...
            {
                return ComputeImplicitElementAccessInformationAsync(
                    symbol,
                    node,
                    state,
                    cancellationToken
        ...
        private static async ValueTask<(
            ...
            CandidateReason reason,
            SyntaxNode indexerReference
        )> ComputeIndexerMemberCRefInformationAsync(
            IPropertySymbol symbol,
            FindReferencesDocumentState state,
            SyntaxNode node,
            CancellationToken cancellationToken
        ...
        {
            var (matched, reason) = await SymbolsMatchAsync(symbol, state, node, cancellationToken)
                .ConfigureAwait(false);
        ...
        private static async ValueTask<(
            ...
            CandidateReason reason,
            SyntaxNode indexerReference
        )> ComputeConditionalAccessInformationAsync(
            IPropertySymbol symbol,
            SyntaxNode node,
            FindReferencesDocumentState state,
            CancellationToken cancellationToken
        ...
        private static async ValueTask<(
            ...
            CandidateReason reason,
            SyntaxNode? indexerReference
        )> ComputeElementAccessInformationAsync(
            IPropertySymbol symbol,
            SyntaxNode node,
            FindReferencesDocumentState state,
            CancellationToken cancellationToken
        )
        {
            ...
            if (
                expression != null
                && (
                    await SymbolsMatchAsync(symbol, state, expression, cancellationToken)
                        .ConfigureAwait(false)
                ).matched
            )
            {
        ...
        private static async ValueTask<(
            ...
            CandidateReason reason,
            SyntaxNode indexerReference
        )> ComputeImplicitElementAccessInformationAsync(
            IPropertySymbol symbol,
            SyntaxNode node,
            FindReferencesDocumentState state,
            CancellationToken cancellationToken
