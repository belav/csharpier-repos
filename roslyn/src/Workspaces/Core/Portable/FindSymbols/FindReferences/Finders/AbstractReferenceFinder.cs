    ...
    internal abstract partial class AbstractReferenceFinder : IReferenceFinder
    {
        ...
        )
        {
            return FindDocumentsWithPredicateAsync(
                project,
                documents,
                static index => index.ContainsGlobalSuppressMessageAttribute,
                cancellationToken
            );
        }
        ...
        public static ValueTask<ImmutableArray<SyntaxToken>> FindMatchingIdentifierTokensAsync(
            FindReferencesDocumentState state,
            string identifier,
            CancellationToken cancellationToken
        ) =>
        ...
        )
        {
            ...
            foreach (var token in tokens)
            {
                ...
                if (matched)
                {
                    var finderLocation = CreateFinderLocation(
                        state,
                        token,
                        reason,
                        cancellationToken
        ...
        public static ReferenceLocation CreateReferenceLocation(
            FindReferencesDocumentState state,
            SyntaxToken token,
            CandidateReason reason,
            CancellationToken cancellationToken
        ...
        protected static Task<ImmutableArray<Document>> FindDocumentsWithForEachStatementsAsync(
            ...
            FindDocumentsWithPredicateAsync(
                project,
                documents,
                static index => index.ContainsForEachStatement,
                cancellationToken
            );
        ...
        )
        {
            ...
            )
            {
                ...
                var expressions = state.SyntaxFacts
                    .GetExpressionsOfObjectCollectionInitializer(node);
                foreach (var expression in expressions)
                {
                    var info = state.SemanticFacts
        ...
        )
        {
            ...
            {
                var awaitExpressionMethod = state.SemanticFacts
