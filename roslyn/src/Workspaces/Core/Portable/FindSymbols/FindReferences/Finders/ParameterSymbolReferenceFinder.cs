...
namespace Microsoft.CodeAnalysis.FindSymbols.Finders
{
    ...
    {
        protected override bool CanFind(IParameterSymbol symbol) => true;
        ...
        )
        {
            ...
            CascadeBetweenPrimaryConstructorParameterAndProperties(
                parameter,
                symbols,
                cancellationToken
            );
        ...
        private static void CascadeBetweenAnonymousDelegateParameters(
            IParameterSymbol parameter,
                    ...
                    if (document != null)
                    {
                        ...
                        if (semanticFacts.ExposesAnonymousFunctionParameterNames)
                        {
                            ...
                            var lambdaNode = parameter
                                .ContainingSymbol.DeclaringSyntaxReferences.Select(r =>
                                    r.GetSyntax(cancellationToken)
                                )
                                .First();
        ...
        private static SyntaxNode? GetContainer(
            SemanticModel semanticModel,
        ...
        )
        {
            for (var current = parameterNode; current != null; current = current.Parent)
            {
                ...
                if (
                    declaredSymbol is IMethodSymbol { MethodKind: not MethodKind.AnonymousFunction }
                )
                    return current;
            }
            ...
            if (parameter.ContainingSymbol is IMethodSymbol containingMethod)
            {
                ...
                if (containingType.IsDelegateType())
                {
                    if (containingMethod.MethodKind == MethodKind.DelegateInvoke)
                    {
                        ...
                        var beginInvokeMethod = containingType
                    ...
                    )
                    {
                        ...
                        AddParameterAtIndex(
                            results,
                            ordinal,
                            containingType.DelegateInvokeMethod?.Parameters
                        );
                    }
                }
            }
