...
{
    internal sealed class ConstructorInitializerSymbolReferenceFinder
        : AbstractReferenceFinder<IMethodSymbol>
    {
        ...
        )
        {
            return FindDocumentsAsync(
                project,
                documents,
                static async (document, name, cancellationToken) =>
                {
                    ...
                    if (index.ProbablyContainsIdentifier(name))
                    {
                        if (index.ContainsThisConstructorInitializer)
                        {
                            ...
                            document.Project.Language == LanguageNames.VisualBasic
        ...
        )
        {
            ...
            if (state.SemanticModel.Language == LanguageNames.VisualBasic)
            {
                tokens = tokens
                    .Concat(
                        await FindMatchingIdentifierTokensAsync(state, "New", cancellationToken)
                            .ConfigureAwait(false)
                    )
                    .Distinct();
            }
            ...
            var totalTokens = tokens.WhereAsArray(
                static (token, tuple) =>
                    TokensMatch(
                        tuple.state,
                        token,
                        tuple.methodSymbol.ContainingType.Name,
