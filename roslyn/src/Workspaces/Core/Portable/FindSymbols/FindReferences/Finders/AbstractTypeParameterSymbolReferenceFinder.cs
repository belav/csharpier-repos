...
{
    internal abstract class AbstractTypeParameterSymbolReferenceFinder
        : AbstractReferenceFinder<ITypeParameterSymbol>
    {
        ...
        )
        {
            ...
            var normalReferences = await FindReferencesInTokensAsync(
                    ...
                    state,
                    tokens.WhereAsArray(
                        static (token, state) => !IsObjectCreationToken(token, state),
                        state
                    ),
                    cancellationToken
                )
            ...
            ImmutableArray<FinderLocation> GetObjectCreationReferences(
