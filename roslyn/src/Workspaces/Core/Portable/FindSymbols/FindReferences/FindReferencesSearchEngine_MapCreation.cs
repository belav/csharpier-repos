...
namespace Microsoft.CodeAnalysis.FindSymbols
{
    ...
    internal partial class FindReferencesSearchEngine
    {
        ...
        )
        {
            ...
            )
            {
                ...
                foreach (var (documents, group, symbol, finder) in results)
                {
                    ...
                    {
                        finalMap
                            .GetOrAdd(document.Project, s_createDocumentMap)
                ...
                foreach (var (project, documentMap) in finalMap)
                {
                    Contract.ThrowIfTrue(
                        documentMap.Any(kvp1 => kvp1.Value.Count != kvp1.Value.ToSet().Count)
                    );
                }
#endif
        ...
        )
        {
            ...
            var finalDocs = documents
                .WhereNotNull()
                .Distinct()
                .Where(d => _documents == null || _documents.Contains(d))
                .ToImmutableArray();
        ...
        )
        {
            ...
            )
            {
                ...
                Contract.ThrowIfTrue(
                    projectMap.Any(kvp => kvp.Value.Count != kvp.Value.ToSet().Count)
                );
                return projectMap;
            }
        }
        ...
        )
        {
            ...
            var sourceSymbol = await SymbolFinder
                .FindSourceDefinitionAsync(searchSymbol, _solution, _cancellationToken)
                .ConfigureAwait(false);
            ...
            var group = await DetermineSymbolGroupAsync(searchSymbol).ConfigureAwait(false);
            if (result.Add(group))
            {
                await _progress.OnDefinitionFoundAsync(group).ConfigureAwait(false);
                        ...
                        Task.Factory
                            .StartNew(
                                async () =>
                                {
                                    ...
                                    var symbolProject = _solution.GetProject(
        ...
        private async Task<SymbolGroup> DetermineSymbolGroupAsync(ISymbol searchSymbol)
        {
            ...
            return new SymbolGroup(
                await SymbolFinder
                    .FindLinkedSymbolsAsync(searchSymbol, _solution, _cancellationToken)
                    .ConfigureAwait(false)
            );
        }
            ...
            if (!symbols.IsDefault)
            {
                foreach (var (symbol, cascadeDirection) in symbols)
                {
                    ...
                    symbolTasks.Add(
                        Task.Factory
                            .StartNew(
                                () => DetermineAllSymbolsCoreAsync(symbol, finalDirection, result),
                                _cancellationToken,
                                TaskCreationOptions.None,
                                _scheduler
                            )
