...
namespace Microsoft.CodeAnalysis.FindSymbols
{
    ...
    internal static partial class DependentTypeFinder
    {
        ...
        private static readonly Func<Location, bool> s_isInMetadata = static loc =>
            loc.IsInMetadata;
        private static readonly Func<Location, bool> s_isInSource = static loc => loc.IsInSource;

        private static readonly Func<INamedTypeSymbol, bool> s_isInterface = static t =>
            t is { TypeKind: TypeKind.Interface };
        private static readonly Func<INamedTypeSymbol, bool> s_isNonSealedClass = static t =>
            t is { TypeKind: TypeKind.Class, IsSealed: false };
        private static readonly Func<INamedTypeSymbol, bool> s_isInterfaceOrNonSealedClass =
            static t => s_isInterface(t) || s_isNonSealedClass(t);

        private static readonly ObjectPool<PooledHashSet<INamedTypeSymbol>> s_symbolSetPool =
        ...
        )
        {
            ...
            async Task DescendInheritanceTreeInProjectAsync(Project project)
            {
                ...
                if (searchInMetadata)
                {
                    ...
                    await AddDescendantMetadataTypesInProjectAsync(tempBuffer, project)
                        .ConfigureAwait(false);

                    // Add all the matches we found to the result set.
                    AssertContents(
                        tempBuffer,
                        assert: s_isInMetadata,
                        "Found type was not from metadata"
                    );
...

                {
                    ...
                    await AddDescendantSourceTypesInProjectAsync(tempBuffer, project)
                        .ConfigureAwait(false);
                    ...
                    AssertContents(
                        tempBuffer,
                        assert: s_isInSource,
                        "Found type was not from source"
                    );
                    AddRange(tempBuffer, result);
            ...
            async Task AddDescendantSourceTypesInProjectAsync(SymbolSet result, Project project)
            {
                ...
                var projectIndex = await ProjectIndex
                    .GetIndexAsync(project, cancellationToken)
                    .ConfigureAwait(false);
                ...
                while (typesToSearchFor.Count > 0)
                {
                    foreach (var type in typesToSearchFor)
                    {
                        ...
                        await AddSourceTypesThatDeriveFromNameAsync(tempBuffer, type.Name)
                            .ConfigureAwait(false);
                    }
                ...
                )
                {
                    foreach (var (documentId, infos) in documentToInfos)
                    {
                        ...
                        var semanticModel = await GetRequiredSemanticModelAsync(documentId)
                            .ConfigureAwait(false);
                        foreach (var info in infos)
                        {
                            ...
                            if (
                                info.TryResolve(semanticModel, cancellationToken)
            ...
            if (searchInMetadata)
            {
                ...
                var projectsThatReferenceMetadataAssembly = await DependentProjectsFinder
                    .GetDependentProjectsAsync(
                        solution,
                        ImmutableArray.Create<ISymbol>(type),
        ...
        )
        {
            ...
            return projectsToExamine
                .OrderBy((p1, p2) => order[p1.Id] - order[p2.Id])
                .ToImmutableArray();
        }
