    ...
    internal partial class FindReferencesSearchEngine
    {
        ...
        )
        {
            ...
            try
            {
                ...
                var projectsToSearch = await GetProjectsToSearchAsync(allSymbols, cancellationToken)
                    .ConfigureAwait(false);
                ...
                await _progressTracker
                    .AddItemsAsync(projectsToSearch.Length, cancellationToken)
                    .ConfigureAwait(false);
                ...
                )
                {
                    ...
                    // below.
                    await symbolSet
                        .InheritanceCascadeAsync(currentProject, cancellationToken)
                        .ConfigureAwait(false);
        ...
        /// </summary>
        private async Task ReportGroupsAsync(
            ImmutableArray<ISymbol> symbols,
            CancellationToken cancellationToken
        )
        {
        ...
        private async ValueTask<SymbolGroup> ReportGroupAsync(
            ISymbol symbol,
            CancellationToken cancellationToken
        )
        {
            ...
            if (!_symbolToGroup.TryGetValue(symbol, out var group))
            {
                var linkedSymbols = await SymbolFinder
                    .FindLinkedSymbolsAsync(symbol, _solution, cancellationToken)
                    .ConfigureAwait(false);
                Contract.ThrowIfFalse(
                    linkedSymbols.Contains(symbol),
                    "Linked symbols did not contain the very symbol we started with."
                );

                group = new SymbolGroup(linkedSymbols);
                Contract.ThrowIfFalse(
                    group.Symbols.Contains(symbol),
                    "Symbol group did not contain the very symbol we started with."
                );
        ...
        {
            using var _1 = PooledDictionary<ISymbol, PooledHashSet<string>>.GetInstance(
                out var symbolToGlobalAliases
            );
            using var _2 = PooledDictionary<Document, MetadataUnifyingSymbolHashSet>.GetInstance(
                out var documentToSymbols
            );
            try
            {
        ...
        )
        {
            ...
            async Task ProcessDocumentAsync(ISymbol symbol, FindReferencesDocumentState state)
            {
        ...
        )
        {
            foreach (var symbol in allSymbols)
            {
                foreach (var finder in _finders)
                {
                    var aliases = await finder
                        .DetermineGlobalAliasesAsync(symbol, project, cancellationToken)
                        .ConfigureAwait(false);
                    if (aliases.Length > 0)
                    {
        ...
        private static PooledHashSet<string> GetGlobalAliasesSet<T>(
        ...
        private static void FreeGlobalAliases(
            PooledDictionary<ISymbol, PooledHashSet<string>> symbolToGlobalAliases
        )
        {
