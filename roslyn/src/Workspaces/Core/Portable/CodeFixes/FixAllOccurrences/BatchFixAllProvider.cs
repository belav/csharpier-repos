...
namespace Microsoft.CodeAnalysis.CodeFixes
{
    ...
    internal sealed class BatchFixAllProvider : FixAllProvider
    {
        ...
        > DetermineDiagnosticsAsync(FixAllContext fixAllContext, IProgressTracker progressTracker)
        {
            ...
            var documentToDiagnostics = await fixAllContext.GetDocumentDiagnosticsToFixAsync()
                .ConfigureAwait(false);
        ...
        private static async Task MergeTextChangesAsync(
            ...
            foreach (var group in allChangedDocumentsInDiagnosticsOrder.GroupBy(d => d.Id))
            {
                ...
                tasks.Add(
                    Task.Run(
                        async () =>
                            await textMerger.TryMergeChangesAsync(allDocChanges, cancellationToken)
                                .ConfigureAwait(false),
                        cancellationToken
                    )
                );
