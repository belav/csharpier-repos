...
namespace Roslyn.Utilities
{
    ...
    internal sealed class TaskQueue
    {
        ...
        private static TTask EndOperation<TTask>(IAsyncToken token, TTask task)
            where TTask : Task
        {
...
#pragma warning disable VSTHRD200 // Use "Async" suffix for async methods (Task wrappers, not asynchronous methods)
        ...
        public Task ScheduleTask(
            ...
            EndOperation(
                BeginOperation(taskName),
                ScheduleTaskInProgress(operation, cancellationToken)
            );

        /// <inheritdoc cref="ScheduleTask(string, Action, CancellationToken)"/>
        public Task<T> ScheduleTask<T>(
            string taskName,
            Func<T> operation,
            CancellationToken cancellationToken
        ) =>
