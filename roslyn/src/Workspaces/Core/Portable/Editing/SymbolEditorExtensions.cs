...
namespace Microsoft.CodeAnalysis.Editing
{
    public static class SymbolEditorExtensions
    {
        ...
        )
        {
            ...
            {
                baseOrInterfaceType = (ITypeSymbol)(
                    await editor
                        .GetCurrentSymbolAsync(baseOrInterfaceType, cancellationToken)
                        .ConfigureAwait(false)
                );
            }

            // look for the base or interface declaration in all declarations of the symbol
            var currentDecls = await editor
                .GetCurrentDeclarationsAsync(symbol, cancellationToken)
                .ConfigureAwait(false);
            ...
            foreach (var decl in currentDecls)
            {
                ...
                var typeRef = gen.GetBaseAndInterfaceTypes(decl)
                    .FirstOrDefault(
                        r =>
                            model.GetTypeInfo(r, cancellationToken).Type.Equals(baseOrInterfaceType)
                    );
                if (typeRef != null)
                {
                    return typeRef;
        ...
        )
        {
            ...
            if (baseType != null)
            {
                ...
                var typeRef = await editor
                    .GetBaseOrInterfaceDeclarationReferenceAsync(
                        symbol,
                        baseType,
                        cancellationToken
                    )
