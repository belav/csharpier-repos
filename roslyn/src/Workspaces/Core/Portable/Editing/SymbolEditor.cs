...
namespace Microsoft.CodeAnalysis.Editing
{
    ...
    public sealed class SymbolEditor
    {
        ...
        )
        {
            ...
            {
                var comp = await solution.GetProject(projectId)
                    .GetCompilationAsync(cancellationToken)
                    .ConfigureAwait(false);
                var symbols = DocumentationCommentId.GetSymbolsForDeclarationId(symbolId, comp)
                    .ToList();
        ...
        /// </summary>
        public async Task<IReadOnlyList<SyntaxNode>> GetCurrentDeclarationsAsync(
            ISymbol symbol,
            CancellationToken cancellationToken = default
        )
        {
        ...
        )
        {
            ...
            var editor = await DocumentEditor.CreateAsync(doc, cancellationToken)
                .ConfigureAwait(false);
        ...
        )
        {
            ...
            throw new ArgumentException(
                "The location specified is not part of the solution.",
                nameof(location)
            );
        }
        ...
        )
        {
            ...
            foreach (var declGroup in declsByDocId)
            {
                ...
                var editor = await solutionEditor.GetDocumentEditorAsync(docId, cancellationToken)
                    .ConfigureAwait(false);
