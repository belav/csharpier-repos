    ...
    internal partial class PatternMatcher
    {
        ...
        )
        {
            ...
            )
            {
                ...
                matchedSpans =
                    includeMatchedSpans && result.Value.MatchedSpansInReverse != null
            ...
            private static PatternMatchKind GetKind(
            ...
            )
            {
                if (patternIndex == patternChunkText.Length)
                {
                    ...
                    var matchedSpansInReverse = includeMatchedSpans
                ...
                for (
                    int humpIndex = candidateHumpIndex, n = candidateHumps.Count;
                    humpIndex < n;
                    humpIndex++
                )
                {
            ...
            {
                return IsAscii(v) ? ToLowerAsciiInvariant(v) : textInfo.ToLower(v);
            }

            private static bool IsAscii(char v) => v < 0x80;

            private static char ToLowerAsciiInvariant(char c) =>
                c is >= 'A' and <= 'Z' ? (char)(c | 0x20) : c;
            ...
            )
            {
                ...
                )
                {
                    ...
                    var resultOpt = TryMatch(
                        patternIndex + possibleHumpMatchLength,
                        humpIndex + 1,
                        contiguous,
                        candidateHumps
                    );
            ...
            private static bool IsBetter(
                CamelCaseResult result,
                CamelCaseResult? currentBestResult,
                in TemporaryArray<TextSpan> candidateHumps
            )
            {
                ...
                return GetKind(result, candidateHumps)
                    < GetKind(currentBestResult.Value, candidateHumps);
            }
