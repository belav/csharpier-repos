...
internal sealed class MapBasedLongestCommonSubsequence<TNode>(IReadOnlyDictionary<TNode, TNode> map)
    ...
    where TNode : notnull
{
    protected override bool ItemsEqual(
        ...
        map.TryGetValue(oldSequence[oldIndex], out var newNode)
        && newNode.Equals(newSequence[newIndex]);

    internal IEnumerable<Edit<TNode>> GetEdits(
        IReadOnlyList<TNode> oldNodes,
        IReadOnlyList<TNode> newNodes,
        TreeComparer<TNode>? treeComparer = null
    )
    {
