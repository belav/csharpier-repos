...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    ...
    internal sealed partial class DiagnosticAnalyzerInfoCache
    {
        ...
        )
        {
            ...
            public readonly bool HasCompilationEndDescriptor = supportedDescriptors.Any(
                DiagnosticDescriptorExtensions.IsCompilationEnd
            );
        }

        private sealed class SuppressionDescriptorsInfo(
            ImmutableArray<SuppressionDescriptor> supportedSuppressions
        )
        {
            public readonly ImmutableArray<SuppressionDescriptor> SupportedSuppressions =
        ...
        internal DiagnosticAnalyzerInfoCache()
        {
            ...
            _suppressionsInfo =
                new ConditionalWeakTable<DiagnosticSuppressor, SuppressionDescriptorsInfo>();
        ...
        public bool TryGetDescriptorForDiagnosticId(
            string diagnosticId,
        ...
        private void PopulateIdToDescriptorMap(ImmutableArray<DiagnosticDescriptor> descriptors)
        {
            foreach (var descriptor in descriptors)
            {
                ...
                else if (existingDescriptor != null && !descriptor.Equals(existingDescriptor))
                {
                    ...
                    if (
                        descriptor
                            .WithMessageFormat(existingDescriptor.MessageFormat)
                            .Equals(existingDescriptor)
                    )
                        continue;
