...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    internal static partial class Extensions
    {
        ...
        public static async Task<ImmutableArray<Diagnostic>> ToDiagnosticsAsync(
            ...
            {
                result.Add(
                    await diagnostic.ToDiagnosticAsync(project, cancellationToken)
        ...
        public static async Task<IList<Location>> ConvertLocationsAsync(
            ...
            {
                var location = await data.ConvertLocationAsync(project, cancellationToken)
                    .ConfigureAwait(false);
                result.Add(location);
            }
        ...
        public static async Task<Location> ConvertLocationAsync(
            ...
            {
                var syntacticDocument = await SyntacticDocument.CreateAsync(
                        document,
                        cancellationToken
                    )
                    .ConfigureAwait(false);
        ...
        public static Location ConvertLocation(
            ...
            if (document == null)
            {
                ...
                return Location.Create(
                    ...
                    new LinePositionSpan(
                        ...
                        new LinePosition(
                            dataLocation.OriginalEndLine,
                            dataLocation.OriginalEndColumn
                        )
                    )
                );
            }
            ...
            return syntaxTree.GetLocation(
                dataLocation.SourceSpan ?? DiagnosticData.GetTextSpan(dataLocation, document.Text)
            );
        }
        ...
        > ToResultBuilderMapAsync(
            ...
            if (documentAnalysisScope != null)
            {
                ...
                {
                    treeToAnalyze = await document.GetSyntaxTreeAsync(cancellationToken)
                        .ConfigureAwait(false);
                }
                else
                {
            ...
            foreach (var analyzer in analyzers)
            {
                ...
                else
                {
                    ...
                    AddAnalyzerDiagnosticsToResult(
                        analyzer,
                        analysisResult.CompilationDiagnostics,
                        ref result,
                        compilation,
                        tree: null,
                        additionalDocumentId: null,
                ...
                if (
                    ...
                    if (documentAnalysisScope != null)
                    {
                        if (treeToAnalyze != null)
                        {
                            var diagnostics = additionalPragmaSuppressionDiagnostics.WhereAsArray(
        ...
        public static async Task<(AnalysisResult result, ImmutableArray<Diagnostic> additionalDiagnostics)> GetAnalysisResultAsync(
            ...
            var result = await GetAnalysisResultAsync(
                ...
                .ConfigureAwait(false);
            var additionalDiagnostics =
                await compilationWithAnalyzers.GetPragmaSuppressionAnalyzerDiagnosticsAsync(
                        documentAnalysisScope,
                        project,
                        analyzerInfoCache,
                        cancellationToken
        ...
        private static async Task<AnalysisResult> GetAnalysisResultAsync(
            ...
            Debug.Assert(
                documentAnalysisScope.Analyzers.ToSet()
            ...
            switch (documentAnalysisScope.Kind)
            {
                case AnalysisKind.Syntax:
                    if (documentAnalysisScope.TextDocument is Document document)
                    {
                        var tree = await document.GetRequiredSyntaxTreeAsync(cancellationToken)
                            .ConfigureAwait(false);
                        return await compilationWithAnalyzers.GetAnalysisResultAsync(
                                tree,
                    ...
                    {
                        return await compilationWithAnalyzers.GetAnalysisResultAsync(
                                documentAnalysisScope.AdditionalFile,
                                documentAnalysisScope.Analyzers,
                                cancellationToken
                            )
                            .ConfigureAwait(false);
                ...
                case AnalysisKind.Semantic:
                    var model = await (
                        (Document)documentAnalysisScope.TextDocument
        ...
        private static async Task<
            ImmutableArray<Diagnostic>
        > GetPragmaSuppressionAnalyzerDiagnosticsAsync(
            this CompilationWithAnalyzers compilationWithAnalyzers,
            DocumentAnalysisScope? documentAnalysisScope,
            Project project,
            DiagnosticAnalyzerInfoCache analyzerInfoCache,
            ...
            else
            {
                if (compilationWithAnalyzers.AnalysisOptions.ConcurrentAnalysis)
                {
                    ...
                    {
                        tasks.Add(
                            AnalyzeDocumentAsync(suppressionAnalyzer, document, span: null, bag.Add)
                        );
                    }
            ...
            async Task AnalyzeDocumentAsync(
                ...
                await suppressionAnalyzer.AnalyzeAsync(
                        ...
                        span,
                        compilationWithAnalyzers,
                        analyzerInfoCache.GetDiagnosticDescriptors,
                        reportDiagnostic,
                        cancellationToken
                    )
                    .ConfigureAwait(false);
