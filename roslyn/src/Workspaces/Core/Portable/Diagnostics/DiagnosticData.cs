...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    ...
    internal sealed class DiagnosticData : IEquatable<DiagnosticData?>
    {
        ...
        public override bool Equals(object? obj) => obj is DiagnosticData data && Equals(data);
        ...
        public bool Equals(DiagnosticData? other)
        {
            ...
            return DataLocation?.OriginalStartLine == other.DataLocation?.OriginalStartLine
                            ...
                            Hash.Combine(
                                ...
                                Hash.Combine(
                                    ...
                                    Hash.Combine(
                                        DocumentId,
                                        Hash.Combine(
                                            DataLocation?.OriginalStartLine ?? 0,
                                            Hash.Combine(
                                                DataLocation?.OriginalStartColumn ?? 0,
                                                (int)Severity
                                            )
                                        )
                                    )
        ...
        public override string ToString()
        {
            return string.Format(
                "{0} {1} {2} {3} {4} {5} ({5}, {6}) [original: {7} ({8}, {9})]",
                Id,
                Severity,
                Message,
                ProjectId,
        ...
        {
            var location = await DataLocation
                .ConvertLocationAsync(project, cancellationToken)
                .ConfigureAwait(false);
            var additionalLocations = await AdditionalLocations
                .ConvertLocationsAsync(project, cancellationToken)
                .ConfigureAwait(false);
        ...
        public static LinePositionSpan GetLinePositionSpan(
        ...
        )
        {
            ...
            var dataLocationStartLine =
                (useMapped ? dataLocation?.MappedStartLine : dataLocation?.OriginalStartLine) ?? 0;
            var dataLocationStartColumn =
                (useMapped ? dataLocation?.MappedStartColumn : dataLocation?.OriginalStartColumn)
                ?? 0;
            var dataLocationEndLine =
                (useMapped ? dataLocation?.MappedEndLine : dataLocation?.OriginalEndLine) ?? 0;
            var dataLocationEndColumn =
                (useMapped ? dataLocation?.MappedEndColumn : dataLocation?.OriginalEndColumn) ?? 0;
            ...
            AdjustBoundaries(
                dataLocationStartLine,
                dataLocationStartColumn,
                dataLocationEndLine,
                dataLocationEndColumn,
                lines,
                out var startLine,
                out var startColumn,
                out var endLine,
        ...
        public static TextSpan GetTextSpan(DiagnosticDataLocation? dataLocation, SourceText text)
        {
            ...
            return EnsureInBounds(
                TextSpan.FromBounds(Math.Max(span.Start, 0), Math.Max(span.End, 0)),
                text
            );
        }
        ...
        )
        {
            ...
            GetLocationInfo(
                ...
                location,
                out var sourceSpan,
                out var originalLineInfo,
                out var mappedLineInfo
            );
        ...
        )
        {
            ...
            using var _ = ArrayBuilder<DiagnosticDataLocation>.GetInstance(
            ...
            foreach (var location in diagnostic.AdditionalLocations)
            {
                if (location.IsInSource)
                {
                    builder.AddIfNotNull(
                        CreateLocation(document.Project.GetDocument(location.SourceTree), location)
                    );
                }
                else if (location.Kind == LocationKind.ExternalFile)
                {
                    ...
                    builder.AddIfNotNull(
                        CreateLocation(document.Project.GetTextDocument(textDocumentId), location)
                    );
                }
            }
        ...
        {
            var diagnosticSpanMappingService = document
                .Project
                .Solution
                .Workspace
                .Services
            ...
            {
                diagnosticSpanMappingService.GetAdjustedDiagnosticSpan(
                    document.Id,
                    location,
                    out sourceSpan,
                    out originalLineInfo,
                    out mappedLineInfo
                );
                return;
            }
