...
namespace Microsoft.CodeAnalysis.CodeCleanup
{
    internal abstract class AbstractCodeCleanerService : ICodeCleanerService
    {
        ...
        private static ImmutableArray<TextSpan> GetTextSpansFromAnnotation(
            SyntaxNode node,
            List<(
                SyntaxAnnotation previousAnnotation,
                SyntaxAnnotation nextAnnotation
            )> annotations,
            CancellationToken cancellationToken
        )
        {
                ...
                foreach (var codeCleaner in codeCleaners)
                {
                    ...
                    )
                    {
                        currentDocument = await codeCleaner
                            .CleanupAsync(currentDocument, spans, options, cancellationToken)
                            .ConfigureAwait(false);
                    }
                ...
                foreach (var codeCleaner in codeCleaners)
                {
                    ...
                    )
                    {
                        currentRoot = await codeCleaner
                            .CleanupAsync(currentRoot, spans, options, services, cancellationToken)
                            .ConfigureAwait(false);
                    }
                }
