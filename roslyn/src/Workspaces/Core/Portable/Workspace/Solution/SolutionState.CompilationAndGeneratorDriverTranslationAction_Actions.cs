    ...
    internal partial class SolutionState
    {
        private abstract partial class CompilationAndGeneratorDriverTranslationAction
        {
            ...
            ) : CompilationAndGeneratorDriverTranslationAction
            {
                ...
                public override CompilationAndGeneratorDriverTranslationAction? TryMergeWithPrior(
                    CompilationAndGeneratorDriverTranslationAction priorAction
                )
                {
                    if (
                        priorAction is TouchDocumentAction priorTouchAction
            ...
            ) : CompilationAndGeneratorDriverTranslationAction
            {
                ...
                {
                    if (
                        priorAction is TouchAdditionalDocumentAction priorTouchAction
                        && priorTouchAction._newState == _oldState
                    )
                    {
                        return new TouchAdditionalDocumentAction(
                            priorTouchAction._oldState,
                            _newState
                        );
                    }
            ...
            internal sealed class RemoveDocumentsAction(ImmutableArray<DocumentState> documents)
                : CompilationAndGeneratorDriverTranslationAction
            {
            ...
            internal sealed class AddDocumentsAction(ImmutableArray<DocumentState> documents)
                : CompilationAndGeneratorDriverTranslationAction
            {
                public override async Task<Compilation> TransformCompilationAsync(
                    Compilation oldCompilation,
            ...
            ) : CompilationAndGeneratorDriverTranslationAction
            {
                ...
                )
                {
                    ...
                    foreach (
                        var documentState in state.DocumentStates.GetStatesInCompilationOrder()
                    )
                    {
                        cancellationToken.ThrowIfCancellationRequested();
                        syntaxTrees.Add(
            ...
            ) : CompilationAndGeneratorDriverTranslationAction
            {
                ...
                )
                {
                    ...
                    {
                        return generatorDriver.WithUpdatedAnalyzerConfigOptions(
                            state.AnalyzerOptions.AnalyzerConfigOptionsProvider
                        );
                    }
                    else
                    {
            ...
            internal sealed class ProjectAssemblyNameAction(string assemblyName)
                : CompilationAndGeneratorDriverTranslationAction
            {
                public override Task<Compilation> TransformCompilationAsync(
                    Compilation oldCompilation,
            ...
            ) : CompilationAndGeneratorDriverTranslationAction
            {
                ...
                )
                {
                    ...
                    {
                        generatorDriver = generatorDriver.RemoveGenerators(
                            referencesToRemove.SelectManyAsArray(r => r.GetGenerators(language))
                        );
                    }

                    if (!referencesToAdd.IsDefaultOrEmpty)
                    {
                        generatorDriver = generatorDriver.AddGenerators(
                            referencesToAdd.SelectManyAsArray(r => r.GetGenerators(language))
                        );
                    }
                ...
                public override GeneratorDriver? TransformGeneratorDriver(GeneratorDriver _)
                {
                    ...
                    var generatorDriver = oldGeneratorDriver
                        .ReplaceAdditionalTexts(
                            newProjectState.AdditionalDocumentStates.SelectAsArray(
                        ...
                        .WithUpdatedAnalyzerConfigOptions(
                            newProjectState.AnalyzerOptions.AnalyzerConfigOptionsProvider
                        )
