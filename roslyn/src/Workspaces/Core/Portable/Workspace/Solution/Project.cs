...
namespace Microsoft.CodeAnalysis
{
    ...
    public partial class Project
    {
        ...
        public bool SupportsCompilation =>
            this.Services.GetService<ICompilationFactoryService>() != null;
...
#pragma warning disable CS0618 // Member is obsolete -- shouldn't be reported here https://github.com/dotnet/roslyn/issues/66409
        public HostLanguageServices LanguageServices =>
            _projectState.LanguageServices.HostLanguageServices;
#pragma warning restore
        ...
        public IReadOnlyList<MetadataReference> MetadataReferences =>
            _projectState.MetadataReferences;
        ...
        public IReadOnlyList<AnalyzerReference> AnalyzerReferences =>
            _projectState.AnalyzerReferences;
        ...
        public IReadOnlyList<DocumentId> AdditionalDocumentIds =>
            _projectState.AdditionalDocumentStates.Ids;

        /// <summary>
        /// All the additional document IDs associated with this project.
        /// </summary>
        internal IReadOnlyList<DocumentId> AnalyzerConfigDocumentIds =>
            _projectState.AnalyzerConfigDocumentStates.Ids;
        ...
        public IEnumerable<TextDocument> AdditionalDocuments =>
            AdditionalDocumentIds.Select(GetAdditionalDocument)!;

        /// <summary>
        /// All the <see cref="AnalyzerConfigDocument"/>s associated with this project.
        /// </summary>
        public IEnumerable<AnalyzerConfigDocument> AnalyzerConfigDocuments =>
            AnalyzerConfigDocumentIds.Select(GetAnalyzerConfigDocument)!;
        ...
        /// </summary>
        internal async ValueTask<Document?> GetDocumentAsync(
        ...
        )
        {
            ...
            return await GetSourceGeneratedDocumentAsync(documentId, cancellationToken)
        ...
        /// </summary>
        internal async ValueTask<TextDocument?> GetTextDocumentAsync(
            DocumentId documentId,
            CancellationToken cancellationToken = default
        )
        {
        ...
        )
        {
            ...
            return ImmutableHashMapExtensions.GetOrAdd(
                ref _idToSourceGeneratedDocumentMap,
                documentId,
                s_createSourceGeneratedDocumentFunction,
                (documentState, this)
        ...
        internal ValueTask<ImmutableArray<Diagnostic>> GetSourceGeneratorDiagnosticsAsync(
        ...
        )
        {
            return ContainsSymbolsAsync(
                (index, cancellationToken) =>
                    ...
                    || index.ProbablyContainsEscapedIdentifier(name),
                cancellationToken
            );
        }
        ...
        private Task<bool> ContainsSymbolsAsync(
            Func<SyntaxTreeIndex, CancellationToken, bool> predicate,
            CancellationToken cancellationToken
        )
        {
        ...
        private Task<bool> ContainsDeclarationAsync(
            Func<TopLevelSyntaxTreeIndex, CancellationToken, bool> predicate,
            CancellationToken cancellationToken
        )
        {
            return ContainsAsync(async d =>
        ...
        private async Task<bool> ContainsAsync(Func<Document, Task<bool>> predicateAsync)
        {
            ...
            var results = await Task.WhenAll(this.Documents.Select(predicateAsync))
                .ConfigureAwait(false);
        ...
        private static readonly Func<
            DocumentId,
            (SourceGeneratedDocumentState state, Project project),
            SourceGeneratedDocument
        > s_createSourceGeneratedDocumentFunction = (documentId, stateAndProject) =>
            new SourceGeneratedDocument(stateAndProject.project, stateAndProject.state);
        ...
        internal Task<bool> HasSuccessfullyLoadedAsync(
            CancellationToken cancellationToken = default
        ...
        /// </summary>
        public Task<VersionStamp> GetSemanticVersionAsync(
            CancellationToken cancellationToken = default
        ...
        /// </summary>
        public TextDocument AddAdditionalDocument(
            string name,
            SourceText text,
            IEnumerable<string>? folders = null,
            string? filePath = null
            ...
            foreach (var documentId in documentIds)
            {
                ...
                if (documentId?.ProjectId != this.Id)
                {
                    throw new ArgumentException(
                        string.Format(WorkspacesResources._0_is_in_a_different_project, documentId)
                    );
                }
            }
        }
        ...
        internal SkippedHostAnalyzersInfo GetSkippedAnalyzersInfo(
