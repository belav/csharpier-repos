    ...
    public partial class ProjectDependencyGraph
    {
        ...
        )
        {
            ...
            var referencesMap = referencedProjectIds.IsEmpty
                ...
                : _referencesMap.SetItem(projectId, referencedProjectIds);
        ...
        )
        {
            ...
            return _lazyReverseReferencesMap.GetValueOrDefault(
                projectId,
                ImmutableHashSet<ProjectId>.Empty
            );
        }
        ...
        )
        {
            if (!_transitiveReferencesMap.TryGetValue(projectId, out var transitiveReferences))
            {
                ...
                _transitiveReferencesMap = _transitiveReferencesMap.Add(
                    projectId,
        ...
        )
        {
            ...
            )
            {
                ...
                _reverseTransitiveReferencesMap = _reverseTransitiveReferencesMap.Add(
                    projectId,
                    reverseTransitiveReferences
                );
            }
        ...
        private void ComputeDependencySets(
            HashSet<ProjectId> seenProjects,
        ...
        )
        {
            foreach (var project in _projectIds)
            {
                if (seenProjects.Add(project))
                {
                    ...
                    using var dependencySet = SharedPools.Default<HashSet<ProjectId>>()
                        .GetPooledObject();
                    ...
                    using var topologicallySeenProjects = SharedPools.Default<HashSet<ProjectId>>()
                        .GetPooledObject();
                    using var sortedProjects = SharedPools.Default<List<ProjectId>>()
                        .GetPooledObject();
                    this.TopologicalSort(
                        dependencySet.Object,
                        topologicallySeenProjects.Object,
                        sortedProjects.Object,
                        cancellationToken
                    );
        ...
        private void ComputedDependencySet(ProjectId project, HashSet<ProjectId> result)
        {
            ...
            {
                var otherProjects = this.GetProjectsThatDirectlyDependOnThisProject_NoLock(project)
                    .Concat(this.GetProjectsThatThisProjectDirectlyDependsOn(project));
        ...
        )
        {
            ...
            foreach (var (_, referencedProjects) in referencesMap)
            {
                Debug.Assert(
                    ...
                    "Unexpected empty value in the forward references map."
                );
                foreach (var referencedProject in referencedProjects)
                {
                    Debug.Assert(
                        projectIds.Contains(referencedProject),
                        "Unexpected reference to unknown project."
                    );
                }
            }
        }
        ...
        )
        {
            ...
            {
                Debug.Assert(
                    !referencingProjects.IsEmpty,
                    "Unexpected empty value in the reverse references map."
                );
