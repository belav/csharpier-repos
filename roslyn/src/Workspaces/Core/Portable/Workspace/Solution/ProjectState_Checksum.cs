...
namespace Microsoft.CodeAnalysis
{
    internal partial class ProjectState
    {
        ...
        public Task<ProjectStateChecksums> GetStateChecksumsAsync(
            ...
            return SpecializedTasks
                .TransformWithoutIntermediateCancellationExceptionAsync(
                    static (lazyChecksums, cancellationToken) =>
                        new ValueTask<ProjectStateChecksums>(
        ...
        public Checksum GetParseOptionsChecksum() =>
            GetParseOptionsChecksum(
                LanguageServices.SolutionServices.GetService<ISerializerService>()
            );
        ...
        private Checksum GetParseOptionsChecksum(ISerializerService serializer) =>
            this.SupportsCompilation
                ? ChecksumCache.GetOrCreate(
                    this.ParseOptions,
                    static (options, serializer) => serializer.CreateParseOptionsChecksum(options),
                    serializer
                )
                : Checksum.Null;
        ...
        )
        {
            try
            {
                using (
                    Logger.LogBlock(
                        FunctionId.ProjectState_ComputeChecksumsAsync,
                        FilePath,
                        cancellationToken
                    )
                )
                {
                    ...
                    var additionalDocumentChecksumTasks = AdditionalDocumentStates.SelectAsArray(
                        static (state, token) => state.GetChecksumAsync(token),
                        cancellationToken
                    );
                    var analyzerConfigDocumentChecksumTasks =
                        AnalyzerConfigDocumentStates.SelectAsArray(
                            static (state, token) => state.GetChecksumAsync(token),
                            cancellationToken
                        );
                    ...
                    var compilationOptionsChecksum = SupportsCompilation
                        ? ChecksumCache.GetOrCreate(
                            CompilationOptions,
                            static (options, tuple) =>
                                tuple.serializer.CreateChecksum(options, tuple.cancellationToken),
                    ...
                    var metadataReferenceChecksums = ChecksumCache.GetOrCreateChecksumCollection(
                        MetadataReferences,
                        serializer,
                        cancellationToken
                    );
                    ...
                    var documentChecksums = new ChecksumCollection(
                        await documentChecksumsTasks.WhenAll().ConfigureAwait(false)
                    );
                    var additionalDocumentChecksums = new ChecksumCollection(
                        await additionalDocumentChecksumTasks.WhenAll().ConfigureAwait(false)
                    );
                    var analyzerConfigDocumentChecksums = new ChecksumCollection(
                    ...
                    return new ProjectStateChecksums(
                        ...
                        new(
                            ...
                            AdditionalDocumentStates.SelectAsArray(static s => s.Id)
                        ),
                        new(
                            analyzerConfigDocumentChecksums,
                            AnalyzerConfigDocumentStates.SelectAsArray(static s => s.Id)
                        )
                    );
                }
            }
