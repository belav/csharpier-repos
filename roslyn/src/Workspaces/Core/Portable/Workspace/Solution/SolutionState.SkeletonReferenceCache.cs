    ...
    private partial class SkeletonReferenceCache
    {
        ...
        )
        {
            ...
            var lazy = s_compilationToSkeletonSet.GetValue(
                compilation,
                compilation =>
                    new AsyncLazy<SkeletonReferenceSet?>(
                        cancellationToken =>
                            Task.FromResult(
                                CreateSkeletonSet(workspace, compilation, cancellationToken)
                            ),
                        cacheResult: true
                    )
            );
        ...
        )
        {
            ...
            try
            {
                ...
                )
                {
                    ...
                    var doNotClone =
                        optionsService != null
                        && optionsService.Options.DisableCloneWhenProducingSkeletonReferences;

                    var compilationToEmit = doNotClone ? compilation : compilation.Clone();
                    var emitResult = compilationToEmit.Emit(
                        stream,
