...
internal partial class SolutionState
{
    ...
    /// cref="MetadataReferenceProperties"/> it might be referenced by.  Skeletons are used in the
    // compilation tracker
    /// to allow cross-language project references with live semantic updating between VB/C# and vice
    // versa.
    /// Specifically, in a cross language case we will build a skeleton ref for the referenced project
    // and have the
    /// referrer use that to understand its semantics.
    /// <para>
    /// This approach works, but has the caveat that live cross-language semantics are only possible
    // when the skeleton
    /// assembly can be built.  This should always be the case for correct code, but it may not be the
    // case for code
    /// with errors depending on if the respective language compiler is resilient to those errors or
    // not. In that case
    /// though where the skeleton cannot be built, this type provides mechanisms to fallback to the last
    // successfully
    /// built skeleton so that a somewhat reasonable experience can be maintained.  If we failed to do
    // this and instead
    /// returned nothing, a user would find that practically all semantic experiences that depended on
    // that particular
    /// project would fail or be seriously degraded (e.g. diagnostics).  To that end, it's better to
    // limp along with
    ...
    /// cref="VersionStamp"/> for a project. As long as the <see
    // cref="Project.GetDependentSemanticVersionAsync"/> for
    /// that project is the same, then all the references of it can be reused.  When an <see
    /// cref="ICompilationTracker"/> forks itself, it will also <see cref="Clone"/> this, allowing
    // previously computed
    /// references to be used by later forks. However, this means that later forks (esp. ones that fail
    // to produce a
    /// skeleton, or which produce a skeleton for different semantics) will not leak backward to a prior
    // <see
    /// cref="ProjectState"/>, causing it to see a view of the world inapplicable to its current
    // snapshot.  A downside
    /// of this is that if a fork happens to a compilation tracker *prior* to the skeleton for it being
    // computed, then
    /// when the skeleton is actually produced it won't be shared forward.  In practice the hope is that
    // this is rare,
    /// and that eventually the compilation trackers will have computed the skeleton and will be able to
    // pass it forward
    /// from that point onwards.
    /// </para>
    /// <para>
    /// The cached data we compute is associated with a particular compilation-tracker.  Because of
    // this, once we
    /// compute the skeleton information for that tracker, we hold onto it for as long as the tracker is
    // itself alive.
    /// The presumption here is that once created, it will likely be needed in the future as well as
    // there will still be
    /// downstream projects of different languages that reference this.  The only time this won't hold
    // true is if there
    /// was a cross language p2p ref, but then it gets removed from the solution.  However, this sort of
    // change should
    /// be rare in a solution, so it's unlikely to happen much, and the only negative is holding onto a
    // little bit more
    /// memory.
    /// </para>
    /// </summary>
    private partial class SkeletonReferenceCache
    {
        ...
        /// <summary>
        /// Static conditional mapping from a compilation to the skeleton set produced for it.  This is
        // valuable for a
        /// couple of reasons. First, a compilation tracker may fork, but produce the same compilation.  As
        // such, we
        /// want to get the same skeleton set for it.  Second, consider the following scenario:
        /// <list type="number">
        /// <item>Project A is referenced by projects B and C (both have a different language than
        // A).</item>
        /// <item>Producing the compilation for 'B' produces the compilation for 'A' which produces the
        // skeleton that 'B' references.</item>
        /// <item>B's compilation is released and then GC'ed.</item>
        /// <item>Producing the compilation for 'C' needs the skeleton from 'A'</item>
        /// </list>
        /// At this point we would not want to re-emit the assembly metadata for A's compilation.  We
        // already did that
        /// for 'B', and it can be enormously expensive to do so again.  So as long as A's compilation
        // lives, we really
        ...
        /// <summary>
        /// Lock around <see cref="_version"/> and <see cref="_skeletonReferenceSet"/> to ensure they are
        // updated/read
        ...
        /// <summary>
        /// Produces a copy of the <see cref="SkeletonReferenceCache"/>, allowing forks of <see
        // cref="ProjectState"/> to
        /// reuse <see cref="MetadataReference"/>s when their dependent semantic version matches ours.  In
        // the case where
        /// the version is different, then the clone will attempt to make a new skeleton reference for that
        // version.  If it
        ...
        public SkeletonReferenceCache Clone()
        {
            lock (_stateGate)
            {
                // pass along the best version/reference-set we computed for ourselves.  That way future
                // ProjectStates
                // can use this data if either the version changed, or they weren't able to build a skeleton for
                // themselves.
                // By passing along a copy we ensure that if they have a different version, they'll end up producing
                // a new
                // SkeletonReferenceSet where they'll store their own data in which will not affect prior
                // ProjectStates.
        ...
        {
            // First, just see if we have cached a reference set that is complimentary with the version of the
            // project
        ...
        {
            // It's acceptable for this computation to be something that multiple calling threads may hit at
            // once.  The
            // implementation inside the compilation tracker does an async-wait on a an internal semaphore to
            // ensure
            ...
            // note: computing the assembly metadata is actually synchronous.  However, this ensures we don't
            // have N
        ...
        )
        {
            ...
            // read in the stream and pass ownership of it to the metadata object.  When it is disposed it will
            // dispose
            // the stream as well.
