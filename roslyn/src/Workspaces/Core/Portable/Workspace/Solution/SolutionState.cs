    ...
    internal partial class SolutionState
    {
        ...
        )
        {
            ...
            _lazyChecksums = AsyncLazy.Create(
                c => ComputeChecksumsAsync(projectsToInclude: null, c)
            );
        ...
        )
        {
            ...
            foreach (var (id, tracker) in _projectIdToTrackerMap)
                builder.Add(
                    id,
                    CanReuse(id) ? tracker : tracker.Fork(tracker.ProjectState, translate: null)
                );
        ...
        )
        {
            try
            {
                ...
                using (this.StateLock.DisposableWait(cancellationToken))
                {
                    ...
                    if (
                        Services
                            .GetService<IWorkpacePartialSolutionsTestHook>()
        ...
        public ImmutableArray<DocumentId> GetRelatedDocumentIds(DocumentId documentId)
        {
            ...
            return FilterDocumentIdsByLanguage(
                this,
                documentIds,
                projectState.ProjectInfo.Language
            );
        }
