...
namespace Microsoft.CodeAnalysis
{
    ...
    public class Document : TextDocument
    {
        ...
        {
            var otherDocument =
                otherTextDocument as Document
                ?? throw new ArgumentException(
                    $"{nameof(otherTextDocument)} isn't a regular document.",
                    nameof(otherTextDocument)
                );
        ...
        {
            get { return this.SupportsSyntaxTree && this.Project.SupportsCompilation; }
        }
        ...
        )
        {
            try
            {
                ...
                var syntaxTree = await this.GetRequiredSyntaxTreeAsync(cancellationToken)
                    .ConfigureAwait(false);
                var compilation = (
                    await this.Project.GetCompilationAsync(cancellationToken).ConfigureAwait(false)
                )!;
                ...
                var original = Interlocked.CompareExchange(
                    ref _model,
                    new WeakReference<SemanticModel>(result),
                    null
                );
        ...
        )
        {
            try
            {
                using (
                    Logger.LogBlock(
                        FunctionId.Workspace_Document_GetTextChanges,
                        this.Name,
                        cancellationToken
                    )
                )
                {
                    ...
                    {
                        throw new ArgumentException(
                            WorkspacesResources.The_specified_document_is_not_a_version_of_this_document
                        );
                    }
                    ...
                    {
                        var tree = (
                            await this.GetSyntaxTreeAsync(cancellationToken).ConfigureAwait(false)
                        )!;
                        var oldTree = await oldDocument.GetSyntaxTreeAsync(cancellationToken)
                            .ConfigureAwait(false);
                    ...
                    oldText = await oldDocument.GetTextAsync(cancellationToken)
                        .ConfigureAwait(false);
        ...
        {
            var documentIdsWithPath = this.Project.Solution
                .GetDocumentIdsWithFilePath(this.FilePath);
            var filteredDocumentIds = this.Project.Solution
        ...
        internal Document WithFrozenPartialSemantics(CancellationToken cancellationToken)
        {
            ...
            {
                var newSolution = this.Project.Solution
        ...
        /// </remarks>
        public Task<DocumentOptionSet> GetOptionsAsync(
            CancellationToken cancellationToken = default
        ...
        private void InitializeCachedOptions(OptionSet solutionOptions)
        {
            var newAsyncLazy = new AsyncLazy<DocumentOptionSet>(
                async c =>
                {
                    var optionsService = Project.Solution.Workspace.Services
                        .GetRequiredService<IOptionService>();
                    var documentOptionSet =
                        await optionsService.GetUpdatedOptionSetForDocumentAsync(
                            this,
                            solutionOptions,
                            c
                        )
