...
#pragma warning disable RS0030 // Do not used banned APIs (backwards compat)
        ...
        private void InitializeCachedOptions(OptionSet solutionOptions)
        {
            var newAsyncLazy = new AsyncLazy<DocumentOptionSet>(
                ...
                {
                    var options = await GetAnalyzerConfigOptionsAsync(cancellationToken)
                        .ConfigureAwait(false);
...
#pragma warning restore
        ...
        internal async ValueTask<StructuredAnalyzerConfigOptions> GetAnalyzerConfigOptionsAsync(
            CancellationToken cancellationToken
        )
        {
            var provider = (ProjectState.ProjectAnalyzerConfigOptionsProvider)
                Project.State.AnalyzerOptions.AnalyzerConfigOptionsProvider;
            return await provider
                .GetOptionsAsync(DocumentState, cancellationToken)
                .ConfigureAwait(false);
        }
    }
}
