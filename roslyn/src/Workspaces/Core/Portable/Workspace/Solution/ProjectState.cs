    ...
    internal partial class ProjectState
    {
            ...
            )
            {
                if (documentState.IsRazorDocument())
                {
                    return _lazyRazorDesignTimeOptions ??= new RazorDesignTimeAnalyzerConfigOptions(
                        _projectState.LanguageServices.WorkspaceServices
                    );
                }
                ...
                var legacyIndentationService =
                    workspace.Services.GetService<ILegacyIndentationManagerWorkspaceService>();
            ...
            : StructuredAnalyzerConfigOptions
        {
            ...
            public override bool TryGetValue(string key, [NotNullWhen(true)] out string? value)
            {
                ...
                if (
                    key is "indent_style" or "tab_width" or "indent_size"
                ...
                )
                {
                    ...
                    if (currentDocument != null && currentDocument.TryGetText(out var text))
                    {
                        try
                        {
                            ...
                            {
                                "indent_style"
                                    => _service.UseSpacesForWhitespace(text) ? "space" : "tab",
