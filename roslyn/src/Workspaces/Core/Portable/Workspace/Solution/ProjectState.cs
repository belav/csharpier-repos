...
namespace Microsoft.CodeAnalysis
{
    internal partial class ProjectState
    {
        ...
        {
            return projectInfo.WithDocuments(ImmutableArray<DocumentInfo>.Empty)
        ...
        )
        {
            ...
            foreach (var state in documentStates.States)
            {
                ...
                {
                    var version = await state.GetTextVersionAsync(cancellationToken)
        ...
        )
        {
            ...
            foreach (var state in documentStates.States)
            {
                ...
                var version = await state.GetTopLevelChangeTextVersionAsync(cancellationToken)
                    .ConfigureAwait(false);
                latestVersion = version.GetNewerVersion(latestVersion);
            }
        ...
        public AnalyzerConfigOptionsResult? GetAnalyzerConfigOptions()
        {
            ...
            return _lazyAnalyzerConfigSet.GetValue(CancellationToken.None)
        ...
        public ProjectState WithParseOptions(ParseOptions options)
        {
            ...
            return With(
                projectInfo: ProjectInfo.WithParseOptions(options)
                    .WithVersion(Version.GetNewerVersion()),
        ...
        public ProjectState WithProjectReferences(IReadOnlyList<ProjectReference> projectReferences)
        {
            ...
            return With(
                projectInfo: ProjectInfo.With(projectReferences: projectReferences)
                    .WithVersion(Version.GetNewerVersion())
            );
        }
        ...
        )
        {
            ...
            return With(
                projectInfo: ProjectInfo.With(metadataReferences: metadataReferences)
                    .WithVersion(Version.GetNewerVersion())
            );
        }
        ...
        )
        {
            ...
            return With(
                projectInfo: ProjectInfo.WithAnalyzerReferences(analyzerReferences)
                    .WithVersion(Version.GetNewerVersion())
            );
        }
