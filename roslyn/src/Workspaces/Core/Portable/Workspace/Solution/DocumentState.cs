...
namespace Microsoft.CodeAnalysis
{
    internal partial class DocumentState : TextDocumentState
    {
        ...
        )
        {
            ...
            {
                var textAndVersion = await newTextSource.GetValueAsync(cancellationToken)
                    .ConfigureAwait(false);
                return CreateTreeAndVersion(
                    newTextSource,
                    cacheKey,
                    filePath,
                    options,
        ...
        )
        {
            try
            {
                ...
                {
                    var newTextAndVersion = await newTextSource.GetValueAsync(cancellationToken)
                        .ConfigureAwait(false);
                    var oldTreeAndVersion = await oldTreeSource.GetValueAsync(cancellationToken)
                        .ConfigureAwait(false);
        ...
        public async ValueTask<SyntaxTree> GetSyntaxTreeAsync(CancellationToken cancellationToken)
        {
            ...
            var treeAndVersion = await _treeSource.GetValueAsync(cancellationToken)
                .ConfigureAwait(false);
        ...
        )
        {
            ...
            treeAndVersion = await _treeSource.GetValueAsync(cancellationToken)
                .ConfigureAwait(false);
            return treeAndVersion.Version;
        }
