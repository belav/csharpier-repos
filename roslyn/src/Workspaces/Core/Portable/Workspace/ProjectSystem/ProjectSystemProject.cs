    ...
    internal sealed partial class ProjectSystemProject
    {
            ...
            using (_gate.DisposableWait())
            {
                ...
                if (_activeBatchScopes > 0)
                {
                    _projectPropertyModificationsInBatch.Add(
                        solutionChanges => updateSolution(solutionChanges, oldValue)
                    );
                }
                else
                {
        ...
        private async Task OnBatchScopeDisposedMaybeAsync(bool useAsync)
        {
            ...
            )
            {
                ...
                _projectSystemProjectFactory.RaiseOnDocumentsAdded(
        ...
        public void AddAnalyzerReference(string fullPath)
        {
            ...
            using (_gate.DisposableWait())
            {
                ...
                foreach (var mappedFullPath in mappedPaths)
                {
                    ...
                    var analyzerPendingRemoval = _analyzersRemovedInBatch.FirstOrDefault(
                        a => a.FullPath == mappedFullPath
                    );
        ...
        private static readonly string s_razorSourceGeneratorMainAssemblyRootedFileName =
            PathUtilities.DirectorySeparatorStr + "Microsoft.NET.Sdk.Razor.SourceGenerators.dll";
        ...
        private OneOrMany<string> GetMappedAnalyzerPaths(string fullPath)
        {
            ...
            )
            {
                ...
                if (!vsixRazorAnalyzers.IsEmpty)
                {
                    if (
                        fullPath.EndsWith(
                            s_razorSourceGeneratorMainAssemblyRootedFileName,
                            StringComparison.OrdinalIgnoreCase
                        )
                    )
                    {
