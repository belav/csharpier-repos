...
namespace Microsoft.CodeAnalysis.CodeActions
{
    ...
    public abstract class CodeAction
    {
        private static readonly Dictionary<
            Type,
            bool
        > s_isNonProgressGetChangedSolutionAsyncOverridden = new();
        private static readonly Dictionary<
            Type,
            bool
        > s_isNonProgressComputeOperationsAsyncOverridden = new();
        ...
        private protected static ImmutableArray<string> RequiresNonDocumentChangeTags =
            ImmutableArray.Create(RequiresNonDocumentChange);
        ...
        public CodeActionPriority Priority
        {
            get
            {
                ...
                if (
                    priority == CodeActionPriority.High
                    && !this.CustomTags.Contains(CanBeHighPriorityTag)
                )
        ...
        private bool IsNonProgressApiOverridden(
            Dictionary<Type, bool> dictionary,
            Func<CodeAction, bool> computeResult
        )
        {
...
#pragma warning restore RS0030 // Do not use banned APIs
        ...
        /// </remarks>
        protected virtual CodeActionPriority ComputePriority() => CodeActionPriority.Default;
        ...
        /// </summary>
        internal void AddCustomTagAndTelemetryInfo(
        ...
        internal Guid GetTelemetryId(FixAllScope? fixAllScope = null)
        {
            ...
            var type =
                isFactoryGenerated && _providerTypeForTelemetry != null
        ...
        private protected virtual async Task<
        ...
        {
            var operations = await this.ComputeOperationsAsync(progress, cancellationToken)
                .ConfigureAwait(false);

            if (operations != null)
            {
                return await this.PostProcessAsync(originalSolution, operations, cancellationToken)
                    .ConfigureAwait(false);
            }
        ...
        /// </summary>
        public Task<ImmutableArray<CodeActionOperation>> GetPreviewOperationsAsync(
            CancellationToken cancellationToken
        ...
        )
        {
            using var _ = TelemetryLogging.LogBlockTimeAggregated(
                FunctionId.SuggestedAction_Preview_Summary,
                $"Total"
            );
...
#pragma warning disable RS0030 // Do not use banned APIs
        ...
        )
        {
            ...
            {
                var operations = await ComputeOperationsAsync(cancellationToken)
                    .ConfigureAwait(false);
                return operations.ToImmutableArrayOrEmpty();
            }
            else
            {
                var changedSolution = await GetChangedSolutionAsync(progress, cancellationToken)
                    .ConfigureAwait(false);
                return changedSolution == null
...
#pragma warning restore RS0030 // Do not use banned APIs
        ...
        > ComputePreviewOperationsAsync(CancellationToken cancellationToken) =>
            await ComputeOperationsAsync(CodeAnalysisProgress.None, cancellationToken)
                .ConfigureAwait(false);
        ...
        )
        {
            ...
            else
            {
                ...
                var changedDocument = await GetChangedDocumentAsync(progress, cancellationToken)
                    .ConfigureAwait(false);
                return changedDocument?.Project.Solution;
            }
        }

        internal async Task<Solution> GetRequiredChangedSolutionAsync(
            IProgress<CodeAnalysisProgress> progressTracker,
            CancellationToken cancellationToken
        )
        {
            var solution = await this.GetChangedSolutionAsync(progressTracker, cancellationToken)
                .ConfigureAwait(false);
            return solution
        ...
        {
            var solution = await GetChangedSolutionAsync(progress, cancellationToken)
                .ConfigureAwait(false);
            ...
            return await this.PostProcessChangesAsync(originalSolution, solution, cancellationToken)
                .ConfigureAwait(false);
        }
        ...
        protected Task<ImmutableArray<CodeActionOperation>> PostProcessAsync(
        ...
        /// <param name="cancellationToken">A cancellation token</param>
        protected Task<Solution> PostProcessChangesAsync(
        ...
        protected virtual async Task<Document> PostProcessChangesAsync(
            Document document,
            CancellationToken cancellationToken
        )
        {
            if (document.SupportsSyntaxTree)
            {
                ...
                var options = await document
                    ...
                    .ConfigureAwait(false);
                return await CleanupDocumentAsync(document, options, cancellationToken)
                    .ConfigureAwait(false);
            }
        ...
        [SuppressMessage(
            ...
            Justification = "This is source compatible"
        )]
        public static CodeAction Create(
            string title,
            Func<CancellationToken, Task<Document>> createChangedDocument,
        ...
        ) : CodeAction
        {
            ...
            protected sealed override CodeActionPriority ComputePriority() => priority;
        ...
        internal class CodeActionWithNestedActions : SimpleCodeAction
        {
            private CodeActionWithNestedActions(
                ...
                : base(
                    title,
                    ComputeEquivalenceKey(nestedActions),
                    priority,
                    createdFromFactoryMethod
                )
            {
