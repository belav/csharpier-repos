        ...
        where TCrefSyntax : SyntaxNode
    {
        ...
        public async Task<Document> ReduceAsync(
            ...
            using (Logger.LogBlock(FunctionId.Simplifier_ReduceAsync, cancellationToken))
            {
                ...
                optionSet ??= await document.GetOptionsAsync(cancellationToken)
...
#if DEBUG
                var originalDocHasErrors = await document.HasAnyErrorsAsync(cancellationToken)
                    .ConfigureAwait(false);
#endif
                ...
                if (reduced != document)
                {
                    ...
                    {
                        await reduced.VerifyNoErrorsAsync(
        ...
        private async Task<Document> ReduceCoreAsync(
            ...
            if (hasImportsToSimplify)
            {
                ...
                semanticModel = await document.GetSemanticModelAsync(cancellationToken)
                    .ConfigureAwait(false);
                root = await semanticModel.SyntaxTree.GetRootAsync(cancellationToken)
                    .ConfigureAwait(false);
            }
                            ...
                            do
                            {
                                if (
                                    ...
                                    if (
                                        ...
                                        currentNodeOrToken = replacedParent.ChildNodesAndTokens()
                                    ...
                                    if (isNode)
                                    {
                                        ...
                                        if (
                                            ...
                                            semanticModelForReduce =
                                                await newDocument.GetSemanticModelAsync(
                                                        cancellationToken
                                                    )
                                                    .ConfigureAwait(false);
                                            newRoot =
                                                await semanticModelForReduce.SyntaxTree.GetRootAsync(
                                                        cancellationToken
                                                    )
                                                    .ConfigureAwait(false);
                                            currentNodeOrToken = newRoot.DescendantNodes()
