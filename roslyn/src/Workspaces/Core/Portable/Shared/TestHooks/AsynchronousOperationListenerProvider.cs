        ...
        : IAsynchronousOperationListenerProvider
    {
        ...
        )
        {
            ...
            while (true)
            {
                ...
                tasks = waiters.Select(x => x.ExpeditedWaitAsync())
                    .Where(t => !t.IsCompleted)
                    .ToList();

                if (remoteHostClient is not null)
                {
                    var isCompleted = await remoteHostClient.TryInvokeAsync<
        ...
        )
        {
            ...
            return _singletonListeners.Where(kv => featureNames.Contains(kv.Key))
