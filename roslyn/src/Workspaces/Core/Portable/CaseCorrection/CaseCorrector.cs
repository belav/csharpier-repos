...
namespace Microsoft.CodeAnalysis.CaseCorrection
{
    internal static class CaseCorrector
    {
        ...
        )
        {
            ...
            return await CaseCorrectAsync(document, root.FullSpan, cancellationToken)
        ...
        /// </summary>
        public static Task<Document> CaseCorrectAsync(
            Document document,
            ImmutableArray<TextSpan> spans,
            CancellationToken cancellationToken = default
        ) =>
            document
                .GetRequiredLanguageService<ICaseCorrectionService>()
                .CaseCorrectAsync(document, spans, cancellationToken);

        /// <summary>
        /// Case correct only things that don't require semantic information
        /// </summary>
        internal static SyntaxNode CaseCorrect(
            SyntaxNode root,
            ImmutableArray<TextSpan> spans,
            ...
            services
                .GetRequiredLanguageService<ICaseCorrectionService>(root.Language)
                .CaseCorrect(root, spans, cancellationToken);
    }
}
