...
namespace Microsoft.CodeAnalysis.Serialization
{
    internal partial class SerializerService
    {
        ...
        private (
            Metadata metadata,
            ImmutableArray<ITemporaryStreamStorageInternal> storages
        )? TryReadMetadataFrom(
            ObjectReader reader,
            SerializationKinds kind,
            CancellationToken cancellationToken
...
#pragma warning restore CA2016
        ...
        private (
            ModuleMetadata metadata,
            ITemporaryStreamStorageInternal storage
        ...
        private void GetTemporaryStorage(
            ObjectReader reader,
            SerializationKinds kind,
            out ITemporaryStreamStorageInternal storage,
            out long length,
            CancellationToken cancellationToken
        )
        {
            ...
            if (stream is UnmanagedMemoryStream unmanagedStream)
            {
                ...
                unsafe
                {
                    metadata = ModuleMetadata.CreateFromMetadata(
                        (IntPtr)unmanagedStream.PositionPointer,
                        (int)unmanagedStream.Length,
