...
internal sealed class RpcClient
{
    ...
    private readonly ConcurrentDictionary<
        int,
        (TaskCompletionSource<object?>, System.Type? expectedReturnType)
        ...
        Task.Run(async () =>
        {
            ...
            )
            {
                ...
                Contract.ThrowIfFalse(
                    _outstandingRequests.TryRemove(
                        response.Id,
                        out var completionSourceAndExpectedType
                    ),
                    $"We got a response for request ID {response.Id} but that was already completed."
                );
                ...
                if (response.Exception != null)
                {
                    completionSource.SetException(
                        new RemoteInvocationException(response.Exception)
                    );
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            completionSource.SetException(
                                new Exception("Unable to deserialize the result", ex)
                            );
                        }
                    }
                }
    ...
    {
        int requestId;
        var requestCompletionSource = new TaskCompletionSource<object?>(
        ...
        do
        {
            ...
            !_outstandingRequests.TryAdd(requestId, (requestCompletionSource, expectedReturnType))
        );
