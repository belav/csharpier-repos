...
namespace Microsoft.CodeAnalysis
{
    public static class CommandLineProject
    {
        ...
        public static ProjectInfo CreateProjectInfo(
            ...
            {
                analyzerLoader.AddDependencyLocation(
                    relativePathResolver.ResolvePath(path, baseFilePath: null)
                );
            }
            ...
            if (commandLineArguments.AppConfigPath != null)
            {
                try
                {
                    using var appConfigStream = new FileStream(
            ...
            {
                var absolutePath =
                    Path.IsPathRooted(fileArg.Path) || string.IsNullOrEmpty(projectDirectory)
        ...
        private static readonly char[] s_folderSplitters = new char[]
        {
        ...
        private static IList<string> GetFolders(string path)
        {
            ...
            {
                return directory.Split(s_folderSplitters, StringSplitOptions.RemoveEmptyEntries)
                    .ToImmutableArray();
            }
        }
    }
