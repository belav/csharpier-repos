...
namespace Microsoft.CodeAnalysis.Editor.UnitTests.MetadataAsSource
{
    ...
    public abstract partial class AbstractMetadataAsSourceTests : IAsyncLifetime
    {
        ...
        internal static async Task GenerateAndVerifySourceLineAsync(
            string source,
            string language,
            string expected
        )
        ...
        internal static async Task TestNotReusedOnAssemblyDiffersAsync(string projectLanguage)
        {
            ...
            var metadataProject = context
                .CurrentSolution.AddProject(projectId, "Metadata", "Metadata", LanguageNames.CSharp)
            ...
            foreach (var source in metadataSources)
            {
                ...
                references.Add(
                    MetadataReference.CreateFromImage(
                        (await metadataProject.GetCompilationAsync()).EmitToArray()
                    )
                );
            ...
            project = project
                .RemoveMetadataReference(references[0])
                .AddMetadataReference(references[1]);
