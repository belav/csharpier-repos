...
namespace Microsoft.CodeAnalysis.Editor.UnitTests.LinkedFiles
{
    public partial class LinkedFileDiffMergingEditorTests : AbstractCodeActionTest
    {
        ...
        public async Task TestCodeActionPreviewAndApply()
        {
            ...
            var codeIssueOrRefactoring = await GetCodeRefactoringAsync(
                workspace,
                new TestParameters()
            );
        ...
        public async Task TestWorkspaceTryApplyChangesDirectCall()
        {
            ...
            var linkedText = await workspace
                .CurrentSolution.GetDocument(linkedDocumentId)
                .GetTextAsync();
            ...
            Assert.Equal(
                expectedMergedText,
                (await workspace.CurrentSolution.GetDocument(documentId).GetTextAsync()).ToString()
            );
            Assert.Equal(
                expectedMergedText,
                (
        ...
        private class TestCodeRefactoringProvider : CodeRefactorings.CodeRefactoringProvider
        {
            ...
            )
            {
                ...
                var linkedDocument = document
                    .Project.Solution.Projects.Single(p => p != document.Project)
                    .Documents.Single();
                ...
                var newSolution = document
                    ...
                    .WithDocumentText(
                        linkedDocument.Id,
                        (await linkedDocument.GetTextAsync()).Replace(0, 6, "private")
                    );

#pragma warning disable RS0005
                context.RegisterRefactoring(
                    CodeAction.Create("Description", (ct) => Task.FromResult(newSolution)),
                    context.Span
                );
