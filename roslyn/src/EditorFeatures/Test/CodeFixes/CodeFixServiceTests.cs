...
";
            ...
            Assert.IsType<MockDiagnosticUpdateSourceRegistrationService>(
                workspace.GetService<IDiagnosticUpdateSourceRegistrationService>()
            );
        ...
        public async Task TestGetFixesAsyncForFixableAndNonFixableAnalyzersAsync()
        {
            ...
            Assert.Equal(
                codeFix.FixableDiagnosticIds.Single(),
                analyzerWithFix.SupportedDiagnostics.Single().Id
            );

            var analyzerWithoutFix = new MockAnalyzerReference.MockDiagnosticAnalyzer(
                "AnalyzerWithoutFixId",
                "Category"
            );
            ...
            // Verify only analyzerWithFix is executed when GetFixesAsync is invoked with
            ...
            // Verify both analyzerWithFix and analyzerWithoutFix are executed when GetFixesAsync is invoked
            // with 'CodeActionRequestPriority.Lowest'.
            _ = await tuple.codeFixService.GetFixesAsync(
                document,
                TextSpan.FromBounds(0, 0),
                priorityProvider: new DefaultCodeActionRequestPriorityProvider(
        ...
        public async Task TestGetFixesAsyncForDocumentDiagnosticAnalyzerAsync()
        {
            ...
            var documentDiagnosticAnalyzer =
                new MockAnalyzerReference.MockDocumentDiagnosticAnalyzer(
            ...
            // Verify both analyzers are executed when GetFixesAsync is invoked with
            // 'CodeActionRequestPriority.Normal'.
            _ = await tuple.codeFixService.GetFixesAsync(
                document,
                TextSpan.FromBounds(0, 0),
        ...
        public async Task TestGetFixesAsyncForGeneratorDiagnosticAsync()
        {
            ...
            var fixCollectionSet = await tuple.codeFixService.GetFixesAsync(
                document,
                TextSpan.FromBounds(0, 0),
                priorityProvider: new DefaultCodeActionRequestPriorityProvider(
                    CodeActionRequestPriority.Default
        ...
        public async Task TestGetCodeFixWithExceptionInRegisterMethod_Diagnostic()
        {
            await GetFirstDiagnosticWithFixWithExceptionValidationAsync(
                new ErrorCases.ExceptionInRegisterMethod()
            );
        }
        ...
        {
            await GetAddedFixesWithExceptionValidationAsync(
                new ErrorCases.ExceptionInRegisterMethod()
            );
        }
        ...
        {
            await GetFirstDiagnosticWithFixWithExceptionValidationAsync(
                new ErrorCases.ExceptionInRegisterMethodAsync()
            );
        }
        ...
        {
            await GetFirstDiagnosticWithFixWithExceptionValidationAsync(
                new ErrorCases.ExceptionInFixableDiagnosticIds2()
            );
        }
        ...
        private static Task<
            ImmutableArray<CodeFixCollection>
        ...
        private static (
            ...
            ServiceSetup(
                ImmutableArray.Create(codefix),
                includeConfigurationFixProviders,
                throwExceptionInFixerCreation,
                additionalDocument,
                code
            );
        ...
        )
        {
            ...
            var logger = SpecializedCollections.SingletonEnumerable(
                new Lazy<IErrorLoggerService>(() => new TestErrorLogger())
            );
        ...
        )
        {
            ...
            // register diagnostic engine to solution crawler
            diagnosticIncrementalAnalyzer = (DiagnosticIncrementalAnalyzer)
        ...
        private class MockAnalyzerReference : AnalyzerReference, ICodeFixProviderFactory
        {
            ...
            private static readonly ImmutableArray<CodeFixProvider> s_defaultFixers =
                ImmutableArray.Create<CodeFixProvider>(new MockFixer());
            ...
            {
                get { return "MockAnalyzerReference"; }
            }
            ...
            public ImmutableArray<CodeFixProvider> GetFixers() => Fixers;
                ...
                public void Execute(GeneratorExecutionContext context)
                {
                    foreach (var tree in context.Compilation.SyntaxTrees)
                    {
                        context.ReportDiagnostic(
                            Diagnostic.Create(s_descriptor, tree.GetLocation(new TextSpan(0, 1)))
                        );
                    }
                }
            }
        ...
        )
        {
            ...
            var fixTitles = fixes
                ...
                .Select(f => f.Action.Title)
                .ToHashSet();
            Assert.Equal(
                expectedNuGetFixerCodeActionWasRegistered,
                fixTitles.Contains(nameof(NuGetCodeFixProvider))
            );
            Assert.Equal(
                expectedVsixFixerCodeActionWasRegistered,
                fixTitles.Contains(nameof(VsixCodeFixProvider))
            );
        }
        ...
        )
        {
            ...
            return await fixService.GetFixesAsync(
                document,
                TextSpan.FromBounds(0, 0),
                CodeActionOptions.DefaultProvider,
                CancellationToken.None
        ...
        public async Task TestRegisteredCodeActionsWithSameEquivalenceKey(string? equivalenceKey)
        {
            ...
            var analyzer = new MockAnalyzerReference.MockDiagnosticAnalyzer(
                ImmutableArray.Create(diagnosticId)
            );
            var fixer = new CodeFixProviderWithDuplicateEquivalenceKeyActions(
                diagnosticId,
                equivalenceKey
            );

            // Verify multiple code actions registered with same equivalence key are not de-duped.
            var fixes = (await GetAddedFixesAsync(fixer, analyzer))
                .SelectMany(fixCollection => fixCollection.Fixes)
                .ToList();
        ...
        public async Task TestAdditionalDocumentCodeFixAsync()
        {
            ...
            var tuple = ServiceSetup(
                fixers,
                additionalDocument: new TestHostDocument(
                    "Additional Document",
                    filePath: "test.txt"
                )
            );
            ...
            var txtDocumentCodeFixes = await tuple.codeFixService.GetFixesAsync(
                txtDocument,
            ...
            var txtDocumentCodeFixTitles = txtDocumentCodeFixes
                .Select(s => s.Fixes.Single().Action.Title)
                .ToImmutableArray();
            ...
            Assert.Equal(
                $"Additional Document{fixer1.Title}",
                changedtxtDocument.GetTextSynchronously(CancellationToken.None).ToString()
            );
            ...
            var logDocumentCodeFixes = await tuple.codeFixService.GetFixesAsync(
                logDocument,
                TextSpan.FromBounds(0, 1),
                CodeActionOptions.DefaultProvider,
                CancellationToken.None
        ...
        internal abstract class AbstractAdditionalFileCodeFixProvider : CodeFixProvider
        {
            ...
            protected AbstractAdditionalFileCodeFixProvider(string title) => Title = title;
...
#pragma warning disable RS0034 // Exported parts should be marked with 'ImportingConstructorAttribute'
        ...
        [Shared]
        internal sealed class AdditionalFileFixerWithDocumentKindsAndExtensions
            : AbstractAdditionalFileCodeFixProvider
        {
            public AdditionalFileFixerWithDocumentKindsAndExtensions()
                : base(nameof(AdditionalFileFixerWithDocumentKindsAndExtensions)) { }
        }
        ...
        [Shared]
        internal sealed class AdditionalFileFixerWithDocumentKinds
            : AbstractAdditionalFileCodeFixProvider
        {
            public AdditionalFileFixerWithDocumentKinds()
                : base(nameof(AdditionalFileFixerWithDocumentKinds)) { }
        }

        [ExportCodeFixProvider(LanguageNames.CSharp, DocumentExtensions = new[] { ".txt" })]
        [Shared]
        internal sealed class AdditionalFileFixerWithDocumentExtensions
            : AbstractAdditionalFileCodeFixProvider
        {
            public AdditionalFileFixerWithDocumentExtensions()
                : base(nameof(AdditionalFileFixerWithDocumentExtensions)) { }
        }

        [ExportCodeFixProvider(LanguageNames.CSharp)]
        [Shared]
        internal sealed class AdditionalFileFixerWithoutDocumentKindsAndExtensions
            : AbstractAdditionalFileCodeFixProvider
        {
            public AdditionalFileFixerWithoutDocumentKindsAndExtensions()
                : base(nameof(AdditionalFileFixerWithoutDocumentKindsAndExtensions)) { }
        }
#pragma warning restore RS0034 // Exported parts should be marked with 'ImportingConstructorAttribute'
        ...
        {
            // This test validates analyzer de-prioritization logic in diagnostic service for lightbulb code
            // path.
            // Basically, we have a certain set of heuristics (detailed in the next comment below), under which
            // an analyzer
            // which is deemed to be an expensive analyzer is moved down from 'Normal' priority code fix bucket
            // to
            // 'Low' priority bucket to improve lightbulb performance. This test validates this logic by
            // performing
            // the following steps:
            //  1. Use 2 snapshots of document, such that the analyzer has a reported diagnostic on the code fix
            // trigger
            //     line in the earlier snapshot based on the flag 'diagnosticOnFixLineInPriorSnapshot'
            //  2. For the second snapshot, mimic whether or not background analysis has computed and cached
            // full document
            //     diagnostics for the document based on the flag 'testWithCachedDiagnostics'.
            //  3. Apply an edit in the second snapshot on the code fix trigger line based on the flag
            // 'editOnFixLine'.
            //     If this flag is false, edit is apply at a different line in the document.
            //  4. Add a new line edit in the second snapshot based on the flag 'addNewLineWithEdit'. This tests
            // the part
            //     of the heuristic where we compare intersecting diagnostics across document snapshots only if
            // both
            //     snapshots have the same number of lines.

            var expectDeprioritization = GetExpectDeprioritization(
                actionKind,
                testWithCachedDiagnostics,
                diagnosticOnFixLineInPriorSnapshot,
                addNewLineWithEdit
            );

            var priorSnapshotFixLine = diagnosticOnFixLineInPriorSnapshot
                ? "int x1 = 0;"
    ...
    {{
        {priorSnapshotFixLine}
            ...
            GetDocumentAndExtensionManager(
                ...
                out var document,
                out var extensionManager,
                out var diagnosticIncrementalAnalyzer,
                analyzerReference
            );
            ...
            var testSpan = diagnosticOnFixLineInPriorSnapshot
                ? root.DescendantNodes()
                    ...
                    .Span
                : root.DescendantNodes()
                    .OfType<CodeAnalysis.CSharp.Syntax.InvocationExpressionSyntax>()
                    .First()
                    .Span;

            // Trigger background analysis to ensure analyzer diagnostics are computed and cached.
            // We enable full solution analysis so the 'AnalyzeDocumentAsync' doesn't skip analysis based on
            // whether the document is active/open.
            workspace.GlobalOptions.SetGlobalOption(
                SolutionCrawlerOptionsStorage.BackgroundAnalysisScopeOption,
                LanguageNames.CSharp,
                BackgroundAnalysisScope.FullSolution
            );
            await diagnosticIncrementalAnalyzer.AnalyzeDocumentAsync(
                sourceDocument,
                bodyOpt: null!,
            ...
            await VerifyCachedDiagnosticsAsync(
                sourceDocument,
                expectedCachedDiagnostic: diagnosticOnFixLineInPriorSnapshot,
                testSpan,
            ...
            {
                // Add a new line at the start of the document to ensure the line with diagnostic in prior snapshot
                // moved.
            ...
            if (testWithCachedDiagnostics)
            {
                ...
                await VerifyCachedDiagnosticsAsync(
                    sourceDocument,
                    expectedCachedDiagnostic: !expectedNoFixes,
                    testSpan,
                    diagnosticIncrementalAnalyzer
                );
            }
            ...
            var normalPriFixes = await tuple.codeFixService.GetFixesAsync(
                sourceDocument,
                testSpan,
                priorityProvider,
                CodeActionOptions.DefaultProvider,
            ...
            var lowPriFixes = await tuple.codeFixService.GetFixesAsync(
                sourceDocument,
                testSpan,
                priorityProvider,
                CodeActionOptions.DefaultProvider,
            ...
            {
                // We expect de-prioritization of analyzer from 'Normal' to 'Low' bucket only if following
                // conditions are met:
                //  1. There are no cached diagnostics from background analysis, i.e. 'testWithCachedDiagnostics ==
                // false'.
                //  2. We have an expensive analyzer that registers SymbolStart/End or SemanticModel actions, both
                // of which have a broad analysis scope.
                //  3. Either of the below is true:
                //     a. We do not have an analyzer diagnostic reported in the prior document snapshot on the
                // edited line OR
                //     b. Number of lines in the prior document snapshot differs from number of lines in the current
                // document snapshot,
                ...
                if (
                    actionKind
                    is not (
                        DeprioritizedAnalyzer.ActionKind.SymbolStartEnd
                        or DeprioritizedAnalyzer.ActionKind.SemanticModel
                    )
                )
                    return false;
                        ...
                        context.RegisterSymbolStartAction(
                            context =>
                            {
                                ...
                                context.RegisterOperationAction(
                                    context => variableDeclarations.Add(context.Operation.Syntax),
                                ...
                                context.RegisterSymbolEndAction(context =>
                                {
                                    foreach (var decl in variableDeclarations)
                                        context.ReportDiagnostic(
                                            Diagnostic.Create(Descriptor, decl.GetLocation())
                                        );
                                });
                            },
        ...
        private sealed class FixerForDeprioritizedAnalyzer : CodeFixProvider
        {
            ...
            public override ImmutableArray<string> FixableDiagnosticIds =>
                ImmutableArray.Create(DeprioritizedAnalyzer.Descriptor.Id);
