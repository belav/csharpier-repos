    ...
    public class FileSystemCompletionHelperTests
    {
        ...
        )
        {
            AssertEx.Equal(
                expected,
                actual.Select(c =>
                    $"'{c.DisplayText}', {string.Join(", ", c.Tags)}, '{c.GetProperty(CommonCompletionItem.DescriptionProperty)}'"
                ),
                itemInspector: c => $"@\"{c}\""
            );
        ...
        public void GetItems_Windows1()
        {
            ...
            AssertItemsEqual(
                fsc.GetTestAccessor().GetItems("", CancellationToken.None),
            ...
            AssertItemsEqual(
                fsc.GetTestAccessor().GetItems(@"X:\A\", CancellationToken.None),
                @"'1', Folder, 'Text|X:\5CA\5C1'",
                @"'2', Folder, 'Text|X:\5CA\5C2'",
                @"'3', Folder, 'Text|X:\5CA\5C3'"
            );
            ...
            AssertItemsEqual(
                fsc.GetTestAccessor().GetItems(@"X:\B\", CancellationToken.None),
                @"'file5.abc', File, C#, 'Text|X:\5CB\5Cfile5.abc'"
            );

            AssertItemsEqual(
                fsc.GetTestAccessor().GetItems(@"Z:\", CancellationToken.None),
                @"'C', Folder, 'Text|Z:\5CC'",
                @"'D', Folder, 'Text|Z:\5CD'"
            );
        ...
        public void GetItems_Windows_NoBaseDirectory()
        {
            var fsc = new TestFileSystemCompletionHelper(
                ...
                allowableExtensions: ImmutableArray.Create(".abc", ".def"),
                drives: new[] { @"X:\" },
                directories: new[] { @"X:", @"X:\A", @"X:\A\1", @"X:\A\2", @"X:\A\3", @"X:\B" },
                files: new[]
                {
                    @"X:\A\1\file1.abc",
                    @"X:\A\2\file2.abc",
        ...
        public void GetItems_Windows_Network()
        {
            var fsc = new TestFileSystemCompletionHelper(
                ...
                directories: new[] { @"\\server\share", @"\\server\share\C", @"\\server\share\D" },
                files: new[]
                {
                    @"\\server\share\C\b.cs",
                    @"\\server\share\C\c.cs",
            ...
            AssertItemsEqual(
                fsc.GetTestAccessor().GetItems(@"\\server\share\", CancellationToken.None),
                @"'C', Folder, 'Text|\5C\5Cserver\5Cshare\5CC'",
                @"'D', Folder, 'Text|\5C\5Cserver\5Cshare\5CD'"
            );

            AssertItemsEqual(
                fsc.GetTestAccessor().GetItems(@"\\server\share\C\", CancellationToken.None),
        ...
        public void GetItems_Unix1()
        {
            var fsc = new TestFileSystemCompletionHelper(
                ...
                allowableExtensions: ImmutableArray.Create(".abc", ".def"),
                drives: Array.Empty<string>(),
                directories: new[] { @"/A", @"/A/1", @"/A/2", @"/A/3", @"/B", @"/C", @"/D" },
                files: new[]
                {
                    @"/A/1/file1.abc",
                    @"/A/2/file2.abc",
            ...
            AssertItemsEqual(
                fsc.GetTestAccessor().GetItems(@"/B/", CancellationToken.None),
                @"'file5.abc', File, C#, 'Text|/B/file5.abc'"
            );
        }
    }
