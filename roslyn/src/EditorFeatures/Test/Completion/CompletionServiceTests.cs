...
";
            ...
            var expectedChange = new TextChange(
                item.Span,
                nameof(DebugAssertTestCompletionProvider)
            );
            var actualChange = (
        ...
        private sealed class DebugAssertTestCompletionProvider : CompletionProvider
        {
            ...
            public override async Task ProvideCompletionsAsync(CompletionContext context)
            {
                var completionItem = CompletionItem.Create(
                    displayText: nameof(DebugAssertTestCompletionProvider),
                    displayTextSuffix: "",
            ...
            )
            {
                return Task.FromResult(
                    CompletionDescription.FromText(nameof(DebugAssertTestCompletionProvider))
                );
            }
            ...
            )
            {
                return Task.FromResult(
                    CompletionChange.Create(
                        new TextChange(item.Span, nameof(DebugAssertTestCompletionProvider))
                    )
                );
            }
