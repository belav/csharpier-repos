...
namespace Microsoft.CodeAnalysis.Editor.UnitTests.Tagging
{
    ...
    public class TagSpanIntervalTreeTests
    {
        ...
        {
            var exportProvider = EditorTestCompositions.Editor.ExportProviderFactory
                .CreateExportProvider();
            var buffer = EditorFactory.CreateBuffer(exportProvider, text);
            var tags = spans.Select(
                s =>
                    new TagSpan<ITextMarkerTag>(
                        new SnapshotSpan(buffer.CurrentSnapshot, s),
                        new TextMarkerTag(string.Empty)
                    )
            );
            return new TagSpanIntervalTree<ITextMarkerTag>(
                buffer,
        ...
        public void TestSingleIntersectingSpanAtStartWithEdit()
        {
            ...
            var spans = tree.GetIntersectingSpans(
        ...
        public void TestManySpansWithEdit()
        {
            ...
            var tree = CreateTree(
                new string('c', 100),
                Enumerable.Range(50, count: 50).Select(s => new Span(s, 1)).ToArray()
            );
            tree.Buffer.Insert(0, new string('c', 100));
            ...
            Assert.Equal(
                50,
                tree.GetIntersectingSpans(new SnapshotSpan(tree.Buffer.CurrentSnapshot, 150, 50))
                    .Count()
            );
            ...
            Assert.Equal(
                26,
                tree.GetIntersectingSpans(new SnapshotSpan(tree.Buffer.CurrentSnapshot, 175, 25))
                    .Count()
            );
        }
        ...
        public void TestManySpansWithEdit2()
        {
            ...
            var tree = CreateTree(
                new string('c', 100),
                Enumerable.Range(0, count: 100).Select(s => new Span(s, 1)).ToArray()
            );
            tree.Buffer.Insert(0, new string('c', 100));
            ...
            Assert.Equal(
                1,
                tree.GetIntersectingSpans(new SnapshotSpan(tree.Buffer.CurrentSnapshot, 0, 1))
                    .Count()
            );
            Assert.Equal(
                1,
                tree.GetIntersectingSpans(new SnapshotSpan(tree.Buffer.CurrentSnapshot, 50, 1))
                    .Count()
            );
            ...
            Assert.Equal(
                2,
                tree.GetIntersectingSpans(new SnapshotSpan(tree.Buffer.CurrentSnapshot, 100, 1))
                    .Count()
            );
        }
        ...
        public void TestManySpansWithDeleteAndEditAtEnd()
        {
            ...
            Assert.Equal(
                51,
                tree.GetIntersectingSpans(new SnapshotSpan(tree.Buffer.CurrentSnapshot, 99, 1))
                    .Count()
            );
        }
        ...
        public void TestTagSpanOrdering()
        {
            ...
            var lastStart = -1;
            foreach (
                var tag in tree.GetIntersectingSpans(
                    new SnapshotSpan(
