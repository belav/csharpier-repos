    ...
    public class WorkCoordinatorTests : TestBase
    {
        ...
        internal async Task Project_Change(BackgroundAnalysisScope analysisScope)
        {
            ...
            var worker = await ExecuteOperation(
                workspace,
                w => w.ChangeProject(project.Id, solution)
            );
            Assert.Equal(0, worker.SyntaxDocumentIds.Count);
        ...
        )
        {
            ...
            var worker = await ExecuteOperation(
                workspace,
                w => w.ChangeProject(project.Id, project.Solution)
            );
        ...
        )
        {
            ...
            var worker = await ExecuteOperation(
                workspace,
                w => w.ChangeDocument(document.Id, SourceText.From("//"))
            );
