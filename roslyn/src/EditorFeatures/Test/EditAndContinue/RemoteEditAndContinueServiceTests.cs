...
namespace Roslyn.VisualStudio.Next.UnitTests.EditAndContinue
{
    ...
    public class RemoteEditAndContinueServiceTests
    {
        private static string Inspect(DiagnosticData d) =>
            ...
            + (
                !string.IsNullOrWhiteSpace(d.DataLocation.UnmappedFileSpan.Path)
                    ? $" {d.DataLocation.UnmappedFileSpan.Path}({d.DataLocation.UnmappedFileSpan.StartLinePosition.Line}, {d.DataLocation.UnmappedFileSpan.StartLinePosition.Character}, {d.DataLocation.UnmappedFileSpan.EndLinePosition.Line}, {d.DataLocation.UnmappedFileSpan.EndLinePosition.Character}):"
                    : ""
            )
            + $" {d.Message}";
        ...
        public async Task Proxy(TestHost testHost)
        {
            ...
            else
            {
                ...
                clientProvider!.AdditionalRemoteParts = new[]
                {
                    typeof(MockEditAndContinueWorkspaceService)
                };
            ...
            var mockDiagnosticService = (MockDiagnosticAnalyzerService)
                localWorkspace.GetService<IDiagnosticAnalyzerService>();
            ...
            var activeStatementSpanProvider = new ActiveStatementSpanProvider(
                (documentId, path, cancellationToken) =>
                {
            ...
            var diagnosticDescriptor = EditAndContinueDiagnosticDescriptors.GetDescriptor(
                EditAndContinueErrorCode.AddingTypeRuntimeCapabilityRequired
            );
            var diagnostic = Diagnostic.Create(
                diagnosticDescriptor,
                Location.Create(syntaxTree, TextSpan.FromBounds(1, 1))
            );
            ...
            var debuggingSession = mockEncService.StartDebuggingSessionImpl = (
                solution,
                debuggerService,
                sourceTextProvider,
            ...
            await sessionProxy.BreakStateOrCapabilitiesChangedAsync(
            ...
            var availability = await remoteDebuggeeModuleMetadataProvider!.GetAvailabilityAsync(
                moduleId1,
                CancellationToken.None
            );
            Assert.Equal(
            ...
            mockEncService.EmitSolutionUpdateImpl = (solution, activeStatementSpanProvider) =>
            {
                ...
                AssertEx.Equal(
                    activeSpans1,
                    activeStatementSpanProvider(documentId, "test.cs", CancellationToken.None)
                        .AsTask()
                        .Result
                );
                ...
                var syntaxError = Diagnostic.Create(
                    diagnosticDescriptor1,
                    Location.Create(syntaxTree, TextSpan.FromBounds(1, 2)),
                    new[] { "doc", "syntax error" }
                );
                ...
                var documentsWithRudeEdits = ImmutableArray.Create(
                    (documentId, ImmutableArray<RudeEditDiagnostic>.Empty)
                );

                return new(updates, diagnostics, documentsWithRudeEdits, syntaxError);
            };

            var (updates, _, _, syntaxErrorData) = await sessionProxy.EmitSolutionUpdateAsync(
                localWorkspace.CurrentSolution,
            ...
            AssertEx.Equal(
                $"[{projectId}] Error ENC1001: test.cs(0, 1, 0, 2): {string.Format(FeaturesResources.ErrorReadingFile, "doc", "syntax error")}",
                Inspect(syntaxErrorData!)
            );
            ...
            await sessionProxy.CommitSolutionUpdateAsync(
                mockDiagnosticService,
                CancellationToken.None
            );
            VerifyReanalyzeInvocation(ImmutableArray.Create(documentId));
            ...
            Assert.Equal(
                span1,
                await sessionProxy.GetCurrentActiveStatementPositionAsync(
                    localWorkspace.CurrentSolution,
                    activeStatementSpanProvider,
                    instructionId1,
                    CancellationToken.None
            ...
            Assert.True(
                await sessionProxy.IsActiveStatementInExceptionRegionAsync(
                    localWorkspace.CurrentSolution,
                    instructionId1,
                    CancellationToken.None
                )
            ...
            var activeStatementSpan1 = new ActiveStatementSpan(
                0,
                span1,
                ActiveStatementFlags.NonLeafFrame | ActiveStatementFlags.PartiallyExecuted,
            ...
            var baseActiveSpans = await sessionProxy.GetBaseActiveStatementSpansAsync(
                localWorkspace.CurrentSolution,
                ImmutableArray.Create(documentId, inProcOnlyDocumentId),
                CancellationToken.None
            );
            ...
            Assert.Empty(
                await sessionProxy.GetAdjustedActiveStatementSpansAsync(
                    inProcOnlyDocument,
                    activeStatementSpanProvider,
                    CancellationToken.None
                )
            ...
            var documentActiveSpans = await sessionProxy.GetAdjustedActiveStatementSpansAsync(
                document,
                activeStatementSpanProvider,
                CancellationToken.None
            );
            ...
            documentActiveSpans = await sessionProxy.GetAdjustedActiveStatementSpansAsync(
                document,
                activeStatementSpanProvider,
                CancellationToken.None
            );
            ...
            mockEncService.GetDocumentDiagnosticsImpl = (document, activeStatementProvider) =>
                ImmutableArray.Create(diagnostic);

            Assert.Empty(
                await proxy.GetDocumentDiagnosticsAsync(
                    inProcOnlyDocument,
                    inProcOnlyDocument,
                    activeStatementSpanProvider,
                    CancellationToken.None
            ...
            await sessionProxy.EndDebuggingSessionAsync(
                solution,
                diagnosticUpdateSource,
                mockDiagnosticService,
                CancellationToken.None
