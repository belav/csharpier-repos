...
namespace Roslyn.VisualStudio.Next.UnitTests.EditAndContinue
{
    ...
    public class RemoteEditAndContinueServiceTests
    {
        ...
        public async Task Proxy(TestHost testHost)
        {
            ...
            var (updates, _, _, syntaxErrorData) = await sessionProxy
                ...
                .ConfigureAwait(false);
            AssertEx.Equal(
                $"[{project.Id}] Error ENC1001: test.cs(0, 1, 0, 2): {string.Format(FeaturesResources.ErrorReadingFile, "doc", "syntax error")}",
            ...
            await sessionProxy
                .DiscardSolutionUpdateAsync(CancellationToken.None)
                .ConfigureAwait(false);
            Assert.True(called);
            ...
            var activeStatementSpan1 = new ActiveStatementSpan(
                ...
                unmappedDocumentId: document1.Id
            );
            ...
            var baseActiveSpans = await sessionProxy
            ...
            var documentActiveSpans = await sessionProxy
            ...
            documentActiveSpans = await sessionProxy
                .GetAdjustedActiveStatementSpansAsync(
                    document1,
                    activeStatementSpanProvider,
                    CancellationToken.None
