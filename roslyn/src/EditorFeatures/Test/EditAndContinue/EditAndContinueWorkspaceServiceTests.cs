    ...
    public sealed partial class EditAndContinueWorkspaceServiceTests : TestBase
    {
        ...
        public async Task StartDebuggingSession_CapturingDocuments(bool captureAllDocuments)
        {
            ...
            using var _ = CreateWorkspace(
                out var solution,
                out var service,
                new[] { typeof(DummyLanguageService) }
            );
        ...
        public async Task DifferentDocumentWithSameContent()
        {
            ...
            Assert.False(
                await debuggingSession.EditSession.HasChangesAsync(
                    solution,
                    s_noActiveSpans,
                    sourceFilePath: null,
                    CancellationToken.None
        ...
        public async Task DesignTimeOnlyDocument_Wpf(bool delayLoad)
        {
            ...
            solution = solution.WithDocumentText(
                documentB.Id,
                SourceText.From("class B { public void RenamedMethod() { } }")
            );
            solution = solution.WithDocumentText(
                documentC.Id,
                SourceText.From("class C { public void RenamedMethod() { } }")
            );
        ...
        public async Task RudeEdits(bool breakMode)
        {
            ...
            AssertEx.Equal(
                new[]
                {
                    "ENC0020: "
                        + string.Format(
                            FeaturesResources.Renaming_0_requires_restarting_the_application,
...
";
        ...
        public async Task RudeEdits_DelayLoadedModule()
        {
            ...
            solution = solution.WithDocumentText(
                document1.Id,
                SourceText.From("class C { public void Renamed() { } }")
            );
            var document2 = solution.Projects.Single().Documents.Single();
        ...
        public async Task FileStatus_CompilationError()
        {
            ...
            solution = solution
                ...
                .AddDocument("Common.cs", "class Common {}", filePath: "Common.cs")
                .Project.AddDocument("B.cs", "class B {}", filePath: "B.cs")
                .Project.Solution.AddProject("C", "C", "C#")
                .AddDocument("Common.cs", "class Common {}", filePath: "Common.cs")
                .Project.AddDocument("C.cs", "class C {}", filePath: "C.cs")
                .Project.Solution;
...
";
        ...
        public async Task ValidSignificantChange_ApplyBeforeFileWatcherEvent(bool saveDocument)
        {
            ...
            var hasChanges = await debuggingSession.EditSession.HasChangesAsync(
                solution,
                s_noActiveSpans,
                sourceFilePath: null,
                CancellationToken.None
        ...
        public async Task ValidSignificantChange_EmitSuccessful(bool breakMode, bool commitUpdate)
        {
            ...
            if (commitUpdate)
            {
                ...
                var commitedUpdateSolutionStatus =
                    await debuggingSession.EditSession.HasChangesAsync(
                        solution,
                        s_noActiveSpans,
                        sourceFilePath: null,
            ...
            else
            {
                ...
                // solution update status after committing an update:
                var discardedUpdateSolutionStatus =
                    await debuggingSession.EditSession.HasChangesAsync(
                        solution,
                        s_noActiveSpans,
                        sourceFilePath: null,
...
";
        ...
        public async Task TwoUpdatesWithLoadedAndUnloadedModule()
        {
            ...
            var projectB = solution
                ...
                .AddDocument("DocB", source1, filePath: "DocB.cs")
                .Project;
        ...
        public async Task ActiveStatements_ForeignDocument(bool withPath, bool designTimeOnly)
        {
            ...
            var project = solution.AddProject(
                "dummy_proj",
                "dummy_proj",
                designTimeOnly ? LanguageNames.CSharp : DummyLanguageService.LanguageName
            );
