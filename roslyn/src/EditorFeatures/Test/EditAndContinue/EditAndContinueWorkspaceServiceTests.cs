...
namespace Microsoft.CodeAnalysis.EditAndContinue.UnitTests
{
    ...
    public sealed partial class EditAndContinueWorkspaceServiceTests : TestBase
    {
        ...
        private TestWorkspace CreateWorkspace(
            ...
            out EditAndContinueService service,
        ...
        )
        {
            var composition = EditorTestCompositions
                .EditorFeatures.RemoveParts(typeof(MockWorkspaceEventListenerProvider))
        ...
        )
        {
            ...
            session
                ...
                .SetTelemetryLogger(
                    (id, message) =>
                        _telemetryLog.Add(
                            $"{id}: {s_timePropertiesRegex.Replace(message.GetMessage(), "")}"
                        ),
                    () => ++_telemetryId
                );
        ...
        public async Task RudeEdits(bool breakMode)
        {
            ...
            AssertEx.Equal(
                new[]
                {
                    "ENC0110: "
                        + string.Format(
                            FeaturesResources.Changing_the_signature_of_0_requires_restarting_the_application_because_it_is_not_supported_by_the_runtime,
...
";
        ...
        public async Task HasChanges_Documents(DocumentKind documentKind)
        {
                ...
                ExecuteImpl = context =>
                {
                    switch (documentKind)
                    {
                        ...
                        case DocumentKind.AnalyzerConfig:
                            var syntaxTree = context.Compilation.SyntaxTrees.Single(t =>
                                t.FilePath.EndsWith("A.cs")
                            );
                            var content = context
                                .AnalyzerConfigOptions.GetOptions(syntaxTree)
            ...
            {
                DocumentKind.Source => solution.AddDocument(
                    documentId,
                    "X",
                    CreateText("xxx"),
                    filePath: pathX
                ),
                DocumentKind.Additional => solution.AddAdditionalDocument(
                    documentId,
                    "X",
                    CreateText("xxx"),
                    filePath: pathX
                ),
                DocumentKind.AnalyzerConfig => solution.AddAnalyzerConfigDocument(
                    documentId,
                    "X",
            ...
            solution = documentKind switch
            {
                ...
                DocumentKind.Additional => solution.WithAdditionalDocumentText(
                    documentId,
                    CreateText("xxx")
                ),
                DocumentKind.AnalyzerConfig => solution.WithAnalyzerConfigDocumentText(
                    documentId,
                    GetAnalyzerConfigText(new[] { ("x", "1") })
                ),
                _ => throw ExceptionUtilities.Unreachable(),
            ...
            solution = documentKind switch
            {
                DocumentKind.Source => solution.WithDocumentText(
                    documentId,
                    CreateText("xxx-changed")
                ),
                DocumentKind.Additional => solution.WithAdditionalDocumentText(
                    documentId,
                    CreateText("xxx-changed")
...
";
        ...
        public async Task TwoUpdatesWithLoadedAndUnloadedModule()
        {
            ...
            var newBaselineA1 = pendingUpdate
                .ProjectBaselines.Single(b => b.ProjectId == projectA.Id)
                .EmitBaseline;
            var newBaselineB1 = pendingUpdate
                .ProjectBaselines.Single(b => b.ProjectId == projectB.Id)
                .EmitBaseline;
                ...
                @"class Test1
{
    ...
    static void F() => <AS:1>Project4::Test2.M();</AS:1>
            ...
            var baseActiveStatementsMap = await debuggingSession
                .EditSession.BaseActiveStatements.GetValueAsync(CancellationToken.None)
                ...
                @"class Test
{
    ...
    static void F()
    {
        ...
        <AS:1>G();</AS:1>
            ...
            (solution, var document) = AddDefaultTestProject(
                solution,
                SourceMarkers.Clear(markedSourceV1)
            );
            ...
            solution = solution.WithDocumentText(
                documentId,
                CreateText(SourceMarkers.Clear(markedSourceV2))
            );
            ...
            solution = solution.WithDocumentText(
                documentId,
                CreateText(SourceMarkers.Clear(markedSourceV3))
            );
            ...
            solution = solution.WithDocumentText(
                documentId,
                CreateText(SourceMarkers.Clear(markedSourceV4))
            );
            ...
            AssertEx.Equal(
                new[] { $"0x06000002 v1 | AS {document.FilePath}: (4,41)-(4,42) => (4,41)-(4,42)" },
                InspectNonRemappableRegions(debuggingSession.EditSession.NonRemappableRegions)
            );
                ...
                @"class Test
{
    ...
    static void F()
    {
        ...
        <AS:1>G();</AS:1>
            ...
            (solution, var document) = AddDefaultTestProject(
                solution,
                SourceMarkers.Clear(markedSource1)
            );
            var documentId = document.Id;
            ...
            solution = solution.WithDocumentText(
                documentId,
                CreateText(SourceMarkers.Clear(markedSource2))
            );
            ...
            solution = solution.WithDocumentText(
                documentId,
                CreateText(SourceMarkers.Clear(markedSource3))
            );
