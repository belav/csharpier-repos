...
namespace Microsoft.CodeAnalysis.EditAndContinue.UnitTests
{
    ...
    public sealed partial class EditAndContinueWorkspaceServiceTests : TestBase
    {
        ...
        private static readonly DocumentActiveStatementSpanProvider s_noDocumentActiveSpans = _ =>
            new(ImmutableArray<TextSpan>.Empty);
        ...
        private EditAndContinueWorkspaceService GetEditAndContinueService(Workspace workspace)
        {
            ...
            accessor.SetReportTelemetry(
                data =>
                    EditAndContinueWorkspaceService.LogDebuggingSessionTelemetry(
                        data,
        ...
        public async Task StartDebuggingSession_CapturingDocuments()
        {
            ...
            AssertEx.Equal(
                ...
                matchingDocuments.Select(e => (solution.GetDocument(e.id).Name, e.state))
                    .OrderBy(e => e.Name)
                    .Select(e => e.ToString())
            );
        ...
        public async Task RunMode_DesignTimeOnlyDocument()
        {
            ...
            solution = solution.WithProjectOutputFilePath(document1.Project.Id, moduleFile.Path)
            ...
            Assert.False(
                await service.HasChangesAsync(
                    solution,
                    s_noSolutionActiveSpans,
                    sourceFilePath: null,
                    CancellationToken.None
                )
            );
            ...
            AssertEx.Equal(
                new[] { "Debugging_EncSession: SessionId=1|SessionCount=0|EmptySessionCount=0" },
                _telemetryLog
            );
        }
        ...
        public async Task BreakMode_DesignTimeOnlyDocument_Wpf(bool delayLoad)
        {
            ...
            var documentA = solution.AddProject("test", "test", LanguageNames.CSharp)
        ...
        public async Task BreakMode_ErrorReadingPdbFile()
        {
            ...
            var document1 = solution.AddProject("test", "test", LanguageNames.CSharp)
                .AddMetadataReferences(
                    TargetFrameworkUtil.GetReferences(TargetFramework.Mscorlib40)
                )
                .AddDocument(
            ...
            await StartDebuggingSessionAsync(
                service,
                solution,
            ...
            AssertEx.Equal(
                new[] { "Debugging_EncSession: SessionId=1|SessionCount=0|EmptySessionCount=1" },
                _telemetryLog
            );
        }
    ...
    System.Console.WriteLine(30); 
            ...
            LoadLibraryToDebuggee(
                ...
                new ManagedEditAndContinueAvailability(
                    ManagedEditAndContinueAvailabilityStatus.NotAllowedForRuntime,
                    "*message*"
                )
            );
...
";
        ...
        public async Task BreakMode_ValidSignificantChange_ApplyBeforeFileWatcherEvent(
            ...
            var hasChanges = await service.HasChangesAsync(
                solution,
                s_noSolutionActiveSpans,
                sourceFilePath: null,
                CancellationToken.None
        ...
        public async Task BreakMode_ValidSignificantChange_FileUpdateNotObservedBeforeDebuggingSessionStart()
        {
            ...
            // the workspace starts with a version of the source that's not updated with the output of single file generator (or design-time build):
            var document2 = solution.AddProject("test", "test", LanguageNames.CSharp)
                .AddMetadataReferences(
                    TargetFrameworkUtil.GetReferences(TargetFramework.Mscorlib40)
                )
                .AddDocument(
        ...
        public async Task BreakMode_ValidSignificantChange_AddedFileNotObservedBeforeDebuggingSessionStart()
        {
            ...
            _debuggerService.IsEditAndContinueAvailable = _ =>
                new ManagedEditAndContinueAvailability(
                    ManagedEditAndContinueAvailabilityStatus.Attach,
        ...
        public async Task ValidSignificantChange_EmitSuccessful(bool breakMode, bool commitUpdate)
        {
            ...
            if (commitUpdate)
            {
                ...
                var commitedUpdateSolutionStatus = await service.HasChangesAsync(
                    solution,
                    s_noSolutionActiveSpans,
                    sourceFilePath: null,
                    CancellationToken.None
            ...
            else
            {
                ...
                // solution update status after committing an update:
                var discardedUpdateSolutionStatus = await service.HasChangesAsync(
                    solution,
                    s_noSolutionActiveSpans,
                    sourceFilePath: null,
                    CancellationToken.None
...
";
        ...
        public async Task ActiveStatements()
        {
            ...
            var baseSpans = await service.GetBaseActiveStatementSpansAsync(
                solution,
                ImmutableArray.Create(document1.Id),
                CancellationToken.None
            );
            ...
            Assert.Equal(
                activeLineSpan11,
                await service.GetCurrentActiveStatementPositionAsync(
                    document1.Project.Solution,
                    (_, _) => new(trackedActiveSpans1),
                    activeInstruction1,
                    CancellationToken.None
                )
            );
            ...
            currentSpans = await service.GetAdjustedActiveStatementSpansAsync(
                document2,
                _ => new(trackedActiveSpans2),
                CancellationToken.None
            );
            AssertEx.Equal(
                new[]
                ...
                @"class Test
{
    ...
    static void F()
    {
        ...
        <AS:1>G();</AS:1>
            ...
            AssertEx.Equal(
                new[] { "0x06000003 v1 | AS (9,14)-(9,18) δ=1", },
            ...
            AssertEx.Equal(
                new[] { "0x06000003 v1 | AS (9,14)-(9,18) δ=5" },
                InspectNonRemappableRegions(debuggingSession.NonRemappableRegions)
            );
                ...
                @"class Test
{
    ...
    static void F()
    {
        ...
        <AS:1>G();</AS:1>
            ...
            (solution, var document) = AddDefaultTestProject(
                solution,
                ActiveStatementsDescription.ClearTags(markedSource2)
            );
            var documentId = document.Id;
            ...
            var span = await service.GetCurrentActiveStatementPositionAsync(
                solution,
                s_noSolutionActiveSpans,
                activeInstructionF1,
                CancellationToken.None
            ...
            span = await service.GetCurrentActiveStatementPositionAsync(
                solution,
                s_noSolutionActiveSpans,
                activeInstructionF1,
                CancellationToken.None
            ...
            AssertEx.Equal(
                new[] { "0x06000003 v1 | AS (7,14)-(7,18) δ=2", },
                InspectNonRemappableRegions(debuggingSession.NonRemappableRegions)
            );
    ...
    static void F()
    {
            ...
            AssertEx.Equal(
                new[]
                {
                    ...
                    (
                        new LinePositionSpan(new LinePosition(7, 14), new LinePosition(7, 18)),
                        ActiveStatementFlags.IsNonLeafFrame
                    )
                },
                spans
