...
namespace Microsoft.CodeAnalysis.Editor.CSharp.AutomaticCompletion
{
    internal partial class AutomaticLineEnderCommandHandler
    {
        ...
        )
        {
            ...
            rootEditor.ReplaceNode(
                oldNode,
                newNode.WithAdditionalAnnotations(s_replacementNodeAnnotation)
            );
        ...
        )
        {
            ...
            if (statement == null || statement.IsMissing)
            {
                var newRoot = ReplaceNodeAndFormat(
                    ...
                    WithBraces(embeddedStatementOwner, documentOptions),
                    cancellationToken
                );
        ...
        )
        {
            ...
            // Print("hello");
            if (
                doStatementNode.WhileKeyword.IsMissing
            ...
            var newRoot = ReplaceNodeAndFormat(
                ...
                root,
                doStatementNode,
                AddBlockToEmbeddedStatementOwner(doStatementNode, documentOptions, innerStatement),
                cancellationToken
            );
            var nextCaretPosition = GetOpenBraceSpanEnd(newRoot);
            return (newRoot, nextCaretPosition);
        ...
        )
        {
            ...
            var newRoot = ReplaceNodeAndFormat(
                ...
                root,
                ifStatementNode,
                AddBlockToEmbeddedStatementOwner(ifStatementNode, documentOptions, innerStatement),
                cancellationToken
            );
            var nextCaretPosition = GetOpenBraceSpanEnd(newRoot);
            return (newRoot, nextCaretPosition);
        ...
        )
        {
            ...
            var formattedNewRoot = ReplaceNodeAndFormat(
                ...
                root,
                elseClauseNode,
                AddBlockToEmbeddedStatementOwner(elseClauseNode, documentOptions, innerStatement),
                cancellationToken
            );
        ...
        )
        {
            ...
            var nextToken = objectCreationExpressionNode
                ...
                var replacementContainerNode = objectCreationNodeContainer.ReplaceSyntax(
                    ...
                    computeReplacementToken: (_, _) =>
                        SyntaxFactory
                            ...
                            .WithTrailingTrivia(
                                objectCreationNodeWithCorrectInitializer.GetTrailingTrivia()
                            ),
        ...
        /// </summary>
        private static bool ShouldAddBraceForNamespaceDeclaration(
            NamespaceDeclarationSyntax namespaceDeclarationNode,
            int caretPosition
        ) =>
        ...
        /// </summary>
        private static bool ShouldAddBraceForBaseFieldDeclaration(
            BaseFieldDeclarationSyntax baseFieldDeclarationNode
        ) =>
        ...
        )
        {
            ...
            if (
                (
                    indexerDeclarationNode.AccessorList == null
        ...
        private static bool ShouldAddBraceForSwitchStatement(
            ...
            !switchStatementNode.SwitchKeyword.IsMissing
        ...
        private static bool ShouldAddBraceForCatchClause(
        ...
        private static bool ShouldAddBraceForFinallyClause(
            FinallyClauseSyntax finallyClauseNode,
            int caretPosition
        ) =>
        ...
        private static bool ShouldAddBraceForDoStatement(
            DoStatementSyntax doStatementNode,
            int caretPosition
        ) =>
        ...
        )
        {
            ...
            else
            {
                ...
                return elseClauseNode.Statement is not BlockSyntax
                    && !WithinEmbeddedStatement(elseClauseNode, caretPosition);
            }
        }

        private static bool ShouldAddBraceForLockStatement(
            LockStatementSyntax lockStatementNode,
            int caretPosition
        ) =>
        ...
        private static bool ShouldAddBraceForUsingStatement(
            UsingStatementSyntax usingStatementNode,
            int caretPosition
        ) =>
        ...
        private static bool ShouldAddBraceForWhileStatement(
            WhileStatementSyntax whileStatementNode,
            int caretPosition
        ) =>
        ...
        private static bool WithinBraces(SyntaxNode? node, int caretPosition)
        {
            ...
            return TextSpan
                .FromBounds(openBrace.SpanStart, closeBrace.Span.End)
        ...
        private static bool ShouldRemoveBraces(SyntaxNode node, int caretPosition) =>
            ...
            {
                ObjectCreationExpressionSyntax objectCreationExpressionNode
                  => ShouldRemoveBraceForObjectCreationExpression(objectCreationExpressionNode),
                AccessorDeclarationSyntax accessorDeclarationNode
        ...
        private static AccessorListSyntax GetAccessorListNode(DocumentOptionSet documentOptions) =>
            SyntaxFactory
                .AccessorList()
                .WithOpenBraceToken(GetOpenBrace(documentOptions))
                .WithCloseBraceToken(GetCloseBrace(documentOptions));

        private static InitializerExpressionSyntax GetInitializerExpressionNode(
            DocumentOptionSet documentOptions
        ) =>
            SyntaxFactory
        ...
        private static BlockSyntax GetBlockNode(DocumentOptionSet documentOptions) =>
            SyntaxFactory
                .Block()
                .WithOpenBraceToken(GetOpenBrace(documentOptions))
                .WithCloseBraceToken(GetCloseBrace(documentOptions));

        private static SyntaxToken GetOpenBrace(DocumentOptionSet documentOptions) =>
            SyntaxFactory
                .Token(
                    leading: SyntaxTriviaList.Empty,
                    kind: SyntaxKind.OpenBraceToken,
                    trailing: SyntaxTriviaList.Create(GetNewLineTrivia(documentOptions))
                )
                .WithAdditionalAnnotations(s_openBracePositionAnnotation);

        private static SyntaxToken GetCloseBrace(DocumentOptionSet documentOptions) =>
            SyntaxFactory.Token(
                leading: SyntaxTriviaList.Empty,
        ...
        private static SyntaxTrivia GetNewLineTrivia(DocumentOptionSet documentOptions)
        {
        ...
        /// </summary>
        private static SyntaxNode WithBraces(SyntaxNode node, DocumentOptionSet documentOptions) =>
            node switch
            {
                BaseTypeDeclarationSyntax baseTypeDeclarationNode
                  => WithBracesForBaseTypeDeclaration(baseTypeDeclarationNode, documentOptions),
                ObjectCreationExpressionSyntax objectCreationExpressionNode
                ...
                FieldDeclarationSyntax fieldDeclarationNode
                    when fieldDeclarationNode.Declaration.Variables.IsSingle()
                  => ConvertFieldDeclarationToPropertyDeclaration(
                      fieldDeclarationNode,
                      documentOptions
                  ),
                EventFieldDeclarationSyntax eventFieldDeclarationNode
                  => ConvertEventFieldDeclarationToEventDeclaration(
                      eventFieldDeclarationNode,
                      documentOptions
                  ),
                BaseMethodDeclarationSyntax baseMethodDeclarationNode
                  => AddBlockToBaseMethodDeclaration(baseMethodDeclarationNode, documentOptions),
                LocalFunctionStatementSyntax localFunctionStatementNode
                ...
                AccessorDeclarationSyntax accessorDeclarationNode
                  => AddBlockToAccessorDeclaration(accessorDeclarationNode, documentOptions),
                _ when node.IsEmbeddedStatementOwner()
                  => AddBlockToEmbeddedStatementOwner(node, documentOptions),
        ...
        private static ObjectCreationExpressionSyntax GetObjectCreationExpressionWithInitializer(
            ...
            objectCreationExpressionNode.WithInitializer(
                GetInitializerExpressionNode(documentOptions)
            );
        ...
        private static PropertyDeclarationSyntax ConvertFieldDeclarationToPropertyDeclaration(
            ...
            SyntaxFactory
                .PropertyDeclaration(
                    ...
                    accessorList: GetAccessorListNode(documentOptions),
                    expressionBody: null,
                    initializer: null,
        ...
        private static BaseMethodDeclarationSyntax AddBlockToBaseMethodDeclaration(
            ...
            baseMethodDeclarationNode
                .WithBody(GetBlockNode(documentOptions))
        ...
        private static SyntaxNode WithoutBraces(SyntaxNode node) =>
            ...
            {
                ObjectCreationExpressionSyntax objectCreationExpressionNode
                  => RemoveInitializerForObjectCreationExpression(objectCreationExpressionNode),
                PropertyDeclarationSyntax propertyDeclarationNode
        ...
        /// </summary>
        private static AccessorDeclarationSyntax RemoveBodyForAccessorDeclarationNode(
            AccessorDeclarationSyntax accessorDeclarationNode
        ) =>
            accessorDeclarationNode
                .WithBody(null)
