...
namespace Microsoft.CodeAnalysis.Editor.CSharp.Formatting
{
    ...
    internal partial class CSharpEditorFormattingService : IEditorFormattingService
    {
        ...
        public bool SupportsFormattingOnTypedCharacter(Document document, char ch)
        {
            ...
            var smartIndentOn =
                options.GetOption(FormattingOptions.SmartIndent, LanguageNames.CSharp)
            ...
            var autoFormattingOnTyping = options.GetOption(
        ...
        private static IEnumerable<AbstractFormattingRule> GetFormattingRules(
            Document document,
            int position,
        ...
        private static async Task<SyntaxToken> GetTokenBeforeTheCaretAsync(
            Document document,
            int caretPosition,
            CancellationToken cancellationToken
        )
        ...
        )
        {
            ...
            var changes = await formatter
                .FormatTokenAsync(document.Project.Solution.Workspace, token, cancellationToken)
                .ConfigureAwait(false);
            return changes;
        }

        private static ISmartTokenFormatter CreateSmartTokenFormatter(
            OptionSet optionSet,
            IEnumerable<AbstractFormattingRule> formattingRules,
            SyntaxNode root
