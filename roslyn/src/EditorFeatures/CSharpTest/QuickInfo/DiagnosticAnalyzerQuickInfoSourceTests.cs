...
",
            ...
            Assert.Collection(
                ...
                info.RelatedSpans
                    .Select(
                        actualSpan =>
                            new Action<TextSpan>(
                                expectedSpan => Assert.Equal(expectedSpan, actualSpan)
                            )
                    )
                    .ToArray()
            );
        ...
        {
            var diagnosticAnalyzerService = workspace
                .ExportProvider
            ...
            var info = await provider.GetQuickInfoAsync(
                new QuickInfoContext(document, position, options, CancellationToken.None)
            );
            return info;
        }
