...
namespace Microsoft.CodeAnalysis.Editor.CSharp.UnitTests.Diagnostics.NamingStyles
{
    public class EditorConfigNamingStyleParserTests
    {
        private static NamingStylePreferences ParseDictionary(Dictionary<string, string> options) =>
            EditorConfigNamingStyleParser.ParseDictionary(
                new DictionaryAnalyzerConfigOptions(options.ToImmutableDictionary())
            );
        ...
        public static void TestAsyncMethodsAndLocalFunctionsRule()
        {
            var dictionary = new Dictionary<string, string>()
            {
                ...
                ["dotnet_naming_rule.async_methods_must_end_with_async.style"] =
                    "end_in_async_style",
            ...
            Assert.Contains(
                new ModifierKind(ModifierKindEnum.IsAsync),
                symbolSpec.RequiredModifierList
            );
            Assert.Equal(
                new[]
                {
        ...
        public static void TestRuleWithoutCapitalization()
        {
            var dictionary = new Dictionary<string, string>()
            {
                ["dotnet_naming_rule.async_methods_must_end_with_async.symbols"] =
                    "any_async_methods",
        ...
        public static void TestPublicMembersCapitalizedRule()
        {
            var dictionary = new Dictionary<string, string>()
            {
                ...
                ["dotnet_naming_symbols.public_symbols.applicable_accessibilities"] =
                    "public,internal,protected,protected_internal",
        ...
        public static void TestNonPublicMembersLowerCaseRule()
        {
            var dictionary = new Dictionary<string, string>()
            {
                ...
                ["dotnet_naming_rule.non_public_members_must_be_lower_case.symbols "] =
                    "non_public_symbols",
                ["dotnet_naming_rule.non_public_members_must_be_lower_case.style   "] =
                    "all_lower_case_style",
                ["dotnet_naming_symbols.non_public_symbols.applicable_kinds  "] =
                    "property,method,field,event,delegate",
        ...
        public static void TestParametersAndLocalsAreCamelCaseRule()
        {
            var dictionary = new Dictionary<string, string>()
            {
                ...
                ["dotnet_naming_rule.parameters_and_locals_are_camel_case.symbols"] =
                    "parameters_and_locals",
                ["dotnet_naming_rule.parameters_and_locals_are_camel_case.style"] =
                    "camel_case_style",
                ["dotnet_naming_symbols.parameters_and_locals.applicable_kinds"] =
                    "parameter,local",
        ...
        )
        {
            ...
            var kinds = typeOrSymbolKinds
                .Select(NamingStylesTestOptionSets.ToSymbolKindOrTypeKind)
                .ToArray();
            var result = ParseDictionary(rule);
            Assert.Equal(
                kinds,
                result.SymbolSpecifications.SelectMany(x => x.ApplicableSymbolKindList)
            );
        }
        ...
        [InlineData(
            ...
            new[] { Accessibility.Internal, Accessibility.ProtectedOrInternal }
        )]
        [InlineData(
            "friend,protected_friend",
            new[] { Accessibility.Friend, Accessibility.ProtectedOrFriend }
        )]
        ...
        )
        {
            ...
            if (specification != null)
            {
                rule["dotnet_naming_symbols.accessibilities.applicable_accessibilities"] =
                    specification;
            }
            ...
            Assert.Equal(
                accessibilities,
                result.SymbolSpecifications.SelectMany(x => x.ApplicableAccessibilityList)
            );
        }
        ...
        public static void TestRequiredModifiersParse()
        {
            ...
            Assert.Equal(
                csharpResult.SymbolSpecifications.SelectMany(
                    x => x.RequiredModifierList.Select(y => y.Modifier)
                ),
                vbResult.SymbolSpecifications.SelectMany(
                    x => x.RequiredModifierList.Select(y => y.Modifier)
                )
            );
            Assert.Equal(
                csharpResult.SymbolSpecifications.SelectMany(
                    x => x.RequiredModifierList.Select(y => y.ModifierKindWrapper)
                ),
                vbResult.SymbolSpecifications.SelectMany(
                    x => x.RequiredModifierList.Select(y => y.ModifierKindWrapper)
                )
        ...
        public static void TestEditorConfigParseForApplicableSymbolKinds()
        {
            ...
            {
                var editorConfigString = EditorConfigNamingStyleParser.ToEditorConfigString(
                    ImmutableArray.Create(applicableSymbolKind)
                );
        ...
        {
            var namingStylePreferences = ParseDictionary(
