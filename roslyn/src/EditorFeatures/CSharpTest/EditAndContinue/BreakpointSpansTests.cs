    ...
    public class BreakpointSpansTests
    {
            ...
            where TDeclaration : SyntaxNode
        {
            ...
            var expectedEnvelope = expectedSpans.IsEmpty
                ? default
                : TextSpan.FromBounds(expectedSpans[0].Start, expectedSpans[^1].End);
            Assert.NotNull(declarationNode);

            var actualEnvelope =
                SyntaxUtilities.TryGetDeclarationBody(declarationNode, symbol: null)?.Envelope
                ?? default;
        ...
        public static IEnumerable<TextSpan> GetBreakpointSequence(SyntaxNode root, int position)
        {
            ...
            {
                if (
                    BreakpointSpans.TryGetClosestBreakpointSpan(root, p, minLength: 0, out var span)
        ...
        C()|]
    {
        ...
        public void InstanceConstructor_Primary_ImplicitBaseInitializer_OnIdentifierOrNonEmptyParameters_Record(
            ...
            [CombinatorialValues(
                ...
                "record C<T>(int a, [|int b|] = default$$);"
            )]
                string source
