...
namespace Microsoft.CodeAnalysis.Editor.Implementation.NavigationCommandHandlers
{
        ...
        : AbstractNavigationCommandHandler<FindDerivedSymbolsCommandArgs>
    {
        ...
        private static async Task<IEnumerable<ISymbol>> GatherSymbolsAsync(
            ISymbol symbol,
            Solution solution,
        ...
        )
        {
            ...
            )
            {
                return await SymbolFinder
                    .FindImplementationsAsync(namedTypeSymbol, solution, null, cancellationToken)
            ...
            {
                return await SymbolFinder
                    .FindImplementationsAsync(namedTypeSymbol2, solution, null, cancellationToken)
                    .ConfigureAwait(false);
            }
            ...
            {
                return await SymbolFinder
                    .FindDerivedClassesAsync(namedTypeSymbol3, solution, null, cancellationToken)
                    .ConfigureAwait(false);
            }
            ...
            {
                return await SymbolFinder
                    .FindOverridesAsync(symbol, solution, null, cancellationToken)
                    .ConfigureAwait(false);
            }
        }
        ...
        )
        {
            try
            {
                using var token = _asyncListener.BeginAsyncOperation(
                    nameof(FindDerivedSymbolsAsync)
                );
                ...
                try
                {
                    ...
                    )
                    {
                        ...
                        var candidates = await GatherSymbolsAsync(
                                candidateSymbolProjectPair.Value.symbol,
                        ...
                        {
                            var definitionItem = candidate.ToNonClassifiedDefinitionItem(
                                document.Project.Solution,
