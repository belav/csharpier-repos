        ...
        where TWorkspaceFixture : TestWorkspaceFixture, new()
    {
        ...
        private static void CompareSigHelpItemsAndCurrentPosition(
            ...
            if (expectedTestItem.MethodDocumentation != null)
            {
                Assert.Equal(
                    expectedTestItem.MethodDocumentation,
                    actualSignatureHelpItem.DocumentationFactory(CancellationToken.None)
                        .GetFullText()
                );
            }
...
{2}
        ...
        protected async Task VerifyItemWithReferenceWorkerAsync(
            ...
            var cursorPosition =
                testWorkspace.Documents.First(d => d.Name == "SourceDocument").CursorPosition.Value;
            ...
            var selectedSpans =
                testWorkspace.Documents.First(d => d.Name == "SourceDocument").SelectedSpans;
...
{1}
            ...
            var selectedSpans =
                testWorkspace.Documents.Single(d => d.Name == "SourceDocument").SelectedSpans;
            if (selectedSpans.Any())
            {
                textSpans = selectedSpans;
            }
