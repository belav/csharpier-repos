...
namespace Microsoft.CodeAnalysis.Test.Utilities.BracePairs
{
    ...
    public abstract class AbstractBracePairsTests
    {
        ...
        private static bool FindMatch(
            IDictionary<string, ImmutableArray<TextSpan>> expected,
            BracePairData bracePair
        )
        {
            foreach (var (_, expectedSpans) in expected)
            {
                ...
                if (
                    sortedSpans.SequenceEqual(ImmutableArray.Create(bracePair.Start, bracePair.End))
                )
                    return true;
            }
