        ...
        where TWorkspaceFixture : TestWorkspaceFixture, new()
    {
        ...
        private CompletionOptions GetCompletionOptions()
        {
            ...
            if (TypeImportCompletionFeatureFlag.HasValue)
                options = options with
                {
                    TypeImportCompletion = TypeImportCompletionFeatureFlag.Value
                };
            ...
            if (ShowNewSnippetExperience.HasValue)
                options = options with
                {
                    ShowNewSnippetExperience = ShowNewSnippetExperience.Value
                };
        ...
        internal virtual CompletionService GetCompletionService(Project project)
        {
            ...
            var completionProviders = completionService
                .GetTestAccessor()
                .GetAllProviders(ImmutableHashSet<string>.Empty, project);
...
{1}
        ...
        )
        {
            ...
            )
            {
                ...
                var item = completionList.ItemsList.FirstOrDefault(
                    i => i.DisplayText == expectedItem
                );
                Assert.Equal(
                    expectedDescription,
                    (
