...
namespace Microsoft.CodeAnalysis.Editor.UnitTests.Completion
{
        ...
        where TWorkspaceFixture : TestWorkspaceFixture, new()
    {
        ...
        internal virtual OptionsCollection NonCompletionOptions => null;
        ...
        internal virtual CompletionService GetCompletionService(Project project)
        {
            ...
            var completionProviders = completionService
                .GetTestAccessor()
        ...
        )
        {
            ...
            {
                trigger = RoslynCompletion.CompletionTrigger
        ...
        {
            foreach (
                var sourceKind in sourceCodeKind.HasValue
                    ? new[] { sourceCodeKind.Value }
        ...
        {
            using var workspaceFixture = GetOrCreateWorkspaceFixture();
            var workspace = workspaceFixture.Target
                .GetWorkspace(markup, GetComposition(), workspaceKind: workspaceKind);

            // Set options that are not CompletionOptions
            NonCompletionOptions?.SetGlobalOptions(workspace.GlobalOptions);

            var currentDocument = workspace.CurrentSolution
            ...
            return await GetCompletionListAsync(
                    GetCompletionService(currentDocument.Project),
                    currentDocument,
                    position,
        ...
        protected async Task VerifyCustomCommitProviderAsync(
        ...
        )
        {
            ...
            using (workspaceFixture.Target.GetWorkspace(markupBeforeCommit, GetComposition()))
            {
                ...
                {
                    await VerifyCustomCommitProviderWorkerAsync(
                        code,
                        position,
                        itemToCommit,
        ...
        )
        {
            ...
            if (sourceCodeKind.HasValue)
            {
                await VerifyProviderCommitWorkerAsync(
                    code,
                    position,
                    itemToCommit,
                    expectedCodeAfterCommit,
        ...
        protected virtual IEqualityComparer<string> GetStringComparer() => StringComparer.Ordinal;
        ...
        )
        {
            await VerifyAsync(
                ...
                hasSuggestionModeItem: hasSuggestionModeItem,
        ...
        )
        {
            await VerifyAsync(
                markup,
                expectedItemOrNull: null,
                expectedDescriptionOrNull: null,
                sourceCodeKind,
                usePreviousCharAsTrigger: usePreviousCharAsTrigger,
                checkForAbsence: false,
                glyph: null,
                matchPriority: null,
                hasSuggestionModeItem: hasSuggestionModeItem,
        ...
        )
        {
            ...
            var document1 = workspaceFixture.Target
        ...
        private async Task VerifyCustomCommitProviderCheckResultsAsync(
            Document document,
            string codeBeforeCommit,
            int position,
            string itemToCommit,
        ...
        )
        {
            ...
            if (
                service.GetProvider(firstItem, document.Project)
            ...
            {
                await VerifyCustomCommitWorkerAsync(
                    service,
                    document,
                    firstItem,
                    codeBeforeCommit,
        ...
        )
        {
            ...
            MarkupTestFile.GetPosition(
                expectedCodeAfterCommit,
                out var actualExpectedCode,
                out int expectedCaretPosition
            );
            ...
            if (
                commitChar.HasValue
                && !CommitManager.IsCommitCharacter(
                    service.GetRules(options),
                    completionItem,
                    commitChar.Value
                )
            )
            ...
            var caretPosition =
                commit.NewPosition ?? textView.Caret.Position.BufferPosition.Position;
        ...
        /// <param name="itemToCommit">The item to commit from the completion provider.</param>
        /// <param name="expectedCodeAfterCommit">The expected code after commit.</param>
        private async Task VerifyProviderCommitWorkerAsync(
            string codeBeforeCommit,
            int position,
            string itemToCommit,
            string expectedCodeAfterCommit,
        ...
        )
        {
            ...
            var firstItem = items.First(i =>
                CompareItems(i.DisplayText + i.DisplayTextSuffix, itemToCommit)
            );
            ...
            else
            {
                ...
                var textChange = new TextChange(
                    new TextSpan(firstItem.Span.End, 0),
                    commitChar.ToString()
                );
                text = text.WithChanges(textChange);
            }
        ...
        protected async Task VerifyItemInEditorBrowsableContextsAsync(
            ...
            string item,
            int expectedSymbolsSameSolution,
            int expectedSymbolsMetadataReference,
        ...
        )
        {
            await VerifyItemWithMetadataReferenceAsync(
                ...
                referencedCode,
                item,
                expectedSymbolsMetadataReference,
                sourceLanguage,
                referencedLanguage
            );
            ...
            {
                await VerifyItemInSameProjectAsync(
                    markup,
                    referencedCode,
                    item,
                    expectedSymbolsSameSolution,
        ...
        protected async Task VerifyItemWithMetadataReferenceAsync(
            string markup,
    ...
    <Project Language=""{0}"" CommonReferences=""true"" AssemblyName=""Project1"">
        ...
        <MetadataReferenceFromSource Language=""{2}"" CommonReferences=""true"" IncludeXmlDocComments=""true"" DocumentationMode=""Diagnose"" {4}>
            <Document FilePath=""ReferencedDocument"">{3}</Document>
                ...
                referencedLanguage,
                SecurityElement.Escape(metadataReferenceCode),
                GetLanguageVersionAttribute(referencedLanguage)
            );
        }

        protected async Task VerifyItemWithAliasedMetadataReferencesAsync(
            string markup,
    ...
    <Project Language=""{0}"" CommonReferences=""true"" AssemblyName=""Project1"" {1}>
        ...
        <MetadataReferenceFromSource Language=""{3}"" CommonReferences=""true"" Aliases=""{4}"" IncludeXmlDocComments=""true"" DocumentationMode=""Diagnose"" {5}>
            <Document FilePath=""ReferencedDocument"">{6}</Document>
                ...
                sourceLanguage,
                GetLanguageVersionAttribute(sourceLanguage),
                SecurityElement.Escape(markup),
                referencedLanguage,
                SecurityElement.Escape(aliases),
                GetLanguageVersionAttribute(referencedLanguage),
                SecurityElement.Escape(referencedCode)
            );
        }

        protected async Task VerifyItemWithProjectReferenceAsync(
            string markup,
            string referencedCode,
            string expectedItem,
            int expectedSymbols,
        ...
        protected static string CreateMarkupForProjectWithAliasedProjectReference(
            string markup,
            string projectAlias,
            string referencedCode,
            string sourceLanguage,
...
<Workspace>
    ...
    <Project Language=""{2}"" CommonReferences=""true"" AssemblyName=""ReferencedProject"" IncludeXmlDocComments=""true"" DocumentationMode=""Diagnose"">
        <Document FilePath=""ReferencedDocument"">{3}</Document>
                ...
                referencedLanguage,
                SecurityElement.Escape(referencedCode),
                SecurityElement.Escape(projectAlias)
            );
        }

        protected static string CreateMarkupForProjectWithProjectReference(
            string markup,
            string referencedCode,
            string sourceLanguage,
            string referencedLanguage
...
<Workspace>
    ...
    <Project Language=""{3}"" CommonReferences=""true"" AssemblyName=""ReferencedProject"" IncludeXmlDocComments=""true"" DocumentationMode=""Diagnose"" {4}>
        ...
        protected static string CreateMarkupForProjectWithMultupleProjectReferences(
            string sourceText,
            string sourceLanguage,
            string referencedLanguage,
...
{GetReferencedProjectElements(referencedLanguage, referencedTexts)}
            ...
            static string GetProjectReferenceElements(string[] referencedTexts)
            {
                ...
                {
                    builder.AppendLine(
                        $"<ProjectReference>ReferencedProject{i}</ProjectReference>"
                    );
                }
...
<Project Language=""{language}"" CommonReferences=""true"" AssemblyName=""ReferencedProject{i}"" IncludeXmlDocComments=""true"" DocumentationMode=""Diagnose"">
  <Document FilePath=""ReferencedDocument{i}"">{SecurityElement.Escape(referencedTexts[i])}</Document>
        ...
        protected static string CreateMarkupForProjecWithVBProjectReference(
            string markup,
            string referencedCode,
            string sourceLanguage,
...
<Workspace>
    ...
    <Project Language=""{2}"" CommonReferences=""true"" AssemblyName=""ReferencedProject"" IncludeXmlDocComments=""true"" DocumentationMode=""Diagnose"">
        ...
        private Task VerifyItemInSameProjectAsync(
            string markup,
            string referencedCode,
            string expectedItem,
            int expectedSymbols,
...
<Workspace>
    <Project Language=""{0}"" CommonReferences=""true"" Name=""ProjectName"" {5}>
        ...
        {
            using (
                var testWorkspace = TestWorkspace.Create(xmlString, composition: GetComposition())
            )
            {
                ...
                if (expectedSymbols >= 1)
                {
                    ...
                    var item = completionList.ItemsList
                    ...
                    {
                        Assert.Contains(
                            GetExpectedOverloadSubstring(expectedSymbols),
                            description.Text,
        ...
        protected async Task VerifyItemWithMscorlib45Async(
            string markup,
            string expectedItem,
...
{1}
        ...
        )
        {
            await VerifyAtPositionAsync(
                code,
                position,
                ItemPartiallyWritten(expectedItemOrNull),
                usePreviousCharAsTrigger,
                expectedItemOrNull,
                expectedDescriptionOrNull,
                sourceCodeKind,
        ...
        )
        {
            VerifyTextualTriggerCharacterWorker(
                ...
                triggerOnLetter: true,
                sourceCodeKind,
                showCompletionInArgumentLists
            );
            VerifyTextualTriggerCharacterWorker(
                markup,
                expectedTriggerCharacter: shouldTriggerWithTriggerOnLettersDisabled,
                triggerOnLetter: false,
                sourceCodeKind,
        ...
        )
        {
            using (var workspace = CreateWorkspace(markup))
            {
                ...
                var trigger = RoslynCompletion.CompletionTrigger
                ...
                if (expectedTriggerCharacter)
                {
                    var assertText =
                        "'"
                        + text.ToString(new TextSpan(position, 1))
                        + "' expected to be textual trigger character";
                ...
                else
                {
                    var assertText =
                        "'"
                        + text.ToString(new TextSpan(position, 1))
                        + "' expected to NOT be textual trigger character";
        ...
        )
        {
            ...
            using (var workspace = CreateWorkspace(initialMarkup))
            {
                ...
                {
                    Assert.True(
                        CommitManager.IsCommitCharacter(service.GetRules(options), item, ch),
                ...
                foreach (var ch in invalidChars)
                {
                    Assert.False(
                        CommitManager.IsCommitCharacter(service.GetRules(options), item, ch),
                        $"Expected '{ch}' NOT to be a commit character"
                    );
                }
            }
        }
        ...
        protected async Task<
            IReadOnlyList<RoslynCompletion.CompletionItem>
        ...
        )
        {
            ...
            var trigger = usePreviousCharAsTrigger
                ? RoslynCompletion.CompletionTrigger
                    .CreateInsertionTrigger(insertedCharacter: code.ElementAt(position - 1))
                : RoslynCompletion.CompletionTrigger.Invoke;
