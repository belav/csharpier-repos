    ...
    internal abstract class EditAndContinueTestHelpers
    {
        ...
        )
        {
            ...
            var documentId = DocumentId.CreateNewId(
                ProjectId.CreateNewId("TestEnCProject"),
                "TestEnCDocument"
            );

            var actualNewActiveStatements = ImmutableArray.CreateBuilder<ActiveStatement>(
                oldActiveStatements.Length
            );
            actualNewActiveStatements.Count = actualNewActiveStatements.Capacity;

            var actualNewExceptionRegions = ImmutableArray.CreateBuilder<
            ...
            AssertSpansEqual(
                expectedNewActiveStatements,
                actualNewActiveStatements.Select(s => s.Span),
                text
            );
        ...
        )
        {
            ...
            var oldTrees = oldDocuments.Select(d => d.GetSyntaxTreeSynchronously(default)!)
                .ToArray();
            var newTrees = newDocuments.Select(d => d.GetSyntaxTreeSynchronously(default)!)
                .ToArray();
        ...
        )
        {
            ...
            AssertEx.Equal(
                ...
                actualSemanticEdits.NullToEmpty()
            ...
            for (var i = 0; i < actualSemanticEdits.Length; i++)
            {
                ...
                Assert.Equal(
                    ...
                    actualSemanticEdit.PartialType?.Resolve(
                        newCompilation,
