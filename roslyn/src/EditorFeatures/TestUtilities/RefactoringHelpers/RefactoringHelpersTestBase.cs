...
namespace Microsoft.CodeAnalysis.Editor.UnitTests.RefactoringHelpers
{
        ...
        where TWorkspaceFixture : TestWorkspaceFixture, new()
    {
        ...
        protected Task TestAsync<TNode>(string text, bool allowEmptyNodes = false)
            where TNode : SyntaxNode => TestAsync(text, Functions<TNode>.True, allowEmptyNodes);

        protected async Task TestAsync<TNode>(
            string text,
        ...
        protected Task TestMissingAsync<TNode>(string text, bool allowEmptyNodes = false)
            where TNode : SyntaxNode =>
            TestMissingAsync(text, Functions<TNode>.True, allowEmptyNodes);

        protected async Task TestMissingAsync<TNode>(
            string text,
            Func<TNode, bool> predicate,
            bool allowEmptyNodes = false
        )
        ...
        private static string GetSelectionAndResultSpans(
            string text,
        ...
        private async Task<TNode?> GetNodeForSelectionAsync<TNode>(
            string text,
            TextSpan selection,
            Func<TNode, bool> predicate,
            bool allowEmptyNodes = false
            ...
            where TNode : SyntaxNode
        {
            ...
            var relevantNodes = await document
                .GetRelevantNodesAsync<TNode>(selection, allowEmptyNodes, CancellationToken.None)
                .ConfigureAwait(false);
