    ...
    internal partial class InlineRenameSession
    {
        ...
        internal class OpenTextBufferManager : ForegroundThreadAffinitizedObject
        {
            ...
            private readonly Dictionary<
                TextSpan,
                RenameTrackingSpan
            > _referenceSpanToLinkedRenameSpanMap = new Dictionary<TextSpan, RenameTrackingSpan>();

            private readonly List<RenameTrackingSpan> _conflictResolutionRenameTrackingSpans =
                new List<RenameTrackingSpan>();
            ...
            private void UpdateReadOnlyRegions(bool removeOnly = false)
            {
                ...
                if (!removeOnly)
                {
                    ...
                    {
                        _readOnlyRegions.Add(
                            readOnlyEdit.CreateDynamicReadOnlyRegion(
                                newReadOnlySpan,
                    ...
                    if (editableSpans.All(s => s.End < _subjectBuffer.CurrentSnapshot.Length))
                    {
                        _readOnlyRegions.Add(
                            readOnlyEdit.CreateDynamicReadOnlyRegion(
                                new Span(_subjectBuffer.CurrentSnapshot.Length, 0),
                                SpanTrackingMode.EdgeExclusive,
                                EdgeInsertionMode.Deny,
                                _isBufferReadOnly
                            )
            ...
            internal IEnumerable<RenameTrackingSpan> GetRenameTrackingSpans() =>
                _referenceSpanToLinkedRenameSpanMap.Values.Where(r => r.Type != RenameSpanKind.None)
            ...
            private void OnTextBufferChanged(object sender, TextContentChangedEventArgs args)
            {
                ...
                using (
                    ...
                    _activeSpan =
                        _referenceSpanToLinkedRenameSpanMap.Where(
                                kvp =>
            ...
            private bool AreAllReferenceSpansMappable()
            {
                ...
                return ActiveTextView == null
                    || _referenceSpanToLinkedRenameSpanMap.Keys.Select(s => s.ToSpan())
                        .All(
                            s =>
                                s.End <= _subjectBuffer.CurrentSnapshot.Length
                                && // span is valid for the snapshot
                                ActiveTextView.GetSpanInView(
                                    _subjectBuffer.CurrentSnapshot.GetSpan(s)
                                ).Count != 0
                        ); // spans were successfully projected
            }
            ...
            internal void ApplyReplacementText(bool updateSelection = true)
            {
                ...
                _session.UndoManager.ApplyCurrentState(
                    ...
                    _referenceSpanToLinkedRenameSpanMap.Values.Where(
                            r => r.Type != RenameSpanKind.None
                        )
            ...
            internal void ApplyConflictResolutionEdits(
                IInlineRenameReplacementInfo conflictResolution,
                ...
                using (new SelectionTracking(this))
                {
                    ...
                    var changes = GetTextChangesFromTextDifferencingServiceAsync(
                            originalDocument,
                            newDocument,
                            cancellationToken
                        )
                    ...
                    var documentReplacements = documents.Select(
                            document =>
                                (
                                    document,
                                    conflictResolution.GetReplacements(document.Id)
                                        .Where(
                                            r => GetRenameSpanKind(r.Kind) != RenameSpanKind.None
                                        )
                                        .ToImmutableArray()
                                )
                        )
                    ...
                    var firstDocumentReplacements = documentReplacements.FirstOrDefault(
                        d => !d.Item2.IsEmpty
                    );
                    var bufferContainsLinkedDocuments =
                        documentReplacements.Length > 1
                        && firstDocumentReplacements.document != null;
                    ...
                    if (linkedDocumentsMightConflict)
                    {
                        ...
                        if (spansMatch)
                        {
                            ...
                            var firstDocumentNewText = conflictResolution.NewSolution.GetDocument(
                                    firstDocumentReplacements.document.Id
                                )
                            ...
                            foreach (var (document, replacements) in documentReplacements)
                            {
                                ...
                                var documentNewText = conflictResolution.NewSolution.GetDocument(
                                        document.Id
                                    )
                                    .GetTextAsync(cancellationToken)
                                    .WaitAndGetResult(cancellationToken);
                    ...
                    {
                        var relevantReplacements = conflictResolution.GetReplacements(document.Id)
                        ...
                        var mergeConflictComments =
                            mergeResult.MergeConflictCommentSpans.ContainsKey(document.Id)
                        ...
                        foreach (var replacement in mergedReplacements)
                        {
                            ...
                            if (
                                _referenceSpanToLinkedRenameSpanMap.ContainsKey(
                                ...
                                var linkedRenameSpan = _session._renameInfo.GetConflictEditSpan(
                                    ...
                                    GetWithoutAttributeSuffix(
                                        ...
                                        document.GetLanguageService<LanguageServices.ISyntaxFactsService>().IsCaseSensitive
                                    ),
                                    cancellationToken
                                );
                                ...
                                else
                                {
                                    ...
                                    _referenceSpanToLinkedRenameSpanMap[replacement.OriginalSpan] =
                                        new RenameTrackingSpan(
                                            _referenceSpanToLinkedRenameSpanMap[
                                                replacement.OriginalSpan
                                            ].TrackingSpan,
            ...
            > GetTextChangesFromTextDifferencingServiceAsync(
                ...
                try
                {
                    using (
                        ...
                        var textDiffService =
                            oldDocument.Project.Solution.Workspace.Services.GetService<IDocumentTextDifferencingService>();
                        return await textDiffService.GetTextChangesAsync(
                                oldDocument,
                                newDocument,
                                cancellationToken
                            )
            ...
            private IEnumerable<InlineRenameReplacement> GetMergedReplacementInfos(
                ...
                var textDiffService =
                    preMergeDocument.Project.Solution.Workspace.Services.GetService<IDocumentTextDifferencingService>();
                var contentType =
                    preMergeDocument.Project.LanguageServices.GetService<IContentTypeLanguageService>()
                ...
                var preMergeDocumentText = preMergeDocument.GetTextAsync(cancellationToken)
                    .WaitAndGetResult(cancellationToken);
                var snapshot = preMergeDocumentText.FindCorrespondingEditorTextSnapshot();
                if (snapshot != null)
                {
                    textBufferCloneService =
                        preMergeDocument.Project.Solution.Workspace.Services.GetService<ITextBufferCloneService>();
                ...
                {
                    preMergeDocumentTextString = preMergeDocument.GetTextAsync(cancellationToken)
                        .WaitAndGetResult(cancellationToken)
                        .ToString();
                }
                ...
                foreach (var replacement in relevantReplacements)
                {
                    var buffer = snapshotSpanToClone.HasValue
                        ? textBufferCloneService.CloneWithUnknownContentType(
                              snapshotSpanToClone.Value
                          )
                    ...
                    yield return new InlineRenameReplacement(
                        replacement.Kind,
                        replacement.OriginalSpan,
                        trackingSpan.GetSpan(buffer.CurrentSnapshot).Span.ToTextSpan()
                    );
                }
            }
            ...
            private struct SelectionTracking : IDisposable
            {
                ...
                public SelectionTracking(OpenTextBufferManager openTextBufferManager)
                {
                    ...
                    var containingSpans =
                        openTextBufferManager._referenceSpanToLinkedRenameSpanMap.Select(
                                kvp =>
                                {
                                    ...
                                    var ss = textView.GetSpanInView(
                                            kvp.Value.TrackingSpan.GetSpan(snapshot)
                                        )
                                        .FirstOrDefault();
                                    if (
                                        ss != default
                ...
                private SnapshotPoint GetNewEndpoint(TextSpan span)
                {
                    ...
                    var endPoint =
                        _openTextBufferManager._referenceSpanToLinkedRenameSpanMap.ContainsKey(span)
                            ...
                            : _openTextBufferManager._referenceSpanToLinkedRenameSpanMap.First(
                                      kvp => kvp.Key.OverlapsWith(span)
                                  )
                    ...
                    return _openTextBufferManager.ActiveTextView.BufferGraph.MapUpToBuffer(
                        ...
                        _openTextBufferManager.ActiveTextView.TextBuffer
                    ).Value;
                }
            }
        }
