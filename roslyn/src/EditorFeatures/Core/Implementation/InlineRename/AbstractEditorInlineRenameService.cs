...
namespace Microsoft.CodeAnalysis.Editor.Implementation.InlineRename
{
    internal abstract partial class AbstractEditorInlineRenameService : IEditorInlineRenameService
    {
        ...
        )
        {
            ...
            if (triggerToken == default)
            {
                return new FailureInlineRenameInfo(
                    EditorFeaturesResources.You_must_rename_an_identifier
                );
            }
        ...
        )
        {
            ...
            if (
                syntaxFactsService.IsTypeNamedVarInVariableOrFieldDeclaration(
                    triggerToken,
                    triggerToken.Parent
                )
            )
            ...
            {
                var originalSymbol = await SymbolFinder
            ...
            {
                return new FailureInlineRenameInfo(
                    EditorFeaturesResources.Renaming_anonymous_type_members_is_not_yet_supported
                );
            }
            ...
            {
                return new FailureInlineRenameInfo(
                    EditorFeaturesResources.Please_resolve_errors_in_your_code_before_renaming_this_element
                );
            }
            ...
            foreach (var location in symbolLocations)
            {
                ...
                {
                    return new FailureInlineRenameInfo(
                        EditorFeaturesResources.You_cannot_rename_elements_that_are_defined_in_metadata
                    );
                }
            ...
            return new SymbolInlineRenameInfo(
                ...
                documentSpans.ToImmutableAndFree(),
                cancellationToken
            );
        }

        private static async Task<SyntaxToken> GetTriggerTokenAsync(
            Document document,
            int position,
            CancellationToken cancellationToken
        )
