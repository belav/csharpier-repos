...
namespace Microsoft.CodeAnalysis.Editor.Implementation.InlineRename
{
          ...
          IFeatureController
    {
        ...
        ) : base(threadingContext, assertIsForeground: true)
        {
            ...
            _triggerView =
                textBufferAssociatedViewService.GetAssociatedTextViews(
            ...
            _debuggingWorkspaceService = workspace.Services
                .GetService<IDebuggingWorkspaceService>();
...
#pragma warning restore IDE0052 // Remove unread private members
        ...
        )
        {
            ...
            _allRenameLocationsTask = ThreadingContext.JoinableTaskFactory.RunAsync(
                async () =>
                {
                    var inlineRenameLocations = await findRenameLocationsTask.JoinAsync()
                    ...
                    await ThreadingContext.JoinableTaskFactory
                        .SwitchToMainThreadAsync(alwaysYield: true, _cancellationTokenSource.Token);
            ...
            if (_optionSet.GetOption(renameOption) != newValue)
            {
                ...
                UpdateReferenceLocationsTask(ThreadingContext.JoinableTaskFactory.RunAsync(
                        async () =>
                        {
                            ...
                            await _allRenameLocationsTask.JoinAsync(CancellationToken.None)
                                .ConfigureAwait(false);
                            await TaskScheduler.Default;

                            return await _renameInfo.FindRenameLocationsAsync(
                                _optionSet,
                                cancellationToken
                            )
                                .ConfigureAwait(false);
        ...
        private void SetReferenceLocations(ImmutableArray<InlineRenameLocation> locations)
        {
            ...
            foreach (var textBuffer in _openTextBuffers.Keys)
            {
                ...
                {
                    var spans = documents.SelectMany(d => locationsByDocument[d.Id])
                        .Select(l => l.TextSpan)
                        .Distinct();
        ...
        private void UpdateConflictResolutionTask()
        {
            ...
            _conflictResolutionTask = ThreadingContext.JoinableTaskFactory.RunAsync(
                async () =>
                {
                    ...
                    var result = await _allRenameLocationsTask.JoinAsync(cancellationToken)
                        .ConfigureAwait(false);
                    await TaskScheduler.Default;

                    return await result.GetReplacementsAsync(
                        replacementText,
                        optionSet,
                        cancellationToken
                    )
        ...
        private void QueueApplyReplacements()
        {
            ...
            var replacementOperation = ThreadingContext.JoinableTaskFactory.RunAsync(
                async () =>
                {
                    var replacementInfo = await _conflictResolutionTask.JoinAsync(
                        CancellationToken.None
                    )
                        .ConfigureAwait(false);
        ...
        private void ApplyRename(Solution newSolution, IWaitContext waitContext)
        {
            ...
            foreach (var id in changedDocumentIDs)
            {
                ...
                {
                    var root = newDocument.GetSyntaxRootSynchronously(
                        waitContext.CancellationToken
                    );
                ...
                {
                    var newText = newDocument.GetTextAsync(waitContext.CancellationToken)
            ...
            if (_workspace.TryApplyChanges(finalSolution))
            {
                ...
                var finalChangedIds = finalChanges.GetProjectChanges()
