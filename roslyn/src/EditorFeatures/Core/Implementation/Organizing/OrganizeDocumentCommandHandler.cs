          ...
          ICommandHandler<SortAndRemoveUnnecessaryImportsCommandArgs>
    {
        ...
        )
        {
            ...
            )
            {
                ...
                if (document != null)
                {
                    var newDocument = OrganizingService.OrganizeAsync(
                        document,
        ...
        )
        {
            ...
            if (document != null)
            {
                var newDocument = Formatter.OrganizeImportsAsync(document, cancellationToken)
                    .WaitAndGetResult(cancellationToken);
                if (document != newDocument)
                {
                    ApplyTextChange(document, newDocument);
        ...
        )
        {
            ...
            if (document != null)
            {
                var newDocument = document.GetLanguageService<IRemoveUnnecessaryImportsService>()
                    .RemoveUnnecessaryImportsAsync(document, cancellationToken)
                    .WaitAndGetResult(cancellationToken);
                newDocument = Formatter.OrganizeImportsAsync(newDocument, cancellationToken)
                    .WaitAndGetResult(cancellationToken);
                if (document != newDocument)
                {
                    ApplyTextChange(document, newDocument);
        ...
        protected static void ApplyTextChange(Document oldDocument, Document newDocument) =>
            oldDocument.Project.Solution.Workspace
