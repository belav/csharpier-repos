    ...
    internal partial class BraceCompletionSessionProvider
    {
              ...
              IBraceCompletionSession
        {
            ...
            ) : base(threadingContext, assertIsForeground: true)
            {
                ...
                this.ClosingPoint = SubjectBuffer.CurrentSnapshot
            ...
            private bool TryStart(CancellationToken cancellationToken)
            {
                ...
                OpeningPoint = SubjectBuffer.CurrentSnapshot
                ...
                var braceResult = _service.GetBraceCompletionAsync(context.Value, cancellationToken)
                ...
                using var caretPreservingTransaction = new CaretPreservingEditTransaction(
                    EditorFeaturesResources.Brace_Completion,
                    _undoHistory,
                    _editorOperations
                );
                ...
                ClosingPoint = SubjectBuffer.CurrentSnapshot
                    .CreateTrackingPoint(
                ...
                {
                    var changesAfterStart = _service.GetTextChangesAfterCompletionAsync(
                        contextAfterStart.Value,
                        cancellationToken
                    )
                        .WaitAndGetResult(cancellationToken);
            ...
            public void PreBackspace(out bool handledCommand)
            {
                ...
                )
                {
                    ...
                    var span = new SnapshotSpan(
                        OpeningPoint.GetPoint(snapshot),
                        ClosingPoint.GetPoint(snapshot)
                    );
            ...
            public void PreOverType(out bool handledCommand)
            {
                ...
                if (!HasForwardTyping && AllowOverType())
                {
                    ...
                    Debug.Assert(
                        caretPos.HasValue && caretPos.Value.Position < closingSnapshotPoint.Position
                    );
                ...
                bool AllowOverType()
                {
                    var context = GetBraceCompletionContext();
                    return context != null
                        && _service.AllowOverTypeAsync(context.Value, cancellationToken)
                            .WaitAndGetResult(cancellationToken);
                }
            }
            ...
            public void PreReturn(out bool handledCommand) => handledCommand = false;
            ...
            public void PostReturn()
            {
                ...
                if (this.GetCaretPosition().HasValue)
                {
                    ...
                    )
                    {
                        ...
                        var documentOptions = context.Value.Document
                            .GetOptionsAsync()
                            .WaitAndGetResult(CancellationToken.None);
                        var changesAfterReturn = _service.GetTextChangeAfterReturnAsync(
                            context.Value,
                            documentOptions,
                        ...
                        {
                            using var caretPreservingTransaction =
                                new CaretPreservingEditTransaction(
                                    EditorFeaturesResources.Brace_Completion,
                                    _undoHistory,
                                    _editorOperations
                                );
            ...
            private void MoveCaretToClosingPoint()
            {
                ...
                var afterBrace = TextView.BufferGraph
                    .MapUpToBuffer(
            ...
            private BraceCompletionContext? GetBraceCompletionContext()
            {
                ...
                return new BraceCompletionContext(
                    document,
            ...
            private void ApplyBraceCompletionResult(BraceCompletionResult result)
            {
                ...
                try
                {
                    Contract.ThrowIfFalse(
                        SubjectBuffer.CurrentSnapshot[
                            OpeningPoint.GetPosition(SubjectBuffer.CurrentSnapshot)
                        ] == OpeningBrace,
                        "The opening point does not match the opening brace character"
                    );
                    Contract.ThrowIfFalse(
                        SubjectBuffer.CurrentSnapshot[
                            ClosingPoint.GetPosition(SubjectBuffer.CurrentSnapshot) - 1
                        ] == ClosingBrace,
                ...
                var caretLine = SubjectBuffer.CurrentSnapshot
