...
namespace Microsoft.CodeAnalysis.Editor.Implementation.IntelliSense.AsyncCompletion
{
    internal class ItemManager : IAsyncCompletionItemManager
    {
        ...
        )
        {
            ...
            var sortedItems = data.InitialList
                .OrderBy(GetOrAddRoslynCompletionItem)
                .ToImmutableArray();
        ...
        )
        {
            ...
            var nonExpanderFilterStates = data.SelectedFilters
                .WhereAsArray(f => !(f.Filter is CompletionExpander));

            var selectedNonExpanderFilters = nonExpanderFilterStates.Where(f => f.IsSelected)
                .SelectAsArray(f => f.Filter);
            var needToFilter =
                selectedNonExpanderFilters.Length > 0
                && selectedNonExpanderFilters.Length < nonExpanderFilterStates.Length;

            var unselectedExpanders = data.SelectedFilters
                .Where(f => !f.IsSelected && f.Filter is CompletionExpander)
                .SelectAsArray(f => f.Filter);
            ...
            var snapshotForDocument = Helpers.TryGetInitialTriggerLocation(
                session,
                out var triggerLocation
            )
            ...
            var completionHelper =
                document != null ? CompletionHelper.GetHelper(document) : _defaultCompletionHelper;
            ...
            if (
                initialRoslynTriggerKind == CompletionTriggerKind.Insertion
            ...
            try
            {
                ...
                {
                    filterMethod = (itemsWithPatternMatches, text) =>
                        CompletionService.FilterItems(completionHelper, itemsWithPatternMatches);
                }
                else
                {
                    filterMethod = (itemsWithPatternMatches, text) =>
                        completionService.FilterItems(document, itemsWithPatternMatches, text);
                }
        ...
        )
        {
            ...
            if (chosenItems.Length == 0)
            {
                ...
                var longestCommonPrefixLength =
                    bestOrFirstMatchResult.RoslynCompletionItem.FilterText
                        .GetCaseInsensitivePrefixLength(filterText);

                for (var i = 1; i < itemsInList.Count; ++i)
                {
                    var item = itemsInList[i];
                    var commonPrefixLength = item.RoslynCompletionItem.FilterText
                        .GetCaseInsensitivePrefixLength(filterText);
            ...
            else
            {
                ...
                selectedItemIndex = itemsInList.IndexOf(
                    i => Equals(i.RoslynCompletionItem, bestItem)
                );
                bestOrFirstMatchResult = itemsInList[selectedItemIndex];
                var deduplicatedListCount = matchingItems.Count(
                    r => !r.RoslynCompletionItem.IsPreferredItem()
                );
                if (deduplicatedListCount == 1 && filterText.Length > 0)
                {
            ...
            if (
                ...
                && !string
                    .Equals(filterText, typeChar.ToString(), StringComparison.OrdinalIgnoreCase)
            ...
            return new FilteredCompletionModel(
                GetHighlightedList(itemsInList),
                selectedItemIndex,
                filters,
                updateSelectionHint,
        ...
        )
        {
            ...
            if (bestMatchResult != null)
            {
                ...
                hardSelect =
                    !hasSuggestedItemOptions
                    && bestMatchResult.Value.VSCompletionItem.FilterText
            ...
            return new FilteredCompletionModel(
                GetHighlightedList(matchResults),
                index,
                filters,
        ...
        private static ImmutableArray<CompletionItemWithHighlight> GetHighlightedList(
            ...
            matchResults.SelectAsArray(
                matchResult =>
                    new CompletionItemWithHighlight(
                        matchResult.VSCompletionItem,
                        matchResult.HighlightedSpans
                    )
            );
        ...
        )
        {
            ...
            Debug.Assert(
                filterText.Length > 0 || item.Rules.MatchPriority != MatchPriority.Default
            );
