    ...
    internal partial class SyntacticClassificationTaggerProvider
    {
        ...
        internal partial class TagComputer
        {
            ...
            )
            {
                ...
                _workQueue = new AsynchronousSerialWorkQueue(
            ...
            internal void IncrementReferenceCount() => _taggerReferenceCount++;
            ...
            private void EnqueueProcessSnapshot(Document newDocument)
            {
                if (newDocument != null)
                {
                    _workQueue.EnqueueBackgroundTask(
                        c => this.EnqueueProcessSnapshotWorkerAsync(newDocument, c),
            ...
            )
            {
                ...
                var currentCachedData =
                    service == null
                        ? null
                        : await service
                              .GetDataToCacheAsync(currentDocument, cancellationToken)
                              .ConfigureAwait(false);
                ...
                _notificationService.RegisterNotification(
                    ...
                    _listener.BeginAsyncOperation(
                        "EnqueueProcessSnapshotWorkerAsync.ReportChangedSpan"
                    ),
            ...
            )
            {
                ...
                if (previousDocument != null)
                {
                    ...
                    if (service != null)
                    {
                        var changeRange = await service
                            .ComputeSyntacticChangeRangeAsync(
                                previousDocument,
                                currentDocument,
                                _diffTimeout,
                                cancellationToken
            ...
            private void OnWorkspaceChanged(object sender, WorkspaceChangeEventArgs args)
            {
                ...
                _workQueue.EnqueueBackgroundTask(
                    c => UpdateLastParsedDocumentAsync(newSolution, c),
                    "UpdateLastParsedDocument",
                    CancellationToken.None
                );
            }
            ...
            )
            {
                ...
                var lastParsedText = await lastDocument
                    .GetTextAsync(cancellationToken)
                    .ConfigureAwait(false);
            ...
            private void ProcessIfThisDocument(Solution newSolution, DocumentId documentId)
            {
                if (_workspace != null)
                {
                    var openDocumentId = _workspace.GetDocumentIdInCurrentContext(
                        _subjectBuffer.AsTextContainer()
                    );
