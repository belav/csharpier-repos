...
namespace Microsoft.CodeAnalysis.Editor.Implementation.CodeDefinitionWindow
{
    ...
    internal class DefinitionContextTracker : ITextViewConnectionListener
    {
        ...
        )
        {
            try
            {
                await _asyncListener
                    .Delay(TimeSpan.FromMilliseconds(250), cancellationToken)
        ...
        )
        {
            ...
            var navigableItems = await GoToDefinitionHelpers
                .GetDefinitionsAsync(document, position, cancellationToken)
                .ConfigureAwait(false);
