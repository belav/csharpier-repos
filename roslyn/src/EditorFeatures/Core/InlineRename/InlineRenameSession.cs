...
namespace Microsoft.CodeAnalysis.Editor.Implementation.InlineRename
{
    internal partial class InlineRenameSession : IInlineRenameSession, IFeatureController
    {
        ...
        private class NullTextBufferException(Document document, SourceText text)
            : Exception("Cannot retrieve textbuffer from document.")
        {
#pragma warning disable IDE0052 // Remove unread private members
        ...
        private void UpdateReferenceLocationsTask()
        {
            ...
            _allRenameLocationsTask = _threadingContext.JoinableTaskFactory.RunAsync(async () =>
            {
                ...
                var inlineRenameLocations = await _renameInfo
                    .FindRenameLocationsAsync(currentOptions, cancellationToken)
                    .ConfigureAwait(false);
        ...
        private void OnWorkspaceChanged(object sender, WorkspaceChangeEventArgs args)
        {
            ...
            {
                Logger.Log(
                    FunctionId.Rename_InlineSession_Cancel_NonDocumentChangedWorkspaceChange,
        ...
        private void SetReferenceLocations(ImmutableArray<InlineRenameLocation> locations)
        {
            ...
            foreach (var textBuffer in _openTextBuffers.Keys)
            {
                ...
                if (
                    !documents.Any(
                        static (d, locationsByDocument) => locationsByDocument.Contains(d.Id),
                        locationsByDocument
                    )
                )
                {
                    _openTextBuffers[textBuffer]
                        .SetReferenceSpans(SpecializedCollections.EmptyEnumerable<TextSpan>());
                }
                else
                {
                    var spans = documents
        ...
        private void UpdateConflictResolutionTask()
        {
            ...
            _conflictResolutionTask = _threadingContext.JoinableTaskFactory.RunAsync(async () =>
            {
                ...
                return await result
                    .GetReplacementsAsync(replacementText, options, cancellationToken)
                    .ConfigureAwait(false);
            });
        ...
        )
        {
            ...
            {
                _workspace.WorkspaceChanged -= OnWorkspaceChanged;
                _textBufferAssociatedViewService.SubjectBuffersConnected -=
                    OnSubjectBuffersConnected;
        ...
        public Task CommitAsync(bool previewChanges, CancellationToken cancellationToken) =>
            CommitWorkerAsync(
                previewChanges,
                canUseBackgroundWorkIndicator: true,
                cancellationToken
            );

        /// <returns><see langword="true"/> if the rename operation was commited, <see
        /// langword="false"/> otherwise</returns>
        private async Task<bool> CommitWorkerAsync(
            bool previewChanges,
            bool canUseBackgroundWorkIndicator,
            CancellationToken cancellationToken
        )
        {
            await _threadingContext.JoinableTaskFactory.SwitchToMainThreadAsync(cancellationToken);
            ...
            {
                if (
                    canUseBackgroundWorkIndicator
                    && this.RenameService.GlobalOptions.GetOption(
                ...
                )
                {
                    ...
                    var factory =
                        _workspace.Services.GetRequiredService<IBackgroundWorkIndicatorFactory>();
                    using var context = factory.Create(
                        _triggerView,
                        TriggerSpan,
                        EditorFeaturesResources.Computing_Rename_information,
                        cancelOnEdit: false,
        ...
        )
        {
            ...
            {
                var info = await _conflictResolutionTask
                    .JoinAsync(cancellationToken)
                    .ConfigureAwait(true);
                ...
                await DismissUIAndRollbackEditsAndEndRenameSessionAsync(
                        ...
                        async () =>
                        {
                            var error = await TryApplyRenameAsync(newSolution, cancellationToken)
                                .ConfigureAwait(false);
        ...
        )
        {
            ...
            await TaskScheduler.Default;
            var finalSolution = CalculateFinalSolutionSynchronously(
                newSolution,
                newSolution.Workspace,
                changedDocumentIDs,
                cancellationToken
            );
            ...
            if (!_workspace.TryApplyChanges(finalSolution))
            {
                ...
                Logger.Log(
                    FunctionId.Rename_TryApplyRename_WorkspaceChanged,
                    message: null,
                    LogLevel.Information
                );
                finalSolution = CalculateFinalSolutionSynchronously(
                    newSolution,
                    _workspace,
                    changedDocumentIDs,
                    cancellationToken
            ...
            static Solution CalculateFinalSolutionSynchronously(
                Solution newSolution,
                Workspace workspace,
                List<DocumentId> changedDocumentIDs,
                CancellationToken cancellationToken
            )
            {
                ...
                foreach (var id in changedDocumentIDs)
                {
                    ...
                    {
                        var root = newDocument.GetRequiredSyntaxRootSynchronously(
                            cancellationToken
                        );
