...
namespace Microsoft.CodeAnalysis.Editor.Shared.Tagging
{
    internal partial class TaggerEventSources
    {
        private class CompositionEventSource : ITaggerEventSource
        {
            ...
            {
                add { _providers.Do(p => p.UIUpdatesPaused += value); }
                remove { _providers.Do(p => p.UIUpdatesPaused -= value); }
            }

            public event EventHandler UIUpdatesResumed
            {
                add { _providers.Do(p => p.UIUpdatesResumed += value); }
                remove { _providers.Do(p => p.UIUpdatesResumed -= value); }
            }
        }
    }
}
