...
namespace Microsoft.CodeAnalysis.Editor.Host
{
    ...
    internal interface IStreamingFindUsagesPresenter
    {
        ...
        /// </summary>
        (FindUsagesContext context, CancellationToken cancellationToken) StartSearchWithCustomColumns(
            string title,
            bool supportsReferences,
    ...
    internal static class IStreamingFindUsagesPresenterExtensions
    {
        ...
        )
        {
            ...
            using var _ =
                ArrayBuilder<(DefinitionItem item, INavigableLocation location)>.GetInstance(
                    out var builder
                );
            ...
            if (builder.Count == 1 && builder[0].item.SourceSpans.Length <= 1)
            {
