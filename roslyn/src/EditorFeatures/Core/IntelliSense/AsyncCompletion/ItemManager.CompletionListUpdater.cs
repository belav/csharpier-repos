    ...
    internal partial class ItemManager
    {
        ...
        private sealed class CompletionListUpdater
        {
            ...
            private CompletionTriggerReason InitialTriggerReason =>
                _snapshotData.InitialTrigger.Reason;
            ...
            )
            {
                ...
                if (_document != null)
                {
                    ...
                    _filterMethod =
                        _completionService == null
                            ...
                            : (
                                (matchResults, text, filteredItemsBuilder) =>
                                    _completionService.FilterItems(
                                        _document,
                                        matchResults,
                                        text,
                                        filteredItemsBuilder
                ...
                else
                {
                    ...
                    _filterMethod = (matchResults, text, filteredMatchResultsBuilder) =>
                        CompletionService.FilterItems(
                            _completionHelper,
                            matchResults,
                            text,
            ...
            )
            {
                ...
                var threadLocalPatternMatchHelper = new ThreadLocal<PatternMatchHelper>(
                    () => new PatternMatchHelper(_filterText),
                    trackAllValues: true
                );
                ...
                try
                {
                    ...
                    var highlightAndFilterTask = Task.Run(
                        () =>
                            GetHighlightedListAndUpdatedFilters(
                                session,
                                itemsToBeIncluded,
                                threadLocalPatternMatchHelper,
                    ...
                    var initialSelection =
                        InitialTriggerReason == CompletionTriggerReason.Backspace
                    ...
                    var finalSelection = UpdateSelectionBasedOnSuggestedDefaults(
                        itemsToBeIncluded,
                        initialSelection.Value,
                        cancellationToken
                    );
                    var (highlightedList, updatedFilters) =
                ...
                (
                    ...
                    IAsyncCompletionSession session,
                    IReadOnlyList<MatchResult> itemsToBeIncluded,
                    ThreadLocal<PatternMatchHelper> patternMatcherHelper,
                    CancellationToken cancellationToken
                )
                {
                    var highLightedList = GetHighlightedList(
                        session,
                        patternMatcherHelper.Value!,
                        itemsToBeIncluded,
                        cancellationToken
                    );
            ...
            private bool ShouldDismissCompletionListImmediately()
            {
                ...
                if (
                    _filterText.Length > 0
                    && char.IsNumber(_filterText[0])
                    && !IsAfterDot(_snapshotData.Snapshot, _applicableToSpan)
                )
                {
                    // Dismiss the session.
                    return true;
                ...
                if (
                    InitialTriggerReason == CompletionTriggerReason.Insertion
            ...
            private async Task AddCompletionItemsAsync(
                List<MatchResult> list,
                ThreadLocal<PatternMatchHelper> threadLocalPatternMatchHelper,
                CancellationToken cancellationToken
            )
            {
                ...
                Enumerable
                    .Range(0, _snapshotData.InitialSortedItemList.Count)
                    .AsParallel()
                ...
                void PromoteDefaultItemsToPreferredState()
                {
                    foreach (var includedDefault in includedDefaults.Values)
                    {
                        ...
                        var promotedDefaultItemMatchResult = new MatchResult(
                            Helpers.PromoteItem(
                                completionItem,
                                includedDefault.IndexInOriginalSortedOrder
                            ),
            ...
            )
            {
                ...
                try
                {
                    ...
                    if (filteredMatchResultsBuilder.Count == 0)
                    {
                        ...
                        if (ShouldSelectSuggestionItemWhenNoItemMatchesFilterText)
                            return new ItemSelection(
                                SelectedItemIndex: SuggestionItemIndex,
                                SelectionHint: UpdateSelectionHint.SoftSelected,
                                UniqueItem: null
                            );
                    ...
                    else
                    {
                        ...
                        var bestResult = GetBestCompletionItemSelectionFromFilteredResults(
                            filteredMatchResultsBuilder
                        );
                        ...
                        for (
                            selectedItemIndex = 0;
                            selectedItemIndex < matchResults.Count;
                            ++selectedItemIndex
                        )
                        {
                            if (
                                Equals(
                                    matchResults[selectedItemIndex].CompletionItem,
                                    bestResult.CompletionItem
                                )
                            )
                                break;
                        }
                        ...
                        if (_filterText.Length > 0)
                        {
                            ...
                            if (
                                matchResults.Count(matchResult =>
                                    matchResult.ShouldBeConsideredMatchingFilterText
                                    && !matchResult.CompletionItem.IsPreferredItem()
                                ) == 1
                            )
                                uniqueItem = GetCorrespondingVsCompletionItem(
                                    matchResults[selectedItemIndex],
                                    cancellationToken
                                );
                        }
                    }
                    ...
                    if (
                        UpdateTriggerReason == CompletionTriggerReason.Insertion
                        ...
                        && !Helpers.IsFilterCharacter(
                            bestOrFirstMatchResult.CompletionItem,
                            typedChar,
                            _filterText
                        )
                    )
            ...
            )
            {
                ...
                if (matchResult.IndexInOriginalSortedOrder >= 0)
                    return _snapshotData.InitialSortedItemList[
                        matchResult.IndexInOriginalSortedOrder
                    ];
            ...
            private ItemSelection? HandleDeletionTrigger(
                IReadOnlyList<MatchResult> items,
                CancellationToken cancellationToken
            )
            {
                ...
                return new(
                    ...
                    SelectionHint: hardSelect
                        ? UpdateSelectionHint.Selected
                        : UpdateSelectionHint.SoftSelected,
                    UniqueItem: moreThanOneMatch || !bestMatchResult.HasValue
                        ? null
                        : GetCorrespondingVsCompletionItem(bestMatchResult.Value, cancellationToken)
                );
            ...
            {
                return session.CreateCompletionList(
                    matchResults.Select(matchResult =>
                    {
                        var vsItem = GetCorrespondingVsCompletionItem(
                            matchResult,
                            cancellationToken
                        );
                ...
                {
                    if (
                        matchResult.CompletionItem.HasDifferentFilterText
                        || matchResult.CompletionItem.HasAdditionalFilterTexts
                    )
                    {
                        ...
                        return patternMatchers
                            .GetHighlightedSpans(
                                matchResult.CompletionItem.GetEntireDisplayText(),
            ...
            private FilteredCompletionModel? HandleAllItemsFilteredOut()
            {
                ...
                var selectedItemIndex = ShouldSelectSuggestionItemWhenNoItemMatchesFilterText
                    ? SuggestionItemIndex
                    : 0;
                ...
                // selection.
                return new FilteredCompletionModel(
                    items: ImmutableArray<CompletionItemWithHighlight>.Empty,
                    selectedItemIndex,
                    filters: _snapshotData.SelectedFilters,
                    selectionHint: UpdateSelectionHint.SoftSelected,
                    centerSelection: true,
                    uniqueItem: null
                );
            ...
            )
            {
                ...
                foreach (var item in matchResults)
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    filters.AddRange(
                        GetCorrespondingVsCompletionItem(item, cancellationToken).Filters
                    );
                }
            ...
            /// </summary>
            private static MatchResult GetBestCompletionItemSelectionFromFilteredResults(
            ...
            )
            {
                ...
                for (int i = 1, n = filteredMatchResults.Count; i < n; i++)
                {
                    ...
                    if (
                        bestResult.MatchedWithAdditionalFilterTexts
                        && !currentResult.MatchedWithAdditionalFilterTexts
                    )
                    {
                        bestResult = currentResult;
                    }
            ...
            private static bool TryGetInitialTriggerLocation(
            ...
            )
            {
                ...
                {
                    if (
                        CompletionItemData.TryGetData(item, out var itemData)
                        && itemData.TriggerLocation.HasValue
                    )
                    {
            ...
            private bool IsHardSelection(RoslynCompletionItem item, bool matchedFilterText)
            {
                ...
                Debug.Assert(
                    _filterText.Length > 0 || item.Rules.MatchPriority != MatchPriority.Default
                );
            ...
            private ItemSelection UpdateSelectionBasedOnSuggestedDefaults(
                IReadOnlyList<MatchResult> items,
            ...
            )
            {
                ...
                if (
                    _snapshotData.Defaults.IsDefaultOrEmpty
                ...
                if (selectedItem.Rules.MatchPriority >= MatchPriority.Preselect)
                {
                    ...
                    if (
                        !selectedItem.IsPreferredItem()
                        || Helpers.TryGetOriginalIndexOfPromotedItem(selectedItem, out _)
                    )
                        return itemSelection;
                }
            ...
            /// </summary>
            private ItemSelection GetDefaultsMatch(
                IReadOnlyList<MatchResult> matches,
                ItemSelection initialSelection,
                CancellationToken cancellationToken
            )
            {
                ...
                else
                {
                    ...
                    while (++inferiorItemIndex < matches.Count)
                    {
                        ...
                        if (
                            ...
                            || itemMatch.Value.IsCaseSensitive
                                != selectedItemMatch.Value.IsCaseSensitive
                        )
                        {
                            break;
                        }
                ...
                var selectionHint =
                    _filterText.Length == 0
                        ? UpdateSelectionHint.Selected
                        : initialSelection.SelectionHint;
                return initialSelection with
                {
                    SelectedItemIndex = bestPromotedItemSoFar,
            ...
            private sealed class FilterStateHelper
            {
                ...
                public FilterStateHelper(ImmutableArray<CompletionFilterWithState> filtersWithState)
                {
                    ...
                    _nonExpanderFilterStates = filtersWithState.WhereAsArray(f =>
                        f.Filter is not CompletionExpander
                    );

                    _selectedNonExpanderFilters = _nonExpanderFilterStates.SelectAsArray(
                        f => f.IsSelected,
                        f => f.Filter
                    );
                    _needToFilter =
                        _selectedNonExpanderFilters.Length > 0
                        && _selectedNonExpanderFilters.Length < _nonExpanderFilterStates.Length;

                    _unselectedExpanders = filtersWithState.SelectAsArray(
                ...
                public bool ShouldBeFilteredOut(VSCompletionItem item) =>
                    ShouldBeFilteredOutOfCompletionList(item)
                    || ShouldBeFilteredOutOfExpandedCompletionList(item);
            ...
            private readonly record struct ItemSelection(
                int SelectedItemIndex,
                UpdateSelectionHint SelectionHint,
                VSCompletionItem? UniqueItem
            );
        }
    }
}
