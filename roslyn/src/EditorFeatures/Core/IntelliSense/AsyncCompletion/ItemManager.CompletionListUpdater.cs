    ...
    internal partial class ItemManager
    {
        ...
        private sealed class CompletionListUpdater
        {
            ...
            private readonly Func<
                ImmutableArray<(RoslynCompletionItem, PatternMatch?)>,
                string,
            ...
            private static readonly ObjectPool<
                List<MatchResult<VSCompletionItem>>
            > s_listOfMatchResultPool = new(factory: () => new(), size: 1);
            ...
            )
            {
                ...
                else
                {
                    ...
                    _filterMethod = (itemsWithPatternMatches, text) =>
                        CompletionService.FilterItems(
                            _completionHelper,
                            itemsWithPatternMatches,
                            text
            ...
            private void AddCompletionItems(
                List<MatchResult<VSCompletionItem>> list,
                CancellationToken cancellationToken
            )
            {
            ...
            )
            {
                ...
                else
                {
                    ...
                    var bestItem = GetBestCompletionItemBasedOnMRUFirstOtherwiseOnPriority(
                        chosenItems
                    );
                    ...
                    if (_filterText.Length > 0)
                    {
                        ...
                        if (
                            matchingItems.Count(r => !r.RoslynCompletionItem.IsPreferredItem()) == 1
                        )
                ...
                if (
                    ...
                    && !Helpers.IsFilterCharacter(
                        bestOrFirstMatchResult.RoslynCompletionItem,
                        typedChar,
                        _filterText
                    )
                )
                ...
                var isHardSelection = IsHardSelection(
                    bestOrFirstMatchResult.RoslynCompletionItem,
                    bestOrFirstMatchResult.MatchedFilterText
                );
            ...
            )
            {
                ...
                for (var i = 0; i < items.Count; ++i)
                {
                    ...
                    {
                        var match = currentMatchResult.CompareTo(
            ...
            {
                using var _ = ArrayBuilder<CompletionItemWithHighlight>.GetInstance(
                    items.Count,
                    out var builder
                );
                builder.AddRange(
                    items.Select(matchResult =>
                    {
                        cancellationToken.ThrowIfCancellationRequested();
                ...
                )
                {
                    if (matchResult.RoslynCompletionItem.HasDifferentFilterText)
                    {
                        ...
                        return completionHelper
                            .GetHighlightedSpans(
                                matchResult.EditorCompletionItem.DisplayText,
                                filterText,
            ...
            /// </summary>
            private RoslynCompletionItem GetBestCompletionItemBasedOnMRUFirstOtherwiseOnPriority(
            ...
            private sealed class FilterStateHelper
            {
                ...
                private bool ShouldBeFilteredOutOfCompletionList(VSCompletionItem item) =>
                    _needToFilter
                    && !item.Filters.Any(filter => _selectedNonExpanderFilters.Contains(filter));
                ...
                public void LogTargetTypeFilterTelemetry(CompletionSessionData sessionData)
                {
                    if (sessionData.TargetTypeFilterExperimentEnabled)
                    {
                        ...
                        if (
                            ...
                            && _selectedNonExpanderFilters.Any(
