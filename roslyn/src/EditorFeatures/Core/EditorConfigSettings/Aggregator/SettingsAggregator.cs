...
namespace Microsoft.CodeAnalysis.Editor.EditorConfigSettings
{
    internal partial class SettingsAggregator : ISettingsAggregator
    {
        ...
        private static ISettingsProviderFactory<T> GetOptionsProviderFactory<T>(Workspace workspace)
        {
            ...
            var commonProvider = workspace.Services
                .GetRequiredService<IWorkspaceSettingsProviderFactory<T>>();
            providers.Add(commonProvider);
            var solution = workspace.CurrentSolution;
            var supportsCSharp = solution.Projects
                .Any(
                    p => p.Language.Equals(LanguageNames.CSharp, StringComparison.OrdinalIgnoreCase)
                );
            var supportsVisualBasic = solution.Projects
                .Any(
                    p =>
                        p.Language
                            .Equals(LanguageNames.VisualBasic, StringComparison.OrdinalIgnoreCase)
                );
            if (supportsCSharp)
            {
