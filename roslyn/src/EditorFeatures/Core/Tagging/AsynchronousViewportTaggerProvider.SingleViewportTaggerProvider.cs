...
internal abstract partial class AsynchronousViewportTaggerProvider<TTag>
    where TTag : ITag
{
    /// <summary>
        ...
        )
    {
        ...
        protected override SpanTrackingMode SpanTrackingMode => _callback.SpanTrackingMode;
        ...
        protected override TaggerDelay EventChangeDelay
            ...
            _viewPortToTag == ViewPortToTag.InView
                ? _callback.EventChangeDelay
                : TaggerDelay.NonFocus;
        ...
        )
        {
            ...
            var visibleSpanOpt = textView.GetVisibleLinesSpan(
                subjectBuffer,
                extraLines: s_standardLineCountAroundViewportToTag
            );
            ...
            var widenedSpanOpt = textView.GetVisibleLinesSpan(
                subjectBuffer,
                extraLines: _callback._extraLinesAroundViewportToTag
            );
            Contract.ThrowIfNull(
                widenedSpanOpt,
                "Should not ever fail getting the widened span as we were able to get the normal visible span"
            );

            var widenedSpan = widenedSpanOpt.Value;
            Contract.ThrowIfFalse(
                widenedSpan.Span.Contains(visibleSpan.Span),
                "The widened span must be at least as large as the visible one."
            );
            ...
            {
                var aboveSpan = new SnapshotSpan(
                    visibleSpan.Snapshot,
                    Span.FromBounds(widenedSpan.Span.Start, visibleSpan.Span.Start)
                );
            ...
            {
                var belowSpan = new SnapshotSpan(
                    visibleSpan.Snapshot,
                    Span.FromBounds(visibleSpan.Span.End, widenedSpan.Span.End)
                );
        ...
        )
        {
            foreach (var spanToTag in context.SpansToTag)
            {
                ...
                await _callback
                    .ProduceTagsAsync(context, spanToTag, cancellationToken)
