            ...
            ICommandHandler<RemoveParametersCommandArgs>
    {
        ...
        )
        {
            ...
            )
            {
                ...
                var changeSignatureContext = changeSignatureService
        ...
        )
        {
            ...
            if (!result.Succeeded)
            {
                ...
                {
                    ShowError(
                        result.ChangeSignatureFailureKind.Value,
                        context.OperationContext,
                        notificationService
                    );
                }
            ...
            if (
                result.ConfirmationMessage != null
            ...
            using var workspaceUndoTransaction = workspace.OpenGlobalUndoTransaction(
                FeaturesResources.Change_signature
            );
        ...
        )
        {
            switch (reason)
            {
                ...
                case ChangeSignatureFailureKind.IncorrectKind:
                    ShowMessage(
                        FeaturesResources.You_can_only_change_the_signature_of_a_constructor_indexer_method_or_delegate,
                        NotificationSeverity.Error,
                        operationContext,
                        notificationService
                    );
