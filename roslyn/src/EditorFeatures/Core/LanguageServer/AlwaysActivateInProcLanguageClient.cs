...
{
    /// <summary>
    /// Language client responsible for handling C# / VB / F# LSP requests in any scenario (both local
    // and codespaces).
    /// This powers "LSP only" features (e.g. cntrl+Q code search) that do not use traditional editor
    // APIs.
        ...
        )
    {
        ...
        {
            // If the LSP editor feature flag is enabled advertise support for LSP features here so they are
            // available locally and remote.
            ...
            // This capability prevents NavigateTo (cntrl+,) from using LSP symbol search when the server also
            // supports WorkspaceSymbolProvider.
            // Since WorkspaceSymbolProvider=true always to allow cntrl+Q VS search to function, we set
            // DisableGoToWorkspaceSymbols=true
            // when not running the experimental LSP editor.  This ensures NavigateTo uses the existing editor
            // APIs.
            // However, when the experimental LSP editor is enabled we want LSP to power NavigateTo, so we set
            // DisableGoToWorkspaceSymbols=false.
            ...
            if (isLspSemanticTokensEnabled)
            {
                ...
                // full/edits/range handling,
                // especially for larger files. With range handling, we only need to compute tokens for whatever is
                // in view, while
                // with full/edits handling we need to compute tokens for the entire file and then potentially run a
                // diff between
        ...
        /// <summary>
        /// When pull diagnostics is enabled, ensure that initialization failures are displayed to the user
        // as
        /// they will get no diagnostics.  When not enabled we don't show the failure box (failure will
        // still be recorded in the task status center)
