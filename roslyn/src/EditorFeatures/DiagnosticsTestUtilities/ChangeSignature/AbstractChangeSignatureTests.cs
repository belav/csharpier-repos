...
namespace Microsoft.CodeAnalysis.Editor.UnitTests.ChangeSignature
{
    ...
    {
        protected override ParseOptions GetScriptOptions() => throw new NotSupportedException();
        ...
        )
        {
            if (expectedCodeAction)
            {
                ...
                await TestActionAsync(
                    workspace,
                    expectedCode,
                    refactoring.CodeActions[index].action,
            ...
            )
            {
                ...
                if (verifyNoDiagnostics)
                {
                    ...
                    if (diagnostics.Length > 0)
                    {
                        Assert.True(
                            false,
                            CreateDiagnosticsString(
                                diagnostics,
                                updatedSignature,
                                testState.InvocationDocument,
                                totalParameters,
                                (await testState.InvocationDocument.GetTextAsync()).ToString()
                            )
                        );
                    }
                }
        ...
        private static string CreateDiagnosticsString(
            ImmutableArray<Diagnostic> diagnostics,
            AddedParameterOrExistingIndex[] permutation,
            Document document,
            int? totalParameters,
            string fileContents
        )
        ...
        {
            var existingParametersKept = signature
        ...
        /// ? s?{m, n}�(S k=0 to s�(sCk * (s-k)!)) * 2^p�- 1 changed signatures to consider.
        /// </summary>
        ...
        )
        {
            ...
            var totalParameters =
                signaturePartCounts[0]
                + signaturePartCounts[1]
                + signaturePartCounts[2]
                + signaturePartCounts[3];
        ...
        private static IEnumerable<int[]> GetAllSignatureSpecifications(int[] signaturePartCounts)
        {
            ...
            var paramParameterIndex =
                signaturePartCounts[0] + signaturePartCounts[1] + signaturePartCounts[2];

            var regularParameterArrangements = GetPermutedSubsets(
                regularParameterStartIndex,
                signaturePartCounts[1]
            );
            var defaultValueParameterArrangements = GetPermutedSubsets(
            ...
            foreach (var regularParameterPart in regularParameterArrangements)
            {
                ...
                {
                    var p1 = startArray
                        .Concat(regularParameterPart)
                        .Concat(defaultValueParameterPart);
        ...
        private static IEnumerable<IEnumerable<int>> GetPermutations(IEnumerable<int> list)
        {
            ...
            {
                var permutationsWithoutElement = GetPermutations(
        ...
        private static IEnumerable<IEnumerable<int>> GetSubsets(IEnumerable<int> list)
        {
            if (!list.Any())
            {
                return SpecializedCollections.SingletonEnumerable(
                    SpecializedCollections.EmptyEnumerable<int>()
                );
            }
            ...
            var withFirstElement = withoutFirstElement.Select(
                without => firstElement.Concat(without)
            );
