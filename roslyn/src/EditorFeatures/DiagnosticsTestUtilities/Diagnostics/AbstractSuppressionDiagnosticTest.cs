    ...
    public abstract class AbstractSuppressionDiagnosticTest : AbstractUserDiagnosticTest
    {
        ...
        )
        {
            ...
            var diagnostics = await DiagnosticProviderTestUtilities.GetAllDiagnosticsAsync(
                ...
                document,
                span,
                includeNonLocalDocumentDiagnostics: parameters.includeNonLocalDocumentDiagnostics
            );
        ...
        )> GetDiagnosticAndFixesAsync(TestWorkspace workspace, TestParameters parameters)
        {
            ...
            var diagnostics = (await testDriver.GetAllDiagnosticsAsync(document, span)).Where(d =>
                fixer.IsFixableDiagnostic(d)
            );

            var filteredDiagnostics = FilterDiagnostics(diagnostics);

            var wrapperCodeFixer = new WrapperCodeFixProvider(
                fixer,
                filteredDiagnostics.Select(d => d.Id)
            );
