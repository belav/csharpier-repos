        ...
        : AbstractBuiltInUnnecessaryCodeStyleDiagnosticAnalyzer
    {
        public CSharpRemoveUnnecessaryLambdaExpressionDiagnosticAnalyzer()
            : base(
                ...
                new LocalizableResourceString(
                    nameof(CSharpAnalyzersResources.Lambda_expression_can_be_removed),
                    CSharpAnalyzersResources.ResourceManager,
                    typeof(CSharpAnalyzersResources)
                )
            ) { }
        ...
        )
        {
            ...
            if (
                lambdaTypeInfo.ConvertedType == null
                || lambdaTypeInfo.ConvertedType.SpecialType is SpecialType.System_Object
            )
                return;

            var lambdaSymbolInfo = semanticModel.GetSymbolInfo(
                anonymousFunction,
                cancellationToken
            );
            ...
            var invokedSymbolInfo = semanticModel.GetSymbolInfo(
                invokedExpression,
                cancellationToken
            );
            ...
            if (
                invokedMethod.IsPartialDefinition && invokedMethod.PartialImplementationPart is null
            )
                return;
            ...
            var invokedTypeArguments = invokedExpression.GetRightmostName()
            ...
            if (
                !IsIdentityOrImplicitConversion(
                    compilation,
            ...
            if (
                invokedMethod
                    .GetAttributes()
                    .Any(a => Equals(a.AttributeClass, conditionalAttributeType))
            )
                return;
            ...
            // that later happens
            if (
                invokedExpression is MemberAccessExpressionSyntax
                {
                    Expression: var accessedExpression
                }
            )
            {
                ...
                var outermostBody = invokedExpression
                    .AncestorsAndSelf()
                    .Last(n =>
                        n
                            is BlockSyntax
                ...
                foreach (
                    var candidate in outermostBody.DescendantNodes().OfType<ExpressionSyntax>()
                )
                {
                    if (
                        candidate != accessedExpression
            ...
            var rewrittenSymbolInfo = rewrittenSemanticModel.GetSymbolInfo(
                rewrittenExpression,
                cancellationToken
            );
            if (
                rewrittenSymbolInfo.Symbol is not IMethodSymbol rewrittenMethod
            ...
            if (
                OverloadsChanged(
                    semanticModel,
                    anonymousFunction.GetRequiredParent(),
                    rewrittenSemanticModel,
                    rewrittenExpression.GetRequiredParent(),
                    cancellationToken
                )
            )
            {
            ...
            var startReportSpan = TextSpan.FromBounds(
                anonymousFunction.SpanStart,
                invokedExpression.SpanStart
            );
            var endReportSpan = TextSpan.FromBounds(
                invokedExpression.Span.End,
                anonymousFunction.Span.End
            );
        ...
        )
        {
            while (node1 != null && node2 != null)
            {
                ...
                var method1 =
                    semanticModel1.GetSymbolInfo(node1, cancellationToken).Symbol as IMethodSymbol;
                var method2 =
                ...
                if (
                    method1 is not null
                    && !method1.Equals(method2, SymbolEqualityComparer.IncludeNullability)
                )
                    return true;
