    ...
    : AbstractBuiltInCodeStyleDiagnosticAnalyzer
{
    ...
    )
    {
        ...
        typeDeclaration =
            typeSymbol.DeclaringSyntaxReferences[0].GetSyntax(cancellationToken)
            as TypeDeclarationSyntax;
        if (typeDeclaration is null)
            return false;

        var options = new CSharpAnalyzerOptionsProvider(
            context.Options.AnalyzerConfigOptionsProvider.GetOptions(typeDeclaration.SyntaxTree),
            context.Options
        );
