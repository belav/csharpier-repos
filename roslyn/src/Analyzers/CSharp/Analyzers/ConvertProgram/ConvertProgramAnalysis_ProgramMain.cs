...
namespace Microsoft.CodeAnalysis.CSharp.Analyzers.ConvertProgram
{
    internal static partial class ConvertProgramAnalysis
    {
        ...
        public static Location GetUseProgramMainDiagnosticLocation(
            CompilationUnitSyntax root,
            bool isHidden
        )
        {
            ...
            var lastPragma = root.GetFirstToken()
                .LeadingTrivia.LastOrDefault(
                    t => t.Kind() is SyntaxKind.PragmaWarningDirectiveTrivia
                );
            ...
            return Location.Create(
                ...
                TextSpan.FromBounds(
                    start,
                    root.Members.OfType<GlobalStatementSyntax>().Last().FullSpan.End
                )
            );
        }
    }
