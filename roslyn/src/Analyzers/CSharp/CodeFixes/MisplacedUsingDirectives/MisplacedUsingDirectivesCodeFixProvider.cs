    ...
    internal sealed partial class MisplacedUsingDirectivesCodeFixProvider : CodeFixProvider
    {
        ...
        public override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            var options = await document
                .GetCSharpCodeFixOptionsProviderAsync(
            ...
            var (placement, preferPreservation) = DeterminePlacement(
                compilationUnit,
                options.UsingDirectivePlacement
            );
        ...
        private static ImmutableArray<UsingDirectiveSyntax> GetAllUsingDirectives(
...
#pragma warning disable RS0030 // Do not used banned APIs (ReduceAsync with SimplifierOptions isn't public)
            return await Simplifier
                .ReduceAsync(newDocument, Simplifier.Annotation, optionSet: null, cancellationToken)
                .ConfigureAwait(false);
#pragma warning restore
        ...
        )
        {
            ...
            var compilationUnitWithoutUsings = compilationUnit.WithUsings(
                SyntaxFactory.List(compilationUnit.Usings.Where(u => u.GlobalKeyword != default))
            );
            var compilationUnitWithoutBlankLine =
                compilationUnitWithoutUsings.Usings.Count == 0
        ...
        {
            var namespaceDeclarations = compilationUnit.Members
            ...
            var usingsToAdd = namespaceDeclarationMap.Values
                ...
                .Select(directive =>
                    directive.WithAdditionalAnnotations(Formatter.Annotation, s_warningAnnotation)
                );
        ...
        private static (
            BaseNamespaceDeclarationSyntax namespaceWithoutUsings,
        ...
        {
            var namespaceDeclarations = usingContainer.Members
                .OfType<BaseNamespaceDeclarationSyntax>();
            var namespaceDeclarationMap = namespaceDeclarations.ToDictionary(
                namespaceDeclaration => namespaceDeclaration,
            ...
            var usingsFromNamespaces = namespaceDeclarationMap.Values
                .SelectMany(result => result.usingsFromNamespace);
        ...
        private static (
            IEnumerable<UsingDirectiveSyntax> deduplicatedUsings,
            IEnumerable<SyntaxTrivia> orphanedTrivia
        ) RemoveDuplicateUsings(
        ...
        )
        {
            ...
            foreach (var usingDirective in usingsToAdd)
            {
                ...
                if (
                    seenUsings.Any(seenUsingDirective =>
        ...
        private static (AddImportPlacement placement, bool preferPreservation) DeterminePlacement(
            CompilationUnitSyntax compilationUnit,
            CodeStyleOption2<AddImportPlacement> styleOption
        )
        {
        ...
        private static (
            CompilationUnitSyntax compilationUnitWithoutHeader,
            ImmutableArray<SyntaxTrivia> header
        ) RemoveFileHeader(CompilationUnitSyntax syntaxRoot, IFileBannerFactsService bannerService)
        {
