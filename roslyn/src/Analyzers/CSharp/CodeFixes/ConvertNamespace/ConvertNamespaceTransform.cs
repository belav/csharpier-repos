...
namespace Microsoft.CodeAnalysis.CSharp.ConvertNamespace
{
    internal static class ConvertNamespaceTransform
    {
        ...
        )
        {
            switch (baseNamespace)
            {
                case FileScopedNamespaceDeclarationSyntax fileScopedNamespace:
                    return await ConvertFileScopedNamespaceAsync(
                            document,
                            fileScopedNamespace,
                            cancellationToken
                        )
                ...
                case NamespaceDeclarationSyntax namespaceDeclaration:
                    var (doc, _) = await ConvertNamespaceDeclarationAsync(
                            document,
        ...
        public static async Task<(Document document, TextSpan semicolonSpan)> ConvertNamespaceDeclarationAsync(
            Document document,
        ...
        )
        {
            ...
            var indentation = await GetIndentationAsync(
                    document,
                    namespaceDeclaration,
                    options,
                    cancellationToken
            ...
            var (updatedDocument, semicolonSpan) = await ReplaceWithFileScopedNamespaceAsync(
                    document,
                    namespaceDeclaration,
                    annotation,
                    cancellationToken
            ...
            return await DedentNamespaceAsync(
                    updatedDocument,
                    indentation,
                    annotation,
                    cancellationToken
        ...
        private static async Task<string?> GetIndentationAsync(
            Document document,
            NamespaceDeclarationSyntax namespaceDeclaration,
            SyntaxFormattingOptions options,
            CancellationToken cancellationToken
        )
        {
            ...
            var openBraceLine = sourceText.Lines
                .GetLineFromPosition(namespaceDeclaration.OpenBraceToken.SpanStart)
                .LineNumber;
            var closeBraceLine = sourceText.Lines
        ...
        )
        {
            ...
            if (
                namespaceDeclaration.OpenBraceToken.LeadingTrivia.Any(
                    t => t.IsSingleOrMultiLineComment() || t.IsDirective
                )
                || namespaceDeclaration.OpenBraceToken.TrailingTrivia.Any(
                    t => t.IsSingleOrMultiLineComment() || t.IsDirective
                )
            )
            {
                fileScopedNamespace = fileScopedNamespace.ReplaceToken(
                    ...
                    firstBodyToken.WithPrependedLeadingTrivia(
                        namespaceDeclaration.OpenBraceToken.GetAllTrivia()
                    )
                );
                firstBodyToken =
                    fileScopedNamespace.SemicolonToken.GetNextNonZeroWidthTokenOrEndOfFile();
            }
            ...
            fileScopedNamespace = fileScopedNamespace.WithAppendedTrailingTrivia(
            ...
            if (
                !namespaceDeclaration.HasTrailingTrivia
