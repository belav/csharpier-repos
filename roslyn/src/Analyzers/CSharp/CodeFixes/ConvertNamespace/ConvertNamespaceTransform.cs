...
{
    public static Task<Document> ConvertAsync(
        Document document,
        ...
        baseNamespace switch
        {
            ...
            NamespaceDeclarationSyntax namespaceDeclaration => ConvertNamespaceDeclarationAsync(
                document,
    ...
    /// </summary>
    public static async Task<Document> ConvertNamespaceDeclarationAsync(
        Document document,
    ...
    )
    {
        ...
        var (updatedRoot, _) = ReplaceWithFileScopedNamespace(
            parsedDocument,
            namespaceDeclaration,
            annotation
        );
        ...
        var updatedParsedDocument = await ParsedDocument
            .CreateAsync(updatedDocument, cancellationToken)
            .ConfigureAwait(false);
        var (dedentedText, _) = DedentNamespace(
    ...
    )
    {
        // Replace the block namespace with the file scoped namespace.
        var annotation = new SyntaxAnnotation();
        var (updatedRoot, semicolonSpan) = ReplaceWithFileScopedNamespace(
            document,
            namespaceDeclaration,
            annotation
        );
    ...
    )
    {
        ...
        ) lastRawStringLiteralData = default;
        ...
        TextChange? TryDedentLine(TextLine textLine)
        {
            ...
            if (
                syntaxTree.IsEntirelyWithinStringLiteral(
                    textLine.Span.Start,
            ...
            )
            {
                if (
                    stringLiteral.Kind()
                    is not SyntaxKind.MultiLineRawStringLiteralToken
                        and not SyntaxKind.Utf8MultiLineRawStringLiteralToken
                )
                    return null;
                ...
                if (lastRawStringLiteralData.stringLiteral != stringLiteral)
                    lastRawStringLiteralData = (
                        stringLiteral,
                        ComputeCommonIndentationLength(
                            text.Lines.GetLineFromPosition(stringLiteral.Span.End)
                        )
                    );
        ...
        int ComputeCommonIndentationLength(TextLine textLine)
        {
            ...
            while (
                commonIndentation < indentation.Length && commonIndentation < textLine.Span.Length
            )
            {
    ...
    )
    {
        ...
        using var _ = ArrayBuilder<TextChange>.GetInstance(
            Math.Max(0, closeBraceLine - openBraceLine - 1),
            out var changes
        );
        for (var line = openBraceLine + 1; line < closeBraceLine; line++)
            changes.AddIfNotNull(
                TryIndentLine(syntaxTree, root, indentation, text.Lines[line], cancellationToken)
            );
    ...
    )
    {
        ...
        if (textLine.Text![textLine.Start] == '#')
        {
            ...
            if (
                token.IsKind(SyntaxKind.HashToken)
                && token.Parent!.Kind()
                    is not (SyntaxKind.RegionDirectiveTrivia or SyntaxKind.EndRegionDirectiveTrivia)
            )
            {
    ...
    )
    {
        ...
        // Replace the block namespace with the file scoped namespace.
        var annotation = new SyntaxAnnotation();
        var updatedRoot = ReplaceWithBlockScopedNamespace(
            parsedDocument,
            fileScopedNamespace,
            options.NewLine,
            options.NewLines,
        ...
        // Auto-formatting options are not relevant since they only control behavior on typing.
        var indentation = FormattingExtensions.CreateIndentationString(
        ...
        var updatedParsedDocument = await ParsedDocument
            .CreateAsync(updatedDocument, cancellationToken)
            .ConfigureAwait(false);
        var indentedText = IndentNamespace(
            updatedParsedDocument,
            indentation,
            annotation,
            cancellationToken
    ...
    {
        var converted = ConvertFileScopedNamespace(
            document,
            namespaceDeclaration,
            lineEnding,
            newLinePlacement
        ...
        var lineWithNextToken = document.Text
            .Lines
        ...
        var (splitPosition, needsAdditionalLineEnding) =
            lineWithNextToken.GetFirstNonWhitespacePosition() < tokenAfterNamespace.SpanStart
                ...
                : (
                    document.Text.Lines.GetLineFromPosition(tokenAfterNamespace.SpanStart).Start,
                    false
                );
        var triviaBeforeSplit = tokenAfterNamespace.LeadingTrivia
            .TakeWhile(trivia => trivia.SpanStart < splitPosition)
            .ToArray();
        var triviaAfterSplit = tokenAfterNamespace.LeadingTrivia
        ...
        if (triviaBeforeSplit.Length > 0)
        {
            ...
            converted = converted.WithCloseBraceToken(
                converted.CloseBraceToken.WithPrependedLeadingTrivia(triviaBeforeSplit)
            );
        }
        ...
        var firstBodyToken = converted.OpenBraceToken
    ...
    private static bool HasLeadingBlankLine(SyntaxToken token, out SyntaxToken withoutBlankLine)
    {
        ...
        if (
            leadingTrivia
            is [(kind: SyntaxKind.WhitespaceTrivia), (kind: SyntaxKind.EndOfLineTrivia), ..]
        )
        {
    ...
    )
    {
        ...
        var originalHadTrailingNewLine =
            namespaceDeclaration.GetTrailingTrivia() is [.., (kind: SyntaxKind.EndOfLineTrivia)];
    ...
    {
        var nameSyntax = fileScopedNamespace.Name
        ...
        var openBraceToken = SyntaxFactory
        ...
        {
            openBraceToken = openBraceToken.WithAppendedTrailingTrivia(
                SyntaxFactory.EndOfLine(lineEnding)
            );
        }

        FileScopedNamespaceDeclarationSyntax adjustedFileScopedNamespace;
        var closeBraceToken = SyntaxFactory
        ...
        {
            closeBraceToken = closeBraceToken.WithAppendedTrailingTrivia(
                SyntaxFactory.EndOfLine(lineEnding)
            );
        ...
        else
        {
            ...
            // closing brace.
            adjustedFileScopedNamespace = fileScopedNamespace.WithAppendedTrailingTrivia(
                SyntaxFactory.EndOfLine(lineEnding)
            );
        }

        // If the file scoped namespace is indented, also indent the newly added braces to match
        var outerIndentation = document.Text
        ...
        if (outerIndentation.Length > 0)
        {
            if (newLinePlacement.HasFlag(NewLinePlacement.BeforeOpenBraceInTypes))
                openBraceToken = openBraceToken.WithLeadingTrivia(
                    openBraceToken.LeadingTrivia.Add(SyntaxFactory.Whitespace(outerIndentation))
                );

            closeBraceToken = closeBraceToken.WithLeadingTrivia(
                closeBraceToken.LeadingTrivia.Add(SyntaxFactory.Whitespace(outerIndentation))
            );
        }
