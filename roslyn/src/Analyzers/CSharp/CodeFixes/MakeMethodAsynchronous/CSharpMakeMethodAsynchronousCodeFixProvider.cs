...
{
    [
        ExportCodeFixProvider(LanguageNames.CSharp, Name = PredefinedCodeFixProviderNames.AddAsync),
        Shared
    ]
    internal class CSharpMakeMethodAsynchronousCodeFixProvider
        : AbstractMakeMethodAsynchronousCodeFixProvider
    {
        ...
        private static readonly SyntaxToken s_asyncToken = SyntaxFactory.Token(
        ...
        protected override bool IsSupportedDiagnostic(
        ...
        protected override bool IsAsyncReturnType(ITypeSymbol type, KnownTypes knownTypes) =>
            IsIAsyncEnumerableOrEnumerator(type, knownTypes) || IsTaskLike(type, knownTypes);
        ...
        )
        {
            return node switch
            {
                MethodDeclarationSyntax method => FixMethod(
                    keepVoid,
                    methodSymbol,
                    method,
                    knownTypes,
                ...
                LocalFunctionStatementSyntax localFunction => FixLocalFunction(
                    keepVoid,
                    methodSymbol,
                    localFunction,
                    knownTypes,
        ...
        )
        {
            ...
            var newModifiers = AddAsyncModifierWithCorrectedTrivia(
                method.Modifiers,
                ref newReturnType
            );
        ...
        )
        {
            ...
            else
            {
                ...
                if (
                    IsIEnumerable(returnType, knownTypes)
                    && IsIterator(methodSymbol, cancellationToken)
                )
                {
                    newReturnType = knownTypes.IAsyncEnumerableOfTTypeOpt is null
                        ...
                        : knownTypes
                            .IAsyncEnumerableOfTTypeOpt.Construct(
                                methodSymbol.ReturnType.GetTypeArguments()[0]
                            )
                            .GenerateTypeSyntax();
                }
                else if (
                    IsIEnumerator(returnType, knownTypes)
                    && IsIterator(methodSymbol, cancellationToken)
                )
                {
                    newReturnType = knownTypes.IAsyncEnumeratorOfTTypeOpt is null
                        ...
                        : knownTypes
                            .IAsyncEnumeratorOfTTypeOpt.Construct(
                                methodSymbol.ReturnType.GetTypeArguments()[0]
                            )
                            .GenerateTypeSyntax();
                }
                ...
                else if (!IsTaskLike(returnType, knownTypes))
                {
                    ...
                    newReturnType = knownTypes
                        .TaskOfTType!.Construct(methodSymbol.ReturnType)
                        .GenerateTypeSyntax();
                }
            }
        ...
        private static bool IsIterator(IMethodSymbol method, CancellationToken cancellationToken) =>
            method.Locations.Any(
                static (loc, cancellationToken) => loc.FindNode(cancellationToken).ContainsYield(),
                cancellationToken
            );

        private static bool IsIAsyncEnumerableOrEnumerator(
        ...
        )
        {
            ...
            var result = SyntaxFactory.TokenList(
                s_asyncToken.WithLeadingTrivia(newReturnType.GetLeadingTrivia())
            );
            newReturnType = newReturnType.WithoutLeadingTrivia();
            return result;
        }

        private static AnonymousFunctionExpressionSyntax FixAnonymousFunction(
