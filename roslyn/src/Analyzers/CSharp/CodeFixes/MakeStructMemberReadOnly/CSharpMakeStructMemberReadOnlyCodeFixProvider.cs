...
[
    ExportCodeFixProvider(
        ...
        Name = PredefinedCodeFixProviderNames.MakeStructMemberReadOnly
    ),
    Shared
]
    ...
    : SyntaxEditorBasedCodeFixProvider
{
    ...
    public override Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        RegisterCodeFix(
            context,
            CSharpAnalyzersResources.Make_member_readonly,
            nameof(CSharpAnalyzersResources.Make_member_readonly)
        );
    ...
    )
    {
        ...
        foreach (var declaration in declarations.OrderByDescending(t => t.SpanStart))
        {
            ...
            if (declaration is MethodDeclarationSyntax or BasePropertyDeclarationSyntax)
            {
                ...
                declaration is AccessorDeclarationSyntax
                {
                    ...
                    {
                        Parent: BasePropertyDeclarationSyntax property
                    } accessorList
                } accessor
            )
            {
                ...
                if (accessorList.Accessors.Count == 1)
                {
                    ...
                    editor.ReplaceNode(property, UpdateReadOnlyModifier(property, add: true));
                }
        ...
        TNode UpdateReadOnlyModifier<TNode>(TNode node, bool add)
            where TNode : SyntaxNode
        {
            return (TNode)
                generator.WithModifiers(node, generator.GetModifiers(node).WithIsReadOnly(add));
        }
    }
}
