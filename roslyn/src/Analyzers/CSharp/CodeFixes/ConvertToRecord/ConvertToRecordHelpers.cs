...
namespace Microsoft.CodeAnalysis.CSharp.ConvertToRecord
{
    internal static class ConvertToRecordHelpers
    {
                                ...
                                Operations: [
                                    IReturnOperation
                                    {
                                        ReturnedValue: IInvocationOperation
                                        {
                                            ...
                                            Arguments: [
                                                IArgumentOperation { Value: IOperation arg },
                                            ]
                                        }
                                    },
                                ]
                            }
                        && arg.WalkDownConversion()
                            is IParameterReferenceOperation { Parameter: IParameterSymbol param }
        ...
        public static INamedTypeSymbol? GetIEquatableType(
            Compilation compilation,
        ...
        )
        {
            ...
            var equatable = compilation
                .GetBestTypeByMetadataName("System.IEquatable`1")
                ?.Construct(containingType);
        ...
        )
        {
            ...
            )
            {
                ...
                var (_, members, _) = analyzer.GetHashedMembers(
                    methodSymbol,
                    methodOperation.BlockBody ?? methodOperation.ExpressionBody
                );
                if (members != default)
                {
        ...
        {
            if (
                GetBlockOfMethodBody(operation) is not { Operations.Length: int opLength }
            ...
            if (
                assignmentValues.Keys.SetEquals(properties)
        ...
        )
        {
            ...
            var assignedUnderlyingFields = assignmentValues
                .Keys
                .SelectAsArray(UnwrapPropertyToField);
        ...
        )
        {
            ...
            )
            {
                ...
                foreach (var arg in args)
                {
                    if (
                        arg is
                        {
                            Parameter: IParameterSymbol param,
                            Value.Syntax: ExpressionSyntax captured
                        }
                    )
                    {
            ...
            var expressions = GetAssignmentExpressionsFromValuesMap(
                positionalParams,
                assignmentValues
            );
        ...
        )
        {
            ...
            {
                var dictionaryBuilder = ImmutableDictionary<ISymbol, ExpressionSyntax>
                    .Empty
                    .ToBuilder();
        ...
        )
        {
            ...
            else if (parameter.Type.SpecialType == SpecialType.System_Object)
            {
                ...
                if (
                    bodyOps is [IReturnOperation { ReturnedValue: IOperation value }]
                ...
                // and a variable assignment happens (either in the if or in a following statement)
                    ...
                    !TryGetBindingCastInFirstIfStatement(
                        bodyOps,
                        parameter,
                        type,
                        fields,
                        out otherC,
                        out statementsToCheck
                    )
                )
                {
                    return ImmutableArray<IFieldSymbol>.Empty;
        ...
        )
        {
            ...
            else if (operation is IIsPatternOperation patternOp)
            {
                ...
                {
                    constantPattern =
                        (patternOp.Pattern as INegatedPatternOperation)?.Pattern
                        as IConstantPatternOperation;
                }
                else
                {
        ...
        )
        {
            ...
            if (
                memberReference1.Member.Equals(memberReference2.Member)
                && leftObject != null
                && rightObject != null
        ...
        )
        {
            ...
            if (
                !TryGetSuccessCondition(
                    whenTrue,
                    whenFalse,
                    bodyOps.Skip(1).AsImmutable(),
                    out var successRequirement,
                    out var remainingStatments
                )
            )
            {
                return false;
            ...
            statementsToCheck = !remainingStatments.IsEmpty()
                ? remainingStatments
                : bodyOps.Skip(1);
            ...
            )
            {
                ...
                if (
                    TryGetAssignmentFromParameterWithExplicitCast(
                        statementsToCheck.FirstOrDefault(),
                        parameter,
                        out otherC
                    )
                )
        ...
        /// </summary>
        private static bool OverridesEquals(
            Compilation compilation,
            IMethodSymbol equals,
            INamedTypeSymbol? equatableType
        )
        {
            if (
                equatableType != null
                && equatableType.GetMembers(nameof(Equals)).FirstOrDefault()
                    is IMethodSymbol equatableEquals
                && equals.Equals(
                    equals.ContainingType.FindImplementationForInterfaceMember(equatableEquals)
                )
            )
            {
                return true;
            ...
            var objectEquals =
                objectType?.GetMembers(nameof(Equals)).FirstOrDefault() as IMethodSymbol;
            var curr = equals;
            while (curr != null)
            {
        ...
        private static bool AreConditionsSatisfiedEitherOrder<T>(
            T firstItem,
            T secondItem,
