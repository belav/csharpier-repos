...
namespace Microsoft.CodeAnalysis.CSharp.CodeFixes.TransposeRecordKeyword
{
    [
        ExportCodeFixProvider(
            ...
            Name = PredefinedCodeFixProviderNames.TransposeRecordKeyword
        ),
        Shared
    ]
    internal class CSharpTransposeRecordKeywordCodeFixProvider : SyntaxEditorBasedCodeFixProvider
    {
        ...
        private static bool TryGetRecordDeclaration(
            ...
            [NotNullWhen(true)] out RecordDeclarationSyntax? recordDeclaration
        )
        {
            recordDeclaration =
                diagnostic.Location.FindNode(cancellationToken) as RecordDeclarationSyntax;
                ...
                if (skippedTriviaIndex >= 0)
                {
                    ...
                    if (tokens.Count == 1)
                    {
                        ...
                        )
                        {
                            ...
                            var remainingLeadingTrivia = SyntaxFactory.TriviaList(
                                leadingTrivia.Skip(skippedTriviaIndex + 1)
                            );
                            var trailingTriviaTakeUntil = remainingLeadingTrivia.IndexOf(
                            ...
                            classOrStructKeyword = classOrStructKeyword
                                .WithLeadingTrivia(
                                    SyntaxFactory.TriviaList(
                                        remainingLeadingTrivia.Skip(trailingTriviaTakeUntil)
                                    )
                                )
                            ...
                            recordKeyword = recordKeyword
                                ...
                                .WithTrailingTrivia(
                                    SyntaxFactory.TriviaList(
                                        remainingLeadingTrivia.Take(trailingTriviaTakeUntil)
                                    )
                                );
        ...
        public override Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            if (
                TryGetRecordDeclaration(diagnostic, cancellationToken, out var recordDeclaration)
