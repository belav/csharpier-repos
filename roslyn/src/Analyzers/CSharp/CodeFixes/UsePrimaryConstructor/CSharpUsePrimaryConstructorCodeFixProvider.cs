...
[
    ExportCodeFixProvider(
        ...
        Name = PredefinedCodeFixProviderNames.UsePrimaryConstructor
    ),
    Shared
]
...
internal partial class CSharpUsePrimaryConstructorCodeFixProvider() : CodeFixProvider
{
    ...
    public override Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        foreach (var diagnostic in context.Diagnostics)
        {
            ...
            {
                var resource =
                    diagnostic.Properties.ContainsKey(AllFieldsName)
                        ? CSharpCodeFixesResources.Use_primary_constructor_and_remove_fields
                    : diagnostic.Properties.ContainsKey(AllPropertiesName)
                        ? CSharpCodeFixesResources.Use_primary_constructor_and_remove_properties
    ...
    )
    {
        ...
        var constructor = semanticModel.GetRequiredDeclaredSymbol(
            constructorDeclaration,
            cancellationToken
        );
        ...
        var constructorDocumentEditor = await solutionEditor
        ...
        ParameterListSyntax UpdateReferencesToNestedMembers(ParameterListSyntax parameterList)
        {
            return parameterList.ReplaceNodes(
                ...
                (nameSyntax, currentNameSyntax) =>
                {
                    ...
                    if (
                        nameSyntax.Parent is not QualifiedNameSyntax qualifiedNameSyntax
                    ...
                    )
                    {
                        ...
                        if (symbol is INamedTypeSymbol { ContainingType: { } containingType })
                            return QualifiedName(
                                containingType.GenerateNameSyntax(),
                                currentNameSyntax
                            );
                    }

                    if (
                        nameSyntax.Parent is not MemberAccessExpressionSyntax memberAccessExpression
                    ...
                    )
                    {
                        var symbol = semanticModel
                            ...
                            .GetAnySymbol();
                        if (
                            symbol is IFieldSymbol { ContainingType: not null }
        ...
        async ValueTask MoveBaseConstructorArgumentsAsync()
        {
            ...
            var argumentList = RemoveElementIndentation(
                typeDeclaration,
                constructorDeclaration,
                constructorDeclaration.Initializer.ArgumentList,
                static list => list.Arguments
            );

            if (
                typeDeclaration.BaseList is { Types: [SimpleBaseTypeSyntax baseType, ..] }
                && semanticModel.GetSymbolInfo(baseType.Type, cancellationToken).GetAnySymbol()
            ...
            )
            {
                ...
                documentEditor.ReplaceNode(
                    ...
                    PrimaryConstructorBaseType(
                            baseType.Type.WithoutTrailingTrivia(),
                            argumentList.WithoutLeadingTrivia()
                        )
        ...
        async ValueTask ProcessConstructorAssignmentsAsync()
        {
            if (constructorDeclaration.ExpressionBody is not null)
            {
                ...
                await ProcessConstructorAssignmentAsync(
                        (AssignmentExpressionSyntax)
                            constructorDeclaration.ExpressionBody.Expression,
            ...
            else
            {
                ...
                foreach (var statement in constructorDeclaration.Body.Statements)
                {
                    ...
                    await ProcessConstructorAssignmentAsync(
                            (AssignmentExpressionSyntax)expressionStatement.Expression,
                            expressionStatement
                        )
                        .ConfigureAwait(false);
                }
        ...
        )
        {
            ...
            var declarationDocumentEditor = await solutionEditor
                .GetDocumentEditorAsync(declarationDocument.Id, cancellationToken)
                .ConfigureAwait(false);

            declarationDocumentEditor.ReplaceNode(
                declaration,
                UpdateDeclaration(declaration, assignmentExpression, expressionStatement)
                    .WithAdditionalAnnotations(Formatter.Annotation)
            );
        }

        SyntaxNode UpdateDeclaration(
        ...
        async ValueTask<ImmutableDictionary<ISymbol, SyntaxNode>> RemoveMembersAsync()
        {
            ...
            if (removeMembers)
            {
                ...
                foreach (var group in removedMembers.Values.GroupBy(n => n.SyntaxTree))
                {
                    ...
                    var documentEditor = await solutionEditor
                        .GetDocumentEditorAsync(memberDocument.Id, cancellationToken)
                        .ConfigureAwait(false);
        ...
        async ValueTask ReplaceReferencesToMemberWithParameterAsync(
            ISymbol member,
        ...
        )
        {
            ...
            var references = await SymbolFinder
                .FindReferencesAsync(member, solution, namedTypeDocuments, cancellationToken)
                .ConfigureAwait(false);

            using var _1 = PooledHashSet<SyntaxNode>.GetInstance(out var nodesToReplace);
            using var _2 = PooledHashSet<XmlEmptyElementSyntax>.GetInstance(
            ...
            foreach (var group in seeTagsToReplace.GroupBy(n => n.SyntaxTree))
            {
                ...
                foreach (var seeTag in group)
                {
                    var paramRefTag = seeTag
                        .ReplaceToken(
                            seeTag.Name.LocalName,
                            Identifier("paramref").WithTriviaFrom(seeTag.Name.LocalName)
                        )
                        .WithAttributes(
                            SingletonList<XmlAttributeSyntax>(XmlNameAttribute(parameterName))
                        );
