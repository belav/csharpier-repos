...
namespace Microsoft.CodeAnalysis.AliasAmbiguousType
{
    internal abstract class AbstractAliasAmbiguousTypeCodeFixProvider : CodeFixProvider
    {
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            var placementOption = await document
            ...
            foreach (
                var symbol in Sort(
                    symbolInfo.CandidateSymbols.Cast<ITypeSymbol>(),
                    placementOption.PlaceSystemNamespaceFirst
                )
            )
            {
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    string.Format(
                        CodeFixesResources.Alias_ambiguous_type_0,
        ...
        private static IEnumerable<ITypeSymbol> Sort(
        ...
        )
        {
            ...
            return types.OrderBy(
                (t1, t2) =>
                {
                    ...
                    for (
                        int i = 0, n = Math.Min(t1NameSegments.Length, t2NameSegments.Length);
                        i < n;
                        i++
                    )
                    {
                        ...
                        var comparer =
                            i == 0 && sortSystemFirst
                                ? SortSystemFirstComparer.Instance
        ...
        private static bool SymbolCandidatesContainsSupportedSymbols(SymbolInfo symbolInfo) =>
            ...
            symbolInfo.CandidateSymbols.All(symbol =>
                symbol.IsKind(SymbolKind.NamedType) && symbol.GetArity() == 0
            );
