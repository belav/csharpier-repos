...
namespace Microsoft.CodeAnalysis.UseNullPropagation
{
    ...
    {
        protected abstract bool TryGetBlock(
            SyntaxNode? node,
            [NotNullWhen(true)] out TStatementSyntax? block
        );
        protected abstract TStatementSyntax ReplaceBlockStatements(
            TStatementSyntax block,
            TStatementSyntax newInnerStatement
        );
        protected abstract SyntaxNode PostProcessElseIf(
            TIfStatementSyntax ifStatement,
            TStatementSyntax newWhenTrueStatement
        );
        protected abstract TElementBindingExpressionSyntax ElementBindingExpression(
            TElementBindingArgumentListSyntax argumentList
        );
        ...
        )
        {
            ...
            {
                var conditionalExpressionOrIfStatement = root.FindNode(
                ...
                {
                    FixConditionalExpression(
                        document,
                        editor,
                        semanticModel,
                        diagnostic,
                        conditionalExpressionOrIfStatement,
                        cancellationToken
                    );
                }
            }
        ...
        )
        {
            ...
            var conditionalPart = root.FindNode(
                diagnostic.AdditionalLocations[1].SourceSpan,
                getInnermostNodeForTie: true
            );
            var whenPart = root.FindNode(
                diagnostic.AdditionalLocations[2].SourceSpan,
                getInnermostNodeForTie: true
            );
        ...
        )
        {
            ...
            var whenTrueStatement = (TStatementSyntax)
                root.FindNode(diagnostic.AdditionalLocations[1].SourceSpan);
            var match = (TExpressionSyntax)
                root.FindNode(
                    diagnostic.AdditionalLocations[2].SourceSpan,
                    getInnermostNodeForTie: true
                );
            ...
            if (isElseIf && TryGetBlock(whenTrueStatement.Parent, out var block))
            {
            ...
            else
            {
                ...
                {
                    newWhenTrueStatement = newWhenTrueStatement.WithLeadingTrivia(
            ...
            if (
                !newWhenTrueStatement.GetTrailingTrivia().Any(syntaxFacts.IsRegularComment)
                && !isElseIf
            )
                newWhenTrueStatement = newWhenTrueStatement.WithTrailingTrivia(
                ...
                if (syntaxFacts.IsSimpleMemberAccessExpression(match.Parent))
                {
                    ...
                    if (arity == 0 && comparer.Equals(name, nameof(Nullable<int>.Value)))
                    {
                        ...
                        return CreateConditionalAccessExpression(
                            ...
                            container,
                            match,
                            memberAccess.GetRequiredParent()
                        );
                    }
                }
            }
            ...
            where TContainer : SyntaxNode
        {
            ...
            if (matchParent is TElementAccessExpressionSyntax elementAccess)
            {
                ...
                var argumentList = (TElementBindingArgumentListSyntax)
