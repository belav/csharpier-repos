...
namespace Microsoft.CodeAnalysis.NewLines.ConsecutiveStatementPlacement
{
    [
        ExportCodeFixProvider(
            ...
            Name = PredefinedCodeFixProviderNames.ConsecutiveStatementPlacement
        ),
        Shared
    ]
    internal sealed class ConsecutiveStatementPlacementCodeFixProvider : CodeFixProvider
    {
        ...
        )
        {
            ...
            var nextTokens = diagnostics.Select(
                d => d.AdditionalLocations[0].FindToken(cancellationToken)
            );
