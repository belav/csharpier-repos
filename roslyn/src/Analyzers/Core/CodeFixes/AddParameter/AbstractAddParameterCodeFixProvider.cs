        ...
        where TObjectCreationExpressionSyntax : SyntaxNode
    {
        ...
        )
        {
            if (node is TInvocationExpressionSyntax invocationExpression)
            {
                ...
                var isConstructorInitializer = candidates.All(m =>
            ...
            ImmutableArray<CodeAction> NestByOverload()
            {
                ...
                foreach (var data in codeFixData)
                {
                    ...
                    var title = GetCodeFixTitle(
                        CodeFixesResources.Add_parameter_to_0,
                        data.Method,
                    ...
                    if (data.CreateChangedSolutionCascading != null)
                    {
                        ...
                        var titleCascading = GetCodeFixTitle(
                            CodeFixesResources.Add_parameter_to_0_and_overrides_implementations,
                            data.Method,
                            includeParameters: true
                        );
        ...
        )
        {
            ...
            foreach (
                var argumentInsertPositionData in methodsAndArgumentsToAdd.OrderByDescending(t =>
                    t.ArgumentInsertionIndex
                )
            )
            {
                ...
                var cascadingFix = AddParameterService.HasCascadingDeclarations(methodToUpdate)
                    ? new Func<CancellationToken, Task<Solution>>(c =>
                        FixAsync(
                            document,
                            methodToUpdate,
                            argumentToInsert,
