...
namespace Microsoft.CodeAnalysis.CodeFixes.MatchFolderAndNamespace
{
        ...
        : CodeFixProvider
    {
        ...
        private static async Task<Solution> FixAllInDocumentAsync(
            Document document,
        ...
        )
        {
            ...
            var targetNamespace = diagnostics.First().Properties[
            ...
            var targetFolders = PathMetadataUtilities.BuildFoldersFromNamespace(
                targetNamespace,
                document.Project.DefaultNamespace
            );
            var documentWithInvalidFolders = document.WithFolders(
                document.Folders.Concat("Force-Namespace-Change")
            );
            var renameActionSet = await Renamer
                .RenameDocumentAsync(
            ...
            var newSolution = await renameActionSet
                .UpdateSolutionAsync(documentWithInvalidFolders.Project.Solution, cancellationToken)
                .ConfigureAwait(false);
