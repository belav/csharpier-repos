...
{
    internal abstract class AbstractMatchFolderAndNamespaceDiagnosticAnalyzer<TNamespaceSyntax>
        : AbstractBuiltInCodeStyleDiagnosticAnalyzer where TNamespaceSyntax : SyntaxNode
    {
        ...
        )
        {
            ...
            var containsNamespace = namespaceDeclaration.DescendantNodes(n => n is TNamespaceSyntax)
            ...
            var isCurrentNamespaceInvalid =
                GetSyntaxFacts()
                    .GetNameOfNamespaceDeclaration(namespaceDeclaration)?.GetDiagnostics()
