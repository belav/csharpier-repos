...
namespace Microsoft.CodeAnalysis.RemoveUnnecessarySuppressions
{
        ...
        : AbstractCodeQualityDiagnosticAnalyzer
    {
        ...
        private static readonly DiagnosticDescriptor s_invalidScopeDescriptor = CreateDescriptor(
            ...
            s_localizableTitle,
            s_localizableInvalidScopeMessage,
            isUnnecessary: true
        );
        private static readonly DiagnosticDescriptor s_invalidOrMissingTargetDescriptor =
            CreateDescriptor(
                IDEDiagnosticIds.InvalidSuppressMessageAttributeDiagnosticId,
                EnforceOnBuildValues.InvalidSuppressMessageAttribute,
                s_localizableTitle,
                s_localizableInvalidOrMissingTargetMessage,
                isUnnecessary: true
            );
        ...
        internal static readonly DiagnosticDescriptor LegacyFormatTargetDescriptor =
            CreateDescriptor(
                ...
                s_localizableLegacyFormatTitle,
                s_localizableLegacyFormatMessage,
                isUnnecessary: false
            );
        ...
        protected sealed override void InitializeWorker(AnalysisContext context)
        {
            context.RegisterCompilationStartAction(
                context =>
                {
                    var suppressMessageAttributeType = context.Compilation
        ...
        protected sealed class CompilationAnalyzer
        {
            ...
            public void AnalyzeAssemblyOrModuleAttribute(
            ...
            )
            {
                ...
                if (
                    !SuppressMessageAttributeState.HasValidScope(
                ...
                if (
                    !_state.HasValidTarget(
                        namedAttributeArguments,
                        targetScope,
                        out var targetHasDocCommentIdFormat,
                        out var targetSymbolString,
                ...
                if (resolvedSymbols.Length > 0 && !targetHasDocCommentIdFormat)
                {
                    ...
                    if (resolvedSymbols.Length == 1)
                    {
                        ...
                        var docCommentId = DocumentationCommentId.CreateDeclarationId(
                            resolvedSymbols[0]
                        );
                    ...
                    reportDiagnostic(
                        Diagnostic.Create(
                            LegacyFormatTargetDescriptor,
                            targetValueOperation.Syntax.GetLocation(),
                            properties!,
                            targetSymbolString
                        )
                    );
