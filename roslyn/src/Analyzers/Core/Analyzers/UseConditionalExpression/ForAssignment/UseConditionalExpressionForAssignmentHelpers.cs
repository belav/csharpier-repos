...
namespace Microsoft.CodeAnalysis.UseConditionalExpression
{
    internal static class UseConditionalExpressionForAssignmentHelpers
    {
        ...
        )
        {
            ...
            // Since discard can discard value of any type, after converting if statement to a conditional
            // expression
            // it can produce compiler error if types are not the same and there is no implicit conversion
            // between them, e.g.:
            // if (flag)
            // {
            //     _ = 5;
            ...
            if (
                trueAssignment?.Target is IDiscardOperation
                && falseAssignment?.Target is IDiscardOperation
                && !AreEqualOrHaveImplicitConversion(
                    trueAssignment.Type,
                    falseAssignment.Type,
            ...
            if (
                ReferencesDeclaredVariableInAssignment(
                    ifOperation.Condition,
                    trueAssignment?.Target,
                    falseAssignment?.Target
                )
            )
                return false;
            ...
            static bool AreEqualOrHaveImplicitConversion(
            ...
            static bool ReferencesDeclaredVariableInAssignment(
                IOperation condition,
                IOperation? trueTarget,
                IOperation? falseTarget
            )
            {
                if (trueTarget is not null || falseTarget is not null)
                {
                    using var _1 = PooledHashSet<ILocalSymbol>.GetInstance(
                    ...
                    foreach (var operation in condition.DescendantsAndSelf())
                    {
                        // `if (x is String s)`
                        if (
                            operation is IDeclarationPatternOperation
                            {
                                DeclaredSymbol: ILocalSymbol local
                            }
                        )
                        ...
                        if (
                            ...
                            {
                                Expression: ILocalReferenceOperation localReference
                            }
                        )
            ...
            static bool ContainsLocalReference(
            ...
            )
            {
                if (target is not null)
                {
                    ...
                    {
                        if (
                            operation is ILocalReferenceOperation { Local: var local }
