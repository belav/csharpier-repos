...
namespace Microsoft.CodeAnalysis.RemoveUnusedMembers
{
        ...
        where TMemberDeclarationSyntax : SyntaxNode
    {
        ...
        private static readonly DiagnosticDescriptor s_removeUnusedMembersRule = CreateDescriptor(
            ...
            new LocalizableResourceString(
                nameof(AnalyzersResources.Private_member_0_is_unused),
                AnalyzersResources.ResourceManager,
                typeof(AnalyzersResources)
            ),
        ...
        internal static readonly DiagnosticDescriptor s_removeUnreadMembersRule = CreateDescriptor(
            ...
            new LocalizableResourceString(
                nameof(AnalyzersResources.Remove_unread_private_members),
                AnalyzersResources.ResourceManager,
                typeof(AnalyzersResources)
            ),
            new LocalizableResourceString(
                nameof(
                    AnalyzersResources.Private_member_0_can_be_removed_as_the_value_assigned_to_it_is_never_read
                ),
                AnalyzersResources.ResourceManager,
                typeof(AnalyzersResources)
            ),
        ...
        protected abstract IEnumerable<TTypeDeclarationSyntax> GetTypeDeclarations(
        ...
        private sealed class CompilationAnalyzer
        {
            ...
            private readonly AbstractRemoveUnusedMembersDiagnosticAnalyzer<
                TDocumentationCommentTriviaSyntax,
                TIdentifierNameSyntax,
                TTypeDeclarationSyntax,
                TMemberDeclarationSyntax
            ...
            )
            {
                ...
                _iNotifyCompletionType = compilation.GetBestTypeByMetadataName(
                    typeof(INotifyCompletion).FullName!
                );
                _deserializationConstructorCheck = new DeserializationConstructorCheck(compilation);
                _attributeSetForMethodsToIgnore = ImmutableHashSet.CreateRange<INamedTypeSymbol?>(
                    GetAttributesForMethodsToIgnore(compilation)
                );
            }

            private static Location GetDiagnosticLocation(ISymbol symbol) => symbol.Locations[0];
            ...
            private void RegisterActions(CompilationStartAnalysisContext compilationStartContext)
            {
                ...
                compilationStartContext.RegisterSymbolStartAction(
                    symbolStartContext =>
                    {
                        ...
                        symbolStartContext.RegisterOperationAction(
                            AnalyzeMemberReferenceOperation,
                        ...
                        symbolStartContext.RegisterOperationAction(
                            AnalyzeNameOfOperation,
                            OperationKind.NameOf
                        );
                        symbolStartContext.RegisterOperationAction(
                            AnalyzeObjectCreationOperation,
                        ...
                        symbolStartContext.RegisterSymbolEndAction(symbolEndContext =>
                            OnSymbolEnd(symbolEndContext, hasUnsupportedOperation)
                        );

                        // Register custom language-specific actions, if any.
                        _analyzer.HandleNamedTypeSymbolStart(
                            symbolStartContext,
                            onSymbolUsageFound
                        );
                    },
                ...
                bool ShouldAnalyze(SymbolStartAnalysisContext context, INamedTypeSymbol namedType)
                {
                    ...
                    {
                        if (
                            IsCandidateSymbol(member.OriginalDefinition)
            ...
            private void OnSymbolUsage(ISymbol? memberSymbol, ValueUsageInfo usageInfo)
            {
                ...
                lock (_gate)
                {
                    ...
                    if (
                        _symbolValueUsageStateMap.TryGetValue(
                            memberSymbol,
                    ...
                    if (valueUsageInfo == ValueUsageInfo.ReadWrite)
                    {
                        Debug.Assert(
                            memberReference.Parent
                                is ICompoundAssignmentOperation compoundAssignment
                                && compoundAssignment.Target == memberReference
                                || memberReference.Parent
                                    is ICoalesceAssignmentOperation coalesceAssignment
                                    && coalesceAssignment.Target == memberReference
                                || memberReference.Parent is IIncrementOrDecrementOperation
                                || memberReference.Parent is IReDimClauseOperation reDimClause
                                    && reDimClause.Operand == memberReference
                        );
                        ...
                        if (
                            memberReference?.Parent?.Parent is IExpressionStatementOperation
            ...
            private void AnalyzeNameOfOperation(OperationAnalysisContext operationContext)
            {
                ...
                {
                    OnSymbolUsage(
                        memberReference.Member.OriginalDefinition,
            ...
            )
            {
                ...
                var first = true;
                using var _1 = PooledHashSet<ISymbol>.GetInstance(
                    out var symbolsReferencedInDocComments
                );
                using var _2 = ArrayBuilder<string>.GetInstance(
                    ...
                    if (TryRemove(member, out var valueUsageInfo) && !valueUsageInfo.IsReadFrom())
                    {
                        ...
                        if (first)
                        {
                            ...
                            AddCandidateSymbolsReferencedInDocComments(
                                ...
                                symbolsReferencedInDocComments,
                                symbolEndContext.CancellationToken
                            );
                            ...
                            // This set is computed once and used for all the iterations of the loop.
                            AddDebuggerDisplayAttributeArguments(
                                namedType,
                                debuggerDisplayAttributeArguments
                            );
                        ...
                        var rule =
                            !valueUsageInfo.IsWrittenTo()
                            && !valueUsageInfo.IsNameOnly()
                            && !symbolsReferencedInDocComments.Contains(member)
            ...
            private LocalizableString GetMessage(DiagnosticDescriptor rule, ISymbol member)
            {
                ...
                return new DiagnosticHelper.LocalizableStringWithArguments(
                    messageFormat,
                    member.ToDisplayString(ContainingTypeAndNameOnlyFormat)
                );
            }
            ...
            {
                using var _ = ArrayBuilder<TDocumentationCommentTriviaSyntax>.GetInstance(
                    out var documentationComments
                );
                ...
                foreach (var group in documentationComments.GroupBy(d => d.SyntaxTree))
                {
                    ...
                    foreach (var docComment in group)
                    {
                        ...
                        foreach (
                            var node in docComment.DescendantNodes().OfType<TIdentifierNameSyntax>()
                        )
                        {
                ...
                void AddAllDocumentationComments()
                {
                    ...
                    foreach (
                        var typeDeclaration in _analyzer.GetTypeDeclarations(
                            namedTypeSymbol,
                            cancellationToken
                        )
                    )
                ...
                static void AddDocumentationComments(
                    ...
                    ArrayBuilder<TDocumentationCommentTriviaSyntax> documentationComments
                )
                {
                    ...
                    foreach (var trivia in firstToken.LeadingTrivia)
                    {
                        if (trivia.HasStructure)
                            documentationComments.AddIfNotNull(
            ...
            )
            {
                ...
                {
                    if (
                        attribute.AttributeClass == _debuggerDisplayAttributeType
