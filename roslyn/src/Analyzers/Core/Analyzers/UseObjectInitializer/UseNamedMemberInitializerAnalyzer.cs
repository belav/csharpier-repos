        ...
        new()
{
    ...
    )
    {
        ...
        {
            foreach (
                var init in this.SyntaxFacts.GetInitializersOfObjectMemberInitializer(initializer)
            )
            {
        ...
        foreach (var subsequentStatement in this.State.GetSubsequentStatements())
        {
            ...
            var expression = (TExpressionSyntax?)
                this.SyntaxFacts.GetExpressionOfMemberAccessExpression(leftMemberAccess);
            ...
            var leftSymbol = this.SemanticModel
                .GetSymbolInfo(leftMemberAccess, cancellationToken)
                .GetAnySymbol();
    ...
    private bool ImplicitMemberAccessWouldBeAffected(SyntaxNode node)
    {
        if (node != null)
        {
            ...
            {
                if (child.IsNode && ImplicitMemberAccessWouldBeAffected(child.AsNode()!))
                {
                    return true;
                }
            }
