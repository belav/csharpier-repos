    ...
    where TIdentifierName : TExpression
{
    ...
    > s_fieldWriteLocationPool = new(() => new());

    private static readonly Func<
        IFieldSymbol,
        ConcurrentSet<SyntaxNode>
    > s_createFieldWriteNodeSet = _ => s_nodeSetPool.Allocate();
    ...
    protected AbstractUseAutoPropertyAnalyzer()
        : base(
            ...
            new LocalizableResourceString(
                nameof(AnalyzersResources.Use_auto_property),
    ...
    protected static void AddFieldWrite(
        ...
        context.RegisterSymbolStartAction(
            context =>
            {
                ...
                context.RegisterSyntaxNodeAction(
                    context => AnalyzePropertyDeclaration(context, namedType, analysisResults),
                ...
                context.RegisterCodeBlockStartAction<TSyntaxKind>(context =>
                {
                    ...
                    RegisterNonConstructorFieldWrites(
                        fieldNames,
                        nonConstructorFieldWrites,
                        context.SemanticModel,
                        context.CodeBlock,
                        context.CancellationToken
                    );
                ...
                bool ShouldAnalyze(SymbolStartAnalysisContext context, INamedTypeSymbol namedType)
                {
                    ...
                    if (context.FilterSpan.HasValue)
                    {
                        ...
                        {
                            if (
                                node is TPropertyDeclaration or TFieldDeclaration
                                && context.ShouldAnalyzeSpan(node.Span, node.SyntaxTree)
                            )
                            {
                                shouldAnalyze = true;
                                break;
                        ...
                        if (
                            !shouldAnalyze
                            && analysisRoot.FirstAncestorOrSelf<SyntaxNode>(node =>
                                node is TPropertyDeclaration or TFieldDeclaration
                            ) == null
                        )
                            return false;
    ...
    )
    {
        ...
        if (
            ...
            is not TVariableDeclarator
            {
                Parent.Parent: TFieldDeclaration fieldDeclaration
            } variableDeclarator
        )
            return;
    ...
    protected virtual bool CanConvert(IPropertySymbol property) => true;
    ...
    )
    {
        foreach (var result in analysisResults)
        {
            ...
            {
                if (
                    result.Property.DeclaredAccessibility != Accessibility.Private
                    && result.Property.SetMethod is null
                    && nonConstructorFieldWrites.TryGetValue(result.Field, out var writeLocations1)
                    && writeLocations1.Any(loc =>
                        !loc.Ancestors().Contains(result.PropertyDeclaration)
                    )
                )
                {
                    continue;
            ...
            // semantics.
            if (
                result.Property.SetMethod is { IsInitOnly: true }
