...
namespace Microsoft.CodeAnalysis.AddRequiredParentheses
{
        ...
        where TLanguageKindEnum : struct
    {
        private static readonly Dictionary<
            (bool includeInFixAll, string equivalenceKey),
                ...
                foreach (var includeInFixAll in includeArray)
                {
                    ...
                    if (includeInFixAll)
                    {
                        properties = properties.Add(
                            AddRequiredParenthesesConstants.IncludeInFixAll,
                            ""
                        );
                    }
                    ...
                    properties = properties.Add(
                        AddRequiredParenthesesConstants.EquivalenceKey,
                        equivalenceKey
                    );
        ...
        private static string GetEquivalenceKey(
            PerLanguageOption2<CodeStyleOption2<ParenthesesPreference>> parentPrecedence
        ...
        private void AnalyzeSyntax(SyntaxNodeAnalysisContext context)
        {
            ...
            var childPrecedence = GetLanguageOption(
                _precedenceService.GetPrecedenceKind(binaryLike)
            );
            var parentPrecedence = GetLanguageOption(
                _precedenceService.GetPrecedenceKind(parentBinaryLike)
            );
            ...
            AddDiagnostics(
                ...
                additionalLocations,
                equivalenceKey,
                includeInFixAll: true
            );
        }

        private void AddDiagnostics(
            SyntaxNodeAnalysisContext context,
            TBinaryLikeExpressionSyntax? binaryLikeOpt,
            int precedence,
        ...
        )
        {
            ...
            )
            {
                ...
                AddDiagnostics(
                    context,
                    left as TBinaryLikeExpressionSyntax,
                    precedence,
                    severity,
                    additionalLocations,
                    equivalenceKey,
                    includeInFixAll: false
                );
                AddDiagnostics(
                    context,
                    right as TBinaryLikeExpressionSyntax,
                    precedence,
                    severity,
                    additionalLocations,
                    equivalenceKey,
