...
namespace Microsoft.CodeAnalysis.ForEachCast
{
    ...
    internal abstract class AbstractForEachCastDiagnosticAnalyzer<
        TSyntaxKind,
        ...
        where TForEachStatementSyntax : SyntaxNode
    {
        ...
        protected AbstractForEachCastDiagnosticAnalyzer()
            : base(
                ...
                title: new LocalizableResourceString(
                    nameof(AnalyzersResources.Add_explicit_cast),
                    AnalyzersResources.ResourceManager,
                    typeof(AnalyzersResources)
                ),
                messageFormat: new LocalizableResourceString(
                    nameof(
                        AnalyzersResources._0_statement_implicitly_converts_1_to_2_Add_an_explicit_cast_to_make_intent_clearer_as_it_may_fail_at_runtime
                    ),
                    AnalyzersResources.ResourceManager,
                    typeof(AnalyzersResources)
                )
        ...
        protected abstract (CommonConversion conversion, ITypeSymbol? collectionElementType) GetForEachInfo(
            SemanticModel semanticModel,
            TForEachStatementSyntax node
        );
            ...
            context.RegisterCompilationStartAction(context =>
            {
                ...
                if (ienumerableType != null && ienumerableOfTType != null)
                    context.RegisterSyntaxNodeAction(
                        context => AnalyzeSyntax(context, ienumerableType, ienumerableOfTType),
                        GetSyntaxKinds()
                    );
            });
        }
        ...
        protected void AnalyzeSyntax(
            ...
            INamedTypeSymbol ienumerableOfTType
        )
        {
            ...
            Contract.ThrowIfFalse(
                option.Value
                    is ForEachExplicitCastInSourcePreference.Always
                        or ForEachExplicitCastInSourcePreference.WhenStronglyTyped
            );

            if (
                semanticModel.GetOperation(node, cancellationToken)
                is not IForEachLoopOperation loopOperation
            )
                return;

            if (
                loopOperation.LoopControlVariable
                    is not IVariableDeclaratorOperation variableDeclarator
            ...
            var collectionType = semanticModel
                .GetTypeInfo(syntaxFacts.GetExpressionOfForeachStatement(node), cancellationToken)
                .Type;
            ...
            var isFixable =
                ...
                || collectionType.AllInterfaces.Any(i => i.Equals(ienumerableType))
        ...
        private static bool IsStronglyTyped(
            INamedTypeSymbol ienumerableOfTType,
            ...
            || collectionType.AllInterfaces.Any(
                i => i.OriginalDefinition.Equals(ienumerableOfTType)
            );
    }
}
