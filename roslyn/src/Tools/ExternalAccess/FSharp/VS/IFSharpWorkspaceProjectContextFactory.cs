...
namespace Microsoft.CodeAnalysis.ExternalAccess.FSharp
{
    ...
    internal sealed class FSharpWorkspaceProjectContextFactory
        : IFSharpWorkspaceProjectContextFactory
    {
        ...
        public FSharpWorkspaceProjectContextFactory(
            IWorkspaceProjectContextFactory factory,
        ...
        IFSharpWorkspaceProjectContext IFSharpWorkspaceProjectContextFactory.CreateProjectContext(
            string filePath,
            string uniqueName
        ) => CreateProjectContext(filePath, uniqueName);

        public FSharpWorkspaceProjectContext CreateProjectContext(
            string filePath,
            string uniqueName
        ) =>
    ...
    internal sealed class FSharpWorkspaceProjectContext : IFSharpWorkspaceProjectContext
    {
        ...
        public FSharpWorkspaceProjectContext(IWorkspaceProjectContext vsProjectContext)
        {
            ...
            _projectReferences = ImmutableDictionary.Create<string, IFSharpWorkspaceProjectContext>(
                StringComparer.OrdinalIgnoreCase
            );
            _metadataReferences = ImmutableHashSet.Create<string>(StringComparer.OrdinalIgnoreCase);
        }

        public void Dispose() => _vsProjectContext.Dispose();
        ...
        public ProjectId Id => _vsProjectContext.Id;

        public string FilePath => _vsProjectContext.ProjectFilePath;

        public int ProjectReferenceCount => _projectReferences.Count;

        public bool HasProjectReference(string filePath) =>
            _projectReferences.ContainsKey(filePath);

        public int MetadataReferenceCount => _metadataReferences.Count;
        ...
        public void SetProjectReferences(IEnumerable<IFSharpWorkspaceProjectContext> projRefs)
        {
            ...
            foreach (
                var reference in _projectReferences.Values.Cast<FSharpWorkspaceProjectContext>()
            )
            {
            ...
            {
                _vsProjectContext.AddProjectReference(
                    reference._vsProjectContext,
        ...
        public void AddMetadataReference(string referencePath) =>
            _vsProjectContext.AddMetadataReference(
                referencePath,
                MetadataReferenceProperties.Assembly
            );
        ...
        public void RemoveSourceFile(string path) => _vsProjectContext.RemoveSourceFile(path);
    }
}
