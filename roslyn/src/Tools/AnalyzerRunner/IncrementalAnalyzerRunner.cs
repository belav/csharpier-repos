...
namespace AnalyzerRunner
{
    public sealed class IncrementalAnalyzerRunner
    {
        ...
        public async Task RunAsync(CancellationToken cancellationToken)
        {
            ...
            workspaceConfigurationService.Options = new(
                CacheStorage: usePersistentStorage ? StorageDatabase.SQLite : StorageDatabase.None
            );
            ...
            foreach (var incrementalAnalyzerName in _options.IncrementalAnalyzerNames)
            {
                ...
                incrementalAnalyzerProvider ??= incrementalAnalyzerProviders
                    .Where(x => x.Metadata.Name == incrementalAnalyzerName)
                    .SingleOrDefault(provider =>
                        provider.Metadata.WorkspaceKinds?.Contains(WorkspaceKind.RemoteWorkspace)
                        ?? false
                    )
                    ?.Value;
                incrementalAnalyzerProvider ??= incrementalAnalyzerProviders
