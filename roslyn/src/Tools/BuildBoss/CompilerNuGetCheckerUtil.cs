...
namespace BuildBoss
{
    ...
    internal sealed class PackageContentsChecker : ICheckerUtil
    {
        ...
        private bool GetPackageAssets(TextWriter textWriter, List<PackageAsset> packageAssets)
        {
            ...
            coreClrAssets = coreClrAssets
                .Select(x => x.WithFileRelativeName(Path.Combine("bincore", x.FileRelativeName)))
                .ToList();
        ...
        {
            var relativeNameMap = new Dictionary<
                string,
                (PackageAsset PackageAsset, string OriginalFilePath)
            >(PathComparer);
            var allGood = true;
