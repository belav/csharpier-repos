...
namespace BoundTreeGenerator
{
    ...
    internal sealed class BoundNodeClassWriter
    {
        ...
        private void WriteUsing(string nsName)
        {
            ...
            {
                case TargetLanguage.CSharp:
                    WriteLine("using {0};", nsName);
                    break;
                case TargetLanguage.VB:
                    WriteLine("Imports {0}", nsName);
                    break;
                default:
                    throw new ArgumentException("Unexpected target language", nameof(_targetLang));
            }
        ...
        )
        {
            switch (_targetLang)
            {
                case TargetLanguage.CSharp:
                {
                    ...
                    IEnumerable<string> fields = isPublic
                        ? new[] { "SyntaxNode syntax" }
                        : new[] { "BoundKind kind", "SyntaxNode syntax" };
                    fields = fields.Concat(
                        from field in AllSpecifiableFields(node)
                ...
                case TargetLanguage.VB:
                {
                    ...
                    IEnumerable<string> fields = isPublic
                        ? new[] { "syntax As SyntaxNode" }
                        : new[] { "kind As BoundKind", "syntax as SyntaxNode" };
                    fields = fields.Concat(
                        from field in AllSpecifiableFields(node)
        ...
        {
            IEnumerable<Field> nullCheckFields = AllFields(node)
                .Where(f => FieldNullHandling(node, f.Name) == NullHandling.Disallow);
                ...
                foreach (Field field in nullCheckFields)
                {
                    ...
                    switch (_targetLang)
                    {
                        case TargetLanguage.CSharp:
                            if (isROArray)
                                WriteLine(
                                    "RoslynDebug.Assert(!{0}.IsDefault, \"Field '{0}' cannot be null (use Null=\\\"allow\\\" in BoundNodes.xml to remove this check)\");",
                                    ToCamelCase(field.Name)
                                );
                            else
                                WriteLine(
                                    "RoslynDebug.Assert({0} is object, \"Field '{0}' cannot be null (make the type nullable in BoundNodes.xml to remove this check)\");",
                                    ToCamelCase(field.Name)
                                );
                            break;
                        ...
                        case TargetLanguage.VB:
                            if (isROArray)
                                WriteLine(
                                    "Debug.Assert(Not ({0}.IsDefault), \"Field '{0}' cannot be null (use Null=\"\"allow\"\" in BoundNodes.xml to remove this check)\")",
                                    ToCamelCase(field.Name)
                                );
                            else
                                WriteLine(
                                    "Debug.Assert({0} IsNot Nothing, \"Field '{0}' cannot be null (use Null=\"\"allow\"\" in BoundNodes.xml to remove this check)\")",
                                    ToCamelCase(field.Name)
                                );
                            break;
                    }
        ...
        private IEnumerable<Field> AllFields(TreeType node)
        {
            if (node == null)
                return Enumerable.Empty<Field>();
            return from t in TypeAndBaseTypes(node) from f in Fields(t) select f;
        }
        ...
        {
            return from f in AllFields(node)
            where FieldNullHandling(node, f.Name) == NullHandling.Always
            select f;
        }
        ...
        {
            return AllFields(node)
                .Where(field => IsDerivedOrListOfDerived("BoundNode", field.Type));
        }
            ...
            return AllFields(node)
                .Where(
                    field =>
                        TypeIsSymbol(field)
                        || (
                            IsImmutableArray(field.Type, out var elementType)
                            && TypeIsSymbol(elementType)
                        )
                );
        }
        ...
        private NullHandling FieldNullHandling(TreeType node, string fieldName)
        {
            ...
            {
                if (
                    _targetLang == TargetLanguage.CSharp
        ...
        {
            var fieldsWithName =
                from f in FieldsIncludingOverrides(node)
            ...
            if (fieldsWithName.Any())
                ...
                throw new InvalidOperationException(
                    $"Field {fieldName} not found in type {node.Name}"
                );
        }
            ...
            switch (_targetLang)
            {
                case TargetLanguage.CSharp:
                    ...
                    if (IsPropertyOverrides(field))
                    {
                        ...
                        WriteLine(
                            "public override {0}{1} {2} {{ get {{ return _{2}; }} }}",
                            (IsNew(field) ? "new " : ""),
                            field.Type,
                            field.Name
                        );
                    ...
                    else if (field.Override)
                    {
                        ...
                        WriteLine(
                            $"public new {field.Type} {field.Name} => base.{field.Name}{suppression};"
                        );
                    }
                    else
                    {
        ...
        private void WriteAccept(string name)
        {
            switch (_targetLang)
            {
                case TargetLanguage.CSharp:
                    ...
                    WriteLine(
                        "public override BoundNode? Accept(BoundTreeVisitor visitor) => visitor.Visit{0}(this);",
                        StripBound(name)
                    );
                    break;
            ...
            switch (_targetLang)
            {
                case TargetLanguage.CSharp:
                {
                    ...
                    if (AllSpecifiableFields(node).Any())
                    {
                        Write("if ");
                        Paren();
                        Or(AllSpecifiableFields(node), field => wasUpdatedCheck(field));
                        UnParen();
                        Blank();
                        Brace();
        ...
        private static bool TypeIsTypeSymbol(Field field) =>
            ...
            switch (_targetLang)
            {
                case TargetLanguage.CSharp:
                    ...
                    foreach (var node in _tree.Types.OfType<Node>())
                    {
                        ...
                        Indent();
                        WriteLine(
                            "return Visit{0}(({1})node, arg);",
                            StripBound(node.Name),
                            node.Name
                        );
                        Outdent();
                    }
                    ...
                    {
                        WriteLine(
                            $"public virtual R Visit{StripBound(node.Name)}({node.Name} node, A arg) => this.DefaultVisit(node, arg);"
                        );
                    }
                    Unbrace();
                    ...
                    {
                        WriteLine(
                            $"public virtual BoundNode? Visit{StripBound(node.Name)}({node.Name} node) => this.DefaultVisit(node);"
                        );
                    }
                    Unbrace();
                    break;
                ...
                case TargetLanguage.VB:
                    ...
                    foreach (var node in _tree.Types.OfType<Node>())
                    {
                        ...
                        Indent();
                        WriteLine(
                            "Return Visit{0}(CType(node, {1}), arg)",
                            StripBound(node.Name),
                            node.Name
                        );
                        Outdent();
                    ...
                    {
                        WriteLine(
                            "Public Overridable Function Visit{0}(node As {1}, arg As A) As R",
                            StripBound(node.Name),
                            node.Name
                        );
                        Indent();
        ...
        private void WriteWalker()
        {
            switch (_targetLang)
            {
                case TargetLanguage.CSharp:
                    ...
                    foreach (var node in _tree.Types.OfType<Node>())
                    {
                        var fields = AllFields(node)
                            .Where(
                                f =>
                                    IsDerivedOrListOfDerived("BoundNode", f.Type)
                                    && !SkipInVisitor(f)
                            );
                        if (!fields.Any())
                        {
                            WriteLine(
                                $"{GetVisitFunctionDeclaration(node.Name, isOverride: true)} => null;"
                            );
                            continue;
                        }
        ...
        private void WriteTreeDumperNodeProducer()
        {
            switch (_targetLang)
            {
                case TargetLanguage.CSharp:
                    ...
                    {
                        Write(
                            "public override TreeDumperNode Visit{0}({1} node, object? arg) => new TreeDumperNode(\"{2}\", null, ",
                            StripBound(node.Name),
                            node.Name,
                            ToCamelCase(StripBound(node.Name))
                        );
                        ...
                        if (allFields.Length > 0)
                        {
                            ...
                            for (int i = 0; i < allFields.Length; ++i)
                            {
                                ...
                                if (IsDerivedType("BoundNode", field.Type))
                                    Write(
                                        "new TreeDumperNode(\"{0}\", null, new TreeDumperNode[] {{ Visit(node.{1}, null) }})",
                                        ToCamelCase(field.Name),
                                        field.Name
                                    );
                                else if (IsListOfDerived("BoundNode", field.Type))
                                {
                                    ...
                                    {
                                        Write(
                                            "new TreeDumperNode(\"{0}\", null, node.{1}.IsDefault ? Array.Empty<TreeDumperNode>() : from x in node.{1} select Visit(x, null))",
                                            ToCamelCase(field.Name),
                                            field.Name
                                        );
                                    }
                                    ...
                                    Write(
                                        "new TreeDumperNode(\"{0}\", node.{1}, null)",
                                        ToCamelCase(field.Name),
                                        field.Name
                                    );
                ...
                case TargetLanguage.VB:
                    ...
                    {
                        WriteLine(
                            "Public Overrides Function Visit{0}(node As {1}, arg As Object) As TreeDumperNode",
                            StripBound(node.Name),
                            node.Name
                        );
                        Indent();
                        Write(
                            "Return New TreeDumperNode(\"{0}\", Nothing, ",
                            ToCamelCase(StripBound(node.Name))
                        );
                        var allFields = AllFields(node).ToArray();
                        if (allFields.Length > 0)
                        {
                            ...
                            for (int i = 0; i < allFields.Length; ++i)
                            {
                                ...
                                if (IsDerivedType("BoundNode", field.Type))
                                    Write(
                                        "New TreeDumperNode(\"{0}\", Nothing, new TreeDumperNode() {{Visit(node.{1}, Nothing)}})",
                                        ToCamelCase(field.Name),
                                        field.Name
                                    );
                                else if (IsListOfDerived("BoundNode", field.Type))
                                    ...
                                    Write(
                                        "New TreeDumperNode(\"{0}\", node.{1}, Nothing)",
                                        ToCamelCase(field.Name),
                                        field.Name
                                    );
            ...
            switch (_targetLang)
            {
                case TargetLanguage.CSharp:
                {
                    ...
                    foreach (var node in _tree.Types.OfType<Node>())
                    {
                        ...
                        {
                            WriteLine(
                                $"{GetVisitFunctionDeclaration(node.Name, isOverride: true)} => node;"
                            );
                            continue;
                        }
                        WriteLine(GetVisitFunctionDeclaration(node.Name, isOverride: true));
                        ...
                        foreach (Field field in AllTypeFields(node))
                        {
                            hadField = true;
                            WriteLine(
                                "TypeSymbol? {0} = this.VisitType(node.{1});",
                                ToCamelCase(field.Name),
                                field.Name
                            );
                        }
                ...
                case TargetLanguage.VB:
                {
                    ...
                    foreach (var node in _tree.Types.OfType<Node>())
                    {
                        ...
                        foreach (Field field in AllTypeFields(node))
                        {
                            hadField = true;
                            WriteLine(
                                "Dim {0} as TypeSymbol = Me.VisitType(node.{1})",
                                ToCamelCase(field.Name),
                                field.Name
                            );
                        }
        ...
        private void WriteNullabilityRewriter()
        {
            switch (_targetLang)
            {
                ...
                case TargetLanguage.CSharp:
                {
                    ...
                    WriteLine(
                        $"private readonly ImmutableDictionary<BoundExpression, (NullabilityInfo Info, TypeSymbol? Type)> {updatedNullabilities};"
                    );
                    WriteLine(
                        $"private readonly NullableWalker.SnapshotManager? {snapshotManager};"
                    );
                    WriteLine(
                        $"private readonly ImmutableDictionary<Symbol, Symbol>.Builder {remappedSymbols};"
                    );
                    ...
                    foreach (var node in _tree.Types.OfType<Node>())
                    {
                        ...
                        foreach (var field in AllSymbolOrSymbolListFields(node))
                        {
                            ...
                            {
                                WriteLine(
                                    $"{field.Type} {ToCamelCase(field.Name)} = GetUpdatedSymbol(node, node.{field.Name});"
                                );
                                hadField = true;
                            }
                            ...
                            {
                                WriteLine(
                                    $"{field.Type} {ToCamelCase(field.Name)} = GetUpdatedArray(node, node.{field.Name});"
                                );
                                hadField = true;
                            }
                        }
                        ...
                        foreach (var field in AllNodeOrNodeListFields(node))
                        {
                            ...
                            WriteNodeVisitCall(
                                field,
                                forceVisit: VisitFieldOnlyInNullabilityRewriter(field)
                            );
                        }
                        ...
                        void writeNullabilityCheck(bool inverted) =>
                            WriteLine(
                                $"if ({(inverted ? "!" : "")}{updatedNullabilities}.TryGetValue(node, out (NullabilityInfo Info, TypeSymbol? Type) infoAndType))"
                            );
        ...
        private bool IsValueType(string typeName) =>
            _valueTypes.Contains(GetGenericType(typeName).TrimEnd('?'));
        ...
        private void WriteNodeVisitCall(Field field, bool forceVisit = false)
        {
            switch (_targetLang)
            {
                case TargetLanguage.CSharp:
                    ...
                    {
                        WriteLine(
                            $"{field.Type} {ToCamelCase(field.Name)} = this.VisitList(node.{field.Name});"
                        );
                    }
                    else
                    {
                        WriteLine(
                            $"{field.Type} {ToCamelCase(field.Name)} = ({field.Type})this.Visit(node.{field.Name});"
                        );
                    }
                    break;
                ...
                case TargetLanguage.VB:
                    ...
                    {
                        WriteLine(
                            "Dim {0} As {2} = Me.VisitList(node.{1})",
                            ToCamelCase(field.Name),
                            field.Name,
                            field.Type
                        );
                    ...
                    {
                        WriteLine(
                            "Dim {0} As {2} = DirectCast(Me.Visit(node.{1}), {2})",
                            ToCamelCase(field.Name),
                            field.Name,
                            field.Type
                        );
