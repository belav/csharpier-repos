    ...
    internal sealed partial class IOperationClassWriter
    {
        ...
        private void WriteFiles()
        {
            ...
            using (
                _writer = new StreamWriter(
                    File.Open(
                        Path.Combine(_location, "OperationKind.Generated.cs"),
                        FileMode.Create
                    ),
                    Encoding.UTF8
                )
        ...
        {
            WriteLine(
                $"namespace Microsoft.CodeAnalysis{(namespaceSuffix is null ? "" : $".{namespaceSuffix}")}"
            );
            Brace();
        }
        ...
        private void WriteInterface(AbstractNode node)
        {
            ...
            WriteLine(
                $"{(node.IsInternal ? "internal" : "public")} interface {node.Name} : {node.Base}"
            );
            Brace();
            ...
            void writeReservedRemarkText()
            {
                if (operationKinds.Any())
                {
                    ...
                    foreach (var kind in operationKinds)
                        WriteLine(
                            $"/// <item><description><see cref=\"OperationKind.{kind}\"/></description></item>"
                        );
        ...
        private void WriteInterfaceProperty(Property prop)
        {
            ...
            WriteComments(
                prop.Comments,
                operationKinds: Enumerable.Empty<string>(),
                writeReservedRemark: false
            );
        ...
        private void WriteOperationKind()
        {
            ...
            var unusedKinds =
                _tree.UnusedOperationKinds.Entries?.Select(e => e.Value).ToList()
                ?? new List<int>();
        ...
        private void WriteClasses()
        {
            ...
            void writeClass(AbstractNode type)
            {
                ...
                bool hasSkippedProperties = !GetAllProperties(
                        type,
                        includeSkipGenerationProperties: true
                    )
                    .SequenceEqual(allProps);
                ...
                writeConstructor(
                    type.IsAbstract ? "protected" : "internal",
                    @class,
                    allProps,
                    baseProperties,
                    type,
                ...
                if (node != null)
                {
                    ...
                    WriteLine(
                        $"public override ITypeSymbol? Type {(node.HasType ? "{ get; }" : "=> null;")}"
                    );

                    WriteLine(
                        $"internal override ConstantValue? OperationConstantValue {(hasConstantValue ? "{ get; }" : "=> null;")}"
                    );
                ...
                )
                {
                    ...
                    var constantValueString = hasConstantValue
                        ? "ConstantValue? constantValue, "
                        : string.Empty;
                    Write(
                        $"SemanticModel? semanticModel, SyntaxNode syntax, {typeParameterString}{constantValueString}bool isImplicit"
                    );
                ...
                void writeProperty(Property prop, string propExtensibility)
                {
                    if (prop.Type == "CommonConversion")
                    {
                        ...
                        WriteLine(
                            $"internal IConvertibleConversion {prop.Name}Convertible {{ get; }}"
                        );
                        WriteLine(
                ...
                void writeAcceptMethods(string visitorName)
                {
                    ...
                    WriteLine(
                        $"public override TResult? Accept<TArgument, TResult>(OperationVisitor<TArgument, TResult> visitor, TArgument argument) where TResult : default => visitor.{visitorName}(this, argument);"
                    );
                }
                ...
                string getKind(AbstractNode node)
                {
                    ...
                    {
                        node =
                            (AbstractNode?)_typeMap[node.Base]
                            ?? throw new InvalidOperationException(
                                $"{node.Name} is not being included in OperationKind, but has no base type!"
                            );
                    }

                    if (node.OperationKind?.Entries.Count > 0)
                    {
                        return node.OperationKind.Entries
                ...
                )
                {
                    if (publicIOperationProps.Count > 0)
                    {
                        ...
                        {
                            Debug.Assert(
                                node.ChildrenOrder != null,
                                $"Encountered null children order for {type.Name}, should have been caught in verifier!"
                            );
                    ...
                    void writeMoveNext(List<Property> orderedProperties)
                    {
                        ...
                        for (; slot < orderedProperties.Count; slot++)
                        {
                            ...
                            {
                                WriteLine(
                                    $"case {slot} when previousIndex + 1 < {prop.Name}.Length:"
                                );
                                Indent();
                    ...
                    void writeMoveNextReversed(List<Property> orderedProperties)
                    {
                        ...
                        for (; slot >= 0; slot--)
                        {
                            ...
                            var previousSlot =
                                slot == (orderedProperties.Count - 1)
                                    ? "int.MaxValue"
                                    : (slot + 1).ToString();
                            var prop = orderedProperties[slot];
                            ...
                            {
                                WriteLine(
                                    $"if (!{prop.Name}.IsEmpty) return (true, {slot}, {prop.Name}.Length - 1);"
                                );
                            }
                            else
                            {
        ...
        private void WriteCloner()
        {
            ...
            foreach (var node in _tree.Types.OfType<Node>())
            {
                ...
                WriteLine(
                    $"{(node.IsInternal ? "internal" : "public")} override IOperation {GetVisitorName(node)}({node.Name} operation, object? argument)"
                );
                Brace();
                ...
                @"public abstract partial class OperationVisitor
    {
        ...
        internal virtual void VisitNoneOperation(IOperation operation) { /* no-op */ }"
            ...
            foreach (var type in types)
            {
                ...
                WriteLine(
                    $"{accessibility} virtual void {GetVisitorName(type)}({type.Name} operation) => DefaultVisit(operation);"
                );
            }
                ...
                @"public abstract partial class OperationVisitor<TArgument, TResult>
    {
        ...
        internal virtual TResult? VisitNoneOperation(IOperation operation, TArgument argument) => default(TResult);"
            ...
            foreach (var type in types)
            {
                ...
                var accessibility = type.IsInternal ? "internal" : "public";
                WriteLine(
                    $"{accessibility} virtual TResult? {GetVisitorName(type)}({type.Name} operation, TArgument argument) => DefaultVisit(operation, argument);"
                );
            }
        ...
        private bool IsIOperationType(string typeName)
        {
            ...
            return _typeMap.ContainsKey(GetTypeNameWithoutNullable(typeName))
                || (
                    IsImmutableArray(typeName, out var innerType)
                    && IsIOperationType(GetTypeNameWithoutNullable(innerType))
                );
        }

        private static string GetTypeNameWithoutNullable(string typeName) =>
            typeName[^1] == '?' ? typeName[..^1] : typeName;
