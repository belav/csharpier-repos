                ...
                try
                {
                    ...
                    using (incposdbEntities entity = new incposdbEntities())
                    {
                        ...
                        invListm = (
                            ...
                            from JAA in JA.DefaultIfEmpty()
                                //from JBCC in JBC.DefaultIfEmpty()
                            where (brIds.Contains(JBB.branchId)) && (I.invType == "s" || I.invType == "sb"
                            || I.invType == "ts" || I.invType == "ss") && (I.membershipId != null)
            ...
            try
            {
                ...
                using (incposdbEntities entity = new incposdbEntities())
                {
                    cachlist = (from C in entity.cashTransfer
                                ...
                                where (C.processType != "balance" && (C.side == "c" || C.side == "v" || C.side == "b" || C.side == "u" || C.side == "sh" || C.side == "shd" || C.side == "mb"))//( C.transType == "p" && C.side==Side)
           ...
           hasCommission = u.hasCommission,
            ...
            public List<CashTransferModel> getstate(List<CashTransferModel> list, string selectedtype, List<CashTransferModel> listAll)
            {
                ...
                list2 = temp.OrderBy(X => X.updateDate).GroupBy(obj => obj.transNum).Select(obj => new CashTransferModel
                {
                    ...
                    processType = (obj.FirstOrDefault().processType == "doc"
            ...
            private List<CashTransferModel> fillList(List<CashTransferModel> payments, string selectedtype, long? Id, int year)
            {
...
// يومية الصندوق
                        ...
                        var invListm = (from I in entity.invoices
                                        ...
                                        select new
                                        {
                                           ...
                                           (I.couponsInvoices.Where(X => X.InvoiceId == I.invoiceId).ToList().Sum(C => C.discountType == 2 ? (C.discountValue * I.total / 100) : C.discountValue))
                                            ...
                                            cachTransferList = entity.cashTransfer.Where(x => x.invId == I.invoiceId && x.processType != "inv").Select(x => new CashTransferModel
                                            {
                                                cashTransId = x.cashTransId,
                ...
                try
                {
                    ...
                    using (incposdbEntities entity = new incposdbEntities())
                    {
                        ...
                        List<ItemUnitInvoiceProfitModel> invListm = (from IT in entity.itemsTransfer
                                                                     ...
                                                                     from JAA in JA.DefaultIfEmpty()
                                                                     from JBCC in JBC.DefaultIfEmpty()
                                                                     where (brIds.Contains(JBCC.branchId)) && (I.invType == "s") && (I.updateDate >= startdt && I.updateDate < enddt)
                        ...
                        foreach (ItemUnitInvoiceProfitModel row in invListm)
                        {
                            ...
                            row.invoiceProfit = row.subTotalNet - (decimal)row.discountValue - row.purchasePrice + (decimal)row.shippingCost + (decimal)row.tax;

                        }

                        List<ItemUnitInvoiceProfitModel> cashtrList = (from C in entity.cashTransfer
                                                                       where C.transType == "p" && (C.side == "e" || C.side == "s" || C.side == "tax" || C.side == "sh") && (C.updateDate >= startdt && C.updateDate < enddt)
                                                                       select new ItemUnitInvoiceProfitModel
                                                                       {
                                                                           cashTransId = C.cashTransId,
                                                                           transType = C.transType,
                ...
                try
                {
                    ...
                    using (incposdbEntities entity = new incposdbEntities())
                    {
                        ...
                        var invListm = (from I in entity.invoices
                                        ...
                                        from JUPUS in JUPUSR.DefaultIfEmpty()
                                        from JBCC in JBC.DefaultIfEmpty()
                                        where (brIds.Contains(JBCC.branchId)) && (I.invType == "sr" || I.invType == "srb" || I.invType == "srw")
                ...
                try
                {
                    ...
                    using (incposdbEntities entity = new incposdbEntities())
                    {
                        ...
                        //searchPredicate = searchPredicate.And(x => x.shippingCompanyId != null);
                          ...
                          invoicesList = (from I in entity.invoices
                                            ...
                                            from y in lj.DefaultIfEmpty()
                                            from g in gj.DefaultIfEmpty()
                                            where (brIds.Contains((long)I.branchId) && I.shippingCompanyId != null
                                            && (I.invType == "ts" || I.invType == "ss"))&& (o.orderPreparingId == entity.orderPreparing.Where(x => x.invoiceId == I.invoiceId).Max(x => x.orderPreparingId))
                                          select new InvoiceModel()
                                            {
                                               ...
                                               // status = s.status,
                                                ...
                                                shippingCost = I.shippingCost,
                                                realShippingCost = I.realShippingCost,
                                                payStatus = I.deserved == 0 ? "payed" : (I.deserved == I.totalNet ? "unpayed" : "partpayed"),
                                                branchCreatorName = entity.branches.Where(X => X.branchId == I.branchCreatorId).FirstOrDefault().name,
                                                itemsCount = entity.itemsTransfer.Where(x => x.invoiceId == I.invoiceId).Count(),
                                                ...
                                                orderStatusList = entity.orderPreparingStatus.Where(X => X.orderPreparingId == o.orderPreparingId ).Select(X => new orderPreparingStatusModel
                                                {
                                                    status = X.status,
                                                    createDate = X.createDate,
                                                    updateDate = X.updateDate,
                            ...
                            if (invoicesList.Count() > 0)
                            {
                                foreach (InvoiceModel row in invoicesList.ToList())
                                {
                                    if (row.orderStatusList != null)
                                    {
                                        TimeSpan tmp = (TimeSpan)(row.orderStatusList.LastOrDefault().createDate - row.orderStatusList.FirstOrDefault().createDate);
                                        row.orderDuration = (decimal)tmp.TotalMinutes;
                                        row.status = row.orderStatusList.LastOrDefault() != null ? row.orderStatusList.LastOrDefault().status : "";
                                    }
                                }
                            }
                        }
        ...
        public string GetStatByInvId(string token)
        {
            ...
            else
            {
                ...
                try
                {
                    List<orderPreparingStatusModel> statusList = new List<orderPreparingStatusModel>();
                    using (incposdbEntities entity = new incposdbEntities())
                    {

                        statusList = (from  os in entity.orderPreparingStatus
                                 //   from I in entity.invoices.Where(x=>x.invoiceId== invoiceId)
                             join o in entity.orderPreparing on os.orderPreparingId equals o.orderPreparingId
