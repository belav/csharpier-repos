        ...
        )
        {
            ...
            Expression MapCollectionCore(Expression destExpression)
            {
                ...
                Expression destination,
                    assignNewExpression;
                UseDestinationValue();
                var addItems = ForEach(
                    itemParam,
                    sourceExpression,
                    Call(destination, addMethod, itemExpr)
                );
                ...
                var clearMethod = isIList
                    ? IListClear
                    : destinationCollectionType.GetMethod("Clear");
                var checkNull = Block(
                    new[] { newExpression, passedDestination },
                ...
                void UseDestinationValue()
                {
                    ...
                    else
                    {
                        ...
                        assignNewExpression = Assign(
                            newExpression,
                            Coalesce(
                                passedDestination,
                                ObjectFactory.GenerateConstructorExpression(passedDestination.Type)
                            )
                        );
                    }
                }
                ...
                {
                    var elementTypeMap = configurationProvider.ResolveTypeMap(
                        sourceElementType,
                        destinationElementType
                    );
