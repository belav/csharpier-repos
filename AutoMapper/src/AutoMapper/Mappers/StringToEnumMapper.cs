...
namespace AutoMapper.Internal.Mappers
{
    ...
    public class StringToEnumMapper : IObjectMapper
    {
        ...
        )
        {
            ...
            foreach (var memberInfo in destinationType.GetFields(TypeExtensions.StaticFlags))
            {
                ...
                if (attributeValue != null)
                {
                    var switchCase = SwitchCase(
                        ToType(
                            Constant(Enum.ToObject(destinationType, memberInfo.GetValue(null))),
                            destinationType
                        ),
            ...
            var enumParse = ToType(
                Call(ParseMethod, Constant(destinationType), sourceExpression, True),
                destinationType
            );
            var parse =
                switchCases != null
                    ? Switch(sourceExpression, enumParse, EqualsMethod, switchCases)
                    : enumParse;
            return Condition(
                Call(IsNullOrEmptyMethod, sourceExpression),
