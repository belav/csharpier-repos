...
public class StringToEnumMapper : IObjectMapper
{
    private static readonly MethodInfo EqualsMethod = typeof(StringToEnumMapper).GetMethod(
        nameof(StringCompareOrdinalIgnoreCase)
    );
    private static readonly MethodInfo ParseMethod = typeof(Enum).StaticGenericMethod(
        "Parse",
        parametersCount: 2
    );
    ...
    )
    {
        ...
        var ignoreCase = True;
        var enumParse = Call(
            ParseMethod.MakeGenericMethod(destinationType),
            sourceExpression,
            ignoreCase
        );
        var enumMember = CheckEnumMember(
            sourceExpression,
            destinationType,
            enumParse,
            EqualsMethod
        ...
        return Condition(
            Equal(Property(sourceExpression, Length), Zero),
            configuration.Default(destinationType),
            enumMember
        );
    }

    internal static Expression CheckEnumMember(
        Expression sourceExpression,
        Type enumType,
        Expression defaultExpression,
        MethodInfo comparison = null
    )
    {
        ...
        foreach (var memberInfo in enumType.GetFields(TypeExtensions.StaticFlags))
        {
            ...
            var (body, testValue) =
                comparison == null
                    ? (attributeConstant, enumToObject)
                    : (ToType(enumToObject, enumType), attributeConstant);
            switchCases ??= new();
            switchCases.Add(SwitchCase(body, testValue));
        }
        return switchCases == null
            ? defaultExpression
            : Switch(sourceExpression, defaultExpression, comparison, switchCases);
    }
