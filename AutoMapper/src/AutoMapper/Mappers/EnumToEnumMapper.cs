...
namespace AutoMapper.Internal.Mappers
{
    ...
    {
        private static readonly MethodInfo TryParseMethod = typeof(Enum)
            .StaticGenericMethod("TryParse", parametersCount: 3);
        ...
        )
        {
            ...
            var ignoreCase = True;
            var tryParse = Call(
                TryParseMethod.MakeGenericMethod(destinationType),
                sourceToString,
                ignoreCase,
                result
            );
            return Block(
                new[] { result },
                Condition(tryParse, result, Convert(sourceExpression, destinationType))
            );
        }
    }
}
