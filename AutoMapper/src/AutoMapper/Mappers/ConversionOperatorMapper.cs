...
namespace AutoMapper.Internal.Mappers
{
    ...
    public class ConversionOperatorMapper : IObjectMapper
    {
        ...
        {
            foreach (
                MethodInfo sourceMethod in sourceType.GetMember(
                    _operatorName,
                    MemberTypes.Method,
        ...
        {
            var conversionOperator = GetConversionOperator(
                sourceExpression.Type,
                destExpression.Type
            );
            return Expression.Call(
                conversionOperator,
                ToType(sourceExpression, conversionOperator.GetParameters()[0].ParameterType)
            );
        }
    }
}
