...
public class ProjectionBuilder : IProjectionBuilder
{
    ...
    )
    {
        ...
        return cachedExpressions.Prepare(
            _configuration.EnableNullPropagationForQueryMapping,
            parameters
        );
    }
    ...
    private QueryExpressions CreateProjection(ProjectionRequest request) =>
        CreateProjection(
            request,
            new FirstPassLetPropertyMaps(_configuration, MemberPath.Empty, new())
        );
    ...
    )
    {
        ...
        var typeMap =
            _configuration.ResolveTypeMap(request.SourceType, request.DestinationType)
    ...
    )
    {
            ...
            foreach (
                var propertyMap in typeMap.PropertyMaps
                    .Where(
                        pm =>
                            pm.CanResolveValue
                            && pm.DestinationMember.CanBeSet()
                            && !typeMap.ConstructorParameterMatches(pm.DestinationName)
                    )
        ...
        Expression TryProjectMember(
            MemberMap memberMap,
            bool? explicitExpansion = null,
            Expression defaultSource = null
        )
        {
            ...
            bool ShouldExpand() =>
                explicitExpansion != true || request.ShouldExpand(letPropertyMaps.GetCurrentPath());
            Expression ProjectMemberCore()
            {
                var memberTypeMap = _configuration.ResolveTypeMap(
                ...
                {
                    mappedExpression = CreateProjectionCore(
                        memberRequest,
                        resolvedSource,
                        memberTypeMap,
                        letPropertyMaps
                    ...
                    )
                    {
                        ...
                        mappedExpression = resolvedSource.IfNullElse(
                            Constant(null, mappedExpression.Type),
                            mappedExpression
                        );
                    }
                }
                else
                {
                    ...
                    mappedExpression = projectionMapper.Project(
                        _configuration,
                        memberRequest,
                        resolvedSource,
                        letPropertyMaps
    ...
    class FirstPassLetPropertyMaps : LetPropertyMaps
    {
        ...
        public override LetPropertyMaps New() =>
            new FirstPassLetPropertyMaps(Configuration, GetCurrentPath(), BuiltProjections);
        ...
        readonly record struct SubQueryPath(
        ...
        )
        {
            ...
            internal bool IsEquivalentTo(SubQueryPath other) =>
                ...
                && Members
                    .Take(Members.Length - 1)
                    .Zip(other.Members, (left, right) => left.MemberMap == right.MemberMap)
                    .All(item => item);
        }
...
[EditorBrowsable(EditorBrowsableState.Never)]
public readonly record struct QueryExpressions(
    LambdaExpression Projection,
    LambdaExpression LetClause = null
)
{
