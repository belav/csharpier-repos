...
public class TypeDetails
{
    ...
    public MemberInfo GetMember(string name)
    {
        ...
        if (
            _nameToMember.TryGetValue(name, out var member)
            && Config.MethodMappingEnabled
            && member is GenericMethod genericMethod
        )
        {
        ...
        IEnumerable<MemberInfo> GetNoArgExtensionMethods(
        ...
        )
        {
            ...
            var definitions = genericInterfaces
            ...
            return extensionMethods.Concat(
                ...
                let genericInterface = definitions.GetValueOrDefault(
    ...
    class GenericMethod : MemberInfo
    {
        ...
        public MethodInfo Close()
        {
            if (_closedMethod == ObjectToString)
            {
                ...
                try
                {
                    _closedMethod = _genericMethod.MakeGenericMethod(
                        _genericInterface.GenericTypeArguments
                    );
                }
    ...
    private IEnumerable<Type> GetTypeInheritance() =>
        Type.IsInterface ? Type.GetInterfaces().Prepend(Type) : Type.GetTypeInheritance();
