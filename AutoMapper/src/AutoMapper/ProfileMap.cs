...
public class ProfileMap
{
    ...
    )
    {
        ...
        ShouldMapProperty =
            profile.ShouldMapProperty ?? configuration?.ShouldMapProperty ?? (p => p.IsPublic());
        ShouldMapMethod =
        ...
        ValueTransformers = profile
            .ValueTransformers.Concat(configuration?.ValueTransformers)
            .ToArray();
        ...
        SourceExtensionMethods = profile
            .SourceExtensionMethods.Concat(globalProfile?.SourceExtensionMethods)
            .ToArray();
        AllPropertyMapActions = profile
            .AllPropertyMapActions.Concat(globalProfile?.AllPropertyMapActions)
            .ToArray();
        AllTypeMapActions = profile
            .AllTypeMapActions.Concat(globalProfile?.AllTypeMapActions)
            .ToArray();
    ...
    {
        foreach (
            var derivedMap in configuration
                .GetAllTypeMaps()
                .Where(tm =>
                    typeMap != tm
    ...
    private void Configure(TypeMap typeMap, IGlobalConfiguration configuration)
    {
        ...
        foreach (var action in AllPropertyMapActions)
        {
            foreach (var propertyMap in typeMap.PropertyMaps)
            {
                ...
                var memberExpression = new MemberConfigurationExpression(
    ...
    public TypeMap CreateClosedGenericTypeMap(
        TypeMapConfiguration openMapConfig,
    ...
    )
    {
        ...
        lock (configuration)
        {
            closedMap = new TypeMap(
                closedTypes.SourceType,
                closedTypes.DestinationType,
                this,
                openMapConfig
            );
        }
    ...
    private void ApplyMemberMaps(TypeMap currentMap, IGlobalConfiguration configuration)
    {
        ...
        {
            var includedMap = configuration.GetIncludedTypeMap(
