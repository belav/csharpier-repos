...
public static class ExpressionBuilder
{
    ...
    public static readonly ParameterExpression ExceptionParameter = Parameter(
        typeof(Exception),
        "ex"
    );
    public static readonly ParameterExpression ContextParameter = Parameter(
        typeof(ResolutionContext),
        "context"
    );
    public static readonly MethodInfo IListClear = typeof(IList).GetMethod(nameof(IList.Clear));
    static readonly MethodInfo ContextCreate = typeof(ResolutionContext).GetInstanceMethod(
        nameof(ResolutionContext.CreateInstance)
    );
    static readonly MethodInfo OverTypeDepthMethod = typeof(ResolutionContext).GetInstanceMethod(
    ...
    static readonly MethodInfo ContextMapMethod = typeof(ResolutionContext).GetInstanceMethod(
        nameof(ResolutionContext.MapInternal)
    );
    static readonly MethodInfo ArrayEmptyMethod = typeof(Array).GetStaticMethod(
        nameof(Array.Empty)
    );
    static readonly ParameterExpression Disposable = Variable(
        typeof(IDisposable),
        "disposableEnumerator"
    );
    static readonly ReadOnlyCollection<ParameterExpression> DisposableArray =
        Disposable.ToReadOnly();
    static readonly MethodInfo DisposeMethod = typeof(IDisposable).GetMethod(
        nameof(IDisposable.Dispose)
    );
    static readonly Expression DisposeCall = IfThen(
        ReferenceNotEqual(Disposable, Null),
        Expression.Call(Disposable, DisposeMethod)
    );
    ...
    public static (List<ParameterExpression> Variables, List<Expression> Expressions) Scratchpad(
        this IGlobalConfiguration configuration
    )
    {
    ...
    )
    {
        ...
        if (typeMap != null)
        {
            ...
            nullCheck =
                ...
                && (
                    destination.NodeType != ExpressionType.Default
        ...
        mapExpression =
            mapExpression == null
                ? ContextMap(typePair, source, destination, memberMap)
                : ToType(mapExpression, typePair.DestinationType);
        return nullCheck
            ? configuration.NullCheckSource(
                profileMap,
                source,
                destination,
                mapExpression,
    ...
    )
    {
        ...
        var ifSourceNull =
            ...
            : mustUseDestination ? ClearDestinationCollection()
            : DefaultDestination();
        ...
        Expression ClearDestinationCollection()
        {
            ...
            statements.Add(
                IfThen(
                    ReferenceNotEqual(collection, Null),
                    Expression.Call(collection, clearMethod)
                )
            );
        ...
        Expression DefaultDestination()
        {
            ...
            {
                return destination.NodeType == ExpressionType.Default
                    ? destination
    ...
    public static Expression ServiceLocator(Type type) =>
        Expression.Call(ContextParameter, ContextCreate, Constant(type));
    ...
    public static Expression CheckContext(TypeMap typeMap) =>
        typeMap.PreserveReferences || typeMap.MaxDepth > 0 ? CheckContextCall : null;
    ...
    public static Expression ApplyTransformers(
        this MemberMap memberMap,
        Expression source,
    ...
    )
    {
        ...
        {
            result = memberMap.ApplyTransformers(
                source,
                configuration,
                perMember.Concat(perMap).Concat(perProfile)
            );
        }
        return result;
    }

    static Expression ApplyTransformers(
        this MemberMap memberMap,
        Expression result,
        IGlobalConfiguration configuration,
        IEnumerable<ValueTransformerConfiguration> transformers
    )
    {
        foreach (var transformer in transformers)
        {
            ...
            {
                result = ToType(
                    configuration.ReplaceParameters(
                        transformer.TransformerExpression,
                        ToType(result, transformer.ValueType)
                    ),
    ...
    public static MemberInfo GetMember(this LambdaExpression lambda) =>
        (
            ...
            && memberExpression.Expression == lambda.Parameters[0]
        )
    ...
    public static Stack<Member> GetChain(this Expression expression)
    {
        ...
        while (expression != null)
        {
            var member = expression switch
            {
                ...
                } when extensionMethod.Has<ExtensionAttribute>() => new Member(
                    expression,
                    extensionMethod,
                    arguments[0]
                ),
    ...
    public static LambdaExpression MemberAccessLambda(
        Type type,
        string memberPath,
        TypeMap typeMap
    ...
    )
    {
        ...
        return Block(variables, statements);
        static Expression ForEachArrayItem(
            List<ParameterExpression> variables,
            List<Expression> statements,
            ParameterExpression loopVar,
            Expression array,
            ...
            statements.Add(
                Loop(
                    IfThenElse(
                        ...
                        Block(
                            Assign(loopVar, ArrayIndex(array, Index)),
                            loopContent,
                            IncrementIndex
                        ),
                        Break(breakLabel)
    ...
    public static MethodCallExpression Call(Expression target, string name) =>
        Expression.Call(target, target.Type.GetInheritedMethod(name));
    ...
    public static Expression Replace(this Expression exp, Expression old, Expression replace) =>
        new ReplaceVisitor().Replace(exp, old, replace);
    ...
    )
    {
        ...
        var returnType =
            (
                ...
                && Nullable.GetUnderlyingType(destinationType) == expression.Type
            )
                ? destinationType
                : expression.Type;
        var defaultReturn =
            (
                defaultValue is { NodeType: ExpressionType.Default }
                && defaultValue.Type == returnType
            )
                ? defaultValue
                : configuration.Default(returnType);
        ...
        static Expression UpdateTarget(Expression sourceExpression, Expression newTarget) =>
            sourceExpression switch
            {
                ...
                MethodCallExpression { Object: null, Arguments: var args } methodCall
                    when args[0] != newTarget => ExtensionMethod(
                    methodCall.Method,
                    newTarget,
                    args
                ),
                MethodCallExpression { Object: Expression target } methodCall
                    when target != newTarget => Expression.Call(
                    newTarget,
                    methodCall.Method,
        ...
        static MethodCallExpression ExtensionMethod(
            MethodInfo method,
