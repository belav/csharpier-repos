...
public static class ObjectFactory
{
    ...
    public static Expression GenerateConstructorExpression(
        Type type,
    ...
    private static Expression CallConstructor(Type type, IGlobalConfiguration configuration)
    {
        var defaultCtor = type.GetConstructor(
            Internal.TypeExtensions.InstanceFlags,
            null,
            Type.EmptyTypes,
            null
        );
        ...
        if (ctorWithOptionalArgs.args == null)
        {
            return InvalidType(
                type,
                $"{type} needs to have a constructor with 0 args or only optional args. Validate your configuration for details."
            );
        }
    ...
    {
        var ctor = typeof(ReadOnlyDictionary<,>).MakeGenericType(typeArguments).GetConstructors()[
            0
        ];
