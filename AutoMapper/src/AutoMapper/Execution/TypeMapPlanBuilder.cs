...
{
    static readonly MethodInfo MappingError = typeof(TypeMapPlanBuilder).GetStaticMethod(
        nameof(MemberMappingError)
    );
    static readonly MethodInfo IncTypeDepthInfo = typeof(ResolutionContext).GetInstanceMethod(
        nameof(ResolutionContext.IncrementTypeDepth)
    );
    static readonly MethodInfo DecTypeDepthInfo = typeof(ResolutionContext).GetInstanceMethod(
        nameof(ResolutionContext.DecrementTypeDepth)
    );
    static readonly MethodInfo CacheDestinationMethod = typeof(ResolutionContext).GetInstanceMethod(
        nameof(ResolutionContext.CacheDestination)
    );
    static readonly MethodInfo GetDestinationMethod = typeof(ResolutionContext).GetInstanceMethod(
    ...
    public TypeMapPlanBuilder(IGlobalConfiguration configuration, TypeMap typeMap)
    {
        ...
        _parameters =
            _configuration.Parameters ?? new ParameterExpression[] { null, null, ContextParameter };
    }
    ...
    ParameterExpression[] GetParameters(
        ParameterExpression first = null,
        ParameterExpression second = null
    )
    {
    ...
    public LambdaExpression CreateMapperLambda()
    {
        ...
        return Lambda(
            _configuration.NullCheckSource(
                _typeMap.Profile,
                _source,
                _initialDestination,
                mapperFunc,
        ...
        foreach (var includedMap in _typeMap.IncludedMembersTypeMaps)
        {
            ...
            _expressions.Add(
                Assign(
                    variable,
                    _configuration
                        .ReplaceParameters(includedMap.MemberExpression, _source)
                        .NullCheck(null)
                )
            );
        }
    ...
    )
    {
        ...
        foreach (var memberMap in MemberMaps())
        {
            ...
            if (
                memberMap.Inline
                && (
                    memberTypeMap.PreserveReferences
        ...
        IEnumerable<MemberMap> MemberMaps()
        {
            ...
            return typeMap.HasDerivedTypesToInclude
                ? memberMaps.Concat(
                    configuration.GetIncludedTypeMaps(typeMap).SelectMany(tm => tm.MemberMaps)
                )
                : memberMaps;
        }
        TypeMap ResolveMemberTypeMap(MemberMap memberMap)
        {
            ...
            return types.ContainsGenericParameters
                ? null
                : configuration.ResolveAssociatedTypeMap(types);
        }
        [Conditional("DEBUG")]
    ...
    Expression ReplaceParameters(LambdaExpression lambda) =>
        _configuration.ReplaceParameters(lambda, GetParameters());
    ...
    private void AddPropertyMaps(List<Expression> actions)
    {
        ...
        foreach (var propertyMap in propertyMaps)
        {
            ...
            {
                property = _initialDestination.IfNullElse(
                    _configuration.Default(property.Type),
                    property
                );
            }
        ...
        foreach (var member in destination.GetMemberExpressions())
        {
            ...
            var ifNull =
                setter == null
                    ? Throw(
                        Constant(
                            new NullReferenceException(
                                $"{member} cannot be null because it's used by ForPath."
                            )
                        ),
                        member.Type
                    )
                    : (Expression)Assign(
                        setter,
                        ObjectFactory.GenerateConstructorExpression(member.Type, _configuration)
                    );
    ...
    private Expression CreateMapperFunc(Expression assignmentFunc)
    {
        ...
        if (overMaxDepth != null)
        {
            mapperFunc = Condition(
                overMaxDepth,
                _configuration.Default(DestinationType),
                mapperFunc
            );
        }
    ...
    )
    {
        ...
        var valueResolver = BuildValueResolverFunc(
            memberMap,
            customSource,
            destinationMemberGetter
        );
        var resolvedValueVariable = Variable(valueResolver.Type, "resolvedValue");
        var destinationMemberValue = DestinationMemberValue(
            memberMap,
            destinationMemberGetter,
        ...
        if (memberMap.Condition != null)
        {
            _expressions.Add(
                IfThen(
                    _configuration.ConvertReplaceParameters(
                        memberMap.Condition,
                        new[]
                        {
                            customSource,
    ...
    )
    {
        ...
        return DestinationType.IsValueType
            ? defaultValue
            : Condition(
                ReferenceEqual(_initialDestination, Null),
                defaultValue,
                destinationMemberGetter
            );
    }
    ...
    {
        var preCondition = _configuration.ConvertReplaceParameters(
            memberMap.PreCondition,
            GetParameters(first: customSource)
        );
    ...
    ParameterExpression SetVariables(
    ...
    private Expression BuildValueResolverFunc(
        MemberMap memberMap,
        Expression customSource,
        ...
        else if (!memberMap.AllowsNullDestinationValues)
        {
            ...
            if (!toCreate.IsAbstract && toCreate.IsClass && !toCreate.IsArray)
            {
                ...
                valueResolverFunc = Coalesce(
                    valueResolverFunc,
                    ToType(ctor, valueResolverFunc.Type)
                );
            }
        }
...
public class MemberPathResolver : IValueResolver
{
    ...
    public MemberInfo GetSourceMember(MemberMap memberMap) =>
        _members.Length == 1 ? _members[0] : null;

    public LambdaExpression ProjectToExpression => _members.Lambda();

    public IValueResolver CloseGenerics(TypeMap typeMap) =>
        _members[0].DeclaringType.ContainsGenericParameters
            ? new MemberPathResolver(
                ReflectionHelper.GetMemberPath(
                    typeMap.SourceType,
                    Array.ConvertAll(_members, m => m.Name),
                    typeMap
                )
            )
            : this;
...
public class FuncResolver : LambdaValueResolver, IValueResolver
{
    ...
    public Expression GetExpression(
        ...
        configuration.ConvertReplaceParameters(
            Lambda,
            new[] { source, destination, destinationMember, ContextParameter }
        );
...
public class ExpressionResolver : LambdaValueResolver, IValueResolver
{
    ...
    )
    {
        ...
        return TryCatch(
            mapFrom,
            Catch(typeof(NullReferenceException), defaultExpression),
...
public class ValueConverter : ValueResolverConfig, IValueResolver
{
    ...
    )
    {
        ...
        return Call(
            ...
            InterfaceType.GetMethod("Convert"),
            ToType(sourceMember, sourceMemberType),
            ContextParameter
        );
...
public class ClassValueResolver : ValueResolverConfig, IValueResolver
{
    ...
    )
    {
        ...
        {
            sourceMember =
                SourceMemberName == null ? null : PropertyOrField(source, SourceMemberName);
        }
        else
        {
        ...
        var parameters = new[] { source, destination, sourceMember, destinationMember }
            .Where(p => p != null)
