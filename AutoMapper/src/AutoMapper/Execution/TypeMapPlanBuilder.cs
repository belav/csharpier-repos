...
public struct TypeMapPlanBuilder
{
        ...
        _expressions.AddRange(
            _variables.Zip(
                ...
                (v, i) =>
                    Assign(
                        v,
                        configuration.ReplaceParameters(i.MemberExpression, source).NullCheck(null)
                    )
            )
        );
    }
    ...
    private Expression TryPathMap(PathMap pathMap)
    {
        ...
        {
            return Block(
                destination.GetMemberExpressions().Select(NullCheck).Append(ExpressionBuilder.Empty)
            );
            Expression NullCheck(MemberExpression memberExpression)
            {
                ...
                return memberExpression.IfNullElse(
                    ifNull,
                    configuration.Default(memberExpression.Type)
                );
            }
    ...
    private Expression CreateMapperFunc(Expression assignmentFunc)
    {
        ...
        mapperFunc = _configuration.NullCheckSource(
            _typeMap.Profile,
            _source,
            _initialDestination,
            mapperFunc,
