...
public abstract class TypeMapConfiguration
{
    ...
    {
        foreach (
            var propertyMap in typeMap.PropertyMaps.Where(p =>
        ...
        IMappingExpressionBase<TSource, TDestination, TMappingExpression>
{
    ...
    public TMappingExpression BeforeMap(Action<TSource, TDestination> beforeFunction) =>
        BeforeMapCore((src, dest, ctxt) => beforeFunction(src, dest));

    private TMappingExpression BeforeMapCore(
    ...
    public TMappingExpression AfterMap(Action<TSource, TDestination> afterFunction) =>
        AfterMapCore((src, dest, ctxt) => afterFunction(src, dest));

    private TMappingExpression AfterMapCore(
        Expression<Action<TSource, TDestination, ResolutionContext>> expr
    )
    {
    ...
    public void ConvertUsing(Type typeConverterType)
    {
        ...
        TypeMapActions.Add(tm =>
            tm.TypeConverter = new ClassTypeConverter(typeConverterType, tm.Types.ITypeConverter())
        );
    }

    public void ConvertUsing(Func<TSource, TDestination, TDestination> mappingFunction) =>
        ConvertUsingCore((src, dest, ctxt) => mappingFunction(src, dest));

    private void ConvertUsingCore(
        Expression<Func<TSource, TDestination, ResolutionContext, TDestination>> expr
    ) => SetTypeConverter(new LambdaTypeConverter(expr));
    ...
    public void ConvertUsing<TTypeConverter>()
        ...
        SetTypeConverter(
            new ClassTypeConverter(
                typeof(TTypeConverter),
                typeof(ITypeConverter<TSource, TDestination>)
            )
        );
    ...
    public void ConvertUsing(Expression<Func<TSource, TDestination>> mappingFunction) =>
        SetTypeConverter(new ExpressionTypeConverter(mappingFunction));
