        ...
        IPropertyMapConfiguration
{
    ...
    public void MapFrom<TValueResolver>()
        ...
        MapFromCore(
            new(typeof(TValueResolver), typeof(IValueResolver<TSource, TDestination, TMember>))
        );

    protected void MapFromCore(ClassValueResolver config) => SetResolver(config);

    protected void SetResolver(IValueResolver config) =>
        PropertyMapActions.Add(pm => pm.SetResolver(config));
    ...
    private void MapFromResult<TResult>(
    ...
    public void Condition(Func<TSource, TDestination, bool> condition) =>
        ConditionCore((src, dest, srcMember, destMember, ctxt) => condition(src, dest));

    public void Condition(Func<TSource, bool> condition) =>
        ConditionCore((src, dest, srcMember, destMember, ctxt) => condition(src));

    private void ConditionCore(
    ...
    public void PreCondition(Func<TSource, bool> condition) =>
        PreConditionCore((src, dest, ctxt) => condition(src));

    public void PreCondition(Func<ResolutionContext, bool> condition) =>
        PreConditionCore((src, dest, ctxt) => condition(ctxt));

    public void PreCondition(Func<TSource, ResolutionContext, bool> condition) =>
        PreConditionCore((src, dest, ctxt) => condition(src, ctxt));

    public void PreCondition(Func<TSource, TDestination, ResolutionContext, bool> condition) =>
        PreConditionCore((src, dest, ctxt) => condition(src, dest, ctxt));

    private void PreConditionCore(
    ...
    public void AddTransform(Expression<Func<TMember, TMember>> transformer) =>
        PropertyMapActions.Add(pm =>
            pm.AddValueTransformation(
                new ValueTransformerConfiguration(pm.DestinationType, transformer)
            )
        );
    ...
    private void SetUseDestinationValue(bool value) =>
        PropertyMapActions.Add(pm => pm.UseDestinationValue = value);
    ...
    public void ConvertUsing<TSourceMember>(
        IValueConverter<TSourceMember, TMember> valueConverter
    ) => ConvertUsingCore(valueConverter);
    ...
    private void ConvertUsingCore<TValueConverter, TSourceMember>(
        Expression<Func<TSource, TSourceMember>> sourceMember = null,
        string sourceMemberName = null
    ) =>
        ConvertUsingCore(
            new(typeof(TValueConverter), typeof(IValueConverter<TSourceMember, TMember>))
            {
                SourceMemberLambda = sourceMember,
                SourceMemberName = sourceMemberName,
            }
    ...
    private void ConvertUsingCore<TSourceMember>(
        IValueConverter<TSourceMember, TMember> valueConverter,
        Expression<Func<TSource, TSourceMember>> sourceMember = null,
        string sourceMemberName = null
    ) =>
        ConvertUsingCore(
            new(valueConverter, typeof(IValueConverter<TSourceMember, TMember>))
            {
                SourceMemberLambda = sourceMember,
                SourceMemberName = sourceMemberName,
            }
    ...
    public void Configure(TypeMap typeMap)
    {
        ...
        var propertyMap = typeMap.FindOrCreatePropertyMapFor(
            destMember,
            typeof(TMember) == typeof(object) ? destMember.GetMemberType() : typeof(TMember)
        );
        Apply(propertyMap);
    }
        ...
        IMemberConfigurationExpression
{
    ...
    public void ConvertUsing(Type valueConverterType, string sourceMemberName) =>
        ConvertUsingCore(valueConverterType, sourceMemberName);

    public void ConvertUsing<TSourceMember, TDestinationMember>(
        IValueConverter<TSourceMember, TDestinationMember> valueConverter,
        string sourceMemberName
    ) =>
        base.ConvertUsingCore(
            new(valueConverter, typeof(IValueConverter<TSourceMember, TDestinationMember>))
            {
                SourceMemberName = sourceMemberName,
            }
        );
