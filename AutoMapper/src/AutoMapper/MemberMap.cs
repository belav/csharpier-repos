...
public class MemberMap : IValueResolver
{
    ...
    public virtual bool? ExplicitExpansion
    {
        get => default;
        set { }
    }
    ...
    public void MapFrom(string sourceMembersPath, MemberInfo[] members)
    {
        ...
        var sourceMembers = sourceType.ContainsGenericParameters
            ? members
            : members[0].DeclaringType.ContainsGenericParameters
                ? ReflectionHelper.GetMemberPath(sourceType, sourceMembersPath, TypeMap)
                : members;
    ...
    )
    {
        ...
        return IncludedMember == null && SourceMembers.Length < 2
            ? expression
            : expression.NullCheck(configuration, this, defaultValue);
    }

    public bool AllowsNullDestinationValues =>
        Profile?.AllowsNullDestinationValuesFor(this) ?? true;
    ...
    Expression IValueResolver.GetExpression(
        IGlobalConfiguration configuration,
        MemberMap memberMap,
        Expression source,
        Expression destination,
        Expression destinationMember
...
public readonly record struct ValueTransformerConfiguration(
    Type ValueType,
