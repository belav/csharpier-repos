...
public class GenericTypeConverterWithTwoArguments : AutoMapperSpecBase
{
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(c => c.CreateMap(typeof(List<>), typeof(List<>)).ConvertUsing(typeof(Converter<,>)));
...
public class GenericTypeConverter : AutoMapperSpecBase
{
            ...
            ITypeConverter<Source<object>, OtherDestination<T>>
    {
        ...
        public static OtherDestination<T> OpenDestinationViaClosedSource =
            new OtherDestination<T>();
        public static Destination<object> ClosedDestinationViaOpenSource =
            new Destination<object>();

        public Destination<T> Convert(
            Source<T> source,
            Destination<T> dest,
            ResolutionContext context
        )
        ...
        int ITypeConverter<Source<T>, int>.Convert(
            Source<T> source,
            int dest,
            ResolutionContext context
        )
        ...
        Destination<T> ITypeConverter<int, Destination<T>>.Convert(
            int source,
            Destination<T> dest,
            ResolutionContext context
        )
    ...
    {
        public static IReadOnlyDictionary<T1, T2> ReadOnlyDictionaryDestination =
            new Dictionary<T1, T2>();

        public IReadOnlyDictionary<T1, T2> Convert(
            Hashtable source,
    ...
    protected override void Because_of()
    {
        ...
        _openGenericToClosedGenericDestination = Mapper.Map<Destination<object>>(
            new OtherSource<int>()
        );
        _closedGenericToOpenGenericDestination = Mapper.Map<OtherDestination<int>>(
            new Source<object>()
        );
    }
    ...
    public void Should_use_generic_converter_with_correct_interface()
    {
        ...
        _openGenericToClosedGenericDestination.ShouldBe(
            Converter<int>.ClosedDestinationViaOpenSource
        );
        _closedGenericToOpenGenericDestination.ShouldBe(
            Converter<int>.OpenDestinationViaClosedSource
        );
    }
    ...
    {
        Mapper
            .Map<IReadOnlyDictionary<int, int>>(new Hashtable())
            .ShouldBeSameAs(Converter<int, int>.ReadOnlyDictionaryDestination);
    }
    ...
    {
        Mapper
            .Map<Destination<int>>(new Source<int>())
            .ShouldBeSameAs(Converter<int>.SomeDestination);
        Mapper
            .Map<Destination<long>>(new Source<long>())
            .ShouldBeSameAs(Converter<long>.SomeDestination);
    }
}
