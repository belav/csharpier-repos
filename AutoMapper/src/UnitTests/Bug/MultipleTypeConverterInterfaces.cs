...
namespace AutoMapper.UnitTests.Bug
{
        ...
        : AutoMapperSpecBase
    {
        ...
        protected override MapperConfiguration Configuration { get; } =
            ...
            {
                cfg.CreateMap(typeof(SourceFoo), typeof(DestinationFoo))
                    .ConvertUsing(typeof(DualConverter));
                cfg.CreateMap(typeof(SourceBar), typeof(DestinationBar))
                    .ConvertUsing(typeof(DualConverter));
            });

        protected override void Because_of()
        {
            _resultFoo = Mapper.Map<SourceFoo, DestinationFoo>(
                new SourceFoo { SourceFooValue = 5 }
            );
            _resultBar = Mapper.Map<SourceBar, DestinationBar>(
                new SourceBar { SourceBarValue = 6 }
            );
        }
