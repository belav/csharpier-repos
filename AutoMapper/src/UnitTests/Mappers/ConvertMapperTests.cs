...
namespace AutoMapper.UnitTests.Mappers
{
    public class ConvertMapperTests : AutoMapperSpecBase
    {
        ...
        public void From_string()
        {
            ...
            ConfigProvider.FindMapper(new TypePair(typeof(string), typeof(DateTime)))
        ...
        {
            ConfigProvider.FindMapper(new TypePair(typeof(string), typeof(Guid)))
                .ShouldBeOfType<ParseStringMapper>();
            var guid = Guid.NewGuid();
            Mapper.Map<Guid>(guid.ToString()).ShouldBe(guid);
            ConfigProvider.FindMapper(new TypePair(typeof(string), typeof(TimeSpan)))
                .ShouldBeOfType<ParseStringMapper>();
            var timeSpan = TimeSpan.FromMinutes(1);
            Mapper.Map<TimeSpan>(timeSpan.ToString()).ShouldBe(timeSpan);
            ConfigProvider.FindMapper(new TypePair(typeof(string), typeof(DateTimeOffset)))
                .ShouldBeOfType<ParseStringMapper>();
