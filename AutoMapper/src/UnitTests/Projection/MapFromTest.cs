...
public class When_mapping_from_chained_properties : AutoMapperSpecBase
{
    ...
    class InnerModel
    {
        public InnerModel(string value) =>
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(c => c.CreateMap<Model, Dto>().ForMember(d => d.Value, o => o.MapFrom("Inner.Value")));

    [Fact]
    public void Should_map_ok() =>
        Map<Dto>(new Model { Inner = new InnerModel("mappedFrom") }).Value.ShouldBe("mappedFrom");
}
...
public class When_mapping_from_private_method : AutoMapperSpecBase
{
    ...
    {
        public InnerModel(string value) =>
            SomeValue = value ?? throw new ArgumentNullException(nameof(value));
