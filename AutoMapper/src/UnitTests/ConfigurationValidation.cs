...
public class When_skipping_validation : NonValidatingSpecBase
{
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(cfg => cfg.CreateMap<Source, Dest>(MemberList.None));
...
public class When_constructor_does_not_match : NonValidatingSpecBase
{
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(cfg => cfg.CreateMap<Source, Dest>());
...
public class When_constructor_does_not_match_ForCtorParam : AutoMapperSpecBase
{
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(cfg => cfg.CreateMap<Source, Dest>().ForCtorParam("value", o => o.MapFrom(s => 4)));
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(cfg =>
        {
            cfg.CreateMap<Source, Destination>(MemberList.Source)
                ...
                .ForMember(
                    d => d.TypedResolvedDest,
                    o => o.MapFrom<MemberResolver, int>(s => s.TypedResolved)
                )
                .ForMember(
                    d => d.ConvertedDest,
                    o => o.ConvertUsing<ValueConverter, int>("Converted")
                )
                .ForMember(
                    d => d.TypedConvertedDest,
                    o => o.ConvertUsing<ValueConverter, int>(s => s.TypedConverted)
                );
        });
    ...
    : AutoMapperSpecBase
{
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(cfg =>
        {
            cfg.CreateMap<ModelObject, ModelDto>().ForMember(dest => dest.Bar, opt => opt.Ignore());
        });
