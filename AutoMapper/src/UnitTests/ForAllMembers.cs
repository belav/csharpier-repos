...
public class ForAllPropertyMaps_ConvertUsing : AutoMapperSpecBase
{
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(cfg =>
        {
            ...
            cfg.Internal()
                .ForAllPropertyMaps(
                    pm => pm.SourceType != null,
                    (tm, mapper) => mapper.ConvertUsing(new EnumToArray())
                );
        });

    [Fact]
    public void ShouldWork() =>
        Map<PostPutWellViewModel>(new Well()).SpecialTags.Single().ShouldBe(SpecialTags.SendState);
}
