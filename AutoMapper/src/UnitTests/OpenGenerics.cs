...
public class ForPathGenericsSource : AutoMapperSpecBase
{
    ...
    public void Should_work() =>
        Map<Destination>(new Source<int> { Inner = new() { Id = 42 } }).InnerId.ShouldBe(42);
}
...
public class ForPathGenerics : AutoMapperSpecBase
{
    ...
    [Fact]
    public void Should_work() =>
        Map<Destination<int>>(new Source<int> { Inner = new() { Id = 42 } }).InnerId.ShouldBe(42);
}
...
public class ReadonlyPropertiesGenerics : AutoMapperSpecBase
{
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(c =>
        {
            c.CreateMap(typeof(Source), typeof(Destination<>))
                .ForMember("Inner", o => o.MapFrom("Inner"));
            c.CreateMap<InnerSource, InnerDestination>();
        });

    [Fact]
    public void Should_work() =>
        Map<Destination<int>>(new Source { Inner = new() { Value = 42 } }).Inner.Value.ShouldBe(42);
}
...
public class ConstructorValidationGenerics : NonValidatingSpecBase
{
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(c => c.CreateMap(typeof(Source<>), typeof(Destination<>)));
...
public class OpenGenerics_With_Struct : AutoMapperSpecBase
{
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(
            mapper => mapper.CreateMap(typeof(Id<>), typeof(long)).ConvertUsing((_, __) => (long)42)
        );
...
public class GenericMapWithUntypedMap : AutoMapperSpecBase
{
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(cfg => cfg.CreateMap(typeof(Source<>), typeof(Destination<>)));
...
public class GenericValueResolver : AutoMapperSpecBase
{
        ...
        >
    {
        public string Resolve(
            ...
            Destination<TKeyDestination, string> destination,
            string destMember,
            ResolutionContext context
        ) => source.Key.ToString();
    }
        ...
        >
    {
        public string Resolve(
            KeyValuePair<TKeySource, TValueSource> source,
            Destination<TKeyDestination, TValueDestination> destination,
            string destMember,
            ResolutionContext context
        ) => source.Value.ToString();
    }
    ...
    {
        public IEnumerable<TDestination> Resolve(
            Source source,
            Destination<TDestination> destination,
            IEnumerable<TDestination> destMember,
            ResolutionContext context
        )
        {
    ...
    public void Should_map_generic_destination()
    {
        ...
        destination.MyKey.ShouldBe("1");
        destination.MyValue.ShouldBe("2");
        var destinationString = Map<Destination<string, string>>(
...
public class GenericMemberValueResolver : AutoMapperSpecBase
{
        ...
        >
    {
        public string Resolve(
            KeyValuePair<TKey, TValue> source,
            Destination<TDestinatonKey, string> destination,
            int sourceMember,
            string destMember,
            ResolutionContext context
        ) => sourceMember.ToString();
        ...
        >
    {
        public string Resolve(
            KeyValuePair<TKey, TValue> source,
            Destination<TDestinatonKey, TDestinatonValue> destination,
            int sourceMember,
            string destMember,
            ResolutionContext context
        ) => sourceMember.ToString();
...
public class RecursiveOpenGenerics : AutoMapperSpecBase
{
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(cfg => cfg.CreateMap(typeof(SourceTree<>), typeof(DestinationTree<>)));
...
public class OpenGenericsValidation : NonValidatingSpecBase
{
    ...
    protected override MapperConfiguration CreateConfiguration() =>
        new(cfg => cfg.CreateMap(typeof(Source<>), typeof(Dest<>)));
