    ...
    public static int RunTests(Type type, string[] args, TestDriverReporter reporter)
    {
        ...
        for (int iter = 0; iter < iterations; ++iter)
        {
            ...
            if (!quiet)
            {
                ...
                if (nskipped > 0)
                    Console.WriteLine(
                        "Regression tests: {0} ran, {1} skipped, {2} failed in {3}",
                        ran,
                        nskipped,
                        failed,
                        type
                    ...
                    Console.WriteLine(
                        "Regression tests: {0} ran, {1} failed in {2}",
                        ran,
                        failed,
                        type
                    );
...
public class TestTimeout
{
    ...
    {
        get { return DateTime.UtcNow - StartTime < Timeout; }
    }

    public static bool IsStressTest
    {
        get { return Environment.GetEnvironmentVariable("MONO_TESTS_STRESS") == "1"; }
    }
