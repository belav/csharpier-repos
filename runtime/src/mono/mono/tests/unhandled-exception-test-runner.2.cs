...
namespace UnhandledExceptionTest
{
    class RunningMode
    {
        ...
        {
            get { return (monoRuntime != null) && (monoRuntime != ""); }
        }
        private bool generateTestConfigurations;
        public bool GTC
        {
            get { return generateTestConfigurations; }
        }
    ...
    class TestDescription
    {
        ...
        public bool USE20
        {
            get { return use20Runtime; }
        }
        private bool rootConfigurationIsLegacy;
        public bool RCIL
        {
            get { return rootConfigurationIsLegacy; }
        }

        private bool exitCodeShouldBeZero;
        public bool EXITZERO
        {
            get { return exitCodeShouldBeZero; }
        }
        private bool continueMarkerExpected;
        public bool CONT
        {
            get { return continueMarkerExpected; }
        }
        private bool rootDomainUnhandledExceptionMarkerExpected;
        public bool RDUE
        {
            get { return rootDomainUnhandledExceptionMarkerExpected; }
        }
        private bool differentDomainUnhandledExceptionMarkerExpected;
        public bool DDUE
        {
            get { return differentDomainUnhandledExceptionMarkerExpected; }
        }
        ...
        public static TestDescription FromCode(int code)
        {
            if ((code >= MIN_CONFIG_CODE) && (code <= MAX_CONFIG_CODE))
            {
                ...
                {
                    string testConfiguration = String.Format(
                        "USE20:{0},RCIL:{1}",
                        BoolToString(testUSE20),
                        BoolToString(testRCIL)
                    );
                    string testArguments = String.Format(
                        "DT:{0},DA:{1},DCIL:{2},DTDA:{3},HRA:{4},HDA:{5}",
                        BoolToString(testDT),
                        BoolToString(testDA),
                        BoolToString(testDCIL),
                        BoolToString(testDTDA),
                        BoolToString(testHRA),
                        BoolToString(testHDA)
                    );
                ...
                if (components.Length == 2)
                {
                    switch (components[0])
                    {
                        ...
                        case "CONT":
                            ...
                            break;
                        case "RDUE":
                            rootDomainUnhandledExceptionMarkerExpected = ParseArgumentValue(
                                components[1]
                            );
                            break;
                        case "DDUE":
                            differentDomainUnhandledExceptionMarkerExpected = ParseArgumentValue(
                                components[1]
                            );
                            break;
                        default:
    ...
    class TestRun
    {
        ...
        public void Print()
        {
            ...
            Console.WriteLine(
                ...
                description.Configuration,
                description.Arguments,
                BoolToString(result.EXITZERO),
                BoolToString(result.CONT),
                BoolToString(result.RDUE),
                BoolToString(result.DDUE)
            );
        }
    ...
    class UnhandledExceptionTester
    {
        ...
        public static int Main(string[] args)
        {
            RunningMode runningMode =
                (args.Length > 0)
                    ? new RunningMode(args[0])
                    : new RunningMode("RUNTIME:mono,GTC:F");
            if (args.Length > 1)
            {
