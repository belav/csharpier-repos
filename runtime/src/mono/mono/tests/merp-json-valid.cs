...
class C
{
    public static void JsonValidateState()
    {
    ...
    public static void JsonValidateMerp(string configDir)
    {
        var monoType = Type.GetType("Mono.Runtime", false);
        ...
        var add_method = monoType.GetMethod(
            "AnnotateMicrosoftTelemetry",
            BindingFlags.NonPublic | BindingFlags.Static
        );
        ...
        {
            var send = monoType.GetMethod(
                "SendExceptionToTelemetry",
                BindingFlags.NonPublic | BindingFlags.Static
            );
            ...
            {
                if (
                    exc2.InnerException != null
                    && exc2.InnerException.Message
                        == "We were unable to start the Microsoft Error Reporting client."
                )
                    caught_expected_exception = true;
                else
                    throw new Exception(
                        String.Format(
                            "Got exception from Merp icall with wrong message {0}",
                            exc2.InnerException != null ? exc2.InnerException.Message : exc2.Message
                        )
                    );
            }
        }
