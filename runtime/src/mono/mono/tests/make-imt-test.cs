...
public class Tests
{
    public static void Main(String[] args)
    {
        ...
        {
            Console.WriteLine(
                "    static Iface_" + iface + " var_" + iface + " = new Impl_" + iface + " ();"
            );
            ...
            for (int i = 0; i < iface; ++i)
            {
                Console.WriteLine(
                    String.Format(
                        "        if ((r = var_{0}.Method_{1} (10,5,3,2)) != {1}) {{",
                        iface,
                        i
                    )
                );
                Console.WriteLine(
                    String.Format(
                        "     Console.WriteLine(\"iface {0} method {1} returned {{0}}\", r);",
                        iface,
                        i
                    )
                );
