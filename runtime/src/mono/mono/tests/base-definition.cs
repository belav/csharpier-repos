...
abstract class test
{
    public static int Main()
    {
        ...
        if (bm == null || bm.DeclaringType != typeof(TestClass) || bm.Name != "get_name")
            return 1;

        m = typeof(SubTestClass).GetMethod("get_name2");
        bm = m.GetBaseDefinition();
        if (bm == null || bm.DeclaringType != typeof(TestClass) || bm.Name != "get_name2")
            return 2;

        m = typeof(SubTestClass).GetMethod("get_name3");
        bm = m.GetBaseDefinition();
        if (bm == null || bm.DeclaringType != typeof(BaseTestClass) || bm.Name != "get_name3")
            return 3;
...
{
    public abstract string name3 { get; }
}
