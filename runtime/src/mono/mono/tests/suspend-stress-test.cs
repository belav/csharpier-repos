...
/*
TODO
Some tests are much more expensive than others (on cycles and synchronization), add a calibration
step to figure out duration.
Some tests are more disrruptive than others, add weights to tests so some are picked more
frequently.
...
Add an external watchdog so we can run the stress test in a loop for very long times and get results
out of it.
...
class Driver
{
    ...
    static int Main(string[] args)
    {
        ...
        {
            Console.WriteLine(
                "thread count {0} duration {1} test {2}",
                threadCount,
                timeInMillis,
                testName
            );
