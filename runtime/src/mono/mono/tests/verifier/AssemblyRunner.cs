...
namespace AssemblyRunner
{
    ...
    public class Runner
    {
        ...
        static AppDomain NewDomain()
        {
            ...
            PolicyStatement statement1 = new PolicyStatement(
                ps,
            ...
            group.AddChild(
                new UnionCodeGroup(new ZoneMembershipCondition(SecurityZone.MyComputer), statement1)
            );
        ...
        static RunResult testWithAppDomain(String path, String assemblyName)
        {
            ...
            catch (TypeInitializationException ve)
            {
                if (ve.InnerException is VerificationException)
                    return RunResult.unverifiable;
                Console.WriteLine(
                    "Warning: test {0} thrown exception: {1} ",
                    assemblyName,
                    ve.InnerException
                );
                return RunResult.invalid;
        ...
        static RunResult decide(RunResult ad, RunResult rt, RunResult pv, String testName)
        {
            if (ad == RunResult.valid)
            {
                ...
                {
                    Console.WriteLine(
                        "Warning: test {0} returned valid under AD but {1} under runtime. PV said {2}, using runtime choice",
                        testName,
                        rt,
                        pv
                    );
            ...
            if (ad == RunResult.unverifiable)
            {
                ...
                if (pv == RunResult.valid)
                    Console.WriteLine(
                        "Warning: test {0} returned unverifiable under AD but {1} under PV, using AD choice",
                        testName,
                        pv
                    );
            ...
            if (ad == RunResult.invalid)
            {
                ...
                if (pv == RunResult.valid)
                    Console.WriteLine(
                        "Warning: test {0} returned invalid under AD but {1} under PV, using AD choice",
                        testName,
                        pv
                    );
            ...
            Console.WriteLine(
                "ERROR: test {0} returned an unknown result under ad {1}, runtime said {2} and PV {3} -- FIXME --",
                testName,
                ad,
                rt,
                pv
        ...
        static void executeTest(String path, String assemblyName, RunResult expected)
        {
            ...
            if (veredict != expected)
                Console.WriteLine(
                    "ERROR: test {0} expected {1} but got {2} AD {3} RT {4} PV {5}",
                    assemblyName,
                    expected,
                    veredict,
                    ad,
