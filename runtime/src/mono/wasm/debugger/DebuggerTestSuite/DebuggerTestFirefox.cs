...
public class DebuggerTestFirefox : DebuggerTestBase
{
    ...
    public DebuggerTestFirefox(ITestOutputHelper testOutput, string driver = "debugger-driver.html")
        : base(testOutput, driver)
    {
        if (insp.Client is not FirefoxInspectorClient)
            throw new Exception(
                $"Bug: client should be {nameof(FirefoxInspectorClient)} for use with {nameof(DebuggerTestFirefox)}"
            );
    ...
    public override async Task InitializeAsync()
    {
        ...
        {
            Func<string, JObject, (string, Task<Result>)> getInitCmdFn = (cmd, args) =>
    ...
    internal override async Task<Result> SetBreakpoint(
        string url_key,
        int line,
        int column,
        bool expect_ok = true,
    ...
    internal override async Task<JObject> EvaluateAndCheck(
        ...
        Func<JToken, Task> locals_fn = null
    )
    {
    ...
    internal override void CheckLocation(
        string script_loc,
        int line,
        int column,
    ...
    private JObject ConvertFirefoxToDefaultFormat(JArray frames, JObject wait_res)
    {
        ...
        {
            var callFrame = JObject.FromObject(
                new
                {
        ...
        return JObject.FromObject(new { callFrames, reason = "other" });
    }

    internal override async Task<JObject> SendCommandAndCheck(
        JObject args,
        string method,
        string script_loc,
        int line,
    ...
    )
    {
        ...
        if (function_name != null)
        {
            AssertEqual(
                ...
                wait_res["callFrames"]?[0]?["functionName"]?.ToString()
            );
        }
    ...
    internal JObject ConvertFromFirefoxToDefaultFormat(KeyValuePair<string, JToken> variable)
    {
        ...
        if (
            value?["type"] == null
            || value["type"].Value<string>() == "object"
            || value["type"].Value<string>() == "string"
        )
        {
                ...
                default:
                    variableValue = JObject.FromObject(
                        new
                        {
                            type = type,
                            value = (string)null,
                            description = value["value"]?["value"]?.Value<string>() == null
                                ? value["value"]["class"].Value<string>()
        ...
        var ret = JObject.FromObject(
            new { name, writable = value["writable"] != null ? value["writable"] : false }
        );
    ...
    )
    {
        ...
        if (
            id.StartsWith("dotnet:evaluationResult:")
            || id.StartsWith("dotnet:valuetype:")
            || id.StartsWith("dotnet:object:")
            || id.StartsWith("dotnet:array:")
            || id.StartsWith("dotnet:pointer:")
        )
        {
            JArray ret = new();
            var o = JObject.FromObject(new { to = id, type = "enumProperties" });
            ...
            foreach (
                var prop in objProps.Value["result"]["value"]["ownProperties"].Value<JObject>()
            )
            {
    ...
    internal override async Task<JObject> StepAndCheck(
        StepKind kind,
        string script_loc,
        int line,
        int column,
    ...
    internal override async Task<Result> SetBreakpointInMethod(
        string assembly,
        string type,
        string method,
        int lineOffset = 0,
    ...
    )
    {
        var req = JObject.FromObject(
            new
            {
                ...
                type = "DotnetDebugger.getMethodLocation",
                typeName = type,
                methodName = method,
        ...
        var arr = new JArray(
            JObject.FromObject(new { lineNumber = m_line + lineOffset, columnNumber = -1 })
        );
    ...
    internal override async Task<(JToken, Result)> EvaluateOnCallFrame(
        string id,
        string expression,
        bool expect_ok = true
    )
    ...
    internal override async Task CheckDateTimeGetter(
        JToken value,
        DateTime expected,
        string label = ""
    ...
    internal override JObject CreateEvaluateArgs(string expression)
    {
        if (string.IsNullOrEmpty(_client.ConsoleActorId))
            throw new Exception(
                $"Cannot create evaluate request because consoleActorId is '{_client.ConsoleActorId}"
            );
    ...
    internal override async Task<JObject> WaitFor(string what)
    {
        ...
        var frames = await cli.SendCommand(
            "frames",
            JObject.FromObject(
                new
                {
        ...
        if (frames.Value["result"]?["value"]?["frames"] is not JArray frames_arr)
            throw new Exception(
                $"Tried to get frames after waiting for '{what}', but got unexpected result: {frames}"
            );
