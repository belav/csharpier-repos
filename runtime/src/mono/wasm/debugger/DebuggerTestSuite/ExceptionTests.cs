...
namespace DebuggerTests
{
    public class ExceptionTests : DebuggerTests
    {
        ...
        public async Task ExceptionTestAll()
        {
            ...
            AssertEqual(
                "DebuggerTests.ExceptionTestsClass.TestCaughtException.run",
            ...
            AssertEqual(
                "DebuggerTests.ExceptionTestsClass.TestUncaughtException.run",
                pause_location["callFrames"]?[0]?["functionName"]?.Value<string>(),
                "pause1"
            );
                    ...
                    if (pause_location != null)
                    {
                        ...
                        )
                        {
                            _testOutput.WriteLine(
                                $"Hit an unexpected managed exception, with function name: {actual_fn_name}. {pause_location}"
                            );
                            throw new XunitException(
                                $"Hit an unexpected managed exception, with function name: {actual_fn_name}"
                            );
                        }
        ...
        public async Task ExceptionTestUncaughtWithReload()
        {
            ...
            await SendCommand("Page.reload", JObject.FromObject(new { ignoreCache = true }));
        ...
        [ConditionalTheory(nameof(RunningOnChrome))]
        [InlineData(
            "[debugger-test] DebuggerTests.ExceptionTestsClassDefault:TestExceptions",
            "System.Exception",
            76,
            "DebuggerTests.ExceptionTestsClassDefault"
        )]
        [InlineData(
            "[debugger-test] DebuggerTests.ExceptionTestsClass:TestExceptions",
            "DebuggerTests.CustomException",
            28,
            "DebuggerTests.ExceptionTestsClass"
        )]
        public async Task ExceptionTestAllWithReload(
            string entry_method_name,
            string class_name,
            int line_number,
            string class_name_pause
        )
        {
            var debugger_test_loc = "dotnet://debugger-test.dll/debugger-exception-test.cs";
            ...
            await SendCommand("Page.enable", null);
            var pause_location = await SendCommandAndCheck(
                JObject.FromObject(new { ignoreCache = true }),
                "Page.reload",
                null,
                0,
                0,
            ...
            while (true)
            {
                ...
                try
                {
                    await insp.WaitFor(Inspector.PAUSE).WaitAsync(TimeSpan.FromSeconds(10));
                }
            ...
            AssertEqual(
                $"{class_name_pause}.TestCaughtException.run",
                pause_location["callFrames"]?[0]?["functionName"]?.Value<string>(),
                "pause0"
            );
            ...
            await CheckValue(
                res.Value["result"],
                JObject.FromObject(new { type = "string", value = $"not implemented caught" }),
                "exception0.message"
            );

            pause_location = await WaitForManagedException(null);
            AssertEqual(
                $"{class_name_pause}.TestUncaughtException.run",
                pause_location["callFrames"]?[0]?["functionName"]?.Value<string>(),
                "pause1"
            );
            ...
            await CheckValue(
                res.Value["result"],
                JObject.FromObject(new { type = "string", value = $"not implemented uncaught" }),
                "exception1.message"
            );
        }
