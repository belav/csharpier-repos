...
namespace DebuggerTests
{
    ...
    {
        public ArrayTests(ITestOutputHelper testOutput)
            : base(testOutput) { }
        ...
        [InlineData(
            213,
            8,
            "DebuggerTests.ArrayTestsClass.GenericValueTypeLocals2",
            false,
            0,
            false
        )]
        ...
        )
        {
            ...
            if (array_elem_props?.Length > 0)
            {
                for (int i = 0; i < array_elem_props.Length; i++)
                {
                    ...
                    {
                        var act_i = prefix_arr.FirstOrDefault(jt =>
                            jt["name"]?.Value<string>() == i_str
                        );
                        Assert.True(
                            act_i != null,
        ...
        public async Task InspectObjectArrayMembers(bool use_cfo)
        {
            ...
            await CheckProps(
                ...
                {
                    id = TString("c#id"),
                    ClassArrayProperty = TArray(
                        "DebuggerTests.SimpleClass[]",
                        "DebuggerTests.SimpleClass[3]"
                    ),
                    ClassArrayField = TArray(
                        "DebuggerTests.SimpleClass[]",
                        "DebuggerTests.SimpleClass[3]"
                    ),
            ...
            await CheckProps(
                ...
                new
                {
                    ...
                    gvclass_arr = TArray(
                        ...
                        "DebuggerTests.SimpleGenericStruct<DebuggerTests.Point>[2]"
                    ),
                    gvclass_arr_empty = TArray(
                        "DebuggerTests.SimpleGenericStruct<DebuggerTests.Point>[]",
                        "DebuggerTests.SimpleGenericStruct<DebuggerTests.Point>[0]"
                    ),
        ...
        public async Task InspectValueTypeArrayLocalsInstanceAsync(bool use_cfo)
        {
            ...
            var pause_location = await EvaluateAndCheck(
                ...
                line,
                col,
                "DebuggerTests.ArrayTestsClass.InstanceMethodValueTypeLocalsAsync<SimpleGenericStruct<Point>>"
            );
        ...
        public async Task InspectValueTypeArrayLocalsInAsyncStaticStructMethod(bool use_cfo)
        {
            ...
            var pause_location = await EvaluateAndCheck(
                ...
                line,
                col,
                "DebuggerTests.Point.AsyncMethod"
            );
        ...
        public async Task InvalidArrayId() =>
            await CheckInspectLocalsAtBreakpointSite(
                ...
                "PlaceholderMethod",
                1,
                "DebuggerTests.Container.PlaceholderMethod",
                ...
                wait_for_event_fn: async (pause_location) =>
                {
                    ...
                    if (!DotnetObjectId.TryParse(c_obj_id, out var id) || id.Scheme != "object")
                        Assert.True(
                            false,
                            "Unexpected object id format. Maybe this test is out of sync with the object id format in dotnet.es6.lib.js?"
                        );
        ...
        public async Task InvalidAccessors() =>
            await CheckInspectLocalsAtBreakpointSite(
                ...
                locals_fn: async (locals) =>
                {
                    ...
                    var pf_arr0_props = await GetProperties(
                    ...
                    foreach (var invalid_accessor in invalid_accessors)
                    {
                        ...
                        AssertEqual(
                            ...
                            "Expected to get undefined result for non-existent accessor"
                        );
                    }
                }
            );
        ...
        public async Task InspectPrimitiveTypeMultiArrayLocals(bool use_cfo)
        {
            ...
            var pause_location = await EvaluateAndCheck(
                ...
                343,
                12,
                "DebuggerTests.MultiDimensionalArray.run"
            );
        ...
        public async Task InspectInlineArray(string varName)
        {
            ...
            Assert.Equal(
                $"Unable to evaluate element access '{varName}[43]': Index is outside the bounds of the inline array",
        ...
        public async Task InspectInlineArray2()
        {
            ...
            var pause_location = await EvaluateAndCheck(
                ...
                460,
                12,
                "DebuggerTests.InlineArray.run2"
            );
            var id = pause_location["callFrames"][0]["callFrameId"].Value<string>();
            await EvaluateOnCallFrameAndCheck(
                id,
            ...
            Assert.Equal(
                $"Unable to evaluate element access 'a3[1]': Index is outside the bounds of the inline array",
                res.Error["result"]?["description"]?.Value<string>()
            );
