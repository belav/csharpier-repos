...
namespace DebuggerTests
{
    public class SourceList : DebuggerTestBase
    {
        ...
        {
            var eval_req = JObject.FromObject(new { expression = "invoke_bad_js_test();" });
        ...
        public async Task ExceptionThrownInJSOutOfBand()
        {
            ...
            Assert.Equal(
                dicFileToUrl["/debugger-driver.html"],
                ex_json["exceptionDetails"]?["url"]?.Value<string>()
            );
        }
        ...
        public async Task InspectLocalsTypesAtBreakpointSite() =>
            await CheckInspectLocalsAtBreakpointSite(
                "dotnet://debugger-test.dll/debugger-test2.cs",
                48,
                8,
                "Types",
        ...
        public async Task InspectSimpleStringLocals() =>
            await CheckInspectLocalsAtBreakpointSite(
                ...
                wait_for_event_fn: async (pause_location) =>
                {
                    ...
                    await CheckProps(
                        strings_arr,
                        new[] { str_null, str_empty, str_spaces, str_esc },
                        "locals#strings"
                    );
                }
            );
        ...
        public async Task InspectNullableLocals(string method_name, bool is_async) =>
            await CheckInspectLocalsAtBreakpointSite(
                ...
                wait_for_event_fn: async (pause_location) =>
                {
                    ...
                    await CheckProps(
                        ...
                        new
                        {
                            ...
                            n_gs_null = TObject(
                                "System.Nullable<DebuggerTests.ValueTypesTest.GenericStruct<int>>",
                                null
                            ),
                        },
                        "locals"
                    ...
                    var n_gs_props = await GetProperties(
                        n_gs["value"]?["objectId"]?.Value<string>()
                    );
                    await CheckProps(
                        n_gs_props,
                        new
            ...
            await CheckInspectLocalsAtBreakpointSite(
                ...
                test_fn: (locals) =>
                {
                    CheckObject(
                        locals,
                        "list",
                        "System.Collections.Generic.Dictionary<Math[], Math.IsMathNull>"
                    );
                    CheckObject(
                        locals,
                        "list_null",
        ...
        public async Task InspectLocalsWithStructs(bool use_cfo)
        {
            ...
            var vt_local_props = await GetObjectOnFrame(
                pause_location["callFrames"][0],
                "vt_local"
            );
            ...
            CheckValueType(
                vt_local_props,
                "SimpleStructField",
                "DebuggerTests.ValueTypesTest.SimpleStruct"
            );
            CheckValueType(
                vt_local_props,
            ...
            var ss_local_props = await GetObjectOnFrame(
                pause_location["callFrames"][0],
                "ss_local"
            );
            await CheckProps(
            ...
            foreach (var (name, bias, dt_kind) in exp)
            {
                ...
                var gres = await InvokeGetter(
                    GetAndAssertObjectWithName(vt_local_props, name),
                    "V"
                );
                await CheckValue(
                    gres.Value["result"],
                    TNumber(0xDEADBEEF + (uint)dt.Month),
                    $"{name}#V"
                );
            }
        ...
        public async Task InspectBoxedLocals(string method_name, bool is_async) =>
            await CheckInspectLocalsAtBreakpointSite(
                "DebuggerTest",
                method_name,
                17,
                ...
                wait_for_event_fn: async (pause_location) =>
                {
                    ...
                    foreach (var name in new[] { "n_gs", "o_gs", "o_n_gs" })
                    {
                        ...
                        var gs_props = await GetProperties(
                            gs["value"]?["objectId"]?.Value<string>()
                        );
                        await CheckProps(
                            gs_props,
                            new
        ...
        public async Task InspectBoxedTypeObject(string method_name, bool is_async) =>
            await CheckInspectLocalsAtBreakpointSite(
                "DebuggerTest",
                method_name,
                9,
        ...
        public async Task InspectBoxedAsClassLocals(string method_name, bool is_async) =>
            await CheckInspectLocalsAtBreakpointSite(
                "DebuggerTest",
                method_name,
                6,
        ...
        public async Task InspectLocalsWithStructsStaticAsync(bool use_cfo)
        {
            ...
            await CheckProps(
                ...
                new
                {
                    V = TGetter("V"),
                    str_member = TString(
                        "set in MethodWithLocalStructsStaticAsync#SimpleStruct#str_member"
                    ),
                    dt = TDateTime(dt),
                    gs = TValueType("DebuggerTests.ValueTypesTest.GenericStruct<System.DateTime>"),
                    Kind = TEnum("System.DateTimeKind", "Utc")
...

            {
                ...
                await CompareObjectPropertiesFor(
                    ...
                    {
                        StringField = TString(
                            "set in MethodWithLocalStructsStaticAsync#SimpleStruct#gs#StringField"
                        ),
        ...
        )
        {
            ...
            var pause_location = await EvaluateAndCheck(
                ...
                line,
                col,
                invoke_async ? "MoveNext" : method_name
            );
            ...
            var frame_locals = await GetProperties(
                pause_location["callFrames"][frame_idx]["callFrameId"].Value<string>()
            );
            await CheckProps(
                frame_locals,
                new
            ...
            var sst_props = await CompareObjectPropertiesFor(
                frame_locals,
                "sst",
                new
                {
        ...
        public async Task InspectLocals()
        {
            ...
            var locals = await GetProperties(
                wait_res["callFrames"][1]["callFrameId"].Value<string>()
            );
        }
        ...
        public async Task InspectLocalsForStructInstanceMethod(bool use_cfo) =>
            await CheckInspectLocalsAtBreakpointSite(
                "dotnet://debugger-test.dll/debugger-array-test.cs",
                258,
                12,
        ...
        public async Task InvalidValueTypeData()
        {
            await CheckInspectLocalsAtBreakpointSite(
                ...
                wait_for_event_fn: async (pause_location) =>
                {
                    var new_id = await CreateNewId(
                        @"MONO._new_or_add_id_props ({ scheme: 'valuetype', idArgs: { containerId: 1 }, props: { klass: 3, value64: 4 }});"
                    );
                    await _invoke_getter(new_id, "NonExistant", expect_ok: false);

                    new_id = await CreateNewId(
                        @"MONO._new_or_add_id_props ({ scheme: 'valuetype', idArgs: { containerId: 1 }, props: { klass: 3 }});"
                    );
                    await _invoke_getter(new_id, "NonExistant", expect_ok: false);

                    new_id = await CreateNewId(
                        @"MONO._new_or_add_id_props ({ scheme: 'valuetype', idArgs: { containerId: 1 }, props: { klass: 3, value64: 'AA' }});"
                    );
                    await _invoke_getter(new_id, "NonExistant", expect_ok: false);
                }
            );
            ...
            async Task<string> CreateNewId(string expr)
            {
                var res = await cli.SendCommand(
                    "Runtime.evaluate",
                    JObject.FromObject(new { expression = expr }),
                    token
                );
                Assert.True(res.IsOk, "Expected Runtime.evaluate to succeed");
                AssertEqual(
                    "string",
                    res.Value["result"]?["type"]?.Value<string>(),
                    "Expected Runtime.evaluate to return a string type result"
                );
            ...
            async Task<Result> _invoke_getter(string obj_id, string property_name, bool expect_ok)
            {
                ...
                AssertEqual(
                    expect_ok,
                    res.IsOk,
                    "Runtime.evaluate result not as expected for {expr}"
                );
        ...
        [Fact]
        public async Task MulticastDelegateTest() =>
            await CheckInspectLocalsAtBreakpointSite(
                "MulticastDelegateTestClass",
                "Test",
                5,
                "Test",
        ...
        public async Task EmptyTypeWithNoLocalsOrParams(string type_name, bool is_async) =>
            await CheckInspectLocalsAtBreakpointSite(
                type_name,
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task StaticMethodWithLocalEmptyStructThatWillGetExpanded(bool is_async) =>
            await CheckInspectLocalsAtBreakpointSite(
                "EmptyStruct",
        ...
        [Fact]
        public async Task PreviousFrameForAReflectedCall() =>
            await CheckInspectLocalsAtBreakpointSite(
                "DebuggerTests.GetPropertiesTests.CloneableStruct",
                "SimpleStaticMethod",
                1,
                "SimpleStaticMethod",
        ...
        public async Task DebugLazyLoadedAssemblyWithEmbeddedPdb()
        {
            ...
            var source_location =
                "dotnet://lazy-debugger-test-embedded.dll/lazy-debugger-test-embedded.cs";
            Assert.Contains(source_location, scripts.Values);
        ...
        {
            var bp = await SetBreakpointInMethod(
                "debugger-test-with-source-link.dll",
                "DebuggerTests.ClassToBreak",
                "TestBreakpoint",
                0
            );
            var pause_location = await EvaluateAndCheck(
            ...
            var sourceToGet = JObject.FromObject(
                new
                {
                    scriptId = pause_location["callFrames"][0]["functionLocation"][
                        "scriptId"
                    ].Value<string>()
                }
            );
