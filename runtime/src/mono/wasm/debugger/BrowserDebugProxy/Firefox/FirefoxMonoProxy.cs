...
internal sealed class FirefoxMonoProxy : MonoProxy
{
    public FirefoxMonoProxy(ILogger logger, string loggerId = null, ProxyOptions options = null)
        : base(logger, loggerId: loggerId, options: options) { }
    ...
    public async Task RunForFirefox(
        TcpClient ideClient,
        int portBrowser,
        ...
        try
        {
            ...
            else if (res["type"] == null || res["type"].Value<string>() != "evaluationResult")
            {
                var o = JObject.FromObject(
                    new { type = "evaluationResult", resultID = res["resultID"].Value<string>() }
                );
                ...
                res["result"] is JObject
                && res["result"]["type"].Value<string>() == "object"
                && res["result"]["class"].Value<string>() == "Array"
            )
            {
                var msgIdNew = new FirefoxMessageId(
                    null,
                    0,
                    res["result"]["actor"].Value<string>()
                );
                var id = int.Parse(res["resultID"].Value<string>().Split('-')[1]);
    ...
    protected override async Task<Result> SendCommandInternal(
        SessionId sessionId,
        string method,
        JObject args,
        CancellationToken token
    ...
    protected override Task SendEventInternal(
        SessionId sessionId,
        string method,
        JObject args,
        CancellationToken token
    ...
    )
    {
        ...
        {
            Contexts.OnDefaultContextUpdate(
                sessionId,
                new FirefoxExecutionContext(
                    new MonoSDBHelper(this, logger, sessionId),
                    0,
                    args["frame"]["consoleActor"].Value<string>()
                )
            );
            return false;
        }
        ...
        switch (args["type"].Value<string>())
        {
                ...
                switch (topFunc)
                {
                    ...
                    case "_mono_wasm_fire_debugger_agent_message_with_data_to_pause":
                    {
                        ...
                        return await OnReceiveDebuggerAgentEvent(
                            sessionId,
                            args,
                            GetLastDebuggerAgentBuffer(args),
                            token
                        );
                    }
                    default:
            ...
            case "resource-available-form":
            {
                ...
                {
                    if (
                        message["resourceType"].Value<string>() == "thread-state"
                        && message["state"].Value<string>() == "paused"
                    )
                    {
            ...
            case "target-available-form":
            {
                Contexts.OnDefaultContextUpdate(
                    ...
                    new FirefoxExecutionContext(
                        new MonoSDBHelper(this, logger, sessionId),
                        0,
                        args["target"]["consoleActor"].Value<string>()
                    )
                );
                var ctx = GetContextFixefox(sessionId);
        ...
        switch (args["type"].Value<string>())
        {
            ...
            case "getBreakpointPositionsCompressed":
            {
                ...
                if (args["to"].Value<string>().StartsWith("dotnet://"))
                {
                    ...
                    var o = JObject.FromObject(
                        new { positions = line, from = args["to"].Value<string>() }
                    );
            ...
            case "removeBreakpoint":
            {
                ...
                var reqToRemove = JObject.FromObject(
                    new
                    {
                        url = args["location"]["sourceUrl"].Value<string>(),
                        lineNumber = args["location"]["line"].Value<int>() - 1,
            ...
            case "slice":
            {
                ...
                var o = JObject.FromObject(
                    new { ownProperties = variables, from = args["to"].Value<string>() }
                );
            ...
            case "prototype":
            {
                if (!DotnetObjectId.TryParse(args?["to"], out DotnetObjectId objectId))
                    return false;
                var o = JObject.FromObject(
                    new { prototype = GetPrototype(args), from = args["to"].Value<string>() }
                );
                await SendEvent(sessionId, "", o, token);
                return true;
            }
            case "enumSymbols":
            {
                ...
                var iterator = JObject.FromObject(
                    new { iterator = o, from = args["to"].Value<string>() }
                );
            ...
            case "evaluateJSAsync":
            {
                ...
                if (context.CallStack != null)
                {
                    ...
                    var o = JObject.FromObject(new { resultID, from = args["to"].Value<string>() });
                    await SendEvent(sessionId, "", o, token);
                    ...
                    try
                    {
                        ...
                        {
                            osend["result"] = JObject.FromObject(
                                new
                                {
                                    type = retValue["type"],
    ...
    internal override void SaveLastDebuggerAgentBufferReceivedToContext(
        SessionId sessionId,
    ...
    internal static Result GetLastDebuggerAgentBuffer(JObject args)
    {
        ...
        result.Add(
            JObject.FromObject(
                new { value = new { value = args?["frame"]?["arguments"]?[0].Value<string>() } }
            )
        );
        Result res = Result.OkFromObject(new { result });
        return res;
    }

    private async Task<bool> SendPauseToBrowser(
        SessionId sessionId,
        JObject args,
        CancellationToken token
    )
    {
        ...
        byte[] newBytes = Convert.FromBase64String(
            res.Value?["result"]?[0]?["value"]?["value"]?.Value<string>()
        );
    ...
    private static JObject ConvertToFirefoxContent(ValueOrError<GetMembersResult> res)
    {
        ...
        foreach (var variable in resVars)
        {
            ...
            {
                variableDesc = JObject.FromObject(
                    new
                    {
            ...
            else
            {
                ...
                else //{"type":"null"}
                {
                    variableDesc.Add(
                        "value",
                        JObject.FromObject(
                            new { type = "null", @class = variable["value"]["className"] }
                        )
                    );
                }
            }
    ...
    protected override async Task SendResume(SessionId id, CancellationToken token)
    {
        ...
        await SendCommand(
            id,
            "",
            JObject.FromObject(new { to = ctx.ThreadName, type = "resume" }),
            token
        );
    }

    internal override Task<Result> SendMonoCommand(
        SessionId id,
        MonoCommands cmd,
        CancellationToken token
    )
    ...
    internal override async Task OnSourceFileAdded(
        SessionId sessionId,
        SourceFile source,
    ...
    protected override async Task<bool> SendCallStack(
        SessionId sessionId,
        ExecutionContext context,
        string reason,
        int thread_id,
    ...
    )
    {
        ...
        for (int j = 0; j < frame_count; j++)
        {
            ...
            var frameItem = JObject.FromObject(
                new
                {
        ...
        foreach (JObject frame in orig_callframes.Value["result"]?["value"]?["frames"])
        {
            ...
            if (
                function_name != null
                && !(
                    function_name.StartsWith("Module._mono_wasm", StringComparison.Ordinal)
        ...
        var o = JObject.FromObject(new { frames = callFrames, from = ctx.ThreadName });

        await SendEvent(sessionId, "", o, token);
        return false;
    }

    internal async Task<bool> OnGetBreakableLines(
        MessageId msg_id,
        string script_id,
        CancellationToken token
    )
    {
        ...
        await SendEvent(
            msg_id,
            "",
            JObject.FromObject(new { lines = src_file.BreakableLines.ToArray(), from = script_id }),
            token
        );
        return true;
    }

    internal override async Task<bool> OnGetScriptSource(
        MessageId msg_id,
        string script_id,
        CancellationToken token
    )
    {
        ...
        try
        {
            ...
            await SendEvent(
                msg_id,
                "",
                JObject.FromObject(new { source, from = script_id }),
                token
            );
        }
        catch (Exception e)
    ...
    internal override Task<DebugStore> LoadStore(
        SessionId sessionId,
