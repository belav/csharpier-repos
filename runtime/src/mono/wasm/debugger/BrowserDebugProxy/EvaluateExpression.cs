    ...
    internal static class EvaluateExpression
    {
        private class FindVariableNMethodCall : CSharpSyntaxWalker
        {
            ...
            )
            {
                ...
                {
                    foreach (
                        (InvocationExpressionSyntax ies, JObject value) in methodCall.Zip(
                            method_values
                        )
                    )
                    {
                ...
                CompilationUnitSyntax UpdateWithNewMethodParam(
                ...
                )
                {
                    ...
                    MethodDeclarationSyntax updatedMethod = method.AddParameterListParameters(
                        SyntaxFactory
                            .Parameter(SyntaxFactory.Identifier(id_name))
            ...
            private string GetTypeFullName(JToken variable)
            {
                ...
                throw new ReturnAsErrorException(
                    $"GetTypefullName: Evaluate of this datatype {type} not implemented yet",
                    "Unsupported"
                );
            }
        }
        ...
        private static SyntaxNode GetExpressionFromSyntaxTree(SyntaxTree syntaxTree)
        {
            ...
            ParenthesizedExpressionSyntax expressionParenthesized =
                returnValue.Expression as ParenthesizedExpressionSyntax;

            return expressionParenthesized?.Expression;
        }

        private static async Task<IList<JObject>> ResolveMemberAccessExpressions(
        ...
        private static async Task<IList<JObject>> ResolveIdentifiers(
        ...
        private static async Task<IList<JObject>> ResolveMethodCalls(
            IEnumerable<InvocationExpressionSyntax> methodCalls,
        ...
        )
        {
            ...
            foreach (InvocationExpressionSyntax methodCall in methodCalls)
            {
                ...
                if (value == null)
                    throw new ReturnAsErrorException(
                        $"Failed to resolve member access for {methodCall}",
                        "ReferenceError"
                    );
        ...
        )
        {
            ...
            IList<JObject> memberAccessValues = await ResolveMemberAccessExpressions(
            ...
            IList<JObject> identifierValues = await ResolveIdentifiers(
                findVarNMethodCall.identifiers,
                resolver,
                token
            );

            syntaxTree = findVarNMethodCall.ReplaceVars(
                syntaxTree,
                memberAccessValues,
                identifierValues,
                null,
            ...
            if (findVarNMethodCall.hasMethodCalls)
            {
                ...
                IList<JObject> methodValues = await ResolveMethodCalls(
                    findVarNMethodCall.methodCall,
                    findVarNMethodCall.memberAccessValues,
                    resolver,
                    token
                );
            ...
            if (findVarNMethodCall.hasElementAccesses)
            {
                ...
                IList<JObject> elementAccessValues = await ResolveElementAccess(
                    findVarNMethodCall.elementAccess,
                    findVarNMethodCall.memberAccessValues,
                    resolver,
                    token
                );
            ...
            CodeAnalysis.TypeInfo TypeInfo = semanticModel.GetTypeInfo(
                expressionTree,
                cancellationToken: token
            );
