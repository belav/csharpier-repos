...
namespace Microsoft.WebAssembly.Diagnostics
{
    internal static class ExpressionEvaluator
    {
        ...
        {
            private static Regex regexForReplaceVarName = new Regex(
                @"[^A-Za-z0-9_]",
            ...
            )
            {
                ...
                void AddLocalVariableWithValue(string idName, JObject value)
                {
                    ...
                    variableDefinitions.Add(
                        ConvertJSToCSharpLocalVariableAssignment(idName, value)
                    );
                }
            }

            private static string ConvertJSToCSharpLocalVariableAssignment(
                string idName,
                JToken variable
            )
            {
                ...
                switch (type)
                {
                    ...
                    case "object":
                        ...
                        {
                            (valueRet, typeRet) = GetNullObject(
                                variable["className"]?.Value<string>()
                            );
                        }
                        else
                        {
        ...
        private static async Task<(IList<JObject>, IList<JObject>, IList<JObject>)> ResolveMethodCalls(
        ...
        )
        {
            ...
            {
                logger.LogDebug(
                    $"Cannot evaluate '{expression}'. Script used to compile it: {newScript.Code}{Environment.NewLine}{cee.Message}"
                );
                throw new ReturnAsErrorException(
                    $"Cannot evaluate '{expression}': {cee.Message}",
                    "CompilationError"
                );
            }
            catch (Exception ex)
            {
                throw new Exception(
                    $"Internal Error: Unable to run {expression}, error: {ex.Message}.",
                    ex
                );
            }
        }
            ...
            : base($"[{className}] {message}")
        {
            ...
            _error = Result.UserVisibleErr(
                JObject.FromObject(
                    ...
                    {
                        result = result,
                        exceptionDetails = new { exception = result, stackTrace = StackTrace }
                    }
                )
            );
        }
