                        ...
                        if (elementAccess.ArgumentList != null)
                        {
                            for (int i = 0; i < elementAccess.ArgumentList.Arguments.Count; i++)
                            {
                                ...
                                else
                                {
                                    ...
                                    indexObject =
                                        await ExpressionEvaluator.EvaluateSimpleExpression(
                                            this,
                                            expression,
                                            expression,
                                            variableDefinitions,
                                    ...
                                    if (type != "number")
                                        throw new InvalidOperationException(
                                            $"Cannot index with an object of type '{type}'"
                                        );
                                    elementIdxStr += indexObject["value"].ToString();
                                }
                            }
                                ...
                                else
                                {
                                    return (JObject)(
                                        ((JArray)rootObject["value"]).FirstOrDefault(
                                            x => x["name"].Value<string>() == elementIdxStr
                                        )["value"]
                                    );
                                }
                            case "object":
                                ...
                                if (methodIds.Length > 1)
                                {
                                    foreach (var methodId in methodIds)
                                    {
                                        MethodInfoWithDebugInformation methodInfo =
                                            await context.SdbAgent.GetMethodInfo(methodId, token);
                                ...
                                try
                                {
                                    var toArrayRetMethod = await context.SdbAgent.InvokeMethod(
                                        objectId.Value,
                                        toArrayId,
                                        isValueType: false,
                                        token
                                    ...
                                    DotnetObjectId.TryParse(
                                        rootObject?["objectId"]?.Value<string>(),
                                        out DotnetObjectId arrayObjectId
                                    );
                                    rootObject["value"] = await context.SdbAgent.GetArrayValues(
                                        arrayObjectId.Value,
                                        token
