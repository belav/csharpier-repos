...
namespace Microsoft.WebAssembly.Diagnostics
{
    internal sealed class MemberReferenceResolver
    {
        ...
        public async Task<JObject> Resolve(string varName, CancellationToken token)
        {
            ...
            if (retObject == null)
            {
                (retObject, string remaining) = await ResolveStaticMembersInStaticTypes(
                    varName,
                    token
                );
            ...
            async Task<JObject> ResolveAsLocalOrThisMember(string name)
            {
                ...
                JToken objRet = valueOrError.Value.FirstOrDefault(
                    objPropAttr => objPropAttr["name"].Value<string>() == nameTrimmed
                );
                if (objRet != null)
            ...
            async Task<JObject> ResolveAsInstanceMember(string expr, JObject baseObject)
            {
                ...
                for (int i = 0; i < parts.Length; i++)
                {
                    ...
                    ValueOrError<GetMembersResult> valueOrError =
                        await proxy.RuntimeGetObjectMembers(sessionId, objectId, null, token);
                    if (valueOrError.IsError)
                    {
                        logger.LogDebug(
                    ...
                    JToken objRet = valueOrError.Value.FirstOrDefault(
                        objPropAttr => objPropAttr["name"]?.Value<string>() == partTrimmed
                    );
                    if (objRet == null)
                        return null;
                    ...
                    if (elementIdx >= 0)
                    {
                        ...
                        switch (objectId.Scheme)
                        {
                            ...
                            case "object":
                                ...
                                int methodId = await context.SdbAgent.GetMethodIdByName(
                                    typeIds[0],
                                    "ToArray",
                                    token
                                );
                            ...
                            default:
                                throw new InvalidOperationException(
                                    $"Cannot apply indexing with [] to an expression of type '{objectId.Scheme}'"
                                );
                        }
                    }
                }
        ...
        public async Task<(JObject, string)> ResolveInvokationInfo(
        ...
        public object ConvertCSharpToJSType(object v, Type type)
        {
            ...
            if (NumericTypes.Contains(v.GetType()))
                return new
                {
                    type = "number",
                    value = v,
                    description = Convert.ToDouble(v).ToString(CultureInfo.InvariantCulture)
                };
            if (v is bool)
                return new
                {
                    type = "boolean",
                    value = v,
                    description = v.ToString().ToLowerInvariant(),
                    className = type.ToString()
                };
            if (v is JObject)
                return v;
