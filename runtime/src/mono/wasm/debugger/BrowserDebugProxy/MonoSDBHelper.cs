...
namespace Microsoft.WebAssembly.Diagnostics
{
        ...
        public static MonoBinaryReader From(Result result)
        {
            ...
            if (result.IsOk)
            {
                newBytes = Convert.FromBase64String(
                    result.Value?["result"]?["value"]?["value"]?.Value<string>()
                );
            }
            ...
            else if (objectId.Scheme == "valuetype")
            {
                if (
                    ...
                    throw new ArgumentException(
                        $"Could not find any valuetype with id: {objectId.Value}",
                        nameof(objectId.Value)
                    );
            }
        }
    ...
    internal sealed class FieldTypeClass
    {
        ...
        )
        {
            ...
            TypeId = typeId;
            IsNotPrivate =
                (Attributes & FieldAttributes.FieldAccessMask & FieldAttributes.Public) != 0;
    ...
    internal sealed class PointerValue
    {
        ...
        public async Task<JObject> GetValue(MonoSDBHelper sdbHelper, CancellationToken token)
        {
            if (_value == null)
            {
                ...
                using var retDebuggerCmdReader = await sdbHelper.SendDebuggerAgentCommand(
                    CmdPointer.GetValue,
                    commandParamsWriter,
                    token
                );
    ...
    internal sealed class MonoSDBHelper
    {
        ...
        private static readonly Regex regexForAsyncLocals =
            new(
                @"\<(?<varName>[^)]*)\>(?<varId>[^)]*)(__)(?<scopeId>\d+)",
                RegexOptions.Singleline
            ); //<testCSharpScope>5__1 // works
        private static readonly Regex regexForVBAsyncLocals =
            new(
                @"\$VB\$ResumableLocal_(?<varName>[^\$]*)\$(?<scopeId>\d+)",
                RegexOptions.Singleline
            ); //$VB$ResumableLocal_testVbScope$2
        private static readonly Regex regexForVBAsyncMethodName =
            new(@"VB\$StateMachine_(\d+)_(?<methodName>.*)", RegexOptions.Singleline); //VB$StateMachine_2_RunVBScope
        private static readonly Regex regexForAsyncMethodName =
            new(@"\<([^>]*)\>([d][_][_])([0-9]*)", RegexOptions.Compiled);
        private static readonly Regex regexForGenericArgs =
            new(@"[`][0-9]+", RegexOptions.Compiled);
        private static readonly Regex regexForNestedLeftRightAngleBrackets =
            new(
                "^(((?'Open'<)[^<>]*)+((?'Close-Open'>)[^<>]*)+)*(?(Open)(?!))[^<>]*",
                RegexOptions.Compiled
            ); // <ContinueWithStaticAsync>b__3_0
        ...
        )
        {
            ...
            method ??= await CreateMethodInfoFromRuntimeInformation(
                asm,
                methodId,
                methodName,
                methodToken,
        ...
        public async Task<MethodInfo> CreateMethodInfoFromRuntimeInformation(
            AssemblyInfo asm,
            int methodId,
            string methodName,
            int methodToken,
        ...
        public async Task<(int, int)> GetVMVersion(CancellationToken token)
        {
            ...
            using var commandParamsWriter = new MonoBinaryWriter();
            using var retDebuggerCmdReader = await SendDebuggerAgentCommand(
                CmdVM.Version,
                commandParamsWriter,
                token
            );
        ...
        internal async Task<MonoBinaryReader> SendDebuggerAgentCommand<T>(
            T command,
            MonoBinaryWriter arguments,
            CancellationToken token,
        ...
        )
        {
            ...
            return !res.IsOk && throwOnError
                ? throw new DebuggerAgentException(
                    $"SendDebuggerAgentCommand failed for {command}: {res}"
                )
        ...
        internal async Task<MonoBinaryReader> SendDebuggerAgentCommandWithParms<T>(
            T command,
            (string data, int length) encoded,
            int type,
            string extraParm,
            CancellationToken token,
            bool throwOnError = true
        )
        {
            ...
            return !res.IsOk && throwOnError
                ? throw new DebuggerAgentException(
                    $"SendDebuggerAgentCommand failed for {command}: {res.Error}"
                )
                : MonoBinaryReader.From(res);
        }
        ...
        public async Task<int> CreateString(string value, CancellationToken token)
        {
            ...
            using var stringDebuggerCmdReader = await SendDebuggerAgentCommand(
        ...
        public async Task<MethodAttributes> GetAttributesFromMethodId(
            int methodId,
            CancellationToken token
        )
        {
        ...
        public async Task<string> GetAssemblyNameFromModule(int moduleId, CancellationToken token)
        {
            ...
            using var ret_debugger_cmd_reader = await SendDebuggerAgentCommand(
                CmdModule.GetInfo,
                command_params_writer,
                token
            );
        ...
        public async Task<string> GetPrettyMethodName(
            int methodId,
            bool isAnonymous,
            CancellationToken token
        )
            ...
            switch (type)
            {
                case ElementType.GenericInst:
                {
                    ...
                    while (match.Success)
                    {
                        ...
                        ret = ret.Insert(
                            match.Index,
                            $"<{string.Join(", ", genericArgs.Take(countArgs))}>"
                        );
                ...
                case ElementType.Class:
                {
                    ...
                    for (int i = 0; i <= countNested; i++)
                    {
                        ...
                        if (klassName.Contains("<>"))
                        {
                            if (anonymousMethodId.LastIndexOf('_') >= 0)
                                anonymousMethodId = klassName.Substring(
                                    klassName.LastIndexOf('_') + 1
                                );
                        }
                        ...
                        {
                            var matchOnClassName = regexForNestedLeftRightAngleBrackets.Match(
                                klassName
                            );
                            if (
                                matchOnClassName.Success
                                && matchOnClassName.Groups["Close"].Captures.Count > 0
                            )
                    ...
                    if (
                        matchOnMethodName.Success
                        && matchOnMethodName.Groups["Close"].Captures.Count > 0
                    )
                    {
                        if (
                            isAnonymous
                            && anonymousMethodId.Length == 0
        ...
        public async Task<int> SetBreakpointNoThrow(
            int methodId,
            long il_offset,
            CancellationToken token
        )
        ...
        )
        {
            ...
            var reader = await SendDebuggerAgentCommand(
        ...
        )
        {
            ...
            var retDebuggerCmdReader = await SendDebuggerAgentCommand(
                CmdType.GetCattrs,
                commandParamsWriter,
                token
            );
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                using var retDebuggerCmdReader2 = await SendDebuggerAgentCommand(
                    CmdMethod.GetDeclaringType,
                    commandCattrParamsWriter,
                    token
                );
        ...
        )
        {
            ...
            try
            {
                ...
                var thisObj = JObjectValueCreator.Create(
                    value: "",
                    type: "object",
                    description: "",
                    writable: false,
                    objectId: dotnetObjectId.ToString()
                );
                thisObj["name"] = "this";
                ...
                retValue ??= await ExpressionEvaluator.CompileAndRunTheExpression(
                    expr,
                    resolver,
                    logger,
                    token
            ...
            {
                logger.LogDebug(
                    $"Could not evaluate DebuggerDisplayAttribute - {expr} - {await GetTypeName(typeId, token)}: {ex}"
                );
            }
            return null;
        }
        ...
        public async Task<ArrayDimensions> GetArrayDimensions(
            int object_id,
            CancellationToken token
        )
        {
        ...
        public async Task<List<int>> GetTypeIdsForObject(
            int object_id,
            bool withParents,
            CancellationToken token
        )
        {
            ...
            if (withParents)
            {
                ...
                using var parentsCmdReader = await SendDebuggerAgentCommand(
                    CmdType.GetParents,
                    commandParentsParamsWriter,
                    token
                );
        ...
        public async Task<int[]> GetMethodIdsByName(
            int type_id,
            string method_name,
        ...
        )
        {
            if (type_id <= 0)
                throw new DebuggerAgentException(
                    $"Invalid type_id {type_id} (method_name: {method_name}"
                );
        ...
        public Task<JObject> InvokeMethod(
            int objectId,
            int methodId,
            bool isValueType,
            CancellationToken token,
        ...
        )
        {
            if (isValueType && !isMethodStatic)
            {
                return ValueCreator.TryGetValueTypeById(objectId, out var valueType)
                    ...
                    : throw new ArgumentException(
                        $"Could not find valuetype with id {objectId}, for method id: {methodId}",
                        nameof(objectId)
                    );
            }
        ...
        )
        {
            if (dotnetObjectId.Scheme == "method")
            {
                ...
                return objectId == null || embeddedMethodId == null
                    ? throw new ArgumentException(
                        $"Invalid object id for a method, with missing container, or methodId",
            ...
            return dotnetObjectId.Scheme is "object" or "valuetype"
                ? InvokeMethod(
                    dotnetObjectId.Value,
                    methodId,
                    isValueType: dotnetObjectId.IsValueType,
                    token
                )
                : throw new ArgumentException(
                    $"Cannot invoke method with id {methodId} on {dotnetObjectId}",
                    nameof(dotnetObjectId)
                );
        }

        public async Task<string> InvokeToStringAsync(
        ...
        public async Task<JObject> GetPointerContent(int pointerId, CancellationToken token)
        {
            ...
            if (pointerValue == null)
                throw new ArgumentException(
                    $"Could not find any pointer with id: {pointerId}",
                    nameof(pointerId)
                );
        ...
        public async Task<bool> IsAsyncMethod(int methodId, CancellationToken token)
        {
            ...
            methodInfo.Info.IsAsync = Convert.ToInt32(
                await GetIsAsyncFromMethodId(methodId, token)
            );
            return methodInfo.Info.IsAsync == 1;
        }
        ...
        public async Task<JArray> GetHoistedLocalVariables(
            ...
            int objectId,
            IEnumerable<JToken> asyncLocals,
            int offset,
            CancellationToken token
        )
        {
            ...
            foreach (var asyncLocal in asyncLocals)
            {
                ...
                else if (IsClosureReferenceField(fieldName)) //same code that has on debugger-libs
                {
                    ...
                    )
                    {
                        ...
                        {
                            var asyncProxyMembersFromObject =
                                await MemberObjectsExplorer.GetObjectMemberValues(
                                    this,
                                    dotnetObjectId.Value,
                                    GetObjectCommandOptions.WithProperties,
                                    token
                            ...
                            var hoistedLocalVariable = await GetHoistedLocalVariables(
                                method,
                                dotnetObjectId.Value,
                                asyncProxyMembersFromObject.Flatten(),
                                offset,
                                token
                            );
        ...
        )
        {
            ...
            if (await IsAsyncMethod(method.DebugId, token))
            {
                ...
                GetMembersResult asyncProxyMembers =
                    await MemberObjectsExplorer.GetObjectMemberValues(
                        this,
                        objectId,
                        GetObjectCommandOptions.WithProperties,
                ...
                var asyncLocals = await GetHoistedLocalVariables(
                    method,
                    objectId,
                    asyncProxyMembers.Flatten(),
                    offset,
                    token
                );
                return asyncLocals;
            ...
            using var localsDebuggerCmdReader = await SendDebuggerAgentCommand(
                CmdFrame.GetValues,
                commandParamsWriter,
                token
            );
            foreach (var var in varIds)
            {
                try
                {
                    var var_json = await ValueCreator.ReadAsVariableValue(
        ...
        public async Task<JArray> GetArrayValues(int arrayId, CancellationToken token)
        {
            ...
            for (int i = 0; i < dimensions.TotalLength; i++)
            {
                var var_json = await ValueCreator.ReadAsVariableValue(
                    retDebuggerCmdReader,
                    dimensions.GetArrayIndexString(i),
                    token
                );
                array.Add(var_json);
            }
        ...
        public async Task<JObject> GetArrayValuesProxy(int arrayId, CancellationToken token)
        {
            ...
            var arrayProxy = JObject.FromObject(
                new
                {
        ...
        public async Task<GetMembersResult> GetValuesFromDebuggerProxyAttributeForObject(
            int objectId,
            int typeId,
            CancellationToken token
        )
        {
            try
            {
                ...
                var retMethod = await InvokeMethod(
                    ctorArgsWriter.GetParameterBuffer(),
                    methodId,
                    token
                );
                if (
                    ...
                    throw new Exception(
                        $"Invoking .ctor ({methodId}) for DebuggerTypeProxy on type {typeId} returned {retMethod}"
                    );
            ...
            {
                logger.LogDebug(
                    $"Could not evaluate DebuggerTypeProxyAttribute of type {await GetTypeName(typeId, token)} - {e}"
                );
            }
        ...
        public async Task<GetMembersResult> GetValuesFromDebuggerProxyAttributeForValueTypes(
            int valueTypeId,
            int typeId,
            CancellationToken token
        )
        ...
        private async Task<int> FindDebuggerProxyConstructorIdFor(
            int typeId,
            CancellationToken token
        )
        {
            try
            {
                ...
                if (parmCount != 1)
                    throw new InternalErrorException(
                        $"Expected to find custom attribute with only one argument, but it has {parmCount} parameters."
                    );
                ...
                {
                    logger.LogDebug(
                        $"DebuggerTypeProxy attribute is only supported with a System.Type parameter type. Got {(ValueTypeId)monoParamTypeId}"
                    );
                    return -1;
                }
                ...
                if (originalClassName.IndexOf('[') > 0)
                {
                    ...
                    for (int k = 0; k < genericTypeArgs.Count; k++)
                    {
                        ...
                        typeToSearch.Append(
                            $"{(k == 0 ? "" : ",")}[{classNameArg}, {assemblyNameArg}]"
                        );
                    }
                    ...
                    {
                        logger.LogDebug(
                            $"Could not find instantiated generic type id for {typeToSearch}."
                        );
                        return -1;
                    }
                    typeProxyTypeId = genericTypeId;
                }
                int[] constructorIds = await GetMethodIdsByName(
                    typeProxyTypeId,
                    ".ctor",
                ...
                if (constructorIds is null)
                    throw new InternalErrorException(
                        $"Could not find any constructor for DebuggerProxy type: {originalClassName}"
                    );
                ...
                foreach (var methodId in constructorIds)
                {
                    ...
                    if (string.IsNullOrEmpty(parameters))
                        throw new InternalErrorException(
                            $"Could not get method's parameter types. MethodId = {methodId}."
                        );
                    if (parameters == $"({expectedConstructorParamType})")
                        return methodId;
                }
                throw new InternalErrorException(
                    $"Could not find a matching constructor for DebuggerProxy type: {originalClassName}"
                );
            }
            catch (Exception e)
            {
        ...
        public ValueTypeClass GetValueTypeClass(int valueTypeId)
        {
            if (ValueCreator.TryGetValueTypeById(valueTypeId, out ValueTypeClass vt))
                return vt;
            throw new ArgumentException(
                $"Could not find any valuetype with id: {valueTypeId}",
                nameof(valueTypeId)
            );
        }

        public Task<GetMembersResult> GetTypeMemberValues(
            DotnetObjectId dotnetObjectId,
            GetObjectCommandOptions getObjectOptions,
            CancellationToken token
        ) =>
            dotnetObjectId.IsValueType
                ? MemberObjectsExplorer.GetValueTypeMemberValues(
                    this,
                    dotnetObjectId.Value,
                    getObjectOptions,
                    token
                )
                : MemberObjectsExplorer.GetObjectMemberValues(
                    this,
                    dotnetObjectId.Value,
                    getObjectOptions,
                    token
        ...
        {
            GetMembersResult members = await MemberObjectsExplorer.GetObjectMemberValues(
                this,
                objectId,
        ...
        )
        {
            ...
            JArray locals = new JArray();
            using var getDebuggerCmdReader = await SendDebuggerAgentCommand(
                CmdFrame.GetValues,
                commandParamsWriter,
                token
            );
            int etype = getDebuggerCmdReader.ReadByte();
            using var setDebuggerCmdReader = await SendDebuggerAgentCommandWithParms(
                CmdFrame.SetValues,
                commandParamsWriter.ToBase64(),
                etype,
                newValue,
                token,
                throwOnError: false
        ...
        public async Task<bool> SetNextIP(
            MethodInfoWithDebugInformation method,
            int threadId,
            IlLocation ilOffset,
            CancellationToken token
        ...
        public async Task<int> CreateByteArray(string diff, CancellationToken token)
        {
            ...
            using var arrayDebuggerCmdReader = await SendDebuggerAgentCommand(
                CmdAppDomain.CreateByteArray,
                commandParamsWriter,
                token
            );
        ...
        public async Task<byte[][]> GetBytesFromAssemblyAndPdb(
            string assemblyName,
            CancellationToken token
        )
        {
