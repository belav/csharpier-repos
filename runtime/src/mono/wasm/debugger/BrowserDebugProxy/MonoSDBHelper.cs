...
namespace Microsoft.WebAssembly.Diagnostics
{
    ...
    internal class MonoBinaryWriter : BinaryWriter
    {
        ...
        )
        {
            switch (constValue.Kind())
            {
                ...
                {
                    int stringId = await SdbHelper.CreateString(
                        sessionId,
                        (string)constValue.Token.Value,
                        token
                    );
        ...
        public async Task<bool> WriteJsonValue(
            SessionId sessionId,
            JObject objValue,
            MonoSDBHelper SdbHelper,
            CancellationToken token
        )
        {
            switch (objValue["type"].Value<string>())
            {
                ...
                case "object":
                {
                    DotnetObjectId.TryParse(
                        objValue["objectId"]?.Value<string>(),
                        out DotnetObjectId objectId
                    );
    ...
    internal class MonoSDBHelper
    {
        ...
        public async Task<AssemblyInfo> GetAssemblyInfo(
            SessionId sessionId,
            int assemblyId,
            CancellationToken token
        )
        ...
        public async Task<MethodInfoWithDebugInformation> GetMethodInfo(
            SessionId sessionId,
            int methodId,
            CancellationToken token
        )
        {
            ...
            if (method == null && !asm.HasSymbols)
            {
                ...
                {
                    logger.LogDebug(
                        $"Unable to find method token: {methodToken} assembly name: {asm.Name} exception: {e}"
                    );
                    return null;
                }
            }
            ...
            {
                logger.LogDebug(
                    $"Unable to find method token: {methodToken} assembly name: {asm.Name}"
                );
                return null;
            }
        ...
        public async Task<TypeInfoWithDebugInformation> GetTypeInfo(
            SessionId sessionId,
            int typeId,
            CancellationToken token
        )
        ...
        public async Task<bool> SetProtocolVersion(SessionId sessionId, CancellationToken token)
        {
            ...
            var retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdVM>(
                sessionId,
                CmdVM.SetProtocolVersion,
                commandParams,
                token
            );
            return true;
        ...
        public async Task<bool> EnableReceiveRequests(
            SessionId sessionId,
            EventKind eventKind,
            CancellationToken token
        )
        {
            ...
            commandParamsWriter.Write((byte)0);
            var retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdEventRequest>(
                sessionId,
                CmdEventRequest.Set,
                commandParams,
                token
        ...
        internal async Task<MonoBinaryReader> SendDebuggerAgentCommandInternal(
            SessionId sessionId,
            int command_set,
            int command,
            MemoryStream parms,
        ...
        )
        {
            Result res = await proxy.SendMonoCommand(
                sessionId,
                MonoCommands.SendDebuggerAgentCommand(
                    GetId(),
                    command_set,
                    command,
                    Convert.ToBase64String(parms.ToArray())
                ),
                token
            );
            ...
            if (!res.IsErr)
            {
                newBytes = Convert.FromBase64String(
                    res.Value?["result"]?["value"]?["value"]?.Value<string>()
                );
            }
        ...
        internal Task<MonoBinaryReader> SendDebuggerAgentCommand<T>(
            SessionId sessionId,
            T command,
            MemoryStream parms,
            CancellationToken token
        ) where T : Enum =>
            SendDebuggerAgentCommandInternal(
                sessionId,
        ...
        internal Task<MonoBinaryReader> SendDebuggerAgentCommandWithParms<T>(
            SessionId sessionId,
            T command,
            MemoryStream parms,
            int type,
            ...
            SendDebuggerAgentCommandWithParmsInternal(
                sessionId,
                (int)GetCommandSetForCommand(command),
                (int)(object)command,
                parms,
        ...
        internal async Task<MonoBinaryReader> SendDebuggerAgentCommandWithParmsInternal(
            SessionId sessionId,
            int command_set,
            int command,
            MemoryStream parms,
        ...
        )
        {
            Result res = await proxy.SendMonoCommand(
                sessionId,
                MonoCommands.SendDebuggerAgentCommandWithParms(
                    GetId(),
                    command_set,
                    command,
                    Convert.ToBase64String(parms.ToArray()),
                    parms.ToArray().Length,
                    type,
                    extraParm
                ),
                token
        ...
        public async Task<int> CreateString(
            SessionId sessionId,
            string value,
            CancellationToken token
        )
        {
            var commandParams = new MemoryStream();
            var commandParamsWriter = new MonoBinaryWriter(commandParams);
            var retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdAppDomain>(
                sessionId,
                CmdAppDomain.GetRootDomain,
                commandParams,
                token
            );
            ...
            retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdAppDomain>(
                sessionId,
                CmdAppDomain.CreateString,
                commandParams,
                token
            );
        ...
        public async Task<int> GetMethodToken(
            SessionId sessionId,
            int methodId,
            CancellationToken token
        )
        {
            ...
            var retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdMethod>(
                sessionId,
                CmdMethod.Token,
                commandParams,
                token
        ...
        public async Task<int> MakeGenericMethod(
            SessionId sessionId,
            int methodId,
            List<int> genericTypes,
            CancellationToken token
        )
        {
            var commandParams = new MemoryStream();
        ...
        public async Task<int> GetMethodIdByToken(
            SessionId sessionId,
            int assembly_id,
            int method_token,
            CancellationToken token
        )
        {
            ...
            var retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdAssembly>(
                sessionId,
                CmdAssembly.GetMethodFromToken,
                commandParams,
                token
            );
            return retDebuggerCmdReader.ReadInt32();
        ...
        public async Task<int> GetAssemblyIdFromType(
            SessionId sessionId,
            int type_id,
            CancellationToken token
        )
        {
            ...
            var retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdType>(
                sessionId,
                CmdType.GetInfo,
                commandParams,
                token
        ...
        public async Task<List<int>> GetTypeParamsOrArgsForGenericType(
            SessionId sessionId,
            int typeId,
            CancellationToken token
        )
        ...
        public async Task<int> GetAssemblyIdFromMethod(
            SessionId sessionId,
            int methodId,
            CancellationToken token
        )
        ...
        public async Task<int> GetAssemblyId(
            SessionId sessionId,
            string asm_name,
            CancellationToken token
        )
        ...
        public async Task<string> GetAssemblyNameFromModule(
            SessionId sessionId,
            int moduleId,
            CancellationToken token
        )
        {
            ...
            var ret_debugger_cmd_reader = await SendDebuggerAgentCommand<CmdModule>(
                sessionId,
                CmdModule.GetInfo,
                command_params,
                token
        ...
        public async Task<string> GetAssemblyName(
            SessionId sessionId,
            int assembly_id,
            CancellationToken token
        )
        ...
        public async Task<string> GetFullAssemblyName(
            SessionId sessionId,
            int assemblyId,
            CancellationToken token
        )
        ...
        public async Task<string> GetAssemblyFileNameFromId(
            SessionId sessionId,
            int assemblyId,
            CancellationToken token
        )
        ...
        public async Task<bool> MethodIsStatic(
            SessionId sessionId,
            int methodId,
            CancellationToken token
        )
        ...
        public async Task<int> GetParamCount(
            SessionId sessionId,
            int methodId,
            CancellationToken token
        )
        ...
        public async Task<int> SetBreakpoint(
            SessionId sessionId,
            int methodId,
            long il_offset,
            CancellationToken token
        ...
        public async Task<bool> ClearSingleStep(
            SessionId sessionId,
            int req_id,
            CancellationToken token
        )
        ...
        public async Task<JObject> GetFieldValue(
            SessionId sessionId,
            int typeId,
            int fieldId,
            CancellationToken token
        )
        {
            ...
            return await CreateJObjectForVariableValue(
                sessionId,
        ...
        public async Task<int> TypeIsInitialized(
            SessionId sessionId,
            int typeId,
            CancellationToken token
        )
        ...
        public async Task<int> TypeInitialize(
            SessionId sessionId,
            int typeId,
            CancellationToken token
        )
        ...
        public async Task<MonoBinaryReader> GetTypePropertiesReader(
            SessionId sessionId,
            int typeId,
            CancellationToken token
        )
        {
            ...
            var reader = await SendDebuggerAgentCommand<CmdType>(
                sessionId,
        ...
        public async Task<List<FieldTypeClass>> GetTypeFields(
            SessionId sessionId,
            int typeId,
            CancellationToken token
        )
        ...
        internal async Task<MonoBinaryReader> GetCAttrsFromType(
            SessionId sessionId,
            int objectId,
            int typeId,
            string attrName,
        ...
        )
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                commandParamsWriter.Write(methodId);
                var retDebuggerCmdReader2 = await SendDebuggerAgentCommand<CmdMethod>(
                    sessionId,
                    CmdMethod.GetDeclaringType,
                    commandParams,
                    token
                );
                var customAttributeTypeId = retDebuggerCmdReader2.ReadInt32();
                var customAttributeName = await GetTypeName(
                    sessionId,
                ...
                for (int k = 0; k < 2; k++)
                {
                    ...
                    {
                        //to typed_args
                        await CreateJObjectForVariableValue(
                            sessionId,
                            retDebuggerCmdReader,
                            "varName",
                            false,
        ...
        public async Task<int> GetAssemblyFromType(
            SessionId sessionId,
            int type_id,
            CancellationToken token
        )
        ...
        public async Task<string> GetValueFromDebuggerDisplayAttribute(
            SessionId sessionId,
            int objectId,
            int typeId,
            CancellationToken token
        )
        {
            ...
            try
            {
                var getCAttrsRetReader = await GetCAttrsFromType(
                    sessionId,
                    objectId,
                    typeId,
                ...
                JArray objectValues = await GetObjectValues(
                    sessionId,
                    objectId,
                    GetObjectCommandOptions.WithProperties
                        | GetObjectCommandOptions.ForDebuggerDisplayAttribute,
                    token
                );

                var thisObj = CreateJObject<string>(
                    value: "",
                    type: "object",
                    description: "",
                    writable: false,
                ...
                var resolver = new MemberReferenceResolver(
                    proxy,
                    context,
                    sessionId,
                    objectValues,
                ...
                if (retValue == null)
                    retValue = await EvaluateExpression.CompileAndRunTheExpression(
                        expr,
                        resolver,
                        token
                    );
            ...
            {
                logger.LogDebug(
                    $"Could not evaluate DebuggerDisplayAttribute - {expr} - {await GetTypeName(sessionId, typeId, token)}"
                );
            }
            return null;
        }
        ...
        public async Task<string> GetTypeNameOriginal(
            SessionId sessionId,
            int typeId,
            CancellationToken token
        )
        ...
        public async Task<int> GetTypeToken(
            SessionId sessionId,
            int typeId,
            CancellationToken token
        )
        ...
        )
        {
            ...
            var retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdString>(
                sessionId,
                CmdString.GetValue,
                commandParams,
                token
        ...
        )
        {
            ...
            var retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdArray>(
                sessionId,
                CmdArray.GetLength,
                commandParams,
                token
        ...
        )
        {
            ...
            var retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdObject>(
                sessionId,
                CmdObject.RefGetType,
                commandParams,
                token
            );
            ...
            if (withParents)
            {
                ...
                commandParamsWriter = new MonoBinaryWriter(commandParams);
                commandParamsWriter.Write(type_id);
                retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdType>(
                    sessionId,
                    CmdType.GetParents,
                    commandParams,
                    token
        ...
        public async Task<string> GetClassNameFromObject(
            SessionId sessionId,
            int object_id,
            CancellationToken token
        )
        ...
        public async Task<int> GetTypeIdFromToken(
            SessionId sessionId,
            int assemblyId,
            int typeToken,
            CancellationToken token
        ...
        public async Task<int> GetMethodIdByName(
            SessionId sessionId,
            int type_id,
            string method_name,
            CancellationToken token
        ...
        public async Task<bool> IsDelegate(
            SessionId sessionId,
            int objectId,
            CancellationToken token
        )
        ...
        public async Task<int> GetDelegateMethod(
            SessionId sessionId,
            int objectId,
            CancellationToken token
        )
        {
            ...
            var retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdObject>(
                sessionId,
                CmdObject.RefDelegateGetMethod,
                commandParams,
                token
            );
            return retDebuggerCmdReader.ReadInt32();
        ...
        public async Task<string> GetDelegateMethodDescription(
            SessionId sessionId,
            int objectId,
            CancellationToken token
        )
        ...
        public async Task<int> GetPropertyMethodIdByName(
            SessionId sessionId,
            int typeId,
            string propertyName,
            CancellationToken token
        ...
        public async Task<JArray> CreateJArrayForProperties(
            SessionId sessionId,
            int typeId,
            byte[] object_buffer,
            JArray attributes,
        ...
        )
        {
            ...
            for (int i = 0; i < nProperties; i++)
            {
                ...
                if (
                    getMethodId == 0
                    || await GetParamCount(sessionId, getMethodId, token) != 0
                    || await MethodIsStatic(sessionId, getMethodId, token)
                )
                    continue;
                ...
                if (
                    attributes
                        .Where(
                            attribute => attribute["name"].Value<string>().Equals(propertyNameStr)
                        )
                        .Any()
                )
                    continue;
        ...
        )
        {
            ...
            var retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdPointer>(
                sessionId,
                CmdPointer.GetValue,
                commandParams,
                token
        ...
        public async Task<JArray> GetPropertiesValuesOfValueType(
            SessionId sessionId,
            int valueTypeId,
            CancellationToken token
        )
        ...
        public JObject CreateJObject<T>(
            T value,
            string type,
            string description,
            bool writable,
        ...
        )
        {
            var ret = JObject.FromObject(
                new { value = new { type, value, description }, writable }
            );
        ...
        public async Task<JObject> CreateJObjectForPtr(
            SessionId sessionId,
            ElementType etype,
        ...
        )
        {
            ...
            return CreateJObject<string>(
                value,
                type,
                value,
                false,
                className,
                $"dotnet:pointer:{pointerId}",
                "pointer"
            );
        }

        public async Task<JObject> CreateJObjectForString(
            SessionId sessionId,
            MonoBinaryReader retDebuggerCmdReader,
            CancellationToken token
        )
        ...
        public async Task<JObject> CreateJObjectForArray(
            SessionId sessionId,
            MonoBinaryReader retDebuggerCmdReader,
            CancellationToken token
        )
        {
            ...
            return CreateJObject<string>(
                ...
                false,
                value.ToString(),
                "dotnet:array:" + objectId,
                null,
                "array"
            );
        }

        public async Task<JObject> CreateJObjectForObject(
            SessionId sessionId,
            MonoBinaryReader retDebuggerCmdReader,
        ...
        )
        {
            ...
            if (!forDebuggerDisplayAttribute)
                debuggerDisplayAttribute = await GetValueFromDebuggerDisplayAttribute(
                    sessionId,
                    objectId,
                    type_id[0],
                    token
        ...
        )
        {
            ...
            if (className.IndexOf("System.Nullable<") == 0) //should we call something on debugger-agent to check???
            {
                ...
                var value = await CreateJObjectForVariableValue(
                    sessionId,
                    retDebuggerCmdReader,
                    name,
                    false,
            ...
            {
                fieldValueType = await CreateJObjectForVariableValue(
                    sessionId,
                    retDebuggerCmdReader,
                    fields.ElementAt(i).Name,
                    true,
                    fields.ElementAt(i).TypeId,
                    false,
                    token
                );
        ...
        public async Task<JObject> CreateJObjectForNull(
            SessionId sessionId,
            MonoBinaryReader retDebuggerCmdReader,
            CancellationToken token
        )
        ...
        public async Task<JObject> CreateJObjectForVariableValue(
            SessionId sessionId,
            MonoBinaryReader retDebuggerCmdReader,
            string name,
            bool isOwn,
        ...
        {
            long initialPos =
                retDebuggerCmdReader == null ? 0 : retDebuggerCmdReader.BaseStream.Position;
            ...
            switch (etype)
            {
                ...
                case (ElementType)ValueTypeId.FixedArray:
                    ret = JObject.FromObject(
                        new { value = new { type = "void", value = "void", description = "void" } }
                    );
                    break;
                ...
                {
                    ret = await CreateJObjectForObject(
                        sessionId,
                        retDebuggerCmdReader,
                        typeIdFromAttribute,
                ...
                {
                    ret = await CreateJObjectForValueType(
                        sessionId,
                        retDebuggerCmdReader,
                        name,
                        initialPos,
        ...
        public async Task<bool> IsAsyncMethod(
            SessionId sessionId,
            int methodId,
            CancellationToken token
        )
        ...
        public async Task<JArray> GetHoistedLocalVariables(
            SessionId sessionId,
            int objectId,
            JArray asyncLocals,
            CancellationToken token
        )
        {
            ...
            foreach (var asyncLocal in asyncLocals)
            {
                ...
                else if (IsClosureReferenceField(fieldName)) //same code that has on debugger-libs
                {
                    if (
                        DotnetObjectId.TryParse(
                            asyncLocal?["value"]?["objectId"]?.Value<string>(),
                            out DotnetObjectId dotnetObjectId
                        )
                    )
                    {
                        if (
                            int.TryParse(dotnetObjectId.Value, out int objectIdToGetInfo)
                            && !objectsAlreadyRead.Contains(objectIdToGetInfo)
                        )
                        {
                            var asyncLocalsFromObject = await GetObjectValues(
                                sessionId,
                                objectIdToGetInfo,
                                GetObjectCommandOptions.WithProperties,
                                token
                            );
                            var hoistedLocalVariable = await GetHoistedLocalVariables(
                                sessionId,
                                objectIdToGetInfo,
                                asyncLocalsFromObject,
                                token
                            );
                            asyncLocalsFull = new JArray(
                                asyncLocalsFull.Union(hoistedLocalVariable)
                            );
                        }
                    }
                }
        ...
        public async Task<JArray> StackFrameGetValues(
            SessionId sessionId,
            MethodInfoWithDebugInformation method,
            int thread_id,
            int frame_id,
            VarInfo[] varIds,
            CancellationToken token
        )
        {
            ...
            {
                retDebuggerCmdReader = await SendDebuggerAgentCommand<CmdFrame>(
                    sessionId,
                    CmdFrame.GetThis,
                    commandParams,
                    token
                ...
                var objectId = retDebuggerCmdReader.ReadInt32();
                var asyncLocals = await GetObjectValues(
                    sessionId,
                    objectId,
                    GetObjectCommandOptions.WithProperties,
                    token
                );
                asyncLocals = await GetHoistedLocalVariables(
                    sessionId,
                    objectId,
                    asyncLocals,
                    token
            ...
            foreach (var var in varIds)
            {
                var var_json = await CreateJObjectForVariableValue(
                    sessionId,
                    retDebuggerCmdReader,
                    var.Name,
                    false,
        ...
        public async Task<JArray> GetValueTypeValues(
            SessionId sessionId,
            int valueTypeId,
        ...
        public async Task<JArray> GetValueTypeProxy(
            SessionId sessionId,
            int valueTypeId,
            CancellationToken token
        )
        {
            ...
            valueTypes[valueTypeId].valueTypeProxy = new JArray(
                valueTypes[valueTypeId].valueTypeJson
            );

            var retDebuggerCmdReader = await GetTypePropertiesReader(
                sessionId,
                valueTypes[valueTypeId].typeId,
                token
            );
            if (retDebuggerCmdReader == null)
                return null;
        ...
        public async Task<JArray> GetArrayValues(
            SessionId sessionId,
            int arrayId,
            CancellationToken token
        )
        ...
        public async Task<int> GetTypeByName(
            SessionId sessionId,
            string typeToSearch,
            CancellationToken token
        )
        ...
        public async Task<JArray> GetValuesFromDebuggerProxyAttribute(
            SessionId sessionId,
            int objectId,
            int typeId,
            CancellationToken token
        )
        {
            try
            {
                ...
                for (int j = 0; j < parmCount; j++)
                {
                    ...
                    if (className.IndexOf('[') > 0)
                    {
                        ...
                        List<int> genericTypeArgs = await GetTypeParamsOrArgsForGenericType(
                            sessionId,
                            typeId,
                            token
                        );
                        ...
                        {
                            var assemblyIdArg = await GetAssemblyIdFromType(
                                sessionId,
                                genericTypeArgs[k],
                                token
                            );
                            var assemblyNameArg = await GetFullAssemblyName(
                                sessionId,
                                assemblyIdArg,
                                token
                            );
                            var classNameArg = await GetTypeNameOriginal(
                                sessionId,
                                genericTypeArgs[k],
                                token
                            );
                    ...
                    DotnetObjectId.TryParse(
                        retMethod?["value"]?["objectId"]?.Value<string>(),
                        out DotnetObjectId dotnetObjectId
                    );
                    var displayAttrs = await GetObjectValues(
                        sessionId,
                        int.Parse(dotnetObjectId.Value),
                        GetObjectCommandOptions.WithProperties
                            | GetObjectCommandOptions.ForDebuggerProxyAttribute,
                        token
                    );
                    return displayAttrs;
                }
            ...
            {
                logger.LogDebug(
                    $"Could not evaluate DebuggerTypeProxyAttribute of type {await GetTypeName(sessionId, typeId, token)} - {e}"
                );
            }
            return null;
        }

        public async Task<JArray> GetObjectValues(
            SessionId sessionId,
            int objectId,
        ...
        )
        {
            ...
            {
                var debuggerProxy = await GetValuesFromDebuggerProxyAttribute(
                    sessionId,
                    objectId,
                    typeId[0],
                    token
            ...
            for (int i = 0; i < typeId.Count; i++)
            {
                if (!getCommandType.HasFlag(GetObjectCommandOptions.AccessorPropertiesOnly))
                {
                    ...
                    foreach (var field in fields)
                    {
                        ...
                        retDebuggerCmdReader.BaseStream.Position = initialPos;
                        var fieldValue = await CreateJObjectForVariableValue(
                            sessionId,
                            retDebuggerCmdReader,
                            field.Name,
                            i == 0,
                            field.TypeId,
                            getCommandType.HasFlag(
                                GetObjectCommandOptions.ForDebuggerDisplayAttribute
                            ),
                            token
                        );
                        if (
                            ret.Where(
                                    attribute =>
                                        attribute["name"]
                                            .Value<string>()
                                            .Equals(fieldValue["name"].Value<string>())
                                )
                                .Any()
                        )
                        {
        ...
        public async Task<JArray> GetObjectProxy(
            SessionId sessionId,
            int objectId,
            CancellationToken token
        )
        {
            var ret = await GetObjectValues(
                sessionId,
                objectId,
                GetObjectCommandOptions.WithSetter,
                token
            );
        ...
        public async Task<bool> SetVariableValue(
            SessionId sessionId,
            int thread_id,
            int frame_id,
            int varId,
        ...
        )
        {
            ...
            retDebuggerCmdReader = await SendDebuggerAgentCommandWithParms<CmdFrame>(
                sessionId,
                CmdFrame.SetValues,
                commandParams,
                etype,
