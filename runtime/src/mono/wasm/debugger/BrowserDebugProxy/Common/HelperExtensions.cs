...
internal static class HelperExtensions
{
    ...
    private static readonly bool TruncateLogMessages = string.IsNullOrEmpty(
    ...
    public static string Truncate(this string message, int maxLen, string suffix = "") =>
        string.Concat(
            message.Substring(0, Math.Min(message.Length, maxLen)).AsSpan(),
            message.Length > maxLen ? suffix : ""
        );

    public static string TruncateLogMessage(this string message) =>
        TruncateLogMessages ? message.Truncate(MaxLogMessageLineLength, ".. truncated") : message;
