...
public class IcuTests : IcuTestsBase
{
    ...
    public static IEnumerable<object?[]> FullIcuWithICustomIcuTestData(bool aot, RunHost host) =>
        ConfigWithAOTData(aot)
            .Multiply(new object[] { true }, new object[] { false })
            .WithRunHosts(host)
            .UnwrapItemsAsArrays();

    [Theory]
    [MemberData(
        nameof(FullIcuWithInvariantTestData),
    ...
    public void FullIcuFromRuntimePackWithInvariant(
        BuildArgs buildArgs,
        bool invariant,
        bool fullIcu,
    ...
    {
        string projectName =
            $"fullIcuInvariant_{fullIcu}_{invariant}_{buildArgs.Config}_{buildArgs.AOT}";
        bool dotnetWasmFromRuntimePack = !(buildArgs.AOT || buildArgs.Config == "Release");

        buildArgs = buildArgs with { ProjectName = projectName };
        buildArgs = ExpandBuildArgs(
            buildArgs,
            extraProperties: $"<InvariantGlobalization>{invariant}</InvariantGlobalization><WasmIncludeFullIcuData>{fullIcu}</WasmIncludeFullIcuData>"
        );
        ...
        (_, string output) = BuildProject(
            ...
            new BuildProjectOptions(
                ...
                GlobalizationMode: invariant ? GlobalizationMode.Invariant
                    : fullIcu ? GlobalizationMode.FullIcu
    ...
    [Theory]
    [MemberData(
        nameof(FullIcuWithICustomIcuTestData),
        parameters: new object[] { false, RunHost.NodeJS | RunHost.Chrome }
    )]
    [MemberData(
        nameof(FullIcuWithICustomIcuTestData),
    ...
    public void FullIcuFromRuntimePackWithCustomIcu(
        BuildArgs buildArgs,
        bool fullIcu,
        RunHost host,
        string id
    )
    {
        ...
        buildArgs = ExpandBuildArgs(
            buildArgs,
            extraProperties: $"<WasmIcuDataFileName>{CustomIcuPath}</WasmIcuDataFileName><WasmIncludeFullIcuData>{fullIcu}</WasmIncludeFullIcuData>"
        );
        ...
        (_, string output) = BuildProject(
            ...
            new BuildProjectOptions(
                InitProject: () =>
                    File.WriteAllText(Path.Combine(_projectDir!, "Program.cs"), programText),
                DotnetWasmFromRuntimePack: dotnetWasmFromRuntimePack,
                GlobalizationMode: fullIcu
                    ? GlobalizationMode.FullIcu
                    : GlobalizationMode.PredefinedIcu,
                PredefinedIcudt: fullIcu ? "" : CustomIcuPath
            )
        );
        if (fullIcu)
            Assert.Contains(
                "$(WasmIcuDataFileName) has no effect when $(WasmIncludeFullIcuData) is set to true.",
                output
            );
    ...
    )
    {
        ...
        buildArgs = ExpandBuildArgs(
            buildArgs,
            extraProperties: $"<WasmIcuDataFileName>{customIcu}</WasmIcuDataFileName>"
        );
        ...
        {
            Assert.Contains(
                $"File in location $(WasmIcuDataFileName)={customIcu} cannot be found neither when used as absolute path nor a relative runtime pack path.",
                output
            );
        }
        else
        {
            Assert.Contains(
                $"Custom ICU file name in path $(WasmIcuDataFileName)={customIcu} must start with 'icudt'.",
                output
            );
        }
    }
