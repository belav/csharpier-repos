...
namespace Wasm.Build.Tests
{
    public class PInvokeTableGeneratorTests : TestMainJsTestBase
    {
        ...
        )
        {
            ...
            Assert.Matches(
                "warning\\sWASM0001.*Could\\snot\\sget\\spinvoke.*Parsing\\sfunction\\spointer\\stypes",
                output
            );
            Assert.Matches(
                "warning\\sWASM0001.*Skipping.*using_sum_one.*because.*function\\spointer",
                output
            );
        ...
        private (BuildArgs buildArgs, string output) SingleProjectForDisabledRuntimeMarshallingTest(
            bool withDisabledRuntimeMarshallingAttribute,
            bool expectSuccess,
            BuildArgs buildArgs,
            string id
        )
        {
            ...
            buildArgs = ExpandBuildArgs(
                buildArgs with
                {
                    ProjectName = $"not_blittable_{buildArgs.Config}_{id}",
                },
        ...
        public static IEnumerable<object?[]> SeparateAssemblyWithDisableMarshallingAttributeTestData(
            string config
        ) =>
            ConfigWithAOTData(aot: false, config: config)
                .Multiply(
                    new object[]
                    { /*libraryHasAttribute*/
                        false, /*appHasAttribute*/
                        false, /*expectSuccess*/
                        false,
                    },
                    new object[]
                    { /*libraryHasAttribute*/
                        true, /*appHasAttribute*/
                        false, /*expectSuccess*/
                        false,
                    },
                    new object[]
        ...
        [Theory]
        [MemberData(
            nameof(SeparateAssemblyWithDisableMarshallingAttributeTestData),
            parameters: "Debug"
        )]
        [MemberData(
            nameof(SeparateAssemblyWithDisableMarshallingAttributeTestData),
        ...
        public void UnmanagedStructsAreConsideredBlittableFromDifferentAssembly(
            BuildArgs buildArgs,
            bool libraryHasAttribute,
            bool appHasAttribute,
            bool expectSuccess,
            RunHost host,
            string id
        ) =>
        ...
        )
        {
            ...
            var libraryBuildArgs = ExpandBuildArgs(
                buildArgs with
                {
                    ProjectName = $"blittable_different_library_{buildArgs.Config}_{id}",
                },
            ...
            buildArgs = ExpandBuildArgs(
                buildArgs with
                {
                    ProjectName = $"blittable_different_app_{buildArgs.Config}_{id}",
                },
        ...
        [Theory]
        [BuildAndRun(host: RunHost.Chrome)]
        public void DllImportWithFunctionPointers_WarningsAsMessages(
            BuildArgs buildArgs,
            RunHost host,
            string id
        )
        ...
        )
        {
            ...
            (_, string output) = BuildForVariadicFunctionTests(
                ...
                buildArgs with
                {
                    ProjectName = $"cb_fnptr_{buildArgs.Config}",
                },
                id
            );

            Assert.Matches(
                "warning\\sWASM0001.*Skipping.*Test::SomeFunction1.*because.*function\\spointer",
                output
            );
        }
        ...
        public void UnmanagedCallback_InFileType(BuildArgs buildArgs, RunHost host, string id)
        {
            ...
            (buildArgs, string output) = BuildForVariadicFunctionTests(
                ...
                buildArgs with
                {
                    ProjectName = $"cb_filetype_{buildArgs.Config}",
                },
                id
            );
        ...
        public void IcallWithOverloadedParametersAndEnum(BuildArgs buildArgs, string id)
        {
            ...
            var libraryBuildArgs = ExpandBuildArgs(
                buildArgs with
                {
                    ProjectName = $"icall_enum_library_{buildArgs.Config}_{id}",
                }
            );
            ...
            string tasksDir = Path.Combine(
                ...
                "tasks",
                BuildTestBase.TargetFrameworkForTasks
            ); // not net472!
                ...
                if (!string.IsNullOrEmpty(tasksDirParent))
                {
                    if (!Directory.Exists(tasksDirParent))
                    {
                        _testOutput.WriteLine(
                            $"Expected {tasksDirParent} to exist and contain TFM subdirectories"
                        );
                    }
                    _testOutput.WriteLine(
                        $"runtime pack tasks dir {tasksDir} contains subdirectories:"
                    );
            ...
            string? taskPath = Directory
                .EnumerateFiles(tasksDir, "WasmAppBuilder.dll", SearchOption.AllDirectories)
                .FirstOrDefault();
            ...
            projectCode = projectCode
                .Replace(
                    "###WasmPInvokeModule###",
                    AddAssembly("System.Private.CoreLib")
                        + AddAssembly("System.Runtime")
                        + AddAssembly(libraryBuildArgs.ProjectName)
                )
                .Replace("###WasmAppBuilder###", taskPath);

            buildArgs = buildArgs with
            {
                ProjectName = $"icall_enum_{buildArgs.Config}_{id}",
                ProjectFileContents = projectCode,
            };
            ...
            (_, output) = BuildProject(
                ...
                new BuildProjectOptions(
                    ...
                    {
                        File.WriteAllText(
                            Path.Combine(_projectDir!, "runtime-icall-table.h"),
                            icallTable
                        );
                    },
                    Publish: buildArgs.AOT,
        ...
        public void BuildNativeInNonEnglishCulture(
            BuildArgs buildArgs,
            string culture,
            RunHost host,
            string id
        ...
        public void CallIntoLibrariesWithNonAlphanumericCharactersInTheirNames(
            BuildArgs buildArgs,
            string[] libraryNames,
            RunHost host,
            string id
        )
        {
            ...
            void GenerateSourceFiles(string outputPath, int baseArg)
            {
                ...
                for (int i = 0; i < libraryNames.Length; i++)
                {
                    dllImportsBuilder.AppendLine(
                        $"[DllImport(\"{libraryNames[i]}\")] static extern int square_{i}(int x);"
                    );
                    csBuilder.AppendLine(
                        $@"Console.WriteLine($""square_{i}: {{square_{i}({i + baseArg})}}"");"
                    );
        ...
        {
            extraProperties +=
                "<AllowUnsafeBlocks>true</AllowUnsafeBlocks><_WasmDevel>true</_WasmDevel>";
