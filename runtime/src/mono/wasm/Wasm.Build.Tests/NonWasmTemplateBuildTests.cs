...
public class NonWasmTemplateBuildTests : TestMainJsTestBase
{
    ...
    public static IEnumerable<object?[]> GetTestData() =>
        new IEnumerable<object?>[] { new object?[] { "Debug" }, new object?[] { "Release" } }
            .AsEnumerable()
            .MultiplyWithSingleArgs("", "/p:RunAOTCompilation=true", "/p:WasmBuildNative=true")
            .MultiplyWithSingleArgs("net6.0", s_previousTargetFramework, s_latestTargetFramework)
            .UnwrapItemsAsArrays()
            .ToList();

    [Theory, TestCategory("no-workload")]
    [MemberData(nameof(GetTestData))]
    public void NonWasmConsoleBuild_WithoutWorkload(
        string config,
    ...
    [Theory]
    [MemberData(nameof(GetTestData))]
    public void NonWasmConsoleBuild_WithWorkload(
        string config,
        string extraBuildArgs,
        string targetFramework
    ) =>
    ...
    private void NonWasmConsoleBuild(
        string config,
        string extraBuildArgs,
        string targetFramework,
    ...
    )
    {
        ...
        directoryBuildTargets ??=
            targetFramework == s_previousTargetFramework
        ...
        new DotNetCommand(s_buildEnv, _testOutput, useDefaultArgs: false)
            ...
            .ExecuteWithCapturedOutput(
                $"build -restore -c {config} -bl:{Path.Combine(s_buildEnv.LogRootPath, $"{id}.binlog")} {extraBuildArgs} -f {targetFramework}"
            )
            .EnsureSuccessful();
