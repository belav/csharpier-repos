...
namespace Wasm.Build.Tests
{
    public class BuildPublishTests : NativeRebuildTestsBase
    {
        ...
        public void BuildThenPublishWithAOT(BuildArgs buildArgs, RunHost host, string id)
        {
            ...
            publishStat =
                (Dictionary<string, FileStat>)
                    _provider.StatFiles(pathsDict.Select(kvp => kvp.Value.fullPath));
            Assert.True(publishStat["pinvoke.o"].Exists);
            Assert.True(publishStat[$"{mainDll}.bc"].Exists);
            CheckOutputForNativeBuild(
                expectAOT: true,
                expectRelinking: false,
                buildArgs,
                output,
                testUnicode
            );
        ...
        )
        {
            if (testUnicode)
            {
                ...
                TestUtils.AssertMatches(
                    @$"{projectNameCore}\S+.dll -> {projectNameCore}\S+.dll.bc",
                    buildOutput,
                    contains: expectAOT
                );
                TestUtils.AssertMatches(
                    @$"{projectNameCore}\S+.dll.bc -> {projectNameCore}\S+.dll.o",
                    buildOutput,
                    contains: expectAOT
                );
            }
            ...
            TestUtils.AssertMatches(
                "pinvoke.c -> pinvoke.o",
                buildOutput,
                contains: expectRelinking || expectAOT
            );
        }
    }
}
