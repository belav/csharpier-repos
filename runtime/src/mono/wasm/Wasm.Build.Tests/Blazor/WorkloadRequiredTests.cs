...
public class WorkloadRequiredTests : BlazorWasmTestBase
{
    ...
    public static TheoryData<string, string, bool> SettingDifferentFromValuesInRuntimePack()
    {
        ...
        foreach (var defaultPair in PropertiesWithTriggerValues)
        {
            foreach (string config in configs)
            {
                data.Add(
                    config,
                    $"<{defaultPair.propertyName}>{defaultPair.triggerValue}</{defaultPair.propertyName}>",
                    true
                );
                data.Add(
                    config,
    ...
    public void WorkloadRequiredForBuild(
        string config,
    ...
    [Theory, TestCategory("no-workload")]
    [MemberData(nameof(SettingDifferentFromValuesInRuntimePack))]
    public void WorkloadRequiredForPublish(
        string config,
        string extraProperties,
        bool workloadNeeded
    ...
    public async Task WorkloadNotRequiredForInvariantGlobalization(
        string config,
        bool invariant,
        bool publish
    )
    {
        ...
        if (invariant)
            AddItemsPropertiesToProject(
                projectFile,
                extraProperties: "<InvariantGlobalization>true</InvariantGlobalization>"
            );

        string counterPath = Path.Combine(
        ...
        if (allText.IndexOf(ccText) < 0)
            throw new Exception(
                "Counter.razor does not have the expected content. Test needs to be updated."
            );
        ...
        GlobalizationMode mode = invariant
            ? GlobalizationMode.Invariant
            : GlobalizationMode.Sharded;
        if (publish)
        {
            (result, _) = BlazorPublish(
                new BlazorBuildOptions(id, config, ExpectSuccess: true, GlobalizationMode: mode)
            );
        }
        else
        {
    ...
    private void CheckWorkloadRequired(
        string config,
        string extraProperties,
        bool workloadNeeded,
        bool publish
    )
    {
        ...
        {
            Assert.DoesNotContain(
                "following workloads must be installed: wasm-tools",
                result.Output
            );
