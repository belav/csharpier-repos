...
public abstract class BlazorWasmTestBase : WasmTemplateTestBase
{
    ...
    public void InitBlazorWasmProjectDir(
        string id,
    ...
    )
    {
        ...
        File.WriteAllText(
            Path.Combine(_projectDir, "nuget.config"),
        ...
        File.Copy(
            Path.Combine(BuildEnvironment.TestDataPath, "Blazor.Directory.Build.props"),
            Path.Combine(_projectDir, "Directory.Build.props")
        );
        File.Copy(
            Path.Combine(BuildEnvironment.TestDataPath, "Blazor.Directory.Build.targets"),
    ...
    protected (CommandResult, string) BlazorBuild(
    ...
    protected (CommandResult, string) BlazorPublish(
        BlazorBuildOptions options,
        params string[] extraArgs
    )
    {
    ...
    )
    {
        ...
        catch (XunitException xe)
        {
            if (xe.Message.Contains("error CS1001: Identifier expected"))
                Utils.DirectoryCopy(
                    _projectDir!,
                    Path.Combine(s_buildEnv.LogRootPath, id),
    ...
    public void AssertBundle(string buildOutput, BlazorBuildOptions blazorBuildOptions)
    {
        if (IsUsingWorkloads)
        {
            ...
            ProjectProviderBase.AssertRuntimePackPath(
                buildOutput,
                blazorBuildOptions.TargetFramework ?? DefaultTargetFramework,
                blazorBuildOptions.RuntimeType
            );
        }
        ...
        string objBuildDir = Path.Combine(
            _projectDir!,
            "obj",
            blazorBuildOptions.Config,
            blazorBuildOptions.TargetFramework!,
            "wasm",
            "for-build"
        );
        ...
        if (blazorBuildOptions.ExpectRelinkDirWhenPublishing)
            Assert.True(
                ...
                $"Could not find expected {objBuildDir}, which gets created when relinking during Build. This is likely a test authoring error"
            );
        else
            Assert.False(
                File.Exists(Path.Combine(objBuildDir, "emcc-link.rsp")),
                $"Found unexpected `emcc-link.rsp` in {objBuildDir}, which gets created when relinking during Build."
            );
    }
    ...
    public string FindBlazorBinFrameworkDir(
        string config,
        bool forPublish,
        string framework = DefaultTargetFrameworkForBlazor
    ) =>
