...
namespace Wasm.Build.Tests
{
    public class SatelliteAssembliesTests : TestMainJsTestBase
    {
        ...
        [Theory]
        [MemberData(
            nameof(SatelliteAssemblyTestData),
        ...
        )
        {
            ...
            bool dotnetWasmFromRuntimePack =
                !nativeRelink && !buildArgs.AOT && buildArgs.Config != "Release";

            string extraProperties =
                (nativeRelink ? $"<WasmBuildNative>true</WasmBuildNative>" : string.Empty)
            ...
            BuildProject(
                ...
                new BuildProjectOptions(
                    ...
                    {
                        Utils.DirectoryCopy(
                            Path.Combine(BuildEnvironment.TestAssetsPath, "resx"),
                            Path.Combine(_projectDir!, "resx")
                        );
                        CreateProgramForCultureTest(
                            _projectDir!,
                            $"{projectName}.resx.words",
                            "TestClass"
                        );
                    },
            ...
            RunAndTestWasmApp(
                ...
                extraXHarnessMonoArgs: host is RunHost.Chrome
                    ? "--fetch-random-delay=200"
                    : string.Empty
            );
        }
        ...
        )
        {
            ...
            string extraProperties =
                $"<WasmBuildNative>{(nativeRelink ? "true" : "false")}</WasmBuildNative>"
                // make ASSERTIONS=1 so that we test with it
                + $"<EmccCompileOptimizationFlag>-O0 -sASSERTIONS=1</EmccCompileOptimizationFlag>"
                + $"<EmccLinkOptimizationFlag>-O1</EmccLinkOptimizationFlag>";
            ...
            BuildProject(
                ...
                new BuildProjectOptions(
                    ...
                    InitProject: () =>
                    {
                        ...
                        Utils.DirectoryCopy(
                            Path.Combine(BuildEnvironment.TestAssetsPath, projectName),
                            rootDir
                        );
                        ...
                        File.Move(
                            ...
                            Path.Combine(_projectDir, "Directory.Build.props")
                        );
                        File.Move(
                            Path.Combine(rootDir, "Directory.Build.targets"),
                            Path.Combine(_projectDir, "Directory.Build.targets")
                        );
                        if (UseWBTOverridePackTargets)
                            File.Move(
                                Path.Combine(rootDir, "WasmOverridePacks.targets"),
                                Path.Combine(_projectDir, "WasmOverridePacks.targets")
                            );

                        CreateProgramForCultureTest(
                            _projectDir,
                            "LibraryWithResources.resx.words",
                            "LibraryWithResources.Class1"
                        );
...
#pragma warning disable xUnit1026
        ...
        public void CheckThatSatelliteAssembliesAreNotAOTed(BuildArgs buildArgs, string id)
        {
            ...
            var bitCodeFileNames = Directory
