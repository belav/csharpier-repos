...
public class WasmSdkBasedProjectProvider : ProjectProviderBase
{
    ...
    public void AssertBundle(BuildArgs buildArgs, BuildProjectOptions buildProjectOptions)
    {
        AssertBundle(
            new(
                ...
                BinFrameworkDir: buildProjectOptions.BinFrameworkDir
                    ?? FindBinFrameworkDir(
                        buildArgs.Config,
                ...
                ExpectedFileType: buildProjectOptions.Publish && buildArgs.Config == "Release"
    ...
    {
        IReadOnlyDictionary<string, DotNetFileName> actualDotnetFiles = AssertBasicBundle(
            assertOptions
        );
        ...
        string runtimeNativeDir = BuildTestBase.s_buildEnv
        ...
        foreach (string nativeFilename in new[] { "dotnet.native.wasm", "dotnet.native.js" })
        {
            if (!actualDotnetFiles.TryGetValue(nativeFilename, out DotNetFileName? dotnetFile))
            {
                throw new XunitException(
                    $"Could not find {nativeFilename}. Actual files on disk: {string.Join($"{Environment.NewLine}  ", actualDotnetFiles.Values.Select(a => a.ActualPath).Order())}"
                );
            }
            // For any *type*, check against the expected path
            TestUtils.AssertSameFile(
                Path.Combine(srcDirForNativeFileToCompareAgainst, nativeFilename),
            ...
            if (assertOptions.ExpectedFileType != NativeFilesType.FromRuntimePack)
            {
                ...
                TestUtils.AssertNotSameFile(
                    Path.Combine(runtimeNativeDir, nativeFilename),
                    actualDotnetFiles[nativeFilename].ActualPath,
                    buildType
                );
            }
