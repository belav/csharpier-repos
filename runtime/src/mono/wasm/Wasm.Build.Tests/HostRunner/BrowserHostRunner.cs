...
public class BrowserHostRunner : IHostRunner
{
            ...
            if (s_binaryPathArg is null)
            {
                if (!string.IsNullOrEmpty(EnvironmentVariables.BrowserPathForTests))
                {
                    if (!File.Exists(EnvironmentVariables.BrowserPathForTests))
                        throw new Exception(
                            $"Cannot find BROWSER_PATH_FOR_TESTS={EnvironmentVariables.BrowserPathForTests}"
                        );
                    s_binaryPathArg =
                        $" --browser-path=\"{EnvironmentVariables.BrowserPathForTests}\"";
                }
                else
                {
                    s_binaryPathArg = "";
    ...
    public string GetXharnessArgsWindowsOS(XHarnessArgsOptions options) =>
        $"-v trace -b {options.host} --browser-arg=--lang={options.environmentLocale} --web-server-use-cop {BinaryPathArg}"; // Windows: chrome.exe --lang=locale

    public string GetXharnessArgsOtherOS(XHarnessArgsOptions options) =>
        $"-v trace -b {options.host} --locale={options.environmentLocale} --web-server-use-cop {BinaryPathArg}"; // Linux: LANGUAGE=locale ./chrome
