...
public static class TestUtils
{
    ...
    public static void AssertFilesExist(
        string dir,
    ...
    public static void AssertSameFile(string file0, string file1, string? label = null) =>
        AssertFile(file0, file1, label, same: true);

    public static void AssertNotSameFile(string file0, string file1, string? label = null) =>
    ...
    )
    {
        ...
        if (same && finfo0.Length != finfo1.Length)
            throw new XunitException(
                $"{label}:{Environment.NewLine}  File sizes don't match for {file0} ({finfo0.Length}), and {file1} ({finfo1.Length})"
            );

        if (!same && finfo0.Length == finfo1.Length)
            throw new XunitException(
                $"{label}:{Environment.NewLine}  File sizes should not match for {file0} ({finfo0.Length}), and {file1} ({finfo1.Length})"
            );
    }
    ...
    public static void AssertEqual(object expected, object actual, string label)
    {
        ...
        throw EqualException.ForMismatchedValues(expected, actual, $"[{label}]\n");
    }
