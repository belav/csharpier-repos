...
namespace Wasm.Build.NativeRebuild.Tests
{
    ...
    public class NativeRebuildTestsBase : BuildTestBase
    {
        ...
        internal (BuildArgs BuildArgs, BuildPaths paths) FirstNativeBuild(
            string programText,
            bool nativeRelink,
            bool invariant,
            BuildArgs buildArgs,
        ...
        {
            buildArgs = GenerateProjectContents(
                buildArgs,
                nativeRelink,
                invariant,
                extraProperties
        ...
        )
        {
            ...
            var newBuildArgs = GenerateProjectContents(
                buildArgs,
                nativeRelink,
                invariant,
                extraProperties
        ...
        protected BuildArgs GenerateProjectContents(
            BuildArgs buildArgs,
            bool nativeRelink,
            bool invariant,
            string extraProperties
        ...
        internal void CompareStat(
            IDictionary<string, FileStat> oldStat,
            IDictionary<string, FileStat> newStat,
            IEnumerable<(string fullpath, bool unchanged)> expected
        )
        {
            ...
            foreach (var expect in expected)
            {
                ...
                {
                    msg.AppendLine(
                        $"[Expected unchanged file: {expectFilename}]{Environment.NewLine}"
                ...
                {
                    msg.AppendLine(
                        $"[Expected changed file: {expectFilename}]{Environment.NewLine}"
                            + $"   {newFs}"
                    );
                }
            }
        ...
        internal IDictionary<string, (string fullPath, bool unchanged)> GetFilesTable(
            bool unchanged,
            params string[] baseDirs
        )
        {
