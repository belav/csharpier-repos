...
internal sealed class WasmSymbolicator
{
    ...
    public WasmSymbolicator(
        string? symbolsMapFile,
        string? symbolPatternsFile,
        bool throwOnMissing,
        ILogger logger
    )
    {
        ...
        if (string.IsNullOrEmpty(symbolPatternsFile) || !File.Exists(symbolPatternsFile))
        {
            if (throwOnMissing)
                throw new ArgumentException(
                    $"Cannot find symbol patterns file: {symbolPatternsFile}"
                );
        ...
        foreach (var line in File.ReadAllLines(symbolsMapFile))
        {
            ...
            {
                _logger.LogWarning(
                    $"Unexpected symbol map format at line {i + 1} in {symbolsMapFile}"
                );
                break;
            }
        ...
        foreach (var line in File.ReadAllLines(symbolPatternsFile))
        {
            ...
            try
            {
                ...
                {
                    _logger.LogWarning(
                        $"Ignoring pattern with missing `{FuncNumGroupName}` group: {line}"
                    );
                    continue;
                }
                _regexes.Add(regex);
            ...
            {
                _logger.LogWarning(
                    $"Failed to compile regex for symbol patterns: {line} with error: {ae.Message}"
                );
                continue;
            }
        }
