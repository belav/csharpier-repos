...
public static unsafe class Raytrace
{
    ...
    public abstract class Intersector
    {
        ...
        public abstract unsafe bool Intersect(
            ref Vec3f pos,
            ref Vec3f dir,
            void* obj,
            ref Vec3f intersection_normal
        );
    }
        ...
        for (int s = 0; s < scene.Length; s++)
        {
            ...
            if (
                intersector.Intersect(
                    ref pos,
                    ref dir,
                    Unsafe.AsPointer(ref sphere),
                    ref intersection_normal
                )
            )
            {
...
public static partial class Program
{
    ...
    internal static void OnClick()
    {
        ...
        Console.WriteLine(
            "Rendering finished in " + (DateTime.UtcNow - now).TotalMilliseconds + " ms"
        );
        RenderCanvas(bytes);
    }
}
