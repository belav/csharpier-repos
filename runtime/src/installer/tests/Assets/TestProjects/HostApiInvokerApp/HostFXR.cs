    ...
    public static class HostFXR
    {
        internal static class hostfxr
        {
            ...
            internal delegate void hostfxr_get_available_sdks_result_fn(
                int sdk_count,
                [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0)] string[] sdk_dirs
            );
        ...
        {
            hostfxr.hostfxr_error_writer_fn writer1 = (message) =>
            {
            ...
            hostfxr.hostfxr_error_writer_fn writer2 = (message) =>
            {
        ...
        static void Test_hostfxr_get_dotnet_environment_info(string[] args)
        {
            ...
            List<hostfxr.hostfxr_dotnet_environment_sdk_info> sdks =
                new List<hostfxr.hostfxr_dotnet_environment_sdk_info>();
            List<hostfxr.hostfxr_dotnet_environment_framework_info> frameworks =
                new List<hostfxr.hostfxr_dotnet_environment_framework_info>();

            hostfxr.hostfxr_get_dotnet_environment_info_result_fn result_fn = (
                IntPtr info,
                IntPtr result_context
            ) =>
            {
                hostfxr.hostfxr_dotnet_environment_info environment_info =
                    Marshal.PtrToStructure<hostfxr.hostfxr_dotnet_environment_info>(info);
                ...
                if ((nuint)env_info_size != environment_info.size)
                    throw new Exception(
                        $"Size field value of hostfxr_dotnet_environment_info struct is {environment_info.size} but {env_info_size} was expected."
                    );

                for (int i = 0; i < (int)environment_info.sdk_count; i++)
                {
                    IntPtr pSdkInfo = new IntPtr(
                        environment_info.sdks.ToInt64()
                    ...
                    if ((nuint)Marshal.SizeOf(sdks[i]) != sdks[i].size)
                        throw new Exception(
                            $"Size field value of hostfxr_dotnet_environment_sdk_info struct is {sdks[i].size} but {Marshal.SizeOf(sdks[i])} was expected."
                        );
                }

                for (int i = 0; i < (int)environment_info.framework_count; i++)
                {
                    IntPtr pFrameworkInfo = new IntPtr(
                        environment_info.frameworks.ToInt64()
                            + (
                                i
                                * Marshal.SizeOf<hostfxr.hostfxr_dotnet_environment_framework_info>()
                            )
                    );
                    frameworks.Add(
                    ...
                    if ((nuint)Marshal.SizeOf(frameworks[i]) != frameworks[i].size)
                        throw new Exception(
                            $"Size field value of hostfxr_dotnet_environment_framework_info struct is {frameworks[i].size} but {Marshal.SizeOf(frameworks[i])} was expected."
                        );
                }
                ...
                if (result_context_as_int != 42)
                    throw new Exception(
                        $"Invalid result_context value: expected 42 but was {result_context_as_int}."
                    );
            };
            ...
            Console.WriteLine(
                $"hostfxr_get_dotnet_environment_info sdk versions:[{string.Join(";", sdks.Select(s => s.version).ToList())}]"
            );
            Console.WriteLine(
                $"hostfxr_get_dotnet_environment_info sdk paths:[{string.Join(";", sdks.Select(s => s.path).ToList())}]"
            );

            Console.WriteLine(
                $"hostfxr_get_dotnet_environment_info framework names:[{string.Join(";", frameworks.Select(f => f.name).ToList())}]"
            );
            Console.WriteLine(
                $"hostfxr_get_dotnet_environment_info framework versions:[{string.Join(";", frameworks.Select(f => f.version).ToList())}]"
            );
            Console.WriteLine(
                $"hostfxr_get_dotnet_environment_info framework paths:[{string.Join(";", frameworks.Select(f => f.path).ToList())}]"
            );
