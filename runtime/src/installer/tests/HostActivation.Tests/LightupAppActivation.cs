            ...
            IDisposable
    {
        ...
        public LightupAppActivation(LightupAppActivation.SharedTestState fixture)
        {
            ...
            _baseDirArtifact = new TestArtifact(
                SharedFramework.CalculateUniqueTestDirectory(sharedLookupDir)
            );
            _fxBaseDir = Path.Combine(_baseDirArtifact.Location, "shared", "Microsoft.NETCore.App");
            _uberFxBaseDir = Path.Combine(
                _baseDirArtifact.Location,
                "shared",
            ...
            var repoDirectories = new RepoDirectoriesProvider(
                builtDotnet: _baseDirArtifact.Location
            );
        ...
        {
            var fixtureLib = sharedTestState.LightupLibFixture_Built.Copy();

            var fixtureApp = sharedTestState.LightupClientFixture.Copy();
            ...
            dotnet
                .Exec("exec", "--additional-deps", libDepsJson, appDll)
                .CaptureStdErr()
                .CaptureStdOut()
                ...
                .HaveStdErrContaining(
                    "Error:"
                        ...
                        + Environment.NewLine
                        + "    package: \'LightupLib\', version: \'1.0.0\'"
                        + Environment.NewLine
                        + "    path: \'LightupLib.dll\'"
                );
        }
        ...
        {
            var fixtureLib = sharedTestState.LightupLibFixture_Published.Copy();
            ...
            dotnet
                .Exec("exec", "--additional-deps", baseDir, appDll)
                .CaptureStdErr()
                .CaptureStdOut()
                .Execute()
                .Should()
        ...
        public void Muxer_activation_of_LightupApp_WithLightupLib_and_Roll_Backwards_From_Release_To_Release_Succeeds()
        {
            ...
            var fixtureApp = GlobalLightupClientFixture.Copy();
            ...
            string runtimeConfig = Path.Combine(
                fixtureApp.TestProject.OutputDirectory,
                "LightupClient.runtimeconfig.json"
            );
        ...
        {
            var fixture = GlobalLightupClientFixture.Copy();
            ...
            string additionalDepsPath = Path.Combine(
                additionalDepsRootPath,
                "shared",
                "Microsoft.NETCore.App",
                "9999.0.0",
                "myAdditionalDeps.deps.json"
            );
            ...
            dotnet
                .Exec("exec", "--additional-deps", additionalDepsRootPath, appDll)
                .EnableTracingAndCaptureOutputs()
                .Execute(expectedToFail: true)
                .Should()
                .Fail()
        ...
        public void SharedFx_With_Higher_Version_Wins_Against_Additional_Deps()
        {
            ...
            string netcoreAssembly = Path.Combine(
                _fxBaseDir,
                "9999.0.0",
            ...
            string uberAssembly = Path.Combine(
                _uberFxBaseDir,
                "7777.0.0",
                "System.Collections.Immutable.dll"
            );
            dotnet
                .Exec("exec", "--additional-deps", additionalDepsPath, appDll)
                .EnableTracingAndCaptureOutputs()
                .Execute()
                .Should()
                .Pass()
        ...
        )
        {
            ...
            JObject depsjson = SharedFramework.CreateDepsJson(
                "Microsoft.NETCore.App",
                "LightupLib/1.0.0",
                "LightupLib"
            );
            ...
            SharedFramework.AddReferenceToDepsJson(
                ...
                "1.0.0",
                immutableCollectionVersionInfo
            );
            SharedFramework.AddReferenceToDepsJson(
                depsFile,
                "LightupLib/1.0.0",
        ...
        )
        {
            ...
            var srcNewtonsoftPath = Path.Combine(
                Path.GetDirectoryName(libDll),
                "Newtonsoft.Json.dll"
            );
            var destNewtonsoftPath = Path.Combine(
