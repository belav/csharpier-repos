    ...
    internal static class FunctionPointerResultExtensions
    {
        ...
        )
        {
            return assertion.HaveStdOutContaining(
                $"Called {methodName}(0xdeadbeef, 42) - call count: {callCount}"
            );
        }

        public static FluentAssertions.AndConstraint<CommandResultAssertions> ExecuteInIsolatedContext(
            this CommandResultAssertions assertion,
            string assemblyName
        )
        {
            return assertion.HaveStdOutContaining(
                $"{assemblyName}: AssemblyLoadContext = \"IsolatedComponentLoadContext("
            );
        }

        public static FluentAssertions.AndConstraint<CommandResultAssertions> ExecuteInDefaultContext(
            this CommandResultAssertions assertion,
            string assemblyName
        )
        {
            return assertion.HaveStdOutContaining(
                $"{assemblyName}: AssemblyLoadContext = \"Default\" System.Runtime.Loader.DefaultAssemblyLoadContext"
            );
        }

        public static FluentAssertions.AndConstraint<CommandResultAssertions> ExecuteWithLocation(
            this CommandResultAssertions assertion,
            string assemblyName,
            string location
        )
