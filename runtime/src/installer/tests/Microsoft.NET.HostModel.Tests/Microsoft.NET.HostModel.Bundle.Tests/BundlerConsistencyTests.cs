...
namespace Microsoft.NET.HostModel.Tests
{
    public class BundlerConsistencyTests : IClassFixture<BundlerConsistencyTests.SharedTestState>
    {
        ...
        private static string BundlerHostName = Binaries.GetExeFileNameForCurrentPlatform(
            SharedTestState.AppName
        );

        private Bundler CreateBundlerInstance(
            ...
            new Bundler(
                ...
                SharedFramework.CalculateUniqueTestDirectory(
                    $"{sharedTestState.App.Location}-bundle"
                ),
                bundleOptions,
        ...
        public void BaseNameComputation()
        {
            ...
            {
                app.PopulateFrameworkDependent(
                ...
                var bundleDir = Directory.CreateDirectory(
                    SharedFramework.CalculateUniqueTestDirectory(
                        Path.Combine(app.Location, "bundle")
                    )
                );
                ...
                bundler
                    .BundleManifest.Files.Where(entry => entry.RelativePath.Equals(depsJsonName))
                    .Single()
                    .Type.Should()
        ...
        public void BundleOptions_IncludedExcludedFiles(BundleOptions options)
        {
            ...
            FileSpec[] fileSpecs = new FileSpec[]
            {
                ...
                new FileSpec(
                    app.RuntimeConfigJson,
                    Path.GetRelativePath(app.Location, app.RuntimeConfigJson)
                ),
