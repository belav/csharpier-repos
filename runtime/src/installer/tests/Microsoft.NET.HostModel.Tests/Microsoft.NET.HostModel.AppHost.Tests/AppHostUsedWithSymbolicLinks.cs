        ...
        : IClassFixture<AppHostUsedWithSymbolicLinks.SharedTestState>
    {
        ...
        public void Run_apphost_behind_symlink(string symlinkRelativePath)
        {
            using (var testDir = TestArtifact.Create("symlink"))
            {
                Directory.CreateDirectory(
                    Path.Combine(testDir.Location, Path.GetDirectoryName(symlinkRelativePath))
                );
                var symlinkFullPath = Path.Combine(testDir.Location, symlinkRelativePath);

                using var symlink = new SymLink(
                    symlinkFullPath,
                    sharedTestState.SelfContainedApp.AppExe
                );
                Command
                    .Create(symlinkFullPath)
                    .CaptureStdErr()
                    .CaptureStdOut()
                    .Execute()
                    .Should()
        ...
        )
        {
            using (var testDir = TestArtifact.Create("symlink"))
            {
                ...
                using var symlink2 = new SymLink(
                    symlink2Path,
                    sharedTestState.SelfContainedApp.AppExe
                );
        ...
        )
        {
            ...
            using (var testDir = TestArtifact.Create("symlink"))
            {
                ...
                using var symlink = new SymLink(
                    Path.Combine(testDir.Location, symlinkRelativePath),
                    sharedTestState.FrameworkDependentApp.AppExe
                );
                Command
                    .Create(symlink.SrcPath)
                    .CaptureStdErr()
                    .CaptureStdOut()
        ...
        public void Run_framework_dependent_app_with_runtime_behind_symlink()
        {
            using (var testDir = TestArtifact.Create("symlink"))
            {
                ...
                Command
                    .Create(sharedTestState.FrameworkDependentApp.AppExe)
        ...
        public void Put_dotnet_behind_symlink()
        {
            using (var testDir = TestArtifact.Create("symlink"))
            {
                ...
                using var symlink = new SymLink(
                    dotnetSymlink,
                    TestContext.BuiltDotNet.DotnetExecutablePath
                );
                Command
                    .Create(symlink.SrcPath, sharedTestState.SelfContainedApp.AppDll)
                    .CaptureStdErr()
                    .CaptureStdOut()
                    .Execute()
                    .Should()
        ...
        public void Put_satellite_assembly_behind_symlink()
        {
            ...
            var firstSatelliteDir = Directory
                .GetDirectories(binDir)
            ...
            var secondSatelliteDir = Directory
                .GetDirectories(binDir)
        ...
        public class SharedTestState : IDisposable
        {
            ...
            {
                var localizedFixture = new TestProjectFixture(
                    "LocalizedApp",
                    RepoDirectoriesProvider.Default
                );
                localizedFixture
