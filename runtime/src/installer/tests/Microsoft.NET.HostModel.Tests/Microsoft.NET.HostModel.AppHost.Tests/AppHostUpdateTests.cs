    ...
    public class AppHostUpdateTests
    {
        ...
        public void ItCanSetWindowsGUISubsystem()
        {
            using (TestDirectory testDirectory = TestDirectory.Create())
            {
                ...
                Assert.Equal(
                    (ushort)Subsystem.WindowsGui,
                    PEUtils.GetWindowsGraphicalUserInterfaceBit(destinationFilePath)
                );
            }
        }
        ...
        public void ItGeneratesExecutableImage()
        {
            ...
            // -rwxr-xr-x
            const UnixFileMode expectedPermissions =
            ...
            File.GetUnixFileMode(destinationFilePath).Should().Be(expectedPermissions);
        }
        ...
        public void CodeSigningFailuresThrow()
        {
            using (TestDirectory testDirectory = TestDirectory.Create())
            {
                ...
                Assert.True(
                    ...
                    $"AppHostSigningException.ExitCode - expected: 1, actual: '{exception.ExitCode}'"
                );
            }
        }
