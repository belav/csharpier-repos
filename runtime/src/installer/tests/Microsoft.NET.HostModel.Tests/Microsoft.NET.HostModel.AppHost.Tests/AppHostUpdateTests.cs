    ...
    public class AppHostUpdateTests
    {
        ...
        public void ItEmbedsAppBinaryPath()
        {
            using (TestDirectory testDirectory = TestDirectory.Create())
            {
                ...
                result.Skip(WindowsFileHeader.Length)
        ...
        private static class CoreFxFileStatusProvider
        {
            ...
            static CoreFxFileStatusProvider()
            {
                if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                {
                    ...
                    {
                        throw new Exception(
                            "Cannot setup _fileStatus via private reflection from CoreFX. Verify if the FileSystem._fileStatus._fileStatus.Mode chain is intact in CoreFX, otherwise adjust this implementation",
                            ex
                        );
                    }
                }
            ...
            public static int GetFileMode(string path)
            {
                ...
                {
                    throw new Exception(
                        "Cannot get stat (2) st_mode via private reflection from CoreFX. Verify if the FileSystem._fileStatus.Initialize logic is exercised via FileInfo.IsReadOnly in CoreFX, otherwise adjust this implementation.",
                        ex
                    );
                }
            }
