...
namespace Microsoft.NET.HostModel.Tests
{
    public class AppHostUpdateTests
    {
        ...
        private readonly static byte[] AppBinaryPathPlaceholderSearchValue = Encoding
            .UTF8
        ...
        public void ItEmbedsAppBinaryPath()
        {
            using (TestDirectory testDirectory = TestDirectory.Create())
            {
                ...
                HostWriter.CreateAppHost(sourceAppHostMock, destinationFilePath, appBinaryFilePath);
                ...
                BitConverter.ToUInt16(result, SubsystemOffset).Should().Be(3);
            }
        }
        ...
        public void ItGeneratesExecutableImage()
        {
            using (TestDirectory testDirectory = TestDirectory.Create())
            {
                ...
                GetLastError().Should().NotBe(4); // EINTR

                GetFilePermissionValue(sourceAppHostMock).Should().Be(Convert.ToInt32("755", 8));
                ...
                GetFilePermissionValue(destinationFilePath).Should().Be(Convert.ToInt32("755", 8));
            }
        ...
        public void CanCodeSignAppHostOnMacOS(string subdir)
        {
            using (TestDirectory testDirectory = TestDirectory.Create(subdir))
            {
                ...
                using (var p = Process.Start(psi))
                {
                    ...
                    Assert.True(
                        p.ExitCode == 0,
                        $"Expected exit code was '0' but '{codesign}' returned '{p.ExitCode}' instead."
                    );
                }
            }
        }
        ...
        )
        {
            ...
            Array.Copy(
                ...
                0,
                content,
                WindowsFileHeader.Length,
                AppBinaryPathPlaceholderSearchValue.Length
            );
        ...
        private static int GetFilePermissionValue(string path)
        {
            ...
            modeValue.Should().BeInRange(0, 511);
        ...
        {
            private static FieldInfo s_fileSystem_fileStatusField,
                s_fileStatus_fileCacheField,
                s_fileStatusModeField;
            ...
            static FileStatusProvider()
            {
                if (!OperatingSystem.IsWindows())
                {
                    try
                    {
                        ...
                        s_fileStatus_fileCacheField = s_fileSystem_fileStatusField
                            .FieldType
                            .GetField("_fileCache", BindingFlags.NonPublic | BindingFlags.Instance);
                        s_fileStatusModeField = s_fileStatus_fileCacheField
                            .FieldType
                    ...
                    {
                        throw new Exception(
                            "Cannot setup _fileStatus via private reflection from libraries. Verify if the FileSystem._fileStatus._fileCache.Mode chain is intact, otherwise adjust this implementation",
                            ex
                        );
                    }
                }
            ...
            public static int GetFileMode(string path)
            {
                ...
                {
                    throw new Exception(
                        "Cannot get stat (2) st_mode via private reflection from libraries. Verify if the FileSystem._fileStatus.Initialize logic is exercised via FileInfo.IsReadOnly in FileStatus.Unix.cs, otherwise adjust this implementation.",
                        ex
                    );
                }
            }
