...
namespace Microsoft.NET.HostModel.ComHost.Tests
{
    public class ClsidMapTests : IClassFixture<ClsidMapTests.SharedTestState>
    {
        ...
        public void PublicComVisibleTypeWithGuidAdded()
        {
            ...
            Assert.Equal(
                SharedTestState.ComVisibleTypeName,
                entry.Property("type").Value.ToString()
            );
            Assert.Equal(
                SharedTestState.ComVisibleAssemblyName,
                entry.Property("assembly").Value.ToString()
            );
        }
        ...
        public void PublicNestedTypeOfPublicTypeAdded()
        {
            ...
            JObject entry = (JObject)comVisibleEntry.Value;
            Assert.Equal(
                SharedTestState.ComVisibleNestedTypeName,
                entry.Property("type").Value.ToString()
            );
        }
        ...
        {
            JObject clsidMap = CreateClsidMap(sharedTestState.ComLibraryFixture);
            JProperty comVisibleEntry = clsidMap.Property(
                SharedTestState.ComVisibleNonPublicNestedGuid
            );
            Assert.Null(comVisibleEntry);
        }
        ...
        public void DefaultProgIdIsTypeName()
        {
            ...
            Assert.Equal(
                SharedTestState.ComVisibleTypeName,
                entry.Property("progid").Value.ToString()
            );
        }
        ...
        public void ExplicitProgIdUsed()
        {
            JObject clsidMap = CreateClsidMap(sharedTestState.ComLibraryFixture);
            JProperty comVisibleEntry = clsidMap.Property(
                SharedTestState.ComVisibleCustomProgIdGuid
            );
            Assert.NotNull(comVisibleEntry);
            JObject entry = (JObject)comVisibleEntry.Value;
            Assert.Equal(
                SharedTestState.ComVisibleCustomProgIdProgId,
                entry.Property("progid").Value.ToString()
            );
        }
        ...
        {
            public const string ComVisibleAssemblyName =
                "ComLibrary, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null";
            ...
            public const string ComVisibleNonPublicNestedGuid =
                "{8a0a7085-aca4-4651-9878-ca42747e2206}";
            public const string ComVisibleCustomProgIdGuid =
                "{f5ad253b-845e-4c91-95a7-3ff2fa0c91cd}";
            ...
            public SharedTestState()
            {
                ...
                ComLibraryMissingGuidFixture = new TestProjectFixture(
                    "ComLibraryMissingGuid",
                    RepoDirectories
                )
                    .EnsureRestored()
                    .BuildProject();

                ComLibraryConflictingGuidFixture = new TestProjectFixture(
                    "ComLibraryConflictingGuid",
                    RepoDirectories
                )
                    .EnsureRestored()
                    .BuildProject();
