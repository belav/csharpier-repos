        ...
        : IClassFixture<BundleExtractToSpecificPath.SharedTestState>
    {
        ...
        private void AbsolutePath()
        {
            ...
            bundleDir
                .Should()
                .OnlyHaveFiles(
                    new[] { Binaries.GetExeFileNameForCurrentPlatform(app.Name), $"{app.Name}.pdb" }
                );
            ...
            extractDir
                .Should()
                .OnlyHaveFiles(
                    BundleHelper.GetExtractedFiles(bundledApp.Manifest, bundledApp.Options)
                );
        }
        ...
        private void RelativePath(string relativePath, BundleOptions bundleOptions)
        {
            ...
            using (
                TestArtifact extractionRoot = new TestArtifact(
                    Path.Combine(Path.GetDirectoryName(singleFile), relativePath)
                )
            )
            {
                var extractedDir = sharedTestState.SelfContainedApp.GetExtractionDir(
                    extractionRoot.Location,
                    manifest
                );
        ...
        private void NonexistentDefault_Fails()
        {
            ...
            Command
                .Create(sharedTestState.BundledApp.Path)
                .CaptureStdErr()
                .CaptureStdOut()
        ...
        [SkipOnPlatform(
            ...
            "On Windows the default extraction path is determined by calling GetTempPath which looks at multiple places and can't really be undefined."
        )]
        private void UndefinedHOME_getpwuidFallback()
        {
            ...
            DirectoryInfo expectedExtractDir = sharedTestState.SelfContainedApp.GetExtractionDir(
        ...
        public class SharedTestState : IDisposable
        {
            ...
            public SharedTestState()
            {
                ...
                File.Copy(
                    ...
                    Path.Combine(SelfContainedApp.NonBundledLocation, Binaries.CoreClr.MockName)
                );
