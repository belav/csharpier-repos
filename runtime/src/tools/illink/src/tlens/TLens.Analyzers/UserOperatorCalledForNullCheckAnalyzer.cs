...
namespace TLens.Analyzers
{
    sealed class UserOperatorCalledForNullCheckAnalyzer : Analyzer
    {
        ...
        readonly Dictionary<MethodDefinition, Counters> operators =
            new Dictionary<MethodDefinition, Counters>();
        ...
        {
            var entries = operators
                .Where(l => l.Value.Ratio > 0)
                .OrderByDescending(l => l.Value.Ratio)
                .Take(maxCount);
            if (!entries.Any())
                return;
            ...
            {
                Console.WriteLine(
                    $"User operator '{e.Key.ToDisplay()}' was called {e.Value.Redundant} [{e.Value.Ratio.ToString("0%")}] times with null values"
                );
            }
        }
    }
