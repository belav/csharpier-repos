...
namespace TLens.Analyzers
{
    sealed class UnnecessaryFieldsAssignmentAnalyzer : Analyzer
    {
        ...
        readonly Dictionary<FieldDefinition, Access> fields =
            new Dictionary<FieldDefinition, Access>();
        readonly Dictionary<FieldDefinition, List<MethodDefinition>> writes =
            new Dictionary<FieldDefinition, List<MethodDefinition>>();
        ...
        {
            var static_entries = fields
                .Where(l => l.Value == Access.Write && l.Key.IsStatic)
