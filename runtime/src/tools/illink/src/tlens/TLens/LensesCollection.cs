    ...
    static class LensesCollection
    {
        ...
        static readonly LensAnalyzerDetails[] all = new[]
        {
            new LensAnalyzerDetails(
                "duplicated-code",
                "Methods which are possible duplicates",
                typeof(DuplicatedCodeAnalyzer)
            ),
            new LensAnalyzerDetails(
                "fields-init",
                "Constructors re-initializing fields to default values",
                typeof(RedundantFieldInitializationAnalyzer)
            ),
            new LensAnalyzerDetails(
                "fields-unread",
                "Fields that are set but never read",
                typeof(UnnecessaryFieldsAssignmentAnalyzer)
            ),
            new LensAnalyzerDetails(
                "ifaces-dispatch",
                "Interfaces which are called sparsely",
                typeof(InterfaceDispatchAnalyzer)
            ),
            new LensAnalyzerDetails(
                "ifaces-types",
                "Interfaces with implementation but no type reference",
                typeof(InterfaceTypeCheckAnalyzers)
            ),
            new LensAnalyzerDetails(
                "inverted-ctors",
                "Constructors calling same type constructor with default values",
                typeof(InverterCtorsChainAnalyzer)
            ),
            new LensAnalyzerDetails(
                "large-arrays",
                "Methods creating large arrays",
            ...
            new LensAnalyzerDetails(
                "large-cctors",
                "Types with large static contructor",
                typeof(LargeStaticCtorAnalyzer)
            ),
            new LensAnalyzerDetails(
                "large-strings",
                "Methods using large strings literals",
            ...
            new LensAnalyzerDetails(
                "operator-null",
                "User operators used for null check",
                typeof(UserOperatorCalledForNullCheckAnalyzer)
            ),
            new LensAnalyzerDetails(
                "single-calls",
                "Methods called sparsely",
                typeof(LimitedMethodCalls)
            ),
            new LensAnalyzerDetails(
                "single-construction",
                "Types with limited number of constructions",
            ...
            new LensAnalyzerDetails(
                ...
                typeof(UnusedParametersAnalyzer)
            ),
        };
        ...
        public static IEnumerable<Analyzer> DefaultAnalyzers =>
            all.Where(l => l.DefaultSet).OrderBy(l => l.Name).Select(l => l.CreateAnalyzer());

        public static Analyzer GetLensByName(string name) =>
            all.FirstOrDefault(l => l.Name == name)?.CreateAnalyzer();
    }
}
