...
namespace ILLink.CodeFix
{
    [
        ExportCodeFixProvider(
            ...
            Name = nameof(UnconditionalSuppressMessageCodeFixProvider)
        ),
        Shared
    ]
    public class UnconditionalSuppressMessageCodeFixProvider : BaseAttributeCodeFixProvider
    {
        ...
        const string UnconditionalSuppressMessageAttribute = nameof(
...
#pragma warning restore RS1035 // Do not use APIs banned for analyzers
            var ruleId = syntaxGenerator.AttributeArgument(
                syntaxGenerator.LiteralExpression(
                    string.IsNullOrWhiteSpace(ruleTitle)
                        ? diagnostic.Id
                        : $"{diagnostic.Id}:{ruleTitle}"
                )
            );
