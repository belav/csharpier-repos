...
{
    public abstract class BaseAttributeCodeFixProvider
        : Microsoft.CodeAnalysis.CodeFixes.CodeFixProvider
    {
        ...
        protected async Task BaseRegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            if (
                await document.GetSyntaxRootAsync(context.CancellationToken).ConfigureAwait(false)
                is not { } root
            )
                return;
            ...
            if (
                FindAttributableParent(targetNode, AttributableParentTargets)
                is not SyntaxNode attributableNode
            )
                return;
            ...
            context.RegisterCodeFix(
                CodeAction.Create(
                    ...
                    createChangedDocument: ct =>
                        AddAttributeAsync(document, diagnostic, targetNode, attributableNode, ct),
        ...
        )
        {
            ...
            if (model.GetSymbolInfo(targetNode, cancellationToken).Symbol is not { } targetSymbol)
                return document;
            if (
                model.Compilation.GetBestTypeByMetadataName(FullyQualifiedAttributeName)
                is not { } attributeSymbol
            )
                return document;
        ...
        private static CSharpSyntaxNode? FindAttributableParent(
            SyntaxNode node,
            AttributeableParentTargets targets
        )
        {
            ...
            while (parentNode is not null)
            {
                switch (parentNode)
                {
                    ...
                    or AccessorDeclarationSyntax
                        when targets.HasFlag(AttributeableParentTargets.MethodOrConstructor):
                    case PropertyDeclarationSyntax
                        when targets.HasFlag(AttributeableParentTargets.Property):
