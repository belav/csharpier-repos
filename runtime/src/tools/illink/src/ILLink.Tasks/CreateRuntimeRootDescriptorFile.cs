...
namespace ILLink.Tasks
{
    public class CreateRuntimeRootILLinkDescriptorFile : Task
    {
        ...
        readonly struct FeatureSwitchMembers : IEquatable<FeatureSwitchMembers>
        {
            ...
            public bool Equals(FeatureSwitchMembers fsm) => fsm._key == _key;
        }
        ...
        readonly Dictionary<string, ClassMembers> classNamesToClassMembers =
            new Dictionary<string, ClassMembers>();
        readonly Dictionary<
            FeatureSwitchMembers,
            Dictionary<string, ClassMembers>
        > featureSwitchMembers = new();
        ...
        void ProcessNamespaces(string namespaceFile)
        {
            ...
            foreach (string namespaceDef in namespaces)
            {
                ...
                {
                    char[] separators = { '"', ' ', '\t' };
                    string[] namespaceDefElements = namespaceDef.Split(
                        separators,
        ...
        void ProcessMscorlib(string typeFile)
        {
            ...
            foreach (string def in types)
            {
                ...
                if (def.StartsWith("BEGIN_ILLINK_FEATURE_SWITCH"))
                {
                    ...
                    {
                        Log.LogError(
                            $"Could not figure out feature switch status in '{typeFile}' for line {def}."
                        );
                    }
                    else
                    {
                        char[] separators = { ',', '(', ')', ' ', '\t', '/' };
                        string[] featureSwitchElements = def.Split(
                            separators,
                            StringSplitOptions.RemoveEmptyEntries
                        );
                        ...
                        {
                            Log.LogError(
                                $"BEGIN_ILLINK_FEATURE_SWITCH is not formatted correctly '{typeFile}' for line {def}."
                            );
                            return;
                        }
                        currentFeatureSwitch = new FeatureSwitchMembers(
                            featureSwitchElements[1],
                            featureSwitchElements[2],
                            featureSwitchElements[3]
                        );
                        if (!featureSwitchMembers.ContainsKey(currentFeatureSwitch.Value))
                        {
                            featureSwitchMembers.Add(
                                currentFeatureSwitch.Value,
                                new Dictionary<string, ClassMembers>()
                            );
                        }
                    }
        ...
        void OutputXml(string iLLinkTrimXmlFilePath, string outputFileName)
        {
            ...
            {
                foreach (
                    (var fs, var members) in featureSwitchMembers.Select(kv => (kv.Key, kv.Value))
                )
                {
                    if (members.Count == 0)
                        continue;
        ...
        )
        {
            ...
            if (
                !typeName.StartsWith("System.Runtime.InteropServices.WindowsRuntime")
        ...
        void AddClass(
            ...
            FeatureSwitchMembers? featureSwitch = null
        )
        {
            ...
            if (fullClassName != null)
            {
                ...
                {
                    Dictionary<string, ClassMembers> currentFeatureSwitchMembers =
        ...
        string GetFullClassName(string classNamespace, string className)
        {
            ...
            {
                classNamespace = classNamespace.Substring(
                    0,
                    classNamespace.Length - suffixToRemove.Length
                );
            }
        ...
        sealed class DefineTracker
        {
            ...
            public DefineTracker(
                HashSet<string> defineConstants,
            ...
            public bool ProcessLine(string line)
            {
                ...
                if (line.StartsWith("#else"))
                {
                    ...
                    {
                        log.LogError(
                            $"Could not figure out ifdefs in '{filePath}' around line {linenum}."
                        );
                    }
                    else
                    {
