        ...
        where TConditionValue : struct, INegate<TConditionValue>
    {
        ...
        internal struct ControlFlowGraphState
        {
            ...
            private readonly Dictionary<
                IControlFlowGraph<TBlock, TRegion>.ControlFlowBranch,
                TState
            > branchInput;
            ...
            public bool TryGetExceptionFinallyState(
                IControlFlowGraph<TBlock, TRegion>.ControlFlowBranch branch,
                out TValue state
            )
            {
                state = default;
        ...
        )
        {
            ...
            if (
                cfg.GetConditionalSuccessor(block)
                is IControlFlowGraph<TBlock, TRegion>.ControlFlowBranch conditionalBranch
            )
            {
            ...
            if (
                cfg.GetFallThroughSuccessor(block)
                is IControlFlowGraph<TBlock, TRegion>.ControlFlowBranch fallThroughBranch
            )
            {
        ...
        public void Fixpoint(TControlFlowGraph cfg, TTransfer transfer)
        {
                ...
                foreach (var block in cfg.Blocks)
                {
                    ...
                    if (isFinallyBlock)
                    {
                        ...
                        TransferOut(
                            ...
                            updateState: (branch, newValue) =>
                            {
                                bool result = cfgState.TryGetExceptionFinallyState(
                                    branch,
                                    out TValue value
                                );
                                Debug.Assert(result);
            ...
            )
            {
                foreach (var exitedFinally in predecessor.FinallyRegions)
                {
                    ...
                    IControlFlowGraph<TBlock, TRegion>.ControlFlowBranch? finallyExit =
                        cfg.GetFallThroughSuccessor(lastFinallyBlock);
