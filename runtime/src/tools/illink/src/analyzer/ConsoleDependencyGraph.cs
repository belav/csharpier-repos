...
namespace LinkerAnalyzer
{
    public class ConsoleDependencyGraph : DependencyGraph
    {
        ...
        void ShowFlatDependencies(VertexData vertex)
        {
            ...
            foreach (var d in flatDeps)
            {
                ...
                if (first)
                {
                    ...
                    Console.WriteLine(
                        $"Distance | {d.Item1.value} [total deps: {flatDeps.Count}]{sizeStr}"
                    );
                    Line();
                    first = false;
                    continue;
                ...
                Console.WriteLine(
                    $"{string.Format("{0,8}", d.Item2)} | {d.Item1.value}{d.Item1.DepsCount}{sizeStr2}"
                );
            }
        }
                ...
                foreach (int index in vertex.parentIndexes)
                {
                    ...
                    while (childVertex.parentIndexes != null)
                    {
                        ...
                        do
                        {
                            ...
                            visited.Contains(childIdx) && pi < childVertex.parentIndexes.Count
                        );
        ...
        public static void Header(string header, params object[] values)
        {
            ...
            Console.WriteLine();
            Console.WriteLine(
                $"--- {formatted} {new string('-', Math.Max(3, 67 - formatted.Length))}"
            );
        }
    }
}
