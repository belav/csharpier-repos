...
#pragma warning disable RS0030 // MethodReference.Parameters is banned
    ...
    readonly struct UnsafeAccessorMarker(LinkContext context, MarkStep markStep)
    {
        ...
        public void ProcessUnsafeAccessorMethod(MethodDefinition method)
        {
            ...
            {
                if (
                    customAttribute.Constructor.DeclaringType.FullName
                    == "System.Runtime.CompilerServices.UnsafeAccessorAttribute"
                )
                {
                    if (
                        customAttribute.HasConstructorArguments
                        && customAttribute.ConstructorArguments[0].Value is int kindValue
                    )
                    {
                        ...
                        {
                            foreach (
                                CustomAttributeNamedArgument prop in customAttribute.Properties
                            )
                            {
                                if (prop.Name == "Name")
                                {
        ...
        void ProcessConstructorAccessor(MethodDefinition method, string? name)
        {
            ...
            if (
                method.ReturnsVoid()
                || method.ReturnType.IsByRefOrPointer()
            ...
            foreach (MethodDefinition targetMethod in targetType.Methods)
            {
                ...
                _markStep.MarkMethodVisibleToReflection(
                    targetMethod,
                    new DependencyInfo(DependencyKind.UnsafeAccessorTarget, method),
