...
namespace Mono.Linker.Steps
{
    public class LinkAttributesParser : ProcessLinkerXmlBase
    {
        ...
        static bool IsRemoveAttributeInstances(string attributeName) =>
            attributeName == "RemoveAttributeInstances"
            || attributeName == "RemoveAttributeInstancesAttribute";

        (CustomAttribute[]? customAttributes, MessageOrigin[]? origins) ProcessAttributes(
            XPathNavigator nav,
            ...
            foreach (XPathNavigator attributeNav in nav.SelectChildren("attribute", string.Empty))
            {
                ...
                if (!string.IsNullOrEmpty(internalAttribute))
                {
                    if (!IsRemoveAttributeInstances(internalAttribute))
                    {
                        LogWarning(
                            attributeNav,
                            DiagnosticId.UnrecognizedInternalAttribute,
                            internalAttribute
                        );
                        continue;
                    }
                    if (provider is not TypeDefinition)
                    {
                        LogWarning(
                            ...
                            nameof(RemoveAttributeInstancesAttribute)
                        );
                        continue;
                    }
        ...
        TypeDefinition? GenerateRemoveAttributeInstancesAttribute()
        {
            ...
            if (
                _context.MarkedKnownMembers.RemoveAttributeInstancesAttributeDefinition
                is TypeDefinition knownTypeDef
            )
            {
                return knownTypeDef;
            }
        ...
        )
        {
            ...
            for (int i = 0; i < attributeType.Methods.Count; ++i)
            {
                ...
                foreach (var p in method.GetMetadataParameters())
                {
                    ...
                    if (
                        parameterType == null
                        || parameterType != _context.TryResolve(args[p.MetadataIndex].Type)
                    )
                        match = false;
                }
            ...
            switch (typeref.MetadataType)
            {
                ...
                case MetadataType.Class:
                    ...
                    var diagnosticContext = new DiagnosticContext(
                        new MessageOrigin(memberWithAttribute),
                ...
                case MetadataType.Array:
                    if (typeref is ArrayType arrayTypeRef)
                    {
                        ...
                        ArrayBuilder<CustomAttributeArgument> elements =
                            new ArrayBuilder<CustomAttributeArgument>();
                        foreach (XPathNavigator elementNav in arrayArgumentIterator)
                        {
                            if (
                                ReadCustomAttributeArgument(elementNav, memberWithAttribute)
                                is CustomAttributeArgument arg
                            )
                            {
