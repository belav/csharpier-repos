...
namespace Mono.Linker.Steps
{
    ...
    public class UnreachableBlocksOptimizer
    {
        ...
        public void ProcessMethod(MethodDefinition method)
        {
            ...
            try
            {
                ...
                if (reducer.RewriteBody())
                    _context.LogMessage(
                        $"Reduced '{reducer.InstructionsReplaced}' instructions in conditional branches for [{method.DeclaringType.Module.Assembly.Name}] method '{method.GetDisplayName()}'."
                    );
            ...
            catch (Exception e)
            {
                throw new InternalErrorException(
                    $"Could not process the body of method '{method.GetDisplayName()}'.",
                    e
                );
            }
        }
        ...
        static bool HasJumpIntoTargetRange(
            ...
            int firstInstr,
            int lastInstr,
            Func<Instruction, int>? mapping = null
        )
        {
            foreach (var instr in instructions)
            {
                switch (instr.OpCode.FlowControl)
                {
                    ...
                    case FlowControl.Cond_Branch:
                        ...
                        {
                            int index =
                                mapping == null ? instructions.IndexOf(target) : mapping(target);
        ...
        MethodResult? AnalyzeMethodForConstantResult(
            in CalleePayload callee,
            Stack<MethodDefinition> callStack
        )
        {
            ...
            switch (_context.Annotations.GetAction(method))
            {
                ...
                case MethodAction.ConvertToStub:
                    ...
                    return constant == null
                        ? null
                        : new MethodResult(constant, !HasSideEffects(method));
            }
        ...
        MethodResult? TryGetMethodCallResult(
            in CalleePayload callee,
            Stack<MethodDefinition> callStack
        )
        {
            ...
            {
                if (
                    !_cache_method_results.TryGetValue(method, out value) && !IsDeepStack(callStack)
                )
                {
        ...
        Instruction? GetSizeOfResult(TypeReference type)
        {
            ...
            return TryGetMethodCallResult(
                new CalleePayload(sizeOfImpl, Array.Empty<Instruction>())
            )?.Instruction;
        }
            ...
            if (method.DeclaringType.MetadataType == MetadataType.String)
            {
                switch (method.Name)
                {
                    ...
                    case "Concat":
                        ...
                        if (
                            !GetConstantValue(arguments[0], out left)
                        ...
                        if (left is string sleft && right is string sright)
                        {
                            ...
                            bool result =
                                method.Name.Length == 11 ? sleft == sright : sleft != sright;
        ...
        static Instruction[]? GetArgumentsOnStack(
        ...
        )
        {
            ...
            if (
                ...
                && HasJumpIntoTargetRange(
                    instructions,
                    index - method.GetMetadataParametersCount() + 1,
                    index
                )
            )
                return null;
        ...
        static MethodDefinition? FindSizeMethod(TypeDefinition? type)
        {
            ...
            return type.Methods.First(
                l => !l.HasMetadataParameters() && l.IsStatic && l.Name == "get_Size"
            );
        }
...
#pragma warning restore RS0030
            ...
            static bool IsCalledWithoutSideEffects(
                MethodDefinition method,
                Collection<Instruction> instructions,
                int index
            )
...
#pragma warning restore RS0030
                ...
                switch (instr.OpCode.Code)
                {
                    ...
                    case Code.Brfalse_S:
                        if (operand == 0)
                        {
                            Rewrite(
                                index,
                                Instruction.Create(OpCodes.Br, (Instruction)instr.Operand)
                            );
                        }
                        else
                        {
                    ...
                    case Code.Switch:
                        ...
                        if (operand < targets.Length)
                        {
                            ...
                            RewriteConditionTo(
                                index,
                                Instruction.Create(
                                    operand == 0 ? OpCodes.Brfalse : OpCodes.Brtrue,
                                    targets[operand]
                                )
                            );
            ...
            public void RewriteToNop(int index, int stackDepth)
            {
                ...
                for (start_index = index; start_index >= 0 && stackDepth > 0; --start_index)
                {
                    stackDepth -= GetStackBehaviourDelta(
                        FoldedInstructions[start_index],
                        out bool undefined
                    );
                    if (undefined)
                        return;
            ...
            public bool RewriteBody()
            {
                ...
                bodySweeper.Process(
                    conditionInstrsToRemove,
                    conditionInstrsToReplace,
                    out var nopInstructions
                );
                ...
                for (int i = 0; i < instructions.Count; ++i)
                {
                    ...
                    switch (instr.OpCode.Code)
                    {
                        ...
                        case Code.Callvirt:
                            ...
                            if (
                                md.IsVirtual
                                || md.CallingConvention == MethodCallingConvention.VarArg
                            )
                                break;

                            Instruction[]? args = GetArgumentsOnStack(
                                md,
                                FoldedInstructions ?? instructions,
                                i
                            );
                            targetResult =
                                args?.Length > 0 && md.IsStatic
                                    ? EvaluateIntrinsicCall(md, args)
                                    : null;

                            targetResult ??= optimizer
                                .TryGetMethodCallResult(new CalleePayload(md, args))
                                ?.Instruction;
                        ...
                        case Code.Ldsfld:
                            ...
                            {
                                targetResult = CodeRewriterStep.CreateConstantResultInstruction(
                                    context,
                                    field.FieldType,
                                    value
                                );
            ...
            void RemoveUnreachableInstructions(BitArray reachable)
            {
                ...
                for (int i = 0; i < reachable.Count; ++i)
                {
                    ...
                    if (
                        ...
                        && index > 0
                        && IsConditionalBranch(Instructions[index - 1].OpCode)
                    )
                    {
                ...
                for (int i = 0; i < FoldedInstructions.Count; ++i)
                {
                    ...
                    if (opcode.FlowControl == FlowControl.Cond_Branch)
                    {
                        ...
                        if (opcode.StackBehaviourPop == StackBehaviour.Popi)
                        {
                            if (
                                i > 0
                                && GetConstantValue(FoldedInstructions[i - 1], out var operand)
                            )
                            {
                                ...
                                if (
                                    operand is null
                                    && (
                                        opcode.Code == Code.Brfalse || opcode.Code == Code.Brfalse_S
                                    )
                                )
                                {
                                    if (IsJumpTargetRange(i, i))
                            ...
                            if (
                                i >= 3
                                && GetConstantValue(FoldedInstructions[i - 3], out operand)
                                && operand is int opint2
                                && IsPairedStlocLdloc(
                                    FoldedInstructions[i - 2],
                                    FoldedInstructions[i - 1]
                                )
                            )
                            {
                            ...
                            if (
                                ...
                                && GetConstantValue(FoldedInstructions[i - 5], out operand)
                                && operand is int opint3
                                && IsPairedStlocLdloc(
                                    FoldedInstructions[i - 4],
                                    FoldedInstructions[i - 3]
                                )
                            )
                            {
                                ...
                                if (
                                    !GetConstantValue(FoldedInstructions[i - 2], out operand)
                                    || operand is not int offset
                                )
                                    continue;
                ...
                switch (first.OpCode.Code)
                {
                    ...
                    case Code.Stloc:
                        if (second.OpCode.Code == Code.Ldloc_S || second.OpCode.Code == Code.Ldloc)
                            return ((VariableDefinition)first.Operand).Index
                                == ((VariableDefinition)second.Operand).Index;
...
#pragma warning restore RS0030
            ...
            public BodySweeper(
                MethodBody body,
                BitArray reachable,
                List<ExceptionHandler>? unreachableEH,
                LinkContext context
            )
            {
                this.body = body;
            ...
            public void Process(
                List<int>? conditionInstrsToRemove,
                List<(int, Instruction)>? conditionInstrsToReplace,
                out List<Instruction>? sentinelNops
            )
            {
            ...
            void CleanRemovedVariables(List<VariableDefinition> variables)
            {
                ...
                foreach (var variable in variables)
                {
                    ...
                    {
                        var objectType = BCL.FindPredefinedType(
                            WellKnownType.System_Object,
                            context
                        );
                        body_variables[index].VariableType =
                            objectType
                            ?? throw new NotSupportedException(
                                "Missing predefined 'System.Object' type"
                            );
                    }
                }
            }
                ...
                for (int i = 0; i < instructions.Count; ++i)
                {
                    ...
                    switch (instr.OpCode.Code)
                    {
                        ...
                        case Code.Ldsfld:
                        {
                            ...
                            if (context.Annotations.TryGetFieldUserValue(field, out object? value))
                            {
                                linstr = CodeRewriterStep.CreateConstantResultInstruction(
                                    context,
                                    field.FieldType,
                                    value
                                );
                        ...
                        case Code.Ceq:
                        {
                            ...
                            {
                                PushOnStack(
                                    Instruction.Create(OpCodes.Ldc_I4, lint == rint ? 1 : 0)
                                );
                                continue;
                            }
                            ...
                            {
                                PushOnStack(
                                    Instruction.Create(OpCodes.Ldc_I4, llong == rlong ? 1 : 0)
                                );
                                continue;
                            }
                        ...
                        case Code.Callvirt:
                        {
                            ...
                            MethodResult? call_result = optimizer.TryGetMethodCallResult(
                                new CalleePayload(md, args),
                                callStack
                            );
...
#pragma warning restore RS0030
                ...
                return CodeRewriterStep.CreateConstantResultInstruction(
                    context,
            ...
            bool GetOperandsConstantValues(
        ...
        readonly record struct CalleePayload(
            MethodDefinition Method,
