...
#pragma warning restore RS0030
        ...
        protected virtual void RewriteBodyToLinkedAway(MethodDefinition method)
        {
            method.ImplAttributes &= ~(
                MethodImplAttributes.AggressiveInlining | MethodImplAttributes.Synchronized
            );
...
#pragma warning disable RS0030 // MethodReference.Parameters is banned. This code already works and doesn't need to be changed
            if (method.HasParameters && method.Parameters.Any(l => l.IsOut))
                throw new NotSupportedException(
                    $"Cannot replace body of method '{method.GetDisplayName()}' because it has an out parameter."
                );
#pragma warning restore RS0030
            ...
            if (method.IsInstanceConstructor() && !method.DeclaringType.IsValueType)
            {
                ...
                if (base_ctor == null)
                    throw new NotSupportedException(
                        $"Cannot replace constructor for '{method.DeclaringType}' when no base default constructor exists"
                    );
...
#pragma warning restore RS0030
        ...
        public static Instruction? CreateConstantResultInstruction(
            LinkContext context,
