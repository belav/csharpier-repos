...
namespace Mono.Linker.Steps
{
    ...
    {
        public CheckSuppressionsDispatcher()
            : base(new List<ISubStep> { new CheckSuppressionsStep() }) { }
        ...
        public override void Process(LinkContext context)
        {
            ...
            redundantSuppressions = redundantSuppressions
                .Where(suppression =>
                    ((DiagnosticId)suppression.SuppressMessageInfo.Id).GetDiagnosticCategory()
                    == DiagnosticCategory.Trimming
                )
                .Where(suppression =>
                    ((DiagnosticId)suppression.SuppressMessageInfo.Id)
            ...
            bool ProviderIsMarked(ICustomAttributeProvider provider)
            {
                if (provider is PropertyDefinition property)
                {
                    return (
                            ...
                            && context.Annotations.IsMarked(property.GetMethod)
                        )
                        || (
                            property.SetMethod != null
                            && context.Annotations.IsMarked(property.SetMethod)
                        );
                }
                ...
                if (provider is EventDefinition @event)
                {
                    return (
                            ...
                            && context.Annotations.IsMarked(@event.AddMethod)
                        )
                        || (
