...
namespace Mono.Linker.Steps
{
    public class DiscoverOperatorsHandler : IMarkHandler
    {
        ...
        Dictionary<TypeDefinition, List<MethodDefinition>> PendingOperatorsForType
        {
            get
            {
                _pendingOperatorsForType ??=
                    new Dictionary<TypeDefinition, List<MethodDefinition>>();
        ...
        TypeDefinition? NonNullableType(TypeReference type)
        {
            ...
            Debug.Assert(
                nullableType != null
                    && nullableType.HasGenericArguments
                    && nullableType.GenericArguments.Count == 1
            );
            ...
            switch (operatorName)
            {
                ...
                case "False":
                    ...
                    if (
                        ...
                        || NonNullableType(method.GetParameter((ParameterIndex)0).ParameterType)
                            != self
                    )
                        return false;
                    // ++ and -- must return the declaring type
                    if (
                        operatorName is "Increment" or "Decrement"
                        && NonNullableType(method.ReturnType) != self
                    )
                        return false;
                    return true;
                // Binary operators
                ...
                case "GreaterThanOrEqual":
                    if (method.GetMetadataParametersCount() != 2)
                        return false;
                    var nnLeft = NonNullableType(
                        method.GetParameter((ParameterIndex)0).ParameterType
                    );
                    var nnRight = NonNullableType(
                        method.GetParameter((ParameterIndex)1).ParameterType
                    );
                    if (nnLeft == null || nnRight == null)
                        return false;
                    // << and >> must take the declaring type and int
                    if (
                        operatorName is "LeftShift" or "RightShift"
                        && (nnLeft != self || nnRight.MetadataType != MetadataType.Int32)
                    )
                        return false;
