...
namespace Mono.Linker
{
    internal sealed class TypeNameResolver
    {
        ...
        public readonly record struct TypeResolutionRecord(
            AssemblyDefinition ReferringAssembly,
        ...
        )
        {
            ...
            if (parsedTypeName is AssemblyQualifiedTypeName assemblyQualifiedTypeName)
            {
                ...
                typeReference = ResolveTypeName(
                    typeAssembly,
                    assemblyQualifiedTypeName.TypeName,
                    typeResolutionRecords
                );
            ...
            bool TryResolveTypeName(
                AssemblyDefinition assemblyDefinition,
                TypeName? typeName,
                [NotNullWhen(true)] out TypeReference? typeReference,
                List<TypeResolutionRecord> typeResolutionRecords
            )
            {
                typeReference = null;
        ...
        )
        {
            ...
            typeReference = ResolveTypeName(
                assembly,
                TypeParser.ParseTypeName(typeNameString),
                typeResolutionRecords
            );
        ...
        )
        {
            if (typeName is AssemblyQualifiedTypeName assemblyQualifiedTypeName)
            {
                ...
                var assemblyFromName = _context.TryResolve(
                    assemblyQualifiedTypeName.AssemblyName.Name
                );
                return assemblyFromName == null
                    ? null
                    : ResolveTypeName(
                        assemblyFromName,
                        assemblyQualifiedTypeName.TypeName,
            ...
            {
                var genericTypeRef = ResolveTypeName(
                    assembly,
                    constructedGenericTypeName.GenericType,
                    typeResolutionRecords
                );
            ...
            {
                var elementType = ResolveTypeName(
                    assembly,
            ...
            TypeDefinition? resolvedType = assembly.MainModule.ResolveType(
                typeName.ToString(),
                _context
            );

            // True type references (like generics and arrays) don't count as actually resolved types, they're
            // just wrappers
