...
namespace Mono.Linker
{
    ...
    public class SerializationMarker
    {
        ...
        Dictionary<SerializerKind, HashSet<ICustomAttributeProvider>> TrackedRoots
        {
            ...
            {
                _trackedRoots ??=
                    new Dictionary<SerializerKind, HashSet<ICustomAttributeProvider>>();
        ...
        public bool IsActive(SerializerKind serializerKind) =>
            ActiveSerializers.HasFlag(serializerKind);

        static DependencyKind ToDependencyKind(SerializerKind serializerKind) =>
        ...
        public void TrackForSerialization(
        ...
        {
            if (
                !Enum.IsDefined<SerializerKind>(serializerKind)
        ...
        public void MarkRecursiveMembers(
            ICustomAttributeProvider provider,
            SerializerKind serializerKind
        )
        {
            ...
            switch (provider)
            {
                ...
                default:
                    throw new ArgumentException(
                        $"{nameof(provider)} has invalid provider type {provider.GetType()}"
                    );
            }
        ...
        void MarkRecursiveMembersInternal(TypeReference typeRef, in DependencyInfo reason)
        {
            ...
            while (typeRef is GenericInstanceType git)
            {
                if (git.HasGenericArguments)
                {
                    foreach (var argType in git.GenericArguments)
                        MarkRecursiveMembersInternal(
                            argType,
                            new DependencyInfo(DependencyKind.GenericArgumentType, typeRef)
                        );
                }
            ...
            if (type.HasProperties)
            {
                foreach (var property in type.Properties)
                {
                    ...
                    if (
                        (get == null || !get.IsPublic || get.IsStatic)
