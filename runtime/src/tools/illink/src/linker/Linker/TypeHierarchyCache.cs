...
namespace Mono.Linker
{
    sealed class TypeHierarchyCache
    {
        ...
        readonly Dictionary<TypeDefinition, HierarchyFlags> _cache =
            new Dictionary<TypeDefinition, HierarchyFlags>();
        ...
        private HierarchyFlags GetFlags(TypeDefinition resolvedType)
        {
            ...
            while (baseType != null)
            {
                ...
                if (baseType.HasInterfaces)
                {
                    ...
                    {
                        if (
                            iface.InterfaceType.Name == "IReflect"
                            && iface.InterfaceType.Namespace == "System.Reflection"
                        )
                        {
                            flags |= HierarchyFlags.IsSystemReflectionIReflect;
                        }
