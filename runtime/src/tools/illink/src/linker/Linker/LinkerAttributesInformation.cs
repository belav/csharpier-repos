...
namespace Mono.Linker
{
    readonly struct LinkerAttributesInformation
    {
        ...
        private static bool TryFindAttributeList(
            List<(Type Type, List<Attribute> Attributes)> list,
            Type type,
            [NotNullWhen(returnValue: true)] out List<Attribute>? foundAttributes
        )
        {
        ...
        public static LinkerAttributesInformation Create(
            LinkContext context,
            IMemberDefinition provider
        )
        {
            ...
            foreach (var customAttribute in context.CustomAttributes.GetCustomAttributes(provider))
            {
                ...
                switch (attributeType.Name)
                {
                    case "RequiresUnreferencedCodeAttribute"
                        when attributeType.Namespace == "System.Diagnostics.CodeAnalysis":
                        attributeValue = ProcessRequiresUnreferencedCodeAttribute(
                            context,
                            provider,
                            customAttribute
                        );
                    ...
                    case "DynamicDependencyAttribute"
                        when attributeType.Namespace == "System.Diagnostics.CodeAnalysis":
                        attributeValue = DynamicDependency.ProcessAttribute(
                            context,
                            provider,
                            customAttribute
                        );
                    ...
                    case "RemoveAttributeInstancesAttribute":
                        ...
                        if (
                            context.Annotations.TryGetPreserve(td, out TypePreserve preserve)
                ...
                else if (!allowMultiple)
                {
                    context.LogWarning(
                        provider,
                        DiagnosticId.AttributeShouldOnlyBeUsedOnceOnMember,
                        attributeValueType.FullName ?? "",
                        (provider is MemberReference memberRef)
                            ? memberRef.GetDisplayName()
                            : provider.FullName
                    );
                    continue;
                }
        ...
        static RequiresUnreferencedCodeAttribute? ProcessRequiresUnreferencedCodeAttribute(
            LinkContext context,
        ...
        )
        {
            ...
            if (
                ...
                && customAttribute.ConstructorArguments[0].Value is string message
            )
            {
            ...
            context.LogWarning(
                ...
                typeof(RequiresUnreferencedCodeAttribute).FullName ?? ""
            );
            return null;
        }
    }
