...
namespace Mono.Linker
{
    ...
    {
        static EmbeddedResource? GetEmbeddedXml(
            AssemblyDefinition assembly,
            Func<Resource, bool> predicate
        )
        {
            return assembly
                    .Modules.SelectMany(mod => mod.Resources)
        ...
        public void ProcessDescriptors(AssemblyDefinition assembly, LinkContext context)
        {
            ...
            {
                context.LogMessage(
                    $"Processing embedded trimming descriptor '{rsc.Name}' from '{assembly.Name}'."
                );
        ...
        public SubstitutionInfo? ProcessSubstitutions(
            AssemblyDefinition assembly,
            LinkContext context
        )
        {
            ...
            {
                context.LogMessage(
                    $"Processing embedded substitution descriptor '{rsc.Name}' from '{assembly.Name}'."
                );
        ...
        static bool ShouldProcessRootDescriptorResource(
            AssemblyDefinition assembly,
            LinkContext context,
            string resourceName
        )
        ...
        protected virtual DescriptorMarker GetExternalResolveStep(
            LinkContext context,
            EmbeddedResource resource,
            AssemblyDefinition assembly
        )
        {
            return new DescriptorMarker(
                context,
                resource.GetResourceStream(),
                resource,
                assembly,
                "resource " + resource.Name + " in " + assembly.FullName
            );
        }

        protected virtual BodySubstitutionParser GetExternalSubstitutionParser(
            LinkContext context,
            EmbeddedResource resource,
            AssemblyDefinition assembly
        )
        {
            return new BodySubstitutionParser(
                context,
                resource.GetResourceStream(),
                resource,
                assembly,
        ...
        protected virtual LinkAttributesParser GetExternalLinkAttributesParser(
            LinkContext context,
            EmbeddedResource resource,
            AssemblyDefinition assembly
        )
        {
            return new LinkAttributesParser(
                context,
                resource.GetResourceStream(),
                resource,
                assembly,
