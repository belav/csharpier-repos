...
namespace Mono.Linker
{
    public class WarningSuppressionWriter
    {
        private readonly Dictionary<
            ...
            HashSet<(int Code, IMemberDefinition Member)>
        > _warnings;
        private readonly FileOutputKind _fileOutputKind;
        readonly LinkContext _context;

        public WarningSuppressionWriter(
            LinkContext context,
        ...
        public void AddWarning(int code, ICustomAttributeProvider provider)
        {
            ...
            if (
                ...
                is not ModuleDefinition module
            )
                return;
        ...
        void OutputSuppressionsXmlFormat(AssemblyNameDefinition assemblyName, string directory)
        {
            ...
            var xmlAssembly = new XElement(
                "assembly",
                new XAttribute("fullname", assemblyName.FullName)
            );
                ...
                xmlAssembly.Add(
                    new XElement(
                        ...
                        new XElement(
                            ...
                            new XAttribute(
                                "name",
                                UnconditionalSuppressMessageAttributeState.ScopeProperty
                            ),
                        ...
                        new XElement(
                            ...
                            new XAttribute(
                                "name",
                                UnconditionalSuppressMessageAttributeState.TargetProperty
                            ),
        ...
        void OutputSuppressionsCSharpFormat(AssemblyNameDefinition assemblyName, string directory)
        {
            using (
                var sw = new StreamWriter(
                    Path.Combine(directory, $"{assemblyName.Name}.WarningSuppressions.cs")
                )
            )
            {
                StringBuilder sb = new StringBuilder("using System.Diagnostics.CodeAnalysis;")
                    .AppendLine()
                    .AppendLine();
                foreach (var warning in GetListOfWarnings(assemblyName))
                {
                    sb.Append("[assembly: UnconditionalSuppressMessage (\"")
                        ...
                        .Append("\", Target = \"")
                        .Append(warning.MemberDocumentationSignature)
                        .AppendLine("\")]");
                }
        ...
        List<(int Code, string MemberDocumentationSignature)> GetListOfWarnings(
            AssemblyNameDefinition assemblyName
        )
        {
            List<(int Code, string MemberDocumentationSignature)> listOfWarnings =
                new List<(int Code, string MemberDocumentationSignature)>();
