...
namespace Mono.Linker.Dataflow
{
    ...
    abstract partial class MethodBodyScanner
    {
        ...
        protected MethodBodyScanner(LinkContext context)
        {
            ...
            this.InterproceduralStateLattice = new InterproceduralStateLattice(
                default,
                default,
                context
            );
            ...
            foreach (var keyValuePair in locals)
            {
                ...
                foreach (var val in localValue)
                {
                    if (
                        ...
                        && localReference.ReferencedType.IsByReference
                    )
                    {
                        string displayName =
        ...
        void TrackNestedFunctionReference(
            ...
            foreach (Instruction operation in methodIL.Instructions)
            {
                ...
                if (knownStacks.ContainsKey(operation.Offset))
                {
                    if (currentStack == null)
                    {
                        ...
                        currentStack = new Stack<StackSlot>(
                            knownStacks[operation.Offset].Reverse()
                        );
                    }
                    else
                    {
                ...
                switch (operation.OpCode.Code)
                {
                    ...
                    case Code.Ldftn:
                        TrackNestedFunctionReference(
                            (MethodReference)operation.Operand,
                    ...
                    case Code.Brtrue_S:
                        ...
                        NewKnownStack(
                            knownStacks,
                            ((Instruction)operation.Operand).Offset,
                            currentStack
                        );
                        break;
                    ...
                    case Code.Ret:
                    {
                        ...
                        if (hasReturnValue)
                        {
                            ...
                            ValidateNoReferenceToReference(
                                locals,
                                methodBody.Method,
                                operation.Offset
                            );
        ...
        )
        {
            ...
            bool isByRef =
                operation.OpCode.Code == Code.Ldloca || operation.OpCode.Code == Code.Ldloca_S;
        ...
        void ScanLdtoken(Instruction operation, Stack<StackSlot> currentStack)
        {
            switch (operation.Operand)
            {
                    ...
                    )
                    {
                        switch (instance.GenericArguments[0])
                        {
                            case GenericParameter genericParam:
                                ...
                                return;
                            case TypeReference underlyingTypeReference
                                when ResolveToTypeDefinition(underlyingTypeReference)
                                    is TypeDefinition underlyingType:
                ...
                case MethodReference methodReference
                    when _context.TryResolve(methodReference) is MethodDefinition resolvedMethod:
                ...
                switch (value)
                {
                    case LocalVariableReferenceValue localReference:
                        StoreMethodLocalValue(
                            locals,
                            source,
                            localReference.LocalDefinition,
                            curBasicBlock
                        );
                        break;
        ...
        private static VariableDefinition GetLocalDef(
            Instruction operation,
            Collection<VariableDefinition> localVariables
        )
        {
            Code code = operation.OpCode.Code;
            ...
            foreach (var value in maybeReferenceValue)
            {
                switch (value)
                {
                    ...
                    case LocalVariableReferenceValue localVariableReferenceValue:
                        if (
                            locals.TryGetValue(
                                localVariableReferenceValue.LocalDefinition,
                                out var valueBasicBlockPair
                            )
                        )
                            dereferencedValue = MultiValue.Meet(
                                dereferencedValue,
                                valueBasicBlockPair.Value
                            );
                        else
                            dereferencedValue = MultiValue.Meet(
                                dereferencedValue,
        ...
        )
        {
            ...
            else
            {
                ...
                foreach (
                    var (argument, refKind) in methodArguments.Zip(
                        calledMethod.GetParameterReferenceKinds()
                    )
                )
                {
        ...
        public TypeDefinition? ResolveToTypeDefinition(TypeReference typeReference) =>
            typeReference.ResolveToTypeDefinition(_context);
