...
namespace Mono.Linker.Dataflow
{
    ...
    public class CompilerGeneratedState
    {
                            ...
                            case OperandType.InlineMethod:
                                {
                                    ...
                                    )
                                    {
                                        ...
                                        {
                                            var alreadyAssociatedMethod = generatedTypeToTypeArgs[
                                                generatedType
                                            ].CreatingMethod;
                                            _context.LogWarning(
                        ...
                        if (typeArg is GenericParameter { Owner: { } owner } param)
                        {
                            ...
                            else
                            {
                                ...
                                )
                                {
                                    ...
                                    context.TryResolve((TypeReference)param.Owner) is { } owningType
                                )
                                {
                                    MapGeneratedTypeTypeParameters(
                                        owningType,
                        ...
                        if (instr.Operand is GenericInstanceMethod gim)
                        {
                            foreach (var tr in gim.GenericArguments)
                            {
                                if (
                                    ...
                                    && compilerGeneratedType == context.TryResolve(git)
                                )
                                {
                                    return git;
                                }
        ...
        )
        {
            ...
            return _cachedTypeToCompilerGeneratedMembers[typeToCache]
                    ?.TryGetValue(method, out callees) == true;
        }
