...
namespace ILLink.RoslynAnalyzer
{
    public readonly struct DataFlowAnalyzerContext
    {
        ...
        public ImmutableArray<ISymbol> GetSpecialIncompatibleMembers(RequiresAnalyzerBase analyzer)
        {
            if (!_enabledAnalyzers.TryGetValue(analyzer, out var members))
                throw new System.ArgumentException(
                    $"Analyzer {analyzer.GetType().Name} is not in the cache"
                );
            return members;
        }
        ...
        public readonly bool AnyAnalyzersEnabled =>
            EnableTrimAnalyzer || _enabledAnalyzers.Count > 0;
        ...
        public static DataFlowAnalyzerContext Create(
            AnalyzerOptions options,
            Compilation compilation,
            ImmutableArray<RequiresAnalyzerBase> requiresAnalyzers
        )
        {
