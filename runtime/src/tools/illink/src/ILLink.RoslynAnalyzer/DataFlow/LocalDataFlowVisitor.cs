...
namespace ILLink.RoslynAnalyzer.DataFlow
{
    ...
    {
        protected readonly LocalStateAndContextLattice<
            TValue,
            TContext,
            TValueLattice,
            TContextLattice
        ...
        protected readonly InterproceduralStateLattice<
            TValue,
            TValueLattice
        ...
        protected TValue TopValue =>
            LocalStateAndContextLattice.LocalStateLattice.Lattice.ValueLattice.Top;
        ...
        bool IsLValueFlowCapture(CaptureId captureId) => lValueFlowCaptures.ContainsKey(captureId);

        bool IsRValueFlowCapture(CaptureId captureId) =>
            !lValueFlowCaptures.TryGetValue(captureId, out var captureKind)
        ...
        public abstract void ApplyCondition(
            TConditionValue condition,
            ref LocalStateAndContext<TValue, TContext> localContextState
        );
        ...
        public abstract TValue GetFieldTargetValue(
            IFieldSymbol field,
        ...
        public abstract TValue HandleArrayElementRead(
            TValue arrayValue,
            TValue indexValue,
            IOperation operation
        );

        public abstract void HandleArrayElementWrite(
            TValue arrayValue,
            TValue indexValue,
            TValue valueToWrite,
            IOperation operation,
        ...
        public override TValue VisitLocalReference(
        ...
        TValue ProcessBinderCall(
            IOperation operation,
            string methodName,
            LocalDataFlowState<TValue, TContext, TValueLattice, TContextLattice> state
        )
        {
            var assemblyType = Compilation.GetTypeByMetadataName(
                "Microsoft.CSharp.RuntimeBinder.Binder"
            );
            Debug.Assert(assemblyType != null);
            if (assemblyType == null)
                return TopValue;
            var method = assemblyType
        ...
        public override TValue VisitDynamicObjectCreation(
            IDynamicObjectCreationOperation operation,
            LocalDataFlowState<TValue, TContext, TValueLattice, TContextLattice> state
        ) => ProcessBinderCall(operation, "InvokeConstructor", state);

        public override TValue VisitDynamicMemberReference(
            IDynamicMemberReferenceOperation operation,
            LocalDataFlowState<TValue, TContext, TValueLattice, TContextLattice> state
        ) =>
            ProcessBinderCall(
                operation,
                operation.GetValueUsageInfo(OwningSymbol).HasFlag(ValueUsageInfo.Write)
                    ? "SetMember"
                    : "GetMember",
                state
            );
        ...
        bool IsReferenceToCapturedVariable(ILocalReferenceOperation localReference)
        {
            ...
            if (local.IsConst)
                return false;
            Debug.Assert(
                local.ContainingSymbol is IMethodSymbol or IFieldSymbol, // backing field for property initializers
            ...
            switch (targetOperation)
            {
                ...
                case IParameterReferenceOperation:
                {
                    ...
                    TValue targetValue = targetOperation switch
                    {
                        ...
                        IParameterReferenceOperation parameterRef => GetParameterTargetValue(
                ...
                case IPropertyReferenceOperation propertyRef:
                {
                    ...
                    if (
                        OwningSymbol is IPropertySymbol
                        && (ControlFlowGraph.OriginalOperation is not IAttributeOperation)
                    )
                        break;

                    // Property may be an indexer, in which case there will be one or more index arguments followed by a value argument
                    ImmutableArray<TValue>.Builder arguments =
        ...
        public override TValue VisitSimpleAssignment(
        ...
        public override TValue VisitCompoundAssignment(
        ...
        )
        {
            ...
            foreach (var capturedReference in capturedReferences.GetKnownValues())
            {
                ...
                var singleValue = ProcessSingleTargetAssignment(
                    targetOperation,
                    operation,
                    state,
                    merge: true
        ...
        public override TValue VisitEventAssignment(
        ...
        public override TValue VisitExpressionStatement(
        ...
        public override TValue VisitInvocation(
        ...
        public override TValue VisitDelegateCreation(
                ...
                case IMethodReferenceOperation methodReference:
                    ...
                    if (method.ContainingSymbol is IMethodSymbol)
                    {
                        ...
                        var localFunctionCFG =
                            ControlFlowGraph.GetLocalFunctionControlFlowGraphInScope(localFunction);
                        InterproceduralState.TrackMethod(
        ...
        public override TValue VisitPropertyReference(
        ...
        public override TValue VisitEventReference(
        ...
        public override TValue VisitImplicitIndexerReference(
        ...
        )
        {
            ...
            return HandleMethodCallHelper(
                getMethod,
                instanceValue,
                ImmutableArray.Create(indexArgumentValue),
                operation,
                state
            );
        }

        public override TValue VisitArrayElementReference(
        ...
        )
        {
            ...
            return HandleArrayElementRead(
                Visit(operation.ArrayReference, state),
                Visit(operation.Indices[0], state),
                operation
            );
        }

        public override TValue VisitInlineArrayAccess(
        ...
        )
        {
            ...
            return HandleArrayElementRead(
                Visit(operation.Instance, state),
                Visit(operation.Argument, state),
                operation
            );
        }

        public override TValue VisitArgument(
        ...
        public override TValue VisitReturn(
        ...
        public override TValue VisitConversion(
        ...
        public override TValue VisitObjectCreation(
        ...
        )
        {
            ...
            if (
                calledMethod.TryGetAttribute(
                    nameof(DoesNotReturnAttribute),
                    out var doesNotReturnAttributeData
                )
            )
            {
            ...
            foreach (var parameterProxy in calledMethod.GetParameters())
            {
                ...
                if (
                    GetConditionValue(argumentOperation, state)
                    is not TConditionValue conditionValue
                )
                    continue;
