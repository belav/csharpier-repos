...
namespace ILLink.RoslynAnalyzer.DataFlow
{
        ...
        : IControlFlowGraph<BlockProxy, RegionProxy>
    {
        ...
        public static ControlFlowBranch? CreateProxyBranch(
            Microsoft.CodeAnalysis.FlowAnalysis.ControlFlowBranch? branch
        )
        {
            if (branch == null)
                return null;
        ...
        public ControlFlowBranch? GetConditionalSuccessor(BlockProxy block) =>
            CreateProxyBranch(block.Block.ConditionalSuccessor);

        public ControlFlowBranch? GetFallThroughSuccessor(BlockProxy block) =>
            CreateProxyBranch(block.Block.FallThroughSuccessor);

        public bool TryGetEnclosingTryOrCatchOrFilter(
            BlockProxy block,
            out RegionProxy tryOrCatchOrFilterRegion
        )
        {
            return TryGetTryOrCatchOrFilter(
                block.Block.EnclosingRegion,
        ...
        {
            return TryGetTryOrCatchOrFilter(
                regionProxy.Region.EnclosingRegion,
                out tryOrCatchOrFilterRegion
            );
        }
        ...
        )
        {
            ...
            while (region != null && region.Kind != ControlFlowRegionKind.Root)
            {
                if (
                    region.Kind
                    is ControlFlowRegionKind.Try
                        or ControlFlowRegionKind.Catch
        ...
        {
            if (
                catchOrFilterOrFinallyRegion.Region.Kind
                is not (
                ...
                throw new ArgumentException(
                    "Must be a catch, filter, or finally region: {}",
                    nameof(catchOrFilterOrFinallyRegion)
                );
        ...
        public IEnumerable<RegionProxy> GetPreviousFilters(RegionProxy catchOrFilterRegion)
        {
            ...
            if (region.Kind is not (ControlFlowRegionKind.Catch or ControlFlowRegionKind.Filter))
                throw new ArgumentException(
                    "Must be a catch or filter region: {}",
                    nameof(catchOrFilterRegion)
                );

            // Should not be called for a catch block that already has a filter.
            if (
                region.Kind is ControlFlowRegionKind.Catch
                && region.EnclosingRegion!.Kind is ControlFlowRegionKind.FilterAndHandler
            )
                throw new ArgumentException(
                    "Must not be a catch block with filter: {}",
                    nameof(catchOrFilterRegion)
                );
