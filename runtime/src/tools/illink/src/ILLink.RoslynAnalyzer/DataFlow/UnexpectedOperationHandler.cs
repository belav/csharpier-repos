...
namespace Microsoft.CodeAnalysis
{
    internal static class UnexpectedOperationHandler
    {
        ...
        public static void Handle(IOperation operation)
        {
            ...
            // but the code is in an invalid state where the unexpected operation is not IInvalidOperation, yet
            // one
            // of its child operations is. For example:
            //
            //     a + = 3;
            //
            // This is represented as an assignment where the target is an IBinaryOperation (a +) whose
            // right-hand side
            ...
            // but do not throw in Release builds as it means new Roslyn version could cause the analyzer to
            // crash
            ...
            throw new NotImplementedException(
                $"Unexpected operation type {operation.GetType()}: {operation.Syntax.GetLocation().GetLineSpan()}"
            );
        }
    }
}
