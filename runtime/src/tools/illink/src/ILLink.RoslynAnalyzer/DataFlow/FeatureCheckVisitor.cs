...
namespace ILLink.RoslynAnalyzer.DataFlow
{
    ...
    public class FeatureChecksVisitor : OperationVisitor<StateValue, FeatureChecksValue?>
    {
        ...
        public override FeatureChecksValue? VisitArgument(
        ...
        public override FeatureChecksValue? VisitPropertyReference(
        ...
        )
        {
            ...
            {
                if (
                    analyzer.IsRequiresCheck(
                        _dataFlowAnalyzerContext.Compilation,
                        operation.Property
                    )
                )
                {
        ...
        public override FeatureChecksValue? VisitBinaryOperator(
        ...
        )
        {
            ...
            if (GetLiteralBool(operation.LeftOperand) is bool leftBool)
            {
                if (Visit(operation.RightOperand, state) is not FeatureChecksValue rightValue)
                    return null;
                return leftBool == expectEqual ? rightValue : rightValue.Negate();
            }
            ...
            if (GetLiteralBool(operation.RightOperand) is bool rightBool)
            {
                if (Visit(operation.LeftOperand, state) is not FeatureChecksValue leftValue)
                    return null;
                return rightBool == expectEqual ? leftValue : leftValue.Negate();
            }

            return null;
        }

        public override FeatureChecksValue? VisitIsPattern(
        ...
        )
        {
            ...
            return patternValue ? value : value.Negate();
