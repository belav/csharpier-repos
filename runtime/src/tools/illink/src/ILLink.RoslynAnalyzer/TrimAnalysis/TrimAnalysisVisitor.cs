...
{
    public class TrimAnalysisVisitor
        : LocalDataFlowVisitor<
            MultiValue,
            FeatureContext,
        ...
        >
    {
        ...
        public override FeatureChecksValue? GetConditionValue(
        ...
        public override void ApplyCondition(
            FeatureChecksValue featureChecksValue,
        ...
        public override MultiValue Visit(IOperation? operation, StateValue argument)
        {
            ...
            if (
                operation != null
                && returnValue.IsEmpty()
                && TryGetConstantValue(operation, out var constValue)
            )
                return constValue;
        ...
        public override MultiValue VisitArrayCreation(
        ...
        )
        {
            ...
            var elements =
                operation.Initializer?.ElementValues.Select(val => Visit(val, state)).ToArray()
                ?? System.Array.Empty<MultiValue>();
        ...
        public override MultiValue VisitConversion(IConversionOperation operation, StateValue state)
        {
            ...
            if (operation.OperatorMethod != null)
                return operation.OperatorMethod.ReturnType.IsTypeInterestingForDataflow()
                    ? new MethodReturnValue(operation.OperatorMethod)
                    : value;
        ...
        public override MultiValue VisitParameterReference(
        ...
        public override MultiValue VisitInstanceReference(
            IInstanceReferenceOperation instanceRef,
            StateValue state
        )
        {
        ...
        public override MultiValue VisitFieldReference(
        ...
        public override MultiValue VisitBinaryOperator(
        ...
        )
        {
            if (
                ...
                && // Optimization - if there is already a constant value available, rely on the Visit(IOperation) instead
                ...
                foreach (var left in leftValue.AsEnumerable())
                {
                    ...
                    else if (left is ConstIntValue leftConstInt)
                    {
                        foreach (var right in rightValue.AsEnumerable())
                        {
                            ...
                            {
                                result = _multiValueLattice.Meet(
                                    result,
                                    new ConstIntValue(leftConstInt.Value | rightConstInt.Value)
                                );
                            }
                        }
                    }
        ...
        public override MultiValue GetFieldTargetValue(
            IFieldSymbol field,
        ...
        )
        {
            TrimAnalysisPatterns.Add(
                new TrimAnalysisFieldAccessPattern(
                    field,
        ...
        public override MultiValue HandleArrayElementRead(
            ...
            foreach (var arraySingleValue in arrayValue.AsEnumerable())
            {
                if (arraySingleValue is ArrayValue arr)
                {
                    if (index == null)
                    {
                        ...
                        arr.IndexValues.TryGetValue(index.Value, out _)
                        || arr.IndexValues.Count < MaxTrackedArrayValues
                    )
                    {
                ...
                case IntrinsicId.Object_GetType:
                    {
                        foreach (var valueNode in instance.AsEnumerable())
                        {
                            ...
                            if (staticType is null)
                            {
                                ...
                                || staticType.TypeKind == TypeKind.Array
                            )
                            {
        ...
        {
            TrimAnalysisPatterns.Add(
                new TrimAnalysisReflectionAccessPattern(
                    method,
                    operation,
                    OwningSymbol,
                    featureContext
        ...
        static bool TryGetConstantValue(IOperation operation, out MultiValue constValue)
        {
            if (operation.ConstantValue.HasValue)
            {
                ...
                if (constantValue == null)
                {
                    ...
                    operation.Type?.TypeKind == TypeKind.Enum
                    && constantValue is int enumConstantValue
                )
                {
                ...
                else
                {
                    switch (operation.Type?.SpecialType)
                    {
                        case SpecialType.System_String
                            when constantValue is string stringConstantValue:
                            constValue = new KnownStringValue(stringConstantValue);
                            return true;
                        case SpecialType.System_Boolean
                            when constantValue is bool boolConstantValue:
                            constValue = new ConstIntValue(boolConstantValue ? 1 : 0);
                            return true;
                        case SpecialType.System_SByte
                            when constantValue is sbyte sbyteConstantValue:
                        ...
                        case SpecialType.System_Int16
                            when constantValue is short int16ConstantValue:
                            constValue = new ConstIntValue(int16ConstantValue);
                            return true;
                        case SpecialType.System_UInt16
                            when constantValue is ushort uint16ConstantValue:
                        ...
                        case SpecialType.System_UInt32
                            when constantValue is uint uint32ConstantValue:
