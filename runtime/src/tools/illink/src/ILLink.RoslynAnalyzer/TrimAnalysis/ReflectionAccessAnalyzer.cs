...
#pragma warning disable CA1822 // Mark members as static - the other partial implementations might need to be instance methods
        internal void GetReflectionAccessDiagnostics(
...
*/
        ...
        internal void GetReflectionAccessDiagnosticsForEventsOnTypeHierarchy(
            in DiagnosticContext diagnosticContext,
            ITypeSymbol typeSymbol,
            string name,
        ...
        {
            foreach (
                var @event in typeSymbol.GetEventsOnTypeHierarchy(e => e.Name == name, bindingFlags)
            )
                GetDiagnosticsForEvent(diagnosticContext, @event);
        }

        internal void GetReflectionAccessDiagnosticsForFieldsOnTypeHierarchy(
            in DiagnosticContext diagnosticContext,
            ITypeSymbol typeSymbol,
            string name,
            BindingFlags? bindingFlags
        ...
        {
            foreach (
                var field in typeSymbol.GetFieldsOnTypeHierarchy(f => f.Name == name, bindingFlags)
            )
                GetDiagnosticsForField(diagnosticContext, field);
        }

        internal void GetReflectionAccessDiagnosticsForPropertiesOnTypeHierarchy(
            in DiagnosticContext diagnosticContext,
            ITypeSymbol typeSymbol,
            string name,
            BindingFlags? bindingFlags
        ...
        {
            foreach (
                var prop in typeSymbol.GetPropertiesOnTypeHierarchy(
                    p => p.Name == name,
                    bindingFlags
                )
            )
        ...
        )
        {
            foreach (
                var c in typeSymbol.GetConstructorsOnType(
                    filter: parameterCount.HasValue
                        ? c => c.Parameters.Length == parameterCount.Value
                        : null,
                    bindingFlags: bindingFlags
                )
            )
        ...
        internal void GetReflectionAccessDiagnosticsForPublicParameterlessConstructor(
            in DiagnosticContext diagnosticContext,
            ITypeSymbol typeSymbol
        )
        {
        ...
        static void ReportRequiresUnreferencedCodeDiagnostic(
            in DiagnosticContext diagnosticContext,
            AttributeData requiresAttributeData,
            ISymbol member
        )
        {
            var message = RequiresUnreferencedCodeUtils.GetMessageFromAttribute(
                requiresAttributeData
            );
        ...
        internal static void GetReflectionAccessDiagnosticsForMethod(
            in DiagnosticContext diagnosticContext,
            IMethodSymbol methodSymbol
        )
        {
            if (
                methodSymbol.IsInRequiresUnreferencedCodeAttributeScope(
                    out var requiresUnreferencedCodeAttributeData
                )
            )
            {
                ReportRequiresUnreferencedCodeDiagnostic(
                    diagnosticContext,
                    requiresUnreferencedCodeAttributeData,
                    methodSymbol
                );
            }
            else
            {
                foreach (
                    var diagnostic in GetDiagnosticsForReflectionAccessToDAMOnMethod(
                        diagnosticContext,
                        methodSymbol
                    )
                )
        ...
        internal static IEnumerable<Diagnostic> GetDiagnosticsForReflectionAccessToDAMOnMethod(
        ...
        )
        {
            if (
                ...
                && FlowAnnotations.GetMethodReturnValueAnnotation(methodSymbol)
                    != DynamicallyAccessedMemberTypes.None
            )
            {
                yield return diagnosticContext.CreateDiagnostic(
            ...
            else
            {
                ...
                {
                    if (
                        FlowAnnotations.GetMethodParameterAnnotation(parameter)
                        != DynamicallyAccessedMemberTypes.None
                    )
                    {
                        yield return diagnosticContext.CreateDiagnostic(
        ...
        internal static void GetReflectionAccessDiagnosticsForProperty(
            in DiagnosticContext diagnosticContext,
        ...
        static void GetDiagnosticsForEvent(
            in DiagnosticContext diagnosticContext,
        ...
        static void GetDiagnosticsForField(
            in DiagnosticContext diagnosticContext,
            IFieldSymbol fieldSymbol
        )
        {
            if (
                fieldSymbol.TryGetRequiresUnreferencedCodeAttribute(
                    out var requiresUnreferencedCodeAttributeData
                )
            )
                ReportRequiresUnreferencedCodeDiagnostic(
