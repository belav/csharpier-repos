...
namespace ILLink.Shared.TrimAnalysis
{
    readonly partial struct DiagnosticContext
    {
        ...
        public partial void AddDiagnostic(DiagnosticId id, params string[] args)
        {
            ...
            Diagnostics.Add(
                Diagnostic.Create(DiagnosticDescriptors.GetDiagnosticDescriptor(id), Location, args)
            );
        }
        ...
        )
        {
            ...
            else
            {
                ...
                DAMArgument.Add(
                    "attributeArgument",
                    expectedAnnotationsValue.DynamicallyAccessedMemberTypes.ToString()
                );
            ...
            Diagnostics.Add(
                Diagnostic.Create(
                    DiagnosticDescriptors.GetDiagnosticDescriptor(id),
                    Location,
                    sourceLocation,
                    DAMArgument?.ToImmutableDictionary(),
                    args
                )
            );
        }
