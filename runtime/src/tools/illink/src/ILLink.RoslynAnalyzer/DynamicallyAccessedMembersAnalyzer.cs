...
namespace ILLink.RoslynAnalyzer
{
    ...
    public class DynamicallyAccessedMembersAnalyzer : DiagnosticAnalyzer
    {
        ...
        internal const string DynamicallyAccessedMembersAttribute = nameof(
            DynamicallyAccessedMembersAttribute
        );
        public const string attributeArgument = "attributeArgument";
        public const string FullyQualifiedDynamicallyAccessedMembersAttribute =
            "System.Diagnostics.CodeAnalysis." + DynamicallyAccessedMembersAttribute;
        public static Lazy<ImmutableArray<RequiresAnalyzerBase>> RequiresAnalyzers { get; } =
            new Lazy<ImmutableArray<RequiresAnalyzerBase>>(GetRequiresAnalyzers);
        ...
        public static ImmutableArray<DiagnosticDescriptor> GetSupportedDiagnostics()
        {
            ...
            void AddRange(DiagnosticId first, DiagnosticId last)
            {
                ...
                {
                    diagDescriptorsArrayBuilder.Add(
                        DiagnosticDescriptors.GetDiagnosticDescriptor((DiagnosticId)i)
                    );
                }
            }
        }
        ...
        public override void Initialize(AnalysisContext context)
        {
            ...
            {
                var dataFlowAnalyzerContext = DataFlowAnalyzerContext.Create(
                    context.Options,
                    context.Compilation,
                ...
                context.RegisterSymbolAction(
                    context =>
                    {
                        ...
                        {
                            foreach (
                                var diagnostic in ProcessGenericParameters(interfaceType, location)
                            )
                                context.ReportDiagnostic(diagnostic);
                        }
                    },
                ...
                context.RegisterSymbolAction(
                    context =>
                    {
                        ...
                        foreach (
                            var diagnostic in ProcessGenericParameters(
                                returnType,
                                GetPrimaryLocation(method.Locations)
                            )
                        )
                            context.ReportDiagnostic(diagnostic);
                        ...
                        foreach (var parameter in method.Parameters)
                        {
                            foreach (
                                var diagnostic in ProcessGenericParameters(
                                    parameter.Type,
                                    GetPrimaryLocation(parameter.Locations)
                                )
                            )
                                context.ReportDiagnostic(diagnostic);
                        }
                ...
                context.RegisterSymbolAction(
                    context =>
                    {
                        ...
                        foreach (
                            var diagnostic in ProcessGenericParameters(
                                field.Type,
                                GetPrimaryLocation(field.Locations)
                            )
                        )
                            context.ReportDiagnostic(diagnostic);
                    },
                ...
                context.RegisterOperationAction(
                    context =>
                    {
                        ...
                        foreach (
                            var diagnostic in ProcessMethodGenericParameters(
                                methodSymbol,
                ...
                context.RegisterSymbolAction(
                    context =>
                    {
                        VerifyDamOnInterfaceAndImplementationMethodsMatch(
                            context,
        ...
        static IEnumerable<Diagnostic> ProcessMethodGenericParameters(
        ...
        static IEnumerable<Diagnostic> ProcessGenericParameters(ISymbol symbol, Location location)
        {
            ...
            if (typeParams != null)
            {
                ...
                for (int i = 0; i < typeParams.Length; i++)
                {
                    ...
                    foreach (
                        var diagnostic in GetDynamicallyAccessedMembersDiagnostics(
                            sourceValue,
                            targetValue,
                            location
                        )
        ...
        static List<Diagnostic> GetDynamicallyAccessedMembersDiagnostics(
            SingleValue sourceValue,
            SingleValue targetValue,
            Location location
        )
        {
        ...
        )
        {
            ...
            if (
                ...
                && field.GetDynamicallyAccessedMemberTypes() != DynamicallyAccessedMemberTypes.None
                && !field.Type.IsTypeInterestingForDataflow()
            )
            ...
            {
                if (
                    method.GetDynamicallyAccessedMemberTypesOnReturnType()
                        != DynamicallyAccessedMemberTypes.None
                    && !method.ReturnType.IsTypeInterestingForDataflow()
                )
                    context.ReportDiagnostic(
                        Diagnostic.Create(
                            DiagnosticDescriptors.GetDiagnosticDescriptor(
                ...
                if (
                    method.GetDynamicallyAccessedMemberTypes()
                        != DynamicallyAccessedMemberTypes.None
                    && !method.ContainingType.IsTypeInterestingForDataflow()
                )
                    context.ReportDiagnostic(
                        Diagnostic.Create(
                            DiagnosticDescriptors.GetDiagnosticDescriptor(
                ...
                {
                    if (
                        parameter.GetDynamicallyAccessedMemberTypes()
                            != DynamicallyAccessedMemberTypes.None
                        && !parameter.Type.IsTypeInterestingForDataflow()
                    )
                        context.ReportDiagnostic(
                            Diagnostic.Create(
                                DiagnosticDescriptors.GetDiagnosticDescriptor(
        ...
        static void VerifyDamOnDerivedAndBaseMethodsMatch(
            SymbolAnalysisContext context,
            IMethodSymbol methodSymbol
        )
        {
            if (
                methodSymbol.TryGetOverriddenMember(out var overriddenSymbol)
                && overriddenSymbol is IMethodSymbol overriddenMethod
        ...
        static void VerifyDamOnMethodsMatch(
            SymbolAnalysisContext context,
            IMethodSymbol overrideMethod,
            IMethodSymbol baseMethod
        )
        {
            var overrideMethodReturnAnnotation = FlowAnnotations.GetMethodReturnValueAnnotation(
                overrideMethod
            );
            var baseMethodReturnAnnotation = FlowAnnotations.GetMethodReturnValueAnnotation(
                baseMethod
            );
            if (overrideMethodReturnAnnotation != baseMethodReturnAnnotation)
            {
                (
                    ...
                    DynamicallyAccessedMemberTypes missingAttribute
                ) = GetTargetAndRequirements(
                    overrideMethod,
                    baseMethod,
                    overrideMethodReturnAnnotation,
                    baseMethodReturnAnnotation
                );

                Location attributableSymbolLocation = GetPrimaryLocation(
                ...
                context.ReportDiagnostic(
                    Diagnostic.Create(
                        ...
                        sourceLocation,
                        DAMArgs?.ToImmutableDictionary(),
                        overrideMethod.GetDisplayName(),
            ...
            foreach (var overrideParam in overrideMethod.GetMetadataParameters())
            {
                ...
                var baseParameterAnnotation = FlowAnnotations.GetMethodParameterAnnotation(
                    baseParam
                );
                var overrideParameterAnnotation = FlowAnnotations.GetMethodParameterAnnotation(
                    overrideParam
                );
                if (overrideParameterAnnotation != baseParameterAnnotation)
                {
                    ...
                    context.ReportDiagnostic(
                        Diagnostic.Create(
                            ...
                            sourceLocation,
                            DAMArgs?.ToImmutableDictionary(),
                            overrideParam.GetDisplayName(),
                            overrideMethod.GetDisplayName(),
                            baseParam.GetDisplayName(),
                            baseMethod.GetDisplayName()
                        )
                    );
                }
                ...
                if (methodTypeParameterAnnotation != overriddenMethodTypeParameterAnnotation)
                {
                    ...
                    context.ReportDiagnostic(
                        Diagnostic.Create(
                            ...
                            GetPrimaryLocation(overrideMethod.TypeParameters[i].Locations),
                            sourceLocation,
                            DAMArgs?.ToImmutableDictionary(),
                            overrideMethod.TypeParameters[i].GetDisplayName(),
                            overrideMethod.GetDisplayName(),
                            baseMethod.TypeParameters[i].GetDisplayName(),
                            baseMethod.GetDisplayName()
                        )
                    );
                }
            ...
            if (
                !overrideMethod.IsStatic
                && overrideMethod.GetDynamicallyAccessedMemberTypes()
                    != baseMethod.GetDynamicallyAccessedMemberTypes()
            )
                context.ReportDiagnostic(
                    Diagnostic.Create(
                        DiagnosticDescriptors.GetDiagnosticDescriptor(
        ...
        )
        {
            foreach (
                ...
                ) in type.GetMemberInterfaceImplementationPairs()
            )
            {
                if (
                    implementationMember is IMethodSymbol implementationMethod
        ...
        static void VerifyDamOnPropertyAndAccessorMatch(
            SymbolAnalysisContext context,
            IMethodSymbol methodSymbol
        )
        {
        ...
        private static (
            ...
            DynamicallyAccessedMemberTypes Requirements
        ) GetTargetAndRequirements(
            IMethodSymbol method,
            IMethodSymbol overriddenMethod,
            DynamicallyAccessedMemberTypes methodAnnotation,
            DynamicallyAccessedMemberTypes overriddenMethodAnnotation
        )
        {
        ...
        private static (Location[]?, Dictionary<string, string?>?) CreateArguments(
        ...
        )
        {
            ...
            DAMArgument.Add(
                DynamicallyAccessedMembersAnalyzer.attributeArgument,
                mismatchedArgument.ToString()
            );
