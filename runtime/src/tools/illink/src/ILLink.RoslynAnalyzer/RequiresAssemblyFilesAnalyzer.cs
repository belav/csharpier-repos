...
namespace ILLink.RoslynAnalyzer
{
    ...
    public sealed class RequiresAssemblyFilesAnalyzer : RequiresAnalyzerBase
    {
        ...
        public const string RequiresAssemblyFilesAttributeFullyQualifiedName =
        ...
        public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics =>
            ImmutableArray.Create(
                ...
                s_requiresAssemblyFilesAttributeMismatch,
                s_requiresAssemblyFilesOnStaticCtor
            );
        ...
        private protected override string RequiresAttributeFullyQualifiedName =>
            RequiresAssemblyFilesAttributeFullyQualifiedName;

        private protected override DiagnosticTargets AnalyzerDiagnosticTargets =>
            DiagnosticTargets.MethodOrConstructor
            | DiagnosticTargets.Property
            | DiagnosticTargets.Event;
        ...
        {
            var isSingleFileAnalyzerEnabled = options.GetMSBuildPropertyValue(
                MSBuildPropertyOptionNames.EnableSingleFileAnalyzer
            );
            if (
                !string.Equals(
                    isSingleFileAnalyzerEnabled?.Trim(),
                    "true",
            ...
            var includesAllContent = options.GetMSBuildPropertyValue(
                MSBuildPropertyOptionNames.IncludeAllContentForSelfExtract
            );
            if (
                string.Equals(
                    includesAllContent?.Trim(),
                    "true",
                    StringComparison.OrdinalIgnoreCase
                )
            )
        ...
        )
        {
            ...
            var isDynamicCodeSupportedProperty = runtimeFeaturesType
                .GetMembers("IsAssemblyFilesSupported")
        ...
        internal override ImmutableArray<ISymbol> GetSpecialIncompatibleMembers(
            Compilation compilation
        )
        {
            ...
            {
                // Properties
                ImmutableArrayOperations.AddIfNotNull(
                    dangerousPatternsBuilder,
                    ImmutableArrayOperations.TryGetSingleSymbol<IPropertySymbol>(
                        assemblyType.GetMembers("Location")
                    )
                );

                // Methods
                dangerousPatternsBuilder.AddRange(
                    assemblyType.GetMembers("GetFile").OfType<IMethodSymbol>()
                );
                dangerousPatternsBuilder.AddRange(
                    assemblyType.GetMembers("GetFiles").OfType<IMethodSymbol>()
                );
            }

            var assemblyNameType = compilation.GetTypeByMetadataName(
            ...
            if (assemblyNameType != null)
            {
                ImmutableArrayOperations.AddIfNotNull(
                    dangerousPatternsBuilder,
                    ImmutableArrayOperations.TryGetSingleSymbol<IPropertySymbol>(
                        assemblyNameType.GetMembers("CodeBase")
                    )
                );
                ImmutableArrayOperations.AddIfNotNull(
                    dangerousPatternsBuilder,
                    ImmutableArrayOperations.TryGetSingleSymbol<IPropertySymbol>(
                        assemblyNameType.GetMembers("EscapedCodeBase")
                    )
                );
            }
        ...
        )
        {
            ...
            {
                if (
                    ImmutableArrayOperations.Contains(
                        dangerousPatterns,
                        member,
                ...
                )
                {
                    ...
                    method.AssociatedSymbol is ISymbol associatedSymbol
                    && ImmutableArrayOperations.Contains(
                        dangerousPatterns,
                        associatedSymbol,
                        SymbolEqualityComparer.Default
