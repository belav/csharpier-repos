...
namespace ILLink.Tasks.Tests
{
    ...
    {
        public static IEnumerable<object[]> AssemblyPathsCases =>
            new List<object[]>
            {
                new object[]
                {
                    new ITaskItem[]
                    {
                        new TaskItem(
                            "Assembly.dll",
                            new Dictionary<string, string> { { "trimmode", "copy" } }
                        )
                    }
                },
                new object[]
                ...
                new object[] { new ITaskItem[] { new TaskItem("path with/spaces/Assembly.dll") } },
                new object[]
                {
                    new ITaskItem[]
                    {
        ...
        {
            var task = new MockTask() { TrimMode = trimMode };
        ...
        public void TrimModeAssemblyPaths(string trimMode)
        {
            var assemblyPaths = new ITaskItem[]
            {
                new TaskItem(
                    "Assembly1.dll",
                    new Dictionary<string, string> { { "IsTrimmable", "true" } }
                ),
                new TaskItem("Assembly2.dll", new Dictionary<string, string>()),
                new TaskItem(
                    "Assembly3.dll",
                    new Dictionary<string, string> { { "IsTrimmable", "false" } }
                ),
            };
            var task = new MockTask() { TrimMode = trimMode, AssemblyPaths = assemblyPaths };
            using var driver = task.CreateDriver();
        ...
        {
            var task = new MockTask() { AssemblyPaths = assemblyPaths };
            using (var driver = task.CreateDriver())
            {
                var context = driver.Context;

                var expectedReferences = assemblyPaths
                    .Select(p => p.ItemSpec)
                ...
                foreach (var item in assemblyPaths)
                {
                    ...
                    AssemblyAction expectedAction = (AssemblyAction)
                        Enum.Parse(typeof(AssemblyAction), trimMode, ignoreCase: true);
                    AssemblyAction actualAction = context.Actions[
        ...
        public void TestAssemblyPathsWithInvalidAction()
        {
            var task = new MockTask()
            {
                AssemblyPaths = new ITaskItem[]
                {
                    new TaskItem(
                        "Assembly.dll",
                        new Dictionary<string, string> { { "TrimMode", "invalid" } }
                    )
                }
            };
        ...
        {
            var task = new MockTask() { OutputDirectory = new TaskItem(outputDirectory) };
            using (var driver = task.CreateDriver())
            {
        ...
        public void TestRootDescriptorFiles(string[] rootDescriptorFiles)
        {
            ...
            using (var driver = task.CreateDriver())
            {
                ...
                Assert.Equal(
                    rootDescriptorFiles.OrderBy(f => f),
                    actualDescriptors.OrderBy(f => f)
                );
            }
        }
            ...
            foreach (var optimization in MockTask.OptimizationNames)
            {
                yield return new object[]
                {
                    new ITaskItem[]
                    {
                        new TaskItem(
                            ...
                            new Dictionary<string, string> { { optimization, "True" } }
                        )
                    }
                };
                yield return new object[]
                {
                    new ITaskItem[]
                    {
                        new TaskItem(
                            "path/to/Assembly.dll",
                            new Dictionary<string, string> { { optimization, "False" } }
                        )
                    }
                };
            }
        ...
        public static IEnumerable<object[]> SingleWarnCases =>
            new List<object[]>
            {
                new object[]
                {
                    ...
                    new ITaskItem[]
                    {
                        new TaskItem(
                            "AssemblyTrue.dll",
                            new Dictionary<string, string> { { "TrimmerSingleWarn", "true" } }
                        ),
                        new TaskItem(
                            "AssemblyFalse.dll",
                            new Dictionary<string, string> { { "TrimmerSingleWarn", "false" } }
                        )
                    },
                },
                new object[]
        ...
        {
            var task = new MockTask() { AssemblyPaths = assemblyPaths, SingleWarn = singleWarn };
            using (var driver = task.CreateDriver())
            {
        ...
        public void TestInvalidPerAssemblyOptimizations()
        {
            var task = new MockTask()
            {
                AssemblyPaths = new ITaskItem[]
                {
                    new TaskItem(
                        "path/to/Assembly.dll",
                        new Dictionary<string, string> { { "Sealer", "invalid" } }
                    )
                }
            };
        ...
        public void CheckGlobalOptimizationsMatchPerAssemblyOptimizations()
        {
            ...
            Assert.Equal(
                optimizationMetadataNames.OrderBy(o => o),
                optimizationPropertyNames.OrderBy(o => o)
            );
        }
        ...
        {
            var task = new MockTask() { NoWarn = noWarn };
            using (var driver = task.CreateDriver())
            {
        ...
        {
            var task = new MockTask() { Warn = warnArg };
            using (var driver = task.CreateDriver())
            {
...
#nullable enable
        ...
        [InlineData(
            false,
            "IL1001,IL2000,IL2054,IL2022",
            null,
            new int[] { 1001, 2000, 2054, 2022 },
            new int[] { }
        )]
        [InlineData(
            false,
            "IL2023,IL6000;IL5042 IL2040",
            "IL4000,IL4001;IL4002 IL4003",
            new int[] { 2023, 2040, 5042, 6000 },
            new int[] { 4000, 4001, 4002, 4003 }
        )]
        [InlineData(
            false,
            "IL3000;IL3000;ABCD",
            "IL2005 IL3005 IL2005",
            new int[] { 3000 },
            new int[] { 2005, 3005 }
        )]
            ...
            using (var driver = task.CreateDriver())
            {
                ...
                if (warnAsError.Length > 0)
                {
                    foreach (var warningCode in warnAsError)
                        Assert.True(
                            actualWarnAsError.ContainsKey(warningCode)
                                && actualWarnAsError[warningCode] == true
                        );
                }
                ...
                if (warnNotAsError.Length > 0)
                {
                    foreach (var warningCode in warnNotAsError)
                        Assert.True(
                            actualWarnAsError.ContainsKey(warningCode)
                                && actualWarnAsError[warningCode] == false
                        );
                }
            }
        }
...
#nullable restore
        ...
        public static IEnumerable<object[]> CustomDataCases =>
            new List<object[]>
            {
                new object[]
                {
                    new ITaskItem[]
                    {
                        new TaskItem(
                            "DataName",
                            new Dictionary<string, string> { { "Value", "DataValue" } }
                        )
                    },
                },
                new object[]
                {
                    new ITaskItem[]
                    {
                        ...
                        new TaskItem(
                            "DataName",
                            new Dictionary<string, string> { { "Value", "DataValue2" } }
                        )
                    },
                },
                new object[]
                {
                    new ITaskItem[]
                    {
                        new TaskItem(
                            "DataName1",
                            new Dictionary<string, string> { { "Value", "DataValue1" } }
                        ),
                        new TaskItem(
                            "DataName2",
                            new Dictionary<string, string> { { "Value", "DataValue2" } }
                ...
                new object[]
                {
                    new ITaskItem[]
                    {
                        new TaskItem(
                            "DataName",
                            new Dictionary<string, string> { { "Value", "data value with spaces" } }
                        )
                    },
                },
            };
        ...
        {
            var task = new MockTask() { CustomData = customData };
            using (var driver = task.CreateDriver())
            {
                var expectedCustomData = customData
                    .Select(c => new { Key = c.ItemSpec, Value = c.GetMetadata("Value") })
                    .GroupBy(c => c.Key)
                    .Select(c => c.Last())
        ...
        public static IEnumerable<object[]> FeatureSettingsCases =>
            new List<object[]>
            {
                new object[]
                {
                ...
                new object[]
                {
                    new ITaskItem[]
                    {
                        ...
                        new TaskItem(
                            "FeatureName",
                            new Dictionary<string, string> { { "Value", "false" } }
                        )
                    },
                },
                new object[]
        ...
        {
            var task = new MockTask() { FeatureSettings = featureSettings };
            using (var driver = task.CreateDriver())
            {
                var expectedSettings = featureSettings
                    .Select(f => new { Feature = f.ItemSpec, Value = f.GetMetadata("Value") })
                    .GroupBy(f => f.Feature)
                    .Select(f => f.Last())
        ...
        {
            var task = new MockTask() { TrimMode = "copy", ExtraArgs = "--trim-mode copyused" };
            using (var driver = task.CreateDriver())
            {
        ...
        {
            var task = new MockTask() { DumpDependencies = dumpDependencies };
            using (var driver = task.CreateDriver())
            {
                Assert.Equal(
                    dumpDependencies,
                    driver.GetDependencyRecorders()?.Single() == MockXmlDependencyRecorder.Singleton
                );
            }
        }
        ...
        public void TestDependenciesFileFormat(string fileFormat)
        {
            ...
            using (var driver = task.CreateDriver())
            {
                switch (fileFormat.ToLower())
                {
                    case "xml":
                        Assert.Equal(
                            MockXmlDependencyRecorder.Singleton,
                            driver.GetDependencyRecorders()?.Single()
                        );
                        break;
                    case "dgml":
                        Assert.Equal(
                            MockDgmlDependencyRecorder.Singleton,
                            driver.GetDependencyRecorders()?.Single()
                        );
                        break;
                    default:
        ...
        {
            var task = new MockTask() { RemoveSymbols = removeSymbols };
            using (var driver = task.CreateDriver())
            {
        ...
        public void TestGlobalTrimMode(string trimMode)
        {
            ...
            using (var driver = task.CreateDriver())
            {
                var expectedAction = (AssemblyAction)
                    Enum.Parse(typeof(AssemblyAction), trimMode, ignoreCase: true);
        ...
        {
            var task = new MockTask() { DefaultAction = defaultAction };
            using (var driver = task.CreateDriver())
            {
                var expectedAction = (AssemblyAction)
                    Enum.Parse(typeof(AssemblyAction), defaultAction, ignoreCase: true);
        ...
        {
            var task = new MockTask() { TrimMode = "invalid" };
        ...
        public static IEnumerable<object[]> CustomStepsCases =>
            new List<object[]>
            {
                new object[]
                {
        ...
        public void TestCustomSteps(ITaskItem[] customSteps)
        {
            ...
            using (var driver = task.CreateDriver())
            {
                foreach (var customStep in customSteps)
                {
                    ...
                    var actualStepNames = driver.Context.Pipeline
                        .GetSteps()
                    ...
                    if (!String.IsNullOrEmpty(beforeStepName))
                    {
                        ...
                        Assert.Equal(
                            stepName,
                            actualStepNames.TakeWhile(s => s != beforeStepName).Last()
                        );
                    }
                    else if (!String.IsNullOrEmpty(afterStepName))
                    {
                        ...
                        Assert.Equal(
                            stepName,
                            actualStepNames.SkipWhile(s => s != afterStepName).ElementAt(1)
                        );
                    }
                    else
                    {
        ...
        public void TestCustomStepOrdering()
        {
            ...
            using (var driver = task.CreateDriver())
            {
                var actualSteps = driver.Context.Pipeline
                    .GetSteps()
                    .Select(s => s.GetType().Name)
                    .ToList();
                Assert.Equal(
                ...
                var actualMarkHandlers = driver.Context.Pipeline.MarkHandlers
        ...
        {
            var task = new MockTask() { RootAssemblyNames = Array.Empty<ITaskItem>() };
            ...
            if (corelibPath == null)
                throw new NotSupportedException(
                    "Running this test in single-file mode is not yet supported."
                );
            ...
            string dotnetRootPath = Path.GetDirectoryName(
                Path.GetDirectoryName(Path.GetDirectoryName(Path.GetDirectoryName(corelibPath)))
            );
            string dotnetPath = Path.Combine(dotnetRootPath, dotnetToolName);
            if (!File.Exists(dotnetPath))
                throw new NotSupportedException(
                    "Running test in a configuration where we can't figure out dotnet root path."
                );
