...
namespace Mono.Linker.Tests.Cases.LinqExpressions
{
    public class CustomOperatorsWithUnusedArgumentTypes
    {
        ...
        public class CustomOperators
        {
            ...
            [Kept]
            public static CustomOperators operator +(CustomOperators left, AdditionType right) =>
                null;

            [Kept]
            public static CustomOperators operator -(SubtractionType left, CustomOperators right) =>
                null;

            // binary operators taking unused other types are removed
            public static CustomOperators operator *(
                CustomOperators left,
                MultiplicationTypeUnused right
            ) => null;

            public static CustomOperators operator /(
                DivisionTypeUnused left,
