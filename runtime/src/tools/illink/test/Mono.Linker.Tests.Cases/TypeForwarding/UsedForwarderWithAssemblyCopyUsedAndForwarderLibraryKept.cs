...
namespace Mono.Linker.Tests.Cases.TypeForwarding
{
    ...
    // The typeref to the type forwarder is updated, so the type forwarder is removed (and the
    // assemblyref along with it)
    ...
    class UsedForwarderWithAssemblyCopyUsedAndForwarderLibraryKept
    {
        static void Main()
        {
            ...
            var t1 = Type.GetType(
                "Mono.Linker.Tests.Cases.TypeForwarding.Dependencies.ImplementationStruct, Forwarder"
            );
