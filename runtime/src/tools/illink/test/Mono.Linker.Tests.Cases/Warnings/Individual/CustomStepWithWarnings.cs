...
namespace Mono.Linker.Tests.Cases.Warnings.Individual
{
    [SetupCompileBefore(
        "CustomStep.dll",
        new[] { "../Dependencies/CustomStep.cs" },
        new[] { "illink.dll" }
    )]
    [SetupLinkerArgument("--custom-step", "CustomStep.CustomStep,CustomStep.dll")]
    [SetupLinkerArgument(
        "--custom-step",
        "CustomStep.CustomStepWithInvalidWarningCode,CustomStep.dll"
    )]
    [LogContains("CustomStep.cs(1,1): warning IL6001: Warning")]
    [LogContains(
        "The provided code '2500' does not fall into the permitted range for external warnings. To avoid possible collisions "
