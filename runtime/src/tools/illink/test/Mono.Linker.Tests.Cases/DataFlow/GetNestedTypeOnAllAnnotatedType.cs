...
namespace Mono.Linker.Tests.Cases.DataFlow
{
    ...
    class GetNestedTypeOnAllAnnotatedType
    {
        ...
        static void TestOnAllAnnotatedParameter(
            [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type parentType
        )
        {
        ...
        {
            var nestedType = parentType.GetNestedType(
                nameof(TestType.NestedType),
                BindingFlags.Public
            );
            nestedType.RequiresAll();
        }

        static void TestWithUnknownBindingFlags(
        ...
        static void TestIfElse(
            int number,
            [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type parentWithAll,
            [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicNestedTypes)]
                Type parentWithoutAll
        )
        {
            Type typeOfParent;
            if (number == 1)
        ...
        static void TestSwitchAllValid(
            int number,
            [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type parentWithAll
        )
        {
