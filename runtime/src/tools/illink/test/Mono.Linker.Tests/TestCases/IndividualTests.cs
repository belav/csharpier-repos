...
namespace Mono.Linker.Tests.TestCases
{
    ...
    {
        private static NPath TestsDirectory =>
            TestDatabase.TestCasesRootDirectory.Parent.Combine("Mono.Linker.Tests");
        ...
        public void CanSkipUnresolved()
        {
            ...
            if (!result.OutputAssemblyPath.Exists())
                Assert.Fail(
                    $"The linked assembly is missing.  Should have existed at {result.OutputAssemblyPath}"
                );
        }
        ...
        public void CanOutputPInvokes()
        {
            ...
            if (!outputPath.Exists())
                Assert.Fail(
                    $"The json file with the list of all the PInvokes found by the linker is missing. Expected it to exist at {outputPath}"
                );
            ...
            using (
                var fsExpected = File.Open(
                    TestsDirectory.Combine("TestCases/Dependencies/PInvokesExpectations.json"),
                    FileMode.Open
                )
            )
            {
        ...
        public void CanGenerateWarningSuppressionFileCSharp()
        {
            ...
            for (int i = 0; i < expectedAssemblies.Length; i++)
            {
                ...
                Assert.That(
                    ...
                    Is.EquivalentTo(
                        File.ReadAllLines(
                            TestsDirectory.Combine(
                                $"TestCases/Dependencies/WarningSuppressionExpectations{i + 1}.cs"
                            )
                        )
                    )
                );
        ...
        public void CanGenerateWarningSuppressionFileXml()
        {
            ...
            Assert.That(
                ...
                Is.EquivalentTo(
                    File.ReadAllLines(
                        TestsDirectory.Combine(
                            $"TestCases/Dependencies/WarningSuppressionExpectations3.xml"
                        )
                    )
                )
            );
        ...
        public void WarningsAreSorted()
        {
            ...
            var loggedMessages = result.Logger
                ...
                .Where(
                    lm =>
                        lm.Category != MessageCategory.Info
                        && lm.Category != MessageCategory.Diagnostic
                )
                .ToList();
            loggedMessages.Sort();

            Assert.That(
                loggedMessages.Select(m => m.ToString()),
                Is.EquivalentTo(
                    File.ReadAllLines(
                        TestsDirectory.Combine($"TestCases/Dependencies/SortedWarnings.txt")
                    )
                )
            );
        }
        ...
        public void InvalidWarningCodeThrows()
        {
            ...
            {
                Assert.AreEqual(
                    "The provided code '2500' does not fall into the permitted range for external warnings. To avoid possible "
                        + "collisions with existing and future ILLink warnings, external messages should use codes starting from 6001. (Parameter 'code')",
                    ex.Message
                );
            }
        }
        ...
        public void CanDumpDependenciesToUncompressedXml()
        {
            ...
            using (var reader = new XmlTextReader(outputPath.ToString()))
            {
                ...
                Assert.That(
                    ...
                    $"Expected to be at the dependencies element, but the current node name is `{reader.Name}`"
                );
            }
        }
        ...
        public void CandumpDependenciesToUncompressedDgml()
        {
            ...
            using (var reader = new XmlTextReader(outputPath.ToString()))
            {
                ...
                Assert.That(
                    reader.Name,
                    Is.EqualTo("DirectedGraph"),
                    $"Expected to be at the DirectedGraph element, but the current node name is `{reader.Name}`"
                );
            }
        }
        ...
        public void CanEnableReducedTracing()
        {
            ...
            Assert.That(
                lineCount,
                Is.LessThan(expectedMaxLines),
                $"There were `{lineCount}` lines in the dump file.  This is more than expected max of {expectedMaxLines} and likely indicates reduced tracing was not enabled.  Dump file can be found at: {outputPath}"
            );
        }
