...
public class ILChecker
{
    public virtual void Check(TrimmedTestCaseResult linkResult, AssemblyDefinition original)
    {
        ...
        ProcessExpectILFailures(
            linkResult,
            original,
            out bool expectILFailures,
            out List<string> expectedFailureMessages
        );
        ...
        if (expectILFailures)
        {
            ...
            foreach (var expectedFailureMessage in expectedFailureMessages)
            {
                ...
                {
                    Assert.Fail(
                        $"Expected IL failure message '{expectedFailureMessage}' was not found in the actual IL failure messages:\n--------------------\n{failures}"
                    );
                }
            }
        }
    ...
    private static bool DisableILDiffing(
    ...
    )
    {
        return linkResult.TestCase
                ...
                .CustomAttributes.FirstOrDefault(
                    attr => attr.AttributeType.Name == nameof(DisableILVerifyDiffingAttribute)
                ) != null;
    }

    private static void ProcessExpectILFailures(
        TrimmedTestCaseResult linkResult,
        AssemblyDefinition original,
        out bool expectILFailures,
    ...
    )
    {
        var attrs = linkResult.TestCase
            ...
            .CustomAttributes.Where(
                attr => attr.AttributeType.Name == nameof(ExpectILFailureAttribute)
            )
            .ToArray();
        ...
        foreach (var attr in attrs)
        {
            var expectedMessageContains = (
                (CustomAttributeArgument[])attr.GetConstructorArgumentValue(0)
            )
    ...
    )
    {
        var attrs = linkResult.TestCase
            .FindTypeDefinition(original)
            .CustomAttributes.Where(
                attr => attr.AttributeType.Name == nameof(SkipILVerifyAttribute)
            );
        ...
        foreach (var attr in attrs)
        {
            ...
            else
            {
                throw new ArgumentException(
                    $"Unhandled constructor argument type of {ctorArg.Type} on {nameof(SkipILVerifyAttribute)}"
                );
            }
        }
    }

    public static ILVerifierResult[] Diff(
        ILVerifierResult[] outputResults,
        ILVerifierResult[] inputResults
    )
    {
    ...
    {
        return new ILVerifier(
            new[] { directory, typeof(object).Assembly.Location.ToNPath().Parent },
            "System.Private.CoreLib"
        );
    }

    protected bool ShouldCheckAssembly(NPath file) =>
        file.ExtensionWithDot == ".exe"
        || file.ExtensionWithDot == ".dll"
        || file.ExtensionWithDot == ".winmd";

    public record class ErrorKey(
        VerifierError Code,
        string TypeFullName,
        string MethodName,
