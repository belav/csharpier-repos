...
public class ILVerifier : IResolver, IDisposable
{
    ...
    public ILVerifierResult[] Verify(NPath assemblyPath) =>
        Verify(LoadAssemblyFromPath(assemblyPath));
    ...
    public ILVerifierResult[] Verify(PEReader reader)
    {
        ...
        return FilterResults(results).Select(r => CreateResult(r, metadataReader)).ToArray();
    }

    private static ILVerifierResult CreateResult(
        ...
        new(
            ...
            r.Method.IsNil ? string.Empty : r.Method.GetMethodSignature(metadataReader)
        );

    protected virtual IEnumerable<VerificationResult> FilterResults(
    ...
    bool TryLoadAssemblyFromFolder(
        string assemblyName,
        NPath folder,
