    ...
    public class ResultChecker
    {
        ...
        static bool ShouldValidateIL(AssemblyDefinition inputAssembly)
        {
            ...
            var caaIsUnsafeFlag = (CustomAttributeArgument caa) =>
                caa.Type.IsTypeOf(WellKnownType.System_String) && (string)caa.Value == "/unsafe";
            var customAttributeHasUnsafeFlag = (CustomAttribute ca) =>
                ca.ConstructorArguments.Any(caaIsUnsafeFlag);
            if (
                GetCustomAttributes(inputAssembly, nameof(SetupCompileArgumentAttribute))
        ...
        protected virtual AssemblyChecker CreateAssemblyChecker(
            ...
            LinkedTestCaseResult linkedTestCase
        )
        {
        ...
        protected virtual void AdditionalChecking(
            LinkedTestCaseResult linkResult,
            AssemblyDefinition original
        )
        {
        ...
        )
        {
                                ...
                                foreach (var loggedMessage in loggedMessages)
                                {
                                    ...
                                    else if (isCompilerGeneratedCode == true)
                                    {
                                        if (
                                            ...
                                            is IMemberDefinition memberDefinition
                                        )
                                        {
                                            if (
            ...
            foreach ((var attrProvider, var attr) in expectedNoWarningsAttributes)
            {
                ...
                foreach (var mc in logger.GetLoggedMessages())
                {
                    ...
                    if (
                        ...
                        && member.FullName.Contains(attrProvider.FullName) != true
                    )
                        continue;
            ...
            )
            {
                ...
                if (
                    actualMember != null
                    && actualMember?.DeclaringType == null
