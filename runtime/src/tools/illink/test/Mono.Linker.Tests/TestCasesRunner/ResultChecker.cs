...
namespace Mono.Linker.Tests.TestCasesRunner
{
    public class ResultChecker
    {
        ...
        public ResultChecker(
            ...
            BaseAssemblyResolver linkedResolver,
            ...
            foreach (var typeRef in linked.MainModule.GetTypeReferences())
            {
                switch (typeRef.Scope)
                {
                    case null:
                        ...
                        var exportedType = linked.MainModule.ExportedTypes.SingleOrDefault(et =>
                            et.FullName == typeRef.FullName
                        );
                        Assert.IsNotNull(
                            exportedType,
                            $"Type reference '{typeRef.FullName}' with null scope has no ExportedType row"
                        );
                        ...
                        switch (exportedType.Scope)
                        {
                            case AssemblyNameReference:
                                ...
                                var assemblyRef = linked.MainModule.AssemblyReferences.Single(ar =>
                                    ar.Name == exportedType.Scope.Name
                                );
                                Assert.IsNotNull(
                                    assemblyRef,
                                    $"Exported type '{exportedType.FullName}' has a reference to assembly '{exportedType.Scope.Name}' which is not a reference of '{linked.FullName}'"
                                );
                                break;
                            default:
                                throw new NotImplementedException(
                                    $"Unexpected scope type '{exportedType.Scope.GetType()}' for exported type '{exportedType.FullName}'"
                                );
                        }
                        continue;
                    case AssemblyNameReference:
                    {
                        // There should be an AssemblyRef row for this assembly
                        var assemblyRef = linked.MainModule.AssemblyReferences.Single(ar =>
                            ar.Name == typeRef.Scope.Name
                        );
                        Assert.IsNotNull(
                            assemblyRef,
                            $"Type reference '{typeRef.FullName}' has a reference to assembly '{typeRef.Scope.Name}' which is not a reference of '{linked.FullName}'"
                        );
                        continue;
                    }
                    default:
                        throw new NotImplementedException(
                            $"Unexpected scope type '{typeRef.Scope.GetType()}' for type reference '{typeRef.FullName}'"
                        );
                }
            }
        }
        ...
        public virtual void Check(TrimmedTestCaseResult linkResult)
        {
            ...
            try
            {
                var original = ResolveOriginalsAssembly(
                    linkResult.ExpectationsAssemblyPath.FileNameWithoutExtension
                );
                ...
                {
                    Assert.IsTrue(
                        linkResult.OutputAssemblyPath.FileExists(),
                        $"The linked output assembly was not found.  Expected at {linkResult.OutputAssemblyPath}"
                    );

                    var linked = ResolveLinkedAssembly(
                        linkResult.OutputAssemblyPath.FileNameWithoutExtension
                    );
                    ...
                    if (
                        !HasActiveSkipKeptItemsValidationAttribute(
                            linkResult.TestCase.FindTypeDefinition(original)
                        )
                    )
                    {
            ...
            bool HasActiveSkipKeptItemsValidationAttribute(ICustomAttributeProvider provider)
            {
                ...
                {
                    object by = attribute.GetPropertyValue(
                        nameof(SkipKeptItemsValidationAttribute.By)
                    );
        ...
        protected virtual AssemblyChecker CreateAssemblyChecker(
            AssemblyDefinition original,
            AssemblyDefinition linked,
            TrimmedTestCaseResult linkedTestCase
        )
        {
        ...
        protected AssemblyDefinition ResolveLinkedAssembly(string assemblyName)
        {
            ...
            return _linkedResolver.Resolve(
                new AssemblyNameReference(cleanAssemblyName, null),
                _linkedReaderParameters
            );
        }
        ...
        void PerformOutputAssemblyChecks(AssemblyDefinition original, NPath outputDirectory)
        {
            var assembliesToCheck = original
                .MainModule.Types.SelectMany(t => t.CustomAttributes)
            ...
            foreach (var assemblyAttr in assembliesToCheck)
            {
                ...
                if (assemblyAttr.AttributeType.Name == nameof(RemovedAssemblyAttribute))
                    Assert.IsFalse(
                        expectedPath.FileExists(),
                        $"Expected the assembly {name} to not exist in {outputDirectory}, but it did"
                    );
                else if (assemblyAttr.AttributeType.Name == nameof(KeptAssemblyAttribute))
                    Assert.IsTrue(
                        expectedPath.FileExists(),
                        $"Expected the assembly {name} to exist in {outputDirectory}, but it did not"
                    );
                else if (assemblyAttr.AttributeType.Name == nameof(SetupLinkerActionAttribute))
                {
                    ...
                    {
                        VerifyCopyAssemblyIsKeptUnmodified(
                            outputDirectory,
                            assemblyName + (assemblyName == "test" ? ".exe" : ".dll")
                        );
                    }
                ...
                else if (assemblyAttr.AttributeType.Name == nameof(SetupLinkerTrimModeAttribute))
                {
                    ...
                    throw new NotImplementedException(
                        $"Unknown assembly assertion of type {assemblyAttr.AttributeType}"
                    );
            }
            ...
            {
                foreach (
                    string assemblyName in Directory.GetFiles(
        ...
        void PerformOutputSymbolChecks(AssemblyDefinition original, NPath outputDirectory)
        {
            ...
            foreach (var symbolAttr in symbolFilesToCheck)
            {
                if (symbolAttr.AttributeType.Name == nameof(RemovedSymbolsAttribute))
                    ...
                    throw new NotImplementedException(
                        $"Unknown symbol file assertion of type {symbolAttr.AttributeType}"
                    );
            }
        }
        ...
        void VerifyExitCode(TrimmedTestCaseResult linkResult, AssemblyDefinition original)
        {
            if (
                TryGetCustomAttribute(
                    original,
                    nameof(ExpectNonZeroExitCodeAttribute),
                    out var attr
                )
            )
            {
                ...
                Assert.AreEqual(
                    expectedExitCode,
                    linkResult.ExitCode,
                    $"Expected exit code {expectedExitCode} but got {linkResult.ExitCode}.  Output was:\n{FormatLinkerOutput()}"
                );
            }
            else
            {
                ...
                {
                    Assert.Fail(
                        $"Linker exited with an unexpected non-zero exit code of {linkResult.ExitCode} and output:\n{FormatLinkerOutput()}"
                    );
                }
            }
        ...
        void VerifyKeptSymbols(CustomAttribute symbolsAttribute)
        {
            ...
            if (
                linkedAssembly.MainModule.SymbolReader.GetType()
                != originalAssembly.MainModule.SymbolReader.GetType()
            )
                Assert.Fail(
                    $"Expected symbol provider of type `{originalAssembly.MainModule.SymbolReader}`, but was `{linkedAssembly.MainModule.SymbolReader}`"
                );
        }
        ...
        void VerifyRemovedSymbols(CustomAttribute symbolsAttribute, NPath outputDirectory)
        {
            ...
            try
            {
                ...
                if (linkedAssembly.MainModule.SymbolReader != null)
                    Assert.Fail(
                        $"Expected no symbols to be found for assembly `{linkedAssembly.MainModule.FileName}`, however, symbols were found of type {linkedAssembly.MainModule.SymbolReader}"
                    );
            }
            catch (AssemblyResolutionException)
            {
                ...
                if (possibleSymbolFilePath.Exists())
                    Assert.Fail(
                        $"Expected no symbols to be found for assembly `{assemblyName}`, however, a symbol file was found at {possibleSymbolFilePath}"
                    );
        ...
        protected virtual void AdditionalChecking(
            TrimmedTestCaseResult linkResult,
            AssemblyDefinition original
        )
        {
        ...
        protected virtual void InitialChecking(
            TrimmedTestCaseResult linkResult,
            AssemblyDefinition original,
            AssemblyDefinition linked
        )
        ...
        void VerifyLinkingOfOtherAssemblies(AssemblyDefinition original)
        {
                    ...
                    foreach (var checkAttrInAssembly in checks[assemblyName])
                    {
                        ...
                        if (linkedType == null && linkedAssembly.MainModule.HasExportedTypes)
                        {
                            ExportedType exportedType =
                                linkedAssembly.MainModule.ExportedTypes.FirstOrDefault(exported =>
                                    exported.FullName == expectedTypeName
                                );
                        ...
                        switch (attributeTypeName)
                        {
                            ...
                            case nameof(RemovedForwarderAttribute):
                                if (
                                    linkedAssembly.MainModule.ExportedTypes.Any(l =>
                                        l.Name == expectedTypeName
                                    )
                                )
                                    Assert.Fail(
                            ...
                            case nameof(RemovedAssemblyReferenceAttribute):
                                Assert.False(
                                    linkedAssembly.MainModule.AssemblyReferences.Any(l =>
                                        l.Name == expectedTypeName
                                    ),
            ...
            {
                Assert.Fail(
                    $"Failed to resolve linked assembly `{e.AssemblyReference.Name}`.  It must not exist in any of the output directories:\n\t{_linkedResolver.GetSearchDirectories().Aggregate((buff, s) => $"{buff}\n\t{s}")}\n"
                );
            }
        }

        void VerifyKeptAttributeInAssembly(
        ...
        void VerifyRemovedAttributeInAssembly(
            CustomAttribute inAssemblyAttribute,
            AssemblyDefinition linkedAssembly
        )
        {
        ...
        void VerifyAttributeInAssembly(
            CustomAttribute inAssemblyAttribute,
            AssemblyDefinition linkedAssembly,
        ...
        void VerifyCustomAttributeKept(
        ...
        void VerifyCustomAttributeRemoved(
            ICustomAttributeProvider provider,
            string expectedAttributeTypeName
        )
        {
        ...
        void VerifyRemovedInterfaceOnTypeInAssembly(
            CustomAttribute inAssemblyAttribute,
        ...
        void VerifyKeptInterfaceOnTypeInAssembly(
            CustomAttribute inAssemblyAttribute,
            TypeDefinition linkedType
        )
        {
        ...
        )
        {
            ...
            Assert.That(
                originalBase.FullName,
                Is.EqualTo(linkedType.BaseType.FullName),
                $"Incorrect base on `{linkedType.FullName}`.  Expected `{originalBase.FullName}` but was `{linkedType.BaseType.FullName}`"
            );
        }

        protected static InterfaceImplementation GetMatchingInterfaceImplementationOnType(
            TypeDefinition type,
        ...
        void VerifyRemovedMemberInAssembly(
            CustomAttribute inAssemblyAttribute,
            TypeDefinition linkedType
        )
        {
        ...
        void VerifyKeptMemberInAssembly(
            CustomAttribute inAssemblyAttribute,
            TypeDefinition linkedType
        )
        {
        ...
        void VerifyKeptReferencesInAssembly(CustomAttribute inAssemblyAttribute)
        {
            ...
            Assert.That(
                ...
                Is.EquivalentTo(expectedReferenceNames)
            );
        }
        ...
        void VerifyKeptResourceInAssembly(CustomAttribute inAssemblyAttribute)
        {
            ...
            Assert.That(
                assembly.MainModule.Resources.Select(r => r.Name),
                Has.Member(resourceName)
            );
        }
        ...
        void VerifyKeptAllTypesAndMembersInAssembly(AssemblyDefinition linked)
        {
            ...
            Assert.That(
                missingInLinked,
                Is.Empty,
                $"Expected all types to exist in the linked assembly {linked.Name}, but one or more were missing"
            );
        ...
        static IEnumerable<ICustomAttributeProvider> GetAttributeProviders(
        ...
        )
        {
                        ...
                        case nameof(LogContainsAttribute):
                            {
                                ...
                                if ((bool)attr.ConstructorArguments[1].Value)
                                    ...
                                    matchedMessages = loggedMessages
                                        .Where(m => m.ToString().Contains(expectedMessage))
                                        .ToList();
                                ;
                                Assert.IsTrue(
                        ...
                        case nameof(ExpectedWarningAttribute):
                            {
                                ...
                                foreach (var loggedMessage in loggedMessages)
                                {
                                    ...
                                    {
                                        if (
                                            loggedMessage.Origin?.Provider
                                            is not IMemberDefinition memberDefinition
                                        )
                                            continue;
                                        ...
                                        {
                                            string actualName =
                                                memberDefinition.DeclaringType.FullName
                                                + "."
                                            ...
                                            if (
                                                ...
                                                && (
                                                    ...
                                                    || actualName.EndsWith(
                                                        "get_" + expectedMember.Name
                                                    )
                                                    || actualName.EndsWith(
                                                        "set_" + expectedMember.Name
                                                    )
                                                )
                                            )
                                            {
                                        ...
                                        )
                                        {
                                            ...
                                            if (
                                                ...
                                                && memberDefinition
                                                    ...
                                                    .Assembly
                                                    .Name
                                                    .Name == expectedAssembly.Name.Name
                                            )
                                            {
                                                expectedWarningFound = true;
                                                loggedMessages.Remove(loggedMessage);
            ...
            foreach ((var attrProvider, var attr) in expectedNoWarningsAttributes)
            {
                ...
                foreach (var mc in logger.GetLoggedMessages())
                {
                    ...
                    if (
                        attrProvider is IMemberDefinition attrMember
                        && (mc.Origin?.Provider is IMemberDefinition member)
                        && member.FullName.Contains(attrMember.FullName) != true
                    )
                        continue;
            ...
            {
                var remainingErrors = loggedMessages.Where(m =>
                    Regex.IsMatch(m.ToString(), @".*(error | warning): \d{4}.*")
                );
                Assert.IsEmpty(
                    remainingErrors,
            ...
            bool LogMessageHasSameOriginMember(
                MessageContainer mc,
            ...
            )
            {
                ...
                if (origin?.Provider is AssemblyDefinition asm)
                    return expectedOriginProvider is AssemblyDefinition expectedAsm
                        && asm.Name.Name == expectedAsm.Name.Name;
        ...
        void VerifyRecordedDependencies(
            AssemblyDefinition original,
        ...
        void VerifyExpectedInstructionSequenceOnMemberInAssembly(
            CustomAttribute inAssemblyAttribute,
            TypeDefinition linkedType
        )
        {
        ...
        static string GetFullMemberNameFromDefinition(
            IMetadataTokenProvider member,
            out string genericMember
        )
        {
            ...
            throw new NotImplementedException(
                $"Getting the full member name has not been implemented for {member}"
            );
        }

        protected TypeDefinition GetOriginalTypeFromInAssemblyAttribute(
            CustomAttribute inAssemblyAttribute
        )
        {
            string assemblyName;
        ...
        Dictionary<string, List<CustomAttribute>> BuildOtherAssemblyCheckTable(
            AssemblyDefinition original
        )
        {
...
#nullable enable
        static bool TryGetCustomAttribute(
            ICustomAttributeProvider caProvider,
            string attributeName,
            [NotNullWhen(true)] out CustomAttribute? customAttribute
        )
        {
            ...
            {
                customAttribute = type.CustomAttributes.FirstOrDefault(
                    attr => attr!.AttributeType.Name == attributeName,
                    null
                );
                return customAttribute is not null;
        ...
        static IEnumerable<CustomAttribute> GetCustomAttributes(
            ICustomAttributeProvider caProvider,
            string attributeName
        )
        {
            if (caProvider is AssemblyDefinition assembly && assembly.EntryPoint != null)
                return assembly.EntryPoint.DeclaringType.CustomAttributes.Where(attr =>
