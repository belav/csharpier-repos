...
namespace Mono.Linker.Tests.TestCasesRunner
{
    public class PeVerifier
    {
        ...
        {
            ProcessSkipAttributes(
                linkResult,
                original,
                out bool skipCheckEntirely,
        ...
        private void ProcessSkipAttributes(
        ...
        )
        {
            ...
            foreach (var attr in peVerifyAttrs)
            {
                ...
                else if (ctorArg.Type.Name == nameof(SkipPeVerifyForToolchian))
                {
                    ...
                    if (skipToolchain == SkipPeVerifyForToolchian.Pedump)
                    {
                        ...
                        throw new ArgumentException(
                            $"Unhandled platform and toolchain values of {Environment.OSVersion.Platform} and {skipToolchain}"
                        );
                }
                ...
                {
                    throw new ArgumentException(
                        $"Unhandled constructor argument type of {ctorArg.Type} on {nameof(SkipPeVerifyAttribute)}"
                    );
                }
            }
        }
        ...
        protected virtual void SetupProcess(Process process, NPath assemblyPath)
        {
            var exeArgs =
                Environment.OSVersion.Platform == PlatformID.Win32NT
                    ? $"/nologo {assemblyPath.InQuotes()}"
                    : $"--verify metadata,code {assemblyPath.InQuotes()}";
