    ...
    public class TestCaseCollector
    {
        ...
        NPath FindTestCaseAssembly(NPath sourceFile)
        {
            ...
            do
            {
                ...
                if (current.Parent.Files("*.csproj").FirstOrDefault() is NPath csproj)
                {
                    ...
                    return _testCaseAssemblyRoot
                        .Combine(relative)
                        .Combine(csproj.ChangeExtension("dll").FileName);
                }
        ...
        public TestCase CreateIndividualCase(Type testCaseType)
        {
            ...
            var pathRelativeToAssembly =
                $"{testCaseType.FullName.Substring(testCaseType.Module.Name.Length - 3).Replace('.', '/')}.cs";
            ...
            if (!CreateCase(testCaseAssemblyPath, fullSourcePath, out TestCase testCase))
                throw new ArgumentException(
                    $"Could not create a test case for `{testCaseType}`.  Ensure the namespace matches it's location on disk"
                );
        ...
        {
            using AssemblyDefinition caseAssemblyDefinition = AssemblyDefinition.ReadAssembly(
                caseAssemblyPath.ToString()
            );
            ...
            {
                Console.WriteLine(
                    $"Could not find the matching type for test case {sourceFile}.  Ensure the file name and class name match"
                );
                return false;
            }
            ...
            MethodDefinition mainMethod = typeDefinition.Methods.FirstOrDefault(m =>
                m.Name
                == (
            ...
            {
                Console.WriteLine(
                    $"The Main() method for {typeDefinition} in {sourceFile} should be static"
                );
                return false;
            }
