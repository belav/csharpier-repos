    ...
    public class TestCaseSandbox
    {
        ...
        static NPath GetArtifactsTestPath()
        {
            ...
            string artifacts = Path.GetFullPath(
                Path.Combine(Path.GetDirectoryName(_linkerAssemblyPath), "..", "..", "..", "..")
            );
        ...
        public TestCaseSandbox(TestCase testCase, NPath rootTemporaryDirectory, string namePrefix)
        {
            ...
            var rootDirectory = rootTemporaryDirectory.Combine(
        ...
        public virtual void Populate(TestCaseCompilationMetadataProvider metadataProvider)
        {
            ...
            foreach (var fileName in metadataProvider.GetReferenceValues())
            {
                if (
                    ...
                    CopyToInputAndExpectations(
                        _testCase.SourceFile.Parent.Combine(fileName.ToNPath())
                    );
            }

            foreach (var referenceDependency in metadataProvider.GetReferenceDependencies())
                CopyToInputAndExpectations(
                    _testCase.SourceFile.Parent.Combine(referenceDependency.ToNPath())
                );
            ...
            {
                var destination = BeforeReferenceSourceDirectoryFor(compileRefInfo.OutputName)
                    .EnsureDirectoryExists();
                compileRefInfo.SourceFiles.Copy(destination);

                destination = BeforeReferenceResourceDirectoryFor(compileRefInfo.OutputName)
                    .EnsureDirectoryExists();
            ...
            {
                var destination = AfterReferenceSourceDirectoryFor(compileRefInfo.OutputName)
                    .EnsureDirectoryExists();
                compileRefInfo.SourceFiles.Copy(destination);

                destination = AfterReferenceResourceDirectoryFor(compileRefInfo.OutputName)
                    .EnsureDirectoryExists();
        ...
        {
            return _directory.Combine(
                $"ref_source_before_{Path.GetFileNameWithoutExtension(outputName)}"
            );
        }
        ...
        {
            return _directory.Combine(
                $"ref_source_after_{Path.GetFileNameWithoutExtension(outputName)}"
            );
        }
        ...
        {
            return _directory.Combine(
                $"ref_resource_before_{Path.GetFileNameWithoutExtension(outputName)}"
            );
        }
        ...
        {
            return _directory.Combine(
                $"ref_resource_after_{Path.GetFileNameWithoutExtension(outputName)}"
            );
        }
    }
}
