...
namespace Mono.Linker.Tests.TestCasesRunner
{
    ...
    {
        readonly AssemblyDefinition originalAssembly,
            linkedAssembly;
        ...
        static bool IsBackingField(FieldDefinition field) =>
        ...
        protected virtual void VerifyTypeDefinition(TypeDefinition original, TypeDefinition linked)
        {
            ...
            {
                foreach (
                    var attr in original.CustomAttributes
                ...
                )
                {
                    ...
                    Assert.AreEqual(
                        1,
                        linkedMembers.RemoveWhere(l => l.Contains(newName)),
                        $"Newly created member '{newName}' was not found"
                    );
                }
            }
        }
        ...
        {
            foreach (
                var keptByAttribute in src.CustomAttributes
                    .Where(ca => ca.AttributeType.IsTypeOf<KeptByAttribute>())
            )
        ...
        void VerifyKeptByAttribute(string keptAttributeProviderName, CustomAttribute attribute)
        {
            ...
            if (attribute.ConstructorArguments.Count == 2)
            {
                ...
                if (attribute.ConstructorArguments[0].Type.IsTypeOf<string>())
                    ...
                    expectedDependency.Source = (
                        (TypeDefinition)attribute.ConstructorArguments[0].Value
                    ).FullName;
                else
            ...
            string errorMessage =
                $"{keptAttributeProviderName} was expected to be kept by {expectedDependency.Source} with reason {expectedDependency.DependencyKind.ToString()}.";
        ...
        void VerifyBaseType(TypeDefinition src, TypeDefinition linked)
        {
            ...
            Assert.AreEqual(
                expectedBaseName,
                linked.BaseType?.FullName,
                $"Incorrect base type on : {linked.Name}"
            );
        }
        ...
        void VerifyOverrides(MethodDefinition original, MethodDefinition linked)
        {
            ...
            var originalBaseTypesOverridden = new HashSet<string>(
                original.Overrides.Select(ov => ov.DeclaringType.FullName)
            );
            var linkedBaseTypesOverridden = new HashSet<string>(
                linked.Overrides.Select(ov => ov.DeclaringType.FullName)
            );
            foreach (var expectedBaseType in expectedBaseTypesOverridden)
            {
                Assert.IsTrue(
                    originalBaseTypesOverridden.Contains(expectedBaseType),
                    $"Method {linked.FullName} was expected to keep override {expectedBaseType}::{linked.Name}, "
                        + "but it wasn't in the unlinked assembly"
                );
                Assert.IsTrue(
                    linkedBaseTypesOverridden.Contains(expectedBaseType),
            ...
            {
                Assert.IsTrue(
                    originalBaseTypesOverridden.Contains(expectedRemovedBaseType),
                    $"Method {linked.FullName} was expected to remove override {expectedRemovedBaseType}::{linked.Name}, "
                        + $"but it wasn't in the unlinked assembly"
                );
                Assert.IsFalse(
                    linkedBaseTypesOverridden.Contains(expectedRemovedBaseType),
            ...
            foreach (var overriddenMethod in linked.Overrides)
            {
                ...
                {
                    Assert.Fail(
                        $"Method {linked.GetDisplayName()} overrides method {overriddenMethod} which does not exist"
                    );
                }
                else if (overriddenDefinition.DeclaringType.IsInterface)
                {
                    Assert.True(
                        linked.DeclaringType
                            .Interfaces
                ...
                else
                {
                    ...
                    while (baseType is not null)
                    {
                        ...
                        if (baseType.Resolve()?.BaseType is null)
                            Assert.Fail(
                                $"Method {linked} overrides method {overriddenMethod} from, but {linked.DeclaringType} does not inherit from type {overriddenMethod.DeclaringType}"
                            );
                    }
                }
            }
        ...
        void VerifyField(FieldDefinition src, FieldDefinition linked)
        {
            ...
            bool expectedKept =
                ShouldBeKept(src) || (compilerGenerated ? !IsBackingField(src) : false);
        ...
        protected virtual void VerifyMethodKept(
            MethodDefinition src,
            MethodDefinition linked,
            bool compilerGenerated
        )
        ...
        static string FormatInstruction(Instruction instr)
        {
            switch (instr.OpCode.FlowControl)
            {
                ...
                case FlowControl.Cond_Branch:
                    ...
                    {
                        string stargets = string.Join(
                            ", ",
                            targets.Select(l => $"il_{l.Offset.ToString("x")}")
                        );
        ...
        void VerifyResources(AssemblyDefinition original, AssemblyDefinition linked)
        {
            ...
            foreach (var resource in linked.MainModule.Resources)
            {
                ...
                Assert.That(
                    ...
                    Is.EquivalentTo(expectedResource.GetResourceData()),
            ...
            Assert.IsEmpty(
                ...
                $"Resource '{expectedResourceNames.FirstOrDefault()}' should be kept."
            );
        }
        ...
        void VerifyExportedTypes(AssemblyDefinition original, AssemblyDefinition linked)
        {
            ...
            Assert.That(
                linked.MainModule.ExportedTypes.Select(l => l.FullName),
        ...
        protected virtual void VerifyPseudoAttributes(MethodDefinition src, MethodDefinition linked)
        {
            ...
            Assert.AreEqual(
                expected,
                linked.Attributes,
                $"Method `{src}' pseudo attributes did not match expected"
            );
        }
        ...
        protected virtual void VerifyPseudoAttributes(TypeDefinition src, TypeDefinition linked)
        {
            ...
            Assert.AreEqual(
                expected,
                linked.Attributes,
                $"Type `{src}' pseudo attributes did not match expected"
            );
        }
        ...
        protected virtual void VerifyPseudoAttributes(FieldDefinition src, FieldDefinition linked)
        {
            ...
            Assert.AreEqual(
                expected,
                linked.Attributes,
                $"Field `{src}' pseudo attributes did not match expected"
            );
        }
        ...
        )
        {
            ...
            Assert.AreEqual(
                expected,
                linked.Attributes,
                $"Property `{src}' pseudo attributes did not match expected"
            );
        }
        ...
        protected virtual void VerifyPseudoAttributes(EventDefinition src, EventDefinition linked)
        {
            ...
            Assert.AreEqual(
                expected,
                linked.Attributes,
                $"Event `{src}' pseudo attributes did not match expected"
            );
        }
        ...
        )
        {
            ...
            Assert.That(
                linkedAttrs,
                Is.EquivalentTo(expectedAttrs),
                $"Custom attributes on `{src}' are not matching"
            );
        }
        ...
        )
        {
            ...
            Assert.That(
                linkedAttrs,
                Is.EquivalentTo(expectedAttrs),
                $"Security attributes on `{src}' are not matching"
            );
        }

        void VerifyPrivateImplementationDetails(TypeDefinition original, TypeDefinition linked)
        {
            var expectedImplementationDetailsMethods = GetCustomAttributeCtorValues<string>(
                    original,
                    nameof(KeptPrivateImplementationDetailsAttribute)
                )
                .Select(attr => attr.ToString())
                .ToList();
            ...
            VerifyPrivateImplementationDetailsType(
                original.Module,
                linked.Module,
                out TypeDefinition srcImplementationDetails,
                out TypeDefinition linkedImplementationDetails
            );
            foreach (var methodName in expectedImplementationDetailsMethods)
            {
                var originalMethod = srcImplementationDetails.Methods
                ...
                var linkedMethod = linkedImplementationDetails.Methods
                    .FirstOrDefault(m => m.Name == methodName);
        ...
        static void VerifyPrivateImplementationDetailsType(
            ModuleDefinition src,
            ModuleDefinition linked,
            out TypeDefinition srcImplementationDetails,
            out TypeDefinition linkedImplementationDetails
        )
        {
            srcImplementationDetails = src.Types
            ...
            linkedImplementationDetails = linked.Types
                .FirstOrDefault(t => IsPrivateImplementationDetailsType(t));
        ...
        )
        {
            ...
            var expectKeptAll = src.CustomAttributes
                .Any(attr =>
            ...
            if (expectKeptAll)
            {
                ...
                {
                    var linkedField = linkedImplementationDetails.Fields
        ...
        static bool IsLdtokenOnPrivateImplementationDetails(
        ...
        void VerifyFixedBufferFields(TypeDefinition src, TypeDefinition linked)
        {
            var fields = src.Fields
                .Where(f =>
                    f.CustomAttributes
                        .Any(attr => attr.AttributeType.Name == nameof(KeptFixedBufferAttribute))
                );
            ...
            foreach (var field in fields)
            {
                ...
                // Some versions of csc name it `<fieldname>e__FixedBuffer0`
                // while mcs and other versions of csc name it `<fieldname>__FixedBuffer0`
                var originalCompilerGeneratedBufferType = src.NestedTypes
                    .FirstOrDefault(t =>
                ...
                var linkedCompilerGeneratedBufferType = linked.NestedTypes
                ...
                var originalElementField = originalCompilerGeneratedBufferType.Fields
                    .FirstOrDefault();
        ...
        void VerifyDelegateBackingFields(TypeDefinition src, TypeDefinition linked)
        {
            ...
            foreach (var nestedType in src.NestedTypes)
            {
                ...
                foreach (var expectedFieldName in expectedFieldNames)
                {
                    ...
                    var linkedField = linkedNestedType?.Fields
                        .FirstOrDefault(f => f.Name == expectedFieldName);
                    ...
                    if (checkNames)
                    {
                        ...
                        )
                        {
                            ...
                            Assert.AreEqual(
                                name,
                                lnkp.Name,
                                "Expected empty generic parameter name"
                            );
                        }
                        else
                        {
                            Assert.AreEqual(
                                srcp.Name,
                                lnkp.Name,
                                "Mismatch in generic parameter name"
                            );
                        }
                    }
                }
                ...
                for (int i = 0; i < src.Parameters.Count; ++i)
                {
                    ...
                    if (checkNames)
                    {
                        if (
                            ...
                            Assert.IsEmpty(
                                lnkp.Name,
                                $"Expected empty parameter name. Parameter {i} of {(src as MethodDefinition)}"
                            );
                        else
                            Assert.AreEqual(
                                srcp.Name,
                                lnkp.Name,
                                $"Mismatch in parameter name. Parameter {i} of {(src as MethodDefinition)}"
                            );
                    }
                }
            }
