...
namespace Mono.Linker.Tests.TestCasesRunner
{
    partial class AssemblyChecker
    {
        ...
        {
            foreach (
                var keptByAttribute in src.CustomAttributes.Where(
                    ca => ca.AttributeType.IsTypeOf<KeptByAttribute>()
                )
            )
        ...
        void VerifyOverrides(MethodDefinition original, MethodDefinition linked)
        {
            ...
            foreach (var overriddenMethod in linked.Overrides)
            {
                ...
                {
                    Assert.True(
                        linked.DeclaringType.Interfaces
        ...
        void VerifyPrivateImplementationDetails(TypeDefinition original, TypeDefinition linked)
        {
            ...
            {
                var originalMethod = srcImplementationDetails.Methods.FirstOrDefault(
                    m => m.Name == methodName
                );
                ...
                var linkedMethod = linkedImplementationDetails.Methods.FirstOrDefault(
                    m => m.Name == methodName
                );
        ...
        {
            srcImplementationDetails = src.Types.FirstOrDefault(
                t => IsPrivateImplementationDetailsType(t)
            );
            ...
            linkedImplementationDetails = linked.Types.FirstOrDefault(
                t => IsPrivateImplementationDetailsType(t)
            );
        ...
        void VerifyDelegateBackingFields(TypeDefinition src, TypeDefinition linked)
        {
            ...
            foreach (var nestedType in src.NestedTypes)
            {
                ...
                var linkedNestedType = linked.NestedTypes.FirstOrDefault(
                    t => t.Name == nestedType.Name
                );
                foreach (var expectedFieldName in expectedFieldNames)
                {
                    var originalField = nestedType.Fields.FirstOrDefault(
                    ...
                    var linkedField = linkedNestedType?.Fields.FirstOrDefault(
                        f => f.Name == expectedFieldName
                    );
