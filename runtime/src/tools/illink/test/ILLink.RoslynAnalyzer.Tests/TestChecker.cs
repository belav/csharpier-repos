    ...
    internal sealed class TestChecker : CSharpSyntaxWalker
    {
        ...
        )
        {
            ...
            _diagnostics = diagnostics
                ...
                .Where(d => d.Location.SourceTree == tree || d.Location.SourceTree == null)
                .ToList();
        ...
        public void Check(bool allowMissingWarnings)
        {
            ...
            if (!allowMissingWarnings && _missing.Any())
            {
                var missingLines = string.Join(
                    ...
                    _missing.Select(
                        md =>
                            $"({md.Attribute.Parent?.Parent?.GetLocation().GetLineSpan()}) {md.Message}"
                    )
                );
                message +=
                    $@"Expected warnings were not generated:{Environment.NewLine}{missingLines}{Environment.NewLine}";
            }
            var unexpected = _unmatched.Where(
                diag =>
                    diag.Location.SourceTree == null
                    || _expectedNoWarnings.Any(
                        attr => attr.Parent?.Parent?.Span.Contains(diag.Location.SourceSpan) == true
                    )
            );
            if (unexpected.Any())
            {
                message +=
                    $"Unexpected warnings were generated:{Environment.NewLine}{string.Join(Environment.NewLine, unexpected)}";
            }
        ...
        private void ValidateDiagnostics(
            CSharpSyntaxNode memberSyntax,
            SyntaxList<AttributeListSyntax> attrLists
        )
        {
            var memberDiagnostics = _unmatched
                .Where(d =>
                {
                    ...
                    {
                        if (
                            _semanticModel.GetDeclaredSymbol(classSyntax)
                            is not ITypeSymbol typeSymbol
                        )
                            throw new NotImplementedException(
                                "Unable to get type symbol for class declaration syntax."
                            );

                        if (typeSymbol.Locations.Length != 1)
                            throw new NotImplementedException(
                                "Type defined in multiple source locations."
                            );
            ...
            foreach (var attrList in attrLists)
            {
                foreach (var attribute in attrList.Attributes)
                {
                    ...
                    if (
                        !TryValidateExpectedDiagnostic(
                            attribute,
                            memberDiagnostics,
                            out int? matchIndexResult,
                    ...
                    case BinaryExpressionSyntax binaryExpressionSyntax:
                        if (
                            ...
                            throw new ArgumentException(
                                "Expression must be a ProducedBy value",
                                nameof(expression)
                            );
        ...
        bool TryValidateExpectedDiagnostic(
            AttributeSyntax attribute,
            List<Diagnostic> diagnostics,
            [NotNullWhen(true)] out int? matchIndex,
            [NotNullWhen(false)] out string? missingDiagnosticMessage
        )
        {
            switch (attribute.Name.ToString())
            {
                case "ExpectedWarning":
                    return TryValidateExpectedWarningAttribute(
                        attribute!,
                        diagnostics,
                        out matchIndex,
                        out missingDiagnosticMessage
                    );
                case "LogContains":
                    return TryValidateLogContainsAttribute(
                        attribute!,
                        diagnostics,
                        out matchIndex,
                        out missingDiagnosticMessage
        ...
        private bool TryValidateExpectedWarningAttribute(
            AttributeSyntax attribute,
            List<Diagnostic> diagnostics,
            out int? matchIndex,
            out string? missingDiagnosticMessage
        )
        {
            ...
            matchIndex = null;
            var args = LinkerTestBase.GetAttributeArguments(attribute);
            string expectedWarningCode = LinkerTestBase.GetStringFromExpression(
                args["#0"],
                _semanticModel
            );
            ...
            if (!expectedWarningCode.StartsWith("IL"))
                throw new InvalidOperationException(
                    $"Expected warning code should start with \"IL\" prefix."
                );

            List<string> expectedMessages = args.Where(
                    arg => arg.Key.StartsWith("#") && arg.Key != "#0"
                )
            ...
            missingDiagnosticMessage =
                $"Warning '{expectedWarningCode}'. Expected to find warning containing:{string.Join(" ", expectedMessages.Select(m => "'" + m + "'"))}"
            ...
            {
                if (
                    !attribute.Parent?.Parent?.Span.Contains(diagnostic.Location.SourceSpan) == true
                )
                    return false;
        ...
        private bool TryValidateLogContainsAttribute(
            AttributeSyntax attribute,
            List<Diagnostic> diagnostics,
            out int? matchIndex,
            out string? missingDiagnosticMessage
        ...
        private void ValidateLogDoesNotContainAttribute(
            AttributeSyntax attribute,
            IReadOnlyList<Diagnostic> diagnosticMessages
        )
        {
            var args = LinkerTestBase.GetAttributeArguments(attribute);
            var arg = args["#0"];
            ...
            if (LogContains(attribute, diagnosticMessages, out var matchIndex, out var findText))
            {
                Assert.True(
                    false,
                    $"LogDoesNotContain failure: Text\n\"{findText}\"\nfound in diagnostic:\n {diagnosticMessages[(int)matchIndex]}"
                );
            }
        }

        private bool LogContains(
            AttributeSyntax attribute,
        ...
        )
        {
            ...
            bool isRegex =
                args.TryGetValue("regexMatch", out var regexMatchExpr)
