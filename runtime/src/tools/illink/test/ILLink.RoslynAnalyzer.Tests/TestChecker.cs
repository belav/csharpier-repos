    ...
    internal sealed class TestChecker : CSharpSyntaxWalker
    {
        ...
        public void Check(bool allowMissingWarnings)
        {
            ...
            var unexpected = _unmatched.Where(diag =>
                ...
                || _expectedNoWarnings.Any(attr =>
                    attr.Parent?.Parent?.Span.Contains(diag.Location.SourceSpan) == true
                )
            );
            if (unexpected.Any())
            {
        ...
        )
        {
            ...
            List<string> expectedMessages = args.Where(arg =>
                    arg.Key.StartsWith("#") && arg.Key != "#0"
                )
