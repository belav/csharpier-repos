        ...
        where TAnalyzer : DiagnosticAnalyzer, new()
    {
        ...
        )
        {
            ...
            for (var i = 0; i < diagnostics.Length; ++i)
            {
                ...
                var applicableAnalyzer = analyzers.FirstOrDefault(
                    a => a.SupportedDiagnostics.Any(dd => dd.Id == diagnosticsId)
                );
        ...
        )> MatchDiagnostics(Diagnostic[] actualResults, DiagnosticResult[] expectedResults)
        {
            ...
            var bestMatch = actualResults
                ...
                .Concat(
                    expectedResults.Select(
                        result => (default(Diagnostic?), (DiagnosticResult?)result)
                    )
                )
                .ToImmutableArray();
    ...
    internal static class IEnumerableExtensions
    {
        ...
        )
        {
            return diagnosticResults
                ...
                .ThenBy(
                    diagnosticResult => diagnosticResult.Spans.FirstOrDefault().Span.Span.Start.Line
                )
                .ThenBy(
                    diagnosticResult =>
                ...
                .ThenBy(
                    diagnosticResult => diagnosticResult.Spans.FirstOrDefault().Span.Span.End.Line
                )
                .ThenBy(
                    diagnosticResult =>
