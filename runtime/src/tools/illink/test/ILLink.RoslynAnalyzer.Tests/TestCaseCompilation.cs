...
namespace ILLink.RoslynAnalyzer.Tests
{
    internal static class TestCaseCompilation
    {
        ...
        public static Task<(
            ...
            List<Diagnostic> ExceptionDiagnostics
        )> CreateCompilation(
            string src,
        ...
        )
        {
            ...
            var comp = CSharpCompilation.Create(
                ...
                syntaxTrees: sources,
                references: (await TestCaseUtils.GetNet6References())
                    .Add(mdRef)
            ...
            return (
                new CompilationWithAnalyzers(
                    comp,
                    SupportedDiagnosticAnalyzers,
                    compWithAnalyzerOptions
                ),
                comp.GetSemanticModel(src),
        ...
        sealed class SimpleAnalyzerOptions : AnalyzerConfigOptionsProvider
        {
            public SimpleAnalyzerOptions((string, string)[]? globalOptions)
            {
                ...
                GlobalOptions = new SimpleAnalyzerConfigOptions(
                    ImmutableDictionary.CreateRange(
                        StringComparer.OrdinalIgnoreCase,
                        globalOptions.Select(x => new KeyValuePair<string, string>(
                            x.Item1,
                            x.Item2
                        ))
                    )
            ...
            {
                public static readonly SimpleAnalyzerConfigOptions Empty =
                    new SimpleAnalyzerConfigOptions(ImmutableDictionary<string, string>.Empty);
