...
build_property.{MSBuildPropertyOptionNames.EnableTrimAnalyzer} = true"
        ...
        public async Task CodeFix_IL2075_MethodAttributeLeavesOnCodeFix()
        {
            ...
            await VerifyDynamicallyAccessedMembersCodeFix(
                source: string.Concat(
                    DynamicallyAccessedMembersAnalyzerTests.GetSystemTypeBase(),
                    test
                ),
        ...
        public async Task CodeFix_IL2079_AttributeTurnsOffCodeFix()
        {
            ...
            var diag = new[]
            {
                ...
                // The source value must declare at least the same requirements as those declared on the target location it is assigned to.
                VerifyCS
                    .Diagnostic(DiagnosticId.DynamicallyAccessedMembersMismatchFieldTargetsField)
                    .WithSpan(14, 3, 14, 10)
                    .WithArguments("C.f2", "C.f1", "'DynamicallyAccessedMemberTypes.PublicMethods'")
            };
            await VerifyDynamicallyAccessedMembersCodeFix(test, test, diag, diag);
        }
        ...
        public async Task CodeFix_IL2082_MismatchThisParamTargetsParam()
        {
            ...
            await VerifyDynamicallyAccessedMembersCodeFix(
                ...
                fixedExpected: new[]
                {
                    ...
                    VerifyCS
                        .Diagnostic(DiagnosticId.ImplicitThisCannotBeStaticallyDetermined)
        ...
        public async Task CodeFix_IL2082_AttributeTurnsOffCodeFix()
        {
            ...
            await VerifyDynamicallyAccessedMembersCodeFix(
                string.Concat(DynamicallyAccessedMembersAnalyzerTests.GetSystemTypeBase(), test),
                string.Concat(DynamicallyAccessedMembersAnalyzerTests.GetSystemTypeBase(), test),
                diag,
                diag
            );
            ...
            await VerifyDynamicallyAccessedMembersCodeFix(
                ...
                baselineExpected: new[]
                {
                    ...
                    VerifyCS
                        ...
                        .WithSpan(14, 23, 14, 24)
                        .WithArguments("C.M(Type)", "Base.M(Type)")
                },
                fixedExpected: Array.Empty<DiagnosticResult>()
            );
        }
