...
namespace ILLink.RoslynAnalyzer.Tests
{
    public abstract class TestCaseUtils
    {
        ...
        public static readonly ReferenceAssemblies Net6PreviewAssemblies = new ReferenceAssemblies(
            "net7.0",
        ...
        public static async Task RunTestFile(
            string suiteName,
            string testName,
            bool allowMissingWarnings,
            params (string, string)[] msbuildProperties
        )
        {
        ...
        private static IEnumerable<string> GetTestDependencies(
            string testDir,
            SyntaxTree testSyntaxTree
        )
        {
            foreach (
                var attribute in testSyntaxTree
                    .GetRoot()
                    .DescendantNodes()
            ...
            )
            {
                ...
                switch (attributeName)
                {
                    case "SetupCompileBefore":
                    {
                        ...
                        foreach (
                            var sourceFile in args["#1"]
                                .DescendantNodes()
                                .OfType<LiteralExpressionSyntax>()
                        )
                            yield return Path.Combine(
                                testDir,
                                LinkerTestBase.GetStringFromExpression(sourceFile)
                            );
                        break;
                    }
        ...
        private static IEnumerable<AdditionalText> GetAdditionalFiles(
            ...
            foreach (var attribute in tree.GetRoot().DescendantNodes().OfType<AttributeSyntax>())
            {
                switch (attribute.Name.ToString())
                {
                    ...
                    case nameof(SetupCompileResourceAttribute):
                        ...
                        if (
                            args?.Count == 2
                            && args?[1].ToString() == "\"ILLink.LinkAttributes.xml\""
                        )
                            break;
                        continue;
                    default:
        ...
        public static void GetDirectoryPaths(
            out string rootSourceDirectory,
            out string testAssemblyPath
        )
        {
            var artifactsBinDirectory = (string)
                AppContext.GetData("ILLink.RoslynAnalyzer.Tests.ArtifactsBinDir")!;
            var LinkerTestDirectory = (string)
                AppContext.GetData("ILLink.RoslynAnalyzer.Tests.LinkerTestDir")!;
            var configuration = (string)
                AppContext.GetData("ILLink.RoslynAnalyzer.Tests.Configuration")!;

            const string tfm = "net7.0";

            rootSourceDirectory = Path.GetFullPath(
                Path.Combine(LinkerTestDirectory, "Mono.Linker.Tests.Cases")
            );
            testAssemblyPath = Path.GetFullPath(
                Path.Combine(
        ...
        )
        {
            ...
            switch (expr.Kind())
            {
                case SyntaxKind.AddExpression:
                    ...
                    return GetStringFromExpression(addExpr.Left, semanticModel)
                        + GetStringFromExpression(addExpr.Right, semanticModel);
        ...
        public static Dictionary<string, ExpressionSyntax> GetAttributeArguments(
