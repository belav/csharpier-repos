...
namespace ILLink.RoslynAnalyzer.Tests
{
    ...
    public static class TestClassGenerator
    {
        ...
        public static string GenerateClassHeader(string suiteName, bool newTestSuite)
        {
            ...
            var suiteNamespacePart =
                suiteClassName.Length < suiteName.Length
                    ? $".{suiteName.Substring(0, idx)}"
                    : string.Empty;
...
";
    ...
    public class TestCaseGenerator : IIncrementalGenerator
    {
        ...
        {
            IncrementalValuesProvider<MetadataReference> metadataReferences =
                context.MetadataReferencesProvider;

            IncrementalValueProvider<Compilation> compilation = context.CompilationProvider;

            IncrementalValueProvider<AnalyzerConfigOptionsProvider> options =
            ...
            IncrementalValuesProvider<(IAssemblySymbol, string?)> assemblyAndSuite =
                metadataReferences
                    .Combine(compilation)
                    .Combine(options)
                    .SelectMany(
                        static (referenceAndCompilationAndOptions, _) =>
                        {
                            var (referenceAndCompilation, options) =
                                referenceAndCompilationAndOptions;
                            var (reference, compilation) = referenceAndCompilation;
                            if (
                                compilation.GetAssemblyOrModuleSymbol(reference)
                                is not IAssemblySymbol asmSym
                            )
                            ...
                            var testCaseBuildOutputRoot = Path.GetFullPath(
                                testCaseBuildOutputRootValue
                            );
                            var referenceDirectory = Path.GetDirectoryName(reference.Display);
                            if (!referenceDirectory.StartsWith(testCaseBuildOutputRoot))
                                return ImmutableArray<(IAssemblySymbol, string?)>.Empty;
                            var suiteName =
                                referenceDirectory.Length < testCaseBuildOutputRoot.Length + 1
                                    ? null
                                    : Path.GetDirectoryName(
                                        referenceDirectory.Substring(
                                            testCaseBuildOutputRoot.Length + 1
                                        )
                                    );
            ...
            IncrementalValuesProvider<(AssemblyMetadata, string?)> metadataAndSuite =
                assemblyAndSuite.SelectMany(
                    static (combined, _) =>
                    {
            ...
            IncrementalValuesProvider<TestCases> testCasesPerAssembly = metadataAndSuite.Select(
                static (combined, cancellationToken) =>
                {
            ...
            IncrementalValueProvider<TestCases> testCases = testCasesPerAssembly
                .Collect()
                .Select(
                    static (testCasesPerAssembly, cancellationToken) =>
                    {
            ...
            IncrementalValueProvider<TestCases> existingTestCases = existingTestTypes
                // Find test methods
                .SelectMany(
                    static (typeSymbol, cancellationToken) =>
                        FindExistingTestCases(typeSymbol, cancellationToken)
                )
                .Collect()
                .Select(
                    static (existingTestCases, cancellationToken) =>
                    {
            ...
            context.RegisterSourceOutput(
                newTestCases,
                static (sourceProductionContext, newTestCases) =>
                {
                    ...
                    sourceBuilder.Append(
                        TestClassGenerator.GenerateClassHeader(suiteName, newTestSuite)
                    );
            ...
            static IEnumerable<(string SuiteName, string TestName)> FindExistingTestCases(
            ...
            )
            {
                ...
                string suiteName = typeFullName.Substring(
                    TestClassGenerator.TestNamespace.Length + 1
                );
            ...
            static TestCases FindTestCases(
                string? explicitSuiteName,
            ...
            )
            {
                ...
                foreach (var typeDefHandle in metadataReader.TypeDefinitions)
                {
                    ...
                    if (typeName == "Program")
                    {
                        ...
                        foreach (var caHandle in typeDef.GetCustomAttributes())
                        {
                            ...
                            if (ca.Constructor.Kind is not HandleKind.MemberReference)
                                continue;
                            var caCtor = metadataReader.GetMemberReference(
                                (MemberReferenceHandle)ca.Constructor
                            );
                            var caType = metadataReader.GetTypeReference(
                                (TypeReferenceHandle)caCtor.Parent
                            );
                            if (
                                metadataReader.GetString(caType.Name)
                    ...
                    foreach (var methodDefHandle in typeDef.GetMethods())
                    {
                        ...
                        if (
                            metadataReader.GetString(methodDef.Name)
                            == (compilerGeneratedProgramType ? "<Main>$" : "Main")
                        )
                        {
                            hasMain = true;
                            break;
                    ...
                    if (topLevelStatementsAssembly)
                        throw new NotImplementedException(
                            "Multiple test cases in an assembly with top-level statements is not supported."
                        );
                    ...
                    if (compilerGeneratedProgramType)
                    {
                        ...
                        if (explicitSuiteName == null)
                            throw new InvalidOperationException(
                                "No suite name supplied for compiler-generated Program type."
                            );
                        testName = metadataReader.GetString(
                    ...
                    else
                    {
                        if (explicitSuiteName != null)
                            throw new InvalidOperationException(
                                $"Suite name should be determined from namespace, but explicit suite '{explicitSuiteName}' was supplied."
                            );
