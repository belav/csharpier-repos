...
namespace IlasmPortablePdbTests
{
    public static class IlasmPortablePdbTesterCommon
    {
        ...
        public static MetadataReaderProvider GetMetadataReaderProvider(
            string dll,
            string pdb,
            PEReader peReader,
            bool embedded
        ...
        public static List<DocumentStub> GetExpectedDocuments(string testName, string testDir)
        {
            switch (testName)
            {
                case "TestDocuments1_win.il":
                    ...
                    {
                        new DocumentStub(
                            Path.Combine(Environment.CurrentDirectory, testDir, testName)
                        ),
        ...
        public static Dictionary<
            string,
            MethodDebugInformationStub
        > GetExpectedForTestMethodDebugInformation(string testName)
        {
            string method1;
            string method2;
            ...
            switch (testName)
            {
                case "TestMethodDebugInformation_unix.il":
                    ...
                    document2 = new DocumentStub(
                        "/tmp/TestMethodDebugInformation/SimpleMathMethods.cs"
                    );
                    break;
                case "TestMethodDebugInformation_win.il":
                    ...
                    document1 = new DocumentStub(
                        "C:\\tmp\\TestMethodDebugInformation\\SimpleMath.cs"
                    );
                    document2 = new DocumentStub(
                        "C:\\tmp\\TestMethodDebugInformation\\SimpleMathMethods.cs"
                    );
                    break;
                default:
                    Assert.Fail();
            ...
            return new Dictionary<string, MethodDebugInformationStub>()
            {
                {
                    method1,
                    new MethodDebugInformationStub(
                        method1,
                        document1,
                ...
                case "TestLocalScopes1.il":
                    return new List<LocalScopeStub>()
                    {
                        new LocalScopeStub(
                            ...
                            0x19,
                            0x19,
                            new List<VariableStub>() { new VariableStub("LOCAL_0", 0) }
                        ),
                        new LocalScopeStub(
                            "Foo",
                            0x6,
                ...
                case "TestLocalScopes4.il":
                    return new List<LocalScopeStub>()
                    {
                        ...
                        new LocalScopeStub(
                            ...
                            0x15,
                            0xa,
                            new List<VariableStub>() { new VariableStub("LOCAL_2", 2) }
                        ),
                        new LocalScopeStub(
                            "Foo",
                            0x10,
                            0x15,
                            0x5,
                            new List<VariableStub>() { new VariableStub("LOCAL_3", 3) }
                        ),
                        new LocalScopeStub(
                            "Foo",
                            0x16,
                            0x1b,
                            0x5,
                            new List<VariableStub>() { new VariableStub("LOCAL_4", 4) }
                        ),
                    };
                default:
                    Assert.Fail();
