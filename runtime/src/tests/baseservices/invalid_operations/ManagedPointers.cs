...
public unsafe class ManagedPointers
{
    ...
    public static void Validate_GeneratedILStubs_NullByRef()
    {
        ...
        {
            var fptr = (delegate* unmanaged<ref int, nint>)
                (delegate* unmanaged<void*, nint>)&PassByRef;
            Assert.Equal(0, fptr(ref Unsafe.NullRef<int>()));
        }

        {
            var fptr = (delegate* unmanaged<ref Guid, nint>)
                (delegate* unmanaged<void*, nint>)&PassByRef;
        ...
        Assert.Throws<NullReferenceException>(() =>
        {
            var fptr = (delegate* unmanaged<ref string, nint>)
                (delegate* unmanaged<void*, nint>)&PassByRef;
