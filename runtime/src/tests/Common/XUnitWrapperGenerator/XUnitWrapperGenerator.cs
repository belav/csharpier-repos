...
public sealed class XUnitWrapperGenerator : IIncrementalGenerator
{
    ...
    )
    {
        ...
        builder.AppendLine(
            "XUnitWrapperLibrary.TestFilter filter = new (args, testExclusionList);"
        );
        ...
        builder.AppendLine(
            $@"if (workitemUploadRoot != null) System.IO.File.WriteAllText(System.IO.Path.Combine(workitemUploadRoot, ""{assemblyName}.testResults.xml.txt""), testResults);"
        );
        builder.AppendLine(
        ...
        if (testInfos.Length > 0)
        {
            ...
            foreach (ITestInfo test in testInfos)
            {
                if (testsLeftInCurrentTestExecutor == 0)
                {
                    ...
                    currentTestExecutor++;
                    testExecutorBuilder.AppendLine(
                        $"static void TestExecutor{currentTestExecutor}(XUnitWrapperLibrary.TestSummary summary, XUnitWrapperLibrary.TestFilter filter, XUnitWrapperLibrary.TestOutputRecorder outputRecorder, System.Diagnostics.Stopwatch stopwatch){{"
                    );
                    builder.AppendLine(
                        $"TestExecutor{currentTestExecutor}(summary, filter, outputRecorder, stopwatch);"
                    );
