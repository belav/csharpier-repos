...
internal struct CompData
{
    internal CompData(
        ...
        IEnumerable<IMethodSymbol> possibleEntryPoints,
...
public sealed class XUnitWrapperGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterImplementationSourceOutput(
            ...
            static (context, data) =>
            {
                ...
                found = false;
                foreach (
                    SyntaxReference node in method.DeclaringSyntaxReferences.Where(n => n != null)
                )
                {
                    foreach (
                        ReturnStatementSyntax rs in node.GetSyntax(context.CancellationToken)
                            .DescendantNodes()
                    ...
                    )
                    {
                        ...
                        found = true;
                        if (
                            rs.Expression is LiteralExpressionSyntax les
                ...
                context.ReportDiagnostic(
                    Diagnostic.Create(Descriptors.XUWG1002, method.Locations[0])
                );
            }
        );
        ...
        context.RegisterImplementationSourceOutput(
            ...
            static (context, data) =>
            {
                ...
                {
                    context.ReportDiagnostic(
                        Diagnostic.Create(Descriptors.XUWG1003, method.Locations[0])
                    );
                }
                ...
                while (containingType != null)
                {
                    ...
                    {
                        context.ReportDiagnostic(
                            Diagnostic.Create(Descriptors.XUWG1004, method.Locations[0])
                        );
                    }
    ...
    private static void AddRunnerSource(
        SourceProductionContext context,
        ImmutableArray<ITestInfo> methods,
    ...
    )
    {
        ...
        else
        {
            context.AddSource(
                "SimpleRunner.g.cs",
                GenerateStandaloneSimpleTestRunner(methods, aliasMap)
            );
        }
    }
    ...
    private static void CheckNoEntryPoint(SourceProductionContext context, CompData compData)
    {
        ...
        {
            context.ReportDiagnostic(
                Diagnostic.Create(Descriptors.XUWG1001, entryPoint.Locations[0])
            );
        }
    }

    private static void AppendAliasMap(
        CodeBuilder builder,
        ImmutableDictionary<string, string> aliasMap
    )
    {
    ...
    )
    {
        ...
        {
            builder.AppendLine(
                "System.Collections.Generic.Dictionary<string, string> testExclusionTable ="
        ...
        builder.AppendLine(
            $@"using (System.IO.StreamWriter tempLogSw = System.IO.File.AppendText(""{assemblyName}.tempLog.xml""))"
        );
        builder.AppendLine(
            $@"using (System.IO.StreamWriter statsCsvSw = System.IO.File.AppendText(""{assemblyName}.testStats.csv""))"
        );
            ...
            if (testInfos.Length > 0)
            {
                ...
                foreach (ITestInfo test in testInfos)
                {
                    if (testsLeftInCurrentTestExecutor == 0)
                    {
                        ...
                        currentTestExecutor++;
                        testExecutorBuilder.AppendLine(
                            $"void TestExecutor{currentTestExecutor}("
                        ...
                        builder.AppendLine(
                            $"TestExecutor{currentTestExecutor}(tempLogSw, statsCsvSw);"
                        );
        ...
        {
            builder.AppendLine(
                $@"string testResults = summary.GetTestResultOutput(""{assemblyName}"");"
            );
            builder.AppendLine(
                $@"string workitemUploadRoot = System.Environment.GetEnvironmentVariable(""HELIX_WORKITEM_UPLOAD_ROOT"");"
            );
            builder.AppendLine($@"if (workitemUploadRoot != null)");
            using (builder.NewBracesScope())
            {
                builder.AppendLine(
                    $@"System.IO.File.WriteAllText(System.IO.Path.Combine(workitemUploadRoot, ""{assemblyName}.testResults.xml.txt""), testResults);"
                );
            }
            builder.AppendLine();

            builder.AppendLine(
                $@"System.IO.File.WriteAllText(""{assemblyName}.testResults.xml"", testResults);"
            );
        }
        ...
        builder.AppendLine(
            "public static class TestCount { public const int Count = "
                + totalTestsEmitted.ToString()
                + "; }"
        );
    ...
    )
    {
        ...
        using (builder.NewBracesScope())
        {
            ...
            builder.AppendLine(
                $@"return await XHarnessRunnerLibrary.RunnerEntryPoint.RunTests(RunTests,"
                    + $@" ""{assemblyName}"","
        ...
        builder.AppendLine(
            "XUnitWrapperLibrary.TestSummary RunTests(XUnitWrapperLibrary.TestFilter filter)"
        );
                ...
                if (testInfos.Length > 0)
                {
                    ...
                    foreach (ITestInfo test in testInfos)
                    {
                        if (testsLeftInCurrentTestExecutor == 0)
                        {
                            ...
                            builder.AppendLine(
                                $"TestExecutor{currentTestExecutor}(filter, tempLogSw, statsCsvSw);"
                            );
                            testsLeftInCurrentTestExecutor = 1; // Break test executors into groups of 1, which empirically seems to work well
                        }
                        else
    ...
    private sealed class ExternallyReferencedTestMethodsVisitor
        : SymbolVisitor<IEnumerable<IMethodSymbol>>
    {
