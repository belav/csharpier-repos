...
class Generics
{
    ...
    class TestSynchronizedMethods
    {
        ...
        static MethodInfo s_nonGenReflectedSynchronizeMethod = typeof(NonGenReflected).GetMethod(
            "Synchronize"
        );
                ...
                var mi = (MethodInfo)
                    s_genReflectedType
                        ...
                        .GetMemberWithSameMetadataDefinitionAs(
                            typeof(GenReflected<>).GetMethod("Synchronize")
                        );
...
#pragma warning restore 649
    ...
    class TestFunctionPointerLoading
    {
        ...
        unsafe class Gen<T> : IFace
        {
            ...
            public Type GrabUnmanagedStdcallSuppressGCFnptr() =>
                typeof(delegate* unmanaged[Stdcall, SuppressGCTransition]<T>);
            ...
            public Type GrabManagedFnptrOverAlsoAlsoGen() =>
                typeof(delegate* <AlsoAlsoGen<T>, MyGen<T>>);
        }
        ...
        public static void Run()
        {
...

            {
                ...
                if (
                    !t.IsFunctionPointer
                    || t.GetFunctionPointerReturnType() != typeof(Atom)
                    || t.IsUnmanagedFunctionPointer
                )
                    throw new Exception();
            }

            {
                ...
                if (
                    !t.IsFunctionPointer
                    || t.GetFunctionPointerReturnType() != typeof(Atom).MakeByRefType()
                    || t.IsUnmanagedFunctionPointer
                )
                    throw new Exception();
            }

            {
                ...
                if (
                    t != o.GrabUnmanagedStdcallFnptr()
                    || t != o.GrabUnmanagedStdcallSuppressGCFnptr()
                )
                    throw new Exception();
            }
...

            {
                ...
                if (
                    !t.IsFunctionPointer
                    || t.GetFunctionPointerReturnType().GetGenericTypeDefinition()
                        != typeof(AlsoGen<>)
