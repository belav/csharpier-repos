...
unsafe class Program
{
    static int Main()
    {
        ...
        {
            Console.WriteLine(
                $"BUG: File size is not in the expected range ({lowerBound} to {upperBound} bytes). Did a libraries change regress size of Hello World?"
            );
            return 1;
        }
...
#error Who dis?
        ...
        {
            throw new Exception(
                $"Vectors HW acceleration state unexpected - expected {ExpectedVectorsAccelerated}, got {Vector.IsHardwareAccelerated}"
            );
        }
        ...
        {
            throw new Exception(
                $"Unexpected vector length - expected {byteVectorLength}, got {Vector<byte>.Count}"
            );
        }
        ...
        Check(
            "AvxVnni.X64",
            ExpectedAvxVnni,
            &AvxVnniX64IsSupported,
            AvxVnni.X64.IsSupported,
            null
        );
        ...
        Check(
            "Avx512F.X64",
            ExpectedAvx512F,
            &Avx512FX64IsSupported,
            Avx512F.X64.IsSupported,
            null
        );
        ...
        Check(
            "Avx512BW.X64",
            ExpectedAvx512BW,
            &Avx512BWX64IsSupported,
            Avx512BW.X64.IsSupported,
            null
        );
        ...
        Check(
            "Avx512CD.X64",
            ExpectedAvx512CD,
            &Avx512CDX64IsSupported,
            Avx512CD.X64.IsSupported,
            null
        );
        ...
        Check(
            "Avx512DQ.X64",
            ExpectedAvx512DQ,
            &Avx512DQX64IsSupported,
            Avx512DQ.X64.IsSupported,
            null
        );
        ...
        Check(
            "Avx512Vbmi",
            ExpectedAvx512Vbmi,
            &Avx512VbmiIsSupported,
            Avx512Vbmi.IsSupported,
            () =>
                Avx512Vbmi
                    .PermuteVar64x8(Vector512<sbyte>.Zero, Vector512<sbyte>.Zero)
                    .Equals(Vector512<sbyte>.Zero)
        );
        Check(
            "Avx512Vbmi.VL",
            ExpectedAvx512Vbmi,
            &Avx512VbmiVLIsSupported,
        ...
        Check(
            "Avx512Vbmi.X64",
            ExpectedAvx512Vbmi,
            &Avx512VbmiX64IsSupported,
            Avx512Vbmi.X64.IsSupported,
            null
        );

        Check(
            "X86Serialize",
            ExpectedX86Serialize,
            &X86SerializeIsSupported,
        ...
        Check(
            "X86Serialize.X64",
            ExpectedX86Serialize,
            &X86SerializeX64IsSupported,
            X86Serialize.X64.IsSupported,
            null
        );
    ...
    static bool IsConstantFalse(delegate* <bool> code)
    {
        return
            ...
            || memcmp(
                (byte*)code,
                new byte[] { 0x55, 0x57, 0x50, 0x48, 0x8D, 0x6C, 0x24, 0x10, 0x33, 0xC0 }
            );
    }
