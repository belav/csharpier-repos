...
{
#if V2
    // Adding new methods to interfaces is incompatible change, but we will make sure that it works
    // anyway
...
{
    // These tests are designed to observe that the fixups we use for versioning continue to respect
    // version boundaries when the code has been copied
    // to another binary and a CHECK_ILBODY fixup is in use (and that fixup passes, but some of the
    // other dependencies may change)
    ...
    {
        // This test is designed to run a representative sample of the sorts of things that R2R can inline
        // across modules
        // In addition to the pattern of extracting the code into this generic, it uses the map file
        // generated by the
        // crossgen2 build to ensure that the expected set of methods was actually compiled into the
        // compilation target.
        // As we remove barriers to further compilation of cross module stuff, we should change this test to
        // validate
        // that the right methods got compiled.
        //
        // Unfortunately, as it is extremely difficult to reliably determine if a method's R2R code was
        // used, we cannot
