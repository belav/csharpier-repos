...
namespace R2RDumpTest
{
    class TestHelpers
    {
        ...
        public static bool XmlDiff(List<XmlNode> testXmlNodes, List<XmlNode> expectedXmlNodes)
        {
            ...
            Dictionary<string, XmlNode> diffTest = testXmlNodes
                .Except(expectedXmlNodes, new XElementEqualityComparer())
                .ToDictionary(node => XmlNodeFullName(node));
            Dictionary<string, XmlNode> diffExpected = expectedXmlNodes
                .Except(testXmlNodes, new XElementEqualityComparer())
                .ToDictionary(node => XmlNodeFullName(node));
