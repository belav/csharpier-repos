...
namespace Wasm.Build.NativeRebuild.Tests
{
    public class NoopNativeRebuildTest : NativeRebuildTestsBase
    {
        ...
        public void NoOpRebuildForNativeBuilds(
            BuildArgs buildArgs,
            bool nativeRelink,
            bool invariant,
            RunHost host,
        ...
        public void BlazorNoopRebuild(string config)
        {
            ...
            File.Move(
                Path.Combine(s_buildEnv.LogRootPath, id, $"{id}-build.binlog"),
        ...
        public void BlazorOnlyLinkRebuild(string config)
        {
            ...
            BlazorBuild(
                new BlazorBuildOptions(id, config, NativeFilesType.Relinked),
                "-p:EmccLinkOptimizationFlag=-O2"
            );
            File.Move(
                Path.Combine(s_buildEnv.LogRootPath, id, $"{id}-build.binlog"),
                Path.Combine(s_buildEnv.LogRootPath, id, $"{id}-build-first.binlog")
