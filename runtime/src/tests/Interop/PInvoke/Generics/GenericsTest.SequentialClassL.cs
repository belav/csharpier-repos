...
unsafe partial class GenericsNative
{
    ...
    [DllImport(nameof(GenericsNative))]
    public static extern SequentialClass<long> AddSequentialClassL(
        SequentialClass<long> lhs,
        SequentialClass<long> rhs
    );

    [DllImport(nameof(GenericsNative))]
    public static extern SequentialClass<long> AddSequentialClassLs(
        [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 1)] SequentialClass<long>[] pValues,
        int count
    );

    [DllImport(nameof(GenericsNative))]
    public static extern SequentialClass<long> AddSequentialClassLs(
        in SequentialClass<long> pValues,
        int count
    );
}
...
unsafe partial class GenericsTest
{
    private static void TestSequentialClassL()
    {
        ...
        Assert.Throws<MarshalDirectiveException>(
            () =>
                GenericsNative.GetSequentialClassLOut(
                    1L,
                    out GenericsNative.SequentialClass<long> value2
                )
        );
        ...
        Assert.Throws<MarshalDirectiveException>(
            () => GenericsNative.AddSequentialClassL(default, default)
        );
        ...
        Assert.Throws<MarshalDirectiveException>(
            () => GenericsNative.AddSequentialClassLs(values, values.Length)
        );

        Assert.Throws<MarshalDirectiveException>(
            () => GenericsNative.AddSequentialClassLs(in values[0], values.Length)
        );
    }
}
