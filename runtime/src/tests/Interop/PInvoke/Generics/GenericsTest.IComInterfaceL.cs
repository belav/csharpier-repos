...
unsafe partial class GenericsNative
{
    ...
    public static extern void GetIComInterfaceLOut(
        [MarshalAs(UnmanagedType.Interface)] out IComInterface<long> value
    );
    ...
    public static extern void GetIComInterfaceLs(
        [MarshalAs(
            UnmanagedType.LPArray,
            ArraySubType = UnmanagedType.Interface,
            SizeParamIndex = 1
        )]
            IComInterface<long>[] pValues,
        int count
    );

    [DllImport(nameof(GenericsNative), EntryPoint = "GetIComInterfaces")]
    public static extern void GetIComInterfaceLs(
        [MarshalAs(UnmanagedType.Interface)] ref IComInterface<long> pValues,
        int count
    );
}
...
unsafe partial class GenericsTest
{
    private static void TestIComInterfaceL()
    {
        ...
        Assert.Throws<MarshalDirectiveException>(
            () => GenericsNative.GetIComInterfaceLOut(out GenericsNative.IComInterface<long> value2)
        );
        ...
        Assert.Throws<MarshalDirectiveException>(
            () => GenericsNative.GetIComInterfaceLs(values, values.Length)
        );

        Assert.Throws<MarshalDirectiveException>(
            () => GenericsNative.GetIComInterfaceLs(ref values[0], values.Length)
        );
    }
}
