...
unsafe partial class GenericsNative
{
    ...
    public static extern void GetIComInterfaceBOut(
        [MarshalAs(UnmanagedType.Interface)] out IComInterface<bool> value
    );
    ...
    public static extern void GetIComInterfaceBs(
        [MarshalAs(
            UnmanagedType.LPArray,
            ArraySubType = UnmanagedType.Interface,
            SizeParamIndex = 1
        )]
            IComInterface<bool>[] pValues,
        int count
    );

    [DllImport(nameof(GenericsNative), EntryPoint = "GetIComInterfaces")]
    public static extern void GetIComInterfaceBs(
        [MarshalAs(UnmanagedType.Interface)] ref IComInterface<bool> pValues,
        int count
    );
}
...
unsafe partial class GenericsTest
{
    private static void TestIComInterfaceB()
    {
        ...
        Assert.Throws<MarshalDirectiveException>(() =>
            GenericsNative.GetIComInterfaceBOut(out GenericsNative.IComInterface<bool> value2)
        );
        ...
        Assert.Throws<MarshalDirectiveException>(() =>
            GenericsNative.GetIComInterfaceBs(values, values.Length)
        );

        Assert.Throws<MarshalDirectiveException>(() =>
            GenericsNative.GetIComInterfaceBs(ref values[0], values.Length)
        );
    }
}
