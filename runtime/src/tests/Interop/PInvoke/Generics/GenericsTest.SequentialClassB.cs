...
unsafe partial class GenericsNative
{
    ...
    [DllImport(nameof(GenericsNative))]
    public static extern SequentialClass<bool> AddSequentialClassB(
        SequentialClass<bool> lhs,
        SequentialClass<bool> rhs
    );

    [DllImport(nameof(GenericsNative))]
    public static extern SequentialClass<bool> AddSequentialClassBs(
        [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 1)] SequentialClass<bool>[] pValues,
        int count
    );

    [DllImport(nameof(GenericsNative))]
    public static extern SequentialClass<bool> AddSequentialClassBs(
        in SequentialClass<bool> pValues,
        int count
    );
}
...
unsafe partial class GenericsTest
{
    private static void TestSequentialClassB()
    {
        ...
        Assert.Throws<MarshalDirectiveException>(() =>
            GenericsNative.GetSequentialClassBOut(
                true,
                out GenericsNative.SequentialClass<bool> value2
            )
        );
        ...
        Assert.Throws<MarshalDirectiveException>(() =>
            GenericsNative.AddSequentialClassB(default, default)
        );
        ...
        Assert.Throws<MarshalDirectiveException>(() =>
            GenericsNative.AddSequentialClassBs(values, values.Length)
        );

        Assert.Throws<MarshalDirectiveException>(() =>
            GenericsNative.AddSequentialClassBs(in values[0], values.Length)
        );
    }
}
