...
public class CallbackTests
{
    ...
    public static void ValidateSetDllImportResolver()
    {
        ...
        Assert.Throws<ArgumentNullException>(
            ...
            "Exception expected for null assembly parameter"
        );
        Assert.Throws<ArgumentNullException>(
            () => NativeLibrary.SetDllImportResolver(assembly, null),
            "Exception expected for null resolver parameter"
        );
        ...
        Assert.Throws<InvalidOperationException>(
            () => NativeLibrary.SetDllImportResolver(assembly, resolver),
            "Should not be able to re-register resolver"
        );
        ...
        Assert.Throws<InvalidOperationException>(
            ...
            "Should not be able to register another resolver"
        );
    }
    ...
    private class Resolver
    {
        ...
        public void Validate(params string[] expectedNames)
        {
            Assert.AreEqual(
                ...
                invocations.Count,
                $"Unexpected invocation count for registered {nameof(DllImportResolver)}."
            );
            for (int i = 0; i < expectedNames.Length; i++)
                Assert.AreEqual(
                    expectedNames[i],
                    invocations[i],
                    $"Unexpected library name received by registered resolver."
                );
        }
        ...
        private IntPtr ResolveDllImport(
            ...
            if (string.Equals(libraryName, NativeLibraryToLoad.InvalidName))
            {
                Assert.AreEqual(
                    ...
                    dllImportSearchPath,
                    $"Unexpected {nameof(dllImportSearchPath)}: {dllImportSearchPath.ToString()}"
                );
