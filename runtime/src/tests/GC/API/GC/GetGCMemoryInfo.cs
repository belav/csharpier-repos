...
public class Test_GetGCMemoryInfo
{
    ...
    public static bool PrintGCMemoryInfo(GCKind kind)
    {
        ...
        Console.WriteLine(
            "last recorded {0} GC#{1}, collected gen{2}, concurrent: {3}, compact: {4}, promoted {5:N0} bytes",
            kind,
            memoryInfo.Index,
        ...
        Console.WriteLine(
            "Total committed {0:N0}, % Pause time in GC: {1}",
            memoryInfo.TotalCommittedBytes,
            memoryInfo.PauseTimePercentage
        );
        Console.WriteLine(
            "This GC observed {0:N0} pinned objects and {1:N0} objects ready for finalization",
        ...
        {
            Console.WriteLine(
                "gen#{0}, size {1:N0}->{2:N0}, frag {3:N0}->{4:N0}",
                i,
                memoryInfo.GenerationInfo[i].SizeBeforeBytes,
                memoryInfo.GenerationInfo[i].SizeAfterBytes,
                memoryInfo.GenerationInfo[i].FragmentationBeforeBytes,
                memoryInfo.GenerationInfo[i].FragmentationAfterBytes
            );
        }
        ...
        if (kind == GCKind.Ephemeral)
        {
            ...
            {
                Console.WriteLine(
                    "FAILED: GC#{0} is supposed to be an ephemeral GC but condemned max gen",
                    memoryInfo.Index
                );
                return false;
            }
        ...
        else if (kind == GCKind.FullBlocking)
        {
            ...
            {
                Console.WriteLine(
                    "FAILED: GC#{0} is supposed to be a full blocking GC but gen is {1}, concurrent {2}",
                    memoryInfo.Index,
                    memoryInfo.Generation,
                    memoryInfo.Concurrent
                );
        ...
        else if (kind == GCKind.Background)
        {
            ...
            {
                Console.WriteLine(
                    "FAILED: GC#{0} is supposed to be a BGC but gen is {1}, concurrent {2}",
                    memoryInfo.Index,
                    memoryInfo.Generation,
                    memoryInfo.Concurrent
                );
    ...
    static object MakeLongLivedSOHAllocations()
    {
        ...
        Console.WriteLine(
            "list has {0} elements, total mem {1:N0}",
            listByteArray.Count,
    ...
    public static int Main()
    {
        ...
        {
            Console.WriteLine(
                "FAILED: last GC#{0} should be NGC2 but was not",
                memoryInfoLastAnyGC.Index
            );
            isTestSucceeded = false;
        }
        ...
        {
            Console.WriteLine(
                "FAILED: We did an additional NGC2, yet last NGC2 index is {0} and current is {1}",
                lastNGC2Index,
                currentNGC2Index
            );
            isTestSucceeded = false;
        ...
        {
            Console.WriteLine(
                "FAILED: No ephemeral GCs happened so far, yet last eph index is {0} and current is {1}",
                lastEphemeralIndex,
                currentEphemeralIndex
            );
            isTestSucceeded = false;
        }

        Console.WriteLine(
            "listByteArray has {0} elements, obj is of type {1}",
            listByteArray.Count,
            obj.GetType()
        );
