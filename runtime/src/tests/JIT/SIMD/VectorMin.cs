...
public partial class VectorTest
{
    ...
    public static int TestEntryPoint()
    {
        ...
        if (VectorMinTest<ulong>.VectorMin(2ul, 3ul, 2ul) != Pass)
            returnVal = Fail;
        if (VectorMinTest<nint>.VectorMin(2, 3, 2) != Pass)
            returnVal = Fail;
        if (VectorMinTest<nuint>.VectorMin(0x80000000u, 0x40000000u, 0x40000000u) != Pass)
            returnVal = Fail;
        ...
        if (!jitLog.Check("Min", "UInt64"))
            returnVal = Fail;
        if (!jitLog.Check("Min", "IntPtr"))
            returnVal = Fail;
        if (!jitLog.Check("Min", "UIntPtr"))
            returnVal = Fail;
        jitLog.Dispose();
