...
public partial class VectorTest
{
        ...
        where T : struct, IComparable<T>, IEquatable<T>
    {
        public static int VectorGet(T value, int index)
        {
            ...
            {
                if (!CheckValue(A[i], value))
                    returnVal = Fail;
            }

            if (!CheckValue(A[index], value))
                returnVal = Fail;

            // Test constant index.
            if (!CheckValue(A[0], value))
                returnVal = Fail;
            if (Vector<T>.Count >= 2)
            {
            ...
            {
                if (!CheckValue(A[8], value))
                    returnVal = Fail;
                if (!CheckValue(A[9], value))
                    returnVal = Fail;
                if (!CheckValue(A[10], value))
                    returnVal = Fail;
                if (!CheckValue(A[11], value))
                    returnVal = Fail;
                if (!CheckValue(A[12], value))
                    returnVal = Fail;
                if (!CheckValue(A[13], value))
                    returnVal = Fail;
                if (!CheckValue(A[14], value))
                    returnVal = Fail;
                if (!CheckValue(A[15], value))
                    returnVal = Fail;
            }
            if (Vector<T>.Count >= 32)
            {
                if (!CheckValue(A[16], value))
                    returnVal = Fail;
                if (!CheckValue(A[17], value))
                    returnVal = Fail;
                if (!CheckValue(A[18], value))
                    returnVal = Fail;
                if (!CheckValue(A[19], value))
                    returnVal = Fail;
                if (!CheckValue(A[20], value))
                    returnVal = Fail;
                if (!CheckValue(A[21], value))
                    returnVal = Fail;
                if (!CheckValue(A[22], value))
                    returnVal = Fail;
                if (!CheckValue(A[23], value))
                    returnVal = Fail;
                if (!CheckValue(A[24], value))
                    returnVal = Fail;
                if (!CheckValue(A[25], value))
                    returnVal = Fail;
                if (!CheckValue(A[26], value))
                    returnVal = Fail;
                if (!CheckValue(A[27], value))
                    returnVal = Fail;
                if (!CheckValue(A[28], value))
                    returnVal = Fail;
                if (!CheckValue(A[29], value))
                    returnVal = Fail;
                if (!CheckValue(A[30], value))
                    returnVal = Fail;
                if (!CheckValue(A[31], value))
                    returnVal = Fail;
            }
        ...
        public static int VectorGetIndexerOutOfRange(T value, int index)
        {
            ...
            {
                Console.WriteLine(
                    "Failed to throw ArgumentOutOfRangeException for index == Count of "
                        + Vector<T>.Count
                );
                returnVal = Fail;
            }
    ...
    public static int TestEntryPoint()
    {
        ...
        if (VectorGetTest<ulong>.VectorGetIndexerOutOfRange(100, 1) == Fail)
            returnVal = Fail;
        if (VectorGetTest<nint>.VectorGet(101, 1) == Fail)
            returnVal = Fail;
        if (VectorGetTest<nint>.VectorGet(100, 1) == Fail)
            returnVal = Fail;
        if (VectorGetTest<nint>.VectorGetIndexerOutOfRange(100, 1) == Fail)
            returnVal = Fail;
        if (VectorGetTest<nuint>.VectorGet(101, 1) == Fail)
            returnVal = Fail;
        if (VectorGetTest<nuint>.VectorGet(100, 1) == Fail)
            returnVal = Fail;
        if (VectorGetTest<nuint>.VectorGetIndexerOutOfRange(100, 1) == Fail)
            returnVal = Fail;
        ...
        if (Sse41.IsSupported || AdvSimd.IsSupported)
        {
            ...
            if (!jitLog.Check("get_Count", "UInt64"))
                returnVal = Fail;
            if (!jitLog.Check("get_Item", "IntPtr"))
                returnVal = Fail;
            if (!jitLog.Check("get_Count", "IntPtr"))
                returnVal = Fail;
            if (!jitLog.Check("get_Item", "UIntPtr"))
                returnVal = Fail;
            if (!jitLog.Check("get_Count", "UIntPtr"))
                returnVal = Fail;
            jitLog.Dispose();
        }
        return returnVal;
