...
public partial class VectorTest
{
    ...
    private class Vector4Test
    {
        public static int VectorAdd()
        {
            ...
            Vector4 C = A + B;
            if (!(CheckValue<float>(C.X, 3)))
                return Fail;
            if (!(CheckValue<float>(C.Y, 3)))
                return Fail;
            if (!(CheckValue<float>(C.Z, 3)))
                return Fail;
            if (!(CheckValue<float>(C.W, 3)))
                return Fail;
            return Pass;
        }
    }
    ...
    public static int TestEntryPoint()
    {
        ...
        if (VectorAddTest<sbyte>.VectorAdd(-1, -2, (sbyte)(-1 - 2)) != Pass)
            returnVal = Fail;
        if (
            VectorAddTest<uint>.VectorAdd(0x41000000u, 0x42000000u, 0x41000000u + 0x42000000u)
            != Pass
        )
            returnVal = Fail;
        if (
        ...
        if (VectorAddTest<nint>.VectorAdd(1, 2, (nint)(1 + 2)) != Pass)
            returnVal = Fail;
        if (
            VectorAddTest<nuint>.VectorAdd(0x41000000u, 0x42000000u, 0x41000000u + 0x42000000u)
            != Pass
        )
            returnVal = Fail;
