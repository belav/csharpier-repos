    ...
    public sealed unsafe class SimpleBinaryOpTest__CompareLessThanOrEqual_Vector128_Single
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(
                SimpleBinaryOpTest__CompareLessThanOrEqual_Vector128_Single testClass
            )
            {
        ...
        )
        {
            ...
            for (var i = 0; i < RetElementCount; i++)
            {
                if (
                    BitConverter.SingleToInt32Bits(
                        Helpers.CompareLessThanOrEqual(left[i], right[i])
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(AdvSimd)}.{nameof(AdvSimd.CompareLessThanOrEqual)}<Single>(Vector128<Single>, Vector128<Single>): {method} failed:"
                    );
