...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    {
        private static void AbsoluteDifference_Vector128_Single()
        {
            var test = new SimpleBinaryOpTest__AbsoluteDifference_Vector128_Single();
    ...
    public sealed unsafe class SimpleBinaryOpTest__AbsoluteDifference_Vector128_Single
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(
                SimpleBinaryOpTest__AbsoluteDifference_Vector128_Single testClass
            )
            {
        ...
        static SimpleBinaryOpTest__AbsoluteDifference_Vector128_Single()
        {
            for (var i = 0; i < Op1ElementCount; i++)
            {
                _data1[i] = TestLibrary.Generator.GetSingle();
        ...
        public SimpleBinaryOpTest__AbsoluteDifference_Vector128_Single()
        {
            Succeeded = true;
        ...
        )
        {
            ...
            {
                if (
                    BitConverter.SingleToInt32Bits(Helpers.AbsoluteDifference(left[i], right[i]))
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(AdvSimd)}.{nameof(AdvSimd.AbsoluteDifference)}<Single>(Vector128<Single>, Vector128<Single>): {method} failed:"
                    );
