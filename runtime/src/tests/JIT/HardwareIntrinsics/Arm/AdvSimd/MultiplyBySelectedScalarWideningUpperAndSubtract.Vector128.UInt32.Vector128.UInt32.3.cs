        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(AdvSimd)}.{nameof(AdvSimd.MultiplyBySelectedScalarWideningUpperAndSubtract)}<UInt64>(Vector128<UInt64>, Vector128<UInt32>, Vector128<UInt32>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
