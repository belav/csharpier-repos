    ...
    public sealed unsafe class LoadPairVector64NonTemporal_SByte
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(LoadPairVector64NonTemporal_SByte testClass)
            {
                _fld = AdvSimd
                    .Arm64
                    .LoadPairVector64NonTemporal((SByte*)(testClass._dataTable.inArrayPtr));
        ...
        private static readonly int RetElementCount =
            Unsafe.SizeOf<(Vector64<SByte>, Vector64<SByte>)>() / sizeof(SByte);
        ...
        )
        {
            ...
            Unsafe.CopyBlockUnaligned(
                ...
                (uint)(Unsafe.SizeOf<SByte>() * Op1ElementCount)
            );
            Unsafe.CopyBlockUnaligned(
                ref Unsafe.As<SByte, byte>(ref outArray[0]),
                ref Unsafe.AsRef<byte>(result),
                (uint)(Unsafe.SizeOf<SByte>() * RetElementCount)
            );
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(AdvSimd.Arm64)}.{nameof(AdvSimd.Arm64.LoadPairVector64NonTemporal)}<SByte>(Vector64<SByte>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
