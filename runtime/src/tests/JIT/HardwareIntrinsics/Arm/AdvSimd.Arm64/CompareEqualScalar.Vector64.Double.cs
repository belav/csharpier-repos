...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__CompareEqualScalar_Vector64_Double
    {
        ...
        )
        {
            ...
            if (
                BitConverter.DoubleToInt64Bits(Helpers.CompareEqual(left[0], right[0]))
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(AdvSimd.Arm64)}.{nameof(AdvSimd.Arm64.CompareEqualScalar)}<Double>(Vector64<Double>, Vector64<Double>): {method} failed:"
                    );
