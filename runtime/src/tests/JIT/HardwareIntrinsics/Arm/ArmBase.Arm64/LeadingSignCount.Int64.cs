    ...
    public sealed unsafe class ScalarUnaryOpTest__LeadingSignCount_Int64
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(ArmBase.Arm64)
                .GetMethod(nameof(ArmBase.Arm64.LeadingSignCount), new Type[] { typeof(Int64) })
                .Invoke(
                    null,
                    new object[]
                    {
        ...
        private void ValidateResult(Int64 data, Int32 result, [CallerMemberName] string method = "")
        {
            ...
            for (int index = 62; (((ulong)data >> index) & 1) == (((ulong)data >> 63) & 1); index--)
            {
                expectedResult++;
            }
            ...
            if (isUnexpectedResult)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(ArmBase.Arm64)}.{nameof(ArmBase.Arm64.LeadingSignCount)}<Int32>(Int64): LeadingSignCount failed:"
                    );
