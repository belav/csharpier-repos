    ...
    public sealed unsafe class VectorTernaryOpTest__ConditionalSelectByte
    {
        ...
        )
        {
            ...
            else
            {
                ...
                {
                    if (
                        result[i] != (byte)((secondOp[i] & firstOp[i]) | (thirdOp[i] & ~firstOp[i]))
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector128)}.{nameof(Vector128.ConditionalSelect)}<Byte>(Vector128<Byte>, Vector128<Byte>, Vector128<Byte>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
