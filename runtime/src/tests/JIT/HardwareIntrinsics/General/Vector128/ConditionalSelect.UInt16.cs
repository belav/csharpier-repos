    ...
    public sealed unsafe class VectorTernaryOpTest__ConditionalSelectUInt16
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var method = typeof(Vector128).GetMethod(
                nameof(Vector128.ConditionalSelect),
                new Type[]
                {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Vector128.ConditionalSelect(_clsVar1, _clsVar2, _clsVar3);
        ...
        )
        {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        result[i]
                        != (ushort)((secondOp[i] & firstOp[i]) | (thirdOp[i] & ~firstOp[i]))
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector128)}.{nameof(Vector128.ConditionalSelect)}<UInt16>(Vector128<UInt16>, Vector128<UInt16>, Vector128<UInt16>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
