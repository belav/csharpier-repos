...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorBinaryOpTest__GreaterThanSingle
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Vector128.GreaterThan(_clsVar1, _clsVar2);
        ...
        )
        {
            ...
            else
            {
                ...
                {
                    if (
                        BitConverter.SingleToInt32Bits(result[i]) != ((left[i] > right[i]) ? -1 : 0)
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector128)}.{nameof(Vector128.GreaterThan)}<Single>(Vector128<Single>, Vector128<Single>): {method} failed:"
                    );
