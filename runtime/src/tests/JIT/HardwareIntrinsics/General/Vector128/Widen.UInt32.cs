...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorWidenTest__WidenUInt32
    {
        ...
        public void RunBasicScenario_UnsafeRead()
        {
            ...
            var result = Vector128.Widen(Unsafe.Read<Vector128<UInt16>>(_dataTable.inArray1Ptr));
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            Unsafe.Write(
                _dataTable.outLowerArrayPtr,
                (((Vector128<UInt32> Lower, Vector128<UInt32> Upper))(result)).Lower
            );
            Unsafe.Write(
                _dataTable.outUpperArrayPtr,
                (((Vector128<UInt32> Lower, Vector128<UInt32> Upper))(result)).Upper
            );
            ValidateResult(
                _dataTable.inArray1Ptr,
                _dataTable.outLowerArrayPtr,
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector128)}.{nameof(Vector128.Widen)}<UInt32>(Vector128<UInt16>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
