...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorWidenTest__WidenUInt16
    {
        private struct DataTable
        {
            ...
            public DataTable(
                Byte[] inArray1,
                UInt16[] outLowerArray,
                UInt16[] outUpperArray,
                int alignment
            )
            {
        ...
        public void RunBasicScenario_UnsafeRead()
        {
            ...
            var result = Vector128.Widen(Unsafe.Read<Vector128<Byte>>(_dataTable.inArray1Ptr));
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            Unsafe.Write(
                _dataTable.outLowerArrayPtr,
                (((Vector128<UInt16> Lower, Vector128<UInt16> Upper))(result)).Lower
            );
            Unsafe.Write(
                _dataTable.outUpperArrayPtr,
                (((Vector128<UInt16> Lower, Vector128<UInt16> Upper))(result)).Upper
            );
            ValidateResult(
                _dataTable.inArray1Ptr,
                _dataTable.outLowerArrayPtr,
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Vector128.Widen(_clsVar1);
        ...
        )
        {
            ...
            Unsafe.CopyBlockUnaligned(
                ref Unsafe.As<UInt16, byte>(ref outLowerArray[0]),
                ref Unsafe.AsRef<byte>(lowerResult),
                (uint)Unsafe.SizeOf<Vector128<UInt16>>()
            );
            Unsafe.CopyBlockUnaligned(
                ref Unsafe.As<UInt16, byte>(ref outUpperArray[0]),
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector128)}.{nameof(Vector128.Widen)}<UInt16>(Vector128<Byte>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
