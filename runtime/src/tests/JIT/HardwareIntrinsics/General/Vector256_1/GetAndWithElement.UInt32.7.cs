...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithElement__GetAndWithElementUInt327
    {
        ...
        public void RunBasicScenario(int imm = 7, bool expectedOutOfRangeException = false)
        {
            ...
            Vector256<UInt32> value = Vector256.Create(
                values[0],
                values[1],
                values[2],
                values[3],
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"Vector256<UInt32.GetElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException."
                    );
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"Vector256<UInt32.WithElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException."
                    );
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        public void RunReflectionScenario(int imm = 7, bool expectedOutOfRangeException = false)
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"Vector256<UInt32.GetElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException."
                    );
                TestLibrary.TestFramework.LogInformation(string.Empty);
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"Vector256<UInt32.WithElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException."
                    );
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        )
        {
            if (result != values[7])
            {
                ...
                TestLibrary
                    .TestFramework
                    .LogInformation($"Vector256<UInt32.GetElement(7): {method} failed:");
                TestLibrary
                    .TestFramework
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation($"Vector256<UInt32.WithElement(7): {method} failed:");
                TestLibrary
                    .TestFramework
                    .LogInformation($"   value: ({string.Join(", ", values)})");
