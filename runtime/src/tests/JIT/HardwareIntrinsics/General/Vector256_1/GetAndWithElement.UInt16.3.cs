...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithElement__GetAndWithElementUInt163
    {
        ...
        public void RunBasicScenario(int imm = 3, bool expectedOutOfRangeException = false)
        {
            ...
            Vector256<UInt16> value = Vector256.Create(
                values[0],
                values[1],
                values[2],
                values[3],
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"Vector256<UInt16.GetElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException."
                    );
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"Vector256<UInt16.WithElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException."
                    );
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        public void RunReflectionScenario(int imm = 3, bool expectedOutOfRangeException = false)
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"Vector256<UInt16.GetElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException."
                    );
                TestLibrary.TestFramework.LogInformation(string.Empty);
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"Vector256<UInt16.WithElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException."
                    );
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        )
        {
            if (result != values[3])
            {
                ...
                TestLibrary
                    .TestFramework
                    .LogInformation($"Vector256<UInt16.GetElement(3): {method} failed:");
                TestLibrary
                    .TestFramework
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation($"Vector256<UInt16.WithElement(3): {method} failed:");
                TestLibrary
                    .TestFramework
                    .LogInformation($"   value: ({string.Join(", ", values)})");
