...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorBinaryOpTest__GreaterThanDouble
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Vector64.GreaterThan(_clsVar1, _clsVar2);
        ...
        )
        {
            ...
            else
            {
                ...
                {
                    if (
                        BitConverter.DoubleToInt64Bits(result[i]) != ((left[i] > right[i]) ? -1 : 0)
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector64)}.{nameof(Vector64.GreaterThan)}<Double>(Vector64<Double>, Vector64<Double>): {method} failed:"
                    );
