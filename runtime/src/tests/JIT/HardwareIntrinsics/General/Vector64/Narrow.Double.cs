...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorBinaryOpTest__NarrowDouble
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Vector64.Narrow(_clsVar1, _clsVar2);
        ...
        )
        {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        result[i]
                        != (float)((i < Op1ElementCount) ? left[i] : right[i - Op1ElementCount])
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector64)}.{nameof(Vector64.Narrow)}<Single>(Vector64<Double>, Vector64<Double>): {method} failed:"
                    );
