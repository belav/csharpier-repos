        ...
        )
        {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        result[i]
                        != (sbyte)((secondOp[i] & firstOp[i]) | (thirdOp[i] & ~firstOp[i]))
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector64)}.{nameof(Vector64.ConditionalSelect)}<SByte>(Vector64<SByte>, Vector64<SByte>, Vector64<SByte>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
