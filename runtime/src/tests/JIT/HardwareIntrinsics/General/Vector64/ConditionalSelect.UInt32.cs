...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorTernaryOpTest__ConditionalSelectUInt32
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var method = typeof(Vector64).GetMethod(
                nameof(Vector64.ConditionalSelect),
                new Type[]
                {
            ...
            if (method is null)
            {
                method = typeof(Vector64).GetMethod(
                    nameof(Vector64.ConditionalSelect),
                    1,
                    new Type[]
                    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Vector64.ConditionalSelect(_clsVar1, _clsVar2, _clsVar3);
        ...
        )
        {
            ...
            else
            {
                ...
                {
                    if (
                        result[i] != (uint)((secondOp[i] & firstOp[i]) | (thirdOp[i] & ~firstOp[i]))
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector64)}.{nameof(Vector64.ConditionalSelect)}<UInt32>(Vector64<UInt32>, Vector64<UInt32>, Vector64<UInt32>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
