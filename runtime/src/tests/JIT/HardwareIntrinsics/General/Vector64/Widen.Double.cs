...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorWidenTest__WidenDouble
    {
        private struct DataTable
        {
            ...
            public DataTable(
                Single[] inArray1,
                Double[] outLowerArray,
                Double[] outUpperArray,
                int alignment
            )
            {
        ...
        public void RunBasicScenario_UnsafeRead()
        {
            ...
            var result = Vector64.Widen(Unsafe.Read<Vector64<Single>>(_dataTable.inArray1Ptr));
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            Unsafe.Write(
                _dataTable.outLowerArrayPtr,
                (((Vector64<Double> Lower, Vector64<Double> Upper))(result)).Lower
            );
            Unsafe.Write(
                _dataTable.outUpperArrayPtr,
                (((Vector64<Double> Lower, Vector64<Double> Upper))(result)).Upper
            );
            ValidateResult(
                _dataTable.inArray1Ptr,
                _dataTable.outLowerArrayPtr,
        ...
        )
        {
            ...
            Unsafe.CopyBlockUnaligned(
                ref Unsafe.As<Double, byte>(ref outLowerArray[0]),
                ref Unsafe.AsRef<byte>(lowerResult),
                (uint)Unsafe.SizeOf<Vector64<Double>>()
            );
            Unsafe.CopyBlockUnaligned(
                ref Unsafe.As<Double, byte>(ref outUpperArray[0]),
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector64)}.{nameof(Vector64.Widen)}<Double>(Vector64<Single>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
