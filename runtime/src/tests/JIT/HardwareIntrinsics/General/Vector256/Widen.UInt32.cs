...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorWidenTest__WidenUInt32
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            Unsafe.Write(_dataTable.outLowerArrayPtr, (((Vector256<UInt32> Lower, Vector256<UInt32> Upper))(result)).Lower);
            Unsafe.Write(_dataTable.outUpperArrayPtr, (((Vector256<UInt32> Lower, Vector256<UInt32> Upper))(result)).Upper);
            ValidateResult(_dataTable.inArray1Ptr, _dataTable.outLowerArrayPtr, _dataTable.outUpperArrayPtr);
        }
        ...
        private void ValidateResult(Vector256<UInt16> op1, void* lowerResult, void* upperResult, [CallerMemberName] string method = "")
        {
            ...
            Unsafe.CopyBlockUnaligned(ref Unsafe.As<UInt32, byte>(ref outLowerArray[0]), ref Unsafe.AsRef<byte>(lowerResult), (uint)Unsafe.SizeOf<Vector256<UInt32>>());
            Unsafe.CopyBlockUnaligned(ref Unsafe.As<UInt32, byte>(ref outUpperArray[0]), ref Unsafe.AsRef<byte>(upperResult), (uint)Unsafe.SizeOf<Vector256<UInt32>>());
        ...
        private void ValidateResult(UInt16[] firstOp, UInt32[] lowerResult, UInt32[] upperResult, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.Widen)}<UInt32>(Vector256<UInt16>): {method} failed:");
