...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorWidenTest__WidenUInt32
    {
        private struct DataTable
        {
            ...
            public DataTable(
                UInt16[] inArray1,
                UInt32[] outLowerArray,
                UInt32[] outUpperArray,
                int alignment
            )
            {
        ...
        public void RunBasicScenario_UnsafeRead()
        {
            ...
            var result = Vector256.Widen(Unsafe.Read<Vector256<UInt16>>(_dataTable.inArray1Ptr));
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            Unsafe.Write(
                _dataTable.outLowerArrayPtr,
                (((Vector256<UInt32> Lower, Vector256<UInt32> Upper))(result)).Lower
            );
            Unsafe.Write(
                _dataTable.outUpperArrayPtr,
                (((Vector256<UInt32> Lower, Vector256<UInt32> Upper))(result)).Upper
            );
            ValidateResult(
                _dataTable.inArray1Ptr,
                _dataTable.outLowerArrayPtr,
        ...
        )
        {
            ...
            Unsafe.CopyBlockUnaligned(
                ref Unsafe.As<UInt32, byte>(ref outLowerArray[0]),
                ref Unsafe.AsRef<byte>(lowerResult),
                (uint)Unsafe.SizeOf<Vector256<UInt32>>()
            );
            Unsafe.CopyBlockUnaligned(
                ref Unsafe.As<UInt32, byte>(ref outUpperArray[0]),
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector256)}.{nameof(Vector256.Widen)}<UInt32>(Vector256<UInt16>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
