...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorWidenTest__WidenDouble
    {
        ...
        public void RunBasicScenario_UnsafeRead()
        {
            ...
            var result = Vector256.Widen(Unsafe.Read<Vector256<Single>>(_dataTable.inArray1Ptr));
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            Unsafe.Write(
                _dataTable.outLowerArrayPtr,
                (((Vector256<Double> Lower, Vector256<Double> Upper))(result)).Lower
            );
            Unsafe.Write(
                _dataTable.outUpperArrayPtr,
                (((Vector256<Double> Lower, Vector256<Double> Upper))(result)).Upper
            );
            ValidateResult(
                _dataTable.inArray1Ptr,
                _dataTable.outLowerArrayPtr,
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector256)}.{nameof(Vector256.Widen)}<Double>(Vector256<Single>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
