...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorUnaryOpTest__FloorDouble
    {
        ...
        public void RunBasicScenario_UnsafeRead()
        {
            ...
            var result = Vector256.Floor(Unsafe.Read<Vector256<Double>>(_dataTable.inArray1Ptr));
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector256)}.{nameof(Vector256.Floor)}<Double>(Vector256<Double>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
