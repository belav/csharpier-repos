...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorUnaryOpTest__NegateSingle
    {
        ...
        public void RunBasicScenario_UnsafeRead()
        {
            ...
            var result = Vector256.Negate(Unsafe.Read<Vector256<Single>>(_dataTable.inArray1Ptr));
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector256)}.{nameof(Vector256.Negate)}<Single>(Vector256<Single>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
