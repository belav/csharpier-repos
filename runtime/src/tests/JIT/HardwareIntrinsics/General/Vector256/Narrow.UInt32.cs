    ...
    public sealed unsafe class VectorBinaryOpTest__NarrowUInt32
    {
        ...
        private struct TestStruct
        {
            ...
            public static TestStruct Create()
            {
                ...
                Unsafe.CopyBlockUnaligned(
                    ref Unsafe.As<Vector256<UInt32>, byte>(ref testStruct._fld2),
        ...
        static VectorBinaryOpTest__NarrowUInt32()
        {
            ...
            Unsafe.CopyBlockUnaligned(
                ref Unsafe.As<Vector256<UInt32>, byte>(ref _clsVar2),
                ref Unsafe.As<UInt32, byte>(ref _data2[0]),
                (uint)Unsafe.SizeOf<Vector256<UInt32>>()
            );
        }
        ...
        public VectorBinaryOpTest__NarrowUInt32()
        {
            ...
            Unsafe.CopyBlockUnaligned(
                ref Unsafe.As<Vector256<UInt32>, byte>(ref _fld2),
                ref Unsafe.As<UInt32, byte>(ref _data2[0]),
                (uint)Unsafe.SizeOf<Vector256<UInt32>>()
            );
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var method = typeof(Vector256).GetMethod(
                ...
                new Type[] { typeof(Vector256<UInt32>), typeof(Vector256<UInt32>) }
            );
        ...
        )
        {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        result[i]
                        != (ushort)((i < Op1ElementCount) ? left[i] : right[i - Op1ElementCount])
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector256)}.{nameof(Vector256.Narrow)}<UInt16>(Vector256<UInt32>, Vector256<UInt32>): {method} failed:"
                    );
