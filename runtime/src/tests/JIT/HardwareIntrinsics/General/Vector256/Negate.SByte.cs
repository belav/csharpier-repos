...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorUnaryOpTest__NegateSByte
    {
        ...
        public void RunBasicScenario_UnsafeRead()
        {
            ...
            var result = Vector256.Negate(Unsafe.Read<Vector256<SByte>>(_dataTable.inArray1Ptr));
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector256)}.{nameof(Vector256.Negate)}<SByte>(Vector256<SByte>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
