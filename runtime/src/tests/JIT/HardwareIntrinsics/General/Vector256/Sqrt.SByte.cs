...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorUnaryOpTest__SqrtSByte
    {
        ...
        public void RunBasicScenario_UnsafeRead()
        {
            ...
            var result = Vector256.Sqrt(Unsafe.Read<Vector256<SByte>>(_dataTable.inArray1Ptr));
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Vector256)}.{nameof(Vector256.Sqrt)}<SByte>(Vector256<SByte>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
