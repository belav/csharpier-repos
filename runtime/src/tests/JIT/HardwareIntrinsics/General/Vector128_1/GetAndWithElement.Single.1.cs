    ...
    public sealed unsafe class VectorGetAndWithElement__GetAndWithElementSingle1
    {
        ...
        public void RunBasicScenario(int imm = 1, bool expectedOutOfRangeException = false)
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"Vector128<Single.GetElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException."
                    );
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"Vector128<Single.WithElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException."
                    );
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        public void RunReflectionScenario(int imm = 1, bool expectedOutOfRangeException = false)
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"Vector128<Single.GetElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException."
                    );
                TestLibrary.TestFramework.LogInformation(string.Empty);
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"Vector128<Single.WithElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException."
                    );
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        )
        {
            if (result != values[1])
            {
                ...
                TestLibrary
                    .TestFramework
                    .LogInformation($"Vector128<Single.GetElement(1): {method} failed:");
                TestLibrary
                    .TestFramework
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation($"Vector128<Single.WithElement(1): {method} failed:");
                TestLibrary
                    .TestFramework
                    .LogInformation($"   value: ({string.Join(", ", values)})");
