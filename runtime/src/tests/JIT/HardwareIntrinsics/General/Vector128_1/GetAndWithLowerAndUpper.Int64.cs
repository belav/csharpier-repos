...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithLowerAndUpper__GetAndWithLowerAndUpperInt64
    {
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation(
                    $"Vector128<Int64>.GetLower(): {method} failed:"
                );
                TestLibrary.TestFramework.LogInformation(
            ...
            {
                TestLibrary.TestFramework.LogInformation(
                    $"Vector128<Int64>.GetUpper(): {method} failed:"
                );
                TestLibrary.TestFramework.LogInformation(
                    $"   value: ({string.Join(", ", values)})"
                );
        ...
        )
        {
            ...
            {
                TestLibrary.TestFramework.LogInformation(
                    $"Vector128<Int64.WithLower(): {method} failed:"
                );
                TestLibrary.TestFramework.LogInformation(
                    $"   value: ({string.Join(", ", values)})"
                );
            ...
            {
                TestLibrary.TestFramework.LogInformation(
                    $"Vector128<Int64.WithUpper(): {method} failed:"
                );
                TestLibrary.TestFramework.LogInformation(
                    $"   value: ({string.Join(", ", values)})"
                );
