...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorGetAndWithElement__GetAndWithElementDouble1
    {
        ...
        public void RunBasicScenario(int imm = 1, bool expectedOutOfRangeException = false)
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation(
                    $"Vector128<Double.GetElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException."
                );
            ...
            {
                TestLibrary.TestFramework.LogInformation(
                    $"Vector128<Double.WithElement({imm}): {nameof(RunBasicScenario)} failed to throw ArgumentOutOfRangeException."
                );
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        public void RunReflectionScenario(int imm = 1, bool expectedOutOfRangeException = false)
        {
            ...
            {
                TestLibrary.TestFramework.LogInformation(
                    $"Vector128<Double.GetElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException."
                );
                TestLibrary.TestFramework.LogInformation(string.Empty);
            ...
            {
                TestLibrary.TestFramework.LogInformation(
                    $"Vector128<Double.WithElement({imm}): {nameof(RunReflectionScenario)} failed to throw ArgumentOutOfRangeException."
                );
                TestLibrary.TestFramework.LogInformation(string.Empty);
        ...
        )
        {
            if (result != values[1])
            {
                ...
                TestLibrary.TestFramework.LogInformation(
                    $"Vector128<Double.GetElement(1): {method} failed:"
                );
                TestLibrary.TestFramework.LogInformation(
        ...
        )
        {
            ...
            {
                TestLibrary.TestFramework.LogInformation(
                    $"Vector128<Double.WithElement(1): {method} failed:"
                );
                TestLibrary.TestFramework.LogInformation(
                    $"   value: ({string.Join(", ", values)})"
                );
