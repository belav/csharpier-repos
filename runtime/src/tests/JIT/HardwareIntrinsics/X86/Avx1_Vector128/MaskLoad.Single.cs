...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__MaskLoadSingle
    {
        ...
        public SimpleBinaryOpTest__MaskLoadSingle()
        {
            ...
            _dataTable = new SimpleBinaryOpTest__DataTable<Single, Single, Single>(
                _data1,
                _data2,
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(Avx)
                .GetMethod(
                    nameof(Avx.MaskLoad),
                    new Type[] { typeof(Single*), typeof(Vector128<Single>) }
                )
                .Invoke(
                    null,
                    new object[]
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Avx.MaskLoad((Single*)_dataTable.inArray1Ptr, _clsVar2);
        ...
        )
        {
            ...
            if (
                ...
                != BitConverter.SingleToInt32Bits(
                    (BitConverter.SingleToInt32Bits(right[0]) < 0) ? left[0] : 0
                )
            )
            {
                succeeded = false;
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        ...
                        != BitConverter.SingleToInt32Bits(
                            (BitConverter.SingleToInt32Bits(right[i]) < 0) ? left[i] : 0
                        )
                    )
                    {
                        succeeded = false;
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation(
                    $"{nameof(Avx)}.{nameof(Avx.MaskLoad)}<Single>(Single*, Vector128<Single>): {method} failed:"
                );
