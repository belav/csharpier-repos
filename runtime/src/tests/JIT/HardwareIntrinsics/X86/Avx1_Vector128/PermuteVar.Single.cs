...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__PermuteVarSingle
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(Avx)
                .GetMethod(
                    ...
                    new Type[] { typeof(Vector128<Single>), typeof(Vector128<Int32>) }
                )
                .Invoke(
                    null,
                    new object[]
        ...
        public void RunStructLclFldScenario_Load()
        {
            ...
            var test = TestStruct.Create();
            var result = Avx.PermuteVar(
                Avx.LoadVector128((Single*)(&test._fld1)),
                Avx.LoadVector128((Int32*)(&test._fld2))
            );
        ...
        )
        {
            ...
            if (
                BitConverter.SingleToInt32Bits(left[1]) != BitConverter.SingleToInt32Bits(result[0])
            )
            {
                succeeded = false;
            }
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation(
                    $"{nameof(Avx)}.{nameof(Avx.PermuteVar)}<Single>(Vector128<Single>, Vector128<Int32>): {method} failed:"
                );
