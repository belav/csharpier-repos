    ...
    public sealed unsafe class HorizontalBinaryOpTest__HorizontalSubtractSingle
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(
                HorizontalBinaryOpTest__HorizontalSubtractSingle testClass
            )
            {
        ...
        )
        {
            ...
            for (var outer = 0; outer < (LargestVectorSize / 16); outer++)
            {
                for (var inner = 0; inner < (8 / sizeof(Single)); inner++)
                {
                    ...
                    if (
                        BitConverter.SingleToInt32Bits(result[i1])
                        != BitConverter.SingleToInt32Bits(left[i3] - left[i3 + 1])
                    )
                    {
                        succeeded = false;
                        break;
                    ...
                    if (
                        BitConverter.SingleToInt32Bits(result[i2])
                        != BitConverter.SingleToInt32Bits(right[i3] - right[i3 + 1])
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Sse3)}.{nameof(Sse3.HorizontalSubtract)}<Single>(Vector128<Single>, Vector128<Single>): {method} failed:"
                    );
