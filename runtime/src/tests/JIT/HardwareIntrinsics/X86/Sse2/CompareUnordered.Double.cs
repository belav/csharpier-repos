    ...
    public sealed unsafe class SimpleBinaryOpTest__CompareUnorderedDouble
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario_Load(
                SimpleBinaryOpTest__CompareUnorderedDouble testClass
            )
            {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Sse2.CompareUnordered(_clsVar1, _clsVar2);
        ...
        )
        {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        ...
                        != ((double.IsNaN(left[i]) || double.IsNaN(right[i])) ? -1 : 0)
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Sse2)}.{nameof(Sse2.CompareUnordered)}<Double>(Vector128<Double>, Vector128<Double>): {method} failed:"
                    );
