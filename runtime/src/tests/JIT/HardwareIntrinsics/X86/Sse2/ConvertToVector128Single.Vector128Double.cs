    ...
    public sealed unsafe class SimpleUnaryOpConvTest__ConvertToVector128SingleVector128Double
    {
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(
                SimpleUnaryOpConvTest__ConvertToVector128SingleVector128Double testClass
            )
            {
        ...
        )
        {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        ...
                        != ((i < 2) ? BitConverter.SingleToInt32Bits((float)firstOp[i]) : 0)
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Sse2)}.{nameof(Sse2.ConvertToVector128Single)}<Single>(Vector128<Double>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
