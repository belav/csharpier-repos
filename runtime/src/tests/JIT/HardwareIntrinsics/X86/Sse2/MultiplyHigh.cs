        ...
        public static unsafe void MultiplyHigh()
        {
            ...
            if (Sse2.IsSupported)
            {
                ...
                using (var ushortTable = TestTableSse2<ushort>.Create(testsCount))
                {
                    ...
                    {
                        (Vector128<short>, Vector128<short>, Vector128<short>) value = shortTable[
                            i
                        ];
                    ...
                    for (int i = 0; i < testsCount; i++)
                    {
                        (Vector128<ushort>, Vector128<ushort>, Vector128<ushort>) value =
                            ushortTable[i];
                    ...
                    if (!shortTable.CheckResult(checkInt16))
                    {
                        PrintError(
                            shortTable,
                            methodUnderTestName,
                            "(short x, short y, short z, ref short a) => (a = (short)((x * y) >> 16)) == z",
                            checkInt16
                        );
                        testResult = Fail;
                    }

                    CheckMethod<ushort> checkUInt16 = (
                        ushort x,
                        ushort y,
                        ushort z,
                        ref ushort a
                    ...
                    if (!ushortTable.CheckResult(checkUInt16))
                    {
                        PrintError(
                            ushortTable,
                            methodUnderTestName,
                            "(ushort x, ushort y, ushort z, ref ushort a) => (a = (ushort)((x * y) >> 16)) == z",
                            checkUInt16
                        );
                        testResult = Fail;
                    }
