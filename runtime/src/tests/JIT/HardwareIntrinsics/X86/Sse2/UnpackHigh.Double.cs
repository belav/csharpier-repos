...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__UnpackHighDouble
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario_Load(SimpleBinaryOpTest__UnpackHighDouble testClass)
            {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Sse2.UnpackHigh(_clsVar1, _clsVar2);
        ...
        )
        {
            ...
            if (
                BitConverter.DoubleToInt64Bits(result[0]) != BitConverter.DoubleToInt64Bits(left[1])
            )
            {
                succeeded = false;
            }
                    ...
                    if (
                        (i % 2 == 0)
                            ? BitConverter.DoubleToInt64Bits(result[i])
                              != BitConverter.DoubleToInt64Bits(left[i / 2 + 1])
                            : BitConverter.DoubleToInt64Bits(result[i])
                              != BitConverter.DoubleToInt64Bits(right[(i - 1) / 2 + 1])
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Sse2)}.{nameof(Sse2.UnpackHigh)}<Double>(Vector128<Double>, Vector128<Double>): {method} failed:"
                    );
