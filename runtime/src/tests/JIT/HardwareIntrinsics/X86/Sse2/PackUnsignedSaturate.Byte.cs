    ...
    public sealed unsafe class SimpleBinaryOpTest__PackUnsignedSaturateByte
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(Sse2)
                .GetMethod(
                    nameof(Sse2.PackUnsignedSaturate),
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Sse2.PackUnsignedSaturate(_clsVar1, _clsVar2);
        ...
        )
        {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        ...
                        != (
                            (i < 8)
                                ? (byte)Math.Min(Math.Max(left[i], byte.MinValue), byte.MaxValue)
                                : (byte)
                                    Math.Min(Math.Max(right[i % 8], byte.MinValue), byte.MaxValue)
                        )
                    )
                    {
                        succeeded = false;
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation(
                    $"{nameof(Sse2)}.{nameof(Sse2.PackUnsignedSaturate)}<Byte>(Vector128<Int16>, Vector128<Int16>): {method} failed:"
                );
