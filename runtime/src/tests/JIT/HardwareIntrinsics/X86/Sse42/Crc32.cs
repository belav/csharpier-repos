        ...
        public static void Test()
        {
            ...
            if (Sse42.IsSupported)
            {
                ...
                for (int i = 0; i < intCrcTable.Length; i++)
                {
                    ...
                    {
                        Console.WriteLine(
                            "{0}: Inputs: 0x{1,8:x}, 0x{2,8:x} Expected: 0x{3,8:x} actual: 0x{4,8:x}",
                            i,
                            s1i,
                            s2i,
                            intCrcTable[i].res,
                    ...
                    resi = Convert.ToUInt32(
                        typeof(Sse42)
                            ...
                            .Invoke(null, new object[] { s1i, s2i })
                    );
                    if (resi != intCrcTable[i].res)
                    {
                        Console.WriteLine(
                            "{0}: Inputs: 0x{1,8:x}, 0x{2,8:x} Expected: 0x{3,8:x} actual: 0x{4,8:x} - Reflection",
                            i,
                            s1i,
                            s2i,
                            intCrcTable[i].res,
                ...
                for (int i = 0; i < shortCrcTable.Length; i++)
                {
                    ...
                    resi = Convert.ToUInt32(
                        typeof(Sse42)
                            ...
                            .Invoke(null, new object[] { s1i, s2s })
                    );
                    if (resi != shortCrcTable[i].res)
                    {
                        Console.WriteLine(
                ...
                for (int i = 0; i < byteCrcTable.Length; i++)
                {
                    ...
                    resi = Convert.ToUInt32(
                        typeof(Sse42)
                            ...
                            .Invoke(null, new object[] { s1i, s2b })
                    );
                    if (resi != byteCrcTable[i].res)
                    {
                        Console.WriteLine(
