    ...
    public sealed unsafe class SimpleTernaryOpTest__MultiplyWideningAndAddSaturateByte
    {
        ...
        private struct TestStruct
        {
            ...
            public static TestStruct Create()
            {
                ...
                {
                    _data0[i] = TestLibrary.Generator.GetByte();
                }
                Unsafe.CopyBlockUnaligned(
                    ref Unsafe.As<Vector128<Int32>, byte>(ref testStruct._fld0),
                ...
                {
                    _data2[i] = (sbyte)TestLibrary.Generator.GetSByte();
                }
                Unsafe.CopyBlockUnaligned(
            ...
            public void RunStructFldScenario(
                SimpleTernaryOpTest__MultiplyWideningAndAddSaturateByte testClass
            )
            {
        ...
        static SimpleTernaryOpTest__MultiplyWideningAndAddSaturateByte()
        {
            ...
            Unsafe.CopyBlockUnaligned(
                ref Unsafe.As<Vector128<Int32>, byte>(ref _clsVar0),
                ref Unsafe.As<Int32, byte>(ref _data0[0]),
                (uint)Unsafe.SizeOf<Vector128<Int32>>()
            );
            for (var i = 0; i < Op1ElementCount; i++)
        ...
        public SimpleTernaryOpTest__MultiplyWideningAndAddSaturateByte()
        {
            ...
            Unsafe.CopyBlockUnaligned(
                ref Unsafe.As<Vector128<Int32>, byte>(ref _fld0),
                ref Unsafe.As<Int32, byte>(ref _data0[0]),
                (uint)Unsafe.SizeOf<Vector128<Int32>>()
            );
            for (var i = 0; i < Op1ElementCount; i++)
        ...
        )
        {
            ...
            {
                int addend2 =
                    right[i * 4 + 3] * left[i * 4 + 3]
                    + right[i * 4 + 2] * left[i * 4 + 2]
                    + right[i * 4 + 1] * left[i * 4 + 1]
                    + right[i * 4] * left[i * 4];
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(AvxVnni)}.{nameof(AvxVnni.MultiplyWideningAndAddSaturate)}<Int32>(Vector128<Int32>, Vector128<Int32>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
