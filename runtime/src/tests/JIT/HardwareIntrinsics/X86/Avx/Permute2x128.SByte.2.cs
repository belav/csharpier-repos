...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    {
        private static void Permute2x128SByte2()
        {
    ...
    public sealed unsafe class ImmBinaryOpTest__Permute2x128SByte2
    {
        private struct TestStruct
        {
            ...
            public static TestStruct Create()
            {
                ...
                Unsafe.CopyBlockUnaligned(
                    ref Unsafe.As<Vector256<SByte>, byte>(ref testStruct._fld2),
            ...
            public void RunStructFldScenario(ImmBinaryOpTest__Permute2x128SByte2 testClass)
            {
        ...
        static ImmBinaryOpTest__Permute2x128SByte2()
        {
            for (var i = 0; i < Op1ElementCount; i++)
            {
                _data1[i] = TestLibrary.Generator.GetSByte();
            ...
            Unsafe.CopyBlockUnaligned(
                ref Unsafe.As<Vector256<SByte>, byte>(ref _clsVar2),
                ref Unsafe.As<SByte, byte>(ref _data2[0]),
                (uint)Unsafe.SizeOf<Vector256<SByte>>()
            );
        }

        public ImmBinaryOpTest__Permute2x128SByte2()
        {
            Succeeded = true;
            ...
            Unsafe.CopyBlockUnaligned(
                ref Unsafe.As<Vector256<SByte>, byte>(ref _fld2),
                ref Unsafe.As<SByte, byte>(ref _data2[0]),
                (uint)Unsafe.SizeOf<Vector256<SByte>>()
            );
            ...
            _dataTable = new SimpleBinaryOpTest__DataTable<SByte, SByte, SByte>(
                _data1,
                _data2,
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(Avx)
                .GetMethod(
                    ...
                    new Type[] { typeof(Vector256<SByte>), typeof(Vector256<SByte>), typeof(byte) }
                )
                .Invoke(
                    null,
                    new object[]
        ...
        )
        {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Avx)}.{nameof(Avx.Permute2x128)}<SByte>(Vector256<SByte>.2, Vector256<SByte>): {method} failed:"
                    );
