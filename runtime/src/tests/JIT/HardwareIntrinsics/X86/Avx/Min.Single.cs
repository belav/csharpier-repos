...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__MinSingle
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Avx.Min(_clsVar1, _clsVar2);
        ...
        private void ValidateResult(
            ...
            if (
                BitConverter.SingleToInt32Bits(MathF.Min(left[0], right[0]))
            ...
            else
            {
                ...
                {
                    if (
                        BitConverter.SingleToInt32Bits(MathF.Min(left[i], right[i]))
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation(
                    $"{nameof(Avx)}.{nameof(Avx.Min)}<Single>(Vector256<Single>, Vector256<Single>): {method} failed:"
                );
