...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__MaxSingle
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Avx.Max(_clsVar1, _clsVar2);
        ...
        private void ValidateResult(
            ...
            if (
                BitConverter.SingleToInt32Bits(MathF.Max(left[0], right[0]))
            ...
            else
            {
                ...
                {
                    if (
                        BitConverter.SingleToInt32Bits(MathF.Max(left[i], right[i]))
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation(
                    $"{nameof(Avx)}.{nameof(Avx.Max)}<Single>(Vector256<Single>, Vector256<Single>): {method} failed:"
                );
