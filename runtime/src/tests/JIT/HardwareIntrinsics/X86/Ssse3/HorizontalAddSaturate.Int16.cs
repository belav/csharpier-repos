    ...
    public sealed unsafe class HorizontalBinaryOpTest__HorizontalAddSaturateInt16
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(
                HorizontalBinaryOpTest__HorizontalAddSaturateInt16 testClass
            )
            {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(Ssse3)
                .GetMethod(
                    nameof(Ssse3.HorizontalAddSaturate),
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Ssse3.HorizontalAddSaturate(_clsVar1, _clsVar2);
        ...
        )
        {
            ...
            for (var outer = 0; outer < (LargestVectorSize / 16); outer++)
            {
                for (var inner = 0; inner < (8 / sizeof(Int16)); inner++)
                {
                    ...
                    if (
                        result[i1]
                        != Math.Clamp((left[i3] + left[i3 + 1]), short.MinValue, short.MaxValue)
                    )
                    {
                        succeeded = false;
                        break;
                    ...
                    if (
                        result[i2]
                        != Math.Clamp((right[i3] + right[i3 + 1]), short.MinValue, short.MaxValue)
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Ssse3)}.{nameof(Ssse3.HorizontalAddSaturate)}<Int16>(Vector128<Int16>, Vector128<Int16>): {method} failed:"
                    );
