...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__SignInt16
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Ssse3.Sign(_clsVar1, _clsVar2);
        ...
        )
        {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        result[i]
                        != (right[i] < 0 ? (short)(-left[i]) : (right[i] > 0 ? left[i] : 0))
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Ssse3)}.{nameof(Ssse3.Sign)}<Int16>(Vector128<Int16>, Vector128<Int16>): {method} failed:"
                    );
