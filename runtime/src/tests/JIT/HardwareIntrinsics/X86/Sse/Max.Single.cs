...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__MaxSingle
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Sse.Max(_clsVar1, _clsVar2);
        ...
        )
        {
            ...
            if (
                BitConverter.SingleToInt32Bits(Math.Max(left[0], right[0]))
            ...
            else
            {
                ...
                {
                    if (
                        BitConverter.SingleToInt32Bits(Math.Max(left[i], right[i]))
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Sse)}.{nameof(Sse.Max)}<Single>(Vector128<Single>, Vector128<Single>): {method} failed:"
                    );
