...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__MinSingle
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Sse.Min(_clsVar1, _clsVar2);
        ...
        )
        {
            ...
            if (
                BitConverter.SingleToInt32Bits(Math.Min(left[0], right[0]))
            ...
            else
            {
                ...
                {
                    if (
                        BitConverter.SingleToInt32Bits(Math.Min(left[i], right[i]))
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Sse)}.{nameof(Sse.Min)}<Single>(Vector128<Single>, Vector128<Single>): {method} failed:"
                    );
