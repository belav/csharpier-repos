...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__OrSingle
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Sse.Or(_clsVar1, _clsVar2);
        ...
        )
        {
            ...
            if (
                (BitConverter.SingleToInt32Bits(left[0]) | BitConverter.SingleToInt32Bits(right[0]))
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        (
                            BitConverter.SingleToInt32Bits(left[0])
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Sse)}.{nameof(Sse.Or)}<Single>(Vector128<Single>, Vector128<Single>): {method} failed:"
                    );
