    ...
    public sealed unsafe class AlternatingTernaryOpTest__MultiplyAddSubtractDouble
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(
                AlternatingTernaryOpTest__MultiplyAddSubtractDouble testClass
            )
            {
        ...
        )
        {
            ...
            for (var i = 0; i < RetElementCount; i += 2)
            {
                ...
                if (
                    BitConverter.DoubleToInt64Bits(
                        Math.Round((firstOp[i + 1] * secondOp[i + 1]) + thirdOp[i + 1], 9)
                    ) != BitConverter.DoubleToInt64Bits(Math.Round(result[i + 1], 9))
                )
                {
                    succeeded = false;
                    break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Fma)}.{nameof(Fma.MultiplyAddSubtract)}<Double>(Vector256<Double>, Vector256<Double>, Vector256<Double>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
