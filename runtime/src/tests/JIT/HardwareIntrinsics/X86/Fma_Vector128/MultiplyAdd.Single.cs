...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleTernaryOpTest__MultiplyAddSingle
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Fma.MultiplyAdd(_clsVar1, _clsVar2, _clsVar3);
        ...
        )
        {
            ...
            if (
                BitConverter.SingleToInt32Bits(
                    MathF.Round((firstOp[0] * secondOp[0]) + thirdOp[0], 3)
                ) != BitConverter.SingleToInt32Bits(MathF.Round(result[0], 3))
            )
            {
                succeeded = false;
            }
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Fma)}.{nameof(Fma.MultiplyAdd)}<Single>(Vector128<Single>, Vector128<Single>, Vector128<Single>): {method} failed:"
                    );
                TestLibrary
                    .TestFramework
