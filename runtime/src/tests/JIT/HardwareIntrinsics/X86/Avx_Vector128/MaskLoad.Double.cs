    ...
    public sealed unsafe class SimpleBinaryOpTest__MaskLoadDouble
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(Avx)
                .GetMethod(
                    nameof(Avx.MaskLoad),
                    new Type[] { typeof(Double*), typeof(Vector128<Double>) }
                )
                .Invoke(
                    null,
                    new object[]
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Avx.MaskLoad((Double*)_dataTable.inArray1Ptr, _clsVar2);
        ...
        )
        {
            ...
            if (
                ...
                != BitConverter.DoubleToInt64Bits(
                    (BitConverter.DoubleToInt64Bits(right[0]) < 0) ? left[0] : 0
                )
            )
            {
                succeeded = false;
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        ...
                        != BitConverter.DoubleToInt64Bits(
                            (BitConverter.DoubleToInt64Bits(right[i]) < 0) ? left[i] : 0
                        )
                    )
                    {
                        succeeded = false;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Avx)}.{nameof(Avx.MaskLoad)}<Double>(Double*, Vector128<Double>): {method} failed:"
                    );
