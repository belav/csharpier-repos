...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__MultiplyLowUInt32
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Sse41.MultiplyLow(_clsVar1, _clsVar2);
        ...
        private void ValidateResult(
            ...
            if (
                result[0]
                != BitConverter.ToUInt32(BitConverter.GetBytes(((ulong)(left[0])) * right[0]), 0)
            ) {
                succeeded = false;
            }
            else
                ...
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        ...
                        != BitConverter.ToUInt32(
                            BitConverter.GetBytes(((ulong)(left[i])) * right[i]),
                            0
                        )
                    ) {
                        succeeded = false;
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation(
                    $"{nameof(Sse41)}.{nameof(Sse41.MultiplyLow)}<UInt32>(Vector128<UInt32>, Vector128<UInt32>): {method} failed:"
                );
