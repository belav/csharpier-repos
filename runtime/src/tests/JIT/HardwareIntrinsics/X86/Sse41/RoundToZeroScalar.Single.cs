    ...
    public sealed unsafe class SimpleBinaryOpTest__RoundToZeroScalarSingle
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario_Load(
                SimpleBinaryOpTest__RoundToZeroScalarSingle testClass
            )
            {
        ...
        )
        {
            ...
            if (
                ...
                != BitConverter.SingleToInt32Bits(
                    (right[0] > 0) ? MathF.Floor(right[0]) : MathF.Ceiling(right[0])
                )
            )
            {
                succeeded = false;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Sse41)}.{nameof(Sse41.RoundToZeroScalar)}<Single>(Vector128<Single>, Vector128<Single>): {method} failed:"
                    );
