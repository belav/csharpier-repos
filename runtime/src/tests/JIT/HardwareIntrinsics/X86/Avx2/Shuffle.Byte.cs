...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__ShuffleByte
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Avx2.Shuffle(_clsVar1, _clsVar2);
        ...
        public void RunClassLclFldScenario_Load()
        {
            ...
            fixed (Vector256<Byte>* pFld1 = &test._fld1)fixed (Vector256<Byte>* pFld2 = &test._fld2)
            {
                var result = Avx2.Shuffle(
                    Avx.LoadVector256((Byte*)(pFld1)),
                    Avx.LoadVector256((Byte*)(pFld2))
        ...
        private void ValidateResult(
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        ...
                        != (
                            i < 16
                                ? (right[i] > 127 ? 0 : left[right[i] & 0x0F])
                                : (right[i] > 127 ? 0 : left[(right[i] & 0x0F) + 16])
                        )
                    ) {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation(
                    $"{nameof(Avx2)}.{nameof(Avx2.Shuffle)}<Byte>(Vector256<Byte>, Vector256<Byte>): {method} failed:"
                );
