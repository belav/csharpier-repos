...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class ImmBinaryOpTest__AlignRightInt324
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Avx2.AlignRight(_clsVar1, _clsVar2, 4);
        ...
        )
        {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        ...
                        != (
                            i < 4
                                ? (i == 3 ? left[0] : right[i + 1])
                                : (i == 7 ? left[4] : right[i + 1])
                        )
                    )
                    {
                        succeeded = false;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Avx2)}.{nameof(Avx2.AlignRight)}<Int32>(Vector256<Int32>.4, Vector256<Int32>): {method} failed:"
                    );
