...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__MultiplyLowUInt16
    {
        ...
        )
        {
            ...
            if (
                result[0]
                != BitConverter.ToUInt16(BitConverter.GetBytes(((uint)(left[0])) * right[0]), 0)
            )
            {
                succeeded = false;
            }
                ...
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        ...
                        != BitConverter.ToUInt16(
                            BitConverter.GetBytes(((uint)(left[i])) * right[i]),
                            0
                        )
                    )
                    {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Avx2)}.{nameof(Avx2.MultiplyLow)}<UInt16>(Vector256<UInt16>, Vector256<UInt16>): {method} failed:"
                    );
