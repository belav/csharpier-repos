...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__MultiplyHighInt16
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Avx2.MultiplyHigh(_clsVar1, _clsVar2);
        ...
        )
        {
            ...
            if (
                ...
                != BitConverter.ToInt16(
                    BitConverter.GetBytes((((int)(left[0])) * right[0]) >> 16),
                    0
                )
            )
            {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        result[i]
                        != BitConverter.ToInt16(
                            BitConverter.GetBytes((((int)(left[i])) * right[i]) >> 16),
                            0
                        )
                    )
                    {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Avx2)}.{nameof(Avx2.MultiplyHigh)}<Int16>(Vector256<Int16>, Vector256<Int16>): {method} failed:"
                    );
