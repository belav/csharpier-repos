...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    {
        private static void BlendUInt164()
        {
            var test = new ImmBinaryOpTest__BlendUInt164();
    ...
    public sealed unsafe class ImmBinaryOpTest__BlendUInt164
    {
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(ImmBinaryOpTest__BlendUInt164 testClass)
            {
        ...
        static ImmBinaryOpTest__BlendUInt164()
        {
            for (var i = 0; i < Op1ElementCount; i++)
            {
                _data1[i] = TestLibrary.Generator.GetUInt16();
        ...
        public ImmBinaryOpTest__BlendUInt164()
        {
            Succeeded = true;
        ...
        )
        {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        ...
                        != (
                            (i < 8)
                                ? (((4 & (1 << i)) == 0) ? left[i] : right[i])
                                : (((4 & (1 << (i - 8))) == 0) ? left[i] : right[i])
                        )
                    )
                    {
                        succeeded = false;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Avx2)}.{nameof(Avx2.Blend)}<UInt16>(Vector256<UInt16>.4, Vector256<UInt16>): {method} failed:"
                    );
