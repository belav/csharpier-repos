...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__MultiplyLowInt32
    {
        ...
        )
        {
            ...
            if (
                result[0]
                != BitConverter.ToInt32(BitConverter.GetBytes(((long)(left[0])) * right[0]), 0)
            )
            {
                succeeded = false;
            }
                ...
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        ...
                        != BitConverter.ToInt32(
                            BitConverter.GetBytes(((long)(left[i])) * right[i]),
                            0
                        )
                    )
                    {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Avx2)}.{nameof(Avx2.MultiplyLow)}<Int32>(Vector256<Int32>, Vector256<Int32>): {method} failed:"
                    );
