        ...
        )
        {
            ...
            if (
                ...
                != BitConverter.ToUInt16(
                    BitConverter.GetBytes((((uint)(left[0])) * right[0]) >> 16),
                    0
                )
            )
            {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        result[i]
                        != BitConverter.ToUInt16(
                            BitConverter.GetBytes((((uint)(left[i])) * right[i]) >> 16),
                            0
                        )
                    )
                    {
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Avx2)}.{nameof(Avx2.MultiplyHigh)}<UInt16>(Vector256<UInt16>, Vector256<UInt16>): {method} failed:"
                    );
