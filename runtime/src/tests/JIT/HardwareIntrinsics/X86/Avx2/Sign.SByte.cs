...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__SignSByte
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario_Load(SimpleBinaryOpTest__SignSByte testClass)
            {
        ...
        )
        {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        result[i]
                        != (right[i] < 0 ? (sbyte)(-left[i]) : (right[i] > 0 ? left[i] : 0))
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Avx2)}.{nameof(Avx2.Sign)}<SByte>(Vector256<SByte>, Vector256<SByte>): {method} failed:"
                    );
