...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class ImmBinaryOpTest__AlignRightUInt162
    {
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Avx2.AlignRight(_clsVar1, _clsVar2, 2);
        ...
        )
        {
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        ...
                        != (
                            i < 8
                                ? (i == 7 ? left[0] : right[i + 1])
                                : (i == 15 ? left[8] : right[i + 1])
                        )
                    )
                    {
                        succeeded = false;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Avx2)}.{nameof(Avx2.AlignRight)}<UInt16>(Vector256<UInt16>.2, Vector256<UInt16>): {method} failed:"
                    );
