    ...
    public sealed unsafe class SimpleUnaryOpTest__RoundToZeroDouble
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario_Load(SimpleUnaryOpTest__RoundToZeroDouble testClass)
            {
                ...
                {
                    var result = Avx.RoundToZero(Avx.LoadVector256((Double*)(pFld1)));
        ...
        public void RunBasicScenario_UnsafeRead()
        {
            ...
            var result = Avx.RoundToZero(Unsafe.Read<Vector256<Double>>(_dataTable.inArray1Ptr));
        ...
        public void RunBasicScenario_Load()
        {
            ...
            var result = Avx.RoundToZero(Avx.LoadVector256((Double*)(_dataTable.inArray1Ptr)));
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(Avx)
                .GetMethod(nameof(Avx.RoundToZero), new Type[] { typeof(Vector256<Double>) })
                .Invoke(
                    null,
        ...
        public void RunClsVarScenario_Load()
        {
            ...
            {
                var result = Avx.RoundToZero(Avx.LoadVector256((Double*)(pClsVar1)));
        ...
        public void RunStructLclFldScenario_Load()
        {
            ...
            var test = TestStruct.Create();
            var result = Avx.RoundToZero(Avx.LoadVector256((Double*)(&test._fld1)));
        ...
        )
        {
            ...
            if (
                ...
                != BitConverter.DoubleToInt64Bits(
                    (firstOp[0] > 0) ? Math.Floor(firstOp[0]) : Math.Ceiling(firstOp[0])
                )
            )
            {
                succeeded = false;
            ...
            else
            {
                for (var i = 1; i < RetElementCount; i++)
                {
                    if (
                        ...
                        != BitConverter.DoubleToInt64Bits(
                            (firstOp[i] > 0) ? Math.Floor(firstOp[i]) : Math.Ceiling(firstOp[i])
                        )
                    )
                    {
                        succeeded = false;
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation(
                    $"{nameof(Avx)}.{nameof(Avx.RoundToZero)}<Double>(Vector256<Double>): {method} failed:"
                );
                TestLibrary.TestFramework.LogInformation(
