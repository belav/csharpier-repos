...
namespace JIT.HardwareIntrinsics.X86
{
    partial public static class Program
    {
        ...
        {
            var test = new ImmBinaryOpTest__InsertVector128Double1();
    ...
    public sealed unsafe class ImmBinaryOpTest__InsertVector128Double1
    {
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(ImmBinaryOpTest__InsertVector128Double1 testClass)
            {
        ...
        static ImmBinaryOpTest__InsertVector128Double1()
        {
            for (var i = 0; i < Op1ElementCount; i++)
            {
                _data1[i] = TestLibrary.Generator.GetDouble();
        ...
        public ImmBinaryOpTest__InsertVector128Double1()
        {
            Succeeded = true;
            ...
            _dataTable = new SimpleBinaryOpTest__DataTable<Double, Double, Double>(
                _data1,
                _data2,
        ...
        )
        {
            ...
            if (
                BitConverter.DoubleToInt64Bits(result[0]) != BitConverter.DoubleToInt64Bits(left[0])
            )
            {
                succeeded = false;
            }
                    ...
                    if (
                        ...
                        != (
                            i < 2
                                ...
                                : BitConverter.DoubleToInt64Bits(right[i - 2])
                        )
                    )
                    {
                        succeeded = false;
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation(
                    $"{nameof(Avx)}.{nameof(Avx.InsertVector128)}<Double>(Vector256<Double>.1, Vector128<Double>): {method} failed:"
                );
