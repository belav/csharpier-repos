...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class ImmBinaryOpTest__ShuffleSingle1
    {
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(ImmBinaryOpTest__ShuffleSingle1 testClass)
            {
        ...
        static ImmBinaryOpTest__ShuffleSingle1()
        {
            for (var i = 0; i < Op1ElementCount; i++)
            {
                _data1[i] = TestLibrary.Generator.GetSingle();
        ...
        public ImmBinaryOpTest__ShuffleSingle1()
        {
            Succeeded = true;
        ...
        public void RunClsVarScenario()
        {
            ...
            var result = Avx.Shuffle(_clsVar1, _clsVar2, 1);
        ...
        )
        {
            ...
            if (
                BitConverter.SingleToInt32Bits(result[0]) != BitConverter.SingleToInt32Bits(left[1])
            )
            {
                succeeded = false;
            }
            else
            {
                ...
                {
                    if (
                        BitConverter.SingleToInt32Bits(result[7])
                        != BitConverter.SingleToInt32Bits(right[4])
                    )
                    {
                        succeeded = false;
                        break;
            ...
            if (!succeeded)
            {
                TestLibrary
                    .TestFramework
                    .LogInformation(
                        $"{nameof(Avx)}.{nameof(Avx.Shuffle)}<Single>(Vector256<Single>.1, Vector256<Single>): {method} failed:"
                    );
