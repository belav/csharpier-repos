...
public class Runtime_90323
{
    ...
    [ConditionalFact(
        ...
        nameof(TestLibrary.PlatformDetection.Is64BitProcess)
    )]
    public static int TestEntryPoint()
    {
        ...
        {
            Console.WriteLine(
                $"Mismatch between codegen and constant folding: {ConvertToSingle(value)} != {(float)(value)}"
            );
            passed = false;
        }
        ...
        {
            Console.WriteLine(
                $"Mismatch between constant folding and expected value: {(float)(value)} != 4.6116866E+18f; 0x{BitConverter.SingleToUInt32Bits((float)(value)):X8} != 0x5E800001"
            );
            passed = false;
        }
