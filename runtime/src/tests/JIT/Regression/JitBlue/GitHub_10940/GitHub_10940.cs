...
// This test checks whether or not the JIT properly spills side effects in the importer when dumping
// multi-reg values
// to temps. If the JIT does not do so correctly, the calls to GetString() and GetDecimal() will be
// reordered and the
// test will fail with exit code 0; if it does do so correctly, the calls will not be reordered and
// the test will
// pass.
