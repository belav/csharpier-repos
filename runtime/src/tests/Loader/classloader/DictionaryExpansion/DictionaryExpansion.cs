...
public class GenClass<T> : GenBase
{
    ...
    public Type FuncOnGenClass(int level)
    {
        switch (level)
        {
            case 0:
                return typeof(T);
            case 1:
                return typeof(TestType1<T>);
            case 2:
                return typeof(TestType2<T>);
            case 3:
                return typeof(TestType3<T>);
            case 4:
                return typeof(TestType4<T>);
            case 5:
                return typeof(TestType5<T>);
            case 6:
                return typeof(TestType6<T>);
            case 7:
                return typeof(TestType7<T>);
            case 8:
                return typeof(TestType8<T>);
            case 9:
                return typeof(TestType9<T>);
            case 10:
                return typeof(TestType10<T>);
            case 11:
                return typeof(TestType11<T>);
            case 12:
                return typeof(TestType12<T>);
            case 13:
                return typeof(TestType13<T>);
            case 14:
                return typeof(TestType14<T>);
            case 15:
                return typeof(TestType15<T>);
            case 16:
                return typeof(TestType16<T>);
            case 17:
                return typeof(TestType17<T>);
            case 18:
                return typeof(TestType18<T>);
            case 19:
                return typeof(TestType19<T>);
            case 20:
                return typeof(TestType20<T>);
            case 21:
                return typeof(TestType21<T>);
            case 22:
                return typeof(TestType22<T>);
            case 23:
                return typeof(TestType23<T>);
            case 24:
                return typeof(TestType24<T>);
            case 25:
            default:
                return typeof(TestType25<T>);
        }
    }
    ...
    static void DoTest_Inner<T1, T2, T3>(int max, GenClass<T1> o1, GenClass<T2> o2, GenClass<T3> o3)
    {
        ...
        for (int i = 0; i < max; i++)
            Assert.Equal(
                o1.FuncOnGenClass(i).ToString(),
                i == 0 ? $"{typeof(T1)}" : $"TestType{i}`1[{typeof(T1)}]"
            );

        Console.WriteLine("TEST: FuncOnGenClass<{0}>", typeof(T2).Name);
        for (int i = 0; i < max; i++)
            Assert.Equal(
                o2.FuncOnGenClass(i).ToString(),
                i == 0 ? $"{typeof(T2)}" : $"TestType{i}`1[{typeof(T2)}]"
            );

        Console.WriteLine("TEST: FuncOnGenClass2<{0}>", typeof(T2).Name);
        for (int i = 0; i < max; i++)
            Assert.Equal(
                o2.FuncOnGenClass2(i).ToString(),
                i == 0 ? $"{typeof(T2)}" : $"TestType{i}`1[{typeof(T2)}]"
            );
        ...
        for (int i = 0; i < max; i++)
            Assert.Equal(
                o3.FuncOnGenClass(i).ToString(),
                i == 0 ? $"{typeof(T3)}" : $"TestType{i}`1[{typeof(T3)}]"
            );
    }

    public static void DoTest_GenClass(int max)
    {
        DoTest_Inner<string, object, Test_DictionaryExpansion>(
            max,
            new GenClass<string>(),
            new GenClass<object>(),
    ...
    {
        DoTest_Inner<object, object, object>(
            max,
            new GenDerived2(),
            new GenDerived2(),
            new GenDerived2()
    ...
    {
        Assert.Equal(
            typeof(KeyValuePair<T, string>).ToString(),
...
public class Test_DictionaryExpansion
{
    ...
    public static void DoTest(int max)
    {
        ...
        for (int i = 0; i < max; i++)
            Assert.Equal(
                GFunc<string>(i).ToString(),
                i == 0 ? "System.String" : $"TestType{i}`1[System.String]"
            );

        Console.WriteLine("TEST: GFunc<object>(i)");
        for (int i = 0; i < max; i++)
            Assert.Equal(
                GFunc<object>(i).ToString(),
                i == 0 ? "System.Object" : $"TestType{i}`1[System.Object]"
            );

        Console.WriteLine("TEST: GFunc2<object>(i)");
        for (int i = 0; i < max; i++)
            Assert.Equal(
                GFunc2<object>(i).ToString(),
                i == 0 ? "System.Object" : $"TestType{i}`1[System.Object]"
            );
        ...
        for (int i = 0; i < max; i++)
            Assert.Equal(
                GFunc<Test_DictionaryExpansion>(i).ToString(),
                i == 0 ? "Test_DictionaryExpansion" : $"TestType{i}`1[Test_DictionaryExpansion]"
            );
    }
        ...
        for (int i = 5; i <= 25; i += 5)
        {
...

            {
                AssemblyBuilder ab = AssemblyBuilder.DefineDynamicAssembly(
                    new AssemblyName("CollectibleAsm" + i),
                    AssemblyBuilderAccess.RunAndCollect
                );
                var tb = ab.DefineDynamicModule("CollectibleMod" + i)
                    .DefineType(
                        "CollectibleGenDerived" + i,
