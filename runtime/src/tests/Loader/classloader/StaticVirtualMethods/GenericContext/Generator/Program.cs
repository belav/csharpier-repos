...
namespace VirtualStaticInterfaceMethodTestGen
{
    class Program
    {
        ...
        static void GenerateImplementations(
            TextWriter tw,
            bool testDefaultImplementations,
        ...
        )
        {
            ...
            )
            {
                ...
                if (constrainedTypeDefinition.ToString().StartsWith("Generic"))
                {
                    GenerateImpl(
                        implClass.Name + "<!0>",
                        $"IFaceNonGeneric{defaultImpSuffix}",
                        "",
                        false
                    );
                    GenerateImpl(
                        implClass.Name + "<!0>",
                        $"IFaceGeneric{defaultImpSuffix}`1",
                        "<string>",
                        true
                    );
                    GenerateImpl(
                    ...
                    GenerateImpl(
                        implClass.Name + "<!0>",
                        $"IFaceCuriouslyRecurringGeneric{defaultImpSuffix}`1",
                        $"<{implTypePrefix} {implClass.Name}<!0>>",
                        true
                    );
                }
                else
                ...
                if (!constrainedTypeDefinition.ToString().Contains("Valuetype"))
                {
                    ...
                    tw.WriteLine(
                        $"    IL_0001:  call       instance void {implClass.BaseType}::.ctor()"
                    );
                ...
                )
                {
                    ...
                    void GenerateMethodBody(bool genericMethod)
                    {
                        ...
                        implsGenerated.WriteLine(
                            $"    call string {CommonCsPrefix}Statics::MakeName(valuetype [System.Runtime]System.RuntimeTypeHandle)"
                        );

                        string methodNameToEmit = implMethodDesc.Name;
                        if (methodNameToEmit.EndsWith("<U>"))
                        {
                            methodNameToEmit = methodNameToEmit.Substring(
                                0,
                                methodNameToEmit.Length - 3
                            );
                        }
                        ...
                        implsGenerated.WriteLine(
                            $"    stsfld string {CommonCsPrefix}Statics::String"
                        );
        ...
        static string GetConstrainedTypeName(
            ConstrainedTypeDefinition isStruct,
            bool withImplPrefix = true
        )
        {
        ...
        static string AppendSuffixToConstrainedType(
            TestScenario scenario,
            string constrainedType,
            out bool invalidScenario
        )
        {
            ...
            switch (scenario.ConstrainedType)
            {
                ...
                case ConstrainedTypeScenario.GenericOverGenericStructOverTypeParameter:
                    ...
                    if (scenario.CallerScenario == CallerMethodScenario.GenericOverConstrainedType)
                        invalidScenario = true;
                    constrainedType =
                        constrainedType + $"<valuetype {CommonPrefix}GenericStruct`1<!!0>>";
                    break;
                default:
        ...
        static void Main(string[] args)
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                using StreamWriter twOutputCommon = new StreamWriter(
                    Path.Combine(outputDir, @$"{CommonAndImplAssemblyName}.il")
                );
                using StreamWriter twOutputTest = new StreamWriter(
                    Path.Combine(outputDir, @$"{TestAssemblyName}.il")
                );
                ...
                foreach (var scenario in TestScenario.GetScenarios())
                {
                    ...
                    string constrainedType = AppendSuffixToConstrainedType(
                        scenario,
                        GetConstrainedTypeName(scenario.ConstrainedTypeDefinition),
                        out bool skipScenario
                    );
                    if (skipScenario)
                        continue;
                    ...
                    switch (scenario.InterfaceType)
                    {
                        ...
                        case InterfaceType.GenericOverString:
                            if (
                                ...
                                == CallerMethodScenario.GenericOverConstrainedType
                            )
                                interfaceTypeSansImplPrefix =
                                    $"IFaceGeneric{defaultImpSuffix}`1<!!1>";
                            else
                                interfaceTypeSansImplPrefix =
                                    $"IFaceGeneric{defaultImpSuffix}`1<string>";
                            break;
                        case InterfaceType.GenericOverObject:
                            if (
                                ...
                                interfaceTypeSansImplPrefix =
                                    $"IFaceGeneric{defaultImpSuffix}`1<object>";
                            break;
                        case InterfaceType.CuriouslyRecurringGeneric:
                            interfaceTypeSansImplPrefix =
                                $"IFaceCuriouslyRecurringGeneric{defaultImpSuffix}`1<{constrainedTypePrefix}{constrainedType}>";
                            break;
                        default:
                            throw new Exception("Unexpected value");
                    }
                    ...
                    string expectedString =
                        constrainedTypePrefix
                        + AppendSuffixToConstrainedType(
                            scenario,
                            GetConstrainedTypeName(
                                scenario.ConstrainedTypeDefinition,
                                withImplPrefix: false
                            ),
                            out _
                        )
                        + "'"
                    ...
                    else
                    {
                        ...
                        switch (scenario.CallerScenario)
                        {
                            ...
                            case CallerMethodScenario.GenericOverString:
                                ...
                                if (
                                    scenario.CallerScenario == CallerMethodScenario.GenericOverInt32
                                )
                                ...
                                CallTestEntrypoint(
                                    $"        call void TestEntrypoint::{basicTestMethodName}<{methodInstantiation}>()"
                                );
                                break;

                            case CallerMethodScenario.GenericOverConstrainedType:
                                mdIndividualTestMethod.Name =
                                    $"{mdIndividualTestMethod.Name}<({(interfaceTypeSansImplPrefix.Contains("`") ? "class " : "")}{CommonPrefix}{interfaceTypeSansImplPrefix}) T,U>";

                                expectedString = expectedString.Replace(
                                    "!!0",
                                    $"{constrainedTypePrefix}{constrainedType}"
                                );
                                expectedString = expectedString.Replace(ImplPrefix, "");
                                ...
                                string callCommand =
                                    GetSetBangBang1IfNeeded("string")
                                    + $"    call void TestEntrypoint::{basicTestMethodName}<{constrainedTypePrefix}{constrainedType},string>()";
                                if (scenario.InterfaceType == InterfaceType.GenericOverObject)
                                    callCommand =
                                        callCommand
                                        + Environment.NewLine
                                        + GetSetBangBang1IfNeeded("object")
                                        + $"        call void TestEntrypoint::{basicTestMethodName}<{constrainedTypePrefix}{constrainedType},object>()";
                                ...
                                string GetSetBangBang1IfNeeded(string bangBang1Expected)
                                {
                                    ...
                                    {
                                        return $"    ldstr \"{bangBang1Expected}\""
                                            + Environment.NewLine
                    ...
                    void EmitILToCallActualMethod(TextWriter twActualIL)
                    {
                        ...
                        switch (scenario.Operation)
                        {
                            case OperationTested.Call:
                                ...
                                twActualIL.WriteLine(
                                    $"    call void class {ImplPrefix}{interfaceTypeSansImplPrefix}::{interfaceMethod}()"
                                );
                                break;
                            ...
                            case OperationTested.Ldftn:
                                EmitConstrainedPrefix();
                                twActualIL.WriteLine(
                                    $"    ldftn void class {ImplPrefix}{interfaceTypeSansImplPrefix}::{interfaceMethod}()"
                                );
                                twActualIL.WriteLine($"    volatile.");
                                twActualIL.WriteLine(
                                    $"    stsfld     native int modreq([System.Runtime]System.Runtime.CompilerServices.IsVolatile) {CommonCsPrefix}Statics::FtnHolder"
                                );
                                twActualIL.WriteLine($"    volatile.");
                                twActualIL.WriteLine(
                                    $"    ldsfld     native int modreq([System.Runtime]System.Runtime.CompilerServices.IsVolatile) {CommonCsPrefix}Statics::FtnHolder"
                                );
                            ...
                            case OperationTested.CreateDelegate:
                                ...
                                twActualIL.WriteLine($"    volatile.");
                                twActualIL.WriteLine(
                                    $"    stsfld     class [System.Runtime] System.Action modreq([System.Runtime] System.Runtime.CompilerServices.IsVolatile) {CommonCsPrefix}Statics::ActionHolder"
                                );
                                twActualIL.WriteLine($"    volatile.");
                                twActualIL.WriteLine(
                                    $"    ldsfld class [System.Runtime] System.Action modreq([System.Runtime] System.Runtime.CompilerServices.IsVolatile) {CommonCsPrefix}Statics::ActionHolder"
                                );
                                twActualIL.WriteLine(
                        ...
                        void EmitConstrainedPrefix()
                        {
                            if (
                                ...
                                twActualIL.WriteLine(
                                    $"    constrained. {constrainedTypePrefix}{constrainedType}"
                                );
                        }
                    }
                }
