...
// the subtypes here that contain a Canonical type are Node <NodeStruct<NodeSys<a>>> and Node
// <NodeClass<NodeSys<a>>>
...
public class SystemMap<a>
{
    ...
    public bool Eval()
    {
        Console.WriteLine(
            ...
            (rootStruct == null) ? "<null>" : rootStruct.ToString()
        );
        Console.WriteLine(
            "Read a static from Node<NodeClass<NodeSys<a>>>.  Got: {0}",
            (rootClass == null) ? "<null>" : rootClass.ToString()
        );
