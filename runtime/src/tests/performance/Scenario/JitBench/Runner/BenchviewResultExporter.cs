...
namespace JitBench
{
    public static class BenchviewResultExporter
    {
        ...
        static ScenarioTestModel[] ConvertRunResult(BenchmarkRunResult runResult)
        {
                ...
                )
                {
                    ...
                    testModels.Add(
                        ConvertRunResult(
                            ...
                            oldMetric => m.Equals(oldMetric) ? newMetric : default(Metric),
                            name,
                            newScenarioModelName
                        )
                    );
                }
            }
            testModels.Insert(
                ...
                ConvertRunResult(
                    runResult,
                    metrics,
                    oldMetric => metrics.Contains(oldMetric) ? oldMetric : default(Metric),
                    null,
                    name
                )
            );
        ...
        static ScenarioTestModel ConvertRunResult(
            BenchmarkRunResult runResult,
        ...
        )
        {
            ...
            {
                testModel.Performance.IterationModels
                    .Add(ConvertIterationResult(iterationResult, metricMapping));
            }
            foreach (var metric in metrics)
            {
                testModel.Performance.Metrics
                    .Add(
                        new MetricModel()
                        {
        ...
        private static string GetFilePathWithoutExtension(
            string outputDir,
            string runId,
