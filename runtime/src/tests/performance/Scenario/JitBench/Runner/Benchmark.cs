...
namespace JitBench
{
    public abstract class Benchmark
    {
        ...
        {
            using (
                var runSectionOutput = new IndentedTestOutputHelper(
                    $"Run {Name} iterations",
                    output
                )
            )
        ...
        )
        {
            ...
            foreach (string projectFileRelativePath in projectFileRelativePaths)
            {
                ...
                // Add TargetFramework and RuntimeFrameworkVersion elements with the requested values to the top
                {
                    ...
                    XmlElement runtimeFrameworkVersionElement = doc.CreateElement(
                    ...
                    targetFrameworkElement.InnerText =
                        DotNetSetup.GetTargetFrameworkMonikerForFrameworkVersion(
        ...
        BenchmarkRunResult MeasureIterations(
            TestRun run,
            ...
            using (var redirector = new ConsoleRedirector(output))
            {
                ...
                using (var harness = new XunitPerformanceHarness(args))
                {
                    ProcessStartInfo startInfo = new ProcessStartInfo(
                        run.DotNetInstallation.DotNetExe,
                    ...
                    output.WriteLine(
                        "XUnitPerfHarness doesn't log env vars it uses to run processes. To workaround, logging them here:"
                    );
                    output.WriteLine(
                        $"Environment variables: {string.Join(", ", extraEnvVars.Select(kv => kv.Key + "=" + kv.Value))}"
                    );
                    output.WriteLine($"Working directory: \"{startInfo.WorkingDirectory}\"");
                    output.WriteLine(
                        $"Command line: \"{startInfo.FileName}\" {startInfo.Arguments}"
                    );
                    ...
                    )
                    {
                        ...
                        PreIterationDelegate = scenario =>
                        {
                            stderr.Clear();
                            stdout.Clear();
                            scenario.Process.ErrorDataReceived += (
                                object sender,
                            ...
                            scenario.Process.OutputDataReceived += (
                                object sender,
                    ...
                    harness.RunScenario(
                        ...
                        sb =>
                        {
                            BenchviewResultExporter.ConvertRunResult(sb, result);
                        }
                    );
                    return result;
                }
        ...
        )
        {
                ...
                foreach (var process in processes)
                {
                    ...
                    foreach (var module in process.Modules)
                    {
                        ...
                        )
                        {
                            ...
                            {
                                Metric m = new Metric(
                                    $"PMC/{process.Name}!{moduleName}/{pmcData.Key.Name}",
                                    pmcData.Key.Unit
                                );
            ...
            {
                output.WriteLine(
                    "Error while processing ETW log: " + scenarioExecutionResult.EventLogFileName
                );
        ...
        /// </summary>
        public virtual bool TryGetBenchviewCustomMetricReporting(
