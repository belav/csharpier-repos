        ...
        public static int Main(string[] args)
        {
            ...
            Task<bool> subprocessTask = Utils.RunSubprocess(
                ...
                duringExecution: (int pid) =>
                {
                    ...
                    Stream continuationStream = IpcClient.SendMessage(
                        stream,
                        processInfoMessage,
                    ...
                    Utils.Assert(
                        envBlock.Length == continuationSizeInBytes,
                        $"Continuation size must equal the reported size in the payload response.  Expected: {continuationSizeInBytes} bytes, Received: {envBlock.Length} bytes"
                    );
                    ...
                    for (int i = 0; i < envCount; i++)
                    {
                        ...
                        string envPair = System.Text.Encoding.Unicode.GetString(
                                envBlock[start..end]
                            )
                            .TrimEnd('\0');
                    ...
                    foreach (var (key, val) in testEnvPairs)
                        Utils.Assert(
                            env.ContainsKey(key) && env[key].Equals(val),
                            $"Did not find test environment pair in the environment block: '{key}' = '{val}'"
                        );
                    Logger.logger.Log($"Saw test values in env");

                    Utils.Assert(
                        end == envBlock.Length,
                        $"Full payload should have been read. Expected: {envBlock.Length}, Received: {end}"
                    );
