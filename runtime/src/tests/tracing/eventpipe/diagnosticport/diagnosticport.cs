...
namespace Tracing.Tests.DiagnosticPortValidation
{
    public class DiagnosticPortValidation
    {
        ...
        private static async Task ConfigureAndWaitForResumeSignal(
            int pid,
            ManualResetEvent mre,
            Func<Task> resumeRuntime
        )
        {
            ...
            string resumeEventName = TestLibrary.Utilities.IsNativeAot
                ? nameof(SentinelEventSource)
                : "EEStartupStart";
            ...
            Task<bool> subprocessTask = Utils.RunSubprocess(
                ...
                duringExecution: (int pid) =>
                {
                    ...
                    if (TestLibrary.Utilities.IsMonoRuntime)
                    {
                        ...
                        Utils.Assert(
                            string.IsNullOrEmpty(processInfo2.ManagedEntrypointAssemblyName)
                        );
                    }
                    else if (TestLibrary.Utilities.IsNativeAot)
                    {
                        string expectedName = System.Reflection
                            .Assembly
                            .GetExecutingAssembly()
                            .GetName()
                            .Name;
                        Utils.Assert(
                            expectedName.Equals(processInfo2.ManagedEntrypointAssemblyName),
                    ...
                    else
                    {
                        ...
                        string expectedName = Path.GetFileNameWithoutExtension(
