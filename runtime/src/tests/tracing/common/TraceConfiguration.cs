...
namespace Tracing.Tests.Common
{
    ...
    public sealed class TraceConfiguration
    {
        ...
        public TraceConfiguration(string outputFile, uint circularBufferMB)
        {
            ...
            m_configurationObject = m_configurationCtor.Invoke(
                new object[] { outputFile, EventPipeSerializationFormat.NetTrace, circularBufferMB }
            );
        }

        public void EnableProvider(string providerName, UInt64 keywords, uint level)
        {
            m_enableProviderMethod.Invoke(
                m_configurationObject,
                new object[] { providerName, keywords, level }
            );
        }
        ...
        public void SetSamplingRate(TimeSpan minDelayBetweenSamples)
        {
            m_setProfilerSamplingRateMethod.Invoke(
                m_configurationObject,
                new object[] { minDelayBetweenSamples }
            );
        }
        ...
        private bool Initialize()
        {
            ...
            Type configurationType = SPC.GetType(
                "System.Diagnostics.Tracing.EventPipeConfiguration"
            );
