...
public abstract class WasmAppBuilderBaseTask : Task
{
    ...
    protected void ProcessSatelliteAssemblies(
        Action<(string fullPath, string culture)> addSatelliteAssemblyFunc
    )
    {
    ...
    protected virtual void UpdateRuntimeConfigJson()
    {
        ...
        if (!File.Exists(RuntimeConfigJsonPath))
        {
            Log.LogMessage(
                ...
                $"Could not find {nameof(RuntimeConfigJsonPath)}={RuntimeConfigJsonPath}. Ignoring."
            );
            return;
        }
        ...
        var rootNode = JsonNode.Parse(
            File.ReadAllText(RuntimeConfigJsonPath),
        ...
        {
            throw new LogAsErrorException(
                $"Could not find node named 'runtimeOptions' in {RuntimeConfigJsonPath}"
            );
        }
        ...
        AddToRuntimeConfig(
            ...
            runtimeArgsArray: runtimeArgsArray,
            perHostConfigs: perHostConfigs
        );
    ...
    protected virtual void AddToRuntimeConfig(
