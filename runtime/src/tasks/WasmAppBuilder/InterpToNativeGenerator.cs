...
internal sealed class InterpToNativeGenerator
{
        ...
        try
        {
            ...
            if (Utils.CopyIfDifferent(tmpFileName, outputPath, useHash: false))
                Log.LogMessage(
                    ...
                    $"Generating managed2native table to '{outputPath}'."
                );
            else
                Log.LogMessage(
                    MessageImportance.Low,
                    $"Managed2native table in {outputPath} is unchanged."
                );
        }
        finally
        {
    ...
    private static void Emit(StreamWriter w, IEnumerable<string> cookies)
    {
        ...
        foreach (var signature in signatures)
        {
            try
            {
                ...
                if (!SignatureMapper.IsVoidSignature(signature))
                {
                    ...
                    w.WriteLine(
                        $"\t*({SignatureMapper.CharToNativeType(signature[0])}*)retval = res;"
                    );
                }
            ...
            catch (InvalidSignatureCharException e)
            {
                throw new LogAsErrorException(
                    $"Element '{e.Char}' of signature '{signature}' can't be handled by managed2native generator"
                );
            }
        }
        ...
        w.WriteLine(
            $"static unsigned int interp_to_native_signatures_count = {signatures.Length};"
        );
        w.WriteLine();
        w.WriteLine(
            """
