...
internal sealed class PInvokeTableGenerator
{
    ...
    public IEnumerable<string> GenPInvokeTable(
    ...
    )
    {
        ...
        )
        {
            ...
            catch (Exception ex)
            {
                Log.LogMessage(
                    MessageImportance.Low,
                    $"Could not get pinvoke, or callbacks for method {method.Name}: {ex}"
                );
                continue;
            }
        }
        ...
        void CollectPInvokesForMethod(MethodInfo method)
        {
            if ((method.Attributes & MethodAttributes.PinvokeImpl) != 0)
            {
                ...
                Log.LogMessage(
                    ...
                    $"[pinvoke] Adding signature {signature} for method '{type.FullName}.{method.Name}'"
                );
    ...
    private string? GenPInvokeDecl(PInvoke pinvoke)
    {
        ...
        {
            Log.LogWarning(
                $"Skipping the following DllImport because '{reason}'. {Environment.NewLine}  {pinvoke.Method}"
            );
            pinvoke.Skip = true;
            return null;
        }
