...
{
    private const string s_fetchReleasesUrl =
        "https://chromiumdash.appspot.com/fetch_releases?channel={0}&num=1";
    private const string s_allJsonUrl = "http://omahaproxy.appspot.com/all.json";
    private const string s_snapshotBaseUrl =
    ...
    private async Task<bool> ExecuteInternalAsync()
    {
        ...
        try
        {
            ...
            (ChromeVersionSpec version, string baseUrl) = await FindVersionFromChromiumDash()
    ...
    private async Task<(
        ChromeVersionSpec versionSpec,
        string baseSnapshotUrl
    )> FindVersionFromHistoryAsync()
    {
        ...
        await foreach (string version in GetVersionsAsync().ConfigureAwait(false))
        {
            ...
            if (curMajorVersion != majorVersion)
            {
                ...
                if (numMajorVersionsTried > 0)
                    Log.LogMessage(
                        ...
                        $"Trying older major version {majorVersion} .."
                    );
            ...
            Log.LogMessage(
                ...
                $"Trying to get details for version {version} from {depsUrl}"
            );
            ...
            if (depsVersionSpec is null)
            {
                Log.LogMessage(
                    MessageImportance.Low,
                    $"Could not get deps for version {version} from {depsUrl}."
                );
                continue;
            }
            ...
            if (!isFirstVersion)
                Log.LogMessage(
                    ...
                    $"Trying to find snapshot url for version {version} .."
                );

            ChromeVersionSpec versionSpec = depsVersionSpec.ToChromeVersionSpec(
                OSIdentifier,
                Channel
            );
            string? baseUrl = await FindSnapshotUrlFromBasePositionAsync(
                    versionSpec,
            ...
            if (!string.IsNullOrEmpty(baseUrl))
            {
                if (numMajorVersionsTried > 1)
                    Log.LogMessage(
                        MessageImportance.High,
                        $"{Environment.NewLine}Using an *older major* version of chrome: {versionSpec.version} . Failed to find snapshots for other {OSIdentifier} {Channel} versions: {string.Join(", ", versionsTried[..^1])} .{Environment.NewLine}"
                    );
            ...
            Log.LogMessage(
                MessageImportance.Low,
                $"Could not find snapshot url for version {version} ."
            );
        }

        throw new LogAsErrorException(
            $"Could not find a snapshot for {OSIdentifier} {Channel}. Versions tried: {string.Join(", ", versionsTried)} . A fixed version+url can be set in eng/testing/ProvisioningVersions.props ."
        );

        async IAsyncEnumerable<string> GetVersionsAsync()
        {
            Stream stream = await GetDownloadFileStreamAsync("history.csv", s_historyUrl)
                .ConfigureAwait(false);
            ...
            */
            while (true)
            {
                ...
                yield return parts.Length >= 4
                    ? parts[2]
                    : throw new LogAsErrorException(
                        $"Failed to find at least 4 fields in history csv at line #{lineNum}: {line}"
                    );
            }
        }
    }
    ...
    private async Task<(
        ChromeVersionSpec version,
        string baseSnapshotUrl
    )> FindVersionFromChromiumDash()
    {
        using Stream stream = await GetDownloadFileStreamAsync(
                "fetch_releases.json",
                string.Format(s_fetchReleasesUrl, Channel)
            )
            .ConfigureAwait(false);
        ...
        if (releases is null)
            throw new LogAsErrorException(
                $"Failed to read chrome versions from {s_fetchReleasesUrl}"
            );

        ChromiumDashRelease? foundRelease = releases
            .Where(rel =>
                string.Equals(
                    rel.platform,
                    OSIdentifier,
        ...
        {
            string availablePlatformIds = string.Join(
                ", ",
                releases.Select(rel => rel.platform).Distinct().Order()
            );
            throw new LogAsErrorException(
                $"Unknown Platform '{OSIdentifier}'. Platforms found "
                    + $"in fetch_releases.json: {availablePlatformIds}"
            );
        }

        string foundV8Version = await FindV8VersionFromChromeVersion(foundRelease.version)
            .ConfigureAwait(false);
        ChromeVersionSpec versionSpec = new(
            os: foundRelease.platform,
            channel: Channel,
        ...
        string? baseSnapshotUrl = await FindSnapshotUrlFromBasePositionAsync(
                versionSpec,
        ...
        async Task<string> FindV8VersionFromChromeVersion(string chromeVersion)
        {
            ...
            try
            {
                ...
                string lkgUrl =
                    $"https://raw.githubusercontent.com/v8/v8/{majorForV8}.{minorForV8}-lkgr/include/v8-version.h";
                Log.LogMessage(MessageImportance.Low, $"Checking {lkgUrl} ...");
                string v8VersionHContents = await s_httpClient
                    .GetStringAsync(lkgUrl)
                    .ConfigureAwait(false);

                var m = V8BuildNumberRegex().Match(v8VersionHContents);
                if (!m.Success)
                    throw new LogAsErrorException(
                        $"Failed to find v8 build number at {lkgUrl}: {v8VersionHContents}"
                    );
            ...
            {
                throw new LogAsErrorException(
                    $"Failed to parse chrome version '{chromeVersion}' to extract the milestone: {ex.Message}"
                );
            }
        }
    }
    ...
    private async Task<(
        ChromeVersionSpec versionSpec,
        string baseSnapshotUrl
    )> FindVersionFromAllJsonAsync()
    {
        using Stream stream = await GetDownloadFileStreamAsync("all.json", s_allJsonUrl)
            .ConfigureAwait(false);
        ...
        PerOSVersions[] foundOSVersions = perOSVersions
            .Where(osv => osv.os == OSIdentifier)
            .ToArray();
        if (foundOSVersions.Length == 0)
        {
            string availableOSIds = string.Join(
                ", ",
                perOSVersions.Select(osv => osv.os).Distinct().Order()
            );
            throw new LogAsErrorException(
                $"Unknown OS identifier '{OSIdentifier}'. OS identifiers found "
        ...
        {
            string availableChannels = string.Join(
                ", ",
                perOSVersions
                    .SelectMany(osv => osv.versions)
            ...
            throw new LogAsErrorException(
                $"Unknown chrome channel '{Channel}'. Channels found in all.json: "
                    + availableChannels
            );
        }

        string? baseSnapshotUrl = await FindSnapshotUrlFromBasePositionAsync(
                foundChromeVersions[0],
                throwIfNotFound: true
            )
            .ConfigureAwait(false);
    ...
    private async Task<Stream> GetDownloadFileStreamAsync(string filename, string url)
    {
        ...
        if (
            ...
            && (DateTime.UtcNow - File.GetLastWriteTimeUtc(filePath)).TotalDays
                < s_versionCheckThresholdDays
        )
        {
            Log.LogMessage(
                MessageImportance.Low,
    ...
    private async Task<string?> FindSnapshotUrlFromBasePositionAsync(
        ChromeVersionSpec version,
    ...
    )
    {
        ...
        string message =
            $"Could not find a chrome snapshot folder under {baseUrl}, "
    ...
    private sealed record ChromiumDashRelease(
        string channel,
        int chromium_main_branch_position,
        string milesone,
        string platform,
        string version
    );
    ...
    private sealed record ChromeVersionSpec(
        string os,
        string channel,
        string version,
        string branch_base_position,
        string v8_version
    );

    private sealed record ChromeDepsVersionSpec(
        string chromium_version,
        string chromium_base_position,
        string v8_version
    )
    {
        public ChromeVersionSpec ToChromeVersionSpec(string os, string channel) =>
            new(os, channel, chromium_version, chromium_base_position, v8_version);
    }
}
