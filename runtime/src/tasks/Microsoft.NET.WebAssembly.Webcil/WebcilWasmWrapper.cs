...
public class WebcilWasmWrapper
{
    ...
    public void WriteWasmWrappedWebcil(Stream outputStream)
    {
        ...
        using (
            var writer = new BinaryWriter(outputStream, System.Text.Encoding.UTF8, leaveOpen: true)
        )
        {
    ...
    private void WriteDataSection(BinaryWriter writer)
    {
        ...
        uint payloadOffset =
            (uint)s_wasmWrapperPrefix.Length
            + 1
            + (uint)putativeULEBDataSectionSize.Length
            + dataSectionSize;
        ...
        if (putativeULEBDataSectionSize.Length != ulebSectionSize.Length)
            throw new InvalidOperationException(
                "adding padding would cause data section's encoded length to chane"
            ); // TODO: fixme: there's upto one extra byte to encode the section length - take away a padding byte.
        ...
        {
            throw new InvalidOperationException(
                "padding would cause segment 0 to need a multi-byte ULEB128 size encoding"
            );
        }
        ...
        {
            throw new Exception(
                $"predited offset {payloadOffset}, actual position {writer.BaseStream.Position}"
            );
        }
