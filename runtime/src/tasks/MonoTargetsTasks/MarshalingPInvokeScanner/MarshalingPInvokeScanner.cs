...
namespace MonoTargetsTasks
{
    public class MarshalingPInvokeScanner : Task
    {
        ...
        public override bool Execute()
        {
            ...
            {
                Log.LogError(
                    $"{nameof(MarshalingPInvokeScanner)}.{nameof(Assemblies)} cannot be empty"
                );
                return false;
            }
        ...
        private static string GetMethodName(MetadataReader mr, MethodDefinition md) =>
            mr.GetString(md.Name);

        private void ResolveInconclusiveTypes(
            HashSet<string> incompatible,
            string assyPath,
            MinimalMarshalingTypeCompatibilityProvider mmtcp
        )
        {
            ...
            foreach (TypeDefinitionHandle typeDefHandle in mdtReader.TypeDefinitions)
            {
                ...
                string fullTypeName = string.Join(
                    ":",
                    mdtReader.GetString(typeDef.Namespace),
                    mdtReader.GetString(typeDef.Name)
                );
                ...
                if (
                    inconclusiveTypes.Contains(fullTypeName)
                    && mmtcp.GetTypeFromDefinition(mdtReader, typeDefHandle, 0)
                ...
                )
                {
                    Log.LogMessage(
                        ...
                        string.Format(
                            "Type {0} is marshaled and requires marshal-ilgen.",
                            fullTypeName
                        )
                    );
        ...
        private bool IsAssemblyIncompatible(
            string assyPath,
            MinimalMarshalingTypeCompatibilityProvider mmtcp
        )
        {
            ...
            foreach (CustomAttributeHandle attrHandle in mdtReader.CustomAttributes)
            {
                ...
                if (attr.Constructor.Kind == HandleKind.MethodDefinition)
                {
                    ...
                    if (
                        mdtReader.GetString(td.Namespace) == "System.Runtime.CompilerServices"
            ...
            foreach (TypeDefinitionHandle typeDefHandle in mdtReader.TypeDefinitions)
            {
                ...
                foreach (MethodDefinitionHandle mthDefHandle in typeDef.GetMethods())
                {
                    ...
                    MethodSignature<Compatibility> sgn = decoder.DecodeMethodSignature(
                        ref sgnBlobReader
                    );
                    if (
                        sgn.ReturnType == Compatibility.Incompatible
                        || sgn.ParameterTypes.Any(p => p == Compatibility.Incompatible)
                    )
                    {
                        Log.LogMessage(
                            MessageImportance.Low,
                            string.Format(
                                "Assembly {0} requires marhsal-ilgen for method {1}.{2}:{3} (first pass).",
                                assyPath,
                                ns,
                                name,
                                mdtReader.GetString(mthDef.Name)
                            )
                        );
