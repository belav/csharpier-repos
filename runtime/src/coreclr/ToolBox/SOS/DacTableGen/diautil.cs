...
namespace Dia.Util
{
    ...
    ***************************************************************************************/
    ...
    class Util
    {
        ...
        {
            IDiaEnumSymbols e;
            parent.findChildren(symTag, symName, (uint)(NameSearchOptions.nsfCaseSensitive), out e);
            ...
            if (e.count > 1)
            {
                for (int i = 1; i < e.count; i++)
                {
                    ...
                    {
                        Shell.Error(
                            "Symbol " + symName + " has " + e.count + " matches. Taking first."
                        );
                        break;
                    }
                }
    ...
    ***************************************************************************************/
    ...
    public class DiaFile
    {
        ...
        private void GetPublicsEnum()
        {
            ...
            m_session.findChildren(
                m_global.IDiaSymbol,
                SymTagEnum.SymTagPublicSymbol,
                null,
                (UInt32)NameSearchOptions.nsNone,
                out m_publicsEnum
            );
        }
        ...
        private class DiaLoadCallback : IDiaLoadCallback2
        {
            ...
            )
            {
                ...
                if (pbData.Type == ImageDebugType.IMAGE_DEBUG_TYPE_CODEVIEW)
                {
                    Debug.Assert(
                        fExecutable == 1,
                        "Got debug directory that wasn't read from an executable"
                    );
                    Debug.Assert(
                        m_debugTimeDateStamp == 0,
                        "Got unexpected duplicate NotifyDebugDir callback"
                    );
                    Debug.Assert(
                        pbData.TimeDateStamp != 0,
                        "Got unexpected 0 debug timestamp in DLL"
                    );
            ...
            public void NotifyOpenPDB(string pdbPath, int resultCode)
            {
                ...
                {
                    Debug.Assert(
                        m_loadedPdbPath == null,
                        "DIA indicated it was loading more than one PDB file!"
                    );
        ...
        private interface IDiaLoadCallback
        {
            void NotifyDebugDir(
                int fExecutable,
                uint cbData,
                [In] ref IMAGE_DEBUG_DIRECTORY pbData
            );

            void NotifyOpenDBG(
                [In, MarshalAs(UnmanagedType.LPWStr)] string dbgPath,
                int resultCode
            );
        ...
        {
            new void NotifyDebugDir(
                int fExecutable,
                uint cbData,
                [In] ref IMAGE_DEBUG_DIRECTORY pbData
            );

            new void NotifyOpenDBG(
                [In, MarshalAs(UnmanagedType.LPWStr)] string dbgPath,
                int resultCode
            );

            new void NotifyOpenPDB(
                [In, MarshalAs(UnmanagedType.LPWStr)] string pdbPath,
                int resultCode
            );
    ...
    ***************************************************************************************/
    ...
    public class DiaSymbol
    {
            ...
            else if (tag == SymTagEnum.SymTagArrayType)
            {
                ...
                try
                {
                    ...
                    for (i = 0; i < e.count; i++)
                    {
                        ...
                        str.Append(
                            "["
                                + GetBoundString(ds.lowerBound)
                                + ".."
                                + GetBoundString(ds.upperBound)
                                + "]"
                        );
                    }
                }
        ...
        {
            get { return m_symbol.virtualAddress; }
        }

        public UInt32 Offset
        {
            get { return (UInt32)m_symbol.offset; }
        }
    ...
    public class DiaDataSymbol : DiaSymbol
    {
        ...
        {
            get { return ((DataKind)m_symbol.dataKind); }
        }

        public bool IsGlobal
        {
            get { return (DataKind == DataKind.DataIsGlobal); }
        }

        public bool IsFileStatic
        {
            get { return (DataKind == DataKind.DataIsFileStatic); }
        }

        public bool IsMember
        {
            get { return (DataKind == DataKind.DataIsMember); }
        }

        public bool IsStaticMember
        {
            get { return (DataKind == DataKind.DataIsStaticMember); }
        }
    }
