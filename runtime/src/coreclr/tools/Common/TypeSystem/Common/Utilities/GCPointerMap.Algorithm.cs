...
namespace Internal.TypeSystem
{
    public partial struct GCPointerMap
    {
        ...
        private static void FromInstanceLayoutHelper(
            ref GCPointerMapBuilder builder,
            MetadataType type
        )
        {
            ...
            foreach (FieldDesc field in type.GetFields())
            {
                ...
                if (fieldType.IsGCPointer)
                {
                    ...
                    {
                        builder.MarkGCPointer(
                            field.Offset.AsInt + type.Context.Target.PointerSize * i
                        );
                    }
                }
                    ...
                    if (fieldDefType.ContainsGCPointers)
                    {
                        for (int i = 0; i < repeat; i++)
                        {
                            ...
                            GCPointerMapBuilder innerBuilder = builder.GetInnerBuilder(
                                field.Offset.AsInt + fieldSize * i,
                                fieldSize
                            );
        ...
        public static GCPointerMap FromStaticLayout(MetadataType type)
        {
            ...
            Debug.Assert(
                builder.ToGCMap().Size * type.Context.Target.PointerSize
                    >= type.GCStaticFieldSize.AsInt
            );
            return builder.ToGCMap();
        }
        ...
        {
            GCPointerMapBuilder builder = new GCPointerMapBuilder(
                type.ThreadGcStaticFieldSize.AsInt,
                type.Context.Target.PointerSize
            );
