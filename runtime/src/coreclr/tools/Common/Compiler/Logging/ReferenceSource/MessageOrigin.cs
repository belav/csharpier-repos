...
namespace Mono.Linker
{
    public readonly struct MessageOrigin : IComparable<MessageOrigin>, IEquatable<MessageOrigin>
    {
            ...
            get
            {
                Debug.Assert(
                    _suppressionContextMember == null
                        || _suppressionContextMember is IMemberDefinition
                        || _suppressionContextMember is AssemblyDefinition
                );
        ...
        public MessageOrigin(
            ...
            int? ilOffset,
            ICustomAttributeProvider? suppressionContextMember
        )
        {
            Debug.Assert(
            ...
            Debug.Assert(
                suppressionContextMember == null
                    || suppressionContextMember is IMemberDefinition
                    || provider is AssemblyDefinition
            );
            FileName = null;
            Provider = provider;
        ...
        public bool Equals(MessageOrigin other) =>
            (FileName, Provider, SourceLine, SourceColumn, ILOffset)
            == (
                other.FileName,
                other.Provider,
                other.SourceLine,
        ...
        public override int GetHashCode() =>
            (FileName, Provider, SourceLine, SourceColumn).GetHashCode();
        ...
        public int CompareTo(MessageOrigin other)
        {
            if (Provider != null && other.Provider != null)
            {
                ...
                TypeDefinition? thisTypeDef =
                    (Provider as TypeDefinition) ?? (Provider as IMemberDefinition)?.DeclaringType;
                ...
                int result = (
                    ...
                    thisTypeDef?.Name,
                    thisMember?.Name
                ).CompareTo((otherAssembly?.Name.Name, otherTypeDef?.Name, otherMember?.Name));
                if (result != 0)
                    return result;
