...
namespace Internal.JitInterface
{
    internal unsafe sealed partial class CorInfoImpl
    {
        ...
        [DllImport(JitLibrary)]
        private extern static IntPtr getLikelyClass(
            PgoInstrumentationSchema* schema,
            uint countSchemaItems,
            byte* pInstrumentationData,
            int ilOffset,
            out uint pLikelihood,
            out uint pNumberOfClasses
        );
            ...
            else
            {
                ...
                for (int i = 0; i < (pgoData.Length); i++)
                {
                    if (
                        ...
                        == PgoInstrumentationKind.TypeHandleHistogramCount
                    )
                    {
        ...
        )
        {
            ...
            fixed (PgoInstrumentationSchema* pSchema = &nativeSchema[index])
            {
                ...
                {
                    IntPtr classType = getLikelyClass(
                        pSchema,
                        2,
                        pInstrumentationData,
                        nativeSchema[index].ILOffset,
                        out uint likelihood,
                        out uint numberOfClasses
                    );
        ...
        private MethodIL HandleToObject(CORINFO_MODULE_STRUCT_* module) =>
            (MethodIL)HandleToObject((IntPtr)module);
        private CORINFO_MODULE_STRUCT_* ObjectToHandle(MethodIL methodIL) =>
                    ...
                    Debug.Assert(
                        ((TypeDesc)result).IsRuntimeDeterminedSubtype
                           ...
                           by resolving the token in the definition. */
                            (
                                (TypeDesc)methodIL.GetMethodILDefinition()
        ...
        private void resolveToken(ref CORINFO_RESOLVED_TOKEN pResolvedToken)
        {
            ...
            if (result is MethodDesc)
            {
                ...
                if (recordToken)
                {
                    _compilation.NodeFactory.Resolver.AddModuleTokenForMethod(
                        method,
        ...
        private void allocMem(
            ...
            ref void* hotCodeBlock,
            ref void* coldCodeBlock,
            ref void* roDataBlock
        )
        {
