...
namespace ILCompiler
{
    internal class Program
    {
        ...
        public int Run()
        {
            ...
            Dictionary<string, string> inputFilePathsArg = _command.Result.GetValueForArgument(
        ...
        private MethodDesc CheckAndParseSingleMethodModeArguments(CompilerTypeSystemContext context)
        {
            ...
            if (
                method.HasInstantiation != (singleMethodGenericArgs != null)
                || (
                    method.HasInstantiation
                    && (method.Instantiation.Length != singleMethodGenericArgs.Length)
                )
            )
            {
                throw new CommandLineException(
        ...
        private static int Main(string[] args) =>
            new CommandLineBuilder(new Crossgen2RootCommand(args))
                ...
                .UseHelp(
                    context =>
                        context.HelpBuilder.CustomizeLayout(Crossgen2RootCommand.GetExtendedHelp)
                )
