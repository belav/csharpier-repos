...
namespace Mono.Linker.Tests.TestCasesRunner
{
    public class TestCaseCompiler
    {
        ...
        public NPath CompileTestIn(
            NPath outputDirectory,
            string outputName,
            IEnumerable<string> sourceFiles,
            string[] commonReferences,
            string[] mainAssemblyReferences,
        ...
        protected virtual CompilerOptions CreateOptionsForSupportingAssembly(
            SetupCompileInfo setupCompileInfo,
            NPath outputDirectory,
            NPath[] sourceFiles,
            NPath[] references,
            string[] defines,
        ...
        {
            var allDefines = defines
                .Concat(setupCompileInfo.Defines ?? Array.Empty<string>())
                .ToArray();
            var allReferences = references
                .Concat(
                    setupCompileInfo.References
                        ?.Select(p =>
        ...
        private IEnumerable<NPath> CompileBeforeTestCaseAssemblies(
            NPath outputDirectory,
            NPath[] references,
            string[] defines,
            IList<NPath> removeFromLinkerInputAssemblies
        )
        {
        ...
        private void CompileAfterTestCaseAssemblies(
            NPath outputDirectory,
            NPath[] references,
            string[] defines,
            IList<NPath> removeFromLinkerInputAssemblies
        ...
        {
            return CollectSourceFilesFrom(
                _sandbox.BeforeReferenceSourceDirectoryFor(info.OutputName)
            );
        }
        ...
        {
            return CollectSourceFilesFrom(
                _sandbox.AfterReferenceSourceDirectoryFor(info.OutputName)
            );
        }
        ...
        protected static NPath MakeSupportingAssemblyReferencePathAbsolute(
            NPath outputDirectory,
        ...
        )
        {
...
#if NETCOREAPP
            if (
                referenceFileName.StartsWith("System.", StringComparison.Ordinal)
            ...
            {
                var frameworkDir = Path.GetFullPath(
        ...
        protected NPath CompileAssembly(CompilerOptions options)
        {
            ...
            throw new NotSupportedException(
                $"Unable to compile sources files with extension `{options.SourceFiles.First().ExtensionWithDot}`"
            );
        }
        ...
        protected virtual NPath CompileCSharpAssemblyWithRoslyn(CompilerOptions options)
        {
            ...
            var compilationOptions = new CSharpCompilationOptions(
                outputKind: options.OutputPath.FileName.EndsWith(".exe")
                    ? OutputKind.ConsoleApplication
                    : OutputKind.DynamicallyLinkedLibrary,
                assemblyIdentityComparer: DesktopAssemblyIdentityComparer.Default
            );
            // Default debug info format for the current platform.
            DebugInformationFormat debugType = RuntimeInformation.IsOSPlatform(OSPlatform.Windows)
                    ...
                    switch (option)
                    {
                        ...
                        default:
                            ...
                            if (splitIndex != -1 && option[..splitIndex] == "/main")
                            {
                                ...
                                compilationOptions = compilationOptions.WithMainTypeName(
                                    mainTypeName
                                );
                                break;
                            }
            ...
            var pdbPath =
                (!emitPdb || debugType == DebugInformationFormat.Embedded)
                    ? null
            ...
            var manifestResources = options.Resources
                .Select(r =>
                {
        ...
        protected virtual NPath CompileCSharpAssemblyWithCsc(CompilerOptions options)
        {
            ...
            return CompileCSharpAssemblyWithRoslyn(options);
#else
            return CompileCSharpAssemblyWithExternalCompiler(
                LocateCscExecutable(),
                options,
                "/shared "
            );
        ...
        protected virtual NPath CompileCSharpAssemblyWithMcs(CompilerOptions options)
        {
            if (Environment.OSVersion.Platform == PlatformID.Win32NT)
                CompileCSharpAssemblyWithExternalCompiler(
                    LocateMcsExecutable(),
                    options,
                    string.Empty
                );
        ...
        protected NPath CompileCSharpAssemblyWithExternalCompiler(
            string executable,
            CompilerOptions options,
            string compilerSpecificArguments
        )
        {
            var capturedOutput = new List<string>();
            var process = new Process();
            process.StartInfo.FileName = executable;
            process.StartInfo.Arguments = OptionsToCompilerCommandLineArguments(
                options,
        ...
        private static string LocateMcsExecutable()
        {
            if (Environment.OSVersion.Platform == PlatformID.Win32NT)
                throw new IgnoreTestException(
                    "We don't have a universal way of locating mcs on Windows"
                );

            return "mcs";
        }

        protected string OptionsToCompilerCommandLineArguments(
            CompilerOptions options,
            string compilerSpecificArguments
        )
        {
            ...
            if (options.Defines != null && options.Defines.Length > 0)
                builder.Append(
                    options.Defines.Aggregate(string.Empty, (buff, arg) => $"{buff} /define:{arg}")
                );

            builder.Append(
                options.References.Aggregate(string.Empty, (buff, arg) => $"{buff} /r:{arg}")
            );

            if (options.Resources != null && options.Resources.Length > 0)
                builder.Append(
                    options.Resources.Aggregate(string.Empty, (buff, arg) => $"{buff} /res:{arg}")
                );
            ...
            builder.Append(
                options.SourceFiles.Aggregate(string.Empty, (buff, arg) => $"{buff} {arg}")
            );
