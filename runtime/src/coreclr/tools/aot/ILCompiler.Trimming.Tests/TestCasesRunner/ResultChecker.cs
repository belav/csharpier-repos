                ...
                if (!HasAttribute(original, nameof(NoLinkedOutputAttribute)))
                {
                    ...
                    if (
                        !HasActiveSkipKeptItemsValidationAttribute(
                            testResult.TestCase.FindTypeDefinition(original)
                        )
                    )
                    {
                        ...
                        case nameof(LogContainsAttribute):
                            {
                                ...
                                if ((bool)attr.ConstructorArguments[1].Value)
                                    ...
                                    matchedMessages = loggedMessages
                                        .Where(m =>
                                            MessageTextContains(m.ToString(), expectedMessage)
                                        )
                                        .ToList();
                                Assert.True(
                                    ...
                                    else if (isCompilerGeneratedCode == true)
                                    {
                                        ...
                                        if (attrProvider is IMemberDefinition expectedMember)
                                        {
                                            ...
                                            if (actualName?.StartsWith(expectedTypeName) == true)
                                            {
                                                ...
                                                if (
                                                    methodDesc.IsConstructor
                                                    && (
                                                        expectedMember is FieldDefinition
                                                        || expectedMember is PropertyDefinition
                                                        || new AssemblyQualifiedToken(
            ...
            foreach ((var attrProvider, var attr) in expectedNoWarningsAttributes)
            {
                ...
                foreach (var mc in logger.GetLoggedMessages())
                {
                    ...
                    if (
                        ...
                        && member.ToString()?.Contains(attrMember.FullName) != true
                    )
                        continue;
            ...
            )
            {
                ...
                if (origin?.MemberDefinition is IAssemblyDesc asm)
                    return expectedOriginProvider is AssemblyDefinition expectedAsm
                        && asm.GetName().Name == expectedAsm.Name.Name;
