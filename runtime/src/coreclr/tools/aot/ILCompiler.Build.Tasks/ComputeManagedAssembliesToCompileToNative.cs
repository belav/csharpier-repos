...
namespace Build.Tasks
{
    public class ComputeManagedAssembliesToCompileToNative : Task
    {
        ...
        /// </summary>
        [Required]
        public ITaskItem[] SdkAssemblies { get; set; }
        ...
        [Output]
        public ITaskItem[] AssembliesToSkipPublish { get; set; }
        ...
        public override bool Execute()
        {
            ...
            foreach (ITaskItem taskItem in Assemblies)
            {
                ...
                if (
                    itemSpec.EndsWith(
                        DotNetAppHostExecutableName,
                        StringComparison.OrdinalIgnoreCase
                    )
                    || itemSpec.Contains(DotNetHostFxrLibraryName)
                    || itemSpec.Contains(DotNetHostPolicyLibraryName)
                )
                {
                ...
                if (
                    itemSpec.IndexOf("microsoft.netcore.app", StringComparison.OrdinalIgnoreCase)
                        != -1
