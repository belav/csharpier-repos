    ...
    partial public class SyntheticVirtualOverrideTests
    {
        ...
        public void TestStructEqualsAndGetHashCode()
        {
            ...
            MetadataType t = _testModule.GetType(
                "SyntheticVirtualOverride",
                "StructWithNoEqualsAndGetHashCode"
            );

            Assert.DoesNotContain(t.GetMethods(), m => m.Name == "Equals");
            Assert.DoesNotContain(t.GetMethods(), m => m.Name == "GetHashCode");

            List<MethodDesc> introducedVirtualMethods = new List<MethodDesc>(
                t.GetAllMethods().Where(m => m.IsVirtual)
            );
            ...
            List<MethodDesc> vtable = virtualSlots
                .Select(t.FindVirtualFunctionTargetMethodOnObjectType)
                .ToList();
        ...
        public void TestUnoverriddenSyntheticEqualsAndGetHashCode()
        {
            ...
            MetadataType baseType = _testModule.GetType(
                "SyntheticVirtualOverride",
                "ClassWithInjectedEqualsAndGetHashCode"
            );
            MetadataType t = _testModule.GetType(
                "SyntheticVirtualOverride",
                "ClassNotOverridingEqualsAndGetHashCode"
            );
        ...
        private sealed class SyntheticVirtualOverrideTypeSystemContext : TestTypeSystemContext
        {
            ...
            private MethodDesc GetGetHashCodeMethod(TypeDesc type)
            {
                ...
                {
                    result = new SyntheticMethod(
                        type,
                        "GetHashCode",
                        new MethodSignature(
                            0,
                            0,
                            type.Context.GetWellKnownType(WellKnownType.Int32),
                ...
                if (!_equalsMethods.TryGetValue(type, out result))
                {
                    result = new SyntheticMethod(
                        ...
                        new MethodSignature(
                            0,
                            0,
                            type.Context.GetWellKnownType(WellKnownType.Boolean),
        ...
        partial private sealed class SyntheticMethod : MethodDesc
        {
