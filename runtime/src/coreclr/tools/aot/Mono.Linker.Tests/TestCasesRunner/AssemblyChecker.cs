    ...
    public class AssemblyChecker
    {
        ...
        public void Verify()
        {
            ...
            foreach (var originalModule in originalAssembly.Modules)
                VerifyModule(
                    originalModule,
                    linkedAssembly.Modules.FirstOrDefault(m => m.Name == originalModule.Name)
                );
        ...
        protected virtual void VerifyTypeDefinition(TypeDefinition original, TypeDefinition? linked)
        {
            ...
            {
                foreach (
                    var attr in original.CustomAttributes.Where(
                        l => l.AttributeType.Name == nameof(CreatedMemberAttribute)
                    )
                )
                {
                    var newName =
                        original.FullName + "::" + attr.ConstructorArguments[0].Value.ToString();
        ...
        protected virtual void VerifyTypeDefinitionKept(
            TypeDefinition original,
            TypeDefinition? linked
        )
        {
            if (linked == null)
            {
            ...
            foreach (var td in original.NestedTypes)
            {
                VerifyTypeDefinition(
                    td,
                    linked?.NestedTypes.FirstOrDefault(l => td.FullName == l.FullName)
                );
                linkedMembers.Remove(td.FullName);
            }
        ...
        private void VerifyFieldKept(FieldDefinition src, FieldDefinition? linked)
        {
            ...
            if (!Equals(src.Constant, linked.Constant))
            {
                Assert.True(
                    false,
                    $"Field '{src}' value doesn's match. Expected {src.Constant}, actual {linked.Constant}"
                );
            }
        ...
        )
        {
            ...
            if (src.Constant != linked.Constant)
            {
                Assert.True(
                    false,
                    $"Property '{src}' value doesn's match. Expected {src.Constant}, actual {linked.Constant}"
                );
            }
        ...
        private void VerifyMemberBackingField(IMemberDefinition src, TypeDefinition linkedType)
        {
            ...
            VerifyFieldKept(
                srcField,
                linkedType?.Fields.FirstOrDefault(l => srcField.Name == l.Name)
            );
        ...
        protected virtual void VerifyArrayInitializers(
            MethodDefinition src,
            MethodDefinition linked
        )
        {
            var expectedIndices = GetCustomAttributeCtorValues<object>(
                    src,
                    nameof(KeptInitializerData)
                )
                .Cast<int>()
                .ToArray();

            var expectKeptAll = src.CustomAttributes.Any(
                attr =>
                    attr.AttributeType.Name == nameof(KeptInitializerData)
            ...
            if (!src.HasBody)
                Assert.True(
                    false,
                    $"`{nameof(KeptInitializerData)}` cannot be used on methods that don't have bodies"
                );

            var srcImplementationDetails = src.Module.Types.FirstOrDefault(
                t =>
                    string.IsNullOrEmpty(t.Namespace)
                    && t.Name.StartsWith("<PrivateImplementationDetails>")
            );
            ...
            var linkedImplementationDetails = linked.Module.Types.FirstOrDefault(
                t =>
                    string.IsNullOrEmpty(t.Namespace)
                    && t.Name.StartsWith("<PrivateImplementationDetails>")
            );
            ...
            if (linkedImplementationDetails == null)
            {
                Assert.True(
                    false,
                    "Could not locate <PrivateImplementationDetails> in the linked assembly"
                );
                return;
            }
            ...
            var possibleInitializerFields = src.Body.Instructions
                .Where(
                    ins => IsLdtokenOnPrivateImplementationDetails(srcImplementationDetails, ins)
                )
            ...
            if (expectKeptAll)
            {
                ...
                {
                    var linkedField = linkedImplementationDetails.Fields.FirstOrDefault(
                        f => f.InitialValue.SequenceEqual(srcField.InitialValue)
                    );
            ...
            else
            {
                foreach (var index in expectedIndices)
                {
                    if (index < 0 || index > possibleInitializerFields.Length)
                        Assert.True(
                            false,
                            $"Invalid expected index `{index}` in {src}.  Value must be between 0 and {expectedIndices.Length}"
                        );
        ...
        {
            var fields = src.Fields.Where(
                f =>
                    f.CustomAttributes.Any(
                        attr => attr.AttributeType.Name == nameof(KeptFixedBufferAttribute)
                    )
            );
            ...
            foreach (var field in fields)
            {
                ...
                // Some versions of csc name it `<fieldname>e__FixedBuffer0`
                // while mcs and other versions of csc name it `<fieldname>__FixedBuffer0`
                var originalCompilerGeneratedBufferType = src.NestedTypes.FirstOrDefault(
                    t =>
                        t.FullName.Contains($"<{field.Name}>")
                        && t.FullName.Contains("__FixedBuffer")
                );
                ...
                var linkedCompilerGeneratedBufferType = linked.NestedTypes.FirstOrDefault(
                    t => t.Name == originalCompilerGeneratedBufferType.Name
                );
                if (linkedCompilerGeneratedBufferType == null)
                {
                    Assert.True(
                        false,
                        $"Missing expected type {originalCompilerGeneratedBufferType}"
                    );
                    return;
                }
                ...
                VerifyTypeDefinitionKept(
                    originalCompilerGeneratedBufferType,
                    linkedCompilerGeneratedBufferType
                );
        ...
        {
            var expectedFieldNames = GetCustomAttributeCtorValues<string>(
                    src,
                    nameof(KeptDelegateCacheFieldAttribute)
                )
            ...
            where T : MemberReference, ICustomAttributeProvider
        {
            ...
            return GetCustomAttributeCtorValues<string>(cap, nameof(KeptMemberAttribute))
                .Any(a => a == (signature ?? member.Name));
        }
