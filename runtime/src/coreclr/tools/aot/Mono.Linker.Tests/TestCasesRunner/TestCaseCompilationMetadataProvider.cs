...
namespace Mono.Linker.Tests.TestCasesRunner
{
    public class TestCaseCompilationMetadataProvider : BaseMetadataProvider
    {
        ...
        public virtual TestRunCharacteristics Characteristics =>
            ...
            | TestRunCharacteristics.SupportsStaticInterfaceMethods;
        ...
        public virtual bool IsIgnored([NotNullWhen(true)] out string? reason)
        {
            ...
            if (ignoreAttribute != null)
            {
                ...
                else
                {
                    throw new ArgumentException(
                        $"Unhandled {nameof(IgnoreTestCaseAttribute)} constructor with {ignoreAttribute.ConstructorArguments} arguments"
                    );
                }
            }
            ...
            if (requirementsAttribute != null)
            {
                ...
                {
                    var testCaseRequirements = (TestRunCharacteristics)
                        requirementsAttribute.ConstructorArguments[0].Value;

                    foreach (var value in Enum.GetValues(typeof(TestRunCharacteristics)))
                    {
                        if (
                            IsRequirementMissing(
                                (TestRunCharacteristics)value,
                ...
                {
                    throw new ArgumentException(
                        $"Unhandled {nameof(TestCaseRequirementsAttribute)} constructor with {requirementsAttribute.ConstructorArguments} arguments"
                    );
                }
            }
        ...
        private bool IsRequirementMissing(
            TestRunCharacteristics requirement,
            TestRunCharacteristics testCaseRequirements
        )
        {
            return testCaseRequirements.HasFlag(requirement)
                && !Characteristics.HasFlag(requirement);
        }
        ...
        {
            return GetOptionAttributeValue(nameof(SetupCSharpCompilerToUseAttribute), string.Empty)!
                .ToLowerInvariant();
        }
        ...
        private static string GetReferenceDir()
        {
            ...
            return Path.Combine(
                ...
                ncaVersion,
                "ref",
                PathUtilities.TFMDirectoryName
            );
        }
        ...
        public virtual IEnumerable<string> GetCommonReferencedAssemblies(NPath workingDirectory)
        {
            yield return workingDirectory
                .Combine("Mono.Linker.Tests.Cases.Expectations.dll")
                .ToString();
            if (Characteristics.HasFlag(TestRunCharacteristics.TargetingNetCore))
            {
        ...
        private SetupCompileInfo CreateSetupCompileAssemblyInfo(CustomAttribute attribute)
        {
            ...
            return new SetupCompileInfo
            {
                ...
                References = ((CustomAttributeArgument[])ctorArguments[2].Value)
                    ?.Select(arg => arg.Value.ToString())
                    .ToArray(),
                Defines = ((CustomAttributeArgument[])ctorArguments[3].Value)
                    ?.Select(arg => arg.Value.ToString())
                    .ToArray(),
        ...
        protected SourceAndDestinationPair[]? ResourcesForAttributeArgument(
