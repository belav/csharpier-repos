...
namespace Mono.Linker.Tests.TestCasesRunner
{
    public class ResultChecker
    {
        ...
        public virtual void Check(ILCompilerTestCaseResult testResult)
        {
            ...
            try
            {
                var original = ResolveOriginalsAssembly(
                    testResult.ExpectationsAssemblyPath.FileNameWithoutExtension
                );
                ...
                if (!HasAttribute(original, nameof(NoLinkedOutputAttribute)))
                {
                    ...
                    if (
                        !HasAttribute(
                            original
                                .MainModule
                                .GetType(testResult.TestCase.ReconstructedFullTypeName),
        ...
        private static void PerformOutputAssemblyChecks(
            AssemblyDefinition original,
            ILCompilerTestCaseResult testResult
        )
        {
...
#pragma warning disable IDE0060 // Remove unused parameter
        private static void PerformOutputSymbolChecks(
            AssemblyDefinition original,
            ILCompilerTestCaseResult testResult
        )
#pragma warning restore IDE0060 // Remove unused parameter
        ...
        )
        {
                                ...
                                foreach (var loggedMessage in loggedMessages)
                                {
                                    ...
                                    else if (isCompilerGeneratedCode == true)
                                    {
                                        ...
                                        )
                                        {
                                            ...
                                            string? actualName =
                                                NameUtils.GetActualOriginDisplayName(methodDesc);
                                            string expectedTypeName =
                                                NameUtils.GetExpectedOriginDisplayName(
                                                    expectedMember.DeclaringType
                                                );
                                            if (
                                                actualName?.Contains(expectedTypeName) == true
                                                && actualName?.Contains(
                                                    "<" + expectedMember.Name + ">"
                                                ) == true
                                            )
                                            ...
                                            if (
                                                actualName?.StartsWith(expectedTypeName) == true
            ...
            )
            {
                ...
                if (
                    NameUtils.GetActualOriginDisplayName(origin?.MemberDefinition)
                    == NameUtils.GetExpectedOriginDisplayName(expectedOriginProvider)
                )
                    return true;
            ...
            static bool MessageTextContains(string message, string value)
            {
                ...
                return message.Contains(value)
                    || message.Contains(NameUtils.ConvertSignatureToIlcFormat(value));
            }
        }
...
#nullable enable
        private static bool TryGetCustomAttribute(
        ...
        private static IEnumerable<CustomAttribute> GetCustomAttributes(
            ICustomAttributeProvider caProvider,
            string attributeName
        )
        {
