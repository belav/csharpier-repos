...
namespace Internal.JitInterface
{
    ...
    public class MethodWithToken
    {
        ...
        )
        {
            ...
            {
                var methodSpecification = moduleToken.MetadataReader
                    .GetMethodSpecification((MethodSpecificationHandle)moduleToken.Handle);
            ...
            {
                var methodDefinition = moduleToken.MetadataReader
                    .GetMethodDefinition((MethodDefinitionHandle)moduleToken.Handle);
            ...
            var memberRef = moduleToken.MetadataReader
                .GetMemberReference((MemberReferenceHandle)moduleToken.Handle);
    ...
    unsafe partial class CorInfoImpl
    {
                    ...
                    if (methodIL != null)
                    {
                        ...
                        else
                        {
                            if (logger.IsVerbose)
                                logger.Writer
                                    .WriteLine(
                                        $"Warning: Method `{MethodBeingCompiled}` was not compiled because it always throws an exception"
                                    );
                        }
                    }
                }
            ...
            switch (id)
            {
                case CorInfoHelpFunc.CORINFO_HELP_READYTORUN_NEW:

                    {
                        ...
                        pLookup = CreateConstLookupToSymbol(
                            _compilation.SymbolNodeFactory
                ...
                case CorInfoHelpFunc.CORINFO_HELP_READYTORUN_STATIC_BASE:

                    {
                        ...
                        pLookup = CreateConstLookupToSymbol(
                            _compilation.SymbolNodeFactory
                                .CreateReadyToRunHelper(ReadyToRunHelperId.CctorTrigger, type)
                        );
                    }
                    break;
                case CorInfoHelpFunc.CORINFO_HELP_READYTORUN_GENERIC_HANDLE:

                    {
                        ...
                        ISymbolNode helper = _compilation.SymbolNodeFactory
                            .GenericLookupHelper(
        ...
        )
        {
            ...
            MethodWithToken targetMethod = new MethodWithToken(
                ...
                constrainedType: null,
                unboxing: false,
                context: entityFromContext(pTargetMethod.tokenContext)
            );
            ...
            if (
                ...
                && (
                    _compilation.NodeFactory.CompilationModuleGroup
                        .VersionsWithMethodBody(methodDesc) || methodDesc.IsPInvoke
                )
            )
            {
                if (
        ...
        private ModuleToken HandleToModuleToken(ref CORINFO_RESOLVED_TOKEN pResolvedToken)
        {
            ...
            if (isFauxMethodIL)
            {
                ...
                if (resultDef is MethodDesc resultMethod)
                {
                    ...
                    Debug.Assert(
                        _compilation.NodeFactory.CompilationModuleGroup
                            .VersionsWithType(((EcmaMethod)resultMethod).OwningType)
                    );
                ...
                else if (resultDef is FieldDesc resultField)
                {
                    ...
                    Debug.Assert(
                        _compilation.NodeFactory.CompilationModuleGroup
                            .VersionsWithType(((EcmaField)resultField).OwningType)
                    );
        ...
        )
        {
            ...
            ISymbolNode stringObject = _compilation.SymbolNodeFactory
                .StringLiteral(new ModuleToken(method.Module, metaTok));
            ...
            if (field.IsStatic)
            {
                ...
                else if (field.OwningType.IsCanonicalSubtype(CanonicalFormKind.Any))
                {
                    ...
                    )
                    {
                        ...
                        _methodCodeNode.Fixups
                            .Add(_compilation.SymbolNodeFactory.CheckFieldOffset(field));
                    }
                }
                else
                {
                    ...
                    )
                    {
                        ...
                        pResult->fieldLookup = CreateConstLookupToSymbol(
                            _compilation.SymbolNodeFactory.FieldAddress(field)
                        );
        ...
        )
        {
            ...
            useInstantiatingStub =
                ...
                || originalMethod.GetCanonMethodTarget(CanonicalFormKind.Specific)
            ...
            else
            {
                ...
                {
                    MethodDesc methodOnUnderlyingType = constrainedType.UnderlyingType
                ...
                MethodDesc directMethod = constrainedType.TryResolveConstraintMethodApprox(
                    exactType,
                    originalMethod,
            ...
            else
            {
                ...
                pResult->exactContextNeedsRuntimeLookup = exactType.IsCanonicalSubtype(
                ...
                )
                {
                    ...
                    devirt =
                        ...
                        || (
                            ...
                            && getIntrinsicID(targetMethod, null)
                                != CorInfoIntrinsics.CORINFO_INTRINSIC_Illegal
                        );
                ...
                )
                {
                    ...
                    else
                    {
                        ...
                        DictionaryEntryKind entryKind = (
                            constrainedType != null && constrainedType.IsValueType
        ...
        private void classMustBeLoadedBeforeCodeIsRun(TypeDesc type)
        {
            if (!type.IsPrimitive)
            {
                ISymbolNode node = _compilation.SymbolNodeFactory
        ...
        )
        {
            ...
            Import typeHandleImport = (Import)_compilation.SymbolNodeFactory
                .CreateReadyToRunHelper(ReadyToRunHelperId.TypeHandle, type);
        ...
        )
        {
            ...
            else
            {
                ...
                {
                    case CorInfoGenericHandleType.CORINFO_HANDLETYPE_CLASS:
                        symbolNode = _compilation.SymbolNodeFactory
                            .CreateReadyToRunHelper(
        ...
        )
        {
            ...
            else if (!_compilation.IsLayoutFixedInCurrentVersionBubble(pMT))
            {
                ...
                else
                {
                    ...
                    pResult->fieldLookup = CreateConstLookupToSymbol(
                        _compilation.SymbolNodeFactory.FieldOffset(field)
                    );
                }
            }
        ...
        partial void PublishProfileData()
        {
            ...
            {
                MethodIL methodIL = _compilation.GetMethodIL(MethodBeingCompiled);
                _profileDataNode.SetProfileData(
                    methodIL.GetILBytes().Length,
                    _bbCounts.Length / sizeof(BlockCounts),
                    _bbCounts
                );
            }
        }
        ...
        private int SizeOfPInvokeTransitionFrame =>
            ...
            * _compilation.NodeFactory.Target.PointerSize;
        private int SizeOfReversePInvokeTransitionFrame =>
            ReadyToRunRuntimeConstants.READYTORUN_ReversePInvokeTransitionFrameSizeInPointerUnits(
                _compilation.NodeFactory.Target.Architecture
            ) * _compilation.NodeFactory.Target.PointerSize;
