...
namespace ILCompiler.DependencyAnalysis.ReadyToRun
{
    public class EnclosingTypeMapNode : ModuleSpecificHeaderTableNode
    {
        ...
        public EnclosingTypeMapNode(EcmaModule module)
            : base(module)
        {
        ...
        public static bool IsSupported(MetadataReader metadata)
        {
            ...
            return (
                (typeDefinitionCount > 10)
                && (typeDefinitionCount <= (int)ReadyToRunEnclosingTypeMap.MaxTypeCount)
            );
        }
        ...
        public override ObjectData GetData(NodeFactory factory, bool relocsOnly = false)
        {
            ...
            foreach (var typeDefinitionHandle in _metadata.TypeDefinitions)
            {
                ...
                else
                {
                    builder.EmitUShort(
                        checked(
                            (ushort)MetadataTokens.GetRowNumber(typeDefinition.GetDeclaringType())
                        )
                    );
                }
            }
