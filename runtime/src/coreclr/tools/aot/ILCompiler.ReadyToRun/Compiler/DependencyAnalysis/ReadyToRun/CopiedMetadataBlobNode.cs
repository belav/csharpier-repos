...
namespace ILCompiler.DependencyAnalysis.ReadyToRun
{
    ...
    public class CopiedMetadataBlobNode : ObjectNode, ISymbolDefinitionNode
    {
        ...
        private void WriteMethodTableRvas(
            NodeFactory factory,
        ...
        )
        {
            ...
            for (int i = 1; i <= rowCount; i++)
            {
                ...
                builder.EmitReloc(
                    factory.CopiedFieldRva(fieldDesc),
                    RelocType.IMAGE_REL_BASED_ADDR32NB
                );
                if (compressedFieldRef)
                {
        ...
        public override unsafe ObjectData GetData(NodeFactory factory, bool relocsOnly = false)
        {
            ...
            BlobReader metadataBlob = new BlobReader(
                _sourceModule.PEReader.GetMetadata().Pointer,
                _sourceModule.PEReader.GetMetadata().Length
            );
            ...
            builder.EmitBytes(
                metadataBlob.ReadBytes(metadataReader.MetadataLength - metadataBlob.Offset)
            );
