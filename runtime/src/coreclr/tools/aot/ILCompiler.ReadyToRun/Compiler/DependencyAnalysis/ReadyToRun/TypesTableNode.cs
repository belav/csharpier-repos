...
namespace ILCompiler.DependencyAnalysis.ReadyToRun
{
    public class TypesTableNode : HeaderTableNode
    {
        ...
        public TypesTableNode(TargetDetails target, EcmaModule module) : base(target)
        {
            _module = module;
        }
        ...
        public override ObjectData GetData(NodeFactory factory, bool relocsOnly = false)
        {
            ...
            )
            {
                ...
                for (;;)
                {
                    TypeDefinition defType = defTypeInfo.MetadataReader
                        .GetTypeDefinition(defTypeHandle);
                ...
                typesHashtable.Append(
                    unchecked((uint)hashCode),
                    section.Place(
                        new UnsignedConstant(
                            ((uint)MetadataTokens.GetRowNumber(defTypeInfo.Handle) << 1) | 0
                        )
                    )
                );
            }
            ...
            )
            {
                ...
                for (;;)
                {
                    ExportedType expType = expTypeInfo.MetadataReader
                        .GetExportedType(expTypeHandle);
                ...
                typesHashtable.Append(
                    ...
                    section.Place(
                        new UnsignedConstant(
                            ((uint)MetadataTokens.GetRowNumber(expTypeInfo.Handle) << 1) | 1
                        )
                    )
                );
            }
