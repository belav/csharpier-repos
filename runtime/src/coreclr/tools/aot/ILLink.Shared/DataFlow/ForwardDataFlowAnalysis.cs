        ...
        where TControlFlowGraph : IControlFlowGraph<TBlock, TRegion>
    {
        ...
        private struct ControlFlowGraphState
        {
            ...
            public bool TryGetExceptionState(TBlock block, out Box<TValue>? state)
            {
                state = null;
                if (
                    !cfg.TryGetEnclosingTryOrCatchOrFilter(
                        block,
                        out TRegion? tryOrCatchOrFilterRegion
                    )
                )
                    return false;
        ...
        public void Fixpoint(TControlFlowGraph cfg, TLattice lattice, TTransfer transfer)
        {
            ...
            while (changed)
            {
                ...
                foreach (var block in cfg.Blocks)
                {
                    ...
                    bool isTryOrCatchOrFilterBlock = cfg.TryGetEnclosingTryOrCatchOrFilter(
                        block,
                        out TRegion? tryOrCatchOrFilterRegion
                    );

                    bool isTryBlock =
                        isTryOrCatchOrFilterBlock
                        && tryOrCatchOrFilterRegion!.Kind == RegionKind.Try;
                    bool isTryStart =
                        isTryBlock && block.Equals(cfg.FirstBlock(tryOrCatchOrFilterRegion!));
                    bool isCatchBlock =
                        isTryOrCatchOrFilterBlock
                        && tryOrCatchOrFilterRegion!.Kind == RegionKind.Catch;
                    bool isCatchStartWithoutFilter =
                        isCatchBlock
                        && block.Equals(cfg.FirstBlock(tryOrCatchOrFilterRegion!))
                        && !cfg.HasFilter(tryOrCatchOrFilterRegion!);
                    bool isFilterBlock =
                        isTryOrCatchOrFilterBlock
                        && tryOrCatchOrFilterRegion!.Kind == RegionKind.Filter;
                    bool isFilterStart =
                        isFilterBlock && block.Equals(cfg.FirstBlock(tryOrCatchOrFilterRegion!));

                    bool isCatchOrFilterStart = isCatchStartWithoutFilter || isFilterStart;

                    bool isFinallyBlock = cfg.TryGetEnclosingFinally(
                        block,
                        out TRegion? finallyRegion
                    );
                    bool isFinallyStart =
                        isFinallyBlock && block.Equals(cfg.FirstBlock(finallyRegion!));
                    ...
                    foreach (var predecessor in cfg.GetPredecessors(block))
                    {
                        ...
                        {
                            TValue oldFinallyInputState = cfgState.GetFinallyInputState(
                                exitedFinally
                            );
                            TValue finallyInputState = lattice.Meet(
                                oldFinallyInputState,
                                predecessorState
                            );
                    ...
                    {
                        TRegion correspondingTry = cfg.GetCorrespondingTry(
                            tryOrCatchOrFilterRegion!
                        );
                        Box<TValue> tryExceptionState = cfgState.GetExceptionState(
                            correspondingTry
                        );
                        ...
                        foreach (
                            TRegion previousFilter in cfg.GetPreviousFilters(
                                tryOrCatchOrFilterRegion!
                            )
                        )
                        {
                            ...
                            Box<TValue> previousFilterExceptionState = cfgState.GetExceptionState(
                                previousFilter
                            );
                            currentState = lattice.Meet(
                                currentState,
                                previousFilterExceptionState.Value
                            );
                        }
                    }
                    if (isFinallyStart)
                    ...
                    if (isFinallyBlock)
                    {
                        ...
                        foreach (var predecessor in cfg.GetPredecessors(block))
                        {
                            var isPredecessorInFinally = cfgState.TryGetExceptionFinallyState(
                                predecessor.Block,
                                out TValue predecessorFinallyState
                            );
                            Debug.Assert(isPredecessorInFinally);
                            exceptionFinallyState = lattice.Meet(
                                exceptionFinallyState.Value,
                                predecessorFinallyState
                            );
                        }
                    ...
                    if (isTryStart || isCatchOrFilterStart)
                    {
                        ...
                        if (isFinallyBlock)
                        {
                            ...
                            exceptionState!.Value = lattice.Meet(
                    ...
                    if (exceptionState?.Value.Equals(oldExceptionState!.Value) == false)
                    {
                        ...
                        while (
                            cfg.TryGetEnclosingTryOrCatchOrFilter(
                                tryOrCatchOrFilterRegion!,
                                out TRegion? enclosingTryOrCatch
                            )
                        )
                        {
                            // Filters can't contain try/catch/filters.
                            Debug.Assert(enclosingTryOrCatch.Kind != RegionKind.Filter);
                            Box<TValue> tryOrCatchExceptionState = cfgState.GetExceptionState(
                                enclosingTryOrCatch
                            );
                            tryOrCatchExceptionState.Value = lattice.Meet(
