...
namespace ILLink.Shared.TrimAnalysis
{
    ...
    internal partial struct HandleCallAction
    {
            ...
            switch (intrinsicId)
            {
                ...
                case IntrinsicId.Type_GetMember:

                    {
                        ...
                        if (calledMethod.HasParametersCount(1))
                        {
                            ...
                            && calledMethod.HasParameterOfType(1, "System.Reflection.BindingFlags")
                        )
                            bindingFlags = GetBindingFlagsFromValue(argumentValues[1]);
                        else if (
                            calledMethod.HasParametersCount(3)
                            && calledMethod.HasParameterOfType(2, "System.Reflection.BindingFlags")
                        )
                        {
                ...
                case IntrinsicId.Type_GetType:

                    {
                        ...
                        if (
                            (
                                calledMethod.HasParametersCount(3)
                                && calledMethod.HasParameterOfType(2, "System.Boolean")
                                && argumentValues[2].AsConstInt() != 0
                            )
                            || (
                                calledMethod.HasParametersCount(5)
                ...
                case IntrinsicId.None:
                    ...
                    if (requiresDataFlowAnalysis)
                    {
                        ...
                        for (
                            int argumentIndex = 0;
                            argumentIndex < argumentValues.Count;
                            argumentIndex++
                        )
                        {
                            if (
                                calledMethod.ParameterReferenceKind(argumentIndex)
                                == ReferenceKind.Out
                            )
                                continue;
                            _requireDynamicallyAccessedMembersAction.Invoke(
                                argumentValues[argumentIndex],
                                _annotations.GetMethodParameterValue(calledMethod, argumentIndex)
                            );
                        }
                    }
                    break;
        ...
        )
        {
            ...
            if (
                calledMethod.HasParametersCount(8)
                && calledMethod.HasParameterOfType(2, "System.Boolean")
            )
            {
            ...
            foreach (var assemblyNameValue in argumentValues[0])
            {
                ...
                else
                {
                    _diagnosticContext.AddDiagnostic(
                        DiagnosticId.UnrecognizedParameterInMethodCreateInstance,
                        calledMethod.GetParameterDisplayName(0),
                        calledMethod.GetDisplayName()
                    );
                }
            }
