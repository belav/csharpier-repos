    ...
    public sealed class UsageBasedMetadataManager : GeneratingMetadataManager
    {
        ...
        )
        {
            ...
            if (module is EcmaModule ecmaModule)
            {
                ...
                {
                    ManifestResource resource = ecmaModule.MetadataReader
                ...
                if (!IsReflectionBlocked(invokeMethod))
                {
                    ...
                    dependencies.Add(
                        factory.ReflectedMethod(
                            invokeMethod.GetCanonMethodTarget(CanonicalFormKind.Specific)
                        ),
        ...
        public override void GetDependenciesDueToEETypePresence(
            ref DependencyList dependencies,
            NodeFactory factory,
            TypeDesc type
        )
        {
            ...
            DataflowAnalyzedTypeDefinitionNode.GetDependencies(
                ref dependencies,
                factory,
                FlowAnnotations,
                type
        ...
        )
        {
            ...
            if (!IsReflectionBlocked(method))
            {
                MethodDesc canonicalMethod = method.GetCanonMethodTarget(
        ...
        )
        {
            if (!IsReflectionBlocked(target))
            {
                ...
                if (target.IsVirtual)
                    dependencies.Add(
                        factory.DelegateTargetVirtualMethod(
                            target.GetCanonMethodTarget(CanonicalFormKind.Specific)
                        ),
                        "Target of a delegate"
                    );
        ...
        )
        {
            ...
            if (scanReflection)
            {
                if (
                    methodIL != null
                    && Dataflow.ReflectionMethodBodyScanner
                ...
                {
                    if (
                        DiagnosticUtilities.TryGetRequiresAttribute(
                            method,
                            DiagnosticUtilities.RequiresUnreferencedCodeAttribute,
                            out _
                        )
        ...
        )
        {
            ...
            )
            {
                ...
                if (fieldOwningType.IsCanonicalSubtype(CanonicalFormKind.Specific))
                {
                    ...
                    {
                        fieldToReport = factory.TypeSystemContext
        ...
        )
        {
            ...
            bool baseMethodRequiresDataflow = FlowAnnotations.RequiresVirtualMethodDataflowAnalysis(
                baseMethod
            );
            bool overridingMethodRequiresDataflow =
                FlowAnnotations.RequiresVirtualMethodDataflowAnalysis(overridingMethod);
        ...
        )
        {
            ...
            bool baseMethodFulfillsRequirement = baseMethod.IsInRequiresScope(
                requiresAttributeName
            );
            bool overridingMethodFulfillsRequirement = overridingMethod.IsInRequiresScope(
                requiresAttributeName
            );
        ...
        )
        {
            ...
            if (
                FlowAnnotations.CompilerGeneratedState
            ...
            : LockFreeReaderHashtable<EcmaModule, AssemblyFeatureInfo>
        {
            ...
            public LinkAttributesHashTable(
                Logger logger,
        ...
        private sealed class AssemblyFeatureInfo
        {
            ...
            )
            {
                ...
                // XML file
                ParseLinkAttributesXml(
                    module,
                    logger,
                    featureSwitchValues,
                    globalAttributeRemoval: false
                );
                ParseLinkAttributesXml(
                    module,
                    logger,
                    featureSwitchValues,
                    globalAttributeRemoval: true
                );
            }

            public void ParseLinkAttributesXml(
                EcmaModule module,
                Logger logger,
                IReadOnlyDictionary<string, bool> featureSwitchValues,
            ...
            )
            {
                EcmaModule xmlModule = globalAttributeRemoval
                    ...
                    : module;
                PEMemoryBlock resourceDirectory = xmlModule.PEReader
                    .GetSectionData(
                        xmlModule.PEReader
                            .PEHeaders
                            .CorHeader
                ...
                {
                    ManifestResource resource = xmlModule.MetadataReader
                        .GetManifestResource(resourceHandle);
                    ...
                    if (resourceName == "ILLink.LinkAttributes.xml")
                    {
                        ...
                        RemovedAttributes.UnionWith(
                            LinkAttributesReader.GetRemovedAttributes(
                                logger,
                                xmlModule.Context,
                                ms,
                                resource,
        ...
        internal sealed class LinkAttributesReader : ProcessLinkerXmlBase
        {
            ...
            private static bool IsRemoveAttributeInstances(string attributeName) =>
                attributeName == "RemoveAttributeInstances"
                || attributeName == "RemoveAttributeInstancesAttribute";
            ...
            {
                var rdr = new LinkAttributesReader(
                    logger,
                    context,
                    documentStream,
                    resource,
