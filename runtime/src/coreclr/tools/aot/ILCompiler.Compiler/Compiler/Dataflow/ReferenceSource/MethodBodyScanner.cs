...
namespace Mono.Linker.Dataflow
{
    ...
    abstract partial class MethodBodyScanner
    {
        ...
        public void Scan(MethodBody methodBody)
        {
            ...
            Dictionary<VariableDefinition, ValueBasicBlockPair> locals = new Dictionary<
                VariableDefinition,
                ValueBasicBlockPair
            >(methodBody.Variables.Count);
                ...
                switch (operation.OpCode.Code)
                {
                    ...
                    case Code.Newarr:

                        {
                            ...
                            currentStack.Push(
                                new StackSlot(
                                    new ArrayValue(count.Value, (TypeReference)operation.Operand)
                                )
                            );
                        }
                        break;
                    ...
                    case Code.Calli:

                        {
                            ...
                            if (
                                GetReturnTypeWithoutModifiers(signature.ReturnType).MetadataType
                                != MetadataType.Void
                            )
                                PushUnknown(currentStack);
                        }
        ...
        protected abstract ValueNode GetMethodParameterValue(
        ...
        )
        {
            ...
            if (code >= Code.Ldarg_0 && code <= Code.Ldarg_3)
            {
                ...
                if (thisMethod.HasImplicitThis())
                {
                    ...
                    {
                        isByRef = thisMethod.Parameters[
                            paramNum - 1
                        ].ParameterType.IsByRefOrPointer();
                    }
                }
                else
                {
        ...
        )
        {
            ...
            bool isByRef =
                operation.OpCode.Code == Code.Ldloca
                || operation.OpCode.Code == Code.Ldloca_S
        ...
        void ScanLdtoken(Instruction operation, Stack<StackSlot> currentStack)
        {
            ...
            {
                StackSlot slot = new StackSlot(
                    new RuntimeTypeHandleForGenericParameterValue(genericParameter)
                );
                currentStack.Push(slot);
                return;
            }
        ...
        )
        {
            ...
            if (methodReturnValue != null)
                currentStack.Push(
                    new StackSlot(methodReturnValue, calledMethod.ReturnType.IsByRefOrPointer())
                );
