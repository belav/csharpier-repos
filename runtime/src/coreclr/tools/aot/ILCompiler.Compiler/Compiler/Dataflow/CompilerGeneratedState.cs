        ...
        private sealed class TypeCache
        {
            ...
            internal TypeCache(MetadataType type, Logger? logger, ILProvider ilProvider)
            {
                    ...
                    if (TryGetStateMachineType(method, out MetadataType? stateMachineType))
                    {
                        Debug.Assert(
                            stateMachineType.ContainingType == type
                                || (
                                    CompilerGeneratedNames.IsGeneratedMemberName(
                        ...
                        _generatedTypeToTypeArgumentInfo ??=
                            new Dictionary<MetadataType, TypeArgumentInfo>();
                        _generatedTypeToTypeArgumentInfo[stateMachineType] = new TypeArgumentInfo(
                            method,
                            null
                        );
                    }
                ...
                static bool HasGenericParameters(MetadataType typeDef)
                {
                    ...
                    return typeDef.Instantiation.Length
                        > typeDef.ContainingType.Instantiation.Length;
                }
                ...
                void MapGeneratedTypeTypeParameters(MetadataType generatedType)
                {
                        ...
                        if (typeArg is GenericParameterDesc { Kind: { } kind } param)
                        {
                            ...
                            else
                            {
                                ...
                                if (
                                    ...
                                    || !CompilerGeneratedNames.IsGeneratedType(owningType.Name)
                                )
                                {
                                    userAttrs = param;
                                }
                    ...
                    _generatedTypeToTypeArgumentInfo[generatedType] = typeInfo with
                    {
