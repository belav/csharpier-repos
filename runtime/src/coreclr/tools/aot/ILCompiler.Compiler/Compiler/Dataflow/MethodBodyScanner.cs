    ...
    internal abstract partial class MethodBodyScanner
    {
        ...
        public virtual void InterproceduralScan(MethodIL startingMethodBody)
        {
            ...
            while (!interproceduralState.Equals(oldInterproceduralState))
            {
                ...
                foreach (
                    var methodBodyValue in oldInterproceduralState.MethodBodies.GetKnownValues()
                )
            ...
            if (
                _annotations.CompilerGeneratedState
        ...
        )
        {
            ...
            TypeDesc paramType = param.ParameterType;

            bool isByRef = opcode == ILOpcode.ldarga || opcode == ILOpcode.ldarga_s;
            ...
            foreach (var value in target.AsEnumerable())
            {
                switch (value)
                {
                    ...
                    case FieldReferenceValue fieldReference
                        when HandleGetField(method, offset, fieldReference.FieldDefinition)
                            .AsSingleValue()
                    ...
                    case FieldValue fieldValue:
                        HandleStoreField(
                            ...
                            offset,
                            fieldValue,
                            DereferenceValue(method, offset, source, locals, ref ipState)
                        );
                        break;
        ...
        /// </remarks>
        protected abstract MultiValue HandleGetField(
            MethodIL methodBody,
            int offset,
            FieldDesc field
        );
            ...
            foreach (var array in arrayToStoreIn.Value.AsEnumerable())
            {
                if (array is ArrayValue arrValue)
                {
                    ...
                    else
                    {
                        ...
                        StoreMethodLocalValue(
                            arrValue.IndexValues,
                            ArrayValue.SanitizeArrayElementValue(valueToStore.Value),
