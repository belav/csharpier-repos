...
namespace Mono.Linker.Dataflow
{
    ...
    public class CompilerGeneratedState
    {
        ...
        readonly Dictionary<
            TypeDefinition,
            Dictionary<MethodDefinition, List<IMemberDefinition>>?
        > _cachedTypeToCompilerGeneratedMembers;
        ...
        public static bool TryGetStateMachineType(
            ...
            [NotNullWhen(true)] out TypeDefinition? stateMachineType
        )
        {
        ...
        TypeDefinition? GetCompilerGeneratedStateForType(TypeDefinition type)
        {
                    ...
                    foreach (var instruction in _context.GetMethodIL(method).Instructions)
                    {
                        switch (instruction.OpCode.OperandType)
                        {
                            case OperandType.InlineMethod:
                                ...
                                {
                                    MethodDefinition? referencedMethod = _context.TryResolve(
                            ...
                            case OperandType.InlineField:

                                {
                                    ...
                                    FieldDefinition? field = _context.TryResolve(
                                    ...
                                    if (
                                        field.DeclaringType is var generatedType
                                        &&
                    ...
                    Debug.Assert(
                        stateMachineType.DeclaringType == type
                            || CompilerGeneratedNames.IsGeneratedMemberName(
                                stateMachineType.DeclaringType.Name
                            )
            ...
            foreach (var userDefinedMethod in userDefinedMethods)
            {
                ...
                compilerGeneratedCallees ??=
                    new Dictionary<MethodDefinition, List<IMemberDefinition>>();
                compilerGeneratedCallees.Add(
                    userDefinedMethod,
                    new List<IMemberDefinition>(callees)
                );
            ...
            static bool HasGenericParameters(TypeDefinition typeDef)
            {
                ...
                return typeDef.GenericParameters.Count
                    > typeDef.DeclaringType.GenericParameters.Count;
            }
            ...
            void MapGeneratedTypeTypeParameters(TypeDefinition generatedType)
            {
                ...
                {
                    var typeArgs = new ICustomAttributeProvider[
                        ...
                        if (typeArg is GenericParameter { Owner: { } owner } param)
                        {
                            ...
                            else
                            {
                                ...
                                if (!CompilerGeneratedNames.IsGeneratedType(owningRef.Name))
                                {
                                    ...
                                    _context.TryResolve((TypeReference)param.Owner) is
                                    { } owningType
                                )
                                {
                        ...
                        if (instr.Operand is GenericInstanceMethod gim)
                        {
                            foreach (var tr in gim.GenericArguments)
                            {
                                if (
                                    ...
                                    && compilerGeneratedType == _context.TryResolve(git)
                                )
                                {
                                    return git;
                                }
        ...
        public bool TryGetCompilerGeneratedCalleesForUserMethod(
            MethodDefinition method,
            [NotNullWhen(true)] out List<IMemberDefinition>? callees
        )
        {
            callees = null;
            ...
            return _cachedTypeToCompilerGeneratedMembers[typeToCache]?.TryGetValue(
                    method,
                    out callees
                ) == true;
        }
        ...
        /// </summary>
        public IReadOnlyList<ICustomAttributeProvider>? GetGeneratedTypeAttributes(
        ...
        public bool TryGetOwningMethodForCompilerGeneratedMember(
            ...
            [NotNullWhen(true)] out MethodDefinition? owningMethod
        )
        {
            owningMethod = null;
            ...
            {
                if (
                    _compilerGeneratedMethodToUserCodeMethod.TryGetValue(
