    ...
    internal static class GenericArgumentDataFlow
    {
        ...
        )
        {
            ProcessGenericArgumentDataFlow(
                ...
                origin,
                type,
                contextMethod.OwningType.Instantiation,
        ...
        private static void ProcessGenericInstantiation(
            in DiagnosticContext diagnosticContext,
            ReflectionMarker reflectionMarker,
        ...
        )
        {
            for (int i = 0; i < instantiation.Length; i++)
            {
                ...
                if (
                    reflectionMarker.Annotations.GetGenericParameterAnnotation(genericParameter)
                    != default
                )
                {
                    ...
                    var requireDynamicallyAccessedMembersAction =
                        new RequireDynamicallyAccessedMembersAction(
                            reflectionMarker,
                            diagnosticContext,
                            genericParameter.GetDisplayName()
                        );
