...
namespace ILLink.Shared.TrimAnalysis
{
    sealed partial class FlowAnnotations
    {
        ...
        /// <returns></returns>
        public DynamicallyAccessedMemberTypes GetParameterAnnotation(
                        ...
                        if (IsTypeInterestingForDataflow(method.DeclaringType))
                        {
                            ...
                            if (methodMemberTypes != DynamicallyAccessedMemberTypes.None)
                            {
                                paramAnnotations = new DynamicallyAccessedMemberTypes[
                                    method.Parameters.Count + offset
                                ];
                    ...
                    for (int i = 0; i < method.Parameters.Count; i++)
                    {
                        ...
                        DynamicallyAccessedMemberTypes pa =
                            GetMemberTypesForDynamicallyAccessedMembersAttribute(
                                method,
                                providerIfNotMember: methodParameter
                            );
                            ...
                            if (annotation != DynamicallyAccessedMemberTypes.None)
                            {
                                if (genericParameterAnnotations == null)
                                    genericParameterAnnotations =
                                        new DynamicallyAccessedMemberTypes[
            ...
            if (type.HasProperties)
            {
                foreach (PropertyDefinition property in type.Properties)
                {
                        ...
                        else
                        {
                            ...
                            if (setMethod.Parameters.Count > 0)
                            {
                                DynamicallyAccessedMemberTypes[] paramAnnotations =
                                    new DynamicallyAccessedMemberTypes[
                                        setMethod.Parameters.Count + offset
                                    ];
                    ...
                    if (getMethod != null)
                    {
                        ...
                        {
                            annotatedMethods.Add(
                                new MethodAnnotations(getMethod, null, annotation, null)
                            );
                        }
                    }
...
#pragma warning restore CA1822
        ...
        internal partial MethodParameterValue GetMethodParameterValue(
            ...
            new(
                method.Method.Parameters[parameterIndex].ParameterType.ResolveToTypeDefinition(
                    _context
                ),
                method.Method,
                parameterIndex,
        ...
        internal partial MethodParameterValue GetMethodParameterValue(
            ...
            GetMethodParameterValue(
                method,
                parameterIndex,
                GetParameterAnnotation(method.Method, parameterIndex + (method.IsStatic() ? 0 : 1))
            );
