...
namespace Mono.Linker.Dataflow
{
    sealed class DynamicallyAccessedMembersTypeHierarchy
    {
        ...
        readonly Dictionary<
            TypeDefinition,
            (DynamicallyAccessedMemberTypes annotation, bool applied)
        > _typesInDynamicallyAccessedMembersHierarchy;
        ...
        public DynamicallyAccessedMembersTypeHierarchy(LinkContext context, MarkStep markStep)
        {
            ...
            _markStep = markStep;
            _typesInDynamicallyAccessedMembersHierarchy =
                new Dictionary<TypeDefinition, (DynamicallyAccessedMemberTypes, bool)>();
        }

        public (
            DynamicallyAccessedMemberTypes annotation,
            bool applied
        ) ProcessMarkedTypeForDynamicallyAccessedMembersHierarchy(TypeDefinition type)
        {
            // We'll use the cache also as a way to detect and avoid recursion for interfaces and annotated base types
            if (
                _typesInDynamicallyAccessedMembersHierarchy.TryGetValue(type, out var existingValue)
            )
                return existingValue;

            DynamicallyAccessedMemberTypes annotation =
                _context.Annotations.FlowAnnotations.GetTypeAnnotation(type);
            bool apply = false;
            ...
            if (type.HasInterfaces)
            {
                foreach (InterfaceImplementation iface in type.Interfaces)
                {
                    ...
                    {
                        var interfaceValue =
                            ProcessMarkedTypeForDynamicallyAccessedMembersHierarchy(interfaceType);
            ...
            bool applyOptimizeTypeHierarchyAnnotations =
                (annotation != DynamicallyAccessedMemberTypes.None)
                && !_context.IsOptimizationEnabled(
            ...
            apply |=
                applyOptimizeTypeHierarchyAnnotations
                && (
                    _context.DisableEventSourceSpecialHandling
        ...
        {
            (var annotation, var applied) = ProcessMarkedTypeForDynamicallyAccessedMembersHierarchy(
                type
            );
            ...
            while (true)
            {
                ...
                {
                    if (
                        candidate.Value.annotation == DynamicallyAccessedMemberTypes.None
                ...
                {
                    ApplyDynamicallyAccessedMembersToTypeHierarchyInner(
                        reflectionMarker,
                        candidateType
                    );
                }
        ...
        )
        {
            ...
            TypeDefinition? baseType = _context.TryResolve(type.BaseType);
            if (baseType != null)
                applied = ApplyDynamicallyAccessedMembersToTypeHierarchyInner(
                    reflectionMarker,
                    baseType
                );
        ...
        void ApplyDynamicallyAccessedMembersToType(
            in ReflectionMarker reflectionMarker,
        ...
        )
        {
            ...
            if (baseType != null)
            {
                ...
                var annotationToApplyToBase = Annotations.GetMissingMemberTypes(
                    annotation,
            ...
            if (
                annotation.HasFlag(DynamicallyAccessedMemberTypesOverlay.Interfaces)
                && type.HasInterfaces
            )
            {
                var annotationToApplyToInterfaces =
                    annotation == DynamicallyAccessedMemberTypes.All
                        ? annotation
        ...
        (DynamicallyAccessedMemberTypes annotation, bool applied) GetCachedInfoForTypeInHierarchy(
            TypeDefinition type
        )
        {
            // The type should be in our cache already
            if (
                !_typesInDynamicallyAccessedMembersHierarchy.TryGetValue(
                    type,
