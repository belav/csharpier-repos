...
namespace ILCompiler
{
    public class UserDefinedTypeDescriptor
    {
        ...
        private uint GetEnumTypeIndex(TypeDesc type)
        {
            ...
            System.Diagnostics.Debug.Assert(
                defType != null,
                "GetEnumTypeIndex was called with non def type"
            );
        ...
        TypeDesc GetDebugType(TypeDesc type)
        {
            ...
            if (
                ...
                || (typeGenericComplexityInfo is DefType defType)
                    && ShouldUseCanonicalTypeRecord(defType)
            )
            {
                ...
                if (
                    (typeGenericComplexityInfo is DefType canonDefType)
                    && ShouldUseCanonicalTypeRecord(canonDefType)
                )
                {
        ...
        private uint GetClassTypeIndex(TypeDesc type, bool needsCompleteType)
        {
            ...
            System.Diagnostics.Debug.Assert(
                defType != null,
                "GetClassTypeIndex was called with non def type"
            );
            ClassTypeDescriptor classTypeDescriptor = new ClassTypeDescriptor
            {
