...
namespace ILCompiler
{
    public class ManagedBinaryEmitter
    {
        public class EmittedTypeDefinition
        {
            ...
            internal EmittedTypeDefinition(
                string name,
                bool isValueType,
                ManagedBinaryEmitter managedEmitter
            )
            {
                Name = name;
            ...
            public EmittedMethodDefinition EmitMethodDefinition(
                string name,
        ...
        public class EmittedMethodDefinition
        {
            ...
            internal EmittedMethodDefinition(
                string name,
                MethodSignature signature,
                ManagedBinaryEmitter managedEmitter
            )
        ...
        public ManagedBinaryEmitter(TypeSystemContext typeSystemContext, string assemblyName)
        {
            ...
            _signatureEmitter = new EcmaSignatureEncoder<EntityProviderForEcmaSignature>(
                new EntityProviderForEcmaSignature(this)
            );
        }

        public EmittedTypeDefinition EmitTypeDefinition(string typeName, bool isValueType)
        {
            EmittedTypeDefinition newTypeDef = new EmittedTypeDefinition(
                typeName,
                isValueType,
                this
            );
        ...
        public EntityHandle EmitMetadataHandleForTypeSystemEntity(TypeSystemEntity entity)
        {
            switch (entity)
            {
                case FieldDesc field:
                    return MakeMemberReferenceHandle(field);
                case MethodDesc method:
                    return MakeMemberReferenceHandle(method);
                case TypeDesc type:
                    return MakeTypeRefOrSpecHandle(type);
        ...
        {
            using (
                FileStream sw = new FileStream(outputPath, FileMode.Create, FileAccess.ReadWrite)
            )
            {
                EmitToStream(sw);
            }
        ...
        public void EmitToStream(Stream stream)
        {
            foreach (var typeDef in _emittedTypes)
            {
                ...
                _metadataBuilder.AddTypeDefinition(
                    ...
                    typeDef.IsValueType
                        ? MakeTypeRefHandle(
                            _typeSystemContext.GetWellKnownType(WellKnownType.ValueType)
                        )
                        : MakeTypeRefHandle(
                            _typeSystemContext.GetWellKnownType(WellKnownType.Object)
                        ),
                    fieldList: MetadataTokens.FieldDefinitionHandle(1),
        ...
        private Dictionary<IAssemblyDesc, AssemblyReferenceHandle> _assemblyRefHandles =
            new Dictionary<IAssemblyDesc, AssemblyReferenceHandle>();
        private Dictionary<TypeDesc, EntityHandle> _typeRefOrSpecHandles =
            new Dictionary<TypeDesc, EntityHandle>();
        private Dictionary<TypeSystemEntity, EntityHandle> _memberRefOrSpecHandles =
            new Dictionary<TypeSystemEntity, EntityHandle>();
        private Dictionary<MethodSignature, BlobHandle> _methodSignatureHandles =
            new Dictionary<MethodSignature, BlobHandle>();
        private Dictionary<FieldDesc, BlobHandle> _fieldSignatureHandles =
            new Dictionary<FieldDesc, BlobHandle>();
        ...
        private BlobHandle MakeSignatureHandle(MethodSignature signature)
        {
            ...
            if (!_methodSignatureHandles.TryGetValue(signature, out handle))
            {
                ...
                _methodSignatureHandles[signature] = handle = _metadataBuilder.GetOrAddBlob(
                    metadataSignature
                );
            }
        ...
        private BlobHandle MakeSignatureHandle(TypeSystemEntity methodOrField)
        {
            ...
            else
            {
                ...
                if (!_fieldSignatureHandles.TryGetValue(field, out handle))
                {
                    ...
                    SignatureTypeEncoder fieldSigEncoder = new BlobEncoder(
                        metadataSignature
                    ).FieldSignature();
                    _signatureEmitter.EncodeTypeSignature(fieldSigEncoder, field.FieldType);

                    _fieldSignatureHandles[field] = handle = _metadataBuilder.GetOrAddBlob(
                        metadataSignature
                    );
                }
            ...
            if (!_assemblyRefHandles.TryGetValue(assemblyRef, out handle))
            {
                ...
                handle = _metadataBuilder.AddAssemblyReference(
                    ...
                    _metadataBuilder.GetOrAddBlob(
                        ImmutableArray.Create<byte>(assemblyName.GetPublicKeyToken())
                    ),
        ...
        private EntityHandle MakeTypeRefOrSpecHandle(TypeDesc type)
        {
            ...
            if (!_typeRefOrSpecHandles.TryGetValue(type, out handle))
            {
                if (!type.IsDefType || !type.IsTypeDefinition || type is RuntimeDeterminedType)
                {
                    ...
                    _signatureEmitter.EncodeTypeSignature(sigEncoder, type);
                    handle = _metadataBuilder.AddTypeSpecification(
                        _metadataBuilder.GetOrAddBlob(sigEncoder.Builder)
                    );
                }
                else
                {
        ...
        private EntityHandle MakeMemberReferenceHandle(TypeSystemEntity methodOrField)
        {
            ...
            if (!_memberRefOrSpecHandles.TryGetValue(methodOrField, out handle))
            {
                ...
                if (method != null && method.HasInstantiation && !method.IsTypicalMethodDefinition)
                {
                    ...
                    GenericTypeArgumentsEncoder argEncoder =
                    ...
                    for (int i = 0; i < method.Instantiation.Length; i++)
                    {
                        ...
                        _signatureEmitter.EncodeTypeSignature(
                            argTypeEncoder,
                            method.Instantiation[i]
                        );
                    }

                    handle = _metadataBuilder.AddMethodSpecification(
                        handle,
                        _metadataBuilder.GetOrAddBlob(methodSpecEncoder.Builder)
                    );
                }
    ...
    {
        public static void EmitLdToken(
            this InstructionEncoder code,
            TypeSystemEntity typeSystemEntity,
            ManagedBinaryEmitter emitter
        )
        {
