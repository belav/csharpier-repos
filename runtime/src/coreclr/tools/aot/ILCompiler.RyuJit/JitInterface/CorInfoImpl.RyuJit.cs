...
#if SUPPORT_JIT
using MethodCodeNode = Internal.Runtime.JitSupport.JitMethodCodeNode;using RyuJitCompilation = ILCompiler.Compilation;
#endif
        ...
        )
        {
            if (_compilation.NeedsRuntimeLookup(helperId, entity))
            {
                ...
                else
                {
                    ...
                    lookup.runtimeLookup.indirections = (ushort)(
                        genericLookup.NumberOfIndirections
                        + (genericLookup.IndirectLastOffset ? 1 : 0)
                    );
            ...
            else if (type.IsDefType)
            {
                ...
                Debug.Assert(
                    !_compilation
                        ...
                        .CanHaveReferenceThroughImportTable
                );
#endif
            }
            else
            ...
            if (field.IsStatic)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        //
                        if (
                            (flags & CORINFO_ACCESS_FLAGS.CORINFO_ACCESS_ADDRESS) != 0
                            && (fieldAccessor != CORINFO_FIELD_ACCESSOR.CORINFO_FIELD_STATIC_TLS)
                        )
                        {
                            fieldFlags |=
                                CORINFO_FIELD_FLAGS.CORINFO_FLG_FIELD_SAFESTATIC_BYREF_RETURN;
                        }
                    }
...
#pragma warning restore CA1822 // Mark members as static
            ...
            return obj switch
            {
                ...
                _
                    => throw new NotImplementedException(
                        $"Unexpected object in isObjectImmutable: {obj}"
                    )
            };
        }
        ...
        private int getArrayOrStringLength(CORINFO_OBJECT_STRUCT_* objHnd)
        {
            ...
            return obj switch
            {
                ...
                FrozenObjectNode frozenObj when frozenObj.ObjectType.IsArray
                    => frozenObj.GetArrayLength(),
                _ => -1
            };
        }
    }
