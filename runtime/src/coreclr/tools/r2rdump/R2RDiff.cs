    ...
    class R2RDiff
    {
        ...
        private void DiffMethodsForModule(int leftModuleIndex, int rightModuleIndex)
        {
            ...
            Dictionary<string, ReadyToRunMethod> leftMethods = new Dictionary<
                ...
                ReadyToRunMethod
            >(
                leftSectionMethods.Select(
                    method =>
                        new KeyValuePair<string, ReadyToRunMethod>(method.SignatureString, method)
                )
            );
        ...
        private static ReadyToRunSectionType[] s_methodSections = new ReadyToRunSectionType[]
        {
                ...
                if (match)
                {
                    ...
                    )
                    {
                        ...
                        RuntimeFunction rightRuntimeFunction =
                            commonMethod.Value.RightMethod.RuntimeFunctions[rtfIndex];
                        int leftOffset = 0;
                        int rightOffset = 0;
                        for (;;)
                        {
