...
public sealed class PerfEventSourceListener
{
    ...
    public PerfEventSourceListener(
    ...
    )
    {
        ...
        traceEventSession.Source.Dynamic.AddCallbackForProviderEvent(
            providerName,
            "Compilation/Start",
            delegate(TraceEvent traceEvent)
            {
    ...
    public void PrintPerfResults()
    {
        ...
        writer.WriteLine(
            $"Command line processing time: {_commandlineParseMsec / _totalRealRuns:F2} ms"
        );
        writer.WriteLine(
            $"Total average compilation time: {_compilationMsec / _totalRealRuns:F2} ms"
        );
        ...
        writer.WriteLine(
            $"Dependency analysis time: {_dependencyAnalysisMsec / _totalRealRuns:F2} ms"
        );
        writer.WriteLine($"Wall clock JIT time: {_jitMsec / _totalRealRuns:F2} ms");
        writer.WriteLine(
            $"Total JIT time: {_totalJitMsec / _totalRealRuns:F2} ms (sum of all threads)"
        );
