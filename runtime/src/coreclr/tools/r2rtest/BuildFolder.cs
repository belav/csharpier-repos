            ...
            if (!options.NoExe)
            {
                foreach (string script in _executionScripts ?? Enumerable.Empty<string>())
                {
                    ...
                    foreach (CompilerRunner runner in compilerRunners)
                    {
                        ...
                        modules.UnionWith(
                            _compilationInputFiles.Select(
                                file => runner.GetOutputFileName(_outputFolder, file)
                            )
                        );
                        folders.Add(Path.GetDirectoryName(script));
                        folders.UnionWith(runner.ReferenceFolders);

                        scriptExecutions[(int)runner.Index] = new ProcessInfo(
                            new ScriptExecutionProcessConstructor(
                                runner,
                                _outputFolder,
                                script,
                                modules,
