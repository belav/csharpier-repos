    ...
    internal class CommandLineOptions
    {
        ...
        private Verbosity VerbosityConverter(string s)
        {
            ...
            catch
            {
                throw new FormatException(
                    "Must be one of minimal, normal, detailed, or diagnostic"
                );
            }
        }
        ...
        void DefineArgumentSyntax(ArgumentSyntax syntax)
        {
            ...
            void CommonOptions()
            {
                ...
                Reference = DefineFileOptionList(
                    name: "r|reference",
                    help: "If a reference is not located on disk at the same location as used in the process, it may be specified with a --reference parameter. Multiple --reference parameters may be specified. The wild cards * and ? are supported by this option."
                );
            ...
            void VerbosityOption()
            {
                ...
                syntax.DefineOption(
                    name: "v|verbosity",
                    value: ref verbosity,
                    help: "Adjust verbosity level. Supported levels are minimal, normal, detailed, and diagnostic.",
                    valueConverter: VerbosityConverter,
                    requireValue: true
                );
            ...
            void CompressedOption()
            {
                ...
                syntax.DefineOption(
                    name: "compressed",
                    value: ref compressed,
                    help: "Generate compressed mibc",
                    requireValue: false
                );
            ...
            var mibcCommand = syntax.DefineCommand(
                name: "create-mibc",
                value: ref command,
                help: "Transform a trace file into a Mibc profile data file."
            );
            if (mibcCommand.IsActive)
            {
                ...
                syntax.DefineOption(
                    name: "precise-debug-info-file",
                    ref preciseDebugInfoFile,
                ...
                syntax.DefineOption(
                    name: "spgo",
                    value: ref Spgo,
                    help: "Base profile on samples in the input. Uses last branch records if available and otherwise raw IP samples.",
                    requireValue: false
                );
                syntax.DefineOption(
                    name: "spgo-min-samples",
                    value: ref SpgoMinSamples,
                    help: $"The minimum number of total samples a function must have before generating profile data for it with SPGO. Default: {SpgoMinSamples}",
                    requireValue: false
                );

                syntax.DefineOption(
                    name: "include-full-graphs",
                    value: ref IncludeFullGraphs,
                    help: "Include all blocks and edges in the written .mibc file, regardless of profile counts",
                    requireValue: false
                );
            ...
            var jittraceCommand = syntax.DefineCommand(
                name: "create-jittrace",
                value: ref command,
                help: "Transform a trace file into a jittrace runtime file."
            );
            if (jittraceCommand.IsActive)
            {
                ...
                syntax.DefineOption(
                    name: "includeReadyToRun",
                    value: ref ProcessR2REvents,
                    help: "Include ReadyToRun methods in the trace file.",
                    requireValue: false
                );
                HelpOption();
            }
            ...
            var mergeCommand = syntax.DefineCommand(
                name: "merge",
                value: ref command,
                help: "Merge multiple Mibc profile data files into one file."
            );
            if (mergeCommand.IsActive)
            {
                ...
                InputFilesToMerge = DefineFileOptionList(
                    name: "i|input",
                    help: "Input .mibc files to be merged. Multiple input arguments are specified as --input file1.mibc --input file2.mibc"
                );
                OutputOption();
                ...
                syntax.DefineOptionList(
                    ...
                    value: ref assemblyNamesAsStrings,
                    help: "If specified, include in Mibc file only references to the specified assemblies. Assemblies are specified as assembly names, not filenames. For instance, `System.Private.CoreLib` not `System.Private.CoreLib.dll`. Multiple --include-reference options may be specified.",
                    requireValue: true
                );
                if (assemblyNamesAsStrings != null)
                {
                    foreach (string asmName in assemblyNamesAsStrings)
                    {
                        ...
                        {
                            throw new FormatException(
                                $"Unable to parse '{asmName}' as an Assembly Name."
                            );
                        }
                    }
                }
                ...
                syntax.DefineOption(
                    name: "inherit-timestamp",
                    value: ref InheritTimestamp,
                    help: "If specified, set the output's timestamp to the max timestamp of the input files"
                );
            ...
            var dumpCommand = syntax.DefineCommand(
                name: "dump",
                value: ref command,
                help: "Dump the contents of a Mibc file."
            );
            if (dumpCommand.IsActive)
            {
                ...
                syntax.DefineParameter(
                    name: "input",
                    ref inputFileToDump,
                    "Name of the input mibc file to dump."
                );
            ...
            var compareMibcCommand = syntax.DefineCommand(
                name: "compare-mibc",
                value: ref command,
                help: "Compare two .mibc files"
            );
            if (compareMibcCommand.IsActive)
            {
                ...
                CompareMibc = DefineFileOptionList(
                    name: "i|input",
                    help: "The input .mibc files to be compared. Specify as --input file1.mibc --input file2.mibc"
                );
                if (CompareMibc.Count != 2)
                    Help = true;

                syntax.DefineOption(
                    name: "dump-worst-overlap-graphs",
                    value: ref DumpWorstOverlapGraphs,
                    help: "Number of graphs to dump to .dot format in dump-worst-overlap-graphs-to directory"
                );
                string dumpWorstOverlapGraphsTo = null;
                syntax.DefineOption(
                    name: "dump-worst-overlap-graphs-to",
                    value: ref dumpWorstOverlapGraphsTo,
                    help: "Number of graphs to dump to .dot format in dump-worst-overlap-graphs-to directory"
                );
