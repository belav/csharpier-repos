...
namespace Microsoft.Diagnostics.Tools.Pgo
{
    internal sealed class PgoRootCommand : RootCommand
    {
        public Option<List<string>> InputFilesToMerge { get; } =
            new(
                new[] { "--input", "-i" },
                result => Helpers.BuildPathList(result.Tokens),
                false,
        ...
        public Option<string> InputFileToDump { get; } =
            new(new[] { "--input", "-i" }, "Name of the input mibc file to dump")
            {
                IsRequired = true,
        ...
        public Option<string> DumpWorstOverlapGraphsTo { get; } =
            new(
                "--dump-worst-overlap-graphs-to",
                "Number of graphs to dump to .dot format in dump-worst-overlap-graphs-to directory"
            );
        public Option<bool> InheritTimestamp { get; } =
            new(
                "--inherit-timestamp",
                "If specified, set the output's timestamp to the max timestamp of the input files"
            );
                        ...
                        foreach (Token token in result.Tokens)
                        {
                            ...
                            catch
                            {
                                throw new FormatException(
                                    $"Unable to parse '{token.Value}' as an Assembly Name."
                                );
                            }
                        }
                    }
        ...
        private Option<bool> _isSorted { get; } = new("--sorted", "Generate sorted output.");
