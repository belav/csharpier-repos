...
namespace Internal.Runtime
{
    ...
    internal static class ThreadStatics
    {
        ...
        )
        {
            ...
            object[] storage = RuntimeImports.RhGetThreadStaticStorageForModule(
                pModuleData->ModuleIndex
            );

            // Check whether thread static storage has already been allocated for this module and type.
            if (
                (storage != null)
                && ((uint)typeTlsIndex < (uint)storage.Length)
                && (storage[typeTlsIndex] != null)
            )
            {
        ...
        internal static unsafe object GetThreadStaticBaseForTypeSlow(
            TypeManagerSlot* pModuleData,
            int typeTlsIndex
        )
        {
            ...
            storage = EnsureThreadStaticStorage(
                pModuleData->ModuleIndex,
                storage,
                requiredSize: typeTlsIndex + 1
            );
        ...
        /// </summary>
        private static object[] EnsureThreadStaticStorage(
            int moduleIndex,
            object[] existingStorage,
            int requiredSize
        )
        {
        ...
        )
        {
            ...
            else
            {
                gcDesc = Internal
                    ...
                    .RuntimeAugments
                    .TypeLoaderCallbacks
                    .GetThreadStaticGCDescForDynamicType(typeManager, typeTlsIndex);
            }
