...
namespace System.Reflection.Runtime.General
{
    internal static partial class TypeResolver
    {
        ...
        )
        {
            ...
            RuntimeTypeInfo? runtimeType = typeDefRefOrSpec.TryResolve(
                reader,
                typeContext,
                ref exception
            );
        ...
        )
        {
            ...
            if (handleType == HandleType.TypeDefinition)
                return typeDefRefOrSpec
                    .ToTypeDefinitionHandle(reader)
                    .ResolveTypeDefinition(reader);
            else if (handleType == HandleType.TypeReference)
                return typeDefRefOrSpec
                    .ToTypeReferenceHandle(reader)
                    .TryResolveTypeReference(reader, ref exception);
            else if (handleType == HandleType.TypeSpecification)
                return typeDefRefOrSpec
                    .ToTypeSpecificationHandle(reader)
                    .TryResolveTypeSignature(reader, typeContext, ref exception);
            else if (handleType == HandleType.ModifiedType)
            {
                NativeFormatModifiedType modifiedType = typeDefRefOrSpec
        ...
        //
        private static RuntimeTypeInfo? TryResolveTypeSignature(
            ...
            switch (typeHandle.HandleType)
            {
                case HandleType.ArraySignature:
                {
                    ArraySignature sig = typeHandle
                        .ToArraySignatureHandle(reader)
                        .GetArraySignature(reader);
                    int rank = sig.Rank;
                    if (rank <= 0)
                ...
                {
                    MethodTypeVariableSignature sig = typeHandle
                        .ToMethodTypeVariableSignatureHandle(reader)
                        .GetMethodTypeVariableSignature(reader);
                ...
                case HandleType.FunctionPointerSignature:
                {
                    ...
                    {
                        RuntimeTypeInfo? parameterType = paramTypeHandle.TryResolve(
                            reader,
                            typeContext,
                            ref exception
                        );
                ...
                case HandleType.TypeInstantiationSignature:
                {
                    ...
                    {
                        RuntimeTypeInfo? genericTypeArgument = genericTypeArgumentHandle.TryResolve(
                            reader,
                            typeContext,
                            ref exception
                        );
                    ...
                    return genericTypeDefinition.GetConstructedGenericTypeNoConstraintCheck(
                        genericTypeArguments.ToArray()
                    );
                }
                ...
                {
                    TypeVariableSignature sig = typeHandle
                        .ToTypeVariableSignatureHandle(reader)
                        .GetTypeVariableSignature(reader);
        ...
        private static RuntimeTypeInfo? TryResolveTypeReference(
        ...
        )
        {
            ...
            {
                RuntimeTypeInfo? outerType = parent
                    .ToTypeReferenceHandle(reader)
                    .TryResolveTypeReference(reader, ref exception);
                if (outerType == null)
                    return null;
            ...
            if (outerTypeInfo != null)
            {
                ...
                if (resolvedType == null)
                {
                    exception = Helpers.CreateTypeLoadException(
                        outerTypeInfo.FullName + "+" + name,
                        outerTypeInfo.Assembly
                    );
                    return null;
                }
            ...
            if (parentType == HandleType.NamespaceReference)
            {
                NamespaceReferenceHandle namespaceReferenceHandle =
                    parent.ToNamespaceReferenceHandle(reader);
                ...
                {
                    parentHandleToSearch = parentHandleToSearch
                        .ToNamespaceReferenceHandle(reader)
                        .GetNamespaceReference(reader)
                        .ParentScopeOrNamespace;
                }
                ScopeReferenceHandle scopeReferenceHandle =
                    parentHandleToSearch.ToScopeReferenceHandle(reader);

                RuntimeAssemblyName assemblyName = scopeReferenceHandle.ToRuntimeAssemblyName(
                    reader
                );
                RuntimeAssemblyInfo runtimeAssembly;
                exception = RuntimeAssemblyInfo.TryGetRuntimeAssembly(
                    assemblyName,
                    out runtimeAssembly
                );
                if (exception != null)
                    return null;
                RuntimeTypeInfo runtimeType = runtimeAssembly.GetTypeCore(
                    fullName,
                    throwOnError: false,
                    ignoreCase: false
                );
