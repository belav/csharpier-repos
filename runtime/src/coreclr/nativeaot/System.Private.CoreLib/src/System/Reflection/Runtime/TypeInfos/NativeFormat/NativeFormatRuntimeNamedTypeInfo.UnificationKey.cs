...
namespace System.Reflection.Runtime.TypeInfos.NativeFormat
{
            ...
            IEquatable<NativeFormatRuntimeNamedTypeInfo>
    {
        ...
        {
            //
            // Q: Why is the type handle part of the unification key when it doesn't participate in the
            // Equals/HashCode computations?
            // A: It's a passenger.
            //
            //    The typeHandle argument is "redundant" in that it can be computed from the rest of the key.
            // However, we have callers (Type.GetTypeFromHandle()) that
            //    already have the typeHandle so to avoid an unnecessary round-trip computation, we require the
            // caller to pass it in separately.
            //    We allow it to ride along in the key object because the ConcurrentUnifier classes we use don't
            // support passing "extra" parameters to
            ...
            public bool Equals(UnificationKey other)
            {
                ...
                // The TypeHandle is not actually part of the key but riding along for convenience (see comment at
                // head of class.)
