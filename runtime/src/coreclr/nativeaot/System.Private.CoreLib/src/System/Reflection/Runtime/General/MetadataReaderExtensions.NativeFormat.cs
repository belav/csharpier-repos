...
namespace System.Reflection.Runtime.General
{
    ...
    {
        public static bool StringOrNullEquals(
        ...
        public static ScopeReferenceHandle ToExpectedScopeReferenceHandle(
            this Handle handle,
            MetadataReader reader
        )
        {
        ...
        // Conversion where a invalid handle type indicates bad metadata rather a mistake by the caller.
        public static NamespaceReferenceHandle ToExpectedNamespaceReferenceHandle(
            this Handle handle,
            MetadataReader reader
        )
        {
        ...
        // Conversion where a invalid handle type indicates bad metadata rather a mistake by the caller.
        public static TypeDefinitionHandle ToExpectedTypeDefinitionHandle(
            this Handle handle,
            MetadataReader reader
        )
        {
        ...
        internal static Type[] GetCustomModifiers(
            this Handle handle,
            MetadataReader reader,
        ...
        )
        {
            ...
            do
            {
                NativeFormatModifiedType modifiedType = handle
                    .ToModifiedTypeHandle(reader)
                    .GetModifiedType(reader);
                if (optional == modifiedType.IsOptional)
                {
                    Type customModifier = modifiedType.ModifierType
        ...
        public static PropertySignature ParsePropertySignature(
            this Handle handle,
            MetadataReader reader
        )
        {
        ...
        {
            if (
                (method.Flags & (MethodAttributes.RTSpecialName | MethodAttributes.SpecialName))
                != (MethodAttributes.RTSpecialName | MethodAttributes.SpecialName)
            )
                return false;

            ConstantStringValueHandle nameHandle = method.Name;
            return nameHandle.StringEquals(ConstructorInfo.ConstructorName, reader)
                || nameHandle.StringEquals(ConstructorInfo.TypeConstructorName, reader);
        }

        private static Exception ParseBoxedEnumConstantValue(
            this ConstantBoxedEnumValueHandle handle,
            MetadataReader reader,
            out object value
        )
        {
        ...
        public static object ParseConstantNumericValue(this Handle handle, MetadataReader reader)
        {
            switch (handle.HandleType)
            {
                case HandleType.ConstantBooleanValue:
                    return handle
                        .ToConstantBooleanValueHandle(reader)
                        .GetConstantBooleanValue(reader)
                        .Value;
                case HandleType.ConstantCharValue:
                    return handle
                        .ToConstantCharValueHandle(reader)
                        .GetConstantCharValue(reader)
                        .Value;
                case HandleType.ConstantByteValue:
                    return handle
                        .ToConstantByteValueHandle(reader)
                ...
                case HandleType.ConstantSingleValue:
                    return handle
                        .ToConstantSingleValueHandle(reader)
                        .GetConstantSingleValue(reader)
                        .Value;
                case HandleType.ConstantDoubleValue:
                    return handle
                        .ToConstantDoubleValueHandle(reader)
                        .GetConstantDoubleValue(reader)
                        .Value;
                default:
                    throw new BadImageFormatException();
            }
        ...
        public static Exception TryParseConstantValue(
            this Handle handle,
            MetadataReader reader,
            out object? value
        )
        {
            ...
            switch (handleType)
            {
                ...
                case HandleType.ConstantStringValue:
                    value = handle
                        .ToConstantStringValueHandle(reader)
                ...
                {
                    return handle
                        .ToConstantBoxedEnumValueHandle(reader)
                        .ParseBoxedEnumConstantValue(reader, out value);
                }
                default:
                {
                    Exception? exception;
        ...
        private static Array TryParseConstantArray(
            this Handle handle,
            MetadataReader reader,
            ...
            switch (handleType)
            {
                ...
                case HandleType.ConstantEnumArray:
                    return TryParseConstantEnumArray(
                        handle.ToConstantEnumArrayHandle(reader),
                        reader,
                        out exception
                    );
                ...
                {
                    HandleCollection constantHandles = handle
                        .ToConstantStringArrayHandle(reader)
                        .GetConstantStringArray(reader)
                        .Value;
                ...
                {
                    HandleCollection constantHandles = handle
                        .ToConstantHandleArrayHandle(reader)
                        .GetConstantHandleArray(reader)
                        .Value;
        ...
        private static Array TryParseConstantEnumArray(
            this ConstantEnumArrayHandle handle,
            MetadataReader reader,
            out Exception? exception
        )
        {
            ...
            Type? elementType = enumArray.ElementType
                .TryResolve(reader, new TypeContext(null, null), ref exception)
                ?.ToType();
            if (exception != null)
                return null;
        ...
        public static Handle GetAttributeTypeHandle(
        ...
        //
        public static bool IsCustomAttributeOfType(
            this CustomAttributeHandle customAttributeHandle,
            MetadataReader reader,
            string ns,
            string name
        )
        {
            ...
            if (handleType == HandleType.TypeDefinition)
            {
                ...
                while (idx-- != 0)
                {
                    ...
                    NamespaceDefinition namespaceDefinition = nsHandle.GetNamespaceDefinition(
                        reader
                    );
                    if (!namespaceDefinition.Name.StringOrNullEquals(namespacePart, reader))
                        return false;
                    if (
                        !namespaceDefinition.ParentScopeOrNamespace
                            .IsNamespaceDefinitionHandle(reader)
                    )
                        return false;
                    nsHandle = namespaceDefinition.ParentScopeOrNamespace
            ...
            {
                TypeReference typeReference = typeHandle
                    .ToTypeReferenceHandle(reader)
                    .GetTypeReference(reader);
                ...
                if (!typeReference.ParentNamespaceOrType.IsNamespaceReferenceHandle(reader))
                    return false;
                NamespaceReferenceHandle nsHandle = typeReference.ParentNamespaceOrType
                ...
                while (idx-- != 0)
                {
                    ...
                    if (!namespaceReference.Name.StringOrNullEquals(namespacePart, reader))
                        return false;
                    if (
                        !namespaceReference.ParentScopeOrNamespace
                            .IsNamespaceReferenceHandle(reader)
                    )
                        return false;
                    nsHandle = namespaceReference.ParentScopeOrNamespace
                        .ToNamespaceReferenceHandle(reader);
                }
        ...
        public static string ToNamespaceName(
            this NamespaceDefinitionHandle namespaceDefinitionHandle,
            MetadataReader reader
        )
        {
            string ns = "";
            for (; ; )
            {
                NamespaceDefinition currentNamespaceDefinition =
                    namespaceDefinitionHandle.GetNamespaceDefinition(reader);
        ...
        public static IEnumerable<NamespaceDefinitionHandle> GetTransitiveNamespaces(
            this MetadataReader reader,
            IEnumerable<NamespaceDefinitionHandle> namespaceHandles
        )
        {
            foreach (NamespaceDefinitionHandle namespaceHandle in namespaceHandles)
            {
                ...
                NamespaceDefinition namespaceDefinition = namespaceHandle.GetNamespaceDefinition(
                    reader
                );
                foreach (
                    NamespaceDefinitionHandle childNamespaceHandle in GetTransitiveNamespaces(
                        reader,
                        namespaceDefinition.NamespaceDefinitions.AsEnumerable()
                    )
                )
        ...
        public static IEnumerable<TypeDefinitionHandle> GetTopLevelTypes(
            this MetadataReader reader,
            IEnumerable<NamespaceDefinitionHandle> namespaceHandles
        )
        {
        ...
        public static IEnumerable<TypeDefinitionHandle> GetTransitiveTypes(
            this MetadataReader reader,
            IEnumerable<TypeDefinitionHandle> typeDefinitionHandles,
            bool publicOnly
        )
        {
            foreach (TypeDefinitionHandle typeDefinitionHandle in typeDefinitionHandles)
            {
                ...
                if (publicOnly)
                {
                    TypeAttributes visibility =
                        typeDefinition.Flags & TypeAttributes.VisibilityMask;
                    if (
                ...
                foreach (
                    TypeDefinitionHandle nestedTypeDefinitionHandle in GetTransitiveTypes(
                        reader,
                        typeDefinition.NestedTypes.AsEnumerable(),
                        publicOnly
                    )
                )
        ...
        public static string ToFullyQualifiedTypeName(
            this NamespaceReferenceHandle namespaceReferenceHandle,
            string typeName,
            MetadataReader reader
        )
        {
            ...
            for (; ; )
            {
                ...
                namespaceReferenceHandle = namespaceReference.ParentScopeOrNamespace
        ...
        public static IEnumerable<NamespaceDefinitionHandle> AsEnumerable(
            this NamespaceDefinitionHandleCollection collection
        )
        {
        ...
        public static IEnumerable<TypeDefinitionHandle> AsEnumerable(
            this TypeDefinitionHandleCollection collection
        )
        {
