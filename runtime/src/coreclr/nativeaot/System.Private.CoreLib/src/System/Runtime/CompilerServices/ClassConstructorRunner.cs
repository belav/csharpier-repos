...
namespace System.Runtime.CompilerServices
{
    internal static partial class ClassConstructorRunner
    {
        ...
        {
            object threadStaticBase = ThreadStatics.GetThreadStaticBaseForType(
                pModuleData,
                typeTlsIndex
            );
            EnsureClassConstructorRun(context);
        ...
        )
        {
            ...
            NoisyLog(
                "EnsureClassConstructorRun, context={0}, thread={1}",
                pContext,
            ...
            {
                NoisyLog(
                    "Cctor already run, context={0}, thread={1}",
                    pContext,
                    CurrentManagedThreadId
                );
                return;
            ...
            try
            {
                ...
                if (DeadlockAwareAcquire(cctor, pContext))
                {
                    ...
                    {
                        NoisyLog(
                            "Acquired cctor lock, context={0}, thread={1}",
                            pContext,
                            currentManagedThreadId
                        );
                        ...
                        {
                            TypeInitializationException priorException = cctors[
                                cctorIndex
                            ].Exception;
                            ...
                            {
                                NoisyLog(
                                    "Calling cctor, context={0}, thread={1}",
                                    pContext,
                                    currentManagedThreadId
                                );
                                ...
                                NoisyLog(
                                    "Set type inited, context={0}, thread={1}",
                                    pContext,
                                    currentManagedThreadId
                                );
                            ...
                            {
                                TypeInitializationException wrappedException =
                                    new TypeInitializationException(
                                        null,
                                        SR.TypeInitialization_Type_NoTypeAvailable,
                                        e
                                    );
                    ...
                    finally
                    {
                        ...
                        NoisyLog(
                            "Releasing cctor lock, context={0}, thread={1}",
                            pContext,
                            currentManagedThreadId
                        );
            ...
            NoisyLog(
                "EnsureClassConstructorRun complete, context={0}, thread={1}",
                pContext,
                CurrentManagedThreadId
            );
        }
        ...
        )
        {
                    ...
                    using (s_cctorGlobalLock.EnterScope())
                    {
                        ...
                        for (; chainStepCount < Cctor.Count; chainStepCount++)
                        {
                            ...
                            if (holdingThread == currentManagedThreadId)
                            {
                                ...
                                DebugLog(
                                    "A class constructor was skipped due to class constructor cycle. context={0}, thread={1}",
                                    pContext,
                                    currentManagedThreadId
                                );
                        ...
                        {
                            NoisyLog(
                                "Mark thread blocked, context={0}, thread={1}",
                                pContext,
                                currentManagedThreadId
                            );

                            unmarkCookie = BlockingRecord.MarkThreadAsBlocked(
                                currentManagedThreadId,
                                cctor
                            );
                        }
            ...
            finally
            {
                ...
                {
                    NoisyLog(
                        "Unmark thread blocked, context={0}, thread={1}",
                        pContext,
                        currentManagedThreadId
                    );
                ...
                using (s_cctorGlobalLock.EnterScope())
                {
                    ...
                    if (resultArray == null)
                    {
                        ...
                        for (int cctorIndex = 0; cctorIndex < s_cctorArraysCount; ++cctorIndex)
                        {
                            ...
                            {
                                if (
                                    segment[i]._pContext == default(StaticClassConstructionContext*)
                                )
                                {
                                    resultArray = segment;
                                    resultIndex = i;
                        ...
                        Debug.Assert(
                            resultArray[resultIndex]._pContext
                                == default(StaticClassConstructionContext*)
                        );
            ...
            public static int MarkThreadAsBlocked(int managedThreadId, CctorHandle blockedOn)
            {
                ...
                using (s_cctorGlobalLock.EnterScope())
                {
                    ...
                    if (found == s_nextBlockingRecordIndex)
                    {
                        ...
                        s_blockingRecords[s_nextBlockingRecordIndex].ManagedThreadId =
                            managedThreadId;
        ...
        private static unsafe void NoisyLog(
            string format,
            StaticClassConstructionContext* pContext,
            int threadId
        )
        ...
        [Conditional("DEBUG")]
        private static unsafe void DebugLog(
            string format,
            StaticClassConstructionContext* pContext,
            int threadId
        )
