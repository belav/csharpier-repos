    ...
    public sealed partial class TypeLoaderEnvironment
    {
        ...
        )
        {
            if (signature1.IsNativeLayoutSignature && signature2.IsNativeLayoutSignature)
            {
                ...
                NativeFormatModuleInfo module1 = ModuleList.GetModuleInfoByHandle(
                ...
                NativeFormatModuleInfo module2 = ModuleList.GetModuleInfoByHandle(
            ...
            else if (signature1.IsNativeLayoutSignature)
            {
                ...
                MethodSignatureComparer comparer = new MethodSignatureComparer(
                    metadataReader,
                    token.AsHandle().ToMethodHandle(metadataReader)
                );
        ...
        public uint GetGenericArgumentCountFromMethodNameAndSignature(
            MethodNameAndSignature signature
        )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var ecmaHandle = (System.Reflection.Metadata.MethodDefinitionHandle)
                    ...
                    if (
                        (sigByte & (byte)System.Reflection.Metadata.SignatureAttributes.Generic)
                        == 0
                    )
                        return 0;
        ...
        public bool TryGetMethodNameAndSignatureFromNativeLayoutSignature(
            ...
            out MethodNameAndSignature nameAndSignature
        )
        {
        ...
        internal static MethodNameAndSignature GetMethodNameAndSignature(
        ...
        {
            methodNameSig = RuntimeSignature.CreateFromNativeLayoutSignature(
                moduleHandle,
                parser.Offset
            );
            ...
            // expensive).
            NativeParser sigParser = parser.GetParserFromRelativeOffset();
            methodSig = RuntimeSignature.CreateFromNativeLayoutSignature(
                moduleHandle,
                sigParser.Offset
            );
        ...
        internal static bool IsStaticMethodSignature(RuntimeSignature methodSig)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var metadataReader = ecmaModuleInfo.MetadataReader;
                    var ecmaHandle = (System.Reflection.Metadata.MethodDefinitionHandle)
                        System.Reflection.Metadata.Ecma335.MetadataTokens.Handle(methodSig.Token);
                    var method = metadataReader.GetMethodDefinition(ecmaHandle);
                    var blobHandle = method.Signature;
                    var blobReader = metadataReader.GetBlobReader(blobHandle);
                    byte sigByte = blobReader.ReadByte();
                    return (
                        (sigByte & (byte)System.Reflection.Metadata.SignatureAttributes.Instance)
                        == 0
                    );
                }
#endif
        ...
        private TypeSystem.MethodSignature TypeSystemSigFromRuntimeSignature(
        ...
        )
        {
...
#endif
            {
                ...
                var metadataUnit = ((TypeLoaderTypeSystemContext)context).ResolveMetadataUnit(
                    nativeFormatModule
                );
                var parser = new Internal.TypeSystem.NativeFormat.NativeFormatSignatureParser(
                    metadataUnit,
                    metadataReader.GetMethod(methodHandle).Signature,
                    metadataReader
                );
        ...
        internal bool GetCallingConverterDataFromMethodSignature(
            TypeSystemContext context,
        ...
        {
            if (methodSig.IsNativeLayoutSignature)
                return GetCallingConverterDataFromMethodSignature_NativeLayout(
                    context,
                    methodSig,
                    typeInstantiation,
                    methodInstantiation,
            ...
            else
            {
                ...
                return GetCallingConverterDataFromMethodSignature_MethodSignature(
                    sig,
                    typeInstantiation,
                    methodInstantiation,
                    out hasThis,
        ...
        internal bool GetCallingConverterDataFromMethodSignature_NativeLayout(
            TypeSystemContext context,
            RuntimeSignature methodSig,
            Instantiation typeInstantiation,
            Instantiation methodInstantiation,
        ...
        )
        {
            ...
            NativeFormatModuleInfo module = ModuleList.Instance.GetModuleInfoByHandle(
                new TypeManagerHandle(methodSig.ModuleHandle)
            );
            NativeReader reader = GetNativeLayoutInfoReader(methodSig);
            NativeParser parser = new NativeParser(reader, methodSig.NativeLayoutOffset);
            ...
            for (uint i = 0; i <= parameterCount; i++)
            {
                ...
                parameters[i] = GetConstructedTypeFromParserAndNativeLayoutContext(
                    ref parser,
                    nativeLayoutContext
                );
                parametersWithGenericDependentLayout[i] =
                    TypeSignatureHasVarsNeedingCallingConventionConverter(
                        ref parserCopy,
                        module,
                        context,
        ...
        )
        {
            ...
            parametersWithGenericDependentLayout[0] =
                UniversalGenericParameterLayout.IsLayoutDependentOnGenericInstantiation(
                    methodSignature.ReturnType
                );
            for (int i = 0; i < methodSignature.Length; i++)
            {
                parametersWithGenericDependentLayout[i + 1] =
                    UniversalGenericParameterLayout.IsLayoutDependentOnGenericInstantiation(
                        methodSignature[i]
                    );
            }
            ...
            parameters[0] = methodSignature.ReturnType.InstantiateSignature(
                typeInstantiation,
                methodInstantiation
            );
            for (int i = 0; i < methodSignature.Length; i++)
            {
                parameters[i + 1] = methodSignature[i].InstantiateSignature(
                    typeInstantiation,
                    methodInstantiation
                );
            }
        ...
        internal bool MethodSignatureHasVarsNeedingCallingConventionConverter(
            TypeSystemContext context,
            RuntimeSignature methodSig
        )
        {
            if (methodSig.IsNativeLayoutSignature)
                return MethodSignatureHasVarsNeedingCallingConventionConverter_NativeLayout(
                    context,
                    methodSig
                );
            else
...
#if SUPPORTS_NATIVE_METADATA_TYPE_LOADING
                var sig = TypeSystemSigFromRuntimeSignature(context, methodSig);
                return UniversalGenericParameterLayout.MethodSignatureHasVarsNeedingCallingConventionConverter(
                    sig
                );
#else
        ...
        /// StructNotDependentOnArgsForSize[GenStructDependencyOnArgsForSize[List[T]]]] = false
        /// </summary>
        private bool TypeSignatureHasVarsNeedingCallingConventionConverter(
            ref NativeParser parser,
            NativeFormatModuleInfo moduleHandle,
            TypeSystemContext context,
        ...
        )
        {
            ...
            switch (kind)
            {
                ...
                case TypeSignatureKind.Lookback:
                {
                    ...
                    return TypeSignatureHasVarsNeedingCallingConventionConverter(
                        ref lookbackParser,
                        moduleHandle,
                        context,
                        investigationLevel
                ...
                case TypeSignatureKind.Instantiation:
                {
                    ...
                    if (
                        !TryGetTypeFromSimpleTypeSignature(
                            ref parser,
                            moduleHandle,
                            out genericTypeDef
                        )
                    ...
                    if (!RuntimeAugments.IsValueType(genericTypeDef))
                    {
                        ...
                        for (uint i = 0; i < data; i++)
                            TypeSignatureHasVarsNeedingCallingConventionConverter(
                                ...
                                moduleHandle,
                                context,
                                HasVarsInvestigationLevel.Ignore
                            );
                        return false;
                    }
                    else
                    {
                        ...
                        if (
                            (result == true)
                            && (investigationLevel == HasVarsInvestigationLevel.Parameter)
                        )
                        {
                            if (
                                ...
                                Environment.FailFast(
                                    "Unable to setup calling convention converter correctly"
                                );
        ...
        private bool TryGetTypeFromSimpleTypeSignature(
            ref NativeParser parser,
            NativeFormatModuleInfo moduleHandle,
        ...
        )
        {
            ...
            {
                var lookbackParser = parser.GetLookbackParser(data);
                return TryGetTypeFromSimpleTypeSignature(
                    ref lookbackParser,
                    moduleHandle,
                    out typeHandle
                );
        ...
        private static RuntimeTypeHandle GetExternalTypeHandle(
            NativeFormatModuleInfo moduleHandle,
            uint typeIndex
        )
        {
        ...
        private bool CompareMethodSigs(
            NativeParser parser1,
            NativeFormatModuleInfo moduleHandle1,
            NativeParser parser2,
            NativeFormatModuleInfo moduleHandle2
        )
        {
            MethodCallingConvention callingConvention1 = (MethodCallingConvention)
                parser1.GetUnsigned();
            MethodCallingConvention callingConvention2 = (MethodCallingConvention)
                parser2.GetUnsigned();
            ...
            if (
                (callingConvention1 & MethodCallingConvention.Generic)
                == MethodCallingConvention.Generic
            )
            {
        ...
        private bool CompareTypeSigs(
            ref NativeParser parser1,
            NativeFormatModuleInfo moduleHandle1,
            ref NativeParser parser2,
            NativeFormatModuleInfo moduleHandle2
        )
        {
            ...
            switch (typeSignatureKind1)
            {
                ...
                case TypeSignatureKind.FunctionPointer:
                {
                    ...
                    {
                        if (
                            !CompareTypeSigs(ref parser1, moduleHandle1, ref parser2, moduleHandle2)
                        )
                            return false;
                    }
                    break;
