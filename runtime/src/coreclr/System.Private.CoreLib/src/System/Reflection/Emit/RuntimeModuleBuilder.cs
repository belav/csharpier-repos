...
namespace System.Reflection.Emit
{
    internal sealed partial class RuntimeModuleBuilder : ModuleBuilder
    {
        ...
        internal RuntimeModuleBuilder(
            RuntimeAssemblyBuilder assemblyBuilder,
        ...
        [LibraryImport(
            ...
            EntryPoint = "ModuleBuilder_GetTypeRef",
            StringMarshalling = StringMarshalling.Utf16
        )]
        private static partial int GetTypeRef(
            QCallModule module,
            string strFullName,
            QCallModule refedModule,
            int tkResolution
        );
        ...
        private int GetMemberRef(Module? refedModule, int tr, int defToken)
        {
            ...
            return GetMemberRef(
                ...
                new QCallModule(ref refedRuntimeModule),
                tr,
                defToken
            );
        }
        ...
        [LibraryImport(
            RuntimeHelpers.QCall,
            EntryPoint = "ModuleBuilder_GetMemberRefFromSignature",
            StringMarshalling = StringMarshalling.Utf16
        )]
        ...
        private int GetMemberRefOfMethodInfo(int tr, RuntimeMethodInfo method)
        {
            ...
            RuntimeModuleBuilder thisModule = this;
            int result = GetMemberRefOfMethodInfo(
                new QCallModule(ref thisModule),
                tr,
                ((IRuntimeMethodInfo)method).Value
            );
            GC.KeepAlive(method);
            return result;
        }
        ...
        )
        {
            ...
            return GetMemberRefOfFieldInfo(
                new QCallModule(ref thisModule),
                tkType,
                new QCallTypeHandle(ref declaringType),
        ...
        [LibraryImport(
            RuntimeHelpers.QCall,
            EntryPoint = "ModuleBuilder_GetArrayMethodToken",
            StringMarshalling = StringMarshalling.Utf16
        )]
        ...
        [LibraryImport(
            RuntimeHelpers.QCall,
            EntryPoint = "ModuleBuilder_GetStringConstant",
            StringMarshalling = StringMarshalling.Utf16
        )]
        ...
        public override int GetMethodMetadataToken(ConstructorInfo constructor)
        {
            ...
            else if (constructor is ConstructorOnTypeBuilderInstantiation conOnTypeBuilderInst)
            {
                ...
                mr = GetMemberRef(
                    ...
                    tr,
                    conOnTypeBuilderInst.MetadataToken
                );
            }
            else if (
                constructor is RuntimeConstructorInfo rtCon
                && !constructor.ReflectedType!.IsArray
            )
            {
        ...
        private protected override ModuleHandle GetModuleHandleImpl() =>
            new ModuleHandle(InternalModule);
        ...
        internal SignatureHelper GetMemberRefSignature(MethodBase? method, int cGenericParameters)
        {
            switch (method)
            {
                ...
                case MethodOnTypeBuilderInstantiation motbi
                    when motbi._method is RuntimeMethodBuilder methodBuilder:
                    return methodBuilder.GetMethodSignature();
                case MethodOnTypeBuilderInstantiation motbi:
                    method = motbi._method;
                    break;
                case ConstructorOnTypeBuilderInstantiation cotbi
                    when cotbi._ctor is RuntimeConstructorBuilder constructorBuilder:
                    return constructorBuilder.GetMethodSignature();
                case ConstructorOnTypeBuilderInstantiation cotbi:
                    method = cotbi._ctor;
                    break;
        ...
        )
        {
            lock (SyncRoot)
            {
                RuntimeEnumBuilder enumBuilder = new RuntimeEnumBuilder(
                    name,
                    underlyingType,
                    visibility,
                    this
        ...
        )
        {
            lock (SyncRoot)
            {
                ...
                return _globalTypeBuilder.DefinePInvokeMethod(
                    name,
                    dllName,
                    entryName,
                    attributes,
        ...
        )
        {
            ...
            return _globalTypeBuilder.DefineMethod(
                name,
                attributes,
                callingConvention,
                returnType,
        ...
        private int GetTypeTokenWorkerNoLock(Type type, bool getGenericDefinition)
        {
            ...
            if (refedModule.Equals(this))
            {
                ...
                typeBuilder = type is RuntimeEnumBuilder enumBuilder
                    ? enumBuilder.m_typeBuilder
                    : type as RuntimeTypeBuilder;
        ...
        private int GetMethodTokenNoLock(MethodInfo method, bool getGenericTypeDefinition)
        {
            ...
            if (method is MethodBuilder methBuilder)
            {
                ...
                {
                    throw new InvalidOperationException(
                        SR.InvalidOperation_CannotImportGlobalFromDifferentModule
                    );
                }

                // method is defined in a different module
                tr = getGenericTypeDefinition
                    ? GetTypeMetadataToken(method.DeclaringType)
                    : GetTypeTokenInternal(method.DeclaringType);
            ...
            else
            {
                ...
                {
                    tr = getGenericTypeDefinition
                        ? GetTypeMetadataToken(declaringType)
                        : GetTypeTokenInternal(declaringType);
        ...
        )
        {
            ...
            if (method.IsGenericMethod)
            {
                ...
                if (
                    !Equals(methodInfoUnbound.Module)
                    || (
                ...
                RuntimeModuleBuilder thisModule = this;
                tk = RuntimeTypeBuilder.DefineMethodSpec(
                    new QCallModule(ref thisModule),
                    tk,
                    sigBytes,
                    sigLength
        ...
        private int GetArrayMethodTokenNoLock(
            Type arrayClass,
            string methodName,
            CallingConventions callingConvention,
            Type? returnType,
        ...
        public override int GetSignatureMetadataToken(SignatureHelper signature)
        {
            ...
            RuntimeModuleBuilder thisModule = this;
            return RuntimeTypeBuilder.GetTokenFromSig(
                new QCallModule(ref thisModule),
                sigBytes,
                sigLength
            );
