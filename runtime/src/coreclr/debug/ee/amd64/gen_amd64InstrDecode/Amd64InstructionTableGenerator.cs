...
namespace Amd64InstructionTableGenerator
{
    ...
    {
        [GeneratedRegex(
            @"^\s*(?<address>0x[a-f0-9]+)\s[^:]*:\s*(?<encoding>[0-9a-f ]*)\t(?<prefixes>(((rex[.WRXB]*)|(rep[nez]*)|(data16)|(addr32)|(lock)|(bnd)|(\{vex\})|([cdefgs]s)) +)*)(?<mnemonic>\S+) *(?<operands>(\S[^#]*?)?)\s*(?<comment>#.*)?$",
        ...
        public Amd64InstructionSample(string disassembly_)
        {
            ...
            if (Debug.debug)
                Console.WriteLine($"new sample: {disassembly}");
            ...
            if (Debug.debug)
                Console.WriteLine($"  opCodeExt:{opCodeExt:x3}");
        }
        ...
        private static (Map, byte, EncodingFlags) parsePrefix(List<byte> encoding)
        {
            ...
            if ((encoding[operandIndex] & RexMask) == (byte)Prefixes.Rex)
            {
                ...
                if (Debug.debug)
                    Console.WriteLine($"  P:REX");

                if ((rex & RexW) != 0)
                {
                    flags |= EncodingFlags.W;
                    if (Debug.debug)
                        Console.WriteLine($"  P:REX.W");
                }
            }
            ...
            switch ((Prefixes)encoding[operandIndex])
            {
                case Prefixes.Secondary:
                    if (Debug.debug)
                        Console.WriteLine($"  P:0F");
                    switch ((Prefixes)encoding[operandIndex + 1])
                    {
                        case Prefixes.F38:
                            if (Debug.debug)
                                Console.WriteLine($"  P:38");
                            map = Map.F38;
                            if (Debug.debug)
                                Console.WriteLine($"  map: 0F 38");
                            operandIndex += 2;
                            break;
                        case Prefixes.F3A:
                            if (Debug.debug)
                                Console.WriteLine($"  P:3A");
                            map = Map.F3A;
                            if (Debug.debug)
                                Console.WriteLine($"  map: 0F 3A");
                            operandIndex += 2;
                            break;
                        default:
                            map = Map.Secondary;
                            if (Debug.debug)
                                Console.WriteLine($"  map: 0F");
                            operandIndex += 1;
                            break;
                    }
                    break;
                ...
                {
                    if (Debug.debug)
                        Console.WriteLine($"  P:VEX3");
                    switch (encoding[operandIndex + 1] & 0x1f)
                    {
                        case 0x1:
                            map = Map.Vex1;
                            if (Debug.debug)
                                Console.WriteLine($"  map: Vex1");
                            break;
                        case 0x2:
                            map = Map.Vex2;
                            if (Debug.debug)
                                Console.WriteLine($"  map: Vex2");
                            break;
                        case 0x3:
                            map = Map.Vex3;
                            if (Debug.debug)
                                Console.WriteLine($"  map: Vex3");
                            break;
                        default:
                    ...
                    if ((byte2 & Vex_ByteW) != 0)
                    {
                        flags |= EncodingFlags.W;
                        if (Debug.debug)
                            Console.WriteLine($"  VEX.W");
                    }
                    if ((byte2 & Vex_ByteL) != 0)
                    {
                        flags |= EncodingFlags.L;
                        if (Debug.debug)
                            Console.WriteLine($"  VEX.L");
                    }
                ...
                case Prefixes.Evex:
                {
                    ...
                    switch (evex_mmm)
                    {
                        case 0x1:
                            map = Map.Evex_0F;
                            if (Debug.debug)
                                Console.WriteLine($"  map: Evex_0F");
                            break;
                        case 0x2:
                            map = Map.Evex_0F38;
                            if (Debug.debug)
                                Console.WriteLine($"  map: Evex_0F38");
                            break;
                        case 0x3:
                            map = Map.Evex_0F3A;
                            if (Debug.debug)
                                Console.WriteLine($"  map: Evex_0F3A");
                            break;
                        default:
                    ...
                    if ((evex_p1 & Evex_ByteW) != 0)
                    {
                        flags |= EncodingFlags.W;
                        if (Debug.debug)
                            Console.WriteLine($"  EVEX.W");
                    }

                    byte evex_LprimeL = (byte)(
                        (evex_p2 & Evex_ByteLprimeLmask) >> Evex_ByteLprimeLshift
                    );
    ...
    internal sealed partial class Amd64InstructionTableGenerator
    {
        ...
        private void SummarizeSamples(bool reg)
        {
            if (Debug.debug)
                Console.WriteLine(
                    $"SummarizeSamples opcodeExt={samples[0].opCodeExt:x3}, useModRmReg={reg}"
                );
            ...
            foreach (var s in samples)
            {
                ...
                if (Debug.debug)
                    Console.WriteLine($"{s.disassembly} => {suffix} ({s.encodingFlags})");
            ...
            if (Debug.debug)
                Console.WriteLine($"  rules-intersection:{rules}");

            SuffixFlags sometimesSuffix = unionSuffix & ~intersectionSuffix;
            if (Debug.debug)
                Console.WriteLine($"  rules-sometimes:{sometimesSuffix}");
            ...
            switch (sometimesSuffix)
            {
                ...
                case SuffixFlags.M8B | SuffixFlags.M4B | SuffixFlags.M2B:
                    if (
                        ...
                        TestHypothesis(
                            (e) => TestLL(SuffixFlags.M2B, SuffixFlags.M4B, SuffixFlags.M8B, e),
                            sometimesSuffix,
                            map
                        )
                    )
                ...
                case SuffixFlags.M16B | SuffixFlags.M32B | SuffixFlags.M64B:
                    if (
                        TestHypothesis(
                            (e) => TestLL(SuffixFlags.M16B, SuffixFlags.M32B, SuffixFlags.M64B, e),
                            sometimesSuffix,
                            map
                        )
                    )
                ...
                case SuffixFlags.M8B | SuffixFlags.M32B | SuffixFlags.M64B:
                    if (
                        TestHypothesis(
                            (e) => TestLL(SuffixFlags.M8B, SuffixFlags.M32B, SuffixFlags.M64B, e),
                            sometimesSuffix,
                            map
                        )
                    )
                ...
                case SuffixFlags.M8B | SuffixFlags.M16B | SuffixFlags.M32B:
                    if (
                        TestHypothesis(
                            (e) => TestLL(SuffixFlags.M8B, SuffixFlags.M16B, SuffixFlags.M32B, e),
                            sometimesSuffix,
                            map
                        )
                    )
                ...
                case SuffixFlags.M4B | SuffixFlags.M8B | SuffixFlags.M16B:
                    if (
                        TestHypothesis(
                            (e) => TestLL(SuffixFlags.M4B, SuffixFlags.M8B, SuffixFlags.M16B, e),
                            sometimesSuffix,
                            map
                        )
                    )
                ...
                case SuffixFlags.M32B | SuffixFlags.M64B:
                    if (
                        TestHypothesis(
                            (e) => TestLL(SuffixFlags.None, SuffixFlags.M32B, SuffixFlags.M64B, e),
                            sometimesSuffix,
                            map
                        )
                    )
            ...
            {
                string op = reg
                    ? $"OpReg(0x{sample.opCodeExt:x3}, 0x{sample.modrm_reg})"
                    : $"Op(0x{sample.opCodeExt:x3})";
        ...
        private static bool TestHypothesis(
            ...
            SuffixFlags sometimesSuffixFlags,
            Dictionary<SuffixFlags, List<Amd64InstructionSample>> suffixMap
        )
        {
        ...
        public static SuffixFlags TestLL(
            SuffixFlags LL00,
            SuffixFlags LL01,
            SuffixFlags LL10,
            EncodingFlags g
        ...
        public static SuffixFlags Amd64W(SuffixFlags W1, SuffixFlags W0, EncodingFlags g) =>
            Test(EncodingFlags.W, W1, W0, g);

        public static SuffixFlags Amd64P(SuffixFlags t, SuffixFlags f, EncodingFlags g) =>
            Test(EncodingFlags.P, f, t, g);

        public static SuffixFlags Amd64b(SuffixFlags b1, SuffixFlags b0, EncodingFlags g) =>
            Test(EncodingFlags.b, b1, b0, g);
        ...
        public static SuffixFlags Amd64WLL(
            SuffixFlags W1LL00,
            SuffixFlags W1LL01,
            SuffixFlags W1LL10,
            SuffixFlags W0LL00,
        ...
        public static SuffixFlags Amd64bLL(
            SuffixFlags b1,
            SuffixFlags b0LL00,
            SuffixFlags b0LL01,
            SuffixFlags b0LL10,
        ...
        public static SuffixFlags Amd64bWLL(
            SuffixFlags b1W0,
            SuffixFlags b1W1,
            SuffixFlags b0LL00,
            SuffixFlags b0LL01,
        ...
        public static SuffixFlags Amd64WbLL(
            SuffixFlags W1b1,
            SuffixFlags W1b0LL00,
            SuffixFlags W1b0LL01,
            SuffixFlags W1b0LL10,
            SuffixFlags W0b1,
            SuffixFlags W0b0LL00,
            SuffixFlags W0b0LL01,
            SuffixFlags W0b0LL10,
            EncodingFlags g
        ) =>
            Amd64W(
                Amd64b(W1b1, TestLL(W1b0LL00, W1b0LL01, W1b0LL10, g), g),
                Amd64b(W0b1, TestLL(W0b0LL00, W0b0LL01, W0b0LL10, g), g),
                g
            );
        ...
        )
        {
            ...
            if (reg)
            {
                ...
                opcodes[Map.None][currentExtension | modrmReg] =
                    $"        {rule + ",", -40} // {map}:0x{opCodeExt:x3}/{modrmReg} {string.Join(",", mnemonics.OrderBy(s => s))}";
            }
            else
            {
                ...
                if (Debug.debug)
                {
                    ...
                    {
                        Console.WriteLine(
                            $"WARNING: REPLACING opcodes[{map}][{opCodeExt:x3}] = {oldstring}"
                        );
                    }
                }
                opcodes[map][opCodeExt] =
                    $"        {rule + ",", -40} // 0x{opCodeExt:x3} {string.Join(",", mnemonics.OrderBy(s => s))}";
                if (Debug.debug)
                {
                    Console.WriteLine(
                        $"add opcodes[{map}][{opCodeExt:x3}] = {opcodes[map][opCodeExt]}"
                    );
