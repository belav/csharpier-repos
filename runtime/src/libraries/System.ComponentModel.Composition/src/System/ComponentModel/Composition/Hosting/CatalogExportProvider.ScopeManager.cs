        ...
        internal sealed class ScopeManager : ExportProvider
        {
            ...
            )
            {
                ...
                foreach (CompositionScopeDefinition childCatalog in _scopeDefinition.Children)
                {
                    ...
                    )
                    {
                        ...
                        // if the rejetecion is enabled and atomic composition is present, we will actually have to do the
                        // work, if not - we just use what we have
            ...
            internal CompositionContainer CreateChildContainer(ComposablePartCatalog childCatalog)
            {
                return new CompositionContainer(
                    childCatalog,
                    _catalogExportProvider._compositionOptions,
                    _catalogExportProvider._sourceProvider!
                );
            }
            ...
            private static ImportDefinition? TranslateImport(ImportDefinition definition)
            {
                ...
                // We will always create a new child CatalogEP to satsify the request, so from the perspecitive of
                // the caller, the policy should
