...
namespace System.Transactions
{
    public static class TransactionInterop
    {
        ...
        )
        {
            ...
            OletxTransaction dTx = GetOletxTransactionFromTransmitterPropagationToken(
                propagationToken
            );
        ...
        public static IDtcTransaction GetDtcTransaction(Transaction transaction)
        {
            ...
            try
            {
                ...
                transactionComObject = (IDtcTransaction)
                    Marshal.GetObjectForIUnknown(transactionNativePtr);
                Marshal.SetComObjectData(
                    transactionComObject,
                    typeof(ITransaction),
                    transactionNative
                );
        ...
        internal static IDtcTransaction GetDtcTransaction(ITransaction transaction)
        {
            ...
            var transactionNative = (IDtcTransaction)
                Marshal.GetObjectForIUnknown(transactionNativePtr);
        ...
        )
        {
            ...
            ITransaction myTransactionNative = GetITransactionFromIDtcTransaction(
                transactionNative
            );
            ...
            if (!tooLate)
            {
                ...
                // If a transaction is found then FindOrCreate will Dispose the oletx
                // created.
                transaction = TransactionManager.FindOrCreatePromotedTransaction(
                    xactInfo.Uow,
                    oleTx
                );
            }
        ...
        internal static ITransaction GetITransactionFromIDtcTransaction(
            IDtcTransaction transactionNative
        )
        {
            if (
                Marshal.GetComObjectData(transactionNative, typeof(ITransaction))
                is not ITransaction myTransactionNative
            )
            {
                unsafe
                {
                    ...
                    if (
                        Marshal.QueryInterface(
                            unknown,
                            in Guids.IID_ITransaction_Guid,
                            out IntPtr transactionNativePtr
                        ) == 0
                    )
                    {
                        Marshal.Release(unknown);
                        myTransactionNative = ComInterfaceMarshaller<ITransaction>.ConvertToManaged(
                            (void*)transactionNativePtr
                        )!;
                        Marshal.SetComObjectData(
                            transactionNative,
                            typeof(ITransaction),
