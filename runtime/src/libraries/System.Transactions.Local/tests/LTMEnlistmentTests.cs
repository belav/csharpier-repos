...
namespace System.Transactions.Tests
{
    public class LTMEnlistmentTests : IDisposable
    {
        ...
        public void SinglePhaseDurable(
            int volatileCount,
            EnlistmentOptions volatileEnlistmentOption,
        ...
        )
        {
            ...
            try
            {
                using (TransactionScope ts = new TransactionScope())
                {
                    ...
                    if (volatileCount > 0)
                    {
                        TestSinglePhaseEnlistment[] volatiles = new TestSinglePhaseEnlistment[
                            volatileCount
                        ];
                        for (int i = 0; i < volatileCount; i++)
                        {
                            // It doesn't matter what we specify for SinglePhaseVote.
                            volatiles[i] = new TestSinglePhaseEnlistment(
                                volatilePhase1Vote,
                                SinglePhaseVote.InDoubt,
                                expectedVolatileOutcome
                            );
        ...
        public void TwoPhaseDurable(
            int volatileCount,
            EnlistmentOptions volatileEnlistmentOption,
            EnlistmentOptions durableEnlistmentOption,
            Phase1Vote volatilePhase1Vote,
            bool commit,
            EnlistmentOutcome expectedVolatileOutcome,
        ...
        )
        {
            ...
            try
            {
                using (TransactionScope ts = new TransactionScope())
                {
                    ...
                    TestEnlistment durable = new TestEnlistment(
                        Phase1Vote.Prepared,
        ...
        public void EnlistDuringPhase0(
            EnlistmentOptions enlistmentOption,
            Phase1Vote phase1Vote,
            bool expectPhase0EnlistSuccess,
            bool commit,
        ...
        )
        {
            ...
            try
            {
                using (TransactionScope ts = new TransactionScope())
                {
                    ...
                    TestEnlistment enlistment = new TestEnlistment(
                        phase1Vote,
                        expectedOutcome,
                        true,
                        expectPhase0EnlistSuccess,
                        outcomeEvent
                    );
            ...
            Assert.True(
                outcomeEvent.WaitOne(TimeSpan.FromSeconds(MaxTransactionCommitTimeoutInSeconds))
            );
            Assert.NotNull(tx);
            Assert.Equal(expectedTxStatus, tx.TransactionInformation.Status);
        }
        ...
        public void EnlistVolatile(
            int volatileCount,
            EnlistmentOptions enlistmentOption,
            Phase1Vote volatilePhase1Vote,
            Phase1Vote lastPhase1Vote,
            bool commit,
            ...
            try
            {
                using (TransactionScope ts = new TransactionScope())
                {
                    ...
                    if (volatileCount > 0)
                    {
                        ...
                        volatiles[volatileCount - 1] = new TestEnlistment(
                            lastPhase1Vote,
                            expectedEnlistmentOutcome,
                            false,
                            true,
                            outcomeEvents[volatileCount - 1]
                        );
