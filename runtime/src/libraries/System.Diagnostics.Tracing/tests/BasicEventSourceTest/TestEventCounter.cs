...
#endif
using Xunit;using System;using System.Collections.Generic;using System.Threading;
...
namespace BasicEventSourceTests
{
    public partial class TestEventCounter
    {
        ...
        [SkipOnTargetFramework(
            ...
            reason: "https://github.com/dotnet/runtime/issues/23380"
        )]
#endif
        ...
        private void Test_Write_Metric(Listener listener)
        {
            ...
            using (var logger = new MyEventSource())
            {
                ...
                tests.Add(
                    new SubTest(
                        "EventCounter: Log 1 event, explicit poll at end",
                        delegate()
                        {
                ...
                tests.Add(
                    new SubTest(
                        "EventCounter: Log 2 events, explicit poll at end",
                        delegate()
                        {
                            listener.EnableTimer(logger, 1); // Set to poll every second, but we dont actually care because the test ends before that.
                            logger.Request(5);
                ...
                tests.Add(
                    new SubTest(
                        "EventCounter: Log 3 events in two polling periods (explicit polling)",
                        delegate()
                        {
                ...
                tests.Add(
                    new SubTest(
                        "EventCounter: Log multiple events in multiple periods",
                        delegate()
                        {
                            ...
                            using (
                                var timer = new System.Threading.Timer(
                                    ...
                                    {
                                        num100msecTimerTicks++;
                                        EventTestHarness.LogWriteLine("Tick");
                                    },
                                    null,
                                    100,
                                    100
                        ...
                        delegate(List<Event> evts)
                        {
                            ...
                            EventTestHarness.LogWriteLine(
                                "Validating: Count={0} RequestSum={1:n3} TimeSum={2:n3} ",
                                evts.Count,
                                requestSum,
                                timeSum
                            );
                            ...
                            Assert.True(
                                4 <= evts.Count,
                                "We expect two metrics at the beginning trigger and two at the end trigger.  evts.Count = "
                                    + evts.Count
                            );
                            Assert.True(
                                evts.Count % 2 == 0,
                                "We expect two metrics for every trigger.  evts.Count = "
                                    + evts.Count
                            );
                            ...
                            Assert.True(
                                4 <= evts.Count,
                                $"FAILURE EventCounter Multi-event: 4 <= {evts.Count} ticks: {num100msecTimerTicks} thread: {Environment.CurrentManagedThreadId}"
                            );
                ...
                tests.Add(
                    new SubTest(
                        ...
                        delegate()
                        {
                            ...
                            var myCounter = new EventCounter(
                                "counter for a transient object",
                                logger
                            );
        ...
        private static void ValidateSingleEventCounter(
            Event evt,
            string counterName,
            int count,
            float mean,
        ...
        )
        {
            ValidateEventCounter(
                ...
                min,
                max,
                ValidateEventHeaderAndGetPayload(evt)
            );
        }
