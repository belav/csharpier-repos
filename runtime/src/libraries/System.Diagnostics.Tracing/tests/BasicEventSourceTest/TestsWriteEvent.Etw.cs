...
namespace BasicEventSourceTests
{
    partial class TestsWriteEvent
    {
        ...
        static partial void Test_WriteEvent_AddEtwTests(List<SubTest> tests, EventSourceTest logger)
        {
            ...
            tests.Add(
                new SubTest(
                    "Write/Basic/EventWithManyTypeArgs",
                    delegate()
                    {
                        logger.EventWithManyTypeArgs(
                            "Hello",
                            1,
                            2,
                            3,
            ...
            tests.Add(
                new SubTest(
                    "Write/Activity/EventWithXferWeirdArgs",
                    delegate()
                    {
                        var actid = Guid.NewGuid();
                        logger.EventWithXferWeirdArgs(
                            actid,
                            (IntPtr)128,
                            true,
                    ...
                    delegate(Event evt)
                    {
                        ...
                        Assert.Equal(
                            (long)SdtEventSources.MyLongEnum.LongVal1,
                            ((IConvertible)evt.PayloadValue(2, "le")).ToInt64(null)
                        );
                    }
                )
            );
