    ...
    public class CryptoStream : Stream, IDisposable
    {
        ...
        private async ValueTask FlushFinalBlockAsync(
            ...
            if (_stream is CryptoStream innerCryptoStream)
            {
                ...
                {
                    await innerCryptoStream.FlushFinalBlockAsync(useAsync, cancellationToken)
                        .ConfigureAwait(false);
                }
            }
            else
        ...
        private async ValueTask<int> ReadAsyncCore(
            ...
            if (blocksToProcess > 1 && _transform.CanTransformMultipleBlocks)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        tempOutputBuffer.AsSpan(0, numOutputBytes)
            ...
            while (bytesToDeliver > 0)
            {
                ...
                {
                    _outputBuffer.AsSpan(0, numOutputBytes)
                        .CopyTo(buffer.Span.Slice(currentOutputIndex));
                    CryptographicOperations.ZeroMemory(
                ...
                {
                    _outputBuffer.AsSpan(0, bytesToDeliver)
                        .CopyTo(buffer.Span.Slice(currentOutputIndex));
            ...
            {
                _outputBuffer.AsSpan(0, _outputBufferIndex)
                    .CopyTo(buffer.Span.Slice(currentOutputIndex));
                bytesToDeliver -= _outputBufferIndex;
                _outputBufferIndex = 0;
        ...
        private async ValueTask WriteAsyncCore(
            ...
            if (_inputBufferIndex > 0)
            {
                ...
                if (buffer.Length >= _inputBlockSize - _inputBufferIndex)
                {
                    ...
                    buffer.Slice(0, _inputBlockSize - _inputBufferIndex)
