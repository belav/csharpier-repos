...
namespace System.Net.NameResolution.Tests
{
    public class TelemetryTest
    {
        ...
        public static void EventSource_ExistsWithCorrectId()
        {
            ...
            Assert.Equal(
                Guid.Parse("4b326142-bfb5-5ed3-8585-7714181d14b0"),
                ...
                .Invoke(
                    async () =>
                    {
                        ...
                        EventWrittenEventArgs[] starts = events
                            .Where(e => e.EventName == "ResolutionStart")
                            .ToArray();
                        Assert.Equal(6, starts.Length);
                        Assert.All(
                            starts,
                            s => Assert.Equal(ValidHostName, Assert.Single(s.Payload).ToString())
                        );

                        EventWrittenEventArgs[] stops = events
                            .Where(e => e.EventName == "ResolutionStop")
                            .ToArray();
            ...
            RemoteExecutor
                .Invoke(
                    async () =>
                    {
                        ...
                        Assert.All(
                            starts,
                            s => Assert.Equal(InvalidHostName, Assert.Single(s.Payload).ToString())
                        );

                        EventWrittenEventArgs[] failures = events
                            .Where(e => e.EventName == "ResolutionFailed")
                            .ToArray();
            ...
            RemoteExecutor
                .Invoke(
                    async () =>
                    {
                        ...
                        Assert.Equal(
                            6,
                            starts.Count(s => Assert.Single(s.Payload).ToString() == ValidIPAddress)
                        );
