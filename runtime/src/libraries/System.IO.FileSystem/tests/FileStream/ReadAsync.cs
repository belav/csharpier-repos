...
namespace System.IO.Tests
{
    public abstract class FileStream_AsyncReads : FileSystemTest
    {
        ...
        public async Task IncompleteReadCantSetPositionBeyondEndOfFile(
            FileShare fileShare,
            FileOptions options
        )
        {
            ...
            {
                Task<int>[] reads = buffers
                    .Select(buffer => fs.ReadAsync(buffer, 0, buffer.Length))
                    .ToArray();
