...
namespace Microsoft.VisualBasic.Tests
{
    ...
    {
        private static bool IsArmOrArm64OrAlpine
        {
            get => PlatformDetection.IsAlpine || PlatformDetection.IsArmOrArm64Process;
        }
        private static bool IsNotArmNorArm64NorAlpine
        {
            get => !IsArmOrArm64OrAlpine;
        }

        // The accuracy to which we can validate some numeric test cases depends on the platform.
        private static readonly int s_precision =
            IsArmOrArm64OrAlpine ? 12
            : (PlatformDetection.IsBrowser || PlatformDetection.IsNetFramework) ? 14
            : 15;
        ...
        public void DDB_Default()
        {
            Assert.Equal(
                57.32680635388748,
                Financial.DDB(10000.0, 4350.0, 84.0, 35.0),
                s_precision - 2
            );
        }
        ...
        public void IPmt_Default()
        {
            Assert.Equal(
                -18.21339598417987,
                Financial.IPmt(0.008, 4, 12, 3000, 0),
                s_precision - 2
            );
        ...
        [InlineData(
            new double[] { -70000.0, 22000.0, 25000.0, 28000.0, 31000.0 },
            0.1,
            0.177435884421108,
            0
        )]
        [InlineData(
            new double[] { -10000.0, 6000.0, -2000.0, 7000.0, 1000.0 },
            0.1,
            0.08667204742917164,
            0
        )]
        [InlineData(
            new double[] { -30000.0, -10000.0, 25000.0, 12000.0, 15000.0 },
            0.1,
            0.10928101434575987,
            0
        )]
        public void IRR(double[] ValueArray, double Guess, double expected, int relativePrecision)
        {
            Assert.Equal(
                expected,
                Financial.IRR(ref ValueArray, Guess),
                s_precision + relativePrecision
            );
        }
        ...
        public void NPer_Default()
        {
            Assert.Equal(
                21.672774889301333,
                Financial.NPer(0.0072, -350.0, 7000.0, 0),
                s_precision - 2
            );
            Assert.Equal(
                21.672774889301333,
                Financial.NPer(0.0072, -350.0, 7000.0),
                s_precision - 2
            );
        }
        ...
        [InlineData(
            0.089,
            new double[] { -10000, 6000, -2000, 7000, 1000 },
            -41.865531602947726,
            -2
        )]
        [InlineData(
            0.011,
            new double[] { -30000, -10000, 25000, 12000, 15000 },
            10423.40257198653,
            -5
        )]
        [InlineData(
        ...
        public void NPV(double Rate, double[] ValueArray, double expected, int relativePrecision)
        {
            Assert.Equal(
                expected,
                Financial.NPV(Rate, ref ValueArray),
                s_precision + relativePrecision
            );
        }
        ...
        public void PPmt_Default()
        {
            Assert.Equal(
                -244.97648292629228,
                Financial.PPmt(0.008, 4, 12, 3000, 0),
                s_precision - 3
            );
        ...
        {
            Assert.Equal(
                0.007886438377633958,
                Financial.Rate(12, -263.0, 3000, 0),
                s_precision - 0
            );
