...
namespace Microsoft.VisualBasic.FileIO.Tests
{
    public class FileSystemTests : System.IO.FileCleanupTestBase
    {
        ...
        private static bool ManualTestsEnabled =>
        ...
        public void CombinePathTest_BaseDirectory_RelativePath()
        {
            ...
            Assert.Equal(
                FileIO.FileSystem.CombinePath(Root, "Test2"),
                System.IO.Path.Combine(Root, "Test2")
            );
        }
        ...
        public void CombinePathTest_RootDirectory_RelativePath()
        {
            ...
            Assert.Equal(
                FileIO.FileSystem.CombinePath(TestDirectory, "Test"),
                System.IO.Path.Combine(TestDirectory, "Test")
            );
        }
        ...
        {
            var FullPathToSourceDirectory = System.IO
                .Path
                .Combine(TestDirectory, "SourceDirectory");
            System.IO.Directory.CreateDirectory(FullPathToSourceDirectory);
            for (int i = 0; i < 6; i++)
            {
                CreateTestFile(
                    SourceData,
                    PathFromBase: "SourceDirectory",
                    TestFileName: $"NewFile{i}"
                );
            }
            var FullPathToTargetDirectory = System.IO
                .Path
                .Combine(TestDirectory, "TargetDirectory");
            ...
            Assert.Throws<System.IO.IOException>(() =>
                FileIO.FileSystem
                    .CopyDirectory(FullPathToSourceDirectory, FullPathToTargetDirectory)
            );
        }
        ...
        public void CopyDirectory_SourceDirectoryName_DestinationDirectoryName_OverwriteFalse()
        {
            ...
            {
                Assert.True(
                    HasExpectedData(CurrentFile, CurrentFile.EndsWith("0") ? DestData : SourceData)
                );
            }
        }
        ...
        public void CopyDirectory_SourceDirectoryName_DestinationDirectoryName_SkipFile()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                CreateTestFile(
                    SourceData,
                    PathFromBase: "SourceDirectory",
                    TestFileName: $"Select_Skip_this_file{i}"
                );
            }
            System.IO.Directory.CreateDirectory(FullPathToTargetDirectory);
            CreateTestFile(
                DestData,
                PathFromBase: "TargetDirectory",
                TestFileName: $"Select_Skip_this_file0"
            );
            FileIO.FileSystem
                .CopyDirectory(
                    FullPathToSourceDirectory,
        ...
        {
            var testFileSource = CreateTestFile(
                TestData: SourceData,
                PathFromBase: null,
                TestFileName: "Select_Skip_this_file"
            );
            var testFileDest = GetTestFilePath();
        ...
        public void CopyFile_SourceFileName_DestinationFileName_UIOptionTestOverWriteTrue()
        {
            var testFileSource = CreateTestFile(
                TestData: SourceData,
                PathFromBase: null,
                TestFileName: "Select_Replace_the_file"
            );
            var testFileDest = GetTestFilePath();
        ...
        public void CreateDirectory_LongPath()
        {
            ...
            Assert.True(
                DirectoryName.Length < 248,
                $"DirectoryName.Length at {DirectoryName.Length} is not < 248"
            );
            ...
            Assert.True(
                FullPathToTargetDirectory.Length < 260,
                $"FullPathToTargetDirectory.Length at {FullPathToTargetDirectory.Length} is not < 260"
            );
            ...
            try
            {
                var VeryLongFullPathToTargetDirectory = System.IO
                    .Path
                    .Combine(TestDirectory, new String('E', 239));
                FileIO.FileSystem.CreateDirectory(VeryLongFullPathToTargetDirectory);
                Assert.True(
                    System.IO.Directory.Exists(VeryLongFullPathToTargetDirectory),
                    $"Directory {VeryLongFullPathToTargetDirectory} does not exist"
                );
            }
            catch (System.IO.PathTooLongException)
            {
                Assert.True(
                    ...
                    "Unexpected Failure on non-Windows Platform"
                );
            }
        ...
        public void CurrentDirectorySet()
        {
            ...
            Assert.Equal(
                System.IO.Path.GetFileName(TestDirectory),
                System.IO.Path.GetFileName(FileIO.FileSystem.CurrentDirectory)
            );
        ...
        public void DeleteDirectory_Directory_DeleteAllContents()
        {
            ...
            var testFileSource = CreateTestFile(
                SourceData,
                PathFromBase: "NewDirectory",
                TestFileName: "TestFile"
            );
            Assert.True(System.IO.File.Exists(testFileSource));
            FileIO.FileSystem
                .DeleteDirectory(FullPathToNewDirectory, DeleteDirectoryOption.DeleteAllContents);
            Assert.False(System.IO.Directory.Exists(FullPathToNewDirectory));
        }
        ...
        public void DeleteDirectory_Directory_UIOption_Delete()
        {
            ...
            var testFileSource = CreateTestFile(
                SourceData,
                PathFromBase: "Select_Yes",
                TestFileName: "DoNotCare"
            );
        ...
        public void DirectoryExists_Directory()
        {
            ...
            Assert.False(
                FileIO.FileSystem
                    .DirectoryExists(System.IO.Path.Combine(TestDirectory, "NewDirectory"))
            );
        }
            ...
            for (int i = 0; i < 6; i++)
            {
                System.IO
                    ...
                    .CreateDirectory(
                        System.IO
                            .Path
                            .Combine(TestDirectory, $"GetDirectories_DirectoryNewSubDirectory{i}")
                    );
            }
            ...
            System.IO
                ...
                .CreateDirectory(
                    System.IO
                        ...
                        .Combine(
                            TestDirectory,
                            $"GetDirectories_DirectoryNewSubDirectory0",
                            $"NewSubSubDirectory"
                        )
                );
            DirectoryList = FileIO.FileSystem.GetDirectories(TestDirectory);
        ...
        {
            var DirectoryList = FileIO.FileSystem
                .GetDirectories(TestDirectory, SearchOption.SearchTopLevelOnly);
            Assert.Equal(0, DirectoryList.Count);
            for (int i = 0; i < 6; i++)
            {
                System.IO
                    ...
                    .CreateDirectory(
                        System.IO
                            ...
                            .Combine(
                                TestDirectory,
                                $"GetDirectories_Directory_SearchOptionNewSubDirectory{i}"
                            )
                    );
            }
            DirectoryList = FileIO.FileSystem
                .GetDirectories(TestDirectory, SearchOption.SearchTopLevelOnly);
            Assert.Equal(6, DirectoryList.Count);
            for (int i = 0; i < 6; i++)
            {
            ...
            System.IO
                ...
                .CreateDirectory(
                    System.IO
                        ...
                        .Combine(
                            TestDirectory,
                            $"GetDirectories_Directory_SearchOptionNewSubDirectory0",
                            $"NewSubSubDirectory"
                        )
                );
            DirectoryList = FileIO.FileSystem
            ...
            Assert.Equal(6, DirectoryList.Count);
            DirectoryList = FileIO.FileSystem
                .GetDirectories(TestDirectory, SearchOption.SearchAllSubDirectories);
            ...
            for (int i = 0; i < 6; i++)
            {
                CreatedDirectories.Add(
                    System.IO
                        .Directory
                        .CreateDirectory(
                            System.IO.Path.Combine(TestDirectory, $"NewSubDirectory00{i}")
                        )
                        .Name
                );
            }
            DirectoryList = FileIO.FileSystem
                .GetDirectories(TestDirectory, SearchOption.SearchTopLevelOnly, "*000", "*001");
            Assert.Equal(2, DirectoryList.Count);
            for (int i = 0; i < 2; i++)
            {
                var DirectoryName = System.IO.Path.Combine(TestDirectory, $"NewSubDirectory00{i}");
                Assert.True(
                    DirectoryList.Contains(DirectoryName),
                    $"{DirectoryName} Is missing from Wildcard Search"
                );
            }
            System.IO
                .Directory
                .CreateDirectory(
                    System.IO
                        .Path
                        .Combine(TestDirectory, $"NewSubDirectory000", $"NewSubSubDirectory000")
                );
            DirectoryList = FileIO.FileSystem
                .GetDirectories(TestDirectory, SearchOption.SearchTopLevelOnly, "*000");
            Assert.Equal(1, DirectoryList.Count);
            DirectoryList = FileIO.FileSystem
                .GetDirectories(TestDirectory, SearchOption.SearchAllSubDirectories, "*000");
            Assert.Equal(2, DirectoryList.Count);
        }
        ...
        public void GetDirectoryInfo_Directory()
        {
            for (int i = 0; i < 6; i++)
            {
                System.IO
                    .Directory
                    .CreateDirectory(System.IO.Path.Combine(TestDirectory, $"NewSubDirectory{i}"));
            }
            System.IO
                .Directory
                .CreateDirectory(
                    System.IO
                        .Path
                        .Combine(TestDirectory, $"NewSubDirectory0", $"NewSubSubDirectory")
                );
        ...
        public void GetDriveInfo_Drive()
        {
            ...
            Assert.Equal(
                FileIO.FileSystem.GetDriveInfo(Drives[0].Name).Name,
                new System.IO.DriveInfo(Drives[0].Name).Name
            );
        }
        ...
        public void GetFiles_Directory()
        {
            ...
            System.IO
                .Directory
                .CreateDirectory(
                    System.IO.Path.Combine(TestDirectory, "GetFiles_DirectoryNewSubDirectory")
                );
            CreateTestFile(
                SourceData,
                PathFromBase: "GetFiles_DirectoryNewSubDirectory",
        ...
        {
            var NewSubDirectoryPath = System.IO
                .Path
                .Combine(TestDirectory, "GetFiles_Directory_SearchOptionNewSubDirectory");
            System.IO.Directory.CreateDirectory(NewSubDirectoryPath);
            CreateTestFile(
                SourceData,
                PathFromBase: "GetFiles_Directory_SearchOptionNewSubDirectory",
                TestFileName: "NewFile"
            );
            var FileList = FileIO.FileSystem.GetFiles(TestDirectory);
            Assert.Equal(0, FileList.Count);
            ...
            FileList = FileIO.FileSystem
                .GetFiles(TestDirectory, SearchOption.SearchAllSubDirectories);
        ...
        public void GetFiles_Directory_SearchOption_Wildcards()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                TestFileList.Add(
                    CreateTestFile(
                        SourceData,
                        PathFromBase: null,
                        TestFileName: $"NewFile{i}{(i % 2 == 0 ? ".vb" : ".cs")}"
                    )
                );
            }
            FileList = FileIO.FileSystem
                .GetFiles(TestDirectory, SearchOption.SearchTopLevelOnly, "*.vb");
            ...
            var NewSubDirectoryPath = System.IO
                .Path
                .Combine(TestDirectory, "GetFiles_Directory_SearchOption_WildcardsNewSubDirectory");
            System.IO.Directory.CreateDirectory(NewSubDirectoryPath);
            TestFileList.Add(
                CreateTestFile(
                    SourceData,
                    PathFromBase: "GetFiles_Directory_SearchOption_WildcardsNewSubDirectory",
                    TestFileName: "NewFile.cs"
                )
            );
            FileList = FileIO.FileSystem
                .GetFiles(TestDirectory, SearchOption.SearchAllSubDirectories, "*.cs");
            Assert.True(
                FileList.Contains(TestFileList[TestFileList.Count - 1]),
        ...
        {
            Assert.Equal(
                FileIO.FileSystem.GetName(TestDirectory),
                System.IO.Path.GetFileName(TestDirectory)
            );
        }
        ...
        {
            Assert.Equal(
                FileIO.FileSystem.GetParentPath(TestDirectory),
        ...
        public void MoveDirectory_Source_DirectoryName_DestinationDirectoryName_UIOptionOverwriteFalse()
        {
            ...
            System.IO.Directory.CreateDirectory(FullPathToTargetDirectory);
            var NewFile0WithPath = CreateTestFile(
                DestData,
                PathFromBase: "TargetDirectory",
                TestFileName: "Select_Skip_this_file0"
            );
            ...
            string[] RemainingSourceFilesWithPath = System.IO
                .Directory
            ...
            string[] DestinationFilesWithPath = System.IO
                .Directory
                .GetFiles(FullPathToTargetDirectory);
        ...
        public void MoveDirectory_SourceDirectoryName_DestinationDirectoryName()
        {
            ...
            Assert.Throws<System.IO.IOException>(() =>
                FileIO.FileSystem
                    .MoveDirectory(FullPathToSourceDirectory, FullPathToTargetDirectory)
            );
        }
        ...
        {
            var SourceFileNameWithPath = CreateTestFile(
                SourceData,
        ...
        public void MoveFile_SourceFileName_DestinationFileName_OverwriteFalse()
        {
            ...
            var DestinationFileNameWithPath = System.IO.Path.Combine(TestDirectory, "NewName");
            FileIO.FileSystem
                .MoveFile(SourceFileNameWithPath, DestinationFileNameWithPath, overwrite: false);
            Assert.False(System.IO.File.Exists(SourceFileNameWithPath));
            Assert.True(System.IO.File.Exists(DestinationFileNameWithPath));
            Assert.True(HasExpectedData(DestinationFileNameWithPath, SourceData));
            SourceFileNameWithPath = DestinationFileNameWithPath;
        ...
        public void MoveFile_SourceFileName_DestinationFileName_OverwriteTrue()
        {
            ...
            var DestinationFileNameWithPath = System.IO.Path.Combine(TestDirectory, "NewName");
            FileIO.FileSystem
                .MoveFile(SourceFileNameWithPath, DestinationFileNameWithPath, overwrite: true);
            Assert.False(System.IO.File.Exists(SourceFileNameWithPath));
            Assert.True(System.IO.File.Exists(DestinationFileNameWithPath));
            Assert.True(HasExpectedData(DestinationFileNameWithPath, SourceData));
            CreateTestFile(
                DestData,
                PathFromBase: null,
                TestFileName: (new System.IO.FileInfo(SourceFileNameWithPath)).Name
            );
            FileIO.FileSystem
                .MoveFile(
                    sourceFileName: DestinationFileNameWithPath,
                    destinationFileName: SourceFileNameWithPath,
                    overwrite: true
                );
            Assert.True(System.IO.File.Exists(SourceFileNameWithPath));
        ...
        public void MoveFile_SourceFileName_DestinationFileName_UIOptionOverWriteFalse()
        {
            ...
            var DestinationFileNameWithPath = System.IO
                .Path
                .Combine(TestDirectory, "Select_Skip_this_file");
            FileIO.FileSystem
                .MoveFile(
        ...
        public void RenameFile_File_NewName()
        {
            ...
            System.IO
                .Directory
                .CreateDirectory(System.IO.Path.Combine(TestDirectory, "NewFDirectory"));
            Assert.Throws<System.IO.IOException>(() =>
        ...
        )
        {
            ...
            Assert.False(
                System.IO.File.Exists(TempFileNameWithPath),
                $"File {TempFileNameWithPath} should not exist!"
            );
