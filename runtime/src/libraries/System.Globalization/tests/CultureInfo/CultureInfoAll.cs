...
namespace System.Globalization.Tests
{
    public class CultureInfoAll
    {
        ...
        {
            Assert.True(
                EnumSystemLocalesEx(EnumLocales, LOCALE_WINDOWS, IntPtr.Zero, IntPtr.Zero),
                "EnumSystemLocalesEx has failed"
            );
        ...
        private void Validate(CultureInfo ci)
        {
            ...
            Assert.Contains(
                GetLocaleInfo(ci, LOCALE_SPARENT),
                new[] { "zh-Hans", "zh-Hant", ci.Parent.Name },
        ...
        private void ValidateDTFI(CultureInfo ci)
        {
            ...
            Assert.Equal(
                GetMonthNames(ci, calId, CAL_SABBREVMONTHNAME1),
                dtfi.AbbreviatedMonthNames
            );
            Assert.Equal(
                GetMonthNames(ci, calId, CAL_SMONTHNAME1 | LOCALE_RETURN_GENITIVE_NAMES),
                dtfi.MonthGenitiveNames
            );
            Assert.Equal(
                GetMonthNames(ci, calId, CAL_SABBREVMONTHNAME1 | LOCALE_RETURN_GENITIVE_NAMES),
                dtfi.AbbreviatedMonthGenitiveNames
            );
            Assert.Equal(GetDayNames(ci, calId, CAL_SSHORTESTDAYNAME1), dtfi.ShortestDayNames);
            Assert.Equal(
                GetLocaleInfo(ci, LOCALE_S1159),
                dtfi.AMDesignator,
                StringComparer.OrdinalIgnoreCase
            );
            Assert.Equal(
                GetLocaleInfo(ci, LOCALE_S2359),
                dtfi.PMDesignator,
                StringComparer.OrdinalIgnoreCase
            );
            ...
            Assert.Equal(
                (int)dtfi.FirstDayOfWeek,
                ConvertFirstDayOfWeekMonToSun(GetLocaleInfoAsInt(ci, LOCALE_IFIRSTDAYOFWEEK))
            );
            Assert.Equal(
                (int)dtfi.CalendarWeekRule,
                GetLocaleInfoAsInt(ci, LOCALE_IFIRSTWEEKOFYEAR)
            );
            ...
            Assert.Contains(
                ...
                new[] { fullDateTimePattern, fullDateTimePattern1 }
            );
        ...
        private void ValidateNFI(CultureInfo ci)
        {
            ...
            Assert.Equal(
                ConvertWin32GroupString(GetLocaleInfo(ci, LOCALE_SMONGROUPING)),
            ...
            Assert.Equal(
                ConvertWin32GroupString(GetLocaleInfo(ci, LOCALE_SGROUPING)),
                nfi.NumberGroupSizes
            );
            Assert.Equal(
                ConvertWin32GroupString(GetLocaleInfo(ci, LOCALE_SGROUPING)),
                nfi.PercentGroupSizes
            );

            Assert.Equal(
                GetLocaleInfoAsInt(ci, LOCALE_INEGATIVEPERCENT),
                nfi.PercentNegativePattern
            );
            Assert.Equal(
                GetLocaleInfoAsInt(ci, LOCALE_IPOSITIVEPERCENT),
        ...
        private void ValidateRegionInfo(CultureInfo ci)
        {
            ...
            Assert.Equal(
                GetLocaleInfo(ci, LOCALE_SNATIVECOUNTRYNAME),
                ri.NativeName,
                StringComparer.OrdinalIgnoreCase
            );
        }
        ...
        private string[] GetTimeFormats(CultureInfo ci, uint flags)
        {
            ...
            Assert.True(
                EnumTimeFormatsEx(EnumTimeFormats, ci.Name, flags, IntPtr.Zero),
                string.Format("EnumTimeFormatsEx failed with culture {0} and flags {1}", ci, flags)
            );
        ...
        private int GetCalendarId(Calendar cal)
        {
            ...
            else
            {
                throw new KeyNotFoundException(
                    string.Format("Got a calendar {0} which we cannot map its Id", cal)
                );
            }
        ...
        private int[] GetOptionalCalendars(CultureInfo ci)
        {
            ...
            Assert.True(
                ...
                "EnumCalendarInfoExEx has been failed."
            );
        ...
        private string[] GetCalendarInfo(CultureInfo ci, int calId, uint calType)
        {
            ...
            Assert.True(
                ...
                "EnumCalendarInfoExEx has been failed in GetCalendarInfo."
            );
        ...
        private int GetLocaleInfoAsInt(CultureInfo ci, uint lcType)
        {
            int data = 0;
            Assert.True(
                GetLocaleInfoEx(ci.Name, lcType | LOCALE_RETURN_NUMBER, ref data, sizeof(int)) > 0,
                string.Format("GetLocaleInfoEx failed with culture {0} and lcType {1}.", ci, lcType)
            );

            return data;
        }

        internal delegate bool EnumLocalesProcEx(
            [MarshalAs(UnmanagedType.LPWStr)] string name,
            uint dwFlags,
            IntPtr param
        );
        internal delegate bool EnumCalendarInfoProcExEx(
            [MarshalAs(UnmanagedType.LPWStr)] string lpCalendarInfoString,
            int Calendar,
            string lpReserved,
            IntPtr lParam
        );
        internal delegate bool EnumTimeFormatsProcEx(
            [MarshalAs(UnmanagedType.LPWStr)] string lpTimeFormatString,
            IntPtr lParam
        );
        ...
        [DllImport("kernel32.dll", CharSet = CharSet.Unicode)]
        internal static extern bool EnumSystemLocalesEx(
        ...
        [DllImport("kernel32.dll", CharSet = CharSet.Unicode)]
        internal static extern int GetCalendarInfoEx(
            string lpLocaleName,
            int Calendar,
            IntPtr lpReserved,
            uint CalType,
        ...
        [DllImport("kernel32.dll", CharSet = CharSet.Unicode)]
        internal static extern bool EnumCalendarInfoExEx(
            EnumCalendarInfoProcExEx pCalInfoEnumProcExEx,
            string lpLocaleName,
            uint Calendar,
            string lpReserved,
            uint CalType,
        ...
        [DllImport("kernel32.dll", CharSet = CharSet.Unicode)]
        internal static extern bool EnumTimeFormatsEx(
            EnumTimeFormatsProcEx lpTimeFmtEnumProcEx,
            string lpLocaleName,
            uint dwFlags,
            IntPtr lParam
        );
        ...
        )
        {
            if (!expectToThrowOnBrowser || PlatformDetection.IsNotBrowser)
            {
                ...
                Assert.True(
                    ci.UseUserOverride,
                    "UseUserOverride for lcid created culture expected to be true"
                );
                Assert.False(
                    ci.IsReadOnly,
                    "IsReadOnly for lcid created culture expected to be false"
                );
                ...
                if (ci.ThreeLetterWindowsLanguageName != "ZZZ")
                {
                    Assert.True(
                        (threeLetterWindowsLanguageName == ci.ThreeLetterWindowsLanguageName)
                            || (
                                threeLetterWindowsLanguageName == "CHT"
                                && ci.ThreeLetterWindowsLanguageName == "ZHH"
                            )
                    );
                }
                ...
                Assert.True(
                    ci.UseUserOverride,
                    "UseUserOverride for named created culture expected to be true"
                );
                Assert.False(
                    ci.IsReadOnly,
                    "IsReadOnly for named created culture expected to be false"
                );
                ...
                Assert.False(
                    ci.UseUserOverride,
                    "UseUserOverride with false user override culture expected to be false"
                );
                Assert.False(
                    ci.IsReadOnly,
                    "IsReadOnly with false user override culture expected to be false"
                );
                ...
                Assert.False(
                    ci.UseUserOverride,
                    "UseUserOverride with Culture created by GetCultureInfo and lcid expected to be false"
                );
                Assert.True(
                    ci.IsReadOnly,
                    "IsReadOnly with Culture created by GetCultureInfo and lcid expected to be true"
                );
                ...
                Assert.False(
                    ci.UseUserOverride,
                    "UseUserOverride with Culture created by GetCultureInfo and name expected to be false"
                );
                Assert.True(
                    ci.IsReadOnly,
                    "IsReadOnly with Culture created by GetCultureInfo and name expected to be true"
                );
                ...
                Assert.False(
                    ci.UseUserOverride,
                    "UseUserOverride with Culture created by GetCultureInfo and sort name expected to be false"
                );
                Assert.True(
                    ci.IsReadOnly,
                    "IsReadOnly with Culture created by GetCultureInfo and sort name expected to be true"
                );
                ...
                Assert.True(
                    lcid == ci.KeyboardLayoutId || (ushort)lcid == (ushort)ci.KeyboardLayoutId
                );
        ...
        )
        {
            if (!expectToThrowOnBrowser || PlatformDetection.IsNotBrowser)
            {
                ...
                found = CultureInfo.GetCultures(CultureTypes.NeutralCultures)
                    .Any(
                        c =>
                ...
                if (!found)
                {
                    found = CultureInfo.GetCultures(CultureTypes.SpecificCultures)
                        .Any(
                            c =>
                                cultureNames.Contains(c.Name, StringComparer.OrdinalIgnoreCase)
                                || c.Name.Equals(
                ...
                Assert.True(
                    found,
                    $"Expected to find the culture {cultureNames[0]} in the enumerated list"
                );
            }
            else
            {
