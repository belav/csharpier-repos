...
namespace System.Text.RegularExpressions.Tests
{
    public class RegexMatchTests
    {
        public static IEnumerable<object[]> Match_MemberData()
        {
            ...
            static IEnumerable<(string Pattern, string Input, RegexOptions Options, int Beginning, int Length, bool ExpectedSuccess, string ExpectedValue)> Cases(
                RegexEngine engine
            )
            {
                ...
                foreach (
                    RegexOptions options in new[] { RegexOptions.None, RegexOptions.IgnoreCase }
                )
                {
                    string Case(string s) =>
                        (options & RegexOptions.IgnoreCase) != 0 ? s.ToUpper() : s;
                ...
                foreach (
                    RegexOptions lineOption in new[]
                    {
                        RegexOptions.None,
                ...
                {
                    if (
                        engine != RegexEngine.NonBacktracking || options != RegexOptions.RightToLeft
                    )
                    {
                        yield return (
                            "\u05D0\u05D1\u05D2\u05D3(\u05D4\u05D5|\u05D6\u05D7|\u05D8)",
            ...
            RemoteExecutor
                .Invoke(
                    async engineString =>
                    {
                        ...
                        // Repeaters
                        Assert.False(
                            (await RegexHelpers.GetRegexAsync(engine, @"a{2147483647,}")).IsMatch(
                                "a"
                            )
                        );
                        Assert.False(
                            (await RegexHelpers.GetRegexAsync(engine, @"a{50,}")).IsMatch("a")
                        ); // cutoff for Boyer-Moore prefix in debug
                        Assert.False(
                            (await RegexHelpers.GetRegexAsync(engine, @"a{51,}")).IsMatch("a")
                        );
                        Assert.False(
                            (await RegexHelpers.GetRegexAsync(engine, @"a{50_000,}")).IsMatch("a")
                        ); // cutoff for Boyer-Moore prefix in release
                        Assert.False(
                            (await RegexHelpers.GetRegexAsync(engine, @"a{50_001,}")).IsMatch("a")
                        );
        ...
        public void IsMatch_Invalid()
        {
            ...
            AssertExtensions.Throws<ArgumentNullException>(
                "pattern",
                () => Regex.IsMatch("input", null)
            );
            AssertExtensions.Throws<ArgumentNullException>(
                "pattern",
                () => Regex.IsMatch("input", null, RegexOptions.None)
            );
            AssertExtensions.Throws<ArgumentNullException>(
                "pattern",
                () => Regex.IsMatch("input", null, RegexOptions.None, TimeSpan.FromSeconds(1))
            );
        ...
        public async Task StressTestAntimirovMode(
            string pattern,
            string input_suffix,
