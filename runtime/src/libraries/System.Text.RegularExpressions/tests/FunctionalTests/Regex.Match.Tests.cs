...
namespace System.Text.RegularExpressions.Tests
{
    public partial class RegexMatchTests
    {
        ...
        private static IEnumerable<(string Pattern, string Input, RegexOptions Options, int Beginning, int Length, bool ExpectedSuccess, string ExpectedValue)> Match_MemberData_Cases(
            RegexEngine engine
        )
        {
            ...
            foreach (RegexOptions options in new[] { RegexOptions.None, RegexOptions.IgnoreCase })
            {
                ...
                yield return (
                    ...
                    true,
                    "hellohi"
                ); // allow backtracking and it succeeds
                yield return (
                    Case(@"a[^wyz]*w"),
                    "abczw",
            ...
            foreach (
                RegexOptions lineOption in new[] { RegexOptions.None, RegexOptions.Singleline }
            )
            {
            ...
            // Anchors
            foreach (
                RegexOptions anchorOptions in new[] { RegexOptions.None, RegexOptions.Multiline }
            )
            {
            ...
            yield return (
                "aaa(?imnsx-imnsx:match this)bbb",
                "AaAmatcH thisBBb",
                RegexOptions.IgnoreCase,
                0,
                16,
            ...
            yield return (
                "aAa(?#ignore this completely)bbb",
                "aAabbb",
                RegexOptions.None,
                0,
                6,
            ...
            yield return (
                "[^a-z0-9]etag|[^a-z0-9]digest",
                "this string has .digest as a substring",
                RegexOptions.None,
                16,
                7,
                true,
            ...
            yield return (
                @".*\dFoo",
                "This1foo should 2Foo match",
                RegexOptions.None,
                0,
                26,
                true,
                "This1foo should 2Foo"
            );
            yield return (
                @".*\dFoo",
                "This1foo shouldn't 2foo match",
                RegexOptions.None,
                0,
                29,
                false,
            ...
            yield return (
                @".*\dfoo",
                "1fooThis1foo should 1foo match",
                RegexOptions.None,
                4,
                9,
                true,
            ...
            if (!RegexHelpers.IsNonBacktracking(engine))
            {
                ...
                yield return (
                    @".*\nfoo",
                    "This shouldn't match",
                    RegexOptions.None | RegexOptions.RightToLeft,
                    0,
                    20,
                    false,
                    ""
                ...
                yield return (
                    @".*\dfoo",
                    "1fooThis2foo should 1foo match",
                    RegexOptions.None | RegexOptions.RightToLeft,
                    8,
                    4,
                    true,
            ...
            // Long inputs
            string longCharacterRange = string.Concat(
                Enumerable.Range(1, 0x2000).Select(c => (char)c)
            );
            foreach (RegexOptions options in new[] { RegexOptions.None, RegexOptions.IgnoreCase })
            {
        ...
        public async Task AllEnginesProduceTheSameCaptures()
        {
            ...
            foreach (
                (string Pattern, string Input, RegexOptions Options, int Beginning, int Length, bool ExpectedSuccess, string ExpectedValue) test in interpreterTests
            )
            {
                Regex interpreterRegex = await RegexHelpers.GetRegexAsync(
                    RegexEngine.Interpreter,
                    test.Pattern,
                    test.Options
                );
                if (interpreterRegex.GetGroupNumbers().Length > 1)
                {
                    Match expectedMatch = interpreterRegex.Match(
                        test.Input,
                        test.Beginning,
                        test.Length
                    );
                    ...
                    foreach (
                        (RegexEngine, HashSet<(string Pattern, string Input, RegexOptions Options, int Beginning, int Length, bool ExpectedSuccess, string ExpectedValue)>) other in otherTests
                    )
                    {
                        if (other.Item2.Contains(test))
                        {
                            Regex otherRegex = await RegexHelpers.GetRegexAsync(
                                other.Item1,
                                test.Pattern,
                                test.Options
                            );
                            Assert.Equal(
                                interpreterRegex.GetGroupNames(),
                                otherRegex.GetGroupNames()
                            );
                            Assert.Equal(
                                interpreterRegex.GetGroupNumbers(),
                                otherRegex.GetGroupNumbers()
                            );

                            Match otherMatch = otherRegex.Match(
                                test.Input,
                                test.Beginning,
                                test.Length
                            );
                            ...
                            {
                                Assert.Equal(
                                    expectedMatch.Groups[i].Value,
                                    otherMatch.Groups[i].Value
                                );

                                // NonBacktracking currently only stores the last capture in a group.
                                if (
                                    !RegexHelpers.IsNonBacktracking(other.Item1)
                                ...
                                )
                                {
                                    ...
                                    {
                                        Assert.Equal(
                                            expectedMatch.Groups[i].Captures[j].Value,
                                            otherMatch.Groups[i].Captures[j].Value
                                        );
                                    }
                                }
                            }
                    ...
                    case RegexEngine.NonBacktracking:
                        VerifyMatch(
                            Regex.Match(
                                input,
                                pattern,
                                options | RegexHelpers.OptionsFromEngine(engine)
                            )
                        );
                        VerifyIsMatch(
                            null,
        ...
        private async Task CreateAndMatch(
            RegexEngine engine,
            string pattern,
            string input,
            RegexOptions options,
        ...
        [SkipOnTargetFramework(
            ...
            "Takes several minutes on .NET Framework"
        )]
        [Theory]
        [MemberData(nameof(Match_VaryingLengthStrings_MemberData))]
        public async Task Match_VaryingLengthStrings(
            RegexEngine engine,
            RegexOptions options,
            int length
        )
        ...
        {
            const string Start = @"((?>abc|(?:def[ghi]",
                End = @")))";
            const string Match = "defg";

            string pattern =
                string.Concat(Enumerable.Repeat(Start, count))
                + string.Concat(Enumerable.Repeat(End, count));
        ...
        private async Task Match_TestThatTimeoutHappens(RegexEngine engine)
        {
            ...
            Regex re = await RegexHelpers.GetRegexAsync(
                engine,
                @"a.{20}^",
                RegexOptions.None,
        ...
        [SkipOnCoreClr(
            "https://github.com/dotnet/runtime/issues/67886",
            ~RuntimeConfiguration.Release
        )]
        [SkipOnCoreClr(
            "https://github.com/dotnet/runtime/issues/67886",
            RuntimeTestModes.JitMinOpts
        )]
        public void Match_InstanceMethods_DefaultTimeout_Throws(RegexEngine engine)
        {
            ...
            RemoteExecutor
                .Invoke(
                    ...
                    ((int)engine).ToString(CultureInfo.InvariantCulture)
                )
                .Dispose();
        }
        ...
        public void Match_CachedPattern_NewTimeoutApplies(RegexOptions options)
        {
            ...
            VerifyIsMatchThrows<RegexMatchTimeoutException>(
                null,
        ...
        public static IEnumerable<object[]> Match_Advanced_TestData()
        {
            foreach (RegexEngine engine in RegexHelpers.AvailableEngines)
            {
                ...
                if (!RegexHelpers.IsNonBacktracking(engine))
                {
                    ...
                    {
                        engine,
                        @"(?<MyDigits>\d+)abc(?(MyDigits)222|111)",
                        "111abc222",
                        RegexOptions.None,
                        0,
                        9,
        ...
        public async Task Match_Advanced(
            RegexEngine engine,
            string pattern,
            string input,
            RegexOptions options,
            ...
            void VerifyMatch(Match match)
            {
                ...
                for (int i = 0; i < match.Groups.Count; i++)
                {
                    ...
                    if (!RegexHelpers.IsNonBacktracking(engine))
                    {
                        ...
                        {
                            RegexAssert.Equal(
                                expected[i].Captures[j].Value,
                                match.Groups[i].Captures[j]
                            );
                            Assert.Equal(
                                expected[i].Captures[j].Index,
                                match.Groups[i].Captures[j].Index
                            );
                            Assert.Equal(
                                expected[i].Captures[j].Length,
                                match.Groups[i].Captures[j].Length
                            );
                        }
                    }
                    else
                    {
                        ...
                        RegexAssert.Equal(
                            expected[i].Captures[lastExpected].Value,
                            match.Groups[i].Captures[0]
                        );
                        Assert.Equal(
                            expected[i].Captures[lastExpected].Index,
                            match.Groups[i].Captures[0].Index
                        );
                        Assert.Equal(
                            expected[i].Captures[lastExpected].Length,
                            match.Groups[i].Captures[0].Length
                        );
                    }
                }
            }
        ...
        public async Task Match_StartatDiffersFromBeginning(
            RegexEngine engine,
            string pattern,
            string input,
            RegexOptions options,
        ...
        )
        {
            ...
            Assert.Equal(
                expectedSuccessBeginning,
                r.Match(input, startat, input.Length - startat).Success
            );
        }

        [Theory]
        [InlineData(
            @"(?<1>\d{1,2})/(?<2>\d{1,2})/(?<3>\d{2,4})\s(?<time>\S+)",
            "08/10/99 16:00",
            "${time}",
            "16:00"
        )]
        ...
        [SkipOnTargetFramework(
            TargetFrameworkMonikers.NetFramework,
            ".NET Framework does not have fix for https://github.com/dotnet/runtime/issues/24749"
        )]
        [SkipOnCoreClr(
            "Long running tests: https://github.com/dotnet/runtime/issues/10680",
            ~RuntimeConfiguration.Release
        )]
        [SkipOnCoreClr(
            "Long running tests: https://github.com/dotnet/runtime/issues/10680",
            ...
            RemoteExecutor
                .Invoke(
                    async engineString =>
                    {
                        ...
                        // Repeaters
                        VerifyIsMatch(
                            (await RegexHelpers.GetRegexAsync(engine, @"a{2147483647,}")),
                            "a",
                            false,
                            Regex.InfiniteMatchTimeout
                        );
                        VerifyIsMatch(
                            (await RegexHelpers.GetRegexAsync(engine, @"a{50,}")),
                            "a",
                            false,
                            Regex.InfiniteMatchTimeout
                        );
                        VerifyIsMatch(
                            (await RegexHelpers.GetRegexAsync(engine, @"a{50_000,}")),
                            "a",
                            false,
                            Regex.InfiniteMatchTimeout
                        ); // cutoff for Boyer-Moore prefix in release
                        ...
                        foreach (int length in new[] { 50, 50_000, char.MaxValue + 1 })
                        {
                            ...
                            if (!RegexHelpers.IsNonBacktracking(engine) || length < 50_000)
                            {
                                ...
                                VerifyIsMatch(
                                    (await RegexHelpers.GetRegexAsync(engine, @$"a{{{length}}}")),
                                    s,
                                    true,
                                    Regex.InfiniteMatchTimeout
                                );
        ...
        public void IsMatch_Invalid()
        {
            ...
            // Pattern is null
            VerifyIsMatchThrows<ArgumentNullException>(
                null,
                "input",
                Regex.InfiniteMatchTimeout,
                pattern: null
        ...
        public async Task IsMatch_SucceedQuicklyDueToLoopReduction(
            RegexEngine engine,
            string pattern,
            string input,
            bool expected
        ...
        {
            Regex r1 = await RegexHelpers.GetRegexAsync(
                engine,
                "[\u012F-\u0130]",
                RegexOptions.IgnoreCase
            );
            Regex r2 = await RegexHelpers.GetRegexAsync(
                engine,
                "[\u012F\u0130]",
                RegexOptions.IgnoreCase
            );
        ...
        {
            string fullpattern = string.Concat(
                string.Concat(
                    Enumerable
                        .Repeat($"({pattern}", pattern_repetition)
                        .Concat(Enumerable.Repeat(")", pattern_repetition))
                ),
                anchor
            );
        ...
        public async Task StressTestDeepNestingOfLoops(
            RegexEngine engine,
            string begin,
            string inner,
            string end,
        ...
        {
            string fullpattern =
                string.Concat(Enumerable.Repeat(begin, pattern_repetition))
                + inner
                + string.Concat(Enumerable.Repeat(end, pattern_repetition));
            string fullinput = string.Concat(Enumerable.Repeat(input, input_repetition));
        ...
        public async Task StressTestNfaMode(
            string pattern,
            string input_suffix,
        ...
        public static IEnumerable<object[]> AllMatches_TestData()
        {
            foreach (RegexEngine engine in RegexHelpers.AvailableEngines)
            {
                ...
                {
                    engine,
                    @"\p{Lu}\p{Ll}\p{Lt}\p{Lm}\p{Lo}\p{Mn}\p{Mc}\p{Me}\p{Nd}\p{Nl}",
                    RegexOptions.None,
                    "=====Aa\u01C5\u02B0\u01BB\u0300\u0903\u04880\u16EE===",
                    new (int, int, string)[]
                    {
                ...
                string pattern_WL = new String(
                    Array.ConvertAll(pattern_orig.ToCharArray(), c => (char)((int)c + 0xFF00 - 32))
                );
        ...
        public async Task AllMatches(
            RegexEngine engine,
            string pattern,
            RegexOptions options,
            string input,
            (int, int, string)[] matches
        )
        {
        ...
        private static void VerifyIsMatchThrows<T>(
            Regex? r,
            string input,
            TimeSpan timeout,
            string? pattern = null,
        ...
        private static void VerifyIsMatch(
            Regex? r,
            string input,
            bool expected,
            TimeSpan timeout,
        ...
        public async Task Match_DisjunctionOverCounting(
            RegexEngine engine,
            string disjunct1,
            string disjunct2,
            string input,
        ...
        public async Task MatchAmbiguousRegexes(
            RegexEngine engine,
            string pattern,
            string input,
            (int, int) expected_match
        )
        {
            Regex r = await RegexHelpers.GetRegexAsync(engine, pattern);
        ...
        public async Task UseRegexConcurrently_ThreadSafe_Success(
            RegexEngine engine,
            TimeSpan timeout
        )
        {
        ...
        public async Task MatchWordsInAnchoredRegexes(
            RegexEngine engine,
            RegexOptions options,
            string pattern,
            string input,
