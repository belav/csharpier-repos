...
namespace System.Text.RegularExpressions.Tests
{
    public partial class RegexParserTests
    {
        ...
        [InlineData(
            @"(?P<abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_>abc",
            RegexOptions.None,
            RegexParseError.InvalidGroupingConstruct,
            3
        )] // Nim: not an error
        ...
        [InlineData(
            ...
            RegexParseError.InsufficientOrInvalidHexDigits,
            3
        )] // Nim: not an error (supports Unicode beyond basic multilingual plane)
        [InlineData(
            @"\x{FFFFFFFF}",
            RegexOptions.None,
            RegexParseError.InsufficientOrInvalidHexDigits,
...
#pragma warning restore RE0001 // Regex issue: Not enough )'s
        ...
        )
        {
            ...
            catch (RegexParseException e)
            {
                ...
                throw new XunitException(
                    $"Expected RegexParseException with error {error} offset {offset} -> Actual error: {regexParseError} offset {e.Offset})"
                );
            }
            catch (Exception e)
            {
                throw new XunitException(
                    $"Expected RegexParseException for pattern '{pattern}' -> Actual: ({e})"
                );
            }
        ...
        static partial void MayThrow(Action action)
        {
            ...
            {
                throw new XunitException(
                    $"Expected RegexParseException or no exception -> Actual: ({e})"
                );
            }
        }
    }
