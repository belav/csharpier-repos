...
namespace System.Text.RegularExpressions.Tests
{
    public static class RegexGeneratorHelper
    {
        ...
        )
        {
            ...
            )
            {
                throw new ArgumentException(
                    string.Join(
                        Environment.NewLine,
                        resultsDiagnostics.Concat(generatorDiagnostics)
                    )
                        + Environment.NewLine
                        + string.Join(
                            Environment.NewLine,
                            generatorResults.GeneratedTrees.Select(t => t.ToString())
                        )
                );
            }
        ...
        )
        {
            ...
            string generatedSource = string.Concat(
                generatorResults.GeneratedTrees.Select(t => t.ToString())
            );
        ...
        )
        {
            Regex[] results = await SourceGenRegexAsync(
                    new[] { (pattern, culture, options, matchTimeout) },
                    cancellationToken
                )
                .ConfigureAwait(false);
            return results[0];
        ...
        )
        {
            ...
            foreach (var regex in regexes)
            {
                ...
                code.Append(
                    $"    [GeneratedRegex({SymbolDisplay.FormatLiteral(regex.pattern, quote: true)}"
                );
                ...
                {
                    code.Append(
                        $", {SymbolDisplay.FormatLiteral(regex.culture.Name, quote: true)}"
                    );
                }
