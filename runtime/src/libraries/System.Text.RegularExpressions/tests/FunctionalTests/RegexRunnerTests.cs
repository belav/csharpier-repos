...
namespace System.Text.RegularExpressions.Tests
{
    public class RegexRunnerTests
    {
        ...
        public async Task EnginesThrowNotImplementedForGoAndFFC(RegexEngine engine)
        {
            ...
            MethodInfo goMethod = typeof(RegexRunner).GetMethod(
                "Go",
                BindingFlags.Instance | BindingFlags.NonPublic
            );
            MethodInfo ffcMethod = typeof(RegexRunner).GetMethod(
                "FindFirstChar",
                BindingFlags.Instance | BindingFlags.NonPublic
            );
            ...
            Assert.Equal(
                typeof(NotImplementedException),
                ffcInvocationException.InnerException.GetType()
            );
        }
        ...
        public async Task EnsureRunmatchValueIsNulledAfterIsMatch(RegexEngine engine)
        {
            ...
            FieldInfo runnerField = typeof(Regex).GetField(
                "_runner",
                BindingFlags.Instance | BindingFlags.NonPublic
            );
            RegexRunner runner = runnerField.GetValue(re) as RegexRunner;
            FieldInfo runmatchField = typeof(RegexRunner).GetField(
                "runmatch",
                BindingFlags.Instance | BindingFlags.NonPublic
            );
            ...
            MethodInfo getTextMethod = typeof(Match).GetMethod(
                "get_Text",
                BindingFlags.Instance | BindingFlags.NonPublic
            );
