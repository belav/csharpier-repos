...
namespace System.Text.RegularExpressions.Symbolic
{
    ...
    internal static class SymbolicRegexThresholds
    {
        ...
        /// <remarks>
        /// By default, all matching starts out using DFAs, where every state transitions to one and only
        // one
        /// state for any minterm (each character maps to one minterm).  Some regular expressions, however,
        // can result
        /// in really, really large DFA state graphs, much too big to actually store.  Instead of failing
        // when we
        /// encounter such state graphs, at some point we instead switch from processing as a DFA to
        // processing as
        /// an NFA.  As an NFA, we instead track all of the states we're in at any given point, and
        // transitioning
        /// from one "state" to the next really means for every constituent state that composes our current
        // "state",
        /// we find all possible states that transitioning out of each of them could result in, and the
        // union of
        /// all of those is our new "state".  This constant represents the size of the graph after which we
        // start
        /// processing as an NFA instead of as a DFA.  This processing doesn't change immediately, however.
        // All
        /// processing starts out in DFA mode, even if we've previously triggered NFA mode for the same
        // regex.
        /// We switch over into NFA mode the first time a given traversal (match operation) results in us
        // needing
        ...
        ///<summary>The environment variable name for a value overriding the default value <see
        // cref="DefaultSymbolicRegexSafeSizeThreshold"/></summary>
