    ...
    internal sealed class RegexCode
    {
        ...
        internal static string OpcodeDescription(int offset, int[] codes, string[] strings)
        {
            ...
            switch (opcode)
            {
                ...
                case Notonelazy:
                    sb.Append(Indent())
                        .Append('\'')
                        .Append(RegexCharClass.CharDescription((char)codes[offset + 1]))
                        .Append('\'');
                    break;
                ...
                case Setlazy:
                    sb.Append(Indent())
                        .Append(RegexCharClass.SetDescription(strings[codes[offset + 1]]));
                    break;
        ...
        public override string ToString()
        {
            ...
            if (LeadingCharClasses != null)
            {
                ...
                {
                    sb.AppendLine(
                        $"{i}: {RegexCharClass.SetDescription(LeadingCharClasses[i].CharClass)}"
                    );
                }
                sb.AppendLine();
            }
