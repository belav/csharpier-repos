...
namespace System.Text.RegularExpressions
{
    ...
    internal static class RegexPrefixAnalyzer
    {
        ...
        public static List<(char[]? Chars, string Set, int Distance)>? FindFixedDistanceSets(
            RegexNode root,
            bool thorough
        )
        {
            ...
            static bool TryFindFixedSets(
                RegexNode node,
                List<(char[]? Chars, string Set, int Distance)> results,
                ref int distance,
                bool thorough
            )
            {
                ...
                switch (node.Kind)
                {
                    ...
                    case RegexNodeKind.Alternate when thorough:
                    {
                        ...
                        for (int i = 0; i < childCount; i++)
                        {
                            ...
                            foreach (
                                (char[]? Chars, string Set, int Distance) fixedSet in localResults
                            )
                            {
                                if (
                        ...
                        )
                        {
                            ...
                            {
                                results.Add(
                                    (null, pair.Value.Set.ToStringClass(), pair.Key + distance)
                                );
                            }
                        }
        ...
        /// </summary>
        public static (RegexNode LoopNode, (char Char, string? String, char[]? Chars) Literal)? FindLiteralFollowingLeadingLoop(
            RegexNode node
        )
        {
            if ((node.Options & RegexOptions.RightToLeft) != 0)
            ...
            {
                case RegexNodeKind.One
                    when !RegexCharClass.CharInClass(nextChild.Ch, firstChild.Str!):
                    return (firstChild, (nextChild.Ch, null, null));

                case RegexNodeKind.Multi
                    when !RegexCharClass.CharInClass(nextChild.Str![0], firstChild.Str!):
