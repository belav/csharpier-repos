...
namespace System.Text.RegularExpressions
{
    ...
    internal sealed class RegexInterpreter : RegexRunner
    {
            ...
            else
            {
                while (c-- != 0)
                {
                    ...
                    if (backreferenceChar != currentChar)
                    {
                        ...
                        Debug.Assert(
                            _culture != null,
                            "If the pattern has backreferences and is IgnoreCase, then _culture must not be null."
                        );
                        if (
        ...
        protected internal override void Scan(ReadOnlySpan<char> text)
        {
            ...
            while (
                _code.FindOptimizations.TryFindNextStartingPosition(
                    text,
                    ref runtextpos,
                    runtextstart
                )
            ...
            while (true)
            {
                ...
                switch (_operator)
                {
                    ...
                    case RegexOpcode.Eol:
                    {
                        ...
                        if (
                            (uint)runtextpos < (uint)inputSpan.Length
                            && inputSpan[runtextpos] != '\n'
                        )
                        {
                            break;
                        }
                            ...
                            if (IsMatched(capnum))
                            {
                                if (
                                    !MatchRef(
                                        ...
                                        MatchLength(capnum),
                                        inputSpan,
                                        (_operator & RegexOpcode.CaseInsensitive) != 0
                                    )
                                )
                                {
                                    break;
