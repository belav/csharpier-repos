...
namespace System.Text.RegularExpressions
{
    ...
    public abstract class RegexRunner
    {
        ...
        /// <summary>Used by a <see cref="Regex"/> object to scan the input <paramref name="text"/> looking
        // for the next match.</summary>
        /// <remarks>This API supports the product infrastructure and is not intended to be used directly
        // from your code.</remarks>
        /// <param name="text">The text to scan for a pattern match.</param>
        /// <exception cref="NotSupportedException">
        /// <see cref="ReadOnlySpan{T}"/>-based <see cref="Regex"/> methods are not supported from <see
        // cref="Regex"/>-derived types
        ...
        protected internal virtual void Scan(ReadOnlySpan<char> text)
        {
            ...
            // implementations.  The only time this should end up being used is if someone is using a
            // Regex-derived
            // type created by .NET Framework's Regex.CompileToAssembly, in which case it will have overridden
            // FindFirstChar and Go but not Scan (which didn't exist yet).  This isn't an officially supported
            // configuration,
            ...
            // We can assume that the passed in 'text' span is a slice of the original text input runtext. That
            // said we need to calculate
            // what the original beginning was and can't do it by just using the lengths of text and runtext,
            // since we can't guarantee that
            // the passed in beginning and length match the size of the original input. We instead use
            // MemoryExtensions Overlaps to find the
            // offset in memory between them. We intentionally use s.Overlaps(text) since we want to get a
            // positive value.
            s.AsSpan().Overlaps(text, out int beginning);

            // The passed in span is sliced from runtextbeg to runtextend already, but in the precompiled
            // scenario
            // we require to use the complete input and to use the full string instead. We first test to ensure
            // that the
            // passed in span matches the original input by using the original runtextbeg. If that is not the
            // case,
            // then it means the user is calling the new span-based APIs using CompiledToAssembly, so we throw
            // NSE
            // so as to prevent a lot of unexpected allocations.
            if (s == null || text != s.AsSpan(beginning, text.Length))
            {
                // If we landed here then we are dealing with a CompiledToAssembly case where the new Span overloads
                // are being called.
        ...
        /// <summary>
        /// This method's body is only kept since it is a protected member that could be called by someone
        // outside
        /// the assembly.
        /// </summary>
        [Obsolete(
        ...
        )
        {
            ...
            // We set runtext before calling InitializeForScan so that runmatch object is initialized with the
            // text
            runtext = text;
