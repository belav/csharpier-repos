...
namespace System.Text.RegularExpressions.Symbolic
{
        ...
        : IEnumerable<(S, SymbolicRegexNode<S>?, SymbolicRegexNode<S>)> where S : notnull
    {
        ...
        private TransitionRegex(
            SymbolicRegexBuilder<S> builder,
            TransitionRegexKind kind,
            S? test,
            TransitionRegex<S>? first,
            TransitionRegex<S>? second,
            SymbolicRegexNode<S>? node
        )
        {
            Debug.Assert(builder is not null);
            Debug.Assert(
                kind is TransitionRegexKind.Leaf
                    && node is not null
                    && Equals(test, default(S))
                    && first is null
                    && second is null
                    || kind is TransitionRegexKind.Conditional
                        && test is not null
                        && first is not null
                        && second is not null
                        && node is null
                    || kind is TransitionRegexKind.Union
                        && Equals(test, default(S))
                        && first is not null
                        && second is not null
                        && node is null
                    || kind is TransitionRegexKind.Lookaround
                        && Equals(test, default(S))
                        && first is not null
                        && second is not null
                        && node is not null
        ...
        private static TransitionRegex<S> Create(
            SymbolicRegexBuilder<S> builder,
            TransitionRegexKind kind,
            S? test,
            TransitionRegex<S>? one,
            TransitionRegex<S>? two,
            SymbolicRegexNode<S>? node
        )
        {
            // Keep transition regexes internalized using the builder
            (TransitionRegexKind, S?, TransitionRegex<S>?, TransitionRegex<S>?, SymbolicRegexNode<S>?) key =
                (kind, test, one, two, node);
            ...
            switch (_kind)
            {
                case TransitionRegexKind.Leaf:
                    ...
                    return Create(
                        ...
                        null,
                        null,
                        _node._builder.MkConcat(_node, node)
                    );
                default:
        ...
        private TransitionRegex<S> IntersectWith(TransitionRegex<S> that, S pathIn)
        {
            ...
            if (_kind == TransitionRegexKind.Union)
            {
                Debug.Assert(_first is not null && _second is not null);
                return Union(
                    _first.IntersectWith(that, pathIn),
                    _second.IntersectWith(that, pathIn)
                );
            }
            ...
            {
                Debug.Assert(
                    that._node is not null && that._first is not null && that._second is not null
                );
                return Lookaround(
                    that._node,
                    that._first.IntersectWith(this, pathIn),
                    that._second.IntersectWith(this, pathIn)
                );
            }
            #endregion
            ...
            Debug.Assert(
                _kind is TransitionRegexKind.Leaf
                    && that._kind is TransitionRegexKind.Leaf
                    && _node is not null
                    && that._node is not null
            );
        ...
        private static TransitionRegex<S> Union(TransitionRegex<S> one, TransitionRegex<S> two)
        {
            ...
            if (
                one._kind == TransitionRegexKind.Conditional
                && two._kind == TransitionRegexKind.Conditional
            )
            {
                Debug.Assert(
                    one._test is not null && one._first is not null && one._second is not null
                );
                Debug.Assert(
                    two._test is not null && two._first is not null && two._second is not null
                );
                ...
                if (one._test.Equals(two._test))
                {
                    return Conditional(
                        one._test,
                        Union(one._first, two._first),
                        Union(one._second, two._second)
                    );
                }
                ...
                {
                    return Conditional(
                        one._builder._solver.Or(one._test, two._test),
                        one._first,
                        one._second
                    );
                }
        ...
        public static TransitionRegex<S> Conditional(
            S test,
            TransitionRegex<S> thencase,
            TransitionRegex<S> elsecase
        ) =>
            (thencase == elsecase || thencase._builder._solver.True.Equals(test))
                ? thencase
        ...
        public IEnumerable<(S, SymbolicRegexNode<S>?, SymbolicRegexNode<S>)> EnumeratePaths(
            S pathCondition
        )
        {
            switch (_kind)
            {
                ...
                case TransitionRegexKind.Conditional:
                    Debug.Assert(_test is not null && _first is not null && _second is not null);
                    foreach (
                        var path in _first.EnumeratePaths(
                            _builder._solver.And(pathCondition, _test)
                        )
                    )
                    {
                        yield return path;
                    ...
                    foreach (
                        var path in _second.EnumeratePaths(
                            _builder._solver.And(pathCondition, _builder._solver.Not(_test))
                        )
                    )
                    {
                        yield return path;
                ...
                default:
                    ...
                    foreach (var path in _first.EnumeratePaths(pathCondition))
                    {
                        SymbolicRegexNode<S> nullabilityTest = path.Item2 is null
                            ? _node
                            : _builder.MkAnd(path.Item2, _node);
                    ...
                    foreach (var path in _second.EnumeratePaths(pathCondition))
                    {
                        ...
                        SymbolicRegexNode<S> nullabilityTest = path.Item2 is null
                            ? _builder.MkNot(_node)
                            : _builder.MkAnd(path.Item2, _builder.MkNot(_node));
                        yield return (path.Item1, nullabilityTest, path.Item3);
                    }
                    break;
            }
        ...
        public SymbolicRegexNode<S> Transition(S minterm, uint context)
        {
            ...
            while (todo.Count > 0)
            {
                ...
                switch (top._kind)
                {
                    ...
                    case TransitionRegexKind.Conditional:
                        Debug.Assert(
                            top._test is not null
                                && top._first is not null
                                && top._second is not null
                        );
                        if (
                            _builder._solver.IsSatisfiable(_builder._solver.And(minterm, top._test))
                        )
                        {
                    ...
                    default:
                        Debug.Assert(
                            top._kind is TransitionRegexKind.Lookaround
                                && top._node is not null
                                && top._first is not null
                                && top._second is not null
                        );
