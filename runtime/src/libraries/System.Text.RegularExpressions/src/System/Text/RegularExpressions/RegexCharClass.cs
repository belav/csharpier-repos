...
namespace System.Text.RegularExpressions
{
    ...
    internal sealed partial class RegexCharClass
    {
        ...
        internal const string NotLetterOrDigitClass =
            "\u0001\0\b\0\u0002\u0004\u0005\u0003\u0001\0\t"; // [^\p{L}\d]
        ...
        internal const string PunctuationClass =
            "\0\0\t\0\u0013\u0014\u0016\u0019\u0015\u0018\u0017\0"; // \p{P}
        internal const string NotPunctuationClass =
            "\0\0\u0009\0\uffed\uffec\uffea\uffe7\uffeb\uffe8\uffe9\0"; // \P{P}
        ...
        private const string ECMAWordRanges =
            "\u0030\u003A\u0041\u005B\u005F\u0060\u0061\u007B\u0130\u0131";
        private const string NotECMAWordRanges =
            "\0\u0030\u003A\u0041\u005B\u005F\u0060\u0061\u007B\u0130\u0131";
        ...
        private static readonly Dictionary<string, string> s_definedCategories = new Dictionary<
            string,
            string
        ...
        **/
        ...
        public void AddCategoryFromName(
            string categoryName,
            bool invert,
        ...
        public void AddCaseEquivalences(CultureInfo culture)
        {
            ...
            if (rangeList != null)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    {
                        if (
                            RegexCaseEquivalences.TryFindCaseEquivalencesForCharWithIBehavior(
                                range.First,
                                culture,
                                ref _caseBehavior,
        ...
        public static bool IsMergeable(string charClass) =>
            charClass != null && !IsNegated(charClass) && !IsSubtraction(charClass);
        ...
        public static bool TryGetOnlyCategories(
            string set,
            Span<UnicodeCategory> categories,
        ...
        /// </remarks>
        public static bool TryGetSingleRange(
            string set,
        ...
        )
        {
            if (
                ...
                set.Length == SetStartIndex + set[SetLengthIndex]
            ) // and no subtraction
            {
        ...
        )
        {
            ...
            ) // and no subtraction
            {
                ...
                if (setLength is 3 or 4)
                {
                    ...
                    range1 = (
                        set[SetStartIndex + 2],
                        setLength == 3 ? LastChar : (char)(set[SetStartIndex + 3] - 1)
                    );
                    return true;
                }
            }
        ...
        public static bool TryGetAsciiSetChars(
            string set,
            [NotNullWhen(true)] out char[]? asciiChars
        )
        {
        ...
        private static bool CanEasilyEnumerateSetContents(string set) =>
            CanEasilyEnumerateSetContents(set, out bool hasSubtraction) && !hasSubtraction;
        ...
        private static bool CanEasilyEnumerateSetContents(string set, out bool hasSubtraction)
        {
            ...
            return set.Length > SetStartIndex
        ...
        public static bool CharInClass(char ch, string set, ref uint[]? asciiLazyCache)
        {
            ...
            Debug.Assert(
                asciiLazyCache is null || asciiLazyCache.Length == CacheArrayLength,
                "set lookup should be able to store two bits for each of the first 128 characters"
            );
        ...
        )
        {
            ...
            Debug.Assert(
                (SetStartIndex & 0x1) == 1,
                "If SetStartIndex is not odd, the calculation below this will be reversed"
            );
            ...
            return CharInCategory(
                ch,
                set.AsSpan(SetStartIndex + start + setLength, categoryLength)
            );
        }
            ...
            switch (chars.Length)
            {
                ...
                case 1:
                    ...
                    if (chars[0] < 128)
                    {
                        ...
                        {
                            return asciiStrings[chars[0]] ??=
                                $"\0\u0002\0{chars[0]}{(char)(chars[0] + 1)}";
                        }
                    }
                    break;
                ...
                case 2:
                    ...
                    {
                        return s_asciiLetterPairStrings[masked0 - 'a'] ??=
                            $"\0\u0004\0{(char)(masked0 & ~0x20)}{(char)((masked0 & ~0x20) + 1)}{(char)masked0}{(char)(masked0 + 1)}";
                    }
                    break;
            }
...
#pragma warning restore CS8500
        ...
        private void ToStringClass(ref ValueStringBuilder vsb)
        {
            ...
            vsb[initialLength + SetLengthIndex] = (char)(
                vsb.Length - initialLength - SetStartIndex
            );
            ...
            if (rangelist != null)
            {
                ...
                )
                {
                    if (rangelist.Count == 2)
                    {
                        ...
                        {
                            rangelist[0] = (
                                (char)(rangelist[0].Last + 1),
                                (char)(rangelist[1].First - 1)
                            );
                ...
                if (
                    ...
                    && rangelist.Count == 2
                    && rangelist[0].First == 0
                    && rangelist[0].Last + 2 == rangelist[1].First
                    && rangelist[1].Last == LastChar
                )
                {
                    var vsb = new ValueStringBuilder(stackalloc char[256]);
        ...
        private static ReadOnlySpan<char> RangesFromProperty(
            string capname,
            bool invert,
            string pattern,
            int currentPos
        )
        {
            ...
            throw new RegexParseException(
                ...
                SR.Format(
                    ...
                    pattern,
                    currentPos,
                    SR.Format(SR.UnrecognizedUnicodeProperty, capname)
                )
            );
        }
        ...
        public static string DescribeSet(string set)
        {
            ...
            if (
                ...
                categoryLength == 0
                && // no categories
                endPosition >= set.Length
                && // no subtraction
                setLength == 2
                && // don't bother handling the case of the single character being 0xFFFF, in which case setLength would be 1
            ...
            if (
                ...
                && // no subtraction
                setLength % 2 == 1
                && // odd number of values because the last range won't include an upper bound
                set[index] == 0
            )
            {
                    ...
                    if (lastindex != -1)
                    {
                        ...
                        switch (group)
                        {
                            ...
                            default:
                                ...
                                Span<char> invertedGroup =
                                    group.Length <= scratch.Length
                                        ? scratch.Slice(0, group.Length)
                                        : new char[group.Length];
                                ...
                                foreach (KeyValuePair<string, string> kvp in s_definedCategories)
                                {
                                    ...
                                    if (
                                        equalsGroup
                                        || invertedGroup.SequenceEqual(kvp.Value.AsSpan())
                                    )
                                    {
                                        desc.Append(equalsGroup ? @"\p{" : @"\P{")
                                            .Append(kvp.Key)
                                            .Append('}');
                                        break;
                                    }
