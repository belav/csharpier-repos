...
namespace System.Text.RegularExpressions
{
    ...
    internal sealed partial class RegexCharClass
    {
        ...
        **/
        ...
        private List<SingleRange> EnsureRangeList() => _rangelist ??= new List<SingleRange>(6);
        ...
        public static bool TryGetSingleUnicodeCategory(
            string set,
        ...
        public static bool CharInClass(char ch, string set, ref int[]? asciiResultCache)
        {
            ...
            Debug.Assert(
                asciiResultCache is null || asciiResultCache.Length == CacheArrayLength,
            ...
            if (ch < 128)
            {
                ...
                if (asciiResultCache is null)
                {
                    Interlocked.CompareExchange(
                        ref asciiResultCache,
                        new int[CacheArrayLength],
                        null
                    );
                }
        ...
        private static RegexCharClass ParseRecursive(string charClass, int start)
        {
            ...
            if (setLength > 0)
            {
                ...
                while (i < end)
                {
                    ...
                    char last = i < end ? (char)(charClass[i] - 1) : LastChar;
                    i++;
        ...
        private void Canonicalize(bool isNonBacktracking)
        {
            ...
            if (rangelist != null)
            {
                ...
                if (
                    !isNonBacktracking
                    && // do not produce the IsSingletonInverse transformation in NonBacktracking mode
                    !_negate
                    && _subtractor is null
        ...
        private static ReadOnlySpan<char> SetFromProperty(
            string capname,
            bool invert,
            string pattern,
            int currentPos
                ...
                if (ch1 == 0)
                {
                    ...
                    if (lastindex != -1)
                    {
                        ...
                        foreach (KeyValuePair<string, string> kvp in s_definedCategories)
                        {
                            ...
                            {
                                desc.Append((short)set[index + 1] > 0 ? "\\p{" : "\\P{")
                                    .Append(kvp.Key)
                                    .Append('}');
                                found = true;
                                break;
