...
namespace System.Text.RegularExpressions
{
    ...
    internal abstract class RegexCompiler
    {
        ...
        protected static readonly FieldInfo s_runtrackcountField = RegexRunnerField(
            "runtrackcount"
        );
        ...
        private static readonly MethodInfo s_isECMABoundaryMethod = RegexRunnerMethod(
            "IsECMABoundary"
        );
        ...
        /// </summary>
        internal static void CompileToAssembly(
            ...
            for (int i = 0; i < regexinfos.Length; i++)
            {
                if (regexinfos[i] is null)
                {
                    throw new ArgumentNullException(
                        nameof(regexinfos),
                        SR.ArgumentNull_ArrayWithNullElements
                    );
                }
                ...
                string fullname =
                    regexinfos[i].Namespace.Length == 0
                        ? regexinfos[i].Name
                ...
                c.GenerateRegexType(
                    ...
                    options,
                    fullname,
                    regexinfos[i].IsPublic,
                    code,
                    regexinfos[i].MatchTimeout
                );
            }
        ...
        protected void Newobj(ConstructorInfo constructor) =>
        ...
        private bool UseToLowerInvariant =>
            _textInfoLocal == null || (_options & RegexOptions.CultureInvariant) != 0;
        ...
        protected void GenerateFindFirstChar()
        {
            ...
            {
                bool needsCulture =
                    _options.HasFlag(RegexOptions.IgnoreCase)
                    || _boyerMoorePrefix?.CaseInsensitive == true;
            ...
            if (
                (
                    _leadingAnchor
                    & (
                        RegexPrefixAnalyzer.Beginning
                        | RegexPrefixAnalyzer.Start
                        | RegexPrefixAnalyzer.EndZ
            ...
            if (_boyerMoorePrefix != null && _boyerMoorePrefix.NegativeUnicode == null)
            {
                ...
                using (RentedLocalBuilder chLocal = RentInt32Local())
                {
                    ...
                    // goto advance;
                    int negativeRange =
                        _boyerMoorePrefix.HighASCII - _boyerMoorePrefix.LowASCII + 1;
                    if (negativeRange > 1)
                    {
                        // Create a string to store the lookup table we use to find the offset.
                        Debug.Assert(
                            _boyerMoorePrefix.Pattern.Length <= char.MaxValue,
                            "RegexBoyerMoore should have limited the size allowed."
                        );
                        string negativeLookup = string.Create(
                            negativeRange,
                            (thisRef: this, beforefirst),
                            static (span, state) =>
                            {
                                ...
                                {
                                    int offset = state.thisRef._boyerMoorePrefix!.NegativeASCII[
                                        i + state.thisRef._boyerMoorePrefix.LowASCII
                                    ];
            ...
            {
                Debug.Assert(
                    _leadingCharClasses.Length == 1,
                    "Only the FirstChars and not MultiFirstChars computation is supported for RightToLeft"
                );
                ...
                if (!RegexCharClass.IsSingleton(_leadingCharClasses[0].CharClass))
                {
                    EmitMatchCharacterClass(
                        _leadingCharClasses[0].CharClass,
            ...
            else
            {
                ...
                bool canUseIndexOf =
                    !_leadingCharClasses[0].CaseInsensitive
                    && (
                        setCharsCount = RegexCharClass.GetSetChars(
                            _leadingCharClasses[0].CharClass,
                            setChars
                        )
                    ) > 0
                ...
                for (; charClassIndex < _leadingCharClasses.Length; charClassIndex++)
                {
                    ...
                    LdindU2();
                    EmitMatchCharacterClass(
                        _leadingCharClasses[charClassIndex].CharClass,
                        _leadingCharClasses[charClassIndex].CaseInsensitive
                    );
        ...
        private bool TryGenerateNonBacktrackingGo(RegexNode node)
        {
            ...
            static bool NodeSupportsNonBacktrackingImplementation(RegexNode node, int maxDepth)
            {
                ...
                if ((node.Options & RegexOptions.RightToLeft) == 0 && maxDepth > 0)
                {
                    ...
                    switch (node.Type)
                    {
                        ...
                        case RegexNode.Setloop:
                            Debug.Assert(
                                node.Next == null || node.Next.Type != RegexNode.Atomic,
                                "Loop should have been transformed into an atomic type."
                            );
                        ...
                        case RegexNode.Lazyloop:
                            supported =
                                ...
                                && NodeSupportsNonBacktrackingImplementation(
                                    node.Child(0),
                                    maxDepth - 1
                                );
                            break;
                        ...
                        case RegexNode.Prevent:
                            supported = NodeSupportsNonBacktrackingImplementation(
                                node.Child(0),
                                maxDepth - 1
                            );
                            break;
                        ...
                        case RegexNode.Alternate:
                            if (
                                ...
                                && (
                                    ...
                                    || // atomic alternate
                                    (node.Next.Type == RegexNode.Capture && node.Next.Next is null)
                                )
                            ) // root alternate
                            {
                ...
                {
                    Debug.WriteLine(
                        $"Unable to use non-backtracking code gen: node {node.Description()} isn't supported."
                    );
                }
#endif
                return supported;
            ...
            void EmitSingleChar(
                ...
                switch (node.Type)
                {
                    ...
                    default:
                        Debug.Assert(
                            node.Type == RegexNode.Notone
                                || node.Type == RegexNode.Notonelazy
                                || node.Type == RegexNode.Notoneloop
        ...
        private void EmitMatchCharacterClass(string charClass, bool caseInsensitive)
        {
            ...
            if (
                !invariant
                && RegexCharClass.TryGetSingleUnicodeCategory(
                    charClass,
            ...
            string bitVectorString = string.Create(
                8,
                (charClass, invariant),
                static (dest, state) => // String length is 8 chars == 16 bytes == 128 bits.
                {
                    for (int i = 0; i < 128; i++)
                    {
                        char c = (char)i;
