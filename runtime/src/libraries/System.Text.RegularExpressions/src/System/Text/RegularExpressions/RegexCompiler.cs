...
namespace System.Text.RegularExpressions
{
    ...
    internal abstract class RegexCompiler
    {
        ...
        private static readonly MethodInfo s_stringIndexOfCharInt = typeof(string)
            ...
            !;
        private static readonly MethodInfo s_stringLastIndexOfCharIntInt = typeof(string)
            .GetMethod("LastIndexOf", new Type[] { typeof(char), typeof(int), typeof(int) })
            !;
            ...
            void GenerateBoyerMoore(RegexBoyerMoore rbm)
            {
                ...
                using (RentedLocalBuilder chLocal = RentInt32Local())
                {
                    ...
                    if (negativeRange > 1)
                    {
                        ...
                        Debug.Assert(
                            rbm.Pattern.Length <= char.MaxValue,
                ...
                using (RentedLocalBuilder testLocal = RentInt32Local())
                {
                    ...
                    for (int i = rbm.Pattern.Length - 2; i >= 0; i--)
                    {
                        ...
                        if (
                            rbm.CaseInsensitive
                            && RegexCharClass.ParticipatesInCaseConversion(rbm.Pattern[charindex])
                        )
                        {
                            CallToLower();
                        }
                        ...
                        for (int i = 0; i < childCount; i++)
                        {
                            ...
                            )
                            {
                                ...
                                for (; i < exclusiveEnd; i++)
                                {
                                    EmitNode(
                                        node.Child(i),
                                        i + 1 < childCount ? node.Child(i + 1) : subsequent,
            ...
            )
            {
                ...
                else
                {
                    if (
                        ...
                        && RegexCharClass.ParticipatesInCaseConversion(node.Ch)
                    )
                    {
                        CallToLower();
                    }
            ...
            void EmitAtomicNodeLoop(RegexNode node)
            {
                ...
                Debug.Assert(
                    node.M == node.N
                        || (
                            node.Next != null
                            && (node.Next.Type is RegexNode.Atomic or RegexNode.Capture)
                        )
                );
            ...
            switch (_regexopcode)
            {
                ...
                case RegexCode.Set | RegexCode.Ci | RegexCode.Rtl:
                    ...
                    else
                    {
                        if (
                            IsCaseInsensitive()
                            && RegexCharClass.ParticipatesInCaseConversion(Operand(0))
                        )
                        {
                            CallToLower();
                        }
                ...
                //:         break Backward;
                {
                    ...
                    for (int i = 0; i < str.Length; i++)
                    {
                        ...
                        if (
                            IsCaseInsensitive()
                            && RegexCharClass.ParticipatesInCaseConversion(str[i])
                        )
                        {
                            CallToLower();
                        }
