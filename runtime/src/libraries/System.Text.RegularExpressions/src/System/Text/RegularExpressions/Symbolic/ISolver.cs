...
namespace System.Text.RegularExpressions.Symbolic
{
    ...
    /// <see cref="BDD"/> representations of those sets, and determining whether an element is in the
    // set.
    /// </summary>
    internal interface ISolver<TSet>
    {
        ...
        /// <summary>Intersects two sets to produce a new one that contains only the elements that's in both
        // (conjunction).</summary>
        TSet And(TSet set1, TSet set2);

        /// <summary>Unions two sets to produce a new one that contains elements that are in either or both
        // (disjunction).</summary>
        TSet Or(TSet set1, TSet set2);

        /// <summary>Unions all of the sets in <paramref name="sets"/> to produce a new one that contains
        // elements that are in any of the sets (disjunction).</summary>
        TSet Or(ReadOnlySpan<TSet> sets);

        /// <summary>Negates the set, producing a new one that contains the elements and only the elements
        // not in the original.</summary>
        TSet Not(TSet set);
