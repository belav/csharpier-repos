...
namespace System.Text.RegularExpressions
{
    internal sealed class RegexNode
    {
            ...
            while (toExamine.Count > 0)
            {
                ...
                switch (node.Type)
                {
                    ...
                    case UpdateBumpalong:
                        Debug.Assert(
                            childCount == 0,
                            $"Expected zero children for {node.TypeName}, got {childCount}."
                        );
                        break;
                    ...
                    case Require:
                        Debug.Assert(
                            childCount == 1,
                            $"Expected one and only one child for {node.TypeName}, got {childCount}."
                        );
                    ...
                    case Testgroup:
                        Debug.Assert(
                            childCount >= 1,
                            $"Expected at least one child for {node.TypeName}, got {childCount}."
                        );
                    ...
                    case Alternate:
                        Debug.Assert(
                            childCount >= 2,
                            $"Expected at least two children for {node.TypeName}, got {childCount}."
                        );
                        for (int i = 0; i < childCount; i++)
                        {
                            toExamine.Push(node.Child(i));
                ...
                switch (node.Type)
                {
                    ...
                    case Setlazy:
                        Debug.Assert(
                            ...
                            $"Expected non-null, non-empty string for {node.TypeName}."
                        );
                        break;
                    ...
                    default:
                        Debug.Assert(
                            node.Str is null,
                            $"Expected null string for {node.TypeName}, got \"{node.Str}\"."
                        );
                        break;
                }
            }
                        ...
                        switch (node.Type)
                        {
                            ...
                            case Setloopatomic when node.N == int.MaxValue:
                                ...
                                if (parent != null && parent.Type == Concatenate)
                                {
                                    parent.InsertChild(
                                        1,
                                        new RegexNode(UpdateBumpalong, node.Options)
                                    );
                                }
                                break;
                        }
            ...
            while (true)
            {
                switch (node.Type)
                {
                    ...
                    case Concatenate:
                        ...
                        if (
                            (
                                existingChild.Type == Alternate
                                || existingChild.Type == Loop
                                || existingChild.Type == Lazyloop
                            ) && (node.Next is null || node.Next.Type != Atomic)
        ...
        private RegexNode ReduceSet()
        {
            ...
            Debug.Assert(
                Type == Set || Type == Setloop || Type == Setloopatomic || Type == Setlazy
            );
            ...
            RegexNode ExtractCommonPrefixes()
            {
                ...
                for (int startingIndex = 0; startingIndex < children.Count - 1; startingIndex++)
                {
                    ...
                    for (int i = startingIndex; i < endingIndex; i++)
                    {
                        ...
                        ProcessOneOrMulti(
                            branch.Type == Concatenate ? branch.Child(0) : branch,
                            startingSpan
                        );
        ...
        internal bool SupportsSimplifiedCodeGenerationImplementation()
        {
                    ...
                    case Alternate:
                        if (
                            ...
                            && (
                                IsAtomicByParent()
                                || // atomic alternate
                                (Next.Type == Capture && Next.Next is null)
                            )
                        ) // root alternate
                        {
                    ...
                    case Capture:
                        ...
                        if (supported)
                        {
                            ...
                            if (supported)
                            {
                                ...
                                supported = Child(0)
                                    .SupportsSimplifiedCodeGenerationImplementation();
                                ...
                                if (supported)
                                {
                                    parent = this;
                                    while (
                                        parent != null && ((parent.Options & HasCapturesFlag) == 0)
                                    )
                                    {
            ...
            {
                Debug.WriteLine(
                    $"Unable to use non-backtracking code gen: node {Description()} isn't supported."
                );
            }
#endif
            return supported;
        ...
        public bool IsNotoneFamily =>
            Type is Notone or Notoneloop or Notoneloopatomic or Notonelazy;
