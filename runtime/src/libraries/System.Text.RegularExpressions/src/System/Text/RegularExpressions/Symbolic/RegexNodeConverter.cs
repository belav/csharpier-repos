...
namespace System.Text.RegularExpressions.Symbolic
{
    ...
    internal sealed class RegexNodeConverter
    {
        ...
        public RegexNodeConverter(
            SymbolicRegexBuilder<BDD> builder,
            Hashtable? captureSparseMapping
        )
        {
            _builder = builder;
        ...
        internal SymbolicRegexNode<BDD> ConvertToSymbolicRegexNode(RegexNode root)
        {
            ...
            Stack<(RegexNode Node, DoublyLinkedList<
                ...
                > Result, DoublyLinkedList<SymbolicRegexNode<BDD>>[]? ChildResults)> stack = new();
            ...
            while (
                stack.TryPop(
                    out (RegexNode Node, DoublyLinkedList<
                        SymbolicRegexNode<BDD>
                    > Result, DoublyLinkedList<SymbolicRegexNode<BDD>>[]? ChildResults) popped
                )
            )
            {
                ...
                if (childResults is null || childResults[0] is null)
                {
                    ...
                    switch (node.Kind)
                    {
                        ...
                        case RegexNodeKind.One:
                            result.AddLast(
                                _builder.CreateSingleton(_builder._solver.CreateFromChar(node.Ch))
                            );
                            break;
                        ...
                        case RegexNodeKind.Notone:
                            result.AddLast(
                                _builder.CreateSingleton(
                                    _builder._solver.Not(_builder._solver.CreateFromChar(node.Ch))
                                )
                            );
                            break;
                        ...
                        case RegexNodeKind.Multi:
                        {
                            ...
                            {
                                result.AddLast(
                                    _builder.CreateSingleton(_builder._solver.CreateFromChar(c))
                                );
                            }
                            break;
                        }
                            ...
                            for (int i = 0; i < node.ChildCount(); ++i)
                            {
                                ...
                                stack.Push(
                                    (
                                        node.Child(i),
                                        childResults[i],
                                        CreateChildResultArray(node.Child(i).ChildCount())
                                    )
                                );
                            }
                            break;
                        ...
                        case RegexNodeKind.Notonelazy:
                        {
                            ...
                            result.AddLast(
                                _builder.CreateLoop(
                                    _builder.CreateSingleton(bdd),
                                    node.Kind is RegexNodeKind.Onelazy or RegexNodeKind.Notonelazy,
                                    node.M,
                                    node.N
                                )
                            );
                        ...
                        case RegexNodeKind.Setlazy:
                        {
                            ...
                            result.AddLast(
                                _builder.CreateLoop(
                                    _builder.CreateSingleton(setBdd),
                                    node.Kind == RegexNodeKind.Setlazy,
                                    node.M,
                                    node.N
                                )
                            );
                        ...
                        default:
                            ...
                            static string UnexpectedNodeType(RegexNode node)
                            {
                                ...
                                string description =
                                    $"Unexpected ({nameof(RegexNodeKind)}: {node.Kind})";
                ...
                else
                {
                    ...
                    switch (node.Kind)
                    {
                        case RegexNodeKind.Concatenate:
                        {
                            ...
                            foreach (
                                DoublyLinkedList<SymbolicRegexNode<BDD>> childResult in childResults
                            )
                            {
                        ...
                        case RegexNodeKind.Alternate:
                        {
                            ...
                            {
                                DoublyLinkedList<SymbolicRegexNode<BDD>> childResult = childResults[
                                    i
                                ];
                                ...
                                SymbolicRegexNode<BDD> elem =
                                    childResult.Count == 1
                                        ...
                                        : _builder.CreateConcatAlreadyReversed(
                                            childResult.EnumerateLastToFirst()
                                        );
                                if (elem.IsNothing)
                                {
                                    continue;
                        ...
                        case RegexNodeKind.Lazyloop:
                        {
                            ...
                            result.AddLast(
                                _builder.CreateLoop(
                                    body,
                                    node.Kind == RegexNodeKind.Lazyloop,
                                    node.M,
                                    node.N
                                )
                            );
                        ...
                        default:
                        {
                            ...
                            int captureNum = RegexParser.MapCaptureNumber(
                                node.M,
            ...
            Debug.Assert(
                rootResult.Count == 1
                    || root.Kind == RegexNodeKind.Concatenate
            ...
            void EnsureWordLetterPredicateInitialized()
            {
                ...
                if (_builder._wordLetterForBoundariesSet.Equals(_builder._solver.Empty))
                {
                    ...
                    _builder._wordLetterForBoundariesSet =
                        UnicodeCategoryConditions.WordLetterForAnchors(
                            (CharSetSolver)_builder._solver
                        );
                }
            }
            ...
            DoublyLinkedList<SymbolicRegexNode<BDD>>[]? CreateChildResultArray(int k) =>
        ...
        private BDD CreateBDDFromSetString(string set)
        {
            ...
            ref BDD? result = ref CollectionsMarshal.GetValueRefOrAddDefault(
                _setBddCache,
                set,
                out _
            );
            ...
            BDD Compute(string set)
            {
                ...
                while (i < catStart + catLength)
                {
                    ...
                    if (categoryCode != 0)
                    {
                        ...
                        BDD cond = MapCategoryCodeToCondition(
                            (UnicodeCategory)(Math.Abs(categoryCode) - 1)
                        );
                ...
                BDD result =
                    conditions.Count == 0
                        ...
                        : (
                            negate
                                ? charSetSolver.And(CollectionsMarshal.AsSpan(conditions))
                                : charSetSolver.Or(CollectionsMarshal.AsSpan(conditions))
                        );
                ...
                BDD MapCategoryCodeToCondition(UnicodeCategory code)
                {
                    Debug.Assert(
                        Enum.IsDefined(code)
                            || code == (UnicodeCategory)(RegexCharClass.SpaceConst - 1),
