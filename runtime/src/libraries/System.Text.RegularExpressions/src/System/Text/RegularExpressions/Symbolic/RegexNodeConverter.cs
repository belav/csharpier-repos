    ...
    internal sealed class RegexNodeConverter
    {
        ...
        internal SymbolicRegexNode<BDD> ConvertToSymbolicRegexNode(RegexNode root)
        {
            ...
            Stack<(
                ...
                DoublyLinkedList<SymbolicRegexNode<BDD>>[]? ChildResults
            )> stack = new();
                    ...
                    switch (node.Kind)
                    {
                        ...
                        case RegexNodeKind.Multi:
                        {
                            ...
                            foreach (char c in str)
                            {
                                result.AddLast(
                                    _builder.CreateSingleton(
                                        _builder._charSetSolver.CreateBDDFromChar(c)
                                    )
                                );
                            }
                            break;
            ...
            BDD Compute(string set)
            {
                ...
                while (i < catStart + catLength)
                {
                    ...
                    while (categoryCode != 0)
                    {
                        ...
                        Debug.Assert(
                            cat >= 0 && cat < categoryCodes.Length,
                            $"Expected {cat} to be in range [0, {categoryCodes.Length})."
                        );
