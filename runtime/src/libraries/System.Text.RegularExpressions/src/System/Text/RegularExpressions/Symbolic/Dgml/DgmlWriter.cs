...
#if DEBUG
using System.Collections.Generic;using System.Diagnostics;using System.IO;
...
namespace System.Text.RegularExpressions.Symbolic.DGML
{
    internal sealed class DgmlWriter
    {
        ...
        internal DgmlWriter(
            TextWriter tw,
            bool hideStateInfo,
            int maxDgmlTransitionLabelLength = -1,
        ...
        public void Write<S>(IAutomaton<S> fa)
        {
            ...
            else
            {
                ...
                {
                    _tw.WriteLine(
                        "<Node Id=\"{0}\" Label=\"{0}\" Category=\"State\" Group=\"{1}\" StateInfo=\"{2}\">",
                        state,
                        _hideStateInfo ? "Collapsed" : "Expanded",
                        GetStateInfo(fa, state)
                    );
                ...
                _tw.WriteLine("<Links>");
                _tw.WriteLine(
                    "<Link Source=\"dfa\" Target=\"{0}\" Label=\"{1}\" Category=\"StartTransition\" />",
                    fa.InitialState,
                    fa.DescribeStartLabel()
                );
                _tw.WriteLine(
                ...
                {
                    _tw.WriteLine(
                        "<Link Source=\"{0}\" Target=\"{1}\" Category=\"EpsilonTransition\" />",
                        move.SourceState,
                        move.TargetState
                    );
                }
                ...
                {
                    _tw.WriteLine(
                        GetNonFinalRuleInfo(fa, move.Key.Item1, move.Key.Item2, move.Value)
                    );
                }
        ...
        private string GetNonFinalRuleInfo<S>(
            IAutomaton<S> aut,
            int source,
            int target,
            List<S> rules
