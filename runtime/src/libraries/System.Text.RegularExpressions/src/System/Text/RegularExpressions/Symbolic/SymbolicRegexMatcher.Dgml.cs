    ...
    internal sealed partial class SymbolicRegexMatcher<TSet>
    {
        ...
        public override void SaveDGML(TextWriter writer, int maxLabelLength)
        {
            lock (this)
            {
                ...
                > transitions = GatherTransitions(this);
                ...
                writer.WriteLine(
                    ...
                    FormatInfo(this, transitions.Count)
                );
                writer.WriteLine(
                ...
                foreach (MatchingState<TSet> state in _stateCache.Values)
                {
                    ...
                    string nodeDgmlView =
                        $"{(info == string.Empty ? info : $"Previous: {info}&#13;")}{(deriv == string.Empty ? "()" : deriv)}";
            ...
            static Dictionary<
                (int Source, int Target),
                (TSet Rule, List<int> NfaTargets)
            > GatherTransitions(SymbolicRegexMatcher<TSet> matcher)
            {
                ...
                foreach (MatchingState<TSet> source in matcher._stateCache.Values)
                {
                    ...
                    for (int i = 0; i < deltas.Length; ++i)
                    {
                        ...
                        if (targetId >= 0)
                        {
                            ...
                            result[key] = (
                                matcher.Solver.Or(entry.Rule, matcher._minterms[i]),
                                entry.NfaTargets
                            );
                        }
                    }
