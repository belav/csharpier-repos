...
namespace System.Text.RegularExpressions.Symbolic
{
    internal sealed partial class SymbolicRegexMatcher<TSet>
    {
        ...
        public override void SaveDGML(TextWriter writer, int maxLabelLength)
        {
            ...
            writer.WriteLine(
                "        <Node Id=\"dfa\" Label=\" \" Group=\"Collapsed\" Category=\"DFA\" DFAInfo=\"{0}\" />",
                FormatInfo(_builder, transitions.Count)
            );
            writer.WriteLine(
                "        <Node Id=\"dfainfo\" Category=\"DFAInfo\" Label=\"{0}\"/>",
                FormatInfo(_builder, transitions.Count)
            );
            foreach (DfaMatchingState<TSet> state in _builder._stateCache)
            {
                writer.WriteLine(
                    "        <Node Id=\"{0}\" Label=\"{0}\" Category=\"State\" Group=\"Collapsed\" StateInfo=\"{1}\">",
                    state.Id,
                    state.DgmlView
                );
                ...
                writer.WriteLine(
                    "        <Node Id=\"{0}info\" Label=\"{1}\" Category=\"StateInfo\"/>",
                    state.Id,
                    state.DgmlView
                );
            }
            ...
            foreach (DfaMatchingState<TSet> initialState in GetInitialStates(this))
            {
                ...
                writer.WriteLine(
                    "        <Link Source=\"dfa\" Target=\"{0}\" Label=\"\" Category=\"StartTransition\" />",
                    initialState.Id
                );
            }
            writer.WriteLine(
                "        <Link Source=\"dfa\" Target=\"dfainfo\" Label=\"\" Category=\"Contains\" />"
            );

            foreach (
                KeyValuePair<
                    (int Source, int Target),
                    (TSet Rule, List<int> NfaTargets)
            ...
            )
            {
                ...
                writer.WriteLine(
                    $"        <Link Source=\"{transition.Key.Source}\" Target=\"{transition.Key.Target}\" Label=\"{label}\" Category=\"NonEpsilonTransition\" {info}/>"
                );
                ...
                {
                    writer.WriteLine(
                        $"        <Link Source=\"{transition.Key.Target}\" Target=\"{nfaTarget}\" Category=\"EpsilonTransition\"/>"
                    );
                }
            }
            ...
            {
                writer.WriteLine(
                    "        <Link Source=\"{0}\" Target=\"{0}info\" Category=\"Contains\" />",
                    state.Id
                );
            }
            ...
            writer.WriteLine(
                "        <Category Id=\"FinalState\" Label=\"Final\" IsTag=\"True\" />"
            );
            writer.WriteLine(
            ...
            writer.WriteLine(
                "        <Category Id=\"InitialState\" Label=\"Initial\" IsTag=\"True\" />"
            );
            writer.WriteLine(
            ...
            writer.WriteLine("        </Style>");
            writer.WriteLine(
                "        <Style TargetType=\"Node\" GroupLabel=\"StateInfo\" ValueLabel=\"True\">"
            );
            ...
            writer.WriteLine("        </Style>");
            writer.WriteLine(
                "        <Style TargetType=\"Node\" GroupLabel=\"DFAInfo\" ValueLabel=\"True\">"
            );
            writer.WriteLine("            <Setter Property=\"Stroke\" Value=\"white\" />");
            writer.WriteLine("            <Setter Property=\"FontSize\" Value=\"18\" />");
            writer.WriteLine("            <Setter Property=\"FontFamily\" Value=\"Arial\" />");
            ...
            static Dictionary<
                (int Source, int Target),
                (TSet Rule, List<int> NfaTargets)
            > GatherTransitions(SymbolicRegexBuilder<TSet> builder)
            {
                Debug.Assert(builder._delta is not null);
                Debug.Assert(builder._minterms is not null);
                Dictionary<(int Source, int Target), (TSet Rule, List<int> NfaTargets)> result =
                    new();
                    ...
                    for (int i = 0; i < deltas.Length; ++i)
                    {
                        ...
                        if (deltas[i] is DfaMatchingState<TSet> target)
                        {
                            ...
                            if (
                                !result.TryGetValue(
                                    key,
                                    out (TSet Rule, List<int> NfaTargets) entry
                                )
                            )
                            {
                            ...
                            result[key] = (
                                builder._solver.Or(entry.Rule, builder._minterms[i]),
                                entry.NfaTargets
                            );
                        }
                    }
            ...
            static string FormatInfo(SymbolicRegexBuilder<TSet> builder, int transitionCount)
            {
                ...
                sb.Append($"Min Terms ({builder._solver.GetMinterms()!.Length}) = ")
                    .AppendJoin(',', DescribeLabels(builder._solver.GetMinterms()!, builder));
                return sb.ToString();
            }

            static IEnumerable<string> DescribeLabels(
                IEnumerable<TSet> labels,
            ...
            static IEnumerable<DfaMatchingState<TSet>> GetInitialStates(
