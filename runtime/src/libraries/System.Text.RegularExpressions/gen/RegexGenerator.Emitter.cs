    ...
    public partial class RegexGenerator
    {
        ...
        )
        {
            ...
            writer.WriteLine(
                rm.MatchTimeout is not null
                    ...
                    : rm.Options != 0
                        ? $"new({Literal(rm.Pattern)}, {Literal(rm.Options)});"
        ...
        )
        {
            ...
            void EmitIndexOf_LeftToRight()
            {
                ...
                writer.WriteLine(
                    $"int i = inputSpan.Slice(pos{offset}).IndexOf({Literal(substring)});"
                );
            ...
            void EmitFixedSet_LeftToRight()
            {
                ...
                List<(char[]? Chars, string Set, int Distance)>? sets = regexTree
                    .FindOptimizations
                    .FixedDistanceSets;
                ...
                if (canUseIndexOf)
                {
                    ...
                    string indexOf = primarySet.Chars!.Length switch
                    {
                        ...
                        2
                            => $"{span}.IndexOfAny({Literal(primarySet.Chars[0])}, {Literal(primarySet.Chars[1])})",
                        3
                            => $"{span}.IndexOfAny({Literal(primarySet.Chars[0])}, {Literal(primarySet.Chars[1])}, {Literal(primarySet.Chars[2])})",
            ...
            void EmitFixedSet_RightToLeft()
            {
                ...
                (char[]? Chars, string Set, int Distance) set = regexTree
                    .FindOptimizations
                    .FixedDistanceSets![0];
                ...
                else
                {
                    using (EmitBlock(writer, "while ((uint)--pos < (uint)inputSpan.Length)"))
                    {
                        using (
                            EmitBlock(
                                writer,
                                $"if ({MatchCharacterClass(options, "inputSpan[pos]", set.Set, negate: false, additionalDeclarations, requiredHelpers)})"
                            )
                        )
                        {
                            writer.WriteLine("base.runtextpos = pos + 1;");
            ...
            void EmitLiteralAfterAtomicLoop()
            {
                ...
                (RegexNode LoopNode, (char Char, string? String, char[]? Chars) Literal) target =
                    regexTree.FindOptimizations.LiteralAfterLoop.Value;
                ...
                writer.WriteLine(
                    target.Literal.String is not null
                        ? $"the string {Literal(target.Literal.String)}."
                ...
                using (block)
                {
                    ...
                    using (
                        EmitBlock(
                            writer,
                            $"while ((uint)prev < (uint)slice.Length && {MatchCharacterClass(options, "slice[prev]", target.LoopNode.Str!, negate: false, additionalDeclarations, requiredHelpers)})"
                        )
                    )
                    {
        ...
        )
        {
            ...
            )
            {
                ...
                ) // char, string, chars, negated
                {
                    ...
                    using (
                        EmitBlock(
                            writer,
                            literal.Item2 is not null
                                ? $"    ({endingPos} = inputSpan.Slice({startingPos}, Math.Min(inputSpan.Length, {endingPos} + {literal.Item2.Length - 1}) - {startingPos}).{lastIndexOfName}({Literal(literal.Item2)})) < 0)"
                                : literal.Item3 is null
                                    ? $"    ({endingPos} = inputSpan.Slice({startingPos}, {endingPos} - {startingPos}).{lastIndexOfName}({Literal(literal.Item1)})) < 0)"
                                    : literal.Item3.Length switch
                                    {
                                        2
                                            => $"    ({endingPos} = inputSpan.Slice({startingPos}, {endingPos} - {startingPos}).{lastIndexOfAnyName}({Literal(literal.Item3[0])}, {Literal(literal.Item3[1])})) < 0)",
                                        3
                                            => $"    ({endingPos} = inputSpan.Slice({startingPos}, {endingPos} - {startingPos}).{lastIndexOfAnyName}({Literal(literal.Item3[0])}, {Literal(literal.Item3[1])}, {Literal(literal.Item3[2])})) < 0)",
                                        _
                                            => $"    ({endingPos} = inputSpan.Slice({startingPos}, {endingPos} - {startingPos}).{lastIndexOfAnyName}({Literal(literal.Item3)})) < 0)",
                                    }
                        )
                    )
                    {
                ...
                if ((node.Options & RegexOptions.RightToLeft) == 0)
                {
                    ...
                    ) // not negated; can't search for both the node.Ch and a negated subsequent char with an IndexOf* method
                    {
                        ...
                        else // set literal
                        {
                            ...
                            writer.WriteLine(
                                (overlap, literal.Item3.Length) switch
                                {
                                    (true, 2)
                                        => $"{startingPos} = {sliceSpan}.IndexOfAny({Literal(literal.Item3[0])}, {Literal(literal.Item3[1])});",
                                    (true, 3)
                                        => $"{startingPos} = {sliceSpan}.IndexOfAny({Literal(literal.Item3[0])}, {Literal(literal.Item3[1])}, {Literal(literal.Item3[2])});",
                                    (true, _)
                                        => $"{startingPos} = {sliceSpan}.IndexOfAny({Literal(literal.Item3)});",

                                    (false, 2)
                                        => $"{startingPos} = {sliceSpan}.IndexOfAny({Literal(node.Ch)}, {Literal(literal.Item3[0])}, {Literal(literal.Item3[1])});",
                                    (false, _)
                                        => $"{startingPos} = {sliceSpan}.IndexOfAny({Literal($"{node.Ch}{literal.Item3}")});",
                                }
                            );
                        }
                    ...
                    )
                    {
                        ...
                        writer.WriteLine(
                            literal2.Item2 is not null
                                ? $"{startingPos} = {sliceSpan}.{indexOfName}({Literal(literal2.Item2)});"
                                : literal2.Item3 is null
                                    ? $"{startingPos} = {sliceSpan}.{indexOfName}({Literal(literal2.Item1)});"
                                    : literal2.Item3.Length switch
                                    {
                                        2
                                            => $"{startingPos} = {sliceSpan}.{indexOfAnyName}({Literal(literal2.Item3[0])}, {Literal(literal2.Item3[1])});",
                                        3
                                            => $"{startingPos} = {sliceSpan}.{indexOfAnyName}({Literal(literal2.Item3[0])}, {Literal(literal2.Item3[1])}, {Literal(literal2.Item3[2])});",
                                        _
                                            => $"{startingPos} = {sliceSpan}.{indexOfAnyName}({Literal(literal2.Item3)});",
                                    }
                        );
                ...
                if (isInLoop)
                {
                    ...
                    EmitStackPush(
                        capturePos is not null && iterationCount is not null
                            ...
                            : capturePos is not null
                                ? new[] { startingPos, capturePos }
                                : iterationCount is not null
                    ...
                    EmitStackPush(
                        !isInLoop
                            ...
                            : iterationMayBeEmpty
                                ...
                                : expressionHasCaptures
                                    ? new[] { "pos", iterationCount, "base.Crawlpos()" }
            ...
            void EmitSingleCharAtomicLoop(RegexNode node, bool emitLengthChecksIfRequired = true)
            {
                ...
                )
                {
                    ...
                    writer.WriteLine(
                        (numSetChars, RegexCharClass.IsNegated(node.Str!)) switch
                        {
                            (2, true)
                                => $".IndexOfAny({Literal(setChars[0])}, {Literal(setChars[1])});",
                            (3, true)
                                => $".IndexOfAny({Literal(setChars[0])}, {Literal(setChars[1])}, {Literal(setChars[2])});",
                            (_, true)
                                => $".IndexOfAny({Literal(setChars.Slice(0, numSetChars).ToString())});",
                            (2, false)
                                => $".IndexOfAnyExcept({Literal(setChars[0])}, {Literal(setChars[1])});",
                            (3, false)
                                => $".IndexOfAnyExcept({Literal(setChars[0])}, {Literal(setChars[1])}, {Literal(setChars[2])});",
                            (_, false)
                                => $".IndexOfAnyExcept({Literal(setChars.Slice(0, numSetChars).ToString())});",
                        }
                    );
                    using (EmitBlock(writer, $"if ({iterationLocal} < 0)"))
                    {
            ...
            void EmitAtomicSingleCharZeroOrOne(RegexNode node)
            {
                ...
                string spaceAvailable = rtl
                    ? "pos > 0"
                    : sliceStaticPos != 0
                        ? $"(uint){sliceSpan}.Length > (uint){sliceStaticPos}"
                        : $"!{sliceSpan}.IsEmpty";
        ...
        private static void ReplaceAdditionalDeclarations(
            IndentedTextWriter writer,
        ...
        private static string DescribeSet(string charClass) =>
            charClass switch
            {
                ...
                RegexCharClass.NotECMASpaceClass
                    => "any character other than a space character (ECMA)",
        ...
        )
        {
            ...
            do
            {
                ...
                if (!skip)
                {
                    ...
                    writer.WriteLine(
                        $"/// {new string(' ', depth * 4)}{BulletPoint} {tag}{nodeDescription}<br/>"
                    );
                }
