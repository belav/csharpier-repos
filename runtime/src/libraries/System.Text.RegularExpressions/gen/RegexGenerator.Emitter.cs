...
namespace System.Text.RegularExpressions.Generator
{
    public partial class RegexGenerator
    {
        ...
        )
        {
            ...
            writer.WriteLine(
                $"{regexMethod.Modifiers} global::System.Text.RegularExpressions.Regex {regexMethod.MethodName}() => global::{GeneratedNamespace}.{generatedClassName}.{regexMethod.GeneratedName}.Instance;"
            );
                    ...
                    using (EmitBlock(writer, $"switch ({sliceSpan}[{sliceStaticPos++}])"))
                    {
                        ...
                        for (int i = 0; i < childCount; i++)
                        {
                            ...
                            if (childStart.Kind is RegexNodeKind.Set)
                            {
                                ...
                                writer.WriteLine(
                                    $"case {string.Join(" or ", setChars.Slice(0, numChars).ToArray().Select(c => Literal(c)))}:"
                                );
                            }
                            else
                            {
                    ...
                    using (
                        EmitBlock(
                            ...
                            literal.Item2 is not null
                                ? $"    ({endingPos} = inputSpan.Slice({startingPos}, Math.Min(inputSpan.Length, {endingPos} + {literal.Item2.Length - 1}) - {startingPos}).LastIndexOf({Literal(literal.Item2)})) < 0)"
                                : literal.Item3 is null
                                    ? $"    ({endingPos} = inputSpan.Slice({startingPos}, {endingPos} - {startingPos}).LastIndexOf({Literal(literal.Item1)})) < 0)"
                                    : literal.Item3.Length switch
                                    {
                                        2
                                            => $"    ({endingPos} = inputSpan.Slice({startingPos}, {endingPos} - {startingPos}).LastIndexOfAny({Literal(literal.Item3[0])}, {Literal(literal.Item3[1])})) < 0)",
                                        3
                                            => $"    ({endingPos} = inputSpan.Slice({startingPos}, {endingPos} - {startingPos}).LastIndexOfAny({Literal(literal.Item3[0])}, {Literal(literal.Item3[1])}, {Literal(literal.Item3[2])})) < 0)",
                                        _
                                            => $"    ({endingPos} = inputSpan.Slice({startingPos}, {endingPos} - {startingPos}).LastIndexOfAny({Literal(literal.Item3)})) < 0)",
                                    }
                        )
                    )
                    {
                    ...
                    )
                    {
                        ...
                        writer.WriteLine(
                            literal2.Item2 is not null
                                ? $"{startingPos} = {sliceSpan}.IndexOf({Literal(literal2.Item2)});"
                                : literal2.Item3 is null
                                    ? $"{startingPos} = {sliceSpan}.IndexOf({Literal(literal2.Item1)});"
                                    : literal2.Item3.Length switch
                                    {
                                        2
                                            => $"{startingPos} = {sliceSpan}.IndexOfAny({Literal(literal2.Item3[0])}, {Literal(literal2.Item3[1])});",
                                        3
                                            => $"{startingPos} = {sliceSpan}.IndexOfAny({Literal(literal2.Item3[0])}, {Literal(literal2.Item3[1])}, {Literal(literal2.Item3[2])});",
                                        _
                                            => $"{startingPos} = {sliceSpan}.IndexOfAny({Literal(literal2.Item3)});",
                                    }
                        );
                ...
                if (!isAtomic)
                {
                    ...
                    if (doneLabel != originalDoneLabel || !GotoWillExitMatch(originalDoneLabel)) // we don't need to back anything out if we're about to exit TryMatchAtCurrentPosition anyway.
                    {
                        ...
                        else
                        {
                            ...
                            writer.WriteLine(
                                $"// If the lazy loop has matched any iterations, backtrack into the last one."
                            );
