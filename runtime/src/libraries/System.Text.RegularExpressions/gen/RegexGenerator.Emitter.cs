    ...
    public partial class RegexGenerator
    {
        ...
        )
        {
            ...
            if (rm.Tree.CaptureNameToNumberMapping is not null)
            {
                ...
                AppendHashtableContents(
                    writer,
                    rm.Tree.CaptureNameToNumberMapping
                        .Cast<DictionaryEntry>()
        ...
        )
        {
            ...
            void EmitFixedSet_LeftToRight()
            {
                ...
                if (canUseIndexOf)
                {
                    ...
                    string indexOf = primarySet.Chars is not null
                        ? primarySet.Chars!.Length switch
                        {
                        ...
                        else if (literal.SetChars is not null) // set literal
                        {
                            ...
                            writer.WriteLine(
                                (overlap, literal.SetChars.Length) switch
                                {
                                    ...
                                    (true, _)
                                        => $"{startingPos} = {sliceSpan}.IndexOfAny({Literal(literal.SetChars)});",

                                    (false, 2)
                                        => $"{startingPos} = {sliceSpan}.IndexOfAny({Literal(node.Ch)}, {Literal(literal.SetChars[0])}, {Literal(literal.SetChars[1])});",
                                    (false, _)
                                        => $"{startingPos} = {sliceSpan}.IndexOfAny({Literal($"{node.Ch}{literal.SetChars}")});",
                                }
                            );
                        }
