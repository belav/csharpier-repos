    ...
    public partial class RegexGenerator : IIncrementalGenerator
    {
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
                            ...
                            if (!SupportsCodeGeneration(regexMethod, out string? reason))
                            {
                                return (
                                    ...
                                    Diagnostic.Create(
                                        ...
                                        regexMethod.MethodSyntax.GetLocation()
                                    )
                                );
                            }
            ...
            IncrementalValueProvider<(
                bool AllowUnsafe,
                string? AssemblyName
            )> compilationDataProvider = context.CompilationProvider.Select(
                (x, _) =>
                    (x.Options is CSharpCompilationOptions { AllowUnsafe: true }, x.AssemblyName)
            );
            ...
            context.RegisterSourceOutput(
                codeOrDiagnostics.Combine(compilationDataProvider),
                static (context, compilationDataAndResults) =>
                {
                    ...
                    foreach (object? result in results)
                    {
                        ...
                        if (
                            result
                            is ValueTuple<RegexMethod, string, Diagnostic> limitedSupportResult
                        )
                        {
                        ...
                        )
                        {
                            if (!regexImpl.Item1.IsDuplicate)
                            {
                                EmitRegexDerivedImplementation(
                                    ...
                                    regexImpl.Item1,
                                    regexImpl.Item2,
                                    compilationDataAndResults.Right.AllowUnsafe
                                );
                                writer.WriteLine();
                            }
                        }
