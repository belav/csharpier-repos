...
namespace System.Data.OleDb
{
    internal sealed class OleDbConnectionInternal : DbConnectionInternal, IDisposable
    {
        ...
        {
            get { return (OleDbConnection?)Owner; }
        }

        internal bool HasSession
        {
            get { return (null != _sessionwrp); }
        }
        ...
        internal UnsafeNativeMethods.ICommandText? ICommandText()
        {
            ...
            Debug.Assert(
                (0 <= hr) || (null == icommandText),
                "CreateICommandText: error with ICommandText"
            );
            if (hr < 0)
            {
        ...
        public override DbTransaction BeginTransaction(IsolationLevel isolationLevel)
        {
            ...
            try
            {
                ...
                UnsafeNativeMethods.ITransactionLocal? value = (
                    unknown as UnsafeNativeMethods.ITransactionLocal
                );
                if (null == value)
                {
        ...
        internal unsafe DataTable? BuildInfoLiterals()
        {
            using (IDBInfoWrapper wrapper = IDBInfo())
            {
                ...
                // https://github.com/dotnet/runtime/issues/44288: check may not be necessary (and thus method may
                // return non-nullable)
                if (null == dbInfo)
                {
                    return null;
                ...
                using (
                    DualCoTaskMem handle = new DualCoTaskMem(
                        dbInfo,
                        null,
                        out literalCount,
                        out literalInfo,
                ...
                )
                {
                    ...
                    // *prgLiteralInfo and sets the value of the fSupported element in all of the structures to FALSE. The
            ...
            using (IDBSchemaRowsetWrapper wrapper = IDBSchemaRowset())
            {
                ...
                )
                {
                    ...
                    if (IntPtr.Zero != schemaRestrictions)
                    {
                        for (int i = 0; i < supportedSchemas.Length; ++i)
                        {
                            supportedSchemas[i]._restrictions = Marshal.ReadInt32(
                                schemaRestrictions,
                                i * 4
                            );
                        }
        ...
        internal bool HasLiveReader(OleDbCommand cmd)
        {
            ...
            {
                reader = ReferenceCollection.FindItem<OleDbDataReader>(
                    OleDbReferenceCollection.DataReaderTag,
                    (dataReader) => cmd == dataReader.Command
                );
            }
        ...
        internal Dictionary<string, OleDbPropertyInfo>? GetPropertyInfo(Guid[] propertySets)
        {
            ...
            using (PropertyIDSet propidset = new PropertyIDSet(propertySets))
            {
                ...
                {
                    using (
                        PropertyInfoSet infoset = new PropertyInfoSet(
                            idbProperties.Value,
                            propidset
                        )
                    )
