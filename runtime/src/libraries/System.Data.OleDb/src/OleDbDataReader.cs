                            ...
                            if (0 <= metaindex)
                            {
                                ...
                                if (isUniq && (null != uniq))
                                {
                                    ...
                                    else if (indexname != uniqueIndexName)
                                    {
                                        ...
                                        {
                                            Debug.WriteLine(
                                                "MultipleUniqueIndexes detected: <"
                                                    + uniqueIndexName
                                                    + "> <"
                                                    + indexname
                                                    + ">"
        ...
        internal void DumpToSchemaTable(UnsafeNativeMethods.IRowset? rowset)
        {
            ...
            for (int index = visibleCount - 1; 0 <= index; --index)
            {
                ...
                if (info.guid.Equals(ODB.DBCOL_SPECIALCOL))
                {
                    ...
                    {
                        Debug.WriteLine(
                            "Filtered Column: DBCOLUMN_GUID=DBCOL_SPECIALCOL DBCOLUMN_NAME="
                                + info.columnName
                                + " DBCOLUMN_KEYCOLUMN="
                                + info.isKeyColumn
                        );
                ...
                )
                {
                    ...
                    {
                        Debug.WriteLine(
                            "Filtered Column: DBCOLUMN_NUMBER="
                                + info.ordinal.ToInt64().ToString(CultureInfo.InvariantCulture)
                                + " DBCOLUMN_NAME="
                                + info.columnName
                        );
                ...
                else if (OleDbDataReader.DoColumnDropFilter(info.flags))
                {
                    ...
                    {
                        Debug.WriteLine(
                            "Filtered Column: DBCOLUMN_FLAGS="
                                + info.flags.ToString("X8", null)
                                + " DBCOLUMN_NAME="
                                + info.columnName
                        );
                    }
