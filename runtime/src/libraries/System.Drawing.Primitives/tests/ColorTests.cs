...
namespace System.Drawing.Primitives.Tests
{
    ...
    {
        public static bool SupportsReadingUpdatedSystemColors =>
            PlatformDetection.IsWindows
            && !PlatformDetection.IsInAppContainer
            && PlatformDetection.IsNotWindowsNanoNorServerCore;

        public static readonly IEnumerable<object[]> NamedArgbValues = new[]
        {
        ...
        public void FromArgb_Roundtrips(int a, int r, int g, int b)
        {
            Color c1 = Color.FromArgb(
                unchecked((int)((uint)a << 24 | (uint)r << 16 | (uint)g << 8 | (uint)b))
            );
            Assert.Equal(a, c1.A);
            Assert.Equal(r, c1.R);
            Assert.Equal(g, c1.G);
        ...
        public static IEnumerable<object[]> ColorNamePairs =>
            ColorNames.Zip(ColorNames.Skip(1), (l, r) => new[] { l[0], r[0] });
        ...
        public static IEnumerable<object[]> InvalidValues =>
            new[] { new object[] { -1 }, new object[] { 256 } };
        ...
        public static IEnumerable<object[]> Equality_MemberData()
        {
            ...
            {
                Color.FromArgb(0, 1, 2, 3),
                Color.FromArgb(0, 0, 2, 3),
                false,
            };
            yield return new object[]
            {
                Color.FromArgb(0, 1, 2, 3),
                Color.FromArgb(0, 1, 0, 3),
                false,
            };
            yield return new object[]
            {
                Color.FromArgb(0, 1, 2, 3),
                Color.FromArgb(0, 1, 2, 0),
                false,
            };

            yield return new object[]
            {
                Color.FromName("SomeName"),
                Color.FromName("SomeName"),
                true,
            };
            yield return new object[]
            ...
            {
                Color.FromName("SomeName"),
                Color.FromName(someNameConstructed),
                true,
            };
        }
        ...
        public void ToKnownColor(KnownColor known) =>
            Assert.Equal(known, Color.FromKnownColor(known).ToKnownColor());
        ...
        public void IsSystemColorTrue(KnownColor known) =>
            Assert.True(Color.FromKnownColor(known).IsSystemColor);

        [Theory, MemberData(nameof(NonSystemColors))]
        public void IsSystemColorFalse(KnownColor known) =>
            Assert.False(Color.FromKnownColor(known).IsSystemColor);
        ...
        public static readonly IEnumerable<object[]> SystemColors = new[]
        {
        ...
        private static extern int SetSysColors(
            int cElements,
            int[] lpaElements,
            int[] lpaRgbValues
        );
