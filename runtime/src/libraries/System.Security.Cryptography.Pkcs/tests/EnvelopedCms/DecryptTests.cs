...
namespace System.Security.Cryptography.Pkcs.EnvelopedCmsTests.Tests
{
    public abstract partial class DecryptTests
    {
        ...
        public void Decrypt_Capi()
        {
            ...
            ContentInfo contentInfo = new ContentInfo(content);
            TestSimpleDecrypt_RoundTrip(
                Certificates.RSAKeyTransferCapi1,
                contentInfo,
                Oids.Aes256,
                SubjectIdentifierType.IssuerAndSerialNumber
            );
        ...
        public void Decrypt_384()
        {
            ...
            ContentInfo contentInfo = new ContentInfo(content);
            TestSimpleDecrypt_RoundTrip(
                Certificates.RSASha384KeyTransfer1,
                contentInfo,
                Oids.Aes256,
                SubjectIdentifierType.IssuerAndSerialNumber
            );
        ...
        public void EncryptToNegativeSerialNumber()
        {
            ...
            X509IssuerSerial issuerSerial = (X509IssuerSerial)
                recipientInfo.RecipientIdentifier.Value;
        ...
        private void TestSimpleDecrypt_RoundTrip(
        ...
        )
        {
            ...
            expectedContentInfo =
                expectedContentInfo
                ?? new ContentInfo(
                    new Oid(contentInfo.ContentType),
                    (byte[])(contentInfo.Content.Clone())
                );
            ...
            using (X509Certificate2 certificate = certLoader.GetCertificate())
            {
                ...
                Assert.Equal(
                    originalCopy.ByteArrayToHex(),
                    ecms.ContentInfo.Content.ByteArrayToHex()
                );
        ...
        internal void VerifySimpleDecrypt(
            byte[] encodedMessage,
            CertLoader certLoader,
        ...
        )
        {
            ...
            using (X509Certificate2 cert = certLoader.TryGetCertificateWithPrivateKey())
            {
                ...
                Assert.Equal(
                    expectedContent.Content.ByteArrayToHex(),
