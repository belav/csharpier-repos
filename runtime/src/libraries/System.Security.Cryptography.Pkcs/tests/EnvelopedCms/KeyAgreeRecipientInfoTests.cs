    ...
    public static partial class KeyAgreeRecipientInfoTests
    {
        ...
        public static void TestMultipleKeyAgree_ShortNotation()
        {
            ...
            Assert.Equal(
                ...
                recipient0Cast.OriginatorIdentifierOrKey.Type
            );
            Assert.Equal(
                SubjectIdentifierOrKeyType.PublicKeyInfo,
                recipient1Cast.OriginatorIdentifierOrKey.Type
            );

            Assert.Equal(
                SubjectIdentifierType.SubjectKeyIdentifier,
                recipient0Cast.RecipientIdentifier.Type
            );
            Assert.Equal(
                SubjectIdentifierType.SubjectKeyIdentifier,
                recipient1Cast.RecipientIdentifier.Type
            );

            Assert.Equal(
                "10DA1370316788112EB8594C864C2420AE7FBA42",
                recipient0Cast.RecipientIdentifier.Value
            );
            Assert.Equal(
                "11DA1370316788112EB8594C864C2420AE7FBA42",
                recipient1Cast.RecipientIdentifier.Value
            );
        }
