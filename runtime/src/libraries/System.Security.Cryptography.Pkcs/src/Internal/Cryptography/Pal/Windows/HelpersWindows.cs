...
namespace Internal.Cryptography.Pal.Windows
{
    internal static class HelpersWindows
    {
        ...
        public static unsafe byte[] GetMsgParamAsByteArray(
        ...
        public static SafeHandle GetMsgParamAsMemory(
            this SafeCryptMsgHandle hCryptMsg,
            CryptMsgParamType paramType,
            int index = 0
        )
        ...
        public static int GetVersion(this SafeCryptMsgHandle hCryptMsg)
        {
            ...
            if (
                !Interop.Crypt32.CryptMsgGetParam(
                    hCryptMsg,
                    CryptMsgParamType.CMSG_VERSION_PARAM,
                    0,
                    out version,
                    ref cbData
                )
        ...
        {
            byte[] oidBytes = hCryptMsg.GetMsgParamAsByteArray(
        ...
        )
        {
            ...
            if (
                !Interop.Crypt32.CryptMsgGetParam(
                    hCryptMsg,
                    CryptMsgParamType.CMSG_CERT_COUNT_PARAM,
                    0,
                    out numCertificates,
                    ref cbNumCertificates
                )
            )
                throw Marshal.GetLastPInvokeError().ToCryptographicException();
            ...
            {
                byte[] encodedCertificate = hCryptMsg.GetMsgParamAsByteArray(
                    CryptMsgParamType.CMSG_CERT_PARAM,
                    index
                );
        ...
        {
            CRYPT_OID_INFO info = Interop.Crypt32.FindOidInfo(
            ...
            switch (certId.dwIdChoice)
            {
                case CertIdChoice.CERT_ID_ISSUER_SERIAL_NUMBER:
                {
                    ...
                    unsafe
                    {
                        ...
                        int nc = Interop.Crypt32.CertNameToStr(
                            (int)MsgEncodingType.All,
                            dataBlobPtr,
                            dwStrType,
                            null,
                            0
                        ...
                        {
                            nc = Interop.Crypt32.CertNameToStr(
                                (int)MsgEncodingType.All,
                                dataBlobPtr,
                                dwStrType,
                                namePtr,
        ...
        public static SubjectIdentifierOrKey ToSubjectIdentifierOrKey(this CERT_ID certId)
        {
            ...
            switch (subjectIdentifierType)
            {
                ...
                default:
                    Debug.Fail(
                        "Only the framework can construct SubjectIdentifier's so if we got a bad value here, that's our fault."
                    );
                    throw new CryptographicException(
                        SR.Format(
                            SR.Cryptography_Cms_Invalid_Subject_Identifier_Type,
        ...
        )
        {
            ...
            AlgorithmIdentifier algorithmId = new AlgorithmIdentifier(
                Oid.FromOidValue(oidValue, OidGroup.PublicKeyAlgorithm),
                keyLength
            );
        ...
        public static AlgorithmIdentifier ToAlgorithmIdentifier(
            this CRYPT_ALGORITHM_IDENTIFIER cryptAlgorithmIdentifier
        )
        {
            ...
            switch (algId)
            {
                        ...
                        unsafe
                        {
                            ...
                            if (
                                !Interop.Crypt32.CryptDecodeObject(
                                    ...
                                    cryptAlgorithmIdentifier.Parameters.pbData,
                                    (int)(cryptAlgorithmIdentifier.Parameters.cbData),
                                    &rc2Parameters,
                                    ref cbSize
                                )
                            )
                ...
                case AlgId.CALG_RC4:
                {
                    ...
                    {
                        using (
                            SafeHandle sh = Interop.Crypt32.CryptDecodeObjectToMemory(
            ...
            AlgorithmIdentifier algorithmIdentifier = new AlgorithmIdentifier(
                Oid.FromOidValue(oidValue, OidGroup.All),
                keyLength
            );
            switch (oidValue)
            {
                case Oids.RsaOaep:
                    algorithmIdentifier.Parameters =
                        cryptAlgorithmIdentifier.Parameters.ToByteArray();
                    break;
            }
        ...
        public static CspParameters GetProvParameters(this SafeProvOrNCryptKeyHandle handle)
        {
            ...
            if (
                !Interop.Advapi32.CryptGetProvParam(
                    handle,
                    CryptProvParam.PP_PROVTYPE,
                    stackSpan,
                    ref size
                )
            )
            ...
            string provName = GetStringProvParam(
                handle,
                CryptProvParam.PP_NAME,
                ref asciiStringBuf,
                ref rented,
                0
            );
            int maxClear = provName.Length;
            string keyName = GetStringProvParam(
                handle,
                CryptProvParam.PP_CONTAINER,
                ref asciiStringBuf,
                ref rented,
                maxClear
            );
        ...
        )
        {
            ...
            for (int i = 0; i < pCryptAttribute->cValue; i++)
            {
                ...
                AsnEncodedData attributeObject =
                    PkcsHelpers.CreateBestPkcs9AttributeObjectAvailable(oid, encodedAttribute);
