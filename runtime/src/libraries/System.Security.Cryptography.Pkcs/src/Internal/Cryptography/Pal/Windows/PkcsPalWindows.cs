    ...
    internal sealed partial class PkcsPalWindows : PkcsPal
    {
        ...
        public sealed override Oid GetEncodedMessageType(ReadOnlySpan<byte> encodedMessage)
        {
            ...
            )
            {
                ...
                {
                    CryptMsgType.CMSG_DATA => Oid.FromOidValue(
                        Oids.Pkcs7Data,
                        OidGroup.ExtensionOrAttribute
                    ),
                    CryptMsgType.CMSG_SIGNED => Oid.FromOidValue(
                        Oids.Pkcs7Signed,
                        OidGroup.ExtensionOrAttribute
                    ),
                    CryptMsgType.CMSG_ENVELOPED => Oid.FromOidValue(
                        Oids.Pkcs7Enveloped,
                        OidGroup.ExtensionOrAttribute
                    ),
                    CryptMsgType.CMSG_SIGNED_AND_ENVELOPED => Oid.FromOidValue(
                        Oids.Pkcs7SignedEnveloped,
                        OidGroup.ExtensionOrAttribute
                    ),
                    CryptMsgType.CMSG_HASHED => Oid.FromOidValue(
                        Oids.Pkcs7Hashed,
                        OidGroup.ExtensionOrAttribute
                    ),
                    CryptMsgType.CMSG_ENCRYPTED => Oid.FromOidValue(
                        Oids.Pkcs7Encrypted,
                        OidGroup.ExtensionOrAttribute
                    ),
        ...
        private static T? GetPrivateKey<T>(
            ...
            where T : AsymmetricAlgorithm
        {
            ...
            using (handle)
            {
                ...
                if (keySpec == CryptKeySpec.CERT_NCRYPT_KEY_SPEC)
                {
                    ...
                    using (
                        SafeNCryptKeyHandle keyHandle = new SafeNCryptKeyHandle(
                ...
                switch (keySpec)
                {
                    ...
                    default:
                        ...
                        throw new NotSupportedException(
                            SR.Format(SR.Cryptography_Cms_UnknownKeySpec, keySpec)
                        );
                }
