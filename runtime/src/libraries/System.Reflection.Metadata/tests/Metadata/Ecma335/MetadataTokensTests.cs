...
namespace System.Reflection.Metadata.Ecma335.Tests
{
    public class MetadataTokensTests
    {
        ...
        private static readonly EntityHandle s_assemblyRefHandle =
            AssemblyReferenceHandle.FromRowId(1);
        private static readonly EntityHandle s_virtualAssemblyRefHandle =
            AssemblyReferenceHandle.FromVirtualIndex(
                AssemblyReferenceHandle.VirtualIndex.System_Runtime
            );
        private static readonly Handle s_virtualBlobHandle = BlobHandle.FromVirtualIndex(
            BlobHandle.VirtualIndex.AttributeUsage_AllowSingle,
            0
        );
        ...
        private static readonly Handle s_winrtPrefixedStringHandle = StringHandle.FromOffset(1)
            .WithWinRTPrefix();
        ...
        private static readonly EntityHandle s_exportedTypeHandle = ExportedTypeHandle.FromRowId(
            42
        );

        // MetadataBuilder handles:
        private static readonly Handle s_writerStringHandle = StringHandle.FromWriterVirtualIndex(
            1
        );
        ...
        public void GetHeapOffset()
        {
            ...
            Assert.Equal(
                -1,
                MetadataTokens.GetHeapOffset((StringHandle)s_winrtPrefixedStringHandle)
            );
            ...
            Assert.Equal(
                1,
                MetadataTokens.GetHeapOffset((UserStringHandle)s_writerUserStringHandle)
            );
            ...
            AssertExtensions.Throws<ArgumentException>(
                "handle",
                () => MetadataTokens.GetHeapOffset(s_assemblyRefHandle)
            );
            AssertExtensions.Throws<ArgumentException>(
                "handle",
                () => MetadataTokens.GetHeapOffset(s_virtualAssemblyRefHandle)
            );
        }
        ...
        public void GetToken()
        {
            ...
            AssertExtensions.Throws<ArgumentException>(
                "handle",
                () => MetadataTokens.GetToken(s_virtualBlobHandle)
            );
            AssertExtensions.Throws<ArgumentException>(
                "handle",
                () => MetadataTokens.GetToken(s_stringHandle)
            );
            AssertExtensions.Throws<ArgumentException>(
                "handle",
                () => MetadataTokens.GetToken(s_winrtPrefixedStringHandle)
            );
            AssertExtensions.Throws<ArgumentException>(
                "handle",
                () => MetadataTokens.GetToken(s_blobHandle)
            );
            AssertExtensions.Throws<ArgumentException>(
                "handle",
                () => MetadataTokens.GetToken(s_guidHandle)
            );
        }
        ...
        public void TryGetTableIndex()
        {
            var kinds =
                ...
                let index = new Func<HandleKind, TableIndex?>(
                    k =>
                    {
                        TableIndex ti;
        ...
        public void HandleFactories_FromToken()
        {
            ...
            Assert.Equal(
                s_exportedTypeHandle,
                MetadataTokens.ExportedTypeHandle(
                    (int)(TokenTypeIds.ExportedType | s_exportedTypeHandle.RowId)
                )
            );
            ...
            AssertExtensions.Throws<ArgumentException>(
                "token",
                () => MetadataTokens.Handle(0x71000001)
            );
            AssertExtensions.Throws<ArgumentException>(
                "token",
                () => MetadataTokens.Handle(0x72000001)
            );
            AssertExtensions.Throws<ArgumentException>(
                "token",
                () => MetadataTokens.Handle(0x73000001)
            );
            AssertExtensions.Throws<ArgumentException>(
                "token",
                () => MetadataTokens.Handle(0x74000001)
            );
            AssertExtensions.Throws<ArgumentException>(
                "token",
                () => MetadataTokens.Handle(0x7a000001)
            );
            AssertExtensions.Throws<ArgumentException>(
                "token",
                () => MetadataTokens.Handle(0x7e000001)
            );
            AssertExtensions.Throws<ArgumentException>(
                "token",
                () => MetadataTokens.Handle(0x7fffffff)
            );
        }
        ...
        public void HandleFactories_FromTableIndex()
        {
            ...
            Assert.Equal(
                s_exportedTypeHandle,
                MetadataTokens.Handle(TableIndex.ExportedType, s_exportedTypeHandle.RowId)
            );

            Assert.Equal(
                s_assemblyRefHandle,
                MetadataTokens.EntityHandle(TableIndex.AssemblyRef, 1)
            );
            Assert.Equal(
                s_exportedTypeHandle,
                MetadataTokens.EntityHandle(TableIndex.ExportedType, s_exportedTypeHandle.RowId)
            );
        ...
        public void SpecificHandleFactories()
        {
            ...
            Assert.Equal(
                1,
                MetadataTokens.GetRowNumber(MetadataTokens.MethodImplementationHandle(1))
            );
            Assert.Equal(
                1,
                MetadataTokens.GetRowNumber(MetadataTokens.MethodSpecificationHandle(1))
            );
            ...
            Assert.Equal(
                1,
                MetadataTokens.GetRowNumber(MetadataTokens.InterfaceImplementationHandle(1))
            );
            ...
            Assert.Equal(
                1,
                MetadataTokens.GetRowNumber(MetadataTokens.PropertyDefinitionHandle(1))
            );
            Assert.Equal(
                1,
                MetadataTokens.GetRowNumber(MetadataTokens.StandaloneSignatureHandle(1))
            );
            ...
            Assert.Equal(
                1,
                MetadataTokens.GetRowNumber(MetadataTokens.GenericParameterConstraintHandle(1))
            );
            ...
            Assert.Equal(
                1,
                MetadataTokens.GetRowNumber(MetadataTokens.DeclarativeSecurityAttributeHandle(1))
            );
            ...
            Assert.Equal(
                1,
                MetadataTokens.GetRowNumber(MetadataTokens.MethodDebugInformationHandle(1))
            );
            ...
            Assert.Equal(
                1,
                MetadataTokens.GetRowNumber(MetadataTokens.CustomDebugInformationHandle(1))
            );
            ...
            Assert.Equal(
                1,
                MetadataTokens.GetHeapOffset((BlobHandle)MetadataTokens.DocumentNameBlobHandle(1))
            );
        }
    }
}
