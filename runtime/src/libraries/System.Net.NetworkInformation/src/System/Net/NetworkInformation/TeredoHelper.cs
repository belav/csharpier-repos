...
{
    // This class wraps the native API NotifyStableUnicastIpAddressTable.  The native function's
    // behavior is:
    //
    // 1. If the address table is already stable, it returns ERROR_SUCCESS and a Mib table handle that
    // we must free.
    //    The passed-in callback will never be called, and the cancelHandle is set to NULL.
    //
    // 2. If the address table is not stable, it returns ERROR_IO_PENDING.  The table handle is set to
    // NULL,
    //    and the cancelHandle is set to a valid handle.  The callback will be called (on a native
    // threadpool thread)
    //    EVERY TIME the address table becomes stable until CancelMibChangeNotify2 is called on the
    // cancelHandle
    //    (via cancelHandle.Dispose()).
    //
    // CancelMibChangeNotify2 guarantees that, by the time it returns, all calls to the callback will be
    // complete
    ...
    internal sealed class TeredoHelper
    {
        ...
        // Used to cancel notification after receiving the first callback, or when the AppDomain is going
        // down.
        ...
        // Returns true if the address table is already stable.  Otherwise, calls callback when it becomes
        // stable.
        ...
        // This callback gets run on a native worker thread, which we don't want to allow arbitrary user
        // code to
        // execute on (it will block AppDomain unload, for one).  Free the MibTable and delegate (exactly
        // once)
        ...
        private static void OnStabilized(IntPtr context, IntPtr table)
        {
            ...
            // Lock the TeredoHelper instance to ensure that only the first call to OnStabilized will get to
            // call
            // RunCallback.  This is the only place that TeredoHelpers get locked, as individual instances are
            // not
