...
namespace System.Net.NetworkInformation
{
    ...
    internal sealed class SystemTcpConnectionInformation : TcpConnectionInformation
    {
        ...
        internal SystemTcpConnectionInformation(in Interop.IpHlpApi.MibTcpRow row)
        {
            ...
            int remotePort = (
                (_state == TcpState.Listen) ? 0 : row.remotePort1 << 8 | row.remotePort2
            );
        ...
        internal SystemTcpConnectionInformation(in Interop.IpHlpApi.MibTcp6RowOwnerPid row)
        {
            ...
            _localEndPoint = new IPEndPoint(
                new IPAddress(row.localAddrAsSpan, row.localScopeId),
                (int)localPort
            );
            _remoteEndPoint = new IPEndPoint(
                new IPAddress(row.remoteAddrAsSpan, row.remoteScopeId),
                (int)remotePort
            );
        }
