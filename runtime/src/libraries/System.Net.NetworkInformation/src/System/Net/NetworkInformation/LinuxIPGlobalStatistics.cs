...
namespace System.Net.NetworkInformation
{
    internal sealed class LinuxIPGlobalStatistics : IPGlobalStatistics
    {
        ...
        public LinuxIPGlobalStatistics(bool ipv4)
        {
            if (ipv4)
            {
                _table = StringParsingHelpers.ParseIPv4GlobalStatisticsFromSnmpFile(
                    NetworkFiles.SnmpV4StatsFile
                );
                _numRoutes = StringParsingHelpers.ParseNumRoutesFromRouteFile(
                    NetworkFiles.Ipv4RouteFile
                );
                _numInterfaces = StringParsingHelpers.ParseNumIPInterfaces(
                    NetworkFiles.Ipv4ConfigFolder
                );
            }
            else
            {
                _table = StringParsingHelpers.ParseIPv6GlobalStatisticsFromSnmp6File(
                    NetworkFiles.SnmpV6StatsFile
                );
                _numRoutes = StringParsingHelpers.ParseNumRoutesFromRouteFile(
                    NetworkFiles.Ipv6RouteFile
                );
                _numInterfaces = StringParsingHelpers.ParseNumIPInterfaces(
                    NetworkFiles.Ipv6ConfigFolder
                );

                // /proc/sys/net/ipv6/conf/default/forwarding
                string forwardingConfigFile = Path.Combine(
                    NetworkFiles.Ipv6ConfigFolder,
                ...
                _table.DefaultTtl = StringParsingHelpers.ParseDefaultTtlFromFile(
                    NetworkFiles.SnmpV4StatsFile
                );
            }
        ...
        {
            get { return _table.DefaultTtl; }
        }

        public override bool ForwardingEnabled
        {
            get { return _table.Forwarding; }
        }
