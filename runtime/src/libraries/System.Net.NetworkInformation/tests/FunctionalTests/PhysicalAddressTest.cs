...
namespace System.Net.NetworkInformation.Tests
{
    public class PhysicalAddressTest
    {
        ...
        public static IEnumerable<object[]> RoundtripParseToString_String_Bytes()
        {
            ...
            {
                "47-FB-74-41-3B",
                new byte[] { 0x47, 0xfb, 0x74, 0x41, 0x3B },
            };
            yield return new object[]
            {
                "00-11-22-33-44-55",
            ...
            {
                "F0-E1-D2-C3-B4-A5",
                new byte[] { 0xf0, 0xe1, 0xd2, 0xc3, 0xb4, 0xa5 },
            };
            yield return new object[]
            {
                "f0-e1-d2-c3-b4-a5",
            ...
            {
                "54-0C-C4-7E-66-54",
                new byte[] { 0x54, 0x0c, 0xc4, 0x7e, 0x66, 0x54 },
            };
            yield return new object[]
            {
                "001122334455",
        ...
        {
            Assert.True(
                PhysicalAddress.TryParse(address.AsSpan(), out PhysicalAddress parsedAddress)
            );
            byte[] addressBytes = parsedAddress.GetAddressBytes();
            Assert.Equal(expectedBytes, addressBytes);
        }
        ...
        public void TryParseSpan_Empty_Success()
        {
            Assert.True(
                PhysicalAddress.TryParse(
                    ...
                    out PhysicalAddress parsedAddress
                )
            );
            byte[] addressBytes = parsedAddress.GetAddressBytes();
            Assert.Empty(addressBytes);
        ...
        {
            FormatException ex = Assert.Throws<FormatException>(
                () => PhysicalAddress.Parse(address.AsSpan())
            );
            Assert.Contains(address, ex.Message);
        }
        ...
        {
            Assert.Equal(
                expectedAddress.Replace("-", "").Replace(":", "").Replace(".", "").ToUpper(),
                new PhysicalAddress(inputBytes).ToString()
            );
        }
        ...
        )
        {
            ...
            Assert.Equal(
                inputBytes,
                PhysicalAddress.Parse(new PhysicalAddress(inputBytes).ToString()).GetAddressBytes()
            );
        }
    }
}
