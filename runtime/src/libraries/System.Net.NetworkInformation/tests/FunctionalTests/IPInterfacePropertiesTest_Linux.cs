            ...
            await Task.Run(
                    () =>
                    {
                        foreach (NetworkInterface nic in NetworkInterface.GetAllNetworkInterfaces())
                        {
                            ...
                            _log.WriteLine(
                                "- Supports IPv4: " + nic.Supports(NetworkInterfaceComponent.IPv4)
                            );
                            _log.WriteLine(
                                "- Supports IPv6: " + nic.Supports(NetworkInterfaceComponent.IPv6)
                            );
                            ...
                            _log.WriteLine(
                                "- Gateway Addresses: " + ipProperties.GatewayAddresses.Count
                            );
                            foreach (
                                GatewayIPAddressInformation gateway in ipProperties.GatewayAddresses
                            )
                            {
                            ...
                            _log.WriteLine(
                                "- Multicast Addresses: " + ipProperties.MulticastAddresses.Count
                            );
                            ...
                            _log.WriteLine(
                                "- Unicast Addresses: " + ipProperties.UnicastAddresses.Count
                            );
                            foreach (
                                UnicastIPAddressInformation uni in ipProperties.UnicastAddresses
                            )
                            {
                            ...
                            _log.WriteLine(
                                "- Wins Addresses: " + ipProperties.WinsServersAddresses.Count
                            );
            ...
            await Task.Run(
                    () =>
                    {
                        ...
                        foreach (NetworkInterface nic in NetworkInterface.GetAllNetworkInterfaces())
                        {
                            ...
                            {
                                _log.WriteLine(
                                    "-- Level: " + level + "; " + ipv6Properties.GetScopeId(level)
                                );
                            }
                        }
                    }
        ...
        public async Task IPInfoTest_IPv4Loopback_ProperAddress()
        {
            await Task.Run(
                    () =>
                    {
                        ...
                        _log.WriteLine(
                            "Loopback IPv4 index: " + NetworkInterface.LoopbackInterfaceIndex
                        );

                        NetworkInterface loopback = NetworkInterface
        ...
        public async Task IPInfoTest_IPv6Loopback_ProperAddress()
        {
            await Task.Run(
                    () =>
                    {
                        ...
                        _log.WriteLine(
                            "Loopback IPv6 index: " + NetworkInterface.IPv6LoopbackInterfaceIndex
                        );
