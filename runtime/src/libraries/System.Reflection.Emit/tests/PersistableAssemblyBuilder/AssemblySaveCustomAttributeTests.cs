...
namespace System.Reflection.Emit.Tests
{
    ...
    public class AssemblySaveCustomAttributeTests
    {
        ...
        private static readonly (ConstructorInfo con, object[] args) s_comVisiblePair = (
            s_comVisibleType.GetConstructor(new Type[] { typeof(bool) }),
            new object[] { true }
        );
        private static readonly (ConstructorInfo con, object[] args) s_guidPair = (
            s_guidType.GetConstructor(new Type[] { typeof(string) }),
            new object[] { "9ED54F84-A89D-4fcd-A854-44251E925F09" }
        );
        ...
        public void MethodFieldWithCustomAttributes()
        {
            ...
            using (TempFile file = TempFile.Create())
            {
                ...
                for (int i = 0; i < types.Length; i++)
                {
                    ...
                    MethodInfo[] methodsFromDisk = typeFromDisk.IsValueType
                        ? typeFromDisk.GetMethods(BindingFlags.DeclaredOnly)
                        : typeFromDisk.GetMethods();
        ...
        private void ValidateAttributes(IList<CustomAttributeData> attributesFromDisk)
        {
            ...
            foreach (var attribute in attributesFromDisk)
            {
                ...
                {
                    Assert.Equal(
                        s_comVisiblePair.con.MetadataToken,
                        attribute.Constructor.MetadataToken
                    );
                    Assert.Equal(
                        s_comVisiblePair.args[0].GetType().FullName,
                        attribute.ConstructorArguments[0].ArgumentType.FullName
                    );
                ...
                else
                {
                    ...
                    Assert.Equal(
                        s_guidPair.args[0].GetType().FullName,
                        attribute.ConstructorArguments[0].ArgumentType.FullName
                    );
        ...
        private static void WriteAssemblyToDisk(
            AssemblyName assemblyName,
            Type[] types,
            string fileLocation,
            List<CustomAttributeBuilder>? assemblyAttributes = null,
            List<CustomAttributeBuilder>? moduleAttributes = null,
            List<CustomAttributeBuilder>? typeAttributes = null,
            List<CustomAttributeBuilder>? methodAttributes = null,
            List<CustomAttributeBuilder>? fieldAttributes = null
        )
        {
        ...
        private static void PopulateMembersForModule(
            ModuleBuilder mb,
            Type[] types,
            List<CustomAttributeBuilder>? moduleAttributes,
            List<CustomAttributeBuilder>? typeAttributes,
            List<CustomAttributeBuilder>? methodAttributes,
            List<CustomAttributeBuilder>? fieldAttributes
        )
        {
        ...
        private static void DefineFieldsAndSetAttributes(
            List<CustomAttributeBuilder>? fieldAttributes,
            FieldInfo[] fields,
            TypeBuilder tb
        )
        ...
        private static void DefineMethodsAndSetAttributes(
            List<CustomAttributeBuilder> methodAttributes,
            TypeBuilder tb,
            MethodInfo[] methods,
            List<CustomAttributeBuilder> paramAttributes
        )
        {
            foreach (var method in methods)
            {
                ...
                {
                    ParameterBuilder pb = meb.DefineParameter(
                        param.Position + 1,
                        param.Attributes,
                        param.Name
                    );
        ...
        public void CreateStructWithPseudoCustomAttributesTest()
        {
            using (TempFile file = TempFile.Create())
            {
                ...
                CustomAttributeBuilder[] fieldAttributes = new[]
                {
                    ...
                    new CustomAttributeBuilder(
                        typeof(FieldOffsetAttribute).GetConstructor(new Type[] { typeof(int) }),
                        new object[] { 2 }
                    ),
                    new CustomAttributeBuilder(s_guidPair.con, s_guidPair.args),
                    new CustomAttributeBuilder(
                ...
                TypeBuilder tb = ab.DefineDynamicModule("Module")
                    .DefineType(type.FullName, type.Attributes, type.BaseType);
                ...
                for (int i = 0; i < attributesFromDisk.Count; i++)
                {
                    switch (attributesFromDisk[i].AttributeType.Name)
                    {
                        case "GuidAttribute":
                            Assert.Equal(
                                s_guidPair.args[0],
                                attributesFromDisk[i].ConstructorArguments[0].Value
                            );
                            break;
                        default:
                            Assert.Fail(
                                $"Not expected attribute : {attributesFromDisk[i].AttributeType.Name}"
                            );
                            break;
                    }
                }
                ...
                IList<CustomAttributeData> fieldAttributesFromDisk =
                ...
                for (int i = 0; i < fieldAttributesFromDisk.Count; i++)
                {
                    switch (fieldAttributesFromDisk[i].AttributeType.Name)
                    {
                        case "FieldOffsetAttribute":
                            Assert.Equal(
                                2,
                                fieldAttributesFromDisk[i].ConstructorArguments[0].Value
                            );
                            break;
                        ...
                        default:
                            Assert.Fail(
                                $"Not expected attribute : {fieldAttributesFromDisk[i].AttributeType.Name}"
                            );
                            break;
                    }
                }
        ...
        public void InterfacesWithPseudoCustomAttributes()
        {
            using (TempFile file = TempFile.Create())
            {
                ...
                TypeBuilder tb = ab.DefineDynamicModule("Module")
                    .DefineType(type.FullName, type.Attributes);
                typeAttributes.ForEach(tb.SetCustomAttribute);
                DefineMethodsAndSetAttributes(
                    methodAttributes,
                    tb,
                ...
                {
                    IList<CustomAttributeData> methodAttributesFromDisk =
                    ...
                    for (int i = 0; i < methodAttributesFromDisk.Count; i++)
                    {
                        switch (methodAttributesFromDisk[i].AttributeType.Name)
                        {
                            ...
                            case "GuidAttribute":
                                Assert.Equal(
                                    s_guidPair.args[0],
                                    methodAttributesFromDisk[i].ConstructorArguments[0].Value
                                );
                                break;
                                        ...
                                        switch (attribute.NamedArguments[j].MemberName)
                                        {
                                            case "CharSet":
                                                Assert.Equal(
                                                    CharSet.Ansi,
                                                    (CharSet)
                                                        attribute.NamedArguments[j].TypedValue.Value
                                                );
                                                break;
                                            case "SetLastError":
                                                Assert.True(
                            ...
                            default:
                                Assert.Fail(
                                    $"Not expected attribute : {methodAttributesFromDisk[i].AttributeType.Name}"
                                );
                                break;
                        }
                    }
                    ...
                    {
                        IList<CustomAttributeData> paramAttributes =
                            ...
                            switch (paramAttributes[i].AttributeType.Name)
                            {
                                ...
                                case "MarshalAsAttribute":
                                    Assert.Equal(
                                        ...
                                        (UnmanagedType)
                                            paramAttributes[i].ConstructorArguments[0].Value
                                    );
                                    Assert.Equal(
                                ...
                                default:
                                    Assert.Fail(
                                        $"Not expected attribute : {paramAttributes[i].AttributeType.Name}"
                                    );
                                    break;
                            }
                        }
        ...
        private static readonly ConstructorInfo marshalAsEnumCtor =
            typeof(MarshalAsAttribute).GetConstructor(new Type[] { typeof(UnmanagedType) });
        private static readonly ConstructorInfo marshalAsShortCtor =
            typeof(MarshalAsAttribute).GetConstructor(new Type[] { typeof(short) });

        public static IEnumerable<object[]> MarshalAsAttributeWithVariousFields()
        {
            yield return new object[]
            {
                new CustomAttributeBuilder(marshalAsEnumCtor, new object[] { UnmanagedType.LPStr }),
                UnmanagedType.LPStr,
            };
            yield return new object[]
            {
                new CustomAttributeBuilder(marshalAsShortCtor, new object[] { (short)21 }),
                UnmanagedType.LPWStr,
            };
            yield return new object[]
            {
                new CustomAttributeBuilder(marshalAsShortCtor, new object[] { (short)19 }),
                UnmanagedType.BStr,
            };
            yield return new object[]
            {
            ...
            yield return new object[]
            {
                new CustomAttributeBuilder(
                    ...
                    new FieldInfo[]
                    {
                        typeof(MarshalAsAttribute).GetField("MarshalType"),
                        typeof(MarshalAsAttribute).GetField("MarshalCookie"),
                    },
                    new object[] { typeof(EmptyTestClass).AssemblyQualifiedName, "MyCookie" }
                ),
                UnmanagedType.CustomMarshaler,
            };
        ...
        public void MarshalAsPseudoCustomAttributesTest(
            CustomAttributeBuilder attribute,
            UnmanagedType expectedType
        )
        {
            using (TempFile file = TempFile.Create())
            {
                ...
                Assert.Equal(
                    expectedType,
                    (UnmanagedType)attributeFromDisk.ConstructorArguments[0].Value
                );
        ...
        public void EnumBuilderSetCustomAttributesTest()
        {
            using (TempFile file = TempFile.Create())
            {
                ...
                EnumBuilder enumBuilder = ab.DefineDynamicModule("Module")
                    .DefineEnum("TestEnum", TypeAttributes.Public, typeof(int));
        ...
        private void AssertEnumAttributes(
            string fullName,
            object value,
            CustomAttributeData testAttrbiute
        )
        {
