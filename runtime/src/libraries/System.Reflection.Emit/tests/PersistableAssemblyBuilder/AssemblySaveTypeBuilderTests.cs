    ...
    public class AssemblySaveTypeBuilderTests
    {
        ...
        private static void AssertTypesAndTypeMembers(Type[] types, Type[] typesFromDisk)
        {
            ...
            for (int i = 0; i < types.Length; i++)
            {
                ...
                AssemblySaveTools.AssertMethods(
                    sourceType.GetMethods(BindingFlags.DeclaredOnly),
                    typeFromDisk.GetMethods(BindingFlags.DeclaredOnly)
                );
        ...
        public void AddInterfaceImplementationTest()
        {
            using (TempFile file = TempFile.Create())
            {
                ...
                TypeBuilder nestedType = tb.DefineNestedType(
                    "NestedType",
                    TypeAttributes.Interface | TypeAttributes.Abstract
                );
                tb.CreateType();
        ...
        public void TypeBuilder_GetMethod_ReturnsMethod()
        {
            ...
            MethodInfo createdConstructedTypeMethod = TypeBuilder.GetMethod(
                genericIntType,
                genericMethod
            );
            ...
            Assert.Equal(
                "Type: U",
                createdConstructedTypeMethod.GetGenericArguments()[0].ToString()
            );
        ...
        public void SaveMultipleGenericTypeParametersToEnsureSortingWorks()
        {
            using (TempFile file = TempFile.Create())
            {
                ...
                Type[] method11Params = m.GetTypes()[2]
                    .GetMethod("TwoParameters")
                    .GetGenericArguments();
                Type[] method12Params = m.GetTypes()[2]
                ...
                Type[] method13Params = m.GetTypes()[2]
