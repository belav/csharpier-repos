...
namespace System.Reflection.Emit
{
    internal sealed class MethodBuilderImpl : MethodBuilder
    {
        ...
        internal BlobBuilder GetMethodSignatureBlob() =>
            MetadataSignatureHelper.MethodSignatureEncoder(
                _module,
                _parameterTypes,
                ReturnType,
                GetSignatureConvention(_callingConventions),
        ...
        )
        {
            ...
            if (
                (callingConventions & CallingConventions.HasThis) != 0
        ...
        internal BindingFlags GetBindingFlags()
        {
            BindingFlags bindingFlags =
                (_attributes & MethodAttributes.MemberAccessMask) == MethodAttributes.Public
                    ? BindingFlags.Public
                    : BindingFlags.NonPublic;
            bindingFlags |=
                (_attributes & MethodAttributes.Static) != 0
                    ? BindingFlags.Static
        ...
        )
        {
            ...
            ParameterBuilderImpl parameter = new ParameterBuilderImpl(
                this,
                position,
                attributes,
                strParamName
        ...
        protected override ILGenerator GetILGeneratorCore(int size)
        {
            ...
            if (
                (_methodImplFlags & MethodImplAttributes.CodeTypeMask) != MethodImplAttributes.IL
        ...
        )
        {
            ...
            switch (con.ReflectedType!.FullName)
            {
                case "System.Runtime.CompilerServices.MethodImplAttribute":
                    int implValue = BinaryPrimitives.ReadUInt16LittleEndian(
                        binaryAttribute.Slice(2)
                    );
                    _methodImplFlags |= (MethodImplAttributes)implValue;
                    return;
                case "System.Runtime.InteropServices.DllImportAttribute":
                    {
                        _dllImportData = DllImportData.CreateDllImportData(
                            CustomAttributeInfo.DecodeCustomAttribute(con, binaryAttribute),
                            out var preserveSig
                        );
        ...
        public override ParameterInfo[] GetParameters()
        {
            ...
            for (int i = 0; i < _parameterTypes.Length; i++)
            {
                ...
                {
                    parameters[i] = new ParameterInfoWrapper(
                        new ParameterBuilderImpl(this, i, ParameterAttributes.None, null),
                        _parameterTypes[i]
                    );
                }
                else
