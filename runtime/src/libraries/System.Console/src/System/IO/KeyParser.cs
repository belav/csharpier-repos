...
internal static class KeyParser
{
    ...
    )
    {
        ...
        if (
            input[1] == 'O' || char.IsAsciiLetter(input[2]) || input.Length == MinimalSequenceLength
        )
        {
            if (
        ...
        int digitCount = !char.IsBetween(input[SequencePrefixLength], '1', '9') // not using IsAsciiDigit as 0 is invalid
            ? 0
            : char.IsDigit(input[SequencePrefixLength + 1])
                ? 2
                : 1;
        ...
        static (ConsoleKey key, ConsoleModifiers modifiers) MapSCO(char character) =>
            character switch
            {
                ...
                _ when char.IsBetween(character, 'Y', 'Z')
                    => (ConsoleKey.F1 + character - 'Y', ConsoleModifiers.Shift),
                _ when char.IsBetween(character, 'a', 'j')
                    => (ConsoleKey.F3 + character - 'a', ConsoleModifiers.Shift),
                _ when char.IsBetween(character, 'k', 'v')
                    => (ConsoleKey.F1 + character - 'k', ConsoleModifiers.Control),
    ...
    private static ConsoleKeyInfo ParseFromSingleChar(char single, bool isAlt)
    {
        ...
        ConsoleKey key = single switch
        {
            ...
            _ when char.IsBetween(single, (char)1, (char)26)
                => ControlAndLetterPressed(single, isAlt, out keyChar, out isCtrl),
            _ when char.IsBetween(single, (char)28, (char)31)
