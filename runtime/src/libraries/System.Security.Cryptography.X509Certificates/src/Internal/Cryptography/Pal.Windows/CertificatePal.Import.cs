...
namespace Internal.Cryptography.Pal
{
    internal sealed partial class CertificatePal : IDisposable, ICertificatePal
    {
        ...
        private static ICertificatePal FromBlobOrFile(
            ReadOnlySpan<byte> rawData,
            string? fileName,
            SafePasswordHandle password,
            X509KeyStorageFlags keyStorageFlags
        )
        {
            ...
            Interop.Crypt32.PfxCertStoreFlags pfxCertStoreFlags = MapKeyStorageFlags(
                keyStorageFlags
            );
            ...
            try
            {
                unsafe
                {
                    fixed (byte* pRawData = rawData)
                    {
                        fixed (char* pFileName = fileName)
                        {
                            Interop.Crypt32.DATA_BLOB certBlob = new Interop.Crypt32.DATA_BLOB(
                                new IntPtr(pRawData),
                                (uint)(loadFromFile ? 0 : rawData.Length)
                            );

                            Interop.Crypt32.CertQueryObjectType objectType = loadFromFile
                    ...
                    if (
                        contentType == Interop.Crypt32.ContentType.CERT_QUERY_CONTENT_PKCS7_SIGNED
                        || contentType
                            == Interop.Crypt32.ContentType.CERT_QUERY_CONTENT_PKCS7_SIGNED_EMBED
                    )
                    {
