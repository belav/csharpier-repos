...
namespace Internal.Cryptography.Pal
{
    internal sealed partial class StorePal : IDisposable, IStorePal, IExportPal, ILoaderPal
    {
                ...
                fixed (byte* pRawData = rawData)
                {
                    fixed (char* pFileName = fileName)
                    {
                        Interop.Crypt32.DATA_BLOB blob = new Interop.Crypt32.DATA_BLOB(
                            new IntPtr(pRawData),
                            (uint)(fromFile ? 0 : rawData!.Length)
                        );
                        bool persistKeySet = (
                            0 != (keyStorageFlags & X509KeyStorageFlags.PersistKeySet)
                        );
                        Interop.Crypt32.PfxCertStoreFlags certStoreFlags = MapKeyStorageFlags(
                            keyStorageFlags
                        );
                        ...
                        if (contentType == Interop.Crypt32.ContentType.CERT_QUERY_CONTENT_PFX)
                        {
                            ...
                            {
                                Interop.Crypt32.DATA_BLOB blob2 = new Interop.Crypt32.DATA_BLOB(
        ...
        public static IExportPal FromCertificate(ICertificatePalCore cert)
        {
            ...
            SafeCertStoreHandle certStore = Interop.crypt32.CertOpenStore(
                ...
                Interop.Crypt32.CertStoreFlags.CERT_STORE_ENUM_ARCHIVED_FLAG
                    ...
                    | Interop.Crypt32.CertStoreFlags.CERT_STORE_DEFER_CLOSE_UNTIL_LAST_FREE_FLAG,
                null
            );
        ...
        {
            Interop.Crypt32.CertStoreFlags certStoreFlags = MapX509StoreFlags(
                storeLocation,
                openFlags
            );
        ...
        )
        {
            ...
            if (
                ...
                dwFlags |=
                    Interop.Crypt32.PfxCertStoreFlags.PKCS12_NO_PERSIST_KEY
        ...
        private static Interop.Crypt32.CertStoreFlags MapX509StoreFlags(
            StoreLocation storeLocation,
            OpenFlags flags
        )
        {
