...
namespace Internal.Cryptography.Pal
{
    ...
    {
        AsymmetricAlgorithm DecodePublicKey(
            Oid oid,
        ...
        string X500DistinguishedNameDecode(
            byte[] encodedDistinguishedName,
            X500DistinguishedNameFlags flag
        );
        byte[] X500DistinguishedNameEncode(
        ...
        byte[] EncodeX509BasicConstraints2Extension(
            bool certificateAuthority,
            bool hasPathLengthConstraint,
            int pathLengthConstraint
        );
        void DecodeX509BasicConstraintsExtension(
            byte[] encoded,
        ...
        void DecodeX509SubjectKeyIdentifierExtension(
            byte[] encoded,
