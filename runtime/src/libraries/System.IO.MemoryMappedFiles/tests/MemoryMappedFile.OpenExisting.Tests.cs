    ...
    public class MemoryMappedFileTests_OpenExisting : MemoryMappedFilesTestBase
    {
        ...
        )
        {
            ...
            using (MemoryMappedFile original = MemoryMappedFile.CreateNew(mapName, Capacity))
            {
                ...
                using (
                    MemoryMappedFile opened = MemoryMappedFile.OpenExisting(
                        mapName,
                        desiredAccessRights | MemoryMappedFileRights.ReadPermissions
                    )
                ) // can't do anything if we don't have read permissions
                {
        ...
        [Theory]
        [MemberData(nameof(MemberData_OpenCreated))]
        public void OpenCreatedFromFile(
            string name,
        ...
        )
        {
            ...
            using (
                MemoryMappedFile original = MemoryMappedFile.CreateFromFile(
                    fs,
                    name,
                    Capacity,
        ...
        {
            if (
                (rights & MemoryMappedFileRights.ReadWriteExecute)
