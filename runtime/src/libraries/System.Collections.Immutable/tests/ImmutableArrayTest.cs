...
#pragma warning restore CS1718 // Comparison made to same variable
        ...
        public void AddRange(IEnumerable<int> source, IEnumerable<int> items)
        {
            Assert.All(
                ...
                it =>
                {
                    ...
                    Assert.Equal(
                        source.Concat(items),
                        array.AddRange(new ReadOnlySpan<int>(itArray))
        ...
        public void AddRangeInvalid(IEnumerable<int> source)
        {
            ...
            TestExtensionsMethods.ValidateDefaultThisBehavior(() =>
                s_emptyDefault.AddRange(source)); // Enumerable overload
            TestExtensionsMethods.ValidateDefaultThisBehavior(() =>
                s_emptyDefault.AddRange(source.ToImmutableArray())); // ImmutableArray overload
            TestExtensionsMethods.ValidateDefaultThisBehavior(() =>
                source.ToImmutableArray().AddRange(s_emptyDefault)); // ImmutableArray overload

            int[] sourceArray = source.ToArray();
            TestExtensionsMethods.ValidateDefaultThisBehavior(() =>
                s_emptyDefault.AddRange(sourceArray)); // Array overload
            TestExtensionsMethods.ValidateDefaultThisBehavior(() =>
                s_emptyDefault.AddRange(new ReadOnlySpan<int>(sourceArray))); // ReadOnlySpan overload

            Assert.Throws<InvalidOperationException>(() =>
                source.ToImmutableArray().AddRange((IEnumerable<int>)s_emptyDefault)); // Enumerable overload
        ...
        public void InsertRangeInvalid(IEnumerable<int> source)
        {
            ...
            AssertExtensions.Throws<ArgumentOutOfRangeException, int>(
                "index",
                span,
                s => immutableArray.InsertRange(immutableArray.Length + 1, s)
            );
            AssertExtensions.Throws<ArgumentOutOfRangeException, int>(
                "index",
                span,
                s => immutableArray.InsertRange(-1, s)
            );
        }
        ...
        public void InsertRangeDefaultInvalid(IEnumerable<int> items)
        {
            ...
            TestExtensionsMethods.ValidateDefaultThisBehavior(
                span,
                s => s_emptyDefault.InsertRange(1, s)
            );
            TestExtensionsMethods.ValidateDefaultThisBehavior(
                span,
                s => s_emptyDefault.InsertRange(-1, s)
            ...
            TestExtensionsMethods.ValidateDefaultThisBehavior(
                span,
                s => s_emptyDefault.InsertRange(0, s)
            );
        ...
        public void RemoveRangeEnumerableInvalid(IEnumerable<int> source)
        {
            ...
            Assert.All(
                ...
                comparer =>
                {
                    ...
                    TestExtensionsMethods.ValidateDefaultThisBehavior(
                        span,
                        s => s_emptyDefault.RemoveRange(items: s, comparer)
                    );
                }
            );
        }
        ...
        )
        {
            ...
            if (sourceIndex == 0 && length == array.Length)
            {
                ...
                CopyAndInvoke(
                    ...
                    {
                        array.CopyTo(
                            new Span<int>(destinationArray, destinationIndex, array.Length)
                        );
        ...
        {
            Assert.All(
                SharedComparers<int>().OfType<IEqualityComparer>(),
                comparer =>
                {
