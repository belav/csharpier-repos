...
namespace IntrinsicsInSystemPrivateCoreLib
{
    ...
    public class IntrinsicsInSystemPrivateCoreLibAnalyzer : DiagnosticAnalyzer
    {
        ...
        private const string MessageFormat =
            "Intrinsics from class '{0}' used without the protection of an explicit if statement checking the correct IsSupported flag or CompExactlyDependsOn";
        private const string Description =
            "ReadyToRun Intrinsic Safety For System.Private.CoreLib.";
        ...
        private const string MessageHelperFormat =
            "Helper '{0}' used without the protection of an explicit if statement checking the correct IsSupported flag or CompExactlyDependsOn";
        ...
        public const string DiagnosticIdConditionParsing =
            "IntrinsicsInSystemPrivateCoreLibConditionParsing";
        private const string MessageNonParseableConditionFormat =
            "Unable to parse condition to determine if intrinsics are correctly used";
        private static readonly DiagnosticDescriptor RuleCantParse = new DiagnosticDescriptor(
            DiagnosticIdConditionParsing,
            Title,
            MessageNonParseableConditionFormat,
            Category,
            DiagnosticSeverity.Error,
            isEnabledByDefault: true,
            description: Description
        ...
        public const string DiagnosticIdAttributeNotSpecificEnough =
            "IntrinsicsInSystemPrivateCoreLibAttributeNotSpecificEnough";
        private const string MessageAttributeNotSpecificEnoughFormat =
            "CompExactlyDependsOn({0}) attribute found which relates to this IsSupported check, but is not specific enough. Suppress this error if this function has an appropriate if condition so that if the meaning of the function is invariant regardless of the result of the call to IsSupported.";
        private static readonly DiagnosticDescriptor RuleAttributeNotSpecificEnough =
            new DiagnosticDescriptor(
                DiagnosticIdAttributeNotSpecificEnough,
                Title,
                MessageAttributeNotSpecificEnoughFormat,
                Category,
                DiagnosticSeverity.Error,
                isEnabledByDefault: true,
                description: Description
        ...
        {
            public IntrinsicsAnalyzerOnLoadData(
                HashSet<INamedTypeSymbol> namedTypesToBeProtected,
        ...
        public override void Initialize(AnalysisContext context)
        {
            ...
            {
                HashSet<INamedTypeSymbol> namedTypesToBeProtected = new HashSet<INamedTypeSymbol>(
                    SymbolEqualityComparer.Default
                );
                INamespaceSymbol systemRuntimeIntrinsicsNamespace = GetNamespace(
                ...
                IntrinsicsAnalyzerOnLoadData onLoadData = new IntrinsicsAnalyzerOnLoadData(
                ...
                foreach (
                    var architectureSpecificNamespace in systemRuntimeIntrinsicsNamespace.GetNamespaceMembers()
                )
                {
        ...
        private static INamedTypeSymbol? GetIsSupportedTypeSymbol(
            SemanticModel model,
            MemberAccessExpressionSyntax memberAccessExpression
        )
        {
            if (
                memberAccessExpression.Name is IdentifierNameSyntax identifierName
                && identifierName.Identifier.Text == "IsSupported"
            )
            {
        ...
        private static INamedTypeSymbol[] GatherAndConditions(
            SemanticModel model,
            ExpressionSyntax expressionToDecompose
        )
        {
            ...
            {
                if (
                    binaryExpression.OperatorToken is SyntaxToken operatorToken
                ...
                )
                {
                    ...
                    if (arrayLen != 0)
                    {
                        var retVal = new INamedTypeSymbol[
                            decomposedLeft.Length + decomposedRight.Length
                        ];
                        ...
                        Array.Copy(
                            ...
                            0,
                            retVal,
                            decomposedLeft.Length,
                            decomposedRight.Length
                        );
                        return retVal;
                    }
                    else
        ...
        private static INamedTypeSymbol[][] DecomposePropertySymbolForIsSupportedGroups_Property(
        ...
        )
        {
            ...
            {
                if (
                    propertyDefiningSyntax is PropertyDeclarationSyntax propertyDeclaration
                    && propertyDeclaration.ExpressionBody
                        is ArrowExpressionClauseSyntax arrowExpression
                )
                {
                    return DecomposeConditionForIsSupportedGroups(
                        context,
                        model,
        ...
        private static INamedTypeSymbol[][] DecomposeConditionForIsSupportedGroups(
            OperationAnalysisContext context,
            SemanticModel model,
            ExpressionSyntax expressionToDecompose
        )
        {
            ...
            if (
                expressionToDecompose is MemberAccessExpressionSyntax
                || expressionToDecompose is IdentifierNameSyntax
            )
            {
                return DecomposePropertySymbolForIsSupportedGroups_Property(
                    context,
                    model,
            ...
            {
                var decomposedLeft = DecomposeConditionForIsSupportedGroups(
                    context,
                    model,
                    binaryExpression.Left
                );
                var decomposedRight = DecomposeConditionForIsSupportedGroups(
                    context,
                    model,
                ...
                )
                {
                    ...
                    if ((decomposedLeft.Length > 1) || (decomposedRight.Length > 1))
                    {
                        context.ReportDiagnostic(
                            Diagnostic.Create(RuleCantParse, expressionToDecompose.GetLocation())
                        );
                    }

                    return new INamedTypeSymbol[][]
                    {
                        GatherAndConditions(model, binaryExpression),
                    };
                }
                else if (
                    binaryExpression.OperatorToken is SyntaxToken operatorToken2
            ...
            {
                var decomposedOperand = DecomposeConditionForIsSupportedGroups(
                    context,
                    model,
                ...
                expressionToDecompose is ConditionalExpressionSyntax conditionalExpressionSyntax
            )
            {
                var decomposedTrue = DecomposeConditionForIsSupportedGroups(
                    context,
                    model,
                    conditionalExpressionSyntax.WhenTrue
                );
                var decomposedFalse = DecomposeConditionForIsSupportedGroups(
                    context,
                    model,
        ...
        private static IEnumerable<INamedTypeSymbol> GetCompExactlyDependsOnUseList(
            ISymbol symbol,
            IntrinsicsAnalyzerOnLoadData onLoadData
        )
        {
                ...
                foreach (var attributeData in symbol.GetAttributes())
                {
                    ...
                    )
                    {
                        if (
                            ...
                            is INamedTypeSymbol attributeTypeSymbol
                        )
                        {
        ...
        private static bool ConditionAllowsSymbol(
            ISymbol symbolOfInvokeTarget,
            INamedTypeSymbol namedTypeThatIsSafeToUse,
            IntrinsicsAnalyzerOnLoadData onLoadData
        )
        {
            HashSet<INamedTypeSymbol> examinedSymbols = new HashSet<INamedTypeSymbol>(
                SymbolEqualityComparer.Default
            );
            ...
            while (symbolsToExamine.Count > 0)
            {
                ...
                foreach (
                    var helperForType in GetCompExactlyDependsOnUseList(
                ...
                if (
                    ...
                    && !examinedSymbols.Contains(symbol.ContainingType)
                )
        ...
        private static bool TypeSymbolAllowsTypeSymbol(
            INamedTypeSymbol namedTypeToCheckForSupport,
            INamedTypeSymbol namedTypeThatProvidesSupport
        )
        {
            HashSet<INamedTypeSymbol> examinedSymbols = new HashSet<INamedTypeSymbol>(
                SymbolEqualityComparer.Default
        ...
        )
        {
            ...
            if (
                symbol == null
                || symbol is ITypeSymbol type && type.SpecialType != SpecialType.None
            )
            {
                return;
            }
...
#pragma warning restore RS1024
            ...
            while (operationSearch != null)
            {
                ...
                if (operationSearch.Kind == OperationKind.LocalFunction)
                {
                    ...
                    ILocalFunctionOperation localFunctionOperation =
                        (ILocalFunctionOperation)operationSearch;
            ...
            if (symbol is IPropertySymbol propertySymbol)
            {
                if (propertySymbol.Name == "IsSupported")
                {
                    ...
                    if (
                        (compExactlyDependsOn != null)
                        && symbolThatMightHaveCompExactlyDependsOnAttribute != null
                    )
                    {
                        foreach (
                            var attributeData in symbolThatMightHaveCompExactlyDependsOnAttribute.GetAttributes()
                        )
                        {
                            if (
                                attributeData.AttributeClass.Equals(
                            ...
                            )
                            {
                                ...
                                {
                                    var namespaceAttributeTypeSymbol = SymbolToNamespaceSymbol(
                                        attributeTypeSymbol
                                    );
                                    var namespaceSymbol = SymbolToNamespaceSymbol(symbol);
                                    if (
                                        (namespaceAttributeTypeSymbol != null)
                                        && (namespaceSymbol != null)
                                    )
                                    {
                                        if (
                                            namespaceAttributeTypeSymbol.Equals(
                                                namespaceSymbol,
                                                SymbolEqualityComparer.Default
                                            )
                                        )
                                        {
                                            ...
                                            {
                                                attributeExplicitlyAllowsExactSymbol =
                                                    attributeTypeSymbol;
                                            }
                                            else
                                            {
                                                attributeExplicitlyAllowsRelatedSymbol =
                                                    attributeTypeSymbol;
                                            }
                                        }
                                    }
                    ...
                    if (
                        (attributeExplicitlyAllowsRelatedSymbol != null)
                        && (attributeExplicitlyAllowsExactSymbol == null)
                    )
                    {
                        context.ReportDiagnostic(
                            Diagnostic.Create(
                                ...
                                attributeExplicitlyAllowsRelatedSymbol.ToDisplayString()
                            )
                        );
                    }
            ...
            {
                foreach (
                    var attributeTypeSymbol in GetCompExactlyDependsOnUseList(
                        symbolThatMightHaveCompExactlyDependsOnAttribute,
                        onLoadData
                    )
                )
            ...
            HashSet<INamedTypeSymbol> notTypes = new HashSet<INamedTypeSymbol>(
                SymbolEqualityComparer.Default
            );
            ...
            foreach (var ancestorNode in ancestorNodes)
            {
                if (previousNode != null)
                {
                    ...
                    bool HandleConditionalCase(
                        ExpressionSyntax condition,
                        SyntaxNode? syntaxOnPositiveCondition,
                        SyntaxNode? syntaxOnNegativeCondition
                    )
                    {
                        if (previousNode == syntaxOnPositiveCondition)
                        {
                            var decomposedCondition = DecomposeConditionForIsSupportedGroups(
                                context,
                            ...
                            HashSet<ISymbol> foundSymbols = new HashSet<ISymbol>(
                                SymbolEqualityComparer.Default
                            );
                            ...
                            for (
                                int andClauseIndex = 0;
                                andClauseIndex < decomposedCondition.Length;
                                andClauseIndex++
                            )
                            {
                                bool foundMatchInAndClause = false;
                                foreach (
                                    var symbolInAndClause in decomposedCondition[andClauseIndex]
                                )
                                {
