...
namespace System
{
    internal static class DateTimeParse
    {
        ...
        internal static bool TryParseExact(
            ReadOnlySpan<char> s,
            ReadOnlySpan<char> format,
            DateTimeFormatInfo dtfi,
            DateTimeStyles style,
        ...
        internal static bool TryParseExactMultiple(
            ReadOnlySpan<char> s,
            string?[]? formats,
            DateTimeFormatInfo dtfi,
            DateTimeStyles style,
        ...
        private static bool MatchWord(ref __DTString str, string target)
        {
            ...
            if (
                str.CompareInfo.Compare(
                    str.Value.Slice(str.Index, target.Length),
                    target,
        ...
        **      FormatException if invalid timezone format is found.
        ...
        //
        private static bool Lex(
            DS dps,
            ref __DTString str,
            scoped ref DateTimeToken dtok,
            scoped ref DateTimeRawInfo raw,
            scoped ref DateTimeResult result,
            scoped ref DateTimeFormatInfo dtfi,
        ...
        )
        {
            ...
            {
                Trace(
                    $"Lex({Hex(str.Value)})\tpos:{str.Index}({Hex(str.m_current)}), {tokenType}, DS.{dps}"
                );
            }
#endif // _LOGGING
            ...
            switch (tokenType)
            {
                ...
                case TokenType.YearNumberToken:
                    ...
                    if (dps == DS.T_NNt)
                    {
                        if (str.Index < str.Length - 1)
                        {
                            char nextCh = str.Value[str.Index];
                            if (
                                (nextCh == TimeFractionDelimiterDot)
                    ...
                    if (dps == DS.T_NNt || dps == DS.T_Nt)
                    {
                        if (str.Index < str.Length - 1)
                        {
                            ...
                            {
                                LexTraceExit(
                                    "0020 (value like \"12:01\" or \"12:\" followed by a non-TZ number",
                                    dps
                                );
                                return false;
                            }
                            ...
                            )
                            {
                                ...
                                case TokenType.SEP_Pm:
                                    ...
                                    else
                                    {
                                        result.SetBadDateTimeFailure();
                                        LexTraceExit(
                                            "0030 (TM.AM/TM.PM Happened more than 1x)",
                                            dps
                                        );
                                    }
                                    break;
                                ...
                                case TokenType.SEP_DateOrOffset:
                                    ...
                                    if (
                                        (
                                            s_dateParsingStates[(int)dps][(int)DTT.YearDateSep]
                                            == DS.ERROR
                                        )
                                        && (
                    ...
                    if (raw.month == -1)
                    {
                        ...
                        )
                        {
                            ...
                            case TokenType.SEP_DateOrOffset:
                                ...
                                if (
                                    (
                                        s_dateParsingStates[(int)dps][(int)DTT.MonthDatesep]
                                        == DS.ERROR
                                    )
                                    && (
                ...
                case TokenType.JapaneseEraToken:
                    ...
                    else
                    {
                        result.SetBadDateTimeFailure();
                        LexTraceExit(
                            "0160 (JapaneseEraToken seen when result.era already set)",
                            dps
                        );
                        return false;
                    }
        ...
        private static void GetDefaultYear(
            ref DateTimeResult result,
            scoped ref DateTimeStyles styles
        )
        {
        ...
        private static bool GetDayOfNN(
            ref DateTimeResult result,
            scoped ref DateTimeStyles styles,
            scoped ref DateTimeRawInfo raw,
            DateTimeFormatInfo dtfi
        ...
        private static bool GetDayOfNNN(
            ref DateTimeResult result,
            scoped ref DateTimeRawInfo raw,
            DateTimeFormatInfo dtfi
        )
        ...
        private static bool GetDayOfMN(
            ref DateTimeResult result,
            scoped ref DateTimeStyles styles,
            scoped ref DateTimeRawInfo raw,
            DateTimeFormatInfo dtfi
        )
        {
            ...
            if (monthDayOrder == ORDER_DM)
            {
                ...
                {
                    result.SetFailure(
                        ParseFailureKind.Format_BadDatePattern,
        ...
        private static bool GetHebrewDayOfNM(
            ref DateTimeResult result,
            scoped ref DateTimeRawInfo raw,
            DateTimeFormatInfo dtfi
        )
        ...
        private static bool GetDayOfNM(
            ref DateTimeResult result,
            scoped ref DateTimeStyles styles,
            scoped ref DateTimeRawInfo raw,
            DateTimeFormatInfo dtfi
        ...
        private static bool GetDayOfYNN(
            ref DateTimeResult result,
            scoped ref DateTimeRawInfo raw,
            DateTimeFormatInfo dtfi
        )
        ...
        private static bool GetDayOfNNY(
            ref DateTimeResult result,
            scoped ref DateTimeRawInfo raw,
            DateTimeFormatInfo dtfi
        )
        ...
        private static bool GetDateOfNNDS(
            ref DateTimeResult result,
            scoped ref DateTimeRawInfo raw,
            DateTimeFormatInfo dtfi
        )
        ...
        //
        private static bool ProcessDateTimeSuffix(
            ref DateTimeResult result,
            scoped ref DateTimeRawInfo raw,
            scoped ref DateTimeToken dtok
        )
        ...
        internal static bool ProcessHebrewTerminalState(
            DS dps,
            scoped ref DateTimeResult result,
            scoped ref DateTimeStyles styles,
            scoped ref DateTimeRawInfo raw,
        ...
        //
        internal static bool ProcessTerminalState(
            DS dps,
            scoped ref DateTimeResult result,
            scoped ref DateTimeStyles styles,
            scoped ref DateTimeRawInfo raw,
        ...
        internal static DateTime Parse(
            ReadOnlySpan<char> s,
            DateTimeFormatInfo dtfi,
            DateTimeStyles styles
        )
        ...
        private static bool DetermineTimeZoneAdjustments(
            ref DateTimeResult result,
            DateTimeStyles styles,
            bool bTimeOnly
        )
        {
            ...
            else
            {
                ...
                if (
                    offsetTicks < DateTimeOffset.MinOffset
                    || offsetTicks > DateTimeOffset.MaxOffset
                )
                {
            ...
            if ((result.flags & ParseFlags.TimeZoneUsed) == 0)
            {
                ...
                if ((styles & DateTimeStyles.AssumeLocal) != 0)
                {
                    ...
                    {
                        result.parsedDate = DateTime.SpecifyKind(
                            result.parsedDate,
                            DateTimeKind.Local
                        );
                        return true;
        ...
        //
        //
        private static bool ParseISO8601(
            scoped ref DateTimeRawInfo raw,
            ref __DTString str,
            DateTimeStyles styles,
            scoped ref DateTimeResult result
        )
        {
            ...
            if (str.Match(TimeDelimiter))
            {
                ...
                if (
                    (str.Match(TimeFractionDelimiterDot)) || (str.Match(TimeFractionDelimiterComma))
                )
                {
        ...
        **Exceptions: FormatException if error in parsing number.
        ...
        internal static bool ParseDigits(
            ref __DTString str,
            int minDigitLen,
            int maxDigitLen,
            out int result
        ...
        **Exceptions: FormatException if errors in parsing.
        ...
        private static bool ParseTimeZoneOffset(
            ref __DTString str,
            int len,
            scoped ref TimeSpan result
        )
        {
        ...
        **Exceptions: FormatException if an abbreviated month name can not be found.
        ...
        )
        {
            ...
            if (str.GetNext())
            {
                ...
                {
                    int tempResult = str.MatchLongestWords(
                        dtfi.InternalGetGenitiveMonthNames(abbreviated: true),
                        ref maxMatchStrLen
                    );
                ...
                {
                    int tempResult = str.MatchLongestWords(
                        dtfi.InternalGetLeapYearMonthNames(),
                        ref maxMatchStrLen
                    );
        ...
        **Exceptions: FormatException if a month name can not be found.
        ...
        )
        {
            ...
            if (str.GetNext())
            {
                if (ReferenceEquals(dtfi, DateTimeFormat.InvariantFormatInfo))
                {
                    ...
                    if (span.Length >= 3)
                    {
                        ...
                        if ((m0 | m1 | m2) <= 0x7F)
                        {
                            ...
                            {
                                case 0x6a616e: /* 'jan' */
                                    SetIfStartsWith(span, "January", 1, ref result, ref maxMatchStrLen);
                                    break;
                                case 0x666562: /* 'feb' */
                                    SetIfStartsWith(
                                        span,
                                    ...
                                    break;
                                case 0x6d6172: /* 'mar' */
                                    SetIfStartsWith(span, "March", 3, ref result, ref maxMatchStrLen);
                                    break;
                                case 0x617072: /* 'apr' */
                                    SetIfStartsWith(span, "April", 4, ref result, ref maxMatchStrLen);
                                    break;
                                case 0x6d6179: /* 'may' */
                                    SetIfStartsWith(span, "May", 5, ref result, ref maxMatchStrLen);
                                    break;
                                case 0x6a756e: /* 'jun' */
                                    SetIfStartsWith(span, "June", 6, ref result, ref maxMatchStrLen);
                                    break;
                                case 0x6a756c: /* 'jul' */
                                    SetIfStartsWith(span, "July", 7, ref result, ref maxMatchStrLen);
                                    break;
                                case 0x617567: /* 'aug' */
                                    SetIfStartsWith(span, "August", 8, ref result, ref maxMatchStrLen);
                                    break;
                                case 0x736570: /* 'sep' */
                                    SetIfStartsWith(
                                        span,
                ...
                {
                    int tempResult = str.MatchLongestWords(
                        dtfi.InternalGetGenitiveMonthNames(abbreviated: false),
                        ref maxMatchStrLen
                    );
                    // We found a longer match in the genitive month name.  Use this as the result.
                    // The result from MatchLongestWords is 0 ~ length of word array.
        ...
        **Exceptions: FormatException if a day of week name can not be found.
        ...
        private static void SetIfStartsWith(
            ReadOnlySpan<char> span,
            [ConstantExpected] string match,
            int matchResult,
            scoped ref int result,
        ...
        **Exceptions:
        ...
        )
        {
            if ((result.flags & ParseFlags.CaptureOffset) != 0)
            {
                if ((result.flags & ParseFlags.TimeZoneUsed) != 0)
                {
                    ...
                    return new DateTime(
                        DateTime.UtcNow.Ticks + result.timeZoneOffset.Ticks,
        ...
        private static bool CheckDefaultDateTime(
            scoped ref DateTimeResult result,
            scoped ref Calendar cal,
            DateTimeStyles styles
        )
        {
            if ((result.flags & ParseFlags.CaptureOffset) != 0)
        ...
        // formats.
        //
        private static string ExpandPredefinedFormat(
            char format,
            scoped ref DateTimeFormatInfo dtfi,
        ...
        private static bool ParseJapaneseEraStart(ref __DTString str, DateTimeFormatInfo dtfi)
        {
            ...
            if (
                ...
                || dtfi.Calendar.ID != CalendarId.JAPAN
                || !str.GetNext()
            )
                return false;
            ...
            switch (ch)
            {
                ...
                case 'M':
                    ...
                    if (tokenLen <= 2)
                    {
                        ...
                        {
                            if (
                                !parseInfo.fUseHebrewNumberParser
                ...
                case 'F':
                    ...
                    if (tokenLen <= DateTimeFormat.MaxSecondsFractionDigits)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                result.SetFailure(
                                    ParseFailureKind.Format_RepeatDateTimePattern,
                                    ch
                                );
                                return false;
                            }
                ...
                case '\'':
                    ...
                    if ((result.flags & ParseFlags.CaptureOffset) != 0)
                    {
                        if (
                            (
                                (result.flags & ParseFlags.Rfc1123Pattern) != 0
                                && quotedSpan.SequenceEqual(GMTName)
                            )
                            || (
                                (result.flags & ParseFlags.UtcSortPattern) != 0
                                && quotedSpan.SequenceEqual(ZuluName)
                            )
                        )
                        {
                            result.flags |= ParseFlags.TimeZoneUsed;
        ...
        **  Only support GMT timezone.
        ...
        )
        {
            ...
            if (formatParam.Length == 1)
            {
                ...
                formatParam = ExpandPredefinedFormat(
                    formatParamChar,
                    ref dtfi,
                    ref parseInfo,
                    ref result
        ...
        private static bool TryParseFormatR(
        ...
        )
        {
            ...
            int day;
            {
                uint digit1 = (uint)(source[5] - '0'),
                    digit2 = (uint)(source[6] - '0');
        ...
        private static bool TryParseFormatO(
            ReadOnlySpan<char> source,
            scoped ref DateTimeResult result
        )
        {
            ...
            double fraction;
            {
                ...
                fraction =
                    (f1 * 1000000 + f2 * 100000 + f3 * 10000 + f4 * 1000 + f5 * 100 + f6 * 10 + f7)
                    / 10000000.0;
            }
            ...
            if (source.Length > 27)
            {
                ...
                switch (offsetChar)
                {
                    ...
                    case '-':
                        ...
                        if (source.Length == 33)
                        {
                            uint oh1 = (uint)(source[28] - '0'),
                                oh2 = (uint)(source[29] - '0');
                        ...
                        {
                            uint om1 = (uint)(source[colonIndex + 1] - '0'),
                                om2 = (uint)(source[colonIndex + 2] - '0');
            ...
            switch (result.failure)
            {
                ...
                case ParseFailureKind.Format_BadDatePattern:
                    return new FormatException(
                        SR.Format(SR.Format_BadDatePattern, new string(result.failureSpanArgument))
                    );
                case ParseFailureKind.Format_BadDateTime:
                    return new FormatException(
                        SR.Format(SR.Format_BadDateTime, new string(result.failureSpanArgument))
                    );
                case ParseFailureKind.Format_BadDateTimeCalendar:
                    return new FormatException(
                        SR.Format(
                            SR.Format_BadDateTimeCalendar,
                            new string(result.failureSpanArgument),
                            result.calendar
                        )
                    );
                case ParseFailureKind.Format_BadDayOfWeek:
                    return new FormatException(
                        SR.Format(SR.Format_BadDayOfWeek, new string(result.failureSpanArgument))
                    );
                case ParseFailureKind.Format_BadFormatSpecifier:
                    return new FormatException(
                        SR.Format(
                            SR.Format_BadFormatSpecifier,
                            new string(result.failureSpanArgument)
                        )
                    );
                case ParseFailureKind.Format_BadQuote:
                    return new FormatException(
                        SR.Format(SR.Format_BadQuote, (char)result.failureIntArgument)
                    );
                case ParseFailureKind.Format_DateOutOfRange:
                    return new FormatException(
                        SR.Format(SR.Format_DateOutOfRange, new string(result.failureSpanArgument))
                    );
                case ParseFailureKind.Format_MissingIncompleteDate:
                    return new FormatException(
                        SR.Format(
                            SR.Format_MissingIncompleteDate,
                            new string(result.failureSpanArgument)
                        )
                    );
                ...
                case ParseFailureKind.Format_RepeatDateTimePattern:
                    return new FormatException(
                        SR.Format(SR.Format_RepeatDateTimePattern, (char)result.failureIntArgument)
                    );
                case ParseFailureKind.Format_UnknownDateTimeWord:
                    return new FormatException(
                        SR.Format(
                            SR.Format_UnknownDateTimeWord,
                            new string(result.failureSpanArgument),
...
#pragma warning restore IDE0060
    ...
    internal ref struct __DTString
    {
        ...
        internal __DTString(ReadOnlySpan<char> str, DateTimeFormatInfo dtfi)
        {
            ...
            m_checkDigitToken = (
                (dtfi.FormatFlags & DateTimeFormatFlags.UseDigitPrefixInTokens) != 0
            );
        }
        ...
        internal bool MatchSpecifiedWord(string target) =>
            ...
            && m_info.Compare(Value.Slice(Index, target.Length), target, CompareOptions.IgnoreCase)
                == 0;

        internal bool MatchSpecifiedWords(
            string target,
            bool checkWordBoundary,
            scoped ref int matchLength
        )
        {
            ...
            if (
                matchLength > valueRemaining
                || m_info.Compare(
                    Value.Slice(Index, matchLength),
                    target,
                    CompareOptions.IgnoreCase
                ) != 0
            )
        ...
        internal bool Match(char ch)
        {
            ...
            if ((Value[Index] == ch) || (ch == ' ' && IsSpaceReplacingChar(Value[Index])))
            {
                m_current = ch;
                return true;
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal bool GetNextDigit() => ++Index < Length && char.IsAsciiDigit(Value[Index]);
        ...
        internal DTSubString GetSubString()
        {
            ...
            while (Index + sub.length < Length)
            {
                ...
                currentType = char.IsAsciiDigit(ch)
