...
namespace System.Text
{
    ...
    internal sealed partial class EncoderLatin1BestFitFallbackBuffer : EncoderFallbackBuffer
    {
        ...
        public override bool Fallback(char charUnknown, int index)
        {
            ...
            Debug.Assert(
                _iCount < 1,
                "[EncoderLatin1BestFitFallbackBuffer.Fallback(non surrogate)] Fallback char "
        ...
        public override bool Fallback(char charUnknownHigh, char charUnknownLow, int index)
        {
            ...
            Debug.Assert(
                _iCount < 1,
                "[EncoderLatin1BestFitFallbackBuffer.Fallback(surrogate)] Fallback char "
                    + ((int)_cBestFit).ToString("X4", CultureInfo.InvariantCulture)
                    + " caused recursive fallback"
            );
        ...
        private static char TryBestFit(char cUnknown)
        {
            ...
            while ((iDiff = (highBound - lowBound)) > 6)
            {
                ...
                if (cTest == cUnknown)
                {
                    // We found it
                    Debug.Assert(
                        index + 1 < s_arrayCharBestFit.Length,
