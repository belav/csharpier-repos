...
namespace System.Text
{
    public static partial class Ascii
    {
            ...
            where T : unmanaged
        {
            if (sizeof(T) == 1)
            {
                if (vector.ExtractMostSignificantBits() != 0)
                {
                    return true;
                }
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe void Widen8To16AndAndWriteTo(
            Vector128<byte> narrowVector,
            char* pDest,
            nuint destOffset
        )
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe void Narrow16To8AndAndWriteTo(
            Vector128<ushort> wideVector,
            byte* pDest,
            nuint destOffset
        )
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe Vector128<TTo> NarrowOrWidenLowerVectorUnsigned<TFrom, TTo>(
            Vector128<TFrom> vector
        )
            where TFrom : unmanaged
            where TTo : unmanaged
