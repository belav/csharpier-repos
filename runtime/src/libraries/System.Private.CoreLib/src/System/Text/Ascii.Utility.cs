...
namespace System.Text
{
    public static partial class Ascii
    {
        ...
        private static unsafe nuint GetIndexOfFirstNonAsciiByte_Vector(
            byte* pBuffer,
            nuint bufferLength
        )
        {
            ...
            if (Vector512.IsHardwareAccelerated && bufferLength >= 2 * (uint)Vector512<byte>.Count)
            {
                if (Vector512.Load(pBuffer).ExtractMostSignificantBits() == 0)
                {
                    ...
                    pBuffer = (byte*)(
                        ((nuint)pBuffer + Vector512.Size) & ~(nuint)(Vector512.Size - 1)
                    );

#if DEBUG
                    long numBytesRead = pBuffer - pOriginalBuffer;
                    Debug.Assert(
                        0 < numBytesRead && numBytesRead <= Vector512.Size,
                        "We should've made forward progress of at least one byte."
                    );
                    Debug.Assert(
                        (nuint)numBytesRead <= bufferLength,
                    ...
                    {
                        Debug.Assert(
                            (nuint)pBuffer % Vector512.Size == 0,
            ...
            )
            {
                if (Vector256.Load(pBuffer).ExtractMostSignificantBits() == 0)
                {
                    ...
                    pBuffer = (byte*)(
                        ((nuint)pBuffer + Vector256.Size) & ~(nuint)(Vector256.Size - 1)
                    );

#if DEBUG
                    long numBytesRead = pBuffer - pOriginalBuffer;
                    Debug.Assert(
                        0 < numBytesRead && numBytesRead <= Vector256.Size,
                        "We should've made forward progress of at least one byte."
                    );
                    Debug.Assert(
                        (nuint)numBytesRead <= bufferLength,
                    ...
                    {
                        Debug.Assert(
                            (nuint)pBuffer % Vector256.Size == 0,
                            "Vector read should be aligned."
                        );
            ...
            )
            {
                if (!VectorContainsNonAsciiChar(Vector128.Load(pBuffer)))
                {
                    ...
                    pBuffer = (byte*)(
                        ((nuint)pBuffer + Vector128.Size) & ~(nuint)(Vector128.Size - 1)
                    );

#if DEBUG
                    long numBytesRead = pBuffer - pOriginalBuffer;
                    Debug.Assert(
                        0 < numBytesRead && numBytesRead <= Vector128.Size,
                        "We should've made forward progress of at least one byte."
                    );
                    Debug.Assert(
                        (nuint)numBytesRead <= bufferLength,
                    ...
                    {
                        Debug.Assert(
                            (nuint)pBuffer % Vector128.Size == 0,
                            "Vector read should be aligned."
                        );
            ...
            Debug.Assert(
                !AllBytesInUInt32AreAscii(currentUInt32),
                "Shouldn't have reached this point if we have an all-ASCII input."
            );
        ...
        )
        {
            ...
            Debug.Assert(
                Sse2.IsSupported || AdvSimd.Arm64.IsSupported,
            ...
            uint currentSseMask = uint.MaxValue,
                secondSseMask = uint.MaxValue;
            uint currentAdvSimdIndex = uint.MaxValue,
                secondAdvSimdIndex = uint.MaxValue;
            byte* pOriginalBuffer = pBuffer;
            ...
            pBuffer = (byte*)(
                ((nuint)pBuffer + SizeOfVector128) & ~(nuint)MaskOfAllBitsInVector128
            );

#if DEBUG
            long numBytesRead = pBuffer - pOriginalBuffer;
            Debug.Assert(
                0 < numBytesRead && numBytesRead <= SizeOfVector128,
                "We should've made forward progress of at least one byte."
            );
            Debug.Assert(
                (nuint)numBytesRead <= bufferLength,
            ...
            if (bufferLength >= 2 * SizeOfVector128)
            {
                byte* pFinalVectorReadPos = (byte*)(
                    (nuint)pBuffer + bufferLength - 2 * SizeOfVector128
                );
                ...
                do
                {
                    if (Sse2.IsSupported)
                    {
                        ...
                        Vector128<byte> secondVector = Sse2.LoadAlignedVector128(
                    ...
                    else if (AdvSimd.Arm64.IsSupported)
                    {
                        ...
                        Vector128<byte> secondVector = AdvSimd.LoadVector128(
                            pBuffer + SizeOfVector128
                        );
            ...
            if (Sse2.IsSupported)
            {
                ...
                Debug.Assert(
                    ContainsNonAsciiByte_Sse2(currentSseMask),
                    "Shouldn't be here unless we see non-ASCII data."
                );
            ...
            uint currentDWord;
            Debug.Assert(
                !AllBytesInUInt32AreAscii(currentDWord),
                "Shouldn't be here unless we see non-ASCII data."
            );
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static unsafe nuint GetIndexOfFirstNonAsciiChar(
            char* pBuffer,
        ...
        private static unsafe nuint GetIndexOfFirstNonAsciiChar_Vector(
            char* pBuffer,
            nuint bufferLength /* in chars */
        )
        {
            ...
            )
            {
                ...
                if (!VectorContainsNonAsciiChar(Vector512.Load((ushort*)pBuffer)))
                {
                    ...
                    Debug.Assert(
                        0 < numCharsRead && numCharsRead <= SizeOfVector512InChars,
            ...
            )
            {
                ...
                if (!VectorContainsNonAsciiChar(Vector256.Load((ushort*)pBuffer)))
                {
                    ...
                    long numCharsRead = pBuffer - pOriginalBuffer;
                    Debug.Assert(
                        0 < numCharsRead && numCharsRead <= SizeOfVector256InChars,
                        "We should've made forward progress of at least one char."
                    );
                    Debug.Assert(
                        (nuint)numCharsRead <= bufferLength,
            ...
            Debug.Assert(
                !AllCharsInUInt32AreAscii(currentUInt32),
                "Shouldn't have reached this point if we have an all-ASCII input."
            );
        ...
        private static unsafe nuint GetIndexOfFirstNonAsciiChar_Intrinsified(
            char* pBuffer,
            nuint bufferLength /* in chars */
        )
        {
            ...
            if (bufferLength >= 2 * Vector128.Size)
            {
                char* pFinalVectorReadPos = (char*)(
                    (nuint)pBuffer + bufferLength - 2 * Vector128.Size
                );
                ...
                {
                    firstVector = Vector128.LoadUnsafe(ref *(ushort*)pBuffer);
                    secondVector = Vector128.LoadUnsafe(
                        ref *(ushort*)pBuffer,
                        SizeOfVector128InChars
                    );
            ...
            if (((byte)bufferLength & (Vector128.Size - 1)) != 0)
            {
                ...
                pBuffer = (char*)(
                    (byte*)pBuffer + (bufferLength & (Vector128.Size - 1)) - Vector128.Size
                );
                firstVector = Vector128.LoadUnsafe(ref *(ushort*)pBuffer);
                if (VectorContainsNonAsciiChar(firstVector))
                {
            ...
            uint currentDWord;
            Debug.Assert(
                !AllCharsInUInt32AreAscii(currentDWord),
                "Shouldn't be here unless we see non-ASCII data."
            );
                ...
                if (UIntPtr.Size == sizeof(ulong))
                {
                    ...
                    if (!AllCharsInUInt64AreAscii(candidateUInt64))
                    {
                        ...
                        pBuffer = (char*)(
                            (byte*)pBuffer
                            + (
                                (nuint)(BitOperations.TrailingZeroCount(candidateUInt64) >> 3)
                                & ~(nuint)1
                            )
                        );
                        goto Finish;
        ...
        /// </summary>
        internal static unsafe nuint NarrowUtf16ToAscii(
            char* pUtf16Buffer,
            byte* pAsciiBuffer,
            nuint elementCount
        )
        {
            ...
            )
            {
                ...
                {
                    currentOffset = NarrowUtf16ToAscii_Intrinsified_512(
                        pUtf16Buffer,
                        pAsciiBuffer,
                        elementCount
                    );
                ...
                {
                    currentOffset = NarrowUtf16ToAscii_Intrinsified_256(
                        pUtf16Buffer,
                        pAsciiBuffer,
                        elementCount
                    );
                ...
                {
                    currentOffset = NarrowUtf16ToAscii_Intrinsified(
                        pUtf16Buffer,
                        pAsciiBuffer,
                        elementCount
                    );
                ...
                do
                {
                    if (IntPtr.Size >= 8)
                    {
                        ...
                        NarrowFourUtf16CharsToAsciiAndWriteToBuffer(
                            ref pAsciiBuffer[currentOffset],
                            utf16Data64Bits
                        );
                    }
                    else
                    {
                        ...
                        NarrowTwoUtf16CharsToAsciiAndWriteToBuffer(
                            ref pAsciiBuffer[currentOffset + 2],
                            utf16Data32BitsLow
                        );
                    }
            ...
            Debug.Assert(
                !AllCharsInUInt32AreAscii(utf16Data32BitsHigh),
                "Shouldn't have reached this point if we have an all-ASCII input."
            );
        ...
        private static bool VectorContainsNonAsciiChar(Vector128<ushort> utf16Vector)
        {
            ...
            if (Sse2.IsSupported)
            {
                ...
                else
                {
                    ...
                    return (
                            Sse2.MoveMask(
                                Sse2.AddSaturate(utf16Vector, asciiMaskForAddSaturate).AsByte()
                            ) & 0b_1010_1010_1010_1010
                        ) != 0;
                }
            }
            else if (AdvSimd.Arm64.IsSupported)
            ...
            where T : unmanaged
        {
            ...
            else
            {
                return Sse41.IsSupported
                    ...
                    : (vector.AsUInt16() & Vector128.Create((ushort)0xFF80))
            ...
            where T : unmanaged
        {
            ...
            else
            {
                return Avx.IsSupported
                    ...
                    : (vector.AsUInt16() & Vector256.Create((ushort)0xFF80))
            ...
            where T : unmanaged
        {
            ...
            {
                return (vector.AsUInt16() & Vector512.Create((ushort)0xFF80))
                    == Vector512<ushort>.Zero;
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static Vector128<byte> ExtractAsciiVector(
            Vector128<ushort> vectorFirst,
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe nuint NarrowUtf16ToAscii_Intrinsified(
            char* pUtf16Buffer,
            byte* pAsciiBuffer,
            nuint elementCount
        )
        {
            ...
            currentOffsetInElements =
                SizeOfVector128 - ((nuint)pAsciiBuffer & MaskOfAllBitsInVector128);
            ...
            do
            {
                ...
                utf16VectorFirst = Vector128.LoadUnsafe(ref utf16Buffer, currentOffsetInElements);
                Vector128<ushort> utf16VectorSecond = Vector128.LoadUnsafe(
                    ref utf16Buffer,
                    currentOffsetInElements + SizeOfVector128 / sizeof(short)
                );
                ...
                Debug.Assert(
                    ((nuint)pAsciiBuffer + currentOffsetInElements) % SizeOfVector128 == 0,
            ...
            Debug.Assert(
                ((nuint)pAsciiBuffer + currentOffsetInElements) % sizeof(ulong) == 0,
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe nuint NarrowUtf16ToAscii_Intrinsified_256(
            char* pUtf16Buffer,
            byte* pAsciiBuffer,
            nuint elementCount
        )
        {
            ...
            currentOffsetInElements =
                Vector256.Size - ((nuint)pAsciiBuffer & MaskOfAllBitsInVector256);

            Debug.Assert(
                0 < currentOffsetInElements && currentOffsetInElements <= Vector256.Size,
                "We wrote at least 1 byte but no more than a whole vector."
            );
            Debug.Assert(
                currentOffsetInElements <= elementCount,
            ...
            Debug.Assert(
                elementCount - currentOffsetInElements >= Vector256.Size,
                "We should be able to run at least one whole vector."
            );
            ...
            do
            {
                ...
                utf16VectorFirst = Vector256.LoadUnsafe(ref utf16Buffer, currentOffsetInElements);
                Vector256<ushort> utf16VectorSecond = Vector256.LoadUnsafe(
                    ref utf16Buffer,
                    currentOffsetInElements + Vector256.Size / sizeof(short)
                );
                ...
                Debug.Assert(
                    ((nuint)pAsciiBuffer + currentOffsetInElements) % Vector256.Size == 0,
                    "Write should be aligned."
                );
            ...
            Debug.Assert(
                ((nuint)pAsciiBuffer + currentOffsetInElements) % Vector128.Size == 0,
                "Destination should be 128-bit-aligned."
            );
            asciiVector = Vector256.Narrow(utf16VectorFirst, utf16VectorFirst);
            asciiVector.GetLower().StoreUnsafe(ref asciiBuffer, currentOffsetInElements);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe nuint NarrowUtf16ToAscii_Intrinsified_512(
            char* pUtf16Buffer,
            byte* pAsciiBuffer,
            nuint elementCount
        )
        {
            ...
            currentOffsetInElements =
                Vector512.Size - ((nuint)pAsciiBuffer & MaskOfAllBitsInVector512);

            Debug.Assert(
                0 < currentOffsetInElements && currentOffsetInElements <= Vector512.Size,
                "We wrote at least 1 byte but no more than a whole vector."
            );
            Debug.Assert(
                currentOffsetInElements <= elementCount,
            ...
            Debug.Assert(
                elementCount - currentOffsetInElements >= Vector512.Size,
                "We should be able to run at least one whole vector."
            );
            ...
            do
            {
                ...
                utf16VectorFirst = Vector512.LoadUnsafe(ref utf16Buffer, currentOffsetInElements);
                Vector512<ushort> utf16VectorSecond = Vector512.LoadUnsafe(
                    ref utf16Buffer,
                    currentOffsetInElements + Vector512.Size / sizeof(short)
                );
                ...
                Debug.Assert(
                    ((nuint)pAsciiBuffer + currentOffsetInElements) % Vector512.Size == 0,
                    "Write should be aligned."
                );
            ...
            Debug.Assert(
                ((nuint)pAsciiBuffer + currentOffsetInElements) % Vector256.Size == 0,
                "Destination should be 256-bit-aligned."
            );
            asciiVector = Vector512.Narrow(utf16VectorFirst, utf16VectorFirst);
            asciiVector.GetLower().StoreUnsafe(ref asciiBuffer, currentOffsetInElements);
        ...
        /// of elements that were able to be converted.
        /// </summary>
        internal static unsafe nuint WidenAsciiToUtf16(
            byte* pAsciiBuffer,
            char* pUtf16Buffer,
            nuint elementCount
        )
        {
            ...
            )
            {
                ...
                if (Vector512.IsHardwareAccelerated && elementCount >= (uint)Vector512<byte>.Count)
                {
                    ...
                    do
                    {
                        ...
                        (Vector512<ushort> utf16LowVector, Vector512<ushort> utf16HighVector) =
                            Vector512.Widen(asciiVector);
                    ...
                    Vector256.IsHardwareAccelerated
                    && elementCount >= (uint)Vector256<byte>.Count
                )
                {
                    // Calculating the destination address outside the loop results in significant
                    // perf wins vs. relying on the JIT to fold memory addressing logic into the
                    ...
                    do
                    {
                        ...
                        (Vector256<ushort> utf16LowVector, Vector256<ushort> utf16HighVector) =
                            Vector256.Widen(asciiVector);
                        utf16LowVector.Store(pCurrentWriteAddress);
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        (Vector128<ushort> utf16LowVector, Vector128<ushort> utf16HighVector) =
                            Vector128.Widen(asciiVector);
                        utf16LowVector.Store(pCurrentWriteAddress);
            ...
            if (remainingElementCount >= 4)
            {
                ...
                do
                {
                    ...
                    WidenFourAsciiBytesToUtf16AndWriteToBuffer(
                        ref pUtf16Buffer[currentOffset],
                        asciiData
                    );
                    currentOffset += 4;
                } while (currentOffset <= finalOffsetWhereCanLoop);
            ...
            Debug.Assert(
                !AllBytesInUInt32AreAscii(asciiData),
                "Shouldn't have reached this point if we have an all-ASCII input."
            );
