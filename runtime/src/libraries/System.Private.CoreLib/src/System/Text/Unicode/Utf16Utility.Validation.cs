...
namespace System.Text.Unicode
{
    partial internal static unsafe class Utf16Utility
    {
        ...
        )
        {
            ...
            int firstNonAsciiIndex = Ascii.GetIndexOfFirstNonAsciiChar(
                new ReadOnlySpan<char>(pInputBuffer, inputLength)
            );
            int numAsciiCharsConsumedJustNow =
                firstNonAsciiIndex < 0 ? inputLength : firstNonAsciiIndex;
            Debug.Assert(
                    ...
                    do
                    {
                        ...
                        if (AdvSimd.Arm64.IsSupported)
                        {
                            mask = GetNonAsciiBytes(
                                AdvSimd
                                    .CompareLessThan(
                                        AdvSimd.Add(utf16Data, vectorA000).AsInt16(),
                                        vector7800.AsInt16()
                                    )
                                    .AsByte(),
                                bitMask128
            ...
            else if (Vector.IsHardwareAccelerated)
            {
                if (inputLength >= Vector<ushort>.Count)
                {
                    ...
                    char* pHighestAddressWhereCanReadOneVector =
                        pEndOfInputBuffer - Vector<ushort>.Count;
                    Debug.Assert(pHighestAddressWhereCanReadOneVector >= pInputBuffer);
