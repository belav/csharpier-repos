    ...
    public partial class ASCIIEncoding : Encoding
    {
        ...
        internal sealed class ASCIIEncodingSealed : ASCIIEncoding
        {
            public override object Clone()
            {
                ...
                return new ASCIIEncoding() { IsReadOnly = false };
            }
        }
        ...
        )
        {
            ...
            )
            {
                ...
                int firstNonAsciiIndex = Ascii.GetIndexOfFirstNonAsciiChar(
                    new ReadOnlySpan<char>(pChars, charsLength)
                );
        ...
        )
        {
            Ascii.FromUtf16(
                new ReadOnlySpan<char>(pChars, charsLength),
                new Span<byte>(pBytes, bytesLength),
                out charsConsumed
            );
            return charsConsumed;
        }

        private protected sealed override unsafe int GetBytesWithFallback(
        ...
        )
        {
            ...
            if (
                ((encoder is null) ? this.EncoderFallback : encoder.Fallback)
        ...
        )
        {
            ...
            )
            {
                ...
                int indexOfFirstNonAscii = Ascii.GetIndexOfFirstNonAsciiByte(
                    new ReadOnlySpan<byte>(pBytes, bytesLength)
                );
        ...
        )
        {
            Ascii.ToUtf16(
                new ReadOnlySpan<byte>(pBytes, bytesLength),
                new Span<char>(pChars, charsLength),
                out bytesConsumed
            );
