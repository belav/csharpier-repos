    ...
    partial internal static class FormattingHelpers
    {
        ...
        public static int CountDigits(ulong value)
        {
            ...
            ReadOnlySpan<byte> powersOf10 = new byte[]
            {
                ...
                0x00,
                0x00,
                0x00, // unused entry to avoid needing to subtract
                0x00,
                0x00,
                0x00,
                0x00,
                ...
                0x23,
                0xC7,
                0x8A, // 10000000000000000000
            };
            Debug.Assert((index + 1) * sizeof(ulong) <= powersOf10.Length);
            ulong powerOf10 = Unsafe.ReadUnaligned<ulong>(
                ref Unsafe.Add(ref MemoryMarshal.GetReference(powersOf10), index * sizeof(ulong))
            );
        ...
        public static int CountDigits(uint value)
        {
            ...
            Debug.Assert(
                table.Length == (32 * sizeof(long)),
                "Every result of uint.Log2(value) needs a long entry in the table."
            );

            long tableValue = Unsafe.ReadUnaligned<long>(
                ref Unsafe.Add(
                    ref MemoryMarshal.GetReference(table),
                    uint.Log2(value) * sizeof(long)
                )
            );
            if (!BitConverter.IsLittleEndian)
            {
