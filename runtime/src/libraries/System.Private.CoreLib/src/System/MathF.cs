...
namespace System
{
    ...
    public static partial class MathF
    {
        ...
        public static float ReciprocalEstimate(float x)
        {
            ...
            {
                return AdvSimd
                    .Arm64.ReciprocalEstimateScalar(Vector64.CreateScalarUnsafe(x))
                    .ToScalar();
            }
            else
            {
        ...
        public static float ReciprocalSqrtEstimate(float x)
        {
            ...
            {
                return AdvSimd
                    .Arm64.ReciprocalSquareRootEstimateScalar(Vector64.CreateScalarUnsafe(x))
                    .ToScalar();
            }
            else
            {
        ...
        public static float Round(float x)
        {
            ...
            if (biasedExponent <= 0x7E)
            {
                ...
                float result =
                    (
                        (biasedExponent == 0x7E)
                        && (float.ExtractTrailingSignificandFromBits(bits) != 0)
                    )
                        ? 1.0f
                        : 0.0f;
            ...
            if (RuntimeHelpers.IsKnownConstant((int)mode))
            {
                ...
                if (mode == MidpointRounding.AwayFromZero)
                {
                    if (AdvSimd.IsSupported)
                        return AdvSimd
                            .RoundAwayFromZeroScalar(Vector64.CreateScalarUnsafe(x))
                            .ToScalar();
        ...
        public static unsafe float Round(float x, int digits, MidpointRounding mode)
        {
            if ((digits < 0) || (digits > maxRoundingDigits))
            {
                throw new ArgumentOutOfRangeException(
                    nameof(digits),
                    SR.ArgumentOutOfRange_RoundingDigits_MathF
                );
            }
