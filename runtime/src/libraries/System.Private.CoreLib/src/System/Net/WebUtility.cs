...
namespace System.Net
{
    public static class WebUtility
    {
                ...
                if (ch == '&')
                {
                    ...
                    if (entityLength >= 0 && inputSlice[entityLength] == ';')
                    {
                        ...
                        if (entityLength > 1 && inputSlice[0] == '#')
                        {
                            ...
                            if (
                                parsedSuccessfully
                                && UnicodeUtility.IsValidUnicodeScalar(parsedValue)
                            )
                            {
        ...
        private static void GetEncodedBytes(
            byte[] originalBytes,
            int offset,
            int count,
            byte[] expandedBytes
        ...
        private static bool IsUrlSafeChar(char ch)
        {
            ...
            */
            ...
            return char.IsAsciiLetter(ch)
                || (
                    (uint)(code - 0x20) <= (uint)('9' - 0x20)
                    && ((1 << (code - 0x20)) & safeSpecialCharMask) != 0
                )
                || (code == (int)'_');
        }
        ...
        private static class HtmlEntities
        {
            ...
            static HtmlEntities()
            {
                ...
                Debug.Assert(
                    s_lookupTable.Count == 253,
                    $"There should be 253 HTML entities, but {nameof(s_lookupTable)} has {s_lookupTable.Count} of them."
                );
            ...
            private static Dictionary<ulong, char> InitializeLookupTable()
            {
                ...
                [
                    ...
                    0x00,
                    0x00,
                    0x00, /*ToUInt64Key("diams")*/
                    0x66,
                    0x26, /*'\x2666'*/
                ];

                var dictionary = new Dictionary<ulong, char>(
                    tableData.Length / (sizeof(ulong) + sizeof(char))
                );
