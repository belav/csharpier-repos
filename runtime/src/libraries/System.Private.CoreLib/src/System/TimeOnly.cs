...
namespace System
{
          ...
          ISpanFormattable
    {
        ...
        public TimeOnly(long ticks)
        {
            if ((ulong)ticks > MaxTimeTicks)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(ticks),
                    SR.ArgumentOutOfRange_TimeOnlyBadTicks
                );
            }
        ...
        private TimeOnly AddTicks(long ticks) =>
            new TimeOnly(
                (_ticks + TimeSpan.TicksPerDay + (ticks % TimeSpan.TicksPerDay))
        ...
        public TimeOnly AddMinutes(double value) =>
            AddTicks((long)(value * TimeSpan.TicksPerMinute));
        ...
        public static TimeOnly FromDateTime(DateTime dateTime) =>
            new TimeOnly(dateTime.TimeOfDay.Ticks);
        ...
        public override bool Equals(object? value) =>
            value is TimeOnly timeOnly && _ticks == timeOnly._ticks;
        ...
        private const ParseFlags ParseFlagsTimeMask =
            ParseFlags.HaveYear
                ...
                switch (format[0])
                {
                    ...
                    case 'O':
                        return string.Create(
                            ...
                            (destination, value) =>
                            {
                                bool b = DateTimeFormat.TryFormatTimeOnlyO(
                                    value.Hour,
                                    value.Minute,
                                    value.Second,
                                    value._ticks % TimeSpan.TicksPerSecond,
                                    destination
                                );
                                Debug.Assert(b);
                            }
                    ...
                    case 'R':
                        return string.Create(
                            ...
                            (destination, value) =>
                            {
                                bool b = DateTimeFormat.TryFormatTimeOnlyR(
                                    value.Hour,
                                    value.Minute,
                                    value.Second,
                                    destination
        ...
        )
        {
            ...
            if (format.Length == 1)
            {
                switch (format[0])
                {
                    ...
                    case 'O':
                        if (
                            !DateTimeFormat.TryFormatTimeOnlyO(
                                Hour,
                                Minute,
                                Second,
