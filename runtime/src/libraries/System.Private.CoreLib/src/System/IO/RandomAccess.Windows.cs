...
namespace System.IO
{
    public static partial class RandomAccess
    {
        ...
        )
        {
            ...
            return AsyncOverSyncWithIoCancellation.InvokeAsync(
                static state =>
                {
                    try
                    {
                        int result = ReadAtOffset(
                            state.handle,
                            state.buffer.Span,
                            state.fileOffset
                        );
                ...
                (handle, buffer, fileOffset, strategy),
                cancellationToken
            );
        }

        private static unsafe (
            SafeFileHandle.OverlappedValueTaskSource? vts,
            int errorCode
        ) QueueAsyncReadFile(
            SafeFileHandle handle,
            Memory<byte> buffer,
        ...
        )
        {
            if (!handle.IsAsync)
            {
                return AsyncOverSyncWithIoCancellation.InvokeAsync(
                    static state =>
                        ReadScatterAtOffset(state.handle, state.buffers, state.fileOffset),
                    (handle, buffers, fileOffset),
                    cancellationToken
                );
            }
            ...
            if (
                ...
                && TryPrepareScatterGatherBuffers<Memory<byte>, MemoryHandler>(
                    buffers,
        ...
        )
        {
            ...
            if (
                CanUseScatterGatherWindowsAPIs(handle)
                && TryPrepareScatterGatherBuffers<ReadOnlyMemory<byte>, ReadOnlyMemoryHandler>(
                    buffers,
                    out MemoryHandle[]? handlesToDispose,
                    out IntPtr segmentsPtr,
                    out int totalBytes
        ...
        )
        {
            ...
            NativeOverlapped* result = handle.ThreadPoolBinding!.UnsafeAllocateNativeOverlapped(
                s_callback,
                resetEvent,
                null
            );
        ...
        internal static bool IsEndOfFile(int errorCode, SafeFileHandle handle, long fileOffset)
        {
            switch (errorCode)
            {
                ...
                case Interop.Errors.ERROR_INVALID_PARAMETER
                    when IsEndOfFileForNoBuffering(handle, fileOffset):
                    return true;
                default:
                    return false;
            }
