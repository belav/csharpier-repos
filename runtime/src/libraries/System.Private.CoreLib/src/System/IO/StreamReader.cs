...
namespace System.IO
{
    ...
    public class StreamReader : TextReader
    {
        ...
        public override int Read(Span<char> buffer) =>
            GetType() == typeof(StreamReader) ? ReadSpan(buffer) : base.Read(buffer); // Defer to Read(char[], ...) if a derived type may have previously overridden it
        ...
        internal virtual int ReadBuffer()
        {
            ...
            do
            {
                ...
                {
                    Debug.Assert(
                        _bytePos <= _encoding.Preamble.Length,
                        "possible bug in _compressPreamble.  Are two threads using this StreamReader at the same time?"
                    );
        ...
        private int ReadBuffer(Span<char> userBuffer, out bool readToUserBuffer)
        {
            ...
            do
            {
                ...
                Debug.Assert(
                    charsRead == 0 && _charPos == 0 && _charLen == 0,
                    "We shouldn't be trying to decode more data if we made progress in an earlier iteration."
                );
                if (readToUserBuffer)
                {
                    charsRead = _decoder.GetChars(
                        new ReadOnlySpan<byte>(_byteBuffer, 0, _byteLen),
                        userBuffer,
                        flush: false
                    );
                }
        ...
        public override string? ReadLine()
        {
            ...
            {
                // Look for '\r' or \'n'.
                ReadOnlySpan<char> charBufferSpan = _charBuffer.AsSpan(
                    _charPos,
                    _charLen - _charPos
                );
                Debug.Assert(
                    !charBufferSpan.IsEmpty,
                    "ReadBuffer returned > 0 but didn't bump _charLen?"
                );
        ...
        private async Task<string?> ReadLineAsyncInternal(CancellationToken cancellationToken)
        {
            ...
            do
            {
                ...
                int idxOfNewline = charBuffer
                    .AsSpan(charPos, charLen - charPos)
                    .IndexOfAny('\r', '\n');
                if (idxOfNewline >= 0)
                {
                    ...
                    else
                    {
                        retVal = string.Concat(
                            arrayPoolBuffer.AsSpan(0, arrayPoolBufferPos),
                            charBuffer.AsSpan(charPos, idxOfNewline)
                        );
                ...
                {
                    char[] newBuffer = ArrayPool<char>.Shared
                ...
                charBuffer
                    .AsSpan(charPos, charLen - charPos)
                    .CopyTo(arrayPoolBuffer.AsSpan(arrayPoolBufferPos));
                ...
                if (n == 0)
                {
                    ...
                    do
                    {
                        ...
                        if (_checkPreamble)
                        {
                            ...
                            if (len == 0)
                            {
                                ...
                                if (_byteLen > 0)
                                {
                                    ...
                                    {
                                        n = _decoder.GetChars(
                                            new ReadOnlySpan<byte>(tmpByteBuffer, 0, _byteLen),
                                            buffer.Span.Slice(charsRead),
                                            flush: false
                                        );
                        ...
                        else
                        {
                            ...
                            _byteLen = await tmpStream
                                .ReadAsync(new Memory<byte>(tmpByteBuffer), cancellationToken)
                                .ConfigureAwait(false);
        ...
        private async ValueTask<int> ReadBufferAsync(CancellationToken cancellationToken)
        {
            ...
            do
            {
                if (_checkPreamble)
                {
                    ...
                    int tmpBytePos = _bytePos;
                    int len = await tmpStream
                        .ReadAsync(tmpByteBuffer.AsMemory(tmpBytePos), cancellationToken)
                        .ConfigureAwait(false);
                    Debug.Assert(
                        len >= 0,
                        "Stream.Read returned a negative number!  This is a bug in your stream class."
        ...
        internal sealed class NullStreamReader : StreamReader
        {
            ...
            internal override ValueTask<int> ReadAsyncInternal(
                Memory<char> buffer,
                CancellationToken cancellationToken
            ) =>
                cancellationToken.IsCancellationRequested
