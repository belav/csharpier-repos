...
#pragma warning restore CA1416
        ...
        )
        {
            ...
            TaskContinuation continuation = new ContinueWithTaskContinuation(
                continuationTask,
                options,
                scheduler
            );
        ...
        public static Task<TResult> FromResult<TResult>(TResult result)
        {
            ...
            else if (typeof(TResult).IsValueType) // help the JIT avoid the value type branches for ref types
            {
                ...
                else if (typeof(TResult) == typeof(int))
                {
                    ...
                    {
                        Task<int> task = TaskCache.s_int32Tasks[
                            value - TaskCache.InclusiveInt32Min
                        ];
                ...
                else if (!RuntimeHelpers.IsReferenceOrContainsReferences<TResult>())
                {
                    ...
                    if (
                        (
                            Unsafe.SizeOf<TResult>() == sizeof(byte)
                            && Unsafe.As<TResult, byte>(ref result) == default(byte)
                        )
                        || (
                            Unsafe.SizeOf<TResult>() == sizeof(ushort)
                            && Unsafe.As<TResult, ushort>(ref result) == default(ushort)
                        )
                        || (
                            Unsafe.SizeOf<TResult>() == sizeof(uint)
                            && Unsafe.As<TResult, uint>(ref result) == default
                        )
                        || (
                            Unsafe.SizeOf<TResult>() == sizeof(ulong)
                            && Unsafe.As<TResult, ulong>(ref result) == default
                        )
                    )
                    {
                        return Task<TResult>.s_defaultResultTask;
        ...
        private static Task Delay(uint millisecondsDelay, CancellationToken cancellationToken) =>
            cancellationToken.IsCancellationRequested
                ...
                : millisecondsDelay == 0
                    ? CompletedTask
                    : cancellationToken.CanBeCanceled
                        ? new DelayPromiseWithCancellation(millisecondsDelay, cancellationToken)
        ...
        public static Task<TResult[]> WhenAll<TResult>(params Task<TResult>[] tasks)
        {
            ...
            int taskCount = tasks.Length;
            if (taskCount == 0)
                return InternalWhenAll<TResult>(tasks); // small optimization in the case of an empty task array
        ...
        public static Task<Task<TResult>> WhenAny<TResult>(params Task<TResult>[] tasks)
        {
            ...
            return intermediate.ContinueWith(
                Task<TResult>.TaskWhenAnyCast.Value,
                default,
                TaskContinuationOptions.ExecuteSynchronously
                    | TaskContinuationOptions.DenyChildAttach,
                TaskScheduler.Default
