...
namespace System.Threading.Tasks
{
    ...
    public enum TaskStatus
    {
        ...
        /// <summary>
        /// The task acknowledged cancellation by throwing an OperationCanceledException with its own
        // CancellationToken
        /// while the token was in signaled state, or the task's CancellationToken was already signaled
        // before the
    ...
    /// purposes. For example, to create a <see cref="Task"/> that runs an action, the factory's
    // StartNew
    /// method may be used:
    /// <code>
    /// // C#
    ...
    // creation
    /// and scheduling must be separated, the constructors may be used, and the task's <see
    // cref="Start()"/>
    ...
    /// </para>
    /// <para>
    /// For developers implementing custom debuggers, several internal and private members of Task may
    // be
    /// useful (these may change from release to release). The Int32 m_taskId field serves as the
    // backing
    /// store for the <see cref="Id"/> property, however accessing this field directly from a debugger
    // may be
    /// more efficient than accessing the same value through the property's getter method (the
    /// s_taskIdCounter Int32 counter is used to retrieve the next available ID for a Task). Similarly,
    // the
    /// Int32 m_stateFlags field stores information about the current lifecycle stage of the Task,
    /// information also accessible through the <see cref="Status"/> property. The m_action
    // System.Object
    /// field stores a reference to the Task's delegate, and the m_stateObject System.Object field
    // stores the
    /// async state passed to the Task by the developer. Finally, for debuggers that parse stack frames,
    // the
    ...
    public class Task : IAsyncResult, IDisposable
    {
        ...
        private TaskStateFlags StateFlags =>
            (TaskStateFlags)(m_stateFlags & ~(int)TaskStateFlags.OptionsMask); // Private property used to help with debugging
        ...
        // When true the Async Causality logging trace is enabled as well as a dictionary to relate
        // operation ids with Tasks
        ...
        // We moved a number of Task properties into this class.  The idea is that in most cases, these
        // properties never
        // need to be accessed during the life cycle of a Task, so we don't want to instantiate them every
        // time.  Once
        // one of these properties needs to be written, we will instantiate a ContingentProperties object
        // and set
        ...
        internal sealed class ContingentProperties
        {
            ...
            /// This should be called when we know the registration isn't useful anymore. Specifically from
            // Finish() if the task has completed
        ...
        // This field will only be instantiated to some non-null value if any ContingentProperties need to
        // be set.
        ...
        internal Task(object? state, TaskCreationOptions creationOptions, bool promiseStyle)
        {
            ...
            // Check the creationOptions. We allow the AttachedToParent option to be specified for promise
            // tasks.
        ...
        /// <summary>
        /// Initializes a new <see cref="Task"/> with the specified action and <see
        ...
        )
        {
            ...
            Debug.Assert(
                m_contingentProperties == null || m_contingentProperties.m_capturedContext == null,
        ...
        ///     Task()
        ///     Task(object action, object state, Task parent, TaskCreationOptions options, TaskScheduler
        // taskScheduler)
        /// </summary>
        /// <param name="action">Action for task to execute.</param>
        /// <param name="state">Object to which to pass to action (may be null)</param>
        /// <param name="scheduler">Task scheduler on which to run thread (only used by continuation
        // tasks).</param>
        /// <param name="cancellationToken">A CancellationToken for the Task.</param>
        ...
        )
        {
            ...
            int illegalInternalOptions = (int)(
                internalOptions
                & ~(
                    InternalTaskOptions.PromiseTask
            ...
            Debug.Assert(
                (((int)creationOptions) | (int)TaskStateFlags.OptionsMask)
                    == (int)TaskStateFlags.OptionsMask,
            ...
            // we need to do this as the very last thing in the construction path, because the CT registration
            // could modify m_stateFlags
            if (cancellationToken.CanBeCanceled)
            {
                Debug.Assert(
                    (internalOptions & InternalTaskOptions.ContinuationTask) == 0,
                    "TaskConstructorCore: Did not expect to see cancelable token for continuation task."
                );
        ...
        /// <summary>
        /// Handles everything needed for associating a CancellationToken with a task which is being
        // constructed.
        ...
        {
            // There is no need to worry about concurrency issues here because we are in the constructor path of
            // the task --
            ...
            try
            {
                // If an unstarted task has a valid CancellationToken that gets signalled while the task is still
                // not queued
                ...
                // We exclude Promise tasks from this, because TaskCompletionSource needs to fully control the inner
                // tasks's lifetime (i.e. not allow external cancellations)
                if (
                    (
                    ...
                    else
                    {
                        ...
                        if (antecedent == null)
                        {
                            ...
                            ctr = cancellationToken.UnsafeRegister(
                                static t => ((Task)t!).InternalCancel(),
                                this
                            );
                        }
                        else
                        {
                            ...
                            // If an antecedent was specified, pack this task, its antecedent and the TaskContinuation together
                            // as a tuple
                            // and use it as the cancellation state object. This will be unpacked in the cancellation callback
                            // so that
                            ...
                            ctr = cancellationToken.UnsafeRegister(
                                ...
                                new TupleSlim<Task, Task, TaskContinuation>(
                                    this,
                                    antecedent,
                                    continuation
                                )
                        ...
                        props.m_cancellationRegistration =
                            new StrongBox<CancellationTokenRegistration>(ctr);
                    }
                }
            }
            catch
        ...
        {
            Debug.Assert(
                ((int)TaskStateFlags.OptionsMask & 1) == 1,
        ...
        internal bool AtomicStateUpdate(int newBits, int illegalBits)
        {
            ...
            return (oldFlags & illegalBits) == 0
                && (
                    Interlocked.CompareExchange(ref m_stateFlags, oldFlags | newBits, oldFlags)
                        == oldFlags
        ...
        private bool AtomicStateUpdateSlow(int newBits, int illegalBits)
        {
            ...
            while (true)
            {
                if ((flags & illegalBits) != 0)
                    return false;
                int oldFlags = Interlocked.CompareExchange(
                    ref m_stateFlags,
                    flags | newBits,
                    flags
                );
        ...
        internal void SetNotificationForWaitCompletion(bool enabled)
        {
            ...
            if (enabled)
            {
                ...
                bool success = AtomicStateUpdate(
        ...
        /// <summary>Gets whether either the end await bit is set or (not xor) the task has not completed
        ...
        /// <summary>Placeholder method used as a breakpoint target by the debugger.  Must not be inlined or
        // optimized.</summary>
        /// <remarks>All joins with a task should end up calling this if their debugger notification bit is
        // set.</remarks>
        ...
        internal bool MarkStarted()
        {
            return AtomicStateUpdate(
                ...
                (int)TaskStateFlags.Canceled | (int)TaskStateFlags.Started
            );
        }
        ...
        // This is called in the case where a new child is added, but then encounters a
        // CancellationToken-related exception.
        ...
        public void Start(TaskScheduler scheduler)
        {
            ...
            if (IsCompletedMethod(flags))
            {
                ThrowHelper.ThrowInvalidOperationException(
                    ExceptionResource.Task_Start_TaskCompleted
                );
            }
        ...
        internal void InternalRunSynchronously(TaskScheduler scheduler, bool waitForCompletion)
        {
            ...
            if (MarkStarted())
            {
                ...
                {
                    // we received an unexpected exception originating from a custom scheduler, which needs to be
                    // wrapped in a TSE and thrown
                    if (!taskQueued)
                    {
            ...
            {
                Debug.Assert(
                    (m_stateFlags & (int)TaskStateFlags.Canceled) != 0,
                    "Task.RunSynchronously: expected TaskStateFlags.Canceled to be set"
                );
        ...
        )
        {
            ...
            // Here we add the InternalTaskOptions.QueuedByRuntime to the internalOptions, so that
            // TaskConstructorCore can skip the cancellation token registration
            Task t = new Task(
                action,
                state,
                creatingTask,
        ...
        /// <returns>The current task if there is one and if AttachToParent is in the options; otherwise,
        // null.</returns>
        ...
        /// </summary>
        /// <remarks>
        /// Tasks that throw unhandled exceptions store the resulting exception and propagate it wrapped in
        // a
        ...
        public TaskStatus Status
        {
            get
            {
                ...
                if ((sf & (int)TaskStateFlags.Faulted) != 0)
                    rval = TaskStatus.Faulted;
                else if ((sf & (int)TaskStateFlags.Canceled) != 0)
                    rval = TaskStatus.Canceled;
                else if ((sf & (int)TaskStateFlags.RanToCompletion) != 0)
                    rval = TaskStatus.RanToCompletion;
                else if ((sf & (int)TaskStateFlags.WaitingOnChildren) != 0)
                    rval = TaskStatus.WaitingForChildrenToComplete;
                else if ((sf & (int)TaskStateFlags.DelegateInvoked) != 0)
                    rval = TaskStatus.Running;
                else if ((sf & (int)TaskStateFlags.Started) != 0)
                    rval = TaskStatus.WaitingToRun;
                else if ((sf & (int)TaskStateFlags.WaitingForActivation) != 0)
        ...
        /// </summary>
        /// <remarks>
        /// A <see cref="Task">Task</see> will complete in Canceled state either if its <see
        // cref="CancellationToken">CancellationToken</see>
        /// was marked for cancellation before the task started executing, or if the task acknowledged the
        // cancellation request on
        ...
        public bool IsCanceled =>
            ...
            (m_stateFlags & ((int)TaskStateFlags.Canceled | (int)TaskStateFlags.Faulted))
            == (int)TaskStateFlags.Canceled;
        ...
        /// <summary>
        /// Gets whether this <see cref="Task"/> threw an OperationCanceledException while its
        // CancellationToken was signaled.
        /// </summary>
        internal bool IsCancellationAcknowledged =>
            (m_stateFlags & (int)TaskStateFlags.CancellationAcknowledged) != 0;
        ...
        public bool IsCompletedSuccessfully =>
            (m_stateFlags & (int)TaskStateFlags.CompletedMask)
            == (int)TaskStateFlags.RanToCompletion;
        ...
        public TaskCreationOptions CreationOptions =>
            Options & (TaskCreationOptions)(~InternalTaskOptions.InternalOptionsMask);
        ...
        public object? AsyncState =>
            (m_stateFlags & (int)InternalTaskOptions.HiddenState) == 0 ? m_stateObject : null;
        ...
        /// <summary>
        /// Provides access to factory methods for creating <see cref="Task"/> and <see
        // cref="Task{TResult}"/> instances.
        /// </summary>
        /// <remarks>
        ...
        /// <remarks>It's a <see cref="Task{VoidTaskResult}"/> so it can be shared with <see
        // cref="AsyncTaskMethodBuilder"/>.</remarks>
        internal static readonly Task<VoidTaskResult> s_cachedCompleted = new Task<VoidTaskResult>(
            false,
            default,
        ...
        /// Only called by IAsyncResult.AsyncWaitHandle, which means that we really do need to instantiate a
        // completion event.
        /// </summary>
        internal ManualResetEventSlim CompletedEvent
        {
            get
            {
                ...
                if (contingentProps.m_completionEvent == null)
                {
                    ...
                    if (
                        Interlocked.CompareExchange(
                            ref contingentProps.m_completionEvent,
                            newMre,
                            null
                        ) != null
                    )
        ...
        internal bool ExceptionRecorded
        {
            get
            {
                ContingentProperties? props = Volatile.Read(ref m_contingentProperties);
                return (props != null)
                    && (props.m_exceptionsHolder != null)
                    && (props.m_exceptionsHolder.ContainsFaultList);
            }
        }
        ...
        /// If the TaskStateFlags.ExecutionContextIsNull flag is set, this means ExecutionContext.Capture
        // returned null, otherwise
        /// If the captured context is the default, nothing is saved, otherwise the m_contingentProperties
        ...
        internal ExecutionContext? CapturedContext
        {
            ...
            set
            {
                // There is no need to atomically set this bit because this set() method is only called during
                // construction, and therefore there should be no contending accesses to m_stateFlags
                if (value == null)
                {
                    m_stateFlags |= (int)TaskStateFlags.ExecutionContextIsNull;
                }
        ...
        internal void ScheduleAndStart(bool needsProtection)
        {
            ...
            Debug.Assert(
                (m_stateFlags & (int)TaskStateFlags.Started) == 0,
            ...
            {
                // For all other task than TaskContinuations we want to log. TaskContinuations log in their
                // constructor
            ...
            catch (Exception e)
            {
                ...
                // Now we need to mark ourselves as "handled" to avoid crashing the finalizer thread if we are
                // called from StartNew(),
                // because the exception is either propagated outside directly, or added to an enclosing parent.
                // However we won't do this for
                // continuation tasks, because in that case we internally eat the exception and therefore we need to
        ...
        /// </summary>
        /// <param name="exceptionObject">An object representing either an Exception or a collection of
        // Exceptions.</param>
        ...
        /// <param name="exceptionObject">An object representing either an Exception or a collection of
        // Exceptions.</param>
        /// <param name="representsCancellation">Whether the exceptionObject is an
        // OperationCanceledException representing cancellation.</param>
        internal void AddException(object exceptionObject, bool representsCancellation)
        {
            ...
            Debug.Assert(
                eoAsException != null
                    || eoAsEnumerableException != null
                    || eoAsEdi != null
                    || eoAsEnumerableEdi != null,
        ...
        private AggregateException? GetExceptions(bool includeTaskCanceledExceptions)
        {
            ...
            if (ExceptionRecorded)
            {
                ...
                Debug.Assert(
                    m_contingentProperties != null
                        && m_contingentProperties.m_exceptionsHolder != null,
                    "ExceptionRecorded should imply this"
                );
        ...
        /// <summary>Gets the ExceptionDispatchInfo containing the OperationCanceledException for this
        // task.</summary>
        ...
        /// <param name="targetContext">The target context on which to propagate the exception.  Null to use
        // the ThreadPool.</param>
        ...
        internal bool IsExceptionObservedByParent =>
            (m_stateFlags & (int)TaskStateFlags.ExceptionObservedByParent) != 0;

        /// <summary>
        /// Checks whether the body was ever invoked. Used by task scheduler code to verify custom
        // schedulers actually ran the task.
        /// </summary>
        internal bool IsDelegateInvoked =>
            (m_stateFlags & (int)TaskStateFlags.DelegateInvoked) != 0;
        ...
        private void FinishSlow(bool userDelegateExecute)
        {
            ...
            else
            {
                ...
                {
                    // Apparently some children still remain. It will be up to the last one to process the completion of
                    // this task on their own thread.
                    ...
                    // Otherwise we risk overwriting the TaskStateFlags.RanToCompletion, _CANCELED or _FAULTED bit which
                    // may have been set by that child task.
                    // Note that the concurrent update by the last child happening in FinishStageTwo could still wipe
                    // out the TaskStateFlags.WaitingOnChildren flag,
                    // but it is not critical to maintain, therefore we dont' need to intruduce a full atomic update
                    // into FinishStageTwo

                    AtomicStateUpdate(
                        (int)TaskStateFlags.WaitingOnChildren,
                ...
                // Now is the time to prune exceptional children. We'll walk the list and removes the ones whose
                // exceptions we might have observed after they threw.
                // we use a local variable for exceptional children here because some other thread may be nulling
                // out m_contingentProperties.m_exceptionalChildren
        ...
        /// It can happen i) either on the thread that originally executed this task (if no children were
        // spawned, or they all completed by the time this task's delegate quit)
        ...
        private void FinishStageTwo()
        {
            ...
            {
                // We transition into the TaskStateFlags.Canceled final state if the task's CT was signalled for
                // cancellation,
                // and the user delegate acknowledged the cancellation request by throwing an OCE,
                // and the task hasn't otherwise transitioned into faulted state. (TaskStateFlags.Faulted trumps
                // TaskStateFlags.Canceled)
                //
                // If the task threw an OCE without cancellation being requestsed (while the CT not being in
                // signaled state),
        ...
        /// <summary>
        /// Final stage of the task completion code path. Notifies the parent (if any) that another of its
        // children are done, and runs continuations.
        /// This function is only separated out from FinishStageTwo because these two operations are also
        // needed to be called from CancellationCleanupLogic()
        /// </summary>
        ...
        internal void NotifyParentIfPotentiallyAttachedTask()
        {
            ...
            if (
                ...
                && ((parent.CreationOptions & TaskCreationOptions.DenyChildAttach) == 0)
                && (
                    ((TaskCreationOptions)(m_stateFlags & (int)TaskStateFlags.OptionsMask))
                    & TaskCreationOptions.AttachedToParent
                ) != 0
            )
            {
        ...
        internal void ProcessChildCompletion(Task childTask)
        {
            ...
            Debug.Assert(
                childTask.m_contingentProperties?.m_parent == this,
            ...
            if (childTask.IsFaulted && !childTask.IsExceptionObservedByParent)
            {
                ...
                {
                    Interlocked.CompareExchange(
                        ref props.m_exceptionalChildren,
                        new List<Task>(),
                        null
                    );
                }

                // In rare situations involving AppDomainUnload, it's possible (though unlikely) for
                // FinishStageTwo() to be called
                // multiple times for the same task.  In that case, AddExceptionsFromChildren() could be nulling
                // m_exceptionalChildren
                // out at the same time that we're processing it, resulting in a NullReferenceException here.  We'll
                // protect
            ...
            {
                // This call came from the final child to complete, and apparently we have previously given up this
                // task's right to complete itself.
        ...
        /// It traverses the list of exceptional children, and appends their aggregate exceptions into this
        // one's exception list
        /// </summary>
        internal void AddExceptionsFromChildren(ContingentProperties props)
        {
            Debug.Assert(props != null);

            // In rare occurrences during AppDomainUnload() processing, it is possible for this method to be
            // called
            // simultaneously on the same task from two different contexts.  This can result in
            // m_exceptionalChildren
            ...
            if (exceptionalChildren != null)
            {
                // This lock is necessary because even though AddExceptionsFromChildren is last to execute, it may
                // still
                // be racing with the code segment at the bottom of Finish() that prunes the exceptional child
                // array.
                lock (exceptionalChildren)
                {
                    foreach (Task task in exceptionalChildren)
                    {
                        ...
                        {
                            TaskExceptionHolder? exceptionHolder = Volatile
                                .Read(ref task.m_contingentProperties)!
                                .m_exceptionsHolder;
        ...
        /// <summary>
        /// Outermost entry function to execute this task. Handles all aspects of executing a task on the
        // caller thread.
        /// </summary>
        internal bool ExecuteEntry()
        {
            // Do atomic state transition from queued to invoked. If we observe a task that's already invoked,
            // we will return false so that TaskScheduler.ExecuteTask can throw an exception back to the custom
            // scheduler.
            // However we don't want this exception to be throw if the task was already canceled, because it's a
            // legitimate scenario for custom schedulers to dequeue a task and mark it as canceled (example:
            ...
            if (
                ...
                && (previousState & (int)TaskStateFlags.Canceled) == 0
            )
            {
        ...
        )
        {
            ...
            if (etwIsEnabled)
            {
                if (log.TasksSetActivityIds)
                    EventSource.SetCurrentThreadActivityId(
                        TplEventSource.CreateGuidForTaskID(this.Id),
                        out savedActivityID
                    );
            ...
            finally
            {
                ...
                if (etwIsEnabled)
                {
                    ...
                    if (previousTask != null)
                        log.TaskCompleted(
                            previousTask.m_taskScheduler!.Id,
                            previousTask.Id,
                            this.Id,
                            IsFaulted
                        );
        ...
        private void HandleException(Exception unhandledException)
        {
            ...
            if (
                unhandledException is OperationCanceledException exceptionAsOce
        ...
        /// </summary>
        /// <param name="continuationAction">The action to invoke when the <see cref="Task"/> has
        // completed.</param>
        /// <param name="continueOnCapturedContext">
        /// true to attempt to marshal the continuation back to the original context captured; otherwise,
        // false.
        ...
        /// <exception cref="InvalidOperationException">The awaiter was not properly
        ...
        )
        {
            ...
            if (continueOnCapturedContext)
            {
                ...
                if (
                    SynchronizationContext.Current is SynchronizationContext syncCtx
                ...
                if (
                    TaskScheduler.InternalCurrent is TaskScheduler scheduler
            ...
            // Now register the continuation, and if we couldn't register it because the task is already
            // completing,
        ...
        /// otherwise in the current task scheduler.
        /// </summary>
        /// <param name="stateMachineBox">The action to invoke when the <see cref="Task"/> has
        // completed.</param>
        /// <param name="continueOnCapturedContext">
        /// true to attempt to marshal the continuation back to the original context captured; otherwise,
        // false.
        ...
        )
        {
            ...
            // method unless you've checked that TplEventSource.Log.IsEnabled() is false (if it becomes true
            // after
        ...
        /// <summary>Creates an awaitable that asynchronously yields back to the current context when
        // awaited.</summary>
        /// <returns>
        /// A context that, when awaited, will asynchronously transition back into the current context at
        // the
        /// time of the await. If the current SynchronizationContext is non-null, that is treated as the
        // current context.
        ...
        /// Waits for the <see cref="Task"/> to complete execution.
        /// </summary>
        /// <param name="timeout">The time to wait, or <see cref="Timeout.InfiniteTimeSpan"/> to wait
        // indefinitely</param>
        ...
        /// <summary>Gets a <see cref="Task"/> that will complete when this <see cref="Task"/> completes or
        // when the specified <see cref="CancellationToken"/> has cancellation requested.</summary>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> to monitor for a
        // cancellation request.</param>
        /// <returns>The <see cref="Task"/> representing the asynchronous wait.  It may or may not be the
        // same instance as the current instance.</returns>
        public Task WaitAsync(CancellationToken cancellationToken) =>
            WaitAsync(Timeout.UnsignedInfinite, TimeProvider.System, cancellationToken);

        /// <summary>Gets a <see cref="Task"/> that will complete when this <see cref="Task"/> completes or
        // when the specified timeout expires.</summary>
        ...
        /// <returns>The <see cref="Task"/> representing the asynchronous wait.  It may or may not be the
        // same instance as the current instance.</returns>
        /// <exception cref="ArgumentNullException">The <paramref name="timeProvider"/> argument is
        // null.</exception>
        ...
        /// <summary>Gets a <see cref="Task"/> that will complete when this <see cref="Task"/> completes,
        // when the specified timeout expires, or when the specified <see cref="CancellationToken"/> has
        ...
        )
        {
            ...
            return new CancellationPromise<VoidTaskResult>(
                this,
                millisecondsTimeout,
                timeProvider,
                cancellationToken
        ...
        /// <summary>Task that's completed when another task, timeout, or cancellation token
        // triggers.</summary>
        private protected sealed class CancellationPromise<TResult>
            : Task<TResult>,
                ITaskCompletionAction
        {
            /// <summary>The source task.  It's stored so that we can remove the continuation from it upon
            // timeout or cancellation.</summary>
            private readonly Task _task;

            /// <summary>Cancellation registration used to unregister from the token source upon timeout or the
            // task completing.</summary>
            private readonly CancellationTokenRegistration _registration;

            /// <summary>The timer used to implement the timeout.  It's stored so that it's rooted and so that
            // we can dispose it upon cancellation or the task completing.</summary>
            ...
            )
            {
                ...
                // If one of the callbacks fired, it's possible they did so prior to our having registered the other
                // callbacks,
                // and thus cleanup may have missed those additional registrations.  Just in case, check here, and
                // if we're
        ...
        /// <summary>
        /// The core wait function, which is only accessible internally. It's meant to be used in places in
        // TPL code where
        ...
        // Separated out to allow it to be optimized (caller is marked NoOptimization for VS parallel
        // debugger
        ...
        private bool InternalWaitCore(int millisecondsTimeout, CancellationToken cancellationToken)
        {
            ...
            //    2. If the task runs inline but takes some time to complete, it will suffer ThreadAbort with
            // possible state corruption,
            //       and it is best to prevent this unless the user explicitly asks to view the value with
            // thread-slipping enabled.
        ...
        private bool SpinThenBlockingWait(
...
#pragma warning disable CA1416 // Validate platform compatibility, issue: https://github.com/dotnet/runtime/issues/44622
                    ...
                    else
                    {
                        ...
                        if (elapsedTimeTicks < millisecondsTimeout)
                        {
                            ...
                            try
                            {
                                returnValue = mres.Wait(
                                    (int)(millisecondsTimeout - elapsedTimeTicks),
                                    cancellationToken
                                );
                            }
                            finally
...
#pragma warning restore CA1416
        ...
        internal void InternalCancel()
        {
            ...
            if ((m_stateFlags & (int)TaskStateFlags.Started) != 0)
            {
                ...
                {
                    // TryDequeue threw. We don't know whether the task was properly dequeued or not. So we must let the
                    // rest of
                    // the cancellation logic run its course (record the request, attempt atomic state transition and do
            ...
            // Note that we do not check for TaskStateFlags.CompletionReserved.  That only applies to
            // promise-style
            ...
            if (popped)
            {
                ...
                mustCleanup = AtomicStateUpdate(
                    (int)TaskStateFlags.Canceled,
                    (int)TaskStateFlags.Canceled | (int)TaskStateFlags.DelegateInvoked
                );
            }
        ...
        {
            // There isn't a cancellation token assigned to the task, which means the task is still going to be
            // in its
            ...
            // - it was run as a continuation, which won't have happened because this method is only invoked
            // once
            // As a result, we can take an optimized path that avoids inflating contingent properties.
            const int IllegalFlags =
                (int)TaskStateFlags.Started
                | (int)TaskStateFlags.CompletedMask
                | (int)TaskStateFlags.DelegateInvoked;
            Debug.Assert(
                (m_stateFlags & IllegalFlags) == 0,
                "The continuation was in an invalid state."
            );
            Debug.Assert(
                (m_stateFlags & (int)TaskStateFlags.WaitingForActivation) != 0,
                "Expected continuation to be waiting for activation"
            );
            Debug.Assert(
                m_contingentProperties is null
        ...
        {
            // Record the cancellation request.
            EnsureContingentPropertiesInitialized().m_internalCancellationRequested =
        ...
        )
        {
            ...
            if (cancellationException != null)
            {
                ...
                Debug.Assert(
                    oce.CancellationToken == tokenToRecord,
        ...
        internal void CancellationCleanupLogic()
        {
            Debug.Assert(
                (
                    m_stateFlags
                    & ((int)TaskStateFlags.Canceled | (int)TaskStateFlags.CompletionReserved)
                ) != 0,
                "Task.CancellationCleanupLogic(): Task not canceled or reserved."
            );
            // We'd like to be able to:
            //     Debug.Assert((m_completionEvent == null) || !m_completionEvent.IsSet,
            // "Task.CancellationCleanupLogic(): Completion event already set.");
            // However, there is a small window for a race condition.  If someone calls Wait() between
            // InternalCancel() and
        ...
        /// <returns>true if the task was transitioned to ran to completion; false if it was already
        // completed.</returns>
        ...
        // internal helper function breaks out logic used by TaskCompletionSource and AsyncMethodBuilder
        // If the tokenToRecord is not None, it will be stored onto the task.
        // If the OperationCanceledException is not null, it will be stored into the task's exception
        // holder.
        // This method is only valid for promise tasks.
        internal bool TrySetCanceled(CancellationToken tokenToRecord, object? cancellationException)
        {
            Debug.Assert(
                cancellationException == null
                    ...
                    || (cancellationException as ExceptionDispatchInfo)?.SourceException
        ...
        {
            // Atomically store the fact that this task is completing.  From this point on, the adding of
            // continuations will
            // result in the continuations being run/launched directly rather than being added to the
            // continuation list.
            // Then if we grabbed any continuations, run them.
            object? continuationObject = Interlocked.Exchange(
                ref m_continuationObject,
        ...
        private void RunContinuations(object continuationObject) // separated out of FinishContinuations to enable it to be inlined
        {
            ...
            {
                // Handle the single IAsyncStateMachineBox case.  This could be handled as part of the
                // ITaskCompletionAction
                // but we want to ensure that inlining is properly handled in the face of schedulers, so its
                // behavior
                // needs to be customized ala raw Actions.  This is also the most important case, as it represents
                // the
            ...
            // Fire the asynchronous continuations first. However, if we're not able to run any continuations
            // synchronously,
            // then we can skip this first pass, since the second pass that tries to run everything
            // synchronously will instead
            ...
            if (canInlineContinuations)
            {
                ...
                {
                    // For StandardTaskContinuations, we respect the TaskContinuationOptions.ExecuteSynchronously
                    // option,
                    // as the developer needs to explicitly opt-into running the continuation synchronously, and if they
                    // do,
                    // they get what they asked for. ITaskCompletionActions are only ever created by the runtime, and we
                    // always
                    // try to execute them synchronously. For all other continuations (related to await), we only run it
                    // synchronously
                    // if it's the first such continuation; otherwise, we force it to run asynchronously so as to not
                    // artificially
                    // delay an await continuation behind other arbitrary user code created as a previous await
                    // continuation.
                        ...
                        if (forceContinuationsAsync)
                        {
                            ...
                            switch (currentContinuation)
                            {
                                ...
                                default:
                                    ...
                                    ((TaskContinuation)currentContinuation).Run(
                                        this,
                                        canInlineContinuationTask: false
                                    );
                                    break;
                            }
                        }
                ...
                switch (currentContinuation)
                {
                    ...
                    default:
                        ...
                        RunOrQueueCompletionAction(
                            (ITaskCompletionAction)currentContinuation,
                            canInlineContinuations
                        );
                        break;
                }
        ...
        )
        {
            ...
            else
            {
                ThreadPool.UnsafeQueueUserWorkItemInternal(
                    new CompletionActionInvoker(completionAction, this),
                    preferLocal: true
                );
            }
        }
        ...
        /// passed the completed task as an argument.
        /// </param>
        /// <param name="cancellationToken"> The <see cref="CancellationToken"/> that will be assigned to
        ...
        /// <remarks>
        /// The returned <see cref="Task{TResult}"/> will not be scheduled for execution until the current
        // task has
        /// completed, whether it completes due to running to completion successfully, faulting due to an
        /// unhandled exception, or exiting out early due to being canceled.
        /// </remarks>
        ...
        )
        {
            ...
            const TaskContinuationOptions IllegalMask =
            ...
            if ((continuationOptions & IllegalMask) == IllegalMask)
            {
                ThrowHelper.ThrowArgumentOutOfRangeException(
                    ExceptionArgument.continuationOptions,
        ...
        )
        {
            ...
            Debug.Assert(
                !continuationTask.IsCompleted,
                "Did not expect continuationTask to be completed"
            );

            // Create a TaskContinuation
            var continuation = new ContinueWithTaskContinuation(
                continuationTask,
                options,
                scheduler
            );
            ...
            if (!continuationTask.IsCompleted)
            {
                ...
                if (
                    (this.Options & (TaskCreationOptions)InternalTaskOptions.PromiseTask) != 0
        ...
        internal void AddCompletionAction(
            ITaskCompletionAction action,
            bool addBeforeOthers = false
        )
        {
        ...
        private bool AddTaskContinuationComplex(object tc, bool addBeforeOthers)
        {
            ...
            Debug.Assert(
                (list != null) || (m_continuationObject == s_taskCompletionSentinel),
        ...
        private bool AddTaskContinuation(object tc, bool addBeforeOthers)
        {
            ...
            // Make sure that, if someone calls ContinueWith() right after waiting for the predecessor to
            // complete,
            ...
            if (
                ...
                || (Interlocked.CompareExchange(ref m_continuationObject, tc, null) != null)
            )
            {
        ...
        {
            // We need to snap a local reference to m_continuations since reading a volatile object is more
            // costly.
            ...
            if (continuationsLocalListRef is null)
            {
                ...
                {
                    // If we fail it means that either AddContinuationComplex won the race condition and
                    // m_continuationObject is now a List
                    // that contains the element we want to remove. Or FinishContinuations set the
                    // s_taskCompletionSentinel.
            ...
            // if continuationsLocalRef == null it means s_taskCompletionSentinel has been set already and there
        ...
        /// </exception>
        /// <exception cref="AggregateException">
        /// At least one of the <see cref="Task"/> instances was canceled -or- an exception was thrown
        // during
        ...
        )
        {
            ...
            // We make sure that the exception behavior of Task.Wait() is replicated the same for tasks handled
            // in either of these codepaths
            //
            ...
            for (int i = tasks.Length - 1; i >= 0; i--)
            {
                ...
                if (!taskIsCompleted)
                {
                    ...
                    {
                        // We either didn't attempt inline execution because we had a non-infinite timeout or we had a
                        // cancellable token.
            ...
            if (waitedOnTaskList != null)
            {
                ...
                returnValue = WaitAllBlockingCore(
                    waitedOnTaskList,
                    millisecondsTimeout,
                    cancellationToken
                );
        ...
        /// <param name="tasks">The tasks, which have already been checked and filtered for
        // completion.</param>
        /// <param name="millisecondsTimeout">The timeout.</param>
        ...
        //      Action<Task> completionAction = delegate { if (Interlocked.Decrement(ref count) == 0)
        // mres.Set(); };
        ...
        /// An array of <see cref="Task"/> instances on which to wait.
        /// </param>
        /// <returns>The index of the completed task in the <paramref name="tasks"/> array
        // argument.</returns>
        /// <exception cref="ArgumentNullException">
        /// The <paramref name="tasks"/> argument is null.
        /// </exception>
        ...
        )
        {
            ...
            {
                Task<Task> firstCompleted = TaskFactory.CommonCWAnyLogic(
                    tasks,
                    isSyncBlocking: true
                );
        ...
        /// <summary>Gets a <see cref="Task{TResult}"/> that's completed successfully with the specified
        // result.</summary>
...
#pragma warning disable 8500 // address of / sizeof of managed types
            // The goal of this function is to be give back a cached task if possible, or to otherwise give back
            // a new task.
            // To give back a cached task, we need to be able to evaluate the incoming result value, and we need
            // to avoid as
            // much overhead as possible when doing so, as this function is invoked as part of the return path
            // from every async
            // method and Task.FromResult. Most tasks won't be cached, and thus we need the checks for those
            // that are to be as
            // close to free as possible. This requires some trickiness given the lack of generic specialization
            // in .NET.
            ...
            if (typeof(TResult) == typeof(int))
            {
                ...
                if (
                    (uint)(value - TaskCache.InclusiveInt32Min)
                    < (TaskCache.ExclusiveInt32Max - TaskCache.InclusiveInt32Min)
                )
                {
            ...
            {
                // For other value types, we special-case default(TResult) if we can efficiently compare bit
                // patterns to default/0,
                // which means any value type that's 1, 2, 4, 8, or 16 bytes in size and that doesn't contain
                // references.
                // We don't need to go through the equality operator of the TResult because we cached a task for
                // default(TResult),
                // so we only need to confirm that this TResult has the same bits as default(TResult).
                if (
                    (sizeof(TResult) == sizeof(byte) && *(byte*)&result == default(byte))
...
#pragma warning restore 8500
        ...
        /// <summary>Creates a <see cref="Task{TResult}"/> that's completed exceptionally with the specified
        // exception.</summary>
        ...
        /// <summary>Creates a <see cref="Task"/> that's completed due to cancellation with the specified
        // token.</summary>
        ...
        /// <summary>Creates a <see cref="Task{TResult}"/> that's completed due to cancellation with the
        ...
        /// </summary>
        /// <param name="action">The work to execute asynchronously</param>
        /// <param name="cancellationToken">A cancellation token that should be used to cancel the
        // work</param>
        /// <returns>A Task that represents the work queued to execute in the ThreadPool.</returns>
        /// <exception cref="ArgumentNullException">
        /// The <paramref name="action"/> parameter was null.
        ...
        /// <summary>
        /// Queues the specified work to run on the ThreadPool and returns a Task(TResult) handle for that
        // work.
        /// </summary>
        ...
        public static Task Run(Func<Task?> function, CancellationToken cancellationToken)
        {
            ...
            // Set lookForOce == true so that unwrap logic can be on the lookout for OCEs thrown as faults from
            // task1, to support in-delegate cancellation.
            UnwrapPromise<VoidTaskResult> promise = new UnwrapPromise<VoidTaskResult>(
                task1,
                lookForOce: true
            );
        ...
        /// <param name="delay">The <see cref="TimeSpan"/> to wait before completing the returned task, or
        // <see cref="Timeout.InfiniteTimeSpan"/> to wait indefinitely.</param>
        /// <param name="timeProvider">The <see cref="TimeProvider"/> with which to interpret <paramref
        // name="delay"/>.</param>
        /// <returns>A task that represents the time delay.</returns>
        /// <exception cref="ArgumentOutOfRangeException"><paramref name="delay"/> represents a negative
        // time interval other than <see cref="Timeout.InfiniteTimeSpan"/>.</exception>
        /// <exception cref="ArgumentOutOfRangeException"><paramref name="delay"/>'s <see
        // cref="TimeSpan.TotalMilliseconds"/> property is greater than 4294967294.</exception>
        /// <exception cref="ArgumentNullException">The <paramref name="timeProvider"/> argument is
        // null.</exception>
        ...
        /// </summary>
        /// <param name="delay">The time span to wait before completing the returned Task</param>
        /// <param name="cancellationToken">The cancellation token that will be checked prior to completing
        // the returned Task</param>
        /// <returns>A Task that represents the time delay</returns>
        /// <exception cref="ArgumentOutOfRangeException">
        /// The <paramref name="delay"/> is less than -1 or greater than the maximum allowed timer duration.
        /// </exception>
        ...
        /// </exception>
        /// <remarks>
        /// If the cancellation token is signaled before the specified time delay, then the Task is
        // completed in
        /// Canceled state.  Otherwise, the Task is completed in RanToCompletion state once the specified
        // time
        /// delay has expired.
        /// </remarks>
        ...
        /// <param name="timeProvider">The <see cref="TimeProvider"/> with which to interpret <paramref
        // name="delay"/>.</param>
        /// <param name="cancellationToken">A cancellation token to observe while waiting for the task to
        // complete.</param>
        /// <returns>A task that represents the time delay.</returns>
        /// <exception cref="ArgumentOutOfRangeException"><paramref name="delay"/> represents a negative
        // time interval other than <see cref="Timeout.InfiniteTimeSpan"/>.</exception>
        ...
        )
        {
            ArgumentNullException.ThrowIfNull(timeProvider);
            return Delay(
                ValidateTimeout(delay, ExceptionArgument.delay),
                timeProvider,
                cancellationToken
            );
        }
        ...
        /// Creates a Task that will complete after a time delay.
        /// </summary>
        /// <param name="millisecondsDelay">The number of milliseconds to wait before completing the
        // returned Task</param>
        /// <returns>A Task that represents the time delay</returns>
        /// <exception cref="ArgumentOutOfRangeException">
        ...
        public static Task Delay(int millisecondsDelay, CancellationToken cancellationToken)
        {
            ...
            {
                ThrowHelper.ThrowArgumentOutOfRangeException(
                    ExceptionArgument.millisecondsDelay,
        ...
        /// <summary>Task that also stores the completion closure and logic for Task.Delay
        ...
        private class DelayPromise : Task
        {
            ...
            internal DelayPromise(uint millisecondsDelay, TimeProvider timeProvider)
            {
                ...
                if (millisecondsDelay != Timeout.UnsignedInfinite) // no need to create the timer if it's an infinite timeout
                {
                    ...
                    if (IsCompleted)
                    {
                        // Handle rare race condition where the timer fires prior to our having stored it into the field, in
                        // which case
                        // the timer won't have been cleaned up appropriately.  This call to close might race with the
                        // Cleanup call to Close,
        ...
        private sealed class DelayPromiseWithCancellation : DelayPromise
        {
                ...
                : base(millisecondsDelay, timeProvider)
            {
                ...
                if (IsCompleted)
                {
                    // Handle rare race condition where the base timer fires prior to our having stored the cancellation
                    // registration into
                    // the field, in which case the call to Cleanup won't dispose of the registration.  In such a case,
                    // Cleanup will have
                    // already been called to dispose of the base timer, so we can just call Dispose on the registration
                    // directly.  This
        ...
        // in a Faulted state,
        /// where its exceptions will contain the aggregation of the set of unwrapped exceptions from each
        // of the supplied tasks.
        /// </para>
        /// <para>
        /// If none of the supplied tasks faulted but at least one of them was canceled, the returned task
        // will end in the Canceled state.
        /// </para>
        /// <para>
        /// If none of the tasks faulted and none of the tasks were canceled, the resulting task will end in
        // the RanToCompletion state.
        /// </para>
        /// <para>
        /// If the supplied array/enumerable contains no tasks, the returned task will immediately
        // transition to a RanToCompletion
        ...
        /// </para>
        /// <para>
        /// If the supplied span contains no tasks, the returned task will immediately transition to a
        // RanToCompletion
        /// state before it's returned to the caller.
        /// </para>
        /// </remarks>
        /// <exception cref="ArgumentException">The <paramref name="tasks"/> array contained a null
        // task.</exception>
            ...
            public void Invoke(Task? completedTask)
            {
                ...
                if (completedTask is not null)
                {
                    ...
                    if (!completedTask.IsCompletedSuccessfully)
                    {
                        ...
                        if (
                            Interlocked.CompareExchange(ref _failedOrCanceled, completedTask, null)
                            != null
                        )
                        {
                            ...
                            while (true)
                            {
                                ...
                                Debug.Assert(
                                    ...
                                    $"Expected Task, got {failedOrCanceled}"
                                );
                                if (
                                    Interlocked.CompareExchange(
                                        ref _failedOrCanceled,
                                        new List<Task> { (Task)failedOrCanceled, completedTask },
                                        failedOrCanceled
                                    ) == failedOrCanceled
                                )
                                {
                ...
                if (Interlocked.Decrement(ref _remainingToComplete) == 0)
                {
                    ...
                    else
                    {
                        ...
                        void HandleTask(Task task)
                        {
                            if (task.IsFaulted)
                            {
                                (observedExceptions ??= new()).AddRange(
                                    task.GetExceptionDispatchInfos()
                                );
                            }
                            else if (task.IsCanceled)
                            {
                        ...
                        {
                            Debug.Assert(
                                observedExceptions.Count > 0,
                                "Expected at least one exception"
                            );

                            // We don't need to TraceOperationCompleted here because TrySetException will call Finish and we'll
                            // log it there
        ...
        /// supplied tasks in the same order as they were provided (e.g. if the input tasks array contained
        // t1, t2, t3, the output
        /// task's Result will return an TResult[] where arr[0] == t1.Result, arr[1] == t2.Result, and
        // arr[2] == t3.Result).
        /// </para>
        /// <para>
        /// If the supplied array/enumerable contains no tasks, the returned task will immediately
        // transition to a RanToCompletion
        /// state before it's returned to the caller.  The returned TResult[] will be an array of 0
        // elements.
        /// </para>
        /// </remarks>
        /// <exception cref="ArgumentNullException">
        ...
        /// </summary>
        /// <param name="tasks">The tasks to wait on for completion.</param>
        /// <returns>A task that represents the completion of one of the supplied tasks.  The return Task's
        // Result is the task that completed.</returns>
        /// <remarks>
        /// The returned task will complete when any of the supplied tasks has completed.  The returned task
        // will always end in the RanToCompletion state
        /// with its Result set to the first task to complete.  This is true even if the first task to
        // complete ended in the Canceled or Faulted state.
        /// </remarks>
        /// <exception cref="ArgumentNullException">
        /// The <paramref name="tasks"/> argument was null.
        /// </exception>
        ...
        /// The <paramref name="tasks"/> array contained a null task, or was empty.
        /// </exception>
        private static Task<TTask> WhenAny<TTask>(ReadOnlySpan<TTask> tasks)
            where TTask : Task
        {
            if (tasks.Length == 2)
            {
        ...
        /// <summary>Creates a task that will complete when either of the supplied tasks have
        // completed.</summary>
        ...
        /// </exception>
        public static Task<Task> WhenAny(Task task1, Task task2) => WhenAny<Task>(task1, task2);
        ...
        /// The <paramref name="task1"/> or <paramref name="task2"/> argument was null.
        /// </exception>
        private static Task<TTask> WhenAny<TTask>(TTask task1, TTask task2)
            where TTask : Task
        {
        ...
        /// <remarks>
        /// This has essentially the same logic as <see cref="TaskFactory.CompleteOnInvokePromise{TTask}"/>,
        // but optimized
        /// for two tasks rather than any number. Exactly two tasks has shown to be the most common use-case
        // by far.
        /// </remarks>
            ...
            where TTask : Task
        {
            ...
            /// <summary>Instantiate the promise and register it with both tasks as a completion
            // action.</summary>
        ...
        /// </exception>
        public static Task<Task> WhenAny(IEnumerable<Task> tasks) => WhenAny<Task>(tasks);
        ...
        /// The <paramref name="tasks"/> collection contained a null task, or was empty.
        /// </exception>
        private static Task<TTask> WhenAny<TTask>(IEnumerable<TTask> tasks)
            where TTask : Task
        {
            // Skip a List allocation/copy if tasks is a collection
            if (tasks is ICollection<TTask> tasksAsCollection)
            {
                ...
                // since if argument was strongly-typed as an array, it would have bound to the array-based
                // overload.
        ...
        // Do not remove: VS debugger calls this API directly using func-eval to populate data in the tasks
        // window
    ...
    internal sealed class SystemThreadingTasks_TaskDebugView
    {
        ...
        public bool CancellationPending =>
            ...
            && m_task.CancellationToken.IsCancellationRequested;
    ...
    /// </summary>
    // NOTE: These options are a subset of TaskContinuationsOptions, thus before adding a flag check it
    // is
    // not already in use.
    [Flags]
    public enum TaskCreationOptions
    {
        ...
        /// <summary>
        /// Specifies that a task will be a long-running, course-grained operation. It provides a hint to
        // the
        ...
        /// <summary>
        /// Specifies that an InvalidOperationException will be thrown if an attempt is made to attach a
        // child task to the created task.
        /// </summary>
        ...
        /// like StartNew or ContinueWith that are performed in the created task will see
        // TaskScheduler.Default as the current scheduler.
        /// </summary>
        HideScheduler = 0x10,
    ...
    internal enum InternalTaskOptions
    {
        ...
        /// <summary>Specifies that the task will be queued by the runtime before handing it over to the
        // user.
        /// This flag will be used to skip the cancellationtoken registration step, which is only meant for
        // unstarted tasks.</summary>
        QueuedByRuntime = 0x2000,

        /// <summary>
        /// Denotes that Dispose should be a complete nop for a Task.  Used when constructing tasks that are
        // meant to be cached/reused.
        /// </summary>
        DoNotDispose = 0x4000,
    }

    /// <summary>
    /// Specifies flags that control optional behavior for the creation and execution of continuation
    // tasks.
    /// </summary>
    [Flags]
    public enum TaskContinuationOptions
    {
        ...
        /// <summary>
        /// In the case of continuation cancellation, prevents completion of the continuation until the
        // antecedent has completed.
        /// </summary>
        ...
        /// <summary>
        /// Specifies that the continuation task should not be scheduled if its antecedent threw an
        // unhandled
    ...
    internal sealed class UnwrapPromise<TResult> : Task<TResult>, ITaskCompletionAction
    {
        ...
        // "Should we check for OperationCanceledExceptions on the outer task and interpret them as proxy
        // cancellation?"
        ...
        private void InvokeCore(Task completingTask)
        {
            switch (_state)
            {
                case STATE_WAITING_ON_OUTER_TASK:
                    ...
                    // We bump the state inside of ProcessCompletedOuterTask because it can also be called from the
                    // constructor.
                    break;
        ...
        /// <param name="lookForOce">Whether or not to look for OperationCanceledExceptions in task's
        // exceptions if it faults.</param>
        ...
        private bool TrySetFromTask(Task task, bool lookForOce)
        {
            ...
            switch (task.Status)
            {
                ...
                case TaskStatus.RanToCompletion:
                    ...
                    result = TrySetResult(
                        task is Task<TResult> taskTResult ? taskTResult.Result : default
                    );
                    break;
            }
            return result;
