...
namespace System
{
    internal unsafe partial class Number
    {
            ...
            where TFloat : unmanaged, IBinaryFloatParseAndFormatInfo<TFloat>
        {
            ...
            else if (normalExponent < TFloat.MinBinaryExponent)
            {
                ...
                int denormalMantissaShift =
                    normalMantissaShift + normalExponent + TFloat.ExponentBias - 1;
            ...
            ulong shiftedExponent =
                ((ulong)(exponent + TFloat.ExponentBias)) << TFloat.DenormalMantissaBits;
            Debug.Assert((shiftedExponent & TFloat.DenormalMantissaMask) == 0);
            Debug.Assert((mantissa & ~TFloat.DenormalMantissaMask) == 0);
            Debug.Assert(
                (
                    shiftedExponent
                    & ~(((1UL << TFloat.ExponentBits) - 1) << TFloat.DenormalMantissaBits)
                ) == 0
            ...
            where TFloat : unmanaged, IBinaryFloatParseAndFormatInfo<TFloat>
        {
            ...
            {
                return AssembleFloatingPointBits<TFloat>(
                    value.ToUInt64(),
                    baseExponent,
            ...
            where TFloat : unmanaged, IBinaryFloatParseAndFormatInfo<TFloat>
        {
            ...
            if (totalDigits <= 19)
            {
                ...
                if (
                    (mantissa <= TFloat.MaxMantissaFastPath)
            ...
            return NumberToFloatingPointBitsSlow<TFloat>(
                ref number,
                positiveExponent,
            ...
            where TFloat : unmanaged, IBinaryFloatParseAndFormatInfo<TFloat>
        {
            ...
            if (
                ...
                && (fractionalDenominatorExponent - (int)(totalDigits))
                    > TFloat.OverflowDecimalExponent
            )
            {
            ...
            where TFloat : unmanaged, IBinaryFloatParseAndFormatInfo<TFloat>
        {
            ...
            exponent = (int)(
                CalculatePower((int)(q)) + upperBit - lz - (-TFloat.MaxBinaryExponent)
            );
            if (exponent <= 0)
            {
                // we have a subnormal?
            ...
            if (
                (product.low <= 1)
                && (q >= TFloat.MinExponentRoundToEven)
                && (q <= TFloat.MaxExponentRoundToEven)
                && ((mantissa & 3) == 1)
            )
            {
        ...
        )
        {
            ...
            ulong precisionMask =
                (bitPrecision < 64) ? (0xFFFFFFFFFFFFFFFFUL >> bitPrecision) : 0xFFFFFFFFFFFFFFFFUL;
        ...
        //   p = log(5**-q)/log(2) = -q* log(5)/log(2)
        //
        internal static int CalculatePower(int q) => (((152170 + 65536) * q) >> 16) + 63;
    }
}
