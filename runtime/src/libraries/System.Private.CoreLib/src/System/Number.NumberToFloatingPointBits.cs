...
namespace System
{
    internal unsafe partial class Number
    {
        public readonly struct FloatingPointInfo
        {
            ...
            public ulong MaxMantissaFastPath
            {
                get => 2UL << DenormalMantissaBits;
            }
            ...
            public FloatingPointInfo(
                ...
                int exponentBias,
                ulong infinityBits,
                int minFastFloatDecimalExponent,
                int maxFastFloatDecimalExponent,
                int infinityExponent,
                int minExponentRoundToEven,
                int maxExponentRoundToEven,
                int maxExponentFastPath
            )
            {
        ...
        )
        {
            ...
            if (totalDigits <= 19)
            {
                ...
                if (
                    (mantissa <= info.MaxMantissaFastPath)
                    && (fastExponent <= info.MaxExponentFastPath)
                )
                {
        ...
        internal static (int Exponent, ulong Mantissa) ComputeFloat(
            long q,
            ulong w,
            FloatingPointInfo info
        )
        {
            int exponent;
            ulong mantissa = 0;
            ...
            if (
                (product.low <= 1)
                && (q >= info.MinExponentRoundToEven)
                && (q <= info.MaxExponentRoundToEven)
                && ((mantissa & 3) == 1)
            )
            {
