    ...
    public struct PoolingAsyncValueTaskMethodBuilder<TResult>
    {
        ...
        public ValueTask<TResult> Task
        {
            get
            {
                ...
                PoolingAsyncValueTaskMethodBuilder<TResult>.StateMachineBox? box = m_task ??=
            ...
            internal static StateMachineBox<TStateMachine> GetOrCreateBox()
            {
                ...
                if (
                    ...
                    if (box is not null)
                    {
                        ...
                        Debug.Assert(
                            s_cacheSize >= 0,
                            "Expected the cache size to be non-negative."
                        );
                ...
                if (Interlocked.Exchange(ref s_cacheLock, 1) == 0)
                {
                    if (
                        ...
                        Debug.Assert(
                            s_cacheSize > 0
                                && s_cacheSize
                                    <= PoolingAsyncValueTaskMethodBuilder.s_valueTaskPoolingCacheSize,
                            "Expected cache size to be within bounds."
                        );
                    }
