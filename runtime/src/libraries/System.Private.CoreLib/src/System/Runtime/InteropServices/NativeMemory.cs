...
namespace System.Runtime.InteropServices
{
    public static unsafe partial class NativeMemory
    {
        ...
        /// <remarks>
        ///     <para>If this method is called with <paramref name="ptr" /> being <see langword="null"/> and
        // <paramref name="byteCount" /> being <c>0</c>, it will be equivalent to a no-op.</para>
        ///     <para>The behavior when <paramref name="ptr" /> is <see langword="null"/> and <paramref
        // name="byteCount" /> is greater than <c>0</c> is undefined.</para>
        /// </remarks>
        [CLSCompliant(false)]
        ...
        /// <param name="destination">A pointer to the destination memory block where the data is to be
        // copied.</param>
        /// <param name="byteCount">The size, in bytes, to be copied from the source location to the
        // destination.</param>
        [CLSCompliant(false)]
        ...
        {
            // This is based on the `mi_count_size_overflow` and `mi_mul_overflow` methods from
            // microsoft/mimalloc.
            // Original source is Copyright (c) 2019 Microsoft Corporation, Daan Leijen. Licensed under the MIT
            // license
