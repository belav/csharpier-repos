        ...
        internal OperatingSystem(PlatformID platform, Version version, string? servicePack)
        {
            if (platform < PlatformID.Win32S || platform > PlatformID.Other)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(platform),
                    platform,
                    SR.Format(SR.Arg_EnumIllegalVal, platform)
                );
            }
                ...
                if (_versionString == null)
                {
                    ...
                    switch (_platform)
                    {
                        ...
                        case PlatformID.Win32Windows:
                            os =
                                (_version.Major > 4 || (_version.Major == 4 && _version.Minor > 0))
...
#elif TARGET_OSX
            return platform.Equals("OSX", StringComparison.OrdinalIgnoreCase)
                || platform.Equals("MACOS", StringComparison.OrdinalIgnoreCase);
#elif TARGET_MACCATALYST
            return platform.Equals("MACCATALYST", StringComparison.OrdinalIgnoreCase)
...
#error Unknown OS
        ...
        private static bool IsOSVersionAtLeast(int major, int minor, int build, int revision)
        {
            ...
            return current.Revision >= revision || (current.Revision == -1 && revision == 0); // it is unavailable on OSX and Environment.OSVersion.Version.Revision returns -1
        }
    }
}
