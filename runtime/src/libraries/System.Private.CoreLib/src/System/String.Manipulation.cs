    ...
    public partial class String
    {
        ...
        public static string Concat(params object?[] args)
        {
            ...
            for (int i = 0; i < strings.Length; i++)
            {
                ...
                Debug.Assert(s != null);
                Debug.Assert(
                    position <= totalLength - s.Length,
                    "We didn't allocate enough space for the result string!"
                );
        ...
        public static string Concat(string? str0, string? str1, string? str2, string? str3)
        {
            ...
            // It can overflow to a positive number so we accumulate the total length as a long.
            long totalLength =
                (long)str0.Length + (long)str1.Length + (long)str2.Length + (long)str3.Length;
        ...
        {
            int length = checked(
                str0.Length + str1.Length + str2.Length + str3.Length + str4.Length
            );
            if (length == 0)
            {
                return Empty;
        ...
        public string Insert(int startIndex, string value)
        {
            ...
            Buffer.Memmove(
                ref Unsafe.Add(ref result._firstChar, startIndex),
        ...
        private static string JoinCore(ReadOnlySpan<char> separator, ReadOnlySpan<string?> values)
        {
            if (values.Length <= 1)
            {
                return values.IsEmpty ? Empty : values[0] ?? Empty;
            }
        ...
        public string PadLeft(int totalWidth, char paddingChar)
        {
            ...
            Buffer.Memmove(
                ref Unsafe.Add(ref result._firstChar, count),
                ref _firstChar,
                (nuint)oldLength
            );
        ...
        public string Remove(int startIndex, int count)
        {
            ...
            Buffer.Memmove(
                ref Unsafe.Add(ref result._firstChar, startIndex),
                ref Unsafe.Add(ref _firstChar, startIndex + count),
                (nuint)(newLength - startIndex)
            );
        ...
        public string Replace(char oldChar, char newChar)
        {
            ...
            ref ushort pDst = ref Unsafe.Add(
                ref result.GetRawStringDataAsUInt16(),
                (uint)copyLength
            );
        ...
        public string Replace(string oldValue, string? newValue)
        {
            ...
            var replacementIndices = new ValueListBuilder<int>(
                stackalloc int[StackallocIntBufferSizeLimit]
            );
            ...
            if (oldValue.Length == 1)
            {
                ...
                if (
                    ...
                    && PackedSpanHelpers.CanUsePackedIndexOf(c)
                )
                {
                    while (true)
                    {
                        int pos = PackedSpanHelpers.IndexOf(
                            ref Unsafe.Add(ref _firstChar, i),
                            c,
                            Length - i
                        );
                        if (pos < 0)
                ...
                else
                {
                    while (true)
                    {
                        int pos = SpanHelpers.NonPackedIndexOfChar(
                            ref Unsafe.Add(ref _firstChar, i),
                            c,
                            Length - i
                        );
            ...
            else
            {
                ...
                while (true)
                {
                    int pos = SpanHelpers.IndexOf(
                        ref Unsafe.Add(ref _firstChar, i),
                        Length - i,
                        ref oldValue._firstChar,
                        oldValue.Length
                    );
                    if (pos < 0)
                    {
        ...
        private string ReplaceHelper(int oldValueLength, string newValue, ReadOnlySpan<int> indices)
        {
            ...
            long dstLength =
                this.Length + ((long)(newValue.Length - oldValueLength)) * indices.Length;
        ...
        private string ReplaceLineEndingsWithLineFeed()
        {
            ...
            int idxOfFirstNewlineChar = this.AsSpan()
                .IndexOfAny(SearchValuesStorage.NewLineCharsExceptLineFeed);
            ...
            builder.Append('\n');
            string retVal = Concat(
                this.AsSpan(0, idxOfFirstNewlineChar),
                builder.AsSpan(),
                remaining
            );
            builder.Dispose();
        ...
        )
        {
            ...
            var sepListBuilder = new ValueListBuilder<int>(
                stackalloc int[StackallocIntBufferSizeLimit]
            );
        ...
        )
        {
            ...
            var lengthListBuilder = new ValueListBuilder<int>(
                stackalloc int[StackallocIntBufferSizeLimit]
            );
                ...
                if (arrIndex == count - 1)
                {
                    ...
                    if ((options & StringSplitOptions.RemoveEmptyEntries) != 0)
                    {
                        while (++i < numReplaces)
                        {
                            ...
                            currIndex =
                                sepList[i] + (lengthList.IsEmpty ? defaultLength : lengthList[i]);
                        }
                    }
                    break;
                }
            ...
            for (int i = 0; i < source.Length; i++)
            {
                for (int j = 0; j < separators.Length; j++)
                {
                    ...
                    if (source[i] == separator[0] && currentSepLength <= source.Length - i)
                    {
                        if (
                            currentSepLength == 1
                            || source.Slice(i, currentSepLength).SequenceEqual(separator)
                        )
                        {
                            sepListBuilder.Append(i);
        ...
        private void ThrowSubstringArgumentOutOfRange(int startIndex, int length)
        {
            ...
            throw new ArgumentOutOfRangeException(
                nameof(length),
                SR.ArgumentOutOfRange_IndexLength
            );
        }
