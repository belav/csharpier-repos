...
namespace System.Buffers
{
    ...
    {
        // This method is the same as GenerateBucketizedFingerprint below, but each bucket only contains 1
        // value.
        ...
        // For example if bucket 0 contains strings ["foo", "bar"], the bitmaps will have the first bit (0th
        // bucket) set like the following:
        // 'f' is 0x66, 'b' is 0x62, so n0Low has the bit set at index 2 and  6, n0High has it set at index
        // 6.
        // 'o' is 0x6F, 'a' is 0x61, so n1Low has the bit set at index 1 and 15, n1High has it set at index
        // 6.
        // 'o' is 0x6F, 'r' is 0x72, so n2Low has the bit set at index 2 and 15, n2High has it set at index
        // 6 and 7.
        // We repeat this for each bucket and then OR together the bitmaps (fingerprints) of each bucket to
        // generate a single bitmap for each nibble.
        ...
        public static string[][] Bucketize(ReadOnlySpan<string> values, int bucketCount, int n)
        {
            ...
            // Group patterns with the same prefix into the same bucket to avoid wasting time during
            // verification steps.
            ...
            for (int i = 0; i < values.Length; i++)
            {
                ...
                {
                    // Potential optimization: We currently merge values with different prefixes into buckets randomly
                    // (round-robin).
