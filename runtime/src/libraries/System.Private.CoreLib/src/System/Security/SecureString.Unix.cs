...
namespace System.Security
{
    ...
    // 1. keeping the data in unmanaged memory so that copies of it aren't implicitly made by the GC
    // moving it around
    // 2. zero'ing out that unmanaged memory so that the string is reliably removed from memory when
    // done with it
