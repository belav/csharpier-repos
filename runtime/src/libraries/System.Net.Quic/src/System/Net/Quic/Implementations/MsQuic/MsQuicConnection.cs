...
namespace System.Net.Quic.Implementations.MsQuic
{
    internal sealed class MsQuicConnection : QuicConnectionProvider
    {
        ...
        private static readonly ConnectionCallbackDelegate s_connectionDelegate =
        ...
        private static uint HandleEventConnected(State state, ref ConnectionEvent connectionEvent)
        {
            ...
            else
            {
                ...
                SOCKADDR_INET inetAddress = MsQuicParameterHelpers.GetINetParam(
                    MsQuicApi.Api,
                    state.Handle,
                    QUIC_PARAM_LEVEL.CONNECTION,
                    (uint)QUIC_PARAM_CONN.LOCAL_ADDRESS
                );
        ...
        )
        {
            if (!state.Connected && state.ConnectTcs != null)
            {
                ...
                Exception ex = QuicExceptionHelpers.CreateExceptionForHResult(
                    hresult,
        ...
        private static uint HandleEventNewStream(State state, ref ConnectionEvent connectionEvent)
        {
            var streamHandle = new SafeMsQuicStreamHandle(
                connectionEvent.Data.PeerStreamStarted.Stream
            );
            if (
                !state.TryQueueNewStream(streamHandle, connectionEvent.Data.PeerStreamStarted.Flags)
            )
            {
        ...
        )
        {
            ...
            try
            {
                    ...
                    else
                    {
                        unsafe
                        {
                            ReadOnlySpan<QuicBuffer> quicBuffer = new ReadOnlySpan<QuicBuffer>(
                                (void*)connectionEvent.Data.PeerCertificateReceived.PlatformCertificateHandle,
                                sizeof(QuicBuffer)
                            );
                            certificate = new X509Certificate2(
                                new ReadOnlySpan<byte>(
                                    quicBuffer[0].Buffer,
                                    (int)quicBuffer[0].Length
                                )
                            );
                            ...
                            if (
                                connectionEvent.Data.PeerCertificateReceived.PlatformCertificateChainHandle
                                != IntPtr.Zero
                            )
                            {
                                quicBuffer = new ReadOnlySpan<QuicBuffer>(
                                    (void*)connectionEvent.Data.PeerCertificateReceived.PlatformCertificateChainHandle,
                                    sizeof(QuicBuffer)
                                );
                ...
                if (state.RemoteCertificateValidationCallback != null)
                {
                    ...
                    if (!success && NetEventSource.Log.IsEnabled())
                        NetEventSource.Error(
                            state,
                            $"{state.TraceId} Remote certificate rejected by verification callback"
                        );
                ...
                if (NetEventSource.Log.IsEnabled())
                    NetEventSource.Info(
                        state,
                        $"{state.TraceId} Certificate validation for '${certificate?.Subject}' finished with ${sslPolicyErrors}"
                    );
            ...
            catch (Exception ex)
            {
                if (NetEventSource.Log.IsEnabled())
                    NetEventSource.Error(
                        state,
                        $"{state.TraceId} Certificate validation failed ${ex.Message}"
                    );
                throw;
            }
        }
            ...
            if (tcs is null)
            {
                ...
                lock (_state)
                {
                    if (_state.NewUnidirectionalStreamsAvailable is null)
                    {
                        ...
                        _state.NewUnidirectionalStreamsAvailable = new TaskCompletionSource(
                            TaskCreationOptions.RunContinuationsAsynchronously
                        );
                    }
            ...
            if (tcs is null)
            {
                ...
                lock (_state)
                {
                    if (_state.NewBidirectionalStreamsAvailable is null)
                    {
                        ...
                        _state.NewBidirectionalStreamsAvailable = new TaskCompletionSource(
                            TaskCreationOptions.RunContinuationsAsynchronously
                        );
                    }
        ...
        internal override int GetRemoteAvailableUnidirectionalStreamCount()
        {
            ...
            return MsQuicParameterHelpers.GetUShortParam(
                ...
                _state.Handle,
                QUIC_PARAM_LEVEL.CONNECTION,
                (uint)QUIC_PARAM_CONN.LOCAL_UNIDI_STREAM_COUNT
            );
        }
        ...
        internal override int GetRemoteAvailableBidirectionalStreamCount()
        {
            ...
            return MsQuicParameterHelpers.GetUShortParam(
                ...
                _state.Handle,
                QUIC_PARAM_LEVEL.CONNECTION,
                (uint)QUIC_PARAM_CONN.LOCAL_BIDI_STREAM_COUNT
            );
        }
        ...
        internal override ValueTask ConnectAsync(CancellationToken cancellationToken = default)
        {
            ...
            {
                SOCKADDR_INET address = MsQuicAddressHelpers.IPEndPointToINet(
                    (IPEndPoint)_remoteEndPoint
                );
                unsafe
                {
                    Debug.Assert(!Monitor.IsEntered(_state));
                    status = MsQuicApi.Api.SetParamDelegate(
                        _state.Handle,
                        QUIC_PARAM_LEVEL.CONNECTION,
                        (uint)QUIC_PARAM_CONN.REMOTE_ADDRESS,
                        (uint)sizeof(SOCKADDR_INET),
                        (byte*)&address
                    );
            ...
            else if (_remoteEndPoint is DnsEndPoint)
            {
                ...
                )
                {
                    ...
                    SOCKADDR_INET quicAddress = MsQuicAddressHelpers.IPEndPointToINet(
            ...
            var tcs = _state.ConnectTcs = new TaskCompletionSource<uint>(
                TaskCreationOptions.RunContinuationsAsynchronously
            );
        ...
        )
        {
            ...
            if (NetEventSource.Log.IsEnabled())
            {
                NetEventSource.Info(
                    state,
                    $"{state.TraceId} Connection received event {connectionEvent.Type}"
                );
            }
        ...
        private void Dispose(bool disposing)
        {
            ...
            if (NetEventSource.Log.IsEnabled())
                NetEventSource.Info(_state, $"{TraceId()} Connection disposing {disposing}");
            ...
            {
                if (NetEventSource.Log.IsEnabled())
                    NetEventSource.Info(_state, $"{TraceId()} Connection releasing handle");
