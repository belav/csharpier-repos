        ...
        else
        {
            ...
            loaded =
                NativeLibrary.TryLoad(
                    $"{Interop.Libraries.MsQuic}.{s_minMsQuicVersion.Major}",
                    typeof(MsQuicApi).Assembly,
                    null,
                    out msQuicHandle
                )
        ...
        try
        {
            ...
            if (StatusFailed(status))
            {
                if (NetEventSource.Log.IsEnabled())
                {
                    NetEventSource.Error(
                        null,
                        $"Cannot retrieve {nameof(QUIC_PARAM_GLOBAL_LIBRARY_VERSION)} from MsQuic library: '{status}'."
                    );
                }
                return;
            }
            ...
            if (StatusFailed(status))
            {
                if (NetEventSource.Log.IsEnabled())
                {
                    NetEventSource.Error(
                        null,
                        $"Cannot retrieve {nameof(QUIC_PARAM_GLOBAL_LIBRARY_GIT_HASH)} from MsQuic library: '{status}'."
                    );
                }
                return;
            }
...
#pragma warning restore CA1810
    ...
    private static bool IsWindowsVersionSupported() =>
        OperatingSystem.IsWindowsVersionAtLeast(
            s_minWindowsVersion.Major,
            s_minWindowsVersion.Minor,
            s_minWindowsVersion.Build,
            s_minWindowsVersion.Revision
        );
