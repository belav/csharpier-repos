    ...
    public sealed class QuicListenerTests : QuicTestBase
    {
        ...
        public async Task AcceptConnectionAsync_ThrowingCallbackOde_KeepRunning()
        {
            ...
            Exception exception = await AssertThrowsQuicExceptionAsync(
                QuicError.CallbackError,
                async () =>
                    await listener.AcceptConnectionAsync().AsTask().WaitAsync(PassingTestTimeout)
            );
            ...
            await using QuicConnection serverConnection = await listener
                .AcceptConnectionAsync()
                .AsTask()
                .WaitAsync(PassingTestTimeout);
        ...
        )
        {
            ...
            clientOptions.HandshakeTimeout = clientShorterTimeout
            ...
            if (clientShorterTimeout)
            {
                ...
                Assert.Equal(
                    SR.Format(SR.net_quic_handshake_timeout, clientOptions.HandshakeTimeout),
                    exception.Message
                );
            }
            else
        ...
        public async Task Listener_AlpnNarrowingDown_Failure(string alpn)
        {
            ...
            bool isAlpnPresentOnInitialAlpnList = listenerOptions.ApplicationProtocols
                .Contains(new SslApplicationProtocol(alpn)); // If the ALPN is not present on initial list, AcceptConnectionAsync will not throw AuthenticationException.
            await using QuicListener listener = await CreateQuicListener(listenerOptions);
            QuicClientConnectionOptions clientOptions = CreateQuicClientOptions(
                listener.LocalEndPoint
            );
