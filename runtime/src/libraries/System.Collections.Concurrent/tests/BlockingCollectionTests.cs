    ...
    public class BlockingCollectionTests
    {
        ...
        )
        {
            ...
            for (int i = 0; i < numOfAdds; ++i)
            {
                ...
                {
                    int indexOfCollectionThatAcceptedTheOperation =
                        BlockingCollection<int>.TryAddToAny(blockingCollections, numberToAdd);
                    if (
            ...
            for (int i = 0; i < numOfTakes; ++i)
            {
                ...
                else
                {
                    int indexOfCollectionThatAcceptedTheOperation =
                        BlockingCollection<int>.TryTakeFromAny(
                            blockingCollections,
                            out itemFromBlockingCollection
                        );
                    if (
                        indexOfCollectionThatAcceptedTheOperation
                        == indexOfBlockingCollectionUnderTest
        ...
        private class FalseOnSecondAddOrTake
            : ConcurrentQueue<int>,
                IProducerConsumerCollection<int>
        {
            ...
            {
                if (Interlocked.Increment(ref _add) == 2)
                    return false;
            ...
            {
                value = default;
                if (Interlocked.Increment(ref _take) == 2)
                    return false;
