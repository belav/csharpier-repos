        ...
        : ConcurrentDictionary_Generic_Tests<string, string>
    {
        ...
        private static List<string> GenerateCollidingStrings(int count)
        {
            static Func<string, int> GetHashCodeFunc(ConcurrentDictionary<string, string> cd)
            {
                ...
                FieldInfo tablesField = AssertNotNull(
                    typeof(ConcurrentDictionary<string, string>).GetField(
                        "_tables",
                        BindingFlags.Instance | BindingFlags.NonPublic
                    )
                );
                Type tablesType = Type.GetType(
                    "System.Collections.Concurrent.ConcurrentDictionary`2+Tables, System.Collections.Concurrent",
                    throwOnError: true
                );
                object tables = AssertNotNull(tablesField.GetValue(cd));

                FieldInfo comparerField = AssertNotNull(
                    tablesType.GetField(
                        "_comparer",
                        BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public
                    )
                );
                comparerField = AssertNotNull(
                    (FieldInfo)tables.GetType().GetMemberWithSameMetadataDefinitionAs(comparerField)
                );
                IEqualityComparer<string> comparer = AssertNotNull(
                    (IEqualityComparer<string>)comparerField.GetValue(tables)
                );

                return comparer.GetHashCode;

                static T AssertNotNull<T>(
                    T value,
            ...
            Func<string, int> nonRandomizedOrdinal = GetHashCodeFunc(
                new ConcurrentDictionary<string, string>(StringComparer.Ordinal)
            );
            Func<string, int> nonRandomizedOrdinalIgnoreCase = GetHashCodeFunc(
