    ...
    public class GroupPrincipalTest : PrincipalTest
    {
        ...
        public void IsMemberOfTest()
        {
            ...
            )
            {
                Assert.True(
                    UserPrincipal
                        .FindByIdentity(DomainContext, "user1499-LargeGroup")
                        .IsMemberOf(group)
                );
                Assert.True(
                    UserPrincipal
                        .FindByIdentity(DomainContext, "user1500-LargeGroup")
                        .IsMemberOf(group)
                );
                Assert.True(
                    UserPrincipal
                        .FindByIdentity(DomainContext, "user1501-LargeGroup")
                        .IsMemberOf(group)
                );
                Assert.True(
                    UserPrincipal
                        .FindByIdentity(DomainContext, "user3000-LargeGroup")
                        .IsMemberOf(group)
                );
                Assert.True(
                    UserPrincipal
                        .FindByIdentity(DomainContext, "user3001-LargeGroup")
                        .IsMemberOf(group)
                );
                Assert.False(
                    UserPrincipal
                        .FindByIdentity(DomainContext, "userNotInLargeGroup")
                        .IsMemberOf(group)
                );
            }
        }
