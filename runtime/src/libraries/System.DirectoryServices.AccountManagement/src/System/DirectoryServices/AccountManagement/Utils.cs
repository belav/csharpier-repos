...
namespace System.DirectoryServices.AccountManagement
{
    internal static class Utils
    {
        ...
        {
            return (
                ADUtils.HexStringToLdapHexString(SecurityIdentifierToLdapHexBindingString(sid))
            );
        }
        ...
        internal static byte[] StringToByteArray(string s)
        {
            if (s.Length % 2 != 0)
            {
                GlobalDebug.WriteLineIf(
                    GlobalDebug.Warn,
                    "Utils",
                    "StringToByteArray: string has bad length " + s.Length
                );
                return null;
            }
            ...
            for (int i = 0; i < (s.Length) / 2; i++)
            {
                ...
                else
                {
                    GlobalDebug.WriteLineIf(
                        GlobalDebug.Warn,
                        "Utils",
                        "StringToByteArray: invalid string: " + s
                    );
                    return null;
                }
            }
        ...
        internal static IntPtr ConvertByteArrayToIntPtr(byte[] bytes)
        {
            ...
            catch (Exception e)
            {
                GlobalDebug.WriteLineIf(
                    GlobalDebug.Error,
                    "Utils",
                    "ConvertByteArrayToIntPtr: caught exception of type "
                        + e.GetType().ToString()
                        + " and message "
                        + e.Message
                );
        ...
        internal static SidType ClassifySID(IntPtr pSid)
        {
            ...
            UnsafeNativeMethods.SID_IDENTIFIER_AUTHORITY identAuth =
                (UnsafeNativeMethods.SID_IDENTIFIER_AUTHORITY)Marshal.PtrToStructure(
                    pIdentAuth,
                    typeof(UnsafeNativeMethods.SID_IDENTIFIER_AUTHORITY)
                );
            ...
            try
            {
                ...
                )
                {
                    if ((error = Marshal.GetLastWin32Error()) == 1008) // ERROR_NO_TOKEN
                    {
                        ...
                        )
                        {
                            ...
                            GlobalDebug.WriteLineIf(
                                GlobalDebug.Error,
                                "Utils",
                                "GetCurrentUserSid: OpenProcessToken failed, gle=" + lastError
                            );
                    ...
                    else
                    {
                        GlobalDebug.WriteLineIf(
                            GlobalDebug.Error,
                            "Utils",
                            "GetCurrentUserSid: OpenThreadToken failed, gle=" + error
                        );

                        throw new PrincipalOperationException(
                            SR.Format(SR.UnableToOpenToken, error)
                        );
                    }
                }
                ...
                if ((getTokenInfoError = Marshal.GetLastWin32Error()) != 122) // ERROR_INSUFFICIENT_BUFFER
                {
                    GlobalDebug.WriteLineIf(
                        GlobalDebug.Error,
                        "Utils",
                        "GetCurrentUserSid: GetTokenInformation (1st try) failed, gle="
                            + getTokenInfoError
                    );
                ...
                if (!success)
                {
                    ...
                    GlobalDebug.WriteLineIf(
                        GlobalDebug.Error,
                        "Utils",
                        "GetCurrentUserSid: GetTokenInformation (2nd try) failed, neededBufferSize="
                            + neededBufferSize
                            + ", gle="
                            + lastError
                    );
                ...
                UnsafeNativeMethods.TOKEN_USER tokenUser =
                    (UnsafeNativeMethods.TOKEN_USER)Marshal.PtrToStructure(
                        pBuffer,
                        typeof(UnsafeNativeMethods.TOKEN_USER)
                    );
            ...
            try
            {
                ...
                if (err != 0)
                {
                    GlobalDebug.WriteLineIf(
                        GlobalDebug.Error,
                        "Utils",
                        "GetMachineDomainSid: LsaOpenPolicy failed, gle="
                ...
                if (err != 0)
                {
                    GlobalDebug.WriteLineIf(
                        GlobalDebug.Error,
                        "Utils",
                        "GetMachineDomainSid: LsaQueryInformationPolicy failed, gle="
                            + SafeNativeMethods.LsaNtStatusToWinError(err)
                    );
                ...
                UnsafeNativeMethods.POLICY_ACCOUNT_DOMAIN_INFO info =
                    (UnsafeNativeMethods.POLICY_ACCOUNT_DOMAIN_INFO)Marshal.PtrToStructure(
                        pBuffer,
                        typeof(UnsafeNativeMethods.POLICY_ACCOUNT_DOMAIN_INFO)
                    );
        ...
        internal static UnsafeNativeMethods.DomainControllerInfo GetDcName(
            string computerName,
            string domainName,
            string siteName,
            int flags
        )
        {
            ...
            try
            {
                int err = UnsafeNativeMethods.DsGetDcName(
                    computerName,
                    domainName,
                    IntPtr.Zero,
                    siteName,
                ...
                if (err != 0)
                {
                    GlobalDebug.WriteLineIf(
                        GlobalDebug.Error,
                        "Utils",
                        "GetDcName: DsGetDcName failed, err=" + err
                    );
                    throw new PrincipalOperationException(
                ...
                UnsafeNativeMethods.DomainControllerInfo domainControllerInfo =
                    (UnsafeNativeMethods.DomainControllerInfo)Marshal.PtrToStructure(
                        domainControllerInfoPtr,
                        typeof(UnsafeNativeMethods.DomainControllerInfo)
                    );
        ...
        internal static unsafe int LookupSid(
            string serverName,
            NetCred credentials,
            byte[] sid,
            out string name,
            ...
            try
            {
                ...
                if (lastErr != 122) // ERROR_INSUFFICIENT_BUFFER
                {
                    GlobalDebug.WriteLineIf(
                        GlobalDebug.Error,
                        "Utils",
                        "LookupSid: LookupAccountSid (1st try) failed, gle=" + lastErr
                    );
                    return lastErr;
                }
                ...
                fixed (char* sbDomainName = new char[domainNameLength])
                {
                    ...
                    if (f == 0)
                    {
                        ...
                        GlobalDebug.WriteLineIf(
                            GlobalDebug.Error,
                            "Utils",
                            "LookupSid: LookupAccountSid (2nd try) failed, gle=" + lastErr
                        );
                        return lastErr;
                    }
        ...
        )
        {
            ...
            else
            {
                GlobalDebug.WriteLineIf(
                    GlobalDebug.Warn,
                    "Utils",
                    "ConstructFakePrincipalFromSID: LookupSid failed (ignoring), serverName="
                        + serverName
                        + ", err="
                        + err
                );
        ...
        internal static bool BeginImpersonation(NetCred credential, out IntPtr hUserToken)
        {
            ...
            if (credential == null)
            {
                GlobalDebug.WriteLineIf(
                    GlobalDebug.Info,
                    "Utils",
                    "BeginImpersonation: nothing to impersonate"
                );
                return false;
            }
            ...
            if (userName == null && password == null)
            {
                GlobalDebug.WriteLineIf(
                    GlobalDebug.Info,
                    "Utils",
                    "BeginImpersonation: nothing to impersonate (2)"
                );
                return false;
            }
            ...
            if (result == 0)
            {
                ...
                GlobalDebug.WriteLineIf(
                    GlobalDebug.Error,
                    "Utils",
                    "BeginImpersonation: LogonUser failed, gle=" + lastError
                );
            ...
            if (result == 0)
            {
                ...
                GlobalDebug.WriteLineIf(
                    GlobalDebug.Error,
                    "Utils",
                    "BeginImpersonation: ImpersonateLoggedOnUser failed, gle=" + lastError
                );
            ...
            try
            {
                ...
                if (err != 0)
                {
                    GlobalDebug.WriteLineIf(
                        GlobalDebug.Error,
                        "Utils",
                        "IsMachineDC: DsRoleGetPrimaryDomainInformation failed, err=" + err
                    );
                    throw new PrincipalOperationException(
                        SR.Format(SR.UnableToRetrieveDomainInfo, err)
                    );
                ...
                UnsafeNativeMethods.DSROLE_PRIMARY_DOMAIN_INFO_BASIC dsRolePrimaryDomainInfo =
                    (UnsafeNativeMethods.DSROLE_PRIMARY_DOMAIN_INFO_BASIC)Marshal.PtrToStructure(
                        dsRoleInfoPtr,
                        typeof(UnsafeNativeMethods.DSROLE_PRIMARY_DOMAIN_INFO_BASIC)
                    );
