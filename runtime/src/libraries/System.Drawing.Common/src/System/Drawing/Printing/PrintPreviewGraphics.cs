...
namespace System.Drawing
{
    ...
    internal sealed class PrintPreviewGraphics
    {
        ...
        public RectangleF VisibleClipBounds
        {
            get
            {
                IntPtr hdevMode = _printPageEventArgs.PageSettings.PrinterSettings
                    .GetHdevmodeInternal();

                using (
                    DeviceContext dc = _printPageEventArgs.PageSettings.PrinterSettings
                        .CreateDeviceContext(hdevMode)
                )
                {
                    using (Graphics graphics = Graphics.FromHdcInternal(dc.Hdc))
                    {
                        if (_printDocument.OriginAtMargins)
                        {
                            ...
                            graphics.TranslateTransform(
                                ...
                                _printDocument.DefaultPageSettings.Margins.Top
                            );
                        }
