        ...
        internal static partial class Gdip
        {
            ...
            static Gdip()
            {
                Debug.Assert(
                    ...
                    "GdiplusInitialization: Initialize should not be called more than once in the same domain!"
                );
            ...
            internal static Exception StatusException(int status)
            {
                ...
                switch (status)
                {
                    ...
                    case FontFamilyNotFound:
                        Debug.Fail(
                            "We should be special casing FontFamilyNotFound so we can provide the font name"
                        );
                        return new ArgumentException(SR.Format(SR.GdiplusFontFamilyNotFound, "?"));

                    case FontStyleNotFound:
                        Debug.Fail(
                            "We should be special casing FontStyleNotFound so we can provide the font name"
                        );
                        return new ArgumentException(
                            SR.Format(SR.GdiplusFontStyleNotFound, "?", "?")
                        );

                    case NotTrueTypeFont:
                        Debug.Fail(
                            "We should be special casing NotTrueTypeFont so we can provide the font name"
                        );
