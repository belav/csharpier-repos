...
namespace System.Threading.Channels
{
    ...
    internal sealed class BoundedChannel<T> : Channel<T>, IDebugEnumerable<T>
    {
        ...
        /// <summary>The delegate that will be invoked when the channel hits its bound and an item is
        // dropped from the channel.</summary>
        ...
        /// <param name="itemDropped">Delegate that will be invoked when an item is dropped from the
        // channel. See <see cref="BoundedChannelFullMode"/>.</param>
        internal BoundedChannel(
        ...
        private sealed class BoundedChannelReader : ChannelReader<T>, IDebugEnumerable<T>
        {
            ...
            // debugger.</summary>
            /// <remarks>
            /// Unlike <see cref="Count"/>, avoids locking so as to not block the debugger if another suspended
            ...
            public override ValueTask<T> ReadAsync(CancellationToken cancellationToken)
            {
                ...
                lock (parent.SyncObj)
                {
                    ...
                    // Otherwise, queue a reader.  Note that in addition to checking whether synchronous continuations
                    // were requested,
                    // we also check whether the supplied cancellation token can be canceled.  The writer calls
                    // UnregisterCancellation
                    // while holding the lock, and if a callback needs to be unregistered and is currently running, it
                    // needs to wait
                    // for that callback to complete so that the subsequent code knows it won't be contending with
                    // another thread
                    // trying to complete the operation.  However, if we allowed a synchronous continuation from this
                    // operation, that
                    // cancellation callback could end up running arbitrary code, including code that called back into
                    // the reader or
                    // writer and tried to take the same lock held by the thread running UnregisterCancellation...
                    // deadlock.  As such,
                    // we only allow synchronous continuations here if both a) the caller requested it and the token
                    // isn't cancelable.
                    var reader = new AsyncOperation<T>(
                        parent._runContinuationsAsynchronously | cancellationToken.CanBeCanceled,
                        cancellationToken
                    );
        ...
        private sealed class BoundedChannelWriter : ChannelWriter<T>, IDebugEnumerable<T>
        {
            ...
            {
                _parent = parent;
                _writerSingleton = new VoidAsyncOperationWithData<T>(
            ...
            public override bool TryComplete(Exception? error)
            {
                ...
                ChannelUtilities.FailOperations<VoidAsyncOperationWithData<T>, VoidResult>(
                    parent._blockedWriters,
                    ChannelUtilities.CreateInvalidCompletionException(error)
                );
                ...
                ChannelUtilities.WakeUpWaiters(
                    ref parent._waitingWritersTail,
                    result: false,
                    error: error
                );
            ...
            public override bool TryWrite(T item)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        T droppedItem =
                            parent._mode == BoundedChannelFullMode.DropNewest
                ...
                if (blockedReader != null)
                {
                    Debug.Assert(
                        ...
                        "Shouldn't have any waiters to wake up"
                    );
            ...
            public override ValueTask WriteAsync(T item, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    ...
                    else if (parent._mode == BoundedChannelFullMode.Wait)
                    {
                        ...
                        var writer = new VoidAsyncOperationWithData<T>(
                            runContinuationsAsynchronously: true,
                            cancellationToken
                        );
                        writer.Item = item;
        ...
        private void AssertInvariants()
        {
            ...
            if (!_items.IsEmpty)
            {
                Debug.Assert(
                    ...
                    "There are items available, so there shouldn't be any blocked readers."
                );
                Debug.Assert(
                    _waitingReadersTail == null,
                    "There are items available, so there shouldn't be any waiting readers."
                );
            }
            if (_items.Count < _bufferedCapacity)
            {
                Debug.Assert(
                    _blockedWriters.IsEmpty,
                    "There's space available, so there shouldn't be any blocked writers."
                );
                Debug.Assert(
                    _waitingWritersTail == null,
                    "There's space available, so there shouldn't be any waiting writers."
                );
            }
            if (!_blockedReaders.IsEmpty)
            {
                Debug.Assert(
                    _items.IsEmpty,
                    "There shouldn't be queued items if there's a blocked reader."
                );
                Debug.Assert(
                    _blockedWriters.IsEmpty,
                    "There shouldn't be any blocked writer if there's a blocked reader."
                );
            }
            if (!_blockedWriters.IsEmpty)
            {
                Debug.Assert(
                    _items.Count == _bufferedCapacity,
                    "We should have a full buffer if there's a blocked writer."
                );
                Debug.Assert(
                    _blockedReaders.IsEmpty,
                    "There shouldn't be any blocked readers if there's a blocked writer."
                );
            }
