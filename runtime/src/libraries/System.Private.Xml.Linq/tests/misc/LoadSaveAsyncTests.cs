...
namespace CoreXml.Test.XLinq
{
    public class LoadSaveAsyncTests : BridgeHelpers
    {
        ...
        public static async Task AlreadyCanceled()
        {
            ...
            await Assert.ThrowsAnyAsync<OperationCanceledException>(
                () =>
                    new XDocument()
                        .SaveAsync(Stream.Null, SaveOptions.None, new CancellationToken(true))
            );
            await Assert.ThrowsAnyAsync<OperationCanceledException>(
                () =>
                    new XDocument()
                        .SaveAsync(StreamWriter.Null, SaveOptions.None, new CancellationToken(true))
            );
            await Assert.ThrowsAnyAsync<OperationCanceledException>(
                () =>
                    new XDocument()
                        .SaveAsync(XmlWriter.Create(Stream.Null), new CancellationToken(true))
            );
        ...
        public static async Task RoundtripSyncAsyncMatches_XmlReader(
            bool document,
        ...
        )
        {
            ...
            using (
                XmlReader syncReader = XmlReader.Create(
            ...
            using (
                XmlReader asyncReader = XmlReader.Create(
                    FilePathUtil.getStream(GetTestFileName()),
                    readerSettings
                )
            )
            using (XmlWriter asyncWriter = XmlWriter.Create(asyncOutput, writerSettings))
            {
                ...
                {
                    XElement asyncElement = await XElement.LoadAsync(
                        asyncReader,
                        loadOptions,
                        CancellationToken.None
                    );
        ...
        [Theory]
        [MemberData(nameof(RoundtripOptions_MemberData))]
        public static async Task RoundtripSyncAsyncMatches_StreamReader(
            bool document,
            LoadOptions loadOptions,
            SaveOptions saveOptions
        )
        {
            ...
            MemoryStream syncOutput = new MemoryStream();
            using (
                StreamReader syncReader = new StreamReader(
                    FilePathUtil.getStream(GetTestFileName())
                )
            )
            ...
            MemoryStream asyncOutput = new MemoryStream();
            using (
                StreamReader asyncReader = new StreamReader(
                    FilePathUtil.getStream(GetTestFileName())
                )
            )
        ...
        [Theory]
        [MemberData(nameof(RoundtripOptions_MemberData))]
        public static async Task RoundtripSyncAsyncMatches_Stream(
            bool document,
            LoadOptions loadOptions,
            SaveOptions saveOptions
        )
        ...
        [Theory]
        [MemberData(nameof(RoundtripOptions_MemberData))]
        public static async Task RoundtripSyncAsyncCheckAndMatches_XmlReader(
            bool document,
            LoadOptions loadOptions,
            SaveOptions saveOptions
        )
        {
            ...
            readerSettings.Async = true;
            writerSettings.Async = true;
            CheckSyncAsyncStream asyncInput = new CheckSyncAsyncStream(
                async: true,
                FilePathUtil.getStream(GetTestFileName())
            );
            MemoryStream asyncOutputMemory = new MemoryStream();
            CheckSyncAsyncStream asyncOutput = new CheckSyncAsyncStream(
                async: true,
                asyncOutputMemory
            );
        ...
        [Theory]
        [MemberData(nameof(RoundtripOptions_MemberData))]
        public static async Task RoundtripSyncAsyncCheckAndMatches_StreamReader(
            bool document,
            LoadOptions loadOptions,
            SaveOptions saveOptions
        )
        ...
        [Theory]
        [MemberData(nameof(RoundtripOptions_MemberData))]
        public static async Task RoundtripSyncAsyncCheckAndMatches_Stream(
            bool document,
            LoadOptions loadOptions,
            SaveOptions saveOptions
        )
        {
            ...
            using (
                Stream syncStream = new CheckSyncAsyncStream(
                    async: false,
                    FilePathUtil.getStream(GetTestFileName())
                )
            )
            ...
            await using (
                Stream asyncStream = new CheckSyncAsyncStream(
                    async: true,
                    FilePathUtil.getStream(GetTestFileName())
                )
            )
    ...
    public class ForceSyncAsyncStream : MemoryStream
    {
        ...
        public override void Flush()
        {
            Assert.False(
                _isAsync,
                "Stream is in asynchronous mode when synchronous Flush is called"
            );
            base.Flush();
        }
        ...
        public override Task FlushAsync(CancellationToken cancellationToken)
        {
            Assert.True(
                _isAsync,
                "Stream is not in asynchronous mode when asynchronous Flush is called"
            );
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            Assert.False(
                _isAsync,
                "Stream is in asynchronous mode when synchronous Write is called"
            );
    ...
    public class CheckSyncAsyncStream : Stream
    {
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            if (!_isAsyncInProgress)
                Assert.False(
                    _isAsync,
                    "Stream is in asynchronous mode when synchronous Read is called"
                );
        ...
        )
        {
            try
            {
                Assert.True(
                    _isAsync,
                    "Stream is not in asynchronous mode when asynchronous Read is called"
                );
                _isAsyncInProgress = true;
