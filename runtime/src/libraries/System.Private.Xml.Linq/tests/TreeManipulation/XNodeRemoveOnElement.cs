...
namespace XLinqTests
{
    public class XNodeRemoveOnElement : XNodeRemove
    {
        ...
        {
            AddChild(
                new TestVariation(OnElement)
                {
                    Attribute = new VariationAttribute("(BVT)Remove from XElement (in XDocument)")
                    {
                        Params = new object[] { false, true, 2 },
                        Priority = 0,
                    },
                }
            );
            ...
            AddChild(
                new TestVariation(OnElement)
                {
                    Attribute = new VariationAttribute("Remove from XElement (standalone)")
                    {
                        Params = new object[] { false, false, 4 },
                        Priority = 1,
                    },
                }
            );
            ...
            AddChild(
                new TestVariation(OnElement)
                {
                    Attribute = new VariationAttribute("Remove from XElement (in XDocument)")
                    {
                        Params = new object[] { false, true, 4 },
                        Priority = 1,
                    },
                }
            );
