...
namespace XLinqTests
{
    ...
    public class ParamsObjectsCreationElem : XLinqTestCase
    {
        ...
        public override void AddChildren()
        {
            ...
            AddChild(
                new TestVariation(XElementDuppAttr)
                {
                    Attribute = new VariationAttribute(
            ...
            AddChild(
                new TestVariation(XElementNotAllowedXDoc)
                {
            ...
            AddChild(
                new TestVariation(XElementNotAllowedXDocType)
                {
            ...
            AddChild(
                new TestVariation(BuildFromQuery)
                {
            ...
            AddChild(
                new TestVariation(IsEmptyProp1)
                {
            ...
            AddChild(
                new TestVariation(IsEmptyProp2)
                {
        ...
        public void BuildFromQuery()
        {
            ...
            XElement e2 = XElement.Parse(
                @"<root><a1 a='a'/><a2/><a3 b='b'/><a4 c='c'/><a5/><a6/><a7/><a8/></root>"
            );

            IEnumerable<XElement> nodes =
                from data1 in e1.Elements()
                join data2 in e2.Elements()
                    on data1.Attribute("id").Value equals data2.Name.LocalName
                select data2;
            ...
            IEnumerable<XAttribute> attributes =
                ...
                join data2 in e2.Elements()
                    on data1.Attribute("id").Value equals data2.Name.LocalName
                select data2.FirstAttribute;

            IEnumerable<ExpectedValue> expectedContent = ExpectedContent(nodes)
                .ProcessNodes()
                .ToList();
            IEnumerable<ExpectedValue> expectedAttributes = ExpectedContent(attributes)
                .Where(n => n.Data is XAttribute)
                .ToList();

            XElement e = CreateElement(
                mode,
                nodes.OfType<object>().Concat(attributes.OfType<object>())
            );
        ...
        public void XElementDuppAttr()
        {
            ...
            object[] paras =
            {
                ...
                new XAttribute("other", "ooo"),
                new XAttribute("id", "a2"),
                null,
                "",
                "text",
                new XElement("aa"),
        ...
        public void XElementValidCreate()
        {
            ...
            {
                new XElement("A"),
                new XElement("A", new XAttribute("a1", "a1")),
                new object[]
                {
                    new XElement("B"),
                    "",
                ...
                new XElement("{NS1}A"),
                new XAttribute("id", "a1"),
                new XAttribute("ie", "ie"),
                new XAttribute("{NS1}id", "b2"),
                "",
                new XAttribute(XNamespace.Xmlns + "NS1", "http://ns1"),
                new XProcessingInstruction("Pi", "data"),
                new XProcessingInstruction("P2", ""),
                null,
                new XComment("comment"),
                new XText("text1"),
            ...
            {
                IEnumerable<ExpectedValue> expectedContent = ExpectedContent(data.Flatten())
                    .ProcessNodes()
                    .ToList();
                IEnumerable<ExpectedValue> expectedAttributes = ExpectedContent(data.Flatten())
                    .Where(n => n.Data is XAttribute)
                    .ToList();
                ...
                TestLog.Compare(
                    expectedContent.EqualAll(e.Nodes(), XNode.EqualityComparer),
                    "Content"
                );
                TestLog.Compare(
        ...
        private IEnumerable<ExpectedValue> ExpectedContent<T>(IEnumerable<T> data)
            where T : class
        {
            return data.Select(n => new ExpectedValue(
                (n is XNode) && (!(n is XText)) && (n as XNode).Parent == null,
                n
            ));
        }
        #endregion
