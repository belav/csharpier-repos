    ...
    public static class ReaderDiffNSAware
    {
        public class NSStack
        {
            ...
            public void Add(int depth, string prefix, string ns)
            {
                ...
                TestLog.Compare(
                    !IsDuplicate(depth, prefix, ns),
                    string.Format("Duplicate: {0}, {1}", prefix, ns)
                );
        ...
        public static void CompareNamespaceAware(
            SaveOptions so,
        ...
        public static void CompareNamespaceAware(XmlReader originalReader, XmlReader filteredReader)
        {
            ...
            while (true)
            {
                ...
                if (filteredReader.NodeType != XmlNodeType.Text)
                {
                    ...
                    TestLog.Compare(
                        originalReader.LocalName,
                        filteredReader.LocalName,
                        "localname"
                    );
                    TestLog.Compare(
                        originalReader.NamespaceURI,
                        filteredReader.NamespaceURI,
                        "Namespaceuri"
                    );
                }
                else
                ...
                if (originalReader.NodeType == XmlNodeType.Element)
                {
                    ...
                    TestLog.Compare(
                        filteredAttrs.Any(),
                        filteredReader.HasAttributes,
                        "has attributes"
                    );
                    TestLog.Compare(
                        filteredAttrs.Length,
                        filteredReader.AttributeCount,
                        "attribute count"
                    );
                    ...
                    for (int i = 0; i < filteredAttrs.Length; i++)
                    {
                        ...
                        TestLog.Compare(
                            filteredReader[i],
                            filteredAttrs[i][3],
                            "value of attribute ... indexer[int]"
                        );
                        TestLog.Compare(
                            filteredReader.GetAttribute(i),
                            filteredAttrs[i][3],
                            "value of attribute ... GetAttribute[int]"
                        );
                    ...
                    for (int i = 0; i < filteredAttrs.Length; i++)
                    {
                        ...
                        TestLog.Compare(
                            filteredReader[n[1], n[2]],
                            n[3],
                            "value of attribute ... indexer[string, string]"
                        );
                        TestLog.Compare(
                            filteredReader.GetAttribute(n[1], n[2]),
                            n[3],
                            "value of attribute ... GetAttribute[string, string]"
                        );
                        TestLog.Compare(
                            filteredReader.MoveToAttribute(n[1], n[2]),
                            "Move to attribute string/string"
                        );
                        TestLog.Compare(
                            SAEqComparer.Instance.Equals(filteredReader.GetSAData(), n),
                            "Move to attribute string/string - wrong position"
                        );
                        VerifyNextFirstAttribute(
                            filteredReader,
                            filteredAttrs,
                            i,
                            " after MoveToAttribute (string,string)"
                        );
                        ...
                        TestLog.Compare(
                            filteredReader[qName],
                            n[3],
                            "value of attribute ... indexer[string]"
                        );
                        TestLog.Compare(
                            filteredReader.GetAttribute(qName),
                            n[3],
                            "value of attribute ... GetAttribute[string]"
                        );
                        TestLog.Compare(
                            filteredReader.MoveToAttribute(qName),
                            "Move to attribute string/string"
                        );
                        TestLog.Compare(
                            SAEqComparer.Instance.Equals(filteredReader.GetSAData(), n),
                        ...
                        {
                            TestLog.Compare(
                                filteredReader.LookupNamespace(n[0] != "" ? n[1] : ""),
                                n[3],
                                "Lookup namespace failed for kept declaration"
                            );
                        }
                    }

                    // Verify that not reported NS are not accessible
                    var duplicateAttrs = origAttrs
                        .Except(filteredAttrs, SAEqComparer.Instance)
                        .ToArray();
                    foreach (var n in duplicateAttrs)
                    {
                        TestLog.Compare(
                            filteredReader[n[1], n[2]],
                            null,
                            "Should not found : value of attribute ... indexer[string, string]"
                        );
                        TestLog.Compare(
                            filteredReader.GetAttribute(n[1], n[2]),
                            null,
                            "Should not found : value of attribute ... GetAttribute[string, string]"
                        );
                        var orig = filteredReader.GetSAData();
                        TestLog.Compare(
                            !filteredReader.MoveToAttribute(n[1], n[2]),
                            "Should not found : Move to attribute string/string"
                        );
                        TestLog.Compare(
                            SAEqComparer.Instance.Equals(filteredReader.GetSAData(), orig),
                            "Should not found : Move to attribute string/string - wrong position"
                        );
                        ...
                        TestLog.Compare(
                            filteredReader[qName],
                            null,
                            "Should not found : value of attribute ... indexer[string]"
                        );
                        TestLog.Compare(
                            filteredReader.GetAttribute(qName),
                            null,
                            "Should not found : value of attribute ... GetAttribute[string]"
                        );
                        ...
                        TestLog.Compare(
                            n[2] == XNamespace.Xmlns || n[1] == "xmlns",
                            "Removed is not namespace declaration"
                        );
                        ...
                        TestLog.Compare(
                            filteredReader.LookupNamespace(n[0] != "" ? n[1] : ""),
                            n[3],
                            "Lookup namespace failed - for removed declaration"
                        );
                    }
                ...
                // nsStack cleanup
                if (
                    originalReader.NodeType == XmlNodeType.EndElement
                    || originalReader.IsEmptyElement
                )
                    nsStack.RemoveDepth(filteredReader.Depth);
            }
        }

        private static void VerifyNextFirstAttribute(
            XmlReader filteredReader,
        ...
        )
        {
            ...
            var orig = filteredReader.GetSAData();
            TestLog.Compare(
                filteredReader.MoveToNextAttribute(),
                shouldNAWork,
            ...
            TestLog.Compare(
                filteredReader.MoveToFirstAttribute(),
                "Move to first attribute should always work :: " + message
            );
            TestLog.Compare(
                SAEqComparer.Instance.Equals(filteredAttrs[0], filteredReader.GetSAData()),
                "MoveToNextAttribute moved to bad position :: " + message
            );
        }
        ...
        private static IEnumerable<string[]> IndexedAttributeEnumerator(
            XmlReader r,
            bool moveToElement
        )
        {
                ...
                while (!found)
                {
                    ...
                    if (!found)
                    {
                        ...
                        TestLog.Compare(
                            ...
                            "The namespace decl was not duplicate"
                        );
                    }
                }
