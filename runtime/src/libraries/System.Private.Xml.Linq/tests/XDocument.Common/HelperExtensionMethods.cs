...
namespace CoreXml.Test.XLinq
{
    public static class Helpers
    {
        ...
        public static XmlReaderSettings WhitespaceReaderSettings
        {
            get { return s_whitespacereaderSetting; }
        }
        ...
        /// <returns></returns>
        public static IEnumerable<T[]> NonRecursiveVariations<T>(
            this T[] source,
            int length,
        ...
        public static bool EqualAll<T1, T2>(
        ...
        public static bool EqualAllAttributes<T1, T2>(
            this IEnumerable<T1> first,
            IEnumerable<T2> second,
            IEqualityComparer<T2> comparer
        )
        ...
        public static bool EqualsAllAttributes<T1>(
            this IEnumerable<T1> first,
        ...
        )
        {
            ...
            if (e.MoveNext())
            {
                ...
                do
                {
                    if (e.Current.Data == null)
                        continue;
                    if (e.Current.Data is XAttribute)
                        continue;
                    if ((e.Current.Data is string) && ((e.Current.Data as string) == string.Empty))
                        continue;
                    if (e.Current.Data is XNode)
                    {
                        ...
                        {
                            concatenator = new ExpectedValue(
                                e.Current.IsOriginalNode,
                                new XText(e.Current.Data as XText),
                                e.Current.IsOriginalNode ? e.Current.Data as XText : null
                            );
                            continue;
                        }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            concatenator = new ExpectedValue(
                                false,
                                new XText(e.Current.Data.ToString())
                            );
                        }
                    }
                } while (e.MoveNext());
        ...
        public static void Verify(this XAttribute at)
        {
            ...
            {
                TestLog.Compare(
                    at.PreviousAttribute.NextAttribute == at,
            ...
            {
                TestLog.Compare(
                    at.NextAttribute.PreviousAttribute == at,
        ...
        public static void Verify(this XElement elem)
        {
            ...
            TestLog.Compare(
                elem.ToString(SaveOptions.DisableFormatting) != null,
                "cannot serialize"
            ); // just checking that the exception is not thrown

            if (elem.IsEmpty)
            {
                TestLog.Compare(elem.FirstNode == null, "IsEmpty has nodes");
                TestLog.Compare(
                    !elem.ToString(SaveOptions.DisableFormatting).Contains("></"),
                    "IsEmpty and serialization (true case)"
                );
            }
            else
            {
                if (elem.FirstNode == null)
                {
                    TestLog.Compare(
                        elem.ToString(SaveOptions.DisableFormatting).Contains("></"),
                        "IsEmpty and serialization (false case)"
                    );
                }
            }
        ...
        public static XAttributeEqualityComparer<XAttribute> MyAttributeComparer =
        ...
        public static bool CheckDTDAfterElement(this IEnumerable<object> newNodes)
        {
            ...
            foreach (object o in newNodes)
            {
                if (o is XElement)
                    wasElement = true;
                if (o is XDocumentType && wasElement)
                    return true;
            }
            return false;
        }
        ...
        public static bool IsXDocValid(this IEnumerable<ExpectedValue> expectedNodes)
        {
            ...
            bool[] flags = new bool[]
            {
                ...
                nn.Where(x => (x is XText))
                    .Select(x => (x as XText).Value.ToCharArray())
                    .Where(x => x.Where(y => y != ' ' && y != '\n' && y != '\t').Any())
                    .Any(),
                nn.Where(x => (x is string))
                    .Select(x => (x as string).ToCharArray())
                    .Where(x => x.Where(y => y != ' ' && y != '\n' && y != '\t').Any())
                    .Any(),
    ...
    public class XAttributeEqualityComparer<T> : IEqualityComparer, IEqualityComparer<T>
        where T : XAttribute
    {
    ...
    public class ExpectedValue
    {
        ...
        public bool IsOriginalNode
        {
            get { return _isOriginalNode; }
        }
    ...
    public class XNodeStructuralEqualityComparer<T> : XNodeBaseEqualityComparer<T> where T : XObject
    {
        ...
        {
            foreach (XNode m in n.Nodes())
                startHash ^= CalculateHashCode(m);
            return startHash;
        }

        public override int ComputeHash4Attributes(XElement n, int startHash)
        {
            foreach (XAttribute a in n.Attributes())
                startHash ^= CalculateHashCode(a);
            return startHash;
        }
    }

    public abstract class XNodeBaseEqualityComparer<T> : IEqualityComparer, IEqualityComparer<T>
        where T : XObject
    {
        ...
        public bool Equals(T n1, T n2)
        {
            ...
            {
                return (
                    (n1 as XNode).ToString(SaveOptions.DisableFormatting)
                    == (n2 as XNode).ToString(SaveOptions.DisableFormatting)
                );
            }
            if (n1 is XAttribute)
            {
