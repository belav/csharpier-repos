...
namespace Microsoft.CSharp.RuntimeBinder.Semantics
{
    ...
    internal readonly partial struct ExpressionBinder
    {
        internal sealed class GroupToArgsBinder
        {
            ...
            {
                Debug.Assert(
                    _pGroup.SymKind == SYMKIND.SK_MethodSymbol
                        || _pGroup.SymKind == SYMKIND.SK_PropertySymbol
                            && 0 != (_pGroup.Flags & EXPRFLAG.EXF_INDEXER)
                );
            ...
            private void LookForCandidates()
            {
                ...
                while (true)
                {
                    ...
                    {
                        bSearchForExpanded = (
                            currentTypeArgsResult == Result.Failure_SearchForExpanded
                        );
                        continue;
                    }
                ...
                if (!_methList.IsEmpty())
                {
                    ...
                    else
                    {
                        ...
                        pmethBest = _pExprBinder.FindBestMethod(
                            ...
                            pTypeThrough,
                            _pArguments,
                            out CandidateFunctionMember pAmbig1,
                            out CandidateFunctionMember pAmbig2
                        );

                        if (null == pmethBest)
                        {
                            if (
                                pAmbig1.@params != pAmbig2.@params
                                || pAmbig1.mpwi.MethProp().Params.Count
                                    != pAmbig2.mpwi.MethProp().Params.Count
            ...
            )
            {
                ...
                if (methprop.HasDefaultParameterValue(index))
                {
                    ...
                    if (
                        pConstValType.IsPredefType(PredefinedType.PT_DATETIME)
                        && (
                            pRawParamType.IsPredefType(PredefinedType.PT_DATETIME)
                    ...
                    )
                    {
                        ...
                        optionalArgument = ExprFactory.CreateConstant(
                            dateTimeType,
                            ConstVal.Get(DateTime.FromBinary(cv.Int64Val))
                        );
                    }
                    ...
                    if (pParamType.IsPredefType(PredefinedType.PT_OBJECT))
                    {
                        ...
                        else
                        {
                            ...
                            AggregateSymbol agg = SymbolLoader.GetPredefAgg(
                                PredefinedType.PT_MISSING
                            );
                            Name name = NameManager.GetPredefinedName(PredefinedName.PN_CAP_VALUE);
                            FieldSymbol field =
                                SymbolLoader.LookupAggMember(name, agg, symbmask_t.MASK_FieldSymbol)
                                as FieldSymbol;
                            FieldWithType fwt = new FieldWithType(field, agg.getThisType());
                            ExprField exprField = ExprFactory.CreateField(
                                agg.getThisType(),
                                null,
                                fwt
                            );
            ...
            private Result DetermineCurrentTypeArgs()
            {
                ...
                // Get the type args.
                if (
                    _pCurrentSym is MethodSymbol methSym
                    && methSym.typeVars.Count != typeArgs.Count
                )
                {
            ...
            private bool ArgumentsAreConvertible()
            {
                ...
                if (_pArguments.carg != 0)
                {
                    ...
                    for (int ivar = 0; ivar < _pArguments.carg; ivar++)
                    {
                        ...
                        bool constraintErrors = !TypeBind.CheckConstraints(
                            var,
                            CheckConstraintsFlags.NoErrors
                        );
                    ...
                    for (int ivar = 0; ivar < _pArguments.carg; ivar++)
                    {
                        ...
                        if (!fresult && !containsErrorSym)
                        {
                            ...
                            else if (ivar == _nArgBest && _pArguments.types[ivar] != var)
                            {
                                ...
                                CType argStripped = _pArguments.types[ivar]
                                    is ParameterModifierType modArg
                                    ? modArg.ParameterType
                                    : _pArguments.types[ivar];
                                CType varStripped = var is ParameterModifierType modVar
                                    ? modVar.ParameterType
                                    : var;
            ...
            private void UpdateArguments()
            {
                ...
                for (int iParam = 0; iParam < _pCurrentParameters.Count; ++iParam)
                {
                    ...
                    Expr pArgumentNew = GenerateOptionalArgument(
                        pMethod,
                        _pCurrentParameters[iParam],
                        iParam
                    );
            ...
            private void ReportErrorsOnSuccess()
            {
                ...
                Debug.Assert(
                    0 == (_pGroup.Flags & EXPRFLAG.EXF_USERCALLABLE)
            ...
            private RuntimeBinderException ReportErrorsOnFailure()
            {
                ...
                if (_pDuplicateSpecifiedName != null)
                {
                    return ErrorHandling.Error(
                        ErrorCode.ERR_DuplicateNamedArgument,
                        _pDuplicateSpecifiedName
                    );
                }
                ...
                if (_misnamed)
                {
                    ...
                    List<Name> paramNames = FindMostDerivedMethod(
                        _misnamed.MethProp(),
                        _pGroup.OptionalObject
                    ).ParameterNames;
                    for (int i = 0; ; ++i)
                ...
                )
                {
                    ...
                    Debug.Assert(
                        !_results.BestResult
                            || _results.BestResult.GetType().OwningAggregate.IsDelegate()
                    );
                ...
                if (_results.UninferableResult || _mpwiCantInferInstArg)
                {
                    ...
                    MethWithType mwtCantInfer = new MethWithType(
                        _results.UninferableResult.Meth(),
                ...
                if (_mwtBadArity)
                {
                    ...
                    return ErrorHandling.Error(
                        cvar > 0 ? ErrorCode.ERR_BadArity : ErrorCode.ERR_HasNoTypeVars,
                        _mwtBadArity,
                        new ErrArgSymKind(_mwtBadArity.Meth()),
                        _pArguments.carg
                    );
                }
