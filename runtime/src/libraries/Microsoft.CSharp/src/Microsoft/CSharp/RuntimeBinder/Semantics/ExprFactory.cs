...
namespace Microsoft.CSharp.RuntimeBinder.Semantics
{
    internal static class ExprFactory
    {
        ...
        public static ExprPropertyInfo CreatePropertyInfo(
            ...
            new ExprPropertyInfo(
                TypeManager.GetPredefAgg(PredefinedType.PT_PROPERTYINFO).getThisType(),
                prop,
                propertyType
            );
        ...
        public static ExprFieldInfo CreateFieldInfo(FieldSymbol field, AggregateType fieldType) =>
            new ExprFieldInfo(
                field,
                fieldType,
                TypeManager.GetPredefAgg(PredefinedType.PT_FIELDINFO).getThisType()
            );

        [RequiresUnreferencedCode(Binder.TrimmerWarning)]
        public static ExprTypeOf CreateTypeOf(CType sourceType) =>
            new ExprTypeOf(
                TypeManager.GetPredefAgg(PredefinedType.PT_TYPE).getThisType(),
                sourceType
            );
        ...
        public static ExprConcat CreateConcat(Expr first, Expr second) =>
            new ExprConcat(first, second);

        [RequiresUnreferencedCode(Binder.TrimmerWarning)]
        public static ExprConstant CreateStringConstant(string str) =>
            CreateConstant(
                TypeManager.GetPredefAgg(PredefinedType.PT_STRING).getThisType(),
                ConstVal.Get(str)
            );
        ...
        public static Expr CreateZeroInit(CType type)
        {
            ...
            if (type.IsEnumType)
            {
                ...
                return CreateConstant(
                    type,
                    ConstVal.Get(Activator.CreateInstance(type.AssociatedSystemType))
                );
            }
        ...
        public static ExprConstant CreateConstant(CType type, ConstVal constVal) =>
            new ExprConstant(type, constVal);

        [RequiresUnreferencedCode(Binder.TrimmerWarning)]
        public static ExprConstant CreateIntegerConstant(int x) =>
            CreateConstant(
                TypeManager.GetPredefAgg(PredefinedType.PT_INT).getThisType(),
                ConstVal.Get(x)
            );
        ...
        public static ExprConstant CreateBoolConstant(bool b) =>
            CreateConstant(
                TypeManager.GetPredefAgg(PredefinedType.PT_BOOL).getThisType(),
                ConstVal.Get(b)
            );
        ...
        public static ExprList CreateList(Expr op1, Expr op2, Expr op3) =>
            CreateList(op1, CreateList(op2, op3));
