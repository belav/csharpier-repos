...
o   S and T differ only in element type. (In other words, S and T have the same number of
dimensions.)
...
System.Collections.Generic.IReadOnlyList<T> and their base interfaces, provided there is an explicit
...
o If type parameter Xi is declared to be contravariant ("in") then either Si must be identical to
Ti,
or Si and Ti must both be reference types.
*   From System.Collections.Generic.IList<T>, System.Collections.Generic.IReadOnlyList<T> and their
base interfaces to a one-dimensional array-type S[], provided there is an implicit or explicit
reference conversion from S[] to System.Collections.Generic.IList<T> or
System.Collections.Generic.IReadOnlyList<T>. This is precisely when either S and T are the same type
or there is an implicit or explicit reference conversion from S to T.

For a type-parameter T that is known to be a reference type (25.7), the following explicit reference
conversions exist:
*   From the effective base class C of T to T and from any base class of C to T.
*   From any interface-type to T.
*   From T to any interface-type I provided there isn't already an implicit reference conversion
from T to I.
*   From a type-parameter U to T provided that T depends on U (25.7). [Note: Since T is known to be
a reference type, within the scope of T, the run-time type of U will always be a reference type,
even if U is not known to be a reference type at compile-time. end note]
...
***************************************************************************************************/
        ...
        public static bool FExpRefConv(CType typeSrc, CType typeDst)
        {
            ...
            if (typeSrc.IsReferenceType && typeDst.IsReferenceType)
            {
                ...
                // For a type-parameter T that is known to be a reference type (25.7), the following explicit
                ...
                if (typeDst is ArrayType arrayDest && typeSrc is AggregateType aggtypeSrc)
                {
                    ...
                    // *    From System.Collections.Generic.IList<T>, System.Collections.Generic.IReadOnlyList<T> and
                    // their base interfaces to a
                    //      one-dimensional array-type S[], provided there is an implicit or explicit reference
                    // conversion from S[] to
                    //      System.Collections.Generic.IList<T> or System.Collections.Generic.IReadOnlyList<T>. This is
                    // precisely when either S and T
...
*   The term wrapping denotes the process of packaging a value, of type T, in an instance of type
T?.
