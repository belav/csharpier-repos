...
namespace System.Net.Sockets
{
    internal sealed class DynamicWinsockMethods
    {
        ...
        private static readonly List<DynamicWinsockMethods> s_methodTable =
            new List<DynamicWinsockMethods>();

        public static DynamicWinsockMethods GetMethods(
        ...
        private static T CreateDelegate<T>(
            Func<IntPtr, T> functionPointerWrapper,
            [NotNull] ref T? cache,
        ...
        internal unsafe AcceptExDelegate GetAcceptExDelegate(SafeSocketHandle socketHandle) =>
            _acceptEx
            ?? CreateDelegate(
                ptr => new SocketDelegateHelper(ptr).AcceptEx,
                ref _acceptEx,
                socketHandle,
                "b5367df1cbac11cf95ca00805f48a192"
            );

        internal unsafe GetAcceptExSockaddrsDelegate GetGetAcceptExSockaddrsDelegate(
            SafeSocketHandle socketHandle
        ) =>
            _getAcceptExSockaddrs
            ?? CreateDelegate<GetAcceptExSockaddrsDelegate>(
                ptr => new SocketDelegateHelper(ptr).GetAcceptExSockaddrs,
                ref _getAcceptExSockaddrs,
                socketHandle,
                "b5367df2cbac11cf95ca00805f48a192"
            );

        internal unsafe ConnectExDelegate GetConnectExDelegate(SafeSocketHandle socketHandle) =>
            _connectEx
            ?? CreateDelegate(
                ptr => new SocketDelegateHelper(ptr).ConnectEx,
                ref _connectEx,
                socketHandle,
                "25a207b9ddf346608ee976e58c74063e"
            );

        internal unsafe DisconnectExDelegate GetDisconnectExDelegate(
            SafeSocketHandle socketHandle
        ) =>
            _disconnectEx
            ?? CreateDelegate(
                ptr => new SocketDelegateHelper(ptr).DisconnectEx,
                ref _disconnectEx,
                socketHandle,
                "7fda2e118630436fa031f536a6eec157"
            );

        internal unsafe WSARecvMsgDelegate GetWSARecvMsgDelegate(SafeSocketHandle socketHandle) =>
            _recvMsg
            ?? CreateDelegate(
                ptr => new SocketDelegateHelper(ptr).WSARecvMsg,
                ref _recvMsg,
                socketHandle,
                "f689d7c86f1f436b8a53e54fe351c322"
            );

        internal unsafe TransmitPacketsDelegate GetTransmitPacketsDelegate(
            SafeSocketHandle socketHandle
        ) =>
            _transmitPackets
            ?? CreateDelegate(
                ptr => new SocketDelegateHelper(ptr).TransmitPackets,
                ref _transmitPackets,
                socketHandle,
                "d9689da01f9011d3997100c04f68c876"
            );

        /// <summary>
        /// The SocketDelegateHelper implements manual marshalling wrappers for the various delegates used
        // for the dynamic Winsock methods.
        /// These wrappers were generated with LibraryImportGenerator and then manually converted to use
        // function pointers as the target instead of a P/Invoke.
        /// </summary>
        private readonly struct SocketDelegateHelper
        {
                ...
                try
                {
                    ...
                    fixed (int* __bytesReceived_gen_native = &bytesReceived)
                    {
                        __retVal_gen_native = (
                            ...
                            __acceptSocketHandle_gen_native,
                            buffer,
                            len,
                            localAddressLength,
                            remoteAddressLength,
                            __bytesReceived_gen_native,
                            overlapped
                        );
                    }
            ...
            internal unsafe void GetAcceptExSockaddrs(
                IntPtr buffer,
            ...
            )
            {
                ...
                fixed (int* __remoteSocketAddressLength_gen_native = &remoteSocketAddressLength)
                {
                    (
                        ...
                        localAddressLength,
                        remoteAddressLength,
                        __localSocketAddress_gen_native,
                        __localSocketAddressLength_gen_native,
                        __remoteSocketAddress_gen_native,
                        __remoteSocketAddressLength_gen_native
                    );
                }
                Marshal.SetLastPInvokeError(Marshal.GetLastSystemError());
            }
            ...
            )
            {
                ...
                try
                {
                    ...
                    __socketHandle_gen_native = socketHandle.DangerousGetHandle();
                    __retVal_gen_native = (
                        (delegate* unmanaged<IntPtr, NativeOverlapped*, int, int, int>)_target
                    )(__socketHandle_gen_native, overlapped, flags, reserved);
                    Marshal.SetLastPInvokeError(Marshal.GetLastSystemError());
                    //
                    // Unmarshal
                    //
