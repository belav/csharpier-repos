...
namespace System.Net.Sockets
{
    ...
    public partial class Socket : IDisposable
    {
        ...
        public IAsyncResult BeginDisconnect(
            ...
            DisconnectOverlappedAsyncResult asyncResult = new DisconnectOverlappedAsyncResult(
                this,
                state,
                callback
            );
        ...
        public void EndDisconnect(IAsyncResult asyncResult)
        {
            ...
            if (castedAsyncResult.EndCalled)
            {
                throw new InvalidOperationException(
                    SR.Format(SR.net_io_invalidendcall, nameof(EndDisconnect))
                );
            }
            ...
            if ((SocketError)castedAsyncResult.ErrorCode != SocketError.Success)
            {
                UpdateStatusAfterSocketErrorAndThrowException(
                    (SocketError)castedAsyncResult.ErrorCode
                );
            }
        }
        ...
        private IAsyncResult BeginAcceptCommon(
            Socket? acceptSocket,
            int receiveSize,
            AsyncCallback? callback,
            object? state
            ...
            AcceptOverlappedAsyncResult asyncResult = new AcceptOverlappedAsyncResult(
                this,
                state,
                callback
            );
            ...
            asyncResult.AcceptSocket = GetOrCreateAcceptSocket(
                acceptSocket,
                false,
                nameof(acceptSocket),
                out acceptHandle
            ...
            try
            {
                errorCode = SocketPal.AcceptAsync(
                    this,
                    _handle,
                    acceptHandle,
                    receiveSize,
            ...
            if (NetEventSource.Log.IsEnabled())
                NetEventSource.Info(this, $"AcceptAsync returns:{errorCode} {asyncResult}");
        ...
        private Socket EndAcceptCommon(
            out byte[]? buffer,
            ...
            {
                if (SocketsTelemetry.Log.IsEnabled())
                    SocketsTelemetry.Log.AfterAccept(SocketError.Interrupted);
            ...
            AcceptOverlappedAsyncResult? castedAsyncResult =
                asyncResult as AcceptOverlappedAsyncResult;
            if (castedAsyncResult == null || castedAsyncResult.AsyncObject != this)
            {
                throw new ArgumentException(SR.net_io_invalidasyncresult, nameof(asyncResult));
            }
            ...
            {
                throw new InvalidOperationException(
                    SR.Format(SR.net_io_invalidendcall, "EndAccept")
                );
            }
        ...
        private static FileStream? OpenFile(string? name) =>
            string.IsNullOrEmpty(name) ? null : File.OpenRead(name);
