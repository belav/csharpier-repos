...
namespace System.Net.Sockets
{
    internal sealed unsafe class SocketAsyncEngine : IThreadPoolWorkItem
    {
        ...
        internal static readonly bool InlineSocketCompletionsEnabled =
        ...
        private static int GetEngineCount()
        {
            ...
            int coresPerEngine =
                architecture == Architecture.Arm64 || architecture == Architecture.Arm ? 8 : 30;

            return Math.Max(
                1,
                (int)Math.Round(Environment.ProcessorCount / (double)coresPerEngine)
            );
        }
        ...
        > _handleToContextMap = new ConcurrentDictionary<IntPtr, SocketAsyncContextWrapper>();
        ...
        private readonly ConcurrentQueue<SocketIOEvent> _eventQueue =
            new ConcurrentQueue<SocketIOEvent>();
        ...
        private void EventLoop()
        {
            ...
            catch (Exception e)
            {
                Environment.FailFast(
                    "Exception thrown from SocketAsyncEngine event loop: " + e.ToString(),
                    e
                );
            }
        }
            ...
            public bool HandleSocketEvents(int numEvents)
            {
                ...
                )
                {
                    ...
                    )
                    {
                        ...
                        {
                            Interop.Sys.SocketEvents events = context.HandleSyncEventsSpeculatively(
                                socketEvent.Events
                            );
