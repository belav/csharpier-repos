...
namespace System.Net.Sockets.Tests
{
    ...
    public class DualModeAcceptAsync : DualModeBase
    {
        ...
        private void DualModeConnect_AcceptAsync_Helper(IPAddress listenOn, IPAddress connectTo)
        {
            using (Socket serverSocket = new Socket(SocketType.Stream, ProtocolType.Tcp))
            {
                ...
                int completedHandle = WaitHandle.WaitAny(
                    waitHandles,
                ...
                if (completedHandle == WaitHandle.WaitTimeout)
                {
                    throw new TimeoutException(
                        "Timed out while waiting for either of client and server connections..."
                    );
                }
                ...
                if (completedHandle == 1) // Client finished
                {
                    ...
                    {
                        throw new TimeoutException(
                            "Timed out while waiting for the server accept..."
                        );
                    }
                }
    ...
    public class DualModeConnectionlessSendTo : DualModeBase
    {
        ...
        {
            DualModeSendTo_IPEndPointToHost_Helper(
                IPAddress.IPv6Loopback,
                IPAddress.IPv6Loopback,
                false
            );
    ...
    public class DualModeConnectionlessBeginSendTo : DualModeBase
    {
        ...
        public void Socket_BeginSendToV4IPEndPointToV4Host_Throws()
        {
            using (Socket socket = new Socket(SocketType.Dgram, ProtocolType.Udp))
            {
                ...
                Assert.Throws<SocketException>(() =>
                {
                    ...
                    IAsyncResult result = socket.BeginSendTo(
                        new byte[1],
                        0,
                        1,
                        SocketFlags.None,
        ...
        {
            DualModeBeginSendTo_EndPointToHost_Helper(
                IPAddress.Loopback,
                IPAddress.Loopback,
                false
            );
        ...
        private void DualModeBeginSendTo_EndPointToHost_Helper(
            IPAddress connectTo,
            IPAddress listenOn,
            bool dualModeServer,
            bool expectedToTimeout = false
    ...
    public class DualModeConnectionlessSendToAsync : DualModeBase
    {
        ...
        {
            DualModeSendToAsync_IPEndPointToHost_Helper(
                IPAddress.Loopback,
                IPAddress.Loopback,
                false
            );
        ...
        private void DualModeSendToAsync_IPEndPointToHost_Helper(
            IPAddress connectTo,
            IPAddress listenOn,
            bool dualModeServer,
            bool expectedToTimeout = false
    ...
    public class DualModeConnectionlessReceiveFrom : DualModeBase
    {
        ...
        [SkipOnPlatform(
            ...
            "ReceiveFrom not supported on Apple platforms"
        )]
    ...
    public class DualModeConnectionlessBeginReceiveFrom : DualModeBase
    {
        ...
        [SkipOnPlatform(
            TestPlatforms.OSX | TestPlatforms.MacCatalyst | TestPlatforms.iOS | TestPlatforms.tvOS,
            "BeginReceiveFrom not supported on Apple platforms"
        )]
        // "The parameter remoteEP must not be of type DnsEndPoint."
        ...
        private void BeginReceiveFrom_Helper(
            IPAddress listenOn,
            IPAddress connectTo,
            bool expectedToTimeout = false
        )
        {
            using (Socket serverSocket = new Socket(SocketType.Dgram, ProtocolType.Udp))
            {
                ...
                IAsyncResult async = serverSocket.BeginReceiveFrom(
                    new byte[1],
                    0,
                    1,
                    SocketFlags.None,
    ...
    public class DualModeConnectionlessReceiveFromAsync : DualModeBase
    {
        ...
        [SkipOnPlatform(
            TestPlatforms.OSX | TestPlatforms.MacCatalyst | TestPlatforms.iOS | TestPlatforms.tvOS,
            "ReceiveFromAsync not supported on Apple platforms"
        )]
        // "The parameter remoteEP must not be of type DnsEndPoint."
    ...
    public class DualModeConnectionlessReceiveMessageFrom : DualModeBase
    {
        ...
        [SkipOnPlatform(
            TestPlatforms.Linux
                ...
                | TestPlatforms.iOS
                | TestPlatforms.tvOS,
            "Expected behavior is different on Apple platforms and Linux"
        )]
