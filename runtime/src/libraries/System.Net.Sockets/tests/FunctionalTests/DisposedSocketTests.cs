...
namespace System.Net.Sockets.Tests
{
    public class DisposedSocket
    {
        ...
        public void EndReceiveFrom_Throws_SocketException()
        {
            ...
            using (sender)
            {
                var iar = receiver.BeginReceiveFrom(
                    new byte[1],
                    0,
                    1,
                    SocketFlags.None,
        ...
        public void EndReceiveMessageFrom_Throws_SocketException()
        {
            ...
            using (sender)
            {
                var iar = receiver.BeginReceiveMessageFrom(
                    new byte[1],
                    0,
                    1,
                    SocketFlags.None,
                ...
                Assert.Throws<SocketException>(() =>
                    receiver.EndReceiveMessageFrom(iar, ref flags, ref remote, out packetInfo)
                );
            }
        }
            ...
            )
            {
                ...
                for (int i = 0; i < 100; i++)
                {
                    ...
                    using (Socket server = listener.Accept())
                    {
                        if (clientAsync)
                        {
                            Task<int> receiveTask = client.ReceiveAsync(
