...
namespace System.Net.Sockets.Tests
{
    public class NetworkStreamTest : ConnectedStreamConformanceTests
    {
        ...
        protected override Task<StreamPair> CreateConnectedStreamsAsync()
        {
            ...
            return Task.FromResult<StreamPair>(
                (
                    ...
                    new NetworkStream(server, ownsSocket: true)
                )
            );
        }
        ...
        {
            await RunWithConnectedNetworkStreamsAsync(
                async (server, client) =>
                {
                    ...
                    Assert.True(
                        SpinWait.SpinUntil(() => client.DataAvailable, 10000),
                        "DataAvailable did not return true in the allotted time"
                    );
                }
            );
        }
        ...
        )
        {
            ...
            )
            {
                ...
                using NetworkStream server = derivedNetworkStream
                    ? (NetworkStream)new DerivedNetworkStream(serverSocket)
            ...
            )
            {
                ...
                using (DerivedNetworkStream serverStream = new DerivedNetworkStream(server))
                {
                    ...
                    await client.SendAsync(
                        new ArraySegment<byte>(new byte[1], 0, 1),
                        SocketFlags.None
                    );
        ...
        public async Task WriteAsync_MultipleConcurrentValueTaskWrites_AsTask_Success()
        {
            await RunWithConnectedNetworkStreamsAsync(
                async (server, client) =>
                {
                    ...
                    Task s1 = server.WriteAsync(new ReadOnlyMemory<byte>(new byte[] { 42 }))
                        .AsTask();
                    Task s2 = server.WriteAsync(new ReadOnlyMemory<byte>(new byte[] { 43 }))
                        .AsTask();
                    Task s3 = server.WriteAsync(new ReadOnlyMemory<byte>(new byte[] { 44 }))
                        .AsTask();
        ...
        /// </summary>
        private static async Task RunWithConnectedNetworkStreamsAsync(
        ...
        )
        {
            ...
            try
            {
                ...
                using (var client = new TcpClient(clientEndpoint.AddressFamily))
                {
                    ...
                    using (
                        NetworkStream serverStream = new NetworkStream(
                            remote.Client,
                            serverAccess,
                            ownsSocket: true
                        )
