...
namespace System.Net.Sockets.Tests
{
    public class SelectTest
    {
        ...
        {
            var readPairs = Enumerable.Range(0, reads)
                .Select(_ => CreateConnectedSockets())
                .ToArray();
            var writePairs = Enumerable.Range(0, writes)
                .Select(_ => CreateConnectedSockets())
                .ToArray();
            try
            {
        ...
        )
        {
            ...
            try
            {
                ...
                Socket[] writes = socketPairs.Skip(socketsPerType)
                    .Take(socketsPerType)
                    .Select(p => p.Key)
                    .ToArray();
                Socket[] errors = socketPairs.Skip(socketsPerType * 2)
                    .Take(socketsPerType)
                    .Select(p => p.Key)
                    .ToArray();
        ...
        public void Select_ReadError_NoneReady(int reads, int errors)
        {
            var readPairs = Enumerable.Range(0, reads)
                .Select(_ => CreateConnectedSockets())
                .ToArray();
            var errorPairs = Enumerable.Range(0, errors)
                .Select(_ => CreateConnectedSockets())
                .ToArray();
            try
            {
