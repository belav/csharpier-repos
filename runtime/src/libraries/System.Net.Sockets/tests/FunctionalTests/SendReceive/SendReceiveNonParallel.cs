...
namespace System.Net.Sockets.Tests
{
    ...
    {
        public SendReceiveNonParallel(ITestOutputHelper output)
            : base(output) { }
        ...
        public async Task SendToRecvFrom_Datagram_UDP(IPAddress loopbackAddress, bool useClone)
        {
            ...
            using var origLeft = new Socket(
                leftAddress.AddressFamily,
                SocketType.Dgram,
                ProtocolType.Udp
            );
            using var origRight = new Socket(
                rightAddress.AddressFamily,
                SocketType.Dgram,
                ProtocolType.Udp
            );
            ...
            _output.WriteLine(
                $"{DateTime.Now}: Sending data from {rightEndpoint} to {leftEndpoint}"
            );
            ...
            using (right)
            {
                ...
                for (int i = 0; i < DatagramsToSend; i++)
                {
                    ...
                    Assert.True(
                        gotAck,
                        $"{DateTime.Now}: Timeout waiting {ReceiverAckTimeout} for receiverAck in iteration {i} after sending {sent}. Receiver is in {leftThread.Status}"
                    );
                    senderAck.Release();
    ...
    {
        public SendReceiveNonParallel_Sync(ITestOutputHelper output)
            : base(output) { }
    }
    ...
    {
        public SendReceiveNonParallel_SyncForceNonBlocking(ITestOutputHelper output)
            : base(output) { }
    }

    public sealed class SendReceiveNonParallel_Apm : SendReceiveNonParallel<SocketHelperApm>
    {
        public SendReceiveNonParallel_Apm(ITestOutputHelper output)
            : base(output) { }
    }

    public sealed class SendReceiveNonParallel_Task : SendReceiveNonParallel<SocketHelperTask>
    {
        public SendReceiveNonParallel_Task(ITestOutputHelper output)
            : base(output) { }
    }
    ...
    {
        public SendReceiveNonParallel_CancellableTask(ITestOutputHelper output)
            : base(output) { }
    }

    public sealed class SendReceiveNonParallel_Eap : SendReceiveNonParallel<SocketHelperEap>
    {
        public SendReceiveNonParallel_Eap(ITestOutputHelper output)
            : base(output) { }
    }
    ...
    {
        public SendReceiveNonParallel_SpanSync(ITestOutputHelper output)
            : base(output) { }
    }
    ...
    {
        public SendReceiveNonParallel_MemoryArrayTask(ITestOutputHelper output)
            : base(output) { }
    }
    ...
    {
        public SendReceiveNonParallel_MemoryNativeTask(ITestOutputHelper output)
            : base(output) { }
    }
}
