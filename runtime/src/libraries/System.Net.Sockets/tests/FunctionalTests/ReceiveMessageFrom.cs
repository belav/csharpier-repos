...
{
    public abstract class ReceiveMessageFrom<T> : SocketTestHelperBase<T>
        where T : SocketHelperBase, new()
    {
        ...
        protected ReceiveMessageFrom(ITestOutputHelper output)
            : base(output) { }
        ...
        public async Task ReceiveSent_TCP_Success(bool ipv6)
        {
            ...
            using (receiver)
            {
                ...
                var r = await ReceiveMessageFromAsync(
                    receiver,
                    receiveBuffer,
        ...
        public async Task ReceiveSent_UDP_Success(bool ipv4)
        {
            ...
            receiver.SetSocketOption(
                ipv4 ? SocketOptionLevel.IP : SocketOptionLevel.IPv6,
            ...
            for (int i = 0; i < DatagramsToSend; i++)
            {
                ...
                SocketReceiveMessageFromResult result = await ReceiveMessageFromAsync(
                    receiver,
                    receiveBuffer,
                    remoteEp
                );
        ...
        )
        {
            ...
            async Task RunTestAsync()
            {
                ...
                Task receiveTask = ReceiveMessageFromAsync(
                    socket,
                    new byte[1],
    ...
    {
        public ReceiveMessageFrom_Sync(ITestOutputHelper output)
            : base(output) { }
    }

    public sealed class ReceiveMessageFrom_SyncForceNonBlocking
        : ReceiveMessageFrom<SocketHelperSyncForceNonBlocking>
    {
        public ReceiveMessageFrom_SyncForceNonBlocking(ITestOutputHelper output)
            : base(output) { }
    }

    public sealed class ReceiveMessageFrom_Apm : ReceiveMessageFrom<SocketHelperApm>
    {
        public ReceiveMessageFrom_Apm(ITestOutputHelper output)
            : base(output) { }
        ...
        public void EndReceiveMessageFrom_NullEndPoint_Throws_ArgumentNullException()
        {
            ...
            IAsyncResult iar = socket.BeginReceiveMessageFrom(
                new byte[1],
                0,
                1,
                SocketFlags.None,
    ...
    {
        public ReceiveMessageFrom_Task(ITestOutputHelper output)
            : base(output) { }
    }

    public sealed class ReceiveMessageFrom_CancellableTask
        : ReceiveMessageFrom<SocketHelperCancellableTask>
    {
        public ReceiveMessageFrom_CancellableTask(ITestOutputHelper output)
            : base(output) { }
    ...
    {
        public ReceiveMessageFrom_Eap(ITestOutputHelper output)
            : base(output) { }
        ...
        public void ReceiveSentMessages_ReuseEventArgs_Success(bool ipv4, int bufferMode)
        {
            ...
            for (int i = 0; i < DatagramsToSend; i++)
            {
                ...
                if (pending)
                    Assert.True(
                        completed.Wait(TimeoutMs),
                        "Expected operation to complete within timeout"
                    );
                completed.Reset();
                ...
                Assert.Equal(
                    ((IPEndPoint)sender.LocalEndPoint).Address,
                    saea.ReceiveMessageFromPacketInfo.Address
                );
            }
        }
    }
    ...
    {
        public ReceiveMessageFrom_SpanSync(ITestOutputHelper output)
            : base(output) { }
    }

    public sealed class ReceiveMessageFrom_SpanSyncForceNonBlocking
        : ReceiveMessageFrom<SocketHelperSpanSyncForceNonBlocking>
    {
        public ReceiveMessageFrom_SpanSyncForceNonBlocking(ITestOutputHelper output)
            : base(output) { }
    }

    public sealed class ReceiveMessageFrom_MemoryArrayTask
        : ReceiveMessageFrom<SocketHelperMemoryArrayTask>
    {
        public ReceiveMessageFrom_MemoryArrayTask(ITestOutputHelper output)
            : base(output) { }
    }

    public sealed class ReceiveMessageFrom_MemoryNativeTask
        : ReceiveMessageFrom<SocketHelperMemoryNativeTask>
    {
        public ReceiveMessageFrom_MemoryNativeTask(ITestOutputHelper output)
            : base(output) { }
    }
}
