    ...
    public class TranscodingStreamTests : ConnectedStreamConformanceTests
    {
        ...
        public void Dispose_MakesMostSubsequentOperationsThrow()
        {
            ...
            Assert.Throws<ObjectDisposedException>(
                () => (object)transcodingStream.BeginRead(new byte[0], 0, 0, null, null)
            );
            Assert.Throws<ObjectDisposedException>(
                () => (object)transcodingStream.BeginWrite(new byte[0], 0, 0, null, null)
            );
            ...
            Assert.Throws<ObjectDisposedException>(
                () => (object)transcodingStream.Read(new byte[0], 0, 0)
            );
            Assert.Throws<ObjectDisposedException>(
                () => (object)transcodingStream.Read(Span<byte>.Empty)
            );
            Assert.Throws<ObjectDisposedException>(
        ...
        )
        {
            ...
            {
                delegatingInnerStreamMock
                    .Setup(o => o.ReadAsync(It.IsAny<Memory<byte>>(), expectedCancellationToken))
                    .Returns<Memory<byte>, CancellationToken>(innerStream.ReadAsync);
            }
        ...
        public async Task WriteAsync_WithPartialData()
        {
            ...
            innerStreamMock
                .Setup(o =>
                    o.WriteAsync(It.IsAny<ReadOnlyMemory<byte>>(), expectedCancellationToken)
                )
                .Returns<ReadOnlyMemory<byte>, CancellationToken>(sink.WriteAsync);
            ...
            innerStreamMock
                .Setup(o => o.WriteAsync(It.IsAny<ReadOnlyMemory<byte>>(), CancellationToken.None))
                .Returns<ReadOnlyMemory<byte>, CancellationToken>(sink.WriteAsync);
        ...
        private static readonly UnicodeEncoding ErrorCheckingUnicodeEncoding = new UnicodeEncoding(
            bigEndian: false,
            byteOrderMark: false,
