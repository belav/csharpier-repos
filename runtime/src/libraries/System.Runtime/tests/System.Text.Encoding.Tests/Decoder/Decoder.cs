    ...
    public class DecoderMiscTests
    {
        ...
        public static unsafe void ConvertTest()
        {
            ...
            fixed (char* pChars = chars)
            {
                ...
                decoder.Convert(
                    pBytes3 + bytes1.Length + bytesUsed,
                    bytes3.Length - bytes1.Length - encodedBytes,
                    pChars,
                    1,
                    true,
                    out bytesUsed,
        ...
        public static unsafe void GetCharsNegativeTest()
        {
            ...
            fixed (char* charsPtr = new char[1])
            {
                ...
                AssertExtensions.Throws<ArgumentNullException>(
                    "chars",
                    () => decoder.GetChars(pBytes, bytes.Length, null, 1, true)
                );
                AssertExtensions.Throws<ArgumentNullException>(
                    "bytes",
                    () => decoder.GetChars(null, bytes.Length, pChars, 1, true)
                );
                AssertExtensions.Throws<ArgumentNullException>(
                    "bytes",
                    () => decoder.GetCharCount(null, bytes.Length, true)
                );

                AssertExtensions.Throws<ArgumentOutOfRangeException>(
                    "byteCount",
                    () => decoder.GetChars(pBytes, -1, pChars, 1, true)
                );
                AssertExtensions.Throws<ArgumentOutOfRangeException>(
                    "charCount",
                    () => decoder.GetChars(pBytes, bytes.Length, pChars, -1, true)
                );
                AssertExtensions.Throws<ArgumentOutOfRangeException>(
                    "count",
                    () => decoder.GetCharCount(pBytes, -1, true)
                );
        ...
        public static void DecoderExceptionFallbackBufferTest()
        {
            ...
            Assert.True(
                fallbackBuffer is DecoderExceptionFallbackBuffer,
        ...
        public static void DecoderFallbackExceptionIndexTests(
            byte[] firstPayload,
            byte[] secondPayload,
            int expectedIndex
        )
        ...
        public static void DecoderReplacementFallbackBufferTest()
        {
            ...
            DecoderFallbackBuffer fallbackBuffer = decoder.FallbackBuffer;
            Assert.True(
                fallbackBuffer is DecoderReplacementFallbackBuffer,
                "Expected to get DecoderReplacementFallbackBuffer type"
            );
            Assert.True(
                fallbackBuffer.Fallback(bytes, 0),
