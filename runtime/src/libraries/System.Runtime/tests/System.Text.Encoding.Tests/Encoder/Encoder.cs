    ...
    public class EncoderMiscTests
    {
        ...
        public static unsafe void ConvertTest()
        {
            ...
            fixed (byte* pBytes = bytes)
            {
                ...
                Assert.True(
                    completed,
                    "Expected to have the full operation compeleted with bytes1"
                );
                for (int i = 0; i < bytes1.Length; i++)
                {
                    Assert.Equal(bytes1[i], pBytes[i]);
                }
                ...
                Assert.True(
                    completed,
                    "Expected to have the full operation compeleted with bytes2"
                );
                for (int i = 0; i < bytes2.Length; i++)
                {
                    Assert.Equal(bytes2[i], pBytes[i]);
                }
                ...
                Assert.True(
                    completed,
                    "Expected to have the full operation compeleted with bytes3"
                );
                for (int i = 0; i < bytes3.Length; i++)
                {
                    Assert.Equal(bytes3[i], pBytes[i]);
                }
                ...
                Assert.True(
                    completed,
                    "Expected to have the full operation compeleted with bytes3+bytes1.Length"
                );
                for (int i = 0; i < bytes2.Length; i++)
                {
                    Assert.Equal(bytes2[i], pBytes[i]);
                ...
                Assert.True(
                    bytes2.Length > bytesUsed,
                    "Expected to use less bytes when there is not enough char buffer"
                );
        ...
        public static unsafe void GetBytesTest()
        {
            ...
            fixed (byte* pBytes = bytes)
            {
                ...
                Assert.Equal(
                    bytes2.Length,
                    encoder.GetByteCount(pChars3 + s1.Length, s3.Length - s1.Length, true)
                );
                for (int i = 0; i < bytesUsed; i++)
                {
                    Assert.Equal(bytes2[i], pBytes[i]);
        ...
        public static unsafe void GetBytesNegativeTest()
        {
            ...
            fixed (byte* bytesPtr = new byte[4])
            {
                ...
                AssertExtensions.Throws<ArgumentNullException>(
                    "bytes",
                    () => encoder.GetBytes(pChars, s.Length, null, 1, true)
                );
                AssertExtensions.Throws<ArgumentNullException>(
                    "chars",
                    () => encoder.GetBytes(null, s.Length, pBytes, 4, true)
                );
                AssertExtensions.Throws<ArgumentNullException>(
                    "chars",
                    () => encoder.GetByteCount(null, s.Length, true)
                );

                AssertExtensions.Throws<ArgumentOutOfRangeException>(
                    "charCount",
                    () => encoder.GetBytes(pChars, -1, pBytes, 4, true)
                );
                AssertExtensions.Throws<ArgumentOutOfRangeException>(
                    "byteCount",
                    () => encoder.GetBytes(pChars, s.Length, pBytes, -1, true)
                );
                AssertExtensions.Throws<ArgumentOutOfRangeException>(
                    "count",
        ...
        public static void EncoderExceptionFallbackBufferTest()
        {
            ...
            Assert.True(
                fallbackBuffer is EncoderExceptionFallbackBuffer,
                "Expected to get EncoderExceptionFallbackBuffer type"
            );
            ...
            Assert.False(
                ...
                "MovePrevious expected to always return false"
            );
        ...
        [InlineData(new char[] { '\ud800' }, new char[] { '\ud800' }, -1)]
        [InlineData(new char[] { '\ud800' }, new char[] { '\udfff', '\udfff' }, 1)]
        public static void EncoderFallbackExceptionIndexTests(
            char[] firstPayload,
            char[] secondPayload,
            int expectedIndex
        )
        ...
        public static void EncoderReplacementFallbackBufferTest()
        {
            ...
            EncoderFallbackBuffer fallbackBuffer = encoder.FallbackBuffer;
            Assert.True(
                fallbackBuffer is EncoderReplacementFallbackBuffer,
                "Expected to get EncoderReplacementFallbackBuffer type"
            );
            Assert.True(
                fallbackBuffer.Fallback(chars[0], 0),
                "Expected we fallback on the given buffer"
            );
            Assert.Equal(1, fallbackBuffer.Remaining);
            Assert.False(
                fallbackBuffer.MovePrevious(),
                "Expected we cannot move back on the replacement buffer as we are at the Beginning of the buffer"
            );
            Assert.Equal('?', fallbackBuffer.GetNextChar());
            Assert.True(
                fallbackBuffer.MovePrevious(),
                "Expected we can move back on the replacement buffer"
            );
            Assert.Equal('?', fallbackBuffer.GetNextChar());
            ...
            Assert.False(
                fallbackBuffer.MovePrevious(),
                "Expected we cannot move back on the replacement buffer as we are rest the buffer"
            );
        }
