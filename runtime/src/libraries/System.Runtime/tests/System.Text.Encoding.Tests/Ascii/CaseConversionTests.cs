...
namespace System.Text.Tests
{
    public static class CaseConversionTests
    {
        ...
        public static void SingleByteConversion()
        {
            ...
            {
                byte expectedToLower = char.IsBetween((char)i, 'A', 'Z')
                    ? (byte)(i - 'A' + 'a')
                    : (byte)i;
                byte expectedToUpper = char.IsBetween((char)i, 'a', 'z')
                    ? (byte)(i + 'A' - 'a')
                    : (byte)i;
                ...
                VerifySingleChar(
                    Ascii.ToLower(sourceByte, destinationByte, out int written),
                    i,
                    expectedToLower,
                    destinationByte[0],
                    written
                ...
                destinationByte[0] = default;
                VerifySingleChar(
                    Ascii.ToUpper(sourceByte, destinationByte, out written),
                    i,
                    expectedToUpper,
                    destinationByte[0],
                    written
                ...
                VerifySingleChar(
                    Ascii.ToLower(sourceByte, destinationChar, out written),
                    i,
                    (char)expectedToLower,
                    destinationChar[0],
                    written
                );
                destinationChar[0] = default;
                VerifySingleChar(
                    Ascii.ToUpper(sourceByte, destinationChar, out written),
                    i,
                    (char)expectedToUpper,
                    destinationChar[0],
                    written
                );
            }
        ...
        {
            char[] sourceChar = new char[1],
                destinationChar = new char[1]; // this test is "optimized" as it performs a LOT of iterations
            byte[] destinationByte = new byte[1];

            for (int i = 0; i <= char.MaxValue; i++)
            {
                char expectedLower = char.IsBetween((char)i, 'A', 'Z')
                    ? (char)(i - 'A' + 'a')
                    : (char)i;
                char expectedUpper = char.IsBetween((char)i, 'a', 'z')
                    ? (char)(i + 'A' - 'a')
                    : (char)i;
                ...
                destinationChar[0] = default;
                VerifySingleChar(
                    Ascii.ToLower(sourceChar, destinationChar, out int written),
                    i,
                    expectedLower,
                    destinationChar[0],
                    written
                ...
                destinationChar[0] = default;
                VerifySingleChar(
                    Ascii.ToUpper(sourceChar, destinationChar, out written),
                    i,
                    expectedUpper,
                    destinationChar[0],
                    written
                ...
                destinationByte[0] = default;
                VerifySingleChar(
                    Ascii.ToLower(sourceChar, destinationByte, out written),
                    i,
                    (byte)expectedLower,
                    destinationByte[0],
                    written
                ...
                destinationByte[0] = default;
                VerifySingleChar(
                    Ascii.ToUpper(sourceChar, destinationByte, out written),
                    i,
                    (byte)expectedUpper,
                    destinationByte[0],
                    written
        ...
        public static IEnumerable<object[]> MultipleValidCharacterConversion_Arguments
        {
            get
            {
                ...
                for (int i = 1; i <= MaxValidAsciiChar; i++)
                {
                    ...
                    {
                        new string((char)i, i),
                        new string(expectedLower, i),
                        new string(expectedUpper, i),
                    };
                }
            }
        }
        ...
        public static void MultipleValidCharacterConversion(
            string sourceChars,
            string expectedLowerChars,
            string expectedUpperChars
        )
        {
            ...
            // char -> char
            VerifyStatus<char>(
                Ascii.ToLower(sourceChars, destinationChars, out int written),
                expectedLowerChars,
                destinationChars,
                written
            );
            VerifyStatus<char>(
                Ascii.ToUpper(sourceChars, destinationChars, out written),
                expectedUpperChars,
                destinationChars,
                written
            );
            // char -> byte
            VerifyStatus<byte>(
                Ascii.ToLower(sourceChars, destinationBytes, out written),
                expectedLowerBytes,
                destinationBytes,
                written
            );
            VerifyStatus<byte>(
                Ascii.ToUpper(sourceChars, destinationBytes, out written),
                expectedUpperBytes,
                destinationBytes,
                written
            );
            // byte -> byte
            VerifyStatus<byte>(
                Ascii.ToLower(sourceBytes, destinationBytes, out written),
                expectedLowerBytes,
                destinationBytes,
                written
            );
            VerifyStatus<byte>(
                Ascii.ToUpper(sourceBytes, destinationBytes, out written),
                expectedUpperBytes,
                destinationBytes,
                written
            );
            // byte -> char
            VerifyStatus<char>(
                Ascii.ToLower(sourceBytes, destinationChars, out written),
                expectedLowerChars,
                destinationChars,
                written
            );
            VerifyStatus<char>(
                Ascii.ToUpper(sourceBytes, destinationChars, out written),
                expectedUpperChars,
                destinationChars,
                written
            );
            ...
            VerifyStatus<byte>(
                Ascii.ToUpperInPlace(sourceBytesCopy, out processed),
                expectedUpperBytes,
                sourceBytesCopy,
                processed
            );
            ...
            VerifyStatus<char>(
                Ascii.ToLowerInPlace(sourceCharsCopy, out processed),
                expectedLowerChars.ToCharArray(),
                sourceCharsCopy,
                processed
            );
            sourceCharsCopy = sourceChars.ToCharArray();
            VerifyStatus<char>(
                Ascii.ToUpperInPlace(sourceCharsCopy, out processed),
                expectedUpperChars.ToCharArray(),
                sourceCharsCopy,
                processed
            );

            static void VerifyStatus<T>(
                OperationStatus status,
        ...
        public static void DestinationTooSmall(
            string sourceChars,
            int destinationSize,
            string expectedLowerChars,
            string expectedUpperChars
