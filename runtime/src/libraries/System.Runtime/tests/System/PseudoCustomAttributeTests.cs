        ...
        public static void GetCustomAttributes(
            ...
            switch (target)
            {
                case Type type:
                    Assert.Equal(
                        attribute,
                        Attribute.GetCustomAttributes(type, typeof(Attribute))
                            .Where((e) => e.GetType() == attributeType)
                            .SingleOrDefault()
                    );
                    Assert.Equal(
                        attribute,
                        Attribute.GetCustomAttributes(type, typeof(Attribute), true)
                            .Where((e) => e.GetType() == attributeType)
                            .SingleOrDefault()
                    );
                    break;
                case MemberInfo memberInfo:
                    Assert.Equal(
                        attribute,
                        Attribute.GetCustomAttributes(memberInfo, typeof(Attribute))
                            .Where((e) => e.GetType() == attributeType)
                            .SingleOrDefault()
                    );
                    Assert.Equal(
                        attribute,
                        Attribute.GetCustomAttributes(memberInfo, typeof(Attribute), true)
                            .Where((e) => e.GetType() == attributeType)
                            .SingleOrDefault()
                    );
                    break;
                case ParameterInfo parameterInfo:
                    Assert.Equal(
                        attribute,
                        Attribute.GetCustomAttributes(parameterInfo, typeof(Attribute))
                            .Where((e) => e.GetType() == attributeType)
                            .SingleOrDefault()
                    );
                    Assert.Equal(
                        attribute,
                        Attribute.GetCustomAttributes(parameterInfo, typeof(Attribute), true)
                            .Where((e) => e.GetType() == attributeType)
                            .SingleOrDefault()
                    );
                    break;
