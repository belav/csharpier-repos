...
namespace System.Text.Unicode.Tests
{
    public class Utf8Tests
    {
        ...
        private static readonly IEnumerable<Rune> s_allValidScalars = Enumerable.Range(
            0x0000,
            0xD800
        )
        ...
        {
            Assert.True(
                Regex.IsMatch(inputHex.ToString(), "^([0-9a-fA-F]{2})*$"),
                "Input must be an even number of hex characters."
            );
