    ...
    public class SingleTests_GenericMath
    {
        ...
        )
        {
            ...
            if (isDefaultProvider)
            {
                ...
                Assert.Equal(
                    expected,
                    NumberHelper<float>.Parse(value, style, NumberFormatInfo.CurrentInfo)
                );
            }
        }
        ...
        )
        {
            ...
            )
            {
                ...
                {
                    Assert.True(
                        NumberHelper<float>.TryParse(value.AsSpan(offset, count), null, out result)
                    );
                    Assert.Equal(expected, result);

                    Assert.Equal(
                        expected,
                        NumberHelper<float>.Parse(value.AsSpan(offset, count), null)
                    );
                }

                Assert.Equal(
                    expected,
                    NumberHelper<float>.Parse(value.AsSpan(offset, count), provider: provider)
                );
            }

            Assert.Equal(
                expected,
                NumberHelper<float>.Parse(value.AsSpan(offset, count), style, provider)
            );
        ...
        )
        {
            if (value != null)
            {
                Assert.Throws(
                    exceptionType,
                    () => NumberHelper<float>.Parse(value.AsSpan(), style, provider)
                );

                Assert.False(
                    NumberHelper<float>.TryParse(value.AsSpan(), style, provider, out float result)
                );
