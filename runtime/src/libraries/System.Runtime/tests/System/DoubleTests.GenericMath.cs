    ...
    public class DoubleTests_GenericMath
    {
        ...
        )
        {
            ...
            if (isDefaultProvider)
            {
                ...
                Assert.Equal(
                    expected,
                    NumberHelper<double>.Parse(value, style, NumberFormatInfo.CurrentInfo)
                );
            }
        }
        ...
        )
        {
            ...
            )
            {
                ...
                {
                    Assert.True(
                        NumberHelper<double>.TryParse(value.AsSpan(offset, count), null, out result)
                    );
                    Assert.Equal(expected, result);

                    Assert.Equal(
                        expected,
                        NumberHelper<double>.Parse(value.AsSpan(offset, count), null)
                    );
                }

                Assert.Equal(
                    expected,
                    NumberHelper<double>.Parse(value.AsSpan(offset, count), provider: provider)
                );
            }

            Assert.Equal(
                expected,
                NumberHelper<double>.Parse(value.AsSpan(offset, count), style, provider)
            );
        ...
        )
        {
            if (value != null)
            {
                Assert.Throws(
                    exceptionType,
                    () => NumberHelper<double>.Parse(value.AsSpan(), style, provider)
                );
