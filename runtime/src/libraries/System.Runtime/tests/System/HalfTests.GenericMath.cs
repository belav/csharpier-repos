    ...
    public class HalfTests_GenericMath
    {
        ...
        )
        {
            ...
            if (isDefaultProvider)
            {
                ...
                Assert.Equal(
                    expected,
                    NumberHelper<Half>.Parse(value, style, NumberFormatInfo.CurrentInfo)
                );
            }
        }
        ...
        )
        {
            ...
            )
            {
                ...
                {
                    Assert.True(
                        NumberHelper<Half>.TryParse(value.AsSpan(offset, count), null, out result)
                    );
                    Assert.Equal(expected, result);

                    Assert.Equal(
                        expected,
                        NumberHelper<Half>.Parse(value.AsSpan(offset, count), null)
                    );
                }

                Assert.Equal(
                    expected,
                    NumberHelper<Half>.Parse(value.AsSpan(offset, count), provider: provider)
                );
            }

            Assert.Equal(
                expected,
                NumberHelper<Half>.Parse(value.AsSpan(offset, count), style, provider)
            );
        ...
        )
        {
            if (value != null)
            {
                Assert.Throws(
                    exceptionType,
                    () => NumberHelper<Half>.Parse(value.AsSpan(), style, provider)
                );

                Assert.False(
                    NumberHelper<Half>.TryParse(value.AsSpan(), style, provider, out Half result)
                );
