...
namespace System.Resources.ResourceWriterTests
{
    public class ResourceReaderTests
    {
            ...
            using (var ms2 = new MemoryStream())
            {
                using (var rw = GenerateResourceStream(s_dict, ms2))
                {
                    ...
                    while (enume.MoveNext())
                    {
                        ...
                        i++;
                        Assert.True(
                            string.Compare(key, found_key) == 0,
                            "expected: " + key + ", but got : " + found_key
                        );
                        Assert.True(
                            string.Compare(value, found_value) == 0,
                            "expected: " + value + ", but got : " + found_value
                        );
                    }
        ...
        public static void ReadV1Resources()
        {
            ...
            {
                {
                    "Arg_InvalidMemberInfo",
                    "The specified member info is not valid for this IReflect."
                },
                {
                    "InvalidOp_GetIdDiffFromMemId",
                ...
                {
                    "NotSupported_AddingMethsNotSupported",
                    "IDispatchEx does not support adding methods."
                },
                {
                    "NotSupported_AddingFieldsNotSupported",
                    "IDispatchEx does not support adding fields."
                },
                { "MissingMember", "Member not found." },
                {
                    "InvalidCast_QIForEnumVarFailed",
                    "QI for IEnumVARIANT failed on the unmanaged server."
                },
                {
                    "InvalidOp_InvalidNewEnumVariant",
                    "Variant returned from Invoke() on member DISPID_NEWENUM is invalid."
                },
                {
                    "Arg_ObjectNotValidForMethodInfo",
                    "The object is not valid for this method info."
                },
                {
                    "Arg_NoAccessSpec",
