...
namespace System.Threading.Tasks.Tests
{
    ...
    public static class TaskContinueWithTests
    {
        ...
        public static void RunContinueWithAsyncStateCheckTests()
        {
            ...
            Assert.True(
                c1.AsyncState == null,
                "RunContinueWithAsyncStateCheckTests: task=>task continuation leaks state"
            );
            Assert.True(
                c2.AsyncState == null,
                "RunContinueWithAsyncStateCheckTests: task=>future continuation leaks state"
            );
            Assert.True(
                c3.AsyncState == null,
                "RunContinueWithAsyncStateCheckTests: future=>task continuation leaks state"
            );
            Assert.True(
                c4.AsyncState == null,
                "RunContinueWithAsyncStateCheckTests: future=>future continuation leaks state"
            );
        }
        ...
        {
            Action<Task, bool, string> EnsureCompletionStatus = delegate(
                Task task,
            ...
            EnsureCompletionStatus(
                c2,
                true,
                "RunContinueWithPreCancelTests: c2 should have completed (canceled) upon construction"
            );
            EnsureCompletionStatus(
                c4,
                false,
                "RunContinueWithPreCancelTests: c4 should NOT have completed (canceled) upon construction"
            );
            EnsureCompletionStatus(
                t1,
                false,
                "RunContinueWithPreCancelTests: t1 should NOT have completed before being started"
            );
            EnsureCompletionStatus(
                c1,
                false,
                "RunContinueWithPreCancelTests: c1 should NOT have completed before antecedent completed"
            );
            EnsureCompletionStatus(
                c3,
                false,
                "RunContinueWithPreCancelTests: c3 should NOT have completed before mres was set"
            );
            EnsureCompletionStatus(
                c5,
                false,
                "RunContinueWithPreCancelTests: c5 should NOT have completed before mres was set"
            );
            ...
            EnsureTaskCanceledExceptionThrown(
                ...
                "RunContinueWithPreCancelTests: Expected c2.Wait to throw AE/TCE"
            );
            ...
            EnsureTaskCanceledExceptionThrown(
                ...
                "RunContinueWithPreCancelTests: Expected c4.Wait to throw AE/TCE"
            );
            c5.Wait();
            ...
            EnsureCompletionStatus(
                t1,
                false,
                "RunContinueWithPreCancelTests: t1 should NOT have completed (post-mres.Set()) before being started"
            );
            EnsureCompletionStatus(
                c1,
                false,
                "RunContinueWithPreCancelTests: c1 should NOT have completed (post-mres.Set()) before antecedent completed"
            );
        ...
        public static void RunContinuationChainingTest()
        {
            ...
            {
                Assert.Fail(
                    string.Format("RunContinuationChainingTest: > FAILED! Exception = {0}", e)
                );
            }
        }
        ...
        public static void RunContinuationCancelTest(bool useTimeSpan)
        {
            ...
            {
                Assert.Fail(
                    string.Format("RunContinuationCancelTest: > Failed!  t1 should have run.")
                );
            }
            ...
            {
                Assert.Fail(
                    string.Format("RunContinuationCancelTest: > Failed!  t3 should have run.")
                );
            }
        }
                        ...
                        for (int z = 0; z < 2; z++)
                        {
                            ...
                            for (int x = 0; x < 2; x++)
                            {
                                ...
                                //
                                {
                                    ...
                                    Assert.True(
                                        continuation.CreationOptions == (TaskCreationOptions)tco,
                                        "RunContinueWithAllParamsTestsNoState: mis-matched CreationOptions"
                                    );
                                    Assert.True(
                                        (result == 5) || (!continuationIsFuture || preCanceled),
        ...
        public static void RunUnwrapTests()
        {
            ...
            ManualResetEvent mres = new ManualResetEvent(false);
            Action<Task, bool, string> checkCompletionState = delegate(
                Task ctask,
                bool shouldBeCompleted,
                string scenario
            )
            ...
            c3 = v3.Unwrap();
            c4 = Task
                .Factory.ContinueWhenAll(new Task[] { taskRoot, futureRoot }, completedTasks =>
                {
                    int sum = 0;
            ...
            c7 = v7.Unwrap();
            c8 = Task
                .Factory.ContinueWhenAny(new Task[] { taskRoot, futureRoot }, winner =>
                {
                    return Task.Factory.StartNew(
                        delegate
                        {
        ...
        public static void RunLazyCancellationTests()
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                TaskContinuationOptions options = useLazyCancellation
                    ? TaskContinuationOptions.LazyCancellation
                    : TaskContinuationOptions.None;
                ...
                for (int j = 0; j < 3; j++)
                {
                    ...
                    string type =
                        useContinueWith ? "ContinueWith"
                        : useContinueWhenAny ? "ContinueWhenAny"
                        : "ContinueWhenAll";
                    Debug.WriteLine(
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                for (int j = 0; j < 3; j++)
                {
...

                    {
                        ...
                        Assert.True(
                            c1.IsCompleted != useLazyCancellation,
                            "RunLazyCancellationTests: Before t1.Start(), c1.IsCompleted = "
                                + c1.IsCompleted
                        );
        ...
        private static void ContinueWithTortureTest(
            int numCanceled,
            int numLeftover,
            int completeAfter,
            int cancelAfter
        )
        {
            ...
            Action<Task> normalAction = (task) =>
            {
                Interlocked.Increment(ref normalCount);
            };
            Action<Task> cancelAction = (task) =>
            {
            ...
            Assert.True(
                normalCount == numLeftover,
                "ContinueWithTortureTest: > FAILED! normalCount mismatch (exp "
                    + numLeftover
                    + " got "
                    + normalCount
                    + ")"
            ...
            Assert.True(
                canceledCount == numCanceled,
                "ContinueWithTortureTest: > FAILED! canceledCount mismatch (exp + "
                    + numCanceled
                    + " got "
                    + canceledCount
                    + ")"
        ...
        private static Task CreateChooseBranch<T>(
