...
namespace System.Threading.Tasks.Tests
{
    public static class TaskRtTests
    {
        ...
        public static void RunRunTests()
        {
            ...
            Assert.True(
                task1.Status == TaskStatus.RanToCompletion,
            ...
            Assert.True(
                future1.Status == TaskStatus.RanToCompletion,
            ...
            Assert.True(
                future1.Result == 17,
                "    > FAILED.  Future(unwrapped) completed but did not run."
            );
            Assert.True(
                future1.Status == TaskStatus.RanToCompletion,
                "    > FAILED.  Future(unwrapped) did not end in RanToCompletion state."
            );
            ...
            Assert.True(
                count == 21,
                "    > FAILED.  Task w/ uncanceled token completed but did not run."
            );
            Assert.True(
                task2.Status == TaskStatus.RanToCompletion,
                "    > FAILED.  Task w/ uncanceled token did not end in RanToCompletion state."
            );
            ...
            Assert.True(
                future2.Result == 27,
                "    > FAILED.  Future w/ uncanceled token completed but did not run."
            );
            Assert.True(
                future2.Status == TaskStatus.RanToCompletion,
                "    > FAILED.  Future w/ uncanceled token did not end in RanToCompletion state."
            );
            ...
            Assert.True(
                count == 31,
                "    > FAILED.  Task(unwrapped) w/ uncanceled token completed but did not run."
            );
            Assert.True(
                task2.Status == TaskStatus.RanToCompletion,
                "    > FAILED.  Task(unwrapped) w/ uncanceled token did not end in RanToCompletion state."
            );
            ...
            Assert.True(
                future2.Result == 37,
                "    > FAILED.  Future(unwrapped) w/ uncanceled token completed but did not run."
            );
            Assert.True(
                future2.Status == TaskStatus.RanToCompletion,
                "    > FAILED.  Future(unwrapped) w/ uncanceled token did not end in RanToCompletion state."
            );
        }
        ...
        public static void RunRunTests_Cancellation_Negative()
        {
            ...
            Assert.True(
                task3.IsCanceled,
                "    > FAILED.  Task w/ canceled token should have ended in Canceled state"
            );

            Task future3 = Task.Run(
                () =>
                {
                    count = 47;
                    return count;
            ...
            Assert.True(
                future3.IsCanceled,
                "    > FAILED.  Future w/ canceled token should have ended in Canceled state"
            );
            ...
            Assert.True(
                task3.IsCanceled,
                "    > FAILED.  Task(unwrapped) w/ canceled token should have ended in Canceled state"
            );
            ...
            Assert.True(
                future3.IsCanceled,
                "    > FAILED.  Future(unwrapped) w/ canceled token should have ended in Canceled state"
            );
        }
        ...
        public static void RunRunTests_FastPathTests()
        {
            ...
            //
            {
                ...
                fastPath1.Wait();
                Assert.True(
                    fastPath1.Status == TaskStatus.RanToCompletion,
            ...
            }
            {
                ...
                Assert.True(
                    fastPath1.Status == TaskStatus.RanToCompletion,
                    "RunRunTests: Expected proxy for already-ran-to-completion future to be in RanToCompletion status"
                );
        ...
        public static void RunRunTests_Unwrap_NegativeCases()
        {
            ...
            //
            Action<UnwrappedScenario> TestUnwrapped = delegate(UnwrappedScenario scenario)
            {
                ...
                bool exceptionExpected =
                    (scenario == UnwrappedScenario.ThrowExceptionInDelegate)
                ...
                try
                {
                    t1.Wait();
                    Assert.False(
                        cancellationExpected || exceptionExpected,
                        "TaskRtTests.RunRunTests: Expected exception or cancellation"
                    );
                    Assert.True(
                        something == 1,
                        "TaskRtTests.RunRunTests: Task completed but apparently did not run"
                    );
                }
                catch (AggregateException ae)
                {
                    Assert.True(
                        cancellationExpected || exceptionExpected,
                        "TaskRtTests.RunRunTests: Didn't expect exception, got " + ae
                    );
                }
                ...
                if (cancellationExpected)
                {
                    Assert.True(
                        t1.IsCanceled,
                        "TaskRtTests.RunRunTests: Expected t1 to be Canceled, was " + t1.Status
                    );
                }
                else if (exceptionExpected)
                {
                    Assert.True(
                        t1.IsFaulted,
                        "TaskRtTests.RunRunTests: Expected t1 to be Faulted, was " + t1.Status
                    );
                }
                else
                {
                    Assert.True(
                        ...
                        "TaskRtTests.RunRunTests: Expected t1 to be RanToCompletion, was "
                            + t1.Status
                    );
                }

                Debug.WriteLine(
                    "RunRunTests: -- testing Task<int> unwrap (scenario={0})",
                    scenario
                );
                ...
                try
                {
                    ...
                    Assert.False(
                        cancellationExpected || exceptionExpected,
                        "RunRunTests: Expected exception or cancellation"
                    );
                    Assert.True(
                        f1.Result == 10,
                        "RunRunTests: Expected f1.Result to be 10, and it was " + f1.Result
                    );
                }
                catch (AggregateException ae)
                {
                    Assert.True(
                        cancellationExpected || exceptionExpected,
                        "RunRunTests: Didn't expect exception, got " + ae
                    );
                }
                ...
                if (cancellationExpected)
                {
                    Assert.True(
                        f1.IsCanceled,
                        "RunRunTests: Expected f1 to be Canceled, was " + f1.Status
                    );
                }
                else if (exceptionExpected)
                {
                    Assert.True(
                        f1.IsFaulted,
                        "RunRunTests: Expected f1 to be Faulted, was " + f1.Status
                    );
                }
                else
                {
                    Assert.True(
                        ...
                        "RunRunTests: Expected f1 to be RanToCompletion, was " + f1.Status
                    );
                }
            };
        ...
        public static void RunFromResult()
        {
            // Test FromResult with value type
            {
                ...
                for (int i = 0; i < tasks.Length; i++)
                    Assert.True(
                        tasks[i].IsCompleted,
                        "TaskRtTests.RunFromResult:    > FAILED: Task "
                            + i
                            + " should have already completed (value)"
                    );

                // Make sure they all completed successfully
                for (int i = 0; i < tasks.Length; i++)
                    Assert.True(
                        tasks[i].Status == TaskStatus.RanToCompletion,
                        "TaskRtTests.RunFromResult:    > FAILED: Task "
                            + i
                            + " should have already completed successfully (value)"
                    );
                ...
                for (int i = 0; i < tasks.Length; i++)
                {
                    for (int j = i + 1; j < tasks.Length; j++)
                    {
                        Assert.False(
                            tasks[i] == tasks[j],
                            "TaskRtTests.RunFromResult:    > FAILED: "
                                + i
                                + " and "
                                + j
                                + " created tasks should not be equal (value)"
                        );
                    }
                }
            ...
            // Test FromResult with reference type
            {
                ...
                for (int i = 0; i < tasks.Length; i++)
                    Assert.True(
                        ...
                        "TaskRtTests.RunFromResult:    > FAILED: Task "
                            + i
                            + " should have already completed  (ref)"
                    );
                ...
                for (int i = 0; i < tasks.Length; i++)
                    Assert.True(
                        ...
                        "TaskRtTests.RunFromResult:    > FAILED: Task "
                            + i
                            + " should have already completed successfully (ref)"
                    );
                    ...
                    for (int j = i + 1; j < tasks.Length; j++)
                    {
                        Assert.False(
                            ...
                            "TaskRtTests.RunFromResult:    > FAILED: "
                                ...
                                + " and "
                                + j
                                + " created tasks should not be equal (ref)"
                        );
                    }
                }
            ...
            // Test FromException
            {
                var exceptions = new Exception[]
                {
                    ...
                    new Exception(),
                    new Exception(),
                }; // includes duplicate values to ensure that tasks from these aren't the same object
                ...
                for (int i = 0; i < tasks.Length; i++)
                    Assert.True(
                        tasks[i].IsCompleted,
                        "Task " + i + " should have already completed"
                    );
                ...
                for (int i = 0; i < tasks.Length; i++)
                    Assert.True(
                        ...
                        "    > FAILED: Task " + i + " should have already faulted"
                    );
                    ...
                    for (int j = i + 1; j < tasks.Length; j++)
                    {
                        Assert.True(
                            ...
                            "    > FAILED: "
                                ...
                                + " and "
                                + j
                                + " created tasks should not be equal"
                        );
                    }
                }
        ...
        public static void RunFromResult_FaultedTask()
        {
            ...
            if (contingentPropertiesField != null)
            {
                ...
                {
                    var exceptionsHolderField = typeof(Task)
                        .GetNestedType("ContingentProperties", BindingFlags.NonPublic)
                        .GetField(
                            "m_exceptionsHolder",
            ...
            Assert.False(
                (bool)isHandledField.GetValue(holderObject),
                "Expected FromException task to be unobserved before accessing Exception"
            );
            var ignored = faultedTask.Exception;
            Assert.True(
                (bool)isHandledField.GetValue(holderObject),
                "Expected FromException task to be observed after accessing Exception"
            );
        }
        ...
        public static void RunDelayTests()
        {
            ...
            Debug.WriteLine(
                "RunDelayTests:    > Waiting for 0-delayed uncanceled tasks to complete.  If we hang, something went wrong."
            );
            try
            {
            ...
            Assert.True(
                task1.Status == TaskStatus.RanToCompletion,
                "    > FAILED.  Expected Delay(0) to run to completion"
            );
            Assert.True(
                task2.Status == TaskStatus.RanToCompletion,
                "    > FAILED.  Expected Delay(TimeSpan(0)) to run to completion"
            );
            Assert.True(
                task3.Status == TaskStatus.RanToCompletion,
                "    > FAILED.  Expected Delay(0,uncanceledToken) to run to completion"
            );
            Assert.True(
                task4.Status == TaskStatus.RanToCompletion,
                "    > FAILED.  Expected Delay(TimeSpan(0),uncanceledToken) to run to completion"
            );
            ...
            Assert.False(
                task7.IsCompleted,
                "RunDelayTests:    > FAILED.  Delay(10000) appears to have completed too soon(1)."
            );
            Task t2 = Task.Delay(10);
            Assert.False(
                task7.IsCompleted,
                "RunDelayTests:    > FAILED.  Delay(10000) appears to have completed too soon(2)."
            );
        }
        ...
        public static void RunDelayTests_NegativeCases()
        {
            ...
            Debug.WriteLine(
                "RunDelayTests:    > Waiting for 0-delayed canceled tasks to complete.  If we hang, something went wrong."
            );
            try
            {
            ...
            Assert.True(
                ...
                "RunDelayTests:    > FAILED.  Expected Delay(0,canceledToken) to end up Canceled"
            );
            Assert.True(
                task6.Status == TaskStatus.Canceled,
                "RunDelayTests:    > FAILED.  Expected Delay(TimeSpan(0),canceledToken) to end up Canceled"
            );
            ...
            Assert.True(
                task8.IsCanceled,
                "RunDelayTests:    > FAILED.  Expected Delay(-1, token) to end up Canceled."
            );
            Assert.True(
                task9.IsCanceled,
                "RunDelayTests:    > FAILED.  Expected Delay(TimeSpan(1,0,0,0), token) to end up Canceled."
            );
            ...
            catch (AggregateException ae)
            {
                Assert.True(
                    ae.InnerException is OperationCanceledException
                        && ((OperationCanceledException)ae.InnerException).CancellationToken
                            == cts2.Token,
        ...
        {
            Action throwException = delegate
            {
        ...
        public static void RunHideSchedulerTests()
        {
            ...
            for (int i = 0; i < schedules.Length; i++)
            {
                ...
                for (int j = 0; j < 2; j++)
                {
                    ...
                    TaskCreationOptions creationOptions = hideScheduler
                        ? TaskCreationOptions.HideScheduler
                        : TaskCreationOptions.None;
                    TaskContinuationOptions continuationOptions = hideScheduler
                    ...
                    Assert.True(
                        hideScheduler
                            == (
                                (outerTask.CreationOptions & TaskCreationOptions.HideScheduler) != 0
                            ),
                        "RunHideSchedulerTests:  FAILED.  CreationOptions mismatch on outerTask"
                    );
                    Assert.True(
                        hideScheduler
                            == (
                                (
                                    outerContinuation.CreationOptions
                                    & TaskCreationOptions.HideScheduler
                                ) != 0
                            ),
                        "RunHideSchedulerTests:  FAILED.  CreationOptions mismatch on outerContinuation"
                    );
        ...
        private class FakeAsyncClass
        {
            ...
            public IAsyncResult StartWrite(
                string s,
                int offset,
                int length,
                AsyncCallback cb,
