...
namespace System.IO.Tests
{
    public class StreamReader_ctorTests
    {
        ...
        public static IEnumerable<object[]> EncodingsWithEncodedDataWithPreamble()
        {
            ...
            foreach (var encoding in encodingInputs)
            {
                foreach (var stringInput in stringInputs)
                {
                    ...
                    {
                        encoding,
                        GetEncodedDataWithPreamble(encoding.GetBytes(stringInput), encoding),
                        stringInput,
                    };
                }
            }
        ...
        public static void CreationFromMemoryStreamWithEncodingTrueDetectsCorrectEncodingWhenPreambleAvailable(
        ...
        public static IEnumerable<object[]> EncodingsWithSlightlyOffPatterns()
        {
            ...
            {
                Encoding.BigEndianUnicode,
                new byte[] { 0xFE, 0xFF, 0x00, 0x00 },
            }; // Possible UTF-32LE, last 2 bytes are the same
            yield return new object[]
            {
                Encoding.BigEndianUnicode,
                new byte[] { 0xFE, 0xFF, 0x00, 0x00, 0xFE, 0xFF },
            }; // Possible UTF-32BE, last 4 bytes are the preamble of UTF-32BE
            yield return new object[]
            {
                Encoding.BigEndianUnicode,
                new byte[] { 0xFE, 0xFF, 0xFE, 0xFF },
            }; // Possible UTF-32BE, last 2 bytes are the same
            yield return new object[]
            {
                Encoding.UTF32,
                new byte[] { 0xFF, 0xFE, 0x00, 0x00, 0xFE, 0xFF },
            }; // Possible UTF-32BE, last 4 bytes are the preamble of UTF-32BE
            yield return new object[]
            {
                Encoding.UTF32,
                new byte[] { 0xFF, 0xFE, 0x00, 0x00, 0xEF, 0xBB, 0xBF },
            }; // Possible UTF-8, last 3 bytes are the preamble of UTF-8
            yield return new object[]
            {
                new UTF32Encoding(bigEndian: true, byteOrderMark: true),
                new byte[] { 0x00, 0x00, 0xFE, 0xFF },
            }; // Possible UnicodeBE, last 2 bytes are UnicodeBE
        }
