...
namespace System.Tests
{
    public static class OperatingSystemTests
    {
        ...
        public static void TestIsOSPlatform_Browser() =>
            TestIsOSPlatform("BROWSER", OperatingSystem.IsBrowser);

        [Fact, PlatformSpecific(TestPlatforms.Browser)]
        public static void TestIsOSVersionAtLeast_Browser() => TestIsOSVersionAtLeast("BROWSER");

        [Fact, PlatformSpecific(TestPlatforms.Wasi)]
        public static void TestIsOSPlatform_Wasi() =>
            TestIsOSPlatform("WASI", OperatingSystem.IsWasi);

        [Fact, PlatformSpecific(TestPlatforms.Wasi)]
        public static void TestIsOSVersionAtLeast_Wasi() => TestIsOSVersionAtLeast("WASI");

        [Fact, PlatformSpecific(TestPlatforms.Linux)]
        public static void TestIsOSPlatform_Linux() =>
            TestIsOSPlatform("Linux", OperatingSystem.IsLinux);

        [Fact, PlatformSpecific(TestPlatforms.Linux)]
        public static void TestIsOSVersionAtLeast_Linux() => TestIsOSVersionAtLeast("Linux");

        [Fact, PlatformSpecific(TestPlatforms.FreeBSD)]
        public static void TestIsOSPlatform_FreeBSD() =>
            TestIsOSPlatform("FreeBSD", OperatingSystem.IsFreeBSD);
        ...
        public static void TestIsOSPlatform_Android() =>
            TestIsOSPlatform("Android", OperatingSystem.IsAndroid);

        [Fact, PlatformSpecific(TestPlatforms.Android)]
        public static void TestIsOSVersionAtLeast_Android() => TestIsOSVersionAtLeast("Android");

        [Fact, PlatformSpecific(TestPlatforms.Android)]
        public static void TestIsOSVersionAtLeast_Android_21() =>
            Assert.True(OperatingSystem.IsAndroidVersionAtLeast(21)); // 21 is our min supported version
        ...
        public static void TestIsOSPlatform_MacOS() =>
            TestIsOSPlatform("macOS", OperatingSystem.IsMacOS);
        ...
        public static void TestIsOSPlatform_MacCatalyst() =>
            TestIsOSPlatform("MacCatalyst", OperatingSystem.IsMacCatalyst);

        [Fact, PlatformSpecific(TestPlatforms.MacCatalyst)]
        public static void TestIsOSVersionAtLeast_MacCatalyst() =>
            TestIsOSVersionAtLeast("MacCatalyst");
        ...
        public static void TestIsOSPlatform_TvOS() =>
            TestIsOSPlatform("tvOS", OperatingSystem.IsTvOS);

        [Fact, PlatformSpecific(TestPlatforms.tvOS)]
        public static void TestIsOSVersionAtLeast_TvOS() => TestIsOSVersionAtLeast("tvOS");

        [Fact, PlatformSpecific(TestPlatforms.Windows)]
        public static void TestIsOSPlatform_Windows() =>
            TestIsOSPlatform("Windows", OperatingSystem.IsWindows);
        ...
        private static void TestIsOSPlatform(string currentOSName, Func<bool> currentOSCheck)
        {
            ...
            {
                bool expected = currentOSName.Equals(
                    platformName,
        ...
        private static void TestIsOSVersionAtLeast(string currentOSName)
        {
            foreach (string platformName in AllKnownPlatformNames)
            {
                bool isCurrentOS = currentOSName.Equals(
                    platformName,
                    StringComparison.OrdinalIgnoreCase
                );
            ...
            AssertVersionChecks(
                currentOSName.Equals("Android", StringComparison.OrdinalIgnoreCase),
                OperatingSystem.IsAndroidVersionAtLeast
            );
            AssertVersionChecks(
                currentOSName == "MacCatalyst"
                    || currentOSName.Equals("iOS", StringComparison.OrdinalIgnoreCase),
                OperatingSystem.IsIOSVersionAtLeast
            );
            AssertVersionChecks(
                currentOSName.Equals("macOS", StringComparison.OrdinalIgnoreCase),
                OperatingSystem.IsMacOSVersionAtLeast
            );
            AssertVersionChecks(
                currentOSName.Equals("MacCatalyst", StringComparison.OrdinalIgnoreCase),
                OperatingSystem.IsMacCatalystVersionAtLeast
            );
            AssertVersionChecks(
                currentOSName.Equals("tvOS", StringComparison.OrdinalIgnoreCase),
                OperatingSystem.IsTvOSVersionAtLeast
            );
            AssertVersionChecks(
                currentOSName.Equals("watchOS", StringComparison.OrdinalIgnoreCase),
                OperatingSystem.IsWatchOSVersionAtLeast
            );
            AssertVersionChecks(
                currentOSName.Equals("Windows", StringComparison.OrdinalIgnoreCase),
        ...
        )
        {
            ...
            Assert.False(
                isOSVersionAtLeast(
                    ...
                    current.Minor,
                    current.Build,
                    Math.Max(current.Revision + 1, 1)
                )
            ); // OSX Revision reports -1

            Assert.Equal(
                isCurrentOS,
                isOSVersionAtLeast(current.Major, current.Minor, current.Build, current.Revision)
            );
        ...
        )
        {
            ...
            Assert.Equal(
                isCurrentOS,
                isOSVersionAtLeast(current.Major, current.Minor, current.Build)
            );

            Assert.Equal(
                isCurrentOS,
                isOSVersionAtLeast(current.Major - 1, current.Minor, current.Build)
            );
            Assert.Equal(
                isCurrentOS,
                isOSVersionAtLeast(current.Major, current.Minor - 1, current.Build)
            );
            Assert.Equal(
                isCurrentOS,
                isOSVersionAtLeast(current.Major, current.Minor, current.Build - 1)
            );
        }
    }
}
