...
namespace System.Globalization.Tests
{
    public class CompareInfoTests : CompareInfoTestsBase
    {
        ...
        public static IEnumerable<object[]> Equals_TestData()
        {
            ...
            {
                new CultureInfo("en-US").CompareInfo,
                CompareInfo.GetCompareInfo("fr-FR"),
                false,
            };
        ...
        [ConditionalTheory(
            ...
            nameof(PlatformDetection.IsNotHybridGlobalization)
        )]
        ...
        )
        {
            ...
            Assert.Equal(
                expected,
                invariantCompare
                    .GetHashCode(source1, options1)
                    .Equals(invariantCompare.GetHashCode(source2, options2))
            );
        }
        ...
        public static IEnumerable<object[]> SortKey_TestData()
        {
            ...
            yield return new object[]
            {
                ...
                "\u30F2",
                CompareOptions.IgnoreSymbols,
                s_expectedHalfToFullFormsComparison,
            };
            yield return new object[]
            {
                s_invariantCompare,
            ...
            yield return new object[]
            {
                new CultureInfo("es-ES").CompareInfo,
                "llegar",
                "lugar",
                CompareOptions.None,
                -1,
        ...
        [ConditionalTheory(
            typeof(CompareInfoTests),
            nameof(IsNotWindowsKanaRegressedVersionAndNotHybridGlobalization)
        )]
        [MemberData(nameof(SortKey_Kana_TestData))]
        public void SortKeyKanaTest(
        ...
        [ConditionalFact(
            typeof(PlatformDetection),
            nameof(PlatformDetection.IsHybridGlobalization)
        )]
        public void SortKeyTestNotSupported()
        {
            ...
            string GetPNSEText(string funcName) =>
                $"{funcName} is not supported when HybridGlobalization=true. Disable it to load larger ICU bundle, then use this option.";
        ...
        )
        {
            ...
            {
                Assert.Equal(
                    Math.Sign(compareInfo.Compare(string1, string2, options)),
                    Math.Sign(SortKey.Compare(sk1, sk2))
                );
            }
            ...
            )
            {
                ...
                Assert.Equal(
                    expectedSortKey.Length,
                    compareInfo.GetSortKeyLength(sourceBoundedMemory.Span, options)
                );

                using BoundedMemory<byte> sortKeyBoundedMemory = BoundedMemory.Allocate<byte>(
                    expectedSortKey.Length
                );
                ...
                Assert.Equal(
                    expectedSortKey.Length,
                    compareInfo.GetSortKey(sourceBoundedMemory.Span, sortKeyBoundedSpan, options)
                );
                Assert.Equal(
                    expectedSortKey,
                    sortKeyBoundedSpan[0..expectedSortKey.Length].ToArray()
                );
            }
        }
        ...
        )
        {
            ...
            if (value.Length == 1)
            {
                Assert.Equal(
                    lastIndexOfExpected,
                    compareInfo.LastIndexOf(source, value[0], startIndex)
                );
            }
        }
        ...
        public void GetHashCode_NullAndEmptySpan()
        {
            ...
            int hashCodeOfNullSpan = CultureInfo.InvariantCulture.CompareInfo.GetHashCode(
                ReadOnlySpan<char>.Empty,
                CompareOptions.None
            );
            int hashCodeOfNotNullEmptySpan = CultureInfo.InvariantCulture.CompareInfo.GetHashCode(
                "".AsSpan(),
                CompareOptions.None
            );
