    ...
    public class CompareInfoIsPrefixTests : CompareInfoTestsBase
    {
        public static IEnumerable<object[]> IsPrefix_TestData()
        {
            ...
            bool behavesLikeNls =
                ...
                || (
                    PlatformDetection.IsHybridGlobalizationOnBrowser
                    && !PlatformDetection.IsBrowserDomSupportedOrNodeJS
                );
            if (behavesLikeNls)
            {
                if (
            ...
            if (PlatformDetection.IsNlsGlobalization || PlatformDetection.ICUVersion.Major >= 65)
            {
                yield return new object[]
                {
                    s_frenchCompare,
                    "b",
                    new string('a', UInt16.MaxValue + 1),
                    CompareOptions.None,
                    false,
                    0,
                };
        ...
        )
        {
            ...
            )
            {
                ...
                Assert.Equal(
                    expected,
                    source.AsSpan().StartsWith(value.AsSpan(), stringComparison)
                );
            }
            ...
            Assert.Equal(
                expected,
                compareInfo.IsPrefix(sourceBoundedMemory.Span, valueBoundedMemory.Span, options)
            );
            if (
                !PlatformDetection.IsHybridGlobalizationOnBrowser
                && !PlatformDetection.IsHybridGlobalizationOnOSX
