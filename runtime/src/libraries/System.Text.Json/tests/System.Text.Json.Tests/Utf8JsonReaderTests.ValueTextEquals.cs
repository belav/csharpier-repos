    ...
    public static partial class Utf8JsonReaderTests
    {
        ...
        public static void TestTextEqualsBasic()
        {
            ...
            var json = new Utf8JsonReader(
                "{\"conne\\u0063tionId\":\"123\",\"availableTransports\":[\"My name is \\\"Ahson\\\"\"]}"u8,
                isFinalBlock: true,
                state: default
            );
            while (json.Read())
            {
                ...
                {
                    if (
                        json.ValueTextEquals("connectionId"u8)
                        && json.ValueTextEquals("connectionId".AsSpan())
                    )
                    {
                        foundId = true;
                    }
                    else if (
                        json.ValueTextEquals("availableTransports"u8)
                        && json.ValueTextEquals("availableTransports".AsSpan())
                    )
                    {
                ...
                else if (json.TokenType == JsonTokenType.String)
                {
                    if (json.ValueTextEquals("123"u8) && json.ValueTextEquals("123".AsSpan()))
                    {
                        ...
                        json.ValueTextEquals("My name is \"Ahson\""u8)
                        && json.ValueTextEquals("My name is \"Ahson\"".AsSpan())
                    )
                    {
        ...
        public static void TestTextEquals(
            string jsonString,
            string lookUpString,
            bool expectedFound
        )
        ...
        [InlineData("{\"name\": \"Joh\\u006e\"}", "", false)]
        public static void TestTextEqualsValue(
            string jsonString,
            string lookUpString,
            bool expectedFound
        )
        ...
        public static void TestTextEqualsLargeMatch()
        {
            ...
            for (int i = 0; i < jsonChars.Length - escapedA.Length + 1; i++)
            {
                ...
                while (json.Read())
                {
                    ...
                    {
                        if (
                            json.ValueTextEquals(lookupSpan)
                ...
                Assert.True(
                    found,
                    $"Json String: {jsonString}  | Look up: {Encoding.UTF8.GetString(lookupSpan.ToArray())}"
                );
            }
        }
        ...
        public static void TestTextEqualsTooSmallToMatch(string jsonString)
        {
            ...
            while (json.Read())
            {
                ...
                {
                    if (
                        json.ValueTextEquals(new byte[] { (byte)'a' })
        ...
        public static void TestTextEqualsTooLargeToMatch(string jsonString)
        {
            ...
            while (json.Read())
            {
                ...
                {
                    if (
                        json.ValueTextEquals(Encoding.UTF8.GetBytes(lookupString))
                        || json.ValueTextEquals(lookupString.AsSpan())
                        || json.ValueTextEquals(lookupString)
                    )
                    {
        ...
        public static void TestTextEqualsMismatchMultiSegment()
        {
            ...
            while (json.Read())
            {
                ...
                {
                    if (
                        json.ValueTextEquals("Hello, \"Ahson\""u8)
