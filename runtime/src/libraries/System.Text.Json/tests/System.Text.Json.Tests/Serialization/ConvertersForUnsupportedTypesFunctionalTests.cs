...
namespace System.Text.Json.Serialization.Tests
{
    public static class ConvertersForUnsupportedTypesFunctionalTests
    {
        ...
        public static void RoundtripValues<T>(string expectedValueAsString, T value)
        {
            ...
            ClassWithProperty<T> deserializedWrappedValue = JsonSerializer.Deserialize<
                ClassWithProperty<T>
            >(json, options);
        ...
        public static IEnumerable<object[]> GetTestData()
        {
            ...
            yield return GetTestCase(
                ulong.MaxValue.ToString(nfi) + "173",
                new Int128(0UL, ulong.MaxValue) * 1000 + 173
            );

            yield return GetTestCase("0", UInt128.Zero);
            yield return GetTestCase(UInt128.MaxValue.ToString(nfi), UInt128.MaxValue);
            yield return GetTestCase(
                ulong.MaxValue.ToString(nfi),
                new UInt128(0UL, ulong.MaxValue)
            );
            yield return GetTestCase(
                ulong.MaxValue.ToString(nfi) + "173",
                new UInt128(0UL, ulong.MaxValue) * 1000 + 173
            );
            ...
            static object[] GetTestCase(string expectedValue, object value) =>
        ...
        internal class HalfConverter : SimpleConverter<Half>
        {
            public override Half Parse(string value) =>
                Half.Parse(value, CultureInfo.InvariantCulture.NumberFormat);

            public override string ToString(Half value) =>
        ...
        internal class UInt128Converter : SimpleConverter<UInt128>
        {
            public override UInt128 Parse(string value) =>
                UInt128.Parse(value, CultureInfo.InvariantCulture.NumberFormat);

            public override string ToString(UInt128 value) =>
                value.ToString(CultureInfo.InvariantCulture.NumberFormat);
        }
        ...
        internal class Int128Converter : SimpleConverter<Int128>
        {
            public override Int128 Parse(string value) =>
                Int128.Parse(value, CultureInfo.InvariantCulture.NumberFormat);

            public override string ToString(Int128 value) =>
                value.ToString(CultureInfo.InvariantCulture.NumberFormat);
        }

        internal class BigIntegerConverter : SimpleConverter<BigInteger>
        {
            public override BigInteger Parse(string value) =>
                BigInteger.Parse(value, CultureInfo.InvariantCulture.NumberFormat);

            public override string ToString(BigInteger value) =>
                value.ToString(CultureInfo.InvariantCulture.NumberFormat);
        }
