...
namespace System.Text.Json.Serialization.Tests
{
    public static class TypeInfoResolverFunctionalTests
    {
        ...
        public static void AddPrefixToEveryPropertyOfClass()
        {
            ...
            Assert.Equal(
                @"{""renamed_TestProperty"":42,""renamed_TestField"":""test value""}",
                json
            );
        ...
        public static void SetCustomConverterForListProperty()
        {
            ...
            TestClassWithLists deserialized = JsonSerializer.Deserialize<TestClassWithLists>(
                json,
                options
            );
        ...
        public static void SetCustomConverterForDictionaryProperty()
        {
                    ...
                    (ti) =>
                    {
                        if (ti.Type == typeof(TestClassWithDictionaries))
                        {
                            ...
                            foreach (var prop in ti.Properties)
                            {
                                if (
                                    prop.Name
                                    == nameof(TestClassWithDictionaries.DictionaryProperty1)
                                )
                                {
            ...
            string json = JsonSerializer.Serialize(originalObj, options);
            Assert.Equal(
                """{"DictionaryProperty1":{"test1":4,"test2":5,"*test*":-1},"DictionaryProperty2":{"foo":1,"bar":8}}""",
                json
            );

            TestClassWithDictionaries deserialized =
                JsonSerializer.Deserialize<TestClassWithDictionaries>(json, options);
        ...
        public static void CreateObjectForDictionaryWithDefaults(bool useTypedCreateObject)
        {
            ...
            resolver
                ...
                .Add(
                    (ti) =>
                    {
                        ...
                        {
                            Func<Dictionary<string, int>> createObj = () =>
                                new Dictionary<string, int> { ["*test*"] = -1 };

                            if (useTypedCreateObject)
                            {
                                JsonTypeInfo<Dictionary<string, int>> typedTi =
                                    ti as JsonTypeInfo<Dictionary<string, int>>;
                                Assert.NotNull(typedTi);
                                typedTi.CreateObject = createObj;
                            }
                            else
            ...
            string json = JsonSerializer.Serialize(originalObj, options);
            Assert.Equal(
                """{"DictionaryProperty1":{"test1":2,"test2":3},"DictionaryProperty2":{}}""",
                json
            );
            ...
            Assert.Null(deserialized.DictionaryProperty1);
            Assert.Equal(
                new Dictionary<string, int> { ["*test*"] = -1, ["foo"] = 123 },
                deserialized.DictionaryProperty2
            );
        }
        ...
        public static void CombineCustomResolverWithDefault()
        {
            TestResolver resolver = new TestResolver(
                (Type type, JsonSerializerOptions options) =>
                {
                    ...
                    JsonTypeInfo<TestClass> ti = JsonTypeInfo.CreateJsonTypeInfo<TestClass>(
                        options
                    );
                    ti.CreateObject = () =>
        ...
        public static void DataContractResolverScenario()
        {
            ...
            DataContractResolver.TestClass result =
                JsonSerializer.Deserialize<DataContractResolver.TestClass>(json, options);
        ...
        internal class SpecifiedContractResolver : DefaultJsonTypeInfoResolver
        {
            ...
            public override JsonTypeInfo GetTypeInfo(Type type, JsonSerializerOptions options)
            {
                ...
                {
                    PropertyInfo? specifiedProperty = type.GetProperty(
        ...
        public static void FieldContractResolverScenario()
        {
            ...
            FieldContractResolver.TestClass result =
                JsonSerializer.Deserialize<FieldContractResolver.TestClass>(json, options);
        ...
        internal class FieldContractResolver : DefaultJsonTypeInfoResolver
        {
            public class TestClass
            {
                ...
                public override bool Equals(object? other) =>
                    other is TestClass tc
                    && (_string, _int, _bool) == (tc._string, tc._int, tc._bool);
            }
