...
{
    partial public static class StreamTests
    {
        [Fact]
        ...
        private static async Task ReadAsync(TestStream stream)
        {
            ...
            LargeDataTestClass obj = await JsonSerializer.DeserializeAsync<LargeDataTestClass>(
                stream,
                options
            );
        ...
        public static async Task LargeJsonFile(int bufferSize)
        {
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                List<SessionResponse> deserializedList = await JsonSerializer.DeserializeAsync<
                    List<SessionResponse>
        ...
        public static async Task VeryLargeJsonFileTest(
            int payloadSize,
            bool ignoreNull,
            bool writeIndented
        )
        {
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                memoryStream.Position = 0;
                List<Order> deserializedList = await JsonSerializer.DeserializeAsync<List<Order>>(
                    memoryStream,
                    options
                );
                Assert.Equal(payloadSize, deserializedList.Count);
        ...
        public static async Task DeepNestedJsonFileTest(
            int depthFactor,
            bool ignoreNull,
            bool writeIndented
        )
