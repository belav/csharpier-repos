...
namespace System.Text.Json.Serialization.Tests
{
    public static partial class StreamTests_DeserializeAsyncEnumerable
    {
        ...
        )
        {
            ...
            int callbackCount = 0;
            await foreach (
                SimpleTestClass item in JsonSerializer.DeserializeAsyncEnumerable<SimpleTestClass>(
                    stream,
                    options
                )
            )
        ...
        )
        {
            ...
            List<TElement> results = await JsonSerializer
                .DeserializeAsyncEnumerable<TElement>(stream, options)
                .ToListAsync();
        ...
        public static void DeserializeAsyncEnumerable_NullStream_ThrowsArgumentNullException()
        {
            AssertExtensions.Throws<ArgumentNullException>(
                "utf8Json",
                () => JsonSerializer.DeserializeAsyncEnumerable<int>(utf8Json: null)
            );
        }
        ...
        public static async Task DeserializeAsyncEnumerable_EnumeratorWithCancellationToken_ThrowsOnCancellation()
        {
            ...
            var token = new CancellationToken(canceled: true);
            using var stream = new MemoryStream(data);
            var cancellableAsyncEnumerable = JsonSerializer
        ...
        public static IEnumerable<object[]> GetAsyncEnumerableSources()
        {
            ...
            yield return WrapArgs(
                Enumerable.Range(0, 100).Select(i => $"lorem ipsum dolor: {i}"),
                500
            );
            yield return WrapArgs(
                Enumerable
            ...
            static object[] WrapArgs<TSource>(IEnumerable<TSource> source, int bufferSize) =>
                new object[] { source, bufferSize };
        }
