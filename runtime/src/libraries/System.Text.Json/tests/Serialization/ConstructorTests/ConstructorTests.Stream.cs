...
namespace System.Text.Json.Serialization.Tests
{
    public abstract partial class ConstructorTests
    {
        ...
        public async Task ReadSimpleObjectAsync()
        {
            ...
            tasks[1] = Task.Run(
                async () =>
                    await RunTestAsync<ClassWithConstructor_SimpleAndComplexParameters>(
                        ClassWithConstructor_SimpleAndComplexParameters.s_data
                    )
            );
            tasks[2] = Task.Run(
                async () =>
            ...
            async Task RunTestAsync<T>()
            {
                ...
                using (MemoryStream stream = new MemoryStream(Encoding.UTF8.GetBytes(input)))
                {
                    ...
                    await Assert.ThrowsAsync<NotSupportedException>(
                        async () => await JsonSerializer.DeserializeAsync<T>(stream, options)
                    );
                }
        ...
        public async Task ExerciseStreamCodePaths()
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                using (MemoryStream stream = new MemoryStream(GeneratePayload(i, value)))
                {
                    ...
                    ClassWithStrings obj = await JsonSerializer.DeserializeAsync<ClassWithStrings>(
                        stream,
                        options
                    );
                    obj.Verify(value);
