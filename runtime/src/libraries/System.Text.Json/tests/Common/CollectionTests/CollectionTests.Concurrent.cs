...
namespace System.Text.Json.Serialization.Tests
{
    public abstract partial class CollectionTests
    {
        ...
        {
            ConcurrentDictionary<string, string> cd = await Serializer.DeserializeWrapper<
                ConcurrentDictionary<string, string>
            >(@"{""key"":""value""}");
            Assert.Equal(1, cd.Count);
            Assert.Equal("value", cd["key"]);

            ConcurrentQueue<string> qc = await Serializer.DeserializeWrapper<
            ...
            ConcurrentStack<string> qs = await Serializer.DeserializeWrapper<
        ...
        public async Task Read_ConcurrentCollection_NoPublicConstructor_Throws(
            Type type,
            string json
        )
        {
        ...
        public async Task Write_ConcurrentCollection()
        {
            Assert.Equal(
                @"[""1""]",
                await Serializer.SerializeWrapper(new BlockingCollection<string> { "1" })
            );

            Assert.Equal(
                @"[""1""]",
                await Serializer.SerializeWrapper(new ConcurrentBag<string> { "1" })
            );
