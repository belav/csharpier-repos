...
namespace System.Text.Json.Serialization.Tests
{
    public abstract partial class CollectionTests
    {
        ...
        public async Task Read_SpecializedCollection()
        {
            ...
            HybridDictionary hd = await Serializer.DeserializeWrapper<HybridDictionary>(
                @"{""key"":""value""}"
            );
            Assert.Equal(1, hd.Count);
            Assert.Equal("value", ((JsonElement)hd["key"]).GetString());

            IOrderedDictionary iod = await Serializer.DeserializeWrapper<OrderedDictionary>(
                @"{""key"":""value""}"
            );
            Assert.Equal(1, iod.Count);
            Assert.Equal("value", ((JsonElement)iod["key"]).GetString());

            ListDictionary ld = await Serializer.DeserializeWrapper<ListDictionary>(
                @"{""key"":""value""}"
            );
        ...
        public async Task Read_SpecializedCollection_Throws()
        {
            ...
            await Assert.ThrowsAsync<InvalidCastException>(async () =>
                await Serializer.DeserializeWrapper<StringCollection>(@"[""1"", ""2""]")
            );

            // Not supported. Not IList, and we don't detect the add method for this collection.
            await Assert.ThrowsAsync<NotSupportedException>(async () =>
                await Serializer.DeserializeWrapper<StringDictionary>(
                    @"[{""Key"": ""key"",""Value"":""value""}]"
                )
            );
            ...
            await Assert.ThrowsAsync<JsonException>(async () =>
                await Serializer.DeserializeWrapper<HybridDictionary>(@"{1:""value""}")
            );
            ...
            // Not supported. Not IList, and we don't detect the add method for this collection.
            await Assert.ThrowsAsync<NotSupportedException>(async () =>
                await Serializer.DeserializeWrapper<NameValueCollection>(
                    @"[""NameValueCollection""]"
                )
            );
        }
