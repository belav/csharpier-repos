...
namespace System.Text.Json.SourceGeneration.Tests
{
    public sealed partial class UnspeakableTypeTests : SerializerTests
    {
        ...
        public async Task CanSerializeUnspeakableRootTypes<T>(
            Envelope<T> envelope,
            string expectedJson,
            bool isBaseTypeDeserializable
        )
        {
        ...
        [Theory]
        [MemberData(nameof(GetUnspeakableTypes))]
        public async Task CanSerializeUnspeakableTypesAsBoxedProperties<T>(
            Envelope<T> envelope,
            string expectedJson,
            bool isBaseTypeDeserializable
        )
        {
            ...
            string json = await Serializer.SerializeWrapper(
                boxedEnvelope,
                UnspeakableTypeContext.Default.Options
            );
        ...
        [Theory]
        [MemberData(nameof(GetUnspeakableTypes))]
        public async Task CanSerializeUnspeakableTypesAsBoxedCollectionElements<T>(
            Envelope<T> envelope,
            string expectedJson,
            bool isBaseTypeDeserializable
        )
        ...
        [Theory]
        [MemberData(nameof(GetUnspeakableTypes))]
        public async Task CanSerializeUnspeakableTypesAsBoxedDictionaryValues<T>(
            Envelope<T> envelope,
            string expectedJson,
            bool isBaseTypeDeserializable
        )
        ...
        public static IEnumerable<object[]> GetUnspeakableTypes()
        {
            ...
            yield return Wrap<BasePoco>(
                new PrivateDerivedPoco { BaseValue = 1, DerivedValue = 2 },
                """{"BaseValue":1}"""
            );
            yield return Wrap<IMyInterface>(
                new MyImplementation { Value = 1, AnotherValue = 2 },
                """{"Value":1}""",
                isBaseTypeDeserializable: false
            );

            static object[] Wrap<T>(
                T value,
                string expectedJson,
                bool isBaseTypeDeserializable = true
