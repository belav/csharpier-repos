    ...
    public class JsonSourceGeneratorDiagnosticsTests
    {
        ...
        public void EmitsDocumentationOnPublicMembersAndDoesNotCauseCS1591()
        {
            ...
            Compilation compilation = CompilationHelper.CreateCompilation(
                ...
                configureParseOptions: options =>
                    options.WithDocumentationMode(DocumentationMode.Diagnose)
            );
        ...
        public void UnsuccessfulSourceGeneration()
        {
            ...
            {{
                ...
                if (explicitRef)
                {
                    ...
                    SyntaxReference syntaxReference = symbol
                        .GetAttributes()
                        .First()
                ...
                (Location, string)[] expectedWarningDiagnostics = new (Location, string)[]
                {
                    (
                        location,
                        "Did not generate serialization metadata for type 'global::ReferencedAssembly.ActiveOrUpcomingEvent[]'."
                    )
                };
        ...
        public void NameClashSourceGeneration()
        {
            ...
            INamedTypeSymbol symbol = (INamedTypeSymbol)
                compilation.GetSymbolsWithName("JsonContext").FirstOrDefault();
            SyntaxReference syntaxReference = new List<AttributeData>(symbol.GetAttributes())[
                1
            ].ApplicationSyntaxReference;
            TextSpan textSpan = syntaxReference.Span;
        ...
        public void WarnOnClassesWithInitOnlyProperties()
        {
            ...
            (Location, string)[] expectedWarningDiagnostics = new (Location, string)[]
            {
                (
                    location,
                    "The type 'Location' defines init-only properties, deserialization of which is currently not supported in source generation mode."
                )
            };
        ...
        public void WarnOnClassesWithMixedInitOnlyProperties()
        {
            ...
            (Location, string)[] expectedWarningDiagnostics = new (Location, string)[]
            {
                (
                    location,
                    "The type 'MyClass' defines init-only properties, deserialization of which is currently not supported in source generation mode."
                )
            };
