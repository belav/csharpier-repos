...
{
    /// <summary>
    /// Tests that the serializer's JsonSerializer.Deserialize(ReadOnlySpan<byte> utf8Json, Type returnType, JsonSerializerOptions options)
    ...
    internal class Program
    {
        static int Main(string[] args)
        {
            string json = "[1]";
            int[] arr = (int[])
                JsonSerializer.Deserialize(Encoding.UTF8.GetBytes(json), typeof(int[]));
