...
namespace System.Text.Json
{
    public sealed partial class Utf8JsonWriter
    {
        ...
        public void WriteNumber(string propertyName, float value) =>
            WriteNumber(
                (propertyName ?? throw new ArgumentNullException(nameof(propertyName))).AsSpan(),
                value
            );
            ...
            Debug.Assert(
                escapedPropertyName.Length
                    < (int.MaxValue / JsonConstants.MaxExpansionFactorWhileTranscoding)
                        - JsonConstants.MaximumFormatSingleLength
                        - 4
            );
        ...
        private void WriteNumberMinimized(ReadOnlySpan<byte> escapedPropertyName, float value)
        {
            Debug.Assert(
                escapedPropertyName.Length
                    < int.MaxValue - JsonConstants.MaximumFormatSingleLength - 4
            );

            int minRequired =
                escapedPropertyName.Length + JsonConstants.MaximumFormatSingleLength + 3; // 2 quotes for property name, and 1 colon
        ...
        private void WriteNumberIndented(ReadOnlySpan<byte> escapedPropertyName, float value)
        {
            ...
            int minRequired =
                indent + escapedPropertyName.Length + JsonConstants.MaximumFormatSingleLength + 4; // 2 quotes for property name, 1 colon, and 1 space
