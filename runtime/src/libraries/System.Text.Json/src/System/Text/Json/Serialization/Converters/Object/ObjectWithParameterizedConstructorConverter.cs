...
namespace System.Text.Json.Serialization.Converters
{
        ...
        : ObjectDefaultConverter<T> where T : notnull
    {
        ...
        )
        {
            ...
            if (state.UseFastPath)
            {
                ...
                if (argumentState.FoundPropertyCount > 0)
                {
                    ...
                    for (int i = 0; i < argumentState.FoundPropertyCount; i++)
                    {
                        ...
                        {
                            Debug.Assert(
                                jsonPropertyInfo == state.Current.JsonTypeInfo.DataExtensionProperty
                            );
                    ...
                    ArrayPool<FoundProperty>.Shared
                        .Return(argumentState.FoundProperties!, clearArray: true);
            ...
            else
            {
                ...
                if (argumentState.FoundPropertyCount > 0)
                {
                    ...
                    ArrayPool<FoundPropertyAsync>.Shared
                        .Return(argumentState.FoundPropertiesAsync!, clearArray: true);
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    if (jsonPropertyInfo.ShouldDeserialize)
                    {
                        ...
                        {
                            argumentState.FoundProperties = ArrayPool<FoundProperty>.Shared
                        ...
                        )
                        {
                            ...
                            var newCache = ArrayPool<FoundProperty>.Shared
            ...
            if (state.Current.PropertyState < StackFramePropertyState.ReadValue)
            {
                ...
                if (
                    !SingleValueReadWithReadAhead(
                        jsonParameterInfo.ConverterBase.ConverterStrategy,
                        ref reader,
                        ref state
                    )
                )
        ...
        )
        {
            ...
            {
                if (
                    !jsonPropertyInfo.ReadJsonExtensionDataValue(
                        ref state,
                        ref reader,
                        out propValue
                    )
            ...
            {
                argumentState.FoundPropertiesAsync = ArrayPool<FoundPropertyAsync>.Shared
                    .Rent(Math.Max(1, state.Current.JsonTypeInfo.PropertyCache!.Count));
            }
            else if (argumentState.FoundPropertyCount == argumentState.FoundPropertiesAsync!.Length)
            {
                // Rare case where we can't fit all the JSON properties in the rented pool; we have to grow.
                // This could happen if there are duplicate properties in the JSON.
                var newCache = ArrayPool<FoundPropertyAsync>.Shared
        ...
        )
        {
            ...
            jsonParameterInfo = state.Current.JsonTypeInfo
                .GetParameter(
                    unescapedPropertyName,
                    ref state.Current,
