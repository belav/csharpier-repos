    ...
    public abstract partial class JsonConverter<T> : JsonConverter
    {
        ...
        private protected void Initialize()
        {
            ...
            CanUseDirectReadOrWrite =
                ConverterStrategy == ConverterStrategy.Value && IsInternalConverter;
        ...
        internal sealed override JsonTypeInfo CreateReflectionJsonTypeInfo(
        ...
        internal sealed override JsonTypeInfo CreateCustomJsonTypeInfo(
            JsonSerializerOptions options
        )
        {
        ...
        )
        {
            ...
            if (
                reader.TokenType == JsonTokenType.Null && !HandleNullOnRead && !state.IsContinuation
            )
            {
        ...
        )
        {
            ...
            )
            {
                ...
                bool canBePolymorphic =
                    CanBePolymorphic || jsonTypeInfo.PolymorphicTypeResolver is not null;
        ...
        )
        {
            ...
            JsonDictionaryConverter<T>? dictionaryConverter =
                this as JsonDictionaryConverter<T>
                ?? (this as JsonMetadataServicesConverter<T>)?.Converter
                    as JsonDictionaryConverter<T>;
...
#nullable restore
        ...
        )
        {
            if (
                !IsInternalConverter
                && options.SerializerContext is null
                ...
                DefaultJsonTypeInfoResolver.TryGetDefaultSimpleConverter(
                    TypeToConvert,
            ...
            )
            {
                ...
                Debug.Assert(
                    defaultConverter.IsInternalConverter && defaultConverter is JsonConverter<T>
                );
                return ((JsonConverter<T>)defaultConverter).ReadAsPropertyNameCore(
                    ref reader,
                    TypeToConvert,
                    options
                );
        ...
        )
        {
            ...
            )
            {
                ...
                ((JsonConverter<T>)defaultConverter).WriteAsPropertyNameCore(
                    writer,
                    value,
                    options,
