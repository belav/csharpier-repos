...
/// When default resolvers are used this will be mapped to <see
// cref="JsonPropertyInfo.ObjectCreationHandling"/>.
///
/// When placed on a type with <see cref="JsonObjectCreationHandling.Populate"/> indicates that all
// members
/// that support population will be populated. When default resolvers are used this will be mapped
// to <see cref="JsonTypeInfo.PreferredPropertyObjectCreationHandling"/>.
///
/// Note that the attribute corresponds only to the preferred values of creation handling for
// properties when placed on a type.
/// For example when <see cref="JsonObjectCreationHandlingAttribute"/> with <see
// cref="JsonObjectCreationHandling.Populate"/> is placed on a class
/// and property is not capable of being populated, it will be replaced.
/// That may be true if i.e. value type doesn't have a setter or property is of type <see
// cref="IEnumerable{T}"/>.
///
/// Only the property type is taken into consideration. For example if property is of type
/// <see cref="IEnumerable{T}"/> has runtime value of type <see cref="List{T}"/> it will not be
// populated
...
/// deserialization will use a copy of the property value which will get assigned back to the setter
// once finished.
/// </remarks>
[AttributeUsage(
