...
namespace Microsoft.Extensions.Configuration.Test
{
    public class ConfigurationManagerTest
    {
        ...
        {
            var config = new ConfigurationManager { ["TestKey"] = "TestValue" };
        ...
        public async Task ProviderDisposeDelayedWaitingOnConcurrentRead(
            Action<IConfiguration> concurrentReadAction
        )
        {
            using var mre = new ManualResetEventSlim(false);
        ...
        public void ChainedAsEnumerateFlattensIntoDictionaryTest(bool removePath)
        {
            ...
            config2.AddConfiguration(config1).Add(memConfigSrc3);
        ...
        public void NewConfigurationRootMayBeBuiltFromExistingWithDuplicateKeys()
        {
            ...
            configurationRoot.AddInMemoryCollection(
                new Dictionary<string, string> { { "keya:keyb", "valueA" } }
            );
            configurationRoot.AddInMemoryCollection(
                new Dictionary<string, string> { { "KEYA:KEYB", "valueB" } }
            );
        ...
        private class BlockReadOnMREProvider : ConfigurationProvider, IDisposable
        {
            ...
            )
            {
                ...
                Assert.True(
                    _mre.Wait(_timeout),
                    "BlockReadOnMREProvider.GetChildKeys() timed out."
                );
