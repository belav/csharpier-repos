        ...
        public static unsafe string WriteString(ReadOnlySpan<char> label, ReadOnlySpan<byte> data)
        {
            ...
            fixed (byte* pData = data)
            {
                return string.Create(
                    encodedSize,
                    (
                        LabelPointer: new IntPtr(pLabel),
                        LabelLength: label.Length,
                        DataPointer: new IntPtr(pData),
                        DataLength: data.Length
                    ),
                    static (destination, state) =>
                    {
                        ReadOnlySpan<byte> data = new ReadOnlySpan<byte>(
                            state.DataPointer.ToPointer(),
                            state.DataLength
                        );
                        ReadOnlySpan<char> label = new ReadOnlySpan<char>(
                            state.LabelPointer.ToPointer(),
                            state.LabelLength
                        );
