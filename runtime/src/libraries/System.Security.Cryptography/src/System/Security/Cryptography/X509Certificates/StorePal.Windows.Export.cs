        ...
        public byte[]? Export(X509ContentType contentType, SafePasswordHandle password)
        {
            ...
            switch (contentType)
            {
                case X509ContentType.Cert:
                {
                    ...
                    try
                    {
                        unsafe
                        {
                            ...
                            Marshal.Copy(
                                (IntPtr)(pCertContext.DangerousCertContext->pbCertEncoded),
                                rawData,
                                0,
                                rawData.Length
                            );
                ...
                case X509ContentType.SerializedStore:
                    return SaveToMemoryStore(
                        Interop.Crypt32.CertStoreSaveAs.CERT_STORE_SAVE_AS_STORE
                    );

                case X509ContentType.Pkcs7:
                    return SaveToMemoryStore(
                        Interop.Crypt32.CertStoreSaveAs.CERT_STORE_SAVE_AS_PKCS7
                    );
