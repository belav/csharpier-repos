...
namespace System.Security.Cryptography.Tests
{
    ...
    public class OpenSslNamedKeysTests
    {
        ...
        private const string TpmTssEngineEcDsaKeyHandleEnvVarName =
            EnvVarPrefix + "TPM_ECDSA_KEY_HANDLE";
        ...
        public static string TpmTssEngineEcDsaKeyHandle { get; } =
            Environment.GetEnvironmentVariable(TpmTssEngineEcDsaKeyHandleEnvVarName);
        public static bool ShouldRunEngineTests { get; } =
            PlatformDetection.OpenSslPresentOnSystem
            && StringToBool(Environment.GetEnvironmentVariable(TestEngineEnabledEnvVarName));
        public static bool ShouldFailTests { get; } =
            StringToBool(Environment.GetEnvironmentVariable(TestEngineEnsureFailingEnvVarName));
        public static bool ShouldRunTpmTssTests =>
            PlatformDetection.OpenSslPresentOnSystem
            && !string.IsNullOrEmpty(TpmTssEngineEcDsaKeyHandle);
        ...
        [ConditionalFact(
            ...
            nameof(PlatformDetection.OpenSslNotPresentOnSystem)
        )]
        ...
        [ConditionalFact(
            typeof(PlatformDetection),
            nameof(PlatformDetection.OpenSslPresentOnSystem)
        )]
        ...
        {
            Assert.ThrowsAny<CryptographicException>(() =>
                SafeEvpPKeyHandle.OpenPrivateKeyFromEngine(TestEngineName, NonExistingEngineKeyName)
            );
            Assert.ThrowsAny<CryptographicException>(() =>
                SafeEvpPKeyHandle.OpenPublicKeyFromEngine(TestEngineName, NonExistingEngineKeyName)
            );
        }
        ...
        {
            using SafeEvpPKeyHandle priKeyHandle = SafeEvpPKeyHandle.OpenPrivateKeyFromEngine(
                TestEngineName,
                TestEngineKeyId
            );
        ...
        {
            using SafeEvpPKeyHandle pubKeyHandle = SafeEvpPKeyHandle.OpenPublicKeyFromEngine(
                TestEngineName,
                TestEngineKeyId
            );
        ...
        {
            using (
                SafeEvpPKeyHandle priKeyHandle = SafeEvpPKeyHandle.OpenPrivateKeyFromEngine(
                    TestEngineName,
                    TestEngineKeyId
                )
            )
            ...
            using (RSA rsaPub = RSA.Create())
            {
                ...
                byte[] signature = rsaPri.SignData(
                    data,
        ...
        {
            using (
                SafeEvpPKeyHandle pubKeyHandle = SafeEvpPKeyHandle.OpenPublicKeyFromEngine(
                    TestEngineName,
                    TestEngineKeyId
                )
            )
