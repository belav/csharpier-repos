...
{
    public class HmacSha512Tests : Rfc4231HmacTests<HmacSha512Tests.Traits>
    {
        ...
        ) => HMACSHA512.HashDataAsync(key, source, destination, cancellationToken);

        protected override ValueTask<byte[]> HashDataOneShotAsync(
            ReadOnlyMemory<byte> key,
            Stream source,
            CancellationToken cancellationToken
        ) => HMACSHA512.HashDataAsync(key, source, cancellationToken);
