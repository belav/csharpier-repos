...
namespace System.Security.Cryptography.Tests
{
    public abstract class HKDFTests
    {
        ...
        private static HkdfTestCase[] Rfc5869TestCases { get; } =
            new HkdfTestCase[7]
            {
                new HkdfTestCase()
                {
        ...
        public class HkdfSpanTests : HKDFTests
        {
            ...
            public void ExtractOverlapsPrkOverKeyMaterial(int ikmOffset, int outputOffset)
            {
                ReadOnlySpan<byte> ikm =
                    "0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b".HexToByteArray();
                ...
                int length =
                    Math.Max(ikm.Length, expectedPrk.Length) + Math.Max(ikmOffset, outputOffset);
            ...
            public void ExtractOverlapsPrkOverSalt(int saltOffset, int outputOffset)
            {
                ...
                int length =
                    Math.Max(ikm.Length, expectedPrk.Length) + Math.Max(saltOffset, outputOffset);
                Span<byte> buffer = new byte[length];
