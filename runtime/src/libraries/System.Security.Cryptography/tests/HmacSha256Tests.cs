...
namespace System.Security.Cryptography.Tests
{
    public class HmacSha256Tests : Rfc4231HmacTests<HmacSha256Tests.Traits>
    {
        ...
        ) => HMACSHA256.HashDataAsync(key, source, destination, cancellationToken);

        protected override ValueTask<byte[]> HashDataOneShotAsync(
            ReadOnlyMemory<byte> key,
            Stream source,
            CancellationToken cancellationToken
        ) => HMACSHA256.HashDataAsync(key, source, cancellationToken);
        ...
        {
            // Verfied with:
            // for _ in {1..1024}; do echo -n "0102030405060708"; done | openssl sha256 -hex -mac HMAC -macopt
        ...
        {
            // Verfied with:
            // for _ in {1..1025}; do echo -n "0102030405060708"; done | openssl sha256 -hex -mac HMAC -macopt
            // hexkey:000102030405060708090A0B0C0D0E0F
            VerifyRepeating(
                input: "0102030405060708",
                1025,
