...
namespace System.Security.Cryptography.X509Certificates.Tests.CertificateCreation
{
    ...
    public static class CertificateRequestUsageTests
    {
        ...
        public static void ReproduceBigExponentCert()
        {
            ...
            X509Extension basicConstraints = new X509BasicConstraintsExtension(
                true,
                false,
                0,
                false
        ...
        private static void SimpleSelfSign(
        ...
        public static void CheckTimeNested()
        {
            ...
            using (RSA rsa = RSA.Create(TestData.RsaBigExponentParams))
            {
                ...
                using (X509Certificate2 issuer = request.CreateSelfSigned(notBefore, notAfter))
                {
                    ...
                    request
                        .Create(issuer, notBefore, notAfter.AddMilliseconds(1), serial)
                        .Dispose(); //);
        ...
        public static void ECDSA_Signing_ECDH()
        {
            ...
            Assert.Equal(
                ...
                publicCertKey.ExportSubjectPublicKeyInfo()
            );
        ...
        public static void ECDSA_Signing_ECDH_NoKeyUsageValidForECDSAAndECDH()
        {
            ...
            Assert.Equal(
                ...
                publicCertEcDhKey.ExportSubjectPublicKeyInfo()
            );
            Assert.Equal(
                expectedSubjectPublicKeyInfo,
                publicCertEcDsaKey.ExportSubjectPublicKeyInfo()
            );
        }
        ...
        public static void ECDSA_Signing_RSAPublicKey()
        {
            ...
            using (ECDsa ecdsa = ECDsa.Create())
            {
                ...
                using (X509Certificate2 cert = request.CreateSelfSigned(now, now.AddDays(1)))
                {
                    X509SignatureGenerator rsaGenerator = X509SignatureGenerator.CreateForRSA(
                        rsa,
                        RSASignaturePadding.Pkcs1
                    );
        ...
        {
            using (
                X509Certificate2 savedCert = new X509Certificate2(
                    TestData.PfxData,
        ...
        public static void CreateSelfSigned_CurveNameIsCaseInsensitive(
            string curveName,
            string hashName
        )
        {
            ...
            using (ECDsa ecdsa = ECDsa.Create(ecCurve))
            {
                ...
                using (
                    X509Certificate2 selfSignedCert = request.CreateSelfSigned(notBefore, notAfter)
                )
        ...
        private class InvalidSignatureGenerator : X509SignatureGenerator
        {
            ...
            internal InvalidSignatureGenerator(
                byte[] keyBytes,
                byte[] parameterBytes,
                byte[] sigAlgBytes
            )
