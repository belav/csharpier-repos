    ...
    public static class PublicKeyTests
    {
        ...
        public static void TestECDsaPublicKey_ValidatesSignature()
        {
            ...
            using (ECDsa publicKey = cert.GetECDsaPublicKey())
            {
                ...
                bool isSignatureValid = publicKey.VerifyData(
                    helloBytes,
                    existingSignature,
        ...
        public static void TestECDHPublicKey_DeriveSecret()
        {
            ...
            using (ECDiffieHellman publicKey = publicEcdhCert.GetECDiffieHellmanPublicKey())
            {
                ...
                byte[] key = otherParty.DeriveKeyFromHash(
                    publicKey.PublicKey,
                    HashAlgorithmName.SHA256
                );
                    ...
                    using (ECDsa publicKey = cert.GetECDsaPublicKey())
                    {
                        ...
                        if (!isSignatureValid)
                        {
                            Assert.True(
                                ...
                                "signature invalid on Android only"
                            );
                            return;
                        }
                        unchecked
                        ...
                        isSignatureValid = publicKey.VerifyData(
                            helloBytes,
                            existingSignature,
                            HashAlgorithmName.SHA256
                        );
            ...
            using (var cert = new X509Certificate2(TestData.ECDsa224Certificate))
            {
                ...
                using (ecdsa)
                {
                    ...
                    byte[] signature = (
                        ...
                        +
                        // s
                        "5e3da9d359001a6be18e2b4e49205e5219f30a9daeb026159f41b9de"
                    ).HexToByteArray();
        ...
        public static void TestDSAPublicKey_VerifiesSignature()
        {
            ...
            using (DSA pubKey = cert.GetDSAPublicKey())
            {
                Assert.True(
                    ...
                    "pubKey verifies signature"
                );
                Assert.False(
                    pubKey.VerifyData(wrongData, signature, HashAlgorithmName.SHA1),
                ...
                Assert.False(
                    pubKey.VerifyData(data, signature, HashAlgorithmName.SHA1),
                    "pubKey verifies tampered signature"
                );
            }
        }
        ...
        public static void TestKey_BrainpoolP160r1()
        {
            if (PlatformDetection.WindowsVersion >= 10)
            {
                TestKey_ECDsaCng(
                    TestData.ECDsabrainpoolP160r1_CertificatePemBytes,
                    TestData.ECDsabrainpoolP160r1_PublicKey
                );
            }
        }
        ...
        public static void ExportSubjectPublicKeyInfo_RSA()
        {
            ...
            Assert.True(
                algSpki.SequenceEqual(key.ExportSubjectPublicKeyInfo()),
                "SequenceEquals(ExportSubjectPublicKeyInfo)"
            );

            // Just right
            Assert.True(
                key.TryExportSubjectPublicKeyInfo(algSpki, out int written),
                nameof(key.TryExportSubjectPublicKeyInfo)
            );
            Assert.Equal(algSpki.Length, written);

            // Too small
            Assert.False(
                key.TryExportSubjectPublicKeyInfo(algSpki.Slice(1), out written),
                nameof(key.TryExportSubjectPublicKeyInfo)
            );
