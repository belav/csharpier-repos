...
namespace LibraryImportGenerator.IntegrationTests
{
    ...
    public class CollectionMarshallingFails
    {
        ...
        {
            var myBoolStruct = new BoolStruct()
            {
                b1 = true,
                b2 = true,
                b3 = false,
            };
            var myBoolStructNative = new BoolStructNative()
            {
                b1 = 1,
                b2 = 1,
                b3 = 0,
            ...
            foreach (var throwOn in new int[] { 0, 1, 5, 9 })
            {
                ...
                BoolStructInMarshaller.Marshaller.ExpectedFreedValues = Enumerable
                    .Range(0, throwOn)
                    .Select(i => myBoolStructNative)
                    .ToArray();
        ...
        public void MultidimensionalArray_CheckOuterArrayIsIndexTracked()
        {
            ...
            foreach (var throwOn in new int[] { 0, 1, 5, 9 })
            {
                ...
                Assert.Throws<ArgumentException>(() =>
                {
                    NativeExportsNE.MarshallingFails
    ...
    public struct BoolStructNative : IEquatable<BoolStructNative>
    {
        ...
        public bool Equals(BoolStructNative other) =>
            other.b1 == b1 && other.b2 == b2 && other.b3 == b3;
    }
    ...
    static class BoolStructArrayMarshaller
    {
        ...
        public static BoolStruct[] ConvertToManaged(nint unmanaged) =>
    ...
    [CustomMarshaller(
        ...
        typeof(FillRangeArrayMarshaller)
    )]
    public static class FillRangeArrayMarshaller
    {
        ...
        public static int ConvertToUnmanaged(IntStructWrapper managed) =>
            Marshaller.ConvertToUnmanaged(managed);
    ...
    {
        public static IntStructWrapper ConvertToManaged(int unmanaged) =>
            new IntStructWrapper() { Value = unmanaged };
    ...
    public class FailingMarshaller<T, TUnmanaged>
        where TUnmanaged : unmanaged, IEquatable<TUnmanaged>
    {
        ...
        public int ExpectedFreeCount
        {
            get => _expectedFreeCount;
            set => (_expectedFreeCount, _freeCount) = (value, 0);
        }
        private TUnmanaged[]? _expectedFreedValues;
        public TUnmanaged[]? ExpectedFreedValues
        {
            get => _expectedFreedValues;
            set =>
                (_expectedFreedValues, ExpectedFreeCount, _freeCount) = (
                    value,
                    value?.Length ?? 0,
                    0
                );
        ...
        public FailingMarshaller(
            Func<T, TUnmanaged> marshal,
            Func<TUnmanaged, T> unmarshal,
            Func<TUnmanaged, int, bool> okayToFree
        )
        {
            _marshal = marshal;
        ...
        public void AssertAllHaveBeenCleaned()
        {
            if (ExpectedFreeCount - _freeCount != 0)
                throw new InvalidMarshallingException(
                    $"Incorrected number of elements freed. Expected {ExpectedFreeCount - _freeCount} more elements to be freed."
                );
            Reset();
        }
        ...
        public void Free(TUnmanaged unmanaged)
        {
            if (!_okayToFree(unmanaged, _freeCount))
                throw new InvalidMarshallingException(
                    "Freed unmanaged value that was not expected to be freed"
                );

            if (_freeCount + 1 > ExpectedFreeCount)
                throw new InvalidMarshallingException(
                    $"Freed too many unmanaged values. Expected to free {ExpectedFreeCount} values."
                );
        ...
        {
            public InvalidMarshallingException(string? message)
                : base(message) { }
        }
    }
}
