...
namespace System.Runtime.InteropServices.Tests
{
    public partial class ComAwareEventInfoTests
    {
        ...
        public void AddEventHandler_ComObjectWithMultipleComEventInterfaceAttribute_ThrowsAmbiguousMatchException()
        {
            ...
            typeBuilder.SetCustomAttribute(
                new CustomAttributeBuilder(
                    typeof(ComEventInterfaceAttribute).GetConstructors()[0],
        ...
        {
            var attribute = new ComAwareEventInfo(
                typeof(NullSourceType),
                nameof(NullSourceType.Event)
            );
            var target = new ComImportObject();
            Delegate handler = new EventHandler(EventHandler);
        ...
        {
            var attribute = new ComAwareEventInfo(
                typeof(NoSuchSourceType),
                nameof(NoSuchSourceType.Event)
            );
            var target = new ComImportObject();
            Delegate handler = new EventHandler(EventHandler);
        ...
        {
            var attribute = new ComAwareEventInfo(
                typeof(NoDispAttributeInterface),
                nameof(NoDispAttributeInterface.Event)
            );
            var target = new ComImportObject();
            Delegate handler = new EventHandler(EventHandler);
        ...
        {
            var attribute = new ComAwareEventInfo(
                typeof(DispAttributeInterface),
                nameof(DispAttributeInterface.Event)
            );
            var target = new ComImportObject();
            Delegate handler = new EventHandler(EventHandler);
