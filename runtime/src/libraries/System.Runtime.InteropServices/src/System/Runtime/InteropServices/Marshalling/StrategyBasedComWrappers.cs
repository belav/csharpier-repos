    ...
    public class StrategyBasedComWrappers : ComWrappers
    {
        ...
        protected virtual IIUnknownInterfaceDetailsStrategy GetOrCreateInterfaceDetailsStrategy()
        {
            if (
                ...
                && ComObject.BuiltInComSupported
                && ComObject.ComImportInteropEnabled
            )
            {
            ...
            [UnconditionalSuppressMessage(
                ...
                Justification = "The usage is guarded, but the analyzer and the trimmer don't understand it."
            )]
            [UnconditionalSuppressMessage(
                "Trimming",
        ...
        protected sealed override unsafe ComInterfaceEntry* ComputeVtables(
            object obj,
        ...
        )
        {
            if (
                GetOrCreateInterfaceDetailsStrategy()
                    .GetComExposedTypeDetails(obj.GetType().TypeHandle) is
                { } details
            )
            {
        ...
        protected sealed override unsafe object CreateObject(
            nint externalComObject,
            CreateObjectFlags flags
        )
        {
            if (
                flags.HasFlag(CreateObjectFlags.TrackerObject)
