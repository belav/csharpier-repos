    ...
    public class RuntimeComApiUsageWithSourceGeneratedComAnalyzer : DiagnosticAnalyzer
    {
        ...
        public override void Initialize(AnalysisContext context)
        {
            ...
            context.RegisterCompilationStartAction(context =>
            {
                ...
                {
                    sourceGeneratedComRecognizers.Add(
                        type =>
                            type.GetAttributes()
                                .Any(
                                    attr =>
                                        generatedComClassAttribute.Equals(
                                            attr.AttributeClass,
                    ...
                    sourceGeneratedComRecognizers.Add(
                        type =>
                            type.GetAttributes()
                                .Any(
                                    attr =>
                                        generatedComInterfaceAttribute.Equals(
                                            attr.AttributeClass,
                                            SymbolEqualityComparer.Default
                                        )
                                )
                ...
                {
                    sourceGeneratedComRecognizers.Add(
                        type => type.Equals(comObjectType, SymbolEqualityComparer.Default)
                    );
                }
                ...
                var methodsOfInterest = new Dictionary<
                    ISymbol,
                    ImmutableArray<Func<IInvocationOperation, (ITypeSymbol, Location)?>>
                >(SymbolEqualityComparer.Default);
                ...
                methodsOfInterest.Add(
                    marshalType.GetMembers("SetComObjectData")[0],
                    firstArgumentTypeLookupOnly
                );
                methodsOfInterest.Add(
                    marshalType.GetMembers("GetComObjectData")[0],
                    firstArgumentTypeLookupOnly
                );
                methodsOfInterest.Add(
                    marshalType.GetMembers("ReleaseComObject")[0],
                    firstArgumentTypeLookupOnly
                );
                methodsOfInterest.Add(
                    marshalType.GetMembers("FinalReleaseComObject")[0],
                    firstArgumentTypeLookupOnly
                );

                foreach (
                    var createAggregatedObject in marshalType.GetMembers("CreateAggregatedObject")
                )
                {
                    if (createAggregatedObject is IMethodSymbol { IsGenericMethod: true })
                    {
                        methodsOfInterest.Add(
                            ...
                            ImmutableArray.Create(
                                CreateTypeArgumentTypeLookup(0),
                                CreateArgumentTypeLookup(1)
                            )
                        );
                    }
                    else
                    {
                        methodsOfInterest.Add(
                            createAggregatedObject,
                            ImmutableArray.Create(CreateArgumentTypeLookup(1))
                        );
                    }
                }
                ...
                foreach (var createWrapperOfType in marshalType.GetMembers("CreateWrapperOfType"))
                {
                    if (createWrapperOfType is IMethodSymbol { IsGenericMethod: true })
                    {
                        methodsOfInterest.Add(
                            ...
                            ImmutableArray.Create(
                                CreateTypeArgumentTypeLookup(0),
                                CreateTypeArgumentTypeLookup(1),
                                firstArgumentTypeLookup
                            )
                        );
                    }
                    else
                ...
                methodsOfInterest.Add(
                    marshalType.GetMembers("GetTypedObjectForIUnknown")[0],
                    ImmutableArray.Create(CreateTypeOfArgumentTypeLookup(1))
                );
                methodsOfInterest.Add(
                    marshalType.GetMembers("GetIUnknownForObject")[0],
                    firstArgumentTypeLookupOnly
                );
                methodsOfInterest.Add(
                    marshalType.GetMembers("GetIDispatchForObject")[0],
                    firstArgumentTypeLookupOnly
                );

                foreach (
                    var getComInterfaceForObject in marshalType.GetMembers(
                ...
                context.RegisterOperationAction(
                    context =>
                    {
                        ...
                        if (
                            methodsOfInterest.TryGetValue(
                                operation.TargetMethod.OriginalDefinition,
                                out ImmutableArray<
                                    Func<IInvocationOperation, (ITypeSymbol, Location)?>
                                > discoverers
                            )
                        )
                        {
                            foreach (
                                Func<
                                    ...
                                    (ITypeSymbol, Location)?
                                > discoverer in discoverers
                            )
                            {
                                ...
                                if (
                                    ...
                                    (ITypeSymbol targetType, Location diagnosticLocation)
                                )
                                {
                ...
                context.RegisterOperationAction(
                    context =>
                    {
                        ...
                        if (
                            operation.Type is INamedTypeSymbol { IsComImport: true }
                            && !enableGeneratedComInterfaceComImportInterop
                        )
                        {
                            IOperation operand = operation.Operand;
                            if (
                                operand is IConversionOperation
                                {
                        ...
                        foreach (var recognizer in sourceGeneratedComRecognizers)
                        {
                            if (recognizer(operation.Type))
                            {
                                ...
                                if (
                                    operand.Type is INamedTypeSymbol { IsComImport: true }
                                    && !enableGeneratedComInterfaceComImportInterop
                                )
                                {
                                    context.ReportDiagnostic(
                ...
                > CreateArgumentTypeLookup(int ordinal) =>
                    invocation =>
                ...
                static Func<
                    IInvocationOperation,
                    (ITypeSymbol Type, Location location)?
                > CreateTypeArgumentTypeLookup(int ordinal) =>
                    invocation =>
                    {
                        ...
                        {
                            location = ((GenericNameSyntax)expression).TypeArgumentList.Arguments[
                                ordinal
                            ].GetLocation();
                        }
                ...
                static Func<
                    IInvocationOperation,
                    (ITypeSymbol Type, Location location)?
                > CreateTypeOfArgumentTypeLookup(int ordinal) =>
                    invocation =>
                        invocation.GetArgumentByOrdinal(ordinal).Value switch
                        {
