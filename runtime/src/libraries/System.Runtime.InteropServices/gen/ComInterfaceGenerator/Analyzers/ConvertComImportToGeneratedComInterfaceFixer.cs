...
namespace Microsoft.Interop.Analyzers
{
    ...
    public sealed class ConvertComImportToGeneratedComInterfaceFixer
        : ConvertToSourceGeneratedInteropFixer
    {
        ...
        )
        {
            bool mayRequireAdditionalWork =
                selectedOptions.TryGetValue(
                    ...
                    out Option mayRequireAdditionalWorkOption
                ) && mayRequireAdditionalWorkOption is Option.Bool(true);
            bool addStringMarshalling =
                selectedOptions.TryGetValue(
                    AddStringMarshallingOption,
                    out Option addStringMarshallingOption
                ) && addStringMarshallingOption is Option.Bool(true);

            return (editor, ct) =>
                ConvertComImportToGeneratedComInterfaceAsync(
                    editor,
                    node,
                    mayRequireAdditionalWork,
                    addStringMarshalling,
                    ct
                );
        }
        ...
        )
        {
            ...
            var generatedComInterfaceAttribute = gen.Attribute(
                gen.TypeExpression(
                        comp.GetTypeByMetadataName(TypeNames.GeneratedComInterfaceAttribute)
                    )
                    ...
                    new[]
                    {
                        gen.AttributeArgument(
                            ...
                            gen.MemberAccessExpression(
                                gen.DottedName(TypeNames.StringMarshalling),
                                gen.IdentifierName(nameof(StringMarshalling.Custom))
                            )
                        ),
                        gen.AttributeArgument(
                            "StringMarshallingCustomType",
                            gen.TypeOfExpression(
                                gen.TypeExpression(
            ...
            {
                generatedComInterfaceAttribute =
                    generatedComInterfaceAttribute.WithAdditionalAnnotations(
            ...
            foreach (var member in gen.GetMembers(node))
            {
                ...
                IMethodSymbol method = (IMethodSymbol)
                    editor.SemanticModel.GetDeclaredSymbol(member, ct);
                ...
                {
                    generatedDeclaration = AddHResultStructAsErrorMarshalling(
                        gen,
                        method,
                        generatedDeclaration
                    );
