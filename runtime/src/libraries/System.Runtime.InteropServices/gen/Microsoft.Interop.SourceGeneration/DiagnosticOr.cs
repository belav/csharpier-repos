...
namespace Microsoft.Interop
{
    ...
    public abstract record DiagnosticOr<T>
    {
        ...
        private sealed record Diag : DiagnosticOr<T>
        {
            ...
            internal Diag(ImmutableArray<DiagnosticInfo> diagnostics) =>
                _diagnostics = diagnostics.ToSequenceEqual();
        ...
        private sealed record Val : DiagnosticOr<T>
        {
            ...
            public override ImmutableArray<DiagnosticInfo> Diagnostics =>
        ...
        private sealed record ValueAndDiagnostic : DiagnosticOr<T>
        {
            ...
            internal ValueAndDiagnostic(T value, ImmutableArray<DiagnosticInfo> diagnostics) =>
                (_value, _diagnostics) = (value, diagnostics.ToSequenceEqual());
        ...
        /// </summary>
        public DiagnosticOr<T> AddDiagnostic(DiagnosticInfo diagnostic) =>
            this switch
            {
                Diag d => new Diag(d.Diagnostics.Add(diagnostic)),
                Val v => new ValueAndDiagnostic(v.Value, ImmutableArray.Create(diagnostic)),
                ValueAndDiagnostic vad
                    => new ValueAndDiagnostic(vad.Value, vad.Diagnostics.Add(diagnostic)),
        ...
        /// </summary>
        public DiagnosticOr<T> WithValue(T value) =>
            this switch
            {
    ...
    public static class DiagnosticOrTHelperExtensions
    {
        ...
        public static (
            ...
            IncrementalValuesProvider<DiagnosticInfo>
        ...
        /// </summary>
        public static (
            IncrementalValuesProvider<SequenceEqualImmutableArray<T>>,
            IncrementalValuesProvider<DiagnosticInfo>
        ) SplitArrays<T>(
            this IncrementalValuesProvider<SequenceEqualImmutableArray<DiagnosticOr<T>>> provider
        )
        {
            var values = provider.Select(
                (arr, ct) =>
                    arr.Where(x => x.HasValue)
                        .Select((x, ct) => x.Value)
                        .ToSequenceEqualImmutableArray()
            );
            var diagnostics = provider.SelectMany(
        ...
        /// </summary>
        public static (
            IncrementalValuesProvider<(T, T2)>,
            IncrementalValuesProvider<DiagnosticInfo>
        ) Split<T, T2>(this IncrementalValuesProvider<(DiagnosticOr<T>, T2)> provider)
        {
            var values = provider
                .Where(x => x.Item1.HasValue)
                .Select(static (x, ct) => (x.Item1.Value, x.Item2));
            var diagnostics = provider
                .Where(x => x.Item1.HasDiagnostic)
        ...
        /// </summary>
        public static IncrementalValuesProvider<T> FilterAndReportDiagnostics<T>(
        ...
        public static (
            IncrementalValuesProvider<T>,
            IncrementalValuesProvider<T2>
        ) FilterAndReportDiagnostics<T, T2>(
            this IncrementalGeneratorInitializationContext ctx,
            IncrementalValuesProvider<DiagnosticOr<T>> diagnosticOrValues,
        ...
        > FilterAndReportDiagnostics<T>(
            this IncrementalGeneratorInitializationContext ctx,
            IncrementalValuesProvider<
                SequenceEqualImmutableArray<DiagnosticOr<T>>
            > diagnosticOrValues
        )
        {
