...
namespace Microsoft.Interop
{
    ...
    /// </summary>
    public abstract record MarshalAsInfo(UnmanagedType UnmanagedType, CharEncoding CharEncoding)
        : MarshallingInfoStringSupport(CharEncoding),
            IForwardedMarshallingInfo
    {
        ...
        bool IForwardedMarshallingInfo.TryCreateAttributeSyntax(
            [NotNullWhen(true)] out AttributeSyntax? attribute
        ) => TryCreateAttributeSyntax(out attribute);
    }

    public sealed record MarshalAsScalarInfo(UnmanagedType UnmanagedType, CharEncoding CharEncoding)
        : MarshalAsInfo(UnmanagedType, CharEncoding)
    {
            ...
            attribute = Attribute(
                ...
                AttributeArgumentList(
                    SingletonSeparatedList(
                        AttributeArgument(
                            CastExpression(
                                TypeSyntaxes.System_Runtime_InteropServices_UnmanagedType,
                                LiteralExpression(
    ...
    /// </summary>
    public sealed class MarshalAsAttributeParser
            ...
            IUseSiteAttributeParser
    {
        ...
        )
        {
            ImmutableDictionary<string, TypedConstant> namedArguments =
                ImmutableDictionary.CreateRange(attributeData.NamedArguments);
            ...
            if (
                namedArguments.TryGetValue(
                    ...
                    out TypedConstant sizeConstArg
                )
            )
            {
            ...
            if (
                namedArguments.TryGetValue(
                    ...
                    out TypedConstant sizeParamIndexArg
                )
            )
            {
                if (
                    !elementInfoProvider.TryGetInfoForParamIndex(
                        attributeData,
                        (short)sizeParamIndexArg.Value!,
                        marshallingInfoCallback,
                        out TypePositionInfo paramIndexInfo
                    )
                )
                {
                    _diagnostics.ReportConfigurationNotSupported(
                        attributeData,
                        nameof(MarshalAsAttribute.SizeParamIndex),
                        sizeParamIndexArg.Value.ToString()
                    );
                }
        ...
        )
        {
            ...
            if (
                !Enum.IsDefined(typeof(UnmanagedType), unmanagedType)
            ...
            foreach (KeyValuePair<string, TypedConstant> namedArg in attributeData.NamedArguments)
            {
                switch (namedArg.Key)
                {
                    ...
                    case nameof(MarshalAsAttribute.MarshalCookie):
                        _diagnostics.ReportConfigurationNotSupported(
                            attributeData,
                            $"{attributeData.AttributeClass!.Name}{Type.Delimiter}{namedArg.Key}"
                        );
                        break;
            ...
            if (
                useSiteAttributes.TryGetUseSiteAttributeInfo(
                    indirectionDepth,
                    out UseSiteAttributeData useSiteAttributeData
                )
            )
            {
            ...
            return new MarshalAsArrayInfo(
                unmanagedType,
                _defaultInfo.CharEncoding,
