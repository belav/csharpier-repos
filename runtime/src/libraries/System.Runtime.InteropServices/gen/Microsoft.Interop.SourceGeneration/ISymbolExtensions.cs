...
namespace Microsoft.Interop
{
    public static class ISymbolExtensions
    {
        ...
        /// </summary>
        public static bool IsAccessibleFromFileScopedClass(
            this INamedTypeSymbol symbol,
            [NotNullWhen(false)] out string? details
        )
        {
            ...
            for (
                ...
                current is INamedTypeSymbol currentType;
                current = currentType.ContainingSymbol
            )
            {
                // a higher enum value is more accessible
                ...
                {
                    details = string.Format(
                        SR.ContainingTypeAccessibilityDetails,
                        current.ToDisplayString(),
                        current.DeclaredAccessibility.ToString().ToLowerInvariant()
                    );
                    return false;
                }
            }
