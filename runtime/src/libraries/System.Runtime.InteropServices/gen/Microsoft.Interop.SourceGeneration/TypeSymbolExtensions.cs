...
namespace Microsoft.Interop
{
    public static class TypeSymbolExtensions
    {
        ...
        public static bool IsStrictlyBlittableInContext(
        ...
        )
        {
            ...
            )
            {
                ...
                else if (t.IsValueType)
                {
                    ...
                    )
                    {
                        ...
                        if (
                            ...
                            is TypeNames.System_Runtime_InteropServices_CLong // CLong is an interop intrinsic type for the C long type
                                ...
                                or TypeNames.System_Runtime_InteropServices_NFloat
                        ) // NFloat is an interop intrinsic type for a pointer-sized floating point type
                        {
                            return true;
                        }
                    ...
                    return t.HasOnlyBlittableFields(
                        seenTypes,
                        compilation,
                        &IsStrictlyBlittableWorker
                    );
        ...
        private static unsafe bool IsBlittableWorker(
            ...
            ImmutableHashSet<ITypeSymbol> seenTypes,
            Compilation compilation,
            delegate* <ITypeSymbol, ImmutableHashSet<ITypeSymbol>, Compilation, bool> isBlittable
        )
        {
        ...
        public static bool IsOfType(this INamedTypeSymbol type, string typeName)
        {
            if (typeName.Contains('<') || typeName.Contains('+') || typeName.Contains('/'))
                throw new ArgumentException(
                    $"Cannot handle type name in the format provided: {typeName}",
                    nameof(typeName)
                );
            ...
            for (int i = typeNameParts.Length - 1; i >= 0; i--)
            {
                ...
                if (current.MetadataName != typeNameParts[i])
                    return false;
                current =
                    (INamespaceOrTypeSymbol)current.ContainingType ?? current.ContainingNamespace;
            }
            return true;
        }
    }
