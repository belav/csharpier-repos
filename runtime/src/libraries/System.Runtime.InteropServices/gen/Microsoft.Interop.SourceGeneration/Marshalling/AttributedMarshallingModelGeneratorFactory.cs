...
namespace Microsoft.Interop
{
    ...
    public class AttributedMarshallingModelGeneratorFactory : IMarshallingGeneratorFactory
    {
                ...
                for (int i = 0; i < numIndirectionLevels; i++)
                {
                    if (marshallingInfo is NativeLinearCollectionMarshallingInfo collectionInfo)
                    {
                        CustomTypeMarshallerData marshallerData =
                            GetMarshallerDataForTypePositionInfo(collectionInfo.Marshallers, info);
        ...
        )
        {
            ...
            TypeSyntax nativeTypeSyntax = ReplacePlaceholderSyntaxWithUnmanagedTypeSyntax(
                marshallerData.NativeType.Syntax,
                marshalInfo,
                unmanagedElementType
            );
        ...
        )
        {
            ...
            if (
                (info.RefKind == RefKind.Out || info.IsManagedReturnPosition)
            ...
            if (
                info.RefKind == RefKind.Ref
                && !marshalInfo.Marshallers.IsDefinedOrDefault(Options.RefMode)
            )
            {
                throw new MarshallingNotSupportedException(info, context)
                {
