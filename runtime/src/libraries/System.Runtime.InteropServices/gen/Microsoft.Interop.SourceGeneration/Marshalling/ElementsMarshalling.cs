...
namespace Microsoft.Interop
{
    ...
    internal abstract class BlittableElementsMarshalling
    {
        ...
        public BlittableElementsMarshalling(
        ...
        protected abstract InvocationExpressionSyntax GetUnmanagedValuesDestination(
            TypePositionInfo info,
            StubCodeContext context
        );
        protected abstract InvocationExpressionSyntax GetManagedValuesSource(
            TypePositionInfo info,
            StubCodeContext context
        );
        protected abstract InvocationExpressionSyntax GetUnmanagedValuesSource(
            TypePositionInfo info,
            StubCodeContext context
        );
        protected abstract InvocationExpressionSyntax GetManagedValuesDestination(
            TypePositionInfo info,
            StubCodeContext context
        );

        protected StatementSyntax GenerateByValueOutMarshalStatement(
            TypePositionInfo info,
            StubCodeContext context
        )
        {
        ...
        protected StatementSyntax GenerateMarshalStatement(
            TypePositionInfo info,
            StubCodeContext context
        )
        {
            ...
            return ExpressionStatement(
                InvocationExpression(
                    ...
                    .AddArgumentListArguments(Argument(destination))
            );
        }

        protected StatementSyntax GenerateByValueOutUnmarshalStatement(
            TypePositionInfo info,
            StubCodeContext context
        )
        {
        ...
        public StatementSyntax GenerateUnmarshalStatement(
            TypePositionInfo info,
            StubCodeContext context
        )
        {
            ...
            return ExpressionStatement(
                InvocationExpression(
                    ...
                    .AddArgumentListArguments(Argument(GetManagedValuesDestination(info, context)))
            );
        }
            ...
            return InvocationExpression(
                MemberAccessExpression(
                    ...
                    GenericName(
                        Identifier("Cast"),
                        TypeArgumentList(
                            SeparatedList(new[] { _unmanagedElementType, _managedElementType })
                        )
                    )
                ),
    ...
    internal abstract class NonBlittableElementsMarshalling
    {
                    ...
                    VariableDeclaration(
                        ...
                        SingletonSeparatedList(
                            VariableDeclarator(Identifier(managedSpanIdentifier))
                                .WithInitializer(
                                    EqualsValueClause(GetManagedValuesSource(info, context))
                                )
                        )
                    )
                ),
                    ...
                    VariableDeclaration(
                        ...
                        SingletonSeparatedList(
                            VariableDeclarator(Identifier(nativeSpanIdentifier))
                                .WithInitializer(
                                    EqualsValueClause(GetUnmanagedValuesDestination(info, context))
                                )
                        )
                    )
                ),
                ...
                LocalDeclarationStatement(
                    VariableDeclaration(
                        ...
                        SingletonSeparatedList(
                            VariableDeclarator(Identifier(nativeSpanIdentifier))
                                .WithInitializer(
                                    EqualsValueClause(GetUnmanagedValuesSource(info, context))
                                )
                        )
                    )
                ),
                LocalDeclarationStatement(
                    VariableDeclaration(
                        ...
                        SingletonSeparatedList(
                            VariableDeclarator(Identifier(managedSpanIdentifier))
                                .WithInitializer(
                                    EqualsValueClause(GetManagedValuesDestination(info, context))
                                )
                        )
                    )
                ),
        ...
        )
        {
            ...
            // Span<TUnmanagedElement> <nativeSpan> = <GetUnmanagedValuesDestination>
            string nativeSpanIdentifier = MarshallerHelpers.GetNativeSpanIdentifier(info, context);
            LocalDeclarationStatementSyntax unmanagedValuesDeclaration = LocalDeclarationStatement(
                VariableDeclaration(
                    GenericName(
                        Identifier(TypeNames.System_Span),
                        TypeArgumentList(SingletonSeparatedList(_unmanagedElementType))
        ...
        protected StatementSyntax GenerateElementCleanupStatement(
            TypePositionInfo info,
            StubCodeContext context
        )
        {
        ...
        )
        {
            ...
            foreach (StubCodeContext.Stage stage in stagesToGeneratePerElement)
            {
                ...
                elementStatements.AddRange(
                    _elementMarshaller.Generate(localElementInfo, elementSubContext)
                );
            }
            ...
            if (elementStatements.Any())
            {
                ...
                if (
                    _elementMarshaller.AsNativeType(_elementInfo).Syntax
                ...
                return MarshallerHelpers
                    .GetForLoop(lengthExpression, elementSetupSubContext.IndexerIdentifier)
