...
#pragma warning restore SA1400 // Access modifier should be declared
    {
        public static StatementSyntax GetNumElementsAssignmentFromManagedValuesSource(
            this IElementsMarshallingCollectionSource source,
            TypePositionInfo info,
            StubCodeContext context
        )
        {
        ...
        public static StatementSyntax GetNumElementsAssignmentFromManagedValuesDestination(
            this IElementsMarshallingCollectionSource source,
            TypePositionInfo info,
            StubCodeContext context
        )
    ...
    internal sealed class NonBlittableElementsMarshalling : ElementsMarshalling
    {
        ...
        )
        {
            ...
            var setNumElements = CollectionSource.GetNumElementsAssignmentFromManagedValuesSource(
                info,
                context
            );
            ...
            LocalDeclarationStatementSyntax managedValuesDeclaration = Declare(
                SpanOf(_elementInfo.ManagedType.Syntax),
                managedSpanIdentifier,
                MethodInvocation(
                    TypeSyntaxes.System_Runtime_InteropServices_MemoryMarshal,
        ...
        )
        {
            ...
            {
                indexConstraintName = MemberAccessExpression(
                    SyntaxKind.SimpleMemberAccessExpression,
                    IdentifierName(nativeSpanIdentifier),
                    IdentifierName("Length")
                );
            ...
            else
            {
                indexConstraintName = IdentifierName(
                    MarshallerHelpers.GetLastIndexMarshalledIdentifier(info, context)
                );
            }
        ...
        )
        {
            ...
            var setNumElements =
                CollectionSource.GetNumElementsAssignmentFromManagedValuesDestination(
                    info,
                    context
                );
            ...
            LocalDeclarationStatementSyntax managedValuesDestination = LocalDeclarationStatement(
                VariableDeclaration(
                    GenericName(
            ...
            if (
                ...
                && info.ByValueContentsMarshalKind is ByValueContentsMarshalKind.Out
            )
            {
                stagesToGenerate = new[]
                {
                    StubCodeContext.Stage.Marshal,
        ...
        )
        {
            ...
            foreach (StubCodeContext.Stage stage in stagesToGeneratePerElement)
            {
                ...
                elementStatements.AddRange(
                    elementMarshaller.Generate(localElementInfo, elementSubContext)
                );
            }
        ...
        {
            var elementStatements = GenerateElementStages(
                info,
                context,
                elementMarshaller,
                elementInfo,
                out var elementSetupSubContext,
                out var localElementInfo,
            ...
            if (elementStatements.Count != 0)
            {
                ...
                if (
                    elementMarshaller.AsNativeType(elementInfo).Syntax
                ...
                if (
                    ...
                    && stagesToGeneratePerElement.Contains(StubCodeContext.Stage.Marshal)
                )
                {
        ...
        private static bool ShouldCleanUpAllElements(TypePositionInfo info, StubCodeContext context)
        {
            ...
            return !context.AdditionalTemporaryStateLivesAcrossStages
                ...
                ? LocalDeclarationStatement(
                    VariableDeclaration(
                        ...
                        SingletonSeparatedList(
                            VariableDeclarator(
                                Identifier(
                                    MarshallerHelpers.GetLastIndexMarshalledIdentifier(
                                        info,
                                        context
                                    )
                                ),
