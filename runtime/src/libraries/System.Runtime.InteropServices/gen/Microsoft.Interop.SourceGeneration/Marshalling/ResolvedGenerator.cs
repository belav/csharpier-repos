...
{
    public record struct ResolvedGenerator(
    ...
    )
    {
        ...
        public bool ResolvedSuccessfully =>
            _resolvedSuccessfully ??= Diagnostics.All(d => !d.IsFatal);
        ...
        public static ResolvedGenerator NotSupported(
            GeneratorDiagnostic.NotSupported notSupportedDiagnostic
        )
        {
            return new(
                s_forwarder,
                ImmutableArray.Create<GeneratorDiagnostic>(notSupportedDiagnostic)
            );
        }
