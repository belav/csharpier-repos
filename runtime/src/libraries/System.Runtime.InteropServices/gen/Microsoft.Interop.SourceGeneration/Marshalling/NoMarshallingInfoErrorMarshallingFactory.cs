...
namespace Microsoft.Interop
{
    public sealed class NoMarshallingInfoErrorMarshallingFactory : IMarshallingGeneratorFactory
    {
        ...
        public ResolvedGenerator Create(TypePositionInfo info, StubCodeContext context)
        {
            ...
            )
            {
                return ResolvedGenerator.NotSupported(
                    new(info, context) { NotSupportedDetails = errorMessage }
                );
            }
            return _inner.Create(info, context);
        }

        public NoMarshallingInfoErrorMarshallingFactory(
        ...
        private static ImmutableDictionary<ManagedTypeInfo, string> DefaultTypeToErrorMessageMap(
            string stringMarshallingAttribute
        ) =>
