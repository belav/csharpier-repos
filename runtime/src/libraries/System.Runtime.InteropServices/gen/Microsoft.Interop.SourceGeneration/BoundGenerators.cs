...
namespace Microsoft.Interop
{
    ...
    {
        public BoundGenerators(
            ImmutableArray<TypePositionInfo> elementTypeInfo,
            Func<TypePositionInfo, IMarshallingGenerator> generatorFactoryCallback
        )
        {
            ImmutableArray<BoundGenerator>.Builder allMarshallers =
                ImmutableArray.CreateBuilder<BoundGenerator>();
            ImmutableArray<BoundGenerator>.Builder paramMarshallers =
                ImmutableArray.CreateBuilder<BoundGenerator>();
            bool foundNativeRetMarshaller = false;
            bool foundManagedRetMarshaller = false;
            NativeReturnMarshaller = new(
                new TypePositionInfo(SpecialTypeInfo.Void, NoMarshallingInfo.Instance),
                new Forwarder()
            );
            ManagedReturnMarshaller = new(
                new TypePositionInfo(SpecialTypeInfo.Void, NoMarshallingInfo.Instance),
                new Forwarder()
            );
            ...
            foreach (TypePositionInfo argType in elementTypeInfo)
            {
                BoundGenerator generator = new BoundGenerator(
                    argType,
                    generatorFactoryCallback(argType)
                );
            ...
            AllMarshallers = MarshallerHelpers
                ...
                ParameterList(
                    SeparatedList(
                        NativeParameterMarshallers.Select(
                            marshaler => marshaler.Generator.AsParameter(marshaler.TypeInfo)
                        )
                    )
                ),
