...
namespace Microsoft.Interop
{
    ...
    public sealed class BoundGenerators
    {
        ...
        public static BoundGenerators Create(
            ImmutableArray<TypePositionInfo> elementTypeInfo,
            IMarshallingGeneratorFactory generatorFactory,
            StubCodeContext context,
            IMarshallingGenerator fallbackGenerator,
            out ImmutableArray<GeneratorDiagnostic> generatorBindingDiagnostics
        )
        {
            BoundGenerator defaultBoundGenerator = new BoundGenerator(
                new TypePositionInfo(SpecialTypeInfo.Void, NoMarshallingInfo.Instance),
                fallbackGenerator
            );
            BoundGenerators result = new();

            ImmutableArray<BoundGenerator>.Builder signatureMarshallers =
                ImmutableArray.CreateBuilder<BoundGenerator>();
            ImmutableArray<BoundGenerator>.Builder nativeParamMarshallers =
                ImmutableArray.CreateBuilder<BoundGenerator>();
            ImmutableArray<BoundGenerator>.Builder managedParamMarshallers =
                ImmutableArray.CreateBuilder<BoundGenerator>();
            ImmutableArray<GeneratorDiagnostic>.Builder generatorDiagnostics =
                ImmutableArray.CreateBuilder<GeneratorDiagnostic>();
            ...
            foreach (TypePositionInfo argType in elementTypeInfo)
            {
                ...
                BoundGenerator generator = new BoundGenerator(
                    argType,
                    CreateGenerator(argType, generatorFactory)
                );
            ...
            nativeParamMarshallers.Sort(
                static (m1, m2) => m1.TypeInfo.NativeIndex.CompareTo(m2.TypeInfo.NativeIndex)
            );
            ...
            if (managedExceptionInfo is not null)
            {
                ...
                managedExceptionMarshaller = new(
                    managedExceptionInfo,
                    CreateGenerator(managedExceptionInfo, generatorFactory)
                );
            }
            ...
            )
            {
                ...
                return generator.ResolvedSuccessfully ? generator.Generator : fallbackGenerator;
            }
        }
        ...
        ) GenerateTargetMethodSignatureData(StubCodeContext context)
        {
            return (
                ParameterList(
                    SeparatedList(
                        NativeParameterMarshallers.Select(marshaler =>
                            marshaler.Generator.AsParameter(marshaler.TypeInfo, context)
                        )
                    )
                ),
        ...
        public bool ManagedNativeSameReturn =>
            ManagedReturnMarshaller.TypeInfo.IsNativeReturnPosition;

        public bool IsManagedVoidReturn =>
            ManagedReturnMarshaller.TypeInfo.ManagedType == SpecialTypeInfo.Void;

        public bool IsUnmanagedVoidReturn =>
            NativeReturnMarshaller.TypeInfo.ManagedType == SpecialTypeInfo.Void;

        public bool HasManagedExceptionMarshaller =>
            ManagedExceptionMarshaller.Generator is not Forwarder;
        ...
        private sealed class ExtendedInvariantsValidator : IMarshallingGeneratorFactory
        {
            ...
            public ExtendedInvariantsValidator(
                ManagedTypeInfo nativeReturnType,
            ...
            public ResolvedGenerator Create(TypePositionInfo info, StubCodeContext context)
            {
                ...
                if (!generator.ResolvedSuccessfully)
                {
                    return generator;
                }
