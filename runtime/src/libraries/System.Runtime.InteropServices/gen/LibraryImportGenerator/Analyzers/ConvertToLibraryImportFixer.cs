...
namespace Microsoft.Interop.Analyzers
{
    ...
    public sealed class ConvertToLibraryImportFixer : ConvertToSourceGeneratedInteropFixer
    {
        ...
        {
            bool allowUnsafe =
                selectedOptions.TryGetValue(Option.AllowUnsafe, out Option? allowUnsafeOption)
                && allowUnsafeOption is Option.Bool(true);
            string? suffix = null;
            bool hasSuffix = false;
            if (
                selectedOptions.TryGetValue(SelectedSuffixOption, out Option? suffixOption)
                && suffixOption is Option.String(string suffixValue)
            )
            {
                hasSuffix = true;
                suffix = suffixValue;
        ...
        )
        {
            ...
            if (!bool.Parse(diagnostic.Properties[ConvertToLibraryImportAnalyzer.ExactSpelling]))
            {
                optionsBuilder.Add(
                    CharSetOption,
                    new Option.String(diagnostic.Properties[ConvertToLibraryImportAnalyzer.CharSet])
                );
            }
            return optionsBuilder.ToImmutable();
        }

        protected override IEnumerable<ConvertToSourceGeneratedInteropFix> CreateAllFixesForDiagnosticOptions(
            SyntaxNode node,
        ...
        )
        {
            bool warnForAdditionalWork =
                ...
                ) && mayRequireAdditionalWork is Option.Bool(true);

            CharSet? charSet =
                options.TryGetValue(CharSetOption, out Option charSetOption)
                && charSetOption is Option.String(string charSetString)
                && Enum.TryParse<CharSet>(charSetString, out CharSet result)
                    ? result
                    : null;
            ...
            yield return new ConvertToSourceGeneratedInteropFix(
                (editor, ct) =>
                    ConvertToLibraryImport(editor, node, warnForAdditionalWork, null, ct),
                selectedOptions
            );
            ...
            if (charSet is not null)
            {
                ...
                if (charSet is CharSet.None or CharSet.Ansi or CharSet.Auto)
                {
                    yield return new ConvertToSourceGeneratedInteropFix(
                        (editor, ct) =>
                            ConvertToLibraryImport(editor, node, warnForAdditionalWork, 'A', ct),
                ...
                if (charSet is CharSet.Unicode or CharSet.Auto)
                {
                    yield return new ConvertToSourceGeneratedInteropFix(
                        (editor, ct) =>
                            ConvertToLibraryImport(editor, node, warnForAdditionalWork, 'W', ct),
        ...
        {
            bool warnForAdditionalWork =
                selectedOptions.TryGetValue(
                    Option.MayRequireAdditionalWork,
                    out Option mayRequireAdditionalWork
                ) && mayRequireAdditionalWork is Option.Bool(true);
            char? suffix =
                selectedOptions.TryGetValue(SelectedSuffixOption, out Option selectedSuffixOption)
                && selectedSuffixOption is Option.String(string selectedSuffix)
                    ? selectedSuffix[0]
                    : null;
            return (editor, ct) =>
                ConvertToLibraryImport(editor, node, warnForAdditionalWork, suffix, ct);
        }
        ...
        )
        {
            ...
            if (
                !methodSymbol.MethodImplementationFlags
        ...
        {
            INamedTypeSymbol? dllImportAttrType = editor.SemanticModel
                .Compilation
            ...
            INamedTypeSymbol libraryImportAttrType = editor.SemanticModel
                .Compilation
            ...
            generatedDeclaration = AddExplicitDefaultBoolMarshalling(
                generator,
                methodSymbol,
                generatedDeclaration,
                "Bool"
            ...
            foreach (SyntaxNode argument in generator.GetAttributeArguments(libraryImportSyntax))
            {
                ...
                else if (IsMatchingNamedArg(attrArg, nameof(DllImportAttribute.CharSet)))
                {
                    if (MethodRequiresStringMarshalling(methodSymbol))
                    {
                        ...
                        {
                            ITypeSymbol stringMarshallingType = editor.SemanticModel
                                .Compilation
                ...
                else if (IsMatchingNamedArg(attrArg, nameof(DllImportAttribute.EntryPoint)))
                {
                    ...
                    if (!dllImportData.ExactSpelling && entryPointSuffix.HasValue)
                    {
                        ...
                        {
                            if (
                                dllImportData.EntryPointName!.LastOrDefault()
                                != entryPointSuffix.Value
                            )
                            {
                                argumentsToRemove.Add(attrArg);
                                argumentsToAdd.Add(
        ...
        {
            AttributeArgumentListSyntax updatedArgList = attribute.ArgumentList
                .WithArguments(
