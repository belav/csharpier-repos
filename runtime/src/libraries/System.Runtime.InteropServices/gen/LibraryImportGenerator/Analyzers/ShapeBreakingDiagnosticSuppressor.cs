...
namespace Microsoft.Interop.Analyzers
{
    ...
    {
        public static readonly SuppressionDescriptor MarkMethodsAsStaticSuppression =
            new SuppressionDescriptor(
                "SYSLIBSUPPRESS0001",
                "CA1822",
                "Do not offer to make methods static when the methods need to be instance methods for a custom marshaller shape."
            );
        ...
        )
        {
            ...
            if (
                FindContainingEntryPointTypeAndManagedType(diagnosedSymbol.ContainingType) is

                (INamedTypeSymbol entryPointMarshallerType, INamedTypeSymbol managedType)
            )
            {
                bool isLinearCollectionMarshaller =
                    ManualTypeMarshallingHelper.IsLinearCollectionEntryPoint(
                        entryPointMarshallerType
                    );
                (MarshallerShape _, StatefulMarshallerShapeHelper.MarshallerMethods methods) =
                    StatefulMarshallerShapeHelper.GetShapeForType(
                        diagnosedSymbol.ContainingType,
                        managedType,
                ...
                if (methods.IsShapeMethod((IMethodSymbol)diagnosedSymbol))
                {
                    ...
                    context.ReportSuppression(
                        Suppression.Create(MarkMethodsAsStaticSuppression, diagnostic)
                    );
                }
            }
        }
        ...
        )? FindContainingEntryPointTypeAndManagedType(INamedTypeSymbol marshallerType)
        {
            for (
                INamedTypeSymbol containingType = marshallerType;
                containingType is not null;
                containingType = containingType.ContainingType
            )
            {
                AttributeData? attrData = containingType
                    .GetAttributes()
                    .FirstOrDefault(attr =>
                        ...
                        && attr.ConstructorArguments[2].Value
                            is INamedTypeSymbol marshallerTypeInAttribute
