...
namespace Microsoft.Interop.Analyzers
{
    ...
    public class CustomTypeMarshallerFixer : CodeFixProvider
    {
        ...
        private const string AddMissingCustomTypeMarshallerFeaturesKey = nameof(
        ...
        private sealed class CustomFixAllProvider : DocumentBasedFixAllProvider
        {
                ...
                switch (fixAllContext.CodeActionEquivalenceKey)
                {
                    case AddMissingCustomTypeMarshallerMembersKey:
                        foreach (
                            IGrouping<
                                TextSpan,
                                Diagnostic
                            > diagnosticsBySpan in diagnostics.GroupBy(d => d.Location.SourceSpan)
                        )
                        {
                    ...
                    case AddMissingCustomTypeMarshallerFeaturesKey:
                        ...
                        )
                        {
                            ...
                            await AddMissingFeatures(fixAllContext, editor, diagnosticsBySpan, node)
                                .ConfigureAwait(false);
                        }
                        break;
                    default:
            ...
            )
            {
                ...
                ITypeSymbol marshallerType = (ITypeSymbol)
                    editor.SemanticModel.GetDeclaredSymbol(node);
                editor.ReplaceNode(
                    node,
                    (node, gen) =>
            ...
            private static async Task AddMissingFeatures(
                FixAllContext fixAllContext,
                DocumentEditor editor,
                IEnumerable<Diagnostic> diagnostics,
                SyntaxNode node
            )
            {
                ...
                ITypeSymbol marshallerType = (ITypeSymbol)
                    editor.SemanticModel.GetDeclaredSymbol(node);
                AttributeData customTypeMarshallerAttribute = marshallerType
                    .GetAttributes()
                    .FirstOrDefault(
                        attr =>
        ...
        public override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            if (
                featuresToAddDiagnostics.Count > 0
                && featuresToAdd != CustomTypeMarshallerFeatures.None
            )
            {
                context.RegisterCodeFix(
                    CodeAction.Create(
        ...
        private static (List<string> missingMembers, List<Diagnostic> fixedDiagnostics) GetRequiredShapeMissingMemberNames(
            IEnumerable<Diagnostic> diagnostics
        )
        {
                ...
                )
                {
                    ...
                    )
                    {
                        missingMemberNames.AddRange(
                            missingMembers.Split(
                                CustomTypeMarshallerAnalyzer.MissingMemberNames.Delimiter
                            )
                        );
                    }
                }
        ...
        private static (CustomTypeMarshallerFeatures featuresToAdd, List<Diagnostic> fixedDiagnostics) GetFeaturesToAdd(
            IEnumerable<Diagnostic> diagnostics
        )
        {
        ...
        private static async Task<Document> AddMissingFeatures(
            Document doc,
            SyntaxNode node,
            CustomTypeMarshallerFeatures featuresToAdd,
        ...
        private static SyntaxNode AddMissingFeatures(
            string attributeName,
            AttributeData? customTypeMarshallerAttribute,
            CustomTypeMarshallerFeatures featuresToAdd,
            SyntaxGenerator gen
        )
        {
                    ...
                    .Where(
                        (a, i) =>
                        {
                            if (a.Key == "Features")
                            {
                                ...
                                featuresArgLocation =
                                    customTypeMarshallerAttribute.ConstructorArguments.Length + i;
            ...
            newAttributeSyntax = featuresArgLocation is null
                ...
                : gen.InsertAttributeArguments(
                    newAttributeSyntax,
                    featuresArgLocation.Value,
                    new[] { featureAttributeArgument }
                );

            return newAttributeSyntax;
        }

        private static async Task<Document> AddMissingMembers(
            Document doc,
            SyntaxNode node,
            List<string> missingMemberNames,
            CancellationToken ct
        )
        {
            var editor = await DocumentEditor.CreateAsync(doc, ct).ConfigureAwait(false);
            ...
            SyntaxNode updatedDeclaration = AddMissingMembers(
                node,
                (ITypeSymbol)editor.SemanticModel.GetDeclaredSymbol(node, ct),
                missingMemberNames,
                editor.SemanticModel.Compilation,
                gen
            );
        ...
        private static SyntaxNode AddMissingMembers(
            SyntaxNode node,
        ...
        )
        {
            ...
            (_, ITypeSymbol managedType, _) = ManualTypeMarshallingHelper.GetMarshallerShapeInfo(
                marshallerType
            );
            ...
            {
                gen.ThrowStatement(
                    gen.ObjectCreationExpression(gen.DottedName("System.NotImplementedException"))
                )
            };
            ...
            foreach (string missingMemberName in missingMemberNames)
            {
                ...
                {
                    case CustomTypeMarshallerAnalyzer
                        .MissingMemberNames
                        .ValueManagedToNativeConstructor:
                        updatedDeclaration = gen.AddMembers(
                            updatedDeclaration,
                            gen.ConstructorDeclaration(
                                ...
                                new[]
                                {
                                    gen.ParameterDeclaration(
                                        "managed",
                                        type: gen.TypeExpression(managedType)
                                    )
                                },
                        ...
                        updatedDeclaration = gen.AddMembers(
                            ...
                            gen.ConstructorDeclaration(
                                ...
                                new[]
                                {
                                    ...
                                    gen.ParameterDeclaration(
                                        "buffer",
                                        type: gen.TypeExpression(spanOfByte)
                                    )
                                },
                                accessibility: Accessibility.Public,
                                statements: throwNotImplementedStatements
                        ...
                        updatedDeclaration = gen.AddMembers(
                            ...
                            gen.MethodDeclaration(
                                ...
                                parameters: new[]
                                {
                                    gen.ParameterDeclaration(
                                        "value",
                                        type: gen.TypeExpression(
                                            toNativeValueMethod?.ReturnType ?? @byte
                                        )
                                    )
                                },
                                accessibility: Accessibility.Public,
                        ...
                        updatedDeclaration = gen.AddMembers(
                            ...
                            gen.MethodDeclaration(
                                ...
                                returnType: gen.TypeExpression(
                                    fromNativeValueMethod?.Parameters[0].Type ?? @byte
                                ),
                                accessibility: Accessibility.Public,
                                statements: throwNotImplementedStatements
                            )
                    ...
                    case ShapeMemberNames.LinearCollection.GetManagedValuesSource:
                        INamedTypeSymbol? getManagedValuesDestinationReturnType =
                            ...
                            gen.MethodDeclaration(
                                ...
                                returnType: gen.TypeExpression(
                                    readOnlySpanOfT.Construct(
                                        getManagedValuesDestinationReturnType?.TypeArguments[0]
                                            ?? @object
                                    )
                                ),
                                accessibility: Accessibility.Public,
                    ...
                    case ShapeMemberNames.LinearCollection.GetManagedValuesDestination:
                        INamedTypeSymbol? getManagedValuesSourceReturnType = (INamedTypeSymbol?)
                            getManagedValuesSourceMethod?.ReturnType;
                        updatedDeclaration = gen.AddMembers(
                            updatedDeclaration,
                            gen.MethodDeclaration(
                                ShapeMemberNames.LinearCollection.GetNativeValuesDestination,
                                ...
                                returnType: gen.TypeExpression(
                                    spanOfT.Construct(
                                        getManagedValuesSourceReturnType?.TypeArguments[0]
                                            ?? @object
                                    )
                                ),
                                accessibility: Accessibility.Public,
