    ...
    public sealed class LibraryImportGenerator : IIncrementalGenerator
    {
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            var methodsWithDiagnostics = attributedMethods.Select(
                static (data, ct) =>
                {
                    DiagnosticInfo? diagnostic = GetDiagnosticIfInvalidMethodForGeneration(
                        data.Syntax,
                        data.Symbol
                    );
                    return diagnostic is not null
                        ? DiagnosticOr<(MethodDeclarationSyntax Syntax, IMethodSymbol Symbol)>.From(
                            diagnostic
                        )
                        : DiagnosticOr<(MethodDeclarationSyntax Syntax, IMethodSymbol Symbol)>.From(
            ...
            context.RegisterDiagnostics(
                context
                    ...
                    .SelectMany(
                        ...
                        {
                            if (
                                data.Left.Right.IsEmpty // no attributed methods
                                || data.Left.Left.Options
                                    is CSharpCompilationOptions { AllowUnsafe: true } // Unsafe code enabled
                                || data.Right.TargetFramework != TargetFramework.Net
                            ) // Downlevel scenarios use forwarders and don't need unsafe code
                            {
            ...
            IncrementalValuesProvider<(
                ...
                .Combine(stubOptions)
                .Select(static (data, ct) => GenerateSource(data.Left, data.Right))
        ...
        private static List<AttributeSyntax> GenerateSyntaxForForwardedAttributes(
            ...
            AttributeData? defaultDllImportSearchPathsAttribute
        )
        {
            ...
            if (unmanagedCallConvAttribute is not null)
            {
                AttributeSyntax unmanagedCallConvSyntax = Attribute(
                    NameSyntaxes.UnmanagedCallConvAttribute
                );
                foreach (
                    KeyValuePair<
                        string,
            ...
            {
                attributes.Add(
                    Attribute(NameSyntaxes.DefaultDllImportSearchPathsAttribute)
                        .AddArgumentListArguments(
                            AttributeArgument(
                                CastExpression(
                                    TypeSyntaxes.DllImportSearchPath,
                                    LiteralExpression(
        ...
        private static LibraryImportCompilationData? ProcessLibraryImportAttribute(
        ...
        )
        {
            ...
            if (
                namedArguments.TryGetValue(
                    nameof(LibraryImportCompilationData.EntryPoint),
            ...
            return new LibraryImportCompilationData(
        ...
        )
        {
            ...
            INamedTypeSymbol? suppressGCTransitionAttrType =
                environment.SuppressGCTransitionAttrType;
            ...
            if (libraryImportData.IsUserDefined.HasFlag(InteropAttributeMember.StringMarshalling))
            {
                ...
                if (
                    libraryImportData.StringMarshalling == StringMarshalling.Custom
                    && libraryImportData.StringMarshallingCustomType is null
                )
                {
                ...
                if (
                    libraryImportData.StringMarshalling != StringMarshalling.Custom
                    && libraryImportData.StringMarshallingCustomType is not null
                )
                {
                    generatorDiagnostics.ReportInvalidStringMarshallingConfiguration(
                        generatedDllImportAttr,
            ...
            var signatureContext = SignatureContext.Create(
                ...
                environment,
                new CodeEmitOptions(
                    SkipInit: targetFramework.TargetFramework == TargetFramework.Net
                ),
            ...
            return new IncrementalStubGenerationContext(
                ...
                locations,
                new SequenceEqualImmutableArray<AttributeSyntax>(
        ...
        )
        {
            var diagnostics = new GeneratorDiagnosticsBag(
                new DiagnosticDescriptorProvider(),
                pinvokeStub.DiagnosticLocation,
                SR.ResourceManager,
                typeof(FxResources.Microsoft.Interop.LibraryImportGenerator.SR)
            );
            ...
            foreach (
                TypePositionInfo typeInfo in pinvokeStub.SignatureContext.ElementTypeInformation
            )
            {
            ...
            var stubGenerator = new PInvokeStubCodeGenerator(
                ...
                diagnostics,
                LibraryImportGeneratorHelpers.CreateGeneratorFactory(
                    pinvokeStub.TargetFramework,
                    pinvokeStub.Options,
                    pinvokeStub.EnvironmentFlags
                ),
                new CodeEmitOptions(
                    SkipInit: pinvokeStub.TargetFramework is (TargetFramework.Net, _)
                )
            );
            ...
            if (stubGenerator.StubIsBasicForwarder || !supportsTargetFramework)
            {
                return (
                    PrintForwarderStub(
                        pinvokeStub.StubMethodSyntaxTemplate,
        ...
        private static MemberDeclarationSyntax PrintForwarderStub(
            ContainingSyntax userDeclaredMethod,
            bool explicitForwarding,
            IncrementalStubGenerationContext stub,
            GeneratorDiagnosticsBag diagnostics
        )
        {
            LibraryImportData pinvokeData = stub.LibraryImportData with
            {
                EntryPoint =
                    stub.LibraryImportData.EntryPoint ?? userDeclaredMethod.Identifier.ValueText,
            };

            if (
                pinvokeData.IsUserDefined.HasFlag(InteropAttributeMember.StringMarshalling)
            ...
            )
            {
                ...
                pinvokeData = pinvokeData with
                {
                    IsUserDefined =
                        pinvokeData.IsUserDefined & ~InteropAttributeMember.StringMarshalling,
                };
            }
        ...
        )
        {
            Debug.Assert(
                !options.GenerateForwarders,
                "GenerateForwarders should have already been handled to use a forwarder stub"
            );
        ...
        )
        {
            ...
            if (
                methodSyntax.Parent is TypeDeclarationSyntax typeDecl
                && !typeDecl.IsInPartialContext(out var nonPartialIdentifier)
            )
            {
                return DiagnosticInfo.Create(
