...
namespace System.Net.Tests
{
    public class HttpWebResponseHeaderTest
    {
            ...
            await LoopbackServer.CreateServerAsync(
                async (server, url) =>
                {
                    ...
                    using (WebResponse response = await getResponse)
                    {
                        ...
                        Assert.Equal(
                            5,
                            int.Parse(httpResponse.GetResponseHeader("Content-Length"))
                        );
        ...
        {
            DateTime expected = TimeZoneInfo.ConvertTimeFromUtc(
                new DateTime(2018, 4, 10, 3, 4, 5, DateTimeKind.Utc),
                TimeZoneInfo.Local
            );
            await LoopbackServer.CreateServerAsync(
                async (server, url) =>
                {
                    ...
                    await server.AcceptConnectionSendResponseAndCloseAsync(
                        HttpStatusCode.OK,
                        "Last-Modified: Tue, 10 Apr 2018 03:04:05 GMT\r\n",
                        "12345"
                    );
