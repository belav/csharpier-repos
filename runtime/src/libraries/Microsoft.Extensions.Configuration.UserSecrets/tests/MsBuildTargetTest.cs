...
namespace Microsoft.Extensions.Configuration.UserSecrets
{
    public class MsBuildTargetTest : IDisposable
    {
        ...
        {
            var testTfm =
                typeof(MsBuildTargetTest).Assembly.GetCustomAttributes<AssemblyMetadataAttribute>()
                    .First(f => f.Key == "TargetFramework").Value;
            var runtimeVersion =
                typeof(MsBuildTargetTest).Assembly.GetCustomAttributes<AssemblyMetadataAttribute>()
                    .First(f => f.Key == "MicrosoftNETCoreAppRuntimeVersion").Value;
            var refPackVersion =
                typeof(MsBuildTargetTest).Assembly.GetCustomAttributes<AssemblyMetadataAttribute>()
                    .First(f => f.Key == "MicrosoftNETCoreAppRefPackageVersion").Value;
            ...
            File.Copy(
                ...
                Path.Combine(_tempDir, libName)
            );
            File.Copy(
                target,
                Path.Combine(_tempDir, "obj", $"test{projectExt}.usersecretstest.targets")
            ); // imitates how NuGet will import this target
...
"
            ...
            switch (projectExt)
            {
                case ".csproj":
                    File.WriteAllText(
                        Path.Combine(_tempDir, "Program.cs"),
                        "public class Program { public static void Main(){}}"
                    );
                    break;
                case ".fsproj":
                    File.WriteAllText(
                        Path.Combine(_tempDir, "Program.fs"),
                        @"
...
"
            ...
            var assemblyInfoFile = Path.Combine(
                _tempDir,
                $"obj/Debug/{testTfm}/test.AssemblyInfo" + sourceExt
            );

            AssertDotNet("restore");

            Assert.False(
                File.Exists(assemblyInfoFile),
                $"{assemblyInfoFile} should not exist but does"
            );
            ...
            Assert.True(
                File.Exists(assemblyInfoFile),
                $"{assemblyInfoFile} should exist but does not"
            );
            var contents = File.ReadAllText(assemblyInfoFile);
            Assert.Contains(
                "assembly: Microsoft.Extensions.Configuration.UserSecrets.UserSecretsIdAttribute(\"xyz123\")",
                contents
            );
