    ...
    internal sealed partial class DtdParser : IDtdParser
    {
        ...
        private async Task ParseExternalSubsetAsync()
        {
            ...
            if (
                !await _readerAdapter
                    .PushExternalSubsetAsync(_systemId, _publicId)
                    .ConfigureAwait(false)
            )
            {
                return;
                            ...
                            )
                            {
                                ...
                                if (_validate)
                                {
                                    attrDef.CheckXmlSpace(
                                        _readerAdapterWithValidation!.ValidationEventHandling!
                                    );
                                }
                            }
                        }
                    ...
                    else
                    {
                        ...
                        )
                        {
                            SendValidationEvent(
                                _curPos - 8,
                                XmlSeverityType.Error,
                                SR.Sch_NotationAttributeOnEmptyElement,
                                elementDecl.Name.ToString()
                            ); // 8 == strlen("NOTATION")
                        }
                ...
                while (true)
                {
                    ...
                    )
                    {
                        SendValidationEvent(
                            XmlSeverityType.Error,
                            new XmlSchemaException(
                                SR.Xml_AttlistDuplNotationValue,
                                notationName,
                                BaseUriStr,
                                (int)LineNo,
                                (int)LinePos
        ...
        private async Task ParseElementDeclAsync()
        {
            ...
            if (_schemaInfo.ElementDecls.TryGetValue(name, out elementDecl))
            {
                ...
                {
                    SendValidationEvent(
                        _curPos - name.Name.Length,
                        XmlSeverityType.Error,
                        SR.Sch_DupElementDecl,
                        GetNameString()
                    );
                }
            }
        ...
        private async Task ParseEntityDeclAsync()
        {
            ...
            switch (await GetTokenAsync(true).ConfigureAwait(false))
            {
                ...
                case Token.Name:
                    ...
                    entity.DeclaredURI =
                        (_externalDtdBaseUri.Length == 0) ? _documentBaseUri : _externalDtdBaseUri;
        ...
        private async Task ParseNotationDeclAsync()
        {
            ...
            else
            {
                ...
                {
                    SendValidationEvent(
                        _curPos - notationName.Name.Length,
                        XmlSeverityType.Error,
                        SR.Sch_DupNotation,
                        notationName.Name
                    );
        ...
        private async Task ParseCommentAsync()
        {
            ...
            try
            {
                if (SaveInternalSubsetValue)
                {
                    ...
                    await _readerAdapter
                        .ParseCommentAsync(_internalSubsetValueSb)
                        .ConfigureAwait(false);
        ...
        private async Task<(string?, string?)> ParseExternalIdAsync(
            Token idTokenType,
            Token declType
        )
        {
                    ...
                    if (await GetTokenAsync(false).ConfigureAwait(false) == Token.Literal)
                    {
                        if (!_whitespaceSeen)
                        {
                            Throw(
                                ...
                                char.ToString(_literalQuoteChar),
                                (int)_literalLineInfo.lineNo,
                                (int)_literalLineInfo.linePos
                            );
                        }
                        systemId = GetValue();
                        _literalLineInfo.linePos++;
            ...
            while (true)
            {
                ...
                switch (_chars[_curPos])
                {
                    ...
                    case '&':
                        ...
                        {
                            SaveParsingBuffer();
                            int endPos = await _readerAdapter
                ...
                else
                {
                    ...
                    else
                    {
                        Throw(
                            ...
                            XmlException.BuildCharExceptionArgs(_chars, _charsUsed, _curPos)
                        );
                    }
                }
        ...
        )
        {
            ...
            SchemaEntity? entity = VerifyEntityReference(
                entityName,
                paramEntity,
                true,
                inAttribute
            ...
            {
                Throw(
                    _curPos - entityName.Name.Length - 1,
                    paramEntity ? SR.Xml_RecursiveParEntity : SR.Xml_RecursiveGenEntity,
                    entityName.Name
                );
            }
