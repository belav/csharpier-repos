...
{
    //
    // NOTE!!! LARGE PORTIONS OF THIS FILE ARE AUTOGENERATED BY GENERATECONVERTER.CS.  DO NOT MANUALLY
    // CHANGE ANY CODE
    ...
    // SUMMARY
    // =======
    // For each Xml type, there is a set of Clr types that can represent it.  Some of these mappings
    // involve
    // loss of fidelity.  For example, xsd:dateTime can be represented as System.DateTime, but only at
    // the expense
    // of normalizing the time zone.  And xs:duration can be represented as System.TimeSpan, but only at
    // the expense
    // of discarding variations such as "P50H", "P1D26H", "P2D2H", all of which are normalized as
    // "P2D2H".
    //
    // Implementations of this class convert between the various Clr representations of Xml types.  Note
    // that
    // in *no* case is the Xml type ever modified.  Only the Clr type is changed.  This means that in
    // cases where
    // the Xml type is part of the representation (such as XmlAtomicValue), the destination value is
    // guaranteed
    // to have the same Xml type.
    //
    // For all converters, converting to typeof(object) is identical to converting to
    // XmlSchemaType.Datatype.ValueType.
    //
    //
    // ATOMIC MAPPINGS
    // ===============
    ...
    //                                                          to primitive, full-fidelity Clr type
    // (use
    //                                                          XmlConvert where possible).  Use Clr
    // rules
    ...
    // LIST MAPPINGS
    // =============
    // The following Clr types can be used to represent Xsd list types: IList, ICollection, IEnumerable,
    // Type[],
    // String.
    //
    //
    ...
    //
    // -----------------------------------------------------------------------------------------------------------
    // System.String        No-op conversion                        Tokenize the string by whitespace,
    // create a
    //                                                              String[] from tokens, and follow
    // List => List
    //                                                              rules.
    //
    // -----------------------------------------------------------------------------------------------------------
    // Clr List Type        Follow List => String[] rules,          Create destination list having the
    // same length
    //                      then concatenate strings from array,    as the source list.  For each item
    // in the
    //                      separating adjacent strings with a      source list, call the atomic
    // converter to
    //                      single space character.                 convert to the destination type.
    // The destination
    //                                                              item type for IList, ICollection,
    // IEnumerable
    //                                                              is typeof(object).  The destination
    // item type
    //                                                              for Type[] is Type.
    //
    // -----------------------------------------------------------------------------------------------------------
    ...
    // UNION MAPPINGS
    // ==============
    // Union types may only be represented using System.Xml.Schema.XmlAtomicValue or System.String.
    // Either the
    ...
    //
    // -----------------------------------------------------------------------------------------------------------
    // System.String        No-op conversion        Follow System.String=>  Call ParseValue in order to
    // determine
    //                                              Other Clr Type rules.   the member type.  Call
    // ChangeType on
    //                                                                      the member type's converter
    // to convert
    //                                                                      to desired Clr type.
    //
    // -----------------------------------------------------------------------------------------------------------
    // XmlAtomicValue       Follow XmlAtomicValue   No-op conversion.       Call ReadValueAs, where
    // destinationType
    ...
    //
    // -----------------------------------------------------------------------------------------------------------
    // xs:int     "10"      Byte             "10" => 10M => (byte) 10          Primitive, full-fidelity
    // for xs:int
    //                                                                         is a truncated decimal
    //
    // -----------------------------------------------------------------------------------------------------------
    // xs:int     "10.10"   Byte             FormatException                   xs:integer parsing rules
    // do not
    //                                                                         allow fractional parts
    //
    // -----------------------------------------------------------------------------------------------------------
    // xs:int     10.10M    Byte             10.10M => (byte) 10               Default Clr rules
    // truncate when
    //                                                                         converting from Decimal
    // to Byte
    //
    // -----------------------------------------------------------------------------------------------------------
    // xs:int     10.10M    Decimal          10.10M => 10.10M                  Decimal => Decimal is
    // no-op
    //
    // -----------------------------------------------------------------------------------------------------------
    ...
    //
    // -----------------------------------------------------------------------------------------------------------
    // xs:byte    300       Byte             300 => 300M => OverflowException  Clr overflows when
    // converting from
    //                                                                         Decimal to Byte
    //
    // -----------------------------------------------------------------------------------------------------------
    // xs:byte    300       XmlAtomicValue   new XmlAtomicValue(xs:byte, 300)  Invalid atomic value
    // created
    //
    // -----------------------------------------------------------------------------------------------------------
    // xs:double  1.234f    String          1.234f => 1.2339999675750732d =>   Converting a Single value
    // to a Double
    //                                      "1.2339999675750732"               value zero-extends it in
    // base-2, so
    //                                                                         "garbage" digits appear
    // when it's
    //                                                                         converted to base-10.
    //
    // -----------------------------------------------------------------------------------------------------------
    // xs:int*    {1, "2",  String          {1, "2", 3.1M} =>                  Delegate to xs:int
    // converter to
    //            3.1M}                     {"1", "2", "3"} => "1 2 3"         convert each item to a
    // string.
    //
    // -----------------------------------------------------------------------------------------------------------
    ...
    //
    // -----------------------------------------------------------------------------------------------------------
    // xs:int*    {1, "2",  Object[]        {1, "2", 3.1M} =>                  xs:int converter uses
    // Int32 by default,
    //            3.1M}                     {(object)1, (object)2, (object)3}  so returns boxed Int32
    // values.
    //
    // -----------------------------------------------------------------------------------------------------------
    ...
    //
    // -----------------------------------------------------------------------------------------------------------
    // (xs:int* | "1 2001"  String          "1 2001"                           No-op conversion even
    // though
    // xs:gYear*)                                                              ParseValue would fail if
    // it were called.
    //
    // -----------------------------------------------------------------------------------------------------------
    ...
    internal abstract class XmlBaseConverter : XmlValueConverter
    {
        ...
        protected XmlBaseConverter(XmlSchemaType schemaType)
        {
            ...
            Debug.Assert(
                ...
                "schemaType or schemaType.Datatype may not be null"
            );
            ...
            Debug.Assert(
                schemaType.Datatype!.Variety != XmlSchemaDatatypeVariety.List,
                "schemaType must be list's item type, not list itself"
            );
        ...
        protected XmlBaseConverter(
            XmlBaseConverter converterAtomic,
            Type clrTypeDefault,
            Type clrListTypeDefault
        )
        {
            _schemaType = converterAtomic._schemaType;
            _typeCode = converterAtomic._typeCode;
        ...
        protected string XmlTypeName
        {
            get
            {
                ...
                if (type != null)
                {
                    ...
                    {
                        // Walk base classes until one with a name is found (in worst case, all simple types derive from
                        // xs:anySimpleType)
        ...
        /// <summary>
        /// Return an array type with the element type of the default V1 Clr mapping of this converter's
        // type.
        /// </summary>
        ...
        /// <summary>
        /// Create an InvalidCastException for cases where either "destinationType" or "sourceType" is not a
        // supported CLR representation
        /// for this Xml type.
        /// </summary>
        ...
        /// <summary>
        /// Convert an XmlQualifiedName to a string, using somewhat different rules than
        // XmlQualifiedName.ToString():
        ///   1. Recognize the built-in xs: and xdt: namespaces and print the short prefix rather than the
        // long namespace
        ...
        /// <summary>
        /// This method is called when a valid conversion cannot be found.  By default, this method throws
        // an error.  It can
        /// be overridden in derived classes to support list conversions.
        /// </summary>
        protected virtual object ChangeListType(
            object value,
            Type destinationType,
            IXmlNamespaceResolver? nsResolver
        )
        ...
        {
            // Parse string as DayTimeDuration and convert it to a DayTimeDuration TimeSpan (it is an error to
            // have year and month parts)
        ...
        protected static byte[] StringToHexBinary(string value)
        {
            try
            {
                return XmlConvert.FromBinHexString(
                    value.AsSpan().Trim(XmlConvert.WhitespaceChars),
                    false
                );
            }
        ...
        {
            // Parse string as YearMonthDuration and convert it to a YearMonthDuration TimeSpan (it is an error
            // to have day and time parts)
            ...
            if (value < (decimal)int.MinValue || value > (decimal)int.MaxValue)
                throw new OverflowException(
                    SR.Format(
                        ...
                        new string[] { XmlConvert.ToString(value), "Int32" }
                    )
                );
        ...
        protected static long DecimalToInt64(decimal value)
        {
            if (value < (decimal)long.MinValue || value > (decimal)long.MaxValue)
                throw new OverflowException(
                    SR.Format(
                        SR.XmlConvert_Overflow,
                        new string[] { XmlConvert.ToString(value), "Int64" }
                    )
                );
    ...
    internal sealed class XmlNumeric10Converter : XmlBaseConverter
    {
        ...
        public override decimal ToDecimal(object value)
        {
            ...
            if (sourceType == XmlAtomicValueType)
                return ((decimal)((XmlAtomicValue)value).ValueAs(DecimalType));
        ...
        public override object ChangeType(int value, Type destinationType)
        {
            ...
            if (destinationType == StringType)
                return XmlConvert.ToString((int)value);
            if (destinationType == XmlAtomicValueType)
                return (new XmlAtomicValue(SchemaType!, (int)value));
            if (destinationType == XPathItemType)
                return (new XmlAtomicValue(SchemaType!, (int)value));
        ...
        public override object ChangeType(long value, Type destinationType)
        {
            ...
            if (destinationType == StringType)
                return XmlConvert.ToString((long)value);
            if (destinationType == XmlAtomicValueType)
                return (new XmlAtomicValue(SchemaType!, (long)value));
            if (destinationType == XPathItemType)
                return (new XmlAtomicValue(SchemaType!, (long)value));
        ...
        )
        {
            ...
            if (destinationType == StringType)
                return ((string)value);
            if (destinationType == XmlAtomicValueType)
                return (new XmlAtomicValue(SchemaType!, (string)value));
            if (destinationType == XPathItemType)
                return (new XmlAtomicValue(SchemaType!, (string)value));
        ...
        private object ChangeTypeWildcardDestination(
            object value,
            Type destinationType,
            IXmlNamespaceResolver? nsResolver
        )
    ...
    internal sealed class XmlNumeric2Converter : XmlBaseConverter
    {
        ...
        public override float ToSingle(object value)
        {
            ...
            if (sourceType == XmlAtomicValueType)
                return ((float)((XmlAtomicValue)value).ValueAs(SingleType));
        ...
        public override object ChangeType(double value, Type destinationType)
        {
            ...
            if (destinationType == StringType)
                return this.ToString((double)value);
            if (destinationType == XmlAtomicValueType)
                return (new XmlAtomicValue(SchemaType!, (double)value));
            if (destinationType == XPathItemType)
                return (new XmlAtomicValue(SchemaType!, (double)value));
    ...
    internal sealed class XmlDateTimeConverter : XmlBaseConverter
    {
        ...
        public override DateTimeOffset ToDateTimeOffset(object value)
        {
            ...
            if (sourceType == XmlAtomicValueType)
                return (DateTimeOffset)((XmlAtomicValue)value).ValueAsDateTime;
        ...
        public override object ChangeType(DateTime value, Type destinationType)
        {
            ...
            if (destinationType == StringType)
                return this.ToString((DateTime)value);
            if (destinationType == XmlAtomicValueType)
                return (new XmlAtomicValue(SchemaType!, (DateTime)value));
            if (destinationType == XPathItemType)
                return (new XmlAtomicValue(SchemaType!, (DateTime)value));
        ...
        )
        {
            ...
            if (destinationType == XmlAtomicValueType)
            {
                if (sourceType == DateTimeType)
                    return (new XmlAtomicValue(SchemaType!, (DateTime)value));
                if (sourceType == DateTimeOffsetType)
                    return (new XmlAtomicValue(SchemaType!, (DateTimeOffset)value));
                if (sourceType == StringType)
                    return (new XmlAtomicValue(SchemaType!, (string)value));
                if (sourceType == XmlAtomicValueType)
                    return ((XmlAtomicValue)value);
    ...
    internal sealed class XmlBooleanConverter : XmlBaseConverter
    {
        ...
        public override object ChangeType(bool value, Type destinationType)
        {
            ...
            if (destinationType == StringType)
                return XmlConvert.ToString((bool)value);
            if (destinationType == XmlAtomicValueType)
                return (new XmlAtomicValue(SchemaType!, (bool)value));
            if (destinationType == XPathItemType)
                return (new XmlAtomicValue(SchemaType!, (bool)value));
    ...
    internal sealed class XmlMiscConverter : XmlBaseConverter
    {
        ...
        public override string ToString(object value, IXmlNamespaceResolver? nsResolver)
        {
            ...
            if (sourceType == StringType)
                return (string)value;
            if (IsDerivedFrom(sourceType, UriType))
                if (TypeCode == XmlTypeCode.AnyUri)
            ...
            if (sourceType == TimeSpanType)
            {
                switch (TypeCode)
                {
                    case XmlTypeCode.DayTimeDuration:
                        return DayTimeDurationToString((TimeSpan)value);
        ...
        )
        {
            ...
            if (destinationType == ByteArrayType)
            {
                switch (TypeCode)
                {
                    ...
                    case XmlTypeCode.HexBinary:
                        return StringToHexBinary((string)value);
                }
            }
            ...
            if (destinationType == TimeSpanType)
            {
                ...
                {
                    case XmlTypeCode.DayTimeDuration:
                        return StringToDayTimeDuration((string)value);
                    case XmlTypeCode.Duration:
            ...
            if (destinationType == XmlAtomicValueType)
                return (new XmlAtomicValue(SchemaType!, (string)value, nsResolver));
    ...
    internal sealed class XmlUntypedConverter : XmlListConverter
    {
        ...
        private XmlUntypedConverter(XmlUntypedConverter atomicConverter, bool allowListToList)
            : base(
                atomicConverter,
                allowListToList ? StringArrayType : StringType,
                allowListToList ? StringArrayArrayType : StringArrayType
            )
        {
            _allowListToList = allowListToList;
        }

        public static readonly XmlValueConverter Untyped = new XmlUntypedConverter(
            new XmlUntypedConverter(),
            false
        );
        public static readonly XmlValueConverter UntypedList = new XmlUntypedConverter(
            new XmlUntypedConverter(),
            true
        );
        ...
        public override string ToString(object value, IXmlNamespaceResolver? nsResolver)
        {
            ...
            if (IsDerivedFrom(sourceType, UriType))
                return AnyUriToString((Uri)value);
            if (sourceType == XmlAtomicValueType)
                return ((string)((XmlAtomicValue)value).ValueAs(StringType, nsResolver));
            if (IsDerivedFrom(sourceType, XmlQualifiedNameType))
                return QNameToString((XmlQualifiedName)value, nsResolver);
        ...
        protected override object ChangeListType(
            object value,
            Type destinationType,
            IXmlNamespaceResolver? nsResolver
        )
        {
            ...
            )
            {
                ...
                if (SupportsType(destinationType))
                    throw new InvalidCastException(
                        SR.Format(SR.XmlConvert_TypeFromString, XmlTypeName, destinationType.Name)
                    );
        ...
        private static bool SupportsType(Type clrType)
        {
            ...
            if (clrType == ByteType)
                return true;
            if (clrType == ByteArrayType)
                return true;
            if (clrType == DateTimeType)
                return true;
            if (clrType == DateTimeOffsetType)
                return true;
            ...
            if (clrType == UriType)
                return true;
            if (clrType == XmlQualifiedNameType)
                return true;
    ...
    internal sealed class XmlAnyConverter : XmlBaseConverter
    {
        ...
        public override DateTimeOffset ToDateTimeOffset(object value)
        {
            ...
            if (sourceType == XmlAtomicValueType)
                return (DateTimeOffset)((XmlAtomicValue)value).ValueAs(DateTimeOffsetType);
        ...
        public override object ChangeType(long value, Type destinationType)
        {
            ...
            if (destinationType == XmlAtomicValueType)
                return (
                    new XmlAtomicValue(
                        XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.Long),
                        (long)value
                    )
                );
        ...
        )
        {
            ...
            {
                if (sourceType == XmlAtomicValueType)
                    return ((XmlAtomicValue)value).ValueAs(DateTimeOffsetType);
            }
            if (destinationType == DecimalType)
            {
                if (sourceType == XmlAtomicValueType)
            ...
            if (destinationType == XmlAtomicValueType)
            {
                ...
                if (sourceType == ByteType)
                    return (
                        new XmlAtomicValue(
                            XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.UnsignedByte),
                            value
                        )
                    );
                if (sourceType == ByteArrayType)
                    return (
                        new XmlAtomicValue(
                            XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.Base64Binary),
                            value
                        )
                    );
                if (sourceType == DateTimeType)
                ...
                if (sourceType == Int16Type)
                    return (
                        new XmlAtomicValue(
                            XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.Short),
                            value
                        )
                    );
                if (sourceType == Int32Type)
                ...
                if (sourceType == SByteType)
                    return (
                        new XmlAtomicValue(
                            XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.Byte),
                            value
                        )
                    );
                if (sourceType == SingleType)
                    return (
                        new XmlAtomicValue(
                            XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.Float),
                            value
                        )
                    );
                if (sourceType == StringType)
                ...
                if (sourceType == TimeSpanType)
                    return (
                        new XmlAtomicValue(
                            XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.Duration),
                            value
                        )
                    );
                if (sourceType == UInt16Type)
                    return (
                        new XmlAtomicValue(
                            XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.UnsignedShort),
                            value
                        )
                    );
                if (sourceType == UInt32Type)
                    return (
                        new XmlAtomicValue(
                            XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.UnsignedInt),
                            value
                        )
                    );
                if (sourceType == UInt64Type)
                    return (
                        new XmlAtomicValue(
                            XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.UnsignedLong),
                            value
                        )
                    );
                if (IsDerivedFrom(sourceType, UriType))
                    return (
                        new XmlAtomicValue(
                            XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.AnyUri),
                            value
                        )
                    );
                if (IsDerivedFrom(sourceType, XmlQualifiedNameType))
            ...
            if (destinationType == XPathItemType)
            {
                if (sourceType == XmlAtomicValueType)
                    return ((XmlAtomicValue)value);
                if (IsDerivedFrom(sourceType, XPathNavigatorType))
    ...
    internal sealed class XmlAnyListConverter : XmlListConverter
    {
        ...
        public static readonly XmlValueConverter ItemList = new XmlAnyListConverter(
            (XmlBaseConverter)XmlAnyConverter.Item
        );
        public static readonly XmlValueConverter AnyAtomicList = new XmlAnyListConverter(
            (XmlBaseConverter)XmlAnyConverter.AnyAtomic
        );
    ...
    internal class XmlListConverter : XmlBaseConverter
    {
        ...
        protected XmlListConverter(
            XmlBaseConverter atomicConverter,
            Type clrTypeDefault,
            Type clrListTypeDefault
        )
            : base(atomicConverter, clrTypeDefault, clrListTypeDefault)
        {
            this.atomicConverter = atomicConverter;
        }
        ...
        public static XmlValueConverter Create(XmlValueConverter atomicConverter)
        {
            ...
            Debug.Assert(
                !(atomicConverter is XmlListConverter)
                    || ((XmlListConverter)atomicConverter).atomicConverter == null,
        ...
        )
        {
            ...
            // Input value must support IEnumerable and destination type should be IEnumerable, ICollection,
            // IList, Type[], or String
            ...
            {
                // Convert from source value to strongly-typed array; special-case each possible item type for
                // performance
                Type? itemTypeDst = destinationType.GetElementType();

                // Converting from object[] to object[] is not necessarily a no-op (i.e. xs:int* stored as an
                // object[]
                // containing String values will need to be converted to an object[] containing Int32 values).
                if (itemTypeDst == ObjectType)
            ...
            // If source value is an array of values having the default representation, then conversion is a
            // no-op
        ...
        /// <summary>
        /// Convert "list" to an array of type T by iterating over each item in "list" and converting it to
        // type "T"
        ...
        private T[] ToArray<T>(object list, IXmlNamespaceResolver? nsResolver)
        {
            ...
            if (listSrc != null)
            {
                ...
                for (int i = 0; i < listSrc.Count; i++)
                    arrDst[i] = (T)
                        this.atomicConverter!.ChangeType(listSrc[i]!, typeof(T), nsResolver);
        ...
        /// <summary>
        /// Tokenize "value" by splitting it on whitespace characters.  Insert tokens into an ArrayList and
        // return the list.
        /// </summary>
        ...
        /// <summary>
        /// Convert a list to a corresponding list of strings.  Then concatenate the strings, which adjacent
        // values delimited
        ...
        /// for this Xml type.
        /// </summary>
        private new InvalidCastException CreateInvalidClrMappingException(
            Type sourceType,
            Type destinationType
        )
        {
