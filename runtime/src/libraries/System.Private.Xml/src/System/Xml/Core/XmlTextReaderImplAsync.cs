...
namespace System.Xml
{
    internal sealed partial class XmlTextReaderImpl : XmlReader, IXmlLineInfo, IXmlNamespaceResolver
    {
        ...
        private async Task InitStreamInputAsync(
            ...
            {
                int read = await stream.ReadAsync(_ps.bytes.AsMemory(_ps.bytesUsed))
                    .ConfigureAwait(false);
                if (read == 0)
                {
        ...
        private async Task ParseDtdAsync()
        {
            ...
            _dtdInfo = await dtdParser.ParseInternalDtdAsync(new DtdParserProxy(this), true)
                .ConfigureAwait(false);
        ...
        private async Task<bool> OpenAndPushAsync(Uri uri)
        {
            ...
            else
            {
                ...
                Stream stream = (Stream)await _xmlResolver.GetEntityAsync(uri, null, typeof(Stream))
                    .ConfigureAwait(false);
                if (stream == null)
                {
                    return false;
