...
namespace System.Xml.Serialization
{
        ...
        private SoapArrayInfo ParseArrayType(string? value)
        {
            ...
            if (len > 0)
            {
                ...
                catch (Exception e)
                {
                    ...
                    throw new ArgumentException(
                        SR.Format(SR.XmlInvalidArrayLength, lengthString),
                        nameof(value)
                    );
                }
            }
    ...
    internal sealed class XmlSerializationReaderCodeGen : XmlSerializationCodeGen
    {
            ...
            )
            {
                ...
                if (mapping.TypeDesc!.IsArrayLike)
                {
                    ...
                    if (mapping.ChoiceIdentifier != null)
                    {
                        ...
                        string castString = choiceUseReflection
                            ? ""
                            : "(" + choiceTypeFullName + "[])";
        ...
        {
            return "Read"
                + (++NextMethodNumber).ToString(CultureInfo.InvariantCulture)
                + "_"
                + CodeIdentifier.MakeValidInternal(name);
        }

        private string NextIdName(string name)
        {
            return "id"
                + (++_nextIdNumber).ToString(CultureInfo.InvariantCulture)
                + "_"
                + CodeIdentifier.MakeValidInternal(name);
        }
        ...
        private void WriteLiteralStructMethod(StructMapping structMapping)
        {
            ...
            if (structMapping.TypeDesc.IsRoot)
            {
                Writer.Indent++;
                Writer.WriteLine(
                    "return ReadTypedPrimitive(new System.Xml.XmlQualifiedName(\""
                        + Soap.UrType
                        + "\", \""
                        + XmlSchema.Namespace
                        + "\"));"
            ...
            else
            {
                ...
                Writer.Indent++;
                string unknownNode =
                    "UnknownNode((object)o, " + ExpectedElements(allMembers) + ");";
                WriteMemberElements(
                    allMembers,
                    unknownNode,
                    unknownNode,
        ...
        )
        {
            ...
            if (create == null)
            {
                string createName =
                    "create"
                    + (++_nextCreateMethodNumber).ToString(CultureInfo.InvariantCulture)
                    + "_"
                    + typeDesc.Name;
            ...
            for (int i = 0; i < members.Length; i++)
            {
                ...
                if (member.IsArrayLike)
                {
                    ...
                    else
                    {
                        ...
                        if (
                            member.Source[member.Source.Length - 1] == '('
                            || member.Source[member.Source.Length - 1] == '{'
                        )
                        {
                            WriteCreateInstance(
