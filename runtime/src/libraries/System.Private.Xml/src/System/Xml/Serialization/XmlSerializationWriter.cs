...
namespace System.Xml.Serialization
{
    ...
    public abstract class XmlSerializationWriter : XmlSerializationGeneratedCode
    {
        ...
        {
            get { return _escapeName; }
            set { _escapeName = value; }
        }
        ...
        {
            get { return _namespaces?.NamespaceList; }
            set
            {
                if (value == null)
                {
        ...
        protected void WriteTypedPrimitive(string? name, string? ns, object o, bool xsiType)
        {
            ...
            switch (Type.GetTypeCode(t))
            {
                case TypeCode.String:
                    ...
                    break;
                case TypeCode.Int32:
                    tryFormatResult = XmlConvert.TryFormat(
                        (int)o,
                        _primitivesBuffer,
                        out charsWritten
                    );
                ...
                case TypeCode.Decimal:
                    ...
                    break;
                case TypeCode.DateTime:
                    tryFormatResult = TryFormatDateTime(
                        (DateTime)o,
                        _primitivesBuffer,
                        out charsWritten
                    );
            ...
            {
                Debug.Assert(
                    tryFormatResult.Value,
                    "Something goes wrong with formatting primitives to the buffer."
                );
#if DEBUG
                const string escapeChars = "<>\"'&";
                ReadOnlySpan<char> span = _primitivesBuffer;
                Debug.Assert(
                    !span.Slice(0, charsWritten).ContainsAny(escapeChars),
                    "Primitive value contains illegal xml char."
                );
#endif
            ...
            if (o != null && _objectsInUse != null)
            {
                if (_objectsInUse.ContainsKey(o))
                    throw new InvalidOperationException(
                        SR.Format(SR.XmlCircularReference, o.GetType().FullName)
                    );
            ...
            if (_namespaces != null)
            {
                foreach (XmlQualifiedName qname in _namespaces.Namespaces)
                {
                    ...
                    if (string.IsNullOrEmpty(aliasNs))
                    {
                        if (alias.Length > 0)
                            throw new InvalidOperationException(
                                SR.Format(SR.XmlInvalidXmlns, alias)
                            );
        ...
        protected Exception CreateUnknownTypeException(Type type)
        {
            if (typeof(IXmlSerializable).IsAssignableFrom(type))
                return new InvalidOperationException(
                    SR.Format(SR.XmlInvalidSerializable, type.FullName)
                );
            TypeDesc typeDesc = new TypeScope().GetTypeDesc(type);
            if (!typeDesc.IsStructLike)
                return new InvalidOperationException(
                    SR.Format(SR.XmlInvalidUseOfType, type.FullName)
                );
        ...
        )
        {
            ...
            return new InvalidOperationException(
                SR.Format(SR.XmlChoiceMismatchChoiceException, elementName, value, enumValue)
            );
        }
        ...
        {
            return new InvalidOperationException(
                SR.Format(SR.XmlInvalidChoiceIdentifierValue, type, identifier)
            );
        }
            ...
            if (type.IsArray)
            {
                ...
                else
                {
                    _w.WriteAttributeString(
                        "arrayType",
                        Soap.Encoding,
                        $"{GetQualifiedName(typeName, typeNs)}[{arrayLength}]"
                    );
                }
            ...
            if (xmlns != null)
            {
                foreach (XmlQualifiedName qname in xmlns.Namespaces)
                {
                    ...
                    if (_namespaces != null)
                    {
                        string? oldNs;
                        if (
                            _namespaces.TryLookupNamespace(prefix, out oldNs)
                            && oldNs != null
                            && oldNs != ns
                        )
                        {
                            throw new InvalidOperationException(
                                SR.Format(SR.XmlDuplicateNs, prefix, ns)
                            );
                        }
                    }
    ...
    internal static class DynamicAssemblies
    {
        ...
        private static readonly ContextAwareTables<object> s_tableIsTypeDynamic =
            new ContextAwareTables<object>();
    ...
    internal sealed class ReflectionAwareCodeGen
    {
        ...
        )
        {
            ...
            throw new InvalidOperationException(
                SR.Format(SR.XmlSerializerUnsupportedType, memberInfos[0])
            );
        }

        private string WriteMethodInfo(
            string escapedName,
            string typeVariable,
            string memberName,
            bool isNonPublic,
        ...
        internal static string GetStringForCreateInstance(
            string type,
            string? cast,
            bool nonPublic,
            string? arg
    ...
    internal sealed class XmlSerializationWriterCodeGen : XmlSerializationCodeGen
    {
            ...
            if (hasDefault)
            {
                if (mapping is EnumMapping)
                {
                    ...
                    if (mapping.TypeDesc!.UseReflection)
                        Writer.Write(
                            ReflectionAwareCodeGen.GetStringForEnumLongValue(
                                source,
        ...
        private string GenerateMembersElement(XmlMembersMapping xmlMembersMapping)
        {
            ...
            if (hasWrapperElement)
            {
                ...
                {
                    string source =
                        $"(({typeof(System.Xml.Serialization.XmlSerializerNamespaces).FullName})p[{xmlnsMember}])";
            ...
            else if (element.Mapping is ArrayMapping arrayMapping)
            {
                if (arrayMapping.IsSoap)
                {
                    ...
                    if (!writeAccessor)
                    {
                        ...
                        Writer.Write(
                            RaCodeGen.GetStringForTypeof(
                                arrayMapping.TypeDesc!.CSharpName,
        ...
        )
        {
            ...
            for (int i = 0; i < choiceMapping.Constants!.Length; i++)
            {
                ...
                ReadOnlySpan<char> choiceNs =
                    colon < 0 ? choiceMapping.Namespace : xmlName.AsSpan(0, colon);
                ReadOnlySpan<char> choiceName = colon < 0 ? xmlName : xmlName.AsSpan(colon + 1);

                if (choiceName.SequenceEqual(element.Name))
                {
                    if (
                        (element.Form == XmlSchemaForm.Unqualified && choiceNs.IsEmpty)
                        || choiceNs.SequenceEqual(element.Namespace)
                    )
                    {
                        if (useReflection)
                            enumValue = choiceMapping
