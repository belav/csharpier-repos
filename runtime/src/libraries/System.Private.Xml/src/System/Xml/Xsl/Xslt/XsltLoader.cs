...
namespace System.Xml.Xsl.Xslt
{
    internal sealed class XsltLoader : IErrorHelper
    {
        ...
        private Stylesheet LoadStylesheet(XmlReader reader, bool include)
        {
            ...
            Debug.Assert(
                !_documentUriInUse.Contains(baseUri),
            ...
            try
            {
                ...
                if (!include)
                {
                    ...
                    {
                        _curStylesheet.Imports[i] = LoadStylesheet(
        ...
        private void LoadRealStylesheet()
        {
            Debug.Assert(
                _input.IsXsltNamespace()
                    && (_input.IsKeyword(_atoms.Stylesheet) || _input.IsKeyword(_atoms.Transform))
            );
        ...
        */
        ...
        private void CheckAttributeSetsDfs(AttributeSet attSet)
        {
            ...
            switch (attSet.CycleCheck)
            {
                ...
                default:
                    ...
                    _compiler.ReportError( /*[XT0720]*/
                        attSet.Content[0].SourceLine!,
                        SR.Xslt_CircularAttributeSet,
        ...
        private void LoadDecimalFormat(NsDecl? stylesheetNsList)
        {
            ...
            for (int idx = 0; idx < NumCharAttrs; idx++)
            {
                characters[idx] = ParseCharAttribute(
                    3 + idx,
                    _decimalFormatAttributes[3 + idx].name,
                    DefaultValues[idx]
                );
            }
            ...
            for (int i = 0; i < NumSignAttrs; i++)
            {
                for (int j = i + 1; j < NumSignAttrs; j++)
                {
                    if (characters[i] == characters[j])
                    {
                        ...
                        bool dummy =
                            _input.MoveToXsltAttribute(3 + j, _decimalFormatAttributes[3 + j].name)
                            || _input.MoveToXsltAttribute(
                                3 + i,
                                _decimalFormatAttributes[3 + i].name
                            );
                        Debug.Assert(
                            dummy,
                            "One of the attrs should have lineInfo. If both are default they can't conflict."
                        );
                        ReportError( /*[XT1300]*/
                            SR.Xslt_DecimalFormatSignsNotDistinct,
                            _decimalFormatAttributes[3 + i].name,
                            _decimalFormatAttributes[3 + j].name
                        );
                        break;
                    }
                }
            ...
            if (_compiler.DecimalFormats.Contains(name))
            {
                ...
                CheckError(
                    ...
                    SR.Xslt_DecimalFormatRedefined,
                    "infinity",
                    infinity
                );
        ...
        private void LoadNamespaceAlias(NsDecl? stylesheetNsList)
        {
            ...
            if (_input.MoveToXsltAttribute(0, "stylesheet-prefix"))
            {
                ...
                {
                    ReportError( /*[XT_005]*/
                        SR.Xslt_EmptyNsAlias,
                        "stylesheet-prefix"
                    );
                }
                else
                {
                    stylesheetNsUri = _input.LookupXmlNamespace(
        ...
        private void LoadGlobalVariableOrParameter(NsDecl? stylesheetNsList)
        {
            ...
            CheckError(
                !_curStylesheet!.AddVarPar(var), /*[XT0630]*/
                SR.Xslt_DupGlobalVariable,
        ...
        private void LoadTemplate(NsDecl? stylesheetNsList)
        {
            ...
            {
                CheckError(
                    !_input.AttributeExists(1, "name"), /*[XT_007]*/
                    SR.Xslt_BothMatchNameAbsent
                );
                CheckError(
                    _input.AttributeExists(3, "mode"), /*[XT_008]*/
                    SR.Xslt_ModeWithoutMatch
                );
                mode = nullMode;
            ...
            {
                ReportError( /*[XT0660]*/
                    SR.Xslt_DupTemplateName,
                    _curTemplate.Name!.QualifiedName
                );
            }
            _curTemplate = null;
        }
        ...
        private XslNode XslApplyTemplates()
        {
            ...
            ctxInfo.SaveExtendedLineInfo(_input);
            return SetInfo(
                F.ApplyTemplates(mode, select, ctxInfo, _input.XslVersion),
                content,
                ctxInfo
            );
        }
        ...
        private XslNode XslForEach()
        {
            ...
            return SetInfo(F.ForEach(select, ctxInfo, _input.XslVersion), content, ctxInfo);
        }
        ...
        private XslNode XslElement()
        {
            ...
            return SetInfo(
                F.Element(name, ns, _input.XslVersion),
        ...
        private XslNode XslAttribute()
        {
            ...
            return SetInfo(
                F.Attribute(name, ns, _input.XslVersion),
        ...
        private XslNode XslSort(int sortNumber)
        {
            ...
            return SetInfo(
                F.Sort(select, lang, dataType, order, caseOrder, _input.XslVersion),
                null,
                ctxInfo
            );
        }
                    ...
                    switch (input.NodeType)
                    {
                        case XmlNodeType.Element:
                            if (input.IsXsltKeyword(atoms.MatchingSubstring))
                            {
                                ...
                                CheckError(
                                    nonMatching != null, /*[???]*/
                                    SR.Xslt_AnalyzeStringChildOrder
                                );
                                CheckError(
                                    matching != null, /*[???]*/
            ...
            for (int i = 1; _input.MoveToLiteralAttribute(i); i++)
            {
                if (!_input.IsXsltNamespace())
                {
                    XslNode att = F.LiteralAttribute(
                        F.QName(_input.LocalName, _input.NamespaceUri, _input.Prefix),
                        _input.Value,
                        _input.XslVersion
                    );
            ...
            if (fallbacksArray.Count == 0)
            {
                fallbacksArray.Add(
                    F.Error(
                        XslLoadException.CreateMessage(
                            extElmLineInfo,
                            SR.Xslt_UnknownExtensionElement,
                            instrName
                        )
                    )
                );
        ...
        {
            Debug.Assert(
                _input.IsRequiredAttribute(attNum),
        ...
        private void ParseWhitespaceRules(string elements, bool preserveSpace)
        {
            if (!string.IsNullOrEmpty(elements))
            {
                ...
                for (int i = 0; i < tokens.Length; i++)
                {
                    ...
                    {
                        namespaceName =
                            _input.LookupXmlNamespace(prefix) ?? _compiler.CreatePhantomNamespace();
                    }
        ...
        {
            Debug.Assert(
                _input.LocalName == "use-attribute-sets",
            ...
            foreach (string qname in XmlConvert.SplitString(_input.Value))
            {
                AddInstruction(
                    list,
                    SetLineInfo(F.UseAttributeSet(CreateXPathQName(qname)), _input.BuildLineInfo())
                );
            }
            if (!_compiler.ExitForwardsCompatible(_input.ForwardCompatibility))
            {
            ...
            if (_input.MoveToXsltAttribute(attNum, "input-type-annotations"))
            {
                ...
                switch (value)
                {
                    ...
                    case "preserve":
                        ...
                        {
                            CheckError(
                                _compiler.inputTypeAnnotations != value, /*[XTSE0265]*/
                                SR.Xslt_InputTypeAnnotations
                            );
                        }
                        break;
                    default:
