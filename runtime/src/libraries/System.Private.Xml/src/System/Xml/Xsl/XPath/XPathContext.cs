...
namespace System.Xml.Xsl.XPath
{
    internal sealed class XPathContext
    {
        ...
        // on practice in this implementation we have "current node" (C), "position of current node in
        // current node-set" (P)
        ...
        // XPath function last() force (1) -> (3) or (2) -> (3) transformation if node-set wasn't cashed
        // yet.
        //
        ...
        // "for"         ==> Tuple1(For1(Tuple3(For3(Content(XmlContext)), Name(For3)=='foo', For3)), True,
        // For1)
        //     Note1: all external references to "foo" expression are still valid because we didn't change
        // Tuple1
        //     Note2: "bar" is still valid because it refers to For1.
        //     Note3: "Name(For1)=='foo'" was rewritten as "Name(For3)=='foo'"
        // "last()"      ==> SetLength(Let)
        // "for"         ==> Tuple1(Let(Tuple3(For3(Content(XmlContext)), Name(For3)=='foo', For3)), True,
        // Tuple4(For1(Let), True, For1))
        ...
        // XSLT compiles for-each and apply-templates in form (2) so converting to (3) shouldn't be a
        // problem
