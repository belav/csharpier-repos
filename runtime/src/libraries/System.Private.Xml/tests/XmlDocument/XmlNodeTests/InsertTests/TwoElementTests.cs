    ...
    public static class TwoElementTests
    {
        ...
        )
        {
            ...
            if (deleteFirst)
                expected[1] =
                    (insertType == InsertType.Append)
                        ? (parent.LastChild.OuterXml + newChild.OuterXml)
                        : (newChild.OuterXml + parent.LastChild.OuterXml);
            else
                expected[1] =
                    (insertType == InsertType.Prepend)
                        ? (newChild.OuterXml + parent.FirstChild.OuterXml)
                        : (parent.FirstChild.OuterXml + newChild.OuterXml);
