        ...
        public static void CloneComplexDocumentTrue()
        {
            ...
            // Test XmlNode.LastChild
            {
                for (var i = 0; i < xmlDocument.ChildNodes.Count; i++)
                {
                    ...
                    {
                        Assert.Equal(
                            xmlDocument.ChildNodes[i].LastChild,
                            cloned.ChildNodes[i].LastChild
                        );
                    }
                    else
                    {
                        Assert.Equal(
                            xmlDocument.ChildNodes[i].LastChild.Value,
                            cloned.ChildNodes[i].LastChild.Value
                        );
                    }
                }
            }
