    ...
    internal class EncodeDecodeTests : SqlXmlConvertTests
    {
        ...
        public override void AddChildren()
        {
            AddChild(
                new CVariation(v1)
                {
                    Attribute = new Variation("EncodeName with null and String.Empty")
                }
            );
            AddChild(
                new CVariation(v2) { Attribute = new Variation("EncodeName with surrogates") }
            );
            AddChild(new CVariation(v3) { Attribute = new Variation("EncodeName in uppercase") });
            AddChild(
                new CVariation(v4)
                {
                    Attribute = new Variation("DecodeName with uppercase/lowercase")
                }
            );
        }
        ...
        public int v2()
        {
            string sVal = "\uD800\uDC00\uDBFF\uDFFF\uD800\uDC00";
            CError.Compare(
                XmlConvert.EncodeName(sVal),
                "_x00010000__x0010FFFF__x00010000_",
                "ent empty"
            );
        ...
        {
            CError.Equals(
                XmlConvert.EncodeName("_xFF71__xff71_"),
                "_x005F_xFF71__x005F_xff71_",
                "EncodeName"
            );
            CError.Equals(
                XmlConvert.EncodeLocalName("_xFF71__xff71_"),
                "_x005F_xFF71__x005F_xff71_",
                "EncodeLocalName"
            );
            CError.Equals(
                XmlConvert.EncodeNmToken("_xFF71__xff71_"),
                "_x005F_xFF71__x005F_xff71_",
                "EncodeNmToken"
            );
