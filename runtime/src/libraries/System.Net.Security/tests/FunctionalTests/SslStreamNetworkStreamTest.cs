...
namespace System.Net.Security.Tests
{
    ...
    public class CertificateSetup : IDisposable
    {
        ...
        public CertificateSetup()
        {
            ...
            (serverCert, serverChain) = TestHelper.GenerateCertificates(
                "localhost",
                nameof(SslStreamNetworkStreamTest),
                longChain: true
            );
        }
...
#pragma warning restore SYSLIB0039
            ...
            using (TcpClient client = new TcpClient())
            {
                ...
                using (
                    SslStream serverStream = new SslStream(
                        server.GetStream(),
                        false,
                        null,
                        null,
        ...
        public async Task SslStream_NetworkStream_Renegotiation_Succeeds(bool useSync)
        {
            ...
            using (SslStream ssl = new SslStream(ns, true, validationCallback))
            {
                ...
                await ssl.AuthenticateAsClientAsync(
                    Configuration.Security.TlsRenegotiationServer,
                    certBundle,
                    SslProtocols.Tls12,
                    false
                );
                ...
                int bytesRead = useSync
                    ? ssl.Read(message, 0, message.Length)
...
#pragma warning restore SYSLIB0039
        ...
        public async Task SslStream_NegotiateClientCertificateAsync_IncompleteIncomingTlsFrame_Throws()
        {
            ...
            ManualChunkingStream clientChunkingStream = new ManualChunkingStream(
                clientStream,
                false
            );
...
#pragma warning restore SYSLIB0039
                ...
                await clientChunkingStream.CommitWriteAsync(
                    clientChunkingStream.PendingWriteLength - 100
                );
        ...
        [SkipOnPlatform(
            ...
            "SslStream Renegotiate is not supported in SslStream on Android"
        )]
        public async Task SslStream_NegotiateClientCertificateAsyncTls13_Succeeds(
            bool sendClientCertificate
        )
        {
            bool negotiateClientCertificateCalled = false;
...
#pragma warning restore SYSLIB0039
        ...
        public async Task SslStream_NestedAuth_Throws()
        {
            ...
            {
                // Start handshake.
                Task task = ssl.AuthenticateAsClientAsync(
                    "foo.com",
                    null,
                    SslProtocols.Tls12,
                    false
        ...
        public async Task SslStream_TargetHostName_Succeeds(bool useEmptyName, bool useCallback)
        {
            ...
            using (X509Certificate2 certificate = Configuration.Certificates.GetServerCertificate())
            {
                ...
                if (useCallback)
                {
                    serverOptions.ServerCertificateSelectionCallback = (sender, name) =>
                    {
                        SslStream stream = (SslStream)sender;
                        Assert.Equal(targetName, stream.TargetHostName);
        ...
        public async Task SslStream_ServerUntrustedCaWithCustomTrust_OK(bool usePartialChain)
        {
            ...
            clientOptions.CertificateChainPolicy
                .CustomTrustStore
        ...
        private async Task SslStream_ClientSendsChain_Core(
        ...
        )
        {
            ...
            ) =>
            {
                ...
                _output.WriteLine(
                    "RemoteCertificateValidationCallback called with {0} and {1} extra certificates",
                    sslPolicyErrors,
        ...
        public async Task SslStream_ClientCertificate_SendsChain()
        {
            ...
            StoreName storeName = OperatingSystem.IsMacOS()
                ? StoreName.My
                : StoreName.CertificateAuthority;
            List<SslStream> streams = new List<SslStream>();
            TestHelper.CleanupCertificates(
                nameof(SslStream_ClientCertificate_SendsChain),
                storeName
            );
            (X509Certificate2 clientCertificate, X509Certificate2Collection clientChain) =
        ...
        public async Task SslStream_ClientCertificateContext_SendsChain()
        {
            (X509Certificate2 clientCertificate, X509Certificate2Collection clientChain) =
                TestHelper.GenerateCertificates(
                    nameof(SslStream_ClientCertificateContext_SendsChain),
                    serverCertificate: false
                );
        ...
        {
            (X509Certificate2 serverCertificate, X509Certificate2Collection chain) =
                TestHelper.GenerateCertificates(
                    nameof(SslStream_EphemeralKey_Throws),
                    ephemeralKey: true
                );
        ...
        public async Task SslStream_RandomSizeWrites_OK(
            int bufferSize,
            int readBufferSize,
            int writeBufferSize,
            bool useAsync
        )
        {
            ...
            using (serverStream)
            using (
                SslStream client = new SslStream(
                    new RandomReadWriteSizeStream(clientStream, readBufferSize)
                )
            )
            using (SslStream server = new SslStream(new RandomReadWriteSizeStream(serverStream)))
            {
                ...
                Task reader = Task.Run(async () =>
                {
                    ...
                    while (true)
                    {
                        if (useAsync)
                        {
                            readLength = await client.ReadAsync(
                                readBuffer.Slice(totalLength, readBufferSize)
                            );
                        }
                        else
                        {
                    ...
                    AssertExtensions.SequenceEqual(
                        dataToCopy.AsSpan(),
                        dataReceived.AsSpan(0, totalLength)
                    );
                });
        ...
        public async Task SslStream_UnifiedHello_Ok(bool useOptionCallback)
        {
            ...
            if (useOptionCallback)
            {
                serverTask = ssl.AuthenticateAsServerAsync(
                    ...
                    {
                        callbackCalled = true;
                        options.ServerCertificate = Configuration.Certificates
                            .GetServerCertificate();
            ...
            Task.WaitAny(
                client.WriteAsync(TlsFrameHelperTests.s_UnifiedHello).AsTask(),
                serverTask
            );
