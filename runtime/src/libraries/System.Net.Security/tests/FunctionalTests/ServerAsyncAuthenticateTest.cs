...
namespace System.Net.Security.Tests
{
    ...
    public class ServerAsyncAuthenticateTest : IDisposable
    {
        ...
        public async Task ServerAsyncAuthenticate_SniSetVersion_Success(SslProtocols version)
        {
            ...
            var clientOptions = new SslClientAuthenticationOptions()
            {
                TargetHost = _serverCertificate.GetNameInfo(
...
#pragma warning restore 0618
        ...
        )
        {
            ...
            )
            {
                ...
                _log.WriteLine(
                    "Connected on {0} {1} ({2} {3})",
                    clientStream.Socket.LocalEndPoint,
                    clientStream.Socket.RemoteEndPoint,
                    clientStream.Socket.Handle,
                    serverStream.Socket.Handle
                );
                _log.WriteLine(
                    "client SslStream#{0} server SslStream#{1}",
                    sslClientStream.GetHashCode(),
                    sslServerStream.GetHashCode()
                );

                _logVerbose.WriteLine(
                ...
                Assert.True(
                    sslServerStream.CipherStrength > 0,
        ...
        )
        {
            Assert.True(
                ...
                "Client didn't supply a cert, the server required one, yet sslPolicyErrors is "
                    + sslPolicyErrors
            );
