...
namespace System.Net.Security.Tests
{
    public class TransportContextTest
    {
        ...
        {
            using (
                var testServer = new DummyTcpServer(
            ...
            using (var client = new TcpClient())
            {
                await client.ConnectAsync(
                    testServer.RemoteEndPoint.Address,
                    testServer.RemoteEndPoint.Port
                );
        ...
        private static void CheckTransportContext(TransportContext context)
        {
            ...
            if (OperatingSystem.IsMacOS())
            {
                Assert.True(
                    cbt2 == null,
                    "ChannelBindingKind.Unique token data is not expected on OSX platform."
                );
            }
            else
            {
                Assert.True(
                    cbt2 != null,
                    "ChannelBindingKind.Unique token data should be returned."
                );
            }

            Assert.True(
                cbt3 == null,
                "ChannelBindingKind.Unknown token data should not be returned."
            );
        }
        ...
        private static void CheckChannelBinding(
            ...
            if (channelBinding != null)
            {
                ...
                Assert.True(
                    !channelBinding.IsInvalid,
                    "Channel binding token should be marked as a valid SafeHandle."
                );
                Assert.True(
                    channelBinding.Size > 0,
                    "Number of bytes in a valid channel binding token should be greater than zero."
                );
                ...
                string expectedPrefix =
                    (kind == ChannelBindingKind.Endpoint) ? PrefixEndpoint : PrefixUnique;
