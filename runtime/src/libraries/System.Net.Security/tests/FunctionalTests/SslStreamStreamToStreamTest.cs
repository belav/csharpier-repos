...
namespace System.Net.Security.Tests
{
    ...
    public abstract class SslStreamStreamToStreamTest
    {
        ...
        protected abstract Task DoHandshake(
        ...
        public static IEnumerable<object[]> SslStream_StreamToStream_Authentication_Success_MemberData()
        {
            ...
            using (X509Certificate2 clientCert = Configuration.Certificates.GetClientCertificate())
            {
                ...
                yield return new object[]
                {
                    new X509Certificate(serverCert.Export(X509ContentType.Pfx)),
                    new X509Certificate(clientCert.Export(X509ContentType.Pfx))
                };
            }
        }
        ...
        public async Task SslStream_StreamToStream_Authentication_IncorrectServerName_Fail()
        {
            ...
            using (var certificate = Configuration.Certificates.GetServerCertificate())
            {
                ...
                await Assert.ThrowsAsync<AuthenticationException>(
                    () => t1.WaitAsync(TestConfiguration.PassingTestTimeout)
                );
                try
                {
        ...
        public async Task SslStream_StreamToStream_Dispose_Throws()
        {
            ...
            using (
                var clientSslStream = new SslStream(
                    DelegateDelegatingStream.NopDispose(stream1),
                    false,
            ...
            )
            {
                ...
                if (
                    this is SslStreamStreamToStreamTest_BeginEnd
        ...
        public async Task SslStream_StreamToStream_EOFDuringFrameRead_ThrowsIOException()
        {
            ...
            using (serverStream)
            {
                ...
                var serverWrappedNetworkStream = new DelegateStream(
                    ...
                    writeAsyncFunc: (buffer, offset, count, token) =>
                        serverStream.WriteAsync(buffer, offset, count, token),
    ...
    {
        protected override async Task DoHandshake(
            SslStream clientSslStream,
            SslStream serverSslStream,
            X509Certificate serverCertificate = null,
            X509Certificate clientCertificate = null
    ...
    public sealed class SslStreamStreamToStreamTest_BeginEnd : SslStreamStreamToStreamTest
    {
        ...
        )
        {
            ...
            await WithServerCertificate(
                ...
                async (certificate, name) =>
                {
                    IAsyncResult clientBeginAuth = clientSslStream.BeginAuthenticateAsClient(
                        name,
                        clientCerts,
                        SslProtocols.None,
                        checkCertificateRevocation: false,
                    ...
                    IAsyncResult serverBeginAuth = serverSslStream.BeginAuthenticateAsServer(
                        certificate,
                        clientCertificateRequired: clientCertificate != null,
                        checkCertificateRevocation: false,
                        null,
    ...
    public sealed class SslStreamStreamToStreamTest_SyncParameters
        : SslStreamStreamToStreamTest_SyncBase
    {
        protected override async Task DoHandshake(
            SslStream clientSslStream,
            SslStream serverSslStream,
    ...
    public sealed class SslStreamStreamToStreamTest_SyncSslOptions
        : SslStreamStreamToStreamTest_SyncBase
    {
        protected override async Task DoHandshake(
            SslStream clientSslStream,
    ...
    public sealed class SslStreamStreamToStreamTest_MemoryAsync : SslStreamStreamToStreamTest
    {
        ...
        public async Task AuthenticateAsServerAsync_VirtualNetwork_CanceledAfterStart_ThrowsOperationCanceledException()
        {
            ...
            {
                var cts = new CancellationTokenSource();
                Task t = serverSslStream.AuthenticateAsServerAsync(
