...
namespace System.Net
{
    ...
    {
        private static readonly Lazy<bool> _hasSystemNetSecurityNative = new Lazy<bool>(
            CheckHasSystemNetSecurityNative
        );
        ...
        )
        {
            if (UseManagedNtlm)
            {
                switch (clientOptions.Package)
                {
                    ...
                    case NegotiationInfoClass.Negotiate:
                        return new ManagedSpnegoNegotiateAuthenticationPal(
                            clientOptions,
                            supportKerberos: HasSystemNetSecurityNative
                        );
                }
            }
            ...
            catch (Interop.NetSecurityNative.GssApiException gex)
            {
                ...
                NegotiateAuthenticationStatusCode statusCode =
                    UnixNegotiateAuthenticationPal.GetErrorCode(gex);
        ...
        internal sealed class UnixNegotiateAuthenticationPal : NegotiateAuthenticationPal
        {
            ...
            public override bool IsEncrypted =>
                (_contextFlags & Interop.NetSecurityNative.GssFlags.GSS_C_CONF_FLAG) != 0;
            ...
            )
            {
                ...
                {
                    TokenImpersonationLevel.Identification => Interop.NetSecurityNative
                        .GssFlags
                        .GSS_C_IDENTIFY_FLAG,
                    TokenImpersonationLevel.Delegation => Interop.NetSecurityNative
                        .GssFlags
                        .GSS_C_DELEG_FLAG,
                    _ => 0,
                };
                ...
                )
                {
                    ...
                    if (_packageType == Interop.NetSecurityNative.PackageType.NTLM)
                    {
                        ...
                        throw new Interop.NetSecurityNative.GssApiException(
                            Interop.NetSecurityNative.Status.GSS_S_NO_CRED,
                            0,
                            SR.net_ntlm_not_possible_default_cred
                        );
                    }
                    if (string.IsNullOrEmpty(_spn))
                    {
                        ...
                        throw new Interop.NetSecurityNative.GssApiException(
                            Interop.NetSecurityNative.Status.GSS_S_BAD_NAME,
                            0,
                            SR.net_nego_not_supported_empty_target_with_defaultcreds
                        );
                    }

                    _credentialsHandle = SafeGssCredHandle.Create(
                        string.Empty,
                        string.Empty,
                        _packageType
                    );
            ...
            )
            {
                ...
                if (
                    serverOptions.Credential == CredentialCache.DefaultNetworkCredentials
            ...
            )
            {
                ...
                if (!_isServer)
                {
                    ...
                    if (NetEventSource.Log.IsEnabled())
                        NetEventSource.Info(
                            this,
                            $"SSPIWrapper.InitializeSecurityContext() returns statusCode:{statusCode}"
                        );
                }
                else
                {
                    ...
                    if (NetEventSource.Log.IsEnabled())
                        NetEventSource.Info(
                            this,
                            $"SSPIWrapper.AcceptSecurityContext() returns statusCode:{statusCode}"
                        );
                }
                ...
                else
                {
                    ...
                    if (NetEventSource.Log.IsEnabled())
                        NetEventSource.Info(
                            this,
                            $"need continue _securityContext:{_securityContext}"
                        );
                }
            ...
            private static Interop.NetSecurityNative.PackageType GetPackageType(string package)
            {
                ...
                else
                {
                    ...
                    throw new Interop.NetSecurityNative.GssApiException(
                        Interop.NetSecurityNative.Status.GSS_S_UNAVAILABLE,
                        0
                    );
                }
            }
            ...
            )
            {
                ...
                if (contextHandle == null)
                {
                    if (NetEventSource.Log.IsEnabled())
                    {
                        ...
                        NetEventSource.Info(
                            this,
                            $"requested protocol = {protocol}, target = {spn}"
                        );
                    }
        ...
        public static bool CheckHasSystemNetSecurityNative()
        {
            try
            {
                ...
                when (e is EntryPointNotFoundException
