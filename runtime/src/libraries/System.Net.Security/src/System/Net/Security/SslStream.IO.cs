...
namespace System.Net.Security
{
    public partial class SslStream
    {
        ...
        private void CloseInternal()
        {
            ...
            if (
                Interlocked.Exchange(ref _nestedRead, 1) == 0
            ...
            where TIOAdapter : IReadWriteAdapter
        {
            ...
            try
            {
                ...
                do
                {
                    message = await ReceiveBlobAsync<TIOAdapter>(cancellationToken)
                        .ConfigureAwait(false);
                    if (message.Size > 0)
                    {
                        await TIOAdapter
...
#pragma warning restore CS0618
        ...
        ) where TIOAdapter : IReadWriteAdapter
        {
            ...
            while (_buffer.EncryptedLength < frameSize)
            {
                ...
                if (
                    frameSize == int.MaxValue && _buffer.EncryptedLength > TlsFrameHelper.HeaderSize
                )
                {
        ...
        ) where TIOAdapter : IReadWriteAdapter
        {
            ...
            try
            {
                ...
                while (true)
                {
                    int payloadBytes = await EnsureFullTlsFrameAsync<TIOAdapter>(cancellationToken)
                        .ConfigureAwait(false);
                    if (payloadBytes == 0)
                    {
                        _receivedEOF = true;
