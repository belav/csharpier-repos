...
#pragma warning restore CA1825
        ...
        )
        {
            ...
            return AuthenticateAsync(
                new AsyncReadWriteAdapter(InnerStream, cancellationToken: default)
            );
        }
        ...
        public override bool IsSigned =>
            IsAuthenticatedCore && (_context.IsIntegrityFlag || _context.IsConfidentialityFlag);
        ...
        private bool CanGetSecureStream =>
            _context!.IsConfidentialityFlag || _context.IsIntegrityFlag;
        ...
        private async ValueTask<int> ReadAsync<TAdapter>(
            TAdapter adapter,
            Memory<byte> buffer,
        ...
        ) where TAdapter : IReadWriteAdapter
        {
            if (Interlocked.Exchange(ref _readInProgress, 1) == 1)
            {
                throw new NotSupportedException(
                    SR.Format(SR.net_io_invalidnestedcall, callerName, "read")
                );
            }
            ...
            try
            {
                ...
                while (true)
                {
                    int readBytes = await ReadAllAsync(adapter, _readHeader, allowZeroRead: true)
        ...
        )
        {
            ...
            return new ValueTask(
                WriteAsync(new AsyncReadWriteAdapter(InnerStream, cancellationToken), buffer)
            );
        }

        private async Task WriteAsync<TAdapter>(TAdapter adapter, ReadOnlyMemory<byte> buffer)
            where TAdapter : IReadWriteAdapter
        {
            if (Interlocked.Exchange(ref _writeInProgress, 1) == 1)
            {
                throw new NotSupportedException(
                    SR.Format(SR.net_io_invalidnestedcall, nameof(Write), "write")
                );
            }
            ...
            try
            {
                while (!buffer.IsEmpty)
                {
                    ...
                    try
                    {
                        encryptedBytes = EncryptData(
                            buffer.Slice(0, chunkBytes).Span,
                            ref _writeBuffer
                        );
                    }
                    catch (Exception e)
        ...
        private async Task AuthenticateAsync<TAdapter>(
            TAdapter adapter,
            [CallerMemberName] string? callerName = null
        ) where TAdapter : IReadWriteAdapter
        {
            ...
            if (Interlocked.Exchange(ref _authInProgress, 1) == 1)
            {
                throw new InvalidOperationException(
                    SR.Format(SR.net_io_invalidnestedcall, callerName, "authenticate")
                );
            }
        ...
        private bool CheckSpn()
        {
            ...
            {
                return _extendedProtectionPolicy.PolicyEnforcement
        ...
        private async Task SendBlobAsync<TAdapter>(TAdapter adapter, byte[]? message)
            where TAdapter : IReadWriteAdapter
        {
            Debug.Assert(_context != null);
            ...
            if (exception != null)
            {
                ...
                await SendAuthResetSignalAndThrowAsync(adapter, message!, exception)
                    .ConfigureAwait(false);
        ...
        private async Task ReceiveBlobAsync<TAdapter>(TAdapter adapter)
            where TAdapter : IReadWriteAdapter
        {
        ...
        private async Task SendAuthResetSignalAndThrowAsync<TAdapter>(
            TAdapter adapter,
            byte[] message,
            Exception exception
        ) where TAdapter : IReadWriteAdapter
        ...
        private unsafe byte[]? GetOutgoingBlob(byte[]? incomingBlob, ref Exception? e)
        {
            ...
            byte[]? message = _context.GetOutgoingBlob(
                incomingBlob,
                false,
