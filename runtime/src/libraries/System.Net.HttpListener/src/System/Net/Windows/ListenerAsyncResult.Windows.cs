            ...
            try
            {
                ...
                else
                {
                    HttpListenerSession listenerSession = (
                        asyncResult.AsyncObject! as HttpListenerSession
                    )!;
        ...
        internal uint QueueBeginGetContext()
        {
            ...
            while (true)
            {
                ...
                if (NetEventSource.Log.IsEnabled())
                    NetEventSource.Info(
                        this,
                        $"Calling Interop.HttpApi.HttpReceiveHttpRequest RequestId: {_requestContext.RequestBlob->RequestId}Buffer:0x {((IntPtr)_requestContext.RequestBlob).ToString("x")} Size: {_requestContext.Size}"
                    );
                ...
                if (NetEventSource.Log.IsEnabled())
                    NetEventSource.Info(
                        this,
                        "Call to Interop.HttpApi.HttpReceiveHttpRequest returned:" + statusCode
                    );
                if (
