...
namespace System.Threading.Tasks.Dataflow.Tests
{
    internal static partial class DataflowTestHelpers
    {
        ...
        internal static Func<int, IEnumerable<int>> ToEnumerable = item =>
            Enumerable.Repeat(item, 1);

        internal static ITargetBlock<int> PostRange(
        ...
        internal static ITargetBlock<T> PostRange<T>(
        ...
        internal static void TestConsumeReserveReleaseArgumentsExceptions<T>(ISourceBlock<T> source)
        {
            DataflowMessageHeader validMessageHeader = new DataflowMessageHeader(1),
                invalidMessageHeader = default(DataflowMessageHeader);
            ITargetBlock<T> validTarget = new BufferBlock<T>(),
                invalidTarget = null;
            bool consumed;
        ...
        internal static void TestToString(Func<string, IDataflowBlock> blockFactory)
        {
            ...
            Assert.Equal(expected: "none", actual: block.ToString());
        ...
        )
        {
            ...
            T item;
            Assert.Equal(
                expected: !reservationIsTargetSpecific,
                actual: block.TryReceive(out item)
        ...
        internal static IPropagatorBlock<U, U> Chain<T, U>(int numBlocks, Func<T> generate)
            where T : IPropagatorBlock<U, U>
        {
