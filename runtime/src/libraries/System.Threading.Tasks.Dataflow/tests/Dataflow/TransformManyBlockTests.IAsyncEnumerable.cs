...
namespace System.Threading.Tasks.Dataflow.Tests
{
    public partial class TransformManyBlockTests
    {
                        ...
                        foreach (int dop in new[] { 1, 2, 8 })
                            foreach (int elementsPerItem in new[] { 1, 5, 100 })
                                foreach (bool ensureOrdered in DataflowTestHelpers.BooleanValues)
                                {
                                    ...
                                    TransformManyBlock<int, int> tb = new TransformManyBlock<
                                        int,
                                        int
                                    >(i => AsyncEnumerable.Repeat(i, elementsPerItem), options);

                                    await Task.WhenAll(
                                        Task.Run(
                                            async delegate // consumer
                                            {
                                                if (ensureOrdered)
                                                {
                                                    int i = 0;
                                                ...
                                                else
                                                {
                                                    ...
                                                    await foreach (
                                                        int result in tb.ReceiveAllAsync()
                                                    )
                                                    {
                                                        results.Add(result);
                                                    }

                                                    IEnumerable<IGrouping<int, int>> messages =
                                                        results.GroupBy(i => i);
                                                    Assert.Equal(Messages, messages.Count());
                                                    Assert.All(
                                                        messages,
                                                        m =>
                                                            Assert.Equal(elementsPerItem, m.Count())
                                                    );
                                                }
                                            }
                                        ),
                                        Task.Run(
                                            async delegate // producer
                                            {
                                                for (int i = 0; i < Messages; i++)
                                                {
        ...
        {
            using Barrier barrier1 = new Barrier(2),
        ...
        public async Task TestChainedSendReceiveAsyncEnumerable()
        {
            ...
            {
                static TransformManyBlock<int, int> func() =>
        ...
        public async Task TestOrdering_Sync_OrderedEnabledAsyncEnumerable(
            int mmpt,
            int dop,
            bool? EnsureOrdered
        )
        {
            ...
            var tb = new TransformManyBlock<int, int>(
                i => DataflowTestHelpers.ToAsyncEnumerable(i),
                options
            );
        ...
        [InlineData(false)]
        [InlineData(true)]
        public async Task TestOrdering_Sync_BlockingEnumeration_NoDeadlockAsyncEnumerable(
            bool ensureOrdered
        )
        {
            ...
            using ManualResetEventSlim mres1 = new ManualResetEventSlim(),
            ...
            static IAsyncEnumerable<int> BlockableIterator(
        ...
        public async Task TestScheduling_MoveNextAsync_RunsOnTargetScheduler()
        {
            ...
            TransformManyBlock<int, int> t = new TransformManyBlock<int, int>(
                Body,
