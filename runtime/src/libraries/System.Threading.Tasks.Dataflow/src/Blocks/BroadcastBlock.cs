...
namespace System.Threading.Tasks.Dataflow
{
          ...
          IDebuggerDisplay
    {
        ...
        )
        {
            ...
            _source = new BroadcastingSourceCore<T>(
                this,
                cloningFunction,
            ...
            Common.WireCancellationToComplete(
                ...
                _source.Completion,
                state => ((BroadcastBlock<T>)state!).Complete(),
                this
            );
        ...
        internal void ConsumeAsyncIfNecessary(bool isReplacementReplica = false)
        {
            ...
            )
            {
                ...
                _boundingState.TaskForInputProcessing = new Task(
                    state => ((BroadcastBlock<T>)state!).ConsumeMessagesLoopCore(),
                    this,
        ...
        {
            get { return _source.GetDebuggingInformation().Value; }
        }
        ...
        private sealed class DebugView
        {
            ...
            public T Value
            {
                get { return _broadcastBlock.ValueForDebugger; }
            }
            ...
            public int Id
            {
                get { return Common.GetBlockId(_broadcastBlock); }
            }
        ...
        private sealed class BroadcastingSourceCore<TOutput>
        {
            ...
            )
            {
                ...
                Debug.Assert(
                    ...
                    "Options are required to configure this block."
                );
            ...
            private void OfferCurrentMessageToNewTarget(ITargetBlock<TOutput> target)
            {
                ...
                DataflowMessageStatus result = target.OfferMessage(
                    new DataflowMessageHeader(_nextMessageId),
                    currentMessage!,
                    _owningSource,
                    consumeToAccept: useCloning
                );
                ...
                else if (result == DataflowMessageStatus.DecliningPermanently)
                {
                    ...
                    Debug.Assert(
                        ...
                        "Messages from a Broadcast should never be missed."
                    );
            }
            ...
            private bool OfferToTargets()
            {
                ...
                lock (ValueLock)
                {
                    ...
                    if (_nextMessageReservedFor == null && _messages.Count > 0)
                    {
                        ...
                        Debug.Assert(
                            _messages.Count > 0,
                            "There must be at least one message to dequeue."
                        );
                ...
                if (header.IsValid)
                {
                    ...
                    if (_itemsRemovedAction != null)
                        _itemsRemovedAction(numDequeuedMessages);
            ...
            )
            {
                ...
                bool useCloning = _cloningFunction != null;
                switch (
                    target.OfferMessage(header, message, _owningSource, consumeToAccept: useCloning)
                )
                {
            ...
            private void CompleteBlockIfPossible_Slow()
            {
                Debug.Assert(
                    _taskForOutputProcessing == null,
                    "There must be no processing tasks."
                );
                Debug.Assert(
