...
namespace System.Threading.Tasks.Dataflow
{
            ...
            IDebuggerDisplay
    {
        ...
        public BatchBlock(int batchSize, GroupingDataflowBlockOptions dataflowBlockOptions)
        {
            ...
            if (
                dataflowBlockOptions.BoundedCapacity > 0
                && dataflowBlockOptions.BoundedCapacity < batchSize
            )
                throw new ArgumentOutOfRangeException(
                    nameof(batchSize),
                    SR.ArgumentOutOfRange_BatchSizeMustBeNoGreaterThanBoundedCapacity
                );
            ...
            if (dataflowBlockOptions.BoundedCapacity > 0)
            {
                ...
                itemCountingFunc = static (owningSource, singleOutputItem, multipleOutputItems) =>
                    BatchBlockTargetCore.CountItems(singleOutputItem, multipleOutputItems);
            }
            ...
            // Initialize target
            _target = new BatchBlockTargetCore(
                this,
                batchSize,
                _source.AddMessage,
                dataflowBlockOptions
        ...
        private sealed class DebugView
        {
            ...
            {
                get { return _targetDebuggingInformation.NumberOfBatchesCompleted; }
            }

            /// <summary>Gets the task being used for input processing.</summary>
            public Task? TaskForInputProcessing
            {
                get { return _targetDebuggingInformation.TaskForInputProcessing; }
            }
            ...
            public int BatchSize
            {
                get { return _batchBlock.BatchSize; }
            }
            ...
            public int Id
            {
                get { return Common.GetBlockId(_batchBlock); }
            }
        ...
        private sealed class BatchBlockTargetCore
        {
            ...
            private sealed class NonGreedyState
            {
                ...
                internal readonly List<
                    KeyValuePair<ISourceBlock<T>, KeyValuePair<DataflowMessageHeader, T>>
                > ReservedSourcesTemp;
            ...
            /// <exception cref="System.ArgumentOutOfRangeException">The <paramref name="batchSize"/> must be positive.</exception>
            /// <exception cref="System.ArgumentNullException">The <paramref name="dataflowBlockOptions"/> is null (Nothing in Visual Basic).</exception>
            internal BatchBlockTargetCore(
                BatchBlock<T> owningBatch,
                int batchSize,
            ...
            )
            {
                ...
                Debug.Assert(
                    ...
                    "Options required to configure the block."
                );
            ...
            )
            {
                ...
                lock (IncomingLock)
                {
                    ...
                    if (
                        _dataflowBlockOptions.Greedy
                        && (
                            _boundingState == null
                            || (
                                _boundingState.CountIsLessThanBound
                                && _nonGreedyState!.PostponedMessages.Count == 0
                    ...
                    {
                        Debug.Assert(
                            _nonGreedyState != null,
                            "_nonGreedyState must have been initialized during construction in non-greedy mode."
                        );
            ...
            )
            {
                ...
                if (releaseReservedMessages)
                {
                    ...
                    {
                        _owningBatch._source.AddException(e);
                    }
                }
            ...
            private bool CanceledOrFaulted
            {
                get
                {
                    return _dataflowBlockOptions.CancellationToken.IsCancellationRequested
                        || _owningBatch._source.HasExceptions;
                }
            }
            ...
            private void CompleteBlockIfPossible()
            {
                ...
                if (!_completionReserved)
                {
                    bool currentlyProcessing =
                        _nonGreedyState != null && _nonGreedyState.TaskForInputProcessing != null;
                    bool completedAllDesiredBatches =
                        _batchesCompleted >= _dataflowBlockOptions.ActualMaxNumberOfGroups;
                    bool noMoreMessages = _decliningPermanently && _messages.Count < _batchSize;

                    bool complete =
                        !currentlyProcessing
                        && (completedAllDesiredBatches || noMoreMessages || CanceledOrFaulted);
                    if (complete)
                    {
            ...
            private bool BatchesNeedProcessing
            {
                get
                {
                    ...
                    bool currentlyProcessing =
                        _nonGreedyState != null && _nonGreedyState.TaskForInputProcessing != null;
                    if (completedAllDesiredBatches || currentlyProcessing || CanceledOrFaulted)
                        return false;
                    ...
                    if (neededMessageCountToCompleteBatch <= 0)
                        return true;
                    ...
                    if (_nonGreedyState != null)
                    {
                        ...
                        if (
                            _nonGreedyState.AcceptFewerThanBatchSize
                            && _nonGreedyState.PostponedMessages.Count > 0
            ...
            private void ProcessAsyncIfNecessary(bool isReplacementReplica = false)
            {
                Debug.Assert(
                    _nonGreedyState != null,
                    "Non-greedy state is required for non-greedy mode."
                );
                Common.ContractAssertMonitorStatus(IncomingLock, held: true);
            ...
            private void ProcessAsyncIfNecessary_Slow(bool isReplacementReplica)
            {
                ...
                _nonGreedyState!.TaskForInputProcessing = new Task(
                    static thisBatchTarget =>
            ...
            private void ProcessMessagesLoopCore()
            {
                ...
                try
                {
                    ...
                    do
                    {
                        ...
                        bool triggered = Volatile.Read(
                            ref _nonGreedyState.AcceptFewerThanBatchSize
                        );
            ...
            private void RetrievePostponedItemsNonGreedy(bool allowFewerThanBatchSize)
            {
                ...
                QueuedMap<ISourceBlock<T>, DataflowMessageHeader> postponed =
                    _nonGreedyState.PostponedMessages;
                KeyValuePair<ISourceBlock<T>, DataflowMessageHeader>[] postponedTemp =
                    _nonGreedyState.PostponedMessagesTemp;
                List<
                    KeyValuePair<ISourceBlock<T>, KeyValuePair<DataflowMessageHeader, T>>
                > reserved = _nonGreedyState.ReservedSourcesTemp;
                ...
                lock (IncomingLock)
                {
                    ...
                    if (
                        ...
                        || (
                            ...
                            && (
                                postponed.Count < _batchSize
                                || boundedCapacityAvailable < _batchSize
                            )
                        )
                    )
                        return;
                ...
                for (int i = 0; i < poppedInitially; i++)
                {
                    ...
                    {
                        var reservedMessage = new KeyValuePair<DataflowMessageHeader, T>(
                        ...
                        >(sourceAndMessage.Key, reservedMessage);
                ...
                while (reserved.Count < _batchSize)
                {
                    ...
                    lock (IncomingLock)
                    {
                        if (!postponed.TryPop(out sourceAndMessage))
                            break;
                    } // Release the lock.  We must not hold it while calling Reserve/Consume/Release.
                    if (sourceAndMessage.Key.ReserveMessage(sourceAndMessage.Value, _owningBatch))
                    {
            ...
            private void RetrievePostponedItemsGreedyBounded(bool allowFewerThanBatchSize)
            {
                ...
                Debug.Assert(
                    _boundingState != null,
                    "Bounding state is required when in bounded mode."
                );
                Common.ContractAssertMonitorStatus(IncomingLock, held: false);
                ...
                lock (IncomingLock)
                {
                    ...
                    // Grab an initial batch of postponed messages.
                    if (boundedCapacityAvailable < itemCountNeededToCompleteBatch)
                        itemCountNeededToCompleteBatch = boundedCapacityAvailable;
                    poppedInitially = postponed.PopRange(
                        postponedTemp,
                        0,
                ...
                Debug.Assert(
                    reserved.Count <= itemCountNeededToCompleteBatch,
                    "Expected the number of reserved sources to be <= the number needed for a batch."
                );
            ...
            private void ConsumeReservedMessagesNonGreedy()
            {
                ...
                Debug.Assert(
                    _nonGreedyState.ReservedSourcesTemp != null,
                    "ReservedSourcesTemp should have been initialized."
                );
                Common.ContractAssertMonitorStatus(IncomingLock, held: false);
                ...
                for (int i = 0; i < reserved.Count; i++)
                {
                    ...
                    if (!consumed)
                    {
                        ...
                        for (int prev = 0; prev < i; prev++)
                            reserved[prev] = default(KeyValuePair<
                                ISourceBlock<T>,
                                KeyValuePair<DataflowMessageHeader, T>
                            >);
                        throw new InvalidOperationException(
                            SR.InvalidOperation_FailedToConsumeReservedMessage
                        );
                    }
                    ...
                    var consumedSourceAndMessage = new KeyValuePair<
                        ISourceBlock<T>,
                        KeyValuePair<DataflowMessageHeader, T>
                    >(sourceAndMessage.Key, consumedMessage);
                ...
                lock (IncomingLock)
                {
                    ...
                    if (_boundingState != null)
                        _boundingState.CurrentCount += reserved.Count;
            ...
            private void ConsumeReservedMessagesGreedyBounded()
            {
                ...
                lock (IncomingLock)
                {
                    // Increment the bounding count with the number of consumed messages
                    if (_boundingState != null)
                        _boundingState.CurrentCount += consumedCount;
            ...
            internal void ReleaseReservedMessages(bool throwOnFirstException)
            {
                ...
                for (int i = 0; i < reserved.Count; i++)
                {
                    ...
                    if (source != null && message.Key.IsValid)
                    {
                        try
                        {
                            source.ReleaseReservation(message.Key, _owningBatch);
                        }
                        catch (Exception e)
                        {
            ...
            internal void OnItemsRemoved(int numItemsRemoved)
            {
                Debug.Assert(
                    numItemsRemoved > 0,
                    "Should only be called for a positive number of items removed."
                );
                Common.ContractAssertMonitorStatus(IncomingLock, held: false);
            ...
            internal sealed class DebuggingInformation
            {
                ...
                public Task? TaskForInputProcessing
                {
                    get { return _target._nonGreedyState?.TaskForInputProcessing; }
                }

                /// <summary>Gets the collection of postponed messages.</summary>
                public QueuedMap<ISourceBlock<T>, DataflowMessageHeader>? PostponedMessages
                {
                    get { return _target._nonGreedyState?.PostponedMessages; }
                }
                ...
                {
                    get { return _target._batchesCompleted; }
                }
            }
        }
    }
