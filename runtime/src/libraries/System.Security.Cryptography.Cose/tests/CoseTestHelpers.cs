...
namespace System.Security.Cryptography.Cose.Tests
{
    public class CoseTestHelpers
    {
        ...
        internal const string SampleContentByteStringCborHex =
            "54546869732069732074686520636F6E74656E742E";
        ...
        internal static CoseHeaderMap GetHeaderMapWithAlgorithm(
        ...
        internal static List<(CoseHeaderLabel, ReadOnlyMemory<byte>)> GetExpectedProtectedHeaders(
        ...
        internal static List<(CoseHeaderLabel, ReadOnlyMemory<byte>)> GetEmptyExpectedHeaders() =>
            new();

        internal static void AddEncoded(
            List<(CoseHeaderLabel, ReadOnlyMemory<byte>)> list,
        ...
        )
        {
            ...
            {
                Assert.True(
                    msg.VerifyDetached(signingKey, expectedContent),
        ...
        )
        {
            ...
            for (int i = 0; i < expectedSignatures; i++)
            {
                ...
                AssertProtectedHeaders(
                    rawSignProtectedHeaders,
                    expectedSignProtectedHeaders ?? GetExpectedProtectedHeaders(algorithm)
                );
            ...
            {
                Assert.True(
                    signature.VerifyDetached(signingKey, expectedContent),
                    "msg.Verify(ecdsa, content)"
                );
            }
            else
            {
            ...
            AssertExtensions.SequenceEqual(
                encodedBodyProtectedHeaders,
                msg.RawProtectedHeaders.Span
            );
            ...
            for (int i = 0; i < signatures.Count; i++)
            {
                ...
                AssertExtensions.SequenceEqual(
                    listOfRawSignProtectedHeaders[i],
                    signatures[i].RawProtectedHeaders.Span
                );

                // Signature
                AssertExtensions.SequenceEqual(
                    listOfSignatureBytes[i],
                    signatures[i].Signature.Span
                );
            }
        ...
        private static void AssertProtectedHeaders(
            byte[] protectedHeadersBytes,
        ...
        )
        {
            ...
            while (reader.PeekState() != CborReaderState.EndMap)
            {
                ...
                {
                    CborReaderState.NegativeInteger or CborReaderState.UnsignedInteger =>
            ...
            foreach (
                (
                    CoseHeaderLabel expectedLabel,
                    ReadOnlyMemory<byte> expectedEncodedValue
                ) in expectedHeaders
            )
            {
                Assert.True(
                    headers.TryGetValue(expectedLabel, out CoseHeaderValue value),
                    "headers.TryGetValue(expectedLabel, out ReadOnlyMemory<byte> encodedValue)"
                );
                AssertExtensions.SequenceEqual(expectedEncodedValue.Span, value.EncodedValue.Span);
            }
        }

        private static ECParameters _ec256Parameters = CreateECParameters(
            "nistP256",
        ...
        private static ECParameters _ec384Parameters = CreateECParameters(
            "nistP384",
        ...
        private static ECParameters _ec512Parameters = CreateECParameters(
            "nistP521",
        ...
        private static ECDsa ES256WithoutPrivateKey =>
            t_es256WithoutPrivateKey ??= CreateECDsa(_ec256Parameters, false);
        private static ECDsa ES384WithoutPrivateKey =>
            t_es384WithoutPrivateKey ??= CreateECDsa(_ec384Parameters, false);
        private static ECDsa ES512WithoutPrivateKey =>
            t_es512WithoutPrivateKey ??= CreateECDsa(_ec512Parameters, false);

        internal static ECDsa DefaultKey => ES256;
        internal static HashAlgorithmName DefaultHash { get; } =
            GetHashAlgorithmNameFromCoseAlgorithm((int)ECDsaAlgorithm.ES256);
        ...
        internal static RSA RSAKeyWithoutPrivateKey =>
            t_rsaKeyWithoutPrivateKey ??= CreateRSA(false);

        private static ECParameters CreateECParameters(
            string curveFriendlyName,
            string base64UrlQx,
            string base64UrlQy,
            string base64UrlPrivateKey
        )
        {
        ...
        private static RSA CreateRSA(bool includePrivateKey)
        {
            var rsaParameters = new RSAParameters
            {
                Modulus = ByteUtils.HexToByteArray(
                    ""
                ),
                Exponent = ByteUtils.HexToByteArray("010001"),
                D = ByteUtils.HexToByteArray(
                    ""
                ),
                P = ByteUtils.HexToByteArray(
                    ""
                ),
                Q = ByteUtils.HexToByteArray(
                    ""
                ),
                DP = ByteUtils.HexToByteArray(
                    ""
                ),
                DQ = ByteUtils.HexToByteArray(
                    ""
                ),
                InverseQ = ByteUtils.HexToByteArray(
                    ""
                ),
            };
        ...
        ) GetKeyHashPaddingTriplet<T>(CoseAlgorithm algorithm, bool useNonPrivateKey = false)
        {
            return algorithm switch
            {
                CoseAlgorithm.ES256 => (
                    GetKey(ES256, ES256WithoutPrivateKey, useNonPrivateKey),
                    HashAlgorithmName.SHA256,
                    null
                ),
                CoseAlgorithm.ES384 => (
                    GetKey(ES384, ES384WithoutPrivateKey, useNonPrivateKey),
                    HashAlgorithmName.SHA384,
                    null
                ),
                CoseAlgorithm.ES512 => (
                    GetKey(ES512, ES512WithoutPrivateKey, useNonPrivateKey),
                    HashAlgorithmName.SHA512,
                    null
                ),
                CoseAlgorithm.PS256 => (
                    GetKey(RSAKey, RSAKeyWithoutPrivateKey, useNonPrivateKey),
                    HashAlgorithmName.SHA256,
            ...
            T GetKey(
                ...
                AsymmetricAlgorithm nonPrivateKey,
                bool useNonPrivateKey
            )
            {
                ...
                throw new InvalidOperationException(
                    $"Specified algorithm {algorithm} doesn't match the type {typeof(T)}"
                );
            }
        }

        internal static Stream GetTestStream(
            byte[] content,
            StreamKind streamKind = StreamKind.Normal
        )
        {
        ...
        internal static CoseSigner GetCoseSigner(
        ...
        internal static bool Sign1Verify(
            CoseMessage msg,
            AsymmetricAlgorithm key,
            byte[] content,
        ...
        )
        {
            ...
            return sign1Msg.Content.HasValue
                ? sign1Msg.VerifyEmbedded(key, associatedData)
                : sign1Msg.VerifyDetached(key, content, associatedData);
        }

        internal static bool MultiSignVerify(
            CoseMessage msg,
            AsymmetricAlgorithm key,
            byte[] content,
            int expectedSignatures,
        ...
        internal static void MultiSignAddSignature(
        ...
        internal static void WriteDummyCritHeaderValue(
            CborWriter writer,
            bool useIndefiniteLength = false
        )
        {
        ...
        internal static byte[] GetCounterSign(
            CoseMultiSignMessage msg,
        ...
        )
        {
            ...
            // signature
            (AsymmetricAlgorithm key, HashAlgorithmName hash, _) =
                GetKeyHashPaddingTriplet<AsymmetricAlgorithm>(algorithm);
            byte[] signatureBytes = GetSignature(
                key,
                hash,
                GetToBeSignedForCounterSign(msg, signature, encodedProtectedHeaders)
            );
        ...
        internal static bool VerifyCounterSign(
            AsymmetricAlgorithm key,
            HashAlgorithmName hash,
            byte[] toBeSigned,
            byte[] signature
        ...
        internal static byte[] GetToBeSignedForCounterSign(
            CoseMultiSignMessage msg,
            CoseSignature signature,
            byte[] signProtected
        )
        ...
        internal static (byte[], byte[]) ReadCounterSign(
