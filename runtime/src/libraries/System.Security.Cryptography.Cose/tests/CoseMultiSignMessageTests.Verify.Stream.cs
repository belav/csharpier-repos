...
namespace System.Security.Cryptography.Cose.Tests
{
    ...
    public class CoseMultiSignMessageTests_VerifyStream_Async
        : CoseMultiSignMessageTests_VerifyStream
    {
        ...
        internal override byte[] Sign(byte[] content, CoseSigner signer)
        {
            if (content == null)
            {
                return CoseMultiSignMessage
                    .SignDetachedAsync(null!, signer)
                    .GetAwaiter()
                    .GetResult();
            }
        ...
        public async Task VerifyAsyncWithUnseekableStream()
        {
            ...
            byte[] encodedMsg = await CoseMultiSignMessage.SignDetachedAsync(
                stream,
    ...
    public class CoseMultiSignMessageTests_VerifyStream_Sync
        : CoseMultiSignMessageTests_VerifyStream
    {
        internal override bool Verify(
            CoseMessage msg,
        ...
        {
            using Stream stream = GetTestStream(s_sampleContent);
            byte[] encodedMsg = CoseMultiSignMessage.SignDetached(
                stream,
                GetCoseSigner(DefaultKey, DefaultHash)
            );
