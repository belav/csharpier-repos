    ...
    public static partial class HttpClientJsonExtensions
    {
        ...
        )
        {
            using (HttpResponseMessage response = await taskResponse.ConfigureAwait(false))
            {
                ...
                return await ReadFromJsonAsyncHelper(
                        response.Content!,
                        type,
                        options,
                        cancellationToken
        ...
        )
        {
            using (HttpResponseMessage response = await taskResponse.ConfigureAwait(false))
            {
                ...
                // GetAsync will usually return Content as not-null.
                // If Content happens to be null, the extension will throw.
                return await ReadFromJsonAsyncHelper<T>(
                        response.Content!,
                        options,
                        cancellationToken
                    )
        ...
        )
        {
            using (HttpResponseMessage response = await taskResponse.ConfigureAwait(false))
            {
                ...
                return await response
                    .Content!.ReadFromJsonAsync(type, context, cancellationToken)
                    .ConfigureAwait(false);
            }
        }
        ...
        )
        {
            using (HttpResponseMessage response = await taskResponse.ConfigureAwait(false))
            {
                response.EnsureSuccessStatusCode();
                return await response
                    .Content!.ReadFromJsonAsync<T>(jsonTypeInfo, cancellationToken)
                    .ConfigureAwait(false);
            }
        }
    }
