...
namespace System.Net.Http.Json.Functional.Tests
{
    public class TranscodingReadStreamTest
    {
        ...
        public async Task ReadAsync_SingleByte()
        {
            ...
            using (
                TranscodingReadStream stream = new TranscodingReadStream(
        ...
        {
            int maxCharBufferSize = Encoding
        ...
        public Task ReadAsync_WithOverflowBuffer_WithBufferSize1(string input) =>
            ReadAsync_WithOverflowBufferAtCharBufferBoundaries(input, bufferSize: 1);
        ...
        private static async Task ReadAsyncTest(Encoding sourceEncoding, string message)
        {
            ...
            {
                object model = await JsonSerializer.DeserializeAsync(
                    transcodingStream,
                    typeof(TestModel)
                );
        ...
        public async Task TestOneToOneTranscodingAsync()
        {
            Encoding sourceEncoding = Encoding.GetEncoding(28591);
            string message =
                '"' + new string('A', TranscodingReadStream.MaxByteBufferSize - 2 + 1) + '"';

            Stream stream = new MemoryStream(sourceEncoding.GetBytes(message));
            using (
                TranscodingReadStream transcodingStream = new TranscodingReadStream(
                    stream,
                    sourceEncoding
                )
            )
