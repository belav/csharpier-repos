...
namespace System.CodeDom.Compiler.Tests
{
    public class CodeCompilerTests
    {
        ...
        [SkipOnTargetFramework(
            ...
            "Occasionally fails in .NET framework, probably caused from a very edge case bug in .NET Framework which we will not be fixing"
        )]
        public void GetResponseFileCmdArgs_ValidCmdArgs_ReturnsExpected(string cmdArgs)
        {
            var compiler = new Compiler();
            string args = compiler.GetResponseFileCmdArgsEntryPoint(
                new CompilerParameters(),
                cmdArgs
            );
        ...
        public void JoinStringArray_Invoke_ReturnsExpected(
            string[] sa,
            string separator,
            string expected
        )
        ...
        {
            public CompilerResults FromDomEntryPoint(
                CompilerParameters options,
                CodeCompileUnit e
            ) => FromDom(options, e);

            public CompilerResults FromDomBatchEntryPoint(
                CompilerParameters options,
                CodeCompileUnit[] ea
            ) => FromDomBatch(options, ea);

            public CompilerResults FromFileEntryPoint(
                CompilerParameters options,
                string fileName
            ) => FromFile(options, fileName);

            public CompilerResults FromFileBatchEntryPoint(
                CompilerParameters options,
                string[] fileNames
            ) => FromFileBatch(options, fileNames);

            public CompilerResults FromSourceEntryPoint(
                CompilerParameters options,
                string source
            ) => FromSource(options, source);

            public CompilerResults FromSourceBatchEntryPoint(
                CompilerParameters options,
                string[] sources
            ) => FromSourceBatch(options, sources);

            public string GetResponseFileCmdArgsEntryPoint(
                CompilerParameters options,
                string cmdArgs
            ) => GetResponseFileCmdArgs(options, cmdArgs);

            public string JoinStringArrayEntryPoint(string[] sa, string separator) =>
                JoinStringArray(sa, separator);
