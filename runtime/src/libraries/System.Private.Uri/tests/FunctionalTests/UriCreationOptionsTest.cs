    ...
    public class UriCreationOptionsTest
    {
        ...
        public static IEnumerable<object[]> DisableCanonicalization_TestData()
        {
            ...
            string[] rawTargets = pathAndQueries
                ...
                .SelectMany(pqf =>
                    unicodeInPathModes.Select(unicodeMode =>
                        unicodeMode switch
                        {
                            0 => pqf,
                            1 => "\u00F6" + pqf,
                    ...
                    foreach (string port in ports)
                    {
                        foreach (string rawTarget in rawTargets)
                        {
                            ...
                            string expectedPath = rawTarget.Substring(
                                0,
                                rawTarget.Length - expectedQuery.Length
                            );
        ...
        public void DisableCanonicalization_IsRespected(
            string uriString,
            string expectedHost,
            int expectedPort,
            string expectedPath,
        ...
        public void DisableCanonicalization_OnlyEqualToUrisWithMatchingFlag()
        {
            ...
            var absolute = new Uri(
                AbsoluteUri + Path,
                new UriCreationOptions { DangerousDisablePathAndQueryCanonicalization = false }
            );
            var absoluteRaw = new Uri(
                AbsoluteUri + Path,
            ...
            var absoluteRawCopy = new Uri(
                AbsoluteUri + Path,
                new UriCreationOptions { DangerousDisablePathAndQueryCanonicalization = true }
            );
            Equal(absoluteRaw, absoluteRawCopy);

            var absoluteRawDifferentPath = new Uri(
                AbsoluteUri + "/bar",
                new UriCreationOptions { DangerousDisablePathAndQueryCanonicalization = true }
            );
            NotEqual(absoluteRaw, absoluteRawDifferentPath);

            var absoluteRawSameAuthority = new Uri(
                AbsoluteUri + ":80" + Path,
                new UriCreationOptions { DangerousDisablePathAndQueryCanonicalization = true }
            );
        ...
        public static IEnumerable<object[]> ImplicitFilePaths_TestData()
        {
            ...
            static object[] Entry(string filePath) =>
                new object[] { $"{filePath}/{FilePathRawData}" };
        }
        ...
        public void DisableCanonicalization_WorksWithDifferentSchemes(string scheme)
        {
            if (scheme == "custom-registered")
            {
                ...
                UriParser.Register(
                    ...
                    scheme,
                    defaultPort: Random.Shared.Next(-1, 65536)
                );
            }
        ...
        {
            var uri = new Uri(
                "http://host/foo?bar=abc#fragment",
                new UriCreationOptions { DangerousDisablePathAndQueryCanonicalization = true }
            );
