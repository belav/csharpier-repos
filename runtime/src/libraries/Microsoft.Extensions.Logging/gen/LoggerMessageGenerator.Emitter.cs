            ...
            private static bool UseLoggerMessageDefine(LoggerMethod lm)
            {
                ...
                if (result)
                {
                    ...
                    foreach (string t in lm.TemplateList)
                    {
                        if (
                            !t.Equals(
                                lm.TemplateParameters[count].Name,
...
"
            ...
            private void GenVariableAssignments(LoggerMethod lm)
            {
                foreach (KeyValuePair<string, string> t in lm.TemplateMap)
                {
                    ...
                    if (index < lm.TemplateParameters.Count)
                    {
                        ...
                        {
                            _builder.AppendLine(
                                $"                var {t.Key} = this._{lm.TemplateParameters[index].Name};"
                            );
                        }
                    }
                }
            ...
            private void GenCases(LoggerMethod lm)
            {
                ...
                foreach (LoggerParameter p in lm.TemplateParameters)
                {
                    ...
                    _builder.AppendLine(
                        $"                    {index++} => new global::System.Collections.Generic.KeyValuePair<string, object?>(\"{name}\", this._{p.Name}),"
                    );
                }

                _builder.AppendLine(
                    $"                    {index++} => new global::System.Collections.Generic.KeyValuePair<string, object?>(\"{{OriginalFormat}}\", \"{ConvertEndOfLineAndQuotationCharactersToEscapeForm(lm.Message)}\"),"
                );
            }
