    ...
    public class CallSiteFactoryTest
    {
        ...
        public void GetService_FactoryCallSite_Transient_DoesNotFail()
        {
            ...
            for (int i = 0; i < 50; i++)
            {
                Assert.Equal(
                    expectedType,
                    serviceProvider.GetService(typeof(IFakeService)).GetType()
                );
                Assert.Equal(
                    expectedType,
                    serviceProvider.GetService(typeof(FakeService)).GetType()
                );
        ...
        public void CreateCallSite_ReturnsService_IfClosedTypeSatisfiesNewGenericConstraint()
        {
            ...
            var callSiteFactory = GetCallSiteFactory(
                ...
                new ServiceDescriptor(
                    typeof(TypeWithParameterlessPublicConstructor),
                    new TypeWithParameterlessPublicConstructor()
                )
            );
            // Act
            var matchingType =
                typeof(IFakeOpenGenericService<TypeWithParameterlessPublicConstructor>);
            var matchingCallSite = callSiteFactory(matchingType);
            // Assert
            Assert.NotNull(matchingCallSite);
        }
        ...
        public void CreateCallSite_ReturnsService_IfClosedTypeSatisfiesInterfaceGenericConstraint()
        {
            ...
            var callSiteFactory = GetCallSiteFactory(
                descriptor,
                new ServiceDescriptor(typeof(string), "")
            );
            // Act
        ...
        public void CreateCallSite_ReturnsService_IfClosedTypeSatisfiesAbstractClassGenericConstraint()
        {
            ...
            var callSiteFactory = GetCallSiteFactory(
                descriptor,
                new ServiceDescriptor(
                    typeof(ClassInheritingAbstractClass),
                    new ClassInheritingAbstractClass()
                )
            );
            // Act
        ...
        {
            // Arrange
            var classInheritingAbstractClassImplementationType =
                typeof(ClassWithAbstractClassConstraint<ClassInheritingAbstractClass>);
            var classInheritingAbstractClassDescriptor = new ServiceDescriptor(
                typeof(IFakeOpenGenericService<ClassInheritingAbstractClass>),
                classInheritingAbstractClassImplementationType,
                ServiceLifetime.Transient
            );
            var classAlsoInheritingAbstractClassImplementationType =
                typeof(ClassWithAbstractClassConstraint<ClassAlsoInheritingAbstractClass>);
            var classAlsoInheritingAbstractClassDescriptor = new ServiceDescriptor(
                typeof(IFakeOpenGenericService<ClassAlsoInheritingAbstractClass>),
                classAlsoInheritingAbstractClassImplementationType,
                ServiceLifetime.Transient
            );
            var classInheritingClassInheritingAbstractClassImplementationType =
                typeof(ClassWithAbstractClassConstraint<ClassInheritingClassInheritingAbstractClass>);
            var classInheritingClassInheritingAbstractClassDescriptor = new ServiceDescriptor(
                typeof(IFakeOpenGenericService<ClassInheritingClassInheritingAbstractClass>),
            ...
            var notMatchingDescriptor = new ServiceDescriptor(
                notMatchingServiceType,
                notMatchingType,
                ServiceLifetime.Transient
            );
        ...
        public void CreateCallSite_ReturnsMatchingTypes_IfClosedTypeSatisfiesBaseClassConstraintAndRegisteredType()
        {
            ...
            var descriptors = new[]
            {
                ...
                classAlsoInheritingAbstractClassDescriptor,
                new ServiceDescriptor(
                    typeof(ClassAlsoInheritingAbstractClass),
                    new ClassAlsoInheritingAbstractClass()
                ),
                classInheritingClassInheritingAbstractClassDescriptor,
                new ServiceDescriptor(
            ...
            Assert.Equal(
                matchingTypes,
                enumerableCall.ServiceCallSites.Select(scs => scs.ImplementationType).ToArray()
            );
        }
        ...
        )
        {
            ...
            var noConstraintDescriptor = new ServiceDescriptor(
                serviceType,
                noConstraintImplementationType,
                ServiceLifetime.Transient
            );
            var structImplementationType = typeof(ClassWithStructConstraint<>);
            var structDescriptor = new ServiceDescriptor(
                serviceType,
                structImplementationType,
                ServiceLifetime.Transient
            );
            var classImplementationType = typeof(ClassWithClassConstraint<>);
            var classDescriptor = new ServiceDescriptor(
                serviceType,
            ...
            var selfConstraintDescriptor = new ServiceDescriptor(
                serviceType,
                selfConstraintImplementationType,
                ServiceLifetime.Transient
            );
            ...
            var callSiteFactory = GetCallSiteFactory(
                noConstraintDescriptor,
                structDescriptor,
                classDescriptor,
                newDescriptor,
                interfaceDescriptor,
                selfConstraintDescriptor,
                serviceValueDescriptor
            );
            ...
            Assert.Equal(
                matchingImplementationTypes.Length,
                enumerableCall.ServiceCallSites.Length
            );
            Assert.Equal(
                matchingImplementationTypes,
                enumerableCall.ServiceCallSites.Select(scs => scs.ImplementationType).ToArray()
            );
        }
        ...
        [Theory]
        [MemberData(
            nameof(CreateCallSite_PicksConstructorWithTheMostNumberOfResolvedParametersData)
        )]
        ...
        public void CreateCallSite_ThrowsIfTypeHasSingleConstructorWithUnresolvableParameters()
        {
            ...
            var ex = Assert.Throws<InvalidOperationException>(() => callSiteFactory(type));
            Assert.Equal(
                $"Unable to resolve service for type '{typeof(IFakeService)}' while attempting to activate '{type}'.",
                ex.Message
            );
        }
        ...
        public void GetSlotTests(int numberOfServices)
        {
            ...
            var callsiteFactory = new CallSiteFactory(
                serviceDescriptors.Take(numberOfServices).ToArray()
            );

            for (int i = 0; i < numberOfServices; i++)
            {
                Assert.Equal(
                    numberOfServices - i - 1,
                    callsiteFactory.GetSlot(serviceDescriptors[i])
                );
            }
        }
        ...
        public void CreateCallSite_ThrowsIfTypeHasNoConstructurWithResolvableParameters(Type type)
        {
            ...
            var ex = Assert.Throws<InvalidOperationException>(() => callSiteFactory(type));
            Assert.Equal(
                $"No constructor for type '{type}' can be instantiated using services from the service container and default values.",
                ex.Message
            );
        }
        ...
        public static TheoryData CreateCallSite_ThrowsIfMultipleNonOverlappingConstructorsCanBeResolvedData =>
            new TheoryData<Type, Func<Type, object>, Type[][]>
            {
                ...
                },
                {
                    ...
                    new[] { new[] { typeof(IFactoryService) }, new[] { typeof(IFakeService) } }
                },
                {
        ...
        [Theory]
        [MemberData(
            nameof(CreateCallSite_ThrowsIfMultipleNonOverlappingConstructorsCanBeResolvedData)
        )]
        ...
        public static TheoryData<ServiceDescriptor[], object> EnumberableCachedAtLowestLevelData =
            new TheoryData<ServiceDescriptor[], object>()
            {
                {
                    new[]
                    {
        ...
        )
        {
            ...
            Assert.Equal(
                typeof(IEnumerable<FakeService>),
                callSite.Cache.Key.ServiceIdentifier.ServiceType
            );
        }
        ...
        public void CallSitesAreUniquePerServiceTypeAndSlot()
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                var factory = GetCallSiteFactory(
                    types.Select(t => ServiceDescriptor.Transient(t, t)).ToArray()
                );
            ...
            using RemoteInvokeHandle remoteHandle = RemoteExecutor.Invoke(
                () =>
                {
                    (Type, Type)[] invalidTestCases = new[]
                    {
                        ...
                        (
                            typeof(IServiceWithoutTrimmingAnnotations<>),
                            typeof(ServiceWithTrimmingAnnotations<>)
                        ),
                        (
                            typeof(IServiceWithPublicConstructors<>),
                            typeof(ServiceWithPublicProperties<>)
                        ),
                        (
                            typeof(IServiceWithTwoGenerics<,>),
                            typeof(ServiceWithTwoGenericsInvalid<,>)
                        ),
                    };
                    ...
                    (Type, Type)[] validTestCases = new[]
                    {
                        ...
                        (
                            typeof(IServiceWithPublicConstructors<>),
                            typeof(ServiceWithPublicConstructors<>)
                        ),
                        (
                            typeof(IServiceWithTwoGenerics<,>),
                            typeof(ServiceWithTwoGenericsValid<,>)
                        ),
                        (
                            typeof(IServiceWithMoreMemberTypes<>),
                            typeof(ServiceWithLessMemberTypes<>)
                        ),
                    };
        ...
        public void VerifyDynamicCodeNotSupportedChecks()
        {
            ...
            using RemoteInvokeHandle remoteHandle = RemoteExecutor.Invoke(
                () =>
                {
                    ...
                    ServiceCallSite callSite = callSiteFactory(
                        typeof(IFakeOpenGenericService<Class3>)
                    );
                    Assert.Equal(CallSiteKind.Constructor, callSite.Kind);
                    Assert.Equal(
                        typeof(ClassWithNoConstraints<Class3>),
                        callSite.ImplementationType
                    );

                    callSite = callSiteFactory(typeof(IServiceWithTwoGenerics<Class3, Class3>));
                    Assert.Equal(CallSiteKind.Constructor, callSite.Kind);
                    Assert.Equal(
                        typeof(ServiceWithTwoGenericsValid<Class3, Class3>),
                        callSite.ImplementationType
                    );
            ...
            Assert.Equal(CallSiteKind.Constructor, callSite.Kind);
            Assert.Equal(
                typeof(ServiceWithTwoGenericsValid<Class3, int>),
                callSite.ImplementationType
            );
            ...
            Assert.Equal(CallSiteKind.Constructor, callSite.Kind);
            Assert.Equal(
                typeof(ServiceWithTwoGenericsValid<int, Class3>),
                callSite.ImplementationType
            );
        ...
        private static IEnumerable<Type> GetParameters(ConstructorCallSite constructorCallSite) =>
            constructorCallSite.ConstructorInfo.GetParameters().Select(p => p.ParameterType);
