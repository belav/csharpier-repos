...
namespace System.Net.WebSockets.Client.Tests
{
    ...
    {
        public InvokerConnectTest_Http2(ITestOutputHelper output)
            : base(output) { }
    ...
    {
        public HttpClientConnectTest_Http2(ITestOutputHelper output)
            : base(output) { }
    ...
    {
        public HttpClientConnectTest_Http2_NoInvoker(ITestOutputHelper output)
            : base(output) { }
        ...
        public async Task ConnectAsync_Http2WithNoInvoker_ThrowsArgumentException(
    ...
    {
        public ConnectTest_Http2(ITestOutputHelper output)
            : base(output) { }
            ...
            await Http2LoopbackServer.CreateClientAndServerAsync(
                async uri =>
                {
                    ...
                    using (var cts = new CancellationTokenSource(TimeOutMilliseconds))
                    {
                        ...
                        HttpRequestException inner = Assert.IsType<HttpRequestException>(
                            ex.InnerException
                        );
                        Assert.Equal(
                            HttpRequestError.ExtendedConnectNotSupported,
                            inner.HttpRequestError
                        );
                        Assert.True(
                            ex.InnerException.Data.Contains("SETTINGS_ENABLE_CONNECT_PROTOCOL")
                        );
                    }
                },
                async server =>
                {
                    Http2LoopbackConnection connection = await server.EstablishConnectionAsync(
                        new SettingsEntry { SettingId = SettingId.EnableConnect, Value = 0 }
                    );
                },
        ...
        [SkipOnPlatform(
            TestPlatforms.Browser,
            "Self-signed certificates are not supported on browser"
        )]
        ...
        public async Task ConnectAsync_Http11Server_DowngradeFail()
        {
            ...
            using (var cts = new CancellationTokenSource(TimeOutMilliseconds))
            {
                ...
                Task t = cws.ConnectAsync(
                    Test.Common.Configuration.WebSockets.SecureRemoteEchoServer,
                    GetInvoker(),
                    cts.Token
                );
            ...
            await Http2LoopbackServer.CreateClientAndServerAsync(
                ...
                async server =>
                {
                    ...
                    (int streamId, HttpRequestData requestData) =
                        await connection.ReadAndParseRequestHeaderAsync(readBody: false);
        ...
        public async Task ConnectAsync_SameHttp2ConnectionUsedForMultipleWebSocketConnection()
        {
            await Http2LoopbackServer.CreateClientAndServerAsync(
                ...
                async server =>
                {
                    await using Http2LoopbackConnection connection =
                        await server.EstablishConnectionAsync(
                            new SettingsEntry { SettingId = SettingId.EnableConnect, Value = 1 }
                        );

                    (int streamId1, _) = await connection.ReadAndParseRequestHeaderAsync(
                        readBody: false
                    );
                    await connection.SendResponseHeadersAsync(
                        streamId1,
                    ...
                    (int streamId2, _) = await connection.ReadAndParseRequestHeaderAsync(
                        readBody: false
                    );
                    await connection.SendResponseHeadersAsync(
                        streamId2,
