...
namespace System.Net.WebSockets.Client.Tests
{
    public sealed class InvokerConnectTest : ConnectTest
    {
        ...
        public static IEnumerable<object[]> ConnectAsync_CustomInvokerWithIncompatibleWebSocketOptions_ThrowsArgumentException_MemberData()
        {
            ...
            yield return NoThrow(
                options => options.ClientCertificates = new X509CertificateCollection()
            );
            yield return Throw(
                options =>
                    options.RemoteCertificateValidationCallback = delegate
                    {
                        return true;
                    }
            );
            ...
            yield return NoThrow(
                options => options.DangerousDeflateOptions = new WebSocketDeflateOptions()
            );
        ...
        [MemberData(
            nameof(
                ConnectAsync_CustomInvokerWithIncompatibleWebSocketOptions_ThrowsArgumentException_MemberData
            )
        )]
        [SkipOnPlatform(TestPlatforms.Browser, "Custom invoker is ignored on Browser")]
        public async Task ConnectAsync_CustomInvokerWithIncompatibleWebSocketOptions_ThrowsArgumentException(
        ...
        )
        {
            using var invoker = new HttpMessageInvoker(
                ...
                {
                    ConnectCallback = (_, _) =>
                        ValueTask.FromException<Stream>(new Exception("ConnectCallback"))
                }
            );
            ...
            Task connectTask = ws.ConnectAsync(
                new Uri("wss://dummy"),
                invoker,
    ...
    public class ConnectTest : ClientWebSocketTestBase
    {
            ...
            using (var cws = new ClientWebSocket())
            {
                ...
                if (PlatformDetection.IsNetCore) // bug fix in netcoreapp: https://github.com/dotnet/corefx/pull/35960
                {
                    Assert.True(
                        ...
                        $"Actual WebSocketErrorCode {ex.WebSocketErrorCode} {ex.InnerException?.Message} \n {ex}"
                    );
                }
                Assert.Equal(WebSocketState.Closed, cws.State);
            }
        ...
        [SkipOnPlatform(
            ...
            "CollectHttpResponseDetails not supported on Browser"
        )]
        public async Task ConnectAsync_HttpResponseDetailsCollectedOnFailure()
        {
            await LoopbackServer.CreateClientAndServerAsync(
                ...
                server =>
                    server.AcceptConnectionSendResponseAndCloseAsync(HttpStatusCode.Unauthorized),
                new LoopbackServer.Options { WebSocketEndpoint = true }
            );
        }
                ...
                async uri =>
                {
                    ...
                    using (var cts = new CancellationTokenSource(TimeOutMilliseconds))
                    {
                        ...
                        Assert.Equal(
                            ...
                            clientWebSocket.HttpStatusCode
                        );
                        Assert.NotEmpty(clientWebSocket.HttpResponseHeaders);
                        Assert.Contains(
                            "Sec-WebSocket-Extensions",
                            clientWebSocket.HttpResponseHeaders
                        );
                    }
                },
                server =>
