...
namespace System.Net.WebSockets.Client.Tests
{
    ...
    {
        public InvokerMemorySendReceiveTest(ITestOutputHelper output)
            : base(output) { }
    ...
    {
        public HttpClientMemorySendReceiveTest(ITestOutputHelper output)
            : base(output) { }
    ...
    {
        public InvokerArraySegmentSendReceiveTest(ITestOutputHelper output)
            : base(output) { }
    ...
    {
        public HttpClientArraySegmentSendReceiveTest(ITestOutputHelper output)
            : base(output) { }
    ...
    {
        public MemorySendReceiveTest(ITestOutputHelper output)
            : base(output) { }
    ...
    {
        public ArraySegmentSendReceiveTest(ITestOutputHelper output)
            : base(output) { }
    ...
    public abstract class SendReceiveTest : ClientWebSocketTestBase
    {
        ...
        public SendReceiveTest(ITestOutputHelper output)
            : base(output) { }
        ...
        public async Task SendReceive_PartialMessageDueToSmallReceiveBuffer_Success(Uri server)
        {
            ...
            )
            {
                ...
                await cws.CloseAsync(
                    ...
                    "PartialMessageDueToSmallReceiveBufferTest",
                    ctsDefault.Token
                );
            }
        }
        ...
        [SkipOnPlatform(
            ...
            "JS Websocket does not support see issue https://github.com/dotnet/runtime/issues/46983"
        )]
        public async Task SendReceive_PartialMessageBeforeCompleteMessageArrives_Success(Uri server)
        {
            ...
            )
            {
                ...
                while (totalBytesReceived < receiveBuffer.Length)
                {
                    WebSocketReceiveResult recvResult = await ReceiveAsync(
                        ...
                        new ArraySegment<byte>(
                            receiveBuffer,
                            totalBytesReceived,
                            receiveBuffer.Length - totalBytesReceived
                        ),
                        ctsDefault.Token
                    );
                ...
                await cws.CloseAsync(
                    WebSocketCloseStatus.NormalClosure,
                    "PartialMessageBeforeCompleteMessageArrives",
                    ctsDefault.Token
                );
            }
        }
                ...
                catch (AggregateException ag)
                {
                    foreach (var ex in ag.InnerExceptions)
                    {
                        ...
                        else if (ex is WebSocketException)
                        {
                            ...
                            Assert.True(
                                (errCode == WebSocketError.InvalidState)
                                    || (errCode == WebSocketError.Success),
                                "WebSocketErrorCode"
                            );
                        }
                        else
        ...
        // This will also pass when no exception is thrown. Current implementation doesn't throw.
        [ActiveIssue(
            "https://github.com/dotnet/runtime/issues/83517",
            typeof(PlatformDetection),
                ...
                catch (Exception ex)
                {
                    if (ex is InvalidOperationException)
                    {
                        ...
                        Assert.True(
                            ...
                            cws.State + " state when InvalidOperationException"
                        );
                    }
                    else if (ex is WebSocketException)
                    {
                        ...
                        Assert.True(
                            WebSocketState.Aborted == cws.State,
                            cws.State + " state when WebSocketException"
                        );
                    ...
                    else if (ex is OperationCanceledException)
                    {
                        Assert.True(
                            WebSocketState.Aborted == cws.State,
                            cws.State + " state when OperationCanceledException"
                        );
                    }
                    else
                    {
        ...
        public async Task SendReceive_VaryingLengthBuffers_Success(Uri server)
        {
            ...
            )
            {
                ...
                await cws.CloseAsync(
                    WebSocketCloseStatus.NormalClosure,
                    "SendReceive_VaryingLengthBuffers_Success",
                    ctsDefault.Token
                );
            }
        }
        ...
        public async Task SendReceive_Concurrent_Success(Uri server)
        {
            ...
            {
                CancellationTokenSource ctsDefault = new CancellationTokenSource(
                    TimeOutMilliseconds
                );
                ...
                for (int i = 0; i < sendBuffer.Length; i++)
                {
                    Task<WebSocketReceiveResult> receive = ReceiveAsync(
                        cws,
                        new ArraySegment<byte>(receiveBuffer, receiveBuffer.Length - i - 1, 1),
                        ctsDefault.Token
                    );
                    Task send = SendAsync(
                        cws,
                        new ArraySegment<byte>(sendBuffer, i, 1),
                        WebSocketMessageType.Binary,
                        true,
                        ctsDefault.Token
                    );
        ...
        public async Task SendReceive_ConnectionClosedPrematurely_ReceiveAsyncFailsAndWebSocketStateUpdated()
        {
            ...
            Func<ClientWebSocket, LoopbackServer, Uri, Task> connectToServerThatAbortsConnection =
                async (clientSocket, server, url) =>
                {
                    var pendingReceiveAsyncPosted = new TaskCompletionSource(
                    ...
                    WebSocketException pendingReceiveException =
                        await Assert.ThrowsAsync<WebSocketException>(() => pendingReceiveAsync);

                    Assert.Equal(
                        WebSocketError.ConnectionClosedPrematurely,
                        pendingReceiveException.WebSocketErrorCode
                    );
                    ...
                    if (PlatformDetection.IsInAppContainer)
                    {
                        ...
                        Assert.Equal(
                            ...
                            (uint)pendingReceiveException.InnerException.HResult
                        );
                    }
        ...
        public async Task ZeroByteReceive_CompletesWhenDataAvailable(Uri server)
        {
            ...
            )
            {
                ...
                Task<WebSocketReceiveResult> t = ReceiveAsync(
                    cws,
