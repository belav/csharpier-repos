    ...
    public abstract class SendReceiveTest : ClientWebSocketTestBase
    {
        ...
        public async Task SendReceive_VaryingLengthBuffers_Success(Uri server)
        {
            ...
            catch (OperationCanceledException ex)
            {
                if (
                    ...
                    && ctsDefault != null
                    && ex.CancellationToken == ctsDefault.Token
                )
                {
                    _output.WriteLine(
                        $"ActiveIssue https://github.com/dotnet/runtime/issues/53957"
                    );
                    _output.WriteLine(
                        $"The test {nameof(SendReceive_VaryingLengthBuffers_Success)} took more than {TimeOutMilliseconds} to finish, it was canceled."
                    );
                    return;
                }
                throw;
        ...
        public async Task SendReceive_Concurrent_Success(Uri server)
        {
            ...
            catch (OperationCanceledException ex)
            {
                ...
                {
                    _output.WriteLine(
                        $"ActiveIssue https://github.com/dotnet/runtime/issues/57519"
                    );
                    _output.WriteLine(
                        $"The test {nameof(SendReceive_Concurrent_Success)} took more than {TimeOutMilliseconds} to finish, it was canceled."
                    );
                    return;
                }
                throw;
