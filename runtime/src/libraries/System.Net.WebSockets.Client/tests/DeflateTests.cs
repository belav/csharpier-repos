...
namespace System.Net.WebSockets.Client.Tests
{
    ...
    public class DeflateTests : ClientWebSocketTestBase
    {
        ...
        [InlineData(
            ...
            14,
            true,
            "permessage-deflate; client_max_window_bits; server_max_window_bits=14"
        )]
        [InlineData(
            10,
            true,
            11,
            true,
            "permessage-deflate; client_max_window_bits=10; server_max_window_bits=11"
        )]
        [InlineData(
            15,
            false,
        ...
        public async Task PerMessageDeflateHeaders(
            int clientWindowBits,
            bool clientContextTakeover,
            int serverWindowBits,
            bool serverContextTakover,
            string expected
        )
        {
            await LoopbackServer.CreateClientAndServerAsync(
                async uri =>
                {
                    ...
                    object webSocketHandle = typeof(ClientWebSocket)
                    ...
                    WebSocketDeflateOptions negotiatedDeflateOptions = (WebSocketDeflateOptions)
                        Type.GetType(
                                "System.Net.WebSockets.WebSocketHandle, System.Net.WebSockets.Client"
                            )
                            .GetField(
                    ...
                    Assert.Equal(
                        clientWindowBits - 1,
                        negotiatedDeflateOptions.ClientMaxWindowBits
                    );
                    Assert.Equal(
                    ...
                    Assert.Equal(
                        serverWindowBits - 1,
                        negotiatedDeflateOptions.ServerMaxWindowBits
                    );
                    Assert.Equal(
                        serverContextTakover,
                        negotiatedDeflateOptions.ServerContextTakeover
                    );
                },
                server =>
