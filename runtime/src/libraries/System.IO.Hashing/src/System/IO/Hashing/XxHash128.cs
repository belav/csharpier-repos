    ...
    public sealed unsafe class XxHash128 : NonCryptographicHashAlgorithm
    {
        ...
        private Hash128 GetCurrentHashAsHash128()
        {
            ...
            if (_state.TotalLength > MidSizeMaxBytes)
            {
                ...
                fixed (byte* secret = _state.Secret)
                {
                    ...
                    current.Low64 = MergeAccumulators(
                        accumulators,
                        secret + SecretMergeAccsStartBytes,
                        _state.TotalLength * Prime64_1
                    );
                    current.High64 = MergeAccumulators(
                        accumulators,
                        secret
                            + SecretLengthBytes
                            - AccumulatorCount * sizeof(ulong)
                            - SecretMergeAccsStartBytes,
                        ~(_state.TotalLength * Prime64_2)
                    );
                }
            }
            else
        ...
        private static void WriteBigEndian128(in Hash128 hash, Span<byte> destination)
        {
            ...
            Unsafe.WriteUnaligned(
                ref Unsafe.AddByteOffset(ref dest0, new IntPtr(sizeof(ulong))),
                low
            );
        }
        ...
        private static Hash128 HashLength0To16(byte* source, uint length, ulong seed)
        {
            ...
            return new Hash128(
                XxHash64.Avalanche(seed ^ BitFlipL),
                XxHash64.Avalanche(seed ^ BitFlipH)
            );
        }
        ...
        private static Hash128 HashLength1To3(byte* source, uint length, ulong seed)
        {
            ...
            uint combinedh = BitOperations.RotateLeft(
                BinaryPrimitives.ReverseEndianness(combinedl),
                13
            );
            const uint SecretXorL = (
                unchecked((uint)DefaultSecretUInt64_0) ^ (uint)(DefaultSecretUInt64_0 >> 32)
            );
            const uint SecretXorH = (
                unchecked((uint)DefaultSecretUInt64_1) ^ (uint)(DefaultSecretUInt64_1 >> 32)
            );
        ...
        private static Hash128 HashLength9To16(byte* source, uint length, ulong seed)
        {
            ...
            ulong m128High = Multiply64To128(
                inputLo ^ inputHi ^ bitflipl,
                Prime64_1,
                out ulong m128Low
            );
        ...
        private static Hash128 HashLengthOver240(byte* source, uint length, ulong seed)
        {
            ...
            fixed (byte* defaultSecret = &MemoryMarshal.GetReference(DefaultSecret))
            {
                ...
                Hash128 h128;
                h128.Low64 = MergeAccumulators(
                    accumulators,
                    secret + SecretMergeAccsStartBytes,
                    length * Prime64_1
                );
                h128.High64 = MergeAccumulators(
                    accumulators,
                    secret
                        + SecretLengthBytes
                        - AccumulatorCount * sizeof(ulong)
        ...
        private static Hash128 AvalancheHash(ulong accLow, ulong accHigh, uint length, ulong seed)
        {
            ...
            ulong h128High =
                (accLow * Prime64_1) + (accHigh * Prime64_4) + ((length - seed) * Prime64_2);
            h128Low = Avalanche(h128Low);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void Mix32Bytes(
            ref ulong accLow,
            ref ulong accHigh,
            byte* input1,
            byte* input2,
