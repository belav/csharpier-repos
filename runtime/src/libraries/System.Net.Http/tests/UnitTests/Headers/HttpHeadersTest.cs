...
namespace System.Net.Http.Tests
{
    ...
    {
        // Note: These are not real known headers, so they won't be returned if we call
        // HeaderDescriptor.Get().
        ...
        public void TryAddWithoutValidation_AddTwoValidValuesAsOneString_BothValuesParsed()
        {
            ...
            // The parser gets called for each value in the raw string. I.e. if we have 1 raw string containing
            // two
        ...
        public void TryAddWithoutValidation_AddNullAndEmptyValuesToKnownHeader_HeaderNotRemovedFromCollection()
        {
            ...
            // TryAddWithoutValidation() adds 'null' as string.empty to distinguish between an empty raw value
            // and no raw
            // value. When the parser is called later, the parser can decide whether empty strings are valid or
            // not.
            // In our case the MockParser returns 'success' with a parsed value of 'null' indicating that it is
            // OK to
            // have empty values.
        ...
        public void TryAddWithoutValidation_AddNullAndEmptyValuesToUnknownHeader_TwoEmptyStringsAddedAsValues()
        {
            ...
            // TryAddWithoutValidation() adds 'null' as string.empty to distinguish between an empty raw value
            // and no raw
            // value. For custom headers we just add what the user gives us. I.e. the result is a header with
            // two empty
            // values.
        ...
        public void TryAddWithoutValidation_AddMultipleValueToSingleValueHeaders_FirstHeaderAddedOthersAreInvalid()
        {
            ...
            // Note that the first value was parsed and added to the 'parsed values' list. The second value
            // however
        ...
        public void TryAddWithoutValidation_AddMultipleValueStringToSingleValueHeaders_MultipleValueStringAddedAsInvalid()
        {
            ...
            // Since parsing the header value fails because it is composed of 2 values, the original string is
            // added
        ...
        public void TryAddWithoutValidation_AddValueContainingNewLine_InvalidValueSaved(
            string headerValue
        )
        {
        ...
        public void Add_MultipleAddValidValueThenAddInvalidValuesToExistingHeader_ThrowAndDontAddHeader()
        {
            ...
            // Make sure the header did not get removed due to the failed add. Note that the first value in the
            // array
        ...
        public void Add_SingleFirstTryAddWithoutValidationForValidValueThenAdd_TwoParsedValuesAdded()
        {
            ...
            // Add() should trigger parsing. TryAddWithoutValidation() doesn't trigger parsing, but Add()
            // triggers
        ...
        public void Add_SingleFirstAddThenTryAddWithoutValidation_TwoParsedValuesAdded()
        {
            ...
            // Add() should trigger parsing. Since TryAddWithoutValidation() is called afterwards the second
            // value is
            // not parsed yet.
            Assert.Equal(1, headers.Parser.TryParseValueCallCount);
        ...
        public void Add_SingleFirstAddThenTryAddWithoutValidationToSingleValueHeader_BothParsedAndInvalidValue()
        {
            ...
            // Add() succeeds since we don't have a value added yet. TryAddWithoutValidation() also succeeds,
            // however
        ...
        public void RemoveParsedValue_ParserWithNoEqualityComparer_CaseSensitiveComparison()
        {
            ...
            // Since we don't provide a comparer, the default string.Equals() is called which is case-sensitive.
            // So
        ...
        public void RemoveParsedValue_AddTwoValuesToKnownHeaderAndCompareWithValueThatDiffersInCase_CustomComparerUsedForComparison()
        {
            ...
            // Our custom comparer (MockComparer) does case-insensitive value comparison. Verify that our custom
            // comparer is used to compare the header value.
            // Note that since we added 2 values a different code path than in the previous test is used. In
            // this
        ...
        public void RemoveParsedValue_FirstAddNewlineCharsValueThenCallRemoveParsedValue_InvalidValueRemains()
        {
            ...
            Assert.True(headers.Contains(headers.Descriptor));
            Assert.Equal(
                new[] { invalidHeaderValue + "\r\ninvalid" },
                headers.GetValues(headers.Descriptor)
            );
        }
        ...
        public void Clear_AddMultipleHeadersAndThenClear_NoHeadersInCollection()
        {
            ...
            // The call to Count() triggers a TryParseValue for the TryAddWithoutValidation() value. Clear()
            // should
        ...
        public void GetValues_HeadersWithEmptyValues_ReturnsEmptyArray()
        {
            ...
            // In the known header case, the MockParser accepts empty values but tells the store to not add the
            // value.
            // Since no value is added for 'knownHeader', HttpHeaders removes the header from the store. This is
            // only
            ...
            // In the custom header case, we add whatever the users adds (besides that we add string.Empty if
            // the
        ...
        public void GetSingleParsedValue_GetSingleValueForExistingHeader_ReturnsAddedValue()
        {
            ...
            // If we only have one value, then GetValues() should return just the value and not wrap it in a
            // List<T>.
        ...
        public void GetSingleParsedValue_GetValuesForExistingHeaderWithInvalidValues_ReturnsOnlyParsedValues()
        {
            ...
            // parsed, however it will be added to the 'invalid values' list and thus is not part of the
            // collection
            ...
            // Since we added only one valid value to 'knownHeader', we expect GetValues() to return a that
            // value.
        ...
        public void Contains_CallContainsForExistingHeader_ReturnsTrue()
        {
            ...
            // Contains() should trigger parsing of values added with TryAddWithoutValidation(): If the value
            // was invalid,
        ...
        public void GetEnumerator_InvalidValueBetweenValidHeaders_EnumeratorReturnsAllValidValuesAndDoesNotRemoveInvalidValue()
        {
            ...
            IDictionary<string, IEnumerable<string>> dict = headers.ToDictionary(
                pair => pair.Key,
                pair => pair.Value
            );
        ...
        public void SetParsedValue_SetTwoValuesToNonExistingHeader_HeaderGetsCreatedAndLastValueAdded()
        {
            ...
            // The following line will remove the previously added values and replace them with the provided
            // value.
        ...
        public void ContainsParsedValue_ContainsParsedValueForNonExistingHeader_ReturnsFalse()
        {
            ...
            // ContainsParsedValue() must not trigger raw value parsing for headers other than the requested
            // one.
        ...
        public void AddHeaders_SourceAndDestinationStoreHaveMultipleHeaders_OnlyHeadersNotInDestinationAreCopiedFromSource()
        {
            ...
            // This header value gets added even though it doesn't have values. But since this is a custom
            // header we
            ...
            // Unlike 'known3', 'custom7' was added even though it only had empty values. The reason is that
            // 'custom7'
        ...
        public void AddHeaders_ResponseHeaderToRequestHeaders_Success()
        {
            ...
            Assert.True(
                request.Headers.NonValidated.TryGetValues(Name, out HeaderStringValues nvValues)
            );
        ...
        public void AddHeaders_RequestHeaderToResponseHeaders_Success()
        {
            ...
            Assert.True(
                response.Headers.NonValidated.TryGetValues(Name, out HeaderStringValues nvValues)
            );
            Assert.Equal(Value, nvValues.Single());
        }
        ...
        [Theory]
        [MemberData(
            nameof(NumberOfHeadersUpToArrayThreshold_AddNonValidated_EnumerateNonValidated)
        )]
        public void Add_WithinArrayThresholdHeaders_EnumerationPreservesOrdering(
            int numberOfHeaders,
            bool addNonValidated,
            bool enumerateNonValidated
        )
        {
        ...
        public void Add_LargeNumberOfHeaders_OperationsStillSupported(int numberOfHeaders)
        {
            ...
            string[] nonValidatedKeysAfterValidation = headers
                .NonValidated.Select(pair => pair.Key)
                .ToArray();
            ...
            Assert.Equal(
                new[] { "1", "invalid", "valid" },
                headers.Select(pair => pair.Key).OrderBy(i => i)
            );
        ...
        public async Task ConcurrentReads_AreThreadSafe()
        {
            ...
            Task writerTask = Task.Run(() =>
            {
                while (Volatile.Read(ref running))
                    Volatile.Write(ref headers, CreateHeaders());
            });
            ...
            void ReaderWorker()
            {
                ...
                tests = new Action<HttpRequestHeaders>[]
                {
                    ...
                    static headers =>
                        Assert.True(
                            headers.TryGetValues("a", out var values) && values.Single() == "b"
                        ),
                    static headers => Assert.False(headers.TryGetValues("b", out _)),
                    static headers =>
                        Assert.True(
                            headers.NonValidated.TryGetValues("a", out var values)
                                && values.Single() == "b"
                        ),
                    static headers => Assert.False(headers.NonValidated.TryGetValues("b", out _)),
                    static headers =>
                        Assert.True(
                            headers.TryGetValues("f", out var values) && values.Single() == "g"
                        ),
                    static headers =>
                        Assert.True(
                            headers.NonValidated.TryGetValues("f", out var values)
                                && values.Single() == "g"
                        ),
                    static headers =>
                        Assert.True(
                            headers.TryGetValues("c", out var values)
                                && values.Count() == 2
                                && values.First() == "d"
                                && values.Last() == "e"
                        ),
                    static headers =>
                        Assert.True(
                            headers.NonValidated.TryGetValues("c", out var values)
                                && values.Count() == 2
                                && values.First() == "d"
                                && values.Last() == "e"
                        ),
                    static headers =>
                        Assert.True(
                            headers.TryGetValues("h", out var values)
                                && values.Count() == 2
                                && values.First() == "i"
                                && values.Last() == "j"
                        ),
                    static headers =>
                        Assert.True(
                            headers.NonValidated.TryGetValues("h", out var values)
                                && values.Count() == 2
                                && values.First() == "i"
                                && values.Last() == "j"
                        ),
                    static headers =>
                        Assert.Equal(
                            "only-if-cached, private",
                            headers.NonValidated["Cache-Control"].ToString()
                        ),
            ...
            static HttpRequestHeaders CreateHeaders()
            {
                ...
                {
                    static headers =>
                        headers.Accept.Add(new MediaTypeWithQualityHeaderValue("text/json")),
                    static headers =>
                ...
                foreach (
                    Action<HttpRequestHeaders> action in actions.OrderBy(_ => Random.Shared.Next())
                )
                {
                    action(headers);
                }
        ...
        {
            MockHeaders headers = new MockHeaders();
            AssertExtensions.ThrowsContains<FormatException>(
                () => headers.Remove("\u0080"),
                "\u0080"
            );
        }
        ...
        [InlineData("\r\n")]
        [InlineData("invalid\r\n")]
        public void TryGetValues_InvalidValuesContainingNewLines_ShouldNotRemoveInvalidValueAndShouldReturnRequestedValue(
            string value
        )
        {
            ...
            // The entry should still exist as the parsing during the validating access should not remove the
            // invalid value.
            Assert.Equal(1, headers.NonValidated.Count);
