...
namespace System.Net.Http.Tests
{
    public class HttpEnvironmentProxyTest
    {
        ...
        public void HttpProxy_Uri_Parsing(
            string _input,
            string _host,
            string _port,
            string _user,
            ...
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    Environment.SetEnvironmentVariable(
                        "all_proxy",
                        "http://foo:PLACEHOLDER@1.1.1.1:3000"
                    );
                    Assert.True(HttpEnvironmentProxy.TryCreate(out p));
                    Assert.NotNull(p);
                    ...
                    Environment.SetEnvironmentVariable(
                        "https_proxy",
                        "http://foo1:PLACEHOLDER1@1.1.1.1:3000"
                    );
                    Assert.True(HttpEnvironmentProxy.TryCreate(out p));
                    Assert.NotNull(p);
            ...
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    Assert.Equal(
                        ...
                        p.Credentials.GetCredential(p.GetProxy(fooHttp), "")
                    );
                    Assert.Equal(
                        CredentialCache.DefaultCredentials,
                        p.Credentials.GetCredential(p.GetProxy(fooHttps), "")
                    );
        ...
        public static IEnumerable<object[]> HttpProxyCgiEnvVarMemberData()
        {
            foreach (bool cgi in new object[] { false, true })
            {
                yield return new object[]
                {
                    "http_proxy",
                    cgi,
                    !cgi || !PlatformDetection.IsWindows,
                };
