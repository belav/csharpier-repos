...
namespace System.Net.Http.Functional.Tests
{
    public abstract class TelemetryTest : HttpClientHandlerTestBase
    {
        ...
        public void EventSource_ExistsWithCorrectId()
        {
            ...
            Assert.Equal(
                Guid.Parse("d30b5633-7ef1-5485-b4e0-94979b102068"),
        ...
        public void EventSource_SuccessfulRequest_LogsStartStop(string testMethod)
        {
            ...
            RemoteExecutor
                .Invoke(
                    static async (useVersionString, testMethod) =>
                    {
                                    ...
                                    .CreateClientAndServerAsync(
                                        async uri =>
                                        {
                                            ...
                                            using HttpClientHandler handler =
                                                CreateHttpClientHandler(version);
        ...
        {
            (EventWrittenEventArgs Event, Guid ActivityId)[] starts = events
                .Where(e => e.Event.EventName == "RequestStart")
                .ToArray();
            foreach (EventWrittenEventArgs startEvent in starts.Select(e => e.Event))
            {
                ...
                Assert.InRange(
                    (HttpVersionPolicy)startEvent.Payload[6],
            ...
            (EventWrittenEventArgs Event, Guid ActivityId)[] stops = events
                .Where(e => e.Event.EventName == "RequestStop")
                .ToArray();
            ...
            (EventWrittenEventArgs Event, Guid ActivityId)[] failures = events
                .Where(e => e.Event.EventName == "RequestFailed")
                .ToArray();
            (EventWrittenEventArgs Event, Guid ActivityId)[] detailedFailures = events
                .Where(e => e.Event.EventName == "RequestFailedDetailed")
                .ToArray();
        ...
        protected static void ValidateConnectionEstablishedClosed(
            ConcurrentQueue<(EventWrittenEventArgs Event, Guid ActivityId)> events,
            Version version,
            Uri uri,
        ...
        {
            EventWrittenEventArgs[] connectionsEstablished = events
                .Select(e => e.Event)
            ...
            foreach (EventWrittenEventArgs connectionEstablished in connectionsEstablished)
            {
                ...
                Assert.True(
                    ip.Equals(IPAddress.Loopback.MapToIPv6())
                        || ip.Equals(IPAddress.Loopback)
                        || ip.Equals(IPAddress.IPv6Loopback)
                );
            }
            Assert.True(
                connectionIds.SetEquals(Enumerable.Range(0, connectionCount).Select(i => (long)i)),
                "ConnectionEstablished has logged an unexpected connectionId."
            );

            EventWrittenEventArgs[] connectionsClosed = events
                .Select(e => e.Event)
                .Where(e => e.EventName == "ConnectionClosed")
                .ToArray();
            ...
            foreach (EventWrittenEventArgs connectionClosed in connectionsClosed)
            {
                ...
                Assert.Equal(
                    new[] { "versionMajor", "versionMinor", "connectionId" },
                    connectionClosed.PayloadNames
                );
                ...
                Assert.True(
                    ...
                    $"ConnectionClosed has logged an unexpected connectionId={connectionId}"
                );
            }
            Assert.Empty(connectionIds);
        }

        private static void ValidateRequestResponseStartStopEvents(
            ConcurrentQueue<(EventWrittenEventArgs Event, Guid ActivityId)> events,
        ...
        {
            (EventWrittenEventArgs Event, Guid ActivityId)[] requestHeadersStarts = events
            ...
            (EventWrittenEventArgs Event, Guid ActivityId)[] requestContentStarts = events
                .Where(e => e.Event.EventName == "RequestContentStart")
                .ToArray();
            ...
            (EventWrittenEventArgs Event, Guid ActivityId)[] requestContentStops = events
                .Where(e => e.Event.EventName == "RequestContentStop")
                .ToArray();
            ...
            (EventWrittenEventArgs Event, Guid ActivityId)[] responseHeadersStarts = events
                .Where(e => e.Event.EventName == "ResponseHeadersStart")
                .ToArray();
            Assert.Equal(count, responseHeadersStarts.Length);
            Assert.All(responseHeadersStarts, r => Assert.Empty(r.Event.Payload));

            (EventWrittenEventArgs Event, Guid ActivityId)[] responseHeadersStops = events
                .Where(e => e.Event.EventName == "ResponseHeadersStop")
                .ToArray();
            ...
            (EventWrittenEventArgs Event, Guid ActivityId)[] responseContentStarts = events
                .Where(e => e.Event.EventName == "ResponseContentStart")
                .ToArray();
            ...
            (EventWrittenEventArgs Event, Guid ActivityId)[] responseContentStops = events
        ...
        private static void ValidateEventCounters(
            ConcurrentQueue<(EventWrittenEventArgs Event, Guid ActivityId)> events,
            int requestCount,
            bool shouldHaveFailures,
            int versionMajor,
            bool requestLeftQueue = false
        )
        {
            ...
            Assert.True(
                eventCounters.TryGetValue("requests-started", out double[] requestsStarted)
            );
            Assert.Equal(requestCount, requestsStarted[^1]);

            Assert.True(
                eventCounters.TryGetValue("requests-started-rate", out double[] requestRate)
            );
            Assert.Contains(requestRate, r => r > 0);

            Assert.True(
                eventCounters.TryGetValue("requests-failed", out double[] requestsFailures)
            );
            Assert.True(
                eventCounters.TryGetValue("requests-failed-rate", out double[] requestsFailureRate)
            );
            if (shouldHaveFailures)
            {
            ...
            Assert.True(
                eventCounters.TryGetValue("current-requests", out double[] currentRequests)
            );
            ...
            Assert.True(
                eventCounters.TryGetValue(
                    ...
                    out double[] http11ConnectionsTotal
                )
            );
            ...
            Assert.True(
                eventCounters.TryGetValue(
                    ...
                    out double[] http20ConnectionsTotal
                )
            );
            ...
            Assert.True(
                eventCounters.TryGetValue(
                    ...
                    out double[] http30ConnectionsTotal
                )
            );
            ...
            Assert.True(
                eventCounters.TryGetValue(
                    ...
                    out double[] http11requestQueueDurations
                )
            );
            ...
            Assert.True(
                eventCounters.TryGetValue(
                    ...
                    out double[] http20requestQueueDurations
                )
            );
            ...
            Assert.True(
                eventCounters.TryGetValue(
                    ...
                    out double[] http30requestQueueDurations
                )
            );
                                    ...
                                    .CreateClientAndServerAsync(
                                        ...
                                        async server =>
                                        {
                                            ...
                                            await using (connection)
                                            {
                                                ...
                                                // Second request
                                                Assert.True(
                                                    await firstRequestFinished.WaitAsync(
                                                        TimeSpan.FromSeconds(10)
                                                    )
                                                );
                                                await connection.ReadRequestDataAsync(
        ...
        public void EventSource_Redirect_LogsRedirect()
        {
            RemoteExecutor
                .Invoke(
                    static async (string useVersionString) =>
                    {
                        Version version = Version.Parse(useVersionString);
                        ...
                        await listener.RunWithCallbackAsync(
                            ...
                            async () =>
                            {
                                await GetFactoryForVersion(version)
                                    .CreateServerAsync(
                                        (originalServer, originalUri) =>
                                        {
                                            return GetFactoryForVersion(version)
                                                .CreateServerAsync(
                                                    async (redirectServer, redirectUri) =>
                                                    {
                                                        ...
                                                        serverTask =
                                                            redirectServer.HandleRequestAsync();
                        ...
                        EventWrittenEventArgs redirectEvent = events
                            .Where(e => e.Event.EventName == "Redirect")
                            .Single()
                            .Event;
        ...
        public static bool SupportsRemoteExecutorAndAlpn =
            RemoteExecutor.IsSupported && PlatformDetection.SupportsAlpn;
        ...
        public void EventSource_Proxy_LogsIPAddress(bool useSsl)
        {
            ...
            RemoteExecutor
                .Invoke(
                    static async (string useVersionString, string useSslString) =>
                    {
                        using var listener = new TestEventListener(
                            "System.Net.Http",
                            EventLevel.Verbose,
                        ...
                        EventWrittenEventArgs[] connectionsEstablishedEvents = events
                            .Select(e => e.Event)
                            .Where(e => e.EventName == "ConnectionEstablished")
                            .ToArray();
        ...
        protected static async Task WaitForEventCountersAsync(
            ConcurrentQueue<(EventWrittenEventArgs Event, Guid ActivityId)> events
        )
        {
        ...
        public void EventSource_ParallelRequests_LogsNewConnectionIdForEachRequest()
        {
            RemoteExecutor
                .Invoke(async () =>
                {
                    ...
                    EventWrittenEventArgs[] requestHeadersStart = events
                        .Select(e => e.Event)
                    ...
                    foreach (EventWrittenEventArgs e in requestHeadersStart)
                    {
                        ...
                        Assert.True(
                            connectionIds.Remove(connectionId),
                            $"RequestHeadersStart has logged an unexpected connectionId={connectionId}."
                        );
                    }
                    Assert.Empty(connectionIds);
                })
