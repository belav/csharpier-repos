...
namespace System.Net.Http.Functional.Tests
{
    public abstract class TelemetryTest : HttpClientHandlerTestBase
    {
        ...
        private string QuicImplementationProvider =>
            UseQuicImplementationProvider?.GetType().Name ?? string.Empty;

        private static QuicImplementationProvider GetQuicImplementationProvider(string provider) =>
            provider.Contains(nameof(QuicImplementationProviders.MsQuic))
                ? QuicImplementationProviders.MsQuic
                : provider.Contains(nameof(QuicImplementationProviders.Mock))
        ...
        public void EventSource_SuccessfulRequest_LogsStartStop(string testMethod)
        {
            ...
            RemoteExecutor
                .Invoke(
                    static async (useVersionString, quicProvider, testMethod) =>
                    {
                        ...
                        await listener.RunWithCallbackAsync(
                            ...
                            async () =>
                            {
                                await GetFactoryForVersion(
                                        version,
                                        GetQuicImplementationProvider(quicProvider)
                                    )
        ...
        private static void ValidateConnectionEstablishedClosed(
            ConcurrentQueue<(EventWrittenEventArgs Event, Guid ActivityId)> events,
            Version version,
            int count = 1
        )
        ...
        public void EventSource_ConnectionPoolAtMaxConnections_LogsRequestLeftQueue()
        {
            if (
                UseVersion.Major == 3
                && UseQuicImplementationProvider == QuicImplementationProviders.Mock
            )
            {
                return;
            }
            ...
            RemoteExecutor
                .Invoke(
                    static async (useVersionString, quicProvider) =>
                    {
                        Version version = Version.Parse(useVersionString);
                        using var listener = new TestEventListener(
                            "System.Net.Http",
