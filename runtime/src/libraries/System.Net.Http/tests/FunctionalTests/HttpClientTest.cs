...
namespace System.Net.Http.Functional.Tests
{
    public sealed class HttpClientTest : HttpClientHandlerTestBase
    {
        ...
        public void GetAsync_NoBaseAddress_InvalidUri_ThrowsException(string uri)
        {
            ...
            )
            {
                Assert.Throws<InvalidOperationException>(
                    ...
                    {
                        client.GetAsync(
                            uri == null ? null : new Uri(uri, UriKind.RelativeOrAbsolute)
                        );
                    }
                );
            }
        ...
        {
            using Socket portReserver = new Socket(
            ...
            HttpRequestException ex = await Assert.ThrowsAsync<HttpRequestException>(
                () => client.GetStreamAsync($"http://localhost:{ep.Port}")
            );
        ...
        public async Task GetAsync_ContentCanBeCanceled(int getMode, int cancelMode)
        {
            ...
            await LoopbackServerFactory.CreateClientAndServerAsync(
                async uri =>
                {
                    try
                    {
                        Exception e = await Assert.ThrowsAsync<TaskCanceledException>(
                            async () =>
                            {
                                switch (getMode)
                                {
            ...
            foreach (var useSsl in BoolValues.Where(b => serverOptions.UseSsl || !b))
            {
                ...
                if (QuicImplementationProviders.Default.IsSupported)
                {
                    ...
                    yield return new object[]
                    {
                        ...
                        HttpVersion.Version30,
                        useSsl,
                        useSsl ? HttpVersion.Version30 : HttpVersion.Version11
                    };
                }
                ...
                if (QuicImplementationProviders.Default.IsSupported)
                {
                    yield return new object[]
                    {
                        ...
                        HttpVersion.Version30,
                        useSsl,
                        useSsl ? HttpVersion.Version20 : HttpVersion.Version11
                    };
                    yield return new object[]
                    {
                        HttpVersion.Version20,
                    ...
                    yield return new object[]
                    {
                        ...
                        HttpVersion.Version30,
                        useSsl,
                        useSsl ? (object)HttpVersion.Version30 : typeof(HttpRequestException)
                    };
                }
