...
namespace System.Net.Http.Functional.Tests
{
    ...
    public abstract class SocketsHttpHandler_Cancellation_Test : HttpClientHandler_Cancellation_Test
    {
        ...
        [InlineData(true)]
        [InlineData(false)]
        public async Task ConnectionFailure_AfterInitialRequestCancelled_SecondRequestSucceedsOnNewConnection(
            bool useSsl
        )
        {
            if (UseVersion == HttpVersion.Version30)
            ...
            await LoopbackServerFactory.CreateClientAndServerAsync(
                async uri =>
                {
                    ...
                    TaskCompletionSource tcsFirstConnectionInitiated = new TaskCompletionSource(
                        TaskCreationOptions.RunContinuationsAsynchronously
                    );
                    TaskCompletionSource tcsFirstRequestCanceled = new TaskCompletionSource(
                        TaskCreationOptions.RunContinuationsAsynchronously
                    );
                    ...
                    using (var client = CreateHttpClient(handler))
                    {
                        ...
                        socketsHandler.ConnectCallback = async (context, token) =>
                        {
                            ...
                            {
                                Assert.True(
                                    tcsFirstConnectionInitiated.Task.IsCompletedSuccessfully
                                );
                            }
                        ...
                        Task<HttpResponseMessage> t1 = client.SendAsync(
        ...
        public async Task RequestsCanceled_NoConnectionAttemptForCanceledRequests()
        {
            ...
            ) =>
            {
                ...
                if (context.InitialRequestMessage == request2)
                    seenRequest2 = true;
                if (context.InitialRequestMessage == request3)
                    seenRequest3 = true;
            ...
            await Assert
                .ThrowsAsync<TaskCanceledException>(() => request2Task)
            ...
            await Assert
                .ThrowsAsync<HttpRequestException>(() => request1Task)
                .WaitAsync(TestHelper.PassingTestTimeout);
            await Assert
                .ThrowsAsync<HttpRequestException>(() => request3Task)
                .WaitAsync(TestHelper.PassingTestTimeout);
        ...
        private static async Task CancelPendingRequest_DropsStalledConnectionAttempt_Impl(
            string versionString
        )
        {
            using var requestCts = new CancellationTokenSource();
            var requestCanceledTcs = new TaskCompletionSource(
                TaskCreationOptions.RunContinuationsAsynchronously
            );
            ...
            using var handler = new SocketsHttpHandler
            {
                ConnectCallback = async (context, cancellation) =>
                {
                    requestCts.Cancel();
                    await Assert
                        .ThrowsAsync<TaskCanceledException>(() => Task.Delay(-1, cancellation))
                        .WaitAsync(TestHelper.PassingTestTimeout);
        ...
        public void PendingConnectionTimeout_HighValue_PendingConnectionIsNotCancelled(int timeout)
        {
            ...
            RemoteExecutor
                .Invoke(
                    static async (versionString, timoutStr) =>
                    {
                        ...
                        using var requestCts = new CancellationTokenSource();
                        var connectionTestTcs = new TaskCompletionSource(
                            TaskCreationOptions.RunContinuationsAsynchronously
                        );
            ...
            await LoopbackServerFactory.CreateClientAndServerAsync(
                async uri =>
                {
                    ...
                    using HttpRequestMessage request = CreateRequest(
                        post ? HttpMethod.Post : HttpMethod.Get,
                        uri,
                        UseVersion
                    );
                    ...
                    if (post)
                    {
                        request.Content = new StreamContent(
                            new DelegateDelegatingStream(new MemoryStream())
                            {
                                CanSeekFunc = () => false,
                                CopyToFunc = (destination, _) =>
                                {
                                    destination.Flush();
                                    Assert.True(
                                        serverReceivedRequest.Task.Wait(
