...
namespace System.Net.Http.Functional.Tests
{
    ...
    public abstract class HttpClientHandlerTest_Headers : HttpClientHandlerTestBase
    {
        ...
        {
            const string HeaderKey = "some-header-123",
                HeaderValue = "this is the expected header value";
        ...
        {
            string userAgent =
                "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.18 Safari/537.36";
            ...
            await LoopbackServerFactory.CreateClientAndServerAsync(
                ...
                async server =>
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        Assert.Equal(
                            largeHeaderValue,
                            requestData.GetSingleHeaderValue("large-header" + i)
                        );
                    }
                }
            );
        ...
        public async Task SendAsync_SpecialHeaderKeyOrValue_Success(
            string key,
            string value,
            bool parsable
        )
        {
            if (
                ...
                && (key == "Content-Location" || key == "Date" || key == "Accept-CharSet")
            )
            {
            ...
            await LoopbackServerFactory.CreateClientAndServerAsync(
                async uri =>
                {
                    ...
                    HttpHeaders headers = contentHeader
                        ? (HttpHeaders)new StringContent("").Headers
        ...
        public async Task GetAsync_LargeHeader_Success(string headerName, int headerValueLength)
        {
            ...
            string headerValue = new string(
                rand.GetItems<char>("ABCDEFGHIJKLMNOPQRSTUVWXYZ", headerValueLength)
            );
            ...
            await LoopbackServerFactory.CreateClientAndServerAsync(
                async uri =>
                {
                    using (HttpClient client = CreateHttpClient())
                    {
                        ...
                        Assert.Equal(
                            isValid ? DateTime.Parse(value) : DateTimeOffset.MinValue,
                ...
                async server =>
                {
                    IList<HttpHeaderData> headers = new HttpHeaderData[]
                    {
                        new HttpHeaderData("Expires", value),
                    };
        ...
        public async Task SendAsync_GetWithValidHostHeader_Success(bool withPort)
        {
            ...
            m.Headers.Host =
                !PlatformDetection.LocalEchoServerIsAvailable && withPort
        ...
        public async Task SendAsync_GetWithInvalidHostHeader_ThrowsException()
        {
            ...
            using (
                HttpClient client = CreateHttpClient(
                    CreateHttpClientHandler(allowAllCertificates: false)
                )
            )
            {
                await Assert.ThrowsAsync<HttpRequestException>(() =>
        ...
        )[] s_nonAsciiHeaders = new[]
        {
            ...
            (
                ...
                Encoding.ASCII,
                ", ",
                new[] { "\u00FF", "\u00C4nd", "Ascii\u00A9", "Latin1-best-fit-to-ascii" }
            ),
            ...
            await LoopbackServerFactory.CreateClientAndServerAsync(
                async uri =>
                {
                    ...
                    )
                    {
                        ...
                        string expected = valueEncoding.GetString(
                            valueEncoding.GetBytes(string.Join(separator, values))
                        );
                ...
                async server =>
                {
                    List<HttpHeaderData> headerData = s_nonAsciiHeaders
                        .Select(h => new HttpHeaderData(
                            h.Name,
                            string.Join(h.Separator, h.Values),
                ...
                async (server, uri) =>
                {
                    ...
                    await server.AcceptConnectionAsync(async connection =>
                    {
                        ...
                        using (HttpResponseMessage response = await getResponse)
                        {
                            ...
                            Assert.Equal(
                                "chunked",
                                Assert.Single(response.Headers.GetValues("Transfer-Encoding"))
                            );
