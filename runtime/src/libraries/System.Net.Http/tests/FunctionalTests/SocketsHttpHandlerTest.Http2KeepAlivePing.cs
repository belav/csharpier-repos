...
namespace System.Net.Http.Functional.Tests
{
    ...
    public sealed class SocketsHttpHandler_Http2KeepAlivePing_Test : HttpClientHandlerTestBase
    {
        ...
        public async Task KeepAlivePingDelay_Infinite_NoKeepAlivePingIsSent()
        {
            await Http2LoopbackServer
                .CreateClientAndServerAsync(
                    async uri =>
                    {
                        SocketsHttpHandler handler = CreateSocketsHttpHandler(
        ...
        public async Task KeepAliveConfigured_KeepAlivePingsAreSentAccordingToPolicy(
            HttpKeepAlivePingPolicy policy
        )
        {
            await Http2LoopbackServer
                .CreateClientAndServerAsync(
                    ...
                    async server =>
                    {
                        ...
                        Assert.False(
                            unexpectedFrames.Any(),
                            "Received unexpected frames: \n"
                                + string.Join(
                                    '\n',
        ...
        private async Task GuardConnectionWriteAsync(
            Func<Task> action,
        ...
        private async Task<HeadersFrame> ReadRequestHeaderFrameAsync(
            bool expectEndOfStream = true,
            CancellationToken cancellationToken = default
        )
        {
            // Receive HEADERS frame for request.
            Frame frame = await _framesChannel.Reader
        ...
        private async Task<int> ReadRequestHeaderAsync(
            bool expectEndOfStream = true,
            CancellationToken cancellationToken = default
        )
        {
