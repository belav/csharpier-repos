...
namespace System.Net.Http.Functional.Tests
{
                        ...
                        await server.AcceptConnectionAsync(
                            async connection =>
                            {
                                ...
                                )
                                {
                                    ...
                                    Task lotsOfDataSent = connection.Socket
                                        .SendAsync(
            ...
            using (HttpClient client = CreateHttpClient())
            {
                await LoopbackServer.CreateServerAsync(
                    async (server, uri) =>
                    {
                        ...
                        await new[] { client.GetStringAsync(uri), serverTask2 }
                            .WhenAllOrAnyFailed();
    ...
    public sealed class SocketsHttpHandlerTest_LocationHeader
    {
        ...
        {
            return s_redirectResponseBefore.Concat(location)
    ...
    public sealed class SocketsHttpHandlerTest_Http2 : HttpClientHandlerTest_Http2
    {
        ...
        public async Task Http2_MultipleConnectionsEnabled_OpenAndCloseMultipleConnections_Success()
        {
            ...
            using (HttpClient client = CreateHttpClient(handler))
            {
                ...
                await connection0.ShutdownIgnoringErrorsAsync(
                    handleRequestTasks[0].Result.LastStreamId
                )
                    .ConfigureAwait(false);
                await connection2.ShutdownIgnoringErrorsAsync(
                    handleRequestTasks[2].Result.LastStreamId
                )
                    .ConfigureAwait(false);
        ...
        public async Task Http2_MultipleConnectionsEnabled_IdleConnectionTimeoutExpired_ConnectionRemovedAndNewCreated()
        {
            ...
            using (HttpClient client = CreateHttpClient(handler))
            {
                ...
                await connection1.WaitForClientDisconnectAsync(false)
                    .WaitAsync(TestHelper.PassingTestTimeout)
                    .ConfigureAwait(false);
                ...
                try
                {
                    await client.GetAsync(server.Address)
                        .WaitAsync(handler.PooledConnectionIdleTimeout)
                        .ConfigureAwait(false);
                }
                catch (Exception)
                {
                    ...
                    if (useSsl)
                    {
                        ...
                        using (
                            X509Certificate2 cert =
                                System.Net.Test.Common.Configuration.Certificates
