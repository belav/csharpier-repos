    ...
    public static class Program
    {
        ...
        )
        {
            ...
            config = new Configuration()
            {
                ...
                MaxRequestHeaderTotalSize = cmdline.ValueForOption<int>(
                    "-maxRequestHeaderTotalSize"
                ),
                ...
                DisplayInterval = TimeSpan.FromSeconds(
                    cmdline.ValueForOption<int>("-displayInterval")
                ),
                DefaultTimeout = TimeSpan.FromSeconds(
                    cmdline.ValueForOption<int>("-clientTimeout")
                ),
                ConnectionLifetime = cmdline
                    .ValueForOption<double?>("-connectionLifetime")
                    .Select(TimeSpan.FromMilliseconds),
                CancellationProbability = Math.Max(
                    0,
                    Math.Min(1, cmdline.ValueForOption<double>("-cancelRate"))
                ),
                MaximumExecutionTime = cmdline
                    .ValueForOption<double?>("-maxExecutionTime")
                ...
                ServerMaxConcurrentStreams = cmdline.ValueForOption<int?>(
                    "-serverMaxConcurrentStreams"
                ),
                ServerMaxFrameSize = cmdline.ValueForOption<int?>("-serverMaxFrameSize"),
                ServerInitialConnectionWindowSize = cmdline.ValueForOption<int?>(
                    "-serverInitialConnectionWindowSize"
                ),
                ServerMaxRequestHeaderFieldSize = cmdline.ValueForOption<int?>(
                    "-serverMaxRequestHeaderFieldSize"
                ),
            };
        ...
        private static async Task<ExitCode> Run(Configuration config)
        {
            ...
            (string name, Func<RequestContext, Task> op)[] usedClientOperations = (
                config.OpIndices,
                config.ExcludedOpIndices
            ) switch
            {
            ...
            string msQuicLibraryVersion = (string)
                msQuicApiType
                    .GetProperty(
            ...
            Console.WriteLine(
                " System.Net.Http: " + GetAssemblyInfo(typeof(System.Net.Http.HttpClient).Assembly)
            );
            ...
            Console.WriteLine(
                "  Client Tracing: "
                    + (
                        config.Trace && config.RunMode.HasFlag(RunMode.client)
                            ? "ON (client.log)"
                            : "OFF"
                    )
            );
            Console.WriteLine(
                "  Server Tracing: "
                    + (
                        config.Trace && config.RunMode.HasFlag(RunMode.server)
                            ? "ON (server.log)"
                            : "OFF"
                    )
            );
            ...
            Console.WriteLine(
                "        Lifetime: "
                    + (
                        config.ConnectionLifetime.HasValue
                            ? $"{config.ConnectionLifetime.Value.TotalMilliseconds}ms"
                            : "(infinite)"
                    )
            );
            Console.WriteLine(
                "      Operations: " + string.Join(", ", usedClientOperations.Select(o => o.name))
            );
            ...
            if (config.HttpVersion == HttpVersion.Version30 && IsQuicSupported)
            {
                unsafe
                {
                    ...
                    object msQuicApiInstance = msQuicApiType
                        ...
                        .GetGetMethod(true)!
                        .Invoke(null, Array.Empty<object?>())!;
                    QUIC_API_TABLE* apiTable = (QUIC_API_TABLE*)(
                        Pointer.Unbox(
                            msQuicApiType
                                .GetProperty("ApiTable")!
                                .GetGetMethod()!
                                .Invoke(msQuicApiInstance, Array.Empty<object?>())!
                        )
                    );
                    ...
                    if (
                        MsQuic.StatusFailed(
                            apiTable->SetParam(
                                null,
                                MsQuic.QUIC_PARAM_GLOBAL_SETTINGS,
            ...
            if (config.RunMode.HasFlag(RunMode.server))
            {
                ...
                Console.WriteLine(
                    $"Starting {(config.UseHttpSys ? "http.sys" : "Kestrel")} server."
                );
        ...
        private static S? Select<T, S>(this T? value, Func<T, S> mapper)
            where T : struct
            where S : struct
        {
