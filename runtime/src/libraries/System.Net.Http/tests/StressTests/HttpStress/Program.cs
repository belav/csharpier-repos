...
public static class Program
{
    ...
    private static bool TryParseCli(string[] args, [NotNullWhen(true)] out Configuration? config)
    {
        ...
        cmd.AddOption(
            new Option("-maxRequestHeaderCount", "Maximum number of headers to place in request")
            {
        ...
        config = new Configuration()
        {
            ...
            ConnectionLifetime = cmdline.ValueForOption<double?>("-connectionLifetime")
                .Select(TimeSpan.FromMilliseconds),
            CancellationProbability = Math.Max(
                0,
                Math.Min(1, cmdline.ValueForOption<double>("-cancelRate"))
            ),
            MaximumExecutionTime = cmdline.ValueForOption<double?>("-maxExecutionTime")
    ...
    private static async Task<ExitCode> Run(Configuration config)
    {
        ...
        ) switch
        {
            ...
            (_, int[] excl)
              => Enumerable.Range(0, clientOperations.Length)
                  .Except(excl)
