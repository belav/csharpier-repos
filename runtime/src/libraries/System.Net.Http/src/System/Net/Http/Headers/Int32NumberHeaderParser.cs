...
namespace System.Net.Http.Headers
{
    ...
    {
        // Note that we don't need a custom comparer even though we have a value type that gets boxed
        // (comparing two
        // equal boxed value types returns 'false' since the object instances used for boxing the two values
        // are
        // different). The reason is that the comparer is only used by HttpHeaders when comparing values in
        // a collection.
        // Value types are never used in collections (in fact HttpHeaderValueCollection expects T to be a
        // reference
        // type).
