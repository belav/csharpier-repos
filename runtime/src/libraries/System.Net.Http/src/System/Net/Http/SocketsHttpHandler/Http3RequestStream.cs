            ...
            IDisposable
    {
        ...
        public async Task<HttpResponseMessage> SendAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    sendRequestTask = SendContentAsync(
                        _request.Content!,
                        _requestBodyCancellationSource.Token
                    );
                }
                ...
                if (
                    ...
                    || await Task.WhenAny(sendRequestTask, readResponseTask).ConfigureAwait(false)
                        == sendRequestTask
                    || sendRequestTask.IsCompleted
                )
                {
                ...
                if (useEmptyResponseContent)
                {
                    ...
                    await DrainContentLength0Frames(_requestBodyCancellationSource.Token)
                        .ConfigureAwait(false);
            ...
            catch (QuicException ex) when (ex.QuicError == QuicError.StreamAborted)
            {
                ...
                switch (code)
                {
                    ...
                    case Http3ErrorCode.RequestRejected:
                        ...
                        HttpProtocolException rejectedException =
                    ...
                    default:
                        ...
                        HttpRequestError httpRequestError =
                            innerException is HttpProtocolException
                                ? HttpRequestError.HttpProtocolError
                                : HttpRequestError.Unknown;
                        throw new HttpRequestException(
                            httpRequestError,
            ...
            catch (QuicException ex) when (ex.QuicError == QuicError.ConnectionAborted)
            {
                ...
                when (ex.QuicError == QuicError.OperationAborted
            ...
            // It is possible for user's Content code to throw an unexpected OperationCanceledException.
                ...
                when (ex.CancellationToken == _requestBodyCancellationSource.Token
        ...
        )
        {
            ...
            if (remaining != -1)
            {
                ...
                if (!_singleDataFrameWritten)
                {
                    ...
                    await _stream
                        .WriteAsync(_sendBuffer.ActiveMemory, cancellationToken)
                        .ConfigureAwait(false);
        ...
        private void BufferHeaders(HttpRequestMessage request)
        {
            ...
            if (request.Content == null)
            {
                if (normalizedMethod.MustHaveRequestBody)
                {
                    ...
                    headerListSize +=
                        HttpKnownHeaderNames.ContentLength.Length + HeaderField.RfcOverhead;
                }
            }
            else
            {
        ...
        private async ValueTask<(
            Http3FrameType? frameType,
            long payloadLength
        )> ReadFrameEnvelopeAsync(CancellationToken cancellationToken)
        {
            long frameType,
                payloadLength;
            int bytesRead;
        ...
        )
        {
            ...
            while (headersLength != 0)
            {
                ...
                _headerDecoder.Decode(
                    _recvBuffer.ActiveSpan.Slice(0, processLength),
                    endHeaders,
                    this
                );
            ...
            switch (ex)
            {
                case QuicException e when (e.QuicError == QuicError.StreamAborted):
                    ...
                    throw HttpProtocolException.CreateHttp3StreamException(
                        (Http3ErrorCode)e.ApplicationErrorCode.Value,
                        e
                    );
                ...
                case QuicException e when (e.QuicError == QuicError.ConnectionAborted):
                    ...
                    Debug.Assert(e.ApplicationErrorCode.HasValue);
                    HttpProtocolException exception =
                        HttpProtocolException.CreateHttp3ConnectionException(
                            (Http3ErrorCode)e.ApplicationErrorCode.Value,
                            SR.net_http_http3_connection_close
                        );
