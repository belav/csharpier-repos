...
namespace System.Net.Http
{
    ...
    internal sealed class FailedProxyCache
    {
        ...
        // _failedProxies will only be flushed (rare but somewhat expensive) if we have more than this
        // number of proxies in our dictionary. See Cleanup() for details.
        ...
        /// <returns>If the proxy can be used, <see cref="Immediate"/>. Otherwise, the next <see
        // cref="Environment.TickCount64"/> that it should be used.</returns>
        ...
        /// <summary>
        /// Renews a proxy prior to its period expiring. Used when all proxies are failed to renew the proxy
        // closest to being renewed.
        /// </summary>
        /// <param name="uri">The <paramref name="uri"/> of the proxy to renew.</param>
        /// <param name="renewTicks">The current renewal time for the proxy. If the value has changed from
        // this, the proxy will not be renewed.</param>
        ...
        /// </summary>
        /// <remarks>
        /// I expect this to never be called by <see cref="Cleanup"/> in a production system. It is only
        // needed in the case
        /// that a system has a very large number of proxies that the PAC script cycles through. It is
        // moderately expensive,
        /// so it's only run periodically and is disabled until we exceed <see
        // cref="LargeProxyConfigBoundary"/> failed proxies.
        /// </remarks>
