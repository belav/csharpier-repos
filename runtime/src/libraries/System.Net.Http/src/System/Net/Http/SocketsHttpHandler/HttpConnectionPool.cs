...
namespace System.Net.Http
{
    ...
    internal sealed class HttpConnectionPool : IDisposable
    {
            ...
            if (_originAuthority != null)
            {
                ...
                hostHeader =
                    (
                        ...
                        != (sslHostName == null ? DefaultHttpPort : DefaultHttpsPort)
                    )
        ...
        )
        {
            ...
            try
            {
                (Stream stream, TransportContext? transportContext) = await ConnectAsync(
                        queueItem.Request,
                        true,
                        cts.Token
                    )
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    if (NetEventSource.Log.IsEnabled())
                        Trace($"QUIC connection failed: {e}");
                        ...
                        ) // prefer HTTP/1.1 if connection is not secured and downgrade is possible
                        {
                            ...
                            )
                            {
                                connection = await http2ConnectionWaiter
                                    .WaitForConnectionAsync(async, cancellationToken)
                                    .ConfigureAwait(false);
                            }
                                ...
                                if (request.IsExtendedConnectRequest)
                                {
                                    ...
                                    if (!connection.IsConnectEnabled)
                                    {
                                        HttpRequestException exception =
                                            new(SR.net_unsupported_extended_connect);
        ...
        private async ValueTask<(Stream, TransportContext?)> ConnectAsync(
            HttpRequestMessage request,
            bool async,
            CancellationToken cancellationToken
        )
        ...
        private sealed class HttpConnectionWaiter<T> : TaskCompletionSourceWithCancellation<T>
        {
            ...
            public async ValueTask<T> WaitForConnectionAsync(
                bool async,
