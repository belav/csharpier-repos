    ...
    internal sealed class HttpConnectionPool : IDisposable
    {
        ...
        private async Task AddHttp11ConnectionAsync(HttpRequestMessage request)
        {
            ...
            using (CancellationTokenSource cts = GetConnectTimeoutCancellationTokenSource())
            {
                try
                {
                    connection = await CreateHttp11ConnectionAsync(request, true, cts.Token)
            ...
            using (CancellationTokenSource cts = GetConnectTimeoutCancellationTokenSource())
            {
                try
                {
                    (Socket? socket, Stream stream, TransportContext? transportContext) =
                        await ConnectAsync(request, true, cts.Token).ConfigureAwait(false);
                    ...
                    {
                        connection = await ConstructHttp2ConnectionAsync(stream, request, cts.Token)
                            .ConfigureAwait(false);
                    }
                }
                catch (OperationCanceledException oce) when (oce.CancellationToken == cts.Token)
        ...
        )
        {
            ...
            while (true)
            {
                ...
                Http3Connection connection = await GetHttp3ConnectionAsync(
                        request,
                        authority,
                        cancellationToken
                    )
        ...
        )
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    {
                        response = await TrySendUsingHttp3Async(request, async, cancellationToken)
                            .ConfigureAwait(false);
                    }
        ...
        private async ValueTask<(Socket?, Stream, TransportContext?)> ConnectAsync(
            HttpRequestMessage request,
            bool async,
            CancellationToken cancellationToken
        )
        {
            ...
            switch (_kind)
            {
                ...
                case HttpConnectionKind.SocksTunnel:
                case HttpConnectionKind.SslSocksTunnel:
                    (socket, stream) = await EstablishSocksTunnel(request, async, cancellationToken)
                        .ConfigureAwait(false);
                    break;
            }
            ...
            if (IsSecure)
            {
                SslStream sslStream = await ConnectHelper
        ...
        private async ValueTask<(Socket?, Stream)> ConnectToTcpHostAsync(
            string host,
            int port,
        ...
        private async ValueTask<(Socket? socket, Stream stream)> EstablishSocksTunnel(
            HttpRequestMessage request,
            bool async,
            CancellationToken cancellationToken
        )
        {
            ...
            (Socket? socket, Stream stream) = await ConnectToTcpHostAsync(
                    _proxyUri.IdnHost,
                    _proxyUri.Port,
                    request,
                    async,
        ...
        {
            if (NetEventSource.Log.IsEnabled())
                Trace("HTTP/1.1 connection failed");
        ...
        public void ReturnHttp11Connection(HttpConnection connection, bool isNewConnection = false)
        {
            ...
            lock (SyncObj)
            {
                ...
                else
                {
                    ...
                    Debug.Assert(
                        _availableHttp11Connections.Count <= _maxHttp11Connections,
                        $"Expected {_availableHttp11Connections.Count} <= {_maxHttp11Connections}"
                    );
                    if (NetEventSource.Log.IsEnabled())
                        connection.Trace("Put connection in pool.");
                    return;
                }
            }
        ...
        public void ReturnHttp2Connection(Http2Connection connection, bool isNewConnection)
        {
            ...
            lock (SyncObj)
            {
                ...
                while (!_http2RequestQueue.IsEmpty)
                {
                    Debug.Assert(
                        (_availableHttp2Connections?.Count ?? 0) == 0,
                        $"With {_availableHttp11Connections.Count} available HTTP2 connections, we shouldn't have a waiter."
                    );
                    ...
                    if (NetEventSource.Log.IsEnabled())
                        connection.Trace("Dequeued waiting HTTP/2 request.");
                }
                ...
                else if (usable)
                {
                    ...
                    if (NetEventSource.Log.IsEnabled())
                        connection.Trace("Put HTTP/2 connection in pool.");
                    return;
                }
            }
        ...
        private struct RequestQueue<T>
        {
            ...
            )
            {
                ...
                TaskCompletionSourceWithCancellation<T> waiter =
                    new TaskCompletionSourceWithCancellation<T>();
            ...
            public bool TryFailNextRequest(Exception e)
            {
                Debug.Assert(
                    ...
                    "Unexpected exception type for connection failure"
                );
