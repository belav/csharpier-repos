...
namespace System.Net.Http
{
    public class MultipartContent : HttpContent, IEnumerable<HttpContent>
    {
        ...
        private static readonly SearchValues<char> s_allowedBoundaryChars = SearchValues.Create(
            ...
            if (boundary.Length > 70)
            {
                throw new ArgumentOutOfRangeException(
                    ...
                    SR.Format(
                        ...
                        SR.net_http_content_field_too_long,
                        70
                    )
                );
            }
        ...
        )
        {
            ...
            try
            {
                ...
                for (int contentIndex = 0; contentIndex < _nestedContent.Count; contentIndex++)
                {
                    ...
                    await content
                        .CopyToAsync(stream, context, cancellationToken)
                        .ConfigureAwait(false);
                }
...
#pragma warning restore CA2016
        ...
        private void SerializeHeadersToStream(Stream stream, HttpContent content, bool writeDivider)
        {
            ...
            // Add headers.
            foreach (
                KeyValuePair<string, HeaderStringValues> headerPair in content.Headers.NonValidated
            )
            {
                Encoding headerValueEncoding =
                    HeaderEncodingSelector?.Invoke(headerPair.Key, content)
        ...
        private static MemoryStream EncodeStringToNewStream(string input)
        {
            return new MemoryStream(
                HttpRuleParser.DefaultHttpEncoding.GetBytes(input),
                writable: false
            );
        }
        ...
        protected internal override bool TryComputeLength(out long length)
        {
            ...
            if (_nestedContent.Count > 1)
            {
                // Internal boundaries
                currentLength +=
                    (_nestedContent.Count - 1)
                    * (CrLfLength + DashDashLength + _boundary.Length + CrLfLength);
            }
