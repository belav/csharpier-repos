...
namespace System.Net.Http.Headers
{
    ...
    public abstract class HttpHeaders : IEnumerable<KeyValuePair<string, IEnumerable<string>>>
    {
        ...
        public override string ToString()
        {
            ...
            foreach (HeaderEntry entry in GetEntries())
            {
                ...
                else
                {
                    ...
                    string? separator =
                        entry.Key.Parser is HttpHeaderParser parser && parser.SupportsMultipleValues
                            ? parser.Separator
        ...
        private IEnumerator<KeyValuePair<string, IEnumerable<string>>> GetEnumeratorCore()
        {
            ...
            Debug.Assert(
                _count != 0 && entries is not null,
                "Caller should have validated the collection is not empty"
            );
        ...
        internal bool RemoveParsedValue(HeaderDescriptor descriptor, object value)
        {
            ...
            if (TryGetAndParseHeaderInfo(descriptor, out HeaderStoreItemInfo? info))
            {
                ...
                if (parsedValues == null)
                {
                    ...
                    {
                        Debug.Assert(
                            parsedValue.GetType() == value.GetType(),
                ...
                if (info.IsEmpty)
                {
                    ...
                    Debug.Assert(
                        headerRemoved,
                        $"Existing header '{descriptor.Name}' couldn't be removed."
                    );
                }
            ...
            lock (sourceInfo)
            {
                ...
                if (descriptor.Parser == null)
                {
                    ...
                    destinationInfo.ParsedAndInvalidValues = CloneStringHeaderInfoValues(
                        sourceInfo.ParsedAndInvalidValues
                    );
                }
                else
                {
                    ...
                    if (sourceInfo.ParsedAndInvalidValues != null)
                    {
                        List<object>? sourceValues =
                            sourceInfo.ParsedAndInvalidValues as List<object>;
                ...
                if (HttpRuleParser.ContainsNewLine(rawValue))
                {
                    if (NetEventSource.Log.IsEnabled())
                        NetEventSource.Error(
                            null,
                            SR.Format(
                                SR.net_http_log_headers_no_newlines,
                                descriptor.Name,
                                rawValue
                            )
                        );
        ...
        )
        {
            ...
            )
            {
                ...
                Debug.Assert(
                    index < value.Length,
                    "Parser must return an index value within the string length."
                );
                ...
                if (
                    parsedValues.Count == 0 && addWhenInvalid && info.ParsedAndInvalidValues is null
                )
                {
                    AddInvalidValue(info, value);
                }
        ...
        private static void AddValueToStoreValue<T>(T value, ref object? currentStoreValue)
            where T : class
        {
        ...
        )
        {
            if (!IsAllowedHeaderName(descriptor))
            {
                throw new InvalidOperationException(
                    SR.Format(SR.net_http_headers_not_allowed_header_name, descriptor.Name)
                );
            }
        ...
        )
        {
            ...
            if (!info.CanAddParsedValue(descriptor.Parser))
            {
                throw new FormatException(
                    SR.Format(
                        ...
                        SR.net_http_headers_single_value_header,
                        descriptor.Name
                    )
                );
            }
            ...
            int index = 0;
            object parsedValue = descriptor.Parser.ParseValue(
                value,
                info.ParsedAndInvalidValues,
                ref index
            );
            ...
            while (index < value.Length)
            {
                parsedValue = descriptor.Parser.ParseValue(
                    value,
                    info.ParsedAndInvalidValues,
                    ref index
                );
        ...
        private HeaderDescriptor GetHeaderDescriptor(string name)
        {
            ...
            {
                throw new FormatException(
                    string.Format(SR.net_http_headers_invalid_header_name, name)
                );
            }
            ...
            throw new InvalidOperationException(
                SR.Format(SR.net_http_headers_not_allowed_header_name, name)
            );
        }
        ...
        )
        {
            ...
            lock (info)
            {
                ...
                else
                {
                    Debug.Assert(
                        length > 1,
                        "The header should have been removed when it became empty"
                    );
                    values = multiValue = new string[length];
                }

                int currentIndex = 0;
                ReadStoreValues<object?>(
                    values,
                    info.ParsedAndInvalidValues,
                    descriptor.Parser,
                    ref currentIndex
        ...
        )
        {
            if (storeValue != null)
            {
                ...
                {
                    values[currentIndex] =
                        parser == null || storeValue is InvalidValue
                            ? storeValue.ToString()
                            : parser.ToString(storeValue);
                    currentIndex++;
                }
                else
                {
                    foreach (object? item in storeValues)
                    {
                        ...
                        values[currentIndex] =
                            parser == null || item is InvalidValue
                                ? item.ToString()
                                : parser.ToString(item);
                        currentIndex++;
                    }
                }
            }
        ...
        internal sealed class HeaderStoreItemInfo
        {
            ...
            private static void AssertContainsSingleParsedValue(List<object> list)
            {
                ...
                Debug.Assert(
                    count == 1,
                    "Only a single parsed value should be stored for this parser"
                );
            }
        ...
        internal HeaderEntry[]? GetEntriesArray()
        {
            ...
            HeaderEntry[] GetEntriesFromDictionary()
            {
                ...
                {
                    entries[i++] = new HeaderEntry { Key = entry.Key, Value = entry.Value };
                }
                return entries;
            }
        }
        ...
        private ref object GetValueRefOrNullRef(HeaderDescriptor key)
        {
            ...
            {
                valueRef = ref CollectionsMarshal.GetValueRefOrNullRef(
                    Unsafe.As<Dictionary<HeaderDescriptor, object>>(store),
                    key
                );
            }
        ...
        private ref object? GetValueRefOrAddDefault(HeaderDescriptor key)
        {
            ...
            ref object? ConvertToDictionaryAndAddDefault(HeaderDescriptor key)
            {
                ...
                return ref CollectionsMarshal.GetValueRefOrAddDefault(
                    dictionary,
                    key,
                    out s_dictionaryGetValueRefOrAddDefaultExistsDummy
                );
            }
            ...
            {
                var dictionary = (Dictionary<HeaderDescriptor, object>)_headerStore!;
                ref object? value = ref CollectionsMarshal.GetValueRefOrAddDefault(
                    dictionary,
                    key,
                    out s_dictionaryGetValueRefOrAddDefaultExistsDummy
                );
