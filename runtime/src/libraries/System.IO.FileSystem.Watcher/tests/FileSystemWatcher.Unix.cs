        ...
        public void FileSystemWatcher_Unix_DoesNotLeak()
        {
            ...
            RemoteExecutor.Invoke(
                testDirectory =>
                {
                    ...
                    Assert.Equal(
                        0,
                        Interop.Sys.SetRLimit(Interop.Sys.RlimitResources.RLIMIT_NOFILE, ref limits)
                    );
                    Assert.Equal(
                        0,
                        Interop.Sys.GetRLimit(Interop.Sys.RlimitResources.RLIMIT_NOFILE, out limits)
                    );
                    ...
                    catch (Exception e)
                    {
                        ...
                        Console.WriteLine(
                            $"Test failed for count={count}, limit={maxFd} and path='{testDirectory}'."
                        );
