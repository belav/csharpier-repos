...
namespace System.IO.Tests
{
    public class Directory_Move_Tests : FileSystemWatcherTest
    {
        ...
        [SkipOnPlatform(
            ...
            "Not supported on OSX/MacCatalyst."
        )]
        [InlineData(1)]
        [InlineData(2)]
        [InlineData(3)]
        ...
        private void DirectoryMove_DifferentWatchedDirectory(WatcherChangeTypes eventType)
        {
            ...
            using (var watcher = new FileSystemWatcher(TestDirectory, "*"))
            {
                ...
                ExpectEvent(
                    ...
                    action,
                    cleanup,
                    new string[] { sourceDir, adjacentDir }
                );
            }
        }
        ...
        )
        {
                ...
                .Select(
                    i =>
                        new
                        {
                            ...
                            DirectoryInUnwatchedDir = Path.Combine(
                                unwatchedTestDirectory,
                                $"dir{i}"
                            )
                        }
                )
            ...
            Action action = () =>
                Array.ForEach(
                    dirs,
                    dir => Directory.Move(dir.DirectoryInWatchedDir, dir.DirectoryInUnwatchedDir)
                );
            ...
            var expectedEvents = dirs.Select(
                dir => new FiredEvent(WatcherChangeTypes.Deleted, dir.DirectoryInWatchedDir)
            );
        ...
        private void DirectoryMove_Multiple_FromUnwatchedToWatched(int filesCount)
        {
            ...
            Action action = () =>
                Array.ForEach(
                    dirs,
                    dir => Directory.Move(dir.DirectoryInUnwatchedDir, dir.DirectoryInWatchedDir)
                );

            List<FiredEvent> events = ExpectEvents(watcher, filesCount, action);
            var expectedEvents = dirs.Select(
                dir => new FiredEvent(WatcherChangeTypes.Created, dir.DirectoryInWatchedDir)
            );
