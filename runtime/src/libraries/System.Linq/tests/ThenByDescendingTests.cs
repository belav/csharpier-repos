...
namespace System.Linq.Tests
{
    public class ThenByDescendingTests : EnumerableTests
    {
        ...
        public void DescendingKeyThenDescendingKey()
        {
            ...
            Assert.Equal(
                expected,
                source.OrderByDescending(e => e.Country).ThenByDescending(e => e.City)
            );
        }
...
And Immortality.".Split(new[] { ' ', '\n', '\r', '-' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            Assert.Equal(
                expected,
                source.OrderBy(word => char.IsUpper(word[0])).ThenByDescending(word => word.Length)
            );
        }
...
And Immortality.".Split(new[] { ' ', '\n', '\r', '-' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            Assert.Equal(
                expected,
                source.OrderBy(word => char.IsUpper(word[0]))
                    .ThenByDescending(
                        word => word.Length,
                        Comparer<int>.Create((w1, w2) => w2.CompareTo(w1))
                    )
            );
        }
        ...
        public void SortsLargeAscendingEnumerableCorrectly(int thenBys)
        {
            ...
            switch (thenBys)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    ordered = ordered.ThenByDescending(i => 0)
                        .ThenByDescending(i => 0)
        ...
        public void SortsLargeRandomizedEnumerableCorrectly(int thenBys)
        {
            ...
            switch (thenBys)
            {
                case 1:
                    ...
                    break;
                case 2:
                    orderedEnumerable = orderedEnumerable.ThenByDescending(i => 0)
                        .ThenByDescending(i => -i);
                    break;
                case 3:
                    orderedEnumerable = orderedEnumerable.ThenByDescending(i => 0)
