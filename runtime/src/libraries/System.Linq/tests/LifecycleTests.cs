    ...
    public class LifecycleTests : EnumerableTests
    {
        ...
        private static IEnumerable<Source> Sources()
        {
            foreach (int size in new[] { 0, 1, 2 })
            {
                yield return new Source(
                    $"Enumerable{size}",
                    NumberRangeGuaranteedNotCollectionType(0, size)
                );
            }
        }
        ...
        private static IEnumerable<Sink> Sinks()
        {
            ...
            yield return new Sink(
                ...
                e => e.SequenceEqual(Enumerable.Range(0, 1)),
                shortCircuits: true
            );
            ...
            private sealed class Enumerator : IEnumerator<T>
            {
                ...
                public void Reset() =>
                    throw new NotSupportedException(
                        $"LINQ operators should not invoke {nameof(Reset)}."
                    );
            }
        }
        ...
        {
            public Operation(string name, TWork operation, bool shortCircuits)
                : base(name, shortCircuits) => Work = operation;
        ...
        private sealed class Binary
            : Operation<Func<IEnumerable<int>, IEnumerable<int>, IEnumerable<int>>>
        {
            public Binary(
                string name,
