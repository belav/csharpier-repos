...
namespace System.Linq.Tests
{
    public class ToDictionaryTests : EnumerableTests
    {
        ...
        )
        {
            ...
            validation(
                new TestReadOnlyCollection<T>(items).ToDictionary(key => key, value => value)
            );
        ...
        private void RunToDictionaryFromKvOnAllCollectionTypes<T>(
            T[] items,
            Action<Dictionary<T, T>> validation
        )
            where T : notnull
        {
            KeyValuePair<T, T>[] kvps = items
                .Select(item => new KeyValuePair<T, T>(item, item))
                .ToArray();
        ...
        public void ToDictionary_PassCustomComparer()
        {
            ...
            Dictionary<int, int> result2 = collection.ToDictionary(
                key => key,
                val => val,
                comparer
            );
            ...
            Dictionary<int, int> result3 = collection
        ...
        public void ToDictionary_KeyValueSelectorsWork()
        {
            ...
            Dictionary<int, int> result = collection.ToDictionary(
                key => key + 10,
                val => val + 100
            );
        ...
        public void ThrowsOnNullKey()
        {
            ...
            var source3 = new[] { ("Chris", 50), ("Bob", 95), (default, 55) };
        ...
        private static void AssertMatches<K, E>(
            IEnumerable<K> keys,
        ...
        public void EmptySource()
        {
            ...
            AssertMatches(
                keys,
                elements,
                source.ToDictionary(e => e.Name, e => e.Score, new AnagramEqualityComparer())
            );
        }
