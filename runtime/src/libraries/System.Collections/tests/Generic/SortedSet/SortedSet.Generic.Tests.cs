        ...
        public void SortedSet_Generic_GetViewBetween_MiddleOfSet(int setLength)
        {
            if (setLength >= 3)
            {
                ...
                foreach (T value in set)
                    if (
                        comparer.Compare(value, firstElement) >= 0
                        && comparer.Compare(value, lastElement) <= 0
                    )
                        expected.Add(value);
        ...
        )
        {
            if (setLength >= 2)
            {
                ...
                if (comparer.Compare(firstElement, lastElement) < 0)
                    AssertExtensions.Throws<ArgumentException>("lowerValue", null, () =>
                        set.GetViewBetween(lastElement, firstElement)
                    );
            }
        }
