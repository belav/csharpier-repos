        ...
        ** Exceptions: ArgumentException if bytes == null.
        ...
        public unsafe BitArray(bool[] values)
        {
            ...
            if (Vector512.IsHardwareAccelerated)
            {
                for (
                    ;
                    i <= (uint)values.Length - Vector512<byte>.Count;
            ...
            else if (Vector256.IsHardwareAccelerated)
            {
                for (
                    ;
                    i <= (uint)values.Length - Vector256<byte>.Count;
            ...
            else if (Vector128.IsHardwareAccelerated)
            {
                for (
                    ;
                    i <= (uint)values.Length - Vector128<byte>.Count * 2u;
        ...
        **             value.Length != this.Length.
        ...
        public unsafe BitArray And(BitArray value)
        {
            ...
            if (Vector512.IsHardwareAccelerated && (uint)count >= Vector512<int>.Count)
            {
                for (
                    ;
                    i < (uint)count - (Vector512<int>.Count - 1u);
                    i += (uint)Vector512<int>.Count
                )
                {
                    Vector512<int> result =
                        Vector512.LoadUnsafe(ref left, i) & Vector512.LoadUnsafe(ref right, i);
            ...
            else if (Vector256.IsHardwareAccelerated && (uint)count >= Vector256<int>.Count)
            {
                for (
                    ;
                    i < (uint)count - (Vector256<int>.Count - 1u);
                    i += (uint)Vector256<int>.Count
                )
                {
            ...
            else if (Vector128.IsHardwareAccelerated && (uint)count >= Vector128<int>.Count)
            {
                for (
                    ;
                    i < (uint)count - (Vector128<int>.Count - 1u);
                    i += (uint)Vector128<int>.Count
                )
                {
        ...
        ** Exceptions: ArgumentOutOfRangeException if count < 0
        ...
        public unsafe void CopyTo(Array array, int index)
        {
            ...
            else if (array is bool[] boolArray)
            {
                ...
                {
                    Vector256<byte> upperShuffleMask_CopyToBoolArray256 = Vector256
                        .Create(
                            0x04040404_04040404,
                            0x05050505_05050505,
                            0x06060606_06060606,
                        ...
                        .AsByte();
                    Vector256<byte> lowerShuffleMask_CopyToBoolArray256 = Vector256.Create(
                        lowerShuffleMask_CopyToBoolArray,
                        upperShuffleMask_CopyToBoolArray
                    );
                    Vector512<byte> shuffleMask = Vector512.Create(
                        lowerShuffleMask_CopyToBoolArray256,
                        upperShuffleMask_CopyToBoolArray256
                    );
                    ...
                    fixed (bool* destination = &boolArray[index])
                    {
                        for (
                            ;
                            (i + Vector512<byte>.Count) <= (uint)m_length;
                            i += (uint)Vector512<byte>.Count
                        )
                        {
                            ulong bits =
                                (ulong)(uint)m_array[i / (uint)BitsPerInt32]
                                + ((ulong)m_array[(i / (uint)BitsPerInt32) + 1] << BitsPerInt32);
                            Vector512<ulong> scalar = Vector512.Create(bits);
                            Vector512<byte> shuffled = Avx512BW.Shuffle(
                                scalar.AsByte(),
                                shuffleMask
                            );
                ...
                else if (Avx2.IsSupported && (uint)m_length >= Vector256<byte>.Count)
                {
                    ...
                    fixed (bool* destination = &boolArray[index])
                    {
                        for (
                            ;
                            (i + Vector256<byte>.Count) <= (uint)m_length;
                            i += (uint)Vector256<byte>.Count
                        )
                        {
                ...
                else if (Ssse3.IsSupported && ((uint)m_length >= Vector512<byte>.Count * 2u))
                {
                    ...
                    fixed (bool* destination = &boolArray[index])
                    {
                        for (
                            ;
                            (i + Vector128<byte>.Count * 2u) <= (uint)m_length;
                            i += (uint)Vector128<byte>.Count * 2u
                        )
                        {
                            int bits = m_array[i / (uint)BitsPerInt32];
