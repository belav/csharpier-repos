    ...
    public sealed class BitArray : ICollection, ICloneable
    {
            ...
            else if (Sse2.IsSupported)
            {
                ...
                fixed (bool* ptr = values)
                {
                    ...
                    )
                    {
                        ...
                        Vector128<byte> upperVector = Sse2.LoadVector128(
                            (byte*)ptr + i + Vector128<byte>.Count
                        );
            ...
            else if (AdvSimd.Arm64.IsSupported)
            {
                ...
                fixed (bool* ptr = values)
                {
                    ...
                    )
                    {
                        ...
                        Vector128<byte> upperVector = AdvSimd.LoadVector128(
                            (byte*)ptr + i + Vector128<byte>.Count
                        );
                        ...
                        int result = AdvSimd.Arm64.ZipLow(lowerPackedIsFalse, upperPackedIsFalse)
                            .AsInt32()
                            .ToScalar();
        ...
        ** Exceptions: ArgumentOutOfRangeException if count < 0
        ...
        private static readonly Vector128<byte> s_lowerShuffleMask_CopyToBoolArray =
            Vector128.Create(0, 0x01010101_01010101).AsByte();
        private static readonly Vector128<byte> s_upperShuffleMask_CopyToBoolArray =
            Vector128.Create(0x02020202_02020202, 0x03030303_03030303).AsByte();
            ...
            else if (array is bool[] boolArray)
            {
                ...
                if (Avx2.IsSupported)
                {
                    Vector256<byte> shuffleMask = Vector256.Create(
                        s_lowerShuffleMask_CopyToBoolArray,
                        s_upperShuffleMask_CopyToBoolArray
                    );
                ...
                else if (Ssse3.IsSupported)
                {
                    ...
                    fixed (bool* destination = &boolArray[index])
                    {
                        ...
                        )
                        {
                            ...
                            Vector128<byte> extractedHigher = Sse2.And(
                                shuffledHigher,
                                s_bitMask128
                            );
                ...
                else if (AdvSimd.IsSupported)
                {
                    ...
                    fixed (bool* destination = &boolArray[index])
                    {
                        ...
                        )
                        {
                            ...
                            Vector128<byte> extractedLower = AdvSimd.And(
                                shuffledLower,
                                s_bitMask128
                            );
