    ...
    internal sealed class ValueHandle
    {
        ...
        public bool TryReadChars(char[] chars, int offset, int count, out int actual)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    if (
                        encoding.TryGetChars(
                            bytes.AsSpan(byteOffset, byteCount),
                            chars.AsSpan(charOffset, charCount),
                            out actualCharCount
                        )
                    )
                    {
                ...
                {
                    throw XmlExceptionHelper.CreateEncodingException(
                        bytes,
                        byteOffset,
                        byteCount,
                        exception
        ...
        {
            Debug.Assert(
                _type == ValueHandleType.Int64
                    || _type == ValueHandleType.TimeSpan
                    || _type == ValueHandleType.DateTime
            );
