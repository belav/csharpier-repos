...
namespace System.Xml
{
    internal abstract class XmlBaseWriter : XmlDictionaryWriter
    {
        ...
        private async Task WriteEndElementAsyncImpl()
        {
            ...
            {
                Element element = _elements![_depth];
                await _writer.WriteEndElementAsync(element.Prefix, element.LocalName!)
                    .ConfigureAwait(false);
            }
        ...
        public override string? LookupPrefix(string ns)
        {
            if (ns == null)
                throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    new ArgumentNullException(nameof(ns))
                );
            ...
            if (count > 0)
            {
                ...
                if (actualByteCount >= 3)
                {
                    ...
                    if (!_isXmlnsAttribute)
                    {
                        ...
                        await _writer.WriteBase64TextAsync(
                                _trailBytes,
                                _trailByteCount,
                                buffer,
                                offset,
        ...
        private async Task FlushTrailBytesAsync()
        {
            ...
            {
                await StartContentAsync().ConfigureAwait(false);
                await _writer.WriteBase64TextAsync(_trailBytes, _trailByteCount, _trailBytes, 0, 0)
                    .ConfigureAwait(false);
                EndContent();
            }
            _trailByteCount = 0;
        ...
        private string GeneratePrefix(string ns, XmlDictionaryString? xNs)
        {
            ...
            while (true)
            {
                ...
                prefix = string.Concat(
                    ...
                    _depth.ToString(CultureInfo.InvariantCulture),
                    "p",
                    prefixId.ToString(CultureInfo.InvariantCulture)
                );
