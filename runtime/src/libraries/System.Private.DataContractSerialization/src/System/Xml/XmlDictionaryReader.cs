...
namespace System.Xml
{
    ...
    public abstract class XmlDictionaryReader : XmlReader
    {
        ...
        public virtual int ReadContentAsChars(char[] chars, int offset, int count)
        {
            ...
            while (true)
            {
                ...
                if (IsTextNode(nodeType))
                {
                    ...
                    if (
                        nodeType == XmlNodeType.Attribute /* || inAttributeText */
                    )
                        break;
            ...
            for (int i = 0; i < strings.Length; i++)
            {
                ...
                if (value == null)
                    throw System
                        ...
                        .DiagnosticUtility
                        .ExceptionUtility
                        .ThrowHelperArgumentNull($"strings[{i}]");
                if (value == s)
                {
                    index = i;
                    return value;
        ...
        public virtual bool TryGetNamespaceUriAsDictionaryString(
            [NotNullWhen(true)] out XmlDictionaryString? namespaceUri
        )
        {
            namespaceUri = null;
            return false;
        ...
        )
        {
            ...
            while (actual < count && IsStartElement(localName, namespaceUri))
            {
                ...
                if (i < short.MinValue || i > short.MaxValue)
                    XmlExceptionHelper.ThrowConversionOverflow(
                        this,
                        i.ToString(NumberFormatInfo.CurrentInfo),
                        "Int16"
                    );
