...
namespace System.Runtime.Serialization
{
    internal class XmlObjectSerializerWriteContext : XmlObjectSerializerContext
    {
        ...
        )
        {
            ...
            if (isNew)
            {
                xmlWriter.WriteAttributeString(
                    ...
                    string.Create(CultureInfo.InvariantCulture, $"i{objectId}")
                );
                return false;
            }
            else
        ...
        internal static void ThrowRequiredMemberMustBeEmitted(string memberName, Type type)
        {
            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                new SerializationException(
                    SR.Format(SR.RequiredMemberMustBeEmitted, memberName, type.FullName)
                )
            );
        }
        ...
        internal static void GetObjectData(
            ISerializable obj,
            SerializationInfo serInfo,
        ...
        public void WriteISerializable(XmlWriterDelegator xmlWriter, ISerializable obj)
        {
            ...
            var serInfo = new SerializationInfo(
                objType,
                XmlObjectSerializer.FormatterConverter /*!UnsafeTypeForwardingEnabled is always false*/
            );
            GetObjectData(obj, serInfo, GetStreamingContext());

            if (
                !UnsafeTypeForwardingEnabled && serInfo.AssemblyName == Globals.MscorlibAssemblyName
            )
            {
