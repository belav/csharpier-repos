...
namespace System.Runtime.Serialization.Json
{
    ...
    internal sealed class JsonFormatReaderGenerator
    {
        ...
        private sealed class CriticalHelper
        {
            ...
            )
            {
                ...
                try
                {
                    BeginMethod(
                        _ilg,
                        "Read"
                            + DataContract.SanitizeTypeName(classContract.XmlName!.Name)
                            + "FromJson",
                ...
                try
                {
                    if (isGetOnlyCollection)
                    {
                        BeginMethod(
                            _ilg,
                            "Read"
                                + DataContract.SanitizeTypeName(collectionContract.XmlName.Name)
                                + "FromJson"
                                + "IsGetOnly",
            ...
            {
                int memberCount =
                    (classContract.BaseClassContract == null)
                        ? 0
                        : ReadMembers(
                            classContract.BaseClassContract,
                            expectedElements,
                            memberLabels,
                            throwDuplicateMemberLabel,
                            memberIndexLocal
