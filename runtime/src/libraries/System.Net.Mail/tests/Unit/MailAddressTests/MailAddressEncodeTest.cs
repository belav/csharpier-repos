...
namespace System.Net.Mail.Tests
{
    public class MailAddressEncodeTest
    {
        public static IEnumerable<object[]> MailAddressFactory_Address()
        {
            ...
            {
                new Func<string, MailAddress>((string address) => new MailAddress(address)),
            };
            yield return new object[]
            {
        ...
        public static IEnumerable<object[]> MailAddressFactory_AddressDisplayName()
        {
            yield return new object[]
            {
                new Func<string, string, MailAddress>(
                    (string address, string displayName) => new MailAddress(address, displayName)
                ),
            };
            yield return new object[]
            {
                new Func<string, string, MailAddress>(
                    ...
                    {
                        Assert.True(
                            MailAddress.TryCreate(address, displayName, out MailAddress result)
                        );
                        return result;
                    }
                ),
        ...
        )
        {
            ...
            string result = testAddress.Encode(2, false);
            Assert.Equal(
                "=?utf-8?Q?jeff_=C3=83=C3=9A=C3=AA=C3=AB=C3=AF=C3=AF?= <jeff@nclmailtest.com>",
                result
            );

            result = testAddress.Encode(2, true);
            Assert.Equal(
                "\"jeff \u00C3\u00DA\u00EA\u00EB\u00EF\u00EF\" <jeff@nclmailtest.com>",
                result
            );
        }
        ...
        )
        {
            ...
            string result = testAddress.Encode(0, false);
            Assert.Equal(
                "=?utf-8?Q?test=C3=9Ctesttesttesttesttesttesttesttesttesttesttesttest?=\r\n "
                    + "=?utf-8?Q?testtesttesttesttesttest?= <test@example.com>",
                result
            );
        ...
        )
        {
            ...
            string result = testAddress.Encode(0, false);
            Assert.Equal(
                "=?utf-8?Q?=C2=AE_=21=23=24=25=26=27=28=29=2B=2C=2D=2E=2F0123456789=3A?="
                    + "\r\n =?utf-8?Q?=3B=3C=3D=3E=3F=40ABCDEFGHIJKLMNOPQRSTUVWXYZ=5B=5C=5D=5E=5F?="
                    + "\r\n =?utf-8?Q?=60abcdefghijklmnopqrstuvwxyz=7B=7C=7D=7E?= <test@example.com>",
                result
            );
        ...
        )
        {
            ...
            string result = collection.Encode(0, false);
            Assert.Equal(
                "\"test\" <test@example.com>, =?utf-8?Q?test=C3=9C?= <test2@example.com>",
                result
            );
        ...
        )
        {
            ...
            Assert.Equal(
                "\"test\" <test@example.com>, =?utf-8?Q?test=C3=9C?= <test2@example.com>,"
                    + " test5@example2.com, =?utf-8?Q?test=C3=9C?= <test2@example.com>",
                result
            );

            result = collection.Encode(0, true);
            Assert.Equal(
                "\"test\" <test@example.com>, \"test\u00DC\" <test2@example.com>, test5@example2.com,"
                    + " \"test\u00DC\" <test2@example.com>",
                result
            );
        }
