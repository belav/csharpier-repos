...
namespace System.Net.Mail
{
    internal static class MailHeaderInfo
    {
        ...
        private static Dictionary<string, int> CreateHeaderDictionary()
        {
            ...
            {
                Debug.Assert(
                    (int)s_headerInfo[i].ID == i,
                    "Header info data structures are not in sync"
                );
        ...
        internal static bool IsUserSettable(string name)
        {
            ...
            return !s_headerDictionary.TryGetValue(name, out index)
                || s_headerInfo[index].IsUserSettable;
        }
        ...
        internal static bool IsSingleton(string name)
        {
            int index;
            return s_headerDictionary.TryGetValue(name, out index)
                && s_headerInfo[index].IsSingleton;
        }
        ...
        internal static bool AllowsUnicode(string name)
        {
            int index;
            return !s_headerDictionary.TryGetValue(name, out index)
                || s_headerInfo[index].AllowsUnicode;
        }
    }
}
