        ...
        private sealed partial class Emitter
        {
            ...
            private void EmitBindMethods_Extensions_OptionsBuilder()
            {
                ...
                if (ShouldEmitMethods(MethodsToGen.OptionsBuilderExt_Bind_T))
                {
                    ...
                    _writer.WriteLine(
                        $"return Bind({Identifier.optionsBuilder}, {Identifier.config}, {Identifier.configureBinder}: null);"
                    );
                    EmitEndBlock();
                }
                ...
                EmitMethodStartBlock(
                    ...
                    "Bind",
                    paramList
                        + $", {TypeDisplayString.NullableActionOfBinderOptions} {Identifier.configureBinder}",
                    documentation
                );

                EmitCheckForNullArgument_WithBlankLine(
                    Identifier.optionsBuilder,
            ...
            private void EmitBindConfigurationMethod()
            {
                ...
                const string documentation =
                    $@"/// <summary>Registers the dependency injection container to bind <typeparamref name=""TOptions""/> against the <see cref=""{Identifier.IConfiguration}""/> obtained from the DI service provider.</summary>";
                string paramList =
                    $"string {Identifier.configSectionPath}, {TypeDisplayString.NullableActionOfBinderOptions} {Identifier.configureBinder} = null";
                ...
                EmitCheckForNullArgument_WithBlankLine(
                    Identifier.configSectionPath,
                    _emitThrowIfNullMethod
                );

                EmitStartBlock(
                    $"{Identifier.optionsBuilder}.{Identifier.Configure}<{Identifier.IConfiguration}>(({Identifier.instance}, {Identifier.config}) =>"
                );
                ...
                EmitStartBlock(
                    $"{Identifier.optionsBuilder}.{Identifier.Services}.{Identifier.AddSingleton}<{Identifier.IOptionsChangeTokenSource}<{Identifier.TOptions}>, {Identifier.ConfigurationChangeTokenSource}<{Identifier.TOptions}>>({Identifier.sp} =>"
                );

                _writer.WriteLine(
                    $"return new {Identifier.ConfigurationChangeTokenSource}<{Identifier.TOptions}>({Identifier.optionsBuilder}.{Identifier.Name}, {Identifier.sp}.GetRequiredService<{Identifier.IConfiguration}>());"
                );
            ...
            {
                paramList =
                    $"this {TypeDisplayString.OptionsBuilderOfTOptions} {Identifier.optionsBuilder}, {paramList}";
                EmitBlankLineIfRequired();
                _writer.WriteLine(documentation);
                EmitInterceptsLocationAnnotations(method);
                EmitStartBlock(
                    $"public static {TypeDisplayString.OptionsBuilderOfTOptions} {methodName}<{Identifier.TOptions}>({paramList}) where {Identifier.TOptions} : class"
                );
            }
        }
    }
