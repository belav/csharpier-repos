...
namespace System.DirectoryServices
{
    public class DirectoryEntryConfiguration
    {
        ...
        public PasswordEncodingMethod PasswordEncoding
        {
            ...
            {
                if (
                    value < PasswordEncodingMethod.PasswordEncodingSsl
                    || value > PasswordEncodingMethod.PasswordEncodingClear
                )
                    throw new InvalidEnumArgumentException(
                        nameof(value),
                        (int)value,
        ...
        {
            // underneath it uses the same handle and binds to the same object, so no permission is required as
            // it has been done in Bind call
            return (string)
                ((UnsafeNativeMethods.IAdsObjectOptions)_entry.AdsObject).GetOption(
