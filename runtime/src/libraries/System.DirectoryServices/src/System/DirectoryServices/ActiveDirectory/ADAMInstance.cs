...
namespace System.DirectoryServices.ActiveDirectory
{
    public class AdamInstance : DirectoryServer
    {
        ...
        public static AdamInstanceCollection FindAll(DirectoryContext context, string partitionName)
        {
            ...
            try
            {
                adamInstanceCollection = ConfigurationSet.FindAdamInstances(
                    context,
                    partitionName,
                    null
                );
        ...
        public void SeizeRoleOwnership(AdamRole role)
        {
            ...
            switch (role)
            {
                ...
                {
                    roleObjectDN = directoryEntryMgr.ExpandWellKnownDN(
        ...
        public override ReplicationOperationInformation GetReplicationOperationInformation()
        {
            ...
            info = GetReplicationInfoHelper(
                _ADAMHandle,
                (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_PENDING_OPS,
                (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_PENDING_OPS,
                null,
                ref advanced,
                0,
        ...
        public override ActiveDirectoryReplicationMetadata GetReplicationMetadata(string objectPath)
        {
            ...
            info = GetReplicationInfoHelper(
                _ADAMHandle,
                (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_METADATA_2_FOR_OBJ,
                (int)DS_REPL_INFO_TYPE.DS_REPL_INFO_METADATA_FOR_OBJ,
                objectPath,
                ref advanced,
                0,
                DirectoryContext.ADAMHandle
        ...
        public override void TriggerSyncReplicaFromNeighbors(string partition)
        {
            ...
            SyncReplicaHelper(
                ...
                partition,
                null,
                DS_REPSYNC_ASYNCHRONOUS_OPERATION | DS_REPSYNC_ALL_SOURCES,
                DirectoryContext.ADAMHandle
            );
        }
        ...
        public ConfigurationSet ConfigurationSet
        {
            get
            {
                ...
                {
                    DirectoryContext configSetContext = Utils.GetNewDirectoryContext(
                        Name,
            ...
            get
            {
                ...
                if (_cachedHostName == null)
                {
                    ...
                    _cachedHostName = (string)PropertyManager
                        .GetPropertyValue(context, serverEntry, PropertyManager.DnsHostName)
                        !;
                }
            ...
            get
            {
                ...
                if (_cachedLdapPort == -1)
                {
                    ...
                    _cachedLdapPort = (int)PropertyManager
                        .GetPropertyValue(context, ntdsaEntry, PropertyManager.MsDSPortLDAP)
                        !;
                }
            ...
            get
            {
                ...
                if (_cachedSslPort == -1)
                {
                    ...
                    _cachedSslPort = (int)PropertyManager
                        .GetPropertyValue(context, ntdsaEntry, PropertyManager.MsDSPortSSL)
                        !;
                }
                        ...
                        else
                        {
                            _cachedDefaultPartition = (string)PropertyManager
                                .GetPropertyValue(
                                    context,
                                    ntdsaEntry,
                                    PropertyManager.MsDSDefaultNamingContext
                                )
                                !;
                        }
                    }
                ...
                else
                {
                    ...
                    if (!Partitions.Contains(value))
                    {
                        throw new ArgumentException(
                            SR.Format(SR.ServerNotAReplica, value),
                            nameof(value)
                        );
                    }
                ...
                if (cachedSiteName == null)
                {
                    ...
                    try
                    {
                        cachedSiteName = (string)PropertyManager
                            .GetPropertyValue(context, siteEntry, PropertyManager.Cn)
                            !;
                    }
                    finally
                    {
                ...
                if (cachedServerObjectName == null)
                {
                    ...
                    try
                    {
                        cachedServerObjectName = (string)PropertyManager
                            .GetPropertyValue(context, rootDSE, PropertyManager.ServerName)
                            !;
                    }
                    catch (COMException e)
                    {
                ...
                if (cachedNtdsaObjectName == null)
                {
                    ...
                    try
                    {
                        cachedNtdsaObjectName = (string)PropertyManager
                            .GetPropertyValue(context, rootDSE, PropertyManager.DsServiceName)
                            !;
                    }
                    catch (COMException e)
                    {
            ...
            get
            {
                ...
                if (cachedNtdsaObjectGuid == Guid.Empty)
                {
                    ...
                    byte[] guidByteArray = (byte[])PropertyManager
                        .GetPropertyValue(context, ntdsaEntry, PropertyManager.ObjectGuid)
                        !;
