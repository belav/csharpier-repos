...
namespace System.Collections.Specialized
{
    ...
    public class NotifyCollectionChangedEventArgs : EventArgs
    {
        ...
        public NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction action)
        {
            if (action != NotifyCollectionChangedAction.Reset)
            {
                throw new ArgumentException(
                    SR.Format(SR.WrongActionForCtor, NotifyCollectionChangedAction.Reset),
                    nameof(action)
                );
            }
        ...
        )
        {
            switch (action)
            {
                case NotifyCollectionChangedAction.Reset:
                    ...
                    {
                        throw new ArgumentException(
                            SR.ResetActionRequiresIndexMinus1,
                            nameof(action)
                        );
                    }
                    break;
                ...
                default:
                    throw new ArgumentException(
                        SR.MustBeResetAddOrRemoveActionForCtor,
                        nameof(action)
                    );
            }
        ...
        )
        {
            ...
            {
                throw new ArgumentException(
                    SR.Format(SR.WrongActionForCtor, NotifyCollectionChangedAction.Replace),
                    nameof(action)
                );
            }
        ...
        )
        {
            ...
            {
                throw new ArgumentException(
                    SR.Format(SR.WrongActionForCtor, NotifyCollectionChangedAction.Move),
                    nameof(action)
                );
            }
            if (index < 0)
        ...
        )
        {
            ...
            _action = action;
            _newItems = _oldItems = changedItems is not null
                ? new ReadOnlyList(changedItems)
                : null;
            _newStartingIndex = index;
            _oldStartingIndex = oldIndex;
        }
