...
{
    partial public class TarWriter_WriteEntryAsync_File_Tests : TarWriter_File_Base
    {
                    ...
                    async (string strFormat) =>
                    {
                        ...
                        using (TempDirectory root = new TempDirectory())
                        {
                            ...
                            await using (MemoryStream archive = new MemoryStream())
                            {
                                ...
                                {
                                    PosixTarEntry entry =
                                        await reader.GetNextEntryAsync() as PosixTarEntry;
                    ...
                    async (string strFormat) =>
                    {
                        ...
                        using (TempDirectory root = new TempDirectory())
                        {
                            ...
                            await using (MemoryStream archive = new MemoryStream())
                            {
                                ...
                                {
                                    await writer.WriteEntryAsync(
                                        fileName: blockDevicePath,
                                        entryName: AssetBlockDeviceFileName
                                    );
                                }
            ...
            RemoteExecutor
                .Invoke(
                    async (string strFormat) =>
                    {
                        ...
                        {
                            TarEntryFormat expectedFormat = Enum.Parse<TarEntryFormat>(strFormat);
                            string characterDevicePath = Path.Join(
                                root.Path,
                            ...
                            await using (MemoryStream archive = new MemoryStream())
                            {
                                ...
                                {
                                    await writer.WriteEntryAsync(
                                        fileName: characterDevicePath,
                                        entryName: AssetCharacterDeviceFileName
                                    );
                                }
