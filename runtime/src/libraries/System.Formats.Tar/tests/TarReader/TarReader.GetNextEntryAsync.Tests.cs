    ...
    public class TarReader_GetNextEntryAsync_Tests : TarTestsBase
    {
        ...
        public async Task GetNextEntryAsync_Cancel()
        {
            ...
            await using (TarReader reader = new TarReader(archiveStream, leaveOpen: false))
            {
                await Assert.ThrowsAsync<TaskCanceledException>(
                    async () => await reader.GetNextEntryAsync(copyData: false, cs.Token)
                );
            }
        }
            ...
            await using (MemoryStream malformed = new MemoryStream())
            {
                ...
                await using (TarReader reader = new TarReader(malformed))
                {
                    await Assert.ThrowsAsync<EndOfStreamException>(
                        async () => await reader.GetNextEntryAsync()
                    );
                }
            }
        }
        ...
        public async Task GetNextEntry_CopyDataTrue_UnseekableArchive_Async()
        {
            ...
            await using (MemoryStream archive = new MemoryStream())
            {
                ...
                archive.Seek(0, SeekOrigin.Begin);
                await using (
                    WrappedStream wrapped = new WrappedStream(
                        archive,
                        canRead: true,
                        canWrite: false,
                        canSeek: false
        ...
        )
        {
            ...
            archive.Seek(0, SeekOrigin.Begin);
            await using WrappedStream wrapped = new WrappedStream(
                archive,
                canRead: true,
                canWrite: false,
                canSeek: false
            ...
            await using (TarReader reader = new TarReader(wrapped)) // Unseekable
            {
                ...
                await Assert.ThrowsAsync<EndOfStreamException>(
                    async () => await entry.DataStream.ReadAsync(b)
                );
            }
        ...
        public async Task BlockAlignmentPadding_DoesNotAffectNextEntries_Async(
            int contentSize,
            bool copyData
        )
        {
