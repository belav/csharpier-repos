    ...
    public abstract partial class TarEntry
    {
        ...
        public void ExtractToFile(string destinationFileName, bool overwrite)
        {
            ...
            )
            {
                throw new InvalidOperationException(
                    SR.Format(SR.TarEntryTypeNotSupportedForExtracting, EntryType)
                );
            }
        ...
        public Stream? DataStream
        {
            ...
            set
            {
                ...
                {
                    throw new InvalidOperationException(
                        SR.Format(SR.TarEntryDoesNotSupportDataStream, Name, EntryType)
                    );
                }
        ...
        {
            (string destinationFullPath, string? linkTargetPath) = GetDestinationAndLinkPaths(
            ...
            if (EntryType == TarEntryType.Directory)
            {
                ...
                TarHelpers.UpdatePendingModificationTimes(
        ...
        private (string, string?) GetDestinationAndLinkPaths(string destinationDirectoryPath)
        {
            ...
            if (fileDestinationPath == null)
            {
                throw new IOException(
                    SR.Format(SR.TarExtractingResultsFileOutside, name, destinationDirectoryPath)
                );
            }
            ...
            if (EntryType is TarEntryType.SymbolicLink)
            {
                ...
                string? linkDestination = GetFullDestinationPath(
                    destinationDirectoryPath,
                    Path.IsPathFullyQualified(linkName)
                        ? linkName
                        : Path.Join(Path.GetDirectoryName(fileDestinationPath), linkName)
                );
        ...
        {
            Debug.Assert(
                Path.IsPathFullyQualified(qualifiedPath),
                $"{qualifiedPath} is not qualified"
            );
            Debug.Assert(
                PathInternal.EndsInDirectorySeparator(destinationDirectoryFullPath),
                "caller must ensure the path ends with a separator."
            );

            string fullPath = Path.GetFullPath(qualifiedPath); // Removes relative segments

            return fullPath.StartsWith(destinationDirectoryFullPath, PathInternal.StringComparison)
                ? fullPath
                : null;
        }
