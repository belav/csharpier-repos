...
namespace System.Formats.Tar
{
    ...
    public static class TarFile
    {
        ...
        )
        {
            ...
            destinationDirectoryName = PathInternal.EnsureTrailingSeparator(
        ...
        )
        {
            ...
            using (TarWriter writer = new TarWriter(destination, TarEntryFormat.Pax, leaveOpen))
            {
                ...
                foreach (
                    (string fullpath, string entryname) in GetFilesForCreation(
                        sourceDirectoryName,
                        basePath.Length
                    )
                )
        ...
        )
        {
            ...
            await using (writer.ConfigureAwait(false))
            {
                ...
                {
                    await writer
                        .WriteEntryAsync(fullpath, entryname, cancellationToken)
                        .ConfigureAwait(false);
                }
            }
        }
            ...
            var fse = new FileSystemEnumerable<(string fullpath, string entryname, bool recurse)>(
                ...
                transform: (ref FileSystemEntry entry) =>
                {
                    ...
                    bool isRealDirectory =
                        entry.IsDirectory && (entry.Attributes & FileAttributes.ReparsePoint) == 0; // not a symlink.
                    string entryName = ArchivingUtils.EntryFromPath(
                        fullPath.AsSpan(basePathLength),
                        appendPathSeparator: isRealDirectory
                    );
