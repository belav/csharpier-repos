...
namespace System.IO.Pipelines.Tests
{
    public class StreamPipeReaderReadAtLeastAsyncTests : ReadAtLeastAsyncTests
    {
        ...
        protected override PipeReader PipeReader =>
            _pipeReader ?? (_pipeReader = PipeReader.Create(Pipe.Reader.AsStream()));

        protected override void SetPipeReaderOptions(
        ...
        )
        {
            _pipeReader = PipeReader.Create(
                ...
                new StreamPipeReaderOptions(pool, bufferSize)
            );
        }

        private static Func<DisposeTrackingBufferPool> CustomPoolFunc = () =>
            new DisposeTrackingBufferPool();
        public static TheoryData<MemoryPool<byte>?, int, bool, bool> TestData =>
            new TheoryData<MemoryPool<byte>?, int, bool, bool>
            {
                ...
                {
                    CustomPoolFunc(),
                    StreamPipeReaderOptions.DefaultMaxBufferSize + 1,
                    false,
                    false
                },
            };
        ...
        public async Task ReadAtLeastAsyncSegmentSizeLessThanMaxBufferSize(
            DisposeTrackingBufferPool? pool,
            int bufferSize,
            bool isSingleSegment,
            bool isFromCustomPool
        )
        {
