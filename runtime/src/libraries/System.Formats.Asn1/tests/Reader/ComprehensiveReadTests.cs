...
namespace System.Formats.Asn1.Tests.Reader
{
    public static class ComprehensiveReadTests
    {
        ...
        public static void ReadMicrosoftComCert()
        {
            ...
            AsnReader versionExplicitWrapper = tbsCertReader.ReadSequence(
            ...
            Assert.Equal(
                new DateTimeOffset(2014, 10, 15, 0, 0, 0, TimeSpan.Zero),
                validityReader.ReadUtcTime()
            );
            Assert.Equal(
                new DateTimeOffset(2016, 10, 15, 23, 59, 59, TimeSpan.Zero),
                validityReader.ReadUtcTime()
            );
            ...
            AssertRefSame(
                encodedPublicKey,
                ref bytes[498],
                "Encoded public key starts at byte 498"
            );
            ...
            AssertRefSame(
                rsaPublicKeyReader.ReadIntegerBytes(),
                ref bytes[506],
                "RSA Modulus is at bytes[502]"
            );
            ...
            AsnReader extensionsContainer = tbsCertReader.ReadSequence(
            ...
            Assert.True(
                sanExtension.TryReadPrimitiveOctetString(out ReadOnlyMemory<byte> sanExtensionBytes)
            );
            ...
            Assert.Equal(
                "www.microsoft.com",
                sanExtensionValue.ReadCharacterString(UniversalTagNumber.IA5String, dnsName)
            );
            Assert.Equal(
                "wwwqa.microsoft.com",
                sanExtensionValue.ReadCharacterString(UniversalTagNumber.IA5String, dnsName)
            ...
            Assert.True(
                basicConstraints.TryReadPrimitiveOctetString(
                    out ReadOnlyMemory<byte> basicConstraintsBytes
                )
            );

            AsnReader basicConstraintsPayload = new AsnReader(
                basicConstraintsBytes,
            ...
            Assert.True(
                keyUsageExtension.ReadBoolean(),
                "keyUsageExtension.ReadBoolean() (IsCritical)"
            );
            Assert.True(
                keyUsageExtension.TryReadPrimitiveOctetString(
                    out ReadOnlyMemory<byte> keyUsageBytes
                )
            );
        ...
        )
        {
            ...
            Assert.True(
                extension.TryReadPrimitiveOctetString(out ReadOnlyMemory<byte> extensionBytes)
            );
            AssertRefSame(
                extensionBytes,
                ref bytes[index],
                $"{oid} extension value is at byte {index}"
            );
        }
        ...
        )
        {
            ...
            if (stringValue != null)
            {
                ...
                Assert.Equal(
                    stringValue,
                    valueReader.ReadCharacterString((UniversalTagNumber)valueTag.TagValue)
                );
