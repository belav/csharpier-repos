    ...
    internal sealed class AsynchronousChannel<T> : IDisposable
    {
        ...
        internal void FlushBuffers()
        {
            TraceHelpers.TraceInfo(
                "tid {0}: AsynchronousChannel<T>::FlushBuffers() called",
        ...
        {
            TraceHelpers.TraceInfo(
                "tid {0}: AsynchronousChannel<T>::SetDone() called",
                Environment.CurrentManagedThreadId
            );
        ...
        private void EnqueueChunk(T[] chunk)
        {
            ...
            {
                TraceHelpers.TraceInfo(
                    "AsynchronousChannel::EnqueueChunk - producer waking consumer"
                );
        ...
        private void WaitUntilNonFull()
        {
            ...
            do
            {
                ...
                if (IsFull)
                {
                    ...
                    TraceHelpers.TraceInfo(
                        "AsynchronousChannel::EnqueueChunk - producer waiting, buffer full"
                    );
        ...
        internal bool TryDequeue([MaybeNullWhen(false), AllowNull] ref T item)
        {
            ...
            Debug.Assert(
                0 <= _consumerChunkIndex && _consumerChunkIndex < _consumerChunk.Length,
        ...
        private bool TryDequeueChunk([NotNullWhen(true)] ref T[]? chunk, ref bool isDone)
        {
            ...
            while (IsChunkBufferEmpty)
            {
                ...
                if (IsChunkBufferEmpty && !IsDone)
                {
                    ...
                    // to wait or not.
                    TraceHelpers.TraceInfo(
                        "AsynchronousChannel::DequeueChunk - consumer possibly waiting"
                    );
                    return false;
                }
                else
        ...
        private T[] InternalDequeueChunk()
        {
            ...
            {
                TraceHelpers.TraceInfo(
                    "BoundedSingleLockFreeChannel::DequeueChunk - consumer waking producer"
                );
                Debug.Assert(_producerEvent != null);
                _producerIsWaiting = 0;
                _producerEvent.Set();
