    ...
    public static class AggregateTests
    {
        ...
        public static void Aggregate_Product_Result(int count)
        {
            Assert.Equal(
                ...
                ParallelEnumerable.Range(1, count)
                    .Aggregate(
                        1L,
                        (x, y) => unchecked(x * y),
        ...
        public static void Aggregate_OperationCanceledException_PreCanceled()
        {
            ...
            AssertThrows.AlreadyCanceled(
                source => source.Aggregate(0, (i, j) => i, (i, j) => i, i => i)
            );
            AssertThrows.AlreadyCanceled(
                source => source.Aggregate(() => 0, (i, j) => i, (i, j) => i, i => i)
            );
        ...
        public static void Aggregate_AggregateException()
        {
            AssertThrows.Wrapped<DeliberateTestException>(
                () => UnorderedSources.Default(2).Aggregate(
                        (i, j) =>
                        {
            ...
            AssertThrows.Wrapped<DeliberateTestException>(
                () => UnorderedSources.Default(2).Aggregate<int, int, int>(
                        0,
                        (i, j) => i,
                        i =>
                        {
