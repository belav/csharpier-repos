...
namespace System.Linq.Parallel.Tests
{
    public static class GroupJoinTests
    {
        ...
        {
            GroupJoin_Unordered_NotPipelined(
        ...
        [MemberData(
            ...
            new[] { 0, 1, 2, KeyFactor * 2 - 1, KeyFactor * 2 }
        )]
        public static void GroupJoin_Multiple(
            Labeled<ParallelQuery<int>> left,
            int leftCount,
        ...
        public static void GroupJoin_Multiple_LeftWithOrderingColisions(
            Labeled<ParallelQuery<int>> left,
            int leftCount,
            Labeled<ParallelQuery<int>> right,
            int rightCount
        ...
        {
            GroupJoin_Unordered_CustomComparator(
                Sources.OuterLoopCount / 64,
                Sources.OuterLoopCount / 64
            );
        }
                ...
                p =>
                {
                    ...
                    if (p.Key % KeyFactor < Math.Min(ElementFactor, rightCount))
                    {
                        ...
                        Assert.Equal(
                            Math.Max(
                                p.Key % (KeyFactor / 2),
                                rightCount + (p.Key % (KeyFactor / 2) - (KeyFactor / 2))
                            ),
                            seenInner
                        );
                    }
        ...
        public static void GroupJoin_CustomComparator_LeftWithOrderingColisions(
            Labeled<ParallelQuery<int>> left,
            int leftCount,
            Labeled<ParallelQuery<int>> right,
            int rightCount
