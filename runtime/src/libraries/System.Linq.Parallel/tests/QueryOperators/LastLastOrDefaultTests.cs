...
namespace System.Linq.Parallel.Tests
{
    ...
    {
        private static Func<int, IEnumerable<int>> Positions = x =>
            new[] { 1, x / 2 + 1, Math.Max(1, x - 1) }.Distinct();
        ...
        [MemberData(nameof(LastUnorderedData), new[] { 0 })]
        [MemberData(nameof(LastData), new[] { 0 })]
        public static void LastOrDefault_Empty(
            Labeled<ParallelQuery<int>> labeled,
            int count,
            int position
        )
        ...
        [MemberData(nameof(LastUnorderedData), new[] { 1, 2, 16 })]
        [MemberData(nameof(LastData), new[] { 1, 2, 16 })]
        public static void Last_NoMatch(
            Labeled<ParallelQuery<int>> labeled,
            int count,
            int position
        )
        ...
        [MemberData(nameof(LastUnorderedData), new[] { 1, 2, 16 })]
        [MemberData(nameof(LastData), new[] { 1, 2, 16 })]
        public static void LastOrDefault_NoMatch(
            Labeled<ParallelQuery<int>> labeled,
            int count,
            int position
        )
        ...
        public static void Last_ArgumentNullException()
        {
            AssertExtensions.Throws<ArgumentNullException>(
                "source",
                () => ((ParallelQuery<bool>)null).Last()
            );
            AssertExtensions.Throws<ArgumentNullException>(
                "source",
                () => ((ParallelQuery<bool>)null).LastOrDefault()
            );

            AssertExtensions.Throws<ArgumentNullException>(
                "predicate",
                () => ParallelEnumerable.Empty<int>().Last(null)
            );
            AssertExtensions.Throws<ArgumentNullException>(
                "predicate",
                () => ParallelEnumerable.Empty<int>().LastOrDefault(null)
            );
        }
    }
}
