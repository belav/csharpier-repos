...
namespace System.Linq.Parallel.Tests
{
    public static class ZipTests
    {
        ...
        [Theory]
        [MemberData(nameof(ZipData), new[] { 0, 1, 2, 16 })]
        public static void Zip_NotPipelined(
            Labeled<ParallelQuery<int>> left,
            int leftCount,
            Labeled<ParallelQuery<int>> right,
            int rightCount
        ...
        [OuterLoop]
        [MemberData(
            nameof(ZipThreadedData),
            new[] { 1, 2, 16, 128, 1024 },
            new[] { 1, 2, 4, 7, 8, 31, 32 }
        )]
        public static void Zip_AsOrdered_ThreadedDeadlock(
            Labeled<ParallelQuery<int>> left,
            int leftCount,
            Labeled<ParallelQuery<int>> right,
            int rightCount,
        ...
        )
        {
            ...
            ParallelQuery<int> query = left
                ...
                .Zip<int, int, int>(right.Item, (a, b) =>
                {
...
#pragma warning restore 618
        ...
        public static void Zip_ArgumentNullException()
        {
            AssertExtensions.Throws<ArgumentNullException>("first", () =>
                ((ParallelQuery<int>)null).Zip(ParallelEnumerable.Range(0, 1), (x, y) => x)
            );
            AssertExtensions.Throws<ArgumentNullException>("second", () =>
                ParallelEnumerable.Range(0, 1).Zip(null, (Func<int, int, int>)((x, y) => x))
            );
            AssertExtensions.Throws<ArgumentNullException>("resultSelector", () =>
                ParallelEnumerable
                    .Range(0, 1)
                    .Zip(ParallelEnumerable.Range(0, 1), (Func<int, int, int>)null)
            );
        }
    }
}
