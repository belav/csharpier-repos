    ...
    public static class CastTests
    {
        ...
        public static void Cast_Unordered_InvalidCastException(int count)
        {
            AssertThrows.Wrapped<InvalidCastException>(
                () => UnorderedSources.Default(count).Cast<double>().ForAll(
                        x =>
                        {
                            ;
                        }
        ...
        {
            _ = count;
            AssertThrows.Wrapped<InvalidCastException>(() => labeled.Item.Cast<double>().ForAll(
                        x =>
                        {
                            ;
                        }
        ...
        {
            AssertThrows.Wrapped<InvalidCastException>(
                () => UnorderedSources.Default(count).Select(x => (Int32)x).Cast<Castable>().ForAll(
                        x =>
                        {
                            ;
                        }
        ...
        )
        {
            _ = count;
            AssertThrows.Wrapped<InvalidCastException>(
                () => labeled.Item.Select(x => (Int32)x).Cast<Castable>().ForAll(
                        x =>
                        {
                            ;
                        }
