...
namespace System.Linq.Parallel.Tests
{
    public static class DegreeOfParallelismTests
    {
            ...
            foreach (object[] results in DegreeData(counts, degrees))
            {
                ...
                if (query.ToString().StartsWith("Partitioner"))
                {
                    yield return new object[]
                    {
                        Labeled.Label(
                            query.ToString(),
                            Partitioner
                                .Create(Enumerable.Range(0, (int)results[1]).ToArray(), false)
                                .AsParallel()
                        ),
                        results[1],
                        results[2],
                ...
                else if (query.ToString().StartsWith("Enumerable.Range"))
                {
                    yield return new object[]
                    {
                        Labeled.Label(
                            query.ToString(),
                            new StrictPartitioner<int>(
                                Partitioner.Create(
                                    Enumerable.Range(0, (int)results[1]),
        ...
        [OuterLoop]
        public static void DegreeOfParallelism_Barrier(
            Labeled<ParallelQuery<int>> labeled,
            int count,
            int degree
        )
        ...
        [OuterLoop]
        public static void DegreeOfParallelism_Pipelining(
            Labeled<ParallelQuery<int>> labeled,
            int count,
            int degree
        )
        ...
        [OuterLoop]
        public static void DegreeOfParallelism_Throttled_Pipelining(
            Labeled<ParallelQuery<int>> labeled,
            int count,
            int degree
        )
        ...
        [Theory]
        [MemberData(
            nameof(NotLoadBalancedDegreeData),
            new[] { 1, 4 },
            new int[]
            { /* same as count */
            }
        ...
        [MemberData(nameof(NotLoadBalancedDegreeData), new[] { 32, 512, 1024 }, new[] { 4, 16 })]
        [OuterLoop]
        public static void DegreeOfParallelism_Aggregate_SeedFunction(
            Labeled<ParallelQuery<int>> labeled,
            int count,
            int degree
        )
        ...
        [OuterLoop]
        public static void DegreeOfParallelism_OutOfRange(
            Labeled<ParallelQuery<int>> labeled,
            int count,
            int degree
        )
        ...
        public static void DegreeOfParallelism_Multiple()
        {
            Assert.Throws<InvalidOperationException>(() =>
                ParallelEnumerable.Range(0, 2).WithDegreeOfParallelism(2).WithDegreeOfParallelism(2)
            );
        }
