...
namespace System.Linq.Parallel.Tests
{
    ...
    public static partial class ParallelQueryCombinationTests
    {
        ...
        private static readonly Operation DefaultSource = (start, count, ignore) =>
            ParallelEnumerable.Range(start, count);
        private static readonly Labeled<Operation> LabeledDefaultSource = Label(
            "Default",
            DefaultSource
        );

        private static readonly Labeled<Operation> Failing = Label(
            "ThrowOnFirstEnumeration",
            (start, count, source) => Enumerables<int>.ThrowOnEnumeration().AsParallel()
        );
        ...
        public static IEnumerable<object[]> UnaryOperators()
        {
            ...
            Labeled<Operation> reverse = UnaryOperations()
                .Select(i => (Labeled<Operation>)i[0])
                .Where(op => op.ToString().Contains("Reverse"))
                .Single();
            foreach (Labeled<Operation> source in UnorderedRangeSources())
            {
        ...
        public static IEnumerable<object[]> UnaryCancelingOperators()
        {
            ...
            yield return new object[]
            {
                Labeled.Label<Func<ParallelQuery<int>, Action, ParallelQuery<int>>>(
                    "GroupBy-Comparer",
                    (source, cancel) =>
                        source.GroupBy(x => x, new CancelingEqualityComparer<int>(cancel))
                            .Select(g => g.Key)
                )
            };
            yield return new object[]
        ...
        {
            return query.Append(
                Label("AsOrdered", (start, count, source) => source(start, count).AsOrdered())
            );
        }
    }
}
