    ...
    public static class ConvertTests
    {
        ...
        )
        {
            List<MethodInfo> opMethods = typeof(ImplicitHalfLiftedOverloaded)
                ...
                .ToList();
            MethodInfo direct = opMethods.First(m =>
                m.GetParameters()[0].ParameterType == typeof(ImplicitHalfLiftedOverloaded?)
            );
            MethodInfo liftNeeded = opMethods.First(m =>
                m.GetParameters()[0].ParameterType == typeof(ImplicitHalfLiftedOverloaded)
            );
            ParameterExpression x = Expression.Parameter(typeof(ImplicitHalfLiftedOverloaded?));
            Expression<Func<ImplicitHalfLiftedOverloaded?, HalfLiftedTo?>> e = Expression.Lambda<
        ...
        {
            Assert.Throws<InvalidOperationException>(() =>
                Expression.Convert(Expression.Constant(new PerverselyNamedMembers(0)), typeof(uint))
            );
        }
