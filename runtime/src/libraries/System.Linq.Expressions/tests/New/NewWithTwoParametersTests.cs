    ...
    public static class NewWithTwoParametersTests
    {
        ...
        )
        {
            ...
            for (int i = 0; i < array1.Length; i++)
            {
                for (int j = 0; j < array2.Length; j++)
                {
                    VerifyNewWithTwoParametersStructWithStringAndValue(
                        array1[i],
                        array2[j],
                        useInterpreter
                    );
        ...
        private static void VerifyNewWithTwoParametersStructWithValue(
            int a,
            double b,
            bool useInterpreter
        )
        ...
        private static void VerifyNewWithTwoParametersStructWithStringAndValue(
            string a,
            S b,
            bool useInterpreter
        )
        ...
        public static void UpdateDifferentReturnsDifferent()
        {
            ...
            NewExpression newExp = Expression.New(
                ...
                Expression.Constant(40.0)
            );
            Assert.NotSame(
                newExp,
                newExp.Update(new[] { Expression.Constant(23), Expression.Constant(40.0) })
            );
            ...
            newExp = Expression.New(
                constructor,
                new[] { Expression.Constant("x"), Expression.Constant(23) },
                members
            );
            Assert.NotSame(
                newExp,
                newExp.Update(new[] { Expression.Constant("x"), Expression.Constant(23) })
            );
        }
