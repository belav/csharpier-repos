...
#pragma warning restore xUnit1013
    ...
    public class Compiler_Tests
    {
        ...
        public static void PropertyAccess(bool useInterpreter)
        {
            ...
            Assert.Equal(
                cust.ContactName,
                Expression.Lambda(Expression.PropertyOrField(c, "ContactName"), c)
                    .Compile(useInterpreter)
                    .DynamicInvoke(cust)
            );
            Assert.Equal(
                cust.Orders,
                Expression.Lambda(Expression.PropertyOrField(c, "Orders"), c)
                    .Compile(useInterpreter)
                    .DynamicInvoke(cust)
            );
            Assert.Equal(
                cust.CustomerID,
                Expression.Lambda(Expression.PropertyOrField(c, "CustomerId"), c)
                    .Compile(useInterpreter)
                    .DynamicInvoke(cust)
            );
            Assert.True(
        ...
        )
        {
            ...
            Expression.Lambda(Expression.Add(p, p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
            Expression.Lambda(Expression.Subtract(p, p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
            Expression.Lambda(Expression.Multiply(p, p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
            Expression.Lambda(Expression.Divide(p, p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
        }
        ...
        )
        {
            ...
            if (testModulo)
                Expression.Lambda(Expression.Modulo(p, p), p)
                    .Compile(useInterpreter)
                    .DynamicInvoke(new object[] { value });
            Expression.Lambda(Expression.Equal(p, p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
            Expression.Lambda(Expression.NotEqual(p, p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
            Expression.Lambda(Expression.LessThan(p, p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
            Expression.Lambda(Expression.LessThanOrEqual(p, p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
            Expression.Lambda(Expression.GreaterThan(p, p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
            Expression.Lambda(Expression.GreaterThanOrEqual(p, p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
        }
        ...
        {
            ParameterExpression p = Expression.Parameter(type, "x");
            Expression.Lambda(Expression.Not(p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
            Expression.Lambda(Expression.Or(p, p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
            Expression.Lambda(Expression.And(p, p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
            Expression.Lambda(Expression.ExclusiveOr(p, p), p)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { value });
        }
        ...
        )
        {
            ...
            Expression.Lambda(Expression.OrElse(x, y), x, y)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { arg1, arg2 });
        ...
        )
        {
            ...
            Expression.Lambda(Expression.And(x, y), x, y)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { arg1, arg2 });
            Expression.Lambda(Expression.Or(x, y), x, y)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { arg1, arg2 });
            Expression.Lambda(Expression.Not(x), x)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { arg1 });
            Expression.Lambda(Expression.Equal(x, y), x, y)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { arg1, arg2 });
            Expression.Lambda(Expression.NotEqual(x, y), x, y)
                .Compile(useInterpreter)
                .DynamicInvoke(new object[] { arg1, arg2 });
        }
