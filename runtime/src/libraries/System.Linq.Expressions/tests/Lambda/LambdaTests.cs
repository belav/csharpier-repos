...
namespace System.Linq.Expressions.Tests
{
    ...
    public class LambdaTests
    {
        ...
        public void Lambda(bool useInterpreter)
        {
            ...
            Assert.Equal(
                typeof(Func<int, double, decimal, Func<int, double, decimal, int>>),
                Expression.Lambda(lambda, paramI, paramJ, paramK).Type
            );

            lambda = (Expression<Func<int, double, decimal, short, int>>)((i, j, k, l) => i);

            Assert.IsType<
                Func<int, double, decimal, short, Func<int, double, decimal, short, int>>
            >(Expression.Lambda(lambda, paramI, paramJ, paramK, paramL).Compile(useInterpreter));
        ...
        public void InvokeComputedLambda(bool useInterpreter)
        {
            ...
            Expression call = Expression.Call(
                null,
                GetType()
                    .GetMethod(nameof(ComputeLambda), BindingFlags.Static | BindingFlags.NonPublic),
                y
            );
        ...
        {
            ParameterExpression[] paramList = Enumerable.Range(0, 20)
                .Select(_ => Expression.Variable(typeof(int)))
                .ToArray();
        ...
        {
            LambdaExpression exp = Expression.Lambda(Expression.Empty());
            Assert.IsAssignableFrom<Expression<Action>>(exp);
            Assert.Equal(typeof(Action), exp.Type);

            exp = Expression.Lambda(Expression.Constant(3));
            ...
            ParameterExpression[] paramList = Enumerable.Range(0, 20)
                .Select(_ => Expression.Variable(typeof(int)))
                .ToArray();
            exp = Expression.Lambda(Expression.Constant(0), paramList);
        ...
        public void AutoQuote(bool useInterpreter)
        {
            ...
            Expression<Func<Expression<Func<int, int>>>> outer = Expression.Lambda<
        ...
        {
            Action act = Expression.Lambda<Action>(Expression.Constant("foo"))
        ...
        public void NameNeedNotBeValidCSharpLabel(bool useInterpreter)
        {
            ...
            exp = Expression.Lambda<Func<int>>(
        ...
        public void UpdateSameReturnsSame()
        {
            ...
            pars = pars.Append(Expression.Parameter(typeof(int))).ToArray();
            Expression<Action<int, int, int>> lambda3 = Expression.Lambda<Action<int, int, int>>(
                body,
                pars
            );
            Assert.Same(lambda3, lambda3.Update(body, pars));
            pars = pars.Append(Expression.Parameter(typeof(int))).ToArray();
            Expression<Action<int, int, int, int>> lambda4 = Expression.Lambda<
        ...
        public void UpdateDoesntRepeatEnumeration()
        {
            ...
            Assert.Same(
                lambda0,
                lambda0.Update(body, new RunOnceEnumerable<ParameterExpression>(pars))
            );
            pars = pars.Append(Expression.Parameter(typeof(int))).ToArray();
            Expression<Action<int>> lambda1 = Expression.Lambda<Action<int>>(body, pars);
            Assert.Same(
                lambda1,
                lambda1.Update(body, new RunOnceEnumerable<ParameterExpression>(pars))
            );
            pars = pars.Append(Expression.Parameter(typeof(int))).ToArray();
            Expression<Action<int, int>> lambda2 = Expression.Lambda<Action<int, int>>(body, pars);
            Assert.Same(
                lambda2,
                lambda2.Update(body, new RunOnceEnumerable<ParameterExpression>(pars))
            );
            pars = pars.Append(Expression.Parameter(typeof(int))).ToArray();
            Expression<Action<int, int, int>> lambda3 = Expression.Lambda<Action<int, int, int>>(
                body,
            ...
            Assert.Same(
                lambda3,
                lambda3.Update(body, new RunOnceEnumerable<ParameterExpression>(pars))
            );
            pars = pars.Append(Expression.Parameter(typeof(int))).ToArray();
            Expression<Action<int, int, int, int>> lambda4 = Expression.Lambda<
                Action<int, int, int, int>
            ...
            Assert.Same(
                lambda4,
                lambda4.Update(body, new RunOnceEnumerable<ParameterExpression>(pars))
            );
        }
        ...
        private static void VerifyUpdateDifferentParamsReturnsDifferent<TDelegate>(
            ...
            if (pars.Length != 0)
            {
                ...
                IEnumerable<ParameterExpression> diffPars =
                    new RunOnceEnumerable<ParameterExpression>(
        ...
        public void CurriedFunctionsReadWrite(bool useInterpreter)
        {
            ...
            for (var i = 0; i < 10; i++)
            {
                ...
                Expression<Func<int, Func<int>>> e = Expression.Lambda<Func<int, Func<int>>>(
                    Expression.Lambda<Func<int>>(body),
                    val
                );
        ...
        public void CurriedFunctionsVariableCaptureSemantics(bool useInterpreter)
        {
            ...
            Expression<Func<Func<int>>> e = Expression.Lambda<Func<Func<int>>>(
                Expression.Block(
                    new[] { f, x },
        ...
        public void ParameterListBehavior(int parCount, string name, bool tailCall)
        {
            ...
            ParameterExpression[] pars = Enumerable.Range(0, parCount)
                .Select(_ => Expression.Parameter(typeof(int)))
                .ToArray();
...
"
        ...
        public void AboveByteMaxArityArgAddress(bool useInterpreter)
        {
            ...
            ParameterExpression[] pars = Enumerable.Range(0, 299)
                ...
                .Append(parToMutate)
                .ToArray();
            Expression<TricentaryIntAndMutableFunc> lambda =
...
"
        ...
        {
            ParameterExpression[] pars = Enumerable.Range(0, ushort.MaxValue)
                .Select(_ => Expression.Parameter(typeof(int)))
                .ToArray();
            LambdaExpression lambda = Expression.Lambda(pars.Last(), pars);
        ...
        public void ValidateThatInterpreterWithSimpleTypeUsesNonDynamicThunk()
        {
            ...
            Assert.True(
                action.Compile(preferInterpretation: true).Method.GetType().Name
                    == "RuntimeMethodInfo"
            );
            Expression<Action<int>> action1 = (int x) => Console.WriteLine(x.ToString());
            Assert.True(
                action1.Compile(preferInterpretation: true).Method.GetType().Name
                    == "RuntimeMethodInfo"
            );
            Expression<Action<int, object>> action2 = (int x, object y) => Console.WriteLine(y);
            Assert.True(
                action2.Compile(preferInterpretation: true).Method.GetType().Name
                    == "RuntimeMethodInfo"
            );

            Expression<Func<object>> func = () => null;
            Assert.True(
                func.Compile(preferInterpretation: true).Method.GetType().Name
                    == "RuntimeMethodInfo"
            );
            Expression<Func<object, object>> func1 = (object o) => null;
            Assert.True(
                func1.Compile(preferInterpretation: true).Method.GetType().Name
                    == "RuntimeMethodInfo"
            );
            Expression<Func<object, object, object>> func2 = (object o, object o2) => null;
            Assert.True(
                func2.Compile(preferInterpretation: true).Method.GetType().Name
                    == "RuntimeMethodInfo"
            );
        }
        ...
        {
            Expression<Action<object, object, object>> complexaction = (
                object o1,
                object o2,
                object o3
            ) => Console.WriteLine("");
            Assert.True(
                complexaction.Compile(preferInterpretation: true).Method.GetType().Name
                    == "RTDynamicMethod"
            );

            Expression<Func<object, object, object, object>> complexfunc = (
                object o1,
                object o2,
                object o3
            ) => null;
            Assert.True(
                complexfunc.Compile(preferInterpretation: true).Method.GetType().Name
                    == "RTDynamicMethod"
            );
        }
