    ...
    public class VariableTests : ParameterExpressionTests
    {
        ...
        public void VariableCannotBeTypeVoid()
        {
            AssertExtensions.Throws<ArgumentException>(
                "type",
                () => Expression.Variable(typeof(void))
            );
            AssertExtensions.Throws<ArgumentException>(
                "type",
                () => Expression.Variable(typeof(void), "var")
            );
        }
        ...
        public void CanUseAsLambdaParameter(bool useInterpreter)
        {
            ...
            Func<int, int> addOne = Expression
                .Lambda<Func<int, int>>(Expression.Add(variable, Expression.Constant(1)), variable)
                .Compile(useInterpreter);
        ...
        public void CannotBePointerType()
        {
            ...
            AssertExtensions.Throws<ArgumentException>(
                "type",
                () => Expression.Variable(typeof(int*), "pointer")
            );
        }
    }
}
