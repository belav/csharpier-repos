...
namespace System.Linq.Expressions.Tests
{
    public class LoopTests
    {
        ...
        public void TypeIsBreaksType()
        {
            ...
            Assert.Equal(
                typeof(void),
                Expression.Loop(Expression.Constant(3), voidLabelTarget).Type
            );
            Assert.Equal(
                typeof(void),
                Expression.Loop(Expression.Constant(3), voidLabelTarget, null).Type
            );
        ...
        {
            string labelName =
                "Not likely to appear for any other reason {E90FAF9D-1934-4FC9-93EB-BCE70B586146}";
            LabelTarget @break = Expression.Label(labelName);
            Expression<Action> lambda = Expression.Lambda<Action>(
                Expression.Loop(Expression.Label(@break), @break)
            );
            Assert.Contains(
                labelName,
                Assert
        ...
        {
            string labelName =
                "Not likely to appear for any other reason {F9C549FE-6E6C-44A2-A434-0147E0D49F7F}";
            LabelTarget @continue = Expression.Label(labelName);
            Expression<Action> lambda = Expression.Lambda<Action>(
                Expression.Loop(Expression.Label(@continue), null, @continue)
            );
        ...
        {
            string labelName =
                "Not likely to appear for any other reason {D3C6FCD8-EA2F-440B-938F-C81560C3BDBA}";
            LabelTarget @break = Expression.Label(labelName);
            Expression<Action> lambda = Expression.Lambda<Action>(
                Expression.Block(
        ...
        {
            string labelName =
                "Not likely to appear for any other reason {1107D64D-9FC4-4533-83E2-0F5F78B48315}";
            LabelTarget @continue = Expression.Label(labelName);
            Expression<Action> lambda = Expression.Lambda<Action>(
                Expression.Block(
        ...
        {
            string labelName =
                "Not likely to appear for any other reason {B9CD9CF5-6C67-41C9-98C0-F445CAAB5082}";
        ...
        public void NoLabelsInfiniteLoop(bool useInterpreter)
        {
            ...
            Action spinThenThrow = Expression
                .Lambda<Action>(
                    Expression.Block(
                        new[] { num },
        ...
        public void NoBreakToLabelInfiniteLoop(bool useInterpreter)
        {
            // Have an error condition so the otherwise-infinite loop can complete.
            ParameterExpression num = Expression.Variable(typeof(int));
            Func<int> spinThenThrow = Expression
                .Lambda<Func<int>>(
                    Expression.Block(
                        new[] { num },
                        Expression.Assign(num, Expression.Constant(0)),
        ...
        public void LoopWithBreak(bool useInterpreter)
        {
            ...
            Func<int, int> factorial = Expression
        ...
        public void UpdateDifferentBodyIsDifferent()
        {
            ...
            Assert.NotSame(
                loop,
                loop.Update(loop.BreakLabel, loop.ContinueLabel, Expression.Empty())
            );
        }
