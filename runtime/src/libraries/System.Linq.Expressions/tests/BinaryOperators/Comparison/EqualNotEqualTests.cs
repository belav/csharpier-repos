    ...
    public static class BinaryEqualTests
    {
        public static IEnumerable<object[]> TestData()
        {
            foreach (bool useInterpreter in new bool[] { true, false })
            {
                ...
                yield return new object[]
                {
                    new TestEnum[] { new TestEnum(), new TestEnum() },
                    useInterpreter
                };
                ...
                yield return new object[]
                {
                    new E?[] { null, E.A, E.B, (E)int.MaxValue, (E)int.MinValue },
                    useInterpreter
                };
            }
        }
            ...
            for (int i = 0; i < array.Length; i++)
            {
                for (int j = 0; j < array.Length; j++)
                {
                    ...
                    GeneralBinaryTests.CompileBinaryExpression(
                        equal,
                        useInterpreter,
                        GeneralBinaryTests.CustomEquals(a, b)
                    );
                }
            }
        }
        ...
        public static void NotEqual(Array array, bool useInterpreter)
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                for (int j = 0; j < array.Length; j++)
                {
                    object a = array.GetValue(i);
                    object b = array.GetValue(j);
                    BinaryExpression equal = Expression.NotEqual(
                        Expression.Constant(a, type),
                        Expression.Constant(b, type)
                    );
                    GeneralBinaryTests.CompileBinaryExpression(
        ...
        public static void ThrowsOnLeftNull()
        {
            AssertExtensions.Throws<ArgumentNullException>(
                "left",
                () => Expression.Equal(null, Expression.Constant(""))
            );
            AssertExtensions.Throws<ArgumentNullException>(
                "left",
                () => Expression.NotEqual(null, Expression.Constant(""))
            );
        }
        ...
        public static void ThrowsOnRightNull()
        {
            ...
            AssertExtensions.Throws<ArgumentNullException>(
                "right",
                () => Expression.NotEqual(Expression.Constant(""), null)
            );
        }
        ...
        public static void ThrowsOnLeftUnreadable()
        {
            ...
            AssertExtensions.Throws<ArgumentException>(
                "left",
                () => Expression.NotEqual(value, Expression.Constant(1))
            );
        }
        ...
        public static void ThrowsOnRightUnreadable()
        {
            ...
            AssertExtensions.Throws<ArgumentException>(
                "right",
                () => Expression.Equal(Expression.Constant(1), value)
            );
            AssertExtensions.Throws<ArgumentException>(
                "right",
                () => Expression.NotEqual(Expression.Constant(1), value)
            );
        }
        ...
        public static void Update_ReferenceEquals(bool useInterpreter)
        {
            ...
            BinaryExpression equal = Expression.Equal(
                Expression.Constant(testClass1),
                Expression.Constant(testClass2)
            );
            BinaryExpression newEqual = equal.Update(
                Expression.Constant(testClass1),
                equal.Conversion,
                Expression.Constant(testClass1)
            );
        ...
        {
            BinaryExpression e = Expression.Equal(
        ...
        {
            BinaryExpression e = Expression.NotEqual(
                Expression.Parameter(typeof(int), "a"),
                Expression.Parameter(typeof(int), "b")
            );
            Assert.Equal("(a != b)", e.ToString());
        ...
        public static void CanCompareDBNullEqual(bool useInterpreter)
        {
            ...
            var lambda = Expression.Lambda<Func<DBNull, DBNull, bool>>(
