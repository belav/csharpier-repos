    ...
    public class TokenBucketRateLimiterTests : BaseRateLimiterTests
    {
        ...
        public override async Task ThrowsWhenWaitingForMoreThanLimit()
        {
            ...
            await Assert.ThrowsAsync<ArgumentOutOfRangeException>(
                async () => await limiter.WaitAsync(2)
            );
        }
        ...
        public override async Task ThrowsWhenWaitingForLessThanZero()
        {
            ...
            await Assert.ThrowsAsync<ArgumentOutOfRangeException>(
                async () => await limiter.WaitAsync(-1)
            );
        }
        ...
        public override async Task CanCancelWaitAsyncBeforeQueuing()
        {
            ...
            var ex = await Assert.ThrowsAsync<TaskCanceledException>(
                () => limiter.WaitAsync(1, cts.Token).AsTask()
            );
            Assert.Equal(cts.Token, ex.CancellationToken);
        ...
        public async Task ReplenishWorksWithTicksOverInt32Max()
        {
            ...
            replenishInternalMethod.Invoke(
                limiter,
                new object[] { tick + 1L * (long)(TimeSpan.TicksPerMillisecond / TickFrequency) }
            );
            Assert.False(wait.IsCompleted);
            ...
            replenishInternalMethod.Invoke(
                limiter,
                new object[] { tick + 2L * (long)(TimeSpan.TicksPerMillisecond / TickFrequency) }
            );
            lease = await wait;
            Assert.True(lease.IsAcquired);
        }
        ...
        public void ReplenishingRateLimiterPropertiesHaveCorrectValues()
        {
            ...
            using ReplenishingRateLimiter limiter = new TokenBucketRateLimiter(
                new TokenBucketRateLimiterOptions(
                    1,
                    QueueProcessingOrder.OldestFirst,
                    2,
            ...
            using ReplenishingRateLimiter limiter2 = new TokenBucketRateLimiter(
                new TokenBucketRateLimiterOptions(
                    1,
                    QueueProcessingOrder.OldestFirst,
                    2,
