...
namespace System.Threading.RateLimiting
{
    ...
    public sealed class ConcurrencyLimiter : RateLimiter
    {
        ...
        private static readonly ConcurrencyLease SuccessfulLease = new ConcurrencyLease(
            true,
            null,
            0
        );
        private static readonly ConcurrencyLease FailedLease = new ConcurrencyLease(false, null, 0);
        private static readonly ConcurrencyLease QueueLimitLease = new ConcurrencyLease(
            false,
            null,
            0,
            "Queue limit reached"
                ...
                while (_queue.Count > 0)
                {
                    ...
                    if (_permitCount >= nextPendingRequest.Count)
                    {
                        ...
                        ConcurrencyLease lease =
                            nextPendingRequest.Count == 0
                                ? SuccessfulLease
                                : new ConcurrencyLease(true, this, nextPendingRequest.Count);
