...
namespace Microsoft.Extensions.Logging.Generators
{
    public partial class LoggerMessageGenerator
    {
        internal sealed class Emitter
        {
            ...
            {
                bool result =
                    (lm.TemplateParameters.Count <= MaxLoggerMessageDefineArguments)
                    && // more args than LoggerMessage.Define can handle
                    (lm.Level != null)
                    && // dynamic log level, which LoggerMessage.Define can't handle
                ...
                if (result)
                {
                    ...
                    {
                        ReadOnlySpan<char> template = RemoveSpecialSymbol(
                            lm.TemplateList[i].AsSpan()
                        );
                        ReadOnlySpan<char> parameter = RemoveSpecialSymbol(
                            lm.TemplateParameters[i].CodeName.AsSpan()
                        );
...
"
            ...
            private void GenFields(LoggerMethod lm, string nestedIndentation)
            {
                ...
                {
                    _builder.AppendLine(
                        $"            {nestedIndentation}private readonly {p.Type} {NormalizeSpecialSymbol(p.CodeName)};"
                    );
                }
            }
            ...
            private void GenFieldAssignments(LoggerMethod lm, string nestedIndentation)
            {
                ...
                {
                    _builder.AppendLine(
                        $"                {nestedIndentation}this.{NormalizeSpecialSymbol(p.CodeName)} = {p.CodeName};"
                    );
                }
            }
            ...
            private void GenVariableAssignments(LoggerMethod lm, string nestedIndentation)
            {
                foreach (KeyValuePair<string, string> t in lm.TemplateMap)
                {
                    ...
                    if (index < lm.TemplateParameters.Count)
                    {
                        ...
                        {
                            _builder.AppendLine(
                                $"                {nestedIndentation}var {t.Key} = this.{NormalizeSpecialSymbol(lm.TemplateParameters[index].CodeName)};"
                            );
                        }
                    }
                }
            ...
            private void GenCases(LoggerMethod lm, string nestedIndentation)
            {
                ...
                foreach (LoggerParameter p in lm.TemplateParameters)
                {
                    ...
                    _builder.AppendLine(
                        $"                    {nestedIndentation}{index++} => new global::System.Collections.Generic.KeyValuePair<string, object?>(\"{name}\", this.{NormalizeSpecialSymbol(p.CodeName)}),"
                    );
                }

                _builder.AppendLine(
                    $"                    {nestedIndentation}{index++} => new global::System.Collections.Generic.KeyValuePair<string, object?>(\"{{OriginalFormat}}\", \"{ConvertEndOfLineAndQuotationCharactersToEscapeForm(lm.Message)}\"),"
                );
            }
        ...
        {nestedIndentation}private static readonly global::System.Action<global::Microsoft.Extensions.Logging.ILogger, "
                    ...
                    _builder.Append(
                        $@"global::System.Exception?> __{lm.UniqueName}Callback =
...
{{
    ...
    {{
        ...
        private static string NormalizeSpecialSymbol(string variableOrTemplateName) =>
            ContainsSpecialSymbol(variableOrTemplateName.AsSpan())
                ? variableOrTemplateName
                : $"_{variableOrTemplateName}";
        ...
        /// <remarks>This code only handles starting symbols. Symbols inside string will be kept.</remarks>
        private static ReadOnlySpan<char> RemoveSpecialSymbol(
            ReadOnlySpan<char> variableOrTemplateName
        ) =>
            ContainsSpecialSymbol(variableOrTemplateName)
                ? variableOrTemplateName.Slice(1)
                : variableOrTemplateName;
    }
}
