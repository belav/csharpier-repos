...
namespace System.Data.SqlTypes.Tests
{
    ...
    {
        private static readonly string[] s_sampleString = new string[]
        {
            "In",
            "its",
            "first",
        ...
        private static readonly UnicodeEncoding s_unicodeEncoding = new UnicodeEncoding(
            bigEndian: false,
            byteOrderMark: false,
        ...
        public static void SqlStringValidComparisonTest(string cultureName, int localeId)
        {
            ...
            const SqlCompareOptions DefaultCompareOption =
            ...
            foreach (
                SqlCompareOptions option in new[]
                {
        ...
        public static void SqlStringNullComparisonTest()
        {
            ...
            Assert.True(
                (bool)(
                    nullSqlString < nonNullSqlString
            ...
            Assert.True(
                (nullSqlString == null && nullSqlString.CompareTo(null) == 0).IsNull,
                "FAILED: (SqlString Null Comparison): Null SqlString not equal to null"
            );
        }
        ...
        private static void SqlStringCompareTest(
            int numberOfItems,
        ...
        private static SortedList<SqlString, SqlString> CreateSortedSqlStringList(
            int numberOfItems,
            SqlCompareOptions compareOption,
            CultureInfo cInfo,
            int localeID
        ...
        private static void VerifySortedSqlStringList(
        ...
        )
        {
            ...
            for (int i = 0; i < items.Count - 1; ++i)
            {
                ...
                Assert.True(
                    (bool)((currentString < nextString) && (nextString >= currentString)),
                    "FAILED: (SqlString Operator Comparison): SqlStrings are out of order"
                );
                Assert.True(
                    (currentString.CompareTo(nextString) < 0)
                        && (nextString.CompareTo(currentString) > 0),
                    "FAILED: (SqlString.CompareTo): SqlStrings are out of order"
                );
                ...
                switch (compareOption)
                {
                    case SqlCompareOptions.BinarySort:
                        Assert.True(
                            CompareBinary(currentString.Value, nextString.Value) < 0,
                            "FAILED: (SqlString BinarySort Comparison): SqlStrings are out of order"
                        );
                        break;
                    ...
                    default:
                        ...
                        CompareOptions cmpOptions = SqlString.CompareOptionsFromSqlCompareOptions(
                            nextString.SqlCompareOptions
                        );
                        ...
                        Assert.True(
                            ...
                            "FAILED: (SqlString Comparison): SqlStrings are out of order"
                        );
                        break;
                }
            }
