...
namespace System.Data
{
    ...
    internal sealed class XmlTreeGen
    {
        ...
        )
        {
            if (props != null)
            {
                foreach (DictionaryEntry entry in props)
                {
                    ...
                    {
                        v = (string)BigIntegerStorage.ConvertFromBigInteger(
            ...
            foreach (Constraint cs in dt.Constraints)
            {
                ...
                if (null != fk)
                {
                    ...
                    else
                    {
                        ...
                        UniqueConstraint? _constraint =
                            (UniqueConstraint?)fk.RelatedTable.Constraints
                                .FindConstraint(
        ...
        )
        {
            ...
            if (
                _schFormat != SchemaFormat.Remoting && _schFormat != SchemaFormat.RemotingSkipSchema
            )
            {
        ...
        )
        {
            ...
            for (int i = 0; i < constraints.Count; i++)
            {
                ...
                else if (constraints[i] is ForeignKeyConstraint && genNested)
                {
                    ...
                    UniqueConstraint? _constraint =
                        (UniqueConstraint?)foreign.RelatedTable.Constraints
                    ...
                    if (_constraint == null)
                    {
                        ...
                        if (0 < fields.Length)
                        {
                            ...
                            for (int k = 0; k < fields.Length; k++)
                            {
                                ...
                                {
                                    encodedName.Append(xpathprefix)
    ...
    internal sealed class NewDiffgramGen
    {
        ...
        internal void Save(XmlWriter xmlw, DataTable? table)
        {
            ...
            if (!EmptyData())
            {
                // write the datapart
                if (table != null)
                    new XmlDataTreeWriter(table, _writeHierarchy)
                        .SaveDiffgramData(_xmlw, _rowsOrder);
                else
        ...
        internal void Save(XmlWriter xw, bool writeSchema)
        {
            ...
            if (fWriteDSElement)
            {
                ...
                if (writeSchema)
                {
                    ...
                    {
                        new XmlTreeGen(SchemaFormat.Public)
                            .Save(null, _dt!, _xmlw, _writeHierarchy);
                    }
                }
            }
