    ...
    public class XmlDataDocument : XmlDocument
    {
        ...
        private XmlElement EnsureDocumentElement()
        {
            ...
            if (docelem == null)
            {
                ...
                docelem = new XmlBoundElement(
                    string.Empty,
                    docElemName,
                    DataSet.Namespace ?? string.Empty,
                    this
                );
                AppendChild(docelem);
            }
...
#pragma warning restore 1717
        ...
        private XmlNode CloneNode(DataPointer dp) =>
            dp.NodeType switch
            {
                ...
                XmlNodeType.DocumentType => CreateDocumentType(
                    dp.Name,
                    dp.PublicId,
                    dp.SystemId,
                    dp.InternalSubset
                ),
                XmlNodeType.XmlDeclaration => CreateXmlDeclaration(
                    dp.Version!,
                    dp.Encoding,
                    dp.Standalone
                ),
                ...
                XmlNodeType.ProcessingInstruction => CreateProcessingInstruction(
                    dp.Name,
                    dp.Value!
                ),
        ...
        private void OnColumnValueChanged(DataRow row, DataColumn col, XmlBoundElement rowElement)
        {
            ...
            else
            {
                ...
                if (!fFound && !Convert.IsDBNull(value))
                {
                    var newElem = new XmlBoundElement(
                        string.Empty,
                        col.EncodedColumnName,
                        col.Namespace,
                        this
