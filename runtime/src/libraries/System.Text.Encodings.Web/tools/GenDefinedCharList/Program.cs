...
namespace GenDefinedCharList
{
    ...
    class Program
    {
            ...
            foreach (string line in allLines)
            {
                ...
                if (IsRangeDefinition(rawName, out rangeName, out isStartOfRange))
                {
                    ...
                    else
                    {
                        ...
                        Debug.Assert(
                            existingRange.FirstCodePoint != 0,
                            "We should've seen the start of this range already."
                        );
                        Debug.Assert(
                            existingRange.LastCodePoint == 0,
                            "We shouldn't have seen the end of this range already."
                        );
                        Debug.Assert(
                            existingRange.Category == category,
                            "Range start Unicode category doesn't match range end Unicode category."
                        );
        ...
        private static bool IsRangeDefinition(
            string rawName,
