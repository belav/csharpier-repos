...
namespace System.IO.Ports.Tests
{
    public class ReadByte_Generic : PortsTest
    {
        ...
        public void NoParityReplaceByte()
        {
            ...
            //        if (!VerifyParityReplaceByte((int)'\0', rndGen.Next(0, numRndByte - 1), new
...
more info on this */
        ...
        private void VerifyParityReplaceByte(
            int parityReplace,
        ...
        )
        {
            ...
            while (true)
            {
                ...
                if (bytesToWrite.Length - rcvBufferSize != com1.BytesToRead)
                {
                    Fail(
                        ...
                        bytesToWrite.Length - rcvBufferSize,
                        com1.BytesToRead
                    );
                }
            ...
            {
                Fail(
                    "ERROR!!! Expected to read {0} char actually read {1} chars",
                    bytesToWrite.Length,
                    rcvBufferSize
                );
            }
