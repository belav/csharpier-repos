...
namespace System.IO.Ports.Tests
{
    ...
    public class ReadBufferSize_Property : PortsTest
    {
        ...
        public void ReadBufferSize_Odd()
        {
            ...
            VerifyException(
                ((new SerialPort()).ReadBufferSize) * 2 + 1,
                typeof(IOException),
        ...
        )
        {
            ...
            )
            {
                ...
                catch (Exception e)
                {
                    ...
                    {
                        Fail(
                            "Err_545498ahpba!!! expected exception {0} and {1} was thrown",
                            expectedException,
                            e.GetType()
                        );
                    }
        ...
        private void VerifyExceptionAfterOpen(int newReadBufferSize, Type expectedException)
        {
            ...
            )
            {
                ...
                catch (Exception e)
                {
                    ...
                    {
                        Fail(
                            "Err_454987ahbopa!!! expected ReadBufferSize={0} and actual={1}",
                            originalReadBufferSize,
                            com.ReadBufferSize
                        );
                    }
        ...
        {
            Debug.WriteLine(
                "Verifying setting ReadBufferSize={0} BEFORE a call to Open() has been made",
                newReadBufferSize
            );
        ...
        private void VerifyReadBufferSizeBeforeOpen(int newReadBufferSize)
        {
            ...
            )
            {
                ...
                for (int j = 0; j < 1; j++)
                {
                    ...
                    Assert.Equal(
                        xmitBytes.Take(newReadBufferSize),
        ...
        private void VerifyReadBufferSize(SerialPort com1)
        {
            ...
            )
            {
                ...
                {
                    Fail(
                        "Err_2971ahius Did not read all expected bytes({0}) bytesRead={1} ReadBufferSize={2}",
                        bytesToRead,
                        bytesRead,
                        com1.ReadBufferSize
                    );
                ...
                for (int i = 0; i < bytesToRead; i++)
                {
                    ...
                    {
                        Fail(
                            "Err_70929apba!!!: Expected to read byte {0} actual={1} at {2}",
                            xmitBytes[i % xmitBytes.Length],
                            rcvBytes[i],
                            i
                        );
                    }
