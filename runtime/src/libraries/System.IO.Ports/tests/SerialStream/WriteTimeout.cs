...
namespace System.IO.Ports.Tests
{
    public class SerialStream_WriteTimeout_Property : PortsTest
    {
        ...
        {
            Debug.WriteLine(
                "Verifying Int32.MaxValue WriteTimeout with Write(byte[] buffer, int offset, int count)"
            );
        ...
        {
            Debug.WriteLine(
                "Verifying 750 WriteTimeout with Write(byte[] buffer, int offset, int count)"
            );
        ...
        public void SuccessiveWriteTimeoutNoData_Write_byte_int_int()
        {
            ...
            )
            {
                ...
                Debug.WriteLine(
                    "Verifying WriteTimeout={0} with successive call to Write(byte[], int, int) and no data",
                    stream.WriteTimeout
                );
        ...
        public void SuccessiveWriteTimeoutNoData_WriteByte()
        {
            ...
            )
            {
                ...
                Debug.WriteLine(
                    "Verifying WriteTimeout={0} with successive call to WriteByte() and no data",
                    stream.WriteTimeout
                );
        ...
        {
            Debug.WriteLine(
                "Verifying 0 WriteTimeout with Write(byte[] buffer, int offset, int count)"
            );
        ...
        public void WriteTimeout_LargeWrite()
        {
            ...
            )
            {
                ...
                Debug.WriteLine(
                    "Verifying {0} WriteTimeout with Write(byte[] , int, int) and writing {1} bytes",
                    LARGEWRITE_TIMEOUT,
                    DEFAULT_WRITE_BYTE_LARGE_ARRAY_SIZE
                );
        ...
        {
            stream.Write(
                new byte[DEFAULT_WRITE_BYTE_LARGE_ARRAY_SIZE],
                0,
                DEFAULT_WRITE_BYTE_LARGE_ARRAY_SIZE
            );
        }
