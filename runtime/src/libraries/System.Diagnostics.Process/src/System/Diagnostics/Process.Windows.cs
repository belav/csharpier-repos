    ...
    public partial class Process : IDisposable
    {
        ...
        public static void EnterDebugMode()
        {
            SetPrivilege(
                ...
                (int)Interop.Advapi32.SEPrivileges.SE_PRIVILEGE_ENABLED
            );
        }
            ...
            if (!_watchingForExit)
            {
                lock (this)
                {
                    if (!_watchingForExit)
                    {
                        ...
                        try
                        {
                            _waitHandle = new Interop.Kernel32.ProcessWaitHandle(
            ...
            get
            {
                using (
                    ...
                    IntPtr processAffinity,
                        systemAffinity;
                    if (
                        !Interop.Kernel32.GetProcessAffinityMask(
                            handle,
                            out processAffinity,
                            out systemAffinity
                        )
        ...
        private unsafe bool StartWithCreateProcess(ProcessStartInfo startInfo)
        {
            ...
            lock (s_createProcessLock)
            {
                try
                {
                    ...
                    if (
                        ...
                        {
                            CreatePipe(
                                out parentOutputPipeHandle,
                                out childOutputPipeHandle,
                                false
                            );
                        }
                        else
                        ...
                        startupInfo.dwFlags =
                            Interop.Advapi32.StartupInfoOptions.STARTF_USESTDHANDLES;
                    }
                    ...
                    {
                        creationFlags |=
                            Interop.Advapi32.StartupInfoOptions.CREATE_UNICODE_ENVIRONMENT;
                    ...
                    if (
                        ...
                        && processInfo.hProcess != new IntPtr(-1)
                    )
            ...
            if (startInfo.RedirectStandardInput)
            {
                Encoding enc =
                    ...
                    ?? GetEncoding((int)Interop.Kernel32.GetConsoleCP());
                _standardInput = new StreamWriter(
                    new FileStream(parentInputPipeHandle!, FileAccess.Write, 4096, false),
                    enc,
                    4096
                );
            ...
            if (startInfo.RedirectStandardOutput)
            {
                Encoding enc =
                    ...
                    ?? GetEncoding((int)Interop.Kernel32.GetConsoleOutputCP());
        ...
        private SafeProcessHandle GetProcessHandle(int access, bool throwIfExited = true)
        {
            ...
            {
                EnsureState(State.HaveId | State.IsLocal);
                SafeProcessHandle handle = ProcessManager.OpenProcess(
                    _processId,
                    access,
                    throwIfExited
                );
