...
namespace System.Net.WebSockets
{
    ...
    internal sealed partial class ManagedWebSocket : WebSocket
    {
        ...
        private static readonly Task<WebSocketReceiveResult> s_cachedCloseTask = Task.FromResult(
        ...
        )
        {
            ...
            Debug.Assert(
                ReceiveAsyncLock != null,
                $"Expected {nameof(ReceiveAsyncLock)} to be non-null"
            );
            ...
            Debug.Assert(
                ...
                $"Invalid keepalive interval: {keepAliveInterval}"
            );
            ...
            _abortSource.Token.UnsafeRegister(
                static s =>
                {
        ...
        private ValueTask SendPrivateAsync(
        ...
        )
        {
            ...
            MessageOpcode opcode = _lastSendWasFragment
        ...
        )
        {
            ...
            try
            {
                ...
                Debug.Assert(
                    !Monitor.IsEntered(StateUpdateLock),
                    $"{nameof(StateUpdateLock)} must never be held when acquiring {nameof(ReceiveAsyncLock)}"
                );
        ...
        )
        {
            try
            {
                ...
                lock (ReceiveAsyncLock) // synchronize with receives in CloseAsync
                {
                    ...
                    ValueTask<ValueWebSocketReceiveResult> receiveValueTask = ReceiveAsyncPrivate<
                        ValueWebSocketReceiveResultGetter,
                        ValueWebSocketReceiveResult
                    >(buffer, cancellationToken);
                    if (receiveValueTask.IsCompletedSuccessfully)
                    {
                        _lastReceiveAsync =
                            receiveValueTask.Result.MessageType == WebSocketMessageType.Close
                                ? s_cachedCloseTask
                                : Task.CompletedTask;
        ...
        private Task ValidateAndReceiveAsync(
            Task receiveTask,
            byte[] buffer,
            CancellationToken cancellationToken
        )
        {
            if (
                ...
                || (
                    ...
                    && !(
                        receiveTask is Task<WebSocketReceiveResult> wsrr
                        && wsrr.Result.MessageType == WebSocketMessageType.Close
                    )
                    && !(
                        receiveTask is Task<ValueWebSocketReceiveResult> vwsrr
                        && vwsrr.Result.MessageType == WebSocketMessageType.Close
                    )
                )
            )
            {
                ValueTask<ValueWebSocketReceiveResult> vt = ReceiveAsyncPrivate<
                    ...
                    ValueWebSocketReceiveResult
                >(buffer, cancellationToken);
                receiveTask = vt.IsCompletedSuccessfully
                    ? (
                          vt.Result.MessageType == WebSocketMessageType.Close
                              ? s_cachedCloseTask
                              : Task.CompletedTask
                      )
                    : vt.AsTask();
        ...
        private readonly struct ValueWebSocketReceiveResultGetter
            : IWebSocketReceiveResultGetter<ValueWebSocketReceiveResult>
        {
            public ValueWebSocketReceiveResult GetResult(
                int count,
                WebSocketMessageType messageType,
                bool endOfMessage,
                WebSocketCloseStatus? closeStatus,
...
#pragma warning restore CA1416
        ...
        {
            Debug.Assert(
                _sendFrameAsyncLock.CurrentCount == 0,
                "Caller should hold the _sendFrameAsyncLock"
            );
        ...
        )
        {
            ...
            {
                payloadBuffer.CopyTo(
                    new Span<byte>(_sendBuffer, headerLength, payloadBuffer.Length)
                );
...
#pragma warning restore CA1416
            if (acquiredLock)
            {
                ...
                ValueTask t = SendFrameLockAcquiredNonCancelableAsync(
                    MessageOpcode.Pong,
                    true,
        ...
        )
        {
            ...
            Debug.Assert(
                ...
                $"Expected sendBuffer to be at least {MaxMessageHeaderLength}, got {sendBuffer.Length}"
            );
        ...
        >(
            ...
            where TWebSocketReceiveResultGetter : struct,
                IWebSocketReceiveResultGetter<TWebSocketReceiveResult> // constrained to avoid boxing and enable inlining
        {
            ...
            try
            {
                while (true) // in case we get control frames that should be ignored from the user's perspective
                {
                    ...
                    {
                        int receiveBufferBytesToCopy = Math.Min(
                ...
                try
                {
                    while (!_receivedCloseFrame)
                    {
                        ...
                        lock (ReceiveAsyncLock)
                        {
                            ...
                            Task newReceiveTask = ValidateAndReceiveAsync(
                                receiveTask,
                                closeBuffer,
                                cancellationToken
                            );
            ...
            try
            {
                ...
                else
                {
                    ...
                    Debug.Assert(
                        ...
                        $"GetByteCount and GetBytes encoded count didn't match"
                    );
                }
            ...
            fixed (byte* toMaskBeg = &MemoryMarshal.GetReference(toMask))
            {
                ...
                if (toMaskEnd - toMaskPtr >= sizeof(int))
                {
                    ...
                    )
                    {
                        ...
                        {
                            Vector<byte> maskVector = Vector.AsVectorByte(
                                new Vector<int>(rolledMask)
                            );
        ...
        private static Exception CreateOperationCanceledException(
        ...
        private readonly struct WebSocketReceiveResultGetter
            : IWebSocketReceiveResultGetter<WebSocketReceiveResult>
        {
