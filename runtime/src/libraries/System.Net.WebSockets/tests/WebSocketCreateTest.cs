...
namespace System.Net.WebSockets.Tests
{
    ...
    {
        protected abstract WebSocket CreateFromStream(
            Stream stream,
            bool isServer,
            string subProtocol,
        ...
        {
            Assert.NotNull(
                CreateFromStream(new MemoryStream(), false, null, Timeout.InfiniteTimeSpan)
            );
            Assert.NotNull(
                CreateFromStream(new MemoryStream(), true, null, Timeout.InfiniteTimeSpan)
            );
        }
        ...
        )
        {
            ...
            )
            {
                ...
                using (
                    WebSocket socket = CreateFromStream(
                        stream,
                        false,
                        null,
        ...
        public async Task ReceiveAsync_InvalidFrameHeader_AbortsAndThrowsException(
            byte firstByte,
            byte secondByte,
            bool shouldFail
        )
            ...
            )
            {
                ...
                using (Socket server = await listener.AcceptAsync())
                {
                    ...
                    await server.SendAsync(
                        new ArraySegment<byte>(new byte[3] { firstByte, secondByte, (byte)'a' }),
                        SocketFlags.None
                    );

                    var buffer = new byte[1];
                    Task<WebSocketReceiveResult> t = websocket.ReceiveAsync(
        ...
        public async Task ReceiveAsync_InvalidPayloadLength_AbortsAndThrowsException(
            byte[] lenBytes,
            bool shouldFail
        )
        {
            ...
            using var stream = new MemoryStream(frame, writable: true);
            using WebSocket websocket = CreateFromStream(
                stream,
                false,
                null,
                Timeout.InfiniteTimeSpan
        ...
        public async Task ReceiveAsync_ServerSplitHeader_ValidDataReceived()
        {
            ...
            )
            {
                ...
                {
                    WebSocket websocket = CreateFromStream(
                        new NetworkStream(server, ownsSocket: false),
                        isServer: true,
                        null,
                        Timeout.InfiniteTimeSpan
                    );
                    ...
                    await client.SendAsync(
                        new ArraySegment<byte>(packets, 0, packet0.Count + partialPacket1.Count),
                        SocketFlags.None
                    );

                    // Read the first packet
                    byte[] received = new byte[1];
                    WebSocketReceiveResult r = await websocket.ReceiveAsync(
                        new ArraySegment<byte>(received),
                        default
                    );
                    ...
                    Task<WebSocketReceiveResult> tr = websocket.ReceiveAsync(
                        new ArraySegment<byte>(received),
                        default
                    );
        ...
        )
        {
            ...
            )
            {
                ...
                )
                {
                    ...
                    await socket.SendAsync(
                        new ArraySegment<byte>(Encoding.UTF8.GetBytes(".close")),
                        WebSocketMessageType.Text,
                        true,
                        default
                    );
                    ...
                    WebSocketReceiveResult recvResult = await socket.ReceiveAsync(
                        new ArraySegment<byte>(new byte[256]),
                        default
                    );
        ...
        private static async Task<Stream> CreateWebSocketStream(
            Uri echoUri,
            Socket client,
            bool secure
        )
