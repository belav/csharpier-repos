...
internal static partial class Interop
{
    internal static partial class HttpApi
    {
        ...
        internal static IPEndPoint s_ipv6Any = new IPEndPoint(
            IPAddress.IPv6Any,
            IPEndPoint.MinPort
        );
        ...
        {
            uint statusCode = HttpInitialize(
                version,
        ...
        [DllImport(Libraries.HttpApi, SetLastError = true)]
        internal static extern unsafe uint HttpCreateServerSession(
            HTTPAPI_VERSION version,
        ...
        [DllImport(Libraries.HttpApi, SetLastError = true)]
        internal static extern unsafe uint HttpCreateUrlGroup(
            ulong serverSessionId,
            ulong* urlGroupId,
            uint reserved
        );
        ...
        [DllImport(Libraries.HttpApi, SetLastError = true)]
        internal static extern unsafe uint HttpReceiveHttpRequest(
        ...
        [DllImport(Libraries.HttpApi, SetLastError = true)]
        internal static extern unsafe uint HttpSendHttpResponse(
            SafeHandle requestQueueHandle,
            ulong requestId,
            uint flags,
            ...
            void* pCachePolicy,
            uint* pBytesSent,
            SafeLocalAllocHandle pRequestBuffer,
            uint requestBufferLength,
            NativeOverlapped* pOverlapped,
            void* pLogData
        );
        ...
        [DllImport(Libraries.HttpApi, SetLastError = true)]
        internal static extern unsafe uint HttpWaitForDisconnect(
            SafeHandle requestQueueHandle,
            ulong connectionId,
            NativeOverlapped* pOverlapped
        );
        ...
        [DllImport(Libraries.HttpApi, SetLastError = true)]
        internal static extern unsafe uint HttpReceiveRequestEntityBody(
            SafeHandle requestQueueHandle,
            ulong requestId,
            uint flags,
            void* pEntityBuffer,
        ...
        [DllImport(Libraries.HttpApi, SetLastError = true)]
        internal static extern unsafe uint HttpSendResponseEntityBody(
            SafeHandle requestQueueHandle,
            ulong requestId,
            uint flags,
        ...
        [DllImport(Libraries.HttpApi, SetLastError = true)]
        internal static extern unsafe uint HttpReceiveClientCertificate(
            SafeHandle requestQueueHandle,
            ulong connectionId,
            uint flags,
        ...
        private static unsafe string? GetKnownHeader(
            ...
            if (NetEventSource.Log.IsEnabled())
            {
                NetEventSource.Info(
                    null,
                    $"HttpApi::GetKnownHeader() pKnownHeader:0x{(IntPtr)pKnownHeader}"
                );
                NetEventSource.Info(
                    null,
                    $"HttpApi::GetKnownHeader() pRawValue:0x{(IntPtr)pKnownHeader->pRawValue} RawValueLength:{pKnownHeader->RawValueLength}"
                );
            }
        ...
        {
            return GetVerb(
                (HTTP_REQUEST*)memoryBlob.ToPointer(),
                (byte*)memoryBlob - (byte*)originalAddress
            );
        }
        ...
        internal static unsafe IPEndPoint? GetRemoteEndPoint(
            ...
            {
                endpoint =
                    new IPEndPoint(IPAddress.Any, IPEndPoint.MinPort).Create(v4address)
                    as IPEndPoint;
            }
            else if (v6address != null)
            {
                endpoint =
                    new IPEndPoint(IPAddress.IPv6Any, IPEndPoint.MinPort).Create(v6address)
                    as IPEndPoint;
            }
        ...
        private static unsafe void CopyOutAddress(
            IntPtr address,
