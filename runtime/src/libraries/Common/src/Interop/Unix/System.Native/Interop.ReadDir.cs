    ...
    internal static partial class Sys
    {
        ...
        internal unsafe struct DirectoryEntry
        {
            ...
            internal ReadOnlySpan<char> GetName(Span<char> buffer)
            {
                ...
                Debug.Assert(
                    nameBytes.Length > 0,
                    "we shouldn't have gotten a garbage value from the OS"
                );

                int charCount = Encoding.UTF8.GetChars(nameBytes, buffer);
                ReadOnlySpan<char> value = buffer.Slice(0, charCount);
                Debug.Assert(
                    NameLength != -1 || !value.Contains('\0'),
                    "should not have embedded nulls if we parsed the end of string"
                );
                return value;
            }
        }
        ...
        [DllImport(
            ...
            EntryPoint = "SystemNative_OpenDir",
            SetLastError = true
        )]
        ...
        [DllImport(
            Libraries.SystemNative,
            EntryPoint = "SystemNative_GetReadDirRBufferSize",
            SetLastError = false
        )]
        ...
        [DllImport(
            Libraries.SystemNative,
            EntryPoint = "SystemNative_ReadDirR",
            SetLastError = false
        )]
        internal static extern unsafe int ReadDirR(
            ...
            byte* buffer,
            int bufferSize,
            out DirectoryEntry outputEntry
        );
