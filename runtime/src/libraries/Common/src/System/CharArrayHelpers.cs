...
namespace System
{
    ...
    {
        internal static bool EqualsOrdinalAsciiIgnoreCase(
            string left,
            char[] right,
            int rightStartIndex,
            int rightLength
        ...
        internal static void DebugAssertArrayInputs(char[] array, int startIndex, int length)
        {
            ...
            Debug.Assert(
                startIndex >= 0,
                $"Expected {nameof(startIndex)} to be >= 0, got {startIndex}"
            );
            Debug.Assert(length >= 0, $"Expected {nameof(length)} to be >= 0, got {length}");
            Debug.Assert(
                startIndex <= array.Length - length,
                $"Expected {startIndex} to be <= {array.Length} - {length}, got {startIndex}"
            );
        }
    }
}
