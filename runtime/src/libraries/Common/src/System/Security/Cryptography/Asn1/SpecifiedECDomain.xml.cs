...
namespace System.Security.Cryptography.Asn1
{
    ...
    internal partial struct SpecifiedECDomain
    {
        ...
        )
        {
            ...
            System.Security.Cryptography.Asn1.FieldID
                .Decode(ref sequenceReader, rebind, out decoded.FieldID);
            System.Security.Cryptography.Asn1.CurveAsn
                .Decode(ref sequenceReader, rebind, out decoded.Curve);

            if (sequenceReader.TryReadPrimitiveOctetString(out tmpSpan))
            {
                decoded.Base = rebindSpan.Overlaps(tmpSpan, out offset)
            ...
            decoded.Order = rebindSpan.Overlaps(tmpSpan, out offset)
                ? rebind.Slice(offset, tmpSpan.Length)
                : tmpSpan.ToArray();
            ...
            {
                tmpSpan = sequenceReader.ReadIntegerBytes();
                decoded.Cofactor = rebindSpan.Overlaps(tmpSpan, out offset)
                    ? rebind.Slice(offset, tmpSpan.Length)
                    : tmpSpan.ToArray();
            }
