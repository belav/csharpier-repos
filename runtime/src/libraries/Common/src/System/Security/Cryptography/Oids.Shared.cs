...
namespace System.Security.Cryptography
{
    internal static partial class Oids
    {
        ...
        internal static Oid secp256r1Oid =>
            s_secp256R1Oid ??= new Oid(secp256r1, nameof(ECCurve.NamedCurves.nistP256));
        internal static Oid secp384r1Oid =>
            s_secp384R1Oid ??= new Oid(secp384r1, nameof(ECCurve.NamedCurves.nistP384));
        internal static Oid secp521r1Oid =>
            s_secp521R1Oid ??= new Oid(secp521r1, nameof(ECCurve.NamedCurves.nistP521));
        ...
        internal static Oid DocumentDescriptionOid =>
            s_documentDescriptionOid ??= InitializeOid(DocumentDescription);
        ...
        internal static Oid Pkcs9ExtensionRequestOid =>
            s_pkcs9ExtensionRequestOid ??= InitializeOid(Pkcs9ExtensionRequest);

        internal static Oid BasicConstraints2Oid =>
            s_basicConstraints2Oid ??= InitializeOid(BasicConstraints2);
        internal static Oid EnhancedKeyUsageOid =>
            s_enhancedKeyUsageOid ??= InitializeOid(EnhancedKeyUsage);
        internal static Oid KeyUsageOid => s_keyUsageOid ??= InitializeOid(KeyUsage);
        internal static Oid AuthorityKeyIdentifierOid =>
            s_authorityKeyIdentifierOid ??= InitializeOid(AuthorityKeyIdentifier);
        internal static Oid SubjectKeyIdentifierOid =>
            s_subjectKeyIdentifierOid ??= InitializeOid(SubjectKeyIdentifier);
        internal static Oid SubjectAltNameOid =>
            s_subjectAltNameOid ??= InitializeOid(SubjectAltName);
        internal static Oid AuthorityInformationAccessOid =>
            s_authorityInformationAccessOid ??= InitializeOid(AuthorityInformationAccess);
        internal static Oid CrlNumberOid => s_crlNumberOid ??= InitializeOid(CrlNumber);
        internal static Oid CrlDistributionPointsOid =>
            s_crlDistributionPointOid ??= InitializeOid(CrlDistributionPoints);

        internal static Oid CommonNameOid => s_commonNameOid ??= InitializeOid(CommonName);
        internal static Oid CountryOrRegionNameOid =>
            s_countryOrRegionOid ??= InitializeOid(CountryOrRegionName);
        internal static Oid LocalityNameOid => s_localityNameOid ??= InitializeOid(LocalityName);
        internal static Oid StateOrProvinceNameOid = s_stateOrProvinceNameOid ??= InitializeOid(
            StateOrProvinceName
        );
        internal static Oid OrganizationOid = s_organizationOid ??= InitializeOid(Organization);
        internal static Oid OrganizationalUnitOid = s_organizationalUnitOid ??= InitializeOid(
            OrganizationalUnit
        );
        ...
        internal static Oid? GetSharedOrNullOid(
            ref AsnValueReader asnValueReader,
