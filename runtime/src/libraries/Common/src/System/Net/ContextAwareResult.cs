...
namespace System.Net
{
    ...
    internal sealed class CallbackClosure
    {
        ...
        {
            get { return _savedCallback; }
        }

        internal ExecutionContext? Context
        {
            get { return _savedContext; }
        }
    }
    ...
    internal sealed partial class ContextAwareResult : LazyAsyncResult
    {
        ...
        internal ExecutionContext? ContextCopy
        {
            get
            {
                ...
                {
                    Debug.Assert(
                        (_flags & StateFlags.ThreadSafeContextCopy) != 0,
                        "Called on completed result."
                    );
                ...
                Debug.Assert(
                    ...
                    "No context captured - specify a callback or forceCaptureContext."
                );
                ...
                if ((_flags & StateFlags.PostBlockFinished) == 0)
                {
                    Debug.Assert(
                        _lock != null,
                        "Must lock (StartPostingAsyncOp()) { ... FinishPostingAsyncOp(); } when calling ContextCopy (unless it's only called after FinishPostingAsyncOp)."
                    );
                    lock (_lock) { }
                }
                ...
                if (InternalPeekCompleted)
                {
                    Debug.Assert(
                        (_flags & StateFlags.ThreadSafeContextCopy) != 0,
                        "Result became completed during call."
                    );
                    throw new InvalidOperationException(SR.net_completed_result);
                }
        ...
        private bool CaptureOrComplete(ref ExecutionContext? cachedContext, bool returnContext)
        {
            Debug.Assert(
                ...
                "Called without calling StartPostingAsyncOp."
            );
        ...
        protected override void Complete(IntPtr userToken)
        {
            if (NetEventSource.Log.IsEnabled())
                NetEventSource.Info(
                    this,
                    $"_context(set):{_context != null} userToken:{userToken}"
                );
