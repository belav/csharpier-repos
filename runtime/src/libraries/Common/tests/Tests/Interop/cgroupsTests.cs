...
namespace Common.Tests
{
    public class cgroupsTests : FileCleanupTestBase
    {
        ...
        [InlineData(true, "1K", 1024)]
        public void ValidateTryReadMemoryValue(
            bool expectedResult,
            string valueText,
            ulong expectedValue
        )
        {
            ...
            Assert.Equal(
                expectedResult,
                Interop.cgroups.TryReadMemoryValueFromFile(path, out ulong val)
            );
            if (expectedResult)
            {
        ...
        [Theory]
        [InlineData(
            "/sys/fs/cgroup/cpu/my_cgroup",
            "/docker/1234",
            "/sys/fs/cgroup/cpu",
            "/docker/1234/my_cgroup"
        )]
        [InlineData("/sys/fs/cgroup/cpu/my_cgroup", "/", "/sys/fs/cgroup/cpu", "/my_cgroup")]
        public void ValidateFindCGroupPath(
        ...
        [InlineData(
            true,
            2,
            "0 0 0:0 / /foo ignore ignore - cgroup2 cgroup2 ignore",
            "ignore",
            "/",
            "/foo"
        )]
        ...
        [InlineData(
            true,
            2,
            "0 0 0:0 / /foo ignore ignore ignore - cgroup2 cgroup2 ignore",
            "cpu",
            "/",
            "/foo"
        )]
        [InlineData(
            true,
            2,
            "0 0 0:0 / /foo-with-dashes ignore ignore - cgroup2 cgroup2 ignore",
            "ignore",
            "/",
            "/foo-with-dashes"
        )]
        [InlineData(
            true,
            1,
            "0 0 0:0 / /foo ignore ignore - cgroup cgroup memory",
            "memory",
            "/",
            "/foo"
        )]
        [InlineData(
            true,
            1,
            "0 0 0:0 / /foo-with-dashes ignore ignore - cgroup cgroup memory",
            "memory",
            "/",
            "/foo-with-dashes"
        )]
        [InlineData(
            true,
            1,
            "0 0 0:0 / /foo ignore ignore - cgroup cgroup cpu,memory",
            "memory",
            "/",
            "/foo"
        )]
        [InlineData(
            true,
            1,
            "0 0 0:0 / /foo ignore ignore - cgroup cgroup memory,cpu",
            "memory",
            "/",
            "/foo"
        )]
        public void ParseValidateMountInfo(
            bool expectedFound,
            int cgroupVersion,
            string procSelfMountInfoText,
            string subsystem,
            string expectedRoot,
        ...
        public void ParseValidateProcCGroup(
            bool expectedFound,
            int cgroupVersion,
