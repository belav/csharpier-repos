...
namespace System.Security.Cryptography.EcDsa.Tests
{
    ...
    public sealed class ECDsaTests_Array : ECDsaTests
    {
        ...
        public void VerifyData_InvalidArguments_Throws(ECDsa ecdsa)
        {
            ...
            AssertExtensions.Throws<ArgumentNullException>(
                "data",
                () => ecdsa.VerifyData(null, -1, -1, null, default(HashAlgorithmName))
            );
            ...
            AssertExtensions.Throws<ArgumentNullException>(
                "hashAlgorithm",
                () => ecdsa.VerifyData(new byte[0], 0, 0, new byte[0], default(HashAlgorithmName))
            );
            AssertExtensions.Throws<ArgumentException>(
                "hashAlgorithm",
                () => ecdsa.VerifyData(new byte[10], new byte[0], new HashAlgorithmName(""))
            );
            AssertExtensions.Throws<ArgumentException>(
                "hashAlgorithm",
    ...
    public sealed class ECDsaTests_Stream : ECDsaTests
    {
        ...
        public void SignData_InvalidArguments_Throws(ECDsa ecdsa)
        {
            ...
            AssertExtensions.Throws<ArgumentException>(
                "hashAlgorithm",
                () => ecdsa.SignData(new MemoryStream(), new HashAlgorithmName(""))
            );
            Assert.ThrowsAny<CryptographicException>(() =>
                ecdsa.SignData(
                    new MemoryStream(),
        ...
        public void VerifyData_InvalidArguments_Throws(ECDsa ecdsa)
        {
            ...
            AssertExtensions.Throws<ArgumentException>(
                "hashAlgorithm",
                () => ecdsa.VerifyData(new MemoryStream(), new byte[0], new HashAlgorithmName(""))
            );
            Assert.ThrowsAny<CryptographicException>(() =>
                ecdsa.VerifyData(
                    new MemoryStream(),
    ...
    public abstract partial class ECDsaTests : ECDsaTestsBase
    {
        ...
        protected virtual bool VerifyHash(
            ECDsa ecdsa,
            byte[] hash,
            int offset,
            int count,
        ...
        public static IEnumerable<object[]> RealImplementations() =>
            new[] { new ECDsa[] { ECDsaFactory.Create() } };
        ...
        public void SignData_MaxOffset_ZeroLength_NoThrow(ECDsa ecdsa)
        {
            ...
            Assert.True(
                VerifyData(ecdsa, Array.Empty<byte>(), signature, HashAlgorithmName.SHA256)
            );
        }
        ...
        public void VerifyData_MaxOffset_ZeroLength_NoThrow(ECDsa ecdsa)
        {
            ...
            Assert.True(
                VerifyData(ecdsa, data, data.Length, 0, signature, HashAlgorithmName.SHA256)
            );
        }
        ...
        {
            foreach (
                HashAlgorithmName hashAlgorithm in new[]
                {
        ...
        )
        {
            ...
            int distinctSignatures = signatures
                .Distinct(
                    EqualityComparer<byte[]>.Create(
            ...
            foreach (byte[] signature in signatures)
            {
                ...
                Assert.False(
                    VerifyData(ecdsa, dataArray, signature, hashAlgorithm),
                    "Verify Tampered 1"
                );
                Assert.False(
                    ecdsa.VerifyHash(halg.ComputeHash(dataArray), signature),
                    "Verify Tampered 4"
                );
            }
        }
