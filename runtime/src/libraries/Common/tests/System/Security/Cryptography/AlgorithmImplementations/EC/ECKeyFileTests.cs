...
namespace System.Security.Cryptography.Tests
{
        ...
        where T : ECAlgorithm
    {
        ...
        public static bool CanDeriveNewPublicKey { get; } =
            EcDiffieHellman.Tests.ECDiffieHellmanFactory.CanDeriveNewPublicKey;

        public static bool SupportsBrainpool { get; } =
            IsCurveSupported(ECCurve.NamedCurves.brainpoolP160r1.Oid);
        public static bool SupportsSect163k1 { get; } =
            IsCurveSupported(EccTestData.Sect163k1Key1.Curve.Oid);
        public static bool SupportsSect283k1 { get; } =
            IsCurveSupported(EccTestData.Sect283k1Key1.Curve.Oid);
        ...
        public static bool SupportsC2pnb163v1Explicit { get; } =
            SupportsC2pnb163v1 || (!ExplicitNamedSameSupport && SupportsExplicitCurves);
...
72fdMHUrkk8pdFnjwuEq6Nj0/kRU44R8GLJVys71lze5GOHeQPlen/5PV7eRU6A=";
            ...
            ReadWriteBase64Pkcs8(base64, EccTestData.GetNistP521Key2(), isSupported: false);
        }
...
NfZ9nLTVjxeD08pD548KWrqmJAeZNsDDqQ==";
            ...
            ReadWriteBase64ECPrivateKey(base64, EccTestData.GetNistP256ReferenceKey());
        }
...
u2L/ySSNizmK/j4LXDwCs/43",
        ...
        [Fact]
        [ActiveIssue(
            "https://github.com/dotnet/runtime/issues/64446",
            typeof(PlatformSupport),
            nameof(PlatformSupport.IsAndroidVersionAtLeast31)
        )]
...
1ZJIzIE7RQ==",
        ...
        public void NoPrivKeyFromPublicOnly()
        {
            using (T key = CreateKey())
            {
                ...
                Assert.ThrowsAny<CryptographicException>(() => key.ExportECPrivateKey());
...
xoMaz20Yx+2TSN5dSm2FcD+0YFI=",
        ...
        )
        {
            ...
            else
            {
                ...
                using (T key = CreateKey())
                {
                    ...
                    Assert.ThrowsAny<CryptographicException>(() =>
                        key.ImportEncryptedPkcs8PrivateKey(
                            (ReadOnlySpan<char>)"ThisBetterNotBeThePassword!",
                            encrypted,
                            out _
                        )
                    );
        ...
        private void ReadWriteBase64ECPrivateKey(
            string base64Pkcs8,
            in ECParameters expected,
            bool isSupported = true
        )
        ...
        private void ReadWriteBase64Pkcs8(
            string base64Pkcs8,
            in ECParameters expected,
            bool isSupported = true
        )
                ...
                using (T key = CreateKey())
                {
                    Exception e = Assert.ThrowsAny<Exception>(() =>
                        key.ImportSubjectPublicKeyInfo(
                            Convert.FromBase64String(base64SubjectPublicKeyInfo),
                            out _
                        )
                    );
        ...
        )
        {
            ...
            using (T key = CreateKey())
            {
                ...
                if (writePublicArrayFunc is not null)
                {
                    ...
                    Assert.True(
                        writePublicSpanFunc(key, publicArrayExport, out int publicExportWritten)
                    );
        ...
        protected delegate bool WriteKeyToSpanFunc(
            T key,
