    ...
    public partial class AesCipherTests
    {
        ...
        public static void VerifyInPlaceEncryption()
        {
            ...
            using (Aes aes = AesFactory.Create())
            {
                ...
                {
                    bytesWritten = encryptor.TransformBlock(
                        buffer,
                        1,
            ...
            using (MemoryStream cipherStream = new MemoryStream(cipher))
            {
                ...
                using (ICryptoTransform decryptor = a.CreateDecryptor())
                {
                    while (true)
                    {
                        ...
                        int numBytesWritten = decryptor.TransformBlock(
                            buffer,
                            0,
                            blockSizeBytes,
                            buffer,
        ...
        public static void StableEncryptDecrypt()
        {
            ...
            using (Aes aes = AesFactory.Create())
            {
                ...
                using (ICryptoTransform encryptor = aes.CreateEncryptor())
                {
                    encrypted2 = encryptor.TransformFinalBlock(
                        s_helloBytes,
                        0,
                        s_helloBytes.Length
                    );
        ...
        )
        {
            ...
            using (Aes aes = AesFactory.Create())
            {
                ...
                {
                    liveOneShotDecryptBytes = aes.DecryptCfb(
                        cipherBytes,
                        iv,
                        paddingMode,
                        feedbackSizeInBits: feedbackSize.Value
                    );
                    liveOneShotEncryptBytes = aes.EncryptCfb(
                        plainBytes,
                        iv,
                        paddingMode,
                        feedbackSizeInBits: feedbackSize.Value
        ...
        private static byte[] AesDecryptDirectKey(
            Aes aes,
            byte[] key,
            byte[] iv,
            byte[] cipherBytes
