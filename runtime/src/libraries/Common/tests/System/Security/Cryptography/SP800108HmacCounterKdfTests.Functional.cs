...
namespace System.Security.Cryptography.Tests
{
    public static partial class SP800108HmacCounterKdfTests
    {
        ...
        public static void Rfc8009Tests(
            byte[] kdk,
            byte[] expected,
        ...
        [Theory]
        [InlineData(
            new byte[] { 0xcf, 0x4b, 0xfe, 0x4f, 0x85, 0xa1, 0x0b, 0xad },
            nameof(HashAlgorithmName.SHA1)
        )]
        [InlineData(
            new byte[] { 0x00, 0x26, 0x4b, 0xbb, 0x14, 0x97, 0x40, 0x54 },
            nameof(HashAlgorithmName.SHA256)
        )]
        [InlineData(
            new byte[] { 0xc7, 0x10, 0x27, 0x87, 0xd8, 0x96, 0xbc, 0x89 },
            nameof(HashAlgorithmName.SHA384)
        )]
        [InlineData(
            new byte[] { 0xdb, 0x3a, 0x18, 0xd9, 0x6c, 0x4a, 0xd4, 0x1e },
        ...
        [InlineData(
            ...
            "label!",
            "context?",
            new byte[] { 0xb6, 0xff, 0x26, 0x61, 0xea, 0x43, 0x76, 0xd2 }
        )]
        [InlineData(
            new byte[] { 0xFE },
            "",
            "context?",
            new byte[] { 0xed, 0xdf, 0x50, 0x06, 0x3c, 0x26, 0x3e, 0xd9 }
        )]
        [InlineData(
            new byte[] { 0xFE },
            "label!",
            "",
            new byte[] { 0x98, 0x83, 0x67, 0x41, 0x3f, 0x2d, 0x90, 0x72 }
        )]
        [InlineData(
            new byte[] { },
            "",
            "",
            new byte[] { 0x18, 0x0f, 0xf7, 0xa2, 0xbc, 0x8d, 0x6e, 0x98 }
        )]
        ...
        [InlineData(
            nameof(HashAlgorithmName.SHA1),
            512 / 8 - 1,
            new byte[] { 0xc9, 0x0f, 0x9d, 0x91, 0x85, 0xe5, 0xeb, 0x9b }
        )]
        [InlineData(
            nameof(HashAlgorithmName.SHA1),
            512 / 8,
            new byte[] { 0x7b, 0xdb, 0x38, 0x28, 0xc0, 0x9f, 0x49, 0x05 }
        )]
        [InlineData(
            nameof(HashAlgorithmName.SHA1),
            512 / 8 + 1,
            new byte[] { 0x6c, 0x3a, 0xba, 0x28, 0x38, 0xad, 0x51, 0x2c }
        )]
        [InlineData(
            nameof(HashAlgorithmName.SHA256),
            512 / 8 - 1,
            new byte[] { 0x88, 0xaa, 0xc7, 0xee, 0x05, 0x65, 0xfd, 0xda }
        )]
        [InlineData(
            nameof(HashAlgorithmName.SHA256),
            512 / 8,
            new byte[] { 0x3d, 0xdc, 0x7d, 0xec, 0x0a, 0xfd, 0x7a, 0xc0 }
        )]
        [InlineData(
            nameof(HashAlgorithmName.SHA256),
            512 / 8 + 1,
            new byte[] { 0x47, 0x95, 0x00, 0xd5, 0x55, 0x1f, 0xb3, 0x85 }
        )]
        [InlineData(
            nameof(HashAlgorithmName.SHA384),
            1024 / 8 - 1,
            new byte[] { 0x84, 0xd8, 0xfd, 0x33, 0x4f, 0x07, 0x81, 0x9b }
        )]
        [InlineData(
            nameof(HashAlgorithmName.SHA384),
            1024 / 8,
            new byte[] { 0x6c, 0xa2, 0x5d, 0x4f, 0x61, 0x2d, 0x0f, 0x20 }
        )]
        [InlineData(
            nameof(HashAlgorithmName.SHA384),
            1024 / 8 + 1,
            new byte[] { 0xe4, 0x0e, 0xbd, 0x41, 0x14, 0xe6, 0x80, 0x59 }
        )]
        [InlineData(
            nameof(HashAlgorithmName.SHA512),
            1024 / 8 - 1,
            new byte[] { 0xa4, 0xe5, 0x24, 0xe8, 0x56, 0x2b, 0x48, 0xa4 }
        )]
        [InlineData(
            nameof(HashAlgorithmName.SHA512),
            1024 / 8,
            new byte[] { 0xba, 0xf6, 0xed, 0xa7, 0x3a, 0xf7, 0x12, 0x27 }
        )]
        [InlineData(
            nameof(HashAlgorithmName.SHA512),
            1024 / 8 + 1,
            new byte[] { 0x34, 0xdf, 0x2d, 0x21, 0xfd, 0xf1, 0x0e, 0x13 }
        )]
#if NET8_0_OR_GREATER
        [InlineData(
            "SHA3-256",
            1088 / 8 - 1,
            new byte[] { 0xa1, 0x96, 0xae, 0x83, 0x56, 0xf4, 0x2a, 0x4b }
        )]
        [InlineData(
            "SHA3-256",
            1088 / 8,
            new byte[] { 0xe7, 0xe9, 0xe0, 0x98, 0x09, 0x54, 0x54, 0x2d }
        )]
        [InlineData(
            "SHA3-256",
            1088 / 8 + 1,
            new byte[] { 0x7d, 0x7a, 0x71, 0xdf, 0x1f, 0x5d, 0x5b, 0x44 }
        )]
        [InlineData(
            "SHA3-384",
            832 / 8 - 1,
            new byte[] { 0xd6, 0x08, 0x69, 0xd0, 0x99, 0x98, 0x6d, 0xcc }
        )]
        [InlineData(
            "SHA3-384",
            832 / 8,
            new byte[] { 0x49, 0x83, 0x06, 0x4e, 0x08, 0xf8, 0x93, 0x62 }
        )]
        [InlineData(
            "SHA3-384",
            832 / 8 + 1,
            new byte[] { 0xcc, 0x03, 0x1f, 0x57, 0x5e, 0x0c, 0xe1, 0xe8 }
        )]
        [InlineData(
            "SHA3-512",
            576 / 8 - 1,
            new byte[] { 0x47, 0xd2, 0x7e, 0x61, 0x01, 0x61, 0x9a, 0xd0 }
        )]
        [InlineData(
            "SHA3-512",
            576 / 8,
            new byte[] { 0xda, 0x56, 0x5b, 0x08, 0x73, 0xbc, 0x4d, 0x33 }
        )]
        [InlineData(
            "SHA3-512",
            576 / 8 + 1,
            new byte[] { 0xd3, 0xa1, 0xfd, 0x76, 0xc4, 0xf9, 0x62, 0xc3 }
        )]
#endif
...
#if NET8_0_OR_GREATER
            if (
                (hashAlgorithmName == "SHA3-256" && !SHA3_256.IsSupported)
                || (hashAlgorithmName == "SHA3-384" && !SHA3_384.IsSupported)
                || (hashAlgorithmName == "SHA3-512" && !SHA3_512.IsSupported)
            )
            {
                throw new SkipTestException(
                    $"Algorithm '{hashAlgorithmName}' is not supported on the current platform."
                );
            }
#endif
        ...
        public static void OutputLength_AroundPrfOutputBoundaries(
            string hashAlgorithmName,
            byte[] expected
        )
        {
        ...
        {
            SP800108HmacCounterKdf kdf = new SP800108HmacCounterKdf(
                s_kdk,
                HashAlgorithmName.SHA256
            );
            kdf.Dispose();
