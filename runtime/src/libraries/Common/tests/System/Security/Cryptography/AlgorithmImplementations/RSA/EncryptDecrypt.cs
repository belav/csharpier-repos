...
namespace System.Security.Cryptography.Rsa.Tests
{
    ...
    public abstract class EncryptDecrypt
    {
        ...
        [Fact]
        public void RsaCryptRoundtrip_OaepSHA1() =>
            RsaCryptRoundtrip(RSAEncryptionPadding.OaepSHA1);
        ...
        public void RoundtripEmptyArray()
        {
            if (OperatingSystem.IsIOS() && !OperatingSystem.IsIOSVersionAtLeast(13, 6))
            {
                throw new SkipTestException(
                    "iOS prior to 13.6 does not reliably support RSA encryption of empty data."
                );
            }
            if (OperatingSystem.IsTvOS() && !OperatingSystem.IsTvOSVersionAtLeast(14, 0))
            {
                throw new SkipTestException(
                    "tvOS prior to 14.0 does not reliably support RSA encryption of empty data."
                );
            }
        ...
        public void RsaPkcsEncryptMaxSize()
        {
            ...
            using (RSA rsa = RSAFactory.Create(rsaParameters))
            {
                ...
                Assert.ThrowsAny<CryptographicException>(() => Encrypt(rsa, data, paddingMode1));
            }
        }
        ...
        public void RsaOaepMaxSize()
        {
            ...
            using (RSA rsa = RSAFactory.Create(rsaParameters))
            {
                void Test(RSAEncryptionPadding paddingMode, int hashSizeInBits)
                {
                    ...
                    Assert.ThrowsAny<CryptographicException>(() => Encrypt(rsa, data, paddingMode));
                }
