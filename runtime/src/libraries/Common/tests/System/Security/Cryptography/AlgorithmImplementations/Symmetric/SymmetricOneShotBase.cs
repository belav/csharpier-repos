    ...
    public abstract class SymmetricOneShotBase
    {
        ...
        )
        {
            ...
            using (SymmetricAlgorithm alg = CreateAlgorithm())
            {
                ...
                {
                    CipherMode.ECB => alg.TryDecryptEcb(
                        ciphertext,
                        destinationBuffer,
                        padding,
                        out bytesWritten
                    ),
                    CipherMode.CBC => alg.TryDecryptCbc(
                        ciphertext,
                        IV,
                        destinationBuffer,
                        out bytesWritten,
                    ...
                    CipherMode.CFB => alg.TryDecryptCfb(
                        ciphertext,
                        IV,
                        destinationBuffer,
                        out bytesWritten,
        ...
        )
        {
            ...
            using (SymmetricAlgorithm alg = CreateAlgorithm())
            {
                ...
                bool result = mode switch
                {
                    CipherMode.ECB => alg.TryEncryptEcb(
                        plaintext,
                        destinationBuffer,
                        padding,
                        out bytesWritten
                    ),
                    CipherMode.CBC => alg.TryEncryptCbc(
                        plaintext,
                        IV,
                        destinationBuffer,
                        out bytesWritten,
                    ...
                    CipherMode.CFB => alg.TryEncryptCfb(
                        plaintext,
                        IV,
                        destinationBuffer,
                        out bytesWritten,
        ...
        )
        {
            using (SymmetricAlgorithm alg = CreateAlgorithm())
            {
                ...
                int expectedCiphertextSize = mode switch
                {
                    ...
                    CipherMode.CFB => alg.GetCiphertextLengthCfb(
                        plaintext.Length,
                        padding,
                        feedbackSize
                    ),
        ...
        public void DecryptOneShot_Cfb8_ToleratesExtraPadding()
        {
            using (SymmetricAlgorithm alg = CreateAlgorithm())
            {
                ...
                byte[] ciphertext = alg.EncryptCfb(
                    excessPadding,
                    IV,
        ...
        public void DecryptOneShot_Cbc_InvalidPadding_DoesNotContainPlaintext(
        ...
        [InlineData(PaddingMode.ANSIX923, 2048)]
        [InlineData(PaddingMode.ISO10126, 2048)]
        public void DecryptOneShot_Ecb_InvalidPadding_DoesNotContainPlaintext(
            PaddingMode paddingMode,
            int ciphertextSize
        )
        {
        ...
        [InlineData(PaddingMode.ANSIX923, 2048)]
        [InlineData(PaddingMode.ISO10126, 2048)]
        public void DecryptOneShot_Cfb_InvalidPadding_DoesNotContainPlaintext(
            PaddingMode paddingMode,
            int ciphertextSize
        )
        {
        ...
        public void DecryptOneShot_Cbc_TooShortDoesNotContainPlaintext(PaddingMode paddingMode)
        {
            using (SymmetricAlgorithm alg = CreateAlgorithm())
            {
                ...
                bool success = alg.TryDecryptCbc(
                    ciphertext,
                    IV,
                    destination,
                ...
                Assert.True(
                    destination.IndexOf(plaintextByte) < 0,
                    "does not contain plaintext data"
                );
                Assert.Equal(0, bytesWritten);
            }
        ...
        public void DecryptOneShot_Cfb8_TooShortDoesNotContainPlaintext(PaddingMode paddingMode)
        {
            using (SymmetricAlgorithm alg = CreateAlgorithm())
            {
                ...
                bool success = alg.TryDecryptCfb(
                    ciphertext,
                    IV,
                    destination,
                    out int bytesWritten,
