        ...
        public void NullArray_Throws()
        {
            using (RSA rsa = RSAFactory.Create())
            {
                AssertExtensions.Throws<ArgumentNullException>(
                    "data",
                    () => SignData(rsa, null, HashAlgorithmName.SHA1, RSASignaturePadding.Pkcs1)
                );
                AssertExtensions.Throws<ArgumentNullException>(
                    "hash",
                    () => SignHash(rsa, null, HashAlgorithmName.SHA1, RSASignaturePadding.Pkcs1)
                );
    ...
    public abstract class SignVerify
    {
        ...
        public void NullPadding_Throws()
        {
            using (RSA rsa = RSAFactory.Create())
            {
                AssertExtensions.Throws<ArgumentNullException>(
                    "padding",
                    () => SignData(rsa, new byte[1], HashAlgorithmName.SHA1, null)
                );
                AssertExtensions.Throws<ArgumentNullException>(
                    "padding",
                    () => SignHash(rsa, new byte[1], HashAlgorithmName.SHA1, null)
                );
                AssertExtensions.Throws<ArgumentNullException>(
                    "padding",
                    () => VerifyData(rsa, new byte[1], new byte[1], HashAlgorithmName.SHA1, null)
                );
                AssertExtensions.Throws<ArgumentNullException>(
                    "padding",
                    () => VerifyHash(rsa, new byte[1], new byte[1], HashAlgorithmName.SHA1, null)
                );
            }
        }
        ...
        public void UseAfterDispose(bool importKey)
        {
            ...
            Assert.Throws<ObjectDisposedException>(() => VerifyData(rsa, sig, data, alg, padding));
        ...
        public void NegativeVerify_WrongAlgorithm()
        {
            using (RSA rsa = RSAFactory.Create())
            {
                ...
                bool signatureMatched = VerifyData(
                    rsa,
                    TestData.HelloBytes,
                    signature,
                    HashAlgorithmName.SHA256,
        ...
        public void PssVerifyHash_MismatchedHashSize()
        {
            ...
            {
                Assert.False(
                    VerifyHash(rsa, hash, sig, HashAlgorithmName.SHA256, RSASignaturePadding.Pss)
                );
            }
        }
