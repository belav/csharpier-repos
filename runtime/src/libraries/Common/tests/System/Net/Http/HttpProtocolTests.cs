...
namespace System.Net.Http.Functional.Tests
{
    public abstract class HttpProtocolTests : HttpClientHandlerTestBase
    {
        ...
        [InlineData(
            "HTTP/1.1 299 This is not a real status code",
            299,
            "This is not a real status code"
        )]
        ...
        public async Task GetAsync_ExpectedStatusCodeAndReason_Success(
            string statusLine,
        ...
        public async Task GetAsync_ExpectedStatusCodeAndReason_PlatformBehaviorTest(
            string statusLine,
            int expectedStatusCode,
            string reason
        )
        ...
        [InlineData("HTTP/1.1 200 O\tK", 200, "O\tK")]
        [InlineData("HTTP/1.1 200 O    \t\t  \t\t\t\t  \t K", 200, "O    \t\t  \t\t\t\t  \t K")]
        public async Task GetAsync_StatusLineNotFollowRFC_SuccessOnCore(
            string statusLine,
            int expectedStatusCode,
            string expectedReason
        )
        ...
        public async Task GetAsync_ResponseHasNormalLineEndings_Success(string lineEnding)
        {
            ...
            await LoopbackServer.CreateClientAndServerAsync(
                ...
                {
                    await server
                        .AcceptConnectionSendCustomResponseAndCloseAsync(
                            "HTTP/1.1 200 OK\nConnection: close\nDate: {DateTimeOffset.UtcNow:R}\nServer: TestServer\nContent-Length: 0\n\n".Replace(
                                "\n",
                                lineEnding
                            )
        ...
        )
        {
            ...
            await LoopbackServer.CreateClientAndServerAsync(
                ...
                {
                    using (
                        HttpMessageInvoker client = new HttpMessageInvoker(
...
#else
                            while (
                                (bytesRead = await respStream.ReadAsync(buffer, 0, buffer.Length))
                                > 0
                            )
#endif
                            {
    ...
    {
        public HttpProtocolTests_Dribble(ITestOutputHelper output)
            : base(output) { }
