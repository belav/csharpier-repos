...
namespace System.Net.Test.Common
{
    public sealed partial class LoopbackServer : GenericLoopbackServer, IDisposable
    {
        ...
        public LoopbackServer(Options options = null)
        {
            ...
            {
                _options.CertificateContext = SslStreamCertificateContext.Create(
                    _options.Certificate ?? Configuration.Certificates.GetServerCertificate(),
                    null
                );
            }
#endif
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        {
            try
            {
                ...
                await _listenSocket.ConnectAsync(
                    Configuration.Http.RemoteLoopServer,
        ...
        public async Task<Connection> EstablishConnectionAsync()
        {
            ...
            try
            {
                ...
                return await Connection
                    .CreateAsync(closableWrapper, stream, _options)
                    .ConfigureAwait(false);
            }
            catch (Exception)
            {
        ...
        {
            await using (
                Connection connection = await EstablishConnectionAsync().ConfigureAwait(false)
            )
            {
        ...
        public sealed class Connection : GenericLoopbackConnection
        {
                ...
                if (httpOptions.UseSsl)
                {
                    ...
                    SslServerAuthenticationOptions sslOptions = new SslServerAuthenticationOptions()
                    {
                        ...
                        ServerCertificateContext =
                            httpOptions.CertificateContext
                    ...
                    await sslStream
                        .AuthenticateAsServerAsync(sslOptions, default)
                        .ConfigureAwait(false);
#else
                    using (
            ...
            public async Task<int> ReadAsync(Memory<byte> buffer, int offset, int size)
            {
                ...
                int readBytes = await _stream
                    .ReadAsync(tmpBuffer, offset, size)
                    .ConfigureAwait(false);
                ...
                while (true)
                {
                    if (_readStart == _readEnd || index == -1)
                    {
                        ...
                        int bytesRead = await _stream
                            .ReadAsync(_readBuffer, _readEnd, _readBuffer.Length - _readEnd)
                            .ConfigureAwait(false);
                        if (bytesRead == 0)
                        {
                            break;
            ...
            public override async Task SendPartialResponseHeadersAsync(
                HttpStatusCode statusCode = HttpStatusCode.OK,
                IList<HttpHeaderData> headers = null
            )
            {
            ...
            public async Task<HttpRequestData> HandleCORSPreFlight(HttpRequestData requestData)
            {
                if (
                    ...
                    && requestData.Headers.Any(h =>
                        h.Name.StartsWith("Access-Control-Request-Method")
                    )
                )
                {
            ...
            public async Task<List<string>> HandleCORSPreFlight(List<string> lines)
            {
                if (
                    PlatformDetection.IsBrowser
                    && lines[0].Contains("OPTIONS")
                    && lines.Any(h => h.StartsWith("Access-Control-Request-Method"))
                )
                {
                    // handle CORS pre-flight
                    await SendResponseAsync(HttpStatusCode.OK).ConfigureAwait(false);
