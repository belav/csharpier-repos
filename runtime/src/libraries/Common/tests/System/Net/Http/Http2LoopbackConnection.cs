...
namespace System.Net.Test.Common
{
    public class Http2LoopbackConnection : GenericLoopbackConnection
    {
        ...
        private Http2LoopbackConnection(
            SocketWrapper socket,
            Stream stream,
            TimeSpan timeout,
            bool transparentPingResponse
        )
        {
        ...
        public static Task<Http2LoopbackConnection> CreateAsync(
            SocketWrapper socket,
            Stream stream,
            Http2Options httpOptions
        )
        {
            return CreateAsync(socket, stream, httpOptions, Http2LoopbackServer.Timeout);
        }

        public static async Task<Http2LoopbackConnection> CreateAsync(
            SocketWrapper socket,
            Stream stream,
            Http2Options httpOptions,
            TimeSpan timeout
        )
        {
            if (httpOptions.UseSsl)
            {
                ...
                using (
                    X509Certificate2 cert =
                        httpOptions.Certificate ?? Configuration.Certificates.GetServerCertificate()
                )
                {
#if !NETFRAMEWORK
            ...
            var con = new Http2LoopbackConnection(
                socket,
                stream,
                timeout,
                httpOptions.EnableTransparentPingResponse
            );
        ...
        private async Task ReadPrefixAsync()
        {
            ...
            {
                throw new Exception(
                    "Connection stream closed while attempting to read connection preface."
                );
            }
        ...
        )
        {
            ...
            await _connectionStream
                .WriteAsync(writeBuffer, 0, writeBuffer.Length, cancellationToken)
                .ConfigureAwait(false);
        }

        public async Task WriteFramesAsync(
            Frame[] frames,
            CancellationToken cancellationToken = default
        )
        {
            byte[] writeBuffer = new byte[
                frames.Sum(frame => Frame.FrameHeaderLength + frame.Length)
            ];
        ...
        {
            int readBytes = await _connectionStream
            ...
            {
                readBytes = await _connectionStream
                    .ReadAsync(buffer, cancellationToken)
                    .ConfigureAwait(false);
                if (readBytes == 0)
                {
        ...
        public async Task<Frame> ReadFrameAsync(CancellationToken cancellationToken)
        {
            ...
            if (
                header.Length > 0
                && !await FillBufferAsync(data, cancellationToken).ConfigureAwait(false)
            )
            {
                throw new Exception(
                    "Connection stream closed while attempting to read frame body."
                );
            }
        ...
        public async Task ReadRstStreamAsync(int streamId)
        {
            ...
            {
                throw new Exception(
                    $"Expected RST_STREAM on stream {streamId}, actual streamId={frame.StreamId}"
                );
            }
        }
        ...
        public async Task WaitForClientDisconnectAsync(bool ignoreUnexpectedFrames = false)
        {
            ...
            if (frame != null)
            {
                ...
                {
                    throw new Exception(
                        $"Unexpected frame received while waiting for client disconnect: {frame}"
                    );
                }
            }
        ...
        public async Task ShutdownIgnoringErrorsAsync(
            int lastStreamId,
        ...
        )
        {
            try
            {
                ...
                await WaitForConnectionShutdownAsync(ignoreUnexpectedFrames: true)
                    .ConfigureAwait(false);
            }
            catch (IOException)
            {
        ...
        private static (int bytesConsumed, int value) DecodeInteger(
        ...
        )
        {
            ...
            if ((headerBlock[0] & 0b10000000) != 0)
            {
                ...
                int bytesDecoded = HuffmanDecoder.Decode(
        ...
        private static (int bytesConsumed, HttpHeaderData headerData) DecodeLiteralHeader(
            ReadOnlySpan<byte> headerBlock,
            byte prefixMask
        )
        {
        ...
        private static (int bytesConsumed, HttpHeaderData headerData) DecodeHeader(
            ReadOnlySpan<byte> headerBlock
        )
        {
            int i = 0;
        ...
        public async IAsyncEnumerable<Frame> ReadRequestHeadersFrames()
        {
            ...
            Assert.Equal(
                FrameFlags.None,
                frame.Flags & ~(FrameFlags.EndStream | FrameFlags.EndHeaders)
            );
        ...
        )> ReadAndParseRequestHeaderAsync(bool readBody = true)
        {
            ...
            await foreach (Frame frame in ReadRequestHeadersFrames())
            {
                switch (frame.Type)
                {
                    case FrameType.Headers:
                        ...
                        endOfStream =
                            (headersFrame.Flags & FrameFlags.EndStream) == FrameFlags.EndStream;
            ...
            while (i < data.Length)
            {
                ...
                headerData = new HttpHeaderData(
                    headerData.Name,
                    headerData.Value,
                    headerData.HuffmanEncoded,
                    headerRaw
                );
        ...
        public async Task<SettingsFrame> ReadAndSendSettingsAsync(
            TimeSpan? ackTimeout,
        ...
        public async Task PingPong()
        {
            ...
            if (pingAck == null || pingAck.Type != FrameType.Ping || !pingAck.AckFlag)
            {
                string faultDetails =
                    pingAck == null
                        ? ""
                        : $" frame.Type:{pingAck.Type} frame.AckFlag: {pingAck.AckFlag}";
        ...
        public async Task SendPingAckAsync(
            long payload,
            CancellationToken cancellationToken = default
        )
        {
        ...
        )
        {
            ...
            if (!isTrailingHeader)
            {
                ...
                bytesGenerated += HPackEncoder.EncodeHeader(
                    ":status",
                    statusCodeString,
                    HPackFlags.None,
                    headerBlock.AsSpan()
            ...
            if (headers != null)
            {
                ...
                {
                    int estimatedHeaderLength =
                        headerData.Name.Length + headerData.Value.Length + 20;
                    Assert.True(
                        estimatedHeaderLength < headerBlock.Length,
                        "A single header exceeds MaxFrameLength"
                    );
                    ...
                    bytesGenerated += HPackEncoder.EncodeHeader(
                        headerData.Name,
                        headerData.Value,
                        headerData.ValueEncoding,
                        headerData.HuffmanEncoded ? HPackFlags.HuffmanEncode : HPackFlags.None,
                        headerBlock.AsSpan(bytesGenerated)
                    );
                }
            }
        ...
        public async Task SendResponseDataAsync(
            int streamId,
        ...
        public override async Task WaitForCancellationAsync(bool ignoreIncomingData = true)
        {
            ...
            do
            {
                ...
                {
                    Assert.True(
                        frame.Type == FrameType.Data || frame.Type == FrameType.RstStream,
                        $"Expected Data or RstStream, got {frame.Type}"
                    );
                }
                else
                {
