...
namespace System.Net.Test.Common
{
    public class Http2LoopbackConnection : GenericLoopbackConnection
    {
        ...
        public async Task WriteFrameAsync(Frame frame)
        {
            ...
            await _connectionStream.WriteAsync(writeBuffer, 0, writeBuffer.Length)
                .ConfigureAwait(false);
        }
        ...
        private async Task<bool> FillBufferAsync(
            ...
            int readBytes = await _connectionStream.ReadAsync(buffer, cancellationToken)
                .ConfigureAwait(false);
            if (readBytes == 0)
            {
                return false;
            ...
            {
                readBytes = await _connectionStream.ReadAsync(buffer, cancellationToken)
                    .ConfigureAwait(false);
                if (readBytes == 0)
                {
