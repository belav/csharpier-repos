...
namespace System.Net.Http.Functional.Tests
{
    ...
    public abstract class HttpClientHandlerTest : HttpClientHandlerTestBase
    {
        ...
        [SkipOnPlatform(
            ...
            "MaxResponseHeadersLength is not supported on Browser"
        )]
        ...
        [SkipOnPlatform(
            TestPlatforms.Browser,
            "ServerCertificateCustomValidationCallback not supported on Browser"
        )]
        public async Task GetAsync_IPv6LinkLocalAddressUri_Success()
        {
            ...
            {
                Address = Configuration.Sockets.LinkLocalAddress,
            };
            ...
            await LoopbackServerFactory.CreateClientAndServerAsync(
                async uri =>
                {
                    using (HttpClient client = CreateHttpClient())
                    {
                        ...
                        {
                            Content = new ByteArrayContent(contentArray),
                            Version = UseVersion,
                        };
                        ...
                        request.Headers.IfUnmodifiedSince = DateTimeOffset.Parse(
                            "Sat, 29 Oct 1994 19:43:31 GMT"
                        );
                        request.Headers.MaxForwards = 10;
                        request.Headers.Add("Origin", "http://www.example-social-network.com");
                        request.Headers.Pragma.Add(new NameValueHeaderValue("no-cache"));
                        request.Headers.ProxyAuthorization = new AuthenticationHeaderValue(
                            "Basic",
                            authSafeValue
                        );
                ...
                {
                    {
                        ...
                        if (PlatformDetection.IsNotBrowser)
                        {
                            Assert.Equal(
                                "utf-8",
                                requestData.GetSingleHeaderValue("Accept-Charset")
                            );
                            Assert.Equal(
                                "gzip, deflate",
                                requestData.GetSingleHeaderValue("Accept-Encoding")
                            );
                            Assert.Equal(
                                "en-US",
                                requestData.GetSingleHeaderValue("Accept-Language")
                            );
                            Assert.Equal(
                                "Thu, 31 May 2007 20:35:00 GMT",
                                requestData.GetSingleHeaderValue("Accept-Datetime")
                            );
                            Assert.Equal(
                                "GET",
                                requestData.GetSingleHeaderValue("Access-Control-Request-Method")
                            );
                            Assert.Equal(
                                "GET",
                                requestData.GetSingleHeaderValue("Access-Control-Request-Headers")
                            );
                        }
                        Assert.Equal("12", requestData.GetSingleHeaderValue("Age"));
                        Assert.Equal(
                            $"Basic {authSafeValue}",
                            requestData.GetSingleHeaderValue("Authorization")
                        );
                        ...
                        if (PlatformDetection.IsNotBrowser)
                        {
                            ...
                            Assert.Equal(
                                "Tue, 15 Nov 1994 08:12:31 GMT",
                                requestData.GetSingleHeaderValue("Date")
                            );
                            Assert.Equal(
                                "100-continue",
                            ...
                            Assert.Equal(
                                ...
                                requestData.GetSingleHeaderValue("Origin")
                            );
                            Assert.Equal(
                                $"Basic {authSafeValue}",
                                requestData.GetSingleHeaderValue("Proxy-Authorization")
                            );
                            Assert.Equal(
                                "Mozilla/5.0",
                                requestData.GetSingleHeaderValue("User-Agent")
                            );
                            Assert.Equal(
                                "http://en.wikipedia.org/wiki/Main_Page",
                                requestData.GetSingleHeaderValue("Referer")
                            );
                            if (PlatformDetection.IsNotNodeJS)
                            {
                                Assert.Equal(
                                    "MyTrailer",
                                    requestData.GetSingleHeaderValue("Trailer")
                                );
                            }
                            Assert.Equal(
                                "1.0 fred, 1.1 example.com (Apache/1.1)",
                                requestData.GetSingleHeaderValue("Via")
                            );
                            Assert.Equal(
                                "1 (Do Not Track Enabled)",
                                requestData.GetSingleHeaderValue("DNT")
                            );
                        }
                        Assert.Equal(
                            "for=192.0.2.60;proto=http;by=203.0.113.43",
                            requestData.GetSingleHeaderValue("Forwarded")
                        );
                        Assert.Equal(
                            "User Name <user@example.com>",
                            requestData.GetSingleHeaderValue("From")
                        );
                        Assert.Equal(
                            "\"37060cd8c284d8af7ad3082f209582d\"",
                            requestData.GetSingleHeaderValue("If-Match")
                        );
                        Assert.Equal(
                            "Sat, 29 Oct 1994 19:43:31 GMT",
                            requestData.GetSingleHeaderValue("If-Modified-Since")
                        );
                        Assert.Equal(
                            "\"737060cd8c284d8af7ad3082f209582d\"",
                            requestData.GetSingleHeaderValue("If-None-Match")
                        );
                        Assert.Equal(
                            "Wed, 21 Oct 2015 07:28:00 GMT",
                            requestData.GetSingleHeaderValue("If-Range")
                        );
                        Assert.Equal(
                            "Sat, 29 Oct 1994 19:43:31 GMT",
                            requestData.GetSingleHeaderValue("If-Unmodified-Since")
                        );
                        ...
                        Assert.Equal(
                            ...
                            requestData.GetSingleHeaderValue("Warning")
                        );
                        Assert.Equal(
                            "XMLHttpRequest",
                            requestData.GetSingleHeaderValue("X-Requested-With")
                        );
                        if (PlatformDetection.IsNotNodeJS)
                        {
                            Assert.Equal(
                                "client1, proxy1, proxy2",
                                requestData.GetSingleHeaderValue("X-Forwarded-For")
                            );
                        }
                        else
                        {
                        ...
                        Assert.Equal(
                            ...
                            requestData.GetSingleHeaderValue("X-Forwarded-Host")
                        );
                        Assert.Equal(
                            "https",
                            requestData.GetSingleHeaderValue("X-Forwarded-Proto")
                        );
                        Assert.Equal("https", requestData.GetSingleHeaderValue("Front-End-Https"));
                        Assert.Equal(
                            "DELETE",
                            requestData.GetSingleHeaderValue("X-Http-Method-Override")
                        );
                        Assert.Equal(
                            "GT-P7320/P7320XXLPG",
                            requestData.GetSingleHeaderValue("X-ATT-DeviceId")
                        );
                        Assert.Equal(
                            "http://wap.samsungmobile.com/uaprof/SGH-I777.xml",
                            requestData.GetSingleHeaderValue("X-Wap-Profile")
                        );
                        Assert.Equal("...", requestData.GetSingleHeaderValue("X-UIDH"));
                        Assert.Equal(
                            "i8XNjC4b8KVok4uw5RftR38Wgp2BFwql",
                            requestData.GetSingleHeaderValue("X-Csrf-Token")
                        );
                        if (PlatformDetection.IsNotNodeJS)
                        {
                            Assert.Equal(
                                "f058ebd6-02f7-4d3f-942e-904344e8cde5, f058ebd6-02f7-4d3f-942e-904344e8cde5",
                                requestData.GetSingleHeaderValue("X-Request-ID")
                            );
                        }
                        else
                        {
                        ...
                        Assert.Equal(
                            "X-Underscore_Name",
                            requestData.GetSingleHeaderValue("X-Underscore_Name")
                        );
                        ...
                        else if (PlatformDetection.IsNotBrowser)
                        {
                            ...
                            Assert.Equal(
                                "close",
                                requestData.GetSingleHeaderValue("Connection"),
                                StringComparer.OrdinalIgnoreCase
                            ); // NetFxHandler uses "Close" vs "close"
                            Assert.Equal(
                                "en.wikipedia.org:8080",
                                requestData.GetSingleHeaderValue("Host")
                            );
                            Assert.Equal(
                                "trailers, deflate",
                                requestData.GetSingleHeaderValue("TE")
                            );
                            Assert.Equal(
                                "HTTPS/1.3, IRC/6.9, RTA/x11, websocket",
                                requestData.GetSingleHeaderValue("Upgrade")
                            );
                            Assert.Equal(
                                "keep-alive",
                                requestData.GetSingleHeaderValue("Proxy-Connection")
                            );
                        }
                    }
                }
        ...
        [ActiveIssue(
            "https://github.com/dotnet/runtime/issues/82880",
            typeof(PlatformDetection),
            nameof(PlatformDetection.IsNodeJS)
        )]
                ...
                async uri =>
                {
                    ...
                    using (HttpResponseMessage resp = await client.GetAsync(uri))
                    {
                        ...
                        Assert.Contains(
                            ...
                            resp.Headers.GetValues("Accept-Patch")
                        );
                        Assert.Contains("bytes", resp.Headers.AcceptRanges);
                        Assert.Equal(
                            TimeSpan.FromSeconds(12),
                            resp.Headers.Age.GetValueOrDefault()
                        );
                        Assert.Contains(
                            "Bearer 63123a47139a49829bcd8d03005ca9d7",
                            resp.Headers.GetValues("Authorization")
                        );
                        ...
                        Assert.Contains(
                            ...
                            resp.Headers.GetValues("Alt-Svc")
                        );
                        Assert.Equal(
                            TimeSpan.FromSeconds(3600),
                            resp.Headers.CacheControl.MaxAge.GetValueOrDefault()
                        );
                        Assert.Contains("close", resp.Headers.Connection);
                        Assert.True(resp.Headers.ConnectionClose.GetValueOrDefault());
                        Assert.Equal(
                            "attachment",
                            resp.Content.Headers.ContentDisposition.DispositionType
                        );
                        Assert.Equal(
                            "\"fname.ext\"",
                        ...
                        Assert.Equal(
                            new Uri("/index.htm", UriKind.Relative),
                            resp.Content.Headers.ContentLocation
                        );
                        if (PlatformDetection.IsNotBrowser)
                        {
                            Assert.Equal(
                                Convert.FromBase64String("Q2hlY2sgSW50ZWdyaXR5IQ=="),
                                resp.Content.Headers.ContentMD5
                            );
                        }
                        Assert.Equal("bytes", resp.Content.Headers.ContentRange.Unit);
                        Assert.Equal(
                            21010,
                            resp.Content.Headers.ContentRange.From.GetValueOrDefault()
                        );
                        Assert.Equal(
                            47021,
                            resp.Content.Headers.ContentRange.To.GetValueOrDefault()
                        );
                        Assert.Equal(
                            47022,
                            resp.Content.Headers.ContentRange.Length.GetValueOrDefault()
                        );
                        ...
                        Assert.Equal(
                            DateTimeOffset.Parse("Tue, 15 Nov 1994 08:12:31 GMT"),
                            resp.Headers.Date.GetValueOrDefault()
                        );
                        Assert.Equal("\"737060cd8c284d8af7ad3082f209582d\"", resp.Headers.ETag.Tag);
                        Assert.Equal(
                            DateTimeOffset.Parse("Thu, 01 Dec 1994 16:00:00 GMT"),
                            resp.Content.Headers.Expires.GetValueOrDefault()
                        );
                        Assert.Equal(
                            DateTimeOffset.Parse("Tue, 15 Nov 1994 12:45:26 GMT"),
                            resp.Content.Headers.LastModified.GetValueOrDefault()
                        );
                        Assert.Contains(
                            "</feed>; rel=\"alternate\"",
                        ...
                        Assert.Equal(
                            new Uri("http://www.w3.org/pub/WWW/People.html"),
                            resp.Headers.Location
                        );
                        Assert.Contains(
                            "CP=\"This is not a P3P policy!\"",
                            resp.Headers.GetValues("P3P")
                        );
                        Assert.Contains(new NameValueHeaderValue("no-cache"), resp.Headers.Pragma);
                        Assert.Contains(
                            new AuthenticationHeaderValue("basic"),
                            resp.Headers.ProxyAuthenticate
                        );
                        Assert.Contains(
                            "max-age=2592000; pin-sha256=\"E9CZ9INDbd+2eRQozYqqbQ2yXLVKB9+xcprMF+44U1g=\"",
                            resp.Headers.GetValues("Public-Key-Pins")
                        );
                        Assert.Equal(
                            TimeSpan.FromSeconds(120),
                            resp.Headers.RetryAfter.Delta.GetValueOrDefault()
                        );
                        Assert.Contains(
                            new ProductInfoHeaderValue("Apache", "2.4.1"),
                            resp.Headers.Server
                        );
                        ...
                        Assert.Contains(
                            ...
                            resp.Headers.GetValues("Strict-Transport-Security")
                        );
                        Assert.Contains("Max-Forwards", resp.Headers.Trailer);
                        Assert.Contains("?", resp.Headers.GetValues("Tk"));
                        Assert.Contains(
                            new ProductHeaderValue("HTTPS", "1.3"),
                            resp.Headers.Upgrade
                        );
                        ...
                        Assert.Contains(
                            "default-src 'self'",
                            resp.Headers.GetValues("X-WebKit-CSP")
                        );
                        Assert.Contains(
                            "5; url=http://www.w3.org/pub/WWW/People.html",
                            resp.Headers.GetValues("Refresh")
                        );
                        Assert.Contains("200 OK", resp.Headers.GetValues("Status"));
                        Assert.Contains(
                            "<origin>[, <origin>]*",
                            resp.Headers.GetValues("Timing-Allow-Origin")
                        );
                        ...
                        Assert.Contains(
                            "f058ebd6-02f7-4d3f-942e-904344e8cde5",
                            resp.Headers.GetValues("X-Request-ID")
                        );
                        Assert.Contains("IE=EmulateIE7", resp.Headers.GetValues("X-UA-Compatible"));
                        Assert.Contains(
                            "1; mode=block",
                            resp.Headers.GetValues("X-XSS-Protection")
                        );
                    }
                },
                server =>
        ...
        [SkipOnPlatform(
            TestPlatforms.Browser,
            "This is blocking forever on Browser, there is infinite default timeout"
        )]
        public async Task SendAsync_ReadFromSlowStreamingServer_PartialDataReturned()
        {
            ...
            await LoopbackServer.CreateServerAsync(
                async (server, url) =>
                {
                    using (HttpClient client = CreateHttpClient())
                    {
                        Task<HttpResponseMessage> getResponse = client.GetAsync(
                            url,
                            HttpCompletionOption.ResponseHeadersRead
                        );
        ...
        public async Task ReadAsStreamAsync_HandlerProducesWellBehavedResponseStream(
            bool? chunked,
            bool enableWasmStreaming,
            bool slowChunks
        )
        {
            if (IsWinHttpHandler && UseVersion >= HttpVersion20.Value)
                ...
                async uri =>
                {
                    ...
                    )
                    {
                        ...
                        )
                        {
                            ...
                            Assert.Equal(
                                PlatformDetection.IsBrowser && !enableWasmStreaming,
...
#endif
                            await Assert.ThrowsAsync<NotSupportedException>(
                                async () => await responseStream.WriteAsync(new byte[1], 0, 1)
                            );
                            ...
                            if (PlatformDetection.IsBrowser)
                            {
                                ...
                                if (slowChunks)
                                {
                                    Assert.Equal(
                                        1,
                                        await responseStream.ReadAsync(new Memory<byte>(buffer2))
                                    );
                                ...
                                Assert.Equal(
                                    1,
                                    await responseStream.ReadAsync(new Memory<byte>(buffer))
                                );
                                ...
                                Assert.Equal(
                                    0,
                                    await responseStream.ReadAsync(Array.Empty<byte>(), 0, 0)
                                );
                ...
                async uri =>
                {
                    ...
                    )
                    {
                        ...
                        )
                        {
                            ...
                            var sizePromise = responseStream.ReadAsync(
                    ...
                    await LoopbackServerFactory.CreateServerAsync(
                        async (server2, url2) =>
                        {
                            await LoopbackServerFactory.CreateServerAsync(
                                async (server3, url3) =>
                                {
                                    var unblockServers = new TaskCompletionSource<bool>(
                                    ...
                                    Task serverTask3 = server3.AcceptConnectionAsync(
                                        async connection3 =>
                                        {
                                            ...
                                            await connection3.SendResponseBodyAsync(
                                                "1234567890",
                                                isFinal: false
                                            );
                                            await unblockServers.Task;
                                    ...
                                    using (HttpClient client = CreateHttpClient())
                                    {
                                        get1 = client.GetAsync(
                                            url1,
                                            HttpCompletionOption.ResponseHeadersRead
                                        );
                                        get2 = client.GetAsync(
                                            url2,
                                            HttpCompletionOption.ResponseHeadersRead
                                        );
                                        response3 = await client.GetAsync(
                                            url3,
                                            HttpCompletionOption.ResponseHeadersRead
                                        );
            ...
            await LoopbackServerFactory.CreateClientAndServerAsync(
                async uri =>
                {
                    ...
                    using (HttpClient client = CreateHttpClient(handler))
                    {
                        ...
                        Assert.Equal(
                            CookieHeaderExpected,
                            response.Headers.GetValues("Cookie").First().ToString()
                        );
                        // Verify Set-Cookie header.
                        Assert.Equal(1, handler.CookieContainer.Count);
                        Assert.Equal(
                            SetCookieExpected,
                            handler.CookieContainer.GetCookieHeader(uri)
                        );
                        // Verify Content-type header.
                        Assert.Equal(
                            ContentTypeHeaderExpected,
        ...
        [InlineData(true, false)]
        [InlineData(true, true)]
        public async Task PostAsync_ThrowFromContentCopy_RequestFails(
            bool syncFailure,
            bool enableWasmStreaming
        )
        {
                        ...
                        var content = new StreamContent(
                            new DelegateStream(
                                ...
                                readAsyncFunc: (buffer, offset, count, cancellationToken) =>
                                    syncFailure
                                        ? throw error
                                        : Task.Delay(1).ContinueWith<int>(_ => throw error)
                            )
                        );
                        var request = new HttpRequestMessage(HttpMethod.Post, uri);
        ...
        public async Task SendAsync_RequestVersion10_ServerReceivesVersion10Request()
        {
            ...
            Version receivedRequestVersion = await SendRequestAndGetRequestVersionAsync(
                new Version(1, 0)
            );
        ...
        [ConditionalFact(
            typeof(PlatformDetection),
            nameof(PlatformDetection.IsNotBrowserDomSupported)
        )]
        ...
        public static bool InvalidRequestUriTest_IsSupported =>
            PlatformDetection.IsNotNetFramework && PlatformDetection.IsNotBrowser;
