...
namespace System.Net.Http.Functional.Tests
{
    ...
    public abstract class HttpClientHandler_Decompression_Test : HttpClientHandlerTestBase
    {
        ...
        [Theory]
        [MemberData(
            nameof(DecompressedResponse_MethodSpecified_DecompressedContentReturned_MemberData)
        )]
        ...
        {
            if (IsWinHttpHandler && (compressionName == "br" || compressionName == "zlib"))
            {
                ...
                async uri =>
                {
                    ...
                    using (HttpClient client = CreateHttpClient(handler))
                    {
                        ...
                        AssertExtensions.SequenceEqual(
                            expectedContent,
                            await client.GetByteArrayAsync(TestAsync, useCopyTo, uri)
                        );
                    }
                },
                async server =>
        ...
        public static IEnumerable<object[]> DecompressedResponse_MethodNotSpecified_OriginalContentReturned_MemberData()
        {
            foreach (bool useCopyTo in new[] { false, true })
            {
                ...
                {
                    "gzip",
                    new Func<Stream, Stream>(
                        s => new GZipStream(s, CompressionLevel.Optimal, leaveOpen: true)
                    ),
                ...
                yield return new object[]
                {
                    "deflate",
                    new Func<Stream, Stream>(
                        s => new ZLibStream(s, CompressionLevel.Optimal, leaveOpen: true)
                    ),
                ...
                yield return new object[]
                {
                    "br",
                    new Func<Stream, Stream>(
                        s => new BrotliStream(s, CompressionLevel.Optimal, leaveOpen: true)
                    ),
                    DecompressionMethods.Deflate | DecompressionMethods.GZip,
                    useCopyTo
                };
        ...
        [Theory]
        [MemberData(
            nameof(DecompressedResponse_MethodNotSpecified_OriginalContentReturned_MemberData)
        )]
        [SkipOnPlatform(TestPlatforms.Browser, "AutomaticDecompression not supported on Browser")]
...
#endif
        [SkipOnPlatform(TestPlatforms.Browser, "AutomaticDecompression not supported on Browser")]
        public async Task DecompressedResponse_EmptyBody_Success(
            string encodingName,
            DecompressionMethods methods
        )
        {
            await LoopbackServer.CreateClientAndServerAsync(
                async uri =>
                {
                    ...
                    using (HttpClient client = CreateHttpClient(handler))
                    {
                        ...
                        Assert.Equal(
                            Array.Empty<byte>(),
                            await client.GetByteArrayAsync(TestAsync, useCopyTo: false, uri)
                        );
                    }
                },
                async server =>
            ...
            await LoopbackServer.CreateServerAsync(
                async (server, url) =>
                {
                    ...
                    using (HttpClient client = CreateHttpClient(handler))
                    {
                        ...
                        Assert.InRange(
                            Regex.Matches(requestLinesString, "Accept-Encoding").Count,
                            1,
                            1
                        );
                        ...
                        if (!string.IsNullOrEmpty(manualAcceptEncodingHeaderValues))
                        {
                            Assert.InRange(
                                Regex
                                    .Matches(requestLinesString, manualAcceptEncodingHeaderValues)
                                    .Count,
                                1,
                                1
                            );
        ...
        [InlineData(
            ...
            "gzip; q=1.0, deflate; q=1.0, br; q=1.0",
            ""
        )]
#endif
        [InlineData(
