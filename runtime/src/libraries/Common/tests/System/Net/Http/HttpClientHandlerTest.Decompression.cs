    ...
    public abstract class HttpClientHandler_Decompression_Test : HttpClientHandlerTestBase
    {
                ...
                case "deflate":
                    ...
                    compress = IsWinHttpHandler
                        ? new Func<Stream, Stream>(
                              s => new DeflateStream(s, CompressionLevel.Optimal, leaveOpen: true)
                          )
                        : new Func<Stream, Stream>(
        ...
        public async Task GetAsync_SetAutomaticDecompression_HeadersRemoved(
            ...
            using (
                ...
                Assert.False(
                    response.Content.Headers.Contains("Content-Encoding"),
                    "Content-Encoding unexpectedly found"
                );
                Assert.False(
                    response.Content.Headers.Contains("Content-Length"),
                    "Content-Length unexpectedly found"
                );
            }
        }
