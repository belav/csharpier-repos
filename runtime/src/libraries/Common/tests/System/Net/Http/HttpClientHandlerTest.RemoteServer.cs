...
namespace System.Net.Http.Functional.Tests
{
    ...
    public sealed class HttpClientHandler_RemoteServerTest : HttpClientHandlerTestBase
    {
        ...
        public static IEnumerable<(
            Configuration.Http.RemoteServer remoteServer,
            Uri uri
        )> RemoteServersAndHeaderEchoUris()
        {
            foreach (
        ...
        )
        {
            ...
            using (HttpClient client = CreateHttpClient(handler))
            {
                try
                {
                    ...
                    if (
                        response.StatusCode
                        is HttpStatusCode.GatewayTimeout
                    ...
                    when (hre.InnerException is SocketException se
                        && (se.SocketErrorCode is SocketError.WouldBlock or SocketError.TryAgain)
                    )
                {
