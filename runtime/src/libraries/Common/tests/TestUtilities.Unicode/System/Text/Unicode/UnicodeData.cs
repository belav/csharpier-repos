...
namespace System.Text.Unicode
{
    public static class UnicodeData
    {
        ...
        private static readonly Lazy<ParsedUnicodeData> _lazyParsedData =
            new Lazy<ParsedUnicodeData>();
            ...
            if ((uint)codePoint >= (uint)_codePointData.Length)
            {
                throw new ArgumentOutOfRangeException(
                    message: FormattableString.Invariant(
                        $"Value U+{(uint)codePoint:X4} is not a valid code point."
                    ),
        ...
        public static bool IsWhiteSpace(int codePoint) =>
            (GetData(codePoint).Flags & CodePointFlags.White_Space) != 0;
    }
}
