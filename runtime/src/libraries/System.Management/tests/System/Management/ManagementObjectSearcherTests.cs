...
namespace System.Management.Tests
{
    public class ManagementObjectSearcherTests
    {
        ...
        public void Related_Instances()
        {
            using (var searcher = new ManagementObjectSearcher())
            {
                var relatedObjectQuery = new RelatedObjectQuery(
                ...
                foreach (ManagementObject instance in instances)
                    Assert.True(
                        expectedAssociatedInstanceClasses.Contains(instance.Path.ClassName),
                        $"Unexpected instance of class {instance.Path.ClassName}"
                    );
            }
        }
        ...
        public void Relationship_Classes()
        {
            using (var searcher = new ManagementObjectSearcher())
            {
                ...
                foreach (ManagementObject instance in instances)
                    Assert.True(
                        expectedReferenceClasses.Contains(instance.Path.ClassName),
                        $"Unexpected instance of class {instance.Path.ClassName}"
                    );
            }
        }
