...
namespace Microsoft.Extensions.Options.Tests
{
    public class OptionsValidationBuilderTests
    {
        ...
        public void ValidateAddErrors()
        {
            ...
            Assert.True(
                EqualResults(ValidateOptionsResult.Fail(errors), r),
                $"{r.FailureMessage} != {ValidateOptionsResult.Fail(errors).FailureMessage}"
            );
        ...
        public void ValidateAddValidationResult()
        {
            ...
            builder.AddResult(
                new ValidationResult("Failure 5", new List<string>() { "member1", "member2" })
            );
            r = builder.Build();
            Assert.True(
                EqualResults(ValidateOptionsResult.Fail(errors), r),
        ...
        public void ValidateAddValidateOptionResult()
        {
            ...
            builder.AddResult(
                ValidateOptionsResult.Fail(
                    new List<string>() { "Failure 9", null, null, "Failure 10" }
                )
            );
            r = builder.Build();
            Assert.True(
        ...
        public void ValidateAddingMixedErrors()
        {
            ...
            builder.AddResult(
                new ValidationResult("Failure 15", new List<string>() { "member1", "member2" })
            );
            r = builder.Build();
            Assert.True(
                EqualResults(ValidateOptionsResult.Fail(errors), r),
            ...
            builder.AddResult(
                ValidateOptionsResult.Fail(
                    new List<string>() { "Failure 19", null, null, "Failure 20" }
                )
            );
            r = builder.Build();
            Assert.True(
        ...
        private static bool EqualResults(ValidateOptionsResult r1, ValidateOptionsResult r2) =>
            ...
            && (
                r1.Failures == r1.Failures
                || (
                    r1.Failures != null
                    && r1.Failures != null
                    && Enumerable.SequenceEqual(r1.Failures, r2.Failures)
                )
            );
    }
}
