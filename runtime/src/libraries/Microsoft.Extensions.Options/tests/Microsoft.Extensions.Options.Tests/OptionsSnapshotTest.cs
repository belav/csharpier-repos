...
namespace Microsoft.Extensions.Options.Tests
{
    public class OptionsSnapshotTest
    {
        ...
        public void SnapshotOptionsAreCachedPerScope()
        {
            ...
            {
                options = scope
                    .ServiceProvider.GetRequiredService<IOptionsSnapshot<FakeOptions>>()
                    .Value;
                Assert.Equal(
                    options,
        ...
        )
        {
            Assert.NotNull(
                services
                    .Where(s => s.ServiceType == serviceType && s.Lifetime == lifetime)
                    .SingleOrDefault()
            );
        }
        ...
        public void RecreateAspNetCore_AddOidc_CustomStateAndAccount_SetsUpConfiguration()
        {
            ...
            // from the root scope.
            var rootOptions = provider.GetRequiredService<
