...
namespace System.Globalization.Tests
{
    public class StringNormalizationAllTests
    {
        ...
        /// </summary>
        private static void VerifyConformanceInvariant(
        ...
        )
        {
            ...
            switch (normForm)
            {
                case NormalizationForm.FormC:
                    ...
                    Assert.True(
                        c2.IsNormalized(normForm),
                        $"'{c2}' is marked as not normalized with form {normForm}"
                    );

                    // c4 is normalized to Form C
                    Assert.True(
                        c4.IsNormalized(normForm),
                        $"'{c4}' is marked as not normalized with form {normForm}"
                    );
                    break;
                ...
                case NormalizationForm.FormD:
                    ...
                    Assert.True(
                        c3.IsNormalized(normForm),
                        $"'{c3}' is marked as not normalized with form {normForm}"
                    );

                    // c5 is normalized to Form D
                    Assert.True(
                        c5.IsNormalized(normForm),
                        $"'{c5}' is marked as not normalized with form {normForm}"
                    );
                    break;
        ...
        private static void AssertEqualsForm(string c, string cForm)
        {
            Assert.True(
                c.Equals(cForm),
                $"'{DumpStringAsCodepoints(c)}' is not matched with the normalized form '{DumpStringAsCodepoints(cForm)} with {cForm} normalization"
            );
        }
