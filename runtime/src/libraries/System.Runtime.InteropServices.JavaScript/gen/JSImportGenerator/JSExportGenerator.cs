    ...
    public sealed class JSExportGenerator : IIncrementalGenerator
    {
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            IncrementalValueProvider<
                ImmutableArray<(StatementSyntax, AttributeListSyntax)>
            > regSyntax = generateSingleStub
                .Select(static (data, ct) => (data.Item2, data.Item3))
                .Collect();
            ...
            IncrementalValueProvider<ImmutableArray<(string, string)>> generated =
                generateSingleStub
                    .Combine(registration)
                    .Select(
                        static (data, ct) =>
                            (data.Left.Item1.NormalizeWhitespace().ToFullString(), data.Right)
                    )
                    .Collect();
        ...
        private static MemberDeclarationSyntax PrintGeneratedSource(
            ContainingSyntaxContext containingSyntaxContext,
            BlockSyntax wrapperStatements,
            string wrapperName
        )
        {
            MemberDeclarationSyntax wrappperMethod = MethodDeclaration(
                ...
                .WithParameterList(
                    ParameterList(
                        SingletonSeparatedList(
                            Parameter(Identifier("__arguments_buffer"))
                                .WithType(
                                    PointerType(ParseTypeName(Constants.JSMarshalerArgumentGlobal))
                                )
                        )
                    )
                )
        ...
        private static MarshallingGeneratorFactoryKey<(
            TargetFramework,
            Version,
            JSGeneratorOptions
        )> CreateGeneratorFactory(StubEnvironment env, JSGeneratorOptions options)
        {
        ...
        )
        {
            ...
            if (methods.IsEmpty)
                return NamespaceDeclaration(IdentifierName(generatedNamespace));
            ...
            FieldDeclarationSyntax field = FieldDeclaration(
                    VariableDeclaration(PredefinedType(Token(SyntaxKind.BoolKeyword)))
                        .WithVariables(
                            SingletonSeparatedList(VariableDeclarator(Identifier("initialized")))
                        )
                )
                .WithModifiers(TokenList(Token(SyntaxKind.StaticKeyword)));

            MemberDeclarationSyntax method = MethodDeclaration(
                    PredefinedType(Token(SyntaxKind.VoidKeyword)),
            ...
            MemberDeclarationSyntax initializerMethod = MethodDeclaration(
                    PredefinedType(Token(SyntaxKind.VoidKeyword)),
                    Identifier(selfInitName)
                )
                .WithAttributeLists(
                    List(
                        new[]
                ...
                .WithModifiers(
                    TokenList(
                        new[] { Token(SyntaxKind.StaticKeyword), Token(SyntaxKind.InternalKeyword) }
                    )
                )
                .WithBody(
                    Block(
                        IfStatement(
                            ...
                            Block(
                                SingletonList<StatementSyntax>(
                                    ExpressionStatement(
                                        InvocationExpression(IdentifierName(initializerName))
                                    )
                                )
                            )
                        )
                ...
                .WithMembers(
                    SingletonList<MemberDeclarationSyntax>(
                        ClassDeclaration(initializerClass)
                            .WithModifiers(
                                TokenList(new SyntaxToken[] { Token(SyntaxKind.UnsafeKeyword) })
                            )
        ...
        private static (
            ...
            AttributeListSyntax,
            ImmutableArray<Diagnostic>
        ) GenerateSource(IncrementalStubGenerationContext incrementalContext)
        {
            ...
            var wrapperName =
                "__Wrapper_"
                + incrementalContext.StubMethodSyntaxTemplate.Identifier
                + "_"
                + incrementalContext.SignatureContext.TypesHash;
            ...
            AttributeListSyntax registrationAttribute = AttributeList(
                SingletonSeparatedList(
                    Attribute(IdentifierName(Constants.DynamicDependencyAttributeGlobal))
                        .WithArgumentList(
