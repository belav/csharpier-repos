        ...
        internal sealed class FilterAndTransform
        {
            ...
            )
            {
                ...
                if (colonIdx >= 0)
                {
                    ...
                    // If the transform spec begins with a - it means you don't want implicit transforms.
                    if (
                        startTransformIdx < endIdx
                        && filterAndPayloadSpec[startTransformIdx] == '-'
                    )
                    {
                        _eventSource.Message("DiagnosticSource: suppressing implicit transforms.");
                        _noImplicitTransforms = true;
            ...
            internal static bool IsActivitySourceEntry(
                ...
                filterAndPayloadSpec
                    .AsSpan(startIdx, endIdx - startIdx)
            ...
            )
            {
                ...
                ReadOnlySpan<char> entry = filterAndPayloadSpec
                    .AsSpan(
                        startIdx + c_ActivitySourcePrefix.Length,
            ...
            private static TransformSpec? MakeImplicitTransforms(Type type)
            {
                ...
                foreach (
                    PropertyInfo property in curTypeInfo.GetProperties(
                        BindingFlags.Static
        ...
        internal sealed class TransformSpec
        {
            ...
            internal sealed class PropertySpec
            {
                ...
                public object? Fetch(object? obj)
                {
                    ...
                    {
                        Log.Message($"Property {objType}.{_propertyName} threw the exception {e}");
                    }
                    return ret;
                }
                ...
                private class PropertyFetch
                {
                    ...
                    public static PropertyFetch FetcherForProperty(Type? type, string propertyName)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                foreach (
                                    PropertyInfo pi in typeInfo.GetProperties(
                                        BindingFlags.Public
                                            | BindingFlags.NonPublic
                                            | BindingFlags.Instance
                                            | BindingFlags.Static
                    ...
                    private static PropertyFetch CreateEnumeratePropertyFetch(
                        Type type,
                        TypeInfo enumerableOfTType
                    )
                    {
                    ...
                    private static PropertyFetch CreatePropertyFetch(
                        Type type,
