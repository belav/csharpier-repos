...
namespace System.Diagnostics.Tests
{
    public class ActivitySourceTests : IDisposable
    {
            ...
            RemoteExecutor
                .Invoke(() =>
                {
                    using (ActivitySource aSource = new ActivitySource("SourceActivityListener"))
                    {
                        ...
                        using (
                            Activity activity = aSource.StartActivity("AllDataRequestedActivity")
                        )
                        {
                            ...
                            Assert.True(
                                object.ReferenceEquals(activity, activity.AddTag("key", "value"))
                            );
                            Assert.True(
                            ...
                            )
                            {
                                ...
                                Assert.True(
                                    object.ReferenceEquals(
                                        activity1,
                                        activity1.AddEvent(new ActivityEvent("e1"))
                                    )
                                );
                ...
                .Invoke(() =>
                {
                    ...
                    )
                    {
                        ...
                        using (Activity activity = aSource.StartActivity()) // passing default name should trigger CallerMemberName attribute.
                        {
                            Assert.NotNull(activity);
                            Assert.True(
                                methodName.IndexOf(activity.OperationName, StringComparison.Ordinal)
                                    >= 0
                            );
                            ...
                            List<KeyValuePair<string, object>> tags = new List<
                            ...
                            using (
                                Activity activity2 = aSource.StartActivity(
                                    ActivityKind.Server,
                                    parentContext,
                                    tags,
                                    links,
            ...
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    using (ActivityListener listener = new ActivityListener())
                    {
                        ...
                        listener.ShouldListenTo = (activitySource) =>
                            activitySource.Name == "" && activitySource.Version == "";
                        listener.SampleUsingParentId = (
                            ref ActivityCreationOptions<string> activityOptions
                        ) => ActivitySamplingResult.AllData;
                        listener.Sample = (
            ...
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    using (Activity a4 = source.StartActivity("a4"))
                    {
                        ...
                        Assert.True(
                            (a4.ActivityTraceFlags & ActivityTraceFlags.Recorded) != 0,
                            $"a4.ActivityTraceFlags failed: {a4.ActivityTraceFlags}"
                        );
                ...
                .Invoke(() =>
                {
                    ...
                    )
                    {
                        ...
                        foreach (ActivityLink link in links)
                        {
                            Assert.NotEqual(
                                default,
                                activity.Links.FirstOrDefault((l) => link == l)
                            );
                        }
                    }

                    using Activity activity1 = source.StartActivity(
                        "a2",
                        ActivityKind.Client,
                        "NoW3CParentId",
                        attributes,
        ...
        public void TestDefaultParentContext()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    listener.ShouldListenTo = (activitySource) =>
                        activitySource.Name == "ParentContext";
                    listener.Sample = (
                        ref ActivityCreationOptions<ActivityContext> activityOptions
                    ) =>
                    {
                    ...
                    ActivitySource.AddActivityListener(listener);
                    using Activity a = aSource.StartActivity(
                        "a",
                        ActivityKind.Server,
                        new ActivityContext(
                            ActivityTraceId.CreateRandom(),
        ...
        public void TestActivityContextParsing()
        {
            ...
            const string invalidW3CContext =
                "00-Z9d43cb30a4cdb4fbeee3a19c29201b0-e82825765f051b47-01";
            Assert.False(ActivityContext.TryParse(invalidW3CContext, null, out context));
            Assert.False(
                ActivityContext.TryParse(invalidW3CContext, null, isRemote: true, out context)
            );
            Assert.False(
                ActivityContext.TryParse(invalidW3CContext, null, isRemote: false, out context)
            );
        }
        ...
        public void TestCreatingActivityUsingDifferentParentIds()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    ActivityTraceId traceId = ActivityTraceId.CreateFromString(
                        "99d43cb30a4cdb4fbeee3a19c29201b0".AsSpan()
                    );
        ...
        public void TestActivityContextIsRemote()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    using ActivityListener listener = new ActivityListener();
                    listener.ShouldListenTo = (activitySource) =>
                        activitySource.Name == "RemoteContext";
        ...
        public void TestTraceIdAutoGeneration()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    using ActivityListener listener = new ActivityListener();
                    listener.ShouldListenTo = (activitySource) =>
                        activitySource.Name == "TraceIdAutoGeneration";
                    ...
                    {
                        ctx = forceGenerateTraceId
                            ? new ActivityContext(activityOptions.TraceId, default, default)
                            : activityOptions.Parent;
                        return ActivitySamplingResult.AllData;
                    };
                    ...
                    if (activity.IdFormat == ActivityIdFormat.W3C)
                    {
                        ...
                        Assert.Equal(
                            default(ActivitySpanId).ToHexString(),
                            ctx.SpanId.ToHexString()
                        );
                    }
                })
                .Dispose();
        ...
        public void TestTraceIdAutoGenerationWithNullParentId()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    using ActivitySource aSource = new ActivitySource(
                        "TraceIdAutoGenerationWithNullParent"
                    );
                    using ActivityListener listener = new ActivityListener();
                    listener.ShouldListenTo = (activitySource) =>
                        activitySource.Name == "TraceIdAutoGenerationWithNullParent";
        ...
        public void TestEventNotificationOrder()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    listener.ShouldListenTo = (activitySource) =>
                        activitySource.Name == "EventNotificationOrder";
                    listener.Sample = (
                        ref ActivityCreationOptions<ActivityContext> activityOptions
                    ) => ActivitySamplingResult.AllData;
        ...
        public void TestAddingSamplerTags()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    using ActivityListener listener = new ActivityListener();
                    listener.ShouldListenTo = (activitySource) =>
                        activitySource.Name == "SamplerTags1";
                    ...
                    Assert.Equal(
                        ...
                        activity.TagObjects.ElementAt(0)
                    );
                    Assert.Equal(
                        new KeyValuePair<string, object>("tag2", "value2"),
                        activity.TagObjects.ElementAt(1)
                    );
                })
                .Dispose();
        }
        ...
        public void TestAddingSamplerTagsUsingParentId()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    using ActivityListener listener = new ActivityListener();
                    listener.ShouldListenTo = (activitySource) =>
                        activitySource.Name == "SamplerTags2";
        ...
        public void TestNotAddedSamplerTags()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    using ActivityListener listener = new ActivityListener();
                    listener.ShouldListenTo = (activitySource) =>
                        activitySource.Name == "SamplerTags3";
                    ...
                    using Activity activity = aSource.StartActivity(
                        "a1",
                        ActivityKind.Client,
                        "NonW3CParentId"
                    );
        ...
        public void TestDefaultTraceIdWithHierarchicalParentId()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    using ActivityListener listener = new ActivityListener();
                    listener.ShouldListenTo = (activitySource) =>
                        activitySource.Name == "SamplerTags4";
        ...
        public void TestAddingSamplerTagsFromMultipleListeners()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    listener1.ShouldListenTo = (activitySource) =>
        ...
        public void TestHasRemoteParent()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    using (
                        Activity activity = aSource.CreateActivity(
                            "a2",
                            ActivityKind.Client,
                            new ActivityContext(
                                ActivityTraceId.CreateRandom(),
        ...
        public void TestAddSamplerAndActivityCreationTags()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    using ActivityListener listener = new ActivityListener();
                    listener.ShouldListenTo = (activitySource) =>
                        activitySource.Name == "SamplerTags7";
                    ...
                    Assert.Equal(
                        new KeyValuePair<string, object>("SamplerTag1", "SamplerValue1"),
                        activity.TagObjects.ElementAt(2)
                    );
                })
                .Dispose();
        }
        ...
        public void RestoreOriginalParentTest()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    listener.ShouldListenTo = (activitySource) =>
                        object.ReferenceEquals(source, activitySource);
                    listener.SampleUsingParentId = (
                        ref ActivityCreationOptions<string> activityOptions
                    ) => ActivitySamplingResult.AllData;
                    listener.Sample = (
                    ...
                    using (Activity c = source.StartActivity("Root"))
                    {
                        ...
                        using (
                            Activity d = source.StartActivity(
                                "Child",
                                default,
                                new ActivityContext(
                                    ActivityTraceId.CreateRandom(),
            ...
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    using (Activity a2 = aSource.CreateActivity("a2", ActivityKind.Server))
                    {
                        ...
                        using (
                            Activity a3 = aSource.CreateActivity(
                                "a3",
                                ActivityKind.Client,
                                "ParentId",
                                tags,
                        ...
                        )
                        {
                            ...
                            using (
                                Activity a4 = aSource.CreateActivity(
                                    "a4",
        ...
        public void TestAddingMorePropertiesWithActivityCreate()
        {
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    using (
                        Activity a = aSource.CreateActivity(
                            "a",
                            ActivityKind.Server,
                            default(string),
                            default,
                    ...
                    )
                    {
                        ...
                        tagsToCheckAgainst = new List<KeyValuePair<string, object>>()
                        {
                            new KeyValuePair<string, object>("tag1", "value1"),
                        };
                        baggageToCheckAgainst = new List<KeyValuePair<string, string>>()
                        {
                            new KeyValuePair<string, string>("baggage1", "value1"),
                        };
            ...
            RemoteExecutor
                .Invoke(() =>
                {
                    ...
                    ) =>
                    {
                        ...
                        {
                            activityOptions = activityOptions with
                            {
                                TraceState = traceStateValueWithStringParent,
                            };
                        }
                        return ActivitySamplingResult.AllDataAndRecorded;
                    };
                    ...
                    SampleActivity<ActivityContext> sampleUsingContext = (
                        ref ActivityCreationOptions<ActivityContext> activityOptions
                    ) =>
                    {
                        ...
                        if (setTraceWithContextParent)
                        {
                            activityOptions = activityOptions with
                            {
                                TraceState = traceStateValueWithContextParent,
                            };
                        }
                        return ActivitySamplingResult.AllDataAndRecorded;
                    };
                    ...
                    using (
                        Activity a = aSource.StartActivity("a", ActivityKind.Server, "SomeParent")
                    )
                    {
                    ...
                    using (
                        Activity a = aSource.CreateActivity(
                            "a",
                            ActivityKind.Server,
                            "00-99d43cb30a4cdb4fbeee3a19c29201b0-e82825765f051b47-01",
                            default,
                            default,
        ...
        //           --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        [InlineData(
            $"Unknown,                  Unknown,                    Unknown,                        null,       true,                   false,              Default,                Default"
        )]
        [InlineData(
            $"Unknown,                  Unknown,                    W3C,                            null,       true,                   false,              W3C,                    W3C"
        )]
        [InlineData(
            $"Unknown,                  Unknown,                    Hierarchical,                   null,       true,                   false,              Hierarchical,           Hierarchical"
        )]
        [InlineData(
            $"Unknown,                  Unknown,                    Unknown,                        NonWC3,     true,                   false,              Hierarchical,           Hierarchical"
        )]
        [InlineData(
            $"Unknown,                  Unknown,                    Unknown,                        W3C,        true,                   false,              W3C,                    W3C"
        )]
        [InlineData(
            $"Unknown,                  Unknown,                    Unknown,                        null,       false,                  false,              W3C,                    W3C"
        )]
        [InlineData(
            $"W3C,                      Unknown,                    Unknown,                        null,       true,                   false,              W3C,                    W3C"
        )]
        [InlineData(
            $"Hierarchical,             Unknown,                    Unknown,                        null,       true,                   false,              Hierarchical,           Hierarchical"
        )]
        [InlineData(
            $"W3C,                      Unknown,                    Hierarchical,                   null,       true,                   false,              W3C,                    W3C"
        )]
        [InlineData(
            $"Hierarchical,             Unknown,                    W3C,                            null,       true,                   false,              Hierarchical,           Hierarchical"
        )]
        [InlineData(
            $"W3C,                      Unknown,                    Unknown,                        NonW3C,     true,                   false,              W3C,                    W3C"
        )]
        [InlineData(
            $"Hierarchical,             Unknown,                    Unknown,                        W3C,        true,                   false,              Hierarchical,           Hierarchical"
        )]
        [InlineData(
            $"W3C,                      Unknown,                    Unknown,                        null,       false,                  false,              W3C,                    W3C"
        )]
        [InlineData(
            $"Hierarchical,             Unknown,                    Unknown,                        null,       false,                  false,              Hierarchical,           Hierarchical"
        )]
        [InlineData(
            $"Unknown,                  W3C,                        Unknown,                        null,       true,                   false,              W3C,                    Default"
        )]
        [InlineData(
            $"Hierarchical,             W3C,                        Unknown,                        null,       true,                   false,              W3C,                    Hierarchical"
        )]
        [InlineData(
            $"Unknown,                  W3C,                        Hierarchical,                   null,       true,                   false,              W3C,                    Hierarchical"
        )]
        [InlineData(
            $"Hierarchical,             W3C,                        Hierarchical,                   null,       true,                   false,              W3C,                    Hierarchical"
        )]
        [InlineData(
            $"Hierarchical,             W3C,                        Hierarchical,                   NonW3C,     true,                   false,              W3C,                    Hierarchical"
        )]
        [InlineData(
            $"Unknown,                  Hierarchical,               Unknown,                        null,       true,                   false,              Hierarchical,           Default"
        )]
        [InlineData(
            $"W3C,                      Hierarchical,               Unknown,                        null,       true,                   false,              Hierarchical,           W3C"
        )]
        [InlineData(
            $"Unknown,                  Hierarchical,               W3C,                            null,       true,                   false,              Hierarchical,           W3C"
        )]
        [InlineData(
            $"W3C,                      Hierarchical,               Hierarchical,                   null,       true,                   false,              Hierarchical,           W3C"
        )]
        [InlineData(
            $"W3C,                      Hierarchical,               Hierarchical,                   W3C,        true,                   false,              Hierarchical,           W3C"
        )]
        [InlineData(
            $"Unknown,                  Hierarchical,               Unknown,                        null,       false,                  false,              Hierarchical,           W3C"
        )]
        [InlineData(
            $"Unknown,                  W3C,                        Unknown,                        null,       false,                  false,              W3C,                    W3C"
        )]
        [InlineData(
            $"Unknown,                  W3C,                        Unknown,                        null,       true,                   true,               W3C,                    Default"
        )]
        [InlineData(
            $"W3C,                      Unknown,                    Unknown,                        null,       true,                   true,               W3C,                    W3C"
        )]
        [InlineData(
            $"Unknown,                  Unknown,                    W3C,                            null,       true,                   true,               W3C,                    W3C"
        )]
        [InlineData(
            $"Unknown,                  Unknown,                    Unknown,                        W3C,        true,                   true,               W3C,                    W3C"
        )]
        [InlineData(
            $"Unknown,                  Unknown,                    Unknown,                        null,       false,                  true,               W3C,                    W3C"
        )]
        [InlineData(
            $"W3C,                      Unknown,                    Unknown,                        NonW3C,     true,                   true,               W3C,                    W3C"
        )]
