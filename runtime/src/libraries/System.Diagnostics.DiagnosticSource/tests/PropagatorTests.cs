...
namespace System.Diagnostics.Tests
{
    public class PropagatorTests
    {
        ...
        private void TestDefaultPropagatorUsingW3CActivity(
        ...
        {
            using Activity a = CreateHierarchicalActivity(
                "LegacyHierarchical1",
                null,
                "LegacyHierarchicalState=1",
                baggage
            );
            using Activity b = CreateHierarchicalActivity(
                "LegacyHierarchical2",
                null,
                "LegacyHierarchicalState=2",
                baggage
            );
        ...
        private void TestDefaultPropagatorUsing(
            Activity a,
            DistributedContextPropagator propagator,
            string state,
            IEnumerable<KeyValuePair<string, string>> baggage
        ...
        private void TestNoOutputPropagatorUsingHierarchicalActivity(
            DistributedContextPropagator propagator,
            string state,
            IEnumerable<KeyValuePair<string, string>> baggage
        )
        ...
        private void TestPassThroughPropagatorUsingHierarchicalActivityWithParentChain(
            DistributedContextPropagator propagator,
            string state,
            IEnumerable<KeyValuePair<string, string>> baggage
        )
        ...
        private void TestPassThroughPropagatorWithNullCurrent(
            DistributedContextPropagator propagator
        )
        {
                ...
                ) =>
                {
                    ...
                    if (fieldName == RequestId)
                    {
                        if (a.IdFormat == ActivityIdFormat.W3C)
                        {
                            Assert.True(
                                false,
                                $"Not expected to get RequestId as we expect the request handled using TraceParenet."
                            );
                        }
                        else
                        {
                            Assert.True(
                                traceParentEncountered,
                                $"Expected to get TraceParent request before getting RequestId."
                            );
                            fieldValue = a.Id;
                        }
        ...
        private void TestBaggageExtraction(DistributedContextPropagator propagator, Activity a)
        {
            ...
            IEnumerable<KeyValuePair<string, string?>>? b = propagator.ExtractBaggage(
                null,
                (
                    object carrier,
                    string fieldName,
                ...
                ) =>
                {
                    ...
                    if (fieldName == CorrelationContext && a.IdFormat != ActivityIdFormat.W3C)
                    {
                        Assert.True(
                            baggageEncountered,
                            $"Expected to get Baggage request before getting Correlation-Context."
                        );
                        fieldValue = GetFormattedBaggage(a.Baggage);
                        return;
                    }
        ...
        internal static string GetFormattedBaggage(
        ...
        private Activity CreateHierarchicalActivity(
            string name,
            string parentId,
            string state,
            IEnumerable<KeyValuePair<string, string?>>? baggage
        ...
        private Activity CreateW3CActivity(
            string name,
            string state,
            IEnumerable<KeyValuePair<string, string?>>? baggage
        )
        ...
        internal static IEnumerable<KeyValuePair<string, string>>? ParseBaggage(
        ...
        )
        {
            ...
            foreach (string part in parts)
            {
                ...
                list.Add(
                    new KeyValuePair<string, string>(
                        WebUtility.UrlDecode(baggageItem[0]).Trim(),
                        WebUtility.UrlDecode(baggageItem[1]).Trim()
                    )
                );
            }
        ...
        internal class CustomPropagator : DistributedContextPropagator
        {
            ...
            public override IReadOnlyCollection<string> Fields { get; } =
                new[] { XTraceParent, XTraceState, XBaggage };
            ...
            )
            {
                ...
                if (activity.Baggage.Count() > 0)
                {
                    setter(
                        carrier,
                        XBaggage,
                        "x=y, " + PropagatorTests.GetFormattedBaggage(activity.Baggage)
                    );
                }
            }
