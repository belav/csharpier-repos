...
namespace System.Composition.Convention.Tests
{
    public class ConventionBuilderTests
    {
        ...
        public void MapType_ShouldReturnProjectedAttributesForType()
        {
            ...
            Attribute[] fooImplAttributes = builder.GetDeclaredAttributes(
                typeof(FooImpl),
                typeof(FooImpl).GetTypeInfo()
            );
            Attribute[] fooImplWithConstructorsAttributes = builder.GetDeclaredAttributes(
        ...
        public void MapType_ConventionSelectedConstructor()
        {
            ...
            ConstructorInfo constructor1 =
                fooImplWithConstructorsTypeInfo.DeclaredConstructors.Where(
            ...
            Assert.Equal(
                0,
                builder.GetCustomAttributes(typeof(FooImplWithConstructors), constructor1).Count()
            );
            Assert.Equal(
                0,
                builder.GetCustomAttributes(typeof(FooImplWithConstructors), constructor2).Count()
            );

            ConstructorInfo ci = constructor3;
            IEnumerable<Attribute> attrs = builder.GetCustomAttributes(
                typeof(FooImplWithConstructors),
                ci
            );
        ...
        public void MapType_OverridingSelectionOfConventionSelectedConstructor()
        {
            ...
            ConstructorInfo constructor1 = fooImplWithConstructors.DeclaredConstructors.Where(
                    c => c.GetParameters().Length == 0
                )
                .Single();
            ConstructorInfo constructor2 = fooImplWithConstructors.DeclaredConstructors.Where(
                    c => c.GetParameters().Length == 1
                )
                .Single();
            ConstructorInfo constructor3 = fooImplWithConstructors.DeclaredConstructors.Where(
                    c => c.GetParameters().Length == 2
                )
                .Single();
            ...
            Assert.Equal(
                0,
                builder.GetCustomAttributes(typeof(FooImplWithConstructors), constructor3).Count()
            );
