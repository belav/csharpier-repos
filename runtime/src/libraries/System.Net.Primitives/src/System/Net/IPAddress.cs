...
namespace System.Net
{
    ...
    public class IPAddress : ISpanFormattable, ISpanParsable<IPAddress>, IUtf8SpanFormattable
    {
        ...
        private bool TryFormatCore<TChar>(Span<TChar> destination, out int charsWritten)
            where TChar : unmanaged, IBinaryInteger<TChar>
        {
            if (IsIPv4)
            {
                ...
                {
                    charsWritten = IPAddressParser.FormatIPv4Address(
                        _addressOrScopeId,
                        destination
                    );
                    return true;
