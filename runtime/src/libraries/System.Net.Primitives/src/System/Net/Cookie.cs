...
namespace System.Net
{
    ...
    public sealed class Cookie
    {
        ...
        private static readonly IndexOfAnyValues<char> s_reservedToNameChars =
            IndexOfAnyValues.Create("\t\r\n=;,");

        private static readonly IndexOfAnyValues<char> s_domainChars = IndexOfAnyValues.Create(
...
#pragma warning restore 0649
        ...
        public bool Expired
        {
            get
            {
                return (m_expires != DateTime.MinValue)
                    && (m_expires.ToLocalTime() <= DateTime.Now);
            }
            set
            {
                if (value)
        ...
        internal bool VerifySetDefaults(
        ...
        )
        {
            ...
            if (
                m_value == null
                || (
                    !(m_value.Length > 2 && m_value.StartsWith('\"') && m_value.EndsWith('\"'))
                    && m_value.AsSpan().IndexOfAny(';', ',') >= 0
                )
            )
            {
                if (shouldThrow)
            ...
            if (
                Comment != null
                && !(Comment.Length > 2 && Comment.StartsWith('\"') && Comment.EndsWith('\"'))
            ...
            )
            {
                if (shouldThrow)
                {
                    throw new CookieException(
                        SR.Format(
                            SR.net_cookie_attribute,
                            CookieFields.CommentAttributeName,
                            Comment
                        )
                    );
                }
                    ...
                    if (!DomainCharsTest(domain))
                    {
                        if (shouldThrow)
                        {
                            throw new CookieException(
                                SR.Format(
                                    SR.net_cookie_attribute,
                                    CookieFields.DomainAttributeName,
                                    domain ?? "<null>"
                                )
                            );
                        }
                ...
                if (!valid)
                {
                    if (shouldThrow)
                    {
                        throw new CookieException(
                            SR.Format(
                                SR.net_cookie_attribute,
                                CookieFields.PortAttributeName,
                                m_port
                            )
                        );
                    }
        ...
        private static bool DomainCharsTest(string name) =>
            !string.IsNullOrEmpty(name) && name.AsSpan().IndexOfAnyExcept(s_domainChars) < 0;
        ...
        {
            get { return m_domain_implicit ? Domain : m_domainKey; }
        }
        ...
        internal void ToString(StringBuilder sb)
        {
            ...
            {
                sb.Append(
                    SpecialAttributeLiteral + CookieFields.VersionAttributeName + EqualsLiteral
                ); // const strings
                if (IsQuotedVersion)
                    sb.Append('"');
                ...
                if (!m_path_implicit && m_path.Length > 0)
                {
                    sb.Append(
                        SeparatorLiteral
                            ...
                            + CookieFields.PathAttributeName
                            + EqualsLiteral
                    ); // const strings
                    sb.Append(m_path);
                }
            ...
            {
                sb.Append(
                    SeparatorLiteral + SpecialAttributeLiteral + CookieFields.PortAttributeName
                ); // const strings
                if (m_port.Length > 0)
                {
        ...
        internal string? ToServerString()
        {
            ...
            if (m_commentUri != null)
            {
                result +=
                    SeparatorLiteral
                    + CookieFields.CommentUrlAttributeName
                    + EqualsLiteral
                    + QuotesLiteral
                    + m_commentUri.ToString()
                    + QuotesLiteral;
            }
            if (m_discard)
            {
            ...
            {
                result +=
                    SeparatorLiteral + CookieFields.DomainAttributeName + EqualsLiteral + m_domain;
            }
            if (Expires != DateTime.MinValue)
            {
                ...
                result +=
                    SeparatorLiteral
                    + CookieFields.MaxAgeAttributeName
                    + EqualsLiteral
            ...
            {
                result +=
                    SeparatorLiteral + CookieFields.PathAttributeName + EqualsLiteral + m_path;
            }
            if (!Plain && !m_port_implicit && m_port != null && m_port.Length > 0)
            {
                // QuotesLiteral are included in _port.
                result +=
                    SeparatorLiteral + CookieFields.PortAttributeName + EqualsLiteral + m_port;
            }
            if (m_version > 0)
            {
                result +=
