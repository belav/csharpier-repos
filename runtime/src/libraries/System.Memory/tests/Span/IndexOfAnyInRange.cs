...
{
    public class IndexOfAnyInRangeTests_Byte : IndexOfAnyInRangeTests<byte>
    {
    ...
    public class IndexOfAnyInRangeTests_Char : IndexOfAnyInRangeTests<char>
    {
    ...
    public class IndexOfAnyInRangeTests_UInt16 : IndexOfAnyInRangeTests<ushort>
    {
        protected override ushort Create(int value) => (ushort)value;
    }
    ...
    public class IndexOfAnyInRangeTests_UInt32 : IndexOfAnyInRangeTests<uint>
    {
        protected override uint Create(int value) => (uint)value;
    }
    ...
    public class IndexOfAnyInRangeTests_UInt64 : IndexOfAnyInRangeTests<ulong>
    {
        protected override ulong Create(int value) => (ulong)value;
    }
    ...
    public class IndexOfAnyInRangeTests_UIntPtr : IndexOfAnyInRangeTests<nuint>
    {
        protected override nuint Create(int value) => (nuint)value;
    }

    public class IndexOfAnyInRangeTests_TimeSpan : IndexOfAnyInRangeTests<TimeSpan>
    {
        protected override TimeSpan Create(int value) => TimeSpan.FromTicks(value);
    }
    ...
    public class IndexOfAnyInRangeTests_RefType : IndexOfAnyInRangeTests<RefType>
    {
        ...
        {
            foreach (
                (RefType low, RefType high) in new (RefType, RefType)[]
                {
        ...
        where T : IComparable<T>
    {
        ...
        public void FindValueInSequence_ReturnsFoundIndexOrMinus1()
        {
            for (int length = 1; length <= 64; length++)
            {
                ...
                Assert.Equal(
                    length - 1,
                    LastIndexOfAnyInRange(array, Create(0), Create(length - 1))
                );
                ...
                if (length > 4)
                {
                    Assert.Equal(
                        length / 4,
                        IndexOfAnyInRange(array, Create(length / 4), Create(length * 3 / 4))
                    );
                    Assert.Equal(
                        length * 3 / 4,
                        LastIndexOfAnyInRange(array, Create(length / 4), Create(length * 3 / 4))
                    );
                    Assert.Equal(
                        0,
                        IndexOfAnyExceptInRange(array, Create(length / 4), Create(length * 3 / 4))
                    );
                    Assert.Equal(
                        length - 1,
        ...
        public void MatrixOfLengthsAndOffsets_Any_FindsValueAtExpectedIndex()
        {
            foreach (int length in Enumerable.Range(1, 40))
            {
                for (int offset = 0; offset < length; offset++)
                {
                    ...
                    Assert.Equal(
                        offset,
                        IndexOfAnyInRange(array, Create(target - 1), Create(target))
                    );
                    Assert.Equal(
                        offset,
                        IndexOfAnyInRange(array, Create(target), Create(target + 1))
                    );
                    Assert.Equal(
                        offset,
                        IndexOfAnyInRange(array, Create(target - 1), Create(target + 1))
                    );

                    Assert.Equal(
                        offset,
                        LastIndexOfAnyInRange(array, Create(target), Create(target))
                    );
                    Assert.Equal(
                        offset,
                        LastIndexOfAnyInRange(array, Create(target - 1), Create(target))
                    );
                    Assert.Equal(
                        offset,
                        LastIndexOfAnyInRange(array, Create(target), Create(target + 1))
                    );
                    Assert.Equal(
                        offset,
                        LastIndexOfAnyInRange(array, Create(target - 1), Create(target + 1))
                    );
                }
            }
        }
        ...
        public void MatrixOfLengthsAndOffsets_AnyExcept_FindsValueAtExpectedIndex()
        {
            foreach (int length in Enumerable.Range(1, 40))
            {
                for (int offset = 0; offset < length; offset++)
                {
                    ...
                    Assert.Equal(
                        offset,
                        IndexOfAnyExceptInRange(array, Create(target), Create(target))
                    );
                    Assert.Equal(
                        offset,
                        IndexOfAnyExceptInRange(array, Create(target - 1), Create(target))
                    );
                    Assert.Equal(
                        offset,
                        IndexOfAnyExceptInRange(array, Create(target), Create(target + 1))
                    );
                    Assert.Equal(
                        offset,
                        IndexOfAnyExceptInRange(array, Create(target - 1), Create(target + 1))
                    );

                    Assert.Equal(
                        offset,
                        LastIndexOfAnyExceptInRange(array, Create(target), Create(target))
                    );
                    Assert.Equal(
                        offset,
                        LastIndexOfAnyExceptInRange(array, Create(target - 1), Create(target))
                    );
                    Assert.Equal(
                        offset,
                        LastIndexOfAnyExceptInRange(array, Create(target), Create(target + 1))
                    );
                    Assert.Equal(
                        offset,
                        LastIndexOfAnyExceptInRange(array, Create(target - 1), Create(target + 1))
                    );
                }
            }
        }
        ...
        protected static int IndexOfAnyInRange(Span<T> span, T lowInclusive, T highInclusive)
        {
            ...
            Assert.Equal(
                result,
                MemoryExtensions.IndexOfAnyInRange(
                    (ReadOnlySpan<T>)span,
                    lowInclusive,
                    highInclusive
                )
            ...
            Assert.Equal(
                result >= 0,
                MemoryExtensions.ContainsAnyInRange(
                    (ReadOnlySpan<T>)span,
                    lowInclusive,
                    highInclusive
                )
        ...
        {
            int result = MemoryExtensions.IndexOfAnyExceptInRange(
                span,
                lowInclusive,
                highInclusive
            );
            ...
            Assert.Equal(
                result >= 0,
                MemoryExtensions.ContainsAnyExceptInRange(
                    (ReadOnlySpan<T>)span,
                    lowInclusive,
                    highInclusive
                )
        ...
        protected static int LastIndexOfAnyExceptInRange(
            Span<T> span,
            T lowInclusive,
            T highInclusive
        )
        {
            int result = MemoryExtensions.LastIndexOfAnyExceptInRange(
                span,
                lowInclusive,
                highInclusive
            );
