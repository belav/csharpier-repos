...
{
    partial public static class IndexerTests
    {
        [Fact]
        public static void IndexerWithIndexTest()
        {
            ...
            Assert.Equal(
                span[new Index(2, fromEnd: false)],
            ...
            Assert.Equal(
                span1[new Index(2, fromEnd: false)],
        ...
        {
            ReadOnlySpan<char> span = "Hello".AsSpan();
            ReadOnlySpan<char> sliced = span[
                new Range(new Index(1, fromEnd: false), new Index(1, fromEnd: true))
            ];
            ...
            Span<char> sliced1 = span1[
                new Range(new Index(2, fromEnd: false), new Index(1, fromEnd: true))
            ];
        ...
        public static void SlicingUsingIndexAndRangeTest()
        {
            ...
            Assert.Throws<ArgumentOutOfRangeException>(
                delegate()
                {
                    var spp = new Span<char>(s.ToCharArray())[range];
                }
            );
            Assert.Throws<ArgumentOutOfRangeException>(
                delegate()
                {
                    var spp = s.AsSpan()[range];
                }
            );
        }
    }
