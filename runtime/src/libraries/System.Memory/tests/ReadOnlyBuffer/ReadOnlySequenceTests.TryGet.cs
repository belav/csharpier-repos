...
namespace System.Memory.Tests
{
    public class ReadOnlySequenceTryGetTests
    {
        ...
        public void Ctor_Array_Offset()
        {
            ...
            Assert.False(
                SequenceMarshal.TryGetReadOnlySequenceSegment(buffer, out _, out _, out _, out _)
            );

            // Array can be retrieved with TryGetReadOnlyMemory
            Assert.True(
                SequenceMarshal.TryGetReadOnlyMemory(buffer, out ReadOnlyMemory<byte> newMemory)
            );
        ...
        public void Ctor_Memory()
        {
            ...
            Assert.False(
                SequenceMarshal.TryGetReadOnlySequenceSegment(
                    buffer,
                    out ReadOnlySequenceSegment<byte> startSegment,
                    out int startIndex,
                    out ReadOnlySequenceSegment<byte> endSegment,
                    out int endIndex
                )
            );
        ...
        public void Ctor_Memory_String()
        {
            ...
            Assert.True(
                SequenceMarshal.TryGetReadOnlyMemory(buffer, out ReadOnlyMemory<char> newMemory)
            );
            ...
            Assert.False(
                SequenceMarshal.TryGetReadOnlySequenceSegment(
                    buffer,
                    out ReadOnlySequenceSegment<char> startSegment,
                    out int startIndex,
                    out ReadOnlySequenceSegment<char> endSegment,
                    out int endIndex
                )
            );
