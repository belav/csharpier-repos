    ...
    public static partial class ReadOnlySpanTests
    {
        ...
        public static void OnSequenceEqualOfEqualSpansMakeSureEveryElementIsCompared(int mode)
        {
            for (int length = 0; length < 100; length++)
            {
                ...
                // Make sure each element of the array was compared once. (Strictly speaking, it would not be
                // illegal for
                // SequenceEqual to compare an element more than once but that would be a non-optimal implementation
                // and
