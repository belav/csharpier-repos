...
namespace System.Buffers.Binary.Tests
{
    public class ReverseEndiannessUnitTests
    {
        ...
        public void ReverseEndianness_Int128AndUInt128(ulong aLower, ulong aUpper)
        {
            ...
            Int128 reversed = new Int128(
                BinaryPrimitives.ReverseEndianness(aUpper),
                BinaryPrimitives.ReverseEndianness(aLower)
            );
        ...
        public void ReverseEndianness_Span_AllElementsReversed<T>(T[] original)
            where T : struct, INumber<T>
        {
            ...
            foreach (int offset in new[] { 1, 2, 3 })
            {
                if (original.Length > offset)
                {
                    ...
                    expected = original
                        .AsSpan(0, original.Length - offset)
                        .ToArray()
                        .Select(ReverseEndianness)
                        .ToArray();
                    actual1 = (T[])original.Clone();
                    ReverseEndianness(
                        actual1.AsSpan(0, actual1.Length - offset),
                        actual1.AsSpan(offset)
                    );
                    ...
                    ReverseEndianness(
                        actual2.AsSpan(offset),
                        actual2.AsSpan(0, actual2.Length - offset)
                    );
        ...
        private static void ReverseEndianness<T>(ReadOnlySpan<T> source, Span<T> destination)
            where T : struct
        {
            if (typeof(T) == typeof(ushort))
            {
            ...
            {
                BinaryPrimitives.ReverseEndianness(
                    MemoryMarshal.Cast<T, short>(source),
            ...
            {
                BinaryPrimitives.ReverseEndianness(
                    MemoryMarshal.Cast<T, int>(source),
            ...
            {
                BinaryPrimitives.ReverseEndianness(
                    MemoryMarshal.Cast<T, long>(source),
                    MemoryMarshal.Cast<T, long>(destination)
                );
                return;
            }
            if (typeof(T) == typeof(nuint))
            {
                BinaryPrimitives.ReverseEndianness(
                    MemoryMarshal.Cast<T, nuint>(source),
                    MemoryMarshal.Cast<T, nuint>(destination)
                );
                return;
            }
            if (typeof(T) == typeof(nint))
            {
                BinaryPrimitives.ReverseEndianness(
                    MemoryMarshal.Cast<T, nint>(source),
                    MemoryMarshal.Cast<T, nint>(destination)
                );
                return;
            }
            if (typeof(T) == typeof(UInt128))
            {
                BinaryPrimitives.ReverseEndianness(
                    MemoryMarshal.Cast<T, UInt128>(source),
                    MemoryMarshal.Cast<T, UInt128>(destination)
                );
                return;
            }
            if (typeof(T) == typeof(Int128))
            {
                BinaryPrimitives.ReverseEndianness(
                    MemoryMarshal.Cast<T, Int128>(source),
                    MemoryMarshal.Cast<T, Int128>(destination)
                );
                return;
            }
            throw new Exception($"Unexpected type {typeof(T)}");
