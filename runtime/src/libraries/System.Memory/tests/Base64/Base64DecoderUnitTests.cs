...
namespace System.Buffers.Text.Tests
{
    public class Base64DecoderUnitTests : Base64TestBase
    {
        ...
        public void BasicDecoding()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                Span<byte> decodedBytes = new byte[
                    Base64.GetMaxDecodedFromUtf8Length(source.Length)
                ];
                Assert.Equal(
                ...
                Assert.True(
                    Base64TestHelper.VerifyDecodingCorrectness(
                        source.Length,
                        decodedBytes.Length,
                        source,
                        decodedBytes
        ...
        {
            ReadOnlySpan<byte> source = stackalloc byte[]
            {
                (byte)'A',
                (byte)'B',
                (byte)'C',
            ...
            for (int numBytes = 5; numBytes < 20; numBytes++)
            {
                ...
                if (numBytes >= 8)
                {
                    Assert.True(
                        ...
                        "Number of Input Bytes: " + numBytes
                    );
                }
                else
                {
        ...
        public void BasicDecodingWithFinalBlockTrueKnownInputInvalid(
            string inputString,
        ...
        public void BasicDecodingNonAsciiInputInvalid(
            string inputString,
            int expectedConsumed,
            int expectedWritten
        )
        ...
        [InlineData("AQIDBA", 4, 3)]
        [InlineData("AQIDBAU", 4, 3)]
        public void BasicDecodingWithFinalBlockFalseKnownInputNeedMoreData(
            string inputString,
            int expectedConsumed,
            int expectedWritten
        )
        ...
        [InlineData("AQIDBA==", 4, 3)]
        [InlineData("AQIDBAU=", 4, 3)]
        public void BasicDecodingWithFinalBlockFalseKnownInputInvalid(
            string inputString,
            int expectedConsumed,
            int expectedWritten
        )
        ...
        public void DecodingInvalidBytes(bool isFinalBlock)
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                ...
                for (int i = 0; i < invalidBytes.Length; i++)
                {
                    ...
                    if (
                        invalidBytes[i] == Base64TestHelper.EncodingPad
                    ...
                    else
                    {
                        ...
                        Assert.True(
                            Base64TestHelper.VerifyDecodingCorrectness(4, 3, source, decodedBytes)
                        );
                    }
                }
            }
        ...
        public void DecodingInvalidBytesPadding(bool isFinalBlock)
        {
            ...
            // The last byte or the last 2 bytes being the padding character is valid, if isFinalBlock = true
            {
                ...
                OperationStatus expectedStatus = isFinalBlock
                    ? OperationStatus.Done
                    : OperationStatus.InvalidData;
        ...
        public void GetMaxDecodedLength()
        {
            ...
            for (int i = 0; i < lengthsNotMultipleOfFour.Length; i++)
            {
                Assert.Equal(
                    expectedOutput[i],
                    Base64.GetMaxDecodedFromUtf8Length(lengthsNotMultipleOfFour[i])
                );
            }
            ...
            )
            {
                ...
                Assert.Equal(
                    ...
                    Base64.DecodeFromUtf8InPlace(
                        testBytes.Slice(0, numberOfBytesToTest),
        ...
        public void EncodeAndDecodeInPlace()
        {
            ...
            for (int value = 0; value < 256; value++)
            {
                ...
                Assert.Equal(
                    OperationStatus.Done,
                    Base64.EncodeToUtf8(sourceBytes, buffer, out int consumed, out int written)
                );
        ...
        public void BasicDecodingIgnoresCharsToBeIgnoredAsConvertToBase64Does(
            string utf8WithCharsToBeIgnored,
            byte[] expectedBytes
        )
        {
        ...
        [Theory]
        [MemberData(nameof(ValidBase64Strings_WithCharsThatMustBeIgnored))]
        public void DecodeInPlaceIgnoresCharsToBeIgnoredAsConvertToBase64Does(
            string utf8WithCharsToBeIgnored,
            byte[] expectedBytes
        )
        {
            Span<byte> utf8BytesWithByteToBeIgnored = UTF8Encoding.UTF8
                .GetBytes(utf8WithCharsToBeIgnored);
        ...
        [Theory]
        [MemberData(
            nameof(BasicDecodingWithExtraWhitespaceShouldBeCountedInConsumedBytes_MemberData)
        )]
        public void BasicDecodingWithExtraWhitespaceShouldBeCountedInConsumedBytes(
            string inputString,
            int expectedConsumed,
            int expectedWritten
        )
