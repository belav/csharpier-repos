    ...
    public class Base64ValidationUnitTests : Base64TestBase
    {
        ...
        public void BasicValidationChars()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                Span<byte> source = new byte[numBytes];
                Base64TestHelper.InitializeDecodableBytes(source, numBytes);
                Span<char> chars = source.ToArray().Select(Convert.ToChar).ToArray().AsSpan();
        ...
        public void ValidateBytesIgnoresCharsToBeIgnoredBytes(
            string utf8WithByteToBeIgnored,
            byte[] expectedBytes
        )
        {
        ...
        [Theory]
        [MemberData(nameof(ValidBase64Strings_WithCharsThatMustBeIgnored))]
        public void ValidateBytesIgnoresCharsToBeIgnoredChars(
            string utf8WithByteToBeIgnored,
            byte[] expectedBytes
        )
        {
        ...
        [InlineData("YQ= =", 1)]
        [InlineData("YQ== ", 1)]
        public void ValidateWithPaddingReturnsCorrectCountBytes(
            string utf8WithByteToBeIgnored,
            int expectedLength
        )
        {
        ...
        [InlineData("YQ= =", 1)]
        [InlineData("YQ== ", 1)]
        public void ValidateWithPaddingReturnsCorrectCountChars(
            string utf8WithByteToBeIgnored,
            int expectedLength
        )
        {
