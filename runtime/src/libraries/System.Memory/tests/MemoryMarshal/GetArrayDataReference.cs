...
namespace System.SpanTests
{
    public static partial class MemoryMarshalTests
    {
        ...
        public static void GetArrayDataReference_NonEmptyInput_ReturnsRefToFirstElement()
        {
            ...
            Assert.True(
                Unsafe.AreSame(ref szArray[0], ref MemoryMarshal.GetArrayDataReference(szArray))
            );
            Assert.True(
                Unsafe.AreSame(
                    ref szArray[0],
                    ref Unsafe.As<byte, int>(
                        ref MemoryMarshal.GetArrayDataReference((Array)szArray)
                    )
                )
            );
            ...
            Assert.True(
                Unsafe.AreSame(
                    ref mdArrayRank2[0, 0],
                    ref Unsafe.As<byte, int>(ref MemoryMarshal.GetArrayDataReference(mdArrayRank2))
                )
            );
            ...
            Assert.Equal(
                0x12345678,
                Unsafe.As<byte, int>(ref MemoryMarshal.GetArrayDataReference(mdArrayCustomBounds))
            );
        }
        ...
        public static unsafe void GetArrayDataReference_EmptyInput_ReturnsRefToWhereFirstElementWouldBe_SzArray()
        {
            ...
            ref int theMdArrayRef = ref Unsafe.As<byte, int>(
                ref MemoryMarshal.GetArrayDataReference((Array)theArray)
            ); // szarray passed to generalized Array helper
        ...
        )
        {
            ...
            IntPtr baselineOffset = Unsafe.ByteOffset(
                ref Unsafe.As<RawObject>(baselineArray).Data,
            ...
            IntPtr emptyArrayOffset = Unsafe.ByteOffset(
                ref Unsafe.As<RawObject>(emptyArray).Data,
                ref MemoryMarshal.GetArrayDataReference(emptyArray)
            );
