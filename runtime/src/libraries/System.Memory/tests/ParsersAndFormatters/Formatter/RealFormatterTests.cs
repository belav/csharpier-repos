    ...
    public class RealFormatterTests : RealFormatterTestsBase
    {
        ...
        protected override string InvariantToStringDouble(double d, string format)
        {
            ...
            bool formatSucceeded = Utf8Formatter.TryFormat(
                ...
                StandardFormat.Parse(format)
            );
            Assert.True(
                formatSucceeded,
                $"Failed to format '{d.ToString(CultureInfo.InvariantCulture)}' using '{format}'."
            );
        ...
        protected override string InvariantToStringSingle(float f, string format)
        {
            ...
            Assert.True(
                formatSucceeded,
                $"Failed to format '{f.ToString(CultureInfo.InvariantCulture)}' using '{format}'."
            );
            return s.Slice(0, bytesWritten).ToUtf16String();
        }
    }
