    ...
    public static partial class FormatterTests
    {
        ...
        private static void ValidateFormatterHelper<T>(FormatterTestData<T> testData)
        {
            ...
            if (success)
            {
                if (!expectedSuccess)
                {
                    if (bytesWritten >= 0 && bytesWritten <= span.Length)
                    {
                        ...
                        throw new TestException(
                            $"This format attempt ({testData}) was expected to fail due to an undersized buffer. Instead, it generated \"{unexpectedOutput}\""
                        );
                    }
                    else
                    {
                        throw new TestException(
                            $"This format attempt ({testData}) was expected to fail due to an undersized buffer. Instead, it succeeded but set 'bytesWritten' to an out of range value: {bytesWritten}"
                        );
                    }
                }
                ...
                {
                    throw new TestException(
                        $"This format attempt ({testData}) succeeded as expected but set 'bytesWritten' to an out of range value: {bytesWritten}"
                    );
                }
                ...
                CanaryCheck(
                    ...
                    new ReadOnlySpan<byte>(
                        ...
                        span.Length - expectedOutputLength
                    ),
                    "'bytesWritten'"
                );
            }
            else
            {
                ...
                {
                    throw new TestException(
                        $"This format attempt ({testData}) was expected to succeed. Instead, it failed."
                    );
                }
                ...
                {
                    throw new TestException(
                        $"This format attempt ({testData}) failed as expected but did not set `bytesWritten` to 0"
                    );
                }
        ...
        private static void CanaryCheck<T>(
        ...
        )
        {
            for (int i = 0; i < canaries.Length; i++)
            {
                if (canaries[i] != 0xcc)
                    throw new TestException(
                        $"BUFFER OVERRUN! This format attempt ({testData}) wrote past {pastWhat}!"
                    );
            }
        }

        private static bool TryFormatUtf8<T>(
            T value,
            Span<byte> buffer,
        ...
        private static bool TryFormatUtf8(
            object value,
            Span<byte> buffer,
            out int bytesWritten,
            StandardFormat format = default
