    ...
    internal static partial class TestData
    {
                ...
                foreach (ParserTestData<DateTimeOffset> testData in DateTimeOffsetParserTestData)
                {
                    ...
                    if (testData.ExpectedSuccess)
                    {
                        if (formatSymbol == 'R' || formatSymbol == 'l')
                        {
                            ...
                            yield return new ParserTestData<DateTime>(
                                testData.Text,
                                expectedDateTime,
        ...
        public static IEnumerable<ParserTestData<DateTimeOffset>> DateTimeOffsetParserTestData
        {
            get
            {
                ...
                foreach (
                    ParserTestData<DateTimeOffset> bad in GenerateCorruptedDateTimeText(
                ...
                foreach (PseudoDateTime pseudoDateTime in PseudoDateTimeTestData)
                {
                    ...
                    if (pseudoDateTime.ExpectSuccess)
                    {
                        ...
                        {
                            throw new Exception(
                                $"Failed on converting {expectedDto.DateTime} to local time. This is probably a piece of data that fails only in certain time zones. Time zone on this machine is {TimeZoneInfo.Local}"
                            );
                        }
                    }
                    else
        ...
        )
        {
            ...
            // Too short
            {
                ...
                )
                {
                    ...
                    {
                        yield return new ParserTestData<DateTimeOffset>(
                            goodText.Substring(0, truncatedLength),
                            default,
                            formatSymbol,
                            expectedSuccess: false
                        );
