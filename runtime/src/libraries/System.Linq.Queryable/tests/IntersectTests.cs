...
namespace System.Linq.Tests
{
    public class IntersectTests : EnumerableBasedTests
    {
        ...
        public void SingleNullWithEmpty()
        {
            ...
            Assert.Empty(
                first.AsQueryable()
                    .Intersect(second.AsQueryable(), EqualityComparer<string>.Default)
            );
        }
        ...
        public void CustomComparer()
        {
            ...
            Assert.Equal(
                expected,
                first.AsQueryable().Intersect(second.AsQueryable(), new AnagramEqualityComparer())
            );
        }
        ...
        public void Intersect1()
        {
            var count = (new int[] { 0, 1, 2 })
                .AsQueryable()
                .Intersect((new int[] { 1, 2, 3 }).AsQueryable())
                .Count();
        ...
        public void Intersect2()
        {
            var count = (new int[] { 0, 1, 2 })
                .AsQueryable()
                .Intersect((new int[] { 1, 2, 3 }).AsQueryable(), EqualityComparer<int>.Default)
                .Count();
            Assert.Equal(2, count);
        }
        ...
        public void IntersectBy()
        {
            ...
            var actual = Enumerable.Range(0, 10)
                .AsQueryable()
                .IntersectBy(Enumerable.Range(5, 20), x => x)
                .ToArray();
        ...
        public void IntersectBy_CustomComparison()
        {
            ...
            var actual = Enumerable.Range(0, 10)
                .AsQueryable()
                .IntersectBy(Enumerable.Range(5, 20), x => x, EqualityComparer<int>.Default)
                .ToArray();
            Assert.Equal(expected, actual);
        }
    }
