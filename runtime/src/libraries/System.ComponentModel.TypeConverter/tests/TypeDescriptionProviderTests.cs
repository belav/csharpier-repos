...
namespace System.ComponentModel.Tests
{
    public class TypeDescriptionProviderTests
    {
        ...
        public void CreateInstance_InvokeWithParent_ReturnsExpected(
            IServiceProvider serviceProvider,
            Type objectType,
            Type[] argTypes,
            object[] args,
        ...
        )
        {
            ...
            Assert.Same(
                result,
                provider.CreateInstance(serviceProvider, objectType, argTypes, args)
            );
            mockParentProvider.Verify(
        ...
        {
            var mockParentProvider = new Mock<TypeDescriptionProvider>(MockBehavior.Strict);
            mockParentProvider.Setup(p => p.GetCache(instance)).Returns(result).Verifiable();
            var provider = new SubTypeDescriptionProvider(mockParentProvider.Object);
        ...
        public void GetExtendedTypeDescriptor_InvokeWithoutParent_ReturnsExpected(object instance)
        {
            ...
            CustomTypeDescriptor result1 = Assert.IsAssignableFrom<CustomTypeDescriptor>(
                provider.GetExtendedTypeDescriptor(instance)
            );
            Assert.Empty(result1.GetProperties());

            // Call again.
            CustomTypeDescriptor result2 = Assert.IsAssignableFrom<CustomTypeDescriptor>(
                provider.GetExtendedTypeDescriptor(instance)
            );
        ...
        {
            foreach (
                ICustomTypeDescriptor result in new ICustomTypeDescriptor[]
                {
                    null,
        ...
        {
            foreach (
                IExtenderProvider[] result in new IExtenderProvider[][]
                {
                    null,
                    new IExtenderProvider[]
                    {
        ...
        )
        {
            ...
            mockParentProvider
                .Protected()
                .Verify("GetExtenderProviders", Times.Once(), instance ?? ItExpr.IsNull<object>());
        ...
        )
        {
            ...
            mockProvider
                ...
                .Verifiable();
            mockProvider.Setup(p => p.GetFullComponentName(component)).CallBase();
            TypeDescriptionProvider provider = mockProvider.Object;
            Assert.Same(result, provider.GetFullComponentName(component));
            mockProvider.Verify(
                p => p.GetTypeDescriptor(component.GetType(), component),
                Times.Once()
            );
        ...
        )
        {
            ...
            Assert.Same(expected, provider.GetReflectionType(instance));
            mockProvider.Verify(
                p => p.GetReflectionType(instance.GetType(), instance),
                Times.Once()
            );
        ...
        )
        {
            ...
            Assert.Same(result, provider.GetReflectionType(objectType, instance));
            mockParentProvider.Verify(
                p => p.GetReflectionType(objectType, instance),
                Times.Exactly(2)
            );
        }
        ...
        public void GetTypeDescriptor_InvokeTypeWithoutParent_ReturnsExpected(Type objectType)
        {
            var provider = new SubTypeDescriptionProvider();
            CustomTypeDescriptor result1 = Assert.IsAssignableFrom<CustomTypeDescriptor>(
                provider.GetTypeDescriptor(objectType)
            );
            Assert.Empty(result1.GetProperties());
        ...
        public void GetTypeDescriptor_InvokeObjectWithoutParent_ReturnsExpected(object instance)
        {
            var provider = new SubTypeDescriptionProvider();
            CustomTypeDescriptor result1 = Assert.IsAssignableFrom<CustomTypeDescriptor>(
                provider.GetTypeDescriptor(instance)
            );
            Assert.Empty(result1.GetProperties());
        ...
        )
        {
            ...
            Assert.Empty(result1.GetProperties());
            mockProvider.Verify(
                p => p.GetTypeDescriptor(instance.GetType(), instance),
                Times.Once()
            );
        ...
        public void GetTypeDescriptor_InvokeTypeObjectWithParent_ReturnsExpected(
            Type objectType,
            object instance,
            ICustomTypeDescriptor result
        )
        {
            ...
            Assert.Same(result, provider.GetTypeDescriptor(objectType, instance));
            mockParentProvider.Verify(
                p => p.GetTypeDescriptor(objectType, instance),
                Times.Exactly(2)
            );
        }
        ...
        {
            var mockParentProvider = new Mock<TypeDescriptionProvider>(MockBehavior.Strict);
            mockParentProvider.Setup(p => p.IsSupportedType(type)).Returns(result).Verifiable();
            var provider = new SubTypeDescriptionProvider(mockParentProvider.Object);
