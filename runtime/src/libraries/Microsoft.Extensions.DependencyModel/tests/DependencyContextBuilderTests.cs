...
namespace Microsoft.Extensions.DependencyModel.Tests
{
    public class DependencyContextBuilderTests
    {
        ...
        private NuGetVersion _defaultVersion = new NuGetVersion(
            1,
            2,
            3,
            new[] { "dev" },
        ...
        public DependencyContext Build(
            CommonCompilerOptions compilerOptions = null,
        ...
        {
            var context = Build(
                target: new NuGetFramework("SomeFramework", new Version(1, 2)),
                runtime: "win8-x86"
            );
        ...
        {
            var context = Build(
                runtimeExports: new[] { Export(PackageDescription("Pack.Age", servicable: false)) }
            );
        ...
        {
            var context = Build(
                runtimeExports: new[] { Export(PackageDescription("Pack.Age", servicable: true)) }
            );
        ...
        public void FillsRuntimeLibraryProperties()
        {
            var context = Build(
                runtimeExports: new[]
                {
                    Export(
                        ...
                        {
                            new LibraryAssetGroup(new LibraryAsset("Dll", "lib/Pack.Age.dll", "")),
            ...
            lib
                .RuntimeAssemblyGroups.GetRuntimeAssets("win8-x64")
                .Should()
                .OnlyContain(l => l == "win8-x64/Pack.Age.dll");
            lib
                .NativeLibraryGroups.GetRuntimeAssets("win8-x64")
                .Should()
        ...
        public void FillsResources()
        {
            var context = Build(
                ...
                {
                    Export(
                        PackageDescription("Pack.Age", version: new NuGetVersion(1, 2, 3)),
                        resourceAssemblies: new[]
                        {
                            new LibraryResourceAssembly(
                                new LibraryAsset("Dll", "resources/en-US/Pack.Age.dll", ""),
                                "en-US"
                            )
                        }
                    )
