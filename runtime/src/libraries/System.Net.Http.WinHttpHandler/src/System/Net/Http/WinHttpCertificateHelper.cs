...
namespace System.Net.Http
{
    internal static class WinHttpCertificateHelper
    {
        ...
        )
        {
            ...
            // Build the chain.
            chain = new X509Chain();
            chain.ChainPolicy.RevocationMode = checkCertificateRevocationList
                ...
                if (NetEventSource.Log.IsEnabled())
                {
                    foreach (X509Certificate cert in remoteCertificateStore)
                    {
                        NetEventSource.Info(
                            ...
                            $"Adding cert to ExtraStore: {cert.Subject}"
                        );
                    }
                }
                ...
                fixed (char* namePtr = hostName)
                {
                        ...
                        )
                        {
                            if (NetEventSource.Log.IsEnabled())
                                NetEventSource.Error(
                                    certificate,
                                    nameof(Interop.Crypt32.CertChainPolicyErrors.CERT_E_CN_NO_MATCH)
                                );
                    ...
                    else
                    {
                        ...
                        if (NetEventSource.Log.IsEnabled())
                            NetEventSource.Error(
                                certificate,
                                $"Failure calling {nameof(Interop.Crypt32.CertVerifyCertificateChainPolicy)}"
                            );
                        sslPolicyErrors |= SslPolicyErrors.RemoteCertificateNameMismatch;
                    }
                }
