...
namespace System.ServiceModel.Syndication.Tests
{
    public static partial class BasicScenarioTests
    {
        ...
        public static void SyndicationFeed_Rss_UriParser()
        {
            ...
            using (XmlReader reader = XmlReader.Create("TestFeeds/RssSpecCustomParser.xml"))
            {
                var formatter = new Rss20FeedFormatter
                {
                    ...
                    {
                        uri = new Uri(
                            $"http://value-{xmlUriData.UriString}-kind-{xmlUriData.UriKind}-localName-{xmlUriData.ElementQualifiedName.Name}-ns-{xmlUriData.ElementQualifiedName.Namespace}-end"
                        );
                        return true;
                    },
                };
            ...
            Assert.Equal(
                new Uri(
                    "http://value-ChannelBase-kind-relativeorabsolute-localName-channel-ns--end"
                ),
                feed.BaseUri
            );
            Assert.Equal(
                new Uri("http://value-ImageUrl-kind-relativeorabsolute-localName-url-ns--end"),
                feed.ImageUrl
            );
            Assert.NotNull(feed.Links);
            Assert.Equal(1, feed.Links.Count);
            Assert.Equal(
                new Uri("http://value-FeedLink-kind-relativeorabsolute-localName-link-ns--end"),
                feed.Links.First().Uri
            );
            ...
            Assert.Equal(
                new Uri("http://value-itemlink-kind-relativeorabsolute-localName-link-ns--end"),
                feed.Items.First().Links.First().Uri
            );
        }
        ...
        public static void SyndicationFeed_Atom_UriParser()
        {
            ...
            Assert.Equal(
                new Uri(
                    "http://value-FeedLogo-kind-relativeorabsolute-localName-logo-ns-http//www.w3.org/2005/Atom-end"
                ),
                feed.ImageUrl
            );
            ...
            Assert.Equal(
                new Uri(
                    "http://value-EntryLinkHref-kind-relativeorabsolute-localName-link-ns-http//www.w3.org/2005/Atom-end"
                ),
                feed.Items.First().Links.First().Uri
            );
            Assert.Equal(
                new Uri(
                    "http://value-EntryContentSrc-kind-relativeorabsolute-localName-content-ns-http://www.w3.org/2005/Atom-end"
                ),
                ((UrlSyndicationContent)feed.Items.First().Content).Url
            );
        }
        ...
        public static void SyndicationFeed_RSS_Optional_Elements()
        {
            using (XmlReader reader = XmlReader.Create("TestFeeds/rssSpecExample.xml"))
            {
                ...
                Assert.Equal(
                    "http://contoso.com/rss",
                    feed.Documentation.GetAbsoluteUri().ToString()
                );
        ...
        {
            List<AllowableDifference> allowableDifferences =
                GetRssFeedPositiveTestAllowableDifferences();
        ...
        public static void SyndicationFeed_Load_Write_RSS_Use_Optional_Element_Properties()
        {
            ...
            ReadWriteSyndicationFeed(
                ...
                verifySyndicationFeedRead: (feed) =>
                {
                    ...
                    Assert.NotNull(feed.Documentation);
                    Assert.True(
                        feed.Documentation.GetAbsoluteUri().ToString() == "http://contoso.com/rss"
                    );
                    ...
                    Assert.Equal("input Name", feed.TextInput.Name);
                    Assert.Equal(
                        "http://www.contoso.no/search?",
                        feed.TextInput.Link.Uri.ToString()
                    );
                }
            );
        }
