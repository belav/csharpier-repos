    ...
    public static class ExecutionContextTests
    {
        ...
        public static void FlowTest()
        {
            ThreadTestHelpers.RunTestInBackgroundThread(() =>
            {
                ...
                // Changing an async local value does not prevent undoing a flow-suppressed execution context. In
                // .NET Core, the
                // execution context is immutable, so changing an async local value changes the execution context
                // instance,
                ...
                // An async flow control cannot be undone when a different execution context is applied. The .NET
                // Framework
                // mutates the execution context when its state changes, and only changes the instance when an
                // execution context
                // is applied (for instance, through ExecutionContext.Run). The framework prevents a suppressed-flow
                // execution
                // context from being applied by returning null from ExecutionContext.Capture, so the only type of
                // execution
                // context that can be applied is one whose flow is not suppressed. After suppressing flow and
                // changing an async
                // local's value, the .NET Framework verifies that a different execution context has not been
                // applied by
                // checking the execution context instance against the one saved from when flow was suppressed. In
                // .NET Core,
                // since the execution context instance will change after changing the async local's value, it
                // verifies that a
                // different execution context has not been applied, by instead ensuring that the current execution
                // context's
                // flow is suppressed.
                {
