...
namespace Castle.DynamicProxy.Tests
{
    ...
    public class ConstructorWithAttributesOnParametersTestCase : BasePEVerifyTestCase
    {
        ...
        {
            var requiredObj = generator.CreateClassProxy(
                typeof(ClassWithAttributesOnConstructorParameters),
                new object[] { 10, "" }
            );
            var constructor = requiredObj.GetType().GetConstructors().First();

            // There should be exactly two parameters with a `RequiredAttribute`:
            var paramWithRequiredAttributeCount = constructor.GetParameters()
                .Where(p => p.IsDefined(typeof(RequiredAttribute), true))
                .Count();
            ...
            var intParamRequiredAttrs = intParam.GetCustomAttributes<RequiredAttribute>(
                inherit: true
            );
            ...
            var stringParam = constructor.GetParameters()
                .SingleOrDefault(p => p.Name == "stringParam");
            Assert.NotNull(stringParam);

            // That parameter should have exactly one `RequiredAttribute` carrying the value "test".
            var stringParamRequiredAttrs = stringParam.GetCustomAttributes<RequiredAttribute>(
                inherit: true
            );
