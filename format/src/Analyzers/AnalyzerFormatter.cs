...
namespace Microsoft.CodeAnalysis.Tools.Analyzers
{
    ...
    {
        public static AnalyzerFormatter CodeStyleFormatter =>
            new AnalyzerFormatter(
        ...
        public static AnalyzerFormatter ThirdPartyFormatter =>
            new AnalyzerFormatter(
        ...
        )
        {
            ...
            var formattablePaths = await GetFormattablePathsAsync(
                    solution,
            ...
            var projectAnalyzers = await FilterAnalyzersAsync(
                    solution,
                    projectAnalyzersAndFixers,
                    formattablePaths,
                    severity,
                    formatOptions.Diagnostics,
                    formatOptions.ExcludeDiagnostics,
                    cancellationToken
                )
                .ConfigureAwait(false);
            ...
            if (formatOptions.SaveFormattedFiles)
            {
                ...
                solution = await FixDiagnosticsAsync(
                        solution,
                        projectAnalyzers,
                        allFixers,
                        projectDiagnostics,
            ...
            logger.LogTrace(
                Resources.Analysis_complete_in_0ms_,
        ...
        private async Task<
            ImmutableDictionary<ProjectId, ImmutableHashSet<string>>
        ...
        )
        {
            ...
            LogDiagnosticLocations(
                solution,
                result.Diagnostics.SelectMany(kvp => kvp.Value),
                options.SaveFormattedFiles,
                options.ChangesAreErrors,
                logger,
                options.LogLevel,
                formattedFiles
            );
        ...
        )
        {
            ...
            var reportedDiagnostics = projectDiagnostics
            ...
            foreach (var diagnosticId in reportedDiagnostics)
            {
                ...
                {
                    logger.LogWarning(
                        Resources.Unable_to_fix_0_No_associated_code_fix_found,
                        diagnosticId
                    );
                    continue;
                }
            ...
            )
            {
                return diagnosticIds.ToImmutableDictionary(
                    id => id,
                    id => fixers.Where(fixer => ContainsFixableId(fixer, id)).ToImmutableArray()
                );
            }
            ...
            static bool ContainsFixableId(CodeFixProvider fixer, string id)
            {
                ...
                if (
                    id == "IDE0005"
                    && fixer.FixableDiagnosticIds.Contains("RemoveUnnecessaryImportsFixable")
                )
                {
                    return true;
                }
        ...
        internal static async Task<
            ImmutableDictionary<ProjectId, ImmutableArray<DiagnosticAnalyzer>>
        > FilterAnalyzersAsync(
            Solution solution,
        ...
        )
        {
            ...
            foreach (var projectId in projectAnalyzersAndFixers.Keys)
            {
                ...
                foreach (var analyzer in filteredAnalyzer)
                {
                    ...
                    var severity = await analyzer
                        .GetSeverityAsync(project, formattablePaths, cancellationToken)
                        .ConfigureAwait(false);
