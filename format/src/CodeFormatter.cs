...
namespace Microsoft.CodeAnalysis.Tools
{
    internal static class CodeFormatter
    {
        ...
        )
        {
            ...
            if (formatOptions.LogLevel <= LogLevel.Debug)
            {
                foreach (var project in workspace.CurrentSolution.Projects)
                {
                    ...
                    {
                        logger.LogDebug(
                            Resources.Project_0_is_using_configuration_from_1,
                            project.Name,
                            configDocument.FilePath
                        );
                    }
                }
            }
            ...
            var projectPath =
                formatOptions.WorkspaceType == WorkspaceType.Project
                    ? formatOptions.WorkspaceFilePath
                    : string.Empty;
            ...
            var documentIdsWithErrors = formattedFiles
                .Select(file => file.DocumentId)
                .Distinct()
                .ToImmutableArray();
        ...
        )
        {
            ...
            return await MSBuildWorkspaceLoader.LoadAsync(
        ...
        )
        {
            ...
            foreach (var project in solution.Projects)
            {
                ...
                foreach (var document in project.Documents)
                {
                    ...
                    if (
                        await GeneratedCodeUtilities
                            .IsGeneratedCodeAsync(syntaxTree, cancellationToken)
                            .ConfigureAwait(false)
                    )
                    {
            ...
            return (
                projectFileCount + sourceGeneratedDocuments.Count,
                formattableDocuments.ToImmutable()
            );
        }
    }
}
