...
partial public abstract class ManyToManyLoadTestBase<TFixture> : IClassFixture<TFixture>
    where TFixture : ManyToManyLoadTestBase<TFixture>.ManyToManyLoadFixtureBase
{
    ...
    )
    {
        ...
        Assert.Equal(
            state == EntityState.Detached ? 0 : 1 + 7 + 7,
    ...
    public virtual async Task Load_collection_using_Query(EntityState state, bool async)
    {
        ...
        Assert.Equal(
            state == EntityState.Detached ? 0 : 1 + 3 + 3,
            context.ChangeTracker.Entries().Count()
        );
    ...
    public virtual async Task Load_collection_already_loaded(EntityState state, bool async)
    {
        ...
        Assert.Equal(
            state == EntityState.Detached ? 0 : 1 + 4 + 4,
            context.ChangeTracker.Entries().Count()
        );
    }
    ...
    public virtual async Task Load_collection_partially_loaded(
        EntityState state,
    ...
    )
    {
        ...
        var left = context
            .Set<EntityOne>()
            .Include(e => e.ThreeSkipPayloadFull.OrderBy(e => e.Id).Take(1))
            .Single(e => e.Id == 3);
        ...
        else
        {
            if (async)
            {
                await (
                    forceIdentityResolution
                        ? collectionEntry.LoadWithIdentityResolutionAsync()
                        : collectionEntry.LoadAsync()
                );
            }
            else
            {
            ...
            Assert.Equal(
                state == EntityState.Detached && !forceIdentityResolution ? 5 : 4,
                left.ThreeSkipPayloadFull.Count
            );
            foreach (var right in left.ThreeSkipPayloadFull)
            {
                Assert.Contains(left, right.OneSkipPayloadFull);
    ...
    [InlineData(EntityState.Detached, true, true)]
    [InlineData(EntityState.Detached, true, false)]
    public virtual async Task Load_collection_partially_loaded_no_explicit_join(
        EntityState state,
        bool forceIdentityResolution,
        bool async
    )
    {
        ...
        var left = context
            .Set<EntityOne>()
            .Include(e => e.TwoSkip.OrderBy(e => e.Id).Take(1))
            .Single(e => e.Id == 3);
        ...
        else
        {
            ...
            Assert.Equal(
                state == EntityState.Detached && !forceIdentityResolution ? 8 : 7,
                left.TwoSkip.Count
            );
            foreach (var right in left.TwoSkip)
            {
    ...
    public virtual void Load_collection_partially_loaded_no_tracking(
        QueryTrackingBehavior queryTrackingBehavior
    )
    {
        using var context = Fixture.CreateContext();
        ...
        {
            Assert.Equal(
                queryTrackingBehavior == QueryTrackingBehavior.NoTracking ? 5 : 4,
                left.ThreeSkipPayloadFull.Count
            );
        }
        else
    ...
    public virtual async Task Load_collection_composite_key(EntityState state, bool async)
    {
        ...
        Assert.Equal(
            state == EntityState.Detached ? 0 : 1 + 2 + 2,
            context.ChangeTracker.Entries().Count()
        );
    }
    ...
    {
        protected override string StoreName => "ManyToManyLoadTest";
    }
}
