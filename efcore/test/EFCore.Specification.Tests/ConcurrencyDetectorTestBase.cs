        ...
        where TFixture : ConcurrencyDetectorTestBase<TFixture>.ConcurrencyDetectorFixtureBase, new()
    {
        ...
        public virtual Task Find(bool async) =>
            ConcurrencyDetectorTest(
                async c => async ? await c.Products.FindAsync(1) : c.Products.Find(1)
            );
        ...
        public virtual Task Count(bool async) =>
            ConcurrencyDetectorTest(
                async c => async ? await c.Products.CountAsync() : c.Products.Count()
            );
        ...
        public virtual Task ToList(bool async) =>
            ConcurrencyDetectorTest(
                async c => async ? await c.Products.ToListAsync() : c.Products.ToList()
            );
        ...
        {
            public ConcurrencyDetectorDbContext(
                DbContextOptions<ConcurrencyDetectorDbContext> options
            ) : base(options) { }
