...
{
    protected NorthwindSelectQueryTestBase(TFixture fixture) : base(fixture) { }
    ...
    public virtual Task Projection_when_arithmetic_expression_precedence(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Order>()
                    .Select(
                        o => new { A = o.OrderID / (o.OrderID / 2), B = o.OrderID / o.OrderID / 2 }
                    ),
            e => (e.A, e.B)
        );
    ...
    public virtual Task Select_customer_table(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>(), entryCount: 91);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_customer_identity(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => c), entryCount: 91);
    ...
    public virtual Task SelectMany_without_result_selector_naked_collection_navigation(
        bool async
    ) => AssertQuery(async, ss => ss.Set<Customer>().SelectMany(c => c.Orders), entryCount: 830);
                                ...
                                Orders = o.Orders
                                    ...
                                    .Select(
                                        e =>
                                            new
                                            {
                                                Title = e.CustomerID == e.Customer.CustomerID
                                                    ? "A"
                                                    : "B"
                                            }
                                    )
        ...
        AssertSingle(
            ...
            ss =>
                ss.Set<Customer>()
                    ...
                    .Select(
                        c =>
                            new CustomerDetailsWithCount(
                                c.CustomerID,
                                c.City,
                                c.Orders
                                    .Select(o => new OrderInfo(o.OrderID, o.OrderDate))
                                    .ToList(),
                                c.Orders.Count
                            )
                    ),
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(c => c.CustomerID.StartsWith("F"))
                    .Select(
                        c => new { c.CustomerID, Orders = c.Orders.Select(o => new OrderDto()) }
                    ),
            elementSorter: e => e.CustomerID,
            elementAsserter: (e, a) =>
            {
                ...
                ss.Set<Customer>()
                    ...
                    .Select(
                        c =>
                            ...
                            {
                                c.CustomerID,
                                ListWithSubList = c.Orders
                                    .OrderBy(e => e.OrderID)
                                    .Select(
                                        o =>
                                            o.OrderDetails.Select(
