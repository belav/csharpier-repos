    ...
    where TFixture : NorthwindQueryFixtureBase<NoopModelCustomizer>, new()
{
    ...
    public virtual Task Projection_when_null_value(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => c.Region));
    ...
    public virtual Task Projection_when_client_evald_subquery(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Select(c => string.Join(", ", c.Orders.Select(o => o.CustomerID).ToList()))
        );
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Employee>()
                    ...
                    .Select(e =>
                        new object[] { e.EmployeeID, e.ReportsTo, EF.Property<string>(e, "Title") }
                    ),
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual async Task Select_bool_closure_with_order_parameter_with_cast_to_nullable(
        bool async
    )
    {
        var boolean = false;
    ...
    public virtual Task Select_scalar(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => c.City));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_anonymous_one(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => new { c.City }), e => e.City);
    ...
    public virtual Task Select_anonymous_constant_in_expression(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Select(c => new { c.CustomerID, Expression = c.CustomerID.Length + 5 }),
            e => e.CustomerID
        );
    ...
    public virtual Task Select_anonymous_conditional_expression(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Product>()
                    .Select(p => new { p.ProductID, IsAvailable = p.UnitsInStock > 0 }),
            e => e.ProductID
        );
    ...
    public virtual Task Select_customer_table(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_customer_identity(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => c));
    ...
    public virtual Task Select_anonymous_empty(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => new { }), e => 1);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_anonymous_literal(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => new { X = 10 }), e => e.X);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_constant_int(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Customer>().Select(c => 0));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_constant_null_string(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => (string)null));
    ...
    {
        var x = 10;
        return AssertQueryScalar(async, ss => ss.Set<Customer>().Select(c => x));
    }

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_scalar_primitive(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Employee>().Select(e => e.EmployeeID));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_scalar_primitive_after_take(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Employee>().Take(9).Select(e => e.EmployeeID));
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    ...
                    {
                        OrderDates = c
                            .Orders.Where(o => o.OrderID < 10500)
                            .OrderBy(o => o.OrderID)
                            .Take(3)
                    ...
                    .Select(c => new
                    {
                        Order = (int?)
                            c
                                ...
                                .Where(o => o.OrderID < 10500)
                                .Select(o =>
                                    o.OrderDetails.OrderBy(od => od.OrderID)
                ...
                (
                    ...
                    .Select(x =>
                        (
                            (
                                from o1 in ss.Set<Order>()
                                where o1.CustomerID == x.City && o1.OrderDate.Value.Year == 1997
                                orderby o1.OrderID
                                select o1
                            )
                                .Distinct()
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_non_matching_value_types_int_to_long_introduces_explicit_cast(
        bool async
    ) =>
        AssertQueryScalar(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_non_matching_value_types_nullable_int_to_long_introduces_explicit_cast(
        bool async
    ) =>
        AssertQueryScalar(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_non_matching_value_types_nullable_int_to_int_doesnt_introduce_explicit_cast(
        bool async
    ) =>
        AssertQueryScalar(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_non_matching_value_types_int_to_nullable_int_doesnt_introduce_explicit_cast(
        bool async
    ) =>
        AssertQueryScalar(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_non_matching_value_types_from_binary_expression_introduces_explicit_cast(
        bool async
    ) =>
        AssertQueryScalar(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_non_matching_value_types_from_binary_expression_nested_introduces_top_level_explicit_cast(
        bool async
    ) =>
        AssertQueryScalar(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_non_matching_value_types_from_unary_expression_introduces_explicit_cast1(
        bool async
    ) =>
        AssertQueryScalar(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_non_matching_value_types_from_unary_expression_introduces_explicit_cast2(
        bool async
    ) =>
        AssertQueryScalar(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_non_matching_value_types_from_length_introduces_explicit_cast(
        bool async
    ) =>
        AssertQueryScalar(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_non_matching_value_types_from_method_call_introduces_explicit_cast(
        bool async
    ) =>
        AssertQueryScalar(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_non_matching_value_types_from_anonymous_type_introduces_explicit_cast(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    public virtual Task Select_over_10_nested_ternary_condition(bool isAsync) =>
        AssertQuery(isAsync, os => from c in os.Set<Customer>() select c.CustomerID == "1" ? "01"
            : c.CustomerID == "2" ? "02"
            : c.CustomerID == "3" ? "03"
            : c.CustomerID == "4" ? "04"
            : c.CustomerID == "5" ? "05"
    ...
    public virtual Task Reverse_without_explicit_ordering(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Employee>().Reverse().Select(e => e.EmployeeID));
    ...
    public virtual Task Reverse_in_join_inner(bool async) =>
        AssertQuery(
            async,
            ss =>
                from c in ss.Set<Customer>().OrderBy(x => x.CustomerID)
                join o in ss.Set<Order>().OrderByDescending(x => x.OrderDate).Reverse()
    ...
    public virtual Task Reverse_in_join_inner_with_skip(bool async) =>
        AssertQuery(
            ...
            ss =>
                from c in ss.Set<Customer>().OrderBy(x => x.CustomerID)
                join o in ss.Set<Order>().OrderByDescending(x => x.OrderID).Skip(2).Reverse()
                    on c.CustomerID equals o.CustomerID
                    into grouping
                from o in grouping.DefaultIfEmpty()
                select new { c.CustomerID, OrderID = (int?)o.OrderID },
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_single_element_from_collection_with_OrderBy_Skip_and_FirstOrDefault(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_single_element_from_collection_with_OrderBy_Distinct_and_FirstOrDefault(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_single_element_from_collection_with_OrderBy_Take_and_SingleOrDefault(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_single_element_from_collection_with_OrderBy_Take_and_FirstOrDefault_with_parameter(
        bool async
    )
    {
        var i = 1;
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_single_element_from_collection_with_multiple_OrderBys_Take_and_FirstOrDefault(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_single_element_from_collection_with_multiple_OrderBys_Take_and_FirstOrDefault_2(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_single_element_from_collection_with_OrderBy_over_navigation_Take_and_FirstOrDefault(
        bool async
    ) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Order>()
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_single_element_from_collection_with_OrderBy_over_navigation_Take_and_FirstOrDefault_2(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Order>()
                ...
                .Select(o =>
                    o.OrderDetails.OrderBy(od => od.Product.ProductName).Take(1).FirstOrDefault()
                )
        );
    ...
    public virtual Task Select_datetime_year_component(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Order>().Select(o => o.OrderDate.Value.Year));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_datetime_month_component(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Order>().Select(o => o.OrderDate.Value.Month));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_datetime_day_of_year_component(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Order>().Select(o => o.OrderDate.Value.DayOfYear));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_datetime_day_component(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Order>().Select(o => o.OrderDate.Value.Day));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_datetime_hour_component(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Order>().Select(o => o.OrderDate.Value.Hour));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_datetime_minute_component(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Order>().Select(o => o.OrderDate.Value.Minute));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_datetime_second_component(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Order>().Select(o => o.OrderDate.Value.Second));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_datetime_millisecond_component(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Order>().Select(o => o.OrderDate.Value.Millisecond));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_datetime_DayOfWeek_component(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Order>().Select(o => (int)o.OrderDate.Value.DayOfWeek)
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_datetime_Ticks_component(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Order>().Select(o => o.OrderDate.Value.Ticks));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_datetime_TimeOfDay_component(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Order>().Select(o => o.OrderDate.Value.TimeOfDay));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_byte_constant(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Customer>().Select(c => c.CustomerID == "ALFKI" ? (byte)1 : (byte)2)
        );
    ...
    public virtual Task Select_short_constant(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Customer>().Select(c => c.CustomerID == "ALFKI" ? (short)1 : (short)2)
        );
    ...
    public virtual Task Select_bool_constant(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Customer>().Select(c => c.CustomerID == "ALFKI" ? true : false)
        );
    ...
    public virtual Task Anonymous_projection_AsNoTracking_Selector(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Order>()
                .Select(o => new { A = o.CustomerID, B = o.OrderDate })
    ...
    public virtual Task Select_GetValueOrDefault_on_DateTime(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Order>().Select(o => o.OrderDate.GetValueOrDefault())
        );
    ...
    public virtual Task Cast_on_top_level_projection_brings_explicit_Cast(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Order>().Select(o => (double?)o.OrderID));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Client_method_in_projection_requiring_materialization_1(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => c.CustomerID.StartsWith("A")).Select(c => c.ToString())
        );
    ...
    public virtual Task Client_method_in_projection_requiring_materialization_2(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => c.CustomerID.StartsWith("A")).Select(c => ClientMethod(c))
        );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task SelectMany_without_result_selector_naked_collection_navigation(
        bool async
    ) => AssertQuery(async, ss => ss.Set<Customer>().SelectMany(c => c.Orders));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task SelectMany_without_result_selector_collection_navigation_composed(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().SelectMany(c => c.Orders.Select(o => o.CustomerID))
        );
    ...
    public virtual Task SelectMany_correlated_with_outer_7(bool async) =>
        AssertQuery(
            ...
            ss =>
                from c in ss.Set<Customer>()
                from o in ss.Set<Order>()
                    .Where(o => c.CustomerID.Length >= o.CustomerID.Length)
                    .OrderBy(o => c.City)
                    .ThenBy(o => o.OrderID)
                    .Take(2)
                    .DefaultIfEmpty()
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task FirstOrDefault_over_empty_collection_of_value_type_returns_correct_results(
        bool async
    ) =>
        AssertQuery(async, ss =>
            from c in ss.Set<Customer>()
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_non_nullable_value_after_FirstOrDefault_on_empty_collection(
        bool async
    ) =>
        AssertQueryScalar(
            async,
    ...
    public virtual async Task Filtered_collection_projection_is_tracked(bool async)
    {
        ...
        var query = context
            .Customers.Where(c => c.CustomerID.StartsWith("A"))
            .Select(c => new
            {
                Customer = c,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task SelectMany_with_collection_being_correlated_subquery_which_references_inner_and_outer_entity(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task SelectMany_with_collection_being_correlated_subquery_which_references_non_mapped_properties_from_inner_and_outer_entity(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
    ...
    public virtual Task Select_with_complex_expression_that_can_be_funcletized(bool async) =>
        AssertQueryScalar(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(c => c.CustomerID == "ALFKI")
                    .Select(c => c.ContactName.IndexOf("")),
            assertOrder: true
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_chained_entity_navigation_doesnt_materialize_intermittent_entities(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Collection_FirstOrDefault_with_entity_equality_check_in_projection(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
    ...
    public virtual Task Collection_FirstOrDefault_with_nullable_unsigned_int_column(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Customer>()
                .Select(c =>
                    c.Orders.OrderBy(o => o.OrderID).Select(o => o.EmployeeID).FirstOrDefault()
                )
        );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task LastOrDefault_member_access_in_projection_translates_to_server(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
    ...
    public virtual Task Projection_with_parameterized_constructor(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(c => c.CustomerID == "ALFKI")
                    .Select(c => new CustomerWrapper(c)),
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Projection_with_parameterized_constructor_with_member_assignment(
        bool async
    ) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<Customer>()
                    .Where(c => c.CustomerID == "ALFKI")
                    .Select(c => new CustomerWrapper(c) { City = c.City }),
            elementSorter: e => e.Customer.CustomerID,
            elementAsserter: (e, a) => Assert.Equal(e.Customer, a.Customer)
        );
        ...
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
            {
                ...
                AssertCollection(
                    ...
                    a.Orders,
                    elementSorter: i => i,
                    elementAsserter: (ie, ia) => Assert.Equal(ie, ia)
                );
            }
        );
    ...
    public virtual Task Coalesce_over_nullable_uint(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Order>().Select(o => o.EmployeeID ?? 0));
    ...
    public virtual Task Project_keyless_entity_FirstOrDefault_without_orderby(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Select(c =>
                    ss.Set<CustomerQuery>().FirstOrDefault(cv => cv.CompanyName == c.CompanyName)
                )
        );
            ...
            ss =>
                ss.Set<Order>()
                    ...
                    .Select(c => new
                    {
                        c.CustomerID,
                        Orders = c
                            .Orders.Where(x => filteredOrderIds.Contains(x.OrderID))
                            .OrderBy(x => x.OrderID)
                            .Select(x => new
                            {
                                c.CustomerID,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Correlated_collection_after_distinct_with_complex_projection_containing_original_identifier(
        bool async
    )
    {
        var filteredOrderIds = new[] { 10248, 10249, 10250 };
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Correlated_collection_after_distinct_not_containing_original_identifier(
        bool async
    )
    {
        var filteredOrderIds = new[] { 10248, 10249, 10250 };
        ...
        return AssertQuery(
                ...
                ss.Set<Order>()
                    ...
                    .Select(c => new
                    {
                        ...
                        Subquery = (
                            from x in ss.Set<Order>()
                            where
                                x.CustomerID == c.CustomerID && filteredOrderIds.Contains(x.OrderID)
                            select new
                            {
                                Outer1 = c.OrderDate,
                                Outer2 = c.CustomerID,
            ...
            elementAsserter: (e, a) =>
            {
                ...
                Assert.Equal(e.CustomerID, a.CustomerID);
                AssertCollection(e.Subquery, a.Subquery, elementSorter: ee =>
                    (ee.Outer1, ee.Outer2, ee.Inner, ee.OrderDate)
                );
            }
        );
    }
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Correlated_collection_after_distinct_with_complex_projection_not_containing_original_identifier(
        bool async
    )
    {
        var filteredOrderIds = new[] { 10248, 10249, 10250 };
        ...
        return AssertQuery(
            ...
            elementAsserter: (e, a) =>
            {
                ...
                Assert.Equal(e.Complex, a.Complex);
                AssertCollection(e.Subquery, a.Subquery, elementSorter: ee =>
                    (ee.Outer1, ee.Outer2, ee.Outer3, ee.Inner, ee.OrderDate)
                );
            }
        );
    }
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Correlated_collection_after_groupby_with_complex_projection_containing_original_identifier(
        bool async
    )
    {
        var filteredOrderIds = new[] { 10248, 10249, 10250 };
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Projecting_Length_of_a_string_property_after_FirstOrDefault_on_correlated_collection(
        bool async
    ) =>
        AssertQueryScalar(
            async,
    ...
    public virtual Task Projecting_count_of_navigation_which_is_generic_list(bool async) =>
        AssertQueryScalar(
            async,
            ss => ss.Set<Customer>().OrderBy(c => c.CustomerID).Select(c => c.Orders.Count),
            assertOrder: true
        );
    ...
    public virtual Task Projecting_count_of_navigation_which_is_generic_collection(bool async)
    {
        ...
        var selector = Expression.Lambda<Func<Customer, int>>(
            Expression.Property(Expression.Property(prm, "Orders"), collectionCount),
            prm
        );

        return AssertQueryScalar(
            async,
            ss => ss.Set<Customer>().OrderBy(c => c.CustomerID).Select(selector),
            assertOrder: true
        );
    }
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Projecting_count_of_navigation_which_is_generic_collection_using_convert(
        bool async
    ) =>
        AssertQueryScalar(
            async,
        ...
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
            {
                ...
                AssertInclude(
                    e.Property1,
                    a.Property1,
                    new ExpectedInclude<OrderDetail>(e => e.Product)
                );
                AssertCollection(e.Property2, a.Property2, elementAsserter: (ei, ai) =>
                    AssertInclude(ei, ai, new ExpectedInclude<OrderDetail>(e => e.Product))
                );
            }
        );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Projection_Distinct_projection_preserves_columns_used_for_distinct_in_subquery(
        bool async
    ) =>
        AssertQuery(
            async,
                ...
                ss.Set<Order>()
                    ...
                    .Select(o => new
                    {
                        ...
                        OrderDate = o.OrderDate.HasValue
                            ? o.OrderDate.Value
                            : new DateTime(o.OrderID - 10000, 1, 1),
                        OrderDate2 = o.OrderDate.HasValue == false
                            ? new DateTime(o.OrderID - 10000, 1, 1)
                            : o.OrderDate.Value,
                    }),
            assertOrder: true,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Correlated_collection_after_groupby_with_complex_projection_not_containing_original_identifier(
        bool async
    )
    {
        var filteredOrderIds = new[] { 10248, 10249, 10250 };
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Collection_projection_selecting_outer_element_followed_by_take(
        bool async
    ) =>
        AssertQuery(
            async,
            ...
            elementAsserter: (e, a) =>
            {
                AssertCollection(e.Customer, a.Customer, elementAsserter: (ee, aa) =>
                    AssertInclude(ee, aa, new ExpectedInclude<Customer>(i => i.Orders))
                );
            }
        );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Take_on_top_level_and_on_collection_projection_with_outer_apply(
        bool async
    ) => AssertFirstOrDefault(async, ss => ss.Set<Order>()
                ...
                .Select(o => new Order
                {
                    ...
                    OrderDate = o.OrderDate,
                    OrderDetails = o
                        .OrderDetails.Select(e => new OrderDetail
                        {
                            OrderID = e.OrderID,
                            Product = e.Product,
        ...
        AssertFirstOrDefault(
            ...
            ss =>
                ss.Set<Customer>()
                    ...
                    {
                        Orders = o
                            .Orders.OrderBy(a => a.OrderDate)
                            .Take(1)
                            .Select(e => new
                            {
                                Title = e.CustomerID == e.Customer.CustomerID ? "A" : "B",
                            })
                            .ToList(),
                    }),
            asserter: (e, a) =>
                AssertCollection(e.Orders, a.Orders, ordered: true, elementAsserter: (ee, aa) =>
                    AssertEqual(ee.Title, aa.Title)
                )
        );
            ...
            ss =>
                ss.Set<Customer>()
                    ...
                    .Select(c => new CustomerDetailsWithCount(
                        c.CustomerID,
                        c.City,
                        c.Orders.Select(o => new OrderInfo(o.OrderID, o.OrderDate)).ToList(),
                        c.Orders.Count
                    )),
            asserter: (e, a) =>
            {
    ...
    {
        public CustomerDetailsWithCount(
            string customerID,
            string city,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Client_projection_with_string_initialization_with_scalar_subquery(
        bool async
    ) =>
        AssertQuery(
            async,
            ...
            ss =>
                ss.Set<Customer>()
                    ...
                    .Select(c => new
                    {
                        c.CustomerID,
                        Order = c
                            .Orders.FirstOrDefault(o => o.OrderID < 11000)
            ...
            elementAsserter: (e, a) =>
            {
                ...
                AssertCollection(
                    ...
                    elementAsserter: (ee, aa) =>
                        AssertCollection(ee, aa, elementSorter: i => (i.OrderID, i.ProductID))
                );
            }
        );
        ...
        AssertFirstOrDefault(
            ...
            asserter: (e, a) =>
                AssertCollection(
                    ...
                    a,
                    elementSorter: e => e,
                    elementAsserter: (ee, aa) => AssertEqual(ee, aa)
                )
        );
                ...
                ss.Set<Customer>()
                    ...
                    .Select(c =>
                        c.Orders.OrderBy(o => o.OrderDate)
                            .Select(e => e.OrderDetails.Select(od => od.ProductID))
                            .FirstOrDefault()
                    ),
            asserter: (e, a) =>
                AssertCollection(
