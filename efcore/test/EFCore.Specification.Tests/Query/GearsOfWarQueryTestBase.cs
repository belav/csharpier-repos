    ...
    where TFixture : GearsOfWarQueryFixtureBase, new()
{
    ...
    public virtual Task Entity_equality_empty(bool async) =>
        AssertQuery(async, ss => ss.Set<Gear>().Where(g => g == new Gear()), assertEmpty: true);
    ...
    public virtual Task ToString_guid_property_projection(bool async) =>
        AssertQuery(
            async,
            ss => ss.Set<CogTag>().Select(ct => new { A = ct.GearNickName, B = ct.Id.ToString() }),
            elementSorter: e => e.B,
            elementAsserter: (e, a) =>
            {
    ...
    public virtual Task ToString_string_property_projection(bool async) =>
        AssertQuery(async, ss => ss.Set<Weapon>().Select(w => w.Name.ToString()));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task ToString_boolean_property_non_nullable(bool async) =>
        AssertQuery(async, ss => ss.Set<Weapon>().Select(w => w.IsAutomatic.ToString()));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task ToString_boolean_property_nullable(bool async) =>
        AssertQuery(async, ss => ss.Set<LocustHorde>().Select(lh => lh.Eradicated.ToString()));
    ...
    public virtual Task Include_with_join_and_inheritance1(bool async) =>
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<Officer>(o => o.CityOfBirth))
        );
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Gear>()
                    ...
                    .Include(g => g.Weapons),
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<Officer>(o => o.Weapons))
        );
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Weapon>()
                    ...
                    .ThenBy(e => e.Id),
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<Weapon>(w => w.Owner)),
            assertOrder: true
        );
    ...
    public virtual Task Where_enum(bool async) =>
        AssertQuery(async, ss => ss.Set<Gear>().Where(g => g.Rank == MilitaryRank.Sergeant));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_nullable_enum_with_constant(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Weapon>().Where(w => w.AmmunitionType == AmmunitionType.Cartridge)
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_nullable_enum_with_null_constant(bool async) =>
        AssertQuery(async, ss => ss.Set<Weapon>().Where(w => w.AmmunitionType == null));
    ...
    public virtual Task Where_nullable_enum_with_non_nullable_parameter(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Weapon>().Where(w => w.AmmunitionType == ammunitionType)
        );
    }
    ...
    {
        await AssertQuery(async, ss =>
            ss.Set<Gear>().Where(g => (g.Rank & MilitaryRank.Corporal) > 0)
        );

        await AssertQuery(async, ss =>
            ss.Set<Gear>().Where(g => (g.Rank & MilitaryRank.Corporal) == MilitaryRank.Corporal)
        );
    }
    ...
    public virtual async Task Where_bitwise_and_integral(bool async)
    {
        ...
        await AssertQuery(async, ss => ss.Set<Gear>().Where(g => ((short)g.Rank & 1) == 1));
    }
    ...
    public virtual Task Where_bitwise_and_nullable_enum_with_constant(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Weapon>().Where(w => (w.AmmunitionType & AmmunitionType.Cartridge) > 0)
        );
...
#pragma warning restore CS0458 // The result of the expression is always 'null'
    ...
    public virtual Task Where_bitwise_and_nullable_enum_with_non_nullable_parameter(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Weapon>().Where(w => (w.AmmunitionType & ammunitionType) > 0)
        );
    }
    ...
    public virtual Task Where_bitwise_or_enum(bool async) =>
        AssertQuery(async, ss => ss.Set<Gear>().Where(g => (g.Rank | MilitaryRank.Corporal) > 0));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Bitwise_projects_values_in_select(bool async) =>
        AssertFirst(async, ss => ss.Set<Gear>()
    ...
    public virtual async Task Where_enum_has_flag(bool async)
    {
        // Constant
        await AssertQuery(async, ss =>
            ss.Set<Gear>().Where(g => g.Rank.HasFlag(MilitaryRank.Corporal))
        );
        ...
        // Casting
        await AssertQuery(async, ss => ss.Set<Gear>().Where(g => g.Rank.HasFlag((MilitaryRank)1)));
        ...
        // QuerySource
        await AssertQuery(async, ss =>
            ss.Set<Gear>().Where(g => MilitaryRank.Corporal.HasFlag(g.Rank))
        );
    }
    ...
    public virtual Task Where_enum_has_flag_subquery_client_eval(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>()
                .Where(g =>
                    g.Rank.HasFlag(
                        ss.Set<Gear>().OrderBy(x => x.Nickname).ThenBy(x => x.SquadId).First().Rank
                    )
                )
        );
    ...
    public virtual Task Where_count_subquery_without_collision(bool async) =>
        AssertQuery(async, ss => ss.Set<Gear>().Where(w => w.Weapons.Count == 2));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_any_subquery_without_collision(bool async) =>
        AssertQuery(async, ss => ss.Set<Gear>().Where(w => w.Weapons.Any()));
    ...
    public virtual Task Select_ternary_operation_with_has_value_not_null(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Weapon>()
                    .Where(w =>
                        w.AmmunitionType.HasValue && w.AmmunitionType == AmmunitionType.Cartridge
                    )
                    .Select(w => new
                    {
                        w.Id,
    ...
    public virtual Task Null_propagation_optimization2(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>()
                .Where(g =>
                    (g.LeaderNickname == null ? null : g.LeaderNickname.EndsWith("us")) == true
                )
        );
    ...
    public virtual Task Null_propagation_optimization3(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>()
                .Where(g =>
                    (g.LeaderNickname != null ? g.LeaderNickname.EndsWith("us") : null) == true
                )
        );
    ...
    public virtual Task Select_null_propagation_optimization8(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>().Select(g => g != null ? g.LeaderNickname + g.LeaderNickname : null)
        );
    ...
    public virtual Task Select_null_propagation_optimization9(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Gear>().Select(g => g != null ? g.FullName.Length : (int?)null)
        );
    ...
    public virtual Task Select_null_propagation_negative1(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Gear>()
                .Select(g => g.LeaderNickname != null ? g.Nickname.Length == 5 : (bool?)null)
        );
    ...
    public virtual Task Select_null_propagation_negative3(bool async) =>
        AssertQuery(
            ...
            ss =>
                ...
                {
                    g2.Nickname,
                    Condition = g2 != null ? g2.LeaderNickname != null : (bool?)null,
                },
            assertOrder: true
        );
    ...
    public virtual Task Select_null_propagation_negative7(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Gear>()
                .Select(g =>
                    null != g.LeaderNickname ? g.LeaderNickname == g.LeaderNickname : (bool?)null
                )
        );
    ...
    public virtual Task Select_null_propagation_negative8(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<CogTag>().Select(t => t.Gear.Squad != null ? t.Gear.AssignedCity.Name : null)
        );
...
#pragma warning restore IDE0031 // Use null propagation
    ...
    public virtual Task Select_null_propagation_works_for_multiple_navigations_with_composite_keys(
        ...
        AssertQuery(async, ss =>
            from t in ss.Set<CogTag>()
            select EF.Property<City>(EF.Property<CogTag>(t.Gear, "Tag").Gear, "AssignedCity")
            != null
    ...
    public virtual Task Optional_Navigation_Null_Coalesce_To_Clr_Type(bool async) =>
        AssertFirst(async, ss =>
            ss.Set<Weapon>()
                .OrderBy(w => w.Id)
                .Select(w => new Weapon { IsAutomatic = (bool?)w.SynergyWith.IsAutomatic ?? false })
        );
    ...
    public virtual Task Where_subquery_boolean(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>()
                .Where(g =>
                    g.Weapons.OrderBy(w => w.Id).Select(w => w.IsAutomatic).FirstOrDefault()
                )
        );
    ...
    public virtual Task Where_subquery_boolean_with_pushdown(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>().Where(g => g.Weapons.OrderBy(w => w.Id).FirstOrDefault().IsAutomatic)
        );
    ...
    public virtual Task Where_subquery_distinct_firstordefault_boolean_with_pushdown(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>()
                .Where(g =>
                    g.HasSoulPatch
                    && g.Weapons.Distinct().OrderBy(w => w.Id).FirstOrDefault().IsAutomatic
                )
        );
    ...
    public virtual Task Where_subquery_union_firstordefault_boolean(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>()
                .Where(g =>
                    g.HasSoulPatch
                    && g.Weapons.Union(g.Weapons).OrderBy(w => w.Id).FirstOrDefault().IsAutomatic
                )
        );
    ...
    public virtual Task Where_subquery_concat_firstordefault_boolean(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>()
                .Where(g =>
                    g.HasSoulPatch
                    && g.Weapons.Concat(g.Weapons).OrderBy(w => w.Id).FirstOrDefault().IsAutomatic
                )
        );
    ...
    public virtual Task Concat_with_count(bool async) =>
        AssertCount(async, ss => ss.Set<Gear>().Concat(ss.Set<Gear>()));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Concat_scalars_with_count(bool async) =>
        AssertCount(async, ss =>
            ss.Set<Gear>().Select(g => g.Nickname).Concat(ss.Set<Gear>().Select(g2 => g2.FullName))
        );
    ...
    public virtual Task Concat_with_scalar_projection(bool async) =>
        AssertQuery(async, ss => ss.Set<Gear>().Concat(ss.Set<Gear>()).Select(g => g.Nickname));
    ...
    public virtual Task Select_Where_Navigation_Null(bool async) =>
        AssertQuery(async, ss => from t in ss.Set<CogTag>() where t.Gear == null select t);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_Where_Navigation_Null_Reverse(bool async) =>
        AssertQuery(async, ss => from t in ss.Set<CogTag>() where null == t.Gear select t);
    ...
    public virtual Task Non_unicode_string_literal_is_used_for_non_unicode_column(bool async) =>
        AssertQuery(async, ss => from c in ss.Set<City>() where c.Location == "Unknown" select c);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Non_unicode_string_literal_is_used_for_non_unicode_column_right(
        bool async
    ) => AssertQuery(async, ss => from c in ss.Set<City>() where "Unknown" == c.Location select c);
...
#pragma warning restore IDE0029 // Use coalesce expression
    ...
    public virtual Task Coalesce_operator_in_predicate(bool async) =>
        AssertQuery(async, ss => ss.Set<Weapon>().Where(w => (bool?)w.IsAutomatic ?? false));
    ...
    public virtual Task Coalesce_operator_in_predicate_with_other_conditions(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Weapon>()
                .Where(w =>
                    w.AmmunitionType == AmmunitionType.Cartridge && ((bool?)w.IsAutomatic ?? false)
                )
        );
    ...
    public virtual Task Optional_navigation_type_compensation_works_with_predicate(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<CogTag>().Where(t => t.Note != "K.I.A." && t.Gear.HasSoulPatch)
        );
    ...
    public virtual Task Optional_navigation_type_compensation_works_with_predicate_negated_complex2(
        ...
        AssertQuery(
            async,
            ss =>
                ss.Set<CogTag>().Where(t => !(!t.Gear.HasSoulPatch ? false : t.Gear.HasSoulPatch)),
            ss =>
                ss.Set<CogTag>()
                    .Where(t =>
                        !(
    ...
    public virtual Task Optional_navigation_type_compensation_works_with_binary_expression(
        ...
        AssertQuery(async, ss =>
            ss.Set<CogTag>().Where(t => t.Gear.HasSoulPatch || t.Note.Contains("Cole"))
        );
    ...
    public virtual Task Optional_navigation_type_compensation_works_with_binary_and_expression(
        ...
        AssertQueryScalar(async, ss =>
            ss.Set<CogTag>().Select(t => t.Gear.HasSoulPatch && t.Note.Contains("Cole"))
        );
    ...
    public virtual Task Optional_navigation_type_compensation_works_with_projection(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<CogTag>().Where(t => t.Note != "K.I.A.").Select(t => t.Gear.SquadId)
        );
    ...
    public virtual Task Optional_navigation_type_compensation_works_with_projection_into_anonymous_type(
        ...
        AssertQuery(
            async,
            ss =>
                ss.Set<CogTag>().Where(t => t.Note != "K.I.A.").Select(t => new { t.Gear.SquadId }),
                ...
                AssertQuery(
                    ...
                    ss =>
                        ss.Set<CogTag>()
                            ...
                            .OrderBy(t => t.Note)
                            .Select(t =>
                                ss.Set<Gear>().OrderBy(g => g.Nickname).Skip(t.Gear.SquadId)
                            ),
                    assertOrder: true,
                ...
                AssertQuery(
                    ...
                    ss =>
                        ss.Set<CogTag>()
                            ...
                            .OrderBy(t => t.Note)
                            .Select(t =>
                                ss.Set<Gear>().OrderBy(g => g.Nickname).Take(t.Gear.SquadId)
                            ),
                    assertOrder: true,
                    elementAsserter: (e, a) => AssertCollection(e, a, ordered: true)
                ),
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_correlated_filtered_collection_returning_queryable_throws(
        bool async
    ) =>
        AssertInvalidMaterializationType(
            () =>
                AssertQuery(
                    ...
                    ss =>
                        ss.Set<CogTag>()
                            .OrderBy(t => t.Note)
                            .Select(t => ss.Set<Gear>().Where(g => g.Nickname == t.GearNickName)),
                    assertOrder: true,
                    elementAsserter: (e, a) => AssertCollection(e, a)
                ),
    ...
    public virtual Task Select_correlated_filtered_collection_works_with_caching(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<CogTag>()
                    .OrderBy(t => t.Note)
                    .Select(t => ss.Set<Gear>().Where(g => g.Nickname == t.GearNickName).ToList()),
            assertOrder: true,
            elementAsserter: (e, a) => AssertCollection(e, a)
        );
    ...
    public virtual Task Where_datetimeoffset_year_component(bool async) =>
        AssertQuery(async, ss => from m in ss.Set<Mission>() where m.Timeline.Year == 2 select m);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_datetimeoffset_month_component(bool async) =>
        AssertQuery(async, ss => from m in ss.Set<Mission>() where m.Timeline.Month == 1 select m);
    ...
    public virtual Task Where_datetimeoffset_day_component(bool async) =>
        AssertQuery(async, ss => from m in ss.Set<Mission>() where m.Timeline.Day == 2 select m);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_datetimeoffset_hour_component(bool async) =>
        AssertQuery(async, ss => from m in ss.Set<Mission>() where m.Timeline.Hour == 10 select m);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_datetimeoffset_minute_component(bool async) =>
        AssertQuery(async, ss => from m in ss.Set<Mission>() where m.Timeline.Minute == 0 select m);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_datetimeoffset_second_component(bool async) =>
        AssertQuery(async, ss => from m in ss.Set<Mission>() where m.Timeline.Second == 0 select m);
    ...
    public virtual Task DateTimeOffset_DateAdd_AddYears(bool async) =>
        AssertQueryScalar(async, ss => from m in ss.Set<Mission>() select m.Timeline.AddYears(1));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task DateTimeOffset_DateAdd_AddMonths(bool async) =>
        AssertQueryScalar(async, ss => from m in ss.Set<Mission>() select m.Timeline.AddMonths(1));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task DateTimeOffset_DateAdd_AddDays(bool async) =>
        AssertQueryScalar(async, ss => from m in ss.Set<Mission>() select m.Timeline.AddDays(1));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task DateTimeOffset_DateAdd_AddHours(bool async) =>
        AssertQueryScalar(async, ss => from m in ss.Set<Mission>() select m.Timeline.AddHours(1));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task DateTimeOffset_DateAdd_AddMinutes(bool async) =>
        AssertQueryScalar(async, ss => from m in ss.Set<Mission>() select m.Timeline.AddMinutes(1));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task DateTimeOffset_DateAdd_AddSeconds(bool async) =>
        AssertQueryScalar(async, ss => from m in ss.Set<Mission>() select m.Timeline.AddSeconds(1));
...
#pragma warning restore IDE0031 // Use null propagation
    ...
    public virtual Task Distinct_with_optional_navigation_is_translated_to_sql(bool async) =>
        AssertQueryScalar(async, ss =>
            (from g in ss.Set<Gear>() where g.Tag.Note != "Foo" select g.HasSoulPatch).Distinct()
        );
    ...
    public virtual Task Sum_with_optional_navigation_is_translated_to_sql(bool async) =>
        AssertSum(async, ss =>
            (from g in ss.Set<Gear>() where g.Tag.Note != "Foo" select g.SquadId)
        );
    ...
    )
    {
        ...
        return AssertQuery(async, ss => ss.Set<Gear>().Where(g => ClientEquals(g.Tag.Note, prm)));
    }

    private static bool ClientEquals(string first, string second) => first == second;
    ...
    public virtual Task Include_reference_on_derived_type_using_EF_Property(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<LocustLeader>()
                    .Include(lc => EF.Property<Gear>((LocustCommander)lc, "DefeatedBy")),
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<LocustCommander>(lc => lc.DefeatedBy))
        );
    ...
    public virtual Task Include_reference_on_derived_type_using_lambda_with_tracking(bool async) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<LocustLeader>().AsTracking().Include(ll => ((LocustCommander)ll).DefeatedBy),
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<LocustCommander>(lc => lc.DefeatedBy))
        );
    ...
    public virtual Task Include_collection_on_derived_type_using_EF_Property(bool async) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<Gear>().Include(o => EF.Property<ICollection<Gear>>((Officer)o, "Reports")),
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<Officer>(o => o.Reports))
        );
    ...
    public virtual Task Projecting_nullable_bool_in_conditional_works(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<CogTag>()
                    .Select(cg => new { Prop = cg.Gear != null ? cg.Gear.HasSoulPatch : false }),
            e => e.Prop
        );
    ...
    public virtual Task Correlated_collections_project_anonymous_collection_result(bool async) =>
        AssertQuery(
            ...
            ss =>
                ...
                {
                    s.Name,
                    Collection = (from m in s.Members select new { m.FullName, m.Rank }).ToList(),
                },
            elementSorter: e => e.Name,
            elementAsserter: (e, a) =>
            {
            ...
            ss =>
                ss.Set<Gear>()
                    ...
                    .Select(o => new
                    {
                        o.FullName,
                        OuterCollection = o
                            .Reports.Where(r => r.FullName != "Foo")
                            .OrderBy(r => r.Rank)
                            .Select(g => new
                            {
                                g.FullName,
                                InnerCollection = g
                                    .Weapons.Where(w => w.Name != "Bar")
        ...
        AssertQuery(
            ...
            ss =>
                ...
                select new
                {
                    g.FullName,
                    First = g
                        .Weapons.OrderBy(w1 => w1.OwnerFullName)
                        .Where(w1 => w1.IsAutomatic)
                        .ToList(),
                    Second = g
                        .Weapons.OrderBy(w2 => w2.IsAutomatic)
        ...
        AssertQuery(
            ...
            ss =>
                ...
                select new
                {
                    o.Nickname,
                    First = o
                        .Weapons.Where(w => w.IsAutomatic)
                        .Select(w => new { w.Name, w.IsAutomatic })
                        .ToArray(),
                    Second = o
                        .Reports.OrderBy(r => r.FullName)
                        ...
                        select new
                        {
                            ...
                            InnerCollection = (
                                ...
                                select new
                                {
                                    w.Id,
                                    InnerFirst = w
                                        .Owner.Weapons.Select(ww => new { ww.Name, ww.IsAutomatic })
                                        .ToList(),
                                    InnerSecond = w
                                        .Owner.Squad.Members.OrderBy(mm => mm.Nickname)
    ...
    public virtual Task Correlated_collections_inner_subquery_selector_references_outer_qsre(
        ...
        AssertQuery(
            ...
            {
                Assert.Equal(e.FullName, a.FullName);
                AssertCollection(e.Collection, a.Collection, elementSorter: ee =>
                    (ee.ReportName, ee.OfficerName)
                );
            }
        );
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Squad>()
                    .OrderBy(s => s.Name)
                    .Select(s =>
                        s.Members.OrderBy(m => m.Nickname).Select(m => m.FullName).FirstOrDefault()
                    ),
            assertOrder: true
        );
    ...
    public virtual Task Correlated_collections_deeply_nested_left_join(bool async) =>
        AssertQuery(
            ...
            ss =>
                ...
                select g
                    .Squad.Members.Where(m => m.HasSoulPatch)
                    .Select(m => new
                    {
                        m.Nickname,
                ...
                ss.Set<Weapon>()
                    ...
                    .Select(w =>
                        w.Owner.Squad.Members.OrderByDescending(m => m.FullName)
                            ...
                            {
                                Weapons = m
                                    .Weapons.Where(ww => !ww.IsAutomatic)
                                    .OrderBy(ww => ww.Id)
                                    .ToList(),
                                m.Rank,
                            })
            ...
            ss =>
                ss.Set<Weapon>()
                    ...
                    .Select(w =>
                        w.Owner != null
                            ? w
                                .Owner.Squad.Members.OrderByDescending(m => m.FullName)
                                .Select(m => new
                                {
                                    Weapons = m
                                        .Weapons.Where(ww => !ww.IsAutomatic)
    ...
    public virtual Task Correlated_collection_with_top_level_Count(bool async) =>
        AssertCount(async, ss => ss.Set<Gear>().Select(g => g.Weapons));
    ...
    public virtual Task Select_required_navigation_on_derived_type(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<LocustLeader>()
                    .Select(ll =>
                        ll is LocustCommander ? ((LocustCommander)ll).HighCommand.Name : null
                    )
        );
    ...
    public virtual Task Where_required_navigation_on_derived_type(bool async) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<LocustLeader>().Where(ll => ((LocustCommander)ll).HighCommand.IsOperational),
            ss =>
                ss.Set<LocustLeader>()
                    .Where(ll =>
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Streaming_correlated_collection_issue_11403_returning_ordered_enumerable_throws(
        bool async
    ) =>
        AssertInvalidMaterializationType(
            () =>
        ...
        AssertQuery(async, ss =>
            ss.Set<Squad>()
                ...
                .Select(s => new
                {
                    s.Name,
                    SquadId = s
                        .Members.Where(m => m.HasSoulPatch)
                        .Select(m => m.SquadId)
                        .FirstOrDefault(),
                })
        );
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Squad>()
                    .Where(s => s.Name == "Kilo")
                    .Where(s =>
                        s.Members.Where(m => m.HasSoulPatch).Select(m => m.SquadId).FirstOrDefault()
                        != 0
                    )
                    .Select(s => s.Name),
            assertEmpty: true
        ...
        AssertQuery(async, ss =>
            ss.Set<Squad>()
                .Select(s => new
                {
                    s.Name,
                    Gear = s
                        .Members.Where(g => g.HasSoulPatch)
                        .Select(g => new { One = 1 })
                        .FirstOrDefault(),
                })
        );
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Gear>()
                    ...
                    .Include(o => o.Reports)
                    .OrderBy(o =>
                        o.Weapons.OrderBy(w => w.Id).Select(w => w.IsAutomatic).FirstOrDefault()
                    )
                    .ThenBy(o => o.Nickname),
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<Officer>(o => o.Reports)),
    ...
    public virtual Task Cast_to_derived_type_after_OfType_works(bool async) =>
        AssertQuery(async, ss => ss.Set<Gear>().OfType<Officer>().Cast<Officer>());
    ...
    public virtual Task Select_subquery_boolean_with_pushdown(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Gear>().Select(g => g.Weapons.OrderBy(w => w.Id).FirstOrDefault().IsAutomatic)
        );
    ...
    public virtual Task Select_subquery_int_with_outside_cast_and_coalesce(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Gear>()
                .Select(g =>
                    (int?)g.Weapons.OrderBy(w => w.Id).Select(w => w.Id).FirstOrDefault() ?? 42
                )
        );
    ...
    public virtual Task Select_subquery_int_with_pushdown_and_coalesce(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Gear>().Select(g => (int?)g.Weapons.OrderBy(w => w.Id).FirstOrDefault().Id ?? 42)
        );
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<City>()
                    ...
                    .Select(c =>
                        c.StationedGears.Select(g => new Officer
                            {
    ...
    public virtual Task Time_of_day_datetimeoffset(bool async) =>
        AssertQueryScalar(async, ss => from m in ss.Set<Mission>() select m.Timeline.TimeOfDay);
    ...
    public virtual Task GroupBy_Property_Include_Select_Count(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Gear>().Include(g => g.CityOfBirth).GroupBy(g => g.Rank).Select(g => g.Count())
        );
    ...
    public virtual Task Include_with_client_method_and_member_access_still_applies_includes(
        ...
        AssertQuery(async, ss =>
            ss.Set<Gear>().Include(g => g.Tag).Select(g => new { g.Nickname, Client(g).FullName })
        );
    ...
    public virtual Task Include_with_projection_of_unmapped_property_still_gets_applied(
        ...
        AssertQueryScalar(async, ss =>
            ss.Set<Gear>().Include(g => g.Weapons).Select(g => g.IsMarcus)
        );
    ...
    public virtual Task GetValueOrDefault_in_projection(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Weapon>().Select(w => w.SynergyWithId.GetValueOrDefault())
        );
    ...
    public virtual Task GetValueOrDefault_in_filter(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Weapon>().Where(w => w.SynergyWithId.GetValueOrDefault() == 0)
        );
    ...
    public virtual Task GetValueOrDefault_on_DateTimeOffset(bool async)
    {
        ...
        return AssertQuery(
            ...
            ss =>
                ss.Set<Mission>()
                    .Where(m => ((DateTimeOffset?)m.Timeline).GetValueOrDefault() == defaultValue),
            assertEmpty: true
        );
    }
    ...
    public virtual Task GetValueOrDefault_with_argument(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Weapon>().Where(w => w.SynergyWithId.GetValueOrDefault(w.Id) == 1)
        );
    ...
    public virtual Task GetValueOrDefault_with_argument_complex(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Weapon>()
                    .Where(w =>
                        (w.SynergyWithId == null ? w.Name.Length + 42 : w.SynergyWithId) > 10
                    )
        );
    ...
    public virtual Task Query_with_complex_let_containing_ordering_and_filter_projecting_firstOrDefault_element_of_let(
        ...
        AssertQuery(
            ...
            ss =>
                ...
                let automaticWeapons = g
                    .Weapons.OrderByDescending(w => w.AmmunitionType)
                    .Where(w => w.IsAutomatic)
    ...
    public virtual Task Null_semantics_is_correctly_applied_for_function_comparisons_that_take_arguments_from_optional_navigation(
        ...
        AssertQuery(
            async,
            ss =>
                ss.Set<CogTag>().Where(t => t.Note.Substring(0, t.Gear.SquadId) == t.GearNickName),
            ss =>
                ss.Set<CogTag>()
                    .Where(t => t.Gear.Maybe(x => t.Note.Substring(0, x.SquadId)) == t.GearNickName)
        );
    ...
    public virtual Task Null_semantics_is_correctly_applied_for_function_comparisons_that_take_arguments_from_optional_navigation_complex(
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<CogTag>()
                    .Where(t => t.Note.Substring(0, t.Gear.Squad.Name.Length) == t.GearNickName),
            ss =>
                ss.Set<CogTag>()
                    .Where(t =>
    ...
    public virtual Task Nav_rewrite_Distinct_with_convert()
        ...
        AssertTranslationFailed(() =>
        {
            using var ctx = CreateContext();
            _ = ctx
                .Factions.Include(f => ((LocustHorde)f).Commander)
    ...
    public virtual Task Where_contains_on_navigation_with_composite_keys(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>().Where(g => ss.Set<City>().Any(c => c.BornGears.Contains(g)))
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_derivied_entity_with_convert_to_parent(bool async) =>
        AssertQuery(async, ss => ss.Set<Faction>().OfType<LocustHorde>().Select(f => (Faction)f));
    ...
    public virtual Task DateTimeOffset_Contains_Less_than_Greater_than(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Mission>()
                .Where(m =>
                    start <= m.Timeline.Date && m.Timeline < end && dates.Contains(m.Timeline)
                )
        );
    }
    ...
    public virtual Task Join_with_inner_being_a_subquery_projecting_single_property(bool async) =>
        AssertQuery(async, ss =>
            from g in ss.Set<Gear>()
            join inner in (from g2 in ss.Set<Gear>() select g2.Nickname) on g.Nickname equals inner
            select g
        );
    ...
    public virtual Task Navigation_based_on_complex_expression3(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Faction>().Where(f => f is LocustHorde).Select(f => ((LocustHorde)f).Commander)
        );
    ...
    public virtual Task Select_as_operator(bool async) =>
        AssertQuery(async, ss => ss.Set<LocustLeader>().Select(ll => ll as LocustCommander));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_datetimeoffset_comparison_in_projection(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Mission>().Select(m => m.Timeline > DateTimeOffset.Now)
        );
    ...
    public virtual async Task Include_after_select_with_entity_projection_throws(bool async) =>
        Assert.Equal(
            ...
            (
                await Assert.ThrowsAsync<InvalidOperationException>(() =>
                    AssertQuery(async, ss =>
                        ss.Set<Faction>().Select(f => f.Capital).Include(c => c.BornGears)
                    )
                )
            ).Message
        );
    ...
    public virtual async Task Include_after_select_anonymous_projection_throws(bool async) =>
        Assert.Equal(
            ...
            (
                await Assert.ThrowsAsync<InvalidOperationException>(() =>
                    AssertQuery(async, ss =>
                        ss.Set<Faction>().Select(f => new { f }).Include(x => x.f.Capital)
                    )
                )
            ).Message
        );
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<CogTag>()
                    .GroupBy(t => new
                    {
                        HasSoulPatch = (bool?)t.Gear.HasSoulPatch,
                        t.Gear.Squad.Name,
                    })
    ...
    public virtual Task FirstOrDefault_navigation_access_entity_equality_in_where_predicate_apply_peneding_selector(
        ...
        AssertQuery(async, ss =>
            ss.Set<Gear>()
                .Where(g =>
                    g.AssignedCity
                    == ss.Set<Gear>().OrderBy(s => s.Nickname).FirstOrDefault().CityOfBirth
                )
        );
    ...
    )
    {
        ...
        return AssertQuery(isAsync, ss =>
            ss.Set<Gear>().Where(g => g.HasSoulPatch == prm ? true : g.CityOfBirthName == prm2)
        );
    }
    ...
    )
    {
        ...
        await AssertQueryScalar(async, ss => ss.Set<Gear>().Select(g => (g.Rank | ranks) == ranks));

        await AssertQueryScalar(async, ss =>
            ss.Set<Gear>().Select(g => (g.Rank | (g.Rank | (ranks | (g.Rank | ranks)))) == ranks)
        );
    }
    ...
    {
        await AssertQuery(async, ss =>
            ss.Set<Weapon>().Where(w => (w.AmmunitionType & AmmunitionType.Cartridge) == null)
        );

        await AssertQuery(async, ss =>
            ss.Set<Weapon>().Where(w => (w.AmmunitionType | AmmunitionType.Shell) == null)
        );

        AmmunitionType? prm = null;
        await AssertQuery(async, ss =>
            ss.Set<Weapon>().Where(w => (w.AmmunitionType | AmmunitionType.Shell) == prm)
        );

        await AssertQuery(async, ss =>
            ss.Set<Weapon>().Where(w => (w.AmmunitionType & prm) == prm)
        );

        prm = AmmunitionType.Shell;
        await AssertQuery(async, ss => ss.Set<Weapon>().Where(w => (w.AmmunitionType & prm) != 0));
    ...
    {
        var prm = true;
        await AssertQuery(async, ss => ss.Set<Gear>().Where(g => (g.HasSoulPatch && prm) != prm));
    ...
    public virtual Task Cast_OfType_works_correctly(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>().Cast<Gear>().OfType<Officer>().Select(o => o.FullName)
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Join_inner_source_custom_projection_followed_by_filter(bool async) =>
        AssertQuery(async, ss => from ll in ss.Set<LocustLeader>() join h in ss.Set<Faction>()
                .OfType<LocustHorde>()
                .Select(f => new
                {
                ...
                on ll.Name equals h.CommanderName where h.IsEradicated != true select h);
    ...
    public virtual Task DateTimeOffset_Date_returns_datetime(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Mission>().Where(m => m.Timeline.Date >= dateTimeOffset.Date)
        );
    }
    ...
    public virtual Task Conditional_with_conditions_evaluating_to_false_gets_optimized(
        ...
        AssertQuery(async, ss =>
            ss.Set<Gear>()
                .Select(g =>
                    g.Nickname == null && g.Nickname != null ? g.CityOfBirthName : g.FullName
                )
        );
    ...
    public virtual Task Projecting_required_string_column_compared_to_null_parameter(bool async)
    {
        ...
        return AssertQueryScalar(async, ss =>
            ss.Set<Gear>().Select(g => g.Nickname == nullParameter)
        );
    }
    ...
    public virtual Task Byte_array_filter_by_SequenceEqual(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Squad>().Where(s => s.Banner5.SequenceEqual(byteArrayParam))
        );
    }
    ...
    public virtual Task Group_by_nullable_property_HasValue_and_project_the_grouping_key(
        ...
        AssertQueryScalar(async, ss =>
            ss.Set<Weapon>().GroupBy(w => w.SynergyWithId.HasValue).Select(g => g.Key)
        );
    ...
    public virtual Task Group_by_nullable_property_and_project_the_grouping_key_HasValue(
        ...
        AssertQueryScalar(async, ss =>
            ss.Set<Weapon>().GroupBy(w => w.SynergyWithId).Select(g => g.Key.HasValue)
        );
    ...
    public virtual Task Checked_context_with_cast_does_not_fail(bool isAsync)
    {
        ...
        {
            return AssertQuery(isAsync, ss =>
                ss.Set<LocustLeader>().Where(w => (byte)w.ThreatLevel >= (short?)5)
            );
        }
    }
    ...
    public virtual Task Checked_context_with_addition_does_not_fail(bool isAsync)
    {
        ...
        {
            return AssertQuery(isAsync, ss =>
                ss.Set<LocustLeader>().Where(w => w.ThreatLevel <= (5 + (long?)w.ThreatLevel))
            );
        }
    }
    ...
    public virtual Task TimeSpan_Hours(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Mission>().Select(m => m.Duration.Hours));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task TimeSpan_Minutes(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Mission>().Select(m => m.Duration.Minutes));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task TimeSpan_Seconds(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Mission>().Select(m => m.Duration.Seconds));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task TimeSpan_Milliseconds(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Mission>().Select(m => m.Duration.Milliseconds));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_TimeSpan_Hours(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Duration.Hours == 1));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_TimeSpan_Minutes(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Duration.Minutes == 2));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_TimeSpan_Seconds(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Duration.Seconds == 3));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_TimeSpan_Milliseconds(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Duration.Milliseconds == 456));
    ...
    public virtual Task Contains_on_collection_of_nullable_byte_subquery_null_constant(
        ...
        AssertQuery(async, ss =>
            ss.Set<LocustLeader>()
                .Where(l =>
                    ss.Set<LocustLeader>().Select(ll => ll.ThreatLevelNullableByte).Contains(null)
                )
        );
    ...
    public virtual Task Contains_on_collection_of_nullable_byte_subquery_null_parameter(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<LocustLeader>()
                .Where(l =>
                    ss.Set<LocustLeader>().Select(ll => ll.ThreatLevelNullableByte).Contains(prm)
                )
        );
    }
    ...
    )
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Gear>().Where(g => (prm & (int)g.Rank) == (int)g.Rank)
        );
    }
    ...
    )
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Gear>().Where(g => (prm & (short)g.Rank) == (short)g.Rank)
        );
    }
    ...
    public virtual Task Trying_to_access_unmapped_property_in_projection(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Gear>().Select(g => g.IsMarcus));
    ...
    public virtual Task Trying_to_access_unmapped_property_inside_aggregate(bool async) =>
        AssertTranslationFailedWithDetails(
            () =>
                AssertQuery(async, ss =>
                    ss.Set<City>().Where(c => c.BornGears.Count(g => g.IsMarcus) > 0)
                ),
            CoreStrings.QueryUnableToTranslateMember(nameof(Gear.IsMarcus), nameof(Gear))
        );
            ...
            () =>
                AssertQuery(async, ss =>
                    ss.Set<Gear>()
                        .Select(g => new
                        {
                            nested = ss.Set<Gear>().Where(gg => gg.IsMarcus).ToList(),
                        })
                ),
            CoreStrings.QueryUnableToTranslateMember(nameof(Gear.IsMarcus), nameof(Gear))
        );
    ...
    public virtual Task Client_member_and_unsupported_string_Equals_in_the_same_query(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>()
                .Where(g =>
                    g.FullName.Equals(g.Nickname, StringComparison.InvariantCulture) || g.IsMarcus
                )
        );
    ...
    public virtual Task CompareTo_used_with_non_unicode_string_column_and_constant(bool async) =>
        AssertQuery(async, ss => ss.Set<City>().Where(c => c.Location.CompareTo("Unknown") == 0));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Coalesce_used_with_non_unicode_string_column_and_constant(bool async) =>
        AssertQuery(async, ss => ss.Set<City>().Select(c => c.Location ?? "Unknown"));
    ...
    public virtual Task SelectMany_without_result_selector_and_non_equality_comparison_converted_to_join(
        ...
        AssertQuery(async, ss =>
            ss.Set<Gear>()
                .SelectMany(g =>
                    ss.Set<Weapon>().Where(x => x.OwnerFullName != g.FullName).DefaultIfEmpty()
                )
        );
    ...
    public virtual async Task Accessing_derived_property_using_hard_and_soft_cast(bool async)
    {
        await AssertQuery(async, ss =>
            ss.Set<LocustLeader>()
                .Where(ll => ll is LocustCommander && ((LocustCommander)ll).HighCommandId != 0)
        );
    ...
    public virtual Task GroupBy_Select_sum(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Mission>().GroupBy(m => m.CodeName).Select(g => g.Sum(m => m.Rating))
        );
    ...
    public virtual Task Sum_with_no_data_nullable_double(bool async) =>
        AssertSum(async, ss =>
            ss.Set<Mission>().Where(m => m.CodeName == "Operation Foobar").Select(m => m.Rating)
        );
    ...
    public virtual Task First_on_byte_array(bool async) =>
        AssertQuery(async, ss => ss.Set<Squad>().Where(e => e.Banner.First() == 0x02));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Array_access_on_byte_array(bool async) =>
        AssertQuery(async, ss => ss.Set<Squad>().Where(e => e.Banner5[2] == 0x06));
    ...
    public virtual Task Composite_key_entity_equal_null(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<LocustLeader>().OfType<LocustCommander>().Where(lc => lc.DefeatedBy == null)
        );
    ...
    public virtual Task Projecting_property_converted_to_nullable_with_comparison(bool async) =>
        AssertQuery(async, ss => ss.Set<CogTag>()
                .Select(x => new
                {
                    x.Note,
    ...
    public virtual Task Projecting_property_converted_to_nullable_with_conditional(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<CogTag>()
                .Select(x => new
                {
                    x.Note,
                    Nullable = x.GearNickName != null
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<CogTag>()
                    ...
                    {
                        x.Note,
                        Function = x.Note.Substring(0, x.Nullable.SquadId),
                    }),
            ss =>
                ss.Set<CogTag>()
                    .Select(x => new
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Gear>()
                    ...
                    {
                        Key = g.Nickname,
                        Subquery = g.Weapons.Select(w => new { w.Id, w.Name }).Distinct().ToList(),
                    }),
            elementSorter: e => e.Key,
            elementAsserter: (e, a) =>
            {
            ...
            ss =>
                ss.Set<Gear>()
                    .Select(g => new
                    {
                        Key = g.Nickname,
                        Subquery = g
                            .Weapons.Select(w => new { w.Name, w.IsAutomatic })
                            .Distinct()
                            .ToList(),
                    }),
            elementSorter: e => e.Key,
                    ...
                    .Select(s => new
                    {
                        ...
                        Members = s
                            .Members.Select(m => new
                            {
                                m,
                                Weapons = m
                                    .Weapons.Where(w => w.OwnerFullName == m.FullName)
                                    .ToList(),
                            })
                            .Distinct(),
                    })
    ...
    public virtual Task Where_DateOnly_Year(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Date.Year == 1990).AsTracking());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_DateOnly_Month(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Date.Month == 11).AsTracking());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_DateOnly_Day(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Date.Day == 10).AsTracking());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_DateOnly_DayOfYear(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Mission>().Where(m => m.Date.DayOfYear == 314).AsTracking()
        );
    ...
    public virtual Task Where_DateOnly_DayOfWeek(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Mission>().Where(m => m.Date.DayOfWeek == DayOfWeek.Saturday).AsTracking()
        );
    ...
    public virtual Task Where_TimeOnly_Hour(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Time.Hour == 10).AsTracking());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_TimeOnly_Minute(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Time.Minute == 15).AsTracking());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_TimeOnly_Second(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Time.Second == 50).AsTracking());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_TimeOnly_Millisecond(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Mission>().Where(m => m.Time.Millisecond == 500).AsTracking()
        );
    ...
    public virtual Task Where_TimeOnly_Add_TimeSpan(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Mission>()
                .Where(m => m.Time.Add(new TimeSpan(3, 0, 0)) == new TimeOnly(13, 15, 50, 500))
                .AsTracking()
        );
    ...
    public virtual Task Where_TimeOnly_IsBetween(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Mission>()
                .Where(m => m.Time.IsBetween(new TimeOnly(10, 0, 0), new TimeOnly(11, 0, 0)))
                .AsTracking()
        );
    ...
    public virtual Task Where_TimeOnly_subtract_TimeOnly(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Mission>()
                .Where(m => m.Time - new TimeOnly(10, 0, 0) == new TimeSpan(0, 0, 15, 50, 500))
                .AsTracking()
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Basic_query_gears(bool async) => AssertQuery(async, ss => ss.Set<Gear>());
    ...
    public virtual Task Comparison_with_value_converted_subclass(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Faction>().Where(f => f.ServerAddress == IPAddress.Loopback)
        );
    ...
    public virtual Task Contains_on_readonly_enumerable(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Weapon>().Where(w => _weaponTypes.Contains(w.AmmunitionType))
        );
    ...
    public virtual Task Where_equals_method_on_nullable_with_object_overload(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Rating.Equals(null)));
    ...
    public virtual Task Where_bool_column_and_Contains(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Gear>().Where(g => g.HasSoulPatch && values.Contains(g.HasSoulPatch))
        );
    }
    ...
    public virtual Task Enum_matching_take_value_gets_different_type_mapping(bool async)
    {
        ...
        return AssertQueryScalar(async, ss =>
            ss.Set<Gear>().OrderBy(g => g.Nickname).Take(1).Select(g => g.Rank & value)
        );
    }
    ...
    public virtual Task Where_subquery_equality_to_null_with_composite_key(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Squad>()
                    .Where(s => s.Members.OrderBy(e => e.Nickname).FirstOrDefault() == null),
            assertEmpty: true
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_subquery_equality_to_null_with_composite_key_should_match_nulls(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Squad>()
                .Where(s =>
                    s.Members.Where(m => m.FullName == "Anthony Carmine")
    ...
    public virtual Task Where_subquery_equality_to_null_without_composite_key(bool async) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<Gear>().Where(s => s.Weapons.OrderBy(e => e.Name).FirstOrDefault() == null),
            assertEmpty: true
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_subquery_equality_to_null_without_composite_key_should_match_null(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>()
                .Where(s =>
                    s.Weapons.Where(w => w.Name == "Hammer of Dawn")
                        .OrderBy(e => e.Name)
                        .FirstOrDefault() == null
                )
        );
    ...
    public virtual Task ElementAt_basic_with_OrderBy(bool async) =>
        AssertElementAt(async, ss => ss.Set<Gear>().OrderBy(g => g.FullName), () => 0);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task ElementAtOrDefault_basic_with_OrderBy(bool async) =>
        AssertElementAtOrDefault(async, ss => ss.Set<Gear>().OrderBy(g => g.FullName), () => 1);
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_subquery_with_ElementAtOrDefault_equality_to_null_with_composite_key(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<Squad>()
                .Where(s => s.Members.OrderBy(e => e.Nickname).ElementAtOrDefault(2) == null)
        );
    ...
    public virtual Task Where_subquery_with_ElementAt_using_column_as_index(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Squad>()
                    .Where(s =>
                        s.Members.OrderBy(m => m.Nickname).ElementAt(s.Id).Nickname == "Cole Train"
                    ),
            ss =>
                ss.Set<Squad>()
                    .Where(s =>
                        s.Members.OrderBy(m => m.Nickname).ElementAtOrDefault(s.Id).Nickname
                        == "Cole Train"
                    )
        );
    ...
    public virtual Task DateTimeOffset_to_unix_time_milliseconds(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Gear>()
                ...
                .Where(s =>
                    s.Squad.Missions.Where(m =>
                            unixEpochMilliseconds == m.Mission.Timeline.ToUnixTimeMilliseconds()
                        )
                        .FirstOrDefault() == null
                )
        );
    ...
    public virtual Task DateTimeOffset_to_unix_time_seconds(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Gear>()
                ...
                .Where(s =>
                    s.Squad.Missions.Where(m =>
                            unixEpochSeconds == m.Mission.Timeline.ToUnixTimeSeconds()
                        )
                        .FirstOrDefault() == null
                )
        );
    ...
    public virtual Task Set_operator_with_navigation_in_projection_groupby_aggregate(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Gear>()
                .Where(x =>
                    ss.Set<Gear>().Concat(ss.Set<Gear>()).Select(x => x.Nickname).Contains("Marcus")
                )
    ...
    public virtual Task Nav_expansion_inside_Contains_argument(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Gear>().Where(x => numbers.Contains(x.Weapons.Any() ? 1 : 0))
        );
    }
        ...
        return AssertQuery(
            ...
            ss =>
                ss.Set<Gear>()
                    .OrderBy(x => x.Nickname)
                    .Select(x =>
                        x.Weapons.OrderBy(g => g.Id)
                            .Take(numbers.OrderBy(xx => xx).Skip(1).FirstOrDefault())
                    ),
            assertOrder: true,
            elementAsserter: (e, a) => AssertCollection(e, a, ordered: true)
        );
    ...
    public virtual Task Nav_expansion_inside_Skip_correlated_to_source(bool async)
    {
        return AssertQuery(async, ss =>
            ss.Set<City>()
                .OrderBy(x => x.Name)
                .Select(x =>
                    x.BornGears.OrderBy(g => g.FullName).Skip(x.StationedGears.Any() ? 1 : 0)
                )
        );
    }
    ...
    public virtual Task Nav_expansion_inside_Take_correlated_to_source(bool async)
    {
        return AssertQuery(async, ss =>
            ss.Set<Gear>()
                .OrderBy(x => x.Nickname)
                .Select(x => x.Weapons.OrderBy(g => g.Id).Take(x.AssignedCity.Name.Length))
        );
    }

    [ConditionalTheory(Skip = "issue #32303")]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Nav_expansion_with_member_pushdown_inside_Take_correlated_to_source(
        bool async
    )
    {
        var numbers = new[] { 0, 1, 2 };
                ...
                ss.Set<Gear>()
                    ...
                    .Select(x =>
                        x.Weapons.OrderBy(g => g.Id)
                            .Take(
                                ss.Set<Gear>()
                                    .OrderBy(xx => xx.Nickname)
                                    .FirstOrDefault()
                                    .AssignedCity.Name.Length
                            )
                    ),
            assertOrder: true,
            elementAsserter: (e, a) => AssertCollection(e, a, ordered: true)
    ...
    public virtual Task Nav_expansion_inside_ElementAt_correlated_to_source(bool async)
    {
        return AssertQuery(async, ss =>
            ss.Set<Gear>()
                .OrderBy(x => x.Nickname)
                .Select(x => x.Weapons.OrderBy(g => g.Id).ElementAt(x.AssignedCity != null ? 1 : 0))
        );
    }
