        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Gear>()
                    .Where(
                        g =>
                            (g.LeaderNickname == null ? null : g.LeaderNickname.EndsWith("us"))
                            == true
                    )
        );
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Gear>()
                    .Where(
                        g =>
                            (g.LeaderNickname != null ? g.LeaderNickname.EndsWith("us") : null)
                            == true
                    )
        );
    ...
    public virtual Task Select_null_propagation_optimization7(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Gear>()
                    .Select(
                        g => null != g.LeaderNickname ? g.LeaderNickname + g.LeaderNickname : null
                    )
        );
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Gear>()
                    .OrderBy(g => g.Nickname)
                    .Where(
                        g => !g.Weapons.Distinct().OrderBy(w => w.Id).LastOrDefault().IsAutomatic
                    ),
            assertOrder: true
        );
                ...
                AssertQuery(
                    ...
                    ss =>
                        ss.Set<CogTag>()
                            ...
                            .OrderBy(t => t.Note)
                            .Select(
                                t => ss.Set<Gear>().OrderBy(g => g.Nickname).Skip(t.Gear.SquadId)
                            ),
                    assertOrder: true,
                ...
                AssertQuery(
                    ...
                    ss =>
                        ss.Set<CogTag>()
                            ...
                            .OrderBy(t => t.Note)
                            .Select(
                                t => ss.Set<Gear>().OrderBy(g => g.Nickname).Take(t.Gear.SquadId)
                            ),
                    assertOrder: true,
                    elementAsserter: (e, a) => AssertCollection(e, a, ordered: true)
                ),
    ...
    public virtual Task Select_required_navigation_on_derived_type(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<LocustLeader>()
                    .Select(
                        ll => ll is LocustCommander ? ((LocustCommander)ll).HighCommand.Name : null
                    )
        );
        ...
        AssertQueryScalar(
            ...
            ss =>
                ss.Set<Gear>()
                    .Select(
                        g =>
                            (int?)g.Weapons.OrderBy(w => w.Id).Select(w => w.Id).FirstOrDefault()
                            ?? 42
                    )
        );
    ...
    public virtual Task GetValueOrDefault_with_argument_complex(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Weapon>()
                    .Where(
                        w => (w.SynergyWithId == null ? w.Name.Length + 42 : w.SynergyWithId) > 10
                    )
        );
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Gear>()
                    ...
                    .Select(
                        g =>
                            new { gear = g, weapon = g.Weapons.OrderBy(w => w.Id).FirstOrDefault() }
                    ),
    ...
    public virtual Task GroupBy_with_boolean_groupin_key_thru_navigation_access(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<CogTag>()
                    .GroupBy(
                        t => new { HasSoulPatch = (bool?)t.Gear.HasSoulPatch, t.Gear.Squad.Name }
                    )
            ...
            ss =>
                ss.Set<CogTag>()
                    ...
                    .Select(
                        g => new { g.Key.HasSoulPatch, Name = g.Key.Name.Maybe(x => x.ToLower()) }
                    ),
    ...
    public virtual Task Enum_array_contains(bool async)
    {
        ...
        return AssertQuery(
            ...
            ss =>
                ss.Set<Weapon>()
                    .Where(
                        w => w.SynergyWith != null && types.Contains(w.SynergyWith.AmmunitionType)
                    )
        );
    }
    ...
    public virtual async Task Accessing_derived_property_using_hard_and_soft_cast(bool async)
    {
        ...
        await AssertQuery(
            ...
            ss =>
                ss.Set<LocustLeader>()
                    .Where(
                        ll => ll is LocustCommander && (ll as LocustCommander).HighCommandId != 0
                    )
        );
    }
    ...
    public virtual Task Projecting_entity_as_well_as_complex_correlated_collection_followed_by_Distinct(
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Gear>()
                    .Select(
                        g => new { g, Weapons = g.Weapons.Where(w => w.Id == g.SquadId).ToList() }
                    )
                    .Distinct(),
            elementSorter: e => e.g.FullName,
            elementAsserter: (e, a) =>
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Squad>()
                    .Where(
                        s =>
                            s.Members.OrderBy(m => m.Nickname).ElementAt(s.Id).Nickname
                            == "Cole Train"
                    ),
            ss =>
                ss.Set<Squad>()
        ...
        return AssertQuery(
            ...
            ss =>
                ss.Set<Gear>()
                    ...
                    .Where(
                        s =>
                            s.Squad.Missions.Where(
                                m =>
                                    unixEpochMilliseconds
                                    == m.Mission.Timeline.ToUnixTimeMilliseconds()
                            )
                                .FirstOrDefault() == null
                    )
        );
            ...
            ss =>
                ss.Set<Gear>()
                    ...
                    .Where(
                        s =>
                            s.Squad.Missions.Where(
                                m => unixEpochSeconds == m.Mission.Timeline.ToUnixTimeSeconds()
                            )
                                .FirstOrDefault() == null
                    )
        );
    ...
    public virtual Task Nav_expansion_with_member_pushdown_inside_Contains_argument(bool async)
    {
        ...
        return AssertQuery(
            ...
            ss =>
                ss.Set<Gear>()
                    .Where(
                        x => weapons.Contains(x.Weapons.OrderBy(w => w.Id).FirstOrDefault().Name)
                    )
        );
    }
        ...
        return AssertQuery(
            ...
            ss =>
                ss.Set<City>()
                    ...
                    .Select(
                        x =>
                            x.BornGears.OrderBy(g => g.FullName)
                                .Skip(x.StationedGears.Any() ? 1 : 0)
                    )
        );
    }
