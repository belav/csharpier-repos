    ...
    where TFixture : GearsOfWarQueryFixtureBase, new()
{
    ...
    public virtual Task Negate_on_binary_expression(bool async) =>
        AssertQuery(async, ss => ss.Set<Squad>().Where(s => s.Id == -(s.Id + s.Id)));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Negate_on_column(bool async) =>
        AssertQuery(async, ss => ss.Set<Squad>().Where(s => s.Id == -s.Id));
    ...
    public virtual Task Negate_on_like_expression(bool async) =>
        AssertQuery(async, ss => ss.Set<Squad>().Where(s => !s.Name.StartsWith("us")));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Entity_equality_empty(bool async) =>
        AssertQuery(async, ss => ss.Set<Gear>().Where(g => g == new Gear()));
...
#pragma warning restore IDE0031 // Use null propagation
                        ...
                        AssertQuery(
                            ...
                            ss =>
                                from lc1 in ss.Set<Faction>()
                                    .Select(
                                        f => (f is LocustHorde) ? ((LocustHorde)f).Commander : null
                                    )
    ...
    public virtual Task Where_TimeSpan_Minutes(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Duration.Minutes == 1));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_TimeSpan_Seconds(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Duration.Seconds == 1));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_TimeSpan_Milliseconds(bool async) =>
        AssertQuery(async, ss => ss.Set<Mission>().Where(m => m.Duration.Milliseconds == 1));
