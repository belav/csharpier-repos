    ...
    where TFixture : NorthwindQueryFixtureBase<NoopModelCustomizer>, new()
{
    ...
    public virtual Task Sum_with_no_arg(bool async) =>
        AssertSum(async, ss => ss.Set<Order>().Select(o => o.OrderID));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Sum_with_no_data_cast_to_nullable(bool async) =>
        AssertSum(async, ss =>
            ss.Set<Order>().Where(o => o.OrderID < 0).Select(o => (int?)o.OrderID)
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Sum_with_no_data_nullable(bool async) =>
        AssertSum(async, ss => ss.Set<Product>(), selector: o => o.SupplierID);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Sum_with_binary_expression(bool async) =>
        AssertSum(async, ss => ss.Set<Order>().Select(o => o.OrderID * 2));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Sum_with_no_arg_empty(bool async) =>
        AssertSum(async, ss => ss.Set<Order>().Where(o => o.OrderID == 42).Select(o => o.OrderID));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Sum_with_arg(bool async) =>
        AssertSum(async, ss => ss.Set<Order>(), selector: o => o.OrderID);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Sum_with_arg_expression(bool async) =>
        AssertSum(async, ss => ss.Set<Order>(), selector: o => o.OrderID + o.OrderID);
    ...
    public virtual Task Sum_over_subquery_is_client_eval(bool async) =>
        AssertSum(async, ss => ss.Set<Customer>(), selector: c => c.Orders.Sum(o => o.OrderID));
    ...
    public virtual Task Average_with_no_arg(bool async) =>
        AssertAverage(async, ss => ss.Set<Order>().Select(o => o.OrderID));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Average_with_binary_expression(bool async) =>
        AssertAverage(async, ss => ss.Set<Order>().Select(o => o.OrderID * 2));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Average_with_arg(bool async) =>
        AssertAverage(async, ss => ss.Set<Order>(), selector: o => o.OrderID);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Average_with_arg_expression(bool async) =>
        AssertAverage(async, ss => ss.Set<Order>(), selector: o => o.OrderID + o.OrderID);
    ...
    public virtual Task Average_over_subquery_is_client_eval(bool async) =>
        AssertAverage(async, ss => ss.Set<Customer>(), selector: c => c.Orders.Sum(o => o.OrderID));
    ...
    public virtual Task Average_over_nested_subquery_is_client_eval(bool async) =>
        AssertAverage(
            ...
            selector: c =>
                (decimal)c.Orders.Average(o => 5 + o.OrderDetails.Average(od => od.ProductID))
        );
    ...
    public virtual Task Average_over_max_subquery_is_client_eval(bool async) =>
        AssertAverage(
            ...
            ss => ss.Set<Customer>().OrderBy(c => c.CustomerID).Take(3),
            selector: c =>
                (decimal)c.Orders.Average(o => 5 + o.OrderDetails.Max(od => od.ProductID))
        );
    ...
    public virtual Task Min_with_no_arg(bool async) =>
        AssertMin(async, ss => ss.Set<Order>().Select(o => o.OrderID));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Min_with_arg(bool async) =>
        AssertMin(async, ss => ss.Set<Order>(), selector: o => o.OrderID);
    ...
    public virtual Task Min_no_data_subquery(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Customer>().Select(c => c.Orders.Where(o => o.OrderID == -1).Min(o => o.OrderID))
        );
    ...
    public virtual Task Max_no_data_subquery(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Customer>().Select(c => c.Orders.Where(o => o.OrderID == -1).Max(o => o.OrderID))
        );
    ...
    public virtual Task Average_no_data_subquery(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Customer>()
                .Select(c => c.Orders.Where(o => o.OrderID == -1).Average(o => o.OrderID))
        );
    ...
    public virtual Task Min_over_subquery_is_client_eval(bool async) =>
        AssertMin(async, ss => ss.Set<Customer>(), selector: c => c.Orders.Sum(o => o.OrderID));
    ...
    public virtual Task Max_with_no_arg(bool async) =>
        AssertMax(async, ss => ss.Set<Order>().Select(o => o.OrderID));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Max_with_arg(bool async) =>
        AssertMax(async, ss => ss.Set<Order>(), selector: o => o.OrderID);
    ...
    public virtual Task Max_over_subquery_is_client_eval(bool async) =>
        AssertMax(async, ss => ss.Set<Customer>(), selector: c => c.Orders.Sum(o => o.OrderID));
    ...
    public virtual Task Count_with_no_predicate(bool async) =>
        AssertCount(async, ss => ss.Set<Order>());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Count_with_predicate(bool async) =>
        AssertCount(async, ss => ss.Set<Order>(), predicate: o => o.CustomerID == "ALFKI");

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Count_with_order_by(bool async) =>
        AssertCount(async, ss => ss.Set<Order>().OrderBy(o => o.CustomerID));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_OrderBy_Count(bool async) =>
        AssertCount(async, ss =>
            ss.Set<Order>().Where(o => o.CustomerID == "ALFKI").OrderBy(o => o.OrderID)
        );
    ...
    public virtual Task OrderBy_Where_Count(bool async) =>
        AssertCount(async, ss =>
            ss.Set<Order>().OrderBy(o => o.OrderID).Where(o => o.CustomerID == "ALFKI")
        );
    ...
    public virtual Task OrderBy_client_Take(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Employee>().OrderBy(o => ClientEvalSelectorStateless()).Take(10)
        );
    ...
    public virtual Task Distinct(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Distinct());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Distinct_Scalar(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => c.City).Distinct());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task OrderBy_Distinct(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().OrderBy(c => c.CustomerID).Select(c => c.City).Distinct()
        );
    ...
    public virtual Task Distinct_Count(bool async) =>
        AssertCount(async, ss => ss.Set<Customer>().Distinct());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_Select_Distinct_Count(bool async) =>
        AssertCount(async, ss => ss.Set<Customer>().Select(c => c.City).Select(c => c).Distinct());
    ...
    public virtual Task Single_Predicate(bool async) =>
        AssertSingle(async, ss => ss.Set<Customer>(), predicate: c => c.CustomerID == "ALFKI");

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_Single(bool async) =>
        AssertSingle(async, ss => ss.Set<Customer>().Where(c => c.CustomerID == "ALFKI"));
    ...
    public virtual Task Where_SingleOrDefault(bool async) =>
        AssertSingleOrDefault(async, ss => ss.Set<Customer>().Where(c => c.CustomerID == "ALFKI"));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task First(bool async) =>
        AssertFirst(async, ss => ss.Set<Customer>().OrderBy(c => c.ContactName));
    ...
    public virtual Task FirstOrDefault(bool async) =>
        AssertFirstOrDefault(async, ss => ss.Set<Customer>().OrderBy(c => c.ContactName));
    ...
    public virtual Task First_inside_subquery_gets_client_evaluated(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c =>
                    c.CustomerID == "ALFKI"
                    && c.Orders.Where(o => o.CustomerID == "ALFKI").First().CustomerID == "ALFKI"
                )
        );
    ...
    public virtual Task Last(bool async) =>
        AssertLast(async, ss => ss.Set<Customer>().OrderBy(c => c.ContactName));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Last_when_no_order_by(bool async) =>
        AssertLast(async, ss => ss.Set<Customer>().Where(c => c.CustomerID == "ALFKI"));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task LastOrDefault_when_no_order_by(bool async) =>
        AssertLastOrDefault(async, ss => ss.Set<Customer>().Where(c => c.CustomerID == "ALFKI"));
    ...
    public virtual Task LastOrDefault(bool async) =>
        AssertLastOrDefault(async, ss => ss.Set<Customer>().OrderBy(c => c.ContactName));
    ...
    public virtual Task Contains_with_subquery(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c => ss.Set<Order>().Select(o => o.CustomerID).Contains(c.CustomerID))
        );
    ...
    public virtual async Task Contains_with_local_array_closure(bool async)
    {
        ...
        await AssertQuery(async, ss => ss.Set<Customer>().Where(c => ids.Contains(c.CustomerID)));
    ...
    public virtual async Task Contains_with_local_uint_array_closure(bool async)
    {
        ...
        await AssertQuery(async, ss => ss.Set<Employee>().Where(e => ids.Contains(e.EmployeeID)));
    ...
    public virtual Task Contains_with_local_array_inline(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => new[] { "ABCDE", "ALFKI" }.Contains(c.CustomerID))
        );
    ...
    public virtual Task Contains_with_local_list_closure(bool async)
    {
        ...
        return AssertQuery(async, ss => ss.Set<Customer>().Where(c => ids.Contains(c.CustomerID)));
    }
    ...
    public virtual Task Contains_with_local_object_list_closure(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c => ids.Contains(EF.Property<object>(c, nameof(Customer.CustomerID))))
        );
    }
    ...
    public virtual Task Contains_with_local_list_inline(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c => new List<string> { "ABCDE", "ALFKI" }.Contains(c.CustomerID))
        );
    ...
    public virtual async Task Contains_with_local_list_inline_closure_mix(bool async)
    {
        ...
        await AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => new List<string> { "ABCDE", id }.Contains(c.CustomerID))
        );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual async Task Contains_with_local_read_only_collection_inline_closure_mix(
        bool async
    )
    {
        var id = "ALFKI";
    ...
    public virtual Task Contains_with_local_non_primitive_list_closure_mix(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => ids.Select(i => i.CustomerID).Contains(c.CustomerID))
        );
    }
    ...
    public virtual Task Contains_with_local_collection_empty_inline(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => !(new List<string>().Contains(c.CustomerID)))
        );
    ...
    public virtual Task Contains_top_level(bool async) =>
        AssertSingleResult(
            ...
            asyncQuery: ss =>
                ss.Set<Customer>().Select(c => c.CustomerID).ContainsAsync("ALFKI", default)
        );
    ...
    public virtual Task Contains_with_local_tuple_array_closure(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<OrderDetail>()
                .Where(o => ids.Contains(new Tuple<int, int>(o.OrderID, o.ProductID)))
        );
    }
    ...
    public virtual Task Contains_with_local_anonymous_type_array_closure(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<OrderDetail>()
                .Where(o => ids.Contains(new { Id1 = o.OrderID, Id2 = o.ProductID }))
        );
    }
    ...
    public virtual Task OfType_Select(bool async) =>
        AssertFirst(async, ss =>
            ss.Set<Order>().OfType<Order>().OrderBy(o => o.OrderID).Select(o => o.Customer.City)
        );
    ...
    public virtual Task Min_with_non_matching_types_in_projection_introduces_explicit_cast(
        ...
        AssertMin(async, ss =>
            ss.Set<Order>().Where(o => o.CustomerID.StartsWith("A")).Select(o => (long)o.OrderID)
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task OrderBy_Take_Last_gives_correct_result(bool async) =>
        AssertLast(async, ss => ss.Set<Customer>().OrderBy(c => c.CustomerID).Take(20));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task OrderBy_Skip_Last_gives_correct_result(bool async) =>
        AssertLast(async, ss => ss.Set<Customer>().OrderBy(c => c.CustomerID).Skip(20));
    ...
    )
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => c.Orders.Contains(someOrder))
        );
    }
    ...
    public virtual Task Contains_with_parameter_list_value_type_id(bool async)
    {
        ...
        return AssertQuery(async, ss => ss.Set<Order>().Where(o => orders.Contains(o)));
    }
    ...
    public virtual Task Array_cast_to_IEnumerable_Contains_with_constant(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Where(c => _customers.Contains(c.CustomerID)));
    ...
    public virtual Task Contains_over_entityType_with_null_should_rewrite_to_identity_equality_subquery(
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Order>()
                    .Where(o => ss.Set<Order>().Where(o => o.CustomerID == "VINET").Contains(null)),
            assertEmpty: true
        );
    ...
    public virtual Task Contains_over_entityType_should_materialize_when_composite(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<OrderDetail>().Where(o => o.ProductID == 42 && ss.Set<OrderDetail>().Contains(o))
        );
    ...
    public virtual Task String_FirstOrDefault_in_projection_does_not_do_client_eval(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Customer>().Select(c => c.CustomerID.FirstOrDefault())
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_constant_Sum(bool async) =>
        AssertSum(async, ss => ss.Set<Employee>(), selector: e => 1);
    ...
    public virtual Task Where_subquery_any_equals_operator(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => ids.Any(li => li == c.CustomerID))
        );
    }
    ...
    public virtual Task Where_subquery_any_equals(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c => new[] { "ABCDE", "ALFKI", "ANATR" }.Any(li => li.Equals(c.CustomerID)))
        );
    ...
    public virtual Task Where_subquery_any_equals_static(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => ids.Any(li => Equals(li, c.CustomerID)))
        );
    }
    ...
    public virtual async Task Where_subquery_where_any(bool async)
    {
        ...
        await AssertQuery(async, ss =>
            ss.Set<Customer>()
                ...
                .Where(c => ids.Any(li => li == c.CustomerID))
        );

        await AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c => c.City == "México D.F.")
                .Where(c => ids.Any(li => c.CustomerID == li))
        );
    }
    ...
    public virtual Task Where_subquery_all_not_equals_operator(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => ids.All(li => li != c.CustomerID))
        );
    }
    ...
    public virtual Task Where_subquery_all_not_equals_static(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => ids.All(li => !Equals(li, c.CustomerID)))
        );
    }
    ...
    public virtual async Task Where_subquery_where_all(bool async)
    {
        ...
        await AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c => c.City == "México D.F.")
                .Where(c => ids.All(li => li != c.CustomerID))
        );

        await AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c => c.City == "México D.F.")
                .Where(c => ids.All(li => c.CustomerID != li))
        );
    }
    ...
    public virtual Task Cast_to_same_Type_Count_works(bool async) =>
        AssertCount(async, ss => ss.Set<Customer>().Cast<Customer>());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Cast_before_aggregate_is_preserved(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Customer>().Select(c => c.Orders.Select(o => (double?)o.OrderID).Average())
        );
    ...
    public virtual Task Enumerable_min_is_mapped_to_Queryable_1(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Customer>().Select(c => c.Orders.Min(o => (double?)o.OrderID))
        );
    ...
    public virtual Task Enumerable_min_is_mapped_to_Queryable_2(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Customer>().Select(c => c.Orders.Select(o => (double?)o.OrderID).Min())
        );
    ...
    public virtual Task Sum_over_explicit_cast_over_column(bool async) =>
        AssertSum(async, ss => ss.Set<Order>(), o => (long?)o.OrderID);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual async Task Count_on_projection_with_client_eval(bool async)
    {
        await AssertCount(async, ss => ss.Set<Order>().Select(o => o.OrderID.ToString("000000")));

        await AssertCount(async, ss =>
            ss.Set<Order>().Select(o => new { Id = o.OrderID.ToString("000000") })
        );

        await AssertCount(async, ss =>
            ss.Set<Order>().Select(o => new { Id = CodeFormat(o.OrderID) })
        );
    }
    ...
    public virtual Task Sum_over_empty_returns_zero(bool async) =>
        AssertSum(async, ss => ss.Set<Order>().Where(o => o.OrderID == 42), o => o.OrderID);
    ...
    public virtual Task Average_after_default_if_empty_does_not_throw(bool async) =>
        AssertAverage(async, ss =>
            ss.Set<Order>().Where(o => o.OrderID == 10243).Select(o => o.OrderID).DefaultIfEmpty()
        );
    ...
    public virtual Task All_true(bool async) =>
        AssertAll(async, ss => ss.Set<Customer>(), predicate: x => true);
    ...
    public virtual Task Not_Any_false(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => !c.Orders.Any(o => false)).Select(c => c.CustomerID)
        );
}
