...
{
    protected NorthwindAggregateOperatorsQueryTestBase(TFixture fixture) : base(fixture)
    {
    }
    ...
    public virtual Task Distinct(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Distinct(), entryCount: 91);
    ...
    public virtual Task First(bool async) =>
        AssertFirst(async, ss => ss.Set<Customer>().OrderBy(c => c.ContactName), entryCount: 1);
    ...
    public virtual Task Last(bool async) =>
        AssertLast(async, ss => ss.Set<Customer>().OrderBy(c => c.ContactName), entryCount: 1);
