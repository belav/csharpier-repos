    ...
    where TFixture : NorthwindQueryFixtureBase<NoopModelCustomizer>, new()
{
    ...
    public virtual async Task Multiple_context_instances_parameter(bool async)
    {
        ...
        var message = async
            ? (
                await Assert.ThrowsAsync<InvalidOperationException>(() => queryAsync(context2))
            ).Message
        ...
        Customer query(NorthwindContext c2) =>
            (from c in context1.Customers from o in c2.Orders select c).First();

        Task<Customer> queryAsync(NorthwindContext c2) =>
            (from c in context1.Customers from o in c2.Orders select c).FirstAsync();
    }

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual async Task Query_when_evaluatable_queryable_method_call_with_repository(
        bool async
    )
    {
    ...
    public virtual Task Lifting_when_subquery_nested_order_by_simple(bool async) =>
        AssertQuery(async, ss =>
            from c1_Orders in ss.Set<Order>()
            join _c1 in (
                from c1 in (from c in ss.Set<Customer>() orderby c.CustomerID select c).Take(2)
    ...
    public virtual void Query_composition_against_ienumerable_set()
    {
        ...
        var results = orders
            .Where(x =>
                x.OrderDate < new DateTime(1996, 7, 12) && x.OrderDate > new DateTime(1996, 7, 4)
            )
    ...
    {
        public static bool Check(string input1, string input2) => false;
    }
...
#pragma warning restore CS1718 // Comparison made to same variable
    ...
    public virtual Task Join_with_entity_equality_local_on_both_sources(bool async)
    {
        ...
        return AssertQuery(async, ss => (from c1 in ss.Set<Customer>() where c1 == local select c1)
                .Join(
                    from c2 in ss.Set<Customer>()
                    where c2 == local
                    select c2,
    ...
    public virtual Task Entity_equality_null(bool async) =>
        AssertQuery(
            async,
            ss => from c in ss.Set<Customer>() where c == null select c.CustomerID,
            assertEmpty: true
        );
    ...
    public virtual Task Entity_equality_null_composite_key(bool async) =>
        AssertQuery(
            async,
            ss => from od in ss.Set<OrderDetail>() where od == null select od,
            assertEmpty: true
        );
    ...
    public virtual Task Entity_equality_not_null(bool async) =>
        AssertQuery(async, ss => from c in ss.Set<Customer>() where c != null select c.CustomerID);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Entity_equality_not_null_composite_key(bool async) =>
        AssertQuery(async, ss => from od in ss.Set<OrderDetail>() where od != null select od);
    ...
    public virtual Task Entity_equality_orderby(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().OrderBy(c => c), assertOrder: true);
    ...
    public virtual Task Entity_equality_orderby_subquery(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .OrderBy(c =>
                        c.Orders.FirstOrDefault() == null
                            ? (int?)null
                            : c.Orders.FirstOrDefault().OrderID
                    ),
            assertOrder: true
        );
    ...
    public virtual Task Queryable_simple_anonymous(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => new { c }), e => e.c.CustomerID);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Queryable_simple_anonymous_projection_subquery(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().Take(91).Select(c => new { c }).Select(a => a.c.City)
        );
    ...
    public virtual Task Queryable_simple_anonymous_subquery(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().Select(c => new { c }).Take(91).Select(a => a.c)
        );
    ...
    public virtual Task Queryable_nested_simple(bool async) =>
        AssertQuery(async, ss =>
            from c1 in (from c2 in (from c3 in ss.Set<Customer>() select c3) select c2)
            select c1
        );
    ...
    public virtual Task Ternary_should_not_evaluate_both_sides(bool async)
    {
        ...
        return AssertQuery(async, ss => ss.Set<Customer>()
                .Select(c => new
                {
                    c.CustomerID,
    ...
    public virtual Task Ternary_should_not_evaluate_both_sides_with_parameter(bool async)
    {
        ...
        return AssertQuery(async, ss => ss.Set<Order>()
                .Select(o => new
                {
    ...
    public virtual Task Null_Coalesce_Short_Circuit(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Distinct()
                .Select(c => new { Customer = c, Test = test ?? values.Contains(1) })
        );
    }
    ...
    public virtual Task Skip_Distinct(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().OrderBy(c => c.ContactName).Skip(5).Distinct());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Skip_Take_Distinct(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().OrderBy(c => c.ContactName).Skip(5).Take(10).Distinct()
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Skip_Take_Any(bool async) =>
        AssertAny(async, ss => ss.Set<Customer>().OrderBy(c => c.ContactName).Skip(5).Take(10));
    ...
    public virtual Task Take_Skip_Distinct(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().OrderBy(c => c.ContactName).Take(10).Skip(5).Distinct()
        );
    ...
    public virtual async Task Take_Skip_Distinct_Caching(bool async)
    {
        ...
        await AssertQuery(async, ss =>
            ss.Set<Customer>().OrderBy(c => c.ContactName).Take(15).Skip(10).Distinct()
        );
    }
    ...
    public virtual Task Take_Distinct(bool async) =>
        AssertQuery(async, ss => ss.Set<Order>().OrderBy(o => o.OrderID).Take(5).Distinct());
    ...
    public virtual Task Distinct_Take_Count(bool async) =>
        AssertCount(async, ss => ss.Set<Order>().Distinct().Take(5));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Take_Distinct_Count(bool async) =>
        AssertCount(async, ss => ss.Set<Order>().Take(5).Distinct());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Take_Where_Distinct_Count(bool async) =>
        AssertCount(async, ss =>
            ss.Set<Order>().Where(o => o.CustomerID == "FRANK").Take(5).Distinct()
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Any_simple(bool async) => AssertAny(async, ss => ss.Set<Customer>());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task OrderBy_Take_Count(bool async) =>
        AssertCount(async, ss => ss.Set<Order>().OrderBy(o => o.OrderID).Take(5));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Take_OrderBy_Count(bool async) =>
        AssertCount(async, ss => ss.Set<Order>().Take(5).OrderBy(o => o.OrderID));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Any_predicate(bool async) =>
        AssertAny(async, ss => ss.Set<Customer>(), predicate: c => c.ContactName.StartsWith("A"));
    ...
    public virtual Task Any_nested_negated(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(c => !ss.Set<Order>().Any(o => o.CustomerID.StartsWith("A"))),
            assertEmpty: true
        );
    ...
    public virtual Task Any_nested_negated2(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c =>
                    c.City != "London" && !ss.Set<Order>().Any(o => o.CustomerID.StartsWith("ABC"))
                )
        );
    ...
    public virtual Task Any_nested_negated3(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c =>
                    !ss.Set<Order>().Any(o => o.CustomerID.StartsWith("ABC")) && c.City != "London"
                )
        );
    ...
    public virtual Task Any_nested(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => ss.Set<Order>().Any(o => o.CustomerID.StartsWith("A")))
        );
    ...
    public virtual Task Any_nested2(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c =>
                    c.City != "London" && ss.Set<Order>().Any(o => o.CustomerID.StartsWith("A"))
                )
        );
    ...
    public virtual Task Any_nested3(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c =>
                    ss.Set<Order>().Any(o => o.CustomerID.StartsWith("A")) && c.City != "London"
                )
        );
    ...
    public virtual Task All_top_level(bool async) =>
        AssertAll(async, ss => ss.Set<Customer>(), predicate: c => c.ContactName.StartsWith("A"));
        ...
        AssertSingleResult(
            ...
            syncQuery: ss =>
                ss.Set<Customer>()
                    .All(c1 =>
                        ss.Set<Customer>()
                            .Any(c2 => ss.Set<Customer>().Any(c3 => c1.CustomerID == c3.CustomerID))
                    ),
            asyncQuery: ss =>
                ss.Set<Customer>()
                    .AllAsync(
                        c1 =>
                            ss.Set<Customer>()
                                .Any(c2 =>
                                    ss.Set<Customer>().Any(c3 => c1.CustomerID == c3.CustomerID)
                                ),
                        default
                    )
        );
                    ...
                    .All(c1 =>
                        ss.Set<Customer>()
                            .Any(c2 =>
                                ss.Set<Customer>()
                                    .Any(c3 =>
                                        EF.Property<string>(c1, "CustomerID") == c3.CustomerID
                                    )
                            )
                    ),
            asyncQuery: ss =>
    ...
    public virtual Task Take_with_single(bool async) =>
        AssertSingle(async, ss => ss.Set<Customer>().OrderBy(c => c.CustomerID).Take(1));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Take_with_single_select_many(bool async) =>
        AssertSingle(async, ss =>
            (
                from c in ss.Set<Customer>()
                from o in ss.Set<Order>()
    ...
    public virtual Task Cast_results_to_object(bool async) =>
        AssertQuery(async, ss => from c in ss.Set<Customer>().Cast<object>() select c);
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_subquery_expression(bool async) => AssertQuery(async, ss =>
        {
    ...
    public virtual Task Where_subquery_expression_same_parametername(bool async) =>
        AssertQuery(async, ss =>
        {
            ...
            return ss.Set<Order>()
                .Where(x =>
                    ss.Set<Order>().Where(expr).Where(o => o.CustomerID == x.CustomerID).Any()
                );
        });
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_DTO_constructor_distinct_with_navigation_translated_to_server(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_DTO_constructor_distinct_with_collection_projection_translated_to_server(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual async Task Select_DTO_constructor_distinct_with_collection_projection_translated_to_server_with_binding_after_client_eval(
        bool async
    )
    {
        using var context = CreateContext();
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_DTO_with_member_init_distinct_in_subquery_used_in_projection_translated_to_server(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_correlated_subquery_filtered_returning_queryable_throws(
        bool async
    ) =>
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_correlated_subquery_ordered_returning_queryable_in_DTO_throws(
        bool async
    ) =>
        AssertInvalidMaterializationType(
            () =>
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_nested_collection_in_anonymous_type_returning_ordered_queryable(
        bool async
    ) =>
        AssertInvalidMaterializationType(
            () =>
                AssertQuery(
                    ...
                    ss =>
                        ...
                        select new
                        {
                            ...
                            OrderIds = ss.Set<Order>()
                                .Where(o =>
                                    o.CustomerID == c.CustomerID && o.OrderDate.Value.Year == 1997
                                )
                                .Select(o => o.OrderID)
                                .OrderBy(o => o),
                            Customer = c,
                ...
                AssertQuery(
                    ...
                    ss =>
                        ...
                        select (
                            ...
                            select (from e3 in ss.Set<Employee>() orderby e3.EmployeeID select e3)
                        ),
                    elementSorter: e => e.Count(),
                    elementAsserter: (e, a) =>
                        AssertCollection(
    ...
    public virtual Task Select_subquery_recursive_trivial(bool async) =>
        AssertQuery(
            ...
            ss =>
                ...
                select (
                    from e2 in ss.Set<Employee>()
                    select (from e3 in ss.Set<Employee>() orderby e3.EmployeeID select e3).ToList()
                ).ToList(),
            elementSorter: e => e.Count(),
            elementAsserter: (e, a) =>
                AssertCollection(
    ...
    public virtual Task Where_query_composition(bool async) =>
        AssertQuery(async, ss =>
            ...
            where
                e1.FirstName
                == ss.Set<Employee>().OrderBy(e => e.EmployeeID).FirstOrDefault().FirstName
            select e1
        );
    ...
    public virtual Task Where_query_composition_is_null(bool async) =>
        AssertQuery(async, ss =>
            from e1 in ss.Set<Employee>().OrderBy(e => e.EmployeeID).Take(3)
    ...
    public virtual Task Where_query_composition_is_not_null(bool async) =>
        AssertQuery(async, ss =>
            from e1 in ss.Set<Employee>().OrderBy(e => e.EmployeeID).Skip(4).Take(3)
            where ss.Set<Employee>().SingleOrDefault(e2 => e2.EmployeeID == e1.ReportsTo) != null
            select e1
        );
    ...
    public virtual Task Where_query_composition_entity_equality_one_element_SingleOrDefault(
        ...
        AssertQuery(
            ...
            ss =>
                from e1 in ss.Set<Employee>()
                where
                    ss.Set<Employee>().SingleOrDefault(e2 => e2.EmployeeID == e1.ReportsTo)
                    == new Employee()
                select e1,
            ss =>
                from e1 in ss.Set<Employee>()
                where
                    ss.Set<Employee>().FirstOrDefault(e2 => e2.EmployeeID == e1.ReportsTo)
                    == new Employee()
                select e1,
            assertEmpty: true
        );
    ...
    public virtual Task Where_query_composition_entity_equality_one_element_Single(bool async) =>
        AssertQuery(
            ...
            ss =>
                from e1 in ss.Set<Employee>()
                where
                    ss.Set<Employee>().Single(e2 => e2.EmployeeID == e1.ReportsTo) == new Employee()
                select e1,
            ss =>
                from e1 in ss.Set<Employee>()
                where
    ...
    public virtual Task Where_query_composition_entity_equality_one_element_First(bool async) =>
        AssertQuery(
            ...
            ss =>
                from e1 in ss.Set<Employee>()
                where
                    ss.Set<Employee>().First(e2 => e2.EmployeeID == e1.ReportsTo) == new Employee()
                select e1,
            ss =>
                from e1 in ss.Set<Employee>()
                where
    ...
    public virtual Task Where_query_composition_entity_equality_no_elements_SingleOrDefault(
        ...
        AssertQuery(
            ...
            ss =>
                from e1 in ss.Set<Employee>()
                where
                    ss.Set<Employee>().SingleOrDefault(e2 => e2.EmployeeID == 42) == new Employee()
                select e1,
            ss =>
                from e1 in ss.Set<Employee>()
    ...
    public virtual Task Where_query_composition_entity_equality_multiple_elements_SingleOrDefault(
        ...
        AssertQuery(
            ...
            ss =>
                ...
                where
                    ss.Set<Employee>()
                        .OrderBy(e2 => e2.EmployeeID)
                        .SingleOrDefault(e2 => e2.EmployeeID != e1.ReportsTo)
            ...
            ss =>
                ...
                where
                    ss.Set<Employee>()
                        .OrderBy(e2 => e2.EmployeeID)
                        .FirstOrDefault(e2 => e2.EmployeeID != e1.ReportsTo)
                        .EmployeeID == 1
                select e1
        );
    ...
    public virtual Task Where_query_composition2(bool async) =>
        AssertQuery(async, ss => from e1 in ss.Set<Employee>().Take(3) where
                e1.FirstName
                == (
    ...
    public virtual Task Where_query_composition2_FirstOrDefault(bool async) =>
        AssertQuery(async, ss => from e1 in ss.Set<Employee>().Take(3) where
                e1.FirstName
                == (from e2 in ss.Set<Employee>().OrderBy(e => e.EmployeeID) select e2)
                    .FirstOrDefault()
                    .FirstName select e1);
        ...
        AssertTranslationFailedWithDetails(
            () =>
                AssertQuery(async, ss =>
                    ...
                    where
                        c1.IsLondon
                        == ss.Set<Customer>().OrderBy(c => c.CustomerID).First().IsLondon
                    select c1
                ),
            CoreStrings.QueryUnableToTranslateMember(nameof(Customer.IsLondon), nameof(Customer))
        );
        ...
        AssertQuery(
            ...
            ss =>
                ...
                where
                    (
                        from e2 in ss.Set<Employee>()
                        where (from e3 in ss.Set<Employee>() orderby e3.EmployeeID select e3).Any()
                        select e2
                    ).Any()
    ...
    public virtual Task SelectMany_simple1(bool async) =>
        AssertQuery(
            async,
            ss => from e in ss.Set<Employee>() from c in ss.Set<Customer>() select new { c, e },
            e => (e.c.CustomerID, e.e.EmployeeID)
        );
    ...
    public virtual Task SelectMany_correlated_subquery_hard(bool async) =>
        AssertQuery(
            async,
            ss =>
                from c1 in (from c2 in ss.Set<Customer>().Take(91) select c2.City).Distinct()
                from e1 in (
                    from e2 in ss.Set<Employee>()
                    where c1 == e2.City
                    select new { e2.City, c1 }
                ).Take(9)
                from e2 in (from e3 in ss.Set<Employee>() where e1.City == e3.City select c1).Take(
                    9
                )
                select new { c1, e1 },
        ...
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c =>
                    ...
                    && c.Orders.Any(o => o.OrderDate == new DateTime(1998, 1, 15))
                )
        );
    ...
    public virtual Task Multiple_joins_Where_Order_Any(bool async) =>
        AssertAny(async, ss => ss.Set<Customer>()
                .Join(
                    ss.Set<Order>(),
                    c => c.CustomerID,
                    o => o.CustomerID,
    ...
    public virtual Task Default_if_empty_top_level_positive(bool async) =>
        AssertQuery(async, ss =>
            from e in ss.Set<Employee>().Where(c => c.EmployeeID > 0).DefaultIfEmpty()
            select e
        );
    ...
    public virtual Task OrderBy_true(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().OrderBy(c => true).Select(c => c));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task OrderBy_integer(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().OrderBy(c => 3).Select(c => c));
    ...
    {
        var param = 5;
        return AssertQuery(async, ss => ss.Set<Customer>().OrderBy(c => param).Select(c => c));
    }
    ...
    public virtual Task OrderBy_shadow(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Employee>()
                    .OrderBy(e => EF.Property<string>(e, "Title"))
        ...
        AssertQuery(async, ss =>
            ss.Set<Order>()
                .Where(o =>
                    o.OrderID <= 10250
                    && ss.Set<Customer>()
                        .OrderBy(c => ss.Set<Customer>().Any(c2 => c2.CustomerID == "ALFKI"))
                        .FirstOrDefault()
                        .City != "Nowhere"
                )
        );
    ...
    public virtual Task OrderBy_Select(bool async) =>
        AssertQuery(
            async,
            ss => ss.Set<Customer>().OrderBy(c => c.CustomerID).Select(c => c.ContactName),
            ss =>
                ss.Set<Customer>()
                    .OrderBy(c => c.CustomerID, StringComparer.Ordinal)
    ...
    public virtual Task OrderByDescending(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .OrderByDescending(c => c.CustomerID, StringComparer.Ordinal)
                    .Select(c => c.City),
            assertOrder: true
        );
    ...
    public virtual Task OrderBy_ThenBy_Any(bool async) =>
        AssertAny(async, ss =>
            ss.Set<Customer>().OrderBy(c => c.CustomerID).ThenBy(c => c.ContactName)
        );
    ...
    public virtual Task OrderBy_arithmetic(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Employee>().OrderBy(e => e.EmployeeID - e.EmployeeID).Select(e => e)
        );
    ...
    public virtual Task OrderBy_ternary_conditions(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Product>()
                    .OrderBy(p => p.UnitsInStock > 10 ? p.ProductID > 40 : p.ProductID <= 40)
    ...
    public virtual Task OrderBy_any(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .OrderBy(p => p.Orders.Any(o => o.OrderID > 11000))
                    .ThenBy(p => p.CustomerID),
            assertOrder: true
        );
    ...
    public virtual Task Select_many_cross_join_same_collection(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().SelectMany(c => ss.Set<Customer>()));
...
#pragma warning restore IDE0029 // Use coalesce expression
    ...
    public virtual Task OrderBy_comparison_operator(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().OrderBy(c => c.Region == "ASK").Select(c => c));
    ...
    public virtual Task Filter_coalesce_operator(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => (c.CompanyName ?? c.ContactName) == "The Big Cheese")
        );
    ...
    public virtual Task Take_skip_null_coalesce_operator(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().OrderBy(c => c.Region ?? "ZZ").Take(10).Skip(5).Distinct()
        );
    ...
    public virtual Task Select_take_skip_null_coalesce_operator3(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().OrderBy(c => c.Region ?? "ZZ").Take(10).Skip(5)
        );
    ...
    public virtual Task Select_Property_when_shadow_unconstrained_generic_method(bool async) =>
        AssertQuery(async, ss =>
            ShadowPropertySelect<Employee, string>(ss.Set<Employee>(), "Title")
        );
    ...
    public virtual Task Where_Property_when_shadow_unconstrained_generic_method(bool async) =>
        AssertQuery(async, ss =>
            ShadowPropertyWhere(ss.Set<Employee>(), "Title", "Sales Representative")
        );
    ...
    protected IQueryable<T> ShadowPropertyWhere<T>(
        IQueryable<T> source,
        object column,
        string value
    ...
    public virtual async Task Where_Property_shadow_closure(bool async)
    {
        ...
        await AssertQuery(async, ss =>
            ss.Set<Employee>().Where(e => EF.Property<string>(e, propertyName) == value)
        );
    ...
    public virtual Task Selected_column_can_coalesce(bool async) =>
        AssertQuery(async, ss =>
            (from c in ss.Set<Customer>() orderby c.Region ?? "ZZ" select c).Select(x => x)
        );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_Where_Subquery_Deep_Single(bool async) => AssertQuery(async, ss => (
                from od in ss.Set<OrderDetail>().Where(od => od.OrderID == 10344)
                where
                    (
                        from o in ss.Set<Order>()
                        where od.OrderID == o.OrderID
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_Where_Subquery_Deep_First(bool async) => AssertQuery(async, ss => (
                from od in ss.Set<OrderDetail>()
                where
                    (
                        from o in ss.Set<Order>()
    ...
    public virtual Task Select_Where_Subquery_Equality(bool async) =>
        AssertQuery(
            async,
            ss =>
                from o in ss.Set<Order>().OrderBy(o => o.OrderID).Take(1)
                // ReSharper disable once UseMethodAny.0
                where
                    (
                        from od in ss.Set<OrderDetail>().OrderBy(od => od.OrderID).Take(2)
                        where
                            (
                                from c in ss.Set<Customer>()
                                where c.CustomerID == o.CustomerID
    ...
    public virtual Task DateTime_parse_is_inlined(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Order>().Where(o => o.OrderDate > DateTime.Parse("1/1/1998 12:00:00 PM"))
        );
    ...
    public virtual Task DateTime_parse_is_parameterized_when_from_closure(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Order>().Where(o => o.OrderDate > DateTime.Parse(date))
        );
    }
    ...
    public virtual Task New_DateTime_is_inlined(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Order>().Where(o => o.OrderDate > new DateTime(1998, 1, 1, 12, 0, 0))
        );
    ...
    public virtual async Task New_DateTime_is_parameterized_when_from_closure(bool async)
    {
        ...
        await AssertQuery(async, ss =>
            ss.Set<Order>().Where(o => o.OrderDate > new DateTime(year, month, date, hour, 0, 0))
        );
    ...
    public virtual Task Random_next_is_not_funcletized_2(bool async) =>
        AssertQuery(async, ss => ss.Set<Order>().Where(o => o.OrderID > Random.Shared.Next(5)));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Random_next_is_not_funcletized_3(bool async) =>
        AssertQuery(async, ss => ss.Set<Order>().Where(o => o.OrderID > Random.Shared.Next(0, 10)));
    ...
    public virtual Task Random_next_is_not_funcletized_5(bool async) =>
        AssertQuery(async, ss => ss.Set<Order>().Where(o => o.OrderID > new Random(15).Next(5)));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Random_next_is_not_funcletized_6(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Order>().Where(o => o.OrderID > new Random(15).Next(0, 10))
        );
    ...
    public virtual Task Concat_string_int(bool isAsync) =>
        AssertQuery(isAsync, ss => ss.Set<Order>().Select(o => o.OrderID + o.CustomerID));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Concat_int_string(bool isAsync) =>
        AssertQuery(isAsync, ss => ss.Set<Order>().Select(o => o.CustomerID + o.OrderID));
    ...
    {
        var parameter = "-";
        return AssertQuery(isAsync, ss => ss.Set<Order>().Select(o => parameter + o.OrderID));
    }

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Concat_constant_string_int(bool isAsync) =>
        AssertQuery(isAsync, ss => ss.Set<Order>().Select(o => "-" + o.OrderID));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task String_concat_with_navigation1(bool async) =>
        AssertQuery(async, ss => ss.Set<Order>().Select(o => o.CustomerID + " " + o.Customer.City));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task String_concat_with_navigation2(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Order>().Select(o => o.Customer.City + " " + o.Customer.City)
        );
    ...
    public virtual Task Where_bitwise_or_with_logical_or(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c =>
                    c.CustomerID == "ALFKI" | c.CustomerID == "ANATR" || c.CustomerID == "ANTON"
                )
        );
    ...
    public virtual Task Where_bitwise_or_with_logical_and(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c =>
                    c.CustomerID == "ALFKI" | c.CustomerID == "ANATR" && c.Country == "Germany"
                )
        );
    ...
    public virtual Task Where_bitwise_binary_not(bool async)
    {
        ...
        return AssertQuery(async, ss => ss.Set<Order>().Where(o => ~o.OrderID == negatedId));
    }

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Where_bitwise_binary_and(bool async) =>
        AssertQuery(async, ss => ss.Set<Order>().Where(o => (o.OrderID & 10248) == 10248));
    ...
    public virtual Task Query_expression_with_to_string_and_contains(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Order>()
                    .Where(o => o.OrderDate != null && o.EmployeeID.Value.ToString().Contains("7"))
        ...
        AssertQuery(
            ...
            ss =>
                (
                    ...
                    from o1 in ss.Set<Order>().Where(o => o.OrderID > 11050).DefaultIfEmpty()
                    from o2 in ss.Set<Order>()
                        .Where(o => o.OrderID <= c.CustomerID.Length + 10250)
                        .DefaultIfEmpty()
                    where o1 != null && o2 != null
                    orderby o1.OrderID, o2.OrderDate
                    select new
    ...
    public virtual Task OrderBy_coalesce_take_distinct(bool async) =>
        AssertQuery(
            async,
            ss => ss.Set<Product>().OrderBy(p => p.UnitPrice ?? 0).Take(15).Distinct(),
            assertOrder: false
        );
    ...
    public virtual Task OrderBy_coalesce_skip_take_distinct(bool async) =>
        AssertQuery(
            async,
            ss => ss.Set<Product>().OrderBy(p => p.UnitPrice ?? 0).Skip(5).Take(15).Distinct(),
            assertOrder: false
        );
...
#pragma warning restore IDE0031 // Use null propagation
    ...
    public virtual async Task Contains_with_DateTime_Date(bool async)
    {
        ...
        await AssertQuery(async, ss =>
            ss.Set<Order>().Where(e => dates.Contains(e.OrderDate.Value.Date))
        );
        ...
        AssertSingleResult(
            ...
            syncQuery: ss =>
                ss.Set<Customer>()
                    .Select(c => new { c.CustomerID })
                    .Distinct()
                    .Count(n => n.CustomerID.StartsWith("A")),
            asyncQuery: ss =>
                ss.Set<Customer>()
                    .Select(c => new { c.CustomerID })
                    .Distinct()
                    .CountAsync(n => n.CustomerID.StartsWith("A"), default)
        );
    ...
    public virtual Task Anonymous_complex_distinct_where(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Select(c => new { A = c.CustomerID + c.City })
                    .Distinct()
                    .Where(n => n.A == "ALFKIBerlin"),
            e => e.A
        );
    ...
    public virtual Task Anonymous_complex_orderby(bool async) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<Customer>().Select(c => new { A = c.CustomerID + c.City }).OrderBy(n => n.A),
            assertOrder: true
        );
    ...
    public virtual Task DTO_member_distinct_where(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Select(c => new DTO<string> { Property = c.CustomerID })
                    .Distinct()
    ...
    public virtual Task DTO_complex_distinct_where(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Select(c => new DTO<string> { Property = c.CustomerID + c.City })
                    .Distinct()
        ...
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                ...
                .Where(outer =>
                    (
                        from c in ss.Set<Customer>()
                        let customers = ss.Set<Customer>()
                            .Where(cc =>
                                ss.Set<Customer>()
                                    .OrderBy(inner => inner.CustomerID)
                                    .Take(10)
                                    .Distinct()
                                    .Any()
                            )
    ...
    public virtual Task Int16_parameter_can_be_used_for_int_column(bool async)
    {
        ...
        return AssertQuery(async, ss => ss.Set<Order>().Where(o => o.OrderID == parameter));
    }
    ...
    public virtual Task Select_take_average(bool async) =>
        AssertAverage(async, ss =>
            ss.Set<Order>().OrderBy(o => o.OrderID).Select(o => o.OrderID).Take(10)
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_take_count(bool async) =>
        AssertCount(async, ss => ss.Set<Customer>().Take(7));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_orderBy_take_count(bool async) =>
        AssertCount(async, ss => ss.Set<Customer>().OrderBy(c => c.Country).Take(7));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_take_long_count(bool async) =>
        AssertLongCount(async, ss => ss.Set<Customer>().Take(7));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_orderBy_take_long_count(bool async) =>
        AssertLongCount(async, ss => ss.Set<Customer>().OrderBy(c => c.Country).Take(7));
    ...
    public virtual Task Select_skip_average(bool async) =>
        AssertAverage(async, ss =>
            ss.Set<Order>().OrderBy(o => o.OrderID).Select(o => o.OrderID).Skip(10)
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_skip_count(bool async) =>
        AssertCount(async, ss => ss.Set<Customer>().Skip(7));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_orderBy_skip_count(bool async) =>
        AssertCount(async, ss => ss.Set<Customer>().OrderBy(c => c.Country).Skip(7));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_skip_long_count(bool async) =>
        AssertLongCount(async, ss => ss.Set<Customer>().Skip(7));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_orderBy_skip_long_count(bool async) =>
        AssertLongCount(async, ss => ss.Set<Customer>().OrderBy(c => c.Country).Skip(7));
    ...
    public virtual Task Select_distinct_average(bool async) =>
        AssertAverage(async, ss => ss.Set<Order>().Select(o => o.OrderID).Distinct());
    ...
    public virtual Task Select_distinct_long_count(bool async) =>
        AssertLongCount(async, ss => ss.Set<Customer>().Distinct());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_distinct_max(bool async) =>
        AssertMax(async, ss => ss.Set<Order>().Select(o => o.OrderID).Distinct());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_distinct_min(bool async) =>
        AssertMin(async, ss => ss.Set<Order>().Select(o => o.OrderID).Distinct());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_distinct_sum(bool async) =>
        AssertSum(async, ss => ss.Set<Order>().Select(o => o.OrderID).Distinct());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Comparing_to_fixed_string_parameter(bool async) =>
        AssertQuery(async, ss => FindLike(ss.Set<Customer>(), "A"));
    ...
    public virtual Task Join_take_count_works(bool async) =>
        AssertCount(async, ss =>
            (
                from o in ss.Set<Order>().Where(o => o.OrderID > 690 && o.OrderID < 710)
    ...
    public virtual Task OrderBy_empty_list_contains(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<Customer>().OrderBy(c => list.Contains(c.CustomerID)).Select(c => c)
        );
    }
    ...
    public virtual Task Manual_expression_tree_typed_null_equality(bool async)
    {
        ...
        var orderCustomer = Expression.MakeMemberAccess(
            orderParameter,
            typeof(Order).GetMember(nameof(Order.Customer))[0]
        );

        var selector = Expression.Lambda<Func<Order, string>>(
            Expression.Condition(
                Expression.NotEqual(orderCustomer, Expression.Constant(null, typeof(Customer))),
        ...
        return AssertQuery(async, ss =>
            ss.Set<Order>().Where(o => o.OrderID < 10300).Select(selector)
        );
    }
    ...
    public virtual Task Let_subquery_with_multiple_occurrences(bool async) =>
        AssertQuery(async, ss =>
            from o in ss.Set<Order>()
            let details = from od in o.OrderDetails
...
#pragma warning restore IDE0031 // Use null propagation
    ...
    public virtual Task Collection_navigation_equal_to_null_for_subquery(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(c =>
                        c.Orders.OrderBy(o => o.OrderID).FirstOrDefault().OrderDetails == null
                    ),
            ss =>
                ss.Set<Customer>()
    ...
    public virtual Task Dependent_to_principal_navigation_equal_to_null_for_subquery(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(c => c.Orders.OrderBy(o => o.OrderID).FirstOrDefault().Customer == null),
            ss =>
                ss.Set<Customer>()
                    .Where(c =>
    ...
    public virtual Task Inner_parameter_in_nested_lambdas_gets_preserved(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c =>
                    c.Orders.Where(o => c == new Customer { CustomerID = o.CustomerID }).Count() > 0
                )
        );
    ...
    public virtual Task Convert_to_nullable_on_nullable_value_is_ignored(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Order>().Select(o => new Order { OrderDate = o.OrderDate.Value })
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Navigation_inside_interpolated_string_is_expanded(bool async) =>
        AssertQuery(async, ss => ss.Set<Order>().Select(o => $"CustomerCity:{o.Customer.City}"));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Client_code_using_instance_method_throws(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => InstanceMethod(c)));
    ...
    public virtual Task Client_code_using_instance_in_static_method(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => StaticMethod(this, c)));

    private static string StaticMethod(
        NorthwindMiscellaneousQueryTestBase<TFixture> containingClass,
        Customer c
    ) => c.City;
    ...
    public virtual Task Client_code_using_instance_in_anonymous_type(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => new { A = this }));
    ...
    public virtual Task Entity_equality_with_null_coalesce_client_side(bool async)
    {
        ...
        return AssertQuery(async, ss => ss.Set<Customer>().Where(c => c == (a ?? b)));
    }
    ...
    public virtual Task EF_Property_include_on_incorrect_property_throws(bool async) =>
        Assert.ThrowsAsync<InvalidOperationException>(async () =>
            await AssertQuery(async, ss =>
                ss.Set<Customer>().Include(c => EF.Property<Customer>(c, "OrderDetails"))
            )
        );
    ...
    public virtual async Task Perform_identity_resolution_reuses_same_instances(
        bool async,
        bool useAsTracking
    )
    {
        ...
        var query = context.Orders.Where(o => orderIds.Contains(o.OrderID)).Select(o => o.Customer);
    ...
    )
    {
        ...
        var query = (
            from c in context.Customers.Where(c => c.CustomerID.StartsWith("A"))
    ...
    public virtual Task Using_string_Equals_with_StringComparison_throws_informative_error(
        ...
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c => c.CustomerID.Equals("ALFKI", StringComparison.InvariantCulture))
        );
    ...
    public virtual Task Using_static_string_Equals_with_StringComparison_throws_informative_error(
        ...
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                .Where(c => string.Equals(c.CustomerID, "ALFKI", StringComparison.InvariantCulture))
        );
    ...
    public virtual Task ToList_over_string(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .OrderBy(c => c.CustomerID)
                    .Select(e => new { Property = e.City.ToList() }),
            assertOrder: true,
    ...
    public virtual Task ToArray_over_string(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .OrderBy(c => c.CustomerID)
                    .Select(e => new { Property = e.City.ToArray() }),
            assertOrder: true,
            elementAsserter: (e, a) => Assert.True(e.Property.SequenceEqual(a.Property))
        );
    ...
    public virtual Task AsEnumerable_over_string(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .OrderBy(c => c.CustomerID)
                    .Select(e => new { Property = e.City.AsEnumerable() }),
            assertOrder: true,
            elementAsserter: (e, a) => Assert.True(e.Property.SequenceEqual(a.Property))
        );
    ...
    public virtual async Task Non_nullable_property_through_optional_navigation(bool async) =>
        Assert.Equal(
            ...
            (
                await Assert.ThrowsAsync<InvalidOperationException>(() =>
                    AssertQuery(async, ss =>
                        ss.Set<Customer>().Select(e => new { e.Region.Length })
                    )
                )
            ).Message
        );
    ...
    public virtual Task DefaultIfEmpty_over_empty_collection_followed_by_projecting_constant(
        ...
        AssertFirstOrDefault(async, ss =>
            ss.Set<Customer>().Where(c => false).DefaultIfEmpty().Select(c => "520")
        );
    ...
    public virtual async Task Skip_0_Take_0_works_when_parameter(bool async)
    {
        ...
        await AssertQuery(async, ss =>
            ss.Set<Customer>().OrderBy(c => c.CustomerID).Skip(1).Take(1)
        );
    }
        ...
        for (var i = 0; i < 10; i++)
        {
            ...
            if (exception != null)
            {
                ...
                Assert.Contains(
                    ...
                    Fixture.ListLoggerFactory.Log.Select(l => l.Id)
                );
            }
            else
            {
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Collection_navigation_equal_to_null_for_subquery_using_ElementAtOrDefault_constant_zero(
        bool async
    ) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<Customer>()
                    .Where(c =>
                        c.Orders.OrderBy(o => o.OrderID).ElementAtOrDefault(0).OrderDetails == null
                    ),
            ss =>
                ss.Set<Customer>()
                    .Where(c => c.Orders.OrderBy(o => o.OrderID).ElementAtOrDefault(0) == null)
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Collection_navigation_equal_to_null_for_subquery_using_ElementAtOrDefault_constant_one(
        bool async
    ) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<Customer>()
                    .Where(c =>
                        c.Orders.OrderBy(o => o.OrderID).ElementAtOrDefault(1).OrderDetails == null
                    ),
            ss =>
                ss.Set<Customer>()
                    .Where(c => c.Orders.OrderBy(o => o.OrderID).ElementAtOrDefault(1) == null)
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Collection_navigation_equal_to_null_for_subquery_using_ElementAtOrDefault_parameter(
        bool async
    )
    {
        var prm = 2;
        ...
        return AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(c =>
                        c.Orders.OrderBy(o => o.OrderID).ElementAtOrDefault(prm).OrderDetails
                        == null
                    ),
            ss =>
                ss.Set<Customer>()
                    .Where(c => c.Orders.OrderBy(o => o.OrderID).ElementAtOrDefault(prm) == null)
        );
    }
    ...
    public virtual Task Subquery_with_navigation_inside_inline_collection(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>().Where(c => new[] { 100, c.Orders.Count }.Sum() > 101)
        );
}
