...
{
    protected NorthwindMiscellaneousQueryTestBase(TFixture fixture) : base(fixture) { }
    ...
    public virtual void Query_composition_against_ienumerable_set()
    {
        ...
        var results = orders
            .Where(
                x =>
                    x.OrderDate < new DateTime(1996, 7, 12)
                    && x.OrderDate > new DateTime(1996, 7, 4)
            )
...
#pragma warning restore CS1718 // Comparison made to same variable
                ...
                ss.Set<Customer>()
                    .Select(
                        c =>
                            ...
                            {
                                Result = test
                                    ?? values.Select(c2 => c2.CustomerID).Contains(c.CustomerID)
                            }
                    )
        );
    }
                                ...
                                .Any(
                                    c2 =>
                                        ss.Set<Customer>()
                                            .Any(
                                                c3 =>
                                                    EF.Property<string>(c1, "CustomerID")
                                                    == c3.CustomerID
                                            )
                                )
                    ),
                ...
                return ss.Set<Order>()
                    .Where(
                        x =>
                            ss.Set<Order>()
                                .Where(expr)
                                .Where(o => o.CustomerID == x.CustomerID)
                                .Any()
                    );
            },
            entryCount: 5
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(
                        c =>
                            c.CustomerID == "ALFKI"
                            && c.Orders.Any(o => o.OrderDate == new DateTime(2008, 10, 24))
                    )
        );
                        ...
                        ss.Set<Customer>()
                            .Where(
                                c =>
                                    c.CustomerID == "ALFKI"
                                    && c.Orders.Exists(
                                        o => o.OrderDate == new DateTime(2008, 10, 24)
                                    )
                            )
                )
        );
    ...
    public virtual Task OrderBy_integer(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().OrderBy(c => 3).Select(c => c), entryCount: 91);
                ...
                ss.Set<Order>()
                    .Where(
                        o =>
                            ...
                            && ss.Set<Customer>()
                                .OrderBy(
                                    c => ss.Set<Customer>().Any(c2 => c2.CustomerID == "ALFKI")
                                )
                                .FirstOrDefault()
                                .City != "Nowhere"
                    ),
...
#pragma warning restore IDE0029 // Use coalesce expression
    ...
    public virtual Task OrderBy_comparison_operator(bool async) =>
        AssertQuery(
            async,
            ss => ss.Set<Customer>().OrderBy(c => c.Region == "ASK").Select(c => c),
            entryCount: 91
        );
    ...
    public virtual Task Random_next_is_not_funcletized_4(bool async) =>
        AssertQuery(async, ss => ss.Set<Order>().Where(o => o.OrderID > new Random(15).Next()));
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(
                        c =>
                            c.CustomerID == "ALFKI" | c.CustomerID == "ANATR"
                            || c.CustomerID == "ANTON"
                    ),
            entryCount: 3
        );
                    ...
                    .Select(
                        o =>
                            new
                            {
                                OrderId = o.OrderID,
                                ss.Set<Customer>()
                                    .SingleOrDefault(c => c.CustomerID == o.CustomerID)
                                    .City
                            }
                    )
                    .OrderBy(o => o.City),
    ...
    public virtual Task Query_expression_with_to_string_and_contains(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Order>()
                    .Where(o => o.OrderDate != null && o.EmployeeID.Value.ToString().Contains("10"))
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Order>()
                    ...
                    .OrderBy(o => o.OrderID)
                    .Select(
                        o => new { Test = new DateTime(1900, 1, 1).AddMinutes(o.OrderID % 25) }
                    ),
            assertOrder: true,
        ...
        AssertQuery(
            ...
            ss =>
                (
                    ...
                    from o1 in ss.Set<Order>().Where(o => o.OrderID > 15000).DefaultIfEmpty()
                    from o2 in ss.Set<Order>()
                        .Where(o => o.OrderID <= c.CustomerID.Length)
                        .DefaultIfEmpty()
                    where o1 != null && o2 != null
                    orderby o1.OrderID, o2.OrderDate
                    select new
                        ...
                        c =>
                            new
                            {
                                ...
                                OuterOrders = c.Orders
                                    .Select(
                                        o =>
                                            new { InnerOrder = c.Orders.Count(), Id = c.CustomerID }
                                    )
                                    .ToList()
                            }
                    ),
...
#pragma warning restore IDE0031 // Use null propagation
    ...
    public virtual Task Collection_navigation_equal_to_null_for_subquery(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(
                        c => c.Orders.OrderBy(o => o.OrderID).FirstOrDefault().OrderDetails == null
                    ),
            ss =>
                ss.Set<Customer>()
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(
                        c =>
                            c.Orders
                                .Where(o => c == new Customer { CustomerID = o.CustomerID })
                                .Count() > 0
                    ),
            entryCount: 89
        );
    ...
    public virtual Task Client_code_using_instance_method_throws(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Select(c => InstanceMethod(c)), entryCount: 91);
    ...
    public virtual Task Using_static_string_Equals_with_StringComparison_throws_informative_error(
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(
                        c => string.Equals(c.CustomerID, "ALFKI", StringComparison.InvariantCulture)
                    ),
            entryCount: 1
        );
                        ...
                        c =>
                            new
                            {
                                ...
                                Order = (c.Orders.Any() ? c.Orders.FirstOrDefault() : null) == null
                                    ...
                                    : new
                                    {
                                        c.Orders.OrderBy(o => o.OrderID).FirstOrDefault().OrderDate
                                    }
                            }
                    ),
            elementSorter: c => c.CustomerID,
    ...
    public class TestDto
    {
        ...
        public static readonly Expression<Func<Customer, TestDto>> Projection = x =>
            new TestDto
            {
                CustomerID = x.CustomerID,
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(
                        c =>
                            c.Orders.OrderBy(o => o.OrderID).ElementAtOrDefault(0).OrderDetails
                            == null
                    ),
            ss =>
                ss.Set<Customer>()
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(
                        c =>
                            c.Orders.OrderBy(o => o.OrderID).ElementAtOrDefault(1).OrderDetails
                            == null
                    ),
            ss =>
                ss.Set<Customer>()
