    ...
    where TFixture : ManyToManyQueryFixtureBase, new()
{
    ...
    public virtual Task Skip_navigation_any_without_predicate(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<EntityOne>()
                    .Where(e => e.ThreeSkipPayloadFull.Where(e => e.Name.Contains("B")).Any()),
            assertEmpty: true
        );
    ...
    public virtual Task Skip_navigation_contains(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<EntityOne>()
                    .Where(e => e.ThreeSkipPayloadFullShared.Contains(new EntityThree { Id = 1 })),
            ss =>
                ss.Set<EntityOne>()
                    .Where(e => e.ThreeSkipPayloadFullShared.Select(i => i.Id).Contains(1))
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Skip_navigation_count_without_predicate(bool async) =>
        AssertQuery(async, ss => ss.Set<EntityOne>().Where(e => e.SelfSkipPayloadLeft.Count > 0));
    ...
    public virtual Task Skip_navigation_count_with_predicate(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<EntityOne>()
                    .OrderBy(e => e.BranchSkip.Count(e => e.Name.StartsWith("L")))
                    .ThenBy(e => e.Id),
            assertOrder: true
        );
    ...
    public virtual Task Skip_navigation_long_count_without_predicate(bool async) =>
        AssertQuery(async, ss => ss.Set<EntityTwo>().Where(e => e.ThreeSkipFull.LongCount() > 0));
    ...
    public virtual Task Skip_navigation_select_subquery_max(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<EntityTwo>().Select(e => e.OneSkip.Max(e => e.Id)));
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<EntityCompositeKey>()
                    .OrderBy(e => e.Key1)
                    .Select(e => e.LeafSkipFull.Cast<EntityRoot>()),
            assertOrder: true,
    ...
    public virtual Task Skip_navigation_of_type(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<EntityCompositeKey>()
                    .OrderBy(e => e.Key1)
                    .Select(e => e.RootSkipShared.OfType<EntityLeaf>()),
            assertOrder: true,
            elementAsserter: (e, a) => AssertCollection(e, a)
        );
        ...
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertInclude(
                    e,
                    a,
                    new ExpectedInclude<EntityCompositeKey>(et => et.RootSkipShared)
                )
        );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Include_skip_navigation_then_include_reference_and_skip_navigation(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Include_skip_navigation_then_include_inverse_works_for_tracking_query(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    public virtual Task Filtered_include_skip_navigation_where(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<EntityThree>()
                    .Include(e => e.OneSkipPayloadFullShared.Where(i => i.Id < 10)),
            elementAsserter: (e, a) =>
                AssertInclude(
                    e,
                    a,
                    new ExpectedFilteredInclude<EntityThree, EntityOne>(
                        ...
                        includeFilter: x => x.Where(i => i.Id < 10)
                    )
                )
        );
            ...
            elementAsserter: (e, a) =>
                AssertInclude(
                    ...
                    new ExpectedFilteredInclude<EntityThree, EntityTwo>(
                        et => et.TwoSkipFull,
                        includeFilter: x => x.OrderBy(i => i.Id)
                    )
                )
        );
    ...
    public virtual Task Filtered_include_skip_navigation_order_by_skip(bool async) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<EntityTwo>().Include(e => e.SelfSkipSharedRight.OrderBy(i => i.Id).Skip(2)),
            elementAsserter: (e, a) =>
                AssertInclude(
                    e,
                    a,
                    new ExpectedFilteredInclude<EntityTwo, EntityTwo>(
                        et => et.SelfSkipSharedRight,
                        includeFilter: x => x.OrderBy(i => i.Id).Skip(2)
                    )
                )
        );
    ...
    public virtual Task Filtered_include_skip_navigation_order_by_take(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<EntityCompositeKey>()
                    .Include(e => e.TwoSkipShared.OrderBy(i => i.Id).Take(2)),
            elementAsserter: (e, a) =>
                AssertInclude(
                    e,
                    a,
                    new ExpectedFilteredInclude<EntityCompositeKey, EntityTwo>(
                        et => et.TwoSkipShared,
                        includeFilter: x => x.OrderBy(i => i.Id).Take(2)
                    )
                )
        );
    ...
    public virtual Task Filtered_include_skip_navigation_order_by_skip_take(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<EntityCompositeKey>()
                    .Include(e => e.ThreeSkipFull.OrderBy(i => i.Id).Skip(1).Take(2)),
            elementAsserter: (e, a) =>
                AssertInclude(
                    e,
                    a,
                    new ExpectedFilteredInclude<EntityCompositeKey, EntityThree>(
                        et => et.ThreeSkipFull,
                        includeFilter: x => x.OrderBy(i => i.Id).Skip(1).Take(2)
                    )
                )
        );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Filtered_include_skip_navigation_where_then_include_skip_navigation(
        bool async
    ) =>
        AssertQuery(
            async,
            ...
            elementAsserter: (e, a) =>
                AssertInclude(
                    ...
                    new ExpectedFilteredInclude<EntityLeaf, EntityCompositeKey>(
                        et => et.CompositeKeySkipFull,
                        includeFilter: x => x.Where(i => i.Key1 < 5)
                    ),
                    new ExpectedInclude<EntityCompositeKey>(
                        et => et.TwoSkipShared,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Filtered_include_skip_navigation_order_by_skip_take_then_include_skip_navigation_where(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Filtered_include_skip_navigation_order_by_skip_take_then_include_skip_navigation_where_EF_Property(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Filtered_include_skip_navigation_where_then_include_skip_navigation_order_by_skip_take(
        bool async
    ) =>
        AssertQuery(
            async,
        ...
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertInclude(
                    e,
                    a,
                    new ExpectedFilteredInclude<EntityTwo, EntityOne>(
                        et => et.OneSkip,
                        includeFilter: x => x.Where(i => i.Id < 10)
                    ),
                    new ExpectedInclude<EntityOne>(et => et.Reference, "OneSkip"),
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Filter_include_on_skip_navigation_combined_with_filtered_then_includes(
        bool async
    ) =>
        AssertQuery(
            async,
            ...
            elementAsserter: (e, a) =>
                AssertInclude(
                    ...
                    a,
                    new ExpectedFilteredInclude<EntityThree, EntityOne>(
                        et => et.OneSkipPayloadFull,
                        includeFilter: x => x.Where(i => i.Id < 10)
                    ),
                    new ExpectedFilteredInclude<EntityOne, EntityTwo>(
                        et => et.TwoSkip,
                        "OneSkipPayloadFull",
                        includeFilter: x => x.OrderBy(e => e.Id).Skip(1).Take(2)
                    ),
                    new ExpectedFilteredInclude<EntityOne, EntityBranch>(
                        et => et.BranchSkip,
                        "OneSkipPayloadFull",
                        includeFilter: x => x.Where(e => e.Id < 20)
                    )
                )
        );
    ...
    public virtual async Task Throws_when_different_filtered_include(bool async) =>
        Assert.Equal(
            CoreStrings
                .MultipleFilteredIncludesOnSameNavigation(
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Filtered_include_on_skip_navigation_then_filtered_include_on_navigation(
        bool async
    ) =>
        AssertQuery(
            async,
            ...
            elementAsserter: (e, a) =>
                AssertInclude(
                    ...
                    new ExpectedFilteredInclude<EntityThree, EntityOne>(
                        et => et.OneSkipPayloadFull,
                        includeFilter: x => x.Where(i => i.Id > 15)
                    ),
                    new ExpectedFilteredInclude<EntityOne, EntityTwo>(
                        et => et.Collection,
                        "OneSkipPayloadFull",
                        includeFilter: x => x.Where(i => i.Id < 5)
                    )
                )
        );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Filtered_include_on_navigation_then_filtered_include_on_skip_navigation(
        bool async
    ) =>
        AssertQuery(
            async,
            ...
            elementAsserter: (e, a) =>
                AssertInclude(
                    ...
                    new ExpectedInclude<EntityThree>(
                        e => e.CollectionInverse,
                        "ThreeSkipPayloadFull"
                    ),
                    new ExpectedInclude<EntityOne>(e => e.JoinThreePayloadFull),
                    new ExpectedInclude<JoinOneToThreePayloadFull>(
                        e => e.Three,
                        "JoinThreePayloadFull"
                    ),
                    new ExpectedInclude<EntityThree>(
                        e => e.ReferenceInverse,
                        "JoinThreePayloadFull.Three"
                    ),
                    new ExpectedInclude<EntityThree>(
                        e => e.ReferenceInverse,
                        "ThreeSkipPayloadFull"
        ...
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertInclude(
                    ...
                    new ExpectedFilteredInclude<EntityOne, EntityTwo>(
                        e => e.Collection,
                        "JoinOnePayloadFull.One",
                        includeFilter: x => x.Where(i => i.Id < 5)
                    ),
                    new ExpectedInclude<EntityTwo>(
                        e => e.Reference,
                        "OneSkipPayloadFull.Collection"
                    ),
                    new ExpectedInclude<EntityTwo>(
                        e => e.Reference,
                        "JoinOnePayloadFull.One.Collection"
                    )
                )
        );
    ...
    [ConditionalTheory(Skip = "Issue#21332")]
    [MemberData(nameof(IsAsyncData))]
    public virtual async Task Throws_when_different_filtered_then_include_via_different_paths(
        bool async
    ) =>
        Assert.Equal(
            CoreStrings
    ...
    public virtual Task Skip_navigation_any_with_predicate_unidirectional(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<UnidirectionalEntityOne>()
                    .Where(e => e.TwoSkipShared.Any(e => e.Name.Contains("B"))),
            assertEmpty: true
        );
    ...
    public virtual Task Join_with_skip_navigation_unidirectional(bool async) =>
        AssertQuery(
            async,
            ss =>
                from t in ss.Set<UnidirectionalEntityTwo>()
        ...
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertInclude(
                    e,
                    a,
                    new ExpectedInclude<UnidirectionalEntityCompositeKey>(et => et.RootSkipShared)
                )
        );
    ...
    public virtual Task Filtered_include_skip_navigation_order_by_unidirectional(bool async) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<UnidirectionalEntityThree>().Include(e => e.TwoSkipFull.OrderBy(i => i.Id)),
            elementAsserter: (e, a) =>
                AssertInclude(
                    e,
                    a,
    ...
    public virtual Task Filtered_include_skip_navigation_order_by_skip_unidirectional(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<UnidirectionalEntityTwo>()
                    .Include(e => e.SelfSkipSharedRight.OrderBy(i => i.Id).Skip(2)),
            elementAsserter: (e, a) =>
                AssertInclude(
                    e,
                    a,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Filtered_include_skip_navigation_order_by_skip_take_unidirectional(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Filtered_include_skip_navigation_where_then_include_skip_navigation_unidirectional(
        bool async
    ) =>
        AssertQuery(
            async,
            ...
            elementAsserter: (e, a) =>
                AssertInclude(
                    ...
                    new ExpectedInclude<UnidirectionalEntityCompositeKey>(
                        et => et.TwoSkipShared,
                        "CompositeKeySkipFull"
                    )
                )
    ...
    public virtual Task Contains_on_skip_collection_navigation_unidirectional(bool async)
    {
        ...
        return AssertQuery(
            ...
            ss =>
                ss.Set<UnidirectionalEntityOne>()
                    .Where(e => e.TwoSkip.Select(i => i.Id).Contains(two.Id))
        );
    }
    ...
    public virtual Task GetType_in_hierarchy_in_querying_base_type_unidirectional(bool async) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<UnidirectionalEntityBranch>()
                    .Where(e => e.GetType() == typeof(UnidirectionalEntityRoot)),
            assertEmpty: true
        );
