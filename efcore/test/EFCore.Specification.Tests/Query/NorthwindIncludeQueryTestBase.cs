    ...
    where TFixture : NorthwindQueryFixtureBase<NoopModelCustomizer>, new()
{
    ...
    public virtual Task Include_property_expression_invalid(bool async) =>
        Assert.ThrowsAsync<InvalidOperationException>(() =>
            AssertQuery(async, ss =>
                ss.Set<Order>().Include(o => new { o.Customer, o.OrderDetails })
            )
        );
    ...
    public virtual async Task Include_closes_reader(bool async)
    {
        ...
        {
            Assert.NotNull(
                await context.Set<Customer>().Include(c => c.Orders).FirstOrDefaultAsync()
            );
    ...
    public virtual Task Include_when_result_operator(bool async) =>
        AssertAny(async, ss => ss.Set<Customer>().Include(c => c.Orders));
    ...
    public virtual Task Include_collection(bool async) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<Customer>().Where(c => c.CustomerID.StartsWith("F")).Include(c => c.Orders),
            elementAsserter: (e, a) =>
    ...
    public virtual Task Include_collection_with_last_no_orderby(bool async) =>
        AssertLast(async, ss => ss.Set<Customer>().Include(c => c.Orders));
    ...
    public virtual Task Include_collection_alias_generation(bool async) =>
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<Order>(o => o.OrderDetails))
        );
    ...
    public virtual async Task Include_collection_dependent_already_tracked(bool async)
    {
        ...
        var customer = async
            ...
            : context.Set<Customer>().Include(c => c.Orders).Single(c => c.CustomerID == "ALFKI");
    ...
    public virtual Task Include_collection_on_additional_from_clause(bool async) =>
        AssertQuery(
            async,
            ss =>
                from c1 in ss.Set<Customer>().OrderBy(c => c.CustomerID).Take(5)
                from c2 in ss.Set<Customer>()
                    .Where(c2 => c2.CustomerID.StartsWith("F"))
                    .Include(c2 => c2.Orders)
                select c2,
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<Customer>(c => c.Orders))
        );
    ...
    public virtual Task Include_collection_when_projection(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Include("Orders").Select(c => c.CustomerID));
    ...
    public virtual Task Include_duplicate_reference2(bool async) =>
        AssertQuery(
            async,
            ss =>
                from o1 in ss.Set<Order>().Include(o => o.Customer).OrderBy(o => o.OrderID).Take(2)
    ...
    public virtual Task Include_duplicate_reference3(bool async) =>
        AssertQuery(
            async,
            ss =>
                from o1 in ss.Set<Order>().OrderBy(o => o.OrderID).Take(2)
                from o2 in ss.Set<Order>()
                    .OrderBy(o => o.OrderID)
                    .Include(o => o.Customer)
                    .Skip(2)
    ...
    public virtual Task Include_reference(bool async) =>
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<Order>(o => o.Customer))
        );
    ...
    public virtual async Task Include_reference_alias_generation(bool async) =>
        await AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<OrderDetail>(od => od.Order))
        );
    ...
    public virtual Task Include_reference_when_projection(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Order>().Include(o => o.Customer).Select(o => o.CustomerID)
        );
    ...
    public virtual Task Include_references_and_collection_multi_level(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<OrderDetail>()
                    .Where(od => od.OrderID % 23 == 13 && od.UnitPrice < 10)
    ...
    public virtual Task Include_collection_Join_GroupBy_Select(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Order>()
                .Where(o => o.OrderID == 10248)
                .Include(o => o.OrderDetails)
                .Join(ss.Set<OrderDetail>(), o => o.OrderID, od => od.OrderID, (o, od) => o)
                .GroupBy(e => e.OrderID)
                .Select(e => e.OrderBy(o => o.OrderID).FirstOrDefault())
        );
    ...
    public virtual Task Include_reference_SelectMany_GroupBy_Select(bool async) =>
        AssertQuery(async, ss =>
            (
                from o in ss.Set<Order>().Include(o => o.Customer).Where(o => o.OrderID == 10248)
                from od in ss.Set<OrderDetail>()
                select o
            )
                .GroupBy(e => e.OrderID)
    ...
    public virtual Task Include_empty_reference_sets_IsLoaded(bool async) =>
        AssertFirst(
            ...
            e => e.Manager == null,
            asserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<Employee>(emp => emp.Manager))
        );
    ...
    public virtual Task Include_is_not_ignored_when_projection_contains_client_method_and_complex_expression(
        ...
        AssertQuery(async, ss =>
            from e in ss.Set<Employee>().Include(e => e.Manager)
    ...
    public virtual Task Multi_level_includes_are_applied_with_skip(bool async) =>
        AssertFirst(
            ...
            {
                AssertEqual(e.CustomerID, a.CustomerID);
                AssertCollection(e.Orders, a.Orders, elementAsserter: (eo, ao) =>
            ...
            elementAsserter: (e, a) =>
                AssertInclude(
                    ...
                    new ExpectedFilteredInclude<Customer, Order>(
                        c => c.Orders,
                        includeFilter: os =>
                            os.OrderBy(o => o.OrderID).Skip(1).OrderByDescending(o => o.OrderDate),
                        assertOrder: true
                    )
                )
        );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Include_with_cycle_does_not_throw_when_AsTracking_NoTrackingWithIdentityResolution(
        bool async
    ) =>
        AssertQuery(async, ss =>
            (
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Outer_identifier_correctly_determined_when_doing_include_on_right_side_of_left_join(
        bool async
    ) =>
        AssertQuery(
            async,
            ss =>
                from cust in ss.Set<Customer>()
