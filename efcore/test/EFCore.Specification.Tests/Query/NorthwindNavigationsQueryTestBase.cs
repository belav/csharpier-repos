...
{
    protected NorthwindNavigationsQueryTestBase(TFixture fixture) : base(fixture) { }
    ...
    public virtual Task Select_Where_Navigation_Scalar_Equals_Navigation_Scalar(bool async) =>
        AssertQuery(
            async,
            ss =>
                from o1 in ss.Set<Order>().Where(o => o.OrderID < 10300)
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_collection_FirstOrDefault_project_anonymous_type_client_eval(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    public virtual Task Select_Where_Navigation_Null(bool async) =>
        AssertQuery(
            async,
            ss => from e in ss.Set<Employee>() where e.Manager == null select e,
            entryCount: 1
        );
    ...
    public virtual Task Select_Where_Navigation_Null_Reverse(bool async) =>
        AssertQuery(
            async,
            ss => from e in ss.Set<Employee>() where null == e.Manager select e,
            entryCount: 1
        );
    ...
    public virtual Task Select_Navigation(bool async) =>
        AssertQuery(async, ss => from o in ss.Set<Order>() select o.Customer, entryCount: 89);
    ...
    public virtual Task Select_Navigations(bool async) =>
        AssertQuery(
            async,
            ss => from o in ss.Set<Order>() select new { A = o.Customer, B = o.Customer },
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Select_collection_navigation_simple_followed_by_ordering_by_scalar(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    public virtual Task Collection_select_nav_prop_any(bool async) =>
        AssertQuery(
            async,
            ss => from c in ss.Set<Customer>() select new { Any = c.Orders.Any() },
            ss =>
                from c in ss.Set<Customer>()
    ...
    public virtual Task Collection_where_nav_prop_any(bool async) =>
        AssertQuery(
            async,
            ss => from c in ss.Set<Customer>() where c.Orders.Any() select c,
            ss => from c in ss.Set<Customer>() where (c.Orders ?? new List<Order>()).Any() select c,
            entryCount: 89
        );
    ...
    public virtual Task Collection_where_nav_prop_any_predicate(bool async) =>
        AssertQuery(
            async,
            ss => from c in ss.Set<Customer>() where c.Orders.Any(o => o.OrderID > 0) select c,
            ss =>
                from c in ss.Set<Customer>()
    ...
    public virtual Task Collection_select_nav_prop_count(bool async) =>
        AssertQuery(
            async,
            ss => from c in ss.Set<Customer>() select new { c.Orders.Count },
            ss => from c in ss.Set<Customer>() select new { (c.Orders ?? new List<Order>()).Count },
    ...
    public virtual Task Collection_where_nav_prop_count(bool async) =>
        AssertQuery(
            async,
            ss => from c in ss.Set<Customer>() where c.Orders.Count() > 5 select c,
            ss =>
                from c in ss.Set<Customer>()
    ...
    public virtual Task Collection_where_nav_prop_count_reverse(bool async) =>
        AssertQuery(
            async,
            ss => from c in ss.Set<Customer>() where 5 < c.Orders.Count() select c,
            ss =>
                from c in ss.Set<Customer>()
    ...
    public virtual Task Collection_select_nav_prop_long_count(bool async) =>
        AssertQuery(
            async,
            ss => from c in ss.Set<Customer>() select new { C = c.Orders.LongCount() },
            ss =>
                from c in ss.Set<Customer>()
    ...
    public virtual Task Collection_select_nav_prop_sum(bool async) =>
        AssertQuery(
            async,
            ss => from c in ss.Set<Customer>() select new { Sum = c.Orders.Sum(o => o.OrderID) },
            ss =>
                from c in ss.Set<Customer>()
    ...
    public virtual Task Collection_where_nav_prop_sum(bool async) =>
        AssertQuery(
            async,
            ss => from c in ss.Set<Customer>() where c.Orders.Sum(o => o.OrderID) > 1000 select c,
            ss =>
                from c in ss.Set<Customer>()
    ...
    public virtual Task Collection_select_nav_prop_first_or_default_then_nav_prop(bool async)
    {
        ...
        return AssertQuery(
            async,
            ss =>
                from c in ss.Set<Customer>().Where(e => e.CustomerID.StartsWith("A"))
                orderby c.CustomerID
                select new
                {
...
#pragma warning disable RCS1146 // Use conditional access.
                    Customer = c.Orders != null
                    && c.Orders.Where(e => orderIds.Contains(e.OrderID)).Any()
#pragma warning restore RCS1146 // Use conditional access.
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Navigation_in_subquery_referencing_outer_query_with_client_side_result_operator_and_count(
        bool async
    ) =>
        AssertQuery(
            async,
