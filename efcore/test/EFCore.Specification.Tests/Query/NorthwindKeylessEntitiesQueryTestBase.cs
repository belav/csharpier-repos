    ...
    where TFixture : NorthwindQueryFixtureBase<NoopModelCustomizer>, new()
{
    ...
    public virtual Task KeylessEntity_where_simple(bool async) =>
        AssertQuery(async, ss => ss.Set<CustomerQuery>().Where(c => c.City == "London"));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task KeylessEntity_by_database_view(bool async) =>
        AssertQuery(async, ss => ss.Set<ProductView>());
    ...
    public virtual Task KeylessEntity_with_defining_query(bool async) =>
        AssertQuery(async, ss => ss.Set<OrderQuery>().Where(ov => ov.CustomerID == "ALFKI"));
    ...
    public virtual Task KeylessEntity_with_defining_query_and_correlated_collection(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<OrderQuery>()
                    .Where(ov => ov.CustomerID == "ALFKI")
                    .Select(ov => ov.Customer)
    ...
    public virtual Task KeylessEntity_with_included_nav(bool async) =>
        AssertQuery(
            async,
            ss =>
                from ov in ss.Set<OrderQuery>().Include(ov => ov.Customer)
                where ov.CustomerID == "ALFKI"
                select ov,
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<OrderQuery>(ov => ov.Customer))
        );
    ...
    public virtual Task KeylessEntity_with_included_navs_multi_level(bool async) =>
        AssertQuery(
            async,
            ss =>
                from ov in ss.Set<OrderQuery>().Include(ov => ov.Customer.Orders)
                where ov.CustomerID == "ALFKI"
                select ov,
            elementAsserter: (e, a) =>
                AssertInclude(
    ...
    public virtual Task Entity_mapped_to_view_on_right_side_of_join(bool async) =>
        AssertQuery(
            ...
            ss =>
                ...
                join pv in ss.Set<ProductView>()
                    on o.CustomerID equals pv.CategoryName
                    into grouping
    ...
    public virtual Task Count_over_keyless_entity(bool async) =>
        AssertCount(async, ss => ss.Set<CustomerQuery>());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Count_over_keyless_entity_with_pushdown(bool async) =>
        AssertCount(async, ss => ss.Set<CustomerQuery>().OrderBy(x => x.ContactTitle).Take(10));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Count_over_keyless_entity_with_pushdown_empty_projection(bool async) =>
        AssertCount(async, ss => ss.Set<CustomerQuery>().Take(10));
}
