...
public abstract class NonSharedPrimitiveCollectionsQueryTestBase : NonSharedModelTestBase
{
    ...
    [ConditionalFact]
    public virtual Task Array_of_string() => TestArray("a", "b");

    [ConditionalFact]
    public virtual Task Array_of_int() => TestArray(1, 2);

    [ConditionalFact]
    public virtual Task Array_of_long() => TestArray(1L, 2L);

    [ConditionalFact]
    public virtual Task Array_of_short() => TestArray((short)1, (short)2);

    [ConditionalFact]
    public virtual Task Array_of_byte() => TestArray((byte)1, (byte)2);

    [ConditionalFact]
    public virtual Task Array_of_double() => TestArray(1d, 2d);

    [ConditionalFact]
    public virtual Task Array_of_float() => TestArray(1f, 2f);

    [ConditionalFact]
    public virtual Task Array_of_decimal() => TestArray(1m, 2m);
    ...
    [ConditionalFact]
    public virtual Task Array_of_bool() => TestArray(true, false);
    ...
    [ConditionalFact]
    public virtual Task Array_of_enum() => TestArray(MyEnum.Label1, MyEnum.Label2);
    ...
    [ConditionalFact(
        Skip = "Currently fails because we don't use the element mapping when serializing to JSON, but just do JsonSerializer.Serialize, #30677"
    )]
    public virtual async Task Parameter_with_inferred_value_converter()
    {
        ...
        var result = await context
            .Set<TestEntity>()
            .SingleAsync(m =>
                ints.Count(i => i == EF.Property<IntWrapper>(m, "PropertyWithValueConverter")) == 1
            );
        Assert.Equal(1, result.Id);
    }
    ...
    public virtual async Task Constant_with_inferred_value_converter()
    {
        ...
        var result = await context
            .Set<TestEntity>()
            .SingleAsync(m =>
                new IntWrapper[] { new(1), new(8) }.Count(i =>
                    i == EF.Property<IntWrapper>(m, "PropertyWithValueConverter")
                ) == 1
            );
        Assert.Equal(1, result.Id);
    }
    ...
    public virtual async Task Inline_collection_in_query_filter()
    {
        var contextFactory = await InitializeAsync<TestContext>(
            onModelCreating: mb =>
                mb.Entity<TestEntity>()
                    .HasQueryFilter(t => new[] { 1, 2, 3 }.Count(i => i > t.Id) == 1),
            seed: context =>
            {
                context.AddRange(new TestEntity { Id = 1 }, new TestEntity { Id = 2 });
                context.SaveChanges();
            }
        );
    ...
    protected override string StoreName => "NonSharedPrimitiveCollectionsTest";
