    ...
    where TFixture : NorthwindQueryFixtureBase<NoopModelCustomizer>, new()
{
    ...
    public virtual Task String_Contains_constant_with_whitespace(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().Where(c => c.ContactName.Contains("     ")));
                        ...
                        g =>
                            new
                            {
                                ...
                                Customers = string.Join(
                                    "|",
                                    g.Where(e => e.ContactName.Length > 10)
    ...
    public virtual async Task String_Compare_nested(bool async)
    {
        await AssertQuery(
            async,
            ss =>
                ss.Set<Customer>().Where(c => string.Compare(c.CustomerID, "M" + c.CustomerID) == 0)
        );
        ...
        await AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(c => 0 >= string.Compare(c.CustomerID, "M" + c.CustomerID)),
            entryCount: 51
        );
        ...
        await AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(
                        c =>
                            c.CustomerID.CompareTo("ALFKI".Replace("ALF".ToUpper(), c.CustomerID))
                            > 0
                    )
        );
    ...
    public virtual async Task DateTime_Compare_to_simple_zero(bool async, bool compareTo)
    {
        ...
        else
        {
            await AssertQuery(
                ...
                ss =>
                    ss.Set<Order>()
                        .Where(c => DateTime.Compare(c.OrderDate.Value, myDatetime) == 0),
                entryCount: 3
            );
            ...
            await AssertQuery(
                ...
                ss =>
                    ss.Set<Order>()
                        .Where(c => 0 != DateTime.Compare(c.OrderDate.Value, myDatetime)),
                entryCount: 827
            );
    ...
    public virtual Task Where_math_abs1(bool async) =>
        AssertQuery(
            async,
            ss => ss.Set<Product>().Where(od => Math.Abs(od.ProductID) > 10),
            entryCount: 67
        );
    ...
    public virtual Task Where_mathf_abs1(bool async) =>
        AssertQuery(
            async,
            ss => ss.Set<Product>().Where(od => MathF.Abs(od.ProductID) > 10),
            entryCount: 67
        );
    ...
    public virtual Task Where_guid_newguid(bool async) =>
        AssertQuery(
            async,
            ss => ss.Set<Customer>().Where(od => Guid.NewGuid() != default),
            entryCount: 91
        );
    ...
    public virtual Task Static_equals_int_compared_to_long(bool async)
    {
        ...
        return AssertQuery(async, ss => ss.Set<Order>().Where(o => Equals(o.OrderID, arg)));
    }
