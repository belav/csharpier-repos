    ...
    where TFixture : NorthwindQueryFixtureBase<NoopModelCustomizer>, new()
{
    ...
    public virtual Task Join_customers_orders_select(bool async) =>
        AssertQuery(
            ...
            ss =>
                from c in ss.Set<Customer>()
                join o in ss.Set<Order>() on c.CustomerID equals o.CustomerID
                select new { c.ContactName, o.OrderID } into p
                select p,
            e => e.OrderID
        );
    ...
    public virtual Task Join_same_collection_multiple(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Customer>()
                ...
                .Join(ss.Set<Customer>(), o => o.c1.CustomerID, i => i.CustomerID, (c12, c3) => c3)
        );
    ...
    public virtual Task GroupJoin_customers_employees_subquery_shadow(bool async) =>
        AssertQuery(
            ...
            ss =>
                (
                    from c in ss.Set<Customer>()
                    join e in ss.Set<Employee>().OrderBy(e => e.City)
    ...
    public virtual Task GroupJoin_customers_employees_subquery_shadow_take(bool async) =>
        AssertQuery(
            ...
            ss =>
                (
                    from c in ss.Set<Customer>()
                    join e in ss.Set<Employee>().OrderBy(e => e.City).Take(5)
                        on c.City equals e.City
                        into employees
                    select employees
                )
    ...
    public virtual Task GroupJoin_simple_subquery(bool async) =>
        AssertQuery(async, ss =>
            from c in ss.Set<Customer>()
            join o in ss.Set<Order>().OrderBy(o => o.OrderID).Take(4)
    ...
    public virtual Task Unflattened_GroupJoin_composed_2(bool async) =>
        AssertQuery(
            ...
            ss =>
                ...
                join c2 in ss.Set<Customer>().Where(n => n.City == "Lisboa")
                    on i.c.CustomerID equals c2.CustomerID
                select new { i, c2 },
            e => e.i.c.CustomerID,
            elementAsserter: (e, a) =>
            {
    ...
    public virtual Task GroupJoin_DefaultIfEmpty2(bool async) =>
        AssertQuery(
            ...
            ss =>
                from e in ss.Set<Employee>()
                join o in ss.Set<Order>().Where(o => o.CustomerID.StartsWith("F"))
...
#pragma warning restore RCS1146 // Use conditional access.
    ...
    public virtual Task GroupJoin_Subquery_with_Take_Then_SelectMany_Where(bool async) =>
        AssertQuery(async, ss =>
            from c in ss.Set<Customer>()
            join o in ss.Set<Order>().OrderBy(o => o.OrderID).Take(100)
                on c.CustomerID equals o.CustomerID
                into lo
    ...
    public virtual Task Inner_join_with_tautology_predicate_converts_to_cross_join(bool async) =>
        AssertQuery(async, ss =>
            from c in ss.Set<Customer>().OrderBy(c => c.CustomerID).Take(10)
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Left_join_with_tautology_predicate_doesnt_convert_to_cross_join(
        bool async
    ) =>
        AssertQuery(async, ss =>
            from c in ss.Set<Customer>().OrderBy(c => c.CustomerID).Take(10)
            join o in ss.Set<Order>().OrderBy(o => o.OrderID).Take(10)
                on c.CustomerID != null equals true
                into grouping
    ...
    private class CustomerViewModel
    {
        ...
        public override bool Equals(object obj)
        {
            ...
            return ReferenceEquals(this, obj)
                || obj.GetType() == GetType() && Equals((CustomerViewModel)obj);
        }
        ...
        public override int GetHashCode() => HashCode.Combine(CustomerID, City);
    }
    ...
    private class OrderDetailViewModel
    {
        ...
        public override bool Equals(object obj)
        {
            ...
            return ReferenceEquals(this, obj)
                || obj.GetType() == GetType() && Equals((OrderDetailViewModel)obj);
        }

        private bool Equals(OrderDetailViewModel orderDetailViewModel) =>
            OrderID == orderDetailViewModel.OrderID && ProductID == orderDetailViewModel.ProductID;
    ...
    public virtual Task SelectMany_with_selecting_outer_entity(bool async) =>
        AssertQuery(async, ss => ss.Set<Customer>().SelectMany(c => c.Orders.Select(o => c)));
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task SelectMany_with_selecting_outer_entity_column_and_inner_column(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Take_in_collection_projection_with_FirstOrDefault_on_top_level(
        bool async
    ) =>
