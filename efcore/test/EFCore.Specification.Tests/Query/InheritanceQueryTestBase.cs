    ...
    where TFixture : InheritanceQueryFixtureBase, new()
{
    ...
    {
        await AssertSingle(async, ss => ss.Set<Coke>());

        await AssertSingle(async, ss => ss.Set<Lilt>());

        await AssertSingle(async, ss => ss.Set<Tea>());
    }

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_query_all_types_when_shared_column(bool async) =>
        AssertQuery(async, ss => ss.Set<Drink>());
    ...
    public virtual Task Can_use_is_kiwi(bool async) =>
        AssertQuery(async, ss => ss.Set<Animal>().Where(a => a is Kiwi));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_use_is_kiwi_with_cast(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Animal>().Select(a => new { Value = a is Kiwi ? ((Kiwi)a).FoundOn : default })
        );
    ...
    public virtual Task Can_use_is_kiwi_with_other_predicate(bool async) =>
        AssertQuery(async, ss => ss.Set<Animal>().Where(a => a is Kiwi && a.CountryId == 1));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_use_is_kiwi_in_projection(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Animal>().Select(a => a is Kiwi));
    ...
    public virtual Task Can_use_of_type_bird_with_projection(bool async) =>
        AssertQuery(async, ss => ss.Set<Animal>().OfType<Bird>().Select(b => new { b.EagleId }));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_use_of_type_bird_first(bool async) =>
        AssertFirst(async, ss => ss.Set<Animal>().OfType<Bird>().OrderBy(a => a.Species));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_use_of_type_kiwi(bool async) =>
        AssertQuery(async, ss => ss.Set<Animal>().OfType<Kiwi>());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_use_backwards_of_type_animal(bool async) =>
        AssertQuery(async, ss => ss.Set<Kiwi>().OfType<Animal>());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_use_of_type_rose(bool async) =>
        AssertQuery(async, ss => ss.Set<Plant>().OfType<Rose>());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_query_all_animals(bool async) =>
        AssertQuery(async, ss => ss.Set<Animal>().OrderBy(a => a.Species), assertOrder: true);
    ...
    public virtual Task Can_query_all_plants(bool async) =>
        AssertQuery(async, ss => ss.Set<Plant>().OrderBy(a => a.Species), assertOrder: true);
    ...
    public virtual Task Filter_on_property_inside_complex_type_on_derived_type(bool async) =>
        Fixture.EnableComplexTypes
            ? AssertQuery(async, ss =>
                ss.Set<Daisy>().Where(d => d.AdditionalInfo.LeafStructure.AreLeavesBig)
            )
            : Task.CompletedTask;
    ...
    public virtual Task Can_filter_all_animals(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Animal>().OrderBy(a => a.Species).Where(a => a.Name == "Great spotted kiwi")
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_query_all_birds(bool async) =>
        AssertQuery(async, ss => ss.Set<Bird>().OrderBy(a => a.Species), assertOrder: true);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_query_just_kiwis(bool async) =>
        AssertSingle(async, ss => ss.Set<Kiwi>());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_query_just_roses(bool async) =>
        AssertSingle(async, ss => ss.Set<Rose>());
    ...
    public virtual Task Can_use_of_type_kiwi_where_south_on_derived_property(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Animal>().OfType<Kiwi>().Where(x => x.FoundOn == Island.South)
        );
    ...
    public virtual Task Discriminator_used_when_projection_over_derived_type(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Kiwi>().Select(k => k.FoundOn));
    ...
    public virtual Task Discriminator_used_when_projection_over_of_type(bool async) =>
        AssertQueryScalar(async, ss => ss.Set<Animal>().OfType<Kiwi>().Select(k => k.FoundOn));
        ...
        TestHelpers.ExecuteWithStrategyInTransaction(
            ...
            context =>
            {
                eagleId = context
                    .Set<Bird>()
                    .AsNoTracking()
                    .Single(e => e.Species == "Aquila chrysaetos canadensis")
                    .Id;
    ...
    public virtual Task Union_siblings_with_duplicate_property_in_subquery(bool async)
        ...
        =>
        AssertQuery(async, ss =>
            ss.Set<Coke>().Cast<Drink>().Union(ss.Set<Tea>()).Where(d => d.SortIndex > 0)
        );
    ...
    public virtual Task OfType_Union_OfType(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Bird>().OfType<Kiwi>().Union(ss.Set<Bird>()).OfType<Kiwi>()
        );
    ...
    public virtual Task Subquery_OfType(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Bird>().OrderBy(b => b.Species).Take(5).Distinct().OfType<Kiwi>()
        );
    ...
    public virtual Task Union_entity_equality(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<Kiwi>().Union(ss.Set<Eagle>().Cast<Bird>()).Where(b => b == null)
        );
    ...
    public virtual Task Byte_enum_value_constant_used_in_projection(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<Kiwi>().Select(k => k.IsFlightless ? Island.North : Island.South)
        );
    ...
    public virtual Task GetType_in_hierarchy_in_leaf_type_with_sibling(bool async) =>
        AssertQuery(async, ss => ss.Set<Animal>().Where(e => e.GetType() == typeof(Eagle)));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task GetType_in_hierarchy_in_leaf_type_with_sibling2(bool async) =>
        AssertQuery(async, ss => ss.Set<Animal>().Where(e => e.GetType() == typeof(Kiwi)));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task GetType_in_hierarchy_in_leaf_type_with_sibling2_reverse(bool async) =>
        AssertQuery(async, ss => ss.Set<Animal>().Where(e => typeof(Kiwi) == e.GetType()));
