    ...
    where TFixture : InheritanceQueryFixtureBase, new()
{
    ...
    public virtual Task Can_query_all_types_when_shared_column(bool async) =>
        AssertQuery(async, ss => ss.Set<Drink>(), entryCount: 3);
    ...
    public virtual Task Can_use_is_kiwi(bool async) =>
        AssertQuery(async, ss => ss.Set<Animal>().Where(a => a is Kiwi), entryCount: 1);
    ...
    public virtual Task Can_use_of_type_kiwi(bool async) =>
        AssertQuery(async, ss => ss.Set<Animal>().OfType<Kiwi>(), entryCount: 1);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_use_backwards_of_type_animal(bool async) =>
        AssertQuery(async, ss => ss.Set<Kiwi>().OfType<Animal>(), entryCount: 1);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_use_of_type_rose(bool async) =>
        AssertQuery(async, ss => ss.Set<Plant>().OfType<Rose>(), entryCount: 1);
    ...
    public virtual Task Can_query_just_kiwis(bool async) =>
        AssertSingle(async, ss => ss.Set<Kiwi>(), entryCount: 1);

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Can_query_just_roses(bool async) =>
        AssertSingle(async, ss => ss.Set<Rose>(), entryCount: 1);
    ...
    public virtual Task GetType_in_hierarchy_in_abstract_base_type(bool async) =>
        AssertQuery(async, ss => ss.Set<Animal>().Where(e => e.GetType() == typeof(Animal)));

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task GetType_in_hierarchy_in_intermediate_type(bool async) =>
        AssertQuery(async, ss => ss.Set<Animal>().Where(e => e.GetType() == typeof(Bird)));
