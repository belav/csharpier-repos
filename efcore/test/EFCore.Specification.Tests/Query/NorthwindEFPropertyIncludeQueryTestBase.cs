...
public abstract class NorthwindEFPropertyIncludeQueryTestBase<TFixture>
    ...
    where TFixture : NorthwindQueryFixtureBase<NoopModelCustomizer>, new()
{
    ...
    public override async Task Include_reference_dependent_already_tracked(bool async)
    {
        ...
        var orders = async
            ? await context
                .Set<Order>()
                .Include(o => EF.Property<Order>(o, "Customer"))
    ...
    private class IncludeRewritingExpressionVisitor : ExpressionVisitor
    {
        ...
        private static readonly MethodInfo _propertyMethod = typeof(EF)
            .GetTypeInfo()
            .GetDeclaredMethod(nameof(EF.Property))!;
                        ...
                        Expression.Quote(
                            Expression.Lambda(
                                typeof(Func<,>).MakeGenericType(
                                    genericArguments[entityTypeIndex],
                                    genericArguments[propertyTypeIndex]
                                ),
                                Expression.Call(
                                    _propertyMethod.MakeGenericMethod(
                                        genericArguments[propertyTypeIndex]
                                    ),
                                    parameterExpression,
