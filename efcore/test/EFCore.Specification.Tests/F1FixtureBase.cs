...
public abstract class F1FixtureBase<TRowVersion> : SharedStoreFixtureBase<F1Context>
{
    ...
    protected override IServiceCollection AddServices(IServiceCollection serviceCollection) =>
        base.AddServices(
            serviceCollection.AddSingleton<ISingletonInterceptor, F1MaterializationInterceptor>()
        );
    ...
    protected virtual void BuildModelExternal(ModelBuilder modelBuilder)
    {
        ...
        {
            b.HasKey(c => c.TeamId);
            ConfigureConstructorBinding<Chassis>(
                b.Metadata,
                nameof(Chassis.TeamId),
                nameof(Chassis.Name)
            );
        });
        ...
        {
            ConfigureConstructorBinding<Gearbox>(
                b.Metadata,
                nameof(Gearbox.Id),
                nameof(Gearbox.Name)
            );
    ...
    )
    {
        ...
        var loaderField = typeof(TLoaderEntity).GetField(
            "_loader",
        ...
        {
            parameterBindings.Add(
                new PropertyParameterBinding(entityType.FindProperty(propertyName)!)
            );
        }
