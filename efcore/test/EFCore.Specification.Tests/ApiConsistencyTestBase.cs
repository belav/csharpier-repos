    ...
    where TFixture : ApiConsistencyTestBase<TFixture>.ApiConsistencyFixtureBase, new()
{
    ...
    protected Type GetEquivalentGenericType(Type parameterType, Type[] genericArguments)
    {
        if (
            ...
            && Fixture
                .GenericFluentApiTypes
                .TryGetValue(parameterType.GetGenericArguments()[0], out var genericBuilder)
    ...
    private string ValidateMetadata(KeyValuePair<Type, (Type, Type, Type, Type)> types)
    {
        ...
        if (conventionBuilderType != null)
        {
            ...
            if (builderProperty == null || builderProperty.PropertyType != conventionBuilderType)
            {
        ...
        > MetadataTypes { get; } =
            new()
            {
                ...
                },
                {
                    ...
                    (typeof(IMutableTypeBase), typeof(IConventionTypeBase), null, typeof(ITypeBase))
                },
                {
                    typeof(IReadOnlyKey),
                    (
