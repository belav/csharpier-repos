    ...
    where TFixture : ApiConsistencyTestBase<TFixture>.ApiConsistencyFixtureBase, new()
{
    ...
    protected Type GetEquivalentGenericType(Type parameterType, Type[] genericArguments)
    {
        ...
        )
        {
            ...
            if (
                Fixture.GenericFluentApiTypes.TryGetValue(builder, out var genericBuilder)
                && genericBuilder.GetGenericArguments().Length == genericArguments.Length
            )
            {
                return typeof(Action<>).MakeGenericType(
                    genericBuilder.MakeGenericType(genericArguments)
                );
            }
            ...
            if (builder.IsGenericType)
            {
                ...
                {
                    return typeof(Action<>).MakeGenericType(
                        builderDefinition.MakeGenericType(genericArguments)
                    );
                }
            }
        }
        ...
        foreach (var tuple in Fixture.MirrorTypes)
        {
            ...
            foreach (var method in tuple.Key.GetMethods(PublicInstance | BindingFlags.DeclaredOnly))
            {
                if (
                    !Fixture.UnmatchedMetadataMethods.Contains(method)
                    && !(
                        Fixture.UnmatchedMirrorMethods.TryGetValue(
                            tuple.Value,
                            out var unmatchedMirrorMethods
                ...
                )
                {
                    ...
                    foreach (
                        var targetMethod in tuple.Value.GetMethods(
                    ...
                    )
                    {
                        if (
                            ...
                            && targetMethod.GetGenericArguments().Length
                                == method.GetGenericArguments().Length
                            && method
                                .GetParameters()
                                .Select(p => p.ParameterType)
            ...
            Assert.False(
                ...
                $"\r\n-- Missing equivalent methods on {tuple.Value.DisplayName()} --\r\n"
                    + string.Join(
                        Environment.NewLine,
                        unmatchedMethods.Select(m => Format(m.Method, m.Type))
                    )
            );
    ...
    {
        var errors = Fixture.MetadataTypes.Select(ValidateMetadata).Where(e => e != null).ToList();
    ...
    private static readonly string MetadataBuilderNamespace =
        typeof(IConventionModelBuilder).Namespace;
    ...
    private string ValidateMetadata(KeyValuePair<Type, (Type, Type, Type, Type)> types)
    {
        ...
        if (readOnlyType != typeof(IAnnotation) && readOnlyType != typeof(IReadOnlyAnnotatable))
        {
        ...
        if (conventionBuilderType != null)
        {
            ...
            if (metadataProperty == null || metadataProperty.PropertyType != conventionType)
            {
    ...
    {
        var errors = Fixture
            .MetadataMethods.Select(typeTuple =>
                ...
                join mutableMethod in typeTuple.Mutable
                    on readonlyMethod.Name equals mutableMethod.Name
                    into mutableGroup
        ...
        var errors = Fixture
            .MetadataMethods.Select(typeTuple =>
                ...
                join conventionMethod in typeTuple.Convention
                    on GetConventionName(mutableMethod) equals conventionMethod.Name
                    into conventionGroup
    ...
    {
        var errors = Fixture
            .MetadataMethods.Select(t => ValidateConventionMethods(t.Convention))
            .Where(e => e != null)
            .ToList();
    ...
    {
        var errors = Fixture
            .MetadataMethods.Select(t => ValidateConventionBuilderMethods(t.ConventionBuilder))
            .Where(e => e != null)
            .ToList();
    ...
    private string ValidateConventionBuilderMethods(IReadOnlyList<MethodInfo> methods)
    {
        ...
        var builderType = methods[0].IsStatic
            ? methods[0].GetParameters()[0].ParameterType
            : declaringType;
        var methodLookup = new Dictionary<string, MethodInfo>(methods.Count);
        foreach (var method in methods)
        {
            methodLookup[
                Fixture.MetadataMethodNameTransformers.TryGetValue(method, out var name)
                    ? name
                    : method.Name
            ] = method;
        }
        ...
        foreach (var keyValuePair in methodLookup)
        {
            ...
            var expectedName =
                methodName.StartsWith("HasNo", StringComparison.Ordinal)
                    ? "CanRemove" + methodName[5..]
                : methodName.StartsWith("Ignore", StringComparison.Ordinal) ? methodName
                : "CanSet"
                    + (
                        methodName.StartsWith("Has", StringComparison.Ordinal)
                        || methodName.StartsWith("Use", StringComparison.Ordinal)
                            ? methodName[3..]
                        : methodName.StartsWith("To", StringComparison.Ordinal) ? methodName[2..]
                        : methodName.StartsWith("With", StringComparison.Ordinal) ? methodName[4..]
                        : methodName
                    );
    ...
    {
        var errors = Fixture
            .MetadataTypes.Select(t =>
                ValidateConventionBuilderMethodReturns(
                    t.Value.ConventionBuilder,
                    t.Value.Convention
                )
            )
            .Where(e => e != null)
    ...
    private string ValidateConventionBuilderMethodReturns(Type builderType, Type conventionType)
    {
        ...
        var extensionType = Fixture
            .MetadataExtensionTypes.GetValueOrDefault(builderType)
        ...
        foreach (var interfaceType in builderType.GetDeclaredInterfaces())
        {
            ...
            var normalizedInterfaceType = interfaceType.IsGenericType
                ...
                : interfaceType;
            var readOnlyInterfaceType = Fixture
                .MetadataTypes.FirstOrDefault(p =>
                    p.Value.ConventionBuilder == normalizedInterfaceType
                )
                .Key;
            ...
            foreach (var method in methods)
            {
                if (
                    (
                        ...
                        && method.ReturnType
                            != Fixture.MetadataTypes[readOnlyInterfaceType].Convention
                    )
                    || Fixture.UnmatchedMetadataMethods.Contains(method)
                ...
                var parameters = method
                    .GetParameters()
                    .Select(p =>
                        GetEquivalentGenericType(p.ParameterType, builderType.GetGenericArguments())
                    )
                    .ToArray();
                ...
                if (hidingMethod == null || hidingMethod.ReturnType != expectedReturn)
                {
            ...
            var interfaceExtensionType = Fixture
                .MetadataExtensionTypes.GetValueOrDefault(readOnlyInterfaceType)
                .ConventionBuilderExtensions;
            ...
            var conventionBuilderExtensionMethods = interfaceExtensionType.GetMethods(
            ...
            foreach (var method in conventionBuilderExtensionMethods)
            {
                var parameters = method.GetParameters();
                if (
                    parameters.First().ParameterType != interfaceType
                    || method.ReturnType != interfaceType
                    || Fixture.UnmatchedMetadataMethods.Contains(method)
                    || Fixture.IsObsolete(method)
                )
                ...
                var methodFound = false;
                var expectedReturn =
                    method.ReturnType == interfaceType ? builderType : conventionType;
                if (extensionType != null)
                {
                    var expectedParameters = new[] { builderType }
                        .Concat(
                            parameters
                                .Skip(1)
                                .Select(p =>
                                    GetEquivalentGenericType(
                                        p.ParameterType,
                                        builderType.GetGenericArguments()
                                    )
                                )
                        )
                        .ToArray();
        ...
        var errors = Fixture
            .MetadataMethods.Select(typeTuple =>
                ...
                join runtimeMethod in typeTuple.Runtime
                    on readOnlyMethod.Name equals runtimeMethod?.Name
                    into runtimeGroup
    ...
    {
        var errors = Fixture
            .MetadataMethods.SelectMany(m => m.ReadOnly.Select(ValidateMethodName))
            .Where(e => e != null)
            .ToList();
    ...
    {
        var errors = Fixture
            .MetadataMethods.SelectMany(m => m.Mutable.Select(ValidateMutableMethod))
            .Where(e => e != null)
            .ToList();
    ...
    private string ValidateMutableMethod(MethodInfo mutableMethod)
    {
        ...
        if (
            ...
            && (
                firstParameter.ParameterType != typeof(Type)
                || mutableMethod.ReturnType != typeof(string)
            )
            && (
    ...
    {
        var errors = Fixture
            .MetadataMethods.SelectMany(m => m.Convention.Select(ValidateConventionMethod))
            .Where(e => e != null)
            .ToList();
    ...
    private string ValidateConventionMethod(MethodInfo conventionMethod)
    {
        ...
        if (
            ...
            && (
                firstParameter.ParameterType != typeof(Type)
                || conventionMethod.ReturnType != typeof(string)
            )
            && (
                firstParameter.ParameterType != typeof(string)
                || conventionMethod.ReturnType != typeof(FieldInfo)
            )
            && (
    ...
    public virtual void Service_implementations_should_use_dependencies_parameter_object()
    {
        ...
        var badServiceTypes = (
            ...
            where
                sd.ServiceType.Namespace.StartsWith("Microsoft.Entity", StringComparison.Ordinal)
            ...
            let ns = it.Namespace
            where
                ns.StartsWith("Microsoft.Entity", StringComparison.Ordinal)
    ...
    public virtual void Async_methods_should_have_overload_with_cancellation_token_and_end_with_async_suffix()
    {
        ...
        var missingOverloads = (
            ...
            where
                !asyncMethodsWithToken.Any(methodWithToken =>
                    methodWithoutToken.Name == methodWithToken.Name
    ...
    protected class ParameterTypeEqualityComparer : IEqualityComparer<Type>
    {
        ...
        public bool Equals(Type sourceParameterType, Type targetParameterType)
        {
            ...
            if (
                _targetMethod.DeclaringType.IsGenericType
                && sourceParameterType
                    == _tests.GetEquivalentGenericType(
                        sourceParameterType,
                        _targetMethod.DeclaringType.GetGenericArguments()
                    )
            )
            {
                return true;
            ...
            )
            {
                ...
                if (
                    sourceExpressionType.IsGenericType
                ...
                )
                {
                    ...
                    if (
                        sourceFuncParameterType == sourceType.GetGenericArguments()[^1]
    ...
    public abstract class ApiConsistencyFixtureBase
    {
        ...
        public virtual Dictionary<Type, HashSet<MethodInfo>> UnmatchedMirrorMethods { get; } =
            new();
        public virtual Dictionary<MethodInfo, string> MetadataMethodNameTransformers { get; } =
            new();
        ...
        public virtual HashSet<PropertyInfo> ComputedDependencyProperties { get; } =
            new()
            {
                ...
                typeof(QueryContextDependencies).GetProperty(
        ...
        > MetadataTypes { get; } =
            new()
            {
                ...
                {
                    typeof(IAnnotation),
                    (typeof(IAnnotation), typeof(IConventionAnnotation), null, null)
                },
                {
                ...
                },
                {
                    ...
                    (
                        typeof(IMutableComplexProperty),
        ...
        public List<(
            IReadOnlyList<MethodInfo> ReadOnly,
        ...
        protected virtual void Initialize()
        {
            ...
            foreach (var extensionTypePair in MetadataExtensionTypes)
            {
                ...
                var readOnlyMethods =
                    extensionTypeTuple
                        .ReadonlyExtensions?.GetMethods(BindingFlags.Public | BindingFlags.Static)
                        .Where(m =>
        ...
        )
        {
            ...
            {
                var readOnlyMethods =
                    typeTuple.Key.GetMethods(PublicInstance).Where(m => !IsObsolete(m)).ToArray()
                    ?? new MethodInfo[0];
                var mutableMethods =
                    typeTuple
