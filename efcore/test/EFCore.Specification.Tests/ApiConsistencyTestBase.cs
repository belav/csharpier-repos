...
namespace Microsoft.EntityFrameworkCore
{
        ...
        where TFixture : ApiConsistencyTestBase<TFixture>.ApiConsistencyFixtureBase, new()
    {
        ...
        private string ValidateMetadata(KeyValuePair<Type, (Type, Type, Type, Type)> types)
        {
            ...
            if (typeof(IAnnotation) != readonlyType && typeof(IReadOnlyAnnotatable) != readonlyType)
            {
        ...
        {
            var errors = Fixture.MetadataMethods.Select(
                    typeTuple =>
        ...
        {
            var errors = Fixture.MetadataMethods.Select(
                    t => ValidateConventionMethods(t.Convention)
                )
                .Where(e => e != null)
                .ToList();
        ...
        {
            var errors = Fixture.MetadataMethods.Select(
                    t => ValidateConventionBuilderMethods(t.ConventionBuilder)
                )
                .Where(e => e != null)
                .ToList();
        ...
        {
            var errors = Fixture.MetadataMethods.SelectMany(
                    m => m.ReadOnly.Select(ValidateMethodName)
                )
                .Where(e => e != null)
                .ToList();
        ...
        {
            var errors = Fixture.MetadataMethods.SelectMany(
                    m => m.Mutable.Select(ValidateMutableMethod)
                )
                .Where(e => e != null)
                .ToList();
        ...
        private string ValidateMutableMethod(MethodInfo mutableMethod)
        {
            ...
            var firstParameter =
                parameters.Length > parameterIndex ? parameters[parameterIndex] : null;
            var name = mutableMethod.Name;
            if (
                firstParameter != null
                && (
        ...
        {
            var errors = Fixture.MetadataMethods.SelectMany(
                    m => m.Convention.Select(ValidateConventionMethod)
                )
                .Where(e => e != null)
                .ToList();
            ...
            var badServiceTypes = (
                ...
                where
                    ...
                    && (
                        ...
                        || !it.GetConstructors()[0].GetParameters()[
                            0
        ...
        public virtual void Public_inheritable_apis_should_be_virtual()
        {
            var nonVirtualMethods = (
                from type in GetAllTypes(TargetAssembly.GetTypes())
                where type.IsVisible && !type.IsSealed
        ...
        public virtual void Async_methods_should_have_overload_with_cancellation_token_and_end_with_async_suffix()
        {
            ...
            var asyncMethodsWithToken = (
                ...
                where
                    method.GetParameters().Any(pi => pi.ParameterType == typeof(CancellationToken))
                select method
            ).ToList();
            ...
            var asyncMethodsWithoutToken = (
                ...
                where
                    ...
                    && method.GetParameters()
                        .All(pi => pi.ParameterType != typeof(CancellationToken))
                select method
            ).ToList();
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            public virtual List<(Type Type, Type ReadonlyExtensions, Type MutableExtensions, Type ConventionExtensions, Type ConventionBuilderExtensions, Type RuntimeExtensions)> MetadataExtensionTypes { get; } =
                new();

            public List<(IReadOnlyList<MethodInfo> ReadOnly, IReadOnlyList<MethodInfo> Mutable, IReadOnlyList<MethodInfo> Convention, IReadOnlyList<MethodInfo> ConventionBuilder, IReadOnlyList<MethodInfo> Runtime)> MetadataMethods { get; } =
                new();
            ...
            protected virtual void Initialize()
            {
                ...
                foreach (var extensionTypeTuple in MetadataExtensionTypes)
                {
                    ...
                    var (mutableType, conventionType, conventionBuilderType, runtimeType) =
                        MetadataTypes[type];
                    var readOnlyMethods =
                        extensionTypeTuple.ReadonlyExtensions?.GetMethods(
                    ...
                    var mutableMethods =
                        extensionTypeTuple.MutableExtensions?.GetMethods(
                                BindingFlags.Public | BindingFlags.Static
                            )
                            .Where(
                                m =>
                    ...
                    var conventionMethods =
                        extensionTypeTuple.ConventionExtensions?.GetMethods(
                                BindingFlags.Public | BindingFlags.Static
                            )
                            .Where(
                                m =>
                    ...
                    var conventionBuilderMethods =
                        extensionTypeTuple.ConventionBuilderExtensions?.GetMethods(
                                BindingFlags.Public | BindingFlags.Static
                            )
                            .Where(
                                m =>
                                    !IsObsolete(m)
                                    && m.GetParameters().First().ParameterType
                                        == conventionBuilderType
                            )
                            .ToArray() ?? new MethodInfo[0];
                    var runtimeMethods =
                        extensionTypeTuple.RuntimeExtensions?.GetMethods(
                                BindingFlags.Public | BindingFlags.Static
                            )
                            .Where(
                                m =>
            ...
            protected void AddInstanceMethods(
                ...
                {
                    var readOnlyMethods =
                        typeTuple.Key.GetMethods(PublicInstance)
                            .Where(m => !IsObsolete(m))
                            .ToArray() ?? new MethodInfo[0];
                    var mutableMethods =
                        typeTuple.Value.Mutable.GetMethods(PublicInstance)
                            .Where(m => !IsObsolete(m))
                            .ToArray() ?? new MethodInfo[0];
                    var conventionMethods =
                        typeTuple.Value.Convention.GetMethods(PublicInstance)
                            .Where(m => !IsObsolete(m))
                            .ToArray() ?? new MethodInfo[0];
                    var conventionBuilderMethods =
                        typeTuple.Value.ConventionBuilder?.GetMethods(PublicInstance)
                            .Where(m => !IsObsolete(m))
                            .ToArray() ?? new MethodInfo[0];
                    var runtimeMethods =
                        typeTuple.Value.Runtime?.GetMethods(PublicInstance)
                            .Where(m => !IsObsolete(m))
                            .ToArray() ?? new MethodInfo[0];
                    MetadataMethods.Add(
                        (
