    ...
    private string ValidateConventionBuilderMethodReturns(Type builderType, Type conventionType)
    {
        ...
        foreach (var interfaceType in builderType.GetDeclaredInterfaces())
        {
            ...
            var readOnlyInterfaceType = Fixture
                .MetadataTypes.FirstOrDefault(
                    p => p.Value.ConventionBuilder == normalizedInterfaceType
                )
                .Key;
            ...
            foreach (var extensionTypePair in MetadataExtensionTypes)
            {
                ...
                var readOnlyMethods =
                    extensionTypeTuple
                        ...
                        .Where(
                            m => !IsObsolete(m) && m.GetParameters().First().ParameterType == type
                        )
                ...
                var mutableMethods =
                    extensionTypeTuple
                        ...
                        .Where(
                            m =>
                                !IsObsolete(m)
                                && m.GetParameters().First().ParameterType == mutableType
                        )
                        .ToArray() ?? new MethodInfo[0];
                ...
                var runtimeMethods =
                    extensionTypeTuple
                        ...
                        .Where(
                            m =>
                                !IsObsolete(m)
                                && m.GetParameters().First().ParameterType == runtimeType
                        )
                        .ToArray() ?? new MethodInfo[0];
                MetadataMethods.Add(
                    (
