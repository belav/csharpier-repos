    ...
    public class SqlServerGeometryTypeMappingTests
    {
        ...
        public void GenerateSqlLiteral_works()
        {
            ...
            Assert.Equal(
                ...
                Literal(new Point(1, 2) { SRID = 4326 }, "geometry")
            );
            Assert.Equal(
                "geography::Parse('POINT (1 2)')",
                Literal(new Point(1, 2) { SRID = 4326 }, "geography")
            );
            Assert.Equal(
                "geography::STGeomFromText('POINT (1 2)', 0)",
                Literal(new Point(1, 2), "geography")
            );
            Assert.Equal(
                "geometry::Parse('POINT (1 2 3)')",
                Literal(new Point(1, 2, 3), "geometry")
            );
            Assert.Equal(
                "geometry::Parse('POINT (1 2 3 4)')",
                Literal(new Point(new CoordinateZM(1, 2, 3, 4)), "geometry")
            );
            Assert.Equal(
                "geometry::Parse('POINT (1 2 NULL 3)')",
                Literal(new Point(new CoordinateM(1, 2, 3)), "geometry")
            );
        }
