...
WHERE ((c[""Discriminator""] = ""Employee"") AND (c[""City""] = ""London""))"
        ...
        public override async Task SelectMany_entity_deep(bool async)
        {
            await AssertQuery(
                async,
                ss =>
                    from e1 in ss.Set<Employee>().Where(e => e.EmployeeID == 1)
        ...
        public override async Task Where_subquery_expression(bool async)
        {
            await AssertQuery(
                ...
                ss =>
                {
                    ...
                    return ss.Set<Order>()
                        .Where(x => x.OrderID < 10300 && ss.Set<Order>().Where(expr).Any());
                },
                entryCount: 52
            );
...
WHERE (c[""Discriminator""] = ""Product"")"
        ...
        public override async Task Where_subquery_on_collection(bool async)
        {
            await AssertQuery(
                ...
                ss =>
                    ss.Set<Product>()
                        .Where(p => p.ProductID == 72)
                        .Where(
                            p =>
                                ss.Set<OrderDetail>()
                                    .Where(o => o.ProductID == p.ProductID)
                                    .Select(odd => odd.Quantity)
                                    .Contains<short>(5)
                        ),
                entryCount: 1
            );
...
WHERE ((c[""Discriminator""] = ""Customer"") AND (c[""City""] = ""London""))"
        ...
        public override async Task DefaultIfEmpty_in_subquery(bool async)
        {
            await AssertQuery(
                async,
                ss =>
                    from c in ss.Set<Customer>().Where(c => c.City == "London")
                    from o in ss.Set<Order>()
                        .Where(o => o.CustomerID == c.CustomerID)
                        .DefaultIfEmpty()
                    where o != null
...
WHERE (c[""Discriminator""] = ""Order"")"
        ...
        public override async Task Let_subquery_with_multiple_occurrences(bool async)
        {
            await AssertQuery(
                async,
                ss =>
                    from o in ss.Set<Order>().Where(or => or.OrderID < 10250)
...
WHERE (c[""Discriminator""] = ""Customer"")"
        ...
        [ConditionalTheory(Skip = "Issue #17246")]
        public override Task Subquery_member_pushdown_does_not_change_original_subquery_model2(
            bool async
...
WHERE ((c[""Discriminator""] = ""Customer"") AND (c[""CustomerID""] IN (""ALFKI"") OR (c[""CustomerID""] = null)))"
        ...
        [ConditionalTheory(Skip = "Issue #17246")]
        public override Task Perform_identity_resolution_reuses_same_instances(
            bool async,
            bool useAsTracking
        )
        {
        ...
        {
            return base.Perform_identity_resolution_reuses_same_instances_across_joins(
                async,
                useAsTracking
            );
        }
