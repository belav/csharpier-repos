...
public class CosmosTestStore : TestStore
{
    ...
    public static CosmosTestStore Create(
        string name,
    ...
    public static CosmosTestStore CreateInitialized(
        string name,
        Action<CosmosDbContextOptionsBuilder> extensionConfiguration = null
    ) =>
        (CosmosTestStore)
    ...
    private static string CreateName(string name) =>
        TestEnvironment.IsEmulator || name == "Northwind" ? name : name + _runId;
                                    ...
                                    switch (reader.Value)
                                    {
                                        ...
                                        case "Data":
                                            while (reader.Read())
                                            {
                                                if (reader.TokenType == JsonToken.StartObject)
                                                {
                                                    var document = serializer.Deserialize<JObject>(
                                                        reader
                                                    );
    ...
    public override void Clean(DbContext context) => CleanAsync(context).GetAwaiter().GetResult();
    ...
    public override void Dispose() =>
        throw new InvalidOperationException(
            "Calling Dispose can cause deadlocks. Use DisposeAsync instead."
        );

    public override async Task DisposeAsync()
    {
        if (_initialized && _dataFilePath == null)
        {
            if (_connectionAvailable == false)
            {
                return;
    ...
    public class FakeEntityType : Annotatable, IEntityType
    {
        ...
        public IEnumerable<IForeignKey> FindDeclaredForeignKeys(
        ...
        public IEnumerable<IForeignKey> FindForeignKeys(
            IReadOnlyList<IReadOnlyProperty> properties
        ) => throw new NotImplementedException();
        ...
        public Func<MaterializationContext, object> GetOrCreateMaterializer(
            IEntityMaterializerSource source
        ) => throw new NotImplementedException();

        public Func<MaterializationContext, object> GetOrCreateEmptyMaterializer(
            IEntityMaterializerSource source
        ) => throw new NotImplementedException();
