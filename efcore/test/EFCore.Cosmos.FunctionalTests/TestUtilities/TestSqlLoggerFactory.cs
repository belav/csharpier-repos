        ...
        public void AssertBaseline(string[] expected, bool assertOrder = true)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        var normalizedExpectedFragment = expectedFragment.Replace(
                                "\r",
                                string.Empty
                            )
                            .Replace("\n", _eol);
            ...
            )
            {
                ...
                if (eventId.Id == CosmosEventId.ExecutingReadItem)
                {
                    ...
                    if (message != null)
                    {
                        ...
                        var partitionKey = structure.Where(i => i.Key == "partitionKey")
                            .Select(i => (string)i.Value)
                            .First();
                        var resourceId = structure.Where(i => i.Key == "resourceId")
                            .Select(i => (string)i.Value)
                            .First();
