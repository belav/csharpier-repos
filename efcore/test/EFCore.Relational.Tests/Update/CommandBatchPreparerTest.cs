...
public class CommandBatchPreparerTest
{
    ...
    public void BatchCommands_does_not_create_separate_batch_without_principal_key_database_generation()
    {
        ...
        var entry = stateManager.GetOrCreateEntry(new FakeEntity { Id = 42, Value = "Test" });
        var relatedEntry1 = stateManager.GetOrCreateEntry(
            new RelatedFakeEntity { Id = 100, RelatedId = 42 }
        );
        var relatedEntry2 = stateManager.GetOrCreateEntry(
            new RelatedFakeEntity { Id = 101, RelatedId = 42 }
        );
        entry.SetEntityState(EntityState.Added);
        ...
        var batches = CreateBatches(
            new[] { relatedEntry1, entry, relatedEntry2 },
            new UpdateAdapter(stateManager)
        );
    ...
    public void BatchCommands_creates_separate_batch_with_principal_key_database_generation()
    {
        ...
        var temporaryIdValue = entry.GetCurrentValue<int>(
            entry.EntityType.GetProperty(nameof(FakeEntity.Id))
        );
        ...
        var relatedEntry2 = stateManager.GetOrCreateEntry(
            new RelatedFakeEntity { Id = 2, RelatedId = temporaryIdValue }
        );
        relatedEntry1.SetEntityState(EntityState.Added);
        relatedEntry2.SetEntityState(EntityState.Added);
        ...
        Assert.Collection(
            ...
            b =>
                Assert.Equal(
                    new[] { relatedEntry1, relatedEntry2 },
                    b.ModificationCommands.Select(m => m.Entries.Single())
                )
        );
    }
    ...
    public void BatchCommands_sorts_entities_while_reassigning_child_tree()
    {
        ...
        var batches = CreateBatches(
            new[] { newEntity, newChildEntity, oldEntity, oldChildEntity },
            modelData
        );
        var batch = Assert.Single(batches);
    ...
    {
        var configuration = FakeRelationalTestHelpers
            .Instance
        ...
        var relatedEntry = stateManager.GetOrCreateEntry(
            new RelatedFakeEntity { RelatedId = temporaryIdValue }
        );
        relatedEntry.SetEntityState(EntityState.Added);
        ...
        var batches = CreateCommandBatchPreparer(factory)
            .BatchCommands(new[] { relatedEntry, entry }, new UpdateAdapter(stateManager));
    ...
    public void Batch_command_does_not_order_non_unique_index_values()
    {
        ...
        fakeEntry2.SetOriginalValue(
            fakeEntry2.EntityType.FindProperty(nameof(FakeEntity.Value)),
            "Test"
        );
...
ForeignKeyConstraint { 'RelatedId' } FakeEntity [Added]" + CoreStrings.SensitiveDataDisabled;
    ...
    )
    {
        ...
        fakeEntry2.SetEntityState(EntityState.Modified);
        fakeEntry2.SetOriginalValue(
            fakeEntry2.EntityType.FindProperty(nameof(FakeEntity.UniqueValue)),
            "Test"
        );
...
FakeEntity [Deleted]" + CoreStrings.SensitiveDataDisabled;
    ...
    public void BatchCommands_works_with_duplicate_values_for_unique_indexes()
    {
        ...
        fakeEntry2.SetEntityState(EntityState.Modified);
        fakeEntry2.SetOriginalValue(
            fakeEntry.EntityType.FindProperty(nameof(FakeEntity.UniqueValue)),
            "Test"
        );
    ...
    )
    {
        ...
        else
        {
            ...
            Assert.Throws<EqualException>(() => Assert.Equal(5, command.ColumnModifications.Count));
        }
    ...
    )
    {
        ...
        {
            // Detect indirect update dependencies. Issue #17947.
            Assert.Throws<EqualException>(() => Assert.Equal(2, commandBatches.Count));
        }
        else
        {
    ...
    private static IModel CreateFKOneToManyModelWithGeneratedIds()
    {
        ...
        modelBuilder.Entity<FakeEntity>(b =>
        {
            ...
            b.HasMany<RelatedFakeEntity>().WithOne().HasForeignKey(c => c.RelatedId);
        });

        modelBuilder.Entity<RelatedFakeEntity>(b => b.Property(c => c.Id).ValueGeneratedOnAdd());
