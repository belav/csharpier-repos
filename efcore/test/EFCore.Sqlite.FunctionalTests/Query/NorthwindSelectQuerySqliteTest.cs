...
namespace Microsoft.EntityFrameworkCore.Query
{
    ...
    {
        public NorthwindSelectQuerySqliteTest(
...
FROM ""Orders"" AS ""o"""
        ...
        public override async Task Select_nested_collection_deep_distinct_no_identifiers(
            bool async
        ) =>
            Assert.Equal(
                SqliteStrings.ApplyNotSupported,
            ...
            Assert.Equal(
                ...
                (
                    await Assert.ThrowsAsync<InvalidOperationException>(
                        () =>
                            base
                                .Collection_projection_selecting_outer_element_followed_by_take(
                                    async
                                )
                    )
                ).Message
            ...
            Assert.Equal(
                ...
                (
                    await Assert.ThrowsAsync<InvalidOperationException>(
                        () =>
                            base
                                .Take_on_top_level_and_on_collection_projection_with_outer_apply(
                                    async
                                )
                    )
                ).Message
