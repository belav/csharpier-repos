...
public class SharedTypeDbSetTest
{
    ...
    )
    {
        ...
        Assert.Same(
            categoryEntry11.GetInfrastructure(),
            context.Entry(category11).GetInfrastructure()
        );
        Assert.Same(
            categoryEntry12.GetInfrastructure(),
            context.Entry(category12).GetInfrastructure()
        );
        Assert.Same(
            categoryEntry21.GetInfrastructure(),
            context.Entry(category21).GetInfrastructure()
        );
        Assert.Same(
            categoryEntry22.GetInfrastructure(),
            context.Entry(category22).GetInfrastructure()
        );
        Assert.Same(
            productEntry11.GetInfrastructure(),
            context.Entry(product11).GetInfrastructure()
        );
        Assert.Same(
            productEntry12.GetInfrastructure(),
            context.Entry(product12).GetInfrastructure()
        );
        Assert.Same(
            productEntry21.GetInfrastructure(),
            context.Entry(product21).GetInfrastructure()
        );
        Assert.Same(
            productEntry22.GetInfrastructure(),
            context.Entry(product22).GetInfrastructure()
        );
    }
    ...
    public void Can_add_no_existing_entities_to_set_to_be_deleted_Enumerable() =>
        TrackNoEntitiesTestEnumerable(
            (c, e) => c.Category2s.RemoveRange(e),
            (c, e) => c.Product1s.RemoveRange(e)
        );

    [ConditionalFact]
    public void Can_add_no_new_entities_to_set_Enumerable_graph() =>
        TrackNoEntitiesTestEnumerable(
            (c, e) => c.Category2s.AddRange(e),
            (c, e) => c.Product1s.AddRange(e)
        );
    ...
    public void Can_add_no_existing_entities_to_set_to_be_attached_Enumerable_graph() =>
        TrackNoEntitiesTestEnumerable(
            (c, e) => c.Category2s.AttachRange(e),
            (c, e) => c.Product1s.AttachRange(e)
        );

    [ConditionalFact]
    public void Can_add_no_existing_entities_to_set_to_be_updated_Enumerable_graph() =>
        TrackNoEntitiesTestEnumerable(
            (c, e) => c.Category2s.UpdateRange(e),
            (c, e) => c.Product1s.UpdateRange(e)
        );
    ...
    public void Throws_when_using_with_IListSource()
    {
        ...
        Assert.Equal(
            ...
            Assert
                .Throws<NotSupportedException>(() => ((IListSource)context.Gu1s).GetList())
                .Message
        );
    }
    ...
    public void Throws_when_using_Local_with_IListSource()
    {
        ...
        Assert.Equal(
            ...
            Assert
                .Throws<NotSupportedException>(() => ((IListSource)context.Gu1s.Local).GetList())
                .Message
        );
    }
