...
public partial class EntityTypeTest
{
    ...
    public void Can_configure_base_type()
    {
        ...
        Assert.Same(
            finalModel.FindEntityType(typeof(A).Name),
            finalModel.FindEntityType(typeof(B).Name).BaseType
        );
    }
    ...
    public void Setting_CLR_base_for_shadow_entity_type_should_throw()
    {
        ...
        Assert.Equal(
            CoreStrings.NotAssignableClrBaseType(
                typeof(B).Name + " (Dictionary<string, object>)",
                typeof(A).Name,
    ...
    public void Setting_shadow_base_for_CLR_entity_type_should_throw()
    {
        ...
        Assert.Equal(
            CoreStrings.NotAssignableClrBaseType(
                typeof(B).Name,
                typeof(A).Name + " (Dictionary<string, object>)",
                typeof(B).Name,
                "Dictionary<string, object>"
            ),
            Assert.Throws<InvalidOperationException>(() => b.BaseType = a).Message
    ...
    public void Properties_should_be_updated_when_base_type_changes()
    {
        ...
        Assert.Equal(
            new[] { "F", "H", "E", "G" },
            dType.GetProperties().Select(p => p.Name).ToArray()
        );
    ...
    public void Adding_keys_throws_when_there_is_a_parent_type()
    {
        ...
        Assert.Equal(
            ...
            Assert
                .Throws<InvalidOperationException>(() => b.SetPrimaryKey(b.AddProperty("G")))
                .Message
        );
        Assert.Equal(
            CoreStrings.DerivedEntityTypeKey(typeof(B).Name, typeof(A).Name),
    ...
    public void Navigations_on_base_type_should_be_inherited()
    {
        ...
        Assert.Equal(
            new[] { "Orders" },
            customerType.GetNavigations().Select(p => p.Name).ToArray()
        );
        Assert.Equal(
            Array.Empty<string>(),
            specialCustomerType.GetNavigations().Select(p => p.Name).ToArray()
        );
        ...
        var specialCustomerForeignKey = orderType.AddForeignKey(
            derivedForeignKeyProperty,
            customerKey,
            specialCustomerType
        );
        ...
        Assert.Same(
            customerType.FindNavigation("Orders"),
            specialCustomerType.FindNavigation("Orders")
        );
    }
    ...
    public void Navigations_should_be_updated_when_base_type_changes()
    {
        ...
        Assert.Equal(
            new[] { nameof(Customer.Orders) },
            customerType.GetNavigations().Select(p => p.Name).ToArray()
        );
        Assert.Equal(
            new[] { nameof(SpecialCustomer.DerivedOrders) },
            specialCustomerType.GetNavigations().Select(p => p.Name).ToArray()
        );
    }
    ...
    public void Adding_navigation_throws_when_grandparent_type_has_navigation_with_same_name()
    {
        ...
        Assert.Equal(
            "Orders",
            ((IReadOnlyEntityType)verySpecialCustomerType).GetNavigations().Single().Name
        );
    }
    ...
    public void Adding_navigation_throws_when_grandchild_type_has_navigation_with_same_name()
    {
        ...
        Assert.Equal(
            CoreStrings.NavigationForWrongForeignKey(
                nameof(Customer.Orders),
                typeof(VerySpecialCustomer).Name,
                "{'CustomerId'}",
                "{'Id'}"
            ),
            Assert
    ...
    public void Setting_base_type_throws_when_parent_contains_duplicate_navigation()
    {
        ...
        Assert.Equal(
            ...
            Assert
                .Throws<InvalidOperationException>(() => specialOrderType.BaseType = orderType)
                .Message
        );
    }
    ...
    public void ForeignKeys_on_base_type_should_be_inherited()
    {
        ...
        Assert.Equal(
            ...
            orderType
                .GetForeignKeys()
                .Select(fk => fk.Properties.Select(p => p.Name).ToArray())
                .ToArray()
        );
        Assert.Equal(
        ...
        Assert.Same(
            customerForeignKey,
            specialOrderType.FindForeignKey(foreignKeyProperty, customerKey, customerType)
        );
    }
    ...
    public void Adding_an_index_throws_if_properties_were_removed()
    {
        ...
        Assert.Equal(
            ...
            Assert
                .Throws<InvalidOperationException>(() => entityType.AddIndex(new[] { idProperty }))
                .Message
        );
    }
    ...
    public void Adding_an_index_throws_when_parent_type_has_index_on_same_properties()
    {
        ...
        Assert.Equal(
            CoreStrings.DuplicateIndex(
                new[] { indexProperty }.Format(),
                typeof(SpecialOrder).Name,
                typeof(Order).Name
            ),
            Assert
                .Throws<InvalidOperationException>(() => specialOrderType.AddIndex(indexProperty))
                .Message
        );
    }
    ...
    public void Adding_an_index_throws_when_child_type_has_index_on_same_properties()
    {
        ...
        Assert.Equal(
            ...
            Assert
                .Throws<InvalidOperationException>(() => orderType.AddIndex(indexProperty))
                .Message
        );
    }
