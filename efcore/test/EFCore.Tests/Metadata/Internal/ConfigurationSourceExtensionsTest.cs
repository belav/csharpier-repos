...
namespace Microsoft.EntityFrameworkCore.Metadata.Internal
{
    public class ConfigurationSourceExtensionsTest
    {
        ...
        public void Overrides_returns_expected_value()
        {
            ...
            Assert.False(
                ConfigurationSource.DataAnnotation.Overrides(ConfigurationSource.Explicit)
            );
            Assert.True(
                ConfigurationSource.DataAnnotation.Overrides(ConfigurationSource.DataAnnotation)
            );
            Assert.True(
                ConfigurationSource.DataAnnotation.Overrides(ConfigurationSource.Convention)
            );
            Assert.False(ConfigurationSource.Convention.Overrides(ConfigurationSource.Explicit));
            Assert.False(
                ConfigurationSource.Convention.Overrides(ConfigurationSource.DataAnnotation)
            );
        ...
        public void Max_returns_expected_value()
        {
            Assert.Equal(
                ...
                ConfigurationSource.Explicit.Max(ConfigurationSource.Convention)
            );
            Assert.Equal(
                ConfigurationSource.Explicit,
                ConfigurationSource.Convention.Max(ConfigurationSource.Explicit)
            );
