...
public class PhysicalAddressToStringConverterTest
{
    ...
    public void Can_convert_String_to_physical_address(string physicalAddress)
    {
        ...
        Assert.Equal(PhysicalAddress.Parse(physicalAddress), converter(physicalAddress));
    }
    ...
    [InlineData(
        new byte[] { 75, 75, 75, 45, 53, 57, 45, 68, 48, 45, 57, 57, 45, 69, 49, 45, 56, 53 }
    )]
    public void Can_convert_bytes_to_physical_address(byte[] bytesPhysicalAddressInvalid)
    {
        var converter = new PhysicalAddressToStringConverter().ConvertFromProviderExpression
            .Compile();
