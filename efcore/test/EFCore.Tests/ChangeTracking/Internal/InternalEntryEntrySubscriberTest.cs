...
public class InternalEntryEntrySubscriberTest
{
    ...
    )
    {
        ...
        Assert.Equal(
            CoreStrings.NonNotifyingCollection(
                "RelatedCollection",
                "FullNotificationEntity",
                "List<ChangedOnlyNotificationEntity>",
                changeTrackingStrategy
            ),
            Assert
                .Throws<InvalidOperationException>(() => entry.SetEntityState(EntityState.Unchanged)
                )
                .Message
        );
    }
    ...
    public void Entry_unsubscribes_to_INotifyPropertyChanging_and_INotifyPropertyChanged()
    {
        ...
        for (var i = 0; i < 10; i++)
        {
            ...
            entries.Add(
                contextServices.GetRequiredService<IStateManager>().GetOrCreateEntry(entities[i])
            );
    ...
    {
        public List<
            Tuple<InternalEntityEntry, INavigationBase, IEnumerable<object>, IEnumerable<object>>
        > CollectionChanged { get; } = new();
