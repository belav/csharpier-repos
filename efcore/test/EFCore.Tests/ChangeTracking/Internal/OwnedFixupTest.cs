...
public class OwnedFixupTest
{
    ...
    public void Detaching_owner_does_not_delete_owned_entities(bool delayCascade)
    {
        ...
        var thing = new Thing
        {
            ...
            {
                new() { OwnedByThingId = Guid.NewGuid() },
                new() { OwnedByThingId = Guid.NewGuid() },
            },
        };
    ...
    public void Can_detach_Added_owner_referencing_detached_weak_owned_entity()
    {
        ...
        Assert.Equal(
            ...
            context.Entry(owner).Reference(e => e.Child1).TargetEntry.State
        );
    ...
    public void Can_get_owned_entity_entry()
    {
        ...
        Assert.Same(
            dependentEntry1.GetInfrastructure(),
    ...
    )
    {
        ...
        Assert.Equal(
            entityState != EntityState.Unchanged || useTrackGraph == null,
    ...
    public void Instance_changed_unidirectional(EntityState entityState)
    {
        ...
        Assert.Equal(
            entityState == EntityState.Added ? 3 : 5,
    ...
    public void Instance_changed_bidirectional(EntityState entityState)
    {
        ...
        Assert.Equal(
            entityState == EntityState.Added ? 4 : 7,
            context.ChangeTracker.Entries().Count()
        );
        Assert.Null(principal.Child2);
    ...
    )
    {
        ...
        var subDependentEntry = dependentEntry2
            .Collection(p => p.SubChildCollection)
    ...
    public void Identity_swapped_unidirectional(EntityState entityState)
    {
        ...
        Assert.Equal(
            entityState == EntityState.Added ? 5 : 9,
            context.ChangeTracker.Entries().Count()
        );
    ...
    public void Identity_swapped_bidirectional(EntityState entityState)
    {
        ...
        Assert.Equal(
            entityState == EntityState.Added ? 7 : 13,
            context.ChangeTracker.Entries().Count()
        );
    ...
    )
    {
        ...
        var newDependentEntry1 = context
            .Entry(principal)
            .Collection(p => p.ChildCollection1)
            .FindEntry(dependent2);
    ...
    public void Parent_changed_unidirectional(EntityState entityState)
    {
        ...
        if (entityState != EntityState.Added)
        {
            Assert.Equal(
                ...
                Assert
                    .Throws<InvalidOperationException>(() => context.ChangeTracker.DetectChanges())
                    .Message
            );
        }
        else
    ...
    public void Parent_and_identity_changed_unidirectional(EntityState entityState)
    {
        ...
        Assert.Equal(
            entityState == EntityState.Added ? 4 : 6,
            context.ChangeTracker.Entries().Count()
        );
        Assert.Null(principal1.Child1);
        Assert.Null(principal1.Child2);
    ...
    public void Parent_and_identity_swapped_unidirectional(EntityState entityState)
    {
        ...
        Assert.Equal(
            entityState == EntityState.Added ? 6 : 10,
            context.ChangeTracker.Entries().Count()
        );
        Assert.Null(principal1.Child1);
    ...
    public void Can_set_nested_dependent_to_null(
        EntityState entityState,
        bool null1,
        bool null2,
        bool nullC
    )
    {
        ...
        void AssertChildren()
        {
            ...
            Assert.Same(
                nullC ? null : newSubChildCollection.Single(),
                newChild.SubChildCollection?.Single()
            );
        }
    }
    ...
    public void Can_save_multiple_deep_owned_entities()
    {
        ...
        {
            ShippingCenters = new List<StreetAddress> { address1, address2 },
        };
    ...
    private class Info
    {
        ...
        public static void OnModelCreating<T>(OwnedNavigationBuilder<T, Info> rob)
            where T : class => rob.Property(e => e.Title);
    }
    ...
    public void Entities_with_owned_custom_enum_pattern_are_tracked_correctly_if_not_shared()
    {
        ...
        Assert.Equal(
            "EUR",
            order.TestOrderItems.Single(e => e.ProductName == "Test Product 1").Price.Currency.Code
        );
        Assert.Equal(
            "USD",
            order.TestOrderItems.Single(e => e.ProductName == "Test Product 3").Price.Currency.Code
        );
    ...
    public void Entities_with_owned_custom_enum_pattern_using_ValueConverter_are_tracked_correctly()
    {
        ...
        Assert.Equal(
            "EUR",
            order.TestOrderItems.Single(e => e.ProductName == "Test Product 2").Price.Currency.Code
        );
        Assert.Equal(
            "USD",
            order.TestOrderItems.Single(e => e.ProductName == "Test Product 3").Price.Currency.Code
        ...
        Assert.Equal(
            "USD",
            order.TestOrderItems.Single(e => e.ProductName == "Test Product 4").Price.Currency.Code
        );
                    ...
                    ip =>
                    {
                        ...
                        ip.Property(p => p.Currency)
                            .HasConversion(
                                v => v.Code,
                                v =>
                                    v == "EUR" ? TestCurrency.EUR
                                    : v == "USD" ? TestCurrency.USD
                                    : null
                            );
                    }
                )
    ...
    public void Equatable_entities_that_comply_are_tracked_correctly()
    {
        ...
        using (var context = new EquatableEntitiesContext("EquatableEntities"))
        {
            ...
            user.SetRoles(
                new[]
                {
                    new Role { Value = "Pascal" },
                    new Role { Value = "Smalltalk" },
                    new Role { Value = "COBOL" },
                }
            );
            ...
            user.SetRoles(new List<Role> { new() { Value = "BASIC" } });
    ...
    private class User
    {
        ...
        {
            if (_roles.Count == roles.Count && !_roles.Except(roles).Any())
            {
                return;
            }
                    ...
                    cb =>
                    {
                        ...
                        cb.OwnsOne(
                            ...
                            sb =>
                            {
                                ...
                                sb.WithOwner(c => c.Parent).HasForeignKey("ParentId", "ChildId");
                            }
                        );
    ...
    private static ICollection<T> CreateChildCollection<T>(
        ...
        collectionType switch
        {
            ...
            CollectionType.ObservableHashSet => new ObservableHashSet<T>(
