    ...
    public class DbLoggerCategoryTest
    {
        ...
        public void Logger_categories_have_the_correct_names()
        {
            ...
            Assert.Equal(
                ...
                DbLoggerCategory.Scaffolding.Name
            );
        ...
        public void DbLoggerCategory_instances_generate_the_correct_names()
        {
            ...
            Assert.Equal(
                DbLoggerCategory.Database.Command.Name,
                new DbLoggerCategory.Database.Command()
            );
            Assert.Equal(
                DbLoggerCategory.Database.Connection.Name,
                new DbLoggerCategory.Database.Connection()
            );
            Assert.Equal(
                DbLoggerCategory.Database.Transaction.Name,
                new DbLoggerCategory.Database.Transaction()
            );
            Assert.Equal(
                DbLoggerCategory.Infrastructure.Name,
                new DbLoggerCategory.Infrastructure()
            );
            ...
            Assert.Equal(
                DbLoggerCategory.Model.Validation.Name,
                new DbLoggerCategory.Model.Validation()
            );
