...
namespace Microsoft.EntityFrameworkCore
{
    public class DbSetTest
    {
        ...
        )
        {
            ...
            Assert.Same(
                categoryEntry2.GetInfrastructure(),
                context.Entry(category2).GetInfrastructure()
            );
            Assert.Same(
            ...
            Assert.Same(
                productEntry2.GetInfrastructure(),
                context.Entry(product2).GetInfrastructure()
            );
        }
        ...
        public async Task Can_enumerate_with_await_foreach_with_cancellation()
        {
            using var context = new EarlyLearningCenter();
            await foreach (
                var _ in context.Categories.AsAsyncEnumerable().WithCancellation(default)
            )
            {
                throw new Exception("DbSet should be empty");
            }
        ...
        private class EarlyLearningCenter : DbContext
        {
            ...
            protected internal override void OnModelCreating(ModelBuilder modelBuilder)
            {
                ...
                modelBuilder
                    .SharedTypeEntity<Dictionary<string, object>>("SharedEntity")
