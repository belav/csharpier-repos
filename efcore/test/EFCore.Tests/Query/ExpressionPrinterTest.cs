...
public class ExpressionPrinterTest
{
    ...
    public void UnaryExpression_printed_correctly()
    {
        ...
        Assert.Equal(
            ...
            _expressionPrinter.PrintExpression(
                Expression.Throw(Expression.Constant("Some exception"))
            )
        );
        Assert.Equal(
            "!(True)",
            _expressionPrinter.PrintExpression(Expression.Not(Expression.Constant(true)))
        );
        Assert.Equal(
    ...
    public void Simple_lambda_printed_correctly() =>
        Assert.Equal(
            "prm => 42",
            _expressionPrinter.PrintExpression(
                Expression.Lambda(Expression.Constant(42), Expression.Parameter(typeof(int), "prm"))
            )
        );
...
{ 
    ...
    public void Enumerable_Constant_printed_correctly() =>
        Assert.Equal(
            @"int[] { 1, 2, 3 }",
            _expressionPrinter.PrintExpression(Expression.Constant(new[] { 1, 2, 3 }))
        );
}
