...
namespace Microsoft.EntityFrameworkCore.TestUtilities
{
    public class SqlServerTestStore : RelationalTestStore
    {
        ...
        public static SqlServerTestStore GetNorthwindStore() =>
            (SqlServerTestStore)SqlServerNorthwindTestStoreFactory
                .Instance
        ...
        public static SqlServerTestStore GetOrCreate(
            string name,
            string scriptPath,
                ...
                async command =>
                {
                    ...
                    while (await dataReader.ReadAsync())
                    {
                        results = results.Concat(
                            new[] { await dataReader.GetFieldValueAsync<T>(0) }
                        );
                    }
        ...
        )
        {
            var builder = new SqlConnectionStringBuilder(TestEnvironment.DefaultConnection)
            {
                MultipleActiveResultSets =
                    multipleActiveResultSets ?? Random.Shared.Next(0, 2) == 1,
                InitialCatalog = name
            };
            if (fileName != null)
            {
