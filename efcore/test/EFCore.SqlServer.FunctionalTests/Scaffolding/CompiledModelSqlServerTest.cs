...
public class CompiledModelSqlServerTest : CompiledModelRelationalTestBase
{
    protected override void BuildBigModel(ModelBuilder modelBuilder, bool jsonColumns)
    {
        ...
        modelBuilder.UseCollation("Latin1_General_CS_AS").UseIdentityColumns(3, 2);
        ...
        modelBuilder.Entity<PrincipalBase>(eb =>
        {
            ...
            eb.HasKey(e => new { e.Id, e.AlternateId }).IsClustered();

            eb.OwnsOne(
                e => e.Owned,
                ob =>
                {
                    ob.Property(e => e.Details).IsSparse().UseCollation("Latin1_General_CI_AI");
    ...
    protected override void AssertBigModel(IModel model, bool jsonColumns)
    {
        ...
        if (jsonColumns)
        {
            ...
            Assert.Equal(
                ...
                principalId.GetValueGenerationStrategy()
            );
        }
        else
        {
        ...
        Assert.Equal(
            CoreStrings.RuntimeModelMissingData,
            Assert
                .Throws<InvalidOperationException>(() => principalId.GetIdentityIncrement())
                .Message
        );
        ...
        var principalTable = StoreObjectIdentifier
            .Create(referenceOwnedType, StoreObjectType.Table)!
            .Value;
        if (jsonColumns)
        {
            Assert.Equal(
    ...
    protected override void BuildTpcModel(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.HasDatabaseMaxSize("20TB").HasPerformanceLevel("High").HasServiceTier("AB");
    ...
    protected override void AssertTpc(IModel model)
    {
        ...
        Assert.Equal(
            "Id",
            id.GetColumnName(
                StoreObjectIdentifier.Create(principalBase, StoreObjectType.Table)!.Value
            )
        );
        Assert.Null(
            id.FindOverrides(
        ...
        Assert.Equal(
            CoreStrings.RuntimeModelMissingData,
            Assert
                .Throws<InvalidOperationException>(() => alternateIndex.GetIncludeProperties())
                .Message
        );
        ...
        Assert.Equal(
            CoreStrings.RuntimeModelMissingData,
            Assert
                .Throws<InvalidOperationException>(() => alternateIndex.GetDataCompression())
                .Message
        );
        ...
        modelBuilder.Entity<PrincipalBase>(eb =>
        {
            eb.ComplexProperty(
                ...
                eb =>
                {
                    ...
                    eb.Property(c => c.Details).IsSparse().UseCollation("Latin1_General_CI_AI");
                }
            );
        });
    }
    ...
    protected override void AssertComplexTypes(IModel model)
    {
        ...
        Assert.Equal(
            SqlServerValueGenerationStrategy.None,
            detailsProperty.GetValueGenerationStrategy()
        );
        Assert.Equal(
            CoreStrings.RuntimeModelMissingData,
            Assert
                .Throws<InvalidOperationException>(() => detailsProperty.GetIdentitySeed())
                .Message
        );
        Assert.Equal(
            CoreStrings.RuntimeModelMissingData,
            Assert
                .Throws<InvalidOperationException>(() => detailsProperty.GetIdentityIncrement())
                .Message
        );
        Assert.Equal(
            CoreStrings.RuntimeModelMissingData,
    ...
    public virtual void Key_HiLo_sequence() =>
        Test(
            ...
            model =>
            {
                ...
                Assert.Same(
                    hiLo,
                    dataEntity.FindPrimaryKey()!.Properties.Single().FindHiLoSequence()
                );
            }
        );
    ...
    public virtual void Key_sequence() =>
        Test(
            ...
            model =>
            {
                ...
                Assert.Same(
                    keySequence,
                    dataEntity!.FindPrimaryKey()!.Properties.Single().FindSequence()
                );
            }
        );
    ...
    public virtual void SpatialTypesTest() =>
        Test(
            modelBuilder =>
                modelBuilder.Entity<SpatialTypes>(eb =>
                {
            ...
            model =>
            {
                ...
                Assert.Equal(
                    SqlServerValueGenerationStrategy.None,
                    pointProperty.GetValueGenerationStrategy()
                );
