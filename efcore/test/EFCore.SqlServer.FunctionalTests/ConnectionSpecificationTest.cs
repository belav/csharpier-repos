...
public class ConnectionSpecificationTest
{
    ...
    {
        var serviceProvider = new ServiceCollection()
            .AddSingleton(_ => new SqlConnection(
                SqlServerNorthwindTestStoreFactory.NorthwindConnectionString
            ))
            .AddDbContext<OwnedConnectionInOnConfiguringContext>()
            .BuildServiceProvider(validateScopes: true);
        ...
        using (SqlServerTestStore.GetNorthwindStore())
        {
            ...
            using var scope = serviceProvider.CreateScope();
            var context =
                scope.ServiceProvider.GetRequiredService<OwnedConnectionInOnConfiguringContext>();
            Assert.True(context.Customers.Any());
        }
    ...
    public void Can_specify_then_change_connection()
    {
        ...
        using (SqlServerTestStore.GetNorthwindStore())
        {
            ...
            using var newConnection = new SqlConnection(
                SqlServerNorthwindTestStoreFactory.NorthwindConnectionString
            );
    ...
    )
    {
        ...
        var serviceProvider = new ServiceCollection()
            ...
            .AddDbContext<UseConfigurationContext>(b =>
                b.UseSqlServer(connectionString).EnableServiceProviderCaching(false)
            )
            .BuildServiceProvider(validateScopes: true);
