...
CREATE TYPE stringAlias FROM nvarchar(50);"
                ...
                var facetedAliasEntityType = model.FindEntityType(
                    typeof(TypeAliasEntityWithFacets)
                );
                Assert.Equal(
                    "datetimeAlias",
                    GetColumnType(
                ...
                Assert.Equal(
                    ...
                    GetColumnType(
                        facetedAliasEntityType!,
                        nameof(TypeAliasEntityWithFacets.FloatAlias)
                    )
                );
                Assert.Equal(
                    "binaryAlias",
            ...
            using (var context = new TypeAliasContext(options))
            {
                ...
                var entityWithFacets = context.Set<TypeAliasEntityWithFacets>()
