...
public class SqliteTypeMappingSourceTest : RelationalTypeMappingSourceTestBase
{
    ...
    public void Does_default_mappings_for_values()
    {
        ...
        Assert.Equal(
            "INTEGER",
            CreateRelationalTypeMappingSource(model).GetMappingForValue(1).StoreType
        );
        Assert.Equal(
            "INTEGER",
            CreateRelationalTypeMappingSource(model).GetMappingForValue(1L).StoreType
        );
        Assert.Equal(
            "INTEGER",
            CreateRelationalTypeMappingSource(model).GetMappingForValue((byte)1).StoreType
        );
        Assert.Equal(
            "INTEGER",
            CreateRelationalTypeMappingSource(model).GetMappingForValue((short)1).StoreType
        );
        Assert.Equal(
            "INTEGER",
            CreateRelationalTypeMappingSource(model).GetMappingForValue((uint)1).StoreType
        );
        Assert.Equal(
            "INTEGER",
            CreateRelationalTypeMappingSource(model).GetMappingForValue((ulong)1).StoreType
        );
        Assert.Equal(
            "INTEGER",
            CreateRelationalTypeMappingSource(model).GetMappingForValue((sbyte)1).StoreType
        );
        Assert.Equal(
            "INTEGER",
            CreateRelationalTypeMappingSource(model).GetMappingForValue((ushort)1).StoreType
        );
        Assert.Equal(
            "TEXT",
            CreateRelationalTypeMappingSource(model).GetMappingForValue(1.0m).StoreType
        );
        Assert.Equal(
            "REAL",
            CreateRelationalTypeMappingSource(model).GetMappingForValue(1.0).StoreType
        );
        Assert.Equal(
            "REAL",
            CreateRelationalTypeMappingSource(model).GetMappingForValue(1.0f).StoreType
        );
    }
