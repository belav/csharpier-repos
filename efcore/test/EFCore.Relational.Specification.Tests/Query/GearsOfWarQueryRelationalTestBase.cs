    ...
    : GearsOfWarQueryTestBase<TFixture> where TFixture : GearsOfWarQueryFixtureBase, new()
{
    ...
    public virtual Task Parameter_used_multiple_times_take_appropriate_inferred_type_mapping(
        bool async
    )
    {
        var place = "Seattle";
    ...
    public virtual async Task Project_discriminator_columns(bool async)
    {
        ...
        await AssertQuery(
            ...
            ss =>
                ss.Set<Faction>()
                    .Select(
                        f => new { f.Id, Discriminator = EF.Property<string>(f, "Discriminator") }
                    ),
        ...
        await AssertQuery(
            ...
            ss =>
                ss.Set<Faction>()
                    ...
                    .Select(
                        lh =>
                            new { lh.Id, Discriminator = EF.Property<string>(lh, "Discriminator") }
                    ),
            elementSorter: e => e.Id
        );
