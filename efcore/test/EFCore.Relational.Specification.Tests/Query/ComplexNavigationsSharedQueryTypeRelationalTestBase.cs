...
{
    public abstract class ComplexNavigationsSharedQueryTypeRelationalTestBase<TFixture>
        ...
        where TFixture : ComplexNavigationsSharedTypeQueryRelationalFixtureBase, new()
    {
        ...
        public virtual Task Filtered_include_basic_Where_split(bool async)
        {
            return AssertQuery(
                ...
                ss =>
                    ss.Set<Level1>()
                        .Include(l1 => l1.OneToMany_Optional1.Where(l2 => l2.Id > 5))
                        .AsSplitQuery(),
                elementAsserter: (e, a) =>
                    AssertInclude(
                        e,
            ...
            var query = ctx.LevelOne.Include(
                    l1 =>
                        l1.OneToMany_Optional1.Where(
                                x => ctx.LevelOne.Count(xx => xx.Id != x.Id) > 1
                            )
                            .OrderBy(x => x.Id)
                            .Take(3)
                )
        ...
        public virtual void Filtered_include_outer_parameter_used_inside_filter_split()
        {
            ...
            using var ctx = CreateContext();
            var query = ctx.LevelOne.AsSplitQuery()
                .Select(
                    l1 =>
                        new
                        {
                            l1.Id,
                            FullInclude = ctx.LevelTwo.Include(l2 => l2.OneToMany_Optional2)
                                .ToList(),
                            FilteredInclude = ctx.LevelTwo.Include(
