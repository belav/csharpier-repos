    ...
    where TFixture : NorthwindQueryRelationalFixture<NoopModelCustomizer>, new()
{
    ...
    public virtual Task FromSqlRaw_queryable_simple(bool async) =>
        AssertQuery(
            async,
            ss =>
                ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                    NormalizeDelimitersInRawString(
    ...
    public virtual Task FromSqlRaw_composed_contains(bool async) =>
        AssertQuery(
            ...
            ss =>
                from c in ss.Set<Customer>()
                where ss.Set<Order>().Select(o => o.CustomerID).Contains(c.CustomerID)
                select c
        );
    ...
    public virtual Task FromSqlRaw_composed_contains2(bool async) =>
        AssertQuery(
            ...
            ss =>
                ...
                where
                    c.CustomerID == "ALFKI"
                    && ss.Set<Order>().Select(o => o.CustomerID).Contains(c.CustomerID)
                select c
        );
    ...
    public virtual Task FromSqlRaw_queryable_multiple_composed(bool async) =>
        AssertQuery(
            async,
            ss =>
                from c in ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
    ...
    )
    {
        ...
        await AssertQuery(
            ...
            ss =>
                ...
                select new { c, o },
            ss =>
                from c in ss.Set<Customer>().Where(x => x.City == city)
                from o in ss.Set<Order>()
                    .Where(x => x.OrderDate >= startDate && x.OrderDate <= endDate)
                where c.CustomerID == o.CustomerID
                select new { c, o }
...
FROM [Customers]"
    ...
    public virtual Task FromSqlRaw_queryable_with_parameters_inline(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<Customer>()
                    .Where(x => x.City == "London" && x.ContactTitle == "Sales Representative")
        );
    ...
    public virtual Task FromSqlInterpolated_queryable_with_parameters_interpolated(bool async)
    {
        ...
        return AssertQuery(
            async,
            ss =>
                ((DbSet<Customer>)ss.Set<Customer>()).FromSqlInterpolated(
    ...
    public virtual Task FromSql_queryable_with_parameters_interpolated(bool async)
    {
        ...
        return AssertQuery(
            async,
            ss =>
                ((DbSet<Customer>)ss.Set<Customer>()).FromSql(
                    NormalizeDelimitersInInterpolatedString(
                        $"SELECT * FROM [Customers] WHERE [City] = {city} AND [ContactTitle] = {contactTitle}"
                    )
                ),
    ...
    public virtual Task FromSqlRaw_queryable_with_null_parameter(bool async)
    {
        ...
        return AssertQuery(
            async,
            ss =>
                ((DbSet<Employee>)ss.Set<Employee>()).FromSqlRaw(
                    NormalizeDelimitersInRawString(
...
AND (([UnitsInStock] + [UnitsOnOrder]) < [ReorderLevel])"
            ...
            ss =>
                ss.Set<Product>()
                    .Where(x =>
                        x.Discontinued != true && (x.UnitsInStock + x.UnitsOnOrder) < x.ReorderLevel
                    )
    ...
    public virtual Task FromSqlRaw_with_SelectMany_and_include(bool async) =>
        AssertQuery(
            async,
            ss =>
                from c1 in ((DbSet<Customer>)ss.Set<Customer>()).FromSqlRaw(
                    NormalizeDelimitersInRawString(
                        "SELECT * FROM [Customers] WHERE [CustomerID] = 'ALFKI'"
                    )
                )
                from c2 in ((DbSet<Customer>)ss.Set<Customer>())
                    .FromSqlRaw(
                        NormalizeDelimitersInRawString(
                ...
                select new { c1, c2 },
            ss =>
                from c1 in ss.Set<Customer>().Where(x => x.CustomerID == "ALFKI")
    ...
    public virtual Task FromSqlRaw_with_join_and_include(bool async) =>
        AssertQuery(
            ...
            ss =>
                ...
                join o in ((DbSet<Order>)ss.Set<Order>())
                    .FromSqlRaw(
                        NormalizeDelimitersInRawString(
                ...
                select new { c, o },
            ss =>
                from c in ss.Set<Customer>().Where(x => x.CustomerID == "ALFKI")
    ...
    public virtual async Task FromSqlInterpolated_parameterization_issue_12213(bool async)
    {
        ...
        var query2 = context
            .Orders.Where(o => o.OrderID <= max && query1.Contains(o.OrderID))
            .Select(o => o.OrderID);
    ...
    public virtual async Task FromSqlRaw_does_not_parameterize_interpolated_string(bool async)
    {
        ...
        await AssertQuery(
            async,
            ss =>
                ((DbSet<Order>)ss.Set<Order>()).FromSqlRaw(
                    NormalizeDelimitersInRawString(
    ...
    public virtual Task Keyless_entity_with_all_nulls(bool async) =>
        AssertQuery(
            async,
            ss =>
                ((DbSet<OrderQuery>)ss.Set<OrderQuery>())
                    .FromSqlRaw(
                        NormalizeDelimitersInRawString(
                    ...
                    .IgnoreQueryFilters(),
            ss =>
                ss.Set<Customer>().Where(x => x.City == "Berlin").Select(x => new OrderQuery(null))
        );
        ...
        await AssertAny(
            ...
            ss =>
                ss.Set<Customer>()
                    ...
                    .Select(x => new OrderQuery(x.CustomerID))
        );
    ...
    public virtual async Task FromSql_used_twice_with_parameters(bool async)
    {
        await AssertAny(
            ...
            ss =>
                ((DbSet<OrderQuery>)ss.Set<OrderQuery>())
                    .FromSqlRaw(
                        NormalizeDelimitersInRawString("SELECT {0} AS [CustomerID]"),
                        "ALFKI"
                    )
                    .IgnoreQueryFilters(),
            ss =>
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task FromSqlRaw_in_subquery_with_positional_dbParameter_without_name(
        bool async
    ) =>
        AssertQuery(
            async,
        ...
        await AssertQuery(
            ...
            ss =>
                ss.Set<Order>()
                    .Where(o =>
                        ss.Set<Customer>()
                            .Where(x => x.City == city && x.ContactTitle == title)
                            .Select(c => c.CustomerID)
                            .Contains(o.CustomerID)
                    )
        );
...
SELECT * FROM [Customers2]"
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual async Task Multiple_occurrences_of_FromSql_with_db_parameter_adds_parameter_only_once(
        bool async
    )
    {
        using var context = CreateContext();
