    ...
    where TFixture : JsonQueryFixtureBase, new()
{
    ...
    public virtual Task Basic_json_projection_owner_entity(bool async) =>
        AssertQuery(async, ss => ss.Set<JsonEntityBasic>());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Basic_json_projection_owner_entity_NoTracking(bool async) =>
        AssertQuery(async, ss => ss.Set<JsonEntityBasic>().AsNoTracking());
    ...
    public virtual Task Basic_json_projection_owner_entity_twice_NoTracking(bool async) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<JsonEntityBasic>().Select(x => new { First = x, Second = x }).AsNoTracking(),
            elementSorter: e => e.First.Id,
            elementAsserter: (e, a) =>
            {
                AssertEqual(e.First, a.First);
        ...
        var message = (
            await Assert.ThrowsAsync<InvalidOperationException>(() =>
                AssertQuery(async, ss =>
                    ss.Set<JsonEntityBasic>().Select(x => x.OwnedReferenceRoot)
                )
            )
        ).Message;
    ...
    public virtual async Task Project_json_collection_in_tracking_query_fails(bool async)
    {
        var message = (
            await Assert.ThrowsAsync<InvalidOperationException>(() =>
                AssertQuery(async, ss =>
                    ss.Set<JsonEntityBasic>().Select(x => x.OwnedCollectionRoot)
                )
            )
        ).Message;
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual async Task Project_json_entity_in_tracking_query_fails_even_when_owner_is_present(
        bool async
    )
    {
        var message = (
    ...
    public virtual Task Basic_json_projection_owned_reference_root(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>().Select(x => x.OwnedReferenceRoot).AsNoTracking()
        );
    ...
    public virtual Task Basic_json_projection_scalar(bool async) =>
        AssertQuery(async, ss => ss.Set<JsonEntityBasic>().Select(x => x.OwnedReferenceRoot.Name));
    ...
    public virtual Task Json_scalar_length(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Where(x => x.OwnedReferenceRoot.Name.Length > 2)
                .Select(x => x.Name)
        );
    ...
    public virtual Task Basic_json_projection_enum_inside_json_entity(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x => new { x.Id, x.OwnedReferenceRoot.OwnedReferenceBranch.Enum }),
    ...
    public virtual Task Json_projection_enum_with_custom_conversion(bool async) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<JsonEntityCustomNaming>()
                    .Select(x => new { x.Id, x.OwnedReferenceRoot.Enum }),
            elementSorter: e => e.Id,
            elementAsserter: (e, a) =>
            {
                Assert.Equal(e.Id, a.Id);
    ...
    public virtual Task Json_property_in_predicate(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Where(x => x.OwnedReferenceRoot.OwnedReferenceBranch.Fraction < 20.5M)
                .Select(x => x.Id)
        );
    ...
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_subquery_reference_pushdown_reference_anonymous_projection(
        bool async
    ) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<JsonEntityBasic>()
                    ...
                    .Take(10)
                    .Distinct()
                    .Select(x => new { x.Entity.OwnedReferenceBranch, x.Scalar.Length })
                    .AsNoTracking(),
            elementSorter: e =>
                (e.OwnedReferenceBranch.Date, e.OwnedReferenceBranch.Fraction, e.Length),
            elementAsserter: (e, a) =>
            {
                AssertEqual(e.OwnedReferenceBranch, a.OwnedReferenceBranch);
    ...
    [ConditionalTheory(Skip = "issue #24263")]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_subquery_reference_pushdown_reference_pushdown_anonymous_projection(
        bool async
    ) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<JsonEntityBasic>()
                    ...
                    .AsNoTracking(),
            elementSorter: e =>
                (e.OwnedReferenceLeaf.SomethingSomething, e.OwnedCollectionLeaf.Count, e.Length),
            elementAsserter: (e, a) =>
            {
                AssertEqual(e.OwnedReferenceLeaf, a.OwnedReferenceLeaf);
    ...
    public virtual Task Json_subquery_reference_pushdown_reference_pushdown_reference(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
                ...
                .Take(10)
                .Distinct()
                .Select(x => x.OwnedReferenceLeaf)
                .AsNoTracking()
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_subquery_reference_pushdown_reference_pushdown_collection(
        bool async
    ) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<JsonEntityBasic>()
                    ...
                    .Take(10)
                    .Distinct()
                    .Select(x => x.OwnedCollectionLeaf)
                    .AsNoTracking(),
            elementAsserter: (e, a) => AssertCollection(e, a, ordered: true)
        );
    ...
    public virtual Task Custom_naming_projection_owner_entity(bool async) =>
        AssertQuery(async, ss => ss.Set<JsonEntityCustomNaming>().Select(x => x));
    ...
    public virtual Task Custom_naming_projection_owned_scalar(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<JsonEntityCustomNaming>()
                .Select(x => x.OwnedReferenceRoot.OwnedReferenceBranch.Fraction)
        );
    ...
    public virtual Task Project_entity_with_single_owned(bool async) =>
        AssertQuery(async, ss => ss.Set<JsonEntitySingleOwned>());
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_json_entity_FirstOrDefault_subquery_with_binding_on_top(
        bool async
    ) =>
        AssertQueryScalar(async, ss =>
            ss.Set<JsonEntityBasic>()
                ...
                .Select(x =>
                    ss.Set<JsonEntityBasic>()
                        ...
                        .Select(xx => xx.OwnedReferenceRoot)
                        .FirstOrDefault()
                        .OwnedReferenceBranch.Date
                )
        );

    [ConditionalTheory(Skip = "issue #28733")]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_json_entity_FirstOrDefault_subquery_with_entity_comparison_on_top(
        bool async
    ) =>
        AssertQueryScalar(async, ss =>
            ss.Set<JsonEntityBasic>()
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_json_entity_FirstOrDefault_subquery_deduplication_and_outer_reference(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Project_json_entity_FirstOrDefault_subquery_deduplication_outer_reference_and_pruning(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    public virtual Task Json_entity_with_inheritance_basic_projection(bool async) =>
        AssertQuery(async, ss => ss.Set<JsonEntityInheritanceBase>());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_entity_with_inheritance_project_derived(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityInheritanceBase>().OfType<JsonEntityInheritanceDerived>()
        );
    ...
    public virtual Task Json_entity_backtracking(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Select(x =>
                    x.OwnedReferenceRoot.OwnedReferenceBranch.OwnedReferenceLeaf.Parent.Date
                )
        );
    ...
    public virtual Task Json_collection_index_in_projection_basic(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>().Select(x => x.OwnedCollectionRoot[1]).AsNoTracking()
        );
    ...
    public virtual Task Json_collection_ElementAt_in_projection(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Select(x => x.OwnedCollectionRoot.AsQueryable().ElementAt(1))
                .AsNoTracking()
        );
    ...
    public virtual Task Json_collection_ElementAtOrDefault_in_projection(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Select(x => x.OwnedCollectionRoot.AsQueryable().ElementAtOrDefault(1))
                .AsNoTracking()
        );
    ...
    public virtual Task Json_collection_index_in_projection_project_collection(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x => x.OwnedCollectionRoot[1].OwnedCollectionBranch)
                    .AsNoTracking(),
            elementAsserter: (e, a) => AssertCollection(e, a, ordered: true)
        );
    ...
    public virtual Task Json_collection_ElementAt_project_collection(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x =>
                        x.OwnedCollectionRoot.AsQueryable().ElementAt(1).OwnedCollectionBranch
                    )
                    .AsNoTracking(),
            elementAsserter: (e, a) => AssertCollection(e, a, ordered: true)
        );
    ...
    public virtual Task Json_collection_index_in_projection_using_parameter(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>().Select(x => x.OwnedCollectionRoot[prm]).AsNoTracking()
        );
    }
    ...
    public virtual Task Json_collection_index_in_projection_using_column(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>().Select(x => x.OwnedCollectionRoot[x.Id]).AsNoTracking()
        );

    private static int MyMethod(int value) => value;

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_index_in_projection_using_untranslatable_client_method(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Select(x => x.OwnedCollectionRoot[MyMethod(x.Id)])
                .AsNoTracking()
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_index_in_projection_using_untranslatable_client_method2(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Select(x =>
                    x.OwnedCollectionRoot[0].OwnedReferenceBranch.OwnedCollectionLeaf[
                        MyMethod(x.Id)
                    ]
                )
                .AsNoTracking()
    ...
    public virtual Task Json_collection_index_outside_bounds2(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x => x.OwnedReferenceRoot.OwnedReferenceBranch.OwnedCollectionLeaf[25])
                    .AsNoTracking(),
    ...
    public virtual Task Json_collection_index_outside_bounds_with_property_access(bool async) =>
        AssertQueryScalar(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .OrderBy(x => x.Id)
                    .Select(x => (int?)x.OwnedCollectionRoot[25].Number),
    ...
    public virtual Task Json_collection_index_in_projection_nested(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Select(x => x.OwnedCollectionRoot[0].OwnedCollectionBranch[prm])
                .AsNoTracking()
        );
    }
    ...
    public virtual Task Json_collection_index_in_projection_nested_project_scalar(bool async)
    {
        ...
        return AssertQueryScalar(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Select(x => x.OwnedCollectionRoot[0].OwnedCollectionBranch[prm].Date)
        );
    }
    ...
    public virtual Task Json_collection_index_in_projection_nested_project_reference(bool async)
    {
        ...
        return AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Select(x => x.OwnedCollectionRoot[0].OwnedCollectionBranch[prm].OwnedReferenceLeaf)
                .AsNoTracking()
        );
    }
        ...
        return AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .OrderBy(x => x.Id)
                    .Select(x =>
                        x.OwnedCollectionRoot[0].OwnedCollectionBranch[prm].OwnedCollectionLeaf
                    )
                    .AsNoTracking(),
            assertOrder: true,
            elementAsserter: (e, a) => AssertCollection(e, a, ordered: true)
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_index_in_projection_nested_project_collection_anonymous_projection(
        bool async
    )
    {
        var prm = 1;
    ...
    public virtual Task Json_collection_index_in_predicate_using_constant(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Where(x => x.OwnedCollectionRoot[0].Name != "Foo")
                .Select(x => x.Id)
        );
    ...
    public virtual Task Json_collection_index_in_predicate_using_variable(bool async)
    {
        ...
        return AssertQueryScalar(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Where(x => x.OwnedCollectionRoot[prm].Name != "Foo")
                .Select(x => x.Id)
        );
    }
    ...
    public virtual Task Json_collection_index_in_predicate_using_column(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Where(x => x.OwnedCollectionRoot[x.Id].Name == "e1_c2")
                    .Select(x => new { x.Id, x }),
            elementSorter: e => e.Id,
            elementAsserter: (e, a) =>
            {
    ...
    public virtual Task Json_collection_index_in_predicate_using_complex_expression1(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Where(x => x.OwnedCollectionRoot[x.Id == 1 ? 0 : 1].Name == "e1_c1")
                    .Select(x => new { x.Id, x }),
            elementSorter: e => e.Id,
            elementAsserter: (e, a) =>
            {
    ...
    public virtual Task Json_collection_index_in_predicate_using_complex_expression2(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Where(x =>
                        x.OwnedCollectionRoot[ss.Set<JsonEntityBasic>().Max(x => x.Id)].Name
                        == "e1_c2"
                    )
                    .Select(x => new { x.Id, x }),
            elementSorter: e => e.Id,
    ...
    public virtual Task Json_collection_ElementAt_in_predicate(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Where(x => x.OwnedCollectionRoot.AsQueryable().ElementAt(1).Name != "Foo")
                .Select(x => x.Id)
        );
    ...
    public virtual Task Json_collection_Any_with_predicate(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Where(j =>
                    j.OwnedReferenceRoot.OwnedCollectionBranch.Any(b =>
                        b.OwnedReferenceLeaf.SomethingSomething == "e1_r_c1_r"
                    )
                )
        );
    ...
    public virtual Task Json_collection_Where_ElementAt(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
                .Where(j =>
                    j.OwnedReferenceRoot.OwnedCollectionBranch.Where(o => o.Enum == JsonEnum.Three)
                        .ElementAt(0)
    ...
    public virtual Task Json_collection_in_projection_with_composition_count(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<JsonEntityBasic>().OrderBy(x => x.Id).Select(x => x.OwnedCollectionRoot.Count)
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_in_projection_with_anonymous_projection_of_scalars(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
                .OrderBy(x => x.Id)
                .Select(x =>
                    x.OwnedCollectionRoot.Select(xx => new { xx.Name, xx.Number }).ToList()
                )
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_in_projection_with_composition_where_and_anonymous_projection_of_scalars(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_in_projection_with_composition_where_and_anonymous_projection_of_primitive_arrays(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
    ...
    public virtual Task Json_nested_collection_filter_in_projection(bool async) =>
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertCollection(e, a, ordered: true, elementAsserter: (ee, aa) =>
                    AssertCollection(ee, aa, ordered: true)
                )
        );
    ...
    public virtual Task Json_nested_collection_anonymous_projection_in_projection(bool async) =>
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertCollection(e, a, ordered: true, elementAsserter: (ee, aa) =>
                    AssertCollection(ee, aa, ordered: true, elementAsserter: (eee, aaa) =>
                    {
                        ...
                        AssertCollection(
                            eee.OwnedCollectionLeaf,
                            aaa.OwnedCollectionLeaf,
                            ordered: true
                        );
                    })
                )
        ...
        AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .OrderBy(x => x.Id)
                    .Select(x =>
                        x.OwnedCollectionRoot.OrderBy(xx => xx.Name).Skip(1).Take(5).ToList()
                    )
                    .AsNoTracking(),
            assertOrder: true,
            elementAsserter: (e, a) => AssertCollection(e, a, ordered: true)
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_skip_take_in_projection_project_into_anonymous_type(
        bool async
    ) =>
        AssertQuery(
            async,
            ...
            elementAsserter: (e, a) =>
            {
                AssertCollection(e, a, ordered: true, elementAsserter: (ee, aa) =>
                {
                    ...
                    AssertCollection(
                        ee.OwnedCollectionBranch,
                        aa.OwnedCollectionBranch,
                        ordered: true
                    );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_skip_take_in_projection_with_json_reference_access_as_final_operation(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    public virtual Task Json_collection_distinct_in_projection(bool async) =>
        AssertQuery(
            ...
            assertOrder: true,
            elementAsserter: (e, a) =>
                AssertCollection(e, a, elementSorter: ee => (ee.Name, ee.Number))
        );
    ...
    public virtual Task Json_multiple_collection_projections(bool async) =>
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
            {
                ...
                AssertCollection(e.Third, a.Third, ordered: true, elementAsserter: (ee, aa) =>
                    AssertCollection(ee, aa, ordered: true)
                );
    ...
    public virtual Task Json_collection_SelectMany(bool async) =>
        AssertQuery(
            async,
            ss => ss.Set<JsonEntityBasic>().SelectMany(x => x.OwnedCollectionRoot).AsNoTracking(),
    ...
    public virtual Task Json_collection_of_primitives_SelectMany(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>().SelectMany(x => x.OwnedReferenceRoot.Names)
        );
    ...
    public virtual Task Json_collection_of_primitives_index_used_in_predicate(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>().Where(x => x.OwnedReferenceRoot.Names[0] == "e1_r1")
        );
    ...
    public virtual Task Json_collection_of_primitives_index_used_in_projection(bool async) =>
        AssertQueryScalar(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .OrderBy(x => x.Id)
                    .Select(x => x.OwnedReferenceRoot.OwnedReferenceBranch.Enums[0]),
            assertOrder: true
        );
    ...
    public virtual Task Json_collection_of_primitives_contains_in_predicate(bool async) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<JsonEntityBasic>().Where(x => x.OwnedReferenceRoot.Names.Contains("e1_r1")),
            assertOrder: true
        );
        ...
        await AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .AsNoTracking()
                    .Select(x =>
                        x.OwnedCollectionRoot.Select(xx => xx.OwnedCollectionBranch).ElementAt(0)
                    ),
            elementAsserter: (e, a) =>
            {
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual async Task Json_collection_Select_entity_in_anonymous_object_ElementAt(
        bool async
    ) =>
        await AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual async Task Json_collection_Select_entity_with_initializer_ElementAt(
        bool async
    ) =>
        await AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_projection_deduplication_with_collection_indexer_in_original(
        bool async
    ) =>
        AssertQuery(
            async,
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x => new
                    {
                        ...
                        Duplicate2 = x.OwnedReferenceRoot.OwnedReferenceBranch.OwnedCollectionLeaf[
                            prm
                        ],
                    })
                    .AsNoTracking(),
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_projection_deduplication_with_collection_in_original_and_collection_indexer_in_target(
        bool async
    )
    {
        var prm = 1;
        ...
        return AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x => new
                    {
                        DuplicateMix = x.OwnedReferenceRoot
                            .OwnedCollectionBranch[0]
                            .OwnedCollectionLeaf[prm],
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_index_in_projection_using_constant_when_owner_is_present(
        bool async
    ) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x => new { x, CollectionElement = x.OwnedCollectionRoot[1] })
                    .AsNoTracking(),
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_index_in_projection_using_constant_when_owner_is_not_present(
        bool async
    ) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x => new { x.Id, CollectionElement = x.OwnedCollectionRoot[1] })
                    .AsNoTracking(),
            elementSorter: e => e.Id,
            elementAsserter: (e, a) =>
            {
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_index_in_projection_using_parameter_when_owner_is_present(
        bool async
    )
    {
        var prm = 1;
        ...
        return AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x => new { x, CollectionElement = x.OwnedCollectionRoot[prm] })
                    .AsNoTracking(),
            elementSorter: e => e.x.Id,
            elementAsserter: (e, a) =>
            {
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_index_in_projection_using_parameter_when_owner_is_not_present(
        bool async
    )
    {
        var prm = 1;
        ...
        return AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x => new { x.Id, CollectionElement = x.OwnedCollectionRoot[prm] })
                    .AsNoTracking(),
            elementSorter: e => e.Id,
            elementAsserter: (e, a) =>
            {
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_after_collection_index_in_projection_using_constant_when_owner_is_present(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_after_collection_index_in_projection_using_constant_when_owner_is_not_present(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_after_collection_index_in_projection_using_parameter_when_owner_is_present(
        bool async
    )
    {
        var prm = 1;
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_after_collection_index_in_projection_using_parameter_when_owner_is_not_present(
        bool async
    )
    {
        var prm = 1;
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_index_in_projection_when_owner_is_not_present_misc1(
        bool async
    )
    {
        var prm = 1;
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_index_in_projection_when_owner_is_present_misc2(
        bool async
    ) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<JsonEntityBasic>()
                    ...
                    {
                        x,
                        CollectionElement = x.OwnedReferenceRoot
                            .OwnedReferenceBranch
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_index_in_projection_when_owner_is_not_present_misc2(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_index_in_projection_when_owner_is_present_multiple(
        bool async
    )
    {
        var prm = 1;
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x => new
                    {
                        ...
                        CollectionElement9 = x.OwnedCollectionRoot[x.Id].OwnedCollectionBranch[
                            x.Id
                        ],
                    })
                    .AsNoTracking(),
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_collection_index_in_projection_when_owner_is_not_present_multiple(
        bool async
    )
    {
        var prm = 1;
    ...
    public virtual Task Group_by_on_json_scalar_using_collection_indexer(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
                .GroupBy(x => x.OwnedCollectionRoot[0].Name)
                .Select(x => new { x.Key, Count = x.Count() })
        );
    ...
    public virtual Task Group_by_Skip_Take_on_json_scalar(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityBasic>()
                .GroupBy(x => x.OwnedReferenceRoot.Name)
                .Select(g => g.OrderBy(x => x.Id).Skip(1).Take(5))
        );
    ...
    public virtual Task Group_by_json_scalar_Skip_First_project_json_scalar(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<JsonEntityBasic>()
                .GroupBy(x => x.OwnedReferenceRoot.Name)
                .Select(g => g.First().OwnedReferenceRoot.OwnedReferenceBranch.Enum)
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_with_include_on_json_entity(bool async) =>
        AssertQuery(async, ss => ss.Set<JsonEntityBasic>().Include(x => x.OwnedReferenceRoot));
    ...
    public virtual Task Json_with_include_on_entity_reference(bool async) =>
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<JsonEntityBasic>(x => x.EntityReference))
        );
    ...
    public virtual Task Json_with_include_on_entity_collection(bool async) =>
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<JsonEntityBasic>(x => x.EntityCollection))
        );
    ...
    public virtual Task Entity_including_collection_with_json(bool async) =>
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertInclude(e, a, new ExpectedInclude<EntityBasic>(x => x.JsonEntityBasics))
        );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_with_projection_of_json_reference_leaf_and_entity_collection(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    public virtual Task Json_with_projection_of_json_reference_and_entity_collection(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x => new { x.OwnedReferenceRoot, x.EntityCollection })
                    .AsNoTracking(),
            elementAsserter: (e, a) =>
            {
                AssertEqual(e.OwnedReferenceRoot, a.OwnedReferenceRoot);
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_with_projection_of_multiple_json_references_and_entity_collection(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_with_projection_of_json_collection_leaf_and_entity_collection(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    public virtual Task Json_with_projection_of_json_collection_and_entity_collection(bool async) =>
        AssertQuery(
            ...
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x => new { x.OwnedCollectionRoot, x.EntityCollection })
                    .AsNoTracking(),
            elementAsserter: (e, a) =>
            {
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_with_projection_of_json_collection_element_and_entity_collection(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_with_projection_of_mix_of_json_collections_json_references_and_entity_collection(
        bool async
    ) =>
        AssertQuery(
            async,
            ss =>
                ss.Set<JsonEntityBasic>()
                    .Select(x => new
                    {
                        ...
                        x.EntityCollection,
                        Reference2 = x.OwnedReferenceRoot.OwnedReferenceBranch.OwnedCollectionLeaf[
                            0
                        ],
    ...
    public virtual Task Json_all_types_entity_projection(bool async) =>
        AssertQuery(async, ss => ss.Set<JsonEntityAllTypes>());
    ...
    public virtual Task Json_boolean_predicate(bool async) =>
        AssertQuery(async, ss => ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestBoolean));
    ...
    public virtual Task Json_boolean_projection(bool async) =>
        AssertQueryScalar(async, ss =>
            ss.Set<JsonEntityAllTypes>().Select(x => x.Reference.TestBoolean)
        );
    ...
    public virtual Task Json_predicate_on_default_string(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>()
                .Where(x => x.Reference.TestDefaultString != "MyDefaultStringInReference1")
        );
    ...
    public virtual Task Json_predicate_on_max_length_string(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestMaxLengthString != "Foo")
        );
    ...
    public virtual Task Json_predicate_on_byte(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestByte != 3)
        );
    ...
    public virtual Task Json_predicate_on_character(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestCharacter != 'z')
        );
    ...
    public virtual Task Json_predicate_on_datetime(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>()
                .Where(x => x.Reference.TestDateTime != new DateTime(2000, 1, 3))
        );
    ...
    public virtual Task Json_predicate_on_datetimeoffset(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>()
                .Where(x =>
                    x.Reference.TestDateTimeOffset
                    != new DateTimeOffset(new DateTime(2000, 1, 4), new TimeSpan(3, 2, 0))
                )
        );
    ...
    public virtual Task Json_predicate_on_decimal(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestDecimal != 1.35M)
        );
    ...
    public virtual Task Json_predicate_on_double(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestDouble != 33.25)
        );
    ...
    public virtual Task Json_predicate_on_guid(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestGuid != new Guid())
        );
    ...
    public virtual Task Json_predicate_on_int16(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestInt16 != 3)
        );
    ...
    public virtual Task Json_predicate_on_int32(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestInt32 != 33)
        );
    ...
    public virtual Task Json_predicate_on_int64(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestInt64 != 333)
        );
    ...
    public virtual Task Json_predicate_on_signedbyte(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestSignedByte != 100)
        );
    ...
    public virtual Task Json_predicate_on_single(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestSingle != 10.4f)
        );
    ...
    public virtual Task Json_predicate_on_timespan(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>()
                .Where(x => x.Reference.TestTimeSpan != new TimeSpan(3, 2, 0))
        );
    ...
    public virtual Task Json_predicate_on_dateonly(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>()
                .Where(x => x.Reference.TestDateOnly != new DateOnly(3, 2, 1))
        );
    ...
    public virtual Task Json_predicate_on_timeonly(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>()
                .Where(x => x.Reference.TestTimeOnly != new TimeOnly(3, 2, 0))
        );
    ...
    public virtual Task Json_predicate_on_unisgnedint16(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestUnsignedInt16 != 100)
        );
    ...
    public virtual Task Json_predicate_on_unsignedint32(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestUnsignedInt32 != 1000)
        );
    ...
    public virtual Task Json_predicate_on_unsignedint64(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestUnsignedInt64 != 10000)
        );
    ...
    public virtual Task Json_predicate_on_enum(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestEnum != JsonEnum.Two)
        );
    ...
    public virtual Task Json_predicate_on_enumwithintconverter(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>()
                .Where(x => x.Reference.TestEnumWithIntConverter != JsonEnum.Three)
        );
    ...
    public virtual Task Json_predicate_on_nullableenum1(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestNullableEnum != JsonEnum.One)
        );
    ...
    public virtual Task Json_predicate_on_nullableenum2(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestNullableEnum != null)
        );
    ...
    public virtual Task Json_predicate_on_nullableenumwithconverterthathandlesnulls1(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>()
                .Where(x =>
                    x.Reference.TestNullableEnumWithConverterThatHandlesNulls != JsonEnum.One
                )
        );
    ...
    public virtual Task Json_predicate_on_nullableenumwithconverterthathandlesnulls2(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>()
                .Where(x => x.Reference.TestNullableEnumWithConverterThatHandlesNulls != null)
        );
    ...
    public virtual Task Json_predicate_on_nullableenumwithconverter1(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>()
                .Where(x => x.Reference.TestNullableEnumWithIntConverter != JsonEnum.Two)
        );
    ...
    public virtual Task Json_predicate_on_nullableenumwithconverter2(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>()
                .Where(x => x.Reference.TestNullableEnumWithIntConverter != null)
        );
    ...
    public virtual Task Json_predicate_on_nullableint321(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestNullableInt32 != 100)
        );
    ...
    public virtual Task Json_predicate_on_nullableint322(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityAllTypes>().Where(x => x.Reference.TestNullableInt32 != null)
        );
    ...
    public virtual Task Json_predicate_on_bool_converted_to_int_zero_one(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityConverters>().Where(x => x.Reference.BoolConvertedToIntZeroOne)
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_predicate_on_bool_converted_to_int_zero_one_with_explicit_comparison(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityConverters>()
                .Where(x => x.Reference.BoolConvertedToIntZeroOne == false)
        );
    ...
    public virtual Task Json_predicate_on_bool_converted_to_string_True_False(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityConverters>().Where(x => x.Reference.BoolConvertedToStringTrueFalse)
        );

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_predicate_on_bool_converted_to_string_True_False_with_explicit_comparison(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityConverters>()
                .Where(x => x.Reference.BoolConvertedToStringTrueFalse == true)
        );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Json_predicate_on_bool_converted_to_string_Y_N_with_explicit_comparison(
        bool async
    ) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityConverters>().Where(x => x.Reference.BoolConvertedToStringYN == false)
        );
    ...
    public virtual Task Json_predicate_on_int_zero_one_converted_to_bool(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityConverters>().Where(x => x.Reference.IntZeroOneConvertedToBool == 1)
        );
    ...
    public virtual Task Json_predicate_on_string_True_False_converted_to_bool(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityConverters>()
                .Where(x => x.Reference.StringTrueFalseConvertedToBool == "False")
        );
    ...
    public virtual Task Json_predicate_on_string_Y_N_converted_to_bool(bool async) =>
        AssertQuery(async, ss =>
            ss.Set<JsonEntityConverters>().Where(x => x.Reference.StringYNConvertedToBool == "N")
        );
    ...
    public virtual Task FromSql_on_entity_with_json_project_json_collection(bool async) =>
        AssertQuery(
            ...
            elementAsserter: (e, a) =>
                AssertCollection(e, a, elementSorter: ee => (ee.Date, ee.Enum, ee.Fraction))
        );
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task FromSql_on_entity_with_json_inheritance_project_reference_on_base(
        bool async
    ) =>
        AssertQuery(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task FromSql_on_entity_with_json_inheritance_project_reference_on_derived(
        bool async
    ) =>
        AssertQuery(
            async,
            ...
            ss =>
                ss.Set<JsonEntityInheritanceDerived>()
                    .OrderBy(x => x.Id)
                    .Select(x => x.CollectionOnDerived),
            elementAsserter: (e, a) =>
                AssertCollection(e, a, elementSorter: ee => (ee.Date, ee.Enum, ee.Fraction)),
