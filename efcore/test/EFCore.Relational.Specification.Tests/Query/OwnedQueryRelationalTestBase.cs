    ...
    where TFixture : OwnedQueryRelationalTestBase<TFixture>.RelationalOwnedQueryFixture, new()
{
    ...
    public virtual Task Query_for_base_type_loads_all_owned_navs_split(bool async) =>
        AssertQuery(async, ss => ss.Set<OwnedPerson>().AsSplitQuery());

    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Query_for_branch_type_loads_all_owned_navs_split(bool async) =>
        AssertQuery(async, ss => ss.Set<Branch>().AsSplitQuery());
    ...
    public virtual Task Query_with_OfType_eagerly_loads_correct_owned_navigations_split(
        bool async
    ) => AssertQuery(async, ss => ss.Set<OwnedPerson>().OfType<LeafA>().AsSplitQuery());
    ...
    public abstract class RelationalOwnedQueryFixture : OwnedQueryFixtureBase
    {
        ...
        //modelBuilder.Entity<OwnedPerson>().OwnsOne(e => e.PersonAddress).Property(e =>
        // e.PlaceType).HasColumnName("PlaceType");
        //modelBuilder.Entity<Branch>().OwnsOne(e => e.BranchAddress).Property(e =>
        // e.PlaceType).HasColumnName("PlaceType");
        //modelBuilder.Entity<LeafA>().OwnsOne(e => e.LeafAAddress).Property(e =>
        // e.PlaceType).HasColumnName("PlaceType");
        //modelBuilder.Entity<LeafB>().OwnsOne(e => e.LeafBAddress).Property(e =>
        // e.PlaceType).HasColumnName("PlaceType");
    }
}
