...
{
    protected override string StoreName => "NonSharedModelUpdatesTestBase";
    ...
    public virtual async Task Principal_and_dependent_roundtrips_with_cycle_breaking(bool async)
    {
        var contextFactory = await InitializeAsync<DbContext>(onModelCreating: mb =>
        {
            mb.Entity<Author>(b =>
            {
                b.HasOne(a => a.AuthorsClub).WithMany().HasForeignKey(a => a.AuthorsClubId);
            });

            mb.Entity<Book>(b =>
            {
                b.HasOne(book => book.Author).WithMany().HasForeignKey(book => book.AuthorId);
            });
        });
        ...
        await ExecuteWithStrategyInTransactionAsync(
            ...
            async context =>
            {
                ...
                var book = await context.Set<Book>().Include(b => b.Author).SingleAsync();
