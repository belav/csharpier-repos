...
public abstract class UpdateSqlGeneratorTestBase
{
    ...
    public virtual void AppendDeleteOperation_creates_full_delete_command_text_with_concurrency_check()
    {
        ...
        AppendDeleteOperation_creates_full_delete_command_text_with_concurrency_check_verification(
            stringBuilder
        );
    }
    ...
    protected abstract void AppendInsertOperation_insert_if_store_generated_columns_exist_verification(
        StringBuilder stringBuilder
    );
    ...
    public virtual void AppendInsertOperation_for_store_generated_columns_but_no_identity()
    {
        ...
        AppendInsertOperation_for_store_generated_columns_but_no_identity_verification(
            stringBuilder
        );
    }

    protected abstract void AppendInsertOperation_for_store_generated_columns_but_no_identity_verification(
        StringBuilder stringBuilder
    );
    ...
    protected abstract void AppendInsertOperation_for_only_identity_verification(
        StringBuilder stringBuilder
    );
    ...
    protected abstract void AppendInsertOperation_for_all_store_generated_columns_verification(
        StringBuilder stringBuilder
    );
    ...
    protected abstract void AppendInsertOperation_for_only_single_identity_columns_verification(
        StringBuilder stringBuilder
    );
    ...
    protected abstract void AppendUpdateOperation_if_store_generated_columns_exist_verification(
        StringBuilder stringBuilder
    );
    ...
    protected abstract void AppendUpdateOperation_if_store_generated_columns_dont_exist_verification(
        StringBuilder stringBuilder
    );
    ...
    protected abstract void AppendUpdateOperation_appends_where_for_concurrency_token_verification(
        StringBuilder stringBuilder
    );
    ...
    public virtual void GenerateNextSequenceValueOperation_returns_statement_with_sanitized_sequence()
    {
        var statement = CreateSqlGenerator()
            .GenerateNextSequenceValueOperation("sequence" + CloseDelimiter + "; --", null);
    ...
    protected IModificationCommand CreateInsertCommand(
    ...
    )
    {
        ...
        var columnModifications = new[]
        {
            new ColumnModificationParameters(
                entry,
                idProperty,
                idProperty.GetTableColumnMappings().Single().Column,
                generator.GenerateNext,
                idProperty.GetTableColumnMappings().Single().TypeMapping,
                identityKey,
                !identityKey,
                true,
                false,
            ...
            new ColumnModificationParameters(
                entry,
                nameProperty,
                nameProperty.GetTableColumnMappings().Single().Column,
                generator.GenerateNext,
                nameProperty.GetTableColumnMappings().Single().TypeMapping,
                false,
                true,
                false,
                false,
            ...
            new ColumnModificationParameters(
                entry,
                quacksProperty,
                quacksProperty.GetTableColumnMappings().Single().Column,
                generator.GenerateNext,
                quacksProperty.GetTableColumnMappings().Single().TypeMapping,
                false,
                true,
                false,
                false,
            ...
            new ColumnModificationParameters(
                entry,
                computedProperty,
                computedProperty.GetTableColumnMappings().Single().Column,
                generator.GenerateNext,
                computedProperty.GetTableColumnMappings().Single().TypeMapping,
                isComputed,
                false,
                false,
                false,
            ...
            new ColumnModificationParameters(
                entry,
                concurrencyProperty,
                concurrencyProperty.GetTableColumnMappings().Single().Column,
                generator.GenerateNext,
                concurrencyProperty.GetTableColumnMappings().Single().TypeMapping,
                false,
                true,
                false,
                false,
    ...
    protected IModificationCommand CreateUpdateCommand(
        bool isComputed = true,
    ...
    {
        var stateManager = TestHelpers
            .CreateContextServices(GetDuckModel())
            .GetRequiredService<IStateManager>();
        var entry = stateManager.GetOrCreateEntry(new Duck());
    ...
    )
    {
        ...
        var modificationCommand = CreateMutableModificationCommandFactory()
        ...
        {
            ((INonTrackedModificationCommand)modificationCommand).AddColumnModification(
                columnModification
            );
        }
