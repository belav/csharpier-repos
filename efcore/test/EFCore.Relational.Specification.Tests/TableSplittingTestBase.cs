...
public abstract class TableSplittingTestBase : NonSharedModelTestBase
{
    ...
    public virtual async Task Can_share_required_columns_with_complex_types()
    {
        await InitializeAsync(
            modelBuilder =>
            {
                ...
                modelBuilder.Entity<PoweredVehicle>(vb =>
                {
                    vb.ComplexProperty(
                        v => v.Engine,
                        eb =>
            ...
            modelBuilder =>
            {
                ...
                modelBuilder.Entity<PoweredVehicle>(vb =>
                {
                    vb.ComplexProperty(
                        ...
                        eb =>
                        {
                            eb.Property<int>("SeatingCapacity")
                                .HasColumnName("SeatingCapacity")
                                .IsConcurrencyToken();
                        }
                    );
    ...
    {
        var contextFactory = await InitializeAsync<Context29196>(onConfiguring: e =>
            e.ConfigureWarnings(w =>
                w.Log(RelationalEventId.OptionalDependentWithoutIdentifyingPropertyWarning)
            )
        );
    ...
    protected async Task InitializeSharedAsync(
        ...
        SharedContextFactory = await InitializeAsync<SharedTableContext>(
            ...
            onConfiguring: options =>
            {
                options
                    .ConfigureWarnings(w =>
                        w.Log(RelationalEventId.OptionalDependentWithAllNullPropertiesWarning)
                    )
                    .ConfigureWarnings(w =>
                        w.Log(RelationalEventId.OptionalDependentWithoutIdentifyingPropertyWarning)
                    )
