    ...
    where TFixture : NorthwindBulkUpdatesFixture<NoopModelCustomizer>, new()
{
    ...
    public virtual Task Delete_SelectMany_subquery(bool async) =>
        AssertDelete(
            ...
            ss =>
                ss.Set<Order>()
                    .Where(e => e.OrderID < 10250)
                    .SelectMany(e => e.OrderDetails.Where(i => i.ProductID > 0)),
            rowsAffectedCount: 5
        );
                ...
                AssertDelete(
                    ...
                    ss =>
                        ss.Set<OrderDetail>()
                            ...
                            .Select(
                                e =>
                                    new OrderDetail { OrderID = e.OrderID, ProductID = e.ProductID }
                            ),
                    rowsAffectedCount: 0
                )
        );
    ...
    public virtual async Task Delete_FromSql_converted_to_subquery(bool async)
    {
        if (async)
        {
            await TestHelpers.ExecuteWithStrategyInTransactionAsync(
                ...
                (DatabaseFacade facade, IDbContextTransaction transaction) =>
                    Fixture.UseTransaction(facade, transaction),
                async context =>
                    await context
                        .Set<OrderDetail>()
                        .FromSqlRaw(
...
WHERE [OrderID] < 10300"
    ...
    public virtual Task Delete_with_left_join(bool async) =>
        AssertDelete(
            async,
            ss =>
                from od in ss.Set<OrderDetail>().Where(e => e.OrderID < 10276)
                join o in ss.Set<Order>()
                    .Where(o => o.OrderID < 10300)
                    .OrderBy(e => e.OrderID)
                    .Skip(0)
    ...
    public virtual Task Update_Where_set_property_plus_constant(bool async) =>
        AssertUpdate(
            ...
            (b, a) =>
                b.Zip(a)
                    .ForEach(e => Assert.Equal(e.First.ContactName + "Abc", e.Second.ContactName))
        );
            ...
            (b, a) =>
                b.Zip(a)
                    .ForEach(
                        e =>
                            Assert.Equal(
                                e.First.ContactName + e.First.CustomerID,
                                e.Second.ContactName
                            )
                    )
        );
        ...
        AssertTranslationFailed(
            RelationalStrings.InvalidPropertyInSetProperty(
                new ExpressionPrinter().PrintExpression(
                    (OrderDetail e) => e.MaybeScalar(e => e.OrderID)
                )
            ),
            () =>
                AssertUpdate(
        ...
        AssertTranslationFailed(
            ...
            () =>
                AssertUpdate(
                    ...
                    e => e.Customer,
                    s =>
                        s.SetProperty(c => c.Customer.ContactName, "Name")
...
WHERE [CustomerID] LIKE 'A%'"
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Update_Where_Join_set_property_from_joined_single_result_table(
        bool async
    ) =>
        AssertUpdate(
            async,
    ...
    [ConditionalTheory]
    [MemberData(nameof(IsAsyncData))]
    public virtual Task Update_Where_Join_set_property_from_joined_single_result_scalar(
        bool async
    ) =>
        AssertUpdate(
            async,
        ...
        AssertUpdate(
            ...
            ss =>
                ss.Set<OrderDetail>()
                    .Where(
                        od =>
                            od.Product.Discontinued && od.Order.OrderDate > new DateTime(1990, 1, 1)
                    ),
            e => e,
