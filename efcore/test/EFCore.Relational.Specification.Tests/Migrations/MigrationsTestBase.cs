        ...
        where TFixture : MigrationsTestBase<TFixture>.MigrationsFixtureBase, new()
    {
        ...
        public virtual Task Add_column_with_defaultValueSql() =>
            Test(
                ...
                builder => { },
                builder =>
                    builder.Entity("People").Property<int>("Sum").HasDefaultValueSql("1 + 2"),
                model =>
                {
                    var table = Assert.Single(model.Tables);
                    Assert.Equal(2, table.Columns.Count);
        ...
        public virtual async Task Add_column_with_computedSql_unspecified()
        {
            ...
            Assert.Equal(
                RelationalStrings.ComputedColumnSqlUnspecified("Sum", "People"),
                ex.Message
            );
        }
        ...
        public virtual Task Add_column_with_required() =>
            Test(
                ...
                model =>
                {
                    ...
                    var column = Assert.Single(table.Columns, c => c.Name == "Name");
                    Assert.Equal(
                        TypeMappingSource.FindMapping(typeof(string)).StoreType,
                        column.StoreType
                    );
        ...
        public virtual Task Alter_column_change_type() =>
            Test(
                ...
                model =>
                {
                    ...
                    Assert.Equal(
                        _typeMappingSource.FindMapping(typeof(long)).StoreType,
                        column.StoreType
                    );
                }
            );
                ...
                builder =>
                    builder.Entity(
                        ...
                        e =>
                        {
                            e.Property<int>("X");
                            e.Property<int>("Y")
                                .HasComputedColumnSql($"{DelimitIdentifier("X")} + 1");
                        }
                    ),
                builder => { },
                model =>
        ...
        public virtual Task Rename_column() =>
            Test(
                ...
                builder =>
                    builder
                        .Entity("People")
                        .Property<string>("SomeColumn")
                        .HasColumnName("SomeOtherColumn"),
                model =>
                {
                    var table = Assert.Single(model.Tables);
                    Assert.Equal(2, table.Columns.Count);
            ...
            Test(
                ...
                builder =>
                    builder
                        ...
                        .IsUnique()
                        .HasFilter(
                            $"{DelimitIdentifier("Name")} IS NOT NULL AND {DelimitIdentifier("Name")} <> ''"
                        ),
                model =>
                {
        ...
        public virtual Task Add_unique_constraint() =>
            Test(
                ...
                model =>
                {
                    ...
                    Assert.Same(
                        table.Columns.Single(c => c.Name == "AlternateKeyColumn"),
                        Assert.Single(uniqueConstraint.Columns)
                    );
                    if (AssertConstraintNames)
        ...
        public virtual Task Add_unique_constraint_composite_with_name() =>
            Test(
                ...
                builder =>
                    builder
                        .Entity("People")
                        .HasAlternateKey("AlternateKeyColumn1", "AlternateKeyColumn2")
                        .HasName("AK_Foo"),
                model =>
                {
                    var table = Assert.Single(model.Tables);
                    ...
                    Assert.Collection(
                        ...
                        c =>
                            Assert.Same(
                                table.Columns.Single(c => c.Name == "AlternateKeyColumn1"),
                                c
                            ),
                        c =>
                            Assert.Same(
                                table.Columns.Single(c => c.Name == "AlternateKeyColumn2"),
                                c
                            )
                    );
                    if (AssertConstraintNames)
        ...
        public virtual Task Add_check_constraint_with_name() =>
            Test(
                ...
                builder =>
                    builder
                        .Entity("People")
                        .HasCheckConstraint("Foo", $"{DelimitIdentifier("DriverLicense")} > 0"),
                model =>
                {
                    // TODO: no scaffolding support for check constraints, https://github.com/aspnet/EntityFrameworkCore/issues/15408
                }
        ...
        public virtual Task Alter_check_constraint() =>
            Test(
                ...
                builder =>
                    builder
                        .Entity("People")
                        .HasCheckConstraint("Foo", $"{DelimitIdentifier("DriverLicense")} > 1"),
                model =>
                {
                    // TODO: no scaffolding support for check constraints, https://github.com/aspnet/EntityFrameworkCore/issues/15408
                }
        ...
        )
        {
            ...
            var sourceModel = modelRuntimeInitializer.Initialize(
                (IModel)sourceModelBuilder.Model,
                designTime: true,
            ...
            var targetModel = modelRuntimeInitializer.Initialize(
                (IModel)targetModelBuilder.Model,
                designTime: true,
                validationLogger: null
            );
        ...
        )
        {
            ...
            var migrationsSqlGenerator =
                serviceProvider.GetRequiredService<IMigrationsSqlGenerator>();
            var modelDiffer = serviceProvider.GetRequiredService<IMigrationsModelDiffer>();
            var migrationsCommandExecutor =
                serviceProvider.GetRequiredService<IMigrationCommandExecutor>();
            ...
            try
            {
                ...
                await migrationsCommandExecutor.ExecuteNonQueryAsync(
                    migrationsSqlGenerator.Generate(operations, targetModel),
                    connection
                );
        ...
        protected virtual Task<T> TestThrows<T>(
            Action<ModelBuilder> buildSourceAction,
            Action<ModelBuilder> buildTargetAction
        ) where T : Exception => TestThrows<T>(b => { }, buildSourceAction, buildTargetAction);
