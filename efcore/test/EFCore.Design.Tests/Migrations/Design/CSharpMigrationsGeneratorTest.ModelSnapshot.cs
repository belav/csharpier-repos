...
"""
            ...
            o =>
            {
                ...
                Assert.Collection(
                    ...
                    t =>
                        Assert.Equal(
                            "Microsoft.EntityFrameworkCore.Migrations.Design.CSharpMigrationsGeneratorTest+EntityWithOneProperty",
                            t.Name
                        ),
                    t =>
                        Assert.Equal(
                            "Microsoft.EntityFrameworkCore.Migrations.Design.CSharpMigrationsGeneratorTest+EntityWithTwoProperties",
                            t.Name
                        )
                );
            }
...
""",
            model =>
            {
                ...
                var animalType = model.FindEntityType(
                    "Microsoft.EntityFrameworkCore.Migrations.Design.CSharpMigrationsGeneratorTest+Animal"
                );
                Assert.Null(animalType.GetTableName());
                Assert.Null(animalType.GetViewName());
                Assert.Equal("TPC", animalType.GetMappingStrategy());

                var petType = model.FindEntityType(
                    "Microsoft.EntityFrameworkCore.Migrations.Design.CSharpMigrationsGeneratorTest+Pet"
                );
                Assert.Null(petType.GetTableName());
                Assert.Null(petType.GetViewName());

                var catType = model.FindEntityType(
                    "Microsoft.EntityFrameworkCore.Migrations.Design.CSharpMigrationsGeneratorTest+Cat"
                );
                Assert.Equal("Cat", catType.GetTableName());
                Assert.Null(catType.GetViewName());

                var dogType = model.FindEntityType(
                    "Microsoft.EntityFrameworkCore.Migrations.Design.CSharpMigrationsGeneratorTest+Dog"
                );
                Assert.Equal("Dog", dogType.GetTableName());
                Assert.Null(dogType.GetViewName());

                var humanType = model.FindEntityType(
                    "Microsoft.EntityFrameworkCore.Migrations.Design.CSharpMigrationsGeneratorTest+Human"
                );
...
"""
            ...
            o =>
            {
                ...
                Assert.Collection(
                    ...
                    t =>
                        Assert.Equal(
                            "Microsoft.EntityFrameworkCore.Migrations.Design.CSharpMigrationsGeneratorTest+AnotherDerivedEntity",
                            t.Name
                        ),
                    t =>
                        Assert.Equal(
...
"""
            ...
            o =>
            {
                ...
                var orderInfo = (IRuntimeEntityType)
                    order.FindNavigation(nameof(Order.OrderInfo)).TargetEntityType;
                Assert.Equal(1, orderInfo.PropertyCount);

                var orderInfoAddress = (IRuntimeEntityType)
...
""",
            model =>
            {
                ...
                var testOwnee = model.FindEntityType(
                    "Microsoft.EntityFrameworkCore.Migrations.Design.CSharpMigrationsGeneratorTest+TestOwnee",
                    "OwnedEntities",
                    testOwner
                );
...
"""
            ...
            o =>
            {
                ...
                Assert.Collection(
                    ...
                    {
                        Assert.Equal(
                            "Microsoft.EntityFrameworkCore.Migrations.Design.CSharpMigrationsGeneratorTest+DuplicateDerivedEntity",
                            t.Name
                        );
                        Assert.Equal(
                            ...
                            t.FindProperty(nameof(DuplicateDerivedEntity.Name))
                                .GetColumnName(
                                    StoreObjectIdentifier.Table(nameof(BaseEntity), "DefaultSchema")
                                )
                        );
                    }
                );
...
""",
            ...
            o =>
            {
                ...
                var complexProperty = entityWithOneProperty.FindComplexProperty(
                ...
                Assert.Equal(
                    "EntityWithOneProperty.EntityWithTwoProperties#EntityWithTwoProperties",
                    complexType.DisplayName()
                );
                Assert.Equal(nameof(EntityWithOneProperty), complexType.GetTableName());
                var alternateIdProperty = complexType.FindProperty(
                    nameof(EntityWithTwoProperties.AlternateId)
                );
                ...
                var nestedComplexProperty = complexType.FindComplexProperty(
                    nameof(EntityWithTwoProperties.EntityWithStringKey)
                );
                ...
                var nestedIdProperty = nestedComplexType.FindProperty(
                    nameof(EntityWithStringKey.Id)
                );
            ...
            (snapshotModel, originalModel) =>
            {
                ...
                for (var i = 0; i < originalProperties.Count; i++)
                {
                    ...
                    Assert.Equal(
                        ...
                        snapshotProperty.DeclaringType.Name
                    );
                ...
                Assert.Collection(
                    ...
                    seed =>
                    {
                        ...
                        Assert.Equal(
                            "[\"2023-09-07T00:00:00\",\"2023-11-14T00:00:00\"]",
                            seed["DateTimeCollection"]
                        );
...
""";
    ...
    protected IModel BuildModelFromSnapshotSource(string code)
    {
        ...
        buildModelMethod.Invoke(Activator.CreateInstance(snapshotType), new object[] { builder });
