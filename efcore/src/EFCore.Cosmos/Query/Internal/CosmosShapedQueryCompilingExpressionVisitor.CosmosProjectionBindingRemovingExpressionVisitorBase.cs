                ...
                if (binaryExpression.Left is ParameterExpression parameterExpression)
                {
                    ...
                    )
                    {
                        ...
                        )
                        {
                            ...
                            projectionExpression is UnaryExpression
                            {
...
#pragma warning restore EF1001 // Internal EF Core API usage.
        ...
        private static Delegate GenerateInitialize(Type entityType, INavigation navigation)
        {
            ...
            return Lambda(Block(typeof(void), getOrCreateExpression), entityParameter).Compile();
        }
        ...
        )
        {
            ...
            if (storeName.Length == 0)
            {
                ...
                if (entityType == null || !entityType.IsDocumentRoot())
                {
                    var ownership = entityType?.FindOwnership();
                    if (ownership != null && !ownership.IsUnique && property.IsOrdinalKeyProperty())
                    {
                    ...
                    if (principalProperty != null)
                    {
                        ...
                        {
                            Check.DebugAssert(
                                principalProperty.DeclaringType.IsAssignableFrom(
        ...
        private static Expression ConvertJTokenToType(Expression jTokenExpression, Type type) =>
            type == typeof(JToken)
                ? jTokenExpression
                : Call(ToObjectWithSerializerMethodInfo.MakeGenericMethod(type), jTokenExpression);
