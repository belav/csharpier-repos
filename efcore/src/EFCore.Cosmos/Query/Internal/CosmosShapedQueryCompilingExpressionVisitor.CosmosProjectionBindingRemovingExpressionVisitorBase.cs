...
public partial class CosmosShapedQueryCompilingExpressionVisitor
{
    private abstract class CosmosProjectionBindingRemovingExpressionVisitorBase : ExpressionVisitor
    {
        ...
        private static readonly PropertyInfo JTokenTypePropertyInfo = typeof(JToken)
            .GetRuntimeProperties()
        ...
        private readonly IDictionary<
            ParameterExpression,
            Expression
        > _materializationContextBindings = new Dictionary<ParameterExpression, Expression>();
        ...
        private readonly IDictionary<
            Expression,
            (IEntityType EntityType, Expression JObjectExpression)
        > _ownerMappings = new Dictionary<Expression, (IEntityType, Expression)>();
        ...
        private static readonly MethodInfo ToObjectWithSerializerMethodInfo =
            typeof(CosmosProjectionBindingRemovingExpressionVisitorBase)
                .GetRuntimeMethods()
                .Single(mi => mi.Name == nameof(SafeToObjectWithSerializer));
        ...
        protected override Expression VisitBinary(BinaryExpression binaryExpression)
        {
            if (binaryExpression.NodeType == ExpressionType.Assign)
            {
                ...
                {
                    if (
                        parameterExpression.Type == typeof(JObject)
                    ...
                    )
                    {
                        ...
                        )
                        {
                            ...
                            projectionExpression is UnaryExpression convertExpression
                        ...
                        if (
                            projectionExpression
                            is ObjectArrayProjectionExpression objectArrayProjectionExpression
                        )
                        {
                            innerAccessExpression =
                                objectArrayProjectionExpression.AccessExpression;
                            _projectionBindings[objectArrayProjectionExpression] =
                                parameterExpression;
                    ...
                    if (parameterExpression.Type == typeof(MaterializationContext))
                    {
                        ...
                        _materializationContextBindings[parameterExpression] =
                            entityProjectionExpression.AccessExpression;
        ...
        protected override Expression VisitMethodCall(MethodCallExpression methodCallExpression)
        {
            ...
            )
            {
                ...
                {
                    if (
                        !(includeExpression.Navigation is INavigation navigation)
        ...
        protected override Expression VisitExtension(Expression extensionExpression)
        {
            switch (extensionExpression)
            {
                ...
                case CollectionShaperExpression collectionShaperExpression:
                {
                    ...
                    var jObjectParameter = Expression.Parameter(
                        typeof(JObject),
                        jArray.Name + "Object"
                    );
                    var ordinalParameter = Expression.Parameter(
                        typeof(int),
                        jArray.Name + "Ordinal"
                    );
                ...
                case IncludeExpression includeExpression:
                {
                    ...
                    Check.DebugAssert(
                        jObjectBlock != null,
                        "The first include must end up on a valid shaper block"
                    );
                    ...
                    jObjectExpressions.Add(
                        jObjectCondition.Update(
                            jObjectCondition.Test,
                            jObjectCondition.IfTrue,
                            shaperBlock
                        )
                    );
            ...
            shaperExpressions.Add(
                Expression.IfThen(
                    ...
                    Expression.Call(
                        ...
                        Expression.Constant(
                            initialize,
                            typeof(Action<>).MakeGenericType(includingClrType)
                        ),
#pragma warning disable EF1001 // Internal EF Core API usage.
...
#pragma warning restore EF1001 // Internal EF Core API usage.
        ...
        {
            if (entity == null || !navigation.DeclaringEntityType.IsAssignableFrom(entityType))
            {
                return;
            }
...
#pragma warning restore EF1001 // Internal EF Core API usage.
        ...
        private static Delegate GenerateInitialize(Type entityType, INavigation navigation)
        {
            ...
            return Expression
                .Lambda(Expression.Block(typeof(void), getOrCreateExpression), entityParameter)
                .Compile();
        }
        ...
        private static readonly MethodInfo IsAssignableFromMethodInfo =
            typeof(IReadOnlyEntityType).GetMethod(
                nameof(IReadOnlyEntityType.IsAssignableFrom),
        ...
        protected abstract ProjectionExpression GetProjection(
            ProjectionBindingExpression projectionBindingExpression
        );

        private static Expression CreateReadJTokenExpression(
        ...
        )
        {
            ...
            if (storeName.Length == 0)
            {
                ...
                if (!entityType.IsDocumentRoot())
                {
                    ...
                    if (!ownership.IsUnique && property.IsOrdinalKeyProperty())
                    {
                    ...
                    if (principalProperty != null)
                    {
                        ...
                        if (_ownerMappings.TryGetValue(jObjectExpression, out var ownerInfo))
                        {
                            ...
                            jObjectExpression is RootReferenceExpression rootReferenceExpression
                        )
                        {
        ...
        )
        {
            ...
            else if (jObjectExpression is ObjectAccessExpression objectAccessExpression)
            {
                ...
                innerExpression = CreateGetValueExpression(
                    innerAccessExpression,
                    ((IAccessExpression)innerAccessExpression).Name,
                    typeof(JObject)
                );
            }
            ...
            if (converter != null)
            {
                ...
                var body = ReplacingExpressionVisitor.Replace(
                    ...
                    Expression.Call(
                        jTokenParameter,
                        JTokenToObjectWithSerializerMethodInfo.MakeGenericMethod(
                ...
                valueExpression = Expression.Invoke(
                    Expression.Lambda(body, jTokenParameter),
                    jTokenExpression
                );
            }
            else
        ...
        private static T SafeToObjectWithSerializer<T>(JToken token) =>
            token == null || token.Type == JTokenType.Null
                ? default
                : token.ToObject<T>(CosmosClientWrapper.Serializer);
    }
}
