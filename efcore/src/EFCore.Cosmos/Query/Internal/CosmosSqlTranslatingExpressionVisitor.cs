...
public class CosmosSqlTranslatingExpressionVisitor : ExpressionVisitor
{
    ...
    private static readonly MethodInfo ConcatMethodInfo = typeof(string).GetRuntimeMethod(
        ...
        )
        {
            ...
            if (entityType.BaseType == null && !entityType.GetDirectlyDerivedTypes().Any())
            {
                ...
                return _sqlExpressionFactory.Constant(
                    (entityType.ClrType == comparisonType) == match
                );
            }
        ...
        switch (extensionExpression)
        {
            ...
            case StructuralTypeShaperExpression entityShaperExpression:
                ...
                if (
                    ...
                    && result
                        is UnaryExpression { Operand.NodeType: ExpressionType.Convert } outerUnary
    ...
    protected override Expression VisitMethodCall(MethodCallExpression methodCallExpression)
    {
        ...
        {
            if (
                methodCallExpression.Method == StringEqualsWithStringComparison
        ...
        Expression TranslateContains(Expression untranslatedItem, Expression untranslatedCollection)
        {
            ...
            if (
                TryRewriteContainsEntity(
                    collection,
                    itemUnchecked ?? untranslatedItem,
                    out var result
                )
            )
            {
            ...
            switch (collection)
            {
                // If the collection was an inline NewArrayExpression with constants only, we get a single constant
                // for that array.
                case SqlConstantExpression
                {
                    Value: IEnumerable values,
                    TypeMapping: var typeMapping
                }:
                {
                ...
                // If the collection was an inline NewArrayExpression with at least one non-constant, the
                // NewArrayExpression makes it
                // as-is to translation, where it (currently) cannot be translated. Identify this case and translate
                // the elements.
                case not SqlExpression
                    when untranslatedCollection is NewArrayExpression { Expressions: var values }:
                {
    ...
    protected override Expression VisitNewArray(NewArrayExpression newArrayExpression)
    {
        ...
        AddTranslationErrorDetails(
            CosmosStrings.CannotTranslateNonConstantNewArrayExpression(newArrayExpression.Print())
        );
        return QueryCompilationContext.NotTranslatedExpression;
    }
        ...
        switch (unaryExpression.NodeType)
        {
            ...
            case ExpressionType.ConvertChecked:
                if (
                    ...
                    // But we let is pass here since we don't have explicit cast mechanism here and in some cases object
                    // convert is due to value types
                    || unaryExpression.Type == typeof(object)
                )
                {
                    return sqlOperand;
    ...
    private static bool TryEvaluateToConstant(
        Expression expression,
