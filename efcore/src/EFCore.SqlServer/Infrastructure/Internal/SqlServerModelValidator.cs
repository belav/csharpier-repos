...
public class SqlServerModelValidator : RelationalModelValidator
{
    ...
    )
    {
        foreach (var index in model.GetEntityTypes().SelectMany(t => t.GetDeclaredIndexes()))
        {
            ...
            {
                var notFound = includeProperties.FirstOrDefault(i =>
                    index.DeclaringEntityType.FindProperty(i) == null
                );
                ...
                var coveredProperty = includeProperties.FirstOrDefault(i =>
                    index.Properties.Any(p => i == p.Name)
                );
