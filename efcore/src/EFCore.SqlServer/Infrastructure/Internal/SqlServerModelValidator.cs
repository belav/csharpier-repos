...
public class SqlServerModelValidator : RelationalModelValidator
{
    ...
    )
    {
        ...
        {
            if (
                ((IConventionEntityType)mappedType).GetUseSqlOutputClauseConfigurationSource()
                is null
            )
            {
                continue;
            ...
            if (firstSqlOutputSetting is null)
            {
                (firstSqlOutputSetting, firstMappedType) = (
                    mappedType.IsSqlOutputClauseUsed(),
                    mappedType
                );
            }
        ...
        if (identityColumns.Count > 1)
        {
            var sb = new StringBuilder().AppendJoin(
                identityColumns.Values.Select(
                    p => "'" + p.DeclaringType.DisplayName() + "." + p.Name + "'"
                )
            );
            throw new InvalidOperationException(
