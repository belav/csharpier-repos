...
public class SqlServerModificationCommandBatch : AffectedCountModificationCommandBatch
{
    ...
    /// <summary>
    ///     The SQL Server limit on parameters, including two extra parameters to sp_executesql (@stmt
    // and @params).
    /// </summary>
    ...
    {
        // If there are any pending bulk insert commands and the new command is incompatible with them (not
        // an insert, insert into a
    ...
    {
        // TryAddCommand above already applied any pending commands if the new command is incompatible with
        // them.
        // So if the new command is an insert, just append it to pending, otherwise do the regular add
        // logic.
    ...
    public override void Execute(IRelationalConnection connection)
    {
        ...
        {
            // SQL Server error: The target table '%.*ls' of the DML statement cannot have any enabled triggers
            // if the statement contains an
            // OUTPUT clause without INTO clause.
            // This occurs when the user hasn't declared in metadata that a table has triggers, but triggers do
        ...
        {
            // SQL Server error: Column '%ls.%.*ls' cannot be referenced in the OUTPUT clause because the column
            // definition contains a
            // subquery or references a function that performs user or system data access [...]
            // See
            // https://docs.microsoft.com/sql/relational-databases/errors-events/mssqlserver-4186-database-engine-error
            throw new DbUpdateException(
