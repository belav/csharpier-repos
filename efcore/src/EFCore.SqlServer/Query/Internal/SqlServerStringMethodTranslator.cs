...
public class SqlServerStringMethodTranslator : IMethodCallTranslator
{
    ...
    )
    {
        ...
        if (IsNullOrEmptyMethodInfo.Equals(method))
        {
            ...
            return _sqlExpressionFactory.OrElse(
                ...
                _sqlExpressionFactory.Like(argument, _sqlExpressionFactory.Constant(string.Empty))
            );
        }
    ...
    )
    {
        ...
        if (
            string.Equals(storeType, "nvarchar(max)", StringComparison.OrdinalIgnoreCase)
        ...
        charIndexExpression = _sqlExpressionFactory.Subtract(
            charIndexExpression,
            _sqlExpressionFactory.Constant(1)
        );
    ...
    private static bool IsLikeWildChar(char c) => c == '%' || c == '_' || c == '[';
