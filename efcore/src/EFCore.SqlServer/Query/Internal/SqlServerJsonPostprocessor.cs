...
/// <summary>
///     Converts <see cref="SqlServerOpenJsonExpression" /> expressions with WITH (the default) to
// OPENJSON without WITH under the following
///     conditions:
///     * When an ordering still exists on the [key] column, i.e. when the ordering of the original
// JSON array needs to be preserved
///       (e.g. limit/offset).
///     * When the column type in the WITH clause is a SQL Server "CLR type" - these are
// incompatible with WITH (e.g. hierarchy id).
/// </summary>
/// <remarks>
...
public class SqlServerJsonPostprocessor : ExpressionVisitor
{
    ...
    public override Expression? Visit(Expression? expression)
    {
        switch (expression)
        {
            ...
            case SelectExpression selectExpression:
            {
                ...
                for (var i = 0; i < selectExpression.Tables.Count; i++)
                {
                    ...
                    if (
                        ...
                        && (
                            // Condition 1: an ordering still refers to the OPENJSON's [key] column - ordering needs to be
                            // preserved.
                            selectExpression
                ...
                // when we mark columns for rewrite we don't yet have the updated SelectExpression, so we store the
                // info in temporary dictionary
                // and now that we have created new SelectExpression we add it to the proper dictionary that we will
                // use for rewrite
                ...
                // Record the OPENJSON expression and its projected column(s), along with the store type we just
            ...
            // JsonScalarExpression over a column coming out of OPENJSON/WITH; this means that the column
            // represents an owned sub-
            // entity, and therefore must have AS JSON. Rewrite the column and simply collapse the paths
            // together.
                ...
                ):
            {
                ...
                // The new OPENJSON (without WITH) always projects a `value` column, instead of a properly named
        ...
        )
        {
            ...
            // Binary data (varbinary) is stored in JSON as base64, which OPENJSON knows how to decode as long
            // the type is
            // specified in the WITH clause. We're now removing the WITH and applying a relational CAST, but
            // that doesn't work
            // for base64 data.
            ...
            //    "AS JSON signifies an owned sub-entity or array of primitives being projected out of
            // OPENJSON/WITH. "
            //    + "Columns referring to sub-entities must be wrapped in Json{Scalar,Query}Expression and will
            // have been already dealt with above");

            if (columnInfo.Path is [])
            {
                // OPENJSON with WITH specified the store type in the WITH, but the version without just always
                // projects
            ...
            else
            {
                ...
                // We need JSON_VALUE to get the individual properties out of those fragments. Note that the
