...
public class EntityFinder<TEntity> : IEntityFinder<TEntity> where TEntity : class
{
    ...
    public virtual TEntity? Find(object?[]? keyValues)
    {
        ...
        var (key, processedKeyValues, _) = ValidateKeyPropertiesAndExtractCancellationToken(
            keyValues!,
            async: false,
            default
        );
        ...
        return FindTracked(key, processedKeyValues)
            ?? _queryRoot.FirstOrDefault(
                BuildLambda(key.Properties, new ValueBuffer(processedKeyValues))
            );
    }
    ...
    )
    {
        ...
        var (key, processedKeyValues, ct) = ValidateKeyPropertiesAndExtractCancellationToken(
            keyValues!,
            async: true,
            cancellationToken
        );
    ...
    )
    {
        ...
        return tracked != null
            ...
            : new ValueTask<object?>(
                _queryRoot.FirstOrDefaultAsync(
                    BuildObjectLambda(key.Properties, new ValueBuffer(processedKeyValues)),
                    ct
                )
            );
    }
