...
{
    private static readonly MethodInfo GenericCreateSet = typeof(DbSetSource)
        .GetTypeInfo()
    ...
    private static Func<DbContext, string?, object> CreateSetFactory<TEntity>()
        where TEntity : class => (c, name) => new InternalDbSet<TEntity>(c, name);
}
