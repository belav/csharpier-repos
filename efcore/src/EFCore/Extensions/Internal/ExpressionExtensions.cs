...
public static class ExpressionExtensions
{
    ...
    )
    {
        ...
        var comparer =
            (propertyBase as IProperty)?.GetValueComparer()
            ?? ValueComparer.CreateDefault(
                propertyBase?.ClrType ?? currentValueExpression.Type,
                favorStructuralComparisons: false
            );
        ...
        where TMemberInfo : MemberInfo
    {
        ...
        Check.DebugAssert(
            ...
            "lambdaExpression.Parameters.Count is "
                + lambdaExpression.Parameters.Count
                + ". Should be 1."
        );
    ...
    public static LambdaExpression? GetLambdaOrNull(this Expression expression) =>
        expression is LambdaExpression lambda ? lambda
    ...
    )
    {
        ...
        for (var i = 1; i < keyProperties.Count; i++)
        {
            predicate = Expression.AndAlso(
                predicate,
                GenerateEqualExpression(entityParameter, keyValuesConstant, keyProperties[i], i)
            );
        }
