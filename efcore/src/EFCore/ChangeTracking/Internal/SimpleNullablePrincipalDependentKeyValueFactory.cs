...
// The methods here box, but this is only used when the primary key is nullable, but the FK is
// non-nullable,
// which is not common.
/// <summary>
    ...
    where TNonNullableKey : struct
{
    ...
    )
    {
        key = (TKey)
            (object)
                ((Func<InternalEntityEntry, TNonNullableKey>)_propertyAccessors.CurrentValueGetter)(
    ...
    )
    {
        key = (TKey)
            (object)
                (
                    (Func<InternalEntityEntry, TNonNullableKey>)
    ...
    )
    {
        key = (TKey)
            (object)
                (
                    (Func<InternalEntityEntry, TNonNullableKey>)
                        _propertyAccessors.OriginalValueGetter!
                )((InternalEntityEntry)entry)!;
        return true;
    }
