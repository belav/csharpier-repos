...
/// </summary>
/// <remarks>
///     Implementations of this type must inherit from the generic <see
// cref="JsonValueReaderWriter{TValue}" />
/// </remarks>
public abstract class JsonValueReaderWriter
{
    ...
    /// <remarks>
    ///     <para>
    ///         The <see cref="Utf8JsonReaderManager.CurrentReader" /> is at the node that contains the
    // value to be read. The value should be read
    ///         as appropriate from the JSON, and then further converted as necessary.
    ///     </para>
    ///     <para>
    ///         Nulls are handled externally to this reader. That is, this method will never be called
    // if the JSON value is "null".
    ///     </para>
    ///     <para>
    ///         In most cases, the value is represented in the JSON document as a simple property
    // value--e.g. a number, boolean, or string.
    ///         However, it could be an array or sub-document. In this case, the <see
    // cref="Utf8JsonReaderManager" /> should be used to parse
    ///         the JSON as appropriate.
    ///     </para>
    /// </remarks>
    /// <param name="manager">The <see cref="Utf8JsonReaderManager" /> for the JSON being read.</param>
    /// <param name="existingObject">Can be used to update an existing object, rather than create a new
    // one.</param>
    ...
    /// </summary>
    /// <param name="writer">The <see cref="Utf8JsonWriter" /> into which the value should be
    // written.</param>
    ...
    /// <summary>
    ///     Creates a <see cref="JsonValueReaderWriter{TValue}" /> instance of the given type, using the
    // <c>Instance</c>
    ///     property to get th singleton instance if possible.
    /// </summary>
    /// <param name="readerWriterType">The type, which must inherit from <see
    // cref="JsonValueReaderWriter{TValue}" />.</param>
