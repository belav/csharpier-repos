...
/// <summary>
///     Allows interception of the <see cref="O:DbContext.SaveChanges" /> and <see
// cref="O:DbContext.SaveChangesAync" /> methods.
/// </summary>
/// <remarks>
///     <para>
///         SaveChanges interceptors can be used to view, change, or suppress execution of the
// SaveChanges call and
///         modify the result before it is returned to EF.
///     </para>
///     <para>
///         Consider inheriting from <see cref="SaveChangesInterceptor" /> if not implementing all
// methods.
///     </para>
///     <para>
///         Use <see
// cref="DbContextOptionsBuilder.AddInterceptors(Microsoft.EntityFrameworkCore.Diagnostics.IInterceptor[])"
// />
...
public interface ISaveChangesInterceptor : IInterceptor
{
    ...
    /// </summary>
    /// <param name="eventData">Contextual information about the <see cref="DbContext" /> being
    // used.</param>
    /// <param name="result">
    ///     Represents the current result if one exists.
    ///     This value will have <see cref="InterceptionResult{Int32}.HasResult" /> set to <see
    // langword="true" /> if some previous
    ///     interceptor suppressed execution by calling <see
    // cref="InterceptionResult{Int32}.SuppressWithResult" />.
    ///     This value is typically used as the return value for the implementation of this method.
    /// </param>
    /// <returns>
    ///     If <see cref="InterceptionResult{Int32}.HasResult" /> is false, the EF will continue as
    // normal.
    ///     If <see cref="InterceptionResult{Int32}.HasResult" /> is true, then EF will suppress the
    // operation it
    ///     was about to perform and use <see cref="InterceptionResult{Int32}.Result" /> instead.
    ///     An implementation of this method for any interceptor that is not attempting to change the
    // result
    ...
    /// </summary>
    /// <remarks>
    ///     This method is still called if an interceptor suppressed creation of a command in <see
    // cref="SavingChanges" />.
    ...
    /// <remarks>
    ///     This method is still called if an interceptor suppressed creation of a command in <see
    // cref="SavingChangesAsync" />.
    ///     In this case, <paramref name="result" /> is the result returned by <see
    // cref="SavingChangesAsync" />.
    /// </remarks>
    /// <param name="eventData">Contextual information about the <see cref="DbContext" /> being
    // used.</param>
    ...
    /// <param name="result">
    ///     Represents the current result if one exists.
    ///     This value will have <see cref="InterceptionResult.IsSuppressed" /> set to <see
    // langword="true" /> if some previous
    ///     interceptor suppressed execution by calling <see cref="InterceptionResult.Suppress" />.
    ///     This value is typically used as the return value for the implementation of this method.
    /// </param>
    /// <returns>
    ///     If <see cref="InterceptionResult.IsSuppressed" /> is <see langword="false" />, then EF will
    // throw the exception.
    ///     If <see cref="InterceptionResult.IsSuppressed" /> is <see langword="true" />, then EF will
