...
/// <summary>
///     A <see cref="DbSet{TEntity}" /> can be used to query and save instances of <typeparamref
// name="TEntity" />.
///     LINQ queries against a <see cref="DbSet{TEntity}" /> will be translated into queries against
// the database.
/// </summary>
/// <remarks>
///     <para>
///         The results of a LINQ query against a <see cref="DbSet{TEntity}" /> will contain the
// results
///         returned from the database and may not reflect changes made in the context that have not
///         been persisted to the database. For example, the results will not contain newly added
// entities
///         and may still contain entities that are marked for deletion.
///     </para>
///     <para>
///         Depending on the database being used, some parts of a LINQ query against a <see
// cref="DbSet{TEntity}" />
///         may be evaluated in memory rather than being translated into a database query.
///     </para>
///     <para>
///         <see cref="DbSet{TEntity}" /> objects are usually obtained from a <see
// cref="DbSet{TEntity}" />
///         property on a derived <see cref="DbContext" /> or from the <see
// cref="DbContext.Set{TEntity}()" />
///         method.
///     </para>
///     <para>
///         Entity Framework Core does not support multiple parallel operations being run on the
// same <see cref="DbContext" />
///         instance. This includes both parallel execution of async queries and any explicit
// concurrent use from multiple threads.
///         Therefore, always await async calls immediately, or use separate DbContext instances for
// operations that execute
///         in parallel. See <see href="https://aka.ms/efcore-docs-threading">Avoiding DbContext
// threading issues</see> for more information
///         and examples.
///     </para>
///     <para>
...
///         <see href="https://aka.ms/efcore-docs-change-tracking">Changing tracking</see> for more
    ...
    where TEntity : class
{
    ...
    /// </summary>
    /// <remarks>
    ///     See <see href="https://aka.ms/efcore-docs-query">Querying data with EF Core</see> for more
    // information and examples.
    /// </remarks>
    ...
    /// <remarks>
    ///     <para>
    ///         This is a convenience method to help with disambiguation of extension methods in the
    // same
    ///         namespace that extend both interfaces.
    ///     </para>
    ///     <para>
    ///         See <see href="https://aka.ms/efcore-docs-query">Querying data with EF Core</see> for
    ...
    /// <summary>
    ///     Gets a <see cref="LocalView{TEntity}" /> that represents a local view of all Added,
    // Unchanged,
    ...
    ///     </para>
    ///     <para>
    ///         This property can be used for data binding by populating the set with data, for example
    // by using the
    ...
    ///         <see cref="ChangeTracker.AutoDetectChangesEnabled" /> has been set to <see
    // langword="false" />.
    ///     </para>
    ///     <para>
    ...
    /// <summary>
    ///     Finds an entity with the given primary key values. If an entity with the given primary key
    // values
    ///     is being tracked by the context, then it is returned immediately without making a request to
    // the
    ///     database. Otherwise, a query is made to the database for an entity with the given primary
    // key values
    ///     and this entity, if found, is attached to the context and returned. If no entity is found,
    // then
    ///     null is returned.
    /// </summary>
    /// <remarks>
    ///     See <see href="https://aka.ms/efcore-docs-find">Using Find and FindAsync</see> for more
    // information and examples.
    /// </remarks>
    ...
    ///     <see cref="EntityEntry{TEntity}" /> for the entity. The entry provides access to change
    // tracking
    ...
    /// <remarks>
    ///     <para>
    ///         Generally, no database interaction will be performed until <see
    // cref="DbContext.SaveChanges()" /> is called.
    ///     </para>
    ///     <para>
    ...
    ///         then it will be tracked in the <see cref="EntityState.Unchanged" /> state. If the
    // primary key
    ...
    ///         An entity is considered to have its primary key value set if the primary key property is
    // set
    ...
    /// <summary>
    ///     Begins tracking the given entity in the <see cref="EntityState.Deleted" /> state such that
    // it will
    ///     be removed from the database when <see cref="DbContext.SaveChanges()" /> is called.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         If the entity is already tracked in the <see cref="EntityState.Added" /> state then the
    // context will
    ///         stop tracking the entity (rather than marking it as <see cref="EntityState.Deleted" />)
    // since the
    ///         entity was previously added to the context and does not exist in the database.
    ///     </para>
    ///     <para>
    ///         Any other reachable entities that are not already being tracked will be tracked in the
    // same way that
    ///         they would be if <see cref="Attach(TEntity)" /> was called before calling this method.
    ///         This allows any cascading actions to be applied when <see cref="DbContext.SaveChanges()"
    // /> is called.
    ///     </para>
    ///     <para>
    ///         Use <see cref="EntityEntry.State" /> to set the state of only a single entity.
    ...
    ///     <para>
    ///         For entity types with generated keys if an entity has its primary key value set
    ///         then it will be tracked in the <see cref="EntityState.Modified" /> state. If the primary
    // key
    ///         value is not set then it will be tracked in the <see cref="EntityState.Added" /> state.
    ///         This helps ensure new entities will be inserted, while existing entities will be
    // updated.
    ///         An entity is considered to have its primary key value set if the primary key property is
    // set
    ///         to anything other than the CLR default for the property type.
    ...
    /// <remarks>
    ///     See <see href="https://aka.ms/efcore-docs-change-tracking">EF Core change tracking</see>
    ///     and <see href="https://aka.ms/efcore-docs-attach-range">Using AddRange, UpdateRange,
    // AttachRange, and RemoveRange</see>
    ///     for more information and examples.
    /// </remarks>
    ...
    /// <summary>
    ///     Begins tracking the given entities in the <see cref="EntityState.Deleted" /> state such that
    // they will
    ///     be removed from the database when <see cref="DbContext.SaveChanges()" /> is called.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         If any of the entities are already tracked in the <see cref="EntityState.Added" /> state
    // then the context will
    ///         stop tracking those entities (rather than marking them as <see
    // cref="EntityState.Deleted" />) since those
    ///         entities were previously added to the context and do not exist in the database.
    ///     </para>
    ///     <para>
    ///         Any other reachable entities that are not already being tracked will be tracked in the
    // same way that
    ///         they would be if <see cref="AttachRange(TEntity[])" /> was called before calling this
    // method.
    ///         This allows any cascading actions to be applied when <see cref="DbContext.SaveChanges()"
    // /> is called.
    ///     </para>
    ...
    ///         Any other reachable entities that are not already being tracked will be tracked in the
    // same way that
    ///         they would be if <see cref="AttachRange(IEnumerable{TEntity})" /> was called before
    // calling this method.
    ///         This allows any cascading actions to be applied when <see cref="DbContext.SaveChanges()"
    // /> is called.
    ///     </para>
    ...
    /// <summary>
    ///     Returns an <see cref="IEnumerator{T}" /> which when enumerated will execute a query against
    // the database
    ...
    /// <summary>
    ///     Returns an <see cref="IEnumerator" /> which when enumerated will execute a query against the
    // database
    ///     to load all entities from the database.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    ///     Returns an <see cref="IAsyncEnumerator{T}" /> which when enumerated will asynchronously
    ...
    ///     </para>
    ///     <para>
    ///         See <see href="https://aka.ms/efcore-docs-services">Accessing DbContext services</see>
    // for more information and examples.
    ///     </para>
    /// </remarks>
    ...
    /// <summary>
    ///     Gets a value indicating whether the collection is a collection of System.Collections.IList
    // objects.
    ///     Always returns <see langword="false" />.
    /// </summary>
    /// <remarks>
    ///     See <see href="https://aka.ms/efcore-docs-local-views">Local views of tracked entities in EF
    // Core</see> for more information and
    ///     examples.
    /// </remarks>
