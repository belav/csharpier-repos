...
public class ReferenceCollectionBuilder : RelationshipBuilderBase
{
    ...
    ///     </para>
    ///     <para>
    ///         If <see cref="HasPrincipalKey" /> is not specified, then an attempt will be made to
    // match
    ///         the data type and order of foreign key properties against the primary key of the
    // principal
    ...
    /// <returns>The same builder instance so that multiple configuration calls can be
    // chained.</returns>
    public virtual ReferenceCollectionBuilder HasForeignKey(
