        ...
        IModelFinalizingConvention
{
    ...
    )
    {
        ...
        var targetEntityTypeBuilder = TryGetTargetEntityTypeBuilder(
            entityTypeBuilder,
            targetClrType,
    ...
    )
    {
        ...
        var navigationCandidates = Dependencies.MemberClassifier
            .GetNavigationCandidates(targetEntityType, useAttributes: true);
        ...
        if (Attribute.IsDefined(inverseNavigationPropertyInfo, typeof(InversePropertyAttribute)))
        {
            var inverseAttribute =
                inverseNavigationPropertyInfo.GetCustomAttribute<InversePropertyAttribute>(true)!;
        ...
        var referencingNavigationsWithAttribute = AddInverseNavigation(
            entityType,
            navigationMemberInfo,
            targetEntityType,
        ...
        if (
            ...
            && (targetOwnership == null || targetOwnership.PrincipalEntityType == entityType)
        )
        {
            if (
                navigationMemberInfo.DeclaringType != entityType.ClrType
                && (
        ...
        if (
            entityType.IsOwned()
            && (ownership == null || ownership.PrincipalEntityType == targetEntityType)
        )
        {
            if (
                navigationMemberInfo.DeclaringType != entityType.ClrType
        ...
        if (ownership != null || targetOwnership != null)
        {
            return null;
        }
        ...
        )
        {
            ...
            var inverseNavigationTargetType = inverseNavigationPropertyInfo.PropertyType
                .TryGetSequenceType();
            if (
                navigationTargetType != null
    ...
    )
    {
        ...
        while (ambiguousInverse == null && targetBaseType != null)
        {
        ...
        {
            if (
                entityType.FindSkipNavigation(navigationMemberInfo)
                is IConventionSkipNavigation existingSkipNavigation
            )
            {
                var existingSkipNavigationInverse = existingSkipNavigation.Inverse;
                var inverseSkipNavigation = targetEntityType.FindSkipNavigation(
                    inverseNavigationMemberInfo
                );
                ...
                )
                {
                    existingInverse.DeclaringEntityType
                        .Builder
                        .HasNoSkipNavigation(existingInverse, fromDataAnnotation: true);
                    inverseSkipNavigation!.DeclaringEntityType
                        .Builder
                        .HasNoSkipNavigation(inverseSkipNavigation, fromDataAnnotation: true);
                }

                if (existingSkipNavigation.IsInModel)
                {
                    entityType.Builder
                        .HasNoSkipNavigation(existingSkipNavigation, fromDataAnnotation: true);
                }

                if (existingSkipNavigationInverse?.IsInModel == true)
                {
                    existingSkipNavigationInverse.DeclaringEntityType
                        .Builder
                        .HasNoSkipNavigation(
                            existingSkipNavigationInverse,
                            fromDataAnnotation: true
                        );
                }
                ...
                existingAmbiguousNavigation?.DeclaringEntityType
                    .Builder
                    .HasNoSkipNavigation(existingAmbiguousNavigation, fromDataAnnotation: true);
        ...
        if (foreignKey.IsOwnership)
        {
            if (navigation.IsOnDependent)
            {
                ...
                ConfigurationSource.DataAnnotation
                    .Overrides(foreignKey.DeclaringEntityType.GetConfigurationSource())
            )
            {
                navigation.DeclaringEntityType
                    .Model
                    .Builder
                    .HasNoEntityType(foreignKey.DeclaringEntityType, fromDataAnnotation: true);
            }
            else
            {
                foreignKey.DeclaringEntityType
                    .Builder
                    .HasNoRelationship(foreignKey, fromDataAnnotation: true);
            }
        }
        else if (
            foreignKey.DeclaringEntityType
                .Builder
                .HasNoRelationship(foreignKey, fromDataAnnotation: true) == null
        )
        {
            foreignKey.Builder
                .HasNavigation((string?)null, navigation.IsOnDependent, fromDataAnnotation: true);
        }
    }
    ...
    )
    {
        ...
        if (
            modelBuilder.Metadata.FindEntityType(declaringType) != null
        ...
        var leastDerivedEntityTypes = modelBuilder.Metadata
            .FindLeastDerivedEntityTypes(declaringType, t => !t.HasSharedClrType && !t.IsOwned());
    ...
    )
    {
        ...
        var foreignKey = navigation.ForeignKey;
        if (foreignKey.DeclaringEntityType.IsOwned() || foreignKey.PrincipalEntityType.IsOwned())
        {
            return;
        }
        ...
        foreach (var entityType in model.GetEntityTypes())
        {
            ...
            foreach (var (navigation, references) in inverseNavigations.Values)
            {
                foreach (var (memberInfo, conventionEntityType) in references)
                {
                    ...
                    while (ambiguousInverse == null && baseType != null)
                    {
    ...
    )
    {
        ...
        while (currentEntityType != null)
        {
            ...
            if (navigationMap != null)
            {
                ...
                {
                    if (
                        memberInfo.GetMemberType().IsAssignableFrom(entityType.ClrType)
    ...
    private static (MemberInfo, IConventionEntityType)? FindAmbiguousInverse(
        ...
        List<(
            MemberInfo Inverse,
            IConventionEntityType InverseEntityType
        )> referencingNavigationsWithAttribute
    )
    {
        ...
        foreach (var referencingTuple in referencingNavigationsWithAttribute)
        {
            ...
            if (
                inverseEntityType is null
                || inverseEntityType.Builder
                    .IsIgnored(
                        referencingTuple.Inverse.GetSimpleMemberName(),
                        fromDataAnnotation: true
                    )
            )
            {
            ...
            if (
                !referencingTuple.Inverse.IsSameAs(navigation)
                || (
    ...
    )
    {
        ...
        {
            inverseNavigations =
                new Dictionary<string, (MemberInfo, List<(MemberInfo, IConventionEntityType)>)>();
        ...
        if (!inverseNavigations.TryGetValue(inverseNavigation.Name, out var inverseTuple))
        {
            ...
            inverseNavigations[inverseNavigation.Name] = (
                inverseNavigation,
                referencingNavigationsWithAttribute
            );
        }
        ...
        {
            if (
                memberInfo.IsSameAs(navigation)
    ...
    )
    {
        ...
        for (var index = 0; index < referencingNavigationsWithAttribute.Count; index++)
        {
            ...
            if (navigation == null)
            {
                ...
                if (otherEntityType != null)
                {
                    ...
                    var existingInverses = targetEntityType
                        .GetNavigations()
                        .Where(n => n.TargetEntityType == otherEntityType)
                        .ToList();

                    if (existingInverses.Count == 0)
                    {
                        otherEntityType.Builder.HasRelationship(targetEntityType, memberInfo, null);
                    }
                    else if (existingInverses.Count == 1)
                    {
                        ...
                        {
                            otherEntityType.Builder
                                .HasRelationship(targetEntityType, memberInfo, null);
                        }
                    }
                }
            }
        ...
        if (anyRemoved && referencingNavigationsWithAttribute.Count == 1)
        {
            var otherEntityType = FindActualEntityType(
                referencingNavigationsWithAttribute[0].Item2
            );
            if (otherEntityType != null)
            {
    ...
    private static Dictionary<
        string,
        (MemberInfo Navigation, List<(MemberInfo, IConventionEntityType)> References)
    >? GetInverseNavigations(IConventionAnnotatable entityType) =>
