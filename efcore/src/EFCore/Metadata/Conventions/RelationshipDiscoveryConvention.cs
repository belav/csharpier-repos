...
namespace Microsoft.EntityFrameworkCore.Metadata.Conventions
{
          ...
          IForeignKeyRemovedConvention
    {
            ...
            )
            {
                        ...
                        )
                        {
                            if (
                                !existingCandidate
                                    ...
                                    .Contains(navigationPropertyInfo)
                            )
                            {
                ...
                if (!entityType.IsKeyless)
                {
                    ...
                    foreach (var inverseCandidateTuple in inverseCandidates)
                    {
                        ...
                        var inverseIsCollection =
                            inverseTargetType != inversePropertyInfo.PropertyType;
                        if (
                        ...
                        if (shouldBeOwnership && inverseIsCollection && navigations.Count == 1)
                        {
                            ...
                            if (targetType.IsInModel && IsImplicitlyCreatedUnusedType(targetType))
                            {
        ...
        )
        {
            ...
            return ((InternalEntityTypeBuilder)entityTypeBuilder).GetTargetEntityTypeBuilder(
                targetClrType,
                navigationMemberInfo,
                null,
                shouldBeOwned
            ...
            foreach (var relationshipCandidate in relationshipCandidates)
            {
                ...
                while (relationshipCandidate.NavigationProperties.Count > 0)
                {
                    ...
                    if (compatibleInverse == null)
                    {
                        ...
                        {
                            var nextSelfRefCandidate = relationshipCandidate
                                .InverseProperties
                                .First();
                            if (
                        ...
                        {
                            foreach (
                                var inverseProperty in relationshipCandidate
                                    .InverseProperties
                                    .ToList()
                            )
                            {
            ...
            if (existingInverse != null)
            {
                if (
                    ...
                    || (
                        !existingInverse
                            .TargetEntityType
                            .IsAssignableFrom(entityTypeBuilder.Metadata)
                        && !entityTypeBuilder
                            .Metadata
        ...
        private void RemoveExtraOwnershipInverse(
        ...
        )
        {
            ...
            )
            {
                ...
                foreach (
                    var navigationProperty in relationshipCandidate.NavigationProperties.ToList()
                )
                {
                    RemoveNavigation(
                        navigationProperty,
                    ...
                    if (existingNavigation.IsOnDependent)
                    {
                        removed =
                            existingNavigation
                                .ForeignKey
                                .Builder
                                .HasNavigation((string?)null, existingNavigation.IsOnDependent)
                            != null;
                    }
        ...
        )
        {
            ...
            {
                if (
                    navigation.Key.GetSimpleMemberName() != name
        ...
        private static bool RemoveAmbiguous(IConventionEntityType entityType, Type targetType)
        {
            ...
            if (ambiguousNavigations?.IsEmpty == false)
            {
                ...
                foreach (var ambiguousNavigation in ambiguousNavigations)
                {
                    ...
                    {
                        newAmbiguousNavigations = newAmbiguousNavigations.Remove(
                            ambiguousNavigation.Key
                        );
                    }
                }
