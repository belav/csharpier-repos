...
public class InternalPropertyBuilder
    : InternalPropertyBaseBuilder<IConventionPropertyBuilder, Property>,
        IConventionPropertyBuilder
{
                    ...
                    )
                    {
                        throw new InvalidOperationException(
                            CoreStrings.KeyPropertyCannotBeNullable(
                                Metadata.Name,
                                Metadata.DeclaringType.DisplayName(),
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual InternalPropertyBuilder? HasSentinel(
        object? sentinel,
        ConfigurationSource configurationSource
    )
    {
    ...
    public virtual bool CanSetSentinel(object? sentinel, ConfigurationSource? configurationSource)
    {
        ...
        else
        {
            try
            {
                return Equals(
                    Metadata.Sentinel,
                    Convert.ChangeType(sentinel, Metadata.ClrType, CultureInfo.InvariantCulture)
                );
            }
            catch (Exception)
            {
    ...
    public new virtual InternalPropertyBuilder? HasField(
        string? fieldName,
        ConfigurationSource configurationSource
    ) => base.HasField(fieldName, configurationSource) == null ? null : this;
    ...
    public new virtual InternalPropertyBuilder? HasField(
        FieldInfo? fieldInfo,
        ConfigurationSource configurationSource
    ) => base.HasField(fieldInfo, configurationSource) == null ? null : this;
    ...
    public new virtual InternalPropertyBuilder? UsePropertyAccessMode(
        ...
        ConfigurationSource configurationSource
    ) => base.UsePropertyAccessMode(propertyAccessMode, configurationSource) == null ? null : this;
    ...
    )
    {
        if (valueGeneratorType == null)
        {
            return HasValueGenerator(
                (Func<IProperty, ITypeBase, ValueGenerator>?)null,
                configurationSource
            );
        }
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual InternalElementTypeBuilder? SetElementType(
        Type? elementType,
        ConfigurationSource configurationSource
    )
    {
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual bool CanSetElementType(
        Type? elementType,
        ConfigurationSource? configurationSource
    ) =>
    ...
    public virtual InternalPropertyBuilder? Attach(InternalTypeBaseBuilder typeBaseBuilder)
    {
        ...
        else
        {
            ...
            newPropertyBuilder =
                Metadata.IsIndexerProperty()
                    ? typeBaseBuilder.IndexerProperty(
                        Metadata.ClrType,
                        Metadata.Name,
                        configurationSource
                    )
        ...
        if (oldElementType != null)
        {
            ...
            {
                var newElementTypeBuilder = new InternalElementTypeBuilder(
                    newElementType,
                    ModelBuilder
                );
                newElementTypeBuilder.MergeAnnotationsFrom(oldElementType);

                var oldElementNullableConfigurationSource =
                    oldElementType.GetIsNullableConfigurationSource();
                if (
                    oldElementNullableConfigurationSource.HasValue
                    && newElementTypeBuilder.CanSetIsRequired(
                        !oldElementType.IsNullable,
                        oldElementNullableConfigurationSource
                    )
                )
                {
                    newElementTypeBuilder.IsRequired(
                        !oldElementType.IsNullable,
                        oldElementNullableConfigurationSource.Value
                    );
                }

                var oldElementUnicodeConfigurationSource =
                    oldElementType.GetIsUnicodeConfigurationSource();
                if (
                    oldElementUnicodeConfigurationSource.HasValue
                    && newElementTypeBuilder.CanSetIsUnicode(
                        oldElementType.IsNullable,
                        oldElementUnicodeConfigurationSource
                    )
                )
                {
                    newElementTypeBuilder.IsUnicode(
                        oldElementType.IsNullable,
                        oldElementUnicodeConfigurationSource.Value
                    );
                }

                var oldElementProviderClrTypeConfigurationSource =
                    oldElementType.GetProviderClrTypeConfigurationSource();
                if (
                    oldElementProviderClrTypeConfigurationSource.HasValue
                    && newElementTypeBuilder.CanSetConversion(
                        oldElementType.GetProviderClrType(),
                        oldElementProviderClrTypeConfigurationSource
                    )
                )
                {
                    newElementTypeBuilder.HasConversion(
                        oldElementType.GetProviderClrType(),
                        oldElementProviderClrTypeConfigurationSource.Value
                    );
                }

                var oldElementConverterConfigurationSource =
                    oldElementType.GetValueConverterConfigurationSource();
                if (
                    oldElementConverterConfigurationSource.HasValue
                    && newElementTypeBuilder.CanSetConverter(
                        oldElementType.GetValueConverter()?.GetType(),
                        oldElementConverterConfigurationSource
                    )
                )
                {
                    newElementTypeBuilder.HasConverter(
                        oldElementType.GetValueConverter()?.GetType(),
                        oldElementConverterConfigurationSource.Value
                    );
                }

                var oldElementPrecisionConfigurationSource =
                    oldElementType.GetPrecisionConfigurationSource();
                if (
                    oldElementPrecisionConfigurationSource.HasValue
                    && newElementTypeBuilder.CanSetPrecision(
                        oldElementType.GetPrecision(),
                        oldElementPrecisionConfigurationSource
                    )
                )
                {
                    newElementTypeBuilder.HasPrecision(
                        oldElementType.GetPrecision(),
                        oldElementPrecisionConfigurationSource.Value
                    );
                }

                var oldElementScaleConfigurationSource =
                    oldElementType.GetScaleConfigurationSource();
                if (
                    oldElementScaleConfigurationSource.HasValue
                    && newElementTypeBuilder.CanSetScale(
                        oldElementType.GetScale(),
                        oldElementScaleConfigurationSource
                    )
                )
                {
                    newElementTypeBuilder.HasScale(
                        oldElementType.GetScale(),
                        oldElementScaleConfigurationSource.Value
                    );
                }

                var oldElementMaxLengthConfigurationSource =
                    oldElementType.GetMaxLengthConfigurationSource();
                if (
                    oldElementMaxLengthConfigurationSource.HasValue
                    && newElementTypeBuilder.CanSetMaxLength(
                        oldElementType.GetMaxLength(),
                        oldElementMaxLengthConfigurationSource
                    )
                )
                {
                    newElementTypeBuilder.HasMaxLength(
                        oldElementType.GetMaxLength(),
                        oldElementMaxLengthConfigurationSource.Value
                    );
                }

                var oldElementTypeMappingConfigurationSource =
                    oldElementType.GetTypeMappingConfigurationSource();
                if (
                    oldElementTypeMappingConfigurationSource.HasValue
                    && newElementTypeBuilder.CanSetTypeMapping(
                        oldElementType.TypeMapping,
                        oldElementTypeMappingConfigurationSource
                    )
                )
                {
                    newPropertyBuilder.HasTypeMapping(
                        oldElementType.TypeMapping,
                        oldElementTypeMappingConfigurationSource.Value
                    );
                }

                var oldElementComparerConfigurationSource =
                    oldElementType.GetValueComparerConfigurationSource();
                if (
                    oldElementComparerConfigurationSource.HasValue
                    && newElementTypeBuilder.CanSetValueComparer(
                        oldElementType.GetValueComparer(),
                        oldElementComparerConfigurationSource
                    )
                )
                {
                    newElementTypeBuilder.HasValueComparer(
                        oldElementType.GetValueComparer(),
                        oldElementComparerConfigurationSource.Value
                    );
                }
            }
        }
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    IConventionPropertyBuilder? IConventionPropertyBuilder.HasSentinel(
        object? sentinel,
        bool fromDataAnnotation
    ) =>
        HasSentinel(
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    bool IConventionPropertyBaseBuilder<IConventionPropertyBuilder>.CanSetField(
        string? fieldName,
        bool fromDataAnnotation
    ) =>
        CanSetField(
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    IConventionElementTypeBuilder? IConventionPropertyBuilder.SetElementType(
        Type? elementType,
        bool fromDataAnnotation
    ) =>
        SetElementType(
