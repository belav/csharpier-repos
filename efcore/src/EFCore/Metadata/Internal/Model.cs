    ...
    public class Model : ConventionAnnotatable, IMutableModel, IConventionModel, IRuntimeModel
    {
        ...
        private EntityType? AddEntityType(EntityType entityType)
        {
            ...
            if (entityType.HasSharedClrType)
            {
                ...
                {
                    var newConfigurationSource = entityType.GetConfigurationSource()
                        .Max(existingTypes.ConfigurationSource);
                    existingTypes.Types.Add(entityType);
                    _sharedTypes[entityType.ClrType] = (
                ...
                else
                {
                    ...
                    _sharedTypes.Add(
                        entityType.ClrType,
                        (entityType.GetConfigurationSource(), types)
                    );
                }
            }
        ...
        public virtual EntityType? FindEntityType(string name)
        {
            ...
            return _entityTypes.TryGetValue(name, out var entityType) ? entityType : null;
        }
        ...
        {
            var referencingForeignKey = entityType.GetDeclaredReferencingForeignKeys()
                .FirstOrDefault();
            ...
            var referencingSkipNavigation = entityType.GetDeclaredReferencingSkipNavigations()
                .FirstOrDefault();
        ...
        public virtual Type? FindClrType(string name) =>
            _entityTypes.TryGetValue(name, out var entityType)
                ? entityType.HasSharedClrType ? null : entityType.ClrType
                : null;
        ...
        ///     doing so can result in application failures when updating to a new Entity Framework Core release.
        /// </summary>
        public virtual bool IsShared(Type type) => _sharedTypes.ContainsKey(type);
        ...
        public virtual string? AddIgnored(Type type, ConfigurationSource configurationSource) =>
            AddIgnored(
                GetDisplayName(Check.NotNull(type, nameof(type))),
                type,
                configurationSource
            );
        ...
        ///     doing so can result in application failures when updating to a new Entity Framework Core release.
        /// </summary>
        public virtual bool IsOwned(Type type) => FindIsOwnedConfigurationSource(type) != null;
        ...
        /// </summary>
        [DebuggerStepThrough]
        void IMutableModel.SetChangeTrackingStrategy(
            ChangeTrackingStrategy? changeTrackingStrategy
