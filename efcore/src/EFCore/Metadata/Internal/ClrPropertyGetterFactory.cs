...
public class ClrPropertyGetterFactory : ClrAccessorFactory<IClrPropertyGetter>
{
    ...
    >(MemberInfo memberInfo, IPropertyBase? propertyBase)
    {
        ...
        var hasStructuralSentinelValueExpression = structuralReadExpression.MakeHasSentinel(
            propertyBase
        );

        readExpression = ConvertReadExpression(readExpression, hasSentinelValueExpression);
        structuralReadExpression = ConvertReadExpression(
        ...
        return new ClrPropertyGetter<TEntity, TStructuralType, TValue>(
            ...
            Expression
                .Lambda<Func<TEntity, bool>>(hasSentinelValueExpression, entityParameter)
                .Compile(),
            Expression
                .Lambda<Func<TStructuralType, TValue>>(
                    structuralReadExpression,
                    structuralParameter
                )
                .Compile(),
            Expression
                .Lambda<Func<TStructuralType, bool>>(
                    hasStructuralSentinelValueExpression,
                    structuralParameter
                )
                .Compile()
        ...
        Expression CreateReadExpression(ParameterExpression parameter, bool fromContainingType)
        {
            ...
            {
                return PropertyBase.CreateMemberAccess(
                    propertyBase,
                    parameter,
                    memberInfo,
                    fromContainingType
            ...
            // This path handles properties that exist only on proxy types and so only exist if the instance is
            // a proxy
        ...
        static Expression ConvertReadExpression(
            Expression expression,
            Expression sentinelExpression
        ) =>
