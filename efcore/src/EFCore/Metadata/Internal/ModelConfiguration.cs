...
public class ModelConfiguration
{
    ...
    )
    {
        ...
        {
            EnsureCompatible(
                TypeConfigurationType.ComplexType,
                type,
                configurationType,
                configuredType
            );
    ...
    public virtual ComplexPropertyConfiguration? FindComplexProperty(Type type) =>
        _complexProperties.TryGetValue(type, out var property) ? property : null;
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public virtual bool RemoveComplexProperty(Type type) => _complexProperties.Remove(type);
