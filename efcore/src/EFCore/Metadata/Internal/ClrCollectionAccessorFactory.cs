...
{
    private static readonly MethodInfo GenericCreate = typeof(ClrCollectionAccessorFactory)
        .GetTypeInfo()
        .GetDeclaredMethod(nameof(CreateGeneric))!;

    private static readonly MethodInfo CreateAndSetMethod = typeof(ClrCollectionAccessorFactory)
        .GetTypeInfo()
        .GetDeclaredMethod(nameof(CreateAndSet))!;

    private static readonly MethodInfo CreateMethod = typeof(ClrCollectionAccessorFactory)
        .GetTypeInfo()
    ...
    private static readonly MethodInfo CreateHashSetMethod = typeof(ClrCollectionAccessorFactory)
        .GetTypeInfo()
    ...
    private static IClrCollectionAccessor? Create(IPropertyBase navigation, IEntityType? targetType)
    {
        ...
        var propertyType =
            navigation.IsIndexerProperty() || navigation.IsShadowProperty()
                ? navigation.ClrType
        ...
        var boundMethod = GenericCreate.MakeGenericMethod(
            memberInfo?.DeclaringType ?? navigation.DeclaringType.ClrType,
            propertyType,
            elementType
        );
        ...
        where TElement : class
    {
        ...
        {
            var memberInfoForRead = navigation.GetMemberInfo(
                forMaterialization: false,
                forSet: false
            );
            navigation.TryGetMemberInfo(
                forMaterialization: false,
                forSet: true,
                out var memberInfoForWrite,
                out _
            );
            navigation.TryGetMemberInfo(
                forMaterialization: true,
                forSet: true,
                out var memberInfoForMaterialization,
                out _
            );

            var memberAccessForRead = (Expression)
                Expression.MakeMemberAccess(entityParameter, memberInfoForRead);
            ...
            getterDelegate = Expression
                .Lambda<Func<TEntity, TCollection>>(memberAccessForRead, entityParameter)
                .Compile();

            if (memberInfoForWrite != null)
            {
                setterDelegate = CreateSetterDelegate(
                    entityParameter,
                    memberInfoForWrite,
                    valueParameter
                );
            ...
            {
                setterDelegateForMaterialization = CreateSetterDelegate(
                    entityParameter,
                    memberInfoForMaterialization,
                    valueParameter
                );
            }
        }
        ...
        if (concreteType != null)
        {
            var isHashSet =
                ...
                && concreteType.GetGenericTypeDefinition() == typeof(HashSet<>);
            if (setterDelegate != null || setterDelegateForMaterialization != null)
            {
                if (isHashSet)
                {
                    createAndSetDelegate =
                        (Func<TEntity, Action<TEntity, TCollection>, TCollection>)
        ...
        static Action<TEntity, TCollection> CreateSetterDelegate(
            ...
            Expression
                .Lambda<Action<TEntity, TCollection>>(
                    Expression
                        .MakeMemberAccess(parameterExpression, memberInfo)
    ...
    private static TCollection CreateCollection<TCollection, TConcreteCollection>()
        where TCollection : class
        where TConcreteCollection : TCollection, new() => new TConcreteCollection();
        ...
        where TElement : class
    {
        var collection = (TCollection)
            (ICollection<TElement>)new HashSet<TElement>(LegacyReferenceEqualityComparer.Instance);
        setterDelegate(entity, collection);
        return collection;
    }
        ...
        where TElement : class
    {
        var collection = (TCollection)
            (ICollection<TElement>)
                new ObservableHashSet<TElement>(LegacyReferenceEqualityComparer.Instance);
        setterDelegate(entity, collection);
        return collection;
    }
