          ...
          IConventionEntityTypeBuilder
    {
        ...
        private InternalKeyBuilder? HasKeyInternal(
            ...
            if (key == null)
            {
                ...
                var containingForeignKeys = actualProperties.SelectMany(
                        p =>
                            p.GetContainingForeignKeys()
                                .Where(k => k.DeclaringEntityType != Metadata)
                    )
                    .ToList();
        ...
        private InternalPropertyBuilder? Property(
            ...
            else
            {
                ...
                if (memberInfo == null)
                {
                    memberInfo = Metadata.ClrType.GetMembersInHierarchy(propertyName)
                        .FirstOrDefault();
                }
                ...
                if (baseEntityType != null)
                {
                    ...
                    var foreignKeysUsingKeyProperties = Metadata.GetDerivedTypesInclusive()
                        ...
                        .Where(
                            fk =>
                                fk.Properties.Any(
                                    p => baseEntityType.FindProperty(p.Name)?.IsKey() == true
                                )
                        );
        ...
        public static EntityType.Snapshot? DetachAllMembers(EntityType entityType)
        {
            ...
            foreach (var relationshipToBeDetached in entityType.GetDeclaredForeignKeys().ToList())
            {
                ...
                if (
                    detachedRelationship.Relationship.Metadata.GetConfigurationSource()
            ...
            foreach (var keyToDetach in entityType.GetDeclaredKeys().ToList())
            {
                ...
                var detachedKey = DetachKey(keyToDetach);
                if (
                    detachedKey.Item1.Metadata.GetConfigurationSource()
            ...
            foreach (var index in entityType.GetDeclaredIndexes().ToList())
            {
                ...
                if (
                    detachedIndex.Metadata.GetConfigurationSource()
                        .Overrides(ConfigurationSource.Explicit)
                ) {
                ...
                if (
                    ...
                    Check.DebugAssert(
                        existingNavigation.ForeignKey.IsOwnership
                            || !(
                                (IReadOnlyNavigation)existingNavigation
            ...
            if (targetShouldBeOwned != true)
            {
                ...
                if (ownership != null)
                {
                    ...
                    if (targetType.IsAssignableFrom(ownership.PrincipalEntityType.ClrType))
                    {
                        if (configurationSource.HasValue)
                        {
                            ownership.PrincipalEntityType.UpdateConfigurationSource(
                                configurationSource.Value
                            );
                        }
