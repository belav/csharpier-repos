...
public class DependencyInjectionMethodParameterBinding : DependencyInjectionParameterBinding
{
    private static readonly MethodInfo GetServiceMethod =
        typeof(InfrastructureExtensions).GetRuntimeMethod(
            ...
            new[] { typeof(IInfrastructure<IServiceProvider>) }
        )!;

    private static readonly MethodInfo GetServiceFromPropertyMethod =
        typeof(DependencyInjectionMethodParameterBinding)
            .GetTypeInfo()
            .GetDeclaredMethod(nameof(GetServiceFromProperty))!;

    private static readonly MethodInfo CreateServiceMethod =
        typeof(DependencyInjectionMethodParameterBinding)
            .GetTypeInfo()
            .GetDeclaredMethod(nameof(CreateService))!;
    ...
    public override Expression BindToParameter(ParameterBindingInfo bindingInfo)
    {
        ...
        if (serviceInstance != null)
        {
            ...
            return Expression.Condition(
                ...
                Expression.Lambda(Expression.Call(serviceInstance, Method, parameters), parameters)
            );
        }
    ...
    private static object? GetServiceFromProperty(
        MaterializationContext materializationContext,
        IPropertyBase property,
        object entity
    ) =>
        materializationContext.Context.GetDependencies().StateManager.GetOrCreateEntry(entity)[
            property
        ];
