...
public class QueryOptimizingExpressionVisitor : ExpressionVisitor
{
    ...
    protected override Expression VisitMethodCall(MethodCallExpression methodCallExpression)
    {
        ...
        {
            var itemExpression =
                left == lambda.Parameters[0] ? right
                : right == lambda.Parameters[0] ? left
                : null;

            if (itemExpression is not null)
            {
                var containsMethodDefinition =
                ...
                {
                    var containsMethod = containsMethodDefinition.MakeGenericMethod(
                ...
                if (
                    (methodInfo == EnumerableMethods.All || methodInfo == QueryableMethods.All)
                    && negated
                )
                {
                    var containsMethod = containsMethodDefinition.MakeGenericMethod(
        ...
        if (
            visited is
            {
                Method:
                {
                    ...
                    DeclaringType:
                    {
                        Name: "Operators" or "EmbeddedOperators",
        ...
        if (expression is MemberExpression { Expression: Expression inner } visitedMemberExpression)
        {
            var (conditional, convert) = inner switch
            {
                ...
                UnaryExpression
                {
                    ...
                    Operand: ConditionalExpression cond
                } conv => (cond, conv),
                _ => (null, null),
            };
            ...
            if (
                ...
                && !(
                    ...
                    && visitedMemberExpression.Member.Name
                        is nameof(Nullable<int>.HasValue)
                            or nameof(Nullable<int>.Value)
                )
            )
            {
