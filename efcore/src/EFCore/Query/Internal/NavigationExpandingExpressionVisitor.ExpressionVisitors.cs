    ...
    public partial class NavigationExpandingExpressionVisitor
    {
        ...
        private class ExpandingExpressionVisitor : ExpressionVisitor
        {
            ...
            )
            {
                ...
                if (
                    ...
                    && innerNavigationExpansionExpression.CardinalityReducingGenericMethodInfo
                        != null
                )
                {
                ...
                var innerKeySelector =
                    _navigationExpandingExpressionVisitor.ProcessLambdaExpression(
                        innerSource,
                        Expression.Lambda(innerKey, innerParameter)
                    );

                var resultSelectorOuterParameter = Expression.Parameter(
        ...
        private sealed class IncludeExpandingExpressionVisitor : ExpandingExpressionVisitor
        {
            private static readonly MethodInfo _fetchJoinEntityMethodInfo =
                typeof(IncludeExpandingExpressionVisitor).GetRequiredDeclaredMethod(
                    nameof(FetchJoinEntity)
                );
            ...
            ) : base(navigationExpandingExpressionVisitor, source, extensibilityHelper)
            {
                ...
                _ignoreAutoIncludes =
                    navigationExpandingExpressionVisitor._queryCompilationContext.IgnoreAutoIncludes;
            }
            ...
            )
            {
                ...
                {
                    var argument = newExpression.Arguments[i];
                    var newRoot = Expression.MakeMemberAccess(
                        currentRoot,
                ...
                foreach (var kvp in entityReference.IncludePaths)
                {
                    ...
                    else
                    {
                        ...
                        var filterExpression =
                            entityReference.IncludePaths[navigationBase].FilterExpression;
                        if (
                            _queryStateManager
                        ...
                        )
                        {
                            ...
                            if (filterExpression == null)
                            {
                                var newResultSelector = Expression.Quote(
                                    Expression.Lambda(
                                        Expression.Call(
                                            _fetchJoinEntityMethodInfo.MakeGenericMethod(
                                                joinParameter.Type,
                                ...
                                subquery = joinMethodCallExpression.Update(
                                    null,
                                    joinMethodCallExpression.Arguments
                                        .Take(4)
...
#pragma warning restore IDE0060 // Remove unused parameter
        ...
        private sealed class GroupingElementReplacingExpressionVisitor : ExpressionVisitor
        {
            ...
            )
            {
                ...
                _keyMemberInfo = parameterExpression.Type.GetRequiredDeclaredProperty(
                    nameof(IGrouping<int, int>.Key)
                );
        ...
        private sealed class RemoveRedundantNavigationComparisonExpressionVisitor
            : ExpressionVisitor
        {
            private readonly IDiagnosticsLogger<DbLoggerCategory.Query> _logger;
            ...
            private Expression ProcessNavigationPath(Expression expression)
            {
                switch (expression)
                {
                    case MemberExpression memberExpression when memberExpression.Expression != null:
                        ...
                        if (
                            innerExpression is NavigationDataExpression navigationDataExpression
