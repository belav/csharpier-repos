...
public class InMemoryExpressionTranslatingExpressionVisitor : ExpressionVisitor
{
    ...
    )
    {
        ...
        )
        {
            ...
            var matchingLeft =
                leftType.IsAssignableFrom(newLeft.Type) ? newLeft
                : converter != null
            ...
            var matchingRight =
                rightType.IsAssignableFrom(newRight.Type) ? newRight
                : converter != null
        ...
        if (
            exactMatch == null && (!property!.ClrType.IsAssignableFrom(newLeft.Type))
    ...
    protected override Expression VisitMemberInit(MemberInitExpression memberInitExpression)
    {
        ...
        for (var i = 0; i < newBindings.Length; i++)
        {
            ...
            if (
                ((MemberAssignment)newBindings[i]).Expression
    ...
    protected override Expression VisitMethodCall(MethodCallExpression methodCallExpression)
    {
        ...
        if (subqueryTranslation != null)
        {
            ...
            if (
                innerExpression is StructuralTypeShaperExpression shaper
                && (convertedType == null || convertedType.IsAssignableFrom(shaper.Type))
            )
            {
    ...
    protected override Expression VisitTypeBinary(TypeBinaryExpression typeBinaryExpression)
    {
        if (
            ...
            && Visit(typeBinaryExpression.Expression)
                is StructuralTypeReferenceExpression typeReference
        )
        {
    ...
    protected override Expression VisitUnary(UnaryExpression unaryExpression)
    {
        ...
        if (
            newOperand is StructuralTypeReferenceExpression typeReference
    ...
    )
    {
        ...
        {
            var entityShaper = (StructuralTypeShaperExpression)
    ...
    )
    {
        ...
        if (
            serverQuery is UnaryExpression { NodeType: ExpressionType.Convert } unaryExpression
    ...
    private static IProperty? FindProperty(Expression? expression)
    {
        ...
        if (
            expression is MethodCallExpression { Method.IsGenericMethod: true } readValueMethodCall
    ...
    )
    {
        ...
        switch (source)
        {
            ...
            case MethodCallExpression { Method.IsGenericMethod: true } methodCallExpression
                when methodCallExpression.Method.GetGenericMethodDefinition()
    ...
    {
        var leftEntityReference = left
            is StructuralTypeReferenceExpression { StructuralType: IEntityType } l
            ? l
            : null;
        var rightEntityReference = right
            is StructuralTypeReferenceExpression { StructuralType: IEntityType } r
            ? r
            : null;
        ...
        if (
            ...
            && (leftEntityReference?.Subquery != null || rightEntityReference?.Subquery != null)
        )
        {
            throw new InvalidOperationException(
                CoreStrings.EntityEqualityOnCompositeKeyEntitySubqueryNotSupported(
...
#pragma warning restore IDE0066 // Convert switch statement to expression
    ...
    private static bool TranslationFailed(Expression? original, Expression? translation) =>
        ...
        && (
            ...
            || translation is StructuralTypeReferenceExpression
        );
