    ...
    public partial class SqliteConnection : DbConnection
    {
        ...
        static SqliteConnection()
        {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                ...
                {
                    var localFolder = appDataType
                        ?.GetRuntimeProperty("LocalFolder")
                        ?.GetValue(currentAppData);
                    var localFolderPath = (string?)
                        storageFolderType?.GetRuntimeProperty("Path")?.GetValue(localFolder);
                    ...
                    var tempFolder = appDataType
                        ?.GetRuntimeProperty("TemporaryFolder")
                        ?.GetValue(currentAppData);
                    var tempFolderPath = (string?)
                        storageFolderType?.GetRuntimeProperty("Path")?.GetValue(tempFolder);
        ...
        private void LoadExtensionCore(string file, string? proc)
        {
            var rc = sqlite3_load_extension(
                Handle,
                utf8z.FromString(file),
                utf8z.FromString(proc),
                out var errmsg
            );
            if (rc != SQLITE_OK)
            {
                throw new SqliteException(
                    Resources.SqliteNativeError(rc, errmsg.utf8_to_string()),
                    rc,
                    rc
                );
            }
        ...
        )
        {
            ...
            var flags = isDeterministic ? SQLITE_DETERMINISTIC : 0;
            var state = new AggregateDefinition<TAccumulate, TResult>(
                name,
                seed,
                func,
                resultSelector
            ...
            _aggregates ??= new Dictionary<
                ...
                (
                    int,
                    object?,
                    delegate_function_aggregate_step?,
                    delegate_function_aggregate_final?
                )
            >(FunctionsKeyComparer.Instance);
