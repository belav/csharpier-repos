...
namespace Microsoft.Data.Sqlite
{
    internal class SqliteParameterBinder : SqliteValueBinder
    {
        ...
        protected override void BindInt64(long value) => sqlite3_bind_int64(_stmt, _index, value);

        protected override void BindNull() => sqlite3_bind_null(_stmt, _index);

        protected override void BindText(string value) =>
            sqlite3_bind_text(
                _stmt,
                _index,
                ShouldTruncate(value.Length) ? value.Substring(0, _size!.Value) : value
            );

        private bool ShouldTruncate(int length) =>
            _size.HasValue && length > _size.Value && _size.Value != -1;
    }
}
