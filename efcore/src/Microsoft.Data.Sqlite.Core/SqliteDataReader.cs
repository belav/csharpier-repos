...
namespace Microsoft.Data.Sqlite
{
    ...
    /// </summary>
    /// <seealso href="https://docs.microsoft.com/dotnet/standard/data/sqlite/types">Data
    // Types</seealso>
    public class SqliteDataReader : DbDataReader
    {
        ...
        public virtual sqlite3_stmt? Handle => _record?.Handle;
        ...
        public override bool HasRows => _record?.HasRows ?? false;
        ...
        public override object this[string name] =>
            _record == null ? throw new InvalidOperationException(Resources.NoData) : _record[name];
        ...
        /// </summary>
        /// <returns><see langword="true" /> if there are more rows; otherwise, <see langword="false"
        // />.</returns>
        ...
        /// </summary>
        /// <returns><see langword="true" /> if there are more result sets; otherwise, <see langword="false"
        // />.</returns>
        /// <exception cref="SqliteException">A SQLite error occurs during execution.</exception>
        /// <seealso
        // href="https://docs.microsoft.com/dotnet/standard/data/sqlite/batching">Batching</seealso>
        ...
        /// <summary>
        ///     Gets the declared data type name of the specified column. The storage class is returned for
        // computed
        ///     columns.
        /// </summary>
        /// <param name="ordinal">The zero-based column ordinal.</param>
        /// <returns>The data type name of the column.</returns>
        /// <remarks>Due to SQLite's dynamic type system, this may not reflect the actual type of the
        // value.</remarks>
        /// <seealso href="https://docs.microsoft.com/dotnet/standard/data/sqlite/types">Data
        // Types</seealso>
        ...
        /// </summary>
        /// <param name="ordinal">The zero-based column ordinal.</param>
        /// <returns><see langword="true" /> if the specified column is <see cref="DBNull" />; otherwise,
        ...
        ///     <see cref="SqliteBlob" /> is returned. Otherwise, the all of the data is read into memory
        // and a
        ///     <see cref="MemoryStream" /> is returned.
        /// </summary>
        /// <param name="ordinal">The zero-based column ordinal.</param>
        /// <returns>The returned object.</returns>
        /// <seealso href="https://docs.microsoft.com/dotnet/standard/data/sqlite/blob-io">BLOB
        // I/O</seealso>
        ...
        /// <seealso
        // href="https://docs.microsoft.com/dotnet/standard/data/sqlite/metadata">Metadata</seealso>
        public override DataTable GetSchemaTable()
        {
            if (_closed)
            {
                throw new InvalidOperationException(
                    Resources.DataReaderClosed(nameof(GetSchemaTable))
                );
            }
            ...
            var numericPrecisionColumn = new DataColumn(
            ...
            var isAutoIncrementColumn = new DataColumn(
                SchemaTableOptionalColumn.IsAutoIncrement,
                typeof(bool)
            );

            var baseCatalogNameColumn = new DataColumn(
                SchemaTableOptionalColumn.BaseCatalogName,
                typeof(string)
            );
            var baseSchemaNameColumn = new DataColumn(
                SchemaTableColumn.BaseSchemaName,
                typeof(string)
            );
            var baseTableNameColumn = new DataColumn(
                SchemaTableColumn.BaseTableName,
                typeof(string)
            );
            var baseColumnNameColumn = new DataColumn(
                SchemaTableColumn.BaseColumnName,
                typeof(string)
            );

            var baseServerNameColumn = new DataColumn(
                ...
                if (tableName != null && columnName != null)
                {
                    using (var command = _command.Connection.CreateCommand())
                    {
                        command.CommandText = new StringBuilder()
                            ...
                            .AppendLine(
                                "NOT EXISTS (SELECT * FROM pragma_index_info(i.name) c2 WHERE c2.name != c.name);"
                            )
                            .ToString();
                    ...
                    if (databaseName != null && !eponymousVirtualTable)
                    {
                        var rc = sqlite3_table_column_metadata(
                            _command.Connection.Handle,
                            databaseName,
                            tableName,
                            columnName,
                            out var dataType,
            ...
            [UnconditionalSuppressMessage(
                "Trimming",
                "IL2111:Method with parameters or return value with `DynamicallyAccessedMembersAttribute`"
