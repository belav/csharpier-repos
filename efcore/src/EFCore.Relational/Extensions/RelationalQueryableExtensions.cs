...
public static class RelationalQueryableExtensions
{
    ...
    ///     </para>
    ///     <para>
    ///         Warning: there is no guarantee that executing this command directly will result in the
    // same behavior as if EF Core had
    ///         executed the command.
    ///     </para>
    ///     <para>
    ///         Note that DbCommand is an <see cref="IDisposable" /> object. The caller is responsible
    // for disposing the returned
    ///         command.
    ///     </para>
    ///     <para>
    ///         This is only typically supported by queries generated by Entity Framework Core.
    ...
    /// <remarks>
    ///     <para>
    ///         If the database provider supports composing on the supplied SQL, you can compose on top
    // of the raw SQL query using
    ///         LINQ operators: <c>context.Blogs.FromSqlRaw("SELECT * FROM Blogs").OrderBy(b =>
    // b.Name)</c>.
    ///     </para>
    ///     <para>
    ///         As with any API that accepts SQL it is important to parameterize any user input to
    // protect against a SQL injection
    ///         attack. You can include parameter place holders in the SQL query string and then supply
    // parameter values as additional
    ///         arguments. Any parameter values you supply will automatically be converted to a <see
    // cref="DbParameter" />.
    ///     </para>
    ///     <para>
    ///         However, <b>never</b> pass a concatenated or interpolated string (<c>$""</c>) with
    // non-validated user-provided values
    ///         into this method. Doing so may expose your application to SQL injection attacks. To use
    // the interpolated string syntax,
    ///         consider using <see cref="FromSql{TEntity}" /> to create parameters.
    ///     </para>
    ///     <para>
    ///         This overload also accepts <see cref="DbParameter" /> instances as parameter values. In
    // addition to using positional
    ///         placeholders as above (<c>{0}</c>), you can also use named placeholders directly in the
    // SQL query string.
    ///     </para>
    ///     <para>
    ...
    /// <param name="source">
    ///     An <see cref="IQueryable{T}" /> to use as the base of the raw SQL query (typically a <see
    // cref="DbSet{TEntity}" />).
    /// </param>
    ...
    ///         As with any API that accepts SQL it is important to parameterize any user input to
    // protect against a SQL injection
    ///         attack. You can include interpolated parameter place holders in the SQL query string.
    ...
    /// <typeparam name="TEntity">The type of the elements of <paramref name="source" />.</typeparam>
    /// <param name="source">
    ///     An <see cref="IQueryable{T}" /> to use as the base of the interpolated string SQL query
    // (typically a <see cref="DbSet{TEntity}" />).
    /// </param>
    /// <param name="sql">The interpolated string representing a SQL query with parameters.</param>
    /// <returns>An <see cref="IQueryable{T}" /> representing the interpolated string SQL
    // query.</returns>
    ...
    /// <summary>
    ///     Returns a new query which is configured to load the collections in the query results in a
    ...
    ///         However, this can cause performance issues when the query loads multiple related
    // collections.
    ///     </para>
    ///     <para>
    ///         The default query splitting behavior for queries can be controlled by
    ///         <see
    // cref="RelationalDbContextOptionsBuilder{TBuilder,TExtension}.UseQuerySplittingBehavior(QuerySplittingBehavior)"
    // />.
    ///     </para>
    ///     <para>
    ///         See <see href="https://aka.ms/efcore-docs-split-queries">EF Core split queries</see> for
    ...
    /// <summary>
    ///     Returns a new query which is configured to load the collections in the query results through
    ...
    // collections.
    ///         However, since separate queries are used, this can result in inconsistent results when
    // concurrent updates occur.
    ///         Serializable or snapshot transactions can be used to mitigate this
    ///         and achieve consistency with split queries, but that may bring other performance costs
    // and behavioral difference.
    ///     </para>
    ///     <para>
    ///         The default query splitting behavior for queries can be controlled by
    ///         <see cref="RelationalDbContextOptionsBuilder{TBuilder,
    // TExtension}.UseQuerySplittingBehavior(QuerySplittingBehavior)" />.
    ///     </para>
    ///     <para>
    ///         See <see href="https://aka.ms/efcore-docs-split-queries">EF Core split queries</see> for
    // more information and examples.
    ...
    /// <typeparam name="TEntity">The type of entity being queried.</typeparam>
    /// <param name="source">The source query.</param>
    /// <returns>A new query where collections will be loaded through separate database
    // queries.</returns>
    ...
    /// <summary>
    ///     Deletes all database rows for the entity instances which match the LINQ query from the
    // database.
    /// </summary>
    /// <remarks>
    ///     <para>
    ...
    // until
    ///         <see cref="DbContext.SaveChanges()" /> is called. It also does not interact with the EF
    // change tracker in any way:
    ///         entity instances which happen to be tracked when this operation is invoked aren't taken
    // into account, and aren't updated
    ///         to reflect the changes.
    ///     </para>
    ///     <para>
    ///         See <see href="https://aka.ms/efcore-docs-bulk-operations">Executing bulk operations
    // with EF Core</see>
    ///         for more information and examples.
    ///     </para>
    /// </remarks>
    ...
    /// <summary>
    ///     Asynchronously deletes database rows for the entity instances which match the LINQ query
    // from the database.
    /// </summary>
    /// <remarks>
    ///     <para>
    ...
    /// <summary>
    ///     Updates all database rows for the entity instances which match the LINQ query from the
    // database.
    /// </summary>
    /// <remarks>
    ///     <para>
    ...
    /// </remarks>
    /// <param name="source">The source query.</param>
    /// <param name="setPropertyCalls">A collection of set property statements specifying properties to
    // update.</param>
    ...
    /// <summary>
    ///     Asynchronously updates database rows for the entity instances which match the LINQ query
    // from the database.
    /// </summary>
    /// <remarks>
    ///     <para>
