    ...
    public class RelationalSqlTranslatingExpressionVisitor : ExpressionVisitor
    {
        ...
        )
        {
            if (entityReferenceExpression.ParameterEntity != null)
            {
                ...
                var allRequiredNonPkPropertiesCondition = entityType
                    .GetProperties()
                    .Where(p => !p.IsNullable && !p.IsPrimaryKey())
                    .ToList();
                if (allRequiredNonPkPropertiesCondition.Count > 0)
                {
                    condition = allRequiredNonPkPropertiesCondition
                ...
                return _sqlExpressionFactory.Case(
                    new List<CaseWhenClause> { new CaseWhenClause(condition, propertyAccess) },
                    elseResult: null
                );
