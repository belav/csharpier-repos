...
public class RelationalSqlTranslatingExpressionVisitor : ExpressionVisitor
{
    ...
    private static readonly MethodInfo LeastMethodInfo =
        typeof(RelationalDbFunctionsExtensions).GetMethod(
            nameof(RelationalDbFunctionsExtensions.Least)
        )!;

    private static readonly MethodInfo GreatestMethodInfo =
        typeof(RelationalDbFunctionsExtensions).GetMethod(
            nameof(RelationalDbFunctionsExtensions.Greatest)
        )!;
    ...
    public virtual SqlExpression? TranslateAverage(SqlExpression sqlExpression)
    {
        ...
        if (inputType == typeof(int) || inputType == typeof(long))
        {
    ...
    protected override Expression VisitBinary(BinaryExpression binaryExpression)
    {
        ...
        )
        {
            ...
            )
            {
                ...
                )
                {
                    ...
                    (
                        genericMethod == QueryableMethods.ElementAt
                        || genericMethod == QueryableMethods.ElementAtOrDefault
                    )
                    && nonNullMethodCallExpression.Arguments[1]
                        is not ConstantExpression { Value: 0 }
                )
                {
                    source = Expression.Call(
                ...
                if (translatedSubquery != null)
                {
                    ...
                    if (
                        ...
                        || RemoveConvert(projection)
                            is StructuralTypeShaperExpression { IsNullable: false }
        ...
        {
            if (
                typeReference.StructuralType is not IEntityType entityType
                || (entityType.BaseType == null && !entityType.GetDirectlyDerivedTypes().Any())
            )
            {
                // No hierarchy
                return _sqlExpressionFactory.Constant(
                    (typeReference.StructuralType.ClrType == comparisonType) == match
                );
            }
            ...
            if (discriminatorProperty == null)
            {
                ...
                {
                    var shaper = (StructuralTypeShaperExpression)
                        typeReference.Subquery.ShaperExpression;
                    var projection = (StructuralTypeProjectionExpression)Visit(
                        shaper.ValueBufferExpression
                    );
                    var subSelectExpression = (SelectExpression)
                        typeReference.Subquery.QueryExpression;
                    ...
                    if (subSelectExpression.Limit == null && subSelectExpression.Offset == null)
                    {
                ...
                {
                    var projection = (StructuralTypeProjectionExpression)Visit(
                        typeReference.Parameter.ValueBufferExpression
                    );
                ...
                SqlExpression GeneratePredicateTpt(StructuralTypeProjectionExpression projection)
                {
                    if (projection.DiscriminatorExpression is CaseExpression caseExpression)
                    {
                        ...
                        var derivedTypeValues = derivedType
                            .GetDerivedTypes()
                        ...
                        {
                            var value = (string)
                                ((SqlConstantExpression)caseWhenClause.Result).Value!;
                        ...
                        var result = predicates.Aggregate(
                            (a, b) => _sqlExpressionFactory.AndAlso(a, b)
                        );
        ...
        bool IsGetTypeMethodCall(
            Expression expression,
            out StructuralTypeReferenceExpression? typeReference
        )
        {
            typeReference = null;
            if (
    ...
    protected override Expression VisitConditional(ConditionalExpression conditionalExpression)
    {
        ...
        return
            ...
            : _sqlExpressionFactory.Case(
                new[] { new CaseWhenClause(sqlTest!, sqlIfTrue!) },
                sqlIfFalse
            );
    }
    ...
    protected override Expression VisitExtension(Expression extensionExpression)
    {
        switch (extensionExpression)
        {
            ...
            case ShapedQueryExpression shapedQueryExpression:
                ...
                if (
                    innerExpression is StructuralTypeShaperExpression ese
                    && (convertedType == null || convertedType.IsAssignableFrom(ese.Type))
                )
                {
                ...
                if (
                    innerExpression is ProjectionBindingExpression pbe
                    && (
                        convertedType == null
                ...
                if (
                    mappedProjectionBindingExpression == null
                    && shaperExpression
                        is BlockExpression
                        {
                            Expressions: [
    ...
    protected override Expression VisitMethodCall(MethodCallExpression methodCallExpression)
    {
        ...
        if (methodCallExpression.TryGetEFPropertyArguments(out var source, out var propertyName))
        {
            ...
            var message = CoreStrings.QueryUnableToTranslateEFProperty(
        ...
        {
            if (arguments[0].Type == typeof(object[]) && arguments[0] is NewArrayExpression)
            {
                return Visit(ConvertObjectArrayEqualityComparison(arguments[0], arguments[1]));
            }
        ...
        )
        {
            ...
            if (
                TryRewriteContainsEntity(
                    enumerable,
                    item == QueryCompilationContext.NotTranslatedExpression ? arguments[1] : item,
                    out var result
                )
            )
            {
        ...
        else if (arguments.Count == 1 && method.IsContainsMethod())
        {
            ...
            if (
                TryRewriteContainsEntity(
                    enumerable!,
                    item == QueryCompilationContext.NotTranslatedExpression ? arguments[0] : item,
                    out var result
                )
            )
            {
        ...
        // translation.
            ...
            && (
                genericMethodDefinition == LeastMethodInfo
                || genericMethodDefinition == GreatestMethodInfo
            )
        ...
        {
            if (method.IsStatic && arguments.Count > 0 && method.DeclaringType == typeof(Queryable))
            {
                ...
                if (
                    TryTranslateAggregateMethodCall(
                        methodCallExpression,
                        out var translatedAggregate
                    )
                )
                {
            ...
            if (
                !TryTranslateAsEnumerableExpression(
                    methodCallExpression.Object,
                    out enumerableExpression
                )
                && TranslationFailed(
                    methodCallExpression.Object,
        ...
        if (
            method == StringEqualsWithStringComparison
    ...
    protected override Expression VisitNewArray(NewArrayExpression newArrayExpression)
    {
        ...
        AddTranslationErrorDetails(
            RelationalStrings.CannotTranslateNonConstantNewArrayExpression(
                newArrayExpression.Print()
            )
        );
        return QueryCompilationContext.NotTranslatedExpression;
    }
    ...
    protected override Expression VisitTypeBinary(TypeBinaryExpression typeBinaryExpression)
    {
        ...
        if (typeReference.StructuralType is not IEntityType entityType)
        {
            return Expression.Constant(
                typeReference.StructuralType.ClrType == typeBinaryExpression.TypeOperand
            );
        }
        ...
        if (discriminatorProperty == null)
        {
            ...
            // TPT or TPC
            var discriminatorValues = derivedType
            ...
            if (typeReference.Parameter != null)
            {
                var typeProjection = (StructuralTypeProjectionExpression)Visit(
                    typeReference.Parameter.ValueBufferExpression
                );
            ...
            SqlExpression GeneratePredicateTpt(
        ...
        {
            if (
                !derivedType.GetRootType().GetIsDiscriminatorMappingComplete()
                || !derivedType
    ...
    protected override Expression VisitUnary(UnaryExpression unaryExpression)
    {
        ...
        if (
            operand is StructuralTypeReferenceExpression typeReference
            ...
            case ExpressionType.TypeAs:
                ...
                if (
                    ...
                    || unaryExpression.Type.UnwrapNullableType()
                        == operand.Type.UnwrapNullableType()
    ...
    private SqlExpression BindProperty(
        StructuralTypeReferenceExpression typeReference,
        IProperty property
    )
    {
        switch (typeReference)
        {
            case { Parameter: StructuralTypeShaperExpression shaper }:
            {
                ...
                var nonPrincipalSharedNonPkProperties =
                ...
                var allRequiredNonPkProperties = entityType
                    .GetProperties()
                ...
                if (
                    nonPrincipalSharedNonPkProperties.Count != 0
    ...
    )
    {
        switch (typeReference)
        {
            case { Parameter: StructuralTypeShaperExpression shaper }:
                ...
                Check.DebugAssert(
                    projection.IsNullable == shaper.IsNullable,
                ...
                switch (genericMethod.Name)
                {
                    ...
                    case nameof(Queryable.Sum)
                        ...
                        enumerableExpression = ProcessSelector(
                            ...
                            arguments[1].UnwrapLambdaFromQuote()
                        );
                        break;
    ...
    )
    {
        ...
        {
            enumerableExpression = new EnumerableExpression(
            ...
            if (TryTranslateAsEnumerableExpression(arguments[0], out var enumerableSource))
            {
                switch (genericMethod.Name)
                {
                    ...
                    case nameof(Queryable.Where) when genericMethod == QueryableMethods.Where:
                        enumerableExpression = ProcessPredicate(
                            enumerableSource,
                            arguments[1].UnwrapLambdaFromQuote()
                        );
                        return enumerableExpression != null;
    ...
    private static Expression RemapLambda(
        ...
        ReplacingExpressionVisitor.Replace(
            ...
            enumerableExpression.Selector,
            lambdaExpression.Body
        );

    private static EnumerableExpression ProcessSelector(
        EnumerableExpression enumerableExpression,
        LambdaExpression lambdaExpression
    ...
    private EnumerableExpression? ProcessPredicate(
        EnumerableExpression enumerableExpression,
        LambdaExpression lambdaExpression
    ) =>
        TranslateInternal(RemapLambda(enumerableExpression, lambdaExpression))
            is SqlExpression predicate
    ...
    private static bool TryEvaluateToConstant(
        Expression expression,
        [NotNullWhen(true)] out SqlConstantExpression? sqlConstantExpression
    )
    {
    ...
    )
    {
        ...
        if (
            item is not StructuralTypeReferenceExpression { StructuralType: IEntityType entityType }
        )
        {
            return false;
        }
        ...
        bool TryRewriteEntityEquality([NotNullWhen(true)] out Expression? result)
        {
            if (IsNullSqlConstantExpression(left) || IsNullSqlConstantExpression(right))
            {
                var nonNullEntityReference = (
                    IsNullSqlConstantExpression(left) ? rightReference : leftReference
                )!;
                var nullComparedEntityType = (IEntityType)nonNullEntityReference.StructuralType;

                if (
                    nonNullEntityReference is
                    { Parameter.ValueBufferExpression: JsonQueryExpression jsonQueryExpression }
                )
                {
                ...
                var nullComparedEntityTypePrimaryKeyProperties = nullComparedEntityType
                    .FindPrimaryKey()
                    ?.Properties;
                ...
                if (
                    nullComparedEntityType.GetRootType() == nullComparedEntityType
                    && nullComparedEntityType.GetMappingStrategy()
                        != RelationalAnnotationNames.TpcMappingStrategy
                )
                {
                    var table =
                        nullComparedEntityType.GetViewOrTableMappings().SingleOrDefault()?.Table
                    ...
                    if (table.IsOptional(nullComparedEntityType))
                    {
                        ...
                        var requiredNonPkProperties = nullComparedEntityType
                            .GetProperties()
                            .Where(p => !p.IsNullable && !p.IsPrimaryKey())
                            .ToList();
            ...
            Check.DebugAssert(
                entityType != null,
                "We checked that at least one side is an entity type before calling this function"
            );
            ...
            if (
                ...
                && (leftReference?.Subquery != null || rightReference?.Subquery != null)
            )
            {
                throw new InvalidOperationException(
                    CoreStrings.EntityEqualityOnCompositeKeyEntitySubqueryNotSupported(
                ...
                foreach (var complexProperty in type.GetComplexProperties())
                {
                    Check.DebugAssert(
                        left is not StructuralTypeReferenceExpression { Subquery: not null }
                            && right
                                is not StructuralTypeReferenceExpression { Subquery: not null },
            ...
            case ParameterBasedComplexPropertyChainExpression chainExpression:
            {
                var lambda = Expression.Lambda(
                    Expression.Call(
                        ...
                        QueryCompilationContext.QueryContextParameter,
                        Expression.Constant(
                            chainExpression.ParameterExpression.Name,
                            typeof(string)
                        ),
                        Expression.Constant(
                            chainExpression.ComplexPropertyChain,
                            typeof(List<IComplexProperty>)
                        ),
                        Expression.Constant(property, typeof(IProperty))
                    ),
                    QueryCompilationContext.QueryContextParameter
    ...
    private Expression CreateComplexPropertyAccessExpression(
        Expression target,
        IComplexProperty complexProperty
    ) =>
        target switch
    ...
    {
        if (
            context.ParameterValues[baseParameterName] is not IEnumerable<TEntity> baseListParameter
        )
        {
            return null;
        }
    ...
    private sealed class StructuralTypeReferenceExpression : Expression
    {
        ...
        public Expression Convert(Type type)
        {
            ...
            return
                ...
                && entityType.GetDerivedTypes().FirstOrDefault(et => et.ClrType == type)
                    is IEntityType derivedEntityType
