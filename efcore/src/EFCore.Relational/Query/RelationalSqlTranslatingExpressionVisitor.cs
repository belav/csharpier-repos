...
public class RelationalSqlTranslatingExpressionVisitor : ExpressionVisitor
{
    ...
    protected override Expression VisitMethodCall(MethodCallExpression methodCallExpression)
    {
        ...
        else
        {
            ...
            if (method.IsStatic && arguments.Count > 0 && method.DeclaringType == typeof(Queryable))
            {
                var genericMethod = method.IsGenericMethod
                    ...
                    : method;
                if (
                    genericMethod == QueryableMethods.AsQueryable
            ...
            if (!abortTranslation && !skipVisitChildren)
            {
        ...
        if (subqueryTranslation != null)
        {
            ...
            if (
                ...
                && (convertedType == null || convertedType.MakeNullable() == innerExpression.Type)
            )
            {
            ...
            if (
                subqueryTranslation.ResultCardinality == ResultCardinality.SingleOrDefault
    ...
    )
    {
        if (entityReferenceExpression.ParameterEntity != null)
        {
            ...
            var nonPrincipalSharedNonPkProperties = entityType
            ...
            var condition = nonPrincipalSharedNonPkProperties
                ...
                .Select(
                    c =>
                        (SqlExpression)
                            _sqlExpressionFactory.NotEqual(c, _sqlExpressionFactory.Constant(null))
                )
    ...
    )
    {
        ...
        if (IsNullSqlConstantExpression(left) || IsNullSqlConstantExpression(right))
        {
            ...
            )
            {
                ...
                {
                    var condition = nullComparedEntityType
                        .GetNonPrincipalSharedNonPkProperties(table)
                        .Where(e => !e.IsNullable)
                        .Select(p =>
                        {
