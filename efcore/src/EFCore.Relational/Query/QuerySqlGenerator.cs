...
public class QuerySqlGenerator : SqlExpressionVisitor
{
    ...
    private static bool IsNonComposedTpc(SelectExpression selectExpression) =>
        ...
        && selectExpression.Projection.Count
            == tpcTablesExpression.SelectExpressions[0].Projection.Count
        && selectExpression.Projection
            .Select(
                (pe, index) =>
                    pe.Expression is ColumnExpression column
                    ...
                    && string.Equals(
                        column.Name,
                        tpcTablesExpression.SelectExpressions[0].Projection[index].Alias,
                        StringComparison.Ordinal
                    )
            )
            .All(e => e);
    ...
    protected override Expression VisitExtension(Expression extensionExpression)
    {
        if (extensionExpression is TpcTablesExpression tpcTablesExpression)
        {
            ...
            {
                GenerateList(
                    tpcTablesExpression.SelectExpressions,
                    e => Visit(e),
    ...
    )
    {
        switch (innerExpression)
        {
            ...
            case SqlUnaryExpression sqlUnaryExpression:
            {
                ...
                if (
                    sqlUnaryExpression.OperatorType == ExpressionType.Negate
