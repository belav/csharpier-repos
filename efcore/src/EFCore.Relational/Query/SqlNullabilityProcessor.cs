...
public class SqlNullabilityProcessor
{
        ...
        switch (tableExpressionBase)
        {
            ...
            case ValuesExpression valuesExpression:
            {
                ...
                for (var i = 0; i < valuesExpression.RowValues.Count; i++)
                {
                    ...
                    var newRowValue = (RowValueExpression)VisitRowValue(
                        rowValue,
    ...
    )
    {
            ...
            switch ((itemNullable, projectionNullable))
            {
                ...
                case (true, true):
                    ...
                    var predicate = VisitSqlBinary(
                        _sqlExpressionFactory.Equal(subqueryProjection, item),
        ...
        inExpression = ProcessInExpressionValues(
            ...
            removeNulls: true,
            removeNullables: false,
            out var valuesHasNull,
            out var nullableValues
        );
        ...
        if (
            allowOptimizedExpansion
            && (!itemNullable || !valuesHasNull && nullableValues.Count == 0)
        )
        {
            return inExpression;
        }
        ...
        )
        {
            ...
            if (inExpression.ValuesParameter is SqlParameterExpression valuesParameter)
            {
                ...
                var values =
                    (IEnumerable?)ParameterValues[valuesParameter.Name] ?? Array.Empty<object>();
        ...
        SqlExpression GenerateNotNullCheck(SqlExpression operand) =>
            OptimizeNonNullableNotExpression(
                _sqlExpressionFactory.Not(
                    ProcessNullNotNull(_sqlExpressionFactory.IsNull(operand), operandNullable: true)
                )
            );
    }
    ...
    )
    {
        ...
        if (sqlBinaryExpression.OperatorType is ExpressionType.Equal or ExpressionType.NotEqual)
        {
            ...
            if (
                optimized is SqlUnaryExpression
                {
                    Operand: ColumnExpression optimizedUnaryColumnOperand
                } optimizedUnary
            )
            {
    ...
    {
        if (
            sqlFunctionExpression is { IsBuiltIn: true, Arguments: not null }
            && string.Equals(
    ...
    )
    {
        ...
        if (sqlUnaryExpression.OperatorType is ExpressionType.Equal or ExpressionType.NotEqual)
        {
            ...
            if (
                result is SqlUnaryExpression
                {
                    Operand: ColumnExpression resultColumnOperand
                } resultUnary
            )
            {
    ...
    private bool IsNull(SqlExpression? expression) =>
        ...
        || expression is SqlParameterExpression { Name: string parameterName }
            && ParameterValues[parameterName] is null;
    ...
    )
    {
        ...
        var rightNullValue =
            rightNullable && right is SqlConstantExpression or SqlParameterExpression;
        ...
        if (
            TryGetBool(right, out var rightBoolValue)
            && !leftNullable
        ...
        if (
            TryGetBool(left, out var leftBoolValue)
            && !rightNullable
    ...
    )
    {
        ...
        // false || a -> a
        if (
            sqlBinaryExpression.Left is SqlConstantExpression
            {
                Value: bool leftBoolValue
            } newLeftConstant
        )
        {
            ...
            case SqlFunctionExpression sqlFunctionExpression:
                {
                    ...
                    if (
                        sqlFunctionExpression is
                        { Instance: not null, InstancePropagatesNullability: true }
                    )
                    {
