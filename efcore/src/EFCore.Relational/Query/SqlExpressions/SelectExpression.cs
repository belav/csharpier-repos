...
public sealed partial class SelectExpression : TableExpressionBase
{
        ...
        : base(null)
    {
...

                {
                    ...
                    else
                    {
                        ...
                        for (var i = 1; i < entityTypes.Length; i++)
                        {
                            ...
                            {
                                Check.DebugAssert(
                                    !propertyNamesMap.ContainsKey(property),
                        ...
                        for (var i = 0; i < entityTypes.Length; i++)
                        {
                            ...
                            var tableExpression = new TableExpression(table);
                            tableExpression.Alias = GenerateUniqueAlias(
                                _usedAliases,
                                tableExpression.Alias
                            );
                            var tableReferenceExpression = new TableReferenceExpression(
        ...
        static ITableBase GetTableBase(IEntityType entityType) =>
            entityType.GetViewOrTableMappings().Single().Table;
    ...
    )
    {
        ...
        {
            Check.DebugAssert(
                expression is SqlExpression || expression is EntityProjectionExpression,
    ...
    public void ApplyGrouping(Expression keySelector)
    {
        ...
        if (
            groupByTerms.Any(
                e =>
                    e is SqlConstantExpression
                    || e is SqlParameterExpression
                    || e is ScalarSubqueryExpression
            )
        )
        {
    ...
    )
    {
        ...
        )
        {
            var tableIndex = selectExpression._tables.FindIndex(
                e => ReferenceEquals(e, tableExpression)
            );
        ...
        )
        {
            ...
            if (unwrappedTable is TableExpression tableExpression)
            {
                ...
                var propertyExpressions = new Dictionary<IProperty, ColumnExpression>();
                var tableReferenceExpression = FindTableReference(
                    selectExpression,
                    tableExpressionBase
                );
                foreach (
            ...
            if (unwrappedTable is SelectExpression subquery)
            {
                ...
                {
                    newPropertyExpressions[property] = subquery.GenerateOuterColumn(
                        tableReferenceExpression,
                        columnExpression
                    );
                }
    ...
    /// </summary>
    [EntityFrameworkInternal]
    public SelectExpression Prune() => Prune(referencedColumns: null);
    ...
    internal SelectExpression Prune(IReadOnlyCollection<string>? referencedColumns = null)
    {
        ...
        for (var i = 0; i < _tables.Count; i++)
        {
            ...
            if (
                ...
                && (table is LeftJoinExpression || table is OuterApplyExpression)
            ...
            {
                _tables[i] = tpcTablesExpression.Prune(
                    _tpcDiscriminatorValues[tpcTablesExpression].Item2,
                    columnsMap[tableAlias]
                );
            }
