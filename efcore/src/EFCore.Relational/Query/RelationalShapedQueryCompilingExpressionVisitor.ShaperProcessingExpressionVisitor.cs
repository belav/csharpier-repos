    ...
    public partial class RelationalShapedQueryCompilingExpressionVisitor
    {
        private sealed class ShaperProcessingExpressionVisitor : ExpressionVisitor
        {
            ...
            private static readonly MethodInfo _isDbNullMethod = typeof(DbDataReader)
                .GetRequiredRuntimeMethod(nameof(DbDataReader.IsDBNull), new[] { typeof(int) });
            ...
            private static readonly MethodInfo _throwReadValueExceptionMethod =
                typeof(ShaperProcessingExpressionVisitor)
                    .GetRequiredDeclaredMethod(nameof(ThrowReadValueException));
            ...
            private static readonly MemberInfo _singleQueryResultCoordinatorResultReadyMemberInfo =
                typeof(SingleQueryResultCoordinator)
                    .GetMember(nameof(SingleQueryResultCoordinator.ResultReady))[0];
            ...
            private readonly IDictionary<
                ParameterExpression,
                int
            > entityTypeIdentifyingExpressionOffsets = new Dictionary<ParameterExpression, int>();
            ...
            )
            {
                ...
                _resultContextParameter = Expression.Parameter(
                    typeof(ResultContext),
                    "resultContext"
                );
                _indexMapParameter = indexMap
                    ? Expression.Parameter(typeof(int[]), "indexMap")
                    : null;
            ...
            )
            {
                ...
                else
                {
                    _valuesArrayExpression = Expression.MakeMemberAccess(
                        _resultContextParameter,
                        _resultContextValuesMemberInfo
                    );
                            ...
                            Expression.Condition(
                                Expression.IsTrue(
                                    Expression.MakeMemberAccess(
                                        _resultCoordinatorParameter,
                                        _singleQueryResultCoordinatorResultReadyMemberInfo
                                    )
                                ),
                                result,
                ...
                switch (extensionExpression)
                {
                    ...
                    case ProjectionBindingExpression projectionBindingExpression when !_inline:
                    {
                        ...
                        if (_containsCollectionMaterialization)
                        {
                            ...
                            {
                                expressionToAdd = Expression.Convert(
                                    expressionToAdd,
                                    typeof(object)
                                );
                            }
                    ...
                    case IncludeExpression includeExpression:
                    {
                        ...
                        )
                        {
                            ...
                            var collectionIdConstant = Expression.Constant(
                                relationalCollectionShaperExpression.CollectionId
                            );

                            _includeExpressions.Add(
                                Expression.Call(
                                    _initializeIncludeCollectionMethodInfo.MakeGenericMethod(
                                        entityType,
                                        includingEntityType
                                    ),
...
#pragma warning restore EF1001 // Internal EF Core API usage.
                        ...
                        )
                        {
                            ...
                            var collectionIdConstant = Expression.Constant(
                                relationalSplitCollectionShaperExpression.CollectionId
                            );

                            _includeExpressions.Add(
                                Expression.Call(
                                    _initializeSplitIncludeCollectionMethodInfo.MakeGenericMethod(
                                        entityType,
                                        includingEntityType
                                    ),
                                    collectionIdConstant,
...
#pragma warning restore EF1001 // Internal EF Core API usage.
                        ...
                        {
                            var navigationExpression = Visit(
                                includeExpression.NavigationExpression
                            );
                            var entityType = entity.Type;
                            var navigation = includeExpression.Navigation;
                            ...
                            var updatedExpression = Expression.Call(
                                _includeReferenceMethodInfo.MakeGenericMethod(
                                    entityType,
                                    includingType,
                                    relatedEntityType
                                ),
                        ...
                        if (!_variableShaperMapping.TryGetValue(key, out var accessor))
                        {
                            ...
                            _expressions.Add(
                                Expression.Assign(
                                    collectionParameter,
                                    Expression.Call(
                                        _initializeCollectionMethodInfo.MakeGenericMethod(
                                            elementType,
                                            collectionType
                                        ),
                                        collectionIdConstant,
                        ...
                        if (!_variableShaperMapping.TryGetValue(key, out var accessor))
                        {
                            ...
                            _expressions.Add(
                                Expression.Assign(
                                    collectionParameter,
                                    Expression.Call(
                                        _initializeSplitCollectionMethodInfo.MakeGenericMethod(
                                            elementType,
                                            collectionType
                                        ),
                                        collectionIdConstant,
                ...
                where TNavigationEntity : class, TParent
            {
                ...
                if (entity is TNavigationEntity)
                {
                    ...
                    collection = clrCollectionAccessor.GetOrCreate(
                        entity,
                ...
                where TIncludedEntity : class
            {
                ...
                )
                {
                    ...
                    var dataReader = executionStrategy.Execute(
                        ...
                        ((RelationalQueryContext, RelationalCommandCache, bool) tup) =>
                            InitializeReader(tup.Item1, tup.Item2, tup.Item3),
