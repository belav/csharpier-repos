...
public class RelationalProjectionBindingExpressionVisitor : ExpressionVisitor
{
    ...
    public virtual Expression Translate(SelectExpression selectExpression, Expression expression)
    {
        ...
        if (result == QueryCompilationContext.NotTranslatedExpression)
        {
            ...
            _entityProjectionCache =
                new Dictionary<EntityProjectionExpression, ProjectionBindingExpression>();
    ...
    public override Expression? Visit(Expression? expression)
    {
        ...
        if (
            ...
            is not NewExpression
                ...
                and not EntityShaperExpression
                and not IncludeExpression
        )
        {
                ...
                switch (expression)
                {
                    ...
                    case MaterializeCollectionNavigationExpression materializeCollectionNavigationExpression:
                        ...
                        )
                        {
                            ...
                            if (
                                subquery is MethodCallExpression methodCallSubquery
        ...
        switch (extensionExpression)
        {
            case EntityShaperExpression entityShaperExpression:
            {
                ...
                )
                {
                    ...
                    {
                        if (
                            !_jsonQueryCache!.TryGetValue(
                                jsonQueryExpression,
                                out var jsonProjectionBinding
                            )
                        )
                        {
                            jsonProjectionBinding = AddClientProjection(
                                jsonQueryExpression,
                                typeof(ValueBuffer)
                            );
