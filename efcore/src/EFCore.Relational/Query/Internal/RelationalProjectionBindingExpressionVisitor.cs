    ...
    public class RelationalProjectionBindingExpressionVisitor : ExpressionVisitor
    {
                ...
                if (_clientEval)
                {
                    switch (expression)
                    {
                        ...
                        case ProjectionBindingExpression projectionBindingExpression:
                            ...
                            {
                                var newIndex = _selectExpression.AddToProjection(
                                    _existingProjections[index]
                                );
                            ...
                            if (projectionBindingExpression.ProjectionMember != null)
                            {
                                ...
                                return new ProjectionBindingExpression(
                                    _selectExpression,
                                    _selectExpression.AddToProjection(mappedProjection),
                                    expression.Type
                                );
                            }
                        ...
                        case MethodCallExpression methodCallExpression:
                        {
                            ...
                            else
                            {
                                ...
                                if (subquery != null)
                                {
                                    ...
                                    if (
                                        !(
                                            subquery.ShaperExpression is ProjectionBindingExpression
                                            || (
                                                subquery.ShaperExpression
                                                    is UnaryExpression unaryExpression
                                                && unaryExpression.NodeType
                                                    == ExpressionType.Convert
                                                && unaryExpression.Type.MakeNullable()
                                                    == unaryExpression.Operand.Type
                                                && unaryExpression.Operand
                                                    is ProjectionBindingExpression
                                            )
                                            || IsAggregateResultWithCustomShaper(
                    ...
                    if (
                        ...
                        if (projectionBindingExpression.ProjectionMember == null)
                        {
                            if (_clientEval)
                            {
                                ...
                                {
                                    indexMap[item.Key] = _selectExpression.AddToProjection(
                                        _existingProjections[item.Value]
                                    );
                                }

                                return entityShaperExpression.Update(
                                    new ProjectionBindingExpression(_selectExpression, indexMap)
                                );
                            }
        ...
        protected override Expression VisitMemberInit(MemberInitExpression memberInitExpression)
        {
            ...
            for (var i = 0; i < newBindings.Length; i++)
            {
                ...
                if (
                    newBindings[i] is MemberAssignment memberAssignment
