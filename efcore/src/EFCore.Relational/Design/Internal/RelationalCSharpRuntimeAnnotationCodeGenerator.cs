...
public class RelationalCSharpRuntimeAnnotationCodeGenerator : CSharpRuntimeAnnotationCodeGenerator
{
    ...
    )
    {
        ...
        if (parameters.IsRuntime)
        {
            ...
            )
            {
                ...
                {
                    parameters.MethodBuilder.AppendLine().AppendLines(methods);
                }
            }
        }
        else
        {
            ...
            )
            {
                ...
                var functionsVariable = Dependencies.CSharpHelper
                    .Identifier("functions", parameters.ScopeVariables, capitalize: false);
                parameters.MainBuilder
                    .Append("var ")
                    .Append(functionsVariable)
                    .AppendLine(" = new Dictionary<string, IDbFunction>();");
    ...
    )
    {
        ...
        using (mainBuilder.Indent())
        {
            ...
            mainBuilder.AppendLine(
                $"var {relationalModelVariable} = new RelationalModel({parameters.TargetName});"
            );
            ...
            CreateAnnotations(model, Generate, relationalModelParameters);
        ...
        )
        {
            ...
            var typeBaseVariable = code.Identifier(
                typeBase.ShortName(),
            ...
            if (typeBase is IComplexType complexType)
            {
                parameters.MainBuilder
                    ...
                    .AppendLine(
                        $"{declaringVariable}.FindComplexProperty({code.Literal(complexType.ComplexProperty.Name)})!.ComplexType;"
                    );
            }
            else
            {
                parameters.MainBuilder
                    .AppendLine()
                    .AppendLine(
                        $"var {typeBaseVariable} = FindEntityType({code.Literal(typeBase.Name)})!;"
                    );
            }
            ...
            foreach (var mapping in typeBase.GetDefaultMappings())
            {
                ...
                mainBuilder
                    .AppendLine()
                    .AppendLine(
                        $"var {tableMappingsVariable} = new List<TableMappingBase<ColumnMappingBase>>();"
                    )
                    .Append($"{typeBaseVariable}.SetRuntimeAnnotation(")
                    .AppendLine(
                        $"{code.Literal(RelationalAnnotationNames.DefaultMappings)}, {tableMappingsVariable});"
                    );
            ...
            if (typeBase.GetTableMappings().Any())
            {
                ...
                mainBuilder
                    ...
                    .Append($"{typeBaseVariable}.SetRuntimeAnnotation(")
                    .AppendLine(
                        $"{code.Literal(RelationalAnnotationNames.TableMappings)}, {tableMappingsVariable});"
                    );
            ...
            {
                var viewMappingsVariable = code.Identifier(
                    "viewMappings",
                    parameters.ScopeVariables,
                    capitalize: false
                );
                mainBuilder
                    ...
                    .Append($"{typeBaseVariable}.SetRuntimeAnnotation(")
                    .AppendLine(
                        $"{code.Literal(RelationalAnnotationNames.ViewMappings)}, {viewMappingsVariable});"
                    );
            ...
            if (typeBase.GetSqlQueryMappings().Any())
            {
                ...
                mainBuilder
                    ...
                    .Append($"{typeBaseVariable}.SetRuntimeAnnotation(")
                    .AppendLine(
                        $"{code.Literal(RelationalAnnotationNames.SqlQueryMappings)}, {sqlQueryMappingsVariable});"
                    );
            ...
            if (typeBase.GetFunctionMappings().Any())
            {
                ...
                mainBuilder
                    ...
                    .Append($"{typeBaseVariable}.SetRuntimeAnnotation(")
                    .AppendLine(
                        $"{code.Literal(RelationalAnnotationNames.FunctionMappings)}, {functionMappingsVariable});"
                    );
            ...
            if (typeBase.GetDeleteStoredProcedureMappings().Any())
            {
                ...
                mainBuilder
                    .AppendLine()
                    .AppendLine(
                        $"var {deleteSprocMappingsVariable} = new List<StoredProcedureMapping>();"
                    )
                    .Append($"{typeBaseVariable}.SetRuntimeAnnotation(")
                    .AppendLine(
            ...
            if (typeBase.GetInsertStoredProcedureMappings().Any())
            {
                ...
                mainBuilder
                    .AppendLine()
                    .AppendLine(
                        $"var {insertSprocMappingsVariable} = new List<StoredProcedureMapping>();"
                    )
                    .Append($"{typeBaseVariable}.SetRuntimeAnnotation(")
                    .AppendLine(
            ...
            if (typeBase.GetUpdateStoredProcedureMappings().Any())
            {
                ...
                mainBuilder
                    .AppendLine()
                    .AppendLine(
                        $"var {updateSprocMappingsVariable} = new List<StoredProcedureMapping>();"
                    )
                    .Append($"{typeBaseVariable}.SetRuntimeAnnotation(")
                    .AppendLine(
    ...
    )
    {
        ...
        mainBuilder
            .Append(
                $"var {tableVariable} = new TableBase({code.Literal(table.Name)}, {code.Literal(table.Schema)}, "
            )
        ...
        CreateAnnotations(table, Generate, tableParameters);

        mainBuilder.AppendLine(
            $"{parameters.TargetName}.DefaultTables.Add({code.Literal(table.Name)}, {tableVariable});"
        );
    ...
    )
    {
        ...
        mainBuilder
            .Append(
                $"var {tableVariable} = new Table({code.Literal(table.Name)}, {code.Literal(table.Schema)}, "
            )
            .AppendLine($"{parameters.TargetName});");
        ...
        foreach (var uniqueConstraint in table.UniqueConstraints)
        {
            Create(
                uniqueConstraint,
                uniqueConstraint.Columns.Select(c => metadataVariables[c]),
                tableParameters
            );
        }
        ...
        mainBuilder
            ...
            .AppendLine(
                $"{code.Literal(table.Name)}, {code.Literal(table.Schema)}), {tableVariable});"
            );
    ...
    )
    {
        ...
        mainBuilder
            .Append(
                $"var {viewVariable} = new View({code.Literal(view.Name)}, {code.Literal(view.Schema)}, "
            )
            .AppendLine($"{parameters.TargetName});");
        ...
        CreateAnnotations(view, Generate, viewParameters);

        mainBuilder
            .Append($"{parameters.TargetName}.Views.Add((")
            .AppendLine(
                $"{code.Literal(view.Name)}, {code.Literal(view.Schema)}), {viewVariable});"
            );
    ...
    )
    {
        ...
        var code = Dependencies.CSharpHelper;
        sqlQueryVariable = code.Identifier(
        ...
        mainBuilder
            .Append(
                $"var {sqlQueryVariable} = new SqlQuery({code.Literal(sqlQuery.Name)}, {parameters.TargetName}, "
            )
        ...
        CreateAnnotations(sqlQuery, Generate, sqlQueryParameters);
    ...
    )
    {
        ...
        var code = Dependencies.CSharpHelper;
        var mainDbFunctionVariable = GetOrCreate(
            function.DbFunctions.First(),
            metadataVariables,
            parameters
        );
        functionVariable = code.Identifier(
        ...
        var mainBuilder = parameters.MainBuilder;
        mainBuilder.AppendLine(
            $"var {functionVariable} = new StoreFunction({mainDbFunctionVariable}, {parameters.TargetName});"
        );
        ...
        foreach (var dbFunction in function.DbFunctions.Skip(1))
        {
            ...
            mainBuilder
                ...
                .AppendLine(
                    $"{functionVariable}.DbFunctions.Add({code.Literal(dbFunction.ModelName)}, {dbFunctionVariable});"
                );
        }
        ...
        foreach (var parameter in function.Parameters)
        {
            ...
            mainBuilder.AppendLine(
                $"var {parameterVariable} = {functionVariable}.FindParameter({code.Literal(parameter.Name)})!;"
            );
        ...
        CreateAnnotations(function, Generate, functionParameters);
        ...
        mainBuilder
            ...
            .AppendLine(
                $"{code.Literal(function.DbFunctions.First().Parameters.Select(p => p.StoreType).ToArray())}),"
            )
    ...
    )
    {
        ...
        var mainBuilder = parameters.MainBuilder;
        mainBuilder.AppendLine(
            $"var {functionVariable} = (IRuntimeDbFunction)this.FindDbFunction({code.Literal(function.ModelName)})!;"
        );
    ...
    )
    {
        ...
        storedProcedureVariable = code.Identifier(
            storeStoredProcedure.Name + "StoreSproc",
            parameters.ScopeVariables,
            capitalize: false
        );
        ...
        mainBuilder
            ...
            .Append(
                $"{code.Literal(storeStoredProcedure.Name)}, {code.Literal(storeStoredProcedure.Schema)}"
            )
            .AppendLine($", {parameters.TargetName});");
        ...
        if (returnValue != null)
        {
            mainBuilder
                .Append(
                    $"{storedProcedureVariable}.ReturnValue = new StoreStoredProcedureReturnValue("
                )
                .AppendLine(
                    $"\"\", {code.Literal(returnValue.StoreType)}, {storedProcedureVariable});"
                );
        }
        ...
        CreateAnnotations(storeStoredProcedure, Generate, sprocParameters);
    ...
    /// <param name="parameters">Additional parameters used during code generation.</param>
    public virtual void Generate(
        IStoreStoredProcedure storedProcedure,
        CSharpRuntimeAnnotationCodeGeneratorParameters parameters
    ) => GenerateSimpleAnnotations(parameters);
    ...
    {
        var code = Dependencies.CSharpHelper;
        var columnVariable = code.Identifier(
            column.Name + "ColumnBase",
            parameters.ScopeVariables,
            capitalize: false
        );
        ...
        var mainBuilder = parameters.MainBuilder;
        var columnType =
            column is JsonColumnBase ? "JsonColumnBase" : "ColumnBase<ColumnMappingBase>";
        mainBuilder
            .Append($"var {columnVariable} = new {columnType}(")
            .Append(
                $"{code.Literal(column.Name)}, {code.Literal(column.StoreType)}, {parameters.TargetName})"
            );
        GenerateIsNullableInitializer(column.IsNullable, mainBuilder, code)
            .AppendLine(";")
            .AppendLine(
                $"{parameters.TargetName}.Columns.Add({code.Literal(column.Name)}, {columnVariable});"
            );

        CreateAnnotations(column, Generate, parameters with { TargetName = columnVariable });
    }
    ...
    )
    {
        ...
        mainBuilder
            .Append(
                $"var {columnVariable} = new StoreStoredProcedureResultColumn({code.Literal(column.Name)}, "
            )
            .Append(
                $"{code.Literal(column.StoreType)}, {code.Literal(column.Position)}, {parameters.TargetName})"
            );
        GenerateIsNullableInitializer(column.IsNullable, mainBuilder, code)
            .AppendLine(";")
    ...
    )
    {
        ...
        mainBuilder
            .Append(
                $"var {parameterVariable} = new StoreStoredProcedureParameter({code.Literal(parameter.Name)}, "
            )
            .Append(
                $"{code.Literal(parameter.StoreType)}, {code.Literal(parameter.Position)}, {parameters.TargetName}"
            )
    ...
    {
        var code = Dependencies.CSharpHelper;
        var uniqueConstraintVariable = code.Identifier(
        ...
        mainBuilder
            .Append("var ")
            .Append(uniqueConstraintVariable)
            .Append(" = new ")
        ...
        {
            var keyVariable = code.Identifier(
                uniqueConstraintVariable + "Uc",
                parameters.ScopeVariables,
                capitalize: false
            );

            mainBuilder
                .AppendLine($"var {keyVariable} = RelationalModel.GetKey(this,")
                .IncrementIndent()
                .AppendLine($"{code.Literal(mappedForeignKey.DeclaringEntityType.Name)},")
                .AppendLine(
                    $"{code.Literal(mappedForeignKey.Properties.Select(p => p.Name).ToArray())});"
                )
                .DecrementIndent();

            mainBuilder.AppendLine($"{uniqueConstraintVariable}.MappedKeys.Add({keyVariable});");
            mainBuilder.AppendLine(
                $"RelationalModel.GetOrCreateUniqueConstraints({keyVariable}).Add({uniqueConstraintVariable});"
            );
        }

        mainBuilder
            .Append(
                $"{parameters.TargetName}.UniqueConstraints.Add({code.Literal(uniqueConstraint.Name)}, "
            )
    ...
    {
        var code = Dependencies.CSharpHelper;
        var indexVariable = code.Identifier(
            index.Name,
            parameters.ScopeVariables,
            capitalize: false
        );
        ...
        mainBuilder
            ...
            .AppendJoin(columns)
            .Append(" }, ")
            .Append(code.Literal(index.IsUnique))
            .AppendLine(");");

        CreateAnnotations(index, Generate, parameters with { TargetName = indexVariable });

        foreach (var mappedIndex in index.MappedIndexes)
        {
            var tableIndexVariable = code.Identifier(
                indexVariable + "Ix",
                parameters.ScopeVariables,
                capitalize: false
            );
            ...
            mainBuilder
                .AppendLine($"var {tableIndexVariable} = RelationalModel.GetIndex(this,")
                .IncrementIndent()
            ...
            mainBuilder.AppendLine(
                $"RelationalModel.GetOrCreateTableIndexes({tableIndexVariable}).Add({indexVariable});"
            );
        }

        mainBuilder.AppendLine(
            $"{parameters.TargetName}.Indexes.Add({code.Literal(index.Name)}, {indexVariable});"
        );
    }
    ...
    )
    {
        ...
        var foreignKeyConstraintVariable = code.Identifier(
            foreignKey.Name,
            parameters.ScopeVariables,
            capitalize: false
        );
        ...
        var mainBuilder = parameters.MainBuilder;
        mainBuilder
            .AppendLine($"var {foreignKeyConstraintVariable} = new ForeignKeyConstraint(")
            .IncrementIndent()
            .AppendLine(
                $"{code.Literal(foreignKey.Name)}, {parameters.TargetName}, {principalTableVariable},"
            )
            .Append("new[] { ")
            .AppendJoin(foreignKey.Columns.Select(c => metadataVariables[c]))
            .AppendLine(" },")
            .Append(
                $"{principalTableVariable}.FindUniqueConstraint({code.Literal(foreignKey.PrincipalUniqueConstraint.Name)})!, "
            )
            .Append(code.Literal(foreignKey.OnDeleteAction))
            .AppendLine(");")
            .DecrementIndent();
        ...
        {
            var foreignKeyVariable = code.Identifier(
                foreignKeyConstraintVariable + "Fk",
                parameters.ScopeVariables,
                capitalize: false
            );

            mainBuilder
                .AppendLine($"var {foreignKeyVariable} = RelationalModel.GetForeignKey(this,")
                .IncrementIndent()
                .AppendLine($"{code.Literal(mappedForeignKey.DeclaringEntityType.Name)},")
                .AppendLine(
                    $"{code.Literal(mappedForeignKey.Properties.Select(p => p.Name).ToArray())},"
                ...
                .AppendLine(
                    $"{code.Literal(mappedForeignKey.PrincipalKey.Properties.Select(p => p.Name).ToArray())});"
                )
                .DecrementIndent();

            mainBuilder.AppendLine(
                $"{foreignKeyConstraintVariable}.MappedForeignKeys.Add({foreignKeyVariable});"
            );
            mainBuilder.AppendLine(
        ...
        mainBuilder
            .AppendLine(
                $"{metadataVariables[foreignKey.Table]}.ForeignKeyConstraints.Add({foreignKeyConstraintVariable});"
            )
            .AppendLine(
                $"{principalTableVariable}.ReferencingForeignKeyConstraints.Add({foreignKeyConstraintVariable});"
            );
    }
    ...
    )
    {
        ...
        var tableMappingVariable = code.Identifier(
            table.Name + "MappingBase",
            parameters.ScopeVariables,
            capitalize: false
        );
        ...
        foreach (var columnMapping in tableMapping.ColumnMappings)
        {
            mainBuilder
                ...
                .Append(
                    $"(ColumnBase<ColumnMappingBase>){metadataVariables[columnMapping.Column]}, "
                )
                .Append(
                    $"{typeBaseVariable}.FindProperty({code.Literal(columnMapping.Property.Name)})!, "
                )
    ...
    )
    {
        ...
        foreach (var columnMapping in tableMapping.ColumnMappings)
        {
            mainBuilder
                .Append(
                    $"RelationalModel.CreateColumnMapping({metadataVariables[columnMapping.Column]}, "
                )
                .Append(
                    $"{typeBaseVariable}.FindProperty({code.Literal(columnMapping.Property.Name)})!, "
                )
    ...
    )
    {
        ...
        var viewMappingVariable = code.Identifier(
            view.Name + "ViewMapping",
            parameters.ScopeVariables,
            capitalize: false
        );
        ...
        foreach (var columnMapping in viewMapping.ColumnMappings)
        {
            mainBuilder
                .Append(
                    $"RelationalModel.CreateViewColumnMapping({metadataVariables[columnMapping.Column]}, "
                )
                .Append(
                    $"{typeBaseVariable}.FindProperty({code.Literal(columnMapping.Property.Name)})!, "
                )
                .Append(viewMappingVariable)
                .AppendLine(");");
        }
    }
    ...
    )
    {
        ...
        {
            mainBuilder.AppendLine(
                $"{sqlQueryMappingVariable}.IsDefaultSqlQueryMapping = {code.Literal(true)};"
            );
        }
        ...
        foreach (var columnMapping in sqlQueryMapping.ColumnMappings)
        {
            mainBuilder
                .Append(
                    $"RelationalModel.CreateSqlQueryColumnMapping({metadataVariables[columnMapping.Column]}, "
                )
                .Append(
                    $"{typeBaseVariable}.FindProperty({code.Literal(columnMapping.Property.Name)})!, "
                )
    ...
    )
    {
        ...
        var functionMappingVariable = code.Identifier(
            storeFunction.Name + "FunctionMapping",
            parameters.ScopeVariables,
            capitalize: false
        );
        ...
        {
            mainBuilder.AppendLine(
                $"{functionMappingVariable}.IsDefaultFunctionMapping = {code.Literal(true)};"
            );
        }
        ...
        foreach (var columnMapping in functionMapping.ColumnMappings)
        {
            mainBuilder
                .Append(
                    $"RelationalModel.CreateFunctionColumnMapping({metadataVariables[columnMapping.Column]}, "
                )
                .Append(
                    $"{typeBaseVariable}.FindProperty({code.Literal(columnMapping.Property.Name)})!, "
                )
                .Append(functionMappingVariable)
                .AppendLine(");");
        }
    }
    ...
    )
    {
        ...
        var sprocVariable = code.Identifier(
            storeSproc.Name + sprocMappingName[0] + "Sproc",
            parameters.ScopeVariables,
            capitalize: false
        );
        mainBuilder.AppendLine(
            $"var {sprocVariable} = {CreateFindSnippet(sprocMapping.StoredProcedure, metadataVariables)};"
        );

        var sprocMappingVariable = code.Identifier(
            storeSproc.Name + "SprocMapping",
            parameters.ScopeVariables,
            capitalize: false
        );
        var tableMappingVariable =
            sprocMapping.TableMapping != null ? metadataVariables[sprocMapping.TableMapping] : null;
        ...
        {
            mainBuilder.AppendLine(
                $"{tableMappingVariable}.{sprocMappingName} = {sprocMappingVariable};"
            );
        }
        ...
        foreach (var parameterMapping in sprocMapping.ParameterMappings)
        {
            mainBuilder
                .Append(
                    $"RelationalModel.CreateStoredProcedureParameterMapping({metadataVariables[parameterMapping.StoreParameter]}, "
                )
                .Append(
                    $"{sprocVariable}.FindParameter({code.Literal(parameterMapping.Parameter.Name)})!, "
                )
                .Append(
                    $"{typeBaseVariable}.FindProperty({code.Literal(parameterMapping.Property.Name)})!, "
                )
                .Append(sprocMappingVariable)
                .AppendLine(");");
        }
        ...
        foreach (var columnMapping in sprocMapping.ResultColumnMappings)
        {
            mainBuilder
                .Append(
                    $"RelationalModel.CreateStoredProcedureResultColumnMapping({metadataVariables[columnMapping.StoreResultColumn]}, "
                )
                .Append(
                    $"{sprocVariable}.FindResultColumn({code.Literal(columnMapping.ResultColumn.Name)})!, "
                )
                .Append(
                    $"{typeBaseVariable}.FindProperty({code.Literal(columnMapping.Property.Name)})!, "
                )
    ...
    )
    {
        ...
        mainBuilder
            ...
            .Append(
                $"{tableVariable}, {additionalParameter ?? ""}{code.Literal(tableMapping.IncludesDerivedTypes)}"
            );

        if (
            tableMapping.IsSharedTablePrincipal.HasValue
            || tableMapping.IsSplitEntityTypePrincipal.HasValue
        )
        {
            mainBuilder.AppendLine(")").AppendLine("{").IncrementIndent();
            ...
            if (tableMapping.IsSplitEntityTypePrincipal.HasValue)
            {
                mainBuilder
                    ...
                    .AppendLine(code.Literal(tableMapping.IsSplitEntityTypePrincipal));
            }
        ...
        mainBuilder
            .AppendLine(
                $"{tableVariable}.AddTypeMapping({tableMappingVariable}, {code.Literal(isOptional)});"
            )
        ...
        if (typeBase is IEntityType entityType)
        {
            foreach (var internalForeignKey in table.GetRowInternalForeignKeys(entityType))
            {
                mainBuilder
                    ...
                    .Append($".AddRowInternalForeignKey({entityTypeVariable}, ")
                    .AppendLine("RelationalModel.GetForeignKey(this,")
                    .IncrementIndent()
                    .AppendLine($"{code.Literal(internalForeignKey.DeclaringEntityType.Name)},")
                    .AppendLine(
                        $"{code.Literal(internalForeignKey.Properties.Select(p => p.Name).ToArray())},"
                    )
                    .AppendLine($"{code.Literal(internalForeignKey.PrincipalEntityType.Name)},")
                    .AppendLine(
                        $"{code.Literal(internalForeignKey.PrincipalKey.Properties.Select(p => p.Name).ToArray())}));"
                    )
                    .DecrementIndent();
            }
        }
    ...
    )
    {
        ...
        if (function.MethodInfo != null)
        {
            ...
            mainBuilder
                .AppendLine(",")
                .AppendLine($"methodInfo: {code.Literal(method.DeclaringType!)}.GetMethod(")
                .IncrementIndent()
        ...
        CreateAnnotations(function, Generate, parameters);
    ...
    )
    {
        ...
        {
            mainBuilder.AppendLine(",").Append("modelSchemaIsNull: ").Append(code.Literal(true));
        }
    ...
    )
    {
        ...
        else
        {
            ...
            {
                AddNamespace(
                    typeof(StoreObjectDictionary<RuntimeEntityTypeMappingFragment>),
                    parameters.Namespaces
                );
                AddNamespace(typeof(StoreObjectIdentifier), parameters.Namespaces);
                var fragmentsVariable = Dependencies.CSharpHelper
            ...
            {
                var sprocVariable = Dependencies.CSharpHelper
    ...
    )
    {
        ...
        mainBuilder
            ...
            .AppendLine(" = new RuntimeEntityTypeMappingFragment(")
            .IncrementIndent()
            .Append(parameters.TargetName)
            .AppendLine(",");
        ...
        CreateAnnotations(fragment, Generate, parameters with { TargetName = overrideVariable });
    ...
    )
    {
        ...
        mainBuilder
            ...
            .AppendLine(" = new RuntimeStoredProcedure(")
            .IncrementIndent()
            .Append(parameters.TargetName)
            .AppendLine(",")
            .Append(code.Literal(storedProcedure.Name))
            .AppendLine(",")
            .Append(code.Literal(storedProcedure.Schema))
            .AppendLine(",")
        ...
        CreateAnnotations(storedProcedure, Generate, parameters);
    }
    ...
    )
    {
        ...
        var mainBuilder = parameters.MainBuilder;
        var parameterVariable = code.Identifier(
            parameter.PropertyName ?? parameter.Name,
            parameters.ScopeVariables,
            capitalize: false
        );
        ...
        mainBuilder
            ...
            .Append(code.Literal(parameter.Direction, fullName: true))
            .Append(", ")
            .Append(code.Literal(parameter.ForRowsAffected))
            .Append(", ")
            .Append(code.Literal(parameter.PropertyName!))
            .Append(", ")
    ...
    /// <param name="parameters">Additional parameters used during code generation.</param>
    public virtual void Generate(
        IStoredProcedureParameter storedProcedure,
        CSharpRuntimeAnnotationCodeGeneratorParameters parameters
    ) => GenerateSimpleAnnotations(parameters);
    ...
    )
    {
        var code = Dependencies.CSharpHelper;
        var mainBuilder = parameters.MainBuilder;
        var resultColumnVariable = code.Identifier(
            resultColumn.Name,
            parameters.ScopeVariables,
            capitalize: false
        );
        ...
        mainBuilder
            .Append("var ")
            .Append(resultColumnVariable)
            .Append(" = ")
            .Append(parameters.TargetName)
            .AppendLine(".AddResultColumn(")
            .IncrementIndent()
            .Append(code.Literal(resultColumn.Name))
            .Append(", ")
            .Append(code.Literal(resultColumn.ForRowsAffected))
            .Append(", ")
    ...
    /// <param name="parameters">Additional parameters used during code generation.</param>
    public virtual void Generate(
        IStoredProcedureResultColumn storedProcedure,
        CSharpRuntimeAnnotationCodeGeneratorParameters parameters
    ) => GenerateSimpleAnnotations(parameters);
    ...
    )
    {
        ...
        else
        {
            ...
            {
                AddNamespace(
                    typeof(StoreObjectDictionary<RuntimeRelationalPropertyOverrides>),
                    parameters.Namespaces
                );
                AddNamespace(typeof(StoreObjectIdentifier), parameters.Namespaces);
                var overridesVariable = Dependencies.CSharpHelper
    ...
    )
    {
        ...
        CreateAnnotations(overrides, Generate, parameters with { TargetName = overrideVariable });
    ...
    )
    {
        ...
        var defaultInstance = (RelationalTypeMapping?)CreateDefaultTypeMapping(
        ...
        Create(
            providerValueComparer ?? relationalTypeMapping.ProviderValueComparer,
            parameters,
            code
        );
        ...
        var sizeDifferent =
            relationalTypeMapping.Size != null
            && relationalTypeMapping.Size != defaultInstance.Size;
        var precisionDifferent =
            relationalTypeMapping.Precision != null
            && relationalTypeMapping.Precision != defaultInstance.Precision;
        var scaleDifferent =
            relationalTypeMapping.Scale != null
            && relationalTypeMapping.Scale != defaultInstance.Scale;
        var dbTypeDifferent =
            relationalTypeMapping.DbType != null
        ...
        )
        {
            ...
            if (storeTypeDifferent)
            {
                GenerateArgument(
                    "storeTypeName",
                    code.Literal(relationalTypeMapping.StoreType),
                    mainBuilder,
                    ref firstParameter
                );
            }
            ...
            if (sizeDifferent)
            {
                GenerateArgument(
                    "size",
                    code.Literal(relationalTypeMapping.Size),
                    mainBuilder,
                    ref firstParameter
                );
            }
            ...
            if (precisionDifferent)
            {
                GenerateArgument(
                    "precision",
                    code.Literal(relationalTypeMapping.Precision),
                    mainBuilder,
                    ref firstParameter
                );
            }
            ...
            if (scaleDifferent)
            {
                GenerateArgument(
                    "scale",
                    code.Literal(relationalTypeMapping.Scale),
                    mainBuilder,
                    ref firstParameter
                );
            }
            ...
            if (dbTypeDifferent)
            {
                GenerateArgument(
                    "dbType",
                    code.Literal(relationalTypeMapping.DbType!, fullName: true),
                    mainBuilder,
                    ref firstParameter
                );
            }
        ...
        if (
            relationalTypeMapping.Converter != null
        ...
        var storeTypePostfixDifferent =
            relationalTypeMapping.StoreTypePostfix != defaultInstance.StoreTypePostfix;
        if (storeTypePostfixDifferent)
        {
            mainBuilder
                .AppendLine(",")
                .Append(
                    $"storeTypePostfix: {code.Literal(relationalTypeMapping.StoreTypePostfix)}"
                );
        }

        if (
            relationalTypeMapping.JsonValueReaderWriter != null
        ...
        )
        {
            ...
            CreateJsonValueReaderWriter(
                relationalTypeMapping.JsonValueReaderWriter,
                parameters,
                code
            );
        }
        ...
        static bool IsSpatialType(Type relationalTypeMappingType) =>
            (
                ...
                && relationalTypeMappingType.GetGenericTypeDefinition()
                    == typeof(RelationalGeometryTypeMapping<,>)
            )
            || (
    ...
    )
    {
        ...
        switch (storeObject.StoreObjectType)
        {
            ...
            case StoreObjectType.SqlQuery:
                builder.Append("SqlQuery(").Append(code.Literal(storeObject.Name)).Append(")");
                break;
            case StoreObjectType.Function:
                builder.Append("DbFunction(").Append(code.Literal(storeObject.Name)).Append(")");
                break;
