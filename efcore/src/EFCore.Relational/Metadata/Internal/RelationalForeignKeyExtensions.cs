...
public static class RelationalForeignKeyExtensions
{
    ...
    )
    {
        ...
        if (
            ...
            && StoreObjectIdentifier.Create(foreignKey.DeclaringEntityType, StoreObjectType.Table)
                != storeObject
            ...
            new[] { StoreObjectIdentifier.Create(entityType1, StoreObjectType.Table)!.Value }
                .Concat(
                    entityType1
                        ...
                        .Select(f => f.StoreObject)
                )
                .Intersect(
                    new[]
                    {
                        StoreObjectIdentifier.Create(entityType2, StoreObjectType.Table)!.Value
                    }.Concat(
                        entityType2
                            .GetMappingFragments(StoreObjectType.Table)
                            .Select(f => f.StoreObject)
