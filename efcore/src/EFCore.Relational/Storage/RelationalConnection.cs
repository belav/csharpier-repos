...
namespace Microsoft.EntityFrameworkCore.Storage
{
          ...
          ITransactionEnlistmentManager
    {
        ...
        public virtual DbConnection DbConnection
        {
            get
            {
                if (_connection == null && _connectionString == null)
                {
                    throw new InvalidOperationException(
                        RelationalStrings.NoConnectionOrConnectionString
                    );
        ...
        public virtual IDbContextTransaction BeginTransaction(IsolationLevel isolationLevel)
        {
            ...
            var interceptionResult = Dependencies.TransactionLogger
            ...
            dbTransaction = Dependencies.TransactionLogger
                .TransactionStarted(
                    this,
                    dbTransaction,
                    transactionId,
        ...
        private IDbContextTransaction CreateRelationalTransaction(
            ...
            CurrentTransaction = Dependencies.RelationalTransactionFactory
                .Create(
                    this,
                    transaction,
                    transactionId,
        ...
        )
        {
            if (ShouldUseTransaction(transaction))
            {
                ...
                transaction = Dependencies.TransactionLogger
        ...
        )
        {
            ...
            try
            {
                if (logger.ShouldLogConnectionOpen(startTime))
                {
                    ...
                    var interceptionResult = await logger.ConnectionOpeningAsync(
                        this,
                        startTime,
                        cancellationToken
                    )
                    ...
                    await logger.ConnectionOpenedAsync(
                        this,
                        startTime,
                        _stopwatch.Elapsed,
                        cancellationToken
        ...
        private void HandleAmbientTransactions()
        {
            ...
            if (current == null)
            {
                var rootTransaction =
                    ...
                    && _ambientTransactions.TryPeek(out var transaction)
                        ? transaction
                        : null;
        ...
        private void HandleTransactionCompleted(object? sender, TransactionEventArgs e)
        {
            ...
            {
                throw new InvalidOperationException(
                    RelationalStrings.NestedAmbientTransactionError
                );
            }
                ...
                if (DbConnectionState != ConnectionState.Closed)
                {
                    ...
                    try
                    {
                        if (logger.ShouldLogConnectionClose(startTime))
                        {
                            ...
                            var interceptionResult = Dependencies.ConnectionLogger
        ...
        protected virtual void ResetState(bool disposeDbConnection)
        {
            ...
            if (disposeDbConnection && _connectionOwned && _connection is not null)
            {
                DisposeDbConnection();
                _connection = null;
                _openedCount = 0;
        ...
        /// </summary>
        protected virtual void DisposeDbConnection() => DbConnection.Dispose();

        /// <summary>
        ///     Template method that by default calls <see cref="System.Data.Common.DbConnection.DisposeAsync" /> but can be overriden by
        ///     providers to make a different call instead.
        /// </summary>
        protected virtual ValueTask DisposeDbConnectionAsync() => DbConnection.DisposeAsync();
    }
}
