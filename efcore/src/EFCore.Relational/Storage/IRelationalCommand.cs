...
public interface IRelationalCommand : IRelationalCommandTemplate
{
    ...
    // the task to complete.</param>
    /// <returns>
    ///     A task that represents the asynchronous operation. The task result contains the result of
    // the command.
    /// </returns>
    /// <exception cref="OperationCanceledException">If the <see cref="CancellationToken" /> is
    // canceled.</exception>
    ...
    /// </summary>
    /// <param name="commandTemplate">A template command from which the command text and parameters will
    // be copied.</param>
