...
#pragma warning restore EF1001 // Internal EF Core API usage.
        ...
        )
        {
            ...
            if (target == null)
            {
                var sourceMigrationsAnnotationsForRemoved = MigrationsAnnotations
                    .ForRemove(source)
                    .ToList();
        ...
        )
        {
            ...
            foreach (var sourceEntityType in source.Model.GetEntityTypes())
            {
                ...
                {
                    mainSourceEntityType = principalSourceTable.EntityTypeMappings
                        .First(m => m.IsSharedTablePrincipal)
                        .EntityType;
        ...
        private sealed class SharedIdentityMap
        {
            ...
            private IUpdateEntry GetMainEntry(IUpdateEntry entry, ITable table)
            {
                ...
                var mainTable = entry.EntityType
                    .GetTableMappings()
                    .First(m => m.IsSplitEntityTypePrincipal)
                    .Table;
                if (mainTable != table)
                {
