...
public class MigrationsModelDiffer : IMigrationsModelDiffer
{
            ...
            else if (type == typeof(DropColumnOperation))
            {
                if (
                    string.IsNullOrWhiteSpace(
                        diffContext.FindColumn((DropColumnOperation)operation)!.ComputedColumnSql
                    )
                )
                {
    ...
    )
    {
        ...
        if (target == null)
        {
            var sourceMigrationsAnnotationsForRemoved = MigrationsAnnotationProvider
                .ForRemove(source)
                .ToList();
    ...
    protected virtual IEnumerable<MigrationOperation> Diff(
        ...
        DiffContext diffContext
    ) => DiffCollection(source, target, diffContext, Diff, Add, Remove, (s, t, _) => s == t);
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    protected virtual IEnumerable<MigrationOperation> Add(ITable target, DiffContext diffContext)
    {
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    protected virtual IEnumerable<MigrationOperation> Remove(ITable source, DiffContext diffContext)
    {
    ...
    private static IEnumerable<IProperty> GetSortedProperties(IEntityType entityType, ITable table)
    {
        ...
        var unorderedGroups =
            new Dictionary<PropertyInfo, SortedDictionary<(int, string), IProperty>>();
        ...
        if (table.EntityTypeMappings.Any(m => m.TypeBase == entityType))
        {
            ...
            )
            {
                ...
                var linkingNavigationProperty = linkingForeignKey.PrincipalToDependent
                    ?.PropertyInfo;
                var properties = GetSortedProperties(linkingForeignKey.DeclaringEntityType, table)
                    .ToList();
                if (
                    linkingNavigationProperty == null
    ...
    private static void AddNestedComplexProperties(
        ITypeBase typeBase,
        List<IProperty> leastPriorityProperties
    )
    {
    ...
    private sealed class PropertyInfoEqualityComparer : IEqualityComparer<PropertyInfo>
    {
        ...
        public int GetHashCode(PropertyInfo obj) => throw new NotSupportedException();
    }
    ...
    protected virtual IEnumerable<MigrationOperation> Diff(
        ...
        DiffCollection(
            ...
            (s, t, _) =>
                ColumnStructureEquals(s, t) && ColumnAnnotationsEqual(s, t, matchValues: false),
    ...
    )
    {
        ...
        if (source is IEntityType sourceEntityType && target is IEntityType targetEntityType)
        {
            var nextSource = sourceTable
                ?.GetRowInternalForeignKeys(sourceEntityType)
                .FirstOrDefault()
                ?.PrincipalEntityType;
            var nextTarget = targetTable
                ?.GetRowInternalForeignKeys(targetEntityType)
                .FirstOrDefault()
                ?.PrincipalEntityType;
        ...
        if (source is IComplexType sourceComplexType && target is IComplexType targetComplexType)
        {
    ...
    )
    {
        ...
        if (
            ...
            || (
                sourceDefault != DBNull.Value
                && !target.ProviderValueComparer.Equals(sourceDefault, targetDefault)
            )
            || source.Comment != target.Comment
            || source.Collation != target.Collation
        ...
        )
        {
            ...
            if (source.Order != target.Order)
            {
                ...
                {
                    alterColumnOperation.AddAnnotation(
    ...
    private void InitializeColumnHelper(
    ...
    )
    {
        ...
        columnOperation.DefaultValue = inline || isNullable ? null : "{}";
    ...
    )
    {
        ...
        var sourceMigrationsAnnotations = RelationalAnnotationProvider.For(
            source,
            designTime: true
        );
        var targetMigrationsAnnotations = RelationalAnnotationProvider.For(
            target,
            designTime: true
        );
    ...
    protected virtual IEnumerable<MigrationOperation> Add(ISequence target, DiffContext diffContext)
    {
        ...
        yield return Initialize(
            operation,
            target,
            RelationalAnnotationProvider.For(target, designTime: true)
        );
    }
    ...
    )
    {
        ...
        {
            _targetIdentityMaps = new Dictionary<ITable, IRowIdentityMap>(
                TableBaseIdentityComparer.Instance
            );
        }
        else
        {
        ...
        {
            _sourceIdentityMaps = new Dictionary<ITable, IRowIdentityMap>(
                TableBaseIdentityComparer.Instance
            );
        }
        else
    ...
    private void AddSeedData(
        ...
        Dictionary<ITable, IRowIdentityMap> identityMaps,
        EntityState initialState
    )
    {
        var sensitiveLoggingEnabled = CommandBatchPreparerDependencies.LoggingOptions
...
#pragma warning restore EF1001 // Internal EF Core API usage.
            ...
            foreach (var mapping in entityType.GetTableMappings())
            {
                ...
                for (var i = 0; i < key.Length; i++)
                {
                    ...
                    key[i] =
                        valueConverter == null ? value : valueConverter.ConvertToProvider(value);
                }
                ...
                {
                    command = CommandBatchPreparerDependencies.ModificationCommandFactory
                ...
                foreach (var columnMapping in mapping.ColumnMappings)
                {
                    ...
                    var existingColumnModification = command.ColumnModifications
                            ...
                            if (sensitiveLoggingEnabled)
                            {
                                throw new InvalidOperationException(
                                    RelationalStrings.ConflictingSeedValuesSensitive(
                                        ...
                                        Convert.ToString(
                                            existingColumnModification.Value,
...
#pragma warning restore EF1001 // Internal EF Core API usage.
    ...
    )
    {
        ...
        foreach (var targetPair in _targetIdentityMaps)
        {
            ...
            if (!tableMapping.TryGetValue(targetTable, out var sourcePair))
            {
                ...
                if (
                    sourceKey == null
                    || !_sourceIdentityMaps.TryGetValue(
                        foundSourceTable!,
                        out var foundSourceIdentityMap
                    )
                )
                {
                ...
                if (!mappingFound || targetKey.Columns.Count != sourceKey.Columns.Count)
                {
                    tableMapping.Add(targetTable, null);
                    continue;
                }
            ...
            foreach (var targetRow in targetIdentityMap.Rows)
            {
                ...
                {
                    var modification = targetRow.ColumnModifications
                ...
                {
                    if (
                        sourceTable.IsExcludedFromMigrations || targetTable.IsExcludedFromMigrations
                    )
                    {
                ...
                if (sourceTable.IsExcludedFromMigrations || targetTable.IsExcludedFromMigrations)
                {
                    targetRow.EntityState = EntityState.Unchanged;
                ...
                foreach (var targetColumnModification in targetRow.ColumnModifications)
                {
                    ...
                    Check.DebugAssert(
                        targetColumnBase is IColumn,
                        "Non-IColumn columns not allowed"
                    );
                    ...
                    {
                        if (
                            targetProperty.GetAfterSaveBehavior() != PropertySaveBehavior.Save
                    ...
                    if (
                        sourceColumn.ProviderClrType == targetColumn.ProviderClrType
    ...
    protected class DiffContext
    {
        ...
        public virtual IColumn? FindColumn(DropColumnOperation operation) =>
            _removedColumns.TryGetValue(operation, out var source) ? source : null;
    }
}
