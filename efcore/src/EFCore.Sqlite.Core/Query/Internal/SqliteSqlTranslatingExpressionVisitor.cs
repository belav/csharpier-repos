...
public class SqliteSqlTranslatingExpressionVisitor : RelationalSqlTranslatingExpressionVisitor
{
    ...
    private static readonly MethodInfo EscapeLikePatternParameterMethod =
        typeof(SqliteSqlTranslatingExpressionVisitor)
            .GetTypeInfo()
    ...
    > RestrictedBinaryExpressions = new Dictionary<ExpressionType, IReadOnlyCollection<Type>>
    {
    ...
    private static readonly IReadOnlyDictionary<Type, string> ModuloFunctions = new Dictionary<
        Type,
        string
    >
    {
    ...
    protected override Expression VisitUnary(UnaryExpression unaryExpression)
    {
        ...
        if (
            visitedExpression is SqlUnaryExpression { OperatorType: ExpressionType.Negate } sqlUnary
        )
        {
    ...
    protected override Expression VisitBinary(BinaryExpression binaryExpression)
    {
        ...
        if (visitedExpression is SqlBinaryExpression sqlBinary)
        {
            ...
            if (
                RestrictedBinaryExpressions.TryGetValue(
                    sqlBinary.OperatorType,
            ...
            switch (translatedPattern)
            {
                    ...
                    ):
                {
                    ...
                    var escapedPatternParameter = _queryCompilationContext.RegisterRuntimeParameter(
                        $"{patternParameter.Name}_{(startsWith ? "startswith" : "endswith")}",
                        lambda
                    );
