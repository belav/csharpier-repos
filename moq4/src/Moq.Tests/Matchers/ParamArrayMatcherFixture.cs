...
namespace Moq.Tests.Matchers
{
    public class ParamArrayMatcherFixture
    {
        ...
        public void Matches_several_matchers_from_params_array(
            object first,
            object second,
            bool shouldMatch
        )
        {
            ...
            var methodCallExpr = (MethodCallExpression)
                ToExpression<IX>(x => x.Method(It.IsAny<int>(), Capture.In(seconds))).Body;
            ...
            Assert.Equal(
                shouldMatch,
                matcher.Matches(new object[] { first, second }, typeof(object[]))
            );
        }
