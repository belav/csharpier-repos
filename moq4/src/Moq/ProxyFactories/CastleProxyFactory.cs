    ...
    internal sealed class CastleProxyFactory : ProxyFactory
    {
        ...
        )
        {
            ...
            else if (mockType.IsDelegateType())
            {
                ...
                var container = generator.CreateClassProxy(
                    typeof(object),
                    additionalInterfaces,
                    options,
                    new Interceptor(interceptor)
                ...
                return Delegate.CreateDelegate(
                    mockType,
                    container,
                    container.GetType().GetMethod("Invoke")
                );
            }
        ...
        private sealed class Invocation : Moq.Invocation
        {
            ...
            protected internal override object CallBase()
            {
                ...
                if (method.DeclaringType.IsInterface && !method.IsAbstract)
                {
                    ...
                    var mostSpecificOverride = FindMostSpecificOverride(
                        method,
                        this.underlying.Proxy.GetType()
                    );
                    return DynamicInvokeNonVirtually(
                        mostSpecificOverride,
                        this.underlying.Proxy,
                        this.Arguments
                    );
                }
#endif
        ...
        public static MethodInfo FindMostSpecificOverride(MethodInfo declaration, Type proxyType)
        {
            return mostSpecificOverrides.GetOrAdd(
                new Pair<MethodInfo, Type>(declaration, proxyType),
                static key =>
                {
                    ...
                    var genericParameterCount = declaration.IsGenericMethod
                        ? declaration.GetGenericArguments().Length
                        : 0;
                    ...
                    )
                    {
                        ...
                        if (candidateMethod?.GetBaseDefinition() != declaration)
                        {
                            ...
                            candidateMethod = implementedInterface.GetMethod(
                                name.ToString(),
                                genericParameterCount,
        ...
        )
        {
            ...
            //    then construct a delegate for it (see ECMA-335 õII.14.4). This does not work
