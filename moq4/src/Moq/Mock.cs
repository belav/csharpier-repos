    ...
    public abstract partial class Mock : IFluentInterface
    {
        ...
        public static Mock<T> Get<T>(T mocked) where T : class
        {
            ...
            if (mocked is IMocked mockedPlain)
            {
                ...
                var imockedType = mocked.GetType()
                    .GetInterfaces()
        ...
        )
        {
            ...
            Mock.SetupRecursive<MethodCall>(
                ...
                setupLast: (targetMock, _, __) =>
                {
                    ...
                    var temporaryAutoSetupProperties =
                        targetMock.AutoSetupPropertiesDefaultValueProvider == null;
                    if (temporaryAutoSetupProperties)
                    {
                        targetMock.AutoSetupPropertiesDefaultValueProvider =
                            targetMock.DefaultValueProvider;
                    }
                    try
                    {
        ...
        private static TSetup SetupRecursive<TSetup>(
            Mock mock,
            LambdaExpression expression,
            Func<Mock, Expression, InvocationShape, TSetup> setupLast,
