...
namespace System.Web.Http.Cors.Test
{
    public class EnableCorsAttributeTest
    {
        ...
        public async Task GetCorsPolicyAsync_DefaultPolicyValues()
        {
            ...
            CorsPolicy corsPolicy = await enableCors.GetCorsPolicyAsync(
        ...
        [InlineData(
            ...
            new[] { "http://example.com", "http://localhost:8080" }
        )]
        [InlineData(
            "http://example.com, http://localhost:8080",
            new[] { "http://example.com", "http://localhost:8080" }
        ...
        [InlineData(
            "foo",
            "The specified policy origin 'foo' is invalid. It must be correctly formed with the scheme, the host, and optionally, the port."
        )]
        [InlineData(
            "://example.com",
            "The specified policy origin '://example.com' is invalid. It must be correctly formed with the scheme, the host, and optionally, the port."
        )]
        [InlineData(
            "http://example.com/",
            "The specified policy origin 'http://example.com/' is invalid. It cannot end with a forward slash."
        )]
        [InlineData(
            "http://example.com#fragment",
            "The specified policy origin 'http://example.com#fragment' is invalid. It must not contain a path, query, or fragment."
        )]
        [InlineData(
            "http://example.com/path",
            "The specified policy origin 'http://example.com/path' is invalid. It must not contain a path, query, or fragment."
        )]
        [InlineData(
            "http://example.com?query=foo",
            "The specified policy origin 'http://example.com?query=foo' is invalid. It must not contain a path, query, or fragment."
        )]
