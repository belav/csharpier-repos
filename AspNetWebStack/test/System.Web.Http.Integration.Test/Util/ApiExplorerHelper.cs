...
namespace System.Web.Http.ApiExplorer
{
    public static class ApiExplorerHelper
    {
        ...
        )
        {
            ...
            ApiDescription[] sortedDescriptions = apiDescriptions
            ...
            for (int i = 0; i < sortedDescriptions.Length; i++)
            {
                ...
                Assert.Equal(
                    ...
                    matchingDescription.RelativePath,
                    ignoreCase: true
                );
                Assert.Equal(
                    expectedResult.HasRequestFormatters,
                    matchingDescription.SupportedRequestBodyFormatters.Count > 0
                );
                Assert.Equal(
                    expectedResult.HasResponseFormatters,
                    matchingDescription.SupportedResponseFormatters.Count > 0
                );
                Assert.Equal(
                    expectedResult.NumberOfParameters,
                    matchingDescription.ParameterDescriptions.Count
                );
            }
        }
