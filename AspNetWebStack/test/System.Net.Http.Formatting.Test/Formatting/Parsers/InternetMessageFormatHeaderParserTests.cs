    ...
    public class InternetMessageFormatHeaderParserTests
    {
        ...
        public void HeaderParserNullBuffer()
        {
            ...
            InternetMessageFormatHeaderParser parser =
                InternetMessageFormatHeaderParserTests.CreateHeaderParser(128, out headers);
            Assert.NotNull(parser);
            int bytesConsumed = 0;
            Assert.ThrowsArgumentNull(
                () =>
        ...
        public void HeaderParserEmptyBuffer()
        {
            ...
            HttpHeaders headers;
            InternetMessageFormatHeaderParser parser =
                InternetMessageFormatHeaderParserTests.CreateHeaderParser(data.Length, out headers);
            Assert.NotNull(parser);
        ...
        public void HeaderParserSpecializedHeaders()
        {
            Dictionary<string, string> headerData = new Dictionary<string, string>
            {
                ...
                {
                    @"JsonProperties1",
                    @"{ ""SessionId"": ""{2813D4D2-46A9-4F4D-8904-E9BDE3712B70}"", ""MessageId"": ""{24AE31D6-63B8-46F3-9975-A3DAF1B6D3F4}"", ""TimeToLive"" : 80, ""CorrelationId"": ""{896DD5BD-1645-44D7-9E7C-D7F70958ECD6}"", ""SequenceNumber"" : 54321, ""DeliveryCount"" : 4, ""To"" : ""http://contoso.com/path2"", ""ReplyTo"" : ""http://fabrikam.com/path2"",  ""SentTimeUtc"" : ""Sun, 06 Nov 1994 10:49:37 GMT"", ""ScheduledEnqueueTimeUtc"" : ""Sun, 06 Nov 1994 10:49:37 GMT""}"
                },
                {
                    @"P3P",
            ...
            for (var cnt = 1; cnt <= data.Length; cnt++)
            {
                ...
                for (int hCnt = 0; hCnt < headerData.Count; hCnt++)
                {
                    ...
                    Assert.Equal(
                        headerData.Values.ElementAt(hCnt),
                        headers.ElementAt(hCnt).Value.ElementAt(0)
                    );
                }
            }
        }
