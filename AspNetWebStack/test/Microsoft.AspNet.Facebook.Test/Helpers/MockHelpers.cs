...
namespace Microsoft.AspNet.Facebook.Test.Helpers
{
    internal class MockHelpers
    {
        ...
        )
        {
            ...
            controllerContext.Setup(c => c.HttpContext.Items)
                .Returns(new Dictionary<object, object>());
            controllerContext.Setup(c => c.HttpContext.Request.Url)
                .Returns(requestUrl ?? new Uri("http://example.com"));
            controllerContext.Setup(c => c.HttpContext.Request.AppRelativeCurrentExecutionFilePath)
                .Returns("~/");
            controllerContext.Setup(c => c.HttpContext.Request.Form)
                .Returns(requestFormData ?? new NameValueCollection());
            controllerContext.Setup(c => c.HttpContext.Request.QueryString)
                .Returns(requestQueryData ?? new NameValueCollection());
            controllerContext.Setup(c => c.HttpContext.Request.Cookies)
        ...
        public static FacebookClient CreateFacebookClient()
        {
            ...
            client.Setup(c => c.ParseSignedRequest(It.IsAny<string>()))
                .Returns((object)signedRequestParameters);
            client.Setup(c => c.GetLoginUrl(It.IsAny<object>()))
        ...
        )
        {
            ...
            client.Setup(p => p.GetUserPermissions(It.IsAny<string>(), It.IsAny<string>()))
                .Returns(permissionsToReturn);
            client.Setup(p => p.GetUserPermissionsStatus(It.IsAny<string>(), It.IsAny<string>()))
