...
#pragma warning restore 0618
        ...
        public async Task Invoke_CallsNext_IfMessageHandlerReturns404WithNoRouteMatched()
        {
            ...
            nextMock.Setup(middleware => middleware.Invoke(It.IsAny<OwinContext>()))
        ...
        public async Task Invoke_IfTryComputeLengthThrows_SendsEmptyErrorResponse()
        {
            ...
            using (HttpMessageHandler messageHandler = CreateStubMessageHandler(response))
            {
                ...
                using (HttpMessageHandlerAdapter product = CreateProductUnderTest(options))
                {
                    ...
                    int statusCode = 0;
                    owinResponseMock.SetupGet(r => r.Environment)
                        .Returns((IDictionary<string, object>)null);
                    owinResponseMock.SetupSet(r => r.StatusCode = It.IsAny<int>())
                        .Callback<int>(s => statusCode = s);
                    int? contentLength = null;
                    Mock<IHeaderDictionary> headersMock = new Mock<IHeaderDictionary>(
                        MockBehavior.Strict
                    );
                    headersMock.As<IDictionary<string, string[]>>()
        ...
        public async Task Invoke_IfTryComputeLengthThrows_DoesNotEvaluateOutputBufferPolicy()
        {
            ...
            using (HttpMessageHandler messageHandler = CreateStubMessageHandler(response))
            {
                ...
                bufferPolicyMock.Setup(p => p.UseBufferedInputStream(It.IsAny<object>()))
                    .Returns(false);
