...
namespace System.Web.Razor.Test.Editor
{
    ...
    {
        private static readonly TestFile SimpleCSHTMLDocument = TestFile.Create(
            "DesignTime.Simple.cshtml"
        );
        private static readonly TestFile SimpleCSHTMLDocumentGenerated = TestFile.Create(
            "DesignTime.Simple.txt"
        );
        ...
        public void ConstructorRequiresNonNullHost()
        {
            Assert.ThrowsArgumentNull(
                () => new RazorEditorParser(null, TestLinePragmaFileName),
                "host"
            );
        }
        ...
        public void ConstructorRequiresNonNullPhysicalPath()
        {
            Assert.ThrowsArgumentNullOrEmptyString(
                () => new RazorEditorParser(CreateHost(), null),
                "sourceFileName"
            );
        }
        ...
        {
            Assert.ThrowsArgumentNullOrEmptyString(
                () => new RazorEditorParser(CreateHost(), String.Empty),
                "sourceFileName"
            );
        }
        ...
        public void CheckForStructureChangesStartsReparseAndFiresDocumentParseCompletedEventIfNoAdditionalChangesQueued()
        {
            ...
            using (RazorEditorParser parser = CreateClientParser())
            {
                ...
                using (ManualResetEventSlim parseComplete = new ManualResetEventSlim(false))
                {
                    ...
                    string generatedCode = capturedArgs.GeneratorResults
                        .GeneratedCode
        ...
        private TextChange CreateDummyChange()
        {
            return new TextChange(
                ...
                new StringTextBuffer(String.Empty),
                3,
                new StringTextBuffer("foo")
            );
        }

        private static Mock<RazorEditorParser> CreateMockParser()
        {
            return new Mock<RazorEditorParser>(CreateHost(), TestLinePragmaFileName)
            {
                CallBase = true,
            };
        }
