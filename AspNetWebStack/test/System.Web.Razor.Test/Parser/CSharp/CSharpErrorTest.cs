    ...
    public class CSharpErrorTest : CsHtmlCodeParserTestBase
    {
        ...
        public void ParseBlockCapturesWhitespaceToEndOfLineInInvalidUsingStatementAndTreatsAsFileCode()
        {
            ParseBlockTest(
                ...
                new StatementBlock(Factory.Code("using          \r\n").AsStatement())
            );
        }
        ...
        {
            ParseBlockTest(
                "(foo bar" + Environment.NewLine + "baz",
                new ExpressionBlock(
                    Factory.MetaCode("(").Accepts(AcceptedCharacters.None),
        ...
        public void ParseBlockReportsErrorIfFinallyBlockUnterminatedAtEOF()
        {
            ParseBlockTest(
                ...
                new StatementBlock(
                    Factory
                        .Code("try { baz(); } finally { var foo = bar; if(foo != null) { bar(); } ")
                        .AsStatement()
                ),
                new RazorError(
                    String.Format(
        ...
        public void ParseBlockRequiresControlFlowStatementsToHaveBraces()
        {
            ...
            ParseBlockTest(
                ...
                new StatementBlock(
                    ...
                    new MarkupBlock(Factory.Markup("<p>Bar</p> ").Accepts(AcceptedCharacters.None)),
                    Factory.Code("else if(bar) ").AsStatement(),
                    new MarkupBlock(Factory.Markup("<p>Baz</p> ").Accepts(AcceptedCharacters.None)),
                    Factory.Code("else ").AsStatement(),
                    new MarkupBlock(Factory.Markup("<p>Boz</p>").Accepts(AcceptedCharacters.None)),
                    Factory.EmptyCSharp().AsStatement()
                ),
        ...
        public void ParseBlockIncludesUnexpectedCharacterInSingleStatementControlFlowStatementError()
        {
            ParseBlockTest(
                "if(foo)) { var bar = foo; }",
                new StatementBlock(Factory.Code("if(foo)) { var bar = foo; }").AsStatement()),
                new RazorError(
                    String.Format(
                        RazorResources.ParseError_SingleLine_ControlFlowStatements_Not_Allowed,
                        "{",
        ...
        {
            ParseBlockTest(
                "if(foo bar" + Environment.NewLine + "baz",
                new StatementBlock(Factory.Code("if(foo bar\r\n").AsStatement()),
                new RazorError(
                    String.Format(
                        RazorResources.ParseError_Expected_CloseBracket_Before_EOF,
                        "(",
        ...
        {
            ParseBlockTest(
                "foreach(foo bar" + Environment.NewLine + "baz",
                new StatementBlock(Factory.Code("foreach(foo bar\r\n").AsStatement()),
                new RazorError(
                    String.Format(
                        RazorResources.ParseError_Expected_CloseBracket_Before_EOF,
                        "(",
        ...
        {
            ParseBlockTest(
                "do { } while(foo bar" + Environment.NewLine + "baz",
                new StatementBlock(Factory.Code("do { } while(foo bar\r\n").AsStatement()),
                new RazorError(
                    String.Format(
                        RazorResources.ParseError_Expected_CloseBracket_Before_EOF,
                        "(",
        ...
        {
            ParseBlockTest(
                "using(foo bar" + Environment.NewLine + "baz",
                new StatementBlock(Factory.Code("using(foo bar\r\n").AsStatement()),
                new RazorError(
                    String.Format(
                        RazorResources.ParseError_Expected_CloseBracket_Before_EOF,
                        "(",
        ...
        {
            ParseBlockTest(
                "if(" + Environment.NewLine + "else { <p>Foo</p> }",
                new StatementBlock(
                    Factory.Code("if(\r\nelse {").AsStatement(),
                    new MarkupBlock(
        ...
        public void ParseBlockCorrectlyParsesMarkupIncorrectyAssumedToBeWithinAStatement()
        {
            ParseBlockTest(
                ...
                new RazorError(RazorResources.ParseError_Unterminated_String_Literal, 25, 1, 14)
            );
        }
