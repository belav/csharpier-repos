...
namespace System.Web.Http.Controllers
{
    public class ExceptionFilterResultTests
    {
        ...
        public async Task ExecuteAsync_IfInnerResultTaskIsFaulted_ExecutesFiltersAndReturnsResultIfHandled()
        {
            ...
            var globalFilter = CreateExceptionFilter(
                (ec, ct) =>
                {
        ...
        public async Task ExecuteAsync_IfInnerResultTaskIsFaulted_AndNoFilterHandles_RunsHandlerAndReturnsResultIfHandled()
        {
            ...
            using (HttpResponseMessage expectedResponse = CreateResponse())
            {
                ...
                IExceptionFilter filter = CreateExceptionFilter(
                    (c, i) =>
                    {
                        log.Add("filter");
                        return Task.FromResult(0);
        ...
        private static HttpActionContext CreateActionContext(HttpRequestMessage request)
        {
            ...
            actionContext.ControllerContext = new HttpControllerContext() { Request = request };
        ...
        private static ExceptionFilterResult CreateProductUnderTest(
        ...
        {
            Mock<IHttpActionResult> actionResultMock = new Mock<IHttpActionResult>(
                MockBehavior.Strict
            );
            actionResultMock.Setup(r => r.ExecuteAsync(It.IsAny<CancellationToken>()))
                .Returns(
                    () =>
                    {
                        return task;
