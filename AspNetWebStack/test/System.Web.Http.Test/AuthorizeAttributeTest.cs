...
namespace System.Web.Http
{
    public class AuthorizeAttributeTest
    {
        ...
        public AuthorizeAttributeTest()
        {
            ...
            _controllerDescriptorMock.Setup(cd => cd.GetCustomAttributes<AllowAnonymousAttribute>())
                .Returns(
                    new Collection<AllowAnonymousAttribute>(
                        Enumerable.Empty<AllowAnonymousAttribute>().ToList()
                    )
                );
            _actionDescriptorMock.Setup(ad => ad.GetCustomAttributes<AllowAnonymousAttribute>())
                .Returns(
                    new Collection<AllowAnonymousAttribute>(
                        Enumerable.Empty<AllowAnonymousAttribute>().ToList()
                    )
        ...
        public void OnAuthorization_IfRequestNotAuthorized_CallsHandleUnauthorizedRequest()
        {
            ...
            authorizeAttributeMock.Setup(a => a.HandleUnauthorizedRequestPublic(_actionContext))
                .Verifiable();
            AuthorizeAttribute attribute = authorizeAttributeMock.Object;
