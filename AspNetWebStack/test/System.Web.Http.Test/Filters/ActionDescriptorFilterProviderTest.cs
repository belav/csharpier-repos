...
namespace System.Web.Http.Filters
{
    ...
    {
        private readonly ActionDescriptorFilterProvider _provider =
        ...
        public void GetFilters_GetsFilterObjectsFromActionDescriptorAndItsControllerDescriptor()
        {
            ...
            Assert.Equal(
                new FilterInfo(filter3, FilterScope.Controller),
                result[0],
                new TestFilterInfoComparer()
            );
            Assert.Equal(
                new FilterInfo(filter1, FilterScope.Action),
                result[1],
                new TestFilterInfoComparer()
            );
            Assert.Equal(
                new FilterInfo(filter2, FilterScope.Action),
                result[2],
                new TestFilterInfoComparer()
            );
        }
        ...
        public class TestFilterInfoComparer : IEqualityComparer<FilterInfo>
        {
            public bool Equals(FilterInfo x, FilterInfo y)
            {
                return (x == null && y == null)
                    || (Object.ReferenceEquals(x.Instance, y.Instance) && x.Scope == y.Scope);
            }
