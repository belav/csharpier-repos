    ...
    public class ConfigurationManagerWrapperTest
    {
        ...
        public void GetConnectionGetsConnectionFromConfig()
        {
            ...
            var configManager = new ConfigurationManagerWrapper(
                new Dictionary<string, IDbFileHandler>(),
                "DataDirectory"
            );
            Func<string, bool> fileExists = path => false;
            Func<string, IConnectionConfiguration> getFromConfig = name =>
                new MockConnectionConfiguration("connection string");
        ...
        public void GetConnectionGetsConnectionFromDataDirectoryIfFileWithSupportedExtensionExists()
        {
            ...
            mockHandler.Setup(m => m.GetConnectionConfiguration(@"DataDirectory\Bar.foo"))
        ...
        public void GetConnectionSdfAndMdfFile_MdfFileWins()
        {
            ...
            mockSdfHandler.Setup(m => m.GetConnectionConfiguration(@"DataDirectory\Bar.sdf"))
                .Returns(new MockConnectionConfiguration("sdf connection"));
            var mockMdfHandler = new Mock<MockDbFileHandler>();
            mockMdfHandler.Setup(m => m.GetConnectionConfiguration(@"DataDirectory\Bar.mdf"))
                .Returns(new MockConnectionConfiguration("mdf connection"));
            var handlers = new Dictionary<string, IDbFileHandler>
            {
            ...
            Func<string, bool> fileExists = path =>
                path.Equals(@"DataDirectory\Bar.mdf") || path.Equals(@"DataDirectory\Bar.sdf");
            Func<string, IConnectionConfiguration> getFromConfig = name => null;
