...
namespace System.Web.Mvc.Html.Test
{
    ...
    public class MetadataOverrideScope : IDisposable
    {
        ...
        public MetadataOverrideScope(ModelMetadata metadata)
        {
            ...
            metadataProvider.Setup(p => p.GetMetadataForType(It.IsAny<Func<object>>(), _modelType))
                .Returns(
                    (Func<object> modelAccessor, Type modelType) =>
        ...
        private ModelMetadata CloneMetadata(Func<object> modelAccessor)
        {
            ...
            {
                clonedMetadata = new CachedDataAnnotationsModelMetadata(
                    cachedMetadata,
                    modelAccessor
                );
            }
            ...
            {
                var provider =
                    (_oldMetadataProvider as DataAnnotationsModelMetadataProvider)
                    ?? AnnotationsProvider;
                ...
                ); // Copying SimpleDisplayText below compensates for null here.
            }
            else
            {
