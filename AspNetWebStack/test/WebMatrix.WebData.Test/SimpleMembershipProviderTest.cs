...
namespace WebMatrix.WebData.Test
{
    public class SimpleMembershipProviderTest
    {
        ...
        public void GenerateTokenHtmlEncodesValues()
        {
            ...
            var generatedBytes = Encoding.Default.GetBytes("|a�x�#�o�?�A8E�");
            generator
                .Setup(g => g.GetBytes(It.IsAny<byte[]>()))
                .Callback(
                    (byte[] array) => Array.Copy(generatedBytes, array, generatedBytes.Length)
                );
