...
namespace System.Web.WebPages.Test
{
    ...
    public class CheckBoxTest : IDisposable
    {
        ...
        public void CheckboxWithObjectAttributes()
        {
            ...
            // Assert
            Assert.Equal(
                @"<input attr=""attr-value"" id=""foo"" name=""foo"" type=""checkbox"" />",
                html.ToHtmlString()
            );
        }
        ...
        public void CheckBoxOverwritesImplicitAttributes()
        {
            ...
            // Assert
            Assert.Equal(
                @"<input checked=""checked"" id=""foo"" name=""foo"" type=""fooType"" />",
                html.ToHtmlString()
            );
        }
        ...
        public void CheckboxAddsUnobtrusiveValidationAttributes()
        {
            ...
            var html = helper.CheckBox(
                fieldName,
                new Dictionary<string, object> { { "data-some-val", "5" } }
            );
        ...
        public void CheckboxWithAttributesFromAnonymousObject_WithUnderscoreInName_TransformsUnderscoresToDashs()
        {
            HtmlHelperTest.AssertHelperTransformsAttributesUnderscoresToDashs(
                (helper, attributes) => helper.CheckBox("foo", attributes)
            );

            HtmlHelperTest.AssertHelperTransformsAttributesUnderscoresToDashs(
                (helper, attributes) => helper.CheckBox("foo", true, attributes)
            );
        }

        public void Dispose()
        {
            // Reset ScopeStorage (written via e.g. HtmlHelper.ValidationInputCssClassName) between tests to
