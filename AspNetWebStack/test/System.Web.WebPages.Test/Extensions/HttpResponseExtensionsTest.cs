...
namespace Microsoft.WebPages.Test.Helpers
{
    public class HttpResponseExtensionsTest
    {
        ...
        public HttpResponseExtensionsTest()
        {
            ...
            mockResponse.Setup(response => response.Redirect(It.IsAny<string>()))
                .Callback((string url) => _redirectUrl = url);
            mockResponse.Setup(response => response.Write(It.IsAny<string>()))
        ...
        public void OutputCacheSetsExpirationTimeBasedOnCurrentContext()
        {
            ...
            cache.Setup(
                c => c.SetCacheability(It.Is<HttpCacheability>(p => p == HttpCacheability.Public))
            )
                .Verifiable();
            cache.Setup(c => c.SetExpires(It.Is<DateTime>(p => p == timestamp.AddSeconds(20))))
                .Verifiable();
            cache.Setup(c => c.SetMaxAge(It.Is<TimeSpan>(p => p == TimeSpan.FromSeconds(20))))
                .Verifiable();
