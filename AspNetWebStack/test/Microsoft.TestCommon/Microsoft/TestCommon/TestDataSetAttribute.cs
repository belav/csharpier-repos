...
namespace Microsoft.TestCommon
{
    public class TestDataSetAttribute : DataAttribute
    {
        ...
        ) : this(declaringType, propertyName, testDataVariations)
        {
            ExtraDataSets = new List<Tuple<Type, string>>
            {
                Tuple.Create(declaringType1, propertyName1)
            };
        }
        ...
        ) : this(declaringType, propertyName, testDataVariations)
        {
            ...
            {
                Tuple.Create(declaringType1, propertyName1),
                Tuple.Create(declaringType2, propertyName2)
            };
        }
        ...
        ) : this(declaringType, propertyName, testDataVariations)
        {
            ExtraDataSets = new List<Tuple<Type, string>>
            {
                Tuple.Create(declaringType1, propertyName1),
                Tuple.Create(declaringType2, propertyName2),
                Tuple.Create(declaringType3, propertyName3)
            };
        }
        ...
        ) : this(declaringType, propertyName, testDataVariations)
        {
            ExtraDataSets = new List<Tuple<Type, string>>
            {
                ...
                Tuple.Create(declaringType2, propertyName2),
                Tuple.Create(declaringType3, propertyName3),
                Tuple.Create(declaringType4, propertyName4)
            };
        }
        ...
        )
        {
            ...
            IEnumerable<IEnumerable<object[]>> finalDataSets = (new[] { baseDataSet })
        ...
        private static IEnumerable<object[]> GetBaseDataSet(
            Type declaringType,
            string propertyName,
        ...
        private static IEnumerable<object[]> GetDataSet(Type declaringType, string propertyName)
        {
            ...
            IEnumerable<object> value =
                ...
                ?? (propertyValue as IEnumerable).Cast<object>();
            if (value == null)
            {
                throw new InvalidOperationException(
                    String.Format(
                        "{0}.{1} is either null or does not implement IEnumerable",
                        declaringType.FullName,
                        propertyName
                    )
                );
