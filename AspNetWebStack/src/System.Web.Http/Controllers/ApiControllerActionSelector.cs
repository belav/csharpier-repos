    ...
    public class ApiControllerActionSelector : IHttpActionSelector
    {
        ...
        private class ActionSelectorCacheItem
        {
            ...
            public ActionSelectorCacheItem(HttpControllerDescriptor controllerDescriptor)
            {
                ...
                _combinedActionNameMapping = _combinedCandidateActions.Select(
                        c => c.ActionDescriptor
                    )
                    .ToLookup(
            ...
            private void InitializeStandardActions()
            {
                ...
                standardActions.StandardActionNameMapping =
                    standardActions.StandardCandidateActions.Select(c => c.ActionDescriptor)
                        .ToLookup(
                            actionDesc => actionDesc.ActionName,
                            StringComparer.OrdinalIgnoreCase
                        );
            ...
            )
            {
                if (candidatesFound.Count > 1)
                {
                    ...
                    return candidatesFound.GroupBy(
                            candidate => _actionParameterNames[candidate.ActionDescriptor].Length
                        )
            ...
            )
            {
                ...
                CandidateActionWithParams[] candidatesWithParams = Array.ConvertAll(
                    candidatesFound,
                    candidate =>
                        new CandidateActionWithParams(candidate, combinedParameterNames, routeData)
                );
            ...
            )
            {
                ...
                return candidatesFound.Where(c => c.CandidateAction.Precedence == highestPrecedence)
                    .AsList();
            }
