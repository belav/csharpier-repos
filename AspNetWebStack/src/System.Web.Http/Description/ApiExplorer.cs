...
namespace System.Web.Http.Description
{
    ...
    public class ApiExplorer : IApiExplorer
    {
        ...
        private static readonly Regex _actionVariableRegex = new Regex(
            String.Format(CultureInfo.CurrentCulture, "{{{0}}}", RouteValueKeys.Action),
            RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant
        );
        private static readonly Regex _controllerVariableRegex = new Regex(
            String.Format(CultureInfo.CurrentCulture, "{{{0}}}", RouteValueKeys.Controller),
            RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant
        );
        ...
        )
        {
            ...
            ApiExplorerSettingsAttribute setting = actionDescriptor
                .GetCustomAttributes<ApiExplorerSettingsAttribute>()
                .FirstOrDefault();
            return (setting == null || !setting.IgnoreApi)
        ...
        )
        {
            ...
            HttpMethodConstraint httpMethodConstraint =
                route.Constraints.Values.FirstOrDefault(c =>
                    typeof(HttpMethodConstraint).IsAssignableFrom(c.GetType())
            ...
            else
            {
                supportedMethods = httpMethodConstraint
                    .AllowedMethods.Intersect(actionHttpMethods)
                    .ToList();
            }
        ...
        private Collection<ApiDescription> ExploreDirectRoute(
            HttpControllerDescriptor controllerDescriptor,
        ...
        )
        {
            ...
            )
            {
                ...
                HttpControllerDescriptor controllerDescriptor;
                if (
                    controllerMappings.TryGetValue(
                        controllerVariableValue,
        ...
        )
        {
            ...
            if (!controllerDescriptor.IsAttributeRouted())
            {
                ...
                ILookup<string, HttpActionDescriptor> actionMappings = controllerServices
                    .GetActionSelector()
                    .GetActionMapping(controllerDescriptor);
                string actionVariableValue;
        ...
        )
        {
            ...
            // parameters
            IList<ApiParameterDescription> parameterDescriptions = CreateParameterDescriptions(
                actionDescriptor,
                parsedRoute,
                route.Defaults
            );
            ...
            // request formatters
            ApiParameterDescription bodyParameter = parameterDescriptions.FirstOrDefault(
                description => description.Source == ApiParameterSource.FromBody
            );
            IEnumerable<MediaTypeFormatter> supportedRequestBodyFormatters =
                bodyParameter != null
                    ? actionDescriptor.Configuration.Formatters.Where(f =>
                        f.CanReadType(bodyParameter.ParameterDescriptor.ParameterType)
                    )
            ...
            IList<HttpMethod> supportedMethods = GetHttpMethodsSupportedByAction(
                route,
                actionDescriptor
            );
        ...
        {
            Collection<ResponseTypeAttribute> responseTypeAttribute =
                actionDescriptor.GetCustomAttributes<ResponseTypeAttribute>();
            Type responseType = responseTypeAttribute
                .Select(attribute => attribute.ResponseType)
                .FirstOrDefault();
        ...
        )
        {
            ...
            return parameterDescriptions.Count(parameter =>
                    ...
                    && parameter.CanConvertPropertiesFromString()
                ) > 1;
        }

        // Set as internal for the unit test.
        internal static bool TryExpandUriParameters(
            IHttpRoute route,
        ...
        )
        {
            ...
            foreach (ApiParameterDescription parameterDescription in parameterDescriptions)
            {
                if (parameterDescription.Source == ApiParameterSource.FromUri)
                {
                    ...
                    )
                    {
                        ...
                        PropertyInfo[] innerTypeProperties = ApiParameterDescription
                            .GetBindableProperties(innerType)
                            .ToArray();
                        if (innerTypeProperties.Any())
                        {
                            ...
                            AddPlaceholderForProperties(
                                parameterValuesForRoute,
                                innerTypeProperties,
                                parameterName + "[0]."
                            );
                            AddPlaceholderForProperties(
            ...
            BoundRouteTemplate boundRouteTemplate = parsedRoute.Bind(
                null,
                parameterValuesForRoute,
        ...
        private static bool IsBindableCollection(Type type)
        {
            ...
            return type.IsArray
                || new CollectionModelBinderProvider().GetBinder(null, type) != null;
        }
        ...
        )
        {
            ...
            HashSet<string> visitedApiDescriptionIds = new HashSet<string>(
                StringComparer.OrdinalIgnoreCase
            );
        ...
        private static bool MatchRegexConstraint(
            IHttpRoute route,
