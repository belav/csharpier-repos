...
namespace System.Web.Mvc
{
    public class FilterAttributeFilterProvider : IFilterProvider
    {
        ...
        {
            // Results are low in number in the common case so use yield return to avoid creating intermediate
            // collections or nested enumerables
            if (controllerContext.Controller != null)
            {
                foreach (
                    FilterAttribute attr in GetControllerAttributes(
                        controllerContext,
                        actionDescriptor
                    )
                )
