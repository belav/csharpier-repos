...
namespace System.Web.Mvc.Html
{
    public static class TextAreaExtensions
    {
        ...
        private static Dictionary<string, object> implicitRowsAndColumns = new Dictionary<
            string,
            object
        >
        {
        ...
        private static Dictionary<string, object> GetRowsAndColumnsDictionary(int rows, int columns)
        {
            if (rows < 0)
            {
                throw new ArgumentOutOfRangeException(
                    "rows",
                    MvcResources.HtmlHelper_TextAreaParameterOutOfRange
                );
            }
            if (columns < 0)
            {
        ...
        )
        {
            ...
            tagBuilder.MergeAttributes(
                htmlHelper.GetUnobtrusiveValidationAttributes(name, modelMetadata)
            );
            ...
            tagBuilder.InnerHtml =
                (innerHtmlPrefix ?? Environment.NewLine) + HttpUtility.HtmlEncode(value);
