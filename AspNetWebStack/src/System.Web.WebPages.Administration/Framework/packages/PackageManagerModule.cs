...
namespace System.Web.WebPages.Administration.PackageManager
{
    internal static class PackageManagerModule
    {
        ...
        internal static readonly string PackageSourceFilePath = VirtualPathUtility.Combine(
        ...
        public static IEnumerable<WebPackageSource> PackageSources
        {
            get
            {
                Debug.Assert(
                    ...
                    "InitFeedsFile must be called before Feeds can be accessed."
                );
        ...
        [SuppressMessage(
            ...
            Justification = "Any exception that occurs indicates that the source file cannot be initialized."
        )]
        public static bool InitPackageSourceFile(
        ...
        public static WebPackageSource GetSource(string sourceName)
        {
            Debug.Assert(
                _packageSources != null,
                "InitFeedsFile must be called before this method can be called."
            );
        ...
        )
        {
            lock (_sourceFileLock)
            {
                return packageSources
                    .Where(source =>
                        source.Name.Equals(sourceName, StringComparison.OrdinalIgnoreCase)
                    )
                    .FirstOrDefault();
            }
        }
        ...
        public static void RemovePackageSource(
            IPackagesSourceFile sourceFile,
            ISet<WebPackageSource> packageSources,
            string name
        )
