    ...
    public class WebProjectManager : IWebProjectManager
    {
        ...
        public WebProjectManager(string remoteSource, string siteRoot)
        {
            ...
            _projectManager = new ProjectManager(
                ...
                localRepository: PackageRepositoryFactory.Default
        ...
        )
        {
            ...
            {
                packagesToUpdate = packagesToUpdate.Where(
                    p =>
                        !String.IsNullOrEmpty(p.Tags)
                        && p.Tags.ToLower().Contains(WebPagesPreferredTag)
                );
            }
            return SourceRepository.GetUpdates(packagesToUpdate, includePrerelease: false)
                .AsQueryable();
        }
        ...
        {
            return SourceRepository.GetUpdates(new[] { package }, includePrerelease: false)
                .SingleOrDefault();
        }
