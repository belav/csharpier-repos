    ...
    public partial class HtmlHelper
    {
        ...
        )
        {
            ...
            if (selectedValues != null)
            {
                IEnumerable values =
                    (allowMultiple)
                        ? ConvertTo(selectedValues, typeof(string[])) as string[]
                        : new[] { ConvertTo(selectedValues, typeof(string)) };

                HashSet<string> selectedValueSet = new HashSet<string>(
                ...
                foreach (SelectListItem item in selectList)
                {
                    ...
                    {
                        selected =
                            item.Selected || selectedValueSet.Contains(item.Value ?? item.Text);
                    }
        ...
        )
        {
            ...
            if (selectedValue != null)
            {
                var newSelectList = new List<SelectListItem>(
                    from item in selectList
                    select new SelectListItem(item)
                );
                var comparer = StringComparer.InvariantCultureIgnoreCase;
                var selectedItem = newSelectList.FirstOrDefault(item =>
                    item.Selected || comparer.Equals(item.Value ?? item.Text, selectedValue)
                );
