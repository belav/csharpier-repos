...
public class Tests
{
    ...
    public class TaskAwaiterOnCompletedHelper : MarshalByRefObject
    {
        ...
        public MethodBase ThrowerMethodInfo =>
            typeof(TaskAwaiterOnCompletedHelper).GetMethod(nameof(WhenCompletedThrower));
        ...
        public void OnUnhandled(object sender, UnhandledExceptionEventArgs args)
        {
            if (args.ExceptionObject is SpecialExn exn)
            {
                try
                {
                    ...
                    {
                        Console.WriteLine(
                            "expected to see {0} in stack trace, but it wasn't there",
                            ThrowerMethodInfo.ToString()
                        );
                        holder.Result = 3;
                        return;
                    }
