...
public class TestRunner
{
    ...
    class ProcessData
    {
        ...
        public StringBuilder stdout,
            stderr;
        public object stdoutLock = new object(),
            stderrLock = new object();
    ...
    public static int Main(String[] args)
    {
        ...
        while (i < args.Length)
        {
            if (args[i].StartsWith("-"))
            {
                ...
                else if (args[i] == "--repeat")
                {
                    ...
                    {
                        Console.WriteLine(
                            "Invalid argument to --repeat command line option, should be > 1"
                        );
                        return 1;
                    }
                    i += 2;
        ...
        else
        {
            ...
            string babysitterXmlList = Environment.GetEnvironmentVariable(
            ...
            if (!String.IsNullOrEmpty(babysitterXmlList))
            {
                ...
                {
                    Console.WriteLine(
                        "Attempted to record XML path to file {0} but failed.",
                        babysitterXmlList
                    );
                }
            }
        ...
        else
        {
            ...
            Console.WriteLine(
                String.Format(
                    "{0} test(s) passed, {1} test(s) did not pass.",
                    passed.Count,
                    failed.Count
                )
            );
    ...
    )
    {
        ...
        using (XmlWriter writer = XmlWriter.Create(path, xmlWriterSettings))
        {
            ...
            writer.WriteAttributeString(
                "total",
                (passed.Count + failed.Count + timedout.Count).ToString()
            );
            ...
            writer.WriteAttributeString(
                "time",
                test_time.TotalSeconds.ToString(CultureInfo.InvariantCulture)
            );
            ...
            foreach (ProcessData pd in passed)
            {
                ...
                writer.WriteAttributeString(
                    "name",
                    String.Format("MonoTests.{0}.{1}", testsuiteName, pd.test)
                );
                writer.WriteAttributeString("executed", "True");
                writer.WriteAttributeString("success", "True");
                writer.WriteAttributeString(
                    "time",
                    pd.duration.TotalSeconds.ToString(CultureInfo.InvariantCulture)
                );
            ...
            foreach (ProcessData pd in timedout)
            {
                ...
                writer.WriteAttributeString(
                    "name",
                    String.Format("MonoTests.{0}.{1}_timedout", testsuiteName, pd.test)
                );
                writer.WriteAttributeString("executed", "True");
                writer.WriteAttributeString("success", "False");
                writer.WriteAttributeString(
    ...
    )
    {
        ...
        using (XmlWriter writer = XmlWriter.Create(path, xmlWriterSettings))
        {
            ...
            foreach (var pd in timedout)
            {
                ...
                writer.WriteAttributeString(
                    "name",
                    testsuiteName + ".tests." + pd.test + "_timedout"
                );
                writer.WriteAttributeString("type", testsuiteName + ".tests");
                writer.WriteAttributeString("method", pd.test.ToString());
                writer.WriteAttributeString("result", "Fail");
