...
class Driver
{
    internal static Dictionary<string, string> BadAssembliesToEnumTable = new Dictionary<
        string,
        string
    >
    {
    ...
    // IGNORED_ASSEMBLY
    internal static Dictionary<string, NuGetData> ignoredAsmTable =
        new Dictionary<string, NuGetData>();

    // IGNORED_ASM_VER
    internal static SortedDictionary<string, NuGetData> ignoredAsmVerTable =
        new SortedDictionary<string, NuGetData>();
    ...
    static void DumpOutput()
    {
        if (dump_asm)
        {
            ...
            {
                sb.AppendLine(
                    $"IGNORED_ASSEMBLY (0x{data.HashCode}, {data.EnumName}, \"{data.ModuleVersionId}\", \"{data.NuGetVersionAndFramework}\"),"
                );
            }
        ...
        if (dump_ver)
        {
            ...
            foreach (var key in ignoredAsmVerTable.Keys)
            {
                ...
                sb.AppendLine(
                    $"IGNORED_ASM_VER ({data.EnumName}, {ver.Major}, {ver.Minor}, {ver.Build}, {ver.Revision}),"
                );
            }
        ...
        if (dump_guids_for_msbuild)
        {
            ...
            var query = ignoredAsmTable
                .Values.GroupBy(data => data.AssemblyName)
                .OrderBy(group => group.Key)
                .Select(group => new
                {
                    AssemblyName = group.Key,
                    NuGets = group
                        .OrderBy(data => data.AssemblyVersion)
                        .ThenBy(data => data.ModuleVersionId),
                });
            ...
            foreach (var g in query)
            {
                foreach (var data in g.NuGets)
                {
                    var ver = data.AssemblyVersion;
                    Console.WriteLine(
                        $"denied:{data.AssemblyName},{data.ModuleVersionId},{ver.Major},{ver.Minor},{ver.Build},{ver.Revision}"
                    );
                }
            }
        }
    ...
    static void Main(string[] args)
    {
        ...
        foreach (
            var f in Directory
                .GetFiles(args[0], "*.nupkg")
                .OrderBy(nupkg => Path.GetFileName(nupkg))
        )
        {
            DumpNuget(f);
        }
    ...
    static void LoadAndDump(ZipArchiveEntry entry, string version)
    {
        ...
        DoParse p = (DoParse)
            ad.CreateInstanceAndUnwrap(typeof(DoParse).Assembly.FullName, typeof(DoParse).FullName);
...
class DoParse : MarshalByRefObject
{
    ...
    static string FileToMoniker(string path) =>
        path.Split(Path.DirectorySeparatorChar).Where(p => p.StartsWith("net")).FirstOrDefault()
        ?? "unknown";
