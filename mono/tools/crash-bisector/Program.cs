...
namespace crashbisector
{
    class BisectInfo
    {
        ...
        bool TryEliminate(IEnumerable<string> methods, int chunkSize)
        {
            ...
            foreach (var i in EliminationOrder(numChunks))
            {
                ...
                if (chunkSize == 1)
                    ...
                    Console.WriteLine(
                        "Running without methods at positions {0} to {1}",
                        firstIndex,
                        lastPlusOneIndex - 1
                    );
        ...
        bool EliminationStep(IEnumerable<string> methods)
        {
            ...
            {
                Console.WriteLine(
                    "Can't eliminate further.  Methods required to crash are:\n{0}",
                    string.Join("\n", methods)
                );
                return true;
            }
            ...
            Console.WriteLine(
                "Couldn't eliminate any method.  Methods required to crash are:\n{0}",
                string.Join("\n", methods)
            );
            return true;
        }
        ...
        bool BisectStep(IEnumerable<string> methods)
        {
            ...
            Console.WriteLine(
                "Splitting into two halves: {0} and {1} methods.",
                firstHalf.Count(),
                secondHalf.Count()
            );
