...
namespace Mono.Profiling.Tests.Stress
{
    ...
    static class Program
    {
        ...
        static readonly Dictionary<string, Predicate<Benchmark>> _filters = new Dictionary<
            string,
            Predicate<Benchmark>
        >
        {
        ...
        static int Main()
        {
            ...
            var classDir = Path.GetFullPath(
                Path.Combine("..", "..", "mcs", "class", "lib", "net_4_x")
            );
            ...
            if (
                Environment.GetEnvironmentVariable("MONO_PROFILER_STRESS_TIMEOUT")
                is string envTimeout
            )
            ...
            if (
                Environment.GetEnvironmentVariable("MONO_PROFILER_STRESS_OPTIONS")
                is string envOptions
            )
                options = envOptions;
            ...
            if (
                Environment.GetEnvironmentVariable("MONO_PROFILER_STRESS_SUSPEND")
                is string envSuspend
            )
            ...
            for (var i = 0; i < benchmarks.Length; i++)
            {
                ...
                var flags = _options
                    .ToDictionary(x => x, _ => rand.Next(0, 2) == 1)
                ...
                var info = new ProcessStartInfo
                {
                    ...
                    FileName = monoPath,
                    Arguments =
                        $"--debug --profile=log:{profOptions} "
                        + string.Join(" ", bench.CommandLine),
                ...
                var resultStr =
                    result.ExitCode == null ? "timed out" : $"exited with code: {result.ExitCode}";

                Console.ForegroundColor =
                    result.ExitCode != 0 ? ConsoleColor.Red : ConsoleColor.Green;
                Console.WriteLine(
            ...
            using (var writer = XmlWriter.Create("TestResult-profiler-stress.xml", settings))
            {
                ...
                writer.WriteComment(
                    $"This file represents the results of running a test suite (seed: {seed})"
                );
                ...
                foreach (var result in results)
                {
                    ...
                    writer.WriteAttributeString(
                        "name",
                        $"MonoTests.profiler-stress.{result.Benchmark.Name}{timeoutStr}"
                    );
                    writer.WriteAttributeString("executed", "True");
                    writer.WriteAttributeString("success", (result.ExitCode == 0).ToString());
                    writer.WriteAttributeString(
                        "time",
                        ((int)result.Stopwatch.Elapsed.TotalSeconds).ToString()
                    );
            ...
            Console.ForegroundColor =
                failures + timeouts != 0 ? ConsoleColor.Red : ConsoleColor.Green;
            Console.WriteLine(
