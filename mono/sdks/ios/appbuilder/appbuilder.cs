...
public class AppBuilder
{
    ...
    void GenMain(string builddir, List<string> assembly_names, bool isinterp)
    {
        ...
        {
            symbols.Add(
                String.Format("mono_aot_module_{0}_info", img.Replace('.', '_').Replace('-', '_'))
            );
        }
        ...
        w.WriteLine(
            "\tmono_jit_set_aot_mode ({0});",
            isinterp ? "MONO_AOT_MODE_INTERP" : "MONO_AOT_MODE_FULL"
        );
        w.WriteLine("}");
    ...
    void Run(String[] args)
    {
        ...
        switch (target)
        {
            ...
            default:
                Console.WriteLine(
                    $"Possible values for the '--target=' argument are 'ios-dev64', 'ios-sim64', got {target}."
                );
                Environment.Exit(1);
                break;
        }
        ...
        ninja.WriteLine(
            $"  command = MONO_PATH=$mono_path $cross -O=gsharedvt,float32 --debug {cross_runtime_args} --aot=mtriple=arm64-ios,static,asmonly,direct-icalls,no-direct-calls,dwarfdebug,{aot_args},outfile=$outfile,data-outfile=$data_outfile $src_file"
        );
        ...
        ninja.WriteLine(
            $"  command = if ! test -f $outfile; then MONO_PATH=$mono_path $cross -O=gsharedvt,float32 --debug {cross_runtime_args} --aot=mtriple=arm64-ios,static,asmonly,direct-icalls,no-direct-calls,dwarfdebug,{aot_args},outfile=$outfile,data-outfile=$data_outfile $src_file; fi"
        );
        ninja.WriteLine("  description = [AOT] $src_file -> $outfile");
        ...
        ninja.WriteLine(
            $"  command = clang -ObjC -isysroot $sysroot -miphoneos-version-min=10.1 -arch arm64 -framework Foundation -framework UIKit -framework GSS -o $appdir/{bundle_executable} $in -liconv -lz $forcelibs"
        );
        ...
        var cultures = CultureInfo
            ...
            .Where(x => !String.IsNullOrEmpty(x.IetfLanguageTag))
        ...
        foreach (var assembly in assemblies)
        {
            ...
            if (isdebug && File.Exists(pdb))
            {
                ...
                ninja.WriteLine(
                    $"build $appdir/{filename_noext}.pdb: cpifdiff $builddir/{filename_noext}.pdb"
                );
            }
            ...
            foreach (var culture in cultures)
            {
                ...
                if (!File.Exists(resource_assembly))
                    continue;

                Directory.CreateDirectory(Path.Combine(builddir, culture));
                File.Copy(
                    resource_assembly,
                    Path.Combine(builddir, culture, resource_filename),
                    true
                );
                ninja.WriteLine(
                    $"build $appdir/{culture}/{resource_filename}: cpifdiff $builddir/{culture}/{resource_filename}"
                );
            }
            ...
            if (isdev)
            {
                ...
                ninja.WriteLine(
                    $"build {destdir}/{filename}.o: {assemble_rule} {destdir}/{filename}.s"
                );
                if (isllvm)
                    ninja.WriteLine(
                        $"build {destdir}/{filename}.llvm.o: {assemble_rule} {destdir}/{filename}.llvm.s"
                    );
                ninja.WriteLine(
                    $"build $appdir/{filename_noext}.aotdata: cp {destdir}/{filename_noext}.aotdata"
                );
        ...
        if (isdev)
        {
            ...
            ninja.WriteLine(
                $"build $appdir/{bundle_executable}: gen-exe {ofiles} $builddir/modules.o "
                    + libs
                    + " $apptemplate_dir/app-device.a"
            );
            ninja.WriteLine(
                $"    forcelibs = -force_load $mono_sdkdir/{runtime}/lib/libmono-native-unified.a"
            );
        ...
        {
            ninja.WriteLine(
                $"build $appdir/{bundle_executable}: cp " + "$apptemplate_dir/app-simulator"
            );
        }
        ...
        if (isdev)
            ...
            ninja.WriteLine(
                $"build $appdir/_CodeSignature: codesign-sim $appdir/{bundle_executable}"
            );
