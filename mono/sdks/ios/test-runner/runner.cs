...
class XunitArgumentsParser
{
    public static XunitFilters ParseArgumentsToFilter(Stack<string> arguments)
    {
        ...
        while (arguments.Count > 0)
        {
            ...
            {
                case "-nomethod":
                    filters.ExcludedMethods.Add(arguments.Pop());
                    break;
                case "-noclass":
                    filters.ExcludedClasses.Add(arguments.Pop());
                    break;
                case "-nonamespace":
                    filters.ExcludedNamespaces.Add(arguments.Pop());
                    break;
                case "-notrait":
                    ParseEqualSeparatedArgument(filters.ExcludedTraits, arguments.Pop());
                    break;
                default:
                    throw new ArgumentException($"Not supported option: '{option}'");
            }
            ;
        }
    ...
    static void ParseEqualSeparatedArgument(
...
{
    public MonoSdksTextUI()
        : base() { }

    public MonoSdksTextUI(TextWriter writer)
        : base(writer) { }
        ...
        if (!result.Test.IsSuite)
        {
            switch (result.ResultState.Status)
            {
                case TestStatus.Passed:
                    Interop.mono_sdks_ui_increment_testcase_result(0);
                    break;
                case TestStatus.Inconclusive:
                case TestStatus.Skipped:
                    Interop.mono_sdks_ui_increment_testcase_result(1);
                    break;
                case TestStatus.Failed:
                    Interop.mono_sdks_ui_increment_testcase_result(2);
                    break;
            }
        }
    }
...
public class TestRunner
{
    public static int Main(string[] args)
    {
        ...
        var summarySink = new DelegatingExecutionSummarySink(
            ...
            {
                writer.WriteLine(
                    $"Tests run: {summary.Total}, Errors: 0, Failures: {summary.Failed}, Skipped: {summary.Skipped}{Environment.NewLine}Time: {TimeSpan.FromSeconds((double)summary.Time).TotalSeconds}s"
                );
            }
        );
        ...
        {
            writer.WriteLine($"[PASS] {args.Message.Test.DisplayName}");
            Interop.mono_sdks_ui_increment_testcase_result(0);
        };
        testSink.Execution.TestSkippedEvent += args =>
        {
            writer.WriteLine($"[SKIP] {args.Message.Test.DisplayName}");
            Interop.mono_sdks_ui_increment_testcase_result(1);
        };
        testSink.Execution.TestFailedEvent += args =>
        {
            writer.WriteLine(
                $"[FAIL] {args.Message.Test.DisplayName}{Environment.NewLine}{ExceptionUtility.CombineMessages(args.Message)}{Environment.NewLine}{ExceptionUtility.CombineStackTraces(args.Message)}"
            );
            Interop.mono_sdks_ui_increment_testcase_result(2);
        };

        testSink.Execution.TestAssemblyStartingEvent += args =>
        {
            writer.WriteLine($"Running tests for {args.Message.TestAssembly.Assembly}");
        };
        testSink.Execution.TestAssemblyFinishedEvent += args =>
        {
            writer.WriteLine($"Finished {args.Message.TestAssembly.Assembly}{Environment.NewLine}");
        };
        ...
        {
            writer.WriteLine($"STARTRESULTXML");
            using (var resultsXmlStream = File.OpenRead(resultsXmlPath))
                resultsXmlStream.CopyTo(((TcpWriter)writer).RawStream);
            writer.WriteLine();
            writer.WriteLine($"ENDRESULTXML");
        }
        ...
        Interop.mono_sdks_ui_set_test_summary_message(
            $"Summary: {(failed ? "Failed" : "Succeeded")} for {assemblyFileName}."
        );
