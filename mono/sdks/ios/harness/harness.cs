    ...
    void RunSim()
    {
        ...
        while (true)
        {
            ...
            if (line.Contains("STARTRESULTXML"))
            {
                ...
                xml_results_file = File.CreateText(
                    Path.Combine(Path.GetDirectoryName(bundle_dir), $"TestResult_{bundle_id}.xml")
                );
                continue;
            }
        ...
        if (
            result_line != null
            && result_line.Contains("Errors: 0")
            && result_line.Contains("Failures: 0")
        )
            Environment.Exit(0);
        else
            Environment.Exit(1);
