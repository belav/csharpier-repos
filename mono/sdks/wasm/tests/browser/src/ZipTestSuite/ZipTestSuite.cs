...
namespace TestSuite
{
    public class Program
    {
        ...
        public static async Task<WebAssembly.Core.Array> ZipGetArchiveEntryStreamLengthPosition(
            ZipArchiveMode mode
        )
        {
        ...
        public static string Compress(string uncompressedString)
        {
            ...
            using (
                var uncompressedStream = new MemoryStream(
                    Encoding.UTF8.GetBytes(uncompressedString)
                )
            )
            {
                using (var compressedStream = new MemoryStream())
                {
                    ...
                    using (
                        var compressorStream = new DeflateStream(
                            compressedStream,
        ...
        public static string Decompress(string compressedString)
        {
            ...
            using (
                var decompressorStream = new DeflateStream(
                    compressedStream,
        ...
        public static string CompressGZip(string uncompressedString)
        {
            ...
            )
            {
                ...
                {
                    using (
                        var gzip = new GZipStream(compressedStream, CompressionMode.Compress, true)
                    )
                    {
        ...
        public static string DecompressGZip(string compressedString)
        {
            ...
            using (
                var decompressorStream = new GZipStream(
                    compressedStream,
                    CompressionMode.Decompress
                )
            )
