...
namespace DebuggerTests
{
    public class ArrayTests : DebuggerTestBase
    {
        ...
        public async Task InspectPrimitiveTypeArrayLocals(
            int line,
            int col,
            string method_name,
            bool test_prev_frame,
            int frame_idx,
            bool use_cfo
        ) =>
        ...
        [InlineData(100, 8, "YetAnotherMethod", true, 2, false)]
        [InlineData(100, 8, "YetAnotherMethod", true, 2, true)]
        public async Task InspectValueTypeArrayLocals(
            int line,
            int col,
            string method_name,
            bool test_prev_frame,
        ...
        [InlineData(100, 8, "YetAnotherMethod", true, 2, false)]
        [InlineData(100, 8, "YetAnotherMethod", true, 2, true)]
        public async Task InspectObjectArrayLocals(
            int line,
            int col,
            string method_name,
            bool test_prev_frame,
        ...
        [InlineData(100, 8, "YetAnotherMethod", true, 2, false)]
        [InlineData(100, 8, "YetAnotherMethod", true, 2, true)]
        public async Task InspectGenericTypeArrayLocals(
            int line,
            int col,
            string method_name,
            bool test_prev_frame,
        ...
        [InlineData(100, 8, "YetAnotherMethod", true, 2, false)]
        [InlineData(100, 8, "YetAnotherMethod", true, 2, true)]
        public async Task InspectGenericValueTypeArrayLocals(
            int line,
            int col,
            string method_name,
            bool test_prev_frame,
        ...
        [InlineData(100, 8, "YetAnotherMethod", true, 2, false)]
        [InlineData(100, 8, "YetAnotherMethod", true, 2, true)]
        public async Task InspectGenericValueTypeArrayLocals2(
            int line,
            int col,
            string method_name,
            bool test_prev_frame,
        ...
        async Task TestSimpleArrayLocals(
            int line,
            int col,
            string entry_method_name,
            string method_name,
            string etype_name,
            string local_var_name_prefix,
            object[] array,
            object[] array_elem_props,
        ...
        )
        {
            ...
            await insp.Ready(
                async (cli, token) =>
                {
                    ...
                    CheckObject(
                        locals,
                        $"{local_var_name_prefix}_arr_null",
                        $"{etype_name}[]",
                        is_null: true
                    );
                    ...
                    { // Use `Runtime.callFunctionOn` to get the properties
                        ...
                        Assert.True(
                            !String.IsNullOrEmpty(l_objectId),
                            $"No objectId found for {name}"
                        );
                    ...
                    {
                        prefix_arr = await GetObjectOnFrame(
                    ...
                    if (array_elem_props?.Length > 0)
                    {
                        for (int i = 0; i < array_elem_props.Length; i++)
                        {
                            ...
                            {
                                var act_i = prefix_arr.FirstOrDefault(
                                    jt => jt["name"]?.Value<string>() == i_str
                                );
                                Assert.True(
                                    act_i != null,
                                    $"[{label}] Couldn't find array element [{i_str}]"
                                );

                                await CheckValue(
                                    act_i["value"],
                                    TObject(etype_name, is_null: true),
                                    label
                                );
                            }
                            else
                    ...
                    var props = await GetObjectOnFrame(
                        pause_location["callFrames"][frame_idx],
                        $"{local_var_name_prefix}_arr_empty"
                    );
            ...
            async Task<JToken> GetObjectWithCFO(string objectId, JObject fn_args = null)
            {
                ...
                var cfo_args = JObject.FromObject(
                    new { functionDeclaration = fn_decl, objectId = objectId }
                );
                ...
                return await GetProperties(
                    result.Value["result"]["objectId"]?.Value<string>(),
                    fn_args
                );
            }
        }
        ...
        public async Task InspectObjectArrayMembers(bool use_cfo)
        {
            ...
            await insp.Ready(
                async (cli, token) =>
                {
                    ...
                    var c_props = await GetObjectOnFrame(
                        pause_location["callFrames"][frame_idx],
                        "c"
                    );
                    await CheckProps(
        ...
        public async Task InspectValueTypeArrayLocalsStaticAsync(bool use_cfo)
        {
            ...
            int col = 12;
            string entry_method_name =
                "[debugger-test] DebuggerTests.ArrayTestsClass:ValueTypeLocalsAsync";
                ...
                async (cli, token) =>
                {
                    ...
                    await CheckProps(
                        ...
                        new
                        {
                            ...
                            gvclass_arr = TArray(
                                "DebuggerTests.SimpleGenericStruct<DebuggerTests.Point>[]",
                                2
                            ),
                            gvclass_arr_empty = TArray(
                                "DebuggerTests.SimpleGenericStruct<DebuggerTests.Point>[]"
                            ),
                            gvclass_arr_null = TObject(
                                "DebuggerTests.SimpleGenericStruct<DebuggerTests.Point>[]",
                                is_null: true
                            ),
                            gvclass = TValueType(
        ...
        public async Task InspectValueTypeArrayLocalsInAsyncStaticStructMethod(bool use_cfo)
        {
            ...
            int col = 12;
            string entry_method_name =
                "[debugger-test] DebuggerTests.ArrayTestsClass:EntryPointForStructMethod";
            int frame_idx = 0;
        ...
        [Fact]
        public async Task InvalidArrayId() =>
            await CheckInspectLocalsAtBreakpointSite(
                "DebuggerTests.Container",
                "PlaceholderMethod",
                1,
                "PlaceholderMethod",
                ...
                wait_for_event_fn: async (pause_location) =>
                {
                    ...
                    if (!DotnetObjectId.TryParse(c_obj_id, out var id) || id.Scheme != "object")
                        Assert.True(
                            false,
                            "Unexpected object id format. Maybe this test is out of sync with the object id format in library_mono.js?"
                        );

                    if (!int.TryParse(id.Value, out var idNum))
                        Assert.True(
                            false,
                            "Expected a numeric value part of the object id: {c_obj_id}"
                        );
        ...
        [Fact]
        public async Task InvalidValueTypeArrayIndex() =>
            await CheckInspectLocalsAtBreakpointSite(
                "DebuggerTests.Container",
                "PlaceholderMethod",
                1,
                ...
                locals_fn: async (locals) =>
                {
                    ...
                    var c_obj = GetAndAssertObjectWithName(
                        await GetProperties(this_obj["value"]["objectId"].Value<string>()),
                        "c"
                    );
                    var c_obj_id = c_obj["value"]?["objectId"]?.Value<string>();
                    Assert.NotNull(c_obj_id);
                    ...
                    var pf_arr_elems = await GetProperties(
                        pf_arr["value"]["objectId"].Value<string>()
                    );

                    if (
                        !DotnetObjectId.TryParse(
                            pf_arr_elems[0]["value"]?["objectId"]?.Value<string>(),
                            out var id
                        )
                    )
                    ...
                    Assert.True(
                        id_args["arrayId"] != null,
                        "ObjectId format for array seems to have changed. Expected to find 'arrayId' in the value. Update this test"
                    );
                    Assert.True(
                        id_args != null,
                        "Expected to get a json as the value part of {id}"
                    );

                    // Try one valid query, to confirm that the id format hasn't changed!
                    id_args["arrayIdx"] = 0;
                    await GetProperties(
                        $"dotnet:valuetype:{id_args.ToString(Newtonsoft.Json.Formatting.None)}",
                        expect_ok: true
                    );
        ...
        [Fact]
        public async Task InvalidAccessors() =>
            await CheckInspectLocalsAtBreakpointSite(
                "DebuggerTests.Container",
                "PlaceholderMethod",
                1,
                ...
                locals_fn: async (locals) =>
                {
                    ...
                    var invalid_accessors = new object[]
                    {
                        "NonExistant",
                        "10000",
                        "-2",
                    ...
                    foreach (var invalid_accessor in invalid_accessors)
                    {
                        ...
                        AssertEqual(
                            "undefined",
                            res.Value["result"]?["type"]?.ToString(),
                            "Expected to get undefined result for non-existant accessor"
                        );
                    }
                }
            );
