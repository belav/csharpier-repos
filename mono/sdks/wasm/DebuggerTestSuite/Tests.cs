        ...
        public async Task CreateJSBreakpoint()
        {
            ...
            await insp.Ready(
                async (cli, token) =>
                {
                    ...
                    Assert.EndsWith(
                        "debugger-driver.html",
                        bp2_res.Value["breakpointId"].ToString()
                    );
        ...
        public async Task CreateBadBreakpoint()
        {
            ...
            await insp.Ready(
                async (cli, token) =>
                {
                    ...
                    var bp1_res = await cli.SendCommand(
        ...
        public async Task ExceptionThrownInJS()
        {
            ...
            await insp.Ready(
                async (cli, token) =>
                {
                    ...
                    Assert.Equal(
                        "Uncaught",
                        eval_res.Error["exceptionDetails"]?["text"]?.Value<string>()
                    );
                }
            );
        }
                ...
                async (cli, token) =>
                {
...

                    {
                        var gres = await InvokeGetter(
                            GetAndAssertObjectWithName(locals, "ss_local"),
                            "V"
                        );
                        await CheckValue(
                            gres.Value["result"],
                            TNumber(0xDEADBEEF + 2),
                            $"ss_local#V"
                        );
                        // Check ss_local.dt
            ...
            await insp.Ready(
                async (cli, token) =>
                {
                    ...
                    var ss_local_as_ss_arg = new
                    {
                        V = TGetter("V"),
                        str_member = TString(
                            "ss_local#SimpleStruct#string#0#SimpleStruct#str_member"
                        ),
                        dt = TValueType("System.DateTime", dt.ToString()),
                        gs = TValueType(
                            "DebuggerTests.ValueTypesTest.GenericStruct<System.DateTime>"
                    ...
                    var ss_arg_props = await GetObjectOnFrame(
                        pause_location["callFrames"][0],
                        "ss_arg"
                    );
        ...
        public async Task CheckUpdatedValueTypeFieldsOnResume()
        {
            ...
            async Task CheckLocals(JToken pause_location, DateTime obj_dt, DateTime vt_dt)
            {
                ...
                var obj_props = await GetObjectOnLocals(locals, "obj");
                {
                    await CheckProps(
                        obj_props,
                        new { DT = TValueType("System.DateTime", obj_dt.ToString()) },
                        "locals#obj.DT",
                        num_fields: 5
                    );
                ...
                var vt_props = await GetObjectOnLocals(locals, "vt");
                {
                    await CheckProps(
                        vt_props,
                        new { DT = TValueType("System.DateTime", vt_dt.ToString()) },
                        "locals#obj.DT",
                        num_fields: 5
                    );
