    ...
    public class ExceptionTests : DebuggerTestBase
    {
        ...
        public async Task ExceptionTestAll()
        {
            ...
            string entry_method_name =
                "[debugger-test] DebuggerTests.ExceptionTestsClass:TestExceptions";
            ...
            await insp.Ready(
                async (cli, token) =>
                {
                    ...
                    var exception_members = await GetProperties(
                        pause_location["data"]["objectId"]?.Value<string>()
                    );
                    ...
                    exception_members = await GetProperties(
                        pause_location["data"]["objectId"]?.Value<string>()
                    );
            ...
            await insp.Ready(
                async (cli, token) =>
                {
                    ...
                    catch (ArgumentException ae)
                    {
                        ...
                        AssertEqual(
                            "Uncaught",
                            eo["exceptionDetails"]?["text"]?.Value<string>(),
                            "text"
                        );
                    ...
                    Assert.True(
                        false,
                        "Expected to get an ArgumentException from the uncaught user exception"
                    );
                }
            );
        }
            ...
            await insp.Ready(
                async (cli, token) =>
                {
                    ...
                    catch (ArgumentException ae)
                    {
                        ...
                        AssertEqual(
                            line,
                            eo["exceptionDetails"]?["lineNumber"]?.Value<int>(),
                            "lineNumber"
                        );
                        AssertEqual(
                            "Uncaught",
        ...
        [InlineData(
            "function () { invoke_static_method ('[debugger-test] DebuggerTests.ExceptionTestsClass:TestExceptions'); }",
            "dotnet://debugger-test.dll/debugger-exception-test.cs",
            28,
            16,
            "run",
            "DebuggerTests.CustomException",
            "not implemented uncaught"
        )]
        ...
        async Task<JObject> WaitForManagedException(JObject pause_location)
        {
            while (true)
            {
                if (pause_location != null)
                {
                    AssertEqual(
                        "exception",
                        pause_location["reason"]?.Value<string>(),
