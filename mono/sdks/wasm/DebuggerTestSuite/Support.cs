...
namespace DebuggerTests
{
    class Inspector
    {
        ...
        void NotifyOf(string what, JObject args)
        {
            if (!notifications.ContainsKey(what))
                throw new Exception(
                    $"Invalid internal state, notifying of {what}, but nobody waiting"
                );
        ...
        public async Task Ready(
            Func<InspectorClient, CancellationToken, Task> cb = null,
            TimeSpan? span = null
        )
        {
    ...
    public class DebuggerTestBase
    {
        ...
        static string FindTestPath()
        {
            ...
            throw new Exception(
                "Missing TEST_SUITE_PATH env var and could not guess path from CWD"
            );
        }
        ...
        internal async Task CheckInspectLocalsAtBreakpointSite(
            string url_key,
            int line,
            int column,
            string function_name,
            string eval_expression,
                ...
                async (cli, token) =>
                {
                    ...
                    await EvaluateAndCheck(
                        ...
                        wait_for_event_fn: async (pause_location) =>
                        {
                            ...
                            Assert.Equal(
                                bp.Value["breakpointId"]?.ToString(),
                                pause_location["hitBreakpoints"]?[0]?.Value<string>()
                            );
        ...
        )
        {
            ...
            await insp.Ready(
                async (cli, token) =>
                {
                    ...
                    {
                        Console.WriteLine(
                            $"Failed to run command {method} with args: {args?.ToString()}\nresult: {res.Error.ToString()}"
                        );
                        Assert.True(
                            false,
                            $"SendCommand for {method} failed with {res.Error.ToString()}"
                        );
                    }
                    ...
                    if (bp_function_name != null)
                        Assert.Equal(
                            bp_function_name,
                            pause_location["callFrames"]?[0]?["functionName"]?.Value<string>()
                        );
                    ...
                    if (locals_fn != null)
                    {
                        var locals = await GetProperties(
                            pause_location["callFrames"][0]["callFrameId"].Value<string>()
                        );
                        locals_fn(locals);
                    }
                }
        ...
        internal async Task<JToken> CheckPointerValue(
            JToken locals,
            string name,
            JToken expected,
            string label = null
        ...
        internal async Task<Result> InvokeGetter(
            JToken obj,
            object arguments,
        ...
        internal async Task<JObject> StepAndCheck(
            StepKind kind,
            string script_loc,
            int line,
            int column,
        ...
        internal async Task<JObject> SendCommandAndCheck(
            JObject args,
            string method,
            string script_loc,
            int line,
        ...
        )
        {
            ...
            if (function_name != null)
                Assert.Equal(
                    function_name,
                    wait_res["callFrames"]?[0]?["functionName"]?.Value<string>()
                );
            ...
            if (script_loc != null)
                CheckLocation(
                    ...
                    column,
                    ctx.scripts,
                    wait_res["callFrames"][0]["location"]
                );
            ...
            {
                var locals = await GetProperties(
                    wait_res["callFrames"][0]["callFrameId"].Value<string>()
                );
                locals_fn(locals);
            }
        ...
        internal async Task CheckDelegate(
            JToken locals,
            string name,
            string className,
            string target
        ...
        internal async Task CheckDelegate(JToken actual_val, JToken exp_val, string label)
        {
            ...
            AssertEqual(
                exp_val["className"]?.Value<string>(),
                actual_val["className"]?.Value<string>(),
                $"{label}-className"
            );
            ...
            var obj = del_props
                .Where(jt => jt["name"]?.Value<string>() == "Target")
                .FirstOrDefault();
            Assert.True(
                obj != null,
                $"[{label}] Property named 'Target' found found in delegate properties"
            );
            ...
            void CheckDelegateTarget(string actual_target, string exp_target)
            {
                ...
                else
                {
                    ...
                    Assert.True(
                        prefix,
                        $"{label}-description, Expected target to start with '{parts[0]}'. Actual: '{actual_target}'"
                    );
                    ...
                    Assert.True(
                        prefix,
                        $"{label}-description, Expected target to end with '{parts[1]}'. Actual: '{remaining}'"
                    );
                }
            }
        }
            ...
            switch (ctype)
            {
                ...
                case "pointer":
                {
                    if (exp_val["is_null"]?.Value<bool>() == true)
                    {
                        ...
                        AssertEqual(
                            exp_val_str,
                            actual_val["value"]?.Value<string>(),
                            $"{label}-value"
                        );
                        AssertEqual(
                            exp_val_str,
                            actual_val["description"]?.Value<string>(),
                            $"{label}-description"
                        );
                    }
                    ...
                    else
                    {
                        ...
                        Assert.False(
                            actual_val["className"]?.Value<string>() == $"{exp_prefix} 0",
                            $"[{label}] Expected a non-null value, but got {actual_val}"
                        );
                    }
                    break;
                }
                ...
                case "getter":
                {
                    ...
                    Assert.True(
                        get != null,
                        $"[{label}] No `get` found. {(actual_val != null ? "Make sure to pass the container object for testing getters, and not the ['value']" : String.Empty)}"
                    );

                    AssertEqual(
                        "Function",
                        get["className"]?.Value<string>(),
                        $"{label}-className"
                    );
                    AssertStartsWith(
                        $"get {exp_val["type_name"]?.Value<string>()} ()",
                        get["description"]?.Value<string>(),
                        $"{label}-description"
                    );
        ...
        internal async Task CheckProps(
            JToken actual,
            object exp_o,
            string label,
            int num_fields = -1
        )
        {
            ...
            Assert.True(
                num_fields == actual.Count(),
                $"[{label}] Number of fields don't match, Expected: {num_fields}, Actual: {actual.Count()}"
            );
            ...
            foreach (var kvp in exp)
            {
                ...
                var actual_obj = actual.FirstOrDefault(
                    jt => jt["name"]?.Value<string>() == exp_name
                );
                if (actual_obj == null)
                {
                    Assert.True(
                        actual_obj != null,
                        $"[{label}] Could not find property named '{exp_name}'"
                    );
                }

                Assert.True(
                    actual_obj != null,
                    $"[{label}] not value found for property named '{exp_name}'"
                );
        ...
        internal async Task CheckValue(JToken actual_val, JToken exp_val, string label)
        {
            ...
            if (exp_val["type"] == null && actual_val["objectId"] != null)
            {
                ...
                await CheckProps(
                    new_val,
                    exp_val,
                    $"{label}-{actual_val["objectId"]?.Value<string>()}"
                );
                return;
            }
            ...
            foreach (var jp in exp_val.Values<JProperty>())
            {
                ...
                var actual_field_val = actual_val
                    .Values<JProperty>()
                    .FirstOrDefault(a_jp => a_jp.Name == jp.Name);
                ...
                Assert.True(
                    actual_field_val != null,
                    $"[{label}] Could not find value field named {jp.Name}"
                );

                Assert.True(
                    exp_val_str == actual_field_val_str,
        ...
        internal async Task<JToken> GetLocalsForFrame(
            JToken frame,
            string script_loc,
            int line,
            int column,
        ...
        internal async Task<JToken> CompareObjectPropertiesFor(
            JToken locals,
            string name,
            object o,
            string label = null,
        ...
        internal async Task<JToken> GetProperties(
            string id,
            JToken fn_args = null,
            bool expect_ok = true
        )
        {
            if (ctx.UseCallFunctionOnBeforeGetProperties && !id.StartsWith("dotnet:scope:"))
            {
                ...
                var cfo_args = JObject.FromObject(
                    new { functionDeclaration = fn_decl, objectId = id }
                );
                if (fn_args != null)
                    cfo_args["arguments"] = fn_args;

                var result = await ctx.cli.SendCommand(
                ...
                AssertEqual(
                    expect_ok,
                    result.IsOk,
                    $"Runtime.getProperties returned {result.IsOk} instead of {expect_ok}, for {cfo_args.ToString()}, with Result: {result}"
                );
                if (!result.IsOk)
                    return null;
            ...
            var get_prop_req = JObject.FromObject(new { objectId = id });

            var frame_props = await ctx.cli.SendCommand(
            ...
            AssertEqual(
                expect_ok,
                frame_props.IsOk,
                $"Runtime.getProperties returned {frame_props.IsOk} instead of {expect_ok}, for {get_prop_req}, with Result: {frame_props}"
            );
            if (!frame_props.IsOk)
                return null;
        ...
        internal async Task<(JToken, Result)> EvaluateOnCallFrame(
            string id,
            string expression,
            bool expect_ok = true
        )
        {
            ...
            var res = await ctx.cli.SendCommand(
            ...
            AssertEqual(
                expect_ok,
                res.IsOk,
                $"Debugger.evaluateOnCallFrame ('{expression}', scope: {id}) returned {res.IsOk} instead of {expect_ok}, with Result: {res}"
            );
            if (res.IsOk)
        ...
        internal async Task<Result> SetBreakpoint(
            string url_key,
            int line,
            int column,
            bool expect_ok = true,
        ...
        )
        {
            ...
            var bp1_res = await ctx.cli.SendCommand(
        ...
        {
            var exc_res = await ctx.cli.SendCommand(
                "Debugger.setPauseOnExceptions",
                JObject.FromObject(new { state = state }),
                ctx.token
            );
            return exc_res;
        }

        internal async Task<Result> SetBreakpointInMethod(
            string assembly,
            string type,
            string method,
            int lineOffset = 0,
        ...
        internal void AssertEqual(object expected, object actual, string label)
        {
            ...
            throw new AssertActualExpectedException(expected, actual, $"[{label}]\n");
        }

        internal void AssertStartsWith(string expected, string actual, string label) =>
            Assert.True(
                actual?.StartsWith(expected),
                $"[{label}] Does not start with the expected string\nExpected: {expected}\nActual:   {actual}"
            );

        internal static Func<int, int, string, string, object> TSimpleClass = (X, Y, Id, Color) =>
            new
            {
                X = TNumber(X),
                Y = TNumber(Y),
        ...
        internal static Func<int, int, string, string, object> TPoint = (X, Y, Id, Color) =>
            new
            {
                X = TNumber(X),
                Y = TNumber(Y),
        ...
        internal static JObject TValueType(
            string className,
        ...
        internal static JObject TPointer(string type_name, bool is_null = false) =>
            JObject.FromObject(
                new
                {
                    ...
                    type_name = type_name,
                    is_null = is_null,
                    is_void = type_name.StartsWith("void*")
                }
            );
