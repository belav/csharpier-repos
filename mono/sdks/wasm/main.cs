...
public class Driver
{
    ...
    public static void StartTest(string name)
    {
        ...
        var testsuite_name = suites
            .Where(ts => ts.Name == args[0])
            .Select(ts => ts.File)
            .FirstOrDefault();
        ...
        testRunner.Exclude(
            "NotWasm,WASM,NotWorking,CAS,InetAccess,NotWorkingRuntimeInterpreter,MultiThreaded,StackWalk,GetCallingAssembly,LargeFileSupport,MobileNotWorking,ManagedCollator,"
                + excludes
        );
