...
class BoolFlag : Flag
{
    public BoolFlag(string name, string description, bool def_value, Action<bool> action)
        : base(name, description, FlagType.BoolFlag)
    {
        Setter = action;
...
class Driver
{
    ...
    static void Usage()
    {
        ...
        Console.WriteLine(
            "\t--native-lib=x  Link the native library 'x' into the final executable."
        );
        Console.WriteLine(
            "\t--preload-file=x Preloads the file or directory 'x' into the virtual filesystem."
        );
        Console.WriteLine(
            "\t--embed-file=x  Embeds the file or directory 'x' into the virtual filesystem."
        );
        ...
        foreach (var flag in option_list)
        {
            if (flag is BoolFlag)
            {
                ...
                Console.WriteLine(
                    "        type: bool  default: "
                        + ((flag as BoolFlag).DefaultValue ? "true" : "false")
                );
            }
        }
    }
    ...
    void GenDriver(string builddir, List<string> profilers, ExecMode ee_mode, bool link_icalls)
    {
        ...
        foreach (var adata in assemblies)
        {
            if (adata.aot)
                symbols.Add(
                    String.Format(
                        "mono_aot_module_{0}_info",
                        adata.name.Replace('.', '_').Replace('-', '_')
                    )
                );
        }
    ...
    int Run(string[] args)
    {
        ...
        AddFlag(
            ...
            new BoolFlag(
                ...
                opts.EnableDynamicRuntime,
                b => opts.EnableDynamicRuntime = b
            )
        );
        AddFlag(
            p,
        ...
        AddFlag(
            p,
            new BoolFlag(
                "linker-exclude-deserialization",
                "Link out .NET deserialization support",
                opts.LinkerExcludeDeserialization,
                b => opts.LinkerExcludeDeserialization = b
            )
        );
        AddFlag(
            p,
            new BoolFlag(
                ...
                opts.EnableCollation,
                b => opts.EnableCollation = b
            )
        );
        ...
        var runtime_js = Path.Combine(
            emit_ninja ? builddir : out_prefix,
            runtimeTemplateOutputName
        );
        if (emit_ninja)
        {
        ...
        var file_list_str = string.Join(
            ",",
            file_list.Select(f => $"\"{Path.GetFileName(f)}\"").Distinct()
        );
        ...
        if (enable_threads)
            wasm_runtime_dir = Path.Combine(
                wasm_runtime_path,
                use_release_runtime ? "threads-release" : "threads-debug"
            );
        else if (enable_dynamic_runtime)
            wasm_runtime_dir = Path.Combine(
                wasm_runtime_path,
                use_release_runtime ? "dynamic-release" : "dynamic-debug"
            );
        else
            wasm_runtime_dir = Path.Combine(
                wasm_runtime_path,
                use_release_runtime ? "release" : "debug"
            );
        ...
        if (!emit_ninja)
        {
            ...
            foreach (var fname in interp_files)
            {
                ...
                File.Copy(Path.Combine(wasm_runtime_dir, fname), Path.Combine(out_prefix, fname));
            }
            ...
            foreach (var asset in assets)
            {
                CopyFile(
                    asset,
                    Path.Combine(out_prefix, Path.GetFileName(asset)),
                    copyType,
                    "Asset: "
                );
            }
        ...
        foreach (var a in assemblies)
        {
            ...
            {
                Console.WriteLine(
                    "Duplicate input assembly: " + assembly + " " + filenames[filename]
                );
                return 1;
            }
        ...
        if (aot_profile != null)
        {
            CopyFile(
                aot_profile,
                Path.Combine(builddir, Path.GetFileName(aot_profile)),
                CopyType.IfNewer,
                ""
            );
        ...
        ninja.WriteLine(
            "cross = $mono_sdkdir/wasm-cross-release/bin/wasm32-unknown-none-mono-sgen"
        );
        ninja.WriteLine("emcc = source $emsdk_env && emcc");
        ninja.WriteLine("wasm_opt = $emscripten_sdkdir/upstream/bin/wasm-opt");
        ninja.WriteLine(
            $"emcc_flags = -Oz -g {emcc_flags}-s DISABLE_EXCEPTION_CATCHING=0 -s ALLOW_MEMORY_GROWTH=1 -s TOTAL_MEMORY=134217728 -s NO_EXIT_RUNTIME=1 -s ERROR_ON_UNDEFINED_SYMBOLS=1 -s \"EXTRA_EXPORTED_RUNTIME_METHODS=[\'ccall\', \'cwrap\', \'setValue\', \'getValue\', \'UTF8ToString\']\" -s \"EXPORTED_FUNCTIONS=[\'___cxa_is_pointer_type\', \'___cxa_can_catch\']\" -s \"DEFAULT_LIBRARY_FUNCS_TO_INCLUDE=[\'setThrew\', \'memset\']\""
        );
        ninja.WriteLine(
            $"aot_base_args = llvmonly,asmonly,no-opt,static,direct-icalls,deterministic,{aot_args}"
        );

        // Rules
        ninja.WriteLine("rule aot");
        ninja.WriteLine(
            $"  command = MONO_PATH=$mono_path $cross --debug {profiler_aot_args} --aot=$aot_args,$aot_base_args,depfile=$depfile,llvm-outfile=$outfile $src_file"
        );
        ninja.WriteLine("  description = [AOT] $src_file -> $outfile");
        ninja.WriteLine("rule aot-instances");
        ninja.WriteLine(
            $"  command = MONO_PATH=$mono_path $cross --debug {profiler_aot_args} --aot=$aot_base_args,llvm-outfile=$outfile,dedup-include=$dedup_image $src_files"
        );
        ...
        ninja.WriteLine(
            $"  command = bash -c '$emcc $emcc_flags {emcc_link_flags} -o $out_js --js-library $tool_prefix/src/library_mono.js --js-library $tool_prefix/src/dotnet_support.js {wasm_core_support_library} $in' {strip_cmd}"
        );
        ninja.WriteLine("  description = [EMCC-LINK] $in -> $out_js");
        ninja.WriteLine("rule linker");
        ninja.WriteLine(
            "  command = mono $tools_dir/monolinker.exe -out $builddir/linker-out -l none --deterministic --disable-opt unreachablebodies --exclude-feature com,remoting,etw $linker_args || exit 1; mono $tools_dir/wasm-tuner.exe --gen-empty-assemblies $out"
        );
        ninja.WriteLine("  description = [IL-LINK]");
        ninja.WriteLine("rule aot-instances-dll");
        ninja.WriteLine(
            "  command = echo > aot-instances.cs; csc /deterministic /out:$out /target:library aot-instances.cs"
        );
        ...
        ninja.WriteLine(
            "  command = mono $tools_dir/wasm-tuner.exe --gen-icall-table $runtime_table $in > $out"
        );
        ninja.WriteLine("rule gen-pinvoke-table");
        ninja.WriteLine(
            "  command = mono $tools_dir/wasm-tuner.exe --gen-pinvoke-table $pinvoke_libs $in > $out"
        );
        ...
        ninja.WriteLine(
            $"build $appdir/{runtimeTemplateOutputName}: cpifdiff $builddir/{runtimeTemplateOutputName}"
        );
        ...
        if (build_wasm)
        {
            ...
            if (add_binding)
            {
                var bindings_source_file = Path.GetFullPath(
                    Path.Combine(src_prefix, "corebindings.c")
                );
                ninja.WriteLine($"build $builddir/corebindings.c: cpifdiff {bindings_source_file}");

                ninja.WriteLine(
                    $"build $builddir/corebindings.o: emcc $builddir/corebindings.c | $emsdk_env"
                );
            ...
            ninja.WriteLine(
                $"build $builddir/driver.o: emcc $builddir/driver.c | $emsdk_env $builddir/driver-gen.c {driver_deps}"
            );
            ninja.WriteLine(
                $"  flags = {driver_cflags} -DDRIVER_GEN=1 -I{runtime_dir}/include/mono-2.0"
            );
            ninja.WriteLine(
                $"build $builddir/pinvoke.o: emcc $builddir/pinvoke.c | $emsdk_env {driver_deps}"
            );
            ninja.WriteLine(
                $"  flags = {driver_cflags} -DDRIVER_GEN=1 -I{runtime_dir}/include/mono-2.0"
            );
            ...
            if (enable_zlib)
            {
                ...
                ninja.WriteLine(
                    $"build $builddir/zlib-helper.o: emcc $builddir/zlib-helper.c | $emsdk_env"
                );
        ...
        else
        {
            ...
            {
                ninja.WriteLine(
                    "build $appdir/dotnet.worker.js: cpifdiff $wasm_runtime_dir/dotnet.worker.js"
                );
            }
        }
        if (enable_aot)
        ...
        foreach (var a in assemblies)
        {
            ...
            else
            {
                ...
                {
                    ninja.WriteLine(
                        $"build $builddir/{filename_pdb}: cpifdiff {source_file_path_pdb}"
                    );
            ...
            if (emit_pdb && infile_pdb != "")
                ninja.WriteLine(
                    $"build $appdir/$deploy_prefix/{filename_pdb}: cpifdiff {infile_pdb}"
                );
            if (a.aot)
            {
        ...
        if (gen_pinvoke)
        {
            ...
            ninja.WriteLine(
                $"build $builddir/pinvoke-table.h: cpifdiff $builddir/pinvoke-table.h.tmp"
            );
            ninja.WriteLine(
                $"build $builddir/pinvoke-table.h.tmp: gen-pinvoke-table {pinvoke_assemblies}"
            );
        ...
        if (build_wasm)
        {
            ...
            ninja.WriteLine(
                $"build $appdir/dotnet.js $appdir/dotnet.wasm: emcc-link $builddir/driver.o $builddir/pinvoke.o {zlibhelper} {wasm_core_bindings} {ofiles} {profiler_libs} {extra_link_libs} {runtime_libs} | $tool_prefix/src/library_mono.js $tool_prefix/src/dotnet_support.js {wasm_core_support} $emsdk_env"
            );
        ...
        switch (copyType)
        {
            ...
            case CopyType.IfNewer:
                ...
                else
                {
                    ...
                    if (
                        srcInfo.LastWriteTime.Ticks > dstInfo.LastWriteTime.Ticks
