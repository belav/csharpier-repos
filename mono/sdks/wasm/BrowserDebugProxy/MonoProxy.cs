...
namespace Microsoft.WebAssembly.Diagnostics
{
    internal class MonoProxy : DevToolsProxy
    {
        ...
        )
        {
            switch (method)
            {
                ...
                case "Debugger.paused":
                {
                    ...
                    if (
                        top_func == "mono_wasm_fire_bp"
                        || top_func == "_mono_wasm_fire_bp"
                        || top_func == "_mono_wasm_fire_exception"
                    )
                    {
            ...
            switch (method)
            {
                case "Target.attachToTarget":
                {
                    ...
                    await DeleteWebDriver(
                        new SessionId(resp.Value["sessionId"]?.ToString()),
                        token
                    );
                    break;
                }
                ...
                case "Runtime.releaseObject":
                {
                    if (
                        !(
                            DotnetObjectId.TryParse(args["objectId"], out var objectId)
        ...
        async Task<Result> RuntimeGetProperties(
            MessageId id,
        ...
        )
        {
            ...
            if (objectId.Scheme == "cfo_res")
            {
                ...
                var value_json_str = res.Value["result"]?["value"]?[
        ...
        async Task<bool> OnPause(SessionId sessionId, JObject args, CancellationToken token)
        {
            ...
            foreach (var frame in orig_callframes)
            {
                ...
                )
                {
                    ...
                    {
                        var exception_obj_id = await SendMonoCommand(
                            sessionId,
                    ...
                    int frame_id = 0;
                    var the_mono_frames = res.Value?["result"]?["value"]?[
                        "frames"
                    ]?.Values<JObject>();
        ...
        async Task<Breakpoint> SetMonoBreakpoint(
            SessionId sessionId,
            string reqId,
        ...
        async Task<DebugStore> LoadStore(SessionId sessionId, CancellationToken token)
        {
            ...
            try
            {
                ...
                {
                    var loaded = await SendMonoCommand(
                        sessionId,
        ...
        async Task<bool> GetPossibleBreakpoints(
            MessageId msg,
        ...
        async Task<bool> OnGetScriptSource(
            MessageId msg_id,
            string script_id,
            CancellationToken token
        )
        ...
        async Task DeleteWebDriver(SessionId sessionId, CancellationToken token)
        {
            ...
            if (hideWebDriver && sessions.Add(sessionId))
            {
                var res = await SendCommand(
                    ...
                    JObject.FromObject(
                        new { source = "delete navigator.constructor.prototype.webdriver" }
                    ),
                    token
                );
