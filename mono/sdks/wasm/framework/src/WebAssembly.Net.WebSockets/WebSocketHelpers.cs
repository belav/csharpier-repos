...
namespace WebAssembly.Net.WebSockets
{
    internal static class WebSocketHelpers
    {
        ...
        private static readonly ArraySegment<byte> s_EmptyPayload = new ArraySegment<byte>(
            new byte[] { },
            0,
            0
        );
        ...
        internal static void ValidateSubprotocol(string subProtocol)
        {
            if (string.IsNullOrWhiteSpace(subProtocol))
            {
                throw new ArgumentException(
                    "Protocol string can not be null or empty",
                    nameof(subProtocol)
                );
            }
        ...
        )
        {
            ...
            {
                throw new ArgumentException(
                    $"The close status description '{statusDescription}' is invalid. When using close status code '{WebSocketCloseStatus.Empty}' the description must be null.",
            ...
            )
            {
                ...
                throw new ArgumentException(
                    $"The close status code '{closeStatusCode}' is reserved for system use only and cannot be specified when calling this method.",
                    nameof(closeStatus)
                );
            }
        }
