...
namespace WebAssembly
{
    ...
    public sealed class Runtime
    {
        ...
        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        internal static extern object NewObjectJS(
            int js_obj_handle,
            object[] _params,
            out int exceptional_result
        );
        ...
        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        internal static extern object BindCoreObject(
            int js_obj_handle,
            int gc_handle,
            out int exceptional_result
        );
        ...
        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        internal static extern object BindHostObject(
            int js_obj_handle,
            int gc_handle,
            out int exceptional_result
        );
        ...
        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        internal static extern object New(
            string className,
            object[] _params,
            out int exceptional_result
        );
        ...
        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        internal static extern object TypedArrayToArray(
            int js_obj_handle,
            out int exceptional_result
        );
        ...
        static int BindJSType(int js_id, Type mappedType)
        {
            if (!bound_objects.TryGetValue(js_id, out JSObject obj))
            {
                var jsobjectnew = mappedType.GetConstructor(
                    BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.ExactBinding,
                    null,
                    new Type[] { typeof(IntPtr) },
                    null
                );
                bound_objects[js_id] = obj = (JSObject)
                    jsobjectnew.Invoke(new object[] { (IntPtr)js_id });
            }
            return (int)(IntPtr)obj.Handle;
        }
            ...
            if (obj != null && obj.RawObject != null)
            {
                ...
                if (exception != 0)
                    throw new JSException(
                        $"Error releasing handle on (js-obj js '{obj.JSHandle}' mono '{(IntPtr)obj.Handle} raw '{obj.RawObject != null})"
                    );
        ...
        static double GetDateValue(object dtv)
        {
            ...
            if (!(dtv is DateTime))
            {
                throw new InvalidCastException(
                    $"Unable to cast object of type {dtv.GetType()} to type DateTime."
                );
            }
            var dt = (DateTime)dtv;
        ...
        public static Enum EnumFromExportContract(Type enumType, object value)
        {
            ...
            if (value is string)
            {
                ...
                foreach (var fi in fields)
                {
                    ...
                    ExportAttribute[] attributes = (ExportAttribute[])
                        fi.GetCustomAttributes(typeof(ExportAttribute), false);
                    ...
                    switch (enumConversionType)
                    {
                        ...
                        case ConvertEnum.Numeric:
                            contractName = (int)
                                Enum.Parse(value.GetType(), contractName.ToString());
                            break;
                        default:
                ...
                throw new ArgumentException(
                    $"Value is a name, but not one of the named constants defined for the enum of type: {enumType}.",
                    nameof(value)
                );
            }
            else
        ...
        internal static unsafe void DumpAotProfileData(ref byte buf, int len, string s)
        {
            ...
            fixed (void* p = &buf)
            {
                ...
                js_dump.SetObjectProperty(
                    "aot_profile_data",
                    WebAssembly.Core.Uint8Array.From(span)
                );
            }
        }
