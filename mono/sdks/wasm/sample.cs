...
namespace GeoLocation
{
    class Program
    {
        ...
        static void GeoFindMe(JSObject output)
        {
                ...
                using (position)
                {
                    using (var coords = position.Coordinates)
                    {
                        ...
                        try
                        {
                            ...
                            if (rspMsg.IsSuccessStatusCode)
                            {
                                var mimeType = getMimeType(
                                    rspMsg.Content?.ReadAsByteArrayAsync().Result
                                );
                                Console.WriteLine(
                                    $"Request: {++requests}  ByteAsync: {rspMsg.Content?.ReadAsByteArrayAsync().Result.Length}  MimeType: {mimeType}"
                                );
                                global.Invoke(
                                    "showMyPosition",
                                    mimeType,
                            ...
                            else
                            {
                                output.SetObjectProperty(
                                    "innerHTML",
                                    $"<p>Latitude is {latitude} ° <br>Longitude is {longitude} </p><br>StatusCode: {rspMsg.StatusCode} <br>Response Message: {rspMsg.Content?.ReadAsStringAsync().Result}</p>"
                                );
                            }
                        }
                        catch (Exception exc2)
                        {
                            ...
                            Console.WriteLine(
                                $"GeoLocation HttpClient InnerException: {exc2.InnerException?.Message}"
                            );
                        }
                    }
                }
            ...
            geoLocation.OnError += (object sender, PositionError e) =>
            {
                output.SetObjectProperty(
                    "innerHTML",
                    $"Unable to retrieve your location: Code: {e.Code} - {e.message}"
                );
            };
        ...
        public DOMObject(object jsobject)
        {
            ...
            if (ManagedJSObject == null)
                throw new NullReferenceException(
                    $"{nameof(jsobject)} must be of type JSObject and non null!"
                );
        }

        public DOMObject(string globalName)
            : this((JSObject)Runtime.GetGlobalObject(globalName)) { }
    ...
    class GeoLocation : DOMObject
    {
        ...
        public GeoLocation(object jsobject)
            : base(jsobject) { }

        public void GetCurrentPosition()
        {
            var success = new Action<object>(
                (pos) =>
                {
    ...
    {
        public Position(object jsobject)
            : base(jsobject) { }

        public Coordinates Coordinates =>
            new Coordinates(ManagedJSObject.GetObjectProperty("coords"));
    }

    class PositionError : DOMObject
    {
        public PositionError(object jsobject)
            : base(jsobject) { }
    ...
    {
        public Coordinates(object jsobject)
            : base(jsobject) { }
