...
public class SimThread
{
    ...
    public void Lock(SimLock lk, string frame)
    {
        foreach (LockRecord lr in locks)
        {
            if (lk.WarnAbout(this, lr.lk))
                Console.WriteLine(
                    "WARNING: tried to acquire lock {0} at {1} while holding {2} at {3}: {4}",
                    lk,
                    frame,
                    lr.lk,
                    lr.frame,
                    lk.GetWarningMessage(this, lr.lk)
                );
            else if (!lk.IsValid(this, lr.lk))
                Console.WriteLine(
                    "ERROR: tried to acquire lock {0} at {1} while holding {2} at {3}: {4}",
                    lk,
                    frame,
                    lr.lk,
                    lr.frame,
                    lk.GetErrorMessage(this, lr.lk)
                );
        }
    ...
    public void Release(SimLock lk, string frame)
    {
        ...
        {
            Console.WriteLine(
                "WARNING: released lock {0} at {1} out of order with {2} at {3}!",
                lk,
                frame,
                top.lk,
                top.frame
        ...
        if (locked != this)
        {
            if (!IsParent(locked))
            {
                ...
                { /*acquiring a global lock*/
                    ...
                    { /*does the thread alread hold it?*/
...
public class LockSimulator
{
    ...
    public LockSimulator(SymbolTable s)
    {
        this.syms = s;
    }
