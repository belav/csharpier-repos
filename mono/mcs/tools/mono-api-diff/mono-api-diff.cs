...
namespace Mono.ApiTools
{
    ...
    class Driver
    {
        static int Main(string[] args)
        {
            ...
            {
                Console.WriteLine(
                    @"Usage: mono-api-diff.exe [options] <assembly 1 xml> <assembly 2 xml>"
                );
                Console.WriteLine();
    ...
    class XMLAssembly : XMLData
    {
        ...
        public override void CompareTo(XmlDocument doc, XmlNode parent, object other)
        {
            ...
            if (version != assembly.version)
                AddWarning(
                    childA,
                    "Assembly version not equal: {0}, {1}",
                    version,
                    assembly.version
                );
        ...
        public override void LoadData(XmlNode node)
        {
            ...
            if (classes.Name != "classes")
                throw new FormatException(
                    $"Expecting <classes>. Got <{classes.Name}> (namespace {name})."
                );
    ...
    class XMLClass : XMLData
    {
        ...
        public override void LoadData(XmlNode node)
        {
            ...
            {
                throw new FormatException(
                    $"Expecting <classes>. Got <{child.Name}> ({type} {name})."
                );
            }
    ...
    class XMLParameter : XMLData
    {
        ...
        public override void CompareTo(XmlDocument doc, XmlNode parent, object other)
        {
            ...
            if (defaultValue != oparm.defaultValue)
                AddWarning(
                    parent,
                    "Parameter default value wrong: {0} != {1}",
                    (defaultValue == null) ? "(no default value)" : defaultValue,
                    (oparm.defaultValue == null) ? "(no default value)" : oparm.defaultValue
                );
            ...
            foreach (DictionaryEntry de in other_properties)
            {
                ...
                if (de.Value == null)
                {
                    if (other_value != null)
                        AddWarning(
                            parent,
                            "Property '{0}' is 'null' and should be '{1}'",
                            de.Key,
                            other_value
                        );
                    continue;
                ...
                AddWarning(
                    parent,
                    "Property '{0}' is '{1}' and should be '{2}'",
                    de.Key,
                    de.Value,
                    other_value == null ? "null" : other_value
                );
            }
        }
    ...
    class XMLAttributes : XMLNameGroup
    {
        ...
        protected override bool CheckIfAdd(string value, XmlNode node)
        {
            if (IsMonoTODOAttribute(value))
            {
                ...
                if (
                    pNode != null
                    && pNode.ChildNodes.Count > 0
                    && pNode.ChildNodes[0].Attributes["value"] != null
                )
                {
        ...
        {
            XMLAttributeProperties other_prop =
                ((XMLAttributes)other).properties[name] as XMLAttributeProperties;
    ...
    abstract class XMLGenericGroup : XMLNameGroup
    {
        ...
        protected override void CompareToInner(string name, XmlNode parent, XMLNameGroup other)
        {
            ...
            if (attributes != g.attributes)
                AddWarning(
                    parent,
                    "Incorrect generic attributes: '{0}' != '{1}'",
                    attributes,
                    g.attributes
                );
        }
        ...
        protected override void CompareToInner(string name, XmlNode parent, XMLNameGroup other)
        {
            ...
            try
            {
                ...
                SignatureFlags flags =
                    signatureFlags != null && signatureFlags.ContainsKey(name)
                        ? (SignatureFlags)signatureFlags[name]
                        : SignatureFlags.None;
                SignatureFlags oflags =
                    methods.signatureFlags != null && methods.signatureFlags.ContainsKey(name)
