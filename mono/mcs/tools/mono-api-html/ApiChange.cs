    ...
    class ApiChanges : Dictionary<string, List<ApiChange>>
    {
        ...
        public void Add(XElement source, XElement target, ApiChange change)
        {
                ...
                if (!change.HasIgnoredChanges)
                {
                    ...
                    if (isField)
                    {
                        State.LogDebugMessage(
                            $"Comparison resulting in no changes (src: {source.GetFieldAttributes()} dst: {target.GetFieldAttributes()}) :{Environment.NewLine}{source}{Environment.NewLine}{target}{Environment.NewLine}{Environment.NewLine}"
                        );
                    }
                    else
                    {
                        State.LogDebugMessage(
                            $"Comparison resulting in no changes (src: {source.GetMethodAttributes()} dst: {target.GetMethodAttributes()}) :{Environment.NewLine}{source}{Environment.NewLine}{target}{Environment.NewLine}{Environment.NewLine}"
                        );
                    }
                }
                return;
            ...
            var changeDescription =
                $"{State.Namespace}.{State.Type}: {change.Header}: {change.SourceDescription}";
