...
namespace Mono.AssemblyLinker
{
    ...
    public class AssemblyLinker
    {
        ...
        private string GetCommand(string str, out string command_arg)
        {
            ...
            string command = str.Substring(1, end_index == -1 ? str.Length - 1 : end_index - 1);
        ...
        private void AddCattr(System.Type attrType, System.Type arg, object value)
        {
            ...
            cattrs.Add(
                new CustomAttributeBuilder(
                    importedAttrType.GetConstructor(new[] { importedArg }),
                    new[] { value }
                )
            );
        }
        ...
        private void ReportInvalidArgument(string option, string value)
        {
            Report(
                1012,
                String.Format("'{0}' is not a valid setting for option '{1}'", value, option)
            );
        }
        ...
        private void ReportMissingArgument(string option)
        {
            Report(
                1003,
                String.Format("Compiler option '{0}' must be followed by an argument", option)
            );
        }
        ...
        private void ReportMissingFileSpec(string option)
        {
            Report(
                1008,
                String.Format("Missing file specification for '{0}' command-line option", option)
            );
        }
                    ...
                    catch (CryptographicException)
                    {
                        if (delaysign != DelaySign.Yes)
                        {
                            if (data.Length == 16)
                            {
                                ...
                                Report(
                                    1019,
                                    "Could not strongname the assembly. "
                            ...
                            else
                            {
                                Report(
                                    1028,
                                    String.Format(
                                        "Key file {0}' is missing it's private key "
...
*/
            ...
            foreach (ResourceInfo res in resources)
            {
                ...
                if (res.isEmbedded)
                {
                    ...
                    mainModule.DefineManifestResource(
                        res.name,
                        stream,
                ...
                else
                {
                    ...
                    if (
                        Path.GetDirectoryName(res.fileName) != null
                        || Path.IsPathRooted(res.fileName)
                    )
                    {
                        ReportWarning(
                            99999,
                            String.Format(
                                "Path '{0}' in the resource name is not supported. Using just file name '{1}'",
                                res.fileName,
                                resourceFileName
                            )
                        );
        ...
        private AssemblyName ReadCustomAttributesFromTemplateFile(
            string templateFile,
            AssemblyName aname
        )
        {
                    ...
                    case "System.Reflection.AssemblyKeyFileAttribute":
                        {
                            ...
                            if (!String.IsNullOrEmpty(key_file_value))
                                keyfile = Path.Combine(
                                    Path.GetDirectoryName(templateFile),
                                    key_file_value
                                );
                        }
                        break;
                    ...
                    case "System.Reflection.AssemblyDescriptionAttribute":
                        {
                            ...
                            string description_value = (string)
                                attr_data.ConstructorArguments[0].Value;
