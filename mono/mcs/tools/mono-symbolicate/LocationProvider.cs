...
namespace Mono
{
    class AssemblyLocationProvider
    {
        ...
        )
        {
            ...
            using (
                var assembly = AssemblyDefinition.ReadAssembly(assemblyFullPath, readerParameters)
            )
            {
                ...
                else
                {
                    ...
                    ilOffset = seqPointInfo.GetILOffset(
        ...
        static void FormatName(TypeReference tr, StringBuilder builder, char sep)
        {
            if (
                tr.IsNested
                && !(tr.MetadataType == MetadataType.Var || tr.MetadataType == MetadataType.MVar)
            )
            {
