...
namespace SqlEditorSharp
{
    ...
    public class SqlEditorSharp : Gtk.VBox
    {
        ...
        {
            get { return sqlTextBuffer; }
        }

        public TextView View
        {
            get { return sqlTextView; }
        }
        ...
        {
            get { return use_hi_lighting; }
            set { use_hi_lighting = value; }
        }
            ...
            while (start_iter.IsEnd == false)
            {
                ...
                )
                {
                    ...
                    )
                    {
                        ...
                        sqlTextBuffer.ApplyTag(freecomment_tag, match_start1, match_end2);
                    ...
                    else
                    {
                        ...
                        ApplyTag(freecomment_tag, normaltext_tag, match_start1, end_iter);
                        break;
                    }
                }
                else
                    ...
                    switch (text[i])
                    {
                        case '-':
                            if (hyphen == 1)
                            {
                                ...
                                ApplyTag(linecomment_tag, normaltext_tag, start_iter, iter);
                            }
                            else
                            {
                                hyphen = 1;
        ...
        void LookForSingleQuotesAndWords(
            ref TextIter start_iter,
            string text,
            int char_count,
            ref int start_word,
        ...
        )
        {
            ...
            for (i = 0; i < char_count; i++)
            {
                ...
                if (CharHasTag(start_iter, freecomment_tag, i) == false)
                {
                    ...
                    else if (start_word != -1)
                    {
                        ...
                        else
                        {
                            ...
                            if (IsTextSQL(text, start_word, i))
                            {
                                ...
                                // hi-light word
                                ApplyTagOffsets(start_iter, start_word, i, sql_tag, normaltext_tag);
                            }
                            start_word = -1;
                            switch (text[i])
                            {
        ...
        void ApplyTag(TextTag apply_tag, TextTag remove_tag, TextIter start_iter, TextIter end_iter)
        {
#if DEBUG
            DebugText(
                start_iter,
                end_iter,
                "ApplyTag() " + "remove: " + remove_tag.Name + " apply: " + apply_tag.Name
            );
#endif // DEBUG

            sqlTextBuffer.RemoveTag(remove_tag, start_iter, end_iter);

            sqlTextBuffer.ApplyTag(apply_tag, start_iter, end_iter);
        }
        ...
        )
        {
            ...
            sqlTextBuffer.RemoveTag(remove_tag, begin_iter, end_iter);

            sqlTextBuffer.ApplyTag(apply_tag, begin_iter, end_iter);
        }
        ...
        bool IsTextSQL(string text, int begin, int end)
        {
            ...
            for (i = 0; sql_keywords[i] != String.Empty; i++)
            {
                ...
                {
                    SqlSharpGtk.DebugWriteLine("Test length: " + text_len + " keyword: " + keyword);
                    ...
                    {
                        Console.WriteLine(
                            "Internal Error: SqlSharpGtk: text.Substring() ArgumentOutOfRange"
                        );
                    }
        ...
        void DebugText(TextIter iter_start, TextIter iter_end, string debugMessage)
        {
#if DEBUG
            string text = sqlTextBuffer.GetText(iter_start, iter_end, false);
            string msg = "[DEBUG-TEXT]: " + debugMessage + " (" + text + ")";
            SqlSharpGtk.DebugWriteLine(msg);
#endif // DEBUG
        }

        static readonly string[] sql_keywords = new string[]
        {
            "DELETE",
            "FROM",
            "SELECT",
