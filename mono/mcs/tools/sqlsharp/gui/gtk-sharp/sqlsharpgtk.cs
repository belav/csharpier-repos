...
namespace Mono.Data.SqlSharp.Gui.GtkSharp
{
    ...
    public class SqlSharpGtk
    {
        ...
        public void CreateGui()
        {
            ...
            sourceFileNotebook.SwitchPage += new GtkSharp.SwitchPageHandler(OnEditorTabSwitched);
        ...
        EditorTab NewEditorTab()
        {
            ...
            editor.View.Show();
            editor.View.KeyPressEvent += new GtkSharp.KeyPressEventHandler(OnKeyPressEventKey);

            lastUnknownFile++;
            string unknownFile = "Unknown" + lastUnknownFile.ToString() + ".sql";
        ...
        Toolbar CreateToolbar()
        {
            ...
            toolbar.AppendItem(
                "DataGrid",
                "Toggle Results to DataGrid or TextView",
                String.Empty,
        ...
        public void LoadProviders()
        {
            ...
            providerList.Add(new DbProvider("SQLCLIENT", "Microsoft SQL Server", "", "", "", true));
            providerList.Add(
                new DbProvider(
                    "TDS",
                    "TDS Generic",
            ...
            providerList.Add(new DbProvider("ODBC", "ODBC", "", "", "", true));
            providerList.Add(new DbProvider("OLEDB", "OLE DB", "", "", "", true));
            providerList.Add(
                new DbProvider(
                    "SQLITE",
                    "SQL Lite",
        ...
        public bool LoadExternalProvider(string strProviderAssembly, string providerConnectionClass)
        {
            try
            {
        ...
        {
            FileSelectionDialog openFileDialog = new FileSelectionDialog(
                "Open File",
        ...
        void ExecuteSQL(ExecuteOutputType outputType, string filename)
        {
            ...
            {
                AppendText(buf, "Error: Unable to create command to execute: " + ec.Message);
                return;
            }
            ...
            {
                AppendText(buf, "Error: Unable to get text from SQL editor: " + et.Message);
                return;
            }
            ...
            if (
                (outputResults == OutputResults.TextView && outputType == ExecuteOutputType.Normal)
                ...
                else
                {
                    ...
                    switch (outputType)
                    {
                        ...
                        case ExecuteOutputType.XmlFile:
                            ...
                            AppendText("Write DataSet to XML file: " + filename);
        ...
        public void OutputDataToHtmlFile(IDataReader rdr, DataTable dt, string file)
        {
            ...
            {
                Error("Error: Unable to setup output results file. " + e.Message);
                return;
            }
        ...
        public void DisplayResult(IDataReader reader, DataTable schemaTable)
        {
            ...
            try
            {
                OutputLine("Fields in Query Result: " + reader.FieldCount);
            }
            catch (Exception e)
            {
                Error("Error: Unable to get FieldCount: " + e.Message);
                return;
            }
            ...
            do
            {
                ...
                else if (reader.RecordsAffected >= 0)
                {
                    ...
                    try
                    {
                        ...
                        AppendText(buf, "SQL Command Records Affected: " + records);
                    }
                    catch (Exception er)
                    {
                        Error("Error: Unable to get records affected: " + er.Message);
                        return;
                    }
                }
                else
        ...
        bool OpenInternalProvider()
        {
            ...
            switch (providerKey.ToUpper())
            {
                case "SQLCLIENT":
                    ...
                    {
                        msg = "Error: unable to create connection: " + e.Message;
                        Error(msg);
                        return false;
                    }
                    break;
        ...
        bool OpenExternalProvider()
        {
            ...
            success = LoadExternalProvider(dbProvider.Assembly, dbProvider.ConnectionClass);
        ...
        public DbDataAdapter CreateInternalDataAdapter(IDbCommand cmd)
        {
            ...
            switch (providerKey.ToUpper())
            {
                case "SQLCLIENT":
                    ...
                    {
                        msg = "Error: unable to create adapter: " + e.Message;
                        Error(msg);
                        return null;
                    }
                    break;
        ...
        public DataTable LoadDataTable(IDbCommand dbcmd)
        {
            ...
            {
                AppendText(
                    "Rows successfully Added or Refreshed in the DataTable: " + rowsAddedOrRefreshed
                );
                ...
                if (fields > 0)
                {
                    status = "Rows Selected: " + rowsAddedOrRefreshed + "  Fields: " + fields;
                }
                else
                {
        ...
        public bool OpenDataSource()
        {
            ...
            {
                msg = "Error: Unable to create Connection object. " + e.Message;
                Error(msg);
                return false;
            }
