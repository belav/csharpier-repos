...
namespace Mono.Data.SqlSharp
{
    ...
    public class SqlSharpCli
    {
        ...
        public void BuildParameters(IDbCommand cmd)
        {
            ...
            {
                ParametersBuilder parmsBuilder = new ParametersBuilder(
                    cmd,
                ...
                for (int p = 0; p < parms.Count; p++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Console.Write(
                                "Enter Parameter " + (p + 1).ToString() + ": " + theParmName + ": "
                            );
                            inValue = Console.ReadLine();
                            prm.Value = inValue;
                        }
        ...
        public void ChangeProviderBackwardsCompat(string[] parms)
        {
            ...
            if (parms.Length == 2)
            {
                ...
                if (provider.Equals("LOADEXTPROVIDER"))
                {
                    OutputLine("          Assembly: " + providerAssembly);
                    OutputLine("  Connection Class: " + providerConnectionClass);
                }
            }
            else
                Console.WriteLine("Error: provider only has one parameter.");
        ...
        {
            if (parms.Length >= 2)
                connectionString = entry.Substring(
                    parms[0].Length,
                    entry.Length - (parms[0].Length + 1)
                );
            else
                connectionString = "";
        }
        ...
        public void ShowInternalVariable(string[] parms)
        {
            ...
            if (GetInternalVariable(parm, out internalVariableValue) == true)
                Console.WriteLine(
                    "Internal Variable - Name: " + parm + "  Value: " + internalVariableValue
                );
        }
        ...
        public void Run(string[] args)
        {
            ...
            if (silent == false)
            {
                ...
                Console.WriteLine(
                    "for Mono.Data.  See http://www.mono-project.com/ for more details.\n"
                );
            ...
            while (
                entry.ToUpper().Equals("\\Q") == false && entry.ToUpper().Equals("\\QUIT") == false
            )
            {
    ...
    public class ParametersBuilder
    {
        ...
        public int ParseParameters()
        {
            ...
            for (int i = 0; i < chars.Length; i++)
            {
                ...
                else if (chars[i] == bindChar && bStringConstFound == false)
                {
                    if (bindChar != '?')
                    {
                        ...
                        if (bindChar.Equals('['))
                        {
                            ...
                            while (i <= chars.Length)
                            {
                                ...
                                else if (ch == ']')
                                {
                                    ...
                                    throw new Exception(
                                        "SQL Parser Error: Invalid character in parameter name"
                                    );
                                i++;
                            }
                            i--;
                            if (endingBracketFound == false)
                                throw new Exception(
                                    "SQL Parser Error: Ending bracket not found for parameter"
                                );
                        }
                        else
                        {
