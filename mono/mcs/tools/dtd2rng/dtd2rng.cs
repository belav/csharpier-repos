...
"
        ...
        XmlSchema GetXmlSchema(XmlTextReader xtr)
        {
            ...
            MethodInfo mi = dtd.GetType().GetMethod("CreateXsdSchema", flag);
        ...
        RelaxngPattern CreatePatternFromType(XmlSchemaType type)
        {
            ...
            if (st == null)
                throw new NotSupportedException(
                    "Complex types are not supported as an attribute type."
                );
            XmlSchemaSimpleTypeRestriction r = st.Content as XmlSchemaSimpleTypeRestriction;
            if (r == null)
                throw new NotSupportedException(
                    "Only simple type restriction is supported as an attribute type."
                );

            RelaxngChoice c = new RelaxngChoice();
            foreach (XmlSchemaFacet f in r.Facets)
            {
                XmlSchemaEnumerationFacet en = f as XmlSchemaEnumerationFacet;
                if (en == null)
                ...
                v.DatatypeLibrary = RemapDatatypeLibrary(r.BaseTypeName.Namespace);
                v.Value = en.Value;
                c.Patterns.Add(v);
            }
            return c;
        ...
        RelaxngPattern CreatePatternFromTypeName(XmlQualifiedName name)
        {
            ...
            data.Type = name.Name;
            data.DatatypeLibrary = RemapDatatypeLibrary(name.Namespace);
            return data;
        }

        string RemapDatatypeLibrary(string ns)
        {
            return ns == XmlSchema.Namespace ? "http://www.w3.org/2001/XMLSchema-datatypes" : ns;
        }
    }
}
