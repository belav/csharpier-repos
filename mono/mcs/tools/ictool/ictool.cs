...
public class ICTool
{
    ...
    private static void EmitPrototypes()
    {
        ...
        map_file.Write("static gpointer icall_map [] = {\n\t");
        ...
        foreach (Type type in types.Values)
        {
            ...
            foreach (MethodInfo method in methods)
            {
                ...
                string func_name = String.Format("ves_icall_{0}_{1}", type.FullName, method.Name);
                ...
                methods_file.WriteLine("{0} ({1});", func_name, Join(", ", args));
                methods_file.WriteLine();
            }
        }
    ...
    private static void EmitStructures()
    {
        ...
        foreach (Peer peer in dg.TopologicalSort())
        {
            ...
            if (peer.IsEnum)
            {
                ...
                {
                    enum_lines.Add(
                        String.Format("\t{0}_{1} = {2}", peer.Name, name, peer.EnumConstants[name])
                    );
                }
        ...
        foreach (XmlNode node in doc.SelectNodes("config/typemap/namespace"))
        {
            ...
            foreach (XmlNode child in node.ChildNodes)
            {
                if (child.Name == "type")
                {
                    ...
                    bool opaque = false;
                    if (
                        child.Attributes["opaque"] != null
                        && child.Attributes["opaque"].Value == "true"
                    )
                        opaque = true;
                    ...
                    if (
                        ...
                        && child.Attributes["default"].Value == "true"
                    )
