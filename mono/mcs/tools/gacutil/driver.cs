...
namespace Mono.Tools
{
    public class Driver
    {
        ...
        public static int Main(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                if (IsSwitch(args[i]))
                {
                    ...
                    {
                        WriteLine(
                            "WARNING: gacutil does not support traced references."
        ...
        )
        {
            ...
            {
                WriteLine(
                    string.Format(failure_msg, name) + "The system cannot find the file specified."
                );
                return false;
            }
            ...
            {
                WriteLine(
                    string.Format(failure_msg, name) + "The file specified is not a valid assembly."
                );
                return false;
            }
            ...
            switch (VerifyStrongName(an, name))
            {
                ...
                case VerificationResult.WeakNamed:
                    WriteLine(
                        string.Format(failure_msg, name)
            ...
            if (Path.GetFileNameWithoutExtension(asmb_file) != asmb_name)
            {
                WriteLine(
                    string.Format(failure_msg, name)
                        + string.Format(
                            "the filename \"{0}\" doesn't match the assembly name \"{1}\"",
                            asmb_file,
                            asmb_name
                        )
                );
            ...
            foreach (ManifestResourceInfo resource_info in resources)
            {
                try
                {
                    Copy(
                        ...
                        Path.Combine(full_path, Path.GetFileName(resource_info.FileName)),
                        true
                    );
                }
                catch
            ...
            if (package != null)
            {
                ...
                {
                    string pkg_path_abs = Path.Combine(
                        gacdir,
                        Path.Combine(an.Name, Path.Combine(version_token, asmb_file))
                    );
            ...
            WriteLine("Installed {0} into the gac ({1})", name, gacdir);

            return true;
        }

        //from MonoDevelop.Core.FileService
        static unsafe string AbsoluteToRelativePath(string baseDirectoryPath, string absPath)
        {
        ...
        private static bool UninstallSpecific(
            string name,
            string package,
            string gacdir,
            string libdir
        )
        {
            ...
            int uninstallFailures = 0;
            Uninstall(
                an.FullName.Replace(" ", String.Empty),
                package,
                gacdir,
                libdir,
                true,
        ...
        private static void List(string name, string gacdir)
        {
            ...
            foreach (DirectoryInfo parent in gacinfo.GetDirectories())
            {
                ...
                {
                    string asmb =
                        Path.Combine(Path.Combine(parent.FullName, dir.Name), parent.Name) + ".dll";
                    if (File.Exists(asmb))
                    {
        ...
        private static void FilteredList(string name, string gacdir)
        {
            ...
            foreach (string dir in dir_list)
            {
                ...
                {
                    WriteLine(
                        AsmbNameFromVersionString(
                            (string)asm_info["assembly"],
        ...
        private static bool InstallFromList(
            bool check_refs,
            string list_file,
            string package,
            string gacdir,
        ...
        )
        {
            ...
            int processed,
                failed;
            string listdir = Path.GetDirectoryName(Path.GetFullPath(list_file));
            ...
            try
            {
                ...
                while ((line = s.ReadLine()) != null)
                {
                    ...
                    string assemblyPath = Path.Combine(listdir, file);
        ...
        private static bool UninstallFromList(
            string list_file,
            string package,
            string gacdir,
            string libdir
        )
        {
            ...
            try
            {
                ...
                while ((line = s.ReadLine()) != null)
                {
                    ...
                    if (name.Length == 0)
                        continue;
                    Uninstall(line, package, gacdir, libdir, true, ref uninstalled, ref failed);
                }
        ...
        private static string AsmbNameFromVersionString(string name, string str)
        {
            ...
            return String.Format(
                ...
                name,
                pieces[0],
                (pieces[1] == String.Empty ? "neutral" : pieces[1]),
                pieces[2]
            );
        }
        ...
        {
            System.Reflection.MethodInfo config = typeof(System.Environment).GetMethod(
        ...
        {
            System.Reflection.PropertyInfo gac = typeof(System.Environment).GetProperty(
                "GacPath",
                System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic
            );
            if (gac == null)
            {
                WriteLine(
                    "ERROR: Mono runtime not detected, please use "
        ...
        {
            System.Reflection.MethodInfo libdir = typeof(System.Environment).GetMethod(
                "internalGetGacPath",
                System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic
            );
            if (libdir == null)
        ...
        private static void ShowHelp(bool detailed)
        {
            ...
            {
                WriteLine(
                    "\t<assembly_path> is the name of the file that contains the "
                        + "\tassembly manifest\n"
                        + "\tExample: -i myDll.dll"
                );
            }
