    ...
    public class LineEditor
    {
        ...
        void GetUnixConsoleReset()
        {
            ...
            try
            {
                var terminfo_driver = Type.GetType("System.ConsoleDriver")
                    ?.GetField("driver", BindingFlags.Static | BindingFlags.NonPublic)
                    ?.GetValue(null);
                if (terminfo_driver == null)
                    return;

                var unix_reset_colors_str =
                    (
                        terminfo_driver
                            ?.GetType()
        ...
        void ShowCompletions(string prefix, string[] completions)
        {
            ...
            if (current_completion == null)
            {
                ...
                current_completion = new CompletionState(
                    left,
                    Console.CursorTop + 1,
                    window_width,
                    window_height
        ...
        void UpdateCompletionWindow()
        {
            if (current_completion != null)
                throw new Exception(
                    "This method should only be called if the window has been hidden"
                );
        ...
        class History
        {
            ...
            {
                Console.WriteLine(
                    "Head={0} Tail={1} Cursor={2} count={3}",
                    head,
                    tail,
                    cursor,
                    count
