...
class MonoServiceRunner : MarshalByRefObject
{
    ...
    static void info(string prefix, string format, params object[] args)
    {
        Syscall.syslog(
            ...
            String.Format("{0}: {1}", prefix, String.Format(format, args))
        );
    }
    ...
    static int Main(string[] args)
    {
        ...
        if (lfp < 0)
        {
            ...
            if (File.Exists(lockfile))
                ...
                error(
                    logname,
                    String.Format("Cannot open/create lock file exclusively: {0}", lockfile)
                );
            return 1;
        }
    ...
    private void MainLoop(ServiceBase[] services)
    {
        try
        {
            ...
            UnixSignal[] sigs = new UnixSignal[] { intr, term, usr1, usr2 };
            ...
            for (bool running = true; running; )
            {
                ...
                if ((intr.IsSet || term.IsSet) && service.CanStop)
                {
                    ...
                    if (service.ExitCode != 0)
                        error(
                            logname,
                            "Service {0} stopped returning a non-zero ExitCode: {1}",
                            service.ServiceName,
                            service.ExitCode
                        );
                    running = false;
