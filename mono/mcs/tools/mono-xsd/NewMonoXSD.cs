...
namespace Mono.Util
{
    public class Driver
    {
        ...
        static readonly string missingOutputForXsdInput =
            "Can only generate one of classes or datasets.";
        static readonly string generatorAssemblyNotFound =
            "Could not load code provider assembly file: {0}";
        static readonly string generatorTypeNotFound =
            "Could not find specified code provider type: {0}";
        static readonly string generatorTypeIsNotCodeGenerator =
            "Specified code provider type was not CodeDomProvider: {0}";
        static readonly string generatorThrewException =
        ...
        public void Run(string[] args)
        {
            ...
            foreach (string arg in args)
            {
                ...
                {
                    if (
                        (arg.EndsWith(".dll") || arg.EndsWith(".exe"))
                        && !arg.Substring(1).StartsWith("generator:")
                        && !arg.Substring(1).StartsWith("g:")
                    )
                    {
                        if (!readingFiles)
            ...
            if (generatorType != null)
            {
                ...
                {
                    Error(
                        generatorThrewException,
                        generatorType.AssemblyQualifiedName.ToString() + " --> " + ex.Message
                    );
                }
            ...
            else if (inference)
            {
                ...
                {
                    string genFile = Path.Combine(
                        outputDir,
                        Path.GetFileNameWithoutExtension(xmlfile) + ".xsd"
                    );
