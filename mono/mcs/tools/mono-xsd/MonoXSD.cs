    ...
    public class MonoXSD
    {
            ...
            foreach (Type t in a.GetTypes())
            {
                ...
                catch (ArgumentException e)
                {
                    throw new ArgumentException(
                        String.Format("Error: We cannot process {0}\n{1}", assembly, e.Message)
                    );
                }
        ...
        public XmlSchemaType WriteSchemaType(Type type)
        {
            ...
            else
            {
                ...
                {
                    throw new ArgumentException(
                        String.Format("There is an error in '{0}'\n\t{1}", type.Name, e.Message)
                    );
                }
                ...
                foreach (object o in attrs)
                {
                    if (o is XmlArrayItemAttribute)
                    {
                        if (type.IsArray == false)
                            throw new ArgumentException(
                                String.Format(
                                    "XmlArrayAttribute is not applicable to {0}, because it is not an array.",
                                    member.Name
                                )
                            );
            ...
            try
            {
                foreach (FieldInfo field in fields)
                    if (IsXmlAttribute(field))
                        ...
                        attributes.Add(field, typeof(System.Xml.Schema.XmlSchemaAnyAttribute));
                    else
            ...
            try
            {
                foreach (PropertyInfo property in properties)
                    if (IsXmlAttribute(property))
                        ...
                        attributes.Add(property, typeof(System.Xml.Schema.XmlSchemaAnyAttribute));
                    else
                    {
        ...
        public void AddElement(XmlSchemaSequence sequence, MemberInfo member, Type type)
        {
            ...
            else if (schema_type_name == null)
            {
                throw new ArgumentException(
                    String.Format(
                        "The type '{0}' cannot be represented in XML Schema.",
                        type.FullName
                    )
                );
            }
        ...
        public XmlQualifiedName GetQualifiedName(Type type)
        {
            ...
            //
            if (
                type.Equals(typeof(System.Xml.XmlNode))
