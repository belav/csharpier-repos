...
class MakeBundle
{
    ...
    static string[] i18n = new string[] { "West", "" };
    static string[] i18n_all = new string[] { "CJK", "MidEast", "Other", "Rare", "West", "" };
    ...
    static int Main(string[] args)
    {
            ...
            switch (args[i])
            {
                ...
                case "--cross":
                    ...
                    if (sdk_path != null || runtime != null)
                        Error(
                            "You can only specify one of --runtime, --sdk or --cross {sdk_path}/{runtime}"
                        );
                    custom_mode = false;
                    autodeps = true;
                    cross_target = args[++i];
                ...
                case "--aot-mode":
                    ...
                    {
                        Console.WriteLine(
                            "Need string of aot mode (full, llvmonly, hybrid). Omit for normal AOT."
                        );
                        return 1;
                    }
        ...
        if (!custom_mode)
        {
            ...
            {
                sdk_path = Path.GetFullPath(
                    Path.Combine(Process.GetCurrentProcess().MainModule.FileName, "..", "..")
                );
                VerifySdk(sdk_path);
            }
            else
    ...
    static void VerifySdk(string path)
    {
        ...
        if (machine_config_file == null && !no_machine_config)
        {
            ...
            {
                Error(
                    $"Could not locate the file machine.config file at ${machine_config_file} use --machine-config FILE or --no-machine-config"
                );
            }
        }
        if (config_file == null && !no_config)
        {
            ...
            {
                Error(
                    $"Could not locate the file config file at ${config_file} use --config FILE or --no-config"
                );
            }
        }
    }

    static string targets_dir = Path.Combine(
    ...
    class PackageMaker
    {
        ...
        public PackageMaker(string runtime, string output)
        {
            ...
            using (Stream runtimeStream = File.OpenRead(runtime))
            {
                ...
                if (
                    runtimeStream.Read(runtimeBuffer, 0, 2) == 2
        ...
        public void WriteIndex()
        {
            ...
            foreach (
                var entry in from entry in locations
                orderby entry.Value.Item1 ascending
                select entry
            )
            {
    ...
    static bool GeneratePackage(List<string> files)
    {
        ...
        {
            Error(
                "--static-ctor not supported with package bundling, you must use native compilation for this"
            );
            return false;
        }
        ...
        if (
            !MaybeAddFile(maker, "systemconfig:", config_file)
            || !MaybeAddFile(maker, "machineconfig:", machine_config_file)
        )
            return false;
...
"
                    ...
                    if (String.IsNullOrEmpty(mono_api_struct_file))
                    {
                        ...
                        template_stream = typeof(MakeBundle).Assembly.GetManifestResourceStream(
                            "bundle-mono-api.inc"
                        );
                    }
                    else
                    ...
                    Action<string> body = delegate(string url)
                    {
                        ...
                        if (compress)
                        {
                            ...
                            GZipStream deflate = new GZipStream(
                                ms,
			...
			Parallel.ForEach (files, body);
                    ...
                    foreach (var url in files)
                    {
                        ...
                        {
                            tc.WriteLine(
                                "extern const unsigned char assembly_data_{0} [];",
                                encoded
                            );
                            tc.WriteLine(
                                "static CompressedAssembly assembly_bundle_{0} = {{{{\"{1}\","
                    ...
                    using (
                        template_stream = System.Reflection.Assembly
                    ...
                    {
                        tc.WriteLine(
                            "\tmono_api.mono_aot_register_module (mono_aot_module_{0}_info);\n",
                            asm
                        );
                    }
                    ...
                    {
                        tc.WriteLine(
                            "\tmono_api.mono_register_config_for_assembly (\"{0}\", assembly_config_{1});\n",
                            ass[0],
                            ass[1]
                        );
                    }
                    ...
                    {
                        Stream template_main_stream = System.Reflection.Assembly
                            .GetAssembly(typeof(MakeBundle))
                    ...
                    if (style == "windows")
                    {
                        ...
                        if (!nomain || custom_main != null)
                        {
                            string cl_cmd = GetCompileAndLinkCommand(
                                compiler,
                                temp_c,
                                temp_o,
                                custom_main,
                        ...
                        else
                        {
                            ...
                            try
                            {
                                string cl_cmd = GetLibrarianCompilerCommand(
                                    compiler,
                                    temp_c,
                                    static_link,
                                    staticLinkCRuntime,
                                ...
                                string lib_cmd = GetLibrarianLinkerCommand(
                                    librarian,
                                    new string[] { temp_o, temp_c_o },
                                    static_link,
                                    staticLinkCRuntime,
                                    output
                                );
                    ...
                    else
                    {
                        ...
                        if (static_link)
                        {
                            ...
                            if (in_tree != null)
                            {
                                smonolib = String.Format(
                                    "{0}/mono/mini/.libs/libmonosgen-2.0.a",
                                    in_tree
                                );
                            ...
                            cmd = String.Format(
                                "{4} -o '{2}' -Wall {7} `pkg-config --cflags mono-2` {9} {0} {3} "
    ...
    static bool QueueAssembly(List<string> files, string codebase)
    {
        ...
        {
            Error(
                "Duplicate assembly name `{0}'. Both `{1}' and `{2}' use same assembly name.",
                name,
                path,
                found
            );
    ...
    static void AotCompile(List<string> files)
    {
        ...
        for (int i = 0; i < files.Count; i++)
        {
            ...
            if (aot_dedup_assembly != null)
            {
                ...
                Execute(
                    String.Format(
                        "MONO_PATH={0} {1} --aot={2},outfile={3}{4}{5} {6}",
            ...
            else
            {
                Execute(
                    String.Format(
                        "MONO_PATH={0} {1} --aot={2},outfile={3}{4} {5}",
                        Path.GetDirectoryName(path),
                        aot_runtime,
                        aot_args,
                        outPath,
        ...
        if (aot_dedup_assembly != null)
        {
            ...
            Execute(
                String.Format(
                    "MONO_PATH={7} {0} --aot={1},outfile={2}{3}{4} {5} {6}",
                    aot_runtime,
                    aot_args,
                    outPath,
                    aot_mode_string,
        ...
        if (
            (aot_mode == "full" || aot_mode == "llvmonly" || aot_mode == "hybrid")
            && cil_strip_path != null
        )
        {
            for (int i = 0; i < files.Count; i++)
    ...
    static void LinkManaged(List<string> files, string outDir)
    {
        ...
        var cmd = String.Format(
            "{0} {1} -b true -out {2} {3} -c link -p copy ",
            runtime,
            managed_linker_path,
            outDir,
            paths.ToString()
    ...
    {
        var override_path = Path.Combine(
            Directory.GetCurrentDirectory(),
            Path.GetFileName(default_path)
        );
    ...
    class StringVersionComparer : IComparer<string>
    {
        public int Compare(string stringA, string stringB)
        {
            ...
            var versionAMatch = System.Text.RegularExpressions.Regex.Match(
                stringA,
                @"\d+(\.\d +) + "
            );
    ...
    class ToolchainProgram
    {
        ...
        public InstalledSDKInfo ParentSDK { get; set; }
        public bool IsVSToolChain
        {
            get { return (Name.Contains("cl.exe") || Name.Contains("lib.exe")); }
        }
        public bool IsGCCToolChain
        {
            get { return !IsVSToolChain; }
        }
    }
    ...
    class SDKHelper
    {
        static protected Microsoft.Win32.RegistryKey GetToolchainRegistrySubKey(string subKey)
        {
            ...
            {
                key =
                    Microsoft.Win32.Registry.LocalMachine.OpenSubKey(@"SOFTWARE" + subKey)
    ...
    class WindowsSDKHelper : SDKHelper
    {
        ...
        Dictionary<string, string> GetInstalledWindowsKitRootFolders()
        {
            ...
            using (var subKey = GetToolchainRegistrySubKey(@"\Microsoft\Microsoft SDKs\Windows\"))
            {
                if (subKey != null)
                {
                    ...
                    {
                        var keyNameIsVersion = System.Text.RegularExpressions.Regex.Match(
                            keyName,
                            @"\d+(\.\d+)+"
                        );
            ...
            using (
                var subKey = GetToolchainRegistrySubKey(@"\Microsoft\Windows Kits\Installed Roots")
            )
            {
                if (subKey != null)
                {
                    ...
                    {
                        var valueNameIsKitsRoot = System.Text.RegularExpressions.Regex.Match(
                            valueName,
                            @"KitsRoot\d*"
                        );
                        if (valueNameIsKitsRoot.Success)
                        {
                            ...
                            if (!rootFolders.ContainsKey(installFolder))
                            {
                                var valueNameIsVersion = System.Text.RegularExpressions.Regex.Match(
                                    valueName,
                                    @"\d+(\.*\d+)+"
                                );
            ...
            if (installedWindowsSDKs == null && installedCRuntimeSDKs == null)
            {
                ...
                foreach (var winKitRoot in rootFolders)
                {
                    ...
                    if (Directory.Exists(winKitIncludeDir))
                    {
                        ...
                        var versions = winKitIncludeDirInfo
                            .GetDirectories("*.*", SearchOption.TopDirectoryOnly)
                            .OrderByDescending(p => p.Name, new StringVersionComparer());

                        foreach (var version in versions)
                        {
                            string versionedWindowsSDKHeaderPath = Path.Combine(
                                version.FullName,
                                "um",
                                "windows.h"
                            );
                            string versionedCRuntimeSDKHeaderPath = Path.Combine(
                                version.FullName,
                                "ucrt",
                                "stdlib.h"
                            );
                            var hasSubVersion = System.Text.RegularExpressions.Regex.Match(
                                version.Name,
                                @"\d+(\.\d+)+"
                            );
                    ...
                    string cRuntimeSDKHeaderPath = Path.Combine(
                        winKitIncludeDir,
                        "ucrt",
                        "stdlib.h"
                    );
                ...
                windowsSDKs = windowsSDKs
                    .OrderByDescending(p => p.Version.ToString(), new StringVersionComparer())
                    .ToList();
                cRuntimeSDKs = cRuntimeSDKs
                    .OrderByDescending(p => p.Version.ToString(), new StringVersionComparer())
                    .ToList();
                ...
                if (!quiet && installedCRuntimeSDKs != null)
                {
                    ...
                    foreach (var cRuntimeSDK in installedCRuntimeSDKs)
                    {
                        ...
                        {
                            Console.WriteLine(
                                "Parent SDK Path: " + cRuntimeSDK.ParentSDK.InstallationFolder
                            );
                            Console.WriteLine(
                                "Parent SDK Version: " + cRuntimeSDK.ParentSDK.Version
                            );
                        }
                    }
                    Console.WriteLine("---------------");
        ...
        InstalledSDKInfo GetInstalledWindowsSDK()
        {
            if (installedWindowsSDK == null)
            {
                ...
                var winSDKVersion = System.Text.RegularExpressions.Regex.Match(
                    GetEnv("WindowsSdkVersion", ""),
                    @"\d+(\.\d+)+"
                );
                ...
                if (winSDKDir.Length != 0 && windowsSDKs != null)
                {
                    ...
                    if (winSDKVersion.Success)
                        windowsSDK = windowsSDKs.Find(
                            x =>
                                (
                                    x.InstallationFolder == winSDKDir
                                    && x.Version.ToString() == winSDKVersion.ToString()
                                )
                        );
                    else
                ...
                if (windowsSDK == null && winSDKVersion.Success && windowsSDKs != null)
                    // Find installed SDK based on requested info.
                    windowsSDK = windowsSDKs.Find(
                        x => x.Version.ToString() == winSDKVersion.ToString()
                    );
        ...
        string FindCRuntimeSDKLibPath(InstalledSDKInfo sdk)
        {
            ...
            cRuntimeLibPath = Path.Combine(
                cRuntimeLibPath,
                "ucrt",
                Target64BitApplication() ? "x64" : "x86"
            );
        ...
        InstalledSDKInfo GetInstalledCRuntimeSDK()
        {
            if (installedCRuntimeSDK == null)
            {
                ...
                {
                    cRuntimeSDK = cRuntimeSDKs.Find(
                        x => x.Version.ToString() == windowsSDK.Version.ToString()
                    );
        ...
        public void AddWindowsSDKLibPaths(List<string> libPaths)
        {
            ...
            if (winSDK != null)
            {
                ...
                if (Directory.Exists(winSDKLibDir))
                    libPaths.Add(
                        Path.Combine(winSDKLibDir, "um", Target64BitApplication() ? "x64" : "x86")
                    );
            }
    ...
    class VisualStudioSDKHelper : SDKHelper
    {
            ...
            if (installedVisualStudioSDKs == null)
            {
                ...
                using (var subKey = GetToolchainRegistrySubKey(@"\Microsoft\VisualStudio\SxS\VS7"))
                {
                    if (subKey != null)
                    {
                        ...
                        {
                            var vsInstalltionFolder = (string)
                                Microsoft.Win32.Registry.GetValue(subKey.ToString(), keyName, "");
                            if (Directory.Exists(vsInstalltionFolder))
                            {
                                var vsSDK = new InstalledSDKInfo(
                                    "VisualStudio",
                                    keyName,
                                    vsInstalltionFolder,
                                    false
                                ...
                                if (Directory.Exists(vcInstallationFolder))
                                    vsSDK.AdditionalSDKs.Add(
                                        new InstalledSDKInfo(
                                            "VisualStudioVC",
                                            keyName,
                                            vcInstallationFolder,
                                            false,
                                            vsSDK
                                        )
                                    );
                ...
                // Sort based on version.
                sdks = sdks.OrderByDescending(
                        p => p.Version.ToString(),
                        new StringVersionComparer()
                    )
                    .ToList();
        ...
        string FindVisualStudioVCFolderPath(InstalledSDKInfo vcSDK, string subPath)
        {
            ...
            if (vcSDK != null && vcSDK.ParentSDK != null)
            {
                ...
                {
                    string msvcVersionPath = Path.Combine(
                        vcSDK.InstallationFolder,
                        "Tools",
                        "MSVC"
                    );
                    ...
                    if (Directory.Exists(msvcVersionPath))
                    {
                        ...
                        var versions = msvcVersionDirInfo
                            .GetDirectories("*.*", SearchOption.TopDirectoryOnly)
                            .OrderByDescending(p => p.Name, new StringVersionComparer());
        ...
        public InstalledSDKInfo GetInstalledVisualStudioVCSDK()
        {
            ...
            {
                List<InstalledSDKInfo> installedVisualStudioSDKs =
                    InitializeInstalledVisualStudioSDKs();
                if (installedVisualStudioSDKs != null)
                {
                    foreach (var currentInstalledSDK in installedVisualStudioSDKs)
                    {
                        // Find installed SDK based on requested info.
                        visualStudioVCSDK = currentInstalledSDK.AdditionalSDKs.Find(
                            x => x.InstallationFolder == vcInstallDir
                        );
            ...
            if (visualStudioVCSDK == null)
            {
                ...
                if (visualStudioSDK != null)
                    visualStudioVCSDK = visualStudioSDK.AdditionalSDKs.Find(
                        x => x.Name == "VisualStudioVC"
                    );
            }
                ...
                if (!string.IsNullOrEmpty(vcSDK?.InstallationFolder))
                {
                    if (Target64BitApplication())
                        toolPath = Path.Combine(
                            new string[] { vcSDK.InstallationFolder, "bin", "amd64", tool }
                        );
                    else
                        toolPath = Path.Combine(
                            new string[] { vcSDK.InstallationFolder, "bin", tool }
                        );
    ...
    class VC15ToolchainProgram : VCToolchainProgram
    {
        ...
        protected ToolchainProgram FindVCToolchainProgram(string tool, InstalledSDKInfo vcSDK)
        {
            if (toolchain == null)
            {
                ...
                if (!string.IsNullOrEmpty(vcSDK?.InstallationFolder))
                {
                    string toolsVersionFilePath = Path.Combine(
                        vcSDK.InstallationFolder,
                        "Auxiliary",
                        "Build",
                    ...
                    if (File.Exists(toolsVersionFilePath))
                    {
                        ...
                        if (lines.Length > 0)
                        {
                            string toolsVersionPath = Path.Combine(
                                vcSDK.InstallationFolder,
                                "Tools",
                                "MSVC",
                                lines[0].Trim()
                ...
                if (!string.IsNullOrEmpty(vcSDK?.InstallationFolder))
                {
                    clangPath = Path.Combine(
                        new string[]
                        {
                            ...
                            "ClangC2",
                            "bin",
                            Target64BitApplication() ? "amd64" : "x86",
                            "clang.exe"
                        }
                    );
    ...
    class VC15Clang : VCToolchainProgram
    {
        ...
        public override ToolchainProgram FindVCToolchainProgram(InstalledSDKInfo vcSDK)
        {
            if (toolchain == null)
            {
                ...
                if (!string.IsNullOrEmpty(vcSDK?.InstallationFolder))
                {
                    string clangVersionFilePath = Path.Combine(
                        vcSDK.InstallationFolder,
                        "Auxiliary",
                        "Build",
                    ...
                    if (File.Exists(clangVersionFilePath))
                    {
                        ...
                        if (lines.Length > 0)
                        {
                            string clangVersionPath = Path.Combine(
                                vcSDK.InstallationFolder,
                                "Tools",
                                "ClangC2",
                                lines[0].Trim()
                            ...
                            clangPath = Path.Combine(
                                clangVersionPath,
                                "bin",
                                Target64BitApplication() ? "HostX64" : "HostX86",
                                "clang.exe"
                            );
    ...
    static string GetMonoDir()
    {
        ...
        {
            using (
                var baseKey = Microsoft.Win32.RegistryKey.OpenBaseKey(
    ...
    static void AddVCSystemLibraries(
        ToolchainProgram program,
        bool staticLinkMono,
        bool staticLinkCRuntime,
    ...
    static void AddGCCSystemLibraries(
        ToolchainProgram program,
        bool staticLinkMono,
        bool staticLinkCRuntime,
        List<string> linkerArgs
    ...
    static void AddSystemLibraries(
        ToolchainProgram program,
        bool staticLinkMono,
        bool staticLinkCRuntime,
        List<string> linkerArgs
    ...
    static string GetMonoLibraryName(
        ToolchainProgram program,
        bool staticLinkMono,
        bool staticLinkCRuntime
    )
    ...
    static string GetMonoLibraryPath(
        ToolchainProgram program,
        bool staticLinkMono,
        bool staticLinkCRuntime
    )
    ...
    static void AddVCCompilerArguments(
        ToolchainProgram program,
        bool staticLinkMono,
        bool staticLinkCRuntime,
    ...
    static void AddGCCCompilerArguments(
        ToolchainProgram program,
        bool staticLinkMono,
        bool staticLinkCRuntime,
        List<string> compilerArgs
    ...
    static void AddCompilerArguments(
        ToolchainProgram program,
        bool staticLinkMono,
        bool staticLinkCRuntime,
        List<string> compilerArgs
    ...
    static void AddVCLinkerArguments(
    ...
    static void AddGCCLinkerArguments(
        ToolchainProgram linker,
        bool staticLinkMono,
        bool staticLinkCRuntime,
        string customMain,
    ...
    static void AddLinkerArguments(
        ToolchainProgram program,
        bool staticLinkMono,
        bool staticLinkCRuntime,
        string customMain,
    ...
    static void AddVCLibrarianCompilerArguments(
    ...
    static void AddGCCLibrarianCompilerArguments(
        ToolchainProgram compiler,
        string sourceFile,
        bool staticLinkMono,
        bool staticLinkCRuntime,
    ...
    static void AddVCLibrarianLinkerArguments(
        ToolchainProgram librarian,
        string[] objectFiles,
        bool staticLinkMono,
        bool staticLinkCRuntime,
        string outputFile,
        List<string> librarianArgs
    )
    {
    ...
    static void AddGCCLibrarianLinkerArguments(
        ToolchainProgram librarian,
        string[] objectFiles,
        bool staticLinkMono,
        bool staticLinkCRuntime,
    ...
    static ToolchainProgram GetAssemblerCompiler()
    {
        ...
        if (vcClangAssembler == null || vcClangAssembler.Path.Length == 0)
        {
            ...
            string asCompiler = Target64BitApplication()
    ...
    static ToolchainProgram GetCCompiler(bool staticLinkMono, bool staticLinkCRuntime)
    {
        ...
        if (!File.Exists(monoLibraryPath) && program.IsVSToolChain)
        {
            ...
            string gccCompiler = Target64BitApplication()
    ...
    {
        ToolchainProgram vcLibrarian = VisualStudioSDKToolchainHelper
            .GetInstance()
            .GetVCLibrarian();
    ...
    static string GetCompileAndLinkCommand(
        ToolchainProgram compiler,
        string sourceFile,
        string objectFile,
        string customMain,
    ...
    )
    {
        ...
        return String.Format(
            "{0} {1}",
            compiler.QuoteArg(compiler.Path),
            String.Join(" ", compilerArgs)
        );
    }

    static string GetLibrarianCompilerCommand(
        ToolchainProgram compiler,
        string sourceFile,
        bool staticLinkMono,
        bool staticLinkCRuntime,
    ...
    )
    {
        ...
        if (compiler.IsVSToolChain)
            AddVCLibrarianCompilerArguments(
                compiler,
                sourceFile,
                staticLinkMono,
                staticLinkCRuntime,
            ...
            AddGCCLibrarianCompilerArguments(
                compiler,
                sourceFile,
                staticLinkMono,
                staticLinkCRuntime,
    ...
    static string GetLibrarianLinkerCommand(
        ToolchainProgram librarian,
        string[] objectFiles,
        bool staticLinkMono,
        bool staticLinkCRuntime,
    ...
    )
    {
        ...
        if (librarian.IsVSToolChain)
            ...
            AddGCCLibrarianLinkerArguments(
                librarian,
                objectFiles,
                staticLinkMono,
                staticLinkCRuntime,
        ...
        return String.Format(
            "{0} {1}",
            librarian.QuoteArg(librarian.Path),
