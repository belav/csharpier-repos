...
#if !MSNET
using Commons.Xml.Nvdl;using Commons.Xml.Relaxng;using Commons.Xml.Relaxng.Rnc;
#endif
...
"
        ...
        static void ValidateRelaxng(RelaxngPattern p, string[] args)
        {
            ...
            for (int i = 2; i < args.Length; i++)
            {
                ...
                RelaxngValidatingReader vr = new RelaxngValidatingReader(xtr, p);
                if (Environment.GetEnvironmentVariable("MONO_XMLTOOL_ERROR_DETAILS") == "yes")
        ...
        static void ValidateDtd(string[] args)
        {
            ...
            {
                XmlValidatingReader xvr = new XmlValidatingReader(new XmlTextReader(args[i]));
        ...
        static void Transform(string[] args)
        {
            ...
            t.Load(args[1]);
            TextWriter output = args.Length > 3 ? File.CreateText(args[3]) : Console.Out;
