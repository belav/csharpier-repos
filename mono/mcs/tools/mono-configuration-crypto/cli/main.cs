...
namespace MonoConfigurationCrypto
{
    class MonoConfigurationCrypto
    {
        ...
        bool ListContainers(Config cfg)
        {
            try
            {
                ...
                foreach (KeyContainer c in kc)
                {
                    count = c.Count;
                    Console.WriteLine(
                        "{0} container '{1}' ({2} key{3})",
                        c.Local ? "Local" : "Global",
                        c.Name,
                        count,
        ...
        bool EncryptSection(Config cfg)
        {
            ...
            Console.WriteLine(
                "Encrypting section '{0}' in config file '{1}' using key container '{2}'...",
                configSection,
                configFile,
                containerName
            );
            ...
            if (String.IsNullOrEmpty(configFile))
            {
                Failure(
                    cfg,
                    "No config file found in directory '{0}'",
                    cfg.ApplicationPhysicalPath
                );
                return true;
            }
        ...
        bool DecryptSection(Config cfg)
        {
            ...
            Console.WriteLine(
                "Decrypting section '{0}' in config file '{1}' using key container '{2}'...",
                configSection,
                configFile,
                containerName
            );
        ...
        bool ImportKey(Config cfg)
        {
            ...
            Console.WriteLine(
                "Importing an RSA key from file '{0}' into the container '{1}'...",
                fileName,
                containerName
            );
        ...
        bool ExportKey(Config cfg)
        {
            ...
            Console.WriteLine(
                "Exporting an RSA key from container '{0}' to file '{1}'...",
                containerName,
                fileName
            );
            if (String.IsNullOrEmpty(containerName))
            ...
            try
            {
                ...
                if (key == null)
                {
                    Failure(
                        cfg,
                        "Container '{0}' exists but it does not contain any keys.",
                        containerName
                    );
                    return true;
                }
        ...
        void Run(string[] args)
        {
            ...
            var options = new OptionSet()
            {
                ...
                {
                    "m|machine|global",
                    "Use machine (global) store for all the key actions",
                    v => cfg.UseMachinePath = true
                },
                {
                    "u|user|local",
                    "Use local (user) store for all the key actions [*]",
                    v => cfg.UseMachinePath = false
                },
                {
                    "l|list",
                    "List all the key container names in the store",
                    v => actions.Add(ListContainers)
                },
                {
                    "c|create",
                    "Creates an RSA public/private key pair",
                    v => actions.Add(CreateKey)
                },
                ...
                { "r|remove", "Remove a container", v => actions.Add(RemoveContainer) },
                {
                    ...
                    (string s) => cfg.FileName = s
                },
                {
                    "cf=|config-file=",
                    String.Format(
                        "Config file name (not path) [{0}]",
                        Config.DefaultConfigFileName
                    ),
                    (string s) => cfg.ConfigFileName = s
                },
                {
                    "n=|name=",
                    String.Format("Container name [{0}]", Config.DefaultContainerName),
                    (string s) => cfg.ContainerName = s
                },
                {
                    "s=|size=",
                    String.Format("Key size [{0}]", Config.DefaultKeySize),
                    (uint s) => cfg.KeySize = s
                },
                {
                    "p=|path=",
                    String.Format(
                        "Application physical path [{0}]",
                        Config.DefaultApplicationPhysicalPath
                    ),
                    (string s) => cfg.ApplicationPhysicalPath = s
                },
                {
                    "d=|dec=|decrypt=",
                    ...
                    (string s) =>
                    {
                        cfg.ConfigSectionName = s;
                        actions.Add(DecryptSection);
                    }
                },
                {
                    "e=|enc=|encrypt=",
