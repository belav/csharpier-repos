...
namespace Mono.Configuration.Crypto
{
    public class ConfigSection
    {
        ...
        ProtectedConfigurationProvider GetProvider(
            string providerName,
        ...
        )
        {
            ...
            if (prov == null)
                throw new InvalidOperationException(
                    String.Format("Provider '{0}' is unknown.", providerName)
                );
        ...
        string LoadSection(
            ...
            out XmlDocument doc,
            out XmlNode section,
            out ProtectedConfigurationProvider provider
        )
        {
            if (!IsValidSection(configFile, configSection))
                throw new InvalidOperationException(
                    String.Format(
                        "Section '{0}' is not a valid section in file '{1}'",
                        configSection,
                        configFile
                    )
                );
            ...
            if (section == null)
                throw new InvalidOperationException(
                    String.Format(
                        "Section '{0}' not found in config file '{1}'",
                        configSection,
                        configFile
                    )
                );
        ...
        )
        {
            ...
            if (encrypted == null)
                throw new InvalidOperationException(
                    String.Format(
                        "Failed to encrypt '{0}' section in config file '{1}'",
                        configSection,
                        configFile
                    )
                );
