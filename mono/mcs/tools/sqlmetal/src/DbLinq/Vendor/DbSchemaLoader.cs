...
namespace DbLinq.Vendor
{
    ...
    class DbSchemaLoader : SchemaLoader
    {
        ...
        protected override void LoadConstraints(
            Database schema,
            DbLinq.Schema.SchemaName schemaName,
            IDbConnection conn,
            DbLinq.Schema.NameFormat nameFormat,
            Names names
        )
        {
            ...
            {
                WriteErrorLine(
                    "Database connection '{0}' doesn't support querying foreign key information.",
                    c.GetType().Name
                );
                return;
            }
            ...
            foreach (DataRow r in foreignKeys.Rows)
            {
                ...
                {
                    WriteErrorLine(
                        "ERROR L46: Table '" + fromTable + "' not found for column " + fromColumn
                    );
                    continue;
                }
        ...
        )
        {
            ...
            {
                WriteErrorLine(
                    "Database connection '{0}' doesn't support querying primary key information.",
                    db.GetType().Name
                );
            }
