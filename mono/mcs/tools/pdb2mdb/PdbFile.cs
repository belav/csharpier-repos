...
namespace Microsoft.Cci.Pdb
{
    internal class PdbFile
    {
                    ...
                    case DEBUG_S_SUBSECTION.LINES:
                    {
                        ...
                        while (bits.Position < endSym)
                        {
                            ...
                            for (int i = 0; i < file.count; i++)
                            {
                                ...
                                bool statement = (
                                    (line.flags & (uint)CV_Line_Flags.fStatement) == 0
                                );
                                if ((sec.flags & 1) != 0)
                                {
        ...
        static void LoadFuncsFromDbiModule(
            BitAccess bits,
            DbiModuleInfo info,
            IntHashTable names,
            ArrayList funcList,
        ...
        )
        {
            ...
            funcs = PdbFunction.LoadManagedFunctions(
                info.moduleName,
                bits,
                (uint)info.cbSyms,
                readStrings
        ...
        )
        {
            ...
            if (dh.sig != -1 || dh.ver != 19990903)
            {
                throw new PdbException(
                    "Unsupported DBI Stream version, sig={0}, ver={1}",
                    dh.sig,
                    dh.ver
                );
            }
