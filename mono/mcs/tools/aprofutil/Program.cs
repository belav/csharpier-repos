    ...
    class MainClass
    {
        ...
        static string ProcessArguments(string[] args)
        {
            ...
            var options = new OptionSet
            {
                ...
                { "take-count=", "Take {VALUE} methods that match", v => TakeCount = int.Parse(v) },
                {
                    "skip-count=",
            ...
            using (var client = new TcpClient("127.0.0.1", Port))
            {
                using (var stream = client.GetStream())
                {
                    ...
                    using (var memoryStream = new MemoryStream(128 * 1024))
                    {
                        ...
                        if (Verbose)
                            ColorWriteLine(
                                $"Read total {memoryStream.Length} bytes...",
                                ConsoleColor.Yellow
                            );
        ...
        public static void Main(string[] args)
        {
            ...
            if (Summary)
            {
                ...
                WriteLine(
                    $"\tModules: {modules.Count.ToString("N0"), 10}{(modules.Count != pd.Modules.Length ? $"  (of {pd.Modules.Length})" : "")}"
                );
                WriteLine(
                    $"\tTypes:   {types.Count.ToString("N0"), 10}{(types.Count != pd.Types.Length ? $"  (of {pd.Types.Length})" : "")}"
                );
                WriteLine(
                    $"\tMethods: {methods.Count.ToString("N0"), 10}{(methods.Count != pd.Methods.Length ? $"  (of {pd.Methods.Length})" : "")}"
                );
            }
            ...
            if (!string.IsNullOrEmpty(Output))
            {
                if (Verbose)
                    ColorWriteLine(
                        $"Going to write the profile to '{Output}'",
                        ConsoleColor.Yellow
                    );
        ...
        public static void Error(string message) =>
            ColorMessage($"Error: {Name}: {message}", ConsoleColor.Red, Console.Error);

        public static void Warning(string message) =>
            ColorMessage($"Warning: {Name}: {message}", ConsoleColor.Yellow, Console.Error);
    }
}
