        ...
        try
        {
            ...
            if (types.Count > 0)
            {
                ...
                MetaData.ConvertTypesToSchemaToStream(
                    (ServiceType[])types.ToArray(typeof(ServiceType)),
                    SdlType.Wsdl,
                    schemaStream
                );
            }
            ...
            if (inputUrl != null)
            {
                if (schemaStream != null)
                    throw new Exception("Only one type source can be specified");
                schemaStream = new MemoryStream();
            ...
            {
                if (schemaStream == null)
                    throw new Exception("No input schema or assembly has been specified");
            ...
            if (outputAssembly != null)
            {
                ...
                schemaStream.Position = 0;
                if (outputCode)
                    MetaData.ConvertCodeSourceStreamToAssemblyFile(
                        writtenFiles,
                        outputAssembly,
                        strongNameFile
                    );
    ...
    static void ReadParameters(string[] args)
    {
        ...
        foreach (string arg in args)
        {
            ...
            switch (parg.ToLower())
            {
                ...
                case "inputassemblyfile":
                case "ia":
                    inputAssembly = param;
                    break;

                case "outputassemblyfile":
                case "oa":
                ...
                case "inputschemafile":
                case "is":
                    inputSchema = param;
                    break;

                case "outputschemafile":
                case "os":
                    outputSchema = param;
                    break;
