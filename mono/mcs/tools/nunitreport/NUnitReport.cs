    ...
    public class NUnitReport
    {
        ...
        static void Main(string[] args)
        {
            ...
            foreach (string file in Directory.GetFiles(input_dir))
            {
                ...
                {
                    sw.WriteLine(
                        "        <tr class='errorrow' onclick=\"toggle('el{0}')\" onmouseover='highlight(this)' onmouseout='unhighlight(this)'>",
                        assembly
                    );
                ...
                sw.WriteLine(
                    @"        <tr id='el{0}' class='errorlist' style='display: none'>",
                    assembly
                );
                sw.WriteLine(@"            <td></td>");
                ...
                {
                    sw.WriteLine(
                        "                    <tr onclick=\"toggle('as{0}ed{1}')\" onmouseover='highlight(this)' onmouseout='unhighlight(this)'>",
                        assembly,
                        test_num
                    );
                    sw.WriteLine(
                        @"                        <td style='width: 17px'><img src='Media/bullet.png' /></td>"
                    );
                    sw.WriteLine(@"                        <td>{0}</td>", ft.Name);
                    sw.WriteLine(@"                    </tr>");
                    sw.WriteLine(
                        @"                    <tr id='as{0}ed{1}' class='errordetail' style='display: none'>",
                        assembly,
                        test_num
                    );
                    sw.WriteLine(@"                        <td></td>");
                    ...
                    if (!string.IsNullOrEmpty(ft.StackTrace.Trim()))
                        sw.WriteLine(
                            @"<br /><br /><strong>StackTrace:</strong><br />{0}",
                            ft.StackTrace.Replace("\n", "<br/>")
                        );
                    sw.WriteLine(@"                        </td>");
                    sw.WriteLine(@"                    </tr>");
        ...
        public static int PopulateFailureTable(
            string filename,
            Dictionary<string, FailedTest> output,
            List<string> ignored
        )
        {
        ...
        public static int FindTestCases(
            XmlElement xe,
            Dictionary<string, FailedTest> output,
            List<string> ignored
        )
        ...
        public static void OutputFailedTestCase(
            XmlElement xe,
            Dictionary<string, FailedTest> output,
            List<string> ignored
        )
        {
            ...
            FailedTest ft = new FailedTest(
                ...
                xe["failure"]["message"].InnerText,
                xe["failure"]["stack-trace"].InnerText
            );
            output[ft.Name] = ft;
        }
        ...
        public static void StartReport(StreamWriter sw)
        {
            ...
            sw.WriteLine(
                @"<script type=""text/ecmascript"" src=""Media/scripts.js""></script></head>"
            );
            sw.WriteLine(@"<body>");
            ...
            sw.WriteLine(@"        </div>");
            sw.WriteLine(
                @"        <div class='headertext'>Class Libraries NUnit Test Results</div>"
            );
            sw.WriteLine(@"    </div>");
            ...
            sw.WriteLine(
                @"        Icons courtesy of <a href='http://www.famfamfam.com/lab/icons/silk'>famfamfam</a>"
            );
            sw.WriteLine(@"    </div>");
