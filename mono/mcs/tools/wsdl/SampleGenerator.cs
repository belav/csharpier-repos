...
namespace Mono.WebServices
{
    public class ConsoleSampleGenerator : SampleGenerator
    {
        ...
        )
        {
            ...
            if (binding == null)
                throw new InvalidOperationException("Binding " + bindingName + " not found");
        ...
        Port FindPort(string portName, string protocol)
        {
            ...
            foreach (Port port in service.Ports)
            {
                ...
                {
                    Binding binding = descriptions.GetBinding(port.Binding);
                    if (GetProtocol(binding) == protocol)
                        return port;
                }
        ...
        {
            if (binding.Extensions.Find(typeof(SoapBinding)) != null)
                return "Soap";
        ...
        Operation FindOperation(PortType portType, string name)
        {
            foreach (Operation oper in portType.Operations)
            {
                ...
                {
                    if (oper.Messages.Input.Name == name)
                        return oper;
                }
                else if (oper.Name == name)
                    return oper;
            }
    ...
    public class SampleGenerator
    {
        ...
        class EncodedType
        {
            public EncodedType(string ns, XmlSchemaElement elem)
            {
                Namespace = ns;
                Element = elem;
            }
        ...
        public string GenerateHttpSoapMessage(
            Port port,
            OperationBinding obin,
            Operation oper,
        ...
        )
        {
            ...
            {
                SoapAddressBinding sab =
                    port.Extensions.Find(typeof(SoapAddressBinding)) as SoapAddressBinding;
                SoapOperationBinding sob =
                    obin.Extensions.Find(typeof(SoapOperationBinding)) as SoapOperationBinding;
        ...
        public string GenerateHttpGetMessage(
            Port port,
            OperationBinding obin,
            Operation oper,
            OperationMessage msg
        )
        {
            ...
            if (msg is OperationInput)
            {
                HttpAddressBinding sab =
                    port.Extensions.Find(typeof(HttpAddressBinding)) as HttpAddressBinding;
                HttpOperationBinding sob =
                    obin.Extensions.Find(typeof(HttpOperationBinding)) as HttpOperationBinding;
                string location =
                    new Uri(sab.Location).AbsolutePath + sob.Location + "?" + BuildQueryString(msg);
                req += "GET " + location + "\n";
                req += "Host: " + GetLiteral("string");
            }
            else
            {
                ...
                MimeXmlBinding mxb =
                    (MimeXmlBinding)obin.Output.Extensions.Find(typeof(MimeXmlBinding))
                    as MimeXmlBinding;
                if (mxb == null)
                    return req;
        ...
        public string GenerateHttpPostMessage(
            Port port,
            OperationBinding obin,
            Operation oper,
            OperationMessage msg
        ...
        public string GenerateSoapMessage(
            OperationBinding obin,
            Operation oper,
            OperationMessage msg
        )
        {
            ...
            MessageBinding msgbin =
                (msg is OperationInput) ? (MessageBinding)obin.Input : (MessageBinding)obin.Output;
            SoapBodyBinding sbb =
                msgbin.Extensions.Find(typeof(SoapBodyBinding)) as SoapBodyBinding;
        ...
        void WriteHeader(XmlTextWriter xtw, SoapHeaderBinding header)
        {
            ...
            if (part == null)
                throw new InvalidOperationException(
                    "Message part " + header.Part + " not found in message " + header.Message
                );
                ...
                else
                {
                    ...
                    if (currentUse == SoapBindingUse.Encoded)
                        xtw.WriteAttributeString(
                            ...
                            GetQualifiedNameString(xtw, elem.SchemaTypeName)
                        );
        ...
        void WriteTypeSample(XmlTextWriter xtw, XmlQualifiedName qname)
        {
            ...
            XmlSchemaSimpleType stype = (XmlSchemaSimpleType)
            ...
            if (id != -1)
            {
                ...
                if (rootName != arrayType)
                    xtw.WriteAttributeString(
                        "type",
                        XmlSchema.InstanceNamespace,
                        GetQualifiedNameString(xtw, rootName)
                    );
            }
            ...
            foreach (XmlSchemaObject at in atts)
            {
                ...
                else if (at is XmlSchemaAttributeGroupRef)
                {
                    ...
                    XmlSchemaAttributeGroup grp = (XmlSchemaAttributeGroup)
                        schemas.Find(gref.RefName, typeof(XmlSchemaAttributeGroup));
        ...
        void WriteComplexContent(XmlTextWriter xtw, string ns, XmlSchemaComplexContent content)
        {
            ...
            {
                XmlSchemaComplexContentRestriction rest = (XmlSchemaComplexContentRestriction)
                    content.Content;
        ...
        void ParseArrayType(XmlSchemaComplexContentRestriction rest, out XmlQualifiedName qtype)
        {
            ...
            if (uatts == null || uatts.Length == 0)
                throw new InvalidOperationException(
                    "arrayType attribute not specified in array declaration"
                );
        ...
        {
            if (
                elem.SchemaTypeName.Namespace == XmlSchema.Namespace
                || schemas.Find(elem.SchemaTypeName, typeof(XmlSchemaSimpleType)) != null
            )
                WriteElementSample(xtw, ns, elem);
            else
            {
