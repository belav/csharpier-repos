...
public class PInvokeTableGenerator
{
        ...
        foreach (var module in modules.Keys)
        {
            ...
            foreach (var pinvoke in pinvokes)
            {
                if (pinvoke.Module == module)
                    Console.WriteLine(
                        "{\"" + pinvoke.EntryPoint + "\", " + pinvoke.TableEntryPoint + "},"
                    );
            }
    ...
    void GenNativeToInterp()
    {
        ...
        foreach (var cb in callbacks)
        {
            ...
            if (!IsBlittable(method.ReturnType))
                Error(
                    "The return type of pinvoke callback method '"
                        + method.FullName
    ...
    public static void CollectPInvokes(
        List<AssemblyDefinition> assemblies,
        out List<Pinvoke> pinvokes,
        out List<PinvokeCallback> callbacks
    )
    {
    ...
    static void ProcessTypeForPinvoke(
        List<Pinvoke> pinvokes,
        List<PinvokeCallback> callbacks,
        TypeDefinition type
    )
        ...
        foreach (var method in type.Methods)
        {
            ...
            foreach (var attr in method.CustomAttributes)
            {
                ...
                if (attr.Constructor.DeclaringType.Name == "MonoPInvokeCallbackAttribute")
                {
                    if (!method.IsStatic)
                        Error(
                            $"Method '{method.FullName}' decored with [MonoPInvokeCallback] needs to be static."
                        );
