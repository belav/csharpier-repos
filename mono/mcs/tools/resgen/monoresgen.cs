...
class ResGen
{
    ...
    {
        string Usage =
            @"Mono Resource Generator version "
            + Consts.MonoVersion
            + @"
Usage:
            ...
            switch (args[i].ToLower())
            {
                ...
                default:
                    ...
                    if (compileMultiple)
                    {
                        ...
                        switch (pair.Length)
                        {
                            case 1:
                                ...
                                resInf.OutputFile = Path.ChangeExtension(
                                    resInf.InputFile,
                                    "resources"
                                );
                                break;
                    ...
                    else
                    {
                        ...
                        {
                            resInf.InputFile = Path.GetFullPath(args[i]);
                            resInf.OutputFile = Path.ChangeExtension(resInf.InputFile, "resources");
                        }
                    }
...
class TxtResourceReader : IResourceReader
{
    ...
    void Load()
    {
        ...
        while ((line = reader.ReadLine()) != null)
        {
                ...
                switch (keywd)
                {
                    ...
                    case "else":
                        ...
                        if (conditional.Count < 1)
                            throw new Exception(
                                String.Format("Found an #{0} without matching #ifdef", keywd)
                            );
            ...
            if (val == null)
                throw new Exception(
                    String.Format("Unsupported escape character in value of key '{0}'.", key)
                );
