...
namespace Mono.Tools
{
    class CertificateManager
    {
        ...
        {
            X509Stores stores = (
                (machine) ? X509StoreManager.LocalMachine : X509StoreManager.CurrentUser
            );
        ...
        static X509CertificateCollection LoadCertificates(
            string filename,
            string password,
            bool verbose
        )
        {
            ...
            switch (Path.GetExtension(filename).ToUpper())
            {
                case ".P7B":
                case ".SPC":
                    SoftwarePublisherCertificate spc = SoftwarePublisherCertificate.CreateFromFile(
                        filename
                    );
                ...
                case ".PFX":
                    ...
                    break;
                default:
                    Console.WriteLine("Unknown file extension: {0}", Path.GetExtension(filename));
                    break;
            }
            return coll;
        }
        ...
        )
        {
            switch (type)
            {
                case ObjectType.Certificate:
                    ...
                    Console.WriteLine(
                        "{0} certificate(s) added to store {1}.",
                        coll.Count,
                        store.Name
                    );
                    break;
                case ObjectType.CRL:
                    ...
                    Console.WriteLine("{0} CRL(s) added to store {1}.", list.Count, store.Name);
                    break;
                default:
        ...
        )
        {
            ...
            switch (type)
            {
                case ObjectType.Certificate:
                    ...
                    SSCX.X509Certificate2 cert = new SSCX.X509Certificate2(
                        store.Certificates[selection - 1].RawData
                    );
                    byte[] data = null;
                    if (pem)
                    {
        ...
        static void DisplayCrl(X509Crl crl, bool machine, bool verbose)
        {
            ...
            Console.WriteLine(
                "  Next Update:   {0} {1}",
                crl.NextUpdate,
                crl.IsCurrent ? String.Empty : "update overdue!"
            );
            ...
            if (verbose)
            {
                ...
                Console.WriteLine(
                    "  Signature:            {0}",
                    CryptoConvert.ToHex(crl.Signature)
                );
                int n = 0;
                foreach (X509Crl.X509CrlEntry entry in crl.Entries)
                {
                    Console.WriteLine(
                        "    #{0}: Serial: {1} revoked on {2}",
                        ++n,
                        CryptoConvert.ToHex(entry.SerialNumber),
        ...
        static void Ssl(string host, bool machine, bool verbose)
        {
            ...
            {
                Console.WriteLine(
                    "Importing certificates from '{0}' into the {1} stores.",
                    host,
            ...
            if (coll != null)
            {
                ...
                for (int i = coll.Count - 1; i >= 0; i--)
                {
                    ...
                    {
                        Console.WriteLine(
                            "This certificate is already in the {0} store.",
                            store.Name
                        );
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Console.WriteLine(
                                    "Certificate not imported into store {0}.",
                                    store.Name
                                );
                            }
                            break;
            ...
            {
                Console.WriteLine(
                    "{0} certificate{1} added to the stores.",
                    n,
                    (n == 1) ? String.Empty : "s"
                );
            }
        ...
        )
        {
            switch (type)
            {
                case ObjectType.Certificate:
                    ...
                    Console.WriteLine(
                        "{0} keys(s) imported to KeyPair {1} persister.",
                        count,
        ...
        static void Main(string[] args)
        {
            ...
            if (action != Action.Ssl)
            {
                ...
                if (store == null)
                {
                    ...
                    Console.WriteLine(
                        "Valid stores are: {0}, {1}, {2}, {3} and {4}",
            ...
            {
                Console.WriteLine(
                    "Access to the {0} '{1}' certificate store has been denied.",
                    (machine ? "machine" : "user"),
                    storeName
                );
                if (verbose)
