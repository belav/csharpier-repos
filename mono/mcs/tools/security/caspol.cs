...
namespace Mono.Tools
{
    ...
    class CasPol
    {
        ...
        static CodeGroup FindCodeGroup(string name, ref CodeGroup parent, ref PolicyLevel pl)
        {
            ...
            Console.WriteLine(
                "CodeGroup with {0} '{1}' was not found!",
                label ? "label" : "name",
                name
            );
            return null;
            ...
            for (; i < args.Length; i++)
            {
                switch (args[++i])
                {
                    ...
                    case "-url":
                        ...
                        break;
                    case "-zone":
                        SecurityZone zone = (SecurityZone)
                            Enum.Parse(typeof(SecurityZone), args[++i]);
        ...
        static bool AddCodeGroup(string[] args, ref int i)
        {
            ...
            cg.AddChild(child);
            SecurityManager.SavePolicyLevel(pl);
            Console.WriteLine("CodeGroup '{0}' added in {1} policy level.", cg.Name, pl.Label);
            return true;
        }
        ...
        static bool ChangeCodeGroup(string[] args, ref int i)
        {
            ...
            SecurityManager.SavePolicyLevel(pl);
            Console.WriteLine("CodeGroup '{0}' modified in {1} policy level.", cg.Name, pl.Label);
            return true;
        }
        ...
        static bool RemoveCodeGroup(string name)
        {
            ...
            SecurityManager.SavePolicyLevel(pl);
            Console.WriteLine("CodeGroup '{0}' removed from {1} policy level.", cg.Name, pl.Label);
            return true;
        }
            ...
            for (; i < args.Length; i++)
            {
                switch (args[i])
                {
                    ...
                    case "-customall":
                        ...
                        Levels.Add(Machine);
                        Levels.Add(
                            SecurityManager.LoadPolicyLevelFromFile(args[++i], PolicyLevelType.User)
                        );
                        break;
                    case "-cu":
                    case "-customuser":
