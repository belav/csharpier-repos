...
namespace Mono.Tools
{
    static class SecurityDeclarationRocks
    {
        ...
        static bool TryProcessPermissionSetAttribute(
            SecurityDeclaration declaration,
            out PermissionSet set
        )
        {
            set = null;
            ...
            if (
                attribute_type.Name != "PermissionSetAttribute"
                || attribute_type.Namespace != "System.Security.Permissions"
            )
                return false;
        ...
        static IPermission CreatePermission(
            SecurityDeclaration declaration,
        ...
        static void CompleteSecurityAttribute(
        ...
        static void CompleteSecurityAttributeFields(
            SSP.SecurityAttribute security_attribute,
            SecurityAttribute attribute
        )
        {
            ...
            foreach (var named_argument in attribute.Fields)
                type.GetField(named_argument.Name)
                    .SetValue(security_attribute, named_argument.Argument.Value);
        }

        static void CompleteSecurityAttributeProperties(
            SSP.SecurityAttribute security_attribute,
            SecurityAttribute attribute
        )
        {
            ...
            foreach (var named_argument in attribute.Properties)
                type.GetProperty(named_argument.Name)
                    .SetValue(security_attribute, named_argument.Argument.Value, null);
        }

        static SSP.SecurityAttribute CreateSecurityAttribute(
            Type attribute_type,
            SecurityDeclaration declaration
        )
        {
            ...
            try
            {
                security_attribute = (SSP.SecurityAttribute)
                    Activator.CreateInstance(
                        attribute_type,
                        new object[] { (SSP.SecurityAction)declaration.Action }
                    );
            }
            catch (MissingMethodException)
            {
                security_attribute = (SSP.SecurityAttribute)
                    Activator.CreateInstance(attribute_type, new object[0]);
            }
    ...
    class PermView
    {
        ...
        static bool ProcessAssemblyOnly(TextWriter tw, AssemblyDefinition ad)
        {
            ...
            foreach (SecurityDeclaration decl in ad.SecurityDeclarations)
            {
                switch (decl.Action)
                {
                    ...
                    default:
                        tw.WriteLine(
                            "Invalid assembly level declaration {0}{1}{2}",
                            decl.Action,
                            Environment.NewLine,
                            decl.ToPermissionSet()
                        );
                        result = false;
                        break;
                }
        ...
        static void ShowSecurity(
            TextWriter tw,
            string header,
            IEnumerable<SecurityDeclaration> declarations
        )
        {
            foreach (SecurityDeclaration declsec in declarations)
            {
                tw.WriteLine(
                    "{0} {1} Permission Set:{2}{3}",
                    header,
                    declsec.Action,
                    Environment.NewLine,
                    declsec.ToPermissionSet()
                );
            }
        }
        ...
        static bool ProcessAssemblyXml(TextWriter tw, AssemblyDefinition ad)
        {
            ...
            foreach (ModuleDefinition module in ad.Modules)
            {
                foreach (TypeDefinition type in module.Types)
                {
                    ...
                    if (
                        (typelem != null)
                        || ((methods.Children != null) && (methods.Children.Count > 0))
                    )
                    {
