...
namespace Mono.ServiceContractTool
{
    public class Driver
    {
                    ...
                    switch (fi.Extension)
                    {
                        ...
                        default:
                            throw new NotSupportedException(
                                "Not supported file extension: " + fi.Extension
                            );
                    }
                }
            }
        ...
        MetadataSet ResolveWithDisco(string url)
        {
            ...
            {
                Console.WriteLine(
                    "Disco failed for the url '{0}' with exception :\n {1}",
                    url,
                    e.Message
                );
                return null;
        ...
        MetadataSet ResolveWithWSMex(string url)
        {
            ...
            try
            {
                ...
                Console.WriteLine(
                    "\nAttempting to download metadata from {0} using WS-MetadataExchange..",
                    url
                );
            ...
            catch (InvalidOperationException e)
            {
                ...
                Console.WriteLine(
                    "WS-MetadataExchange query failed for the url '{0}' with exception :\n {1}",
                    url,
                    msg
                );
            }
        ...
        {
            object[] a = type.GetCustomAttributes(typeof(ServiceContractAttribute), true);
            if (a.Length > 0)
                generator.GenerateServiceContractType(ContractDescription.GetContract(type));
        }

        ServiceContractGenerationOptions GetGenerationOption()
        {
            ServiceContractGenerationOptions go = ServiceContractGenerationOptions.ClientClass;
            if (co.GenerateAsync)
