...
namespace Mono.ILASM
{
    public class TypeDef : ICustomAttrTarget, IDeclSecurityTarget, IComparable
    {
        ...
        public void AddOverride(MethodDef body, BaseTypeRef parent, string name)
        {
            ...
            override_list.Add(new DictionaryEntry(body, new DictionaryEntry(parent, name)));
        }
        ...
        public void AddOverride(string sig, BaseMethodRef decl)
        {
            ...
            override_long_list.Add(new DictionaryEntry(sig, decl));
        }
        ...
        public void AddFieldDef(FieldDef fielddef)
        {
            ...
            if (field_table[entry] != null)
                Report.Error(
                    "Duplicate field declaration: " + fielddef.Type.FullName + " " + fielddef.Name
                );
        ...
        public void AddMethodDef(MethodDef methoddef)
        {
            if (method_table[methoddef.Signature] != null)
                Report.Error(
                    methoddef.StartLocation,
                    "Duplicate method declaration: " + methoddef.Signature
                );
        ...
        public void Define(CodeGen code_gen)
        {
            ...
            if (outer != null)
            {
                ...
                if (vis == PEAPI.TypeAttr.Private || vis == PEAPI.TypeAttr.Public)
                {
                    ...
                    Report.Warning(
                        location,
                        String.Format(
                            "Nested class '{0}' has non-nested visibility, set to such.",
                            NestedFullName
                        )
                    );
                    attr = attr ^ vis;
                    attr |= (
                        vis == PEAPI.TypeAttr.Public
                            ? PEAPI.TypeAttr.NestedPublic
                            : PEAPI.TypeAttr.NestedPrivate
                    );
                }
            }
            ...
            if (parent != null)
            {
                ...
                {
                    Report.Error("this type can not be a base type: " + parent);
                }
                ...
                if (
                    !IsValueType(name_space, name)
                    && !IsEnumType(name_space, name)
                    && is_value_class
                    && (attr & PEAPI.TypeAttr.Sealed) == 0
                )
                {
            ...
            else
            {
                if (outer != null)
                {
                    if (!outer.IsDefined)
                        outer.Define(code_gen);
                    classdef = outer.PeapiType.AddNestedClass(attr, name_space, name);
                }
                else
                {
                    if (is_value_class || is_enum_class)
                    ...
                    {
                        classdef = code_gen.PEFile.AddClass(attr, name_space, name);
                    }
                }
        ...
        public void DefineContents(CodeGen code_gen)
        {
            ...
            foreach (FieldDef fielddef in field_list)
            {
                ...
                {
                    fielddef.Attributes |=
                        PEAPI.FieldAttr.SpecialName | PEAPI.FieldAttr.RTSpecialName;
                }
            ...
            if (override_list != null)
            {
                foreach (DictionaryEntry entry in override_list)
                {
                    ...
                    BaseMethodRef over_meth = parent_type.GetMethodRef(
                        body.RetType,
                        body.CallConv,
                        over_name,
                        body.ParamTypeList(),
                    ...
                    classdef.AddMethodOverride(over_meth.PeapiMethod, body.PeapiMethodDef);
                }
            }
            ...
            if (override_long_list != null)
            {
                foreach (DictionaryEntry entry in override_long_list)
                {
                    ...
                    classdef.AddMethodOverride(decl.PeapiMethod, body.PeapiMethodDef);
                }
            }
        }

        public PEAPI.Method ResolveMethod(
        ...
        {
            string signature = MethodDef.CreateSignature(
                ret_type,
                call_conv,
                name,
                param,
        ...
        public PEAPI.Method ResolveVarargMethod(
            BaseTypeRef ret_type,
            PEAPI.CallConv call_conv,
            string name,
            BaseTypeRef[] param,
        ...
        private PEAPI.Method ResolveAsMethodRef(
            BaseTypeRef ret_type,
            PEAPI.CallConv call_conv,
            string name,
            BaseTypeRef[] param,
        ...
        )
        {
            ...
            ExternMethodRef methodref = (ExternMethodRef)
                type_ref.GetMethodRef(ret_type, call_conv, name, param, gen_param_count);
