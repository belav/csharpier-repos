...
namespace Mono.ILASM
{
    public class MethodDef : ICustomAttrTarget, IDeclSecurityTarget
    {
        ...
        public MethodDef(
            CodeGen codegen,
            PEAPI.MethAttr meth_attr,
            PEAPI.CallConv call_conv,
            PEAPI.ImplAttr impl_attr,
            string name,
        ...
        public void AddPInvokeInfo(
            PEAPI.PInvokeAttr pinvoke_attr,
        ...
        public PEAPI.MethodDef Resolve(CodeGen code_gen, PEAPI.ClassDef classdef)
        {
            ...
            if (classdef == null)
                methoddef = code_gen.PEFile.AddMethod(
                    meth_attr,
                    impl_attr,
                    name,
                    ret_param.PeapiParam,
                    param_array
                );
            else
        ...
        public PEAPI.MethodRef GetVarargSig(PEAPI.Type[] opt, string full_signature)
        {
            if (!is_resolved)
                throw new InternalErrorException(
                    "Methods must be resolved before a vararg sig can be created."
                );
        ...
        protected void WriteCode(CodeGen code_gen, PEAPI.MethodDef methoddef)
        {
                ...
                if (IsAbstract)
                {
                    if (!type_def.IsAbstract)
                        Report.Error(
                            start,
                            String.Format(
                                "Abstract method '{0}' in non-abstract class '{1}'",
                                Name,
                                type_def.FullName
                            )
                        );
            ...
            if (pinvoke_info)
            {
                methoddef.AddPInvokeInfo(
                    pinvoke_mod.ModuleRef,
                    (pinvoke_name != null ? pinvoke_name : name),
                    pinvoke_attr
                );
            }
            ...
            if ((impl_attr & PEAPI.ImplAttr.Runtime) == PEAPI.ImplAttr.Runtime)
            {
                if (inst_list.Count > 0)
                    Report.Error(
                        start,
                        String.Format(
                            "Method cannot have body if it is non-IL runtime-supplied, '{0}'",
                            FullName
                        )
                    );
            }
            ...
            {
                if (
                    ((impl_attr & PEAPI.ImplAttr.Native) != 0)
                    || ((impl_attr & PEAPI.ImplAttr.Unmanaged) != 0)
                )
                    Report.Error(
                        start,
                        String.Format("Cannot compile native/unmanaged method, '{0}'", FullName)
                    );
            }
            ...
            if (inst_list.Count > 0)
            {
                ...
                if ((impl_attr & PEAPI.ImplAttr.InternalCall) != 0)
                    Report.Error(
                        start,
                        String.Format(
                            "Method cannot have body if it is an internal call, '{0}'",
                            FullName
                        )
                    );
                ...
                if (pinvoke_info)
                    Report.Error(
                        start,
                        String.Format("Method cannot have body if it is pinvoke, '{0}'", FullName)
                    );
            }
            else
            {
        ...
        static string CreateVarargSignature(
            BaseTypeRef RetType,
            string name,
            BaseTypeRef[] param_list,
            bool include_optional
