    ...
    public class Driver
    {
        ...
        private class DriverMain
        {
            ...
            private void ProcessFile(string file_path)
            {
                ...
                {
                    Console.WriteLine("File does not exist: {0}", file_path);
                    Environment.Exit(2);
                }
                Report.AssembleFile(file_path, null, target_string, output_file);
                ...
                catch (Exception)
                {
                    Console.Write(
                        "{0} ({1}, {2}): ",
                        file_path,
                        scanner.Reader.Location.line,
                        scanner.Reader.Location.column
                    );
                    throw;
                }
                finally
