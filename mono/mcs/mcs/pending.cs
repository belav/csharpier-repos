...
namespace Mono.CSharp
{
    ...
    public class PendingImplementation
    {
        ...
        PendingImplementation(
            ...
            MissingInterfacesInfo[] missing_ifaces,
            MethodSpec[] abstract_methods,
            int total
        )
        {
        ...
        Report Report
        {
            get { return container.Module.Compiler.Report; }
        }
        ...
        static readonly MissingInterfacesInfo[] EmptyMissingInterfacesInfo =
            new MissingInterfacesInfo[0];
        ...
        static public PendingImplementation GetPendingImplementations(TypeDefinition container)
        {
            ...
            //
            bool implementing_abstract = (
                (b != null) && b.IsAbstract && (container.ModFlags & Modifiers.ABSTRACT) == 0
            );
            ...
            var pending = new PendingImplementation(
                container,
                missing_interfaces,
                abstract_methods,
                total
                ...
                for (int i = 0; i < p.methods.Count; ++i)
                {
                    ...
                    for (int ii = i + 1; ii < p.methods.Count; ++ii)
                    {
                        ...
                        for (int pi = 0; pi < cp.Length; ++pi)
                        {
                            ...
                            //
                            if (
                                (cp[pi].ModFlags & Parameter.Modifier.RefOutMask)
                                == (tp[pi].ModFlags & Parameter.Modifier.RefOutMask)
                            )
                                continue;

                            if (
                                (
                                    (cp[pi].ModFlags | tp[pi].ModFlags)
        ...
        /// </summary>
        public MethodSpec IsInterfaceMethod(
            MemberName name,
            TypeSpec ifaceType,
            MethodData method,
            out MethodSpec ambiguousCandidate,
            ref bool optional
        )
        {
        ...
        )
        {
            InterfaceMethod(
                ...
                ifaceType,
                method,
                clear_one ? Operation.ClearOne : Operation.ClearAll,
                out ambiguousCandidate,
                ref optional
            );
        }
        ...
        )
        {
            ...
            bool is_indexer =
                method.method is Indexer.SetIndexerMethod
                || method.method is Indexer.GetIndexerMethod;
            MethodSpec m;
        ...
        void DefineProxy(TypeSpec iface, MethodSpec base_method, MethodSpec iface_method)
        {
            ...
            if (iface_method.IsGeneric)
            {
                var gnames = iface_method
                    .GenericDefinition.TypeParameters.Select(l => l.Name)
                    .ToArray();
            ...
            for (int i = 0; i < param.Count; i++)
            {
                ...
                ParameterAttributes attr = ParametersCompiled.GetParameterAttribute(
                    param.FixedParameters[i].ModFlags
                );
                proxy.DefineParameter(i + 1, attr, name);
            }

            int top = param.Count;
            var ec = new EmitContext(
                new ProxyMethodContext(container),
                proxy.GetILGenerator(),
                null,
                null
            );
            ec.EmitThis();
        ...
        bool BaseImplements(TypeSpec iface_type, MethodSpec mi, out MethodSpec base_method)
        {
            ...
            if (mi.IsAccessor && container.Interfaces != null)
            {
                ...
                if (new_implementation)
                {
                    ...
                    {
                        var indexer_params = getter
                            ? mi.Parameters
                            : IndexerSpec.CreateParametersFromSetter(mi, mi.Parameters.Count - 1);
                        var ptype = getter
                            ? mi.ReturnType
                            : mi.Parameters.Types[mi.Parameters.Count - 1];
                    ...
                    var prop = MemberCache.FindMember(
            ...
            while (true)
            {
                ...
                foreach (var candidate in candidates)
                {
                    ...
                    for (int i = 0; i < parameters.Count; ++i)
                    {
                        ...
                        //
                        if (
                            (parameters.FixedParameters[i].ModFlags & Parameter.Modifier.RefOutMask)
                            == (
                                candidate_param.FixedParameters[i].ModFlags
                                & Parameter.Modifier.RefOutMask
                            )
                        )
                            continue;
                    ...
                    if (
                        mi.IsGeneric && !Method.CheckImplementingMethodConstraints(container, m, mi)
                    )
                    {
                        return true;
                    }
                ...
                if (base_method != null)
                {
                    if (similar_candidate != null)
                    {
                        ...
                        Report.Warning(
                            1956,
                            1,
                            ((MemberCore)base_method.MemberDefinition).Location,
                            ...
                            similar_candidate.GetSignatureForError(),
            ...
            if (!base_method.IsVirtual)
            {
                ...
                if (base_builder != null)
                {
                    ...
                    //
                    base_builder.__SetAttributes(
                        base_builder.Attributes
                            | MethodAttributes.Virtual
                            | MethodAttributes.Final
                            | MethodAttributes.NewSlot
                    );
        ...
        public bool VerifyPendingMethods()
        {
            ...
            for (i = 0; i < top; i++)
            {
                ...
                for (int j = 0; j < pending_implementations[i].methods.Count; ++j)
                {
                    ...
                    if (type.IsInterface)
                    {
                        var need_proxy = pending_implementations[i].need_proxy[j];
                        ...
                        if (candidate != null)
                        {
                            ...
                            else
                            {
                                Report.Error(
                                    ...
                                    candidate.GetSignatureForError(),
                                    candidate.ReturnType.GetSignatureForError(),
                                    mi.ReturnType.GetSignatureForError()
                                );
                            }
                        }
                        else
