    ...
    public sealed class ModuleContainer : TypeContainer
    {
        ...
        sealed class StaticDataContainer : CompilerGeneratedContainer
        {
            ...
            public StaticDataContainer(ModuleContainer module)
                : base(
                    module,
                    new MemberName("<PrivateImplementationDetails>", Location.Null),
            ...
            public FieldSpec DefineInitializedData(byte[] data, Location loc)
            {
                ...
                if (!size_types.TryGetValue(data.Length, out size_type))
                {
                    ...
                    //
                    size_type = new Struct(
                        this,
                        new MemberName("$ArrayType=" + data.Length, loc),
                        Modifiers.PRIVATE | Modifiers.COMPILER_GENERATED,
                        null
                    );
                ...
                if (!data_hashes.TryGetValue(data_hash, out fs))
                {
                    ...
                    const Modifiers fmod =
                        Modifiers.STATIC | Modifiers.INTERNAL | Modifiers.READONLY;
                    var fbuilder = TypeBuilder.DefineField(
                        name,
                        size_type.CurrentType.GetMetaInfo(),
                        ModifiersExtensions.FieldAttr(fmod) | FieldAttributes.HasFieldRVA
                    );
        ...
        public sealed class PatternMatchingHelper : CompilerGeneratedContainer
        {
            public PatternMatchingHelper(ModuleContainer module)
                : base(
                    module,
                    new MemberName("<PatternMatchingHelper>", Location.Null),
            ...
            Method GenerateNumberMatcher()
            {
                ...
                var m = new Method(
                    this,
                    new TypeExpression(Compiler.BuiltinTypes.Bool, loc),
                    Modifiers.PUBLIC | Modifiers.STATIC | Modifiers.DEBUGGER_HIDDEN,
                    new MemberName("NumberMatcher", loc),
                    parameters,
                    null
                );
                ...
                var if_enum = new If(
                    new Binary(
                        ...
                        new Is(
                            top_block.GetParameterReference(0, loc),
                            new TypeExpression(Compiler.BuiltinTypes.Enum, loc),
                            loc
                        ),
                        new Binary(
                ...
                var system_convert = new MemberAccess(
                ...
                //
                var lv_converted = LocalVariable.CreateCompilerGenerated(
                    Compiler.BuiltinTypes.Object,
                    top_block,
                    loc
                );
                ...
                var gettypecode = new Invocation(
                    new MemberAccess(system_convert, "GetTypeCode", loc),
                    arguments_gettypecode
                );
                ...
                var changetype = new Invocation(
                    new MemberAccess(system_convert, "ChangeType", loc),
                    arguments_changetype
                );

                expl_block.AddStatement(
                    new StatementExpression(
                        new SimpleAssign(
                            new LocalVariableReference(lv_converted, loc),
                            changetype,
                            loc
                        )
                    )
                ...
                var equals_invocation = new Invocation(
                    new MemberAccess(new LocalVariableReference(lv_converted, loc), "Equals"),
                    equals_arguments
                );
                expl_block.AddStatement(new Return(equals_invocation, loc));

                var catch_block = new ExplicitBlock(top_block, loc, loc);
                catch_block.AddStatement(
                    new Return(new BoolLiteral(Compiler.BuiltinTypes, false, loc), loc)
                );
                top_block.AddStatement(
                    new TryCatch(
                        expl_block,
                        new List<Catch>() { new Catch(catch_block, loc) },
                        loc,
                        false
                    )
                );
        ...
        {
            get { return attrs_cache; }
        }

        public override AttributeTargets AttributeTargets
        {
            get { return AttributeTargets.Assembly; }
        }
        ...
        internal DocumentationBuilder DocumentationBuilder { get; set; }
        ...
        {
            get { return DefaultCharSet.HasValue; }
        }

        public bool HasExtensionMethod
        {
            get { return has_extenstion_method; }
            set { has_extenstion_method = value; }
        }
        ...
        public AwaiterDefinition GetAwaiter(TypeSpec type)
        {
            ...
            awaiter.IsCompleted =
                MemberCache.FindMember(
                    type,
                    MemberFilter.Property("IsCompleted", Compiler.BuiltinTypes.Bool),
            ...
            awaiter.INotifyCompletion =
                !nc.Define() || type.ImplementsInterface(nc.TypeSpec, false);
        ...
        public Binary.PredefinedOperator[] GetPredefinedEnumAritmeticOperators(
            TypeSpec enumType,
            bool nullable
        )
        {
            ...
            var operators = new[]
            {
                new Binary.PredefinedOperator(
                    enumType,
                    underlying,
                    mask
                        | Binary.Operator.AdditionMask
                        | Binary.Operator.SubtractionMask
                        | Binary.Operator.DecomposedMask,
                    enumType
                ),
                new Binary.PredefinedOperator(
                    underlying,
                ...
                new Binary.PredefinedOperator(
                    enumType,
                    mask | Binary.Operator.SubtractionMask,
                    underlying
                ),
            };
