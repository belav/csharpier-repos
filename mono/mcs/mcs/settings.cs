...
namespace Mono.CSharp
{
    ...
    public class CompilerSettings
    {
        ...
        {
            get { return source_files.Count > 0 ? source_files[0] : null; }
        }

        public bool HasKeyFileOrContainer
        {
            get { return StrongNameKeyFile != null || StrongNameKeyContainer != null; }
        }
    ...
    public class CommandLineParser
    {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                if (arg[0] == '@')
                {
                    ...
                    if (response_file_list.Contains(response_file))
                    {
                        report.Error(
                            1515,
                            "Response file `{0}' specified multiple times",
                            response_file
                        );
                        return false;
                    }
        ...
        void AddResource(AssemblyResource res, CompilerSettings settings)
        {
            ...
            if (settings.Resources.Contains(res))
            {
                report.Error(
                    1508,
                    "The resource identifier `{0}' has already been used in this assembly",
                    res.Name
                );
                return;
            }
            ...
            if (source_file_index.TryGetValue(path, out index))
            {
                ...
                if (fileName.Equals(other_name))
                    ...
                    report.Warning(
                        2002,
                        1,
                        "Source filenames `{0}' and `{1}' both refer to the same file: {2}",
                        fileName,
                        other_name,
                        path
                    );
        ...
        ParseResult ParseOption(string option, ref string[] args, CompilerSettings settings)
        {
            ...
            switch (arg.ToLowerInvariant())
            {
                ...
                case "/target":
                    switch (value)
                    {
                        ...
                        default:
                            report.Error(
                                2019,
                                "Invalid target type for -target. Valid options are `exe', `winexe', `library' or `module'"
                            );
                    ...
                    foreach (string d in value.Split(argument_value_separator))
                    {
                        ...
                        if (!Tokenizer.IsValidIdentifier(conditional))
                        {
                            report.Warning(
                                2029,
                                1,
                                "Invalid conditional define symbol `{0}'",
                                conditional
                            );
                            continue;
                        }
                ...
                case "/bugreport":
                    ...
                    //
                    output.WriteLine(
                        "To file bug reports, please visit: http://www.mono-project.com/Bugs"
                    );
                    return ParseResult.Success;
                    ...
                    switch (s.Length)
                    {
                        ...
                        case 3:
                            if (s[2] != "public" && s[2] != "private")
                            {
                                report.Error(
                                    1906,
                                    "Invalid resource visibility option `{0}'. Use either `public' or `private' instead",
                                    s[2]
                                );
                                return ParseResult.Error;
                            }
                ...
                case "/debug":
                    if (
                        ...
                        || value.Equals("pdbonly", StringComparison.OrdinalIgnoreCase)
                        || value.Equals("portable", StringComparison.OrdinalIgnoreCase)
                        || idx < 0
                    )
                    {
                    ...
                    if (value.Length > 0)
                    {
                        report.Error(
                            1902,
                            "Invalid debug option `{0}'. Valid options are `full' or `pdbonly'",
                            value
                        );
                    }
                    else
                ...
                case "/platform":
                    ...
                    switch (value.ToLowerInvariant())
                    {
                        ...
                        default:
                            report.Error(
                                1672,
                                "Invalid -platform option `{0}'. Valid options are `anycpu', `anycpu32bitpreferred', `arm', `x86', `x64' or `itanium'",
                                value
                            );
                            return ParseResult.Error;
                    }
                ...
                case "/langversion":
                    ...
                    else
                    {
                        report.Error(
                            1617,
                            "Invalid -langversion option `{0}'. It must be `ISO-1', `ISO-2', Default, Latest or value in range 1 to 7.2",
                            value
                        );
                    }
            ...
            switch (arg)
            {
                ...
                case "-m":
                    report.Warning(
                        -29,
                        1,
                        "Compatibility: Use -main:CLASS instead of --main CLASS or -m CLASS"
                    );
                    if ((i + 1) >= args.Length)
                    {
                ...
                case "--define":
                    report.Warning(
                        -29,
                        1,
                        "Compatibility: Use -d:SYMBOL instead of --define SYMBOL"
                    );
                    if ((i + 1) >= args.Length)
                    {
                        Error_RequiresArgument(arg);
                ...
                case "--output":
                    report.Warning(
                        -29,
                        1,
                        "Compatibility: Use -out:FILE instead of --output FILE or -o FILE"
                    );
                    if ((i + 1) >= args.Length)
                    {
                        Error_RequiresArgument(arg);
                ...
                case "--linkres":
                    report.Warning(
                        -29,
                        1,
                        "Compatibility: Use -linkres:VALUE instead of --linkres VALUE"
                    );
                    if ((i + 1) >= args.Length)
                    {
                        Error_RequiresArgument(arg);
                ...
                case "--target":
                    report.Warning(
                        -29,
                        1,
                        "Compatibility: Use -target:KIND instead of --target KIND"
                    );
                    if ((i + 1) >= args.Length)
                    {
                        Error_RequiresArgument(arg);
                ...
                case "--wlevel":
                    report.Warning(
                        -29,
                        1,
                        "Compatibility: Use -warn:LEVEL instead of --wlevel LEVEL"
                    );
                    if ((i + 1) >= args.Length)
                    {
                        Error_RequiresArgument(arg);
                ...
                case "--recurse":
                    report.Warning(
                        -29,
                        1,
                        "Compatibility: Use -recurse:PATTERN option instead --recurse PATTERN"
                    );
                    if ((i + 1) >= args.Length)
                    {
                        Error_RequiresArgument(arg);
        ...
        void Version()
        {
            string version = System.Reflection.MethodBase
                .GetCurrentMethod()
