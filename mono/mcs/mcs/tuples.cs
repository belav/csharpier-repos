...
namespace Mono.CSharp
{
    ...
    public class NamedTupleSpec : TypeSpec
    {
        ...
        private NamedTupleSpec(InflatedTypeSpec tupleDefinition, IList<string> elements)
            : base(
                tupleDefinition.Kind,
                tupleDefinition.DeclaringType,
                tupleDefinition.MemberDefinition,
                null,
                tupleDefinition.Modifiers
            )
        {
        ...
        {
            get { return tuple.TypeArguments; }
        }
        ...
        public MemberSpec FindElement(IMemberContext mc, string name, Location loc)
        {
            ...
            for (int i = 0; i < elements.Count; ++i)
            {
                ...
                var ms = MemberCache.FindMember(
                    tuple,
                    MemberFilter.Field(member_name, null),
        ...
        public static NamedTupleSpec MakeType(
            ModuleContainer module,
            InflatedTypeSpec tupleType,
            IList<string> names
        )
        {
            // TODO: cache it
    ...
    class TupleDeconstruct : ExpressionStatement, IAssignMethod
    {
        ...
        protected override Expression DoResolve(ResolveContext rc)
        {
            ...
            if (src_type.IsTupleType)
            {
                ...
                {
                    var expr_variable = LocalVariable.CreateCompilerGenerated(
                        source.Type,
                        rc.CurrentBlock,
                        loc
                    );
                    source = new CompilerAssign(
                        expr_variable.CreateReferenceExpression(rc, loc),
                        source,
                        loc
                    );
                ...
                for (int i = 0; i < target_count; ++i)
                {
                    var element_src =
                        tupleLiteral == null
                            ? new MemberAccess(instance, NamedTupleSpec.GetElementPropertyName(i))
                            : tupleLiteral.Elements[i].Expr;
                ...
                for (int i = 0; i < target_count; ++i)
                {
                    ...
                    if (
                        element_target != null
                        && src_names.Contains((element_target as VariableReference)?.Name)
                    )
                    {
                    ...
                    {
                        targetExprs[i] = new SimpleAssign(targetExprs[i], element_srcs[i]).Resolve(
                            rc
                        );
                    }
                }
