    ...
    public struct MemberFilter : IEquatable<MemberSpec>
    {
        ...
        public static MemberFilter Constructor(AParametersCollection param)
        {
            return new MemberFilter(
                Mono.CSharp.Constructor.ConstructorName,
                0,
        ...
        public static MemberFilter Method(
            string name,
            int arity,
            AParametersCollection param,
            TypeSpec type
    ...
    public class MemberCache
    {
        ...
        void AddMember(string name, MemberSpec member, bool removeHiddenMembers)
        {
            if (member.Kind == MemberKind.Operator)
            {
                ...
                //
                if (
                    !BuiltinTypeSpec.IsPrimitiveType(dt)
                    || dt.BuiltinType == BuiltinTypeSpec.Type.Char
                )
                {
                    switch (dt.BuiltinType)
                    {
                        ...
                        default:
                            if (
                                name == Operator.GetMetadataName(Operator.OpType.Implicit)
                                || name == Operator.GetMetadataName(Operator.OpType.Explicit)
                            )
                            {
        ...
        {
            var member_param =
                member is IParametersMember ? ((IParametersMember)member).Parameters : null;
            ...
            for (int i = 0; i < existing.Count; ++i)
            {
                ...
                if (
                    (
                        entry.DeclaringType == member.DeclaringType
                        && entry.IsAccessor == member.IsAccessor
                    )
                )
                    return false;
                ...
                if (
                    ...
                    && AParametersCollection.HasSameParameterDefaults(entry_param, member_param)
                )
                    return false;
            }
        ...
        public static MemberSpec FindMember(
            TypeSpec container,
            MemberFilter filter,
        ...
        {
            if (
                filter.Kind == MemberKind.Method
                && container.Kind == MemberKind.TypeParameter
                && filter.Parameters == null
            )
                throw new NotSupportedException(
                    "type parameters methods cannot be lookup up due to two stage setup"
                );
            ...
            do
            {
                if (container.MemberCache.member_hash.TryGetValue(filter.Name, out applicable))
                {
                    ...
                    for (int i = applicable.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (
                            (restrictions & BindingRestriction.OverrideOnly) != 0
                            && (entry.Modifiers & Modifiers.OVERRIDE) == 0
                        )
                            continue;
                        ...
                        if (
                            (restrictions & BindingRestriction.DeclaredOnly) != 0
                            && container.IsInterface
                            && entry.DeclaringType != container
                        )
                            continue;
        ...
        // compacted
        //
        public static IList<MemberSpec> FindMembers(
            TypeSpec container,
            string name,
            bool declaredOnlyClass
        )
        {
            do
            {
                ...
                if (
                    container.MemberCache.member_hash.TryGetValue(name, out applicable)
                    || declaredOnlyClass
                )
                    return applicable;
        ...
        public static IList<MemberSpec> FindInterfaceMembers(
            TypeParameterSpec typeParameter,
            string name
        )
        {
        ...
        //
        public static TypeSpec FindNestedType(
            TypeSpec container,
            string name,
            int arity,
            bool declaredOnlyClass
                    ...
                    for (int i = applicable.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (arity < 0)
                        {
                            if (best_match == null)
                            {
                                ...
                                System.Math.Abs(ts.Arity + arity)
                                < System.Math.Abs(best_match.Arity + arity)
                            )
                            {
                                best_match = ts;
                            }
        ...
        //
        public List<MethodSpec> FindExtensionMethods(
        ...
        // the best match
        //
        public static MemberSpec FindBaseMember(
            MemberCore member,
        ...
        )
        {
            ...
            bool member_with_accessors =
                mkind == MemberKind.Indexer || mkind == MemberKind.Property;
            ...
            do
            {
                if (container.MemberCache.member_hash.TryGetValue(name, out applicable))
                {
                    for (int i = 0; i < applicable.Count; ++i)
                    {
                        ...
                        //
                        if (
                            (entry.Kind & ~MemberKind.Destructor & mkind & MemberKind.MaskType) == 0
                        )
                        {
                        ...
                        //
                        if (
                            member_with_accessors
                            && (
                                (entry.Modifiers & (Modifiers.OVERRIDE | Modifiers.SEALED))
                                == Modifiers.OVERRIDE
                            )
                        )
                        {
                        ...
                        //
                        if (
                            ambig_candidate == null
                            && (entry.Kind & mkind & (MemberKind.Method | MemberKind.Indexer)) != 0
                        )
                        {
                            bestCandidate = null;
        ...
        //
        public static T GetMember<T>(TypeSpec container, T spec)
            where T : MemberSpec
        {
            IList<MemberSpec> applicable;
            ...
            throw new InternalErrorException(
                "Missing member `{0}' on inflated type `{1}'",
        ...
        public static List<FieldSpec> GetAllFieldsForDefiniteAssignment(
            TypeSpec container,
        ...
        public static IList<MemberSpec> GetCompletitionMembers(
            IMemberContext ctx,
            TypeSpec container,
            string name
        )
            ...
            foreach (var entry in container.MemberCache.member_hash)
            {
                foreach (var name_entry in entry.Value)
                {
                    ...
                    if (
                        (
                            name_entry.Kind
                            & (MemberKind.Constructor | MemberKind.Destructor | MemberKind.Operator)
                        ) != 0
                    )
                        continue;
        ...
        //
        public static IList<MemberSpec> GetUserOperator(
            TypeSpec container,
            Operator.OpType op,
            bool declaredOnly
        )
        {
            ...
            do
            {
                ...
                if (
                    (
                        (op == Operator.OpType.Implicit || op == Operator.OpType.Explicit)
                        && (mc.state & StateFlags.HasConversionOperator) != 0
                    )
        ...
        public void InflateTypes(MemberCache inflated_cache, TypeParameterInflator inflator)
        {
            foreach (var item in member_hash)
            {
                ...
                for (int i = 0; i < item.Value.Count; ++i)
                {
                    ...
                    if (
                        (member.Kind & MemberKind.NestedMask) != 0
        ...
        public void RemoveHiddenMembers(TypeSpec container)
        {
            foreach (var entry in member_hash)
            {
                ...
                while (
                    values[container_members_start_at].DeclaringType != container
                    && ++container_members_start_at < entry.Value.Count
                )
                    ;
                ...
                for (int i = 0; i < container_members_start_at; ++i)
                {
                    ...
                    var member_param =
                        member is IParametersMember
                            ? ((IParametersMember)member).Parameters
        ...
        public void VerifyClsCompliance(TypeSpec container, Report report)
        {
            ...
            else
            {
                ...
                locase_members = new Dictionary<string, MemberSpec[]>(
                    btype.MemberCache.locase_members
            ...
            foreach (var entry in container.MemberCache.member_hash)
            {
                for (int i = 0; i < entry.Value.Count; ++i)
                {
                    ...
                    if (
                        (name_entry.Modifiers & (Modifiers.OVERRIDE | Modifiers.COMPILER_GENERATED))
                        != 0
                    )
                        continue;
                    ...
                    if (!is_imported_type)
                    {
                        ...
                        if (p_a != null && !name_entry.IsAccessor)
                        {
                            ...
                            for (int ii = i + 1; ii < entry.Value.Count; ++ii)
                            {
                                ...
                                var res = ParametersCompiled.IsSameClsSignature(
                                    p_a.Parameters,
                                    p_b.Parameters
                                );
                                if (res != 0)
                    ...
                    if (
                        i > 0
                        || name_entry.Kind == MemberKind.Constructor
                        || name_entry.Kind == MemberKind.Indexer
                    )
                        continue;
        ...
        static void ReportOverloadedMethodClsDifference(
            MemberSpec a,
            MemberSpec b,
            int res,
            Report report
        ...
        public bool CheckExistingMembersOverloads(
            MemberCore member,
            ...
            for (int i = entries.Count - 1; i >= 0; --i)
            {
                ...
                if (method_param_count > 0)
                {
                    ...
                    do
                    {
                        ...
                        var a_byref =
                            (pd.FixedParameters[ii].ModFlags & Parameter.Modifier.RefOutMask) != 0;
                        var b_byref =
                            (
                                parameters.FixedParameters[ii].ModFlags
                                & Parameter.Modifier.RefOutMask
                            ) != 0;
                    ...
                    if (
                        member is Operator
                        && ce.Kind == MemberKind.Operator
                        && ((MethodSpec)ce).ReturnType != ((Operator)member).ReturnType
                    )
                        continue;
                    ...
                    if (pd != null && member is MethodCore)
                    {
                        ...
                        while (
                            ...
                            && (
                                parameters.FixedParameters[ii].ModFlags
                                & Parameter.Modifier.ModifierMask
                        ...
                        if (ii >= 0)
                        {
                            ...
                            if (
                                (member.ModFlags & Modifiers.PARTIAL) != 0
                                && (mc.Modifiers & Modifiers.PARTIAL) != 0
                            )
                            {
                        ...
                        if (
                            !NamedTupleSpec.CheckOverrideName(pm, pm_member)
                            || !NamedTupleSpec.CheckOverrideName(
                                pm.MemberType,
                                pm_member.MemberType
                            )
                        )
                        {
                            Report.Error(
                ...
                if ((ce.Kind & MemberKind.Method) != 0)
                {
                    ...
                    if (
                        method_a != null
                        && method_b != null
                        && (method_a.ModFlags & method_b.ModFlags & Modifiers.PARTIAL) != 0
                    )
                    {
                        ...
                        {
                            if (
                                (method_a.ModFlags & partial_modifiers)
                                    == (method_b.ModFlags & partial_modifiers)
                            ...
                            if (
                                (method_a.ModFlags & Modifiers.UNSAFE)
                                != (method_b.ModFlags & Modifiers.UNSAFE)
                            )
                            {
                                Report.SymbolRelatedToPreviousError(ce);
                                Report.Error(
                    ...
                    bool is_reserved_a =
                        member is AbstractPropertyEventMethod || member is Operator;
