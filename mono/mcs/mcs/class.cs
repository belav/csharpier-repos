...
namespace Mono.CSharp
{
    ...
    public abstract class TypeContainer : MemberCore
    {
        ...
        public TypeDefinition PartialContainer
        {
            ...
            protected set { main_container = value; }
        }
        ...
        protected void AddPartial(TypeDefinition next_part, TypeDefinition existing)
        {
            ...
            if (
                (existing.ModFlags & Modifiers.AccessibilityMask)
                    != (next_part.ModFlags & Modifiers.AccessibilityMask)
                && (
            ...
            if ((next_part.ModFlags & Modifiers.DEFAULT_ACCESS_MODIFIER) != 0)
            {
                existing.ModFlags |=
                    next_part.ModFlags
                    & ~(Modifiers.DEFAULT_ACCESS_MODIFIER | Modifiers.AccessibilityMask);
            }
        ...
        {
            if (Parent != null && Parent.MemberName != null)
                return Parent.GetSignatureForDocumentation()
                    + "."
                    + MemberName.GetSignatureForDocumentation();
    ...
    public abstract class TypeDefinition : TypeContainer, ITypeDefinition
    {
        ...
        Dictionary<string, FullNamedExpression> Cache =
        ...
        {
            get { return PartialContainer.MemberName.TypeParameters; }
        }
        ...
        {
            get { return (caching_flags & Flags.HasInstanceConstructor) != 0; }
            set { caching_flags |= Flags.HasInstanceConstructor; }
        }

        public bool HasInstanceField
        {
            get { return (caching_flags & Flags.HasInstanceField) != 0; }
            set { caching_flags |= Flags.HasInstanceField; }
        }
        ...
        {
            get { return (caching_flags & Flags.HasUserOperators) != 0; }
            set { caching_flags |= Flags.HasUserOperators; }
        }
        ...
        {
            get { return iface_exprs; }
        }

        public bool IsGenericOrParentIsGeneric
        {
            get { return all_type_parameters != null; }
        }

        public bool IsTopLevel
        {
            get { return !(Parent is TypeDefinition); }
        }

        public bool IsPartial
        {
            get { return (ModFlags & Modifiers.PARTIAL) != 0; }
        }
        ...
        bool IsPartialPart
        {
            get { return PartialContainer != this; }
        }

        public MemberCache MemberCache
        {
            get { return spec.MemberCache; }
        }
        ...
        {
            get { return all_type_parameters; }
        }
        ...
        public override string[] ValidAttributeTargets
        {
            get
            {
                return PrimaryConstructorParameters != null
                    ? attribute_targets_primary
                    : attribute_targets;
            }
        }
        ...
        public virtual void AddNameToContainer(MemberCore symbol, string name)
        {
            ...
            if (
                ...
                && (symbol is ClassOrStruct || symbol is Interface)
            )
            {
        ...
        public void AddConstructor(Constructor c, bool isDefault)
        {
            ...
            if (!isDefault)
                AddNameToContainer(
                    c,
                    is_static ? Constructor.TypeConstructorName : Constructor.ConstructorName
                );
            ...
            if (Kind == MemberKind.Struct)
            {
                ...
                if (first_field.Parent != field.Parent)
                {
                    ...
                    Report.Warning(
                        ...
                        first_field.GetSignatureForError()
                    );
                }
            }
        ...
        {
            get { return spec.BaseType; }
        }

        protected virtual TypeAttributes TypeAttr
        {
            get { return ModifiersExtensions.TypeAttr(ModFlags, IsTopLevel); }
        }

        public int TypeParametersCount
        {
            get { return MemberName.Arity; }
        }
        ...
        public void ResolveFieldInitializers(BlockContext ec)
        {
            ...
            if (ec.IsStatic)
            {
                ...
                int i;
                ExpressionStatement[] init = new ExpressionStatement[
                    initialized_static_fields.Count
                ];
            ...
            for (int i = 0; i < initialized_fields.Count; ++i)
            {
                ...
                if (s == null)
                {
                    initialized_fields[i] = new FieldInitializer(
                        fi.Field,
                        ErrorExpression.Instance,
                        Location.Null
                    );
                    continue;
                }
                ...
                //
                if (
                    fi.IsDefaultInitializer
                    && Kind != MemberKind.Struct
                    && ec.Module.Compiler.Settings.Optimize
                )
                    continue;
        ...
        public override string GetSignatureForMetadata()
        {
            ...
            {
                return Parent.GetSignatureForMetadata()
                    + "+"
                    + TypeNameParser.Escape(FilterNestedName(MemberName.Basename));
            }
            ...
            for (int i = 0; i < members.Count; ++i)
            {
                ...
                if (!pair_found)
                {
                    Report.Error(
                        ...
                        o_a.GetSignatureForError(),
                        Operator.GetName(matching_type)
                    );
                }
            }
        ...
        bool CreateTypeBuilder()
        {
            ...
            //
            int type_size =
                Kind == MemberKind.Struct && !HasInstanceField && !(this is StateMachine) ? 1 : 0;
        ...
        {
            if (
                Module.DeclaringAssembly.SymbolWriter == null
        ...
        public MethodSpec CreateHoistedBaseCallProxy(ResolveContext rc, MethodSpec method)
        {
            ...
            {
                string name = CompilerGeneratedContainer.MakeName(
                    method.Name,
                    null,
                    "BaseCallProxy",
                    hoisted_base_call_proxies.Count
                );
                ...
                if (method.IsGeneric)
                {
                    ...
                    for (int i = 0; i < hoisted_tparams.Length; ++i)
                    {
                        ...
                        tp_name += "_Proxy";
#endif
                        var local_tp = new TypeParameter(
                            tp,
                            null,
                            new MemberName(tp_name, Location),
                            null
                        );
                    ...
                    var inflator = new TypeParameterInflator(
                        this,
                        null,
                        hoisted_tparams,
                        targs.Arguments
                ...
                for (int i = 0; i < base_parameters.Length; ++i)
                {
                    ...
                    base_parameters[i] = new Parameter(
                        new TypeExpression(local_param_types[i], Location),
                        base_param.Name,
                        base_param.ModFlags,
                        null,
                        Location
                ...
                var cloned_params = ParametersCompiled.CreateFullyResolved(
                    base_parameters,
                    local_param_types
                );
                if (method.Parameters.HasArglist)
                {
                    cloned_params.FixedParameters[0] = new Parameter(
                        null,
                        "__arglist",
                ...
                proxy_method = new Method(
                    this,
                    new TypeExpression(return_type, Location),
        ...
        bool DoDefineBaseType()
        {
            ...
            if (IsPartialPart)
            {
                ...
                {
                    if (
                        PartialContainer.base_type_expr != null
                ...
                if (PrimaryConstructorParameters != null)
                {
                    ...
                    {
                        PartialContainer.PrimaryConstructorParameters =
        ...
        protected virtual bool DoDefineMembers()
        {
            ...
            if (iface_exprs != null)
            {
                foreach (var iface_type in iface_exprs)
                {
                    ...
                    if (iface_type.Arity > 0)
                    {
                        ...
                        if (
                            ((InflatedTypeSpec)iface_type).HasDynamicArgument()
                            && !IsCompilerGenerated
                        )
                        {
                            Report.Error(
                                ...
                                GetSignatureForError(),
                                iface_type.GetSignatureForError()
                            );
                            return false;
                        }
                    }
                    ...
                    if (iface_type.IsGenericOrParentIsGeneric)
                    {
                        foreach (var prev_iface in iface_exprs)
                        {
                            ...
                            Report.Error(
                                ...
                                GetSignatureForError(),
                                prev_iface.GetSignatureForError(),
                                iface_type.GetSignatureForError()
                            );
                        }
                    }
            ...
            if (HasEquals && !HasGetHashCode)
            {
                Report.Warning(
                    ...
                    3,
                    Location,
                    "`{0}' overrides Object.Equals(object) but does not override Object.GetHashCode()",
                    GetSignatureForError()
                );
            }
        ...
        void ComputeIndexerName()
        {
            ...
            foreach (var indexer in indexers)
            {
                ...
                if (indexer.Name != class_indexer_name)
                    Report.Error(
                        668,
                        ((Indexer)indexer.MemberDefinition).Location,
        ...
        public override void VerifyMembers()
        {
            ...
            if (
                !IsCompilerGenerated
                && Compiler.Settings.WarningLevel >= 3
                && this == PartialContainer
            )
            {
        ...
        public override void Emit()
        {
            ...
            if (!IsCompilerGenerated)
            {
                if (!IsTopLevel)
                {
                    ...
                    var conflict_symbol = MemberCache.FindBaseMember(
                        this,
                        out candidate,
                        ref overrides
                    );
                    ...
                    else
                    {
                        if ((ModFlags & Modifiers.NEW) == 0)
                        {
                            ...
                            Report.Warning(
                                ...
                                GetSignatureForError(),
                                candidate.GetSignatureForError()
                            );
                        }
                    }
                }
            ...
            if (
                (TypeBuilder.Attributes & TypeAttributes.StringFormatMask) == 0
        ...
        public bool VerifyImplements(InterfaceMemberBase mb)
        {
            ...
            Report.Error(
                ...
                mb.GetSignatureForError(),
                mb.InterfaceType.GetSignatureForError()
            );
            return false;
        }
        ...
        {
            get { return (cached_method & CachedMethods.Equals) != 0; }
        }
        ...
        {
            get { return (cached_method & CachedMethods.GetHashCode) != 0; }
        }

        public bool HasStaticFieldInitializer
        {
            get { return (cached_method & CachedMethods.HasStaticFieldInitializer) != 0; }
            set
            {
                if (value)
    ...
    {
        public const TypeAttributes StaticClassAttribute =
        ...
        protected virtual Constructor DefineDefaultConstructor(bool is_static)
        {
            ...
            {
                mods =
                    ((ModFlags & Modifiers.ABSTRACT) != 0) ? Modifiers.PROTECTED : Modifiers.PUBLIC;
                parameters =
                    PrimaryConstructorParameters ?? ParametersCompiled.EmptyReadOnlyParameters;
            }

            var c = new Constructor(this, MemberName.Name, mods, null, parameters, Location);
            if (Kind == MemberKind.Class)
                c.Initializer = new GeneratedBaseInitializer(
                    Location,
    ...
    public sealed class Class : ClassOrStruct
    {
            ...
            : base(parent, name, attrs, MemberKind.Class)
        {
            ...
            this.ModFlags = ModifiersExtensions.Check(
                AllowedModifiers,
                mod,
                accmods,
                Location,
        ...
        public override void SetBaseTypes(List<FullNamedExpression> baseTypes)
        {
            ...
            if (
                pmn.Name == "Object"
                && !pmn.IsGeneric
                && Parent.MemberName.Name == "System"
                && Parent.MemberName.Left == null
            )
                Report.Error(
                    537,
                    Location,
        ...
        public override AttributeTargets AttributeTargets
        {
            get { return AttributeTargets.Class; }
        }
        ...
        protected override bool DoDefineMembers()
        {
            ...
            if (
                (ModFlags & (Modifiers.SEALED | Modifiers.STATIC))
                == (Modifiers.SEALED | Modifiers.STATIC)
            )
            {
                Report.Error(
                    441,
        ...
        public override void GetCompletionStartingWith(string prefix, List<string> results)
        {
            ...
            while (bt != null)
            {
                results.AddRange(
                    MemberCache
                        .GetCompletitionMembers(this, bt, prefix)
                        .Where(l => l.IsStatic)
                        .Select(l => l.Name)
                );
                bt = bt.BaseType;
        ...
        protected override TypeSpec[] ResolveBaseTypes(out FullNamedExpression base_class)
        {
            ...
            else
            {
                ...
                else if (base_type.IsSealed)
                {
                    ...
                    PartialContainer.IsStatic
                    && base_type.BuiltinType != BuiltinTypeSpec.Type.Object
                )
                {
                    Report.Error(
                        713,
    ...
    public sealed class Struct : ClassOrStruct
    {
        bool is_unmanaged,
            ...
            requires_delayed_unmanagedtype_check;
        bool InTransit;
            ...
            : base(parent, name, attrs, MemberKind.Struct)
        {
            var accmods = IsTopLevel ? Modifiers.INTERNAL : Modifiers.PRIVATE;
            this.ModFlags =
                ModifiersExtensions.Check(AllowedModifiers, mod, accmods, Location, Report)
                | Modifiers.SEALED;
            spec = new TypeSpec(Kind, null, this, null, ModFlags);
        }
        ...
        public override AttributeTargets AttributeTargets
        {
            get { return AttributeTargets.Struct; }
        }
        ...
        )
        {
            ...
            if (a.Type == pa.StructLayout)
            {
                ...
                for (int i = 0; i < Members.Count; ++i)
                {
                    ...
                    ff.CharSetValue = (CharSet)
                        System.Enum.Parse(typeof(CharSet), value.GetValue().ToString());
                }
            }
        }
        ...
        protected override bool DoDefineMembers()
        {
            ...
            if (
                PrimaryConstructorParameters != null
                || (initialized_fields != null && !HasUserDefaultConstructor())
            )
            {
                generated_primary_constructor = DefineDefaultConstructor(false);
        ...
        {
            get { return has_unmanaged_check_done; }
        }
        ...
        protected override TypeAttributes TypeAttr
        {
            get
            {
                const TypeAttributes DefaultTypeAttributes =
                    TypeAttributes.AutoLayout | TypeAttributes.Abstract | TypeAttributes.Interface;
    ...
    public abstract class InterfaceMemberBase : MemberBase
    {
        ...
        protected override bool CheckBase()
        {
            ...
            if ((ModFlags & Modifiers.OVERRIDE) != 0)
            {
                if (base_member == null)
                {
                    if (candidate == null)
                    {
                        if (
                            this is Method
                            && ((Method)this).ParameterInfo.IsEmpty
                            && MemberName.Name == Destructor.MetadataName
                            && MemberName.Arity == 0
                        )
                        {
                            Report.Error(
                    ...
                    else
                    {
                        ...
                        if (this is Event)
                            Report.Error(
                                ...
                                GetSignatureForError(),
                                TypeManager.GetFullNameSignature(candidate)
                            );
                ...
                if (candidate != null)
                {
                    ...
                    var m1 = MemberCache.GetMember(
                        base_member.DeclaringType.GetDefinition(),
                        base_member
                    );
                    var m2 = MemberCache.GetMember(
                        candidate.DeclaringType.GetDefinition(),
                        candidate
                    );
                    ...
                    Report.Error(
                        ...
                        GetSignatureForError(),
                        m1.GetSignatureForError(),
                        m2.GetSignatureForError(),
                ...
                if (oa != null)
                {
                    if (
                        OptAttributes == null
                        || !OptAttributes.Contains(Module.PredefinedAttributes.Obsolete)
                    )
                    {
                        Report.SymbolRelatedToPreviousError(base_member);
                        Report.Warning(
            ...
            if (
                base_member == null
                && candidate != null
                && (!(candidate is IParametersMember) || !(this is IParametersMember))
            )
            ...
            else
            {
                if ((ModFlags & Modifiers.NEW) == 0)
                {
                    ...
                    if (!IsCompilerGenerated)
                    {
                        Report.SymbolRelatedToPreviousError(base_member);
                        if (
                            (base_member.Kind & MemberKind.NestedMask) == 0
                            && !IsInterface
                            && (
                                base_member.Modifiers
        ...
        protected virtual bool CheckOverrideAgainstBase(MemberSpec base_member)
        {
            ...
            )
            {
                ...
                Report.Error(
                    ...
                    GetSignatureForError(),
                    TypeManager.CSharpSignature(base_member)
                );
                ok = false;
            }
            ...
            if (!TypeSpecComparer.Override.IsEqual(MemberType, base_member_type))
            {
                ...
                if (((base_member_type.Kind ^ MemberType.Kind) & MemberKind.ByRef) != 0)
                {
                    Report.Error(
                        8148,
                        Location,
                        "`{0}': must {2}return by reference to match overridden member `{1}'",
                        GetSignatureForError(),
                        base_member.GetSignatureForError(),
                        base_member_type.Kind == MemberKind.ByRef ? "" : "not "
                    );
                }
                else if (this is PropertyBasedMember)
                {
                    Report.Error(
                        ...
                        GetSignatureForError(),
                        base_member_type.GetSignatureForError(),
                        base_member.GetSignatureForError()
                    );
                }
                else
            ...
            else if (!NamedTupleSpec.CheckOverrideName(MemberType, base_member_type))
            {
                ...
                Report.Error(
                    8139,
                    Location,
                    "`{0}': cannot change return type tuple element names when overriding inherited member `{1}'",
                    GetSignatureForError(),
                    base_member.GetSignatureForError()
                );
                ok = false;
        ...
        public override bool Define()
        {
            if (IsInterface)
            {
                ModFlags =
                    ...
                    | Modifiers.ABSTRACT
                    | Modifiers.VIRTUAL
                    | (ModFlags & (Modifiers.UNSAFE | Modifiers.NEW));
            ...
            for (int i = 0; i < parameters.Count; ++i)
            {
                ...
                if (
                    ...
                    && (
                        IsExplicitImpl
                        || this is Operator
                        || (this is Indexer && parameters.Count == 1)
                    )
                )
        ...
        public override void Emit()
        {
            ...
            if (
                (ModFlags & Modifiers.EXTERN) != 0
                && !is_external_implementation
                && (OptAttributes == null || !OptAttributes.HasResolveError())
            )
            {
                if (this is Constructor)
                {
                    Report.Warning(
                        ...
                        1,
                        Location,
                        "Constructor `{0}' is marked `external' but has no external implementation specified",
                        GetSignatureForError()
                    );
                }
                else
        ...
        /// </summary>
        protected virtual MemberSpec FindBaseMember(
        ...
        public override string GetSignatureForDocumentation()
        {
            if (IsExplicitImpl)
                return Parent.GetSignatureForDocumentation()
                    + "."
                    + InterfaceType.GetSignatureForDocumentation(true)
                    + "#"
                    + ShortName;
