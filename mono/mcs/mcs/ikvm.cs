...
namespace Mono.CSharp
{
    ...
    class AssemblyDefinitionStatic : AssemblyDefinition
    {
        ...
        //
        public AssemblyDefinitionStatic(
        ...
        public override void Emit()
        {
            ...
            else if (module.Compiler.Settings.RuntimeMetadataVersion != null)
            {
                Builder.__SetImageRuntimeVersion(
                    module.Compiler.Settings.RuntimeMetadataVersion,
                    0x20000
                );
            }
            else
        ...
        protected override List<AssemblyReferenceMessageInfo> GetNotUnifiedReferences(
    ...
    class StaticLoader : AssemblyReferencesLoader<Assembly>, IDisposable
    {
            ...
            : base(compiler)
        {
            ...
            domain = new Universe(
                UniverseOptions.MetadataOnly
                    | UniverseOptions.ResolveMissingMembers
                    | UniverseOptions.DisableFusion
                    | UniverseOptions.DecodeVersionInfoAttributeBlobs
                    | UniverseOptions.DeterministicOutput
                    | UniverseOptions.DisableDefaultAssembliesLookup
            );
            ...
            if (compiler.Settings.StdLib)
            {
                ...
                string fx_path = corlib_path.Substring(
                    0,
        ...
        Assembly AssemblyReferenceResolver(object sender, IKVM.Reflection.ResolveEventArgs args)
        {
            ...
            foreach (var assembly in domain.GetAssemblies())
            {
                ...
                if (
                    !domain.CompareAssemblyIdentity(
                        refname,
                        false,
                        assembly.FullName,
                        false,
                ...
                )
                {
                    if (
                        ...
                        && (
                            ...
                            || version_mismatch.GetName().Version < assembly.GetName().Version
                        )
                        && !is_fx_assembly
                    )
                    {
            ...
            if (
                Fusion.ParseAssemblyName(args.Name, out referenced_assembly)
                    == ParseAssemblyResult.OK
                && CompiledAssembly.Name == referenced_assembly.Name
            )
                        ...
                        using (RawModule module = domain.OpenRawModule(stream, file))
                        {
                            ...
                            foreach (var entry in loaded_names)
                            {
                                ...
                                if (
                                    ...
                                    == entry.Item3.ManifestModule.ModuleVersionId
                                )
                                    return entry.Item3;

                                if (
                                    ((an.Flags | loaded_name.Flags) & AssemblyNameFlags.PublicKey)
                                    == 0
                                )
                                {
                                ...
                                if (
                                    (an.Flags & AssemblyNameFlags.PublicKey)
                                        == (loaded_name.Flags & AssemblyNameFlags.PublicKey)
                                    && (
                                        domain.CompareAssemblyIdentity(
                                            an.FullName,
                                            false,
                                            loaded_name.FullName,
                                            false,
                                            out result
                                        )
                                    )
