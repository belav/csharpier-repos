...
namespace Mono.CSharp
{
    ...
    class DocumentationBuilder
    {
        ...
        XmlNode GetDocCommentNode(MemberCore mc, string name)
        {
            ...
            catch (Exception ex)
            {
                Report.Warning(
                    ...
                    1,
                    mc.Location,
                    "XML documentation comment on `{0}' is not well-formed XML markup ({1})",
                ...
                return doc.CreateComment(
                    String.Format(
                        "FIXME: Invalid documentation markup was found for member {0}",
                        name
                    )
                );
            }
            ...
            else
            {
                ...
                if (exception != null)
                {
                    Report.Warning(
                        ...
                        1,
                        mc.Location,
                        "Unable to include XML fragment `{0}' of file `{1}'. {2}",
                        path,
                        file,
                        exception.Message
                    );
        ...
        {
            if (mn.Left == null)
                return context.LookupNamespaceOrType(
                    mn.Name,
                    mn.Arity,
                    LookupMode.Probing,
                    Location.Null
            ...
            if (ns != null)
                return ns.LookupTypeOrNamespace(
                    context,
                    mn.Name,
                    mn.Arity,
                    LookupMode.Probing,
        ...
        void HandleXrefCommon(MemberCore mc, XmlElement xref)
        {
            ...
            if (session == null)
                session = new ParserSession { UseJayGlobalArrays = true };

            SeekableStreamReader seekable = new SeekableStreamReader(
                s,
                encoding,
            ...
            if (ParsedName != null || ParsedOperator.HasValue)
            {
                ...
                if (ParsedParameters != null)
                {
                    ...
                    try
                    {
                        var context = new DocumentationMemberContext(
                            mc,
                            ParsedName ?? MemberName.Null
                        );
                ...
                if (type != null)
                {
                    if (member_name == null)
                        member_name = ParsedOperator.HasValue
                            ? Operator.GetMetadataName(ParsedOperator.Value)
                            : ParsedName.Name;

                    int parsed_param_count;
                    if (
                        ParsedOperator == Operator.OpType.Explicit
                        || ParsedOperator == Operator.OpType.Implicit
                    )
                    {
                            ...
                            foreach (var m in members)
                            {
                                ...
                                if (ParsedParameters != null)
                                {
                                    ...
                                    )
                                    {
                                        if (
                                            pm.MemberType
                                            != ParsedParameters[parsed_param_count].TypeSpec
                                        )
                                        {
                    ...
                    if (member == null && parameters_match >= 0)
                    {
                        ...
                        if (parameters_match == parsed_param_count + 1)
                        {
                            Report.Warning(
                                ...
                                1,
                                mc.Location,
                                "Invalid return type in XML comment cref attribute `{0}'",
                                cref
                            );
                        }
                    }
        ...
        //
        void CheckParametersComments(
            MemberCore member,
            IParametersMember paramMember,
            XmlElement el
        )
        {
    ...
    sealed class DocumentationMemberContext : IMemberContext
    {
        ...
        {
            get { return host.CurrentType; }
        }

        public TypeParameters CurrentTypeParameters
        {
            get { return contextName.TypeParameters; }
        }

        public MemberCore CurrentMemberDefinition
        {
            get { return host.CurrentMemberDefinition; }
        }
