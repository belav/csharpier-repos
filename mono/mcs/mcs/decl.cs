...
namespace Mono.CSharp
{
    ...
    public class MemberName
    {
        ...
        public int Arity
        {
            get { return TypeParameters == null ? 0 : TypeParameters.Count; }
        }

        public bool IsGeneric
        {
            get { return TypeParameters != null; }
        }
        ...
        {
            string s =
                TypeParameters == null ? null : "<" + TypeParameters.GetSignatureForError() + ">";
            s = Name + s;
        ...
        public bool Equals(MemberName other)
        {
            ...
            if (
                ...
                && (
                    ...
                    || TypeParameters.Count != other.TypeParameters.Count
                )
            )
                return false;
    ...
    public abstract class MemberCore : Attributable, IMemberContext, IMemberDefinition
    {
        ...
        protected MemberCore(TypeContainer parent, MemberName name, Attributes attrs)
        {
            ...
            member_name = name;
            caching_flags =
                Flags.Obsolete_Undetected
                | Flags.ClsCompliance_Undetected
        ...
        protected bool CheckAbstractAndExtern(bool has_block)
        {
            ...
            {
                if (
                    (ModFlags & (Modifiers.ABSTRACT | Modifiers.EXTERN | Modifiers.PARTIAL)) == 0
                    && !(Parent is Delegate)
                )
                {
        ...
        {
            get { return (caching_flags & Flags.CanBeReused) != 0; }
            set
            {
                caching_flags = value
                    ? (caching_flags | Flags.CanBeReused)
                    : (caching_flags & ~Flags.CanBeReused);
            }
        }
        ...
        public bool IsAccessibleAs(TypeSpec p)
        {
            ...
            for (TypeSpec p_parent; p != null; p = p_parent)
            {
                ...
                for (
                    MemberCore mc = this;
                    !same_access_restrictions && mc != null && mc.Parent != null;
                    mc = mc.Parent
                )
                {
                    ...
                    switch (pAccess)
                    {
                        ...
                        case Modifiers.PRIVATE:
                            ...
                            if (al == Modifiers.PRIVATE)
                            {
                                ...
                                {
                                    same_access_restrictions =
                                        decl.CurrentType.MemberDefinition
                                        == p_parent.MemberDefinition;
                                } while (
                                    !same_access_restrictions
                                    && !decl.PartialContainer.IsTopLevel
        ...
        public virtual FullNamedExpression LookupNamespaceOrType(
            string name,
            int arity,
            LookupMode mode,
            Location loc
        ...
        {
            get { return CLSAttributeValue.HasValue; }
        }
            ...
            if (HasClsCompliantAttribute)
            {
                ...
                if ((caching_flags & Flags.ClsCompliantAttributeFalse) != 0)
                {
                    if (Parent is Interface && Parent.IsClsComplianceRequired())
                    {
                        ...
                        Parent.Kind == MemberKind.Class
                        && (ModFlags & Modifiers.ABSTRACT) != 0
                        && Parent.IsClsComplianceRequired()
                    )
                    {
                        Report.Warning(
                            3011,
                ...
                if (
                    Parent.Kind != MemberKind.Namespace
                    && Parent.Kind != 0
                    && !Parent.IsClsComplianceRequired()
                )
                {
    ...
    public abstract class MemberSpec
    {
        ...
        {
            get { return (state & StateFlags.IsAccessor) != 0; }
            set { state = value ? state | StateFlags.IsAccessor : state & ~StateFlags.IsAccessor; }
        }
        ...
        public bool IsGeneric
        {
            get { return (state & StateFlags.IsGeneric) != 0; }
            set { state = value ? state | StateFlags.IsGeneric : state & ~StateFlags.IsGeneric; }
        }
        ...
        {
            get { return (state & StateFlags.IsNotCSharpCompatible) != 0; }
            set
            {
                state = value
        ...
        {
            if (
                (state & (StateFlags.MissingDependency | StateFlags.MissingDependency_Undetected))
                == 0
            )
                return null;
        ...
        public abstract List<MissingTypeSpecReference> ResolveMissingDependencies(
            MemberSpec caller
        );
        ...
        public bool IsAccessible(IMemberContext ctx)
        {
            ...
            var parentType = /* this as TypeSpec ?? */
            DeclaringType;
            ...
            if ((ma & Modifiers.INTERNAL) != 0)
            {
                ...
                var assembly =
                    ctype == null
                        ? ctx.Module.DeclaringAssembly
                        : ctype.MemberDefinition.DeclaringAssembly;
