...
namespace Mono.CSharp
{
    ...
    public class EmitContext : BuilderContext
    {
        ...
        {
            get { return CurrentAnonymousMethod.Storey as AsyncTaskStorey; }
        }
        ...
        {
            get { return (flags & Options.AccurateDebugInfo) != 0; }
        }

        public bool HasMethodSymbolBuilder
        {
            get { return methodSymbols != null; }
        }

        public bool HasReturnLabel
        {
            get { return return_label.HasValue; }
        }
        ...
        {
            get { return member_context.IsStatic && (flags & Options.ConstructorScope) != 0; }
        }
        ...
        {
            get { return member_context; }
        }

        public ModuleContainer Module
        {
            get { return member_context.Module; }
        }
        ...
        {
            get { return member_context.Module.Compiler.Report; }
        }
        ...
        {
            get { return return_label.Value; }
        }

        public List<IExpressionCleanup> StatementEpilogue
        {
            get { return epilogue_expressions; }
        }
        ...
        public void AssertEmptyStack()
        {
            ...
            if (ig.__StackHeight != 0)
                throw new InternalErrorException(
                    "Await yields with non-empty stack in `{0}",
        ...
        public bool Mark(Location loc)
        {
            ...
            methodSymbols.MarkSequencePoint(
                ig.ILOffset,
                sf.SourceFileEntry,
                loc.Row,
                loc.Column,
        ...
        internal DynamicSiteClass CreateDynamicSite()
        {
            if (dynamic_site_container == null)
            {
                ...
                dynamic_site_container = new DynamicSiteClass(
                    CurrentTypeDefinition.Parent.PartialContainer,
                    mc,
                    member_context.CurrentTypeParameters
                );
        ...
        //
        public StackFieldExpr GetTemporaryField(
            TypeSpec type,
            bool initializedFieldRequired = false
        )
        {
        ...
        public void FreeTemporaryLocal(LocalBuilder b, TypeSpec t)
        {
            if (temporary_storage == null)
            {
                temporary_storage = new Dictionary<TypeSpec, object>(
                    ReferenceEquality<TypeSpec>.Default
                );
    ...
    struct CallEmitter
    {
        ...
        )
        {
            ...
            {
                call_op = IsVirtualCallRequired(InstanceExpression, method)
                    ? OpCodes.Callvirt
                    : OpCodes.Call;
                ...
                if (HasAwaitArguments)
                {
                    ...
                    var ie = new InstanceEmitter(
                        instance_copy,
                        IsAddressCall(instance_copy, call_op, method.DeclaringType)
                    );
                ...
                else if (!InstanceExpressionOnStack)
                {
                    var ie = new InstanceEmitter(
                        InstanceExpression,
                        IsAddressCall(InstanceExpression, call_op, method.DeclaringType)
                    );
            ...
            if (
                ...
                && (
                    InstanceExpression.Type.IsGenericParameter
                    || InstanceExpression.Type.IsStructOrEnum
                )
            )
            {
        ...
        static bool IsVirtualCallRequired(Expression instance, MethodSpec method)
        {
            ...
            if (
                !method.IsVirtual
                && Expression.IsNeverNull(instance)
        ...
        static bool IsAddressCall(Expression instance, OpCode callOpcode, TypeSpec declaringType)
        {
            ...
            return (
                    ...
                    && (
                        ...
                        || (callOpcode == OpCodes.Call && declaringType.IsStruct)
                    )
                )
                || instance_type.IsGenericParameter
                || declaringType.IsNullableType;
        }
    }
    ...
    static class ExpressionAnalyzer
    {
        public static bool RequiresBoxing(Expression instance)
        {
            ...
            if (
                instance_type.IsGenericParameter
                && !(instance is This)
                && TypeSpec.IsReferenceType(instance_type)
            )
                return true;
