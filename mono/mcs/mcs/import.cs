...
namespace Mono.CSharp
{
    public abstract class MetadataImporter
    {
        ...
        protected struct AttributesTypeInfoReader
        {
            ...
            public bool IsDynamicObject()
            {
                ...
                return dynamicFlags != null
                    && DynamicPosition < dynamicFlags.Length
                    && dynamicFlags[DynamicPosition];
            }
                    ...
                    foreach (var ca in cad)
                    {
                        ...
                        switch (dt.Name)
                        {
                            case "DynamicAttribute":
                                ...
                                if (
                                    arg_type.IsArray
                                    && MetaType.GetTypeCode(arg_type.GetElementType())
                                        == TypeCode.Boolean
                                )
                                {
                                    var carg =
                                    ...
                                    {
                                        if (
                                            MetaType.GetTypeCode(carg[i].ArgumentType)
                                            == TypeCode.Boolean
                                        )
                            ...
                            case "TupleElementNamesAttribute":
                                ...
                                if (
                                    tuple_arg_type.IsArray
                                    && MetaType.GetTypeCode(tuple_arg_type.GetElementType())
                                        == TypeCode.String
                                )
                                {
                                    var carg =
        ...
        protected MetadataImporter(ModuleContainer module)
        {
            ...
            import_cache = new Dictionary<MetaType, TypeSpec>(
                1024,
                ReferenceEquality<MetaType>.Default
            );
            compiled_types = new Dictionary<MetaType, TypeSpec>(
                40,
                ReferenceEquality<MetaType>.Default
            );
            assembly_2_definition = new Dictionary<Assembly, IAssemblyDefinition>(
                ReferenceEquality<Assembly>.Default
            );
        ...
        {
            get { return assembly_2_definition.Values; }
        }
        ...
        public FieldSpec CreateField(FieldInfo fi, TypeSpec declaringType)
        {
            ...
            switch (fa & FieldAttributes.FieldAccessMask)
            {
                ...
                default:
                    ...
                    if (
                        IgnoreCompilerGeneratedField
                        && HasAttribute(
                            CustomAttributeData.GetCustomAttributes(fi),
            ...
            try
            {
                field_type = ImportType(
                    fi.FieldType,
                    new AttributesTypeInfoReader(fi),
                    declaringType
                );
            ...
            catch (Exception e)
            {
                ...
                throw new InternalErrorException(
                    e,
                    "Cannot import field `{0}.{1}' referenced in assembly `{2}'",
            ...
            {
                Constant c =
                    field_type.Kind == MemberKind.MissingType
            ...
            else
            {
                ...
                )
                {
                    ...
                    var element_field = CreateField(
                        fi.FieldType.GetField(FixedField.FixedElementName),
                        declaringType
                    );
                    return new FixedFieldSpec(
                        module,
                        declaringType,
                        definition,
                        fi,
        ...
        )
        {
            ...
            var event_type = ImportType(
                ei.EventHandlerType,
                new AttributesTypeInfoReader(ei),
                declaringType
            );
                ...
                if (declaringType != null && parent_owned_count > 0)
                {
                    ...
                    while (read_count != parent_owned_count)
                    {
                        ...
                        if (tparams_count != 0)
                        {
                            ...
                            {
                                import_cache.Add(
                                    tparams[parent_owned_count - read_count + i],
                                    parent_tp[i]
                                );
                            }
                        }
        ...
        TypeParameterSpec[] CreateGenericParameters(int first, MetaType[] tparams)
        {
            ...
            for (int pos = first; pos < tparams.Length; ++pos)
            {
                ...
                tspec[index] = (TypeParameterSpec)CreateType(
                    type,
                    new AttributesTypeInfoReader(),
                    false
                );
            }
        ...
        TypeSpec[] CreateGenericArguments(
            int first,
            MetaType[] tparams,
            AttributesTypeInfoReader dtype
        )
        {
            ...
            for (int pos = first; pos < tparams.Length; ++pos)
            {
                ...
                if (type.HasElementType)
                {
                    ...
                    if (!type.IsArray)
                    {
                        throw new NotImplementedException(
                            "Unknown element type " + type.ToString()
                        );
                    }
                ...
                else
                {
                    ...
                    if (!IsMissingType(type) && type.IsGenericTypeDefinition)
                    {
                        ...
                        var targs = CreateGenericArguments(
                            start_pos,
        ...
        public MethodSpec CreateMethod(MethodBase mb, TypeSpec declaringType)
        {
            ...
            else
            {
                ...
                returnType = ImportType(
                    mi.ReturnType,
                    new AttributesTypeInfoReader(mi.ReturnParameter),
                    declaringType
                );
                ...
                {
                    if (
                        (mod & (Modifiers.STATIC | Modifiers.PUBLIC))
                    ...
                    )
                    {
                        if (
                            ...
                            && name[1] == 'p'
                            && name[0] == 'o'
                            && (mb.Attributes & MethodAttributes.SpecialName) != 0
                        )
                        {
                ...
                if ((mod & Modifiers.OVERRIDE) != 0)
                {
                    ...
                    if (kind == MemberKind.Method && declaringType.BaseType != null)
                    {
                        ...
                        {
                            var filter = MemberFilter.Method(
                                name,
                                tparams != null ? tparams.Length : 0,
                                parameters,
                                null
                            );
                            var candidate = MemberCache.FindMember(
                                btype,
                                filter,
                            ...
                            //
                            const Modifiers conflict_mask =
                                Modifiers.AccessibilityMask & ~Modifiers.INTERNAL;
                            if (
                                candidate != null
                                && (candidate.Modifiers & conflict_mask) == (mod & conflict_mask)
                                && !candidate.IsStatic
                            )
                            {
            ...
            if (tparams != null)
            {
                var gmd = new ImportedGenericMethodDefinition(
                    (MethodInfo)mb,
                    returnType,
                    parameters,
                    tparams,
        ...
        //
        AParametersCollection CreateParameters(
            TypeSpec parent,
            ParameterInfo[] pi,
            MethodBase method
        )
        {
            int varargs =
                method != null && (method.CallingConvention & CallingConventions.VarArgs) != 0
                    ? 1
                    : 0;
            ...
            for (int i = 0; i < pi.Length; i++)
            {
                ...
                {
                    if (
                        (p.Attributes & (ParameterAttributes.Out | ParameterAttributes.In))
                    ...
                    i == 0
                    && method.IsStatic
                    && (parent.Modifiers & Modifiers.METHOD_EXTENSION) != 0
                    && HasAttribute(
                ...
                else
                {
                    ...
                    if (!is_params && p.IsOptional)
                    {
                        ...
                        if (
                            ...
                            && ptype.Kind != MemberKind.TypeParameter
                            && (value != null || TypeSpec.IsReferenceType(ptype))
                        )
                        {
                            if (value == null)
                            {
                                default_value = Constant.CreateConstantFromValue(
                                    ptype,
                                    null,
                                    Location.Null
                                );
                            ...
                            else
                            {
                                ...
                                if (ptype.IsEnum)
                                {
                                    default_value = new EnumConstant(
                            ...
                            for (int ii = 0; ii < attrs.Count; ++ii)
                            {
                                ...
                                if (
                                    dt.Name == "CallerLineNumberAttribute"
                                    && (
                                        ptype.BuiltinType == BuiltinTypeSpec.Type.Int
                                        || Convert.ImplicitNumericConversionExists(
                                            module.Compiler.BuiltinTypes.Int,
                                            ptype
                                        )
                                    )
                                )
                                    ...
                                    dt.Name == "CallerFilePathAttribute"
                                    && Convert.ImplicitReferenceConversionExists(
                                        module.Compiler.BuiltinTypes.String,
                                        ptype
                                    )
                                )
                                    mod |= Parameter.Modifier.CallerFilePath;
                                else if (
                                    dt.Name == "CallerMemberNameAttribute"
                                    && Convert.ImplicitReferenceConversionExists(
                                        module.Compiler.BuiltinTypes.String,
                                        ptype
                                    )
                        ...
                        {
                            default_value = new DefaultValueExpression(
                                new TypeExpression(ptype, Location.Null),
                                Location.Null
                            );
                        }
        ...
        //
        public PropertySpec CreateProperty(
            PropertyInfo pi,
            TypeSpec declaringType,
            MethodSpec get,
            MethodSpec set
        )
        {
            ...
            if (set != null)
            {
                ...
                if (mod == 0)
                {
                    ...
                    {
                        set_based_param = IndexerSpec.CreateParametersFromSetter(
                            set,
                            set_param_count
                        );
                    }
                ...
                else
                {
                    ...
                    if (
                        ...
                        != (set.Modifiers & Modifiers.AccessibilityMask)
                    )
                    {
                        ...
                        if (get_acc != Modifiers.PUBLIC)
                        {
                            ...
                            if (get_acc != set_acc)
                            {
                                var get_restr = ModifiersExtensions.IsRestrictedModifier(
                                    get_acc,
                                    set_acc
                                );
                                var set_restr = ModifiersExtensions.IsRestrictedModifier(
                                    set_acc,
                                    get_acc
                                );
            ...
            if (!param.IsEmpty)
            {
                if (is_valid_property)
                {
                    ...
                    if (is_valid_property)
                    {
                        spec = new IndexerSpec(
                            declaringType,
                            new ImportedParameterMemberDefinition(pi, type, param, this),
                            type,
                            param,
                            pi,
                            mod
                        ...
                        declaringType.MemberDefinition.IsComImport
                        && param.FixedParameters[0].HasDefaultValue
                    )
                    {
                        //
            ...
            if (spec == null)
                spec = new PropertySpec(
                    MemberKind.Property,
                    declaringType,
                    new ImportedMemberDefinition(pi, type, this),
                    type,
                    pi,
                    mod
                );
        ...
        TypeSpec CreateType(MetaType type, AttributesTypeInfoReader dtype, bool canImportBaseType)
        {
            ...
            if (type.IsNested && !type.IsGenericParameter)
                declaring_type = CreateType(
                    type.DeclaringType,
                    new AttributesTypeInfoReader(type.DeclaringType),
                    true
                );
            else
                declaring_type = null;
        ...
        )
        {
            ...
            {
                spec = new TypeSpec(
                    MemberKind.MissingType,
                    declaringType,
                    new ImportedTypeDefinition(type, this),
                    type,
                    Modifiers.PUBLIC
                );
            ...
            if (type.IsGenericType && !type.IsGenericTypeDefinition)
            {
                ...
                else
                {
                    ...
                    {
                        spec = declaringType.MakeGenericType(
                            module,
                            targs.Skip(targs_pos).Take(declaringType.Arity).ToArray()
                        );
                    ...
                    for (int i = nested_hierarchy.Count; i != 0; --i)
                    {
                        ...
                        if (t.Arity > 0)
                        {
                            spec = spec.MakeGenericType(
                                module,
                                targs.Skip(targs_pos).Take(spec.Arity).ToArray()
                            );
                            targs_pos += t.Arity;
                        }
                    }
                    ...
                    if (spec.Kind == MemberKind.MissingType)
                    {
                        spec = new TypeSpec(
                            MemberKind.MissingType,
                            spec,
                            new ImportedTypeDefinition(type_def, this),
                            type_def,
                            Modifiers.PUBLIC
                        );
                        spec.MemberCache = MemberCache.Empty;
                    ...
                    {
                        if (
                            (type_def.Attributes & TypeAttributes.VisibilityMask)
                        ...
                        spec = MemberCache.FindNestedType(
                            spec,
                            name,
                            targs.Length - targs_pos,
                            false
                        );
            ...
            {
                const BindingFlags underlying_member =
                ...
                foreach (var type_member in type_members)
                {
                    spec = new EnumSpec(
                        declaringType,
                        definition,
                        CreateType(type_member.FieldType),
                        type,
                        mod
                    );
                    break;
        ...
        TypeParameterSpec CreateTypeParameter(MetaType type, TypeSpec declaringType)
        {
            ...
            var import_special =
                type.GenericParameterAttributes & GenericParameterAttributes.SpecialConstraintMask;
            ...
            if ((import_special & GenericParameterAttributes.NotNullableValueTypeConstraint) != 0)
            {
                ...
                (import_special & GenericParameterAttributes.DefaultConstructorConstraint) != 0
            )
            {
            ...
            foreach (var t in types)
            {
                ...
                if (prev_namespace != t.Namespace)
                {
                    ns =
                        t.Namespace == null
                            ? targetNamespace
                            : targetNamespace.GetNamespace(t.Namespace, true);
        ...
        TypeSpec ImportType(MetaType type, AttributesTypeInfoReader dtype, TypeSpec currentType)
        {
            ...
            {
                if (
                    compiled_type.BuiltinType == BuiltinTypeSpec.Type.Object
                    && dtype.IsDynamicObject()
                )
                    return module.Compiler.BuiltinTypes.Dynamic;
            }
            else
        ...
        Constant ReadDecimalConstant(IList<CustomAttributeData> attrs)
        {
            ...
            foreach (var ca in attrs)
            {
                var dt = ca.Constructor.DeclaringType;
                if (
                    dt.Name != "DecimalConstantAttribute"
                    || dt.Namespace != CompilerServicesNamespace
                )
                    continue;
                ...
                foreach (var a in attrs)
                {
                    ...
                    )
                    {
                        ...
                        if (name == "AttributeUsageAttribute")
                        {
                            ...
                            bag.AttributeUsage = new AttributeUsageAttribute(
                                (AttributeTargets)a.ConstructorArguments[0].Value
                            );
                        ...
                        if (name == "CoClassAttribute")
                        {
                            ...
                            bag.CoClass = importer.ImportType(
                                (MetaType)a.ConstructorArguments[0].Value
                            );
                            continue;
                        }
                        ...
                        if (name == "AsyncMethodBuilderAttribute")
                        {
                            ...
                            bag.AsyncMethodBuilder = importer.ImportType(
                                (MetaType)a.ConstructorArguments[0].Value
                            );
                            continue;
                        }
    ...
    public class ImportedModuleDefinition
    {
        ...
        public List<Attribute> ReadAssemblyAttributes()
        {
            ...
            var field = t.GetField(
                AssemblyAttributesPlaceholder.AssemblyFieldName,
    ...
    public class ImportedAssemblyDefinition : IAssemblyDefinition
    {
        ...
        {
            get { return aname.FullName; }
        }

        public bool HasStrongName
        {
            get { return aname.GetPublicKey().Length != 0; }
        }
    ...
    class ImportedParameterMemberDefinition : ImportedMemberDefinition, IParametersMember
    {
        ...
        protected ImportedParameterMemberDefinition(
            MethodBase provider,
            TypeSpec type,
        ...
        public ImportedParameterMemberDefinition(
            PropertyInfo provider,
            TypeSpec type,
            AParametersCollection parameters,
            MetadataImporter importer
        )
    ...
    {
        public ImportedMethodDefinition(
            MethodBase provider,
            TypeSpec type,
            AParametersCollection parameters,
            MetadataImporter importer
        ...
        {
            get { return (MethodBase)provider; }
        }
    }
    ...
    class ImportedGenericMethodDefinition : ImportedMethodDefinition, IGenericMethodDefinition
    {
        ...
        public ImportedGenericMethodDefinition(
            MethodInfo provider,
            TypeSpec type,
            AParametersCollection parameters,
            TypeParameterSpec[] tparams,
    ...
    class ImportedTypeDefinition : ImportedDefinition, ITypeDefinition
    {
        ...
        {
            get { return importer.GetAssemblyDefinition(provider.Module.Assembly); }
        }

        bool ITypeDefinition.IsComImport
        {
            get { return ((MetaType)provider).IsImport; }
        }
        ...
        {
            get { return ((MetaType)provider).Namespace; }
        }

        public int TypeParametersCount
        {
            get { return tparams == null ? 0 : tparams.Length; }
        }
        ...
        public static void Error_MissingDependency(
            IMemberContext ctx,
            ...
            for (int i = 0; i < missing.Count; ++i)
            {
                ...
                else if (definition.DeclaringAssembly.IsMissing)
                {
                    if (definition.IsTypeForwarder)
                    {
                        report.Error(
                            ...
                            name,
                            definition.DeclaringAssembly.FullName
                        );
                    }
                    else
                    {
                ...
                else if (definition.IsTypeForwarder)
                {
                    report.Error(
                        731,
                        loc,
                        "The type forwarder for type `{0}' in assembly `{1}' has circular dependency",
                        name,
                        definition.DeclaringAssembly.FullName
                    );
                }
                else
                {
                    report.Error(
                        ...
                        name,
                        t.MemberDefinition.DeclaringAssembly.FullName
                    );
                }
            }
        }
        ...
        public void LoadMembers(TypeSpec declaringType, bool onlyTypes, ref MemberCache cache)
        {
            ...
            const MethodAttributes explicit_impl =
            ...
            catch (Exception e)
            {
                throw new InternalErrorException(
                    e,
                    "Could not import type `{0}' from `{1}'",
                    declaringType.GetSignatureForError(),
            ...
            if (cache == null)
            {
                ...
                foreach (var member in all)
                {
                    ...
                    catch (Exception e)
                    {
                        throw new InternalErrorException(
                            e,
                            "Could not import nested type `{0}' from `{1}'",
                            t.FullName,
                            declaringType.MemberDefinition.DeclaringAssembly.FullName
                        );
                    }
                    ...
                    switch (member.MemberType)
                    {
                        ...
                        case MemberTypes.Method:
                            ...
                            )
                            {
                                ...
                                if (
                                    MetadataImporter.HasAttribute(
                                        CustomAttributeData.GetCustomAttributes(mb),
                                        "CompilerGeneratedAttribute",
                                        MetadataImporter.CompilerServicesNamespace
                                    )
                                )
                                    continue;
                            }
                            ...
                            if (imported.Kind == MemberKind.Method && !imported.IsGeneric)
                            {
                                if (possible_accessors == null)
                                    possible_accessors = new Dictionary<MethodBase, MethodSpec>(
                                        ReferenceEquality<MethodBase>.Default
                                    );
                        ...
                        case MemberTypes.Property:
                            ...
                            catch (Exception ex)
                            {
                                throw new InternalErrorException(
                                    ex,
                                    "Could not import property `{0}' inside `{1}'",
                                    p.Name,
                                    declaringType.GetSignatureForError()
                                );
                            }
