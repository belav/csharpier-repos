...
namespace Mono.CSharp
{
    ...
    public class SourceFile : IEquatable<SourceFile>
    {
        ...
        static readonly byte[] MD5Algorith =
        {
            96,
            166,
            110,
        ...
        public SourceFile(
            ...
            string path,
            int index,
            Func<SourceFile, SeekableStreamReader> inputStreamDelegate
        )
        ...
        public SourceFileEntry CreateSymbolInfo(
        ...
        )
        {
            ...
            file = new SourceFileEntry(
                symwriter,
                GetFullPathName(pathMap),
    ...
    public struct Location : IEquatable<Location>
    {
        ...
        public Location(SourceFile file, int row, int column)
        {
            ...
            else
            {
                ...
                int max = checkpoint_index < 10 ? checkpoint_index : 10;
                for (int i = 0; i < max; i++)
                {
                ...
                long l =
                    column + (delta << column_bits) + (target << (line_delta_bits + column_bits));
#if FULL_AST
                token = l;
#else
        ...
        public int File
        {
            get
            {
                ...
                if (checkpoints.Length <= CheckpointIndex)
                    throw new Exception(
                        String.Format(
                            "Should not happen. Token is {0:X04}, checkpoints are {1}, index is {2}",
                            token,
                            checkpoints.Length,
                            CheckpointIndex
                        )
    ...
    public class LocationsBag
    {
        ...
        Dictionary<object, List<Location>> simple_locs = new Dictionary<object, List<Location>>(
            ReferenceEquality<object>.Default
        );
        Dictionary<MemberCore, MemberLocations> member_locs = new Dictionary<
            MemberCore,
            MemberLocations
        >(ReferenceEquality<MemberCore>.Default);
        ...
        public void AddMember(
            MemberCore member,
            IList<Tuple<Modifiers, Location>> modLocations,
            Location location
        )
        {
