...
namespace Mono.CSharp
{
    public static class ConstantFold
    {
        ...
        //
        static bool DoBinaryNumericPromotions(
            ResolveContext rc,
            ref Constant left,
            ref Constant right
        )
        ...
        /// </summary>
        public static Constant BinaryFold(
            ResolveContext ec,
        ...
        )
        {
                ...
                switch (oper)
                {
                    ...
                    case Binary.Operator.ExclusiveOr:
                        result = BinaryFold(
                            ec,
                            oper,
                            ((EnumConstant)left).Child,
                            ((EnumConstant)right).Child,
                            loc
                        );
                        if (result != null)
            ...
            switch (oper)
            {
                case Binary.Operator.BitwiseOr:
                    ...
                    //
                    if (
                        (lt.BuiltinType == BuiltinTypeSpec.Type.Bool && right is NullLiteral)
                    ...
                    )
                    {
                        ...
                        // null | false => null
                        if (
                            (right is NullLiteral && left.IsDefaultValue)
                            || (left is NullLiteral && right.IsDefaultValue)
                        )
                        ...
                        return ReducedExpression.Create(
                            new BoolConstant(ec.BuiltinTypes, true, loc),
                            b
                        );
                    }
                    ...
                    {
                        ulong res = ((ULongConstant)left).Value | ((ULongConstant)right).Value;
                ...
                case Binary.Operator.BitwiseAnd:
                    ...
                    )
                    {
                        ...
                        if (
                            ...
                            return ReducedExpression.Create(
                                new BoolConstant(ec.BuiltinTypes, false, loc),
                                b
                            );
                ...
                case Binary.Operator.Addition:
                    ...
                    //
                    if (
                        lt.BuiltinType == BuiltinTypeSpec.Type.String
                        || rt.BuiltinType == BuiltinTypeSpec.Type.String
                    )
                    {
                        if (lt == rt)
                    ...
                    try
                    {
                        if (left is DoubleConstant)
                        {
                            ...
                            if (ec.ConstantCheckState)
                                res = checked(
                                    ((DoubleConstant)left).Value + ((DoubleConstant)right).Value
                                );
                            else
                                res = unchecked(
                                    ((DoubleConstant)left).Value + ((DoubleConstant)right).Value
                        ...
                        else if (left is ULongConstant)
                        {
                            ...
                            if (ec.ConstantCheckState)
                                res = checked(
                                    ((ULongConstant)left).Value + ((ULongConstant)right).Value
                                );
                            else
                                res = unchecked(
                                    ((ULongConstant)left).Value + ((ULongConstant)right).Value
                        ...
                        else if (left is LongConstant)
                        {
                            ...
                            if (ec.ConstantCheckState)
                                res = checked(
                                    ((LongConstant)left).Value + ((LongConstant)right).Value
                                );
                            else
                                res = unchecked(
                                    ((LongConstant)left).Value + ((LongConstant)right).Value
                        ...
                        else if (left is UIntConstant)
                        {
                            ...
                            if (ec.ConstantCheckState)
                                res = checked(
                                    ((UIntConstant)left).Value + ((UIntConstant)right).Value
                                );
                            else
                                res = unchecked(
                                    ((UIntConstant)left).Value + ((UIntConstant)right).Value
                        ...
                        else if (left is DecimalConstant)
                        {
                            ...
                            if (ec.ConstantCheckState)
                                res = checked(
                                    ((DecimalConstant)left).Value + ((DecimalConstant)right).Value
                                );
                            else
                                res = unchecked(
                                    ((DecimalConstant)left).Value + ((DecimalConstant)right).Value
                ...
                case Binary.Operator.LeftShift:
                    ...
                    switch (left.Type.BuiltinType)
                    {
                        case BuiltinTypeSpec.Type.ULong:
                            return new ULongConstant(
                                ec.BuiltinTypes,
                                ((ULongConstant)left).Value << lshift_val,
                                left.Location
                            );
                        case BuiltinTypeSpec.Type.Long:
                            return new LongConstant(
                                ec.BuiltinTypes,
                                ((LongConstant)left).Value << lshift_val,
                                left.Location
                            );
                        case BuiltinTypeSpec.Type.UInt:
                            return new UIntConstant(
                                ec.BuiltinTypes,
                                ((UIntConstant)left).Value << lshift_val,
                                left.Location
                            );
                    }
                    ...
                    if (left.Type.BuiltinType == BuiltinTypeSpec.Type.Int)
                        return new IntConstant(
                            ec.BuiltinTypes,
                            ((IntConstant)left).Value << lshift_val,
                            left.Location
                        );
                ...
                case Binary.Operator.RightShift:
                    ...
                    switch (left.Type.BuiltinType)
                    {
                        case BuiltinTypeSpec.Type.ULong:
                            return new ULongConstant(
                                ec.BuiltinTypes,
                                ((ULongConstant)left).Value >> rshift_val,
                                left.Location
                            );
                        case BuiltinTypeSpec.Type.Long:
                            return new LongConstant(
                                ec.BuiltinTypes,
                                ((LongConstant)left).Value >> rshift_val,
                                left.Location
                            );
                        case BuiltinTypeSpec.Type.UInt:
                            return new UIntConstant(
                                ec.BuiltinTypes,
                                ((UIntConstant)left).Value >> rshift_val,
                                left.Location
                            );
                    }
                    ...
                    if (left.Type.BuiltinType == BuiltinTypeSpec.Type.Int)
                        return new IntConstant(
                            ec.BuiltinTypes,
                            ((IntConstant)left).Value >> rshift_val,
                            left.Location
                        );
                ...
                case Binary.Operator.Equality:
                    if (
                        TypeSpec.IsReferenceType(lt) && TypeSpec.IsReferenceType(rt)
                    ...
                    )
                    {
                        ...
                        if (left is StringConstant && right is StringConstant)
                            return new BoolConstant(
                                ec.BuiltinTypes,
                                ((StringConstant)left).Value == ((StringConstant)right).Value,
                                left.Location
                            );
                    ...
                    bool_res = false;
                    if (left is DoubleConstant)
                        bool_res = ((DoubleConstant)left).Value == ((DoubleConstant)right).Value;
                    else if (left is FloatConstant)
                        bool_res =
                            ((FloatConstant)left).DoubleValue == ((FloatConstant)right).DoubleValue;
                    else if (left is ULongConstant)
                        bool_res = ((ULongConstant)left).Value == ((ULongConstant)right).Value;
                    else if (left is LongConstant)
                        bool_res = ((LongConstant)left).Value == ((LongConstant)right).Value;
                    else if (left is UIntConstant)
                        bool_res = ((UIntConstant)left).Value == ((UIntConstant)right).Value;
                    else if (left is IntConstant)
                        bool_res = ((IntConstant)left).Value == ((IntConstant)right).Value;
                    else
                        return null;
                ...
                case Binary.Operator.LessThan:
                    if (right is NullLiteral)
                    {
                        if (left is NullLiteral)
                        {
                            ...
                            lifted_int.ResolveAsType(ec);
                            return (Constant)
                                new Binary(oper, lifted_int, right).ResolveOperator(ec);
                        }
                    }
