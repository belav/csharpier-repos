...
namespace Mono.CSharp
{
    ...
    public class AwaitStatement : YieldStatement<AsyncInitializer>
    {
        ...
        bool IsDynamic
        {
            get { return awaiter_definition == null; }
        }
        ...
        public Expression GetResultExpression(EmitContext ec)
        {
            ...
            if (IsDynamic)
            {
                ...
                return new Invocation(
                    new MemberAccess(fe_awaiter, "GetResult"),
                    new Arguments(0)
                ).Resolve(rc);
            }

            var mg_result = MethodGroupExpr.CreatePredefined(
                awaiter_definition.GetResult,
                fe_awaiter.Type,
                loc
            );
        ...
        public void EmitPrologue(EmitContext ec)
        {
            ...
            using (ec.With(BuilderContext.Options.OmitDebugInfo, true))
            {
                ...
                if (IsDynamic)
                {
                    ...
                    completed_expr = new DynamicConversion(
                        ec.Module.Compiler.BuiltinTypes.Bool,
                        0,
                        dargs,
                        loc
                    ).Resolve(rc);
    ...
    public class AsyncInitializer : StateMachineInitializer
    {
        ...
        {
            get { return "async state machine block"; }
        }
        ...
        {
            var catch_value = LocalVariable.CreateCompilerGenerated(
                ec.Module.Compiler.BuiltinTypes.Exception,
                block,
                Location
            );
            ...
            ((AsyncTaskStorey)Storey).EmitSetException(
                ec,
                new LocalVariableReference(catch_value, Location)
            );
    ...
    class AsyncTaskStorey : StateMachine
    {
        ...
        public AsyncTaskStorey(
            ...
            : base(
                block,
                initializer.Host,
                context.CurrentMemberDefinition as MemberBase,
                context.CurrentTypeParameters,
                "async",
                MemberKind.Struct
            )
        {
        ...
        public Field AddAwaiter(TypeSpec type)
        {
            ...
            var field = AddCompilerGeneratedField(
                "$awaiter" + awaiters++.ToString("X"),
                new TypeExpression(type, Location),
                true
            );
            field.Define();
        ...
        public Field AddCapturedLocalVariable(TypeSpec type, bool requiresUninitialized = false)
        {
            ...
            var field = AddCompilerGeneratedField(
                "$stack" + locals_captured++.ToString("X"),
                new TypeExpression(type, Location),
                true
            );
            field.Define();
        ...
        protected override bool DoDefineMembers()
        {
            ...
            if (return_type.IsCustomTaskType())
            {
                ...
                var set_result_sign = MemberFilter.Method(
                    "SetResult",
                    0,
                    ParametersCompiled.CreateFullyResolved(bt.MemberDefinition.TypeParameters),
                    btypes.Void
                );
                set_result = new PredefinedMember<MethodSpec>(Module, bt, set_result_sign).Resolve(
                    Location
                );

                var set_exception_sign = MemberFilter.Method(
                    "SetException",
                    0,
                    ParametersCompiled.CreateFullyResolved(btypes.Exception),
                    btypes.Void
                );
                set_exception = new PredefinedMember<MethodSpec>(
                    Module,
                    bt,
                    set_exception_sign
                ).Resolve(Location);

                var builder_factory_sign = MemberFilter.Method(
                    "Create",
                    0,
                    ParametersCompiled.EmptyReadOnlyParameters,
                    bt_inflated
                );
                builder_factory = new PredefinedMember<MethodSpec>(
                    Module,
                    bt,
                    builder_factory_sign
                ).Resolve(Location);
                if (builder_factory?.IsStatic == false)
                    throw new NotImplementedException("report better error message");

                var builder_start_sign = MemberFilter.Method(
                    "Start",
                    1,
                ...
                builder_start = new PredefinedMember<MethodSpec>(
                    Module,
                    bt,
                    builder_start_sign
                ).Resolve(Location);
                ...
                var set_statemachine_sign = MemberFilter.Method(
                    "SetStateMachine",
                    0,
                    ParametersCompiled.CreateFullyResolved(istate_machine.TypeSpec),
                    btypes.Void
                );
                set_statemachine = new PredefinedMember<MethodSpec>(
                    Module,
                    bt,
                    set_statemachine_sign
                ).Resolve(Location);
                ;

                var task_sign = MemberFilter.Property(
                    "Task",
                    return_type.MemberDefinition as TypeSpec
                );
            ...
            if (has_task_return_type && HasAwaitInsideFinally)
            {
                ...
                //
                rfield = AddCompilerGeneratedField(
                    "$return",
                    new TypeExpression(bt.TypeArguments[0], Location)
                );
            }
            else
            {
            ...
            var set_state_machine = new Method(
                this,
                new TypeExpression(Compiler.BuiltinTypes.Void, Location),
                ...
                ParametersCompiled.CreateFullyResolved(
                    new Parameter(
                        new TypeExpression(istate_machine.TypeSpec, Location),
                        "stateMachine",
            ...
            ToplevelBlock block = new ToplevelBlock(
                Compiler,
                set_state_machine.ParameterInfo,
                Location
            );
            ...
            if (has_task_return_type)
            {
                ...
                {
                    HoistedReturnValue = TemporaryVariableReference.Create(
                        bt.TypeArguments[0],
                        StateMachineMethod.Block,
                        Location
                    );
                }
            }
        ...
        void EmitOnCompleted(EmitContext ec, Expression awaiter, bool unsafeVersion)
        {
            ...
            if (return_type.Kind == MemberKind.Void)
            {
                predefined = unsafeVersion
                    ? pm.AsyncVoidMethodBuilderOnCompletedUnsafe
                    : pm.AsyncVoidMethodBuilderOnCompleted;
            }
            else if (return_type == Module.PredefinedTypes.Task.TypeSpec)
            {
                predefined = unsafeVersion
                    ? pm.AsyncTaskMethodBuilderOnCompletedUnsafe
                    : pm.AsyncTaskMethodBuilderOnCompleted;
            }
            else if (return_type.IsGenericTask)
            {
                predefined = unsafeVersion
                    ? pm.AsyncTaskMethodBuilderGenericOnCompletedUnsafe
                    : pm.AsyncTaskMethodBuilderGenericOnCompleted;
                has_task_return_type = true;
            }
            else
            {
                ...
                predefined = new PredefinedMember<MethodSpec>(
                    Module,
                    return_type.MemberDefinition.GetAsyncMethodBuilder(),
                    on_completed_sign
                );
                has_task_return_type = return_type.IsGeneric;
            }
            ...
            if (has_task_return_type)
                on_completed = MemberCache.GetMember<MethodSpec>(
                    set_result.DeclaringType,
                    on_completed
                );
            ...
            args.Add(
                new Argument(new CompilerGeneratedThis(CurrentType, Location), Argument.AType.Ref)
            );
            using (ec.With(BuilderContext.Options.OmitDebugInfo, true))
            {
        ...
        public void EmitInitializer(EmitContext ec)
        {
            ...
            instance.AddressOf(ec, AddressOp.Store);
            ec.Emit(
                struct_builder ? OpCodes.Call : OpCodes.Callvirt,
                builder_start.MakeGenericMethod(Module, instance.Type)
            );
