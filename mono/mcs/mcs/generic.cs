...
namespace Mono.CSharp
{
    public class VarianceDecl
    {
        ...
        public static Variance CheckTypeVariance(
            TypeSpec t,
            Variance expected,
        ...
        )
        {
            ...
            if (tp != null)
            {
                var v = tp.Variance;
                if (
                    expected == Variance.None && v != expected
    ...
    public class Constraints
    {
        ...
        static bool CheckConflictingInheritedConstraint(
            TypeParameterSpec spec,
            TypeSpec ba,
            TypeSpec bb,
            IMemberContext context,
        ...
        public static void Error_ConflictingConstraints(
            IMemberContext context,
            ...
            for (int i = 0; i < constraints.Count; ++i)
            {
                ...
                if (constraint_tp != null)
                {
                    ...
                    if (constraint_tp.HasTypeConstraint)
                    {
                        ...
                        {
                            if (
                                !CheckConflictingInheritedConstraint(
                                    spec,
                                    constraint_tp.BaseType,
                                    context,
                                    constraint.Location
                                )
                            )
    ...
    public class TypeParameter : MemberCore, ITypeDefinition
    {
        ...
        {
            this.constraints = constraints;
            this.spec = new TypeParameterSpec(
                null,
                index,
                this,
            ...
            : base(null, name, attrs)
        {
            this.spec = new TypeParameterSpec(
                parentSpec,
                spec.DeclaredPosition,
                this,
                spec.SpecialConstraint,
                spec.Variance,
                null
            )
            {
        ...
        {
            get { return Module.DeclaringAssembly; }
        }
        ...
        {
            get { return attribute_target; }
        }

        public Variance Variance
        {
            get { return spec.Variance; }
        }
        ...
        public void EmitConstraints(GenericTypeParameterBuilder builder)
        {
            ...
            if (spec.InterfacesDefined != null)
                builder.SetInterfaceConstraints(
                    spec.InterfacesDefined.Select(l => l.GetMetaInfo()).ToArray()
                );
        ...
        public void ErrorInvalidVariance(IMemberContext mc, Variance expected)
        {
            ...
            string input_variance =
                Variance == Variance.Contravariant ? "contravariant" : "covariant";
            string gtype_variance;
            switch (expected)
            {
                case Variance.Contravariant:
                    gtype_variance = "contravariantly";
                    break;
                case Variance.Covariant:
                    gtype_variance = "covariantly";
                    break;
                default:
                    gtype_variance = "invariantly";
                    break;
            }
    ...
    public class TypeParameterSpec : TypeSpec
    {
        ...
        {
            get { return (spec & SpecialConstraint.Constructor) != 0; }
        }

        public bool HasSpecialClass
        {
            get { return (spec & SpecialConstraint.Class) != 0; }
        }

        public bool HasSpecialStruct
        {
            get { return (spec & SpecialConstraint.Struct) != 0; }
        }
        ...
        public bool HasAnyTypeConstraint
        {
            get
            {
                return (spec & (SpecialConstraint.Class | SpecialConstraint.Struct)) != 0
                    || ifaces != null
                    || targs != null
                    || HasTypeConstraint;
            }
        ...
        public bool IsConstrained
        {
            get
            {
                return spec != SpecialConstraint.None
                    || ifaces != null
                    || targs != null
                    || HasTypeConstraint;
            }
            ...
            get
            {
                ...
                if (targs != null)
                {
                    foreach (var ta in targs)
                    {
                        ...
                        if (
                            tp != null
                            && (tp.spec & (SpecialConstraint.Class | SpecialConstraint.Struct)) != 0
                        )
                            continue;
        ...
        public static TypeParameterSpec[] InflateConstraints(
        ...
        public static TypeParameterSpec[] InflateConstraints<T>(
            TypeParameterSpec[] tparams,
            Func<T, TypeParameterInflator> inflatorFactory,
            T arg
        )
        {
            ...
            for (int i = 0; i < tparams.Length; ++i)
            {
                ...
                )
                {
                    ...
                    constraints[i] = (TypeParameterSpec)
                        constraints[i].InflateMember(inflator.Value);
                }
            }
        ...
        protected override void InitializeMemberCache(bool onlyTypes)
        {
            ...
            if (targs != null)
            {
                foreach (var ta in targs)
                {
                    ...
                    if (
                        b_type != null
                        && !b_type.IsStructOrEnum
                        && TypeSpec.IsBaseClass(b_type, BaseType, false)
                    )
                        BaseType = b_type;
                }
            }
            ...
            if (
                BaseType.BuiltinType != BuiltinTypeSpec.Type.Object
                && BaseType.BuiltinType != BuiltinTypeSpec.Type.ValueType
            )
            {
    ...
    public class InflatedTypeSpec : TypeSpec
    {
        ...
        public TypeParameterSpec[] Constraints
        {
            get
            {
                if (constraints == null)
                {
                    constraints = TypeParameterSpec.InflateConstraints(
                        MemberDefinition.TypeParameters,
                        l => l.CreateLocalInflator(context),
                        this
                    );
                }
        ...
        {
            get { return (state & StateFlags.ConstraintsChecked) != 0; }
            set
            {
                state = value
                    ? state | StateFlags.ConstraintsChecked
        ...
        {
            get { return (open_type.state & StateFlags.InflatedExpressionType) != 0; }
        }

        public override bool IsArrayGenericInterface
        {
            get { return (open_type.state & StateFlags.GenericIterateInterface) != 0; }
        }

        public override bool IsGenericTask
        {
            get { return (open_type.state & StateFlags.GenericTask) != 0; }
        }

        public override bool IsNullableType
        {
            get { return (open_type.state & StateFlags.InflatedNullableType) != 0; }
        }

        public override bool IsTupleType
        {
            get { return (open_type.state & StateFlags.Tuple) != 0; }
        }
    ...
    public class TypeParameters
    {
        ...
        public void Create(TypeSpec declaringType, int parentOffset, TypeContainer parent)
        {
            ...
            for (int i = 0; i < types.Length; ++i)
            {
                ...
                if (
                    tp.Variance != Variance.None
                    && !(
                        declaringType != null
                        && (
                            declaringType.Kind == MemberKind.Interface
                            || declaringType.Kind == MemberKind.Delegate
                        )
                    )
                )
                {
    ...
    {
        public GenericOpenTypeExpr(
            TypeSpec type, /*UnboundTypeArguments args,*/
            Location loc
        )
    ...
    struct ConstraintChecker
    {
        ...
        )
        {
            ...
            {
                if (
                    Convert.ImplicitReferenceConversionExists(atype, ttype)
                    || Convert.ImplicitBoxingConversion(null, atype, ttype) != null
                )
                    return true;
            }
        ...
        static bool HasDefaultConstructor(TypeSpec atype)
        {
            ...
            var found = MemberCache.FindMember(
                tdef,
    ...
    class TypeInference
    {
        ...
        )
        {
            ...
            for (int i = 0; i < arg_count; i++)
            {
                ...
                {
                    if (
                        arg_count == params_arguments_start + 1
                        && TypeManager.HasElementType(a.Type)
                    )
                        method_parameter = methodParameters.Types[params_arguments_start];
                    else
                        method_parameter = TypeManager.GetElementType(
        ...
        )
        {
            ...
            for (int i = 0; i < arg_count; i++)
            {
                ...
                TypeSpec t_i = methodParameters[
                    i >= methodParameters.Length ? methodParameters.Length - 1 : i
                ];
    ...
    public class TypeInferenceContext
    {
        ...
        void AddToBounds(BoundInfo bound, int index, bool voidAllowed)
        {
            ...
            //
            if (
                (bound.Type.Kind == MemberKind.Void && !voidAllowed)
                || bound.Type.IsPointer
                || bound.Type.IsSpecialRuntimeType
                || bound.Type.IsByRefLike
                || bound.Type == InternalType.MethodGroup
                || bound.Type == InternalType.AnonymousMethod
                || bound.Type == InternalType.VarOutType
        ...
        //
        public bool FixIndependentTypeArguments(
            ResolveContext ec,
            TypeSpec[] methodParameters,
            ref bool fixed_any
        )
        ...
        public bool FixType(ResolveContext ec, int i)
        {
                ...
                switch (bound.Kind)
                {
                    ...
                    case BoundKind.Lower:
                        for (; cii != applicable.Length; ++cii)
                        {
                            ...
                            //
                            if (
                                !Convert.ImplicitConversionExists(
                                    ec,
                                    bound.GetTypeExpression(),
                                    candidates[cii].Type
                                )
                            )
                            {
                    ...
                    case BoundKind.Upper:
                        for (; cii != applicable.Length; ++cii)
                        {
                            ...
                            if (
                                !Convert.ImplicitConversionExists(
                                    ec,
                                    candidates[cii].GetTypeExpression(),
                                    bound.Type
                                )
                            )
                                applicable[cii] = false;
            ...
            for (int ci = 0; ci < applicable.Length; ++ci)
            {
                ...
                if (best_candidate != null)
                {
                    ...
                    if (
                        bound.Type.BuiltinType != BuiltinTypeSpec.Type.Dynamic
        ...
        public TypeSpec InflateGenericArgument(IModuleContext context, TypeSpec parameter)
        {
            ...
            if (tp != null)
            {
                ...
                //
                if (
                    tp.DeclaredPosition < tp_args.Length
                    && tp_args[tp.DeclaredPosition] == parameter
                )
        ...
        int LowerBoundInference(TypeSpec u, TypeSpec v, bool inversed)
        {
            ...
            {
                AddToBounds(
                    new BoundInfo(u, inversed ? BoundKind.Upper : BoundKind.Lower),
                    pos,
                    false
                );
                return 1;
            ...
            if (v.IsGenericOrParentIsGeneric)
            {
                ...
                foreach (TypeSpec u_candidate in u_candidates)
                {
                    ...
                    {
                        TypeSpec[] second_unique_candidate_targs = TypeSpec.GetAllTypeArguments(
                            u_candidate
                        );
                        if (
                            TypeSpecComparer.Equals(
                ...
                if (unique_candidate_targs != null)
                {
                    ...
                    for (int i = 0; i < unique_candidate_targs.Length; ++i)
                    {
                        ...
                        {
                            bool upper_bound =
                                (variance == Variance.Contravariant && !inversed)
        ...
        public int OutputTypeInference(ResolveContext ec, Expression e, TypeSpec t)
        {
            ...
            if (e is MethodGroupExpr)
            {
                ...
                Arguments args = DelegateCreation.CreateDelegateMethodArguments(
                    ec,
                    invoke.Parameters,
                    param_types,
                    e.Location
                );
                mg = mg.OverloadResolve(
                    ec,
                    ref args,
                    null,
                    OverloadResolver.Restrictions.CovariantDelegate
