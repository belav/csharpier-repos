...
namespace Mono.CSharp
{
    ...
    public class TypeSpec : MemberSpec
    {
        ...
        static TypeSpec()
        {
            ...
            if (GenericTypeBuilder == null)
                GenericTypeBuilder = assembly.GetType(
        ...
        {
            get { return MemberDefinition.TypeParametersCount; }
        }
        ...
        {
            get { return BuiltinTypeSpec.Type.None; }
        }

        public bool HasDynamicElement
        {
            get { return (state & StateFlags.HasDynamicElement) != 0; }
        }

        public bool HasNamedTupleElement
        {
            get { return (state & StateFlags.HasNamedTupleElement) != 0; }
        }
        ...
        public bool IsArray
        {
            get { return Kind == MemberKind.ArrayType; }
        }
        ...
        {
            get { return Kind == MemberKind.Interface; }
        }

        public bool IsClass
        {
            get { return Kind == MemberKind.Class; }
        }
        ...
        {
            get { return Kind == MemberKind.Delegate; }
        }
        ...
        public virtual bool IsExpressionTreeType
        {
            ...
            {
                state = value
                    ? state | StateFlags.InflatedExpressionType
                    : state & ~StateFlags.InflatedExpressionType;
            }
        }
        ...
        public virtual bool IsArrayGenericInterface
        {
            ...
            {
                state = value
                    ? state | StateFlags.GenericIterateInterface
                    : state & ~StateFlags.GenericIterateInterface;
            }
        }
        ...
        {
            get { return (state & StateFlags.Tuple) != 0; }
            set { state = value ? state | StateFlags.Tuple : state & ~StateFlags.Tuple; }
        }
        ...
        {
            get { return Kind == MemberKind.PointerType; }
        }
        ...
        {
            get { return (state & StateFlags.SpecialRuntimeType) != 0; }
            set
            {
                state = value
                    ? state | StateFlags.SpecialRuntimeType
                    : state & ~StateFlags.SpecialRuntimeType;
            }
        }
        ...
        {
            get { return Kind == MemberKind.Struct; }
        }

        public bool IsStructOrEnum
        {
            get { return (Kind & (MemberKind.Struct | MemberKind.Enum)) != 0; }
        }
        ...
        {
            get { return (ITypeDefinition)definition; }
        }
        ...
        public virtual string GetSignatureForDocumentation(bool explicitName)
        {
            ...
            else if (MemberDefinition.Namespace != null)
            {
                sb.Append(
                    explicitName
                        ? MemberDefinition.Namespace.Replace('.', '#')
        ...
        protected virtual void InitializeMemberCache(bool onlyTypes)
        {
            ...
            catch (Exception e)
            {
                throw new InternalErrorException(
                    e,
                    "Unexpected error when loading type `{0}'",
                    GetSignatureForError()
                );
            }
        ...
        public static bool IsReferenceType(TypeSpec t)
        {
            switch (t.Kind)
            {
                ...
                case MemberKind.InternalCompilerType:
                    ...
                    //
                    return t == InternalType.NullLiteral
        ...
        public InflatedTypeSpec MakeGenericType(IModuleContext context, TypeSpec[] targs)
        {
            if (targs.Length == 0 && !IsNested)
                throw new ArgumentException(
                    "Empty type arguments for type " + GetSignatureForError()
                );
            ...
            if (inflated_instances == null)
            {
                inflated_instances = new Dictionary<TypeSpec[], InflatedTypeSpec>(
                    TypeSpecComparer.Default
                );
            ...
            if (!inflated_instances.TryGetValue(targs, out instance))
            {
                if (GetDefinition() != this && !IsNested)
                    throw new InternalErrorException(
                        "`{0}' must be type definition or nested non-inflated type to MakeGenericType",
                        GetSignatureForError()
                    );
    ...
    public sealed class BuiltinTypeSpec : TypeSpec
    {
            ...
            : this(MemberKind.InternalCompilerType, "", name, builtinKind)
        {
            ...
            state =
                (
                    state
                    & ~(
                        StateFlags.CLSCompliant_Undetected
                        | StateFlags.Obsolete_Undetected
        ...
        {
            return type.BuiltinType >= Type.FirstPrimitive
                && type.BuiltinType <= Type.LastPrimitive;
        }
        ...
        public static class Override
        {
            public static bool IsEqual(TypeSpec a, TypeSpec b)
            {
                ...
                if (tp_a != null)
                {
                    ...
                    return tp_b != null
                        && tp_a.IsMethodOwned == tp_b.IsMethodOwned
                        && tp_a.DeclaredPosition == tp_b.DeclaredPosition;
                }
                ...
                if (ac_a != null)
                {
                    ...
                    return ac_b != null
                        && ac_a.Rank == ac_b.Rank
                        && IsEqual(ac_a.Element, ac_b.Element);
                }

                if (
                    a.BuiltinType == BuiltinTypeSpec.Type.Dynamic
                    || b.BuiltinType == BuiltinTypeSpec.Type.Dynamic
                )
                    return b.BuiltinType == BuiltinTypeSpec.Type.Object
                        || a.BuiltinType == BuiltinTypeSpec.Type.Object;
            ...
            public static bool IsEqual(AParametersCollection a, AParametersCollection b)
            {
                ...
                for (int i = 0; i < a.Count; ++i)
                {
                    ...
                    if (
                        (a.FixedParameters[i].ModFlags & Parameter.Modifier.RefOutMask)
                        != (b.FixedParameters[i].ModFlags & Parameter.Modifier.RefOutMask)
                    )
                        return false;
                }
    ...
    class InternalType : TypeSpec, ITypeDefinition
    {
        ...
        {
            get { return InternalTypeAssembly.Instance; }
        }
        ...
        void ITypeDefinition.LoadMembers(
            TypeSpec declaringType,
            bool onlyTypes,
    ...
    public abstract class ElementTypeSpec : TypeSpec, ITypeDefinition
    {
        ...
        IAssemblyDefinition ITypeDefinition.DeclaringAssembly
        {
            get { return Element.MemberDefinition.DeclaringAssembly; }
        }
        ...
        {
            get { return Element.MemberDefinition.IsImported; }
        }
        ...
        bool? IMemberDefinition.CLSAttributeValue
        {
            get { return Element.MemberDefinition.CLSAttributeValue; }
        }
    ...
    public class ArrayContainer : ElementTypeSpec
    {
        ...
        public override IList<TypeSpec> Interfaces
        {
            get { return BaseType.Interfaces; }
        }
        ...
        public MethodInfo GetAddressMethod()
        {
            ...
            var address = mb.GetArrayMethod(
                ...
                ReferenceContainer.MakeType(module, Element).GetMetaInfo(),
                arg_types
            );
        ...
        public MethodInfo GetSetMethod()
        {
            ...
            var set = mb.GetArrayMethod(
                ...
                "Set",
                CallingConventions.HasThis | CallingConventions.Standard,
                module.Compiler.BuiltinTypes.Void.GetMetaInfo(),
                arg_types
            );
