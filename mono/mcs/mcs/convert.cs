...
namespace Mono.CSharp
{
    ...
    static class Convert
    {
        ...
        static bool IList_To_Array(TypeSpec list, ArrayContainer array)
        {
            ...
            return ImplicitReferenceConversionExists(array.Element, arg_type)
                || ExplicitReferenceConversionExists(array.Element, arg_type);
        }

        public static Expression ImplicitTypeParameterConversion(
            Expression expr,
            TypeParameterSpec expr_type,
        ...
        )
        {
            ...
            if (target_type.IsGenericParameter)
            {
                if (expr_type.TypeArguments != null && expr_type.HasDependencyOn(target_type))
                {
                    ...
                    if (
                        ...
                        && !((TypeParameterSpec)target_type).IsReferenceType
                    )
            ...
            if (
                ...
                || TypeSpec.IsBaseClass(base_type, target_type, false)
                || base_type.ImplementsInterface(target_type, true)
            )
            {
                if (expr == null)
                    return EmptyExpression.Null;
        ...
        static Expression ExplicitTypeParameterConversionFromT(
            Expression source,
        ...
        )
        {
            ...
            if (target_type.IsInterface)
                return source == null
                    ? EmptyExpression.Null
                    : new ClassCast(source, target_type, true);
        ...
        )
        {
            ...
            if (
                TypeSpecComparer.IsEqual(effective, source_type)
                || TypeSpec.IsBaseClass(effective, source_type, false)
            )
        ...
        //
        public static bool ImplicitReferenceConversionExists(
            TypeSpec expr_type,
            TypeSpec target_type
        )
        {
        ...
        )
        {
            ...
            switch (expr_type.Kind)
            {
                ...
                case MemberKind.Delegate:
                    ...
                    //
                    return expr_type.ImplementsInterface(target_type, false)
                        || TypeSpecComparer.Variant.IsEqual(expr_type, target_type);
                ...
                case MemberKind.Interface:
                    ...
                    if (target_type.IsInterface)
                        return TypeSpecComparer.Variant.IsEqual(expr_type, target_type)
                            || expr_type.ImplementsInterface(target_type, true);

                    return target_type.BuiltinType == BuiltinTypeSpec.Type.Object
                        || target_type.BuiltinType == BuiltinTypeSpec.Type.Dynamic;
                ...
                case MemberKind.InternalCompilerType:
                    ...
                    if (expr_type == InternalType.NullLiteral)
                    {
                        ...
                        return TypeSpec.IsReferenceType(target_type)
                            || target_type.Kind == MemberKind.PointerType;
                    }
        ...
        public static Expression ImplicitBoxingConversion(
            Expression expr,
            TypeSpec expr_type,
            TypeSpec target_type
        )
        {
            switch (target_type.BuiltinType)
            {
                ...
                case BuiltinTypeSpec.Type.Enum:
                    ...
                    if (expr_type.IsEnum)
                        return expr == null
                            ? EmptyExpression.Null
                            : new BoxedCast(expr, target_type);
            ...
            if (expr_type.IsNullableType)
            {
                ...
                var res = ImplicitBoxingConversion(
                    expr,
                    Nullable.NullableInfo.GetUnderlyingType(expr_type),
                    target_type
                );
            ...
            //
            if (
                target_type.IsInterface
                && TypeSpec.IsValueType(expr_type)
                && expr_type.ImplementsInterface(target_type, true)
            )
            {
                return expr == null ? EmptyExpression.Null : new BoxedCast(expr, target_type);
        ...
        public static Expression ImplicitNulableConversion(
            ResolveContext ec,
            Expression expr,
            TypeSpec target_type
        )
        {
            ...
            if (expr_type == InternalType.NullLiteral)
                return ec == null
                    ? EmptyExpression.Null
                    : Nullable.LiftedNull.Create(target_type, expr.Location);
        ...
        static Expression ImplicitNumericConversion(
            Expression expr,
            TypeSpec expr_type,
            TypeSpec target_type
        )
                ...
                case BuiltinTypeSpec.Type.SByte:
                    ...
                    switch (target_type.BuiltinType)
                    {
                        case BuiltinTypeSpec.Type.Int:
                            return expr == null
                                ? EmptyExpression.Null
                                : new OpcodeCast(expr, target_type, OpCodes.Conv_I4);
                        case BuiltinTypeSpec.Type.Long:
                            return expr == null
                                ? EmptyExpression.Null
                                : new OpcodeCast(expr, target_type, OpCodes.Conv_I8);
                        case BuiltinTypeSpec.Type.Double:
                            return expr == null
                                ? EmptyExpression.Null
                                : new OpcodeCast(expr, target_type, OpCodes.Conv_R8);
                        case BuiltinTypeSpec.Type.Float:
                            return expr == null
                                ? EmptyExpression.Null
                                : new OpcodeCast(expr, target_type, OpCodes.Conv_R4);
                        case BuiltinTypeSpec.Type.Short:
                            return expr == null
                                ? EmptyExpression.Null
                                : new OpcodeCast(expr, target_type, OpCodes.Conv_I2);
                        case BuiltinTypeSpec.Type.Decimal:
                            return expr == null
                                ? EmptyExpression.Null
                                : new OperatorCast(expr, target_type);
                    }
                ...
                case BuiltinTypeSpec.Type.Byte:
                    ...
                    switch (target_type.BuiltinType)
                    {
                        ...
                        case BuiltinTypeSpec.Type.ULong:
                            return expr == null
                                ? EmptyExpression.Null
                                : new OpcodeCast(expr, target_type, OpCodes.Conv_U8);
                        case BuiltinTypeSpec.Type.Long:
                            return expr == null
                                ? EmptyExpression.Null
                                : new OpcodeCast(expr, target_type, OpCodes.Conv_I8);
        ...
        static Expression ImplicitTupleLiteralConversion(
            ResolveContext rc,
            Expression source,
            TypeSpec targetType,
            Location loc
        )
        {
            ...
            for (int i = 0; i < targetType.Arity; ++i)
            {
                ...
                {
                    elementSrc = new MemberAccess(
                        instance,
                        NamedTupleSpec.GetElementPropertyName(i)
                    ).Resolve(rc);
                }
        ...
        static bool ImplicitTupleLiteralConversionExists(Expression source, TypeSpec targetType)
        {
            ...
            for (int i = 0; i < targetType.Arity; ++i)
            {
                if (tupleLiteralElements != null)
                {
                    if (
                        !ImplicitStandardConversionExists(
                            tupleLiteralElements[i].Expr,
                            targetTypeArgument[i]
                        )
                    )
                    {
                        return false;
                ...
                else
                {
                    if (
                        !ImplicitStandardConversionExists(
                            new EmptyExpression(srcTypeArgument[i]),
                            targetTypeArgument[i]
                        )
                    )
                    {
        ...
        //
        public static bool ImplicitConversionExists(
            ResolveContext ec,
            Expression expr,
            TypeSpec target_type
        )
        {
            ...
            return UserDefinedConversion(
                    ...
                    expr,
                    target_type,
                    UserConversionRestriction.ImplicitOnly | UserConversionRestriction.ProbingOnly,
                    Location.Null
                ) != null;
        }
        ...
        )
        {
            ...
            {
                if (
                    target_type.IsDelegate
                    && rc.Module.Compiler.Settings.Version != LanguageVersion.ISO_1
                )
                {
                    MethodGroupExpr mg = expr as MethodGroupExpr;
                    if (mg != null)
                        return DelegateCreation.ImplicitStandardConversionExists(
                            rc,
                            mg,
                            target_type
                        );
            ...
            {
                if (
                    target_type == rc.Module.PredefinedTypes.IFormattable.TypeSpec
                    || target_type == rc.Module.PredefinedTypes.FormattableString.TypeSpec
                )
                    return true;
            }
        ...
        public static bool ImplicitStandardConversionExists(Expression expr, TypeSpec target_type)
        {
            ...
            //
            if (
                target_type.IsPointer
                && expr.Type.IsPointer
                && ((PointerContainer)target_type).Element.Kind == MemberKind.Void
            )
                return true;
        ...
        //
        static TypeSpec FindMostSpecificSource(
            ResolveContext rc,
        ...
        /// </summary>
        public static TypeSpec FindMostSpecificTarget(
        ...
        /// </summary>
        public static Expression ImplicitUserConversion(
            ResolveContext ec,
            Expression source,
            TypeSpec target,
            Location loc
        )
        {
            return UserDefinedConversion(
                ...
                source,
                target,
                UserConversionRestriction.ImplicitOnly,
                loc
            );
        }
        ...
        /// </summary>
        static Expression ExplicitUserConversion(
            ResolveContext ec,
            Expression source,
            TypeSpec target,
            Location loc
        ...
        static void FindApplicableUserDefinedConversionOperators(
            ResolveContext rc,
            IList<MemberSpec> operators,
            Expression source,
            TypeSpec target,
            UserConversionRestriction restr,
            ref List<MethodSpec> candidates
        )
        {
        ...
        )
        {
            ...
            if (source_type.IsNullableType)
            {
                ...
                if (
                    implicitOnly
                    && !TypeSpec.IsReferenceType(target_type)
                    && !target_type.IsNullableType
                )
                {
            ...
            const MemberKind user_conversion_kinds =
                MemberKind.Class | MemberKind.Struct | MemberKind.TypeParameter;

            if (
                (source_type.Kind & user_conversion_kinds) != 0
                && source_type.BuiltinType != BuiltinTypeSpec.Type.Decimal
            )
            {
                bool declared_only = source_type.IsStruct;

                var operators = MemberCache.GetUserOperator(
                    source_type,
                    Operator.OpType.Implicit,
                    declared_only
                );
                if (operators != null)
                {
                    FindApplicableUserDefinedConversionOperators(
                        rc,
                        operators,
                        source_type_expr,
                        target_type,
                ...
                {
                    operators = MemberCache.GetUserOperator(
                        source_type.TypeArguments[0],
                        Operator.OpType.Implicit,
                        declared_only
                    );
                    if (operators != null)
            ...
            if (
                (target.Kind & user_conversion_kinds) != 0
                && target_type.BuiltinType != BuiltinTypeSpec.Type.Decimal
            )
            {
                ...
                if (t_x.IsNullableType && (target.IsNullableType || !implicitOnly))
                {
                    ...
                    if (t_x != target)
                    {
                        ...
                        if (target.IsNullableType)
                            source = new Nullable.LiftedConversion(source, unwrap, target).Resolve(
                                rc
                            );
                    }
                }
        ...
        /// </summary>
        public static Expression ImplicitConversion(
            ResolveContext ec,
            Expression expr,
            TypeSpec target_type,
            Location loc
        ...
        /// </summary>
        public static Expression ImplicitConversionStandard(
            ResolveContext ec,
            Expression expr,
            TypeSpec target_type,
            Location loc
        ...
        )
        {
            ...
            {
                if (
                    expr_type != InternalType.NullLiteral
                    && expr_type != InternalType.AnonymousMethod
                    && expr_type != InternalType.ThrowExpr
                )
                    return expr;
                return null;
            }
                ...
                switch (target_type.Kind)
                {
                    ...
                    case MemberKind.TypeParameter:
                        ...
                        return new DynamicConversion(
                            target_type,
                            explicit_cast ? CSharpBinderFlags.ConvertExplicit : 0,
                            args,
                            loc
                        ).Resolve(ec);
                }
            ...
            if (expr_type == InternalType.DefaultType)
            {
                return new DefaultValueExpression(
                    new TypeExpression(target_type, expr.Location),
                    expr.Location
                ).Resolve(ec);
            }
            ...
            if (expr is IntegralConstant && target_type.IsEnum)
            {
                ...
                if (i.IsZeroInteger)
                {
                    ...
                    return new EnumConstant(
                        new IntLiteral(ec.BuiltinTypes, 0, i.Location),
                        target_type
                    );
                }
            }
            ...
            if (
                expr_type == InternalType.Arglist
                && target_type == ec.Module.PredefinedTypes.ArgIterator.TypeSpec
            )
                return expr;
            ...
            {
                if (
                    target_type == ec.Module.PredefinedTypes.IFormattable.TypeSpec
                    || target_type == ec.Module.PredefinedTypes.FormattableString.TypeSpec
                )
        ...
        /// </summary>
        public static Expression ImplicitConversionRequired(
            ResolveContext ec,
            Expression source,
            TypeSpec target_type,
            Location loc
        ...
        ///
        /// </summary>
        public static Expression ExplicitNumericConversion(
            ResolveContext rc,
            Expression expr,
            TypeSpec target_type
        )
            ...
            switch (expr.Type.BuiltinType)
            {
                    ...
                    switch (target_type.BuiltinType)
                    {
                        ...
                        case BuiltinTypeSpec.Type.UIntPtr:
                            return new OperatorCast(
                                new ConvCast(expr, rc.BuiltinTypes.ULong, ConvCast.Mode.I1_U8),
                                target_type,
                                target_type,
                                true
                            );
                ...
                case BuiltinTypeSpec.Type.Short:
                    ...
                    switch (target_type.BuiltinType)
                    {
                        ...
                        case BuiltinTypeSpec.Type.UIntPtr:
                            return new OperatorCast(
                                new ConvCast(expr, rc.BuiltinTypes.ULong, ConvCast.Mode.I2_U8),
                                target_type,
                                target_type,
                                true
                            );
                ...
                case BuiltinTypeSpec.Type.ULong:
                    ...
                    switch (target_type.BuiltinType)
                    {
                        ...
                        case BuiltinTypeSpec.Type.IntPtr:
                            return new OperatorCast(
                                EmptyCast.Create(expr, rc.BuiltinTypes.Long),
                                target_type,
                                true
                            );
                    }
                ...
                case BuiltinTypeSpec.Type.UIntPtr:
                    ...
                    switch (target_type.BuiltinType)
                    {
                        case BuiltinTypeSpec.Type.SByte:
                            return new ConvCast(
                                new OperatorCast(expr, expr.Type, rc.BuiltinTypes.UInt, true),
                                target_type,
                                ConvCast.Mode.U4_I1
                            );
                        case BuiltinTypeSpec.Type.Short:
                        ...
                        case BuiltinTypeSpec.Type.Long:
                            return EmptyCast.Create(
                                new OperatorCast(expr, expr.Type, rc.BuiltinTypes.ULong, true),
                                target_type
                            );
                    }
                    break;
                case BuiltinTypeSpec.Type.IntPtr:
                    if (target_type.BuiltinType == BuiltinTypeSpec.Type.UInt)
                        return EmptyCast.Create(
                            new OperatorCast(expr, expr.Type, rc.BuiltinTypes.Int, true),
                            target_type
                        );
                    if (target_type.BuiltinType == BuiltinTypeSpec.Type.ULong)
                        return EmptyCast.Create(
                            new OperatorCast(expr, expr.Type, rc.BuiltinTypes.Long, true),
                            target_type
                        );
        ...
        /// </summary>
        public static bool ExplicitReferenceConversionExists(
            TypeSpec source_type,
            TypeSpec target_type
        )
        {
        ...
        )
        {
            ...
            //
            if (
                source_type.BuiltinType == BuiltinTypeSpec.Type.Object
                && TypeManager.IsGenericParameter(target_type)
            )
            ...
            bool target_is_value_type =
                target_type.Kind == MemberKind.Struct || target_type.Kind == MemberKind.Enum;
            ...
            if (
                source_type.BuiltinType == BuiltinTypeSpec.Type.Object
                || source_type.BuiltinType == BuiltinTypeSpec.Type.Dynamic
            )
            {
            ...
            //
            if (
                source_type.Kind == MemberKind.Class
                && TypeSpec.IsBaseClass(target_type, source_type, true)
            )
                return source == null ? EmptyExpression.Null : new ClassCast(source, target_type);
            ...
            if (source_type.Kind == MemberKind.Interface)
            {
                if (!target_type.IsSealed || target_type.ImplementsInterface(source_type, true))
                {
                    ...
                    //
                    return target_is_value_type
                        ? new UnboxCast(source, target_type)
                        : (Expression)new ClassCast(source, target_type);
                }
            ...
            if (source_array != null)
            {
                ...
                if (target_array != null)
                {
                    ...
                    if (source_array.Rank == target_array.Rank)
                    {
                        ...
                        //
                        if (
                            (source_type.Kind & target_element.Kind & MemberKind.TypeParameter)
            ...
            if (
                target_type.IsInterface
                && !source_type.IsSealed
                && !source_type.ImplementsInterface(target_type, true)
            )
            {
                return source == null ? EmptyExpression.Null : new ClassCast(source, target_type);
            }
            ...
            //
            if (
                source_type.IsDelegate
                && target_type.IsDelegate
                && source_type.MemberDefinition == target_type.MemberDefinition
            )
            {
                ...
                for (i = 0; i < tparams.Length; ++i)
                {
                    ...
                    else if (tparams[i].Variance == Variance.Contravariant)
                    {
                        ...
                        //
                        if (
                            TypeSpec.IsReferenceType(targs_src[i])
                            && TypeSpec.IsReferenceType(targs_dst[i])
                        )
                            continue;
                    }
        ...
        /// </summary>
        public static Expression ExplicitConversionCore(
            ResolveContext ec,
            Expression expr,
            TypeSpec target_type,
            Location loc
        )
        {
            ...
            if (expr_type.IsEnum)
            {
                TypeSpec real_target = target_type.IsEnum
                    ? EnumSpec.GetUnderlyingType(target_type)
                    : target_type;
                Expression underlying = EmptyCast.Create(
                    expr,
                    EnumSpec.GetUnderlyingType(expr_type)
                );
                ...
                if (
                    ne == null
                    && (
                        real_target.BuiltinType == BuiltinTypeSpec.Type.IntPtr
                        || real_target.BuiltinType == BuiltinTypeSpec.Type.UIntPtr
                    )
                )
            ...
            if (target_type.IsEnum)
            {
                ...
                else
                {
                    ...
                    //
                    if (
                        expr_type.BuiltinType == BuiltinTypeSpec.Type.IntPtr
                        || expr_type.BuiltinType == BuiltinTypeSpec.Type.UIntPtr
                    )
                    {
        ...
        /// </summary>
        public static Expression ExplicitConversionStandard(
            ResolveContext ec,
            Expression expr,
            TypeSpec target_type,
            Location l
        )
        {
            ...
            if (
                ...
                && expr.Type.IsPointer
                && target_type.IsPointer
                && ((PointerContainer)expr.Type).Element.Kind == MemberKind.Void
            )
                return EmptyCast.Create(expr, target_type);
        ...
        ///   type is expr.Type to `target_type'.
        /// </summary>
        public static Expression ExplicitConversion(
            ResolveContext ec,
            Expression expr,
            TypeSpec target_type,
            Location loc
        )
        {
            ...
            if (target_type.IsNullableType)
            {
                ...
                if (e != null)
                    return TypeSpec.IsReferenceType(expr.Type)
                        ? new UnboxCast(expr, target_type)
                        : Nullable.Wrap.Create(e, target_type);
            }
            else if (expr_type.IsNullableType)
            {
                e = ImplicitBoxingConversion(
                    expr,
                    Nullable.NullableInfo.GetUnderlyingType(expr_type),
                    target_type
                );
