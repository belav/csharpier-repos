...
namespace Mono.CSharp
{
    ...
    public abstract class Constant : Expression
    {
        ...
        )
        {
            if (
                !expl
                && IsLiteral
                && type.BuiltinType != BuiltinTypeSpec.Type.Double
        ...
        public virtual Constant ConvertImplicitly(TypeSpec type)
        {
            ...
            if (fail)
            {
                ...
                throw new InternalErrorException(
                    "Missing constant conversion between `{0}' and `{1}'",
...
#if STATIC
            throw new InternalErrorException(
                "Constant value `{0}' has unexpected underlying type `{1}'",
                v,
        ...
        public override Constant ConvertExplicitly(bool in_checked_context, TypeSpec target_type)
        {
            switch (target_type.BuiltinType)
            {
                ...
                case BuiltinTypeSpec.Type.Char:
                    ...
                    {
                        if (
                            Value < (float)char.MinValue
                            || Value > (float)char.MaxValue
                            || float.IsNaN(Value)
                        )
                            throw new OverflowException();
                    }
        ...
        public override Constant ConvertExplicitly(bool in_checked_context, TypeSpec target_type)
        {
            switch (target_type.BuiltinType)
            {
                ...
                case BuiltinTypeSpec.Type.Char:
                    ...
                    {
                        if (
                            Value < (double)char.MinValue
                            || Value > (double)char.MaxValue
                            || double.IsNaN(Value)
                        )
                            throw new OverflowException();
                    }
    ...
    public class StringConstant : Constant
    {
        ...
        public override void Emit(EmitContext ec)
        {
            ...
            var str = Value;
            if (
                ec.Module.GetResourceStrings != null
                && !ec.Module.GetResourceStrings.TryGetValue(str, out str)
            )
            {
                str = Value;
            }
    ...
    class NameOf : StringConstant
    {
        ...
        bool ResolveArgumentExpression(ResolveContext rc, Expression expr)
        {
            ...
            if (sn != null)
            {
                ...
                var res = sn.LookupNameExpression(
                    rc,
                    MemberLookupRestrictions.IgnoreAmbiguity
                        | MemberLookupRestrictions.NameOfExcluded
                );
        ...
        static bool IsLeftResolvedExpressionValid(Expression expr)
        {
            ...
            if (fe != null)
            {
                return fe.InstanceExpression == null
                    || IsLeftResolvedExpressionValid(fe.InstanceExpression);
            }

            var pe = expr as PropertyExpr;
            if (pe != null)
                return pe.InstanceExpression == null
                    || IsLeftResolvedExpressionValid(pe.InstanceExpression);
    ...
    public class NullConstant : Constant
    {
        ...
        public override Constant ConvertExplicitly(bool inCheckedContext, TypeSpec targetType)
        {
            ...
            if (
                targetType.Kind == MemberKind.InternalCompilerType
                && targetType.BuiltinType != BuiltinTypeSpec.Type.Dynamic
            )
                return null;
