...
namespace Mono.CSharp
{
    ...
    public class Report
    {
        ...
        public void RuntimeMissingSupport(Location loc, string feature)
        {
            Error(
                -88,
                loc,
                "Your .NET Runtime does not support `{0}'. Please use the latest Mono runtime instead.",
                feature
            );
        }
        ...
        public void Warning(int code, int level, Location loc, string message)
        {
            ...
            if (warning_regions_table != null && !loc.IsNull)
            {
                WarningRegions regions;
                if (
                    warning_regions_table.TryGetValue(loc.File, out regions)
                    && !regions.IsWarningEnabled(code, loc.Row)
                )
                    return;
            }
        ...
        {
            get { return reporting_disabled > 0; }
        }
    ...
    class SessionReportPrinter : ReportPrinter
    {
        ...
        public bool IsEmpty
        {
            get { return merged_messages == null && common_messages == null; }
        }
    ...
    public class ConsoleReportPrinter : StreamReportPrinter
    {
        ...
        static ConsoleReportPrinter()
        {
            ...
            if (p == -1)
                ...
                prefix =
                    GetBackground(config.Substring(p + 1)) + GetForeground(config.Substring(0, p));
            postfix = "\x001b[0m";
        }
