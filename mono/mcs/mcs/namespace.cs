...
namespace Mono.CSharp
{
    public class RootNamespace : Namespace
    {
        ...
        //
        public List<string> FindExtensionMethodNamespaces(
            IMemberContext ctx,
            string name,
            int arity
        )
    ...
    public class Namespace
    {
            ...
            : this()
        {
            ...
            if (root == null)
                throw new InternalErrorException(
                    "Root namespaces must be created using RootNamespace"
                );
        ...
        )
        {
            ...
            foreach (var ts in found)
            {
                if (ts.Arity == arity)
                {
                    ...
                    {
                        if (
                            (ts.Modifiers & Modifiers.INTERNAL) != 0
                            && !ts.MemberDefinition.IsInternalAsPublic(ctx.Module.DeclaringAssembly)
                            && mode != LookupMode.IgnoreAccessibility
                        )
                            continue;
                    ...
                    if (
                        (best.Modifiers & Modifiers.INTERNAL) != 0
                        && !best.MemberDefinition.IsInternalAsPublic(ctx.Module.DeclaringAssembly)
                    )
                        continue;
                ...
                if (arity < 0)
                {
                    if (best == null)
                    {
                        ...
                        System.Math.Abs(ts.Arity + arity) < System.Math.Abs(best.Arity + arity)
                    )
                    {
                        best = ts;
                    }
        ...
        public FullNamedExpression LookupTypeOrNamespace(
            IMemberContext ctx,
            string name,
            int arity,
            LookupMode mode,
        ...
        //
        public List<MethodSpec> LookupExtensionMethod(
            IMemberContext invocationContext,
            string name,
            int arity
        )
        {
        ...
        public void AddType(ModuleContainer module, TypeSpec ts)
        {
            ...
            {
                var extension_method_allowed = ts.MemberDefinition.IsImported
                    ? (ts.Modifiers & Modifiers.METHOD_EXTENSION) != 0
                    : (ts.IsStatic || ts.MemberDefinition.IsPartial);
        ...
        //
        public static TypeSpec IsImportedTypeOverride(
        ...
        {
            var ts_accessible =
                (ts.Modifiers & Modifiers.PUBLIC) != 0
                || ts.MemberDefinition.IsInternalAsPublic(module.DeclaringAssembly);
            var found_accessible =
                (found.Modifiers & Modifiers.PUBLIC) != 0
                || found.MemberDefinition.IsInternalAsPublic(module.DeclaringAssembly);
        ...
        public void VerifyClsCompliance()
        {
            ...
            var locase_types = new Dictionary<string, List<TypeSpec>>(
    ...
    public class CompilationSourceFile : NamespaceContainer
    {
        ...
        //
        void CreateUnitSymbolInfo(
            MonoSymbolFile symwriter,
            List<KeyValuePair<string, string>> pathMap
        )
        {
    ...
    public class NamespaceContainer : TypeContainer, IMemberContext
    {
        ...
        {
            var existing = ns.LookupType(
                this,
                next_part.MemberName.Name,
                next_part.MemberName.Arity,
                LookupMode.Probing,
                Location.Null
            );
        ...
        public override void AddTypeContainer(TypeContainer tc)
        {
            ...
            if (names_container.DefinedNames.TryGetValue(name, out mc))
            {
                ...
                if (
                    (mc.ModFlags & Modifiers.PARTIAL) != 0
                    && (tc is ClassOrStruct || tc is Interface)
                )
                {
        ...
        public ExtensionMethodCandidates LookupExtensionMethod(
            IMemberContext invocationContext,
            string name,
            int arity,
            int position
        )
        {
            ...
            do
            {
                candidates = container.LookupExtensionMethodCandidates(
                    invocationContext,
                    name,
                    arity,
                    ref position
                ...
                while (mn != null)
                {
                    ...
                    var methods = container_ns.LookupExtensionMethod(
                        invocationContext,
                        name,
                        arity
                    );
                    if (methods != null)
                    {
                        return new ExtensionMethodCandidates(
                            invocationContext,
                            methods,
                            container,
                            position
        ...
        ExtensionMethodCandidates LookupExtensionMethodCandidates(
            IMemberContext invocationContext,
            string name,
            int arity,
            ref int position
        ...
        )
        {
            ...
            for (
                NamespaceContainer container = this;
                container != null;
                container = container.Parent
            )
            {
        ...
        FullNamedExpression Lookup(string name, int arity, LookupMode mode, Location loc)
        {
            ...
            foreach (Namespace using_ns in namespace_using_table)
            {
                ...
                var better = Namespace.IsImportedTypeOverride(
                    Module,
                    texpr_match.Type,
                    texpr_fne.Type
                );
        ...
        void Error_AmbiguousReference(
            string name,
            FullNamedExpression a,
            FullNamedExpression b,
            Location loc
        )
        {
        ...
        public static Expression LookupStaticUsings(
            IMemberContext mc,
            string name,
            int arity,
            Location loc
        )
        {
            for (var m = mc.CurrentMemberDefinition; m != null; m = m.Parent)
            {
                ...
                if (candidates != null)
                {
                    var expr = Expression.MemberLookupToExpression(
                        mc,
                        candidates,
                        false,
                        null,
                        name,
                        arity,
                        Expression.MemberLookupRestrictions.None,
                        loc
                    );
                    if (expr != null)
                        return expr;
        ...
        void DoDefineNamespace()
        {
            ...
            if (clauses != null)
            {
                ...
                namespace_using_table =
                    namespaces == null ? new Namespace[0] : namespaces.ToArray();
                if (types != null)
    ...
    public class UsingAliasNamespace : UsingNamespace
    {
        ...
        public override void Define(NamespaceContainer ctx)
        {
            ...
            //
            resolved =
                NamespaceExpression.ResolveAsTypeOrNamespace(new AliasContext(ctx), false)
