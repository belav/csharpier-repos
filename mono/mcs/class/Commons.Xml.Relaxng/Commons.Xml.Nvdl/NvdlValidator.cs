...
namespace Commons.Xml.Nvdl
{
        ...
        NvdlSection GetSection(NvdlSection section)
        {
            ...
            else if (qnameStack.Count > 0 && rules.Triggers.Length > 0)
            {
                for (int t = 0; t < rules.Triggers.Length; t++)
                {
                    ...
                    XmlQualifiedName parent = (XmlQualifiedName)qnameStack.Peek();
                    if (
                        st.Cover(Reader.LocalName, Reader.NamespaceURI)
    ...
    internal class NvdlSection
    {
        ...
        public NvdlSection(NvdlDispatcher dispatcher, NvdlSection parentState)
        {
            ...
            {
                foreach (
                    SimpleAction a in FindElementRule(
            ...
            {
                foreach (NvdlInterpretation pi in parentState.ilist)
                {
        ...
        private NvdlInterpretation CreateInterp(
            NvdlDispatcher d,
            SimpleMode m,
            SimpleAction a,
        ...
        private void PopulateInterp(NvdlDispatcher d, NvdlInterpretation i, NvdlSection parentState)
        {
        ...
        private SimpleRule FindElementRule(SimpleMode mode, XmlReader reader)
        {
            ...
            throw new NvdlValidationException(
                "NVDL internal error: should not happen. No matching rule was found.",
