...
namespace Commons.Xml.Nvdl
{
    internal class SimplifiedItem : IXmlLineInfo
    {
        ...
        public string Location
        {
            ...
            {
                return line != 0
                    ? String.Format("{0} ({1},{2})", sourceUri, line, column)
                    : String.Empty;
            }
        }
    }
    ...
    internal class SimpleRules : SimplifiedItem
    {
        ...
        private void SimplifyPhase1(NvdlCompileContext ctx)
        {
            ...
            if (rules.Modes.Count > 0)
            {
                if (rules.Rules.Count > 0)
                    throw new NvdlCompileException(
                        "Modes and rules cannot coexist in 'rules' element.",
                        rules
                    );
                else if (rules.StartMode == null)
                    throw new NvdlCompileException(
                        "startMode is missing in 'rules' element when modes are specified.",
                        rules
                    );
                ...
                if (startMode == null)
                    throw new NvdlCompileException(
                        "Matching 'mode' element specified by 'startMode' does not exist.",
                        rules
                    );
            }
            else
            {
                if (rules.Rules.Count == 0)
                    throw new NvdlCompileException(
                        "Neither modes nor rules exists in 'rules' element.",
                        rules
                    );
            ...
            triggers = (SimpleTrigger[])tl.ToArray(typeof(SimpleTrigger));

            modes = (SimpleMode[])new ArrayList(ctx.GetCompiledModes()).ToArray(typeof(SimpleMode));
        }
    ...
    internal class SimpleTrigger : SimplifiedItem
    {
        ...
        public SimpleTrigger(NvdlTrigger trigger)
        {
            ...
            names = (XmlQualifiedName[])al.ToArray(typeof(XmlQualifiedName));
        }
    ...
    internal class SimpleMode : SimplifiedItem
    {
        ...
        public SimpleMode(NvdlMode mode, NvdlCompileContext ctx)
        {
            ...
            if (mode.Name == null)
                throw new NvdlCompileException("'mode' element must have a name.", mode);
            this.name = mode.Name;
            SimplifyPhase1(mode, ctx);
        }

        public SimpleMode(string name, NvdlNestedMode mode, NvdlCompileContext ctx)
        {
            FillLocation(mode);
        ...
        private void SimplifyPhase1(NvdlModeBase mode, NvdlCompileContext ctx)
        {
            NvdlModeCompileContext mctx = new NvdlModeCompileContext(mode);
        ...
        private void ConsumeIncludes(ArrayList al, NvdlCompileContext ctx)
        {
            ...
            for (int i = 0; i < al.Count; i++)
            {
                ...
                for (int j = i + 1; j < al.Count; j++)
                {
                    ...
                    if (r1.IsAny && r2.IsAny)
                        throw new NvdlCompileException(
                            "collision in mode was found. Two anyNamespace elements.",
                            this
                        );
                    if (r1.IsAny || r2.IsAny)
                        continue;
                    if (Nvdl.NSMatches(r1.NS, 0, r1.Wildcard, r2.NS, 0, r2.Wildcard))
    ...
    internal class SimpleRule : SimplifiedItem
    {
        ...
        public SimpleRule(NvdlRule rule, bool matchAttributes, NvdlCompileContext ctx)
        {
            FillLocation(rule);
            ...
            else
            {
                ...
                if (nss.Wildcard == null)
                    ...
                    throw new NvdlCompileException(
                        "'wildCard' attribute can specify at most one character string.",
                        rule
                    );
                else
        ...
        private void SimplifyPhase1(NvdlRule r, NvdlCompileContext ctx)
        {
            ...
            {
                NvdlNoCancelAction nca = a as NvdlNoCancelAction;
                if (nca != null)
                {
                    ...
                    if (ra != null)
                        ...
                        al.Add(new SimpleValidate((NvdlValidate)nca, ctx));
                    else if (nca is NvdlAllow)
                        al.Add(new SimpleValidate((NvdlAllow)nca, ctx));
                    else
                        al.Add(new SimpleValidate((NvdlReject)nca, ctx));
                }
                else if (nca == null)
                    ctx.CancelledRules.Add(this, this);
            }
            actions = (SimpleAction[])al.ToArray(typeof(SimpleAction));
        }

        private void SimplifyModeUsage(NvdlNoCancelAction nca, NvdlCompileContext ctx)
        {
            NvdlModeUsage usage = nca.ModeUsage;
            if (usage.NestedMode != null && ctx.GetCompiledMode(usage) == null)
            {
                SimpleMode sm = new SimpleMode(String.Empty, usage.NestedMode, ctx);
            ...
            foreach (NvdlContext c in usage.Contexts)
            {
                ...
                {
                    SimpleMode sm = new SimpleMode(String.Empty, c.NestedMode, ctx);
    ...
    internal class SimpleValidate : SimpleAction
    {
        ...
        static NvdlValidate CreateBuiltInValidate(NvdlAction a)
        {
            ...
            doc.AppendChild(doc.CreateElement("schema", Nvdl.Namespace));
        ...
        public SimpleValidate(NvdlValidate validate, NvdlCompileContext ctx)
            : base(validate)
        {
            // 6.4.7
            generator = ctx.Config.GetGenerator(validate, ctx.Rules.SchemaType);
        }
        ...
        {
            XmlDocument doc = new XmlDocument();
            XmlElement el = doc.CreateElement("virtualElement", Nvdl.InstanceNamespace);
            for (int i = 0; i < reader.AttributeCount; i++)
            {
                reader.MoveToAttribute(i);
                if (reader.NamespaceURI != ns)
                    continue;
                el.SetAttribute(reader.LocalName, reader.NamespaceURI, reader.Value);
            }
            reader.MoveToElement();
            XmlReader r = generator.CreateAttributeValidator(new XmlNodeReader(el), resolver);
            while (!r.EOF)
                r.Read();
        }
    }
    ...
    internal class SimpleResultAction : SimpleAction
    {
        ...
        public SimpleResultAction(NvdlResultAction ra, NvdlCompileContext ctx)
            : base(ra)
        {
    ...
    internal class SimpleModeUsage : SimplifiedItem
    {
        ...
        public SimpleModeUsage(NvdlModeUsage usage)
        {
            ...
            for (int i = 0; i < contexts.Length; i++)
                contexts[i] = new SimpleContext(usage.Contexts[i]);
        }
        ...
        internal void ResolveModes(NvdlCompileContext ctx, SimpleMode current)
        {
            ...
            if (mode == null)
                throw new NvdlCompileException(
                    "mode does not contain either referenced modeUsage or nested mode.",
                    null
                );
        }
    ...
    internal class SimpleContext : SimplifiedItem
    {
        ...
        public SimpleContext(NvdlContext context)
        {
            ...
            try
            {
                ...
                {
                    string spath = spathws.Trim(Nvdl.Whitespaces);
                    if (spath.Length == 0)
                        continue;
                    ParsePath(al, TrimName(spath));
                }
                path = (SimplePath[])al.ToArray(typeof(SimplePath));
            }
            catch (XmlException ex)
            {
                throw new NvdlCompileException(
                    String.Format("Invalid path string: {0}", path),
                    ex,
                    context
                );
            }
        ...
        private void ParsePath(ArrayList al, string path)
        {
            ...
            do
            {
                ...
                {
                    steps.Add(
                        new SimplePathStep(TrimName(path.Substring(start, idx - start)), true)
                    );
                    start = idx + 2;
                }
                else
                {
                    steps.Add(
                        new SimplePathStep(TrimName(path.Substring(start, idx - start)), false)
                    );
                    start = idx + 1;
                }
        ...
        internal void ResolveModes(NvdlCompileContext ctx, SimpleMode current)
        {
            ...
            if (useMode == null)
                throw new NvdlCompileException(
                    String.Format("Specified mode '{0}' was not found.", useModeName),
                    this
                );
        }
    }
