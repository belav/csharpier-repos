...
namespace Commons.Xml.Nvdl
{
    internal class NvdlFilteredXmlReader : XmlReader, IXmlLineInfo
    {
        ...
        private void AddAttribute()
        {
            ...
            {
                AttributeInfo[] newArr = new AttributeInfo[attributeCount * 2];
        ...
        public override bool Read()
        {
            ...
            if (attributeValidators.Count > 0)
            {
                ...
                {
                    ((SimpleValidate)attributeValidators[ns]).ValidateAttributes(reader, ns);
                }
            }
            ...
            get
            {
                switch (placeHolder)
                {
                    ...
                    case XmlNodeType.EndElement:
                        return "placeholder";
                    case XmlNodeType.Attribute:
                        return placeHolderLocalNameAttr ? "localName" : "ns";
                    case XmlNodeType.Text:
                        return String.Empty;
                    default:
        ...
        public override string Value
        {
            get
            {
                switch (placeHolder)
                {
                    ...
                    case XmlNodeType.Attribute:
                    case XmlNodeType.Text:
                        return placeHolderLocalNameAttr ? reader.LocalName : reader.NamespaceURI;
                    default:
                        return reader.Value;
                }
            }
        ...
        public override string GetAttribute(int i)
        {
            if (placeHolder == XmlNodeType.Element)
                ...
                return reader.GetAttribute(attributes[i].LocalName, attributes[i].NamespaceURI);
        }
        ...
        public override bool MoveToNextAttribute()
        {
            switch (placeHolder)
            {
                ...
                default:
                    ...
                    {
                        if (
                            attributes[i].LocalName != reader.LocalName
            ...
            switch (placeHolder)
            {
                ...
                default:
                    if (i < 0 || attributeCount <= i)
                        throw new IndexOutOfRangeException();
                    reader.MoveToAttribute(attributes[i].LocalName, attributes[i].NamespaceURI);
                    break;
            }
        }
