            ...
            foreach (string s in ar_colors)
            {
                ...
                if (mi.Length == 1)
                {
                    if (mi[0].MemberType == MemberTypes.Property)
                    {
                        ...
                        Assert.AreEqual(
                            Convert.ToByte(col[1]),
                            color.A,
                            col[0] + " Color A is wrong"
                        );
                        Assert.AreEqual(
                            Convert.ToByte(col[2]),
                            color.R,
                            col[0] + " Color R is wrong"
                        );
                        Assert.AreEqual(
                            Convert.ToByte(col[3]),
                            color.G,
                            col[0] + " Color G is wrong"
                        );
                        Assert.AreEqual(
                            Convert.ToByte(col[4]),
                            color.B,
                            col[0] + " Color B is wrong"
                        );
                        ...
                        Assert.AreEqual(
                            ...
                            color.GetBrightness(),
                            0.001F,
                            col[0] + " Color.GetBrightness() is wrong"
                        );
                        Assert.AreEqual(
                            float.Parse(col[6]),
                            color.GetHue(),
                            0.001F,
                            col[0] + " Color.GetHue() is wrong"
                        );
                        Assert.AreEqual(
                            float.Parse(col[7]),
                            color.GetSaturation(),
                            0.001F,
                            col[0] + " Color.GetSaturation() is wrong"
                        );

                        Assert.AreEqual(
                            true,
                            color.IsNamedColor,
                            col[0] + " IsNamedColor is wrong"
                        );
                        Assert.AreEqual(
                            false,
                            color.IsSystemColor,
                            col[0] + " IsSystemColor is wrong"
                        );
                        Assert.AreEqual(
                            true,
                            color.IsKnownColor,
                            col[0] + " IsKnownColor is wrong"
                        );
                    }
                    else
                    {
