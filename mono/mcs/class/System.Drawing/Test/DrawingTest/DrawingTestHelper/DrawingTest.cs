...
namespace DrawingTestHelper
{
    ...
    public sealed class ExpectedResultsHash
    {
        ...
        public static readonly XmlSerializer TestSuiteSerializer = new XmlSerializer(
            typeof(ExpectedResults)
        );
    ...
    public abstract class DrawingTest : IDisposable
    {
        ...
        protected static readonly ExpectedResultsHash ExpectedResults = new ExpectedResultsHash();
        protected static readonly Cache cache = new Cache();
        ...
        public static double CalculateNorm(Bitmap bitmap)
        {
            ...
            for (int x = 1; x <= size_x; x++)
            {
                ...
                for (int y = 1; y <= size_y; y++)
                {
                    ...
                    {
                        ComplexF cur = matrix[
                            (size_x - x) + size_x * (size_y - y) + size_x * size_y * (size_z - z)
                        ];
        ...
        private static void AssertAlmostEqual(
            float expected,
            float actual,
            float tolerance,
            string message
        ...
        {
            float error = System.Math.Abs(
                (expected - actual) / (expected + actual + float.Epsilon)
            );
    ...
    internal class NetDrawingTest : DrawingTest
    {
        ...
        protected override Bitmap GetReferenceImage(string testName)
        {
            ...
            try
            {
                ...
                {
                    return new Bitmap(
                        "/Developer/MonoTouch/Source/mono/mcs/class/System.Drawing/Test/DrawingTest/Test/PNGs/"
                            + fileName
                    );
                }
                else
        ...
        private class NetForm : MonoTouch.UIKit.UIViewController, IMyForm
        {
            ...
            public NetForm(string title, Image anImage)
                : base()
            {
                //base.Text = title;
                image = anImage;
        ...
        public override string CalculateSHA1()
        {
            ...
            BitmapData data = _bitmap.LockBits(r, ImageLockMode.ReadOnly, _bitmap.PixelFormat);
