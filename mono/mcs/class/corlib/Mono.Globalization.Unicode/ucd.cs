...
"
        ...
        UcdCharacterProperty Parse(string line)
        {
            ...
            return new UcdCharacterProperty()
            {
                ...
                CanonicalCombiningClass =
                    tokens[3].Length > 0 ? (byte?)byte.Parse(tokens[3]) : null,
                BidiClass =
                    tokens[4].Length > 0
                        ? (UcdBidiClass)Enum.Parse(typeof(UcdBidiClass), tokens[4])
                        : UcdBidiClass.None,
                DecompositionType =
                    decomp_type != null
                        ? ParseDecompositionType(decomp_type)
                ...
                SimpleUppercaseMapping =
                    tokens[12].Length > 0 ? int.Parse(tokens[12], NumberStyles.HexNumber) : 0,
                SimpleLowercaseMapping =
                    tokens[13].Length > 0 ? int.Parse(tokens[13], NumberStyles.HexNumber) : 0,
                SimpleTitlecaseMapping =
                    tokens[14].Length > 0 ? int.Parse(tokens[14], NumberStyles.HexNumber) : 0,
            };
        }
	...
	guint32 lower;
        ...
        public void GenerateUnicodeCategoryListC(UcdCharacterProperty[] ucd)
        {
            ...
            w.WriteLine(
                "static const GUnicodeType *unicode_category [{0}]  = {{",
                catalog.CategoryRanges.Length
            );
        ...
        public void GenerateSimpleTitlecaseMappingListC(UcdCharacterProperty[] ucd)
        {
            ...
            foreach (var ucp in ucd)
            {
                ...
                if (count > 0)
                    w.WriteLine(',');
                w.Write(
                    "\t{{0x{0:X06}, 0x{1:X06}, 0x{2:X06}}}",
                    ucp.Codepoint,
                    ucp.SimpleUppercaseMapping,
                    ucp.SimpleTitlecaseMapping
                );
                count++;
            }
            w.WriteLine();
        ...
        void GenerateSimpleCaseMappingListC(UcdCharacterProperty[] ucd, bool upper, bool small)
        {
            ...
            foreach (var cpr in catalog.SimpleCases)
            {
                ...
                w.WriteLine(
                    "static const {0} simple_{1}_case_mapping_{2}_table{3} [] = {{",
                    small ? "guint16" : "guint32",
                    upper ? "upper" : "lower",
                    small ? "lowarea" : "higharea",
                    nTable
                );
            ...
            w.WriteLine(
                "static const {0} *simple_{1}_case_mapping_{2} [] = {{",
                small ? "guint16" : "guint32",
                upper ? "upper" : "lower",
                small ? "lowarea" : "higharea"
            );
            ...
            for (int i = 0; i < nTable; i++)
            {
                if (i > 0)
                    w.WriteLine(",");
                w.Write(
                    "\tstatic const guint8 simple_{0}_case_mapping_{1}_table{2}",
                    upper ? "upper" : "lower",
                    small ? "lowarea" : "higharea",
                    i
                );
