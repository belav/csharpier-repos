...
namespace Mono.Globalization.Unicode
{
    ...
    {
        public static TailoringComparer Instance = new TailoringComparer();
    ...
    class Tailoring
    {
        ...
        public void Serialize(TextWriter w)
        {
            ...
            foreach (char c in TargetString)
                w.Write("{0:X04},", (int)c);
            w.WriteLine("}} {0}", TargetString.Length > 1 ? "!" : "");
            if (Before != 0)
    ...
    {
        public static TailoringStoreComparer Instance = new TailoringStoreComparer();
        ...
        void Run(string[] args)
        {
            ...
            foreach (FileInfo fi in new DirectoryInfo(dirname).GetFiles("*.xml"))
            {
                ...
                try
                {
                    ...
                    RelaxngValidatingReader rvr = new RelaxngValidatingReader(inst, p);
...
/*gboolean*/ public readonly bool FrenchSort;
        ...
        {
            XmlElement langElem = doc.SelectSingleNode("/ldml/identity/language") as XmlElement;
            string lang = langElem.GetAttribute("type");
            XmlElement terr = doc.SelectSingleNode("/ldml/identity/territory") as XmlElement;
            string lcid = lang + (terr != null ? "-" + terr.GetAttribute("type") : null);
            ...
            XmlElement collation =
                doc.SelectSingleNode("/ldml/collations/collation[@type='standard']") as XmlElement;
                ...
                switch (el.LocalName)
                {
                    case "reset":
                        ...
                        if (cpElem != null)
                            v = new string(
                                (char)(
                                    int.Parse(cpElem.GetAttribute("hex"), NumberStyles.HexNumber)
                                ),
                                1
                            );
                        else
                    ...
                    case "x":
                        ...
                        if (contraction != null && el.LastChild.InnerText != contraction)
                            throw new Exception(
                                "When there are sequential 'x' elements for single tailoring, those 'extend' text must be identical."
                            );
