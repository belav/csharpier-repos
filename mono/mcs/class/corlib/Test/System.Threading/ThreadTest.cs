...
namespace MonoTests.System.Threading
{
    ...
    public class ThreadTest
    {
        ...
        public void TestPriority1()
        {
            ...
            try
            {
                ...
                Assert.AreEqual(
                    ...
                    before,
                    "#40 Unexpected priority before thread start: "
                );
                TestThread.Start();
        ...
        public void TestThreadState()
        {
            ...
            try
            {
                ...
                Assert.IsTrue(
                    TestThread.ThreadState == ThreadState.Running
                        || (TestThread.ThreadState & ThreadState.Unstarted) != 0,
            ...
            Assert.IsTrue(
                (ThreadState.Stopped & TestThread.ThreadState) != 0
        ...
        public void CurrentThread_Domains()
        {
            ...
            var o = (DomainClass)
                ad.CreateInstanceAndUnwrap(
                    typeof(DomainClass).Assembly.FullName,
                    typeof(DomainClass).FullName
                );
            Assert.IsTrue(o.Run());
            AppDomain.Unload(ad);
        }
        ...
        void WaitSuspended(string s, Thread t)
        {
            ...
            {
                Assert.IsTrue(
                    (state & ThreadState.SuspendRequested) != 0,
                    s + ": expected SuspendRequested state"
                );
                Thread.Sleep(10);
                n++;
            ...
            Assert.IsTrue(
                (state & ThreadState.SuspendRequested) == 0,
                s + ": SuspendRequested state not expected"
            );
        }
    ...
    public class ThreadStateTest
    {
        ...
        public void StoppedThreadsThrowThreadStateException()
        {
            ...
            Assert.Throws<ThreadStateException>(
                () =>
                {
                    var isb = t.ApartmentState;
                },
                "ApartmentState getter"
            );
            Assert.Throws<ThreadStateException>(
    ...
    public class ThreadTest_ManagedThreadId
    {
        ...
        public void ManagedThreadId_AppDomains()
        {
            ...
            ad1 = AppDomain.CreateDomain(
                "AppDomain 1",
                currentDomain.Evidence,
                currentDomain.SetupInformation
            );
            ad2 = AppDomain.CreateDomain(
                "AppDomain 2",
                currentDomain.Evidence,
                currentDomain.SetupInformation
            );
    ...
    public class ThreadApartmentTest
    {
        ...
        public void GetStackTraces()
        {
            ...
            {
                var res =
                    (Dictionary<Thread, SD.StackTrace>)
                        typeof(Thread)
                            .GetMethod(
                                "Mono_GetStackTraces",
                                BindingFlags.NonPublic | BindingFlags.Static
    ...
    public class TestUtil
    {
        ...
        public static void WhileAlive(Thread t, bool alive, string s)
        {
            ...
            while (t.IsAlive == alive)
            {
                ...
                {
                    if (alive)
                        Assert.Fail("Timeout while waiting for not alive state. " + s);
                    else
                        Assert.Fail("Timeout while waiting for alive state. " + s);
                }
            }
        }
