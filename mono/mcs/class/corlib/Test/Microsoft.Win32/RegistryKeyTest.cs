    ...
    public class RegistryKeyTest
    {
        ...
        public void FromHandle()
        {
            ...
            try
            {
                ...
                using (
                    RegistryKey key = RegistryKey.FromHandle(new SafeRegistryHandle(handle, true))
                )
                {
        ...
        public void FromHandle_InvalidHandle()
        {
            ...
            {
                using (
                    RegistryKey key = RegistryKey.FromHandle(
                        new SafeRegistryHandle(IntPtr.Zero, true)
                    )
                )
                {
                    Assert.AreEqual(String.Empty, key.Name, "#A0");
        ...
        public void GetValue_Expand()
        {
            ...
            try
            {
                using (RegistryKey createdKey = Registry.CurrentUser.CreateSubKey(subKeyName))
                {
                    ...
                    createdKey.SetValue(
                        "name1",
                        "%MONO_TEST1%/%MONO_TEST2%",
                        RegistryValueKind.ExpandString
                    );
                    createdKey.SetValue("name2", "%MONO_TEST1%/%MONO_TEST2%");
                    createdKey.SetValue("name3", "just some text", RegistryValueKind.ExpandString);

                    Assert.AreEqual("123/456", createdKey.GetValue("name1"), "#A1");
                    Assert.AreEqual(
                        "%MONO_TEST1%/%MONO_TEST2%",
                        createdKey.GetValue("name2"),
                        "#A2"
                    );
                    ...
                    Assert.AreEqual(
                        "123/456",
                        createdKey.GetValue("name1", null, RegistryValueOptions.None),
                        "#A7"
                    );
                    Assert.AreEqual(
                        "%MONO_TEST1%/%MONO_TEST2%",
                        createdKey.GetValue("name2", null, RegistryValueOptions.None),
                        "#A8"
                    );
                    Assert.AreEqual(
                        "just some text",
                        createdKey.GetValue("name3", null, RegistryValueOptions.None),
                        "#A9"
                    );
        ...
        public void OpenRemoteBaseKey_MachineName_DoesNotExist()
        {
            ...
            try
            {
                RegistryKey.OpenRemoteBaseKey(RegistryHive.CurrentUser, "DOESNOTEXIST");
                Assert.Fail("#1");
            }
            catch (IOException ex)
            {
        ...
        public void SetValue2_Key_ReadOnly()
        {
            ...
            using (RegistryKey softwareKey = Registry.CurrentUser.OpenSubKey("software"))
            {
                try
                {
                    softwareKey.SetValue("name1", "value1", RegistryValueKind.String);
                    Assert.Fail("#1");
                }
                catch (UnauthorizedAccessException ex)
                {
            ...
            using (RegistryKey softwareKey = Registry.CurrentUser.OpenSubKey("software", true))
            {
                try
                {
                    ...
                    using (RegistryKey createdKey = softwareKey.OpenSubKey(subKeyName))
                    {
                        try
                        {
                            createdKey.SetValue("name1", "value1", RegistryValueKind.String);
                            Assert.Fail("#1");
                        }
                        catch (UnauthorizedAccessException ex)
                        {
        ...
        public void SetValue2_Name_Empty()
        {
            ...
            try
            {
                createdKey.SetValue(string.Empty, "value1", RegistryValueKind.String);
                string[] names = createdKey.GetValueNames();
                Assert.IsNotNull(names, "#A1");
                Assert.AreEqual(1, names.Length, "#A2");
                Assert.IsNotNull(names[0], "#A3");
                ...
                createdKey.SetValue(null, "value2", RegistryValueKind.String);
                names = createdKey.GetValueNames();
                Assert.IsNotNull(names, "#B1");
                Assert.AreEqual(1, names.Length, "#B2");
                Assert.IsNotNull(names[0], "#B3");
        ...
        public void SetValue2_Name_MaxLength()
        {
            ...
            try
            {
                using (RegistryKey createdKey = Registry.CurrentUser.CreateSubKey(subKeyName))
                {
                    ...
                    createdKey.SetValue(name, "value1", RegistryValueKind.String);
                    Assert.IsNotNull(createdKey.GetValue(name), "#A1");
                    createdKey.DeleteValue(name);
                    Assert.IsNull(createdKey.GetValue(name), "#A2");
                    ...
                    createdKey.SetValue(name, "value2", RegistryValueKind.String);
                    Assert.IsNotNull(createdKey.GetValue(name), "#B1");
                    createdKey.DeleteValue(name);
                    Assert.IsNull(createdKey.GetValue(name), "#B2");
        ...
        public void SetValue2_Name_Null()
        {
            ...
            try
            {
                createdKey.SetValue(null, "value1", RegistryValueKind.String);
                string[] names = createdKey.GetValueNames();
                Assert.IsNotNull(names, "#A1");
                Assert.AreEqual(1, names.Length, "#A2");
                Assert.IsNotNull(names[0], "#A3");
                ...
                createdKey.SetValue(string.Empty, "value2", RegistryValueKind.String);
                names = createdKey.GetValueNames();
                Assert.IsNotNull(names, "#B1");
                Assert.AreEqual(1, names.Length, "#B2");
                Assert.IsNotNull(names[0], "#B3");
        ...
        public void SetValue2_Value_Null()
        {
            ...
            try
            {
                try
                {
                    createdKey.SetValue("Name", null, RegistryValueKind.String);
                    Assert.Fail("#1");
                }
                catch (ArgumentNullException ex)
                {
        ...
        public void SubKeyCount()
        {
            ...
            try
            {
                ...
                using (RegistryKey createdKey = Registry.CurrentUser.OpenSubKey(subKeyName))
                {
                    ...
                    using (
                        RegistryKey createdKey2 = Registry.CurrentUser.OpenSubKey(subKeyName, true)
                    )
                    {
        ...
        public void bug79059()
        {
            ...
            using (RegistryKey softwareKey = Registry.CurrentUser.OpenSubKey("software", true))
            {
                try
                {
                    ...
                    {
                        using (
                            RegistryKey softwareKey2 = Registry.CurrentUser.OpenSubKey("software")
                        ) { }
                        createdKey.Close();
                        softwareKey.DeleteSubKeyTree(subKeyName);
                    }
