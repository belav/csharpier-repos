    ...
    public class CharTest
    {
        ...
        public void TestGetUnicodeCategory()
        {
            ...
            // TODO - ConnectorPunctuation
            {
                ...
                char c3 = (char)0x007F;
                char c4 = (char)0x00F;
                Assert.AreEqual(UnicodeCategory.Control, Char.GetUnicodeCategory(c1), "Control");
                Assert.AreEqual(UnicodeCategory.Control, Char.GetUnicodeCategory(c2), "Control");
                Assert.AreEqual(UnicodeCategory.Control, Char.GetUnicodeCategory(c3), "Control");
                Assert.AreEqual(UnicodeCategory.Control, Char.GetUnicodeCategory(c4), "Control");
            }
            {
        ...
        {
            Assert.AreEqual(
                Char.GetUnicodeCategory("\uD800\uDF80", 0),
                UnicodeCategory.OtherLetter,
                "#C01"
            );
            Assert.AreEqual(
                Char.GetUnicodeCategory("\uD800\uDF80", 1),
                UnicodeCategory.Surrogate,
                "#C02"
            );
            Assert.AreEqual(
        ...
        public void TestGetUnicodeCategoryAstralPlanes()
        {
            ...
            for (int codepoint = 0x10000; codepoint < up_to; codepoint += increment)
            {
                ...
                Assert.AreEqual(
                    Char.GetUnicodeCategory(combined[0]),
                    UnicodeCategory.Surrogate,
                    "#D02"
                );
                Assert.AreEqual(
                    Char.GetUnicodeCategory(combined[1]),
                    UnicodeCategory.Surrogate,
                    "#D03"
                );
                Assert.AreNotEqual(
