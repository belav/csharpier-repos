...
namespace MonoTests.System
{
    ...
    public class TimeZoneTest
    {
        ...
        {
            Assert.IsTrue(
                "CET" == t1.StandardName || "W. Europe Standard Time" == t1.StandardName,
                "A01"
            );
            Assert.IsTrue(
                "CEST" == t1.DaylightName || "W. Europe Daylight Time" == t1.DaylightName,
                "A02"
            );
            ...
            Assert.AreEqual(
                "03/31/2002 02:00:00",
                d1.Start.ToString("G", CultureInfo.InvariantCulture),
                "A03"
            );
            Assert.AreEqual(
                "10/27/2002 03:00:00",
                d1.End.ToString("G", CultureInfo.InvariantCulture),
                "A04"
            );
            ...
            Assert.AreEqual(
                "03/31/1996 02:00:00",
                d2.Start.ToString("G", CultureInfo.InvariantCulture),
                "A06"
            );
            Assert.AreEqual(
                "10/27/1996 03:00:00",
                d2.End.ToString("G", CultureInfo.InvariantCulture),
                "A07"
            );
        ...
        {
            Assert.IsTrue(
                "EST" == t1.StandardName || "Eastern Standard Time" == t1.StandardName,
                "B01"
            );
            Assert.IsTrue(
                "EDT" == t1.DaylightName || "Eastern Daylight Time" == t1.DaylightName,
                "B02"
            );
        ...
        private void NZST(TimeZone t1)
        {
            ...
            Assert.AreEqual(
                36000000000L /*hour*/
                    * 13L,
                t1.GetUtcOffset(d3).Ticks,
                "E12"
            );
            Assert.AreEqual(
                36000000000L /*hour*/
                    * 12L,
                t1.GetUtcOffset(d4).Ticks,
                "E13"
            );
            Assert.AreEqual(
                36000000000L /*hour*/
                    * 13L,
                t1.GetUtcOffset(d5).Ticks,
                "E14"
            );
            ...
            switch (t1.StandardName)
            {
                ...
                case "GMT":
                    ...
                    if (string.IsNullOrEmpty(t1.DaylightName))
                        Assert.Ignore(
                            "This test may fail due to: http://www.openradar.me/38174449. This value is also empty on recent Android versions."
                        );
#endif
                    GMT(t1);
                    break;
        ...
        public void ToLocalTimeAtDSTBoundaries()
        {
            ...
            Assert.IsTrue(
                tz.ToLocalTime(dst_start_utc.Subtract(new TimeSpan(0, 1, 0)))
                    < tz.ToLocalTime(dst_start_utc),
                "0:1:59 < 0:3:00"
            );
            Assert.IsTrue(
                tz.ToLocalTime(dst_start_utc)
                    < tz.ToLocalTime(dst_start_utc.Add(new TimeSpan(0, 1, 0))),
                "0:3:00 < 0:3:01"
            );
            Assert.IsTrue(
                tz.ToLocalTime(dst_start_utc.Add(new TimeSpan(0, 1, 0)))
                    < tz.ToLocalTime(dst_start_utc.Add(new TimeSpan(0, 59, 0))),
                "0:3:01 < 0:3:59"
            );
            Assert.IsTrue(
                tz.ToLocalTime(dst_start_utc.Add(new TimeSpan(0, 59, 0)))
                    < tz.ToLocalTime(dst_start_utc.Add(new TimeSpan(1, 0, 0))),
                "0:3:59 < 0:4:00"
            );
            Assert.IsTrue(
                tz.ToLocalTime(dst_start_utc.Add(new TimeSpan(1, 0, 0)))
                    < tz.ToLocalTime(dst_start_utc.Add(new TimeSpan(1, 1, 0))),
                "0:4:00 < 0:4:01"
            );
        }
        ...
        public void GetUTCNowAtDSTBoundaries()
        {
            ...
            TimeZoneInfo.AdjustmentRule adjustment = TimeZoneInfo.AdjustmentRule
            ...
            TimeZoneInfo tzInfo = TimeZoneInfo.CreateCustomTimeZone(
                "MY Standard Time",
                TimeSpan.Zero,
                "MST",
                "MST",
        ...
        public void GetUtcOffsetAtDSTBoundary()
        {
            ...
            if (dst_end == DateTime.MinValue)
                Assert.Ignore(
                    tz.StandardName + " did not observe daylight saving time during " + year + "."
                );
            ...
            Assert.AreEqual(
                dstOffset,
                tz.GetUtcOffset(
                    dst_end.Add(daylightChanges.Delta.Negate().Add(TimeSpan.FromSeconds(1)))
                )
            );
            Assert.AreEqual(
                dstOffset,
                tz.GetUtcOffset(dst_end.Add(daylightChanges.Delta.Negate()))
            );
        }
        ...
        public void FindSystemTimeZoneById()
        {
            TimeZoneInfo tzi = TimeZoneInfo.FindSystemTimeZoneById(
                TimeZoneInfoTest.MapTimeZoneId("America/Toronto")
            );
            Assert.IsTrue(
                "EDT" == tzi.DaylightName || "Eastern Daylight Time" == tzi.DaylightName,
                "DaylightName"
            );
            Assert.IsTrue(
                "EST" == tzi.StandardName || "Eastern Standard Time" == tzi.StandardName,
                "StandardName"
            );
