...
namespace MonoTests.System
{
	...
	private string[] Results2 = {NumberFormatInfo.CurrentInfo.CurrencySymbol+"127.00000",
        ...
        {
            "(" + NumberFormatInfo.InvariantInfo.CurrencySymbol + "128.00)",
            "-128",
            "-1.280000e+002",
            "-128.00",
            "-128",
        ...
        {
            NumberFormatInfo.InvariantInfo.CurrencySymbol + "127.00000",
            "00127",
            "1.27000e+002",
            "127.00000",
            "127",
        ...
        public void TestToString()
        {
            //test ToString()
            Assert.IsTrue(
                String.Compare(MyString1, MySByte1.ToString()) == 0,
                "MyString1, MySByte1.ToString()"
            );
            Assert.IsTrue(
                String.Compare(MyString2, MySByte2.ToString()) == 0,
                "MyString2, MySByte2.ToString()"
            );
            Assert.IsTrue(
                String.Compare(MyString3, MySByte3.ToString()) == 0,
                "MyString3, MySByte3.ToString()"
            );
		...
		for (int i=0; i < Formats1.Length; i++) {
            ...
            {
                Assert.IsTrue(
                    String.Compare(ResultsNfi1[i], MySByte2.ToString(Formats1[i], Nfi)) == 0,
                    "i="
                        + i
                        + ", ResultsNfi1[i]="
                        + ResultsNfi1[i]
                        + ", MySByte2.ToString(Formats1[i]="
                        + Formats1[i]
                        + "): Expected "
                        + ResultsNfi1[i]
                        + " but got "
                        + MySByte2.ToString(Formats1[i], Nfi)
                );
                Assert.IsTrue(
                    String.Compare(ResultsNfi2[i], MySByte3.ToString(Formats2[i], Nfi)) == 0,
                    "ResultsNfi2[i], MySByte3.ToString(Formats2[i], Nfi):"
                        + ResultsNfi2[i]
                        + "<==>"
                        + MySByte3.ToString(Formats2[i], Nfi)
                );
            }
            try
            {
        ...
        public void ToString_Defaults()
        {
            ...
            Assert.AreEqual(
                def,
                i.ToString((IFormatProvider)null),
