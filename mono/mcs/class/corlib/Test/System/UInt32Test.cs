...
namespace MonoTests.System
{
    ...
    public class UInt32Test
    {
        ...
        {
            NumberFormatInfo.InvariantInfo.CurrencySymbol + "0.00",
            "0",
            "0.000000e+000",
            "0.00",
            "0",
        ...
        {
            NumberFormatInfo.InvariantInfo.CurrencySymbol + "4,294,967,295.00000",
            "4294967295",
            "4.29497e+009",
            "4294967295.00000",
            "4.295e+09",
        ...
        public void SetUp()
        {
            ...
            string perPattern = new string[] { "n %", "n%", "%n" }[
                NumberFormatInfo.CurrentInfo.PercentPositivePattern
            ];
        ...
        public void TestParse()
        {
            //test Parse(string s)
            Assert.IsTrue(
                MyUInt32_1 == UInt32.Parse(MyString1),
                "Parse problem on \"" + MyString1 + "\""
            );
            Assert.IsTrue(
                MyUInt32_2 == UInt32.Parse(MyString2),
                "Parse problem on \"" + MyString2 + "\""
            );
            Assert.IsTrue(
                MyUInt32_3 == UInt32.Parse(MyString3),
                "Parse problem on \"" + MyString3 + "\""
            );
            try
            {
                UInt32.Parse(null);
            ...
            catch (Exception e)
            {
                Assert.IsTrue(
                    ...
                    "Did not get ArgumentNullException type"
                );
            }
            try
            {
            ...
            catch (Exception e)
            {
                Assert.IsTrue(
                    ...
                    "Did not get FormatException type"
                );
            }
            try
            {
            ...
            catch (Exception e)
            {
                Assert.IsTrue(
                    ...
                    "Did not get OverflowException type on '"
                        + "4294967296"
                        + "'. Instead, got: '"
                        + e.GetType()
                        + "'"
        ...
        public void TestParseExponent()
        {
            ...
            Assert.AreEqual(
                21,
                uint.Parse("2.1E1", NumberStyles.AllowDecimalPoint | NumberStyles.AllowExponent),
                "A#7"
            );
            Assert.AreEqual(
                520,
                uint.Parse(".52E3", NumberStyles.AllowDecimalPoint | NumberStyles.AllowExponent),
                "A#8"
            );
            Assert.AreEqual(
                32500000,
                uint.Parse("32.5E6", NumberStyles.AllowDecimalPoint | NumberStyles.AllowExponent),
                "A#9"
            );
            Assert.AreEqual(
                890,
                uint.Parse("8.9000E2", NumberStyles.AllowDecimalPoint | NumberStyles.AllowExponent),
                "A#10"
            );
        ...
        public void TestTryParse()
        {
            ...
            Assert.AreEqual(
                false,
                UInt32.TryParse("100000000", NumberStyles.HexNumber, Nfi, out result)
            );
            Assert.AreEqual(false, UInt32.TryParse("10000000000", out result));
            Assert.AreEqual(false, UInt32.TryParse("-10000000000", out result));
            Assert.AreEqual(
                true,
                UInt32.TryParse("7fffffff", NumberStyles.HexNumber, Nfi, out result)
            );
            Assert.AreEqual(Int32.MaxValue, result);
            Assert.AreEqual(
                true,
                UInt32.TryParse("80000000", NumberStyles.HexNumber, Nfi, out result)
            );
            Assert.AreEqual(Int32.MaxValue + (uint)1, result);
            Assert.AreEqual(
                true,
                UInt32.TryParse("ffffffff", NumberStyles.HexNumber, Nfi, out result)
            );
            ...
            Assert.IsFalse(
                uint.TryParse(
                    Nfi.CurrencySymbol + "-",
                    NumberStyles.AllowLeadingSign | NumberStyles.AllowCurrencySymbol,
                    Nfi,
                    out result
                )
            );
        ...
        public void TestToString()
        {
            ...
            {
                Assert.Fail(
                    "TestToString: Assert.Failed on TestNumber="
                        + TestNumber
                        + " with exception: "
                        + e.ToString()
                );
            ...
            for (int i = 0; i < Formats1.Length; i++)
            {
                ...
                {
                    Assert.Fail(
                        "TestToString: MyUInt32_2.ToString(Formats1[i]) i="
                            + i
                            + ". e = "
                            + e.ToString()
                    );
                ...
                {
                    Assert.Fail(
                        "TestToString: MyUInt32_3.ToString(Formats2[i]) i="
                            + i
                            + ". e = "
                            + e.ToString()
                    );
