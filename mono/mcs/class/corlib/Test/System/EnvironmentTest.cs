    ...
    public class EnvironmentTest
    {
        private void ExpandEquals(string toExpand, string toMatch)
        {
            ...
            Assert.AreEqual(
                toMatch,
                expanded,
                "ExpandEnvironmentVariables(" + toExpand + ").Match"
            );
        }
        ...
        private void ExpandStartsEnds(string toExpand, string start, string end)
        {
            string expanded = Environment.ExpandEnvironmentVariables(toExpand);
            Assert.IsTrue(
                expanded.StartsWith(start),
                "ExpandEnvironmentVariables(" + toExpand + ").Start"
            );
            Assert.IsTrue(
                expanded.EndsWith(end),
                "ExpandEnvironmentVariables(" + toExpand + ").End"
            );
        }
        ...
        private void ExpandDifferent(string toExpand)
        {
            ...
            Assert.IsFalse(
                (toExpand == expanded),
                "ExpandEnvironmentVariables(" + toExpand + ").Different"
            );
        }
        ...
        public void SetEnvironmentVariable_Target_Invalid()
        {
            Environment.SetEnvironmentVariable(
                "MONO",
                "GO",
                (EnvironmentVariableTarget)Int32.MinValue
            );
        }
