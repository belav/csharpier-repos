    ...
    public class DirectoryTest
    {
        ...
        public void EnumerateFilesListSymlinks()
        {
            ...
            var files3 = new List<string>(
                Directory.EnumerateFileSystemEntries(TempFolder, "*.src")
            ).ToArray();
            Array.Sort(files3);
        ...
        */
        ...
        public void Delete()
        {
            ...
            try
            {
                ...
                Assert.IsFalse(
                    Directory.Exists(path + DSC + "DirectoryTest.Test.Delete.1.2"),
                    "#3"
                );
        ...
        public void GetFiles_SubDirInPattern()
        {
            ...
            Directory.CreateDirectory(
                $"{DirPath}{Path.DirectorySeparatorChar}something{Path.DirectorySeparatorChar}else"
            );
            File.WriteAllText(
                $"{DirPath}{Path.DirectorySeparatorChar}something{Path.DirectorySeparatorChar}else{Path.DirectorySeparatorChar}file",
                "hello"
            );

            var r = Directory.GetFiles(
                DirPath,
                $"something{Path.DirectorySeparatorChar}else{Path.DirectorySeparatorChar}*",
