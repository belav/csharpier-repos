...
namespace MonoTests.System.IO
{
    ...
    public class MemoryStreamTest
    {
        ...
        class ExceptionalStream : MemoryStream
        {
            ...
            public ExceptionalStream(byte[] buffer, bool writable)
        ...
        public void BeginReadIsBlockingNextRead()
        {
            ...
            {
                using (
                    var testStream = new SignaledMemoryStream(testStreamData, begin_read_unblock)
                )
                {
                    IAsyncResult begin_read_1_ares = testStream.BeginRead(
                        readBytes,
                        0,
                        5,
                        null,
        ...
        public void BeginRead_BeginWrite()
        {
            ...
            {
                using (
                    MemoryStream stream = new SignaledMemoryStream(
                        testStreamData,
                        begin_read_unblock
                    )
                )
        ...
        public void TestAsyncArgumentExceptions()
        {
            ...
            using (var stream = new ExceptionalStream())
            {
                ...
                Assert.IsTrue(
                    ...
                    {
                        stream.WriteAsync(buffer, 0, 1000);
                    }),
                    "#2"
                );
                Assert.IsTrue(
                    Throws<ArgumentException>(() =>
                    {
                        stream.ReadAsync(buffer, 0, 1000);
                    }),
                    "#3"
                );
                Assert.IsTrue(
                    Throws<ArgumentException>(() =>
                    {
                        stream.WriteAsync(buffer, 0, 1000, new CancellationToken(true));
                    }),
                    "#4"
                );
                Assert.IsTrue(
                    Throws<ArgumentException>(() =>
                    {
                        stream.ReadAsync(buffer, 0, 1000, new CancellationToken(true));
                    }),
                    "#5"
                );
                Assert.IsTrue(
                    Throws<ArgumentException>(() =>
                    {
                        stream.WriteAsync(null, 0, buffer.Length, new CancellationToken(true));
                    }),
                    "#6"
                );
                Assert.IsTrue(
                    Throws<ArgumentException>(() =>
                    {
                        stream.ReadAsync(null, 0, buffer.Length, new CancellationToken(true));
                    }),
                    "#7"
                );
                Assert.IsTrue(
                    Throws<ArgumentException>(() =>
                    {
                        stream.WriteAsync(buffer, 1000, buffer.Length, new CancellationToken(true));
                    }),
                    "#8"
                );
                Assert.IsTrue(
                    Throws<ArgumentException>(() =>
                    {
                        stream.ReadAsync(buffer, 1000, buffer.Length, new CancellationToken(true));
                    }),
                    "#9"
                );
        ...
        public void WritableOverride()
        {
            ...
            Assert.IsTrue(
                ...
                {
                    stream.Write(null, 0, buffer.Length);
                }),
                "#1.1"
            );
            stream.Close();
            ...
            Assert.IsFalse(
                Throws<Exception>(() =>
                {
                    estream.Write(buffer, 0, buffer.Length);
                }),
                "#4"
            );
