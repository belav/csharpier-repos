...
namespace MonoTests.System.IO
{
    ...
    public class BinaryReaderTest
    {
        ...
        public void Ctor1()
        {
            ...
            try
            {
                f = new FileStream(_codeFileName, FileMode.Open, FileAccess.Read);
                r = new BinaryReader(f);
                ...
                for (i = 0; i < 7; i++)
                {
                    try
                    {
                        ...
                        Assert.AreEqual(
                            arr_int[i],
                            arr_int2[i],
                            "#2E Wrong Readed Int32 in iteration " + i
                        );
                    }
                    catch (IOException e)
                    {
        ...
        public void ReadDecimal()
        {
            ...
            try
            {
                stream = new MemoryStream(
                    new byte[] { 0, 0, 0, 0, 0, 0, 65, 0, 0, 0, 0, 0, 0, 0, 0, 128, 0, 0, 0, 0, 0 }
                );
                reader = new BinaryReader(stream);
        ...
        public void ReadDecimalException()
        {
            MemoryStream stream = new MemoryStream(
                new byte[] { 0, 0, 0, 0, 0, 0, 65, 0, 0, 0, 0, 0, 0, 0, 0, 87, 98, 0, 0, 0, 0, 0 }
            );
        ...
        {
            MemoryStream stream = new MemoryStream(
                new byte[] { 0, 0, 0, 0, 0, 0, 65, 0, 0, 0, 0, 0, 0, 0, 0, 87, 98, 0, 0, 0, 0 }
            );
            BinaryReader reader = new BinaryReader(stream);
            try
            {
        ...
        {
            MemoryStream stream = new MemoryStream(
                new byte[] { 65, 1, 32, 43, 5, 3, 54, 0, 34, 5, 7, 4, 23, 4, 76, 34, 76, 2, 6, 45 }
            );
            BinaryReader reader = new BinaryReader(stream);
            try
            {
