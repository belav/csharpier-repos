    ...
    public class FileTest
    {
        ...
        public void TestExists()
        {
            ...
            try
            {
                ...
                Assert.IsFalse(
                    File.Exists(tmpFolder + Path.DirectorySeparatorChar + "doesnotexist"),
                    "#5"
                );
            }
            finally
        ...
        public void Delete_File_ReadOnly()
        {
            if (RunningOnUnix)
                Assert.Ignore(
                    "ReadOnly files can be deleted on unix since fdef50957f508627928c7876a905d5584da45748."
                );
        ...
        public void Open_CreateNewMode_ReadAccess()
        {
            ...
            {
                stream = File.Open(
                    tmpFolder + Path.DirectorySeparatorChar + "AFile.txt",
                    FileMode.CreateNew,
                    FileAccess.Read
                );
                Assert.Fail("#1");
        ...
        public void LastWriteTimeSubMs()
        {
            ...
            try
            {
                ...
                for (var i = 0; i < 200; i++)
                {
                    ...
                    // sanity check
                    Assert.IsTrue(
                        diff.TotalSeconds >= 0 && diff.TotalSeconds < 2.0,
                        $"Iteration #{i} failed, diff.TotalSeconds: {diff.TotalSeconds}, untouched: {untouched.ToString(fmt)}, now: {now.ToString(fmt)}"
                    );
                    ...
                    Assert.IsTrue(
                        touched >= untouched,
                        $"Iteration #{i} failed, untouched: {untouched.ToString(fmt)} touched: {touched.ToString(fmt)}"
                    );
                }
            }
            finally
        ...
        public void LastWriteTimeSubMsCopy()
        {
            ...
            if (
                Version.TryParse(
                    Environment.GetEnvironmentVariable("SIMULATOR_RUNTIME_VERSION"),
                    out Version simulatorVersion
                )
                && simulatorVersion.Major < 11
                && new DriveInfo("/").DriveFormat == "apfs"
            )
                Assert.Inconclusive(
                    "This test doesn't work on old iOS Simulator versions running on newer macOS with APFS."
                );
#endif
            string path = tmpFolder + Path.DirectorySeparatorChar + "lastWriteTimeSubMs";
            if (File.Exists(path))
            ...
            try
            {
                ...
                for (var i = 0; i < 200; i++)
                {
                    ...
                    Assert.IsTrue(
                        copy_touched >= untouched,
                        $"Iteration #{i} failed, untouched: {untouched.ToString(fmt)} copy_touched: {copy_touched.ToString(fmt)}"
                    );
                    File.Delete(path_copy);
                }
            }
        ...
        {
            string path =
                tmpFolder + Path.DirectorySeparatorChar + "GetLastAccessTimeUtcException3";
            DeleteFile(path);
            ...
            if (Path.InvalidPathChars.Length > 1)
            {
                try
                {
                    File.SetCreationTime(
                        Path.InvalidPathChars[1].ToString(),
        ...
        {
            string path =
                tmpFolder + Path.DirectorySeparatorChar + "SetCreationTimeFileNotFoundException1";
            DeleteFile(path);
        ...
        {
            string path =
                tmpFolder + Path.DirectorySeparatorChar + "SetLastAccessTimeFileNotFoundException1";
            DeleteFile(path);
        ...
        {
            string path =
                tmpFolder + Path.DirectorySeparatorChar + "SetLastWriteTimeFileNotFoundException1";
            DeleteFile(path);
        ...
        public void SetLastWriteTimeUtc_Path_DoesNotExist()
        {
            string path =
                tmpFolder
                + Path.DirectorySeparatorChar
                + "SetLastWriteTimeUtcFileNotFoundException1";
            DeleteFile(path);
        ...
        public void SymLinkStats()
        {
            ...
            try
            {
                ...
                Assert.IsTrue(
                    File.Exists(path1),
                    "File.Exists must return true for path1 symlink stats"
                );
                Assert.IsTrue(
                    File.Exists(path2),
                    "File.Exists must return true for path2 symlink stats"
                );
                ...
                try
                {
                    ...
                    Assert.AreNotEqual(
                        File.GetLastWriteTime(path1),
                        File.GetLastWriteTime(path2),
                        "Path1 and Path2 should not have the same last write times"
                    );
                    Assert.AreNotEqual(
                        File.GetCreationTime(path1),
                        File.GetCreationTime(path2),
                        "Path1 and Path2 should not have the same creation times"
                    );
                    Assert.AreNotEqual(
                        File.GetLastAccessTime(path1),
                        File.GetLastAccessTime(path2),
                        "Path1 and Path2 should not have the same last access times"
                    );
                }
                catch (IOException e)
                {
                ...
                Assert.IsTrue(
                    File.Exists(path1),
                    "Original file must still exist for symlink stats"
                );
                Assert.IsFalse(
                    File.Exists(path2),
                    "File.Delete must delete symlink only for symlink stats"
                );
        ...
        public void SymLinkLoop()
        {
            ...
            try
            {
                ...
                Assert.IsTrue(
                    File.Exists(path1),
                    "File.Exists must return true for path1 symlink loop"
                );
                Assert.IsTrue(
                    File.Exists(path2),
                    "File.Exists must return true for path2 symlink loop"
                );
