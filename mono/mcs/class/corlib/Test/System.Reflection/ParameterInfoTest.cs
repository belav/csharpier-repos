...
namespace MonoTests.System.Reflection
{
    ...
    public class ParameterInfoTest
    {
        ...
        {
            MethodInfo mi = typeof(object).GetMethod("Equals", new Type[1] { typeof(object) });
        ...
        {
            ParameterInfo[] info = typeof(ParameterInfoTest)
                .GetMethod("paramMethod")
                .GetParameters();
        ...
        public static void Sample2(
            [DecimalConstantAttribute(2, 2, 2, 2, 2)] decimal a,
            [DateTimeConstantAttribute(123456)] DateTime b
        ) { }
        ...
        public void PseudoCustomAttributes()
        {
            ...
            Assert.AreEqual(
                1,
                info[1].GetCustomAttributes(typeof(InAttribute), true).Length,
                "#A2"
            );
            Assert.AreEqual(
                1,
                info[2].GetCustomAttributes(typeof(OutAttribute), true).Length,
                "#A3"
            );
            Assert.AreEqual(
                1,
                info[3].GetCustomAttributes(typeof(OptionalAttribute), true).Length,
                "#A4"
            );
            Assert.AreEqual(2, info[4].GetCustomAttributes(true).Length, "#A5");

            ParameterInfo[] pi = typeof(ParameterInfoTest)
                .GetMethod("marshalAsMethod")
                .GetParameters();
        ...
        {
            var metInfo = GetType()
                .GetMethod("TestCustomAttribute_Method", new Type[] { typeof(string) });
        ...
        public class TestNestedClass
        {
            public static void TestMethod(
                ...
                TestNestedClass a2,
                List<int> a3,
                List<StringBuilder> a4,
                List<TestNestedClass> a5,
                StringBuilder a6,
                Dictionary<int, string> a7,
                out int a8,
                ref int a9,
                out TestNestedClass a10,
                out List<int> a11
            )
            {
                a8 = 0;
        ...
        public void RawDefaultValue_Nullable()
        {
            ...
            Assert.AreEqual(
                new DecimalConstantAttribute(1, 2, 3, 4, 5).Value,
                type.GetMethod("M2").GetParameters()[0].RawDefaultValue
            );
            Assert.AreEqual(null, type.GetMethod("M3").GetParameters()[0].RawDefaultValue);
            Assert.AreEqual(
                new DateTime(1),
                type.GetMethod("M4").GetParameters()[0].RawDefaultValue
            );
        ...
        public void CustomModifiersOrder()
        {
            ...
            Assert.AreEqual(
                new[] { typeof(A), typeof(A), typeof(B), typeof(A) },
                requiredCustomModifiers
            );
            var optionalCustomModifiers = parameter.GetOptionalCustomModifiers();
            Assert.AreEqual(
                new[] { typeof(C), typeof(C), typeof(D), typeof(C) },
