...
namespace MonoTests.System.Text
{
    ...
    public class UTF8EncodingTest
    {
        ...
        public void TestMaxCharCount()
        {
            ...
            Assert.AreEqual(
                UTF8enc.GetMaxByteCount(50),
                UTF8encWithBOM.GetMaxByteCount(50),
                "UTF #2"
            );
        }
        ...
        public void T1_Correct_GreekWord_kosme()
        {
            ...
            Assert.AreEqual(
                ...
                BitConverter.ToString(utf8.GetBytes(s)),
                "Reconverted"
            );
        }
        ...
        {
            Encoding e = Encoding.GetEncoding(65001).Clone() as Encoding;
        ...
        {
            new UTF8Encoding(false, true).GetString(new byte[] { 0xED, 0xA2, 0x8C });
        }
        ...
        public void DecoderFallbackSimple()
        {
            ...
            AssertType.AreEqual(
                1,
                e.GetDecoder().GetCharCount(new byte[] { (byte)183 }, 0, 1),
                "#1"
            );
            AssertType.AreEqual(
                1,
                e.GetDecoder().GetChars(new byte[] { (byte)183 }, 0, 1, new char[100], 0),
                "#2"
            );
            AssertType.AreEqual(1, e.GetString(new byte[] { (byte)183 }).Length, "#3");
        }
        ...
        public void Bug415628()
        {
            using (
                var f = File.Open(
                    TestResourceHelper.GetFullPathOfResource("Test/resources/415628.bin"),
                    FileMode.Open
                )
            )
            {
        ...
        public void EncodingFallback()
        {
            ...
            for (byte b = 0xF0; b <= 0xF3; b++)
            {
                ...
                t.TestDecoderFallback(
                    data,
                    "??",
                    new byte[] { b, 0xB0, 0xB0 },
                    new byte[] { 0xC0 }
                );
            }
