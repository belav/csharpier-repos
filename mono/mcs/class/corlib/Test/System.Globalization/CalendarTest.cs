...
namespace MonoTests.System.Globalization
{
    sealed class Date
    {
        ...
        public int Era
        {
            ...
            {
                if (value < 1 || value > 10)
                    throw new ArgumentOutOfRangeException("Era", "Era must be between 1 and 10.");
                _era = value;
            }
        }
    ...
    public class CalendarTest
    {
        ...
        protected void SetUp()
        {
            ...
            acal = new Calendar[] { gcal, jucal, hical, hecal, jacal, tacal, kcal, tbcal };
        ...
        private void RowCheck(params Date[] adate)
        {
            if (adate.Length != acal.Length)
                throw new ArgumentException(
                    "Number of Date arguments doesn't match " + "length of calendar array."
                );
            ...
            for (int i = 0; i < acal.Length; i++)
            {
                ...
                StringWriter sw = new StringWriter();
                sw.Write("Calendar {0} computes wrong DateTime.", cal);
        ...
        {
            GregorianCalendar gc = new GregorianCalendar(GregorianCalendarTypes.Arabic);
            Assert.AreEqual(
        ...
        {
            Assert.AreEqual(0, GregorianCalendar.CurrentEra, "C01 GregorianCalendar.CurrentEra");
            Assert.AreEqual(0, HebrewCalendar.CurrentEra, "C02 HebrewCalendar.CurrentEra");
            Assert.AreEqual(0, HijriCalendar.CurrentEra, "C03 HijriCalendar.CurrentEra");
            Assert.AreEqual(0, JapaneseCalendar.CurrentEra, "C04 JapaneseCalendar.CurrentEra");
            Assert.AreEqual(0, JulianCalendar.CurrentEra, "C05 JulianCalendar.CurrentEra");
            Assert.AreEqual(0, KoreanCalendar.CurrentEra, "C06 KoreanCalendar.CurrentEra");
            Assert.AreEqual(0, TaiwanCalendar.CurrentEra, "C07 TaiwanCalendar.CurrentEra");
            Assert.AreEqual(
                0,
        ...
        public void TestErasProperty()
        {
            foreach (Calendar cal in acal)
            {
                ...
                if (cal is JapaneseCalendar)
                    check = 5;
                Assert.AreEqual(check, cal.Eras.Length, String.Format("D01 {0}.Eras.Length", cal));
                cal.Eras[0] = 29;
        ...
        {
            Assert.AreEqual(2029, gcal.TwoDigitYearMax, "E01 TwoDigitYearMax GregorianCalendar");
            Assert.AreEqual(5790, hecal.TwoDigitYearMax, "E02 TwoDigitYearMax HebrewCalendar");
            Assert.AreEqual(1451, hical.TwoDigitYearMax, "E03 TwoDigitYearMax HijriCalendar");
            Assert.AreEqual(99, jacal.TwoDigitYearMax, "E04 TwoDigitYearMax JapaneseCalendar");
            Assert.AreEqual(2029, jucal.TwoDigitYearMax, "E05 TwoDigitYearMax JulianCalendar");
            Assert.AreEqual(4362, kcal.TwoDigitYearMax, "E06 TwoDigitYearMax KoreanCalendar");
            Assert.AreEqual(99, tacal.TwoDigitYearMax, "E07 TwoDigitYearMax TaiwanCalendar");
            Assert.AreEqual(
                2572,
                tbcal.TwoDigitYearMax,
            ...
            foreach (Calendar cal in acal)
            {
                ...
                Assert.IsFalse(
                    exception,
                    String.Format("E09 {0}.TwoDigitYearMax 99 " + " out of range exception", cal)
                );
                ...
                Assert.IsTrue(
                    exception,
                    String.Format(
                        "E10 {0}.TwoDigitYearMax out " + " of range exception value {1}",
                        cal,
                        m
                    )
                );
        ...
        public void TestToFourDigitYear()
        {
            foreach (Calendar cal in acal)
            {
                ...
                Assert.AreEqual(
                    ...
                    String.Format("F03 {0}.ToFourDigitYear(30)", cal)
                );
                Assert.AreEqual(
                    mod,
                    cal.ToFourDigitYear(mod),
                    String.Format("F04 {0}.ToFourDigitYear({1})", cal, mod)
                );
                ...
                Assert.IsTrue(
                    exception,
                    String.Format("F05 {0}.ToFourDigitYear(-1) exception", cal)
                );
                exception = false;
                try
                {
                ...
                Assert.IsTrue(
                    exception,
                    String.Format("F05 {0}.ToFourDigitYear(15000) exception", cal)
                );
            }
        }
...
*/
        ...
        public void TestDaysInYear(Calendar calendar, int year)
        {
            ...
            Assert.AreEqual(
                daysInYear,
                daysInMonths,
                string.Format("Calendar:{0} Year:{1}", calendar.GetType(), year)
            );
        }
        ...
        {
            var calendars = new List<Calendar>(acal) { new UmAlQuraCalendar() };
