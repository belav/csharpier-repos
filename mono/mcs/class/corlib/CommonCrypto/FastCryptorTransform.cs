    ...
    unsafe class FastCryptorTransform : ICryptoTransform
    {
        ...
        )
        {
            ...
            else if (iv.Length < BlockSizeByte)
            {
                string msg = String.Format(
                    "IV is too small ({0} bytes), it should be {1} bytes long.",
                    iv.Length,
                    BlockSizeByte
                );
        ...
        int Transform(byte[] input, int inputOffset, byte[] output, int outputOffset, int length)
        {
            ...
            fixed (byte* outputBuffer = &output[0])
            {
                CCCryptorStatus s = Cryptor.CCCryptorUpdate(
                    handle,
                    (IntPtr)(inputBuffer + inputOffset),
                    in_len,
                    (IntPtr)(outputBuffer + outputOffset),
                    out_len,
                    ref len
                );
        ...
        )
        {
            ...
            if (
                !encrypt
                && (0 > len)
                && ((padding == PaddingMode.None) || (padding == PaddingMode.Zeros))
            )
            {
        ...
        private bool KeepLastBlock
        {
            ...
            {
                return (
                    (!encrypt) && (padding != PaddingMode.None) && (padding != PaddingMode.Zeros)
                );
            }
        }
        ...
        private byte[] FinalEncrypt(byte[] inputBuffer, int inputOffset, int inputCount)
        {
            ...
            {
                outputOffset = InternalTransformBlock(
                    inputBuffer,
                    inputOffset,
                    total - BlockSizeByte,
                    res,
