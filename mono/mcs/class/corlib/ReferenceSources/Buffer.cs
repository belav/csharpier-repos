...
namespace System
{
    partial class Buffer
    {
        ...
        public static unsafe byte GetByte(Array array, int index)
        {
            ...
            return *(byte*)(
                Unsafe.AsPointer<byte>(ref Unsafe.Add<byte>(ref array.GetRawSzArrayData(), index))
            );
        }
        ...
        internal static void Memmove<T>(ref T destination, ref T source, nuint elementCount)
        {
            if (!RuntimeHelpers.IsReferenceOrContainsReferences<T>())
            {
                ...
                {
                    fixed (
                        byte* pDestination = &Unsafe.As<T, byte>(ref destination),
                            pSource = &Unsafe.As<T, byte>(ref source)
                    )
                        Memmove(
                            pDestination,
                            pSource,
                            (uint)elementCount * (uint)Unsafe.SizeOf<T>()
                        );
                }
            }
            else
            {
                unsafe
                {
                    fixed (
                        ...
                        RuntimeImports.Memmove_wbarrier(
                            pDestination,
                            pSource,
                            (uint)elementCount,
