...
namespace System
{
    ...
    sealed class AndroidTzData : IAndroidTimeZoneDB
    {
            ...
            where T : struct
        {
            ...
            if ((r = data.Read(buffer, 0, size)) < size)
                throw new InvalidOperationException(
                    string.Format(
                        "Error reading '{0}': read {1} bytes, expected {2}",
                        tzdataPath,
                        r,
                        size
                    )
        ...
        unsafe void ReadZoneTab(int zoneTabOffset, int zoneTabSize)
        {
            ...
            if ((r = data.Read(zoneTab, 0, zoneTab.Length)) < zoneTab.Length)
                throw new InvalidOperationException(
                    string.Format(
                        "Error reading zonetab: read {0} bytes, expected {1}",
                        r,
                        zoneTabSize
                    )
                );
    ...
    partial class TimeZoneInfo
    {
        ...
        sealed class ZoneInfoDB : IAndroidTimeZoneDB
        {
            ...
            internal static readonly string ZoneDirectoryName =
                Environment.GetEnvironmentVariable("ANDROID_ROOT") + "/usr/share/zoneinfo/";
            ...
            public ZoneInfoDB(string zoneInfoDB = null)
            {
                ...
                {
                    ReadDatabase(
                        Path.Combine(zoneRoot, IndexFileName),
                        out names,
                        out starts,
                        out lengths,
                        out offsets
            ...
            )
            {
                using (var file = File.OpenRead(path))
                {
                    ...
                    int numEntries = (int)(
                        file.Length / (TimeZoneNameLength + 3 * TimeZoneIntSize)
                    );
            ...
            static void Fill(Stream stream, byte[] nbuf, int required)
            {
                int read = 0,
                    offset = 0;
                while (
                    offset < required && (read = stream.Read(nbuf, offset, required - offset)) > 0
                )
                    offset += read;
                if (read != required)
                    throw new EndOfStreamException(
                        "Needed to read " + required + " bytes; read " + read + " bytes"
                    );
            }
        ...
        static class AndroidTimeZones
        {
            ...
            {
                return db == null ? new string[0] : db.GetAvailableIds();
            }
            ...
            static string GetDefaultTimeZoneName()
            {
                ...
                int n = 0;
                string defaultTimeZone = Environment.GetEnvironmentVariable(
                    "__XA_OVERRIDE_TIMEZONE_ID__"
                );
                ...
                // Used by the tests
                if (
                    Environment.GetEnvironmentVariable("__XA_USE_JAVA_DEFAULT_TIMEZONE_ID__")
                    == null
                )
