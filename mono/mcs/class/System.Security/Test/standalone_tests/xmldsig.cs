...
public class MyClass
{
    ...
    public static void Main(string[] args)
    {
        ...
        try
        {
            ...
            exc14n = (
                CryptoConfig.CreateFromName("http://www.w3.org/2001/10/xml-exc-c14n#WithComments")
                != null
            );
    ...
    static TextReader GetReader(string filename)
    {
        ...
        Stream stream =
            resolver.GetEntity(resolver.ResolveUri(null, filename), null, typeof(Stream)) as Stream;
        if (useDecentReader)
            return new XmlSignatureStreamReader(new StreamReader(stream));
        else
...
DumpSignedXml (s);
            ...
            foreach (KeyInfoClause kic in s.KeyInfo)
            {
                if (kic is KeyInfoX509Data)
                {
                    ...
                    {
                        System.Security.Cryptography.X509Certificates.X509Certificate x509 =
    ...
    static void Merlin()
    {
        ...
        foreach (
            FileInfo fi in new DirectoryInfo("merlin-xmldsig-twenty-three").GetFiles(
                "signature-*.xml"
            )
        )
        {
    ...
    static void Phaos()
    {
        ...
        foreach (FileInfo fi in new DirectoryInfo(".").GetFiles("signature-*.xml"))
        {
            ...
            {
                Console.WriteLine(
                    "NOT RUN: " + fi.Name + " : System.Security.dll doesn't support HMAC-MD5."
                );
                skip++;
                continue;
            }
