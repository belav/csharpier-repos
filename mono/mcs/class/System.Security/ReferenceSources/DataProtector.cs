    ...
    public abstract class DataProtector
    {
        ...
        )
        {
            ...
            if (String.IsNullOrWhiteSpace(applicationName))
                throw new ArgumentException(
                    Crypto.SR.Cryptography_DataProtector_InvalidAppNameOrPurpose,
                    nameof(applicationName)
                );
            if (String.IsNullOrWhiteSpace(primaryPurpose))
                throw new ArgumentException(
                    Crypto.SR.Cryptography_DataProtector_InvalidAppNameOrPurpose,
                    nameof(primaryPurpose)
                );
        ...
        protected virtual byte[] GetHashedPurpose()
        {
            if (m_hashedPurpose == null)
            {
                ...
                using (
                    HashAlgorithm sha256 = HashAlgorithm.Create(
        ...
        public byte[] Protect(byte[] userData)
        {
            ...
            if (PrependHashedPurposeToPlaintext)
            {
                ...
                Array.Copy(
                    userData,
                    0,
                    userDataWithHashedPurpose,
                    hashedPurpose.Length,
                    userData.Length
                );
        ...
        public byte[] Unprotect(byte[] encryptedData)
        {
            ...
            if (PrependHashedPurposeToPlaintext)
            {
                ...
                // Don't touch this code block without crypto board review
                {
                    ...
                    bool hashedPurposeOK =
                        plainTextWithHashedPurpose.Length >= hashedPurpose.Length;
                    for (int i = 0; i < hashedPurpose.Length; i++)
                    {
                        ...
                        if (
                            hashedPurpose[i]
                            != plainTextWithHashedPurpose[i % plainTextWithHashedPurpose.Length]
                        )
                        {
                    ...
                    if (!hashedPurposeOK)
                    {
                        throw new CryptographicException(
                            Crypto.SR.Cryptography_DataProtector_InvalidPurpose
                        );
                    }
                }
