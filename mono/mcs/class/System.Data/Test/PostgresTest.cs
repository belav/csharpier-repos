...
namespace Test.Mono.Data.PostgreSqlClient
{
    class PostgresTest
    {
        ...
        static void DropTable(IDbConnection cnc)
        {
            ...
            dropCommand.CommandText = "drop table mono_postgres_test";
        ...
        static object CallStoredProcedure(IDbConnection cnc)
        {
            ...
            callStoredProcCommand.CommandText = "version";
        ...
        static IDataReader SelectDataUsingCommand(IDbConnection cnc)
        {
            ...
            selectCommand.CommandText = "SET DATESTYLE TO 'ISO'";
        ...
        static object SelectAggregate(IDbConnection cnc, String agg)
        {
            ...
            selectCommand.CommandText = "select " + agg + "from mono_postgres_test";
        ...
        static void ReadResult(IDataReader rdr, DataTable dt)
        {
            ...
            Console.WriteLine("   Total Columns: " + dt.Rows.Count);

            // display the schema
            foreach (DataRow schemaRow in dt.Rows)
            {
                foreach (DataColumn schemaCol in dt.Columns)
                    Console.WriteLine(schemaCol.ColumnName + " = " + schemaRow[schemaCol]);
                Console.WriteLine();
            }
            ...
            while (rdr.Read())
            {
                ...
                for (c = 0; c < rdr.FieldCount; c++)
                {
                    ...
                    metadataValue = "    Col " + c + ": " + dr["ColumnName"];

                    // column data
                    if (rdr.IsDBNull(c) == true)
                        dataValue = " is NULL";
                    else
                        dataValue = ": " + rdr.GetValue(c);
            ...
            Console.WriteLine("   Total Rows Retrieved: " + nRows);
        }
            ...
            else
            {
                do
                {
                    ...
                    if (rdr.RecordsAffected != -1)
                    {
                        ...
                        Console.WriteLine(
                            "Result is from a SQL Command not (INSERT,UPDATE,DELETE).   Records Affected: "
                                + rdr.RecordsAffected
                        );
                    else
                    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Setting ConnectionString: " + connectionString);
