    ...
    public class BinarySerializationTest
    {
        ...
        public void RemotingFormatDataTableTest()
        {
            ...
            Assert.AreEqual(
                ...
                dt.RemotingFormat,
                "#1 Default value for RemotingFormat of DataTable is Xml"
            );
            ...
            Assert.AreEqual(
                ...
                dt.RemotingFormat,
                "#2 RemotingFormat Should be Binary"
            );
        }
        ...
        public void RemotingFormatDataSetTest()
        {
            ...
            Assert.AreEqual(
                SerializationFormat.Xml,
                ds.RemotingFormat,
                "#2 Default value for RemotingFormat of DataSet is Xml"
            );
            ...
            Assert.AreEqual(
                SerializationFormat.Binary,
                ds.RemotingFormat,
                "#3 RemotingFormat Should be Binary"
            );
            ...
            Assert.AreEqual(
                SerializationFormat.Binary,
                ds.RemotingFormat,
                "#4 RemotingFormat should be Binary for DataSet"
            );
            Assert.AreEqual(
                SerializationFormat.Binary,
                dt.RemotingFormat,
                "#5 RemotingFormat should be Binary for DataTable"
            );
            ...
            Assert.AreEqual(
                SerializationFormat.Xml,
                ds.RemotingFormat,
                "#6 RemotingFormat should be Xml for DataSet"
            );
            Assert.AreEqual(
                SerializationFormat.Xml,
                dt.RemotingFormat,
                "#7 RemotingFormat should be  Xml for DataTable"
            );
        ...
        public void DataTableSerializationTest1()
        {
            ...
            Assert.AreEqual(
                tb1.Columns.Count,
                dt.Columns.Count,
                "#2 ColumnCount property is Different"
            );
            ...
            Assert.AreEqual(
                tb1.CaseSensitive,
                dt.CaseSensitive,
                "#6 CaseSensitive property is Different"
            );
            Assert.AreEqual(tb1.Locale.LCID, dt.Locale.LCID, "#7 LocaleLCID property is Different");
            Assert.AreEqual(
                tb1.MinimumCapacity,
                dt.MinimumCapacity,
                "#8 NameSpace property is Different"
            );
            //Test Constraints
            Assert.AreEqual(
                tb1.Constraints.Count,
                dt.Constraints.Count,
                "#9 No. of Constraints is Different"
            );
            for (int i = 0; i < tb1.Constraints.Count; i++)
                Assert.AreEqual(
                    tb1.Constraints[i].GetType(),
                    dt.Constraints[i].GetType(),
                    "#10 Constraint : {0} is Different",
                    tb1.Constraints[i]
                );
            //Test for Table Data
        ...
        public void DataSetSerializationTest1()
        {
            ...
            Assert.AreEqual(
                ds.CaseSensitive,
                ds1.CaseSensitive,
                "#5 CaseSensitive property value is different"
            );
            Assert.AreEqual(ds.Locale.LCID, ds1.Locale.LCID, "#6 DataSet LocaleLCID is different");
            Assert.AreEqual(
                ds.EnforceConstraints,
                ds1.EnforceConstraints,
                "#7 EnforceConstraints property value is different"
            );
            ...
            //Table1
            Assert.AreEqual(
                ds.Tables[0].Constraints.Count,
                ds1.Tables[0].Constraints.Count,
                "#8 Number of constraint is different for Table :{0}",
                ds.Tables[0].TableName
            );

            for (int i = 0; i < ds.Tables[0].Constraints.Count; i++)
                Assert.AreEqual(
                    ds.Tables[0].Constraints[i].GetType(),
                    ds1.Tables[0].Constraints[i].GetType(),
                    "#9 Constraint : {0} is Different",
                    ds.Tables[0].Constraints[i]
                );

            //Table2
            Assert.AreEqual(
                ds.Tables[1].Constraints.Count,
                ds1.Tables[1].Constraints.Count,
                "#10 Number of constraint is different for Table :{0}",
                ds.Tables[1].TableName
            );

            for (int i = 0; i < ds.Tables[1].Constraints.Count; i++)
                Assert.AreEqual(
                    ds.Tables[1].Constraints[i].GetType(),
                    ds1.Tables[1].Constraints[i].GetType(),
                    "#11 Constraint : {0} is Different",
                    ds.Tables[1].Constraints[i]
                );
            //Relations
            Assert.AreEqual(
                ds.Relations.Count,
                ds1.Relations.Count,
                "#12 Relation count is different"
            );
            for (int i = 0; i < ds.Relations.Count; i++)
                Assert.AreEqual(
                    ds.Relations[i].RelationName,
                    ds1.Relations[i].RelationName,
                    "#13 Relation Name is different for relation :{0}",
                    ds.Relations[i].RelationName
                );

            for (int i = 0; i < ds.Relations.Count; i++)
                Assert.AreEqual(
                    ds.Relations[i].ParentTable.TableName,
                    ds1.Relations[i].ParentTable.TableName,
                    "#14 Relation Name is different for relation :{0}",
                    ds.Relations[i].ParentTable.TableName
                );

            for (int i = 0; i < ds.Relations.Count; i++)
                Assert.AreEqual(
                    ds.Relations[i].ChildTable.TableName,
                    ds1.Relations[i].ChildTable.TableName,
                    "#15 Relation Name is different for relation :{0}",
                    ds.Relations[i].ChildTable.TableName
                );
...
*/
            ...
            ForeignKeyConstraint fKey2 = new ForeignKeyConstraint(
                tb2.Columns["eid"],
                tb3.Columns["eid"]
            );
            DataRelation rel = new DataRelation(
            ...
            Assert.AreEqual(
                ds.Tables[0].Constraints.Count,
                ds1.Tables[0].Constraints.Count,
                "#2 Number of Constraints differs in Table :{0}",
                ds.Tables[0]
            );
            for (int i = 0; i < ds.Tables[0].Constraints.Count; i++)
                Assert.AreEqual(
            ...
            Assert.AreEqual(
                ds.Tables[1].Constraints.Count,
                ds1.Tables[1].Constraints.Count,
                "#4 Number of Constraints differs in Table :{0}",
                ds.Tables[1]
            );
            for (int i = 0; i < ds.Tables[1].Constraints.Count; i++)
                Assert.AreEqual(
                    ds.Tables[1].Constraints[i].GetType(),
                    ds1.Tables[1].Constraints[i].GetType(),
                    "#5 Constraint : {0} is Different",
                    ds.Tables[1].Constraints[i]
                );
            //Table3
            Assert.AreEqual(
                ds.Tables[2].Constraints.Count,
                ds1.Tables[2].Constraints.Count,
                "#5 Number of Constraints differs in Table :{0}",
                ds.Tables[2]
            );
            for (int i = 0; i < ds.Tables[2].Constraints.Count; i++)
                Assert.AreEqual(
                    ds.Tables[2].Constraints[i].GetType(),
                    ds1.Tables[2].Constraints[i].GetType(),
                    "#6 Constraint : {0} is Different",
                    ds.Tables[2].Constraints[i]
                );
            //Table4
            Assert.AreEqual(
                ds.Tables[3].Constraints.Count,
                ds1.Tables[3].Constraints.Count,
                "#7 Number of Constraints differs in Table :{0}",
                ds.Tables[3]
            );
            for (int i = 0; i < ds.Tables[3].Constraints.Count; i++)
                Assert.AreEqual(
                    ds.Tables[3].Constraints[i].GetType(),
                    ds1.Tables[3].Constraints[i].GetType(),
                    "#8 Constraint : {0} is Different",
                    ds.Tables[3].Constraints[i]
                );
            //Relations
            Assert.AreEqual(
                ds.Relations.Count,
                ds1.Relations.Count,
                "#8 Number of realtions differ"
            );
            for (int i = 0; i < ds.Relations.Count; i++)
                Assert.AreEqual(
                    ds.Relations[i].RelationName,
                    ds.Relations[i].RelationName,
                    "#9 Relation : {0} differs",
                    ds.Relations[i]
                );
        }
