...
namespace Mono.Data.Sqlite
{
    ...
    public sealed partial class SqliteConnection : DbConnection, ICloneable
    {
            ...
            if (connection.State == ConnectionState.Open)
            {
                ...
                using (DataTable tbl = connection.GetSchema("Catalogs"))
                {
                    foreach (DataRow row in tbl.Rows)
                    {
                        str = row[0].ToString();
                        if (
                            String.Compare(str, "main", true, CultureInfo.InvariantCulture) != 0
        ...
        [Obsolete(
            "This functionality is being removed from a future version of the SQLite provider"
        )]
        ...
        [Obsolete(
            "Use one of the standard BeginTransaction methods, this one will be removed soon"
        )]
        public SqliteTransaction BeginTransaction(IsolationLevel isolationLevel, bool deferredLock)
        {
            return (SqliteTransaction)BeginDbTransaction(
                deferredLock == false ? IsolationLevel.Serializable : IsolationLevel.ReadCommitted
            );
        }
        ...
        internal static SortedList<string, string> ParseConnectionString(string connectionString)
        {
            ...
            int n;
            SortedList<string, string> ls = new SortedList<string, string>(
        ...
        static internal string FindKey(
            SortedList<string, string> items,
            string key,
            string defValue
        )
        {
        ...
        public override void Open()
        {
            ...
            {
                bool usePooling = (
                    SqliteConvert.ToBoolean(FindKey(opts, "Pooling", Boolean.FalseString)) == true
                );
                bool bUTF16 = (
                    SqliteConvert.ToBoolean(FindKey(opts, "UseUTF16Encoding", Boolean.FalseString))
                    == true
                );
                int maxPoolSize = Convert.ToInt32(FindKey(opts, "Max Pool Size", "100"));

                _defaultTimeout = Convert.ToInt32(
                    FindKey(opts, "Default Timeout", "30"),
                    CultureInfo.CurrentCulture
                );
                ...
                _defaultIsolation = (IsolationLevel)
                    Enum.Parse(
                        ...
                        FindKey(opts, "Default IsolationLevel", "Serializable"),
                        true
                    );
                if (
                ...
                SQLiteDateFormats dateFormat = (SQLiteDateFormats)
                    Enum.Parse(
                        typeof(SQLiteDateFormats),
                ...
                if (
                    SqliteConvert.ToBoolean(FindKey(opts, "Read Only", Boolean.FalseString)) == true
                )
                ...
                else
                {
                    ...
                    if (
                        SqliteConvert.ToBoolean(FindKey(opts, "FailIfMissing", Boolean.FalseString))
                        == false
                    )
                        flags |= SQLiteOpenFlagsEnum.Create;
                }
                if (
                    SqliteConvert.ToBoolean(
                        FindKey(opts, "FileProtectionComplete", Boolean.FalseString)
                    )
                )
                    flags |= SQLiteOpenFlagsEnum.FileProtectionComplete;
                if (
                    SqliteConvert.ToBoolean(
                        FindKey(opts, "FileProtectionCompleteUnlessOpen", Boolean.FalseString)
                    )
                )
                ...
                if (
                    SqliteConvert.ToBoolean(
                        FindKey(opts, "FileProtectionNone", Boolean.FalseString)
                    )
                )
                ...
                _binaryGuid = (
                    SqliteConvert.ToBoolean(FindKey(opts, "BinaryGUID", Boolean.TrueString)) == true
                );
                ...
                using (SqliteCommand cmd = CreateCommand())
                {
                    ...
                    if (
                        String.Compare(defValue, "Default", StringComparison.OrdinalIgnoreCase) != 0
                    )
                    {
                        cmd.CommandText = String.Format(
                            CultureInfo.InvariantCulture,
        ...
        private string ExpandFileName(string sourceFile)
        {
            ...
            if (sourceFile.StartsWith(_dataDirectory, StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    if (
                        sourceFile[_dataDirectory.Length] == Path.DirectorySeparatorChar
                ...
                sourceFile = Path.Combine(
                    dataDirectory,
                    sourceFile.Substring(_dataDirectory.Length)
                );
            }
        ...
        private DataTable Schema_Columns(string strCatalog, string strTable, string strColumn)
        {
            ...
            string master =
                (String.Compare(strCatalog, "temp", true, CultureInfo.InvariantCulture) == 0)
                    ? _tempmasterdb
                    : _masterdb;
            ...
            using (SqliteDataReader rdTables = cmdTables.ExecuteReader())
            {
                ...
                {
                    if (
                        String.IsNullOrEmpty(strTable)
                        || String.Compare(
                            strTable,
                            rdTables.GetString(2),
                            true,
                            CultureInfo.InvariantCulture
                        ) == 0
                    )
                    {
                        try
                        {
                            ...
                            using (
                                SqliteDataReader rd = (SqliteDataReader)
                            ...
                            using (DataTable tblSchema = rd.GetSchemaTable(true, true))
                            {
                                foreach (DataRow schemaRow in tblSchema.Rows)
                                {
                                    if (
                                        String.Compare(
                                            schemaRow[SchemaTableColumn.ColumnName].ToString(),
                                            strColumn,
                                            true,
                                            CultureInfo.InvariantCulture
                                        ) == 0
                                    ...
                                    )
                                    {
                                        ...
                                        row["NUMERIC_PRECISION"] = schemaRow[
                                            SchemaTableColumn.NumericPrecision
                                        ];
                                        row["NUMERIC_SCALE"] = schemaRow[
                                            SchemaTableColumn.NumericScale
                                        ];
                                        row["TABLE_NAME"] = rdTables.GetString(2);
                                        row["COLUMN_NAME"] = schemaRow[
                                            SchemaTableColumn.ColumnName
                                        ];
                                        row["TABLE_CATALOG"] = strCatalog;
                                        row["ORDINAL_POSITION"] = schemaRow[
                                            SchemaTableColumn.ColumnOrdinal
                                        ];
                                        row["COLUMN_HASDEFAULT"] = (
                                            schemaRow[SchemaTableOptionalColumn.DefaultValue]
                                            != DBNull.Value
                                        );
                                        row["COLUMN_DEFAULT"] = schemaRow[
                                            SchemaTableOptionalColumn.DefaultValue
                                        ];
                                        row["IS_NULLABLE"] = schemaRow[
                                            SchemaTableColumn.AllowDBNull
                                        ];
                                        row["DATA_TYPE"] = schemaRow["DataTypeName"]
                                            .ToString()
                                            .ToLower(CultureInfo.InvariantCulture);
                                        row["EDM_TYPE"] = SqliteConvert
                                            .DbTypeToTypeName(
                                                (DbType)schemaRow[SchemaTableColumn.ProviderType]
                                            )
                                            .ToString()
                                            .ToLower(CultureInfo.InvariantCulture);
                                        row["CHARACTER_MAXIMUM_LENGTH"] = schemaRow[
                                            SchemaTableColumn.ColumnSize
                                        ];
                                        row["TABLE_SCHEMA"] = schemaRow[
                                            SchemaTableColumn.BaseSchemaName
                                        ];
                                        row["PRIMARY_KEY"] = schemaRow[SchemaTableColumn.IsKey];
                                        row["AUTOINCREMENT"] = schemaRow[
                            ...
                            using (SqliteDataReader rd = (SqliteDataReader)cmd.ExecuteReader())
                            {
                                while (rd.Read())
                                {
                                    ...
                                    )
                                    {
                                        ...
                                        using (
                                            SqliteCommand cmdIndexes = new SqliteCommand(
                                                String.Format(
                                                    CultureInfo.InvariantCulture,
                                                    "SELECT * FROM [{0}].[{2}] WHERE [type] LIKE 'index' AND [name] LIKE '{1}'",
                                                    strCatalog,
                                                    rd.GetString(1).Replace("'", "''"),
                                                    master
                                                ),
                                                this
                                            )
                                        ...
                                        using (
                                            SqliteDataReader rdIndexes = cmdIndexes.ExecuteReader()
                                        )
                                        {
                                            while (rdIndexes.Read())
                                            {
                                                ...
                                                {
                                                    row["INDEX_DEFINITION"] = rdIndexes.GetString(
                                                        4
                                                    );
                                                    break;
                                                }
                                        ...
                                        if (
                                            ...
                                            && rd.GetString(1)
                                                .StartsWith(
                                                    "sqlite_autoindex_" + rdTables.GetString(2),
                                                    StringComparison.InvariantCultureIgnoreCase
                                                ) == true
                                        )
                                        {
                                            using (
                                                SqliteCommand cmdDetails = new SqliteCommand(
                                                    String.Format(
                                                        CultureInfo.InvariantCulture,
                                            ...
                                            using (
                                                SqliteDataReader rdDetails =
                                                    cmdDetails.ExecuteReader()
                                            )
                                            {
                                                int nMatches = 0;
                                                ...
                                                {
                                                    if (
                                                        primaryKeys.Contains(rdDetails.GetInt32(1))
                                                        == false
                                                    )
                                                    {
                                                        nMatches = 0;
        ...
        private DataTable Schema_Triggers(string catalog, string table, string triggerName)
        {
            ...
            if (String.IsNullOrEmpty(catalog))
                catalog = "main";
            string master =
                (String.Compare(catalog, "temp", true, CultureInfo.InvariantCulture) == 0)
                    ? _tempmasterdb
                    : _masterdb;
        ...
        private DataTable Schema_Tables(string strCatalog, string strTable, string strType)
        {
            ...
            using (SqliteDataReader rd = (SqliteDataReader)cmd.ExecuteReader())
            {
                while (rd.Read())
                {
                    ...
                    if (
                        String.Compare(strType, strItem, true, CultureInfo.InvariantCulture) == 0
                        || strType == null
                    )
                    {
                        if (
        ...
        private DataTable Schema_Views(string strCatalog, string strView)
        {
            ...
            using (SqliteDataReader rd = (SqliteDataReader)cmd.ExecuteReader())
            {
                ...
                {
                    if (
                        String.Compare(rd.GetString(1), strView, true, CultureInfo.InvariantCulture)
                            == 0
                ...
                while (rdTables.Read())
                {
                    ...
                    )
                    {
                        ...
                        if (primaryKeys.Count == 1 && maybeRowId == true)
                        {
                            ...
                            row["CONSTRAINT_NAME"] = String.Format(
                                CultureInfo.InvariantCulture,
                                "{1}_PK_{0}",
                                rdTables.GetString(2),
                                master
                        ...
                        using (SqliteDataReader rdIndexes = cmdIndexes.ExecuteReader())
                        {
                            while (rdIndexes.Read())
                            {
                                ...
                                )
                                {
                                    ...
                                    {
                                        using (
                                            SqliteCommand cmdIndex = new SqliteCommand(
                                                String.Format(
                                                    CultureInfo.InvariantCulture,
                                                    "PRAGMA [{0}].index_info([{1}])",
                                                    strCatalog,
                                        ...
                                        using (SqliteDataReader rdIndex = cmdIndex.ExecuteReader())
                                        {
                                            while (rdIndex.Read())
                                            {
                                                ...
                                                int sortMode;
                                                int onError;
                                                _sql.GetIndexColumnExtendedInfo(
                                                    strCatalog,
                                                    rdIndexes.GetString(1),
                                                    rdIndex.GetString(2),
                                                    out sortMode,
                                                    out onError,
                                                    out collationSequence
                                                );

                                                if (
                                                    String.IsNullOrEmpty(collationSequence) == false
                                                )
        ...
        private DataTable Schema_ViewColumns(string strCatalog, string strView, string strColumn)
        {
            ...
            using (
                SqliteCommand cmdViews = new SqliteCommand(
                    String.Format(
                        CultureInfo.InvariantCulture,
                        "SELECT * FROM [{0}].[{1}] WHERE [type] LIKE 'view'",
                        strCatalog,
            ...
            using (SqliteDataReader rdViews = cmdViews.ExecuteReader())
            {
                while (rdViews.Read())
                {
                    ...
                    {
                        using (
                            SqliteCommand cmdViewSelect = new SqliteCommand(
                                String.Format(
                                    CultureInfo.InvariantCulture,
                                    "SELECT * FROM [{0}].[{1}]",
                                    strCatalog,
                        ...
                        )
                        {
                            ...
                            using (
                                SqliteDataReader rdViewSelect = cmdViewSelect.ExecuteReader(
                            ...
                            using (
                                DataTable tblSchemaView = rdViewSelect.GetSchemaTable(false, false)
                            )
                            using (DataTable tblSchema = rd.GetSchemaTable(false, false))
                            {
                                for (n = 0; n < tblSchema.Rows.Count; n++)
                                {
                                    ...
                                    if (
                                        String.Compare(
                                            viewRow[SchemaTableColumn.ColumnName].ToString(),
                                            strColumn,
                                            true,
                                            CultureInfo.InvariantCulture
                                        ) == 0
                                    ...
                                    )
                                    {
                                        ...
                                        row["TABLE_NAME"] = schemaRow[
                                            SchemaTableColumn.BaseTableName
                                        ];
                                        row["COLUMN_NAME"] = schemaRow[
                                            SchemaTableColumn.BaseColumnName
                                        ];
                                        row["VIEW_COLUMN_NAME"] = viewRow[
                                            SchemaTableColumn.ColumnName
                                        ];
                                        row["COLUMN_HASDEFAULT"] = (
                                            viewRow[SchemaTableOptionalColumn.DefaultValue]
                                            != DBNull.Value
                                        );
                                        row["COLUMN_DEFAULT"] = viewRow[
                                            SchemaTableOptionalColumn.DefaultValue
                                        ];
                                        row["ORDINAL_POSITION"] = viewRow[
                                            SchemaTableColumn.ColumnOrdinal
                                        ];
                                        ...
                                        row["EDM_TYPE"] = SqliteConvert
                                            .DbTypeToTypeName(
                                                (DbType)viewRow[SchemaTableColumn.ProviderType]
                                            )
                                            .ToString()
                                            .ToLower(CultureInfo.InvariantCulture);
                                        row["CHARACTER_MAXIMUM_LENGTH"] = viewRow[
                                            SchemaTableColumn.ColumnSize
                                        ];
                                        row["TABLE_SCHEMA"] = viewRow[
                                            SchemaTableColumn.BaseSchemaName
                                        ];
                                        row["PRIMARY_KEY"] = viewRow[SchemaTableColumn.IsKey];
                                        row["AUTOINCREMENT"] = viewRow[
                                            SchemaTableOptionalColumn.IsAutoIncrement
                                        ];
                ...
                while (rdTables.Read())
                {
                    ...
                    )
                    {
                        try
                        {
                            ...
                            using (
                                SqliteCommand cmdKey = new SqliteCommand(
                                    String.Format(
                                        CultureInfo.InvariantCulture,
                            ...
                            using (SqliteDataReader rdKey = cmdKey.ExecuteReader())
                            {
                                while (rdKey.Read())
                                {
                                    ...
                                    row["TABLE_CATALOG"] = strCatalog;
                                    row["TABLE_NAME"] = builder.UnquoteIdentifier(
                                        rdTables.GetString(2)
                                    );
                                    row["CONSTRAINT_TYPE"] = "FOREIGN KEY";
                                    row["IS_DEFERRABLE"] = false;
                                    row["INITIALLY_DEFERRED"] = false;
                                    row["FKEY_FROM_COLUMN"] = builder.UnquoteIdentifier(
                                        rdKey[3].ToString()
                                    );
                                    row["FKEY_TO_CATALOG"] = strCatalog;
                                    row["FKEY_TO_TABLE"] = builder.UnquoteIdentifier(
                                        rdKey[2].ToString()
                                    );
                                    row["FKEY_TO_COLUMN"] = builder.UnquoteIdentifier(
                                        rdKey[4].ToString()
                                    );
