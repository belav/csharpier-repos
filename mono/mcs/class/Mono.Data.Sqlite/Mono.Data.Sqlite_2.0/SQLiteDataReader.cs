    ...
    public sealed class SqliteDataReader : DbDataReader
    {
        ...
        public override decimal GetDecimal(int i)
        {
            ...
            return Decimal.Parse(
                _activeStatement._sql.GetText(_activeStatement, i),
        ...
        internal DataTable GetSchemaTable(bool wantUniqueInfo, bool wantDefaultValue)
        {
            ...
            for (int n = 0; n < _fieldCount; n++)
            {
                ...
                row[SchemaTableColumn.NumericPrecision] = SqliteConvert.DbTypeToNumericPrecision(
                    typ
                );
                ...
                if (String.IsNullOrEmpty(strColumn) == false)
                {
                    ...
                    if (arSize.Length > 1)
                    {
                        ...
                        if (arSize.Length > 1)
                        {
                            ...
                            if (
                                GetSQLiteType(n).Type == DbType.String
                                || GetSQLiteType(n).Type == DbType.Binary
                            )
                            {
                                row[SchemaTableColumn.ColumnSize] = Convert.ToInt32(
                                    arSize[0],
                                    CultureInfo.InvariantCulture
                                );
                            }
                            ...
                            {
                                row[SchemaTableColumn.NumericPrecision] = Convert.ToInt32(
                                    arSize[0],
                                    CultureInfo.InvariantCulture
                                );
                                if (arSize.Length > 1)
                                    row[SchemaTableColumn.NumericScale] = Convert.ToInt32(
                                        arSize[1],
                                        CultureInfo.InvariantCulture
                                    );
                            }
                    ...
                    {
                        if (
                            (string)row[SchemaTableOptionalColumn.BaseCatalogName] != strCatalog
                        ...
                        foreach (DataRow rowIndexes in tblIndexes.Rows)
                        {
                            ...
                            foreach (DataRow rowColumnIndex in tblIndexColumns.Rows)
                            {
                                if (
                                    String.Compare(
                                        (string)rowColumnIndex["COLUMN_NAME"],
                                        strColumn,
                                        true,
                                        CultureInfo.InvariantCulture
                                    ) == 0
                                )
                                {
                                    ...
                                    if (
                                        tblIndexColumns.Rows.Count == 1
                                        && (bool)rowIndexes["PRIMARY_KEY"] == true
        ...
        private SQLiteType GetSQLiteType(int i)
        {
            ...
            if (typ.Affinity == TypeAffinity.Uninitialized)
                typ.Type = SqliteConvert.TypeNameToDbType(
                    _activeStatement._sql.ColumnType(_activeStatement, i, out typ.Affinity)
                );
            else
