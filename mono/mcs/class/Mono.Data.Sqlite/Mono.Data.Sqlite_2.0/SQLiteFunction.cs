...
namespace Mono.Data.Sqlite
{
    ...
    public abstract class SqliteFunction : IDisposable
    {
        ...
        void SetReturnValue(IntPtr context, object returnValue)
        {
            ...
            switch (SqliteConvert.TypeToAffinity(t))
            {
                ...
                case TypeAffinity.Int64:
                    _base.ReturnInt64(
                        context,
                        Convert.ToInt64(returnValue, CultureInfo.CurrentCulture)
                    );
                    return;
                case TypeAffinity.Double:
                    _base.ReturnDouble(
                        context,
                        Convert.ToDouble(returnValue, CultureInfo.CurrentCulture)
                    );
                    return;
                case TypeAffinity.Text:
        ...
        {
            return Compare(
                SqliteConvert.UTF8ToString(ptr1, len1),
                SqliteConvert.UTF8ToString(ptr2, len2)
            );
        }
        ...
        {
            return Compare(
                SQLite3_UTF16.UTF16ToString(ptr1, len1),
                SQLite3_UTF16.UTF16ToString(ptr2, len2)
            );
        }
        ...
        /// </summary>
#if !PLATFORM_COMPACTFRAMEWORK
        [global::System.Security.Permissions.FileIOPermission(
            global::System.Security.Permissions.SecurityAction.Assert,
        ...
        static SqliteFunction()
        {
            try
            {
                ...
                System.Reflection.Assembly[] arAssemblies = System.AppDomain
                    .CurrentDomain
                    .GetAssemblies();
                int w = arAssemblies.Length;
                System.Reflection.AssemblyName sqlite = System.Reflection
                    .Assembly
                    .GetCallingAssembly()
                    .GetName();
        ...
        internal static SqliteFunction[] BindFunctions(SQLiteBase sqlbase)
        {
            ...
            foreach (SqliteFunctionAttribute pr in _registeredFunctions)
            {
                ...
                f._base = sqlbase;
                f._InvokeFunc =
                    (pr.FuncType == FunctionType.Scalar)
                        ? new SQLiteCallback(f.ScalarCallback)
                        : null;
                f._StepFunc =
                    (pr.FuncType == FunctionType.Aggregate)
                        ? new SQLiteCallback(f.StepCallback)
                        : null;
                f._FinalFunc =
                    (pr.FuncType == FunctionType.Aggregate)
                        ? new SQLiteFinalCallback(f.FinalCallback)
                        : null;
                f._CompareFunc =
                    (pr.FuncType == FunctionType.Collation)
                        ? new SQLiteCollation(f.CompareCallback)
                        : null;
                f._CompareFunc16 =
                    (pr.FuncType == FunctionType.Collation)
                        ? new SQLiteCollation(f.CompareCallback16)
                        : null;

                if (pr.FuncType != FunctionType.Collation)
                    sqlbase.CreateFunction(
                        pr.Name,
                        pr.Arguments,
                        (f is SqliteFunctionEx),
                        f._InvokeFunc,
                        f._StepFunc,
                        f._FinalFunc
                    );
                else
                {
                    ...
                    sqlbase.CreateCollation(
                        pr.Name,
                        collation_callback,
                        collation_callback16,
        ...
        internal static int collation_callback(
            IntPtr puser,
            int len1,
            IntPtr pv1,
            int len2,
        ...
        [Mono.Util.MonoPInvokeCallback(typeof(SQLiteCollation))]
        internal static int collation_callback16(
            IntPtr puser,
            int len1,
            IntPtr pv1,
            int len2,
