    ...
    internal class SQLite3 : SQLiteBase
    {
        ...
        )
        {
            ...
            if (_sql == null)
            {
...
#if !SQLITE_STANDARD
                int n = UnsafeNativeMethods.sqlite3_open_interop(
                    ToUTF8(strFilename),
                    (int)flags,
                    out db
                );
                ...
                {
                    n = UnsafeNativeMethods.sqlite3_open_v2(
                        ToUTF8(strFilename),
                        out db,
                        (int)flags,
                        IntPtr.Zero
        ...
        internal override int Reset(SqliteStatement stmt)
        {
            ...
            if (n == 17) // SQLITE_SCHEMA
            {
                ...
                string str;
                using (
                    SqliteStatement tmp = Prepare(
                        null,
                        stmt._sqlStatement,
                        null,
                        (uint)(stmt._command._commandTimeout * 1000),
                        out str
                    )
                )
                {
        ...
        )
        {
            ...
            try
            {
                ...
                {
#if !SQLITE_STANDARD
                    n = UnsafeNativeMethods.sqlite3_prepare_interop(
                        _sql,
                        psql,
                        b.Length - 1,
                        out stmt,
...
#else
                    n = UnsafeNativeMethods.sqlite3_prepare(
                        _sql,
                        psql,
                        b.Length - 1,
                        out stmt,
                ...
                if (stmt != IntPtr.Zero)
                    cmd = new SqliteStatement(
                        this,
                        stmt,
                        strSql.Substring(0, strSql.Length - strRemain.Length),
                        previous
                    );
...
#else
            int n = UnsafeNativeMethods.sqlite3_bind_double_interop(
                stmt._sqlite_stmt,
                index,
                ref value
            );
...
#else
            int n = UnsafeNativeMethods.sqlite3_bind_int64_interop(
                stmt._sqlite_stmt,
                index,
                ref value
            );
        ...
        {
            byte[] b = ToUTF8(value);
            int n = UnsafeNativeMethods.sqlite3_bind_text(
                stmt._sqlite_stmt,
                index,
                b,
                b.Length - 1,
        ...
        {
            int n = UnsafeNativeMethods.sqlite3_bind_blob(
                stmt._sqlite_stmt,
                index,
                blobData,
                blobData.Length,
        ...
        internal override string Bind_ParamName(SqliteStatement stmt, int index)
        {
            ...
            int len;
            return UTF8ToString(
                UnsafeNativeMethods.sqlite3_bind_parameter_name_interop(
                    stmt._sqlite_stmt,
                    index,
                    out len
                ),
...
#else
            return UTF8ToString(
                UnsafeNativeMethods.sqlite3_bind_parameter_name(stmt._sqlite_stmt, index),
                -1
            );
#endif
        }
        ...
        {
            return UnsafeNativeMethods.sqlite3_bind_parameter_index(
                stmt._sqlite_stmt,
                ToUTF8(paramName)
            );
        }
        ...
        internal override string ColumnName(SqliteStatement stmt, int index)
        {
            ...
            int len;
            return UTF8ToString(
                UnsafeNativeMethods.sqlite3_column_name_interop(stmt._sqlite_stmt, index, out len),
                len
            );
#else
            return UTF8ToString(
                UnsafeNativeMethods.sqlite3_column_name(stmt._sqlite_stmt, index),
                -1
            );
#endif
        }
        ...
        )
        {
            int len;
#if !SQLITE_STANDARD
            IntPtr p = UnsafeNativeMethods.sqlite3_column_decltype_interop(
                stmt._sqlite_stmt,
                index,
                out len
            );
        ...
        internal override string ColumnOriginalName(SqliteStatement stmt, int index)
        {
            ...
            int len;
            return UTF8ToString(
                UnsafeNativeMethods.sqlite3_column_origin_name_interop(
                    stmt._sqlite_stmt,
                    index,
                    out len
                ),
      ...
      throw new NotImplementedException ();
#else
            return UTF8ToString(
                UnsafeNativeMethods.sqlite3_column_origin_name(stmt._sqlite_stmt, index),
                -1
            );
#endif
        }
        ...
        internal override string ColumnDatabaseName(SqliteStatement stmt, int index)
        {
            ...
            int len;
            return UTF8ToString(
                UnsafeNativeMethods.sqlite3_column_database_name_interop(
                    stmt._sqlite_stmt,
                    index,
                    out len
                ),
      ...
      throw new NotImplementedException ();
#else
            return UTF8ToString(
                UnsafeNativeMethods.sqlite3_column_database_name(stmt._sqlite_stmt, index),
                -1
            );
#endif
        }
        ...
        internal override string ColumnTableName(SqliteStatement stmt, int index)
        {
            ...
            int len;
            return UTF8ToString(
                UnsafeNativeMethods.sqlite3_column_table_name_interop(
                    stmt._sqlite_stmt,
                    index,
                    out len
                ),
      ...
      throw new NotImplementedException ();
#else
            return UTF8ToString(
                UnsafeNativeMethods.sqlite3_column_table_name(stmt._sqlite_stmt, index),
                -1
            );
#endif
        }
        ...
        )
        {
            ...
            dtLen = -1;
            csLen = -1;
            n = UnsafeNativeMethods.sqlite3_table_column_metadata(
                _sql,
                ToUTF8(dataBase),
                ToUTF8(table),
                ToUTF8(column),
...
#else
            return UTF8ToString(
                UnsafeNativeMethods.sqlite3_column_text(stmt._sqlite_stmt, index),
                -1
            );
#endif
        }
        ...
        )
        {
            ...
            SQLiteCallback func_callback =
                func == null ? null : new SQLiteCallback(scalar_callback);
            SQLiteCallback funcstep_callback =
                funcstep == null ? null : new SQLiteCallback(step_callback);
            SQLiteFinalCallback funcfinal_callback =
                funcfinal == null ? null : new SQLiteFinalCallback(final_callback);
            ...
            n = UnsafeNativeMethods.sqlite3_create_function_interop(
                _sql,
                ToUTF8(strFunction),
                nArgs,
                4,
...
#else
            n = UnsafeNativeMethods.sqlite3_create_function(
                _sql,
                ToUTF8(strFunction),
                nArgs,
                4,
        ...
        {
            int n = UnsafeNativeMethods.sqlite3_create_collation(
                _sql,
                ToUTF8(strCollation),
                2,
                user_data,
            ...
            if (n == 0)
                UnsafeNativeMethods.sqlite3_create_collation(
                    _sql,
                    ToUTF8(strCollation),
                    1,
                    user_data,
        ...
        )
        {
            ...
            return UnsafeNativeMethods.sqlite3_context_collcompare(
                context,
                b1,
                b1.Length,
                b2,
        ...
        )
        {
            ...
            int type;
            int enc;
            IntPtr p = UnsafeNativeMethods.sqlite3_context_collseq(
                context,
                out type,
                out enc,
                out len
        ...
        internal override void ChangePassword(byte[] newPasswordBytes)
        {
            ...
            throw new PlatformNotSupportedException();
#else
            int n = UnsafeNativeMethods.sqlite3_rekey(
                _sql,
                newPasswordBytes,
        ...
        )
        {
            ...
            rc = UnsafeNativeMethods.sqlite3_index_column_info_interop(
                _sql,
                ToUTF8(database),
                ToUTF8(index),
                ToUTF8(column),
