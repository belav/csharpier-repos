...
namespace Mono.Data.Sqlite
{
    ...
    internal sealed class SqliteKeyReader : IDisposable
    {
                    ...
                    for (int i = 0; i < pair.Value.Count; i++)
                    {
                        ...
                        using (
                            DataTable tbl = cnn.GetSchema(
                                "Indexes",
                                new string[] { pair.Key, null, table }
                            )
                        )
                        {
                            ...
                            {
                                using (
                                    DataTable tblTables = cnn.GetSchema(
                                        "Tables",
                                        new string[] { pair.Key, null, table }
                                    )
                                )
                                {
                                    ...
                                    using (
                                        DataTable indexColumns = cnn.GetSchema(
                                            "IndexColumns",
                                            new string[]
                                            {
                                                pair.Key,
                                                null,
                                                table,
                                                (string)preferredRow["INDEX_NAME"],
                                            }
                                        )
                                    )
                                    {
                                        ...
                                        for (int x = 0; x < indexColumns.Rows.Count; x++)
                                        {
                                            ...
                                            foreach (DataRow row in schema.Rows)
                                            {
                                                ...
                                                if (
                                                    (string)row[SchemaTableColumn.BaseColumnName]
                                                        == (string)
                                            ...
                                            if (addKey == true)
                                                cols.Add(
                                                    (string)indexColumns.Rows[x]["COLUMN_NAME"]
                                                );
                                        }
        ...
        internal int GetOrdinal(string name)
        {
            ...
            {
                if (
                    String.Compare(name, _keyInfo[n].columnName, StringComparison.OrdinalIgnoreCase)
                    == 0
                )
                    return n;
            }
