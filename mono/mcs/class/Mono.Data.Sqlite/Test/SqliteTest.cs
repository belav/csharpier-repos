...
namespace Test.Mono.Data.Sqlite
{
    class SqliteTest
    {
        ...
        static void Test(bool v3, string encoding)
        {
            ...
            dbcmd.CommandText =
                "INSERT INTO MONO_TEST  " + "(NID, NDESC, NTIME) " + "VALUES(:NID,:NDESC,:NTIME)";
            ...
            Console.WriteLine(
                "Insert modified rows with parameters = 1, 2: "
                    + dbcmd.ExecuteNonQuery()
                    + " , "
                    + dbcmd.LastInsertRowID()
            );
            ...
            Console.WriteLine(
                "Insert with null modified rows and ID = 1, 3: "
                    + dbcmd.ExecuteNonQuery()
                    + " , "
                    + dbcmd.LastInsertRowID()
            );
            ...
            dbcmd.CommandText =
                "INSERT INTO MONO_TEST  " + "(NID, NDESC, NTIME) " + "VALUES(?,?,?)";
            ...
            Console.WriteLine(
                "Insert with unnamed parameters = 1, 5: "
                    + dbcmd.ExecuteNonQuery()
                    + " , "
                    + dbcmd.LastInsertRowID()
            );
