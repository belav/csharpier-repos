...
namespace MonoTests.Mono.Data.Sqlite
{
    ...
    public class SqliteCommandUnitTests
    {
            ...
            try
            {
                using (
                    SqliteCommand createCommand = new SqliteCommand(
                        "CREATE TABLE t1(t  TEXT,  f FLOAT, i INTEGER, b TEXT);",
                        _conn
                    )
                )
                using (
                    SqliteCommand insertCommand = new SqliteCommand(
                        "INSERT INTO t1  (t, f, i, b ) VALUES('" + stringvalue + "',123,123,'123')",
                        _conn
                    )
                )
                {
        ...
        public void Delete()
        {
            using (_conn)
            using (
                SqliteCommand insCmd = new SqliteCommand(
                    "INSERT INTO t1 VALUES ('todelete',0.1,0,'')",
                    _conn
                )
            )
            using (
                SqliteCommand delCmd = new SqliteCommand(
                    "DELETE FROM t1 WHERE t = 'todelete'",
                    _conn
                )
            )
            {
        ...
        public void Insert()
        {
            ...
            using (
                SqliteCommand insCmd = new SqliteCommand(
                    "INSERT INTO t1 VALUES ('inserted',0.1,0,'')",
                    _conn
                )
            )
            {
        ...
        public void Update()
        {
            ...
            using (
                SqliteCommand insCmd = new SqliteCommand(
                    "INSERT INTO t1 VALUES ('toupdate',0.1,0,'')",
                    _conn
                )
            )
            using (
                SqliteCommand updCmd = new SqliteCommand(
                    "UPDATE t1 SET t = 'updated' ,f = 2.0, i = 2, b = '' WHERE t = 'toupdate'",
                    _conn
                )
            )
            {
        ...
        public void ScalarReturn()
        {
            ...
            using (
                SqliteCommand cmd = new SqliteCommand(
                    "SELECT COUNT(*) FROM t1 WHERE  t LIKE '%äöüß'",
                    _conn
                )
            )
            {
        ...
        public void InsertWithTransaction()
        {
            ...
            using (
                SqliteCommand c1 = new SqliteCommand(
                    "INSERT INTO t1 VALUES ('a',0.1,0,'0')",
                    _conn,
                    t
                )
            )
            using (
                SqliteCommand c2 = new SqliteCommand(
                    "INSERT INTO t1 VALUES ('b',1.2,0,'0')",
                    _conn,
                    t
                )
            )
            using (
                SqliteCommand c3 = new SqliteCommand(
                    "INSERT INTO t1 VALUES ('c',0.3,1,'0')",
                    _conn,
                    t
                )
            )
            using (
                SqliteCommand c4 = new SqliteCommand(
                    "INSERT INTO t1 VALUES ('d',0.4,0,'1')",
                    _conn,
                    t
                )
            )
