    ...
    public class SqliteiOS82BugTests
    {
            ...
            try
            {
                using (
                    var createCommand = new SqliteCommand(
                        "CREATE TABLE Company (RecordId int, Name text);",
                        cnn
                    )
                )
                using (
                    var insertCommand = new SqliteCommand(
                        "INSERT INTO Company VALUES (1, 'Test CO')",
                        cnn
                    )
                )
                {
        ...
        public void SqliteSelectTestBug27864()
        {
            ...
            catch (SqliteException ex)
            {
                ...
                if (
                    BCL.Tests.TestRuntime.CheckSystemVersion(8, 2)
                    && !BCL.Tests.TestRuntime.CheckSystemVersion(9, 0)
                )
