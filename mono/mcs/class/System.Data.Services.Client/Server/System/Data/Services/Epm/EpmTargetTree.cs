...
namespace System.Data.Services.Common
{
    ...
    internal sealed class EpmTargetTree
    {
        ...
        {
            get { return this.countOfNonContentProperties == 0; }
        }
        ...
        internal void Add(EntityPropertyMappingInfo epmInfo)
        {
            ...
            EpmTargetPathSegment currentSegment = isSyndication
                ? this.SyndicationRoot
            ...
            Debug.Assert(
                ...
                "Must have been validated during EntityPropertyMappingAttribute construction"
            );
            ...
            for (int i = 0; i < targetSegments.Length; i++)
            {
                ...
                if (targetSegment.Length == 0)
                {
                    throw new InvalidOperationException(
                        Strings.EpmTargetTree_InvalidTargetPath(targetName)
                    );
                }
                ...
                {
                    throw new InvalidOperationException(
                        Strings.EpmTargetTree_AttributeInMiddle(targetSegment)
                    );
                }

                EpmTargetPathSegment foundSegment = activeSubSegments.SingleOrDefault(segment =>
                    segment.SegmentName == targetSegment
                ...
                {
                    currentSegment = new EpmTargetPathSegment(
                        targetSegment,
                        namespaceUri,
                        namespacePrefix,
                        currentSegment
            ...
            if (currentSegment.HasContent)
            {
                throw new ArgumentException(
                    Strings.EpmTargetTree_DuplicateEpmAttrsWithSameTargetName(
                        EpmTargetTree.GetPropertyNameFromEpmInfo(currentSegment.EpmInfo),
                        currentSegment.EpmInfo.DefiningType.Name,
        ...
        internal void Remove(EntityPropertyMappingInfo epmInfo)
        {
            ...
            for (int i = 0; i < targetSegments.Length; i++)
            {
                ...
                EpmTargetPathSegment foundSegment = activeSubSegments.FirstOrDefault(segment =>
                    segment.SegmentName == targetSegment
                    && (isSyndication || segment.SegmentNamespaceUri == namespaceUri)
                );
                if (foundSegment != null)
            ...
            if (currentSegment.HasContent)
            {
                ...
                do
                {
                    ...
                    currentSegment.ParentSegment != null
                    && !currentSegment.HasContent
        ...
        private static bool HasMixedContent(
            EpmTargetPathSegment currentSegment,
            bool ancestorHasContent
        )
        {
            foreach (
                EpmTargetPathSegment childSegment in currentSegment.SubSegments.Where(s =>
                    !s.IsAttribute
                )
            )
            {
        ...
        private static String GetPropertyNameFromEpmInfo(EntityPropertyMappingInfo epmInfo)
        {
            {
                if (
                    epmInfo.Attribute.TargetSyndicationItem
                    != SyndicationItemProperty.CustomProperty
                )
                {
