...
namespace System.Data.Services.Common
{
    ...
    internal sealed class EpmSourceTree
    {
        ...
        internal void Add(EntityPropertyMappingInfo epmInfo)
        {
            ...
            foreach (String propertyName in sourceName.Split('/'))
            {
                if (propertyName.Length == 0)
                {
                    throw new InvalidOperationException(
                        Strings.EpmSourceTree_InvalidSourcePath(
                ...
                foundProperty = activeSubProperties.SingleOrDefault(e =>
            ...
            {
                Debug.Assert(
                    Object.ReferenceEquals(foundProperty, currentProperty),
                    "currentProperty variable should have been updated already to foundProperty"
                );

                if (foundProperty.EpmInfo.DefiningType.Name == epmInfo.DefiningType.Name)
                {
                    throw new InvalidOperationException(
                        Strings.EpmSourceTree_DuplicateEpmAttrsWithSameSourceName(
