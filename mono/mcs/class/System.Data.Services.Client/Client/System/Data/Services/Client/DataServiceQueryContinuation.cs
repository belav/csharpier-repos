    ...
    public abstract class DataServiceQueryContinuation
    {
        ...
        internal static DataServiceQueryContinuation Create(Uri nextLinkUri, ProjectionPlan plan)
        {
            ...
            var constructors = typeof(DataServiceQueryContinuation<>)
                .MakeGenericType(plan.ProjectedType)
                .GetConstructors(BindingFlags.NonPublic | BindingFlags.Instance);
            Debug.Assert(constructors.Length == 1, "constructors.Length == 1");
            object result = Util.ConstructorInvoke(
                constructors[0],
                new object[] { nextLinkUri, plan }
            );
        ...
        {
            QueryComponents result = new QueryComponents(
                this.NextLinkUri,
                Util.DataServiceVersionEmpty,
                this.Plan.LastSegmentType,
                null,
                null
            );
            return result;
