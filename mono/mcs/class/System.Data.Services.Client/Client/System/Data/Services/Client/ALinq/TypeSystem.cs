...
namespace System.Data.Services.Client
{
    ...
    internal static class TypeSystem
    {
...
#if !ASTORIA_LIGHT
        private const string VisualBasicAssemblyFullName =
            "Microsoft.VisualBasic, Version=8.0.0.0, Culture=neutral, PublicKeyToken="
            + AssemblyRef.MicrosoftPublicKeyToken;
#else
        private const string VisualBasicAssemblyFullName =
            "Microsoft.VisualBasic, Version=2.0.5.0, Culture=neutral, PublicKeyToken="
            + AssemblyRef.MicrosoftSilverlightPublicKeyToken;
#endif

        [System.Diagnostics.CodeAnalysis.SuppressMessage(
            "Microsoft.Performance",
            "CA1810:InitializeReferenceTypeStaticFieldsInline",
            Justification = "Cleaner code"
        )]
        static TypeSystem()
        {
#if !ASTORIA_LIGHT
...
#endif
            expressionMethodMap = new Dictionary<MethodInfo, string>(
                ExpectedCount,
                EqualityComparer<MethodInfo>.Default
            );
            expressionMethodMap.Add(
            ...
            expressionMethodMap.Add(
                typeof(Math).GetMethod("Round", new Type[] { typeof(decimal) }),
                @"round"
            );
            expressionMethodMap.Add(
...
#if !ASTORIA_LIGHT
            expressionMethodMap.Add(
                typeof(Math).GetMethod("Floor", new Type[] { typeof(decimal) }),
                @"floor"
            );
#endif
            expressionMethodMap.Add(
                typeof(Math).GetMethod("Ceiling", new Type[] { typeof(double) }),
                @"ceiling"
            );
#if !ASTORIA_LIGHT
            expressionMethodMap.Add(
                typeof(Math).GetMethod("Ceiling", new Type[] { typeof(decimal) }),
                @"ceiling"
            );
#endif

            Debug.Assert(
                expressionMethodMap.Count == ExpectedCount,
                "expressionMethodMap.Count == ExpectedCount"
            );

            expressionVBMethodMap = new Dictionary<string, string>(
            ...
            propertiesAsMethodsMap = new Dictionary<PropertyInfo, MethodInfo>(
        ...
        {
            return (
                expressionMethodMap.TryGetValue(mi, out methodName)
                || (
                    mi.DeclaringType.Assembly.FullName == VisualBasicAssemblyFullName
                    && expressionVBMethodMap.TryGetValue(
