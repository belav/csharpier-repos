...
namespace System.Data.Services.Client
{
    ...
    internal class ExpressionNormalizer : DataServiceExpressionVisitor
    {
        ...
        private readonly Dictionary<Expression, Pattern> _patterns = new Dictionary<
            Expression,
            Pattern
        ...
        internal override Expression VisitBinary(BinaryExpression b)
        {
            ...
            if (visited.NodeType == ExpressionType.Equal)
            {
                ...
                {
                    visited = CreateRelationalOperator(
            ...
            Pattern pattern;
            if (
                _patterns.TryGetValue(visited.Left, out pattern)
                && pattern.Kind == PatternKind.Compare
                && IsConstantZero(visited.Right)
            )
            {
        ...
        internal override Expression VisitUnary(UnaryExpression u)
        {
            ...
            if (
                (
                    visited.NodeType == ExpressionType.Convert
                    || visited.NodeType == ExpressionType.TypeAs
                ) && visited.Type.IsAssignableFrom(visited.Operand.Type)
            )
            {
                if (
                    !ClientConvert.IsKnownNullableType(visited.Operand.Type)
                        && !ClientConvert.IsKnownNullableType(visited.Type)
                    || visited.Operand.Type == visited.Type
                )
                {
                    if (
                        !(
                            ClientType.CheckElementTypeIsEntity(visited.Operand.Type)
        ...
        internal Expression VisitMethodCallNoRewrite(MethodCallExpression call)
        {
            ...
            if (
                ...
                && visited.Method.DeclaringType.FullName
        ...
        private static readonly MethodInfo s_relationalOperatorPlaceholderMethod =
            typeof(ExpressionNormalizer).GetMethod(
