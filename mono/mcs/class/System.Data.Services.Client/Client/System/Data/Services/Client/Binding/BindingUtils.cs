...
namespace System.Data.Services.Client
{
    ...
    internal static class BindingUtils
    {
        internal static void ValidateEntitySetName(string entitySetName, object entity)
        {
            if (String.IsNullOrEmpty(entitySetName))
            {
                throw new InvalidOperationException(
                    Strings.DataBinding_Util_UnknownEntitySetName(entity.GetType().FullName)
                );
            }
        }
        ...
        internal static Type GetCollectionEntityType(Type collectionType)
        {
            while (collectionType != null)
            {
                if (
                    ...
                    && WebUtil.IsDataServiceCollectionType(
        ...
        internal static void VerifyObserverNotPresent<T>(
            object oec,
        ...
        {
            Debug.Assert(
                BindingEntityInfo.IsDataServiceCollection(oec.GetType()),
                "Must be an DataServiceCollection."
            );
            ...
            {
                throw new InvalidOperationException(
                    Strings.DataBinding_CollectionPropertySetterValueHasObserver(
                        sourceProperty,
                        sourceType
                    )
                );
