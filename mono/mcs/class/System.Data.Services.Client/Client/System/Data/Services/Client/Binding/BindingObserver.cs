...
namespace System.Data.Services.Client
{
    ...
    internal sealed class BindingObserver
    {
        ...
        )
        {
            Debug.Assert(
                context != null,
                "Must have been validated during DataServiceCollection construction."
            );
        ...
        internal void StartTracking<T>(
            DataServiceCollection<T> collection,
        ...
        )
        {
            ...
            if (!BindingEntityInfo.IsEntityType(typeof(T)))
            {
                throw new ArgumentException(
                    Strings.DataBinding_DataServiceCollectionArgumentMustHaveEntityType(typeof(T))
                );
            }
...
#if ASTORIA_LIGHT
        internal bool LookupParent<T>(
            DataServiceCollection<T> collection,
            out object parentEntity,
            out string parentProperty
        )
        {
        ...
        internal void OnPropertyChanged(object source, PropertyChangedEventArgs eventArgs)
        {
...
#if DEBUG
            Debug.Assert(
                this.bindingGraph.IsTracking(source),
                "Entity must be part of the graph if it has the event notification registered."
            );
#endif
            ...
            else
            {
                ...
                object sourcePropertyValue = BindingEntityInfo.GetPropertyValue(
                    source,
                    sourceProperty,
                    out bpi
                );
        ...
        )
        {
            ...
            Debug.Assert(
                ...
                "We only register this event for DataServiceCollections."
            );
#if DEBUG
            Debug.Assert(
                this.bindingGraph.IsTracking(collection),
                "Collection must be part of the graph if it has the event notification registered."
            );
#endif
            object source;
            ...
            switch (eventArgs.Action)
            {
                ...
                case NotifyCollectionChangedAction.Remove:
                    this.OnDeleteFromCollection(eventArgs, source, sourceProperty, collection);
                    break;
                ...
                default:
                    throw new InvalidOperationException(
                        Strings.DataBinding_CollectionChangedUnknownAction(eventArgs.Action)
                    );
            }
        }
        ...
        )
        {
            ...
            Debug.Assert(
                (source == null && sourceProperty == null)
                    || (source != null && !String.IsNullOrEmpty(sourceProperty)),
            ...
            bool contextOperationRequired =
                ...
                && (
                    ...
                    || (
                        ...
                        && targetDescriptor.State != EntityStates.Deleted
                    )
                );
            ...
            {
                throw new InvalidOperationException(
                    Strings.DataBinding_BindingOperation_DetachedSource
                );
            }
        ...
        )
        {
            ...
            bool contextOperationRequired =
                this.IsContextTrackingEntity(target) && !this.DetachBehavior;
        ...
        )
        {
            ...
            Debug.Assert(
                BindingEntityInfo.IsEntityType(source.GetType()),
                "source must be an entity with keys."
            );
            Debug.Assert(
                !String.IsNullOrEmpty(sourceProperty),
                "sourceProperty must be a non-empty string for update operations."
            );
            Debug.Assert(
                !String.IsNullOrEmpty(sourceEntitySet),
                "sourceEntitySet must be non-empty string for update operation."
            );
            ...
            EntityDescriptor targetDescriptor =
                target != null ? this.Context.GetEntityDescriptor(target) : null;
        ...
        internal bool IsContextTrackingEntity(object entity)
        {
            Debug.Assert(
                entity != null,
                "entity must be provided when checking for context tracking."
            );
        ...
        {
            Debug.Assert(
                !this.AttachBehavior || this.Context.ApplyingChanges,
            ...
            Debug.Assert(
                BindingEntityInfo.IsEntityType(entity.GetType()),
                "entity must be an entity with keys."
            );
            Debug.Assert(
                !String.IsNullOrEmpty(propertyName) || propertyValue == null,
                "When propertyName is null no propertyValue should be provided."
            );
        ...
        )
        {
            Debug.Assert(
                collection != null,
                "Must have a valid collection to which entities are added."
            );
            ...
            if (eventArgs.NewItems != null)
            {
                foreach (object target in eventArgs.NewItems)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            Strings.DataBinding_BindingOperation_ArrayItemNull("Add")
                        );
                    }
        ...
        )
        {
            Debug.Assert(
                collection != null,
                "Must have a valid collection from which entities are removed."
            );
            Debug.Assert(
                (source == null && sourceProperty == null)
                    || (source != null && !String.IsNullOrEmpty(sourceProperty)),
        ...
        private void DeepRemoveCollection(
            IEnumerable collection,
            object source,
            string sourceProperty,
            Action<object> itemValidator
        )
        {
            foreach (object target in collection)
            {
                ...
                this.CollectUnTrackingInfo(target, source, sourceProperty, untrackingInfo);
        ...
        {
            foreach (
                var ed in this.Context.Entities.Where(x =>
                    x.ParentEntity == currentEntity && x.State == EntityStates.Added
                )
            )
            {
        ...
        private bool IsContextTrackingLink(object source, string sourceProperty, object target)
        {
            ...
            Debug.Assert(
                BindingEntityInfo.IsEntityType(target.GetType()),
                "target must be an entity with keys."
            );

            return this.Context.GetLinkDescriptor(source, sourceProperty, target)
                != default(LinkDescriptor);
        }
        ...
        private void ValidateCollectionItem(object target)
        {
            ...
            {
                throw new InvalidOperationException(
                    Strings.DataBinding_BindingOperation_ArrayItemNull("Remove")
                );
            }
            ...
            {
                throw new InvalidOperationException(
                    Strings.DataBinding_BindingOperation_ArrayItemNotEntity("Remove")
                );
            }
        }
