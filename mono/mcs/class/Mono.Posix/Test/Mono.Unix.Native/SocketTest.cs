...
namespace MonoTests.Mono.Unix.Native
{
    ...
    public class SocketTest
    {
        ...
        void SetTimeout(int socket)
        {
            ...
            if (
                Syscall.setsockopt(
                    ...
                    UnixSocketOptionName.SO_RCVTIMEO,
                    timeout
                ) < 0
                || Syscall.setsockopt(
                    socket,
                    UnixSocketProtocol.SOL_SOCKET,
                    UnixSocketOptionName.SO_SNDTIMEO,
                    timeout
                ) < 0
            )
                ...
                (so1, so2) =>
                {
                    ...
                    if (
                        Syscall.setsockopt(
                            so1,
                            UnixSocketProtocol.SOL_SOCKET,
                            UnixSocketOptionName.SO_LINGER,
                            linger
                        ) < 0
                    )
                        UnixMarshal.ThrowExceptionForLastError();
        ...
        public void SockaddrInTest()
        {
            ...
            Assert.AreEqual(
                "{sin_family=AF_INET, sin_port=htons(5678), sin_addr=127.0.0.1}",
                address1.ToString()
            );
        }
        ...
        public void SockaddrIn6Test()
        {
            ...
            Assert.AreEqual(
                "{sin6_family=AF_INET6, sin6_port=htons(1234), sin6_flowinfo=2, sin6_addr=::1, sin6_scope_id=3}",
                address1.ToString()
            );
        }
        ...
        public void BindConnect()
        {
            WithSockets(
                ...
                UnixSocketProtocol.IPPROTO_UDP,
                (so1, so2) =>
                {
                ...
                (inner_so1, inner_so2) =>
                {
                    WithSocketPair(
                        (so1, so2) =>
                        {
                            ...
                            for (
                                offset = Syscall.CMSG_FIRSTHDR(msghdr2);
                                offset != -1;
                                offset = Syscall.CMSG_NXTHDR(msghdr2, offset)
                            )
                            {
