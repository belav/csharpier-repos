...
namespace MonoTests.System.Windows.Forms
{
    ...
    public class DefaultLayoutTest : TestHelper
    {
        ...
        public void TestDockFillWithPadding()
        {
            ...
            f.Show();
            Assert.AreEqual(
                new Size(f.ClientSize.Width - 30, f.ClientSize.Height - 30),
                c.Size,
                "K1"
            );
    ...
    public class DockingTests : TestHelper
    {
        ...
        public void TestDockFillFirst()
        {
            ...
            Assert.AreEqual(
                new Rectangle(
                    b2.Width,
                    0,
                    f.ClientRectangle.Width - b2.Width,
        ...
        public void TestDockFillLast()
        {
            ...
            f.Show();
            Assert.AreEqual(
                new Rectangle(0, 0, f.ClientRectangle.Width, f.ClientRectangle.Height),
                b1.Bounds,
                "B1"
            );
        ...
        public void ResettingDockToNone()
        {
            ...
            Assert.IsTrue(
                c.Left > 0,
                string.Format("A1: c.Left ({0}) must be greater than 0", c.Left)
            );
            Assert.IsTrue(
                c.Top > 0,
                string.Format("A2: c.Top ({0}) must be greater than 0", c.Top)
            );

            c.Dock = DockStyle.None;
            Assert.IsTrue(
                c.Left > 0,
                string.Format("A3: c.Left ({0}) must be greater than 0", c.Left)
            );
            Assert.IsTrue(
                c.Top > 0,
                string.Format("A4: c.Top ({0}) must be greater than 0", c.Top)
            );

            f.ClientSize = new Size(400, 400);
            Assert.IsTrue(
                c.Left > 70,
                string.Format("A5: c.Left ({0}) must be greater than 70", c.Left)
            );
            Assert.IsTrue(
                c.Top > 70,
                string.Format("A6: c.Top ({0}) must be greater than 70", c.Top)
            );
