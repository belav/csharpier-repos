...
namespace MonoTests.System.Windows.Forms
{
    ...
    public class TreeViewTest : TestHelper
    {
        ...
        public void SelectedNodeNullTest()
        {
            ...
            tv.BeforeSelect += new TreeViewCancelEventHandler(
                delegate(object sender, TreeViewCancelEventArgs e)
                {
                    if (e.Node == null)
                        Assert.Fail("BeforeSelect should not be called with a null node");
                }
            );
            tv.AfterSelect += new TreeViewEventHandler(
                ...
                {
                    if (e.Node == null)
                        Assert.Fail("AfterSelect should not be called with a null node");
                }
            );
        ...
        public void MethodToString()
        {
            ...
            Assert.AreEqual(
                @"System.Windows.Forms.TreeView, Nodes.Count: 2, Nodes[0]: TreeNode: A",
                tv.ToString(),
                "A2"
            );
        }
