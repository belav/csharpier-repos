...
namespace MonoTests.System.Windows.Forms
{
    ...
    public class ImageListTest : TestHelper
    {
        ...
        {
            PropertyDescriptor colordepth_prop = TypeDescriptor.GetProperties(typeof(ImageList))[
                "ColorDepth"
            ];
            PropertyDescriptor imagesize_prop = TypeDescriptor.GetProperties(typeof(ImageList))[
                "ImageSize"
            ];
            PropertyDescriptor transparentcolor_prop = TypeDescriptor.GetProperties(
                typeof(ImageList)
            )["TransparentColor"];
        ...
        public void ToStringMethodTest()
        {
            ImageList myimagelist = new ImageList();
            Assert.AreEqual(
                "System.Windows.Forms.ImageList Images.Count: 0, ImageSize: {Width=16, Height=16}",
                myimagelist.ToString(),
                "#T3"
            );
        }
