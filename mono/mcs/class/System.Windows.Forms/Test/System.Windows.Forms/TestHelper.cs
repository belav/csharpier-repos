...
namespace MonoTests.System.Windows.Forms
{
    public class TestHelper
    {
        ...
        protected virtual void TearDown()
        {
            ...
            {
                Console.WriteLine(
                    "HEY! You created " + c.ToString() + " form(s) and you didn't dispose of them!"
                );
        ...
        public class FormWatcher : Form
        {
            ...
            public FormWatcher(int[] watches)
                : base()
            {
            ...
            protected override void WndProc(ref Message m)
            {
                foreach (int i in watches)
                {
                    ...
                    {
                        Console.WriteLine(
                            (this.Name != "" && this.Name != null ? this.Name : "FormWatcher")
                                + " received message "
                                + m.ToString()
                        );
                        break;
        ...
        public class ControlWatcher : Control
        {
            ...
            public ControlWatcher(int[] watches)
                : base()
            {
                this.watches = watches;
            }
            ...
            protected override void WndProc(ref Message m)
            {
                foreach (int i in watches)
                {
                    ...
                    {
                        Console.WriteLine(
                            (this.Name != "" && this.Name != null ? this.Name : "ControlWatcher")
                                + " received message "
                                + m.ToString()
                        );
                        break;
        ...
        public class ButtonWatcher : Button
        {
            ...
            public ButtonWatcher(int[] watches)
                : base()
            {
                this.watches = watches;
            ...
            protected override void WndProc(ref Message m)
            {
                foreach (int i in watches)
                {
                    ...
                    {
                        Console.WriteLine(
                            (this.Name != "" && this.Name != null ? this.Name : "ButtonWatcher")
                                + " received message "
                                + m.ToString()
                        );
                        break;
            ...
            for (int j = 0; j < sorted.Length; j++)
            {
                ...
                switch (Type.GetTypeCode(value.GetType()))
                {
                    ...
                    case TypeCode.DateTime:
                        code = "new System.DateTime (" + ((DateTime)value).Ticks + ")";
                        break;
                    ...
                    case TypeCode.Object:
                        ...
                        if (value is Exception)
                        {
                            ...
                            writer.WriteLine(
                                tab
                                    + "\tAssert.Fail (\"Expected '{0}', but no exception was thrown.\", \"{1}\");",
                                value.GetType().FullName,
                                message
                            );
                            writer.WriteLine(tab + "}} catch ({0} ex) {{", value.GetType().Name);
                            writer.WriteLine(
                                tab + "\tAssert.AreEqual (@\"{0}\", ex.Message);",
                                ((Exception)value).Message.Replace("\"", "\"\"")
                            );
                            writer.WriteLine(tab + "} catch (Exception ex) {");
                            writer.WriteLine(
                                tab
                                    + "\tAssert.Fail (\"Expected '{0}', got '\" + ex.GetType ().FullName + \"'.\", \"{1}\");",
                                value.GetType().FullName,
                                message
                            );
                        ...
                        else
                        {
                            if (maxrecursive > level)
                            {
                                DumpObject(
                                    value,
                                    ((MemberInfo)members[name]).DeclaringType,
                                    showName + "." + name,
                                    writer,
                                    tabs,
                                    prefix,
