...
namespace MonoTests.System.Resources
{
    ...
    public class ResXResourceReaderTest : MonoTests.System.Windows.Forms.TestHelper
    {
        ...
        public void ExternalFileReference_Icon()
        {
            ...
            File.Copy(
                TestResourceHelper.GetFullPathOfResource("Test/resources/32x32.ico"),
                refFile
            );
        ...
        public void ExternalFileReference_RelativePath()
        {
            ...
            using (StreamWriter sw = new StreamWriter(resxFile, false, Encoding.UTF8))
            {
                sw.Write(
                    string.Format(
                        ...
                        typeof(StreamReader).AssemblyQualifiedName,
                        string.Empty,
                        Consts.AssemblyCorlib
                    )
                );
            ...
            using (ResXResourceReader r = new ResXResourceReader(resxFile))
            {
                ...
                catch (ArgumentException ex)
                {
                    ...
                    Assert.IsTrue(
                        inner.Message.IndexOf(
                            Path.Combine(Directory.GetCurrentDirectory(), relRefFile)
                        ) != -1,
                        "#A12"
                    );
            ...
            using (ResXResourceReader r = new ResXResourceReader(resxFile))
            {
                ...
                catch (ArgumentException ex)
                {
                    ...
                    Assert.IsTrue(
                        inner.Message.IndexOf(Path.Combine(Path.GetTempPath(), relRefFile)) != -1,
                        "#D12"
                    );
                    ...
                    Assert.IsTrue(
                        inner2.Message.IndexOf(Path.Combine(Path.GetTempPath(), relRefFile)) != -1,
                        "#D16"
                    );
                }
        ...
        public void ResHeader_ResMimeType()
        {
            ...
            resXContent = string.Format(
                CultureInfo.InvariantCulture,
                resXTemplate,
                ResXResourceWriter.ResMimeType.ToUpper(),
                "1.0",
                Consts.AssemblySystem_Windows_Forms
            );
            using (ResXResourceReader r = new ResXResourceReader(new StringReader(resXContent)))
            ...
            resXContent = string.Format(
                CultureInfo.InvariantCulture,
                resXTemplate,
                "<![CDATA[" + ResXResourceWriter.ResMimeType + "]]>",
                "1.0",
                Consts.AssemblySystem_Windows_Forms
            );
            using (ResXResourceReader r = new ResXResourceReader(new StringReader(resXContent)))
            ...
            resXContent = string.Format(
                CultureInfo.InvariantCulture,
                resXTemplate,
                "<![CDATA[" + ResXResourceWriter.ResMimeType.ToUpper() + "]]>",
                "1.0",
                Consts.AssemblySystem_Windows_Forms
            );
            using (ResXResourceReader r = new ResXResourceReader(new StringReader(resXContent)))
            ...
            resXContent = string.Format(
                CultureInfo.InvariantCulture,
                resXTemplate,
                "<whatever>" + ResXResourceWriter.ResMimeType + "</whatever>",
                "1.0",
                Consts.AssemblySystem_Windows_Forms
            );
            using (ResXResourceReader r = new ResXResourceReader(new StringReader(resXContent)))
            ...
            resXContent = string.Format(
                CultureInfo.InvariantCulture,
                resXTemplate,
                "<whatever>" + ResXResourceWriter.ResMimeType.ToUpper() + "</whatever>",
                "1.0",
                Consts.AssemblySystem_Windows_Forms
            );
            using (ResXResourceReader r = new ResXResourceReader(new StringReader(resXContent)))
                ...
                while (enumerator.MoveNext())
                {
                    ...
                    switch ((string)enumerator.Key)
                    {
                        case "foo":
                            ...
                            Bitmap bitmap =
                                node.GetValue(
                                    new AssemblyName[] { typeof(Bitmap).Assembly.GetName() }
                                ) as Bitmap;
                        ...
                        case "panel_label.Locked":
                            ...
                            Assert.AreEqual(
                                true,
                                node.GetValue(
                                    new AssemblyName[] { typeof(int).Assembly.GetName() }
                                ),
                                "#B2"
                            );
                            break;
  ...
  <data name=""test"" mimetype=""application/x-microsoft.net.object.binary.base64"">
	<value>
        ...
        private static void WriteEmbeddedResource(string name, string filename)
        {
            ...
            Stream input = typeof(ResXResourceReaderTest).Assembly.GetManifestResourceStream(name);
