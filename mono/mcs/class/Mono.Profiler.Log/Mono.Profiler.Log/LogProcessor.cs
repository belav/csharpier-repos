...
namespace Mono.Profiler.Log
{
    public sealed class LogProcessor
    {
        ...
        public LogProcessor(
            LogStream stream,
            LogEventVisitor immediateVisitor,
            LogEventVisitor sortedVisitor
        )
        {
            if (stream == null)
                    ...
                    switch (extType)
                    {
                        ...
                        case LogEventType.AllocationNoBacktrace:
                            ev = new AllocationEvent
                            {
                                ...
                                VTablePointer =
                                    StreamHeader.FormatVersion >= 15 ? ReadPointer() : 0,
                                ObjectPointer = ReadObject(),
                                ObjectSize = (long)_reader.ReadULeb128(),
                                Backtrace = ReadBacktrace(
                                    extType == LogEventType.AllocationBacktrace
                                ),
                            };
                            break;
                        default:
                    ...
                    switch (extType)
                    {
                        ...
                        case LogEventType.GCHandleCreationBacktrace:
                            ev = new GCHandleCreationEvent
                            {
                                ...
                                ObjectPointer = ReadObject(),
                                Backtrace = ReadBacktrace(
                                    extType == LogEventType.GCHandleCreationBacktrace
                                ),
                            };
                            break;
                        ...
                        case LogEventType.GCHandleDeletionBacktrace:
                            ev = new GCHandleDeletionEvent
                            {
                                ...
                                Handle = (long)_reader.ReadULeb128(),
                                Backtrace = ReadBacktrace(
                                    extType == LogEventType.GCHandleDeletionBacktrace
                                ),
                            };
                            break;
                        ...
                        case LogMetadataType.Image:
                            if (load)
                            {
                                ...
                                if (StreamHeader.FormatVersion >= 16)
                                {
                                    ...
                                    ile.ModuleVersionId =
                                        guid == string.Empty ? Guid.Empty : Guid.Parse(guid);
                                }
                        ...
                        case LogMetadataType.Assembly:
                            if (load)
                            {
                                ev = new AssemblyLoadEvent
                                {
                                    ...
                                    ImagePointer =
                                        StreamHeader.FormatVersion >= 14 ? ReadPointer() : 0,
                                    Name = _reader.ReadCString(),
                                };
                            }
                            else if (unload)
                    ...
                    switch (extType)
                    {
                        ...
                        case LogEventType.ExceptionThrowBacktrace:
                            ev = new ThrowEvent
                            {
                                ObjectPointer = ReadObject(),
                                Backtrace = ReadBacktrace(
                                    extType == LogEventType.ExceptionThrowBacktrace
                                ),
                            };
                            break;
                    ...
                    switch (extType)
                    {
                        ...
                        case LogEventType.HeapObject:
                        {
                            HeapObjectEvent hoe = new HeapObjectEvent
                            {
                                ...
                                ObjectSize = (long)_reader.ReadULeb128(),
                                Generation =
                                    StreamHeader.FormatVersion >= 16 ? _reader.ReadByte() : 0,
                            };

                            var list = new HeapObjectEvent.HeapObjectReference[
                                (int)_reader.ReadULeb128()
                            ];
                ...
                case LogEventType.Sample:
                    switch (extType)
                    {
                        ...
                        case LogEventType.SampleCounterDescriptions:
                        {
                            ...
                            var list = new CounterDescriptionsEvent.CounterDescription[
                                (int)_reader.ReadULeb128()
                            ];
                            ...
                            for (var i = 0; i < list.Length; i++)
                            {
                                ...
                                list[i] = new CounterDescriptionsEvent.CounterDescription
                                {
                                    Section = section,
                                    SectionName =
                                        section == LogCounterSection.User
                                            ? _reader.ReadCString()
                                            : null,
                                    CounterName = _reader.ReadCString(),
                                    Type =
                                        StreamHeader.FormatVersion < 15
                                            ? (LogCounterType)_reader.ReadByte()
                                            : (LogCounterType)_reader.ReadULeb128(),
                                    Unit =
                                        StreamHeader.FormatVersion < 15
                                            ? (LogCounterUnit)_reader.ReadByte()
                                            : (LogCounterUnit)_reader.ReadULeb128(),
                                    Variance =
                                        StreamHeader.FormatVersion < 15
                                            ? (LogCounterVariance)_reader.ReadByte()
                                            : (LogCounterVariance)_reader.ReadULeb128(),
                                    Index = (long)_reader.ReadULeb128(),
                                };
                            }
                            ...
                            while (true)
                            {
                                ...
                                switch (counterType)
                                {
                                    case LogCounterType.String:
                                        value =
                                            _reader.ReadByte() == 1 ? _reader.ReadCString() : null;
                                        break;
                                    ...
                                    default:
                                        throw new LogException(
                                            $"Invalid counter type ({counterType})."
                                        );
                                }

                                list.Add(
                                    new CounterSamplesEvent.CounterSample
                                    {
                                        Index = index,
                                        Type = counterType,
                                        Value = value,
                    ...
                    switch (extType)
                    {
                        case LogEventType.RuntimeJitHelper:
                        {
                            ...
                            ev = new JitHelperEvent
                            {
                                ...
                                Name =
                                    helperType == LogJitHelper.SpecificTrampoline
                                        ? _reader.ReadCString()
                                        : null,
                            };
                            break;
