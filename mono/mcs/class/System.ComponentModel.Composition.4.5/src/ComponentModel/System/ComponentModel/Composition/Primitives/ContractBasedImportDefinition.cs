...
namespace System.ComponentModel.Composition.Primitives
{
    ...
    {
        // Unlike contract name, both metadata and required metadata has a sensible default; set it to an
        // empty
        ...
        ///         <see cref="ImportDefinition.IsPrerequisite"/>, <see
        // cref="ImportDefinition.IsRecomposable"/>
        ...
        ///     The type identity of the export type expected. Use <see
        // cref="AttributedModelServices.GetTypeIdentity(Type)"/>
        ///     to generate a type identity for a given type. If no specific type is required pass <see
        // langword="null"/>.
        /// </param>
        ...
        ///     the exports used to satisfy this import. If no specific <see cref="CreationPolicy"/> is
        // needed
        ...
        /// </summary>
        /// <value>
        ///     A <see cref="string"/> that is generated by <see
        // cref="AttributedModelServices.GetTypeIdentity(Type)"/>
        ///     on the type that this import expects. If the value is <see langword="null"/> then this
        // import
        ...
        /// </summary>
        /// <value>
        ///     An <see cref="IEnumerable{T}"/> of pairs of metadata keys and types of the <see
        // cref="Export"/> required by the
        ...
        public virtual IEnumerable<KeyValuePair<string, Type>> RequiredMetadata
        {
            ...
            {
                Contract.Ensures(
                    Contract.Result<IEnumerable<KeyValuePair<string, Type>>>() != null
                );

                // NOTE : unlike other arguments, we validate this one as late as possible, because its validation
                // may lead to type loading
        ...
        public override Expression<Func<ExportDefinition, bool>> Constraint
        {
            get
            {
                ...
                {
                    this._constraint = ConstraintServices.CreateConstraint(
                        this.ContractName,
                        this.RequiredTypeIdentity,
                        this.RequiredMetadata,
                        this.RequiredCreationPolicy
                    );
                }
        ...
        /// <summary>
        ///     Executes an optimized version of the contraint given by the <see cref="Constraint"/>
        // property
        /// </summary>
        ...
        private bool MatchRequiredMatadata(ExportDefinition definition)
        {
            ...
            foreach (KeyValuePair<string, Type> metadataItem in this.RequiredMetadata)
            {
                ...
                else
                {
                    ...
                    {
                        // this is a pretty expensive check, but we only invoke it when metadata values are null, which is
                        // very rare
                        return false;
                    }
                }
