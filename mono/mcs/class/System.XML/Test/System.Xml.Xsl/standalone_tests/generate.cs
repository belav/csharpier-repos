...
namespace MonoTests.oasis_xslt
{
    public class Generator : IDisposable
    {
        ...
        void Run()
        {
            string resultExceptionsFilename = Path.Combine(
                EnvOptions.OutputDir,
                "res-exceptions.lst"
            );
            ...
            foreach (
                XmlElement testCase in catalog.SelectNodes("test-suite/test-catalog/test-case")
            )
            {
        ...
        void ProcessTestCase(XmlElement testCase)
        {
            ...
            if (stt.Succeeded)
                ...
                resultExceptionsWriter.WriteLine(
                    "{0}\t{1}",
                    testid,
                    stt.Exception.GetType().ToString()
                );
        }
    }
}
