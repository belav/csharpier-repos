...
namespace MonoTests.System.Xml
{
    ...
    public class XmlWriterTests
    {
        ...
        public void WriteNodeXmlDecl()
        {
            ...
            xtr.Read();
            xtw.WriteNode(xtr, false);
            Assert.AreEqual("<?xml version='1.0'?>", writer.ToString());
            xtr.Close();
        }
        ...
        public void WriteNodeEmptyElement()
        {
            ...
            XmlTextReader xtr = new XmlTextReader(sr);
            xtw.WriteNode(xtr, false);
            Assert.AreEqual(xml.Replace("'", "\""), writer.ToString());
            xtr.Close();
        }
        ...
        {
            return new XPathDocument(XmlReader.Create(new StringReader(xml))).CreateNavigator();
        }
        ...
        {
            string xml =
                "<!DOCTYPE root [<!ELEMENT root EMPTY> <!ATTLIST root implicit NMTOKEN 'nam'>]><root attr='val' />";
            XPathNavigator nav = new XPathDocument(new StringReader(xml)).CreateNavigator();
            XmlWriterSettings s = new XmlWriterSettings();
            s.OmitXmlDeclaration = true;
            StringWriter sw = new StringWriter();
        ...
        public void WriteStringDifferentBehavior()
        {
            ...
            xw.Close();
            AssertType.AreEqual(
                "<?xml version='1.0' encoding='utf-16'?><foo></foo>".Replace('\'', '"'),
                sw.ToString()
            );
        }
    ...
    class ConformanceLevelAuto : InvalidWriteState
    {
        ...
        {
            get { return new XmlWriterSettings() { ConformanceLevel = ConformanceLevel.Auto }; }
        }
    }
    ...
    class ConformanceLevelDocument : InvalidWriteState
    {
        public override XmlWriterSettings Settings
        {
            get { return new XmlWriterSettings() { ConformanceLevel = ConformanceLevel.Document }; }
        }
    }
    ...
    class ConformanceLevelFragment : InvalidWriteState
    {
        public override XmlWriterSettings Settings
        {
            get { return new XmlWriterSettings() { ConformanceLevel = ConformanceLevel.Fragment }; }
        }
    }
}
