...
namespace MonoTests.System.Xml
{
    ...
    public class XmlNodeTests
    {
        ...
        {
            document.LoadXml(
                "<root xmlns='urn:default' attr='value' "
        ...
        public void InsertAfter()
        {
            ...
            Assert.AreEqual(
                "good_child",
                docelem.LastChild.PreviousSibling.Name,
                "InsertAfter.Next"
            );
        ...
        private void OnChange(object o, XmlNodeChangedEventArgs e)
        {
            switch (normalize2Count)
            {
                ...
                default:
                    Assert.Fail(
                        String.Format(
                            "Unexpected event. Action = {0}, node type = {1}, node name = {2}, node value = {3}",
                            e.Action,
                            e.Node.NodeType,
                            e.Node.Name,
                            e.Node.Value
        ...
        {
            document.LoadXml(
                "<root><c1 xmlns='urn:foo'><c2 xmlns:foo='urn:foo' xmlns='urn:bar'><c3 xmlns=''/></c2></c1></root>"
            );
            Assert.AreEqual(
                String.Empty,
                document.DocumentElement.GetPrefixOfNamespace("urn:foo"),
                "root"
            );
            Assert.AreEqual(
                String.Empty,
            ...
            Assert.AreEqual(
                String.Empty,
                document.DocumentElement.FirstChild.GetPrefixOfNamespace("urn:foo"),
                "c2"
            );
            Assert.AreEqual(
                "foo",
                document.DocumentElement.FirstChild.FirstChild.GetPrefixOfNamespace("urn:foo"),
                "c3"
            );
        ...
        {
            document.LoadXml(
                "<root>This is <b>mixed</b> content. Also includes <![CDATA[CDATA section]]>.<!-- Should be ignored --></root>"
            );
        ...
        public void InnerXmlWithXmlns()
        {
            ...
            string expected =
                "<KeyValue xmlns=\"http://www.w3.org/2000/09/xmldsig#\"><DSAKeyValue>blablabla</DSAKeyValue></KeyValue>";
