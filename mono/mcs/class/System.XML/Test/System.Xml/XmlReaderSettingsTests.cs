...
namespace MonoTests.System.Xml
{
    ...
    public class XmlReaderSettingsTests
    {
        ...
        public void DefaultValue()
        {
            ...
            Assert.IsTrue(
                0 == (s.ValidationFlags & ValidationFlags.ProcessInlineSchema),
                "ProcessInlineSchema"
            );
            Assert.AreEqual(false, s.IgnoreProcessingInstructions, "IgnorePI");
            Assert.IsTrue(
                0 == (s.ValidationFlags & ValidationFlags.ProcessSchemaLocation),
                "ProcessSchemaLocation"
            );
            Assert.IsTrue(
                0 == (s.ValidationFlags & ValidationFlags.ReportValidationWarnings),
                "ReportValidationWarnings"
            );
            Assert.IsTrue(
                0 != (s.ValidationFlags & ValidationFlags.ProcessIdentityConstraints),
                "ProcessIdentityConstraints"
            );
            // No one should use this flag BTW if someone wants
            // code to be conformant to W3C XML Schema standard.
            Assert.IsTrue(
                0 != (s.ValidationFlags & ValidationFlags.AllowXmlAttributes),
                "AllowXmlAttributes"
            );
        ...
        {
            StringReader sr = new StringReader("<root attr='   value   '>test\rstring</root>");
        ...
        public void CheckCharactersAndNormalization()
        {
            ...
            // is set by default).
            StringReader sr = new StringReader("<root attr='&#0;'>&#x0;</root>");
            XmlReaderSettings settings = new XmlReaderSettings();
            settings.CheckCharacters = false;
            XmlReader xtr = XmlReader.Create(sr, settings);
        ...
        public void CreateAndSettings()
        {
            ...
            Assert.IsNotNull(
                XmlReader
                    .Create(TestResourceHelper.GetFullPathOfResource("Test/XmlFiles/simple.xml"))
                    .Settings
            );
        }
        ...
        public void CreateAndNameTable()
        {
            ...
            XmlReaderSettings s = new XmlReaderSettings();
            XmlReader.Create(new StringReader("<root/>"), s, String.Empty).Read();
            XmlReader.Create(new StringReader("<root/>"), s, (XmlParserContext)null).Read();
            XmlReader.Create(CreateStream("<root/>"), s, String.Empty).Read();
        ...
        public void CreateConformanceDocument()
        {
            ...
            XmlReader xr = XmlReader.Create(new StringReader("<foo/><bar/>"), s);
            while (!xr.EOF)
                xr.Read();
        }
        ...
        public void CreateConformanceFragment()
        {
            ...
            XmlReader xr = XmlReader.Create(new StringReader("<foo/><bar/>"), settings);
            while (!xr.EOF)
                xr.Read();
        }
        ...
        public void CreateKeepConformance()
        {
            ...
            settings.ConformanceLevel = ConformanceLevel.Fragment;
            xr = XmlReader.Create(XmlReader.Create(new StringReader("<foo/>"), settings), settings);
            while (!xr.EOF)
                xr.Read();
        ...
        public void ValidationEventHandler()
        {
            ...
            XmlReader r = XmlReader.Create(new StringReader("<root/>"), settings);
            while (!r.EOF)
                r.Read();
        }
        ...
        public void LegacyXmlSettingsAreDisabled()
        {
            ...
            var enableLegacyXmlSettingsMethod = typeof(XmlReaderSettings).GetMethod(
