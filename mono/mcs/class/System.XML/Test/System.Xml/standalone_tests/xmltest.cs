...
public class Test
{
    ...
    {
        Console.WriteLine(
            "WARNING: This test code is outdated. Use Test/System.Xml/W3C/xmlconf.exe instead."
        );
    ...
    static void RunOASISTest()
    {
        ...
        foreach (
            FileInfo fi in new DirectoryInfo(@"xml-test-suite/xmlconf/oasis").GetFiles("*.xml")
        )
        {
            try
            {
            ...
            foreach (FileInfo fi in di.GetFiles("*.xml"))
            {
                try
                {
                    ...
                    while (!xr.EOF)
                        xr.Read();
                    Console.WriteLine(
                        "Incorrectly valid: " + subdir + "/" + di.Name + "/" + fi.Name
                    );
                }
