...
public class XsdTest
{
    ...
    static ValidationEventHandler noValidateHandler = new ValidationEventHandler(NoValidate);
...
",
            ...
            switch (s)
            {
                ...
                default:
                    if (s.StartsWith("--report:"))
                        ReportWriter = new StreamWriter(s.Substring(9));
                    else
                        specificTarget = s;
                    break;
            }
    ...
    void RunTest(string subdir)
    {
        ...
        foreach (XmlElement test in doc.SelectNodes("/tests/test"))
        {
            ...
            if (specificTarget != null && schemaFile.IndexOf(specificTarget) < 0)
                continue;
            ...
            try
            {
                ...
                schema.Compile(
                    noValidate ? noValidateHandler : null,
                    noResolver ? null : new XmlUrlResolver()
                );
            ...
            catch (XmlSchemaException ex)
            {
                if (isValidSchema)
                    Report(
                        ...
                        true,
                        "should succeed",
                        reportDetails ? ex.ToString() : ex.Message
                    );
            ...
            catch (Exception ex)
            {
                if (stopOnError)
                    throw;
                Report(schemaFile, true, "unexpected", reportDetails ? ex.ToString() : ex.Message);
                continue;
            }
            finally
            {
            ...
            try
            {
                XmlTextReader ixtr = new XmlTextReader(Path.Combine(basePath, instanceFile));
                xvr = ixtr;
                if (version2)
                {
                    ...
                    if (noValidate)
                        settings.ValidationEventHandler += noValidateHandler;
                    if (noResolver)
    ...
    void Report(string id, bool compile, string category, string s)
    {
        ...
        if (reportAsXml)
        {
            ...
            ReportWriter.WriteLine("{0}/{1} : {2} {3}", phase, category, id, s);
    }
}
