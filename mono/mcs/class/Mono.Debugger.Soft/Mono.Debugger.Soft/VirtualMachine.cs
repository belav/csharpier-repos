...
namespace Mono.Debugger.Soft
{
    public class VirtualMachine : Mirror
    {
        ...
        public PrimitiveValue CreateValue(object value)
        {
            ...
            if (!value.GetType().IsPrimitive)
                throw new ArgumentException(
                    "value must be of a primitive type instead of '" + value.GetType() + "'",
                    "value"
                );
        ...
        public ExceptionEventRequest CreateExceptionRequest(
            TypeMirror exc_type,
            bool caught,
            bool uncaught
        )
        ...
        )
        {
            if (Version.AtLeast(2, 54))
                return new ExceptionEventRequest(
                    this,
                    exc_type,
                    caught,
                    uncaught,
        ...
        public void EnableEvents(EventType[] events, SuspendPolicy suspendPolicy)
        {
            foreach (EventType etype in events)
            {
                if (etype == EventType.Breakpoint)
                    throw new ArgumentException(
                        "Breakpoint events cannot be requested using EnableEvents",
                        "events"
                    );
        ...
        internal void connect()
        {
            ...
            if (version.MajorVersion != Connection.MAJOR_VERSION)
                throw new NotSupportedException(
                    String.Format(
                        "The debuggee implements protocol version {0}.{1}, while {2}.{3} is required.",
                ...
                case EventType.VMStart:
                    ...
                    queue_event_set(
                        new EventSet(
                            this,
                            spolicy,
                            new Event[] { new VMStartEvent(vm, req_id, thread_id) }
                        )
                    );
                    break;
                case EventType.VMDeath:
                    queue_event_set(
                        new EventSet(
                            this,
                            spolicy,
                            new Event[] { new VMDeathEvent(vm, req_id, exit_code) }
                        )
                    );
                    break;
                case EventType.VMDisconnect:
                    queue_event_set(
                        new EventSet(
                            this,
                            spolicy,
                            new Event[] { new VMDisconnectEvent(vm, req_id) }
                        )
                    );
                    break;
                default:
        ...
        */
        ...
        internal T TryGetObject<T>(long id)
            where T : ObjectMirror
        {
            lock (objects_lock)
            {
                if (objects == null)
        ...
        internal T GetObject<T>(long id, long domain_id, long type_id)
            where T : ObjectMirror
        {
        ...
        internal T GetObject<T>(long id)
            where T : ObjectMirror
        {
        ...
        internal Value DecodeValue(ValueImpl v, Dictionary<int, Value> parent_vtypes)
        {
            ...
            {
                if (
                    Version.AtLeast(2, 46)
                    && (v.Type == ElementType.Ptr || v.Type == ElementType.FnPtr)
                )
        ...
        internal ValueImpl EncodeValue(Value v, List<Value> duplicates = null)
        {
            if (v is PrimitiveValue)
            {
                ...
                if (val == null)
                    ...
                    {
                        Type = (ElementType)ValueTypeId.VALUE_TYPE_ID_NULL,
                        Objid = 0,
                    };
                else
            ...
            else if (v is StructMirror)
            {
                ...
                return new ValueImpl
                {
                    ...
                    Klass = (v as StructMirror).Type.Id,
                    Fields = EncodeFieldValues(
                        (v as StructMirror).Fields,
                        (v as StructMirror).Type.GetFields(),
                        duplicates,
                        1
                    ),
                };
        ...
        internal ValueImpl[] EncodeFieldValues(
            IList<Value> values,
            FieldInfoMirror[] field_info,
            List<Value> duplicates,
            int fixedSize
        )
        {
            ValueImpl[] res = new ValueImpl[values.Count];
            for (int i = 0; i < values.Count; ++i)
            {
                if (fixedSize > 1 || field_info[i].FixedSize > 1)
                    res[i] = EncodeValueFixedSize(
                        values[i],
                        duplicates,
                        fixedSize > 1 ? fixedSize : field_info[i].FixedSize
                    );
                else
                    res[i] = EncodeValue(values[i], duplicates);
            }
    ...
    {
        internal CommandException(ErrorCode error_code, string error_message)
            : base(
                "Debuggee returned error code "
                    + error_code
                    + (
                        error_message == null || error_message.Length == 0
                            ? "."
                            : " - " + error_message + "."
                    )
            )
