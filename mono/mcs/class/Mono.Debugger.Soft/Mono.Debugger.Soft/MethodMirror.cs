    ...
    public class MethodMirror : Mirror
    {
        ...
        public bool IsConstructor
        {
            get
            {
                ...
                return ((attr & (int)MethodAttributes.RTSpecialName) != 0 && (Name == ".ctor"));
            }
        }
        ...
        public ParameterInfoMirror[] GetParameters()
        {
            if (param_info == null)
            {
                ...
                // Return
                ret_param = new ParameterInfoMirror(
                    this,
                    -1,
                    vm.GetType(pi.ret_type),
                    null,
                ...
                {
                    param_info[i] = new ParameterInfoMirror(
                        this,
                        i,
                        vm.GetType(pi.param_types[i]),
                        pi.param_names[i],
                        0
                    );
                }
        ...
        public LocalVariable[] GetLocals()
        {
            if (locals == null)
            {
                ...
                for (int i = 0; i < li.names.Length; ++i)
                    locals[i + pi.Length] = new LocalVariable(
                        vm,
                        this,
                        i,
                        li.types[i],
                        li.names[i],
                        li.live_range_start[i],
                        li.live_range_end[i],
                        false
                    );
                ...
                if (locals[i].Name == name)
                {
                    if (res != null)
                        throw new AmbiguousMatchException(
                            "More that one local has the name '" + name + "'."
                        );
                    res = locals[i];
                }
            }
        ...
        public string SourceFile
        {
            get
            {
                if (debug_info == null)
                    debug_info = vm.conn.Method_GetDebugInfo(id);
                return debug_info.source_files.Length > 0
                    ? debug_info.source_files[0].source_file
                    : null;
            }
        }
                ...
                if (locations == null)
                {
                    ...
                    for (int i = 0; i < il_offsets.Count; ++i)
                        res[i] = new Location(
                            ...
                            -1,
                            il_offsets[i],
                            debug_info.source_files[i].source_file,
                            line_numbers[i],
                            debug_info.column_numbers[i],
                            debug_info.end_line_numbers[i],
                            debug_info.end_column_numbers[i],
                            debug_info.source_files[i].hash
                        );
                    locations = res;
                }
                return locations;
        ...
        internal int il_offset_to_line_number(
            int il_offset,
            out string src_file,
            out byte[] src_hash,
            out int column_number,
            out int end_line_number,
            out int end_column_number
        )
        {
            if (debug_info == null)
                debug_info = vm.conn.Method_GetDebugInfo(id);
        ...
        public C.MethodDefinition Metadata
        {
            ...
            {
                if (meta == null)
                    meta = (C.MethodDefinition)
                        DeclaringType.Assembly.Metadata.MainModule.LookupToken(MetadataToken);
                return meta;
            }
        }
#endif
