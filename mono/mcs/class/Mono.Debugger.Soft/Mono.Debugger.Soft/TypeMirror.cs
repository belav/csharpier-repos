    ...
    public class TypeMirror : Mirror, IInvokable
    {
        ...
        public bool IsNested
        {
            get
            {
                ...
                return masked >= TypeAttributes.NestedPublic
                    && masked <= TypeAttributes.NestedFamORAssem;
            }
        }
        ...
        public FieldInfoMirror[] GetFields()
        {
            ...
            for (int i = 0; i < res.Length; ++i)
                res[i] = new FieldInfoMirror(
                    this,
                    ids[i],
                    names[i],
                    vm.GetType(types[i]),
                    (FieldAttributes)attrs[i]
                );
        ...
        public PropertyInfoMirror[] GetProperties(BindingFlags bindingAttr)
        {
            ...
            for (int i = 0; i < res.Length; ++i)
                res[i] = new PropertyInfoMirror(
                    this,
                    info[i].id,
                    info[i].name,
                    vm.GetMethod(info[i].get_method),
                    vm.GetMethod(info[i].set_method),
        ...
        public Value[] GetValues(IList<FieldInfoMirror> fields, ThreadMirror thread)
        {
            ...
            try
            {
                return vm.DecodeValues(
                    vm.conn.Type_GetValues(id, ids, thread != null ? thread.Id : 0)
                );
            }
            ...
            get
            {
                if (meta == null)
                {
                    if (Assembly.Metadata == null || MetadataToken == 0)
                        return null;
                    meta = (C.TypeDefinition)
                        Assembly.Metadata.MainModule.LookupToken(MetadataToken);
                }
                return meta;
            }
        }
        ...
        void AppendCustomAttrs(
            IList<CustomAttributeDataMirror> attrs,
            TypeMirror type,
            bool inherit
        )
        {
        ...
        public MethodMirror[] GetMethodsByNameFlags(
            string name,
            BindingFlags flags,
            bool ignoreCase
        )
        ...
        {
            return ObjectMirror.InvokeMethod(
                vm,
                thread,
                method,
                null,
        ...
        {
            return ObjectMirror.BeginInvokeMethod(
                vm,
                thread,
                method,
                null,
        ...
        {
            return ObjectMirror.InvokeMethodAsyncWithResult(
                vm,
                thread,
                method,
                null,
        ...
        public InterfaceMappingMirror GetInterfaceMap(TypeMirror interfaceType)
        {
            ...
            if (iface_map == null)
            {
                ...
                for (int i = 0; i < ifacemap.Length; ++i)
                {
                    ...
                    InterfaceMappingMirror map = new InterfaceMappingMirror(
                        vm,
                        this,
                        vm.GetType(info.iface_id),
                        imethods,
                        tmethods
                    );
