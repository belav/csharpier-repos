...
namespace Mono.Debugger.Soft
{
    ...
    public class EnumMirror : StructMirror
    {
        ...
        internal EnumMirror(VirtualMachine vm, TypeMirror type, PrimitiveValue value)
            : base(vm, type, new Value[] { value })
        {
            if (type == null)
            ...
            if (
                value.Value == null
                || !value.Value.GetType().IsPrimitive
                || t.Name != vm.RootDomain.GetCorrespondingType(value.Value.GetType()).Name
            )
                throw new ArgumentException(
                    "Value '" + value.Value + "' does not match the type of the enum."
                );
        }
