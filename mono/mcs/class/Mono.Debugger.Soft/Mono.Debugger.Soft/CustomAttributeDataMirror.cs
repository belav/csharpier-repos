    ...
    public sealed class CustomAttributeDataMirror
    {
        ...
        )
        {
            ...
            this.ctorArgs = Array.AsReadOnly<CustomAttributeTypedArgumentMirror>(
                ctorArgs != null
                    ? UnboxValues<CustomAttributeTypedArgumentMirror>(ctorArgs)
                    : new CustomAttributeTypedArgumentMirror[0]
            );

            this.namedArgs = Array.AsReadOnly<CustomAttributeNamedArgumentMirror>(
                namedArgs != null
                    ? UnboxValues<CustomAttributeNamedArgumentMirror>(namedArgs)
                    : new CustomAttributeNamedArgumentMirror[0]
            );
        }
                ...
                for (int j = 0; j < attr.named_args.Length; ++j)
                {
                    ...
                    while (named_arg == null && t != null)
                    {
                        if (arg.is_property)
                        {
                            ...
                            {
                                if (prop.Id == arg.id)
                                    named_arg = new CustomAttributeNamedArgumentMirror(
                                        prop,
                                        null,
                                        val
                                    );
                        ...
                        else if (vm.Version.AtLeast(2, 12))
                        { // we don't have the field ID before 2.12
