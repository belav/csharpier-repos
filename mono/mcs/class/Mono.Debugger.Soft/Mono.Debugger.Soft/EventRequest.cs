                    ...
                    foreach (var ass in value)
                        if (ass == null)
                            throw new ArgumentException(
                                "one of the elements of the array is null."
                            );
                }
        ...
        internal void SendReq(List<Modifier> mods)
        {
            if (!enabled)
            {
                ...
                if (AssemblyFilter != null)
                    mods.Add(
                        new AssemblyModifier()
                        {
                            Assemblies = AssemblyFilter.Select(x => x.Id).ToArray(),
                        }
                    );
        ...
        protected void CheckDisabled()
        {
            if (Enabled)
                throw new InvalidOperationException(
                    "Request objects can only be modified while they are disabled."
                );
        }
