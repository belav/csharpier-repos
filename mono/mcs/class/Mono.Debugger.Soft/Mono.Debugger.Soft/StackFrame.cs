            ...
            get
            {
                if (location == null)
                {
                    ...
                    if (il_offset == -1)
                        ...
                        line_number = method.il_offset_to_line_number(
                            il_offset,
                            out src_file,
                            out hash,
                            out column_number,
                            out end_line_number,
                            out end_column_number
                        );
                    ...
                    location = new Location(
                        ...
                        0,
                        il_offset,
                        src_file != null ? src_file : method.SourceFile,
                        line_number,
                        column_number,
                        end_line_number,
                        end_column_number,
        ...
        public Value GetValue(LocalVariable var)
        {
            ...
            if (var.Method != Method)
                throw new ArgumentException(
                    "Local variable doesn't belong to this frame's method."
                );
        ...
        public void SetThis(Value value)
        {
            ...
            if (Method.IsStatic || !Method.DeclaringType.IsValueType)
                throw new InvalidOperationException(
                    "The frame's method needs to be a valuetype instance method."
                );
        ...
        public void SetValue(LocalVariable var, Value value)
        {
            ...
            catch (CommandException ex)
            {
                if (ex.ErrorCode == ErrorCode.INVALID_ARGUMENT)
                    throw new ArgumentException(
                        "Value does not match the type of the local variable."
                    );
                else
                    throw;
            }
