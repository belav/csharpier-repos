...
namespace System.Runtime.Serialization.Json
{
    internal partial class JsonFormatReaderGenerator
    {
        ...
        {
            internal JsonFormatClassReaderDelegate GenerateClassReader(
                ClassDataContract classContract
            )
            {
                return (
                    XmlReaderDelegator xr,
                    XmlObjectSerializerReadContextComplexJson ctx,
                    XmlDictionaryString emptyDictionaryString,
                    XmlDictionaryString[] memberNames
                ) =>
                    new JsonFormatReaderInterpreter(classContract).ReadFromJson(
                        xr,
                        ctx,
            ...
            internal JsonFormatCollectionReaderDelegate GenerateCollectionReader(
            ...
            )
            {
                return (
                    XmlReaderDelegator xr,
                    XmlObjectSerializerReadContextComplexJson ctx,
                    XmlDictionaryString emptyDS,
                    XmlDictionaryString inm,
                    CollectionDataContract cc
                ) =>
                    new JsonFormatReaderInterpreter(
                        collectionContract,
                        false
                    ).ReadCollectionFromJson(xr, ctx, emptyDS, inm, cc);
            }

            internal JsonFormatGetOnlyCollectionReaderDelegate GenerateGetOnlyCollectionReader(
                CollectionDataContract collectionContract
            )
            {
                return (
                    ...
                    new JsonFormatReaderInterpreter(
                        collectionContract,
                        true
                    ).ReadGetOnlyCollectionFromJson(xr, ctx, emptyDS, inm, cc);
            }
        }
    }
    ...
    class JsonFormatReaderInterpreter
    {
        ...
        public JsonFormatReaderInterpreter(
            CollectionDataContract collectionContract,
            bool isGetOnly
        )
        {
        ...
        public object ReadCollectionFromJson(
            XmlReaderDelegator xmlReader,
            XmlObjectSerializerReadContextComplexJson context,
            XmlDictionaryString emptyDictionaryString,
        ...
        public void ReadGetOnlyCollectionFromJson(
            XmlReaderDelegator xmlReader,
            XmlObjectSerializerReadContextComplexJson context,
            XmlDictionaryString emptyDictionaryString,
            XmlDictionaryString itemName,
        ...
        bool InvokeFactoryMethod(ClassDataContract classContract)
        {
            ...
            {
                objectLocal = CodeInterpreter.ConvertValue(
                    context.GetRealObject((IObjectReference)objectLocal, Globals.NewObjectId),
                    Globals.TypeOfObject,
                    classContract.UnderlyingType
                );
                return true;
        ...
        void ReadMembers(ClassDataContract classContract, ExtensionDataObject extensionData)
        {
            ...
            while (XmlObjectSerializerReadContext.MoveToNextElement(xmlReader))
            {
                ...
                idx = context.GetJsonMemberIndex(
                    xmlReader,
                    memberNames,
                    memberIndex,
                    extensionData
            ...
            if (!CheckRequiredElements(expectedElements, requiredElements))
                XmlObjectSerializerReadContextComplexJson.ThrowMissingRequiredMembers(
                    objectLocal,
                    memberNames,
        ...
        )
        {
            ...
            if (memberCount <= index && index < memberCount + classContract.Members.Count)
            {
                ...
                if (!expectedElements.Load(index))
                    XmlObjectSerializerReadContextComplexJson.ThrowDuplicateMemberException(
                        objectLocal,
                        memberNames,
                        memberIndex
                    );
                ...
                else
                {
                    ...
                    if (type.IsValueType)
                        ...
                        value = CodeInterpreter.ConvertValue(
                            context.GetExistingObject(objectId, type, name, string.Empty),
                            Globals.TypeOfObject,
                            type
                        );
                }
            ...
            else
            {
                ...
                while ((nodeType = xmlReader.MoveToContent()) != XmlNodeType.EndElement)
                {
                    ...
                    if (keyParseMode == KeyParseMode.AsString)
                        ...
                        key = keyDataContract.ParseMethod.Invoke(
                            null,
                            new object[] { jsonMemberName }
                        );

                    if (keyTypeNullableDepth > 0)
                    {
                        var keyOriginal = WrapNullableObject(
                            keyType,
                            key,
                            valueType,
                            keyTypeNullableDepth
        ...
        bool TryReadPrimitiveArray(Type itemType, out bool readResult)
        {
            ...
            {
                var mi = typeof(JsonReaderDelegator).GetMethod(
                    readArrayMethod,
                    Globals.ScanAllMembers
                );
                var args = new object[]
        ...
        object ReadCollectionItem(CollectionDataContract collectionContract, Type itemType)
        {
            ...
            )
            {
                ...
                var v = DataContractJsonSerializer.ReadJsonValue(
                    revisedContract,
                    xmlReader,
                    context
                );
