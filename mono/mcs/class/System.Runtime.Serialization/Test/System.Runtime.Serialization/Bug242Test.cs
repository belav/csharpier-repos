...
namespace MonoTests.System.Runtime.Serialization
{
    ...
    public class Bug242Test
    {
        ...
        public void TestMixListArraySerialize()
        {
            ...
            Assert.AreEqual(
                clonedData.DataArray.Length,
                data.DataArray.Length,
                "#1 clonedData.DataArray.Length"
            );
            Assert.AreEqual(
                clonedData.DataList.Count,
                data.DataList.Count,
                "#2 clonedData.DataList.Count"
            );
            Assert.AreEqual(
                clonedData.DataIList.Count,
                data.DataIList.Count,
                "#3 clonedData.DataIList.Count"
            );
            Assert.AreEqual(
                clonedData.DataICollection.Count,
                data.DataICollection.Count,
                "#4 clonedData.DataICollection.Count"
            );
            Assert.AreEqual(
                clonedData.DataIEnumerable.Count(),
                data.DataIEnumerable.Count(),
                "#5 clonedData.DataIEnumerable.Count()"
            );
        }
    }
}
