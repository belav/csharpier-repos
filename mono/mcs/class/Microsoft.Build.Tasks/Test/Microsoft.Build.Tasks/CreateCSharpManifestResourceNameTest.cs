...
namespace MonoTests.Microsoft.Build.Tasks
{
    ...
    public class CreateCSharpManifestResourceNameTest
    {
        string[,] resx_no_culture_files,
            resx_with_culture_files;
        string[,] non_resx_no_culture_files,
            non_resx_with_culture_files;

        public CreateCSharpManifestResourceNameTest()
        {
            string sample_cs_path = TestResourceHelper.GetFullPathOfResource(
            ...
            resx_with_culture_files = new string[,]
            {
                ...
                { "foo with space.de.resx", "RandomName", junk_file, "bar with space.fr.resx" },
                {
                    TestResourceHelper.GetFullPathOfResource(
                        "Test/resources/foo with space.de.resx"
                    ),
                    null,
                    "Sample.cs",
                    null,
        ...
        public void TestNoRootNamespaceWithCulture()
        {
            CheckResourceNames(
                ...
                {
                    // With dependent file
                    "Mono.Tests.Sample.de",
                    "Mono.Tests.Sample.de",
                    "Mono.Tests.Sample.fr",
                    "bar with space.fr",
                    "Mono.Tests.Sample.de",
                    "Mono.Tests.Sample.de",
                    // W/o dependent file
        ...
        public void TestInvalidCulture()
        {
            ...
            CheckResourceNames(
                files,
                new string[] { "RN1.RN2.Foo.invalid.txt", "RN1.RN2.Foo.invalid" },
                "RN1.RN2"
            );
        }
        ...
        {
            Assert.AreEqual(
                files.GetUpperBound(0) + 1,
                names.Length,
                "Number of files and names must match"
            );
            ...
            for (int i = 0; i <= files.GetUpperBound(0); i++)
            {
                ...
                Assert.AreEqual(
                    files[i, 1] != null,
                    group[i].HasMetadata("LogicalName"),
                    "A4 #" + (i + 1)
                );
                if (files[i, 1] != null)
                    Assert.AreEqual(
                        files[i, 1],
                        group[i].GetMetadata("LogicalName"),
                        "A5 #" + (i + 1)
                    );
                Assert.AreEqual(
                    files[i, 2] != null,
                    group[i].HasMetadata("DependentUpon"),
                    "A6 #" + (i + 1)
                );
                if (files[i, 2] != null)
                    Assert.AreEqual(
                        files[i, 2],
                        group[i].GetMetadata("DependentUpon"),
                        "A7 #" + (i + 1)
                    );
                if (has_targetpaths && files[i, 3] != null)
                    Assert.AreEqual(
                        files[i, 3],
                        group[i].GetMetadata("TargetPath"),
                        "A8 #" + (i + 1)
                    );
            }
        }
