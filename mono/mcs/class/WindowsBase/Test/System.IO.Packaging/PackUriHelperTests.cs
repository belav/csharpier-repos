...
namespace MonoTests.System.IO.Packaging
{
    ...
    public class PackUriHelperTests
    {
        ...
        {
            Assert.AreEqual(
                "pack://http:,,www.test.com,pack.pkg/",
                PackUriHelper.Create(new Uri("http://www.test.com/pack.pkg")).ToString(),
                "#1"
            );
            Assert.AreEqual(
                "pack://http:,,www.test.com,pack.pkg/",
                PackUriHelper
                    .Create(new Uri("http://www.test.com/pack.pkg"), null, null)
                    .ToString(),
                "#2"
            );
            Assert.AreEqual(
                "pack://http:,,www.test.com,pack.pkg/main.html#frag",
                PackUriHelper
                    .Create(
                        new Uri("http://www.test.com/pack.pkg"),
                        new Uri("/main.html", UriKind.Relative),
                        "#frag"
                    )
                    .ToString(),
                "#3"
        ...
        public void CreateTest2()
        {
            ...
            Assert.AreEqual(
                "pack://pack:,,http:%2C%2Cwww.test.com%2Cpack1.pkg,/",
                PackUriHelper.Create(uri).ToString()
            );
        }
        ...
        public void CreatePartUri()
        {
            ...
            Assert.AreEqual(
                new Uri(part1.ToString(), UriKind.Relative),
                PackUriHelper.CreatePartUri(part1),
                "#2"
            );
        }
        ...
        {
            Assert.AreEqual(
                new Uri(a.ToString().ToUpperInvariant(), UriKind.Relative),
                PackUriHelper.GetNormalizedPartUri(a)
            );
        }
        ...
        public void GetPackageUriTest()
        {
            Assert.AreEqual(
                ...
                PackUriHelper.GetPackageUri(
                    PackUriHelper.Create(a, new Uri("/test.html", UriKind.Relative))
                )
            );
        }
        ...
        public void GetPartUriTest()
        {
            ...
            var pack_part = new Uri(
                @"pack://pack:,,http:%2C%2Cwww.test.com%2Cpack1.pkg,/main.html"
            );
        ...
        public void GetRelationshipPartUriTest()
        {
            Assert.AreEqual(
                "/_rels/file1.rels",
                PackUriHelper.GetRelationshipPartUri(part1).ToString()
            );
        }
        ...
        public void GetRelativeUriTest()
        {
            ...
            Assert.AreEqual(
                ...
                PackUriHelper.GetRelativeUri(src, dest),
                "#1"
            );

            dest = new Uri("/1/2", UriKind.Relative);
            Assert.AreEqual(
                new Uri("../../2", UriKind.Relative),
                PackUriHelper.GetRelativeUri(src, dest),
                "#2"
            );
            ...
            Assert.AreEqual(
                ...
                PackUriHelper.GetRelativeUri(src, src),
                "#4"
            );
            ...
            Assert.AreEqual(
                new Uri("../images/picture.jpg", UriKind.Relative),
                PackUriHelper.GetRelativeUri(src, dest),
                "#7"
            );
        }
        ...
        {
            Assert.IsFalse(
                PackUriHelper.IsRelationshipPartUri(new Uri("/_rels/Whatever", UriKind.Relative))
            );
            Assert.IsTrue(
                PackUriHelper.IsRelationshipPartUri(
                    new Uri("/_rels/Whatever.rels", UriKind.Relative)
                )
            );
        }
        ...
        public void ResolvePartUri2()
        {
            ...
            // Can't be empty url
            Assert.AreEqual(
                new Uri("/1/2/MyFile", UriKind.Relative),
                PackUriHelper.ResolvePartUri(src, dest),
                "#1"
            );
            Assert.AreEqual(
                new Uri("/1/2/3/4", UriKind.Relative),
                PackUriHelper.ResolvePartUri(dest, src),
                "#1"
            );
        }
