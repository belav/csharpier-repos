...
namespace MonoTests.System.DirectoryServices
{
    ...
    public class DirectoryServicesSearchResultTest
    {
        ...
        public void SearchResult_Path()
        {
            ...
            Assert.AreEqual(
                results[0].Path.ToLower(),
                (configuration.ServerRoot + configuration.BaseDn).ToLower()
            );
            Assert.AreEqual(results[0].Path, results[0].GetDirectoryEntry().Path);

            // MS works only with "LDAP" while RFC2255 states "ldap"
            Assert.AreEqual(
                results[1].Path.ToLower(),
                (
        ...
        public void SearchResult_Properties()
        {
            ...
            Assert.AreEqual(
                ((ResultPropertyValueCollection)result.Properties["cn"])[0],
                "Barak Tsabari"
            );
            Assert.AreEqual(
                ((ResultPropertyValueCollection)result.Properties["objectClass"])[0],
                "person"
            );
            // MS works only with "LDAP" while RFC2255 states "ldap"
            Assert.AreEqual(
                (
                    (string)((ResultPropertyValueCollection)result.Properties["AdsPath"])[0]
                ).ToLower(),
                (
                    configuration.ServerRoot
