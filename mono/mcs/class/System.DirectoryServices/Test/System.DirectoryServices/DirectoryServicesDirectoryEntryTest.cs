...
namespace MonoTests.System.DirectoryServices
{
    ...
    public class DirectoryServicesDirectoryEntryTest
    {
        ...
        {
            if (
                (configuration.AuthenticationType != AuthenticationTypes.ServerBind)
        ...
        {
            string barakTsabariDN =
                configuration.ServerRoot
                + "cn=Barak Tsabari,ou=Human Resources,ou=people"
                + (
                    (configuration.BaseDn.Length == 0) ? String.Empty : ("," + configuration.BaseDn)
                );
            de = new DirectoryEntry(
                barakTsabariDN,
                configuration.Username,
            ...
            using (
                DirectoryEntry barakTsabariDE = new DirectoryEntry(
                    barakTsabariDN,
                    configuration.Username,
                    configuration.Password,
                    configuration.AuthenticationType
            ...
            {
                Assert.AreEqual(
                    barakTsabariDE.Properties["telephoneNumber"].Value,
                    oldTelephoneNumber
                );
                de.CommitChanges();
            }
            ...
            // restore object state
            using (
                DirectoryEntry ouHumanResources = new DirectoryEntry(
                    configuration.ServerRoot
            ...
            )
            {
                ...
                )
                {
                    ...
                    cnBarakTsabari.Properties["cn"].Value = "Barak Tsabari";
                    cnBarakTsabari.Properties["facsimileTelephoneNumber"].Value = "+1 906 777 8853";
                    ((PropertyValueCollection)cnBarakTsabari.Properties["ou"]).Add(
                        "Human Resources"
                    );
                    ((PropertyValueCollection)cnBarakTsabari.Properties["ou"]).Add("People");
                    cnBarakTsabari.Properties["sn"].Value = "Tsabari";
            ...
            using (
                DirectoryEntry ouRnD = new DirectoryEntry(
                    configuration.ServerRoot
                        + "ou=R&D,ou=people"
                        + (
                            (configuration.BaseDn.Length == 0)
                ...
                try
                {
                    ...
                    )
                    {
                        ...
                        Assert.Fail(
                            "Object "
                                + barakTsabariDN
                                + " was not moved from old location on the server."
                        );
                    }
                }
                catch (AssertionException ae)
            ...
            using (
                DirectoryEntry barakTsabariDE = new DirectoryEntry(
                    configuration.ServerRoot
                        + "cn=Barak Tsabari,ou=R&D,ou=people"
                        + (
                            (configuration.BaseDn.Length == 0)
                                ? String.Empty
                                : ("," + configuration.BaseDn)
            ...
            using (
                DirectoryEntry barakTsabariDE = new DirectoryEntry(
                    configuration.ServerRoot
                        + "cn=MyUser,ou=Human Resources,ou=people"
                        + (
                            (configuration.BaseDn.Length == 0)
                                ? String.Empty
                                : ("," + configuration.BaseDn)
        ...
        public void DirectoryEntry_Parent()
        {
            ...
            Assert.AreEqual(
                de.Parent.Path.ToLower(),
                (configuration.ServerRoot + GetParentDN(configuration.BaseDn)).ToLower()
            );
        ...
        public void DirectoryEntry_Properties1()
        {
            ...
            try
            {
                ...
                Assert.Fail(
                    "Properties should not be accessible after deleting an entry from the server"
                );
            }
            catch (AssertionException ae)
            {
        ...
        public void DirectoryEntry_SchemaClassName()
        {
            ...
            Assert.AreEqual(
                ((PropertyValueCollection)de2.Properties["structuralObjectClass"]).Value,
                null
            );
        }
        ...
        public void DirectoryEntry_CommitChanges1()
        {
            ...
            {
                // new entry
                string newEmployeeDN =
                    configuration.ServerRoot
                    + "cn=New Employee,ou=Human Resources,ou=people"
                    + (
                        (configuration.BaseDn.Length == 0)
                            ? String.Empty
                            : ("," + configuration.BaseDn)
            ...
            string oldTelephone = (string)
                ((PropertyValueCollection)de.Properties["telephoneNumber"]).Value;
            ...
            using (
                DirectoryEntry cnBarakTsabari = new DirectoryEntry(
                    barakTsabariDN,
                    configuration.Username,
                    configuration.Password,
                    configuration.AuthenticationType
            ...
            )
            {
                ...
                Assert.AreEqual(
                    ((PropertyValueCollection)cnBarakTsabari.Properties["telephoneNumber"]).Value,
                    oldTelephone
                );
        ...
        public void DirectoryEntry_CommitChanges2()
        {
            ...
            using (
                DirectoryEntry barakTsabariDE1 = new DirectoryEntry(
                    barakTsabariDN,
                    configuration.Username,
                    configuration.Password,
                    configuration.AuthenticationType
            ...
            )
            {
                ...
                using (
                    DirectoryEntry barakTsabariDE2 = new DirectoryEntry(
                        barakTsabariDN,
                        configuration.Username,
                        configuration.Password,
                        configuration.AuthenticationType
                ...
                )
                {
                    ...
                    string oldTelephone = (string)
                        (
                            (PropertyValueCollection)barakTsabariDE1.Properties["telephoneNumber"]
                        ).Value;
                    string newTelephone = "+972 3 6078596";
                    string oldFacsimilieTelephoneNumber = (string)
                        (
                            (PropertyValueCollection)
                                barakTsabariDE1.Properties["facsimileTelephoneNumber"]
                        ).Value;
                    ...
                    barakTsabariDE1.Properties["facsimileTelephoneNumber"].Value =
                        newFacsimilieTelephoneNumber1;

                    barakTsabariDE2.Properties["facsimileTelephoneNumber"].Value =
                        newFacsimilieTelephoneNumber2;
                    ...
                    Assert.AreEqual(
                        de.Properties["facsimileTelephoneNumber"].Value,
                        newFacsimilieTelephoneNumber1
                    );
        ...
        public void DirectoryEntry_DeleteTree()
        {
            ...
            string johnSmithDN =
                configuration.ServerRoot
                + "cn=John Smith,ou=Human Resources,ou=people"
                + (
                    (configuration.BaseDn.Length == 0) ? String.Empty : ("," + configuration.BaseDn)
                );
        ...
        public void DirectoryEntry_DeleteTree2()
        {
            ...
            using (
                DirectoryEntry johnSmithDE = new DirectoryEntry(
                    johnSmithDN,
                    configuration.Username,
                    configuration.Password,
                    configuration.AuthenticationType
        ...
        {
            string barakTsabariHumanResourcesDN =
                configuration.ServerRoot
                + "cn=Barak Tsabari,ou=Human Resources,ou=people"
                + (
                    (configuration.BaseDn.Length == 0) ? String.Empty : ("," + configuration.BaseDn)
                );
            string barakTsabariDevQaDN =
                configuration.ServerRoot
                + "cn=Barak Tsabari,ou=DevQA,ou=people"
                + (
                    (configuration.BaseDn.Length == 0) ? String.Empty : ("," + configuration.BaseDn)
                );
            ...
            )
            {
                ...
                using (
                    DirectoryEntry devQaDE = new DirectoryEntry(
                        devQaOU,
                        configuration.Username,
                        configuration.Password,
                        configuration.AuthenticationType
                ...
                )
                {
                    ...
                    using (
                        DirectoryEntry humanResourcesDE = new DirectoryEntry(
                            humanRwsourcesOU,
                            configuration.Username,
                            configuration.Password,
                            configuration.AuthenticationType
        ...
        public void DirectoryEntry_RefreshCache()
        {
            ...
            Assert.AreEqual(
                ((PropertyValueCollection)de.Properties["description"]).Value,
                newValue
            );
            ...
            )
            {
                ...
                )
                {
                    ...
                    ((PropertyValueCollection)newEmployeeDE.Properties["objectClass"]).Add(
                        "person"
                    );
                    ((PropertyValueCollection)newEmployeeDE.Properties["objectClass"]).Add(
                        "organizationalPerson"
        ...
        {
            string barakTsabariOldDN =
                configuration.ServerRoot
                + "cn=Barak Tsabari,ou=Human Resources,ou=people"
                + (
                    (configuration.BaseDn.Length == 0) ? String.Empty : ("," + configuration.BaseDn)
                );
            string barakTsabariNewDN =
                configuration.ServerRoot
                + "cn=My Name,ou=Human Resources,ou=people"
                + (
                    (configuration.BaseDn.Length == 0) ? String.Empty : ("," + configuration.BaseDn)
                );
