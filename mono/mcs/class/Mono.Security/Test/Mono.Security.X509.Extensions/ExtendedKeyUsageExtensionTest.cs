    ...
    public class ExtendedKeyUsageExtensionTest
    {
        ...
        public void KeyPurpose_NotCritical()
        {
            ...
            foreach (string oid in CommonKeyPurposes)
            {
                ...
                Assert.AreEqual(
                    "30-13-06-03-55-1D-25-04-0C-30-0A-06-08-2B-06-01-05-05-07-03-0"
                        + oid[oid.Length - 1].ToString(),
                    BitConverter.ToString(eku.GetBytes()),
                    oid
                );
            }
        }
        ...
        public void KeyPurpose_Critical()
        {
            ...
            foreach (string oid in CommonKeyPurposes)
            {
                ...
                eku.KeyPurpose.Add(oid);
                Assert.AreEqual(
                    "30-16-06-03-55-1D-25-01-01-FF-04-0C-30-0A-06-08-2B-06-01-05-05-07-03-0"
                        + oid[oid.Length - 1].ToString(),
                    BitConverter.ToString(eku.GetBytes()),
                    oid
                );
