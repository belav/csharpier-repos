...
namespace MonoTests.System.IdentityModel.Tokens.net_4_5
{
    ...
    public class BootstrapContextTest
    {
        ...
        private static readonly byte[] SerializedBootstrapContextByteArray = new byte[]
        {
            0x00,
            0x01,
            0x00,
        ...
        private static readonly string SerializedBootstrapContextSecurityTokenString =
            "<UserNameSecurityToken Id=\"uuid-927c0b98-ba18-49d2-a653-306d60f85751-3\" Username=\""
            + user
            + "\" Password=\""
            + password
            + "\"/>";
        private static readonly byte[] SerializedBootstrapContextSecurityToken = new byte[]
        {
            0x00,
            0x01,
            0x00,
        ...
        public void Serialize_StringToken_Works()
        {
            ...
            using (var s = new MemoryStream())
            {
                ...
                s.Position = 0;
                BootstrapContext bootstrapContext2 =
                    binaryFormatter.Deserialize(s) as BootstrapContext;
                ...
                Assert.AreEqual(
                    ...
                    bootstrapContext2.SecurityToken,
                    "#4"
                );
                Assert.AreEqual(
                    bootstrapContext.SecurityTokenHandler,
                    bootstrapContext2.SecurityTokenHandler,
                    "#5"
                );
            }
        }
        ...
        public void Ctor_SecurityToken_NullToken_Throws()
        {
            BootstrapContext bootstrapContext = new BootstrapContext(
                null,
                new SimpleSecurityTokenHandler()
            );
            Assert.Fail("Should have thrown");
        }
        ...
        {
            BootstrapContext bootstrapContext = new BootstrapContext(
                new UserNameSecurityToken("user", "password"),
                null
            );
            Assert.Fail("Should have thrown");
        }
        ...
        {
            var securityToken = new UserNameSecurityToken(
                user,
                password,
                "uuid-927c0b98-ba18-49d2-a653-306d60f85751-3"
            );
            var securityTokenHandler = new SimpleSecurityTokenHandler();
            BootstrapContext bootstrapContext = new BootstrapContext(
                securityToken,
            ...
            using (var s = new MemoryStream())
            {
                ...
                Assert.AreEqual(
                    SerializedBootstrapContextSecurityTokenString.Replace(" /", "/"),
                    bootstrapContext2.Token.Replace(" /", "/"),
                    "#2"
                );
                Assert.AreEqual(bootstrapContext.TokenBytes, bootstrapContext2.TokenBytes, "#3");
        ...
        public void Deserialize_SecurityTokenAndHandler_Works()
        {
            ...
            using (var s = new MemoryStream(SerializedBootstrapContextSecurityToken))
            {
                ...
                Assert.IsNotNull(bootstrapContext, "#1");
                Assert.AreEqual(
                    SerializedBootstrapContextSecurityTokenString,
            ...
            public override void WriteToken(XmlWriter writer, SecurityToken token)
            {
                ...
                if (unst == null)
                    throw new ArgumentException(
                        "Token must be of type UserNameSecurityToken",
                        "token"
                    );
