...
public class Test
{
    ...
    static void generate_password(int length, int numNonAlphNum)
    {
        Console.WriteLine(
            ...
            Membership.GeneratePassword(length, numNonAlphNum)
        );
    }
    ...
    static void dump_list(MembershipUserCollection users)
    {
        ...
        foreach (MembershipUser u in users)
        {
            Console.WriteLine(
                ...
                u.UserName,
                u.Email,
                u.IsLockedOut ? "lockedout" : "notlockedout"
            );
        }
    }
    ...
    {
        Console.WriteLine(
            "password for user {0}: {1}",
            username,
            Membership.Provider.GetPassword(username, answer)
        );
    }
