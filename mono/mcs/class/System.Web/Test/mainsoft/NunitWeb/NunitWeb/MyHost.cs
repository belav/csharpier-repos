...
namespace MonoTests.SystemWeb.Framework
{
    internal class MyHost : MarshalByRefObject
    {
        ...
        private static void RethrowException(Exception inner)
        {
            ...
            {
                throw new Exception(
                    "Cannot serialize exception of type " + inner.GetType().Name,
                    serializableInner
                );
            }
            Exception outer;
            try
            { //Try create a similar exception and keep the inner intact
                outer = (Exception)
            ...
            { //Failed to create a similar, fallback to the inner, ruining the call stack
                throw inner;
            }
            throw outer;
        }
        ...
        private static MyData GetMyData(HttpWorkerRequest wr)
        {
            ...
            if (fd == null)
                throw new ArgumentException(
                    "Invalid worker request. Probable reason is using WebTest.Invoke from a real web application"
                );
