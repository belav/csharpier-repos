namespace System.Web.Mvc
{
    ...
    public class NameValueCollectionValueProvider : IValueProvider, IUnvalidatedValueProvider
    {
        ...
        private readonly Dictionary<string, ValueProviderResultPlaceholder> _values =
            new Dictionary<string, ValueProviderResultPlaceholder>(
                StringComparer.OrdinalIgnoreCase
            );
        ...
        )
        {
            ...
            foreach (string key in unvalidatedCollection)
            {
                if (key != null)
                {
                    ...
                    _values[key] = new ValueProviderResultPlaceholder(
                        key,
                        validatedCollection,
        ...
        private sealed class ValueProviderResultPlaceholder
        {
            ...
            {
                _validatedResultPlaceholder = new Lazy<ValueProviderResult>(
                    () => GetResultFromCollection(key, validatedCollection, culture),
                    LazyThreadSafetyMode.None
                );
                _unvalidatedResultPlaceholder = new Lazy<ValueProviderResult>(
                    () => GetResultFromCollection(key, unvalidatedCollection, culture),
                    LazyThreadSafetyMode.None
                );
            }
