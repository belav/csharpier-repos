    ...
    public class OutputCacheAttribute : ActionFilterAttribute, IExceptionFilter
    {
        ...
        internal string GetChildActionUniqueId(ActionExecutingContext filterContext)
        {
            ...
            uniqueIdBuilder.Append(
                GetUniqueIdFromActionParameters(filterContext, SplitVaryByParam(VaryByParam))
            );
        ...
        )
        {
            ...
            keys = (keys ?? keyValues.Keys)
                .Select(key => key.ToUpperInvariant())
                .OrderBy(key => key, StringComparer.Ordinal);

            return DescriptorUtil.CreateUniqueId(
                keys.Concat(keys.Select(key => keyValues.ContainsKey(key) ? keyValues[key] : null))
            );
        }
