namespace System.Web.Mvc.Async
{
    ...
    internal sealed class AsyncActionMethodSelector
    {
        ...
        {
            List<MethodInfo> methodsMatchingName = GetMatchingAliasedMethods(
                controllerContext,
                actionName
            );
            methodsMatchingName.AddRange(NonAliasedMethods[actionName]);
            List<MethodInfo> finalMethods = RunSelectionFilters(
                controllerContext,
                methodsMatchingName
            );
        ...
        private ActionDescriptorCreator GetActionDescriptorDelegate(MethodInfo entryMethod)
        {
            ...
            if (IsAsyncSuffixedMethod(entryMethod))
            {
                string completionMethodName =
                    entryMethod.Name.Substring(0, entryMethod.Name.Length - "Async".Length)
                    + "Completed";
                ...
                {
                    throw Error.AsyncActionMethodSelector_CouldNotFindMethod(
        ...
        )
        {
            ...
            var methods =
                from methodInfo in AliasedMethods
        ...
        private MethodInfo GetMethodByName(string methodName)
        {
            List<MethodInfo> methods = (
                from MethodInfo methodInfo in ControllerType.GetMember(
                    methodName,
                    MemberTypes.Method,
        ...
        {
            return IsValidActionMethod(
                methodInfo,
                true /* stripInfrastructureMethods */
            );
        }

        private static bool IsValidActionMethod(
            MethodInfo methodInfo,
            bool stripInfrastructureMethods
        )
        {
        ...
        private void PopulateLookupTables()
        {
            ...
            NonAliasedMethods = actionMethods
                .Except(AliasedMethods)
        ...
        )
        {
            ...
            {
                ICollection<ActionMethodSelectorAttribute> attrs =
