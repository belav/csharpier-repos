...
namespace RabbitMQ.Client.Apigen
{
    public class Apigen
    {
        ...
        public Apigen(ArrayList args)
        {
            ...
            if ((args.Count < 2) || (m_apiName == null))
            {
                Usage();
            }
        ...
        {
            get { return "RabbitMQ.Client.Framing." + FramingSubnamespace; }
        }

        public string ImplNamespaceBase
        {
            get { return "RabbitMQ.Client.Framing.Impl." + FramingSubnamespace; }
        }
        ...
        public void EmitPublic()
        {
            ...
            EmitLine(
                "    public override int MajorVersion { get { return " + m_majorVersion + "; } }"
            );
            EmitLine("    ///<summary>Protocol minor version (= " + m_minorVersion + ")</summary>");
            EmitLine(
                "    public override int MinorVersion { get { return " + m_minorVersion + "; } }"
            );
            EmitLine("    ///<summary>Protocol API name (= " + m_apiName + ")</summary>");
            EmitLine(
                "    public override string ApiName { get { return \"" + m_apiName + "\"; } }"
            );
        ...
        public void EmitClassMethods(AmqpClass c)
        {
            foreach (AmqpMethod m in c.m_Methods)
            {
                ...
                foreach (AmqpField f in m.m_Fields)
                {
                    ...
                    EmitLine(
                        "    " + MapDomain(f.Domain) + " " + MangleClass(f.Name) + " { get; }"
                    );
                }
                EmitLine("  }");
            }
        ...
        public bool HasFactoryMethod(AmqpClass c)
        {
            foreach (Type t in m_modelTypes)
            {
                ...
                {
                    AmqpContentHeaderFactoryAttribute f =
                        (AmqpContentHeaderFactoryAttribute)Attribute(
                            method,
                            typeof(AmqpContentHeaderFactoryAttribute)
                        );
        ...
        public void EmitClassProperties(AmqpClass c)
        {
            ...
            string propertiesBaseClass = hasCommonApi
            ...
            EmitLine(
                "    public override string ProtocolClassName { get { return \""
                    + c.Name
                    + "\"; } }"
            );
            EmitLine("");
            ...
            foreach (AmqpField f in c.m_Fields)
            {
                ...
                {
                    EmitLine(
                        "      m_" + MangleMethod(f.Name) + "_present = reader.ReadPresence();"
                    );
                }
            }
                ...
                if (!IsBoolean(f))
                {
                    EmitLine(
                        "      if (m_"
                            ...
                            + MangleMethod(f.Name)
                            + " = reader.Read"
                            + MangleClass(ResolveDomain(f.Domain))
                            + "(); }"
                    );
                }
            }
            ...
            EmitLine("      sb.Append(\"(\");");
            {
                ...
                foreach (AmqpField f in c.m_Fields)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Emit(
                                " sb.Append(m_" + x + "_present ? m_" + x + ".ToString() : \"_\");"
                            );
                        }
                    }
                    remaining--;
        ...
        public void EmitClassMethodImplementations(AmqpClass c)
        {
            foreach (AmqpMethod m in c.m_Methods)
            {
                ...
                EmitLine(
                    "  public class "
                        + MangleMethodClass(c, m)
                        + ": RabbitMQ.Client.Impl.MethodBase, I"
                        + MangleMethodClass(c, m)
                        + " {"
                );
                ...
                {
                    EmitLine(
                        "    public " + MapDomain(f.Domain) + " m_" + MangleMethod(f.Name) + ";"
                    );
                }
                EmitLine("");
                foreach (AmqpField f in m.m_Fields)
                ...
                {
                    EmitLine(
                        "      m_" + MangleMethod(f.Name) + " = init" + MangleClass(f.Name) + ";"
                    );
                }
                EmitLine("    }");
                EmitLine("");
                EmitLine(
                    "    public override int ProtocolClassId { get { return " + c.Index + "; } }"
                );
                EmitLine(
                    "    public override int ProtocolMethodId { get { return " + m.Index + "; } }"
                );
                EmitLine(
                    "    public override string ProtocolMethodName { get { return \""
                        + c.Name
                        + "."
                        + m.Name
                        + "\"; } }"
                ...
                EmitLine(
                    "    public override bool HasContent { get { return "
        ...
        {
            AmqpContentHeaderFactoryAttribute factoryAnnotation =
                (AmqpContentHeaderFactoryAttribute)Attribute(
                    method,
                    typeof(AmqpContentHeaderFactoryAttribute)
                );
        ...
        )
        {
...

            {
                AmqpMethodMappingAttribute methodMapping =
                    Attribute(method, typeof(AmqpMethodMappingAttribute))
            ...
            {
                throw new Exception(
                    "Could not find AMQP class or method for IModel method " + method.Name
                );
            }
        }
        ...
        public void EmitModelMethod(MethodInfo method)
        {
            ...
            AmqpMethodMappingAttribute replyMapping =
                ...
                as AmqpMethodMappingAttribute;
            if (
                Attribute(method, typeof(AmqpForceOneWayAttribute)) == null
            ...
            )
            {
                ...
                amqpReplyMethod = amqpClass.MethodNamed(
                    replyMapping == null
                ...
                {
                    throw new Exception(
                        "Could not find AMQP reply method for IModel method " + method.Name
                    );
                }
            }
            ...
            foreach (ParameterInfo pi in parameters)
            {
                AmqpNowaitArgumentAttribute nwAttr =
                    Attribute(pi, typeof(AmqpNowaitArgumentAttribute))
                    as AmqpNowaitArgumentAttribute;
                if (nwAttr != null)
                {
                    nowaitParameter = pi;
            ...
            string contentHeaderExpr =
                contentHeaderParameter == null
                    ? "null"
                    : " ("
                        + MangleClass(amqpClass.Name)
                        + "Properties) "
                        + contentHeaderParameter.Name;
            ...
            foreach (ParameterInfo pi in parameters)
            {
                if (pi != contentHeaderParameter && pi != contentBodyParameter)
                {
                    if (Attribute(pi, typeof(AmqpUnsupportedAttribute)) != null)
                    {
                        ...
                        EmitLine(
                            "        throw new UnsupportedMethodFieldException(\""
                                + method.Name
                                + "\",\""
                                + pi.Name
                                + "\");"
                    ...
                    else
                    {
                        AmqpFieldMappingAttribute fieldMapping =
                            Attribute(pi, typeof(AmqpFieldMappingAttribute))
                            as AmqpFieldMappingAttribute;
                        if (fieldMapping != null)
                        {
                            EmitLine(
                                "      __req.m_" + fieldMapping.m_fieldName + " = @" + pi.Name + ";"
                            );
                        }
                        else
                        {
            ...
            if (nowaitParameter != null)
            {
                ...
                EmitLine(
                    "        ModelSend(__req," + contentHeaderExpr + "," + contentBodyExpr + ");"
                );
            ...
            else
            {
                ...
                EmitLine(
                    "      RabbitMQ.Client.Impl.MethodBase __repBase = ModelRpc(__req,"
                        + contentHeaderExpr
                        + ","
                        + contentBodyExpr
                        + ");"
                ...
                EmitLine(
                    "      " + replyImplClass + " __rep = __repBase as " + replyImplClass + ";"
                );
                EmitLine(
                ...
                else
                {
                    ...
                    AmqpFieldMappingAttribute returnMapping =
                        Attribute(
                            ...
                            typeof(AmqpFieldMappingAttribute)
                        ) as AmqpFieldMappingAttribute;
                    if (returnMapping == null)
                    {
                        ...
                        foreach (FieldInfo fi in method.ReturnType.GetFields())
                        {
                            AmqpFieldMappingAttribute returnFieldMapping =
                                Attribute(fi, typeof(AmqpFieldMappingAttribute))
                                as AmqpFieldMappingAttribute;
                            if (returnFieldMapping != null)
                            {
                                EmitLine(
                                    "      __result."
                                        ...
                                        + " = __rep."
                                        + fieldPrefix
                                        + returnFieldMapping.m_fieldName
                                        + ";"
                                );
                            }
                            else
                ...
                if (parameters.Length > 0)
                {
                    ...
                    foreach (ParameterInfo pi in parameters)
                    {
                        ...
                        else
                        {
                            ...
                            Emit(
                                "            __impl.m_"
                                    + (fieldMapping == null ? pi.Name : fieldMapping.m_fieldName)
                            );
                        }
                        remaining--;
                        if (remaining > 0)
