...
namespace I18N.CJK
{
    ...
    public class ISO2022JPEncoding : MonoEncoding
    {
        ...
        readonly bool allow_1byte_kana,
            allow_shift_io;
...
#if !DISABLE_UNSAFE
        protected override unsafe int GetBytesInternal(
            char* chars,
            int charCount,
            byte* bytes,
            int byteCount,
        ...
        {
            if (state != null)
                return ((ISO2022JPEncoder)state).GetBytesImpl(
                    chars,
                    charCount,
                    bytes,
                    byteCount,
            ...
            return new ISO2022JPEncoder(this, allow_1byte_kana, allow_shift_io).GetBytesImpl(
                chars,
                charCount,
                bytes,
                byteCount,
...
#else
        protected override int GetBytesInternal(
            char[] chars,
            int charIndex,
            int charCount,
            byte[] bytes,
        ...
        {
            if (state != null)
                return ((ISO2022JPEncoder)state).GetBytesInternal(
                    chars,
                    charIndex,
                    charCount,
                    bytes,
            ...
            return new ISO2022JPEncoder(this, allow_1byte_kana, allow_shift_io).GetBytesInternal(
                chars,
                charIndex,
                charCount,
                bytes,
        ...
        {
            return new ISO2022JPEncoder(this, allow_1byte_kana, allow_shift_io).GetByteCount(
                chars,
                charIndex,
                charCount,
                true
        ...
        {
            return new ISO2022JPEncoder(this, allow_1byte_kana, allow_shift_io).GetBytes(
                chars,
                charIndex,
                charCount,
                bytes,
        ...
        {
            return new ISO2022JPDecoder(allow_1byte_kana, allow_shift_io).GetCharCount(
                bytes,
                index,
                count
            );
        ...
        {
            return new ISO2022JPDecoder(allow_1byte_kana, allow_shift_io).GetChars(
                bytes,
                byteIndex,
                byteCount,
                chars,
    ...
    internal class ISO2022JPEncoder : MonoEncoder
    {
        ...
        private unsafe void SwitchMode(
            byte* bytes,
            ref int byteIndex,
            ref int byteCount,
            ref ISO2022JPMode cur,
            ISO2022JPMode next
        )
        {
            if (cur == next)
        ...
        )
        {
            ...
            for (int i = charIndex; i < end; i++, charCount--)
            {
                ...
                // kana.
                if (!allow_1byte_kana && ch >= 0xFF60 && ch <= 0xFFA0)
