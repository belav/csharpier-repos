...
public class Test
{
    ...
    static void Run(string[] args)
    {
        string s;
        using (
            StreamReader sr = new StreamReader(args[0], Encoding.GetEncoding(int.Parse(args[1])))
        )
        {
            s = sr.ReadToEnd();
        }
        ...
        string s2;
        using (
            StreamReader sr = new StreamReader(args[3], Encoding.GetEncoding(int.Parse(args[2])))
        )
        {
            s2 = sr.ReadToEnd();
        }
