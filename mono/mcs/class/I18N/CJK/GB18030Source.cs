...
namespace I18N.CJK
{
    internal unsafe class GB18030Source
    {
        ...
        static readonly int gbx2uniSize,
            uni2gbxSize;
        ...
        static GB18030Source()
        {
            ...
            if (ret != IntPtr.Zero)
            {
                ...
                gbx2uniSize =
                    (gbx2uni[0] << 24) + (gbx2uni[1] << 16) + (gbx2uni[2] << 8) + (gbx2uni[3]);
                gbx2uni += 4;
                uni2gbx = gbx2uni + gbx2uniSize;
                uni2gbxSize =
                    (uni2gbx[0] << 24) + (uni2gbx[1] << 16) + (uni2gbx[2] << 8) + (uni2gbx[3]);
                uni2gbx += 4;
            }
        }
        ...
        static readonly GB18030Map[] ranges = new GB18030Map[]
        {
            ...
            new GB18030Map(
                0x0452,
                0x200F,
                FromGBXRaw(0x81, 0x30, 0xD3, 0x30, false),
                FromGBXRaw(0x81, 0x36, 0xA5, 0x31, false),
                false
            ),
            ...
            new GB18030Map(
                0x2643,
                0x2E80,
                FromGBXRaw(0x81, 0x37, 0xA8, 0x39, false),
                FromGBXRaw(0x81, 0x38, 0xFD, 0x38, false),
                false
            ),
            ...
            new GB18030Map(
                0x361B,
                0x3917,
                FromGBXRaw(0x82, 0x30, 0xA6, 0x33, false),
                FromGBXRaw(0x82, 0x30, 0xF2, 0x37, false),
                false
            ),
            ...
            new GB18030Map(
                0x3CE1,
                0x4055,
                FromGBXRaw(0x82, 0x31, 0xD4, 0x38, false),
                FromGBXRaw(0x82, 0x32, 0xAF, 0x32, false),
                false
            ),
            ...
            new GB18030Map(
                0x4160,
                0x4336,
                FromGBXRaw(0x82, 0x32, 0xC9, 0x37, false),
                FromGBXRaw(0x82, 0x32, 0xF8, 0x37, false),
                false
            ),
            // rawmap: 4337-0x44D6
            new GB18030Map(
                0x44D7,
                0x464B,
                FromGBXRaw(0x82, 0x33, 0xA3, 0x39, false),
                FromGBXRaw(0x82, 0x33, 0xC9, 0x31, false),
                false
            ),
            ...
            new GB18030Map(
                0x478E,
                0x4946,
                FromGBXRaw(0x82, 0x33, 0xE8, 0x38, false),
                FromGBXRaw(0x82, 0x34, 0x96, 0x38, false),
                false
            ),
            ...
            new GB18030Map(
                0x49B8,
                0x4C76,
                FromGBXRaw(0x82, 0x34, 0xA1, 0x31, false),
                FromGBXRaw(0x82, 0x34, 0xE7, 0x33, false),
                false
            ),
            ...
            new GB18030Map(
                0x9FA6,
                0xD7FF,
                FromGBXRaw(0x82, 0x35, 0x8F, 0x33, false),
                FromGBXRaw(0x83, 0x36, 0xC7, 0x38, false),
                false
            ),
            ...
            new GB18030Map(
                0xE865,
                0xF92B,
                FromGBXRaw(0x83, 0x36, 0xD0, 0x30, false),
                FromGBXRaw(0x84, 0x30, 0x85, 0x34, false),
                false
            ),
            // rawmap: 0xF92C-FA29
            new GB18030Map(
                0xFA2A,
                0xFE2F,
                FromGBXRaw(0x84, 0x30, 0x9C, 0x38, false),
                FromGBXRaw(0x84, 0x31, 0x85, 0x37, false),
                false
            ),
            // rawmap: 0xFE30-FFE5
            new GB18030Map(
                0xFFE6,
                0xFFFF,
                FromGBXRaw(0x84, 0x31, 0xA2, 0x34, false),
                FromGBXRaw(0x84, 0x31, 0xA4, 0x39, false),
                false
            ),
        };
        ...
        public static long FromGBX(byte[] bytes, int start)
        {
            ...
            for (int i = 0; i < ranges.Length; i++)
            {
                ...
                if (linear <= m.GEnd)
                    return linear - gbxBase - m.GStart + m.UStart;
                if (m.GStart != 0)
                {
        ...
        {
            // 126 = 0xFE - 0x80
            return (((b1 - (supp ? 0x90 : 0x81)) * 10 + (b2 - 0x30)) * 126 + (b3 - 0x81)) * 10
                + b4
                - 0x30
                + (supp ? 0x10000 : 0);
        }
        ...
        void Run()
        {
            ...
            foreach (XmlElement e in doc.SelectNodes("/characterMapping/assignments/a"))
                AddMap(e);

            using (FileStream fs = File.Create("gb18030.table"))
            {
                byte[] size = new byte[4];
                for (int i = 0, len = gbx2uniMap.Length; i < 4; i++, len >>= 8)
        ...
        {
            int u = int.Parse(e.GetAttribute("u"), NumberStyles.HexNumber);
            byte[] b = new byte[4];
            int idx = 0;
            foreach (string s in e.GetAttribute("b").Split(' '))
                b[idx++] = byte.Parse(s, NumberStyles.HexNumber);
            if (idx != 4)
                return;
        ...
        {
            int gbx = (int)(FromGBXRaw(b[0], b[1], b[2], b[3], false) - gbxBase);
