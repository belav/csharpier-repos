...
namespace System.ServiceModel.Activities
{
    ...
    static class MessagingActivityHelper
    {
        ...
        {
            Type messageType =
                (messageArgument == null) ? TypeHelper.ObjectType : messageArgument.ArgumentType;
        ...
        public static Message InitializeCorrelationHandles(
        ...
        )
        {
            ...
            if (
                CorrelationMessageProperty.TryGet(messageProperties, out correlationMessageProperty)
            )
            {
                InitializeCorrelationHandles(
                    context,
        ...
        )
        {
            ...
            if (additionalKeys != null && additionalCorrelations != null)
            {
                ...
                IEnumerator<CorrelationInitializer> enumerator =
                    additionalCorrelations.GetEnumerator();
                ...
                foreach (InstanceKey key in additionalKeys)
                {
                    ...
                    while (enumerator.MoveNext())
                    {
                        QueryCorrelationInitializer queryCorrelation =
                            enumerator.Current as QueryCorrelationInitializer;
                        if (queryCorrelation != null)
                        {
                            CorrelationHandle handle = (
                                queryCorrelation.CorrelationHandle != null
                                    ? queryCorrelation.CorrelationHandle.Get(context)
                                    : null
                            );
                            if (handle == null)
                            {
        ...
        public static CorrelationCallbackContext CreateCorrelationCallbackContext(
            MessageProperties messageProperties
        )
        {
            CallbackContextMessageProperty callbackMessageContextProperty;
            if (
                CallbackContextMessageProperty.TryGet(
                    messageProperties,
                    out callbackMessageContextProperty
                )
            )
            {
                EndpointAddress listenAddress;
                IDictionary<string, string> context;
                callbackMessageContextProperty.GetListenAddressAndContext(
                    out listenAddress,
                    out context
                );
        ...
        public static CorrelationContext CreateCorrelationContext(
            MessageProperties messageProperties
        )
        {
        ...
        public static bool CompareContextEquality(
            IDictionary<string, string> context1,
            IDictionary<string, string> context2
        )
        {
        ...
        public static InArgument<CorrelationHandle> CreateReplyCorrelatesWith(
            InArgument<CorrelationHandle> requestCorrelatesWith
        )
        {
            Fx.Assert(requestCorrelatesWith != null, "Argument cannot be null!");

            VariableValue<CorrelationHandle> variableValue =
                requestCorrelatesWith.Expression as VariableValue<CorrelationHandle>;
            if (variableValue != null)
            {
                return new InArgument<CorrelationHandle>(variableValue.Variable);
            }

            VisualBasicValue<CorrelationHandle> vbvalue =
                requestCorrelatesWith.Expression as VisualBasicValue<CorrelationHandle>;
            if (vbvalue != null)
            {
                return new InArgument<CorrelationHandle>(
                    new VisualBasicValue<CorrelationHandle>(vbvalue.ExpressionText)
                );
            }
            ...
            Fx.Assert(
                expression != null,
                "Failed to clone CorrelationHandle using XAML roundtrip!"
            );

            return new InArgument<CorrelationHandle>(expression);
        }

        public static void ValidateCorrelationInitializer(
            ActivityMetadata metadata,
                ...
                foreach (CorrelationInitializer correlation in correlationInitializers)
                {
                    if (correlation is RequestReplyCorrelationInitializer && isReply)
                    {
                        ...
                        metadata.AddValidationError(
                            SR.ReplyShouldNotIncludeRequestReplyHandle(displayName, operationName)
                        );
                    }
                    ...
                    if (queryCorrelation != null)
                    {
                        ...
                        {
                            metadata.AddValidationError(
                                SR.QueryCorrelationInitializerWithEmptyMessageQuerySet(
                                    displayName,
                                    operationName
                                )
                            );
                    ...
                    if (correlation.CorrelationHandle == null)
                    {
                        if (correlation is QueryCorrelationInitializer)
                        {
                            ...
                            else
                            {
                                ...
                                metadata.AddValidationError(
                                    SR.NullCorrelationHandleInMultipleQueryCorrelation
                                );
                            }
                        }
                        else
                        {
                            metadata.AddValidationError(
                                SR.NullCorrelationHandleInInitializeCorrelation(
