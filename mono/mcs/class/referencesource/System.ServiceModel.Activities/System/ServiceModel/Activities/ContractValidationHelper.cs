...
namespace System.ServiceModel.Activities
{
    ...
    static class ContractValidationHelper
    {
        public static void ValidateReceiveWithReceive(Receive receive1, Receive receive2)
        {
            ...
            if (
                receive1.InternalContent is ReceiveMessageContent
                && receive2.InternalContent is ReceiveMessageContent
            )
            {
                ReceiveMessageContent receiveMessage1 =
                    receive1.InternalContent as ReceiveMessageContent;
                ReceiveMessageContent receiveMessage2 =
                    receive2.InternalContent as ReceiveMessageContent;

                ValidateReceiveWithReceive(receiveMessage1, receiveMessage2, receiveOperationName);
            }
            else if (
                receive1.InternalContent is ReceiveParametersContent
                && receive2.InternalContent is ReceiveParametersContent
            )
            {
                ReceiveParametersContent receiveParameters1 =
                    receive1.InternalContent as ReceiveParametersContent;
                ReceiveParametersContent receiveParameters2 =
                    receive2.InternalContent as ReceiveParametersContent;

                ValidateReceiveParametersWithReceiveParameters(
                    receiveParameters1,
                    receiveParameters2,
            ...
            {
                ValidateSendReplyWithSendReply(
                    receive1.FollowingReplies[0],
                    receive2.FollowingReplies[0]
                );
            }
            else if (
            ...
            if (
                (
                    receive1.InternalReceive.AdditionalData.IsInsideTransactedReceiveScope
                    != receive2.InternalReceive.AdditionalData.IsInsideTransactedReceiveScope
                )
                || (
                    receive1.InternalReceive
                        .AdditionalData
                        .IsFirstReceiveOfTransactedReceiveScopeTree
                    != receive2.InternalReceive
                        .AdditionalData
                        .IsFirstReceiveOfTransactedReceiveScopeTree
                )
            )
        ...
        static void ValidateReceiveWithReceive(
            ReceiveMessageContent receive1,
            ReceiveMessageContent receive2,
            string receiveOperationName
        )
        {
            Fx.Assert(receive1 != null && receive2 != null, "Validation argument cannot be null!");
        ...
        static void ValidateReceiveParametersWithReceiveParameters(
            ReceiveParametersContent receiveParameters1,
            ReceiveParametersContent receiveParameters2,
            string receiveOperationName
        )
        {
            Fx.Assert(
                receiveParameters1 != null && receiveParameters2 != null,
                "Validation argument cannot be null!"
            );
        ...
        public static void ValidateSendReplyWithSendReply(
            SendReply sendReply1,
            SendReply sendReply2
        )
        {
            Fx.Assert(
                sendReply1 != null && sendReply2 != null,
                "Validation argument cannot be null!"
            );
            ...
            if (
                sendReply1.InternalContent is SendMessageContent
            ...
            )
            {
                ...
                if (
                    sendMessage1.InternalDeclaredMessageType
                ...
                sendReply1.InternalContent is SendParametersContent
                && sendReply2.InternalContent is SendParametersContent
            )
            {
                SendParametersContent sendReplyParameters1 =
                    sendReply1.InternalContent as SendParametersContent;
                SendParametersContent sendReplyParameters2 =
                    sendReply2.InternalContent as SendParametersContent;
                ...
                {
                    if (
                        sendReplyParameters1.ArgumentNames[i]
        ...
        public static void ValidateFault(
        ...
        )
        {
            ...
            for (int index = 0; index < targetOperation.Faults.Count; index++)
            {
                ...
                if (targetFault.DetailType == faultType)
                {
                    ...
                    string action =
                        overridingAction
                        ?? NamingHelper.GetMessageAction(targetOperation, false) + name;
                    ...
                    if (targetFault.Namespace != targetOperation.DeclaringContract.Namespace)
                    {
                        Constraint.AddValidationError(
                            ...
                            new ValidationError(
                                SR2.PropertyMismatch(
                                    targetOperation.DeclaringContract.Namespace,
                                    "Fault Namespace",
                                    targetFault.Namespace,
                                    targetOperation.Name,
                                    targetOperation.DeclaringContract.Name
                                )
                            )
                    ...
                    if (targetFault.HasProtectionLevel)
                    {
                        Constraint.AddValidationError(
                            context,
                            new ValidationError(
                                SR2.ProtectionLevelNotSupported(
                                    targetOperation.Name,
                                    targetOperation.DeclaringContract.Name
                                )
                            )
            ...
            if (
                overridingAction == null
                    && targetMessage.Action
                        != NamingHelper.GetMessageAction(targetOperation, isResponse)
        ...
        )
        {
            ...
            if (MessageBuilder.IsMessageContract(declaredMessageType))
            {
                ...
                if (targetMessage.MessageType != null)
                {
                    if (declaredMessageType != targetMessage.MessageType)
                    {
                        Constraint.AddValidationError(
                            ...
                            new ValidationError(
                                SR2.PropertyMismatch(
                                    declaredMessageType.ToString(),
                                    "type",
                                    targetMessage.MessageType.ToString(),
                                    operation.Name,
                ...
                declaredMessageType != null
                && declaredMessageType.IsAssignableFrom(
            ...
            )
            {
                ...
                if (targetMessage.Body == null)
                {
                    Constraint.AddValidationError(
                        context,
                        new ValidationError(SR2.BodyCannotBeNull)
                    );
                }
                else
                {
                    if (isResponse)
                    {
                        if (targetMessage.Body.ReturnValue == null)
                        {
                            Constraint.AddValidationError(
                                context,
                                new ValidationError(SR2.ExtraReturnValue)
                            );
                        }
                        else if (
                            !targetMessage.Body
                        ...
                        )
                        {
                            Constraint.AddValidationError(
                                context,
                                new ValidationError(
                                    SR2.FirstParameterDoesnotMatchTheReturnValue(
                                        declaredMessageType.FullName,
                                        targetMessage.Body.ReturnValue.Type.Name,
                                        operation.Name,
                                        operation.DeclaringContract.Name
                                    )
                                )
                    ...
                    else
                    {
                        ...
                        else if (targetMessage.Body.Parts.Count > 1)
                        {
                            Constraint.AddValidationError(
                                context,
                                new ValidationError(
                                    SR2.MessageContentCannotHaveMoreThanOneParameter(
                                        operation.Name,
                                        operation.DeclaringContract.Name
                                    )
                                )
                        ...
                        else
                        {
                            ...
                            )
                            {
                                Constraint.AddValidationError(
                                    context,
                                    new ValidationError(
                                        SR2.MessageTypeMismatch(
                                            targetMessage.Body.Parts[0].Type.FullName,
                                            operation.Name,
                                            operation.DeclaringContract.Name
                                        )
                                    )
            ...
            if (targetMessage.Properties.Count > 0)
            {
                Constraint.AddValidationError(
                    context,
                    new ValidationError(
                        SR2.MessagePropertyIsNotSupported(
                            operation.Name,
                            operation.DeclaringContract.Name
                        )
                    )
            ...
            if (targetMessage.HasProtectionLevel)
            {
                Constraint.AddValidationError(
                    context,
                    new ValidationError(
                        SR2.ProtectionLevelIsNotSupported(
                            operation.Name,
                            operation.DeclaringContract.Name
                        )
                    )
            ...
            else
            {
                ...
                {
                    XmlQualifiedName xmlQualifiedName = MessageBuilder.XsdDataContractExporter
                        .GetRootElementName(declaredMessageType);
                    if (xmlQualifiedName == null)
                    {
                        xmlQualifiedName = MessageBuilder.XsdDataContractExporter
                ...
                {
                    XmlTypeMapping xmlTypeMapping = MessageBuilder.XmlReflectionImporter
                ...
                if (
                    isResponse
                    && targetMessage.Body.ReturnValue != null
                    && targetMessage.Body.ReturnValue.Type != TypeHelper.VoidType
                )
                {
                    if (targetMessage.Body.Parts.Count > 0)
                    {
                        Constraint.AddValidationError(
                            context,
                            new ValidationError(
                                SR2.NotSupportMoreThanOneParametersInMessageContract(
                                    operation.Name,
                                    operation.DeclaringContract.Name
                                )
                            )
                ...
                {
                    if (
                        targetMessage.Body.WrapperName != null
                        && targetMessage.Body.WrapperName != String.Empty
                    )
                    {
                        Constraint.AddValidationError(
                            context,
                            new ValidationError(
                                SR2.WrapperNotSupportedInMessageContract(
                                    operation.Name,
                                    operation.DeclaringContract.Name
                                )
                            )
                    ...
                    if (
                        targetMessage.Body.WrapperNamespace != null
                        && targetMessage.Body.WrapperNamespace != String.Empty
                    )
                    {
                        Constraint.AddValidationError(
                            context,
                    ...
                    if (declaredMessageType != targetPart.Type)
                    {
                        if (declaredMessageType != null)
                        {
                            Constraint.AddValidationError(
                                context,
                                new ValidationError(
                                    SR2.ParameterTypeMismatch(
                                        declaredMessageType.FullName,
                                        targetPart.Type.FullName,
                                        operation.Name,
                                        operation.DeclaringContract.Name
                                    )
                                )
        ...
        public static void ValidateParametersContent(
            NativeActivityContext context,
            MessageDescription targetMessage,
        ...
        )
        {
            ...
            if (targetMessageBody.WrapperName == null)
            {
                Constraint.AddValidationError(
                    context,
                    new ValidationError(
                        SR2.UnwrappedMessageNotSupported(
                            targetOperation.Name,
                            targetOperation.DeclaringContract.Name
                        )
                    )
            ...
            bool targetHasReturnValue =
                ...
                && targetMessageBody.ReturnValue.Type != TypeHelper.VoidType;
            if (targetHasReturnValue)
            {
                if (
                    iterator.MoveNext()
                    && (string)iterator.Key == targetMessageBody.ReturnValue.Name
                )
                {
                    Argument argument = (Argument)iterator.Value;
                    if (
                        argument != null
                        && argument.ArgumentType != targetMessageBody.ReturnValue.Type
                    )
                    {
                        Constraint.AddValidationError(
                            context,
                            new ValidationError(
                                SR2.FirstParameterDoesnotMatchTheReturnValue(
                                    argument.ArgumentType.FullName,
                                    targetMessageBody.ReturnValue.Type.FullName,
                                    targetOperation.Name,
                                    targetOperation.DeclaringContract.Name
                                )
                            )
            ...
            if (hitCount != parameters.Count)
            {
                foreach (string name in parameters.Keys)
                {
                    ...
                    {
                        if (
                            !targetHasReturnValue
                            || targetHasReturnValue && name != targetMessageBody.ReturnValue.Name
                        )
                        {
                            Constraint.AddValidationError(
                                context,
