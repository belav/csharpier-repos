    ...
    public sealed class InitializeCorrelation : NativeActivity
    {
        ...
        protected override void Execute(NativeActivityContext context)
        {
            CorrelationHandle correlationHandle =
                (this.Correlation == null) ? null : this.Correlation.Get(context);
            ...
            if (extension != null)
            {
                ...
                foreach (KeyValuePair<string, InArgument<string>> pair in this.CorrelationData)
                {
                    Fx.Assert(
                        pair.Value != null,
                        "pair.Value should be validated during cache metadata"
                    );
                    dictionary.Add(pair.Key, pair.Value.Get(context));
                }

                correlationHandle.InitializeBookmarkScope(
                    context,
                    extension.GenerateKey(dictionary)
                );
            }
            else
            {
