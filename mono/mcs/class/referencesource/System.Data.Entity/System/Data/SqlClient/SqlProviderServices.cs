    ...
    public sealed class SqlProviderServices : DbProviderServices
    {
        ...
        )
        {
            Debug.Assert(
                ...
                "CreateCommandDefinition passed null provider manifest to CreateDbCommandDefinition?"
            );
            Debug.Assert(
                commandTree != null,
                "CreateCommandDefinition did not validate commandTree argument?"
            );
        ...
        )
        {
            ...
            {
                if (
                    commandTree.CommandTreeKind != DbCommandTreeKind.Delete
        ...
        )
        {
            ...
            if (
                TypeSemantics.IsPrimitiveType(parameterType, PrimitiveTypeKind.String)
                || TypeSemantics.IsPrimitiveType(parameterType, PrimitiveTypeKind.Binary)
            )
            {
                int? size = GetParameterSize(
                    parameterType,
                    ((parameter.Direction & ParameterDirection.Output) == ParameterDirection.Output)
                );
                if (!size.HasValue)
                {
        ...
        )
        {
            ...
            if (underlyingReader == null)
            {
                throw EntityUtil.ProviderIncompatible(
                    Strings.SqlProvider_NeedSqlDataReader(fromReader.GetType())
                );
            }
        ...
        )
        {
            ...
            SqlDbType sqlDbType = GetSqlDbType(
                type,
                isOutParam,
                version,
                out size,
        ...
        )
        {
            ...
            string databaseName,
                dataFileName,
                logFileName;
            GetOrGenerateDatabaseNameAndGetFileNames(
                sqlConnection,
                out databaseName,
                out dataFileName,
                out logFileName
            );
            string createDatabaseScript = SqlDdlBuilder.CreateDatabaseScript(
                databaseName,
                dataFileName,
                logFileName
            );
                        ...
                        if (EntityUtil.IsCatchableExceptionType(ie))
                        {
                            throw new InvalidOperationException(
                                ...
                                new AggregateException(
                                    Strings.SqlProvider_IncompleteCreateDatabaseAggregate,
                                    e,
                                    ie
                                )
                            );
        ...
        {
            SqlProviderManifest sqlManifest = (
                storeItemCollection.StoreProviderManifest as SqlProviderManifest
            );
            if (sqlManifest == null)
            {
                throw EntityUtil.Argument(
        ...
        private static string GenerateDatabaseName(string mdfFileName)
        {
            ...
            strippedFileName =
                strippedFileName.Length > 30 ? strippedFileName.Substring(0, 30) : strippedFileName;

            string databaseName = databaseName = String.Format(
        ...
        )
        {
            ...
            if (
                string.IsNullOrEmpty(connectionBuilder.InitialCatalog)
        ...
        )
        {
            ...
            else if (!string.IsNullOrEmpty(attachDBFile))
            {
                ...
                {
                    throw EntityUtil.InvalidOperation(
                        Strings.SqlProvider_DdlGeneration_CannotDeleteDatabaseNoInitialCatalog
                    );
                }
            }
        ...
        {
            return SqlDdlBuilder.CreateObjectsScript(
                storeItemCollection,
                createSchemas: version != SqlVersion.Sql8
            );
        }
        ...
        )
        {
            ...
            catch (SqlException e)
            {
                ...
                if (
                    ...
                    && (
                        string.IsNullOrEmpty(connectionBuilder.UserID)
                        || string.IsNullOrEmpty(connectionBuilder.Password)
                    )
                )
                {
                    throw new InvalidOperationException(
                        Strings.SqlProvider_CredentialsMissingForMasterConnection,
                        e
                    );
                }
                throw;
