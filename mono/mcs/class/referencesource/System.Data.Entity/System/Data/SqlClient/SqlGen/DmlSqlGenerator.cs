...
namespace System.Data.SqlClient.SqlGen
{
    ...
    internal static class DmlSqlGenerator
    {
        ...
        )
        {
            ...
            bool useGeneratedValuesVariable = UseGeneratedValuesVariable(
                tree,
                sqlVersion,
                translator
            );
            EntityType tableType = (EntityType)
                ((DbScanExpression)tree.Target.Expression).Target.ElementType;
            ...
            if (useGeneratedValuesVariable)
            {
                ...
                foreach (EdmMember column in tableType.KeyMembers)
                {
                    ...
                    string columnType = SqlGenerator.GenerateSqlForStoreType(
                        sqlVersion,
                        column.TypeUsage
                    );
                ...
                Debug.Assert(
                    !first,
                    "if useGeneratedValuesVariable is true, it implies some columns do not have values"
                );
                commandText.AppendLine(")");
            }
            ...
            if (useGeneratedValuesVariable)
            {
                ...
                commandText.Append(" into ").AppendLine(s_generatedValuesVariableName);
            }
        ...
        /// </summary>
        private static bool UseGeneratedValuesVariable(
            DbInsertCommandTree tree,
            SqlVersion sqlVersion,
        ...
        private class ExpressionTranslator : BasicExpressionVisitor
        {
            ...
            internal SqlParameter CreateParameter(object value, TypeUsage type)
            {
                ...
                SqlParameter parameter = SqlProviderServices.CreateSqlParameter(
                    s_parameterNames.GetName(_parameters.Count),
                    type,
                    ParameterMode.In,
                    value,
                    preventTruncation,
            ...
            public override void Visit(DbScanExpression expression)
            {
                ...
                if (expression.Target.DefiningQuery != null)
                {
                    ...
                    {
                        Debug.Assert(
                            _commandTree.CommandTreeKind == DbCommandTreeKind.Update,
