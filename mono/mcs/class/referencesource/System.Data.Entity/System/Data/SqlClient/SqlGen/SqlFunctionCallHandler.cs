...
namespace System.Data.SqlClient.SqlGen
{
    ...
    internal static class SqlFunctionCallHandler
    {
        ...
        {
            Dictionary<string, FunctionHandler> functionHandlers = new Dictionary<
                string,
                FunctionHandler
            >(15, StringComparer.Ordinal);
        ...
        private static Dictionary<string, FunctionHandler> InitializeCanonicalFunctionHandlers()
        {
            ...
            functionHandlers.Add(
                ...
                HandleCanonicalFunctionCurrentDateTimeOffset
            );
            functionHandlers.Add(
            ...
            functionHandlers.Add(
                ...
                HandleCanonicalFunctionCreateDateTimeOffset
            );
        ...
        {
            Dictionary<string, string> functionNameToOperatorDictionary = new Dictionary<
                string,
                string
        ...
        > InitializeDateAddFunctionNameToDatepartDictionary()
        {
            Dictionary<string, string> dateAddFunctionNameToDatepartDictionary = new Dictionary<
                string,
                string
            >(5, StringComparer.Ordinal);
        ...
        > InitializeDateDiffFunctionNameToDatepartDictionary()
        {
            Dictionary<string, string> dateDiffFunctionNameToDatepartDictionary = new Dictionary<
                string,
                string
            >(5, StringComparer.Ordinal);
        ...
        > InitializeGeographyStaticMethodFunctionsDictionary()
        {
            Dictionary<string, FunctionHandler> staticGeographyFunctions =
        ...
        > InitializeGeographyInstancePropertyFunctionsDictionary()
        {
            Dictionary<string, string> instancePropGeographyFunctions =
        ...
        > InitializeRenamedGeographyInstanceMethodFunctions()
        {
            Dictionary<string, string> renamedInstanceMethodFunctions =
                new Dictionary<string, string>();
        ...
        > InitializeGeometryStaticMethodFunctionsDictionary()
        {
            Dictionary<string, FunctionHandler> staticGeometryFunctions =
                new Dictionary<string, FunctionHandler>();
        ...
        > InitializeGeometryInstancePropertyFunctionsDictionary()
        {
            Dictionary<string, string> instancePropGeometryFunctions =
                new Dictionary<string, string>();
        ...
        )
        {
            ...
            string functionNameWithoutSrid = (
                TypeSemantics.IsPrimitiveType(
                    functionExpression.ResultType,
                    PrimitiveTypeKind.Geometry
                )
            ...
            {
                return HandleFunctionDefaultGivenName(
                    sqlgen,
                    functionExpression,
                    functionNameWithSrid
                );
            }
            else
            {
                Debug.Assert(
                    ...
                    "FromText function should have text or text + srid arguments only"
                );
                return HandleFunctionDefaultGivenName(
                    sqlgen,
                    functionExpression,
        ...
        )
        {
            ...
            else
            {
                ...
                WriteFunctionArguments(
                    sqlgen,
                    functionExpression.Arguments.Concat(new[] { sridExpression }),
                    result
                );
                return result;
            }
        ...
        )
        {
            ...
            {
                EntityUtil.Metadata(
                    System.Data.Entity.Strings.SqlGen_NiladicFunctionsCannotHaveParameters
                );
            }
        ...
        )
        {
            ...
            if (
                staticMethodsMap.TryGetValue(
                    functionExpression.Function.Name,
                    out staticFunctionHandler
                )
            )
            {
                ...
                instancePropertiesMap.TryGetValue(
                    functionExpression.Function.Name,
                    out instancePropertyName
                )
            )
            {
                Debug.Assert(
                    functionExpression.Function.Parameters.Count > 0
                        && Helper.IsSpatialType(
                            functionExpression.Function.Parameters[0].TypeUsage
                        ),
            ...
            else
            {
                ...
                if (
                    !renamedInstanceMethodsMap.TryGetValue(
                        functionExpression.Function.Name,
                        out effectiveFunctionName
                    )
                )
                {
                ...
                return WriteInstanceFunctionCall(
                    ...
                    functionExpression,
                    isPropertyAccess: false,
                    castReturnTypeTo: castResultType
                );
            }
        }
        ...
        )
        {
            SqlBuilder result = new SqlBuilder();
            Debug.Assert(
                e.Arguments.Count > 0 && e.Arguments.Count <= 2,
                "There should be 1 or 2 arguments for operator"
            );
        ...
        )
        {
            ...
            if (null == constExpr)
            {
                throw EntityUtil.InvalidOperation(
                    System.Data.Entity.Strings.SqlGen_InvalidDatePartArgumentExpression(
        ...
        )
        {
            ...
            Debug.Assert(
                e.Arguments.Count == 1,
                "Canonical datepart functions should have exactly one argument"
            );
            result.Append(e.Arguments[0].Accept(sqlgen));
        ...
        )
        {
            ...
            return HandleCanonicalFunctionDateTimeTypeCreation(
                sqlgen,
                typeName,
                e.Arguments,
                true,
        ...
        )
        {
            ...
            bool isPrimitiveType = TypeHelpers.TryGetPrimitiveTypeKind(
                e.Arguments[0].ResultType,
                out typeKind
            );
            Debug.Assert(
                isPrimitiveType,
                "Expecting primitive type as input parameter to TruncateTime"
            );
        ...
        )
        {
            ...
            Debug.Assert(
                e.Arguments.Count <= 2,
                "Round or truncate should have at most 2 arguments"
            );
            result.Append(e.Arguments[0].Accept(sqlgen));
            result.Append(", ");
        ...
        )
        {
            ...
            searchParamBuilder.Append(
                SqlProviderManifest.EscapeLikeText(
                    constSearchParamExpression.Value as string,
                    false,
                    out escapingOccurred
                )
            );
            if (insertPercentEnd == true)
                searchParamBuilder.Append("%");

            DbConstantExpression escapedSearchParamExpression = new DbConstantExpression(
                constSearchParamExpression.ResultType,
        ...
        )
        {
            ...
            if (
                (constSearchParamExpression != null)
                && (string.IsNullOrEmpty(constSearchParamExpression.Value as string) == false)
            )
            {
                TranslateConstantParameterForLike(
                    sqlgen,
                    args[0],
                    constSearchParamExpression,
                    result,
                    true,
                    true
                );
        ...
        {
            return CastReturnTypeToGivenType(
                e,
        ...
        private static bool CastReturnTypeToInt32(SqlGenerator sqlgen, DbFunctionExpression e)
        {
            ...
            {
                TypeUsage storeType = sqlgen.StoreItemCollection.StoreProviderManifest.GetStoreType(
                    e.Arguments[i].ResultType
                );
        ...
        private static bool CastReturnTypeToInt16(DbFunctionExpression e)
        {
            return CastReturnTypeToGivenType(
                e,
                _functionRequiresReturnTypeCastToInt16,
        ...
        private static bool CastReturnTypeToSingle(DbFunctionExpression e)
        {
            ...
            return CastReturnTypeToGivenType(
                e,
                _functionRequiresReturnTypeCastToSingle,
        ...
        private static bool CastReturnTypeToGivenType(
            DbFunctionExpression e,
            Set<string> functionsRequiringReturnTypeCast,
