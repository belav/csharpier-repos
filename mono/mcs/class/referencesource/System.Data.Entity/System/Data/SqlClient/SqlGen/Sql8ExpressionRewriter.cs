...
namespace System.Data.SqlClient.SqlGen
{
    ...
    internal class Sql8ExpressionRewriter : DbExpressionRebinder
    {
        ...
        internal static DbQueryCommandTree Rewrite(DbQueryCommandTree originalTree)
        {
            ...
            Sql8ExpressionRewriter rewriter = new Sql8ExpressionRewriter(
                originalTree.MetadataWorkspace
            );
            DbExpression newQuery = rewriter.VisitExpression(originalTree.Query);
            return DbQueryCommandTree.FromValidExpression(
                originalTree.MetadataWorkspace,
                originalTree.DataSpace,
                newQuery
            );
        }
        #endregion
        ...
        public override DbExpression Visit(DbSkipExpression e)
        {
            ...
            DbExpression rightInput = VisitExpressionBinding(e.Input)
                .Sort(VisitSortOrder(e.SortOrder))
                .Limit(VisitExpression(e.Count));
            ...
            IList<DbPropertyExpression> sortExpressions = new List<DbPropertyExpression>(
                e.SortOrder.Count
            );
            ...
            DbExpression exceptExpression = TransformIntersectOrExcept(
                leftInput,
                rightInput,
        ...
        {
            bool negate =
                (expressionKind == DbExpressionKind.Except)
                || (expressionKind == DbExpressionKind.Skip);
            bool distinct =
                (expressionKind == DbExpressionKind.Except)
                || (expressionKind == DbExpressionKind.Intersect);
            ...
            {
                if (
                    RemoveNonSortProperties(
                        leftFlattenedProperties,
                        rightFlattenedProperties,
                        sortExpressionsOverLeft,
                        leftInputBinding.VariableName,
            ...
            Debug.Assert(
                leftFlattenedProperties.Count != 0,
                "The inputs to INTERSECT or EXCEPT have no properties"
            );
        ...
        )
        {
            ...
            Debug.Assert(
                properties.Count != 0,
                "No nested properties when FlattenProperties called?"
            );
            ...
            for (int i = 0; i < properties.Count; i++)
            {
                ...
                {
                    Debug.Assert(
                        TypeSemantics.IsEntityType(properties[i].TypeUsage)
                            || TypeSemantics.IsRowType(properties[i].TypeUsage),
        ...
        )
        {
            ...
            {
                return AreMatching(
                    (DbPropertyExpression)expr1.Instance,
                    (DbPropertyExpression)expr2.Instance,
                    expr1BindingVariableName,
            ...
            return (
                String.Equals(
                    instance1.VariableName,
                    expr1BindingVariableName,
                    StringComparison.Ordinal
                )
                && String.Equals(
                    instance2.VariableName,
                    expr2BindingVariableName,
                    StringComparison.Ordinal
                )
            );
        ...
        {
            List<KeyValuePair<string, DbExpression>> projectColumns = new List<
