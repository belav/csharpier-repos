...
namespace System.Data.Metadata.Edm
{
    ...
    internal sealed class ObjectItemAttributeAssemblyLoader : ObjectItemAssemblyLoader
    {
        ...
        {
            Debug.Assert(
                Create == sessionData.ObjectItemAssemblyLoaderFactory,
                "Why is there a different factory creating this class"
            );
        }
        #endregion
        ...
        {
            Debug.Assert(
                IsSchemaAttributePresent(SourceAssembly),
        ...
        {
            if (
                SessionData.TypesInLoading.TryGetValue(clrType.FullName, out edmType)
            ...
            )
            {
                ...
                if (edmType.ClrType != clrType)
                {
                    SessionData.EdmItemErrors.Add(
                        new EdmItemError(
                            System.Data.Entity.Strings.NewTypeConflictsWithExistingType(
                                clrType.AssemblyQualifiedName,
                                edmType.ClrType.AssemblyQualifiedName
                            ),
                            edmType
                        )
                    );
        ...
        {
            Debug.Assert(
                !SessionData.TypesInLoading.ContainsKey(clrType.FullName),
                "This should be called only after looking in typesInLoading"
            );
            Debug.Assert(
                SessionData.EdmItemErrors.Count > 0
                    || // had an error during loading
                    clrType
                        .GetCustomAttributes(
                            typeof(EdmTypeAttribute),
                            false /*inherit*/
                        )
                        .Length == 0
                    || // not a type we track
            ...
            if (
                SessionData.LockedAssemblyCache.TryGetValue(
                    clrType.Assembly,
                    out immutableCacheEntry
                )
            )
            {
                Debug.Assert(
        ...
        {
            foreach (
                EdmRelationshipAttribute roleAttribute in SourceAssembly.GetCustomAttributes(
                    typeof(EdmRelationshipAttribute),
                    false /*inherit*/
                )
            )
            {
                ...
                if (roleAttribute.Role1Name == roleAttribute.Role2Name)
                {
                    SessionData.EdmItemErrors.Add(
                        new EdmItemError(
                            System.Data.Entity.Strings.SameRoleNameOnRelationshipAttribute(
                                roleAttribute.RelationshipName,
                ...
                if (!errorEncountered)
                {
                    AssociationType associationType = new AssociationType(
                        roleAttribute.RelationshipName,
                        roleAttribute.RelationshipNamespaceName,
                        roleAttribute.IsForeignKey,
                        DataSpace.OSpace
                    );
                    ...
                    Debug.Assert(
                        !CacheEntry.ContainsType(associationType.FullName),
                        "Relationship type must not be present in the list of types"
                    );
        ...
        )
        {
            ...
            if (!TryGetRelationshipEndEntityType(clrType, out entityType))
            {
                SessionData.EdmItemErrors.Add(
                    new EdmItemError(
                        System.Data.Entity.Strings.RoleTypeInEdmRelationshipAttributeIsInvalidType(
            ...
            associationType.AddKeyMember(
                new AssociationEndMember(roleName, entityType.GetReferenceType(), multiplicity)
            );
        }
        ...
        {
            Debug.Assert(
                clrType.Assembly == SourceAssembly,
                "Why are we loading a type that is not in our assembly?"
            );
            Debug.Assert(
                !SessionData.TypesInLoading.ContainsKey(clrType.FullName),
                "Trying to load a type that is already loaded???"
            );
            Debug.Assert(!clrType.IsGenericType, "Generic type is not supported");

            EdmType edmType = null;

            EdmTypeAttribute[] typeAttributes = (EdmTypeAttribute[])
                clrType.GetCustomAttributes(
                    typeof(EdmTypeAttribute),
                    false /*inherit*/
                );
            ...
            if (typeAttributes.Length != 0)
            {
                if (clrType.IsNested)
                {
                    SessionData.EdmItemErrors.Add(
                        new EdmItemError(
                            System.Data.Entity.Strings.NestedClassNotSupported(
                                clrType.FullName,
                ...
                string cspaceTypeName = String.IsNullOrEmpty(typeAttribute.Name)
                    ? clrType.Name
                    : typeAttribute.Name;
                if (String.IsNullOrEmpty(typeAttribute.NamespaceName) && clrType.Namespace == null)
                {
                    SessionData.EdmItemErrors.Add(
                        new EdmItemError(Strings.Validator_TypeHasNoNamespace, edmType)
                    );
                    return;
                }

                string cspaceNamespaceName = String.IsNullOrEmpty(typeAttribute.NamespaceName)
                    ? clrType.Namespace
                ...
                {
                    Debug.Assert(
                        typeAttribute is EdmEnumTypeAttribute,
                        "Invalid type attribute encountered"
                    );
                        ...
                        SessionData.EdmItemErrors.Add(
                            new EdmItemError(
                                Strings.Validator_UnsupportedEnumUnderlyingType(
                                    clrType.GetEnumUnderlyingType().FullName
                                ),
                                edmType
                            )
                        );
            ...
            Debug.Assert(
                !CacheEntry.ContainsType(edmType.Identity),
                "This type must not be already present in the list of types for this assembly"
            );
        ...
        private EdmType ResolveBaseType(Type type)
        {
            EdmType edmType;
            if (
                type.GetCustomAttributes(typeof(EdmEntityTypeAttribute), false).Length > 0
        ...
        private bool TryFindNullParametersInRelationshipAttribute(
            EdmRelationshipAttribute roleAttribute
        )
        {
            if (roleAttribute.RelationshipName == null)
            {
                SessionData.EdmItemErrors.Add(
                    new EdmItemError(
                        System.Data.Entity.Strings.NullRelationshipNameforEdmRelationshipAttribute(
            ...
            if (roleAttribute.RelationshipNamespaceName == null)
            {
                SessionData.EdmItemErrors.Add(
                    new EdmItemError(
                        System.Data.Entity.Strings.NullParameterForEdmRelationshipAttribute(
        ...
        private void LoadPropertiesFromType(StructuralType structuralType)
        {
            ...
            foreach (PropertyInfo property in properties)
            {
                ...
                else if (property.IsDefined(typeof(EdmScalarPropertyAttribute), false))
                {
                    ...
                    {
                        newMember = LoadScalarProperty(
                            structuralType.ClrType,
                            property,
                            out isEntityKeyProperty
                        );
                    }
                }
        ...
        internal void ResolveNavigationProperty(
        ...
        )
        {
            ...
            // are all EdmPropertyAttributes that we need to process. If the current property is not an EdmPropertyAttribute
            // we will just ignore it and skip to the next property.
            object[] relationshipPropertyAttributes = propertyInfo.GetCustomAttributes(
                typeof(EdmRelationshipNavigationPropertyAttribute),
                false
            );

            Debug.Assert(
                relationshipPropertyAttributes.Length == 1,
                "There should be exactly one property for every navigation property"
            );
            ...
            if (
                !TryGetLoadedType(propertyInfo.PropertyType, out propertyType)
                || !(
            ...
            )
            {
                ...
                SessionData.EdmItemErrors.Add(
                    new EdmItemError(
                        System.Data.Entity.Strings.Validator_OSpace_InvalidNavPropReturnType(
                            propertyInfo.Name,
            ...
            EdmRelationshipNavigationPropertyAttribute attribute =
                (EdmRelationshipNavigationPropertyAttribute)relationshipPropertyAttributes[0];

            EdmMember member = null;
            EdmType type;
            if (
                SessionData.TypesInLoading.TryGetValue(
            ...
            )
            {
                ...
                if (relationshipType != null)
                {
                    ...
                    else
                    {
                        SessionData.EdmItemErrors.Add(
                            new EdmItemError(
                                System.Data.Entity.Strings.TargetRoleNameInNavigationPropertyNotValid(
                                    propertyInfo.Name,
                                    propertyInfo.DeclaringType.FullName,
                    ...
                    )
                    {
                        SessionData.EdmItemErrors.Add(
                            new EdmItemError(
                                System.Data.Entity.Strings.NavigationPropertyRelationshipEndTypeMismatch(
            ...
            else
            {
                SessionData.EdmItemErrors.Add(
                    new EdmItemError(
                        System.Data.Entity.Strings.RelationshipNameInNavigationPropertyNotValid(
                            propertyInfo.Name,
                            propertyInfo.DeclaringType.FullName,
                            attribute.RelationshipName
                        ),
        ...
        {
            Debug.Assert(
                property.IsDefined(typeof(EdmScalarPropertyAttribute), false),
                "The property must have a scalar attribute"
            );
            EdmMember member = null;
            ...
            else
            {
                ...
                Debug.Assert(
                    attrs.Length == 1,
                    "Every property can exactly have one ScalarProperty Attribute"
                );
        ...
        private void ResolveEnumTypeProperty(StructuralType declaringType, PropertyInfo clrProperty)
        {
            ...
            Debug.Assert(
                (
                    Nullable.GetUnderlyingType(clrProperty.PropertyType) ?? clrProperty.PropertyType
                ).IsEnum,
            ...
            if (
                !TryGetLoadedType(clrProperty.PropertyType, out propertyType)
            ...
            {
                var edmScalarPropertyAttribute = (EdmScalarPropertyAttribute)
                    clrProperty
                ...
                EdmProperty enumProperty = new EdmProperty(
                    ...
                    TypeUsage.Create(
                        propertyType,
                        new FacetValues() { Nullable = edmScalarPropertyAttribute.IsNullable }
                    ),
                    clrProperty,
                ...
                if (
                    declaringType.BuiltInTypeKind == BuiltInTypeKind.EntityType
        ...
        private void ResolveComplexTypeProperty(StructuralType type, PropertyInfo clrProperty)
        {
            ...
            )
            {
                ...
                SessionData.EdmItemErrors.Add(
                    new EdmItemError(
                        System.Data.Entity.Strings.Validator_OSpace_ComplexPropertyNotComplex(
                            clrProperty.Name,
                            clrProperty.DeclaringType.FullName,
                            clrProperty.PropertyType.FullName
                        ),
            ...
            {
                EdmProperty newProperty = new EdmProperty(
                    clrProperty.Name,
                    TypeUsage.Create(propertyType, new FacetValues { Nullable = false }),
                    clrProperty,
                    type.ClrType.TypeHandle
                );
        ...
        private void TrackClosure(Type type)
        {
            if (
                ...
                && !(
                    type.IsGenericType
                    && (
                        EntityUtil.IsAnICollection(type)
                        || // EntityCollection<>, List<>, ICollection<>
