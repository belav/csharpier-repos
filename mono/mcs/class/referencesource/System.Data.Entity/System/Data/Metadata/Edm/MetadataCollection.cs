...
namespace System.Data.Metadata.Edm
{
    ...
    {
        // The way the collection supports both case sensitive and insensitive search is that it maintains
        // two lists: one list
        // for keep tracking of the order (the ordered list) and another list sorted case sensitively (the
        // sorted list) by the
        // identity of the item.  When a look up on ordinal is requested, the ordered list is used.  When a
        // look up on the name
        // is requested, the sorted list is used.  The two list must be kept in sync for all update
        // operations.  For case
        // sensitive name lookup, the sorted list is searched.  For case insensitive name lookup, a binary
        // search is used on the
        // sorted list to find the match.

        // Note: Care must be taken when modifying logic in this class to call virtual methods in this
        // class.  Since virtual methods
        // can be override by a derived class, the possible results must be thought through.  If needed, add
        ...
        /// <summary>structure to contain the indexes of items whose identity match by
        // OrdinalIgnoreCase</summary>
        private struct OrderedIndex
        {
            /// <summary>the index of the item whose identity was used to create the initial dictionary
            // entry</summary>
        ...
        /// <exception cref="System.ArgumentOutOfRangeException">Thrown if the index is out of the range for
        ...
        /// <exception cref="System.InvalidOperationException">Thrown if the item passed in or the
        // collection itself instance is in ReadOnly state</exception>
        /// <exception cref="System.ArgumentException">Thrown if the MetadataCollection already contains an
        // item with the same identity</exception>
        /// <exception cref="System.ArgumentException">Thrown if the item passed into Setter has null or
        // String.Empty identity</exception>
        ...
        /// <param name="updateIfFound">Whether the item should be updated if a matching item is
        // found.</param>
        /// <returns>
        ...
        )
        {
            ...
            if (collectionData.IdentityDictionary.TryGetValue(identity, out orderIndex))
            {
                ...
                {
                    // turn the previously unique identity by ignore case into a multiple item for identity by ignore
                    // case
        ...
        // Its the approximate (x86) point at which doing a OrdinalCaseInsenstive linear search on
        // _orderedItems.
        ...
        /// <exception cref="System.InvalidOperationException">Thrown if the item passed in or the
        // collection itself instance is in ReadOnly state</exception>
        /// <exception cref="System.ArgumentException">Thrown if the item that is being added already
        // belongs to another ItemCollection</exception>
        /// <exception cref="System.ArgumentException">Thrown if the ItemCollection already contains an item
        ...
        internal bool AtomicAddRange(List<T> items)
        {
            ...
            // Check if the exchange was done, if not, then someone must have changed the data in the meantime,
            // so
            // return false
        ...
        /// <exception cref="System.ArgumentNullException">Thrown if item argument passed in is
        // null</exception>
        /// <exception cref="System.ArgumentException">Thrown if the item passed in has null or String.Empty
        // identity</exception>
        ...
        /// <exception cref="System.ArgumentException">Thrown if ignoreCase and an exact match does not
        // exist, but has multiple inexact matches</exception>
        ...
        /// <exception cref="System.ArgumentOutOfRangeException">Thrown if the arrayIndex is less than
        // zero</exception>
        /// <exception cref="System.ArgumentException">Thrown if the array argument passed in with respect
        // to the arrayIndex passed in not big enough to hold the MetadataCollection being copied</exception>
        ...
        /// <exception cref="System.ArgumentNullException">Thrown if the identity argument is null
        // </exception>
