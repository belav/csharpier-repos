...
namespace System.Data.Metadata.Edm
{
    internal sealed class ObjectItemLoadingSessionData
    {
        ...
        // keep the list of new assemblies that got loaded in this load assembly call. The reason why we
        // need to keep a seperate
        // list of assemblies is that we keep track of errors, and if there are no errors, only then do we
        // add the list of assemblies
        ...
        // List of known assemblies - this list is initially passed by the caller and we keep adding to it,
        // as and when we load
        // an assembly
        ...
        {
            get { return this._loadMessageLogger; }
        }
                ...
                if (_edmItemCollection != null && _conventionCSpaceTypeNames == null)
                {
                    ...
                    foreach (var edmType in _edmItemCollection.GetItems<EdmType>())
                    {
                        ...
                        )
                        {
                            ...
                            {
                                _conventionCSpaceTypeNames[edmType.Name] = new KeyValuePair<
                                    EdmType,
                                    int
        ...
        internal object LoaderCookie
        {
            get
            {
                ...
                {
                    Debug.Assert(
                        _loaderFactory == null || (object)_loaderFactory == _originalLoaderCookie,
        ...
        )
        {
            ...
            _loaderFactory =
                (Func<Assembly, ObjectItemLoadingSessionData, ObjectItemAssemblyLoader>)
                    loaderCookie;
            _originalLoaderCookie = loaderCookie;
            if (
                _loaderFactory == ObjectItemConventionAssemblyLoader.Create
                && _edmItemCollection != null
            )
            {
                foreach (
                    KnownAssemblyEntry entry in _knownAssemblies.GetEntries(
                        _loaderFactory,
                        edmItemCollection
                    )
                )
                {
                    foreach (EdmType type in entry.CacheEntry.TypesInAssembly.OfType<EdmType>())
                    {
                        if (Helper.IsEntityType(type))
                        {
                            ...
                            _cspaceToOspace.Add(
                                _edmItemCollection.GetItem<StructuralType>(
                                    entityType.CSpaceTypeName
                                ),
                                entityType
                            );
                        }
                        ...
                        else if (Helper.IsEnumType(type))
                        {
                            ...
                            _cspaceToOspace.Add(
                                _edmItemCollection.GetItem<EnumType>(enumType.CSpaceTypeName),
                                enumType
                            );
                        }
                        else
                        {
                            ...
                            _cspaceToOspace.Add(
                                _edmItemCollection.GetItem<StructuralType>(type.FullName),
                                type
                            );
                        }
                    }
