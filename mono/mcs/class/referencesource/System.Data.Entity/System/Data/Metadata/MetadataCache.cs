...
namespace System.Data.Metadata.Edm
{
    ...
    internal static class MetadataCache
    {
        ...
        /// </summary>
        private static readonly List<StoreMetadataEntry> _metadataEntriesRemovedFromCache =
            new List<StoreMetadataEntry>();
        ...
        internal static List<MetadataArtifactLoader> SplitPaths(string paths)
        {
            ...
            while (indexStart != -1)
            {
                ...
                dataDirPaths.Add(
                    paths.Substring(macroPathBeginIndex, indexEnd - macroPathBeginIndex)
                );
            ...
            results = paths.Split(
                new string[] { MetadataCache.s_metadataPathSeparator },
        ...
        private static void DoCacheClean<T>(Dictionary<string, T> cache, object objectToLock)
        ...
        {
            EdmMetadataEntry entry = GetCacheEntry<EdmMetadataEntry>(
                _edmLevelCache,
                cacheKey,
                _edmLevelLock,
                new EdmMetadataEntryConstructor(),
                out entryToken
            );
        ...
        )
        {
            StoreMetadataEntry entry = GetCacheEntry<StoreMetadataEntry>(
                ...
                cacheKey,
                _storeLevelLock,
                new StoreMetadataEntryConstructor(),
                out entryToken
            );
        ...
        private static T GetCacheEntry<T>(
            ...
            string cacheKey,
            object objectToLock,
            IMetadataEntryConstructor<T> metadataEntry,
            out object entryToken
        )
            where T : MetadataEntry
        {
        ...
        private static void LoadItemCollection<T>(
            IItemCollectionLoader<T> itemCollectionLoader,
            T entry
        )
            where T : MetadataEntry
        {
        ...
        internal static void Clear()
        {
            ...
            Memoizer<string, List<MetadataArtifactLoader>> artifactLoaderCacheTemp = new Memoizer<
                string,
                List<MetadataArtifactLoader>
        ...
        private abstract class MetadataEntry
        {
            ...
            protected void UpdateMetadataEntry(
                ...
                FileIOPermission filePermissions
            )
            {
                Debug.Assert(
                    _entryTokenReference.IsAlive,
                    "You must call Ensure token before you call this method"
                );
                Debug.Assert(
                    _markEntryForCleanup == false,
                    "The entry must not be marked for cleanup"
                );
            ...
            internal bool PeriodicCleanUpThread()
            {
                ...
                {
                    Debug.Assert(
                        _entryTokenReference.IsAlive == false,
                        "Entry Token must never be alive if the entry is marked for cleanup"
                    );
            ...
            internal object EnsureToken()
            {
                ...
                if (_entryTokenReference.IsAlive)
                {
                    Debug.Assert(
                        _markEntryForCleanup == false,
                        "An entry with alive token cannot be marked for cleanup"
                    );
                ...
                {
                    Debug.Assert(
                        _weakReferenceItemCollection.IsAlive,
                ...
                else
                {
                    ...
                    {
                        Debug.Assert(
                            _markEntryForCleanup,
                            "Since the strong reference is null, this entry must be marked for cleanup"
                        );
            ...
            internal void CheckFilePermission()
            {
                Debug.Assert(
                    _itemCollection != null,
                    "Item collection must be present since we want to reuse the metadata"
                );
                Debug.Assert(_entryTokenReference.IsAlive, "This entry must be in use");
                Debug.Assert(
                    _markEntryForCleanup == false,
                    "The entry must not marked for cleanup"
                );
                Debug.Assert(
                    _weakReferenceItemCollection.IsAlive,
        ...
        private class EdmMetadataEntry : MetadataEntry
        {
            ...
            {
                get { return (EdmItemCollection)this.ItemCollection; }
            }
        ...
        private class StoreMetadataEntry : MetadataEntry
        {
            ...
            {
                get { return (StorageMappingItemCollection)this.ItemCollection; }
            }
            ...
            internal void LoadStoreCollection(
                EdmItemCollection edmItemCollection,
                MetadataArtifactLoader loader
            )
            {
        ...
        /// </summary>
        /// <typeparam name="T"></typeparam>
        interface IItemCollectionLoader<T>
            where T : MetadataEntry
        {
        ...
        private struct StoreItemCollectionLoader : IItemCollectionLoader<StoreMetadataEntry>
        {
            ...
            internal StoreItemCollectionLoader(
                EdmItemCollection edmItemCollection,
                MetadataArtifactLoader loader
            )
            {
                ...
                if (
                    (loader.GetPaths(DataSpace.SSpace) == null)
