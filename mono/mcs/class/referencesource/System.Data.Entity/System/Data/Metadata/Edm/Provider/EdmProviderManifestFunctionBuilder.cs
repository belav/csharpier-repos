...
namespace System.Data.Metadata.Edm
{
    internal sealed class EdmProviderManifestFunctionBuilder
    {
        ...
        internal EdmProviderManifestFunctionBuilder(
            System.Collections.ObjectModel.ReadOnlyCollection<PrimitiveType> edmPrimitiveTypes
        )
        {
            ...
            {
                Debug.Assert(
                    (int)edmType.PrimitiveTypeKind < primitiveTypeUsages.Length
                        && (int)edmType.PrimitiveTypeKind >= 0,
                    "Invalid PrimitiveTypeKind value?"
                );
                Debug.Assert(
                    primitiveTypeUsages[(int)edmType.PrimitiveTypeKind] == null,
                    "Duplicate PrimitiveTypeKind value in EDM primitive types?"
                );
        ...
        {
            Debug.Assert(
                !string.IsNullOrEmpty(aggregateFunctionName)
                    && !string.IsNullOrWhiteSpace(aggregateFunctionName),
                "Aggregate function name should be valid"
            );

            FunctionParameter returnParameter = CreateReturnParameter(returnTypeKind);
            FunctionParameter collectionParameter = CreateAggregateParameter(
                collectionArgumentElementTypeKind
            );
        ...
        )
        {
            ...
            FunctionParameter[] parameters = parameterDefinitions
                .Select(paramDef => CreateParameter(paramDef.Value, paramDef.Key))
                .ToArray();
        ...
        )
        {
            return new FunctionParameter(
                parameterName,
                this.primitiveTypes[(int)primitiveParameterType],
                ParameterMode.In
            );
        }
        ...
        private FunctionParameter CreateReturnParameter(PrimitiveTypeKind primitiveReturnType)
        {
            return new FunctionParameter(
                ...
                this.primitiveTypes[(int)primitiveReturnType],
