...
namespace System.Data.Metadata.Edm
{
    ...
    internal sealed class ObjectItemConventionAssemblyLoader : ObjectItemAssemblyLoader
    {
        ...
        private const BindingFlags RootEntityPropertyReflectionBindingFlags =
            PropertyReflectionBindingFlags & ~BindingFlags.DeclaredOnly
        ...
        protected override void LoadTypesFromAssembly()
        {
            foreach (Type type in EntityUtil.GetTypesSpecial(SourceAssembly))
            {
                ...
                if (TryGetCSpaceTypeMatch(type, out cspaceType))
                {
                    ...
                    {
                        Debug.Assert(
                            ospaceType is StructuralType || Helper.IsEnumType(ospaceType),
                            "Only StructuralType or EnumType expected."
                        );
                            ...
                            SessionData.EdmItemErrors.Add(
                                new EdmItemError(
                                    Strings.Validator_OSpace_Convention_AmbiguousClrType(
                                        cspaceType.Name,
                                        previousOSpaceType.ClrType.FullName,
                                        type.FullName
                                    ),
                                    previousOSpaceType
                                )
            ...
            {
                Debug.Assert(
                    CacheEntry.ClosureAssemblies.Count == 0,
                    "How did we get closure assemblies?"
                );
        ...
        private bool TryCreateType(Type type, EdmType cspaceType, out EdmType newOSpaceType)
        {
            ...
            Debug.Assert(
                cspaceType is StructuralType || Helper.IsEnumType(cspaceType),
                "Structural or enum type expected"
            );
        ...
        )
        {
            ...
            newOSpaceType = null;
            Debug.Assert(
                TypesMatchByConvention(type, cspaceType),
                "The types passed as parameters don't match by convention."
            );
            ...
            if (cspaceType.BaseType != null)
            {
                ...
                {
                    string message = Strings.Validator_OSpace_Convention_BaseTypeIncompatible(
                        type.BaseType.FullName,
                        type.FullName,
                        cspaceType.BaseType.FullName
                    );
        ...
        )
        {
            ...
            Debug.Assert(
                TypesMatchByConvention(enumType, cspaceEnumType),
                "The types passed as parameters don't match by convention."
            );
            ...
            if (
                !UnderlyingEnumTypesMatch(enumType, cspaceEnumType)
            ...
            newOSpaceType = new ClrEnumType(
                enumType,
                cspaceEnumType.NamespaceName,
                cspaceEnumType.Name
            );
        ...
        private bool UnderlyingEnumTypesMatch(Type enumType, EnumType cspaceEnumType)
        {
            ...
            )
            {
                ...
                underlyingEnumType.PrimitiveTypeKind
            ...
            while (ospaceSortedEnumMemberNamesEnumerator.MoveNext())
            {
                if (
                    ...
                    && cspaceSortedEnumMemberEnumerator.Current.Value.Equals(
                        Convert.ChangeType(
                            Enum.Parse(enumType, ospaceSortedEnumMemberNamesEnumerator.Current),
                            enumUnderlyingType,
        ...
        {
            EdmType ospaceType;
            bool foundValue = SessionData.CspaceToOspace.TryGetValue(
                baseCSpaceType,
                out ospaceType
            );
            if (!foundValue)
            ...
            Debug.Assert(
                !foundValue || ospaceType is StructuralType,
                "Structural type expected (if found)."
            );
        ...
        )
        {
            ...
            if (
                !TryFindAndCreateEnumProperties(
                    type,
                    cspaceType,
                    ospaceType,
                    clrProperties,
        ...
        )
        {
            ...
            foreach (
                EdmProperty cspaceProperty in cspaceType
                    .GetDeclaredOnlyMembers<EdmProperty>()
                    .Where(m => Helper.IsComplexType(m.TypeUsage.EdmType))
            )
            {
                PropertyInfo clrProperty = clrProperties.FirstOrDefault(p =>
                    MemberMatchesByConvention(p, cspaceProperty)
                );
                if (clrProperty != null)
                {
                    typeClosureToTrack.Add(
                        new KeyValuePair<EdmProperty, PropertyInfo>(cspaceProperty, clrProperty)
                    );
                }
                else
                {
        ...
        )
        {
            ...
            foreach (
                NavigationProperty cspaceProperty in cspaceType.GetDeclaredOnlyMembers<NavigationProperty>()
            )
            {
                PropertyInfo clrProperty = clrProperties.FirstOrDefault(p =>
                    NonPrimitiveMemberMatchesByConvention(p, cspaceProperty)
                );
                if (clrProperty != null)
                {
                    bool needsSetter =
                        cspaceProperty.ToEndMember.RelationshipMultiplicity
        ...
        )
        {
            ...
            if (
                SessionData.CspaceToOspace.TryGetValue(
                    cspaceProperty.RelationshipType,
                    out ospaceRelationship
                )
            )
            {
                ...
                {
                    EdmType findType;
                    foundTarget = SessionData.CspaceToOspace.TryGetValue(
                        (StructuralType)
                            ((CollectionType)cspaceProperty.TypeUsage.EdmType).TypeUsage.EdmType,
                        out findType
                    );
                    if (foundTarget)
                    {
                ...
                navigationProperty.ToEndMember = (RelationshipEndMember)
                    ((RelationshipType)ospaceRelationship).Members.First(e =>
                        e.Name == cspaceProperty.ToEndMember.Name
                    );
                navigationProperty.FromEndMember = (RelationshipEndMember)
                    ((RelationshipType)ospaceRelationship).Members.First(e =>
                        e.Name == cspaceProperty.FromEndMember.Name
                    );
            ...
            else
            {
                EntityTypeBase missingType = cspaceProperty
                    .RelationshipType.RelationshipEndMembers.Select(e =>
                        ((RefType)e.TypeUsage.EdmType).ElementType
                    )
        ...
        )
        {
            foreach (
                EdmProperty cspaceProperty in cspaceType
                    .GetDeclaredOnlyMembers<EdmProperty>()
                    .Where(p => Helper.IsPrimitiveType(p.TypeUsage.EdmType))
            )
            {
                PropertyInfo clrProperty = clrProperties.FirstOrDefault(p =>
                    MemberMatchesByConvention(p, cspaceProperty)
        ...
        )
        {
            ...
            foreach (
                EdmProperty cspaceProperty in cspaceType
                    .GetDeclaredOnlyMembers<EdmProperty>()
                    .Where(p => Helper.IsEnumType(p.TypeUsage.EdmType))
            )
            {
                PropertyInfo clrProperty = clrProperties.FirstOrDefault(p =>
                    MemberMatchesByConvention(p, cspaceProperty)
        ...
        private void CreateRelationships()
        {
            ...
            foreach (
                AssociationType cspaceAssociation in SessionData.EdmItemCollection.GetItems<AssociationType>()
            )
            {
                Debug.Assert(
                    cspaceAssociation.RelationshipEndMembers.Count == 2,
                    "Relationships are assumed to have exactly two ends"
                );
                ...
                )
                {
                    ...
                    AssociationType ospaceAssociation = new AssociationType(
                        cspaceAssociation.Name,
                        cspaceAssociation.NamespaceName,
                        cspaceAssociation.IsForeignKey,
                        DataSpace.OSpace
                    );
                    for (int i = 0; i < cspaceAssociation.RelationshipEndMembers.Count; i++)
                    {
                        ...
                        ospaceAssociation.AddKeyMember(
                            new AssociationEndMember(
                                cspaceEnd.Name,
                                ospaceEndType.GetReferenceType(),
                                cspaceEnd.RelationshipMultiplicity
                            )
                        );
                    }
        ...
        private static bool MemberMatchesByConvention(
            PropertyInfo clrProperty,
        ...
        {
            return !clrProperty.PropertyType.IsValueType
                && !clrProperty.PropertyType.IsAssignableFrom(typeof(string))
        ...
        )
        {
            ...
            Debug.Assert(
                clrProperty.CanRead && clrProperty.CanWrite,
                "The clr property has to have a setter and a getter."
            );
            ...
            Debug.Assert(
                Helper.IsScalarType(propertyType),
                "Property has to be primitive or enum."
            );
            ...
            bool isKeyMember =
                ...
                && ((EntityType)cspaceType).KeyMemberNames.Contains(clrProperty.Name);
            ...
            EdmProperty ospaceProperty = new EdmProperty(
                cspaceProperty.Name,
