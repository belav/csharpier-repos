...
namespace System.Data.Common.CommandTrees.Internal
{
    ...
    internal sealed class ExpressionKeyGen : DbExpressionVisitor
    {
        ...
        private void VisitVariableName(string varName)
        {
            ...
            var _notSupportedVarNames = new Regex(
                "^" + ExpressionBuilder.DbExpressionBuilder.AliasGenerator.Prefix + "[0-9]+"
            );
            Debug.Assert(
        ...
        {
            Debug.Assert(
                TypeSemantics.IsScalarType(e.ResultType),
            ...
            switch (((PrimitiveType)primitive.EdmType).PrimitiveTypeKind)
            {
                ...
                case PrimitiveTypeKind.DateTimeOffset:
                    _key.Append(
                        ((DateTimeOffset)e.Value).ToString("o", CultureInfo.InvariantCulture)
                    );
                    break;
        ...
        public override void Visit(DbGroupByExpression e)
        {
            ...
            foreach (var a in e.Aggregates)
            {
                ...
                if (ga != null)
                {
                    _key.Append("GA(");
                    Debug.Assert(
                        ga.Arguments.Count == 1,
                        "Group aggregate must have one argument."
                    );
