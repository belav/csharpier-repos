...
namespace System.Data.Common.EntitySql
{
    ...
    internal static class CqlErrorHelper
    {
        ...
        )
        {
            ...
            if (TypeSemantics.IsAggregateFunction(functionType))
            {
                formatString = TypeHelpers.IsCanonicalFunction(functionType)
                    ? (Func<object, object, object, string>)
                        System.Data.Entity.Strings.NoCanonicalAggrFunctionOverloadMatch
                    : (Func<object, object, object, string>)
                        System.Data.Entity.Strings.NoAggrFunctionOverloadMatch;
            }
            else
            {
                formatString = TypeHelpers.IsCanonicalFunction(functionType)
                    ? (Func<object, object, object, string>)
                        System.Data.Entity.Strings.NoCanonicalFunctionOverloadMatch
                    : (Func<object, object, object, string>)
                        System.Data.Entity.Strings.NoFunctionOverloadMatch;
            }
        ...
        )
        {
            throw EntityUtil.EntitySqlError(
                ...
                String.Format(
                    ...
                    "{0} {1}",
                    System.Data.Entity.Strings.AliasNameAlreadyUsed(aliasName),
                    contextMessage
                )
            );
        }
                ...
                case BuiltInTypeKind.RowType:
                    ...
                    for (int i = 0; i < leftRow.Members.Count; i++)
                    {
                        ReportIncompatibleCommonType(
                            ...
                            rootLeftType,
                            rootRightType,
                            leftRow.Members[i].TypeUsage,
                            rightRow.Members[i].TypeUsage
                        );
                    }
                    break;
        ...
        {
            if (
                type.BuiltInTypeKind == BuiltInTypeKind.RowType
