...
namespace System.Data.Common.Utils.Boolean
{
    ...
    internal sealed class Solver
    {
        ...
        readonly Dictionary<Vertex, Vertex> _knownVertices = new Dictionary<Vertex, Vertex>(
        ...
        private Vertex IfThenElse(Vertex condition, Vertex then, Vertex @else)
        {
            ...
            int topVariable = DetermineTopVariable(
                condition,
                then,
                @else,
                out topVariableDomainCount
            );
        ...
        private static Vertex EvaluateFor(Vertex vertex, int variable, int variableAssigment)
        {
            ...
            // to that variable.
            Debug.Assert(
                variableAssigment < vertex.Children.Length,
                "variable assignment out of range"
            );
            ...
            if (!vertex.IsSink())
            {
                ...
                Debug.Assert(
                    _knownVertices.TryGetValue(vertex, out comparisonVertex)
                        && comparisonVertex.Equals(vertex),
                    "vertex not created by this solver"
                );
            }
        }
        #endregion
        ...
        private class VertexValueComparer : IEqualityComparer<Vertex>
        {
            ...
            public int GetHashCode(Vertex vertex)
            {
                ...
                Debug.Assert(
                    ...
                    "internal vertices must have at least 2 children"
                );
                unchecked
                {
                    return ((vertex.Children[0].GetHashCode() << 5) + 1)
                        + vertex.Children[1].GetHashCode();
                }
            }
        }
    }
