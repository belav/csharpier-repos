...
namespace System.Data.Common.Utils
{
    ...
    internal static class MetadataHelper
    {
            ...
            where T : EdmType
        {
            T resultType;
            if (
                TryGetWrappedReturnEdmTypeFromFunctionImport<T>(
                    functionImport,
                    resultSetIndex,
                    out resultType
                )
            ...
            {
                if (
                    typeof(EntityType).Equals(typeof(T)) && resultType is EntityType
            ...
            where T : EdmType
        {
            ...
            if (
                TryGetFunctionImportReturnCollectionType(
                    functionImport,
                    resultSetIndex,
                    out collectionType
                )
        ...
        )
        {
            ...
            if (
                ...
                && returnParameter.TypeUsage.EdmType.BuiltInTypeKind
        ...
        )
        {
            ...
            {
                spatialNormalizedEdmType = PrimitiveType.GetEdmPrimitiveType(
                    isGeographic ? PrimitiveTypeKind.Geography : PrimitiveTypeKind.Geometry
                );
            }

            EdmType modelEdmType;
            if (
                !MetadataHelper.TryDetermineCSpaceModelType<TElement>(workspace, out modelEdmType)
        ...
        internal static bool TryDetermineCSpaceModelType<T>(
            MetadataWorkspace workspace,
            out EdmType modelEdmType
        )
        {
        ...
        )
        {
            ...
            workspace.ImplicitLoadAssemblyForType(
                nonNullabelType,
            ...
            if (
                objectItemCollection.TryGetItem<EdmType>(
                    nonNullabelType.FullName,
                    out objectEdmType
                )
            )
            {
        ...
        internal static bool HasDiscreteDomain(EdmType edmType)
        {
            ...
            return primitiveType != null
                && primitiveType.PrimitiveTypeKind == PrimitiveTypeKind.Boolean;
        }
        ...
        internal static AssociationEndMember GetOtherAssociationEnd(AssociationEndMember endMember)
        {
            ...
            {
                Debug.Assert(
                    Object.ReferenceEquals(endMember, members[1]),
                    "didn't match other member"
                );
        ...
        )
        {
            ...
            StorageEntityContainerMapping containerMapping =
                MappingMetadataHelper.GetEntityContainerMap(
                    (StorageMappingItemCollection)itemCollection,
                    table.EntityContainer
                );
        ...
        {
            return GetTypeAndSubtypesOf(
                type,
                workspace.GetItemCollection(DataSpace.CSpace),
        ...
        )
        {
            Dictionary<EntityType, Set<EntityType>> graph =
                new Dictionary<EntityType, Set<EntityType>>();
        ...
        )
        {
            ...
            EntityType thisEndsEntityType = (EntityType)
                ((RefType)thisEnd.TypeUsage.EdmType).ElementType;
            ...
            foreach (ReferentialConstraint constraint in assocType.ReferentialConstraints)
            {
                ...
                {
                    otherEndProperties = Helpers.AsSuperTypeList<EdmProperty, EdmMember>(
        ...
        )
        {
            ...
            Debug.Assert(
                entitySet1.EntityContainer == entitySet2.EntityContainer,
            ...
            foreach (EntitySetBase extent in entitySet1.EntityContainer.BaseEntitySets)
            {
                if (Helper.IsRelationshipSet(extent))
                {
                    ...
                    if (
                        IsExtentAtSomeRelationshipEnd(assocSet, entitySet1)
        ...
        {
            Debug.Assert(
                associationType.Members.Contains(endName),
                "EndName should be a valid name"
            );
            entitySet = null;
            AssociationSet retValue = null;
            ReadOnlyMetadataCollection<EntitySetBase> baseEntitySets =
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                if (Object.ReferenceEquals(extent.ElementType, associationType))
                {
                    ...
                    if (es.Name == entitySetName)
                    {
                        Debug.Assert(
                            retValue == null,
                            "There should be only one AssociationSet, given an assocationtype, end name and entity set"
                        );
                        retValue = assocSet;
                        entitySet = es;
#if !DEBUG
        ...
        internal static PrimitiveTypeKind GetPrimitiveTypeKind(TypeUsage typeUsage)
        {
            Debug.Assert(
                null != typeUsage
                    && null != typeUsage.EdmType
                    && typeUsage.EdmType.BuiltInTypeKind == BuiltInTypeKind.PrimitiveType
            );
        ...
        )
        {
            ...
            foreach (
                StructuralType type in GetTypeAndSubtypesOf(
                    superType,
                    edmItemCollection,
        ...
        internal static string GenerateHashForAllExtentViewsContent(
            ...
            IEnumerable<KeyValuePair<string, string>> extentViews
        )
        {
            CompressingHashBuilder builder = new CompressingHashBuilder(
                CreateMetadataHashAlgorithm(schemaVersion)
            );
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage(
            ...
            "CA5350:Microsoft.Cryptographic.Standard",
        ...
        {
            TypeUsage edmTypeUsage = storeTypeUsage
                .GetModelTypeUsage()
                .ShallowCopy(FacetValues.NullFacetValues);
        ...
        )
        {
            ...
            if (
                entityType.TryGetNavigationProperty(
                    relationshipType,
                    fromName,
                    toName,
