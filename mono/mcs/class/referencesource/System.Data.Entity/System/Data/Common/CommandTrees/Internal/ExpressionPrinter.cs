    ...
    internal class ExpressionPrinter : TreePrinter
    {
        ...
        private class PrinterVisitor : DbExpressionVisitor<TreeNode>
        {
            ...
            internal TreeNode VisitFunction(EdmFunction func, IList<DbExpression> args)
            {
                ...
                AppendParameters(
                    ...
                    func.Parameters.Select(
                        fp => new KeyValuePair<string, TypeUsage>(fp.Name, fp.TypeUsage)
                    )
                );
                if (args != null)
                {
            ...
            public override TreeNode Visit(DbLambdaExpression expression)
            {
                ...
                AppendParameters(
                    lambdaInfo,
                    expression.Lambda.Variables.Select(
                        v => new KeyValuePair<string, TypeUsage>(v.VariableName, v.ResultType)
                    )
                );
                AppendArguments(
                    lambdaInfo,
