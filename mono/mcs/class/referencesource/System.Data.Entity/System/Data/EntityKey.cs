...
namespace System.Data
{
    ...
    public sealed class EntityKey : IEquatable<EntityKey>
    {
        ...
        )] // Justification: these are internal so they cannot be modified publically
        ...
        )
        {
            ...
            CheckKeyValues(
                new KeyValueReader(entityKeyValues),
                out _keyNames,
                out _singletonKeyValue,
                out _compositeKeyValues
            );
        ...
        internal EntityKey(EntitySet entitySet, IExtendedDataRecord record)
        {
            ...
            Debug.Assert(
                entitySet.EntityContainer.Name != null,
                "entitySet.EntityContainer.Name is null"
            );
        ...
        internal EntityKey(EntitySetBase entitySet)
        {
            ...
            Debug.Assert(
                ...
                "EntitySet.EntityContainer cannot be null."
            );
            ...
            get
            {
                if (!IsTemporary)
                {
                    ...
                    else
                    {
                        ...
                        {
                            keyValues[i] = new EntityKeyMember(
                                _keyNames[i],
                                _compositeKeyValues[i]
                            );
                        }
                    }
                    return keyValues;
...
#if DEBUG
            if (
                ((object)NoEntitySetKey == (object)key1)
                || ((object)EntityNotValidKey == (object)key1)
                || ((object)NoEntitySetKey == (object)key2)
                || ((object)EntityNotValidKey == (object)key1)
            // || (null==(object)key1) || (null==(object)key2)) //To check for internal use of null==key
            )
            {
                Debug.Assert(
                    typeof(EntityKey).Assembly != System.Reflection.Assembly.GetCallingAssembly(),
        ...
        internal static bool CompositeValuesWithBinaryEqual(EntityKey key1, EntityKey key2)
        {
            for (int i = 0; i < key1._compositeKeyValues.Length; ++i)
            {
                ...
                // key names between the keys.
                    ...
                    !ValuesWithBinaryEqual(key1._keyNames[i], key1._compositeKeyValues[i], key2)
                )
                {
                    return false;
                }
        ...
        internal KeyValuePair<string, DbExpression>[] GetKeyValueExpressions(EntitySet entitySet)
        {
            ...
            if (_singletonKeyValue != null)
            {
                ...
                Debug.Assert(
                    singletonKeyMember != null,
                    "Metadata for singleton key member shouldn't be null."
                );
                keyColumns = new[]
                {
                    DbExpressionBuilder
            ...
            else
            {
                ...
                {
                    Debug.Assert(
                        _compositeKeyValues[i] != null,
                        "Values within key-value pairs cannot be null."
                    );
                    ...
                    keyColumns[i] = DbExpressionBuilder
                        .Constant(Helper.GetModelTypeUsage(keyMember), _compositeKeyValues[i])
                        .As(keyMember.Name);
                }
            }
        ...
        internal static void GetEntitySetName(
            string qualifiedEntitySetName,
            ...
            else
            {
                ...
                if (numExpectedKeyValues == 1)
                {
                    lock (_nameLookup)
                    {
                        ...
                        {
                            if (
                                EntityUtil.IsNull(keyValuePair.Value)
                            ...
                            keyNames[0] = tokenizeStrings
                                ? EntityKey.LookupSingletonName(keyValuePair.Key)
                                : keyValuePair.Key;
        ...
        )
        {
            ...
            Debug.Assert(
                entitySet != null && entitySet.ElementType.IsAssignableFrom(entityType),
                "Entity types do not match"
            );
            Debug.Assert(
                numExpectedKeyValues > 0,
                "Should be expecting a positive number of key-values."
            );
            ...
            if (entitySet != null)
            {
                ...
                if (_singletonKeyValue != null)
                {
                    ...
                    if (_keyNames[0] != keyMembers[0].Name)
                    {
                        if (isArgumentException)
                        {
                            throw EntityUtil.MissingKeyValue(
                                argumentName,
                                keyMembers[0].Name,
                                entitySet.ElementType.FullName
                            );
                        ...
                        {
                            throw EntityUtil.MissingKeyValueInvalidOperation(
                                keyMembers[0].Name,
                                entitySet.ElementType.FullName
                            );
                        }
        ...
        )
        {
            ...
            Debug.Assert(
                Helper.IsScalarType(keyMember.TypeUsage.EdmType),
                ...
                if (entitySetKeyType != keyValue.GetType())
                {
                    if (isArgumentException)
                    {
                        throw EntityUtil.IncorrectValueType(
                            argumentName,
                            keyMember.Name,
                            entitySetKeyType.FullName,
                            keyValue.GetType().FullName
                        );
                    }
                    else
                    {
            ...
            else
            {
                ...
                if (
                    workspace.TryGetObjectSpaceType(
                        (EnumType)keyMemberEdmType,
                ...
                else
                {
                    if (isArgumentException)
                    {
                        throw EntityUtil.NoCorrespondingOSpaceTypeForEnumKeyField(
                            argumentName,
                            keyMember.Name,
                            keyMemberEdmType.FullName
                        );
                    }
                    else
                    {
                        throw EntityUtil.NoCorrespondingOSpaceTypeForEnumKeyFieldInvalidOperation(
                            keyMember.Name,
                            keyMemberEdmType.FullName
                        );
                    }
        ...
        private void AssertCorrectState(EntitySetBase entitySet, bool isTemporary)
        {
            if (_singletonKeyValue != null)
            {
                ...
                Debug.Assert(
                    _compositeKeyValues == null,
                    "The EntityKey is marked as both a singleton key and a composite key - this is illegal."
                );
                if (entitySet != null)
                {
                    Debug.Assert(
                        entitySet.ElementType.KeyMembers.Count == 1,
                        "For a singleton key, the number of key fields must be exactly 1."
                    );
                }
            }
            else if (_compositeKeyValues != null)
            {
                ...
                for (int i = 0; i < _compositeKeyValues.Length; ++i)
                {
                    Debug.Assert(
                        _compositeKeyValues[i] != null,
                        "Values passed to a composite EntityKey cannot be null."
                    );
                }
            }
            else if (!IsTemporary)
            {
                ...
                Debug.Assert(
                    ...
                    "The EntityKeyValues property for Static EntityKeys must return null."
                );
                Debug.Assert(
                    this.EntityContainerName == null,
                    "The EntityContainerName property for Static EntityKeys must return null."
                );
                Debug.Assert(
                    this.EntitySetName != null,
                    "The EntitySetName property for Static EntityKeys must not return null."
                );
            }
            else
            {
                ...
                Debug.Assert(
                    this.EntityKeyValues == null,
                    "The EntityKeyValues property for temporary EntityKeys must return null."
                );
            }
        }
        ...
        private void DeserializeMembers()
        {
            if (
                CheckKeyValues(
                    new KeyValueReader(_deserializedMembers),
                    true,
                    true,
                    out _keyNames,
                    out _singletonKeyValue,
