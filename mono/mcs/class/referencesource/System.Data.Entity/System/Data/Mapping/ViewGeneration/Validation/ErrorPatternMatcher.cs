    ...
    internal class ErrorPatternMatcher
    {
        ...
        private void MatchMissingMappingErrors()
        {
            if (m_viewgenContext.ViewTarget == ViewTarget.QueryView)
            {
                ...
                Set<EdmType> unmapepdTypesInExtent = new Set<EdmType>(
                    MetadataHelper.GetTypeAndSubtypesOf(
                        m_viewgenContext.Extent.ElementType,
                        m_viewgenContext.EdmItemCollection,
                        false /*isAbstract*/
                    )
                );
                ...
                if (unmapepdTypesInExtent.Count > 0)
                {
                    ...
                    m_errorLog.AddEntry(
                        new ErrorLog.Record(
                            ...
                            Strings.ViewGen_Missing_Type_Mapping(
                ...
                if (condition.RestrictedMemberSlot.MemberPath.Equals(member))
                {
                    ...
                    foreach (
                        NegatedConstant negatedConst in condition
                            .Domain.Values.Select(cellConstant => cellConstant as NegatedConstant)
        ...
        )
        {
            foreach (var leftCellWrapper in wrappers)
            {
                ...
                int indexOfMemberInProjection = leftCellQuery
                    .GetProjectedMembers()
                    .TakeWhile(path => !path.Equals(leftMember))
                    .Count();
                ...
                if (indexOfMemberInProjection < leftCellQuery.GetProjectedMembers().Count())
                {
                    MemberPath rightmember = (
                        (MemberProjectedSlot)
                            rightCellQuery.ProjectedSlotAt(indexOfMemberInProjection)
                    ).MemberPath;
        ...
        private void MatchConditionErrors()
        {
            ...
            Set<CompositeCondition> setOfconditions = new Set<CompositeCondition>(
                new ConditionComparer()
            );
            Dictionary<CompositeCondition, LeftCellWrapper> firstLCWForCondition = new Dictionary<
                CompositeCondition,
                LeftCellWrapper
            >(new ConditionComparer());
            ...
            foreach (var leftCellWrapper in leftCellWrappers)
            {
                ...
                CellQuery cellQuery = leftCellWrapper.OnlyInputCell.GetLeftQuery(
                ...
                foreach (MemberRestriction condition in cellQuery.GetConjunctsFromWhereClause())
                {
                    ...
                    if (
                        ...
                        !leftCellWrapper.OnlyInputCell.CQuery.WhereClause.Equals(
                            leftCellWrapper.OnlyInputCell.SQuery.WhereClause
                        )
                    ...
                    )
                    {
                        ...
                        CheckThatConditionMemberIsNotMapped(
                            memberPath,
                            leftCellWrappers,
                    ...
                    if (m_viewgenContext.ViewTarget == ViewTarget.UpdateView)
                    {
                        ...
                        {
                            MemberPath rightMemberPath = GetRightMemberPath(
                                memberPath,
                                leftCellWrapper
                            );
                            if (
                                rightMemberPath != null
                ...
                if (condMembersValues.Count > 0) //it is possible that there are no condition members
                {
                    ...
                    if (setOfconditions.Contains(condMembersValues))
                    {
                        ...
                        if (
                            !RightSideEqual(
                                firstLCWForCondition[condMembersValues],
                                leftCellWrapper
                            )
                        )
                        {
                            ...
                            m_errorLog.AddEntry(
                                new ErrorLog.Record(
                                    ...
                                    Strings.Viewgen_ErrorPattern_DuplicateConditionValue(
                                        BuildCommaSeparatedErrorString<MemberPath>(
                                            condMembersValues.Keys
                                        )
                                    ),
                                    ToIEnum(
                                        firstLCWForCondition[condMembersValues].OnlyInputCell,
                                        leftCellWrapper.OnlyInputCell
                                    ),
                                    ""
                                )
        ...
        private MemberPath GetRightMemberPath(
        ...
        private void MatchSplitErrors()
        {
            ...
            var nonAssociationWrappers = leftCellWrappers.Where(r =>
                !(r.LeftExtent is AssociationSet) && !(r.RightCellQuery.Extent is AssociationSet)
            );
                            ...
                            m_errorLog.AddEntry(
                                new ErrorLog.Record(
                                    ...
                                    Strings.Viewgen_ErrorPattern_TableMappedToMultipleES(
                                        leftCellWrapper.LeftExtent.ToString(),
                                        leftCellWrapper.RightCellQuery.Extent.ToString(),
        ...
        private void MatchPartitionErrors()
        {
            ...
            foreach (var fragment1 in mappingFragments)
            {
                foreach (var fragment2 in mappingFragments.Skip(++i))
                {
                    ...
                    bool isSDisjoint = CompareS(
                        ComparisonOP.IsDisjointFrom,
                        m_viewgenContext,
                        fragment1,
                        fragment2,
                        rightFragmentQuery1,
                        ...
                        else
                        {
                            ...
                            else if (is1SubsetOf2_C || is2SubsetOf1_C)
                            {
                                ...
                                if (CSideHasDifferentEntitySets(fragment1, fragment2))
                                {
                                    ...
                                    errorString.Append(
                                        Strings.Viewgen_ErrorPattern_Partition_Disj_Subs_Ref
                                    );
                                }
                                else
                                {
                                    ...
                                    //TestCase (2)
                                    errorString.Append(
                                        Strings.Viewgen_ErrorPattern_Partition_Disj_Subs
                                    );
                                }
                            }
                            ...
                            m_errorLog.AddEntry(
                                new ErrorLog.Record(
                                    true,
                                    ViewGenErrorCode.ErrorPatternInvalidPartitionError,
                        ...
                        else
                        {
                            ...
                            else if (is1SubsetOf2_C || is2SubsetOf1_C)
                            {
                                if (CSideHasDifferentEntitySets(fragment1, fragment2))
                                {
                                    ...
                                    errorString.Append(
                                        Strings.Viewgen_ErrorPattern_Partition_Eq_Subs_Ref
                                    );
                                }
                                else
                                {
                                    ...
                                    if (fragment1.LeftExtent.Equals(fragment2.LeftExtent))
                                    {
                                        ...
                                        GetTypesAndConditionForWrapper(
                                            fragment1,
                                            out firstCellWrapperHasCondition,
                                            out edmTypesForFirstCellWrapper
                                        );
                                        GetTypesAndConditionForWrapper(
                                            fragment2,
                                            out secondCellWrapperHasCondition,
                                            out edmTypesForSecondCellWrapper
                                        );
                                        if (
                                            !firstCellWrapperHasCondition
                                        ...
                                        )
                                        {
                                            if (
                                                (
                                                    (
                                                        edmTypesForFirstCellWrapper.Except(
                                                            edmTypesForSecondCellWrapper
                                                        )
                                                    ).Count() != 0
                                                )
                                                || (
                                                    (
                                                        edmTypesForSecondCellWrapper.Except(
                                                            edmTypesForFirstCellWrapper
                                                        )
                                                    ).Count() != 0
                                                )
                                            )
                                            {
                                                if (
                                                    !CheckForStoreConditions(fragment1)
                                                    || !CheckForStoreConditions(fragment2)
                                                )
                                                {
                                                    IEnumerable<string> edmTypesForErrorString =
                                                        edmTypesForFirstCellWrapper
                                                            .Select(it => it.FullName)
                                                            .Union(
                                                                edmTypesForSecondCellWrapper.Select(
                                                                    it => it.FullName
                                                                )
                                                            );
                                                    m_errorLog.AddEntry(
                                    ...
                                    errorString.Append(
                                        Strings.Viewgen_ErrorPattern_Partition_Eq_Subs
                                    );
                                }
                            }
                            else //unknown
                            {
                                ...
                                )
                                {
                                    ...
                                    errorString.Append(
                                        Strings.Viewgen_ErrorPattern_Partition_Eq_Unk_Association
                                    );
                                }
                                else
                                {
                                    ...
                                    errorString.Append(
                                        Strings.Viewgen_ErrorPattern_Partition_Eq_Unk
                                    );
                                }
                            }
                    ...
                    else if (is1SubsetOf2_S || is2SubsetOf1_S) //proper subset - note: else if ensures inverse need not be checked
                    {
                        ...
                        if (
                            (is1SubsetOf2_S && is1SubsetOf2_C == true && !(is2SubsetOf1_C == true))
                            || (
                                is2SubsetOf1_S
                                && is2SubsetOf1_C == true
                                && !(is1SubsetOf2_C == true)
                        ...
                        { //error
                            ...
                            else if (isCEqual) //equal
                            {
                                ...
                                if (CSideHasDifferentEntitySets(fragment1, fragment2))
                                {
                                    ...
                                    errorString.Append(
                                        " " + Strings.Viewgen_ErrorPattern_Partition_Sub_Eq_Ref
                                    );
                                }
                                else
                                {
                                    ...
                                    //TestCase (10)
                                    errorString.Append(
                                        Strings.Viewgen_ErrorPattern_Partition_Sub_Eq
                                    );
                                }
                            }
                            else
        ...
        )
        {
            ...
            foreach (var anotherFragment in mappingFragments)
            {
                ...
                {
                    CellQuery anotherCellQuery = anotherCell.GetLeftQuery(
                        m_viewgenContext.ViewTarget
                    );
        ...
        )
        {
            ...
            if ((lookingForC && IsQueryView()) || (!lookingForC && !IsQueryView()))
            {
                ...
                {
                    Debug.Fail(
                        "Unexpected comparison operator, only IsDisjointFrom and IsContainedIn are expected"
                    );
                    return false;
                }
        ...
        private bool RightSideEqual(LeftCellWrapper wrapper1, LeftCellWrapper wrapper2)
        {
            ...
            return m_viewgenContext.RightFragmentQP.IsEquivalentTo(
                rightFragmentQuery1,
                rightFragmentQuery2
            );
        }
        ...
        private FragmentQuery CreateRightFragmentQuery(LeftCellWrapper wrapper)
        {
            return FragmentQuery.Create(
                wrapper.OnlyInputCell.CellLabel.ToString(),
