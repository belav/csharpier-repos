    ...
    internal class FunctionImportMappingComposable : FunctionImportMapping
    {
            ...
            : base(functionImport, targetFunction)
        {
            ...
            Debug.Assert(
                ...
                "Non-null structuralTypeMappings must not be empty."
            );
            EdmType resultType;
            Debug.Assert(
            ...
            Debug.Assert(
                targetFunctionKeys == null || targetFunctionKeys.Length > 0,
                "Keys must be null or non-empty."
            );
        ...
        internal Node GetInternalTree(Command targetIqtCommand, IList<Node> targetIqtArguments)
        {
            if (m_internalTreeNode == null)
            {
                ...
                )
                {
                    ...
                    CollectionType expectedCollectionType = (CollectionType)
                    ...
                    Node castNode = itree.CreateNode(
                        itree.CreateSoftCastOp(expectedElementType),
                        varRefNode
                    );
            ...
            Debug.Assert(
                m_commandParameters.Length == targetIqtArguments.Count,
                "m_commandParameters.Length == targetIqtArguments.Count"
            );
            ...
            for (int i = 0; i < m_commandParameters.Length; ++i)
            {
                ...
                if (TypeSemantics.IsEnumerationType(argumentNode.Op.Type))
                {
                    argumentNode = targetIqtCommand.CreateNode(
                        targetIqtCommand.CreateSoftCastOp(
                            TypeHelpers.CreateEnumUnderlyingTypeUsage(argumentNode.Op.Type)
                        ),
                        argumentNode
                    );
                }
        ...
        private sealed class FunctionViewOpCopier : OpCopier
        {
            ...
            internal static Node Copy(
                Command cmd,
                Node viewNode,
                Dictionary<string, Node> viewArguments
            )
            {
            ...
            public override Node Visit(VarRefOp op, Node n)
            {
                ...
                Node argNode;
                if (
                    op.Var.VarType == VarType.Parameter
                    && m_viewArguments.TryGetValue(
                        ((ParameterVar)op.Var).ParameterName,
                        out argNode
                    )
                )
                {
        ...
        )
        {
            ...
            {
                queryExpression = GenerateStructuralTypeResultMappingView(
                    storeFunctionInvoke,
                    errors,
        ...
        private IEnumerable<DbExpression> GetParametersForTargetFunctionCall()
        {
            ...
            foreach (var targetParameter in this.TargetFunction.Parameters)
            {
                ...
                var functionImportParameter = this.FunctionImport.Parameters.Single(p =>
                    p.Name == targetParameter.Name
                );
        ...
        )
        {
            ...
            if (m_structuralTypeMappings.Count == 1)
            {
                ...
                var entityTypeMappingView = GenerateStructuralTypeMappingView(
                    type,
                    propertyMappings,
                    binding.Variable,
                    errors
            ...
            else
            {
                ...
                Debug.Assert(
                    m_structuralTypeMappings.All(m => m.Item2.Count > 0),
                    "In multi-type mapping each type must have conditions."
                );
                List<DbExpression> structuralTypePredicates = m_structuralTypeMappings
                    .Select(m =>
                        GenerateStructuralTypeConditionsPredicate(m.Item2, binding.Variable)
                    )
                    .ToList();
                ...
                foreach (var mapping in m_structuralTypeMappings)
                {
                    ...
                    var structuralTypeMappingView = GenerateStructuralTypeMappingView(
                        type,
                        propertyMappings,
                        binding.Variable,
                        errors
        ...
        private DbExpression GenerateStructuralTypeMappingView(
        ...
        )
        {
            ...
            Debug.Assert(
                properties.Count == propertyMappings.Count,
                "properties.Count == propertyMappings.Count"
            );
            ...
            for (int i = 0; i < propertyMappings.Count; ++i)
            {
                ...
                var propertyMappingView = GeneratePropertyMappingView(
                    propertyMapping,
                    row,
                    new List<string>() { propertyMapping.EdmProperty.Name },
                    errors
                );
        ...
        private DbExpression GenerateStructuralTypeConditionsPredicate(
            List<StorageConditionPropertyMapping> conditions,
            DbExpression row
        )
        {
            Debug.Assert(conditions.Count > 0, "conditions.Count > 0");
            DbExpression predicate = Helpers.BuildBalancedTreeInPlace(
                conditions.Select(c => GeneratePredicate(c, row)).ToArray(),
                (prev, next) => prev.And(next)
            );
            return predicate;
        }

        private DbExpression GeneratePredicate(
            StorageConditionPropertyMapping condition,
            DbExpression row
        )
        {
            Debug.Assert(
                condition.EdmProperty == null,
                "C-side conditions are not supported in function mappings."
            );
            ...
            if (condition.IsNull.HasValue)
            {
                return condition.IsNull.Value
                    ? (DbExpression)columnRef.IsNull()
                    : (DbExpression)columnRef.IsNull().Not();
            }
            else
            {
        ...
        {
            Debug.Assert(
                mapping is StorageScalarPropertyMapping,
                "Complex property mapping is not supported in function imports."
            );
            var scalarPropertyMapping = (StorageScalarPropertyMapping)mapping;
            return GenerateScalarPropertyMappingView(
                scalarPropertyMapping.EdmProperty,
                scalarPropertyMapping.ColumnProperty,
                row
            );
        }

        private DbExpression GenerateScalarPropertyMappingView(
        ...
        {
            Debug.Assert(
                row.ResultType.EdmType.BuiltInTypeKind == BuiltInTypeKind.RowType,
                "Input type is expected to be a row type."
            );
            var rowType = (RowType)row.ResultType.EdmType;
            Debug.Assert(
                rowType.Properties.Contains(column.Name),
                "Column name must be resolvable in the TVF result type."
            );
        ...
        private DbExpression GenerateScalarResultMappingView(DbExpression storeFunctionInvoke)
        {
            ...
            if (
                !MetadataHelper.TryGetFunctionImportReturnCollectionType(
                    this.FunctionImport,
                    0,
                    out functionImportReturnType
                )
            )
            {
            ...
            Debug.Assert(
                TypeSemantics.IsCollectionType(queryExpression.ResultType),
