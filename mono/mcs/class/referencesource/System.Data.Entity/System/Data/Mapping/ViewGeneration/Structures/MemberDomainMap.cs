...
namespace System.Data.Mapping.ViewGeneration.Structures
{
    ...
    internal class MemberDomainMap : InternalBase
    {
        ...
        private MemberDomainMap(
            ...
            Dictionary<MemberPath, CellConstantSet> nonConditionDomainMap,
        ...
        {
            m_conditionDomainMap = new Dictionary<MemberPath, CellConstantSet>(
            ...
            {
                domainMap = Domain.ComputeConstantDomainSetsForSlotsInUpdateViews(
                    extentCells,
                    m_edmItemCollection
                );
            }
            ...
            {
                domainMap = Domain.ComputeConstantDomainSetsForSlotsInQueryViews(
                    extentCells,
                    m_edmItemCollection,
                    isValidationEnabled
                );
            ...
            foreach (Cell cell in extentCells)
            {
                ...
                foreach (MemberRestriction condition in cellQuery.GetConjunctsFromWhereClause())
                {
                    ...
                    {
                        domainValues = Domain.DeriveDomainFromMemberPath(
                            memberPath,
                            edmItemCollection,
                            isValidationEnabled
                        );
                    ...
                    if (!domainValues.Contains(Constant.Null))
                    {
                        ...
                        if (
                            condition.Domain.Values.All(conditionConstant =>
                    ...
                    if (
                        ...
                        || (
                            !domainValues.Contains(Constant.Null)
                            && condition.Domain.Values.Contains(Constant.Null)
                        )
                    )
                    {
                        string message = System.Data.Entity.Strings.ViewGen_InvalidCondition(
                            memberPath.PathToString(false)
                        );
                        ErrorLog.Record record = new ErrorLog.Record(
                            true,
                            ViewGenErrorCode.InvalidCondition,
                            message,
                            cell,
                            String.Empty
                        );
            ...
            m_nonConditionDomainMap = new Dictionary<MemberPath, CellConstantSet>(
                MemberPath.EqualityComparer
            );
            foreach (Cell cell in extentCells)
            {
                ...
                foreach (MemberProjectedSlot slot in cellQuery.GetAllQuerySlots())
                {
                    ...
                    if (
                        m_conditionDomainMap.ContainsKey(member) == false
                        && m_nonConditionDomainMap.ContainsKey(member) == false
                    )
                    {
                        CellConstantSet memberSet = Domain.DeriveDomainFromMemberPath(
                            member,
                            m_edmItemCollection,
                        ...
                        m_nonConditionDomainMap.Add(
                            member,
                            new CellConstantSetInfo(memberSet, slot)
                        );
                    }
                }
            }
        ...
        {
            var domainMap = m_conditionDomainMap.ToDictionary(
                p => p.Key,
                p => new Set<Constant>(p.Value, Constant.EqualityComparer)
            );
        ...
        private void ExpandDomainsIfNeeded(
            Dictionary<MemberPath, CellConstantSet> domainMapForMembers
        )
        {
            ...
            foreach (MemberPath path in domainMapForMembers.Keys)
            {
                ...
                if (path.IsScalarType() && possibleValues.Any(c => c is NegatedConstant) == false)
                {
                    if (MetadataHelper.HasDiscreteDomain(path.EdmType))
                    {
                        // for a discrete domain, add all values that are not currently represented
                        // in the domain
                        Set<Constant> completeDomain = Domain.DeriveDomainFromMemberPath(
                            path,
                            m_edmItemCollection,
        ...
        )
        {
            ...
            ReduceEnumerableDomainToEnumeratedValues(
                target,
        ...
        )
        {
            foreach (MemberPath member in domainMap.Keys)
            {
                ...
                CellConstantSet domain = Domain.DeriveDomainFromMemberPath(
                    member,
                    edmItemCollection,
                    true /* leaveDomainUnbounded */
                );
                ...
                { // domainMap has extra members -- we should get rid of them
                    if (config.IsNormalTracing)
                    {
                        Helpers.FormatTraceLine(
                            "Changed domain of {0} from {1} - subtract {2}",
                            member,
                            domainMap[member],
                            extra
                        );
        ...
        )
        {
            foreach (Cell cell in cells)
            {
                ...
                for (int i = 0; i < cQuery.NumProjectedSlots; i++)
                {
                    ...
                    cDomain.Unite(
                        sDomain.Where(constant =>
                            !constant.IsNull() && !(constant is NegatedConstant)
                        )
                    );

                    if (
                        updateDomainMap.IsConditionMember(sPath)
        ...
        private static void ExpandNegationsInDomainMap(
        ...
        internal void UpdateConditionMemberDomain(
            MemberPath path,
            IEnumerable<Constant> domainValues
        )
        {
            // update domainMap
        ...
        private void AddToDomainMap(MemberPath member, IEnumerable<Constant> domainValues)
        {
            ...
            m_conditionDomainMap[member] = Domain.ExpandNegationsInDomain(
                possibleValues,
                possibleValues
            );
        }
