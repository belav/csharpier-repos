...
namespace System.Data.Mapping
{
    ...
    internal class StorageComplexTypeMapping
    {
        ...
        private Dictionary<string, ComplexType> m_types = new Dictionary<string, ComplexType>(
            StringComparer.Ordinal
        ); //Types for which the mapping holds true for.
        private Dictionary<string, ComplexType> m_isOfTypes = new Dictionary<string, ComplexType>(
            StringComparer.Ordinal
        ); //Types for which the mapping holds true for
        ...
        internal ComplexType GetOwnerType(string memberName)
        {
            foreach (ComplexType type in m_types.Values)
            {
                ...
                if (
                    (type.Members.TryGetValue(memberName, false, out tempMember))
