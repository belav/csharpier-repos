...
namespace System.Data.Mapping.ViewGeneration.Validation
{
    ...
    internal class RewritingValidator
    {
        ...
        internal void Validate()
        {
            ...
            Dictionary<MemberValueBinding, CellTreeNode> plainMemberValueTrees =
                CreateMemberValueTrees(false);
            ...
            Dictionary<MemberValueBinding, CellTreeNode> complementMemberValueTrees =
                CreateMemberValueTrees(true);

            WhereClauseVisitor plainWhereClauseVisitor = new WhereClauseVisitor(
                _basicView,
                plainMemberValueTrees
            );
            WhereClauseVisitor complementWhereClauseVisitor = new WhereClauseVisitor(
                _basicView,
                complementMemberValueTrees
            );
            ...
            foreach (LeftCellWrapper wrapper in _viewgenContext.AllWrappersForExtent)
            {
                ...
                CellTreeNode complementSQueryTreeForCondition =
                    complementWhereClauseVisitor.GetCellTreeNode(cell.SQuery.WhereClause);
                Debug.Assert(
                    complementSQueryTreeForCondition != null,
                    "Rewriting for S-side query is unsatisfiable"
                );
                ...
                if (complementSQueryTreeForCondition != _basicView)
                {
                    ...
                    sQueryTree = new OpCellTreeNode(
                        _viewgenContext,
                        CellTreeOpType.IJ,
                        complementSQueryTreeForCondition,
                        _basicView
                    );
                }
                else
                ...
                )
                {
                    ...
                    ReportConstraintViolation(
                        ...
                        ViewGenErrorCode.PartitionConstraintViolation,
                        cQueryTree.GetLeaves().Concat(sQueryTree.GetLeaves())
                    );
                }

                CellTreeNode plainSQueryTreeForCondition = plainWhereClauseVisitor.GetCellTreeNode(
                    cell.SQuery.WhereClause
                );
                Debug.Assert(
                    plainSQueryTreeForCondition != null,
                    "Rewriting for S-side query is unsatisfiable"
                );
                if (plainSQueryTreeForCondition != null)
                {
                    ...
                    // (a) swapped members
                    DomainConstraintVisitor.CheckConstraints(
                        plainSQueryTreeForCondition,
                        wrapper,
                        _viewgenContext,
                        _errorLog
                    );
                    ...
                    // (b) projected members
                    CheckConstraintsOnProjectedConditionMembers(
                        plainMemberValueTrees,
                        wrapper,
                        sQueryTree,
                        inExtentCondition
        ...
        )
        {
            ...
            FragmentQuery cMinusS = FragmentQuery.Create(
                BoolExpression.CreateAnd(cMinusSx.Condition, inExtentCondition)
            );
            FragmentQuery sMinusC = FragmentQuery.Create(
                BoolExpression.CreateAnd(sMinusCx.Condition, inExtentCondition)
            );
        ...
        )
        {
            ...
            HashSet<LeftCellWrapper> relevantCellWrappers = new HashSet<LeftCellWrapper>(
                relevantWrappers
            );
            List<LeftCellWrapper> relevantWrapperList = new List<LeftCellWrapper>(
            ...
            _errorLog.AddEntry(
                new ErrorLog.Record(true, errorCode, builder.ToString(), relevantCellWrappers, "")
            );
        }

        // according to case statements, where WHEN ... THEN was replaced by ELSE
        private Dictionary<MemberValueBinding, CellTreeNode> CreateMemberValueTrees(
            bool complementElse
        )
        {
            Dictionary<MemberValueBinding, CellTreeNode> memberValueTrees =
                new Dictionary<MemberValueBinding, CellTreeNode>();
            ...
            foreach (MemberPath column in _domainMap.ConditionMembers(_viewgenContext.Extent))
            {
                ...
                for (int i = 0; i < domain.Count; i++)
                {
                    ...
                    FragmentQuery memberConditionQuery = QueryRewriter.CreateMemberConditionQuery(
                        column,
                        domainValue,
                        _keyAttributes,
                        _domainMap
                    ...
                    if (_viewgenContext.TryGetCachedRewriting(memberConditionQuery, out rewriting))
                    {
                        ...
                        CellTreeNode cellTreeNode = QueryRewriter.TileToCellTree(
                            rewriting,
                            _viewgenContext
                        );
                ...
                if (complementElse && domain.Count > 1)
                {
                    ...
                    MemberValueBinding lastMemberValue = new MemberValueBinding(
                        column,
                        lastDomainValue
                    );
        ...
        )
        {
            ...
            foreach (MemberPath column in _domainMap.ConditionMembers(_viewgenContext.Extent))
            {
                ...
                MemberProjectedSlot slot =
                    wrapper.RightCellQuery.ProjectedSlotAt(index) as MemberProjectedSlot;
                if (slot != null)
                {
                    foreach (Constant domainValue in _domainMap.GetDomain(column))
                    {
                        ...
                        if (
                            memberValueTrees.TryGetValue(
                                new MemberValueBinding(column, domainValue),
                                out sQueryTreeForDomainValue
                            )
                        )
                        {
                            BoolExpression cWhereClause = PropagateCellConstantsToWhereClause(
                                wrapper,
                                wrapper.RightCellQuery.WhereClause,
                                domainValue,
                                column,
                                _viewgenContext.MemberMaps
                            );
                            FragmentQuery cCombinedQuery = FragmentQuery.Create(cWhereClause);
                            CellTreeNode sCombinedTree =
                                (sQueryTree == _basicView)
                            ...
                            if (
                                !CheckEquivalence(
                                    cCombinedQuery,
                                    sCombinedTree.RightFragmentQuery,
                                    inExtentCondition,
                                    out unsatisfiedConstraint
                                )
                            )
                            {
                                string memberLossMessage = Strings.ViewGen_CQ_DomainConstraint(
                                    slot.ToUserString()
                                );
                                ReportConstraintViolation(
                                    memberLossMessage,
                                    unsatisfiedConstraint,
                                    ViewGenErrorCode.DomainConstraintViolation,
                                    sCombinedTree
                                        .GetLeaves()
        ...
        )
        {
            ...
            Debug.Assert(
                constant is ScalarConstant || constant.IsNull() || constant is NegatedConstant,
            ...
            MemberRestriction restriction = new ScalarRestriction(
                joinSlot.MemberPath,
                allowedValues,
                possibleValues
            );

            BoolExpression result = BoolExpression.CreateAnd(
                expression,
                BoolExpression.CreateLiteral(restriction, memberMaps.QueryDomainMap)
            );
            return result;
        }
        #endregion
        ...
        )
        {
            ...
            BoolExpression resultingExpr = BoolExpression.CreateAnd(
                expression,
                BoolExpression.CreateLiteral(restriction, memberMaps.QueryDomainMap)
            );
        ...
        /// </summary>
        private void CheckConstraintsOnNonNullableMembers(
            Dictionary<MemberValueBinding, CellTreeNode> memberValueTrees,
            LeftCellWrapper wrapper,
            CellTreeNode sQueryTree,
            BoolExpression inExtentCondition
        )
        {
            ...
            {
                bool isColumnSimpleType =
                    (column.EdmType as System.Data.Metadata.Edm.SimpleType) != null;

                if (!column.IsNullable && isColumnSimpleType)
                {
                    FragmentQuery cFragment = AddNullConditionOnCSideFragment(
                        wrapper,
                        column,
                        _viewgenContext.MemberMaps
                    );
                    ...
                    )
                    {
                        _errorLog.AddEntry(
                            new ErrorLog.Record(
                                ...
                                Strings.Viewgen_NullableMappingForNonNullableColumn(
                                    wrapper.LeftExtent.ToString(),
                                    column.ToFullString()
                                ),
                                wrapper.Cells,
                                ""
        ...
        private class WhereClauseVisitor
            : Visitor<DomainConstraint<BoolLiteral, Constant>, CellTreeNode>
        {
            ...
            {
                return (
                    _memberValueTrees.TryGetValue(
                        new MemberValueBinding(memberPath, value),
                        out singleNode
                    )
                );
            }
        ...
        internal class DomainConstraintVisitor : CellTreeNode.SimpleCellTreeVisitor<bool, bool>
        {
            ...
            {
                DomainConstraintVisitor visitor = new DomainConstraintVisitor(
                    wrapper,
                    context,
                    errorLog
                );
            ...
            internal override bool VisitLeaf(LeafCellTreeNode node, bool dummy)
            {
                ...
                if (thisQuery != thatQuery)
                {
                    ...
                    {
                        MemberProjectedSlot thisSlot =
                            thisQuery.ProjectedSlotAt(i) as MemberProjectedSlot;
                        if (thisSlot != null)
                        {
                            MemberProjectedSlot thatSlot =
                ...
                {
                    string columnsString = MemberPath.PropertiesToUserString(
                        collidingColumns,
                        false
                    );
                    string message = Strings.ViewGen_NonKeyProjectedWithOverlappingPartitions(
                        columnsString
                    );
                    ErrorLog.Record record = new ErrorLog.Record(
                        true,
                        ViewGenErrorCode.NonKeyProjectedWithOverlappingPartitions,
                        message,
                        new LeftCellWrapper[] { m_wrapper, node.LeftCellWrapper },
                        String.Empty
                    );
