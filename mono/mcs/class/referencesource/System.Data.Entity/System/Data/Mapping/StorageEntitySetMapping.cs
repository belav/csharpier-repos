...
namespace System.Data.Mapping
{
    ...
    internal class StorageEntitySetMapping : StorageSetMapping
    {
        ...
        internal StorageEntitySetMapping(
            EntitySet extent,
            StorageEntityContainerMapping entityContainerMapping
        )
            : base(extent, entityContainerMapping)
        {
            m_modificationFunctionMappings =
                new List<StorageEntityTypeModificationFunctionMapping>();
        ...
        internal override void Print(int index)
        {
            ...
            foreach (
                StorageEntityTypeModificationFunctionMapping m in m_modificationFunctionMappings
            )
            {
                m.Print(index + 10);
            }
        ...
        internal void AddModificationFunctionMapping(
        ...
        {
            Debug.Assert(
                null != modificationFunctionMapping,
                "modification function mapping must not be null"
            );
            Debug.Assert(
                modificationFunctionMapping.EntityType.Equals(this.Set.ElementType)
                    || Helper.IsSubtypeOf(
                        modificationFunctionMapping.EntityType,
                        this.Set.ElementType
                    ),
                "attempting to add a modification function mapping with the wrong entity type"
            );
            foreach (
                StorageEntityTypeModificationFunctionMapping existingMapping in m_modificationFunctionMappings
            )
            {
                Debug.Assert(
