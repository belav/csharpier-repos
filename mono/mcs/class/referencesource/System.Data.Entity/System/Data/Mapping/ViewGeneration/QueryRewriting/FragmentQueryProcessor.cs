    ...
    internal class FragmentQueryProcessor : TileQueryProcessor<FragmentQuery>
    {
        ...
        internal override FragmentQuery Difference(FragmentQuery qA, FragmentQuery qB)
        {
            return FragmentQuery.Create(
                qA.Attributes,
                BoolExpression.CreateAndNot(qA.Condition, qB.Condition)
            );
        }
        ...
        )
        {
            ...
            foreach (DomainVariable<BoolLiteral, Constant> var in variables)
            {
                ...
                if (variableCondition != null)
                {
                    ...
                    if (
                        (false == view.Attributes.Contains(conditionMember))
                    ...
                    ) //Don't add member to the projected list if the condition involves a
                    {
                        foreach (Constant value in conditionValues.Values)
                        {
                            ...
                            DomainConstraint<BoolLiteral, Constant> constraint =
                                new DomainConstraint<BoolLiteral, Constant>(
                                    var,
                                    new Set<Constant>(
                                        new Constant[] { value },
            ...
            if (newProjectedAttributes.Count > 0)
            {
                ...
                FragmentQuery derivedView = new FragmentQuery(
                    String.Format(CultureInfo.InvariantCulture, "project({0})", view.Description),
                    view.FromVariable,
        ...
        {
            internal static readonly AttributeSetComparator DefaultInstance =
                new AttributeSetComparator();
