...
namespace System.Data.Mapping.ViewGeneration.QueryRewriting
{
    ...
    {
        private BoolExpr<DomainConstraint<BoolLiteral, Constant>> _kbExpression = TrueExpr<
        ...
        {
            base.AddFact(fact);
            _kbExpression = new AndExpr<DomainConstraint<BoolLiteral, Constant>>(
                _kbExpression,
                fact
            );
        }
        ...
        {
            CreateVariableConstraintsRecursion(
                extent.ElementType,
        ...
        )
        {
            ...
            if (assocSet != null)
            {
                BoolExpression assocSetExpr = BoolExpression.CreateLiteral(
                    new RoleBoolean(assocSet),
                    domainMap
                );
                ...
                foreach (AssociationSetEnd end in assocSet.AssociationSetEnds)
                {
                    ...
                    derivedTypes.UnionWith(
                        MetadataHelper.GetTypeAndSubtypesOf(
                            end.CorrespondingAssociationEndMember.TypeUsage.EdmType,
                            edmItemCollection,
                            false
                        )
                    );

                    BoolExpression typeCondition = CreateIsOfTypeCondition(
                        new MemberPath(end.EntitySet),
                        derivedTypes,
                        domainMap
                    );

                    BoolExpression inRoleExpression = BoolExpression.CreateLiteral(
                        new RoleBoolean(end),
                        domainMap
                    );
                    ...
                    if (
                        MetadataHelper.IsEveryOtherEndAtLeastOne(
                            assocSet,
                    ...
                    if (
                        MetadataHelper.DoesEndKeySubsumeAssociationSetKey(
                            assocSet,
                            end.CorrespondingAssociationEndMember,
                            associationkeys
                        )
                ...
                foreach (ReferentialConstraint constraint in assocType.ReferentialConstraints)
                {
                    ...
                    IEnumerable<EdmMember> toProperties = Helpers.AsSuperTypeList<
                        EdmProperty,
                        EdmMember
                    ...
                    if (
                        Helpers.IsSetEqual(
                            toProperties,
                            toEntitySet.ElementType.KeyMembers,
                    ...
                    )
                    {
                        ...
                        )
                        {
                            ...
                            BoolExpression inRoleExpression1 = BoolExpression.CreateLiteral(
                                new RoleBoolean(assocSet.AssociationSetEnds[0]),
                                domainMap
                            );
                            BoolExpression inRoleExpression2 = BoolExpression.CreateLiteral(
                                new RoleBoolean(assocSet.AssociationSetEnds[1]),
                                domainMap
                            );
            ...
            foreach (ReferentialConstraint constraint in assocType.ReferentialConstraints)
            {
                ...
                )
                {
                    ...
                    BoolExpression fromSetExpression = BoolExpression.CreateLiteral(
                        new RoleBoolean(fromEntitySet),
                        domainMap
                    );
                    BoolExpression toSetExpression = BoolExpression.CreateLiteral(
        ...
        )
        {
            ...
            possibleTypes.UnionWith(
                MetadataHelper.GetTypeAndSubtypesOf(edmType, edmItemCollection, true)
            );
            ...
            foreach (EdmType possibleType in possibleTypes)
            {
                ...
                HashSet<EdmType> derivedTypes = new HashSet<EdmType>();
                derivedTypes.UnionWith(
                    MetadataHelper.GetTypeAndSubtypesOf(possibleType, edmItemCollection, false)
                );
                if (derivedTypes.Count != 0)
                {
                    ...
                    BoolExpression typeConditionComplement = BoolExpression.CreateNot(
                        typeCondition
                    );
                    ...
                    )
                    {
                        ...
                        if (
                            domainMap.IsConditionMember(childPath)
                            || domainMap.IsProjectedConditionMember(childPath)
                        )
                        {
                            BoolExpression nullCondition;
                            List<Constant> childDomain = new List<Constant>(
                            ...
                            if (isScalar)
                            {
                                nullCondition = BoolExpression.CreateLiteral(
                                    new ScalarRestriction(
                                        ...
                                        new Domain(ScalarConstant.Undefined, childDomain)
                                    ),
                                    domainMap
                                );
                            }
                            else
                            {
                                nullCondition = BoolExpression.CreateLiteral(
                                    new TypeRestriction(
                                        new MemberProjectedSlot(childPath),
                                        new Domain(TypeConstant.Undefined, childDomain)
                                    ),
                                    domainMap
                                );
                            }
        ...
        )
        {
            Domain typeDomain = new Domain(
                derivedTypes.Select(derivedType => (Constant)new TypeConstant(derivedType)),
                domainMap.GetDomain(currentPath)
            );
            BoolExpression typeCondition = BoolExpression.CreateLiteral(
                new TypeRestriction(new MemberProjectedSlot(currentPath), typeDomain),
                domainMap
            );
