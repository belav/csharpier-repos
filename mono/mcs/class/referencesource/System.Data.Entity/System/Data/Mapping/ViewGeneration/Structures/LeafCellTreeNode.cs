...
namespace System.Data.Mapping.ViewGeneration.Structures
{
    ...
    internal class LeafCellTreeNode : CellTreeNode
    {
        ...
        #region Fields
        internal static readonly IEqualityComparer<LeafCellTreeNode> EqualityComparer =
            new LeafCellTreeNodeComparer();
        ...
        )
        {
            ...
            for (int i = 0; i < cellQuery.NumProjectedSlots; i++)
            {
                ...
                if (requiredSlots[i] && slot == null)
                {
                    ...
                    ConstantProjectedSlot defaultValue = new ConstantProjectedSlot(
                        Domain.GetDefaultValueForMemberPath(
                            memberPath,
                            GetLeaves(),
                            ViewgenContext.Config
                        ),
                        memberPath
                    );
            ...
            if (
                (cellQuery.Extent.EntityContainer.DataSpace == DataSpace.SSpace)
            ...
            )
            {
                ...
                )
                {
                    ...
                    if (
                        TryGetWithRelationship(
                            colocatedAssociationSetMap,
                            this.m_cellWrapper.LeftExtent,
                            cellQuery.SourceExtentMemberPath,
                            ref foreignKeySlots,
            ...
            ExtentCqlBlock result = new ExtentCqlBlock(
                cellQuery.Extent,
                cellQuery.SelectDistinctFlag,
        ...
        private bool TryGetWithRelationship(
        ...
        )
        {
            ...
            StorageEndPropertyMapping foreignKeyEndMap = GetForeignKeyEndMapFromAssocitionMap(
                colocatedAssociationSetMap,
                thisExtent
            );
            if (
                foreignKeyEndMap == null
                || foreignKeyEndMap.EndMember.RelationshipMultiplicity
            ...
            EntityType toEndEntityType = (EntityType)
                ((RefType)(toEnd.TypeUsage.EdmType)).ElementType;
            EntityType fromEndEntityType = (EntityType)(
            ...
            {
                IEnumerable<StorageScalarPropertyMapping> scalarPropertyMaps = propertyMaps.Where(
                    propMap => (propMap.EdmProperty.Equals(edmProperty))
                );
                Debug.Assert(
                    scalarPropertyMaps.Count() == 1,
                    "Can't Map the same column multiple times in the same end"
                );
                ...
                MemberProjectedSlot sSlot = new MemberProjectedSlot(
        ...
        private StorageEndPropertyMapping GetForeignKeyEndMapFromAssocitionMap(
            StorageAssociationSetMapping colocatedAssociationSetMap,
            EntitySetBase thisExtent
        )
        {
