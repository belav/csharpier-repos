...
namespace System.Data.Mapping.ViewGeneration.Validation
{
    ...
    internal class ForeignConstraint : InternalBase
    {
        ...
        internal static List<ForeignConstraint> GetForeignConstraints(EntityContainer container)
        {
            ...
            foreach (EntitySetBase extent in container.BaseEntitySets)
            {
                ...
                foreach (ReferentialConstraint constraint in relationType.ReferentialConstraints)
                {
                    ...
                    ForeignConstraint foreignKeyConstraint = new ForeignConstraint(
                        relationSet,
                        parentExtent,
                        childExtent,
        ...
        )
        {
            ...
            {
                string message =
                    System.Data.Entity.Strings.ViewGen_Foreign_Key_Missing_Table_Mapping(
                        ToUserString(),
                        ChildTable.Name
                    );
                ...
                ErrorLog.Record record = new ErrorLog.Record(
                    true,
                    ViewGenErrorCode.ForeignKeyMissingTableMapping,
                    message,
                    parentRewriter.UsedCells,
                    String.Empty
                );
            ...
            if (
                CheckIfConstraintMappedToForeignKeyAssociation(
                    childRewriter,
                    parentRewriter,
                    cells,
                    errorLog
            ...
            {
                GuaranteeForeignKeyConstraintInCSpace(
                    childRewriter,
                    parentRewriter,
                    errorLog,
                    config
            ...
            {
                GuaranteeMappedRelationshipForForeignKey(
                    childRewriter,
                    parentRewriter,
                    cells,
                    errorLog,
        ...
        )
        {
            ...
            FragmentQueryProcessor qp = FragmentQueryProcessor.Merge(
                childContext.RightFragmentQP,
                parentContext.RightFragmentQP
            );
            ...
            if (false == cImpliesP)
            {
                ...
                string message =
                    System.Data.Entity.Strings.ViewGen_Foreign_Key_Not_Guaranteed_InCSpace(
                        ToUserString()
                    );
                ...
                ErrorLog.Record record = new ErrorLog.Record(
                    true,
                    ViewGenErrorCode.ForeignKeyNotGuaranteedInCSpace,
                    message,
                    allWrappers,
                    String.Empty
                );
        ...
        )
        {
            ...
            ExtentKey primaryKey = ExtentKey.GetPrimaryKeyForEntityType(
                prefix,
            ...
            foreach (Cell cell in cells)
            {
                ...
                if (
                    ...
                    && CheckParentColumnsForForeignKey(
                        ...
                        cells,
                        parentEnd,
                        ref errorListForInvalidParentColumnsForForeignKey
                    ) == false
                )
                {
                ...
                else if (parentEnd != null)
                {
                    ...
                    foundCell = CheckConstraintWhenOnlyParentMapped(
                        cell,
                        parentSet,
                        assocSet,
                        parentEnd,
            ...
            if (!foundValidParentColumnsForForeignKey)
            {
                Debug.Assert(
                    errorListForInvalidParentColumnsForForeignKey != null
                        && errorListForInvalidParentColumnsForForeignKey.Count > 0
                );
            ...
            if (foundCell == false)
            {
                ...
                string message =
                    System.Data.Entity.Strings.ViewGen_Foreign_Key_Missing_Relationship_Mapping(
                        ToUserString()
                    );

                IEnumerable<LeftCellWrapper> parentWrappers = GetWrappersFromContext(
                    parentContext,
                    ParentTable
                );
                IEnumerable<LeftCellWrapper> childWrappers = GetWrappersFromContext(
                    childContext,
                    ChildTable
                );
                Set<LeftCellWrapper> bothExtentWrappers = new Set<LeftCellWrapper>(parentWrappers);
                bothExtentWrappers.AddRange(childWrappers);
                ErrorLog.Record record = new ErrorLog.Record(
                    true,
                    ViewGenErrorCode.ForeignKeyMissingRelationshipMapping,
                    message,
                    bothExtentWrappers,
                    String.Empty
                );
        ...
        )
        {
            ...
            if ((childPropertiesSet.Count != 0) && (parentPropertiesSet.Count != 0))
            {
                var foreignKeyAssociations = childContext
                    .EntityContainerMapping.EdmEntityContainer.BaseEntitySets.OfType<AssociationSet>()
                ...
                foreach (AssociationType association in foreignKeyAssociations)
                {
                    ReferentialConstraint refConstraint =
                        association.ReferentialConstraints.FirstOrDefault();
                    ...
                    var childRefPropertiesCollection = childPropertiesSet.Where(it =>
                        it.SetEquals(new Set<EdmProperty>(refConstraint.ToProperties))
                    );
                    var parentRefPropertiesCollection = parentPropertiesSet.Where(it =>
                        it.SetEquals(new Set<EdmProperty>(refConstraint.FromProperties))
                    );
                    if (
                        (
                            childRefPropertiesCollection.Count() != 0
                            && parentRefPropertiesCollection.Count() != 0
                        )
                    )
                    {
                        foreach (var parentRefProperties in parentRefPropertiesCollection)
                        {
                            var parentIndexes = GetPropertyIndexes(
                                parentRefProperties,
                                refConstraint.FromProperties
                            );
                            foreach (var childRefProperties in childRefPropertiesCollection)
                            {
                                var childIndexes = GetPropertyIndexes(
                                    childRefProperties,
                                    refConstraint.ToProperties
                                );
        ...
        private Set<int> GetPropertyIndexes(
        ...
        )
        {
            ...
            RoleBoolean endRoleBoolean = new RoleBoolean(
            ...
            FragmentQuery cNodeQuery = FragmentQuery.Create(
                pNode.RightFragmentQuery.Attributes,
                endCondition
            );
        ...
        )
        {
            ...
            if (parentEnd.RelationshipMultiplicity == RelationshipMultiplicity.Many)
            {
                ...
                string message =
                    System.Data.Entity.Strings.ViewGen_Foreign_Key_UpperBound_MustBeOne(
                        ToUserString(),
                        cell.CQuery.Extent.Name,
                        parentEnd.Name
                    );
                ErrorLog.Record record = new ErrorLog.Record(
                    true,
                    ViewGenErrorCode.ForeignKeyUpperBoundMustBeOne,
                    message,
                    cell,
                    String.Empty
                );
            ...
            if (
                MemberPath.AreAllMembersNullable(ChildColumns) == false
            ...
            )
            {
                ...
                string message =
                    System.Data.Entity.Strings.ViewGen_Foreign_Key_LowerBound_MustBeOne(
                        ToUserString(),
                        cell.CQuery.Extent.Name,
                        parentEnd.Name
                    );
                ErrorLog.Record record = new ErrorLog.Record(
                    true,
                    ViewGenErrorCode.ForeignKeyLowerBoundMustBeOne,
                    message,
                    cell,
                    String.Empty
                );
        ...
        )
        {
            ...
            if (entitySet == null || endSet.Equals(entitySet) == false)
            {
                ...
                string message =
                    System.Data.Entity.Strings.ViewGen_Foreign_Key_ParentTable_NotMappedToEnd(
                        ToUserString(),
                        ChildTable.Name,
                        cell.CQuery.Extent.Name,
                        parentEnd.Name,
                ...
                ErrorLog.Record record = new ErrorLog.Record(
                    true,
                    ViewGenErrorCode.ForeignKeyParentTableNotMappedToEnd,
                    message,
                    cell,
                    String.Empty
                );
        ...
        private static EntitySet FindEntitySetForColumnsMappedToEntityKeys(
            IEnumerable<Cell> cells,
        ...
        )
        {
            ...
            foreach (AssociationSetEnd relationEnd in relationSet.AssociationSetEnds)
            {
                ...
                ExtentKey primaryKey = ExtentKey.GetPrimaryKeyForEntityType(
                    prefix,
                    relationEnd.EntitySet.ElementType
                );
                ...
                if (endIndexes != null)
                {
                    ...
                    if (
                        Helpers.IsSetEqual(columnIndexes, endIndexes, EqualityComparer<int>.Default)
                    )
                    {
        ...
        )
        {
            ...
            {
                Debug.Assert(
                    context.Extent.Equals(extent),
                    "ViewgenContext extent and expected extent different"
                );
        ...
        private bool CheckForeignKeyColumnOrder(Set<Cell> cells, ErrorLog errorLog)
        {
            ...
            foreach (Cell childCell in childCells)
            {
                ...
                foreach (List<int> childSlotNums in allChildSlotNums)
                {
                    ...
                    foreach (int childSlotNum in childSlotNums)
                    {
                        ...
                        MemberProjectedSlot childSlot = (MemberProjectedSlot)
                    ...
                    {
                        List<List<int>> allParentSlotNums = GetSlotNumsForColumns(
                            parentCell,
                            ParentColumns
                        );
                        ...
                        foreach (List<int> parentSlotNums in allParentSlotNums)
                        {
                            ...
                            {
                                MemberProjectedSlot parentSlot = (MemberProjectedSlot)
                ...
                string message = EntityRes.GetString(
                ...
                ErrorLog.Record record = new ErrorLog.Record(
                    true,
                    ViewGenErrorCode.ForeignKeyColumnOrderIncorrect,
                    message,
                    new Cell[] { errorParentCell, childCell },
                    String.Empty
                );
                errorLog.AddEntry(record);
                return false;
            ...
            Debug.Assert(
                foundParentCell == true,
                "Some cell that mapped the parent's key must be present!"
            );
            Debug.Assert(
                foundChildCell == true,
                "Some cell that mapped the child's foreign key must be present according to the requires clause!"
            );
            return true;
        }
        ...
        internal string ToUserString()
        {
            ...
            string result = System.Data.Entity.Strings.ViewGen_Foreign_Key(
                m_fKeySet.Name,
                m_childTable.Name,
                childColsString,
                m_parentTable.Name,
