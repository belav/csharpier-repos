    ...
    internal class ViewgenContext : InternalBase
    {
        ...
        )
        {
            ...
            MemberDomainMap domainMap =
                viewTarget == ViewTarget.QueryView ? queryDomainMap : updateDomainMap;

            m_memberMaps = new MemberMaps(
                viewTarget,
                MemberProjectionIndex.Create(extent, m_edmItemCollection),
                queryDomainMap,
                updateDomainMap
            );
            ...
            MemberDomainMap rightDomainMap =
                viewTarget == ViewTarget.QueryView ? updateDomainMap : queryDomainMap;
            foreach (LeftCellWrapper leftCellWrapper in m_cellWrappers)
            {
                ...
                rightKB.CreateAssociationConstraints(
                    rightExtent,
                    rightDomainMap,
                    m_edmItemCollection
                );
            ...
            {
                CreateConstraintsForForeignKeyAssociationsAffectingThisWarapper(
                    rightKB,
                    rightDomainMap
                );
            }
        ...
        )
        {
            ...
            var entityTypes = m_cellWrappers
                .Select(it => it.RightExtent)
                .OfType<EntitySet>()
                .Select(it => it.ElementType);
            //Get all the foreign key association sets in these entity sets
            var allForeignKeyAssociationSets = this
            ...
            var oneToOneForeignKeyAssociationSetsForThisWrapper =
                allForeignKeyAssociationSets.Where(it =>
                    oneToOneForeignKeyAssociationsForThisWrapper.Contains(it.ElementType)
                );
            ...
            {
                rightKB.CreateEquivalenceConstraintForOneToOneForeignKeyAssociation(
                    assocSet,
                    rightDomainMap,
                    m_edmItemCollection
                );
        ...
        private void CheckConcurrencyControlTokens()
        {
            ...
            Set<EdmMember> tokenMembers = MetadataHelper.GetConcurrencyMembersForTypeHierarchy(
                extentType,
                m_edmItemCollection
            );
            ...
            if (tokenMembers.Count > 0)
            {
                foreach (LeftCellWrapper wrapper in m_cellWrappers)
                {
                    Set<MemberPath> conditionMembers = new Set<MemberPath>(
                        wrapper.OnlyInputCell.CQuery.WhereClause.MemberRestrictions.Select(oneOf =>
                    ...
                    if (conditionMembers.Count > 0)
                    {
                        ...
                        builder.AppendLine(
                            Strings.ViewGen_Concurrency_Invalid_Condition(
                                MemberPath.PropertiesToUserString(conditionMembers, false),
                                m_extent.Name
                            )
                        );
                        ErrorLog.Record record = new ErrorLog.Record(
                            true,
                            ViewGenErrorCode.ConcurrencyTokenHasCondition,
                            builder.ToString(),
                            new LeftCellWrapper[] { wrapper },
                            String.Empty
                        );
                        ExceptionHelpers.ThrowMappingException(record, m_config);
                    }
        ...
        private bool CreateLeftCellWrappers(IEnumerable<Cell> extentCells, ViewTarget viewTarget)
        {
            ...
            List<Cell> alignedCells = AlignFields(
                extentCellsList,
                m_memberMaps.ProjectedSlotMap,
                viewTarget
            );
            ...
            for (int i = 0; i < alignedCells.Count; i++)
            {
                ...
                BoolExpression fromVariable = BoolExpression.CreateLiteral(
                    new CellIdBoolean(m_identifiers, extentCellsList[i].CellNumber),
                    m_memberMaps.LeftDomainMap
                );
