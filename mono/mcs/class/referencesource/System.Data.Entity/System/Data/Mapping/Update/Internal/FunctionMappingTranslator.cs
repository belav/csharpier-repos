    ...
    internal abstract class ModificationFunctionMappingTranslator
    {
        ...
        private sealed class EntitySetTranslator : ModificationFunctionMappingTranslator
        {
            private readonly Dictionary<
                EntityType,
                StorageEntityTypeModificationFunctionMapping
            > m_typeMappings;
            ...
            internal EntitySetTranslator(StorageEntitySetMapping setMapping)
            {
                Debug.Assert(
                    null != setMapping
                        && null != setMapping.ModificationFunctionMappings
                        && 0 < setMapping.ModificationFunctionMappings.Count,
                    "set mapping must exist and must specify function mappings"
                );
                m_typeMappings =
                    new Dictionary<EntityType, StorageEntityTypeModificationFunctionMapping>();
                foreach (
                    StorageEntityTypeModificationFunctionMapping typeMapping in setMapping.ModificationFunctionMappings
                )
                {
            ...
            )
            {
                ...
                var currentReferenceEnds =
                    new Dictionary<AssociationEndMember, IEntityStateEntry>();
                var originalReferenceEnds =
                    new Dictionary<AssociationEndMember, IEntityStateEntry>();
                ...
                {
                    command = new FunctionUpdateCommand(
                        functionMapping,
                        translator,
                        stateEntries.ToList().AsReadOnly(),
                        stateEntry
                    );
            ...
            {
                Func<DbDataRecord, int, EntityKey> getEntityKey = (record, ordinal) =>
                    (EntityKey)record[ordinal];
                Action<DbDataRecord, Action<IEntityStateEntry>> findMatch = (
                    record,
                    registerTarget
                ) =>
                {
            ...
            > GetFunctionMapping(ExtractedStateEntry stateEntry)
            {
                ...
                StorageEntityTypeModificationFunctionMapping typeMapping = m_typeMappings[
                    entityType
                ];
            ...
            )
            {
                // bind all parameters
                foreach (
                    StorageModificationFunctionParameterBinding parameterBinding in functionMapping.ParameterBindings
                )
                {
                    ...
                    if (null != parameterBinding.MemberPath.AssociationSetEnd)
                    {
                        ...
                        if (!hasTarget)
                        {
                            if (endMember.RelationshipMultiplicity == RelationshipMultiplicity.One)
                            {
                                ...
                                throw EntityUtil.Update(
                                    Strings.Update_MissingRequiredRelationshipValue(
                                        entitySetName,
                                        associationSetName
                                    ),
                                    null,
                        ...
                        else
                        {
                            ...
                            EdmProperty keyProperty = (EdmProperty)
                                parameterBinding.MemberPath.Members[0];
                    ...
                    else
                    {
                        ...
                        result = parameterBinding.IsCurrent
                            ? stateEntry.Current
                            : stateEntry.Original;
        ...
        private sealed class AssociationSetTranslator : ModificationFunctionMappingTranslator
        {
            ...
            )
            {
                ...
                EntityUtil.ValidateNecessaryModificationFunctionMapping(
                    ...
                    stateEntry.Source,
                    "AssociationSet",
                    m_mapping.AssociationSet.Name
                );

                // initialize a new command
                StorageModificationFunctionMapping functionMapping = isInsert
                    ? m_mapping.InsertFunctionMapping
                    : m_mapping.DeleteFunctionMapping;
                FunctionUpdateCommand command = new FunctionUpdateCommand(
                    functionMapping,
                    translator,
                    new[] { stateEntry.Source }.ToList().AsReadOnly(),
                    stateEntry
                );
                ...
                )
                {
                    ...
                    Debug.Assert(
                        2 == parameterBinding.MemberPath.Members.Count,
                        "relationship parameter binding member "
                    ...
                    AssociationEndMember endMember = (AssociationEndMember)
                        parameterBinding.MemberPath.Members[1];
