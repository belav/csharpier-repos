...
namespace System.Data.Query.ResultAssembly
{
    ...
    internal sealed class BridgeDataReader : DbDataReader, IExtendedDataRecord
    {
        ...
        /// </summary>
        private IEnumerator<
            KeyValuePair<Shaper<RecordState>, CoordinatorFactory<RecordState>>
            ...
            : base()
        {
            ...
            Debug.Assert(
                depth == 0 || nextResultShaperInfos == null,
                "Nested data readers should not have multiple result sets."
            );

            NextResultShaperInfoEnumerator =
                nextResultShaperInfos != null ? nextResultShaperInfos : null;
        ...
        /// <returns></returns>
        static internal DbDataReader Create(
        ...
        )
        {
            ...
            DbDataReader result = new BridgeDataReader(
                shaperInfo.Key,
                shaperInfo.Value, /*depth:*/
                0,
                GetNextResultShaperInfo(storeDataReader, workspace, nextResultColumnMaps)
                    .GetEnumerator()
            );
            return result;
        }
        ...
        private static KeyValuePair<
            Shaper<RecordState>,
            CoordinatorFactory<RecordState>
        > CreateShaperInfo(
            DbDataReader storeDataReader,
            ColumnMap columnMap,
            MetadataWorkspace workspace
        )
        {
            ...
            ShaperFactory<RecordState> shaperFactory = Translator.TranslateColumnMap<RecordState>(
                cacheManager,
                columnMap,
                workspace,
                null,
            ...
            Shaper<RecordState> recordShaper = shaperFactory.Create(
                storeDataReader,
                null,
                workspace,
            ...
            return new KeyValuePair<Shaper<RecordState>, CoordinatorFactory<RecordState>>(
                recordShaper,
                recordShaper.RootCoordinator.TypedCoordinatorFactory
            );
        }
        ...
        override public DataTable GetSchemaTable()
        {
            throw EntityUtil.NotSupported(
                System.Data.Entity.Strings.ADP_GetSchemaTableIsNotSupported
            );
        }
