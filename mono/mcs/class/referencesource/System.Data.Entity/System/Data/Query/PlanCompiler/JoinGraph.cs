...
namespace System.Data.Query.PlanCompiler
{
    ...
    internal class JoinEdge
    {
        ...
        )
        {
            ...
            m_rightVars = rightVars;
            PlanCompiler.Assert(
                m_leftVars.Count == m_rightVars.Count,
                "Count mismatch: " + m_leftVars.Count + "," + m_rightVars.Count
            );
        }
        #endregion
        ...
        {
            get { return m_joinKind; }
            set { m_joinKind = value; }
        }
        ...
        )
        {
            ...
            JoinKind joinKind =
                joinOpType == OpType.LeftOuterJoin ? JoinKind.LeftOuter : JoinKind.Inner;
    ...
    internal class JoinGraph
    {
        ...
        )
        {
            ...
            predicate.GetEquiJoinPredicates(
                leftDefinitions,
                rightDefinitions,
                out tempLeftVars,
                out tempRightVars,
        ...
        private AugmentedNode BuildAugmentedNodeTree(Node node)
        {
            ...
            switch (node.Op.OpType)
            {
                ...
                case OpType.FullOuterJoin:
                    ...
                    augmentedNode = new AugmentedJoinNode(
                        m_vertexes.Count,
                        node,
                        left,
                        right,
        ...
        private bool AddJoinEdge(AugmentedJoinNode joinNode, ColumnVar leftVar, ColumnVar rightVar)
        {
            ...
            //
            if (
                leftTableNode.LastVisibleId < joinNode.Id
            ...
            JoinEdge newJoinEdge = JoinEdge.CreateJoinEdge(
                leftTableNode,
                rightTableNode,
                joinNode,
                leftVar,
        ...
        private void BuildJoinEdges(AugmentedJoinNode joinNode, int maxVisibility)
        {
            ...
            if (
                joinNode.Node.Op.OpType == OpType.FullOuterJoin
            ...
            JoinKind joinKind =
                (opType == OpType.LeftOuterJoin) ? JoinKind.LeftOuter : JoinKind.Inner;
        ...
        private bool GenerateTransitiveEdge(JoinEdge edge1, JoinEdge edge2)
        {
            PlanCompiler.Assert(
                ...
                "need a common table for transitive predicate generation"
            );
            ...
            if (
                edge1.JoinKind == JoinKind.LeftOuter
            ...
            IEnumerable<KeyValuePair<ColumnVar, ColumnVar>> orderedEdge1Vars =
                CreateOrderedKeyValueList(edge1.RightVars, edge1.LeftVars);
            IEnumerable<KeyValuePair<ColumnVar, ColumnVar>> orderedEdge2Vars =
                CreateOrderedKeyValueList(edge2.LeftVars, edge2.RightVars);

            IEnumerator<KeyValuePair<ColumnVar, ColumnVar>> orderedEdge1VarsEnumerator =
                orderedEdge1Vars.GetEnumerator();
            IEnumerator<KeyValuePair<ColumnVar, ColumnVar>> orderedEdge2VarsEnumerator =
                orderedEdge2Vars.GetEnumerator();

            List<ColumnVar> leftVars = new List<ColumnVar>();
            List<ColumnVar> rightVars = new List<ColumnVar>();

            bool hasMore =
                orderedEdge1VarsEnumerator.MoveNext() && orderedEdge2VarsEnumerator.MoveNext();
            while (hasMore)
            {
                if (
                    orderedEdge1VarsEnumerator.Current.Key == orderedEdge2VarsEnumerator.Current.Key
                )
                {
                    leftVars.Add(orderedEdge1VarsEnumerator.Current.Value);
                    rightVars.Add(orderedEdge2VarsEnumerator.Current.Value);
                    hasMore =
                        orderedEdge1VarsEnumerator.MoveNext()
                ...
                else if (edge1.JoinKind == JoinKind.LeftOuter)
                {
                    ...
                    orderedEdge1VarsEnumerator.Current.Key.Id
            ...
            JoinEdge newEdge = JoinEdge.CreateTransitiveJoinEdge(
                edge1.Left,
                edge2.Right,
                edge1.JoinKind,
                leftVars,
            ...
            {
                JoinEdge reverseEdge = JoinEdge.CreateTransitiveJoinEdge(
                    edge2.Right,
                    edge1.Left,
                    edge1.JoinKind,
                    rightVars,
        ...
        {
            List<KeyValuePair<ColumnVar, ColumnVar>> edgeVars = new List<
                KeyValuePair<ColumnVar, ColumnVar>
            >(keyVars.Count);
        ...
        {
            foreach (
                AugmentedJoinNode augmentedJoinNode in m_vertexes
                    .OfType<AugmentedJoinNode>()
            ...
            )
            {
                ...
                )
                {
                    ...
                    List<JoinEdge> newJoinEdges = new List<JoinEdge>(
                    ...
                    foreach (JoinEdge joinEdge in augmentedJoinNode.JoinEdges)
                    {
                        ...
                        if (
                            !ContainsJoinEdgeForTable(joinEdge.Right.JoinEdges, joinEdge.Left.Table)
                        )
                        {
                            ...
                            {
                                newJoinEdge.AddCondition(
                                    augmentedJoinNode,
                                    joinEdge.RightVars[i],
                                    joinEdge.LeftVars[i]
                                );
                            }
                        }
        ...
        private bool AreAllTableRowsPreserved(AugmentedNode root, AugmentedTableNode table)
        {
            ...
            do
            {
                ...
                if (
                    parent.Node.Op.OpType != OpType.LeftOuterJoin
        ...
        private bool IsConstraintPresentForTurningIntoInnerJoin(JoinEdge joinEdge)
        {
            ...
            if (
                m_constraintManager.IsParentChildRelationship(
                    joinEdge.Right.Table.TableMetadata.Extent,
                    joinEdge.Left.Table.TableMetadata.Extent,
                    out fkConstraints
                )
            )
            {
                PlanCompiler.Assert(
                    fkConstraints != null && fkConstraints.Count > 0,
                    "invalid fk constraints?"
                );
                foreach (ForeignKeyConstraint fkConstraint in fkConstraints)
                {
                    ...
                    {
                        if (
                            fkConstraint.ParentKeys.Count == joinEdge.RightVars.Count
        ...
        )
        {
            ...
            AugmentedNode leastCommonAncestor = GetLeastCommonAncestor(
                tableJoinEdge.Right,
                replacingTableJoinEdge.Right
            );
            return !CanGetFileredByJoins(tableJoinEdge, leastCommonAncestor, true)
        ...
        )
        {
            ...
            while (currentParent != null && currentNode != leastCommonAncestor)
            {
                ...
                if (
                    currentParent.Node != joinEdge.JoinNode.Node
                    && (
                        disallowAnyJoin
                        || currentParent.Node.Op.OpType != OpType.LeftOuterJoin
        ...
        )
        {
            ...
            while (currentNode.Parent != null && currentNode != leastCommonAncestor)
            {
                ...
                if (
                    currentNode.Parent.Node.Op.OpType == OpType.LeftOuterJoin
        ...
        {
            MarkTableAsEliminated<Var>(
                tableNode,
                replacementNode,
                tableNode.Table.Columns,
                replacementNode.Table.Columns
            );
        }
        ...
        private void EliminateStarSelfJoin(List<JoinEdge> joinEdges)
        {
            ...
            foreach (List<JoinEdge> joinList in compatibleGroups.Where(l => l.Count > 1))
            {
                ...
                foreach (JoinEdge joinEdge in joinList)
                {
                    ...
                    if (
                        CanBeEliminatedViaStarJoinBasedOnOtherJoinParticipation(
                            joinEdge,
                            smallestEdge
                        )
                    )
        ...
        private void EliminateStarSelfJoins(AugmentedTableNode tableNode)
        {
            ...
            Dictionary<md.EntitySetBase, List<JoinEdge>> groupedEdges =
                new Dictionary<md.EntitySetBase, List<JoinEdge>>();
        ...
        private void EliminateLeftTable(JoinEdge joinEdge)
        {
            ...
            MarkTableAsEliminated<ColumnVar>(
                joinEdge.Left,
                joinEdge.Right,
                joinEdge.LeftVars,
                joinEdge.RightVars
        ...
        {
            PlanCompiler.Assert(
                joinEdge.JoinKind == JoinKind.LeftOuter,
        ...
        private bool RightTableHasKeyReferences(JoinEdge joinEdge)
        {
            ...
            return m_varRefManager.HasKeyReferences(
                keys,
                joinEdge.Right.Node,
                joinEdge.JoinNode.Node
            );
        }
        ...
        )
        {
            ...
            else
            {
                ...
                return TryEliminateRightTable(
                    ...
                    fkConstraint.ChildMultiplicity == md.RelationshipMultiplicity.One
                );
            }
        }
            ...
            foreach (string keyProp in fkConstraint.ChildKeys)
            {
                ...
                for (int pos = 0; pos < parentVars.Count; pos++)
                {
                    ...
                    if (rightVar.ColumnMetadata.Name.Equals(keyProp))
                    {
                        ...
                        if (
                            !fkConstraint.GetParentProperty(
                                rightVar.ColumnMetadata.Name,
                                out parentPropertyName
        ...
        )
        {
            ...
            if (
                (!allowRefsForJoinedOnFkOnly || joinEdge.RightVars.Count != fkConstraintKeyCount)
        ...
        private Node BuildFilterForNullableColumns(Node inputNode, VarVec nonNullableColumns)
        {
            ...
            foreach (Var v in remappedVarVec)
            {
                ...
                Node isNotNullNode = m_command.CreateNode(
                    m_command.CreateConditionalOp(OpType.IsNull),
                    varRefNode
                );
                isNotNullNode = m_command.CreateNode(
                    m_command.CreateConditionalOp(OpType.Not),
                    isNotNullNode
                );
                if (predNode == null)
                {
        ...
        private Node RebuildPredicate(AugmentedJoinNode joinNode, out int minLocationId)
        {
            ...
            if (joinNode.OtherPredicate != null)
            {
                ...
                )
                {
                    ...
                    minLocationId = GetLeastCommonAncestor(
                        minLocationId,
                        GetLocationId(newVar, minLocationId)
                    );
                }
            }
            ...
            for (int i = 0; i < joinNode.LeftVars.Count; i++)
            {
                ...
                minLocationId = GetLeastCommonAncestor(
                    minLocationId,
                    GetLocationId(newLeftVar, minLocationId)
                );
                minLocationId = GetLeastCommonAncestor(
                    minLocationId,
                    GetLocationId(newRightVar, minLocationId)
                );
                ...
                Node equalsNode = m_command.CreateNode(
                ...
                if (predicateNode != null)
                {
                    predicateNode = PlanCompilerUtil.CombinePredicates(
                        equalsNode,
                        predicateNode,
                        m_command
                    );
        ...
        private Node RebuildNodeTree(
            AugmentedJoinNode joinNode,
            out Dictionary<Node, int> predicates
        )
        {
            //
            ...
            else
            {
                ...
                Node newJoinNode = m_command.CreateNode(
                    joinNode.Node.Op,
                    leftNode,
                    rightNode,
                    localPredicateNode
        ...
        private Node RebuildNodeTree(AugmentedTableNode tableNode)
        {
            ...
            //
            Node filterNode = BuildFilterForNullableColumns(
        ...
        private Node ClassifyPredicate(
            ...
            Node result,
            Dictionary<Node, int> outPredicates
        )
        {
