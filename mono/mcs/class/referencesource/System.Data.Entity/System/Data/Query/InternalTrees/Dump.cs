    ...
    internal class Dump : BasicOpVisitor, IDisposable
    {
        ...
        protected override void VisitNestOp(NestBaseOp op, Node n)
        {
            ...
            if (null != ssnOp)
            {
                attrs.Add(
                    "Discriminator",
                    (ssnOp.Discriminator == null) ? "<null>" : ssnOp.Discriminator.ToString()
                );
            }
                ...
                foreach (CollectionInfo ci in op.CollectionInfo)
                {
                    ...
                    if (0 != ci.FlattenedElementVars.Count)
                    {
                        attrs2.Add(
                            ...
                            FormatVarList(sb, ci.FlattenedElementVars)
                        );
                    }
                    if (0 != ci.Keys.Count)
                    {
        ...
        internal class ColumnMapDumper : ColumnMapVisitor<Dump>
        {
            ...
            )
            {
                using (
                    new AutoXml(
                        dumper,
                        "MultipleDiscriminatorPolymorphic",
                        GetAttributes(columnMap)
                    )
                )
                {
