...
namespace System.Data.Query.PlanCompiler
{
        ...
        : ColumnMapVisitorWithResults<ColumnMap, ColumnMapTranslatorTranslationDelegate>
    {
        ...
        private static Var GetReplacementVar(
            Var originalVar,
            Dictionary<Var, Var> replacementVarMap
        )
        {
            ...
            ColumnMap result = Translate(
                ...
                delegate(ColumnMap columnMap)
                {
                    ...
                    if (null != varRefColumnMap)
                    {
                        ...
                        {
                            columnMap = new VarRefColumnMap(
                                varRefColumnMap.Type,
                                varRefColumnMap.Name,
                                replacementVar
                            );
                        }
                    }
            ...
            ColumnMap result = Translate(
                ...
                delegate(ColumnMap columnMap)
                {
                    ...
                    if (null != varRefColumnMap)
                    {
                        ...
                        if (
                            !varToCommandColumnMap.TryGetValue(
                                varRefColumnMap.Var,
                                out commandAndColumn
                            )
                        )
                        {
                            throw EntityUtil.InternalError(
                                EntityUtil.InternalErrorCode.UnknownVar,
                                1,
                                varRefColumnMap.Var.Id
                            ); // shouldn't have gotten here without having a resolveable var
                        }
                        columnMap = new ScalarColumnMap(
                            varRefColumnMap.Type,
                            varRefColumnMap.Name,
                            commandAndColumn.Key,
                            commandAndColumn.Value
                        );
                    }
        ...
        )
        {
            ...
            if (newEntitySetColumnMap != entityIdentity.EntitySetColumnMap)
            {
                entityIdentity = new DiscriminatedEntityIdentity(
                    (SimpleColumnMap)newEntitySetColumnMap,
                    entityIdentity.EntitySetMap,
                    entityIdentity.Keys
                );
            }
        ...
        )
        {
            ...
            {
                columnMap = new ComplexTypeColumnMap(
                    columnMap.Type,
                    columnMap.Name,
        ...
        )
        {
            ...
            if (newDiscriminator != columnMap.Discriminator || newElement != columnMap.Element)
            {
                columnMap = new DiscriminatedCollectionColumnMap(
                    ...
                    (SimpleColumnMap)newDiscriminator,
        ...
        {
            EntityIdentity newEntityIdentity = VisitEntityIdentity(
                columnMap.EntityIdentity,
                translationDelegate
            );
            VisitList(columnMap.Properties, translationDelegate);
        ...
        )
        {
            ...
            {
                TypedColumnMap newTypeChoice = (TypedColumnMap)
                    kv.Value.Accept(this, translationDelegate);

                if (newTypeChoice != kv.Value)
                {
                    if (newTypeChoices == columnMap.TypeChoices)
                    {
                        newTypeChoices = new Dictionary<object, TypedColumnMap>(
                            columnMap.TypeChoices
                        );
                    }
            ...
            if (
                newTypeDiscriminator != columnMap.TypeDiscriminator
            ...
            )
            {
                columnMap = new SimplePolymorphicColumnMap(
                    ...
                    columnMap.Name,
                    columnMap.Properties,
                    (SimpleColumnMap)newTypeDiscriminator,
                    newTypeChoices
                );
            }
            return translationDelegate(columnMap);
