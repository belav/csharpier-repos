...
namespace System.Data.Query.InternalTrees
{
    ...
    internal static class ColumnMapFactory
    {
        ...
        {
            FunctionImportStructuralTypeMappingKB resultMapping = mapping.GetResultMapping(
                resultSetIndex
            );
            ...
            if (resultMapping.NormalizedEntityTypeMappings.Count == 0) // no explicit mapping; use default non-polymorphic reader
            {
                ...
                return CreateColumnMapFromReaderAndType(
                    ...
                    baseStructuralType,
                    entitySet,
                    resultMapping.ReturnTypeColumnsRenameMapping
                );
            }
            ...
            ScalarColumnMap[] discriminatorColumns = CreateDiscriminatorColumnMaps(
                storeDataReader,
                mapping,
                resultSetIndex
            );
            ...
            Dictionary<EntityType, TypedColumnMap> typeChoices = new Dictionary<
                EntityType,
                TypedColumnMap
            ...
            {
                ColumnMap[] propertyColumnMaps = GetColumnMapsForType(
                    storeDataReader,
                    entityType,
                    resultMapping.ReturnTypeColumnsRenameMapping
                );
                EntityColumnMap entityColumnMap = CreateEntityTypeElementColumnMap(
                    storeDataReader,
                    entityType,
                    entitySet,
                    propertyColumnMaps,
            ...
            MultipleDiscriminatorPolymorphicColumnMap polymorphicMap =
                new MultipleDiscriminatorPolymorphicColumnMap(
                    TypeUsage.Create(baseStructuralType),
                    baseStructuralType.Name,
                    baseTypeColumnMaps,
                    discriminatorColumns,
                    typeChoices,
                    (object[] discriminatorValues) =>
                        mapping.Discriminate(discriminatorValues, resultSetIndex)
                );
            CollectionColumnMap collection = new SimpleCollectionColumnMap(
                baseStructuralType.GetCollectionType().TypeUsage,
                baseStructuralType.Name,
                polymorphicMap,
                null,
                null
        ...
        )
        {
            ...
            {
                elementColumnMap = new ComplexTypeColumnMap(
                    TypeUsage.Create(edmType),
                    edmType.Name,
                    propertyColumnMaps,
                    null
            ...
            else if (Helper.IsScalarType(edmType))
            {
                ...
                elementColumnMap = new ScalarColumnMap(
                    TypeUsage.Create(edmType),
                    edmType.Name,
                    0,
                    0
            ...
            CollectionColumnMap collection = new SimpleCollectionColumnMap(
                edmType.GetCollectionType().TypeUsage,
                edmType.Name,
                elementColumnMap,
                null,
                null
        ...
        )
        {
            ...
            )
            {
                ...
                var propertyUnderlyingType =
                    Nullable.GetUnderlyingType(prop.PropertyType) ?? prop.PropertyType;
                Type propType = propertyUnderlyingType.IsEnum
                    ? propertyUnderlyingType.GetEnumUnderlyingType()
                    : prop.PropertyType;

                EdmType modelType;
                int ordinal;
                if (
                    TryGetColumnOrdinalFromReader(reader, prop.Name, out ordinal)
                ...
                )
                {
                    memberInfo.Add(
                        Tuple.Create(
                            Expression.Bind(
                                prop,
                                Expression.Parameter(prop.PropertyType, "placeholder")
                            ),
                            ordinal,
            ...
            )
            {
                ...
                if (memberGroup.Count() != 1)
                {
                    throw EntityUtil.InvalidOperation(
                        Strings.ObjectContext_TwoPropertiesMappedToSameColumn(
                            ...
                            String.Join(
                                ", ",
                                memberGroup.Select(tuple => tuple.Item3.Name).ToArray()
                            )
                        )
                    );
                }
                ...
                propertyMaps[i] = new ScalarColumnMap(
                    modelProp.TypeUsage,
                    modelProp.Name,
                    0,
                    ordinal
            ...
            NewExpression newExpr =
                null == constructor ? Expression.New(type) : Expression.New(constructor);
            ...
            InitializerMetadata initMetadata = InitializerMetadata.CreateProjectionInitializer(
                (EdmItemCollection)workspace.GetItemCollection(DataSpace.CSpace),
                init,
                members
            );
            ...
            RecordColumnMap rowMap = new RecordColumnMap(
                TypeUsage.Create(rowType),
                "DefaultTypeProjection",
                propertyMaps,
                null
            );
        ...
        )
        {
            ...
            {
                int keyOrdinal = GetMemberOrdinalFromReader(
                    storeDataReader,
                    keyMember,
                    edmType,
                    renameList
                ...
                Debug.Assert(
                    keyOrdinal >= 0,
                    "keyMember for entity is not found by name in the data reader?"
                );
                ...
                Debug.Assert(
                    null != keyColumnMap,
                    "keyMember for entity isn't in properties collection for the entity?"
                );
            ...
            EntityColumnMap result = new EntityColumnMap(
                TypeUsage.Create(edmType),
                edmType.Name,
                propertyColumnMaps,
                entityIdentity
        ...
        )
        {
            ...
            foreach (EdmMember member in members)
            {
                ...
                int ordinal = GetMemberOrdinalFromReader(
                    storeDataReader,
                    member,
                    edmType,
                    renameList
                ...
                propertyColumnMaps[index] = new ScalarColumnMap(
                    member.TypeUsage,
                    member.Name,
                    0,
                    ordinal
        ...
        )
        {
            ...
            EdmType discriminatorType = MetadataItem.EdmProviderManifest
            ...
            ScalarColumnMap[] discriminatorColumns = new ScalarColumnMap[
                discriminatorColumnNames.Count
            ];
            for (int i = 0; i < discriminatorColumns.Length; i++)
            {
                string columnName = discriminatorColumnNames[i];
                ScalarColumnMap columnMap = new ScalarColumnMap(
                    discriminatorTypeUsage,
                    columnName,
                    0,
                    GetDiscriminatorOrdinalFromReader(
                        storeDataReader,
                        columnName,
                        mapping.FunctionImport
                    )
                );
        ...
        )
        {
            ...
            {
                throw EntityUtil.CommandExecutionDataReaderMissingColumnForType(
                    member,
                    currentType
                );
            }
        ...
        )
        {
            ...
            return
                renameList == null
                || renameList.Count == 0
                || !renameList.Any(m => m.Key == member.Name)
                ? member.Name
        ...
        )
        {
            ...
            {
                throw EntityUtil.CommandExecutionDataReaderMissinDiscriminatorColumn(
                    columnName,
                    functionImport
                );
            }
