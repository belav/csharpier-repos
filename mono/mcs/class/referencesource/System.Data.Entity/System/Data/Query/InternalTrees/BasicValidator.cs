...
namespace System.Data.Query.InternalTrees
{
    ...
    internal class BasicValidator : BasicOpVisitor
    {
        ...
        {
            Assert(
                arity == n.Children.Count,
                "Op Arity mismatch for Op {0}: Expected {1} arguments; found {2} arguments",
                n.Op.OpType,
                arity,
                n.Children.Count
            );
        ...
        protected static void AssertBoolean(TypeUsage type)
        {
            Assert(
                ...
                "Type Mismatch: Expected Boolean; found {0} instead",
        ...
        protected static void AssertCollectionType(TypeUsage type)
        {
            Assert(
                ...
                "Type Mismatch: Expected Collection type: Found {0}",
                TypeHelpers.GetFullName(type)
            );
        }
        ...
        {
            Assert(
                op.IsRelOp || op.IsPhysicalOp,
                "OpType Mismatch: Expected RelOp or PhysicalOp; found {0}",
                op.OpType
            );
        }
        ...
        protected static void AssertOpType(Op op, OpType opType)
        {
            Assert(
                op.OpType == opType,
                "OpType Mismatch: Expected {0}; found {1}",
                op.OpType,
                Dump.AutoString.ToString(opType)
            );
        }
        ...
        public override void Visit(CaseOp op, Node n)
        {
            ...
            Assert(
                ...
                "CaseOp: Expected odd number of arguments, and at least 3; found {0}",
                n.Children.Count
            );
            ...
            {
                Assert(
                    TypeSemantics.IsBooleanType(n.Children[i].Op.Type),
                    "Encountered a when node with a non-boolean return type"
                );
            }
        ...
        public override void Visit(DerefOp op, Node n)
        {
            VisitScalarOpDefault(op, n);
            Assert(
                TypeSemantics.IsEntityType(op.Type),
                "Expected an entity type. Found " + op.Type
            );
            Assert(
                TypeSemantics.IsReferenceType(n.Child0.Op.Type),
                "Expected a ref type. Found " + n.Child0.Op.Type
            );
        ...
        public override void Visit(RelPropertyOp op, Node n)
        {
            VisitScalarOpDefault(op, n);
            Assert(
                m_command.IsRelPropertyReferenced(op.PropertyInfo),
                "no such rel property:",
                op.PropertyInfo
            );
            Assert(
                TypeSemantics.IsEntityType(n.Child0.Op.Type),
                "argument to RelPropertyOp must be an entity type. Found: ",
                n.Child0.Op.Type
            );
        }
        ...
        public override void Visit(FunctionOp op, Node n)
        {
            VisitScalarOpDefault(op, n);
            Assert(
                op.Function.Parameters.Count == n.Children.Count,
                "FunctionOp: Argument count ({0}) does not match parameter count ({1})",
                n.Children.Count,
                op.Function.Parameters.Count
            );
        ...
        protected override void VisitJoinOp(JoinBaseOp op, Node n)
        {
            ...
            if (op.OpType == OpType.CrossJoin)
            {
                Assert(
                    n.Children.Count >= 2,
                    "CrossJoinOp needs at least 2 arguments; found only {0}",
                    n.Children.Count
                );
                return;
            }
        ...
        public override void Visit(ConstrainedSortOp op, Node n)
        {
            ...
            Assert(
                TypeSemantics.IsIntegerNumericType(n.Child1.Op.Type),
                "ConstrainedSortOp Skip Count Node must have an integer result type"
            );
            AssertScalarOp(n.Child2.Op);
            Assert(
                TypeSemantics.IsIntegerNumericType(n.Child2.Op.Type),
                "ConstrainedSortOp Limit Node must have an integer result type"
            );
        }
        ...
        public override void Visit(PhysicalProjectOp op, Node n)
        {
            ...
            Assert(
                n.Children.Count >= 1,
                "PhysicalProjectOp needs at least 1 arg: found {0}",
                n.Children.Count
            );
            foreach (Node chi in n.Children)
            {
        ...
        public override void Visit(MultiStreamNestOp op, Node n)
        {
            ...
            Assert(
                n.Children.Count > 1,
                "MultiStreamNestOp needs at least 2 arguments: found {0}",
                n.Children.Count
            );
            foreach (Node chi in n.Children)
            {
