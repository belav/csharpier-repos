...
namespace System.Data.Query.ResultAssembly
{
    ...
    /// </summary>
    sealed class BridgeDataRecord : DbDataRecord, IExtendedDataRecord
    {
        #region state
        ...
        private void AssertSequentialAccess(int ordinal, long dataOffset, string methodName)
        {
            ...
            if (_lastColumnRead == ordinal)
            {
                ...
                {
                    throw EntityUtil.NonSequentialArrayOffsetAccess(
                        dataOffset,
                        _lastDataOffsetRead + 1,
                        methodName
                    );
                ...
                else
                {
                    ...
                    if (null != coordinator)
                    {
                        BridgeDataReader nestedReader = new BridgeDataReader(
                            Shaper,
                            coordinator.TypedCoordinatorFactory,
                            Depth + 1,
                            nextResultShaperInfos: null
                        );
                        result = nestedReader;
                        _currentNestedRecord = null;
                    ...
                    {
                        Debug.Fail(
                            "unexpected type of nested object result: "
