...
namespace System.Data.EntityClient
{
    ...
    sealed internal class EntityCommandDefinition : DbCommandDefinition
    {
        ...
        )
        {
            ...
            DbProviderServices storeProviderServices = DbProviderServices.GetProviderServices(
            ...
            try
            {
                if (DbCommandTreeKind.Query == commandTree.CommandTreeKind)
                {
                    ...
                    Debug.Assert(
                        mappedCommandList.Count > 0,
                        "empty providerCommandInfo collection and no exception?"
                    ); // this shouldn't ever happen.

                    // Then, generate the store commands from the resulting command tree(s)
                    _mappedCommandDefinitions = new List<DbCommandDefinition>(
                        mappedCommandList.Count
                    );

                    foreach (ProviderCommandInfo providerCommandInfo in mappedCommandList)
                    {
                        DbCommandDefinition providerCommandDefinition =
                            storeProviderServices.CreateCommandDefinition(
                                providerCommandInfo.CommandTree
                            );
                ...
                {
                    Debug.Assert(
                        DbCommandTreeKind.Function == commandTree.CommandTreeKind,
                        "only query and function command trees are supported"
                    );
                    ...
                    TypeUsage storeResultType = DetermineStoreResultType(
                        entityCommandTree.MetadataWorkspace,
                        mapping,
                        0,
                        out _columnMapGenerators[0]
                    );
                    for (int i = 1; i < resultSetCount; i++)
                    {
                        DetermineStoreResultType(
                            entityCommandTree.MetadataWorkspace,
                            mapping,
                            i,
                            out _columnMapGenerators[i]
                        );
                    }
                    // Copy over parameters (this happens through a more indirect route in the plan compiler, but
                    // it happens nonetheless)
                    List<KeyValuePair<string, TypeUsage>> providerParameters =
                        new List<KeyValuePair<string, TypeUsage>>();
                    foreach (
                        KeyValuePair<string, TypeUsage> parameter in entityCommandTree.Parameters
                    )
                    {
                    ...
                    DbFunctionCommandTree providerCommandTree = new DbFunctionCommandTree(
                        entityCommandTree.MetadataWorkspace,
                        DataSpace.SSpace,
                        mapping.TargetFunction,
                        storeResultType,
                        providerParameters
                    );

                    DbCommandDefinition storeCommandDefinition =
                ...
                {
                    EntityParameter parameter = CreateEntityParameterFromQueryParameter(
                        queryParameter
                    );
                ...
                _parameters =
                    new System.Collections.ObjectModel.ReadOnlyCollection<EntityParameter>(
                        parameterList
                    );
            }
        ...
        private TypeUsage DetermineStoreResultType(
            ...
            int resultSetIndex,
            out IColumnMapGenerator columnMapGenerator
        )
        {
            ...
            TypeUsage storeResultType;
            {
                ...
                )
                {
                    ...
                    storeResultType = mapping.GetExpectedTargetResultType(
                        workspace,
                        resultSetIndex
                    );
                }
                ...
                {
                    FunctionParameter returnParameter = MetadataHelper.GetReturnParameter(
                        functionImport,
                        resultSetIndex
                    );
                    if (returnParameter != null && returnParameter.TypeUsage != null)
                    {
                        ...
                        Debug.Assert(
                            storeResultType.EdmType.BuiltInTypeKind
                        ...
                        ScalarColumnMap scalarColumnMap = new ScalarColumnMap(
                            elementType,
                            string.Empty,
                            0,
                            0
                        );
                        SimpleCollectionColumnMap collectionColumnMap =
        ...
        {
            Debug.Assert(
                functionCommandTree.DataSpace == DataSpace.CSpace,
                "map from CSpace->SSpace function"
            );
            Debug.Assert(functionCommandTree != null, "null functionCommandTree");
            Debug.Assert(
                !functionCommandTree.EdmFunction.IsComposableAttribute,
        ...
        )
        {
            ...
            Debug.Assert(
                TypeSemantics.IsScalarType(queryParameter.Value),
        ...
        /// <param name="entityCommand"></param>
        /// <param name="behavior"></param>
        internal DbDataReader ExecuteStoreCommands(
        ...
        )
        {
            ...
            CommandHelper.SetStoreProviderCommandState(
                entityCommand,
                entityTransaction,
            ...
            {
                DbProviderServices storeProviderServices = DbProviderServices.GetProviderServices(
                    entityCommand.Connection.StoreProviderFactory
                );
            ...
            try
            {
                reader = storeProviderCommand.ExecuteReader(
        ...
        )
        {
            ...
            storeProviderServices.SetParameterValue(
                storeParameter,
                parameterTypeUsage,
