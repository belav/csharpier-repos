...
namespace System.Data.Objects
{
    ...
    internal sealed class RelationshipEntry : ObjectStateEntry
    {
        ...
        )
        {
            ...
            if (
                _relationshipWrapper.AssociationSet.ElementType.AssociationEndMembers[0].Name
                == associationMember.Name
            )
            {
        ...
        internal void DeleteUnnecessaryKeyEntries()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (relatedEntry.IsKeyEntry)
                {
                    ...
                    foreach (
                        RelationshipEntry relationshipEntry in _cache.FindRelationshipsByKey(
                            entityKey
                        )
                    )
                    {
        ...
        )
        {
            Debug.Assert(
                ...
                "Invalid requested state for relationsihp"
            );
            Debug.Assert(
                this.State != EntityState.Modified,
                "Invalid initial state for relationsihp"
            );
