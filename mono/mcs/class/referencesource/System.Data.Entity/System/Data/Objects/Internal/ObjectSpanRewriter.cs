...
namespace System.Data.Objects.Internal
{
    ...
    {
        internal static bool EntityTypeEquals(
            EntityTypeBase entityType1,
        ...
        )
        {
            ...
            if (rewriter != null)
            {
                ...
                {
                    Debug.Assert(
                        rewriter.SpanIndex != null
                            || tree.Query.ResultType.EdmEquals(newQuery.ResultType),
        ...
        /// <summary>
        /// Gets the metadata workspace the will be used to retrieve required metadata, for example
        // association types.
        /// </summary>
        ...
        /// <summary>
        /// Gets a value indicating whether relationship span is required (ObjectQuery sets this to 'false'
        // for NoTracking queries).
        /// </summary>
        ...
        /// This dictionary is initially empty before <see cref="RewriteQuery"/> is called and will remain
        // so
        ...
        /// </summary>
        /// <returns>The rewritten version of <see cref="Query"/> if spanning was required; otherwise
        // <c>null</c>.</returns>
        ...
        internal SpanTrackingInfo InitializeTrackingInfo(bool createAssociationEndTrackingInfo)
        {
            ...
            info.ColumnNames = new AliasGenerator(
        ...
        internal virtual SpanTrackingInfo CreateEntitySpanTrackingInfo(
            ...
            switch (expression.ResultType.EdmType.BuiltInTypeKind)
            {
                ...
                case BuiltInTypeKind.CollectionType:
                    return RewriteCollection(
                        expression,
                        (CollectionType)expression.ResultType.EdmType
                    );
        ...
        private DbExpression RewriteEntity(DbExpression expression, EntityType entityType)
        {
            ...
            if (relationshipSpans != null)
            {
                ...
                // Track column index to span information, starting at the current column count (which could be
                // zero) plus 1.
                // 1 is added because the column containing the root entity will be added later to provide column
                // zero.
                ...
                )
                {
                    ...
                    // columns do not indicate that they include the target association end member of this relationship
                    // span.
                    if (
                        null == tracking.FullSpannedEnds
                    ...
                    )
                    {
                        ...
                        if (!TryGetNavigationSource(relSpan.Value, out columnDef))
                        {
                            ...
                            columnDef =
                                navSource.NavigateAllowingAllRelationshipsInSameTypeHierarchy(
                                    relSpan.Key,
                                    relSpan.Value
                                );
                        }
        ...
        private DbExpression RewriteRow(DbExpression expression, RowType rowType)
        {
            ...
            {
                // NOTE: We rely on the fact that today span cannot be done over queries containing
                // DbLambdaExpressions
            ...
            {
                // At least one column was spanned, so build a new row constructor that defines the new row,
                // including spanned columns.
                List<DbExpression> columnArguments = new List<DbExpression>(
                    rowType.Properties.Count
                );
                ...
                // In particular, if the input row is null and we construct a new row as a projection over its
                // columns
                ...
                if (newRow == null)
                {
                    DbExpression condition =
                        DbExpressionBuilder.CreateIsNullExpressionAllowingRowTypeArgument(
                            expression
                        );
                ...
                {
                    rewritten = DbLambda
                        .Create(rewritten, lambdaExpression.Lambda.Variables)
                        .Invoke(lambdaExpression.Arguments);
                }
        ...
        )
        {
            ...
            // If the collection expression is a project expression, get a strongly typed reference to it for
            // later use.
            ...
            if (this.RelationshipSpan)
            {
                ...
                target = RelationshipNavigationVisitor.FindNavigationExpression(
                    target,
                    _aliasGenerator,
                    out navInfo
                );
            ...
            // If a relationship navigation expression defines this collection, make the Ref that is the
            // navigation source
            // and the source association end available for possible use when the projection over the collection
            // is rewritten.
            if (navInfo != null)
            {
            ...
            else
            {
                ...
                // navigation
                // cannot be optimized for the entities produced by this collection expression (if it is a
            ...
            {
                // Create a Lambda function that binds the original navigation source expression under the variable
                // name
                // used in the navigation expression and the relationship span columns, and which has its Lambda
                // body
                // defined by the rewritten collection expression.
                List<DbVariableReferenceExpression> formals =
                    new List<DbVariableReferenceExpression>(1);
                formals.Add(navInfo.SourceVariable);

                List<DbExpression> args = new List<DbExpression>(1);
                args.Add(navInfo.Source);

                result = DbExpressionBuilder.Invoke(
                    DbExpressionBuilder.Lambda(result, formals),
                    args
                );
            }
        ...
        /// most one in each direction and the type of each end is Ref to the same Entity type, or a
        // supertype).
        /// </summary>
        /// <param name="entityType">The Entity type for which the applicable { from, to } end pairings
        // should be retrieved.</param>
        /// <returns>
        ///     A List of association end members pairings that describes the available { from, to }
        // navigations
        ///     for the specified Entity type that are valid for Relationship Span; or <c>null</c> if no
        // such pairings exist.
        /// </returns>
        private List<
            KeyValuePair<AssociationEndMember, AssociationEndMember>
        > GetRelationshipSpanEnds(EntityType entityType)
        {
                ...
                )
                {
                    ...
                    if (2 == association.AssociationEndMembers.Count)
                    {
                        ...
                        if (IsValidRelationshipSpan(entityType, association, end0, end1))
                        {
                            ...
                            retList.Add(
                                new KeyValuePair<AssociationEndMember, AssociationEndMember>(
                                    end0,
                                    end1
                                )
                            );
        ...
        /// <summary>
        /// Determines whether the specified { from, to } relationship end pairing represents a navigation
        // that is
        /// valid for a relationship span sourced by an instance of the specified entity type.
        /// </summary>
        /// <param name="compareType">The Entity type which valid 'from' ends must reference (or a supertype
        // of that Entity type)</param>
        /// <param name="associationType">The Association type to consider.</param>
        /// <param name="fromEnd">The candidate 'from' end, which will be checked based on the Entity type
        // it references</param>
        /// <param name="toEnd">The candidate 'to' end, which will be checked base on the upper bound of its
        // multiplicity</param>
        /// <returns>
        ///     <c>True</c> if the end pairing represents a valid navigation from an instance of the
        ...
        )
        {
            ...
            //     |_A3_2  (*<-->1) A3_2Owner
            //
            // An instance of 'A' would need ALL the 'AOwner', 'BOwner', 'A1Owner', 'A3_1Owner' and 'A3_2Owner'
            // ends
            // spanned in because an instance of 'A' could actually be an instance of A, B, A1, A2, A3_1 or
            // A3_2.
            ...
            //     (this follows from the fact that an instance of 'A' may be an instance of any of its derived
            // types.
            //      Navigation for a subtype relationship will return null if the Entity instance navigation
            // source
            //      is not actually of the required subtype).
            //
            if (
                !associationType.IsForeignKey
                && (
            ...
            )
            {
                ...
                return (
                    ObjectSpanRewriter.EntityTypeEquals(compareType, fromEntityType)
        ...
        private class NavigationInfo
        {
            ...
            )
            {
                ...
                this._sourceRef = (DbVariableReferenceExpression)
                    rewrittenNavigation.NavigationSource;
        ...
        private class RelationshipNavigationVisitor : DefaultExpressionVisitor
        {
            ...
            {
                Debug.Assert(
                    TypeSemantics.IsCollectionType(expression.ResultType),
                ...
                {
                    Debug.Assert(
                        visitor._original != null && visitor._rewritten != null,
                        "Expression was rewritten but no navigation was found?"
                    );
            ...
            public override DbExpression Visit(DbRelationshipNavigationExpression expression)
            {
                ...
                DbVariableReferenceExpression sourceRef = new DbVariableReferenceExpression(
                    expression.NavigationSource.ResultType,
                    varName
                );

                this._rewritten = sourceRef.Navigate(
