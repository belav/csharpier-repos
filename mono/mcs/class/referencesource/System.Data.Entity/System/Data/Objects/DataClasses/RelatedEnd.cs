...
namespace System.Data.Objects.DataClasses
{
    ...
    public abstract class RelatedEnd : IRelatedEnd
    {
        ...
        internal bool IsForeignKey
        {
            get
            {
                Debug.Assert(
                    ...
                    "the IsForeignKey property shouldn't be used in detached scenarios"
                );
        ...
        {
            get { return this._relationMetadata; }
        }

        internal RelationshipEndMember ToEndMember
        {
            get { return this._toEndProperty; }
        }

        internal bool UsingNoTracking
        {
            get { return this._usingNoTracking; }
        }
        ...
        {
            get { return this._fromEndProperty; }
        }
        ...
        )
        {
            ...
            if (
                ...
                && (
                    !IsForeignKey || !IsDependentEndOfReferentialConstraint(checkIdentifying: false)
                )
            )
            {
                throw EntityUtil.InvalidEntityStateSource();
            ...
            Debug.Assert(
                ...
                "If we are attached to a context, we should have a relationship set."
            );
            ...
            {
                Debug.Assert(
                    _relationshipSet.BuiltInTypeKind == BuiltInTypeKind.AssociationSet,
                    "Non-AssociationSet Relationship Set?"
                );
                ...
                if (
                    !targetEntitySet.ElementType.EdmEquals(targetEntityType)
                ...
                )
                {
                    ...
                    TypeUsage targetOSpaceTypeUsage =
                        ObjectContext.MetadataWorkspace.GetOSpaceTypeUsage(
                            TypeUsage.Create(targetEntityType)
                        );
                ...
                if (associationMetadata.IsForeignKey)
                {
                    ...
                    Debug.Assert(
                        ...
                        "Mismatched foreign key properties?"
                    );
                ...
                else
                {
                    ...
                    sourceBuilder = new StringBuilder(
                        "SELECT VALUE [TargetEntity] FROM (SELECT VALUE x FROM "
                    );
                    sourceBuilder.Append("[");
            ...
            IEnumerable<EdmMember> parameterMembers =
            ...
            foreach (EdmMember parameterMember in parameterMembers)
            {
                ...
                if (null == _sourceQueryParamProperties)
                {
                    ...
                    value = _wrappedOwner
                        .EntityKey.EntityKeyValues.Single(ekv => ekv.Key == parameterMember.Name)
                        .Value;
                }
                else
                {
                ...
                if (null == value)
                {
                    ...
                    Debug.Assert(
                        Helper.IsScalarType(parameterEdmType),
                        "Only primitive or enum type expected for parameters"
                    );
        ...
        private object GetCurrentValueFromEntity(EdmMember member)
        {
            ...
            StateManagerTypeMetadata metaType =
                _context.ObjectStateManager.GetOrAddStateManagerTypeMetadata(member.DeclaringType);
        ...
        {
            ObjectQuery<TEntity> sourceQuery = CreateSourceQuery<TEntity>(
                mergeOption,
                out hasResults
            );
            if (null == sourceQuery)
        ...
        )
        {
            ...
            if (refreshedCollection == null)
            {
                ...
                foreach (TEntity entity in collection)
                {
                    ...
                    {
                        EntityWrapperFactory.UpdateNoTrackingWrapper(
                            wrapper,
                            ObjectContext,
                            targetEntitySet
                        );
            ...
            try
            {
                ...
                if (constraint != null)
                {
                    ...
                    if (IsDependentEndOfReferentialConstraint(checkIdentifying: false))
                    {
                        Debug.Assert(
                            collection.Count == 1,
                            "Dependant should attach to single principal"
                        );
                        if (
                            !VerifyRIConstraintsWithRelatedEntry(
                                constraint,
                                ownerEntry.GetCurrentEntityValue,
                                collection[0].ObjectStateEntry.EntityKey
                            )
                        )
                        {
                    ...
                    else
                    {
                        foreach (IEntityWrapper wrappedTarget in collection)
                        {
                            ...
                            if (
                                targetRelatedEnd.IsDependentEndOfReferentialConstraint(
                                    checkIdentifying: false
                                )
                            )
                            {
                                EntityEntry targetEntry = stateManager.FindEntityEntry(
                                    ((EntityReference)targetRelatedEnd).WrappedOwner.Entity
                                );
                                Debug.Assert(
                                    targetEntry != null,
                                    "Both entities should be attached."
                                );
                                if (
                                    !VerifyRIConstraintsWithRelatedEntry(
                                        constraint,
                                        targetEntry.GetCurrentEntityValue,
                                        ownerEntry.EntityKey
                                    )
                                )
                                {
                                    throw EntityUtil.InconsistentReferentialConstraintProperties();
        ...
        )
        {
            ...
            EntityEntry stateEntry = ObjectContext.ObjectStateManager.FindEntityEntry(
                wrappedEntity.Entity
            );
            if (
                null == stateEntry
                || !Object.ReferenceEquals(stateEntry.Entity, wrappedEntity.Entity)
            )
            {
                if (allowCollection)
                {
        ...
        internal void CheckRelationEntitySet(EntitySet set)
        {
            ...
            if (
                (((AssociationSet)_relationshipSet).AssociationSetEnds[_navigation.To] != null)
                && (
                    ((AssociationSet)_relationshipSet).AssociationSetEnds[_navigation.To].EntitySet
                    != set
                )
            )
            {
                throw EntityUtil.EntitySetIsNotValidForRelationship(
                    ...
                    set.Name,
                    _navigation.To,
                    _relationshipSet.EntityContainer.Name,
                    _relationshipSet.Name
                );
            }
        }
        ...
        {
            Debug.Assert(wrappedEntity != null, "IEntityWrapper instance is null.");
            EntityEntry entry = this.ObjectContext.ObjectStateManager.FindEntityEntry(
                wrappedEntity.Entity
            );
            if (entry != null && entry.State == EntityState.Deleted)
            {
        ...
        )
        {
            ...
            if (
                Object.ReferenceEquals(this.ObjectContext, targetRelatedEnd.ObjectContext)
                && this.ObjectContext != null
            )
            {
                ...
                if (UsingNoTracking != targetRelatedEnd.UsingNoTracking)
                {
                    throw EntityUtil.CannotCreateRelationshipBetweenTrackedAndNoTrackedEntities(
                        UsingNoTracking ? this._navigation.From : this._navigation.To
                    );
                }
            }
            ...
            )
            {
                ...
                if (!this.IsForeignKey && !relationshipAlreadyExists && !UsingNoTracking)
                {
                    ...
                    if (
                        ...
                        && !this.WrappedOwner.EntityKey.IsTemporary
                        && IsDependentEndOfReferentialConstraint(false)
                    )
                    {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            ...
                            )
                            {
                                ...
                                if (
                                    !attachedRelatedEnd.WrappedOwner.Context.ObjectStateManager.TransactionManager.WrappedEntities.ContainsKey(
                                        entityToAdd.Entity
                                    )
                                )
                                {
                            ...
                            if (
                                entityToAdd.RequiresRelationshipChangeTracking
                                && TargetAccessor.HasProperty
                            )
                            {
                                Debug.Assert(
                                    this.CheckIfNavigationPropertyContainsEntity(wrappedTarget),
                        ...
                        finally
                        {
                            if (doCleanup)
                            {
                                ...
                                attachedRelatedEnd.FixupOtherEndOfRelationshipForRemove(
                                    entityToAdd, /*preserveForeignKey*/
                                    false
                                );

                                // Remove the target entity from the source related end
                                attachedRelatedEnd.RemoveFromCache(
                                    entityToAdd, /*resetIsLoaded*/
        ...
        )
        {
            ...
            )
            {
                ...
                if (
                    wrappedEntity.RequiresRelationshipChangeTracking
                ...
                )
                {
                    ...
                    if (doAttach)
                    {
                        EntityEntry entry = _context.ObjectStateManager.GetEntityEntry(
                            wrappedEntity.Entity
                        );
                        wrappedEntity.RelationshipManager.CheckReferentialConstraintProperties(
                            entry
                        );
                    }
                }
            }
            WalkObjectGraphToIncludeAllRelatedEntities(
                wrappedEntity,
                addRelationshipAsUnchanged,
                doAttach
            );
        ...
        )
        {
            if (
                ...
                this.TargetAccessor.HasProperty
            ...
            if (
                ...
                && IsDependentEndOfReferentialConstraint(false)
            ) // don't check the nullability of the "from" properties
            {
            ...
            if (!UsingNoTracking && this.ObjectContext != null && !this.IsForeignKey)
            {
                MarkRelationshipAsDeletedInObjectStateManager(
                    wrappedEntity,
                    _wrappedOwner,
                    _relationshipSet,
                    _navigation
            ...
            if (doFixup)
            {
                ...
                )
                {
                    ...
                    if (
                        ...
                        && !ReferenceEquals(
                            _context.ObjectStateManager.EntityInvokingFKSetter,
                            wrappedEntity.Entity
                        )
                    )
                    {
                    ...
                    if (constraint.ToRole == this.FromEndProperty)
                    {
                        if (checkIdentifying)
                        {
                            ...
                            bool allPropertiesAreKeyProperties =
                                RelatedEnd.CheckIfAllPropertiesAreKeyProperties(
                                    entityType.KeyMemberNames,
        ...
        )
        {
            ...
            )
            {
                ...
                if (otherEndAsRef != null)
                {
                    if (otherEndAsRef.NavigationPropertyIsNullOrMissing())
                    {
                        ...
                        Debug.Assert(
                            ObjectContext != null,
                            "Expected attached context at this point."
                        );
                        if (
                            cacheEntry != null
                        ...
                        !otherEndAsRef.CheckIfNavigationPropertyContainsEntity(this._wrappedOwner)
                    )
                    {
            ...
            if (null == cacheEntry)
            {
                ...
                AddGraphToObjectStateManager(
                    wrappedEntity, /*relationshipAlreadyExists*/
                    false,
                    addRelationshipAsUnchanged,
                    doAttach
                );
            ...
            else if (null == FindRelationshipEntryInObjectStateManager(wrappedEntity))
            {
                ...
                if (this.ObjectContext != null && wrappedEntity.Context != null)
                {
                    ...
                    )
                    {
                        ...
                        {
                            EntityEntry entry =
                                this.ObjectContext.ObjectStateManager.GetEntityEntry(
                                    wrappedEntity.Entity
                                );
                            wrappedEntity.RelationshipManager.CheckReferentialConstraintProperties(
                                entry
        ...
        internal void ExcludeEntity(IEntityWrapper wrappedEntity)
        {
            ...
            if (
                !_context.ObjectStateManager.TransactionManager.TrackProcessedEntities
                || !(
                ...
                || _context.ObjectStateManager.TransactionManager.ProcessedEntities.Contains(
                    wrappedEntity
                )
            )
            {
                ...
                )
                {
                    ...
                    if (!this.IsForeignKey)
                    {
                        RemoveRelationshipFromObjectStateManager(
                            wrappedEntity,
                            _wrappedOwner,
                            _relationshipSet,
                            _navigation
                            ...
                            foreach (IEntityWrapper wrappedRelatedEntity in GetWrappedEntities())
                            {
                                ...
                                )
                                {
                                    if (
                                        !VerifyRIConstraintsWithRelatedEntry(
                                            constraint,
                                            dependent.GetCurrentEntityValue,
                                            ownerEntry.EntityKey
                                        )
                                    )
                                    {
        ...
        private EntityKey ExtractPrincipalKey(IEntityWrapper wrappedRelatedEntity)
        {
            ...
            {
                principalKey = _context.ObjectStateManager.CreateEntityKey(
                    principalEntitySet,
                    wrappedRelatedEntity.Entity
                );
            }
            return principalKey;
        }
        ...
        )
        {
            ...
            for (int i = 0; i < constraint.FromProperties.Count; ++i)
            {
                ...
                Debug.Assert(
                    currentValue != null,
                    "currentValue is part of Key on an attached entity, it must not be null"
                );
                ...
                if (fireEvent = (null != deletedEntities) && (deletedEntities.Count > 0))
                {
                    foreach (IEntityWrapper wrappedEntity in deletedEntities)
                    {
                        Remove(
                            ...
                            true, /*deleteEntity*/
                            false, /*deleteOwner*/
                            true, /*applyReferentialConstraints*/
        ...
        internal void DetachAll(EntityState ownerEntityState)
        {
            ...
            foreach (IEntityWrapper wrappedEntity in deletedEntities)
            {
                ...
                {
                    DetachRelationshipFromObjectStateManager(
                        wrappedEntity,
                        _wrappedOwner,
                        _relationshipSet,
                        _navigation
        ...
        )
        {
            ...
            Debug.Assert(
                this._context != null && wrappedEntity.Context != null,
                "should be called only if both entities are attached"
            );
            Debug.Assert(
                this._context == wrappedEntity.Context,
        ...
        internal static void RemoveEntityFromObjectStateManager(IEntityWrapper wrappedEntity)
        {
            ...
            if (
                ...
                && wrappedEntity.Context.ObjectStateManager.TransactionManager.PromotedKeyEntries.TryGetValue(
                    wrappedEntity.Entity,
                    out entry
                )
            )
        ...
        )
        {
            ...
            Debug.Assert(
                relationshipSet == null
                    || !(relationshipSet.ElementType as AssociationType).IsForeignKey,
                "for IsForeignKey relationships ObjectStateEntries don't exist"
            );

            RelationshipEntry deletedEntry = MarkRelationshipAsDeletedInObjectStateManager(
                wrappedEntity,
                wrappedOwner,
                relationshipSet,
                navigation
        ...
        )
        {
            ...
            {
                entry = wrappedEntity.Context.ObjectStateManager.FindEntityEntry(
                    wrappedEntity.Entity
                );
        ...
        )
        {
            ...
            )
            {
                EntityKey ownerKey = wrappedOwner.EntityKey;
                EntityKey entityKey = wrappedEntity.EntityKey;
                RelationshipEntry entry = wrappedEntity.Context.ObjectStateManager.FindRelationship(
                    relationshipSet,
                    new KeyValuePair<string, EntityKey>(navigation.From, ownerKey),
                    new KeyValuePair<string, EntityKey>(navigation.To, entityKey)
                );
        ...
        )
        {
            ...
            foreach (RelatedEnd relatedEnd in wrappedEntity1.RelationshipManager.Relationships)
            {
                ...
                doCascadeDelete =
                    CheckCascadeDeleteFlag(relatedEnd.FromEndProperty)
        ...
        )
        {
            ...
            try
            {
                        ...
                        if (associationset != null)
                        {
                            if (
                                ...
                                throw EntityUtil.EntitySetIsNotValidForRelationship(
                                    ...
                                    entitySet.Name,
                                    _navigation.From,
                                    ((AssociationSet)set).EntityContainer.Name,
                                    ((AssociationSet)set).Name
                                );
                        }
                    }
                ...
                ) //Only Association relationship is supported
                {
                    ...
                    {
                        Debug.Assert(
                            !foundFromRelationEnd,
                            "More than one related end was found with the same role name."
                        );
                ...
                if (this.IsEmpty())
                {
                    ...
                    if (entityRef != null && entityRef.DetachedEntityKey != null)
                    {
                        ...
                        EntitySet targetEntitySet = detachedKey.GetEntitySet(
        ...
        )
        {
            ...
            Debug.Assert(
                context.MetadataWorkspace != null,
                "The context should not have a null metadata workspace."
            );
        ...
        internal void DetachContext()
        {
            if (
                this._context != null
                && this.ObjectContext.ObjectStateManager.TransactionManager.IsAttachTracking
                && this.ObjectContext.ObjectStateManager.TransactionManager.OriginalMergeOption
                    == MergeOption.NoTracking
            )
            {
        ...
        internal RelatedEnd GetOtherEndOfRelationship(IEntityWrapper wrappedEntity)
        {
            ...
            EnsureRelationshipNavigationAccessorsInitialized();
            return (RelatedEnd)
                wrappedEntity.RelationshipManager.GetRelatedEnd(
                    _navigation.Reverse,
                    _relationshipFixer
                );
        }
...
#pragma warning restore 612
        ...
        internal NavigationProperty NavigationProperty
        {
            ...
            {
                if (
                    this.navigationPropertyCache == null
                ...
                )
                {
                    ...
                    EntityType entityType =
                        _wrappedOwner.Context.MetadataWorkspace.GetItem<EntityType>(
                            _wrappedOwner.IdentityType.FullName,
                            DataSpace.OSpace
                        );
                    NavigationProperty member;
                    if (
                        !entityType.NavigationProperties.TryGetValue(
        ...
        private void EnsureRelationshipNavigationAccessorsInitialized()
        {
            ...
            if (!RelationshipNavigation.IsInitialized)
            {
                ...
                )
                {
                    ...
                    if (
                        associationType.AssociationEndMembers.TryGetValue(
                            sourceRoleName,
                            false,
                            out sourceEnd
                        )
