...
namespace System.Data.EntityModel.SchemaObjectModel
{
    ...
    internal class TypeUsageBuilder
    {
        ...
        {
            bool noErrors = true;
            Dictionary<string, Facet> defaultFacets = edmType
                .GetAssociatedFacetDescriptions()
                .ToDictionary(f => f.FacetName, f => f.DefaultValueFacet);
            ...
            foreach (Facet defaultFacet in defaultFacets.Values)
            {
                ...
                if (_facetValues.TryGetValue(defaultFacet.Name, out value))
                {
                    ...
                    {
                        calculatedFacets.Add(
                            defaultFacet.Name,
                            Facet.Create(defaultFacet.Description, value)
                        );
                    }
            ...
            foreach (KeyValuePair<string, object> value in _facetValues)
            {
                ...
                else if (value.Key == EdmProviderManifest.ConcurrencyModeFacetName)
                {
                    ...
                    && (edmType as PrimitiveType).PrimitiveTypeKind == PrimitiveTypeKind.String
        ...
        internal void ValidateEnumFacets(SchemaEnumType schemaEnumType)
        {
            ...
            {
                if (
                    value.Key != EdmProviderManifest.NullableFacetName
        ...
        private bool InternalHandleAttribute(XmlReader reader)
        {
            ...
            else if (SchemaElement.CanHandleAttribute(reader, XmlConstants.DefaultValueAttribute))
            {
                ...
                SchemaElement.CanHandleAttribute(reader, EdmProviderManifest.PrecisionFacetName)
            )
            {
            ...
            else if (SchemaElement.CanHandleAttribute(reader, EdmProviderManifest.ScaleFacetName))
            {
                ...
                SchemaElement.CanHandleAttribute(
                    reader,
            ...
            )
            {
                ...
                SchemaElement.CanHandleAttribute(reader, EdmProviderManifest.MaxLengthFacetName)
            )
            {
            ...
            else if (SchemaElement.CanHandleAttribute(reader, EdmProviderManifest.UnicodeFacetName))
            {
                ...
                SchemaElement.CanHandleAttribute(reader, EdmProviderManifest.CollationFacetName)
            )
            {
                ...
                SchemaElement.CanHandleAttribute(reader, EdmProviderManifest.FixedLengthFacetName)
            )
            {
                ...
                SchemaElement.CanHandleAttribute(reader, EdmProviderManifest.NullableFacetName)
            )
            {
                HandleNullableAttribute(reader);
                return true;
        ...
        private void ValidateAndSetDecimalFacets(EdmType type, Dictionary<string, Facet> facets)
        {
            ...
            if (
                facets.TryGetValue(EdmProviderManifest.PrecisionFacetName, out precisionFacet)
            ...
            )
            {
                ...
                if (
                    precision < precisionFacetDescription.MinValue.Value
            ...
            Facet scaleFacet;
            if (
                facets.TryGetValue(EdmProviderManifest.ScaleFacetName, out scaleFacet)
            ...
            )
            {
                ...
                if (
                    scale < scaleFacetDescription.MinValue.Value
                ...
                else if (precision.HasValue)
                {
                    if (precision < scale)
                    {
                        _element.AddError(
                            ...
                            EdmSchemaErrorSeverity.Error,
                            System.Data.Entity.Strings.BadPrecisionAndScale(precision, scale)
                        );
                    }
                }
            }
        ...
        private void ValidateLengthFacets(EdmType type, Dictionary<string, Facet> facets)
        {
            ...
            if (
                !facets.TryGetValue(EdmProviderManifest.MaxLengthFacetName, out maxLenFacet)
        ...
        private void ValidateSpatialFacets(EdmType type, Dictionary<string, Facet> facets)
        {
            ...
            Facet isStrictFacet;
            if (
                this._element.Schema.DataModel == SchemaDataModelOption.EntityDataModel
                && (
                    !facets.TryGetValue(EdmProviderManifest.IsStrictFacetName, out isStrictFacet)
                    || (bool)isStrictFacet.Value != false
                )
            )
            {
                _element.AddError(
                    ErrorCode.UnexpectedSpatialType,
                    EdmSchemaErrorSeverity.Error,
                    System.Data.Entity.Strings.SpatialWithUseStrongSpatialTypesFalse
                );
            }
            ...
            //here we are assuming if the facet should be here or not is already get checked before
            if (
                !facets.TryGetValue(EdmProviderManifest.SridFacetName, out sridFacet)
            ...
            FacetDescription facetDescription = Helper.GetFacet(
                primitiveType.FacetDescriptions,
                EdmProviderManifest.SridFacetName
            );
        ...
        internal void ValidateDefaultValue(SchemaType type)
        {
            ...
            else
            {
                _element.AddError(
                    ...
                    EdmSchemaErrorSeverity.Error,
                    System.Data.Entity.Strings.DefaultNotAllowed
                );
            }
        }
        ...
        private void ValidateBooleanDefaultValue(ScalarType scalar)
        {
            if (!scalar.TryParse(_default, out _defaultObject))
                _element.AddError(
                    ...
                    EdmSchemaErrorSeverity.Error,
                    System.Data.Entity.Strings.InvalidDefaultBoolean(_default)
                );
        }
        ...
        private void ValidateIntegralDefaultValue(ScalarType scalar, long minValue, long maxValue)
        {
            if (!scalar.TryParse(_default, out _defaultObject))
                _element.AddError(
                    ErrorCode.InvalidDefault,
                    EdmSchemaErrorSeverity.Error,
                    System.Data.Entity.Strings.InvalidDefaultIntegral(_default, minValue, maxValue)
                );
        }
        ...
        private void ValidateDateTimeDefaultValue(ScalarType scalar)
        {
            if (!scalar.TryParse(_default, out _defaultObject))
            {
                _element.AddError(
                    ErrorCode.InvalidDefault,
                    EdmSchemaErrorSeverity.Error,
                    Strings.InvalidDefaultDateTime(
                        _default,
        ...
        private void ValidateTimeDefaultValue(ScalarType scalar)
        {
            if (!scalar.TryParse(_default, out _defaultObject))
            {
                _element.AddError(
                    ErrorCode.InvalidDefault,
                    EdmSchemaErrorSeverity.Error,
                    Strings.InvalidDefaultTime(_default, ScalarType.TimeFormat.Replace(@"\", ""))
                );
            }
        }
        ...
        private void ValidateDecimalDefaultValue(ScalarType scalar)
        {
            ...
            _element.AddError(
                ErrorCode.InvalidDefault,
                EdmSchemaErrorSeverity.Error,
                System.Data.Entity.Strings.InvalidDefaultDecimal(_default, 38, 38)
            );
        }
        ...
        private void ValidateGuidDefaultValue(ScalarType scalar)
        {
            if (!scalar.TryParse(_default, out _defaultObject))
                _element.AddError(
                    ErrorCode.InvalidDefault,
                    EdmSchemaErrorSeverity.Error,
                    System.Data.Entity.Strings.InvalidDefaultGuid(_default)
                );
        }
        #endregion
        #endregion
