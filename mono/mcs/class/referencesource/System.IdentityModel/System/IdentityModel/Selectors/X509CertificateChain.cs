...
#pragma warning suppress 56503
        ...
        [Fx.Tag.SecurityNote(
            Critical = "Builds chain trust through interop calls.",
        ...
        public bool Build(X509Certificate2 certificate)
        {
            ...
            {
                if (
                    certificate.Extensions[CAPI.szOID_CRL_DIST_POINTS] != null
            ...
            BuildChain(
                this.useMachineContext
                    ? new IntPtr(CAPI.HCCE_LOCAL_MACHINE)
                    : new IntPtr(CAPI.HCCE_CURRENT_USER),
                certificate.Handle,
            ...
            CAPI.CERT_CHAIN_POLICY_PARA PolicyPara = new CAPI.CERT_CHAIN_POLICY_PARA(
                Marshal.SizeOf(typeof(CAPI.CERT_CHAIN_POLICY_PARA))
            );
            CAPI.CERT_CHAIN_POLICY_STATUS PolicyStatus = new CAPI.CERT_CHAIN_POLICY_STATUS(
                Marshal.SizeOf(typeof(CAPI.CERT_CHAIN_POLICY_STATUS))
            );

            // Ignore peertrust.  Peer trust caused the chain to succeed out-of-the-box in Vista.
            // This new flag is only available in Vista.
            PolicyPara.dwFlags =
                (uint)chainPolicy.VerificationFlags | CAPI.CERT_CHAIN_POLICY_IGNORE_PEER_TRUST_FLAG;

            if (
                !CAPI.CertVerifyCertificateChainPolicy(
                    new IntPtr(this.chainPolicyOID),
                    safeCertChainHandle,
                    ref PolicyPara,
                    ref PolicyStatus
                )
            ...
            try
            {
                if (applicationPolicy != null && applicationPolicy.Count > 0)
                {
                    ...
                    ChainPara.RequestedUsage.Usage.rgpszUsageIdentifier =
                        applicationPolicyHandle.DangerousGetHandle();
                }
                ...
                if (certificatePolicy != null && certificatePolicy.Count > 0)
                {
                    ...
                    ChainPara.RequestedIssuancePolicy.Usage.cUsageIdentifier = (uint)
                    ...
                    ChainPara.RequestedIssuancePolicy.Usage.rgpszUsageIdentifier =
                        certificatePolicyHandle.DangerousGetHandle();
                }
        ...
        [Fx.Tag.SecurityNote(
            Critical = "Uses unmanaged code to create an in memory store which links to the original cert store."
                + "User must protect the store handle."
        )]
        [SecurityCritical]
        static SafeCertStoreHandle ExportToMemoryStore(
            X509Certificate2Collection collection,
            IntPtr pCertContext
        )
        {
            CAPI.CERT_CONTEXT certContext = (CAPI.CERT_CONTEXT)
                Marshal.PtrToStructure(pCertContext, typeof(CAPI.CERT_CONTEXT));

            // No extra store nor intermediate certificates
            if (
                (collection == null || collection.Count <= 0)
                && certContext.hCertStore == IntPtr.Zero
            )
            {
            ...
            if (collection != null && collection.Count > 0)
            {
                ...
                {
                    if (
                        !CAPI.CertAddCertificateLinkToStore(
                            certStoreHandle,
                            x509.Handle,
            ...
            //
            using (
                SafeCertContextHandle safeCertContext = CAPI.CertCreateCertificateContext(
            ...
            )
            {
                ...
                //
                X509Certificate2 intermediatesCert = new X509Certificate2(
                ...
                // object.
                //
                CAPI.CERT_CONTEXT intermediatesCertContext = (CAPI.CERT_CONTEXT)
                    Marshal.PtrToStructure(intermediatesCert.Handle, typeof(CAPI.CERT_CONTEXT));
        ...
        [Fx.Tag.SecurityNote(
            Critical = "Copies the oid collection to unamanged memory."
