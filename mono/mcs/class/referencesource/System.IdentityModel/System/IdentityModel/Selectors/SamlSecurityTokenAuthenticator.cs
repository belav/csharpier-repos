...
namespace System.IdentityModel.Selectors
{
    ...
    public class SamlSecurityTokenAuthenticator : SecurityTokenAuthenticator
    {
        ...
        )
        {
            ...
            {
                canBeValidated = this.supportingAuthenticators[i]
                    .CanValidateToken(samlToken.Assertion.SigningToken);
                if (canBeValidated)
                    break;
            }
            if (!canBeValidated)
            ...
            if (
                ...
                || (this.audienceUriMode == AudienceUriMode.BearerKeyOnly)
                    && (samlToken.SecurityKeys.Count < 1)
            )
            {
                // throws if not found.
                ...
                for (int i = 0; i < samlToken.Assertion.Conditions.Conditions.Count; i++)
                {
                    SamlAudienceRestrictionCondition audienceCondition =
                        samlToken.Assertion.Conditions.Conditions[i]
        ...
        protected virtual bool ValidateAudienceRestriction(
            ...
            for (int i = 0; i < audienceRestrictionCondition.Audiences.Count; i++)
            {
                ...
                for (int j = 0; j < this.allowedAudienceUris.Count; j++)
                {
                    if (
                        ...
                        Uri.IsWellFormedUriString(this.allowedAudienceUris[j], UriKind.Absolute)
                    )
                    {
        ...
        public virtual ClaimSet ResolveClaimSet(SecurityKeyIdentifier keyIdentifier)
        {
            ...
            if (keyIdentifier.TryFind<RsaKeyIdentifierClause>(out rsaKeyIdentifierClause))
            {
                ...
                keyIdentifier.TryFind<EncryptedKeyIdentifierClause>(
                    out encryptedKeyIdentifierClause
                )
            )
            {
                return new DefaultClaimSet(
        ...
        public virtual IIdentity ResolveIdentity(SecurityKeyIdentifier keyIdentifier)
        {
            ...
            if (keyIdentifier.TryFind<RsaKeyIdentifierClause>(out rsaKeyIdentifierClause))
            {
                return SecurityUtils.CreateIdentity(
                    rsaKeyIdentifierClause.Rsa.ToXmlString(false),
                    this.GetType().Name
                );
            }
        ...
        bool IsCurrentlyTimeEffective(SamlSecurityToken token)
        {
            ...
            {
                return SecurityUtils.IsCurrentlyTimeEffective(
                    token.Assertion.Conditions.NotBefore,
                    token.Assertion.Conditions.NotOnOrAfter,
                    this.maxClockSkew
                );
            }
