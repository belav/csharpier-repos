...
namespace System.IdentityModel.Tokens
{
    ...
    public class SamlSecurityTokenRequirement
    {
        ...
        static X509CertificateValidationMode DefaultValidationMode =
        ...
        public SamlSecurityTokenRequirement(XmlElement element)
        {
            ...
            foreach (XmlAttribute attribute in element.Attributes)
            {
                ...
                )
                {
                    ...
                    {
                        throw DiagnosticUtility.ThrowHelperInvalidOperation(
                            SR.GetString(SR.ID7022, attribute.Value)
                        );
                    }
            ...
            if (certificateValidationMode == X509CertificateValidationMode.Custom)
            {
                ...
                _certificateValidator = CustomTypeElement.Resolve<X509CertificateValidator>(
                    new CustomTypeElement(customValidatorType)
                );
            }
            else if (foundCustomX509Validator)
            {
                _certificateValidator = X509Util.CreateCertificateValidator(
            ...
            foreach (Uri audience in tokenAudiences)
            {
                if (audience != null)
                {
                    ...
                    else
                    {
                        ...
                        audienceLeftPart = baseUri.MakeRelativeUri(
                            new Uri(resolved.GetLeftPart(UriPartial.Path))
                        );
                    }
