...
#pragma warning restore 1591
        ...
        public MruSessionSecurityTokenCache(
            int maximumSize,
            IEqualityComparer<SessionSecurityTokenCacheKey> comparer
        )
        ...
        )
        {
            ...
            // null comparer is ok
            this.items = new Dictionary<SessionSecurityTokenCacheKey, CacheEntry>(
                maximumSize,
                comparer
            );
        ...
        public override SessionSecurityToken Get(SessionSecurityTokenCacheKey key)
        {
            ...
            lock (this.syncRoot)
            {
                ...
                if (
                    this.mruEntry.Node != null
                    && key != null
                    && key.Equals(this.mruEntry.Node.Value)
                )
                {
        ...
        public override void RemoveAll(string endpointId, System.Xml.UniqueId contextId)
        {
            ...
            Dictionary<SessionSecurityTokenCacheKey, CacheEntry> entriesToDelete =
                new Dictionary<SessionSecurityTokenCacheKey, CacheEntry>();
            ...
            if (this.items.Count >= this.maximumSize)
            {
                ...
                if (DiagnosticUtility.ShouldTrace(TraceEventType.Information))
                {
                    TraceUtility.TraceString(
                        ...
                        SR.GetString(SR.ID8003, this.maximumSize, this.sizeAfterPurge)
                    );
                }
            }
        }
