...
namespace System.IdentityModel.Tokens
{
    ...
    public class Saml2SecurityTokenHandler : SecurityTokenHandler
    {
        ...
        /// </summary>
        public const string TokenProfile11ValueType =
        ...
        // Saml2SecurityTokenHandler instance when validating tokens.</param>
        ...
        /// <summary>
        /// Gets or sets the <see cref="X509CertificateValidator"/> that is used by the current instance to
        // validate
        ...
        public X509CertificateValidator CertificateValidator
        {
            ...
            set { this.samlSecurityTokenRequirement.CertificateValidator = value; }
        }

        /// <summary>
        /// Gets or sets a <see cref="SecurityTokenSerializer"/> that will be used to serialize and
        // deserialize
        /// a <see cref="SecurityKeyIdentifier"/>. For example, SamlSubject SecurityKeyIdentifier or
        // Signature
        ...
        {
            get { return this.samlSecurityTokenRequirement; }
            set
            {
                if (value == null)
                {
        ...
        /// <summary>
        /// Creates a <see cref="SecurityKeyIdentifierClause"/> to be used as the security token reference
        // when the token is not attached to the message.
        /// </summary>
        ...
        /// <summary>
        /// Creates a <see cref="SecurityToken"/> based on a information contained in the <see
        // cref="SecurityTokenDescriptor"/>.
        /// </summary>
        /// <param name="tokenDescriptor">The <see cref="SecurityTokenDescriptor"/> that has creation
        // information.</param>
        ...
        public override SecurityToken CreateToken(SecurityTokenDescriptor tokenDescriptor)
        {
            ...
            Saml2Assertion assertion = new Saml2Assertion(
                this.CreateIssuerNameIdentifier(tokenDescriptor)
            );
            ...
            // Conditions
            assertion.Conditions = this.CreateConditions(
        ...
        /// <exception cref="ArgumentException">The token is not of assignable from <see
        // cref="Saml2SecurityToken"/>.</exception>
        ...
        // Saml2SecurityToken.Assertion.IssuerToken is null.</exception>
        /// <exception cref="SecurityTokenValidationException">Thrown if
        // Saml2SecurityToken.Assertion.SigningToken is null.</exception>
        ...
        public override ReadOnlyCollection<ClaimsIdentity> ValidateToken(SecurityToken token)
        {
            ...
            try
            {
                ...
                this.ValidateConditions(
                    ...
                    SamlSecurityTokenRequirement.ShouldEnforceAudienceRestriction(
                ...
                Saml2SubjectConfirmation subjectConfirmation = samlToken
                    .Assertion
                    .Subject
        ...
        /// </summary>
        /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
        // cref="SecurityToken"/>.</param>
        /// <param name="token">The <see cref="SecurityToken"/> to serialize.</param>
        /// <exception cref="ArgumentNullException">The input argument 'writer' or 'token' is
        // null.</exception>
        /// <exception cref="ArgumentException">The input argument 'token' is not a <see
        // cref="Saml2SecurityToken"/>.</exception>
        ...
        /// <summary>
        /// Indicates whether the current XML element can be read as a token of the type handled by this
        // instance.
        /// </summary>
        /// <param name="reader">An <see cref="XmlReader"/> reader positioned at a start element. The reader
        // should not be advanced.</param>
        ...
        /// <param name="reader">An <see cref="XmlReader"/> reader positioned at a start element. The reader
        // should not be advanced.</param>
        /// <returns>'True' if the ReadKeyIdentifierClause can read the element. 'False'
        // otherwise.</returns>
        ...
        /// <returns>"True' if the given SecurityKeyIdentifierClause can be serialized. 'False'
        // otherwise.</returns>
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> reader positioned at a <see
        // cref="SecurityKeyIdentifierClause"/> element.</param>
        ...
        public override SecurityKeyIdentifierClause ReadKeyIdentifierClause(XmlReader reader)
        {
            ...
            if (reader.IsEmptyElement)
            {
                throw DiagnosticUtility.ThrowHelperXml(
                    reader,
                    SR.GetString(SR.ID3061, WSSE.Elements.SecurityTokenReference, WSSE.Namespace)
                );
            }
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> reader positioned at a <see
        // cref="Saml2SecurityToken"/> element.</param>
        /// <returns>An instance of <see cref="Saml2SecurityToken"/>.</returns>
        /// <exception cref="InvalidOperationException">Is thrown if 'Configuration',
        // 'Configruation.IssuerTokenResolver' or 'Configuration.ServiceTokenResolver is null.</exception>
        public override SecurityToken ReadToken(XmlReader reader)
        {
            ...
            ReadOnlyCollection<SecurityKey> keys = this.ResolveSecurityKeys(
                assertion,
        ...
        /// </summary>
        /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
        // cref="SecurityKeyIdentifierClause"/>.</param>
        /// <param name="securityKeyIdentifierClause">The <see cref="SecurityKeyIdentifierClause"/> to be
        // serialized.</param>
        /// <exception cref="ArgumentNullException">Input parameter 'writer' or
        // 'securityKeyIdentifierClause' is null.</exception>
        ...
        )
        {
            ...
            WrappedSaml2AssertionKeyIdentifierClause wrappedClause =
                securityKeyIdentifierClause as WrappedSaml2AssertionKeyIdentifierClause;
...
#pragma warning suppress 56504 // bogus - thinks reader.LocalName, reader.NamespaceURI need validation
                throw DiagnosticUtility.ThrowHelperXml(
                    reader,
                    SR.GetString(SR.ID3061, reader.LocalName, reader.NamespaceURI)
                );
            }
            ...
            )
            {
                ...
                EncryptedKeyIdentifierClause encryptedKeyClause =
                    skic as EncryptedKeyIdentifierClause;
            ...
            SymmetricAlgorithm decryptor = symmetricKey.GetSymmetricAlgorithm(
                encryptedData.Algorithm
            );
            byte[] plainText = encryptedData.Decrypt(decryptor);

            // Save off the encrypting credentials for roundtrip
            encryptingCredentials = new ReceivedEncryptingCredentials(
                decryptionKey,
                new SecurityKeyIdentifier(matchingClause),
                encryptedData.Algorithm
            );
        ...
        /// <returns>'True' if reader contains a <see cref="Saml2SecurityKeyIdentifierClause"/>. 'False'
        // otherwise.</returns>
        ...
        internal static Uri ReadSimpleUriElement(
            XmlReader reader,
            UriKind kind,
        ...
        )
        {
            ...
            try
            {
                ...
                if (!allowLaxReading && !UriUtil.CanCreateValidUri(value, kind))
                {
                    throw DiagnosticUtility.ThrowHelperXml(
                        reader,
                        SR.GetString(kind == UriKind.RelativeOrAbsolute ? SR.ID0019 : SR.ID0013)
                    );
                }
        ...
        /// </summary>
        /// <param name="claim">The <see cref="Claim"/> from which to generate a <see
        // cref="Saml2Attribute"/>.</param>
        ...
        /// <summary>
        /// Creates <see cref="Saml2AttributeStatement"/> from a <see cref="SecurityTokenDescriptor"/> and a
        // <see cref="ClaimsIdentity"/>
        /// </summary>
        /// <remarks>This method may return null if the token descriptor does not contain any subject or the
        // subject does not have any claims.
        /// </remarks>
        /// <param name="subject">The <see cref="ClaimsIdentity"/> that contains claims which will be
        // converted to SAML Attributes.</param>
        /// <param name="tokenDescriptor">The <see cref="SecurityTokenDescriptor"/> that contains
        // information on building the <see cref="Saml2AttributeStatement"/>.</param>
        ...
        )
        {
            ...
            if (subject.Claims != null)
            {
                ...
                ICollection<Saml2Attribute> collectedAttributes = this.CollectAttributeValues(
                    attributes
                );
        ...
        /// <returns>A <see cref="ICollection{T}"/> of <see cref="Saml2Attribute"/> with common attributes
        // collected into value lists.</returns>
        protected virtual ICollection<Saml2Attribute> CollectAttributeValues(
            ICollection<Saml2Attribute> attributes
        )
        {
            Dictionary<SamlAttributeKeyComparer.AttributeKey, Saml2Attribute> distinctAttributes =
                new Dictionary<SamlAttributeKeyComparer.AttributeKey, Saml2Attribute>(
                    attributes.Count,
            ...
            foreach (Saml2Attribute saml2Attribute in attributes)
            {
                if (saml2Attribute != null)
                {
                    SamlAttributeKeyComparer.AttributeKey attributeKey =
                        new SamlAttributeKeyComparer.AttributeKey(saml2Attribute);
        ...
        /// </summary>
        /// <param name="subject">The delegate of this <see cref="ClaimsIdentity"/> will be serialized into
        // a <see cref="Saml2Attribute"/>.</param>
        /// <param name="attributes">A <see cref="ICollection{T}"/> of <see cref="Saml2Attribute"/>.</param>
        /// <param name="tokenDescriptor">The <see cref="SecurityTokenDescriptor"/> that contains
        // information on building the delegate.</param>
        ...
        )
        {
            ...
            attributes.Add(
                this.CreateAttribute(
                    new Claim(
                        ClaimTypes.Actor,
                        this.CreateXmlStringFromAttributes(collectedAttributes),
        ...
        /// <remarks>The string is of the form ""&lt;Actor&gt;&lt;Attribute name,
        // ns&gt;&lt;AttributeValue&gt;...&lt;/AttributeValue&gt;,
        // ...&lt;/Attribute&gt;...&lt;/Actor&gt;"</remarks>
        ...
        /// <summary>
        /// Creates an <see cref="IEnumerable{T}"/> of <see cref="Saml2Statement"/> to be included in the
        // assertion.
        /// </summary>
        /// <remarks>
        /// <para>
        ...
        /// </remarks>
        /// <param name="tokenDescriptor">The <see cref="SecurityTokenDescriptor"/> that contains
        // information on creating the <see cref="Saml2Statement"/>.</param>
        ...
        )
        {
            ...
            Saml2AttributeStatement attributeStatement = this.CreateAttributeStatement(
            ...
            Saml2AuthenticationStatement authenticationStatement =
        ...
        /// </summary>
        /// <param name="authInfo">
        /// An AuthenticationInformation object containing the state to be wrapped as a
        // Saml2AuthenticationStatement
        /// object.
        /// </param>
        /// <param name="tokenDescriptor">The token descriptor.</param>
        /// <returns>
        /// The Saml2AuthenticationStatement to add to the assertion being created or null to ignore the
        ...
        )
        {
            ...
            {
                throw DiagnosticUtility.ThrowHelperInvalidOperation(
                    SR.GetString(SR.ID4270, "AuthenticationMethod", "SAML2")
                );
            }
            else if (authenticationInstant == null)
            {
                throw DiagnosticUtility.ThrowHelperInvalidOperation(
                    SR.GetString(SR.ID4270, "AuthenticationInstant", "SAML2")
                );
            }
            ...
            if (
                !UriUtil.TryCreateValidUri(
                    this.DenormalizeAuthenticationType(authenticationMethod),
                    UriKind.Absolute,
                    out saml2AuthenticationClass
                )
            )
            {
                throw DiagnosticUtility.ThrowHelperInvalidOperation(
                    SR.GetString(SR.ID4185, authenticationMethod)
                );
            }

            Saml2AuthenticationContext authCtx = new Saml2AuthenticationContext(
                saml2AuthenticationClass
            );
            DateTime authInstantTime = DateTime
                ...
                .ToUniversalTime();
            Saml2AuthenticationStatement authnStatement = new Saml2AuthenticationStatement(
                authCtx,
                authInstantTime
            );
            ...
            {
                if (
                    !string.IsNullOrEmpty(authInfo.DnsName)
                    || !string.IsNullOrEmpty(authInfo.Address)
                )
                {
                    authnStatement.SubjectLocality = new Saml2SubjectLocality(
                        authInfo.Address,
                        authInfo.DnsName
                    );
                }
        ...
        protected virtual Saml2Subject CreateSamlSubject(SecurityTokenDescriptor tokenDescriptor)
        {
            ...
            if (tokenDescriptor.Subject != null && tokenDescriptor.Subject.Claims != null)
            {
                foreach (Claim claim in tokenDescriptor.Subject.Claims)
                {
                    if (claim.Type == ClaimTypes.NameIdentifier)
                    {
                        ...
                        {
                            nameIdentifierFormat = claim.Properties[
            ...
            if (nameIdentifierClaim != null)
            {
                ...
                if (
                    ...
                    && UriUtil.CanCreateValidUri(nameIdentifierFormat, UriKind.Absolute)
                )
                {
        ...
        /// </summary>
        /// <param name="tokenDescriptor">Retrieve some scope encrypting credentials from the Scope
        // object</param>
        ...
        )
        {
            ...
            if (enforceAudienceRestriction)
            {
                ...
                {
                    foreach (
                        Saml2AudienceRestriction audienceRestriction in conditions.AudienceRestrictions
                    )
                    {
                        SamlSecurityTokenRequirement.ValidateAudienceRestriction(
        ...
        /// <returns>A string that represents the denormalized authentication type used to obtain the
        // token.</returns>
        ...
        /// <exception cref="ArgumentException">The input argument 'token' can not be cast as a
        // 'Saml2SecurityToken'.</exception>
        /// <exception cref="SecurityTokenValidationException">The Saml2SecurityToken.Assertion.Id.Value is
        // null or empty.</exception>
        /// <exception cref="SecurityTokenReplayDetectedException">The token is found in the <see
        ...
        protected override void DetectReplayedToken(SecurityToken token)
        {
            ...
            {
                string issuer =
                    (samlToken.Assertion.Issuer.Value != null)
                        ? samlToken.Assertion.Issuer.Value
                        : String.Empty;
        ...
        /// <exception cref="ArgumentNullException">The input argument 'token' is null.</exception>
        /// <exception cref="SecurityTokenValidationException">The Saml2SecurityToken's validity period is
        ...
        /// This function will never return a value further from now than
        // Configuration.TokenReplayCacheExpirationPeriod.</remarks>
        ...
        /// </summary>
        /// <param name="saml2AuthenticationContextClassReference">A string representing the <see
        // cref="Saml2Constants.AuthenticationContextClasses"/></param>
        ...
        )
        {
            ...
            if (nameId != null)
            {
                ...
                {
                    claim.Properties[ClaimProperties.SamlNameIdentifierFormat] = nameId
                        .Format
                        .AbsoluteUri;
                }
                ...
                {
                    claim.Properties[ClaimProperties.SamlNameIdentifierSPProvidedId] =
                        nameId.SPProvidedId;
                }
                ...
                else
                {
                    foreach (string value in attribute.Values)
                    {
                        if (value != null)
                        {
                            ...
                            {
                                claim.Properties[ClaimProperties.SamlAttributeNameFormat] =
                                    attribute.NameFormat.AbsoluteUri;
                            }

                            if (attribute.FriendlyName != null)
                            {
                                claim.Properties[ClaimProperties.SamlAttributeDisplayName] =
        ...
        /// <summary>
        /// This method gets called when a special type of Saml2Attribute is detected. The Saml2Attribute
        // passed in
        /// wraps a Saml2Attribute that contains a collection of AttributeValues, each of which will get
        // mapped to a
        ...
        /// <param name="subject">The <see cref="ClaimsIdentity"/> that is the subject of this
        // token.</param>
        /// <param name="issuer">The issuer of the claim.</param>
        /// <exception cref="InvalidOperationException">Will be thrown if the Saml2Attribute does not
        // contain any
        ...
        )
        {
            ...
            foreach (string attributeValue in attribute.Values)
            {
                ...
                {
                    using (
                        XmlDictionaryReader dicReader = XmlDictionaryReader.CreateTextReader(
                                ...
                                else
                                {
                                    ...
                                    for (int k = 0; k < innerAttribute.Values.Count; ++k)
                                    {
                                        ...
                                        if (string.IsNullOrEmpty(originalIssuer))
                                        {
                                            claim = new Claim(
                                                innerAttribute.Name,
                                                innerAttribute.Values[k],
                                                innerAttribute.AttributeValueXsiType,
                                                issuer
                                            );
                                        }
                                        else
                                        ...
                                        {
                                            claim.Properties[
                                                ClaimProperties.SamlAttributeNameFormat
                                            ] = innerAttribute.NameFormat.AbsoluteUri;
                                        }

                                        if (innerAttribute.FriendlyName != null)
                                        {
                                            claim.Properties[
                                                ClaimProperties.SamlAttributeDisplayName
                                            ] = innerAttribute.FriendlyName;
                                        }
            ...
            if (statement.AuthenticationContext.ClassReference != null)
            {
                subject.AddClaim(
                    new Claim(
                        ...
                        this.NormalizeAuthenticationContextClassReference(
        ...
        {
            Collection<Saml2AuthenticationStatement> authnStatements =
                new Collection<Saml2AuthenticationStatement>();
            ...
            foreach (Saml2Statement statement in statements)
            {
                ...
                Saml2AuthenticationStatement authnStatement =
                    statement as Saml2AuthenticationStatement;
                ...
                Saml2AuthorizationDecisionStatement authzStatement =
                    statement as Saml2AuthorizationDecisionStatement;
        ...
        protected virtual ClaimsIdentity CreateClaims(Saml2SecurityToken samlToken)
        {
            ...
            ClaimsIdentity subject = new ClaimsIdentity(
                AuthenticationTypes.Federation,
                SamlSecurityTokenRequirement.NameClaimType,
                SamlSecurityTokenRequirement.RoleClaimType
            );
        ...
        protected virtual void ValidateConfirmationData(
        ...
        )
        {
            ...
            if (
                null != confirmationData.NotBefore
                && DateTimeUtil.Add(now, Configuration.MaxClockSkew)
            ...
            if (
                null != confirmationData.NotOnOrAfter
                && DateTimeUtil.Add(now, Configuration.MaxClockSkew.Negate())
        ...
        /// <summary>
        /// Resolves the collection of <see cref="SecurityKey"/> referenced in a <see
        // cref="Saml2Assertion"/>.
        /// </summary>
        /// <param name="assertion"><see cref="Saml2Assertion"/> to process.</param>
        /// <param name="resolver"><see cref="SecurityTokenResolver"/> to use in resolving the <see
        // cref="SecurityKey"/>.</param>
        /// <returns>A read only collection of <see cref="SecurityKey"/> contained in the
        // assertion.</returns>
        ...
        )
        {
            ...
            {
                if (
                    null != subjectConfirmation.SubjectConfirmationData
            ...
            else if (Saml2Constants.ConfirmationMethods.HolderOfKey == subjectConfirmation.Method)
            {
                ...
                foreach (
                    SecurityKeyIdentifier keyIdentifier in subjectConfirmation
                        .SubjectConfirmationData
                        .KeyIdentifiers
                )
                {
                    ...
                    {
                        if (null != resolver && resolver.TryResolveSecurityKey(clause, out key))
                        {
                            holderKeys.Add(key);
                            break;
                        }
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> reader positioned at a <see
        // cref="Saml2NameIdentifier"/> element.</param>
        ...
        protected virtual Saml2NameIdentifier ReadSubjectId(XmlReader reader, string parentElement)
        {
            ...
            if (reader.IsStartElement(Saml2Constants.Elements.NameID, Saml2Constants.Namespace))
            {
                ...
                reader.IsStartElement(Saml2Constants.Elements.EncryptedID, Saml2Constants.Namespace)
            )
            {
                return this.ReadEncryptedId(reader);
            }
            else if (
                reader.IsStartElement(Saml2Constants.Elements.BaseID, Saml2Constants.Namespace)
            )
            {
                ...
                if (
                    ...
                    || XmlUtil.EqualsQName(
                        declaredType,
                        Saml2Constants.Types.BaseIDAbstractType,
                        Saml2Constants.Namespace
                    )
                )
                {
                ...
                if (
                    XmlUtil.EqualsQName(
                        declaredType,
                        Saml2Constants.Types.NameIDType,
                        Saml2Constants.Namespace
                    )
                )
                {
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see cref="Saml2Action"/>
        // element.</param>
        ...
        protected virtual Saml2Action ReadAction(XmlReader reader)
        {
            ...
            try
            {
                ...
                XmlUtil.ValidateXsiType(
                    reader,
                    Saml2Constants.Types.ActionType,
                    Saml2Constants.Namespace
                );
        ...
        /// </para>
        /// </remarks>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see cref="Saml2Advice"/>
        // element.</param>
        ...
        protected virtual Saml2Advice ReadAdvice(XmlReader reader)
        {
            ...
            try
            {
                ...
                XmlUtil.ValidateXsiType(
                    reader,
                    Saml2Constants.Types.AdviceType,
                    Saml2Constants.Namespace
                );
                ...
                if (!isEmpty)
                {
                    ...
                    while (reader.IsStartElement())
                    {
                        ...
                        if (
                            reader.IsStartElement(
                                Saml2Constants.Elements.AssertionIDRef,
                                Saml2Constants.Namespace
                            )
                        )
                        {
                            ...
                            reader.IsStartElement(
                                Saml2Constants.Elements.AssertionURIRef,
                                Saml2Constants.Namespace
                            )
                        )
                        {
                        ...
                        )
                        {
                            ...
                            reader.IsStartElement(
                                Saml2Constants.Elements.EncryptedAssertion,
                                Saml2Constants.Namespace
                            )
                        )
                        {
                        ...
                        else
                        {
                            TraceUtility.TraceString(
                                ...
                                SR.GetString(SR.ID8006, reader.LocalName, reader.NamespaceURI)
                            );
                            reader.Skip();
                        }
                    }
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see cref="Saml2Assertion"/>
        // element.</param>
        ...
        protected virtual Saml2Assertion ReadAssertion(XmlReader reader)
        {
            ...
            XmlDictionaryReader plaintextReader = XmlDictionaryReader.CreateDictionaryReader(
                reader
            );

            Saml2Assertion assertion = new Saml2Assertion(
                new Saml2NameIdentifier("__TemporaryIssuer__")
            );
            ...
            // Throw if wrong element
            if (
                !plaintextReader.IsStartElement(
                    Saml2Constants.Elements.Assertion,
                    Saml2Constants.Namespace
                )
            )
...
#pragma warning suppress 56504 // bogus - thinks plaintextReader.LocalName, plaintextReader.NamespaceURI need validation
                throw DiagnosticUtility.ThrowHelperXml(
                    plaintextReader,
                    SR.GetString(SR.ID3061, plaintextReader.LocalName, plaintextReader.NamespaceURI)
                );
            }
            ...
            EnvelopedSignatureReader realReader = new EnvelopedSignatureReader(
                plaintextReader,
                wrappedSerializer,
            ...
            try
            {
                ...
                XmlUtil.ValidateXsiType(
                    realReader,
                    Saml2Constants.Types.AssertionType,
                    Saml2Constants.Namespace
                );
                ...
                if (!StringComparer.Ordinal.Equals(assertion.Version, version))
                {
                    throw DiagnosticUtility.ThrowHelperXml(
                        realReader,
                        SR.GetString(SR.ID4100, version)
                    );
                }
                ...
                // <Subject> 0-1
                if (
                    realReader.IsStartElement(
                        Saml2Constants.Elements.Subject,
                        Saml2Constants.Namespace
                    )
                )
                {
                ...
                if (
                    realReader.IsStartElement(
                        Saml2Constants.Elements.Conditions,
                        Saml2Constants.Namespace
                    )
                )
                {
                ...
                if (
                    realReader.IsStartElement(
                        Saml2Constants.Elements.Advice,
                        Saml2Constants.Namespace
                    )
                )
                {
                ...
                while (realReader.IsStartElement())
                {
                    ...
                    if (
                        realReader.IsStartElement(
                            Saml2Constants.Elements.Statement,
                            Saml2Constants.Namespace
                        )
                    )
                    {
                        ...
                        realReader.IsStartElement(
                            Saml2Constants.Elements.AttributeStatement,
                            Saml2Constants.Namespace
                        )
                    )
                    {
                        ...
                        realReader.IsStartElement(
                            Saml2Constants.Elements.AuthnStatement,
                            Saml2Constants.Namespace
                        )
                    )
                    {
                        ...
                        realReader.IsStartElement(
                            Saml2Constants.Elements.AuthzDecisionStatement,
                            Saml2Constants.Namespace
                        )
                    )
                    {
                ...
                if (null == assertion.Subject)
                {
                    ...
                    {
                        if (
                            statement is Saml2AuthenticationStatement
        ...
        /// </summary>
        /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
        // cref="Saml2Assertion"/>.</param>
        /// <param name="data">The <see cref="Saml2Assertion"/> to serialize.</param>
        /// <exception cref="ArgumentNullException">The <paramref name="writer"/> or <paramref name="data"/>
        // parameters are null.</exception>
        /// <exception cref="InvalidOperationException"> The <paramref name="data"/>  has both <see
        // cref="EncryptingCredentials"/> and <see cref="ReceivedEncryptingCredentials"/> properties
        // null.</exception>
        /// <exception cref="InvalidOperationException">The <paramref name="data"/> must have a <see
        // cref="Saml2Subject"/> if no <see cref="Saml2Statement"/> are present.</exception>
        /// <exception cref="InvalidOperationException">The SAML2 authentication, attribute, and
        // authorization decision <see cref="Saml2Statement"/> require a <see
        // cref="Saml2Subject"/>.</exception>
        /// <exception cref="CryptographicException">Token encrypting credentials must have a Symmetric Key
        // specified.</exception>
        protected virtual void WriteAssertion(XmlWriter writer, Saml2Assertion data)
        {
            ...
            // If an EncryptingCredentials is present then check if this is not of type
            // ReceivedEncryptinCredentials.
            // ReceivedEncryptingCredentials mean that it was credentials that were hydrated from a token
            // received
            // on the wire. We should not directly use this while re-serializing a token.
            if (
                (null != data.EncryptingCredentials)
                && !(data.EncryptingCredentials is ReceivedEncryptingCredentials)
            )
            {
                plaintextStream = new MemoryStream();
                writer = plaintextWriter = XmlDictionaryWriter.CreateTextWriter(
                    plaintextStream,
                    Encoding.UTF8,
                    false
                );
...
#pragma warning suppress 56506
                    writer = signatureWriter = new EnvelopedSignatureWriter(
                        writer,
                ...
                writer.WriteAttributeString(
                    Saml2Constants.Attributes.IssueInstant,
                    XmlConvert.ToString(
                        data.IssueInstant.ToUniversalTime(),
                        DateTimeFormats.Generated
                    )
                );
            ...
            if (null != plaintextWriter)
            {
                ...
                SymmetricAlgorithm symmetricAlgorithm = encryptingKey.GetSymmetricAlgorithm(
                    data.EncryptingCredentials.Algorithm
                );
                encryptedData.Encrypt(
                    symmetricAlgorithm,
                    plaintextStream.GetBuffer(),
                    0,
        ...
        /// this method.
        /// </remarks>
        /// <param name="reader">An <see cref="XmlReader"/> positioned at a <see cref="Saml2Attribute"/>
        // element.</param>
        ...
        protected virtual Saml2Attribute ReadAttribute(XmlReader reader)
        {
            ...
            try
            {
                ...
                string originalIssuer = reader.GetAttribute(
                    Saml2Constants.Attributes.OriginalIssuer,
                ...
                if (!isEmpty)
                {
                    ...
                    )
                    {
                        ...
                        if (
                            attributeValueXsiTypePrefix != null
                            && attributeValueXsiTypeSuffix != null
                        )
                        {
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see
        // cref="Saml2Attribute"/>.</param>
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see
        // cref="Saml2AttributeStatement"/> element.</param>
        ...
        protected virtual Saml2AttributeStatement ReadAttributeStatement(XmlReader reader)
        {
            ...
            try
            {
                ...
                XmlUtil.ValidateXsiType(
                    reader,
                    Saml2Constants.Types.AttributeStatementType,
                    Saml2Constants.Namespace,
                    isStatementElement
                );
                ...
                while (reader.IsStartElement())
                {
                    if (
                        reader.IsStartElement(
                            Saml2Constants.Elements.EncryptedAttribute,
                            Saml2Constants.Namespace
                        )
                    )
                    {
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see
        // cref="Saml2AudienceRestriction"/> element.</param>
        ...
        protected virtual Saml2AudienceRestriction ReadAudienceRestriction(XmlReader reader)
        {
            ...
            if (reader.IsStartElement(Saml2Constants.Elements.Condition, Saml2Constants.Namespace))
            {
                ...
                !reader.IsStartElement(
                    Saml2Constants.Elements.AudienceRestriction,
                    Saml2Constants.Namespace
                )
            )
            {
            ...
            try
            {
                ...
                XmlUtil.ValidateXsiType(
                    reader,
                    Saml2Constants.Types.AudienceRestrictionType,
                    Saml2Constants.Namespace,
                    isConditionElement
                );
                ...
                if (
                    !reader.IsStartElement(
                        Saml2Constants.Elements.Audience,
                        Saml2Constants.Namespace
                    )
                )
                {
                ...
                audienceRestriction = new Saml2AudienceRestriction(
        ...
        /// </summary>
        /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
        // cref="Saml2AudienceRestriction"/>.</param>
        /// <param name="data">The <see cref="Saml2AudienceRestriction"/> to serialize.</param>
        protected virtual void WriteAudienceRestriction(
            XmlWriter writer,
            Saml2AudienceRestriction data
        )
        {
            if (null == writer)
            {
        ...
        /// </remarks>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see
        // cref="Saml2AuthenticationContext"/> element.</param>
        ...
        protected virtual Saml2AuthenticationContext ReadAuthenticationContext(XmlReader reader)
        {
            ...
            if (
                !reader.IsStartElement(
                    Saml2Constants.Elements.AuthnContext,
                    Saml2Constants.Namespace
                )
            )
            {
            ...
            try
            {
                ...
                XmlUtil.ValidateXsiType(
                    reader,
                    Saml2Constants.Types.AuthnContextType,
                    Saml2Constants.Namespace
                );
                ...
                if (
                    reader.IsStartElement(
                        Saml2Constants.Elements.AuthnContextClassRef,
                        Saml2Constants.Namespace
                    )
                )
                {
                ...
                if (
                    reader.IsStartElement(
                        Saml2Constants.Elements.AuthnContextDecl,
                        Saml2Constants.Namespace
                    )
                )
                {
                ...
                if (
                    reader.IsStartElement(
                        Saml2Constants.Elements.AuthnContextDeclRef,
                        Saml2Constants.Namespace
                    )
                )
                {
                ...
                while (
                    reader.IsStartElement(
                        Saml2Constants.Elements.AuthenticatingAuthority,
                        Saml2Constants.Namespace
                    )
                )
                {
        ...
        /// </summary>
        /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
        // cref="Saml2AuthenticationContext"/>.</param>
        ...
        )
        {
            ...
            if (null != data.ClassReference)
            {
                writer.WriteElementString(
                    Saml2Constants.Elements.AuthnContextClassRef,
                    Saml2Constants.Namespace,
                    data.ClassReference.AbsoluteUri
                );
            }
            ...
            if (null != data.DeclarationReference)
            {
                writer.WriteElementString(
                    Saml2Constants.Elements.AuthnContextDeclRef,
                    Saml2Constants.Namespace,
                    data.DeclarationReference.AbsoluteUri
                );
            }
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see
        // cref="Saml2AuthenticationStatement"/> element.</param>
        ...
        protected virtual Saml2AuthenticationStatement ReadAuthenticationStatement(XmlReader reader)
        {
            ...
            try
            {
                ...
                XmlUtil.ValidateXsiType(
                    reader,
                    Saml2Constants.Types.AuthnStatementType,
                    Saml2Constants.Namespace,
                    isStatementElement
                );
                ...
                if (string.IsNullOrEmpty(value))
                {
                    throw DiagnosticUtility.ThrowHelperXml(
                        ...
                        SR.GetString(
                            SR.ID0001,
                            Saml2Constants.Attributes.AuthnInstant,
                            Saml2Constants.Elements.AuthnStatement
                        )
                    );
                }
                ...
                if (
                    reader.IsStartElement(
                        Saml2Constants.Elements.SubjectLocality,
                        Saml2Constants.Namespace
                    )
                )
                {
        ...
        /// </summary>
        /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
        // cref="Saml2AuthenticationStatement"/>.</param>
        ...
        )
        {
            ...
            writer.WriteAttributeString(
                Saml2Constants.Attributes.AuthnInstant,
                XmlConvert.ToString(
                    data.AuthenticationInstant.ToUniversalTime(),
                    DateTimeFormats.Generated
                )
            );
            ...
            {
                writer.WriteAttributeString(
                    Saml2Constants.Attributes.SessionIndex,
                    data.SessionIndex
                );
            }
            ...
            {
                writer.WriteAttributeString(
                    Saml2Constants.Attributes.SessionNotOnOrAfter,
                    XmlConvert.ToString(
                        data.SessionNotOnOrAfter.Value.ToUniversalTime(),
                        DateTimeFormats.Generated
                    )
                );
            }
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see
        // cref="Saml2AuthorizationDecisionStatement"/> element.</param>
        ...
        )
        {
            ...
            try
            {
                ...
                XmlUtil.ValidateXsiType(
                    reader,
                    Saml2Constants.Types.AuthzDecisionStatementType,
                    Saml2Constants.Namespace,
                    isStatementElement
                );
                ...
                if (string.IsNullOrEmpty(value))
                {
                    throw DiagnosticUtility.ThrowHelperXml(
                        ...
                        SR.GetString(
                            SR.ID0001,
                            Saml2Constants.Attributes.Decision,
                            Saml2Constants.Elements.AuthzDecisionStatement
                        )
                    );
                }
                ...
                if (null == value)
                {
                    throw DiagnosticUtility.ThrowHelperXml(
                        ...
                        SR.GetString(
                            SR.ID0001,
                            Saml2Constants.Attributes.Resource,
                            Saml2Constants.Elements.AuthzDecisionStatement
                        )
                    );
                }
                ...
                do
                {
                    ...
                    reader.IsStartElement(Saml2Constants.Elements.Action, Saml2Constants.Namespace)
                );

                // <Evidence> 0-1
                if (
                    reader.IsStartElement(
                        Saml2Constants.Elements.Evidence,
                        Saml2Constants.Namespace
                    )
                )
                {
        ...
        /// </summary>
        /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
        // cref="Saml2AuthorizationDecisionStatement"/>.</param>
...
#pragma warning suppress 56506 // Resource are never null
            writer.WriteAttributeString(
                Saml2Constants.Attributes.Resource,
                data.Resource.Equals(Saml2AuthorizationDecisionStatement.EmptyResource)
                    ? data.Resource.ToString()
                    : data.Resource.AbsoluteUri
            );
        ...
        /// method.
        /// </remarks>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see cref="Saml2Conditions"/>
        // element.</param>
        ...
        protected virtual Saml2Conditions ReadConditions(XmlReader reader)
        {
            ...
            // throw if wrong element
            if (
                !reader.IsStartElement(Saml2Constants.Elements.Conditions, Saml2Constants.Namespace)
            )
            {
                reader.ReadStartElement(
                    Saml2Constants.Elements.Conditions,
            ...
            try
            {
                ...
                XmlUtil.ValidateXsiType(
                    reader,
                    Saml2Constants.Types.ConditionsType,
                    Saml2Constants.Namespace
                );
                ...
                if (!string.IsNullOrEmpty(value))
                {
                    conditions.NotOnOrAfter = XmlConvert.ToDateTime(
                        value,
                        DateTimeFormats.Accepted
                    );
                }
                ...
                if (!isEmpty)
                {
                    ...
                    while (reader.IsStartElement())
                    {
                        ...
                        if (
                            reader.IsStartElement(
                                Saml2Constants.Elements.Condition,
                                Saml2Constants.Namespace
                            )
                        )
                        {
                            ...
                            if (
                                ...
                                || XmlUtil.EqualsQName(
                                    declaredType,
                                    Saml2Constants.Types.ConditionAbstractType,
                                    Saml2Constants.Namespace
                                )
                            )
                            {
                            ...
                            )
                            {
                                ...
                                XmlUtil.EqualsQName(
                                    declaredType,
                                    Saml2Constants.Types.OneTimeUseType,
                                    Saml2Constants.Namespace
                                )
                            )
                            {
                                if (conditions.OneTimeUse)
                                {
                                    throw DiagnosticUtility.ThrowHelperXml(
                                        reader,
                                        SR.GetString(SR.ID4115, Saml2Constants.Elements.OneTimeUse)
                                    );
                                }
                                ...
                                XmlUtil.EqualsQName(
                                    declaredType,
                                    Saml2Constants.Types.ProxyRestrictionType,
                                    Saml2Constants.Namespace
                                )
                            )
                            {
                                if (null != conditions.ProxyRestriction)
                                {
                                    throw DiagnosticUtility.ThrowHelperXml(
                                        ...
                                        SR.GetString(
                                            SR.ID4115,
                                            Saml2Constants.Elements.ProxyRestricton
                                        )
                                    );
                                }
                        ...
                        )
                        {
                            ...
                            reader.IsStartElement(
                                Saml2Constants.Elements.OneTimeUse,
                                Saml2Constants.Namespace
                            )
                        )
                        {
                        ...
                        )
                        {
                            if (null != conditions.ProxyRestriction)
                            {
                                throw DiagnosticUtility.ThrowHelperXml(
                                    reader,
                                    SR.GetString(SR.ID4115, Saml2Constants.Elements.ProxyRestricton)
                                );
                            }
        ...
        protected virtual void WriteConditions(XmlWriter writer, Saml2Conditions data)
        {
            ...
            {
                writer.WriteAttributeString(
                    Saml2Constants.Attributes.NotBefore,
                    XmlConvert.ToString(
                        data.NotBefore.Value.ToUniversalTime(),
                        DateTimeFormats.Generated
                    )
                );
            }
            ...
            {
                writer.WriteAttributeString(
                    Saml2Constants.Attributes.NotOnOrAfter,
                    XmlConvert.ToString(
                        data.NotOnOrAfter.Value.ToUniversalTime(),
                        DateTimeFormats.Generated
                    )
                );
            }
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see cref="Saml2Evidence"/>
        // element.</param>
        ...
        protected virtual Saml2Evidence ReadEvidence(XmlReader reader)
        {
            ...
            try
            {
                ...
                XmlUtil.ValidateXsiType(
                    reader,
                    Saml2Constants.Types.EvidenceType,
                    Saml2Constants.Namespace
                );
                ...
                if (
                    0 == evidence.AssertionIdReferences.Count
        ...
        protected virtual void WriteEvidence(XmlWriter writer, Saml2Evidence data)
        {
            ...
            if (
                (data.AssertionIdReferences == null || 0 == data.AssertionIdReferences.Count)
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see cref="Saml2NameIdentifier"/>
        // element.</param>
        ...
        /// </summary>
        /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
        // cref="Saml2NameIdentifier"/>.</param>
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a than can be positioned at a
        // ds:KeyInfo element.</param>
        /// <param name="assertion">The <see cref="Saml2Assertion"/> that is having the signature
        // checked.</param>
        ...
        /// </summary>
        /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
        // cref="SecurityKeyIdentifier"/>.</param>
        /// <param name="data">The <see cref="SecurityKeyIdentifier"/> to serialize.</param>
        /// <exception cref="ArgumentNullException">The input parameter 'writer' or 'data' is
        // null.</exception>
        ...
        /// <exception cref="ArgumentNullException">The input parameter 'writer' or 'data' is
        // null.</exception>
        /// <exception cref="CryptographicException">Saml2NameIdentifier encrypting credentials must have a
        // Symmetric Key specified.</exception>
        protected virtual void WriteNameId(XmlWriter writer, Saml2NameIdentifier data)
        {
            ...
            if (data.EncryptingCredentials != null)
            {
                ...
                try
                {
                    ...
                    using (
                        XmlWriter plaintextWriter = XmlDictionaryWriter.CreateTextWriter(
                            plaintextStream,
                            Encoding.UTF8,
                            false
                        )
                    ...
                    writer.WriteStartElement(
                        Saml2Constants.Elements.EncryptedID,
                        Saml2Constants.Namespace
                    );
        ...
        protected virtual Saml2NameIdentifier ReadNameIdType(XmlReader reader)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(value))
                {
                    if (!UriUtil.CanCreateValidUri(value, UriKind.Absolute))
                    {
                        throw DiagnosticUtility.ThrowHelperXml(
                            ...
                            SR.GetString(
                                SR.ID0011,
                                Saml2Constants.Attributes.Format,
                                Saml2Constants.Elements.NameID
                            )
                        );
                    }
                ...
                // According to section 8.3.6, if the name identifier format is of type
                // 'urn:oasis:names:tc:SAML:2.0:nameid-format:entity'
                // the name identifier value must be a uri and name qualifier, spname qualifier, and spproded id
                // must be omitted.
                if (
                ...
                )
                {
                    ...
                    if (
                        !string.IsNullOrEmpty(nameIdentifier.NameQualifier)
        ...
        /// <returns>An instance of <see cref="Saml2NameIdentifier"/> representing the EncryptedId that was
        // read</returns>
        /// <exception cref="ArgumentNullException">The input parameter 'reader' is null.</exception>
        /// <exception cref="XmlException">The 'reader' is not positioned at an 'EncryptedID'
        // element.</exception>
        protected virtual Saml2NameIdentifier ReadEncryptedId(XmlReader reader)
        {
            ...
            {
                using (
                    XmlDictionaryReader wrappedReader = new WrappedXmlDictionaryReader(
                        XmlReader.Create(sr),
                        XmlDictionaryReaderQuotas.Max
                    )
                )
        ...
        protected virtual void WriteNameIdType(XmlWriter writer, Saml2NameIdentifier data)
        {
            ...
            {
                writer.WriteAttributeString(
                    Saml2Constants.Attributes.NameQualifier,
                    data.NameQualifier
                );
            }
            ...
            {
                writer.WriteAttributeString(
                    Saml2Constants.Attributes.SPNameQualifier,
                    data.SPNameQualifier
                );
            }
            ...
            {
                writer.WriteAttributeString(
                    Saml2Constants.Attributes.SPProvidedID,
                    data.SPProvidedId
                );
            }
        ...
        /// </remarks>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see
        // cref="Saml2ProxyRestriction"/> element.</param>
        ...
        /// </summary>
        /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
        // cref="Saml2ProxyRestriction"/>.</param>
        ...
        protected virtual void WriteProxyRestriction(XmlWriter writer, Saml2ProxyRestriction data)
        {
            ...
            {
                writer.WriteAttributeString(
                    Saml2Constants.Attributes.Count,
                    XmlConvert.ToString(data.Count.Value)
                );
            }
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see cref="Saml2Statement"/>
        // element.</param>
        ...
        protected virtual Saml2Statement ReadStatement(XmlReader reader)
        {
            ...
            else
            {
                ...
                throw DiagnosticUtility.ThrowHelperXml(
                    reader,
                    SR.GetString(SR.ID4105, declaredType.Name, declaredType.Namespace)
                );
            }
        }
        ...
        protected virtual void WriteStatement(XmlWriter writer, Saml2Statement data)
        {
            ...
            Saml2AuthorizationDecisionStatement authzStatement =
                data as Saml2AuthorizationDecisionStatement;
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see cref="Saml2Subject"/>
        // element.</param>
...
#pragma warning suppress 56506 // SubjectConfirmations is never null
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see
        // cref="Saml2SubjectConfirmation"/> element.</param>
        ...
        protected virtual Saml2SubjectConfirmation ReadSubjectConfirmation(XmlReader reader)
        {
            ...
            try
            {
                ...
                XmlUtil.ValidateXsiType(
                    reader,
                    Saml2Constants.Types.SubjectConfirmationType,
                    Saml2Constants.Namespace
                );
                ...
                if (string.IsNullOrEmpty(method))
                {
                    throw DiagnosticUtility.ThrowHelperXml(
                        ...
                        SR.GetString(
                            SR.ID0001,
                            Saml2Constants.Attributes.Method,
                            Saml2Constants.Elements.SubjectConfirmation
                        )
                    );
                }
                ...
                Saml2SubjectConfirmation subjectConfirmation = new Saml2SubjectConfirmation(
                    new Uri(method)
                );
                ...
                {
                    // <NameID> | <EncryptedID> | <BaseID> 0-1
                    subjectConfirmation.NameIdentifier = this.ReadSubjectId(
                        reader,
                        Saml2Constants.Elements.SubjectConfirmation
                    );
                    ...
                    if (
                        reader.IsStartElement(
                            Saml2Constants.Elements.SubjectConfirmationData,
                            Saml2Constants.Namespace
                        )
                    )
                    {
                        subjectConfirmation.SubjectConfirmationData =
                            this.ReadSubjectConfirmationData(reader);
                    }
        ...
        /// </summary>
        /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
        // cref="Saml2SubjectConfirmation"/>.</param>
        /// <param name="data">The <see cref="Saml2SubjectConfirmation"/> to serialize.</param>
        protected virtual void WriteSubjectConfirmation(
            XmlWriter writer,
            Saml2SubjectConfirmation data
        )
        {
            if (null == writer)
            {
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see
        // cref="Saml2SubjectConfirmationData"/> element.</param>
            ...
            try
            {
                ...
                if (null != type)
                {
                    ...
                    )
                    {
                        ...
                        !XmlUtil.EqualsQName(
                            type,
                            Saml2Constants.Types.SubjectConfirmationDataType,
                            Saml2Constants.Namespace
                        )
                    )
                    {
                        throw DiagnosticUtility.ThrowHelperXml(
                            reader,
                            SR.GetString(SR.ID4112, type.Name, type.Namespace)
                        );
                    }
                }
                ...
                if (requireKeyInfo && isEmpty)
                {
                    throw DiagnosticUtility.ThrowHelperXml(
                        reader,
                        SR.GetString(SR.GetString(SR.ID4111))
                    );
                }
                ...
                if (!string.IsNullOrEmpty(value))
                {
                    confirmationData.NotBefore = XmlConvert.ToDateTime(
                        value,
                        DateTimeFormats.Accepted
                    );
                }
                ...
                if (!string.IsNullOrEmpty(value))
                {
                    confirmationData.NotOnOrAfter = XmlConvert.ToDateTime(
                        value,
                        DateTimeFormats.Accepted
                    );
                }
                ...
                if (!string.IsNullOrEmpty(value))
                {
                    if (!UriUtil.CanCreateValidUri(value, UriKind.Absolute))
                    {
                        throw DiagnosticUtility.ThrowHelperXml(
                            ...
                            SR.GetString(
                                SR.ID0011,
                                Saml2Constants.Attributes.Recipient,
                                Saml2Constants.Elements.SubjectConfirmationData
                            )
                        );
                    }
                ...
                if (!isEmpty)
                {
                    ...
                    if (!requireKeyInfo && XmlNodeType.EndElement != reader.NodeType)
                    {
                        ...
                        throw DiagnosticUtility.ThrowHelperXml(
                            reader,
                            SR.GetString(SR.ID4114, Saml2Constants.Elements.SubjectConfirmationData)
                        );
                    }
        ...
        /// </remarks>
        /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
        // cref="Saml2SubjectConfirmationData"/>.</param>
        ...
        )
        {
            ...
            {
                writer.WriteAttributeString(
                    Saml2Constants.Attributes.InResponseTo,
        ...
        /// </summary>
        /// <param name="reader">A <see cref="XmlReader"/> positioned at a <see
        // cref="Saml2SubjectLocality"/> element.</param>
        ...
        protected virtual Saml2SubjectLocality ReadSubjectLocality(XmlReader reader)
        {
            ...
            try
            {
                ...
                XmlUtil.ValidateXsiType(
                    reader,
                    Saml2Constants.Types.SubjectLocalityType,
                    Saml2Constants.Namespace
                );
        ...
        /// </summary>
        /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
        // cref="Saml2SubjectLocality"/>.</param>
        ...
        internal class WrappedSerializer : SecurityTokenSerializer
        {
            ...
            /// <param name="writer">A <see cref="XmlWriter"/> to serialize the <see
            // cref="SecurityKeyIdentifierClause"/>.</param>
            /// <param name="keyIdentifierClause">The <see cref="SecurityKeyIdentifierClause"/> to
            // serialize.</param>
