...
namespace System.IdentityModel.Tokens
{
    ...
    {
        Dictionary<string, string> _configuredTrustedIssuers = new Dictionary<string, string>(
            new ThumbprintKeyComparer()
        );
        ...
        public override void LoadCustomConfiguration(XmlNodeList customConfiguration)
        {
            ...
            if (configNodes.Count != 1)
            {
                throw DiagnosticUtility.ThrowHelperInvalidOperation(
                    SR.GetString(SR.ID7019, typeof(ConfigurationBasedIssuerNameRegistry).Name)
                );
            }
            ...
            foreach (XmlNode node in customConfigElement.ChildNodes)
            {
                ...
                if (childElement != null)
                {
                    ...
                    )
                    {
                        ...
                        string issuerName =
                            ((nameAttribute == null) || string.IsNullOrEmpty(nameAttribute.Value))
                                ? String.Empty
                                : String.Intern(nameAttribute.Value);
                    ...
                    )
                    {
                        ...
                        string thumbprint = childElement
                            .Attributes.GetNamedItem(ConfigurationStrings.Thumbprint)
                            .Value;
                        thumbprint = thumbprint.Replace(" ", "");
        ...
        public override string GetIssuerName(SecurityToken securityToken)
        {
            ...
            if (x509SecurityToken != null)
            {
                ...
                if (_configuredTrustedIssuers.ContainsKey(thumbprint))
                {
                    ...
                    issuerName = string.IsNullOrEmpty(issuerName)
                        ? x509SecurityToken.Certificate.Subject
                        : issuerName;

                    if (TD.GetIssuerNameSuccessIsEnabled())
                    {
                        TD.GetIssuerNameSuccess(
        ...
        public void AddTrustedIssuer(string certificateThumbprint, string name)
        {
            ...
            {
                throw DiagnosticUtility.ThrowHelperArgumentNullOrEmptyString(
            ...
            {
                throw DiagnosticUtility.ThrowHelperInvalidOperation(
                    SR.GetString(SR.ID4265, certificateThumbprint)
                );
            }
