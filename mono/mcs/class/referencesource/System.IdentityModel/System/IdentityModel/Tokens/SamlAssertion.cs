...
namespace System.IdentityModel.Tokens
{
    ...
    public class SamlAssertion : ICanonicalWriterEndRootElementCallback
    {
        ...
        SamlAdvice advice;
        readonly ImmutableCollection<SamlStatement> statements =
            new ImmutableCollection<SamlStatement>();
        ...
        )
        {
            ...
            if (!effectiveReader.CanCanonicalize)
            {
                ...
                XmlDictionaryWriter writer = XmlDictionaryWriter.CreateBinaryWriter(
                    stream,
                    samlSerializer.DictionaryManager.ParentDictionary
                );
                writer.WriteNode(effectiveReader, false);
                writer.Flush();
                stream.Position = 0;
                effectiveReader = XmlDictionaryReader.CreateBinaryReader(
                    stream.GetBuffer(),
                    0,
                    (int)stream.Length,
                    samlSerializer.DictionaryManager.ParentDictionary,
            ...
            SignedXml signedXml = new SignedXml(
                new StandardSignedInfo(samlSerializer.DictionaryManager),
                samlSerializer.DictionaryManager,
                keyInfoSerializer
            );
            ...
            this.verificationKey = SamlSerializer.ResolveSecurityKey(
        ...
        ReadOnlyCollection<SecurityKey> BuildCryptoList()
        {
            ...
            for (int i = 0; i < this.statements.Count; ++i)
            {
                ...
                if (statement != null)
                {
                    ...
                    InMemorySymmetricSecurityKey inMemorySymmetricSecurityKey =
                        crypto as InMemorySymmetricSecurityKey;
                    if (inMemorySymmetricSecurityKey != null)
                    {
                        ...
                        {
                            if (
                                (cryptoList[j] is InMemorySymmetricSecurityKey)
                                && (cryptoList[j].KeySize == inMemorySymmetricSecurityKey.KeySize)
                            )
                            {
                                byte[] key1 = (
                                    (InMemorySymmetricSecurityKey)cryptoList[j]
                                ).GetSymmetricKey();
        ...
        void ICanonicalWriterEndRootElementCallback.OnEndOfRootElement(
...
#pragma warning suppress 56506 // samlSerializer.DictionaryManager is never null.
            ...
            {
                this.conditions = samlSerializer.LoadConditions(
                    wrappedReader,
                    keyInfoSerializer,
                    outOfBandTokenResolver
                );
            ...
            {
                this.advice = samlSerializer.LoadAdvice(
                    wrappedReader,
                    keyInfoSerializer,
                    outOfBandTokenResolver
                );
...
#pragma warning suppress 56506 // samlSerializer.DictionaryManager is never null.
                if (
                    wrappedReader.IsStartElement(
                        samlSerializer.DictionaryManager.XmlSignatureDictionary.Signature,
                ...
                {
                    SamlStatement statement = samlSerializer.LoadStatement(
                        wrappedReader,
                        keyInfoSerializer,
                        outOfBandTokenResolver
                    );
        ...
        )
        {
            ...
            if (this.signingCredentials != null)
            {
                ...
                )
                {
                    ...
                    SamlDelegatingWriter delegatingWriter = new SamlDelegatingWriter(
                        dictionaryWriter,
                        this.hashStream,
                        this,
                        samlSerializer.DictionaryManager.ParentDictionary
