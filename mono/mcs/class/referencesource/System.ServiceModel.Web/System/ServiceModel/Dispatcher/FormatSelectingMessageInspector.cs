...
namespace System.ServiceModel.Description
{
    ...
    class FormatSelectingMessageInspector : IDispatchMessageInspector
    {
        ...
        )
        {
            ...
            Fx.Assert(
                !this.formatters.ContainsKey(operationName),
            ...
            if (this.automaticFormatSelectionEnabled)
            {
                ...
                )
                {
                    ...
                    )
                    {
                        ...
                        {
                            if (
                                TrySetFormatFromCache(operationName, acceptHeader)
                                || TrySetFormatFromAcceptHeader(
                                    operationName,
                                    acceptHeader,
                                    true /* matchCharSet */
                                )
        ...
        {
            Fx.Assert(
                this.caches.ContainsKey(operationName),
                "The calling method is responsible for ensuring that the 'operationName' key exists in the caches dictionary."
            );
            Fx.Assert(
                acceptHeader != null,
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage(
            ...
            Justification = "Given that media types are generally in lowercase, it is awkward to normalize to uppercase."
        )]
        bool TrySetFormatFromAcceptHeader(
            string operationName,
            string acceptHeader,
            bool matchCharSet
        )
        {
            ...
            for (int i = 0; i < acceptHeaderElements.Count; i++)
            {
                ...
                if (
                    (subType[0] == '*' && subType.Length == 1)
                    && ((type[0] == '*' && type.Length == 1) || wildcardMediaTypes.Contains(type))
                )
                {
                ...
                foreach (MultiplexingFormatMapping mapping in mappings)
                {
                    ...
                    if (
                        this.formatters[operationName].SupportsMessageFormat(format)
                        && mapping.CanFormatResponse(
                            acceptHeaderElements[i],
                            matchCharSet,
                            out contentType
                        )
                    )
        ...
        void SetFormatFromDefault(string operationName, string acceptHeader)
        {
            ...
            if (DiagnosticUtility.ShouldTraceInformation)
            {
                TraceUtility.TraceEvent(
                    ...
                    TraceCode.AutomaticFormatSelectedOperationDefault,
                    SR2.GetString(
                        SR2.TraceCodeAutomaticFormatSelectedOperationDefault,
                        format.ToString()
                    )
                );
            }
        ...
        void SetFormatAndContentType(WebMessageFormat format, string contentType)
        {
            ...
            if (DiagnosticUtility.ShouldTraceInformation)
            {
                TraceUtility.TraceEvent(
                    TraceEventType.Information,
                    TraceCode.AutomaticFormatSelectedRequestBased,
                    SR2.GetString(
                        SR2.TraceCodeAutomaticFormatSelectedRequestBased,
                        format.ToString(),
                        contentType
                    )
                );
