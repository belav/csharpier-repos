...
namespace System.ServiceModel.Description
{
    ...
    public abstract partial class MetadataImporter
    {
        ...
        internal static IEnumerable<PolicyConversionContext> GetPolicyConversionContextEnumerator(
            ...
            PolicyAlternatives policyAlternatives
        )
        {
            return ImportedPolicyConversionContext.GetPolicyConversionContextEnumerator(
                endpoint,
                policyAlternatives,
                MetadataImporterQuotas.Defaults
            );
        }
        ...
        internal sealed class ImportedPolicyConversionContext : PolicyConversionContext
        {
            ...
            ImportedPolicyConversionContext(
                ServiceEndpoint endpoint,
                IEnumerable<XmlElement> endpointAssertions,
                Dictionary<
                    OperationDescription,
                ...
                : base(endpoint)
            {
                ...
                this.endpointAssertions = new PolicyAssertionCollection(
            ...
            public static IEnumerable<PolicyConversionContext> GetPolicyConversionContextEnumerator(
                ServiceEndpoint endpoint,
                PolicyAlternatives policyAlternatives,
                MetadataImporterQuotas quotas
            )
            {
                IEnumerable<
                    Dictionary<MessageDescription, IEnumerable<XmlElement>>
                > messageAssertionEnumerator;
                IEnumerable<
                    Dictionary<FaultDescription, IEnumerable<XmlElement>>
                > faultAssertionEnumerator;
                IEnumerable<
                    Dictionary<OperationDescription, IEnumerable<XmlElement>>
                ...
                messageAssertionEnumerator = PolicyIterationHelper.GetCartesianProduct<
                    MessageDescription,
                    IEnumerable<XmlElement>
                >(policyAlternatives.MessageBindingAlternatives);
                operationAssertionEnumerator = PolicyIterationHelper.GetCartesianProduct<
                    OperationDescription,
                    IEnumerable<XmlElement>
                ...
                )
                {
                    ...
                    )
                    {
                        ...
                        {
                            foreach (
                                IEnumerable<XmlElement> endpointAssertionsSelection in policyAlternatives.EndpointAlternatives
                            )
                            {
                                ImportedPolicyConversionContext conversionContext;
                                try
                                {
                                    conversionContext = new ImportedPolicyConversionContext(
                                        endpoint,
                                        endpointAssertionsSelection,
                                        operationAssertionsSelection,
                                        messageAssertionsSelection,
                                        faultAssertionsSelection,
                                        quotas
                                    );
                                }
            ...
            static class PolicyIterationHelper
            {
                ...
                //
                internal static IEnumerable<Dictionary<K, V>> GetCartesianProduct<K, V>(
                    Dictionary<K, IEnumerable<V>> sets
                )
                {
                ...
                static KeyValuePair<K, IEnumerator<V>>[] InitializeCounter<K, V>(
                    Dictionary<K, IEnumerable<V>> sets,
                    Dictionary<K, V> counterValue
                )
                {
                    KeyValuePair<K, IEnumerator<V>>[] digits = new KeyValuePair<K, IEnumerator<V>>[
                        sets.Count
                    ];
                    ...
                    {
                        digits[i] = new KeyValuePair<K, IEnumerator<V>>(
                            kvp.Key,
                ...
                static bool IncrementCounter<K, V>(
                    KeyValuePair<K, IEnumerator<V>>[] digits,
                    Dictionary<K, IEnumerable<V>> sets,
                    Dictionary<K, V> counterValue
                )
                {
                    ...
                    for (
                        currentDigit = 0;
                        currentDigit < digits.Length && !digits[currentDigit].Value.MoveNext();
                        currentDigit++
                    )
                    {
                        IEnumerator<V> newDigit = sets[digits[currentDigit].Key].GetEnumerator();
                        digits[currentDigit] = new KeyValuePair<K, IEnumerator<V>>(
                            digits[currentDigit].Key,
                            newDigit
                        );
