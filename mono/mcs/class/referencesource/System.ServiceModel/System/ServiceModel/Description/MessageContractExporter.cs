...
namespace System.ServiceModel.Description
{
    ...
    abstract class MessageContractExporter
    {
        ...
        {
            new MessageBindingExporter(exporter, endpointContext).ExportMessageBinding(
                operation,
                messageContractExporterType
            );
        }
        ...
        void AddElementToSchema(XmlSchemaElement element, string elementNs, XmlSchemaSet schemaSet)
        {
            ...
            if (parentOperation.OperationMethod != null)
            {
                ...
                {
                    if (
                        existingElement.Operation.OperationMethod == parentOperation.OperationMethod
                    )
                        return;
                    if (!SchemaHelper.IsMatch(element, existingElement.Element))
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(
                                SR.GetString(
                                    SR.CannotHaveTwoOperationsWithTheSameElement5,
                                    parentOperation.OperationMethod.DeclaringType,
                                    parentOperation.OperationMethod.Name,
                                    qname,
                                    existingElement.Operation.OperationMethod.DeclaringType,
                                    existingElement.Operation.Name
                                )
                            )
                        );
                    }
                ...
                {
                    ExportedMessages.ElementTypes.Add(
                        qname,
                        new OperationElement(element, parentOperation)
                    );
                }
            }
            SchemaHelper.AddElementToSchema(
                element,
                SchemaHelper.GetSchema(elementNs, schemaSet),
                schemaSet
            );
        }
        ...
        protected MessageContractExporter(
            WsdlExporter exporter,
            WsdlContractConversionContext context,
            OperationDescription operation,
            IOperationBehavior extension
        )
        {
        ...
        void ExportMessage(int messageIndex, object state)
        {
            try
            {
                ...
                if (CreateMessage(description, messageIndex, out wsdlMessage))
                {
                    ...
                    StreamFormatter streamFormatter = StreamFormatter.Create(
                        description,
                        operation.Name,
                        isRequest
                    );
                    if (streamFormatter != null)
                    {
                        ExportStreamBody(
                            wsdlMessage,
                            streamFormatter.WrapperName,
                            streamFormatter.WrapperNamespace,
                            streamFormatter.PartName,
                            streamFormatter.PartNamespace,
                            IsRpcStyle(),
                            false /*IsOperationInherited(operation)*/
                        );
                    }
                    else
                    {
        ...
        void ExportAnyMessage(WsdlNS.Message message, MessagePartDescription part)
        {
            ...
            XmlSchema schema = SchemaHelper.GetSchema(
                DataContractSerializerMessageContractImporter.GenericMessageTypeName.Namespace,
                schemas
            );
            ...
            )
            {
                ...
                genericMessageType.Name = DataContractSerializerMessageContractImporter
                    .GenericMessageTypeName
                    .Name;
            ...
            string partName = string.IsNullOrEmpty(part.UniquePartName)
                ? part.Name
                : part.UniquePartName;
        ...
        protected void ExportStreamBody(
            ...
            string partNs,
            bool isRpc,
            bool skipSchemaExport
        )
        {
            XmlSchemaSet schemas = this.exporter.GeneratedXmlSchemas;
            XmlSchema schema = SchemaHelper.GetSchema(
                DataContractSerializerMessageContractImporter.StreamBodyTypeName.Namespace,
                schemas
            );
            if (
                !schema.SchemaTypes.Contains(
            ...
            )
            {
                ...
                streamBodyType.Name = DataContractSerializerMessageContractImporter
                    .StreamBodyTypeName
                    .Name;
                XmlSchemaSimpleTypeRestriction contentRestriction =
                    new XmlSchemaSimpleTypeRestriction();
                contentRestriction.BaseTypeName = XmlSchemaType
                    .GetBuiltInSimpleType(XmlTypeCode.Base64Binary)
                    .QualifiedName;
            ...
            if (!isRpc && wrapperName != null)
                wrapperSequence = ExportWrappedPart(
                    message,
                    wrapperName,
                    wrapperNs,
                    schemas,
        ...
        void ExportFault(FaultDescription fault)
        {
            ...
            WsdlExporter.WSAddressingHelper.AddActionAttribute(
                fault.Action,
                operationFault,
                this.exporter.PolicyVersion
            );
            operationFault.Message = new XmlQualifiedName(
                faultMessage.Name,
        ...
        XmlQualifiedName ExportFaultElement(FaultDescription fault)
        {
            ...
            XmlQualifiedName typeName = ExportType(
                fault.DetailType,
                fault.Name,
                operation.Name,
                out xsdType
            ...
            ExportGlobalElement(
                ...
                typeName,
                xsdType,
                this.exporter.GeneratedXmlSchemas
            );
            return elementName;
        }
        ...
        protected XsdDataContractExporter DataContractExporter
        {
            get
            {
                ...
                if (
                    !exporter.State.TryGetValue(
                        typeof(XsdDataContractExporter),
        ...
        protected XmlQualifiedName ExportType(
            Type type,
            string partName,
        ...
        )
        {
            ...
            if (type == null)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    new InvalidOperationException(
                        SR.GetString(SR.SFxExportMustHaveType, operationName, partName)
                    )
                );
            if (type == typeof(void))
                return null;
        ...
        )
        {
            if (message.Parts[partName] != null)
            {
                if (IsNullOrEmpty(elementName))
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new ArgumentException(
                            SR.GetString(SR.SFxPartNameMustBeUniqueInRpc, partName)
                        )
                    );
                int i = 1;
                while (message.Parts[partName + i] != null)
                {
                    if (i == Int32.MaxValue)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new ArgumentException(
                                SR.GetString(SR.SFxTooManyPartsWithSameName, partName)
                            )
                        );
                    i++;
                }
            ...
            EnsureXsdImport(
                IsNullOrEmpty(elementName) ? typeName.Namespace : elementName.Namespace,
        ...
        protected void ExportGlobalElement(
            string elementName,
            string elementNs,
            bool isNillable,
            XmlQualifiedName typeName,
...
#if DEBUG
            Fx.Assert(
                NamingHelper.IsValidNCName(elementName),
                "Name value has to be a valid NCName."
            );
            if (xsdType == null)
                Fx.Assert(
                    NamingHelper.IsValidNCName(typeName.Name),
                    "Name value has to be a valid NCName."
                );
#endif
        ...
        protected XmlSchemaSequence ExportWrappedPart(
            WsdlNS.Message message,
            string elementName,
            string elementNs,
            XmlSchemaSet schemaSet,
        ...
        protected bool CreateMessage(
            MessageDescription message,
            int messageIndex,
            out WsdlNS.Message wsdlMessage
        )
        {
            wsdlMessage = null;
            ...
            if (
                ExportedMessages.WsdlMessages.ContainsKey(
                    new MessageDescriptionDictionaryKey(contractContext.Contract, message)
                )
            )
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    new ArgumentException(
                        SR.GetString(SR.MultipleCallsToExportContractWithSameContract)
                    )
                );
            ...
            {
                typedMessageKey = new TypedMessageKey(
                    message.MessageType,
            ...
            WsdlNS.OperationMessage wsdlOperationMessage = contractContext.GetOperationMessage(
                message
            );
            wsdlOperationMessage.Message = new XmlQualifiedName(
                wsdlMessage.Name,
                wsdlMessage.ServiceDescription.TargetNamespace
            );
            this.ExportedMessages.WsdlMessages.Add(
                new MessageDescriptionDictionaryKey(contractContext.Contract, message),
                wsdlMessage
            );
        ...
        protected bool CreateHeaderMessage(
            MessageDescription message,
            out WsdlNS.Message wsdlMessage
        )
        {
            ...
            if (
                ExportedMessages.WsdlHeaderMessages.ContainsKey(
                    new MessageDescriptionDictionaryKey(contractContext.Contract, message)
                )
            )
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
            ...
            if (message.IsTypedMessage)
            {
                ...
                if (
                    ExportedMessages.TypedHeaderMessages.TryGetValue(
                        typedMessageKey,
                        out wsdlMessage
                    )
                )
                {
                    this.ExportedMessages.WsdlHeaderMessages.Add(
                        new MessageDescriptionDictionaryKey(contractContext.Contract, message),
                        wsdlMessage
                    );
                    return false;
        ...
        {
            string messageNameBase = XmlName.IsNullOrEmpty(messageDescription.MessageName)
                ? null
            ...
            if (string.IsNullOrEmpty(messageNameBase))
            {
                ...
                if (messageDescription.Direction == MessageDirection.Input)
                    messageNameBase = string.Format(
                        System.Globalization.CultureInfo.InvariantCulture,
                        "{0}_{1}_Input{2}Message",
                        portTypeName,
                        operationName,
                        callbackString
                    );
        ...
        {
            WsdlNS.Message wsdlBodyMessage = this.ExportedMessages.WsdlMessages[
                new MessageDescriptionDictionaryKey(
                    this.contractContext.Contract,
                    messageDescription
                )
            ];

            string messageNameBase = string.Format(
                System.Globalization.CultureInfo.InvariantCulture,
                "{0}_Headers",
                wsdlBodyMessage.Name
            );
        ...
        protected string GetFaultMessageName(string faultName)
        {
            ...
            string operationName = contractContext.GetOperation(operation).Name;
            // Microsoft: composing names have potential problem of generating name that looks like an encoded name, consider avoiding '_'
            string faultNameBase = String.Format(
                System.Globalization.CultureInfo.InvariantCulture,
                "{0}_{1}_{2}_FaultMessage",
                portTypeName,
                operationName,
                faultName
            );
        ...
        protected void ExportMessagePart(
            WsdlNS.Message message,
            ...
            string wrapperNs,
            XmlSchemaSequence wrapperSequence,
            XmlSchemaSet schemaSet
        )
        {
            ...
            if (generateElement)
            {
                ...
                else
                {
                    ...
                    wsdlPart = AddMessagePart(
                        message,
                        partName,
                        new XmlQualifiedName(elementName, part.Namespace),
                        XmlQualifiedName.Empty
                    );
                }
            }
            else
            {
                if (String.IsNullOrEmpty(typeName.Name))
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidOperationException(
                            SR.GetString(SR.SFxAnonymousTypeNotSupported, message.Name, partName)
                        )
                    );
        ...
        class MessageBindingExporter
        {
            ...
            internal MessageBindingExporter(
                WsdlExporter exporter,
                WsdlEndpointConversionContext endpointContext
            )
            {
                this.endpointContext = endpointContext;
                this.exportedMessages = (MessageExportContext)
                    exporter.State[typeof(MessageExportContext)];
                this.soapVersion = SoapHelper.GetSoapVersion(endpointContext.WsdlBinding);
                this.exporter = exporter;
            }

            internal void ExportMessageBinding(
                OperationDescription operation,
                Type messageContractExporterType
            )
            {
                ...
                WsdlNS.SoapOperationBinding soapOperationBinding =
                    SoapHelper.GetOrCreateSoapOperationBinding(
                        endpointContext,
                        operation,
                        exporter
                    );
                ...
                soapOperationBinding.Style = isRpc
                    ? WsdlNS.SoapBindingStyle.Rpc
                    : WsdlNS.SoapBindingStyle.Document;
                if (isRpc)
                {
                    WsdlNS.SoapBinding soapBinding = (WsdlNS.SoapBinding)
                        endpointContext.WsdlBinding.Extensions.Find(typeof(WsdlNS.SoapBinding));
                ...
                foreach (MessageDescription message in operation.Messages)
                {
                    WsdlNS.MessageBinding wsdlMessageBinding = endpointContext.GetMessageBinding(
                        message
                    );

                    WsdlNS.Message headerMessage;
                    if (
                        this.exportedMessages.WsdlHeaderMessages.TryGetValue(
                            new MessageDescriptionDictionaryKey(
                                this.endpointContext.Endpoint.Contract,
                                message
                            ),
                            out headerMessage
                        )
                    ...
                    {
                        XmlQualifiedName wsdlHeaderMessageName = new XmlQualifiedName(
                            headerMessage.Name,
            ...
            {
                SoapHelper.CreateSoapFaultBinding(
                    fault.Name,
                    endpointContext,
                    endpointContext.GetFaultBinding(fault),
                    isEncoded
                );
            }

            void ExportMessageBodyBinding(
                MessageDescription messageDescription,
                bool isRpc,
                bool isEncoded,
                WsdlNS.MessageBinding messageBinding
            )
            {
                WsdlNS.SoapBodyBinding bodyBinding = SoapHelper.GetOrCreateSoapBodyBinding(
                    endpointContext,
                    messageBinding,
                    exporter
                );
                ...
                if (isRpc)
                {
                    string ns;
                    if (
                        !ExportedMessages.WrapperNamespaces.TryGetValue(
                            new MessageDescriptionDictionaryKey(
                                endpointContext.ContractConversionContext.Contract,
                                messageDescription
                            ),
                            out ns
                        )
            ...
            void ExportMessageHeaderBinding(
                MessageHeaderDescription header,
                XmlQualifiedName messageName,
                bool isEncoded,
                WsdlNS.MessageBinding messageBinding
            )
            {
#if DEBUG
                Fx.Assert(
                    NamingHelper.IsValidNCName(messageName.Name),
                    "Name value has to be a valid NCName."
                );
#endif
                WsdlNS.SoapHeaderBinding headerBinding = SoapHelper.CreateSoapHeaderBinding(
                    endpointContext,
                    messageBinding
                );
                headerBinding.Part = string.IsNullOrEmpty(header.UniquePartName)
                    ? header.Name
                    : header.UniquePartName;
                headerBinding.Message = messageName;
                headerBinding.Use = isEncoded
                    ? WsdlNS.SoapBindingUse.Encoded
                    : WsdlNS.SoapBindingUse.Literal;
                if (isEncoded)
                    headerBinding.Encoding = XmlSerializerOperationFormatter.GetEncoding(
                        soapVersion
                    );
            }

            static bool GetStyleAndUse(
                OperationDescription operation,
                Type messageContractExporterType,
                out bool isRpc,
                out bool isEncoded
            )
            {
                ...
                if (
                    messageContractExporterType
                        == typeof(DataContractSerializerMessageContractExporter)
                    || messageContractExporterType == null
                )
                {
                    DataContractSerializerOperationBehavior dataContractSerializerBehavior =
                        operation.Behaviors.Find<DataContractSerializerOperationBehavior>();
                    if (dataContractSerializerBehavior != null)
                    {
                        isRpc =
                            dataContractSerializerBehavior.DataContractFormatAttribute.Style
                ...
                if (
                    messageContractExporterType == typeof(XmlSerializerMessageContractExporter)
                    || messageContractExporterType == null
                )
                {
                    XmlSerializerOperationBehavior xmlSerializerBehavior =
                        operation.Behaviors.Find<XmlSerializerOperationBehavior>();
                    if (xmlSerializerBehavior != null)
                    {
                        isRpc =
                            xmlSerializerBehavior.XmlSerializerFormatAttribute.Style
                            == OperationFormatStyle.Rpc;
        ...
        protected sealed class MessageDescriptionDictionaryKey
        {
            ...
            public MessageDescriptionDictionaryKey(
            ...
            public override bool Equals(object obj)
            {
                ...
                if (
                    key != null
                    && key.MessageDescription == this.MessageDescription
        ...
        internal sealed class OperationMessageKey
        {
            ...
            public override bool Equals(object obj)
            {
                ...
                if (
                    key != null
                    && key.methodInfo == this.methodInfo
    ...
    {
        internal DataContractSerializerMessageContractExporter(
            WsdlExporter exporter,
            WsdlContractConversionContext context,
            OperationDescription operation,
            IOperationBehavior extension
        ...
        {
            return ((DataContractSerializerOperationBehavior)extension)
                ...
                if (CreateHeaderMessage(description, out wsdlMessage))
                {
                    foreach (MessageHeaderDescription header in description.Headers)
                    {
                        ...
                        ExportMessagePart(
                            ...
                            true /*isOptional*/
                            ,
                            IsTypeNullable(header.Type),
                            false /*IsOperationInherited(operation)*/
                            ,
                            true /*generateElement*/
                            ,
                            null /*wrapperNamespace*/
                            ,
                            null /*wrapperSequence*/
                            ,
                            SchemaSet
                        );
                    }
        ...
        {
            MessageDescription description = operation.Messages[messageIndex];
            WsdlNS.Message wsdlMessage = this.ExportedMessages.WsdlMessages[
                new MessageDescriptionDictionaryKey(this.contractContext.Contract, description)
            ];

            DataContractFormatAttribute dataContractFormatAttribute = (
                (DataContractSerializerOperationBehavior)extension
            ).DataContractFormatAttribute;
            XmlSchemaSequence wrapperSequence = null;
            ...
            if (dataContractFormatAttribute.Style == OperationFormatStyle.Document && isWrapped)
                wrapperSequence = ExportWrappedPart(
                    wsdlMessage,
                    description.Body.WrapperName,
                    description.Body.WrapperNamespace,
                    SchemaSet,
                    false /*isOperationInherited*/
                );
            XmlSchemaType xsdType;
            if (OperationFormatter.IsValidReturnValue(description.Body.ReturnValue))
            {
                ...
                Type dataContractType =
                    DataContractSerializerOperationFormatter.GetSubstituteDataContractType(
                        description.Body.ReturnValue.Type,
                        out isQueryable
                    );
                XmlQualifiedName typeName = ExportType(
                    dataContractType,
                    description.Body.ReturnValue.Name,
                    operation.Name,
                    out xsdType
                );
                ExportMessagePart(
                    ...
                    true /*isOptional*/
                    ,
                    IsTypeNullable(description.Body.ReturnValue.Type),
                    false /*isOperationInherited*/
                    ,
        ...
        protected override object GetExtensionData()
        {
            return new ExtensionData(
                ((DataContractSerializerOperationBehavior)extension).DataContractFormatAttribute
            );
        }
    ...
    {
        internal XmlSerializerMessageContractExporter(
            WsdlExporter exporter,
            WsdlContractConversionContext context,
            OperationDescription operation,
            IOperationBehavior extension
        ...
        protected override bool IsEncoded()
        {
            return ((XmlSerializerOperationBehavior)extension)
        ...
        protected override object OnExportMessageContract()
        {
            ...
            if (result == null)
            {
                ...
                XmlSerializerOperationBehavior serializerBehavior =
                    this.extension as XmlSerializerOperationBehavior;
        ...
        protected override void ExportHeaders(int messageIndex, object state)
        {
            ...
            if (description.Headers.Count > 0)
            {
                XmlSerializerOperationBehavior.Reflector.OperationReflector operationReflector =
                    (XmlSerializerOperationBehavior.Reflector.OperationReflector)state;
        ...
        protected override void ExportBody(int messageIndex, object state)
        {
            ...
            string portTypeName = contractContext.WsdlPortType.Name;
            string portTypeNs = contractContext.WsdlPortType.ServiceDescription.TargetNamespace;
            MessageDescriptionDictionaryKey key = new MessageDescriptionDictionaryKey(
                this.contractContext.Contract,
                description
            );
        ...
        {
            XmlQualifiedName elementName;
            XmlMembersMapping mapping = operationReflector.ImportFaultElement(
                fault,
                out elementName
            );
        ...
        {
            return new ExtensionData(
                ((XmlSerializerOperationBehavior)this.extension).XmlSerializerFormatAttribute
            );
        }
        ...
        )
        {
            ...
            if (isDocWrapped)
            {
                ...
                AddMessagePart(
                    message,
                    "parameters",
                    new XmlQualifiedName(membersMapping.XsdElementName, membersMapping.Namespace),
                    XmlQualifiedName.Empty
                );
                return;
            }
            ...
            for (int i = 0; i < membersMapping.Count; i++)
            {
                ...
                string partName =
                    (isHeader || generateElement)
                        ? NamingHelper.XmlName(member.MemberName)
                        : member.XsdElementName;
                if (generateElement)
                    AddMessagePart(
                        message,
                        partName,
                        new XmlQualifiedName(member.XsdElementName, member.Namespace),
                        XmlQualifiedName.Empty
                    );
                else
                {
                    if (string.IsNullOrEmpty(member.TypeName))
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(
                                SR.GetString(
                                    SR.SFxAnonymousTypeNotSupported,
                                    message.Name,
                                    partName
                                )
                            )
        ...
        XmlSerializerOperationBehavior.Reflector Reflector
        {
            get
            {
                ...
                if (
                    !exporter.State.TryGetValue(
                        typeof(XmlSerializerOperationBehavior.Reflector),
                        out reflector
                    )
                )
                {
                    reflector = new XmlSerializerOperationBehavior.Reflector(
                        contractContext.Contract.Namespace,
                        contractContext.Contract.ContractType
                    );
