...
namespace System.ServiceModel.Description
{
    ...
    class MessageContractImporter
    {
        ...
        )
        {
            ...
            foreach (
                WsdlNS.OperationBinding wsdlOperationBinding in endpointContext.WsdlBinding
                    .Operations
            )
            {
                OperationDescription operation = endpointContext.GetOperationDescription(
                ...
                if (isReferencedContract || OperationHasBeenHandled(operation))
                {
                    ...
                    foreach (
                        WsdlNS.MessageBinding wsdlMessageBinding in wsdlOperationBinding.Faults
                    )
                    {
        ...
        static void MarkSoapExtensionsAsHandled(WsdlNS.NamedItem item)
        {
            ...
            {
                WsdlNS.ServiceDescriptionFormatExtension ext =
                    o as WsdlNS.ServiceDescriptionFormatExtension;
        ...
        {
            new MessageContractImporter(
                importer,
                contractContext,
                schemaImporter
            ).ImportMessageContract();
        }
        ...
        void TraceImportInformation(OperationDescription operation)
        {
            if (DiagnosticUtility.ShouldTraceInformation)
            {
                ...
                TraceUtility.TraceEvent(
                    ...
                    TraceCode.CannotBeImportedInCurrentFormat,
                    SR.GetString(SR.TraceCodeCannotBeImportedInCurrentFormat),
                    new DictionaryTraceRecord(ht),
                    null,
                    null
                );
            }
        ...
        bool CanImportOperation(OperationDescription operation, out OperationInfo operationInfo)
        {
            ...
            Collection<WsdlNS.OperationBinding> wsdlOperationBindings =
                contractContext.GetOperationBindings(wsdlOperation);

            return CanImportOperation(
                    operation,
                    wsdlOperation,
        ...
        bool CanImportFault(WsdlNS.OperationFault fault, OperationDescription description)
        {
            ...
            if (
                !ValidateFault(
                    ...
                    description,
                    out detailElement,
                    out detailElementTypeName,
                    out detailElementQname
                )
            )
                return false;
        ...
        void ImportOperationContract(OperationDescription operation, OperationInfo operationInfo)
        {
            ...
            foreach (WsdlNS.OperationMessage operationMessage in wsdlOperation.Messages)
            {
                ImportMessage(
                    ...
                    operationInfo.AreAllMessagesWrapped
                );
                isReply = true;
            }
            ...
            this.CurrentSchemaImporter
                .SetOperationSupportFaults(operation, this.faultImportOptions.UseMessageFormat);
            ...
            foreach (WsdlNS.OperationBinding wsdlOperationBinding in wsdlOperationBindings)
            {
                foreach (MessageDescription message in operation.Messages)
                {
                    WsdlNS.OperationMessage wsdlOperationMessage =
                        contractContext.GetOperationMessage(message);
                    WsdlNS.ServiceDescriptionCollection wsdlDocuments =
                        wsdlOperationMessage.Operation
                            .PortType
                            .ServiceDescription
                            .ServiceDescriptions;
                    WsdlNS.Message wsdlMessage = wsdlDocuments.GetMessage(
                        wsdlOperationMessage.Message
                    );

                    WsdlNS.MessageBinding messageBinding =
                        (message.Direction == MessageDirection.Input)
                            ? (WsdlNS.MessageBinding)wsdlOperationBinding.Input
                            : (WsdlNS.MessageBinding)wsdlOperationBinding.Output;

                    if (messageBinding != null)
                        ImportMessageBinding(
                            messageBinding,
                            wsdlMessage,
                            message,
                            operationInfo.Style,
        ...
        bool CanImportOperation(
            OperationDescription operation,
            WsdlNS.Operation wsdlOperation,
            Collection<WsdlNS.OperationBinding> operationBindings,
        ...
        )
        {
            ...
            foreach (WsdlNS.OperationBinding operationBinding in operationBindings)
            {
                ...
                foreach (MessageDescription message in operation.Messages)
                {
                    WsdlNS.OperationMessage operationMessage = contractContext.GetOperationMessage(
                        message
                    );
                    ...
                    if (wsdlMessage != null)
                    {
                        ...
                        if (messageBinding != null)
                        {
                            ...
                            if (
                                !CanImportMessageBinding(
                                    messageBinding,
                                    wsdlMessage,
                                    operationStyle,
                                    out isMessageEncoded
                                )
                            )
                                return false;
                            ...
                            if (isOperationEncoded == null)
                                ...
                                AddError(
                                    SR.GetString(
                                        SR.SFxInconsistentWsdlOperationUseInBindingMessages,
                ...
                foreach (WsdlNS.FaultBinding faultBinding in operationBinding.Faults)
                {
                    ...
                    if (isOperationEncoded == null)
                        ...
                        AddError(
                            SR.GetString(
                                SR.SFxInconsistentWsdlOperationUseInBindingFaults,
                ...
                {
                    StyleAndUse operationStyleAndUse = GetStyleAndUse(
                        operationStyle,
                        isOperationEncoded.Value
                    );
                    if (operationStyleAndUse != styleAndUse)
                    {
                        AddError(
                            SR.GetString(
                                SR.SFxInconsistentWsdlOperationUseAndStyleInBinding,
                                operation.Name,
                                operationBinding.Binding.Name,
                                GetUse(operationStyleAndUse),
                                GetStyle(operationStyleAndUse),
                                prevOperationBinding.Binding.Name,
                                GetUse(styleAndUse.Value),
                                GetStyle(styleAndUse.Value)
                            )
                        );
                    }
                    if (operationStyleAndUse < styleAndUse)
            ...
            foreach (WsdlNS.OperationMessage wsdlOperationMessage in wsdlOperation.Messages)
            {
                ...
                if (
                    !CanImportMessage(
                        wsdlMessage,
                        wsdlOperationMessage.Name,
                        out inferredMessageStyle,
                        ref areAllMessagesWrapped
                    )
                )
                    return false;
            ...
            {
                if (
                    inferredOperationStyle.Value != style
                    && inferredOperationStyle.Value == OperationFormatStyle.Document
                )
                    AddError(
                        SR.GetString(
                            SR.SFxInconsistentWsdlOperationStyleInOperationMessages,
                            operation.Name,
                            inferredOperationStyle,
                            style
                        )
                    );
            }
        ...
        bool CanImportMessage(
            WsdlNS.Message wsdlMessage,
            string operationName,
            out OperationFormatStyle? inferredStyle,
            ref bool areAllMessagesWrapped
        )
        {
            ...
            foreach (WsdlNS.MessagePart part in messageParts)
            {
                ...
                if (inferredStyle == null)
                    ...
                    AddError(
                        SR.GetString(
                            SR.SFxInconsistentWsdlOperationStyleInMessageParts,
                            operationName
                        )
                    );
            }
        ...
        {
            MessageDescription messageDescription = contractContext.GetMessageDescription(
                wsdlOperationMessage
            );
            OperationDescription operation = contractContext.GetOperationDescription(
                wsdlOperationMessage.Operation
            );
            ...
            if (wsdlMessage.Parts.Count == 1)
            {
                ...
                if (
                    TryImportStream(
                        ...
                        messageDescription,
                        isReply,
                        areAllMessagesWrapped
                    )
                )
                    return;
                if (
                    areAllMessagesWrapped
                    && TryImportWrappedMessage(
                        messageDescription,
            ...
            foreach (WsdlNS.MessagePart part in messageParts)
            {
                ...
                MessagePartDescription partDesc = CurrentSchemaImporter.ImportMessagePart(
                    part,
                    false /*isHeader*/
                    ,
                    isEncoded
        ...
        {
            return (styleAndUse == StyleAndUse.RpcLiteral || styleAndUse == StyleAndUse.RpcEncoded)
                ? "rpc"
                : "document";
        }
        ...
        {
            return (
                styleAndUse == StyleAndUse.RpcEncoded || styleAndUse == StyleAndUse.DocumentEncoded
            )
                ? "encoded"
                : "literal";
        }
        ...
        static void SetWrapperName(OperationDescription operation)
        {
            ...
            if (messages != null && messages.Count > 0)
            {
                ...
                if (messages.Count > 1)
                {
                    ...
                    if (response != null)
                    {
                        response.Body.WrapperName = TypeLoader
                            .GetBodyWrapperResponseName(operation.Name)
                            .EncodedName;
        ...
        )
        {
            ...
            if (IsNullOrEmpty(detailElementTypeName))
                detailElementTypeRef = faultImporter.ImportFaultElement(
                    detailElementQname,
                    detailElement,
                    isEncoded
                );
            else
                detailElementTypeRef = faultImporter.ImportFaultType(
                    detailElementQname,
                    detailElementTypeName,
                    isEncoded
                );
            ...
            faultDescription.ElementName = new XmlName(
                detailElementQname.Name,
                true /*isEncoded*/
            );
        ...
        bool ValidateFault(
            WsdlNS.OperationFault fault,
            OperationDescription description,
            out XmlSchemaElement detailElement,
            out XmlQualifiedName detailElementTypeName,
            out XmlQualifiedName detailElementQname
        )
        {
            detailElement = null;
            ...
            {
                TraceFaultCannotBeImported(
                    fault.Name,
                    description.Name,
                    SR.GetString(
                        SR.SFxWsdlOperationFaultNeedsMessageAttribute2,
                        fault.Name,
            ...
            if (faultMessage.Parts.Count != 1)
            {
                TraceFaultCannotBeImported(
                    fault.Name,
                    description.Name,
                    SR.GetString(SR.UnsupportedWSDLOnlyOneMessage)
                );
                description.Faults.Remove(contractContext.GetFaultDescription(fault));
                return false;
            }
            ...
            if (IsNullOrEmpty(detailElementQname) || !IsNullOrEmpty(faultMessageDetail.Type))
            {
                TraceFaultCannotBeImported(
                    fault.Name,
                    description.Name,
                    SR.GetString(SR.UnsupportedWSDLTheFault)
                );
                description.Faults.Remove(contractContext.GetFaultDescription(fault));
                return false;
            }
        ...
        bool TryImportAnyMessage(
            WsdlNS.MessagePart part,
            MessageDescription description,
            bool isReply
        )
        {
            return CheckAndAddPart(
                part.Type,
                DataContractSerializerMessageContractImporter.GenericMessageTypeName,
                part.Name,
                string.Empty,
        ...
        bool CanImportStream(
            WsdlNS.MessagePart part,
            out OperationFormatStyle? style,
            ref bool areAllMessagesWrapped
        )
        {
            ...
            {
                XmlSchemaComplexType complexType = GetElementComplexType(
                    part.Element,
                    allSchemas,
                    out ns,
                    out elementFormDefault
                );
                if (complexType != null)
                {
                    ...
                    if (
                        ...
                        && rootSequence.Items[0] is XmlSchemaElement
                    )
                        return CheckPart(
                            ((XmlSchemaElement)rootSequence.Items[0]).SchemaTypeName,
        ...
        )
        {
            ...
            if (areAllMessagesWrapped && IsWrapperPart(part))
            {
                XmlSchemaSequence rootSequence = GetRootSequence(
                    GetElementComplexType(part.Element, allSchemas, out ns, out elementFormDefault)
                );
                if (
                    rootSequence != null
                    && rootSequence.Items.Count == 1
                ...
                )
                {
                    ...
                    description.Body.WrapperName = new XmlName(
                        part.Element.Name,
                        true /*isEncoded*/
                    ).EncodedName;
                    ...
                    else
                    {
                        return CheckAndAddPart(
                            ...
                            DataContractSerializerMessageContractImporter.StreamBodyTypeName,
                            element.Name,
                            GetLocalElementNamespace(ns, element, elementFormDefault),
                            typeof(Stream),
                            description,
                            isReply
                        );
        ...
        {
            return (IsWrapperPart(wsdlPart))
                ? CurrentSchemaImporter.CanImportWrapperElement(wsdlPart.Element)
                : false;
        }

        bool TryImportWrappedMessage(
        ...
        )
        {
            ...
            if (CanImportWrappedMessage(wsdlPart))
            {
                ...
                MessagePartDescription[] parts = CurrentSchemaImporter.ImportWrapperElement(
                    elementName
                );
                if (parts == null)
                    return false;
                messageDescription.Body.WrapperName = new XmlName(
                    elementName.Name,
                    true /*isEncoded*/
                ).EncodedName;
                ...
                if (parts.Length > 0)
                {
                    ...
                    if (
                        ...
                        && !CheckIsRef(requestMessage, parts[0])
                    )
                    {
        ...
        static XmlSchemaElement FindSchemaElement(
            XmlSchemaSet schemaSet,
            XmlQualifiedName elementName,
            out XmlSchema containingSchema
        )
        {
        ...
        static void CollectEncodedAndLiteralSchemas(
            WsdlNS.ServiceDescriptionCollection serviceDescriptions,
            XmlSchemas encodedSchemas,
            XmlSchemas literalSchemas,
            XmlSchemaSet allSchemas
        )
        {
                    ...
                    foreach (WsdlNS.MessagePart part in message.Parts)
                    {
                        ...
                        if (part.Element != null && !part.Element.IsEmpty)
                        {
                            ...
                            if (element != null)
                            {
                                ...
                                {
                                    XmlSchemaType type = FindSchemaType(
                                        allSchemas,
                                        element.SchemaTypeName
                                    );
                                    if (type != null)
        ...
        )
        {
            ...
            foreach (WsdlNS.Binding binding in description.Bindings)
            {
                if (
                    operation != null
                    && !new XmlQualifiedName(operation.PortType.Name, targetNamespace).Equals(
                        binding.Type
                    )
                )
                    continue;
                foreach (WsdlNS.OperationBinding bindingOperation in binding.Operations)
                {
                    if (bindingOperation.Input != null)
                        foreach (object extension in bindingOperation.Input.Extensions)
                        {
                            ...
                            {
                                WsdlNS.SoapHeaderBinding header =
                                    extension as WsdlNS.SoapHeaderBinding;
                    ...
                    if (bindingOperation.Output != null)
                        foreach (object extension in bindingOperation.Output.Extensions)
                        {
                            if (operation != null)
                            {
                                ...
                                {
                                    WsdlNS.SoapBodyBinding body =
                                        extension as WsdlNS.SoapBodyBinding;
                                    if (body != null)
                                    {
                                        if (body.Use == WsdlNS.SoapBindingUse.Encoded)
                                            isEncoded = true;
        ...
        static string GetLocalElementNamespace(
            string ns,
        ...
        )
        {
            XmlSchemaForm elementForm =
                (element.Form != XmlSchemaForm.None) ? element.Form : elementFormDefault;
        ...
        {
            AddWarning(
                SR.GetString(SR.SFxFaultCannotBeImported, faultName, operationName, message)
            );
        }

        static bool CheckAndAddPart(
            XmlQualifiedName typeNameFound,
            XmlQualifiedName typeNameRequired,
            string name,
            string ns,
            Type type,
            MessageDescription description,
        ...
        static XmlSchemaComplexType GetElementComplexType(
            XmlQualifiedName elementName,
            XmlSchemaSet schemaSet,
            out string ns,
            out XmlSchemaForm elementFormDefault
        )
        {
            XmlSchema schema;
        ...
        )
        {
            ...
            foreach (object extension in messageBinding.Extensions)
            {
                ...
                if (soapHeaderBinding != null)
                {
                    ...
                    if (
                        !CanImportMessageHeaderBinding(
                            soapHeaderBinding,
                            wsdlMessage,
                            style,
                            out currentIsEncoded
                        )
                    )
                        return false;
                    if (isMessageEncoded == null)
                        ...
                        AddError(
                            SR.GetString(
                                SR.SFxInconsistentWsdlOperationUseInBindingExtensions,
                                messageBinding.OperationBinding.Name,
                                messageBinding.OperationBinding.Binding.Name
                            )
                        );
                    ...
                    if (soapBodyBinding != null)
                    {
                        ...
                        foreach (string partName in messageParts)
                        {
                            ...
                            else if (!bodyPartsFromBindings.Contains(partName))
                            {
                                AddError(
                                    SR.GetString(
                                        SR.SFxInconsistentBindingBodyParts,
                                        messageBinding.OperationBinding.Name,
                                        messageBinding.OperationBinding.Binding.Name,
                                        partName
                                    )
        ...
        private bool CanImportFaultBinding(
            WsdlNS.FaultBinding faultBinding,
            OperationFormatStyle style,
            out bool isFaultEncoded
        )
        {
            bool? isEncoded = null;
            bool currentIsEncoded;
        ...
        bool CanImportMessageBodyBinding(
            WsdlNS.SoapBodyBinding bodyBinding,
            OperationFormatStyle style,
            out bool isEncoded
        )
        {
        ...
        bool CanImportMessageHeaderBinding(
        ...
        )
        {
            ...
            WsdlNS.Message wsdlHeaderMessage = wsdlMessage.ServiceDescription
                .ServiceDescriptions
            ...
            if (headerStyle != style)
                AddError(
                    SR.GetString(
                        SR.SFxInconsistentWsdlOperationStyleInHeader,
                        part.Name,
                        headerStyle,
                        style
                    )
        ...
        void ImportMessageHeaderBinding(
            WsdlNS.SoapHeaderBinding headerBinding,
            WsdlNS.Message wsdlMessage,
            MessageDescription description,
            OperationFormatStyle style,
        ...
        internal abstract class SchemaImporter
        {
            ...
            internal MessagePartDescription ImportMessagePart(
                WsdlNS.MessagePart part,
                bool isHeader,
                bool isEncoded
            )
                ...
                if (!IsNullOrEmpty(part.Type))
                {
                    bodyPart = isHeader
                        ? (MessagePartDescription)
                            new MessageHeaderDescription(part.Name, String.Empty)
                        : new MessagePartDescription(part.Name, String.Empty);
            ...
            {
                return ImportParameterElement(
                    FindSchemaElement(this.schemaSet, elementName),
                    elementName.Namespace,
                    isHeader,
                    isMultiple
                );
            ...
            )
            {
                ...
                MessagePartDescription part = isHeader
                    ? (MessagePartDescription)new MessageHeaderDescription(element.Name, ns)
                    : new MessagePartDescription(element.Name, ns);
                part.Multiple = isMultiple;
                part.BaseType = ImportElement(
                    part,
                    element,
                    false /*isEncoded*/
                );
            ...
            internal abstract MessagePartDescription[] ImportWrapperElement(
                XmlQualifiedName elementName
            );
            internal abstract void SetOperationStyle(
                OperationDescription operation,
                OperationFormatStyle style
            );
            internal abstract bool GetOperationIsEncoded(OperationDescription operation);
            internal abstract void SetOperationIsEncoded(
                OperationDescription operation,
                bool isEncoded
            );
        ...
        internal class DataContractSerializerSchemaImporter : SchemaImporter
        {
            ...
            internal const String FailedReferenceTypeExceptionKey =
                "System.Runtime.Serialization.FailedReferenceType";
                ...
                : base(importer)
            {
                DataContractSerializerOperationGenerator =
                    new DataContractSerializerOperationGenerator(
                        DataContractImporter.CodeCompileUnit
                    );
            }
            ...
            XsdDataContractImporter DataContractImporter
            {
                get
                {
                    ...
                    )
                    {
                        ...
                        dataContractImporter = new XsdDataContractImporter(
            ...
            {
                DataContractSerializerSchemaImporter faultImporter =
                    DataContractSerializerSchemaImporter.Get(importer);
                if (IsNullOrEmpty(detailElementTypeName))
            ...
            internal override MessagePartDescription[] ImportWrapperElement(
                XmlQualifiedName elementName
            )
            {
                string ns;
                ...
                MessagePartDescription[] parts = new MessagePartDescription[
                ...
                for (int i = 0; i < rootSequence.Items.Count; i++)
                {
                    ...
                    if (localElement == null)
                        return null;
                    parts[i] = ImportParameterElement(
                        localElement,
                        GetLocalElementNamespace(ns, localElement, elementFormDefault),
                        false /*isHeader*/
                        ,
                        false /*isMultiple*/
                    );
            ...
            )
            {
                ...
                ICollection<CodeTypeReference> knownTypeRefs =
                    DataContractImporter.GetKnownTypeReferences(typeName);
                DataContractSerializerOperationGenerator.Add(
                    part,
                    typeRef,
                    knownTypeRefs,
                    false /*IsNonNillableReferenceType*/
                );
            ...
            internal static bool TryGetFailedReferenceType(
                Exception ex,
                out Type failedReferenceType
            )
            {
                if (null == ex)
            ...
            )
            {
                ...
                CodeTypeReference typeRef = DataContractImporter.GetCodeTypeReference(
                    typeName,
                    element
                );
                ICollection<CodeTypeReference> knownTypeRefs =
                ...
                DataContractSerializerOperationGenerator.Add(
                    ...
                    typeRef,
                    knownTypeRefs,
                    !element.IsNillable && !IsValueType(typeName)
                );
                if (typeRef.ArrayRank == 0)
                    return typeRef.BaseType;
                else
            ...
            internal override CodeTypeReference ImportFaultElement(
                XmlQualifiedName elementName,
                XmlSchemaElement element,
                bool isEncoded
            )
            ...
            internal override CodeTypeReference ImportFaultType(
                XmlQualifiedName elementName,
                XmlQualifiedName typeName,
                bool isEncoded
            )
            ...
            internal override void PreprocessSchema()
            {
                ...
                compileValidationEventHandler = new ValidationEventHandler(
            ...
            {
                DataContractSerializerOperationBehavior operationBehavior = operation.Behaviors
                    .Find<DataContractSerializerOperationBehavior>();
                if (operationBehavior == null)
                {
                    operationBehavior = new DataContractSerializerOperationBehavior(
                        operation,
                        new DataContractFormatAttribute()
                    );
            ...
            internal override void SetOperationIsEncoded(
                OperationDescription operation,
                bool isEncoded
            )
            {
        ...
        internal class XmlSerializerSchemaImporter : SchemaImporter
        {
                ...
                : base(importer)
            {
                ...
                {
                    options = (XmlSerializerImportOptions)
                        importer.State[typeof(XmlSerializerImportOptions)];
                }
                else
                {
                    object compileUnit;
                ...
                CollectEncodedAndLiteralSchemas(
                    importer.WsdlDocuments,
                    encodedSchemas,
                    literalSchemas,
                    schemaSet
                );
                ...
                lock (schemaImporterLock)
                {
                    xmlImporter = new XmlSchemaImporter(
                        ...
                        new ImportContext(codeIdentifiers, false)
                    );
                }
            ...
            )
            {
                ...
                for (int i = 0; i < membersMapping.Count; i++)
                {
                    ...
                    MessagePartDescription part = new MessagePartDescription(
                        xmlName,
                        member.Namespace == null ? string.Empty : member.Namespace
                    );
                    xmlSerializerOperationGenerator.Add(
                        part,
                        member,
                        membersMapping,
                        false /*isEncoded*/
            ...
            {
                XmlName memberName = new XmlName(
                    elementName.Name,
                    true /*isEncoded*/
                );
            ...
            {
                XmlSerializerOperationBehavior operationBehavior = operation.Behaviors
            ...
            {
                XmlSerializerFormatAttribute operationAttribute = GetFormatAttribute(
                    operation,
                    true /*createNew*/
                );
        ...
        class OperationInfo
        {
            ...
            {
                get { return areAllMessagesWrapped; }
            }
        }
    }
    ...
    static class ValidWsdl
    {
        internal static bool Check(
            WsdlNS.SoapHeaderBinding soapHeaderBinding,
            WsdlNS.MessageBinding messageBinding,
            WsdlWarningHandler warningHandler
        )
        {
            if (soapHeaderBinding.Message == null || soapHeaderBinding.Message.IsEmpty)
            {
                ...
                string warning = SR.GetString(
                    SR.IgnoreSoapHeaderBinding3,
                    messageBinding.OperationBinding.Name,
        ...
        internal static bool Check(
            WsdlNS.SoapFaultBinding soapFaultBinding,
            WsdlNS.FaultBinding faultBinding,
            WsdlWarningHandler warningHandler
        )
        {
