...
namespace System.ServiceModel.Description
{
    ...
    {
        readonly Dictionary<WsdlNS.NamedItem, WsdlImportException> importErrors =
            new Dictionary<WsdlNS.NamedItem, WsdlImportException>();
        bool isFaulted = false;

        readonly Dictionary<XmlQualifiedName, WsdlContractConversionContext> importedPortTypes =
            new Dictionary<XmlQualifiedName, WsdlContractConversionContext>();
        readonly Dictionary<XmlQualifiedName, WsdlEndpointConversionContext> importedBindings =
            new Dictionary<XmlQualifiedName, WsdlEndpointConversionContext>();
        readonly Dictionary<WsdlNS.Port, ServiceEndpoint> importedPorts =
            new Dictionary<WsdlNS.Port, ServiceEndpoint>();

        readonly KeyedByTypeCollection<IWsdlImportExtension> wsdlExtensions;

        readonly WsdlNS.ServiceDescriptionCollection wsdlDocuments =
            new WsdlNS.ServiceDescriptionCollection();
        readonly XmlSchemaSet xmlSchemas = WsdlExporter.GetEmptySchemaSet();
        readonly Dictionary<string, XmlElement> policyDocuments =
            ...
            : base(policyImportExtensions, quotas)
        {
            ...
            this.wsdlExtensions = new KeyedByTypeCollection<IWsdlImportExtension>(
                wsdlImportExtensions
            );
        ...
        internal override XmlElement ResolvePolicyReference(
        ...
        public override ServiceEndpointCollection ImportAllEndpoints()
        {
            ...
            foreach (WsdlNS.Port wsdlPort in this.GetAllPorts())
            {
                ...
                ServiceEndpoint endpoint = ImportWsdlPort(
                    wsdlPort,
                    ErrorBehavior.DoNotThrowExceptions
                );
                if (endpoint != null)
        ...
        public Collection<Binding> ImportAllBindings()
        {
            ...
            foreach (WsdlNS.Binding wsdlBinding in this.GetAllBindings())
            {
                ...
                importedBindingContext = ImportWsdlBinding(
                    wsdlBinding,
                    ErrorBehavior.DoNotThrowExceptions
                );
        ...
        public ContractDescription ImportContract(WsdlNS.PortType wsdlPortType)
        {
            ...
            return ImportWsdlPortType(
                wsdlPortType,
                WsdlPortTypeImportOptions.ReuseExistingContracts,
                ErrorBehavior.RethrowExceptions
            );
        }
        ...
        ContractDescription ImportWsdlPortType(
            WsdlNS.PortType wsdlPortType,
            WsdlPortTypeImportOptions importOptions,
        ...
        )
        {
            ...
            if (
                importOptions == WsdlPortTypeImportOptions.IgnoreExistingContracts
                || !TryFindExistingContract(wsdlPortTypeQName, out contractDescription)
            )
            {
                EnsureBeforeImportCalled();
                ...
                try
                {
                    contractDescription = CreateContractDescription(
                        wsdlPortType,
                        wsdlPortTypeQName
                    );
                    WsdlContractConversionContext contractContext =
                        new WsdlContractConversionContext(contractDescription, wsdlPortType);

                    foreach (WsdlNS.Operation wsdlOperation in wsdlPortType.Operations)
                    {
                        OperationDescription operationDescription = CreateOperationDescription(
                            wsdlPortType,
                            wsdlOperation,
                            contractDescription
                        );
                        ...
                        foreach (
                            WsdlNS.OperationMessage wsdlOperationMessage in wsdlOperation.Messages
                        )
                        {
                            MessageDescription messageDescription;
                            if (
                                TryCreateMessageDescription(
                                    wsdlOperationMessage,
                                    operationDescription,
                                    out messageDescription
                                )
                            )
                                contractContext.AddMessage(
                                    messageDescription,
                                    wsdlOperationMessage
                                );
                        }
                        ...
                        foreach (WsdlNS.OperationFault wsdlOperationFault in wsdlOperation.Faults)
                        {
                            ...
                            if (
                                TryCreateFaultDescription(
                                    wsdlOperationFault,
                                    operationDescription,
                                    out faultDescription
                                )
                            )
...
#pragma warning suppress 56500 // covered by FxCOP
        ...
        WsdlEndpointConversionContext ImportWsdlBinding(
        ...
        )
        {
            ...
            if (!importedBindings.TryGetValue(wsdlBindingQName, out bindingEndpointContext))
            {
                ...
                try
                {
                    ...
                    ContractDescription contractDescription = GetOrImportContractDescription(
                        wsdlBinding.Type,
                    ...
                    ServiceEndpoint newWsdlBindingEndpoint = new ServiceEndpoint(
                        contractDescription
                    );
                    bindingEndpointContext = new WsdlEndpointConversionContext(
                        contractContext,
                    ...
                    foreach (WsdlNS.OperationBinding wsdlOperationBinding in wsdlBinding.Operations)
                        try
                        {
                            OperationDescription operation =
                                Binding2DescriptionHelper.FindOperationDescription(
                                    wsdlOperationBinding,
                                    wsdlDocuments,
                                    bindingEndpointContext
                                );
                            bindingEndpointContext.AddOperationBinding(
                                operation,
                                wsdlOperationBinding
                            );
                            ...
                            for (int i = 0; i < operation.Messages.Count; i++)
                            {
                                ...
                                WsdlNS.MessageBinding wsdlMessageBinding =
                                    Binding2DescriptionHelper.FindMessageBinding(
                                        wsdlOperationBinding,
                                        message
                                    );

                                bindingEndpointContext.AddMessageBinding(
                                    message,
                                    wsdlMessageBinding
                                );
                            }
                            ...
                            foreach (FaultDescription fault in operation.Faults)
                            {
                                WsdlNS.FaultBinding wsdlFaultBinding =
                                    Binding2DescriptionHelper.FindFaultBinding(
                                        wsdlOperationBinding,
                                        fault
                                    );
...
#pragma warning suppress 56500 // covered by FxCOP
                    ...
                    newWsdlBindingEndpoint.Binding = CreateBinding(
                        bindingEndpointContext,
                        bindingQName
                    );
        ...
        ServiceEndpoint ImportWsdlPort(WsdlNS.Port wsdlPort, ErrorBehavior errorBehavior)
        {
            ...
            if (!importedPorts.TryGetValue(wsdlPort, out endpoint))
            {
                ...
                try
                {
                    ...
                    WsdlEndpointConversionContext bindingEndpointContext = ImportWsdlBinding(
                        wsdlBinding,
                        ErrorBehavior.RethrowExceptions
                    );
...
#pragma warning suppress 56500 // covered by FxCOP
        ...
        static bool TryCreateMessageDescription(
        ...
        )
        {
            ...
            messageDescription.MessageName = WsdlNamingHelper.GetOperationMessageName(
                wsdlOperationMessage
            );
        ...
        static bool TryCreateFaultDescription(
            WsdlNS.OperationFault wsdlOperationFault,
            OperationDescription operationDescription,
            out FaultDescription faultDescription
        )
        {
            ...
            faultDescription.SetNameOnly(
                new XmlName(
                    wsdlOperationFault.Name,
                    true /*isEncoded*/
                )
            );
...
#pragma warning suppress 56500 // covered by FxCOP
        ...
        ContractDescription GetOrImportContractDescription(
            XmlQualifiedName wsdlPortTypeQName,
            out bool wasExistingContractDescription
        )
        {
            ContractDescription contractDescription;
            ...
            if (!TryFindExistingContract(wsdlPortTypeQName, out contractDescription))
            {
                ...
                contractDescription = ImportWsdlPortType(
                    wsdlPortType,
                    WsdlPortTypeImportOptions.IgnoreExistingContracts,
                    ErrorBehavior.RethrowExceptions
                );
...
#pragma warning suppress 56500 // covered by FxCOP
        ...
        bool TryFindExistingContract(
            XmlQualifiedName wsdlPortTypeQName,
            out ContractDescription existingContract
        )
        {
            XmlQualifiedName contractQName = WsdlNamingHelper.GetContractName(wsdlPortTypeQName);
...
#pragma warning suppress 56500 // covered by FxCOP
            ...
            foreach (object ext in item.Extensions)
            {
                ...
                if (item.Extensions.IsRequired(ext) || IsNonSoapWsdl11BindingExtension(ext))
                {
                    string errorMsg = SR.GetString(
                        SR.RequiredWSDLExtensionIgnored,
                        qName.Name,
                        qName.Namespace
                    );
                    WsdlImportException wie = WsdlImportException.Create(
                        item,
                ...
                else
                {
                    ...
                    string errorMsg = SR.GetString(
                        SR.OptionalWSDLExtensionIgnored,
                        qName.Name,
                        qName.Namespace,
                        xPath
                    );
        ...
        {
            if (
                ext is WsdlNS.HttpAddressBinding
            ...
            else if (extension is WsdlNS.ServiceDescriptionFormatExtension)
            {
                WsdlConfigNS.XmlFormatExtensionAttribute[] xfeAttributes =
                    (WsdlConfigNS.XmlFormatExtensionAttribute[])
                        ServiceReflector.GetCustomAttributes(
                            extension.GetType(),
                            typeof(WsdlConfigNS.XmlFormatExtensionAttribute),
                            false
                        );
                ...
                if (xfeAttributes.Length > 0)
                {
                    return new XmlQualifiedName(
                        xfeAttributes[0].ElementName,
                        xfeAttributes[0].Namespace
                    );
                }
            }
            WsdlImportException wie = WsdlImportException.Create(
                ...
                new InvalidOperationException(
                    SR.GetString(
                        SR.UnknownWSDLExtensionIgnored,
        ...
        {
            internal static OperationDescription FindOperationDescription(
            ...
            )
            {
                ...
                {
                    WsdlNS.Operation wsdlOperation = FindWsdlOperation(
                        wsdlOperationBinding,
                        wsdlDocuments
                    );
                    operation = endpointContext.ContractConversionContext
                        .GetOperationDescription(wsdlOperation);
                }
                else
                {
                    operation = FindOperationDescription(
            ...
            internal static WsdlNS.MessageBinding FindMessageBinding(
                WsdlNS.OperationBinding wsdlOperationBinding,
            ...
            internal static WsdlNS.FaultBinding FindFaultBinding(
                WsdlNS.OperationBinding wsdlOperationBinding,
            ...
            static WsdlNS.Operation FindWsdlOperation(
                WsdlNS.OperationBinding wsdlOperationBinding,
                WsdlNS.ServiceDescriptionCollection wsdlDocuments
            )
            {
                WsdlNS.PortType wsdlPortType = wsdlDocuments.GetPortType(
            ...
            static MatchResult MatchOperationParameterName(
                WsdlNS.MessageBinding wsdlMessageBinding,
                WsdlNS.OperationMessage wsdlOperationMessage
            )
            {
                ...
                string wsdlOperationMessageDecodedName = WsdlNamingHelper
                    .GetOperationMessageName(wsdlOperationMessage)
                    .DecodedName;
                if (
                    (wsdlOperationMessageName == null)
                    && (wsdlMessageBindingName == wsdlOperationMessageDecodedName)
                )
                {
                    return MatchResult.Partial;
                }
                else if (
                    (wsdlMessageBindingName == null)
                    && (wsdlOperationMessageName == wsdlOperationMessageDecodedName)
                )
                {
                    return MatchResult.Partial;
                }
            ...
            static OperationDescription FindOperationDescription(
                ContractDescription contract,
                WsdlNS.OperationBinding wsdlOperationBinding
            )
            {
            ...
            )
            {
                ...
                if (
                    FindMessage(operationDescription.Messages, MessageDirection.Output)
                ...
                // Either both have output message or neither have an output message;
                if (
                    FindMessage(operationDescription.Messages, MessageDirection.Input)
                    != (wsdlOperationBinding.Input != null)
                )
                    return false;
            ...
            private static bool FindMessage(
                MessageDescriptionCollection messageDescriptionCollection,
        ...
        internal static class WSAddressingHelper
        {
            internal static string GetWsaActionUri(WsdlNS.OperationMessage wsdlOperationMessage)
            {
                ...
                return (actionUri == null)
                    ? CreateDefaultWsaActionUri(wsdlOperationMessage)
                    : actionUri;
            }

            internal static string FindWsaActionAttribute(
                WsdlNS.OperationMessage wsdlOperationMessage
            )
            {
            ...
            static string CreateDefaultWsaActionUri(WsdlNS.OperationMessage wsdlOperationMessage)
            {
                ...
                string ns =
                    wsdlOperationMessage.Operation.PortType.ServiceDescription.TargetNamespace
                    ?? string.Empty;
                string portTypeName = wsdlOperationMessage.Operation.PortType.Name;
                XmlName operationMessageName = WsdlNamingHelper.GetOperationMessageName(
                    wsdlOperationMessage
                );
                ...
                string baseActionUri = ns.EndsWith(delimiter, StringComparison.OrdinalIgnoreCase)
                    ? ns
                    : ns + delimiter;

                string actionUri = string.Format(
            ...
            internal static EndpointAddress ImportAddress(WsdlNS.Port wsdlPort)
            {
                ...
                if (wsdlPort != null)
                {
                    XmlElement addressing10Element = wsdlPort.Extensions
                        .Find(AddressingStrings.EndpointReference, Addressing10Strings.Namespace);
                    XmlElement addressing200408Element = wsdlPort.Extensions
                        .Find(
                    ...
                    {
                        return EndpointAddress.ReadFrom(
            ...
            internal static AddressingVersion FindAddressingVersion(
            ...
            internal static SupportedAddressingMode DetermineSupportedAddressingMode(
                MetadataImporter importer,
                PolicyConversionContext context
            )
            {
                // Do not remove this assertion - the message encoding binding element importer owns it.
                XmlElement addressingAssertion = PolicyConversionContext.FindAssertion(
        ...
        static class WsdlNamingHelper
        {
            ...
            internal static XmlName GetOperationMessageName(
                WsdlNS.OperationMessage wsdlOperationMessage
            )
            {
        ...
        {
            internal static void SetInitiatingTerminating(
                OperationDescription operationDescription,
                WsdlNS.Operation wsdlOperation
            )
            {
                XmlAttribute isInitiating = FindAttribute(
                    wsdlOperation.ExtensibleAttributes,
                    WsdlExporter.NetSessionHelper.IsInitiating,
                ...
                XmlAttribute isTerminating = FindAttribute(
                    wsdlOperation.ExtensibleAttributes,
                    WsdlExporter.NetSessionHelper.IsTerminating,
                    WsdlExporter.NetSessionHelper.NamespaceUri
                );
            ...
            internal static void SetSession(
                ContractDescription contractDescription,
                WsdlNS.PortType wsdlPortType
            )
            {
                XmlAttribute usingSession = FindAttribute(
                    wsdlPortType.ExtensibleAttributes,
                    WsdlExporter.NetSessionHelper.UsingSession,
                    WsdlExporter.NetSessionHelper.NamespaceUri
                );
            ...
            static XmlAttribute FindAttribute(
        ...
        {
            public const string soapTransportUriKey =
                "TransportBindingElementImporter.TransportUri";
            ...
            )
            {
                ...
                if (policy != null)
                {
                    wsdlBindingQName = new XmlQualifiedName(
                        policy.Attributes[bindingAttrName].Value,
                        policy.Attributes[bindingAttrNamespace].Value
                    );
            ...
            static string AddPolicyUri(WsdlNS.Binding wsdlBinding, string name)
            {
                ...
                string id = string.Format(
                    ...
                    "{0}_{1}_BindingAdHocPolicy",
                    wsdlBinding.Name,
                    name
                );
                string newPolicyUris = string.Format(
                        System.Globalization.CultureInfo.InvariantCulture,
                        "#{0} {1}",
                        id,
                        policyUris
            ...
            static XmlElement CreatePolicyElement(
                string elementName,
                string value,
                XmlQualifiedName wsdlBindingQName
            )
            ...
            internal static void InsertAdHocTransportPolicy(
                WsdlNS.ServiceDescriptionCollection wsdlDocuments
            )
            {
        ...
        IEnumerable<WsdlNS.Binding> FindBindingsForContract(ContractDescription contract)
        {
            ...
            {
                if (
                    wsdlBinding.Type.Name == qName.Name
        ...
        IEnumerable<WsdlNS.Port> FindPortsForBinding(WsdlNS.Binding binding)
        {
            ...
            {
                if (
                    wsdlPort.Binding.Name == binding.Name
        ...
        internal static IEnumerable<MetadataSection> CreateMetadataDocuments(
            WsdlNS.ServiceDescriptionCollection wsdlDocuments,
            XmlSchemaSet xmlSchemas,
            IEnumerable<XmlElement> policyDocuments
        )
        {
        ...
        {
            PolicyAlternatives policyAlternatives = this.PolicyReader
                .GetPolicyAlternatives(endpointContext);
            IEnumerable<PolicyConversionContext> policyContexts =
                GetPolicyConversionContextEnumerator(
            ...
            foreach (PolicyConversionContext policyConversionContext in policyContexts)
            {
                ...
                {
                    AppendUnImportedPolicyErrorMessage(
                        ref unImportedPolicyMessage,
                        endpointContext,
...
#pragma warning suppress 56506
        ...
        static UnrecognizedAssertionsBindingElement CollectUnrecognizedAssertions(
            PolicyConversionContext policyContext,
            WsdlEndpointConversionContext endpointContext
        )
        {
            XmlQualifiedName bindingQName = new XmlQualifiedName(
                endpointContext.WsdlBinding.Name,
                endpointContext.WsdlBinding.ServiceDescription.TargetNamespace
            );
            UnrecognizedAssertionsBindingElement unknownBindingElement =
                new UnrecognizedAssertionsBindingElement(
                    bindingQName,
                    policyContext.GetBindingAssertions()
                );
        ...
        static void AppendUnImportedPolicyErrorMessage(
            ref StringBuilder unImportedPolicyMessage,
            WsdlEndpointConversionContext endpointContext,
            PolicyConversionContext policyContext
        )
        {
            ...
            if (policyContext.GetBindingAssertions().Count != 0)
                AddUnImportedPolicyString(
                    unImportedPolicyMessage,
                    endpointContext.WsdlBinding,
                    policyContext.GetBindingAssertions()
                );
        ...
        static void AddUnImportedPolicyString(
            StringBuilder stringBuilder,
            WsdlNS.NamedItem item,
            IEnumerable<XmlElement> unimportdPolicy
        )
        {
            stringBuilder.AppendLine(
                SR.GetString(SR.UnImportedAssertionList, CreateXPathString(item))
            );
        ...
        static string CreateXPathString(WsdlNS.NamedItem item)
        {
            ...
            string documentPath = string.Format(
                CultureInfo.InvariantCulture,
                xPathDocumentFormatString,
                wsdlNs
            );

            if (localName != null)
            {
                itemPath = string.Format(
                    CultureInfo.InvariantCulture,
            ...
            Fx.Assert(
                rest != null,
                "GetXPathParameters Method should never set rest to null. this happened for: "
                    + item.GetType()
            );
            return documentPath + itemPath + rest;
        }

        static void GetXPathParameters(
            WsdlNS.NamedItem item,
            out string wsdlNs,
            out string localName,
            ref string nameValue,
            ref string rest
        )
        {
        ...
        void LogImportError(WsdlNS.NamedItem item, WsdlImportException wie)
        {
            ...
            if (wie.InnerException != null && (wie.InnerException is WsdlImportException))
            {
                ...
                string dependencyMessage = SR.GetString(
                    SR.WsdlImportErrorDependencyDetail,
                    GetElementName(wieInner.SourceItem),
                    GetElementName(item),
                    CreateXPathString(wieInner.SourceItem)
                );
                errormessage = SR.GetString(
                    SR.WsdlImportErrorMessageDetail,
                    GetElementName(item),
                    CreateXPathString(wie.SourceItem),
                    dependencyMessage
                );
            }
            else
        ...
        static Exception CreateBeforeImportExtensionException(
            IWsdlImportExtension importer,
            Exception e
        )
        {
            string errorMessage = SR.GetString(
                SR.WsdlExtensionBeforeImportError,
        ...
        class WsdlImportException : Exception
        {
            ...
            internal static WsdlImportException Create(
                WsdlNS.NamedItem item,
            ...
            internal WsdlNS.NamedItem SourceItem
            {
        ...
        internal class WsdlPolicyReader
        {
            ...
            IEnumerable<IEnumerable<XmlElement>> GetPolicyAlternatives(
                WsdlNS.NamedItem item,
            ...
            void LogPolicyNormalizationWarning(XmlElement contextAssertion, string warningMessage)
            {
                ...
                else
                {
                    ...
                    warningMsg.AppendLine(
                        SR.GetString(SR.XPathPointer, SR.GetString(SR.XPathUnavailable))
                    );
                }
            ...
            internal static bool ContainsPolicy(WsdlNS.Binding wsdlBinding)
            {
                ...
                foreach (WsdlNS.OperationBinding wsdlOperationBinding in wsdlBinding.Operations)
                {
                    ...
                    if (
                        wsdlOperationBinding.Input != null
                        && HasPolicyAttached(wsdlOperationBinding.Input)
                    )
                    {
                        return true;
                    }
                    if (
                        wsdlOperationBinding.Output != null
                        && HasPolicyAttached(wsdlOperationBinding.Output)
                    )
                    {
                        return true;
                    }
            ...
            IEnumerable<XmlElement> GetReferencedPolicy(
                WsdlNS.NamedItem item,
                WsdlNS.ServiceDescription wsdl
            )
            {
            ...
            internal PolicyAlternatives GetPolicyAlternatives(
                WsdlEndpointConversionContext endpointContext
            )
            {
                ...
                //
                WsdlNS.ServiceDescription bindingWsdl = endpointContext.WsdlBinding
                    .ServiceDescription;
                IEnumerable<IEnumerable<XmlElement>> wsdlBindingAlternatives =
                    this.GetPolicyAlternatives(endpointContext.WsdlBinding, bindingWsdl);
                if (endpointContext.WsdlPort != null)
                {
                    IEnumerable<IEnumerable<XmlElement>> wsdlPortAlternatives =
                        this.GetPolicyAlternatives(
                            endpointContext.WsdlPort,
                            endpointContext.WsdlPort.Service.ServiceDescription
                        );
                    policyAlternatives.EndpointAlternatives = PolicyHelper.CrossProduct<XmlElement>(
                        wsdlBindingAlternatives,
                        wsdlPortAlternatives,
                        new YieldLimiter(this.importer.Quotas.MaxYields, this.importer)
                    );
                }
                else
                {
                ...
                //
                policyAlternatives.OperationBindingAlternatives = new Dictionary<
                    OperationDescription,
                ...
                policyAlternatives.MessageBindingAlternatives =
                    new Dictionary<MessageDescription, IEnumerable<IEnumerable<XmlElement>>>();
                policyAlternatives.FaultBindingAlternatives =
                    new Dictionary<FaultDescription, IEnumerable<IEnumerable<XmlElement>>>();

                foreach (
                    OperationDescription operation in endpointContext.Endpoint.Contract.Operations
                )
                {
                    ...
                    try
                    {
                        ...
                        foreach (MessageDescription message in operation.Messages)
                        {
                            WsdlNS.MessageBinding wsdlMessageBinding =
                                endpointContext.GetMessageBinding(message);
                            CreateMessageBindingAlternatives(
                                policyAlternatives,
                                bindingWsdl,
                                message,
                                wsdlMessageBinding
                        ...
                        foreach (FaultDescription fault in operation.Faults)
                        {
                            ...
                            CreateFaultBindingAlternatives(
                                policyAlternatives,
                                bindingWsdl,
                                fault,
                                wsdlFaultBinding
...
#pragma warning suppress 56500 // covered by FxCOP
            ...
            void CreateMessageBindingAlternatives(
            ...
            )
            {
                try
                {
                    IEnumerable<IEnumerable<XmlElement>> messageAlternatives =
                        this.GetPolicyAlternatives(wsdlMessageBinding, bindingWsdl);
...
#pragma warning suppress 56500 // covered by FxCOP
            ...
            void CreateFaultBindingAlternatives(
                PolicyAlternatives policyAlternatives,
                WsdlNS.ServiceDescription bindingWsdl,
                FaultDescription fault,
                WsdlNS.FaultBinding wsdlFaultBinding
            )
            {
                try
                {
                    IEnumerable<IEnumerable<XmlElement>> faultAlternatives =
                        this.GetPolicyAlternatives(wsdlFaultBinding, bindingWsdl);
...
#pragma warning suppress 56500 // covered by FxCOP
            ...
            internal XmlElement ResolvePolicyReference(
                string policyReference,
                XmlElement contextPolicyAssertion
            )
            {
                return this.policyDictionary
                    .ResolvePolicyReference(policyReference, contextPolicyAssertion);
            }
            ...
            class WsdlPolicyDictionary
            {
                ...
                > embeddedPolicyDictionary =
                    new Dictionary<WsdlNS.ServiceDescription, Dictionary<string, XmlElement>>();
                readonly Dictionary<string, XmlElement> externalPolicyDictionary =
                    new Dictionary<string, XmlElement>();
                readonly Dictionary<XmlElement, WsdlNS.ServiceDescription> policySourceTable =
                    new Dictionary<XmlElement, WsdlNS.ServiceDescription>();
                ...
                internal WsdlPolicyDictionary(WsdlImporter importer)
                {
                    ...
                    //
                    foreach (
                        KeyValuePair<string, XmlElement> policyDocument in importer.policyDocuments
                    )
                    {
                ...
                {
                    if (
                        policyDocument.Value.NamespaceURI != MetadataStrings.WSPolicy.NamespaceUri
                        && policyDocument.Value.NamespaceURI
                            != MetadataStrings.WSPolicy.NamespaceUri15
                    )
                    {
                        string warningMsg = SR.GetString(
                            SR.UnrecognizedPolicyDocumentNamespace,
                            policyDocument.Value.NamespaceURI
                        );
                        importer.LogImportWarning(warningMsg);
                        return;
                    ...
                    if (
                        PolicyHelper.GetNodeType(policyDocument.Value)
                        != PolicyHelper.NodeType.Policy
                    )
                    {
                        string warningMsg = SR.GetString(
                            SR.UnsupportedPolicyDocumentRoot,
                ...
                internal XmlElement ResolvePolicyReference(
                    string policyReference,
                    WsdlNS.ServiceDescription wsdlDocument
                )
                {
                    XmlElement policy;
                    if (policyReference[0] != '#')
                ...
                static string CreateKeyFromPolicy(string identifier, XmlElement policyElement)
                {
                    ...
                    string policyUri = string.Format(
                        CultureInfo.InvariantCulture,
                        "{0}{1}",
                        identifier,
                        policyId
                ...
                static readonly string wspPolicy = string.Format(
                    CultureInfo.InvariantCulture,
                    "{0}:{1}",
                    MetadataStrings.WSPolicy.Prefix,
                    MetadataStrings.WSPolicy.Elements.Policy
                );
                static readonly string xmlId = string.Format(
                    CultureInfo.InvariantCulture,
                    "{0}:{1}",
                    MetadataStrings.Xml.Prefix,
                    MetadataStrings.Xml.Attributes.Id
                );
                static readonly string wsuId = string.Format(
                    CultureInfo.InvariantCulture,
                    "{0}:{1}",
                    MetadataStrings.Wsu.Prefix,
                    MetadataStrings.Wsu.Attributes.Id
                ...
                internal static string CreateIdXPath(
                    WsdlNS.ServiceDescription wsdl,
                    XmlElement element,
                    string key
                )
