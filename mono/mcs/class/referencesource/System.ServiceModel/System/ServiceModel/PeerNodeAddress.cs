...
namespace System.ServiceModel
{
    ...
    public sealed class PeerNodeAddress
    {
        ...
        internal EndpointAddress10 InnerEPR
        {
            get
            {
                return this.endpointAddress == null
                    ? null
                    : EndpointAddress10.FromEndpointAddress(this.endpointAddress);
            }
        ...
        internal IList<IPAddress> ipAddressesDataMember
        {
            ...
            set
            {
                ipAddresses = new ReadOnlyCollection<IPAddress>(
                    (value == null) ? new IPAddress[0] : value
                );
            }
        }
        ...
        )
        {
            if (endpointAddress == null)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    new ArgumentNullException("endpointAddress")
                );
            if (ipAddresses == null)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    new ArgumentNullException("ipAddresses")
                );
