...
namespace System.ServiceModel.Channels
{
    ...
    internal static class MessageEncoderCompressionHandler
    {
        ...
        internal static void DecompressBuffer(
            ref ArraySegment<byte> buffer,
        ...
        )
        {
            ...
            using (
                BufferManagerOutputStream bufferedOutStream = new BufferManagerOutputStream(
                    SR.MaxReceivedMessageSizeExceeded,
                    1024,
                    maxDecompressedSize,
                    bufferManager
                )
        ...
        internal static void CompressBuffer(
            ref ArraySegment<byte> buffer,
            BufferManager bufferManager,
            CompressionFormat compressionFormat
        )
            ...
            )
            {
                ...
                using (
                    Stream ds =
                        compressionFormat == CompressionFormat.GZip
                            ? (Stream)
                                new GZipStream(bufferedOutStream, CompressionMode.Compress, true)
                            : (Stream)
                                new DeflateStream(bufferedOutStream, CompressionMode.Compress, true)
                )
                {
        ...
        internal static Stream GetDecompressStream(
        ...
        internal static Stream GetCompressStream(
