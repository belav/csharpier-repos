...
namespace System.ServiceModel.Channels
{
    ...
    class WindowsStreamSecurityUpgradeProvider : StreamSecurityUpgradeProvider
    {
        ...
        public WindowsStreamSecurityUpgradeProvider(
            ...
            : base(context.Binding)
        {
            ...
            this.listenUri = TransportSecurityHelpers.GetListenUri(
        ...
        protected override void OnOpen(TimeSpan timeout)
        {
            if (!isClient)
            {
                SecurityTokenRequirement sspiTokenRequirement =
                    TransportSecurityHelpers.CreateSspiTokenRequirement(
                        this.Scheme,
                        this.listenUri
                    );
                this.serverCredential = TransportSecurityHelpers.GetSspiCredential(
        ...
        class WindowsStreamSecurityUpgradeAcceptor : StreamSecurityUpgradeAcceptorBase
        {
            ...
            {
                AcceptUpgradeAsyncResult result = new AcceptUpgradeAsyncResult(
                    this,
                    callback,
                    state
                );
        ...
        class WindowsStreamSecurityUpgradeInitiator : StreamSecurityUpgradeInitiatorBase
        {
                ...
                : base(FramingUpgradeString.Negotiate, remoteAddress, via)
            {
                ...
                this.clientTokenProvider = TransportSecurityHelpers.GetSspiTokenProvider(
                    parent.securityTokenManager,
                    remoteAddress,
                    via,
                    parent.Scheme,
                    out this.identityVerifier
                );
            }
            ...
            internal override void Open(TimeSpan timeout)
            {
                ...
                this.credential = TransportSecurityHelpers.GetSspiCredential(
                    this.clientTokenProvider,
                    timeoutHelper.RemainingTime(),
                    out this.impersonationLevel,
                    out this.allowNtlm
                );
            }
            ...
            static SecurityMessageProperty CreateServerSecurity(NegotiateStream negotiateStream)
            {
                ...
                {
                    ReadOnlyCollection<IAuthorizationPolicy> authorizationPolicies =
                        SecurityUtils.CreatePrincipalNameAuthorizationPolicies(principalName);
                    SecurityMessageProperty result = new SecurityMessageProperty();
                    result.TransportToken = new SecurityTokenSpecification(
                        null,
                        authorizationPolicies
                    );
                    result.ServiceSecurityContext = new ServiceSecurityContext(
                        authorizationPolicies
                    );
                    return result;
                }
            ...
            class OpenAsyncResult : AsyncResult
            {
                ...
                bool HandleOpenTokenProviderComplete(IAsyncResult result)
                {
                    ...
                    IAsyncResult getCredentialResult =
                        TransportSecurityHelpers.BeginGetSspiCredential(
                            parent.clientTokenProvider,
                            timeoutHelper.RemainingTime(),
                            onGetSspiCredential,
                            this
                ...
                {
                    parent.credential = TransportSecurityHelpers.EndGetSspiCredential(
                        result,
                        out parent.impersonationLevel,
                        out parent.allowNtlm
                    );
                    return true;
                }
