...
namespace System.ServiceModel.ComIntegration
{
    ...
    {
        internal static string Getkeyword(
            string moniker,
            out MonikerHelper.MonikerAttribute keyword
        )
        {
            ...
            foreach (
                MonikerHelper.KeywordInfo keywordInfo in MonikerHelper.KeywordInfo.KeywordCollection
            )
            {
            ...
            switch (moniker[index])
            {
                ...
                case '\'':
                {
                    ...
                    for (; index < moniker.Length; index++)
                    {
                        ...
                        {
                            if (
                                (index < (moniker.Length - 1))
                                && (moniker[index + 1] == quoteToCompare)
                            )
                            {
        ...
        internal static void Parse(
            string displayName,
            ref Dictionary<MonikerHelper.MonikerAttribute, string> propertyTable
        )
        {
