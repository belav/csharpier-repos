...
namespace System.ServiceModel.ComIntegration
{
    ...
    static class ComPlusServiceHostTrace
    {
        ...
        )
        {
            ...
            {
                ComPlusServiceHostSchema record = new ComPlusServiceHostSchema(
                    info.AppID,
                    info.Clsid
                );
                TraceUtility.TraceEvent(
                    type,
                    traceCode,
                    ServiceModelSR.GetString(description),
                    record
                );
            }
        }
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                XmlQualifiedName contractQName = new XmlQualifiedName(
                    contract.Name,
                    contract.Namespace
                );
                ComPlusServiceHostCreatedServiceContractSchema record =
                    new ComPlusServiceHostCreatedServiceContractSchema(
                        info.AppID,
                        info.Clsid,
                        contractQName,
            ...
            if (DiagnosticUtility.ShouldTrace(type))
            {
                foreach (ServiceEndpoint endpoint in endpointCollection)
                {
                    ComPlusServiceHostCreatedServiceEndpointSchema record =
                        new ComPlusServiceHostCreatedServiceEndpointSchema(
                            info.AppID,
                            info.Clsid,
    ...
    static class ComPlusDllHostInitializerTrace
    {
        public static void Trace(TraceEventType type, int traceCode, string description, Guid appid)
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ComPlusDllHostInitializerSchema record = new ComPlusDllHostInitializerSchema(appid);
                TraceUtility.TraceEvent(
                    type,
                    traceCode,
                    ServiceModelSR.GetString(description),
            ...
            if (DiagnosticUtility.ShouldTrace(type))
            {
                foreach (ServiceEndpointElement endpointElement in service.Endpoints)
                {
                    ComPlusDllHostInitializerAddingHostSchema record =
                        new ComPlusDllHostInitializerAddingHostSchema(
                            appid,
                            clsid,
    ...
    static class ComPlusTLBImportTrace
    {
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ComPlusTLBImportSchema record = new ComPlusTLBImportSchema(iid, typeLibraryID);
                TraceUtility.TraceEvent(
                    type,
                    traceCode,
                    ServiceModelSR.GetString(description),
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ComPlusTLBImportFromAssemblySchema record = new ComPlusTLBImportFromAssemblySchema(
                    iid,
                    typeLibraryID,
                    assembly
                );
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ComPlusTLBImportConverterEventSchema record =
                    new ComPlusTLBImportConverterEventSchema(
                        iid,
                        typeLibraryID,
                        eventKind,
                        eventCode,
    ...
    static class ComPlusInstanceCreationTrace
    {
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ...
                ComPlusInstanceCreationRequestSchema record =
                    new ComPlusInstanceCreationRequestSchema(
                        info.AppID,
                        info.Clsid,
                        from,
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ...
                ComPlusInstanceCreationSuccessSchema record =
                    new ComPlusInstanceCreationSuccessSchema(
                        info.AppID,
                        info.Clsid,
                        from,
                        incomingTransactionID,
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ComPlusInstanceReleasedSchema record = new ComPlusInstanceReleasedSchema(
                    info.AppID,
                    info.Clsid,
                    instanceID
                );
    ...
    {
        internal static readonly Guid IID_IComThreadingInfo = new Guid(
            "000001ce-0000-0000-C000-000000000046"
        );
        internal static readonly Guid IID_IObjectContextInfo = new Guid(
        ...
        public static void Trace(TraceEventType type, int traceCode, string description)
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ...
                ComPlusActivitySchema record = new ComPlusActivitySchema(
                    guidActivityID,
                    guidLogicalThreadID,
    ...
    {
        static readonly Guid IID_IComThreadingInfo = new Guid(
            "000001ce-0000-0000-C000-000000000046"
        );
        static readonly Guid IID_IObjectContextInfo = new Guid(
            "75B52DDB-E8ED-11d1-93AD-00AA00BA3258"
        );
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ...
                if (traceContextTransaction)
                {
                    ...
                    {
                        record = new ComPlusMethodCallContextTxSchema(
                            from,
                            info.AppID,
                            info.Clsid,
                            iid,
                            ...
                            SafeNativeMethods.GetCurrentThreadId(),
                            callerIdentity,
                            guidContextTrsansactionID
                        );
                    }
                }
                else
                {
                    record = new ComPlusMethodCallSchema(
                        from,
                        info.AppID,
                        info.Clsid,
                        iid,
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ComPlusMethodCallTxMismatchSchema record = new ComPlusMethodCallTxMismatchSchema(
                    from,
                    info.AppID,
                    info.Clsid,
                    iid,
        ...
        public static void Trace(
            ...
            Guid iid,
            int instanceID,
            Guid guidIncomingTransactionID
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ComPlusMethodCallNewTxSchema record = new ComPlusMethodCallNewTxSchema(
                    from,
                    info.AppID,
                    info.Clsid,
                    iid,
    ...
    static class ComPlusServiceMonikerTrace
    {
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ...
                propertyTable.TryGetValue(
                    MonikerHelper.MonikerAttribute.SpnIdentity,
                    out spnIdentity
                );
                propertyTable.TryGetValue(
                    MonikerHelper.MonikerAttribute.UpnIdentity,
                    out upnIdentity
                );
                propertyTable.TryGetValue(
                    MonikerHelper.MonikerAttribute.DnsIdentity,
                    out dnsIdentity
                );
                propertyTable.TryGetValue(
                    MonikerHelper.MonikerAttribute.MexAddress,
                    out mexAddress
                );
                propertyTable.TryGetValue(
                ...
                ComPlusServiceMonikerSchema record = new ComPlusServiceMonikerSchema(
                    address,
                    contract,
                    contractNamespace,
                    wsdl,
    ...
    static class ComPlusWsdlChannelBuilderTrace
    {
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ...
                {
                    ComPlusWsdlChannelBuilderSchema record = new ComPlusWsdlChannelBuilderSchema(
                        bindingQname,
                        contractQname,
                        serviceName,
                        (contractType != null) ? contractType.ToString() : null,
    ...
    static class ComPlusMexChannelBuilderMexCompleteTrace
    {
        public static void Trace(
            ...
            int traceCode,
            string description,
            ServiceEndpointCollection serviceEndpointsRetrieved
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ...
                ComPlusMexBuilderMetadataRetrievedEndpoint[] endpoints =
                    new ComPlusMexBuilderMetadataRetrievedEndpoint[serviceEndpointsRetrieved.Count];
                ...
                ComPlusMexBuilderMetadataRetrievedSchema record =
                    new ComPlusMexBuilderMetadataRetrievedSchema(endpoints);
                TraceUtility.TraceEvent(
                    type,
                    traceCode,
                    ServiceModelSR.GetString(description),
    ...
    static class ComPlusMexChannelBuilderTrace
    {
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ComPlusMexChannelBuilderSchema record = new ComPlusMexChannelBuilderSchema(
                    contract.Name,
                    contract.Namespace,
                    binding.Name,
                    binding.Namespace,
    ...
    static class ComPlusTypedChannelBuilderTrace
    {
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ComPlusTypedChannelBuilderSchema record = new ComPlusTypedChannelBuilderSchema(
    ...
    static class ComPlusChannelCreatedTrace
    {
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ComPlusChannelCreatedSchema record = new ComPlusChannelCreatedSchema(
                    address,
                    (contractType != null) ? contractType.ToString() : null
                );
                TraceUtility.TraceEvent(
    ...
    static class ComPlusDispatchMethodTrace
    {
        public static void Trace(
            ...
            Dictionary<
                UInt32,
                System.ServiceModel.ComIntegration.DispatchProxy.MethodInfo
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ...
                {
                    ComPlusDispatchMethodSchema record = new ComPlusDispatchMethodSchema(
                        methodInfo.opDesc.Name,
    ...
    static class ComPlusTxProxyTrace
    {
        ...
        )
        {
            if (DiagnosticUtility.ShouldTrace(type))
            {
                ComPlusTxProxySchema record = new ComPlusTxProxySchema(
                    appid,
                    clsid,
                    transactionID,
                    instanceID
