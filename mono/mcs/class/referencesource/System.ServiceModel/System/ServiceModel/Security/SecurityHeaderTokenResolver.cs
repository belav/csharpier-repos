...
namespace System.ServiceModel.Security
{
    ...
    internal sealed class SecurityHeaderTokenResolver
        : SecurityTokenResolver,
            System.IdentityModel.IWrappedTokenKeyResolver
    {
        ...
        )
        {
            ...
            if (
                (allowedReferenceStyle == SecurityTokenReferenceStyle.External)
            ...
            this.tokens[this.tokenCount++] = new SecurityTokenEntry(
                token,
                tokenParameters,
        ...
        internal SecurityToken ResolveToken(
            SecurityKeyIdentifier keyIdentifier,
            bool matchOnlyExternalTokens,
            bool resolveIntrinsicKeyClause
        )
        {
            ...
            for (int i = 0; i < keyIdentifier.Count; i++)
            {
                SecurityToken token = ResolveToken(
                    keyIdentifier[i],
                    matchOnlyExternalTokens,
                    resolveIntrinsicKeyClause
                );
                if (token != null)
                {
                    return token;
        ...
        SecurityKey ResolveSecurityKeyCore(
            SecurityKeyIdentifierClause keyIdentifierClause,
            bool createIntrinsicKeys
        )
        {
            ...
            {
                if (
                    SecurityUtils.TryCreateKeyFromIntrinsicKeyClause(
                        keyIdentifierClause,
                        this,
                        out securityKey
                    )
        ...
        )
        {
            ...
            for (int i = 0; i < this.tokenCount; i++)
            {
                if (
                    matchOnlyExternal
                    && tokens[i].AllowedReferenceStyle != SecurityTokenReferenceStyle.External
                )
                {
                    continue;
                }
                ...
                if (
                    ...
                    && tokens[i].TokenParameters.MatchesKeyIdentifierClause(
                        token,
                        keyIdentifierClause,
                ...
                else if (tokens[i].TokenParameters == null)
                {
                    ...
                    if (
                        tokens[i].AllowedReferenceStyle == SecurityTokenReferenceStyle.Internal
                        && MatchDirectReference(token, keyIdentifierClause)
                    )
                    {
                        resolvedToken = token;
                        break;
            ...
            if ((resolvedToken == null) && (keyIdentifierClause is EncryptedKeyIdentifierClause))
            {
                ...
                {
                    resolvedToken = SecurityUtils.CreateTokenFromEncryptedKeyClause(
                        keyClause,
                        unwrappingToken
                    );
                }
            ...
            if (
                (resolvedToken == null)
                && (keyIdentifierClause is X509RawDataKeyIdentifierClause)
                && (!matchOnlyExternal)
                && (resolveIntrinsicKeyClause)
            )
            {
                resolvedToken = new X509SecurityToken(
                    new X509Certificate2(
                        ((X509RawDataKeyIdentifierClause)keyIdentifierClause).GetX509RawData()
                    )
                );
            }
            ...
            if ((resolvedToken != null) && (derivationNonce != null))
            {
                ...
                int derivationLength =
                    (keyIdentifierClause.DerivationLength == 0)
                ...
                for (int i = 0; i < this.tokenCount; ++i)
                {
                    DerivedKeySecurityToken derivedKeyToken =
                        this.tokens[i].Token as DerivedKeySecurityToken;
                ...
                if (!alreadyDerived)
                {
                    string psha1Algorithm = SecurityUtils.GetKeyDerivationAlgorithm(
                        this.securityHeader
                            .StandardsManager
                    ...
                    resolvedToken = new DerivedKeySecurityToken(
                        -1,
                        0,
                        derivationLength,
                        null,
        ...
        public override string ToString()
        {
            using (StringWriter writer = new StringWriter(CultureInfo.InvariantCulture))
            {
                ...
                {
                    writer.WriteLine(
                        "    TokenEntry[{0}] = (AllowedReferenceStyle={1}, Token={2}, Parameters={3})",
                        i,
                        this.tokens[i].AllowedReferenceStyle,
                        this.tokens[i].Token.GetType(),
                        tokens[i].TokenParameters
                    );
                }
                writer.WriteLine("    )");
        ...
        internal bool TryResolveSecurityKey(
            SecurityKeyIdentifierClause keyIdentifierClause,
            bool createIntrinsicKeys,
            out SecurityKey key
        )
