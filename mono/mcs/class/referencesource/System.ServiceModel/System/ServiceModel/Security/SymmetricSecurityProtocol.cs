...
namespace System.ServiceModel.Security
{
    ...
    sealed class SymmetricSecurityProtocol : MessageSecurityProtocol
    {
        ...
        public SymmetricSecurityProtocol(
        ...
        {
            InitiatorServiceModelSecurityTokenRequirement tokenRequirement =
                CreateInitiatorSecurityTokenRequirement();
            this.Factory
                .SecurityTokenParameters
                .InitializeSecurityTokenRequirement(tokenRequirement);
        ...
        public override void OnOpen(TimeSpan timeout)
        {
            ...
            if (this.Factory.ActAsInitiator)
            {
                ...
                InitiatorServiceModelSecurityTokenRequirement tokenProviderRequirement =
                    CreateInitiatorTokenRequirement();

                // 2. Create a provider
                SecurityTokenProvider tokenProvider = this.Factory
                    .SecurityTokenManager
                    .CreateSecurityTokenProvider(tokenProviderRequirement);
        ...
        public override void OnAbort()
        {
            if (this.Factory.ActAsInitiator)
            {
                SecurityTokenProvider provider =
                    this.initiatorSymmetricTokenProvider ?? this.initiatorAsymmetricTokenProvider;
                if (provider != null)
                {
        ...
        )
        {
            ...
            TimeoutHelper timeoutHelper = new TimeoutHelper(timeout);
            if (
                TryGetTokenSynchronouslyForOutgoingSecurity(
                    message,
                    correlationState,
                    false,
                    timeoutHelper.RemainingTime(),
            ...
            )
            {
                SetUpDelayedSecurityExecution(
                    ref message,
                    prerequisiteWrappingToken,
                    token,
                    tokenParameters,
                    supportingTokens,
                    GetSignatureConfirmationCorrelationState(correlationState, newCorrelationState)
                );
        ...
        WrappedKeySecurityToken CreateWrappedKeyToken(
            ...
            SecurityTokenReferenceStyle wrappingTokenReferenceStyle
        )
        {
            int keyLength = Math.Max(
                128,
                this.Factory.OutgoingAlgorithmSuite.DefaultSymmetricKeyLength
            );
            ...
            string wrappingAlgorithm = this.Factory
                .OutgoingAlgorithmSuite
                .DefaultAsymmetricKeyWrapAlgorithm;
            SecurityKeyIdentifierClause clause = wrappingTokenParameters.CreateKeyIdentifierClause(
                wrappingToken,
                wrappingTokenReferenceStyle
            );
        ...
        SecurityToken GetInitiatorToken(
        ...
        )
        {
            ...
            if (this.Factory.SecurityTokenParameters.HasAsymmetricKey)
            {
                ...
                bool isAsymmetricTokenInMessage = SendSecurityHeader.ShouldSerializeToken(
                ...
                token = CreateWrappedKeyToken(
                    asymmetricToken,
                    this.Factory.SecurityTokenParameters,
                    (isAsymmetricTokenInMessage)
        ...
        bool TryGetTokenSynchronouslyForOutgoingSecurity(
            Message message,
            SecurityProtocolCorrelationState correlationState,
            bool isBlockingCall,
            TimeSpan timeout,
        ...
        )
        {
            ...
            if (factory.ApplyIntegrity || factory.ApplyConfidentiality)
            {
                if (factory.ActAsInitiator)
                {
                    ...
                    SecurityTokenProvider provider = GetTokenProvider();
                    SecurityToken providerToken = GetTokenAndEnsureOutgoingIdentity(
                        provider,
        ...
        SecurityToken GetCorrelationToken(
            ...
            out SecurityTokenParameters correlationTokenParameters
        )
        {
            ...
            if (!(token is WrappedKeySecurityToken))
            {
                throw TraceUtility.ThrowHelperError(
                    new MessageSecurityException(
                        SR.GetString(SR.IncomingSigningTokenMustBeAnEncryptedKey)
                    ),
                    message
                );
            }
        ...
        )
        {
            ...
            if (this.Factory.ActAsInitiator)
            {
                ...
                SecurityToken outOfBandToken = GetCorrelationToken(
                    correlationStates,
                    out outOfBandTokenParameters
                );
                securityHeader.ConfigureSymmetricBindingClientReceiveHeader(
                    outOfBandToken,
                    outOfBandTokenParameters
                );
            ...
            else
            {
                if (factory.RecipientSymmetricTokenAuthenticator != null)
                {
                    securityHeader.ConfigureSymmetricBindingServerReceiveHeader(
                        this.Factory.RecipientSymmetricTokenAuthenticator,
                        this.Factory.SecurityTokenParameters,
                ...
                securityHeader.ConfigureOutOfBandTokenResolver(
                    MergeOutOfBandResolvers(
                        supportingAuthenticators,
                        this.Factory.RecipientOutOfBandTokenResolverList
                    )
                );
            }
            ...
            if (factory.RequireIntegrity)
            {
                ...
                if (factory.ActAsInitiator)
                {
                    if (!factory.SecurityTokenParameters.HasAsymmetricKey)
                    {
                        ReadOnlyCollection<IAuthorizationPolicy> signingTokenPolicies =
                            this.initiatorTokenAuthenticator.ValidateToken(signingToken);
                ...
                else
                {
                    AttachRecipientSecurityProperty(
                        message,
                        signingToken,
                        this.Factory.SecurityTokenParameters.HasAsymmetricKey,
        ...
        sealed class SecureOutgoingMessageAsyncResult : GetOneTokenAndSetUpSecurityAsyncResult
        {
            ...
            public SecureOutgoingMessageAsyncResult(
                Message m,
                SymmetricSecurityProtocol binding,
                SecurityTokenProvider provider,
            ...
            protected override void OnGetTokenDone(
                ref Message message,
                SecurityToken providerToken,
                TimeSpan timeout
            )
            {
                SecurityTokenParameters tokenParameters;
                SecurityToken prerequisiteWrappingToken;
                SecurityToken token = symmetricBinding.GetInitiatorToken(
                    providerToken,
                    message,
                    timeout,
                    out tokenParameters,
                ...
                symmetricBinding.SetUpDelayedSecurityExecution(
                    ref message,
                    prerequisiteWrappingToken,
                    token,
                    tokenParameters,
                    this.SupportingTokens,
                    Binding.GetSignatureConfirmationCorrelationState(
                        OldCorrelationState,
                        NewCorrelationState
                    )
                );
