...
namespace System.ServiceModel.Security
{
    class WSKeyInfoSerializer : KeyInfoSerializer
    {
        static Func<
            KeyInfoSerializer,
            IEnumerable<SecurityTokenSerializer.SerializerEntries>
        > CreateAdditionalEntries(
        ...
        )
        {
            ...
            {
                List<SecurityTokenSerializer.SerializerEntries> serializerEntries =
                    new List<SecurityTokenSerializer.SerializerEntries>();

                if (securityVersion == SecurityVersion.WSSecurity10)
                {
                    serializerEntries.Add(
                        new System.IdentityModel.Tokens.WSSecurityJan2004(keyInfoSerializer)
                    );
                }
                else if (securityVersion == SecurityVersion.WSSecurity11)
                {
                    serializerEntries.Add(
                        new System.IdentityModel.Tokens.WSSecurityXXX2005(keyInfoSerializer)
                    );
                }
                else
                {
        ...
        public WSKeyInfoSerializer(
            ...
            : base(
                ...
                CreateAdditionalEntries(securityVersion, secureConversationVersion)
            ) { }
        ...
        abstract class WSSecureConversation : SecurityTokenSerializer.SerializerEntries
        {
            ...
            protected abstract class SctStrEntry : StrEntry
            {
                ...
                public override bool CanReadClause(XmlDictionaryReader reader, string tokenType)
                {
                    if (
                        tokenType != null
                        && tokenType != parent.SerializerDictionary.SecurityContextTokenType.Value
                    )
                    {
                        return false;
                    }
                ...
                {
                    System.Xml.UniqueId uri = XmlHelper.GetAttributeAsUniqueId(
                        reader,
                    ...
                    return new SecurityContextKeyIdentifierClause(
                        uri,
                        generation,
                        derivationNonce,
                        derivationLength
                ...
                )
                {
                    ...
                    writer.WriteAttributeString(
                        ...
                        null,
                        parent.SerializerDictionary.SecurityContextTokenReferenceValueType.Value
                    );
                    writer.WriteEndElement();
                }

                protected abstract void WriteGeneration(
                    XmlDictionaryWriter writer,
