...
namespace System.ServiceModel.Security.Tokens
{
    ...
    public class SecurityContextSecurityToken
        : SecurityToken,
            TimeBoundedCache.IExpirableItem,
            IDisposable
    {
        byte[] cookieBlob;
        ...
        internal SecurityContextSecurityToken(SecurityContextSecurityToken sourceToken, string id)
            : this(
                ...
                id,
                sourceToken.key,
                sourceToken.keyGeneration,
                sourceToken.keyEffectiveTime,
                sourceToken.keyExpirationTime,
                sourceToken.AuthorizationPolicies
            ) { }

        internal SecurityContextSecurityToken(
            SecurityContextSecurityToken sourceToken,
            string id,
            byte[] key,
            UniqueId keyGeneration,
            DateTime keyEffectiveTime,
            ...
            : base()
        {
            this.id = id;
            this.Initialize(
                sourceToken.contextId,
                key,
                sourceToken.ValidFrom,
                sourceToken.ValidTo,
                authorizationPolicies,
                sourceToken.isCookieMode,
                keyGeneration,
                keyEffectiveTime,
                keyExpirationTime
            );
            this.cookieBlob = sourceToken.cookieBlob;
            this.bootstrapMessageProperty =
                (sourceToken.bootstrapMessageProperty == null)
                    ? null
                    : (SecurityMessageProperty)sourceToken.BootstrapMessageProperty.CreateCopy();
        }
        ...
        SecurityContextSecurityToken(SecurityContextSecurityToken from)
        {
            ...
            this.Initialize(
                from.contextId,
                from.key,
                from.tokenEffectiveTime,
                from.tokenExpirationTime,
                authorizationPolicies,
                from.isCookieMode,
                from.keyGeneration,
                from.keyEffectiveTime,
                from.keyExpirationTime
            );
            this.cookieBlob = from.cookieBlob;
            this.bootstrapMessageProperty =
                (from.bootstrapMessageProperty == null)
                    ? null
                    : (SecurityMessageProperty)from.BootstrapMessageProperty.CreateCopy();
        }
        ...
        {
            get { return this.bootstrapMessageProperty; }
            set { this.bootstrapMessageProperty = value; }
        }
        ...
        {
            get { return this.keyExpirationTime; }
        }
        ...
        public ReadOnlyCollection<IAuthorizationPolicy> AuthorizationPolicies
        {
            ...
            internal set { this.authorizationPolicies = value; }
        }
        ...
        public override string ToString()
        {
            return String.Format(
                ...
                "SecurityContextSecurityToken(Identifier='{0}', KeyGeneration='{1}')",
                this.contextId,
                this.keyGeneration
            );
        }
        ...
        {
            if (typeof(T) == typeof(SecurityContextKeyIdentifierClause))
                return new SecurityContextKeyIdentifierClause(this.contextId, this.keyGeneration)
                    as T;
        ...
        {
            SecurityContextKeyIdentifierClause sctKeyIdentifierClause =
        ...
        {
            SecurityContextCookieSerializer cookieSerializer = new SecurityContextCookieSerializer(
                securityStateEncoder,
                null
            );
            byte[] cookieBlob = cookieSerializer.CreateCookieFromSecurityContext(
                contextId,
                id,
                key,
                validFrom,
