...
namespace System.ServiceModel.Dispatcher
{
    ...
    class XmlSerializerOperationFormatter : OperationFormatter
    {
        ...
        public XmlSerializerOperationFormatter(
        ...
        )
        {
            ...
            try
            {
                ...
                if (serializer != null)
                {
                    ...
                    foreach (
                        MessageHeaderDescription headerDescription in messageDescription.Headers
                    )
                    {
                        ...
                        if (!headerDescription.IsUnknownHeaderCollection)
                        {
                            if (headerDescription.TypedHeader)
                            {
                                if (messageHeaderOfTHelper == null)
                                    messageHeaderOfTHelper = new MessageHeaderOfTHelper(
                                        parameters.Length
                                    );
                                headerValues[headerIndex++] =
                                    messageHeaderOfTHelper.GetContentAndSaveHeaderAttributes(
                                        parameters[headerDescription.Index],
                                        headerDescription
                                    );
                            }
                            else
                    ...
                    XmlDictionaryWriter bufferWriter = XmlDictionaryWriter.CreateTextWriter(
                        memoryStream
                    );
                    ...
                    doc.Load(
                        new XmlTextReader(memoryStream) { DtdProcessing = DtdProcessing.Prohibit }
                    );
                    ...
                    foreach (XmlElement element in doc.DocumentElement.ChildNodes)
                    {
                        ...
                        else
                        {
                            if (matchingHeaderDescription.TypedHeader)
                                messageHeaderOfTHelper.GetHeaderAttributes(
                                    matchingHeaderDescription,
                                    out mustUnderstand,
                                    out relay,
                                    out actor
                                );
                ...
                if (
                    unknownHeaderDescription != null
                    && parameters[unknownHeaderDescription.Index] != null
                )
                {
                    foreach (
                        object unknownHeader in (IEnumerable)
                            parameters[unknownHeaderDescription.Index]
                    )
                    {
                        XmlElement element = (XmlElement)GetContentOfMessageHeaderOfT(
        ...
        )
        {
            try
            {
                ...
                if (serializer == null)
                {
                    if (unknownHeaderDescription != null)
                    {
                        ...
                        parameters[unknownHeaderDescription.Index] = unknownHeaders.ToArray(
                            unknownHeaderDescription.TypedHeader
                ...
                for (int headerIndex = 0; headerIndex < headers.Count; headerIndex++)
                {
                    ...
                    MessageHeaderDescription matchingHeaderDescription = headerDescriptionTable.Get(
                        header.Name,
                        header.Namespace
                    );
                    if (matchingHeaderDescription != null)
                    {
                        ...
                        if (matchingHeaderDescription.TypedHeader)
                        {
                            ...
                            messageHeaderOfTHelper.SetHeaderAttributes(
                                matchingHeaderDescription,
                    ...
                    )
                    {
                        if (!headerDescription.IsUnknownHeaderCollection)
                        {
                            ...
                            if (headerDescription.TypedHeader && parameterValue != null)
                                parameterValue = messageHeaderOfTHelper.CreateMessageHeader(
                                    headerDescription,
                                    parameterValue
                                );
        ...
        private static void AddUnknownHeader(
            MessageHeaderDescription unknownHeaderDescription,
            ArrayList unknownHeaders,
            XmlDocument xmlDoc,
            XmlDictionaryWriter bufferWriter,
        ...
        )
        {
            ...
            if (unknownHeader != null && unknownHeaderDescription.TypedHeader)
                unknownHeader = TypedHeaderManager.Create(
        ...
        protected override void WriteBodyAttributes(
            ...
            try
            {
                ...
                if (messageInfo.RpcEncodedTypedMessageBodyParts == null)
                {
                    SerializeBody(
                        writer,
                        version,
                        messageInfo.BodySerializer,
                        messageDescription.Body.ReturnValue,
                        messageDescription.Body.Parts,
                        returnValue,
                        parameters
                    );
                    return;
                ...
                object[] bodyPartValues = new object[
                    messageInfo.RpcEncodedTypedMessageBodyParts.Count
                ];
                ...
                SerializeBody(
                    ...
                    null /*returnPart*/
                    ,
                    messageInfo.RpcEncodedTypedMessageBodyParts,
                    null /*returnValue*/
                    ,
                    bodyPartValues
                );
        ...
        class MessageHeaderOfTHelper
        {
            ...
            internal object GetContentAndSaveHeaderAttributes(
            ...
            )
            {
                ...
                if (headerDescription.Multiple)
                {
                    ...
                    MessageHeader<object>[] messageHeaderOfTAttributes = new MessageHeader<object>[
                        messageHeaderOfTArray.Length
                    ];
                    Array tArray = Array.CreateInstance(
                    ...
                    {
                        tArray.SetValue(
                            GetContentOfMessageHeaderOfT(
                                headerDescription,
                                messageHeaderOfTArray[i],
                                out mustUnderstand,
                                out relay,
                                out actor
                            ),
                        ...
                        messageHeaderOfTAttributes[i] = new MessageHeader<object>(
                            null,
                            mustUnderstand,
                            actor,
                            relay
                ...
                {
                    object content = GetContentOfMessageHeaderOfT(
                        headerDescription,
                        parameterValue,
                        out mustUnderstand,
                        out relay,
                    ...
                    attributes[headerDescription.Index] = new MessageHeader<object>(
                        null,
                        mustUnderstand,
                        actor,
                        relay
            ...
            internal void GetHeaderAttributes(
                MessageHeaderDescription headerDescription,
            ...
            )
            {
                ...
                if (headerDescription.Multiple)
                {
                    MessageHeader<object>[] messageHeaderOfTAttributes = (MessageHeader<object>[])
                        attributes[headerDescription.Index];
                    ...
                    matchingMessageHeaderOfTAttribute =
                        (MessageHeader<object>)attributes[headerDescription.Index];
            ...
            internal void SetHeaderAttributes(
                MessageHeaderDescription headerDescription,
                bool mustUnderstand,
                bool relay,
                string actor
            )
            {
                if (headerDescription.Multiple)
                {
                    ...
                    ((List<MessageHeader<object>>)attributes[headerDescription.Index]).Add(
                        new MessageHeader<object>(null, mustUnderstand, actor, relay)
                    );
                }
                else
                    attributes[headerDescription.Index] = new MessageHeader<object>(
            ...
            internal object CreateMessageHeader(
                MessageHeaderDescription headerDescription,
                object headerValue
            )
            {
                if (headerDescription.Multiple)
                {
                    IList<MessageHeader<object>> messageHeaderOfTAttributes =
                        (IList<MessageHeader<object>>)attributes[headerDescription.Index];
                    object[] messageHeaderOfTArray = (object[])
                        Array.CreateInstance(
                            TypedHeaderManager.GetMessageHeaderType(headerDescription.Type),
                            messageHeaderOfTAttributes.Count
                        );
                    Array headerValues = (Array)headerValue;
                    for (int i = 0; i < messageHeaderOfTArray.Length; i++)
                    {
                        MessageHeader<object> messageHeaderOfTAttribute =
                            messageHeaderOfTAttributes[i];
                        messageHeaderOfTArray[i] = TypedHeaderManager.Create(
                            headerDescription.Type,
                            headerValues.GetValue(i),
                            messageHeaderOfTAttribute.MustUnderstand,
                            messageHeaderOfTAttribute.Relay,
                            messageHeaderOfTAttribute.Actor
                        );
                    }
                ...
                else
                {
                    MessageHeader<object> messageHeaderOfTAttribute =
                        (MessageHeader<object>)attributes[headerDescription.Index];
                    return TypedHeaderManager.Create(
                        headerDescription.Type,
                        headerValue,
                        messageHeaderOfTAttribute.MustUnderstand,
                        messageHeaderOfTAttribute.Relay,
