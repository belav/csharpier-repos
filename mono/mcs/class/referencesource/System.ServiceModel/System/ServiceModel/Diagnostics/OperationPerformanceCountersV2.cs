...
namespace System.ServiceModel.Diagnostics
{
    ...
    sealed class OperationPerformanceCountersV2 : OperationPerformanceCountersBase
    {
        ...
        internal OperationPerformanceCountersV2(
            string service,
            string contract,
        ...
        static CounterSetInstance CreateCounterSetInstance(string name)
        {
            return counterSetInstanceCache.Get(name)
                ?? operationCounterSet.CreateCounterSetInstance(name);
        }
        ...
        // Immediately disposes and nulls the CounterSetInstance. This differs from Dispose because Dispose
        // is "lazy" in that
        // it holds weak references to the instances so we don't get corrupted state if the values are
        // updated later. This
        // method is used in situations when we need to delete the instance immediately and know the values
        // won't be updated.
