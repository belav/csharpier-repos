...
namespace System.ServiceModel.Diagnostics
{
    ...
    class ServiceModelPerformanceCounters
    {
        ...
        )
        {
            ...
            {
                this.operationPerfCounters = new Dictionary<
                    string,
                    OperationPerformanceCountersBase
                >(contractDescription.Operations.Count);
                this.actionToOperation = new SortedList<string, string>(
                    contractDescription.Operations.Count
                );
                ...
                {
                    Fx.Assert(
                        null != opDescription.Messages,
                        "OperationDescription.Messages should not be null"
                    );
                    Fx.Assert(
                        opDescription.Messages.Count > 0,
                        "OperationDescription.Messages should not be empty"
                    );
                    Fx.Assert(
                        null != opDescription.Messages[0],
                        "OperationDescription.Messages[0] should not be null"
                    );
                    if (
                        null != opDescription.Messages[0].Action
                    ...
                    if (!this.operationPerfCounters.TryGetValue(opDescription.Name, out c))
                    {
                        OperationPerformanceCountersBase counters =
                            PerformanceCountersFactory.CreateOperationCounters(
                ...
                EndpointPerformanceCountersBase endpointCounters =
                    PerformanceCountersFactory.CreateEndpointCounters(
                        serviceHost.Description.Name,
                        contractDescription.Name,
                        endpointDispatcher.PerfCounterBaseId
                    );
        ...
        internal OperationPerformanceCountersBase GetOperationPerformanceCountersFromMessage(
            Message message
        )
        {
        ...
        {
            Fx.Assert(
                PerformanceCounters.Scope == PerformanceCounterScope.All,
                "Only call GetOparationPerformanceCounters when performance counter scope is All"
            );

            OperationPerformanceCountersBase counters;
            Dictionary<string, OperationPerformanceCountersBase> opPerfCounters =
                this.operationPerfCounters;
    ...
    internal class ServiceModelPerformanceCountersEntry
    {
        ...
        public ServiceModelPerformanceCountersEntry(
            DefaultPerformanceCounters defaultServiceCounters
        )
        {
