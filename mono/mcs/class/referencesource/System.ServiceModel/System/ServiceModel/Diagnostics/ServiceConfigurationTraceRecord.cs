...
namespace System.ServiceModel.Diagnostics
{
    ...
    class ServiceConfigurationTraceRecord : TraceRecord
    {
        ...
        {
            get { return BuildEventId("ServiceConfiguration"); }
        }
        ...
        internal override void WriteTo(XmlWriter xml)
        {
            xml.WriteElementString(
                "FoundServiceElement",
                ((bool)(this.serviceElement != null)).ToString(CultureInfo.InvariantCulture)
            );
            if (this.serviceElement != null)
            {
                if (!string.IsNullOrEmpty(this.serviceElement.ElementInformation.Source))
                {
                    xml.WriteElementString(
                        ...
                        this.serviceElement.ElementInformation.Source
                    );
                    xml.WriteElementString(
                ...
                this.WriteElementString(
                    ...
                    this.serviceElement.BehaviorConfiguration,
                    xml
                );
                ...
                xml.WriteElementString(
                    "OpenTimeout",
                    this.serviceElement.Host.Timeouts.OpenTimeout.ToString()
                );
                xml.WriteElementString(
                    "CloseTimeout",
                    this.serviceElement.Host.Timeouts.CloseTimeout.ToString()
                );
                xml.WriteEndElement();
                if (this.serviceElement.Host.BaseAddresses.Count > 0)
                {
                    ...
                    foreach (
                        BaseAddressElement baseAddress in this.serviceElement.Host.BaseAddresses
                    )
                    {
                ...
                foreach (ServiceEndpointElement serviceEndpoint in this.serviceElement.Endpoints)
                {
                    ...
                    this.WriteElementString(
                        "BindingNamespace",
                        serviceEndpoint.BindingNamespace,
                        xml
                    );
                    ...
                    xml.WriteElementString(
                        "ListenUriMode",
                        serviceEndpoint.ListenUriMode.ToString()
                    );
