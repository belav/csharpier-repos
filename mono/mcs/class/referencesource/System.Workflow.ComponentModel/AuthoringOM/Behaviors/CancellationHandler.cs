namespace System.Workflow.ComponentModel
{
    ...
    internal sealed class CancellationHandlerValidator : CompositeActivityValidator
    {
        public override ValidationErrorCollection Validate(ValidationManager manager, object obj)
        {
            ...
            CancellationHandlerActivity cancellationHandlerActivity =
                obj as CancellationHandlerActivity;
            if (cancellationHandlerActivity == null)
                throw new ArgumentException(
                    SR.GetString(
                        SR.Error_UnexpectedArgumentType,
                        typeof(CancellationHandlerActivity).FullName
                    ),
                    "obj"
                );
                ...
                validationErrors.Add(
                    new ValidationError(
                        SR.GetString(
                            ...
                            typeof(CancellationHandlerActivity).FullName,
                            cancellationHandlerActivity.QualifiedName
                        ),
            ...
            // cancellation handlers can not contain fault handlers, compensation handler and cancellation
            // handler
            if (
                ((ISupportAlternateFlow)cancellationHandlerActivity).AlternateFlowActivities.Count
                > 0
            )
                validationErrors.Add(
                    new ValidationError(
