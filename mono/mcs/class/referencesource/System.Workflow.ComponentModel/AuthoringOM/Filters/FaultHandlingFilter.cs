            ...
            IActivityEventListener<ActivityExecutionStatusChangedEventArgs>
    {
        public static DependencyProperty FaultProcessedProperty =
            DependencyProperty.RegisterAttached(
                "FaultProcessed",
                typeof(bool),
                typeof(FaultAndCancellationHandlingFilter),
        ...
        public override ActivityExecutionStatus HandleFault(
            Activity activity,
            ActivityExecutionContext executionContext,
            Exception exception
        )
        {
            ...
            Activity handlersActivity = FaultAndCancellationHandlingFilter.GetFaultHandlers(
                executionContext.Activity
            );
            if (
                handlersActivity != null
                && (
                    handlersActivity.ExecutionStatus != ActivityExecutionStatus.Closed
                    && handlersActivity.ExecutionStatus != ActivityExecutionStatus.Initialized
                )
            )
            {
            ...
            {
                handlersActivity = FaultAndCancellationHandlingFilter.GetCancellationHandler(
                    executionContext.Activity
                );
        ...
        public void OnEvent(object sender, ActivityExecutionStatusChangedEventArgs e)
        {
            ...
            if (e.Activity == context.Activity)
            {
                ...
                )
                {
                    ...
                    )
                    {
                        ...
                        CompositeActivity exceptionHandlersActivity =
                            FaultAndCancellationHandlingFilter.GetFaultHandlers(context.Activity);
                        if (exceptionHandlersActivity != null)
                        {
                            // listen for FaultHandler status change events
                            exceptionHandlersActivity.RegisterForStatusChange(
                    ...
                    else if (context.Activity.ExecutionResult == ActivityExecutionResult.Canceled)
                    {
                        ...
                        Activity cancelHandler =
                            FaultAndCancellationHandlingFilter.GetCancellationHandler(
                                context.Activity
                            );
                ...
                (e.Activity is FaultHandlersActivity || e.Activity is CancellationHandlerActivity)
        ...
        internal static CompositeActivity GetFaultHandlers(Activity activityWithExceptionHandlers)
        {
            ...
            CompositeActivity compositeActivity =
                activityWithExceptionHandlers as CompositeActivity;
