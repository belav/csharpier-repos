namespace System.Workflow.ComponentModel
{
    ...
    internal abstract class ActivityExecutionFilter : ActivityExecutor, ISupportWorkflowChanges
    {
        ...
        )
        {
            ...
            return NextActivityExecutorInChain(executionContext.Activity)
                .Execute(activity, executionContext);
        }

        public override ActivityExecutionStatus Cancel(
            Activity activity,
            ActivityExecutionContext executionContext
        )
        {
            ...
            ActivityExecutor nextActivityExecutor = NextActivityExecutorInChain(
                executionContext.Activity
            );
            if (
                !(nextActivityExecutor is ActivityExecutionFilter)
                && executionContext.Activity.HasPrimaryClosed
            )
