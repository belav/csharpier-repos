    ...
    public sealed class DependencyProperty : ISerializable
    {
        ...
        )
        {
            ...
            if (knownProperties[byteVal] != null)
            {
                throw new InvalidOperationException(
                    SR.GetString(
                        SR.Error_AlreadyRegisteredAs,
                        knownProperties[byteVal].dependencyProperty.ToString()
                    )
                );
            }

            dependencyProperty.KnownIndex = byteVal;
            knownProperties[byteVal] = new KnownDependencyProperty(
                dependencyProperty,
                propertyValidity
            );
        }
        ...
        )
        {
            if (validatorType == null)
                ...
                throw new ArgumentException(
                    SR.GetString(SR.Error_ValidatorTypeIsInvalid),
                    "validatorType"
                );
        ...
        public static IList<DependencyProperty> FromType(Type ownerType)
        {
            ...
            lock (((ICollection)DependencyProperty.dependencyProperties).SyncRoot)
            {
                ...
                {
                    if (
                        TypeProvider.IsSubclassOf(ownerType, dependencyProperty.ownerType)
        ...
        private static DependencyProperty ValidateAndRegister(
            string name,
            System.Type propertyType,
            System.Type ownerType,
            PropertyMetadata defaultMetadata,
        ...
        )
        {
            ...
            bool isEvent = (
                typeof(System.Delegate).IsAssignableFrom(propertyType)
                && (
                    defaultMetadata == null
                    || (defaultMetadata.Options & DependencyPropertyOptions.DelegateProperty) == 0
                )
            );
            ...
            if (fieldInfo == null)
            {
                string error = SR.GetString(
                    (isEvent)
                        ? SR.Error_DynamicEventNotSupported
                        : SR.Error_DynamicPropertyNotSupported,
                    new object[] { ownerType.FullName, name }
                );
            ...
            lock (((ICollection)DependencyProperty.dependencyProperties).SyncRoot)
            {
                if (
                    ...
                    throw new InvalidOperationException(
                        SR.GetString(
                            SR.Error_DPAlreadyExist,
                            new object[] { name, ownerType.FullName }
                        )
                    );
        ...
        )
        {
            ...
            this.isEvent = (
                typeof(System.Delegate).IsAssignableFrom(this.propertyType)
                && (
                    this.defaultMetadata == null
                    || (this.defaultMetadata.Options & DependencyPropertyOptions.DelegateProperty)
                        == 0
                )
            );
        }
        ...
        {
            get { return this.knownIndex; }
            set { this.knownIndex = value; }
        }

        internal bool IsKnown
        {
            get { return (this.knownIndex != 0); }
        }
        ...
        internal PropertyValidity Validity
        {
            ...
            {
                return IsKnown
                    ? knownProperties[this.knownIndex].propertyValidity
