    ...
    public class ActivityTypeCodeDomSerializer : TypeCodeDomSerializer
    {
        ...
        )
        {
            ...
            foreach (CodeTypeMember member in typeDecl.Members)
            {
                ...
                if (
                    method != null
                    && method.Name.Equals(_initMethodName)
        ...
        public override CodeTypeDeclaration Serialize(
            IDesignerSerializationManager manager,
            object root,
            ICollection members
        )
        {
            ...
            CodeMemberMethod method =
                codeTypeDeclaration.UserData[_initMethodKey] as CodeMemberMethod;
            if (method != null && rootActivity is CompositeActivity)
            {
                ...
                beginInit.Left = new CodePropertyReferenceExpression(
                    new CodeThisReferenceExpression(),
                    "CanModifyActivities"
                );
                ...
                endInit.Left = new CodePropertyReferenceExpression(
                    new CodeThisReferenceExpression(),
                    "CanModifyActivities"
                );
            ...
            foreach (CodeTypeMember member in codeTypeDeclaration.Members)
            {
                ...
                if (field != null)
                {
                    foreach (object objectActivity in members)
                    {
                        ...
                        if (
                            field.Name == manager.GetName(activity)
                            && (int)activity.GetValue(ActivityMarkupSerializer.StartLineProperty)
                                != -1
                        ...
                        )
                        {
                            ...
                            field.LinePragma = new CodeLinePragma(
                                (string)
                                    rootActivity.GetValue(
                                        ActivityCodeDomSerializer.MarkupFileNameProperty
                                    ),
