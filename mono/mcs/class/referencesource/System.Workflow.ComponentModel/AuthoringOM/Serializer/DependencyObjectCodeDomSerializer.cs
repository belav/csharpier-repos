        ...
        public override object Serialize(IDesignerSerializationManager manager, object obj)
        {
            ...
            if (manager.Context == null)
                throw new ArgumentException(
                    "manager",
                    SR.GetString(SR.Error_MissingContextProperty)
                );
            ...
            try
            {
                if (activity != null)
                {
                    CodeDomSerializer componentSerializer =
                    ...
                    if (componentSerializer == null)
                        throw new InvalidOperationException(
                            SR.GetString(
                                ...
                                typeof(CodeDomSerializer).FullName
                            )
                        );
                    retVal =
                        componentSerializer.Serialize(manager, activity) as CodeStatementCollection;
                }
                else
                {
                ...
                if (retVal != null)
                {
                    ...
                    if (objectExpression != null)
                    {
                        ...
                        List<DependencyProperty> dependencyProperties =
                            new List<DependencyProperty>(dependencyObject.MetaDependencyProperties);
                        foreach (
                            DependencyProperty dp in dependencyObject.DependencyPropertyValues.Keys
                        )
                        {
                            if (dp.IsAttached)
                            {
                            ...
                            )
                            {
                                ...
                                if (fieldInfo != null && !fieldInfo.IsPublic)
                                    param1 = new CodeMethodInvokeExpression(
                                        ...
                                        "FromName",
                                        new CodePrimitiveExpression(dependencyProperty.Name),
                                        new CodeTypeOfExpression(dependencyProperty.OwnerType)
                                    );
                                else
                                    param1 = new CodeFieldReferenceExpression(
                                ...
                                if (param1 != null && param2 != null)
                                {
                                    ...
                                    if (value is ActivityBind)
                                        codeMethodInvokeExpr = new CodeMethodInvokeExpression(
                                            objectExpression,
                                            "SetBinding",
                                            new CodeExpression[]
                                            {
                                                ...
                                                new CodeCastExpression(
                                                    new CodeTypeReference(typeof(ActivityBind)),
                                                    param2
                                                ),
                                            }
                                        );
                                        ...
                                        codeMethodInvokeExpr = new CodeMethodInvokeExpression(
                                            objectExpression,
                                            (dependencyProperty.IsEvent)
                                                ? "AddHandler"
                                                : "SetValue",
                                            new CodeExpression[] { param1, param2 }
                                        );
                                    ...
                                    foreach (CodeStatement statement in codeStatements)
                                    {
                                        if (
                                            ...
                                            && ((CodeAssignStatement)statement).Left
                                        ...
                                        )
                                        {
                                            ...
                                            if (
                                                prop.PropertyName == dependencyProperty.Name
                                                && prop.TargetObject.Equals(objectExpression)
                                            )
                        ...
                        if (eventBindingService == null)
                        {
                            ...
                            // event handlers.
                            foreach (
                                EventDescriptor eventDesc in TypeDescriptor.GetEvents(
                                    dependencyObject
                                )
                            )
                            {
                                string handler =
                                    WorkflowMarkupSerializationHelpers.GetEventHandlerName(
                                        dependencyObject,
                                        eventDesc.Name
                                    );
                                if (!string.IsNullOrEmpty(handler))
                                {
                                    ...
                                    CodeDelegateCreateExpression listener =
                                        new CodeDelegateCreateExpression(
                                            new CodeTypeReference(eventDesc.EventType),
                        ...
                        {
                            Hashtable typeNames =
                                dependencyObject.UserData[UserDataKeys.DesignTimeTypeNames]
                                as Hashtable;
                            foreach (object key in typeNames.Keys)
                            {
                                ...
                                else if (key is string)
                                {
                                    ...
                                    Debug.Assert(
                                        indexOfDot != -1,
                                        "Wrong property name in DesignTimeTypeNames hashtable."
                                    );
                                    if (indexOfDot != -1)
                                    {
                                ...
                                if (
                                    ...
                                    && !string.IsNullOrEmpty(ownerTypeName)
                                )
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        CodeExpression param1 = new CodeFieldReferenceExpression(
                                            new CodeTypeReferenceExpression(ownerTypeName),
                                            propName + "Property"
                                        );
