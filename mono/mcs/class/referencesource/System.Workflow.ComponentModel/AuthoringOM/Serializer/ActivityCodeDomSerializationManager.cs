namespace System.Workflow.ComponentModel.Serialization
{
    ...
    public class ActivityCodeDomSerializationManager : IDesignerSerializationManager
    {
        ...
        public ActivityCodeDomSerializationManager(IDesignerSerializationManager manager)
        {
            ...
            this.serializationManager.SerializationComplete += new EventHandler(
                OnSerializationComplete
            );
            if (this.serializationManager is DesignerSerializationManager)
                ((DesignerSerializationManager)this.serializationManager).SessionDisposed +=
                    new EventHandler(OnSessionDisposed);
        }
        ...
        {
            add { this.serializationCompleteEventHandler += value; }
            remove { this.serializationCompleteEventHandler -= value; }
        }
        ...
        [SuppressMessage(
            ...
            MessageId = "System.String.LastIndexOf(System.String)",
            Justification = "This is not a security threat since it is called only in XAML design time scenarios"
        )]
        ...
        public object GetService(Type serviceType)
        {
            ...
            if (serviceType == typeof(IReferenceService))
                service = new ActivityCodeDomReferenceService(
                    this.serializationManager.GetService(serviceType) as IReferenceService
                );
        ...
        public object GetSerializer(Type objectType, Type serializerType)
        {
            ...
            )
            {
                ...
                {
                    if (
                        string.Equals(pd.Name, "SynchronizationHandles", StringComparison.Ordinal)
                        && TypeProvider.IsAssignable(typeof(Activity), pd.ComponentType)
                    )
                ...
                else
                {
                    ...
                    ExpressionContext context =
                        Context[typeof(ExpressionContext)] as ExpressionContext;
                    if (
                        context != null
                        && context.Expression is CodePropertyReferenceExpression
                        && string.Equals(
                            ((CodePropertyReferenceExpression)context.Expression).PropertyName,
        ...
        private bool UseUserDefinedSerializer(Type objectType, Type serializerType)
        {
            ...
            if (
                ...
                || typeof(Activity).IsAssignableFrom(objectType)
            )
                return true;
            ...
            object serializer = this.serializationManager.GetSerializer(objectType, serializerType);
            if (
                serializer.GetType().Assembly != typeof(CodeDomSerializer).Assembly
            ...
            if (
                activity != null
                && activity.Site != null
                && activity.Site.Container != null
                && objectType.Namespace != null
                && objectType.Namespace.Equals(typeof(System.Drawing.Image).Namespace)
            )
                return true;
        ...
        {
            add { this.resolveNameEventHandler += value; }
            remove { this.resolveNameEventHandler -= value; }
        }
        ...
        protected IDesignerSerializationManager SerializationManager
        {
            ...
            set { this.serializationManager = value; }
        }
    }
    #endregion
}
