        ...
        public void GetObjectData(object obj, SerializationInfo info, StreamingContext context)
        {
            if (Activity.ContextIdToActivityMap == null)
                throw new InvalidOperationException(
                    SR.GetString(SR.Error_ActivitySaveLoadNotCalled)
                );
            ...
            else if (!isDanglingActivity)
            {
                ...
                string[] names = null;
                MemberInfo[] members = FormatterServicesNoSerializableCheck.GetSerializableMembers(
                    obj.GetType(),
                    out names
                );
                ...
                {
                    Debug.Assert(
                        members[0].Name == "dependencyPropertyValues"
                            && members[1].Name == "disposed"
                    );
                    IDictionary<DependencyProperty, object> instanceProperties =
                        (IDictionary<DependencyProperty, object>)memberDatas[0];
                ...
                {
                    string wMarkup =
                        activity.GetValue(Activity.WorkflowXamlMarkupProperty) as string;
                    if (!string.IsNullOrEmpty(wMarkup))
                    {
                        ...
                        string rMarkup =
                            activity.GetValue(Activity.WorkflowRulesMarkupProperty) as string;
                        ...
                        if (changeActions != null)
                        {
                            ...
                            )
                            {
                                ...
                                )
                                {
                                    ...
                                    info.AddValue(
                                        "workflowChanges",
                                        changeActionsStringWriter.ToString()
                                    );
                                }
                            }
                        }
            ...
            do
            {
                ...
                if (!activity.Parent.Activities.Contains(activity))
                {
                    ...
                    if (
                        ...
                        || !activeContextActivities.Contains(activity)
                    )
                    {
                        isDangling = true;
                        break;
        ...
        private sealed class ActivitySerializedRef : IObjectReference, IDeserializationCallback
        {
            ...
            object IObjectReference.GetRealObject(StreamingContext context)
            {
                ...
                if (Activity.DefinitionActivity == null)
                {
                    ...
                    rootActivityDefinition = Activity.OnResolveActivityDefinition(
                        this.type,
                        this.workflowMarkup,
                        this.rulesMarkup,
                        createNewDef,
                    ...
                    if (createNewDef)
                    {
                        ArrayList changeActions = Activity.OnResolveWorkflowChangeActions(
                        ...
                        rootActivityDefinition.SetValue(
                            WorkflowChanges.WorkflowChangeActionsProperty,
                            changeActions
                        );
                        rootActivityDefinition.SetValue(
                ...
                if (this.cachedActivity != null)
                {
                    ...
                    MemberInfo[] members =
                        FormatterServicesNoSerializableCheck.GetSerializableMembers(
                            this.cachedActivity.GetType(),
                            out currentMemberNames
                        );
                    if (members.Length == 2)
                    {
                        Debug.Assert(
                        ...
                        if (this.memberData != null && this.disposed != null)
                        {
                            FormatterServices.PopulateObjectMembers(
                                this.cachedActivity,
                                members,
                                new object[] { this.memberData, this.disposed }
                            );
                            done = true;
                        }
                        else if (this.memberData != null)
                        {
                            FormatterServices.PopulateObjectMembers(
                                this.cachedActivity,
                                new MemberInfo[] { members[0] },
                                new object[] { this.memberData }
                            );
                            done = true;
                        }
                        else if (this.disposed != null)
                        {
                            FormatterServices.PopulateObjectMembers(
                                this.cachedActivity,
                                new MemberInfo[] { members[1] },
                                new object[] { this.disposed }
                            );
                            done = true;
                        }
                    }
                    ...
                    if (!done && this.memberDatas != null)
                    {
                        ...
                        for (int index = 0; index < currentMemberNames.Length; index++)
                            currentMemberDatas[index] = this.memberDatas[
                                Position(currentMemberNames[index])
                            ];
            ...
            private int Position(String name)
            {
                if (this.memberNames.Length > 0 && this.memberNames[this.lastPosition].Equals(name))
                {
                    ...
                    (++this.lastPosition < this.memberNames.Length)
                    && (this.memberNames[this.lastPosition].Equals(name))
                )
                {
                    return this.lastPosition;
                }
