        ...
        public override ValidationErrorCollection Validate(ValidationManager manager, object obj)
        {
            ...
            if (cancelHandlerCount > 1)
                validationErrors.Add(
                    new ValidationError(
                        SR.GetString(
                            SR.Error_MoreThanOneCancelHandler,
            ...
            if (exceptionHandlersCount > 1)
                validationErrors.Add(
                    new ValidationError(
                        SR.GetString(
                            SR.Error_MoreThanOneFaultHandlersActivityDecl,
                            compositeActivity.GetType().Name
                        ),
                        ErrorNumbers.Error_ScopeMoreThanOneFaultHandlersActivityDecl
                    )
                );
            ...
            if (compensationHandlerCount > 1)
                validationErrors.Add(
                    new ValidationError(
                        SR.GetString(
                            SR.Error_MoreThanOneCompensationDecl,
                            compositeActivity.GetType().Name
                        ),
                        ErrorNumbers.Error_ScopeMoreThanOneCompensationDecl
                    )
                );
            ...
            {
                foreach (
                    Activity childActivity in Helpers.GetAllEnabledActivities(compositeActivity)
                )
                    validationErrors.AddRange(
                        ValidationHelpers.ValidateActivity(manager, childActivity)
                    );
            }
