namespace System.Workflow.ComponentModel.Compiler
{
    ...
    public class DependencyObjectValidator : Validator
    {
        public override ValidationErrorCollection Validate(ValidationManager manager, object obj)
        {
            ...
            foreach (
                DependencyProperty prop in DependencyProperty.FromType(dependencyObject.GetType())
            )
            {
            ...
            foreach (DependencyProperty prop in allProperties)
            {
                ...
                if (validationVisibility != ValidationOption.None)
                    validationErrors.AddRange(
                        ValidateDependencyProperty(dependencyObject, prop, manager)
                    );
            }
        ...
        private ValidationErrorCollection ValidateDependencyProperty(
        ...
        )
        {
            ...
            if (activity == null)
                throw new InvalidOperationException(
                    SR.GetString(SR.Error_ContextStackItemMissing, typeof(Activity).FullName)
                );
            ...
            try
            {
                ...
                {
                    if (
                        !dependencyProperty.PropertyType.IsValueType
                    ...
                    )
                    {
                        errors.Add(
                            new ValidationError(
                                SR.GetString(
                                    SR.Error_PropertyDefaultIsReference,
                                    dependencyProperty.Name
                                ),
                                ErrorNumbers.Error_PropertyDefaultIsReference
                            )
                        );
                    }
                    else if (
                ...
                if (
                    propValue == null
                    || propValue == dependencyProperty.DefaultMetadata.DefaultValue
                )
                {
                    if (dependencyProperty.IsEvent)
                    {
                        ...
                        if (propValue == null)
                            propValue = WorkflowMarkupSerializationHelpers.GetEventHandlerName(
                                dependencyObject,
                                dependencyProperty.Name
                            );
                        ...
                        if (propValue is string && !string.IsNullOrEmpty((string)propValue))
                            errors.AddRange(
                                ValidateEvent(activity, dependencyProperty, propValue, manager)
                            );
                    }
                    else
                    {
                ...
                bool checkNotSet =
                    ...
                    || (
                        ...
                        && (((CompositeActivity)activity).EnabledActivities.Count != 0)
                    );

                if (
                    validationVisibility == ValidationOption.Required
                    && (
                        propValue == null
                        || (propValue is string && string.IsNullOrEmpty((string)propValue))
                    )
                ...
                {
                    errors.Add(
                        ValidationError.GetNotSetValidationError(GetFullPropertyName(manager))
                    );
                }
                else if (propValue != null)
                {
                    ...
                    else if (dependencyProperty.ValidatorType != null)
                    {
                        ...
                        {
                            validator =
                                Activator.CreateInstance(dependencyProperty.ValidatorType)
                                as Validator;
                            if (validator == null)
                                errors.Add(
                                    new ValidationError(
                                        SR.GetString(
                                            SR.Error_CreateValidator,
                                            dependencyProperty.ValidatorType.FullName
                                        ),
                                        ErrorNumbers.Error_CreateValidator
                                    )
                                );
                            else
        ...
        private ValidationErrorCollection ValidateEvent(
            Activity activity,
            DependencyProperty dependencyProperty,
            object propValue,
            ValidationManager manager
            ...
            if (propValue is string && !string.IsNullOrEmpty((string)propValue))
            {
                ...
                if (!handlerExists)
                {
                    ValidationError error = new ValidationError(
                        SR.GetString(
                            SR.Error_CantResolveEventHandler,
                            dependencyProperty.Name,
                            propValue as string
                        ),
                        ErrorNumbers.Error_CantResolveEventHandler
                    );
