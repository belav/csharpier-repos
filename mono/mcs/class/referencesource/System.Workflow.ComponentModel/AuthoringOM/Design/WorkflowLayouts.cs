namespace System.Workflow.ComponentModel.Design
{
    ...
    internal abstract class DefaultWorkflowLayout : WorkflowLayout
    {
        ...
        public override Size Extent
        {
            get
            {
                Size rootDesignerSize =
                    (this.parentView.RootDesigner != null)
                        ? this.parentView.RootDesigner.Size
                        : Size.Empty;
                Size totalSize = new Size(
                    rootDesignerSize.Width + DefaultWorkflowLayout.Separator.Width * 2,
                    rootDesignerSize.Height + DefaultWorkflowLayout.Separator.Height * 2
                );
                Size clientSize = this.parentView.ViewPortSize;
                return new Size(
                    Math.Max(totalSize.Width, clientSize.Width),
        ...
        {
            get { return new Point(DefaultWorkflowLayout.Separator); }
        }
        ...
        public override void OnPaint(PaintEventArgs e, ViewPortData viewPortData)
        {
            ...
            if (
                this.parentView.RootDesigner != null
                && this.parentView.RootDesigner.Bounds.Width >= 0
                && this.parentView.RootDesigner.Bounds.Height >= 0
            )
            {
    ...
    internal sealed class ActivityRootLayout : DefaultWorkflowLayout
    {
        ...
        public override void OnPaint(PaintEventArgs e, ViewPortData viewPortData)
        {
            ...
            )
            {
                ...
                rootBounds.Inflate(
                    ActivityRootLayout.Separator.Width / 2,
                    ActivityRootLayout.Separator.Height / 2
                );
                ActivityDesignerPaint.DrawDropShadow(
                    graphics,
                    rootBounds,
                    AmbientTheme.WorkflowBorderPen.Color,
    ...
    internal sealed class WorkflowRootLayout : DefaultWorkflowLayout
    {
        ...
        private Size Offset
        {
            get
            {
                ...
                return new Size(
                    Math.Max(0, (totalSize.Width - layoutExtent.Width) / 2),
                    Math.Max(0, (totalSize.Height - layoutExtent.Height) / 2)
                );
            }
        }
        #endregion
    ...
    internal sealed class PrintPreviewLayout : WorkflowLayout
    {
        ...
        internal PrintPreviewLayout(
            IServiceProvider serviceProvider,
            WorkflowPrintDocument printDoc
        )
            : base(serviceProvider)
        {
        ...
        public override Size Extent
        {
            get
            {
                ...
                maxSize.Width =
                    (this.rowColumns.Width * this.pageSize.Width)
                    + ((this.rowColumns.Width + 1) * (PageSeparator.Width));
                maxSize.Height =
                    (this.rowColumns.Height * this.pageSize.Height)
                    + ((this.rowColumns.Height + 1) * (PageSeparator.Height));
                return maxSize;
            }
        }
        ...
        public override Point RootDesignerAlignment
        {
            get
            {
                ...
                Size printableAreaPerPage = new Size(
                    this.pageSize.Width - (PageMargins.Left + PageMargins.Right),
                    this.pageSize.Height - (PageMargins.Top + PageMargins.Bottom)
                );
                Size totalPrintableArea = new Size(
                    this.rowColumns.Width * printableAreaPerPage.Width,
            ...
            foreach (PageLayoutData pageLayoutData in this.pageLayoutInfo)
            {
                ...
                if (!intersectedPhysicalViewPort.IsEmpty)
                {
                    Point deltaLocation = new Point(
                        intersectedPhysicalViewPort.X - pageLayoutData.ViewablePageBounds.X,
                        intersectedPhysicalViewPort.Y - pageLayoutData.ViewablePageBounds.Y
                    );

                    Size deltaSize = new Size(
                        pageLayoutData.ViewablePageBounds.Width - intersectedPhysicalViewPort.Width,
                        pageLayoutData.ViewablePageBounds.Height
                            - intersectedPhysicalViewPort.Height
                    );
        ...
        public override Rectangle MapOutRectangleFromLayout(Rectangle logicalRectangle)
        {
            ...
            foreach (PageLayoutData pageLayoutData in this.pageLayoutInfo)
            {
                ...
                {
                    Point deltaLocation = new Point(
                        intersectedLogicalViewPort.X - pageLayoutData.LogicalPageBounds.X,
                        intersectedLogicalViewPort.Y - pageLayoutData.LogicalPageBounds.Y
                    );

                    Size deltaSize = new Size(
                        pageLayoutData.LogicalPageBounds.Width - intersectedLogicalViewPort.Width,
                    ...
                    transformedViewPort =
                        (transformedViewPort.IsEmpty)
                            ...
                            : Rectangle.Union(transformedViewPort, insersectedPhysicalViewPort);
                }
            }
            ...
            foreach (PageLayoutData pageLayoutData in this.pageLayoutInfo)
            {
                if (pageLayoutData.PageBounds.Contains(logicalPoint))
                {
                    Point delta = new Point(
                        logicalPoint.X - pageLayoutData.ViewablePageBounds.Left,
                        logicalPoint.Y - pageLayoutData.ViewablePageBounds.Top
                    );
                    logicalPoint = new Point(
                        pageLayoutData.LogicalPageBounds.Left + delta.X,
        ...
        public override Point MapOutCoOrdFromLayout(Point logicalPoint)
        {
            foreach (PageLayoutData pageLayoutData in this.pageLayoutInfo)
            {
                ...
                {
                    Point delta = new Point(
                        logicalPoint.X - pageLayoutData.LogicalPageBounds.Left,
                        logicalPoint.Y - pageLayoutData.LogicalPageBounds.Top
                    );
                    logicalPoint = new Point(
                        pageLayoutData.ViewablePageBounds.Left + delta.X,
        ...
        public override void OnPaint(PaintEventArgs e, ViewPortData viewPortData)
        {
            ...
            coOrdTxMatrix.Scale(
                viewPortData.Scaling.Width / (float)this.parentView.Zoom * 100.0f,
                viewPortData.Scaling.Height / (float)this.parentView.Zoom * 100.0f
            );

            //Make sure that we now clear the translation factor
            Matrix graphicsMatrics = new Matrix();
            graphicsMatrics.Scale(
        ...
        public override void OnPaintWorkflow(PaintEventArgs e, ViewPortData viewPortData)
        {
            ...
            using (
                Font headerFooterFont = new Font(
                    ambientTheme.Font.FontFamily,
                    ambientTheme.Font.Size / this.scaling,
            ...
            )
            {
                ...
                WorkflowPrintDocument.HeaderFooterData headerFooterData =
                    new WorkflowPrintDocument.HeaderFooterData();
                ...
                foreach (PageLayoutData pageLayoutData in this.pageLayoutInfo)
                {
                    ...
                    //will not contain any logical bitmap areas in which case we atleast need to draw the pages properly
                    if (
                        !pageLayoutData.PageBounds.IntersectsWith(physicalViewPort)
                        || pageLayoutData.PageBounds.Width <= 0
                        || pageLayoutData.PageBounds.Height <= 0
                    )
                        continue;
                    ...
                    if (!intersectedViewPort.IsEmpty)
                    {
                        ...
                        bitmapDrawingPoint.X =
                            pageLayoutData.ViewablePageBounds.X
                            + Math.Abs(pageLayoutData.LogicalPageBounds.X - intersectedViewPort.X);
                        bitmapDrawingPoint.Y =
                            pageLayoutData.ViewablePageBounds.Y
                            + Math.Abs(pageLayoutData.LogicalPageBounds.Y - intersectedViewPort.Y);
                        points = new Point[] { bitmapDrawingPoint };
                        coOrdTxMatrix.TransformPoints(points);
                        bitmapDrawingPoint = new Point(
                            points[0].X - viewPortData.Translation.X,
                        ...
                        ActivityDesignerPaint.DrawImage(
                            graphics,
                            memoryBitmap,
                            new Rectangle(bitmapDrawingPoint, viewPortBitmapArea.Size),
                            viewPortBitmapArea,
                            DesignerContentAlignment.Fill,
                            1.0f,
                            WorkflowTheme.CurrentTheme.AmbientTheme.DrawGrayscale
                    ...
                    graphics.DrawRectangle(
                        ambientTheme.ForegroundPen,
                        pageLayoutData.ViewablePageBounds.Left - 3,
                        pageLayoutData.ViewablePageBounds.Top - 3,
                        pageLayoutData.ViewablePageBounds.Width + 6,
                        pageLayoutData.ViewablePageBounds.Height + 6
                    );
        ...
        public override void Update(Graphics graphics, LayoutUpdateReason reason)
        {
            ...
            else
            {
                ...
                PointF scaleFactor = new PointF(
                    (
                        (float)this.printDocument.PageSetupData.PagesWide
                        * (float)printableArea.Width
                        / (float)rootDesignerSize.Width
                    ),
                    (
                        (float)this.printDocument.PageSetupData.PagesTall
            ...
            this.pageSize.Width = Convert.ToInt32(
                Math.Ceiling(((float)this.pageSize.Width) / this.scaling)
            );
            this.pageSize.Height = Convert.ToInt32(
            ...
            IDesignerOptionService designerOptionService =
                this.serviceProvider.GetService(typeof(IDesignerOptionService))
                as IDesignerOptionService;
            if (designerOptionService != null)
            {
                object separator = designerOptionService.GetOptionValue(
                    "WinOEDesigner",
                    "PageSeparator"
                );
                PageSeparator =
            ...
            PageSeparator = new Size(
                Convert.ToInt32(Math.Ceiling(((float)PageSeparator.Width) / this.scaling)),
            ...
            this.headerFooterMargins.Top = Convert.ToInt32(
                (float)this.printDocument.PageSetupData.HeaderMargin / this.scaling
            );
            this.headerFooterMargins.Bottom = Convert.ToInt32(
                (float)this.printDocument.PageSetupData.FooterMargin / this.scaling
            );
            this.previewTime = DateTime.Now;

            //STEP6: Calculate the the row columns
            Size viewablePageSize = new Size(
                this.pageSize.Width - (PageMargins.Left + PageMargins.Right),
                this.pageSize.Height - (PageMargins.Top + PageMargins.Bottom)
            );
            ...
            for (int row = 0; row < this.rowColumns.Height; row++)
            {
                for (int column = 0; column < this.rowColumns.Width; column++)
                {
                    ...
                    Rectangle logicalBounds = new Rectangle(
                        column * viewablePageSize.Width,
                        row * viewablePageSize.Height,
                        viewablePageSize.Width,
                        viewablePageSize.Height
                    );
                    Rectangle pageBounds = new Rectangle(pageLocation, this.pageSize);
                    Rectangle viewablePageBounds = new Rectangle(
        ...
        private Margins GetAdjustedMargins(Graphics graphics)
        {
            ...
            Margins adjustedMargins = new Margins(
                Math.Max(margins.Left, hardMargins.Left),
        ...
        {
            get { return this.pageSeparator; }
            set { this.pageSeparator = value; }
        }

        private Margins PageMargins
        {
            get { return this.pageMargins; }
            set { this.pageMargins = value; }
        }
        #endregion
        ...
        private struct PageLayoutData
        {
            ...
            public PageLayoutData(
                Rectangle logicalPageBounds,
                Rectangle pageBounds,
                Rectangle viewablePageBounds,
                Point rowColumnPos
            )
            {
