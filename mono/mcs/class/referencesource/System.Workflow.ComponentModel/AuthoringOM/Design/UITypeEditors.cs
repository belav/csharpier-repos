    ...
    public class TypeBrowserEditor : UITypeEditor
    {
        ...
        )
        {
            ...
            if (editorService != null)
            {
                ...
                if (
                    typeDescriptorContext.PropertyDescriptor != null
                    && typeDescriptorContext.PropertyDescriptor.Attributes != null
                )
                    typeFilterProvAttr =
                        typeDescriptorContext.PropertyDescriptor.Attributes[
                            typeof(TypeFilterProviderAttribute)
                ...
                if (typeFilterProvider == null)
                    typeFilterProvider =
                        (
                            (typeDescriptorContext.Instance is object[])
                                ? ((object[])typeDescriptorContext.Instance)[0]
                ...
                {
                    typeFilterProvider =
                        typeDescriptorContext.PropertyDescriptor as ITypeFilterProvider;
                }
                ...
                using (
                    TypeBrowserDialog dlg = new TypeBrowserDialog(
                        serviceProvider,
                        typeFilterProvider as ITypeFilterProvider,
                        oldTypeName
                    )
                )
                {
    ...
    public class BindUITypeEditor : UITypeEditor
    {
        ...
        )
        {
            ...
            if (context != null && context.PropertyDescriptor is DynamicPropertyDescriptor)
            {
                ...
                {
                    using (
                        ActivityBindForm bindDialog = new ActivityBindForm(
                    ...
                    )
                    {
                        if (DialogResult.OK == bindDialog.ShowDialog())
                        {
                            ...
                            if (bindDialog.CreateNew)
                            {
                                    ...
                                    if (properties != null)
                                    {
                                        properties.Add(
                                            CustomProperty.CreateCustomProperty(
                                                this.serviceProvider,
                                                bindDialog.NewMemberName,
                                                context.PropertyDescriptor,
                                                context.Instance
                                            )
                                        );
                                        CustomActivityDesignerHelper.SetCustomProperties(
                                            properties,
                                            context
                                        );
                                    }
                                ...
                                {
                                    ActivityBindPropertyDescriptor.CreateField(
                                        context,
                                        bindDialog.Binding,
                                        true
                                    );
            ...
            else
            {
                DesignerHelpers.ShowError(
                    this.serviceProvider,
                    SR.GetString(SR.Error_MultipleSelectNotSupportedForBindAndPromote)
                );
            }
