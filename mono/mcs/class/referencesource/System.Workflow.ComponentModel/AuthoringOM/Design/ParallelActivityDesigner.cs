...
namespace System.Workflow.ComponentModel.Design
{
    ...
    public class ParallelActivityDesigner : StructuredCompositeActivityDesigner
    {
        ...
        public override object LastSelectableObject
        {
            get
            {
                ...
                object firstComponent =
                    (ContainedDesigners.Count > 0) ? ContainedDesigners[0].Activity : null;
                CompositeActivityDesigner firstDesigner =
                    (firstComponent is Activity)
                        ? ActivityDesigner.GetDesigner(firstComponent as Activity)
        ...
        )
        {
            ...
            int index =
                (activityDesigner != null) ? containedDesigners.IndexOf(activityDesigner) : -1;
            if (
                direction == DesignerNavigationDirection.Left
                && index >= 0
                && index < containedDesigners.Count
            )
                nextObject = (
                    (ActivityDesigner)
        ...
        protected override void OnLayoutPosition(ActivityDesignerLayoutEventArgs e)
        {
            ...
            if (Expanded && ActiveDesigner == this)
            {
                ...
                if (compositeDesignerTheme != null)
                {
                    ...
                    Point location =
                        (connectionPoints.Count > 0)
                            ? new Point(Location.X, connectionPoints[0].Y)
                            : Location;
        ...
        protected override Size OnLayoutSize(ActivityDesignerLayoutEventArgs e)
        {
            ...
            if (Expanded && ActiveDesigner == this && compositeDesignerTheme != null)
            {
                ...
                foreach (ActivityDesigner activityDesigner in ContainedDesigners)
                    activityDesigner.Size = new Size(
                        activityDesigner.Size.Width,
        ...
        protected override Rectangle[] GetDropTargets(Point dropPoint)
        {
            ...
            if (containedDesigners.Count > 0)
            {
                ...
                for (int i = 0; i < containedDesigners.Count - 1; i++)
                {
                    ...
                    dropTargets[i + 1].Location = new Point(
                        designerBounds1.Right
                            + (designerBounds2.Left - designerBounds1.Right) / 2
                            - ((designerTheme != null) ? designerTheme.ConnectorSize.Width : 0) / 2,
                        designerBounds1.Top
                    );
                    dropTargets[i + 1].Size = new Size(
                        ((designerTheme != null) ? designerTheme.ConnectorSize.Width : 0),
                        designerBounds1.Height
                    );
                }

                //Add last connector
                ActivityDesigner lastDesigner = containedDesigners[containedDesigners.Count - 1];
                dropTargets[containedDesigners.Count].Location = new Point(
                    bounds.Right
                        - ((designerTheme != null) ? designerTheme.ConnectorSize.Width : 0),
                    lastDesigner.Location.Y
                );
                dropTargets[containedDesigners.Count].Size = new Size(
                    ((designerTheme != null) ? designerTheme.ConnectorSize.Width : 0),
                    lastDesigner.Size.Height
                );
            }
            else
            {
                dropTargets[0].Location = new Point(
                    Location.X
                        + (
                            Size.Width
                            - ((designerTheme != null) ? designerTheme.ConnectorSize.Width : 0)
                ...
                dropTargets[0].Size = new Size(
                    ((designerTheme != null) ? designerTheme.ConnectorSize.Width : 0),
                    Location.Y + Size.Height - dropTargets[0].Location.Y
                );
            }
        ...
        private void DrawParallelDropTargets(ActivityDesignerPaintEventArgs e, int index)
        {
            ...
            if (containedDesigners.Count > 0)
            {
                ...
                {
                    ActivityDesigner firstDesigner = containedDesigners[0];
                    ReadOnlyCollection<Point> firstDesignerConnections =
                        firstDesigner.GetConnections(DesignerEdges.Top | DesignerEdges.Bottom);
            ...
            if (compositeActivity != null && branchActivity != null)
            {
                ...
                CompositeActivityDesigner.InsertActivities(
                    ...
                    new ConnectorHitTestInfo(
                        ...
                        compositeActivity.Activities.Count
                    ),
                    new List<Activity>(new Activity[] { branchActivity }).AsReadOnly(),
                    DR.GetString(DR.AddingBranch, branchActivity.GetType().Name)
                );
