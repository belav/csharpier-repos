namespace System.Workflow.ComponentModel.Design
{
    ...
    internal sealed class IDPropertyDescriptor : DynamicPropertyDescriptor
    {
        ...
        public override void SetValue(object component, object value)
        {
            ...
            if (activity != null)
            {
                ...
                if (site == null)
                    throw new Exception(
                        SR.GetString(SR.General_MissingService, typeof(ISite).FullName)
                    );

                IIdentifierCreationService identifierCreationService =
                    site.GetService(typeof(IIdentifierCreationService))
                    as IIdentifierCreationService;
    ...
    {
        internal NamePropertyDescriptor(
            IServiceProvider serviceProvider,
            PropertyDescriptor actualPropDesc
        )
            : base(serviceProvider, actualPropDesc) { }
        ...
        {
            get { return SR.GetString(SR.Activity); }
        }

        public override string Description
        {
            get { return SR.GetString(SR.RootActivityNameDesc); }
        }
        ...
        public override void SetValue(object component, object value)
        {
            ...
            if (activity != null)
            {
                ...
                IIdentifierCreationService identifierCreationService =
                    activity.Site.GetService(typeof(IIdentifierCreationService))
                    as IIdentifierCreationService;
                if (identifierCreationService == null)
                    throw new Exception(
                        SR.GetString(
                ...
                bool isVB = (
                    CompilerHelpers.GetSupportedLanguage(activity.Site) == SupportedLanguages.VB
                );
                Type designedType = Helpers.GetDataSourceClass(
                    Helpers.GetRootActivity(activity),
                    activity.Site
                );
                if (designedType != null)
                {
                    MemberInfo matchingMember = ActivityBindPropertyDescriptor.FindMatchingMember(
                        name,
                        designedType,
                        isVB
                    );
                ...
                IMemberCreationService memberCreationService =
                    activity.Site.GetService(typeof(IMemberCreationService))
                    as IMemberCreationService;
                if (memberCreationService == null)
                    throw new InvalidOperationException(
                        SR.GetString(
                            SR.General_MissingService,
                            typeof(IMemberCreationService).FullName
                        )
                    );
                ...
                memberCreationService.UpdateTypeName(
                    ((Activity)host.RootComponent).GetValue(WorkflowMarkupSerializer.XClassProperty)
                        as string,
                    newClassName
                );
    ...
    {
        internal static PropertyDescriptorCollection FilterProperties(
            IServiceProvider serviceProvider,
        ...
        internal static void FilterProperties(
            IServiceProvider serviceProvider,
            object propertyOwner,
            IDictionary props
        )
        {
            ...
            {
                foreach (
                    PropertyDescriptor property in GetPropertiesForEvents(
                        serviceProvider,
                        propertyOwner
                    )
                )
        ...
        )
        {
            ...
            foreach (object key in properties.Keys)
            {
                ...
                if (
                    string.Equals(propDesc.Name, "Name", StringComparison.Ordinal)
                    && typeof(Activity).IsAssignableFrom(propDesc.ComponentType)
                )
                {
                    //Activity id
                    ...
                    !(propDesc is ActivityBindPropertyDescriptor)
                    && ActivityBindPropertyDescriptor.IsBindableProperty(propDesc)
                )
                {
                    if (
                        typeof(Type).IsAssignableFrom(propDesc.PropertyType)
                        && !(propDesc is ParameterInfoBasedPropertyDescriptor)
                    )
                        propDesc = new TypePropertyDescriptor(serviceProvider, propDesc);
                    newProperties[key] = new ActivityBindPropertyDescriptor(
                        serviceProvider,
                        propDesc,
                        propertyOwner
                    );
        ...
        internal static PropertyDescriptorCollection GetPropertiesForEvents(
            IServiceProvider serviceProvider,
            object eventOwner
        )
        {
            ...
            IEventBindingService eventBindingService =
                serviceProvider.GetService(typeof(IEventBindingService)) as IEventBindingService;
                ...
                foreach (EventDescriptor eventDesc in TypeDescriptor.GetEvents(eventOwner))
                {
                    if (eventDesc.IsBrowsable)
                    {
                        PropertyDescriptor propertyDescriptor =
                            eventBindingService.GetEventProperty(eventDesc);
                        if (
                            !(propertyDescriptor is ActivityBindPropertyDescriptor)
                            && ActivityBindPropertyDescriptor.IsBindableProperty(propertyDescriptor)
                        )
                            properties.Add(
                                new ActivityBindPropertyDescriptor(
                                    serviceProvider,
                                    propertyDescriptor,
                                    eventOwner
                                )
    ...
    internal class ActivityBindPropertyDescriptor : DynamicPropertyDescriptor
    {
        ...
        internal ActivityBindPropertyDescriptor(
            IServiceProvider serviceProvider,
            PropertyDescriptor realPropertyDescriptor,
            object propertyOwner
        )
        ...
        {
            get { return RealPropertyDescriptor.IsReadOnly; }
        }
        ...
        public override void SetValue(object component, object value)
        {
            ...
            {
                ComponentChangeDispatcher componentChangeDispatcher = new ComponentChangeDispatcher(
                    ServiceProvider,
                    dependencyObj,
                    this
                );
                try
                {
                    if (dependencyProperty.IsEvent && ServiceProvider != null)
                    {
                        ...
                        if (eventBindingService != null)
                        {
                            EventDescriptor eventDescriptor = eventBindingService.GetEvent(
                                RealPropertyDescriptor
                            );
            ...
            else
            {
                if (
                    ...
                    && dependencyObj.IsBindingSet(dependencyProperty)
                )
                {
                    ComponentChangeDispatcher componentChangeDispatcher =
                        new ComponentChangeDispatcher(ServiceProvider, dependencyObj, this);
                    try
                    {
        ...
        internal static IList<MemberInfo> GetBindableMembers(
            object obj,
        ...
        )
        {
            ...
            Type objectType =
                (obj == rootActivity)
                    ? Helpers.GetDataSourceClass(rootActivity, context)
                    : obj.GetType();

            Type memberType = PropertyDescriptorUtils.GetBaseType(
            ...
            if (objectType != null && memberType != null)
            {
                DependencyProperty dependencyProperty = DependencyProperty.FromName(
                    ...
                    context.PropertyDescriptor.ComponentType
                );
                ...
                foreach (MemberInfo memberInfo in objectType.GetMembers(bindingFlags))
                {
                    ...
                    object[] nonUserCodeAttributes = memberInfo.GetCustomAttributes(
                        typeof(System.Diagnostics.DebuggerNonUserCodeAttribute),
                        false
                    );
                    if (
                        nonUserCodeAttributes != null
                        && nonUserCodeAttributes.Length > 0
                        && nonUserCodeAttributes[0]
                            is System.Diagnostics.DebuggerNonUserCodeAttribute
                    )
                        continue;

                    object[] browsableAttributes = memberInfo.GetCustomAttributes(
                    ...
                    if (browsableAttributes.Length > 0)
                    {
                        ...
                        BrowsableAttribute browsableAttribute =
                            browsableAttributes[0] as BrowsableAttribute;
                        ...
                        else
                        {
                            try
                            {
                                AttributeInfoAttribute attributeInfoAttribute =
                                    browsableAttributes[0] as AttributeInfoAttribute;
                                if (
                                    attributeInfoAttribute != null
                    ...
                    if (
                        memberInfo.DeclaringType == typeof(System.Object)
                        && (
                            string.Equals(memberInfo.Name, "Equals", StringComparison.Ordinal)
                            || string.Equals(
                                memberInfo.Name,
                                "ReferenceEquals",
                    ...
                    if (includeEvents && memberInfo is EventInfo)
                    {
                        ...
                        isProtectedOrPublicMember = (
                            (addAccessor != null && addAccessor.IsFamily)
                            || (removeAccessor != null && removeAccessor.IsFamily)
                            || (addAccessor != null && addAccessor.IsPublic)
                            || (removeAccessor != null && removeAccessor.IsPublic)
                        );
                        isInternalMember = (
                            (addAccessor != null && addAccessor.IsAssembly)
                            || (removeAccessor != null && removeAccessor.IsAssembly)
                        );

                        addMember = TypeProvider.IsAssignable(
                            memberType,
                    ...
                    else if (memberInfo is PropertyInfo)
                    {
                        ...
                        isProtectedOrPublicMember = (
                            (getAccessor != null && getAccessor.IsFamily)
                            || (setAccessor != null && setAccessor.IsFamily)
                            || (getAccessor != null && getAccessor.IsPublic)
                            || (setAccessor != null && setAccessor.IsPublic)
                        );
                        isInternalMember = (
                            (getAccessor != null && getAccessor.IsAssembly)
                            || (setAccessor != null && setAccessor.IsAssembly)
                        );
                        addMember = (
                            getAccessor != null
                            && TypeProvider.IsAssignable(memberType, propertyInfo.PropertyType)
                        );
                    }

                    //We only want to allow binding to protected, public and internal members of baseType
                    if (
                        memberInfo.DeclaringType != objectType
                        && !isProtectedOrPublicMember
                        && !(memberInfo.DeclaringType.Assembly == null && isInternalMember)
                    )
                        addMember = false;
        ...
        )
        {
            ...
            {
                Type boundType = PropertyDescriptorUtils.GetBaseType(
                    context.PropertyDescriptor,
                    context.Instance,
                    context
                );
                ...
                if (activity != null && boundType != null)
                {
                    ...
                    {
                        bool isVB = (
                            CompilerHelpers.GetSupportedLanguage(context) == SupportedLanguages.VB
                        );
                        ...
                        if (designedType != null)
                        {
                            ...
                            MemberInfo matchingMember =
                                ActivityBindPropertyDescriptor.FindMatchingMember(
                                    fieldName,
                                    designedType,
                                    isVB
                                );
                            if (matchingMember != null)
                            {
                                ...
                                else if (matchingMember is PropertyInfo)
                                {
                                    MethodInfo getMethod = (
                                        (PropertyInfo)matchingMember
                                    ).GetGetMethod();
                                    MethodInfo setMethod = (
                                        (PropertyInfo)matchingMember
                                    ).GetSetMethod();
                                    isPrivate = (
                                        (getMethod != null && getMethod.IsPrivate)
                                ...
                                { //need to find the type of the member the path references (and if the path is valid at all)
                                    ...
                                    pathWalker.MemberFound += delegate(
                                        object sender,
                                        PathMemberInfoEventArgs eventArgs
                                    )
                                    {
                                    ...
                                    if (
                                        pathWalker.TryWalkPropertyPath(
                                            designedType,
                                            activityBind.Path
                                        )
                                    )
                                    {
                                        ...
                                        memberType = BindHelpers.GetMemberType(
                                    ...
                                    else
                                    {
                                        ...
                                        if (throwOnError)
                                            throw new InvalidOperationException(
                                                SR.GetString(
                                                    SR.Error_MemberWithSameNameExists,
                                                    activityBind.Path,
                                ...
                                if (
                                    (matchingMember.DeclaringType == designedType || !isPrivate)
                            ...
                            else
                            {
                                ...
                                else if (activity is CompositeActivity)
                                {
                                    ...
                                    {
                                        foreach (
                                            Activity existingActivity in Helpers.GetAllNestedActivities(
                            ...
                            IMemberCreationService memberCreationService =
                                context.GetService(typeof(IMemberCreationService))
                                as IMemberCreationService;
                            if (memberCreationService == null)
                            {
                                if (throwOnError)
                                    ...
                                    if (throwOnError)
                                        throw new InvalidOperationException(
                                            SR.GetString(
                                                ...
                                                typeof(IDesignerHost).FullName
                                            )
                                        );
                                }
                                else
                                {
                                    memberCreationService.CreateField(
                ...
                else
                {
                    if (activity == null && throwOnError)
                        throw new InvalidOperationException(
                            SR.GetString(SR.Error_InvalidActivityIdentifier, activityBind.Name)
                        );

                    if (boundType == null && throwOnError)
                        throw new InvalidOperationException(
                            SR.GetString(
                                SR.Error_PropertyTypeNotDefined,
                                context.PropertyDescriptor.Name,
                                typeof(ActivityBind).Name,
                                typeof(IDynamicPropertyTypeProvider).Name
                            )
                        );
                }
            }
        ...
        internal static bool IsBindableProperty(PropertyDescriptor propertyDescriptor)
        {
            ...
            if (
                typeof(DependencyObject).IsAssignableFrom(propertyDescriptor.ComponentType)
                && dependencyProperty != null
    ...
    internal sealed class ActivityBindNamePropertyDescriptor : DynamicPropertyDescriptor
    {
        ...
        public override void SetValue(object component, object value)
        {
            ...
            if (String.IsNullOrEmpty(id))
                throw new InvalidOperationException(
                    SR.GetString(SR.Error_ActivityIdentifierCanNotBeEmpty)
                );
            ...
            if (activity != null)
            {
                if (Helpers.ParseActivityForBind(activity, id) == null)
                    throw new InvalidOperationException(
                        SR.GetString(SR.Error_InvalidActivityIdentifier, id)
                    );
            }
    ...
    internal sealed class ReadonlyTypeDescriptonProvider : TypeDescriptionProvider
    {
        ...
        public override ICustomTypeDescriptor GetTypeDescriptor(Type type, object instance)
        {
            ...
            ICustomTypeDescriptor readonlyTypeDescriptor = new ReadonlyTypeDescriptor(
                realTypeDescriptor
            );
    ...
    internal sealed class ReadonlyTypeDescriptor : CustomTypeDescriptor
    {
        ...
        public override PropertyDescriptorCollection GetProperties()
        {
            ...
            foreach (PropertyDescriptor property in properties)
            {
                BrowsableAttribute browsable =
                    property.Attributes[typeof(BrowsableAttribute)] as BrowsableAttribute;
                if (
                    browsable != null
                    && browsable.Browsable
                    && !(property is ReadonlyPropertyDescriptor)
                )
        ...
        public override EventDescriptorCollection GetEvents(Attribute[] attributes)
        {
            ...
            {
                BrowsableAttribute browsable =
                    e.Attributes[typeof(BrowsableAttribute)] as BrowsableAttribute;
    ...
    internal sealed class ReadonlyEventDescriptor : EventDescriptor
    {
        ...
        public override string Category
        {
            get { return this.realEventDescriptor.Category; }
        }
        public override AttributeCollection Attributes
        {
            get
        ...
        public override string Description
        {
            get { return this.realEventDescriptor.Description; }
        }
        public override Type ComponentType
        {
            get { return this.realEventDescriptor.ComponentType; }
        }
        public override Type EventType
        {
            get { return this.realEventDescriptor.EventType; }
        }
        public override bool IsMulticast
        {
            get { return this.realEventDescriptor.IsMulticast; }
        }
