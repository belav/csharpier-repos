namespace System.Workflow.ComponentModel.Design
{
    ...
    internal sealed class FreeFormDragDropManager : DragDropManager
    {
        ...
        private static Cursor DragMoveCursor = new Cursor(
            typeof(WorkflowView),
            "Resources.DragMoveCursor.cur"
        );
        private static Cursor DragCopyCursor = new Cursor(
            typeof(WorkflowView),
            "Resources.DragCopyCursor.cur"
        );
        private static Cursor MoveCursor = new Cursor(
            typeof(WorkflowView),
            "Resources.MoveCursor.cur"
        );
        ...
        protected override bool OnDragOver(DragEventArgs eventArgs)
        {
            ...
            if (this.draggedDesignerImages != null)
            {
                Point[] previousLocations = GetDesignerLocations(
                    DragInitiationPoint,
                    this.movedDesignerImagePoint,
                    DraggedActivities
                );
            ...
            if (this.draggedDesignerImages != null)
            {
                ...
                Point[] newLocations = GetDesignerLocations(
                    DragInitiationPoint,
                    this.movedDesignerImagePoint,
                    DraggedActivities
                );
        ...
        protected override bool OnMouseMove(MouseEventArgs eventArgs)
        {
            ...
            if (eventArgs.Button == MouseButtons.None)
            {
                ...
                showMoveCursor |= (
                    MessageHitTestContext != null
                    && MessageHitTestContext.AssociatedDesigner != null
                    && ActivityDesigner.GetParentDesigner(
                        MessageHitTestContext.AssociatedDesigner.Activity
                    ) is FreeformActivityDesigner
                    && (MessageHitTestContext.HitLocation & HitTestLocations.ActionArea) == 0
                );
        ...
        )
        {
            ...
            if (
                ...
                || this.draggedDesignerImages.Count == 0
        ...
        )
        {
            ...
            )
            {
                ...
                )
                {
                    ...
                    Point[] locations = GetDesignerLocations(
                        DragInitiationPoint,
                        this.movedDesignerImagePoint,
                        DraggedActivities
                    );
                    ...
                    for (int i = 0; i < this.draggedDesignerImages.Count; i++)
                    {
                        ...
                        ActivityDesignerPaint.DrawImage(
                            eventArgs.Graphics,
                            this.draggedDesignerImages[i],
                            new Rectangle(
                                new Point(
                                    locations[i].X - 2 * ambientTheme.Margin.Width,
                                    locations[i].Y - 2 * ambientTheme.Margin.Height
                                ),
                                imageSize
                            ),
                ...
                if (locations.Length == this.draggedDesignerImages.Count)
                {
                    ...
                    for (int i = 0; i < this.draggedDesignerImages.Count; i++)
                    {
                        Rectangle rectangle = new Rectangle(
                            locations[i],
                            this.draggedDesignerImages[i].Size
                        );
                        rectangle.Inflate(
        ...
        internal static Point[] GetDesignerLocations(
            Point startPoint,
            Point endPoint,
            ICollection<Activity> activitiesToMove
        )
        {
            ...
            foreach (Activity activityToMove in activitiesToMove)
            {
                ...
                if (designerToMove != null && !startPoint.IsEmpty)
                {
                    ...
                    location = new Point(
                        designerToMove.Location.X + delta.Width,
