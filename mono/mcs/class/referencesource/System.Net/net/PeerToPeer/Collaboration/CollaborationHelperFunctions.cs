    ...
    internal static class CollaborationHelperFunctions
    {
                    ...
                    if (!s_Initialized)
                    {
                        if (!PeerToPeerOSHelper.SupportsP2P)
                            throw new PlatformNotSupportedException(
                                SR.GetString(SR.P2P_NotAvailable)
                            );
                        int errorCode = UnsafeCollabNativeMethods.PeerCollabStartup(
                            c_CollabVersion
                        );
                        if (errorCode != 0)
                        {
        ...
        internal static GUID ConvertGuidToGUID(Guid guid)
        {
            ...
            if (guid != null)
            {
                ...
                newGuid.data1 = (uint)(
                    Convert.ToUInt32(guidString.Substring(startVal, endVal - startVal), 16)
                );
                startVal = endVal + 1;
                endVal = guidString.IndexOf('-', endVal + 1);
                newGuid.data2 = (ushort)(
                    Convert.ToUInt16(guidString.Substring(startVal, endVal - startVal), 16)
                );
                startVal = endVal + 1;
                endVal = guidString.IndexOf('-', endVal + 1);
                newGuid.data3 = (ushort)(
                    Convert.ToUInt16(guidString.Substring(startVal, endVal - startVal), 16)
                );
        ...
        // </SecurityKernel>
        [System.Security.SecurityCritical]
        internal static PeerContact ConvertPEER_CONTACTToPeerContact(
            PEER_CONTACT pc,
            bool isMyContact
        )
        {
            ...
            peerContact.EmailAddress =
                (pc.pwzEmailAddress != null) ? new MailAddress(pc.pwzEmailAddress) : null;
            if (!isMyContact)
            ...
            if (data != null)
            {
                SafeCertStore certHandle = UnsafeCollabNativeMethods.CertOpenStore(
                    new IntPtr( /*CERT_STORE_PROV_PKCS7*/
                        5
                    ),
                    0x00000001 /*X509_ASN_ENCODING*/
                        | 0x00010000 /*PKCS_7_ASN_ENCODING*/
                    ,
                    IntPtr.Zero,
                ...
                if (certHandle == null || certHandle.IsInvalid)
                {
                    ...
                    throw PeerToPeerException.CreateFromHr(
                        SR.GetString(SR.Collab_CredentialsError),
                        win32ErrorCode
                    );
                }
                try
        ...
        // </SecurityKernel>
        [System.Security.SecurityCritical]
        internal static PEER_CONTACT ConvertPeerContactToPEER_CONTACT(
            PeerContact peerContact,
            ref SafeCollabMemory safeCredentials
        )
        {
            ...
            if (peerContact.Credentials != null)
            {
                SafeCertStore certHandle = UnsafeCollabNativeMethods.CertOpenStore(
                    ...
                    0,
                    IntPtr.Zero,
                    0x00002000 /*CERT_STORE_CREATE_NEW_FLAG*/
                        | 0x00000001 /*CERT_STORE_NO_CRYPT_RELEASE_FLAG*/
                    ,
                    IntPtr.Zero
                );
                ...
                try
                {
                    ...
                    bool returnCode = UnsafeCollabNativeMethods.CertSaveStore(
                        certHandle,
                        0x00000001 /*X509_ASN_ENCODING*/
                            | 0x00010000 /*PKCS_7_ASN_ENCODING*/
                        ,
                    ...
                    if ((pd.cbData != 0) && (returnCode))
                    {
                        ...
                        returnCode = UnsafeCollabNativeMethods.CertSaveStore(
                            certHandle,
                            0x00000001 /*X509_ASN_ENCODING*/
                                | 0x00010000 /*PKCS_7_ASN_ENCODING*/
                            ,
        ...
        internal static PeerObject ConvertPEER_OBJECTToPeerObject(PEER_OBJECT po)
        {
            ...
            return new PeerObject(
                ConvertGUIDToGuid(po.guid),
                data,
                (PeerScope)po.dwPublicationScope
            );
        }
        ...
        // </SecurityKernel>
        [System.Security.SecurityCritical]
        internal static PeerApplication ConvertPEER_APPLICATIONToPeerApplication(
            PEER_APPLICATION pa
        )
        {
            byte[] data = null;
        ...
        internal static PeerEndPoint ConvertPEER_ENDPOINTToPeerEndPoint(PEER_ENDPOINT pe)
        {
            ...
            {
                pe.peerAddress.sin6.sin6_addr0,
                pe.peerAddress.sin6.sin6_addr1,
                pe.peerAddress.sin6.sin6_addr2,
                pe.peerAddress.sin6.sin6_addr3,
                pe.peerAddress.sin6.sin6_addr4,
                pe.peerAddress.sin6.sin6_addr5,
                pe.peerAddress.sin6.sin6_addr6,
                pe.peerAddress.sin6.sin6_addr7,
                pe.peerAddress.sin6.sin6_addr8,
                pe.peerAddress.sin6.sin6_addr9,
                pe.peerAddress.sin6.sin6_addr10,
                pe.peerAddress.sin6.sin6_addr11,
        ...
        internal static PEER_ADDRESS ConvertIPEndpointToPEER_ADDRESS(IPEndPoint endPoint)
        {
            ...
            CollaborationHelperFunctions.ByteArrayToSin6Addr(
        ...
        internal static void CleanEventVars(
            ref RegisteredWaitHandle waitHandle,
        ...
        internal static void AddMyPresenceChanged(
            EventHandler<PresenceChangedEventArgs> callback,
        ...
        )
        {
            ...
            lock (lockPresenceChangedEvent)
            {
                if (presenceChanged == null)
                {
                    ...
                    regPresenceChangedWaitHandle = ThreadPool.RegisterWaitForSingleObject(
                        presenceChangedEvent, //Event that triggers the callback
        ...
        )
        {
            ...
            lock (lockAppChangedEvent)
            {
                if (applicationChanged == null)
                {
                    ...
                    regAppChangedWaitHandle = ThreadPool.RegisterWaitForSingleObject(
                        appChangedEvent, //Event that triggers the callback
        ...
        )
        {
            ...
            lock (lockObjChangedEvent)
            {
                if (objectChanged == null)
                {
                    ...
                    regObjChangedWaitHandle = ThreadPool.RegisterWaitForSingleObject(
                        objChangedEvent, //Event that triggers the callback
        ...
        internal static void ThrowIfInvitationResponseInvalid(PeerInvitationResponse response)
        {
            ...
            if (
                response.PeerInvitationResponseType < PeerInvitationResponseType.Declined
