...
namespace System.Net.PeerToPeer
{
    ...
    public class ResolveCompletedEventArgs : AsyncCompletedEventArgs
    {
        ...
        {
            get { return m_PeerNameRecordCollection; }
        }
    }
    ...
    internal class PeerNameResolverHelper : IDisposable
    {
        ...
        private const int PEER_E_NO_MORE = (int)(
            ((int)1 << 31) | ((int)FACILITY_P2P << 16) | NO_MORE_RECORDS
        );
        ...
        internal PeerNameResolverHelper(
            PeerName peerName,
            Cloud cloud,
            int MaxRecords,
            object userState,
            PeerNameResolver parent,
            int NewTraceEventId
        )
        {
        ...
        internal void StartAsyncResolve()
        {
            ...
            m_RegisteredWaitHandle = ThreadPool.RegisterWaitForSingleObject(
                m_EndPointInfoAvailableEvent, //Event that triggers the callback
            ...
            //------------------------------------------
            Int32 result = UnsafeP2PNativeMethods.PeerPnrpStartResolve(
                m_PeerName.ToString(),
            ...
            if (result != 0)
            {
                ...
                PeerToPeerException ex = PeerToPeerException.CreateFromHr(
            ...
            lock (m_Lock)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        unsafe
                        {
                            PEER_PNRP_ENDPOINT_INFO* pEndPointInfo = (PEER_PNRP_ENDPOINT_INFO*)
                                shEndPointInfo.DangerousGetHandle();
                            record = new PeerNameRecord();
                            record.PeerName = new PeerName(
                            ...
                            if (pEndPointInfo->payLoad.cbPayload != 0)
                            {
                                ...
                                Marshal.Copy(
                                    pEndPointInfo->payLoad.pbPayload,
                                    record.Data,
                                    0,
                                    (int)pEndPointInfo->payLoad.cbPayload
                                );
                            }
...
#if BIGENDIAN
                                addressFamily =
                                    AddressFamilyBuffer[1] + ((int)AddressFamilyBuffer[0] << 8);
#else
                                addressFamily =
                                    AddressFamilyBuffer[0] + ((int)AddressFamilyBuffer[1] << 8);
#endif
                                byte[] buffer = new byte[
                                    ((AddressFamily)addressFamily == AddressFamily.InterNetwork)
                                        ? SystemNetHelpers.IPv4AddressSize
                                        : SystemNetHelpers.IPv6AddressSize
                                ];
                                Marshal.Copy(pSOCKADDR, buffer, 0, buffer.Length);
                                IPEndPoint ipe = SystemNetHelpers.IPEndPointFromSOCKADDRBuffer(
                                    buffer
                                );
                                record.EndPointCollection.Add(ipe);
                                ppSOCKADDRs = (IntPtr)(
                                    (long)ppSOCKADDRs + Marshal.SizeOf(typeof(IntPtr))
                                );
                            }
                        }
                    }
                    ...
                    if (parent != null)
                    {
                        parent.PrepareToRaiseProgressChangedEvent(
                            m_AsyncOp,
        ...
        public void ContineCancelCallback(object state)
        {
            ...
            try
            {
                ...
                PeerNameResolver parent =
                    m_PeerNameResolverWeakReference.Target as PeerNameResolver;
                if (parent != null)
                {
    ...
    public class PeerNameResolver
    {
        ...
        private Dictionary<object, PeerNameResolverHelper> m_PeerNameResolverHelperList =
            new Dictionary<object, PeerNameResolverHelper>();

        public PeerNameResolver()
        {
            OnResolveProgressChangedDelegate = new SendOrPostCallback(
                ResolveProgressChangedWaitCallback
            );
        ...
        public PeerNameRecordCollection Resolve(PeerName peerName, Cloud cloud, int maxRecords)
        {
            ...
            if (maxRecords <= 0)
            {
                throw new ArgumentOutOfRangeException(
                    "maxRecords",
                    SR.GetString(SR.Pnrp_MaxRecordsParameterMustBeGreaterThanZero)
                );
            }
            ...
            int result = UnsafeP2PNativeMethods.PeerPnrpResolve(
                peerName.ToString(),
                NativeCloudName,
            ...
            if (ActualCountOfEndPoints != 0)
            {
                try
                {
                    unsafe
                    {
                        ...
                        PEER_PNRP_ENDPOINT_INFO* pEndPoints =
                            (PEER_PNRP_ENDPOINT_INFO*)pEndPointInfoArray;
        ...
        internal void PrepareToRaiseProgressChangedEvent(
            AsyncOperation asyncOP,
            ResolveProgressChangedEventArgs args
        )
        {
        ...
        internal void PrepareToRaiseCompletedEvent(
            AsyncOperation asyncOP,
            ResolveCompletedEventArgs args
        )
        {
