...
namespace System.Web.DynamicData
{
    ...
    public class DynamicControlParameter : Parameter, IWhereParametersProvider
    {
        ...
        public virtual IEnumerable<Parameter> GetWhereParameters(IDynamicDataSource dataSource)
        {
            ...
            if (control == null)
            {
                throw new InvalidOperationException(
                    String.Format(
                        ...
                        DynamicDataResources.DynamicControlParameter_DynamicDataSourceControlNotFound,
                        ControlId
                    )
                );
            }
            ...
            IControlParameterTarget paramTarget = DynamicDataManager.GetControlParameterTarget(
                control
            );
            ...
            if (paramTarget == null)
            {
                throw new InvalidOperationException(
                    String.Format(
                        CultureInfo.CurrentCulture,
                        DynamicDataResources.DynamicControlParameter_DynamicDataSourceControlCannotBeUsedAsParent,
                        ControlId
                    )
                );
            }
            ...
            MetaTable table = MetaTableHelper.GetTableWithFullFallback(
                dataSource,
                HttpContext.Current.ToWrapper()
            );
            ...
            {
                return GetForeignKeyControlWhereParameters(
                    control,
                    paramTarget,
                    (MetaForeignKeyColumn)column
                );
            }
            return GetPropertyControlWhereParameters(control, paramTarget, column);
        }

        private IEnumerable<Parameter> GetPropertyControlWhereParameters(
            Control control,
        ...
        private IEnumerable<Parameter> GetPrimaryKeyControlWhereParameters(
            Control control,
            IControlParameterTarget paramTarget
        )
        {
        ...
        private IEnumerable<Parameter> GetForeignKeyControlWhereParameters(
            Control control,
            IControlParameterTarget paramTarget,
            MetaForeignKeyColumn column
        )
            ...
            if (parentTable != null)
            {
                ...
                foreach (var fkName in column.ForeignKeyNames)
                {
                    ...
                    var controlParameter = new ControlParameter()
                    {
                        Name = fkName,
                        ControlID = control.UniqueID,
                        PropertyName = paramTarget.GetPropertyNameExpression(
                            parentTablePKColumn.Name
                        ),
                    };

                    DataSourceUtil.SetParameterTypeCodeAndDbType(
                        controlParameter,
                        parentTablePKColumn
                    );
        ...
        protected override object Evaluate(HttpContext context, Control control)
        {
            ...
            throw new InvalidOperationException(
                String.Format(
                    CultureInfo.CurrentCulture,
                    DynamicDataResources.DynamicParameter_NeedExpansion,
                    typeof(DynamicControlParameter).Name
                )
            );
        }
    }
