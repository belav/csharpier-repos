...
namespace System.Web.DynamicData.ModelProviders
{
    internal sealed class EFTableProvider : TableProvider
    {
        ...
        public EFTableProvider(
            EFDataModelProvider dataModel,
            EntitySet entitySet,
            EntityType entityType,
            Type entityClrType,
            Type parentEntityClrType,
            Type rootEntityClrType,
            string name
        )
            : base(dataModel)
        {
            ...
            {
                if (
                    EFColumnProvider.IsSupportedEdmMemberType(m)
                    && IsPublicProperty(entityClrType, m.Name)
                )
                {
                    EFColumnProvider entityMember = new EFColumnProvider(
                        entityType,
                        this,
                        m,
                        keyMembers.Contains(m)
                    );
        ...
        private static string CreateEntitySqlQueryString(EntitySet entitySet)
        {
            ...
            return QuoteEntitySqlIdentifier(entitySet.EntityContainer.Name)
                + "."
                + QuoteEntitySqlIdentifier(entitySet.Name);
        }
