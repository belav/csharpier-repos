...
namespace System.Web.DynamicData.ModelProviders
{
    internal sealed class EFColumnProvider : ColumnProvider
    {
            ...
            : base(table)
        {
            ...
            {
                _isAssociation = true;
                long key = EFAssociationProvider.BuildRelationshipKey(
                    entityType,
        ...
        private bool DetermineIsForeignKeyComponent(EdmProperty property)
        {
            ...
            return navigationProperties.Any(n =>
                EFAssociationProvider
                    .GetDependentPropertyNames(
                        n,
                        !IsPrimaryKey /* checkRelationshipType */
                    )
        ...
        private static bool IsServerGenerated(EdmProperty property)
        {
            ...
            )
            {
                return "Identity" == (string)generated.Value
                    || "Computed" == (string)generated.Value;
            }
            return false;
        }
            ...
            get
            {
                if (base.ColumnType == null)
                {
                    ...
                    else if (edmType is CollectionType)
                    {
                        ...
                        base.ColumnType = ((EFDataModelProvider)this.Table.DataModel).GetClrType(
                            ((CollectionType)edmType).TypeUsage.EdmType
                        );
                    }
        ...
        internal static bool IsSupportedEdmMemberType(EdmMember member)
        {
            ...
            return edmType is EntityType
                || edmType is CollectionType
                || edmType is PrimitiveType
                || edmType is EnumType;
        }
    }
}
