...
namespace System.Web.DynamicData.ModelProviders
{
    internal sealed class EFAssociationProvider : AssociationProvider
    {
        public EFAssociationProvider(EFColumnProvider column, NavigationProperty navigationProperty)
        {
            ...
            else
            {
                EntityType value = (EntityType)
                    navigationProperty.ToEndMember.TypeUsage.EdmType.MetadataProperties
            ...
            if (IsForeignKeyReference)
            {
                ...
                if (!foreignKeyNames.Any())
                {
                    ...
                    foreach (
                        ColumnProvider toEntityColumn in ToTable.Columns.Where(c => c.IsPrimaryKey)
                    )
                    {
        ...
        private bool IsForeignKeyReference
        {
            get
            {
                return Direction == AssociationDirection.OneToOne
                    || Direction == AssociationDirection.ManyToOne;
            }
        }

        internal static long BuildRelationshipKey(
        ...
        internal static IEnumerable<string> GetDependentPropertyNames(
            NavigationProperty navigationProperty
        )
        {
            return GetDependentPropertyNames(
                navigationProperty,
                true /*checkRelationshipType*/
            );
        }

        internal static IEnumerable<string> GetDependentPropertyNames(
            NavigationProperty navigationProperty,
            bool checkRelationshipType
        )
        {
