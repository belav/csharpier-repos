    ...
    internal sealed class DLinqAssociationProvider : AssociationProvider
    {
        public DLinqAssociationProvider(DLinqColumnProvider column)
        {
            ...
            Debug.Assert(
                ...
                "Many to Many is not supported by Linq to SQL"
            );
        ...
        private void SetAssociationKeyInfo(MetaAssociation association)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                DLinqColumnProvider thisEntityMemberComponent = FindColumn(
                    column.Table,
                    thisKeyMetaDataMember.Name
                );
        ...
        private void SetOtherEndOfAssociation(MetaAssociation association)
        {
            ...
            DLinqDataModelProvider parentEntityDataContext = (DLinqDataModelProvider)
                entityMemberParentEntity.DataModel;
            if (association.OtherMember != null)
            {
                ToColumn = parentEntityDataContext.ColumnLookup[
                    (PropertyInfo)association.OtherMember.Member
                ];
            }
            else
            {
                ToTable = ((DLinqDataModelProvider)FromColumn.Table.DataModel).DLinqTables
        ...
        {
            //
            return (DLinqColumnProvider)
                table.Columns.First(member => member.Name.Equals(columnName));
        }
