...
namespace System.Web.DynamicData
{
    public abstract class QueryableFilterUserControl : UserControl
    {
        ...
        internal void Initialize(
            MetaColumn column,
            IQueryableDataSource iQueryableDataSource,
        ...
        private DefaultValueMapping DefaultValueMapping
        {
            get
            {
                if (_defaultValueMapping == null)
                {
                    ...
                    _defaultValueMapping = DynamicDataExtensions.GetDefaultValueMapping(
                        QueryableDataSource,
                        _context
                    );
                }
        ...
        public static IQueryable ApplyEqualityFilter(
            IQueryable source,
            string propertyName,
            object value
        )
        {
            ...
            Expression propertyExpression = CreatePropertyExpression(
                parameterExpression,
                propertyName
            );
            if (Nullable.GetUnderlyingType(propertyExpression.Type) != null && value != null)
            {
                propertyExpression = Expression.Convert(
                    propertyExpression,
                    Misc.RemoveNullableFromType(propertyExpression.Type)
                );
            }
