...
namespace System.Web.Services.Protocols
{
    ...
    internal class XmlReturn
    {
        ...
        internal static object[] GetInitializers(LogicalMethodInfo[] methodInfos)
        {
            ...
            bool serviceDefaultIsEncoded = SoapReflector.ServiceDefaultIsEncoded(
                WebServiceReflector.GetMostDerivedType(methodInfos)
            );
            XmlReflectionImporter importer = SoapReflector.CreateXmlImporter(
            ...
            TraceMethod caller = Tracing.On
                ? new TraceMethod(typeof(XmlReturn), "GetInitializers", methodInfos)
                : null;
            if (Tracing.On)
                Tracing.Enter(
                    Tracing.TraceId(Res.TraceCreateSerializer),
                    caller,
                    new TraceMethod(typeof(XmlSerializer), "FromMappings", xmlMappings, evidence)
                );
    ...
    public class XmlReturnReader : MimeReturnReader
    {
        ...
        public override object Read(WebResponse response, Stream responseStream)
        {
            try
            {
                ...
                if (Tracing.On)
                    Tracing.Enter(
                        Tracing.TraceId(Res.TraceReadResponse),
                        caller,
                        new TraceMethod(xmlSerializer, "Deserialize", reader)
                    );
