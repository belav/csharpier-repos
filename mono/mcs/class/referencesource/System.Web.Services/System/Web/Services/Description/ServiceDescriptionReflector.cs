...
namespace System.Web.Services.Description
{
    ...
    public class ServiceDescriptionReflector
    {
        ...
        public ServiceDescriptionReflector()
        {
            ...
            for (int i = 0; i < reflectors.Length; i++)
            {
                ProtocolReflector reflector = (ProtocolReflector)
                    Activator.CreateInstance(reflectorTypes[i]);
            ...
            if (
                (enabledProtocols & WebServiceProtocols.HttpPost) == 0
                && (enabledProtocols & WebServiceProtocols.HttpPostLocalhost) != 0
            )
            {
        ...
        public void Reflect(Type type, string url)
        {
            ...
            {
                XmlSchema[] descriptionSchemas = new XmlSchema[
        ...
        void CheckForDuplicateMethods(LogicalMethodInfo[] methods)
        {
            ...
            foreach (LogicalMethodInfo method in methods)
            {
                ...
                string key =
                    method.Binding == null ? messageName : method.Binding.Name + "." + messageName;
                LogicalMethodInfo existingMethod = (LogicalMethodInfo)messageNames[key];
                if (existingMethod != null)
                {
                    throw new InvalidOperationException(
                        Res.GetString(
                            Res.BothAndUseTheMessageNameUseTheMessageName3,
                            method,
                            existingMethod,
                            XmlConvert.EncodeLocalName(messageName)
                        )
                    );
                }
