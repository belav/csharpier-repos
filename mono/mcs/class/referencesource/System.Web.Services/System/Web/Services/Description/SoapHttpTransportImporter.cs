        ...
        public override void ImportClass()
        {
            ...
            if (ImportContext.Style == ServiceDescriptionImportStyle.Client)
            {
                ...
                if (ImportContext is Soap12ProtocolImporter)
                {
                    soap11 = false;
                    // add version code
                    CodeTypeReferenceExpression versionEnumTypeReference =
                        new CodeTypeReferenceExpression(typeof(SoapProtocolVersion));
                    CodeFieldReferenceExpression versionEnumFieldReference =
                        new CodeFieldReferenceExpression(
                            versionEnumTypeReference,
                            Enum.Format(
                                typeof(SoapProtocolVersion),
                                SoapProtocolVersion.Soap12,
                                "G"
                            )
                        );
                    CodePropertyReferenceExpression versionPropertyReference =
                        new CodePropertyReferenceExpression(
                            new CodeThisReferenceExpression(),
                            "SoapVersion"
                        );
                    CodeAssignStatement assignVersionStatement = new CodeAssignStatement(
                        versionPropertyReference,
                        versionEnumFieldReference
                    );
                ...
                ProtocolImporterUtil.GenerateConstructorStatements(
                    ...
                    urlKey,
                    baseUrl,
                    soap11 && !ImportContext.IsEncodedBinding
                );
            }
