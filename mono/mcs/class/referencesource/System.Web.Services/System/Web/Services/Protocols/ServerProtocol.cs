...
namespace System.Web.Services.Protocols
{
    ...
    public abstract class ServerProtocol
    {
        ...
        internal string GenerateFaultString(Exception e, bool htmlEscapeMessage)
        {
            ...
            else
            {
                ...
                {
                    string text = htmlEscapeMessage
                        ? HttpUtility.HtmlEncode(inner.Message)
                        : inner.Message;
                    if (text.Length == 0)
                        text = e.GetType().Name;
                    builder.Append(text);
        ...
        delegate string CreateCustomKeyForAspNetWebServiceMetadataCache(
            Type protocolType,
            Type serverType,
            string originalKey
        );

        static string DefaultCreateCustomKeyForAspNetWebServiceMetadataCache(
            Type protocolType,
            Type serverType,
            string originalKey
        )
        ...
        static CreateCustomKeyForAspNetWebServiceMetadataCache GetCreateCustomKeyForAspNetWebServiceMetadataCacheDelegate(
            Type serverType
        )
        {
            PartialTrustHelpers.FailIfInPartialTrustOutsideAspNet();
            string key = "CreateCustomKeyForAspNetWebServiceMetadataCache-" + serverType.FullName;
            CreateCustomKeyForAspNetWebServiceMetadataCache result =
                (CreateCustomKeyForAspNetWebServiceMetadataCache)HttpRuntime.Cache.Get(key);
            if (result == null)
            {
                ...
                else
                {
                    ...
                    {
                        return (string)
                            createKeyMethod.Invoke(null, new object[] { pt, st, originalString });
                    };
                }
        ...
        string CreateKey(
            Type protocolType,
            Type serverType,
            bool excludeSchemeHostPort = false,
        ...
        )
        {
            ...
            int length =
                protocolTypeName.Length
                + url.Length
                + serverTypeName.Length
                + typeHandleString.Length;
            ...
            CreateCustomKeyForAspNetWebServiceMetadataCache createKey =
                ServerProtocol.GetCreateCustomKeyForAspNetWebServiceMetadataCacheDelegate(
                    serverType
                );
        ...
        internal bool IsCacheUnderPressure(Type protocolType, Type serverType)
        {
            ...
            if (item != null)
            {
                return item.Pressure < threshold
                    ? Interlocked.Increment(ref item.Pressure) >= threshold
                    : false;
            }
            else
            {
    ...
    {
        internal ServerProtocol Create(
            Type type,
            HttpContext context,
            HttpRequest request,
            HttpResponse response,
                ...
                if (serverProtocol != null)
                {
                    ...
                    if (!serverProtocol.WriteException(e, serverProtocol.Response.OutputStream))
                        throw new InvalidOperationException(
                            Res.GetString(Res.UnableToHandleRequest0),
                            e
                        );
                }
                return null;
