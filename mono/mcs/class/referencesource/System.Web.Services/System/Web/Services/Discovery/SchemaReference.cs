...
namespace System.Web.Services.Discovery
{
    ...
    public sealed class SchemaReference : DiscoveryReference
    {
        ...
        public string Ref
        {
            get { return reference == null ? "" : reference; }
            set { reference = value; }
        }
        ...
        )
        {
            ...
            foreach (XmlSchemaExternal external in schema.Includes)
            {
                if (
                    ...
                    || external.SchemaLocation.Length == 0
            ...
            get
            {
                if (ClientProtocol == null)
                    throw new InvalidOperationException(
                        Res.GetString(Res.WebMissingClientProtocol)
                    );
        ...
        protected internal override void Resolve(string contentType, Stream stream)
        {
            if (ContentType.IsHtml(contentType))
                ClientProtocol.Errors[Url] = new InvalidContentTypeException(
                    Res.GetString(Res.WebInvalidContentType, contentType),
                    contentType
                );
            ...
            foreach (XmlSchemaExternal external in schema.Includes)
            {
                ...
                catch (Exception e)
                {
                    ...
                    throw new InvalidDocumentContentsException(
                        Res.GetString(
                            Res.TheSchemaDocumentContainsLinksThatCouldNotBeResolved,
                            location
                        ),
                        e
                    );
