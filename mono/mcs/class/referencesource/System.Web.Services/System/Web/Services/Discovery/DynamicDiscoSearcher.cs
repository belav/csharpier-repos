...
namespace System.Web.Services.Discovery
{
    ...
    internal abstract class DynamicDiscoSearcher
    {
        ...
        protected bool IsExcluded(string url)
        {
            if (excludedUrlsTable == null)
            {
                ...
                foreach (string s in excludedUrls)
                {
                    ...
                    excludedUrlsTable.Add(
                        MakeAbsExcludedPath(s).ToLower(CultureInfo.InvariantCulture),
                        null
                    );
                }
            }
        ...
        protected void ScanDirectory(string directory)
        {
            ...
            bool primaryFound = ScanDirByPattern(
                directory,
                true /*primary*/
                ,
        ...
        protected bool ScanDirByPattern(
            string dir,
            bool IsPrimary,
            DiscoverySearchPattern[] patterns
        )
        {
            ...
            if (CompModSwitches.DynamicDiscoverySearcher.TraceVerbose)
                Debug.WriteLine(
                    "= DynamicDiscoSearcher.ScanDirByPattern(): dir="
                        + dir
                        + "  Phys.dir="
                        + directory.Name
                );
            ...
            for (int i = 0; i < patterns.Length; i++)
            {
                ...
                foreach (FileInfo file in files)
                {
                    if ((file.Attributes & FileAttributes.Directory) == 0)
                    {
                        if (CompModSwitches.DynamicDiscoverySearcher.TraceVerbose)
                            Debug.WriteLine(
                                "=== DynamicDiscoSearcher.ScanDirByPattern(): file.Name="
                                    + file.Name
                                    + "  fileToSkipFirst="
                                    + fileToSkipFirst
                            );
