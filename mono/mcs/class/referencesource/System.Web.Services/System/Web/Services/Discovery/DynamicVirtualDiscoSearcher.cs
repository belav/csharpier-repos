...
namespace System.Web.Services.Discovery
{
    ...
    internal class DynamicVirtualDiscoSearcher : DynamicDiscoSearcher
    {
        ...
        private /*static*/
        Hashtable webApps = new Hashtable();
        ...
        protected override void SearchSubDirectories(string nameAdsiDir)
        {
            if (CompModSwitches.DynamicDiscoverySearcher.TraceVerbose)
                Debug.WriteLine(
                    "DynamicVirtualDiscoSearcher.SearchSubDirectories(): nameAdsiDir=" + nameAdsiDir
                );
        ...
        protected override DirectoryInfo GetPhysicalDir(string dir)
        {
                ...
                { //SchemaClassName == "IIsWebDirectory"
                    ...
                    if (!dir.StartsWith(rootPathAsdi, StringComparison.Ordinal))
                    {
                        throw new ArgumentException(
                            Res.GetString(Res.WebVirtualDisoRoot, dir, rootPathAsdi),
                            "dir"
                        );
                    }
            ...
            catch (Exception e)
            {
                ...
                if (CompModSwitches.DynamicDiscoverySearcher.TraceVerbose)
                    Debug.WriteLine(
                        "+++ DynamicVirtualDiscoSearcher.GetPhysicalDir(): dir="
                            + dir
                            + " Exception="
                            + e.ToString()
                    );
        ...
        private string GetWebServerForUrl(string url)
        {
            ...
            foreach (DirectoryEntry obj in w3Service.Children)
            {
                ...
                {
                    if (CompModSwitches.DynamicDiscoverySearcher.TraceVerbose)
                        Debug.WriteLine("GetWebServerForUrl() bindingsEntry=" + bindingsEntry);
                    ...
                    else
                    {
                        if (
                            ip.Length == 0
                            || string.Compare(ip, uri.Host, StringComparison.OrdinalIgnoreCase) == 0
                        )
                            return site.Path;
                    }
                }
        ...
        private AppSettings GetAppSettings(DirectoryEntry entry)
        {
            ...
            if (obj == null)
            {
                ...
                lock (webApps)
                {
                    obj = webApps[key];
                    if (obj == null)
                    { //make sure other thread not taken care of
