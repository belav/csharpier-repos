...
namespace System.Web.Services.Protocols
{
    ...
    public sealed class SoapServerType : ServerType
    {
        ...
        {
            get { return serviceDefaultIsEncoded; }
        }

        public bool ServiceRoutingOnSoapAction
        {
            get { return routingOnSoapAction; }
        }
            ...
            : base(type)
        {
            ...
            routingOnSoapAction =
                SoapReflector.GetSoapServiceRoutingStyle(soapServiceAttribute)
            ...
            for (int i = 0; i < extensionTypes.Count; i++)
            {
                ...
                if (element == null)
                    continue;
                SoapReflectedExtension extension = new SoapReflectedExtension(
                    element.Type,
                    null,
                    element.Priority
                );
            ...
            HighPriExtensions = (SoapReflectedExtension[])
                highPri.ToArray(typeof(SoapReflectedExtension));
            LowPriExtensions = (SoapReflectedExtension[])
                lowPri.ToArray(typeof(SoapReflectedExtension));
            Array.Sort(HighPriExtensions);
            Array.Sort(LowPriExtensions);
            HighPriExtensionInitializers = SoapReflectedExtension.GetInitializers(
                type,
                HighPriExtensions
            );
            LowPriExtensionInitializers = SoapReflectedExtension.GetInitializers(
                type,
                LowPriExtensions
            );
            ...
            TraceMethod caller = Tracing.On
                ? new TraceMethod(this, ".ctor", type, protocolsSupported)
                : null;
            if (Tracing.On)
                Tracing.Enter(
...
#pragma warning restore 618
            ...
            for (int i = 0; i < soapMethods.Length; i++)
            {
                ...
                serverMethod.extensionInitializers = SoapReflectedExtension.GetInitializers(
                ...
                serverMethod.inHeaderMappings = (SoapHeaderMapping[])
                    inHeaders.ToArray(typeof(SoapHeaderMapping));
                if (serverMethod.outHeaderSerializer != null)
                    serverMethod.outHeaderMappings = (SoapHeaderMapping[])
                        outHeaders.ToArray(typeof(SoapHeaderMapping));
                ...
                if (soap11 && !routingOnSoapAction && soapMethod.requestElementName.IsEmpty)
                    throw new SoapException(
                        Res.GetString(
                            Res.TheMethodDoesNotHaveARequestElementEither1,
                            serverMethod.methodInfo.Name
                        ),
                ...
                else
                {
                    ...
                    {
                        SoapServerMethod duplicateMethod = (SoapServerMethod)
                            methods[soapMethod.action];
                        throw new SoapException(
                            Res.GetString(
                                Res.TheMethodsAndUseTheSameSoapActionWhenTheService3,
                                serverMethod.methodInfo.Name,
                                duplicateMethod.methodInfo.Name,
                                soapMethod.action
                            ),
                            new XmlQualifiedName(Soap.Code.Client, Soap.Namespace)
                        );
                ...
                else
                {
                    ...
                    {
                        SoapServerMethod duplicateMethod = (SoapServerMethod)
                            methods[soapMethod.requestElementName];
                        throw new SoapException(
                            Res.GetString(
                                Res.TheMethodsAndUseTheSameRequestElementXmlns4,
                                serverMethod.methodInfo.Name,
                                duplicateMethod.methodInfo.Name,
                                soapMethod.requestElementName.Name,
                                soapMethod.requestElementName.Namespace
                            ),
                            new XmlQualifiedName(Soap.Code.Client, Soap.Namespace)
                        );
                    }
    ...
    public class SoapServerProtocol : ServerProtocol
    {
        ...
        protected virtual XmlReader GetReaderForMessage(SoapServerMessage message, int bufferSize)
        {
            ...
            Int64 timeoutTicks =
                Int64.MaxValue - timeout <= nowTicks ? Int64.MaxValue : nowTicks + timeout;
            ...
            if (enc != null)
            {
                if (timeoutTicks == Int64.MaxValue)
                {
                    reader = new XmlTextReader(
                        new StreamReader(message.Stream, enc, true, bufferSize)
                    );
                }
                else
                {
        ...
        internal override bool Initialize()
        {
            ...
            if (
                ...
                && null
                    == (
                        serverType = (SoapServerType)GetFromCache(
            ...
            try
            {
                message.highPriConfigExtensions = SoapMessage.InitializeExtensions(
                    serverType.HighPriExtensions,
                    serverType.HighPriExtensionInitializers
                );
                //
                // Allow derived classes to modify the high priority extensions list.
                //
                message.highPriConfigExtensions = ModifyInitializedExtensions(
                    PriorityGroup.High,
                    message.highPriConfigExtensions
                );
                ...
                if (!message.Stream.CanSeek)
                    throw new InvalidOperationException(
                        "Non-Seekable stream "
                            + message.Stream.GetType().FullName
                            + " Web services SOAP handler assumes a seekable stream."
                    );
            ...
            if (extensionException == null)
            {
                try
                {
                    SoapReflectedExtension[] otherReflectedExtensions =
                        (SoapReflectedExtension[])CombineExtensionsHelper(
                            serverMethod.extensions,
                            serverType.LowPriExtensions,
                            typeof(SoapReflectedExtension)
                        );
                    object[] otherInitializers = (object[])CombineExtensionsHelper(
                        serverMethod.extensionInitializers,
                        serverType.LowPriExtensionInitializers,
                        typeof(object)
                    );
                    message.otherExtensions = SoapMessage.InitializeExtensions(
                    ...
                    //
                    message.otherExtensions = ModifyInitializedExtensions(
                        PriorityGroup.Low,
                        message.otherExtensions
                    );
                    message.allExtensions = (SoapExtension[])CombineExtensionsHelper(
                        message.highPriConfigExtensions,
                        message.otherExtensions,
                        typeof(SoapExtension)
                    );
                }
                catch (Exception e)
                {
        ...
        private void GuessVersion()
        {
            ...
            if (IsSupported(WebServiceProtocols.AnyHttpSoap))
            {
                ...
                if (
                    ...
                    || ContentType.MatchesBase(Request.ContentType, ContentType.ApplicationSoap)
                )
        ...
        internal override Exception OnewayInitException
        {
            ...
            {
                Debug.Assert(
                    isOneWay || (onewayInitException == null),
                    "initException is meant to be used for oneWay methods only."
                );
        ...
        */
        ...
        private static Array CombineExtensionsHelper(Array array1, Array array2, Type elementType)
        {
            ...
            if (elementType == typeof(SoapReflectedExtension))
                ...
                throw new ArgumentException(
                    Res.GetString(
                        Res.ElementTypeMustBeObjectOrSoapExtensionOrSoapReflectedException
                    ),
                    "elementType"
                );
        ...
        private void CheckHelperVersion()
        {
            ...
            { // unknown envelope ns -- version mismatch
                ...
                throw new SoapException(
                    Res.GetString(
                        Res.WebInvalidEnvelopeNamespace,
                        requestNamespace,
                        helper.EnvelopeNs
                    ),
            ...
            else if (!IsSupported(helper.Protocol))
            { // known envelope ns but we don't support this version -- version mismatch
        ...
        internal override object[] ReadParameters()
        {
            ...
            if (!ContentType.IsSoap(message.ContentType))
                throw new SoapException(
                    Res.GetString(
                        Res.WebRequestContent,
                        message.ContentType,
                        helper.HttpContentType
                    ),
                    new XmlQualifiedName(Soap.Code.Client, Soap.Namespace),
                    new SoapFaultSubCode(Soap12FaultCodes.UnsupportedMediaTypeFaultCode)
                );
            ...
            {
                throw new SoapException(
                    Res.GetString(Res.WebRequestUnableToRead),
                    new XmlQualifiedName(Soap.Code.Client, Soap.Namespace),
                    e
                );
            }
            ...
            if (
                ...
                && !ContentType.MatchesBase(message.ContentType, helper.HttpContentType)
            )
                throw new SoapException(
                    Res.GetString(
                        Res.WebRequestContent,
            ...
            try
            {
                if (!reader.IsStartElement(Soap.Element.Envelope, helper.EnvelopeNs))
                    throw new InvalidOperationException(
                        Res.GetString(Res.WebMissingEnvelopeElement)
                    );
                ...
                bool checkRequiredHeaders =
                    (this.serverMethod.wsiClaims & WsiProfiles.BasicProfile1_1) != 0
                    && version != SoapProtocolVersion.Soap12;
                string missingHeader = new SoapHeaderHandling().ReadHeaders(
                    reader,
                    serverMethod.inHeaderSerializer,
                    message.Headers,
                    serverMethod.inHeaderMappings,
                    SoapHeaderDirection.In,
                    helper.EnvelopeNs,
                    serverMethod.use == SoapBindingUse.Encoded ? helper.EncodingNs : null,
                ...
                if (missingHeader != null)
                {
                    throw new SoapHeaderException(
                        Res.GetString(Res.WebMissingHeader, missingHeader),
                ...
                if (Tracing.On)
                    Tracing.Enter(
                        Tracing.TraceId(Res.TraceReadRequest),
                        caller,
                        new TraceMethod(
                ...
                bool useDeserializationEvents =
                    !isEncodedSoap
                    && (WebServicesSection.Current.SoapEnvelopeProcessing.IsStrict || Tracing.On);
                if (useDeserializationEvents)
                {
                    XmlDeserializationEvents events = Tracing.On
                        ? Tracing.GetDeserializationEvents()
                        : RuntimeUtils.GetDeserializationEvents();
                    values = (object[])
                        serverMethod.parameterSerializer.Deserialize(reader, null, events);
                }
                else
                {
                    values = (object[])
        ...
        internal override void WriteReturns(object[] returnValues, Stream outputStream)
        {
            ...
            SoapHeaderHandling.GetHeaderMembers(
                message.Headers,
                this.Target,
                serverMethod.outHeaderMappings,
            ...
            if (isEncoded)
            {
                ...
                writer.WriteAttributeString(
                    ...
                    "types",
                    null,
                    SoapReflector.GetEncodedNamespace(
                        serverType.serviceNamespace,
                        serverType.serviceDefaultIsEncoded
                    )
                );
            }
            ...
            SoapHeaderHandling.WriteHeaders(
                writer,
                serverMethod.outHeaderSerializer,
                message.Headers,
                serverMethod.outHeaderMappings,
                SoapHeaderDirection.Out,
                isEncoded,
            ...
            if (Tracing.On)
                Tracing.Enter(
                    Tracing.TraceId(Res.TraceWriteResponse),
                    caller,
                    new TraceMethod(
        ...
        internal override bool WriteException(Exception e, Stream outputStream)
        {
            ...
            if (e is SoapException)
                ...
                serverMethod != null
                && serverMethod.rpc
                && helper.Version == SoapProtocolVersion.Soap12
                && e is ArgumentException
            )
                // special case to handle soap 1.2 rpc "BadArguments" fault
                soapException = SoapException.Create(
                    Version,
                    Res.GetString(Res.WebRequestUnableToProcess),
                    new XmlQualifiedName(Soap.Code.Client, Soap.Namespace),
                    null,
                    null,
                    null,
                    new SoapFaultSubCode(Soap12FaultCodes.RpcBadArgumentsFaultCode),
                    e
                );
            else
                soapException = SoapException.Create(
            ...
            if (!disableExtensions)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (!extensionStream.HasWritten)
                    {
                        ...
                        extensionException = SoapException.Create(
                            Version,
                            Res.GetString(Res.WebExtensionError),
                            new XmlQualifiedName(Soap.Code.Server, Soap.Namespace),
                            ex
                        );
                    }
                ...
                if (extensionException != null)
                {
                    Response.ContentType = ContentType.Compose(
        ...
        bool WriteException_TryWriteFault(
            SoapServerMessage message,
            Stream outputStream,
        ...
        private static XmlElement CreateUpgradeEnvelope(
            XmlDocument doc,
            string prefix,
            string envelopeNs
        )
        {
            XmlElement envelopeElement = doc.CreateElement(
                Soap12.Prefix,
                Soap12.Element.UpgradeEnvelope,
                Soap12.Namespace
            );
        ...
        internal XmlReader GetXmlReader()
        {
            ...
            bool checkEncoding =
                serverMethod != null
                && (serverMethod.wsiClaims & WsiProfiles.BasicProfile1_1) != 0
                && Version != SoapProtocolVersion.Soap12;
            if (checkEncoding && enc != null)
            {
                ...
                {
                    throw new InvalidOperationException(
                        Res.GetString(Res.WebWsiContentTypeEncoding)
                    );
                }
            }
            XmlReader reader = GetReaderForMessage(
                Message,
                RequestResponseUtils.GetBufferSize(Request.ContentLength)
            );
            if (reader == null)
        ...
        internal class SoapEnvelopeReader : XmlTextReader
        {
            ...
            internal SoapEnvelopeReader(TextReader input, Int64 timeout)
                : base(input)
            {
                this.readerTimedout = timeout;
            }

            internal SoapEnvelopeReader(Stream input, Int64 timeout)
                : base(input)
            {
                this.readerTimedout = timeout;
            }
    ...
    internal abstract class SoapServerProtocolHelper
    {
        ...
        internal HttpStatusCode SetResponseErrorCode(
            HttpResponse response,
            SoapException soapException
        )
        {
            if (
                soapException.SubCode != null
                && soapException.SubCode.Code == Soap12FaultCodes.UnsupportedMediaTypeFaultCode
            )
            {
        ...
        internal abstract void WriteFault(
            XmlWriter writer,
            SoapException soapException,
            HttpStatusCode statusCode
        );
