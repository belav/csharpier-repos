namespace System.Web.Services.Protocols
{
    ...
    internal class RemoteDebugger : INotifySource2
    {
        ...
        private static Guid IID_NotifyConnectionClassGuid = new Guid(
            "12A5B9F0-7A1C-4fcb-8163-160A30F519B5"
        );
        private static Guid IID_NotifyConnection2Guid = new Guid(
            "1AF04045-6659-4aaa-9F4B-2741AC56224B"
        );
        ...
        internal static bool IsClientCallOutEnabled()
        {
            ...
            {
                enabled =
                    !CompModSwitches.DisableRemoteDebugging.Enabled
                    && Debugger.IsAttached
                    && Connection != null;
            }
            catch (Exception e)
                ...
                if (connection == null && getConnection)
                {
                    lock (InternalSyncObject)
                    {
                        if (connection == null)
                        {
                            ...
                            TraceMethod method = Tracing.On
                                ? new TraceMethod(typeof(RemoteDebugger), "get_Connection")
                                : null;
                            if (Tracing.On)
                                Tracing.Enter("RemoteDebugger", method);

                            object unk;
                            int result = UnsafeNativeMethods.CoCreateInstance(
                                ref IID_NotifyConnectionClassGuid,
                                null,
                                INPROC_SERVER,
        ...
        private INotifySink2 NotifySink
        {
            get
            {
                ...
                {
                    TraceMethod method = Tracing.On
                        ? new TraceMethod(this, "get_NotifySink")
                        : null;
                    if (Tracing.On)
                        Tracing.Enter("RemoteDebugger", method);
        ...
        private static void CloseSharedResources()
        {
            if (connection != null)
            {
                lock (InternalSyncObject)
                {
                    ...
                    {
                        TraceMethod method = Tracing.On
                            ? new TraceMethod(typeof(RemoteDebugger), "CloseSharedResources")
                            : null;
                        if (Tracing.On)
                            Tracing.Enter("RemoteDebugger", method);
        ...
        internal void NotifyClientCallOut(WebRequest request)
        {
            try
            {
                ...
                TraceMethod method = Tracing.On
                    ? new TraceMethod(this, "NotifyClientCallOut")
                    : null;
                if (Tracing.On)
                    Tracing.Enter("RemoteDebugger", method);

                UnsafeNativeMethods.OnSyncCallOut(
                    NotifySink,
                    callId,
                    out bufferPtr,
                    ref bufferSize
        ...
        internal void NotifyClientCallReturn(WebResponse response)
        {
            try
            {
                ...
                TraceMethod method = Tracing.On
                    ? new TraceMethod(this, "NotifyClientCallReturn")
                    : null;
                if (Tracing.On)
                    Tracing.Enter("RemoteDebugger", method);
        ...
        internal void NotifyServerCallEnter(ServerProtocol protocol, string stringBuffer)
        {
            try
            {
                ...
                TraceMethod method = Tracing.On
                    ? new TraceMethod(this, "NotifyServerCallEnter")
                    : null;
                if (Tracing.On)
                    Tracing.Enter("RemoteDebugger", method);
        ...
        internal void NotifyServerCallExit(HttpResponse response)
        {
            try
            {
                ...
                TraceMethod method = Tracing.On
                    ? new TraceMethod(this, "NotifyServerCallExit")
                    : null;
                if (Tracing.On)
                    Tracing.Enter("RemoteDebugger", method);

                UnsafeNativeMethods.OnSyncCallExit(
                    NotifySink,
                    callId,
                    out bufferPtr,
                    ref bufferSize
        ...
        void INotifySource2.SetNotifyFilter(
