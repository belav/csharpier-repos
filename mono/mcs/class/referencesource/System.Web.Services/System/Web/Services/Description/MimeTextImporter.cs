    ...
    internal class MimeTextImporter : MimeImporter
    {
        ...
        internal override MimeReturn ImportReturn()
        {
            MimeTextBinding mimeTextBinding = (MimeTextBinding)
                ImportContext.OperationBinding.Output.Extensions.Find(typeof(MimeTextBinding));
            if (mimeTextBinding == null)
                return null;
            if (mimeTextBinding.Matches.Count == 0)
            {
                ImportContext.UnsupportedOperationBindingWarning(
                    Res.GetString(Res.MissingMatchElement0)
                );
                return null;
            }
        ...
        internal override void GenerateCode(
            MimeReturn[] importedReturns,
            MimeParameterCollection[] importedParameters
        )
        {
            for (int i = 0; i < importedReturns.Length; i++)
            {
                ...
                {
                    GenerateCode(
                        (MimeTextReturn)importedReturns[i],
        ...
        )
        {
            ...
            for (int i = 0; i < matches.Count; i++)
            {
                ...
                string name = members.AddUnique(
                    CodeIdentifier.MakeValid(
                        match.Name.Length == 0 ? methodName + "Match" : match.Name
                    ),
                    match
                );
                ...
                WebCodeGenerator.AddCustomAttribute(
                    metadata,
                    typeof(MatchAttribute),
                    new CodeExpression[] { pattern },
                    propertyNames,
                    propertyValues
                );
                ...
                CodeTypeMember member = WebCodeGenerator.AddMember(
                    codeClass,
                    fieldTypeName,
                    name,
                    null,
                ...
                if (match.Matches.Count == 0 && match.Type.Length > 0)
                {
                    ...
                    ProtocolImporter.AddWarningComment(
                        member.Comments,
