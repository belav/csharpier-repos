...
namespace System.Web.Services.Description
{
    ...
    internal abstract class HttpProtocolImporter : ProtocolImporter
    {
        ...
        {
            //
            HttpUrlEncodedBinding httpUrlEncodedBinding = (HttpUrlEncodedBinding)
                OperationBinding.Input.Extensions.Find(typeof(HttpUrlEncodedBinding));
        ...
        MimeParameter ImportUrlParameter(MessagePart part)
        {
            ...
            parameter.TypeName = IsRepeatingParameter(part)
        ...
        bool IsRepeatingParameter(MessagePart part)
        {
            XmlSchemaComplexType type = (XmlSchemaComplexType)
                Schemas.Find(part.Type, typeof(XmlSchemaComplexType));
            if (type == null)
                return false;
            ...
            if (type.ContentModel.Content == null)
                throw new ArgumentException(
                    Res.GetString(Res.Missing2, type.Name, type.ContentModel.GetType().Name),
                    "part"
                );
            if (type.ContentModel.Content is XmlSchemaComplexContentExtension)
            {
                return ((XmlSchemaComplexContentExtension)type.ContentModel.Content).BaseTypeName
                    == new XmlQualifiedName(Soap.ArrayType, Soap.Encoding);
            }
            else if (type.ContentModel.Content is XmlSchemaComplexContentRestriction)
            {
                return ((XmlSchemaComplexContentRestriction)type.ContentModel.Content).BaseTypeName
                    == new XmlQualifiedName(Soap.ArrayType, Soap.Encoding);
            }
            return false;
        }
        ...
        static void AppendMetadata(
            CodeAttributeDeclarationCollection from,
            CodeAttributeDeclarationCollection to
        )
        {
        ...
        CodeMemberMethod GenerateMethod(HttpMethodInfo method)
        {
            MimeParameterCollection parameters =
                method.MimeParameters != null ? method.MimeParameters : method.UrlParameters;
            ...
            if (method.MimeParameters != null)
                formatterTypes[1] = new CodeTypeOfExpression(
                    method.MimeParameters.WriterType.FullName
                );
            else
            ...
            CodeMemberMethod mainCodeMethod = WebCodeGenerator.AddMethod(
                ...
                method.Name,
                new CodeFlags[parameterTypeNames.Length],
                parameterTypeNames,
                parameterNames,
            ...
            for (int i = 0; i < parameters.Count; i++)
            {
                AppendMetadata(
                    ...
                    mainCodeMethod.Parameters[i].CustomAttributes
                );
            }
            ...
            if (Style == ServiceDescriptionImportStyle.Client)
            {
                ...
                if (oldAsync)
                {
                    ...
                    CodeMemberMethod endCodeMethod = WebCodeGenerator.AddMethod(
                        this.CodeTypeDeclaration,
                        "End" + method.Name,
                        new CodeFlags[1],
                ...
                if (newAsync)
                {
                    ...
                    invokeParams = new CodeExpression[5];
                    CreateInvokeParams(invokeParams, method, parameterNames);
                    invokeParams[3] = new CodeFieldReferenceExpression(
                        new CodeThisReferenceExpression(),
                        callbackMember
                    );
        ...
        )
        {
            ...
            CodeExpression left = new CodePropertyReferenceExpression(
                new CodeThisReferenceExpression(),
                "Url"
            );
            CodeExpression right = new CodePrimitiveExpression(method.Href);
            invokeParams[1] = new CodeBinaryOperatorExpression(
                left,
        ...
        {
            HttpOperationBinding httpOperationBinding = (HttpOperationBinding)
                OperationBinding.Extensions.Find(typeof(HttpOperationBinding));
            if (httpOperationBinding == null)
                throw OperationBindingSyntaxException(
                    Res.GetString(Res.MissingHttpOperationElement0)
                );
            ...
            if (hasInputPayload)
            {
                ...
                if (method.MimeParameters == null)
                {
                    UnsupportedOperationWarning(
                        Res.GetString(Res.NoInputMIMEFormatsWereRecognized0)
                    );
                    return null;
                }
            }
            else
            {
                ...
                {
                    UnsupportedOperationWarning(
                        Res.GetString(Res.NoInputHTTPFormatsWereRecognized0)
                    );
                    return null;
                }
            }
        ...
        protected override CodeTypeDeclaration BeginClass()
        {
            ...
            HttpAddressBinding httpAddressBinding =
                Port == null
                    ? null
                    : (HttpAddressBinding)Port.Extensions.Find(typeof(HttpAddressBinding));
            ...
            ProtocolImporterUtil.GenerateConstructorStatements(
                ctor,
                url,
                serviceDescription.AppSettingUrlKey,
                serviceDescription.AppSettingBaseUrl,
                false
            );
        ...
        protected override void EndNamespace()
        {
            for (int i = 0; i < importers.Length; i++)
            {
                importers[i].GenerateCode(
                    (MimeReturn[])importedReturns[i].ToArray(typeof(MimeReturn)),
                    (MimeParameterCollection[])
                        importedParameters[i].ToArray(typeof(MimeParameterCollection))
                );
            }
