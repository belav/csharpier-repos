...
namespace System.Web.Services.Discovery
{
    ...
    public sealed class DiscoveryDocumentReference : DiscoveryReference
    {
        ...
        /// </devdoc>
        private static DiscoveryDocument GetDocumentNoParse(
            ref string url,
        ...
        )
        {
            ...
            try
            {
                XmlTextReader reader = new XmlTextReader(
                    new StreamReader(stream, RequestResponseUtils.GetEncoding(contentType))
                );
                ...
                if (!DiscoveryDocument.CanRead(reader))
                {
                    ...
                    ArgumentException exception = new ArgumentException(
                        Res.GetString(Res.WebInvalidFormat)
                    );
                    throw new InvalidOperationException(
                        Res.GetString(Res.WebMissingDocument, url),
                        exception
                    );
                }
        ...
        protected internal override void Resolve(string contentType, Stream stream)
        {
            ...
            { // probably xml...
                ...
                else
                {
                    ...
                    stream.Position = 0;
                    XmlTextReader newReader = new XmlTextReader(
                        new StreamReader(stream, RequestResponseUtils.GetEncoding(contentType))
                    );
                    ...
                    while (newReader.NodeType != XmlNodeType.Element)
                    {
                        if (newReader.NodeType == XmlNodeType.ProcessingInstruction)
                        {
                            ...
                            sb.Append("/>");
                            XmlTextReader piReader = new XmlTextReader(
                            ...
                            if (
                                type != null
                                && ContentType.MatchesBase(type, ContentType.TextXml)
                                && alternate != null
                                && string.Compare(
                                    alternate,
                                    "yes",
            ...
            foreach (object o in document.References)
            {
                if (o is DiscoveryReference)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            Res.GetString(Res.WebEmptyRef, r.GetType().FullName, Url)
                        );
                    }
