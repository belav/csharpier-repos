        ...
        internal SoapClientType(Type type)
        {
            ...
            if (Tracing.On)
                Tracing.Enter(
                    ...
                    caller,
                    new TraceMethod(typeof(XmlSerializer), "FromMappings", xmlMappings, type)
                );
            ...
            for (int i = 0; i < extensionTypes.Count; i++)
            {
                ...
                SoapReflectedExtension extension = new SoapReflectedExtension(
                    extensionTypes[i].Type,
                    null,
                    extensionTypes[i].Priority
                );
            ...
            for (int i = 0; i < soapMethodList.Count; i++)
            {
                ...
                clientMethod.extensionInitializers = SoapReflectedExtension.GetInitializers(
                ...
                for (int j = 0; j < soapMethod.headers.Length; j++)
                {
                    ...
                    if (
                        (mapping.direction & (SoapHeaderDirection.Out | SoapHeaderDirection.Fault))
                        != 0
                    )
                        outHeaders.Add(mapping);
                }
                clientMethod.inHeaderMappings = (SoapHeaderMapping[])
                    inHeaders.ToArray(typeof(SoapHeaderMapping));
                if (clientMethod.outHeaderSerializer != null)
                    clientMethod.outHeaderMappings = (SoapHeaderMapping[])
                        outHeaders.ToArray(typeof(SoapHeaderMapping));
                methods.Add(soapMethod.name, clientMethod);
            }
        }

        internal static void GenerateXmlMappings(
            Type type,
            ArrayList soapMethodList,
    ...
    public class SoapHttpClientProtocol : HttpWebClientProtocol
    {
        ...
        public void Discover()
        {
            if (clientType.Binding == null)
                throw new InvalidOperationException(
                    Res.GetString(
                        Res.DiscoveryIsNotPossibleBecauseTypeIsMissing1,
            ...
            foreach (object item in doc.References)
            {
                System.Web.Services.Discovery.SoapBinding soapBinding =
                    item as System.Web.Services.Discovery.SoapBinding;
                if (soapBinding != null)
                {
                    if (
                        clientType.Binding.Name == soapBinding.Binding.Name
            ...
            throw new InvalidOperationException(
                Res.GetString(
                    Res.TheBindingNamedFromNamespaceWasNotFoundIn3,
                    clientType.Binding.Name,
                    clientType.Binding.Namespace,
                    Url
                )
            );
        }
        ...
        protected virtual XmlWriter GetWriterForMessage(SoapClientMessage message, int bufferSize)
        {
            ...
            XmlTextWriter writer = new XmlTextWriter(
                new StreamWriter(
                    message.Stream,
                    RequestEncoding != null ? RequestEncoding : new UTF8Encoding(false),
                    bufferSize
                )
            );
            /*
        ...
        protected virtual XmlReader GetReaderForMessage(SoapClientMessage message, int bufferSize)
        {
            Encoding enc =
                message.SoapVersion == SoapProtocolVersion.Soap12
                    ? RequestResponseUtils.GetEncoding2(message.ContentType)
                    : RequestResponseUtils.GetEncoding(message.ContentType);
            if (bufferSize < 512)
                bufferSize = 512;
        ...
        internal override void InitializeAsyncRequest(WebRequest request, object internalAsyncState)
        {
            ...
            invokeState.Message = BeforeSerialize(
                request,
                invokeState.MethodName,
            ...
            if (asyncResult.Request != null)
            {
                ...
                catch (XmlException e)
                {
                    ...
                    exception = new InvalidOperationException(
                        Res.GetString(Res.WebResponseBadXml),
                        e
                    );
                }
                catch (Exception e)
        ...
        private static Array CombineExtensionsHelper(
            Array array1,
            Array array2,
            Array array3,
            Type elementType
        )
        {
            ...
            if (elementType == typeof(SoapReflectedExtension))
                ...
                throw new ArgumentException(
                    Res.GetString(Res.ElementTypeMustBeObjectOrSoapReflectedException),
                    "elementType"
                );
        ...
        )
        {
            ...
            SoapReflectedExtension[] allExtensions =
                (SoapReflectedExtension[])CombineExtensionsHelper(
                    clientType.HighPriExtensions,
                    method.extensions,
                    clientType.LowPriExtensions,
            ...
            object[] allExtensionInitializers = (object[])CombineExtensionsHelper(
                ...
                method.extensionInitializers,
                clientType.LowPriExtensionInitializers,
                typeof(object)
            );
            SoapExtension[] initializedExtensions = SoapMessage.InitializeExtensions(
                allExtensions,
                allExtensionInitializers
            );
            ...
            if (this.version == SoapProtocolVersion.Soap12)
                message.ContentType = ContentType.Compose(
                    ContentType.ApplicationSoap,
                    RequestEncoding != null ? RequestEncoding : Encoding.UTF8,
                    soapAction
                );
            else
                message.ContentType = ContentType.Compose(
        ...
        void Serialize(SoapClientMessage message)
        {
            ...
            if (isEncoded)
            {
                ...
                writer.WriteAttributeString(
                    ...
                    SoapReflector.GetEncodedNamespace(
                        clientType.serviceNamespace,
                        clientType.serviceDefaultIsEncoded
                    )
                );
            }
            ...
            if (Tracing.On)
                Tracing.Enter(
                    Tracing.TraceId(Res.TraceWriteRequest),
                    caller,
                    new TraceMethod(
        ...
        )
        {
            ...
            if (!isSoap || (isSoap && (httpResponse != null) && (httpResponse.ContentLength == 0)))
            {
                ...
                if (statusCode == 400)
                    ...
                    throw new InvalidOperationException(
                        Res.GetString(Res.WebResponseContent, message.ContentType, HttpContentType)
                            + Environment.NewLine
                            + RequestResponseUtils.CreateResponseExceptionString(
                                response,
                                responseStream
            ...
            if (envelopeNs == null || envelopeNs.Length == 0)
                ...
                throw new SoapException(
                    Res.GetString(Res.WebInvalidEnvelopeNamespace, envelopeNs, EnvelopeNs),
                    SoapException.VersionMismatchFaultCode
                );

            reader.MoveToContent();
            SoapHeaderHandling headerHandler = new SoapHeaderHandling();
            headerHandler.ReadHeaders(
                reader,
                method.outHeaderSerializer,
                message.Headers,
                method.outHeaderMappings,
                SoapHeaderDirection.Out | SoapHeaderDirection.Fault,
                envelopeNs,
                method.use == SoapBindingUse.Encoded ? encodingNs : null,
                false
            );
            reader.MoveToContent();
                ...
                else
                {
                    ...
                    if (useDeserializationEvents)
                    {
                        ...
                        message.SetParameterValues(
                            (object[])method.returnSerializer.Deserialize(reader, null, events)
                        );
                    }
                    else
                    {
        ...
        SoapException ReadSoapException(XmlReader reader)
        {
            ...
            else
            {
                ...
                {
                    if (
                        reader.NamespaceURI == Soap.Namespace
                        || reader.NamespaceURI == Soap12.Namespace
                        || reader.NamespaceURI == null
                        || reader.NamespaceURI.Length == 0
                    )
                    {
                        if (
                            reader.LocalName == Soap.Element.FaultCode
                            || reader.LocalName == Soap12.Element.FaultCode
                        )
                        {
                            if (soap12)
                        ...
                        else if (reader.LocalName == Soap12.Element.FaultReason)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    if (
                                        reader.LocalName == Soap12.Element.FaultReasonText
                                        && reader.NamespaceURI == Soap12.Namespace
                                    )
                                    {
                                        faultString = reader.ReadElementString();
                            ...
                            reader.LocalName == Soap.Element.FaultActor
                            || reader.LocalName == Soap12.Element.FaultNode
                        )
                        {
                            faultActor = reader.ReadElementString();
                        }
                        else if (
                            reader.LocalName == Soap.Element.FaultDetail
            ...
            if (detail != null || soap12) // with soap 1.2, can't tell if fault is for header
                ...
                return new SoapHeaderException(
                    faultString,
                    faultCode,
                    faultActor,
                    faultRole,
        ...
        private XmlQualifiedName ReadFaultCode(XmlReader reader)
        {
            ...
            if (colon >= 0)
            {
                ...
                if (ns == null)
                    throw new InvalidOperationException(
                        Res.GetString(Res.WebQNamePrefixUndefined, prefix)
                    );
            }
            reader.ReadEndElement();
