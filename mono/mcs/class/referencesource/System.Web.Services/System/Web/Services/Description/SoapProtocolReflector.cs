...
namespace System.Web.Services.Description
{
    ...
    internal class SoapProtocolReflector : ProtocolReflector
    {
        ...
        internal SoapReflectionImporter SoapImporter
        {
            get
            {
                SoapReflectionImporter soapImporter =
                    ReflectionContext[typeof(SoapReflectionImporter)] as SoapReflectionImporter;
                if (soapImporter == null)
                {
                    soapImporter = SoapReflector.CreateSoapImporter(
                        DefaultNamespace,
                        SoapReflector.ServiceDefaultIsEncoded(ServiceType)
                    );
        ...
        internal SoapSchemaExporter SoapExporter
        {
            get
            {
                SoapSchemaExporter soapExporter =
                    ReflectionContext[typeof(SoapSchemaExporter)] as SoapSchemaExporter;
        ...
        protected override bool ReflectMethod()
        {
            ...
            if (!soapMethod.oneWay)
                CreateMessage(
                    ...
                    soapMethod.responseMappings
                );

            CreateHeaderMessages(
                soapMethod.name,
                soapMethod.use,
                soapMethod.inHeaderMappings,
                soapMethod.outHeaderMappings,
                soapMethod.headers,
                soapMethod.rpc
            );
            ...
            if (
                ...
                && soapMethod.methodInfo.OutParameters.Length > 0
            )
        ...
        void CreateHeaderMessages(
            string methodName,
            SoapBindingUse use,
            XmlMembersMapping inHeaderMappings,
            XmlMembersMapping outHeaderMappings,
            SoapReflectedHeader[] headers,
            bool rpc
        )
        {
            //
            ...
            for (int i = 0; i < headers.Length; i++)
            {
                ...
                ServiceDescriptionFormatExtension soapHeaderBinding = CreateSoapHeaderBinding(
                    new XmlQualifiedName(message.Name, Binding.ServiceDescription.TargetNamespace),
                    part.Name,
                    rpc ? member.Namespace : null,
                    use
                );
                ...
                if ((soapHeader.direction & SoapHeaderDirection.Fault) != 0)
                {
                    if (soapMethod.IsClaimsConformance)
                    {
                        throw new InvalidOperationException(
                            Res.GetString(
                                Res.BPConformanceHeaderFault,
                                soapMethod.methodInfo.ToString(),
                                soapMethod.methodInfo.DeclaringType.FullName,
                                "Direction",
        ...
        )
        {
            if (
                members.Count == 1
                && members[0].Any
                && members[0].ElementName.Length == 0
                && !wrapped
            )
            {
                    ...
                    for (int i = 0; i < members.Count; i++)
                    {
                        ...
                        if (rpc)
                        {
                            ...
                            if (member.TypeName == null || member.TypeName.Length == 0)
                            {
                                throw new InvalidOperationException(
                                    Res.GetString(
                                        Res.WsdlGenRpcLitAnonimousType,
                        ...
                        else
                        {
                            ...
                            part.Element = new XmlQualifiedName(
        ...
        protected virtual SoapHeaderBinding CreateSoapHeaderBinding(
