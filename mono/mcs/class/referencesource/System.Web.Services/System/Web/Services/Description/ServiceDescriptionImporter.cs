    ...
    public class ServiceDescriptionImporter
    {
        ...
        ProtocolImporter FindImporterByName(string protocolName)
        {
            ...
            throw new ArgumentException(
                Res.GetString(Res.ProtocolWithNameIsNotRecognized1, protocolName),
                "protocolName"
            );
        }
            ...
            foreach (WebReference webReference in webReferences)
            {
                ...
                foreach (DictionaryEntry entry in webReference.Documents)
                {
                    AddDocument(
                        ...
                        schemas,
                        descriptions,
                        webReference.ValidationWarnings
                    );
                }

                importer.Schemas.Add(schemas);
                foreach (ServiceDescription source in descriptions)
                    importer.AddServiceDescription(
                        source,
                        webReference.AppSettingUrlKey,
                        webReference.AppSettingBaseUrl
                    );
                ...
                webReference.Warnings = importer.Import(
                    webReference.ProxyCode,
                    context,
                    exportedMappings,
                    webReference.ValidationWarnings
        ...
        internal static void AddDocument(
            string path,
            object document,
            XmlSchemas schemas,
        ...
        private ServiceDescriptionImportWarnings Import(
            CodeNamespace codeNamespace,
        ...
        )
        {
            ...
            if (!allSchemas.Contains(Soap.Encoding))
            {
                ...
                references[ServiceDescription.SoapEncodingSchema] =
                    ServiceDescription.SoapEncodingSchema;
            }
            ...
            foreach (ServiceDescription description in serviceDescriptions)
            {
                foreach (Message message in description.Messages)
                {
                    foreach (MessagePart part in message.Parts)
                    {
                        ...
                        if (part.Element != null && !part.Element.IsEmpty)
                        {
                            if (isEncoded)
                                throw new InvalidOperationException(
                                    Res.GetString(
                                        Res.CanTSpecifyElementOnEncodedMessagePartsPart,
                                        part.Name,
                                        message.Name
                                    )
                                );
                            XmlSchemaElement element = (XmlSchemaElement)
                                allSchemas.Find(part.Element, typeof(XmlSchemaElement));
                            if (element != null)
                            {
                                AddSchema(
                        ...
                        {
                            XmlSchemaType type = (XmlSchemaType)
                                allSchemas.Find(part.Type, typeof(XmlSchemaType));
                            if (type != null)
                            {
                                AddSchema(
                                    type.Parent as XmlSchema,
