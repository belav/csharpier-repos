...
namespace System.Web.Services.Protocols
{
                ...
                switch (value)
                {
                    ...
                    default:
                        throw new ArgumentException(
                            Res.GetString(Res.WebHeaderInvalidMustUnderstand, value)
                        );
                }
            }
        }
        ...
        public string EncodedRelay
        {
            ...
            set
            {
                switch (value)
                {
                    ...
                    default:
                        throw new ArgumentException(
                            Res.GetString(Res.WebHeaderInvalidRelay, value)
                        );
                }
            }
        }
    ...
    public sealed class SoapHeaderHandling
    {
        ...
        )
        {
            ...
            events.OnUnreferencedObject = new UnreferencedObjectEventHandler(
            ...
            if (Tracing.On)
                Tracing.Enter(
                    Tracing.TraceId(Res.TraceReadHeaders),
                    caller,
                    new TraceMethod(serializer, "Deserialize", reader, encodingStyle)
                );
        ...
        )
        {
            ...
            for (int i = 0; i < headers.Count; i++)
            {
                ...
                if (header is SoapUnknownHeader)
                {
                    ...
                    (headerPosition = FindMapping(mappings, header, direction)) >= 0
                    && !headerSet[headerPosition]
                )
                {
            ...
            TraceMethod caller = Tracing.On
                ? new TraceMethod(typeof(SoapHeaderHandling), "WriteHeaders")
                : null;
            if (Tracing.On)
                Tracing.Enter(
                    Tracing.TraceId(Res.TraceWriteHeaders),
                    caller,
                    new TraceMethod(
                        serializer,
                        "Serialize",
            ...
            foreach (SoapHeader header in otherHeaders)
            {
                ...
                { // encoded headers already appended to members mapping
                    ...
                    if (Tracing.On)
                        Tracing.Enter(
                            Tracing.TraceId(Res.TraceWriteHeaders),
                            caller,
                            new TraceMethod(headerSerializer, "Serialize", writer, header)
                        );
            ...
            if (mappings != null)
            {
                for (int i = 0; i < mappings.Length; i++)
                {
                    ...
                    if (mapping.repeats)
                    {
                        ...
                        MemberHelper.SetValue(
                            mapping.memberInfo,
                            target,
                            list.ToArray(mapping.headerType)
                        );
                    }
                    else
                    {
                ...
                if (!headerHandled[i])
                {
                    ...
                    if (header.MustUnderstand && !header.DidUnderstand)
                    {
                        throw new SoapHeaderException(
                            Res.GetString(
                                Res.WebCannotUnderstandHeader,
                                GetHeaderElementName(header)
                            ),
        ...
        public static void EnsureHeadersUnderstood(SoapHeaderCollection headers)
        {
            for (int i = 0; i < headers.Count; i++)
            {
                ...
                {
                    throw new SoapHeaderException(
                        Res.GetString(Res.WebCannotUnderstandHeader, GetHeaderElementName(header)),
                        new XmlQualifiedName(Soap.Code.MustUnderstand, Soap.Namespace)
                    );
                }
            }
