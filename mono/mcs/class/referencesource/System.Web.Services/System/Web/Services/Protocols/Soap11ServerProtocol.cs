...
namespace System.Web.Services.Protocols
{
    ...
    internal class Soap11ServerProtocolHelper : SoapServerProtocolHelper
    {
        ...
        internal override SoapServerMethod RouteRequest()
        {
            ...
            if (methodUriString == null)
                throw new SoapException(
                    Res.GetString(Res.UnableToHandleRequestActionRequired0),
            ...
            {
                if (
                    methodUriString.StartsWith("\"", StringComparison.Ordinal)
                    && methodUriString.EndsWith("\"", StringComparison.Ordinal)
                )
            ...
            else
            {
                ...
                catch (Exception e)
                {
                    ...
                    throw new SoapException(
                        Res.GetString(Res.TheRootElementForTheRequestCouldNotBeDetermined0),
            ...
            if (Tracing.On)
                Tracing.Enter(
                    "RouteRequest",
                    caller,
                    new TraceMethod(ServerType, "GetMethod", methodKey),
                    Tracing.Details(ServerProtocol.Request)
                );
            ...
            if (method == null)
            {
                if (ServerType.routingOnSoapAction)
                    throw new SoapException(
                        Res.GetString(Res.WebHttpHeader, Soap.Action, (string)methodKey),
                        new XmlQualifiedName(Soap.Code.Client, Soap.Namespace)
                    );
                else
                    throw new SoapException(
                        Res.GetString(
                            Res.TheRequestElementXmlnsWasNotRecognized2,
                            ((XmlQualifiedName)methodKey).Name,
                            ((XmlQualifiedName)methodKey).Namespace
                        ),
                        new XmlQualifiedName(Soap.Code.Client, Soap.Namespace)
                    );
            }
        ...
        internal override void WriteFault(
            XmlWriter writer,
        ...
        )
        {
            ...
            if (ServerProtocol.ServerMethod != null)
                SoapHeaderHandling.WriteHeaders(
                    writer,
                    ServerProtocol.ServerMethod.outHeaderSerializer,
                    message.Headers,
                    ServerProtocol.ServerMethod.outHeaderMappings,
                    SoapHeaderDirection.Fault,
                    ServerProtocol.ServerMethod.use == SoapBindingUse.Encoded,
            ...
            if (
                code.Namespace != null
                && code.Namespace.Length > 0
                && writer.LookupPrefix(code.Namespace) == null
            )
