    ...
    internal class LinkGrep
    {
                ...
                if ((match = tagRegex.Match(text, textpos)).Success)
                {
                    ...
                    if (String.Compare(tag, "link", StringComparison.OrdinalIgnoreCase) == 0)
                    {
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            if (
                                String.Compare(attrName, "type", StringComparison.OrdinalIgnoreCase)
                                    == 0
                            ...
                            )
                            {
                                ...
                                String.Compare(attrName, "rel", StringComparison.OrdinalIgnoreCase)
                                    == 0
                                && String.Compare(
                                    attrValue,
                                    "alternate",
                            ...
                            )
                            {
                                ...
                                String.Compare(attrName, "href", StringComparison.OrdinalIgnoreCase)
                                == 0
                            )
                            {
                                href = attrValue;
        ...
        private static readonly Regex tagRegex = new Regex(
            "\\G<"
                ...
                + //     quoted value
                "\"(?<attrval>[^\"]*)\""
                + //          double quoted attrval
                "|'(?<attrval>[^\']*)'"
                + //          single quoted attrval
                "|(?<attrval>[a-zA-Z0-9\\-._:]+)"
                + //          attrval with no quotes (SGML-approved chars)
                ")"
                + //     end quoted value
                ")*"
                + // end attribute
