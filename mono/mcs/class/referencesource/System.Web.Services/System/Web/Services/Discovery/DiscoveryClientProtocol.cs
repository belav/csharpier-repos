...
namespace System.Web.Services.Discovery
{
    ...
    public class DiscoveryClientProtocol : HttpWebClientProtocol
    {
            ...
            if (discoRef == null)
            {
                if (errorInValidDocument != null)
                {
                    StringBuilder errorMessage = new StringBuilder(
                        Res.GetString(Res.TheDocumentWasUnderstoodButContainsErrors)
                    );
                ...
                else if (allErrorsAreHtmlContentType)
                {
                    throw new InvalidOperationException(
                        Res.GetString(Res.TheHTMLDocumentDoesNotContainDiscoveryInformation)
                    );
                }
                else
                {
                    ...
                    if (same)
                        throw new InvalidOperationException(
                            Res.GetString(
                                Res.TheDocumentWasNotRecognizedAsAKnownDocumentType,
                                specialErrorMessages[0]
                            )
                        );
                    else
                    {
                        Exception e;
                        StringBuilder errorMessage = new StringBuilder(
                            Res.GetString(Res.WebMissingResource, url)
                        );
                        foreach (DictionaryEntry entry in Errors)
                        {
                            ...
                            if (
                                ...
                                == string.Compare(
                                    refType,
                                    typeof(ContractReference).FullName,
                            ...
                            )
                            {
                                ...
                                == string.Compare(
                                    refType,
                                    typeof(SchemaReference).FullName,
                                    StringComparison.Ordinal
                                )
                            )
                            {
                                ...
                                == string.Compare(
                                    refType,
                                    typeof(DiscoveryDocumentReference).FullName,
                                    StringComparison.Ordinal
                                )
                            )
                            {
                            ...
                            errorMessage
                                .Append("\n- ")
                                .Append(Res.GetString(Res.WebDiscoRefReport, refType, e.Message));
        ...
        public Stream Download(ref string url, ref string contentType)
        {
            ...
            if (httpResponse != null)
            {
                ...
                {
                    string errorMessage = RequestResponseUtils.CreateResponseExceptionString(
                        httpResponse
                    );
                    throw new WebException(
                        Res.GetString(Res.ThereWasAnErrorDownloading0, url),
                        new WebException(
                            errorMessage,
                            null,
            ...
            try
            {
                ...
                if (
                    response.ResponseUri.Scheme == Uri.UriSchemeFtp
            ...
            try
            {
                ...
                for (int i = 0; i < results.Results.Count; i++)
                {
                    ...
                    if (typeName == null || typeName.Length == 0)
                        throw new InvalidOperationException(
                            Res.GetString(Res.WebRefInvalidAttribute, "referenceType")
                        );
                    ...
                    try
                    {
                        ...
                        Debug.Assert(
                            Documents[reference.Url] != null,
                            "Couldn't deserialize file " + results.Results[i].Filename
                        );
                    }
                    finally
                    {
        ...
        private static string GetRelativePath(string fullPath, string relativeTo)
        {
            ...
            {
                if (
                    currentDir.Length <= fullPath.Length
                    && string.Compare(
                        currentDir,
                        fullPath.Substring(0, currentDir.Length),
        ...
        public DiscoveryClientResultCollection WriteAll(string directory, string topLevelFilename)
        {
            ...
            DictionaryEntry[] entries = new DictionaryEntry[
                Documents.Count + InlinedSchemas.Keys.Count
            ];
            int i = 0;
            ...
            foreach (DictionaryEntry entry in entries)
            {
                ...
                string filename =
                    reference == null
                        ? DiscoveryReference.FilenameFromUrl(Url)
    ...
    public sealed class DiscoveryClientResultCollection : CollectionBase
    {
        ...
        {
            get { return (DiscoveryClientResult)List[i]; }
            set { List[i] = value; }
        }
