    ...
    internal class XsltLoader : IErrorHelper
    {
        ...
        private bool V1
        {
            get
            {
                Debug.Assert(
                    compiler.Version != 0,
                    "Version should be already decided at this point"
                );
        ...
        {
            Uri resolvedBaseUri =
                (baseUri.Length != 0) ? xmlResolver.ResolveUri(null, baseUri) : null;
        ...
        private XmlReader CreateReader(Uri uri, XmlResolver xmlResolver)
        {
            ...
            throw new XslLoadException(
                Res.Xslt_CannotLoadStylesheet,
                uri.ToString(),
                input == null ? "null" : input.GetType().ToString()
            );
        }
        ...
        private Stylesheet LoadStylesheet(XmlReader reader, bool include)
        {
            ...
            Debug.Assert(
                !documentUriInUse.Contains(baseUri),
            ...
            try
            {
                ...
                if (!include)
                {
                    ...
                    {
                        curStylesheet.Imports[i] = LoadStylesheet(
                            importHrefs[i], /*include:*/
                            false
                        );
                    }
                }
        ...
        private void LoadSimplifiedStylesheet()
        {
            ...
            XslNode lre = LoadLiteralResultElement( /*asStylesheet:*/
                true
            );
            if (lre != null)
            {
        ...
        private void LoadRealStylesheet()
        {
            Debug.Assert(
                input.IsXsltNamespace()
                    && (input.IsKeyword(atoms.Stylesheet) || input.IsKeyword(atoms.Transform))
            );
        ...
        private void LoadOutput()
        {
                    ...
                    )
                    {
                        if (currentPrec == output.MethodPrec && !output.Method.Equals(method))
                        {
                            ReportWarning( /*[XT1560]*/
                                Res.Xslt_AttributeRedefinition,
                                "method"
                            );
                        }
            ...
            if (input.MoveToXsltAttribute(6, "encoding"))
            {
                if (output.EncodingPrec <= currentPrec)
                {
                    ...
                    catch (ArgumentException)
                    {
                        if (!input.ForwardCompatibility)
                        {
                            ReportWarning( /*[XT_004]*/
                                Res.Xslt_InvalidEncoding,
                                input.Value
                            );
                        }
                    }
            ...
            bool escapeUriAttributes =
                ParseYesNoAttribute(7, "escape-uri-attributes") != TriState.False;
            if (!escapeUriAttributes)
                ReportNYI("xsl:output/@escape-uri-attributes == flase()");

            bool includeContentType =
                ParseYesNoAttribute(8, "include-content-type") != TriState.False;
            ...
            if (triState != TriState.Unknown)
            {
                if (output.StandalonePrec <= currentPrec)
                {
                    XmlStandalone standalone =
                        (triState == TriState.True) ? XmlStandalone.Yes : XmlStandalone.No;
        ...
        */
        ...
        private void CheckAttributeSetsDfs(AttributeSet attSet)
        {
            ...
            switch (attSet.CycleCheck)
            {
                ...
                default:
                    ...
                    compiler.ReportError( /*[XT0720]*/
                        attSet.Content[0].SourceLine,
                        Res.Xslt_CircularAttributeSet,
        ...
        private void LoadDecimalFormat(NsDecl stylesheetNsList)
        {
            ...
            {
                compiler.EnterForwardsCompatible();
                name = ResolveQName( /*ignoreDefaultNs:*/
                    true,
                    input.Value
                );
                if (!compiler.ExitForwardsCompatible(input.ForwardCompatibility))
            ...
            {
                characters[idx] = ParseCharAttribute(
                    3 + idx,
                    decimalFormatAttributes[3 + idx].name,
                    DefaultValues[idx]
                );
            }
                ...
                for (int j = i + 1; j < NumSignAttrs; j++)
                {
                    if (characters[i] == characters[j])
                    {
                        ...
                        bool dummy =
                            ...
                            || input.MoveToXsltAttribute(
                                3 + i,
                                decimalFormatAttributes[3 + i].name
                            );
                        Debug.Assert(
                            dummy,
                        ...
                        ReportError( /*[XT1300]*/
                            Res.Xslt_DecimalFormatSignsNotDistinct,
                            decimalFormatAttributes[3 + i].name,
                            decimalFormatAttributes[3 + j].name
                        );
                        break;
                    }
                }
            ...
            if (compiler.DecimalFormats.Contains(name))
            {
                ...
                CheckError(
                    infinity != format.InfinitySymbol, /*[XT1290]*/
                    Res.Xslt_DecimalFormatRedefined,
                    "infinity",
                    infinity
                );
                input.MoveToXsltAttribute(2, "NaN");
                CheckError(
                    nan != format.NanSymbol, /*[XT1290]*/
                    Res.Xslt_DecimalFormatRedefined,
                    "NaN",
                    nan
                );
                for (int idx = 0; idx < NumCharAttrs; idx++)
                {
                    ...
                    CheckError(
                        characters[idx] != format.Characters[idx], /*[XT1290]*/
                        Res.Xslt_DecimalFormatRedefined,
                        decimalFormatAttributes[3 + idx].name,
            ...
            else
            {
                ...
                DecimalFormatDecl format = new DecimalFormatDecl(
                    name,
                    infinity,
                    nan,
        ...
        private void LoadNamespaceAlias(NsDecl stylesheetNsList)
        {
            ...
            if (input.MoveToXsltAttribute(0, "stylesheet-prefix"))
            {
                ...
                {
                    ReportError( /*[XT_005]*/
                        Res.Xslt_EmptyNsAlias,
                        "stylesheet-prefix"
                    );
                }
                else
                {
                    stylesheetNsUri = input.LookupXmlNamespace(
            ...
            if (
                compiler.SetNsAlias(
                    ...
                    resultNsUri,
                    resultPrefix,
                    curStylesheet.ImportPrecedence
                )
            )
            {
        ...
        private void LoadGlobalVariableOrParameter(NsDecl stylesheetNsList, XslNodeType nodeType)
        {
            ...
            CheckError(
                !curStylesheet.AddVarPar(var), /*[XT0630]*/
                Res.Xslt_DupGlobalVariable,
        ...
        private void LoadTemplate(NsDecl stylesheetNsList)
        {
            ...
            {
                CheckError(
                    !input.AttributeExists(1, "name"), /*[XT_007]*/
                    Res.Xslt_BothMatchNameAbsent
                );
                CheckError(
                    input.AttributeExists(3, "mode"), /*[XT_008]*/
                    Res.Xslt_ModeWithoutMatch
                );
                mode = nullMode;
                if (input.AttributeExists(2, "priority"))
                {
                    if (V1)
                    {
                        ReportWarning( /*[XT_008]*/
                            Res.Xslt_PriorityWithoutMatch
                        );
                    }
                    else
                    {
            ...
            SetInfo(
                curTemplate,
                LoadEndTag(LoadInstructions(InstructionFlags.AllowParam)),
                ctxInfo
            );
            ...
            {
                ReportError( /*[XT0660]*/
                    Res.Xslt_DupTemplateName,
                    curTemplate.Name.QualifiedName
                );
            }
            curTemplate = null;
        }
        ...
        private void LoadImportSchema()
        {
            ...
            ReportError( /*[XTSE1650]*/
                Res.Xslt_SchemaDeclaration,
                input.ElementName
            );
            input.SkipNode();
        }
            ...
            if (input.MoveToFirstChild())
            {
                do
                {
                    switch (input.NodeType)
                    {
                        ...
                        case XmlNodeType.Element:
                            if (
                                input.IsNs(atoms.UrnMsxsl)
                                && (input.IsKeyword(atoms.Assembly) || input.IsKeyword(atoms.Using))
                            )
                            {
                                if (scriptCode.Length != 0)
                                {
                                    ReportError( /*[XT_012]*/
                                        Res.Xslt_ScriptNotAtTop,
                                        input.QualifiedName
                                    );
                                    input.SkipNode();
                                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    asmLocation = Assembly
                        .LoadFrom(ResolveUri(href, input.BaseUri).ToString())
                        .Location;
        ...
        XsltAttribute[] usingAttributes = { new XsltAttribute("namespace", V1Req | V2Req) };
                    ...
                    switch (input.NodeType)
                    {
                        case XmlNodeType.Element:
                            ...
                            if (nspace == atoms.UriXsl)
                            {
                                ...
                                result = (
                                    Ref.Equal(name, atoms.ApplyImports)
                                        ? XslApplyImports()
                                        : Ref.Equal(name, atoms.ApplyTemplates)
                                            ? XslApplyTemplates()
                                            : Ref.Equal(name, atoms.CallTemplate)
                                                ? XslCallTemplate()
                                                : Ref.Equal(name, atoms.Copy)
                                                    ? XslCopy()
                                                    : Ref.Equal(name, atoms.CopyOf)
                                                        ? XslCopyOf()
                                                        : Ref.Equal(name, atoms.Fallback)
                                                            ? XslFallback()
                                                            : Ref.Equal(name, atoms.If)
                                                                ? XslIf()
                                                                : Ref.Equal(name, atoms.Choose)
                                                                    ? XslChoose()
                                                                    : Ref.Equal(name, atoms.ForEach)
                                                                        ? XslForEach()
                                                                        : Ref.Equal(
                                                                            name,
                                                                            atoms.Message
                            ...
                            {
                                atTop = false;
                                result = LoadLiteralResultElement( /*asStylesheet:*/
                                    false
                                );
                            }
                            break;
                    ...
                    switch (input.NodeType)
                    {
                        case XmlNodeType.Element:
                            ...
                            )
                            {
                                ...
                                && input.IsXsltKeyword(atoms.Fallback)
                            )
                            {
                                XslFallback();
                            }
            ...
            if (V1)
            {
                if (content.Count != 0)
                {
                    ...
                    else
                    {
                        return SetInfo(
                            f.Error(
                                XslLoadException.CreateMessage(
                                    contentInfo, /*[XT0260]*/
                                    Res.Xslt_NotEmptyContents,
                                    atoms.ApplyImports
                                )
        ...
        private XslNode XslApplyTemplates()
        {
            ...
            ctxInfo.SaveExtendedLineInfo(input);
            return SetInfo(
                f.ApplyTemplates(mode, select, ctxInfo, input.XslVersion),
                content,
                ctxInfo
            );
        }
        ...
        XsltAttribute[] callTemplateAttributes = { new XsltAttribute("name", V1Req | V2Req) };
        ...
        XsltAttribute[] ifAttributes = { new XsltAttribute("test", V1Req | V2Req) };
        ...
        XsltAttribute[] forEachAttributes = { new XsltAttribute("select", V1Req | V2Req) };
        ...
        private XslNode XslForEach()
        {
            ...
            return SetInfo(f.ForEach(select, ctxInfo, input.XslVersion), content, ctxInfo);
        }
        ...
        private XslNode XslValueOf()
        {
            ...
            return SetInfo(
                f.XslNode(
                    doe ? XslNodeType.ValueOfDoe : XslNodeType.ValueOf,
                    null,
                    select,
                    input.XslVersion
                ),
        ...
        private VarPar XslVarPar()
        {
            ...
            if (nodeType == XslNodeType.Param && curFunction != null)
            {
                ...
                {
                    CheckError(
                        required != TriState.Unknown, /*[???]*/
                        Res.Xslt_RequiredOnFunction,
                        name.ToString()
                    );
                }
            ...
            CheckError(
                (required == TriState.True) && (select != null || content.Count != 0), /*[???]*/
                Res.Xslt_RequiredAndSelect,
                name.ToString()
            );
        ...
        XsltAttribute[] commentAttributes = { new XsltAttribute("select", V2Opt) };
        ...
        private List<XslNode> LoadContent(bool hasSelect)
        {
            ...
            List<XslNode> content = LoadInstructions();
            CheckError(
                hasSelect && content.Count != 0, /*[XT0620]*/
                Res.Xslt_ElementCntSel,
                parentName
            );
            // Load the end tag only if the content is not empty
            if (content.Count != 0)
        ...
        private XslNode XslText()
        {
            ...
            SerializationHints hints = doe
                ? SerializationHints.DisableOutputEscaping
                : SerializationHints.None;
        ...
        private XslNode XslElement()
        {
            ...
            CheckError(
                ns == XmlReservedNs.NsXmlNs, /*[XT_024]*/
                Res.Xslt_ReservedNS,
                ns
            );
            ...
            return SetInfo(
                f.Element(name, ns, input.XslVersion),
        ...
        private XslNode XslAttribute()
        {
            ...
            return SetInfo(
                f.Attribute(name, ns, input.XslVersion),
        ...
        private XslNode XslSort(int sortNumber)
        {
            ...
            return SetInfo(
                f.Sort(select, lang, dataType, order, caseOrder, input.XslVersion),
                null,
                ctxInfo
            );
        }
        ...
        private XslNode XslAnalyzeString()
        {
                    ...
                    switch (input.NodeType)
                    {
                        case XmlNodeType.Element:
                            if (input.IsXsltKeyword(atoms.MatchingSubstring))
                            {
                                ...
                                CheckError(
                                    nonMatching != null, /*[???]*/
                                    Res.Xslt_AnalyzeStringChildOrder
                                );
                                CheckError(
                                    matching != null, /*[???]*/
                                    Res.Xslt_AnalyzeStringDupChild,
            ...
            CheckError(
                matching == nonMatching, /*[XTSE1130]*/
                Res.Xslt_AnalyzeStringEmpty
            );
        ...
        private XslNode XslResultDocument()
        {
            ...
            bool escapeUriAttributes =
                ParseYesNoAttribute(11, "escape-uri-attributes") != TriState.False;
            bool includeContentType =
                ParseYesNoAttribute(12, "include-content-type") != TriState.False;
            ...
            settings.OmitXmlDeclaration =
                ParseYesNoAttribute(16, "omit-xml-declaration") == TriState.True;
            settings.Standalone =
                ParseYesNoAttribute(17, "standalone") == TriState.True
                    ? XmlStandalone.Yes
                    : XmlStandalone.No;
            ...
            if (format != null)
                ReportNYI("xsl:result-document/@format");
            ...
            if (normalizationForm != null)
                ReportNYI("xsl:result-document/@normalization-form");
            if (undeclarePrefixes)
            ...
            if (useCharacterMaps != null)
                ReportNYI("xsl:result-document/@use-character-maps");
        ...
        private XslNode LoadLiteralResultElement(bool asStylesheet)
        {
            ...
            for (int i = 1; input.MoveToLiteralAttribute(i); i++)
            {
                if (!input.IsXsltNamespace())
                {
                    XslNode att = f.LiteralAttribute(
                        f.QName(input.LocalName, input.NamespaceUri, input.Prefix),
                        input.Value,
                        input.XslVersion
                    );
            ...
            if (fallbacksArray.Count == 0)
            {
                fallbacksArray.Add(
                    f.Error(
                        XslLoadException.CreateMessage(
                            extElmLineInfo,
                            Res.Xslt_UnknownExtensionElement,
                            instrName
                        )
                    )
                );
        ...
        private bool ResolveQName(
            bool ignoreDefaultNs,
            string qname,
        ...
        private QilName ParseQNameAttribute(int attNum)
        {
            ...
            if (result == null && required)
            {
                result = f.QName(
                    ...
                    compiler.CreatePhantomNamespace(),
                    compiler.PhantomNCName
                );
            }
            return result;
        ...
        {
            Debug.Assert(
                input.IsRequiredAttribute(attNum),
        ...
        private void ParseWhitespaceRules(string elements, bool preserveSpace)
        {
            if (elements != null && elements.Length != 0)
            {
                ...
                for (int i = 0; i < tokens.Length; i++)
                {
                    ...
                    curStylesheet.AddWhitespaceRule(
                        index,
        ...
        private XmlQualifiedName ParseOutputMethod(string attValue, out XmlOutputMethod method)
        {
            ...
            else
            {
                if (!input.ForwardCompatibility)
                {
                    ReportWarning( /*[XT1570]*/
                        Res.Xslt_InvalidMethod,
                        attValue
                    );
                }
            }
        ...
        {
            Debug.Assert(
                input.LocalName == "use-attribute-sets",
                "we are positioned on this attribute"
            );
            Debug.Assert(
                list != null && list.Count == 0,
                "It happened that we always add use-attribute-sets first. Otherwise we can't call list.Clear()"
            );
            ...
            foreach (string qname in XmlConvert.SplitString(input.Value))
            {
                AddInstruction(
                    list,
                    SetLineInfo(f.UseAttributeSet(CreateXPathQName(qname)), input.BuildLineInfo())
                );
            }
            if (!compiler.ExitForwardsCompatible(input.ForwardCompatibility))
            {
            ...
            if (input.MoveToXsltAttribute(attNum, "input-type-annotations"))
            {
                ...
                switch (value)
                {
                    ...
                    case "preserve":
                        ...
                        {
                            CheckError(
                                compiler.inputTypeAnnotations != value, /*[XTSE0265]*/
                                Res.Xslt_InputTypeAnnotations
                            );
                        }
                        break;
                    default:
        ...
        private static NsDecl MergeNamespaces(NsDecl thisList, NsDecl parentList)
        {
            ...
            while (parentList != null)
            {
                ...
                {
                    if (
                        Ref.Equal(tmp.Prefix, parentList.Prefix)
                        && (
                            tmp.Prefix != null
