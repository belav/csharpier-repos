...
namespace System.Xml.Xsl.Xslt
{
    ...
    partial internal class QilGenerator : IErrorHelper
    {
        ...
        private QilNode InvokeOnCurrentNodeChanged()
        {
            ...
            QilIterator i;
            return f.Loop(
                i = f.Let(f.InvokeOnCurrentNodeChanged(curLoop.GetCurrent())),
                f.Sequence()
            );
        }
        ...
        {
            Debug.Assert(
                !curLoop.IsFocusSet && !funcFocus.IsFocusSet,
        ...
        private void CompileInitializationCode()
        {
            ...
            {
                QilFunction initFunction = f.Function(
        ...
        private QilIterator GetNsVar(QilList nsList)
        {
            ...
            newVar.DebugName = f.QName("ns" + this.nsVars.Count, XmlReservedNs.NsXslDebug)
                .ToString();
            ...
            for (i = pos; i < source.Length; i++)
            {
                ...
                if (ch == '{' || ch == '}')
                {
                    ...
                    { // single '}'
                        ...
                        {
                            ReportError( /*[XT0370]*/
                                Res.Xslt_SingleRightBraceInAvt,
                                source
                            );
                            return null;
                        }
        ...
        private QilNode CompileCallTemplate(XslNodeEx node)
        {
            ...
            QilNode result;
            {
                ...
                else
                {
                    ...
                    {
                        compiler.ReportError( /*[XT0710]*/
                            node.SourceLine,
                            Res.Xslt_InvalidCallTemplate,
        ...
        private QilNode CompileLangAttribute(string attValue, bool fwdCompat)
        {
            ...
            else
            {
                ...
                result = f.Loop(
                    ...
                    f.Conditional(
                        f.Eq(
                            f.InvokeLangToLcid(i, fwdCompat),
                            f.Int32(XsltLibrary.InvariantCultureLcid)
                        ),
        ...
        private QilNode CompileLangToLcid(QilNode lang, bool fwdCompat)
        {
            ...
            {
                return f.Double(
                    XsltLibrary.LangToLcidInternal(
            ...
            if (result != null)
            {
                    ...
                    else
                    {
                        if (!fwdCompat)
                        {
                            ...
                            string prefix,
                                local,
                                nsUri;
                            bool isValid = compiler.ParseQName(
                                dataType,
                                out prefix,
                                out local,
                                (IErrorHelper)this
                ...
                else
                {
                    ...
                    result = f.Loop(
                        dt = f.Let(result),
                        f.Conditional(
                            f.Eq(dt, f.String(DtNumber)),
                            f.False(),
                            f.Conditional(
                                f.Eq(dt, f.String(DtText)),
                                f.True(),
                                fwdCompat
                                    ? f.True()
                                    : f.Loop(
            ...
            if (result != null)
            {
                ...
                else
                {
                    ...
                    result = f.Loop(
                        i = f.Let(result),
                        f.Conditional(
                            f.Eq(i, f.String(value1)),
                            f.String("1"),
                            fwdCompat
                                ? f.String("0")
                                : f.Conditional(
                                    f.Eq(i, f.String(value0)),
                                    f.String("0"),
                                    f.Error(
                                        lastScope.SourceLine,
                                        Res.Xslt_BistateAttribute,
        ...
        private QilNode MatchPattern(QilNode pattern, QilIterator testNode)
        {
            ...
            for (int i = list.Count - 1; 0 <= i; i--)
            {
                ...
                result = f.Or(refReplacer.Replace(filter.Body, filter.Variable, testNode), result);
            }
            return result;
        }
            ...
            else
            {
                ...
                switch (nodeKinds)
                {
                    case XmlNodeKindFlags.Document:
                        return f.IsType(testNode, T.Document);
                    case XmlNodeKindFlags.Element:
                        result = f.IsType(testNode, T.Element);
                        break;
                    case XmlNodeKindFlags.Attribute:
                        result = f.IsType(testNode, T.Attribute);
                        break;
                    case XmlNodeKindFlags.Text:
                        return f.IsType(testNode, T.Text);
                    case XmlNodeKindFlags.Comment:
                        return f.IsType(testNode, T.Comment);
                    case XmlNodeKindFlags.PI:
                        return f.And(
                            f.IsType(testNode, T.PI),
                            f.Eq(f.LocalNameOf(testNode), f.LocalNameOf(current))
                        );
                    case XmlNodeKindFlags.Namespace:
                        return f.And(
                            f.IsType(testNode, T.Namespace),
                            f.Eq(f.LocalNameOf(testNode), f.LocalNameOf(current))
                        );
                    default:
        ...
        private QilNode PlaceMarker(QilNode countPattern, QilNode fromPattern, bool multiple)
        {
            ...
            */
            ...
            countMatches = f.Filter(
                i = f.For(f.AncestorOrSelf(GetCurrentNode())),
        ...
        private QilNode PlaceMarkerAny(QilNode countPattern, QilNode fromPattern)
        {
            ...
            */
            ...
            {
                fromMatches = f.Filter(
                    i = f.For(f.Preceding(GetCurrentNode())),
                    MatchPattern(fromPattern, i)
                );
                F = f.Filter(i = f.For(fromMatches), f.Eq(f.PositionOf(i), f.Int32(1)));
                AF = f.Loop(
                    ...
                    f.Filter(
                        j = f.For(
                            f.Filter(
                                k = f.For(f.NodeRange(i, GetCurrentNode())),
            ...
            return f.Loop(
                k = f.Let(f.Length(AF)),
                f.Conditional(f.Eq(k, f.Int32(0)), f.Sequence(), k)
            );
        }
        ...
        private QilNode CompileGroupingSizeAttribute(string attValue, bool fwdCompat)
        {
            ...
            else
            {
                ...
                return f.Loop(
                    i,
                    f.Conditional(
                        f.And(f.Lt(f.Double(0), i), f.Lt(i, f.Double(int.MaxValue))),
                        i,
                        f.Double(0)
                    )
                );
        ...
        private void CompileKeys()
        {
            ...
            for (int idx = 0; idx < compiler.Keys.Count; idx++)
            {
                foreach (Key key in compiler.Keys[idx])
                {
                    ...
                    QilFunction func = f.Function(
                        f.FormalParameterList(context, value),
                        f.Filter(values, f.Not(f.IsEmpty(f.Filter(keys, f.Eq(keys, value))))),
                        f.False()
                    );
        ...
        private QilNode CompileMatchPattern(string pttrn)
        {
            ...
            catch (XslLoadException e)
            {
                ...
                result = f.Loop(f.For(ptrnBuilder.FixupNode), f.Error(f.String(e.Message)));
        ...
        private IList<XslNode> AddRemoveImplicitArgs(IList<XslNode> args, XslFlags flags)
        {
            ...
            if ((flags & XslFlags.FocusFilter) != 0)
            {
                ...
                if ((flags & XslFlags.Current) != 0)
                {
                    SetArg(args, pos++, nameCurrent, GetCurrentNode());
                }
                if ((flags & XslFlags.Position) != 0)
                {
                    SetArg(args, pos++, namePosition, GetCurrentPosition());
                }
                if ((flags & XslFlags.Last) != 0)
                {
                    SetArg(args, pos++, nameLast, GetLastPosition());
                }
            }
            return args;
        }
        ...
        )
        {
            ...
            if (applyFunction == null)
            {
                ...
                for (int i = 0; i < actualArgs.Count; i++)
                {
                    ...
                    QilParameter formalArg = f.Parameter(
                ...
                QilTernary builtinTemplates = f.BaseFactory.Conditional(
                    f.IsType(current, elementOrDocumentType),
                    loopOnContent,
                    f.Conditional(
                        f.IsType(current, textOrAttributeType),
                ...
                applyFunction.Definition = matcherBuilder.BuildMatcher(
                    current,
