...
namespace System.Xml.Xsl.Xslt
{
    ...
    internal class XsltInput : IErrorHelper
    {
        ...
        public XmlNodeType NodeType
        {
            get
            {
                return nodeType == XmlNodeType.Element && 0 < currentRecord
        ...
        {
            get { return records[currentRecord].localName; }
        }
        public string NamespaceUri
        {
            get { return records[currentRecord].nsUri; }
        }
        public string Prefix
        {
            get { return records[currentRecord].prefix; }
        }
        public string Value
        {
            get { return records[currentRecord].value; }
        }
        public string BaseUri
        {
            get { return records[currentRecord].baseUri; }
        }
        public string QualifiedName
        {
            get { return records[currentRecord].QualifiedName; }
        }
        ...
        {
            get { return records[currentRecord].start; }
        }
        public Location End
        {
            get { return records[currentRecord].end; }
        }
        ...
        private void FillupRecord(ref Record rec)
        {
            ...
            if (readerLineInfo != null)
            {
                rec.start = new Location(
                    readerLineInfo.LineNumber,
                    readerLineInfo.LinePosition - PositionAdjustment(reader.NodeType)
                );
            }
        }
        ...
        private bool ReadAttribute(ref Record rec)
        {
            ...
            else if (rec.prefix.Length == 0 && Ref.Equal(rec.localName, atoms.Xmlns))
            { // xmlns="NS_FOO"
                string atomizedValue = atoms.NameTable.Add(reader.Value);
            ...
            /* Read Attribute Value */{
                ...
                if (readerLineInfo != null)
                {
                    ...
                    rec.valueStart = new Location(
                        readerLineInfo.LineNumber,
                        readerLineInfo.LinePosition + correction
                    );
                ...
                if (readerLineInfo != null)
                {
                    Debug.Assert(reader.NodeType != XmlNodeType.EntityReference);
                    int correction =
                        ((reader.NodeType == XmlNodeType.EndEntity) ? 1 : lastText.Length) + 1;
                    rec.end = new Location(
                        readerLineInfo.LineNumber,
                        readerLineInfo.LinePosition + correction
                    );
        ...
        {
            Debug.Assert(
                nodeType != XmlNodeType.Element || IsEmptyElement,
                ...
                switch (reader.NodeType)
                {
                    ...
                    case XmlNodeType.EntityReference:
                        ...
                        )
                        {
                            ...
                            if (
                                textIsWhite
                                && !XmlCharType.Instance.IsOnlyWhitespace(
                                    records[curTextNode].value
                                )
                            )
                            {
                                textIsWhite = false;
        ...
        {
            Debug.Assert(
                attributes != null && attributes[attNum].name == attName,
        ...
        {
            return (
                    attributes[attNum].flags
                    & (compiler.Version == 2 ? XsltLoader.V2Req : XsltLoader.V1Req)
                ) != 0;
        }
        ...
        {
            return IsKeyword(kwd) && IsNullNamespace();
        }
        ...
        {
            Debug.Assert(
                nodeType != XmlNodeType.Element || attributesRead,
        ...
        public ContextInfo GetAttributes(XsltAttribute[] attributes)
        {
            ...
            bool SS =
                IsXsltNamespace() && (IsKeyword(atoms.Stylesheet) || IsKeyword(atoms.Transform));
            ...
            for (int attNum = 1; MoveToAttributeBase(attNum); attNum++)
            {
                if (IsNullNamespace())
                {
                    ...
                    {
                        if (Ref.Equal(localName, atoms.ExcludeResultPrefixes) && (SS || V2))
                        {
                            attExclude = attNum;
                        }
                        else if (Ref.Equal(localName, atoms.ExtensionElementPrefixes) && (SS || V2))
                        {
                            attExtension = attNum;
                        }
                        else if (Ref.Equal(localName, atoms.XPathDefaultNamespace) && (V2))
                        {
                            attNamespace = attNum;
                        }
                        else if (Ref.Equal(localName, atoms.DefaultCollation) && (V2))
                        {
                            attCollation = attNum;
                        }
                        else if (Ref.Equal(localName, atoms.UseWhen) && (V2))
                        {
                            attUseWhen = attNum;
                        }
                        else
                        {
                            ReportError( /*[XT0090]*/
                                Res.Xslt_InvalidAttribute,
                                QualifiedName,
                                records[0].QualifiedName
                            );
                        }
                    }
                }
        ...
        private void InsertExNamespaces(int attExPrefixes, ContextInfo ctxInfo, bool extensions)
        {
            ...
            if (MoveToLiteralAttribute(attExPrefixes))
            {
                Debug.Assert(
                    extensions
                        ? IsKeyword(atoms.ExtensionElementPrefixes)
                        : IsKeyword(atoms.ExcludeResultPrefixes)
                );
                string value = Value;
                if (value.Length != 0)
                {
        ...
        public string LookupXmlNamespace(string prefix)
        {
            ...
            {
                Debug.Assert(
                    Ref.Equal(atoms.NameTable.Get(nsUri), nsUri),
        ...
        internal class ContextInfo
        {
            ...
            public void SaveExtendedLineInfo(XsltInput input)
            {
                ...
                elemNameLi = new SourceLineInfo(
                    ...
                    lineInfo.Start.Line,
                    lineInfo.Start.Pos + 1 + elemNameLength
                );
            ...
            internal class EmptyElementEndTag : ISourceLineInfo
            {
                ...
                public Location Start
                {
                    get { return new Location(elementTagLi.End.Line, elementTagLi.End.Pos - 2); }
                }
                public Location End
                {
