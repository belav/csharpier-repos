...
namespace System.Xml.Xsl.Runtime
{
    ...
    internal class XmlExtensionFunction
    {
        ...
        public bool CanBind()
        {
            ...
            {
                if (
                    methSearch.Name.Equals(this.name, comparison)
                    && (this.numArgs == -1 || methSearch.GetParameters().Length == this.numArgs)
                )
                {
        ...
        public void Bind()
        {
                ...
                )
                {
                    if (methMatch != null)
                        throw new XslTransformException( /*[XT_037]*/
                            Res.XmlIl_AmbiguousExtensionMethod,
                            this.namespaceUri,
                            this.name,
            ...
            if (methMatch == null)
            {
                ...
                foreach (MethodInfo methSearch in methods)
                {
                    if (
                        methSearch.Name.Equals(this.name, comparison)
                        && methSearch.GetParameters().Length == this.numArgs
                    )
                        throw new XslTransformException( /*[XT_038]*/
                            Res.XmlIl_NonPublicExtensionMethod,
                            this.namespaceUri,
                            this.name
                        );
                }
                throw new XslTransformException( /*[XT_039]*/
                    Res.XmlIl_NoExtensionMethod,
                    this.namespaceUri,
                    this.name,
                    this.numArgs.ToString(CultureInfo.InvariantCulture)
                );
            ...
            if (methMatch.IsGenericMethodDefinition)
                throw new XslTransformException( /*[XT_040]*/
                    Res.XmlIl_GenericExtensionMethod,
                    this.namespaceUri,
                    this.name
                );
        ...
        public object Invoke(object extObj, object[] args)
        {
            ...
            Debug.Assert(
                args.Length == this.argClrTypes.Length,
