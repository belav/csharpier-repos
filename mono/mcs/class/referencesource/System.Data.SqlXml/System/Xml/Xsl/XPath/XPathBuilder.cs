        ...
        */
                ...
                f.IsEmpty(
                    f.Loop(
                        ...
                        f.Filter(
                            ...
                            CompareValues(
                                op,
                                f.XPathNodeValue(leftEnd),
                                f.XPathNodeValue(rightEnd),
                                compType
                            )
                        )
                    )
        ...
        )
        {
            return (XmlNodeKindFlags)(
                (int)inputTypeMask
                & (int)XPathNodeType2QilXmlNodeKind[(int)nodeType]
                & (int)XPathAxisMask[(int)xpathAxis]
            );
        }

        QilNode BuildAxisFilter(
            QilNode qilAxis,
            XPathAxis xpathAxis,
            XPathNodeType nodeType,
            string name,
        ...
        public virtual QilNode Predicate(QilNode nodeset, QilNode predicate, bool isReverseStep)
        {
            if (isReverseStep)
            {
                ...
                // this way this wat we hold invariant that path expresion are always DOD and make predicates on
                // reverse axe
                ...
                if (FunctionTable.TryGetValue(name, out func))
                {
                    ...
                    switch (func.id)
                    {
                        ...
                        case FuncId.Position:
                            return GetCurrentPosition();
                        case FuncId.Count:
                            return f.XsltConvert(f.Length(f.DocOrderDistinct(args[0])), T.DoubleX);
                        case FuncId.LocalName:
                            return args.Count == 0
                                ? f.LocalNameOf(GetCurrentNode())
                                : LocalNameOfFirstNode(args[0]);
                        case FuncId.NamespaceUri:
                            return args.Count == 0
                                ? f.NamespaceUriOf(GetCurrentNode())
                        ...
                        case FuncId.String:
                            return args.Count == 0
                                ? f.XPathNodeValue(GetCurrentNode())
                                : f.ConvertToString(args[0]);
                        case FuncId.Number:
                            return args.Count == 0
                                ? f.XsltConvert(f.XPathNodeValue(GetCurrentNode()), T.DoubleX)
                        ...
                        case FuncId.Substring:
                            return args.Count == 2
                                ? f.InvokeSubstring(args[0], args[1])
                                : f.InvokeSubstring(args[0], args[1], args[2]);
                        case FuncId.StringLength:
                            return f.XsltConvert(
                                f.StrLength(
                                    args.Count == 0 ? f.XPathNodeValue(GetCurrentNode()) : args[0]
                                ),
                                T.DoubleX
                            );
                        case FuncId.Normalize:
                            return f.InvokeNormalizeSpace(
                                args.Count == 0 ? f.XPathNodeValue(GetCurrentNode()) : args[0]
                            );
        ...
        QilNode NameOf(QilNode arg)
        {
            ...
            if (arg is QilIterator)
            {
                QilIterator p,
                    ln;
                return f.Loop(
                    p = f.Let(f.PrefixOf(arg)),
                    f.Loop(
                        ln = f.Let(f.LocalNameOf(arg)),
                        f.Conditional(
                            f.Eq(f.StrLength(p), f.Int32(0)),
                            ln,
                            f.StrConcat(p, f.String(":"), ln)
                        )
                    )
                );
            }
        ...
        internal class FixupVisitor : QilReplaceVisitor
        {
            ...
            QilNode fixupCurrent,
                fixupPosition,
                fixupLast; // fixup nodes we are replacing
            QilIterator current;
                ...
                else if (unknown == fixupPosition)
                {
                    ...
                    if (!justCount)
                    {
                        ...
                        else if (this.current != null)
                        {
                            ...
                            unknown = f.XsltConvert(
                                f.PositionOf((QilIterator)this.current),
                                T.DoubleX
                            );
                        }
                    }
