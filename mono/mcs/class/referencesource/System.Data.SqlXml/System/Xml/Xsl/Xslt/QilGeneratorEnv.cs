...
namespace System.Xml.Xsl.Xslt
{
    ...
    internal partial class QilGenerator : IXPathEnvironment
    {
            ...
            else
            {
                ...
                if (ns == XmlReservedNs.NsMsxsl)
                {
                    ...
                    else if (name == "utc")
                    {
                        ...
                        return f.InvokeMsUtc( /*datetime:*/
                            f.ConvertToString(args[0])
                        );
                    }
                ...
                switch (xmlTypeFormalArg.TypeCode)
                {
                    ...
                    case XmlTypeCode.Node:
                        args[i] = xmlTypeFormalArg.IsSingleton
                            ? f.ConvertToNode(args[i])
                            : f.ConvertToNodeSet(args[i]);
                        break;
        ...
        private static readonly XmlTypeCode[] argFnDocument =
        {
            XmlTypeCode.Item,
            XmlTypeCode.Node,
        };
        private static readonly XmlTypeCode[] argFnKey = { XmlTypeCode.String, XmlTypeCode.Item };
        private static readonly XmlTypeCode[] argFnFormatNumber =
        {
            XmlTypeCode.Double,
            XmlTypeCode.String,
            XmlTypeCode.String,
        ...
        private static Dictionary<string, FunctionInfo> CreateFunctionTable()
        {
            ...
            table.Add(
                "format-number",
                new FunctionInfo(FuncId.FormatNumber, 2, 3, argFnFormatNumber)
            );
            table.Add(
                "unparsed-entity-uri",
        ...
        private QilNode CompileFnKey(QilNode name, QilNode keys, IFocus env)
        {
            ...
            if (keys.XmlType.IsNode)
            {
                ...
                else
                {
                    result = f.Loop(
                        i = f.For(keys),
                        CompileSingleKey(name, f.ConvertToString(i), env)
                    );
                }
            }
            ...
            else
            {
                result = f.Loop(
                    ...
                    f.Loop(
                        k = f.Let(keys),
                        f.Conditional(
                            f.Not(f.IsType(k, T.AnyAtomicType)),
                            f.Loop(
                                i = f.For(f.TypeAssert(k, T.NodeS)),
                                CompileSingleKey(n, f.ConvertToString(i), env)
                            ),
        ...
        private QilNode CompileSingleKey(QilNode name, QilNode key, IFocus env)
        {
            ...
            if (name.NodeType == QilNodeType.LiteralString)
            {
                ...
                nsUri = ResolvePrefixThrow( /*ignoreDefaultNs:*/
                    true,
                    prefix
                );
            ...
            else
            {
                ...
                result = f.Invoke(
                    generalKey,
                    f.ActualParameterList(i, resolvedName, key, env.GetCurrent())
                );
        ...
        private QilFunction CreateGeneralKeyFunction()
        {
            ...
            {
                fdef = f.Conditional(
                    f.Eq(resolvedName, compiler.Keys[idx][0].Name.DeepClone(f.BaseFactory)),
                    CompileSingleKey(compiler.Keys[idx], key, context),
                    fdef
                );
            }

            QilFunction result = f.Function(
                f.FormalParameterList(name, resolvedName, key, context),
                fdef,
                f.False()
            );
        ...
        private QilNode CompileFnDocument(QilNode uris, QilNode baseNode)
        {
            ...
            else
            {
                ...
                result = f.Conditional(
                    f.Not(f.IsType(u, T.AnyAtomicType)),
                    f.DocOrderDistinct(
                        f.Loop(
        ...
        )
        {
            ...
            if (resolvedName != null)
            {
                ...
                {
                    QilIterator fmtIdx = f.Let(
                        f.InvokeRegisterDecimalFormatter(formatPicture, format)
                    );
                    fmtIdx.DebugName = f.QName(
                            "formatter" + this.formatterCnt++,
        ...
        private QilNode CompileUnparsedEntityUri(QilNode n)
        {
            f.CheckString(n);
            return f.Error(
                lastScope.SourceLine,
                Res.Xslt_UnsupportedXsltFunction,
                "unparsed-entity-uri"
            );
        }
