        ...
        public QilNode GetCurrent()
        {
            ...
            switch (focusType)
            {
                case SingletonFocusType.InitialDocumentNode:
                    return f.Root(f.XmlContext());
                case SingletonFocusType.InitialContextNode:
                    return f.XmlContext();
                default:
                    Debug.Assert(
                        focusType == SingletonFocusType.Iterator && current != null,
    ...
    internal struct FunctionFocus : IFocus
    {
        ...
        public void StartFocus(IList<QilNode> args, XslFlags flags)
        {
            ...
            if ((flags & XslFlags.Current) != 0)
            {
                ...
                Debug.Assert(
                    this.current.Name.NamespaceUri == XmlReservedNs.NsXslDebug
                        && this.current.Name.LocalName == "current"
                );
            }
            if ((flags & XslFlags.Position) != 0)
            {
                ...
                Debug.Assert(
                    this.position.Name.NamespaceUri == XmlReservedNs.NsXslDebug
                        && this.position.Name.LocalName == "position"
                );
            }
            if ((flags & XslFlags.Last) != 0)
            {
                ...
                Debug.Assert(
                    this.last.Name.NamespaceUri == XmlReservedNs.NsXslDebug
                        && this.last.Name.LocalName == "last"
                );
            }
            this.isSet = true;
        }
