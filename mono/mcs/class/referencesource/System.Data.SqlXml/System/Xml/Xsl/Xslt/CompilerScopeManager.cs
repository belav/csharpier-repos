...
namespace System.Xml.Xsl.Xslt
{
    ...
    internal sealed class CompilerScopeManager<V>
    {
        ...
        {
            Debug.Assert(
                flag == (flag & ScopeFlags.ExclusiveFlags) && (flag & (flag - 1)) == 0 && flag != 0,
                "One exclusive flag"
            );
        ...
        {
            Debug.Assert(
                flag == (flag & ScopeFlags.InheritedFlags) && (flag & (flag - 1)) == 0 && flag != 0,
                "one inherited flag"
            );
            ScopeFlags flags = records[lastRecord].flags;
            ...
            Debug.Assert(
                (records[lastRecord].flags & ScopeFlags.CompatibilityFlags)
        ...
        internal struct NamespaceEnumerator
        {
            ...
            public bool MoveNext()
            {
                while (LastPredefRecord < --currentRecord)
                {
                    if (scope.records[currentRecord].IsNamespace)
                    {
                        // This is a namespace declaration
                        if (
                            scope.LookupNamespace(
                                scope.records[currentRecord].ncName,
                                lastRecord,
                                currentRecord + 1
                            ) == null
                        )
            ...
            public ScopeRecord Current
            {
                ...
                {
                    Debug.Assert(
                        LastPredefRecord <= currentRecord && currentRecord <= scope.lastRecord,
