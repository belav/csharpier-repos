...
namespace System.Xml.Xsl.Xslt
{
    ...
    internal class XslAstAnalyzer : XslVisitor<XslFlags>
    {
        ...
        private Dictionary<Template, Stylesheet> fwdApplyImportsGraph =
            new Dictionary<Template, Stylesheet>();
        private Dictionary<QilName, List<ProtoTemplate>> revApplyTemplatesGraph =
        ...
        private Dictionary<ModeName, VarPar> applyTemplatesParams =
            ...
            where V : XslNode
        {
            ...
            public void AddEdge(V v1, V v2)
            {
                ...
                Debug.WriteLineIf(
                    DiagnosticsSwitches.XslTypeInference.TraceVerbose,
                    v1.TraceName + " -> " + v2.TraceName
                );
            }
        ...
        private void TraceResults()
        {
            ...
            if (DiagnosticsSwitches.XslTypeInference.TraceVerbose)
            {
                ...
                {
                    Debug.WriteLine(
                        varPar.TraceName + " = " + (varPar.Flags & XslFlags.TypeFilter)
                    );
                }
                Debug.WriteLine(string.Empty);
            }
        ...
        protected override XslFlags VisitCallTemplate(XslNode node)
        {
            ...
            {
                Debug.WriteLineIf(
                    DiagnosticsSwitches.XslTypeInference.TraceError,
                    "Unknown template " + node.Name.QualifiedName,
                    "Error"
                );
            }
            else
        ...
        */
        ...
        protected override XslFlags VisitUseAttributeSet(XslNode node)
        {
            ...
            if (!compiler.AttributeSets.TryGetValue(node.Name, out attSet))
            {
                Debug.WriteLineIf(
                    DiagnosticsSwitches.XslTypeInference.TraceError,
                    "Unknown attribute-set " + node.Name.QualifiedName,
                    "Error"
                );
            }
        ...
        internal class XPathAnalyzer : IXPathBuilder<XslFlags>
        {
            ...
            // "nodeset[predicate]"
            public virtual XslFlags Predicate(
                XslFlags nodeset,
                XslFlags predicate,
                bool isReverseStep
            )
            ...
            public virtual XslFlags Variable(string prefix, string name)
            {
                ...
                if (typeDonor == null)
                {
                    Debug.WriteLineIf(
                        DiagnosticsSwitches.XslTypeInference.TraceError,
                        "Unresolved variable " + Compiler.ConstructQName(prefix, name),
                        "Error"
                    );
