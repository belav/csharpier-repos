...
namespace System.Xml.Xsl.Qil
{
    ...
    internal sealed class QilXmlReader
    {
        ...
        static QilXmlReader()
        {
            ...
            foreach (
                MethodInfo mi in typeof(QilFactory).GetMethods(
            ...
            )
            {
                ...
                if (i == parms.Length)
                {
                    ...
                    if (
                        !nameToFactoryMethod.ContainsKey(mi.Name)
                        || nameToFactoryMethod[mi.Name].GetParameters().Length < parms.Length
                    )
        ...
        public QilExpression Read()
        {
            ...
            Debug.Assert(
                this.fwdDecls.Keys.Count == 0,
                "One or more forward declarations were never defined"
            );
            Debug.Assert(
                this.stk.Peek()[0].NodeType == QilNodeType.QilExpression,
                "Serialized qil tree did not contain a QilExpression node"
            );
        ...
        private void EndElement()
        {
            ...
            switch (r.LocalName)
            {
                ...
                case "Function":
                {
                    ...
                    Debug.Assert(
                        !this.inFwdDecls || ann.XmlType != null,
                        "Forward decl for "
                            + r.LocalName
                            + " '"
                            + id
                            + "' must have an xmlType attribute"
                    );
                    ...
                    switch (r.LocalName)
                    {
                        case "Parameter":
                            Debug.Assert(
                                list.Count <= (this.inFwdDecls ? 0 : 1),
                                "Parameter '" + id + "' must have 0 or 1 arguments"
                            );
                            Debug.Assert(
                                ann.XmlType != null,
                                "Parameter '" + id + "' must have an xmlType attribute"
                            );
                        ...
                        case "Let":
                            Debug.Assert(
                                list.Count == (this.inFwdDecls ? 0 : 1),
                                "Let '" + id + "' must have 0 or 1 arguments"
                            );
                            if (this.inFwdDecls)
                        ...
                        default:
                            Debug.Assert(
                                list.Count == (this.inFwdDecls ? 2 : 3),
                                "Function '" + id + "' must have 2 or 3 arguments"
                            );
                            if (this.inFwdDecls)
                                ...
                                nd = f.Function(
                                    ...
                                    list[1],
                                    list[2],
                                    ann.XmlType != null ? ann.XmlType : list[1].XmlType
                                );
                            break;
                    }
                    ...
                    {
                        Debug.Assert(
                            !this.scope.ContainsKey(id),
                            "Multiple nodes have id '" + id + "'"
                        );
                        this.fwdDecls[id] = nd;
                        this.scope[id] = nd;
                    }
                    else
                    {
                        if (this.fwdDecls.ContainsKey(id))
                        {
                            ...
                            Debug.Assert(
                                r.LocalName == Enum.GetName(typeof(QilNodeType), nd.NodeType),
                                "Id '"
                                    + id
                                    + "' is not not bound to a "
                                    + r.LocalName
                                    + " forward decl"
                            );
                            nd = this.fwdDecls[id];
                ...
                {
                    Debug.Assert(
                        ann.ClrNamespace != null,
                        "XsltInvokeEarlyBound must have a clrNamespace attribute"
                    );
                    Debug.Assert(
                        list.Count == 2,
                        "XsltInvokeEarlyBound must have exactly 2 arguments"
                    );
                    Debug.Assert(
                        list.XmlType != null,
                        "XsltInvokeEarlyBound must have an xmlType attribute"
                    );
                    MethodInfo mi = null;
                    ...
                    Debug.Assert(
                        mi != null,
                        "Cannot find method " + ann.ClrNamespace + "." + name.ToString()
                    );
                ...
                default:
                {
                    ...
                    Debug.Assert(
                        nameToFactoryMethod.ContainsKey(r.LocalName),
                        "Method " + r.LocalName + " could not be found on QilFactory"
                    );
                    facMethod = nameToFactoryMethod[r.LocalName];
                    Debug.Assert(
                        facMethod.GetParameters().Length == list.Count,
                        "NodeType " + r.LocalName + " does not allow " + list.Count + " parameters"
                    );
            ...
            if (s != null && s.Length > 0)
            {
                ...
                for (int i = 0; i < codes.Length; i++)
                    types[i] = XmlQueryTypeFactory.Type(
                        (XmlTypeCode)Enum.Parse(typeof(XmlTypeCode), codes[i]),
                        strict
                    );
