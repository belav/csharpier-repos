    ...
    internal class NumberAction : ContainerAction
    {
                    ...
                    case NumberingSequence.LCLetter:
                        if (dblVal <= MaxAlphabeticValue)
                        {
                            ...
                            ConvertToAlphabetic(
                                sb,
                                dblVal,
                                seq == NumberingSequence.UCLetter ? 'A' : 'a',
                                26
                            );
                            return sb.ToString();
                        }
        ...
        internal override bool CompileAttribute(Compiler compiler)
        {
            ...
            else if (Ref.Equal(name, compiler.Atoms.Count))
            {
                ...
                this.countKey = compiler.AddQuery(
                    value, /*allowVars:*/
                    true, /*allowKey:*/
                    true, /*pattern*/
                    true
            ...
            {
                this.from = value;
                this.fromKey = compiler.AddQuery(
                    value, /*allowVars:*/
                    true, /*allowKey:*/
                    true, /*pattern*/
                    true
        ...
        private int numberAny(Processor processor, ActionFrame frame)
        {
            ...
            if (this.fromKey != Compiler.InvalidQueryKey)
            {
                ...
                Debug.Assert(
                    processor.Matches(startNode, this.fromKey)
                        || // we hit 'from' or
...
/*CalculatingFormat:*/
                    frame.StoredOutput = Format(
                        ...
                        this.letterAvt == null
                            ? this.letter
                            : ParseLetter(this.letterAvt.Evaluate(processor, frame)),
                        this.groupingSepAvt == null
                            ? this.groupingSep
                            : this.groupingSepAvt.Evaluate(processor, frame),
                        this.groupingSizeAvt == null
                            ? this.groupingSize
                            : this.groupingSizeAvt.Evaluate(processor, frame)
                    );
