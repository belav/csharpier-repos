...
#endif
{
    ...
    public class XhtmlCssHandler : IHttpHandler, IRequiresSessionState
    {
        ...
        {
            String cssQueryStringValue = (String)
                context.Request.QueryString[XhtmlConstants.CssQueryStringName];
            String response;
            if (cssQueryStringValue != null)
            {
                // Recall that Page.Cache is application level
                if (
                    cssQueryStringValue.StartsWith(
                        XhtmlConstants.SessionKeyPrefix,
            ...
            {
                throw new HttpException(404, SR.GetString(SR.XhtmlCssHandler_IdNotPresent));
            }
            if (response == null)
            {
                throw new HttpException(404, SR.GetString(SR.XhtmlCssHandler_StylesheetNotFound));
            }
