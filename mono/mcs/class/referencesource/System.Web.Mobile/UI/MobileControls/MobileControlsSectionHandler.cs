...
namespace System.Web.UI.MobileControls
{
    ...
    public class MobileControlsSectionHandler : IConfigurationSectionHandler
    {
            ...
            if (!String.IsNullOrEmpty(cookielessDataDictionaryType))
            {
                ...
                if (t == null)
                {
                    throw new ConfigurationErrorsException(
                        SR.GetString(
                            SR.MobileControlsSectionHandler_TypeNotFound,
                    ...
                    case "device":
                        ...
                        foreach (XmlNode currentChild in nextNode.ChildNodes)
                        {
                            ...
                            if (!currentChild.Name.Equals("control"))
                            {
                                throw new ConfigurationErrorsException(
                                    SR.GetString(
                                        SR.MobileControlsSectionHandler_UnknownElementName,
                                        "<control>"
                                    ),
                                    currentChild
                                );
        ...
        {
            String nameOfDeviceToInheritFrom = helper.RemoveStringAttribute("inheritsFrom", false);
            ...
            String predicateClass = helper.RemoveStringAttribute(
                "predicateClass",
                propertiesRequired
            );
            // If a predicate class is specified, so must a method.
            String predicateMethod = helper.RemoveStringAttribute(
                "predicateMethod",
                predicateClass != null
            );
            String pageAdapterClass = helper.RemoveStringAttribute(
                "pageAdapter",
                propertiesRequired
            );
            ...
            {
                Type predicateClassType = CheckedGetType(
                    predicateClass,
                    "PredicateClass",
                    helper,
                    null,
            ...
            {
                pageAdapterType = CheckedGetType(
                    pageAdapterClass,
                    "PageAdapterClass",
                    helper,
                    typeof(IPageAdapter),
        ...
        )
        {
            ...
            {
                throw new ConfigurationErrorsException(
                    SR.GetString(SR.MobileControlsSectionHandler_TypeNotFound, typename, whereUsed),
                    helper.Node
                );
            }
