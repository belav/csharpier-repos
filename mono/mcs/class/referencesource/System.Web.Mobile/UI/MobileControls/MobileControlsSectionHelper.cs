...
namespace System.Web.UI.MobileControls
{
    ...
    {
        private static void AddControlAdapters(
            IndividualDeviceConfig deviceConfig,
        ...
        internal static ControlsConfig CreateControlsConfig(MobileControlsSection controlSection)
        {
            ...
            config["sessionStateHistorySize"] = controlSection.SessionStateHistorySize
                .ToString(CultureInfo.InvariantCulture);
            config["cookielessDataDictionaryType"] = controlSection.CookielessDataDictionaryType
                .AssemblyQualifiedName;
            config["allowCustomAttributes"] = controlSection.AllowCustomAttributes
                .ToString(CultureInfo.InvariantCulture);
            ...
            foreach (DeviceElement device in controlSection.Devices)
            {
                ...
                if (!config.AddDeviceConfig(device.Name, deviceConfig))
                {
                    ...
                    throw new ConfigurationErrorsException(
                        SR.GetString(
                            SR.MobileControlsSectionHandler_DuplicatedDeviceName,
                            device.Name
                        )
                    );
                }
        ...
        )
        {
            ...
            return new IndividualDeviceConfig(
                config,
                device.Name,
                predicateDelegate,
                device.PageAdapter,
