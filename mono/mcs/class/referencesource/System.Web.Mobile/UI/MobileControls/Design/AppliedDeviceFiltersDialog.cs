...
namespace System.Web.UI.Design.MobileControls
{
            ...
            IRefreshableDeviceSpecificEditor
    {
        ...
        internal AppliedDeviceFiltersDialog(IDeviceSpecificDesigner designer, int mergingContext)
            : base(designer.UnderlyingControl.Site)
        {
            ...
            _lblAvailableFilters.Text = SR.GetString(
                SR.AppliedDeviceFiltersDialog_AvailableDeviceFilters
            );
            _appliedFiltersList.LblTitle.Text = SR.GetString(
                SR.AppliedDeviceFiltersDialog_AppliedDeviceFilters
            );
            _btnEditFilters.Text = SR.GetString(SR.GenericDialog_Edit);
            _btnApplyFilter.Text = SR.GetString(SR.AppliedDeviceFiltersDialog_ApplyDeviceFilter);
            _lblArgument.Text = SR.GetString(SR.AppliedDeviceFiltersDialog_Argument);
            _cmdOK.Text = SR.GetString(SR.GenericDialog_OKBtnCaption);
            _cmdCancel.Text = SR.GetString(SR.GenericDialog_CancelBtnCaption);

            int tabOffset = GenericUI.InitDialog(this, _designer, mergingContext);
            ...
            _cbAvailableFilters.SelectedIndexChanged += new EventHandler(OnAvailableFilterSelected);
            _cbAvailableFilters.TextChanged += new EventHandler(OnAvailableFilterSelected);
            _btnApplyFilter.Click += new EventHandler(OnApplyFilter);
            _btnEditFilters.Click += new EventHandler(OnEditFilters);
            _appliedFiltersList.TvList.AfterSelect += new TreeViewEventHandler(
                OnAppliedFilterSelected
            );
            _appliedFiltersList.TvList.AfterLabelEdit += new NodeLabelEditEventHandler(
                OnAfterLabelEdit
            );
        ...
        private void InitializeComponent()
        {
            this._appliedFiltersList =
                new System.Web.UI.Design.MobileControls.Util.EditableTreeList(false, true, 16);
        ...
        private bool RestoreState(String deviceSpecificID)
        {
            if (null != deviceSpecificID)
            {
                ...
                AppliedFiltersCachedState state = (AppliedFiltersCachedState)
        ...
        {
            DeviceFilterNode filterUsed = FindAvailableFilter(runtimeChoice.Filter);

            ChoiceTreeNode choice = new ChoiceTreeNode(filterUsed, runtimeChoice, _designer);
        ...
        private void SaveChoices()
        {
            ...
            {
                AppliedFiltersCachedState state = (AppliedFiltersCachedState)entry.Value;
                state.SaveChoicesFromTreeView(_designer, (String)entry.Key);
            }
        }
        ...
        private void SelectFirstAvailableFilter()
        {
            ...
            {
                DeviceFilterNode filter = (DeviceFilterNode)_cbAvailableFilters.Items[0];
        ...
        private void SelectNextAvailableFilter(DeviceFilterNode currentFilter)
        {
            ...
            {
                _cbAvailableFilters.SelectedItem = _cbAvailableFilters.Items[index + 1];
            }
            else if (index > 0)
            {
                _cbAvailableFilters.SelectedItem = _cbAvailableFilters.Items[index - 1];
        ...
        {
            get { return FindAppliedFilter("") != null; }
        }

        private bool DefaultFilterIsAvailable
        {
            get { return FindAvailableFilter("") != null; }
        }
        ...
        {
            get { return _cbAvailableFilters.Text != null && _cbAvailableFilters.Text.Length > 0; }
        }

        private bool AppliedFilterIsSelected
        {
            get { return SelectedAppliedFilter != null; }
        }

        private ChoiceTreeNode SelectedAppliedFilter
        {
            get { return (ChoiceTreeNode)_appliedFiltersList.TvList.SelectedNode; }
        }

        private void UpdateUI()
        {
            if (AppliedFilterIsSelected && !IsDefaultFilter(SelectedAppliedFilter.Name))
            {
        ...
        private bool ValidateAppliedFilters()
        {
            ...
            if (duplicateChoices.Count > 0)
            {
                GenericUI.ShowWarningMessage(
                    SR.GetString(SR.AppliedDeviceFiltersDialog_Title),
                    SR.GetString(
                        SR.AppliedDeviceFiltersDialog_DuplicateChoices,
        ...
        private void OnAfterLabelEdit(Object sender, NodeLabelEditEventArgs e)
        {
            ...
            if (
                !DeviceFilterEditorDialog.NewLabelIsLegal(
        ...
        private void OnApplyFilter(Object sender, EventArgs e)
        {
            ...
            ChoiceTreeNode choice = new ChoiceTreeNode(filter, _designer);
        ...
        private void OnRemove(Object sender, EventArgs e)
        {
            ...
            {
                if (
                    !GenericUI.ConfirmYesNo(
                        SR.GetString(SR.AppliedDeviceFiltersDialog_Title),
                        SR.GetString(SR.AppliedDeviceFiltersDialog_AssociatedItemsWillBeLost)
                    )
                )
                {
                    return;
    ...
    internal class AppliedFiltersCachedState : DeviceSpecificDialogCachedState
    {
        ...
        internal AppliedFiltersCachedState(TreeView treeView)
        {
        ...
        {
            get { return _cachedTreeView; }
        }

        internal void Restore(TreeView treeView)
        {
    ...
    internal class ChoiceTreeNode : TreeNode
    {
        ...
        internal ChoiceTreeNode(DeviceFilterNode filter, IDeviceSpecificDesigner designer)
            : base()
        {
            Name = filter.Name;
            _designer = designer;
        ...
        {
            get { return _includeArgument; }
            set { _includeArgument = value; }
        }
        ...
        public override String ToString()
        {
            ...
            {
                name = new StringBuilder(SR.GetString(SR.DeviceFilter_DefaultChoice));
            }
