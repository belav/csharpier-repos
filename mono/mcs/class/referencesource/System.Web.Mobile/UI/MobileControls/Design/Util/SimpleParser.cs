...
namespace System.Web.UI.Design.MobileControls.Util
{
    ...
    internal class SimpleParser : BaseParser
    {
        ...
        private const RegexOptions _options = RegexOptions.Singleline | RegexOptions.Multiline;
            ...
            for (; ; )
            {
                ...
                else if ((match = _endtagRegex.Match(text, textPos)).Success)
                {
                    ...
                    while (!stack.IsEmpty())
                    {
                        ...
                        if (
                            String.Compare(endTag, startTag, StringComparison.OrdinalIgnoreCase)
                            != 0
                        )
                        {
