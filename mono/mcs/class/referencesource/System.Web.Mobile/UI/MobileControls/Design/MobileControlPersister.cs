...
namespace System.Web.UI.Design.MobileControls
{
    ...
    internal sealed class MobileControlPersister
    {
        ...
        private static string GetDeclarativeType(Type type, IDesignerHost host)
        {
            ...
            if (host != null)
            {
                IWebFormReferenceManager refMgr = (IWebFormReferenceManager)
                    host.GetService(typeof(IWebFormReferenceManager));
                Debug.Assert(
                    refMgr != null,
                    "Did not get back IWebFormReferenceManager service from host."
                );
        ...
        ///    The services interface exposed by the webforms designer.
        /// </param>
        private static void PersistCollectionProperty(
            TextWriter sw,
            object component,
        ...
        {
            Debug.Assert(
                typeof(ICollection).IsAssignableFrom(propDesc.PropertyType),
            ...
            while (e.MoveNext())
            {
                ...
                PersistNameAttribute pna = (PersistNameAttribute)
                    TypeDescriptor.GetAttributes(collItem.GetType())[typeof(PersistNameAttribute)];
                ...
                {
                    PersistChildrenAttribute pca = (PersistChildrenAttribute)
                        TypeDescriptor.GetAttributes(collItem.GetType())[
        ...
        ///    The services interface exposed by the webforms designer.
        /// </param>
        private static void PersistComplexProperty(
            TextWriter sw,
            object component,
            PropertyDescriptor propDesc,
            IDesignerHost host
        )
        {
            ...
            if (
                (tagProps.GetStringBuilder().Length != 0)
        ...
        ///    The services interface exposed by the webforms designer.
        /// </param>
        internal static void PersistInnerProperties(
            TextWriter sw,
            object component,
            IDesignerHost host
        )
        {
            ...
            for (int i = 0; i < propDescs.Count; i++)
            {
                ...
                PersistenceModeAttribute persistenceMode = (PersistenceModeAttribute)
                    propDescs[i].Attributes[typeof(PersistenceModeAttribute)];
                ...
                if (propDescs[i].PropertyType == typeof(string))
                {
                    ...
                    if (dataBindings == null || dataBindings[propDescs[i].Name] == null)
                    {
                        ...
                        {
                            Debug.Fail(
                                "Cannot persist inner string property marked with PersistenceMode.InnerProperty"
                            );
                        }
                    }
                }
                else if (typeof(ITemplate).IsAssignableFrom(propDescs[i].PropertyType))
                {
                    ...
                    {
                        Debug.Fail(
                            "Cannot persist template property "
                                + propDescs[i].Name
                                + " not marked with PersistenceMode.InnerProperty"
                        );
                    }
                }
                /* AUI change 03/21/01 */
                ...
                else if (typeof(ICollection).IsAssignableFrom(propDescs[i].PropertyType))
                {
                    ...
                    if (
                        (persistenceMode.Mode == PersistenceMode.InnerProperty)
                    ...
                    {
                        Debug.Fail(
                            "Cannot persist collection property "
                                + propDescs[i].Name
                                + " not marked with PersistenceMode.InnerProperty or PersistenceMode.InnerDefaultProperty"
                        );
                    }
                }
                else
                {
                    ...
                    {
                        Debug.Fail(
                            "Cannot persist complex property "
                                + propDescs[i].Name
                                + " not marked with PersistenceMode.InnerProperty"
                        );
                    }
        ...
        /// </param>
        private static void PersistStringProperty(
            TextWriter sw,
            object component,
            PropertyDescriptor propDesc,
            PersistenceMode mode
        ...
        {
            Debug.Assert(
                propDesc.PropertyType == typeof(string),
        ...
        /// </param>
        private static void PersistAttributes(
            TextWriter sw,
            object component,
            string prefix,
            PropertyDescriptor propDesc
        )
        {
            ...
            if (propDesc != null)
            {
                ...
                properties = TypeDescriptor.GetProperties(
                    ...
                    new Attribute[] { PersistenceModeAttribute.Attribute }
                );
            }
            else
            {
            ...
            for (int i = 0; i < properties.Count; i++)
            {
                ...
                DesignOnlyAttribute doAttr = (DesignOnlyAttribute)
                    properties[i].Attributes[typeof(DesignOnlyAttribute)];
                ...
                DefaultValueAttribute defValAttr = (DefaultValueAttribute)
                    properties[i].Attributes[typeof(DefaultValueAttribute)];
                ...
                if (
                    properties[i].SerializationVisibility == DesignerSerializationVisibility.Content
                )
                {
                ...
                else
                {
                    ...
                    if (obj is ListDictionary)
                    {
                        ...
                        while (enumerator.MoveNext())
                        {
                            ...
                            if ((propName.Length == 0) || (persistValue == null))
                                continue;
        ...
        {
            Debug.Assert(
                typeof(ITemplate).IsAssignableFrom(propDesc.PropertyType),
        ...
        internal static void PersistControl(TextWriter sw, Control control)
        {
            ...
            Debug.Assert(
                host != null,
                "Did not get a valid IDesignerHost reference. Expect persistence problems!"
            );
        ...
        internal static void PersistControl(TextWriter sw, Control control, IDesignerHost host)
        {
            ...
            {
                PersistChildrenAttribute pca = (PersistChildrenAttribute)
                    TypeDescriptor.GetAttributes(control.GetType())[
                        typeof(PersistChildrenAttribute)
                    ];
        ...
        ///    The services interface exposed by the webforms designer.
        /// </param>
        private static void PersistChildControls(
            TextWriter sw,
        ...
        private static void PersistDataBoundLiteralControl(
            TextWriter sw,
            DesignerDataBoundLiteralControl control
        )
        {
            ...
            Debug.Assert(
                textBinding != null,
                "Did not get a Text databinding from DesignerDataBoundLiteralControl"
            );
