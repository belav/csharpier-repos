...
namespace System.ServiceModel.WasHosting
{
    ...
    static class MetabaseSettingsIis7Constants
    {
        ...
        internal const string ClientCertMapAuthenticationName =
            "system.webServer/security/authentication/clientCertificateMappingAuthentication";
        internal const string IisClientCertMapAuthenticationName =
            "system.webServer/security/authentication/iisClientCertificateMappingAuthentication";
        internal const string AnonymousAuthenticationSectionName =
            "system.webServer/security/authentication/anonymousAuthentication";
        internal const string BasicAuthenticationSectionName =
            "system.webServer/security/authentication/basicAuthentication";
        internal const string DigestAuthenticationSectionName =
            "system.webServer/security/authentication/digestAuthentication";
        internal const string WindowsAuthenticationSectionName =
    ...
    class MetabaseSettingsIis7 : MetabaseSettingsIis
    {
            ...
            : base()
        {
            if (!Iis7Helper.IsIis7)
            {
                DiagnosticUtility.DebugAssert(
                    "MetabaseSettingsIis7 constructor must not be called when running outside of IIS7"
                );
        ...
        void PopulateSiteProperties()
        {
            ...
            IDictionary<string, List<string>> bindingList =
                ServerManagerWrapper.GetProtocolBindingTable(site);
            ...
            //
            string[] protocols = ServerManagerWrapper
                .GetEnabledProtocols(site)
                .Split(
                    MetabaseSettingsIis7Constants.CommaSeparator.ToCharArray(),
            ...
            foreach (string protocolValue in protocols)
            {
                ...
                if (string.IsNullOrEmpty(protocol) || this.Protocols.Contains(protocol))
                {
                    ...
                    string.Compare(protocol, Uri.UriSchemeHttp, StringComparison.OrdinalIgnoreCase)
                        == 0
                    || string.Compare(
                        protocol,
                        Uri.UriSchemeHttps,
        ...
        {
            Debug.Print(
                "MetabaseSettingsIis7.CreateTransportSettings() calling ServerManager.GetWebConfiguration() virtualPath: "
                    + relativeVirtualPath
            );
            ...
            Configuration config = ServerManagerWrapper.GetWebConfiguration(
        ...
        void ProcessAnonymousAuthentication(
            Configuration config,
            ref HostedServiceTransportSettings transportSettings
        )
        {
            ConfigurationSection section = ServerManagerWrapper.GetSection(
                config,
                MetabaseSettingsIis7Constants.AnonymousAuthenticationSectionName
            );
            ...
            if (
                ...
                && (
                    (bool)
                        ServerManagerWrapper.GetAttributeValue(
                            section,
                            MetabaseSettingsIis7Constants.EnabledAttributeName
                        )
                )
            )
            {
        ...
        void ProcessBasicAuthentication(
            Configuration config,
            ref HostedServiceTransportSettings transportSettings
        )
        {
            ConfigurationSection section = ServerManagerWrapper.GetSection(
                config,
                MetabaseSettingsIis7Constants.BasicAuthenticationSectionName
            );
            ...
            )
            {
                ...
                transportSettings.Realm = (string)
                    ServerManagerWrapper.GetAttributeValue(
                        section,
                        MetabaseSettingsIis7Constants.RealmAttributeName
                    );
            }
        }

        void ProcessWindowsAuthentication(
            Configuration config,
            ref HostedServiceTransportSettings transportSettings
        )
        {
            ConfigurationSection section = ServerManagerWrapper.GetSection(
                config,
                MetabaseSettingsIis7Constants.WindowsAuthenticationSectionName
            );
            ...
            )
            {
                ...
                List<string> providerList = ServerManagerWrapper.GetProviders(
                    section,
                    MetabaseSettingsIis7Constants.ProviderElementName,
            ...
            try
            {
                ConfigurationElement element = section.GetChildElement(
                    MetabaseSettingsIis7Constants.ExtendedProtectionElementName
                );
                if (element != null)
                {
                    ...
                    List<string> spnList;
                    ServerManagerWrapper.ReadIisExtendedProtectionPolicy(
                        element,
                        out tokenChecking,
                        out flags,
                        out spnList
                    );
                    transportSettings.IisExtendedProtectionPolicy = BuildExtendedProtectionPolicy(
                        tokenChecking,
                        flags,
                        spnList
                    );
        ...
        void ProcessDigestAuthentication(
            Configuration config,
            ref HostedServiceTransportSettings transportSettings
        )
        {
            ConfigurationSection section = ServerManagerWrapper.GetSection(
                config,
                MetabaseSettingsIis7Constants.DigestAuthenticationSectionName
            );
        ...
        )
        {
            ConfigurationSection section = ServerManagerWrapper.GetSection(
                config,
                MetabaseSettingsIis7Constants.SecurityAccessSectionName
            );
            ...
            if (section != null)
            {
                int sslFlags = (int)
                    ServerManagerWrapper.GetAttributeValue(
                        section,
                        MetabaseSettingsIis7Constants.SslFlagsAttributeName
                    );
            ...
            section = ServerManagerWrapper.GetSection(
                config,
                MetabaseSettingsIis7Constants.IisClientCertMapAuthenticationName
            );
            if (
                (section != null)
                && (
            ...
            else
            {
                ...
                section = ServerManagerWrapper.GetSection(
                    config,
                    MetabaseSettingsIis7Constants.ClientCertMapAuthenticationName
                );
                if (
                    (section != null)
                    && (
        ...
        static class ServerManagerWrapper
        {
            ...
            )
            {
                ...
                {
                    providerList.Add(
                        (string)ServerManagerWrapper.GetAttributeValue(element, valueAttributeName)
                    );
                }
                return providerList;
            }
            ...
            internal static IDictionary<string, List<string>> GetProtocolBindingTable(Site site)
            {
                ...
                foreach (Microsoft.Web.Administration.Binding binding in site.Bindings)
                {
                    ...
                    Debug.Print(
                        "MetabaseSettingsIis7.ctor() adding Protocol: "
                            + protocol
                            + " BindingInformation: "
                            + bindingInformation
                    );
            ...
            internal static void ReadIisExtendedProtectionPolicy(
                ...
                out List<string> spnList
            )
            {
                tokenChecking = (ExtendedProtectionTokenChecking)
                    element.GetAttributeValue(
                        MetabaseSettingsIis7Constants.TokenCheckingAttributeName
                    );
                flags = (ExtendedProtectionFlags)
                    element.GetAttributeValue(MetabaseSettingsIis7Constants.FlagsAttributeName);
                spnList = new List<string>();
                foreach (ConfigurationElement configElement in element.GetCollection())
                {
                    spnList.Add(
                        (string)configElement[MetabaseSettingsIis7Constants.NameAttributeName]
                    );
                }
            }
    ...
    class MetabaseSettingsIis7V2 : MetabaseSettingsIis
    {
            ...
            : base()
        {
            ...
            {
                DiagnosticUtility.DebugAssert(
                    "MetabaseSettingsIis7V2 constructor must not be called when running outside of IIS7"
                );
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperInternal(true);
            }
        ...
        void PopulateSiteProperties()
        {
            ...
            //
            IDictionary<string, List<string>> bindingList =
                WebConfigurationManagerWrapper.GetProtocolBindingTable(site);
            ...
            string[] protocols = WebConfigurationManagerWrapper
                .GetEnabledProtocols(site)
                .Split(
                    MetabaseSettingsIis7Constants.CommaSeparator.ToCharArray(),
                    StringSplitOptions.RemoveEmptyEntries
        ...
        {
            ConfigurationSection section = WebConfigurationManagerWrapper.WebConfigGetSection(
                siteName,
                virtualPath,
                MetabaseSettingsIis7Constants.AnonymousAuthenticationSectionName
            );
            ...
            if (
                ...
                && (
                    (bool)
                        WebConfigurationManagerWrapper.GetValue(
                            section,
                            MetabaseSettingsIis7Constants.EnabledAttributeName
                        )
                )
        ...
        )
        {
            ...
            )
            {
                transportSettings.AuthFlags = transportSettings.AuthFlags | AuthFlags.AuthBasic;
                transportSettings.Realm = (string)
                    section.GetAttribute(MetabaseSettingsIis7Constants.RealmAttributeName).Value;
            }
        }
            ...
            )
            {
                ...
                try
                {
                    ...
                    if (element != null)
                    {
                        ...
                        ExtendedProtectionFlags flags;
                        List<string> spnList;
                        WebConfigurationManagerWrapper.ReadIisExtendedProtectionPolicy(
                            element,
                            out tokenChecking,
                            out flags,
                            out spnList
                        );
                        transportSettings.IisExtendedProtectionPolicy =
                            BuildExtendedProtectionPolicy(tokenChecking, flags, spnList);
                    }
                }
                catch (COMException e)
                {
        ...
        )
        {
            ...
            // Check whether IIS client certificate mapping is enabled.
            section = WebConfigurationManagerWrapper.WebConfigGetSection(
                siteName,
                virtualPath,
                MetabaseSettingsIis7Constants.IisClientCertMapAuthenticationName
            );
        ...
        static class WebConfigurationManagerWrapper
        {
            ...
            internal static ConfigurationElement GetSite(string siteName)
            {
                ConfigurationSection sitesSection = WebConfigGetSection(
                    null,
                    null,
                    MetabaseSettingsIis7Constants.SitesSectionName
                );
                ConfigurationElementCollection sitesCollection = sitesSection.GetCollection();

                return FindElement(
                    sitesCollection,
                    MetabaseSettingsIis7Constants.NameAttributeName,
                    siteName
                );
            }
            ...
            )
            {
                return (ConfigurationSection)
                    GetSectionMethod.Invoke(
                        null,
                        new object[] { siteName, virtualPath, sectionName }
                    );
            }
            ...
            )
            {
                IDictionary<string, List<string>> bindingList =
                    new Dictionary<string, List<string>>();
                foreach (
                    ConfigurationElement binding in site.GetCollection(
                        MetabaseSettingsIis7Constants.BindingsElementName
                    )
                )
                {
                    string protocol = (
                    ...
                    Debug.Print(
                        "MetabaseSettingsIis7V2.ctor() adding Protocol: "
                            + protocol
                            + " BindingInformation: "
                            + bindingInformation
                    );
            ...
            internal static List<string> GetProviderList(ConfigurationElement section)
            {
                List<string> providerList = new List<string>();
                foreach (
                    ConfigurationElement element in section.GetCollection(
                        MetabaseSettingsIis7Constants.ProviderElementName
                    )
                )
                {
                    providerList.Add(
                        (string)element[MetabaseSettingsIis7Constants.ValueAttributeName]
                    );
                }
                return providerList;
            }
