...
namespace System.Web.Hosting
{
    ...
    internal sealed class CustomLoaderHelper : MarshalByRefObject
    {
        ...
        private static readonly string _customLoaderTargetFrameworkName = new FrameworkName(
            ".NETFramework",
            new Version(4, 5, 1)
        ).ToString();
        ...
        )
        {
            ...
            AppDomain newAppDomainForCustomLoader = AppDomain.CreateDomain(
                "aspnet-custom-loader-" + Guid.NewGuid(),
                null,
                setup
            );
            try
            {
                ...
                CustomLoaderHelper helper = (CustomLoaderHelper)
                    newAppDomainForCustomLoader.CreateInstanceAndUnwrap(
                        _customLoaderAssemblyName,
                        _customLoaderTypeName,
                        ignoreCase: false,
        ...
        private ObjectHandle GetCustomLoaderImpl(string customLoaderPhysicalPath)
        {
            ...
            AssemblyName customLoaderAssemblyName = AssemblyName.GetAssemblyName(
                customLoaderPhysicalPath
            );
            Assembly customLoaderAssembly = Assembly.Load(customLoaderAssemblyName);
            CustomLoaderAttribute customLoaderAttribute =
                customLoaderAssembly.GetCustomAttribute<CustomLoaderAttribute>();
            ...
            if (customLoaderAttribute == null)
            {
                throw new InvalidOperationException(
                    String.Format(
                        ...
                        ApplicationServicesStrings.CustomLoader_NoAttributeFound,
                        customLoaderAssemblyName
                    )
                );
            }
            ...
            if (!ObjectImplementsComInterface(customLoader, IID_ICustomLoader))
            {
                throw new InvalidOperationException(
                    String.Format(
                        CultureInfo.CurrentCulture,
                        ApplicationServicesStrings.CustomLoader_MustImplementICustomLoader,
                        customLoader.GetType()
                    )
                );
            }
