...
namespace System.Xml
{
    ...
    public class XmlConvert
    {
        ...
        )
        {
            ...
            if (first)
            {
                ...
                )
                {
                    ...
                    {
                        bufBld.Append(
                            ((Int32)name[0]).ToString("X4", CultureInfo.InvariantCulture)
                        );
                        copyPosition = 1;
                    }
            ...
            for (; position < length; position++)
            {
                ...
                )
                {
                    ...
                    {
                        bufBld.Append(
                            ((Int32)name[position]).ToString("X4", CultureInfo.InvariantCulture)
                        );
        ...
        internal static Exception TryVerifyName(string name)
        {
            ...
            if (endPos != name.Length)
            {
                return new XmlException(
                    endPos == 0 ? Res.Xml_BadStartNameChar : Res.Xml_BadNameChar,
        ...
        public static string VerifyWhitespace(string content)
        {
            ...
            if (pos != -1)
            {
                throw new XmlException(
                    Res.Xml_InvalidWhitespaceCharacter,
        ...
        {
#else
        internal static unsafe void VerifyCharData(
            string data,
            ExceptionType invCharExceptionType,
            ExceptionType invSurrogateExceptionType
        )
        {
            ...
            for (; ; )
            {
...
#else
                while (
                    i < len && (xmlCharType.charProperties[data[i]] & XmlCharType.fCharData) != 0
                )
                {
#endif
                    i++;
                ...
                if (XmlCharType.IsHighSurrogate(ch))
                {
                    ...
                    {
                        throw CreateException(
                            Res.Xml_InvalidSurrogateMissingLowChar,
...
#else
                while (
                    i < endPos && (xmlCharType.charProperties[data[i]] & XmlCharType.fCharData) != 0
                )
                {
#endif
                    i++;
        ...
        {
            return CreateException(
                Res.Xml_InvalidSurrogateHighChar,
                ((uint)hi).ToString("X", CultureInfo.InvariantCulture),
                exceptionType,
                lineNo,
                linePos
        ...
        {
            return CreateInvalidCharException(
                data,
                length,
                invCharPos,
        ...
        {
            return CreateException(
                index == 0 ? Res.Xml_BadStartNameChar : Res.Xml_BadNameChar,
        ...
        )
        {
            return (name == null)
                ...
                : new ArgumentException(Res.GetString(Res.Xml_EmptyName), argumentName);
        }
    }
}
