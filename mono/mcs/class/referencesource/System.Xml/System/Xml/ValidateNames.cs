...
namespace System.Xml
{
    ...
    internal static class ValidateNames
    {
        ...
        internal static unsafe int ParseNameNoNamespaces(string s, int offset)
        {
            ...
            {
                if (
                    (xmlCharType.charProperties[s[i]] & XmlCharType.fNCStartNameSC) != 0
                    || s[i] == ':'
                )
                ...
                {
                    if (
                        (xmlCharType.charProperties[s[i]] & XmlCharType.fNCNameSC) != 0
                        || s[i] == ':'
                    )
                    { // if (xmlCharType.IsNCNameSingleChar(s[i]))
                        i++;
        ...
        internal static void ThrowInvalidName(string s, int offsetStartChar, int offsetBadChar)
        {
            ...
            if (
                ...
                && !XmlCharType.Instance.IsStartNCNameSingleChar(s[offsetBadChar])
            )
            {
                ...
                throw new XmlException(
                    Res.GetString(
                        Res.Xml_BadStartNameChar,
                        XmlExceptionHelper.BuildCharExceptionArgs(s, offsetBadChar)
                    )
                );
#endif
            }
            ...
            if ((flags & Flags.CheckLocalName) != 0)
            {
                ...
                switch (nodeKind)
                {
                    case XPathNodeType.Element:
                        ...
                        {
                            if (throwOnError)
                                throw new XmlException(Res.Xdom_Empty_LocalName, string.Empty);
                            return false;
                        }
                        break;
                    ...
                    case XPathNodeType.ProcessingInstruction:
                        ...
                        {
                            if (throwOnError)
                                throw new XmlException(Res.Xml_InvalidPIName, localName);
                            return false;
                        }
                        break;
                    ...
                    case XPathNodeType.Namespace:
                        ...
                        else
                        {
                            ...
                            if (prefix.Equals("xml"))
                            {
                                ...
                                {
                                    if (throwOnError)
                                        throw new XmlException(Res.Xml_XmlPrefix, string.Empty);
                                    return false;
                                }
                            }
                            else if (prefix.Equals("xmlns"))
                            {
                                ...
                                if (throwOnError)
                                    throw new XmlException(Res.Xml_XmlnsPrefix, string.Empty);
                                return false;
                            }
        ...
        internal static void SplitQName(string name, out string prefix, out string lname)
        {
            ...
            else if (0 == colonPos || (name.Length - 1) == colonPos)
            {
                ...
                throw new ArgumentException(
                    Res.GetString(
                        Res.Xml_BadNameChar,
                        XmlExceptionHelper.BuildCharExceptionArgs(':', '\0')
                    ),
                    "name"
                );
#endif
