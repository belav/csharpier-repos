...
namespace System.Xml
{
    internal sealed partial class XmlSqlBinaryReader : XmlReader, IXmlNamespaceResolver
    {
        ...
        {
            get { return this.qnameOther.localname; }
        }

        public override string NamespaceURI
        {
            get { return this.qnameOther.namespaceUri; }
        }

        public override string Prefix
        {
            get { return this.qnameOther.prefix; }
        }
        ...
        {
            get { return this.state == ScanState.EOF; }
        }
        ...
        {
            get { return ScanState2ReadState[(int)this.state]; }
        }
        ...
        public override bool ReadContentAsBoolean()
        {
            ...
            try
            {
                if (SetupContentAsXXX("ReadContentAsBoolean"))
                {
                        ...
                        switch (this.token)
                        {
                            ...
                            case BinXmlToken.XSD_QNAME:
                                throw new InvalidCastException(
                                    Res.GetString(
                                        Res.XmlBinary_CastNotSupported,
                                        this.token,
                                        "Boolean"
                                    )
                                );
                    ...
                    {
                        throw new XmlException(
                            Res.Xml_ReadContentAsFormatException,
                            "Boolean",
                            e,
                            null
                        );
        ...
        private bool FillAllowEOF()
        {
            ...
            if (mark >= 0 && mark < end)
            {
                ...
                for (int i = 0; i < this.attrCount; i++)
                {
                    ...
                    Debug.Assert(
                        (this.attributes[i].contentPos >= 0)
                            && (this.attributes[i].contentPos <= (end))
                    );
                }
                this.pos = pos;
                this.mark = 0;
        ...
        bool ReadInit(bool skipXmlDecl)
        {
            ...
            if (BinXmlToken.XmlDecl == PeekToken())
            {
                ...
                this.attributes[0]
                    .Set(
                        new QName(string.Empty, this.xnt.Add("version"), string.Empty),
                        ParseText()
                    );
                this.attrCount = 1;
                if (BinXmlToken.Encoding == PeekToken())
                {
                    ...
                    this.attributes[1]
                        .Set(
                            new QName(string.Empty, this.xnt.Add("encoding"), string.Empty),
                            ParseText()
                        );
                    this.attrCount++;
                }
                ...
                switch (standalone)
                {
                    ...
                    case 1:
                    case 2:
                        this.attributes[this.attrCount]
                            .Set(
                                new QName(string.Empty, this.xnt.Add("standalone"), string.Empty),
                                (standalone == 1) ? "yes" : "no"
                            );
                        this.attrCount++;
                        break;
                ...
                if (BinXmlToken.Attr == token)
                {
                    ...
                    else if (n.namespaceUri.Length != 0)
                    {
                        throw ThrowXmlException(
                            Res.XmlBinary_AttrWithNsNoPrefix,
                            n.localname,
                            n.namespaceUri
                        );
                    }
                ...
                for (int j = i + 1; j < this.attrCount; j++)
                {
                    if (this.attributes[j].MatchNS(localname, namespaceUri))
                        throw new XmlException(
                            ...
                            this.attributes[i].name.ToString()
                        );
                }
            }
        }
        ...
        void HashCheckForDuplicateAttributes()
        {
            ...
            for (int i = 0; i < this.attrCount; i++)
            {
                string localname,
                    namespaceUri;
                int hash = this.attributes[i]
                    .GetLocalnameAndNamespaceUriAndHash(hasher, out localname, out namespaceUri);
        ...
        void ImplReadDoctype()
        {
            ...
            if (BinXmlToken.System == PeekToken())
            {
                ...
                this.attributes[this.attrCount++]
                    .Set(
                        new QName(string.Empty, this.xnt.Add("SYSTEM"), string.Empty),
                        ParseText()
                    );
            }
            if (BinXmlToken.Public == PeekToken())
            {
                ...
                this.attributes[this.attrCount++]
                    .Set(
                        new QName(string.Empty, this.xnt.Add("PUBLIC"), string.Empty),
                        ParseText()
                    );
            }
        ...
        void ImplReadNest()
        {
            ...
            this.prevNameInfo = new NestedBinXml(
                this.symbolTables,
                this.docState,
                this.prevNameInfo
            );
            ...
            fixed (byte* pb = this.data)
            {
                ...
                if (!attr)
                {
                    ...
                    for (; ; )
                    {
                        ...
                        if (
                            pb[pos + 1] != 0
                            || (xmlCharType.charProperties[pb[pos]] & XmlCharType.fWhitespace) == 0
                        )
                            break;
                        pos = posNext;
                    }
            ...
            switch (token)
            {
                ...
                case BinXmlToken.SQL_TINYINT:
                    value = GetValueConverter(XmlTypeCode.UnsignedByte)
                        .ChangeType(this.data[this.tokDataPos], returnType, namespaceResolver);
                    break;
