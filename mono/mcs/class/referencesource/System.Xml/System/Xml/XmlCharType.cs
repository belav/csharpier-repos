...
namespace System.Xml
{
    ...
    internal struct XmlCharType
    {
        ...
        private const int s_NCNameSurHighStart =
            SurHighStart + ((s_NCNameSurCombinedStart - 0x10000) / 1024);
        private const int s_NCNameSurHighEnd =
            SurHighStart + ((s_NCNameSurCombinedEnd - 0x10000) / 1024);
        private const int s_NCNameSurLowStart =
        ...
        static void InitInstance()
        {
            lock (StaticLock)
            {
                ...
                UnmanagedMemoryStream memStream = (UnmanagedMemoryStream)
                    Assembly.GetExecutingAssembly().GetManifestResourceStream("XmlCharType.bin");
        ...
        internal static void SplitSurrogateChar(
            int combinedChar,
            out char lowChar,
            out char highChar
        )
