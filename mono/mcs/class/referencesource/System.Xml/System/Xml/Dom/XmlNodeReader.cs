...
namespace System.Xml
{
    ...
    internal class XmlNodeReaderNavigator
    {
        ...
        public String Name
        {
            get
            {
                ...
                {
                    Debug.Assert(
                        curNode.NodeType == XmlNodeType.XmlDeclaration
                            || curNode.NodeType == XmlNodeType.DocumentType
                    );
                    if (this.bOnAttrVal)
        ...
        {
            get { return this.bCreatedOnAttribute; }
        }
        ...
        public String GetAttribute(String name, String ns)
        {
            ...
            switch (curNode.NodeType)
            {
                ...
                case XmlNodeType.XmlDeclaration:
                    return (ns.Length == 0)
                        ? GetDeclarationAttr((XmlDeclaration)curNode, name)
                        : null;
                case XmlNodeType.DocumentType:
                    return (ns.Length == 0)
                        ? GetDocumentTypeAttr((XmlDocumentType)curNode, name)
                        : null;
            }
            return null;
        }
            ...
            if (this.bOnAttrVal)
            {
                ...
                else
                {
                    while (
                        ...
                        && ((curNode = curNode.ParentNode) != null)
                    )
                        level--;
                }
                this.bOnAttrVal = false;
    ...
    public class XmlNodeReader : XmlReader, IXmlNamespaceResolver
    {
        ...
        public override bool MoveToAttribute(string name, string namespaceURI)
        {
            ...
            readerNav.ResetMove(ref curDepth, ref nodeType);
            String ns = (namespaceURI == null) ? String.Empty : namespaceURI;
            if (readerNav.MoveToAttribute(name, ns))
            { //, ref curDepth ) ) {
                curDepth++;
                nodeType = readerNav.NodeType;
                if (bInReadBinary)
        ...
        private bool Read(bool fSkipChildren)
        {
            ...
            if (readState == ReadState.Initial)
            {
                ...
                if (
                    (readerNav.NodeType == XmlNodeType.Document)
                    || (readerNav.NodeType == XmlNodeType.DocumentFragment)
                )
                {
        ...
        private bool ReadNextNode(bool fSkipChildren)
        {
            ...
            if (bDrillDown)
            {
                ...
                else if (readerNav.NodeType == XmlNodeType.Element && !readerNav.IsEmptyElement)
                {
                    nodeType = XmlNodeType.EndElement;
                    return true;
                }
        ...
        {
            get { return readerNav.Document.DtdSchemaInfo; }
        }
    }
}
