...
namespace System.Xml
{
    ...
    internal class XmlLoader
    {
                ...
                switch (r.NodeType)
                {
                    case XmlNodeType.Element:
                        ...
                        else
                        {
                            ...
                            if (schemaInfo != null)
                            {
                                element.XmlName = doc.AddXmlName(
                                    element.Prefix,
                                    element.LocalName,
        ...
        private XmlAttribute LoadAttributeNode()
        {
            ...
            if (schemaInfo != null)
            {
                attr.XmlName = doc.AddAttrXmlName(
                    attr.Prefix,
                    attr.LocalName,
                    attr.NamespaceURI,
                    schemaInfo
        ...
        private void LoadAttributeValue(XmlNode parent, bool direct)
        {
            ...
            while (r.ReadAttributeValue())
            {
                ...
                switch (r.NodeType)
                {
                    ...
                    case XmlNodeType.EntityReference:
                        node = direct
                            ? new XmlEntityReference(reader.LocalName, doc)
                            : doc.CreateEntityReference(reader.LocalName);
                        if (r.CanResolveEntity)
                        {
                            r.ResolveEntity();
        ...
        private XmlEntityReference LoadEntityReferenceNode(bool direct)
        {
            ...
            XmlEntityReference eref = direct
                ? new XmlEntityReference(reader.Name, this.doc)
                : doc.CreateEntityReference(reader.Name);
        ...
        private XmlDeclaration LoadDeclarationNode()
        {
            ...
            // For readers that do not break xml decl into attributes, we must parse the xml decl ourselfs. We
        ...
        private XmlDocumentType LoadDocumentTypeNode()
        {
            ...
            XmlDocumentType dtNode = doc.CreateDocumentType(
                localName,
                publicId,
                systemId,
                internalSubset
        ...
        // becaouse we do not want to let users extend these (if we would allow this, XmlDataDocument would
        // have a problem, becaouse
        ...
        )
        {
            ...
            XmlParserContext pc = new XmlParserContext(
                null,
                new XmlNamespaceManager(this.doc.NameTable),
                null,
                null,
                null,
                null,
                ...
                if (schInfo.Notations != null)
                {
                    foreach (SchemaNotation scNot in schInfo.Notations.Values)
                    {
                        dtNode.Notations.SetNamedItem(
                            new XmlNotation(scNot.Name.Name, scNot.Pubid, scNot.SystemLiteral, doc)
                        );
                    }
                }
        ...
        private XmlParserContext GetContext(XmlNode node)
        {
            ...
            while (node != null && node != doc)
            {
                if (node is XmlElement && ((XmlElement)node).HasAttributes)
                {
                    ...
                    {
                        if (
                            attr.Prefix == doc.strXmlns
                            && prefixes.Contains(attr.LocalName) == false
                        )
                        {
            ...
            if (fCheckElemAttrs && cAttrs > 0)
            {
                for (int i = cAttrs - 1; i >= 0; --i)
                {
                    ...
                    if (attr.Prefix == doc.strXmlns)
                    {
                        ...
                        {
                            // Add this namespace, so it we will behave corectly when setting "<bar xmlns:p="BAR"><foo2
                            // xmlns:p="FOO"/></bar>" as
...
#pragma warning disable 618
        ...
        private XmlReader CreateInnerXmlReader(
            String xmlFragment,
            XmlNodeType nt,
