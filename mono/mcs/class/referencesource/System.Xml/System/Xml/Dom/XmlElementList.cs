    ...
    internal class XmlElementList : XmlNodeList
    {
        ...
        private XmlNode NextElemInPreOrder(XmlNode curNode)
        {
            ...
            if (retNode == null)
            {
                ...
                retNode = curNode;
                while (retNode != null && retNode != rootNode && retNode.NextSibling == null)
                {
        ...
        private bool IsMatch(XmlNode curNode)
        {
            if (curNode.NodeType == XmlNodeType.Element)
            {
                ...
                else
                {
                    if (
                        (
                            Ref.Equal(this.localName, asterisk)
                            || Ref.Equal(curNode.LocalName, this.localName)
                        )
                        && (
                            Ref.Equal(this.namespaceURI, asterisk)
                            || curNode.NamespaceURI == this.namespaceURI
                        )
                    )
                    {
                        return true;
