    ...
    public class CodeIdentifier
    {
        ...
        public static string MakePascal(string identifier)
        {
            ...
            if (identifier.Length <= 2)
                ...
                return char.ToUpper(identifier[0], CultureInfo.InvariantCulture)
                        .ToString(CultureInfo.InvariantCulture) + identifier.Substring(1);
            else
                return identifier;
        }
        ...
        public static string MakeCamel(string identifier)
        {
            ...
            if (identifier.Length <= 2)
                ...
                return char.ToLower(identifier[0], CultureInfo.InvariantCulture)
                        .ToString(CultureInfo.InvariantCulture) + identifier.Substring(1);
            else
                return identifier;
        }
                ...
                throw new ArgumentException(
                    Res.GetString(
                        ...
                        "Invalid identifier character "
                            + ((Int16)c).ToString(CultureInfo.InvariantCulture)
                    ),
                    "c"
                );
#endif
        ...
        static bool IsValid(char c)
        {
            ...
            switch (uc)
            {
                ...
                default:
                    ...
                    // use exception in the place of Debug.Assert to avoid throwing asserts from a server process such as aspnet_ewp.exe
                    throw new ArgumentException(
                        Res.GetString(Res.XmlInternalErrorDetails, "Unhandled category " + uc),
                        "c"
                    );
#else
                    return false;
        ...
        internal static void CheckValidIdentifier(string ident)
        {
            if (!CodeGenerator.IsValidLanguageIndependentIdentifier(ident))
                throw new ArgumentException(
                    Res.GetString(Res.XmlInvalidIdentifier, ident),
                    "ident"
                );
        }
        ...
        static int GetCSharpName(Type t, Type[] parameters, int index, StringBuilder sb)
        {
            ...
            if (nameEnd > 0)
            {
                ...
                sb.Append("<");
                int arguments =
                    Int32.Parse(name.Substring(nameEnd + 1), CultureInfo.InvariantCulture) + index;
