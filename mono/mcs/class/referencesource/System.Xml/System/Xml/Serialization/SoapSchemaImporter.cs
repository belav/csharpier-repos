    ...
    public class SoapSchemaImporter : SchemaImporter
    {
            ...
            if (baseType != null)
            {
                for (int i = 0; i < mapping.Members.Length; i++)
                {
                    ...
                    if (member.Accessor.Mapping is StructMapping)
                        MakeDerived(
                            (StructMapping)member.Accessor.Mapping,
                            baseType,
        ...
        ElementAccessor ImportElement(XmlSchemaElement element, string ns)
        {
            if (!element.RefName.IsEmpty)
            {
                throw new InvalidOperationException(
                    Res.GetString(
                        Res.RefSyntaxNotSupportedForElements0,
        ...
        TypeMapping ImportElementType(XmlSchemaElement element, string ns)
        {
            ...
            else if (element.SchemaType != null)
            {
                ...
                if (element.SchemaType is XmlSchemaComplexType)
                {
                    ...
                    if (!(mapping is ArrayMapping))
                    {
                        throw new InvalidOperationException(
                            Res.GetString(
                                Res.XmlInvalidSchemaElementType,
                                parentType.Name,
            ...
            else if (!element.SubstitutionGroup.IsEmpty)
            {
                ...
                throw new InvalidOperationException(
                    Res.GetString(
                        Res.XmlInvalidSubstitutionGroupUse,
                        parentType.Name,
                        parentType.Namespace
                    )
                );
        ...
        )
        {
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                if (item is XmlSchemaAttributeGroup)
                {
                    throw new InvalidOperationException(
                        Res.GetString(
                            Res.XmlSoapInvalidAttributeUse,
                            type.Name,
                            type.QualifiedName.Namespace
                        )
                    );
            ...
            else if (type.ContentModel != null && type.ContentModel is XmlSchemaComplexContent)
            {
                ...
                if (model.Content is XmlSchemaComplexContentExtension)
                {
                    ...
                    {
                        ImportGroup(
                            ((XmlSchemaComplexContentExtension)model.Content).Particle,
                            members,
                            typeNs
                        );
                    }
                ...
                else if (model.Content is XmlSchemaComplexContentRestriction)
                {
                    ...
                    {
                        ImportGroup(
                            ((XmlSchemaComplexContentRestriction)model.Content).Particle,
                            members,
                            typeNs
                        );
                    }
        ...
        void ImportGroup(XmlSchemaParticle group, CodeIdentifiers members, string ns)
        {
            if (group is XmlSchemaChoice)
            {
                ...
                throw new InvalidOperationException(
                    Res.GetString(Res.XmlSoapInvalidChoice, parentType.Name, parentType.Namespace)
                );
            }
            else
        ...
        void ImportGroupMembers(XmlSchemaParticle particle, CodeIdentifiers members, string ns)
        {
            ...
            if (particle is XmlSchemaGroupRef)
            {
                throw new InvalidOperationException(
                    Res.GetString(
                        Res.XmlSoapUnsupportedGroupRef,
                        parentType.Name,
                        parentType.Namespace
                    )
                );
            ...
            else if (particle is XmlSchemaGroupBase)
            {
                ...
                if (group.IsMultipleOccurrence)
                    throw new InvalidOperationException(
                        Res.GetString(
                            Res.XmlSoapUnsupportedGroupRepeat,
                            parentType.Name,
                            parentType.Namespace
                        )
                    );
                for (int i = 0; i < group.Items.Count; i++)
                {
                    ...
                    if (item is XmlSchemaGroupBase || item is XmlSchemaGroupRef)
                        throw new InvalidOperationException(
                            Res.GetString(
                                Res.XmlSoapUnsupportedGroupNested,
                                parentType.Name,
                                parentType.Namespace
                            )
                        );
                        ...
                        throw new InvalidOperationException(
                            Res.GetString(
                                Res.XmlSoapUnsupportedGroupAny,
                                parentType.Name,
                                parentType.Namespace
                            )
                        );
        ...
        ArrayMapping ImportArrayMapping(XmlSchemaType type, string ns)
        {
            ...
            if (
                !(
                    type.DerivedFrom.Name == Soap.Array
                    && type.DerivedFrom.Namespace == Soap.Encoding
                )
            )
                return null;
            ...
            for (int i = 0; i < restriction.Attributes.Count; i++)
            {
                ...
                if (
                    attribute != null
                    && attribute.RefName.Name == Soap.ArrayType
                    && attribute.RefName.Namespace == Soap.Encoding
                )
                {
                    ...
                    {
                        string dims;
                        XmlQualifiedName typeName = TypeScope.ParseWsdlArrayType(
                            arrayType,
                            out dims,
                            attribute
                        );
