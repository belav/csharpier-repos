...
namespace System.Xml.Serialization.Advanced
{
            ...
            if (Names[name] != null)
            {
                if (Names[name].GetType() != extension.GetType())
                {
                    throw new InvalidOperationException(
                        Res.GetString(Res.XmlConfigurationDuplicateExtension, name)
                    );
                }
                return -1;
            }
    ...
    internal class MappedTypeDesc
    {
        ...
        internal CodeTypeDeclaration ExportTypeDefinition(
            ...
            foreach (CodeTypeDeclaration type in code.Types)
            {
                if (clrType == type.Name)
                {
                    if (codeClass != null)
                        throw new InvalidOperationException(
                            Res.GetString(
                                Res.XmlExtensionDuplicateDefinition,
