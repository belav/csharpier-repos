...
namespace System.Xml.Serialization
{
    ...
    public abstract class CodeExporter
    {
        ...
        internal static CodeAttributeDeclaration FindAttributeDeclaration(
            Type type,
            ...
            for (int i = 0; i < attributes.Length; i++)
            {
                if (attributes[i] is AssemblyInformationalVersionAttribute)
                {
                    AssemblyInformationalVersionAttribute version =
                        (AssemblyInformationalVersionAttribute)attributes[i];
        ...
        internal static void AddWarningComment(CodeCommentStatementCollection comments, string text)
        {
            ...
            comments.Add(
                new CodeCommentStatement(Res.GetString(Res.XmlCodegenWarningDetails, text), false)
            );
        }
        ...
        internal void ExportRoot(StructMapping mapping, Type includeType)
        {
            if (!rootExported)
            {
                ...
                {
                    if (
                        !derived.ReferencedByElement
                        && derived.IncludeInSchema
                ...
                foreach (TypeMapping m in Scope.TypeMappings)
                {
                    if (m is ArrayMapping)
                    {
                        ...
                        if (
                            ShouldInclude(arrayMapping)
                            && !typesIncluded.Contains(arrayMapping.TypeDesc.FullName)
                        )
                        {
                            CodeAttributeDeclaration include = new CodeAttributeDeclaration(
                                includeType.FullName
        ...
        internal CodeTypeDeclaration ExportEnum(EnumMapping mapping, Type type)
        {
            ...
            {
                // Add [FlagsAttribute]
                CodeAttributeDeclaration flags = new CodeAttributeDeclaration(
        ...
        )
        {
            ...
            {
                attribute.Arguments.Add(
                    new CodeAttributeArgument("Namespace", new CodePrimitiveExpression(ns))
                );
            }
            if (!includeInSchema)
            {
        ...
        )
        {
            ...
            )
            {
                CodeAttributeDeclaration attribute = new CodeAttributeDeclaration(type.FullName);
                attribute.Arguments.Add(
                    new CodeAttributeArgument(new CodeTypeOfExpression(derived.TypeDesc.FullName))
                );
                metadata.Add(attribute);
        ...
        internal static void ExportConstant(
        ...
        internal CodeMemberProperty CreatePropertyDeclaration(
        ...
        )
        {
            ...
            prop.Name = name;
            prop.Attributes =
                (prop.Attributes & ~MemberAttributes.AccessMask) | MemberAttributes.Public;

            //add get
            CodeMethodReturnStatement ret = new CodeMethodReturnStatement();
            ret.Expression = new CodeFieldReferenceExpression(
