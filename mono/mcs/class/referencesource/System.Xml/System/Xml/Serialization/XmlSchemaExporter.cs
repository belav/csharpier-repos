                ...
                foreach (MemberMapping member in mapping.Members)
                {
                    if (member.Attribute != null)
                        throw new InvalidOperationException(
                            Res.GetString(Res.XmlBareAttributeMember, member.Attribute.Name)
                        );
                    else if (member.Text != null)
                        throw new InvalidOperationException(
                            Res.GetString(Res.XmlBareTextMember, member.Text.Name)
                        );
                    else if (member.Elements == null || member.Elements.Length == 0)
                        continue;

                    if (
                        member.TypeDesc.IsArrayLike && !(member.Elements[0].Mapping is ArrayMapping)
                    )
                        throw new InvalidOperationException(
                            Res.GetString(Res.XmlIllegalArrayElement, member.Elements[0].Name)
                        );
...
#pragma warning restore 429
            ...
            switch (mapping.TypeDesc.Kind)
            {
                ...
                case TypeKind.Serializable:
                {
                    ...
                    else if (serializableMapping.Schema != null)
                    {
                        ...
                        {
                            throw new InvalidOperationException(
                                Res.GetString(Res.XmlDuplicateNamespace, anyNs)
                            );
                        }
                        if (element != null)
                            element.SchemaType = type;
        ...
        internal static string ExportDefaultValue(TypeMapping mapping, object value)
        {
            ...
            throw new InvalidOperationException(
                Res.GetString(Res.XmlInvalidDefaultValue, value.ToString(), pm.TypeDesc.Name)
            );
        }
        ...
        )
        {
            ...
            if (type == null)
            {
                if (!mapping.IncludeInSchema)
                    throw new InvalidOperationException(
                        Res.GetString(Res.XmlCannotIncludeInSchema, mapping.TypeDesc.Name)
                    );
        ...
        )
        {
            ...
            for (int i = 0; i < members.Length; i++)
            {
                ...
                if (member.Text != null)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            Res.GetString(Res.XmlIllegalMultipleText, name)
                        );
                    }
