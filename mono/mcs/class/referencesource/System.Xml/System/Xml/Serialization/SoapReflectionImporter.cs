...
namespace System.Xml.Serialization
{
    ...
    public class SoapReflectionImporter
    {
        ...
        )
        {
            ...
            element.Mapping = ImportMembersMapping(
                members,
                ns,
                hasWrapperElement,
                writeAccessors,
            ...
            switch (model.TypeDesc.Kind)
            {
                ...
                case TypeKind.Struct:
                    if (model.TypeDesc.IsOptionalValue)
                    {
                        ...
                        if (
                            baseAttributes.SoapType != null
                            && baseAttributes.SoapType.Namespace != null
                        )
                        ...
                        if (baseMapping == null)
                            baseMapping = ImportTypeMapping(
                                modelScope.GetTypeModel(baseTypeDesc.Type),
                                dataType,
                                limiter
                            );
                ...
                default:
                    throw new NotSupportedException(
                        Res.GetString(Res.XmlUnsupportedSoapTypeKind, model.TypeDesc.FullName)
                    );
            }
        }
        ...
        )
        {
            ...
            if (model.TypeDesc.BaseTypeDesc != null)
            {
                StructMapping baseMapping = ImportStructLikeMapping(
                    (StructModel)modelScope.GetTypeModel(model.Type.BaseType, false),
                    limiter
                );
            ...
            foreach (MemberInfo memberInfo in model.GetMemberInfos())
            {
                ...
                )
                {
                    if (model.TypeDesc.IsValueType)
                        throw new NotSupportedException(
                            Res.GetString(Res.XmlRpcRefsInValueType, model.TypeDesc.FullName)
                        );
                    if (member.TypeDesc.IsValueType)
                        throw new NotSupportedException(
                            Res.GetString(Res.XmlRpcNestedValueType, member.TypeDesc.FullName)
                        );
                }
        ...
        ArrayMapping ImportArrayLikeMapping(ArrayModel model, RecursionLimiter limiter)
        {
            ...
            if (
                itemTypeMapping.TypeDesc.IsValueType
                && !itemTypeMapping.TypeDesc.IsPrimitive
                && !itemTypeMapping.TypeDesc.IsEnum
            )
                throw new NotSupportedException(
                    Res.GetString(Res.XmlRpcArrayOfValueTypes, model.TypeDesc.FullName)
                );
        ...
        void SetArrayMappingType(ArrayMapping mapping)
        {
            ...
            {
                throw new InvalidOperationException(
                    Res.GetString(Res.XmlInvalidSoapArray, mapping.TypeDesc.FullName)
                );
            }
            ...
            for (int i = 0; i < mappings.Length; i++)
            {
                try
                {
                    ...
                    MemberMapping mapping = ImportMemberMapping(
                        ...
                        hasWrapperElement ? XmlSchemaForm.Unqualified : XmlSchemaForm.Qualified,
                        limiter
                    );
        ...
        )
        {
            ...
            bool checkSpecified =
                XmlReflectionImporter.FindSpecifiedMember(
        ...
        )
        {
            ...
            if ((flags & SoapAttributeFlags.Attribute) == SoapAttributeFlags.Attribute)
            {
                ...
                if ((flags & SoapAttributeFlags.Attribute) != flags)
                    throw new InvalidOperationException(
                        Res.GetString(Res.XmlInvalidElementAttribute)
                    );
                ...
                attribute.Mapping = ImportTypeMapping(
                    ...
                    (a.SoapAttribute == null ? String.Empty : a.SoapAttribute.DataType),
                    limiter
                );
            ...
            else
            {
                ...
                element.Mapping = ImportTypeMapping(
                    modelScope.GetTypeModel(accessorType),
                    (a.SoapElement == null ? String.Empty : a.SoapElement.DataType),
                    limiter
                );
        ...
        object GetDefaultValue(TypeDesc fieldTypeDesc, SoapAttributes a)
        {
            ...
            if (fieldTypeDesc.Kind == TypeKind.Enum)
            {
                if (fieldTypeDesc != typeScope.GetTypeDesc(a.SoapDefaultValue.GetType()))
                    throw new InvalidOperationException(
                        Res.GetString(
                            Res.XmlInvalidDefaultEnumValue,
                            a.SoapDefaultValue.GetType().FullName,
                            fieldTypeDesc.FullName
                        )
                    );
                string strValue = Enum.Format(a.SoapDefaultValue.GetType(), a.SoapDefaultValue, "G")
                    .Replace(",", " ");
                string numValue = Enum.Format(
