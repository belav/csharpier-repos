...
namespace System.Xml.Serialization
{
    ...
    public class XmlSchemaImporter : SchemaImporter
    {
        ...
        {
            TypeMapping typeMapping = ImportType(
                typeName,
                typeof(TypeMapping),
                baseType,
        ...
        public XmlMembersMapping ImportMembersMapping(XmlQualifiedName name)
        {
            return new XmlMembersMapping(
                Scope,
                ImportElement(name, typeof(MembersMapping), null),
                XmlMappingAccess.Read | XmlMappingAccess.Write
            );
        }
        ...
        )
        {
            if (!element.RefName.IsEmpty)
            {
                ...
                ElementAccessor topAccessor = ImportElement(
                    element.RefName,
                    desiredMappingType,
                    baseType
                );
            ...
            if (element.Name.Length == 0)
            {
                ...
                throw new InvalidOperationException(
                    Res.GetString(Res.XmlElementHasNoName, parentType.Name, parentType.Namespace)
                );
            }
            ...
            TypeMapping mapping = ImportElementType(
                element,
                identifier,
                desiredMappingType,
                baseType,
        ...
        )
        {
            ...
            else if (element.SchemaType != null)
            {
                if (element.SchemaType is XmlSchemaComplexType)
                    mapping = ImportType(
                        (XmlSchemaComplexType)element.SchemaType,
                        ns,
                        identifier,
                        desiredMappingType,
                        baseType,
                    ...
                    mapping = ImportDataType(
                        (XmlSchemaSimpleType)element.SchemaType,
                        ns,
                        identifier,
                        baseType,
                        TypeFlags.CanBeElementValue
                ...
                mapping = ImportElementType(
                    FindElement(element.SubstitutionGroup),
                    identifier,
                    desiredMappingType,
                    baseType,
                    ns
        ...
        )
        {
            ...
            else if (context is XmlSchemaAny)
            {
                typeName = FindExtendedAnyElement(
                    (XmlSchemaAny)context,
                    ((flags & TypeFlags.CanBeTextValue) != 0),
                    compileUnit,
                    mainNamespace,
                    out typeOwner
                );
            ...
            if (typeName != null && typeName.Length > 0)
            {
                ...
                catch (ArgumentException)
                {
                    if (qname.IsEmpty)
                    {
                        throw new InvalidOperationException(
                            Res.GetString(
                                Res.XmlImporterExtensionBadLocalTypeName,
                                typeOwner.GetType().FullName,
                                typeName
                            )
                        );
                    }
                    else
                    {
                        throw new InvalidOperationException(
                            Res.GetString(
                                Res.XmlImporterExtensionBadTypeName,
                                typeOwner.GetType().FullName,
                                qname.Name,
                                qname.Namespace,
                                typeName
            ...
            foreach (XmlSchema schema in Schemas)
            {
                ...
                foreach (object item in schema.SchemaTypes.Values)
                {
                    if (item is XmlSchemaType)
                    {
                        ...
                        if (
                            type.DerivedFrom == baseName
                            && TypesInUse[type.Name, schema.TargetNamespace] == null
                        )
                        {
                            ImportType(
                                type.QualifiedName,
        ...
        )
        {
            if (type.Redefined != null)
            {
                ...
                throw new NotSupportedException(
                    Res.GetString(Res.XmlUnsupportedRedefine, type.Name, typeNs)
                );
            }
            if (desiredMappingType == typeof(TypeMapping))
            {
                ...
                throw new ArgumentException(
                    Res.GetString(Res.XmlInternalError),
                    "desiredMappingType"
                );
        }
        ...
        MembersMapping ImportMembersType(XmlSchemaType type, string typeNs, string identifier)
        {
            if (!type.DerivedFrom.IsEmpty)
                throw new InvalidOperationException(Res.GetString(Res.XmlMembersDeriveError));
            CodeIdentifiers memberScope = new CodeIdentifiers();
            memberScope.UseCamelCasing = true;
            bool needExplicitOrder = false;
            MemberMapping[] members = ImportTypeMembers(
                type,
                typeNs,
                identifier,
                memberScope,
        ...
        )
        {
            ...
            string typeName =
                type.Name == null || type.Name.Length == 0
                    ? GenerateUniqueTypeName(identifier, typeNs)
                    : GenerateUniqueTypeName(identifier);
            structMapping.TypeDesc = new TypeDesc(
                typeName,
                typeName,
                TypeKind.Struct,
                baseTypeDesc,
            ...
            bool needExplicitOrder = false;
            structMapping.Members = ImportTypeMembers(
                type,
                typeNs,
                identifier,
                members,
            ...
            for (int i = 0; i < structMapping.Members.Length; i++)
            {
                ...
                if (baseMember != null && baseMember.TypeDesc != structMapping.Members[i].TypeDesc)
                    throw new InvalidOperationException(
                        Res.GetString(
                            Res.XmlIllegalOverride,
                            type.Name,
                            baseMember.Name,
                            baseMember.TypeDesc.FullName,
        ...
        StructMapping ImportStructDataType(
        ...
        )
        {
            ...
            if (duplicateTypes)
            {
                ...
                member.ChoiceIdentifier.Mapping = ImportEnumeratedChoice(
                    member.Elements,
                    ns,
                    member.Name + "ChoiceType"
                );
                member.ChoiceIdentifier.MemberIds = new string[member.Elements.Length];
                ConstantMapping[] constants = (
                    (EnumMapping)member.ChoiceIdentifier.Mapping
                ).Constants;
                for (int i = 0; i < member.Elements.Length; i++)
                {
                ...
                for (int i = 0; i < group.Items.Count; i++)
                {
                    ...
                    else if (item is XmlSchemaAny)
                    {
                        if (GenerateOrder)
                        {
                            AddScopeElements(
                                choiceElements,
                                ImportAny((XmlSchemaAny)item, true, ns),
                    ...
                    else if (item is XmlSchemaElement)
                    {
                        ...
                        if (headElement != null)
                        {
                            ...
                            {
                                if (elements[j].IsMultipleOccurrence)
                                    groupRepeats = true;
                                AddScopeElement(
                                    choiceElements,
                                    ImportElement(
                                        ...
                                        typeof(TypeMapping),
                                        null,
                                        elements[j].QualifiedName.Namespace,
                                        true
                                    ),
                                    ref duplicateElements,
                                    allowDuplicates
        ...
        )
        {
            ...
            if (scopeElement != null)
            {
                if (!allowDuplicates)
                {
                    throw new InvalidOperationException(
                        Res.GetString(
                            Res.XmlDuplicateElementInScope,
                            element.Name,
                            element.Namespace
                        )
                    );
                ...
                if (scopeElement.Mapping.TypeDesc != element.Mapping.TypeDesc)
                {
                    throw new InvalidOperationException(
                        Res.GetString(
                            Res.XmlDuplicateElementInScope1,
                            element.Name,
                            element.Namespace
                        )
                    );
            ...
            if (particle is XmlSchemaGroupRef)
            {
                ...
                if (!refGroup.RefName.IsEmpty)
                {
                    ...
                    ImportGroupMembers(
                        ...
                        elementsScope,
                        refGroup.RefName.Namespace,
                        groupRepeats | refGroup.IsMultipleOccurrence,
                        ref mixed,
                        ref needExplicitOrder,
                        allowDuplicates,
                        allowUnboundedElements
            ...
            foreach (XmlSchema schema in Schemas.SchemaSet.Schemas())
            {
                for (int j = 0; j < schema.Items.Count; j++)
                {
                    ...
                    if (item is XmlSchemaElement)
                    {
                        ...
                        if (
                            !equivalentElement.IsAbstract
                            && equivalentElement.SubstitutionGroup.Namespace
        ...
        ElementAccessor[] ImportAny(XmlSchemaAny any, bool makeElement, string targetNamespace)
        {
            ...
            mapping.TypeDesc = Scope.GetTypeDesc(
            ...
            if (GenerateOrder && any.Namespace != null)
            {
                ...
                if (list.Type == NamespaceList.ListType.Set)
                {
                    ...
                    ElementAccessor[] accessors = new ElementAccessor[
        ...
        )
        {
            ...
            if (items.Attributes != null && items.Attributes.Count > 0)
                return null;
            if (items.AnyAttribute != null)
                return null;
            if (items.Particle == null)
            ...
            if (item is XmlSchemaChoice)
            {
                ...
                if (!choice.IsMultipleOccurrence)
                    return null;
                bool needExplicitOrder = false;
                MemberMapping choiceMember = ImportChoiceGroup(
                    choice,
                    identifier,
                    null,
                    null,
            ...
            else if (item is XmlSchemaAll || item is XmlSchemaSequence)
            {
                ...
                ElementAccessor itemAccessor = ImportElement(
                    itemElement,
                    identifier,
                    typeof(TypeMapping),
                    null,
        ...
        bool IsCyclicReferencedType(XmlSchemaElement element, List<string> identifiers)
        {
            if (!element.RefName.IsEmpty)
            {
                ...
                string refElementIdentifier = CodeIdentifier.MakeValid(
                    Accessor.UnescapeName(refElement.Name)
                );
                ...
                if (refType is XmlSchemaComplexType)
                {
                    ...
                    if (
                        (items.Particle is XmlSchemaSequence || items.Particle is XmlSchemaAll)
                        && items.Particle.Items.Count == 1
        ...
        )
        {
            ...
            if (items.Particle == null)
                return null;
            if (!(items.Particle is XmlSchemaAll || items.Particle is XmlSchemaSequence))
                return null;
            if (items.Attributes != null && items.Attributes.Count > 0)
                return null;
        ...
        )
        {
            ...
            members.Add(member.Name, member);
            member.TypeDesc = accessor.IsList
        ...
        void ImportAnyAttributeMember(
            XmlSchemaAnyAttribute any,
            CodeIdentifiers members,
            CodeIdentifiers membersScope
        )
        {
        ...
        )
        {
            ...
            member.Name = CodeIdentifier.MakeValid(
                xmlnsMemberName == null ? "Namespaces" : xmlnsMemberName
            );
            member.Name = membersScope.AddUnique(member.Name, member);
            members.Add(member.Name, member);
        ...
        )
        {
            ...
            if (!attribute.RefName.IsEmpty)
            {
                if (attribute.RefName.Namespace == XmlReservedNs.NsXml)
                    ...
                    return ImportAttribute(
                        FindAttribute(attribute.RefName),
                        identifier,
            ...
            if (attribute.Name.Length == 0)
                throw new InvalidOperationException(Res.GetString(Res.XmlAttributeHasNoName));
            if (identifier.Length == 0)
        ...
        )
        {
            ...
            if (dataType.Content is XmlSchemaSimpleTypeRestriction)
            {
                ...
                foreach (object o in restriction.Facets)
                {
                    ...
                    {
                        return ImportEnumeratedDataType(
                            dataType,
                            typeNs,
                            identifier,
                            flags,
                ...
                else
                {
                    AddReference(
                        ...
                        TypesInUse,
                        Res.XmlCircularTypeReference
                    );
                    mapping = ImportDataType(
                        FindDataType(restriction.BaseTypeName, flags),
                ...
                dataType.Content is XmlSchemaSimpleTypeList
                || dataType.Content is XmlSchemaSimpleTypeUnion
            )
            {
        ...
        TypeMapping ImportEnumeratedDataType(
            XmlSchemaSimpleType dataType,
            string typeNs,
            string identifier,
            TypeFlags flags,
        ...
        )
        {
            ...
            if (content is XmlSchemaSimpleTypeRestriction)
            {
                ...
                for (int i = 0; i < restriction.Facets.Count; i++)
                {
                    ...
                    {
                        XmlCustomFormatter.ToDefaultValue(
                            enumeration.Value,
        ...
        PrimitiveMapping ImportNonXsdPrimitiveDataType(
            XmlSchemaSimpleType dataType,
            string ns,
            TypeFlags flags
        )
        ...
        XmlSchemaAttributeGroup FindAttributeGroup(XmlQualifiedName name)
        {
            ...
            if (group == null)
                throw new InvalidOperationException(
                    Res.GetString(Res.XmlMissingAttributeGroup, name.Name)
                );
        ...
        XmlSchemaSimpleType FindDataType(XmlQualifiedName name, TypeFlags flags)
        {
            ...
            else
            {
                ...
                {
                    throw new InvalidOperationException(
                        Res.GetString(Res.XmlInvalidEncoding, name.ToString())
                    );
                }
                else
                {
                    throw new InvalidOperationException(
                        Res.GetString(Res.XmlMissingDataType, name.ToString())
                    );
                }
            }
        }
        ...
        XmlSchemaElement FindElement(XmlQualifiedName name)
        {
            ...
            if (element == null)
                throw new InvalidOperationException(
                    Res.GetString(Res.XmlMissingElement, name.ToString())
                );
            return element;
        }
        ...
        XmlSchemaAttribute FindAttribute(XmlQualifiedName name)
        {
            ...
            if (attribute == null)
                throw new InvalidOperationException(
                    Res.GetString(Res.XmlMissingAttribute, name.Name)
                );
        ...
        )
        {
            ...
            {
                string typeName = ex.ImportAnyElement(
                    any,
                    mixed,
                    Schemas,
                    this,
        ...
        )
        {
            ...
            foreach (SchemaImporterExtension ex in Extensions)
            {
                string typeName = ex.ImportSchemaType(
                    name,
                    ns,
                    context,
                    Schemas,
