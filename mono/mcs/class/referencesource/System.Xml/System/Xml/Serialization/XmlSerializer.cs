                ...
                lock (cache)
                {
                    ...
                    if (tempAssembly == null)
                    {
                        ...
                        if (assembly == null)
                        {
                            ...
                            tempAssembly = GenerateTempAssembly(
                                this.mapping,
                                type,
                                defaultNamespace
                            );
                        ...
                        else
                        {
                            ...
                            this.mapping = XmlReflectionImporter.GetTopLevelMapping(
                                type,
                                defaultNamespace
                            );
                            tempAssembly = new TempAssembly(
                                new XmlMapping[] { this.mapping },
                                assembly,
                                contract
                            );
                        }
...
#pragma warning restore 618
        ...
        )
        {
            ...
            for (int i = 0; i < types.Length; i++)
            {
                ...
                {
                    throw new InvalidOperationException(
                        Res.GetString(Res.XmlPregenTypeDynamic, type.FullName)
                    );
                }
                if (assembly == null)
                ...
                else if (type.Assembly != assembly)
                {
                    throw new ArgumentException(
                        Res.GetString(Res.XmlPregenOrphanType, type.FullName, assembly.Location),
                        "types"
                    );
                }
            }
            return TempAssembly.GenerateAssembly(
                mappings,
                types,
                null,
                null,
                XmlSerializerCompilerParameters.Create(
                    parameters, /* needTempDirAccess = */
                    true
                ),
                assembly,
                new Hashtable()
        ...
        {
            add { events.OnUnknownNode += value; }
        ...
        {
            add { events.OnUnknownAttribute += value; }
            remove { events.OnUnknownAttribute -= value; }
        }

        /// <include file='doc\XmlSerializer.uex' path='docs/doc[@for="XmlSerializer.UnknownElement"]/*' />
        public event XmlElementEventHandler UnknownElement
        {
            add { events.OnUnknownElement += value; }
        ...
        public event UnreferencedObjectEventHandler UnreferencedObject
        {
            ...
            remove { events.OnUnreferencedObject -= value; }
        }
        ...
        void SerializePrimitive(XmlWriter xmlWriter, object o, XmlSerializerNamespaces namespaces)
        {
            ...
            switch (Type.GetTypeCode(primitiveType))
            {
                ...
                default:
                    ...
                    {
                        throw new InvalidOperationException(
                            Res.GetString(Res.XmlUnxpectedType, primitiveType.FullName)
                        );
                    }
                    break;
            }
