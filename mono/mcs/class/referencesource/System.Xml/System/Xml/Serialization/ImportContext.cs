    ...
    internal class SchemaGraph
    {
        ...
        internal void Depends(XmlSchemaObject item, ArrayList refs)
        {
            ...
            if (typeof(XmlSchemaType).IsAssignableFrom(t))
            {
                ...
                if (item is XmlSchemaComplexType)
                {
                    ...
                    if (particle is XmlSchemaGroupRef)
                    {
                        ...
                        particle = (
                            (XmlSchemaGroup)
                                schemas.Find(refGroup.RefName, typeof(XmlSchemaGroup), false)
                        ).Particle;
                    }
                        ...
                        if (memberTypes != null)
                        {
                            for (int i = 0; i < memberTypes.Length; i++)
                            {
                                XmlSchemaType type = (XmlSchemaType)
                                    schemas.Find(memberTypes[i], typeof(XmlSchemaType), false);
                                AddRef(refs, type);
                            }
                        }
                    }
            ...
            else if (t == typeof(XmlSchemaElement))
            {
                ...
                if (!el.SubstitutionGroup.IsEmpty)
                {
                    if (el.SubstitutionGroup.Namespace != XmlSchema.Namespace)
                    {
                        XmlSchemaElement head = (XmlSchemaElement)
                            schemas.Find(el.SubstitutionGroup, typeof(XmlSchemaElement), false);
                        AddRef(refs, head);
                    }
                }
                ...
                {
                    el = (XmlSchemaElement)
                        schemas.Find(el.RefName, typeof(XmlSchemaElement), false);
                    AddRef(refs, el);
                }
                else if (!el.SchemaTypeName.IsEmpty)
                {
                    XmlSchemaType type = (XmlSchemaType)
                        schemas.Find(el.SchemaTypeName, typeof(XmlSchemaType), false);
                    AddRef(refs, type);
                }
                else
                {
            ...
            else if (t == typeof(XmlSchemaGroupRef))
            {
                XmlSchemaGroup group = (XmlSchemaGroup)
                    schemas.Find(((XmlSchemaGroupRef)item).RefName, typeof(XmlSchemaGroup), false);
                AddRef(refs, group);
            }
            ...
            else if (t == typeof(XmlSchemaAttribute))
            {
                ...
                if (!at.RefName.IsEmpty)
                {
                    at = (XmlSchemaAttribute)
                        schemas.Find(at.RefName, typeof(XmlSchemaAttribute), false);
                    AddRef(refs, at);
                }
                else if (!at.SchemaTypeName.IsEmpty)
                {
                    XmlSchemaType type = (XmlSchemaType)
                        schemas.Find(at.SchemaTypeName, typeof(XmlSchemaType), false);
                    AddRef(refs, type);
                }
                else
                {
                ...
                if (attrs != null)
                {
                    for (int i = 0; i < attrs.Length; i++)
                    {
                        ...
                        )
                        {
                            ...
                            XmlSchemaType type = (XmlSchemaType)
                                schemas.Find(qname, typeof(XmlSchemaType), false);
                            AddRef(refs, type);
                        }
                    }
                }
