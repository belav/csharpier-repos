...
namespace System.Xml
{
    internal partial class XsdCachingReader : XmlReader, IXmlLineInfo
    {
        ...
        public override string Value
        {
            get
            {
                return returnOriginalStringValues
                    ? cachedNode.OriginalStringValue
                    : cachedNode.RawValue;
            }
        }
        ...
        {
            namespaceURI =
                (namespaceURI == null) ? string.Empty : coreReaderNameTable.Get(namespaceURI);
            ...
            for (int i = 0; i < attributeCount; i++)
            {
                ...
                if (
                    Ref.Equal(attribute.LocalName, name)
        ...
        public override bool MoveToAttribute(string name, string ns)
        {
            ...
            {
                attribute = attributeEvents[i];
                if (Ref.Equal(attribute.LocalName, name) && Ref.Equal(attribute.Namespace, ns))
                {
                    currentAttrIndex = i;
                    cachedNode = attributeEvents[i];
                    return true;
        ...
        public override bool MoveToElement()
        {
            if (
                ...
                || cachedNode.NodeType != XmlNodeType.Attribute
            )
            {
                return false;
            }
        ...
        internal void SwitchTextNodeAndEndElement(string textValue, string originalStringValue)
        {
            Debug.Assert(
                coreReader.NodeType == XmlNodeType.EndElement
                    || (coreReader.NodeType == XmlNodeType.Element && coreReader.IsEmptyElement)
            );
        ...
        {
            get { return cacheState == CachingReaderState.ReaderClosed && coreReader.EOF; }
        }
        ...
        {
            get { return coreReaderNameTable; }
        }
        ...
        {
            get { return cachedNode.LineNumber; }
        }

        int IXmlLineInfo.LinePosition
        {
            get { return cachedNode.LinePosition; }
        }
