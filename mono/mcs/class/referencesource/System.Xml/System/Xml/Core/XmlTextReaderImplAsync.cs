    ...
    internal partial class XmlTextReaderImpl : XmlReader, IXmlLineInfo, IXmlNamespaceResolver
    {
        private void CheckAsyncCall()
        {
            if (!useAsync)
            {
                throw new InvalidOperationException(
                    Res.GetString(Res.Xml_ReaderAsyncNotSetException)
                );
            }
        }
        ...
        private async Task FinishInitUriStringAsync()
        {
            ...
            try
            {
                ...
                {
                    await ProcessDtdFromParserContextAsync(laterInitParam.inputContext)
                        .ConfigureAwait(false);
                }
            }
            catch
        ...
        {
            // init ParsingState
            await InitTextReaderInputAsync(reportedBaseUri, laterInitParam.inputTextReader)
                .ConfigureAwait(false);
        ...
        public override async Task SkipAsync()
        {
            ...
            switch (curNode.type)
            {
                ...
                case XmlNodeType.Element:
                    ...
                    // skip content
                    while (
                        await outerReader.ReadAsync().ConfigureAwait(false) && index > initialDepth
                    )
                        ;
        ...
        private async Task<int> ReadContentAsBase64_AsyncHelper(
            Task<bool> task,
            byte[] buffer,
            int index,
            int count
        ...
        private async Task<int> ReadElementContentAsBase64Async_Helper(
            Task<bool> task,
            byte[] buffer,
            int index,
            int count
        ...
        )
        {
            ...
            await PushExternalEntityOrSubsetAsync(publicId, systemId, ps.baseUri, null)
                .ConfigureAwait(false);
...
#if !SILVERLIGHT
        private Task InitStreamInputAsync(
            Uri baseUri,
            string baseUriStr,
            Stream stream,
            Encoding encoding
        ...
        private async Task InitStreamInputAsync(
            Uri baseUri,
            string baseUriStr,
            Stream stream,
            byte[] bytes,
        ...
        private Task ParseEndElementAsync_Finish(
            int nameLen,
            ...
            while (true)
            {
                ...
                switch (parseEndElement_NextFunc)
                {
                    case ParseEndElementParseFunction.CheckEndTag:
                        task = ParseEndElementAsync_CheckEndTag(
                            nameLen,
                            startTagNode,
                            endTagLineInfo
                        );
        ...
        private async Task ParseEndElementAsync_Finish(
            Task task,
            int nameLen,
            NodeData startTagNode,
            LineInfo endTagLineInfo
        ...
        private Task ParseEndElementAsync_CheckEndTag(
            int nameLen,
            NodeData startTagNode,
            LineInfo endTagLineInfo
        )
        ...
        private Task<Tuple<int, int, int, bool>> parseText_dummyTask = Task.FromResult(
            new Tuple<int, int, int, bool>(0, 0, 0, false)
        );
        ...
        {
            Task<Tuple<int, int, int, bool>> task = ParseTextAsync(
                outOrChars,
                ps.chars,
                ps.charPos,
                0,
            ...
            while (true)
            {
                ...
                switch (parseText_NextFunction)
                {
                    ...
                    case ParseTextFunction.Entity:
                        task = ParseTextAsync_ParseEntity(
                            outOrChars,
                            chars,
                            pos,
                            rcount,
        ...
        private async Task<Tuple<int, int, int, bool>> ParseTextAsync_AsyncFunc(
            Task<Tuple<int, int, int, bool>> task
        )
        {
            while (true)
            {
            ...
            for (; ; )
            {
                ...
                switch (c)
                {
                    ...
                    case (char)0xD:
                        ...
                        {
                            lastParseTextState = new ParseTextState(
                                outOrChars,
                                chars,
                                pos,
                                rcount,
        ...
        private async Task<Tuple<int, int, int, bool>> ParseTextAsync_ParseEntity(
            int outOrChars,
            char[] chars,
            int pos,
            int rcount,
        ...
        private async Task<Tuple<int, int, int, bool>> ParseTextAsync_Surrogate(
            int outOrChars,
            char[] chars,
            int pos,
            int rcount,
        ...
        private async Task<Tuple<int, int, int, bool>> ParseTextAsync_ReadData(
            int outOrChars,
            char[] chars,
            int pos,
            int rcount,
        ...
        private Task<Tuple<int, int, int, bool>> ParseTextAsync_PartialValue(
            int pos,
            int rcount,
            int rpos,
            int orChars,
        ...
        )
        {
            ...
            return Task.FromResult(
                new Tuple<int, int, int, bool>(startPos, endPos, outOrChars, c == '<')
            );
        }
        ...
        private async Task<bool> ParseRootLevelWhitespaceAsync()
        {
            ...
            if (nodeType == XmlNodeType.None)
            {
                ...
                if (
                    ...
                    || await ZeroEndingStreamAsync(ps.charPos).ConfigureAwait(false)
                )
                {
                    return false;
                }
        ...
        private async Task<Tuple<int, EntityType>> HandleEntityReferenceAsync(
        ...
        private async Task<EntityType> HandleGeneralEntityReferenceAsync(
            string name,
            bool isInAttributeValue,
        ...
        private async Task ParseDtdAsync()
        {
            ...
            dtdInfo = await dtdParser
                .ParseInternalDtdAsync(new DtdParserProxy(this), true)
                .ConfigureAwait(false);
        ...
        private async Task<Tuple<EntityType, int>> ParseNumericCharRefAsync(
            bool expand,
            BufferBuilder internalSubsetBuilder
        )
        {
        ...
        private async Task PushExternalEntityOrSubsetAsync(
            string publicId,
            string systemId,
            Uri baseUri,
            string entityName
        ...
        private async Task<bool> OpenAndPushAsync(Uri uri)
        {
            ...
            if (xmlResolver.SupportsType(uri, typeof(TextReader)))
            {
                ...
                PushParsingState();
                await InitTextReaderInputAsync(uri.ToString(), uri, textReader)
                    .ConfigureAwait(false);
            }
            else
            {
