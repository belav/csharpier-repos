        ...
        public async Task<object> ReadTypedValueAsync()
        {
            ...
            switch (outerReader.NodeType)
            {
                ...
                case XmlNodeType.Element:
                    ...
                    XmlSchemaDatatype dtype =
                        (SchemaType is XmlSchemaDatatype)
                            ? (XmlSchemaDatatype)SchemaType
                            : ((XmlSchemaType)SchemaType).Datatype;
                    if (dtype != null)
                    {
                            ...
                            for (; ; )
                            {
                                if (!await outerReader.ReadAsync().ConfigureAwait(false))
                                {
                                    throw new InvalidOperationException(
                                        Res.GetString(Res.Xml_InvalidOperation)
                                    );
                                }
        ...
        private async Task ParseDtdFromParserContextAsync()
        {
            ...
            IDtdInfo dtdInfo = await dtdParser
                .ParseFreeFloatingDtdAsync(
                    ...
                    parserContext.DocTypeName,
                    parserContext.PublicId,
                    parserContext.SystemId,
