    ...
    internal partial class XmlTextReaderImpl : XmlReader, IXmlLineInfo, IXmlNamespaceResolver
    {
            ...
            )
        {
            ...
            else
            {
                ...
                InitStreamInput(
                    GetTempResolver().ResolveUri(null, context.BaseURI),
                    xmlFragment,
                    enc
                );
            }
        ...
        {
            InitStringInput(
                (context == null) ? string.Empty : context.BaseURI,
                Encoding.Unicode,
                string.Concat("<?xml ", xmlFragment, "?>")
            );
        ...
        private void FinishInitStream()
        {
            ...
            InitStreamInput(
                laterInitParam.inputbaseUri,
                reportedBaseUri,
                laterInitParam.inputStream,
                laterInitParam.inputBytes,
                laterInitParam.inputByteCount,
                enc
            );
            ...
            : this(settings.GetXmlResolver(), settings, context)
        {
            ...
            if (context != null)
            {
                Debug.Assert(
                    ...
                    "BaseURI can come either from XmlParserContext or from the constructor argument, not from both"
                );
        ...
        {
            get { return curNode.type == XmlNodeType.Attribute ? curNode.quoteChar : '"'; }
        }
#endif
        ...
        {
            get { return xmlContext.xmlSpace; }
        }

        // Returns the current xml:lang scope.</para>
        public override string XmlLang
        {
            get { return xmlContext.xmlLang; }
        }
        ...
        {
            get { return parsingFunction == ParsingFunction.Eof; }
        }
        ...
        public override string GetAttribute(string localName, string namespaceURI)
        {
            ...
            {
                if (
                    Ref.Equal(nodes[i].localName, localName) && Ref.Equal(nodes[i].ns, namespaceURI)
                )
                {
...
#else
                if (
                    curNode.nextAttrValueChunk == null
                    || entityHandling == EntityHandling.ExpandEntities
                )
                {
                    NodeData simpleValueNode = AddNode(index + attrCount + 1, curNode.depth + 1);
        ...
        public override void ResolveEntity()
        {
            ...
            {
                switch (
                    HandleGeneralEntityReference(curNode.localName, true, true, curNode.LinePos)
                )
                {
            ...
            {
                switch (
                    HandleGeneralEntityReference(curNode.localName, false, true, curNode.LinePos)
                )
                {
                    case EntityType.ExpandedInAttribute:
                    case EntityType.Expanded:
        ...
        {
            if (
                outerReader.NodeType == XmlNodeType.EntityReference
                && parsingFunction == ParsingFunction.AfterResolveEntityInContent
            )
            {
                if (!outerReader.Read())
        ...
        public override int ReadValueChunk(char[] buffer, int index, int count)
        {
            ...
            {
                throw new InvalidOperationException(
                    Res.GetString(Res.Xml_InvalidReadValueChunk, curNode.type)
                );
            }
            // check arguments
            if (buffer == null)
        ...
        {
            get { return curNode.LineNo; }
        }

        // Returns the line position of the current node
        public int LinePosition
        {
            get { return curNode.LinePos; }
        }
            ...
            set
            {
                ...
                if (value)
                {
                    if (namespaceManager is NoNamespaceManager)
                    {
                        if (
                            fragment
                            && fragmentParserContext != null
                            && fragmentParserContext.NamespaceManager != null
                        )
                        {
        ...
        internal bool Normalization
        {
            get
            {
                Debug.Assert(
                    v1Compat,
                    "XmlTextReaderImpl.Normalization property cannot be accessed on reader created via XmlReader.Create."
                );
                return normalize;
            }
            set
        ...
        internal WhitespaceHandling WhitespaceHandling
        {
            get
            {
                Debug.Assert(
                    v1Compat,
                    "XmlTextReaderImpl.WhitespaceHandling property cannot be accessed on reader created via XmlReader.Create."
                );
            ...
            set
            {
                Debug.Assert(
                    v1Compat,
                    "XmlTextReaderImpl.WhitespaceHandling property cannot be changed on reader created via XmlReader.Create."
                );
                if (readState == ReadState.Closed)
                {
                    throw new InvalidOperationException(Res.GetString(Res.Xml_InvalidOperation));
        ...
        internal DtdProcessing DtdProcessing
        {
            get
            {
                Debug.Assert(
                    v1Compat,
                    "XmlTextReaderImpl.DtdProcessing property cannot be accessed on reader created via XmlReader.Create."
                );
            ...
            set
            {
                Debug.Assert(
                    v1Compat,
                    "XmlTextReaderImpl.DtdProcessing property cannot be changed on reader created via XmlReader.Create."
                );
        ...
        internal void ResetState()
        {
            Debug.Assert(
                v1Compat,
                "XmlTextReaderImpl.ResetState cannot be called on reader created via XmlReader.Create."
            );
        ...
        internal TextReader GetRemainder()
        {
            Debug.Assert(
                v1Compat,
                "XmlTextReaderImpl.GetRemainder cannot be called on reader created via XmlReader.Create."
            );
                ...
                case ParsingFunction.InIncrementalRead:
                    if (!InEntity)
                    {
                        stringBuilder.Append(
                            ps.chars,
                            incReadLeftStartPos,
                            incReadLeftEndPos - incReadLeftStartPos
                        );
                    }
                    break;
            }
        ...
        internal int ReadChars(char[] buffer, int index, int count)
        {
            Debug.Assert(
                v1Compat,
                "XmlTextReaderImpl.ReadChars cannot be called on reader created via XmlReader.Create."
            );
        ...
        internal int ReadBase64(byte[] array, int offset, int len)
        {
            Debug.Assert(
                v1Compat,
                "XmlTextReaderImpl.ReadBase64 cannot be called on reader created via XmlReader.Create."
            );
            Debug.Assert(outerReader is XmlTextReader);
        ...
        internal int ReadBinHex(byte[] array, int offset, int len)
        {
            Debug.Assert(
                v1Compat,
                "XmlTextReaderImpl.ReadBinHex cannot be called on reader created via XmlReader.Create."
            );
            Debug.Assert(outerReader is XmlTextReader);
        ...
        {
            get { return ps.charsUsed; }
        }

        internal int DtdParserProxy_CurrentPosition
        {
            get { return ps.charPos; }
            set
            {
        ...
        {
            get { return parsingStatesStackTop + 1; }
        }

        internal bool DtdParserProxy_IsEntityEolNormalized
        {
            get { return ps.eolNormalized; }
        }

#if !SILVERLIGHT
        internal IValidationEventHandling DtdParserProxy_ValidationEventHandling
        {
            get { return validationEventHandling; }
            set { validationEventHandling = value; }
        }
#endif
        ...
        {
            get { return ps.LineNo; }
        }

        internal int DtdParserProxy_LineStartPosition
        {
            get { return ps.lineStartPos; }
        }
        ...
        internal int DtdParserProxy_ParseNamedCharRef(
            bool expand,
            BufferBuilder internalSubsetBuilder
        )
        {
        ...
        internal void DtdParserProxy_OnSystemId(
            string systemId,
        ...
        internal void DtdParserProxy_OnPublicId(
            string publicId,
            LineInfo keywordLineInfo,
        ...
        )
        {
            SendValidationEvent(
                severity,
                new XmlSchemaException(code, arg, ps.baseUriStr, lineNo, linePos)
            );
        }
...
#if !SILVERLIGHT
#endif
            get { return attrCount > 0 && parsingFunction >= ParsingFunction.InReadAttributeValue; }
        }
        ...
        private void FinishAttributeValueIterator()
        {
            ...
            if (parsingFunction == ParsingFunction.InReadAttributeValue)
            {
                ...
                parsingFunction = nextParsingFunction;
                nextParsingFunction =
                    (index > 0) ? ParsingFunction.ElementContent : ParsingFunction.DocumentContent;
            }
        }
        ...
        {
            get { return validationEventHandling != null; }
        }
        ...
        private void OpenUrl()
        {
            ...
            try
            {
                ...
                CompressedStack.Run(
                    compressedStack,
                    new ContextCallback(OpenUrlDelegate),
                    tmpResolver
                );
#endif
            }
...
#else
            if (
                (newEncoding.WebName != ps.encoding.WebName || ps.decoder is SafeAsciiDecoder)
                && !afterResetState
            )
            {
#endif
        ...
        private Encoding CheckEncoding(string newEncodingName)
        {
            ...
            if (
                0 == String.Compare(newEncodingName, "ucs-2", StringComparison.OrdinalIgnoreCase)
                || 0
                    == String.Compare(newEncodingName, "utf-16", StringComparison.OrdinalIgnoreCase)
                || 0
                    == String.Compare(
                        newEncodingName,
                        "iso-10646-ucs-2",
        ...
        void UnDecodeChars()
        {
            ...
            Debug.Assert(
                ps.appendMode,
                "UnDecodeChars cannot be called after ps.appendMode has been changed to false"
            );
        ...
        private bool ParseXmlDeclaration(bool isTextDecl)
        {
            ...
            if (
                !XmlConvert.StrEqual(ps.chars, ps.charPos, 5, XmlDeclarationBegining)
                        ...
                        if (encoding == null)
                        {
                            ...
                            if (afterResetState)
                            {
                                ...
                                if (
                                    ...
                                    && !(ps.encoding is Ucs4Encoding)
                                )
                                {
                                    Throw(
                                        Res.Xml_EncodingSwitchAfterResetState,
                                        (ps.encoding.GetByteCount("A") == 1) ? "UTF-8" : "UTF-16"
                                    );
                                }
                            }
#endif
                    ...
                    switch (xmlDeclState)
                    {
                        ...
                        case 2:
                            if (XmlConvert.StrEqual(ps.chars, ps.charPos, pos - ps.charPos, "yes"))
                            {
                                ...
                                XmlConvert.StrEqual(ps.chars, ps.charPos, pos - ps.charPos, "no")
                            )
                            {
        ...
        private void ParseElement()
        {
            ...
            unsafe
            {
                ...
                if ((xmlCharType.charProperties[chars[pos]] & XmlCharType.fNCStartNameSC) != 0)
                {
                    ...
                    pos + 1 < ps.charsUsed
                    && xmlCharType.IsNCNameSurrogateChar(chars[pos + 1], chars[pos])
                )
                {
                    pos += 2;
                }
            ...
            else
            {
                ...
                if (
                    prefixLen == lastPrefix.Length
                    && XmlConvert.StrEqual(chars, startPos, prefixLen, lastPrefix)
                )
                {
                    curNode.SetNamedNode(
                        XmlNodeType.Element,
            ...
            else
            {
                ...
                else
                {
                    Throw(
                        pos,
                        Res.Xml_BadNameChar,
                        XmlException.BuildCharExceptionArgs(chars, ps.charsUsed, pos)
                    );
                }
        ...
        private void AddDefaultAttributesAndNormalize()
        {
            ...
            if (normalize && attlistInfo.HasNonCDataAttributes)
            {
                ...
                for (int i = index + 1; i < index + 1 + attrCount; i++)
                {
                    ...
                    IDtdAttributeInfo attributeInfo = attlistInfo.LookupAttribute(
                        attr.prefix,
                        attr.localName
                    );
                    if (attributeInfo != null && attributeInfo.IsNonCDataType)
                    {
                        ...
                        if (DtdValidation && standalone && attributeInfo.IsDeclaredInExternal)
                        {
                            ...
                            if (oldValue != attr.StringValue)
                            {
                                SendValidationEvent(
                                    XmlSeverityType.Error,
                                    Res.Sch_StandAloneNormalization,
                                    attr.GetNameWPrefix(nameTable),
                                    attr.LineNo,
                                    attr.LinePos
                                );
                            }
            ...
            IEnumerable<IDtdDefaultAttributeInfo> defaultAttributes =
                attlistInfo.LookupDefaultAttributes();
            if (defaultAttributes != null)
            {
                ...
                if (attrCount >= MaxAttrDuplWalkCount)
                {
                    ...
                    Array.Sort<object>(
                        nameSortedAttributes,
                        DtdDefaultAttributeInfoToNodeDataComparer.Instance
                    );
                }
                ...
                foreach (IDtdDefaultAttributeInfo defaultAttributeInfo in defaultAttributes)
                {
                    if (AddDefaultAttributeDtd(defaultAttributeInfo, true, nameSortedAttributes))
                    {
                        ...
                        )
                        {
                            ...
                            string qname =
                                (prefix.Length == 0)
                                    ? defaultAttributeInfo.LocalName
                                    : (prefix + ':' + defaultAttributeInfo.LocalName);
                            SendValidationEvent(
                                XmlSeverityType.Error,
                                Res.Sch_UnSpecifiedDefaultAttributeInExternalStandalone,
                                qname,
                                curNode.LineNo,
                                curNode.LinePos
                            );
        ...
        private void ParseEndElement()
        {
            ...
            else
            {
                ...
                if (
                    !XmlConvert.StrEqual(chars, ps.charPos, prefLen, startTagNode.prefix)
                    || chars[colonPos] != ':'
...
#if SILVERLIGHT
                    if (
                        xmlCharType.IsNCNameSingleChar(chars[pos])
                        ||
#else // Optimization due to the lack of inlining when a method uses byte*
                    if (
                        ((xmlCharType.charProperties[chars[pos]] & XmlCharType.fNCNameSC) != 0)
                        ||
#endif
                        (
                            chars[pos] == ':'
            ...
            for (; ; )
            {
                ...
                unsafe
                {
                    while (
                        (
                            (
                                xmlCharType.charProperties[tmpch0 = chars[pos]]
                                & XmlCharType.fWhitespace
                            ) != 0
                        )
                    )
                    {
                ...
                unsafe
                {
                    ...
                    if (
                        (
                            xmlCharType.charProperties[tmpch1 = chars[pos]]
                            & XmlCharType.fNCStartNameSC
                        ) != 0
                    )
                    {
#endif
                        ...
                        pos + 1 < ps.charsUsed
                        && xmlCharType.IsNCNameSurrogateChar(chars[pos + 1], tmpch1)
                    )
                    {
                        startNameCharSize = 2;
                    }
                ...
                unsafe
                {
                    for (; ; )
                    {
                        ...
                        if (
                            (
                                (
                                    xmlCharType.charProperties[tmpch2 = chars[pos]]
                                    & XmlCharType.fNCNameSC
                                ) != 0
                            )
                        )
                        {
                            ...
                            pos + 1 < ps.charsUsed
                            && xmlCharType.IsNCNameSurrogateChar(chars[pos + 1], tmpch2)
                        )
                        {
                            pos += 2;
                        }
                    ...
                    else
                    {
                        ...
                        unsafe
                        {
                            ...
                            if (
                                (
                                    (
                                        xmlCharType.charProperties[chars[pos]]
                                        & XmlCharType.fNCStartNameSC
                                    ) != 0
                                )
                            )
                ...
                unsafe
                {
                    while (
                        (
                            (
                                xmlCharType.charProperties[tmpch3 = chars[pos]]
                                & XmlCharType.fAttrValue
                            ) != 0
                        )
                    )
                    {
                ...
                if (tmpch3 == quoteChar)
                {
                    ...
                    if (normalize)
                    {
                        ...
                        Debug.Assert(
                            val == XmlComplianceUtil.CDataNormalize(val),
                            "The attribute value is not CDATA normalized!"
                        );
                    }
#endif
#endif
        ...
        private void AttributeDuplCheck()
        {
            if (attrCount < MaxAttrDuplWalkCount)
            {
                for (int i = index + 1; i < index + 1 + attrCount; i++)
                {
                    ...
                    {
                        if (
                            Ref.Equal(attr1.localName, nodes[j].localName)
                            && Ref.Equal(attr1.ns, nodes[j].ns)
                        )
                        {
                            Throw(
                                Res.Xml_DupAttributeName,
                                nodes[j].GetNameWPrefix(nameTable),
                                nodes[j].LineNo,
                                nodes[j].LinePos
                            );
                        }
            ...
            else
            {
                ...
                for (int i = 1; i < attrCount; i++)
                {
                    ...
                    if (
                        Ref.Equal(attr1.localName, attr2.localName) && Ref.Equal(attr1.ns, attr2.ns)
                    )
                    {
                        Throw(
                            Res.Xml_DupAttributeName,
                            attr2.GetNameWPrefix(nameTable),
                            attr2.LineNo,
                            attr2.LinePos
                        );
                    }
            ...
            switch (attr.localName)
            {
                ...
                case "space":
                    ...
                    switch (XmlConvert.TrimString(attr.StringValue))
                    {
                        ...
                        default:
                            Throw(
                                Res.Xml_InvalidXmlSpace,
                                attr.StringValue,
                                attr.lineInfo.lineNo,
                                attr.lineInfo.linePos
                            );
                            break;
                    }
                    break;
        ...
        private void ParseAttributeValueSlow(int curPos, char quoteChar, NodeData attr)
        {
                        ...
                        case '&':
                            ...
                            switch (HandleEntityReference(true, EntityExpandType.All, out pos))
                            {
                                ...
                                case EntityType.Unexpanded:
                                    if (
                                        parsingMode == ParsingMode.Full
                                        && ps.entityId == attributeBaseEntityId
                                    )
                                    {
                                        // construct text value chunk
                                        int valueChunkLen =
                                            stringBuilder.Length - valueChunkStartPos;
                                        if (valueChunkLen > 0)
                                        {
            ...
            if (attr.nextAttrValueChunk != null)
            {
                ...
                if (valueChunkLen > 0)
                {
                    ...
                    textChunk.SetValueNode(
                        XmlNodeType.Text,
                        stringBuilder.ToString(valueChunkStartPos, valueChunkLen)
                    );
                    AddAttributeChunkToList(attr, textChunk, ref lastChunk);
                }
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        !fullValue
                        && orChars <= 0x20
                        && stringBuilder.Length < MinWhitespaceLookahedCount
                    );

                    // determine the value node type
                    XmlNodeType nodeType =
                        (stringBuilder.Length < MinWhitespaceLookahedCount)
                            ? GetTextNodeType(orChars)
                            : XmlNodeType.Text;
                    if (nodeType == XmlNodeType.None)
                    {
            ...
            for (; ; )
            {
                ...
                switch (c)
                {
                    ...
                    case '&':
                        ...
                        if (
                            (charRefEndPos = ParseCharRefInline(pos, out charCount, out entityType))
                            > 0
                        )
                        {
                            if (rcount > 0)
        ...
        void FinishReadValueChunk()
        {
            ...
            if (incReadState == IncrementalReadState.ReadValueChunk_OnPartialValue)
            {
                Debug.Assert(
                    (index > 0)
                        ? nextParsingFunction == ParsingFunction.ElementContent
                        : nextParsingFunction == ParsingFunction.DocumentContent
                );
                SkipPartialTextValue();
            }
        ...
        private bool ParseRootLevelWhitespace()
        {
            ...
            if (nodeType == XmlNodeType.None)
            {
                ...
                if (
                    ps.chars[ps.charPos] == '<'
                    || ps.charsUsed - ps.charPos == 0
                    || ZeroEndingStream(ps.charPos)
                )
                {
                    return false;
                }
        ...
        private EntityType HandleEntityReference(
        ...
        )
        {
            ...
            {
                EntityType entityType;
                charRefEndPos = ParseNumericCharRef(
            ...
            else
            {
                ...
                // general entity reference
                if (
                    expandType == EntityExpandType.OnlyCharacter
                    || (
                        entityHandling != EntityHandling.ExpandEntities
                ...
                EntityType entType = HandleGeneralEntityReference(
                    entityName,
                    isInAttributeValue,
                    false,
                    entityLinePos
        ...
        )
        {
            ...
            if (
                dtdInfo == null
                && fragmentParserContext != null
                && fragmentParserContext.HasDtdInfo
            ...
            if (dtdInfo == null || ((entity = dtdInfo.LookupEntity(name)) == null))
            {
#if !SILVERLIGHT // Needed only for XmlTextReader (when used from XmlDocument)
            ...
            {
                Throw(
                    Res.Xml_ExternalEntityInStandAloneDocument,
                    entity.Name,
                    ps.LineNo,
                    entityStartLinePos
                );
...
#else
                    return (isInAttributeValue && validatingReaderCompatFlag)
        ...
        {
            get { return parsingStatesStackTop >= 0; }
        }
        ...
        private bool ParsePI(BufferBuilder piInDtdStringBuilder)
        {
            ...
            {
                Throw(
                    target.Equals("xml") ? Res.Xml_XmlDeclNotFirst : Res.Xml_InvalidPIName,
                    target
                );
            }
            ps.charPos = nameEndPos;
            ...
            if (EatWhitespaces(piInDtdStringBuilder) == 0)
            {
                ...
                if (ch != '?' || ps.chars[ps.charPos + 1] != '>')
                {
                    Throw(
                        Res.Xml_BadNameChar,
                        XmlException.BuildCharExceptionArgs(ps.chars, ps.charsUsed, ps.charPos)
                    );
                }
            }
        ...
        private bool ParsePIValue(out int outStartPos, out int outEndPos)
        {
            ...
            for (; ; )
            {
                ...
                {
                    while (
                        ((xmlCharType.charProperties[tmpch = chars[pos]] & XmlCharType.fText) != 0)
                        && tmpch != '?'
                    )
                    {
                        pos++;
        ...
        private bool ParseDoctypeDecl()
        {
            if (dtdProcessing == DtdProcessing.Prohibit)
            {
                ThrowWithoutLineInfo(
                    v1Compat ? Res.Xml_DtdIsProhibited : Res.Xml_DtdIsProhibitedEx
                );
            }
...
#else
            if (
                (validatingReaderCompatFlag || !v1Compat)
                && (dtdInfo.HasDefaultAttributes || dtdInfo.HasNonCDataAttributes)
            )
            {
#endif
        ...
        void SkipUntil(char stopChar, bool recognizeLiterals)
        {
            ...
            for (; ; )
            {
...
#if SILVERLIGHT
                while (
                    xmlCharType.IsAttributeValueChar(ch = chars[pos])
                    && ch != stopChar
                    && ch != '-'
                    && ch != '?'
                )
                ...
                unsafe
                {
                    while (
                        (
                            (xmlCharType.charProperties[ch = chars[pos]] & XmlCharType.fAttrValue)
                            != 0
                        )
                        && chars[pos] != stopChar
                        && ch != '-'
        ...
        private int ParseCharRefInline(int startPos, out int charCount, out EntityType entityType)
        {
            ...
            {
                return ParseNumericCharRefInline(
                    startPos,
                    true,
                    null,
                    out charCount,
        ...
        )
        {
            for (; ; )
            {
                ...
                int charCount;
                switch (
                    newPos = ParseNumericCharRefInline(
                        ps.charPos,
                        expand,
        ...
        )
        {
            ...
            if (val <= char.MaxValue)
            {
                char ch = (char)val;
                if (
                    !xmlCharType.IsCharData(ch)
                    && ((v1Compat && normalize) || (!v1Compat && checkCharacters))
                )
                {
                    Throw(
                        (ps.chars[startPos + 2] == 'x') ? startPos + 3 : startPos + 2,
                        Res.Xml_InvalidCharacter,
                        XmlException.BuildCharExceptionArgs(ch, '\0')
                    );
                }
            ...
            else
            {
                ...
                if (normalize)
                {
                    ...
                    Throw(
                        (ps.chars[startPos + 2] == 'x') ? startPos + 3 : startPos + 2,
                        Res.Xml_InvalidCharacter,
                        XmlException.BuildCharExceptionArgs(high, low)
                    );
                }
        ...
        private NodeData AddAttribute(int endNamePos, int colonPos)
        {
            ...
            else
            {
                ...
                if (
                    prefixLen == lastPrefix.Length
                    && XmlConvert.StrEqual(ps.chars, startPos, prefixLen, lastPrefix)
                )
                {
                    return AddAttribute(
                        nameTable.Add(ps.chars, colonPos + 1, endNamePos - colonPos - 1),
                        lastPrefix,
                        null
                    );
                }
        ...
        private void AddNamespace(string prefix, string uri, NodeData attr)
        {
            if (uri == XmlReservedNs.NsXmlNs)
            {
                if (Ref.Equal(prefix, XmlNs))
                {
                    Throw(
                        Res.Xml_XmlnsPrefix,
                        (int)attr.lineInfo2.lineNo,
                        (int)attr.lineInfo2.linePos
                    );
                }
                else
                {
                    Throw(
                        Res.Xml_NamespaceDeclXmlXmlns,
                        prefix,
                        (int)attr.lineInfo2.lineNo,
                        (int)attr.lineInfo2.linePos
                    );
            ...
            {
                Throw(
                    Res.Xml_BadNamespaceDecl,
                    (int)attr.lineInfo.lineNo,
                    (int)attr.lineInfo.linePos
                );
            }
        ...
        [ResourceExposure(ResourceScope.Machine)]
#endif
        private void PushExternalEntityOrSubset(
            string publicId,
            string systemId,
            Uri baseUri,
            string entityName
        )
        {
            ...
            catch (Exception e)
            {
                ...
                Throw(
                    new XmlException(
                        entityName == null
                            ? Res.Xml_ErrorOpeningExternalDtd
                            : Res.Xml_ErrorOpeningExternalEntity,
            ...
            {
                ThrowWithoutLineInfo(
                    Res.Xml_CannotResolveExternalSubset,
                    new string[] { (publicId != null ? publicId : string.Empty), systemId },
                    null
                );
            }
            else
            {
                Throw(
                    dtdProcessing == DtdProcessing.Ignore
                        ? Res.Xml_CannotResolveEntityDtdIgnored
                        : Res.Xml_CannotResolveEntity,
                    entityName
                );
            }
        }
        ...
        private bool OpenAndPush(Uri uri)
        {
            ...
            if (xmlResolver.SupportsType(uri, typeof(TextReader)))
            {
                TextReader textReader = (TextReader)
                    xmlResolver.GetEntity(uri, null, typeof(TextReader));
                if (textReader == null)
                {
                    return false;
                }
            ...
            else
            {
                ...
                Debug.Assert(
                    xmlResolver.SupportsType(uri, typeof(Stream)),
        ...
        private void PushInternalEntity(IDtdEntityInfo entity)
        {
            ...
            InitStringInput(
                (entity.DeclaredUriString != null) ? entity.DeclaredUriString : string.Empty,
                enc,
                ...
                if (currentEntities.ContainsKey(entity))
                {
                    Throw(
                        entity.IsParameterEntity
                            ? Res.Xml_RecursiveParEntity
                            : Res.Xml_RecursiveGenEntity,
                        entity.Name,
                        parsingStatesStack[parsingStatesStackTop].LineNo,
                        parsingStatesStack[parsingStatesStackTop].LinePos
                    );
                }
            }
        ...
        private void PushParsingState()
        {
            ...
            {
                ParsingState[] newParsingStateStack = new ParsingState[
                    parsingStatesStack.Length * 2
                ];
                Array.Copy(
                    parsingStatesStack,
                    0,
                    newParsingStateStack,
                    0,
                    parsingStatesStack.Length
                );
        ...
        private int IncrementalRead(Array array, int index, int count)
        {
            if (array == null)
            {
                throw new ArgumentNullException(
                    (incReadDecoder is IncrementalReadCharsDecoder) ? "buffer" : "array"
                );
            }
            if (count < 0)
            {
                throw new ArgumentOutOfRangeException(
                    (incReadDecoder is IncrementalReadCharsDecoder) ? "count" : "len"
                );
            }
            if (index < 0)
            {
                throw new ArgumentOutOfRangeException(
                    (incReadDecoder is IncrementalReadCharsDecoder) ? "index" : "offset"
                );
            }
        ...
        private int IncrementalRead()
        {
            ...
            for (; ; )
            {
                    ...
                    unsafe
                    {
                        if (incReadState == IncrementalReadState.Attributes)
                        {
                            while (
                                (
                                    (
                                        xmlCharType.charProperties[c = chars[pos]]
                                        & XmlCharType.fAttrValue
                                    ) != 0
                                )
                                && c != '/'
                                ...
                                case '/':
                                {
                                    ...
                                    if (
                                        XmlConvert.StrEqual(
                                            ...
                                            ps.charPos + 2,
                                            endPos - ps.charPos - 2,
                                            curNode.GetNameWPrefix(nameTable)
                                        )
                                        && (
                                            ps.chars[endPos] == '>'
                                            || xmlCharType.IsWhiteSpace(ps.chars[endPos])
                                        )
                                    )
                                    {
                                        if (--incReadDepth > 0)
                ...
                if (charsParsed > 0)
                {
                    ...
                    Debug.Assert(
                        count == charsParsed || incReadDecoder.IsFull,
        ...
        private void FinishIncrementalRead()
        {
            ...
            Debug.Assert(
                IncrementalRead() == 0,
                "Previous call of IncrementalRead should eat up all characters!"
            );
            incReadDecoder = null;
        }
            ...
            for (; ; )
            {
                ...
                switch (chars[pos])
                {
                    ...
                    case (char)0xD:
                        Debug.Assert(
                            ps.eolNormalized,
                            "Entity replacement text for attribute values should be EOL-normalized!"
                        );
                        pos++;
                        continue;
                    // eol A, tab
                    ...
                    case '&':
                        ...
                        switch (
                            HandleEntityReference(true, EntityExpandType.OnlyCharacter, out pos)
                        )
                        {
                            case EntityType.CharacterDec:
                            case EntityType.CharacterHex:
                            case EntityType.CharacterNamed:
                                ...
                                if (
                                    normalize
                                    && xmlCharType.IsWhiteSpace(chars[ps.charPos])
                                    && pos - ps.charPos == 1
                                )
                                {
        ...
        private void ThrowUnexpectedToken(string expectedToken1, string expectedToken2)
        {
            ...
            {
                Throw(
                    Res.Xml_UnexpectedTokens2,
                    new string[3] { unexpectedToken, expectedToken1, expectedToken2 }
                );
            }
            else
            {
        ...
        private void ParseDtdFromParserContext()
        {
            ...
            // Parse DTD
            dtdInfo = dtdParser.ParseFreeFloatingDtd(
                fragmentParserContext.BaseURI,
                fragmentParserContext.DocTypeName,
                fragmentParserContext.PublicId,
                fragmentParserContext.SystemId,
                fragmentParserContext.InternalSubset,
        ...
        bool InitReadContentAsBinary()
        {
            ...
            {
                throw new InvalidOperationException(
                    Res.GetString(Res.Xml_MixingV1StreamingWithV2Binary)
                );
            }
        ...
        void SetupFromParserContext(XmlParserContext context, XmlReaderSettings settings)
        {
            ...
            if (context.NamespaceManager != null)
            {
                ...
                Debug.Assert(nt != null);
                Debug.Assert(
                    context.NameTable == null || context.NameTable == nt,
                    "This check should have been done in XmlParserContext constructor."
                );
            }
        ...
        {
            Debug.Assert(
                curNode.type == XmlNodeType.Whitespace
                    && newNodeType == XmlNodeType.SignificantWhitespace,
                "Incorrect node type change!"
            );
        ...
        {
            get { return curNode.schemaType; }
            set { curNode.schemaType = value; }
        }

        internal object InternalTypedValue
        {
            get { return curNode.typedValue; }
            set { curNode.typedValue = value; }
        }
        ...
        internal ConformanceLevel V1ComformanceLevel
        {
            get
            {
                return fragmentType == XmlNodeType.Element
                    ? ConformanceLevel.Fragment
                    : ConformanceLevel.Document;
            }
        }

        private bool AddDefaultAttributeDtd(
            IDtdDefaultAttributeInfo defAttrInfo,
            bool definedInDtd,
        ...
        )
        {
            ...
            else
            {
                ...
                {
                    if (
                        (object)nodes[i].localName == (object)localName
            ...
            NodeData attr = AddDefaultAttributeInternal(
                defAttrInfo.LocalName,
                null,
                defAttrInfo.Prefix,
                defAttrInfo.DefaultValueExpanded,
                defAttrInfo.LineNumber,
                defAttrInfo.LinePosition,
                defAttrInfo.ValueLineNumber,
                defAttrInfo.ValueLinePosition,
                defAttrInfo.IsXmlAttribute
            );
        ...
        internal bool AddDefaultAttributeNonDtd(SchemaAttDef attrDef)
        {
            ...
            for (int i = index + 1; i < index + 1 + attrCount; i++)
            {
                if (
                    ...
                    && (
                        ((object)nodes[i].prefix == (object)prefix)
                        || ((object)nodes[i].ns == (object)ns && ns != null)
                    )
                )
                {
                    return false;
            ...
            NodeData attr = AddDefaultAttributeInternal(
                ...
                ns,
                prefix,
                attrDef.DefaultValueExpanded,
                attrDef.LineNumber,
                attrDef.LinePosition,
                attrDef.ValueLineNumber,
                attrDef.ValueLinePosition,
                attrDef.Reserved != SchemaAttDef.Reserve.None
            );
            Debug.Assert(attr != null);

            attr.schemaType =
                (attrDef.SchemaType == null)
                    ? (object)attrDef.Datatype
        ...
        private NodeData AddDefaultAttributeInternal(
            string localName,
            string ns,
            string prefix,
            string value,
        ...
        {
            set { disableUndeclaredEntityCheck = value; }
        }
#endif
...
#endif
        static internal unsafe void AdjustLineInfo(
            char[] chars,
            int startPos,
            int endPos,
            bool isNormalized,
        ...
        internal static void BlockCopyChars(
            char[] src,
            int srcOffset,
            char[] dst,
            int dstOffset,
        ...
        )
        {
            ...
            Buffer.BlockCopy(
                ...
                dst,
                dstOffset * sizeof(char),
                count * sizeof(char)
            );
#endif
        }

        internal static void BlockCopy(
            byte[] src,
            int srcOffset,
            byte[] dst,
            int dstOffset,
