        ...
        internal void Write(char[] array, int offset, int count)
        {
            ...
            for (; ; )
            {
                ...
                unsafe
                {
                    while (
                        i < endPos
                        && (xmlCharType.charProperties[ch = array[i]] & XmlCharType.fAttrValue) != 0
                    )
                        ...
                        if (XmlCharType.IsHighSurrogate(ch))
                        {
                            ...
                            else
                            {
                                throw new ArgumentException(
                                    Res.GetString(Res.Xml_SurrogatePairSplit)
                                );
                            }
                        }
        ...
        internal void Write(string text)
        {
            ...
            for (; ; )
            {
                unsafe
                {
                    while (
                        i < len
                        && (xmlCharType.charProperties[ch = text[i]] & XmlCharType.fAttrValue) != 0
                    )
        ...
        internal void WriteRawWithSurrogateChecking(string text)
        {
            ...
            for (; ; )
            {
                ...
                if (XmlCharType.IsHighSurrogate(ch))
                {
                    ...
                    throw new ArgumentException(
                        Res.GetString(Res.Xml_InvalidSurrogateMissingLowChar)
                    );
                }
                else if (XmlCharType.IsLowSurrogate(ch))
                {
