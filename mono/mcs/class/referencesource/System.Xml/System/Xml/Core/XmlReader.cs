...
namespace System.Xml
{
    ...
    {
        private static uint IsTextualNodeBitmap = 0x6018; // 00 0110 0000 0001 1000
        ...
        private static uint CanReadContentAsBitmap = 0x1E1BC; // 01 1110 0001 1011 1100
        ...
        private static uint HasValueBitmap = 0x2659C; // 10 0110 0101 1001 1100
        ...
        public virtual string ReadElementString()
        {
            ...
            if (!this.IsEmptyElement)
            {
                ...
                {
                    throw new XmlException(
                        Res.Xml_UnexpectedNodeInSimpleContent,
...
#pragma warning restore 618
        ...
        )
        {
            return new InvalidOperationException(
                AddLineInfo(
                    Res.GetString(
                        Res.Xml_InvalidReadElementContentAs,
                        new string[] { methodName, nodeType.ToString() }
                    ),
                    lineInfo
                )
            ...
            if (byteCount >= 2 && (bytes[0] == 0xdf && bytes[1] == 0xff))
            {
                if (inputContext != null)
                    throw new ArgumentException(
                        Res.GetString(Res.XmlBinary_NoParserContext),
                        "inputContext"
                    );
                ...
                switch (nt)
                {
                    ...
                    case XmlNodeType.ProcessingInstruction:
                        result +=
                            ...
                            + reader.Name
                            + "\", Value=\""
                            + XmlConvert.EscapeValueForDebuggerDisplay(reader.Value)
                            + '"';
                        break;
                    case XmlNodeType.Text:
                    case XmlNodeType.Whitespace:
