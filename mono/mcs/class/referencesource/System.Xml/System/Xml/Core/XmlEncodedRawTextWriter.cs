...
namespace System.Xml
{
    ...
    internal partial class XmlEncodedRawTextWriter : XmlRawWriter
    {
            ...
            fixed (char* pDstBegin = bufChars)
            {
                ...
                for (; ; )
                {
                    ...
                    while (
                        ...
                        && (
                            (
                                (xmlCharType.charProperties[(ch = *pSrc)] & XmlCharType.fAttrValue)
                                != 0
                            )
                        )
                    )
        ...
        protected unsafe void WriteRawWithCharChecking(char* pSrcBegin, char* pSrcEnd)
        {
            fixed (char* pDstBegin = bufChars)
            {
                ...
                for (; ; )
                {
                    ...
                    while (
                        pDst < pDstEnd
                        && (((xmlCharType.charProperties[(ch = *pSrc)] & XmlCharType.fText) != 0))
                    )
                    {
                        *pDst = (char)ch;
                        pDst++;
        ...
        protected unsafe void ValidateContentChars(
            string chars,
            string propertyName,
            ...
            if (allowOnlyWhitespace)
            {
                if (!xmlCharType.IsOnlyWhitespace(chars))
                {
                    throw new ArgumentException(
                        Res.GetString(Res.Xml_IndentCharsNotWhitespace, propertyName)
                    );
                }
            }
            else
