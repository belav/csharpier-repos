...
namespace System.Xml.XPath.DataBinding
{
    ...
    public sealed class XPathDocumentView : IBindingList, ITypedList
    {
        ...
        public PropertyDescriptorCollection GetItemProperties(PropertyDescriptor[] listAccessors)
        {
            ...
            {
                XPathNodeViewPropertyDescriptor propdesc =
                    listAccessors[listAccessors.Length - 1] as XPathNodeViewPropertyDescriptor;
                if (null != propdesc)
        ...
        XPathNodeView FillRow(XPathNode ndRow, Shape shape)
        {
            ...
            switch (shape.BindingType)
            {
                ...
                case BindingType.All:
                    ...
                    if (
                        shape.BindingType == BindingType.Sequence
                    ...
                    if (lastSubShape.BindingType == BindingType.Text)
                    { //Attributes followed by simpe content or mixed content
            ...
            switch (shape.BindingType)
            {
                ...
                case BindingType.Repeat:
                {
                    ...
                    if (subShape.BindingType == BindingType.ElementNested)
                    {
                        ...
                        XPathNode nd;
                        while (null != (nd = iter.Node) && subShape.IsParticleMatch(iter.Particle))
                        {
                            rows.Add(nd);
                            iter.Next();
                        }
                    ...
                    else
                    {
                        ...
                        XPathNode nd;
                        while (null != (nd = iter.Node) && shape.IsParticleMatch(iter.Particle))
                        {
        ...
        void DeriveShapeFromRows()
        {
            ...
            for (int i = 0; (i < rows.Count) && (null == schemaInfo); i++)
            {
                ...
                Debug.Assert(
                    null != nd
                        && (
                            XPathNodeType.Attribute == nd.NodeType
                            || XPathNodeType.Element == nd.NodeType
                        )
                );
                if (null != nd)
                {
                ...
                else if ('@' == ch)
                {
                    if (0 == i)
                        throw new XmlException(
                            Res.XmlDataBinding_XPathAttrNotFirst,
                            (string[])null
                        );
                    pos++;
                    if (pos >= xpath.Length)
        ...
        string ParseName(string xpath, ref int pos)
        {
            char ch;
            int start = pos++;
            while (pos < xpath.Length && '/' != (ch = xpath[pos]) && ':' != ch)
                pos++;
        ...
        class ContentIterator
        {
            ...
            public XPathNode Node
            {
                get { return this.node; }
            }
            public object Particle
            {
                get { return this.currentParticle; }
            }
            ...
            public bool Next()
            {
                if (null != this.node)
                {
                    this.node = TreeNavigationHelper.GetContentSibling(
                        this.node,
