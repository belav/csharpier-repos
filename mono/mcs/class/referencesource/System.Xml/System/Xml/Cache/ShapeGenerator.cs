...
using System;using System.Xml;using System.Xml.Schema;using System.Xml.XPath;using System.Collections;using System.Diagnostics;using System.ComponentModel;using System.Text;
...
namespace System.Xml.XPath.DataBinding
{
    internal sealed class ShapeGenerator
    {
        ...
        Shape ProcessParticleElement(XmlSchemaElement xse)
        {
            ...
            s = new Shape(
                GenName(xse.QualifiedName),
                complex ? BindingType.ElementNested : BindingType.Element
            );
            s.AddParticle(xse);
        ...
        Shape ProcessParticleGroup(XmlSchemaGroupBase xsg, BindingType bt)
        {
            ...
            foreach (XmlSchemaParticle xsp in xsg.Items)
            {
                ...
                if (sub != null)
                { //sub can be null if the child particle is xs:any
                    if (sb.Length > 0)
                        sb.Append('_');
                    sb.Append(sub.Name);
                }
