...
namespace MS.Internal.Xml.Cache
{
    ...
    internal sealed class XPathDocumentBuilder : XmlRawWriter
    {
        ...
        )
        {
            ...
            this.idxNmsp = NewNamespaceNode(
                out this.pageNmsp,
                this.nameTable.Add("xml"),
                this.nameTable.Add(XmlReservedNs.NsXml),
                null,
                0
            );
        ...
        public void WriteStartElement(string prefix, string localName, string ns, string baseUri)
        {
            ...
            this.elemNameIndex[hash] = LinkSimilarElements(
                this.elemNameIndex[hash].Page,
                this.elemNameIndex[hash].Index,
                this.pageParent,
                this.idxParent
            );
            ...
            if (this.elemIdMap != null)
                this.idAttrName = (XmlQualifiedName)
                    this.elemIdMap[new XmlQualifiedName(localName, prefix)];
        }
        ...
        /// Must be called when an element node's children have been fully enumerated.
        /// </summary>
        internal override void WriteFullEndElement(
            string prefix,
            string localName,
            string namespaceName
        )
                ...
                switch (this.textBldr.TextType)
                {
                    case TextBlockType.Text:
                        ...
                        if (this.lineInfo != null)
                        {
                            ...
                            if (
                                this.textBldr.LineNumber
                                != this.pageParent[this.idxParent].LineNumber
                            )
                                goto case TextBlockType.Whitespace;

                            // If position is not within 256 of parent, don't collapse text
                            int posDiff =
                                this.textBldr.LinePosition
                                - this.pageParent[this.idxParent].LinePosition;
                    ...
                    case TextBlockType.Whitespace:
                        ...
                        CachedTextNode();
                        this.pageParent[this.idxParent].SetValue(
        ...
        public override void WriteEndAttribute()
        {
            ...
            if (this.idAttrName != null)
            {
                ...
                if (
                    this.pageSibling[this.idxSibling].LocalName == this.idAttrName.Name
        ...
        internal override void WriteNamespaceDeclaration(string prefix, string namespaceName)
        {
            ...
            if (idxOverride != 0)
            {
                ...
                while (idxOrig != idxOverride || pageOrig != pageOverride)
                {
                    ...
                    idxTemp = NewNamespaceNode(
                        out pageTemp,
                        pageOrig[idxOrig].LocalName,
                        pageOrig[idxOrig].Value,
                        pageTemp,
                        idxTemp
                    );
                ...
                if (idxOverride != 0)
                    ...
                    Debug.Assert(
                        prefix.Equals("xml"),
                        "xmlns:xml namespace declaration should always be present in the list."
                    );
            }
        ...
        private void ComputeLineInfo(bool isTextNode, out int lineNumOffset, out int linePosOffset)
        {
            ...
            else
            {
                Debug.Assert(
                    ...
                    "HasLineInfo should have been checked before this."
                );
        ...
        private void CachedTextNode()
        {
            ...
            string text;
            Debug.Assert(
                this.textBldr.HasText || (this.idxSibling == 0 && this.idxParent == 0),
                "Cannot create empty text node unless it's a top-level text node."
            );
            Debug.Assert(
                this.idxSibling == 0 || !this.pageSibling[this.idxSibling].IsText,
