        ...
        public DateTime ToZulu()
        {
            switch (InternalKind)
            {
                ...
                case XsdDateTimeKind.LocalEastOfZulu:
                    ...
                    return new DateTime(
                        dt.Subtract(new TimeSpan(ZoneHour, ZoneMinute, 0)).Ticks,
                        DateTimeKind.Utc
                    );
                case XsdDateTimeKind.LocalWestOfZulu:
                    // Adjust to UTC and then convert to local in the current time zone
                    return new DateTime(
                        dt.Add(new TimeSpan(ZoneHour, ZoneMinute, 0)).Ticks,
                        DateTimeKind.Utc
                    );
                default:
                    return dt;
            ...
            else
            {
                ...
                if (left.InternalTypeCode != right.InternalTypeCode)
                {
                    throw new ArgumentException(
                        Res.GetString(Res.Sch_XsdDateTimeCompare, left.TypeCode, right.TypeCode)
                    );
                }
                // Convert both to UTC
        ...
        struct Parser
        {
            ...
            public bool Parse(string text, XsdDateTimeFlags kinds)
            {
                ...
                if (Test(kinds, XsdDateTimeFlags.DateTime | XsdDateTimeFlags.Date))
                {
                    ...
                    if (ParseDate(start))
                    {
                        ...
                        {
                            if (
                                ParseChar(start + Lzyyyy_MM_dd, 'T')
                        ...
                        {
                            if (
                                ParseZoneAndWhitespace(start + Lzyyyy_MM_dd)
                                || (
                                    ParseChar(start + Lzyyyy_MM_dd, 'T')
                                    && ParseTimeAndZoneAndWhitespace(start + Lzyyyy_MM_ddT)
                                )
                    ...
                    if (Parse4Dig(start, ref year) && 1 <= year)
                    {
                        if (Test(kinds, XsdDateTimeFlags.GYearMonth))
                        {
                            if (
                                ...
                                && Parse2Dig(start + Lzyyyy_, ref month)
                                && 1 <= month
                                && month <= 12
                ...
                if (Test(kinds, XsdDateTimeFlags.GMonthDay | XsdDateTimeFlags.GMonth))
                {
                    if (
                        ParseChar(start, '-')
                        && ParseChar(start + Lz_, '-')
                        && Parse2Dig(start + Lz__, ref month)
                        && 1 <= month
                        && month <= 12
                    )
                    {
                        ...
                        {
                            if (
                                Parse2Dig(start + Lz__mm_, ref day)
                                && 1 <= day
                        ...
                        {
                            if (
                                ParseZoneAndWhitespace(start + Lz__mm)
                                || (
                                    ParseChar(start + Lz__mm, '-')
                                    && ParseChar(start + Lz__mm_, '-')
            ...
            private bool ParseDate(int start)
            {
                return Parse4Dig(start, ref year)
                    ...
                    && Parse2Dig(start + Lzyyyy_MM_, ref day)
                    && 1 <= day
            ...
            static int[] Power10 = new int[maxFractionDigits]
            {
                -1,
                10,
                100,
            ...
            private bool ParseTime(ref int start)
            {
                if (
                    ...
                    && Parse2Dig(start + LzHH_, ref minute)
                    && minute < 60
                    && ParseChar(start + LzHH_mm, ':')
                    && Parse2Dig(start + LzHH_mm_, ref second)
                    && second < 60
                )
                {
                    start += LzHH_mm_ss;
            ...
            private bool ParseZoneAndWhitespace(int start)
            {
                if (start < length)
                {
                    ...
                    {
                        if (
                            Parse2Dig(start + Lz_, ref zoneHour)
                            && zoneHour <= 99
                            && ParseChar(start + Lz_zz, ':')
                            && Parse2Dig(start + Lz_zz_, ref zoneMinute)
                            && zoneMinute <= 99
                        )
                        {
                            if (ch == '-')
