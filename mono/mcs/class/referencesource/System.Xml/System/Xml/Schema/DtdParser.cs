...
namespace System.Xml
{
    internal partial class DtdParser : IDtdParser
    {
        ...
        static DtdParser()
        {
            ...
            Debug.Assert(
                (int)Token.IDREFS == (int)XmlTokenizedType.IDREFS
                    && (int)XmlTokenizedType.IDREFS == 3
            );
            Debug.Assert(
                (int)Token.ENTITY == (int)XmlTokenizedType.ENTITY
                    && (int)XmlTokenizedType.ENTITY == 4
            );
            Debug.Assert(
                (int)Token.ENTITIES == (int)XmlTokenizedType.ENTITIES
                    && (int)XmlTokenizedType.ENTITIES == 5
            );
            Debug.Assert(
                (int)Token.NMTOKEN == (int)XmlTokenizedType.NMTOKEN
                    && (int)XmlTokenizedType.NMTOKEN == 6
            );
            Debug.Assert(
                (int)Token.NMTOKENS == (int)XmlTokenizedType.NMTOKENS
                    && (int)XmlTokenizedType.NMTOKENS == 7
            );
            Debug.Assert(
                (int)Token.NOTATION == (int)XmlTokenizedType.NOTATION
                    && (int)XmlTokenizedType.NOTATION == 8
            );
#endif
        }
...
#if !SILVERLIGHT
            IDtdParserAdapterWithValidation raWithValidation =
        ...
        private void InitializeFreeFloatingDtd(
            string baseUri,
            string docTypeName,
            string publicId,
            string systemId,
        ...
        )
        {
            ...
            {
                schemaInfo.DocTypeName = new XmlQualifiedName(
        ...
        IDtdInfo IDtdParser.ParseFreeFloatingDtd(
            string baseUri,
            string docTypeName,
            string publicId,
            string systemId,
        ...
        {
            get { return externalEntitiesDepth == 0; }
        }

        private bool IgnoreEntityReferences
        {
            get { return scanningFunction == ScanningFunction.CondSection3; }
        }

        private bool SaveInternalSubsetValue
        {
            get { return readerAdapter.EntityStackLength == 0 && internalSubsetValueSb != null; }
        }
            ...
            get
            {
                return scanningFunction == ScanningFunction.SubsetContent
                    || (
                        ...
                        && savedScanningFunction == ScanningFunction.SubsetContent
                    );
            }
        }
        ...
        private void AddUndeclaredNotation(string notationName)
        {
            ...
            UndeclaredNotation un = new UndeclaredNotation(
                notationName,
                LineNo,
                LinePos - notationName.Length
            );
            ...
            for (; ; )
            {
                ...
                switch (chars[curPos])
                {
                    ...
                    case '&':
                        ...
                        {
                            SaveParsingBuffer();
                            int endPos = readerAdapter.ParseNumericCharRef(
                        ...
                        else
                        {
                            ...
                            if (literalType == LiteralType.AttributeValue)
                            {
                                int endPos = readerAdapter.ParseNamedCharRef(
                                    true,
                                    SaveInternalSubsetValue ? internalSubsetValueSb : null
                                );
                                LoadParsingBuffer();
        ...
        private XmlQualifiedName GetNameQualified(bool canHavePrefix)
        {
            ...
            if (colonPos == -1)
            {
                return new XmlQualifiedName(
                    nameTable.Add(chars, tokenStartPos, curPos - tokenStartPos)
                );
            }
            else
            {
        ...
        private string GetValueWithStrippedSpaces()
        {
            ...
            string val =
                (stringBuilder.Length == 0)
                    ? new string(chars, tokenStartPos, curPos - tokenStartPos - 1)
            ...
            if (SaveInternalSubsetValue)
            {
                ...
                if (internalSubsetValueEndPos - readerCurPos > 0)
                {
                    internalSubsetValueSb.Append(
                        chars,
                        readerCurPos,
                        internalSubsetValueEndPos - readerCurPos
                    );
                }
            }
        ...
        private bool HandleEntityReference(
        ...
        private SchemaEntity VerifyEntityReference(
            XmlQualifiedName entityName,
            bool paramEntity,
            bool mustBeDeclared,
            bool inAttribute
        ...
        private void SendValidationEvent(XmlSeverityType severity, string code, string arg)
        {
            ...
            SendValidationEvent(
                severity,
                new XmlSchemaException(code, arg, BaseUriStr, (int)LineNo, (int)LinePos)
            );
        }
        ...
        private int LineNo
        {
            get { return readerAdapter.LineNo; }
        }

        private int LinePos
        {
            get { return curPos - readerAdapter.LineStartPosition; }
        }
        ...
        void Throw(int curPos, string res, string arg)
        {
            ...
            readerAdapter.Throw(
                new XmlException(
                    ...
                    (int)LineNo,
                    (int)LinePos,
                    baseUri == null ? null : baseUri.ToString()
                )
            );
        }
        ...
        private void ThrowUnexpectedToken(int pos, string expectedToken1, string expectedToken2)
        {
            ...
            else
            {
                Throw(
                    ...
                    new string[2] { unexpectedToken, expectedToken1 }
                );
            }
        }
        ...
        {
            if (
                xmlCharType.IsNCNameSingleChar(chars[startPos])
#if XML10_FIFTH_EDITION
