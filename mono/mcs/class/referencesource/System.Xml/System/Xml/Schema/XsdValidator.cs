    ...
    internal sealed class XsdValidator : BaseValidator
    {
        ...
        private void ProcessXsiAttributes(out XmlQualifiedName xsiType, out string xsiNil)
        {
            ...
            if (reader.Depth == 0)
            {
                ...
                foreach (
                    string ns in nsManager.GetNamespacesInScope(XmlNamespaceScope.ExcludeXml).Values
                )
                {
                    LoadSchema(ns, null);
                }
                    ...
                    else if (Ref.Equal(objectNs, NsXsi))
                    {
                        if (Ref.Equal(objectName, XsiSchemaLocation))
                        {
                            xsiSchemaLocation = (string[])
                                dtStringArray.ParseValue(reader.Value, NameTable, nsManager);
                        }
                        ...
                        else if (Ref.Equal(objectName, XsiType))
                        {
                            xsiType = (XmlQualifiedName)
                                dtQName.ParseValue(reader.Value, NameTable, nsManager);
                        }
            ...
            if (elementDecl != null)
            {
                ...
                else
                {
                    ...
                    if (
                        ...
                        && xsiType.Namespace == NsXs
                    )
                    {
                        XmlSchemaSimpleType simpleType =
                            DatatypeImplementation.GetSimpleTypeFromXsdType(
                                new XmlQualifiedName(xsiType.Name, NsXs)
                            );
                        if (simpleType != null)
                        {
                    ...
                    for (int k = 0; k < constraints[j].axisFields.Count; ++k)
                    {
                        ...
                        if (laxis.MoveToStartElement(reader.LocalName, reader.NamespaceURI))
                        {
                            ...
                            { // nextElement can be null when xml/xsd are not valid
                                ...
                                {
                                    SendValidationEvent(
                                        Res.Sch_FieldSimpleTypeExpected,
                                        reader.LocalName
                                    );
                                }
                                else
                    ...
                    for (int j = 0; j < constraints[i].axisFields.Count; ++j)
                    {
                        ...
                        if (laxis.isMatched)
                        {
                            ...
                            if (laxis.Ks[laxis.Column] != null)
                            {
                                ...
                                SendValidationEvent(
                                    Res.Sch_FieldSingleValueExpected,
                                    reader.LocalName
                                );
                            }
                            else
