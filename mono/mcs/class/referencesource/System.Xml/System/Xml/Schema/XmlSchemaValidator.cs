...
namespace System.Xml.Schema
{
    ...
    public sealed class XmlSchemaValidator
    {
        ...
        private static readonly XmlSchemaDatatype dtCDATA = XmlSchemaDatatype.FromXmlTokenizedType(
        ...
        {
            get { return validationEventSender; }
            set { validationEventSender = value; }
        }
        ...
        public void AddSchema(XmlSchema schema)
        {
            ...
            if (
                ...
                && schemaSet.FindSchemaByNSAndUrl(schema.BaseUri, tns, oldLocations) == null
            )
            {
            ...
            if (schema.ErrorCount == 0)
            {
                ...
                { //Check for its imports
                    ...
                    if (
                        validatedNamespaces[tns] != null
                        && schemaSet.FindSchemaByNSAndUrl(impSchema.BaseUri, tns, oldLocations)
                            == null
                    )
                    {
                        SendValidationEvent(
                            Res.Sch_ComponentAlreadySeenForNS,
                            tns,
        ...
        public void Initialize()
        {
            if (currentState != ValidatorState.None && currentState != ValidatorState.Finish)
            {
                throw new InvalidOperationException(
                    Res.GetString(
                        Res.Sch_InvalidStateTransition,
                        new string[]
                        {
        ...
        )
        {
            ...
            { //
                XmlSchemaObject pvtAttribute =
                    currentState == ValidatorState.TopLevelAttribute ? partialValidationType : null;
                AttributeMatchState attributeMatchState;
                attributeDef = compiledSchemaInfo.GetAttributeXsd(
                    currentElementDecl,
                    attQName,
                    pvtAttribute,
                ...
                switch (attributeMatchState)
                {
                    case AttributeMatchState.UndeclaredElementAndAttribute:
                        ...
                        {
                            SendValidationEvent(
                                Res.Sch_NoAttributeSchemaFound,
                                attQName.ToString(),
                    ...
                    case AttributeMatchState.AttributeNameMismatch:
                        attrValid = false;
                        SendValidationEvent(
                            Res.Sch_SchemaAttributeNameMismatch,
                            new string[]
                            {
                                attQName.ToString(),
            ...
            { //Attribute from xsi namespace
                ...
                if (
                    Ref.Equal(lName, xsiTypeString)
                    || Ref.Equal(lName, xsiNilString)
                    || Ref.Equal(lName, xsiSchemaLocationString)
                    || Ref.Equal(lName, xsiNoNamespaceSchemaLocationString)
                )
                {
        ...
        public void ValidateEndOfAttributes(XmlSchemaInfo schemaInfo)
        {
            CheckStateTransition(
                ...
                MethodNames[(int)ValidatorState.EndOfAttributes]
            );
        ...
        {
            ValidatorState toState =
                validationStack.Length > 1 ? ValidatorState.Text : ValidatorState.TopLevelTextOrWS;
            CheckStateTransition(toState, MethodNames[(int)toState]);
                ...
                switch (contentType)
                {
                    ...
                    case XmlSchemaContentType.ElementOnly:
                        ...
                        else
                        {
                            ...
                            SendValidationEvent(
                                Res.Sch_InvalidTextInElementExpecting,
                                new string[]
                                {
                                    BuildElementName(context.LocalName, context.Namespace),
        ...
        public void SkipToEndElement(XmlSchemaInfo schemaInfo)
        {
            if (validationStack.Length <= 1)
            {
                throw new InvalidOperationException(
                    Res.GetString(
                        Res.Sch_InvalidEndElementMultiple,
                        MethodNames[(int)ValidatorState.SkipToEndElement]
                    )
                );
            }
            CheckStateTransition(
        ...
        public XmlSchemaParticle[] GetExpectedParticles()
        {
            if (
                currentState == ValidatorState.Start
                || currentState == ValidatorState.TopLevelTextOrWS
            )
                ...
                for (int i = 0; i < currentElementDecl.DefaultAttDefs.Count; ++i)
                {
                    ...
                    if (!attPresence.Contains(attdef.Name))
                    {
                        ...
                        if (attributeNS.Length > 0)
                        {
                            ...
                            {
                                SendValidationEvent(
                                    Res.Sch_DefaultAttributeNotApplied,
                                    new string[2]
                                    {
        ...
        private object InternalValidateEndElement(XmlSchemaInfo schemaInfo, object typedValue)
        {
            if (validationStack.Length <= 1)
            {
                throw new InvalidOperationException(
                    Res.GetString(
                        Res.Sch_InvalidEndElementMultiple,
                        MethodNames[(int)ValidatorState.EndElement]
                    )
                );
            }
            CheckStateTransition(
                        ...
                        switch (contentType)
                        {
                            ...
                            case XmlSchemaContentType.ElementOnly:
                                ...
                                { //Cannot pass in typedValue for complex content
                                    throw new InvalidOperationException(
                                        Res.GetString(Res.Sch_InvalidEndElementCallTyped)
                                    );
                                }
                                break;
        ...
        )
        {
            ...
            {
                object typedValue;
                Exception exception = dtStringArray.TryParseValue(
                    xsiSchemaLocation,
                    nameTable,
                    nsResolver,
                    out typedValue
                ...
                {
                    SendValidationEvent(
                        Res.Sch_InvalidValueDetailedAttribute,
                        new string[]
                        {
                            "schemaLocation",
                            xsiSchemaLocation,
                            dtStringArray.TypeCodeString,
                            exception.Message,
                        },
                        exception
                    );
        ...
        )
        {
            ...
            if (context.NeedValidateChildren)
            {
                ...
                if (
                    ...
                    && context.ElementDecl.Presence == SchemaDeclBase.Use.Fixed
                )
                { //Mixed with default or fixed
                    SendValidationEvent(
                        Res.Sch_ElementInMixedWithFixed,
                        QNameString(context.LocalName, context.Namespace)
                    );
                    return null;
                }
                ...
                if (substitution)
                {
                    ...
                    { //It is not element ref but a local element
                        ...
                        SendValidationEvent(
                            Res.Sch_InvalidElementSubstitution,
            ...
            if (memberElem != null)
            {
                ...
                if (!head.IsEmpty)
                {
                    ...
                    if (headElem != null)
                    {
                        ...
                        {
                            SendValidationEvent(
                                Res.Sch_SubstitutionNotAllowed,
            ...
            if (!context.IsNill)
            {
                ...
                { //default value maybe present
                    ...
                    { //There was xsi:type
                        ...
                        {
                            SendValidationEvent(
                                Res.Sch_InvalidElementDefaultValue,
                                new string[]
                                {
                                    currentElementDecl.DefaultValueRaw,
                                    QNameString(context.LocalName, context.Namespace),
        ...
        private object ValidateAtomicValue(object parsedValue, out XmlSchemaSimpleType memberType)
        {
            ...
            if (!context.IsNill)
            {
                ...
                if (exception != null)
                {
                    ...
                    SendValidationEvent(
                        Res.Sch_ElementValueDataTypeDetailed,
                        new string[]
                        {
                            QNameString(context.LocalName, context.Namespace),
                            stringValue,
            ...
            if (elementDecl == null && processContents != XmlSchemaContentProcessing.Skip)
            {
                if (isRoot && partialValidationType != null)
                {
                    if (partialValidationType is XmlSchemaElement)
                    {
                        ...
                        {
                            SendValidationEvent(
                                Res.Sch_SchemaElementNameMismatch,
        ...
        )
        {
            ...
            {
                object typedVal = null;
                Exception exception = dtQName.TryParseValue(
                    xsiType,
                    nameTable,
                    nsResolver,
                    out typedVal
                ...
                if (exception != null)
                {
                    SendValidationEvent(
                        Res.Sch_InvalidValueDetailedAttribute,
                        new string[] { "type", xsiType, dtQName.TypeCodeString, exception.Message },
                        exception
                    );
                }
                else
        ...
        private SchemaAttDef CheckIsXmlAttribute(XmlQualifiedName attQName)
        {
            ...
            if (
                Ref.Equal(attQName.Namespace, NsXml)
                && (validationFlags & XmlSchemaValidationFlags.AllowXmlAttributes) != 0
            )
        ...
        {
            get { return (validationFlags & XmlSchemaValidationFlags.ProcessInlineSchema) != 0; }
        }

        internal bool ProcessSchemaLocation
        {
            get { return (validationFlags & XmlSchemaValidationFlags.ProcessSchemaLocation) != 0; }
        }
        ...
        private void CheckStateTransition(ValidatorState toState, string methodName)
        {
            if (!ValidStates[(int)currentState, (int)toState])
            {
                if (currentState == ValidatorState.None)
                {
                    throw new InvalidOperationException(
                        Res.GetString(
                            Res.Sch_InvalidStartTransition,
                ...
                throw new InvalidOperationException(
                    Res.GetString(
                        Res.Sch_InvalidStateTransition,
                        new string[] { MethodNames[(int)currentState], methodName }
                    )
                );
            }
        ...
        private void AddIdentityConstraints()
        {
            ...
            for (int i = 0; i < context.Constr.Length; ++i)
            {
                ...
                )
                {
                    ...
                    for (
                        int level = this.validationStack.Length - 1;
                        level
                            >= (
                                (this.startIDConstraint >= 0)
                    ...
                    )
                    {
                        ...
                        for (int j = 0; j < constraintStructures.Length; ++j)
                        {
                            if (
                                ...
                                == context.Constr[i].constraint.refer
                            )
                            {
                                find = true;
                                ...
                                context.Constr[i].qualifiedTable = constraintStructures[
                                    j
                                ].keyrefTable;
                                break;
                            }
        ...
        private void ElementIdentityConstraints()
        {
            ...
            for (int i = this.startIDConstraint; i < this.validationStack.Length; i++)
            {
                ...
                // else
                ConstraintStruct[] constraintStructures = (
                    (ValidationState)this.validationStack[i]
                ).Constr;
                for (int j = 0; j < constraintStructures.Length; ++j)
                {
        ...
        )
        {
            for (int ci = this.startIDConstraint; ci < this.validationStack.Length; ci++)
            {
                ...
                // else
                ConstraintStruct[] constraintStructures = (
                    (ValidationState)this.validationStack[ci]
                ).Constr;
                    ...
                    if (names == null || names.Count == 0)
                    {
                        if (context.TooComplex)
                        {
                            SendValidationEvent(
                                ...
                                sender,
                                new XmlSchemaValidationException(
                                    Res.Sch_InvalidElementContentComplex,
                                    new string[]
                                    {
                                        BuildElementName(context.LocalName, context.Namespace),
                                        BuildElementName(name),
                                        Res.GetString(Res.Sch_ComplexContentModel),
                                    },
                                    sourceUri,
                                    lineNo,
                                    linePos
                    ...
                    else
                    {
                        ...
                        if (context.TooComplex)
                        {
                            SendValidationEvent(
                                ...
                                sender,
                                new XmlSchemaValidationException(
                                    Res.Sch_InvalidElementContentExpectingComplex,
                                    new string[]
                                    {
                                        BuildElementName(context.LocalName, context.Namespace),
                                        BuildElementName(name),
                        ...
                        else
                        {
                            SendValidationEvent(
                                ...
                                sender,
                                new XmlSchemaValidationException(
                                    Res.Sch_InvalidElementContentExpecting,
                                    new string[]
                                    {
                                        BuildElementName(context.LocalName, context.Namespace),
                                        BuildElementName(name),
                ...
                { //Base ContentValidator: Empty || TextOnly || Mixed || Any
                    if (contentType == XmlSchemaContentType.Empty)
                    {
                        SendValidationEvent(
                            ...
                            sender,
                            new XmlSchemaValidationException(
                                Res.Sch_InvalidElementInEmptyEx,
                                new string[]
                                {
                                    QNameString(context.LocalName, context.Namespace),
                                    name.ToString(),
                    ...
                    else if (!contentValidator.IsOpen)
                    {
                        SendValidationEvent(
                            ...
                            sender,
                            new XmlSchemaValidationException(
                                Res.Sch_InvalidElementInTextOnlyEx,
                                new string[]
                                {
                                    QNameString(context.LocalName, context.Namespace),
                                    name.ToString(),
            ...
            if (names == null || names.Count == 0)
            {
                if (context.TooComplex)
                {
                    SendValidationEvent(
                        ...
                        sender,
                        new XmlSchemaValidationException(
                            Res.Sch_IncompleteContentComplex,
                            new string[]
                            {
                                BuildElementName(context.LocalName, context.Namespace),
                                Res.GetString(Res.Sch_ComplexContentModel),
            ...
            else
            {
                ...
                if (context.TooComplex)
                {
                    SendValidationEvent(
                        ...
                        sender,
                        new XmlSchemaValidationException(
                            Res.Sch_IncompleteContentExpectingComplex,
                            new string[]
                            {
                                BuildElementName(context.LocalName, context.Namespace),
                                PrintExpectedElements(names, getParticles),
                ...
                else
                {
                    SendValidationEvent(
                        ...
                        sender,
                        new XmlSchemaValidationException(
                            Res.Sch_IncompleteContentExpecting,
                            new string[]
                            {
                                BuildElementName(context.LocalName, context.Namespace),
                                PrintExpectedElements(names, getParticles),
        ...
        internal static string PrintExpectedElements(ArrayList expected, bool getParticles)
        {
            if (getParticles)
            {
                string ContinuationString = Res.GetString(
                    Res.Sch_ContinuationString,
                    new string[] { " " }
                );
            ...
            { //In case of one element in a namespace or any
                ...
                else
                {
                    if (name.Namespace.Length != 0)
                    {
                        builder.Append(
                            Res.GetString(
                                Res.Sch_ElementNameAndNamespace,
                                name.Name,
                                name.Namespace
                            )
                        );
