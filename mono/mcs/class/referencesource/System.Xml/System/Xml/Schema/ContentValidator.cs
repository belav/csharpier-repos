...
namespace System.Xml.Schema
{
    ...
    sealed class RangeNode : InteriorNode
    {
        ...
        public override void Dump(StringBuilder bb, SymbolsDictionary symbols, Positions positions)
        {
            ...
            bb.Append(
                "{"
                    + Convert.ToString(min, NumberFormatInfo.InvariantInfo)
                    + ", "
                    + Convert.ToString(max, NumberFormatInfo.InvariantInfo)
                    + "}"
            );
        }
    }
    ...
    sealed class LeafRangeNode : LeafNode
    {
        ...
        {
            get { return nextIteration; }
            set { nextIteration = value; }
        }
    ...
    class ContentValidator
    {
        ...
        public bool PreserveWhitespace
        {
            get
            {
                return contentType == XmlSchemaContentType.TextOnly
        ...
        public ContentValidator Finish(bool useDFA)
        {
            ...
            if (contentNode == null)
            {
                if (ContentType == XmlSchemaContentType.Mixed)
                {
                    ...
                    Debug.WriteLineIf(
                        DiagnosticsSwitches.XmlSchemaContentModel.Enabled,
                        "\t\t\tContentType:  " + ctype
                    );
            ...
            LeafNode endMarker = new LeafNode(
                positions.Add(symbols.AddName(XmlQualifiedName.Empty, null), null)
            );
        ...
        private void Dump(StringBuilder bb, BitSet[] followpos, int[][] transitionTable)
        {
            ...
            for (int i = 0; i < positions.Count; i++)
            {
                bb.AppendLine(
                    i
                        + " "
                        + positions[i].symbol.ToString(NumberFormatInfo.InvariantInfo)
                        + " "
                        + symbols.NameOf(positions[i].symbol)
                );
            }
...
#if RANGE_DEBUG
            WriteRunningPositions(
                "Current running positions to match",
                runningPositions,
                name,
                matchCount
            );
            ...
            { //There is a match
...
#if RANGE_DEBUG
                    WriteRunningPositions(
                        "Removing unmatched entries till the first match",
                        runningPositions,
                ...
                while (k < matchCount)
                {
                    ...
                    { //If match found, get the follow positions of the current matched position
                        ...
                        SequenceNode.WriteBitSet(rposInfo.curpos);
                        Debug.WriteIf(
                            DiagnosticsSwitches.XmlSchemaContentModel.Enabled,
                            "Follow pos of Matched position: "
                        );
                        SequenceNode.WriteBitSet(followpos[pos]);
#endif
            ...
            if (matchCount > 0)
            {
                ...
                for (k = matchCount - 1; k >= 0; k--)
                {
                    ...
                    hasSeenFinalPosition =
                        hasSeenFinalPosition || currentRunningPosition.Get(endMarkerPos); //Accepting position reached if the current position BitSet contains the endPosition
                    while (
                        matchCount < 10000
                        && currentRunningPosition.Intersects(positionsWithRangeTerminals)
                    )
                    {
...
#if RANGE_DEBUG
        private void WriteRunningPositions(
            string text,
                ...
                for (int rcnt = 0; rcnt < runningPositions[counter].rangeCounters.Length; rcnt++)
                {
                    Debug.WriteIf(
                        ...
                        "RangeCounter["
                            + rcnt
                            + "]"
                            + runningPositions[counter].rangeCounters[rcnt]
                            + " "
                    );
                }
                Debug.WriteLineIf(DiagnosticsSwitches.XmlSchemaContentModel.Enabled, "");
    ...
    sealed class AllElementsContentValidator : ContentValidator
    {
        ...
        {
            if (
                context.CurrentState.AllElementsRequired == countRequired
        ...
        public override ArrayList ExpectedElements(ValidationState context, bool isRequiredOnly)
        {
            ...
            foreach (DictionaryEntry entry in elements)
            {
                if (
                    ...
                    && (!isRequiredOnly || isRequired[(int)entry.Value])
                )
                {
                    if (names == null)
                    {
            ...
            foreach (DictionaryEntry entry in elements)
            {
                ...
                )
                {
                    AddParticleToExpected(
                        this.particles[(int)entry.Value] as XmlSchemaParticle,
                        schemaSet,
                        expectedParticles
                    );
                }
