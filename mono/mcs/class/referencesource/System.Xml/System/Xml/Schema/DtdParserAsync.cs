...
namespace System.Xml
{
    internal partial class DtdParser : IDtdParser
    {
        ...
        async Task<IDtdInfo> IDtdParser.ParseFreeFloatingDtdAsync(
            string baseUri,
            string docTypeName,
            string publicId,
            string systemId,
        ...
        private async Task ParseExternalSubsetAsync()
        {
            ...
            if (
                !await readerAdapter
                    .PushExternalSubsetAsync(systemId, publicId)
                    .ConfigureAwait(false)
            )
            {
                return;
...
#if DEBUG
            Debug.Assert(
                readerAdapter.EntityStackLength == 0
                ...
                switch (token)
                {
                    ...
                    case Token.RightBracket:
                        if (ParsingInternalSubset)
                        {
                            ...
                            // check '>'
                            if (
                                await GetTokenAsync(false).ConfigureAwait(false)
                                != Token.GreaterThan
                            )
                            {
        ...
        private async Task ParseAttlistDeclAsync()
        {
            ...
            for (; ; )
            {
                    ...
                    case Token.GreaterThan:
                        ...
                        if (v1Compat)
                        {
                            ...
                            )
                            {
                                ...
                                if (validate)
                                {
                                    attrDef.CheckXmlSpace(
                                        readerAdapterWithValidation.ValidationEventHandling
                                    );
                                }
                            }
                        }
                ...
                await ParseAttlistTypeAsync(attrDef, elementDecl, attrDefAlreadyExists)
                    .ConfigureAwait(false);
        ...
        )
        {
            ...
            if (IsAttributeValueType(token))
            {
                    ...
                    case Token.ID:
                        ...
                        if (validate && elementDecl.IsIdDeclared)
                        {
                            ...
                            )
                            {
                                SendValidationEvent(
                                    XmlSeverityType.Error,
                                    Res.Sch_IdAttrDeclared,
                                    elementDecl.Name.ToString()
                                );
                            }
                        }
#endif
                ...
                if (validate)
                {
                    ...
                    {
                        if (
                            elementDecl.ContentValidator != null
                        ...
                        )
                        {
                            SendValidationEvent(
                                curPos - 8,
                                XmlSeverityType.Error,
                                Res.Sch_NotationAttributeOnEmptyElement,
                                elementDecl.Name.ToString()
                            ); // 8 == strlen("NOTATION")
                        }
                ...
                for (; ; )
                {
                    ...
                    )
                    {
                        SendValidationEvent(
                            XmlSeverityType.Error,
                            new XmlSchemaException(
                                Res.Xml_AttlistDuplNotationValue,
                                notationName,
                                BaseUriStr,
                                (int)LineNo,
                                (int)LinePos
        ...
        private async Task ParseElementDeclAsync()
        {
            ...
            if (schemaInfo.ElementDecls.TryGetValue(name, out elementDecl))
            {
                ...
                {
                    SendValidationEvent(
                        curPos - name.Name.Length,
                        XmlSeverityType.Error,
                        Res.Sch_DupElementDecl,
                        GetNameString()
                    );
                }
#endif
                ...
                case Token.LeftParen:
                    ...
                    switch (await GetTokenAsync(false).ConfigureAwait(false))
                    {
                        case Token.PCDATA:
                        {
                            ...
                            await ParseElementMixedContentAsync(pcv, startParenEntityId)
                                .ConfigureAwait(false);
                        ...
                        case Token.None:
                        {
                            ...
                            await ParseElementOnlyContentAsync(pcv, startParenEntityId)
                                .ConfigureAwait(false);
        ...
        private async Task ParseElementOnlyContentNoValidationAsync()
        {
            ...
            switch (await GetTokenAsync(false).ConfigureAwait(false))
            {
                ...
                case Token.LeftParen:
                    ...
                    //   instead.
                    currentFrame = new ParseElementOnlyContentNoValidation_LocalFrame();
                    localFrames.Push(currentFrame);
                    goto RecursiveCall;
        ...
        private async Task ParseElementMixedContentNoValidationAsync()
        {
            ...
            for (; ; )
            {
                switch (await GetTokenAsync(false).ConfigureAwait(false))
                {
                    case Token.RightParen:
                        if (
                            await GetTokenAsync(false).ConfigureAwait(false) != Token.Star
                            && hasNames
                        )
                        {
                            ThrowUnexpectedToken(curPos, "*");
        ...
        private async Task ParseElementOnlyContentAsync(
            ParticleContentValidator pcv,
            int startParenEntityId
        )
        {
            Stack<ParseElementOnlyContent_LocalFrame> localFrames =
                new Stack<ParseElementOnlyContent_LocalFrame>();
            ParseElementOnlyContent_LocalFrame currentFrame =
                new ParseElementOnlyContent_LocalFrame(startParenEntityId);
            localFrames.Push(currentFrame);
        ...
        private async Task ParseElementMixedContentAsync(
            ParticleContentValidator pcv,
            int startParenEntityId
        )
        {
        ...
        private async Task ParseEntityDeclAsync()
        {
            ...
            switch (await GetTokenAsync(true).ConfigureAwait(false))
            {
                ...
                case Token.Name:
                    ...
                    entity.DeclaredURI =
                        (externalDtdBaseUri.Length == 0) ? documentBaseUri : externalDtdBaseUri;
        ...
        private async Task ParseNotationDeclAsync()
        {
            ...
            else
            {
                ...
                {
                    SendValidationEvent(
                        curPos - notationName.Name.Length,
                        XmlSeverityType.Error,
                        Res.Sch_DupNotation,
                        notationName.Name
                    );
        ...
        private async Task ParseCommentAsync()
        {
            ...
            try
            {
                ...
                {
                    await readerAdapter
                        .ParseCommentAsync(internalSubsetValueSb)
                        .ConfigureAwait(false);
        ...
        private async Task<Tuple<string, string>> ParseExternalIdAsync(
            Token idTokenType,
            Token declType
        )
        {
            ...
            if (idTokenType == Token.SYSTEM)
            {
                ...
                {
                    Throw(
                        curPos - systemId.Length - 1,
                        Res.Xml_FragmentId,
                        new string[] { systemId.Substring(systemId.IndexOf('#')), systemId }
                    );
                }
            ...
            else
            {
                ...
                if (declType == Token.DOCTYPE && !freeFloatingDtd)
                {
                    ...
                    if (await GetTokenAsync(false).ConfigureAwait(false) == Token.Literal)
                    {
                        ...
                        {
                            Throw(
                                Res.Xml_ExpectingWhiteSpace,
                                new string(literalQuoteChar, 1),
                                (int)literalLineInfo.lineNo,
                                (int)literalLineInfo.linePos
                            );
                        }
                        systemId = GetValue();
                        literalLineInfo.linePos++;
                    ...
                    default:
                        ...
                        switch (scanningFunction)
                        {
                            case ScanningFunction.Name:
                                return await ScanNameExpectedAsync().ConfigureAwait(false);
                            case ScanningFunction.QName:
                                return await ScanQNameExpectedAsync().ConfigureAwait(false);
                            case ScanningFunction.Nmtoken:
                                return await ScanNmtokenExpectedAsync().ConfigureAwait(false);
                            case ScanningFunction.SubsetContent:
                                return await ScanSubsetContentAsync().ConfigureAwait(false);
                            case ScanningFunction.Doctype1:
                                return await ScanDoctype1Async().ConfigureAwait(false);
                            ...
                            case ScanningFunction.Element2:
                                return await ScanElement2Async().ConfigureAwait(false);
                            case ScanningFunction.Element3:
                                return await ScanElement3Async().ConfigureAwait(false);
                            ...
                            case ScanningFunction.Attlist1:
                                return await ScanAttlist1Async().ConfigureAwait(false);
                            case ScanningFunction.Attlist2:
                                return await ScanAttlist2Async().ConfigureAwait(false);
                            ...
                            case ScanningFunction.Attlist6:
                                return await ScanAttlist6Async().ConfigureAwait(false);
                            case ScanningFunction.Attlist7:
                                return ScanAttlist7();
                            case ScanningFunction.Notation1:
                                return await ScanNotation1Async().ConfigureAwait(false);
                            case ScanningFunction.SystemId:
                                return await ScanSystemIdAsync().ConfigureAwait(false);
                            case ScanningFunction.PublicId1:
                                return await ScanPublicId1Async().ConfigureAwait(false);
                            case ScanningFunction.PublicId2:
                                return await ScanPublicId2Async().ConfigureAwait(false);
                            case ScanningFunction.Entity1:
                                return await ScanEntity1Async().ConfigureAwait(false);
                            case ScanningFunction.Entity2:
                                return await ScanEntity2Async().ConfigureAwait(false);
                            case ScanningFunction.Entity3:
                                return await ScanEntity3Async().ConfigureAwait(false);
                            case ScanningFunction.CondSection1:
                                return await ScanCondSection1Async().ConfigureAwait(false);
                            case ScanningFunction.CondSection2:
                                return ScanCondSection2();
                            case ScanningFunction.CondSection3:
                                return await ScanCondSection3Async().ConfigureAwait(false);
        ...
        private async Task<Token> ScanLiteralAsync(LiteralType literalType)
        {
            ...
            for (; ; )
            {
                ...
                {
                    while (
                        (xmlCharType.charProperties[chars[curPos]] & XmlCharType.fAttrValue) != 0
                        && chars[curPos] != '%'
                    )
                    {
                        curPos++;
                ...
                switch (chars[curPos])
                {
                            ...
                            if (Normalize)
                            {
                                if (literalType == LiteralType.AttributeValue)
                                {
                                    stringBuilder.Append(
                                        readerAdapter.IsEntityEolNormalized
                                            ? "\u0020\u0020"
                                            : "\u0020"
                    ...
                    case '&':
                        ...
                        if (chars[curPos + 1] == '#')
                        {
                            ...
                            int endPos = await readerAdapter
                                .ParseNumericCharRefAsync(
                                    SaveInternalSubsetValue ? internalSubsetValueSb : null
                                )
                                .ConfigureAwait(false);
                            LoadParsingBuffer();
                        ...
                        else
                        {
                            ...
                            {
                                int endPos = await readerAdapter
                                    .ParseNamedCharRefAsync(false, null)
                                    .ConfigureAwait(false);
                                LoadParsingBuffer();
                ...
                switch (chars[curPos])
                {
                    case 'N':
                        ...
                        if (
                            ...
                            || chars[curPos + 4] != 'D'
                            || chars[curPos + 5] != 'E'
                            || xmlCharType.IsNameSingleChar(chars[curPos + 6])
        ...
        private async Task<Token> ScanCondSection3Async()
        {
            ...
            for (; ; )
            {
                ...
                {
                    while (
                        (xmlCharType.charProperties[chars[curPos]] & XmlCharType.fText) != 0
                        && chars[curPos] != ']'
                    )
                    {
                        curPos++;
        ...
        private async Task ScanQNameAsync(bool isQName)
        {
            ...
            for (; ; )
            {
...
#else
                    if (
                        (xmlCharType.charProperties[chars[curPos]] & XmlCharType.fNCStartNameSC)
                            != 0
                        || chars[curPos] == ':'
                    )
                    { // if ( xmlCharType.IsStartNCNameSingleChar(chars[curPos]) || chars[curPos] == ':' ) {
                        ...
                        curPos + 1 < charsUsed
                        && xmlCharType.IsNCNameSurrogateChar(chars[curPos + 1], chars[curPos])
                    )
                    {
                        curPos += 2;
                    }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            Throw(
                                ...
                                XmlException.BuildCharExceptionArgs(chars, charsUsed, curPos)
                            );
                        }
                    }
                }
                ...
                unsafe
                {
                    for (; ; )
                    {
...
#else
                        if (
                            (xmlCharType.charProperties[chars[curPos]] & XmlCharType.fNCNameSC) != 0
                        )
                        { // while ( xmlCharType.IsNCNameSingleChar(chars[curPos]) ) {
                            ...
                            curPos + 1 < charsUsed
                            && xmlCharType.IsNameSurrogateChar(chars[curPos + 1], chars[curPos])
                        )
                        {
                            curPos += 2;
                        }
                ...
                // end of buffer
                    ...
                    || (
                        curPos + 1 == charsUsed
                        && xmlCharType.IsNCNameHighSurrogateChar(chars[curPos])
                    )
#endif
                )
                {
        ...
        // Entity handling
        //
        private Task<bool> HandleEntityReferenceAsync(
            bool paramEntity,
            bool inLiteral,
            bool inAttribute
        )
        {
            ...
            return HandleEntityReferenceAsync(
                ScanEntityName(),
                paramEntity,
                inLiteral,
                inAttribute
        ...
        private async Task<bool> HandleEntityReferenceAsync(
            XmlQualifiedName entityName,
            bool paramEntity,
            bool inLiteral,
            bool inAttribute
        )
        {
            ...
            {
                Throw(
                    curPos - entityName.Name.Length - 1,
                    paramEntity ? Res.Xml_RecursiveParEntity : Res.Xml_RecursiveGenEntity,
                    entityName.Name
                );
            }
