    ...
    public sealed class XmlSchemaInference
    {
        ...
        internal XmlSchemaSet InferSchema1(XmlReader instanceDocument, XmlSchemaSet schemas)
        {
            ...
            if (xtr.NodeType == XmlNodeType.Element)
            {
                ...
                {
                    if (
                        elem.Name == xtr.LocalName
                        && elem.QualifiedName.Namespace == xtr.NamespaceURI
                    )
                    {
        ...
        )
        {
            ...
            else
            {
                    ...
                    /*if (parentSchema.AttributeFormDefault = XmlSchemaForm.Unqualified && childURI.Length == 0)
                {
                    ...
                    if (
                        prefix.Length != 0
                        && String.Compare(prefix, "xml", StringComparison.OrdinalIgnoreCase) != 0
                    )
        ...
        )
        {
            ...
            do
            {
                ...
                && (xtr.NodeType != XmlNodeType.CDATA)
                && (xtr.NodeType != XmlNodeType.Element)
            ...
            while (!xtr.EOF && (xtr.NodeType != XmlNodeType.EndElement))
            {
                            ...
                            if (iChildNumber == 1)
                            {
                                ...
                                do
                                {
                                    ...
                                    && (
                                        (xtr.NodeType == XmlNodeType.CDATA)
                                        || (xtr.NodeType == XmlNodeType.Text)
                                        || (xtr.NodeType == XmlNodeType.Comment)
                                        || (xtr.NodeType == XmlNodeType.ProcessingInstruction)
                                        || (xtr.NodeType == XmlNodeType.Whitespace)
                                        || (xtr.NodeType == XmlNodeType.SignificantWhitespace)
                                        || (xtr.NodeType == XmlNodeType.XmlDeclaration)
                                    )
                                );
                ...
                else if (xtr.NodeType == XmlNodeType.Element)
                {
                    ...
                    else
                    {
                        ...
                        XmlSchemaElement subelement = FindMatchingElement(
                            bCreatingNewType || bCreatingNewSequence,
                            xtr,
                            ct,
                            ref lastUsedSeqItem,
                            ref bParticleChanged,
                            parentSchema,
                            Maxoccursflag
                        );
                    }
        ...
        private XmlSchemaType GetEffectiveSchemaType(XmlSchemaElement elem, bool bCreatingNewType)
        {
            ...
            { //creating new type, hence look up pre-compiled info
                ...
                {
                    effectiveSchemaType =
                        schemaSet.GlobalTypes[elem.SchemaTypeName] as XmlSchemaType;
                    if (effectiveSchemaType == null)
                    {
                        effectiveSchemaType = XmlSchemaType.GetBuiltInSimpleType(
                            elem.SchemaTypeName
                        );
                    }
                    if (effectiveSchemaType == null)
                    {
                        effectiveSchemaType = XmlSchemaType.GetBuiltInComplexType(
                            elem.SchemaTypeName
                        );
                    }
                }
            ...
            if (ct.Particle.GetType() == typeof(XmlSchemaSequence))
            {
                ...
                { // <sequence minOccurs="0" maxOccurs="unbounded"><choice><element>...</choice></sequence>
                    ...
                    for (int i = 0; i < xsch.Items.Count; ++i)
                    {
                        ...
                        if (
                            (el.Name == xtr.LocalName) && (parentSchema.TargetNamespace == childURI)
                        )
        ...
        internal XmlSchemaAttribute FindAttributeRef(
