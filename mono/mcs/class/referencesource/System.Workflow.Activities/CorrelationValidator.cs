...
namespace System.Workflow.Activities
{
    internal static class CorrelationSetsValidator
    {
        internal static ValidationErrorCollection Validate(ValidationManager manager, Object obj)
        {
            ...
            if (
                !(activity is CallExternalMethodActivity)
            ...
            Type interfaceType =
                (activity is CallExternalMethodActivity)
                    ? ((CallExternalMethodActivity)activity).InterfaceType
            ...
            if (interfaceType.ContainsGenericParameters)
            {
                ValidationError error = new ValidationError(
                    string.Format(
                        ...
                        SR.GetString(SR.Error_GenericMethodsNotSupported),
                        interfaceType.FullName
                    ),
                    ErrorNumbers.Error_GenericMethodsNotSupported
                );
            ...
            object[] attributes = interfaceType.GetCustomAttributes(
            ...
            if (attributes.Length == 0)
            {
                ValidationError error = new ValidationError(
                    string.Format(
                        CultureInfo.CurrentCulture,
                        SR.GetString(SR.Error_ExternalDataExchangeException),
                        interfaceType.FullName
                    ),
                    ErrorNumbers.Error_TypeNotExist
                );
                error.PropertyName = "InterfaceType";
                validationErrors.Add(error);
                return validationErrors;
            ...
            {
                ValidationErrorCollection interfaceErrors = ValidateHostInterface(
                    manager,
                    interfaceType,
                    activity
                );
            ...
            {
                if (
                    ((CallExternalMethodActivity)activity).MethodName == null
                    || ((CallExternalMethodActivity)activity).MethodName.Length == 0
                )
                    return validationErrors;

                MethodInfo methodInfo = interfaceType.GetMethod(
                    ((CallExternalMethodActivity)activity).MethodName,
                ...
                if (methodInfo == null || methodInfo.IsSpecialName)
                {
                    validationErrors.Add(
                        new ValidationError(
                            SR.GetString(
                                SR.Error_MissingMethodName,
                                activity.Name,
                                ((CallExternalMethodActivity)activity).MethodName
                            ),
                            ErrorNumbers.Error_MissingMethodName
                        )
                    );
                    return validationErrors;
                }
            ...
            {
                if (
                    ((HandleExternalEventActivity)activity).EventName == null
                    || ((HandleExternalEventActivity)activity).EventName.Length == 0
                )
                    return validationErrors;

                EventInfo eventInfo = interfaceType.GetEvent(
                    ((HandleExternalEventActivity)activity).EventName,
                    BindingFlags.Instance | BindingFlags.Public
                );
                if (eventInfo == null)
                {
            ...
            attributes = interfaceType.GetCustomAttributes(
            ...
            CorrelationToken correlator =
                activity.GetValue(
                    (activity is CallExternalMethodActivity)
                        ? CallExternalMethodActivity.CorrelationTokenProperty
                        : HandleExternalEventActivity.CorrelationTokenProperty
                ) as CorrelationToken;

            object[] correlationParameterAttributes = interfaceType.GetCustomAttributes(
                    ...
                    validationErrors.Add(
                        new ValidationError(
                            string.Format(
                                CultureInfo.CurrentCulture,
                                SR.GetString(
                                    SR.Error_CorrelationTokenSpecifiedForUncorrelatedInterface
                                ),
            ...
            if (correlator == null || String.IsNullOrEmpty(correlator.Name))
            {
                validationErrors.Add(
                    new ValidationError(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            SR.GetString(SR.Error_MissingCorrelationTokenProperty),
                            activity.QualifiedName
                        ),
                        ErrorNumbers.Error_ParameterPropertyNotSet,
                        false,
                        "CorrelationToken"
                    )
                );
            ...
            if (String.IsNullOrEmpty(correlator.OwnerActivityName))
            {
                validationErrors.Add(
                    new ValidationError(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            SR.GetString(SR.Error_MissingCorrelationTokenOwnerNameProperty),
                            activity.QualifiedName
                        ),
                        ErrorNumbers.Error_ParameterPropertyNotSet,
                        false,
            ...
            if (sourceActivity == null)
                sourceActivity = Helpers.ParseActivityForBind(
                    activity,
                    correlator.OwnerActivityName
                );
            if (sourceActivity != null)
            ...
            if (!ownerIsParent)
            {
                ValidationError error = new ValidationError(
                    string.Format(
                        CultureInfo.CurrentCulture,
                        SR.GetString(SR.Error_OwnerActivityIsNotParent),
                        activity.QualifiedName
                    ),
                    ErrorNumbers.Error_ParameterPropertyNotSet
                );
            ...
            attributes =
                targetMember.GetCustomAttributes(typeof(CorrelationInitializerAttribute), false)
                as object[];
            if (attributes.Length > 0)
                initializer = true;
            ...
            if (initializer)
            {
                if (replicatorParent != null && activity is HandleExternalEventActivity)
                {
                    ValidationError error = new ValidationError(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            SR.GetString(SR.Error_InitializerInReplicator),
                            replicatorParent.QualifiedName
                        ),
                        ErrorNumbers.Error_InitializerInReplicator,
                        false
                    );
                    error.PropertyName = "CorrelationToken";
                    validationErrors.Add(error);
                    ...
                    if (!isValid)
                    {
                        ValidationError error = new ValidationError(
                            string.Format(
                                CultureInfo.CurrentCulture,
                                SR.GetString(
                                    SR.Error_CorrelationTokenInReplicator,
                                    correlator.Name,
                                    replicatorParent.QualifiedName
                                )
                            ),
                            ErrorNumbers.Error_CorrelationTokenInReplicator,
                            true
                        );
                        error.PropertyName = "CorrelationToken";
                        validationErrors.Add(error);
                ...
                //if (rootActivity is RootActivity)
                {
                    if (!initializer)
                    {
                        ...
                        walker.FoundActivity += delegate(Walker w, WalkerEventArgs args)
                        {
                            ...
                            if (
                                !(currentActivity is CallExternalMethodActivity)
                                && !(currentActivity is HandleExternalEventActivity)
                            )
                                return;

                            CorrelationToken existingCorrelationTokenValue =
                            ...
                            attributes =
                                existingTargetMember.GetCustomAttributes(
                            ...
                            if (activity is HandleExternalEventActivity)
                            {
                                ...
                                if (
                                    txnlParent != null
                                    && IsFollowerInTxnlScope(txnlParent, activity)
                                )
                                    initFollowerInTxnlScope = true;
                            }

                            string existingQualifiedCorrelationToken = null;
                            sourceActivity = activity.GetActivityByName(
                                existingCorrelationTokenValue.OwnerActivityName
                            );
                            if (sourceActivity == null)
                                sourceActivity = Helpers.ParseActivityForBind(
                                    activity,
                            ...
                            if (
                                (correlator.Name == existingCorrelationTokenValue.Name)
                                && IsOwnerActivitySame(
                                    correlator.OwnerActivityName,
                                    existingCorrelationTokenValue.OwnerActivityName,
                                    activity,
                        ...
                        if (!isValid)
                        {
                            ValidationError error = new ValidationError(
                                string.Format(
                                    CultureInfo.CurrentCulture,
                                    SR.GetString(SR.Error_UninitializedCorrelation)
                                ),
                            ...
                            if (ownerNameValidated)
                            {
                                error = new ValidationError(
                                    string.Format(
                                        CultureInfo.CurrentCulture,
                                        SR.GetString(
                                            SR.Error_MisMatchCorrelationTokenOwnerNameProperty
                                        ),
                                        correlator.Name
                                    ),
                                    ErrorNumbers.Error_UninitializedCorrelation,
                        ...
                        if (initFollowerInTxnlScope)
                        {
                            ValidationError error = new ValidationError(
                                string.Format(
                                    CultureInfo.CurrentCulture,
                                    SR.GetString(SR.Error_InitializerFollowerInTxnlScope)
                                ),
                                ErrorNumbers.Error_InitializerFollowerInTxnlScope,
                                false
                            );
                            error.PropertyName = "CorrelationToken";
                            validationErrors.Add(error);
        ...
        private static Activity GetTransactionalScopeParent(Activity activity)
        {
            ...
            {
                if (
                    parent is CompensatableTransactionScopeActivity
                    || parent is TransactionScopeActivity
                )
                {
                    return parent;
                }
        ...
        private static bool IsOwnerActivitySame(
            string ownerActivityName,
            string existingOwnerActivityName,
        ...
        )
        {
            ...
            if (existingowner == null)
                existingowner = Helpers.ParseActivityForBind(
                    existingActivity,
            ...
            if (
                owner != null
                && existingowner != null
                && owner.QualifiedName.Equals(existingowner.QualifiedName)
            )
                return true;
        ...
        private static ValidationErrorCollection ValidateHostInterface(
            IServiceProvider serviceProvider,
            Type interfaceType,
            Activity activity
        )
        {
            Dictionary<Type, ValidationErrorCollection> typesValidated =
                serviceProvider.GetService(typeof(Dictionary<Type, ValidationErrorCollection>))
                as Dictionary<Type, ValidationErrorCollection>;
            ...
            if (typesValidated == null)
            {
                ...
                if (serviceContainer != null)
                    serviceContainer.AddService(
                        typeof(Dictionary<Type, ValidationErrorCollection>),
                        typesValidated
                    );
            }
            ...
            if (attributes.Length == 0)
            {
                object[] dsAttribs = interfaceType.GetCustomAttributes(
                    typeof(ExternalDataExchangeAttribute),
                    false
                );
                ...
                {
                    typesValidated[interfaceType]
                        .AddRange(ValidateHostInterfaceMembers(interfaceType, activity));
                    typesValidated[interfaceType]
                        .AddRange(ValidateHostInterfaceAttributes(interfaceType));
                }
                else
                {
                    typesValidated[interfaceType]
                        .AddRange(ValidateInvalidHostInterfaceAttributes(interfaceType));
                }
            }
        ...
        private static ValidationErrorCollection ValidateHostInterfaceMembers(
            Type interfaceType,
            Activity activity
        )
        {
            ...
            foreach (MemberInfo memberInfo in interfaceType.GetMembers())
            {
                ...
                if (methodInfo.IsGenericMethod)
                {
                    ValidationError error = new ValidationError(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            SR.GetString(SR.Error_GenericMethodsNotSupported),
                            (memberInfo is EventInfo) ? delegateType.Name : methodInfo.Name
                        ),
                        ErrorNumbers.Error_GenericMethodsNotSupported
                    );
                    if (memberInfo is EventInfo)
                ...
                if (methodInfo.ReturnType != typeof(void) && (memberInfo is EventInfo))
                {
                    ValidationError error = new ValidationError(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            SR.GetString(SR.Error_ReturnTypeNotVoid),
                            (memberInfo is EventInfo) ? delegateType.Name : methodInfo.Name
                        ),
                        ErrorNumbers.Error_ReturnTypeNotVoid
                    );
                    if (memberInfo is EventInfo)
                        error.UserData.Add(typeof(EventInfo), ((EventInfo)memberInfo).Name);
                    else
                ...
                foreach (ParameterInfo param in methodInfo.GetParameters())
                {
                    if (param.IsOut || param.IsRetval)
                    {
                        ValidationError error = new ValidationError(
                            string.Format(
                                CultureInfo.CurrentCulture,
                                SR.GetString(SR.Error_OutRefParameterNotSupported),
                                (memberInfo is EventInfo) ? delegateType.Name : methodInfo.Name,
                                param.Name
                            ),
        ...
        private static ValidationErrorCollection ValidateHostInterfaceAttributes(Type interfaceType)
        {
            ...
            foreach (
                object interfaceAttribute in interfaceType.GetCustomAttributes(
                    typeof(CorrelationParameterAttribute),
                    false
                )
            )
            {
                CorrelationParameterAttribute parameterAttribute =
                    Helpers.GetAttributeFromObject<CorrelationParameterAttribute>(
                        interfaceAttribute
                    );
                if (
                    String.IsNullOrEmpty(parameterAttribute.Name)
                    || parameterAttribute.Name.Trim().Length == 0
                )
                {
                    ValidationError error = new ValidationError(
                        SR.GetString(
                            CultureInfo.CurrentCulture,
                            SR.Error_CorrelationAttributeInvalid,
                            typeof(CorrelationParameterAttribute).Name,
                            "Name",
                            interfaceType.Name
                        ),
                        ErrorNumbers.Error_CorrelationAttributeInvalid
                    );
                ...
                if (parameterAttrs.Contains(parameterAttribute.Name))
                {
                    ValidationError error = new ValidationError(
                        SR.GetString(
                            CultureInfo.CurrentCulture,
                            SR.Error_DuplicateCorrelationAttribute,
                            typeof(CorrelationParameterAttribute).Name,
                            parameterAttribute.Name,
                            interfaceType.Name
                        ),
                        ErrorNumbers.Error_DuplicateCorrelationAttribute
                    );
                    error.UserData.Add(typeof(CorrelationParameterAttribute), interfaceType.Name);
                    validationErrors.Add(error);
                    continue;
            ...
            foreach (MemberInfo memberInfo in interfaceType.GetMembers())
            {
                if (memberInfo is MethodInfo && !((MethodInfo)memberInfo).IsSpecialName)
                {
                    ...
                    int aliasLength = memberInfo
                        .GetCustomAttributes(typeof(CorrelationInitializerAttribute), false)
                        .Length;
                    ...
                    if (aliasLength > 0)
                    {
                        foreach (string paramName in parameterAttrs)
                        {
                            ...
                            if (correlationAliasAttrs.Contains(paramName))
                                paramPath = (
                                    (CorrelationAliasAttribute)correlationAliasAttrs[paramName]
                                ).Path;
                ...
                else if (memberInfo is EventInfo)
                {
                    ...
                    FillCorrelationAliasAttrs(
                        delegateType,
                        delegateCorrelationAliasAttrs,
                        validationErrors
                    );
                    if (delegateTypeCorrelationAliasAttrs[delegateType] == null)
                        delegateTypeCorrelationAliasAttrs.Add(
                            delegateType,
                            delegateCorrelationAliasAttrs
                        );
                ...
                foreach (string paramName in correlationAliasAttrs.Keys)
                {
                    if (!parameterAttrs.Contains(paramName))
                    {
                        ...
                        if (
                            ...
                            && (
                                (Hashtable)
                                    delegateTypeCorrelationAliasAttrs[
                                        Helpers.GetDelegateFromEvent((EventInfo)memberInfo)
                                    ]
                            )[paramName] != null
                        )
                            continue;
                        ...
                        ValidationError error = new ValidationError(
                            SR.GetString(
                                CultureInfo.CurrentCulture,
                                SR.Error_CorrelationParameterNotFound,
                                typeof(CorrelationAliasAttribute).Name,
                                paramName,
                                memberInfo.Name,
                                typeof(CorrelationParameterAttribute).Name,
                                interfaceType.Name
                            ),
                            ErrorNumbers.Error_CorrelationParameterNotFound
                        );
            ...
            foreach (string paramName in parameterAttrs)
            {
                foreach (DictionaryEntry memberEntry in memberInfoCorrelationAliasAttrs)
                {
                    ...
                    if (paramType == null)
                    {
                        ...
                        paramTypes.ContainsKey(paramName)
                        && (Type)paramTypes[paramName] != paramType
                    )
                    {
                        ...
                        ValidationError error = new ValidationError(
                            SR.GetString(
                                CultureInfo.CurrentCulture,
                                SR.Error_CorrelationTypeNotConsistent,
                                paramPath,
                                typeof(CorrelationAliasAttribute).Name,
                                (memberInfo.DeclaringType == interfaceType)
                                    ? memberInfo.Name
                                    : memberInfo.DeclaringType.Name,
                                paramType.Name,
                                ((Type)paramTypes[paramName]).Name,
                                paramName,
                                interfaceType.Name
                            ),
            ...
            if (initializerCount == 0)
            {
                ValidationError error = new ValidationError(
                    SR.GetString(
                        CultureInfo.CurrentCulture,
                        SR.Error_CorrelationInitializerNotDefinied,
                        interfaceType.Name
                    ),
                    ErrorNumbers.Error_CorrelationInitializerNotDefinied
                );
        ...
        private static ValidationErrorCollection ValidateInvalidHostInterfaceAttributes(
            Type interfaceType
        )
        {
            if (interfaceType == null)
            ...
            foreach (MemberInfo memberInfo in interfaceType.GetMembers())
            {
                if (
                    memberInfo
                        ...
                        .Length != 0
                    || memberInfo
                        .GetCustomAttributes(typeof(CorrelationAliasAttribute), false)
                        .Length != 0
                )
                {
                    corrAttrsFound = true;
            ...
            if (corrAttrsFound)
            {
                ValidationError error = new ValidationError(
                    SR.GetString(
                        CultureInfo.CurrentCulture,
                        SR.Error_MissingCorrelationParameterAttribute,
                        interfaceType.Name
                    ),
                    ErrorNumbers.Error_MissingCorrelationParameterAttribute
                );
                error.UserData.Add(typeof(CorrelationParameterAttribute), interfaceType.Name);
                validationErrors.Add(error);
            }
        ...
        private static void FillCorrelationAliasAttrs(
            MemberInfo memberInfo,
            Hashtable correlationAliasAttrs,
            ValidationErrorCollection validationErrors
        )
        {
            foreach (
                object memberAttribute in memberInfo.GetCustomAttributes(
                    typeof(CorrelationAliasAttribute),
                    false
                )
            )
            {
                CorrelationAliasAttribute aliasAttribute =
                    Helpers.GetAttributeFromObject<CorrelationAliasAttribute>(memberAttribute);
                // fill validation errors, name, path, duplicate check
                if (
                    String.IsNullOrEmpty(aliasAttribute.Name)
                    || aliasAttribute.Name.Trim().Length == 0
                )
                {
                    ValidationError error = new ValidationError(
                        SR.GetString(
                ...
                if (
                    String.IsNullOrEmpty(aliasAttribute.Path)
                    || aliasAttribute.Path.Trim().Length == 0
                )
                {
                    ValidationError error = new ValidationError(
                        SR.GetString(
        ...
        private static Type GetCorrelationParameterType(
            string parameterPropertyName,
            object parametersCollection
        )
        {
            ...
            if (parsedPropertyName.Length == 1)
            {
                ...
                {
                    foreach (
                        CodeParameterDeclarationExpression parameterDeclaration in (CodeParameterDeclarationExpressionCollection)parametersCollection
                    )
                    {
                        if (
                            String.Compare("e", parameterDeclaration.Name, StringComparison.Ordinal)
                            == 0
                        )
            ...
            if (parametersCollection is CodeParameterDeclarationExpressionCollection)
            {
                ...
                )
                {
                    if (
                        String.Compare(
                            parsedPropertyName[0],
                            parameterDeclaration.Name,
                            StringComparison.Ordinal
                        ) == 0
                    )
                        correlationParameterType =
                            parameterDeclaration.UserData[typeof(Type)] as Type;
                }
            }
            else if (parametersCollection is ParameterInfo[])
            {
            ...
            for (
                index = 1;
                index < parsedPropertyName.Length && correlationParameterType != null;
                index++
            )
            {
    ...
    internal static class ParameterBindingValidator
    {
        internal static ValidationErrorCollection Validate(ValidationManager manager, Object obj)
        {
            ...
            WorkflowParameterBindingCollection parameterBinding =
                (activity is CallExternalMethodActivity)
                    ? ((CallExternalMethodActivity)activity).ParameterBindings
                    : ((HandleExternalEventActivity)activity).ParameterBindings;
        ...
        )
        {
            ...
            {
                if (
                    TypeProvider.IsAssignable(
                        typeof(ExternalDataEventArgs),
            ...
            if (isEvent && (!canBeIntercepted || parameters.Length != 2))
                validationErrors.Add(
                    new ValidationError(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            SR.GetString(SR.Error_InvalidEventArgsSignature, operation)
                        ),
            ...
            foreach (WorkflowParameterBinding parameterBinding in parameterBindings)
            {
                    ...
                    if (isEvent)
                        validationErrors.Add(
                            new ValidationError(
                                string.Format(
                                    CultureInfo.CurrentCulture,
                                    SR.GetString(SR.Error_InvalidEventPropertyName, paramName)
                                ),
                                ErrorNumbers.Error_FieldNotExists,
                                false,
                                "ParameterBindings"
                        ...
                        validationErrors.Add(
                            new ValidationError(
                                string.Format(
                                    CultureInfo.CurrentCulture,
                                    SR.GetString(SR.Error_InvalidMethodPropertyName, paramName)
                                ),
                                ErrorNumbers.Error_FieldNotExists,
                                false,
                                "ParameterBindings"
                ...
                if (parameterBinding.IsBindingSet(WorkflowParameterBinding.ValueProperty))
                    paramValue = parameterBinding.GetBinding(
                ...
                if (paramInfo != null)
                {
                    ...
                    ValidationErrorCollection variableErrors = ValidationHelpers.ValidateProperty(
                        manager,
                        activity,
                        paramValue,
        ...
        private static void ValidateParameterSerializabiltiy(
            ValidationErrorCollection validationErrors,
            Type type
        )
        {
            ...
            if (attrs.Length == 0 && serializableType == null)
            {
                validationErrors.Add(
                    new ValidationError(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            SR.GetString(SR.Error_EventArgumentValidationException),
                            type.FullName
                        ),
                        ErrorNumbers.Error_FieldNotExists,
                        false,
