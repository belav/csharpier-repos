...
namespace System.Workflow.Activities.Rules
{
    internal static class ConditionHelper
    {
        ...
        internal static RuleDefinitions Load_Rules_DT(
            IServiceProvider serviceProvider,
            DependencyObject activity
        )
        {
            RuleDefinitions rules = (RuleDefinitions)
                activity.GetValue(RuleDefinitions.RuleDefinitionsProperty);
            if (rules == null)
            {
                WorkflowDesignerLoader loader = (WorkflowDesignerLoader)
                ...
                if (loader != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(loader.FileName))
                        rulesFileName = Path.Combine(
                            Path.GetDirectoryName(loader.FileName),
                            Path.GetFileNameWithoutExtension(loader.FileName)
                        );
                    rulesFileName += ".rules";
                        ...
                        using (TextReader ruleFileReader = loader.GetFileReader(rulesFileName))
                        {
                            ...
                            else
                            {
                                using (XmlReader xmlReader = XmlReader.Create(ruleFileReader))
                                    rules =
                                        new WorkflowMarkupSerializer().Deserialize(xmlReader)
                                        as RuleDefinitions;
                            }
                        }
                    }
    ...
    internal static class EnumHelper
    {
        ...
        public static Type GetUnderlyingType(Type type)
        {
            ...
            ) // designTimeType = type as System.Workflow.ComponentModel.Compiler.DesignTimeType;
            {
                ...
                Debug.Assert(
                    methodInfo != null,
                    "Missing GetEnumType method on the DesignTimeType!"
                );
                if (methodInfo != null)
                {
