namespace System.Workflow.Activities
{
            ...
            IActivityEventListener<ActivityExecutionStatusChangedEventArgs>
    {
        ...
        )
        {
            ...
            List<ListenEventActivitySubscriber> eventActivitySubscribers =
                new List<ListenEventActivitySubscriber>();
            ...
            for (int i = 0; i < this.EnabledActivities.Count; ++i)
            {
                ...
                ListenEventActivitySubscriber eventActivitySubscriber =
                    new ListenEventActivitySubscriber(eventDriven);
        ...
        protected override ActivityExecutionStatus Cancel(ActivityExecutionContext executionContext)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        EventDrivenActivity eventDrivenChild =
                            this.EnabledActivities[i] as EventDrivenActivity;
        ...
        )
        {
            ...
            Debug.Assert(
                addedActivity is EventDrivenActivity,
                "Listen only contains EventDriven activities"
            );

            ListenActivity listen = executionContext.Activity as ListenActivity;

            if (
                listen.ExecutionStatus == ActivityExecutionStatus.Executing
                && listen.ActivityState != null
                && !listen.IsListenTrigerred
            )
            {
        ...
        static DependencyProperty IsListenTrigerredProperty = DependencyProperty.Register(
            "IsListenTrigerred",
            typeof(bool),
        ...
        static DependencyProperty ActivityStateProperty = DependencyProperty.Register(
            "ActivityState",
            typeof(List<ListenEventActivitySubscriber>),
            typeof(ListenActivity)
        );

        private bool IsListenTrigerred
        {
            get { return (bool)base.GetValue(IsListenTrigerredProperty); }
            set { base.SetValue(IsListenTrigerredProperty, value); }
        }
        ...
        private sealed class ListenEventActivitySubscriber : IActivityEventListener<QueueEventArgs>
        {
            ...
            void IActivityEventListener<QueueEventArgs>.OnEvent(object sender, QueueEventArgs e)
            {
                ...
                if (
                    !parentActivity.IsListenTrigerred
                    && parentActivity.ExecutionStatus != ActivityExecutionStatus.Canceling
                    && parentActivity.ExecutionStatus != ActivityExecutionStatus.Closed
                )
                {
                    ...
                    {
                        EventDrivenActivity eventDriven =
                            parentActivity.EnabledActivities[i] as EventDrivenActivity;
                        ListenEventActivitySubscriber eventSubscriber =
                            parentActivity.ActivityState[i];
                        eventDriven.EventActivity.Unsubscribe(context, eventSubscriber);
                    }

                    eventDrivenActivity.RegisterForStatusChange(
    ...
    internal sealed class ListenValidator : CompositeActivityValidator
    {
        public override ValidationErrorCollection Validate(ValidationManager manager, object obj)
        {
            ...
            if (listen == null)
                throw new ArgumentException(
                    SR.GetString(SR.Error_UnexpectedArgumentType, typeof(ListenActivity).FullName),
                    "obj"
                );
            ...
            if (listen.EnabledActivities.Count < 2)
                validationErrors.Add(
                    new ValidationError(
                        SR.GetString(SR.Error_ListenLessThanTwoChildren),
                        ErrorNumbers.Error_ListenLessThanTwoChildren
                    )
                );
