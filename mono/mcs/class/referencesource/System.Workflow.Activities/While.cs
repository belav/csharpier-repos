            ...
            IActivityEventListener<ActivityExecutionStatusChangedEventArgs>
    {
        ...
        public Activity DynamicActivity
        {
            get
            {
                ...
                {
                    Activity[] dynamicChildren = this.GetDynamicActivities(
        ...
        private bool TryNextIteration(ActivityExecutionContext context)
        {
            if (
                ...
                || !this.Condition.Evaluate(this, context)
            )
                return false;
            else
            {
                if (this.EnabledActivities.Count > 0)
                {
                    ...
                    ActivityExecutionContext innerContext = contextManager.CreateExecutionContext(
                        this.EnabledActivities[0]
                    );
    ...
    internal sealed class WhileValidator : CompositeActivityValidator
    {
        public override ValidationErrorCollection Validate(ValidationManager manager, object obj)
        {
            ...
            if (whileActivity == null)
                throw new ArgumentException(
                    SR.GetString(SR.Error_UnexpectedArgumentType, typeof(WhileActivity).FullName),
                    "obj"
                );
            if (whileActivity.EnabledActivities.Count != 1)
                validationErrors.Add(
                    new ValidationError(
                        SR.GetString(SR.Error_WhileShouldHaveOneChild),
                        ErrorNumbers.Error_WhileShouldHaveOneChild
                    )
                );
