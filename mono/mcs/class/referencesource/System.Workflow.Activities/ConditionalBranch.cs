        ...
        public override ValidationErrorCollection Validate(ValidationManager manager, object obj)
        {
            ...
            if (ifElseBranch == null)
                throw new ArgumentException(
                    SR.GetString(
                        ...
                        typeof(IfElseBranchActivity).FullName
                    ),
                    "obj"
                );
            ...
            if (ifElse == null)
                validationErrors.Add(
                    new ValidationError(
                        SR.GetString(SR.Error_ConditionalBranchParentNotConditional),
                        ErrorNumbers.Error_ConditionalBranchParentNotConditional
                    )
                );

            bool isLastBranch = (
                ifElse != null
                && ifElse.EnabledActivities.Count > 1
                && (ifElse.EnabledActivities[ifElse.EnabledActivities.Count - 1] == ifElseBranch)
            );
