    ...
    public class ExternalDataExchangeService : WorkflowRuntimeService
    {
            ...
            : this()
        {
            ...
            settings =
                ...
                as ExternalDataExchangeServiceSection;
            if (settings == null)
                throw new ArgumentException(
                    String.Format(
                        CultureInfo.CurrentCulture,
                        SR.GetString(SR.Error_ConfigurationSectionNotFound),
                        configSectionName
                    )
                );
        }
            ...
            : this()
        {
            ...
            foreach (string key in parameters.Keys)
            {
                if (
                    key.Equals(
                        configurationSectionAttributeName,
                ...
                else
                {
                    throw new ArgumentException(
                        String.Format(
                            ...
                            SR.GetString(SR.Error_UnknownConfigurationParameter),
                            key
                        ),
                        "parameters"
                    );
        ...
        internal void InterceptService(object service, bool add)
        {
            ...
            foreach (Type type in interfaceTypes)
            {
                ...
                if (this.Runtime != null && this.Runtime.GetService(type) != null && add)
                    throw new InvalidOperationException(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            SR.GetString(SR.Error_ExternalDataExchangeServiceExists),
                            type
                        )
                    );
                ...
                foreach (EventInfo e in events)
                {
                    ...
                    lock (handlersLock)
                    {
                        ...
                        {
                            handler = new WorkflowMessageEventHandler(
                                type,
                                e,
                                this.enqueueMessageWrapper
                            );
            ...
            if (!isDataExchangeService)
                throw new InvalidOperationException(
                    string.Format(
                        CultureInfo.CurrentCulture,
                        SR.GetString(SR.Error_ServiceMissingExternalDataExchangeInterface)
                    )
                );
        }
