...
namespace MS.Internal.Xml.Linq.ComponentModel
{
    ...
    abstract class XPropertyDescriptor<T, TProperty> : PropertyDescriptor
        where T : XObject
    {
    ...
    class XElementAttributePropertyDescriptor : XPropertyDescriptor<XElement, object>
    {
        ...
        {
            return value = new XDeferredSingleton<XAttribute>(
                (e, n) => e.Attribute(n),
        ...
        protected override void OnChanging(object sender, XObjectChangeEventArgs args)
        {
            ...
            switch (args.ObjectChange)
            {
                case XObjectChange.Remove:
                    XAttribute a = sender as XAttribute;
                    changeState =
                        a != null && value.element == a.parent && value.name == a.Name ? a : null;
                    break;
            }
        }
    }
        ...
        : XPropertyDescriptor<XElement, IEnumerable<XElement>>
    {
        ...
        public override object GetValue(object component)
        {
            return value = new XDeferredAxis<XElement>(
            ...
            switch (args.ObjectChange)
            {
                ...
                case XObjectChange.Name:
                    ...
                    if (
                        ...
                        && value.element != e
                        && value.name != null
                        && (value.name == e.Name || value.name == changeState)
                    )
                    {
                        changeState = null;
                        OnValueChanged(value.element, EventArgs.Empty);
    ...
    class XElementElementPropertyDescriptor : XPropertyDescriptor<XElement, object>
    {
        ...
        public override object GetValue(object component)
        {
            return value = new XDeferredSingleton<XElement>(
                (e, n) => e.Element(n),
                component as XElement,
                null
            );
        ...
        protected override void OnChanged(object sender, XObjectChangeEventArgs args)
        {
            ...
            switch (args.ObjectChange)
            {
                case XObjectChange.Add:
                    ...
                    if (
                        e != null
                        && value.element == e.parent
                        && value.name == e.Name
                        && value.element.Element(value.name) == e
                    )
                    {
                        OnValueChanged(value.element, EventArgs.Empty);
                    }
        ...
        protected override void OnChanged(object sender, XObjectChangeEventArgs args)
        {
            ...
            switch (args.ObjectChange)
            {
                ...
                case XObjectChange.Remove:
                    ...
                    if (
                        e != null
                        && value.element == (changeState as XContainer)
                        && (value.name == e.Name || value.name == null)
                    )
                    {
                        changeState = null;
                ...
                case XObjectChange.Name:
                    ...
                    if (
                        ...
                        && value.element == e.parent
                        && value.name != null
                        && (value.name == e.Name || value.name == (changeState as XName))
                    )
                    {
                        changeState = null;
                        OnValueChanged(value.element, EventArgs.Empty);
    ...
    class XDeferredAxis<T> : IEnumerable<T>, IEnumerable
        where T : XObject
    {
