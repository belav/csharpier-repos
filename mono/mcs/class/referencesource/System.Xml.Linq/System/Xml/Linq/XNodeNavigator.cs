...
namespace System.Xml.XPath
{
    internal class XNodeNavigator : XPathNavigator, IXmlLineInfo
    {
        ...
        static readonly int[] ElementContentMasks =
        {
            ...
            0, // Attribute
            0, // Namespace
            (1 << (int)XmlNodeType.CDATA) | (1 << (int)XmlNodeType.Text), // Text
        ...
        new const int TextMask = (1 << (int)XmlNodeType.CDATA) | (1 << (int)XmlNodeType.Text);
        ...
        )
        {
            throw new NotSupportedException(
                System.Xml.Linq.Res.GetString(System.Xml.Linq.Res.NotSupported_CheckValidity)
            );
        }
        ...
        {
            throw new NotSupportedException(
                System.Xml.Linq.Res.GetString(System.Xml.Linq.Res.NotSupported_MoveToId)
            );
        }
            ...
            if (a != null && parent != null && !IsXmlNamespaceDeclaration(a))
            {
                switch (scope)
                {
                    ...
                    case XPathNamespaceScope.All:
                        ...
                        {
                            a = GetNextNamespaceDeclarationGlobal(a);
                        } while (a != null && HasNamespaceDeclarationInScope(a, parent));
                        if (
                            a == null
    ...
    {
        public object Evaluate<T>(XNode node, string expression, IXmlNamespaceResolver resolver)
            where T : class
        {
    ...
    public static class Extensions
    {
        ...
        )
        {
            ...
            return (IEnumerable<XElement>)
                new XPathEvaluator().Evaluate<XElement>(node, expression, resolver);
        }
