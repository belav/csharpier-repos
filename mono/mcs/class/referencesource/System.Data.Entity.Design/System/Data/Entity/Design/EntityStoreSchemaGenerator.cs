...
namespace System.Data.Entity.Design
{
    ...
    public sealed partial class EntityStoreSchemaGenerator
    {
        ...
        )
        {
            ...
            _loader = new EntityStoreSchemaGeneratorDatabaseSchemaLoader(
        ...
        private IList<EdmSchemaError> DoGenerateStoreMetadata(
            IEnumerable<EntityStoreSchemaFilterEntry> filters,
            Version targetEntityFrameworkVersion
        )
        {
            ...
            try
            {
                ...
                DbProviderFactory providerFactory = DbProviderServices.GetProviderFactory(
                    _loader.ProviderInvariantName
                );
                ...
                DbProviderManifest storeManifest = providerServices.GetProviderManifest(
                ...
                Debug.Assert(
                    _namespaceName != null,
                    "_namespaceName should not be null at this point, did you add a new ctor?"
                );

                session.ItemCollection = new StoreItemCollection(
                    providerFactory,
                    providerServices.GetProviderManifest(_providerManifestToken),
                    _providerManifestToken
                );
                ...
                string entityContainerName =
                    this._namespaceName.Replace(".", string.Empty) + CONTAINER_SUFFIX;
                ...
                {
                    Debug.Assert(
                        type.KeyMembers.Count > 0,
                        "Why do we have Entities without keys in our valid Entities collection"
                    );
        ...
        public void WriteStoreSchema(XmlWriter writer)
        {
            ...
            //   ...
            // </Schema>
            var xmlPrefixToNamespace = new KeyValuePair<string, string>(
                "store",
        ...
        {
            return CreateStoreSchemaConnection(
                providerInvariantName,
                connectionString,
        ...
        )
        {
            ...
            catch (ArgumentException e)
            {
                throw EDesignUtil.Argument(
                    Strings.EntityClient_InvalidStoreProvider(providerInvariantName),
                    e
                );
            }
            ...
            {
                throw EDesignUtil.ProviderIncompatible(
                    Strings.ProviderFactoryReturnedNullFactory(providerInvariantName)
                );
            }
            providerConnection.ConnectionString = connectionString;

            MetadataWorkspace workspace = GetProviderSchemaMetadataWorkspace(
                providerServices,
                providerConnection,
                targetEntityFrameworkVersion
            );
        ...
        )
        {
            ...
            Debug.Assert(
                EntityFrameworkVersions.IsValidVersion(targetEntityFrameworkVersion),
                "EntityFrameworkVersions.IsValidVersion(targetEntityFrameworkVersion)"
            );
            string csdlName;
            string ssdlName;
            ...
            try
            {
                ...
                string manifestToken = providerServices.GetProviderManifestToken(
                    providerConnection
                );
                DbProviderManifest providerManifest = providerServices.GetProviderManifest(
                    manifestToken
                );
                ...
                string location = Strings.DbProviderServicesInformationLocationPath(
                    providerConnection.GetType().Name,
                    ssdlName
                );
                List<string> ssdlLocations = new List<string>(1);
                ssdlLocations.Add(location);
                StoreItemCollection storeItemCollection = new StoreItemCollection(
                    new XmlReader[] { ssdl },
                    ssdlLocations.AsReadOnly(),
                    out errors
                );
                ThrowOnError(errors);
                ...
                location = Strings.DbProviderServicesInformationLocationPath(
                ...
                location = Strings.DbProviderServicesInformationLocationPath(
                    providerConnection.GetType().Name,
                    DbProviderManifest.StoreSchemaMapping
                );
            ...
            {
                if (csdl != null)
                    ((IDisposable)csdl).Dispose();
                if (ssdl != null)
                    ((IDisposable)ssdl).Dispose();
                if (msl != null)
                    ((IDisposable)msl).Dispose();
            }
        }

        private static void ForceViewGeneration(MetadataWorkspace workspace)
        {
            ReadOnlyCollection<EntityContainer> containers = workspace.GetItems<EntityContainer>(
                DataSpace.SSpace
            );
            Debug.Assert(containers.Count != 0, "no s space containers found");
            Debug.Assert(
                containers[0].BaseEntitySets.Count != 0,
                "no entity sets in the sspace container"
            );
                ...
                if (!MetadataUtil.CheckIfAllErrorsAreWarnings(errors))
                {
                    throw EDesignUtil.ProviderIncompatible(
                        ...
                        EntityUtil.InvalidSchemaEncountered(
                            MetadataUtil.CombineErrorMessage(errors)
                        )
                    );
                }
            }
        ...
        private EntitySet CreateEntitySet(LoadMethodSessionState session, EntityType type)
        {
            ...
            switch (dbObjectType)
            {
                ...
                default:
                    Debug.Assert(
                        ...
                        "did you change to a call that could have different types?"
                    );
        ...
        private void CreateEdmFunctions(LoadMethodSessionState session)
        {
            using (FunctionDetailsReader reader = _loader.LoadFunctionDetails(session.Filters))
            {
                ...
                List<FunctionDetailsReader.Memento> parameters =
                    new List<FunctionDetailsReader.Memento>();
                while (reader.Read())
                {
        ...
        )
        {
            ...
            if (row.ReturnType != null)
            {
                ...
                else
                {
                    ...
                    errors.Add(
                        new EdmSchemaError(
                            excludedForTarget
                                ? Strings.UnsupportedFunctionReturnDataTypeForTarget(
                                    row.ProcedureName,
                                    row.ReturnType
                                )
            ...
            else if (row.IsTvf)
            {
                ...
                if (
                    session.TryGetTvfReturnType(functionKey, out tvfReturnType)
                    && !session.InvalidTypes.Contains(tvfReturnType)
                )
                {
                    ...
                    returnParameter = new FunctionParameter(
                        EdmConstants.ReturnType,
                        TypeUsage.Create(collectionType),
                        ParameterMode.ReturnValue
                    );
                }
                else
            ...
            for (int i = 0; i < parameters.Count && !row.IsParameterNameNull; i++)
            {
                ...
                {
                    parameterType = GetScalarFunctionTypeUsage(
                        session,
                        row.ParameterType,
                        out excludedForTarget
                    );
                ...
                if (parameterType != null)
                {
                    ...
                    string parameterName = EntityModelSchemaGenerator.CreateValidEcmaName(
                        row.ParameterName,
                        'p'
                    );
                ...
                else
                {
                    ...
                    errors.Add(
                        new EdmSchemaError(
                            excludedForTarget
                                ? Strings.UnsupportedFunctionParameterDataTypeForTarget(
                                    row.ProcedureName,
                                    row.ParameterName,
                                    i,
                                    typeValue
            ...
            string functionName = EntityModelSchemaGenerator.CreateValidEcmaName(
                row.ProcedureName,
                'f'
            );
            ...
            EdmFunction function = new EdmFunction(
                ...
                new EdmFunctionPayload
                {
                    Schema = row.Schema,
                    StoreFunctionName =
                        functionName != row.ProcedureName ? row.ProcedureName : null,
        ...
        )
        {
            ...
            if (
                session.TryGetStorePrimitiveType(dataType, out primitiveType, out excludedForTarget)
            )
            {
        ...
        )
        {
            ...
            if (!AreRelationshipColumnsTheTypesEntireKey(pkEntityType, columns, r => r.PKColumn))
            {
                session.AddErrorsForType(
                    pkEntityType,
                    new EdmSchemaError(
                        Strings.UnsupportedDbRelationship(firstRow.RelationshipName),
            ...
            RelationshipMultiplicity pkMultiplicity = someFkColmnsAreNullable
            ...
            AssociationEndMember pkEnd = CreateAssociationEnd(
                session,
                pkEntityType,
                pkMultiplicity,
                usedEndNames,
            ...
            AssociationEndMember fkEnd = CreateAssociationEnd(
                session,
                fkEntityType,
                fkMultiplicity,
                usedEndNames,
            ...
            List<EdmSchemaError> errors = new List<EdmSchemaError>();
            bool isValid = CreateReferentialConstraint(
                session,
                type,
                pkEnd,
                fkEnd,
            ...
            if (!this.GenerateForeignKeyProperties)
            {
                ...
                if (isValid)
                {
                    ...
                    foreach (
                        var toPropertyOfAddedAssociation in session.AssociationTypes.SelectMany(t =>
                            t.ReferentialConstraints.SelectMany(refconst => refconst.ToProperties)
                        )
                    )
                    {
                        foreach (
        ...
        )
        {
            ...
            DbObjectKey pkKey = new DbObjectKey(
                row[table.PKCatalogColumn],
                row[table.PKSchemaColumn],
                row[table.PKTableColumn],
                DbObjectType.Unknown
            );
            DbObjectKey fkKey = new DbObjectKey(
                row[table.FKCatalogColumn],
                row[table.FKSchemaColumn],
                row[table.FKTableColumn],
                DbObjectType.Unknown
            );
        ...
        )
        {
            ...
            if (!toPropertiesAreFullyContainedInPk && toPropertiesContainedAtLeastOnePK)
            {
                ...
                string primaryKeys = MetadataUtil.MembersToCommaSeparatedString(
                    (System.Collections.IEnumerable)toType.KeyMembers
                );
                errorMessage = Strings.UnsupportedForeignKeyPattern(
                    association.Name,
                    foreignKeys,
                    primaryKeys,
                    toType.Name
        ...
        private void CreateViewEntityTypes(LoadMethodSessionState session)
        {
            CreateTableTypes(
                session,
                _loader.LoadViewDetails(session.Filters),
                CreateEntityType,
                DbObjectType.View
            );
        }
        ...
        private void CreateTableEntityTypes(LoadMethodSessionState session)
        {
            CreateTableTypes(
                session,
                _loader.LoadTableDetails(session.Filters),
                CreateEntityType,
                DbObjectType.Table
            );
        }
        ...
        private void CreateTvfReturnRowTypes(LoadMethodSessionState session)
        {
            CreateTableTypes(
                session,
                _loader.LoadFunctionReturnTableDetails(session.Filters),
        ...
        )
        {
            ...
            foreach (TableDetailsRow row in tableDetailsRows)
            {
                ...
                if (rowKey != currentKey)
                {
                    ...
                    {
                        createType(session, singleTableColumns, primaryKeys, objectType, null);
        ...
        )
        {
            ...
            Debug.Assert(
                primaryKeys == null || primaryKeys.Count > 0,
                "There must be at least one key columns at this point in time"
            );

            IList<string> excludedColumns;
            var properties = CreateEdmProperties(
                session,
                columns,
                tableKey,
                errors,
            ...
            var excludedKeyColumns = (
                primaryKeys != null ? primaryKeys.Intersect(excludedColumns) : new string[0]
            ).ToArray();
                ...
                foreach (EdmMember pkColumn in properties.Where(p => primaryKeys.Contains(p.Name)))
                {
                    ...
                    )
                    {
                        ...
                        var tableColumn = columns
                            .Where(c => c.ColumnName == pkColumn.Name)
                            .Single();
                        errors.Add(
                            new EdmSchemaError(
                                Strings.InvalidTypeForPrimaryKey(
                                    tableColumn.GetMostQualifiedTableName(),
            ...
            if (excludedKeyColumns.Length > 0)
            {
                ...
                foreach (string columnName in excludedKeyColumns)
                {
                    if (status == EntityCreationStatus.ReadOnly)
                    {
                        errors.Add(
                            new EdmSchemaError(
                                Strings.ExcludedColumnWasAKeyColumnEntityIsReadOnly(
                                    columnName,
                                    columns[0].GetMostQualifiedTableName()
                                ),
                    ...
                    else
                    {
                        Debug.Assert(
                            status == EntityCreationStatus.Invalid,
                            "Did we change some code above to make it possible to be something different?"
                        );
                        errors.Add(
                            new EdmSchemaError(
        ...
        )
        {
            ...
            foreach (TableDetailsRow row in columns)
            {
                ...
                )
                {
                    ...
                    if (!row.IsDataTypeNull())
                    {
                        message = excludedForTarget
                            ? Strings.UnsupportedDataTypeForTarget(
                                row.DataType,
                                row.GetMostQualifiedTableName(),
                                row.ColumnName
                            )
                            : Strings.UnsupportedDataType(
                                row.DataType,
                                row.GetMostQualifiedTableName(),
                                row.ColumnName
                            );
                    ...
                    {
                        message = Strings.UnsupportedDataTypeUnknownType(
                            row.ColumnName,
                            row.GetMostQualifiedTableName()
                        );
                    }
                ...
                Dictionary<string, Facet> facets = primitiveType
                    .GetAssociatedFacetDescriptions()
                    .ToDictionary(fd => fd.FacetName, fd => fd.DefaultValueFacet);
                facets[DbProviderManifest.NullableFacetName] = Facet.Create(
                    facets[DbProviderManifest.NullableFacetName].Description,
                    row.IsNullable
                );
                ...
                if (primitiveType.PrimitiveTypeKind == PrimitiveTypeKind.Decimal)
                {
                    ...
                    if (facets.TryGetValue(DbProviderManifest.PrecisionFacetName, out precision))
                    {
                        ...
                        {
                            if (
                                row.Precision < precision.Description.MinValue
                            ...
                            facets[precision.Name] = Facet.Create(
                    ...
                    if (facets.TryGetValue(DbProviderManifest.ScaleFacetName, out scale))
                    {
                        ...
                        {
                            if (
                                row.Scale < scale.Description.MinValue
                ...
                )
                {
                    ...
                    if (
                        facets.TryGetValue(
                            DbProviderManifest.PrecisionFacetName,
                            out datetimePrecision
                        )
                    )
                    {
                        if (
                            !row.IsDateTimePrecisionNull()
                            && !datetimePrecision.Description.IsConstant
                        )
                        {
                            if (
                                row.DateTimePrecision < datetimePrecision.Description.MinValue
                            ...
                            facets[datetimePrecision.Name] = Facet.Create(
                                datetimePrecision.Description,
                                (byte)row.DateTimePrecision
                            );
                        }
                    }
                }
                ...
                )
                {
                    ...
                    if (facets.TryGetValue(DbProviderManifest.MaxLengthFacetName, out maxLength))
                    {
                        ...
                        {
                            if (
                                row.MaximumLength < maxLength.Description.MinValue
                            ...
                            facets[maxLength.Name] = Facet.Create(
                ...
                {
                    Facet facet = Facet.Create(
                        System.Data.Metadata.Edm.Converter.StoreGeneratedPatternFacet,
                ...
                members.Add(
                    new EdmProperty(row.ColumnName, TypeUsage.Create(primitiveType, facets.Values))
                );
            }
        ...
        private bool InferKeyColumns(
            ...
            List<string> pKeys,
            DbObjectKey tableKey,
            ref ICollection<string> primaryKeys
        )
        {
        ...
        private void FixupKeylessEntitySets(
        ...
        )
        {
            ...
            if (session.ReadOnlyEntities.Count > 0)
            {
                ...
                //
                MetadataWorkspace metadataWorkspace = CreateMetadataWorkspace(
                    entityContainer,
                    session
                );
        ...
        /// <param name="session"></param>
        /// <returns></returns>
        private MetadataWorkspace CreateMetadataWorkspace(
            EntityContainer entityContainer,
            LoadMethodSessionState session
        )
        {
        ...
        private void FixupViewEntitySetDefiningQuery(
            EntitySet entitySet,
            MetadataWorkspace metadataWorkspace
        )
        {
            DbExpressionBinding inputBinding = DbExpressionBuilder.BindAs(
                DbExpressionBuilder.Scan(entitySet),
                entitySet.Name
            );
            List<KeyValuePair<string, DbExpression>> projectList = new List<
                KeyValuePair<string, DbExpression>
            >(entitySet.ElementType.Members.Count);
            foreach (EdmMember member in entitySet.ElementType.Members)
            {
                Debug.Assert(
                    member.BuiltInTypeKind == BuiltInTypeKind.EdmProperty,
            ...
            DbQueryCommandTree dbCommandTree = new DbQueryCommandTree(
                metadataWorkspace,
                DataSpace.SSpace,
                query
            );
            ...
            //
            entitySet.DefiningQuery = (
                DbProviderServices
                    .GetProviderServices(_loader.EntityConnection.StoreProviderFactory)
            ...
            Debug.Assert(
                !String.IsNullOrEmpty(entitySet.DefiningQuery),
                "DefiningQuery must not be null or empty"
            );
        }
    }
}
