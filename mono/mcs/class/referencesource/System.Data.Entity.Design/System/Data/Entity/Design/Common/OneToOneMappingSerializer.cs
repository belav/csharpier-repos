...
namespace System.Data.Entity.Design.Common
{
    internal class OneToOneMappingSerializer
    {
        ...
        {
            internal Dictionary<EntityType, EntityType> StoreEntityTypeToModelEntityType =
                new Dictionary<EntityType, EntityType>();
            internal Dictionary<EdmProperty, EdmProperty> StoreEdmPropertyToModelEdmProperty =
                new Dictionary<EdmProperty, EdmProperty>();
            internal Dictionary<EntitySet, EntitySet> StoreEntitySetToModelEntitySet =
            ...
            internal List<CollapsedEntityAssociationSet> CollapsedEntityAssociationSets =
                new List<CollapsedEntityAssociationSet>();

            internal List<Tuple<EdmFunction, EdmFunction>> StoreFunctionToFunctionImport =
                new List<Tuple<EdmFunction, EdmFunction>>();
        }
        ...
        internal class CollapsedEntityAssociationSet
        {
            ...
            public AssociationSet ModelAssociationSet
            {
                ...
                set
                {
                    Debug.Assert(
                        ...
                        "why is this getting set multiple times, it should only be set after the new set is created"
                    );
            ...
            public void GetStoreAssociationSetEnd(
                int index,
            ...
            )
            {
                ...
                Debug.Assert(
                    AssociationSets.Count == 2,
                    "This code depends on only having exactly two AssociationSets"
                );
                GetFromAssociationSetEnd(
                    AssociationSets[index],
                    AssociationSets[(index + 1) % 2],
                    out storeAssociationSetEnd,
                    out multiplicity,
                    out deleteBehavior
                );
            }

            private void GetFromAssociationSetEnd(
                AssociationSet definingSet,
            ...
            )
            {
                ...
                AssociationSetEnd multiplicityAssociationSetEnd = GetAssociationSetEnd(
                    multiplicitySet,
                    false
                );
            ...
            {
                Debug.Assert(
                    set.ElementType.ReferentialConstraints.Count == 1,
                    "no referenctial constraint for association[0]"
                );
                ReferentialConstraint constraint = set.ElementType.ReferentialConstraints[0];

                Debug.Assert(
                    set.AssociationSetEnds.Count == 2,
                    "Associations are assumed to have two ends"
                );
                int toEndIndex,
                    fromEndIndex;
                if (
                    set.AssociationSetEnds[0].CorrespondingAssociationEndMember
                    == constraint.FromRole
                )
                {
                    fromEndIndex = 0;
            ...
            private bool IsAtLeastOneColumnFKInBothAssociations(
                ReferentialConstraint constraint0,
                ReferentialConstraint constraint1
            )
            {
            ...
            private bool IsAtLeastOneColumnOfBothDependentRelationshipColumnSetsNonNullable(
                ReferentialConstraint constraint0,
                ReferentialConstraint constraint1
            )
            {
                return ToPropertyHasNonNullableColumn(constraint0)
            ...
            private bool AreAllEntityColumnsMappedAsToColumns(
                ReferentialConstraint constraint0,
                ReferentialConstraint constraint1
            )
            {
            ...
            private static void AddToPropertyNames(
            ...
            private bool IsEntityDependentSideOfBothAssociations(
                ReferentialConstraint constraint0,
                ReferentialConstraint constraint1
            )
            {
                return ((RefType)constraint0.ToRole.TypeUsage.EdmType).ElementType
                        == _storeEntitySet.ElementType
                    && ((RefType)constraint1.ToRole.TypeUsage.EdmType).ElementType
                        == _storeEntitySet.ElementType;
            }

            private void GetConstraints(
                out ReferentialConstraint constraint0,
                out ReferentialConstraint constraint1
            )
            {
                Debug.Assert(
                    _storeAssociationSets.Count == 2,
                    "don't call this method if you don't have two associations"
                );
                Debug.Assert(
                    _storeAssociationSets[0].ElementType.ReferentialConstraints.Count == 1,
                    "no referenctial constraint for association[0]"
                );
                Debug.Assert(
                    _storeAssociationSets[1].ElementType.ReferentialConstraints.Count == 1,
                    "no referenctial constraint for association[1]"
                );
        ...
        )
        {
            ...
            _xmlNamespace = EntityFrameworkVersions.GetSchemaNamespace(
                schemaVersion,
                DataSpace.CSSpace
            );
        }
        ...
        private void WriteEntityContainerMappingElement(XmlWriter writer)
        {
            ...
            {
                AssociationSet modelAssociationSet =
                    _lookups.StoreAssociationSetToModelAssociationSet[set];
        ...
        private void WriteFunctionImportMappingElement(
            XmlWriter writer,
        ...
        )
        {
            ...
            Debug.Assert(
                storeFunction.ReturnParameters.Count == 1,
                "storeFunction.ReturnParameters.Count == 1"
            );
            Debug.Assert(
            ...
            if (tvfReturnType != null)
            {
                ...
                Debug.Assert(
                    functionImport.ReturnParameter.TypeUsage.EdmType.BuiltInTypeKind
                        == BuiltInTypeKind.CollectionType,
                    "functionImport is expected to return Collection(ComplexType)"
                );
                var modelCollectionType = (CollectionType)
                    functionImport.ReturnParameter.TypeUsage.EdmType;
                Debug.Assert(
                    modelCollectionType.TypeUsage.EdmType.BuiltInTypeKind
                ...
                writer.WriteAttributeString(
                    ...
                    modelComplexType.FullName
                );
                foreach (EdmProperty storeProperty in tvfReturnType.Properties)
                {
                    EdmProperty modelProperty = _lookups.StoreEdmPropertyToModelEdmProperty[
                        storeProperty
                    ];
        ...
        private void WriteAssociationSetMappingElement(
            XmlWriter writer,
        ...
        )
        {
            ...
            {
                writer.WriteStartElement(
                    StorageMslConstructs.AssociationSetMappingElement,
                    _xmlNamespace
                );
                writer.WriteAttributeString(
                    StorageMslConstructs.AssociationSetMappingNameAttribute,
                    collapsedAssociationSet.ModelAssociationSet.Name
                );
                writer.WriteAttributeString(
                    StorageMslConstructs.AssociationSetMappingTypeNameAttribute,
                    collapsedAssociationSet.ModelAssociationSet.ElementType.FullName
                );
                writer.WriteAttributeString(
                    StorageMslConstructs.AssociationSetMappingStoreEntitySetAttribute,
                ...
                for (int i = 0; i < collapsedAssociationSet.AssociationSets.Count; i++)
                {
                    ...
                    AssociationSetEnd modelEnd =
                        _lookups.StoreAssociationSetEndToModelAssociationSetEnd[storeEnd];
        ...
        )
        {
            if (!model.ElementType.IsForeignKey)
            {
                ...
                writer.WriteAttributeString(
                    StorageMslConstructs.AssociationSetMappingTypeNameAttribute,
                    model.ElementType.FullName
                );

                // all column names must be the primary key of the
                // end, but as columns in the Fk table.
                AssociationSetEnd foreignKeyTableEnd = GetAssociationSetEndForForeignKeyTable(
                    store
                );
                writer.WriteAttributeString(
                    StorageMslConstructs.AssociationSetMappingStoreEntitySetAttribute,
                    foreignKeyTableEnd.EntitySet.Name
                );
        ...
        private void WriteEndPropertyElement(
            XmlWriter writer,
        ...
        )
        {
            ...
            {
                EdmProperty modelKeyMember = _lookups.StoreEdmPropertyToModelEdmProperty[
                    storeKeyMember
                ];
        ...
        private void WriteEntitySetMappingElement(
            XmlWriter writer,
            EntitySet store,
            EntitySet model
        )
        ...
        private void WriteScalarPropertyElement(
            XmlWriter writer,
            EdmProperty store,
            EdmProperty model
        )
        {
            Debug.Assert(
                store.TypeUsage.EdmType.BuiltInTypeKind == BuiltInTypeKind.PrimitiveType,
                "only expect scalar type properties"
            );
            Debug.Assert(
                model.TypeUsage.EdmType.BuiltInTypeKind == BuiltInTypeKind.PrimitiveType,
                "only expect scalar type properties"
            );
