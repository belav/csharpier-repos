...
namespace System.Data.Entity.Design
{
    ...
    public sealed class EntityModelSchemaGenerator
    {
        ...
        public EntityModelSchemaGenerator(
            EntityContainer storeEntityContainer,
            string namespaceName,
            string modelEntityContainerName
        )
        {
        ...
        )
        {
            ...
            Initialize(
                ...
                storeItemCollection
                    ...
                    .Where(f =>
                        f.IsFromProviderManifest == false
        ...
        private void Initialize(
            EntityContainer storeEntityContainer,
            IEnumerable<EdmFunction> storeFunctions,
            string namespaceName,
            string modelEntityContainerName
        )
        {
            ...
            {
                throw EDesignUtil.InvalidStoreEntityContainer(
                    storeEntityContainer.Name,
            ...
            {
                EDesignUtil.CheckStringArgument(
                    modelEntityContainerName,
                    "modelEntityContainerName"
                );
                ...
                {
                    throw EDesignUtil.DuplicateEntityContainerName(
                        modelEntityContainerName,
                        storeEntityContainer.Name
                    );
                }
        ...
        {
            get { return _modelEntityContainer; }
        }
        ...
        public PluralizationService PluralizationService
        {
            ...
            {
                Debug.Assert(
                    this._pluralizationServiceHandler != null,
                    "all constructor should call initialize() to set the handler"
                );
                return this._pluralizationServiceHandler.Service;
            }
            set { this._pluralizationServiceHandler = new EntityDesignPluralizationHandler(value); }
        }
        ...
        private class LoadMethodSessionState
        {
            ...
            public UniqueIdentifierService UsedGlobalModelTypeNames = new UniqueIdentifierService(
                false
            );
            public UniqueIdentifierService UsedEntityContainerItemNames =
                new UniqueIdentifierService(false);
            public Dictionary<EdmProperty, AssociationType> FkProperties =
                new Dictionary<EdmProperty, AssociationType>();
            public Dictionary<
                EntitySet,
            ...
            > CandidateCollapsedAssociations =
                new Dictionary<
                    ...
                    OneToOneMappingSerializer.CollapsedEntityAssociationSet
                >();
            public OneToOneMappingSerializer.MappingLookups MappingLookups =
                new OneToOneMappingSerializer.MappingLookups();
        ...
        private IList<EdmSchemaError> InternalGenerateMetadata()
        {
            ...
            try
            {
                ...
                if (
                    this.GenerateForeignKeyProperties
                    && this._targetEntityFrameworkVersion < EntityFrameworkVersions.Version2
                )
                {
                    session.AddError(
                        Strings.UnableToGenerateForeignKeyPropertiesForV1,
                        ModelBuilderErrorCode.UnableToGenerateForeignKeyPropertiesForV1,
                ...
                EntityContainer modelEntityContainer = new EntityContainer(
                ...
                foreach (EntitySetBase storeSet in _storeEntityContainer.BaseEntitySets)
                {
                    switch (storeSet.BuiltInTypeKind)
                    {
                        case BuiltInTypeKind.AssociationSet:
                            ...
                            if (
                                this.GenerateForeignKeyProperties
                                || !EntityStoreSchemaGenerator.IsFkPartiallyContainedInPK(
                            ...
                            else
                            {
                                session.AddError(
                                    errorMessage,
                                    ModelBuilderErrorCode.UnsupportedForeinKeyPattern,
                                    EdmSchemaErrorSeverity.Error,
                                    null
                                );
                            }
                        ...
                        case BuiltInTypeKind.EntitySet:
                            ...
                            session.CandidateCollapsedAssociations
                                .Add(
                                    set,
                        ...
                        default:
                            // error
                            throw EDesignUtil.MissingGenerationPatternForType(
                                storeSet.BuiltInTypeKind
                            );
                    }
                }
                ...
                Set<AssociationSet> associationSetsFromCollapseCandidateRejects =
                    new Set<AssociationSet>();
                IEnumerable<OneToOneMappingSerializer.CollapsedEntityAssociationSet> invalidCandidates =
                    FindAllInvalidCollapsedAssociationCandidates(session);

                // now that we have gone through all of the association sets,
                foreach (
                    OneToOneMappingSerializer.CollapsedEntityAssociationSet collapsed in invalidCandidates
                )
                {
                ...
                foreach (
                    AssociationSet storeAssociationSet in (IEnumerable<AssociationSet>)
                        associationSetsFromCollapseCandidateRejects
                )
                {
                    if (
                    ...
                    {
                        AssociationSet set = CreateModelAssociationSet(
                            session,
                            storeAssociationSet
                        );
                ...
                foreach (
                    OneToOneMappingSerializer.CollapsedEntityAssociationSet collapsed in session.MappingLookups
                        .CollapsedEntityAssociationSets
                )
                {
                ...
                {
                    Debug.Assert(
                        EntityFrameworkVersions.Latest == EntityFrameworkVersions.Version3,
                        "Did you add a new framework version"
                    );
                    // add LazyLoadingEnabled=true to the EntityContainer
                    MetadataProperty lazyLoadingAttribute = new MetadataProperty(
                    ...
                    modelEntityContainer.AddMetadataProperties(
                        new List<MetadataProperty>() { lazyLoadingAttribute }
                    );
                ...
                if (!EntityStoreSchemaGenerator.HasErrorSeverityErrors(session.Errors))
                {
                    ...
                    foreach (
                        EntityType type in session.MappingLookups
                    ...
                    foreach (
                        AssociationType type in session.MappingLookups
                    ...
                    foreach (
                        OneToOneMappingSerializer.CollapsedEntityAssociationSet set in session.MappingLookups
                            .CollapsedEntityAssociationSets
                    )
                    {
        ...
        private void InternalWriteModelSchema(XmlWriter writer)
        {
            ...
            Debug.Assert(
                _targetEntityFrameworkVersion != null,
                "_targetEntityFrameworkVersion != null"
            );

            if (this._hasAnnotationNamespace)
            {
                KeyValuePair<string, string> namespacesPrefix = new KeyValuePair<string, string>(
                    DesignXmlConstants.AnnotationPrefix,
                    DesignXmlConstants.EdmAnnotationNamespace
                );
                MetadataItemSerializer.WriteXml(
                    writer,
                    _edmItemCollection,
                    _namespaceName,
        ...
        private void MapFunctions(
            ...
            EntityContainer modelEntityContainer
        )
        {
            if (
                _storeFunctions == null
                || _storeFunctions.Length == 0
                || _targetEntityFrameworkVersion < EntityFrameworkVersions.Version3
            )
            {
                return;
            }
            ...
            var interestingStoreFunctions = _storeFunctions.Where(f =>
                f.IsComposableAttribute
            ...
            foreach (var storeFunction in interestingStoreFunctions)
            {
                ...
                string functionImportName = CreateModelName(
                    storeFunction.Name,
                    session.UsedEntityContainerItemNames
                );
                ...
                {
                    session.AddError(
                        Strings.UnableToGenerateFunctionImportParameterName(
                            failedStoreParameterName,
                            storeFunction.Identity
                        ),
                        ModelBuilderErrorCode.UnableToGenerateFunctionImportParameterName,
                ...
                var complexType = CreateModelComplexTypeForTvfResult(
                    session,
                    functionImportName,
                    tvfReturnType
                );
        ...
        private FunctionParameter CreateFunctionImportParameter(
            FunctionParameter storeParameter,
            UniqueIdentifierService usedParameterNames
        )
        {
            Debug.Assert(
                storeParameter.Mode == ParameterMode.In,
                "Function import mapping is supported only for 'In' parameters."
            );
        ...
        )
        {
            ...
            string name = CreateModelName(
                functionImportName + "_Result",
                session.UsedGlobalModelTypeNames
            );
            ...
            {
                EdmProperty property = CreateModelProperty(
                    session,
                    storeProperty,
                    usedPropertyNames
                );
        ...
        private IEnumerable<OneToOneMappingSerializer.CollapsedEntityAssociationSet> FindAllInvalidCollapsedAssociationCandidates(
            LoadMethodSessionState session
        )
        {
            Set<OneToOneMappingSerializer.CollapsedEntityAssociationSet> invalid =
                new Set<OneToOneMappingSerializer.CollapsedEntityAssociationSet>();
            Dictionary<
                EntitySet,
                OneToOneMappingSerializer.CollapsedEntityAssociationSet
            > newCandidates =
                ...
                >();
            foreach (
                OneToOneMappingSerializer.CollapsedEntityAssociationSet collapsed in session.CandidateCollapsedAssociations
                    .Values
            )
            {
            ...
            foreach (
                OneToOneMappingSerializer.CollapsedEntityAssociationSet collapsed in newCandidates.Values
            )
            {
                foreach (AssociationSet set in collapsed.AssociationSets)
                {
                    ...
                    if (newCandidates.ContainsKey(end0Set) && newCandidates.ContainsKey(end1Set))
                    {
                        OneToOneMappingSerializer.CollapsedEntityAssociationSet collapsed0 =
                            session.CandidateCollapsedAssociations[end0Set];
                        OneToOneMappingSerializer.CollapsedEntityAssociationSet collapsed1 =
                            session.CandidateCollapsedAssociations[end1Set];
        ...
        private bool IsAssociationPartOfCandidateCollapsedAssociation(
            LoadMethodSessionState session,
        ...
        private void SaveAssociationForCollapsedAssociationCandidate(
            LoadMethodSessionState session,
            AssociationSet storeAssociationSet
        )
        {
            foreach (AssociationSetEnd end in storeAssociationSet.AssociationSetEnds)
            {
                ...
                if (
                    session.CandidateCollapsedAssociations.TryGetValue(end.EntitySet, out collapsed)
                )
                {
        ...
        private void SetupFields()
        {
            ...
            if (_namespaceName == null)
            {
                ...
                int index = targetSchemaNamespace.IndexOf(
                    "Target",
                    StringComparison.OrdinalIgnoreCase
                );
                if (index > 0)
                {
                    _namespaceName = CreateValildModelNamespaceName(
                        targetSchemaNamespace.Substring(0, index) + NAMESPACE_NAME_SUFFIX
                    );
                }
                else
                {
                    ...
                    _namespaceName = CreateValildModelNamespaceName(
                        targetSchemaNamespace + NAMESPACE_NAME_SUFFIX
                    );
                }
            }
            ...
            if (_modelEntityContainerName == null)
            {
                ...
                if (dotIndex > 0)
                {
                    _modelEntityContainerName = CreateModelName(
                        targetSchemaNamespace.Substring(0, dotIndex) + ENTITY_CONTAINER_NAME_SUFFIX
                    );
                }
                else
                {
                    _modelEntityContainerName = CreateModelName(
                        targetSchemaNamespace + ENTITY_CONTAINER_NAME_SUFFIX
                    );
                }

                int targetIndex = _modelEntityContainerName.IndexOf(
                    "Target",
                    StringComparison.OrdinalIgnoreCase
                );
                if (targetIndex > 0)
                {
                    _modelEntityContainerName = CreateModelName(
                        targetSchemaNamespace.Substring(0, targetIndex)
        ...
        private EntitySet CreateModelEntitySet(
            LoadMethodSessionState session,
        ...
        )
        {
            ...
            string name = CreateModelName(
                this._pluralizationServiceHandler.GetEntitySetName(storeEntitySet.Name),
                session.UsedEntityContainerItemNames
            );
        ...
        private EntityType CreateModelEntityType(
            LoadMethodSessionState session,
        ...
        )
        {
            ...
            Debug.Assert(
                storeEntityType.BaseType == null,
                "we are assuming simple generation from a database where no types will have a base type"
            );
            ...
            string name = CreateModelName(
                this._pluralizationServiceHandler.GetEntityTypeName(storeEntityType.Name),
                session.UsedGlobalModelTypeNames
            );
            ...
            foreach (EdmProperty storeProperty in storeEntityType.Properties)
            {
                ...
                EdmMember member;
                bool isKey = storeEntityType.KeyMembers
                ...
                if (
                    isKey
                    || this.GenerateForeignKeyProperties
                    || !session.FkProperties.TryGetValue(storeProperty, out association)
                )
                {
                    EdmProperty property = CreateModelProperty(
                        session,
        ...
        private EdmProperty CreateModelProperty(
            LoadMethodSessionState session,
            EdmProperty storeProperty,
            UniqueIdentifierService usedPropertyNames
        )
        {
        ...
        private void AddStoreGeneratedPatternAnnoation(
        ...
        private AssociationSet CreateModelAssociationSet(
            LoadMethodSessionState session,
            OneToOneMappingSerializer.CollapsedEntityAssociationSet collapsedAssociationSet
        )
        {
            // create the association
            string associationName = CreateModelName(
                collapsedAssociationSet.EntitySet.Name,
                session.UsedGlobalModelTypeNames
            );
            ...
            string associationSetName = CreateModelName(
                collapsedAssociationSet.EntitySet.Name,
                session.UsedEntityContainerItemNames
            );
            ...
            for (int i = 0; i < collapsedAssociationSet.AssociationSets.Count; i++)
            {
                ...
                collapsedAssociationSet.GetStoreAssociationSetEnd(
                    i,
                    out storeEnd,
                    out multiplicity,
                    out deleteBehavior
                );
                AssociationEndMember end = CreateAssociationEndMember(
                    session,
                ...
                EntitySet entitySet = session.MappingLookups.StoreEntitySetToModelEntitySet[
                    storeEnd.EntitySet
                ];
        ...
        private AssociationSetEnd CreateModelAssociationSetEnd(
            LoadMethodSessionState session,
            AssociationSetEnd storeEnd,
            AssociationSet parentModelAssociationSet
        )
        {
            AssociationEndMember associationEnd = session.MappingLookups
        ...
        private AssociationType CreateModelAssociationType(
            LoadMethodSessionState session,
        ...
        )
        {
            ...
            if (_targetEntityFrameworkVersion > EntityFrameworkVersions.Version1)
            {
                isFkAssociation =
                    this.GenerateForeignKeyProperties
                    || RequiresModelReferentialConstraint(storeAssociationType);
            }
            AssociationType association = new AssociationType(
                name,
                _namespaceName,
            ...
            KeyValuePair<string, RelationshipMultiplicity> endMultiplicityOverride =
                CreateEndMultiplicityOverride(session, storeAssociationType, association);

            foreach (
                AssociationEndMember storeEndMember in storeAssociationType.RelationshipEndMembers
            )
            {
                AssociationEndMember end = CreateAssociationEndMember(
                    session,
            ...
            ReferentialConstraint constraint = CreateReferentialConstraint(
                session,
        ...
        private KeyValuePair<string, RelationshipMultiplicity> CreateEndMultiplicityOverride(
            LoadMethodSessionState session,
            AssociationType storeAssociation,
            AssociationType modelAssociation
        )
        {
            ...
            if (
                storeConstraint.FromRole.RelationshipMultiplicity
        ...
        )
        {
            ...
            Debug.Assert(
                storeAssociation.ReferentialConstraints.Count <= 1,
                "We don't have a reason to have more than one constraint yet"
            );
            ...
            Debug.Assert(
                storeConstraint.FromProperties.Count != 0,
                "No properties in the constraint, why does the constraint exist?"
            );
            Debug.Assert(
                storeConstraint.ToProperties[0].DeclaringType.BuiltInTypeKind
            ...
            if (
                !this.GenerateForeignKeyProperties
                && !RequiresModelReferentialConstraint(storeConstraint, toType)
            )
            {
                return null;
            }
            ...
            AssociationEndMember fromRole = session.MappingLookups
                .StoreAssociationEndMemberToModelAssociationEndMember[
                (AssociationEndMember)storeConstraint.FromRole
            ];
            AssociationEndMember toRole = session.MappingLookups
                .StoreAssociationEndMemberToModelAssociationEndMember[
                (AssociationEndMember)storeConstraint.ToRole
            ];
            for (int index = 0; index < count; index++)
            {
                fromProperties[index] = session.MappingLookups.StoreEdmPropertyToModelEdmProperty[
                    storeConstraint.FromProperties[index]
                ];
                toProperties[index] = session.MappingLookups.StoreEdmPropertyToModelEdmProperty[
                    storeConstraint.ToProperties[index]
                ];
            }
        ...
        private static bool RequiresModelReferentialConstraint(
            ReferentialConstraint storeConstraint,
            EntityType toType
        )
        {
        ...
        private void CreateModelNavigationProperties(
            LoadMethodSessionState session,
            AssociationType association
        )
        {
        ...
        private void CreateModelNavigationProperty(
            LoadMethodSessionState session,
            AssociationEndMember from,
            AssociationEndMember to
        )
        {
            ...
            string name = CreateModelName(
                this._pluralizationServiceHandler.GetNavigationPropertyName(to, to.Name),
                usedMemberNames
            );
        ...
        private void LoadNameLookupWithUsedMemberNames(
            EntityType entityType,
            UniqueIdentifierService usedMemberNames
        )
        {
        ...
        private AssociationEndMember CreateAssociationEndMember(
            LoadMethodSessionState session,
            AssociationEndMember storeEndMember,
            KeyValuePair<string, RelationshipMultiplicity> endMultiplicityOverride,
            UniqueIdentifierService usedEndMemberNames
        )
        {
            RelationshipMultiplicity multiplicity = storeEndMember.RelationshipMultiplicity;
            if (
                endMultiplicityOverride.Key != null
                && endMultiplicityOverride.Key == storeEndMember.Name
            )
            {
                multiplicity = endMultiplicityOverride.Value;
            }
            return CreateAssociationEndMember(
                session,
                storeEndMember,
                usedEndMemberNames,
                multiplicity,
        ...
        private AssociationEndMember CreateAssociationEndMember(
            ...
            AssociationEndMember storeEndMember,
            UniqueIdentifierService usedEndMemberNames,
            RelationshipMultiplicity multiplicityOverride,
            OperationAction deleteBehaviorOverride
        )
        {
            Debug.Assert(
                storeEndMember.TypeUsage.EdmType.BuiltInTypeKind == BuiltInTypeKind.RefType,
                "The type is not a ref type"
            );
            Debug.Assert(
                ((RefType)storeEndMember.TypeUsage.EdmType).ElementType.BuiltInTypeKind
                    == BuiltInTypeKind.EntityType,
                "the ref is not holding on to an EntityType"
            );

            EntityType storeEntityType = (
                (EntityType)((RefType)storeEndMember.TypeUsage.EdmType).ElementType
            );
            EntityType modelEntityType = session.MappingLookups.StoreEntityTypeToModelEntityType[
                storeEntityType
            ];

            string name = CreateModelName(storeEndMember.Name, usedEndMemberNames);
            AssociationEndMember end = new AssociationEndMember(
                name,
                modelEntityType.GetReferenceType(),
        ...
        internal static string CreateValidNamespaceName(
            string storeNamespaceName,
            char appendToFrontIfFirstCharIsInvalid
        )
        {
        ...
        internal static string CreateValidEcmaName(
            string name,
            char appendToFrontIfFirstCharIsInvalid
        )
        {
