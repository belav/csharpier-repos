...
namespace System.Numerics
{
    ...
    internal struct BigIntegerBuilder
    {
        ...
        public uint High
        {
            get { return _iuLast == 0 ? _uSmall : _rgu[_iuLast]; }
        }
        ...
        public void Mul(ref BigIntegerBuilder regMul)
        {
            ...
            else
            {
                ...
                for (int iu = cuBase; --iu >= 0; )
                {
                    ...
                    {
                        for (
                            int iuDst = iu + regMul._iuLast + 1;
                            uCarry != 0 && iuDst <= _iuLast;
                            iuDst++
                        )
        ...
        private static void ModDivCore(
            ref BigIntegerBuilder regNum,
            ref BigIntegerBuilder regDen,
            bool fQuo,
            ref BigIntegerBuilder regQuo
        )
        {
            ...
            for (int iu = cuQuo; --iu >= 0; )
            {
                ...
                while (
                    uuRem <= uint.MaxValue
                    && uuQuo * uDenNext > NumericsHelpers.MakeUlong((uint)uuRem, uNumNext)
                )
                {
                    uuQuo--;
                    uuRem += uDen;
            ...
            else
            {
                ...
                if (cbitShift > 0)
                {
                    for (int iuSrc = cuShift + 1, iuDst = 0; iuSrc < cuSrc; iuSrc++, iuDst++)
                        _rgu[iuDst] =
                            (rguSrc[iuSrc - 1] >> cbitShift)
                            | (rguSrc[iuSrc] << (kcbitUint - cbitShift));
        ...
        public void ShiftLeft(int cuShift, int cbitShift)
        {
            ...
            else
            {
                ...
                for (; iuSrc > 0; iuSrc--, iuDst--)
                    _rgu[iuDst] =
                        (rguSrc[iuSrc] << cbitShift)
                        | (rguSrc[iuSrc - 1] >> (kcbitUint - cbitShift));
