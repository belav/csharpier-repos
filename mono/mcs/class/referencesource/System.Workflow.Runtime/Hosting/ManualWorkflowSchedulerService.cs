...
namespace System.Workflow.Runtime.Hosting
{
    ...
    public class ManualWorkflowSchedulerService : WorkflowSchedulerService
    {
        ...
        private Dictionary<Guid, DefaultWorkflowSchedulerService.WorkItem> scheduleRequests =
            new Dictionary<Guid, DefaultWorkflowSchedulerService.WorkItem>();
            ...
            lock (locker)
            {
                ...
                if (!scheduleRequests.ContainsKey(workflowInstanceId))
                    scheduleRequests.Add(
                        workflowInstanceId,
                        new DefaultWorkflowSchedulerService.WorkItem(callback, workflowInstanceId)
                    );
            }
        ...
        private bool HasExpiredTimer(Guid workflowInstanceId, out Guid timerId)
        {
            lock (locker)
            {
                CallbackInfo ci = pendingScheduleRequests.FindByPriority(
                    DateTime.UtcNow,
                    delegate(CallbackInfo c)
                    {
