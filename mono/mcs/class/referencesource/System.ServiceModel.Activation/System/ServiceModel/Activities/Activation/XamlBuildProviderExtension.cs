    ...
    sealed class XamlBuildProviderExtension : System.Xaml.Hosting.IXamlBuildProviderExtension
    {
        ...
        )
        {
            ...
            if (workflowService != null && workflowService.Body != null)
            {
                ...
                {
                    this.generatedPrimaryTypeName =
                        GeneratedNamespace + "." + activityName + ExpressionRootFactorySuffix;
                }
                        ...
                        foreach (Tuple<string, Stream> stream in streams)
                        {
                            try
                            {
                                WorkflowService service =
                                    WorkflowServiceHostFactory.CreatetWorkflowService(
                                        stream.Item2,
                            ...
                            catch (Exception e)
                            {
                                ...
                                if (
                                    Fx.IsFatal(e)
                                    || !WorkflowServiceHostFactory.TryWrapSupportedVersionException(
                                        stream.Item1,
                                        e,
                                        out newException
                                    )
        ...
        internal static ICompiledExpressionRoot GetExpressionRoot(
        ...
        )
        {
            ...
            {
                string fullTypeNameToSearch =
                    GeneratedNamespace
                    + "."
                    + WorkflowServiceHostFactory.GetSupportedVersionGeneratedTypeName(
                        supportedVersionXamlxfilePath
                    )
                    + ExpressionRootFactorySuffix;
            ...
            if (
                generatedType != workflowServiceType
                && workflowServiceType.IsAssignableFrom(generatedType)
            )
            {
                MethodInfo createExpressionRootMethod = generatedType.GetMethod(
                    CreateExpressionRootMethodName,
                    BindingFlags.Public | BindingFlags.Static
                );
                if (createExpressionRootMethod != null)
                {
                    return (ICompiledExpressionRoot)
        ...
        )
        {
            ...
            if (generatedSource)
            {
                ...
                this.GenerateExpressionRootFactory(
                    assemblyBuilder,
                    buildProvider,
                    GeneratedNamespace,
                    activityName
        ...
        )
        {
            ...
            CodeTypeDeclaration codeTypeDeclaration = new CodeTypeDeclaration(
                activityName + ExpressionRootFactorySuffix
            );
            ...
            CodeObjectCreateExpression expr = new CodeObjectCreateExpression(
                typeRef,
                new CodeArgumentReferenceExpression(ActivityParameterName)
            );
        ...
        void GenerateSource(
            ...
            AssemblyBuilder assemblyBuilder,
            WorkflowService workflowService,
            out string codeFileName,
            out bool generatedSource,
            out string activityName
        )
        {
            // Get unique file and type name for the workflowservice
            codeFileName = assemblyBuilder.GetTempFilePhysicalPath(
                assemblyBuilder.CodeDomProvider.FileExtension
            );

            if (isSupportedVersion)
            {
                activityName = WorkflowServiceHostFactory.GetSupportedVersionGeneratedTypeName(
                    filePath
                );
            }
            else
            {
                activityName =
                    workflowService.Name.LocalName
                    + "_"
            ...
            TextExpressionCompilerSettings settings = new TextExpressionCompilerSettings
            {
                ...
                Language = CodeDomProvider.GetLanguageFromExtension(
                    assemblyBuilder.CodeDomProvider.FileExtension
                ),
        ...
        [SuppressMessage(
            ...
            Justification = "The XamlBuildProvider.VirtualPath non-aptca property cannot be used in a destructive manner."
        )]
