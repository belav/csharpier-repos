...
namespace System.ServiceModel.Activation
{
    ...
    {
        [Fx.Tag.SecurityNote(
            Critical = "Stores the securitycritical callback values, we need to protect these values"
        )]
        [SecurityCritical]
        ...
        [Fx.Tag.SecurityNote(
            Critical = "Keeps track of thread static data (HttpContext, CurrentCulture, CurrentUICulture) that is used for AspNetCompatibility mode, caller must use with care"
        )]
        [SecurityCritical]
        ...
        [SecurityCritical]
        public HostedHttpRequestAsyncResult(
            HttpApplication context,
            bool flowContext,
            bool ensureWFService,
            ...
            : base(callback, state)
        {
            ...
            this.isWebSocketRequest =
                ...
                && AspNetEnvironment.Current.IsWebSocketModuleLoaded
                && this.context.Context.IsWebSocketRequest;
            ...
            if (ensureWFService)
            {
                ...
                if (
                    ServiceHostingEnvironment.IsConfigurationBasedService(
                        context,
                        out this.configurationBasedServiceVirtualPath
                    )
                )
                {
            ...
            )
            {
                if (DiagnosticUtility.ShouldTraceVerbose)
                {
                    TraceUtility.TraceEvent(
                        ...
                        TraceCode.WebHostDebugRequest,
                        SR.TraceCodeWebHostDebugRequest,
                        this
                    );
                }
            ...
            Action<object> iotsCallback =
                (AspNetPartialTrustHelpers.NeedPartialTrustInvoke || flowContext)
                    ? WaitOnBeginRequestWithFlow
                    : WaitOnBeginRequest;
        ...
        {
            [Fx.Tag.SecurityNote(
                Critical = "Access the value of corresponding static field and prevent someone from changing its value"
            )]
        ...
        public static AsyncCallback ProcessRequestCompleteCallback
        {
            ...
            get
            {
                ...
                {
                    processRequestCompleteCallback = Fx.ThunkCallback(
                        new AsyncCallback(ProcessRequestComplete)
                    );
                }
        ...
        public bool IISSupportsExtendedProtection
        {
            get
            {
                ...
                {
                    HostedHttpRequestAsyncResult.iisSupportsExtendedProtection =
                        this.IISSupportsExtendedProtectionInternal();
                }
        ...
        private bool IISSupportsExtendedProtectionInternal()
        {
            DiagnosticUtility.DebugAssert(
                ...
                "OS must support ExtendedProtection"
            );
        ...
        [SecurityCritical]
        public static void ExecuteSynchronous(
            HttpApplication context,
            bool flowContext,
            bool ensureWFService
        )
        ...
        public static void ExecuteSynchronous(
            HttpApplication context,
            string routeServiceVirtualPath,
            bool flowContext,
            bool ensureWFService
        )
        {
            ...
            try
            {
                result = new HostedHttpRequestAsyncResult(
                    context,
                    routeServiceVirtualPath,
                    flowContext,
                    ensureWFService,
        ...
        [Fx.Tag.SecurityNote(
            Miscellaneous = "RequiresReview - Can be called outside of a user context."
        )]
        ...
        public HostedImpersonationContext ImpersonationContext
        {
            ...
            [SecuritySafeCritical]
            get { return this.impersonationContext; }
        }
        ...
        public HostedThreadData HostedThreadData
        {
            ...
            [SecuritySafeCritical]
            get { return this.hostedThreadData; }
        }
        ...
        public Stream GetInputStream()
        {
            try
            {
                ...
                if (
                    ...
                    && AppSettings.UseClassicReadEntityMode
                )
                {
                    mode = ReadEntityBodyMode.Classic;
                }
        ...
        internal void CompleteOperation(Exception exception)
        {
            if (
                ...
                && Interlocked.CompareExchange(ref this.state, State.Completed, State.Running)
                    == State.Running
            )
            {
        ...
        public void Abort()
        {
            if (
                this.state == State.Running
                && Interlocked.CompareExchange(ref this.state, State.Aborted, State.Running)
                    == State.Running
            )
            {
                int currentStreamedReadState = Interlocked.Exchange(
                    ref this.streamedReadState,
                    StreamedReadState.AbortStarted
                );
                ...
                {
                    Fx.Assert(
                        currentStreamedReadState == StreamedReadState.ReceiveStarted,
                        string.Format(
                            CultureInfo.InvariantCulture,
                            "currentStramedReadState is not ReceivedStarted: {0}",
                    ...
                    Fx.Assert(
                        this.streamedReadState
                            == StreamedReadState.ReceiveFinishedAfterAbortStarted,
                        string.Format(
                            CultureInfo.InvariantCulture,
                            "currentStramedReadState is not ReceiveFinished: {0}",
                            this.streamedReadState
                        )
                    );
                    this.CompleteAsynchronously(null);
        ...
        internal bool TryStartStreamedRead()
        {
            return Interlocked.CompareExchange(
                    ref this.streamedReadState,
                    StreamedReadState.ReceiveStarted,
                    StreamedReadState.None
                ) == StreamedReadState.None;
        }
        ...
        internal void SetStreamedReadFinished()
        {
            ...
            Fx.Assert(
                this.streamedReadState == StreamedReadState.Aborted,
                string.Format(
                    CultureInfo.InvariantCulture,
                    "currentStramedReadState is not Aborted: {0}",
                    this.streamedReadState
                )
            );
        ...
        X509Certificate2 HttpChannelListener.IHttpAuthenticationContext.GetClientCertificate(
        ...
        {
            return new System.ServiceModel.Diagnostics.HttpRequestTraceRecord(
        ...
        void HandleRequest()
        {
            ...
            HttpHostedTransportConfiguration transportConfiguration =
                HostedTransportConfigurationManager.GetConfiguration(this.OriginalRequestUri.Scheme)
            ...
            {
                transportManager = transportConfiguration.GetHttpTransportManager(
            ...
            {
                InvalidOperationException invalidOpException = new InvalidOperationException(
                    SR.Hosting_TransportBindingNotFound(OriginalRequestUri.ToString())
                );

                ServiceActivationException activationException = new ServiceActivationException(
            ...
            this.RequestUri = new Uri(
                transportManager.ListenUri,
                this.OriginalRequestUri.PathAndQuery
            );
            Fx.Assert(
                object.ReferenceEquals(this.RequestUri.Scheme, Uri.UriSchemeHttp)
                    || object.ReferenceEquals(this.RequestUri.Scheme, Uri.UriSchemeHttps),
                "Scheme must be Http or Https."
            );

            ServiceHostingEnvironment.EnsureServiceAvailableFast(
                relativeVirtualPath,
        ...
        void LogServiceActivationException(ServiceActivationException activationException)
        {
            ...
            {
                TD.ServiceActivationException(
                    activationException != null ? activationException.ToString() : string.Empty,
                    activationException
                );
            }
            DiagnosticUtility
        ...
        [Fx.Tag.SecurityNote(
            Critical = "Calls getters with LinkDemands in ASP .NET objects, completes the request.",
            Safe = "Does not leak control or mutable/harmful data, no potential for harm."
        )]
        [SecuritySafeCritical]
