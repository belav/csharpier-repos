...
namespace System.Web.UI
{
    ...
    public abstract class UpdatePanelControlTrigger : UpdatePanelTrigger
    {
        ...
        {
            get { return _controlID ?? String.Empty; }
        ...
        protected Control FindTargetControl(bool searchNamingContainers)
        {
            if (String.IsNullOrEmpty(ControlID))
            {
                throw new InvalidOperationException(
                    String.Format(
                        ...
                        AtlasWeb.UpdatePanelControlTrigger_NoControlID,
                        Owner.ID
                    )
                );
            }
            Control foundControl = ControlUtil.FindTargetControl(
                ControlID,
                Owner,
            ...
            if (foundControl == null)
            {
                throw new InvalidOperationException(
                    String.Format(
                        CultureInfo.InvariantCulture,
                        AtlasWeb.UpdatePanelControlTrigger_ControlNotFound,
                        ControlID,
                        Owner.ID
                    )
                );
