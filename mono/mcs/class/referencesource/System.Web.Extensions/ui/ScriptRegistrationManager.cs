...
namespace System.Web.UI
{
    ...
    internal sealed class ScriptRegistrationManager
    {
        ...
        private static void CheckScriptTagTweenSpace(
        ...
        )
        {
            ...
            if (control.Page == null)
            {
                throw new ArgumentException(
                    AtlasWeb.ScriptRegistrationManager_ControlNotOnPage,
                    "control"
                );
            }
            ...
            if (sm != null)
            {
                RegisteredArrayDeclaration entry = new RegisteredArrayDeclaration(
                    control,
                    arrayName,
                    arrayValue
                );
        ...
        /// </summary>
        public static void RegisterFallbackScriptForAjaxPostbacks(
            Control control,
            Type type,
            string key,
        ...
        internal void RegisterDispose(Control control, string disposeScript)
        {
            ...
            if (parentUpdatePanel != null)
            {
                ...
                RegisteredDisposeScript entry = new RegisteredDisposeScript(
                    control,
                    disposeScript,
                    parentUpdatePanel
                );
        ...
        )
        {
            ...
            if (sm != null)
            {
                RegisteredExpandoAttribute entry = new RegisteredExpandoAttribute(
                    control,
                    controlId,
                    attributeName,
                    attributeValue,
        ...
        )
        {
            ...
            if (sm != null)
            {
                RegisteredHiddenField entry = new RegisteredHiddenField(
                    control,
                    hiddenFieldName,
        ...
        )
        {
            ...
            List<RegisteredArrayDeclaration> entriesToRender =
                new List<RegisteredArrayDeclaration>();
            ...
            foreach (RegisteredArrayDeclaration entry in ScriptArrays)
            {
                ...
                bool isActive =
                    ((lastControl != null) && (child == lastControl))
        ...
        public void RenderActiveExpandos(List<UpdatePanel> updatePanels, HtmlTextWriter writer)
        {
            ...
            List<RegisteredExpandoAttribute> entriesToRender =
            ...
            foreach (RegisteredExpandoAttribute activeRegistration in entriesToRender)
            {
                ...
                {
                    value =
                        "\"" + HttpUtility.JavaScriptStringEncode(activeRegistration.Value) + "\"";
                }
                ...
                PageRequestManager.EncodeString(
                    writer,
                    PageRequestManager.ExpandoToken,
                    propertyReference,
                    value
                );
            }
        ...
        private void RenderActiveScriptBlocks(
            List<UpdatePanel> updatePanels,
            HtmlTextWriter writer,
            string token,
        ...
        )
        {
            ...
            foreach (RegisteredScript activeRegistration in entriesToRender)
            {
                ...
                if (
                    _fallbackScripts != null
                    && _fallbackScripts.TryGetValue(
                        new ScriptKey(activeRegistration.Type, activeRegistration.Key),
                        out fallbackScriptPath
                    )
                )
                {
        ...
        public void RenderActiveScriptDisposes(
            List<UpdatePanel> updatePanels,
            HtmlTextWriter writer
        )
        {
        ...
        public void RenderActiveSubmitStatements(
            List<UpdatePanel> updatePanels,
            HtmlTextWriter writer
        )
        {
            ...
            foreach (RegisteredScript activeRegistration in entriesToRender)
            {
                PageRequestManager.EncodeString(
                    writer,
                    PageRequestManager.OnSubmitToken,
                    null,
                    activeRegistration.Script
                );
            }
        ...
        private static void WriteScriptWithTags(
            HtmlTextWriter writer,
            string token,
        ...
        )
        {
            ...
            int lastIndex = 0;
            for (
                Match match = ScriptTagRegex.Match(scriptContent, lastIndex);
                match.Success;
                match = ScriptTagRegex.Match(scriptContent, lastIndex)
            )
            {
                ...
                else
                {
                    ...
                    int indexOfScriptEndTag = scriptContent.IndexOf(
                        "</script>",
                        indexOfEndOfScriptBeginTag,
                    ...
                    if (indexOfScriptEndTag == -1)
                    {
                        throw new InvalidOperationException(
                            String.Format(
                                CultureInfo.InvariantCulture,
                                AtlasWeb.ScriptRegistrationManager_NoCloseTag,
                                activeRegistration.Type.FullName,
                                activeRegistration.Key
                            )
                        );
                    }
                    string scriptBlockContents = scriptContent.Substring(
                        indexOfEndOfScriptBeginTag,
                        (indexOfScriptEndTag - indexOfEndOfScriptBeginTag)
                    );
            ...
            CheckScriptTagTweenSpace(
                ...
                scriptContent,
                lastIndex,
                scriptContent.Length - lastIndex
            );
            ...
            if (lastIndex == 0)
            {
                throw new InvalidOperationException(
                    String.Format(
                        CultureInfo.InvariantCulture,
                        AtlasWeb.ScriptRegistrationManager_NoTags,
                        activeRegistration.Type.FullName,
                        activeRegistration.Key
                    )
                );
