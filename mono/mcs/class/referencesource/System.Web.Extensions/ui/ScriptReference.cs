...
namespace System.Web.UI
{
    ...
    public class ScriptReference : ScriptReferenceBase
    {
        ...
        private static readonly Hashtable _scriptPathCache = Hashtable.Synchronized(
            new Hashtable()
        );
        ...
        internal ScriptReference(
            string name,
            IClientUrlResolver clientUrlResolver,
            ...
            : this()
        {
            ...
            Debug.Assert(
                clientUrlResolver != null && clientUrlResolver is ScriptManager,
                "The clientUrlResolver must be the ScriptManager."
            );
            Name = name;
        ...
        {
            get { return (_assembly == null) ? String.Empty : _assembly; }
            set
            {
                _assembly = value;
                _scriptInfo = null;
        ...
        {
            get { return ScriptInfo.Assembly; }
        }

        internal string EffectivePath
        {
            get { return String.IsNullOrEmpty(Path) ? ScriptInfo.Path : Path; }
        }

        internal string EffectiveResourceName
        {
            get { return ScriptInfo.ResourceName; }
        }
        ...
        [Category("Behavior"), DefaultValue(""), ResourceDescription("ScriptReference_Name")]
        public string Name
        {
        ...
        private string AddCultureName(ScriptManager scriptManager, string resourceName)
        {
            ...
            CultureInfo culture = (
                scriptManager.EnableScriptLocalization
                    ? DetermineCulture(scriptManager)
        ...
        private static string AddCultureName(CultureInfo culture, string resourceName)
        {
            ...
            {
                resourceName =
                    resourceName.Substring(0, resourceName.Length - 2) + culture.Name + ".js";
            }
            return resourceName;
        }

        internal bool DetermineResourceNameAndAssembly(
        ...
        )
        {
            ...
            bool isDebug = ShouldUseDebugScript(
                resourceName,
                assembly,
                isDebuggingEnabled,
        ...
        internal CultureInfo DetermineCulture(ScriptManager scriptManager)
        {
            if ((ResourceUICultures == null) || (ResourceUICultures.Length == 0))
            {
                ...
                {
                    return ScriptResourceHandler.DetermineNearestAvailableCulture(
                        GetAssembly(scriptManager),
        ...
        private static string GetDebugName(string releaseName)
        {
            ...
            if (!releaseName.EndsWith(".js", StringComparison.Ordinal))
            {
                throw new InvalidOperationException(
                    String.Format(
                        CultureInfo.CurrentUICulture,
                        AtlasWeb.ScriptReference_InvalidReleaseScriptName,
                        releaseName
                    )
                );
            }
        ...
        )
        {
            ...
            if (!String.IsNullOrEmpty(EffectiveResourceName))
            {
                ...
                isDebuggingEnabled = DetermineResourceNameAndAssembly(
                    scriptManager,
                    isDebuggingEnabled,
                    ref resourceName,
                    ref assembly
        ...
        private static string GetScriptPathCached(
            string resourceName,
            Assembly assembly,
            CultureInfo culture
        )
        {
            Tuple<string, Assembly, CultureInfo> key = Tuple.Create(
                resourceName,
                assembly,
                culture
            );
        ...
        protected internal override string GetUrl(ScriptManager scriptManager, bool zip)
        {
            ...
            {
                throw new InvalidOperationException(
                    AtlasWeb.ScriptReference_NameAndPathCannotBeEmpty
                );
            }
            ...
            else if (!String.IsNullOrEmpty(ScriptInfo.Path))
            {
                ...
                if (useCdnPath)
                {
                    ...
                    if (!String.IsNullOrEmpty(resourceName))
                    {
                        ...
                        hasDebugResource = DetermineResourceNameAndAssembly(
                            scriptManager,
                            IsDebuggingEnabled(scriptManager),
                            ref resourceName,
                            ref assembly
                        );
                    }
                    string cdnPath = GetUrlForCdn(
                        scriptManager,
                        resourceName,
                        assembly,
                        hasDebugResource
        ...
        )
        {
            ...
            string cdnPath = isDebuggingEnabled
                ? (
                    secureConnection
                        ? ScriptInfo.CdnDebugPathSecureConnection
                        : ScriptInfo.CdnDebugPath
                )
            ...
            if (
                ...
                && String.IsNullOrEmpty(
                    isDebuggingEnabled ? ScriptInfo.CdnDebugPath : ScriptInfo.CdnPath
                )
            )
            {
                ScriptResourceInfo scriptResourceInfo = ScriptResourceInfo.GetInstance(
                    assembly,
                    resourceName
                );
                if (scriptResourceInfo != null)
                {
                    cdnPath = secureConnection
                        ? scriptResourceInfo.CdnPathSecureConnection
                        : scriptResourceInfo.CdnPath;
                }
            }
            return String.IsNullOrEmpty(cdnPath)
                ? null
                : ClientUrlResolver.ResolveClientUrl(AddCultureName(scriptManager, cdnPath));
        }

        private string GetUrlFromName(
            ScriptManager scriptManager,
            IControl scriptManagerControl,
            bool zip,
            bool useCdnPath
        )
        {
            string resourceName = EffectiveResourceName;
            Assembly assembly = GetAssembly(scriptManager);
            bool hasDebugResource = DetermineResourceNameAndAssembly(
                scriptManager,
                IsDebuggingEnabled(scriptManager),
                ref resourceName,
                ref assembly
...
#pragma warning restore 618
        ...
        internal bool ShouldUseDebugScript(
            string releaseName,
            Assembly assembly,
            bool isDebuggingEnabled,
        ...
        )
        {
            ...
            if (isDebuggingEnabled)
            {
                ...
                if (
                    ...
                    && !WebResourceUtil.AssemblyContainsWebResource(assembly, debugName)
                )
                {
        ...
        internal class ScriptEffectiveInfo
        {
            ...
            public ScriptEffectiveInfo(ScriptReference scriptReference)
            {
                ...
                if (
                    ...
                    && String.IsNullOrEmpty(LoadSuccessExpression)
                )
                {
            ...
            {
                get { return _cdnDebugPathSecureConnection; }
            }
