namespace System.Web.UI.WebControls.Expressions
{
    ...
    public class MethodExpression : ParameterDataSourceExpression
    {
        ...
        {
            get { return (string)ViewState["TypeName"] ?? String.Empty; }
            set { ViewState["TypeName"] = value; }
        }

        public string MethodName
        {
            get { return (string)ViewState["MethodName"] ?? String.Empty; }
        ...
        public bool IgnoreIfNotFound
        {
            ...
            set { ViewState["IgnoreIfNotFound"] = value; }
        }
        ...
        internal MethodInfo ResolveMethod()
        {
            if (String.IsNullOrEmpty(MethodName))
            {
                throw new InvalidOperationException(
                    AtlasWeb.MethodExpression_MethodNameMustBeSpecified
                );
            }
            ...
            else if (MethodName.Contains("{0}"))
            {
                ...
                throw new InvalidOperationException(
                    AtlasWeb.MethodExpression_DataSourceMustBeIDynamicDataSource
                );
            }
        ...
        public override IQueryable GetQueryable(IQueryable source)
        {
            ...
            if (
                ...
                || !parameterArray[0].ParameterType.IsAssignableFrom(source.GetType())
            )
            {
                throw new InvalidOperationException(
                    String.Format(
                        CultureInfo.CurrentCulture,
                        AtlasWeb.MethodExpression_FirstParamterMustBeCorrectType,
                        MethodName,
                        source.GetType()
                    )
                );
            ...
            for (int i = 1; i < parameterArray.Length; ++i)
            {
                ...
                arguments[i] = DataSourceHelper.BuildObjectValue(
                    value,
                    param.ParameterType,
                    param.Name
                );
            ...
            if (result != null)
            {
                ...
                )
                {
                    throw new InvalidOperationException(
                        String.Format(
                            CultureInfo.CurrentCulture,
                            AtlasWeb.MethodExpression_ChangingTheReturnTypeIsNotAllowed,
