namespace System.Web.UI.WebControls
{
    ...
    [Designer(
        "System.Web.UI.Design.QueryExtenderDesigner, " + AssemblyRef.SystemWebExtensionsDesign
    )]
    ...
    public class QueryExtender : Control
    {
                ...
                if (_dataSource == null)
                {
                    if (String.IsNullOrEmpty(TargetControlID))
                    {
                        throw new InvalidOperationException(
                            AtlasWeb.DataSourceControlExtender_TargetControlIDMustBeSpecified
                        );
                    }
                    _dataSource =
                        DataBoundControlHelper.FindControl(this, TargetControlID)
                        as IQueryableDataSource;
                    //
                    if (_dataSource == null)
                    {
                        throw new InvalidOperationException(
        ...
        protected internal override void OnInit(EventArgs e)
        {
            if (!DesignMode)
            {
                DataSource.QueryCreated += new EventHandler<QueryCreatedEventArgs>(
