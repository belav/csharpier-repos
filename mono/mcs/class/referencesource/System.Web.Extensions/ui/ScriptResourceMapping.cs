namespace System.Web.UI
{
    ...
    public class ScriptResourceMapping : IScriptResourceMapping
    {
        ...
        )
        {
            ...
            if (
                String.IsNullOrEmpty(definition.ResourceName)
                && String.IsNullOrEmpty(definition.Path)
            )
            {
                throw new ArgumentException(
                    AtlasWeb.ScriptResourceDefinition_NameAndPathCannotBeEmpty,
                    "definition"
                );
            }
        ...
        private void EnsureAbsoluteOrAppRelative(string path)
        {
            if (
                ...
                && !UrlPath.IsAppRelativePath(path)
                && // ~/foo..
                !UrlPath.IsRooted(path)
                && // /foo
        ...
        IScriptResourceDefinition IScriptResourceMapping.GetDefinition(
            string name,
            Assembly assembly
        )
        {
