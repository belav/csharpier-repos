...
namespace System.Web.UI
{
    ...
    internal class ScriptControlManager
    {
        ...
        public void AddScriptReferences(List<ScriptReferenceBase> scriptReferences)
        {
            ...
            {
                Debug.Fail(
                    "AddScriptReferences should only be called once per request but it was already called during this request."
                );
            }
#endif
        ...
        private void AddScriptReferencesForScriptControls(
            List<ScriptReferenceBase> scriptReferences
        )
        {
        ...
        private static void AddScriptReferenceForScriptControl(
            List<ScriptReferenceBase> scriptReferences,
            IScriptControl scriptControl
        )
        {
            IEnumerable<ScriptReference> scriptControlReferences =
            ...
            if (scriptControlReferences != null)
            {
                ...
                foreach (ScriptReference scriptControlReference in scriptControlReferences)
                {
                    if (scriptControlReference != null)
                    {
                        ...
                        {
                            urlResolverWrapper = new ClientUrlResolverWrapper(
                                scriptControlAsControl
                            );
                        }
        ...
        private void AddScriptReferencesForExtenderControls(
            List<ScriptReferenceBase> scriptReferences
        )
        {
            // PERF: Use field directly to avoid creating Dictionary if not already created
        ...
        private static void AddScriptReferenceForExtenderControl(
            List<ScriptReferenceBase> scriptReferences,
            IExtenderControl extenderControl
        )
        {
            IEnumerable<ScriptReference> extenderControlReferences =
            ...
            where TExtenderControl : Control, IExtenderControl
        {
            ...
            if (!_pagePreRenderRaised)
            {
                throw new InvalidOperationException(
                    AtlasWeb.ScriptControlManager_RegisterExtenderControlTooEarly
                );
            }
            if (_scriptReferencesRegistered)
            {
                throw new InvalidOperationException(
                    AtlasWeb.ScriptControlManager_RegisterExtenderControlTooLate
                );
            }
            ...
            where TScriptControl : Control, IScriptControl
        {
            ...
            {
                throw new InvalidOperationException(
                    AtlasWeb.ScriptControlManager_RegisterScriptControlTooEarly
                );
            }
            if (_scriptReferencesRegistered)
            {
                throw new InvalidOperationException(
                    AtlasWeb.ScriptControlManager_RegisterScriptControlTooLate
                );
            }
        ...
        private void RegisterScriptsForScriptDescriptors(
            IEnumerable<ScriptDescriptor> scriptDescriptors,
            Control control
        )
        {
