...
namespace System.Web.UI
{
    ...
    public sealed class ScriptResourceAttribute : Attribute
    {
        ...
        [SuppressMessage(
            ...
            MessageId = "string",
            Justification = "Refers to 'string resource', not string the type."
        )]
        ...
        )
        {
            ...
            foreach (Match match in matches)
            {
                ...
                try
                {
                    ...
                    {
                        output.Append(
                            ScriptResourceHandler.GetScriptResourceUrl(
                                assembly,
        ...
        )
        {
            ...
            if (
                ...
                && (
                    ...
                    || (releaseResourceInfo == ScriptResourceInfo.Empty)
                )
            )
            {
            ...
            try
            {
                ...
                {
                    releaseResourceManager = GetResourceManager(
                        releaseResourceInfo.ScriptResourceName,
                        assembly
                    );
                    releaseNeutralSet = releaseResourceManager.GetResourceSet(
                        CultureInfo.InvariantCulture,
                        true,
                        true
                    );
        ...
        )
        {
            using (
                StreamReader reader = new StreamReader(
                    assembly.GetManifestResourceStream(resourceInfo.ScriptName),
                    true
                )
            )
            {
                if (resourceInfo.IsDebug)
                {
                    ...
                    output.AppendLine(
                        "// FileVersion: " + AssemblyUtil.GetAssemblyFileVersion(assembly)
                    );
                }
