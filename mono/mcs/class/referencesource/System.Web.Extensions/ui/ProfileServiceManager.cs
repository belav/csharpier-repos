...
namespace System.Web.UI
{
    ...
    public class ProfileServiceManager
    {
        ...
        internal static void ConfigureProfileService(
        ...
        )
        {
            ...
            if (proxies != null)
            {
                foreach (ScriptManagerProxy proxy in proxies)
                {
                    if (proxy.HasProfileServiceManager)
                    {
                        ...
                        // combine urls
                        profileServiceUrl = ApplicationServiceManager.MergeServiceUrls(
                            profileManager.Path,
                            profileServiceUrl,
                            proxy
                        );
                        ...
                        if (profileManager.HasLoadProperties)
                        {
                            ...
                            {
                                loadedProperties = ProfileServiceManager.MergeProperties(
                                    loadedProperties,
                                    profileManager._loadProperties
                                );
                            }
                        }
                    }
            ...
            ProfileServiceManager.GenerateInitializationScript(
                ref sb,
                context,
                scriptManager,
                profileServiceUrl,
        ...
        )
        {
            ...
            if (ApplicationServiceHelper.ProfileServiceEnabled)
            {
                ...
                defaultServicePath = scriptManager.ResolveClientUrl(
                    "~/" + System.Web.Script.Services.WebServiceData._profileServiceFileName
                );
            ...
            if (!String.IsNullOrEmpty(serviceUrl))
            {
                ...
                )
                {
                    throw new InvalidOperationException(
                        AtlasWeb.ProfileServiceManager_LoadProperitesWithNonDefaultPath
                    );
                }
                if (sb == null)
                {
                ...
                else if (context != null)
                {
                    ...
                    foreach (string propertyFullName in loadedProperties)
                    {
                        GetSettingsProperty(
                            ...
                            propertyFullName,
                            topLevelSettings,
                            ref profileGroups, /* ensure exists */
                            true
                        );
                    }
        ...
        internal static ArrayList MergeProperties(
        ...
        )
        {
            ...
            foreach (string property in newProperties)
            {
                if (!String.IsNullOrEmpty(property))
                {
                    ...
                    if (
                        (trimmedProperty.Length > 0)
                        && !existingProperties.Contains(trimmedProperty)
                    )
                    {
            ...
            if (ensureExists && !exists)
            {
                throw new ArgumentException(
                    string.Format(
                        ...
                        AtlasWeb.AppService_UnknownProfileProperty,
                        fullPropertyName
                    )
                );
            }
        ...
        private static void RenderProfileProperties(
            StringBuilder sb,
            SortedList<string, object> topLevelSettings,
            SortedList<string, SortedList<string, object>> profileGroups
        )
        {
            ...
            sb.Append(
                serializer.Serialize(
                    topLevelSettings,
                    JavaScriptSerializer.SerializationFormat.JavaScript
                )
            );
            sb.Append(";\n");
        ...
        {
            get { return _loadProperties != null && _loadProperties.Length > 0; }
        }
