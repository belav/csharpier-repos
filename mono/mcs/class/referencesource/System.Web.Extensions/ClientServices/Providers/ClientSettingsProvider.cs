...
namespace System.Web.ClientServices.Providers
{
    ...
    public class ClientSettingsProvider : SettingsProvider, IApplicationSettingsProvider
    {
        ...
        public static SettingsPropertyCollection GetPropertyMetadata(string serviceUri)
        {
            ...
            else
            {
                object o = ProxyHelper.CreateWebRequestAndGetResponse(
                    serviceUri + "/GetPropertiesMetadata",
                    ref cookies,
                    id.Name,
                    null,
                    null,
        ...
        private static void AddToColl(
            ProfilePropertyMetadata p,
            SettingsPropertyCollection retColl,
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            string temp = System.Configuration.ConfigurationManager.AppSettings[
                "ClientSettingsProvider.ServiceUri"
            ];
            if (!string.IsNullOrEmpty(temp))
                ServiceUri = temp;

            temp = System.Configuration.ConfigurationManager.AppSettings[
            ...
            else
            {
                ...
                if (obj != null)
                {
                    ...
                    foreach (KeyValuePair<string, object> de in ret)
                    {
                        ...
                        if (
                            de.Value != null
                            && !setting.PropertyType.IsAssignableFrom(de.Value.GetType())
                        )
                        {
        ...
        )
        {
            ...
            else
            {
                ...
                object o = ProxyHelper.CreateWebRequestAndGetResponse(
                    GetServiceUri() + "/SetPropertiesForCurrentUser",
                    ref cookies,
                    id.Name,
                    _ConnectionString,
                    _ConnectionStringProvider,
        ...
        private void GetPropertyValuesFromSQL()
        {
            if (_UsingFileSystemStore || _UsingIsolatedStore)
            {
                ...
                if (
                    cd.SettingsNames.Length != cd.SettingsStoredAs.Length
                    || cd.SettingsValues.Length != cd.SettingsStoredAs.Length
                )
                {
                    return; // Bad!
                }
                ...
                {
                    AddProperty(
                        cd.SettingsNames[iter],
                        cd.SettingsStoredAs[iter],
                        cd.SettingsValues[iter]
                    );
                }
                return;
            }
        ...
        private void OnUserValidated(object src, UserValidatedEventArgs e)
        {
            ...
            if (
                _Properties != null
                && _Properties.Count > 0
                && string.Compare(e.UserName, _UserName, StringComparison.OrdinalIgnoreCase) != 0
            )
            {
                try
                {
