...
namespace System.Web.ClientServices.Providers
{
    ...
    public class ClientFormsAuthenticationMembershipProvider : MembershipProvider
    {
        ...
        public static bool ValidateUser(string username, string password, string serviceUri)
        {
            ...
            bool validated = ValidateUserByCallingLogin(
                username,
                password,
                false,
                serviceUri,
            ...
            if (validated)
            {
                Thread.CurrentPrincipal = new ClientRolePrincipal(
        ...
        private static bool ValidateUserByCallingLogin(
            string username,
            string password,
            bool rememberMe,
            string serviceUri,
                ...
                if (!String.IsNullOrEmpty(attribUnrecognized))
                    throw new ArgumentException(
                        string.Format(
                            ...
                            AtlasWeb.AttributeNotRecognized,
                            attribUnrecognized
                        )
                    );
        }
            ...
            lock (this)
            {
                ...
                if (ValidateUserCore(username, password, rememberMeInt, ref promptCount, true))
                {
                    if (UserValidated != null)
                        UserValidated(
                            this,
                            new UserValidatedEventArgs(Thread.CurrentPrincipal.Identity.Name)
                        );
                    return true;
                }
        ...
        public void Logout()
        {
            ...
            lock (this)
            {
                if (!ConnectivityStatus.IsOffline)
                {
                    ...
                    {
                        ProxyHelper.CreateWebRequestAndGetResponse(
                            GetServiceUri() + "/Logout",
                            ref cookies,
                            p.Identity.Name,
                            _ConnectionString,
        ...
        public string ServiceUri
        {
            ...
            set
            {
                ...
                {
                    _UsingWFCService = _ServiceUri.EndsWith(
                        ".svc",
        ...
        /////////////////////////////////////////////////////////////////////////////////
        // Private methods
        private bool ValidateUserCore(
            string username,
            string password,
            int rememberMeInt,
            ref int promptCount,
            bool tryToUseLastLoggedInUser
        )
        {
            ...
            string lastLoggedInUser = (
                tryToUseLastLoggedInUser ? GetLastUserNameFromOffileStore() : null
            );
            ...
            if (
                ...
                && Thread.CurrentPrincipal.Identity is ClientFormsIdentity
            )
            ...
            if (
                (Thread.CurrentPrincipal is ClientRolePrincipal)
                && (Thread.CurrentPrincipal.Identity is ClientFormsIdentity)
            ...
            if (
                !string.IsNullOrEmpty(lastLoggedInUser)
                && string.Compare(lastLoggedInUser, username, StringComparison.OrdinalIgnoreCase)
                    == 0
            )
            {
                if (!ConnectivityStatus.IsOffline)
                ...
                {
                    validated = ProxyHelper.DoAnyCookiesExist(
                        GetServiceUri(),
                        lastLoggedInUser,
                        _ConnectionString,
                        _ConnectionStringProvider
            ...
            {
                if (cookies == null)
                    cookies = ProxyHelper.ConstructCookieContainer(
                        GetServiceUri(),
                        username,
                        _ConnectionString,
                        _ConnectionStringProvider
                ...
                Thread.CurrentPrincipal = new ClientRolePrincipal(
                    new ClientFormsIdentity(username, password, this, "ClientForms", true, cookies)
                );
            }
            if (
                currentUser != null
                && string.Compare(username, currentUser, StringComparison.OrdinalIgnoreCase) != 0
            )
                SqlHelper.DeleteAllCookies(
                    currentUser,
                    _ConnectionString,
        ...
        private string GetLastUserNameFromOffileStore()
        {
            ...
            using (
                DbConnection conn = SqlHelper.GetConnection(
                    null,
                    _ConnectionString,
                    _ConnectionStringProvider
                )
            ...
            )
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(username))
                    {
                        ...
                        SqlHelper.AddParameter(
                            conn,
                            cmd,
                            "@Date",
                            DateTime.Now.ToFileTimeUtc().ToString(CultureInfo.InvariantCulture)
                        );
                        cmd.Transaction = trans;
                        cmd.ExecuteNonQuery();
                    }
