...
namespace System.Web.Script.Services
{
    ...
    internal class WebServiceData : JavaScriptTypeResolver
    {
        ...
        private static WebServiceData GetApplicationService(string appRelativePath)
        {
            ...
            if (slashIndex == 1)
            {
                ...
                if (name.Equals(_profileServiceFileName, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    name.Equals(_authenticationServiceFileName, StringComparison.OrdinalIgnoreCase)
                )
                {
                    return new WebServiceData(
                        typeof(System.Web.Security.AuthenticationService),
                        false
                    );
                }
        ...
        internal static WebServiceData GetWebServiceData(
            HttpContext context,
            string virtualPath,
            bool failIfNoData,
            bool pageMethods
            ...
            if (data == null)
            {
                if (HostingEnvironment.VirtualPathProvider.FileExists(virtualPath))
                {
                    ...
                    try
                    {
                        ...
                        {
                            object page = BuildManager.CreateInstanceFromVirtualPath(
                                virtualPath,
                    ...
                    if (compiledType != null)
                    {
                        ...
                        BuildDependencySet deps = BuildManager.GetCachedBuildDependencySet(
                            context,
                            virtualPath
                        );
                        if (deps != null)
                    ...
                    if (inlineScript)
                    {
                        ...
                        throw new InvalidOperationException(
                            String.Format(
                                ...
                                AtlasWeb.WebService_NoWebServiceDataInlineScript,
                                virtualPath
                            )
                        );
                    }
...
#pragma warning disable 0436
            ScriptingJsonSerializationSection.ApplicationSettings settings =
                new ScriptingJsonSerializationSection.ApplicationSettings();
#pragma warning restore 0436
            ...
            : this(new WebServiceTypeData(type.Name, type.Namespace, type))
        {
            ...
            if (!_pageMethods)
            {
                ...
                {
                    throw new InvalidOperationException(
                        AtlasWeb.WebService_NoScriptServiceAttribute
                    );
                }
            }
        }
        ...
        internal WebServiceData(
            ...
            Dictionary<string, WebServiceMethodData> methods
        )
            : this(typeData)
        {
            _methods = methods;
        ...
        private void AddMethod(Dictionary<string, WebServiceMethodData> methods, MethodInfo method)
        {
            ...
            object[] responseAttribs = method.GetCustomAttributes(
                typeof(ScriptMethodAttribute),
                true
            );
            ...
            WebServiceMethodData wmd = new WebServiceMethodData(
                this,
                method,
                (WebMethodAttribute)wmAttribs[0],
                sm
            );
        ...
        private void EnsureMethods()
        {
            ...
            lock (this)
            {
                ...
                Dictionary<string, WebServiceMethodData> methods = new Dictionary<
                    string,
        ...
        {
            Dictionary<string, WebServiceTypeData> clientTypeDictionary =
            ...
            if (!_methods.TryGetValue(methodName, out methodData))
            {
                throw new ArgumentException(
                    String.Format(
                        ...
                        AtlasWeb.WebService_UnknownWebMethod,
                        methodName
                    ),
                    "methodName"
                );
        ...
        private void ProcessClientTypes()
        {
            Debug.Assert(
                ...
                "ProcessClientTypes shouldn't be called after it has already been successfully run."
            );
        ...
        [SuppressMessage(
            ...
            MessageId = "_clientTypeNameDictionary",
        ...
        internal void ProcessClientType(Type t, bool force, bool isWCF)
        {
            ...
            if (t.IsEnum)
            {
                ...
                {
                    enumData = new WebServiceEnumData(
                        t.Name,
                        t.Namespace,
                        t,
                        Enum.GetNames(t),
                        Enum.GetValues(t),
                        Enum.GetUnderlyingType(t) == typeof(ulong)
                    );
                }
                _enumTypesDictionary[GetTypeStringRepresentation(enumData.TypeName, false)] =
                    enumData;
                return;
            }
            ...
            else
            {
                ...
                {
                    string typeStringRepresentation = GetTypeStringRepresentation(
                        t.FullName,
                        false
                    );
                    _clientTypesDictionary[typeStringRepresentation] = new WebServiceTypeData(
                        t.Name,
                        t.Namespace,
                        t
                    );
        ...
        [SuppressMessage(
            ...
            "CA2301:EmbeddableTypesInContainersRule",
            MessageId = "_clientTypeNameDictionary",
            Justification = "This is used by ASP.Net web services which is a legacy technology. Fun fact : The current code path suggests that this method is never hit!!"
        )]
        private void ProcessKnownTypes(Type t)
        {
            ...
            if (!alreadyProcessed)
            {
                IList<WebServiceTypeData> knownTypes = WebServiceTypeData.GetKnownTypes(
                    t,
                    typeData
                );
        ...
        {
            get { return ClientTypeDictionary.Values; }
        }
        ...
        internal Dictionary<string, WebServiceTypeData> ClientTypeDictionary
        {
            ...
            set { _clientTypesDictionary = value; }
        }
        ...
        internal Dictionary<string, WebServiceEnumData> EnumTypeDictionary
        {
            ...
            set { _enumTypesDictionary = value; }
        }
