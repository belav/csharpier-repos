    ...
    internal class WebServiceTypeData
    {
        ...
        internal WebServiceTypeData(string name, string ns, Type type)
        {
            ...
            else
            {
                ...
                if (type == null)
                { // for WCF known types
                    _stringRepresentation = string.Format(
        ...
        private static bool CheckIfEnum(
            ...
            out XmlSchemaSimpleTypeRestriction simpleTypeRestriction
        )
        {
        ...
        static bool CheckIfEnumRestriction(XmlSchemaSimpleTypeRestriction restriction)
        {
            ...
            {
                return (
                    restriction.BaseTypeName.Name == StringLocalName
        ...
        internal static List<WebServiceTypeData> GetKnownTypes(
            Type type,
            WebServiceTypeData typeData
        )
        {
            ...
            foreach (XmlSchema schema in schemas)
            {
                ...
                foreach (XmlSchemaObject schemaObj in schema.Items)
                {
                    ...
                    if (
                        schemaType != null
                        && !(
                            schemaType.Name == typeData.TypeName
                            && schemaTargetNamespace == typeData.TypeNamespace
                        )
                    ...
                    )
                    {
                        ...
                        if (
                            CheckIfEnum(
                                ...
                                out simpleTypeRestriction
                            )
                        )
                        {
                            knownTypeData = ImportEnum(
                                XmlConvert.DecodeName(schemaType.Name),
                        ...
                        {
                            knownTypeData = new WebServiceTypeData(
                                XmlConvert.DecodeName(schemaType.Name),
                                schemaTargetNamespace
                            );
                        }
        ...
        internal static WebServiceTypeData GetWebServiceTypeData(Type type)
        {
            ...
            if (!qname.IsEmpty)
            {
                if (type.IsEnum)
                {
                    ...
                    typeData = new WebServiceEnumData(
                        XmlConvert.DecodeName(qname.Name),
                        XmlConvert.DecodeName(qname.Namespace),
                        Enum.GetNames(type),
                        Enum.GetValues(type),
                        isUlong
                    );
                ...
                {
                    typeData = new WebServiceTypeData(
                        XmlConvert.DecodeName(qname.Name),
                        XmlConvert.DecodeName(qname.Namespace)
                    );
                }
        ...
        static XmlElement ImportAnnotation(
            XmlSchemaAnnotation annotation,
        ...
        static WebServiceEnumData ImportEnum(
            string typeName,
        ...
        )
        {
            ...
            XmlQualifiedName baseTypeName = ImportActualType(
                annotation,
            ...
            foreach (XmlSchemaFacet facet in restriction.Facets)
            {
                ...
                string valueInnerText = GetInnerText(
                    typeQualifiedName,
                    ImportAnnotation(enumFacet.Annotation, EnumerationValueAnnotationName)
                );
                long value;
