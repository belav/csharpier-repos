...
namespace System.Web.Script.Serialization
{
    ...
    internal class JavaScriptObjectDeserializer
    {
        ...
        internal static object BasicDeserialize(
            string input,
            int depthLimit,
            JavaScriptSerializer serializer
        )
        {
            JavaScriptObjectDeserializer jsod = new JavaScriptObjectDeserializer(
                input,
                depthLimit,
                serializer
            );
            ...
            if (jsod._s.GetNextNonEmptyChar() != null)
            {
                throw new ArgumentException(
                    String.Format(
                        ...
                        AtlasWeb.JSON_IllegalPrimitive,
                        jsod._s.ToString()
                    )
                );
            }
        ...
        private JavaScriptObjectDeserializer(
            string input,
            int depthLimit,
            JavaScriptSerializer serializer
        )
        ...
        private IList DeserializeList(int depth)
        {
            ...
            while ((c = _s.GetNextNonEmptyChar()) != null && c != ']')
            {
                ...
                {
                    throw new ArgumentException(
                        _s.GetDebugString(AtlasWeb.JSON_InvalidArrayExpectComma)
                    );
                }
            }
            if (expectMore)
            {
                throw new ArgumentException(
                    _s.GetDebugString(AtlasWeb.JSON_InvalidArrayExtraComma)
                );
            }
            if (c != ']')
            {
        ...
        private void ThrowIfMaxJsonDeserializerMembersExceeded(int count)
        {
            if (count >= AppSettings.MaxJsonDeserializerMembers)
            {
                throw new InvalidOperationException(
                    SR.GetString(
                        SR.CollectionCountExceeded_JavaScriptObjectDeserializer,
        ...
        private object DeserializePrimitiveObject()
        {
            ...
            throw new ArgumentException(
                String.Format(CultureInfo.InvariantCulture, AtlasWeb.JSON_IllegalPrimitive, input)
            );
        }
        ...
        private object DeserializeStringIntoDateTime()
        {
            ...
            Match match = Regex.Match(
                _s.Substring(pos + DateTimeSuffixLength),
            ...
            if (long.TryParse(ticksStr, out ticks))
            {
                ...
                DateTime dt = new DateTime(
                    ticks * 10000 + JavaScriptSerializer.DatetimeMinTimeTicks,
                    DateTimeKind.Utc
                );
                return dt;
            }
