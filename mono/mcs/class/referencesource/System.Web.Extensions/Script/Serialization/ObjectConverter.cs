...
namespace System.Web.Script.Serialization
{
    internal static class ObjectConverter
    {
        ...
        private static bool AddItemToList(
            IList oldList,
            IList newList,
            Type elementType,
        ...
        private static bool AssignToPropertyOrField(
        ...
        )
        {
            ...
            PropertyInfo propInfo = serverType.GetProperty(
                memberName,
            ...
            FieldInfo fieldInfo = serverType.GetField(
                memberName,
                BindingFlags.Instance | BindingFlags.IgnoreCase | BindingFlags.Public
            );
        ...
        private static bool ConvertDictionaryToObject(
        ...
        )
        {
            ...
            if (
                suggestedType != null
                && suggestedType.GetCustomAttributes(typeof(DataContractAttribute), false).Length
                    > 0
            )
                    ...
                    if (throwOnError)
                    {
                        throw new InvalidOperationException(
                            String.Format(
                                ...
                                AtlasWeb.JSON_DictionaryTypeNotSupported,
                                type.FullName
                            )
                        );
                    }
            ...
            if (type != null && !type.IsAssignableFrom(o.GetType()))
            {
                ...
                throw new InvalidOperationException(
                    String.Format(
                        CultureInfo.InvariantCulture,
                        AtlasWeb.JSON_DeserializerTypeMismatch,
                        type.FullName
                    )
                );
            }
            ...
            foreach (string memberName in memberNames)
            {
                ...
                if (
                    !AssignToPropertyOrField(propertyValue, o, memberName, serializer, throwOnError)
                )
                {
                    convertedObject = null;
                    return false;
        ...
        internal static object ConvertObjectToType(
            object o,
            Type type,
            JavaScriptSerializer serializer
        )
        ...
        private static bool ConvertObjectToTypeMain(
            object o,
            Type type,
            JavaScriptSerializer serializer,
            bool throwOnError,
        ...
        )
        {
            ...
            return ConvertObjectToTypeInternal(
                o,
                type,
                serializer,
                throwOnError,
        ...
        private static bool ConvertObjectToTypeInternal(
            object o,
            Type type,
            JavaScriptSerializer serializer,
            bool throwOnError,
        ...
        )
        {
            ...
            if (dictionary != null)
            {
                return ConvertDictionaryToObject(
                    dictionary,
                    type,
                    serializer,
                    throwOnError,
            ...
            if (throwOnError)
            {
                throw new InvalidOperationException(
                    String.Format(
                        ...
                        AtlasWeb.JSON_CannotConvertObjectToType,
                        o.GetType(),
                        type
                    )
                );
        ...
        private static bool ConvertListToObject(
            IList list,
            Type type,
            JavaScriptSerializer serializer,
            bool throwOnError,
            out IList convertedList
        )
        {
            ...
            else if (type.IsGenericType && type.GetGenericArguments().Length == 1)
            {
                ...
                if (strongTypedEnumerable.IsAssignableFrom(type))
                {
                    ...
                    IList newList = null;
                    if (
                        IsClientInstantiatableType(type, serializer)
                        ...
                        if (t.IsAssignableFrom(type))
                        {
                            if (throwOnError)
                            {
                                throw new InvalidOperationException(
                                    String.Format(
                                        CultureInfo.InvariantCulture,
                                        AtlasWeb.JSON_CannotCreateListType,
                                        type.FullName
                                    )
                                );
                            }
            ...
            // If the custom type implements IList and it's instantiable. Use that type.
                ...
                IsClientInstantiatableType(type, serializer) && typeof(IList).IsAssignableFrom(type)
            )
            {
            ...
            if (throwOnError)
            {
                throw new InvalidOperationException(
                    String.Format(
                        CultureInfo.CurrentCulture,
                        AtlasWeb.JSON_ArrayTypeNotSupported,
                        type.FullName
                    )
                );
            }
        ...
        private static bool IsGenericDictionary(Type type)
        {
            return type != null
                ...
                && (
                    ...
                    || type.GetGenericTypeDefinition() == _idictionaryGenericType
                )
        ...
        internal static bool TryConvertObjectToType(
            object o,
            Type type,
            JavaScriptSerializer serializer,
            out object convertedObject
