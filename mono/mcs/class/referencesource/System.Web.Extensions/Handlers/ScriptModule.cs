...
namespace System.Web.Handlers
{
    ...
    public class ScriptModule : IHttpModule
    {
        ...
        private static Type _authenticationServiceType =
            typeof(System.Web.Security.AuthenticationService); // disambiguation required
        ...
        private static bool ShouldSkipAuthorization(HttpContext context)
        {
            ...
            if (
                !ApplicationServiceHelper.AuthenticationServiceEnabled
        ...
        private void EndRequestHandler(object sender, EventArgs e)
        {
            ...
            if ((o != null) && ((bool)o == true))
            {
                ...
                string errorMessage = (string)
                    context.Items[PageRequestManager.AsyncPostBackErrorMessageKey];
        ...
        private static void HttpResponse_Redirecting(object sender, EventArgs e)
        {
            ...
            if (PageRequestManager.IsAsyncPostBackRequest(new HttpRequestWrapper(context.Request)))
            {
                ...
                context.Items[PageRequestManager.AsyncPostBackRedirectLocationKey] =
                    redirectLocation;
                ...
                PageRequestManager.EncodeString(
                    response.Output,
                    PageRequestManager.UpdatePanelVersionToken,
                    String.Empty,
                    PageRequestManager.UpdatePanelVersionNumber
                );
                // url encode the location in a way that javascript unescape() will be able to reverse
                redirectLocation = String.Join(
                    " ",
                    redirectLocation.Split(' ').Select(part => HttpUtility.UrlEncode(part))
                );
                PageRequestManager.EncodeString(
                    response.Output,
                    PageRequestManager.PageRedirectToken,
                    String.Empty,
                    redirectLocation
                );
            }
            else if (RestHandlerFactory.IsRestRequest(context))
            {
                ...
                RestHandler.WriteExceptionJsonString(
                    context,
                    new InvalidOperationException(AtlasWeb.WebService_RedirectError),
