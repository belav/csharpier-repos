...
namespace System.Workflow.Activities
{
    ...
    static class ValidationHelper
    {
                ...
                switch (uc)
                {
                    ...
                    default:
                    {
                        ...
                        if (
                            isTypeName
                            && !nextMustBeStartChar
                            && !previousWasNamespaceSeparatorChar
                            && IsNamespaceSeparatorChar(ch, ref nextMustBeStartChar)
                        )
                        {
        ...
        )
        {
            ...
            if (serviceAttributes != null)
            {
                if (serviceAttributes.MaxItemsInObjectGraph < 0)
                {
                    validationErrors.Add(
                        new ValidationError(
                            SR2.GetString(SR2.Error_Validation_InvalidMaxItemsInObjectGraph),
            ...
            Dictionary<string, string> implementedInferredContracts =
                new Dictionary<string, string>();

            foreach (
                ReceiveActivity receiveActivity in GetActivities<ReceiveActivity>(rootActivity)
            )
            {
                ...
                if (toVerify.ContainsKey(typeName) && !toAddTo.ContainsKey(typeName))
                {
                    validationErrors.Add(
                        new ValidationError(
                            ...
                            WorkflowServicesErrorNumbers.Error_ContractNameDuplicate,
                            false
                        )
                    );
                }
            ...
            Dictionary<Type, Hashtable> implementedServiceOperations =
                new Dictionary<Type, Hashtable>();
            foreach (
                ReceiveActivity receiveActivity in GetActivities<ReceiveActivity>(rootActivity)
            )
            {
            ...
            Dictionary<Type, Hashtable> notImplementedServiceOperations =
                new Dictionary<Type, Hashtable>();
            ...
            foreach (Type contractType in implementedServiceOperations.Keys)
            {
                ...
                while (interfacesQueue.Count > 0)
                {
                    ...
                    if (
                        !contractList.Contains(currentInterfaceType)
                ...
                foreach (Type currentContractType in contractList)
                {
                    ...
                    foreach (MethodInfo methodInfo in currentContractType.GetMethods())
                    {
                        ...
                        string operationName = ServiceOperationHelpers.GetOperationName(
                            manager,
                            methodInfo
                        );
                        ...
                        if (!implementedServiceOperations.ContainsKey(currentContractType))
                        {
                            ...
                            if (!notImplementedServiceOperations.ContainsKey(currentContractType))
                            {
                                ...
                                notImplementedServiceOperations.Add(
                                    currentContractType,
                                    serviceOperationHashTable
                                );
                            }
                            else if (
                        ...
                        )
                        {
                            ...
                            implementedServiceOperations[currentContractType][operationName]
                            != (object)methodInfo
                        )
                        {
                            validationErrors.Add(
            ...
            else
            {
                if (string.IsNullOrEmpty(endpoint.Name))
                {
                    validationErrors.Add(
                        new ValidationError(
                            SR2.GetString(
                                SR2.Error_Validation_ChannelTokenNameNotSpecified,
                                activity.Name
                            ),
                ...
                if (
                    string.IsNullOrEmpty(endpoint.EndpointName)
                ...
                )
                {
                    validationErrors.Add(
                        new ValidationError(
                            SR2.GetString(
                                SR2.Error_Validation_ChannelTokenConfigurationNameNotSpecified,
                                activity.Name
                            ),
                ...
                if (!string.IsNullOrEmpty(endpoint.OwnerActivityName))
                {
                    ...
                    Activity sourceActivity = activity.GetActivityByName(
                    ...
                    if (!ownerIsParentOrItself)
                    {
                        validationErrors.Add(
                            new ValidationError(
                                SR2.GetString(
                                    SR2.Error_Validation_OwnerActivityNameNotFound,
                                    endpoint.OwnerActivityName,
                                    activity.Name
                                ),
        ...
        )
        {
            ...
            if (string.IsNullOrEmpty(contextToken.Name))
            {
                validationErrors.Add(
                    new ValidationError(
                        SR2.GetString(
                            SR2.Error_Validation_ContextTokenNameNotSpecified,
                            activity.Name
                        ),
        ...
        )
        {
            ...
            if (string.IsNullOrEmpty(operationInfo.Name))
            {
                validationErrors.Add(
                    new ValidationError(
                        SR2.GetString(
                            SR2.Error_Validation_OperationNameNotSpecified,
                            activity.Name
                        ),
                        WorkflowServicesErrorNumbers.Error_OperationNameNotSpecified,
                        false,
            ...
            if (operationInfo is OperationInfo)
            {
                ...
                if (
                    !string.IsNullOrEmpty(currentOperationInfo.Name)
                ...
                )
                {
                    validationErrors.Add(
                        new ValidationError(
                            ...
                            WorkflowServicesErrorNumbers.Error_OperationNameInvalid,
                            false,
                            "ServiceOperationInfo"
                        )
                    );
                ...
                if (string.IsNullOrEmpty(currentOperationInfo.ContractName))
                {
                    validationErrors.Add(
                        new ValidationError(
                            SR2.GetString(
                                SR2.Error_Validation_ContractNameNotSpecified,
                                activity.Name
                            ),
                ...
                foreach (
                    OperationParameterInfo operationParameterInfo in currentOperationInfo.Parameters
                )
                {
                ...
                )
                {
                    if (
                        ...
                        && operationParameterInfo.Position > maxPosition
                    )
                    {
                        validationErrors.Add(
                            new ValidationError(
                                SR2.GetString(
                                    SR2.Error_Validation_OperationParameterPosition,
                                    operationParameterInfo.Name,
                                    currentOperationInfo.Name,
                    ...
                    if (parameterIndexs.Contains(operationParameterInfo.Position))
                    {
                        validationErrors.Add(
                            new ValidationError(
                                SR2.GetString(
                                    SR2.Error_Validation_OperationParameterPositionDuplicate,
                                    operationParameterInfo.Name,
                                    currentOperationInfo.Name,
                                    currentOperationInfo.ContractName
                                ),
                    ...
                    if (
                        operationParameterInfo.Position != -1
                        && !IsValidTypeNameOrIdentifier(operationParameterInfo.Name, false)
                    )
                    {
                        validationErrors.Add(
                            new ValidationError(
                                SR2.GetString(
                                    SR2.Error_Validation_OperationParameterNameInvalid,
                                    operationParameterInfo.Name,
                                    currentOperationInfo.Name,
                                    currentOperationInfo.ContractName
                                ),
                    ...
                    if (parameterNames.Contains(operationParameterInfo.Name))
                    {
                        validationErrors.Add(
                            new ValidationError(
                                SR2.GetString(
                                    SR2.Error_Validation_OperationParameterNameDuplicate,
                                    operationParameterInfo.Name,
                                    currentOperationInfo.Name,
                                    currentOperationInfo.ContractName
                                ),
                    ...
                    if (
                        operationParameterInfo.Position != -1
                        && operationParameterInfo.ParameterType == typeof(void)
                    )
                    {
                        validationErrors.Add(
                            new ValidationError(
                                SR2.GetString(
                                    SR2.Error_Validation_OperationParameterType,
                                    operationParameterInfo.Name,
                                    currentOperationInfo.Name,
                                    currentOperationInfo.ContractName
                                ),
            ...
            else
            {
                ...
                if (currentOperationInfo.ContractType == null)
                {
                    validationErrors.Add(
                        new ValidationError(
                            SR2.GetString(
                                SR2.Error_Validation_ContractTypeNotSpecified,
                                activity.Name
                            ),
            ...
            else if (!contractType.IsInterface)
            {
                validationErrors.Add(
                    new ValidationError(
                        SR2.GetString(
                            SR2.Error_Validation_ContractTypeNotInterface,
            ...
            else if (!ServiceOperationHelpers.IsValidServiceContract(contractType))
            {
                validationErrors.Add(
                    new ValidationError(
                        SR2.GetString(
                            SR2.Error_ServiceContractAttributeMissing,
                            contractType.FullName
                        ),
            ...
            else
            {
                ...
                if (methodInfo == null)
                {
                    validationErrors.Add(
                        new ValidationError(
                            SR2.GetString(
                                SR2.Error_Validation_OperationNotInContract,
                                operationInfo.Name,
                                contractType.FullName
                            ),
                ...
                else if (ServiceOperationHelpers.IsAsyncOperation(manager, methodInfo))
                {
                    validationErrors.Add(
                        new ValidationError(
                            SR2.GetString(
                                SR2.Error_Validation_AsyncPatternOperationNotSupported,
                                operationInfo.Name
                            ),
                ...
                else
                {
                    ...
                    if (isOneWay && methodInfo.ReturnType != typeof(void))
                    {
                        validationErrors.Add(
                            new ValidationError(
                                SR2.GetString(
                                    SR2.Error_Validation_ReturnTypeInOneWayOperation,
                                    operationInfo.Name,
                                    contractType.FullName
                                ),
                    ...
                    {
                        if (
                            parameter.Position >= methodInfo.GetParameters().Length
                        ...
                        )
                        {
                            validationErrors.Add(
                                new ValidationError(
                                    SR2.GetString(
                                        SR2.Error_Validation_OperationParameterDirectionInOneWayOperation,
                                        parameter.Name,
                                        operationInfo.Name,
                                        contractType.FullName
                                    ),
                    ...
                    validationErrors.AddRange(
                        ValidationHelper.ValidateServiceModelAttributes(
                            activity,
                            contractType,
                            methodInfo,
                            manager
            ...
            foreach (ParameterInfo parameter in GetParameterInfo(methodInfo))
            {
                if (!(parameter.IsOut || parameter.Position == -1) || !isOneWayOperation)
                {
                    string parameterName =
                        (parameter.Position == -1) ? "(ReturnValue)" : parameter.Name;
                    ...
                    if (!parameterBindings.Contains(parameterName) || parameterValue == null)
                    {
        ...
        )
        {
            ...
            ValidationErrorCollection validationErrors = new ValidationErrorCollection();
            object[] serviceContractAttributes = contractType.GetCustomAttributes(
            ...
            else
            {
                if (!methodInfo.ReflectedType.IsAssignableFrom(contractType))
                {
                    validationErrors.Add(
                        new ValidationError(
                            SR2.GetString(
                                SR2.Error_OperationNotInContract,
                                methodInfo.Name,
                                contractType.FullName
                            ),
                            WorkflowServicesErrorNumbers.Error_OperationNotInContract,
                ...
                {
                    object[] operationContractAttrributes = methodInfo.GetCustomAttributes(
                        typeof(OperationContractAttribute),
                        true
                    );
                    if (
                        operationContractAttrributes == null
                        || operationContractAttrributes.Length == 0
                    )
                    {
                        validationErrors.Add(
                            new ValidationError(
                                SR2.GetString(
                                    SR2.Error_OperationContractAttributeMissing,
                                    methodInfo.Name
                                ),
                                WorkflowServicesErrorNumbers.Error_OperationContractAttributeMissing,
                                false,
                                "ServiceOperationInfo"
                            )
                        );
                    ...
                    else if (activity is ReceiveActivity)
                    {
                        ...
                        else if (serviceContractAttributes[0] is AttributeInfoAttribute)
                        {
                            AttributeInfoAttribute attribInfoAttrib =
                                serviceContractAttributes[0] as AttributeInfoAttribute;
                            if (
                            ...
                            {
                                contractSessionMode =
                                    ServiceOperationHelpers.GetContractSessionMode(
                                        manager,
                        ...
                        )
                        {
                            validationErrors.Add(
                                new ValidationError(
                                    SR2.GetString(
                                        SR2.Error_Validation_OperationIsOneWay,
                                        methodInfo.Name
                                    ),
                                    WorkflowServicesErrorNumbers.Error_OperationIsOneWay,
                                    false,
                                    "CanCreateInstance"
                                )
                            );
                        ...
                        if (
                            receiveActivity.CanCreateInstance == true
                            && !ServiceOperationHelpers.IsInitiatingOperation(manager, methodInfo)
                        )
                        {
                            validationErrors.Add(
                                new ValidationError(
                                    SR2.GetString(
                                        SR2.Error_OperationNotInitiating,
                                        methodInfo.Name
                                    ),
                                    WorkflowServicesErrorNumbers.Error_OperationNotInitiating,
                                    false,
                                    "CanCreateInstance"
                                )
                            );
