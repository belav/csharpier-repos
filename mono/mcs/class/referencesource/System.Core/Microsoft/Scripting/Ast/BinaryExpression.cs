    ...
    public class BinaryExpression : Expression
    {
        ...
        private Expression ReduceMember()
        {
            ...
            else
            {
                ...
                return Expression.Block(new ParameterExpression[] { temp1, temp2 }, e1, e2, e3, e4);
            }
        }
        ...
        {
            get { return IsLifted && TypeUtils.IsNullableType(Type); }
        }
    ...
    public partial class Expression
    {
                ...
                )
                {
                    if (method.ReturnType != typeof(bool) || liftToNull)
                    {
                        return new MethodBinaryExpression(
                            ...
                            left,
                            right,
                            TypeUtils.GetNullableType(method.ReturnType),
                            method
                        );
                    }
                    else
        ...
        )
        {
            ...
            if (
                ...
                && TypeUtils.IsNullableType(right.Type)
        ...
        private static void VerifyOpTrueFalse(ExpressionType nodeType, Type left, MethodInfo opTrue)
        {
            ...
            if (!ParameterIsAssignable(pmsOpTrue[0], left))
            {
                if (
                    !(
                        TypeUtils.IsNullableType(left)
                        && ParameterIsAssignable(pmsOpTrue[0], TypeUtils.GetNonNullableType(left))
                    )
                )
        ...
        )
        {
            return TypeUtils.AreEquivalent(left, right)
                ...
                && TypeUtils.AreEquivalent(
                    pms[1].ParameterType,
                    TypeUtils.GetNonNullableType(right)
                );
        }
                ...
                if (method != null)
                {
                    ...
                    returnType =
                        (
                            ...
                            && TypeUtils.AreEquivalent(
                                method.ReturnType,
                                TypeUtils.GetNonNullableType(left.Type)
                            )
                        )
                            ? left.Type
                            : method.ReturnType;
            ...
            if (method == null)
            {
                ...
                if (method != null)
                {
                    ...
                    returnType =
                        (
                            TypeUtils.IsNullableType(left.Type)
                            && method.ReturnType == TypeUtils.GetNonNullableType(left.Type)
                        )
                            ? left.Type
                            : method.ReturnType;
                    return new MethodBinaryExpression(
        ...
        private static Type ValidateCoalesceArgTypes(Type left, Type right)
        {
            ...
            if (left.IsValueType && !TypeUtils.IsNullableType(left))
            {
                ...
                TypeUtils.IsNullableType(left)
                && TypeUtils.IsImplicitlyConvertible(right, leftStripped)
            )
            {
                return leftStripped;
            }
        ...
        {
            return TypeUtils.IsInteger(left) && TypeUtils.GetNonNullableType(right) == typeof(int);
        }
