...
namespace System.Dynamic
{
    ...
    public class DynamicObject : IDynamicMetaObjectProvider
    {
        ...
        private sealed class MetaDynamic : DynamicMetaObject
        {
            ...
            {
                ReadOnlyCollectionBuilder<Expression> paramArgs =
                    new ReadOnlyCollectionBuilder<Expression>(args.Length);
                    ...
                    if (((ParameterExpression)args[i]).IsByRef)
                    {
                        ...
                        block.Add(
                            Expression.Assign(
                                args[i],
                                Expression.Convert(
                                    Expression.ArrayIndex(callArgs, Expression.Constant(i)),
                                    args[i].Type
                                )
                            )
                        );
            ...
            )
            {
                ...
                var callDynamic = BuildCallMethodWithResult(
                    methodName,
                    binder,
                    args,
                    fallbackResult,
                    ...
                    Expression.Block(
                        ...
                        Expression.Condition(
                            Expression.Call(
                                GetLimitedSelf(),
                                typeof(DynamicObject).GetMethod(methodName),
                                BuildCallArgs(binder, args, callArgs, null)
                            ),
                            Expression.Block(
                                ReferenceArgAssign(callArgs, args),
                                Expression.Empty()
            ...
            private bool IsOverridden(string method)
            {
                ...
                foreach (MethodInfo mi in methods)
                {
                    if (
                        ...
                        && mi.GetBaseDefinition().DeclaringType == typeof(DynamicObject)
                    )
                    {
                        return true;
                    }
