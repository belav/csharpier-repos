...
namespace System.Linq.Expressions.Compiler
{
    ...
    partial class LambdaCompiler
    {
        ...
        private void EmitBinaryExpression(Expression expr, CompilationFlags flags)
        {
            ...
            )
            {
                ...
                if (
                    ...
                    && TypeUtils.IsNullableType(b.Right.Type)
                )
                {
                ...
                if (
                    ConstantCheck.IsNull(b.Right)
                    && !ConstantCheck.IsNull(b.Left)
                    && TypeUtils.IsNullableType(b.Left.Type)
                )
                {
        ...
        private void EmitBinaryMethod(BinaryExpression b, CompilationFlags flags)
        {
            if (b.IsLifted)
            {
                ParameterExpression p1 = Expression.Variable(
                    TypeUtils.GetNonNullableType(b.Left.Type),
                    null
                );
                ParameterExpression p2 = Expression.Variable(
                    TypeUtils.GetNonNullableType(b.Right.Type),
                    null
                );
        ...
        )
        {
            ...
            ConstructorInfo ci = resultType.GetConstructor(
