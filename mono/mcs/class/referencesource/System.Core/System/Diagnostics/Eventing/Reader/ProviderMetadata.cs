...
namespace System.Diagnostics.Eventing.Reader
{
    ...
    public class ProviderMetadata : IDisposable
    {
        ...
        )
        {
            ...
            handle = NativeWrapper.EvtOpenProviderMetadata(
                this.session.Handle,
                this.providerName,
                this.logFilePath,
        ...
        public Guid Id
        {
            ...
            {
                return (Guid)
                    NativeWrapper.EvtGetPublisherMetadataProperty(
                        this.handle,
                        UnsafeNativeMethods.EvtPublisherMetadataPropertyId
                    ...
                    lock (syncObject)
                    {
                        ...
                        for (int index = 0; index < arraySize; index++)
                        {
                            string channelName = (string)
                                NativeWrapper.EvtGetObjectArrayProperty(
                                    elHandle,
                                    index,
                                    (int)
                                        UnsafeNativeMethods.EvtPublisherMetadataPropertyId
                            ...
                            if (
                                flag
                                == (int)
                                    UnsafeNativeMethods.EvtChannelReferenceFlags
                            ...
                            {
                                channelRefDisplayName = NativeWrapper.EvtFormatMessage(
                                    this.handle,
                                ...
                                if (channelRefMessageId != -1)
                                {
                                    if (this.defaultProviderHandle.IsInvalid)
                                    {
                                        this.defaultProviderHandle =
                                            NativeWrapper.EvtOpenProviderMetadata(
                                                this.session.Handle,
                                                null,
                                                null,
                                                this.cultureInfo.LCID,
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    case UnsafeNativeMethods.EvtPublisherMetadataPropertyId
                        .EvtPublisherMetadataLevels:
                        propName = UnsafeNativeMethods.EvtPublisherMetadataPropertyId
                            .EvtPublisherMetadataLevelName;
                        propValue = UnsafeNativeMethods.EvtPublisherMetadataPropertyId
                            .EvtPublisherMetadataLevelValue;
                        propMessageId = UnsafeNativeMethods.EvtPublisherMetadataPropertyId
                ...
                for (int index = 0; index < arraySize; index++)
                {
                    string generalName = (string)
                        NativeWrapper.EvtGetObjectArrayProperty(elHandle, index, (int)propName);
                    ...
                    if (generalMessageId == -1)
                    {
                        if (providerHandle != this.defaultProviderHandle)
                        {
                            if (this.defaultProviderHandle.IsInvalid)
                            {
                                this.defaultProviderHandle = NativeWrapper.EvtOpenProviderMetadata(
                                    this.session.Handle,
                                    null,
                                    null,
                                    this.cultureInfo.LCID,
                            ...
                            switch (objectTypeName)
                            {
                                case ObjectTypeName.Level:
                                    generalDisplayName = FindStandardLevelDisplayName(
                                        generalName,
                                        generalValue
                                    );
                                    break;
                                case ObjectTypeName.Opcode:
                                    generalDisplayName = FindStandardOpcodeDisplayName(
                                        generalName,
                                        generalValue >> 16
                                    );
                                    break;
                                case ObjectTypeName.Keyword:
                                    generalDisplayName = FindStandardKeywordDisplayName(
                                        generalName,
                                        generalValueKeyword
                                    );
                                    break;
                                case ObjectTypeName.Task:
                                    generalDisplayName = FindStandardTaskDisplayName(
                                        generalName,
                                        generalValue
                                    );
                                    break;
                    ...
                    {
                        generalDisplayName = NativeWrapper.EvtFormatMessage(
                            providerHandle,
                    ...
                    switch (objectTypeName)
                    {
                        case ObjectTypeName.Level:
                            levelList.Add(
                                new EventLevel(generalName, (int)generalValue, generalDisplayName)
                            );
                            break;
                        case ObjectTypeName.Opcode:
                            opcodeList.Add(
                        ...
                        using (emHandle)
                        {
                            uint emId = (uint)
                                NativeWrapper.EvtGetEventMetadataProperty(
                                    emHandle,
                                    UnsafeNativeMethods.EvtEventMetadataPropertyId
                            ...
                            if (messageId == -1)
                                ...
                                emMessage = NativeWrapper.EvtFormatMessage(
                                    this.handle,
                                    (uint)messageId
                                );
