...
namespace System.Diagnostics.Eventing
{
    ...
    public class EventProviderTraceListener : TraceListener
    {
                ...
                if (m_initializedDelim == 0)
                {
                    ...
                    if (m_delimiter.Length == 0)
                        throw new ArgumentException(
                            SR.GetString(SR.Argument_NeedNonemptyDelimiter)
                        );
                }
                return m_delimiter;
            }
        ...
        //
        public sealed override void TraceData(
        ...
        )
        {
            ...
            if (
                Filter != null
                && !Filter.ShouldTrace(eventCache, source, eventType, id, null, null, null, null)
            )
            {
                return;
            }
        ...
        )
        {
            ...
            {
                m_provider.WriteMessageEvent(
                    s_callStackString + eventCache.Callstack,
                    (byte)eventType,
                    (long)eventType & s_keyWordMask
                );
            }
        ...
        )
        {
            ...
            if (args == null)
            {
                ...
                {
                    m_provider.WriteMessageEvent(
                        format + s_callStackString + eventCache.Callstack,
                        (byte)eventType,
                        (long)eventType & s_keyWordMask
                    );
                }
        ...
        public sealed override void TraceTransfer(
            TraceEventCache eventCache,
            String source,
            int id,
            string message,
