...
namespace System.Diagnostics.PerformanceData
{
    ...
    {
        private static readonly bool s_platformNotSupported = (
        ...
        )
        {
            ...
            if (s_platformNotSupported)
            {
                throw new System.PlatformNotSupportedException(
                    SR.GetString(SR.Perflib_PlatformNotSupported)
                );
            }
            if (!PerfProviderCollection.ValidateCounterSetInstanceType(instanceType))
            {
                throw new ArgumentException(
                    SR.GetString(SR.Perflib_Argument_InvalidCounterSetInstanceType, instanceType),
                    "instanceType"
                );
            }
        ...
        public void AddCounter(Int32 counterId, CounterType counterType)
        {
            if (m_provider == null)
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.Perflib_InvalidOperation_NoActiveProvider, m_providerGuid)
                );
            }
            if (!PerfProviderCollection.ValidateCounterType(counterType))
            {
                throw new ArgumentException(
                    SR.GetString(SR.Perflib_Argument_InvalidCounterType, counterType),
                    "counterType"
                );
            }
            if (m_instanceCreated)
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.Perflib_InvalidOperation_AddCounterAfterInstance, m_counterSet)
                );
            }
            ...
            lock (m_lockObject)
            {
                if (m_instanceCreated)
                {
                    throw new InvalidOperationException(
                        SR.GetString(
                            SR.Perflib_InvalidOperation_AddCounterAfterInstance,
                            m_counterSet
                        )
                    );
                }
                if (m_idToCounter.ContainsKey(counterId))
                {
                    throw new ArgumentException(
                        SR.GetString(
                            SR.Perflib_Argument_CounterAlreadyExists,
                            counterId,
                            m_counterSet
                        ),
                        "CounterId"
        ...
        public void AddCounter(Int32 counterId, CounterType counterType, String counterName)
        {
            ...
            {
                throw new ArgumentException(
                    SR.GetString(SR.Perflib_Argument_EmptyCounterName),
                    "counterName"
                );
            }
            if (!PerfProviderCollection.ValidateCounterType(counterType))
            ...
            lock (m_lockObject)
            {
                ...
                if (m_stringToId.ContainsKey(counterName))
                {
                    throw new ArgumentException(
                        SR.GetString(
                            SR.Perflib_Argument_CounterNameAlreadyExists,
                            counterName,
                            m_counterSet
                        ),
                        "CounterName"
        ...
        public CounterSetInstance CreateCounterSetInstance(String instanceName)
        {
            ...
            {
                throw new ArgumentException(
                    SR.GetString(SR.Perflib_Argument_EmptyInstanceName),
                    "instanceName"
                );
            }
            if (m_provider == null)
            ...
            if (!m_instanceCreated)
            {
                lock (m_lockObject)
                {
                    if (!m_instanceCreated)
                    {
                        ...
                        {
                            throw new ArgumentException(
                                SR.GetString(SR.Perflib_Argument_ProviderNotFound, m_providerGuid),
                                "ProviderGuid"
                            );
                        }
                        ...
                        if (m_idToCounter.Count == 0)
                        {
                            throw new InvalidOperationException(
                                SR.GetString(
                                    SR.Perflib_InvalidOperation_CounterSetContainsNoCounter,
                                    m_counterSet
                                )
                            );
                        }
                        ...
                        unsafe
                        {
                            uint CounterSetInfoSize =
                                (uint)sizeof(UnsafeNativeMethods.PerfCounterSetInfoStruct)
                                + (uint)m_idToCounter.Count
                                    * (uint)sizeof(UnsafeNativeMethods.PerfCounterInfoStruct);
                            ...
                            if (CounterSetBuffer == null)
                            {
                                throw new InsufficientMemoryException(
                                    SR.GetString(
                                        SR.Perflib_InsufficientMemory_CounterSetTemplate,
                                        m_counterSet,
                                        CounterSetInfoSize
                                    )
                                );
                            ...
                            CounterSetInfo =
                                (UnsafeNativeMethods.PerfCounterSetInfoStruct*)CounterSetBuffer;
                            ...
                            foreach (KeyValuePair<Int32, CounterType> CounterDef in m_idToCounter)
                            {
                                ...
                                {
                                    CounterInfo = (UnsafeNativeMethods.PerfCounterInfoStruct*)(
                                        CounterSetBuffer + CounterSetInfoUsed
                                    );
                            ...
                            Status = UnsafeNativeMethods.PerfSetCounterSetInfo(
