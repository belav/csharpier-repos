...
namespace System.IO
{
    ...
    internal class LogStream : BufferedStream2
    {
        ...
        internal const LogRetentionOption DefaultRetention =
        ...
        )
        {
            ...
            int flagsAndAttributes =
                (int)FileOptions.None
                | (
                    UnsafeNativeMethods.SECURITY_SQOS_PRESENT
            ...
            _modeSav =
                (retention != LogRetentionOption.SingleFileUnboundedSize)
                    ? FileMode.Create
                    : FileMode.OpenOrCreate;
            _flagsAndAttributesSav = flagsAndAttributes;
            _seekToEndSav =
                (retention != LogRetentionOption.SingleFileUnboundedSize) ? false : true;
        ...
        )
        {
            ...
            int oldMode = UnsafeNativeMethods.SetErrorMode(
                UnsafeNativeMethods.SEM_FAILCRITICALERRORS
            );
            try
            {
                _handle = UnsafeNativeMethods.SafeCreateFile(
                    filePath,
                    fAccess,
                    share,
                    secAttrs,
            ...
            Debug.Assert(
                UnsafeNativeMethods.GetFileType(_handle) == UnsafeNativeMethods.FILE_TYPE_DISK,
        ...
        [System.Security.SecurityCritical]
        protected override unsafe void WriteCore(
            byte[] buffer,
            int offset,
            int count,
            bool blockForWrite,
                ...
                case LogRetentionOption.LimitedCircularFiles:
                    if ((lastPos >= _maxFileSize) && (handle == _handle))
                    {
                        lock (m_lockObject)
                        {
                            ...
                            _currentFileNum++;
                            if (
                                (_retention == LogRetentionOption.LimitedCircularFiles)
                                && (_currentFileNum > _maxNumberOfFiles)
                            )
                            {
                                _currentFileNum = 1;
                            }
                            else if (
                                (_retention == LogRetentionOption.LimitedSequentialFiles)
                                && (_currentFileNum > _maxNumberOfFiles)
                            )
                            {
                                _DisableLogging();
                            ...
                            {
                                _fileNameWithoutExt = Path.Combine(
                                    Path.GetDirectoryName(_pathSav),
                                    Path.GetFileNameWithoutExtension(_pathSav)
                                );
                            ...
                            string path =
                                (_currentFileNum == 1)
                                    ? _pathSav
                                    : _fileNameWithoutExt
                                        + _currentFileNum.ToString(CultureInfo.InvariantCulture)
                                        + _fileExt;
                            try
                            {
                                _Init(
