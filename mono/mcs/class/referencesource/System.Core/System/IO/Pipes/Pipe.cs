...
namespace System.IO.Pipes
{
    ...
    public sealed class AnonymousPipeServerStream : PipeStream
    {
            ...
            : base(direction, bufferSize)
        {
            ...
            UnsafeNativeMethods.SECURITY_ATTRIBUTES secAttrs = PipeStream.GetSecAttrs(
                inheritability
            );
            ...
            : base(direction, 0)
        {
            ...
            if (serverSafePipeHandle.IsInvalid)
            {
                throw new ArgumentException(
                    SR.GetString(SR.Argument_InvalidHandle),
            ...
            if (
                UnsafeNativeMethods.GetFileType(serverSafePipeHandle)
                != UnsafeNativeMethods.FILE_TYPE_PIPE
            )
            {
                throw new IOException(SR.GetString(SR.IO_IO_InvalidPipeHandle));
            }
            if (
                UnsafeNativeMethods.GetFileType(clientSafePipeHandle)
                != UnsafeNativeMethods.FILE_TYPE_PIPE
            )
            {
                throw new IOException(SR.GetString(SR.IO_IO_InvalidPipeHandle));
        ...
        )
        {
            ...
            {
                bSuccess = UnsafeNativeMethods.CreatePipe(
                    out serverHandle,
                    out m_clientHandle,
                    secAttrs,
                    bufferSize
            ...
            bSuccess = UnsafeNativeMethods.DuplicateHandle(
        ...
        public override PipeTransmissionMode ReadMode
        {
            ...
            set
            {
                ...
                if (value < PipeTransmissionMode.Byte || value > PipeTransmissionMode.Message)
                {
                    throw new ArgumentOutOfRangeException(
                        "value",
                        SR.GetString(SR.ArgumentOutOfRange_TransmissionModeByteOrMsg)
                    );
                }
                ...
                {
                    throw new NotSupportedException(
                        SR.GetString(SR.NotSupported_AnonymousPipeMessagesNotSupported)
                    );
                }
            }
        }
    ...
    public sealed class AnonymousPipeClientStream : PipeStream
    {
        ...
        private void Init(PipeDirection direction, SafePipeHandle safePipeHandle)
        {
            ...
            if (
                UnsafeNativeMethods.GetFileType(safePipeHandle)
                != UnsafeNativeMethods.FILE_TYPE_PIPE
            )
            {
                throw new IOException(SR.GetString(SR.IO_IO_InvalidPipeHandle));
    ...
    public sealed class NamedPipeServerStream : PipeStream
    {
            ...
            : base(direction, transmissionMode, outBufferSize)
        {
            ...
            if ((options & ~(PipeOptions.WriteThrough | PipeOptions.Asynchronous)) != 0)
            {
                throw new ArgumentOutOfRangeException(
                    "options",
                    SR.GetString(SR.ArgumentOutOfRange_OptionsInvalid)
                );
            }
            if (inBufferSize < 0)
            {
            ...
            )
            {
                throw new ArgumentOutOfRangeException(
                    "maxNumberOfServerInstances",
                    SR.GetString(SR.ArgumentOutOfRange_MaxNumServerInstances)
                );
            }
            if (
                inheritability < HandleInheritability.None
            ...
            )
            {
                throw new ArgumentOutOfRangeException(
                    "additionalAccessRights",
                    SR.GetString(SR.ArgumentOutOfRange_AdditionalAccessLimited)
                );
            }
            ...
            if (Environment.OSVersion.Platform == PlatformID.Win32Windows)
            {
                throw new PlatformNotSupportedException(
                    SR.GetString(SR.PlatformNotSupported_NamedPipeServers)
                );
            }
            ...
            )
            {
                throw new ArgumentOutOfRangeException(
                    "pipeName",
                    SR.GetString(SR.ArgumentOutOfRange_AnonymousReserved)
                );
            }
        ...
        {
            Debug.Assert(
                fullPipeName != null && fullPipeName.Length != 0,
                "fullPipeName is null or empty"
            );
            Debug.Assert(
            ...
            int openMode =
                ...
                | (
                    maxNumberOfServerInstances == 1
                        ? UnsafeNativeMethods.FILE_FLAG_FIRST_PIPE_INSTANCE
                        : 0
                )
                | (int)options
                | (int)rights;
            ...
            SafePipeHandle handle = UnsafeNativeMethods.CreateNamedPipe(
                fullPipeName,
                openMode,
                pipeModes,
                maxNumberOfServerInstances,
        ...
        public void WaitForConnection()
        {
            ...
            else
            {
                if (!UnsafeNativeMethods.ConnectNamedPipe(InternalHandle, UnsafeNativeMethods.NULL))
                {
                    ...
                    if (
                        errorCode == UnsafeNativeMethods.ERROR_PIPE_CONNECTED
                        && State == PipeState.Connected
                    )
                    {
                        throw new InvalidOperationException(
                            SR.GetString(SR.InvalidOperation_PipeAlreadyConnected)
                        );
                    }
        ...
        private static RuntimeHelpers.TryCode tryCode = new RuntimeHelpers.TryCode(
            ImpersonateAndTryCode
        );
        private static RuntimeHelpers.CleanupCode cleanupCode = new RuntimeHelpers.CleanupCode(
        ...
        public String GetImpersonationUserName()
        {
            ...
            StringBuilder userName = new StringBuilder(
                UnsafeNativeMethods.CREDUI_MAX_USERNAME_LENGTH + 1
            );

            if (
                !UnsafeNativeMethods.GetNamedPipeHandleState(
                    InternalHandle,
        ...
        private void CheckConnectOperationsServer()
        {
            ...
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.InvalidOperation_PipeHandleNotSet)
                );
            }
            // object disposed
        ...
        private void CheckDisconnectOperations()
        {
            ...
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.InvalidOperation_PipeNotYetConnected)
                );
            }
            if (State == PipeState.Disconnected)
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.InvalidOperation_PipeAlreadyDisconnected)
                );
            }
            if (InternalHandle == null)
            {
    ...
    public sealed class NamedPipeClientStream : PipeStream
    {
            ...
            : base(direction, 0)
        {
            ...
            if (serverName == null)
            {
                throw new ArgumentNullException(
                    "serverName",
                    SR.GetString(SR.ArgumentNull_ServerName)
                );
            }
            if (pipeName.Length == 0)
            {
            ...
            )
            {
                throw new ArgumentOutOfRangeException(
                    "impersonationLevel",
                    SR.GetString(SR.ArgumentOutOfRange_ImpersonationInvalid)
                );
            }
            if (
                inheritability < HandleInheritability.None
            ...
            : base(DirectionFromRights(desiredAccessRights), 0)
        {
            ...
            )
            {
                throw new ArgumentOutOfRangeException(
                    "desiredAccessRights",
                    SR.GetString(SR.ArgumentOutOfRange_InvalidPipeAccessRights)
                );
            }
        ...
        public void Connect(int timeout)
        {
            ...
            if (timeout < 0 && timeout != Timeout.Infinite)
            {
                throw new ArgumentOutOfRangeException(
                    "timeout",
                    SR.GetString(SR.ArgumentOutOfRange_InvalidTimeout)
                );
            }
