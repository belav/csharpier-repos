...
namespace System.IO
{
    ...
    internal abstract class BufferedStream2 : Stream
    {
        ...
        private /*volatile*/
        int _pendingBufferCopy; // How many buffer writes are pending.
        private /*volatile*/
        int _writePos; // Write pointer within shared buffer.

        // Should we use a separate buffer for reading Vs writing?
        private /*volatile*/
        int _readPos; // Read pointer within shared buffer.
        private /*volatile*/
        int _readLen; // Number of bytes read in buffer from file.
        ...
        public override long Position
        {
            ...
            set
            {
                if (value < 0)
                    throw new ArgumentOutOfRangeException(
                        "value",
                        Helper.GetResourceString("ArgumentOutOfRange_NeedNonNegNum")
                    );
        ...
        [MethodImplAttribute(MethodImplOptions.Synchronized)]
        public override int Read( /*[In, Out]*/
            byte[] array,
            int offset,
            int count
        )
        {
            if (array == null)
                throw new ArgumentNullException(
                    "array",
                    Helper.GetResourceString("ArgumentNull_Buffer")
                );
            if (offset < 0)
                throw new ArgumentOutOfRangeException(
                    "offset",
        ...
        )
        {
            ...
            if (numBytes <= n)
            {
                ...
                return BufferedStreamAsyncResult.Complete(
                    numBytes,
                    userCallback,
                    stateObject,
                    true
        ...
        public override long Seek(long offset, SeekOrigin origin)
        {
            ...
            if (_readLen > 0)
            {
                ...
                Debug.Assert(
                    _readLen >= 0 && _readPos <= _readLen,
                    "_readLen should be nonnegative, and _readPos should be less than or equal _readLen"
                );
                Debug.Assert(
                    curPos == Position,
                    "Seek optimization: curPos != Position!  Buffer math was mangled."
                );
            }
            return curPos;
        }
        ...
        protected long UnderlyingStreamPosition
        {
            // Making the getter thread safe is not very useful anyways
            get { return pos; }
            set { Interlocked.Exchange(ref pos, value); }
        }
        ...
        [ResourceExposure(ResourceScope.None)]
        [ResourceConsumption(ResourceScope.AppDomain, ResourceScope.AppDomain)]
        protected abstract IAsyncResult BeginReadCore(
            byte[] bytes,
            int offset,
            int numBytes,
            AsyncCallback userCallback,
        ...
        [ResourceExposure(ResourceScope.None)]
        [ResourceConsumption(ResourceScope.AppDomain, ResourceScope.AppDomain)]
        protected abstract IAsyncResult BeginWriteCore(
            byte[] bytes,
            int offset,
            int numBytes,
            AsyncCallback userCallback,
    ...
    unsafe internal sealed class BufferedStreamAsyncResult : IAsyncResult
    {
        ...
        internal static IAsyncResult Complete(
