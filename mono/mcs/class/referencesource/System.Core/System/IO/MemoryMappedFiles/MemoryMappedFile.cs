...
namespace System.IO.MemoryMappedFiles
{
    public class MemoryMappedFile : IDisposable
    {
        ...
        {
            Debug.Assert(
                handle != null && !handle.IsClosed && !handle.IsInvalid,
        ...
        )
        {
            ...
            SafeMemoryMappedFileHandle handle = OpenCore(
                mapName,
                inheritability,
                (int)desiredAccessRights,
                false
        ...
        )
        {
            ...
            if (capacity < 0)
            {
                throw new ArgumentOutOfRangeException(
                    "capacity",
                    SR.GetString(SR.ArgumentOutOfRange_PositiveOrDefaultCapacityRequired)
                );
            }
            ...
            if (mode == FileMode.Append)
            {
                throw new ArgumentException(
                    SR.GetString(SR.Argument_NewMMFAppendModeNotAllowed),
                    "mode"
                );
            }
            ...
            {
                throw new ArgumentException(
                    SR.GetString(SR.Argument_NewMMFWriteAccessNotAllowed),
                    "access"
                );
            }
            ...
            FileStream fileStream = new FileStream(
                path,
                mode,
                GetFileStreamFileSystemRights(access),
                FileShare.None,
                0x1000,
                FileOptions.None
            );
            ...
            if (fileStream.Length > capacity)
            {
                ...
                throw new ArgumentOutOfRangeException(
                    "capacity",
                    SR.GetString(SR.ArgumentOutOfRange_CapacityGEFileSizeRequired)
                );
            }
        ...
        )
        {
            if (fileStream == null)
            {
                throw new ArgumentNullException(
                    "fileStream",
                    SR.GetString(SR.ArgumentNull_FileStream)
                );
            }
            ...
            SafeMemoryMappedFileHandle handle = CreateCore(
                fileStream.SafeFileHandle,
                mapName,
                inheritability,
        ...
        )
        {
            ...
            if (capacity <= 0)
            {
                throw new ArgumentOutOfRangeException(
                    "capacity",
                    SR.GetString(SR.ArgumentOutOfRange_NeedPositiveNumber)
                );
            }
            ...
            SafeMemoryMappedFileHandle handle = CreateCore(
                new SafeFileHandle(new IntPtr(-1), true),
                mapName,
                inheritability,
                memoryMappedFileSecurity,
                access,
        ...
        )
        {
            ...
            try
            {
                handle = UnsafeNativeMethods.CreateFileMapping(
                    fileHandle,
                    secAttrs,
        ...
        {
            SafeMemoryMappedFileHandle handle = UnsafeNativeMethods.OpenFileMapping(
                desiredAccessRights,
            ...
            try
            {
                ...
                if (handle == null || handle.IsInvalid)
                {
                    throw new InvalidOperationException(
                        SR.GetString(SR.InvalidOperation_CantCreateFileMapping)
                    );
                }
            }
            finally
        ...
        )
        {
            ...
            if (size < 0)
            {
                throw new ArgumentOutOfRangeException(
                    "size",
                    SR.GetString(SR.ArgumentOutOfRange_PositiveOrDefaultSizeRequired)
                );
            }
        ...
        internal static Int32 GetFileMapAccess(MemoryMappedFileAccess access)
        {
            ...
            {
                return UnsafeNativeMethods.FILE_MAP_EXECUTE
