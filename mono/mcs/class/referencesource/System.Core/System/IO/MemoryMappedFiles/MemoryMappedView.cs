...
namespace System.IO.MemoryMappedFiles
{
    internal class MemoryMappedView : IDisposable
    {
        ...
        )
        {
            ...
            ulong extraMemNeeded =
                (ulong)offset % (ulong)MemoryMappedFile.GetSystemPageAllocationGranularity();
            ...
            // create the view
            SafeMemoryMappedViewHandle viewHandle = UnsafeNativeMethods.MapViewOfFile(
                memMappedFileHandle,
                MemoryMappedFile.GetFileMapAccess(access),
                offsetHigh,
                offsetLow,
            ...
            UnsafeNativeMethods.MEMORY_BASIC_INFORMATION viewInfo =
                new UnsafeNativeMethods.MEMORY_BASIC_INFORMATION();
            UnsafeNativeMethods.VirtualQuery(
                viewHandle,
                ref viewInfo,
                (IntPtr)Marshal.SizeOf(viewInfo)
            );
            ...
            if (
                ((viewInfo.State & UnsafeNativeMethods.MEM_RESERVE) != 0) || (viewSize < nativeSize)
            )
            {
                ulong allocSize = (nativeSize == 0) ? viewSize : nativeSize;
                IntPtr tempHandle = UnsafeNativeMethods.VirtualAlloc(
                    viewHandle,
                    (UIntPtr)allocSize,
                    UnsafeNativeMethods.MEM_COMMIT,
            ...
            MemoryMappedView mmv = new MemoryMappedView(
                viewHandle,
                (long)extraMemNeeded,
                size,
                access
            ...
            if (m_viewHandle != null)
            {
                unsafe
                {
                    ...
                    try
                    {
                        ...
                        bool canRetry = (
                            !success && error == UnsafeNativeMethods.ERROR_LOCK_VIOLATION
                        );
                        ...
                        for (Int32 w = 0; canRetry && w < MaxFlushWaits; w++)
                        {
                            ...
                            {
                                success = UnsafeNativeMethods.FlushViewOfFile(
                                    firstPagePtr,
                                    capacity
                                );
                                if (success)
