    ...
    internal static class __Error
    {
        ...
        internal static void EndWaitForConnectionCalledTwice()
        {
            ...
            throw new ArgumentException(
                SR.GetString(SR.InvalidOperation_EndWaitForConnectionCalledMultiple)
            );
        }
        ...
        internal static void WinIOError(int errorCode, String maybeFullPath)
        {
            ...
            bool isInvalidPath =
                errorCode == UnsafeNativeMethods.ERROR_INVALID_NAME
                || errorCode == UnsafeNativeMethods.ERROR_BAD_PATHNAME;
            ...
            switch (errorCode)
            {
                    ...
                    else
                    {
                        throw new FileNotFoundException(
                            String.Format(
                                ...
                                SR.GetString(SR.IO_FileNotFound_FileName),
                                str
                            ),
                            str
                        );
                ...
                case UnsafeNativeMethods.ERROR_PATH_NOT_FOUND:
                    if (str.Length == 0)
                    {
                        throw new DirectoryNotFoundException(
                            SR.GetString(SR.IO_PathNotFound_NoPathName)
                        );
                    }
                    else
                    {
                        throw new DirectoryNotFoundException(
                            String.Format(
                                CultureInfo.CurrentCulture,
                                SR.GetString(SR.IO_PathNotFound_Path),
                                str
                            )
                        );
                    }
                ...
                case UnsafeNativeMethods.ERROR_ACCESS_DENIED:
                    ...
                    else
                    {
                        throw new UnauthorizedAccessException(
                            String.Format(
                                CultureInfo.CurrentCulture,
                                SR.GetString(SR.UnauthorizedAccess_IODenied_Path),
                                str
                            )
                        );
                    }
                ...
                case UnsafeNativeMethods.ERROR_ALREADY_EXISTS:
                    ...
                    throw new IOException(
                        SR.GetString(SR.IO_IO_AlreadyExists_Name, str),
                ...
                case UnsafeNativeMethods.ERROR_INVALID_DRIVE:
                    throw new DriveNotFoundException(
                        String.Format(
                            CultureInfo.CurrentCulture,
                            SR.GetString(SR.IO_DriveNotFound_Drive),
                            str
                        )
                    );
                ...
                case UnsafeNativeMethods.ERROR_INVALID_PARAMETER:
                    throw new IOException(
                        UnsafeNativeMethods.GetMessage(errorCode),
                        UnsafeNativeMethods.MakeHRFromErrorCode(errorCode)
                    );
                ...
                case UnsafeNativeMethods.ERROR_SHARING_VIOLATION:
                    ...
                    {
                        throw new IOException(
                            SR.GetString(SR.IO_IO_SharingViolation_NoFileName),
                            UnsafeNativeMethods.MakeHRFromErrorCode(errorCode)
                        );
                    }
                    else
                    {
                        throw new IOException(
                            SR.GetString(SR.IO_IO_SharingViolation_File, str),
                            UnsafeNativeMethods.MakeHRFromErrorCode(errorCode)
                        );
                    }
                ...
                case UnsafeNativeMethods.ERROR_FILE_EXISTS:
                    ...
                    throw new IOException(
                        String.Format(
                            CultureInfo.CurrentCulture,
                            SR.GetString(SR.IO_IO_FileExists_Name),
                            str
                        ),
                        UnsafeNativeMethods.MakeHRFromErrorCode(errorCode)
                    );
