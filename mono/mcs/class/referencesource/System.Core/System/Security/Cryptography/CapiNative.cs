...
using System.Core;
#endif
using System.Diagnostics;using System.Diagnostics.CodeAnalysis;using System.Diagnostics.Contracts;using System.Globalization;using System.Runtime.CompilerServices;using System.Runtime.ConstrainedExecution;using System.Runtime.InteropServices;using System.Runtime.Versioning;using System.Security;using System.Text;using Microsoft.Win32.SafeHandles;
...
namespace System.Security.Cryptography
{
    ...
    internal static class CapiNative
    {
        ...
        {
            // MS_ENH_RSA_AES_PROV
            public const string MicrosoftEnhancedRsaAes =
                "Microsoft Enhanced RSA and AES Cryptographic Provider";
            public const string MicrosoftEnhancedRsaAesPrototype =
        ...
        internal const uint CALG_RSA_KEYX = (
...
#pragma warning restore 618
        ...
        internal static class UnsafeNativeMethods
        {
            ...
            public static extern int _AxlPublicKeyBlobToPublicKeyToken(
                ref CRYPTOAPI_BLOB pCspPublicKeyBlob,
        ...
        internal static SafeCspHandle AcquireCsp(
            string keyContainer,
            string providerName,
        ...
        {
            Contract.Ensures(
                Contract.Result<SafeCspHandle>() != null
            ...
            if (
                !UnsafeNativeMethods.CryptAcquireContext(
                    out cspHandle,
                    keyContainer,
                    providerName,
                    providerType,
            ...
            )
            {
                ...
                if (
                    ...
                    && (
                        error == (int)CapiNative.ErrorCode.ProviderTypeNotDefined
        ...
        internal static byte[] ExportSymmetricKey(SafeCapiKeyHandle key)
        {
            ...
            Contract.Ensures(
                Contract.Result<byte[]>() != null && Contract.Result<byte[]>().Length > 0
            );
            ...
            int keySize = 0;
            if (
                !UnsafeNativeMethods.CryptExportKey(
                    key,
            ...
            Debug.Assert(
                keyBlob.Length >= keyDataOffset + keyLength,
                "Key blob is in an unexpected format."
            );
        ...
        /// </summary>
        [System.Security.SecurityCritical]
        internal static byte[] GetHashParameter(
            SafeCapiHashHandle hashHandle,
            CapiNative.HashParameter parameter
        )
        {
            Contract.Requires(hashHandle != null);
            Contract.Requires(
                CapiNative.HashParameter.AlgorithmId <= parameter
                    && parameter <= CapiNative.HashParameter.HashSize
            );
            ...
            where T : struct
        {
            ...
            if (
                !UnsafeNativeMethods.CryptGetProvParam(
                    provider,
                    parameter,
                    buffer,
                    ref bufferSize,
            ...
            Debug.Assert(
                Marshal.SizeOf(typeof(T)) <= bufferSize,
        ...
        )
        {
            Contract.Requires(provider != null);
            Contract.Requires(
                ((int)algorithm & (int)AlgorithmClass.DataEncryption)
                    == (int)AlgorithmClass.DataEncryption
            );
            Contract.Requires(key != null);
            Contract.Ensures(
            ...
            int blobSize =
                Marshal.SizeOf(typeof(BLOBHEADER)) + Marshal.SizeOf(typeof(int)) + key.Length;
            ...
            Buffer.BlockCopy(
                ...
                0,
                keyBlob,
                Marshal.SizeOf(typeof(BLOBHEADER)) + Marshal.SizeOf(typeof(int)),
                key.Length
            );
            ...
            {
                if (
                    !UnsafeNativeMethods.CryptImportKey(
                        provider,
                        keyBlob,
                        keyBlob.Length,
                        SafeCapiKeyHandle.InvalidHandle,
        ...
        {
            SafeLocalAllocHandle safeLocalAllocHandle = UnsafeNativeMethods.LocalAlloc(
                uFlags,
                sizetdwBytes
            );
        ...
        /// <returns></returns>
        [SecuritySafeCritical]
        internal static unsafe bool DecodeObject(
            IntPtr pszStructType,
            IntPtr pbEncoded,
            uint cbEncoded,
        ...
        )
        {
            ...
            if (
                !UnsafeNativeMethods.CryptDecodeObject(
        ...
        )
        {
            ...
            IntPtr pv = UnsafeNativeMethods.CryptFindOIDInfo(dwKeyType, pvKey, dwGroupId);
        ...
        )
        {
            ...
            if (pvKey.IsInvalid)
            {
                throw new CryptographicException(
                    "SR.GetString(SR.Cryptography_InvalidHandle)",
                    "pvKey"
                );
            }
            CRYPT_OID_INFO pOIDInfo = new CRYPT_OID_INFO(Marshal.SizeOf(typeof(CRYPT_OID_INFO)));
    ...
    internal class X509Utils
    {
        ...
        internal static SafeLocalAllocHandle StringToAnsiPtr(string s)
        {
            ...
            SafeLocalAllocHandle pb = CapiNative.LocalAlloc(
                CapiNative.LMEM_FIXED,
                new IntPtr(arr.Length)
            );
