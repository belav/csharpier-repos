...
namespace System.Security.Cryptography.X509Certificates
{
    ...
    public static class ECDsaCertificateExtensions
    {
        ...
        public static ECDsa GetECDsaPublicKey(this X509Certificate2 certificate)
        {
            ...
            X509Native.CERT_CONTEXT pCertContext = (X509Native.CERT_CONTEXT)
                Marshal.PtrToStructure(certHandle, typeof(X509Native.CERT_CONTEXT));

            IntPtr pSubjectPublicKeyInfo = new IntPtr(
                (long)pCertContext.pCertInfo
                    + (long)Marshal.OffsetOf(typeof(X509Native.CERT_INFO), "SubjectPublicKeyInfo")
            );

            X509Native.CERT_PUBLIC_KEY_INFO certPublicKeyInfo = (X509Native.CERT_PUBLIC_KEY_INFO)
            ...
            using (
                SafeBCryptKeyHandle bcryptKeyHandle = BCryptNative.ImportAsymmetricPublicKey(
                    certPublicKeyInfo,
                    0
                )
            )
        ...
        {
            byte[] keyBlob = BCryptNative.ExportBCryptKey(
                bcryptKeyHandle,
                BCryptNative.BCRYPT_ECCPUBLIC_BLOB
            );
            ...
            else
            {
                ...
                foreach (X509Extension extension in certificate.Extensions)
                {
                    ...
                    if (extension.Oid.Value == "2.5.29.15")
                    {
                        ...
                        //it is ECDSA.
                        if (
                            ext.KeyUsages.HasFlag(X509KeyUsageFlags.DigitalSignature)
