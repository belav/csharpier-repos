    ...
    public sealed class AesCryptoServiceProvider : Aes
    {
        ...
        public override ICryptoTransform CreateDecryptor(byte[] key, byte[] iv)
        {
            ...
            using (
                SafeCapiKeyHandle importedKey = CapiNative.ImportSymmetricKey(
                    m_cspHandle,
                    GetAlgorithmId(keyCopy.Length * 8),
                    keyCopy
                )
            )
            {
        ...
        private static KeySizes[] FindSupportedKeySizes(SafeCspHandle csp, out int defaultKeySize)
        {
            ...
            // reasonable value.
            Contract.Ensures(
                Contract.Result<KeySizes[]>().Length == 0
                    || (
                        Contract.ValueAtReturn<int>(out defaultKeySize) > 0
                        && Contract.ValueAtReturn<int>(out defaultKeySize) % 8 == 0
                    )
            );
            ...
            if (s_supportedKeySizes == null)
            {
                ...
                CapiNative.PROV_ENUMALGS algorithm =
                    CapiNative.GetProviderParameterStruct<CapiNative.PROV_ENUMALGS>(
                        csp,
                ...
                while (algorithm.aiAlgId != CapiNative.AlgorithmId.None)
                {
                    ...
                    algorithm = CapiNative.GetProviderParameterStruct<CapiNative.PROV_ENUMALGS>(
                        csp,
                        CapiNative.ProviderParameter.EnumerateAlgorithms,
        ...
        private static CapiNative.AlgorithmId GetAlgorithmId(int keySize)
        {
            ...
            Contract.Ensures(
                (
                    (
                        ((int)Contract.Result<CapiNative.AlgorithmId>())
                        & (int)CapiNative.AlgorithmClass.DataEncryption
                    ) == (int)CapiNative.AlgorithmClass.DataEncryption
                )
