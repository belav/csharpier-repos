...
namespace System.Security.Cryptography
{
    ...
    internal sealed class CapiHashAlgorithm : IDisposable
    {
        ...
        )
        {
            ...
            Contract.Requires(
                (CapiNative.AlgorithmClass)((uint)algorithm & (uint)CapiNative.AlgorithmClass.Hash)
                    == CapiNative.AlgorithmClass.Hash
            );
            Contract.Ensures(
                m_cspHandle != null && !m_cspHandle.IsInvalid && !m_cspHandle.IsClosed
            );
            Contract.Ensures(
            ...
            m_cspHandle = CapiNative.AcquireCsp(
                null,
                provider,
                providerType,
