...
namespace System.Data
{
    ...
    internal sealed class XmlTreeGen
    {
            ...
            foreach (Constraint constr in table.Constraints)
            {
                if (fromTable)
                {
                    ...
                    { // if parent table does not exist , no need to create FKConst
                        if (
                            !_tables.Contains(
                                (DataTable)(((ForeignKeyConstraint)constr).RelatedTable)
                            )
                        )
                        {
                            continue;
        ...
        )
        {
            ...
            if (
                top.Length == 0
                || schFormat == SchemaFormat.WebServiceSkipSchema
            ...
            for (int i = 0; i < top.Length; i++)
            {
                ...
                if (
                    ...
                    || Common.ADP.IsEmpty(top[i].Namespace)
                ...
                )
                {
                    ...
                    if (!fNestedInDataset)
                    {
                        ...
                        if (
                            ...
                            node.SetAttribute(
                                Keywords.REF,
                                ((string)prefixes[top[i].Namespace]) + ':' + top[i].EncodedTableName
                            );
            ...
            if (schFormat != SchemaFormat.Remoting && schFormat != SchemaFormat.RemotingSkipSchema)
            {
                ...
                foreach (string ns in namespaces.Keys)
                {
                    ...
                    if (
                        schFormat != SchemaFormat.WebService
                ...
                foreach (string ns in namespaces.Keys)
                {
                    ...
                    else
                    {
                        xw = new XmlTextWriter(
                            filePath + fileName + "_" + prefixes[ns] + ".xsd",
                            null
                        );
                    }
        ...
        internal XmlElement FillDataSetElement(XmlDocument xd, DataSet ds, DataTable dt)
        {
            ...
            { // No DataSet
                if (dt != null)
                {
                    ...
                    if (dt.CaseSensitive)
                    { // WebData 111631 :it is a bug to go and write casesensitive attrib as 'true', by default
                        dsElement.SetAttribute(
                            Keywords.MSD_CASESENSITIVE,
                            Keywords.MSDNS,
                            Keywords.TRUE
        ...
        internal XmlElement GetSchema(string NamespaceURI)
        {
            ...
            if (schemaEl == null)
            {
                ...
                if (!Common.ADP.IsEmpty(NamespaceURI))
                {
                    string prefix =
                        Keywords.APP
                        + Convert.ToString(++prefixCount, CultureInfo.InvariantCulture);
        ...
        )
        {
            ...
            else
            {
                ...
                )
                {
                    ...
                    root.SetAttribute(
                        Keywords.REF,
                        prefixes[col.Namespace] + ":" + col.EncodedColumnName
                    );
                ...
                if (schFormat != SchemaFormat.Remoting)
                    root.SetAttribute(
                        ...
                        NewDiffgramGen.QualifiedName(
                            (string)prefixes[table.TypeName.Namespace],
                            table.TypeName.Name
                        )
                    );
                ...
                for (int j = 0; j < childRelations.Count; j++)
                {
                    ...
                    else
                    {
                        ...
                        NestedTable.SetAttribute(
                            Keywords.REF,
                            ((string)prefixes[childTable.Namespace])
                                + ':'
            ...
            if (schFormat != SchemaFormat.Remoting)
            {
                ...
                xpathprefix =
                    table.Namespace.Length != 0
                        ? (string)prefixes[table.Namespace] + ':'
                        : String.Empty;
            }
    ...
    internal sealed class NewDiffgramGen
    {
        ...
        internal void Save(XmlWriter xmlw, DataTable table)
        {
            ...
            if (!EmptyData())
            {
                // write the datapart
                if (table != null)
                    new XmlDataTreeWriter(table, this._writeHierarchy).SaveDiffgramData(
                        _xmlw,
                        rowsOrder
                    );
                else
        ...
        private void GenerateRow(DataRow row)
        {
            ...
            object val = (
                table.XmlText == null ? DBNull.Value : row[table.XmlText, DataRowVersion.Original]
            );
        ...
        private void GenerateColumn(DataRow row, DataColumn col, DataRowVersion version)
        {
            ...
            switch (col.ColumnMapping)
            {
                ...
                case MappingType.Element:
                    ...
                    if (
                        !col.IsCustomType
                        || !col.IsValueCustomTypeInstance(columnValue)
                    ...
                    { // Columns type is CDT
                        ...
                        {
                            if (
                                col.IsValueCustomTypeInstance(columnValue) /* && valuesType != typeof(Type)*/
                            )
                            { // value is also CDT
                ...
                if (col.columnMapping != MappingType.Hidden)
                {
                    ...
                    )
                    {
                        if (col.columnMapping != MappingType.SimpleContent)
                        {
                            ...
                            if (
                                !col.IsCustomType
                                || !col.IsValueCustomTypeInstance(value)
                        ...
                        { // Columns type is CDT
                            if (
                                col.IsValueCustomTypeInstance(value) /*&& !(value is Type) && valuesType != typeof(Type)*/
                            )
                            { // value is also CDT
