...
namespace System.Data
{
    ...
    internal sealed class RecordManager
    {
        ...
        {
            Debug.Assert(
                (record < lastFreeRecord) && (-1 == freeRecordList.IndexOf(record)),
                "accesing free record"
            );
            Debug.Assert(
                (null == rows[record])
                    || (record == rows[record].oldRecord)
                    || (record == rows[record].newRecord)
                    || (record == rows[record].tempRecord),
        ...
        internal void VerifyRecord(int record, DataRow row)
        {
            ...
            Debug.Assert(
                (null == rows[record]) || (row == rows[record]),
                "record of a different row"
            );
        }
    }
