...
namespace System.Data
{
    ...
    internal sealed class XmlDataLoader
    {
            ...
            */
            ...
            if (isTableLevel || (dataSet != null && dataSet.fTopLevelTable))
            {
                ...
                DataTable topTable = (DataTable)
                    nodeToSchemaMap.GetSchemaForNode(e, FIgnoreNamespace(e));
                if (topTable != null)
                {
        ...
        internal void LoadData(XmlReader reader)
        {
            ...
            bool fEnforce = isTableLevel
                ? dataTable.EnforceConstraints
            ...
            { // Create XML to dataset map
                nodeToSchemaMap = isTableLevel
                    ? new XmlToDatasetMap(dataReader.NameTable, dataTable)
            ...
            if (topMostNode != null)
            { // Do we have top node?
                if (!isDiffgram && !isTableLevel)
                { // Not a diffgram  and not DataSet?
                    DataTable table =
                        nodeToSchemaMap.GetSchemaForNode(topMostNode, FIgnoreNamespace(topMostNode))
                        as DataTable;
        ...
        private void LoadTopMostTable(DataTable table)
        {
            ...
            while (entryDepth < dataReader.Depth)
            {
                ...
                { // Process nodes based on type
                    case XmlNodeType.Element: // It's an element
                        object o = nodeToSchemaMap.GetColumnSchema(
                            table,
                            dataReader,
                            FIgnoreNamespace(dataReader)
                        );
            ...
            while (entryChild < childRowsStack.Count)
        ...
        private void LoadTable(DataTable table, bool isNested)
        {
            ...
            for (int i = dataReader.AttributeCount - 1; i >= 0; --i)
            {
                ...
                c =
                    nodeToSchemaMap.GetColumnSchema(table, dataReader, FIgnoreNamespace(dataReader))
                    as DataColumn;
                ...
                { // Now handle some diffgram attributes
                    ...
                    else if (dataReader.NamespaceURI == Keywords.MSDNS)
                    {
                        if (dataReader.LocalName == Keywords.ROWORDER)
                        {
                            ...
                            dataReader.LocalName.StartsWith("hidden", StringComparison.Ordinal)
                        )
                        {
                            // Hidden column ?
                            c = collection[
                                XmlConvert.DecodeName(dataReader.LocalName.Substring(6))
                            ];
                                    ...
                                    else
                                    {
                                        ...
                                        { // Not a nested column nor nested table.
                                            ...
                                            DataTable misplacedTable =
                                                nodeToSchemaMap.GetTableForNode(
                                                    dataReader,
                                                    FIgnoreNamespace(dataReader)
                                                );
                                            // Try to get table for node
        ...
        private bool ProcessXsdSchema()
        {
            if (
                (
                    (object)dataReader.LocalName == XSD_SCHEMA
                    && (object)dataReader.NamespaceURI == XSDNS
                )
            )
            {
                // Found XSD schema
                ...
                (
                    (object)dataReader.LocalName == XDR_SCHEMA
                    && (object)dataReader.NamespaceURI == XDRNS
                )
                || (
                    (object)dataReader.LocalName == SQL_SYNC
                    && (object)dataReader.NamespaceURI == UPDGNS
                )
            )
            {
