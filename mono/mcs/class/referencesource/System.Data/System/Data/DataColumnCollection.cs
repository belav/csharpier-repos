...
namespace System.Data
{
    ...
    [
        ...
        Editor(
            "Microsoft.VSDesigner.Data.Design.ColumnsCollectionEditor, "
    ...
    public sealed class DataColumnCollection : InternalDataCollectionBase
    {
        ...
        {
            get { return columnsImplementingIChangeTracking; }
        }
        internal int ColumnsImplementingIChangeTrackingCount
        {
            get { return nColumnsImplementingIChangeTracking; }
        }
        ...
        internal DataColumn this[string name, string ns]
        {
            get
            {
                DataColumn column;
                if (
                    (columnFromName.TryGetValue(name, out column))
                    && (column != null)
                    && (column.Namespace == ns)
                )
                {
        ...
        {
            add { onCollectionChangedDelegate += value; }
            remove { onCollectionChangedDelegate -= value; }
        }

        internal event CollectionChangeEventHandler CollectionChanging
        {
            add { onCollectionChangingDelegate += value; }
            remove { onCollectionChangingDelegate -= value; }
        }

        internal event CollectionChangeEventHandler ColumnPropertyChanged
        {
            add { onColumnPropertyChangedDelegate += value; }
            remove { onColumnPropertyChangedDelegate -= value; }
        }
                ...
                if (table.ParentRelations[i].ChildKey.ContainsColumn(column))
                {
                    if (!fThrowException)
                        ...
                        throw ExceptionBuilder.CannotRemoveChildKey(
                            table.ParentRelations[i].RelationName
                        );
                }
            }
            for (int i = 0; i < table.ChildRelations.Count; i++)
            {
                if (table.ChildRelations[i].ParentKey.ContainsColumn(column))
                {
                    if (!fThrowException)
                        ...
                        throw ExceptionBuilder.CannotRemoveChildKey(
                            table.ChildRelations[i].RelationName
                        );
                }
            }
                ...
                if (table.Constraints[i].ContainsColumn(column))
                    if (!fThrowException)
                        ...
                        throw ExceptionBuilder.CannotRemoveConstraint(
                            table.Constraints[i].ConstraintName,
                            table.Constraints[i].Table.TableName
                        );
            }
            if (table.DataSet != null)
            {
                for (
                    ParentForeignKeyConstraintEnumerator en =
                        new ParentForeignKeyConstraintEnumerator(table.DataSet, table);
                    en.GetNext();
        ...
        internal void RegisterColumnName(string name, DataColumn column)
        {
            ...
            if (
                (column == null)
                && NamesEqual(name, MakeName(defaultNameIndex), true, table.Locale) != 0
            )
            {
                do
                {
