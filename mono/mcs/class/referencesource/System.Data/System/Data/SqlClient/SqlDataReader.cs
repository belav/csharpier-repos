...
namespace System.Data.SqlClient
{
    ...
    public class SqlDataReader : DbDataReader, IDataReader
    {
        ...
        {
            set { _browseModeInfoConsumed = value; }
        }
                ...
                for (int index = 0; index < metaData.Length; index++)
                {
                    ...
                    if (!colMetaData.isHidden)
                    {
                        ...
                        if (length > TdsEnums.MAXSIZE)
                        {
                            ...
                            SqlDbType.NChar == colMetaData.type
        ...
        internal DataTable BuildSchemaTable()
        {
            ...
            for (int i = 0; i < md.Length; i++)
            {
                ...
                {
                    Debug.Assert(
                        col.baseTI != null && col.baseTI.metaType != null,
                        "col.baseTI and col.baseTI.metaType should not be null."
                    );
                    schemaRow[Size] =
                        (
                            col.baseTI.metaType.IsSizeInCharacters
                            && (col.baseTI.length != 0x7fffffff)
                        )
                            ? (col.baseTI.length / 2)
                            : col.baseTI.length;
                }
                ...
                {
                    schemaRow[Size] =
                        (col.metaType.IsSizeInCharacters && (col.length != 0x7fffffff))
                            ? (col.length / 2)
                            : col.length;
                }
                ...
                schemaRow[NonVersionedProviderType] = (int)(
                    col.cipherMD != null ? col.baseTI.type : col.type
                ...
                )
                {
                    ...
                    switch (col.type)
                    {
                        ...
                        case SqlDbType.Time:
                            Debug.Assert(
                                TdsEnums.UNKNOWN_PRECISION_SCALE == col.scale
                                    || (0 <= col.scale && col.scale <= 7),
                                "Invalid scale for Time column: " + col.scale
                            );
                            schemaRow[Size] = TdsEnums.WHIDBEY_TIME_LENGTH[
                                TdsEnums.UNKNOWN_PRECISION_SCALE != col.scale
                                    ? col.scale
                                    : col.metaType.Scale
                            ];
                        ...
                        case SqlDbType.DateTime2:
                            Debug.Assert(
                                TdsEnums.UNKNOWN_PRECISION_SCALE == col.scale
                                    || (0 <= col.scale && col.scale <= 7),
                                "Invalid scale for DateTime2 column: " + col.scale
                            );
                            schemaRow[Size] = TdsEnums.WHIDBEY_DATETIME2_LENGTH[
                                TdsEnums.UNKNOWN_PRECISION_SCALE != col.scale
                                    ? col.scale
                                    : col.metaType.Scale
                            ];
                        ...
                        case SqlDbType.DateTimeOffset:
                            Debug.Assert(
                                TdsEnums.UNKNOWN_PRECISION_SCALE == col.scale
                                    || (0 <= col.scale && col.scale <= 7),
                                "Invalid scale for DateTimeOffset column: " + col.scale
                            );
                            schemaRow[Size] = TdsEnums.WHIDBEY_DATETIMEOFFSET_LENGTH[
                                TdsEnums.UNKNOWN_PRECISION_SCALE != col.scale
                                    ? col.scale
                                    : col.metaType.Scale
                            ];
                ...
                else if (_typeSystem != SqlConnectionString.TypeSystem.SQLServer2000)
                {
                    ...
                    { // Additional metadata for Xml.
                        ...
                        schemaRow[XmlSchemaCollectionOwningSchema] =
                            col.xmlSchemaCollectionOwningSchema;
                ...
                if (col.cipherMD != null)
                {
                    Debug.Assert(col.baseTI != null, @"col.baseTI should not be null.");
                    Debug.Assert(
                        col.baseTI.metaType != null,
                        @"col.baseTI.metaType should not be null."
                    );
        ...
        private bool TryCleanPartialRead()
        {
            ...
            if (_stateObj._pendingData)
            {
                ...
                Debug.Assert(
                    TdsParser.IsValidTdsToken(token),
                    string.Format(
                        "Invalid token after performing CleanPartialRead: {0,-2:X2}",
                        token
                    )
                );
            }
                        ...
                        if (parser.State == TdsParserState.OpenLoggedIn)
                        {
                            ...
                            else
                            {
                                ...
                                Debug.Assert(
                                    TdsParser.IsValidTdsToken(token),
                                    string.Format(
                                        "DataReady is false, but next token is invalid: {0,-2:X2}",
                                        token
                                    )
                                );
                            }
        ...
        internal virtual void CloseReaderFromConnection()
        {
            var parser = _parser;
            Debug.Assert(
                parser == null || parser.State != TdsParserState.OpenNotLoggedIn,
                "Reader on a connection that is not logged in"
            );
                ...
                if (stateObj != null)
                {
                    ...
                    if (_snapshot != null)
                    {
                        ...
                        CleanupAfterAsyncInvocationInternal(
                            stateObj,
                            resetNetworkPacketTaskSource: false
                        );
                    }
        ...
        private bool TryConsumeMetaData()
        {
            ...
            while (
                _parser != null && _stateObj != null && _stateObj._pendingData && !_metaDataConsumed
            )
            {
                if (
        ...
        private string GetDataTypeNameInternal(_SqlMetaData metaData)
        {
            ...
            if (
                _typeSystem <= SqlConnectionString.TypeSystem.SQLServer2005
                && metaData.IsNewKatmaiDateTimeType
            )
            {
            ...
            else if (_typeSystem != SqlConnectionString.TypeSystem.SQLServer2000)
            {
                ...
                { // For all other types, including Xml - use data in MetaType.
                    ...
                    {
                        Debug.Assert(
                            metaData.baseTI != null && metaData.baseTI.metaType != null,
        ...
        public override Stream GetStream(int i)
        {
            ...
            MetaType mt = _metaData[i].metaType;
            if (
                ((!mt.IsBinType) || (mt.SqlDbType == SqlDbType.Timestamp))
                && (mt.SqlDbType != SqlDbType.Variant)
            )
            {
        ...
        internal virtual long GetBytesInternal(
            int i,
            long dataIndex,
            byte[] buffer,
            int bufferIndex,
        ...
        private bool TryGetBytesInternal(
            int i,
            long dataIndex,
            byte[] buffer,
            int bufferIndex,
        ...
        )
        {
            ...
            try
            {
...
#else
                {
                    ...
                    {
                        Debug.Assert(
                            !HasActiveStreamOrTextReaderOnColumn(i),
                            "Column has an active Stream or TextReader"
                        );
                        ...
                        // If there are an unknown (-1) number of bytes left for a PLP, read its size
                        if (
                            (-1 == _sharedState._columnDataBytesRemaining)
                            && (_metaData[i].metaType.IsPlp)
                        )
                        {
                            ulong left;
                        ...
                        int bytesRead;
                        bool result = TryGetBytesInternalSequential(
                            i,
                            buffer,
                            bufferIndex,
                            length,
        ...
        internal int GetBytesInternalSequential(
            int i,
            byte[] buffer,
            int index,
            int length,
        ...
        internal bool TryGetBytesInternalSequential(
            int i,
            byte[] buffer,
            int index,
            int length,
            ...
            try
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            // Read in data
                            bool result = _stateObj.TryReadPlpBytes(
                                ref buffer,
                                index,
                                length,
                                out bytesRead
                        ...
                        else
                        {
                            ...
                            int bytesToRead = (int)
                                Math.Min((long)length, _sharedState._columnDataBytesRemaining);
                            bool result = _stateObj.TryReadByteArray(
                                buffer,
                                index,
                                bytesToRead,
                                out bytesRead
        ...
        public override TextReader GetTextReader(int i)
        {
            ...
            {
                Debug.Assert(
                    _metaData[i].baseTI != null,
                    "_metaData[i].baseTI should not be null."
                );
            ...
            if (
                ((!mt.IsCharType) && (mt.SqlDbType != SqlDbType.Variant))
        ...
        )
        {
            ...
            try
            {
                ...
                if (
                    (_sharedState._nextColumnDataToRead == (i + 1))
                    && (_sharedState._nextColumnHeaderToRead == (i + 1))
        ...
        private long GetCharsFromPlpData(
            int i,
            long dataIndex,
            char[] buffer,
            int bufferIndex,
        ...
        )
        {
            ...
            try
            {
...
#else
                {
                    ...
                    Debug.Assert(
                        !HasActiveStreamOrTextReaderOnColumn(i),
                        "Column has active Stream or TextReader"
                    );
                    // don't allow get bytes on non-long or non-binary columns
                    Debug.Assert(
                        _metaData[i].metaType.IsPlp,
                    ...
                    Debug.Assert(
                        IsCommandBehavior(CommandBehavior.SequentialAccess),
                        "GetCharsFromPlpData called for non-Sequential access"
                    );
                    ...
                    if (-1 == _sharedState._columnDataBytesRemaining)
                    {
                        _sharedState._columnDataBytesRemaining = (long)
                            _parser.PlpBytesLeft(_stateObj);
                    }
                    ...
                    {
                        cch = (long)
                            _parser.ReadPlpUnicodeChars(ref buffer, bufferIndex, length, _stateObj);
                        _columnDataBytesRead += (cch << 1);
                    }
                    else
                    {
                        cch = (long)
                            _parser.ReadPlpAnsiChars(
                                ref buffer,
                                bufferIndex,
                                length,
                                _metaData[i],
        ...
        public override DateTime GetDateTime(int i)
        {
            ...
            if (
                _typeSystem <= SqlConnectionString.TypeSystem.SQLServer2005
                && _metaData[i].IsNewKatmaiDateTimeType
            )
            {
        ...
        private object GetSqlValueFromSqlBufferInternal(SqlBuffer data, _SqlMetaData metaData)
        {
            ...
            Debug.Assert(
                !data.IsEmpty || data.IsNull || metaData.type == SqlDbType.Timestamp,
                "Data has been read, but the buffer is empty"
            );
        ...
        public virtual int GetSqlValues(object[] values)
        {
            ...
            try
            {
                ...
                int copyLen =
                    (values.Length < _metaData.visibleColumns)
                        ? values.Length
                        : _metaData.visibleColumns;
        ...
        public override int GetValues(object[] values)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < copyLen; i++)
                {
                    ...
                    values[_metaData.indexMap[i]] = GetValueFromSqlBufferInternal(
                        _data[i],
                        _metaData[i]
                    );
        ...
        {
            Debug.Assert(
                _typeSystem == SqlConnectionString.TypeSystem.SQLServer2000,
                "Should not be in this function under anything else but SQLServer2000"
            );
                            ...
                            switch (_altRowStatus)
                            {
                                case ALTROWSTATUS.AltRow:
                                    ...
                                    _SqlMetaDataSet altMetaDataSet =
                                        _altMetaDataSetCollection.GetAltMetaData(altRowId);
        ...
        private bool TryReadColumn(int i, bool setTimeout, bool allowPartiallyReadColumn = false)
        {
            CheckDataIsReady(
                columnIndex: i,
                permitAsync: true,
                allowPartiallyReadColumn: allowPartiallyReadColumn
            );
...
#endif //DEBUG
                    Debug.Assert(
                        _sharedState._nextColumnHeaderToRead <= _metaData.Length,
                        "_sharedState._nextColumnHeaderToRead too large"
                    );
                    Debug.Assert(
                        _sharedState._nextColumnDataToRead <= _metaData.Length,
                        "_sharedState._nextColumnDataToRead too large"
                    );
            ...
            if (!_data[_sharedState._nextColumnDataToRead].IsNull)
            {
                ...
                if (
                    !_parser.TryReadSqlValue(
                        ...
                        columnMetaData,
                        (int)_sharedState._columnDataBytesRemaining,
                        _stateObj,
                        _command != null
                            ? _command.ColumnEncryptionSetting
        ...
        private bool TryReadColumnInternal(int i, bool readHeaderOnly = false)
        {
            ...
            if (i < _sharedState._nextColumnHeaderToRead)
            {
                ...
                else
                {
                    ...
                    Debug.Assert(
                        i == _sharedState._nextColumnDataToRead
                            || // Either we haven't read the column yet
                            (
                                (i + 1 < _sharedState._nextColumnDataToRead)
                                && (IsCommandBehavior(CommandBehavior.SequentialAccess))
                            )
                            || // Or we're in sequential mode and we've read way past the column (i.e. it was not the last column we read)
                            (!_data[i].IsEmpty || _data[i].IsNull)
                            || // Or we should have data stored for the column (unless the column was null)
        ...
        private bool WillHaveEnoughData(int targetColumn, bool headerOnly = false)
        {
            ...
            if (
                (_lastColumnWithDataChunkRead == _sharedState._nextColumnDataToRead)
            ...
            int bytesRemaining = Math.Min(
                checked(_stateObj._inBytesRead - _stateObj._inBytesUsed),
            ...
            if (
                (targetColumn >= _sharedState._nextColumnDataToRead)
                && (_sharedState._nextColumnDataToRead < _sharedState._nextColumnHeaderToRead)
            )
            {
                ...
                else
                {
                    ...
                    bytesRemaining = checked(
                        bytesRemaining - (int)_sharedState._columnDataBytesRemaining
                    );
                }
            }
            ...
            while ((bytesRemaining >= 0) && (currentColumn <= targetColumn))
            {
                ...
                if (!_stateObj.IsNullCompressionBitSet(currentColumn))
                {
                    ...
                    if (
                        (metaType.IsLong)
                        || (metaType.IsPlp)
                        || (metaType.SqlDbType == SqlDbType.Udt)
                        || (metaType.SqlDbType == SqlDbType.Structured)
                    )
                    {
                    ...
                    byte typeAndMask = (byte)(
                        _metaData[currentColumn].tdsType & TdsEnums.SQLLenMask
                    );
        ...
        private bool TryResetBlobState()
        {
            ...
            if (_sharedState._nextColumnDataToRead < _sharedState._nextColumnHeaderToRead)
            {
                if (
                    (_sharedState._nextColumnHeaderToRead > 0)
                    && (_metaData[_sharedState._nextColumnHeaderToRead - 1].metaType.IsPlp)
                )
                {
            ...
            if (null != parser && null != _resetOptionsString)
            {
                ...
                if (parser.State == TdsParserState.OpenLoggedIn)
                {
                    Bid.CorrelationTrace(
                        "<sc.SqlDataReader.RestoreServerSettings|Info|Correlation> ObjectID%d#, ActivityID %ls\n",
                        ObjectID
                    );
                    Task executeTask = parser.TdsExecuteSQLBatch(
                        _resetOptionsString,
                        (_command != null) ? _command.CommandTimeout : 0,
                        null,
                        stateObj,
                        sync: true
                    );
        ...
        internal bool TrySetAltMetaDataSet(_SqlMetaDataSet metaDataSet, bool metaDataConsumed)
        {
            if (_altMetaDataSetCollection == null)
            {
                ...
                && object.ReferenceEquals(
                    _snapshot._altMetaDataSetCollection,
                    _altMetaDataSetCollection
                )
            )
            {
                _altMetaDataSetCollection = (_SqlMetaDataSetCollection)
                    _altMetaDataSetCollection.Clone();
            }
        ...
        private void CheckDataIsReady()
        {
            ...
            Debug.Assert(
                !_sharedState._dataReady || _metaData != null,
                "Data is ready, but there is no metadata?"
            );
        ...
        private void CheckHeaderIsReady(
            int columnIndex,
            bool permitAsync = false,
        ...
        )
        {
            ...
            if (
                ...
                (
                    ...
                    || (_lastColumnWithDataChunkRead > columnIndex)
                )
            )
            { // Read past column
                throw ADP.NonSequentialColumnAccess(
                    columnIndex,
                    Math.Max(_sharedState._nextColumnHeaderToRead - 1, _lastColumnWithDataChunkRead)
                );
            }
        }
        ...
        )
        {
            ...
            if (
                ...
                (
                    ...
                    (
                        ...
                        && (HasActiveStreamOrTextReaderOnColumn(columnIndex))
                    )
                )
            )
            { // Has a Stream or TextReader on a partially-read column
                throw ADP.NonSequentialColumnAccess(
                    columnIndex,
                    Math.Max(_sharedState._nextColumnDataToRead, _lastColumnWithDataChunkRead + 1)
                );
            }
        }

        [Conditional("DEBUG")]
        private void AssertReaderState(
            bool requireData,
            bool permitAsync,
            int? columnIndex = null,
            bool enforceSequentialAccess = false
        )
        {
            Debug.Assert(
                !_sharedState._dataReady || _metaData != null,
            ...
            Debug.Assert(
                permitAsync || _currentTask == null,
                "Call while async operation is pending"
            );
            Debug.Assert(
                _metaData != null,
                "_metaData is null, check MetaData before calling this method"
            );
            Debug.Assert(!requireData || _sharedState._dataReady, "No data is ready to be read");
            if (columnIndex.HasValue)
            {
                Debug.Assert(
                    columnIndex.Value >= 0 && columnIndex.Value < _metaData.Length,
        ...
        public override Task<bool> NextResultAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    source.SetException(
                        ADP.ExceptionWithStackTrace(ADP.DataReaderClosed("NextResultAsync"))
                    );
                    return source.Task;
                }
                ...
                moreFunc = (t) =>
                {
                    ...
                    {
                        Bid.Trace(
                            "<sc.SqlDataReader.NextResultAsync> attempt retry %d#\n",
                            ObjectID
                        );
                        PrepareForAsyncContinuation();
                    }
        ...
        internal Task<int> GetBytesAsync(
            int i,
            byte[] buffer,
            int index,
            int length,
        ...
        )
        {
            ...
            if (IsClosed)
            {
                ...
                source.SetException(
                    ADP.ExceptionWithStackTrace(ADP.DataReaderClosed("GetBytesAsync"))
                );
                return source.Task;
            }
            ...
            if (
                (_sharedState._nextColumnHeaderToRead <= _lastColumnWithDataChunkRead)
                || (_sharedState._nextColumnDataToRead < _lastColumnWithDataChunkRead)
            )
            {
                TaskCompletionSource<int> source = new TaskCompletionSource<int>();
                ...
                moreFunc = (t) =>
                {
                    ...
                    if (TryReadColumnHeader(i))
                    {
                        ...
                        {
                            // Timeout
                            return ADP.CreatedTaskWithException<int>(
                                ADP.ExceptionWithStackTrace(ADP.IO(SQLMessage.Timeout()))
                            );
                        }
                        else
                        {
                            ...
                            int totalBytesRead;
                            var readTask = GetBytesAsyncReadDataStage(
                                i,
                                buffer,
                                index,
                                length,
            ...
            else
            {
                ...
                try
                {
                    return GetBytesAsyncReadDataStage(
                        i,
                        buffer,
                        index,
                        length,
        ...
        private Task<int> GetBytesAsyncReadDataStage(
            int i,
            byte[] buffer,
            int index,
            int length,
        ...
        )
        {
            ...
            if (!TryGetBytesInternalSequential(i, buffer, index, length, out bytesRead))
            {
                ...
                if (!isContinuation)
                {
                    ...
                    Task original = Interlocked.CompareExchange(
                        ref _currentTask,
                        source.Task,
                        null
                    );
                ...
                else
                {
                    ...
                    retryTask.ContinueWith(
                        (t) => CompleteRetryable(t, source, timeoutCancellationSource),
            ...
            try
            {
                if (IsClosed)
                {
                    return ADP.CreatedTaskWithException<bool>(
                        ADP.ExceptionWithStackTrace(ADP.DataReaderClosed("ReadAsync"))
                    );
                }
                ...
                {
                    return ADP.CreatedTaskWithException<bool>(
                        ADP.ExceptionWithStackTrace(SQL.PendingBeginXXXExists())
                    );
                }
                ...
                try
                {
                    ...
                    if (
                        (!_haltRead)
                        && (
                            (!_sharedState._dataReady) || (WillHaveEnoughData(_metaData.Length - 1))
                        )
                    )
                    {
#if DEBUG
        ...
        public override Task<bool> IsDBNullAsync(int i, CancellationToken cancellationToken)
        {
            ...
            if (
                (_sharedState._nextColumnHeaderToRead > i)
            ...
            )
            {
                ...
                else
                {
                    ...
                    return ADP.CreatedTaskWithException<bool>(
                        ADP.ExceptionWithStackTrace(ADP.DataReaderClosed("IsDBNullAsync"))
                    );
                }
            }
            else
        ...
        public override Task<T> GetFieldValueAsync<T>(int i, CancellationToken cancellationToken)
        {
                ...
                )
                {
                    ...
                    if ((data != null) && (metaData != null))
                    {
                        return Task.FromResult<T>(
                            GetFieldValueFromSqlBufferInternal<T>(data[i], metaData[i])
                        );
                    }
                    else
                    {
                        ...
                        return ADP.CreatedTaskWithException<T>(
                            ADP.ExceptionWithStackTrace(ADP.DataReaderClosed("GetFieldValueAsync"))
                        );
                    }
                }
            }
            ...
            moreFunc = (t) =>
            {
                ...
                {
                    return Task.FromResult<T>(
                        GetFieldValueFromSqlBufferInternal<T>(_data[i], _metaData[i])
                    );
                }
                else
                {
                    ...
                    .ContinueWith(
                        (retryTask) =>
                        {
                            if (retryTask.IsFaulted)
                            {
                                ...
                                TaskCompletionSource<T> exceptionSource =
        ...
        private Task<T> InvokeRetryable<T>(
            Func<Task, Task<T>> moreFunc,
        ...
        )
        {
            try
            {
                ...
                {
                    task.ContinueWith(
                        (t) => CompleteRetryable(t, source, objectToDispose),
                        TaskScheduler.Default
                    );
                }
            }
        ...
        private void PrepareAsyncInvocation(bool useSnapshot)
        {
            ...
            {
                Debug.Assert(
                    !_stateObj._asyncReadWithoutSnapshot,
            ...
            else
            {
                Debug.Assert(
                    _snapshot == null,
                    "Can prepare async invocation without snapshot if there is currently a snapshot"
                );
        ...
        private void CleanupAfterAsyncInvocation(bool ignoreCloseToken = false)
        {
            ...
            if (stateObj != null)
            {
                ...
                if (
                    (ignoreCloseToken)
                    || (!_cancelAsyncOnCloseToken.IsCancellationRequested)
                ...
                )
                {
                    ...
                    lock (stateObj)
                    {
                        ...
                        { // reader not closed while we waited for the lock
                            ...
                            Debug.Assert(
                                _snapshot == null && !_stateObj._asyncReadWithoutSnapshot,
        ...
        private void CleanupAfterAsyncInvocationInternal(
            ...
            bool resetNetworkPacketTaskSource = true
        )
        {
        ...
        private void SwitchToAsyncWithoutSnapshot()
        {
            ...
            Debug.Assert(
                _stateObj != null && !_stateObj._asyncReadWithoutSnapshot,
