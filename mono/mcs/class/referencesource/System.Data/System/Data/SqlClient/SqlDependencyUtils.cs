...
namespace System.Data.SqlClient
{
    ...
    { // MBR, since ref'ed by ProcessDispatcher.
        ...
        internal static readonly SqlDependencyPerAppDomainDispatcher SingletonInstance =
            new SqlDependencyPerAppDomainDispatcher(); // singleton object
        ...
        private SqlDependencyPerAppDomainDispatcher()
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependencyPerAppDomainDispatcher|DEP> %d#",
                ObjectID
            );
            try
            {
        ...
        private void UnloadEventHandler(object sender, EventArgs e)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependencyPerAppDomainDispatcher.UnloadEventHandler|DEP> %d#",
                ObjectID
            );
            try
            {
        ...
        internal void AddDependencyEntry(SqlDependency dep)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependencyPerAppDomainDispatcher.AddDependencyEntry|DEP> %d#, SqlDependency: %d#",
                ObjectID,
                dep.ObjectID
            );
            try
        ...
        internal string AddCommandEntry(string commandHash, SqlDependency dep)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependencyPerAppDomainDispatcher.AddCommandEntry|DEP> %d#, commandHash: '%ls', SqlDependency: %d#",
                ObjectID,
                commandHash,
                dep.ObjectID
            );
            try
            {
                lock (this)
                {
                    ...
                    else
                    {
                        ...
                        if (
                            _commandHashToNotificationId.TryGetValue(
                                commandHash,
                                out notificationId
                            )
                        )
                        {
                            ...
                            if (
                                !_notificationIdToDependenciesHash.TryGetValue(
                                    notificationId,
                                    out dependencyList
                                )
                            )
                        ...
                        else
                        {
                            ...
                            finally
                            {
                                ...
                                _notificationIdToDependenciesHash.Add(
                                    notificationId,
                                    dependencyList
                                );
                            }
                        ...
                        Debug.Assert(
                            _notificationIdToDependenciesHash.Count
                                == _commandHashToNotificationId.Count,
                            "always keep these maps in sync!"
                        );
                    }
                }
            }
        ...
        internal void InvalidateCommandID(SqlNotification sqlNotification)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependencyPerAppDomainDispatcher.InvalidateCommandID|DEP> %d#, commandHash: '%ls'",
                ObjectID,
                sqlNotification.Key
            );
            try
                    ...
                    foreach (SqlDependency dependency in dependencyList)
                    {
                        ...
                        try
                        {
                            dependency.Invalidate(
                                sqlNotification.Type,
                                sqlNotification.Info,
                                sqlNotification.Source
                            );
                        }
                        catch (Exception e)
                        {
        ...
        internal void InvalidateServer(string server, SqlNotification sqlNotification)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependencyPerAppDomainDispatcher.Invalidate|DEP> %d#, server: '%ls'",
                ObjectID,
                server
            );
            try
        ...
        internal SqlDependency LookupDependencyEntry(string id)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependencyPerAppDomainDispatcher.LookupDependencyEntry|DEP> %d#, Key: '%ls'",
                ObjectID,
                id
            );
            try
        ...
        private void LookupDependencyEntryWithRemove(string id)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependencyPerAppDomainDispatcher.LookupDependencyEntryWithRemove|DEP> %d#, id: '%ls'",
                ObjectID,
                id
            );
            try
        ...
        private List<SqlDependency> LookupCommandEntryWithRemove(string notificationId)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependencyPerAppDomainDispatcher.LookupCommandEntryWithRemove|DEP> %d#, commandHash: '%ls'",
                ObjectID,
                notificationId
            );
            try
        ...
        private void RemoveDependencyFromCommandToDependenciesHash(SqlDependency dependency)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependencyPerAppDomainDispatcher.RemoveDependencyFromCommandToDependenciesHash|DEP> %d#, SqlDependency: %d#",
                ObjectID,
                dependency.ObjectID
            );
            try
            {
                lock (this)
                {
                    ...
                    )
                    {
                        ...
                        {
                            Bid.NotificationsTrace(
                                "<sc.SqlDependencyPerAppDomainDispatcher.RemoveDependencyFromCommandToDependenciesHash|DEP> Removed SqlDependency: %d#, with ID: '%ls'.\n",
                                dependency.ObjectID,
                                dependency.Id
                            );
                    ...
                    Debug.Assert(
                        commandHashesToRemove.Count == notificationIdsToRemove.Count,
                        "maps should be kept in sync"
                    );
        ...
        internal void StartTimer(SqlDependency dep)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependencyPerAppDomainDispatcher.StartTimer|DEP> %d#, SqlDependency: %d#",
                ObjectID,
                dep.ObjectID
            );
            try
        ...
        private static void TimeoutTimerCallback(object state)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependencyPerAppDomainDispatcher.TimeoutTimerCallback|DEP> AppDomainKey: '%ls'",
