...
namespace System.Data.SqlClient
{
    ...
    sealed internal class TdsParser
    {
        ...
        // textptr sequence
        private static readonly byte[] s_longDataHeader =
        {
            0x10,
            0xff,
            0xff,
        ...
        private static readonly byte[] s_xmlMetadataSubstituteSequence =
        {
            0xe7,
            0xff,
            0xff,
        ...
        {
            get { return _serverSupportsColumnEncryption; }
            set { _serverSupportsColumnEncryption = value; }
        }
        ...
        internal SqlInternalTransaction CurrentTransaction
        {
            ...
            {
                Debug.Assert(
                    value == _currentTransaction
                        || null == _currentTransaction
                        || null == value
                        || (null != _currentTransaction && !_currentTransaction.IsLocal),
        ...
        private bool IncludeTraceHeader
        {
            ...
            {
                return (
                    _isDenali && Bid.TraceOn && Bid.IsOn(ActivityCorrelator.CorrelationTracePoints)
                );
            }
        }
        ...
        )
        {
            ...
            if (integratedSecurity || authType == SqlAuthenticationMethod.ActiveDirectoryIntegrated)
            {
                ...
                Bid.Trace(
                    "<sc.TdsParser.Connect|SEC> SSPI or Active Directory Authentication Library for SQL Server based integrated authentication\n"
                );
            }
            else
            {
            ...
            if (_connHandler.ConnectionOptions.TransparentNetworkIPResolution && !disableTnir)
            {
                if (isFirstTransparentAttempt)
                    transparentNetworkResolutionState =
                        TransparentNetworkResolutionState.SequentialMode;
                else
                    transparentNetworkResolutionState =
                        TransparentNetworkResolutionState.ParallelMode;
            }
            else
            ...
            if (
                authType == SqlAuthenticationMethod.ActiveDirectoryPassword
                || (
                    authType == SqlAuthenticationMethod.ActiveDirectoryIntegrated
                    && _connHandler._fedAuthRequired
                )
            )
            {
                Debug.Assert(
        ...
        {
            TdsParserStateObject session = new TdsParserStateObject(
                this,
                (SNIHandle)_pMarsPhysicalConObj.Handle,
                true
            );
            if (Bid.AdvancedOn)
            {
                Bid.Trace(
                    "<sc.TdsParser.CreateSession|ADV> %d# created session %d\n",
                    ObjectID,
                    session.ObjectID
                );
            }
        ...
        private void SendPreLoginHandshake(byte[] instanceName, bool encrypt)
        {
            ...
            byte[] payload = new byte[
                (int)PreLoginOptions.NUMOPT * 5 + TdsEnums.MAX_PRELOGIN_PAYLOAD_LENGTH
            ];
            int payloadLength = 0;
                ...
                switch (option)
                {
                    case (int)PreLoginOptions.VERSION:
                        ...
                        payload[payloadLength++] = (byte)(
                            (systemDataVersion.Revision & 0xff00) >> 8
                        );
                        offset += 6;
                        optionDataSize = 6;
                        break;
                    ...
                    case (int)PreLoginOptions.TRACEID:
                        ...
                        Bid.Trace(
                            ...
                            _connHandler._clientConnectionId.ToString(),
                            actId.ToString()
                        );
                        break;
        ...
        )
        {
            ...
            Debug.Assert(
                _physicalStateObj._syncOverAsync,
            ...
            if (_physicalStateObj._inBytesRead == 0)
            {
                ...
                _physicalStateObj.AddError(
                    new SqlError(
                        0,
                        (byte)0x00,
                        TdsEnums.FATAL_ERROR_CLASS,
                        _server,
                        SQLMessage.PreloginError(),
                        "",
                        0
                    )
                );
            ...
            if (
                _physicalStateObj._inBytesPacket > TdsEnums.MAX_PACKET_SIZE
                || _physicalStateObj._inBytesPacket <= 0
            )
            {
            ...
            while (option != (byte)PreLoginOptions.LASTOPT)
            {
                switch (option)
                {
                    case (int)PreLoginOptions.VERSION:
                        ...
                        int level = (payload[payloadOffset + 2] << 8) | payload[payloadOffset + 3];
                    ...
                    case (int)PreLoginOptions.ENCRYPT:
                                ...
                                if (serverOption == EncryptionOptions.NOT_SUP)
                                {
                                    _physicalStateObj.AddError(
                                        new SqlError(
                                            ...
                                            TdsEnums.FATAL_ERROR_CLASS,
                                            _server,
                                            SQLMessage.EncryptionNotSupportedByServer(),
                                            "",
                                            0
                                        )
                                    );
                                ...
                                if (serverOption == EncryptionOptions.REQ)
                                {
                                    _physicalStateObj.AddError(
                                        new SqlError(
                                            ...
                                            TdsEnums.FATAL_ERROR_CLASS,
                                            _server,
                                            SQLMessage.EncryptionNotSupportedByClient(),
                                            "",
                                            0
                                        )
                                    );
                        ...
                        )
                        {
                            ...
                            bool shouldValidateServerCert =
                                (encrypt && !trustServerCert)
                                || (
                                    (
                                        authType != SqlAuthenticationMethod.NotSpecified
                            ...
                            UInt32 info =
                                (
                                    shouldValidateServerCert
                                        ? TdsEnums.SNI_SSL_VALIDATE_CERTIFICATE
                                        : 0
                    ...
                    case (int)PreLoginOptions.MARS:
                        ...
                        Debug.Assert(
                            payload[payloadOffset] == 0 || payload[payloadOffset] == 1,
                            "Value for Mars PreLoginHandshake option not equal to 1 or 0!"
                        );
                        break;
                    ...
                    case (int)PreLoginOptions.FEDAUTHREQUIRED:
                        ...
                        if (payload[payloadOffset] != 0x00 && payload[payloadOffset] != 0x01)
                        {
                            ...
                            throw SQL.ParsingErrorValue(
                                ParsingErrorState.FedAuthRequiredPreLoginResponseInvalidValue,
                                (int)payload[payloadOffset]
                            );
                        }
                    ...
                    default:
                        Debug.Assert(
                            false,
                            "UNKNOWN option in ConsumePreLoginHandshake, option:" + option
                        );
        ...
        internal void Deactivate(bool connectionIsDoomed)
        {
            ...
            Debug.Assert(
                connectionIsDoomed || null == _pendingTransaction,
        ...
        internal void RollbackOrphanedAPITransactions()
        {
            ...
            if (
                null != currentTransaction
                && currentTransaction.HasParentTransaction
                && currentTransaction.IsOrphaned
            )
            {
                currentTransaction.CloseFromConnection();
                Debug.Assert(
        ...
        {
            Debug.Assert(
                !callerHasConnectionLock || _connHandler._parserLock.ThreadMayHaveLock(),
            ...
            // can be deleted)
            SqlErrorCollection temp = stateObj.GetFullErrorAndWarningCollection(
                out breakConnection
            );

            Debug.Assert(
                temp.Count > 0,
                "TdsParser::ThrowExceptionAndWarning called with no exceptions or warnings!"
            );
            if (temp.Count == 0)
            {
                Bid.Trace(
            ...
            Debug.Assert(
                _connHandler != null,
                "TdsParser::ThrowExceptionAndWarning called with null connectionHandler!"
            );
            ...
            if (breakConnection)
            {
                if (
                    ...
                    && (
                        ...
                        || _connHandler.ConnectionOptions.MultiSubnetFailover
                        || _loginWithFailover
                    )
                    && (temp.Count == 1)
                    && (
                        (temp[0].Number == TdsEnums.TIMEOUT_EXPIRED)
            ...
            Debug.Assert(
                temp != null,
                "TdsParser::ThrowExceptionAndWarning: 0 errors in collection"
            );
            if (temp != null && temp.Count > 0)
            {
                ...
                if (
                    exception.Procedure == TdsEnums.INIT_ADAL_PACKAGE
                    || exception.Procedure == TdsEnums.INIT_SSPI_PACKAGE
                )
                {
        ...
        internal SqlError ProcessSNIError(TdsParserStateObject stateObj)
        {
            ...
            Debug.Assert(
                SniContext.Undefined != stateObj.DebugOnlyCopyOfSniContext
                    || (
                        (_fMARS)
                        && ((_state == TdsParserState.Closed) || (_state == TdsParserState.Broken))
                    ),
            ...
            string sqlContextInfo = Res.GetString(
                Enum.GetName(typeof(SniContext), stateObj.SniContext)
            );

            string providerRid = String.Format(
            ...
            Debug.Assert(
                !ADP.IsEmpty(providerName),
                String.Format(
                    (IFormatProvider)null,
                    "invalid providerResourceId '{0}'",
                    providerRid
                )
            );
            ...
            if (sniError.sniError == 0)
            {
                ...
                Debug.Assert(
                    errorMessage.Length > iColon + 1 && errorMessage[iColon + 1] == ' ',
                    "Expecting a space after the ':' character"
                );
            ...
            errorMessage = String.Format(
                (IFormatProvider)null,
                "{0} (provider: {1}, error: {2} - {3})",
                sqlContextInfo,
                providerName,
                (int)sniError.sniError,
                errorMessage
            );

            return new SqlError(
                (int)sniError.nativeError,
                0x00,
                TdsEnums.FATAL_ERROR_CLASS,
                _server,
                errorMessage,
                sniError.function,
                (int)sniError.lineNumber,
                win32ErrorCode
            );
        }
        ...
        internal void CheckResetConnection(TdsParserStateObject stateObj)
        {
            ...
            {
                Debug.Assert(
                    stateObj._outputPacketNumber == 1 || stateObj._outputPacketNumber == 2,
                    "In ResetConnection logic unexpectedly!"
                );
                try
                {
                    if (_fMARS && !stateObj._fResetEventOwned)
                    {
                        ...
                        stateObj._fResetEventOwned = _resetConnectionEvent.WaitOne(
                            stateObj.GetTimeoutRemaining(),
                            false
                        );
                        ...
                        if (!stateObj._fResetEventOwned)
                        {
                            ...
                            stateObj.AddError(
                                new SqlError(
                                    ...
                                    _server,
                                    _connHandler.TimeoutErrorInternal.GetErrorMessage(),
                                    "",
                                    0,
                    ...
                    if (_fResetConnection)
                    {
                        ...
                        Debug.Assert(
                            !stateObj._fResetConnectionSent,
                        ...
                        else
                        {
                            // if we are reseting, set bit in header by or'ing with other value
                            stateObj._outBuff[1] = (Byte)(
                                stateObj._outBuff[1] | TdsEnums.ST_RESET_CONNECTION
                            );
                        }
        ...
        internal void WritePartialLong(long v, int length, TdsParserStateObject stateObj)
        {
            ...
            else
            {
                ...
                for (int index = 0; index < length; index++)
                {
                    stateObj._outBuff[stateObj._outBytesUsed + index] = (byte)(
                        (v >> (index * 8)) & 0xff
                    );
                }
        ...
        )
        {
            ...
            try
            {
                ...
                Debug.Assert(
                    result == true,
                    "Should never return false when _syncOverAsync is set"
                );
                return dataReady;
            }
            finally
        ...
        )
        {
            ...
            Debug.Assert(
                (SniContext.Undefined != stateObj.SniContext)
                    && // SniContext must not be Undefined
                    (
                        (stateObj._attentionSent)
                        || (
                            (SniContext.Snix_Execute != stateObj.SniContext)
                ...
                String.Format(
                    "Unexpected SniContext on call to TryRun; SniContext={0}",
                    stateObj.SniContext
                )
            );
            ...
            do
            {
                ...
                if (!IsValidTdsToken(token))
                {
                    ...
                    Bid.Trace(
                        "<sc.TdsParser.Run|ERR> Potential multi-threaded misuse of connection, unexpected TDS token found %d#\n",
                        ObjectID
                    );
                ...
                switch (token)
                {
                    ...
                    case TdsEnums.SQLINFO:
                    {
                        ...
                        {
                            Debug.Assert(
                                error.Class < TdsEnums.MIN_ERROR_CLASS,
                    ...
                    case TdsEnums.SQLALTMETADATA:
                    {
                        ...
                        if (
                            !TryProcessAltMetaData(tokenLength, stateObj, out cleanupAltMetaDataSet)
                        )
                        {
                            return false;
                        }
                        ...
                        if (null != dataStream)
                        {
                            ...
                            if (
                                !dataStream.TrySetAltMetaDataSet(
                                    cleanupAltMetaDataSet,
                                    (TdsEnums.SQLALTMETADATA != metadataConsumedByte)
                                )
                            )
                            {
                                return false;
                    ...
                    case TdsEnums.SQLALTROW:
                    {
                        ...
                        if (
                            ...
                            != (RunBehavior.ReturnImmediately & runBehavior)
                        )
                        {
                            ushort altRowId;
                            ...
                            if (
                                !TrySkipRow(
                                    stateObj._cleanupAltMetaDataSetArray.GetAltMetaData(altRowId),
                                    stateObj
                                )
                            )
                            { // skip altRow
                            ...
                            if (env[ii] != null && !this.Connection.IgnoreEnvChange)
                            {
                                switch (env[ii].type)
                                {
                                    ...
                                    case TdsEnums.ENV_ENLISTDTC:
                                        ...
                                        {
                                            _currentTransaction.TransactionId = env[
                                                ii
                                            ].newLongValue; // this is defined as a ULongLong in the server and in the TDS Spec.
                                        }
                                        else
                                        {
                                            TransactionType transactionType =
                                                (TdsEnums.ENV_BEGINTRAN == env[ii].type)
                                        ...
                                        _retainedTransactionId =
                                            SqlInternalTransaction.NullTransactionId;
                                        break;
                    ...
                    case TdsEnums.SQLSESSIONSTATE:
                    {
                        if (
                            !TryProcessSessionState(
                                stateObj,
                                tokenLength,
                                _connHandler._currentSessionData
                            )
                        )
                        {
                            return false;
                    ...
                    case TdsEnums.SQLCOLMETADATA:
                    {
                        if (tokenLength != TdsEnums.VARNULL)
                        {
                            ...
                            if (
                                !TryProcessMetaData(
                                    ...
                                    out metadata,
                                    cmdHandler != null
                                        ? cmdHandler.ColumnEncryptionSetting
                        ...
                        if (null != dataStream)
                        {
                            ...
                            if (
                                !dataStream.TrySetMetaData(
                                    stateObj._cleanupMetaData,
                                    (
                                        TdsEnums.SQLTABNAME == peekedToken
                                        || TdsEnums.SQLCOLINFO == peekedToken
                                    )
                                )
                            )
                            {
                    ...
                    {
                        Debug.Assert(
                            stateObj._cleanupMetaData != null,
                            "Reading a row, but the metadata is null"
                        );

                        if (token == TdsEnums.SQLNBCROW)
                        {
                            if (
                                !stateObj.TryStartNewRow(
                                    isNullCompressed: true,
                                    nullBitmapColumnsCount: stateObj._cleanupMetaData.Length
                                )
                            )
                            {
                                return false;
                        ...
                        if (null != bulkCopyHandler)
                        {
                            ...
                            if (
                                !TryProcessRow(
                                    stateObj._cleanupMetaData,
                                    bulkCopyHandler.CreateRowBuffer(),
                                    bulkCopyHandler.CreateIndexMap(),
                                    stateObj
                                )
                            )
                            {
                    ...
                    default:
                        Debug.Assert(
                            false,
                            "Unhandled token:  " + token.ToString(CultureInfo.InvariantCulture)
                        );
                        break;
                }

                Debug.Assert(
                    stateObj._pendingData || !dataReady,
            ...
            if ((stateObj._pendingData) && (!dataReady))
            {
                ...
                Debug.Assert(
                    IsValidTdsToken(token),
                    string.Format(
                        "DataReady is false, but next token is not valid: {0,-2:X2}",
                        token
                    )
                );
            }
            ...
            if (stateObj._attentionReceived)
            {
                ...
                if (stateObj._attentionSent)
                {
                    ...
                    if (
                        RunBehavior.Clean != (RunBehavior.Clean & runBehavior)
        ...
        private bool TryProcessEnvChange(
            int tokenLength,
            TdsParserStateObject stateObj,
            ...
            while (tokenLength > processedLength)
            {
                ...
                switch (env.type)
                {
                    ...
                    case TdsEnums.ENV_CHARSET:
                        ...
                        {
                            Debug.Assert(
                                env.newValue.Length > TdsEnums.CHARSET_CODE_PAGE_OFFSET,
                    ...
                    case TdsEnums.ENV_PACKETSIZE:
                        ...
                        if (_physicalStateObj.SetPacketSize(packetSize))
                        {
                            ...
                            UInt32 result = SNINativeMethodWrapper.SNISetInfo(
                                _physicalStateObj.Handle,
                                SNINativeMethodWrapper.QTypes.SNI_QUERY_CONN_BUFSIZE,
                    ...
                    case TdsEnums.ENV_COLLATION:
                        Debug.Assert(
                            env.newLength == 5 || env.newLength == 0,
                            "Improper length in new collation!"
                        );
                        ...
                        Debug.Assert(
                            env.oldLength == 5 || env.oldLength == 0,
                            "Improper length in old collation!"
                        );
                        if (env.oldLength == 5)
                        {
                    ...
                    case TdsEnums.ENV_TRANSACTIONENDED:
                        ...
                        env.newLength = byteLength;
                        Debug.Assert(
                            env.newLength == 0 || env.newLength == 8,
                            "Improper length for new transaction id!"
                        );
                        ...
                        if (env.newLength > 0)
                        {
                            ...
                            Debug.Assert(
                                env.newLongValue != SqlInternalTransaction.NullTransactionId,
                                "New transaction id is null?"
                        ...
                        env.oldLength = byteLength;
                        Debug.Assert(
                            env.oldLength == 0 || env.oldLength == 8,
                            "Improper length for old transaction id!"
                        );
                        ...
                        if (env.oldLength > 0)
                        {
                            ...
                            Debug.Assert(
                                env.oldLongValue != SqlInternalTransaction.NullTransactionId,
                                "Old transaction id is null?"
                            ); // the server guarantees that zero is an invalid transaction id.
                        }
                        else
                        {
        ...
        )
        {
            ...
            {
                Debug.Assert(
                    TdsEnums.DONE_MORE != (status & TdsEnums.DONE_MORE),
                ...
                Debug.Assert(
                    stateObj._inBytesUsed == stateObj._inBytesRead && stateObj._inBytesPacket == 0,
                    "DONE_ATTN received with more data left on wire"
                );
            }
            ...
            if (
                (TdsEnums.DONE_ERROR == (TdsEnums.DONE_ERROR & status))
            ...
            if (
                (TdsEnums.DONE_SRVERROR == (TdsEnums.DONE_SRVERROR & status))
                && (RunBehavior.Clean != (RunBehavior.Clean & run))
            )
            {
                stateObj.AddError(
        ...
        private bool TryProcessFeatureExtAck(TdsParserStateObject stateObj)
        {
            ...
            if (
                _connHandler.ConnectionOptions.ColumnEncryptionSetting
                    ...
                    else
                    {
                        if (sdata._delta[stateId]._version <= seqNum)
                        {
                            ...
                            if (sv._recoverable != recoverable)
                            {
                                ...
                                {
                                    Debug.Assert(
                                        sdata._unrecoverableStatesCount > 0,
                                        "Unrecoverable states count >0"
                                    );
                                    sdata._unrecoverableStatesCount--;
                                }
                                else
        ...
        private bool TryProcessLoginAck(TdsParserStateObject stateObj, out SqlLoginAck sqlLoginAck)
        {
            ...
            if (
                _connHandler.ConnectionOptions.UserInstance
            ...
            )
            {
                stateObj.AddError(
                    new SqlError(
                        ...
                        TdsEnums.FATAL_ERROR_CLASS,
                        Server,
                        SQLMessage.UserInstanceFailure(),
                        "",
                        0
                    )
                );
        ...
        )
        {
            ...
            {
                Bid.Trace(
                    "<sc.TdsParser.TryProcessFedAuthInfo> FEDAUTHINFO token stream length = {0}\n",
                    tokenLen
                );
            }
            ...
            if (tokenLen < sizeof(uint))
            {
                ...
                throw SQL.ParsingErrorLength(
                    ParsingErrorState.FedAuthInfoLengthTooShortForCountOfInfoIds,
                    tokenLen
                );
            }
            ...
            {
                Bid.Trace(
                    "<sc.TdsParser.TryProcessFedAuthInfo> CountOfInfoIDs = {0}\n",
            ...
            if (tokenLen > 0)
            {
                ...
                int totalRead = 0;
                bool successfulRead = stateObj.TryReadByteArray(
                    tokenData,
                    0,
                    tokenLen,
                    out totalRead
                ...
                for (uint i = 0; i < optionsCount; i++)
                {
                    ...
                    uint dataLen = BitConverter.ToUInt32(
                        tokenData,
                        checked((int)(currentOptionOffset + 1))
                    );
                    uint dataOffset = BitConverter.ToUInt32(
                        tokenData,
                        checked((int)(currentOptionOffset + 5))
                    );
                    ...
                    if (Bid.AdvancedOn)
                    {
                        Bid.Trace(
                            ...
                            id,
                            dataLen.ToString(CultureInfo.InvariantCulture),
                    ...
                    if (dataOffset < totalOptionsSize || dataOffset >= tokenLen)
                    {
                        ...
                        throw SQL.ParsingErrorOffset(
                            ParsingErrorState.FedAuthInfoInvalidOffset,
                    ...
                    {
                        Bid.Trace(
                            "<sc.TdsParser.TryProcessFedAuthInfo> FedAuthInfoData: {0}\n",
                            data
                        );
                    }
            ...
            else
            {
                ...
                throw SQL.ParsingErrorLength(
                    ParsingErrorState.FedAuthInfoLengthTooShortForData,
                    tokenLen
                );
            }
            ...
            if (
                String.IsNullOrWhiteSpace(tempFedAuthInfo.stsurl)
                || String.IsNullOrWhiteSpace(tempFedAuthInfo.spn)
            )
            {
        ...
        internal bool TryProcessError(byte token, TdsParserStateObject stateObj, out SqlError error)
        {
            ...
            Debug.Assert(
                ((errorClass >= TdsEnums.MIN_ERROR_CLASS) && token == TdsEnums.SQLERROR)
        ...
        internal bool TryProcessReturnValue(
            int length,
            TdsParserStateObject stateObj,
        ...
        {
            Debug.Assert(
                isReturnValue == (cipherTable == null),
                "Ciphertable is not set iff this is a return value"
            );
            ...
            if (cipherTable.HasValue)
            {
                ...
                if (index >= cipherTable.Value.Size)
                {
                    Bid.Trace(
                        ...
                        index,
                        cipherTable.Value.Size
                    );
                    throw SQL.ParsingErrorValue(
                        ParsingErrorState.TceInvalidOrdinalIntoCipherInfoTable,
                        index
                    );
                }
            }
            ...
            Debug.Assert(
                col.cipherMD == null,
                "col.cipherMD should be null in TryProcessTceCryptoMetadata."
            );
            ...
            {
                col.cipherMD = new SqlCipherMetadata(
                    cipherTable.HasValue ? (SqlTceCipherInfoEntry?)cipherTable.Value[index] : null,
                    index,
        ...
        internal int GetCodePage(SqlCollation collation, TdsParserStateObject stateObj)
        {
            ...
            if (0 != collation.sortId)
            {
                ...
                Debug.Assert(
                    0 != codePage,
                    "GetCodePage accessed codepage array and produced 0!, sortID ="
                        + ((Byte)(collation.sortId)).ToString((IFormatProvider)null)
                );
            }
            else
            {
                    ...
                    try
                    {
                        ...
                        if (sharedState != null && sharedState._dataReady)
                        {
                            ...
                            if (0 == sharedState._nextColumnHeaderToRead)
                            {
                                ...
                                if (
                                    !stateObj.Parser.TrySkipRow(stateObj._cleanupMetaData, stateObj)
                                )
                                {
                                    throw SQL.SynchronousCallMayNotPend();
                                }
                            ...
                            else
                            {
                                ...
                                if (
                                    sharedState._nextColumnDataToRead
                                    < sharedState._nextColumnHeaderToRead
                                )
                                {
                                    if (
                                        (sharedState._nextColumnHeaderToRead > 0)
                                        && (
                                            metadata[sharedState._nextColumnHeaderToRead - 1]
                                                .metaType
                                                .IsPlp
                                        )
                                    )
                                    ...
                                    else if (0 < sharedState._columnDataBytesRemaining)
                                    {
                                        if (
                                            !stateObj.TrySkipLongBytes(
                                                sharedState._columnDataBytesRemaining
                                            )
                                        )
                                        {
                                            throw SQL.SynchronousCallMayNotPend();
        ...
        internal bool TryProcessAltMetaData(
            int cColumns,
            TdsParserStateObject stateObj,
        ...
        internal bool TryProcessCipherInfoTable(
            TdsParserStateObject stateObj,
            out SqlTceCipherInfoTable? cipherTable
        )
        {
            // Read count
            short tableSize = 0;
        ...
        internal bool TryProcessMetaData(
            int cColumns,
            TdsParserStateObject stateObj,
            out _SqlMetaDataSet metaData,
            SqlCommandColumnEncryptionSetting columnEncryptionSetting
        ...
        {
            return tdsType == TdsEnums.SQLTIME
                || tdsType == TdsEnums.SQLDATETIME2
                || tdsType == TdsEnums.SQLDATETIMEOFFSET;
        }
                ...
                if (col.length == TdsEnums.SQL_USHORTVARMAXLEN)
                {
                    ...
                    if (tdsType == TdsEnums.SQLXMLTYPE)
                    {
                        ...
                        if ((schemapresent & 1) != 0)
                        {
                            ...
                            {
                                if (
                                    !stateObj.TryReadString(
                                        byteLen,
                                        out col.xmlSchemaCollectionDatabase
                                    )
                                )
                                {
                                    return false;
                            ...
                            if (byteLen != 0)
                            {
                                if (
                                    !stateObj.TryReadString(
                                        byteLen,
                                        out col.xmlSchemaCollectionOwningSchema
                                    )
                                )
                                {
                                    return false;
                            ...
                            if (byteLen != 0)
                            {
                                if (
                                    !stateObj.TryReadString(
                                        shortLen,
                                        out col.xmlSchemaCollectionName
                                    )
                                )
                                {
                                    return false;
        ...
        internal bool TryProcessTableName(
            int length,
            TdsParserStateObject stateObj,
            out MultiPartTableName[] multiPartTableNames
        )
        {
            int tablesAdded = 0;
        ...
        )
        {
            ...
            if (_isShilohSP1)
            {
                ...
                Debug.Assert(
                    nParts == 0,
                    "ProcessTableName:Unidentified parts in the table name token stream!"
                );
            }
            else
            {
                ...
                length -= (tableLen * 2); // wide bytes
                mpt = new MultiPartTableName(
                    MultipartIdentifier.ParseMultipartIdentifier(
                        tableName,
                        "[\"",
                        "]\"",
                        Res.SQL_TDSParserTableName,
                        false
                    )
                );
            }
        ...
        private bool TryProcessColInfo(
            _SqlMetaDataSet columns,
            SqlDataReader reader,
            TdsParserStateObject stateObj,
            out _SqlMetaDataSet metaData
        )
        {
            ...
            for (int i = 0; i < columns.Length; i++)
            {
                ...
                col.isDifferentName = (
                    TdsEnums.SQLDifferentName == (status & TdsEnums.SQLDifferentName)
                );
                ...
                {
                    Debug.Assert(
                        reader.TableNames.Length >= col.tableNum,
        ...
        internal bool TryProcessColumnHeader(
            SqlMetaDataPriv col,
            TdsParserStateObject stateObj,
            int columnOrdinal,
            out bool isNull,
        ...
        )
        {
            ...
            for (int i = 0; i < columns.Length; i++)
            {
                ...
                Debug.Assert(
                    md != null,
                    "_SqlMetaData should not be null for column "
                ...
                else
                {
                    ...
                    if (
                        !TryReadSqlValue(
                            data,
                            md,
                            md.metaType.IsPlp ? (Int32.MaxValue) : (int)len,
                            stateObj,
                            SqlCommandColumnEncryptionSetting.Disabled /*Column Encryption Disabled for Bulk Copy*/
                            ,
                            md.column
                    ...
                    if (stateObj._longlen != 0)
                    {
                        throw new SqlTruncateException(
                            Res.GetString(Res.SqlMisc_TruncationMaxDataMessage)
                        );
                    }
                }
                data.Clear();
            ...
            switch (columnEncryptionSetting)
            {
                ...
                default:
                    ...
                    return (
                        ...
                        && connection.ConnectionOptions.ColumnEncryptionSetting
                            == SqlConnectionColumnEncryptionSetting.Enabled
                    );
            }
        }
        ...
        )
        {
            switch (type)
            {
                ...
                case TdsEnums.SQLTEXT:
                    ...
                    string stringValue;
                    if (
                        !stateObj.TryReadStringWithEncoding(
                            length,
                            encoding,
                            isPlp,
                            out stringValue
                ...
                default:
                    Debug.Assert(
                        false,
                        "Unknown tds type for SqlString!"
        ...
        )
        {
            ...
            switch (tdsType)
            {
                ...
                case TdsEnums.SQLINT8:
                    ...
                    if (tdsType == TdsEnums.SQLBIT || tdsType == TdsEnums.SQLBITN)
                    {
                ...
                case TdsEnums.SQLDATETIM4:
                    ...
                    value.SetToDateTime(
                        daypartShort,
                        timepartShort * SqlDateTime.SQLTicksPerMinute
                    );
                    break;
        ...
        )
        {
            ...
            Debug.Assert(
                isPlp || !IsNull(md.metaType, (ulong)length),
            ...
            switch (tdsType)
            {
                ...
                case TdsEnums.SQLNTEXT:
                    if (
                        !TryReadSqlStringValue(value, tdsType, length, md.encoding, isPlp, stateObj)
                    )
                    {
                        return false;
                    }
            ...
            Debug.Assert(
                ...
                "ReadSqlValue did not read plp field completely, longlen ="
                    + stateObj._longlen.ToString((IFormatProvider)null)
                    + ",longlenleft="
                    + stateObj._longlenleft.ToString((IFormatProvider)null)
            );
            return true;
        }
        ...
        )
        {
            ...
            switch (tdsType)
            {
                ...
                case TdsEnums.SQLDATETIMEOFFSET:
                    Debug.Assert(
                        8 <= length && length <= 10,
                        "invalid length for datetimeoffset type!"
                    );
        ...
        )
        {
            switch (tdsType)
            {
                ...
                default:
                    Debug.Assert(
                        false,
                        "Unknown SqlType!" + tdsType.ToString(CultureInfo.InvariantCulture)
                    );
                    break;
            } // switch
        ...
        )
        {
            Debug.Assert(
                _isShiloh == true,
                "Shouldn't be dealing with sql_variaint in pre-SQL2000 server!"
            );
            ...
            MetaType mt = MetaType.GetSqlDataType(
                type,
                0 /*no user datatype*/
                ,
                0 /* no lenData, non-nullable type */
            );
            ...
            switch (type)
            {
                ...
                case TdsEnums.SQLNUMERICN:
                {
                    Debug.Assert(
                        cbPropsExpected == 2,
                        "SqlVariant: invalid PropBytes for decimal/numeric type!"
                    );
                ...
                case TdsEnums.SQLBIGVARBINARY:
                    ...
                    Debug.Assert(
                        cbPropsExpected == 2,
                        "SqlVariant: invalid PropBytes for binary type!"
                    );
                    ...
                    Debug.Assert(
                        lenMax != TdsEnums.SQL_USHORTVARMAXLEN,
                ...
                case TdsEnums.SQLNVARCHAR:
                {
                    Debug.Assert(
                        cbPropsExpected == 7,
                        "SqlVariant: invalid PropBytes for character type!"
                    );
                ...
                case TdsEnums.SQLDATETIMEOFFSET:
                {
                    Debug.Assert(
                        cbPropsExpected == 1,
                        "SqlVariant: invalid PropBytes for time/datetime2/datetimeoffset type!"
                    );
                ...
                default:
                    Debug.Assert(
                        false,
                        "Unknown tds type in SqlVariant!"
                            + type.ToString(CultureInfo.InvariantCulture)
                    );
                    break;
            } // switch
        ...
        //
        internal Task WriteSqlVariantValue(
            object value,
            int length,
            int offset,
            TdsParserStateObject stateObj,
        ...
        )
        {
            Debug.Assert(
                _isShiloh == true,
                "Shouldn't be dealing with sql_variant in pre-SQL2000 server!"
            );
            ...
            switch (mt.TDSType)
            {
                ...
                case TdsEnums.SQLUNIQUEID:
                {
                    ...
                    Debug.Assert(
                        (length == b.Length) && (length == 16),
                        "Invalid length for guid type in com+ object"
                    );
        ...
        //
        internal Task WriteSqlVariantDataRowValue(
            object value,
            TdsParserStateObject stateObj,
            bool canAccumulate = true
        )
        ...
        internal void WriteSqlVariantHeader(
            int length,
            byte tdstype,
            byte propbytes,
            TdsParserStateObject stateObj
        ...
        internal void WriteSqlVariantDateTime2(DateTime value, TdsParserStateObject stateObj)
        {
            ...
            WriteDateTime2(
                value,
                dateTime2MetaData.Scale,
                (int)(dateTime2MetaData.MaxLength),
                stateObj
            );
        }
        ...
        private void WriteSqlMoney(SqlMoney value, int length, TdsParserStateObject stateObj)
        {
            ...
            if (length == 4)
            {
                ...
                if (
                    decimalValue < TdsEnums.SQL_SMALL_MONEY_MIN
                    || decimalValue > TdsEnums.SQL_SMALL_MONEY_MAX
                )
                {
        ...
        private void WriteDateTimeOffset(
        ...
        private bool TryReadDecimalBits(int length, TdsParserStateObject stateObj, out int[] bits)
        {
            ...
            Debug.Assert(
                (length > 0) && (length <= TdsEnums.MAX_NUMERIC_LEN - 1) && (length % 4 == 0),
        ...
        private Task WriteEncodingChar(
            string s,
            Encoding encoding,
            TdsParserStateObject stateObj,
            bool canAccumulate = true
        ...
        internal int GetEncodingCharLength(
            string value,
            int numChars,
            int charOffset,
            Encoding encoding
        ...
        //
        internal bool TryGetDataLength(
        ...
        )
        {
            ...
            {
                Debug.Assert(
                    colmeta.tdsType == TdsEnums.SQLXMLTYPE
        ...
        //
        internal bool TryGetTokenLength(
            byte token,
            TdsParserStateObject stateObj,
            out int tokenLength
        )
        {
            ...
            { // Handle Yukon specific exceptions
                ...
                else if (token == TdsEnums.SQLXMLTYPE)
                {
                    ...
                    Debug.Assert(
                        tokenLength == TdsEnums.SQL_USHORTVARMAXLEN,
        ...
        internal int WriteSessionRecoveryFeatureRequest(
            SessionData reconnectData,
        ...
        )
        {
            ...
            {
                Debug.Assert(
                    reconnectData._unrecoverableStatesCount == 0,
                    "Unrecoverable state count should be 0"
                );
                int initialLength = 0; // sizeof(DWORD) - length itself
                initialLength +=
                    1
                    + 2
                        * TdsParserStaticMethods.NullAwareStringLength(
                            reconnectData._initialDatabase
                        );
                initialLength +=
                    1
                    + 2
                ...
                for (int i = 0; i < SessionData._maxNumberOfSessionStates; i++)
                {
                    if (reconnectData._initialState[i] != null)
                    {
                        initialLength +=
                            1 /* StateId*/
                            + StateValueLength(reconnectData._initialState[i].Length);
                    }
                }
                ...
                currentLength +=
                    ...
                    + 2
                        * (
                            reconnectData._initialDatabase == reconnectData._database
                                ? 0
                                : TdsParserStaticMethods.NullAwareStringLength(
                                    reconnectData._database
                                )
                        );
                currentLength +=
                    1
                    + 2
                        * (
                            reconnectData._initialLanguage == reconnectData._language
                                ? 0
                                : TdsParserStaticMethods.NullAwareStringLength(
                                    reconnectData._language
                                )
                        );
                currentLength +=
                    (
                        reconnectData._collation != null
                        && !SqlCollation.AreSame(
                            reconnectData._collation,
                            reconnectData._initialCollation
                        )
                    )
                        ? 6
                        : 1;
                ...
                for (int i = 0; i < SessionData._maxNumberOfSessionStates; i++)
                {
                    ...
                    {
                        Debug.Assert(
                            reconnectData._delta[i]._recoverable,
                            "State should be recoverable"
                        );
                        writeState[i] = true;
                        if (
                            reconnectData._initialState[i] != null
                            && reconnectData._initialState[i].Length
                                == reconnectData._delta[i]._dataLength
                        )
                        {
                            writeState[i] = false;
                            ...
                            {
                                if (
                                    reconnectData._initialState[i][j]
                                    != reconnectData._delta[i]._data[j]
                                )
                                {
                                    writeState[i] = true;
                                    break;
                        ...
                        if (writeState[i])
                        {
                            currentLength +=
                                1 /* StateId*/
                                + StateValueLength(reconnectData._delta[i]._dataLength);
                        }
                    }
                }
                if (write)
                {
                    ...
                    WriteIdentifier(
                        reconnectData._database != reconnectData._initialDatabase
                            ? reconnectData._database
                            : null,
                        _physicalStateObj
                    );
                    ...
                    WriteIdentifier(
                        reconnectData._language != reconnectData._initialLanguage
                            ? reconnectData._language
                            : null,
                        _physicalStateObj
                    );
        ...
        internal int WriteFedAuthFeatureRequest(
            FederatedAuthenticationFeatureExtensionData fedAuthFeatureData,
            bool write /* if false just calculates the length */
        )
        {
            Debug.Assert(
                fedAuthFeatureData.libraryType == TdsEnums.FedAuthLibrary.ADAL
                    || fedAuthFeatureData.libraryType == TdsEnums.FedAuthLibrary.SecurityToken,
            ...
            switch (fedAuthFeatureData.libraryType)
            {
                ...
                case TdsEnums.FedAuthLibrary.SecurityToken:
                    Debug.Assert(
                        fedAuthFeatureData.accessToken != null,
                ...
                default:
                    Debug.Assert(
                        false,
                        "Unrecognized library type for fedauth feature extension request"
                    );
                    break;
            }
            ...
            if (write)
            {
                ...
                switch (fedAuthFeatureData.libraryType)
                {
                    case TdsEnums.FedAuthLibrary.ADAL:
                        Debug.Assert(
                            _connHandler._federatedAuthenticationInfoRequested == true,
                    ...
                    case TdsEnums.FedAuthLibrary.SecurityToken:
                        Debug.Assert(
                            ...
                            "_federatedAuthenticationRequested field should be true"
                        );
                    ...
                    default:
                        Debug.Assert(
                            false,
                            "Unrecognized FedAuthLibrary type for feature extension request"
                        );
                        break;
                }

                options |= (byte)(
                    fedAuthFeatureData.fedAuthRequiredPreLoginResponse == true ? 0x01 : 0x00
                );
                ...
                switch (fedAuthFeatureData.libraryType)
                {
                    case TdsEnums.FedAuthLibrary.ADAL:
                        ...
                        switch (fedAuthFeatureData.authentication)
                        {
                            ...
                            default:
                                Debug.Assert(
                                    false,
                                    "Unrecognized Authentication type for fedauth ADAL request"
                                );
                                break;
                        }
                    ...
                    case TdsEnums.FedAuthLibrary.SecurityToken:
                        ...
                        _physicalStateObj.WriteByteArray(
                            fedAuthFeatureData.accessToken,
                            fedAuthFeatureData.accessToken.Length,
                            0
                        );
                        break;
                    default:
        ...
        internal int WriteTceFeatureRequest(
            bool write /* if false just calculates the length */
        )
        {
        ...
        internal int WriteGlobalTransactionsFeatureRequest(
            bool write /* if false just calculates the length */
        )
        {
        ...
        )
        {
            ...
            Debug.Assert(
                recoverySessionData == null
                    || (requestedFeatures & TdsEnums.FeatureExtension.SessionRecovery) != 0,
                "Recovery session data without session recovery feature request"
            );
            Debug.Assert(
                TdsEnums.MAXLEN_HOSTNAME >= rec.hostName.Length,
                "_workstationId.Length exceeds the max length for this value"
            );

            Debug.Assert(
                !(rec.useSSPI && _connHandler._fedAuthRequired),
                "Cannot use SSPI when server has responded 0x01 for FedAuthRequired PreLogin Option."
            );
            Debug.Assert(
                !rec.useSSPI || (requestedFeatures & TdsEnums.FeatureExtension.FedAuth) == 0,
                "Cannot use both SSPI and FedAuth"
            );
            Debug.Assert(
                fedAuthFeatureExtensionData == null
                    || (requestedFeatures & TdsEnums.FeatureExtension.FedAuth) != 0,
            ...
            Debug.Assert(
                rec.userName == null
                    || (rec.userName != null && TdsEnums.MAXLEN_USERNAME >= rec.userName.Length),
            ...
            Debug.Assert(
                rec.credential == null
                    || (
                        rec.credential != null
                        && TdsEnums.MAXLEN_USERNAME >= rec.credential.UserId.Length
                    ),
            ...
            Debug.Assert(
                rec.password == null
                    || (rec.password != null && TdsEnums.MAXLEN_PASSWORD >= rec.password.Length),
            ...
            Debug.Assert(
                rec.credential == null
                    || (
                        rec.credential != null
                        && TdsEnums.MAXLEN_PASSWORD >= rec.credential.Password.Length
                    ),
                "_credential.Password.Length exceeds the max length for this value"
            );

            Debug.Assert(
                rec.credential != null || rec.userName != null || rec.password != null,
                "cannot mix the new secure password system and the connection string based password"
            );
            Debug.Assert(
                rec.newSecurePassword != null || rec.newPassword != null,
                "cannot have both new secure change password and string based change password"
            );
            Debug.Assert(
            ...
            Debug.Assert(
                TdsEnums.MAXLEN_SERVERNAME >= rec.serverName.Length,
                "_dataSource.Length exceeds the max length for this value"
            );
            Debug.Assert(
                TdsEnums.MAXLEN_LANGUAGE >= rec.language.Length,
                "_currentLanguage .Length exceeds the max length for this value"
            );
            Debug.Assert(
                TdsEnums.MAXLEN_DATABASE >= rec.database.Length,
                "_initialCatalog.Length exceeds the max length for this value"
            );
            Debug.Assert(
                TdsEnums.MAXLEN_ATTACHDBFILE >= rec.attachDBFilename.Length,
            ...
            Debug.Assert(
                TdsEnums.MAXLEN_CLIENTINTERFACE >= clientInterfaceName.Length,
            ...
            checked
            {
                length +=
                    (
                        ...
                        + rec.serverName.Length
                        + clientInterfaceName.Length
                        + rec.language.Length
                        + rec.database.Length
                        + rec.attachDBFilename.Length
                    ) * 2;
                if (useFeatureExt)
                {
                    length += 4;
            ...
            if (
                !rec.useSSPI
                && !(
                    _connHandler._federatedAuthenticationInfoRequested
                    || _connHandler._federatedAuthenticationRequested
                )
            )
            {
                checked
            ...
            else
            {
                if (rec.useSSPI)
                {
                    ...
                    Debug.Assert(
                        SniContext.Snix_Login == _physicalStateObj.SniContext,
                        String.Format(
                            (IFormatProvider)null,
                            "Unexpected SniContext. Expecting Snix_Login, actual value is '{0}'",
                            _physicalStateObj.SniContext
                        )
                    );
            ...
            if (useFeatureExt)
            {
                checked
                {
                    ...
                    if ((requestedFeatures & TdsEnums.FeatureExtension.FedAuth) != 0)
                    {
                        Debug.Assert(
                            fedAuthFeatureExtensionData != null,
                            "fedAuthFeatureExtensionData should not null."
                        );
                        length += WriteFedAuthFeatureRequest(
                            fedAuthFeatureExtensionData.Value,
                            write: false
                        );
                    }
            ...
            try
            {
                ...
                {
                    WriteInt(
                        (TdsEnums.DENALI_MAJOR << 24)
                            | (TdsEnums.DENALI_INCREMENT << 16)
                            | TdsEnums.DENALI_MINOR,
                        _physicalStateObj
                    );
                }
                else
                ...
                WriteInt(
                    TdsParserStaticMethods.GetCurrentProcessIdForTdsLoginOnly(),
                    _physicalStateObj
                ); //MDAC 84718
                ...
                */
                ...
                {
                    Bid.Trace(
                        "<sc.TdsParser.TdsLogin|ADV> %d#, TDS Login7 flags = %d:\n",
                        ObjectID,
                        log7Flags
                    );
                }
                ...
                )
                {
                    ...
                    _physicalStateObj._traceChangePasswordLength =
                        encryptedChangePasswordLengthInBytes;
                    if (rec.newSecurePassword != null)
                    {
        ...
        internal void SendFedAuthToken(SqlFedAuthToken fedAuthToken)
        {
            ...
            Debug.Assert(
                fedAuthToken.accessToken != null,
                "fedAuthToken.accessToken cannot be null"
            );
                        ...
                        else
                        {
                            ...
                            _physicalStateObj.AddError(
                                new SqlError(
                                    ...
                                    _server,
                                    Res.GetString(
                                        Res.SQL_ADALInitializeError,
                                        authentication.ToString("G"),
                                        result.ToString("X")
                                    ),
                                    TdsEnums.INIT_ADAL_PACKAGE,
                                    0
                                )
                            );
        ...
        internal byte[] GetDTCAddress(int timeout, TdsParserStateObject stateObj)
        {
            ...
            using (
                SqlDataReader dtcReader = TdsExecuteTransactionManagerRequest(
                    null,
            ...
            {
                Debug.Assert(
                    SniContext.Snix_Read == stateObj.SniContext,
                    String.Format(
                        (IFormatProvider)null,
                        "The SniContext should be Snix_Read but it actually is {0}",
                        stateObj.SniContext
                    )
                );
                ...
                {
                    Debug.Assert(
                        dtcReader.GetName(0) == "TM Address",
                        "TdsParser: GetDTCAddress did not return 'TM Address'"
                    );
                    ...
                    else
                    {
                        Debug.Assert(
                            false,
                            "unexpected length (> Int32.MaxValue) returned from dtcReader.GetBytes"
                        );
        ...
        internal void PropagateDistributedTransaction(
            byte[] buffer,
            int timeout,
            TdsParserStateObject stateObj
        )
        {
            ...
            TdsExecuteTransactionManagerRequest(
                buffer,
                TdsEnums.TransactionManagerRequestType.Propagate,
                null,
                TdsEnums.TransactionManagerIsolationLevel.Unspecified,
                timeout,
                null,
            ...
            try
            {
                ...
                switch (request)
                {
                    ...
                    case TdsEnums.TransactionManagerRequestType.Begin:
                        ...
                        {
                            Debug.Assert(
                                _currentTransaction == null || true == _fResetConnection,
                    ...
                    case TdsEnums.TransactionManagerRequestType.Commit:
                        ...
                        Debug.Assert(
                            ...
                            "Should not have a transaction name on Commit"
                        );
                ...
                if (
                    (
                        ...
                        || (request == TdsEnums.TransactionManagerRequestType.Propagate)
                    )
                    && (
                        (transaction == null)
                        || (transaction.TransactionId != _retainedTransactionId)
                    )
                )
                {
        ...
        internal void FailureCleanup(TdsParserStateObject stateObj, Exception e)
        {
            ...
            {
                Bid.Trace(
                    "<sc.TdsParser.FailureCleanup|ERR> Exception caught on ExecuteXXX: '%ls' \n",
                    e.ToString()
                );
            }
        ...
        )
        {
            ...
            bool needToTakeParserLock =
                (!callerHasConnectionLock) && (!_connHandler.ThreadHasParserLockForClose);
            Debug.Assert(
                !_connHandler.ThreadHasParserLockForClose || sync,
                "Thread shouldn't claim to have the parser lock if we are doing async writes"
            ); // Since we have the possibility of pending with async writes, make sure the thread doesn't claim to already have the lock
            Debug.Assert(
                needToTakeParserLock || _connHandler._parserLock.ThreadMayHaveLock(),
        ...
        )
        {
            ...
            Debug.Assert(
                !firstCall || !_connHandler.ThreadHasParserLockForClose,
                "Thread should not already have connection lock"
            );
            Debug.Assert(
                firstCall || _connHandler._parserLock.ThreadMayHaveLock(),
            ...
            try
            {
                ...
                try
                {
                    ...
                    for (int ii = startRpc; ii < rpcArray.Length; ii++)
                    {
                        ...
                        if (startParam == 0 || ii > startRpc)
                        {
                            if (rpcext.ProcID != 0 && _isShiloh)
                            {
                                ...
                                Debug.Assert(
                                    rpcext.ProcID < 255,
                                    "rpcExec:ProcID can't be larger than 255"
                                );
                        ...
                        for (int i = (ii == startRpc) ? startParam : 0; i < parameters.Length; i++)
                        {
                            ...
                            if (
                                ...
                                && !(
                                    ...
                                    || (
                                        cmd.ColumnEncryptionSetting
                                            == SqlCommandColumnEncryptionSetting.UseConnectionSetting
                                        && cmd.Connection.IsColumnEncryptionSettingEnabled
                                    )
                                )
                            )
                            {
                                throw SQL.ParamInvalidForceColumnEncryptionSetting(
                                    param.ParameterName,
                            ...
                            if (mt.IsNewKatmaiType)
                            {
                                WriteSmiParameter(
                                    param,
                                    i,
                                    0 != (rpcext.paramoptions[i] & TdsEnums.RPC_PARAM_DEFAULT),
                                    stateObj
                                );
                                continue;
                            }
                            ...
                            int actualSize;
                            int size = mt.IsSizeInCharacters
                                ...
                                if (!isNull)
                                {
                                    if (isSqlVal)
                                    {
                                        ...
                                        if (precision != 0)
                                        {
                                            ...
                                            {
                                                throw ADP.ParameterValueOutOfRange(
                                                    (SqlDecimal)value
                                                );
                                            }
                                        }
                            ...
                            bool isParameterEncrypted =
                                0 != (rpcext.paramoptions[i] & TdsEnums.RPC_PARAM_ENCRYPTED);

                            // Additional information we need to send over wire to the server when writing encrypted parameters.
                            SqlColumnEncryptionInputParameterInfo encryptedParameterInfoToWrite =
                                null;
                            ...
                            {
                                Debug.Assert(
                                    mt.TDSType != TdsEnums.SQLVARIANT
                                ...
                                if (!isNull)
                                {
                                    try
                                    {
                                        if (isSqlVal)
                                        {
                                            serializedValue = SerializeUnencryptedSqlValue(
                                                value,
                                                mt,
                                                actualSize,
                                                param.Offset,
                                        ...
                                        else
                                        {
                                            ...
                                            serializedValue = SerializeUnencryptedValue(
                                                value,
                                                mt,
                                        ...
                                        Debug.Assert(
                                            ...
                                            "serializedValue should not be null in TdsExecuteRPC."
                                        );
                                        encryptedValue = SqlSecurityUtility.EncryptWithKey(
                                            serializedValue,
                                    ...
                                    {
                                        throw SQL.ParamEncryptionFailed(
                                            param.ParameterName,
                                            null,
                                            e
                                        );
                                ...
                                encryptedParameterInfoToWrite =
                                    new SqlColumnEncryptionInputParameterInfo(
                                        param.GetMetadataForTypeInfo(),
                                        param.CipherMetadata
                                    );
                            ...
                            if (mt.IsAnsiType)
                            {
                                ...
                                if ((!isNull) && (!isDataFeed))
                                {
                                    ...
                                    codePageByteSize = GetEncodingCharLength(
                                        s,
                                        actualSize,
                                        param.Offset,
                                        _defaultEncoding
                            ...
                            else
                            {
                                ...
                                else if (mt.SqlDbType == SqlDbType.Udt)
                                {
                                    ...
                                    String[] names = SqlParameter.ParseTypeName(
                                        param.UdtTypeName,
                                        true /* is UdtTypeName */
                                    );
                                    if (
                                        !ADP.IsEmpty(names[0])
                                        && TdsEnums.MAX_SERVERNAME < names[0].Length
                                    )
                                    {
                                    ...
                                    if (
                                        !ADP.IsEmpty(names[1])
                                        && TdsEnums.MAX_SERVERNAME < names[names.Length - 2].Length
                                    )
                                    {
                                        throw ADP.ArgumentOutOfRange("names");
                                    }
                            ...
                            if (_isYukon && (mt.SqlDbType == SqlDbType.Xml))
                            {
                                if (
                                    (
                                        (param.XmlSchemaCollectionDatabase != null)
                                        && (param.XmlSchemaCollectionDatabase != ADP.StrEmpty)
                                    )
                                    || (
                                        (param.XmlSchemaCollectionOwningSchema != null)
                                        && (param.XmlSchemaCollectionOwningSchema != ADP.StrEmpty)
                                    )
                                    || (
                                        (param.XmlSchemaCollectionName != null)
                                        && (param.XmlSchemaCollectionName != ADP.StrEmpty)
                                    )
                                )
                                {
                            ...
                            if (!isNull)
                            {
                                ...
                                else
                                {
                                    // for codePageEncoded types, WriteValue simply expects the number of characters
                                    // For plp types, we also need the encoded byte size
                                    writeParamTask = WriteValue(
                                        value,
                                        mt,
                                        isParameterEncrypted ? (byte)0 : param.GetActualScale(),
                                        actualSize,
                                        codePageByteSize,
                                        isParameterEncrypted ? 0 : param.Offset,
                                        stateObj,
                                        isParameterEncrypted ? 0 : param.Size,
                                        isDataFeed
                                    );
                                }
                            }
                            ...
                            if (isParameterEncrypted)
                            {
                                writeParamTask = WriteEncryptionMetadata(
                                    writeParamTask,
                                    encryptedParameterInfoToWrite,
                                    stateObj
                                );
                            ...
                            else
                            {
                                Debug.Assert(
                                    writeParamTask == null,
                                    "Should not have a task when executing sync"
                                );
                            }
#endif
                        } // parameter for loop
                    ...
                    Debug.Assert(
                        !sync || execFlushTask == null,
                        "Should not get a task when executing sync"
                    );
            ...
            {
                Debug.Assert(
                    firstCall || !releaseConnectionLock,
        ...
        private void WriteParameterName(string parameterName, TdsParserStateObject stateObj)
        {
            ...
            {
                Debug.Assert(
                    parameterName.Length <= 0xff,
                    "parameter name can only be 255 bytes, shouldn't get to TdsParser!"
                );
        ...
        private static readonly IEnumerable<MSS.SqlDataRecord> __tvpEmptyValue =
            new List<MSS.SqlDataRecord>().AsReadOnly();

        private void WriteSmiParameter(
            SqlParameter param,
            int paramIndex,
            bool sendDefault,
            TdsParserStateObject stateObj
        )
        {
            ...
            if (!_isKatmai)
            {
                MetaType mt = MetaType.GetMetaTypeFromSqlDbType(
                    metaData.SqlDbType,
        ...
        private void WriteSmiParameterMetaData(
            MSS.SmiParameterMetaData metaData,
            bool sendDefault,
            TdsParserStateObject stateObj
        )
        {
            ...
            byte status = 0;
            if (
                ParameterDirection.Output == metaData.Direction
                || ParameterDirection.InputOutput == metaData.Direction
            )
            {
        ...
        )
        {
            switch (metaData.SqlDbType)
            {
                ...
                case SqlDbType.Image:
                    ...
                    WriteUnsignedShort(
                        unchecked((ushort)MSS.SmiMetaData.UnlimitedMaxLengthIndicator),
                        stateObj
                    );
                    break;
                case SqlDbType.Int:
                ...
                case SqlDbType.Xml:
                    ...
                    // Is there a schema
                    if (
                        ADP.IsEmpty(metaData.TypeSpecificNamePart1)
                        && ADP.IsEmpty(metaData.TypeSpecificNamePart2)
        ...
        private void WriteTvpTypeInfo(
            MSS.SmiExtendedMetaData metaData,
            TdsParserStateObject stateObj
        )
        {
            Debug.Assert(
                SqlDbType.Structured == metaData.SqlDbType && metaData.IsMultiValued,
            ...
            else
            {
                ...
                MSS.SmiDefaultFieldsProperty defaults = (MSS.SmiDefaultFieldsProperty)
                    metaData.ExtendedProperties[MSS.SmiPropertySelector.DefaultFields];
        ...
        private void WriteTvpColumnMetaData(
            MSS.SmiExtendedMetaData md,
            bool isDefault,
            TdsParserStateObject stateObj
        )
        {
        ...
        )
        {
            ...
            MSS.SmiOrderProperty orderProperty = (MSS.SmiOrderProperty)
                metaData.ExtendedProperties[MSS.SmiPropertySelector.SortOrder];
            MSS.SmiUniqueKeyProperty uniqueKeyProperty = (MSS.SmiUniqueKeyProperty)
                metaData.ExtendedProperties[MSS.SmiPropertySelector.UniqueKey];

            // Build list from
            List<TdsOrderUnique> columnList = new List<TdsOrderUnique>(
        ...
        /// </summary>
        /// <returns></returns>
        internal void WriteEncryptionEntries(
            ref SqlTceCipherInfoTable cekTable,
            TdsParserStateObject stateObj
        )
        {
        ...
        )
        {
            ...
            if (!metadataCollection.cekTable.HasValue || !ShouldEncryptValuesForBulkCopy())
            {
        ...
        /// </summary>
        /// <returns></returns>
        internal void WriteTceUserTypeAndTypeInfo(
            SqlMetaDataPriv mdPriv,
            TdsParserStateObject stateObj
        )
        {
                ...
                if (metadataCollection[i] != null)
                {
                    ...
                    { // TCE Supported
                        ...
                        { // TCE enabled on connection options
                            flags |= (UInt16)(
                                md.isEncrypted ? (UInt16)(TdsEnums.IsEncrypted << 8) : (UInt16)0
                            );
                        }
                    }
                    ...
                    switch (md.type)
                    {
                        ...
                        case SqlDbType.Xml:
                            //
                            stateObj.WriteByteArray(
                                s_xmlMetadataSubstituteSequence,
                                s_xmlMetadataSubstituteSequence.Length,
                                0
                            );
                            break;
                        case SqlDbType.Udt:
        ...
        )
        {
            ...
            Debug.Assert(
                ShouldEncryptValuesForBulkCopy(),
                "Encryption attempted when not requested"
            );
            ...
            switch (metadata.baseTI.metaType.NullableType)
            {
                ...
                case TdsEnums.SQLIMAGE:
                    ...
                    actualLengthInBytes =
                        (isSqlType) ? ((SqlBinary)value).Length : ((byte[])value).Length;
                    if (metadata.baseTI.length > 0 && actualLengthInBytes > metadata.baseTI.length)
                    { // see comments agove
                ...
                case TdsEnums.SQLNTEXT:
                    actualLengthInBytes =
                        ((isSqlType) ? ((SqlString)value).Value.Length : ((string)value).Length)
                        * 2;
        ...
        )
        {
            ...
            try
            {
                ...
                else
                {
                    Debug.Assert(
                        metatype.IsLong
                            && (
                                (
                                    metatype.SqlDbType == SqlDbType.VarBinary
                                    && value is StreamDataFeed
                                )
                                || (
                                    (
                                        metatype.SqlDbType == SqlDbType.VarChar
                                        || metatype.SqlDbType == SqlDbType.NVarChar
                                    )
                ...
                if (metatype.IsLong)
                {
                    switch (metatype.SqlDbType)
                    {
                        ...
                        case SqlDbType.Image:
                            ...
                            WriteTokenLength(
                                metadata.tdsType,
                                ccbStringBytes == 0 ? ccb : ccbStringBytes,
                                stateObj
                            );
                            break;
                ...
                if (isSqlType)
                {
                    internalWriteTask = WriteSqlValue(
                        value,
                        metatype,
                        ccb,
                        ccbStringBytes,
                ...
                else if (metatype.SqlDbType != SqlDbType.Udt || metatype.IsLong)
                {
                    ...
                    Debug.Assert(
                        _asyncWrite || stateObj.WaitForAccumulatedWrites() == null,
                        "Should not have accumulated writes when writing sync"
                    );
                }
                else
                {
                ...
                if (
                    ...
                    && SqlBulkCopy.SetAlwaysTaskOnWrite == true
                )
                {
                ...
                { //i.e. the write was async.
                    resultTask = WriteBulkCopyValueSetupContinuation(
                        internalWriteTask,
                        saveEncoding,
                        saveCollation,
                        saveCodePage,
        ...
        private Task WriteBulkCopyValueSetupContinuation(
            Task internalWriteTask,
        ...
        )
        {
            ...
            if (
                null != transaction
                && SqlInternalTransaction.NullTransactionId != transaction.TransactionId
            )
            {
        ...
        )
        {
            ...
            Bid.NotificationsTrace(
                "<sc.TdsParser.WriteQueryNotificationHeader|DEP> NotificationRequest: userData: '%ls', options: '%ls', timeout: '%d'\n",
        ...
        private void WriteTraceHeaderData(TdsParserStateObject stateObj)
        {
            ...
            Bid.Trace(
                "<sc.TdsParser.WriteTraceHeaderData|INFO> ActivityID %ls\n",
                actId.ToString()
            );
        }
        ...
        )
        {
            ...
            */
            ...
            // TotalLength  - DWORD  - including all headers and lengths, including itself
            int totalHeaderLength = this.IncludeTraceHeader
                ? (4 + marsHeaderSize + notificationHeaderSize + traceHeaderSize)
                : (4 + marsHeaderSize + notificationHeaderSize);
            Debug.Assert(
                stateObj._outBytesUsed == stateObj._outputHeaderLen,
                "Output bytes written before total header length"
            );
        ...
        private void WriteTokenLength(byte token, int length, TdsParserStateObject stateObj)
        {
            ...
            if (tokenLength == 0)
            {
                switch (token & TdsEnums.SQLLenMask)
                {
                    case TdsEnums.SQLFixedLen:
                        Debug.Assert(
                            length == 0x01 << ((token & 0x0c) >> 2),
                            "length does not match encoded length in token"
                        );
                        tokenLength = 0;
                        break;
        ...
        private Task GetTerminationTask(
            Task unterminatedWriteTask,
            object value,
            MetaType type,
            int actualLength,
            TdsParserStateObject stateObj,
        ...
        private Task WriteUnterminatedSqlValue(
            object value,
            MetaType type,
            int actualLength,
            int codePageByteSize,
        ...
        )
        {
            ...
            switch (type.NullableType)
            {
                ...
                case TdsEnums.SQLUNIQUEID:
                {
                    ...
                    Debug.Assert(
                        (actualLength == b.Length) && (actualLength == 16),
                        "Invalid length for guid type in com+ object"
                    );
                ...
                case TdsEnums.SQLINTN:
                    ...
                    else
                    {
                        Debug.Assert(
                            type.FixedLength == 8,
                            "invalid length for SqlIntN type:  "
                                + type.FixedLength.ToString(CultureInfo.InvariantCulture)
                        );
                ...
                case TdsEnums.SQLNUMERICN:
                    Debug.Assert(
                        type.FixedLength <= 17,
                        "Decimal length cannot be greater than 17 bytes"
                    );
                ...
                case TdsEnums.SQLUDT:
                    Debug.Assert(
                        false,
                        "Called WriteSqlValue on UDT param.Should have already been handled"
                    );
                ...
                default:
                    Debug.Assert(
                        false,
                        "Unknown TdsType!" + type.NullableType.ToString("x2", (IFormatProvider)null)
                    );
                    break;
            } // switch
            // return point for accumualated writes, note: non-accumulated writes returned from their case
        ...
        private class ConstrainedTextWriter : TextWriter
        {
            ...
            {
                get { return _size > 0 && _written >= _size; }
            }
            ...
            public override void Write(char value)
            {
                ...
                Debug.Assert(
                    _size < 0 || _written <= _size,
                    string.Format(
                        "Length of data written exceeds specified length.  Written: {0}, specified: {1}",
                        _written,
                        _size
                    )
                );
        ...
        private async Task WriteXmlFeed(
            XmlDataFeed feed,
            TdsParserStateObject stateObj,
            bool needBom,
            Encoding encoding,
        ...
        )
        {
            ...
            ConstrainedTextWriter writer = new ConstrainedTextWriter(
                new StreamWriter(new TdsOutputStream(this, stateObj, preambleToSkip), encoding),
                size
            );
        ...
        private async Task WriteTextFeed(
            TextDataFeed feed,
            Encoding encoding,
            bool needBom,
            TdsParserStateObject stateObj,
        ...
        )
        {
            ...
            ConstrainedTextWriter writer = new ConstrainedTextWriter(
                new StreamWriter(new TdsOutputStream(this, stateObj, null), encoding),
                size
            );
        ...
        private async Task WriteStreamFeed(
            StreamDataFeed feed,
            TdsParserStateObject stateObj,
            int len
        )
        ...
        // For MAX types, this method can only write everything in one big chunk. If multiple
        // chunk writes needed, please use WritePlpBytes/WritePlpChars
        private Task WriteUnterminatedValue(
            object value,
            MetaType type,
            byte scale,
            int actualLength,
        ...
        {
            Debug.Assert(
                (null != value) && (DBNull.Value != value),
                "unexpected missing or empty object"
            );
            ...
            switch (type.NullableType)
            {
                ...
                case TdsEnums.SQLUDT:
                {
                    ...
                    Debug.Assert(
                        isDataFeed || value is byte[],
                        "Value should be an array of bytes"
                    );
                    Debug.Assert(
                        !isDataFeed || value is StreamDataFeed,
                    ...
                    if (isDataFeed)
                    {
                        ...
                        return NullIfCompletedWriteTask(
                            WriteStreamFeed((StreamDataFeed)value, stateObj, paramSize)
                        );
                    }
                    else
                    {
                ...
                {
                    Debug.Assert(
                        !isDataFeed || (value is TextDataFeed || value is XmlDataFeed),
                        "Value must be a TextReader or XmlReader"
                    );
                    Debug.Assert(
                        isDataFeed || (value is string || value is byte[]),
                    ...
                    if (isDataFeed)
                    {
                        ...
                        {
                            return NullIfCompletedWriteTask(
                                WriteTextFeed(tdf, _defaultEncoding, false, stateObj, paramSize)
                            );
                        }
                    }
                    else
        ...
        /// </summary>
        private Task WriteEncryptionMetadata(
            Task terminatedWriteTask,
            SqlColumnEncryptionInputParameterInfo columnEncryptionParameterInfo,
            TdsParserStateObject stateObj
        )
        {
            Debug.Assert(
                columnEncryptionParameterInfo != null,
                @"columnEncryptionParameterInfo cannot be null"
            );
        ...
        /// </summary>
        private void WriteEncryptionMetadata(
            SqlColumnEncryptionInputParameterInfo columnEncryptionParameterInfo,
            TdsParserStateObject stateObj
        )
        {
            ...
            stateObj.WriteByteArray(
                columnEncryptionParameterInfo.SerializedWireFormat,
        ...
        // For MAX types, this method can only write everything in one big chunk. If multiple
        // chunk writes needed, please use WritePlpBytes/WritePlpChars
        private byte[] SerializeUnencryptedValue(
            object value,
            MetaType type,
            byte scale,
            int actualLength,
        ...
        // For MAX types, this method can only write everything in one big chunk. If multiple
        // chunk writes needed, please use WritePlpBytes/WritePlpChars
        private byte[] SerializeUnencryptedSqlValue(
            object value,
            MetaType type,
            int actualLength,
            int offset,
            ...
            switch (type.NullableType)
            {
                ...
                case TdsEnums.SQLDATETIMN:
                    ...
                    if (type.FixedLength == 4)
                    {
                        ...
                        bPart = SerializeShort(
                            dt.TimeTicks / SqlDateTime.SQLTicksPerMinute,
                            stateObj
                        );
                        Buffer.BlockCopy(bPart, 0, b, current, 2);
        ...
        )
        {
            ...
            else
            {
                ...
                {
                    Debug.Assert(
                        type.FixedLength <= 0xff,
                        "WriteParameterVarLen: invalid one byte length!"
                    );
        ...
        private bool TryReadPlpUnicodeCharsChunk(
            char[] buff,
            int offst,
            int len,
            TdsParserStateObject stateObj,
        ...
        {
            Debug.Assert(
                (buff == null && len == 0) || (buff.Length >= offst + len),
                "Invalid length sent to ReadPlpUnicodeChars()!"
            );
            Debug.Assert(
                (stateObj._longlen != 0) && (stateObj._longlen != TdsEnums.SQL_PLP_NULL),
        ...
        // Returns the actual chars read.
        internal bool TryReadPlpUnicodeChars(
            ref char[] buff,
            int offst,
            int len,
            TdsParserStateObject stateObj,
        ...
        )
        {
            ...
            Debug.Assert(
                ((ulong)stateObj._longlen != TdsEnums.SQL_PLP_NULL),
                "Out of sync plp read request"
            );

            Debug.Assert(
                (buff == null && offst == 0) || (buff.Length >= offst + len),
                "Invalid length sent to ReadPlpUnicodeChars()!"
            );
            charsLeft = len;
            ...
            while (charsLeft > 0)
            {
                ...
                if (stateObj._longlenleft == 1 && (charsLeft > 0))
                {
                    ...
                    Debug.Assert(
                        ...
                        "ReadPlpUnicodeChars: Odd byte left at the end!"
                    );
                    byte b2;
        ...
        )
        {
            ...
            while ((totalBytesSkipped < cb) && (stateObj._longlenleft > 0))
            {
                ...
                bytesSkipped =
                    ((cb - totalBytesSkipped) < (ulong)bytesSkipped)
                        ? (int)(cb - totalBytesSkipped)
                        : bytesSkipped;
        ...
        {
            return String.Format( /*IFormatProvider*/
                null,
                ...
                null == _connHandler
                    ? "(null)"
                    : _connHandler.ObjectID.ToString((IFormatProvider)null),
                _fMARS,
                ...
                null == _sniSpnBuffer
                    ? "(null)"
                    : _sniSpnBuffer.Length.ToString((IFormatProvider)null),
                _physicalStateObj != null
                    ? "(null)"
                    : _physicalStateObj.ErrorCount.ToString((IFormatProvider)null),
                _physicalStateObj != null
                    ? "(null)"
                    : _physicalStateObj.WarningCount.ToString((IFormatProvider)null),
                _physicalStateObj != null
                    ? "(null)"
                    : _physicalStateObj.PreAttentionErrorCount.ToString((IFormatProvider)null),
                _physicalStateObj != null
                    ? "(null)"
                    : _physicalStateObj.PreAttentionWarningCount.ToString((IFormatProvider)null),
                null == _statistics,
