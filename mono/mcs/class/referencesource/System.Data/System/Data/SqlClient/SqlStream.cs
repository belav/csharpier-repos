        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            if ((offset < 0) || (count < 0))
            {
                throw ADP.ArgumentOutOfRange(
                    String.Empty,
                    (offset < 0 ? ADP.ParameterOffset : ADP.ParameterCount)
                );
            }
            ...
            if (_bom > 0)
            {
                ...
                if (
                    (cBufferedData < 2)
                    || ((_bufferedData[0] == 0xDF) && (_bufferedData[1] == 0xFF))
                )
                {
                    _bom = 0;
                }
        ...
        public long GetChars(long dataIndex, char[] buffer, int bufferIndex, int length)
        {
            if (_xmlReader == null)
            {
                SqlStream sqlStream = new SqlStream(
                    _columnOrdinal,
                    _reader,
                    true /* addByteOrderMark */
                    ,
                    false /* processAllRows*/
                    ,
                    false /*advanceReader*/
                );
            ...
            switch (_xmlReader.NodeType)
            {
                ...
                case XmlNodeType.Text:
                    if (canReadChunk)
                    {
                        ...
                        while (
                            (
                                read = _xmlReader.ReadValueChunk(
                                    writeNodeBuffer,
                                    0,
                                    WriteNodeBufferSize
                                )
                ...
                case XmlNodeType.DocumentType:
                    _xmlWriter.WriteDocType(
                        _xmlReader.Name,
                        _xmlReader.GetAttribute("PUBLIC"),
                        _xmlReader.GetAttribute("SYSTEM"),
                        _xmlReader.Value
                    );
                    break;
