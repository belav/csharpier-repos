...
namespace System.Data
{
    ...
    internal static class LocalDBAPI
    {
                    ...
                    try
                    {
                        ...
                        if (s_userInstanceDLLHandle == IntPtr.Zero)
                        {
                            SNINativeMethodWrapper.SNIQueryInfo(
                                SNINativeMethodWrapper.QTypes.SNI_QUERY_LOCALDB_HMODULE,
                                ref s_userInstanceDLLHandle
                            );
                            if (s_userInstanceDLLHandle != IntPtr.Zero)
                            {
                                Bid.Trace(
                                    "<sc.LocalDBAPI.UserInstanceDLLHandle> LocalDB - handle obtained"
                                );
                            }
                            else
                            {
                                SNINativeMethodWrapper.SNI_Error sniError =
                                    new SNINativeMethodWrapper.SNI_Error();
                                SNINativeMethodWrapper.SNIGetLastError(sniError);
                                throw CreateLocalDBException(
                                    errorMessage: Res.GetString("LocalDB_FailedGetDLLHandle"),
                                    sniError: (int)sniError.sniError
                                );
                            }
                        }
                    }
                    ...
                    try
                    {
                        ...
                        if (s_localDBCreateInstance == null)
                        {
                            ...
                            if (functionAddr == IntPtr.Zero)
                            {
                                ...
                                Bid.Trace(
                                    "<sc.LocalDBAPI.LocalDBCreateInstance> GetProcAddress for LocalDBCreateInstance error 0x{%X}",
                                    hResult
                                );
                                throw CreateLocalDBException(
                                    errorMessage: Res.GetString("LocalDB_MethodNotFound")
                                );
                            }
                            s_localDBCreateInstance = (LocalDBCreateInstanceDelegate)
                                Marshal.GetDelegateForFunctionPointer(
                                    functionAddr,
                                    typeof(LocalDBCreateInstanceDelegate)
                                );
                        }
                    }
                    finally
                ...
                if (s_localDBFormatMessage == null)
                {
                    ...
                    try
                    {
                        ...
                        if (s_localDBFormatMessage == null)
                        {
                            ...
                            s_localDBFormatMessage = (LocalDBFormatMessageDelegate)
                                Marshal.GetDelegateForFunctionPointer(
                                    functionAddr,
                                    typeof(LocalDBFormatMessageDelegate)
                                );
                        }
                    }
                    finally
        ...
        internal static string GetLocalDBMessage(int hrCode)
        {
            ...
            try
            {
                ...
                int hResult = LocalDBFormatMessage(
                    hrLocalDB: hrCode,
                    dwFlags: const_LOCALDB_TRUNCATE_ERR_MESSAGE,
        ...
        static SqlException CreateLocalDBException(
            string errorMessage,
        ...
        {
            Debug.Assert(
                (localDbError == 0) || (sniError == 0),
                "LocalDB error and SNI error cannot be specified simultaneously"
            );
            Debug.Assert(
            ...
            if (localDbError != 0)
                collection.Add(
                    new SqlError(
                        ...
                        TdsEnums.FATAL_ERROR_CLASS,
                        instance,
                        GetLocalDBMessage(localDbError),
                        null,
                        0
                    )
                );
        ...
        internal static void DemandLocalDBPermissions()
        {
            if (!_partialTrustAllowed)
            {
                ...
                {
                    object partialTrustFlagValue = AppDomain.CurrentDomain.GetData(
                        const_partialTrustFlagKey
                    );
        ...
        internal static void CreateLocalDBInstance(string instance)
        {
            ...
            if (s_configurableInstances == null)
            {
                ...
                try
                {
                    ...
                    if (s_configurableInstances == null)
                    {
                        Dictionary<string, InstanceInfo> tempConfigurableInstances = new Dictionary<
                            string,
                            InstanceInfo
                        >(StringComparer.OrdinalIgnoreCase);
#if !NO_CONFIGURATION
                        object section = PrivilegedConfigurationManager.GetSection(
                            "system.data.localdb"
                        );
                        if (section != null) // if no section just skip creation
                        {
                            // validate section type
                            LocalDBConfigurationSection configSection =
                                section as LocalDBConfigurationSection;
                            if (configSection == null)
                                throw CreateLocalDBException(
                                    errorMessage: Res.GetString("LocalDB_BadConfigSectionType")
                                );
                            foreach (
                                LocalDBInstanceElement confElement in configSection.LocalDbInstances
                            )
                            {
                                Debug.Assert(
                                    confElement.Name != null && confElement.Version != null,
                                    "Both name and version should not be null"
                                );
            ...
            if (instanceInfo.version.Contains("\0"))
                throw CreateLocalDBException(
                    errorMessage: Res.GetString("LocalDB_InvalidVersion"),
                    instance: instance
                );
            ...
            if (hr < 0)
                throw CreateLocalDBException(
                    errorMessage: Res.GetString("LocalDB_CreateFailed"),
                    instance: instance,
                    localDbError: hr
                );
            Bid.Trace(
                "<sc.LocalDBAPI.CreateLocalDBInstance> Finished creation of instance %ls",
                instance
            );
