...
{ // MBR since ref'ed by other appdomains.
    ...
    private class SqlConnectionContainer
    {
        ...
        internal SqlConnectionContainer(
            SqlConnectionContainerHashHelper hashHelper,
            string appDomainKey,
            bool useDefaults
        )
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlConnectionContainer|DEP> %d#, queue: '%ls'",
                ObjectID,
                hashHelper.Queue
            );
            ...
            try
            {
...
#if DEBUG
                SqlConnectionString connectionStringOptions = new SqlConnectionString(
                    _hashHelper.ConnectionStringBuilder.ConnectionString
                );
                Bid.NotificationsTrace(
                    "<sc.SqlConnectionContainer|DEP> Modified connection string: '%ls'\n",
                ...
                { // Only create if user did not specify service & database.
                    _sprocName = SqlConnection.FixupDatabaseTransactionName(
                        SQL.SqlNotificationStoredProcedureDefault + "-" + guid
                    );
        ...
        internal bool AppDomainUnload(string appDomainKey)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlConnectionContainer.AppDomainUnload|DEP> %d#, AppDomainKey: '%ls'",
                ObjectID,
                appDomainKey
            );
            try
            {
                ...
                lock (_appDomainKeyHash)
                {
                    ...
                    { // Do nothing if AppDomain did not call Start!
                        Bid.NotificationsTrace(
                            "<sc.SqlConnectionContainer.AppDomainUnload|DEP> _appDomainKeyHash contained AppDomainKey: '%ls'.\n",
                            appDomainKey
                        );
                        ...
                        Debug.Assert(
                            0 == value,
                            "We did not reach 0 at end of loop in AppDomainUnload!"
                        );
                        Debug.Assert(
                            !_appDomainKeyHash.ContainsKey(appDomainKey),
                            "Key not removed after AppDomainUnload!"
                        );

                        if (_appDomainKeyHash.ContainsKey(appDomainKey))
                        {
                            Bid.NotificationsTrace(
                                "<sc.SqlConnectionContainer.AppDomainUnload|DEP|ERR> ERROR - after the Stop() loop, _appDomainKeyHash for AppDomainKey: '%ls' entry not removed from hash.  Count: %d'\n",
                                appDomainKey,
                                _appDomainKeyHash[appDomainKey]
                            );
                        }
                    }
                    else
        ...
        private void AsynchronouslyQueryServiceBrokerQueue()
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlConnectionContainer.AsynchronouslyQueryServiceBrokerQueue|DEP> %d#",
                ObjectID
            );
            try
            {
        ...
        private void AsyncResultCallback(IAsyncResult asyncResult)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlConnectionContainer.AsyncResultCallback|DEP> %d#",
                ObjectID
            );
            try
            {
        ...
        private void CreateQueueAndService(bool restart)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlConnectionContainer.CreateQueueAndService|DEP> %d#",
                ObjectID
            );
            try
            {
                ...
                try
                {
                    ...
                    com.CommandText =
                        ...
                        + " BEGIN"
                        + " if ((SELECT COUNT(*) FROM sys.services WHERE name = "
                        + nameLiteral
                        + ") > 0)"
                        + "   DROP SERVICE "
                        + _escapedQueueName
        ...
        internal void IncrementStartCount(string appDomainKey, out bool appDomainStart)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlConnectionContainer.IncrementStartCount|DEP> %d#",
                ObjectID
            );
            try
            {
                ...
                Bid.NotificationsTrace(
                    "<sc.SqlConnectionContainer.IncrementStartCount|DEP> %d#, incremented _startCount: %d\n",
                    _staticInstance.ObjectID,
                    result
                );
        ...
        private void ProcessNotificationResults(SqlDataReader reader)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlConnectionContainer.ProcessNotificationResults|DEP> %d#",
                ObjectID
            );
            try
            {
                ...
                try
                {
                    if (!_stop)
                    {
                        while (reader.Read())
                        {
                            ...
                            {
                                Bid.NotificationsTrace(
                                    "<sc.SqlConnectionContainer.ProcessNotificationResults|DEP> column: %ls, value: %ls\n",
                                    reader.GetName(i),
                            ...
                            )
                            {
                                ...
                                if (null != payload)
                                {
                                    SqlNotification notification =
                                        SqlNotificationParser.ProcessMessage(payload);
                                    if (null != notification)
                                    {
                                        string key = notification.Key;
                                        Bid.NotificationsTrace(
                                            "<sc.SqlConnectionContainer.ProcessNotificationResults|DEP> Key: '%ls'\n",
                                            key
                                        );
                                        ...
                                        { // Ensure ';' present.
                                            ...
                                            else
                                            {
                                                Debug.Assert(
                                                    false,
                                                    "Received notification but do not have an associated PerAppDomainDispatcher!"
                                                );
                                                Bid.NotificationsTrace(
                ...
                finally
                {
                    ...
                    { // This should only happen if failure occurred, or if non-QN format received.
                        _com.CommandText =
                            (null != _beginConversationQuery)
                                ? _beginConversationQuery
                                : _receiveQuery; // If we're doing the initial query, we won't have a conversation Guid to begin yet.
        ...
        { // Unused arg required by TimerCallback.
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlConnectionContainer.Restart|DEP> %d#",
                ObjectID
            );
            try
            {
                ...
                catch (Exception e)
                {
                    ...
                    if (!_stop)
                    {
                        ...
                        _retryTimer = new Timer(
                            new TimerCallback(Restart),
                            null,
                            _defaultWaitforTimeout,
                            Timeout.Infinite
                        );
        ...
        internal bool Stop(string appDomainKey, out bool appDomainStop)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlConnectionContainer.Stop|DEP> %d#",
                ObjectID
            );
            try
            {
                ...
                if (null != appDomainKey)
                {
                    ...
                    lock (_appDomainKeyHash)
                    {
                        ...
                        { // Do nothing if AppDomain did not call Start!
                            ...
                            Bid.NotificationsTrace(
                                "<sc.SqlConnectionContainer.Stop|DEP> _appDomainKeyHash contained AppDomainKey: '%ls', pre-decrement Count: '%d'.\n",
                                appDomainKey,
                                value
                            );
                            if (value > 0)
                        ...
                        else
                        {
                            ...
                            Debug.Assert(
                                false,
                                "Unexpected state on Stop() - no AppDomainKey entry in hashtable!"
                            );
                        }
                    }
                }
        ...
        private void SynchronouslyQueryServiceBrokerQueue()
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlConnectionContainer.SynchronouslyQueryServiceBrokerQueue|DEP> %d#",
                ObjectID
            );
            try
            {
        ...
        private void TearDownAndDispose()
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlConnectionContainer.TearDownAndDispose|DEP> %d#",
                ObjectID
            );
            try
            {
                ...
                { // Lock to ensure Stop() (with Cancel()) complete prior to TearDown.
                    try
                    {
                        ...
                        if (
                            ConnectionState.Closed != _con.State
                            && ConnectionState.Broken != _con.State
                        )
                        {
                            if (_com.Parameters.Count > 1)
        ...
        internal static SqlNotification ProcessMessage(SqlXml xmlMessage)
        {
            using (XmlReader xmlReader = xmlMessage.CreateReader())
            {
                ...
                )
                {
                    ...
                    while (
                        (MessageAttributes.All != messageAttributes)
                    ...
                    )
                    {
                        try
                        {
                            switch (xmlReader.LocalName)
                            {
                                case TypeAttribute:
                                    try
                                    {
                                        SqlNotificationType temp = (SqlNotificationType)
                                            Enum.Parse(
                                                typeof(SqlNotificationType),
                                                xmlReader.Value,
                                                true
                                            );
                                ...
                                case SourceAttribute:
                                    try
                                    {
                                        SqlNotificationSource temp = (SqlNotificationSource)
                                            Enum.Parse(
                                                typeof(SqlNotificationSource),
                                                xmlReader.Value,
                                                true
                                            );
                                    ...
                                    try
                                    {
                                        ...
                                        switch (value)
                                        {
                                            ...
                                            case "query template limit":
                                                ...
                                                break;
                                            default:
                                                SqlNotificationInfo temp = (SqlNotificationInfo)
                                                    Enum.Parse(
                                                        typeof(SqlNotificationInfo),
                                                        value,
                                                        true
                                                    );
                                                if (
                        ...
                        catch (ArgumentException e)
                        {
                            ...
                            Bid.Trace(
                                "<sc.SqlDependencyProcessDispatcher.ProcessMessage|DEP|ERR> Exception thrown - Enum.Parse failed to parse the value '%ls' of the attribute '%ls'.\n",
                                xmlReader.Value,
                                xmlReader.LocalName
                            );
                            return null;
                    ...
                    using (
                        XmlTextReader xmlMessageReader = new XmlTextReader(
                            xmlReader.Value,
                            XmlNodeType.Element,
                            null
                        )
    ...
    private class SqlConnectionContainerHashHelper
    {
        ...
        */
        ...
        public override bool Equals(object value)
        {
            ...
            else
            {
                if (
                    ...
                    || // If XOR of null identities false - false.
                    (_identity == null && temp._identity != null)
                )
                {
                    result = false;
                ...
                {
                    if (
                        temp._identity.Equals(_identity)
                        && temp._connectionString == _connectionString
                        && String.Equals(temp._queue, _queue, StringComparison.OrdinalIgnoreCase)
                    )
                    {
        ...
        public override int GetHashCode()
        {
            ...
            {
                hashValue = unchecked(
                    _connectionString.GetHashCode() + _queue.GetHashCode() + hashValue
                );
            }
            else
            {
    ...
    private static SqlDependencyProcessDispatcher _staticInstance =
        new SqlDependencyProcessDispatcher(null);
    ...
    {
        Debug.Assert(
            null == _staticInstance,
            "Real constructor called with static instance already created!"
        );
        IntPtr hscp;
        Bid.NotificationsScopeEnter(
            out hscp,
            "<sc.SqlDependencyProcessDispatcher|DEP> %d#",
            ObjectID
        );
        try
        {
            ...
            Bid.NotificationsTrace(
                "<sc.SqlDependencyProcessDispatcher|DEP> %d#, AppDomain.CurrentDomain.FriendlyName: %ls\n",
                ObjectID,
            ...
            _sqlDependencyPerAppDomainDispatchers =
                new Dictionary<string, SqlDependencyPerAppDomainDispatcher>();
        }
        finally
        {
            Bid.ScopeLeave(ref hscp);
    ...
    )
    {
        ...
        Bid.NotificationsScopeEnter(
            out hscp,
            "<sc.SqlDependencyProcessDispatcher.GetHashString|DEP> %d#, queue: %ls",
            _staticInstance.ObjectID,
            queue
        );
        try
        {
            ...
            return new SqlConnectionContainerHashHelper(
                identity,
    ...
    private void Invalidate(string server, SqlNotification sqlNotification)
    {
        ...
        Bid.NotificationsScopeEnter(
            out hscp,
            "<sc.SqlDependencyProcessDispatcher.Invalidate|DEP> %d#, server: %ls",
            ObjectID,
            server
        );
        try
        {
            Debug.Assert(
                ...
                "Instance method called on non _staticInstance instance!"
            );
    ...
    private void AppDomainUnloading(object state)
    {
        ...
        Bid.NotificationsScopeEnter(
            out hscp,
            "<sc.SqlDependencyProcessDispatcher.AppDomainUnloading|DEP> %d#",
            ObjectID
        );
        try
        {
            ...
            {
                List<SqlConnectionContainerHashHelper> containersToRemove =
                    new List<SqlConnectionContainerHashHelper>();
    ...
    )
    {
        ...
        Bid.NotificationsScopeEnter(
            out hscp,
            "<sc.SqlDependencyProcessDispatcher.Start|DEP> %d#, queue: '%ls', appDomainKey: '%ls', perAppDomainDispatcher ID: '%d'",
            ObjectID,
            queueService,
            appDomainKey,
            dispatcher.ObjectID
        ...
        try
        {
            ...
            SqlConnectionContainerHashHelper hashHelper = GetHashHelper(
                connectionString,
            ...
            Bid.NotificationsTrace(
                "<sc.SqlDependencyProcessDispatcher.Start|DEP> Modified connection string: '%ls'\n",
                connectionStringOptions.UsersConnectionStringForTrace()
            );
#endif
            bool started = false;
            ...
            lock (_connectionContainers)
            {
                ...
                {
                    Bid.NotificationsTrace(
                        "<sc.SqlDependencyProcessDispatcher.Start|DEP> %d#, hashtable miss, creating new container.\n",
                        ObjectID
                    );
    ...
    )
    {
        ...
        Bid.NotificationsScopeEnter(
            out hscp,
            "<sc.SqlDependencyProcessDispatcher.Stop|DEP> %d#, queue: '%ls'",
            ObjectID,
            queueService
        );
        try
        {
            ...
            Bid.NotificationsTrace(
                "<sc.SqlDependencyProcessDispatcher.Stop|DEP> Modified connection string: '%ls'\n",
                connectionStringOptions.UsersConnectionStringForTrace()
            );
#endif
            ...
            lock (_connectionContainers)
            {
                if (_connectionContainers.ContainsKey(hashHelper))
                {
                    ...
                    Bid.NotificationsTrace(
                        "<sc.SqlDependencyProcessDispatcher.Stop|DEP> %d#, hashtable hit, container: %d\n",
                        ObjectID,
                        container.ObjectID
                    );
