...
namespace System.Data.SqlClient
{
    ...
    internal sealed class TdsParserStateObject
    {
        ...
        // read from the netlib, this variable is _inBytesRead.  Then, we need to also keep track of how
        // many
        // bytes we have used as we consume the bytes from the buffer, that variable is _inBytesUsed.
        // Third,
        // we need to keep track of how many bytes are left in the packet, so that we know when we have
        // reached
        // the end of the packet and so we need to consume the next header.  That variable is
        // _inBytesPacket.
        ...
        // SNI variables                                                     // multiple resultsets in one
        // batch.
        ...
        private Dictionary<IntPtr, SNIPacket> _pendingWritePackets =
            new Dictionary<IntPtr, SNIPacket>(); // Stores write packets that have been sent to SNI, but have not yet finished writing (i.e. we are waiting for SNI's callback)
        ...
        //  At maximum number of secure string that need to be stored is two; one for login password and the
        // other for new change password
        ...
        // This is useful is you are purposefully running the replay from a different thread (e.g. during
        // SqlDataReader.Close)
        internal bool _permitReplayStackTraceToDiffer = false;

        // Used to indicate that the higher level object believes that this stateObj has enough data to
        ...
        //
        //    Type type =
        // typeof(SqlDataReader).Assembly.GetType("System.Data.SqlClient.TdsParserStateObject");
        //    System.Reflection.FieldInfo field = type.GetField("_forceAllPends",
        ...
        // CompletePendingReadWithSuccess() or CompletePendingReadWithFailure(int errorCode) in
        ...
        internal TdsParserStateObject(TdsParser parser, SNIHandle physicalConnection, bool async)
        {
            ...
            Debug.Assert(
                _parser._physicalStateObj._outBuff.Length
                    == _parser._physicalStateObj._inBuff.Length,
                "Unexpected unequal buffers."
            );
        ...
        {
            get { return _debugOnlyCopyOfSniContext; }
        }
#endif
        ...
        internal bool IsOrphaned
        {
            ...
            {
                Debug.Assert(
                    (0 == _activateCount && !_owner.IsAlive) // in pool
                        || (1 == _activateCount && _owner.IsAlive && _owner.Target != null)
                        || (1 == _activateCount && !_owner.IsAlive),
                ...
                Debug.Assert(
                    value == null
                        ...
                        || (
                            ...
                            && (((SqlDataReader)value).Command == _owner.Target)
                        ),
                    "Should not be changing the owner of an owned stateObj"
                );
        ...
        internal bool TimeoutHasExpired
        {
            ...
            {
                Debug.Assert(
                    0 == _timeoutMilliseconds || 0 == _timeoutTime,
                    "_timeoutTime hasn't been reset"
                );
        ...
        {
            Debug.Assert(
                !isNullCompressed || nullBitmapColumnsCount > 0,
                "Null-Compressed row requires columns count"
            );
        ...
        {
            // Removing one byte since TryReadByteArray\TryReadByte will aggressively read the next packet if
            // there is no data left - so we need to ensure there is a spare byte
        ...
        private struct NullBitmap
        {
            ...
            internal bool TryInitialize(TdsParserStateObject stateObj, int columnsCount)
            {
                ...
                {
                    Bid.Trace(
                        "<sc.TdsParserStateObject.NullBitmap.Initialize|INFO|ADV> %d#, NBCROW bitmap received, column count = %d\n",
                        stateObj.ObjectID,
                        columnsCount
                    );
                    Bid.TraceBin(
                        "<sc.TdsParserStateObject.NullBitmap.Initialize|INFO|ADV> NBCROW bitmap data: ",
                        _nullBitmap,
                        (UInt16)_nullBitmap.Length
                    );
                }
            ...
            /// if the bitmat value is false (if this method returns false), the value can be either null or
            // non-null - no guarantee in this case.
            /// To determine whether it is null or not, read it from the TDS (per NBCROW design spec, for
            // IMAGE/TEXT/NTEXT columns server might send
            ...
            internal bool IsGuaranteedNull(int columnOrdinal)
            {
                ...
                Debug.Assert(
                    columnOrdinal >= 0 && columnOrdinal < _columnsCount,
        ...
        // If this object is part of a TdsParserSessionPool, then this *must* be called inside the pool's
        // lock
        ...
        internal void Cancel(int objectID)
        {
            ...
            try
            {
                // Keep looping until we either grabbed the lock (and therefore sent attention) or the connection
                // closes\breaks
                while (
                    (!hasLock)
                    ...
                    { // Lock for the time being - since we need to synchronize the attention send.
                        ...
                        if ((!_cancelled) && (objectID == _allowObjectID) && (objectID != -1))
                        {
                            ...
                            if (_pendingData && !_attentionSent)
                            {
                                ...
                                // Keep looping until we have the parser lock (and so are allowed to write), or the conneciton
                                // closes\breaks
                                while (
                                    (!hasParserLock)
                                    && (_parser.State != TdsParserState.Closed)
                                ...
                                )
                                {
                                    ...
                                    finally
                                    {
                                        if (hasParserLock)
                                        {
                                            ...
                                            {
                                                _parser.Connection.ThreadHasParserLockForClose =
                                                    false;
                                            }
        ...
        public void CheckSetResetConnectionState(UInt32 error, CallbackType callbackType)
        {
            ...
            Debug.Assert(
                _parser.MARSOn,
                "Should not be calling CheckSetResetConnectionState on non MARS connection"
            );
        ...
        internal int DecrementPendingCallbacks(bool release)
        {
            ...
            {
                Bid.Trace(
                    "<sc.TdsParserStateObject.DecrementPendingCallbacks|ADV> %d#, after decrementing _pendingCallbacks: %d\n",
                    ObjectID,
                    _pendingCallbacks
                );
            }
            ...
            // NOTE: TdsParserSessionPool may call DecrementPendingCallbacks on a TdsParserStateObject which is
            // already disposed
            // This is not dangerous (since the stateObj is no longer in use), but we need to add a workaround
            // in the assert for it
            Debug.Assert(
                (remaining == -1 && _sessionHandle == null) || (0 <= remaining && remaining < 3),
                string.Format(
                    "_pendingCallbacks values is invalid after decrementing: {0}",
                    remaining
                )
            );
            return remaining;
            ...
            if (_writePacketCache != null)
            {
                lock (_writePacketLockObject)
                {
                    ...
                    finally
                    {
                        ...
                        // Do not set _writePacketCache to null, just in case a WriteAsyncCallback completes after this
                        // point
                    }
                }
            }
        ...
        internal int IncrementPendingCallbacks()
        {
            ...
            Debug.Assert(
                ...
                string.Format(
                    "_pendingCallbacks values is invalid after incrementing: {0}",
                    remaining
                )
            );
            return remaining;
        ...
        internal bool TryProcessHeader()
        {
            Debug.Assert(
                _inBytesPacket == 0,
                "there should not be any bytes left in packet when ReadHeader is called"
            );

            // if the header splits buffer reads - special case!
            if ((_partialHeaderBytesRead > 0) || (_inBytesUsed + _inputHeaderLen > _inBytesRead))
            {
                // VSTS 219884: when some kind of MITM (man-in-the-middle) tool splits the network packets, the
                // message header can be split over
                ...
                do
                {
                    ...
                    Debug.Assert(
                        _partialHeaderBytesRead <= _inputHeaderLen,
                        "Read more bytes for header than required"
                    );
                    if (_partialHeaderBytesRead == _inputHeaderLen)
                    {
                        ...
                        _inBytesPacket =
                            (
                                ...
                                | (int)_partialHeaderBuffer[TdsEnums.HEADER_LEN_FIELD_OFFSET + 1]
                            ) - _inputHeaderLen;
            ...
            else
            {
                ...
                _inBytesPacket =
                    (
                        ...
                        | (int)_inBuff[_inBytesUsed + TdsEnums.HEADER_LEN_FIELD_OFFSET + 1]
                    ) - _inputHeaderLen;
            ...
            if (_inBytesPacket < 0)
            {
                ...
                // NOTE: usually we do not proactively apply checks to TDS data, but this situation happened several
                // times
        ...
        // This ensure that there is data available to be read in the buffer and that the header has been
        // processed
        ...
        internal bool TryPrepareBuffer()
        {
            ...
            if ((_inBytesPacket == 0) && (_inBytesUsed < _inBytesRead))
            {
                ...
                Debug.Assert(
                    _inBytesPacket != 0,
                    "_inBytesPacket cannot be 0 after processing header!"
                );
                AssertValidState();
            }
            ...
            {
                // If the _inBytesPacket is not zero, then we have data left in the packet, but the data in the
                // packet
                // spans the buffer, so we can read any amount of data possible, and we do not need to call
                // ProcessHeader
                ...
                else if (_inBytesPacket == 0)
                {
                    ...
                    if (_inBytesUsed == _inBytesRead)
                    {
                        ...
                        // VSTS 219884: it can happen that the TDS packet header and its data are split across two network
                        // packets.
        ...
        internal bool SetPacketSize(int size)
        {
            ...
            Debug.Assert(
                (_outBuff == null && _inBuff == null) || (_outBuff.Length == _inBuff.Length),
                "Buffers are not in consistent state"
            );
            Debug.Assert(
                (_outBuff == null && _inBuff == null) || this == _parser._physicalStateObj,
            ...
            { // We only check _inBuff, since two buffers should be consistent.
                ...
                else if (size != _inBuff.Length)
                {
                    ...
                    {
                        // if we still have data left in the buffer we must keep that array reference and then copy into new
                        // one
                        byte[] temp = _inBuff;
                        ...
                        if (
                            (temp.Length < _inBytesUsed + remainingData)
                            || (_inBuff.Length < remainingData)
                        )
                        {
                            string errormessage =
                                Res.GetString(Res.SQL_InvalidInternalPacketSize)
                                + ' '
                                + temp.Length
                                + ", "
                                + _inBytesUsed
        ...
        // look at the next byte without pulling it off the wire, don't just returun _inBytesUsed since we
        // may
        ...
        public bool TryReadByteArray(byte[] buff, int offset, int len, out int totalRead)
        {
            ...
            Debug.Assert(
                buff == null || buff.Length >= len,
                "Invalid length sent to ReadByteArray()!"
            );
            ...
            while (len > 0)
            {
                ...
                int bytesToRead = Math.Min(
                    len,
                    Math.Min(_inBytesPacket, _inBytesRead - _inBytesUsed)
                );
            ...
            if (
                (_messageStatus != TdsEnums.ST_EOM)
                && ((_inBytesPacket == 0) || (_inBytesUsed == _inBytesRead))
            )
            {
                if (!TryPrepareBuffer())
                {
        ...
        internal bool TryReadByte(out byte value)
        {
            ...
            Debug.Assert(
                _inBytesUsed >= 0 && _inBytesUsed <= _inBytesRead,
            ...
            // return the byte from the buffer and increment the counter for number of bytes used in the in
            // buffer
        ...
        internal bool TryReadInt64(out long value)
        {
            ...
            if ((_bTmpRead > 0) || (((_inBytesUsed + 8) > _inBytesRead) || (_inBytesPacket < 8)))
            {
                ...
                {
                    Debug.Assert(
                        _bTmpRead + bytesRead == 8,
                        "TryReadByteArray returned true without reading all data required"
                    );
                    _bTmpRead = 0;
                    AssertValidState();
        ...
        internal bool TryReadStringWithEncoding(
            int length,
        ...
        )
        {
            ...
            if (null == encoding)
            {
                // Bug 462435:CR: TdsParser.DrainData(stateObj) hitting timeout exception after Connection
                // Resiliency change
                // http://vstfdevdiv:8080/web/wi.aspx?pcguid=22f9acc9-569a-41ff-b6ac-fac1b6370209&id=462435
                // Need to skip the current column before throwing the error - this ensures that the state shared
                // between this and the data reader is consistent when calling DrainData
                if (isPlp)
                {
                    ulong ignored;
        ...
        // NOTE: This method must be retriable WITHOUT replaying a snapshot
        // Every time you call this method increment the offst and decrease len by the value of
        // totalBytesRead
        internal bool TryReadPlpBytes(ref byte[] buff, int offst, int len, out int totalBytesRead)
        {
            ...
            Debug.Assert(
                ...
                "Invalid length sent to ReadPlpBytes()!"
            );
            bytesLeft = len;
        ...
        // Does not handle null values or NBC bitmask, ensure the value is not null before calling this
        // method
        ...
        internal bool TryReadNetworkPacket()
        {
...
#if DEBUG
            Debug.Assert(
                !_shouldHaveEnoughData || _attentionSent,
...
#endif
                        Bid.Trace(
                            "<sc.TdsParser.ReadNetworkPacket|INFO|ADV> Async packet replay\n"
                        );
                        return true;
                    }
#if DEBUG
            ...
            if (_failAsyncPends)
            {
                throw new InvalidOperationException(
                    "Attempted to pend a read when _failAsyncPends test hook was enabled"
                );
            }
...
#endif
            Debug.Assert(
                (_snapshot != null) ^ _asyncReadWithoutSnapshot,
        ...
        internal void ReadSniSyncOverAsync()
        {
            ...
            try
            {
                ...
                error = SNINativeMethodWrapper.SNIReadSyncOverAsync(
                    handle,
                    ref readPacket,
                    GetTimeoutRemaining()
                );
                ...
                { // Success - process results!
                    Debug.Assert(
                        ADP.PtrZero != readPacket,
                        "ReadNetworkPacket cannot be null in syncronous operation!"
                    );
                    ...
                    if (!_attentionSent)
                    {
                        AddError(
                            new SqlError(
                                ...
                                _parser.Server,
                                _parser.Connection.TimeoutErrorInternal.GetErrorMessage(),
                                "",
                                0,
                        ...
                        // Grab a reference to the _networkPacketTaskSource in case it becomes null while we are trying to
                        // use it
                        TaskCompletionSource<object> source = _networkPacketTaskSource;

                        if (_parser.Connection.IsInPool)
                        {
                            // Dev11 Bug 390048 : Timing issue between OnTimeout and ReadAsyncCallback results in SqlClient's
                            // packet parsing going out of sync
                            // We should never timeout if the connection is currently in the pool: the safest thing to do here
                            // is to doom the connection to avoid corruption
                            Debug.Assert(
                                _parser.Connection.IsConnectionDoomed,
                        ...
                        if (source != null)
                        {
                            Task.Delay(AttentionTimeoutSeconds * 1000)
                                .ContinueWith(_ =>
                                {
                                    ...
                                    if (!source.Task.IsCompleted)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            // So it is safe for us to break the connection and cancel the Task (since we are not sure that
                                            // ReadAsyncCallback will ever be called)
                                            if (
                                                (pendingCallback == 3) && (!source.Task.IsCompleted)
                                            )
                                            {
                                                Debug.Assert(
                                                    source == _networkPacketTaskSource,
                                                    "_networkPacketTaskSource which is being timed is not the current task source"
                                                );
                                                ...
                                                // This is needed since the timeout error added above is non-fatal (and so throwing it won't break
                                                // the connection)
                                                _parser.State = TdsParserState.Broken;
                                                _parser.Connection.BreakConnection();

                                                // If we didn't get an exception (something else observed it?) then ensure that the task is
                                                // cancelled
                                                if (!exceptionStored)
                                                {
                                                    source.TrySetCanceled();
        ...
        internal void ReadSni(TaskCompletionSource<object> completion)
        {
            Debug.Assert(
                _networkPacketTaskSource == null
                    || ((_asyncReadWithoutSnapshot) && (_networkPacketTaskSource.Task.IsCompleted)),
            ...
            try
            {
                ...
                // -1 == Infinite
                //  0 == Already timed out (NOTE: To simulate the same behavior as sync we will only timeout on 0 if
                // we receive an IO Pending from SNI)
                    ...
                    if (handle != null)
                    {
                        ...
                        if (
                            !(
                                ...
                                || TdsEnums.SNI_SUCCESS_IO_PENDING == error
                            )
                        )
                        {
                ...
                { // Success - process results!
                    Debug.Assert(
                        ADP.PtrZero != readPacket,
                        "ReadNetworkPacket should not have been null on this async operation!"
                    );
                ...
                // That is handled by user who initiated async read, or by ReadNetworkPacket which is sync over
                // async.
            }
            finally
            {
        ...
        /// </summary>
        /// <param name="throwOnException">If true then an exception will be thrown if the connection is
        // found to be dead, otherwise no exception will be thrown</param>
        ...
        {
            Debug.Assert(
                _parser.Connection == null || _parser.Connection.Pool != null,
                "Shouldn't be calling IsConnectionAlive on non-pooled connections"
            );
            bool isAlive = true;
            ...
            if (DateTime.UtcNow.Ticks - _lastSuccessfulIOTimer._value > CheckConnectionWindow)
            {
                ...
                )
                {
                    ...
                    (_pendingCallbacks > 1)
                    || ((_parser.Connection != null) && (!_parser.Connection.IsInPool))
                )
                {
        ...
        /// <summary>
        /// Checks to see if the underlying connection is still valid (used by idle connection resiliency -
                ...
                else
                {
                    ...
                    if (!stateObj._attentionSent)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // For DB Mirroring Failover during login, never break the connection, just close the TdsParser
                                // (Devdiv 846298)
                                _parser.Disconnect();
                            }
                            else if (
        ...
        public void ProcessSniPacket(IntPtr packet, UInt32 error)
        {
            ...
            {
                UInt32 dataSize = 0;
                UInt32 getDataError = SNINativeMethodWrapper.SNIPacketGetData(
                    packet,
                    _inBuff,
                    ref dataSize
                );
                ...
                if (getDataError == TdsEnums.SNI_SUCCESS)
                {
                    if (_inBuff.Length < dataSize)
                    {
                        ...
                        throw SQL.InvalidInternalPacketSize(
                            Res.GetString(Res.SqlMisc_InvalidArraySizeMessage)
                        );
                    }
                    ...
                    {
                        Bid.TraceBin(
                            "<sc.TdsParser.ReadNetworkPacketAsyncCallback|INFO|ADV> Packet read",
                            _inBuff,
                            (UInt16)_inBytesRead
                        );
                    }
        ...
        { // Key never used.
            ...
            {
                Debug.Assert(
                    IntPtr.Zero == packet || IntPtr.Zero != packet && source != null,
            ...
            {
                // pendingCallbacks may be 2 after decrementing, this indicates that a fatal timeout is occuring,
                // and therefore we shouldn't complete the task
                ...
                if ((processFinallyBlock) && (source != null) && (pendingCallbacks < 2))
                {
                    if (error == 0)
                    {
                        if (_executionContext != null)
                        {
                            ExecutionContext.Run(
                                _executionContext,
                                (state) => source.TrySetResult(null),
                                null
                            );
                        }
                        else
                    ...
                    else
                    {
                        if (_executionContext != null)
                        {
                            ExecutionContext.Run(
                                _executionContext,
                                (state) => ReadAsyncCallbackCaptureException(source),
                                null
                            );
                        }
                        else
        ...
        private void ReadAsyncCallbackCaptureException(TaskCompletionSource<object> source)
        {
            ...
            try
            {
                ...
                {
                    // Connection was closed by another thread before we parsed the packet, so no error was added to the
                    // collection
                    throw ADP.ClosedConnectionError();
                }
            }
            ...
            if (!captureSuccess)
            {
                ...
                // This is unusual, but possible if a fatal timeout occured on another thread (which should mean
                // that the connection is now broken)
                Debug.Assert(
                    _parser.State == TdsParserState.Broken
                        || _parser.State == TdsParserState.Closed
                        || _parser.Connection.IsConnectionDoomed,
...
#pragma warning disable 420 // a reference to a volatile field will not be treated as volatile
        ...
        { // Key never used.
            ...
            try
            {
                ...
                {
                    Bid.Trace(
                        "<sc.TdsParser.WriteAsyncCallback|Info> write async returned error code %d\n",
                        (int)sniError
                    );
                    try
                    {
                    ...
                    catch (Exception e)
                    {
                        ...
                        else
                        {
                            ...
                            if (writeCompletionSource != null)
                            {
                                var delayedException = Interlocked.Exchange(
                                    ref _delayedWriteAsyncCallbackException,
                                    null
                                );
            ...
            finally
            {
                ...
                if (SqlCommand.DebugForceAsyncWriteDelay > 0)
                {
                    new Timer(
                        ...
                        null,
                        SqlCommand.DebugForceAsyncWriteDelay,
                        Timeout.Infinite
                    );
                }
                else
...
#pragma warning restore 420
        ...
        //
        // Takes a secure string and offsets and saves them for a write latter when the information is
        // written out to SNI Packet
        //  This method is provided to better handle the life cycle of the clear text of the secure string
        //  This method also ensures that the clear text is not held in the unpined managed buffer so that
        // it avoids getting moved around by CLR garbage collector
        //  TdsParserStaticMethods.EncryptPassword operation is also done in the unmanaged buffer for the
        // clear text later
        //
        internal void WriteSecureString(SecureString secureString)
        {
            ...
            Debug.Assert(
                _securePasswords[0] == null || _securePasswords[1] == null,
                "There are more than two secure passwords"
            );
            ...
            // Given current TDS format and implementation it is not possible that one of secure string is the
            // last item and exactly fill up the output buffer
            //  if this ever happens and it is correct situation, the packet needs to be written out after
            // _outBytesUsed is update
            Debug.Assert(
                (_outBytesUsed + lengthInBytes) < _outBuff.Length,
                "Passwords cannot be splited into two different packet or the last item which fully fill up _outBuff!!!"
            );

            _outBytesUsed += lengthInBytes;
        }

        // ResetSecurePasswordsInformation: clears information regarding secure passwords when login is
        // done; called from TdsParser.TdsLogin
...
#pragma warning restore 420
        ...
        internal void WriteByte(byte b)
        {
            ...
            Debug.Assert(
                _outBytesUsed <= _outBuff.Length,
                "ERROR - TDSParser: _outBytesUsed > _outBuff.Length"
            );

            // check to make sure we haven't used the full amount of space available in the buffer, if so, flush
            // it
        ...
        private void WriteByteArraySetupContinuation(
            Byte[] b,
            int len,
            ...
            else
            {
                ...
                Debug.Assert(
                    ...
                    String.Format(
                        ...
                        "Unexpected argument {0,-2:x2} to WritePacket",
                        flushMode
                    )
                );
            }
...
#pragma warning disable 420 // a reference to a volatile field will not be treated as volatile
        ...
        )
        {
            ...
            if (!sync)
            {
                // Add packet to the pending list (since the callback can happen any time after we call
                // SNIWritePacket)
            ...
            if (sniError == TdsEnums.SNI_SUCCESS_IO_PENDING)
            {
                ...
                if (!canAccumulate)
                {
                    ...
                    // Ensure that setting _writeCompletionSource completes before checking
                    // _delayedWriteAsyncCallbackException
                    Thread.MemoryBarrier();

                    // Check for a stored exception
                    delayedException = Interlocked.Exchange(
                        ref _delayedWriteAsyncCallbackException,
                        null
                    );
                    if (delayedException != null)
            ...
            else
            {
                ...
                if (sniError == TdsEnums.SNI_SUCCESS)
                {
                    ...
                    if (!sync)
                    {
                        ...
                        Debug.Assert(
                            packetPointer != IntPtr.Zero,
                            "Packet added to list has an invalid pointer, can not remove from pending list"
                        );
...
#pragma warning restore 420
        ...
        internal void SendAttention(bool mustTakeWriteLock = false)
        {
            if (!_attentionSent)
            {
                ...
                SNINativeMethodWrapper.SNIPacketSetData(
                    attnPacket,
                    SQL.AttentionHeader,
                    TdsEnums.HEADER_LEN,
                    null,
                ...
                try
                {
                    ...
                    if (!_skipSendAttention)
                    {
                        ...
                        if (
                            (mustTakeWriteLock) && (!_parser.Connection.ThreadHasParserLockForClose)
                        )
                        {
                            releaseLock = true;
                ...
                {
                    Bid.TraceBin(
                        "<sc.TdsParser.WritePacket|INFO|ADV>  Packet sent",
                        _outBuff,
        ...
        private Task WriteSni(bool canAccumulate)
        {
            ...
            SNINativeMethodWrapper.SNIPacketSetData(
                ...
                _outBytesUsed,
                _securePasswords,
                _securePasswordOffsetsInBuffer
            );

            uint sniError;
            Debug.Assert(
                Parser.Connection._parserLock.ThreadMayHaveLock(),
            ...
            // Check to see if the timeout has occured.  This time out code is special case code to allow BCP
            // writes to timeout to fix bug 350558, eventually we should make all writes timeout.
        ...
        internal SNIPacket GetResetWritePacket()
        {
            if (_sniPacket != null)
            {
                SNINativeMethodWrapper.SNIPacketReset(
                    Handle,
                    SNINativeMethodWrapper.IOType.WRITE,
                    _sniPacket,
                    SNINativeMethodWrapper.ConsumerNumber.SNI_Consumer_SNI
                );
            }
            else
            {
        ...
        internal void ClearAllWritePackets()
        {
            ...
            {
                Debug.Assert(
                    _pendingWritePackets.Count == 0 && _asyncWriteCount == 0,
                    "Should not clear all write packets if there are packets pending"
                );
        ...
        private IntPtr AddPacketToPendingList(SNIPacket packet)
        {
            Debug.Assert(
                packet == _sniPacket,
                "Adding a packet other than the current packet to the pending list"
            );
            _sniPacket = null;
        ...
        private void RemovePacketFromPendingList(IntPtr pointer)
        {
            ...
            lock (_writePacketLockObject)
            {
                ...
                else
                {
                    Debug.Assert(
                        false,
                        "Removing a packet from the pending list that was never added to it"
                    );
                }
#endif
            }
        ...
        private void SniWriteStatisticsAndTracing()
        {
            ...
            if (Bid.AdvancedOn)
            {
                ...
                {
                    for (
                        int i = _tracePasswordOffset;
                        i < _tracePasswordOffset + _tracePasswordLength;
                        i++
                    )
                    {
                        _outBuff[i] = 0;
                ...
                {
                    for (
                        int i = _traceChangePasswordOffset;
                        i < _traceChangePasswordOffset + _traceChangePasswordLength;
                        i++
                    )
                    {
                        _outBuff[i] = 0;
        ...
        /// <summary>
        /// Gets the full list of errors and warnings (including the pre-attention ones), then wipes all
        ...
        internal SqlErrorCollection GetFullErrorAndWarningCollection(out bool broken)
        {
            ...
            lock (_errorAndWarningsLock)
            {
                ...
                // We also process the pre-attention error lists here since, if we are here and they are populated,
                // then an error occured while sending attention so we should show the errors now (otherwise they'd
                // be
                // lost)
        ...
        private void AddErrorsToCollection(
            SqlErrorCollection inCollection,
            ref SqlErrorCollection collectionToAddTo,
            ref bool broken
        )
        {
        ...
        internal void StoreErrorAndWarningForAttention()
        {
            ...
            {
                Debug.Assert(
                    _preAttentionErrors == null && _preAttentionWarnings == null,
                    "Can't store errors for attention because there are already errors stored"
                );
        ...
        internal void RestoreErrorAndWarningAfterAttention()
        {
            ...
            {
                Debug.Assert(
                    _errors == null && _warnings == null,
                    "Can't restore errors after attention because there are already other errors"
                );

                _hasErrorOrWarning = (
                    ((_preAttentionErrors != null) && (_preAttentionErrors.Count > 0))
        ...
        /// <summary>
        /// Debug Only: Ensures that the TdsParserStateObject has no lingering state and can safely be
        // re-used
        /// </summary>
        [Conditional("DEBUG")]
        internal void AssertStateIsClean()
        {
            ...
            )
            {
                // Async reads
                Debug.Assert(
                    _snapshot == null && !_snapshotReplay,
                ...
                Debug.Assert(
                    ...
                    "StateObj has a stored execution context from an async read"
                );
                // Async writes
                Debug.Assert(_asyncWriteCount == 0, "StateObj still has outstanding async writes");
                Debug.Assert(
                    _delayedWriteAsyncCallbackException == null,
                    "StateObj has an unobserved exceptions from an async write"
                );
                // Attention\Cancellation\Timeouts
                Debug.Assert(
                    !_attentionReceived && !_attentionSent && !_attentionSending,
                    string.Format(
                        "StateObj is still dealing with attention: Sent: {0}, Received: {1}, Sending: {2}",
                        _attentionSent,
                        _attentionReceived,
                        _attentionSending
                    )
        ...
        internal void CompletePendingReadWithSuccess(bool resetForcePendingReadsToWait)
        {
            ...
            Debug.Assert(
                _forcePendingReadsToWaitForUser,
                "Not forcing pends to wait for user - can't force complete"
            );
        ...
        class StateSnapshot
        {
            ...
            {
                if (
                    _stateObj._cleanupAltMetaDataSetArray != null
                ...
                {
                    _stateObj._cleanupAltMetaDataSetArray = (_SqlMetaDataSetCollection)
                        _stateObj._cleanupAltMetaDataSetArray.Clone();
                }
            }
            ...
            {
                Debug.Assert(
                    _snapshotInBuffCurrent == _snapshotInBuffs.Count,
                    "Should not be reading new packets when not replaying last packet"
                );
            }
            ...
            internal void CheckStack(StackTrace trace)
            {
                ...
                {
                    Debug.Assert(
                        _stateObj._permitReplayStackTraceToDiffer
                            || prev.Stack.ToString() == trace.ToString(),
                        "The stack trace on subsequent replays should be the same"
                    );
                }
            }
#endif
            ...
            internal void Snap()
            {
                ...
                Debug.Assert(
                    ...
                    "Has partially read data when snapshot taken"
                );
                Debug.Assert(
                    _stateObj._partialHeaderBytesRead == 0,
                    "Has partially read header when shapshot taken"
                );
#endif
