...
namespace System.Data.SqlClient
{
    ...
    internal static class SqlSecurityUtility
    {
        ...
        internal static bool CompareBytes(
            byte[] buffer1,
            byte[] buffer2,
            int buffer2Index,
            int lengthToCompare
        ...
        /// <returns></returns>
        private static string ValidateAndGetEncryptionAlgorithmName(
        ...
        )
        {
            if (TdsEnums.CustomCipherAlgorithmId == cipherAlgorithmId)
            {
                ...
                {
                    throw SQL.NullColumnEncryptionAlgorithm(
                        SqlClientEncryptionAlgorithmFactoryList
                            .GetInstance()
                            .GetRegisteredCipherAlgorithmNames()
                    );
                }
            ...
            {
                throw SQL.UnknownColumnEncryptionAlgorithmId(
                    cipherAlgorithmId,
                    GetRegisteredCipherAlgorithmIds()
                );
            }
        }
        ...
        /// </summary>
        internal static byte[] EncryptWithKey(
            byte[] plainText,
            SqlCipherMetadata md,
            string serverName
        )
        {
        ...
        /// </summary>
        internal static byte[] DecryptWithKey(
            byte[] cipherText,
            SqlCipherMetadata md,
            string serverName
        )
        {
            ...
            catch (Exception e)
            {
                ...
                string keyStr = GetBytesAsString(
                    md.EncryptionKeyInfo.Value.encryptedKey,
                    fLast: true,
                    countOfBytes: 10
                );
        ...
        internal static void DecryptSymmetricKey(SqlCipherMetadata md, string serverName)
        {
            Debug.Assert(
                serverName != null,
                @"serverName should not be null in DecryptSymmetricKey."
            );
            Debug.Assert(md != null, "md should not be null in DecryptSymmetricKey.");
            Debug.Assert(
                md.EncryptionInfo.HasValue,
                "md.EncryptionInfo should not be null in DecryptSymmetricKey."
            );
            Debug.Assert(
                md.EncryptionInfo.Value.ColumnEncryptionKeyValues != null,
            ...
            foreach (
                SqlEncryptionKeyInfo keyInfo in md.EncryptionInfo.Value.ColumnEncryptionKeyValues
            )
            {
                try
                {
            ...
            if (null == symKey)
            {
                Debug.Assert(
                    null != lastException,
                    "CEK decryption failed without raising exceptions"
                );
                throw lastException;
            }

            Debug.Assert(
                encryptionkeyInfoChosen.HasValue,
                "encryptionkeyInfoChosen must have a value."
            );
            ...
            string algorithmName = ValidateAndGetEncryptionAlgorithmName(
                md.CipherAlgorithmId,
                md.CipherAlgorithmName
            ); // may throw
            SqlClientEncryptionAlgorithmFactoryList
                .GetInstance()
                .GetAlgorithm(symKey, md.EncryptionType, algorithmName, out cipherAlgorithm); // will validate algorithm name and type
