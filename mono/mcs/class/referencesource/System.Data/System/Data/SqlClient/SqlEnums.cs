...
namespace System.Data.SqlClient
{
    ...
    internal sealed class MetaType
    {
        ...
        {
            return ((int)type >= 0 && ((int)type <= (int)SqlDbType.Variant));
        }
        ...
        )
        {
            switch (Type.GetTypeCode(dataType))
            {
                ...
                case TypeCode.Object:
                    ...
                    {
                        return (
                            (inferLen && !((SqlString)value).IsNull)
                                ? PromoteStringType(((SqlString)value).Value)
                                : MetaNVarChar
                        ); // MDAC 87587
                    }
                    else if (
                        dataType == typeof(IEnumerable<DbDataRecord>)
        ...
        /// </exception>
        [Conditional("DEBUG")]
        private static void AssertIsUserDefinedTypeInstance(
            object sqlValue,
            string failedAssertMessage
        )
        {
            Type type = sqlValue.GetType();
            Microsoft.SqlServer.Server.SqlUserDefinedTypeAttribute[] attributes =
        ...
        internal static SqlDbType GetSqlDbTypeFromOleDbType(short dbType, string typeName)
        {
            ...
            switch ((OleDbType)dbType)
            {
                ...
                case OleDbType.Currency:
                    sqlType =
                        (typeName == MetaTypeName.SMALLMONEY)
                ...
                case OleDbType.Binary:
                    sqlType =
                        (typeName == MetaTypeName.BINARY) ? SqlDbType.Binary : SqlDbType.VarBinary;
                    break;
                ...
                case OleDbType.BSTR:
                    ...
                    sqlType =
                        (typeName == MetaTypeName.NCHAR) ? SqlDbType.NChar : SqlDbType.NVarChar;
                    break;
        ...
        internal static MetaType GetSqlDataType(int tdsType, UInt32 userType, int length)
        {
            switch (tdsType)
            {
                ...
                case TdsEnums.SQLINTN:
                    return (
                        (4 <= length)
                            ? ((4 == length) ? MetaInt : MetaBigInt)
                            : ((2 == length) ? MetaSmallInt : MetaTinyInt)
                    );
                ...
                default:
                    Debug.Assert(
                        false,
                        "Unknown type " + tdsType.ToString(CultureInfo.InvariantCulture)
                    );
        ...
        private static readonly MetaType MetaSmallVarBinary = new MetaType(
            ...
            false,
            false,
            TdsEnums.SQLVARBINARY,
            TdsEnums.SQLBIGBINARY,
            ADP.StrEmpty,
            typeof(System.Byte[]),
            typeof(SqlBinary),
            TdsEnums.SmallVarBinary,
            DbType.Binary,
            2
        );
        ...
        private static readonly MetaType MetaSmallDateTime = new MetaType(
            ...
            false,
            false,
            TdsEnums.SQLDATETIM4,
            TdsEnums.SQLDATETIMN,
            MetaTypeName.SMALLDATETIME,
            typeof(System.DateTime),
            typeof(SqlDateTime),
