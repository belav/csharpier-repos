...
namespace System.Data.SqlClient
{
    ...
    /// </summary>
    sealed class SqlSymmetricKeyCache
    {
        ...
        internal bool GetKey(
            ...
            string serverName,
            out SqlClientSymmetricKey encryptionKey
        )
        {
            ...
            StringBuilder cacheLookupKeyBuilder = new StringBuilder(
                serverName,
                capacity: serverName.Length
                    + SqlSecurityUtility.GetBase64LengthFromByteLength(keyInfo.encryptedKey.Length)
                    + keyInfo.keyStoreName.Length
                    + 2 /*separators*/
            );
            ...
            {
                Debug.Assert(
                    SqlConnection.ColumnEncryptionTrustedMasterKeyPaths != null,
                ...
                if (
                    SqlConnection.ColumnEncryptionTrustedMasterKeyPaths.TryGetValue(
                        serverName,
                        out trustedKeyPaths
                    )
                )
                ...
                if (
                    !SqlConnection.TryGetColumnEncryptionKeyStoreProvider(
                        keyInfo.keyStoreName,
                        out provider
                    )
                )
                {
                    throw SQL.UnrecognizedKeyStoreProviderName(
                        keyInfo.keyStoreName,
                ...
                try
                {
                    plaintextKey = provider.DecryptColumnEncryptionKey(
                        keyInfo.keyPath,
                ...
                catch (Exception e)
                {
                    ...
                    string keyHex = SqlSecurityUtility.GetBytesAsString(
                        keyInfo.encryptedKey,
                        fLast: true,
                        countOfBytes: 10
                    );
