...
namespace System.Data.SqlClient
{
    ...
    internal sealed class SqlInternalConnectionSmi : SqlInternalConnection
    {
        ...
        {
            get { return _smiContext; }
        }

        internal SmiConnection SmiConnection
        {
            get { return _smiConnection; }
        }
        ...
        protected override bool UnbindOnTransactionCompletion
        {
            get
            {
                return ConnectionOptions.TransactionBinding
                    == SqlConnectionString.TransactionBindingEnum.ImplicitUnbind;
            }
        }
        ...
        private SysTx.Transaction ContextTransaction { get; set; }
        ...
        internal void AutomaticEnlistment()
        {
            ...
            {
                if (
                    null != currentSystemTransaction
                    && contextTransaction != currentSystemTransaction
                )
                {
        ...
        )
        {
            ...
            {
                Bid.Trace(
                    "<sc.SqlInternalConnectionSmi.ExecuteTransaction|ADV> %d#, transactionRequest=%ls, transactionName='%ls', isolationLevel=%ls, internalTransaction=#%d transactionId=0x%I64x.\n",
                    base.ObjectID,
                    ...
                    (null != internalTransaction)
                        ? internalTransaction.TransactionId
            ...
            switch (transactionRequest)
            {
                case TransactionRequest.Begin:
                    ...
                    Debug.Assert(
                        _smiEventSink.HasMessages || null != _currentTransaction,
        ...
        protected override byte[] GetDTCAddress()
        {
            ...
            if (Bid.AdvancedOn)
            {
                ...
                {
                    Bid.TraceBin(
                        "<sc.SqlInternalConnectionSmi.GetDTCAddress|ADV> whereAbouts",
                        whereAbouts,
                        (UInt16)whereAbouts.Length
                    );
                }
                else
                {
        ...
        internal void GetCurrentTransactionPair(
            out long transactionId,
            out SysTx.Transaction transaction
        )
        {
        ...
        protected override void PropagateTransactionCookie(byte[] transactionCookie)
        {
            if (Bid.AdvancedOn)
            {
                ...
                {
                    Bid.TraceBin(
                        "<sc.SqlInternalConnectionSmi.PropagateTransactionCookie|ADV> transactionCookie",
                        transactionCookie,
                        (UInt16)transactionCookie.Length
                    );
                }
                else
                {
                    Bid.Trace(
                        "<sc.SqlInternalConnectionSmi.PropagateTransactionCookie|ADV> null\n"
                    );
                }
            }
        ...
        private void TransactionEnded(long transactionId, TransactionState transactionState)
        {
            ...
            if (null != _currentTransaction)
            {
                ...
                {
                    Debug.Assert(
                        _currentTransaction.TransactionId == transactionId,
        ...
        private void TransactionStarted(long transactionId, bool isDistributed)
        {
            ...
            else
            {
                TransactionType transactionType =
                    (isDistributed) ? TransactionType.Distributed : TransactionType.LocalFromTSQL;
                _currentTransaction = new SqlInternalTransaction(
                    this,
                    transactionType,
                    null,
