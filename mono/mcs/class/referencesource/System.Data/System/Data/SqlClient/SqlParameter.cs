...
namespace System.Data.SqlClient
{
    ...
    public sealed partial class SqlParameter : DbParameter, IDbDataParameter, ICloneable
    {
        ...
        {
            get { return _columnEncryptionCipherMetadata; }
        ...
        internal MSS.SmiParameterMetaData MetaDataForSmi(out ParameterPeekAheadValue peekAhead)
        {
            ...
            if (0 == maxLen)
            {
                ...
                (
                    maxLen > MSS.SmiMetaData.MaxBinaryLength
                    && (SqlDbType.Binary == mt.SqlDbType || SqlDbType.VarBinary == mt.SqlDbType)
                )
                || (
                    maxLen > MSS.SmiMetaData.MaxANSICharacters
                    && (SqlDbType.Char == mt.SqlDbType || SqlDbType.VarChar == mt.SqlDbType)
                )
                || (
                    maxLen > MSS.SmiMetaData.MaxUnicodeCharacters
                    && (SqlDbType.NChar == mt.SqlDbType || SqlDbType.NVarChar == mt.SqlDbType)
                )
            )
            {
                maxLen = -1;
            ...
            if (SqlDbType.Xml == mt.SqlDbType)
            {
                ...
                SqlDbType.Udt == mt.SqlDbType
                || (SqlDbType.Structured == mt.SqlDbType && !ADP.IsEmpty(this.TypeName))
            )
            {
                ...
                if (
                    (
                        !ADP.IsEmpty(typeSpecificNamePart1)
                        && TdsEnums.MAX_SERVERNAME < typeSpecificNamePart1.Length
                    )
                    || (
                        !ADP.IsEmpty(typeSpecificNamePart2)
                        && TdsEnums.MAX_SERVERNAME < typeSpecificNamePart2.Length
                    )
                    || (
                        !ADP.IsEmpty(typeSpecificNamePart3)
                        && TdsEnums.MAX_SERVERNAME < typeSpecificNamePart3.Length
                    )
                )
                {
                    throw ADP.ArgumentOutOfRange("names");
            ...
            return new MSS.SmiParameterMetaData(
                mt.SqlDbType,
                maxLen,
                precision,
                scale,
        ...
        {
            get { return _isSqlParameterSqlType; }
        ...
        internal string ParameterNameFixed
        {
            get
            {
                ...
                Debug.Assert(
                    parameterName.Length <= TdsEnums.MAX_PARAMETER_NAME_LENGTH,
        ...
        {
            get { return GetMetaTypeOnly().SqlDbType; }
            set
            {
                MetaType metatype = _metaType;
                // HACK!!!
                ...
                {
                    PropertyTypeChanging();
                    _metaType = MetaType.GetMetaTypeFromSqlDbType(
                        value,
                        value == SqlDbType.Structured
                    );
                }
            }
        }
        ...
        [Browsable(false), EditorBrowsableAttribute(EditorBrowsableState.Advanced)]
                ...
                else
                {
                    ...
                    switch (actualType)
                    {
                        ...
                        case SqlDbType.Xml:
                            {
                                ...
                                _actualSize = (
                                    (ShouldSerializeSize() && (_actualSize <= coercedSize))
                                        ? _actualSize
                                        : coercedSize
                                );
                                if (_actualSize == -1)
                        ...
                        case SqlDbType.Timestamp:
                            coercedSize =
                                ((!_isNull) && (!_coercedValueIsDataFeed))
                                    ? (BinarySize(val, _coercedValueIsSqlType))
                                    : 0;
                            _actualSize = (ShouldSerializeSize() ? Size : 0);
                            _actualSize = (
                                (ShouldSerializeSize() && (_actualSize <= coercedSize))
                        ...
                        case SqlDbType.Time:
                            _actualSize = (
                                isSqlVariant ? 5 : MetaType.GetTimeSizeFromScale(GetActualScale())
                            );
                            break;
                        case SqlDbType.DateTime2:
                            ...
                            _actualSize =
                                ...
                                + (
                                    isSqlVariant
                                        ? 5
                                        : MetaType.GetTimeSizeFromScale(GetActualScale())
                                );
                            break;
        ...
        internal static object CoerceValue(
            object value,
            MetaType destinationType,
            out bool coercedToDataFeed,
            out bool typeChanged,
        ...
        )
        {
            ...
            { // Special case for Xml types (since we need to convert SqlXml into a string)
                try
                {
                    ...
                    if ((typeof(string) == destinationType.ClassType))
                    {
                        ...
                        {
                            value = MetaType.GetStringFromXml(
                                (XmlReader)(((SqlXml)value).CreateReader())
                            );
                        }
                        ...
                        else
                        {
                            value = Convert.ChangeType(
                                value,
                                destinationType.ClassType,
                                (IFormatProvider)null
                            );
                        }
                    }
                    else if (
                        (DbType.Currency == destinationType.DbType)
                    ...
                    )
                    {
                        ...
                        (typeof(SqlBytes) == currentType)
                        && (typeof(byte[]) == destinationType.ClassType)
                    )
                    {
                        typeChanged = false; // Do nothing
                    }
                        ...
                        (typeof(string) == currentType)
                        && (SqlDbType.Time == destinationType.SqlDbType)
                    )
                    {
                        ...
                        (typeof(string) == currentType)
                        && (SqlDbType.DateTimeOffset == destinationType.SqlDbType)
                    )
                    {
                        value = DateTimeOffset.Parse((string)value, (IFormatProvider)null);
                    }
                    else if (
                        (typeof(DateTime) == currentType)
                        && (SqlDbType.DateTimeOffset == destinationType.SqlDbType)
                    )
                    {
                    ...
                    )
                    {
                        ...
                        destinationType.ClassType == typeof(byte[])
                        && value is Stream
                        && allowStreaming
                    )
                    {
            ...
            Debug.Assert(
                allowStreaming || !coercedToDataFeed,
                "Streaming is not allowed, but type was coerced into a data feed"
            );
            Debug.Assert(
                value.GetType() == currentType ^ typeChanged,
                "Incorrect value for typeChanged"
            );
            return value;
        }
        ...
        internal void FixStreamDataForNonPLP()
        {
            ...
            Debug.Assert(
                false,
                "_coercedValueIsDataFeed was true, but the value was not a known DataFeed type"
            );
        }
        ...
        private void GetActualFieldsAndProperties(
            out List<MSS.SmiExtendedMetaData> fields,
            out SmiMetaDataPropertyCollection props,
            out ParameterPeekAheadValue peekAhead
        )
        {
            fields = null;
            props = null;
            ...
            if (value is DataTable)
            {
                ...
                if (hasKey)
                {
                    ...
                    props[MSS.SmiPropertySelector.UniqueKey] = new MSS.SmiUniqueKeyProperty(
                        new List<bool>(keyCols)
                    );
                }
            }
                    ...
                    if (enumerator.MoveNext())
                    {
                        ...
                        if (0 < fieldCount)
                        {
                            ...
                            SmiOrderProperty.SmiColumnOrder[] sort =
                                new SmiOrderProperty.SmiColumnOrder[fieldCount];
                            fields = new List<MSS.SmiExtendedMetaData>(fieldCount);
                            for (int i = 0; i < fieldCount; i++)
                            {
                                SqlMetaData colMeta = firstRecord.GetSqlMetaData(i);
                                fields.Add(
                                    MSS.MetaDataUtilsSmi.SqlMetaDataToSmiExtendedMetaData(colMeta)
                                );
                                ...
                                if (SortOrder.Unspecified != colMeta.SortOrder)
                                {
                                    ...
                                    {
                                        throw SQL.SortOrdinalGreaterThanFieldCount(
                                            i,
                                            colMeta.SortOrdinal
                                        );
                                    }
                            ...
                            {
                                props = new SmiMetaDataPropertyCollection();
                                props[MSS.SmiPropertySelector.UniqueKey] =
                                    new MSS.SmiUniqueKeyProperty(new List<bool>(keyCols));
                            }
                            ...
                            if (hasDefault)
                            {
                                ...
                                props[MSS.SmiPropertySelector.DefaultFields] =
                                    new MSS.SmiDefaultFieldsProperty(new List<bool>(defaultFields));
                            }
                            ...
                            if (0 < sortCount)
                            {
                                ...
                                if (maxSortOrdinal >= sortCount)
                                {
                                    ...
                                    Debug.Assert(
                                        i < sortCount,
                                        "SqlParameter.GetActualFieldsAndProperties: SortOrdinal hole-finding algorithm failed!"
                                    );
            ...
            else if (value is DbDataReader)
            {
                ...
                foreach (SmiExtendedMetaData md in fields)
                {
                    Debug.Assert(
                        null != md,
                        "Shouldn't be able to have holes, since original loop algorithm prevents such."
                    );
                }
#endif
        ...
        internal object GetCoercedValue()
        {
            ...
            { // will also be set during parameter Validation
                ...
                {
                    bool typeChanged;
                    _coercedValue = CoerceValue(
                        Value,
                        _internalMetaType,
                        out _coercedValueIsDataFeed,
                        out typeChanged
                    );
        ...
        [Conditional("DEBUG")]
        internal void AssertPropertiesAreValid(
            object value,
            bool? isSqlType = null,
            bool? isDataFeed = null,
            bool? isNull = null
        )
        {
            Debug.Assert(
                !isSqlType.HasValue || (isSqlType.Value == (value is INullable)),
                "isSqlType is incorrect"
            );
            Debug.Assert(
                !isDataFeed.HasValue || (isDataFeed.Value == (value is DataFeed)),
                "isDataFeed is incorrect"
            );
            Debug.Assert(
                !isNull.HasValue || (isNull.Value == ADP.IsNull(value)),
                "isNull is incorrect"
            );
        }
        ...
        { // MDAC 67063
            ...
            else if (!ShouldSerializeSize() && !_metaType.IsFixed)
            {
                ...
                (!ShouldSerializePrecision() && !ShouldSerializeScale())
                && (_metaType.SqlDbType == SqlDbType.Decimal)
            )
            {
        ...
        internal MetaType ValidateTypeLengths(bool yukonOrNewer)
        {
            ...
            { // if type has 2 byte length
                ...
                if ((mt.IsNCharType) && (yukonOrNewer))
                    maxSizeInBytes =
                        ((sizeInCharacters * sizeof(char)) > actualSizeInBytes)
                            ? sizeInCharacters * sizeof(char)
                            : actualSizeInBytes;
                else
                {
                    // Notes:
                    ...
                    else
                    {
                        ...
                        { // widening the SqlDbType is automatic
                            ...
                            default:
                                Debug.Assert(
                                    false,
                                    "Missed metatype in SqlCommand.BuildParamList()"
                                );
                                break;
                        }
                    }
        ...
        internal static string[] ParseTypeName(string typeName, bool isUdtTypeName)
        {
            ...
            try
            {
                ...
                return MultipartIdentifier.ParseMultipartIdentifier(
                    typeName,
                    "[\"",
                    "]\"",
                    '.',
        ...
        internal sealed class SqlParameterConverter : ExpandableObjectConverter
        {
            ...
            )
            {
                ...
                if (
                    ...
                    || !ADP.IsEmpty(p.XmlSchemaCollectionOwningSchema)
                    || !ADP.IsEmpty(p.XmlSchemaCollectionName)
                )
                {
                ...
                System.Reflection.ConstructorInfo ctor = typeof(SqlParameter).GetConstructor(
                    ctorParams
                );
