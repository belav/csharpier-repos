...
namespace System.Data.SqlClient
{
    ...
    internal sealed class SqlBuffer
    {
        ...
        {
            get { return (StorageType.Empty == _type); }
        }
            ...
            get
            {
                ...
                if (StorageType.DateTime == _type)
                {
                    return SqlDateTime.ToDateTime(
                        _value._dateTimeInfo.daypart,
                        _value._dateTimeInfo.timepart
                    );
                }
        ...
        internal string KatmaiDateTimeString
        {
            get
            {
                ...
                if (StorageType.Time == _type)
                {
                    ...
                    return new DateTime(_value._timeInfo.ticks).ToString(
                        __katmaiTimeFormatByScale[scale],
                ...
                if (StorageType.DateTimeOffset == _type)
                {
                    ...
                    return dto.ToString(
                        __katmaiDateTimeOffsetFormatByScale[scale],
                        DateTimeFormatInfo.InvariantInfo
                    );
                }
                return (String)this.Value; // anything else we haven't thought of goes through boxing.
            ...
            get
            {
                ...
                if (StorageType.DateTimeOffset == _type)
                {
                    ...
                    return new DateTimeOffset(
                        GetTicksFromDateTime2Info(_value._dateTimeOffsetInfo.dateTime2Info)
                            + offset.Ticks,
                        offset
                    );
                }
        ...
        internal void SetToDateTime2(byte[] bytes, int length, byte scale, byte denormalizedScale)
        {
            ...
            FillInTimeInfo(
                ref _value._dateTime2Info.timeInfo,
                bytes,
                length - 3,
                scale,
                denormalizedScale
        ...
        )
        {
            ...
            FillInTimeInfo(
                ref _value._dateTimeOffsetInfo.dateTime2Info.timeInfo,
                bytes,
                length - 5,
                scale,
                denormalizedScale
            ...
            _value._dateTimeOffsetInfo.offset = (Int16)(
                bytes[length - 2] + (bytes[length - 1] << 8)
            ); // 2 bytes for offset (Int16)
            _isNull = false;
        }
        ...
        internal void SetToDateTimeOffset(DateTimeOffset dateTimeOffset, byte scale)
        {
            ...
            _value._dateTimeOffsetInfo.dateTime2Info.date = utcDateTime
        ...
        private static void FillInTimeInfo(
            ref TimeInfo timeInfo,
            byte[] timeBytes,
            int length,
            byte scale,
            byte denormalizedScale
        )
        {
            ...
            Debug.Assert(
                0 <= denormalizedScale && denormalizedScale <= 7,
                "invalid denormalized scale: " + denormalizedScale
            );

            Int64 tickUnits =
                (Int64)timeBytes[0] + ((Int64)timeBytes[1] << 8) + ((Int64)timeBytes[2] << 16);
            if (length > 3)
            {
