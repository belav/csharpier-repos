...
namespace System.Data.SqlClient
{
    ...
    internal class SessionData
    {
        ...
        public void AssertUnrecoverableStateCountIsCorrect()
        {
            ...
            Debug.Assert(
                unrecoverableCount == _unrecoverableStatesCount,
    ...
    internal sealed class SqlInternalConnectionTds : SqlInternalConnection, IDisposable
    {
        ...
        internal class SyncAsyncLock
        {
            ...
            {
                get { return semaphore.CurrentCount == 0; }
            }
            ...
            : base(connectionOptions)
        {
...
#endif
            Debug.Assert(
                reconnectSessionData == null || connectionOptions.ConnectRetryCount > 0,
            ...
            Debug.Assert(
                credential == null || !connectionOptions.IntegratedSecurity,
                "Cannot use SqlCredential and Integrated Security"
            );
            Debug.Assert(
                credential == null || !connectionOptions.ContextConnection,
            ...
            try
            {
...
#else
                {
                    ...
                    int connectionEstablishCount = applyTransientFaultHandling
                        ...
                        : 1;
                    int transientRetryIntervalInMilliSeconds =
                        connectionOptions.ConnectRetryInterval * 1000; // Max value of transientRetryInterval is 60*1000 ms. The max value allowed for ConnectRetryInterval is 60
                    for (int i = 0; i < connectionEstablishCount; i++)
                    {
                        ...
                        {
                            if (
                                i + 1 == connectionEstablishCount
                                || !applyTransientFaultHandling
                                || _timeout.IsExpired
                                || _timeout.MillisecondsRemaining
                                    < transientRetryIntervalInMilliSeconds
        ...
        {
            get { return _clientConnectionId; }
        }

        internal Guid OriginalClientConnectionId
        {
            get { return _originalClientConnectionId; }
        }
        ...
        {
            get { return _currentFailoverPartner; }
        }
            ...
            get
            {
                return (
                    String.Format(
                        ...
                        "{0:00}.{1:00}.{2:0000}",
                        _loginAck.majorVersion,
                        (short)_loginAck.minorVersion,
                        _loginAck.buildNum
                    )
                );
            }
        ...
        protected override void ChangeDatabaseInternal(string database)
        {
            ...
            Threading.Tasks.Task executeTask = _parser.TdsExecuteSQLBatch(
                "use " + database,
        ...
        public override void Dispose()
        {
            ...
            {
                Bid.Trace(
                    "<sc.SqlInternalConnectionTds.Dispose|ADV> %d# disposing\n",
                    base.ObjectID
                );
            }
            try
            {
                ...
                Debug.Assert(
                    parser != null && _fConnectionOpen || parser == null && !_fConnectionOpen,
        ...
        internal override void ValidateConnectionForExecute(SqlCommand command)
        {
            ...
            if (
                ...
                || (parser.State == TdsParserState.Closed)
            )
            {
            ...
            else
            {
                ...
                Debug.Assert(
                    !parser._physicalStateObj._pendingData,
        ...
        internal void CheckEnlistedTransactionBinding()
        {
            ...
            if (enlistedTransaction != null)
            {
                bool requireExplicitTransactionUnbind =
                    ...
                    == SqlConnectionString.TransactionBindingEnum.ExplicitUnbind;
                ...
                if (requireExplicitTransactionUnbind)
                {
                    ...
                    if (
                        ...
                        || !enlistedTransaction.Equals(currentTransaction)
                    )
                    {
        ...
        protected override void InternalDeactivate()
        {
            ...
            {
                Debug.Assert(
                    null != _parser || IsConnectionDoomed,
        ...
        private void ResetConnection()
        {
            ...
            Debug.Assert(
                !_parser._physicalStateObj._pendingData,
            ...
            if (_fResetConnection)
            {
                ...
                if (IsShiloh)
                {
                    ...
                    _parser.PrepareResetConnection(
                        IsTransactionRoot && !IsNonPoolableTransactionRoot
                    );
                }
        ...
        )
        {
            ...
            if (
                transactionRequest == TransactionRequest.Commit
                || transactionRequest == TransactionRequest.Rollback
                || transactionRequest == TransactionRequest.IfRollback
            )
            {
        ...
        )
        {
            ...
            {
                Debug.Assert(
                    null != internalTransaction,
                    "Begin Transaction request without internal transaction"
                );
        ...
        )
        {
            ...
            Debug.Assert(
                !ThreadHasParserLockForClose || _parserLock.ThreadMayHaveLock(),
            ...
            try
            {
                ...
                if (
                    ...
                    && internalTransaction.RestoreBrokenConnection
        ...
        private void CompleteLogin(bool enlistOK)
        {
            ...
            { // ROR should not affect state of connection recovery
                ...
                {
                    Bid.Trace(
                        "<sc.SqlInternalConnectionTds.CompleteLogin|ERR> %d#, Server did not acknowledge the federated authentication request\n",
                        ObjectID
                    );
            ...
            Debug.Assert(
                SniContext.Snix_Login == Parser._physicalStateObj.SniContext,
                String.Format(
                    (IFormatProvider)null,
                    "SniContext should be Snix_Login; actual Value: {0}",
                    Parser._physicalStateObj.SniContext
                )
            );
        ...
        )
        {
            ...
            requestedFeatures |=
                TdsEnums.FeatureExtension.Tce | TdsEnums.FeatureExtension.GlobalTransactions;
            _parser.TdsLogin(
                login,
                requestedFeatures,
        ...
        )
        {
            ...
            timeoutErrorInternal.SetInternalSourceType(
                useFailoverPartner
        ...
        {
            Debug.Assert(
                object.ReferenceEquals(connectionOptions, this.ConnectionOptions),
                "ConnectionOptions argument and property must be the same"
            ); // consider removing the argument
            ...
            if (isParallel)
            {
                ...
                if (timeout.IsInfinite)
                {
                    timeoutUnitInterval = checked(
                        (long)(failoverTimeoutStep * (1000L * ADP.DefaultConnectionTimeout))
                    );
                }
                else
                {
                    timeoutUnitInterval = checked(
                        (long)(failoverTimeoutStep * timeout.MillisecondsRemaining)
                    );
                }
            }
            ...
            while (true)
            {
                Boolean isFirstTransparentAttempt =
                ...
                if (isParallel)
                {
                    ...
                    {
                        nextTimeoutInterval = Math.Max(
                            ADP.MinimumTimeoutForTnirMs,
                            nextTimeoutInterval
                        );
                    }
                ...
                Debug.Assert(
                    SniContext.Undefined == Parser._physicalStateObj.SniContext,
                    String.Format(
                        (IFormatProvider)null,
                        "SniContext should be Undefined; actual Value: {0}",
                        Parser._physicalStateObj.SniContext
                    )
                );
                ...
                try
                {
                    ...
                    if (
                        connectionOptions.MultiSubnetFailover
                        && null != ServerProvidedFailOverPartner
                    )
                    {
                    ...
                    {
                        Bid.Trace(
                            "<sc.SqlInternalConnectionTds.LoginNoFailover> Routed to %ls",
                ...
                if (null != ServerProvidedFailOverPartner)
                {
                    ...
                    Debug.Assert(
                        ConnectionOptions.ApplicationIntent != ApplicationIntent.ReadOnly,
                        "FAILOVER+AppIntent=RO: Should already fail (at LOGSHIPNODE in OnEnvChange)"
                    );
                ...
                {
                    Bid.Trace(
                        "<sc.SqlInternalConnectionTds.LoginNoFailover|ADV> %d#, sleeping %d{milisec}\n",
                        ObjectID,
                        sleepInterval
                    );
                }
            ...
            if (null != PoolGroupProviderInfo)
            {
                ...
                PoolGroupProviderInfo.FailoverCheck(
                    this,
                    false,
                    connectionOptions,
                    ServerProvidedFailOverPartner
        ...
        private bool ShouldDisableTnir(SqlConnectionString connectionOptions)
        {
            ...
            Boolean isFedAuthEnabled =
                this._accessTokenInBytes != null
                || connectionOptions.Authentication
            ...
            bool isTnirExplicitlySpecifiedInConnectionOptions =
                connectionOptions.Parsetable[SqlConnectionString.KEY.TransparentNetworkIPResolution]
                != null;

            return isTnirExplicitlySpecifiedInConnectionOptions
                ? false
                : (isAzureEndPoint || isFedAuthEnabled);
        }
        ...
        )
        {
            ...
            {
                Bid.Trace(
                    "<sc.SqlInternalConnectionTds.LoginWithFailover|ADV> %d#, useFailover=%d{bool}, primary=",
                    ObjectID,
                    useFailoverHost
                );
            ...
            { // No point in resolving the failover partner when we're going to override it below
                ...
                ResolveExtendedServerName(
                    failoverServerInfo,
                    !redirectedUserInstance && failoverHost != primaryServerInfo.UserServerName,
                    connectionOptions
                );
            }
            ...
            {
                timeoutUnitInterval = checked(
                    (long)ADP.FailoverTimeoutStep
                    * ADP.TimerFromSeconds(ADP.DefaultConnectionTimeout)
                );
            }
            else
            {
                timeoutUnitInterval = checked(
                    (long)(ADP.FailoverTimeoutStep * timeout.MillisecondsRemaining)
                );
            }
            ...
            while (true)
            {
                ...
                TimeoutTimer intervalTimer = TimeoutTimer.StartMillisecondsTimeout(
                    nextTimeoutInterval
                );
                ...
                if (useFailoverHost)
                {
                    ...
                    if (
                        null != ServerProvidedFailOverPartner
                        && failoverServerInfo.ResolvedServerName != ServerProvidedFailOverPartner
                    )
                    {
                        if (Bid.AdvancedOn)
                        {
                            Bid.Trace(
                                "<sc.SqlInternalConnectionTds.LoginWithFailover|ADV> %d#, new failover partner=%ls\n",
                                ObjectID,
                                ServerProvidedFailOverPartner
                            );
                        }
                ...
                try
                {
                    ...
                    if (_routingInfo != null)
                    {
                        ...
                        Bid.Trace(
                            "<sc.SqlInternalConnectionTds.LoginWithFailover> Routed to %ls",
                            _routingInfo.ServerName
                        );
                ...
                if (1 == attemptNumber % 2)
                {
                    ...
                    {
                        Bid.Trace(
                            "<sc.SqlInternalConnectionTds.LoginWithFailover|ADV> %d#, sleeping %d{milisec}\n",
                            ObjectID,
                            sleepInterval
                        );
                    }
        ...
        private void ResolveExtendedServerName(
            ServerInfo serverInfo,
            bool aliasLookup,
            SqlConnectionString options
        )
        {
            if (serverInfo.ExtendedServerName == null)
            {
                ...
                { // We skip this for UserInstances...
                    ...
                    if (_currentSessionData != null && !string.IsNullOrEmpty(host))
                    {
                        ...
                        if (
                            _currentSessionData._resolvedAliases.TryGetValue(host, out hostPortPair)
                        )
                        {
        ...
        )
        {
            ...
            {
                Bid.Trace(
                    "<sc.SqlInternalConnectionTds.AttemptOneLogin|ADV> %d#, timout=%I64d{msec}, server=",
                    ObjectID,
                    timeout.MillisecondsRemaining
                );
        ...
        protected override object ObtainAdditionalLocksForClose()
        {
            ...
            Debug.Assert(
                obtainParserLock || _parserLock.ThreadMayHaveLock(),
        ...
        internal void BreakConnection()
        {
            ...
            Bid.Trace(
                "<sc.SqlInternalConnectionTds.BreakConnection|RES|CPOOL> %d#, Breaking connection.\n",
                ObjectID
            );
        ...
        internal void OnEnvChange(SqlEnvChange rec)
        {
            Debug.Assert(
                !IgnoreEnvChange,
                "This function should not be called if IgnoreEnvChange is set!"
            );
            switch (rec.type)
            {
                ...
                case TdsEnums.ENV_ROUTING:
                    ...
                    if (
                        string.IsNullOrEmpty(rec.newRoutingInfo.ServerName)
                        || rec.newRoutingInfo.Protocol != 0
                        || rec.newRoutingInfo.Port == 0
                    )
                    {
        ...
        internal void OnFedAuthInfo(SqlFedAuthInfo fedAuthInfo)
        {
            ...
            if (_dbConnectionPool != null)
            {
                ...
                _dbConnectionPoolAuthenticationContextKey =
                    new DbConnectionPoolAuthenticationContextKey(
                        fedAuthInfo.stsurl,
                        fedAuthInfo.spn
                    );
                ...
                if (
                    _dbConnectionPool.AuthenticationContexts.TryGetValue(
                        _dbConnectionPoolAuthenticationContextKey,
                ...
                )
                {
                    ...
                    TimeSpan contextValidity =
                        dbConnectionPoolAuthenticationContext.ExpirationTime.Subtract(
                            DateTime.UtcNow
                        );
                    ...
                    if (contextValidity <= _dbAuthenticationContextUnLockedRefreshTimeSpan)
                    {
                        Bid.Trace(
                            ...
                            ObjectID,
                            dbConnectionPoolAuthenticationContext.ExpirationTime.ToLongTimeString(),
                    ...
                    else if (contextValidity <= _dbAuthenticationContextLockedRefreshTimeSpan)
                    {
                        ...
                        Debug.Assert(
                            !attemptRefreshTokenLocked || fedAuthToken != null,
                            "Either Lock should not have been obtained or fedAuthToken should not be null."
                        );
                        Debug.Assert(
                        ...
                        {
                            Bid.Trace(
                                "<sc.SqlInternalConnectionTds.OnFedAuthInfo> %d#, The attempt to get a new access token succeeded under the locked mode."
                            );
                        }
                    }
                    ...
                    {
                        Bid.Trace(
                            "<sc.SqlInternalConnectionTds.OnFedAuthInfo> %d#, Found an authentication context in the cache that does not need a refresh at this time. Re-using the cached token.\n",
                            ObjectID
                        );
                    }
                }
            ...
            Debug.Assert(
                fedAuthToken != null && fedAuthToken.accessToken != null,
                "fedAuthToken and fedAuthToken.accessToken cannot be null."
            );
        ...
        /// <returns></returns>
        internal bool TryGetFedAuthTokenLocked(
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    Bid.Trace(
                        "<sc.SqlInternalConnectionTds.TryGetFedAuthTokenLocked> %d#, Acquired the lock to update the authentication context.The expiration time is %s. Current Time is %s.\n",
                        ObjectID,
                        dbConnectionPoolAuthenticationContext.ExpirationTime.ToLongTimeString(),
                        DateTime.UtcNow.ToLongTimeString()
                    );
        ...
        internal SqlFedAuthToken GetFedAuthToken(SqlFedAuthInfo fedAuthInfo)
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    )
                    {
                        ...
                        fedAuthToken.accessToken =
                            ADALNativeWrapper.ADALGetAccessTokenForWindowsIntegrated(
                                fedAuthInfo.stsurl,
                                fedAuthInfo.spn,
                                _clientConnectionId,
                    ...
                    else if (_credential != null)
                    {
                        ...
                        fedAuthToken.accessToken = ADALNativeWrapper.ADALGetAccessToken(
                            _credential.UserId,
                    ...
                    if (_forceAdalRetry)
                    {
                        ...
                        throw new AdalException(
                            "Force retry in GetFedAuthToken",
                            ActiveDirectoryAuthentication.GetAccessTokenTansisentError,
                            3399614468,
                            6
                        );
                    }
                ...
                catch (AdalException adalException)
                {
                    ...
                    if (
                        ActiveDirectoryAuthentication.GetAccessTokenTansisentError != errorCategory
                        || _timeout.IsExpired
                    ...
                    )
                    {
                        ...
                        Bid.Trace(
                            "<sc.SqlInternalConnectionTds.GetFedAuthToken.ADALException category:> %d#  <error:> %s#\n",
                            (int)errorCategory,
                            errorStatus
                        );
                        ...
                        sqlErs.Add(
                            new SqlError(
                                ...
                                ActiveDirectoryAuthentication.AdalGetAccessTokenFunctionName,
                                0
                            )
                        );

                        // Error[1]
                        string errorMessage1 = Res.GetString(
                            Res.SQL_ADALInnerException,
                            errorStatus,
                            adalException.GetState()
                        );
                        sqlErs.Add(
                            new SqlError(
                                0,
                    ...
                    Bid.Trace(
                        "<sc.SqlInternalConnectionTds.GetFedAuthToken|ADV> %d#, sleeping %d{Milliseconds}\n",
                        ObjectID,
                        sleepInterval
                    );
                    Bid.Trace(
            ...
            Debug.Assert(fedAuthToken != null, "fedAuthToken should not be null.");
            Debug.Assert(
                fedAuthToken.accessToken != null && fedAuthToken.accessToken.Length > 0,
            ...
            if (_dbConnectionPool != null)
            {
                ...
                _newDbConnectionPoolAuthenticationContext =
                    new DbConnectionPoolAuthenticationContext(
                        fedAuthToken.accessToken,
                        expirationTime
                    );
            }
        ...
        internal void OnFeatureExtAck(int featureId, byte[] data)
        {
            ...
            switch (featureId)
            {
                case TdsEnums.FEATUREEXT_SRECOVERY:
                {
                    ...
                    if (!_sessionRecoveryRequested)
                    {
                        throw SQL.ParsingErrorFeatureId(
                            ParsingErrorState.UnrequestedFeatureAckReceived,
                            featureId
                        );
                    }
...
#endif
                    Debug.Assert(
                        _currentSessionData._unrecoverableStatesCount == 0,
                ...
                case TdsEnums.FEATUREEXT_FEDAUTH:
                {
                    ...
                    Debug.Assert(
                        _fedAuthFeatureExtensionData != null,
                    ...
                    switch (_fedAuthFeatureExtensionData.Value.libraryType)
                    {
                        ...
                        case TdsEnums.FedAuthLibrary.SecurityToken:
                            ...
                            if (data.Length != 0)
                            {
                                ...
                                throw SQL.ParsingError(
                                    ParsingErrorState.FedAuthFeatureAckContainsExtraData
                                );
                            }
                            break;
                        ...
                        default:
                            ...
                            throw SQL.ParsingErrorLibraryType(
                                ...
                                (int)_fedAuthFeatureExtensionData.Value.libraryType
                            );
                    }
                    ...
                    if (_newDbConnectionPoolAuthenticationContext != null)
                    {
                        ...
                        Debug.Assert(
                            newAuthenticationContextInCacheAfterAddOrUpdate != null,
                        ...
                        if (
                            newAuthenticationContextInCacheAfterAddOrUpdate
                            == _newDbConnectionPoolAuthenticationContext
                        )
                        {
                            Bid.Trace(
                                "<sc.SqlInternalConnectionTds.OnFeatureExtAck|ERR> %d#, Updated the new dbAuthenticationContext in the _dbConnectionPool.AuthenticationContexts. \n",
                                ObjectID
                            );
                        }
                        else
                        {
                            Bid.Trace(
                                "<sc.SqlInternalConnectionTds.OnFeatureExtAck|ERR> %d#, AddOrUpdate attempted on _dbConnectionPool.AuthenticationContexts, but it did not update the new value. \n",
                                ObjectID
                            );
                        }
#endif
                ...
                case TdsEnums.FEATUREEXT_TCE:
                {
                    ...
                    if (
                        0 == supportedTceVersion
                        || supportedTceVersion > TdsEnums.MAX_SUPPORTED_TCE_VERSION
                    )
                    {
                        Bid.Trace(
                        ...
                        throw SQL.ParsingErrorValue(
                            ParsingErrorState.TceInvalidVersion,
                            supportedTceVersion
                        );
                    }
                    ...
                    Debug.Assert(
                        _tceVersionSupported == TdsEnums.MAX_SUPPORTED_TCE_VERSION,
                        "Client support TCE version 1"
                    );
        ...
        {
            get { return _threadIdOwningParserLock == Thread.CurrentThread.ManagedThreadId; }
            set
            {
                Debug.Assert(
                    _parserLock.ThreadMayHaveLock(),
    ...
    internal sealed class ServerInfo
    {
        ...
        {
            get { return m_userServerName; }
            private set { m_userServerName = value; }
        }
