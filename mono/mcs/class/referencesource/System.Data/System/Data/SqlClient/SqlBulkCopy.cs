...
namespace System.Data.SqlClient
{
    ...
    sealed internal class BulkCopySimpleResultSet
    {
        ...
        {
            get { return (Result)_results[idx]; }
        }
    ...
    public sealed class SqlBulkCopy : IDisposable
    {
        ...
        {
            set { _setAlwaysTaskOnWrite = value; }
            get { return _setAlwaysTaskOnWrite; }
        }
#endif
            ...
            : this(connection)
        {
            ...
            if (
                ...
                && IsCopyOption(SqlBulkCopyOptions.UseInternalTransaction)
            )
            {
        ...
        {
            add { _rowsCopiedEventHandler += value; }
            remove { _rowsCopiedEventHandler -= value; }
        }
        ...
        private string CreateInitialQuery()
        {
            ...
            try
            {
                parts = MultipartIdentifier.ParseMultipartIdentifier(
                    this.DestinationTableName,
                    "[\"",
                    "]\"",
                    Res.SQL_BulkCopyDestinationTableName,
                    true
                );
            }
            catch (Exception e)
            ...
            TDSCommand =
                "select @@trancount; SET FMTONLY ON select * from "
                + this.DestinationTableName
                + " SET FMTONLY OFF ";
#if !PROJECTK
        ...
        private Task<BulkCopySimpleResultSet> CreateAndExecuteInitialQueryAsync(
            out BulkCopySimpleResultSet result
        )
        {
...
#if !PROJECTK
            Bid.Trace(
                "<sc.SqlBulkCopy.CreateAndExecuteInitialQueryAsync|INFO> Initial Query: '%ls' \n",
                TDSCommand
            );
            ...
            Task executeTask = _parser.TdsExecuteSQLBatch(
                ...
                null,
                _stateObj,
                sync: !_isAsyncBulkCopy,
                callerHasConnectionLock: true
            );
#else
            Task executeTask = _parser.TdsExecuteSQLBatch(
                TDSCommand,
            ...
            else
            {
                ...
                result = null;
                return executeTask.ContinueWith<BulkCopySimpleResultSet>(
                    t =>
                    {
        ...
        //
        private string AnalyzeTargetAndCreateUpdateBulkCommand(
        ...
        )
        {
            ...
            Debug.Assert(
                ...
                "Where are the results from the initial query?"
            );
            ...
            else
            {
                isInTransaction = (bool)(
                    0
                    < (SqlInt32)(
                        internalResults[TranCountResultId][TranCountRowId][TranCountValueId]
                    )
                );
            }
#endif //PROJECTK
            ...
            for (int i = 0; i < metaDataSet.Length; i++)
            {
                ...
                //
                if (
                    (metadata.type == SqlDbType.Timestamp)
                ...
                for (assocId = 0; assocId < _localColumnMappings.Count; assocId++)
                {
                    if (
                        (
                            _localColumnMappings[assocId]._destinationColumnOrdinal
                            == metadata.ordinal
                        )
                        || (
                            UnquotedName(_localColumnMappings[assocId]._destinationColumnName)
                            == metadata.column
                        )
                    )
                    {
                        ...
                        _sortedColumnMappings.Add(
                            new _ColumnMapping(
                                _localColumnMappings[assocId]._internalSourceColumnOrdinal,
                                metadata
                            )
                        );
                        nmatched++;
                        ...
                        else
                        {
                            ...
                            AppendColumnNameAndTypeName(
                                updateBulkCommandText,
                                metadata.column,
                                typeof(SqlDbType).GetEnumName(metadata.type)
                            );
#else
                            AppendColumnNameAndTypeName(
                                updateBulkCommandText,
                        ...
                        switch (metadata.metaType.NullableType)
                        {
                            ...
                            case TdsEnums.SQLDECIMALN:
                                ...
                                //
                                updateBulkCommandText.AppendFormat(
                                    (IFormatProvider)null,
                                    "({0},{1})",
                                    metadata.precision,
                                    metadata.scale
            ...
            )
            {
                ...
                {
                    updateBulkCommandText.Append(
                        (addSeparator ? ", " : "") + "ALLOW_ENCRYPTED_VALUE_MODIFICATIONS"
                    );
                    addSeparator = true;
                }
                updateBulkCommandText.Append(")");
...
#if !PROJECTK
            Bid.CorrelationTrace(
                "<sc.SqlBulkCopy.SubmitUpdateBulkCommand|Info|Correlation> ObjectID%d#, ActivityID %ls\n",
                ObjectID
            );
                ...
                try
                {
                    ...
                    try
                    {
                        Debug.Assert(
                            ...
                            "Internal transaction exists during dispose"
                        );
        ...
        //
        private object GetValueFromSourceRow(
            int destRowIndex,
            out bool isSqlType,
            out bool isDataFeed,
            out bool isNull
            ...
            switch (_rowSourceType)
            {
                            ...
                            switch (_currentRowMetadata[destRowIndex].Method)
                            {
                                case ValueMethod.DataFeedStream:
                                    return new StreamDataFeed(
                                        _DbDataReaderRowSource.GetStream(sourceOrdinal)
                                    );
                                case ValueMethod.DataFeedText:
                                    return new TextDataFeed(
                                        _DbDataReaderRowSource.GetTextReader(sourceOrdinal)
                                    );
                                case ValueMethod.DataFeedXml:
                                    ...
                                    Debug.Assert(
                                        ...
                                        "Should not be reading row as an XmlReader if bulk copy source is not a SqlDataReader"
                                    );
                                    return new XmlDataFeed(
                                        _SqlDataReaderRowSource.GetXmlReader(sourceOrdinal)
                                    );
                                default:
                                    Debug.Assert(
                                        false,
                                        string.Format(
                                            "Current column is marked as being a DataFeed, but no DataFeed compatible method was provided. Method: {0}",
                                            _currentRowMetadata[destRowIndex].Method
                                        )
                                    );
                                    isDataFeed = false;
                                    object columnValue = _DbDataReaderRowSource.GetValue(
                                        sourceOrdinal
                                    );
                            ...
                            switch (_currentRowMetadata[destRowIndex].Method)
                            {
                                ...
                                default:
                                    Debug.Assert(
                                        false,
                                        string.Format(
                                            "Current column is marked as being a SqlType, but no SqlType compatible method was provided. Method: {0}",
                                            _currentRowMetadata[destRowIndex].Method
                                        )
                                    );
                                    value = (INullable)
                        ...
                        else
                        {
                            ...
                            else if (!isNull)
                            {
                                Debug.Assert(
                                    ...
                                    "IsDBNull returned false, but GetValue returned a null INullable"
                                );
                            }
#endif
                            return value;
                ...
                var rowSourceAsIDataReader = _DbDataReaderRowSource;
                        ...
                        if (
                            (_enableStreaming)
                            && (_SqlDataReaderRowSource == null)
                            && (rowSourceAsIDataReader.IsDBNull(sourceOrdinal))
                        )
                        {
                            isSqlType = false;
                            isNull = true;
                ...
                case ValueSourceType.RowArray:
                {
                    ...
                    Debug.Assert(
                        sourceOrdinal < _currentRowLength,
        ...
        private bool ReadFromRowSource()
        {
            switch (_rowSourceType)
            {
                ...
                case ValueSourceType.DataTable:
                    ...
                    Debug.Assert(
                        (_rowStateToSkip & DataRowState.Deleted) != 0,
                        "Deleted is a permitted rowstate?"
                    );
        ...
        private SourceColumnMetadata GetColumnMetadata(int ordinal)
        {
...
#if !PROJECTK
            if (
                ((_SqlDataReaderRowSource != null) || (_dataTableSource != null))
                && (
                    (metadata.metaType.NullableType == TdsEnums.SQLDECIMALN)
                    || (metadata.metaType.NullableType == TdsEnums.SQLNUMERICN)
                )
            )
            {
#else
            ...
            )
            {
                ...
                switch (_rowSourceType)
                {
                    ...
                    default:
                        ...
                        Debug.Assert(
                            false,
                            string.Format("Unknown value source: {0}", _rowSourceType)
                        );
                        break;
                }
#endif //PROJECTK
            ...
            else if ((_enableStreaming) && (metadata.length == MAX_LENGTH))
            {
                ...
                if (_SqlDataReaderRowSource != null)
                {
                    ...
                    if (
                        (metadata.type == SqlDbType.VarBinary)
                        && (mtSource.IsBinType)
                        && (mtSource.SqlDbType != SqlDbType.Timestamp)
                        && _SqlDataReaderRowSource.IsCommandBehavior(
                    ...
                    // For text and XML there is memory gain from streaming on destination side even if reader is non-sequential
                        ...
                        (
                            (metadata.type == SqlDbType.VarChar)
                            || (metadata.type == SqlDbType.NVarChar)
                        )
                        && (mtSource.IsCharType)
                    ...
                    )
                    {
                        ...
                        (metadata.type == SqlDbType.Xml) && (mtSource.SqlDbType == SqlDbType.Xml)
                    )
                    {
                        isDataFeed = true;
        ...
        //
        private void AppendColumnNameAndTypeName(
            StringBuilder query,
            string columnName,
            string typeName
        )
            ...
            try
            {
                ...
                switch (type.NullableType)
                {
                    ...
                    case TdsEnums.SQLDECIMALN:
                        ...
                        value = SqlParameter.CoerceValue(
                            value,
                            mt,
                            out coercedToDataFeed,
                            out typeChanged,
                            false
                        );
                        ...
                        {
                            throw SQL.BulkLoadCannotConvertValue(
                                value.GetType(),
                                mt,
                                ADP.ParameterValueOutOfRange(sqlValue)
                            );
                        }
                        break;
                    ...
                    case TdsEnums.SQLXMLTYPE:
                        ...
                        Debug.Assert(
                            (value is XmlReader)
                                || (value is SqlCachedBuffer)
                                || (value is string)
                                || (value is SqlString)
                                || (value is XmlDataFeed),
                            "Invalid value type of Xml datatype"
                        );
                    ...
                    default:
                        Debug.Assert(
                            false,
                            "Unknown TdsType!"
                                + type.NullableType.ToString("x2", (IFormatProvider)null)
                        );
        ...
        public void WriteToServer(DataTable table, DataRowState rowState)
        {
            ...
            try
            {
                statistics = SqlStatistics.StartTimer(Statistics);
                _rowStateToSkip =
                    ((rowState == 0) || (rowState == DataRowState.Deleted))
                            ...
                            switch (this._rowSourceType)
                            {
                                ...
                                case ValueSourceType.RowArray:
                                    index = ((DataRow[])_rowSource)[0]
                                        .Table.Columns.IndexOf(unquotedColumnName);
                                    break;
                                case ValueSourceType.DbDataReader:
                                case ValueSourceType.IDataReader:
#endif //PROJECTK
                                    ...
                                    {
#if !PROJECTK
                                        index = ((IDataRecord)this._rowSource).GetOrdinal(
                                            unquotedColumnName
                                        );
#else
                                index = _DbDataReaderRowSource.GetOrdinal(unquotedColumnName);
                                    ...
                                    {
                                        throw (
                                            SQL.BulkLoadNonMatchingColumnName(unquotedColumnName, e)
                                        );
                                    }
#if !PROJECTK
                                    break;
        ...
        private Task ReadWriteColumnValueAsync(int col)
        {
            ...
            if (!isDataFeed)
            {
                ...
                { // If we are transparently encrypting
                    Debug.Assert(_parser.ShouldEncryptValuesForBulkCopy());
                    value = _parser.EncryptColumnValue(
                        value,
                        metadata,
                        metadata.column,
                        _stateObj,
            ...
            if (metadata.type != SqlDbType.Variant)
            {
                ...
                writeTask = _parser.WriteBulkCopyValue(
                    value,
                    metadata,
                    _stateObj,
                    isSqlType,
            ...
            else
            {
                ...
                {
                    variantInternalType = _SqlDataReaderRowSource.GetVariantInternalStorageType(
        ...
        private void RegisterForConnectionCloseNotification<T>(ref Task<T> outterTask)
        {
            ...
            connection.RegisterForConnectionCloseNotification<T>(
                ref outterTask,
                this,
                SqlReferenceCollection.BulkCopyTag
            );
        }
                ...
                { // > 0?
                    ...
                    { // decrement counter
                        ...
                        try
                        {
                            ...
                            {
                                exception = ADP.OpenConnectionRequired(
        ...
        private TaskCompletionSource<object> ContinueTaskPend(
            Task task,
            TaskCompletionSource<object> source,
            Func<TaskCompletionSource<object>> action
        )
        {
            if (task == null)
            {
        ...
        )
        {
            ...
            try
            {
                ...
                for (
                    i = rowsSoFar;
                    (totalRows <= 0 || i < totalRows) && _hasMoreRowToCopy == true;
                    i++
                )
                {
        ...
        // Returned Task could be null in two cases: (1) _isAsyncBulkCopy == false, or (2) _isAsyncBulkCopy == true but all async writes finished synchronously.
        //
        private Task CopyBatchesAsync(
            BulkCopySimpleResultSet internalResults,
            string updateBulkCommandText,
            CancellationToken cts,
            TaskCompletionSource<object> source = null
        )
        {
            Debug.Assert(
                source == null || !source.Task.IsCompleted,
            ...
            try
            {
                while (_hasMoreRowToCopy)
                {
                    ...
                    if (commandTask == null)
                    {
                        Task continuedTask = CopyBatchesAsyncContinued(
                            internalResults,
                            updateBulkCommandText,
                            cts,
                            source
                    ...
                    else
                    {
                        Debug.Assert(
                            _isAsyncBulkCopy,
                            "Task should not pend while doing sync bulk copy"
                        );
                        if (source == null)
                        {
                            source = new TaskCompletionSource<object>();
        ...
        private Task CopyBatchesAsyncContinued(
            BulkCopySimpleResultSet internalResults,
            string updateBulkCommandText,
            CancellationToken cts,
            TaskCompletionSource<object> source
            ...
            try
            {
                ...
                if (task != null)
                {
                    ...
                    AsyncHelper.ContinueTask(
                        ...
                        () =>
                        {
                            Task continuedTask = CopyBatchesAsyncContinuedOnSuccess(
                                internalResults,
                                updateBulkCommandText,
                                cts,
                                source
                        ...
                        _connection.GetOpenTdsConnection(),
                        _ => CopyBatchesAsyncContinuedOnError(cleanupParser: false),
                ...
                {
                    return CopyBatchesAsyncContinuedOnSuccess(
                        internalResults,
                        updateBulkCommandText,
                        cts,
                        source
        ...
        // If this returns true, then the caller is responsible for starting the next stage
        private Task CopyBatchesAsyncContinuedOnSuccess(
            BulkCopySimpleResultSet internalResults,
            string updateBulkCommandText,
            CancellationToken cts,
            TaskCompletionSource<object> source
        ...
        //
        private void WriteToServerInternalRestContinuedAsync(
            BulkCopySimpleResultSet internalResults,
            CancellationToken cts,
            TaskCompletionSource<object> source
        )
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        Debug.Fail(
                            "Unexpected exception during CleanUpstateObjectOnError (ignored)",
                            cleanupEx.ToString()
                        );
                    }
        ...
        )
        {
            ...
            try
            {
                ...
                if (reconnectTask != null)
                {
                    if (_isAsyncBulkCopy)
                    {
                        CancellationTokenRegistration regReconnectCancel =
                            new CancellationTokenRegistration();
                        TaskCompletionSource<object> cancellableReconnectTS =
                        ...
                        AsyncHelper.SetTimeoutException(
                            ...
                            () =>
                            {
                                return SQL.BulkLoadInvalidDestinationTable(
                                    _destinationTableName,
                                    SQL.CR_ReconnectTimeout()
                                );
                            },
                            CancellationToken.None
                        );
                        AsyncHelper.ContinueTask(
                            cancellableReconnectTS.Task,
                            source,
                            () =>
                            {
                            ...
                            exceptionConverter: (ex) =>
                                SQL.BulkLoadInvalidDestinationTable(_destinationTableName, ex)
                        );
                        return;
                    }
                    else
                ...
                else
                {
                    Debug.Assert(
                        internalResults != null,
                        "Executing initial query finished synchronously, but there were no results"
                    );
