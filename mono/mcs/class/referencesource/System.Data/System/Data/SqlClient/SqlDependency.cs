...
namespace System.Data.SqlClient
{
    ...
    public sealed class SqlDependency
    {
        ...
        private static Dictionary<
            string,
            Dictionary<IdentityUserNamePair, List<DatabaseServicePair>>
        > _serverUserHash = new Dictionary<
            string,
            Dictionary<IdentityUserNamePair, List<DatabaseServicePair>>
        ...
        public SqlDependency(SqlCommand command)
            : this(command, null, SQL.SqlDependencyTimeoutDefault) { }
        ...
        public SqlDependency(SqlCommand command, string options, int timeout)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependency|DEP> %d#, options: '%ls', timeout: '%d'",
                ObjectID,
                options,
                timeout
            );
        ...
        {
            get { return _dependencyFired; }
        }

        [ResCategoryAttribute(Res.DataCategory_Data), ResDescriptionAttribute(Res.SqlDependency_Id)]
        public string Id
        {
            get { return _id; }
        }
        ...
        public event OnChangeEventHandler OnChange
        {
            ...
            add
            {
                ...
                Bid.NotificationsScopeEnter(
                    out hscp,
                    "<sc.SqlDependency.OnChange-Add|DEP> %d#",
                    ObjectID
                );
                try
                {
            ...
            remove
            {
                ...
                Bid.NotificationsScopeEnter(
                    out hscp,
                    "<sc.SqlDependency.OnChange-Remove|DEP> %d#",
                    ObjectID
                );
                try
                {
        ...
        public void AddCommandDependency(SqlCommand command)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependency.AddCommandDependency|DEP> %d#",
                ObjectID
            );
            try
            {
        ...
        private static void ObtainProcessDispatcher()
        {
            ...
            if (nativeStorage == null)
            {
                ...
                Bid.NotificationsTrace(
                    "<sc.SqlDependency.ObtainProcessDispatcher|DEP> AppDomain.CurrentDomain.FriendlyName: %ls\n",
                ...
                if (null != masterDomain)
                {
                    ...
                    if (null != handle)
                    {
                        SqlDependencyProcessDispatcher dependency = (SqlDependencyProcessDispatcher)
                            handle.Unwrap();
                ...
                else
                {
                    ...
                    throw ADP.InternalError(
                        ADP.InternalErrorCode.SqlDependencyProcessDispatcherFailureAppDomain
                    );
                }
            }
            else
        ...
        private static SqlDependencyProcessDispatcher GetDeserializedObject(
            BinaryFormatter formatter,
            MemoryStream stream
        )
        {
            ...
            Debug.Assert(
                result.GetType() == typeof(SqlDependencyProcessDispatcher),
                "Unexpected type stored in native!"
            );
        ...
        internal static bool Start(string connectionString, string queue, bool useDefaults)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependency.Start|DEP> AppDomainKey: '%ls', queue: '%ls'",
                AppDomainKey,
                queue
            );
            try
                    ...
                    try
                    {
                        ...
                        { // Default listener.
                            ...
                            { // CER to ensure that if Start succeeds we add to hash completing setup.
                                ...
                                result = _processDispatcher.StartWithDefault(
                                    connectionString,
                                    out server,
                                    out identity,
                                    out user,
                                ...
                                Bid.NotificationsTrace(
                                    "<sc.SqlDependency.Start|DEP> Start (defaults) returned: '%d', with service: '%ls', server: '%ls', database: '%ls'\n",
                                    result,
                                    service,
                                    server,
                                    database
                            ...
                            finally
                            {
                                ...
                                { // If success, add to hashtable.
                                    IdentityUserNamePair identityUser = new IdentityUserNamePair(
                                        identity,
                                        user
                                    );
                                    DatabaseServicePair databaseService = new DatabaseServicePair(
                                        database,
                                        service
                                    );
                        ...
                        { // Start with specified service/queue & database.
                            result = _processDispatcher.Start(
                                connectionString,
                                queue,
                                _appDomainKey,
                                SqlDependencyPerAppDomainDispatcher.SingletonInstance
        ...
        )
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependency.Stop|DEP> AppDomainKey: '%ls', queue: '%ls'",
                AppDomainKey,
                queue
            );
            try
                        ...
                        try
                        {
                            ...
                            if (useDefaults)
                            {
                                ...
                                { // CER to ensure that if Stop succeeds we remove from hash completing teardown.
                                    // Start using process wide default service/queue & database from connection string.
                                    result = _processDispatcher.Stop(
                                        connectionString,
                                        out server,
                                        out identity,
                                        out user,
                                ...
                                finally
                                {
                                    ...
                                    { // If success, remove from hashtable.
                                        Debug.Assert(
                                            !ADP.IsEmpty(server) && !ADP.IsEmpty(database),
                                            "Server or Database null/Empty upon successfull Stop()!"
                                        );
                                        IdentityUserNamePair identityUser =
                                            new IdentityUserNamePair(identity, user);
                                        DatabaseServicePair databaseService =
                                            new DatabaseServicePair(database, service);
                                        RemoveFromServerUserHash(
                                            server,
                                            identityUser,
                                            databaseService
                                        );
        ...
        private static bool AddToServerUserHash(
            string server,
            IdentityUserNamePair identityUser,
            DatabaseServicePair databaseService
        )
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependency.AddToServerUserHash|DEP> server: '%ls', database: '%ls', service: '%ls'",
                server,
            ...
            try
            {
                ...
                lock (_serverUserHash)
                {
                    ...
                    if (!_serverUserHash.ContainsKey(server))
                    {
                        ...
                        identityDatabaseHash =
                            new Dictionary<IdentityUserNamePair, List<DatabaseServicePair>>();
        ...
        private static void RemoveFromServerUserHash(
            string server,
            IdentityUserNamePair identityUser,
            DatabaseServicePair databaseService
        )
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependency.RemoveFromServerUserHash|DEP> server: '%ls', database: '%ls', service: '%ls'",
                server,
                databaseService.Database,
                databaseService.Service
            );
                    ...
                    if (_serverUserHash.ContainsKey(server))
                    {
                        ...
                        if (identityDatabaseHash.ContainsKey(identityUser))
                        {
                            ...
                            else
                            {
                                ...
                                Debug.Assert(
                                    false,
                                    "Unexpected state - hash did not contain database!"
                                );
                            }
                        }
                        else
        ...
        internal static string GetDefaultComposedOptions(
            string server,
            string failoverServer,
            IdentityUserNamePair identityUser,
            string database
        )
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependency.GetDefaultComposedOptions|DEP> server: '%ls', failoverServer: '%ls', database: '%ls'",
                server,
                failoverServer,
                database
            );
            try
            {
                ...
                lock (_serverUserHash)
                {
                    if (!_serverUserHash.ContainsKey(server))
                    {
                        ...
                        { // Special error for no calls to start.
                            ...
                            !ADP.IsEmpty(failoverServer)
                            && _serverUserHash.ContainsKey(failoverServer)
                        )
                        {
                            Bid.NotificationsTrace(
                    ...
                    { // No exact database match found.
                        ...
                        { // If only one database for this server/user, use it.
                            ...
                            Debug.Assert(
                                temp.Length == 1,
                                "If databaseList.Count==1, why does copied array have length other than 1?"
                            );
                            string quotedDatabase = FixupServiceOrDatabaseName(
                                resultingPair.Database
                            );
                            string quotedService = FixupServiceOrDatabaseName(
                                resultingPair.Service
                            );
                            result =
                                "Service=" + quotedService + ";Local Database=" + quotedDatabase;
                        }
                        else
                ...
                Debug.Assert(
                    !ADP.IsEmpty(result),
                    "GetDefaultComposedOptions should never return null or empty string!"
                );
                Bid.NotificationsTrace(
        ...
        internal void AddToServerList(string server)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependency.AddToServerList|DEP> %d#, server: '%ls'",
                ObjectID,
                server
            );
            try
            {
                lock (_serverList)
                {
                    ...
                    { // If less than 0, item was not found in list.
                        Bid.NotificationsTrace(
                            "<sc.SqlDependency.AddToServerList|DEP> Server not present in hashtable, adding server: '%ls'.\n",
                            server
                        );
        ...
        internal string ComputeHashAndAddToDispatcher(SqlCommand command)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependency.ComputeHashAndAddToDispatcher|DEP> %d#, SqlCommand: %d#",
                ObjectID,
                command.ObjectID
            );
            try
        ...
        )
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependency.Invalidate|DEP> %d#",
                ObjectID
            );
            try
            {
                ...
                lock (_eventHandlerLock)
                {
                    ...
                    )
                    {
                        ...
                        else
                        {
                            Debug.Assert(
                                false,
                                "Received notification twice - we should never enter this state!"
                            );
                            Bid.NotificationsTrace(
        ...
        internal void StartTimer(SqlNotificationRequest notificationRequest)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependency.StartTimer|DEP> %d#",
                ObjectID
            );
            try
            {
                if (_expirationTime == DateTime.MaxValue)
                {
                    ...
                    if (
                        ...
                        && notificationRequest.Timeout < seconds
                        && notificationRequest.Timeout != 0
                    )
                    {
        ...
        private void AddCommandInternal(SqlCommand cmd)
        {
            ...
            { // Don't bother with BID if command null.
                ...
                Bid.NotificationsScopeEnter(
                    out hscp,
                    "<sc.SqlDependency.AddCommandInternal|DEP> %d#, SqlCommand: %d#",
                    ObjectID,
                    cmd.ObjectID
                );
                try
                    ...
                    else
                    {
                        ...
                        lock (_eventHandlerLock)
                        {
                            ...
                            else
                            {
                                ...
                                Debug.Assert(
                                    0 == _eventList.Count,
                                    "How can we have an event at this point?"
                                );
        ...
        private string ComputeCommandHash(string connectionString, SqlCommand command)
        {
            ...
            Bid.NotificationsScopeEnter(
                out hscp,
                "<sc.SqlDependency.ComputeCommandHash|DEP> %d#, SqlCommand: %d#",
                ObjectID,
                command.ObjectID
            );
            try
            {
                ...
                for (int i = 0; i < command.Parameters.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (type == typeof(Byte[]))
                        {
                            ...
                            {
                                builder.Append(
                                    temp[j].ToString("x2", CultureInfo.InvariantCulture)
                                );
                            }
                        }
                ...
                Bid.NotificationsTrace(
                    "<sc.SqlDependency.ComputeCommandHash|DEP> ComputeCommandHash result: '%ls'.\n",
                    result
                );
                return result;
            }
