...
namespace System.Data.SqlClient
{
    ...
    internal class TdsParserSessionPool
    {
        ...
        internal TdsParserSessionPool(TdsParser parser)
        {
            ...
            {
                Bid.Trace(
                    "<sc.TdsParserSessionPool.ctor|ADV> %d# created session pool for parser %d\n",
                    ObjectID,
                    parser.ObjectID
                );
            }
        ...
        {
            get { return (null == _freeStateObjects); }
        }
        ...
        internal TdsParserStateObject GetSession(object owner)
        {
            ...
            lock (_cache)
            {
                ...
                else if (_freeStateObjectCount > 0)
                {
                    ...
                    Debug.Assert(
                        session != null,
                        "There was a null session in the free session list?"
                    );
                }
                else
                {
        ...
        internal void PutSession(TdsParserStateObject session)
        {
            ...
            lock (_cache)
            {
                if (IsDisposed)
                {
                    ...
                    Debug.Assert(
                        _cachedCount == 0,
                        "SessionPool is disposed, but there are still sessions in the cache?"
                    );
                    session.Dispose();
                }
        ...
        internal string TraceString()
        {
            return String.Format( /*IFormatProvider*/
                ...
                _objectID,
                null == _freeStateObjects
                    ? "(null)"
                    : _freeStateObjectCount.ToString((IFormatProvider)null),
                _cachedCount,
                _cache.Count
            );
        }
        ...
        {
            get { return _cachedCount - _freeStateObjectCount; }
        }
    }
}
