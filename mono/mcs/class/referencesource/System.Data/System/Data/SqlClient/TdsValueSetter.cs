...
namespace System.Data.SqlClient
{
    ...
    internal class TdsValueSetter
    {
        ...
        internal void SetDBNull()
        {
            Debug.Assert(
                !_plpUnknownSent,
                "Setting a column to null that we already stated sending!"
            );
            if (_isPlp)
            {
            ...
            else
            {
                switch (_metaData.SqlDbType)
                {
                    ...
                    case SqlDbType.Xml:
                        Debug.Assert(
                            false,
                            "PLP-only types shouldn't get to this point. Type: "
                                + _metaData.SqlDbType
                        );
                        break;
                    ...
                    case SqlDbType.Structured:
                        Debug.Assert(
                            false,
                            "Not yet implemented.  Not needed until Structured UDTs"
                        );
                        break;
                    default:
        ...
        private void SetBytesNoOffsetHandling(
            long fieldOffset,
            byte[] buffer,
            int bufferOffset,
            int length
        )
        {
            ...
            else
            {
                ...
                Debug.Assert(
                    0 == _currentOffset,
                    "SetBytes doesn't yet support chunking for non-plp data: " + _currentOffset
                );

#endif
                Debug.Assert(
                    !MetaType
                        .GetMetaTypeFromSqlDbType(_metaData.SqlDbType, _metaData.IsMultiValued)
                        .IsLong,
        ...
        internal void SetBytesLength(long length)
        {
            ...
            if (0 == length)
            {
                if (_isPlp)
                {
                    Debug.Assert(
                        !_plpUnknownSent,
                        "A plpUnknown has already been sent before setting length to zero."
                    );
        ...
        internal int SetChars(long fieldOffset, char[] buffer, int bufferOffset, int length)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    Debug.Assert(
                        0 == _currentOffset,
                        "SetChars doesn't yet support chunking for non-plp data: " + _currentOffset
                    );
#endif
        ...
        internal void SetString(string value, int offset, int length)
        {
            ...
            {
                Debug.Assert(
                    null != _variantType && SqlDbType.NVarChar == _variantType.SqlDbType,
        ...
        internal void SetDateTime(DateTime value)
        {
            ...
            else
            {
                ...
                if (SqlDbType.SmallDateTime == _metaData.SqlDbType)
                {
                    ...
                    Debug.Assert(
                        0 <= dt.days && dt.days <= UInt16.MaxValue,
                ...
                { // date and datetime2
                    ...
                    {
                        Int64 time =
                            value.TimeOfDay.Ticks / TdsEnums.TICKS_FROM_SCALE[_metaData.Scale];
                        _stateObj.WriteByteArray(
                            BitConverter.GetBytes(time),
                            (int)_metaData.MaxLength - 3,
                            0
                        );
                    }
        ...
        internal void SetGuid(Guid value)
        {
            ...
            Debug.Assert(
                SmiMetaData.DefaultUniqueIdentifier.MaxLength == bytes.Length,
                "Invalid length for guid bytes: " + bytes.Length
            );
            ...
            {
                Debug.Assert(
                    _metaData.MaxLength == bytes.Length,
                    "Unexpected uniqueid metadata length: " + _metaData.MaxLength
                );
        ...
        private void CheckSettingOffset(long offset)
        {
            ...
            Debug.Assert(
                ...
                "Invalid offset passed. Should be: " + _currentOffset + ", but was: " + offset
            );
#endif
        }
        #endregion
