    ...
    internal sealed class SqlConnectionString : DbConnectionOptions
    {
        ...
        internal static class DEFAULT
        {
            internal const ApplicationIntent ApplicationIntent =
                DbConnectionStringDefaults.ApplicationIntent;
            internal const string Application_Name = TdsEnums.SQL_PROVIDER_NAME;
            internal const bool Asynchronous = false;
            internal const string AttachDBFilename = "";
            internal const PoolBlockingPeriod PoolBlockingPeriod =
                DbConnectionStringDefaults.PoolBlockingPeriod;
            ...
            internal const bool MultiSubnetFailover =
                DbConnectionStringDefaults.MultiSubnetFailover;
            internal const bool TransparentNetworkIPResolution =
                DbConnectionStringDefaults.TransparentNetworkIPResolution;
            ...
            : base(connectionString, GetParseSynonyms(), false)
        {
            ...
            _persistSecurityInfo = ConvertValueToBoolean(
                KEY.Persist_Security_Info,
                DEFAULT.Persist_Security_Info
            );
            ...
            _multiSubnetFailover = ConvertValueToBoolean(
                KEY.MultiSubnetFailover,
                DEFAULT.MultiSubnetFailover
            );
            _transparentNetworkIPResolution = ConvertValueToBoolean(
                KEY.TransparentNetworkIPResolution,
                DEFAULT.TransparentNetworkIPResolution
            );
            ...
            _connectRetryCount = ConvertValueToInt32(
                ...
                DEFAULT.Connect_Retry_Count
            );
            _connectRetryInterval = ConvertValueToInt32(
                KEY.Connect_Retry_Interval,
                DEFAULT.Connect_Retry_Interval
            );
            ...
            _trustServerCertificate = ConvertValueToBoolean(
                ...
                DEFAULT.TrustServerCertificate
            );
            ...
            if (_contextConnection)
            {
                ...
                {
                    if (
                        (string)entry.Key != KEY.Context_Connection
            ...
            if (
                (_packetSize < TdsEnums.MIN_PACKET_SIZE) || (TdsEnums.MAX_PACKET_SIZE < _packetSize)
            )
            {
            ...
            )
            {
                ...
                if (_multiSubnetFailover)
                {
                    throw SQL.MultiSubnetFailoverWithFailoverPartner(
                        serverProvidedFailoverPartner: false,
            ...
            if (
                typeSystemVersionString.Equals(
            ...
            )
            {
                ...
                typeSystemVersionString.Equals(
                    TYPESYSTEMVERSION.SQL_Server_2000,
                    StringComparison.OrdinalIgnoreCase
                )
            )
            {
                ...
                typeSystemVersionString.Equals(
                    TYPESYSTEMVERSION.SQL_Server_2005,
                    StringComparison.OrdinalIgnoreCase
                )
            )
            {
                ...
                typeSystemVersionString.Equals(
                    TYPESYSTEMVERSION.SQL_Server_2008,
                    StringComparison.OrdinalIgnoreCase
                )
            )
            {
                ...
                typeSystemVersionString.Equals(
                    TYPESYSTEMVERSION.SQL_Server_2012,
                    StringComparison.OrdinalIgnoreCase
                )
            )
            {
            ...
            if (
                transactionBindingString.Equals(
                    TRANSACIONBINDING.ImplicitUnbind,
                    StringComparison.OrdinalIgnoreCase
                )
            )
            {
                ...
                transactionBindingString.Equals(
                    TRANSACIONBINDING.ExplicitUnbind,
                    StringComparison.OrdinalIgnoreCase
                )
            )
            {
            ...
            if (
                _applicationIntent == ApplicationIntent.ReadOnly
                && !String.IsNullOrEmpty(_failoverPartner)
            )
            ...
            if (
                Authentication == SqlClient.SqlAuthenticationMethod.ActiveDirectoryIntegrated
                && (HasUserIdKeyword || HasPasswordKeyword)
            )
            {
        ...
        {
            get { return _typeSystemAssemblyVersion; }
        }
        ...
        internal static Hashtable GetParseSynonyms()
        {
            ...
            if (null == hash)
            {
                ...
                Debug.Assert(
                    SqlConnectionStringBuilder.KeywordsCount + SynonymCount == hash.Count,
        ...
        internal static void VerifyLocalHostAndFixup(
            ref string host,
            bool enforceLocalHost,
            bool fixup
        )
        {
            if (ADP.IsEmpty(host))
            {
                ...
                !CompareHostName(ref host, @".", fixup)
