...
namespace System.Data
{
    ...
    internal sealed class Select
    {
        ...
        private void AnalyzeExpression(BinaryNode expr)
        {
            ...
            else if (expr.op == Operators.And)
            {
                ...
                else
                {
                    ...
                    {
                        while (
                            unaryNode.op == Operators.Noop
                            && unaryNode.right is UnaryNode
                            && ((UnaryNode)unaryNode.right).op == Operators.Noop
                        )
                        {
                ...
                this.linearExpression = (
                    this.linearExpression == null
                        ? e
                        : new BinaryNode(table, Operators.And, e, this.linearExpression)
                );
                return;
            }
            else if (IsSupportedOperator(expr.op))
            {
                if (expr.left is NameNode && expr.right is ConstNode)
                {
                    ColumnInfo canColumn = (ColumnInfo)
                        candidateColumns[((NameNode)(expr.left)).column.Ordinal];
                    canColumn.expr = (
                        canColumn.expr == null
                            ? expr
                            : new BinaryNode(table, Operators.And, expr, canColumn.expr)
                    );
                ...
                else if (expr.right is NameNode && expr.left is ConstNode)
                {
            ...
            this.linearExpression = (
                this.linearExpression == null
                    ? expr
                    : new BinaryNode(table, Operators.And, expr, this.linearExpression)
            );
            return;
        }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (this.linearExpression != this.expression)
                        {
                            ...
                            while (matchedCandidates < j)
                            { // Microsoft : j = index.IndexDesc.Length
        ...
        private void BuildLinearExpression()
        {
            ...
            Debug.Assert(
                matchedCandidates > 0 && matchedCandidates <= lenId,
        ...
        public DataRow[] SelectRows()
        {
            ...
            if (
                index == null
                && (IndexFields.Length > 0 || this.linearExpression == this.expression)
            )
            {
            ...
            { // Microsoft : Either dont have rowFilter or only linear search expression
                ...
                System.Globalization.CompareInfo comparer =
                    (
                        (
                            ...
                            && (null != (namedNode = expr.right as NameNode))
                            && (namedNode.column.DataType == typeof(Guid))
                        )
                        || (
                            isRConst
                            && !isLConst
                            ...
                            && (null != (namedNode = expr.left as NameNode))
                            && (namedNode.column.DataType == typeof(Guid))
                        )
                    )
        ...
        private int Evaluate(int record)
        {
            ...
            for (int i = 0; i < matchedCandidates; i++)
            {
                ...
                Debug.Assert(
                    candidateColumns[columnOrdinal] != null,
                    "How come this is not a candidate column"
                );
                Debug.Assert(
                    candidateColumns[columnOrdinal].expr != null,
        ...
        private Range GetBinaryFilteredRecords()
        {
            ...
            Debug.Assert(
                matchedCandidates <= index.IndexFields.Length,
        ...
        private int CompareRecords(int record1, int record2)
        {
            ...
            long id1 =
                table.recordManager[record1] == null ? 0 : table.recordManager[record1].rowID;
            long id2 =
                table.recordManager[record2] == null ? 0 : table.recordManager[record2].rowID;
            ...
            if (
                diff == 0
                && record1 != record2
                && table.recordManager[record1] != null
                && table.recordManager[record2] != null
            )
            {
        ...
        private void Sort(int left, int right)
        {
            ...
            do
            {
                ...
                do
                {
                    while (CompareRecords(records[i], record) < 0)
                        i++;
                    while (CompareRecords(records[j], record) > 0)
                        j--;
                    if (i <= j)
                    {
                        int r = records[i];
