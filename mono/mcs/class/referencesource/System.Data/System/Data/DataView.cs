...
namespace System.Data
{
    ...
    [
        Designer(
            "Microsoft.VSDesigner.Data.VS.DataViewDesigner, " + AssemblyRef.MicrosoftVSDesigner
        ),
        Editor(
            "Microsoft.VSDesigner.Data.Design.DataSourceEditor, " + AssemblyRef.MicrosoftVSDesigner,
            ...
            ISupportInitializeNotification
    {
        ...
        /// </remarks>
        private Dictionary<DataRow, DataRowView> rowViewCache = new Dictionary<
            DataRow,
            DataRowView
        >(DataRowReferenceComparer.Default);

        /// <summary>
        /// This collection allows expression maintaince to (add / remove) from the index when it really
        // should be a (change / move).
        /// </summary>
        private readonly Dictionary<DataRow, DataRowView> rowViewBuffer = new Dictionary<
            DataRow,
            DataRowView
        >(DataRowReferenceComparer.Default);
        ...
        {
            internal static readonly DataRowReferenceComparer Default =
        ...
        /// <summary>
        /// Allow construction of DataView with <see cref="System.Predicate&lt;DataRow&gt;"/> and <see
        // cref="System.Comparison&lt;DataRow&gt;"/>
        /// </summary>
        ...
        public bool ApplyDefaultSort
        {
            ...
            {
                Bid.Trace(
                    "<ds.DataView.set_ApplyDefaultSort|API> %d#, %d{bool}\n",
                    ObjectID,
                    value
                );
        ...
        /// <devdoc>
        ///    <para>
        ///       Indicates whether the <see cref='System.Data.DataView.Sort'/> property should be
        // persisted.
        ///    </para>
        /// </devdoc>
        ...
        {
            get { return GetRowView(GetRow(recordIndex)); }
        }
        ...
        internal void Delete(DataRow row)
        {
            if (null != row)
            {
                ...
                Bid.ScopeEnter(
                    out hscp,
                    "<ds.DataView.Delete|API> %d#, row=%d#",
                    ObjectID,
                    row.ObjectID
                );
                try
        ...
        internal int IndexOf(DataRowView rowview)
        {
            if (null != rowview)
            {
                ...
                if ((null != index) && (DataRowState.Detached != rowview.Row.RowState))
                {
                    ...
                    if (
                        rowViewCache.TryGetValue(rowview.Row, out cached)
                        && ((object)cached == (object)rowview)
                    )
                    {
        ...
        private int IndexOfDataRowView(DataRowView rowview)
        {
            ...
            return index.GetIndex(
                rowview.Row.GetRecordFromVersion(
                    rowview.Row.GetDefaultRowVersion(this.RowStateFilter) & ~DataRowVersion.Proposed
                )
            );
        }
        ...
        [
            ResCategoryAttribute(Res.DataCategory_Data),
            ResDescriptionAttribute(Res.DataViewListChangedDescr)
        ]
        ...
        { // NOTE: this function had keepIndex previosely
            if (property != null)
            {
                ...
                try
                {
                    if (
                        (null == findIndexes)
                        || !findIndexes.TryGetValue(property.Name, out findIndex)
                    )
                    {
                        created = true;
        ...
        {
            get { return GetSortDescriptions(); }
        }
        ...
        protected virtual void IndexListChanged(object sender, ListChangedEventArgs e)
        {
            ...
            if (addNewRow != null && index.RecordCount == 0)
            { // Microsoft : 83032 Clear the newly added row as the underlying index is reset.
                FinishAddNew(false);
            }
            if (ListChangedType.Reset == e.ListChangedType)
            {
        ...
        internal void SetDataViewManager(DataViewManager dataViewManager)
        {
            ...
            if (this.dataViewManager != dataViewManager)
            {
                ...
                if (dataViewManager != null)
                {
                    ...
                    try
                    {
                        // Microsoft: check that we will not do unnesasary operation here if dataViewSetting.Sort ==
                        // this.Sort ...
        ...
        internal void UpdateIndex(bool force, bool fireEvent)
        {
            ...
            Bid.ScopeEnter(
                out hscp,
                "<ds.DataView.UpdateIndex|INFO> %d#, force=%d{bool}\n",
                ObjectID,
                force
            );
            try
...
/*default*/null
        ...
        public virtual bool Equals(DataView view)
        {
            if (
                ...
                || // case insensitive
                (string.Compare(this.Sort, view.Sort, StringComparison.OrdinalIgnoreCase) != 0)
                || // case insensitive
