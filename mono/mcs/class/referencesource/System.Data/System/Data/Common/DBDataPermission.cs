    ...
    public abstract class DBDataPermission : CodeAccessPermission, IUnrestrictedPermission
    {
        ...
        { // V1.0.5000, for CreatePermission
            ...
            if (!_isUnrestricted)
            {
                ...
                if (
                    permissionAttribute.ShouldSerializeConnectionString()
                    || permissionAttribute.ShouldSerializeKeyRestrictions()
                )
                { // MDAC 86773
                    Add(
                        permissionAttribute.ConnectionString,
                        permissionAttribute.KeyRestrictions,
        ...
        { // MDAC 84804
            ...
            bool flag = (
                !IsUnrestricted()
                && !AllowBlankPassword
                && ((null == keyvalues) || (0 == keyvalues.Count))
            );
            return flag;
        }
                ...
                if (null != children)
                {
                    foreach (SecurityElement keyElement in children)
                    {
                        ...
                        if (
                            ...
                            || (
                                (null != tag)
                                && (XmlStr._add == tag.ToLower(CultureInfo.InvariantCulture))
                            )
                        )
                        {
                            ...
                            if (null != behavr)
                            {
                                behavior = (KeyRestrictionBehavior)
                                    Enum.Parse(typeof(KeyRestrictionBehavior), behavr, true);
                            }
