...
namespace System.Data.Common
{
    ...
    { // V1.0.3300, MDAC 69629
        ...
        {
            get { return (IDbDataAdapter)this; }
        }
...
#pragma warning restore 618
        ...
        { // V1.0.3300
            ...
            Bid.ScopeEnter(
                out hscp,
                "<comm.DbDataAdapter.FillSchema|API> %d#, dataTable, schemaType=%d{ds.SchemaType}\n",
                ObjectID,
                (int)schemaType
            );
            try
        ...
        { // V1.0.3300
            ...
            Bid.ScopeEnter(
                out hscp,
                "<comm.DbDataAdapter.FillSchema|API> %d#, dataTable, schemaType, command, behavior=%d{ds.CommandBehavior}\n",
                ObjectID,
                (int)behavior
            );
            try
        ...
        { // V1.0.3300
            ...
            Bid.ScopeEnter(
                out hscp,
                "<comm.DbDataAdapter.Fill|API> %d#, dataSet, srcTable='%ls'\n",
                ObjectID,
                srcTable
            );
            try
                    ...
                    if (!ADP.IsEmpty(columnName))
                    {
                        ...
                        if ((null != dbparameter) && dbparameter.SourceColumnNullMapping)
                        {
                            ...
                            parameter.Value = ADP.IsNull(parameter.Value)
                                ? ParameterValueNullValue
                                : ParameterValueNonNullValue;
                        }
                    }
                }
            }
        ...
        { // V1.0.3300
            ...
            Bid.ScopeEnter(
                out hscp,
                "<comm.DbDataAdapter.Update|API> %d#, dataSet, srcTable='%ls'",
                ObjectID,
                srcTable
            );
            try
        ...
        { // V1.0.3300
            ...
            Bid.ScopeEnter(
                out hscp,
                "<comm.DbDataAdapter.Update|API> %d#, dataRows[], tableMapping",
                ObjectID
            );
            try
            {
                        ...
                        foreach (DataRow dataRow in dataRows)
                        {
                            ...
                            if (1 == maxBatchCommands)
                            {
                                if (null != dataCommand)
                                {
                                    ...
                                    batchCommands[0].UpdatedRowSource =
                                    ...
                                    if (null != dataCommand)
                                    {
                                        ...
                                        )
                                        {
                                            ...
                                            batchCommands[commandCount].StatementType =
                                                statementType;
                                            batchCommands[commandCount].UpdatedRowSource =
                                                dataCommand.UpdatedRowSource;
                            ...
                            try
                            {
                                ...
                                else if (null != dataCommand)
                                {
                                    ...
                                    if (ConnectionState.Open == state)
                                    {
                                        ...
                                        batchCommands[0].RecordsAffected =
            ...
            for (int bc = 0; bc < commandCount; ++bc)
            {
                ...
                if (
                    GetBatchedRecordsAffected(
                        batchCommand.CommandIdentifier,
                        out rowAffected,
                        out batchCommands[bc].Errors
                    )
                )
                {
                ...
                if (
                    (null == batchCommands[bc].Errors) && batchCommands[bc].RecordsAffected.HasValue
                )
                {
                    // determine possible concurrency violations per datarow
                    if (
                    ...
                    )
                    {
                        ...
                        if (0 == rowAffected)
                        {
                            ...
                            batchCommands[bc].Errors = ADP.UpdateConcurrencyViolation(
                                batchCommands[bc].StatementType,
                                0,
                                1,
                    ...
                    if (
                        ...
                        && (0 != (UpdateRowSource.OutputParameters & batchCommand.UpdatedRowSource))
                        && (0 != rowAffected)
                    ) // MDAC 71174
                    {
        ...
        )
        {
            ...
            for (int i = 0; i < commandCount; i++)
            {
                ...
                if (
                    (null == batchCommands[i].Errors)
                    && batchCommands[i].RecordsAffected.HasValue
                    && (0 != batchCommands[i].RecordsAffected.Value)
                )
                {
