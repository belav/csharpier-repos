            ...
            for (int i = 0; i < rows.Length; i++)
            {
                ...
                { // row.Table changed from prev. row.
                    ...
                    if (key.HasValue)
                    {
                        ...
                        ndxSearch = new Index(
                            dst,
                            dst.primaryKey.Key.GetIndexDesc(),
                            DataViewRowState.OriginalRows | DataViewRowState.Added,
                            (IFilter)null
                        );
                ...
                if (
                    targetRow.Table.dependentColumns != null
                    && targetRow.Table.dependentColumns.Count > 0
                )
                ...
                if (MissingSchemaAction.Ignore != missingSchemaAction)
                {
                    ...
                    for (int i = 0; i < table.Columns.Count; i++)
                    {
                        ...
                        if (dest == null)
                        {
                            ...
                            {
                                if (!isStandAlonetable)
                                    dataSet.RaiseMergeFailed(
                                        targetTable,
                                        Res.GetString(
                                            Res.DataMerge_MissingColumnDefinition,
                                            table.TableName,
                                            src.ColumnName
                                        ),
                                        missingSchemaAction
                            ...
                            )
                            {
                                if (!isStandAlonetable)
                                    dataSet.RaiseMergeFailed(
                                        targetTable,
                                        Res.GetString(
                                            Res.DataMerge_DataTypeMismatch,
                                            src.ColumnName
                                        ),
                    ...
                    if (targetPKey.Length != tablePKey.Length)
                    {
                        ...
                        else if (tablePKey.Length != 0)
                        {
                            dataSet.RaiseMergeFailed(
                                targetTable,
                                Res.GetString(Res.DataMerge_PrimaryKeyMismatch),
                                missingSchemaAction
                            );
                        }
                    }
                        ...
                        for (int i = 0; i < targetPKey.Length; i++)
                        {
                            ...
                            )
                            {
                                dataSet.RaiseMergeFailed(
                                    table,
                                    Res.GetString(
                                        Res.DataMerge_PrimaryKeyColumnsMismatch,
                                        targetPKey[i].ColumnName,
                                        tablePKey[i].ColumnName
                                    ),
                                    missingSchemaAction
            ...
            for (int i = 0; i < table.Constraints.Count; i++)
            {
                ...
                if (dest == null)
                {
                    dataSet.RaiseMergeFailed(
                        table,
                        Res.GetString(
                            Res.DataMerge_MissingConstraint,
        ...
        private void MergeRelation(DataRelation relation)
        {
            ...
            if (iDest >= 0)
            {
                ...
                for (int i = 0; i < relation.ParentKey.ColumnsReference.Length; i++)
                {
                    ...
                    if (
                        0
                        != string.Compare(dest.ColumnName, src.ColumnName, false, dest.Table.Locale)
                    )
                    {
                        dataSet.RaiseMergeFailed(
                            null,
                            Res.GetString(
                                Res.DataMerge_ReltionKeyColumnsMismatch,
