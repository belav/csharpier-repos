...
namespace System.Data
{
    ...
    public sealed class DataTableReader : DbDataReader
    {
        ...
        public override Int32 GetValues(object[] values)
        {
            ...
            Array.Copy(
                currentDataRow.ItemArray,
                values,
                currentDataRow.ItemArray.Length > values.Length
                    ? values.Length
                    : currentDataRow.ItemArray.Length
            );
            return (
                currentDataRow.ItemArray.Length > values.Length
                    ? values.Length
        ...
        private void ValidateState(string caller)
        {
            ...
            if ((currentDataRow == null) || (currentDataTable == null))
            { //|| (currentDataRow != currentDataTable.Rows[rowCounter])) do we need thios check!
                ReaderIsInvalid = true;
                throw ExceptionBuilder.InvalidDataTableReader(currentDataTable.TableName);
            }
            ...
            if (
                (currentDataRow.RowState == DataRowState.Deleted)
                || (currentDataRow.RowState == DataRowState.Detached)
                || currentRowRemoved
            )
            ...
            */
            switch (args.Action)
            {
                case DataRowAction.Add:
                    ...
                    */
                    if (currentDataRow == currentDataTable.Rows[rowCounter + 1])
