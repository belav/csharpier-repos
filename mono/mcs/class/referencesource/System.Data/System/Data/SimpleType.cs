...
namespace System.Data
{
    ...
    internal sealed class SimpleType : ISerializable
    {
        ...
        private SimpleType(SerializationInfo info, StreamingContext context)
        {
            ...
            this.baseSimpleType = (SimpleType)
                info.GetValue("SimpleType.BaseSimpleType", typeof(SimpleType));
        ...
        void ISerializable.GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            info.AddValue(
                ...
                xmlQN != null ? true : false
            );
            info.AddValue("SimpleType.XmlBaseType.Name", xmlQN != null ? xmlQN.Name : null);
            info.AddValue(
                "SimpleType.XmlBaseType.Namespace",
                xmlQN != null ? xmlQN.Namespace : null
            );
        ...
        internal void LoadTypeValues(XmlSchemaSimpleType node)
        {
            ...
            if (node.Content is XmlSchemaSimpleTypeRestriction)
            {
                ...
                if (
                    baseSimpleType != null
                    && baseSimpleType.Name != null
                    && baseSimpleType.Name.Length > 0
                )
                {
                ...
                foreach (XmlSchemaFacet facet in content.Facets)
                {
                    ...
                    if (facet is XmlSchemaEnumerationFacet)
                        enumeration = !Common.ADP.IsEmpty(enumeration)
        ...
        {
            get { return (XmlQualifiedName)xmlBaseType; }
        }
        ...
        */
        ...
        internal XmlNode ToNode(XmlDocument dc, Hashtable prefixes, bool inRemoting)
        {
            ...
            if (!inRemoting)
            {
                if (baseSimpleType != null)
                {
                    ...
                    {
                        string prefix =
                            (prefixes != null) ? (string)prefixes[baseSimpleType.Namespace] : null;
                        if (prefix != null)
                        {
        ...
        internal string HasConflictingDefinition(SimpleType otherSimpleType)
        {
            ...
            if (
                string.Compare(this.BaseType, otherSimpleType.BaseType, StringComparison.Ordinal)
                != 0
            )
                return ("BaseType");
            if (
                (this.BaseSimpleType == null && otherSimpleType.BaseSimpleType != null)
                && (
                    this.BaseSimpleType.HasConflictingDefinition(otherSimpleType.BaseSimpleType)
                ).Length != 0
            )
        ...
        internal bool CanHaveMaxLength()
        {
            ...
            if (
                string.Compare(rootType.BaseType, "string", StringComparison.OrdinalIgnoreCase) == 0
            )
                return true;
            return false;
        }
