...
namespace System.Data
{
    ...
    internal sealed class Index
    {
        ...
        {
            Bid.Trace(
                "<ds.Index.Index|API> %d#, table=%d, recordStates=%d{ds.DataViewRowState}\n",
                ObjectID,
            ...
            this.table = table;
            _listeners = new Listeners<DataViewListener>(
                ObjectID,
        ...
        public bool Equal(IndexField[] indexDesc, DataViewRowState recordStates, IFilter rowFilter)
        {
            ...
            {
                if (
                    IndexFields[loop].Column != indexDesc[loop].Column
        ...
        {
            get { return _hasRemoteAggregate; }
        }
        ...
        {
            get { return records.HasDuplicates; }
        }
...
#if DEBUG
            Debug.Assert(
                records.CheckUnique(records.root) != HasDuplicates,
                "CheckUnique difference"
            );
#endif
        ...
        private int CompareRecords(int record1, int record2)
        {
            ...
            else
            {
                ...
                return table
                    .Rows.IndexOf(table.recordManager[record1])
        ...
        private int CompareDuplicateRecords(int record1, int record2)
        {
            ...
            {
                Debug.Assert(
                    0 == CompareDataRows(record1, record2),
                    "duplicate record not a duplicate by user function"
                );
            }
            ...
            int diff = table
                ...
                .rowID.CompareTo(table.recordManager[record2].rowID);

            // if they're two records in the same row, we need to be able to distinguish them.
            if ((diff == 0) && (record1 != record2))
            {
                diff = ((int)table.recordManager[record1].GetRecordState(record1)).CompareTo(
                    (int)table.recordManager[record2].GetRecordState(record2)
                );
            }
            return diff;
        }
        ...
        private bool DoListChanged
        {
            ...
            {
                return (
                    !suspendEvents && _listeners.HasListeners && !table.AreIndexEventsSuspended
                );
            }
        }
        ...
        private void OnListChanged(ListChangedEventArgs e)
        {
            ...
            Debug.Assert(
                DoListChanged,
                "supposed to check DoListChanged before calling to delay create ListChangedEventArgs"
            );
        ...
        private void MaintainDataView(ListChangedType changedType, int record, bool trackAddRemove)
        {
            ...
            _listeners.Notify(
                changedType,
                ((0 <= record) ? table.recordManager[record] : null),
                trackAddRemove,
        ...
        {
            Bid.Trace(
                "<ds.Index.RecordChanged|API> %d#, oldIndex=%d, newIndex=%d\n",
                ObjectID,
                oldIndex,
                newIndex
            );
        ...
        where TElem : class
    {
        ...
        internal void Add(TElem listener)
        {
            ...
            Debug.Assert(
                !Index.ContainsReference(listeners, listener),
                "already contains reference"
            );
