    ...
    internal sealed class SchemaMapping
    {
            ...
            if (null != _xmlMap)
            {
                for (int i = 0; i < _xmlMap.Length; ++i)
                {
                    if (0 != _xmlMap[i])
                    {
                        ...
                        if (
                            (null == xml)
                            && (_readerDataValues[i] is System.Data.SqlTypes.SqlString)
                        )
                        {
                        ...
                        using (IDataReader nestedReader = (IDataReader)readerValue)
                        {
                            if (!nestedReader.IsClosed)
                            {
                                ...
                                string chapterTableName =
                                    _tableMapping.SourceTable + _fieldNames[i]; // MDAC 70908
            ...
            try
            {
                ...
                if (0 < mappingCount)
                {
                    ...
                    if (
                        ...
                        && !ADP.IsEmpty(commonBaseTable)
                        && ADP.IsEmpty(_dataTable.TableName)
                    )
                    {
        ...
        { // MDAC 71613
            if (null != _dataSet)
            {
                ...
                chapterColumn.ColumnName; // MDAC 72815
