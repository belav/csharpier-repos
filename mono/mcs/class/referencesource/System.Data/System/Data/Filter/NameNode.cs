...
namespace System.Data
{
    ...
    internal sealed class NameNode : ExpressionNode
    {
        ...
        internal static string ParseName(char[] text, int start, int pos)
        {
            ...
            {
                Debug.Assert(
                    text[checked((int)pos - 1)] == '`',
                    "Invalid identifyer bracketing, pos = "
                        + pos.ToString(CultureInfo.InvariantCulture)
                        + ", end = "
                        + text[checked((int)pos - 1)].ToString(CultureInfo.InvariantCulture)
                );
            ...
            else if (text[start] == '[')
            {
                Debug.Assert(
                    text[checked((int)pos - 1)] == ']',
                    "Invalid identifyer bracketing of name "
