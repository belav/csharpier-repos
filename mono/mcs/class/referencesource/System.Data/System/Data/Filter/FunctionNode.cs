...
namespace System.Data
{
    ...
    internal sealed class FunctionNode : ExpressionNode
    {
        ...
        internal override object Eval(DataRow row, DataRowVersion version)
        {
            ...
            Debug.Assert(
                this.argumentCount == funcs[info].argumentCount
                    || funcs[info].IsVariantArgumentList,
            ...
            else if (funcs[info].id != FunctionId.Iif)
                ...
                for (int i = 0; i < this.argumentCount; i++)
                {
                    ...
                    if (funcs[info].IsValidateArguments)
                    {
                        ...
                        if (argumentValues[i].GetType() != funcs[info].parameters[i])
                        {
                            ...
                            if (
                                funcs[info].parameters[i] == typeof(int)
                            ...
                            )
                            {
                                ...
                                (funcs[info].id == FunctionId.Trim)
                                || (funcs[info].id == FunctionId.Substring)
                                || (funcs[info].id == FunctionId.Len)
                            )
                            {
                                if (
            ...
            switch (id)
            {
                case FunctionId.Abs:
                    Debug.Assert(
                        ...
                        "Invalid argument argumentCount for "
                            + funcs[info].name
                            + " : "
                            + argumentCount.ToString(FormatProvider)
                    );
                ...
                case FunctionId.Charindex:
                    ...
                    Debug.Assert(
                        ...
                        "Invalid argument type for " + funcs[info].name
                    );
                    Debug.Assert(
                ...
                case FunctionId.Iif:
                    Debug.Assert(
                        argumentCount == 3,
                        "Invalid argument argumentCount: " + argumentCount.ToString(FormatProvider)
                    );
        ...
        internal bool IsAggregate
        {
            ...
            {
                bool aggregate =
                    (funcs[this.info].id == FunctionId.Sum)
