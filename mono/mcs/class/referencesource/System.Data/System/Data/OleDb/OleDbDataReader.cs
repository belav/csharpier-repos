...
namespace System.Data.OleDb
{
    ...
    public sealed class OleDbDataReader : DbDataReader
    {
        ...
        private void BuildSchemaTableInfo(object handle, bool filterITypeInfo, bool filterChapters)
        {
            ...
            {
                Bid.Trace(
                    "<oledb.IUnknown.QueryInterface|API|OLEDB|RET> %08X{HRESULT}\n",
        ...
        )
        {
            ...
            for (int i = 0, offset = 0; i < columnCount; ++i, offset += ODB.SizeOf_tagDBCOLUMNINFO)
            {
                ...
                info.size = (
                    ((maxsize < 0) || (Int32.MaxValue < maxsize)) ? Int32.MaxValue : (int)maxsize
                );
#endif
        ...
        private void BuildSchemaTableRowset(object handle)
        {
            ...
            Bid.Trace(
                "<oledb.IUnknown.QueryInterface|API|OLEDB|rowset_row> %d, IColumnsRowset\n",
                ObjectID
            );
        ...
        override public Int64 GetBytes(
            int ordinal,
            Int64 dataIndex,
            byte[] buffer,
            Int32 bufferIndex,
        ...
        override public Int64 GetChars(
            int ordinal,
            Int64 dataIndex,
            char[] buffer,
            Int32 bufferIndex,
        ...
        )
        {
            ...
            if (null != imultipleResults)
            {
                ...
                for (int loop = 0; ; ++loop)
                {
                    ...
                    { // MDAC 72126 (reason for more than 1000 iterations)
        ...
        private Bindings[] CreateBindingsFromMetaData(bool allowMultipleAccessor)
        {
            ...
            for (int index = 0; index < metadata.Length; ++index)
            {
                ...
                Debug.Assert(
                    !NativeDBType.HasHighBit(getType),
                    "CreateAccessor - unexpected high bits on datatype"
                );
        ...
        private void ReleaseRowHandles()
        {
            ...
            Bid.Trace(
                "<oledb.IRowset.ReleaseRows|API|OLEDB> %d#, Request=%Id\n",
                ObjectID,
                _rowFetchedCount
            );
                ...
                )
                {
                    ...
                    foreach (DataRow dataRow in table.Rows)
                    {
                        ...
                        bool nullsVal =
                            (null != nulls)
                            && (
                                dataRow.IsNull(nulls, DataRowVersion.Default)
                                || (
                                    ODB.DBPROPVAL_IN_ALLOWNULL
                                    == Convert.ToInt32(
                                        dataRow[nulls, DataRowVersion.Default],
                    ...
                    if (AdapterSwitches.DataSchema.TraceVerbose)
                    {
                        Debug.WriteLine(
                            "Filtered Column: DBCOLUMN_FLAGS="
                                + info.flags.ToString("X8", (System.IFormatProvider)null)
                                + " DBCOLUMN_NAME="
                                + info.columnName
                        );
                    }
        ...
        static internal void GenerateSchemaTable(
