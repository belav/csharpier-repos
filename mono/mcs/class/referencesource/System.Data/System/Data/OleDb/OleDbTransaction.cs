...
namespace System.Data.OleDb
{
    ...
    public sealed class OleDbTransaction : DbTransaction
    {
        ...
        private sealed class WrappedTransaction : WrappedIUnknown
        {
            ...
            internal OleDbHResult Commit()
            {
                ...
                try
                {
                    ...
                    Bid.Trace(
                        "<oledb.ITransactionLocal.Commit|API|OLEDB> handle=%p\n",
                        base.handle
                    );
                    RuntimeHelpers.PrepareConstrainedRegions();
                    try { }
            ...
            protected override bool ReleaseHandle()
            {
                ...
                {
                    Bid.Trace(
                        "<oledb.ITransactionLocal.Abort|API|OLEDB|INFO> handle=%p\n",
                        base.handle
                    );
                    OleDbHResult hr = (OleDbHResult)
        ...
        public OleDbTransaction Begin(IsolationLevel isolevel)
        {
            ...
            Bid.ScopeEnter(
                out hscp,
                "<oledb.OleDbTransaction.Begin|API> %d#, isolevel=%d{IsolationLevel}",
                ObjectID,
                (int)isolevel
            );
            try
            {
                ...
                OleDbTransaction transaction = new OleDbTransaction(
                    _parentConnection,
                    this,
                    isolevel
                );
