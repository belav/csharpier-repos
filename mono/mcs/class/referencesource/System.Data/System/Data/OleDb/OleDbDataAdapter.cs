...
namespace System.Data.OleDb
{
    [
        ...
        ToolboxItem(
            "Microsoft.VSDesigner.Data.VS.OleDbDataAdapterToolboxItem, "
                + AssemblyRef.MicrosoftVSDesigner
        ),
        Designer(
            "Microsoft.VSDesigner.Data.VS.OleDbDataAdapterDesigner, "
                + AssemblyRef.MicrosoftVSDesigner
        ),
    ]
    public sealed class OleDbDataAdapter : DbDataAdapter, IDbDataAdapter, ICloneable
    {
        ...
        public int Fill(DataTable dataTable, object ADODBRecordSet)
        {
            ...
            Bid.ScopeEnter(
                out hscp,
                "<oledb.OleDbDataAdapter.Fill|API> %d#, dataTable, ADODBRecordSet\n",
                ObjectID
            );
            try
            {
        ...
        public int Fill(DataSet dataSet, object ADODBRecordSet, string srcTable)
        {
            ...
            Bid.ScopeEnter(
                out hscp,
                "<oledb.OleDbDataAdapter.Fill|API> %d#, dataSet, ADODBRecordSet, srcTable='%ls'\n",
                ObjectID,
                srcTable
            );
            try
                    ...
                    if (multipleResults)
                    {
                        ...
                        if (0 > hr)
                        {
                            ...
                            if (ODB.ADODB_NextResultError != (int)hr)
                            {
                                ...
                                if (null != errorInfo)
                                {
                                    OleDbHResult hresult = ODB.GetErrorDescription(
                                        errorInfo,
                                        hr,
                                        out message
                                    );
