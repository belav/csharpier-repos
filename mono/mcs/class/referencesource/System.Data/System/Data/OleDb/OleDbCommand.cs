...
namespace System.Data.OleDb
{
    ...
    [
        ...
        ToolboxItem(true),
        Designer(
            "Microsoft.VSDesigner.Data.VS.OleDbCommandDesigner, " + AssemblyRef.MicrosoftVSDesigner
        )
    ]
    public sealed class OleDbCommand : DbCommand, ICloneable, IDbCommand
    {
        ...
        private UnsafeNativeMethods.ICommandWithParameters ICommandWithParameters()
        {
            ...
            UnsafeNativeMethods.ICommandWithParameters value = (
                _icommandText as UnsafeNativeMethods.ICommandWithParameters
            );
            if (null == value)
            {
        ...
        {
            Debug.Assert(
                System.Data.CommandType.Text == CommandType
                    || System.Data.CommandType.StoredProcedure == CommandType,
        ...
        )
        {
            ...
            OleDbHResult hr = commandWithParameters.SetParameterInfo(
                (IntPtr)bindInfo.Length,
                ordinals,
                bindInfo
            );
            Bid.Trace(
                ...
                if (null != icmdtxt)
                {
                    ...
                    lock (icmdtxt)
                    {
                        ...
                        if (icmdtxt == _icommandText)
                        {
                            ...
                            hr = icmdtxt.Cancel();
                            Bid.Trace(
                                "<oledb.ICommandText.Cancel|API|OLEDB|RET> %08X{HRESULT}\n",
                                hr
                            );
                        }
                    }
        ...
        private void CloseInternalCommand()
        {
            ...
            UnsafeNativeMethods.ICommandText ict =
                Interlocked.Exchange<UnsafeNativeMethods.ICommandText>(ref _icommandText, null);
            if (null != ict)
            {
                lock (ict)
                {
            ...
            try
            {
                    ...
                    try
                    {
                        ...
                        switch (resultType)
                        {
                            ...
                            case ODB.PrepareICommandText:
                                ...
                                OleDbDataReader.GenerateSchemaTable(
                                    dataReader,
                                    _icommandText,
                                    behavior
                                );
                ...
                { // optimized code path for ExecuteNonQuery to not create a OleDbDataReader object
                    try
                    {
                        ...
                        {
                            UnsafeNativeMethods.IMultipleResults multipleResults =
                                (UnsafeNativeMethods.IMultipleResults)executeResult;

                            // may cause a Connection.ResetState which closes connection
                            nextResultsFailure = OleDbDataReader.NextResults(
                                multipleResults,
                                _connection,
                                this,
        ...
        private bool InitializeCommand(CommandBehavior behavior, bool throwifnotsupported)
        {
            ...
            if (_lastChangeID != changeid)
            {
                ...
                {
                    Bid.Trace(
                        "<oledb.ICommandText.SetCommandText|API|OLEDB> %d#, DBGUID_DEFAULT, CommandText='",
                        ObjectID
                    );
            ...
            if (0 < count)
            {
                ...
                if (_executeQuery)
                {
                    ...
                    dbprops[1] = new tagDBPROP(
                        ODB.DBPROP_ACCESSORDER,
                        false,
                        ODB.DBPROPVAL_AO_RANDOM
                    ); // MDAC 73030
