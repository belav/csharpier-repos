...
namespace System.Xml
{
    ...
    public class XmlDataDocument : XmlDocument
    {
        ...
        private void BindToDataSet()
        {
            ...
            dataSet.Tables.CollectionChanging += new CollectionChangeEventHandler(
                this.OnDataSetTablesChanging
            );
            dataSet.Relations.CollectionChanging += new CollectionChangeEventHandler(
                this.OnDataSetRelationsChanging
            );
            dataSet.DataRowCreated += new DataRowCreatedEventHandler(this.OnDataRowCreated);
            dataSet.PropertyChanging += new PropertyChangedEventHandler(
                this.OnDataSetPropertyChanging
            );
            ...
            {
                rel.PropertyChanging += new PropertyChangedEventHandler(
        ...
        private void BindToTable(DataTable t)
        {
            ...
            t.Columns.CollectionChanging += new CollectionChangeEventHandler(
                this.OnTableColumnsChanging
            );

            foreach (DataColumn col in t.Columns)
            {
                // Hook column properties changes, so we can react properly to ROM changes.
                col.PropertyChanging += new PropertyChangedEventHandler(
                    this.OnColumnPropertyChanging
                );
            }
        }
...
#pragma warning restore 1717
        ...
        private XmlNode CloneNode(DataPointer dp)
        {
            ...
            throw new InvalidOperationException(
                Res.GetString(Res.DataDom_CloneNode, dp.NodeType.ToString())
            );
        }
            ...
            if (parentRowInTree != parentRowInRelation)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (
                            parentRowInTree.tempRecord != -1
                            && colInParent.CompareValueTo(parentRowInTree.tempRecord, comparedValue)
                                != 0
                        )
                        {
                            EnsureNonRowDocumentElement().AppendChild(childElement);
        ...
        private void OnNodeChanging(object sender, XmlNodeChangedEventArgs args)
        {
            ...
            if (DataSet.EnforceConstraints != false)
                throw new InvalidOperationException(
                    Res.GetString(Res.DataDom_EnforceConstraintsShouldBeOff)
                );
        }
        ...
        private void OnColumnPropertyChanging(object oColumn, PropertyChangedEventArgs args)
        {
            ...
            if (args.PropertyName == "Namespace")
                throw new InvalidOperationException(
                    Res.GetString(Res.DataDom_ColumnNamespaceChange)
                );
        ...
        private void OnTablePropertyChanging(object oTable, PropertyChangedEventArgs args)
        {
            ...
            if (args.PropertyName == "Namespace")
                throw new InvalidOperationException(
                    Res.GetString(Res.DataDom_TableNamespaceChange)
                );
        }
        ...
        )
        {
            ...
            if (rel != null && rel.Nested)
                throw new InvalidOperationException(
                    Res.GetString(Res.DataDom_DataSetNestedRelationsChange)
                );
