...
namespace Microsoft.SqlServer.Server
{
    ...
    //  This approach to adding properties is added combat the growing number of sparsely-used
    // properties
    ...
    internal class SmiMetaDataPropertyCollection
    {
        ...
        private static readonly SmiDefaultFieldsProperty __emptyDefaultFields =
            new SmiDefaultFieldsProperty(new List<bool>());
        private static readonly SmiOrderProperty __emptySortOrder = new SmiOrderProperty(
            new List<SmiOrderProperty.SmiColumnOrder>()
        );
        private static readonly SmiUniqueKeyProperty __emptyUniqueKey = new SmiUniqueKeyProperty(
            new List<bool>()
        );
        ...
        {
            get { return _properties[(int)key]; }
            set
            {
                if (null == value)
                {
        ...
        {
            get { return new List<SmiMetaDataProperty>(_properties); }
        }
    ...
    internal class SmiUniqueKeyProperty : SmiMetaDataProperty
    {
        ...
        {
            Debug.Assert(
                0 == _columns.Count || countToMatch == _columns.Count,
                "SmiDefaultFieldsProperty.CheckCount: DefaultFieldsProperty size ("
                    + _columns.Count
                    + ") not equal to checked size ("
                    + countToMatch
                    + ")"
            );
        }
    ...
    internal class SmiDefaultFieldsProperty : SmiMetaDataProperty
    {
        ...
        {
            Debug.Assert(
                0 == _defaults.Count || countToMatch == _defaults.Count,
                "SmiDefaultFieldsProperty.CheckCount: DefaultFieldsProperty size ("
                    + _defaults.Count
                    + ") not equal to checked size ("
                    + countToMatch
