        ...
        internal String String
        {
            get
            {
                ...
                else
                {
                    ...
                    System.IO.Stream byteStream = new System.IO.MemoryStream(
                        (byte[])_object,
                        false
                    );
        ...
        internal Int64 BytesLength
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    Debug.Assert(StorageType.ByteArray == _type, "Wrong storage type: " + _type);
                    Debug.Assert(
                        value > 0 && value <= ((byte[])_object).Length,
                        "Invalid BytesLength"
                    );
                ...
                else
                {
                    ...
                    Debug.Assert(
                        value > 0
                            && (
                                (
                                    StorageType.CharArray == _type
                                    && value <= ((char[])_object).Length
                                )
                                || (
                                    StorageType.String == _type && value <= ((string)_object).Length
                                )
                            ),
                        "Invalid CharsLength"
                    );
                ...
                switch (_type)
                {
                    ...
                    case StorageType.SqlDecimal:
                        return new SmiMetaData(
                            SqlDbType.Decimal,
                            17,
                            ((SqlDecimal)_object).Precision,
                            ((SqlDecimal)_object).Scale,
                            0,
