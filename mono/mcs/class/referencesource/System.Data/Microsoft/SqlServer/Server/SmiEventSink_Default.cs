...
namespace Microsoft.SqlServer.Server
{
    ...
    internal class SmiEventSink_Default : SmiEventSink
    {
        ...
        protected SqlException ProcessMessages(bool ignoreWarnings, bool ignoreNonFatalMessages)
        {
            ...
            {
                Debug.Assert(
                    null == _warnings || 0 != _warnings.Count,
                    "empty warning collection?"
                ); // must be something in the collection
        ...
        internal override void MessagePosted(
            int number,
            byte state,
            byte errorClass,
            string server,
        ...
        )
        {
            if (null == _parent)
            {
                ...
                {
                    Bid.Trace(
                        "<sc.SmiEventSink_Default.MessagePosted|ADV> %d#, number=%d state=%d errorClass=%d server='%ls' message='%ls' procedure='%ls' linenumber=%d.\n",
                        0,
                        number,
                        state,
                        errorClass,
