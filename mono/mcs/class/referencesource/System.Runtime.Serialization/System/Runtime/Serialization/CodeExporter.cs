...
namespace System.Runtime.Serialization
{
    ...
    class CodeExporter
    {
        ...
        )
        {
            ...
            foreach (KeyValuePair<XmlQualifiedName, DataContract> pair in dataContractSet)
            {
                ...
                if (!(dataContract.IsBuiltInDataContract || dataContract is CollectionDataContract))
                {
                    ...
                    if (
                        contractCodeDomInfo.IsProcessed
            ...
            if (this.options != null)
            {
                foreach (KeyValuePair<string, string> pair in options.Namespaces)
                {
                    ...
                    if (
                        this.clrNamespaces.TryGetValue(
                            clrNamespace,
                            out currentDataContractNamespace
                        )
                    )
                    {
                        if (dataContractNamespace != currentDataContractNamespace)
                            throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                                new InvalidOperationException(
                                    SR.GetString(
                                        SR.CLRNamespaceMappedMultipleTimes,
                                        currentDataContractNamespace,
        ...
        {
            [Fx.Tag.SecurityNote(
                Critical = "Critical because it calls the CodeProvider.Supports(..) method that has a LinkDemand.",
                Safe = "Safe because it doesn't leak security sensitive information."
            )]
            [SecuritySafeCritical]
            get
            {
                return (CodeProvider == null)
                    ? true
                    : CodeProvider.Supports(GeneratorSupport.DeclareEvents);
            }
        }
        ...
        bool SupportsDeclareValueTypes
        {
            ...
            get
            {
                return (CodeProvider == null)
                    ? true
                    : CodeProvider.Supports(GeneratorSupport.DeclareValueTypes);
            }
        }
        ...
        bool SupportsAssemblyAttributes
        {
            ...
            get
            {
                return (CodeProvider == null)
                    ? true
                    : CodeProvider.Supports(GeneratorSupport.AssemblyAttributes);
            }
        }
        ...
        bool SupportsNestedTypes
        {
            ...
            get
            {
                return (CodeProvider == null)
                    ? true
                    : CodeProvider.Supports(GeneratorSupport.NestedTypes);
            }
        }
        ...
        {
            [Fx.Tag.SecurityNote(
                Critical = "Critical because it calls the CodeProvider.FileExtension property that has a LinkDemand.",
                Safe = "Safe because it doesn't leak security sensitive information."
            )]
            [SecuritySafeCritical]
        ...
        )
        {
            ...
            {
                if (
                    dataContractSet.TryGetReferencedCollectionType(
                        stableName,
                        dataContract,
                        out type
                    )
                ...
                return dataContractSet.TryGetReferencedCollectionType(
                    stableName,
                    dataContract,
                    out type
                );
            else
            {
                ...
                {
                    stableName = SchemaImporter.ImportActualType(
                        xmlDataContract.XsdType.Annotation,
                        stableName,
        ...
        [Fx.Tag.SecurityNote(
            Critical = "Critical because it calls the System.CodeDom.Compiler.CodeGenerator.ValidateIdentifiers(..) method that has a LinkDemand for FullTrust."
        )]
        [SecurityCritical]
            ...
            try
            {
                    ...
                    if (!contractCodeDomInfo.IsProcessed)
                    {
                        if (dataContract is ClassDataContract)
                        {
                            ...
                            ExportEnumDataContract(
                                (EnumDataContract)dataContract,
                                contractCodeDomInfo
                            );
                        else if (dataContract is XmlDataContract)
                            ExportXmlDataContract(
                                (XmlDataContract)dataContract,
                                contractCodeDomInfo
                            );
                        else
                            throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                                new InvalidDataContractException(
                                    SR.GetString(
                                        ...
                                        DataContract.GetClrTypeFullName(dataContract.GetType()),
                ...
                if (dataContractSet.DataContractSurrogate != null)
                {
                    CodeNamespace[] namespaces = new CodeNamespace[
                        codeCompileUnit.Namespaces.Count
                    ];
        ...
        void ExportClassDataContractHierarchy(
            XmlQualifiedName typeName,
            ClassDataContract classContract,
            ContractCodeDomInfo contractCodeDomInfo,
            Dictionary<XmlQualifiedName, object> contractNamesInHierarchy
        )
        {
        ...
        void InvokeProcessImportedType(CollectionBase collection)
        {
            ...
            foreach (object obj in objects)
            {
                ...
                CodeTypeDeclaration newCodeTypeDeclaration =
                    DataContractSurrogateCaller.ProcessImportedType(
        ...
        CollectionDataContract GenericListContract
        {
            get
            {
                return dataContractSet.GetDataContract(Globals.TypeOfListGeneric)
        ...
        CollectionDataContract GenericDictionaryContract
        {
            get
            {
                return dataContractSet.GetDataContract(Globals.TypeOfDictionaryGeneric)
                    as CollectionDataContract;
            }
        }
        ...
        {
            ContractCodeDomInfo contractCodeDomInfo = dataContractSet.GetContractCodeDomInfo(
                dataContract
            );
            ...
            if (!contractCodeDomInfo.IsProcessed)
            {
                ...
                else
                {
                    ...
                    if (type == null)
                    {
                        ...
                        CodeNamespace ns = GetCodeNamespace(
                            clrNamespace,
                        ...
                        if (type == null)
                        {
                            ...
                            if (
                                NamespaceContainsType(ns, typeName)
                                || GlobalTypeNameConflicts(clrNamespace, typeName)
                            )
                            {
                                for (int i = 1; ; i++)
                                {
                                    string uniqueName = AppendToValidClrIdentifier(
                                        typeName,
                                        i.ToString(NumberFormatInfo.InvariantInfo)
                                    );
                                    if (
                                        !NamespaceContainsType(ns, uniqueName)
                                        && !GlobalTypeNameConflicts(clrNamespace, uniqueName)
                                    )
                                    {
                                        typeName = uniqueName;
                                        break;
                                    ...
                                    if (i == Int32.MaxValue)
                                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                                            new InvalidDataContractException(
                                                SR.GetString(SR.CannotComputeUniqueName, typeName)
                                            )
                                        );
                                }
                            }
                            ...
                            contractCodeDomInfo.TypeReference = new CodeTypeReference(
                                (clrNamespace == null || clrNamespace.Length == 0)
                                    ? typeName
        ...
        )
        {
            ...
            DataContract containingDataContract = dataContractSet[
                new XmlQualifiedName(containingTypeName, dataContract.StableName.Namespace)
            ];
            if (containingDataContract == null)
                return null;
            string nestedTypeName = XmlConvert.DecodeName(
                originalName.Substring(nestedTypeIndex + 1)
            );
            nestedTypeName = GetClrIdentifier(nestedTypeName, Globals.DefaultTypeName);

            ContractCodeDomInfo containingContractCodeDomInfo = GetContractCodeDomInfo(
                containingDataContract
            );
        ...
        static CodeTypeDeclaration CreateTypeDeclaration(string typeName, DataContract dataContract)
        {
            ...
            CodeAttributeDeclaration debuggerStepThroughAttribute = new CodeAttributeDeclaration(
        ...
        CodeTypeReference GetReferencedType(DataContract dataContract)
        {
            ...
            if (
                TryGetReferencedType(dataContract.StableName, dataContract, out type)
            ...
            else if (dataContract.GenericInfo != null)
            {
                ...
                XmlQualifiedName genericStableName =
                    dataContract.GenericInfo.GetExpandedStableName();
                    ...
                    throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new InvalidDataContractException(
                            SR.GetString(
                                ...
                                dataContract.StableName.Name,
                                dataContract.StableName.Namespace,
                                genericStableName.Name,
                                genericStableName.Namespace
                            )
                        )
                    );

                typeReference = GetReferencedGenericType(
                    dataContract.GenericInfo,
        ...
        bool HasDefaultCollectionNames(CollectionDataContract collectionContract)
        {
            ...
            if (
                collectionContract.IsDictionary
                && (
                    collectionContract.KeyName != Globals.KeyLocalName
                    || collectionContract.ValueName != Globals.ValueLocalName
                )
            )
                return false;

            XmlQualifiedName expectedType = itemContract.GetArrayTypeName(
                collectionContract.IsItemTypeNullable
            );
            return (
                collectionContract.StableName.Name == expectedType.Name
                && collectionContract.StableName.Namespace == expectedType.Namespace
            );
        }
            ...
            if (collectionContract.IsDictionary && SupportsGenericTypeReference)
            {
                ...
                if (
                    !TryGetReferencedType(
                        GenericDictionaryName,
                        GenericDictionaryContract,
                        out type
                    )
                )
                    type = Globals.TypeOfDictionaryGeneric;
                ClassDataContract itemContract =
                    collectionContract.ItemContract as ClassDataContract;
                DataMember keyMember = itemContract.Members[0];
                DataMember valueMember = itemContract.Members[1];
                CodeTypeReference keyTypeReference = GetElementTypeReference(
                    keyMember.MemberTypeContract,
                    keyMember.IsNullable
                );
                CodeTypeReference valueTypeReference = GetElementTypeReference(
                    valueMember.MemberTypeContract,
                    valueMember.IsNullable
                );
        ...
        )
        {
            ...
            if (
                ...
                && TryGetReferencedType(GenericListName, GenericListContract, out type)
            )
            {
                typeReference = GetCodeTypeReference(type);
        ...
        CodeTypeReference GetSurrogatedTypeReference(DataContract dataContract)
        {
            IDataContractSurrogate dataContractSurrogate =
                this.dataContractSet.DataContractSurrogate;
        ...
        CodeTypeReference GetReferencedGenericType(
            GenericInfo genInfo,
            ...
            if (genInfo.Parameters != null)
            {
                ...
                for (int i = 0; i < genInfo.Parameters.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        isParamValueType = (
                            paramTypeReference != null && paramTypeReference.ArrayRank == 0
                        ); // only value type information we can get from CodeTypeReference
                    }
                ...
                if (enableStructureCheck)
                    dataContract = DataContract
                        ...
                        .BindGenericParameters(
                            paramContracts,
                            new Dictionary<DataContract, DataContract>()
                        );
            }
            return typeReference;
        }
            ...
            foreach (CodeTypeMember member in containingType.Members)
            {
                if (member is CodeTypeDeclaration)
                {
                    if (
                        String.Compare(
                            typeName,
                            ((CodeTypeDeclaration)member).Name,
        ...
        )
        {
            if (generateSerializable)
            {
                ...
                AddImportStatement(
                    Globals.TypeOfSerializableAttribute.Namespace,
        ...
        )
        {
            ...
            dataContractAttribute.Arguments.Add(
                new CodeAttributeArgument(
                    Globals.NamespaceProperty,
                    new CodePrimitiveExpression(classDataContract.StableName.Namespace)
                )
            );
            if (classDataContract.IsReference != Globals.DefaultIsReference)
                dataContractAttribute.Arguments.Add(
                    new CodeAttributeArgument(
                        Globals.IsReferenceProperty,
                        new CodePrimitiveExpression(classDataContract.IsReference)
                    )
                );
            type.CustomAttributes.Add(dataContractAttribute);
            AddImportStatement(
                Globals.TypeOfDataContractAttribute.Namespace,
                contractCodeDomInfo.CodeNamespace
            );
            ...
            {
                ContractCodeDomInfo baseContractCodeDomInfo = GetContractCodeDomInfo(
                    classDataContract.BaseContract
                );
                Fx.Assert(
                    baseContractCodeDomInfo.IsProcessed,
                    "Cannot generate code for type if code for base type has not been generated"
                );
                ...
                if (baseContractCodeDomInfo.ReferencedTypeExists)
                {
                    Type actualType = (Type)
                        baseContractCodeDomInfo.TypeReference.UserData[codeUserDataActualTypeKey];
            ...
            if (classDataContract.Members != null)
            {
                for (int i = 0; i < classDataContract.Members.Count; i++)
                {
                    ...
                    CodeTypeReference memberType = GetElementTypeReference(
                    ...
                    string fieldName = GetMemberName(
                        AppendToValidClrIdentifier(propertyName, Globals.DefaultFieldSuffix),
                        contractCodeDomInfo
                    );
                    ...
                    CodeMemberProperty property = CreateProperty(
                        memberType,
                        propertyName,
                        fieldName,
                        dataMember.MemberTypeContract.IsValueType && SupportsDeclareValueTypes,
                        raisePropertyChanged
                    );
                    if (dataContractSet.DataContractSurrogate != null)
                        property.UserData.Add(
                            surrogateDataKey,
                            dataContractSet.GetSurrogateData(dataMember)
                        );
                    ...
                    if (dataMember.EmitDefaultValue != Globals.DefaultEmitDefaultValue)
                        dataMemberAttribute.Arguments.Add(
                            new CodeAttributeArgument(
                                Globals.EmitDefaultValueProperty,
                    ...
                    {
                        CodeAttributeDeclaration optionalFieldAttribute =
        ...
        {
            if (
                dataContractSet.KnownTypesForObject != null
                && SchemaImporter.IsObjectContract(dataContract)
            )
            {
            ...
            else if (dataContract is ClassDataContract)
            {
                ...
                if (contractCodeDomInfo.ReferencedTypeExists)
                    return GetKnownTypeContracts(
                        ...
                        new Dictionary<DataContract, object>()
                    );
            }
            return null;
        }
        ...
        DataContractDictionary GetKnownTypeContracts(
            ClassDataContract dataContract,
            Dictionary<DataContract, object> handledContracts
        )
        {
            ...
            if (dataContract.Members != null)
            {
                ...
                foreach (DataMember dataMember in dataContract.Members)
                {
                    ...
                    if (
                        !objectMemberHandled
                        && dataContractSet.KnownTypesForObject != null
                        && SchemaImporter.IsObjectContract(memberContract)
                    )
                    {
                    ...
                    {
                        ContractCodeDomInfo memberCodeDomInfo = GetContractCodeDomInfo(
                            memberContract
                        );
                        ...
                        if (memberCodeDomInfo.ReferencedTypeExists)
                        {
                            AddKnownTypeContracts(
                                ...
                                GetKnownTypeContracts(
                                    (ClassDataContract)memberContract,
                                    handledContracts
                                )
                            );
                        }
        ...
        void AddKnownTypeContracts(
            ClassDataContract dataContract,
            DataContractDictionary knownContracts
        )
        {
            ...
            {
                if (
                    dataContract.StableName != pair.Key
                    && !dataContract.KnownDataContracts.ContainsKey(pair.Key)
                    && !pair.Value.IsBuiltInDataContract
                )
        ...
        {
            DataContractDictionary knownContractDictionary = GetKnownTypeContracts(
                dataContract,
                new Dictionary<DataContract, object>()
            );
        ...
        void AddPropertyChangedNotifier(ContractCodeDomInfo contractCodeDomInfo, bool isValueType)
        {
            ...
            )
            {
                ...
                contractCodeDomInfo
                    .GetMemberNames()
                    .Add(raisePropertyChangedEventMethod.Name, null);
            }
        }
        ...
        void ThrowIfReferencedBaseTypeSealed(Type baseType, DataContract dataContract)
        {
            if (baseType.IsSealed)
                throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    new InvalidOperationException(
                        SR.GetString(
                            SR.CannotDeriveFromSealedReferenceType,
                            dataContract.StableName.Name,
                            dataContract.StableName.Namespace,
        ...
        )
        {
            ...
            if (enumDataContract.IsFlags)
            {
                ...
                AddImportStatement(
                    Globals.TypeOfFlagsAttribute.Namespace,
                    contractCodeDomInfo.CodeNamespace
                );
            }
            ...
            dataContractAttribute.Arguments.Add(
                new CodeAttributeArgument(
                    Globals.NamespaceProperty,
                    new CodePrimitiveExpression(enumDataContract.StableName.Namespace)
                )
            );
            type.CustomAttributes.Add(dataContractAttribute);
            AddImportStatement(
            ...
            if (enumDataContract.Members != null)
            {
                for (int i = 0; i < enumDataContract.Members.Count; i++)
                {
                    ...
                    if (enumDataContract.IsULong)
                        enumMember.InitExpression = new CodeSnippetExpression(
                            enumDataContract.GetStringFromEnumValue(longValue)
                        );
                    else
        ...
        )
        {
            ...
            if (
                ...
                throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    new InvalidDataContractException(
                        SR.GetString(
                            SR.InvalidClrNamespaceGeneratedForISerializable,
                            dataContract.StableName.Name,
                            dataContract.StableName.Namespace,
                            DataContract.GetDataContractNamespaceFromUri(
                                dataContract.StableName.Namespace
                            ),
            ...
            string expectedName =
                (nestedTypeIndex <= 0 || nestedTypeIndex == dataContractName.Length - 1)
                    ? dataContractName
                    : dataContractName.Substring(nestedTypeIndex + 1);
            if (contractCodeDomInfo.TypeDeclaration.Name != expectedName)
                throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                    new InvalidDataContractException(
                        SR.GetString(
                            SR.InvalidClrNameGeneratedForISerializable,
                            dataContract.StableName.Name,
                            dataContract.StableName.Namespace,
        ...
        )
        {
            ...
            bool foundDictionaryBase = TryGetReferencedDictionaryType(
                collectionContract,
                out baseTypeReference
            );
                ...
                )
                {
                    ...
                    else
                    {
                        string expectedTypeName =
                            Globals.ArrayPrefix + itemContract.StableName.Name;
                        string expectedTypeNs = DataContract.GetCollectionNamespace(
                            itemContract.StableName.Namespace
                        );
                        throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidOperationException(
                                SR.GetString(
                                    SR.ReferencedBaseTypeDoesNotExist,
                                    dataContractName,
                                    collectionContract.StableName.Namespace,
                                    expectedTypeName,
                                    expectedTypeNs,
            ...
            collectionContractAttribute.Arguments.Add(
                new CodeAttributeArgument(
                    Globals.NamespaceProperty,
                    new CodePrimitiveExpression(collectionContract.StableName.Namespace)
                )
            );
            ...
            collectionContractAttribute.Arguments.Add(
                new CodeAttributeArgument(
                    Globals.ItemNameProperty,
                    new CodePrimitiveExpression(GetNameForAttribute(collectionContract.ItemName))
                )
            );
            if (foundDictionaryBase)
            {
                collectionContractAttribute.Arguments.Add(
                    new CodeAttributeArgument(
                        Globals.KeyNameProperty,
                        new CodePrimitiveExpression(GetNameForAttribute(collectionContract.KeyName))
                    )
                );
                collectionContractAttribute.Arguments.Add(
                    new CodeAttributeArgument(
                        Globals.ValueNameProperty,
            ...
            AddImportStatement(
                Globals.TypeOfCollectionDataContractAttribute.Namespace,
                contractCodeDomInfo.CodeNamespace
            );
        ...
        )
        {
            ...
            else
            {
                ...
                XmlQualifiedName typeName = xmlDataContract.IsAnonymous
                    ? SchemaImporter.ImportActualType(
                        xmlDataContract.XsdType.Annotation,
                ...
                typeNameField.InitExpression = new CodeObjectCreateExpression(
                    Globals.TypeOfXmlQualifiedName,
                    new CodePrimitiveExpression(typeName.Name),
                    new CodePrimitiveExpression(typeName.Namespace)
                );
                ...
                bool isElementNameDifferent =
                    (
                        xmlDataContract.TopLevelElementName != null
                        && xmlDataContract.TopLevelElementName.Value
                            != xmlDataContract.StableName.Name
                    )
                    || (
                        xmlDataContract.TopLevelElementNamespace != null
        ...
        )
        {
            ...
            if (
                CanDeclareAssemblyAttribute(contractCodeDomInfo)
        ...
        string GetMemberName(string memberName, ContractCodeDomInfo contractCodeDomInfo)
        {
            ...
            if (contractCodeDomInfo.GetMemberNames().ContainsKey(memberName))
            {
                ...
                for (int i = 1; ; i++)
                {
                    uniqueMemberName = AppendToValidClrIdentifier(
                        memberName,
                        i.ToString(NumberFormatInfo.InvariantInfo)
                    );
        ...
        void AddBaseMemberNames(
            ContractCodeDomInfo baseContractCodeDomInfo,
            ContractCodeDomInfo contractCodeDomInfo
        )
        {
            if (!baseContractCodeDomInfo.ReferencedTypeExists)
            {
                Dictionary<string, object> baseMemberNames =
                    baseContractCodeDomInfo.GetMemberNames();
        ...
        [Fx.Tag.SecurityNote(
            Critical = "Critical because it calls the CodeGenerator.IsValidLanguageIndependentIdentifier(..) method that has a LinkDemand.",
            Safe = "Safe because it doesn't leak security sensitive information."
        )]
        [SecuritySafeCritical]
                    ...
                    else
                    {
                        ...
                        if (ClrNamespaces.ContainsKey(clrNamespace))
                        {
                            ...
                            for (int i = 1; ; i++)
                            {
                                uniqueNamespace =
                                    (
                                        (clrNamespace.Length == 0)
                                            ? Globals.DefaultClrNamespace
                                            : clrNamespace
                                    ) + i.ToString(NumberFormatInfo.InvariantInfo);
        ...
        static void AddNamespaceFragment(
            StringBuilder builder,
            int fragmentOffset,
            int fragmentLength,
            Dictionary<string, object> fragments
            ...
            if (fragments.ContainsKey(nsFragment))
            {
                for (int i = 1; ; i++)
                {
                    ...
                    if (i == Int32.MaxValue)
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                            new InvalidDataContractException(
                                SR.GetString(SR.CannotComputeUniqueName, nsFragment)
                            )
                        );
                }
            }
        ...
        CodeTypeReference CodeTypeIPropertyChange
        {
            get
            {
                return GetCodeTypeReference(typeof(System.ComponentModel.INotifyPropertyChanged));
            }
        }
        ...
        CodeParameterDeclarationExpression StreamingContextParameter
        {
            ...
            {
                return new CodeParameterDeclarationExpression(
                    GetCodeTypeReference(Globals.TypeOfStreamingContext),
        ...
        CodeMemberMethod ReadXmlMethod
        {
            get
            {
                ...
                CodeParameterDeclarationExpression readerArg =
                    new CodeParameterDeclarationExpression(typeof(XmlReader), "reader");
                ...
                setNode.Left = new CodeFieldReferenceExpression(
                    ThisReference,
                    Globals.NodeArrayFieldName
                );
        ...
        CodeMemberMethod WriteXmlMethod
        {
            get
            {
                ...
                CodeParameterDeclarationExpression writerArg =
                    new CodeParameterDeclarationExpression(typeof(XmlWriter), "writer");
        ...
        CodeMemberMethod GetSchemaStaticMethod
        {
            get
            {
                ...
                CodeParameterDeclarationExpression paramDeclaration =
                    new CodeParameterDeclarationExpression(Globals.TypeOfXmlSchemaSet, "schemas");
        ...
        CodeConstructor ISerializableBaseConstructor
        {
            get
            {
                ...
                setObjectData.Left = new CodePropertyReferenceExpression(
                    ThisReference,
                    Globals.SerializationInfoFieldName
                );
        ...
        CodeConstructor ISerializableDerivedConstructor
        {
            get
            {
                ...
                derivedConstructor.BaseConstructorArgs.Add(
                    new CodeVariableReferenceExpression(Globals.SerializationInfoFieldName)
                );
                derivedConstructor.BaseConstructorArgs.Add(
                    new CodeVariableReferenceExpression(Globals.ContextFieldName)
                );
        ...
        CodeMemberProperty SerializationInfoProperty
        {
            get
            {
                return CreateProperty(
                    GetCodeTypeReference(Globals.TypeOfSerializationInfo),
                    Globals.SerializationInfoPropertyName,
                    Globals.SerializationInfoFieldName,
                    false /*isValueType*/
                );
            }
        ...
        CodeMemberMethod GetObjectDataMethod
        {
            get
            {
                ...
                CodeVariableDeclarationStatement getEnumerator =
                    new CodeVariableDeclarationStatement();
                getEnumerator.Type = GetCodeTypeReference(
                    Globals.TypeOfSerializationInfoEnumerator
                );
                ...
                CodePropertyReferenceExpression getCurrentName =
                    new CodePropertyReferenceExpression(
                        new CodeVariableReferenceExpression(Globals.SerializationEntryFieldName),
                        Globals.NameProperty
                    );
                CodePropertyReferenceExpression getCurrentValue =
                    new CodePropertyReferenceExpression(
                        new CodeVariableReferenceExpression(Globals.SerializationEntryFieldName),
                        Globals.ValueProperty
                    );
                ...
                loop.InitStatement = loop.IncrementStatement = new CodeSnippetStatement(
                    String.Empty
                );
        ...
        CodeMemberProperty ExtensionDataObjectProperty
        {
            get
            {
                ...
                propertyGet.Expression = new CodeFieldReferenceExpression(
                    ThisReference,
                ...
                propertySet.Left = new CodeFieldReferenceExpression(
                    ThisReference,
                    Globals.ExtensionDataObjectFieldName
                );
        ...
        CodeMemberEvent PropertyChangedEvent
        {
            get
            {
                ...
                propertyChangedEvent.Type = GetCodeTypeReference(
                    typeof(PropertyChangedEventHandler)
                );
        ...
        )
        {
            return CreateProperty(
                ...
                fieldName,
                isValueType,
                EnableDataBinding && SupportsDeclareEvents
            );
        }
            ...
            if (raisePropertyChanged)
            {
                ...
                if (!isValueType)
                {
                    left = new CodeMethodInvokeExpression(
                        new CodeTypeReferenceExpression(Globals.TypeOfObject),
                        "ReferenceEquals",
