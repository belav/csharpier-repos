...
namespace System.Runtime.Serialization
{
...
#endif
    {
        ...
        internal override bool IsBuiltInDataContract
        {
            get
            {
                return UnderlyingType == Globals.TypeOfXmlElement
        ...
        class XmlDataContractCriticalHelper : DataContract.DataContractCriticalHelper
        {
                ...
                : base(type)
            {
                ...
                object[] xmlRootAttributes =
                    (UnderlyingType == null)
                        ? null
                        : UnderlyingType.GetCustomAttributes(Globals.TypeOfXmlRootAttribute, false);
                if (xmlRootAttributes == null || xmlRootAttributes.Length == 0)
                {
                    if (hasRoot)
                    {
                        ...
                        topLevelElementNamespace =
                            (this.StableName.Namespace == Globals.SchemaNamespace)
        ...
        internal CreateXmlSerializableDelegate GenerateCreateXmlSerializableDelegate()
        {
            ...
            if (type.IsValueType)
            {
                System.Reflection.Emit.LocalBuilder local = ilg.DeclareLocal(
                    type,
                    type.Name + "Value"
                );
                ilg.Ldloca(local);
...
#if !NO_DYNAMIC_CODEGEN
        [Fx.Tag.SecurityNote(
            Miscellaneous = "RequiresReview - Calculates whether this Xml type requires MemberAccessPermission for deserialization."
