    ...
    partial class JsonFormatWriterGenerator
    {
        ...
        )
        {
            try
            {
                if (TD.DCJsonGenWriterStartIsEnabled())
                {
                    TD.DCJsonGenWriterStart(
                        "Collection",
                        collectionContract.UnderlyingType.FullName
                    );
                }
...
#if !NO_DYNAMIC_CODEGEN
        [Fx.Tag.SecurityNote(
            Miscellaneous = "RequiresReview - handles all aspects of IL generation including initializing the DynamicMethod."
        ...
        partial class CriticalHelper
        {
            ...
            )
            {
                ...
                try
                {
                    BeginMethod(
                        ilg,
                        "Write" + classContract.StableName.Name + "ToJson",
            ...
            )
            {
                ...
                try
                {
                    BeginMethod(
                        ilg,
                        "Write" + collectionContract.StableName.Name + "ToJson",
            ...
            void DemandMemberAccessPermission(bool memberAccessFlag)
            {
                if (memberAccessFlag)
                {
                    ilg.Call(
                        contextArg,
                        XmlFormatGeneratorStatics.DemandMemberAccessPermissionMethod
                    );
                }
            }
            ...
            void DemandSerializationFormatterPermission(ClassDataContract classContract)
            {
                if (!classContract.HasDataContract && !classContract.IsNonAttributedType)
                {
                    ilg.Call(
                        contextArg,
                        XmlFormatGeneratorStatics.DemandSerializationFormatterPermissionMethod
                    );
                }
            }
            ...
            private LocalBuilder LoadMemberValue(DataMember member)
            {
                ...
                LocalBuilder memberValue = ilg.DeclareLocal(
                    member.MemberType,
                    member.Name + "Value"
                );
            ...
            )
            {
                ...
                if (writeArrayMethod != null)
                {
                    ...
                    ilg.Call(
                        xmlWriterArg,
                        typeof(JsonWriterDelegator).GetMethod(
                            writeArrayMethod,
                            Globals.ScanAllMembers,
                            null,
                            new Type[]
            ...
            )
            {
                ...
                ilg.Load(memberValue);
                ilg.ConvertValue(memberValue.LocalType, Globals.TypeOfObject);
                LocalBuilder typeHandleValue = ilg.DeclareLocal(
