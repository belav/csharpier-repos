...
namespace System.Activities.Runtime
{
    ...
    class ActivityInstanceMap
    {
        ...
        private static void AddBlockingActivity(
        ...
        )
        {
            ...
            {
                ActivityBlockingUpdate.AddBlockingActivity(
                    ref updateErrors,
            ...
            {
                string updatedId = updatedActivity.MapEntry.IsRemoval
                    ? null
                    : updatedActivity.NewId.ToString();
                ActivityBlockingUpdate.AddBlockingActivity(
                    ref updateErrors,
                    updatedId,
                    originalId.ToString(),
        ...
        private static bool CanCompensationOrConfirmationHandlerReferenceAddedSymbols(
        ...
        )
        {
            for (int j = 0; j < instanceList.Count; j++)
            {
                ...
                if (
                    ...
                    || !IsNonDefaultSecondaryRoot(activityInstance, secondaryRootInstances)
                )
                {
                    continue;
                }
                ...
                int[] parentOfSecondaryRootOriginalQID = new int[
                    secondaryRootOriginalQID.Length - 1
                ];
                Array.Copy(
                ...
                for (int i = 0; i < parentOfSecondaryRootOriginalQID.Length; i++)
                {
                    ...
                    DynamicUpdateMap.UpdatedActivity updatedActivity =
                        rootUpdateMap.GetUpdatedActivity(
                            new QualifiedId(currentQIDBuilder.ToArray()),
                            rootIdSpace
                        );
                    if (updatedActivity.MapEntry != null)
                    {
                        ...
                        do
                        {
                            if (
                                !entry.IsRemoval
                                && entry.HasEnvironmentUpdates
                                && entry.EnvironmentUpdateMap.IsAdditionToNoSymbols
                            )
                            {
                                int[] noSymbolAddActivityIDArray = currentQIDBuilder.ToArray();
                                noSymbolAddActivityIDArray[noSymbolAddActivityIDArray.Length - 1] =
                                    entry.OldActivityId;
                                QualifiedId noSymbolAddActivityQID = new QualifiedId(
                                    noSymbolAddActivityIDArray
                                );
                                DynamicUpdateMap.UpdatedActivity noSymbolAddUpdatedActivity =
                                    rootUpdateMap.GetUpdatedActivity(
                                ...
                                AddBlockingActivity(
                                    ...
                                    noSymbolAddActivityQID,
                                    SR.VariableOrArgumentAdditionToReferencedEnvironmentNoDUSupported,
                                    null
                                );
                                return true;
                            }
        ...
        private static bool IsInvalidEnvironmentUpdate(
            InstanceList instanceList,
            DynamicUpdateMap.UpdatedActivity updatedActivity,
            ref Collection<ActivityBlockingUpdate> updateErrors
        )
            ...
            for (int j = 0; j < instanceList.Count; j++)
            {
                ...
                if (activityInstance != null)
                {
                    ...
                    if (error != null)
                    {
                        AddBlockingActivity(
                            ref updateErrors,
                            updatedActivity,
                            new QualifiedId(instanceList.ActivityId),
                            error,
                            activityInstance.Id
                        );
                        return true;
                ...
                else
                {
                    ...
                    if (environment != null)
                    {
                        ...
                        if (
                            (
                                envMap.HasVariableEntries
                                && TryGatherSchedulableExpressions(
                                    envMap.VariableEntries,
                                    out dummyIndexes
                                )
                            )
                            || (
                                envMap.HasPrivateVariableEntries
                                && TryGatherSchedulableExpressions(
        ...
        )
        {
            ...
            if (updatedActivity.MapEntry.IsRemoval)
            {
                ...
                error = SR.CannotRemoveExecutingActivityUpdateError(
                    oldQualifiedId,
                    updatedActivity.MapEntry.DisplayName
                );
            }
        ...
        {
            IList<InstanceListNeedingUpdate> instanceListsToUpdate =
                new List<InstanceListNeedingUpdate>();
            ...
            for (int i = 0; i < this.rawDeserializedLists.Length; i++)
            {
                ...
                DynamicUpdateMap.UpdatedActivity updatedActivity = updateMap.GetUpdatedActivity(
                    oldQualifiedId,
                    rootIdSpace
                );

                if (
                    CanCompensationOrConfirmationHandlerReferenceAddedSymbols(
                        list,
                        updateMap,
                        rootIdSpace,
                        secondaryRootInstances,
                ...
                else if (updatedActivity.MapEntry.IsParentRemovedOrBlocked)
                {
                    ...
                    IsRemovalOrRTUpdateBlockedOrBlockedByUser(
                        updatedActivity,
                        oldQualifiedId,
                        out error
                    )
        ...
        )
        {
            ...
            foreach (InstanceListNeedingUpdate update in this.updateList)
            {
                ...
                if (update.MapEntry.HasEnvironmentUpdates)
                {
                    ...
                    Fx.Assert(
                        update.NewActivity != null,
                        "TryGetUpdateMapEntryFromRootMap should have thrown if it couldn't map to an activity"
                    );
                    instanceList.UpdateEnvironments(
                        update.MapEntry.EnvironmentUpdateMap,
                        update.NewActivity
                    );
                }
        ...
        private static bool TryGatherSchedulableExpressions(
            IList<EnvironmentUpdateMapEntry> entries,
            out List<int> addedLocationReferenceIndexes
        )
        {
        ...
        public void UpdateInstanceByActivityParticipation(
        ...
        )
        {
            foreach (InstanceListNeedingUpdate participant in this.updateList)
            {
                ...
                {
                    Fx.Assert(
                        participant.UpdateMap == null && participant.MapEntry == null,
                        "UpdateMap and MapEntry must be null if we are here."
                    );
                        ...
                        try
                        {
                            ...
                            if (updateContext.IsUpdateDisallowed)
                            {
                                ActivityBlockingUpdate.AddBlockingActivity(
                                    ref updateErrors,
                                    instance.Activity,
                                    new QualifiedId(participant.OriginalId).ToString(),
                                    updateContext.DisallowedReason,
                                    instance.Id
                                );
                                continue;
                            }
            ...
            for (int i = this.updateList.Count - 1; i >= 0; i--)
            {
                ...
                for (int j = 0; j < participant.InstanceList.Count; j++)
                {
                    ...
                    )
                    {
                        ...
                        instance.ResolveNewArgumentsDuringDynamicUpdate(
                            activityExecutor,
        ...
        )
        {
            Fx.Assert(
                ...
                "We should always have deserialized some lists."
            );

            this.instanceMapping = new Dictionary<Activity, InstanceList>(
                this.rawDeserializedLists.Length
            );
            ...
            Func<ActivityInstance, ActivityExecutor, bool> processInstanceCallback = new Func<
                ActivityInstance,
                ActivityExecutor,
                bool
            >(OnActivityInstanceLoaded);
        ...
        internal class InstanceList : HybridCollection<IActivityReference>
        {
            ...
            public void UpdateEnvironments(EnvironmentUpdateMap map, Activity activity)
            {
                if (base.SingleItem != null)
                {
                    IActivityReferenceWithEnvironment reference =
                        base.SingleItem as IActivityReferenceWithEnvironment;
                    if (reference != null)
                    {
                ...
                else
                {
                    ...
                    {
                        IActivityReferenceWithEnvironment reference =
                            base.MultipleItems[i] as IActivityReferenceWithEnvironment;
                        if (reference != null)
                        {
                            reference.UpdateEnvironment(map, activity);
                        }
        ...
        class InstanceListNeedingUpdate
        {
            ...
            {
                get { return this.MapEntry == null && this.NewId == null; }
            }

            public bool MapEntryExists
            {
                get { return this.MapEntry != null; }
            }
