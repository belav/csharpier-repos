...
namespace System.Activities.DynamicUpdate
{
    ...
    static class ActivityComparer
    {
        public static bool HasPrivateMemberOtherThanArgumentsChanged(
            ...
            bool isMemberOfUpdatedIdSpace,
            out DynamicUpdateMap argumentChangesMap
        )
        {
            Fx.Assert(
                currentElement != null && originalElement != null,
                "Both activities must be non-null."
            );
            ...
            if (
                (
                    !isMemberOfUpdatedIdSpace
                    || IsAnyNameless(currentElement.ImplementationVariables)
                    || IsAnyNameless(originalElement.ImplementationVariables)
                )
                && !ListEquals(
                    currentElement.ImplementationVariables,
                    originalElement.ImplementationVariables,
                    CompareVariableEquality
                )
            )
            {
                return true;
            ...
            if (currentPrivateIdSpace == null && originalPrivateIdSpace == null)
            {
                ...
                (currentPrivateIdSpace != null && originalPrivateIdSpace == null)
                || (currentPrivateIdSpace == null && originalPrivateIdSpace != null)
            )
            {
                return true;
            ...
            if (
                !ListEquals<ActivityDelegate>(
                    currentElement.ImplementationDelegates,
                    originalElement.ImplementationDelegates,
                    CompareDelegateEquality
                )
            )
            {
                return true;
            ...
            PrivateIdSpaceMatcher privateIdSpaceMatcher = new PrivateIdSpaceMatcher(
                nestedFinalizer,
                originalPrivateIdSpace,
                currentPrivateIdSpace
            );
            return !privateIdSpaceMatcher.Match(out argumentChangesMap);
        }

        public static bool ListEquals(
            IList<RuntimeDelegateArgument> currentArguments,
            IList<RuntimeDelegateArgument> originalArguments
        )
        {
            return ListEquals(
                currentArguments,
                originalArguments,
                CompareRuntimeDelegateArgumentEquality
            );
        }

        public static bool ListEquals(
            IList<ArgumentInfo> currentArguments,
            IList<ArgumentInfo> originalArguments
        )
        {
        ...
        public static bool ListEquals<T>(
            IList<T> currentMembers,
            IList<T> originalMembers,
        ...
        {
            return AreVariableNamesIdentical(leftVar.Name, rightVar.Name)
        ...
        static IList<ActivityDelegate> GetDeclaredDelegates(
            IList<ActivityDelegate> collection,
        ...
        static bool CompareChildEquality(
            Activity currentChild,
            IdSpace currentIdSpace,
            Activity originalChild,
            IdSpace originalIdSpace
        )
        {
            if (currentChild == null && originalChild == null)
            {
                ...
                (currentChild == null && originalChild != null)
                || (currentChild != null && originalChild == null)
            )
            {
                return false;
            ...
            if (
                (
                    currentChild.MemberOf == currentIdSpace
                    && originalChild.MemberOf != originalIdSpace
                )
                || (
                    currentChild.MemberOf != currentIdSpace
                    && originalChild.MemberOf == originalIdSpace
        ...
        static bool CompareDelegateEquality(
            ActivityDelegate currentDelegate,
            ActivityDelegate originalDelegate
        )
        {
            Fx.Assert(
                currentDelegate != null && originalDelegate != null,
                "Both currentDelegate and originalDelegate must be non-null."
            );

            if (
                !ListEquals(
                    currentDelegate.RuntimeDelegateArguments,
                    originalDelegate.RuntimeDelegateArguments
                )
            )
            {
                return false;
            ...
            bool isImplementation =
                currentDelegate.ParentCollectionType == ActivityCollectionType.Implementation;
            Fx.Assert(
                originalDelegate.ParentCollectionType == currentDelegate.ParentCollectionType,
                "Mismatched delegates"
            );
            IdSpace currentIdSpace = isImplementation
                ? currentDelegate.Owner.ParentOf
                : currentDelegate.Owner.MemberOf;
            IdSpace originalIdSpace = isImplementation
                ? originalDelegate.Owner.ParentOf
                : originalDelegate.Owner.MemberOf;

            return CompareChildEquality(
                currentDelegate.Handler,
                currentIdSpace,
                originalDelegate.Handler,
                originalIdSpace
            );
        }
        ...
        {
            Fx.Assert(
                currentVariable != null && originalVariable != null,
                "Both currentVariable and originalVariable must be non-null."
            );
            ...
            Fx.Assert(
                currentVariable.IsPublic == originalVariable.IsPublic,
                "Mismatched variables"
            );
            IdSpace currentIdSpace = currentVariable.IsPublic
                ? currentVariable.Owner.MemberOf
                : currentVariable.Owner.ParentOf;
            IdSpace originalIdSpace = originalVariable.IsPublic
                ? originalVariable.Owner.MemberOf
                : originalVariable.Owner.ParentOf;
            return CompareChildEquality(
                currentVariable.Default,
                currentIdSpace,
                originalVariable.Default,
                originalIdSpace
            );
        }

        static bool CompareArgumentEquality(
            RuntimeArgument currentArgument,
            RuntimeArgument originalArgument
        )
        {
            Fx.Assert(
                currentArgument != null && originalArgument != null,
                "Both currentArgument and originalArgument must be non-null."
            );

            if (
                currentArgument.Name != originalArgument.Name
            ...
            if (currentArgument.BoundArgument == null && originalArgument.BoundArgument == null)
            {
                ...
                (currentArgument.BoundArgument != null && originalArgument.BoundArgument == null)
                || (currentArgument.BoundArgument == null && originalArgument.BoundArgument != null)
            )
            {
                return false;
            ...
            return CompareChildEquality(
                currentArgument.BoundArgument.Expression,
                currentArgument.Owner.MemberOf,
                originalArgument.BoundArgument.Expression,
                originalArgument.Owner.MemberOf
            );
        }

        static bool CompareRuntimeDelegateArgumentEquality(
            RuntimeDelegateArgument newRuntimeDelegateArgument,
            RuntimeDelegateArgument oldRuntimeDelegateArgument
        )
        {
            // compare Name, Type and Direction
            if (
                !newRuntimeDelegateArgument.Name.Equals(oldRuntimeDelegateArgument.Name)
            ...
            return CompareDelegateArgumentEquality(
                newRuntimeDelegateArgument.BoundArgument,
                oldRuntimeDelegateArgument.BoundArgument
            );
        }

        static bool CompareDelegateArgumentEquality(
            DelegateArgument newBoundArgument,
            DelegateArgument oldBoundArgument
        )
        {
        ...
        class PrivateIdSpaceMatcher
        {
            ...
            public bool Match(out DynamicUpdateMap argumentChangesMap)
            {
                ...
                {
                    nextOriginalSubrootId = GetIndexOfNextSubtreeRoot(
                        this.originalPrivateIdSpace,
                        nextOriginalSubrootId
                    );
                    nextUpdatedSubrootId = GetIndexOfNextSubtreeRoot(
                        this.updatedPrivateIdSpace,
                        nextUpdatedSubrootId
                    );

                    if (nextOriginalSubrootId != -1 && nextUpdatedSubrootId != -1)
                    {
                        // found next disjoint subtree pair to match
                        this.PrepareToMatchSubtree(
                            this.updatedPrivateIdSpace[nextUpdatedSubrootId],
                            this.originalPrivateIdSpace[nextOriginalSubrootId]
                        );
                    }
                ...
                while (this.matchedActivities.Count > 0)
                {
                    ...
                    Fx.Assert(
                        originalActivity.MemberOf == this.originalPrivateIdSpace
                            && currentActivity.MemberOf == this.updatedPrivateIdSpace,
                        "neither activities must be a reference."
                    );

                    if (
                        currentActivity.GetType() != originalActivity.GetType()
                    ...
                    if (
                        !ActivityComparer.ListEquals(
                            ActivityComparer.GetDeclaredChildren(
                    ...
                    if (
                        !ActivityComparer.ListEquals(
                            ActivityComparer.GetDeclaredChildren(
                                currentActivity.ImportedChildren,
                                currentActivity
                            ),
                            ActivityComparer.GetDeclaredChildren(
                    ...
                    if (
                        !ActivityComparer.ListEquals<ActivityDelegate>(
                            ActivityComparer.GetDeclaredDelegates(
                                currentActivity.Delegates,
                                currentActivity
                            ),
                            ActivityComparer.GetDeclaredDelegates(
                    ...
                    if (
                        !ActivityComparer.ListEquals<ActivityDelegate>(
                            ActivityComparer.GetDeclaredDelegates(
                                currentActivity.ImportedDelegates,
                                currentActivity
                            ),
                            ActivityComparer.GetDeclaredDelegates(
                    ...
                    if (
                        !ActivityComparer.ListEquals<Variable>(
                            currentActivity.RuntimeVariables,
                            originalActivity.RuntimeVariables,
            ...
            )
            {
                ...
                if (entry.HasEnvironmentUpdates)
                {
                    if (entry.EnvironmentUpdateMap.HasArgumentEntries)
                    {
                        ...
                        )
                        {
                            if (!argumentEntry.IsAddition)
                            {
                                ...
                                RuntimeArgument originalArg = originalActivity.RuntimeArguments[
                                    argumentEntry.OldOffset
                                ];
                                RuntimeArgument updatedArg = currentActivity.RuntimeArguments[
                    ...
                    foreach (
                        RuntimeArgument oldRuntimeArgument in originalActivity.RuntimeArguments
                    )
                    {
                        if (
                            newArgumentInfos.IndexOf(new ArgumentInfo(oldRuntimeArgument))
                            == EnvironmentUpdateMapEntry.NonExistent
                        )
                        {
                            // this is a removed argument.
                            if (
                                oldRuntimeArgument.IsBound
                                && oldRuntimeArgument.BoundArgument.Expression != null
                                && oldRuntimeArgument.BoundArgument.Expression.MemberOf
                                    == originalActivity.MemberOf
                            )
                            {
                    ...
                    currentActivity.RuntimeArguments != null
                    && currentActivity.RuntimeArguments.Count > 0
                )
                {
                    Fx.Assert(
                        currentActivity.RuntimeArguments.Count
                            == originalActivity.RuntimeArguments.Count,
                        "RuntimeArguments.Count for both currentActivity and originalActivity must be equal."
                    );
                    ...
                    {
                        if (
                            !this.TryPreparingArgumentExpressions(
                                originalActivity.RuntimeArguments[i],
                                currentActivity.RuntimeArguments[i]
                            )
                        )
                        {
                            return false;
            ...
            bool TryPreparingArgumentExpressions(
            ...
            )
            {
                ...
                {
                    Fx.Assert(
                        originalArg.BoundArgument != null
                            && originalArg.BoundArgument.Expression != null,
                        "both Expressions are either non-null or null."
                    );
                    this.PrepareToMatchSubtree(
                        updatedArg.BoundArgument.Expression,
                        originalArg.BoundArgument.Expression
                    );
                }
            ...
            {
                Fx.Assert(
                    currentActivity != null && originalActivity != null,
                    "both activities must not be null."
                );
            ...
            bool CompareDelegateEqualityAndAddActivitiesPair(
                ActivityDelegate currentDelegate,
                ActivityDelegate originalDelegate
            )
            {
                ...
                {
                    Fx.Assert(
                        originalDelegate.Handler != null,
                        "both handlers are either non-null or null."
                    );
            ...
            bool CompareVariableEqualityAndAddActivitiesPair(
            ...
            )
            {
                ...
                {
                    Fx.Assert(
                        originalVariable.Default != null,
                        "both Defaults are either non-null or null."
                    );
