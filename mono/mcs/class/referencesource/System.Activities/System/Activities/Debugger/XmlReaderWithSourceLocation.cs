...
namespace System.Activities.Debugger
{
    ...
    internal class XmlReaderWithSourceLocation : XmlWrappingReader
    {
        ...
        public XmlReaderWithSourceLocation(TextReader underlyingTextReader)
        {
            ...
            CharacterSpottingTextReader characterSpottingTextReader =
                new CharacterSpottingTextReader(underlyingTextReader);
            this.BaseReader = XmlReader.Create(characterSpottingTextReader);
            UnitTestUtility.Assert(
                this.BaseReaderAsLineInfo != null,
        ...
        public Dictionary<DocumentLocation, DocumentLocation> StartElementLocations
        {
            get
            {
                ...
                {
                    this.startElementLocations =
                        new Dictionary<DocumentLocation, DocumentLocation>();
                }
        ...
        public override bool Read()
        {
            ...
            if (this.NodeType == Xml.XmlNodeType.Element)
            {
                ...
                {
                    DocumentLocation startElementBracket = this.FindStartElementBracket(
                        elementLocation
                    );
                ...
                if (attributeCount > 0)
                {
                    for (int i = 0; i < attributeCount; i++)
                    {
                        ...
                        DocumentRange attributeValueRange = this.FindAttributeValueLocation(
                            memberLocation
                        );
            ...
            else if (this.NodeType == Xml.XmlNodeType.Text)
            {
                UnitTestUtility.Assert(
                    this.contentStartLocationStack.Count > 0,
                    "Adding Text with out StartElement?"
                );
        ...
        private DocumentRange FindEmptyElementRange(DocumentLocation elementLocation)
        {
            ...
            UnitTestUtility.Assert(
                startBracket != null,
                "XmlReader should guarantee there must be a start angle bracket."
            );
            UnitTestUtility.Assert(
                endBracket != null,
                "XmlReader should guarantee there must be an end angle bracket."
            );
