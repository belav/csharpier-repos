...
namespace System.Activities.Debugger
{
    ...
    public static class SourceLocationProvider
    {
        ...
        [Fx.Tag.SecurityNote(
            Critical = "Asserting FileIOPermission(Read) for the specified file name that is contained the attached property on the XAML.",
        ...
        )
        {
            ...
            if (
                string.IsNullOrEmpty(symbolString)
                && rootActivity.Children != null
                && rootActivity.Children.Count > 0
            )
                    ...
                    if (wfSymbol != null)
                    {
                        ...
                        if (rootActivity.RootActivity != rootActivity)
                        {
                            Fx.Assert(
                                ...
                                "Compiled XAML implementation always have a parent."
                            );
                        ...
                        return GetSourceLocations(
                            rootActivity,
                            wfSymbol,
                            translateInternalActivityToOrigin: false
                        );
                    }
                }
            ...
            try
            {
                ...
                if (
                    TryGetSourceLocation(rootActivity, sourcePath, checksum, out tempSourceLocation)
                ) // already has source location.
                {
        ...
        public static Dictionary<object, SourceLocation> GetSourceLocations(
            Activity rootActivity,
        ...
        )
        {
            return GetSourceLocations(
                rootActivity,
                symbol,
                translateInternalActivityToOrigin: true
            );
        }
        ...
        )
        {
            ...
            if (!workflowRoot.IsMetadataFullyCached)
            {
                ...
                ActivityUtilities.CacheRootMetadata(
                    ...
                    ProcessActivityTreeOptions.ValidationOptions,
                    null,
                    ref validationErrors
                );
            }

            Dictionary<object, SourceLocation> newMapping =
        ...
        [Fx.Tag.SecurityNote(
            Miscellaneous = "RequiresReview - We are deserializing XAML from a file. The file may have been read under and Assert for FileIOPermission. The data hould be validated and not cached."
        )]
        internal static object Deserialize(byte[] buffer, Assembly localAssembly)
        {
            using (MemoryStream memoryStream = new MemoryStream(buffer))
            {
                ...
                {
                    using (
                        XamlDebuggerXmlReader xamlDebuggerReader = new XamlDebuggerXmlReader(
                            streamReader,
                    ...
                    {
                        xamlDebuggerReader.SourceLocationFound +=
                            XamlDebuggerXmlReader.SetSourceLocation;

                        using (
                            XamlReader activityBuilderReader =
                                ActivityXamlServices.CreateBuilderReader(xamlDebuggerReader)
                        )
                        {
        ...
        public static void CollectMapping(
        ...
        [Fx.Tag.SecurityNote(
            Miscellaneous = "RequiresReview - We are dealing with activity and SourceLocation information that came from the user, possibly under an Assert for FileIOPermission. The data hould be validated and not cached."
        )]
        static void CollectMapping(
            Activity rootActivity1,
            Activity rootActivity2,
            Dictionary<object, SourceLocation> mapping,
            string path,
        ...
        )
        {
            ...
            if (
                (requirePrepareForRuntime && !realRoot1.IsRuntimeReady)
            ...
            Activity realRoot2 =
                (rootActivity2.RootActivity != null) ? rootActivity2.RootActivity : rootActivity2;
            if (
                rootActivity1 != rootActivity2
                    && (requirePrepareForRuntime && !realRoot2.IsRuntimeReady)
            ...
            while (pairsRemaining.Count > 0)
            {
                ...
                if (TryGetSourceLocation(activity2, path, checksum, out sourceLocation))
                {
                    ...
                    !(
                        (activity2 is IExpressionContainer)
                        || (activity2 is IValueSerializableExpression)
                    )
                ) // Expression is known not to have source location.
                {
                ...
                if (
                    !(
                        (activity1 is IExpressionContainer)
                        || (activity2 is IExpressionContainer)
                        || (activity1 is IValueSerializableExpression)
                        || (activity2 is IValueSerializableExpression)
                    )
                )
                {
        ...
        )
        {
            ...
            if (
                AttachablePropertyServices.TryGetProperty<int>(
                    obj,
                    XamlDebuggerXmlReader.StartLineName,
                    out startLine
                )
                && AttachablePropertyServices.TryGetProperty<int>(
                    obj,
                    XamlDebuggerXmlReader.StartColumnName,
                    out startColumn
                )
                && AttachablePropertyServices.TryGetProperty<int>(
                    obj,
                    XamlDebuggerXmlReader.EndLineName,
                    out endLine
                )
                && AttachablePropertyServices.TryGetProperty<int>(
                    obj,
                    XamlDebuggerXmlReader.EndColumnName,
                    out endColumn
                )
        ...
        public static ICollection<ActivitySymbol> GetSymbols(
            Activity rootActivity,
        ...
        )
        {
            ...
            Activity realRoot =
                (rootActivity.RootActivity != null) ? rootActivity.RootActivity : rootActivity;
            ...
            while (activitiesRemaining.Count > 0)
            {
                ...
                SourceLocation sourceLocation;
                object origin =
                    currentActivity.Origin == null ? currentActivity : currentActivity.Origin;
                if (
                    !visited.Contains(currentActivity)
                    && sourceLocations.TryGetValue(origin, out sourceLocation)
                )
                {
                    symbols.Add(
                        new ActivitySymbol
                ...
                foreach (
                    Activity childActivity in WorkflowInspectionServices.GetActivities(
                        currentActivity
                    )
                )
                {
