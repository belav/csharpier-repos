...
namespace System.Activities.Validation
{
    ...
    static class ValidationHelper
    {
                ...
                foreach (RuntimeArgument argument in requiredArgumentsNotInOverloadGroups)
                {
                    if (CheckIfArgumentIsNotBound(argument, inputs))
                    {
                        ActivityUtilities.Add(
                            ...
                            new ValidationError(
                                SR.RequiredArgumentValueNotSupplied(argument.Name),
                                false,
                                argument.Name,
                                activity
                            )
            ...
            if (!overloadGroups.IsNullOrEmpty())
            {
                ...
                if (configuredCount == 0)
                {
                    if (overloadGroupsWithNoRequiredArgs == 0)
                    {
                        ActivityUtilities.Add(
                            ref validationErrors,
                            new ValidationError(SR.NoOverloadGroupsAreConfigured, false, activity)
                        );
                    }
                }
                ...
                {
                    HashSet<RuntimeArgument> configuredOverloadSet = new HashSet<RuntimeArgument>(
                    ...
                    )
                    {
                        ...
                        if (disjointGroups != null && disjointGroups.Contains(groupName))
                        {
                            ...
                            )
                            {
                                ActivityUtilities.Add(
                                    ref validationErrors,
                                    new ValidationError(
                                        SR.ExtraOverloadGroupPropertiesConfigured(
                                            configuredGroupName,
                                            configuredArgument.Name,
                                            groupName
                                        ),
                                        false,
                                        activity
                        ...
                        else if (overlappingGroups != null && overlappingGroups.Contains(groupName))
                        {
                            ...
                            HashSet<RuntimeArgument> overloadGroupSet =
                                new HashSet<RuntimeArgument>(overloadGroups[groupName]);
                            IEnumerable<RuntimeArgument> intersectSet = overloadGroupSet.Intersect(
                                configuredOverloadSet
                            );
                            List<RuntimeArgument> exceptList = overloadGroupSet
                                .Except(intersectSet)
                                .ToList();

                            foreach (
                                RuntimeArgument configuredArgument in exceptList.FindAll(
                                    checkIfArgumentIsBound
                                )
                            )
                            {
                                ActivityUtilities.Add(
                ...
                else
                {
                    ...
                    ActivityUtilities.Add(
                        ref validationErrors,
                        new ValidationError(
                            SR.MultipleOverloadGroupsConfigured(
                                configuredGroups.AsCommaSeparatedValues()
                            ),
                            false,
                            activity
                        )
        ...
        )
        {
            ...
            return ValidateOverloadGroupDefinitions(
                activity,
                equivalenceInfo,
                overloadGroups,
                ref validationErrors
        ...
        {
            Fx.Assert(
                equivalenceInfo != null,
                "equivalenceInfo should have been setup before calling this method"
            );
            ...
            {
                Hashtable keysVisited = new Hashtable(
                ...
                foreach (
                    KeyValuePair<
                        string,
                        List<string>
                    > entry in equivalenceInfo.EquivalentGroupsDictionary
                )
                {
                    if (!keysVisited.Contains(entry.Key))
                    {
                        ...
                        IEnumerable<string> sortedList = equivalentGroups.OrderBy(
                            (s) => s,
                        ...
                        ActivityUtilities.Add(
                            ref validationErrors,
                            new ValidationError(
                                SR.OverloadGroupsAreEquivalent(sortedList.AsCommaSeparatedValues()),
                                false,
                                activity
                            )
                        );
                ...
                )
                {
                    ...
                    if (index > 0)
                    {
                        ActivityUtilities.Add(
                            ref validationErrors,
                            new ValidationError(
                                SR.OverloadGroupHasSubsets(
                                    entry.Key,
                                    subsetGroups.AsCommaSeparatedValues()
                                ),
                                false,
                                activity
                            )
        ...
        static OverloadGroupEquivalenceInfo GetOverloadGroupEquivalence(
            Dictionary<string, List<RuntimeArgument>> groupDefinitions
        )
        {
            ...
            if (!groupDefinitions.IsNullOrEmpty())
            {
                ...
                for (int i = 0; i < groupNames.Length; i++)
                {
                    ...
                    HashSet<RuntimeArgument> group1Args = new HashSet<RuntimeArgument>(
                        groupDefinitions[group1]
                    );
                    for (int j = i + 1; j < groupNames.Length; j++)
                    {
                        string group2 = groupNames[j];
                        HashSet<RuntimeArgument> group2Args = new HashSet<RuntimeArgument>(
                            groupDefinitions[group2]
                        );
        ...
        static bool CheckIfArgumentIsNotBound(
        ...
        )
        {
            if (
                argument.Owner != null
                && argument.Owner.Parent == null
                && ArgumentDirectionHelper.IsOut(argument.Direction)
            )
            {
        ...
        static bool CheckIfArgumentIsBound(
            RuntimeArgument argument,
            IDictionary<string, object> inputs
        )
        {
        ...
        public class OverloadGroupEquivalenceInfo
        {
            ...
            )
            {
                ...
                {
                    Fx.Assert(
                        !listValues.Contains(listEntry),
                        string.Format(
                            CultureInfo.InvariantCulture,
                            "Duplicate group entry '{0}' getting added.",
                            listEntry
                        )
                    );
