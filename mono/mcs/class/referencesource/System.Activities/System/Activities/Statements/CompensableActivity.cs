...
namespace System.Activities.Statements
{
    ...
    public sealed class CompensableActivity : NativeActivity<CompensationToken>
    {
        static Constraint noCompensableActivityInSecondaryRoot =
            CompensableActivity.NoCompensableActivityInSecondaryRoot();
        ...
        CompensationParticipant CompensationParticipant
        {
            ...
            set { this.compensationParticipant = value; }
        }
        ...
        protected override void CacheMetadata(NativeActivityMetadata metadata)
        {
            ...
            if (
                !this.IsSingletonActivityDeclared(
                    CompensationActivityStrings.WorkflowImplicitCompensationBehavior
                )
            )
            {
                WorkflowCompensationBehavior workflowCompensationBehavior =
                    new WorkflowCompensationBehavior();
                this.DeclareSingletonActivity(
                    CompensationActivityStrings.WorkflowImplicitCompensationBehavior,
        ...
        static Constraint NoCompensableActivityInSecondaryRoot()
        {
            ...
            DelegateInArgument<CompensableActivity> element =
                new DelegateInArgument<CompensableActivity> { Name = "element" };
        ...
        protected override void Execute(NativeActivityContext context)
        {
            ...
            {
                compensationExtension.SetupWorkflowCompensationBehavior(
                    context,
                    new BookmarkCallback(OnWorkflowCompensationBehaviorScheduled),
                    GetSingletonActivity(
                        CompensationActivityStrings.WorkflowImplicitCompensationBehavior
                    )
                );
            }
        ...
        void OnWorkflowCompensationBehaviorScheduled(
        ...
        )
        {
            ...
            if (parentToken != null)
            {
                if (
                    compensationExtension
                        .Get(parentToken.CompensationId)
            ...
            CompensationTokenData tokenData = new CompensationTokenData(
                compensationExtension.GetNextId(),
                parentCompensationId
            )
            {
            ...
            else
            {
                ...
                if (TD.CompensationStateIsEnabled())
                {
                    TD.CompensationState(
                        ...
                        tokenData.CompensationState.ToString()
                    );
                }
        ...
        )
        {
            ...
            CompensationTokenData token = compensationExtension.Get(
                this.currentCompensationId.Get(context)
            );
        ...
        void ScheduleSecondaryRoot(
            NativeActivityContext context,
            CompensationExtension compensationExtension,
            CompensationTokenData token
        )
        {
            ...
            Fx.Assert(
                token.BookmarkTable[CompensationBookmarkName.OnSecondaryRootScheduled] == null,
                "Bookmark should not be already initialized in the bookmark table."
            );
            token.BookmarkTable[CompensationBookmarkName.OnSecondaryRootScheduled] =
                context.CreateBookmark(new BookmarkCallback(OnSecondaryRootScheduled));
        ...
        )
        {
            ...
            {
                Fx.Assert(
                    compensationToken.BookmarkTable[CompensationBookmarkName.Canceled] == null,
                    "Bookmark should not be already initialized in the bookmark table."
                );
                compensationToken.BookmarkTable[CompensationBookmarkName.Canceled] =
                ...
                compensationExtension.NotifyMessage(
                    ...
                    CompensationBookmarkName.OnCancellation
                );
            }
            else if (compensationToken.CompensationState == CompensationState.Compensating)
            {
                Fx.Assert(
                    compensationToken.BookmarkTable[CompensationBookmarkName.Compensated] == null,
                    "Bookmark should not be already initialized in the bookmark table."
                );
                compensationToken.BookmarkTable[CompensationBookmarkName.Compensated] =
                    context.CreateBookmark(new BookmarkCallback(OnCanceledOrCompensated));

                compensationExtension.NotifyMessage(
                    context,
                    compensationToken.CompensationId,
                    CompensationBookmarkName.OnCompensation
                );
            }
        }
        ...
        )
        {
            ...
            {
                CompensationTokenData parentToken = compensationExtension.Get(
                    compensationToken.ParentCompensationId
                );
                Fx.Assert(parentToken != null, "parentToken must be valid");
    ...
    class WhileExpression : CodeActivity<bool>
    {
        ...
        {
            RuntimeArgument assertFlagArgument = new RuntimeArgument(
                "AssertFlag",
                typeof(bool),
            ...
            RuntimeArgument elementsArgument = new RuntimeArgument(
                "Elements",
        ...
        protected override bool Execute(CodeActivityContext context)
        {
            // While(env => (assertFlag.Get(env) != false) && index.Get(env) < elements.Get(env).Count())
            return (
                (this.AssertFlag.Get(context) != false)
                && (this.Index.Get(context) < this.Elements.Get(context).Count())
            );
        }
    }
