...
namespace Microsoft.VisualBasic.Activities
{
            ...
            ITextExpression
    {
        ...
        {
            get { return VisualBasicHelper.Language; }
        }
        ...
        protected override Location<TResult> Execute(CodeActivityContext context)
        {
            if (!this.invoker.IsStaticallyCompiled)
            {
                if (this.expressionTree != null)
                {
                    ...
                    {
                        this.locationFactory = ExpressionUtilities.CreateLocationFactory<TResult>(
                            this.expressionTree
                        );
                    }
        ...
        protected override void CacheMetadata(CodeActivityMetadata metadata)
        {
            ...
            CodeActivityPublicEnvironmentAccessor publicAccessor =
                CodeActivityPublicEnvironmentAccessor.Create(metadata);
            this.expressionTree = this.CompileLocationExpression(
                publicAccessor,
                out validationError
            );
        ...
        public Expression GetExpressionTree()
        {
            if (this.IsMetadataCached)
            {
                if (this.expressionTree == null)
                {
                    ...
                    CodeActivityPublicEnvironmentAccessor publicAccessor =
                        CodeActivityPublicEnvironmentAccessor.CreateWithoutArgument(metadata);
                    try
                    {
                        this.expressionTree = this.CompileLocationExpression(
                            publicAccessor,
                ...
                Fx.Assert(
                    this.expressionTree.NodeType == ExpressionType.Lambda,
                    "Lambda expression required"
                );
                return ExpressionUtilities.RewriteNonCompiledExpressionTree(
                    (LambdaExpression)this.expressionTree
                );
            }
            else
            {
        ...
        )
        {
            ...
            try
            {
                expressionTreeToReturn = VisualBasicHelper.Compile<TResult>(
                    this.ExpressionText,
                    publicAccessor,
                    true
                );
                ...
                if (
                    !publicAccessor.ActivityMetadata.HasViolations
                    && (
                        expressionTreeToReturn == null
                        || !ExpressionUtilities.IsLocation(
                            expressionTreeToReturn,
                            typeof(TResult),
                ...
                )
                {
                    ...
                    expressionTreeToReturn = null;
                    validationError = SR.CompilerErrorSpecificExpression(
                        this.ExpressionText,
                        errorMessage
                    );
                }
