    ...
    public class StringLengthAttribute : ValidationAttribute
    {
        ...
        public override string FormatErrorMessage(string name)
        {
            ...
            bool useErrorMessageWithMinimum =
                this.MinimumLength != 0 && !this.CustomErrorMessageSet;

            string errorMessage = useErrorMessageWithMinimum
                ? DataAnnotationsResources.StringLengthAttribute_ValidationErrorIncludingMinimum
        ...
        private void EnsureLegalLengths()
        {
            if (this.MaximumLength < 0)
            {
                throw new InvalidOperationException(
                    DataAnnotationsResources.StringLengthAttribute_InvalidMaxLength
                );
            }
            ...
            if (this.MaximumLength < this.MinimumLength)
            {
                throw new InvalidOperationException(
                    String.Format(
                        CultureInfo.CurrentCulture,
                        DataAnnotationsResources.RangeAttribute_MinGreaterThanMax,
                        this.MaximumLength,
                        this.MinimumLength
                    )
                );
