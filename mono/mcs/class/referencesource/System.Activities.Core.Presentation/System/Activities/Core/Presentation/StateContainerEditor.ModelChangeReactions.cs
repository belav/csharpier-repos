    ...
    partial class StateContainerEditor
    {
        ...
        void OnEditingScopeCompleted(object sender, EditingScopeEventArgs e)
        {
            ...
            foreach (Change change in e.EditingScope.Changes)
            {
                if (change is PropertyChange)
                {
                    ...
                    if (
                        propertyChange.Owner.ItemType == typeof(Transition)
                    ...
                    )
                    {
                        ...
                        {
                            Connector connector = this.GetConnectorInStateMachine(
        ...
        void OnViewStateChanged(object sender, ViewStateChangedEventArgs e)
        {
            ...
            if (!this.internalViewStateChange)
            {
                ...
                if (
                    (
                        e.ParentModelItem.ItemType == typeof(State)
                        || (
                            e.ParentModelItem.ItemType == typeof(StateMachine)
                    ...
                    if (this.modelItemToUIElement.ContainsKey(modelItem))
                    {
                        if (e.NewValue != null)
                        {
                            FreeFormPanel.SetLocation(
                                this.modelItemToUIElement[modelItem],
                                (Point)e.NewValue
                            );
                ...
                // only added to the outmost editor
                    ...
                    e.Key.Equals(ConnectorLocationViewStateKey)
                    && !this.GetStateMachineContainerEditor().internalViewStateChange
                )
                {
                    ...
                    if (changedConnector != null)
                    {
                        ...
                        {
                            Fx.Assert(
                                e.NewValue is PointCollection,
                                "e.NewValue is not PointCollection"
                            );
