...
namespace System.Activities.Presentation
{
    ...
    sealed class TypeCollectionPropertyEditor : DialogPropertyValueEditor
    {
        ...
        public override void ShowDialog(PropertyValue propertyValue, IInputElement commandSource)
        {
            ...
            ModelProperty editingProperty = activityModelItem.Properties[
                propertyValue.ParentProperty.PropertyName
            ];
            bool allowDuplication = GetOptionValueOrUseDefault(
                editingProperty.Attributes,
                TypeCollectionPropertyEditor.AllowDuplicate,
                true
            );
            Func<Type, bool> filter = GetOptionValueOrUseDefault<Func<Type, bool>>(
                editingProperty.Attributes,
                TypeCollectionPropertyEditor.Filter,
                null
            );
            Type defaultType = GetOptionValueOrUseDefault<Type>(
                editingProperty.Attributes,
                TypeCollectionPropertyEditor.DefaultType,
                typeof(Object)
            );
            ...
            {
                using (
                    var commitEditingScope = inputData.BeginEdit(
                        System.Activities.Core.Presentation.SR.ChangeTypeCollectionEditingScopeDesc
                    )
                )
                {
                    inputData.Clear();
