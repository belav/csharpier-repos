...
namespace System.Reflection.Emit
{
    ...
    public sealed class MethodRental : _MethodRental
    {
        ...
        ) // [in] flags
        {
            if (methodSize <= 0 || methodSize >= 0x3f0000)
                throw new ArgumentException(
                    Environment.GetResourceString("Argument_BadSizeForData"),
                    "methodSize"
                );
                ...
                if (!typeBuilder.IsCreated())
                    throw new NotSupportedException(
                        Environment.GetResourceString(
                            "NotSupported_NotAllTypesAreBaked",
                            typeBuilder.Name
                        )
                    );
            ...
            {
                SwapMethodBody(
                    rType.GetTypeHandleInternal(),
                    methodtoken,
                    rgIL,
                    methodSize,
                    flags,
                    JitHelpers.GetStackCrawlMarkHandle(ref mark)
                );
            }
        }
        ...
        ); // [in] stack crawl mark used to find caller
