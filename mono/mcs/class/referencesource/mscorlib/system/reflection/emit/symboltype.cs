...
#endif
    {
        ...
        internal static Type FormCompoundType(char[] bFormat, Type baseType, int curIndex)
        {
            ...
            if (bFormat[curIndex] == '&')
            {
                ...
                if (curIndex != bFormat.Length)
                    ...
                    throw new ArgumentException(
                        Environment.GetResourceString("Argument_BadSigFormat")
                    );
            ...
            if (bFormat[curIndex] == '[')
            {
                ...
                while (bFormat[curIndex] != ']')
                {
                    ...
                    if (
                        (bFormat[curIndex] >= '0' && bFormat[curIndex] <= '9')
                        || bFormat[curIndex] == '-'
                    )
                    {
        ...
        public override Type MakePointerType()
        {
            return SymbolType.FormCompoundType(
                (new String(m_bFormat) + "*").ToCharArray(),
                m_baseType,
                0
            );
        }
        ...
        public override Type MakeArrayType(int rank)
        {
            ...
            SymbolType st =
                SymbolType.FormCompoundType(
                    (new String(m_bFormat) + s).ToCharArray(),
                    m_baseType,
                    0
                ) as SymbolType;
            return st;
