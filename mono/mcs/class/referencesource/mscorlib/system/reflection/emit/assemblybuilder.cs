...
namespace System.Reflection.Emit
{
    ...
    internal sealed class InternalAssemblyBuilder : RuntimeAssembly
    {
        ...
        public override String[] GetManifestResourceNames()
        {
            throw new NotSupportedException(
                Environment.GetResourceString("NotSupported_DynamicAssembly")
            );
        }
    ...
    public sealed class AssemblyBuilder : Assembly, _AssemblyBuilder
    {
        ...
        private ModuleBuilder GetOnDiskAssemblyModuleBuilder()
        {
            ...
            {
                Module module = AssemblyBuilder.GetOnDiskAssemblyModule(
                    InternalAssembly.GetNativeHandle()
                );
        ...
        internal ModuleBuilder GetModuleBuilder(InternalModuleBuilder module)
        {
            ...
            lock (SyncRoot)
            {
                ...
                if (
                    m_onDiskAssemblyModuleBuilder != null
                    && m_onDiskAssemblyModuleBuilder.InternalModule == module
                )
                    return m_onDiskAssemblyModuleBuilder;
#endif // !FEATURE_CORECLR
        ...
        {
            get { return InternalAssembly.SyncRoot; }
        }

        internal InternalAssemblyBuilder InternalAssembly
        {
            get { return m_internalAssemblyBuilder; }
        }
        ...
        )
        {
            ...
            )
            {
                throw new ArgumentException(
                    Environment.GetResourceString("Arg_EnumIllegalVal", (int)access),
                    "access"
                );
            }
...
#pragma warning disable 618 // We deal with legacy attributes here as well for compat
                foreach (CustomAttributeBuilder attribute in assemblyAttributes)
                {
                    ...
                    else if (attribute.m_con.DeclaringType == typeof(SecurityCriticalAttribute))
                    {
                        ...
                        if (
                            attribute.m_constructorArgs != null
...
#pragma warning restore 618
            ...
            m_internalAssemblyBuilder = (InternalAssemblyBuilder)nCreateDynamicAssembly(
                domain,
                name,
                evidence,
                ref stackMark,
            ...
            m_assemblyData = new AssemblyBuilderData(
                m_internalAssemblyBuilder,
                name.Name,
                access,
                dir
            );
            m_assemblyData.AddPermissionRequests(
                requiredPermissions,
                optionalPermissions,
                refusedPermissions
            );
        ...
        **********************************************/
        ...
        [ResourceExposure(ResourceScope.None)]
        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        private static extern Assembly nCreateDynamicAssembly(
            AppDomain domain,
            AssemblyName name,
            Evidence identity,
            ref StackCrawlMark stackMark,
        ...
        **********************************************/
        ...
        [System.Security.SecuritySafeCritical] // auto-generated
        [MethodImplAttribute(MethodImplOptions.NoInlining)] // Methods containing StackCrawlMark local var has to be marked non-inlineable
        public ModuleBuilder DefineDynamicModule(String name, bool emitSymbolInfo) // specify if emit symbol info or not
        {
            Contract.Ensures(Contract.Result<ModuleBuilder>() != null);
        ...
        )
        {
            ...
            BCLDebug.Log(
                "DYNIL",
                "## DYNIL LOGGING: AssemblyBuilder.DefineDynamicModule( " + name + " )"
            );
...
#error FEATURE_MULTIMODULE_ASSEMBLIES should always go with !FEATURE_CORECLR
            ...
            { // We need to define a new module
                int tkFile;
                InternalModuleBuilder internalDynModule =
                    (InternalModuleBuilder)DefineDynamicModule(
                        InternalAssembly,
                        emitSymbolInfo,
                        name,
                        name,
            ...
            { // We will reuse the manifest module
                ...
                if (emitSymbolInfo)
                {
                    pInternalSymWriter = ModuleBuilder.nCreateISymWriterForDynamicModule(
                        dynModule.InternalModule,
                        name
                    );
                }
            }
            ...
            if (m_fManifestModuleUsedAsDefinedModule == true)
                throw new InvalidOperationException(
                    Environment.GetResourceString("InvalidOperation_NoMultiModuleAssembly")
                );
            ...
            if (emitSymbolInfo)
            {
                ...
                if (symWriter == null)
                {
                    ...
                    throw new TypeLoadException(
                        Environment.GetResourceString(ResId.MissingType, "SymWriter")
                    );
                }
...
#pragma warning restore 618
        ...
        **********************************************/
        ...
        public ModuleBuilder DefineDynamicModule(
            ...
            bool emitSymbolInfo
        ) // specify if emit symbol info or not
        {
            Contract.Ensures(Contract.Result<ModuleBuilder>() != null);
        ...
        private ModuleBuilder DefineDynamicModuleInternal(
            ...
            bool emitSymbolInfo, // specify if emit symbol info or not
            ref StackCrawlMark stackMark
        ) // stack crawl mark used to find caller
        {
            lock (SyncRoot)
            {
                return DefineDynamicModuleInternalNoLock(
                    name,
                    fileName,
                    emitSymbolInfo,
                    ref stackMark
        ...
        ) // stack crawl mark used to find caller
        {
            ...
            if (!String.Equals(fileName, Path.GetFileName(fileName)))
                throw new ArgumentException(
                    Environment.GetResourceString("Argument_NotSimpleFileName"),
                    "fileName"
                );
            Contract.Ensures(Contract.Result<ModuleBuilder>() != null);
            Contract.EndContractBlock();
            ...
            BCLDebug.Log(
                "DYNIL",
                "## DYNIL LOGGING: AssemblyBuilder.DefineDynamicModule( "
                    + name
                    + ", "
                    + fileName
                    + ", "
            ...
            if (m_assemblyData.m_isSaved == true)
            {
                ...
                throw new InvalidOperationException(
                    Environment.GetResourceString("InvalidOperation_CannotAlterAssembly")
                );
            }
            ...
            if (emitSymbolInfo)
            {
                ...
                if (symWriter == null)
                {
                    // cannot find SymWriter - throw TypeLoadException since we couldnt find the type.
                    throw new TypeLoadException(
                        Environment.GetResourceString("MissingType", "SymWriter")
                    );
                }
                try
                {
        ...
        internal void CheckContext(params Type[] types)
        {
            ...
            foreach (Type type in types)
            {
                ...
                if (type.Module == null || type.Module.Assembly == null)
                    throw new ArgumentException(
                        Environment.GetResourceString("Argument_TypeNotValid")
                    );
        ...
        **********************************************/
        ...
        )
        {
            ...
            BCLDebug.Log(
                "DYNIL",
                "## DYNIL LOGGING: AssemblyBuilder.DefineResource( " + name + ", " + fileName + ")"
            );
            ...
            m_assemblyData.AddResWriter(
                new ResWriterData(resWriter, null, name, fileName, fullFileName, attribute)
            );
            return resWriter;
        }
        ...
        **********************************************/
        ...
        )
        {
            ...
            BCLDebug.Log(
                "DYNIL",
                "## DYNIL LOGGING: AssemblyBuilder.AddResourceFile( " + name + ", " + fileName + ")"
            );
            ...
            if (File.UnsafeExists(fullFileName) == false)
                throw new FileNotFoundException(
                    Environment.GetResourceString("IO.FileNotFound_FileName", fileName),
                    fileName
                );
            m_assemblyData.AddResWriter(
                new ResWriterData(null, null, name, fileName, fullFileName, attribute)
            );
        }
        ...
        public override String Location
        {
            ...
            [System.Security.SecurityCritical] // auto-generated
#endif
            get { return InternalAssembly.Location; }
        }

        public override String ImageRuntimeVersion
        {
            get { return InternalAssembly.ImageRuntimeVersion; }
        }
        ...
        public override String CodeBase
        {
            ...
            [System.Security.SecurityCritical] // auto-generated
#endif
            get { return InternalAssembly.CodeBase; }
        }
        ...
        {
            get { return InternalAssembly.FullName; }
        }
        ...
        {
            get { return InternalAssembly.Evidence; }
        }

        public override PermissionSet PermissionSet
        {
            [SecurityCritical]
            get { return InternalAssembly.PermissionSet; }
        }

        public override SecurityRuleSet SecurityRuleSet
        {
            get { return InternalAssembly.SecurityRuleSet; }
        }
#endif // FEATURE_CAS_POLICY

        public override Module ManifestModule
        {
            get { return m_manifestModuleBuilder.InternalModule; }
        }

        public override bool ReflectionOnly
        {
            get { return InternalAssembly.ReflectionOnly; }
        }
        ...
        {
            get { return InternalAssembly.GlobalAssemblyCache; }
        }

        public override Int64 HostContext
        {
            get { return InternalAssembly.HostContext; }
        }
        ...
        **********************************************/
        ...
        {
            if (
                m_assemblyData.m_strResourceFileName != null
                || m_assemblyData.m_resourceBytes != null
                || m_assemblyData.m_nativeVersion != null
            )
                throw new ArgumentException(
                    Environment.GetResourceString("Argument_NativeResourceAlreadyDefined")
                );
        ...
        private void DefineUnmanagedResourceNoLock(String resourceFileName)
        {
            ...
            if (File.Exists(strFullFileName) == false)
                throw new FileNotFoundException(
                    Environment.GetResourceString("IO.FileNotFound_FileName", resourceFileName),
                    resourceFileName
                );
        ...
        *
        **********************************************/
        public ModuleBuilder GetDynamicModule(String name) // the name of module for the look up
        {
            lock (SyncRoot)
            {
        ...
        private ModuleBuilder GetDynamicModuleNoLock(String name) // the name of module for the look up
        {
            if (name == null)
                throw new ArgumentNullException("name");
            if (name.Length == 0)
            ...
            BCLDebug.Log(
                "DYNIL",
                "## DYNIL LOGGING: AssemblyBuilder.GetDynamicModule( " + name + " )"
            );
        ...
        **********************************************/
        ...
        ) // file kind for the assembly.
        {
            ...
            if (tmpModule == null || !InternalAssembly.Equals(tmpModule.Assembly))
                throw new InvalidOperationException(
                    Environment.GetResourceString(
                        "InvalidOperation_EntryMethodNotDefinedInAssembly"
                    )
                );
            ...
            if (tmpMB != null)
                ...
                m_assemblyData.m_entryPointModule = GetModuleBuilder(
                    (InternalModuleBuilder)tmpModule
                );

            MethodToken entryMethodToken = m_assemblyData.m_entryPointModule.GetMethodToken(
                entryMethod
            );
        ...
        **********************************************/
        ...
        private void SetCustomAttributeNoLock(CustomAttributeBuilder customBuilder)
        {
            customBuilder.CreateCustomAttribute(
                ...
                AssemblyBuilderData.m_tkAssembly
            ); // This is the AssemblyDef token
        ...
        **********************************************/
        ...
        )
        {
            ...
            throw new InvalidOperationException(
                Environment.GetResourceString("InvalidOperation_CantSaveTransientAssembly")
            );
        }
        ...
        )
        {
            ...
            BCLDebug.Log(
                "DYNIL",
                "## DYNIL LOGGING: AssemblyBuilder.Save( " + assemblyFileName + " )"
            );
            ...
            try
            {
                ...
                if (
                    (m_assemblyData.m_access & AssemblyBuilderAccess.Save)
                ...
                if (assemblyModule != null)
                {
                    ...
                    {
                        tkAttrs2[i] = assemblyModule
                            .InternalGetConstructorToken(m_assemblyData.m_CACons[i], true)
                            .Token;
                    }
                    assemblyModule.PreSave(
                        assemblyFileName,
                ...
                RuntimeModule runtimeAssemblyModule =
                    (assemblyModule != null)
                        ? assemblyModule.ModuleHandle.GetRuntimeModule()
                        : null;
                ...
                if (assemblyModule == null)
                {
                    ...
                    for (i = 0; i < m_assemblyData.m_iCAs; i++)
                    {
                        tkAttrs2[i] = onDiskAssemblyModule
                            .InternalGetConstructorToken(m_assemblyData.m_CACons[i], true)
                            .Token;
                    }
                }
                ...
                for (i = 0; i < size; i++)
                {
                    ...
                    if (mBuilder.IsTransient() == false && mBuilder != assemblyModule)
                    {
                        ...
                        mBuilder.m_moduleData.FileToken = AddFile(
                            GetNativeHandle(),
                            mBuilder.m_moduleData.m_strFileName
                        );
                        ...
                        SetFileHashValue(
                            GetNativeHandle(),
                            mBuilder.m_moduleData.FileToken,
                            strModFileName
                        );
                    }
                }
                ...
                for (i = 0; i < m_assemblyData.m_iPublicComTypeCount; i++)
                {
                    ...
                    if (type is RuntimeType)
                    {
                        ...
                        if (modBuilder != assemblyModule)
                            DefineNestedComType(
                                type,
                                modBuilder.m_moduleData.FileToken,
                                type.MetadataToken
                            );
                    }
                    else
                    {
                        ...
                        if (modBuilder != assemblyModule)
                            DefineNestedComType(
                                type,
                                modBuilder.m_moduleData.FileToken,
                                typeBuilder.MetadataTokenInternal
                            );
                    }
                }
...
#pragma warning disable 618
                if (m_assemblyData.m_RequiredPset != null)
                    AddDeclarativeSecurity(
                        m_assemblyData.m_RequiredPset,
                        SecurityAction.RequestMinimum
                    );

                if (m_assemblyData.m_RefusedPset != null)
                    AddDeclarativeSecurity(
                        m_assemblyData.m_RefusedPset,
                        SecurityAction.RequestRefuse
                    );

                if (m_assemblyData.m_OptionalPset != null)
                    AddDeclarativeSecurity(
                        m_assemblyData.m_OptionalPset,
...
#pragma warning restore 618
                ...
                for (i = 0; i < size; i++)
                {
                    ...
                    AddStandAloneResource(
                        GetNativeHandle(),
                        tempRes.m_strName,
                        tempRes.m_strFileName,
                        tempRes.m_strFullFileName,
                        (int)tempRes.m_attribute
                    );
                }
                ...
                if (assemblyModule == null)
                {
                    onDiskAssemblyModule.DefineNativeResource(
                        portableExecutableKind,
                        imageFileMachine
                    );
                    ...
                    int entryPoint =
                        (m_assemblyData.m_entryPointModule != null)
                ...
                else
                {
                    ...
                    if (
                        m_assemblyData.m_entryPointModule != null
                        && m_assemblyData.m_entryPointModule != assemblyModule
                    )
                        assemblyModule.SetEntryPoint(
                            new MethodToken(
                                m_assemblyData.m_entryPointModule.m_moduleData.FileToken
                            )
                        );
        ...
        internal bool IsPersistable()
        {
            ...
            if (
                (m_assemblyData.m_access & AssemblyBuilderAccess.Save) == AssemblyBuilderAccess.Save
            )
            {
                return true;
            }
        ...
        **********************************************/
        ...
        internal int DefineExportedTypeInMemory(Type type, int tkResolutionScope, int tkTypeDef)
        {
            ...
            {
                // Use full type name for non-nested types.
                return AddExportedTypeInMemory(
                    GetNativeHandle(),
                    type.FullName,
                    tkResolutionScope,
                    tkTypeDef,
            ...
            tkResolutionScope = DefineExportedTypeInMemory(
                enclosingType,
                tkResolutionScope,
                tkTypeDef
            );
        ...
        [System.Security.SecurityCritical] // auto-generated
        private static Module DefineDynamicModule(
            RuntimeAssembly containingAssembly,
            bool emitSymbolInfo,
            String name,
            String filename,
        ...
        )
        {
            ...
            DefineDynamicModule(
                containingAssembly.GetNativeHandle(),
                emitSymbolInfo,
                name,
                filename,
