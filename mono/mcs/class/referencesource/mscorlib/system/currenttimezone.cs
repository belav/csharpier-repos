...
namespace System
{
    ...
    internal class CurrentSystemTimeZone : TimeZone
    {
        ...
        public override DaylightTime GetDaylightChanges(int year)
        {
            if (year < 1 || year > 9999)
            {
                throw new ArgumentOutOfRangeException(
                    "year",
                    Environment.GetResourceString("ArgumentOutOfRange_Range", 1, 9999)
                );
            }
                ...
                lock (InternalSyncObject)
                {
                    if (!m_CachedDaylightChanges.Contains(objYear))
                    {
                        ...
                        else
                        {
                            ...
                            DaylightTime currentDaylightChanges = new DaylightTime(
                                start,
                                end,
                                delta
                            );
        ...
        public override TimeSpan GetUtcOffset(DateTime time)
        {
            ...
            {
                return new TimeSpan(
                    TimeZone.CalculateUtcOffset(time, GetDaylightChanges(time.Year)).Ticks
                        + m_ticksOffset
                );
            }
        }
        ...
        //
        private static DateTime GetDayOfWeek(
            int year,
            bool fixedDate,
            int month,
            int targetDayOfWeek,
        ...
        )
        {
            ...
            if (fixedDate)
            {
                ...
                time = new DateTime(
                    year,
                    month,
                    (day < numberOfSunday) ? day : numberOfSunday,
                    hour,
                    minute,
                    second,
                    millisecond,
