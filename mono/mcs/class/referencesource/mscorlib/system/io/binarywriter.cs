...
#pragma warning restore 169
        ...
        public virtual unsafe void Write(char ch)
        {
            if (Char.IsSurrogate(ch))
                throw new ArgumentException(
                    Environment.GetResourceString("Arg_SurrogatesNotAllowedAsSingleChar")
                );
            Contract.EndContractBlock();
                ...
                while (numLeft > 0)
                {
                    ...
                    checked
                    {
                        ...
                        fixed (char* pChars = value)
                        {
                            ...
                            {
                                byteLen = _encoder.GetBytes(
                                    pChars + charStart,
                                    charCount,
                                    pBytes,
                                    _largeByteBuffer.Length,
                                    charCount == numLeft
                                );
                            }
                        }
                    ...
                    totalBytes += byteLen;
                    Contract.Assert(
                        totalBytes <= len && byteLen <= _largeByteBuffer.Length,
