...
namespace System.IO
{
    ...
    public abstract class TextReader : IDisposable
    {
...
#if FEATURE_ASYNC_IO
        [NonSerialized]
        private static Func<object, string> _ReadLineDelegate = state =>
            ((TextReader)state).ReadLine();
        ...
        public virtual async Task<String> ReadToEndAsync()
        {
            ...
            StringBuilder sb = new StringBuilder(4096);
            while (
                (len = await ReadAsyncInternal(chars, 0, chars.Length).ConfigureAwait(false)) != 0
            )
            {
                sb.Append(chars, 0, len);
            }
        ...
        internal virtual Task<int> ReadAsyncInternal(char[] buffer, int index, int count)
        {
            ...
            Tuple<TextReader, char[], int, int> tuple = new Tuple<TextReader, char[], int, int>(
                this,
                buffer,
                index,
                count
