...
namespace System.IO
{
    ...
    public sealed class BufferedStream : Stream
    {
        ...
        public BufferedStream(Stream stream, Int32 bufferSize)
        {
            ...
            if (bufferSize <= 0)
                throw new ArgumentOutOfRangeException(
                    "bufferSize",
                    Environment.GetResourceString("ArgumentOutOfRange_MustBePositive", "bufferSize")
                );
            ...
            BCLDebug.Perf(
                ...
                "FileStream is buffered - don't wrap it in a BufferedStream"
            );
            BCLDebug.Perf(
                !(stream is MemoryStream),
                "MemoryStream shouldn't be wrapped in a BufferedStream!"
            );
            BCLDebug.Perf(
                !(stream is BufferedStream),
                "BufferedStream shouldn't be wrapped in another BufferedStream!"
            );
        ...
        private async Task FlushWriteAsync(CancellationToken cancellationToken)
        {
            ...
            await _stream
                .WriteAsync(_buffer, 0, _writePos, cancellationToken)
        ...
        )
        {
            ...
            return BeginReadFromUnderlyingStream(
                buffer,
                offset + bytesFromBuffer,
                count - bytesFromBuffer,
                callback,
        ...
        private IAsyncResult BeginReadFromUnderlyingStream(
            Byte[] buffer,
            Int32 offset,
            Int32 count,
            AsyncCallback callback,
        ...
        {
            Task<Int32> readOp = ReadFromUnderlyingStreamAsync(
                buffer,
                offset,
                count,
                ...
                if (count >= _bufferSize)
                {
                    if (useApmPattern)
                    {
                        ...
                        _stream.BeginRead(
                            ...
                            offset,
                            count,
                            BeginEndAwaitableAdapter.Callback,
                            _beginEndAwaitable
                        );
                    ...
                    else
                    {
                        return bytesAlreadySatisfied
                            + await _stream
                                .ReadAsync(array, offset, count, cancellationToken)
                                .ConfigureAwait(false);
                    }
                }
        ...
        )
        {
            ...
            // Delegate to the async implementation.
            return BeginWriteToUnderlyingStream(
                buffer,
                offset,
                count,
                callback,
        ...
        private IAsyncResult BeginWriteToUnderlyingStream(
            Byte[] buffer,
            Int32 offset,
            Int32 count,
            AsyncCallback callback,
        ...
        {
            Task writeOp = WriteToUnderlyingStreamAsync(
                buffer,
                offset,
                count,
                CancellationToken.None,
        ...
        )
        {
            ...
            // Delegate to the async implementation.
            return WriteToUnderlyingStreamAsync(
                buffer,
                offset,
                count,
                cancellationToken,
                ...
                { // if (!useBuffer)
                    ...
                    if (_writePos > 0)
                    {
                        ...
                        )
                        {
                            ...
                            {
                                await _stream
                                    .WriteAsync(_buffer, 0, totalUserBytes, cancellationToken)
                                    .ConfigureAwait(false);
                            }
                            _writePos = 0;
                            return;
                    ...
                    {
                        await _stream
                            .WriteAsync(array, offset, count, cancellationToken)
                            .ConfigureAwait(false);
                    }
                }
            }
        ...
        public override void SetLength(Int64 value)
        {
            if (value < 0)
                throw new ArgumentOutOfRangeException(
                    "value",
                    Environment.GetResourceString("ArgumentOutOfRange_NegFileSize")
                );
            Contract.EndContractBlock();
