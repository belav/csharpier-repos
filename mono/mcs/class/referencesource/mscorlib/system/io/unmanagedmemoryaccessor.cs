...
namespace System.IO
{
    ...
    public class UnmanagedMemoryAccessor : IDisposable
    {
        ...
        )
        {
            ...
            if (buffer.ByteLength < (UInt64)(offset + capacity))
            {
                throw new ArgumentException(
                    Environment.GetResourceString("Argument_OffsetAndCapacityOutOfBounds")
                );
            }
            ...
            unsafe
            {
                ...
                try
                {
                    ...
                    {
                        throw new ArgumentException(
                            Environment.GetResourceString("Argument_UnmanagedMemAccessorWrapAround")
                        );
                    }
                }
                finally
        ...
        public bool CanRead
        {
            get { return _isOpen && _canRead; }
        }

        public bool CanWrite
        {
            get { return _isOpen && _canWrite; }
        }
        ...
        public void Read<T>(Int64 position, out T structure)
            where T : struct
        {
            if (position < 0)
            {
            ...
            if (!_isOpen)
            {
                throw new ObjectDisposedException(
                    ...
                    Environment.GetResourceString("ObjectDisposed_ViewAccessorClosed")
                );
            }
            if (!CanRead)
            {
                throw new NotSupportedException(
                    Environment.GetResourceString("NotSupported_Reading")
                );
            }
        ...
        [System.Security.SecurityCritical] // auto-generated_required
        public int ReadArray<T>(Int64 position, T[] array, Int32 offset, Int32 count)
            where T : struct
        {
            if (array == null)
            {
            ...
            {
                throw new ArgumentException(
                    Environment.GetResourceString("Argument_OffsetAndLengthOutOfBounds")
                );
            }
            Contract.EndContractBlock();
            if (!CanRead)
        ...
        public void Write(Int64 position, Decimal value)
        {
            ...
            int flags =
                ((int)decimalArray[12])
                | ((int)decimalArray[13] << 8)
                | ((int)decimalArray[14] << 16)
                | ((int)decimalArray[15] << 24);
            int lo =
                ((int)decimalArray[0])
                | ((int)decimalArray[1] << 8)
                | ((int)decimalArray[2] << 16)
                | ((int)decimalArray[3] << 24);
            int mid =
                ((int)decimalArray[4])
                | ((int)decimalArray[5] << 8)
                | ((int)decimalArray[6] << 16)
                | ((int)decimalArray[7] << 24);
            int hi =
                ((int)decimalArray[8])
                | ((int)decimalArray[9] << 8)
                | ((int)decimalArray[10] << 16)
                | ((int)decimalArray[11] << 24);
            bits[0] = lo;
            bits[1] = mid;
            bits[2] = hi;
            bits[3] = flags;
        ...
        [System.Security.SecurityCritical] // auto-generated_required
        public void Write<T>(Int64 position, ref T structure)
            where T : struct
        {
            if (position < 0)
            {
        ...
        private byte InternalReadByte(Int64 position)
        {
            ...
            Contract.Assert(
                position <= _capacity - sizeof(byte),
        ...
        private void EnsureSafeToRead(Int64 position, int sizeOfType)
        {
            ...
            if (position > _capacity - sizeOfType)
            {
                ...
                {
                    throw new ArgumentException(
                        Environment.GetResourceString("Argument_NotEnoughBytesToRead"),
                        "position"
                    );
                }
            }
        ...
        private void EnsureSafeToWrite(Int64 position, int sizeOfType)
        {
            ...
            if (position > _capacity - sizeOfType)
            {
                ...
                {
                    throw new ArgumentException(
                        Environment.GetResourceString("Argument_NotEnoughBytesToWrite", "Byte"),
                        "position"
                    );
                }
            }
