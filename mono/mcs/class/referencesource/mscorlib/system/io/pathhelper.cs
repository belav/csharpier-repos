...
namespace System.IO
{
    ...
    //
    internal unsafe struct PathHelper
    { // should not be serialized
        ...
        internal unsafe int GetFullPathName()
        {
            if (useStackAlloc)
            {
                ...
                int result = Win32Native.GetFullPathName(
                    m_arrayPtr,
                    Path.MaxPath + 1,
                    finalBuffer,
                    IntPtr.Zero
                ...
                if (result > Path.MaxPath)
                {
                    ...
                    result = Win32Native.GetFullPathName(
                        m_arrayPtr,
                        result,
                        finalBuffer,
                        IntPtr.Zero
        ...
        internal unsafe bool TryExpandShortFileName()
        {
            ...
            if (useStackAlloc)
            {
                ...
                if (r == 0)
                {
                    ...
                    if (
                        lastErr == Win32Native.ERROR_FILE_NOT_FOUND
                        || lastErr == Win32Native.ERROR_PATH_NOT_FOUND
                    )
            ...
            else
            {
                ...
                if (r == 0)
                {
                    ...
                    int lastErr = Marshal.GetLastWin32Error();
                    if (
                        Win32Native.ERROR_FILE_NOT_FOUND == lastErr
                        || Win32Native.ERROR_PATH_NOT_FOUND == lastErr
                    )
                        doNotTryExpandShortFileName = true;
            ...
            else
            {
                if (ignoreCase)
                {
                    return m_sb.ToString()
                        .StartsWith(compareTo, StringComparison.OrdinalIgnoreCase);
                }
                else
                {
