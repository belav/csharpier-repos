...
namespace System.IO
{
    ...
    internal sealed unsafe class FileStreamAsyncResult : IAsyncResult
    {
        // README:
        ...
        {
            get { return _numBufferedBytes; }
        }

        internal int NumBytesRead
        {
            get { return _numBytes + _numBufferedBytes; }
        }
        ...
        internal static FileStreamAsyncResult CreateBufferedReadResult(
            int numBufferedBytes,
            AsyncCallback userCallback,
            Object userStateObject,
            bool isWrite
        ...
        public WaitHandle AsyncWaitHandle
        {
            ...
            get
            {
                ...
                if (_waitHandle == null)
                {
                    ...
                    if (
                        Interlocked.CompareExchange<ManualResetEvent>(ref _waitHandle, mre, null)
                        == null
                    )
                    {
                        if (_isComplete)
        ...
        internal void CallUserCallback()
        {
            ...
            if (_userCallback != null)
            {
                ...
                ThreadPool.QueueUserWorkItem(
                    state => ((FileStreamAsyncResult)state).CallUserCallbackWorker(),
                    this
                );
            }
            else
        ...
        )
        {
            BCLDebug.Log(
                String.Format(
                    "AsyncFSCallback called.  errorCode: " + errorCode + "  numBytes: " + numBytes
                )
            );
        ...
        )
        {
...
#if FEATURE_MACL
            if (
                (!useRights && (access & FileAccess.Write) == 0)
                    ...
                    if (!useRights)
                        throw new ArgumentException(
                            Environment.GetResourceString(
                                "Argument_InvalidFileMode&AccessCombo",
                                mode,
                                access
                            )
                        );
                        ...
                        throw new ArgumentException(
                            Environment.GetResourceString(
                                "Argument_InvalidFileMode&RightsCombo",
                                mode,
                                fileSystemRights
                            )
                        );
            ...
            if (useRights && (mode == FileMode.Truncate))
            {
                ...
                else
                {
                    throw new ArgumentException(
                        Environment.GetResourceString(
                            "Argument_InvalidFileModeTruncate&RightsCombo",
                            mode,
                            fileSystemRights
                        )
                    );
...
#if FEATURE_MACL
            if (
                (!useRights && (access & FileAccess.Read) != 0)
                || (useRights && (fileSystemRights & FileSystemRights.ReadAndExecute) != 0)
            )
#else
...
#endif //FEATURE_MACL
            {
                if (mode == FileMode.Append)
                    throw new ArgumentException(
                        Environment.GetResourceString("Argument_InvalidAppendMode")
                    );
                else
                    read = true;
            }
...
#endif // FEATURE_CAS_POLICY
            {
                ...
                if (
                    ...
                    || (
                        useRights
                        && ((fileSystemRights & FileSystemRights.Synchronize) != 0)
                ...
                AccessControlActions control = specifiedAcl
                ...
                if (checkHost)
                {
                    FileSecurityState state = new FileSecurityState(
                        FileSecurityState.ToFileSecurityState(secAccess),
                        path,
                        filePath
                    );
                    state.EnsureState();
            ...
            flagsAndAttributes |= (
                Win32Native.SECURITY_SQOS_PRESENT | Win32Native.SECURITY_ANONYMOUS
            );
#endif
            ...
            try
            {
                ...
                _handle = Win32Native.SafeCreateFile(
                    tempPath,
                    fAccess,
                    share,
                    secAttrs,
                ...
                if (_handle.IsInvalid)
                {
                    ...
                    int errorCode = Marshal.GetLastWin32Error();
                    if (
                        errorCode == __Error.ERROR_PATH_NOT_FOUND
            ...
            else
            {
                ...
                _canWrite =
                    ((fileSystemRights & FileSystemRights.WriteData) != 0)
...
#pragma warning restore 618
        public FileStream(SafeFileHandle handle, FileAccess access, int bufferSize, bool isAsync)
        {
            ...
            Contract.Assert(
                handleType == Win32Native.FILE_TYPE_DISK
                    ...
                    || handleType == Win32Native.FILE_TYPE_CHAR,
            ...
            if (_isAsync)
            {
                ...
                catch (ApplicationException)
                {
                    ...
                    throw new ArgumentException(
                        Environment.GetResourceString("Arg_HandleNotAsync")
                    );
                }
                if (!b)
                {
        ...
        private void SetLengthCore(long value)
        {
            ...
            if (!Win32Native.SetEndOfFile(_handle))
            {
                ...
                if (hr == __Error.ERROR_INVALID_PARAMETER)
                    throw new ArgumentOutOfRangeException(
                        "value",
                        Environment.GetResourceString("ArgumentOutOfRange_FileLengthTooBig")
                    );
                __Error.WinIOError(hr, String.Empty);
            }
        ...
        private unsafe int ReadCore(byte[] buffer, int offset, int count)
        {
            ...
            if (r == -1)
            {
                ...
                else
                {
                    if (hr == ERROR_INVALID_PARAMETER)
                        throw new ArgumentException(
                            Environment.GetResourceString("Arg_HandleNotSync")
                        );
        ...
        [System.Security.SecuritySafeCritical] // auto-generated
        [HostProtection(ExternalThreading = true)]
        private FileStreamAsyncResult BeginReadAsync(
            byte[] array,
            int offset,
            int numBytes,
            AsyncCallback userCallback,
        ...
        )
        {
            ...
            if (_isPipe)
            {
                ...
                if (_readPos < _readLen)
                {
                    ...
                    return FileStreamAsyncResult.CreateBufferedReadResult(
                        n,
                        userCallback,
                        stateObject,
                        false
        ...
        [System.Security.SecuritySafeCritical] // auto-generated
        [HostProtection(ExternalThreading = true)]
        private FileStreamAsyncResult BeginWriteAsync(
            byte[] array,
            int offset,
            int numBytes,
            AsyncCallback userCallback,
        ...
        )
        {
            ...
            if (endReadTask == null)
                s_endReadTask = endReadTask = EndReadTask; // benign initialization ----
            ...
            if (readTask._asyncResult.IsAsync && cancellationToken.CanBeCanceled)
            {
                ...
                if (cancelReadHandler == null)
                    s_cancelReadHandler = cancelReadHandler = CancelTask<int>; // benign initialization ----
        ...
        )
        {
            ...
            if (endWriteTask == null)
                s_endWriteTask = endWriteTask = EndWriteTask; // benign initialization ----
            writeTask._asyncResult = BeginWriteAsync(
                buffer,
                offset,
                count,
                endWriteTask,
            ...
            if (writeTask._asyncResult.IsAsync && cancellationToken.CanBeCanceled)
            {
                ...
                if (cancelWriteHandler == null)
                    s_cancelWriteHandler = cancelWriteHandler = CancelTask<VoidTaskResult>; // benign initialization ----
