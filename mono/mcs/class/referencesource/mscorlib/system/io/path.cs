...
namespace System.IO
{
    ...
    public static class Path
    {
        ...
        private static readonly char[] InvalidPathCharsWithAdditionalChecks =
        {
            '\"',
            '<',
            '>',
        ...
        private static string InternalGetDirectoryName(string path)
        {
            if (path != null)
            {
                ...
                string normalizedPath = NormalizePath(
                    path,
                    fullCheck: false,
                    expandShortPaths: AppContextSwitches.UseLegacyPathHandling
                );
                ...
                )
                {
                    try
                    {
                        ...
                        int pos = 0;
                        while (
                            pos < tempPath.Length && (tempPath[pos] != '?' && tempPath[pos] != '*')
                        )
                            pos++;
        ...
        [System.Security.SecuritySafeCritical]
        private static string NewNormalizePathLimitedChecks(
            string path,
            int maxPathLength,
            bool expandShortPaths
        )
        {
            ...
            if (
                PathInternal.IsPathTooLong(normalized)
                || PathInternal.AreSegmentsTooLong(normalized)
            )
                throw new PathTooLongException();
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        private static string NewNormalizePath(
            string path,
            int maxPathLength,
            bool expandShortPaths
        )
        {
            ...
            return LongPathHelper.Normalize(
                path,
                (uint)maxPathLength,
                checkInvalidCharacters: !PathInternal.IsDevice(path),
        ...
        )
        {
            ...
            while (index < path.Length)
            {
                ...
                if (
                    currentChar == DirectorySeparatorChar
                ...
                )
                {
                    ...
                    if (numSigChars == 0)
                    {
                        ...
                        if (numDots > 0)
                        {
                            ...
                            if (path[start] != '.')
                                throw new ArgumentException(
                                    Environment.GetResourceString("Arg_PathIllegal")
                                );
                        ...
                        if (numSpaces > 0 && firstSegment)
                        {
                            ...
                            if (
                                index + 1 < path.Length
                                && (
                                    path[index + 1] == DirectorySeparatorChar
                ...
                { // Normal character logic
                    ...
                    if (firstSegment && currentChar == VolumeSeparatorChar)
                    {
                        ...
                        bool validPath = (
                            (numDots == 0) && (numSigChars >= 1) && (driveLetter != ' ')
                        );
                        if (!validPath)
                            throw new ArgumentException(
                                Environment.GetResourceString("Arg_PathIllegal")
            ...
            {
                if (
                    newBuffer.OrdinalStartsWith("http:", false)
                    || newBuffer.OrdinalStartsWith("file:", false)
                )
                {
                    throw new ArgumentException(
                        Environment.GetResourceString("Argument_PathUriFormatNotSupported")
                    );
                }
            }
            ...
            if (result != 0)
            {
                ...
                if (newBuffer.Length > 1 && newBuffer[0] == '\\' && newBuffer[1] == '\\')
                {
                    ...
                    if (startIndex == result)
                        throw new ArgumentException(
                            Environment.GetResourceString("Arg_PathIllegalUNC")
                        );
                    ...
                    if (newBuffer.OrdinalStartsWith("\\\\?\\globalroot", true))
                        throw new ArgumentException(
                            Environment.GetResourceString("Arg_PathGlobalRoot")
                        );
                }
            }
