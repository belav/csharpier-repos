...
namespace System
{
...
#endif
    {
        ...
        )
        {
            ...
            for (
                int stringToJoinIndex = startIndex;
                stringToJoinIndex <= endIndex;
                stringToJoinIndex++
            )
            {
            ...
            {
                UnSafeCharBuffer charBuffer = new UnSafeCharBuffer(
                    pointerToJointString,
                    jointLength
                );
                ...
                for (
                    int stringToJoinIndex = startIndex + 1;
                    stringToJoinIndex <= endIndex;
                    stringToJoinIndex++
                )
                {
                ...
                Contract.Assert(
                    *(pointerToJointString + charBuffer.Length) == '\0',
        ...
        internal static extern int nativeCompareOrdinalEx(
            String strA,
            int indexA,
            String strB,
            int indexB,
        ...
        internal static unsafe string SmallCharToUpper(string strIn)
        {
            ...
            fixed (
                char* inBuff = &strIn.m_firstChar,
                    outBuff = &strOut.m_firstChar
            )
            {
        ...
        public static bool Equals(String a, String b, StringComparison comparisonType)
        {
            ...
            switch (comparisonType)
            {
                ...
                case StringComparison.InvariantCulture:
                    return (
                        CultureInfo.InvariantCulture.CompareInfo.Compare(a, b, CompareOptions.None)
                        == 0
                    );
        ...
        [ResourceExposure(ResourceScope.None)]
        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        internal static extern int InternalMarvin32HashString(
            string s,
            int strLen,
        ...
        public String[] Split(String[] separator, Int32 count, StringSplitOptions options)
        {
            ...
            {
                throw new ArgumentException(
                    Environment.GetResourceString("Arg_EnumIllegalVal", (int)options)
                );
            }
            Contract.EndContractBlock();
        ...
        private String[] InternalSplitOmitEmptyEntries(
            Int32[] sepList,
            Int32[] lengthList,
            Int32 numReplaces,
            int count
        ...
        private unsafe int MakeSeparatorList(char[] separator, ref int[] sepList)
        {
            ...
            else
            {
                ...
                fixed (
                    char* pwzChars = &this.m_firstChar,
                        pSepChars = separator
                )
                {
        ...
        //--------------------------------------------------------------------
        [System.Security.SecuritySafeCritical] // auto-generated
        private unsafe int MakeSeparatorList(
            String[] separators,
            ref int[] sepList,
            ref int[] lengthList
        )
                ...
                for (int i = 0; i < Length && foundCount < sepListCount; i++)
                {
                    for (int j = 0; j < separators.Length; j++)
                    {
                        ...
                        if (pwzChars[i] == separator[0] && currentSepLength <= Length - i)
                        {
                            if (
                                currentSepLength == 1
                                || String.CompareOrdinal(this, i, separator, 0, currentSepLength)
                                    == 0
                            )
                            {
                                sepList[foundCount] = i;
        ...
        {
            Contract.Assert(
                startIndex >= 0 && startIndex <= this.Length,
                "StartIndex is out of range!"
            );
            Contract.Assert(
                length >= 0 && startIndex <= this.Length - length,
        ...
        )
        {
            ...
            {
                int doubleCheck = encoding.GetChars(
                    bytes,
                    byteLength,
                    pTempChars,
                    stringLength,
        ...
        {
            Contract.Assert(
                cbNativeBuffer >= (Length + 1) * Marshal.SystemMaxDBCSCharSize,
            ...
            {
                throw new ArgumentException(
                    Environment.GetResourceString("Interop_Marshal_Unmappable_Char")
                );
            }
        ...
        private String CtorCharCount(char c, int count)
        {
            if (count > 0)
            {
                ...
                throw new ArgumentOutOfRangeException(
                    "count",
                    Environment.GetResourceString("ArgumentOutOfRange_MustBeNonNegNum", "count")
                );
        }
        ...
        private unsafe String CtorCharPtr(char* ptr)
        {
            ...
            if (ptr < (char*)64000)
                throw new ArgumentException(
                    Environment.GetResourceString("Arg_MustBeStringPtrNotAtom")
                );
#endif // FEATURE_PAL
        ...
        public static int Compare(String strA, String strB, StringComparison comparisonType)
        {
            ...
            if (
                (uint)(comparisonType - StringComparison.CurrentCulture)
        ...
        )
        {
            ...
            switch (comparisonType)
            {
                ...
                case StringComparison.OrdinalIgnoreCase:
                    return (
                        TextInfo.CompareOrdinalIgnoreCaseEx(
                            strA,
                            indexA,
                            strB,
                            indexB,
                ...
                fixed (char* srcThis = &m_firstChar)
                {
                    fixed (char* srcInsert = &value.m_firstChar)
                    {
                        fixed (char* dst = &result.m_firstChar)
                        {
                            ...
                            wstrcpy(
                                dst + startIndex + insertLength,
                                srcThis + startIndex,
                                oldLength - startIndex
                            );
                        }
                    }
                }
        ...
        public static String Concat(String str0, String str1)
        {
            ...
            Contract.Ensures(
                Contract.Result<String>().Length
                    == (str0 == null ? 0 : str0.Length) + (str1 == null ? 0 : str1.Length)
            );
            Contract.EndContractBlock();
        ...
        public static String IsInterned(String str)
        {
            ...
            Contract.Ensures(
                Contract.Result<String>() == null || Contract.Result<String>().Length == str.Length
            );
            Contract.EndContractBlock();
        ...
        public CharEnumerator GetEnumerator()
        {
            ...
            BCLDebug.Perf(
                false,
                "Avoid using String's CharEnumerator until C# special cases foreach on String - use the indexed property on String instead."
            );
