...
namespace System.Runtime.InteropServices
{
    ...
    // In COM world, events are handled by so-called event sinks. What these are? COM-based Object
    // Models
    // (OMs) define "source" interfaces that need to be implemented by the COM clients to receive
    // events. So,
    // event sinks are COM objects implementing a source interfaces. Once an event sink is passed to the
    // COM
    // server (through a mechanism known as 'binding/advising to connection point'), COM server will be
    // calling source interface methods to "fire events" (advising, connection points, firing events
    // etc. -
    // is all COM jargon).
    //
    // There are few interesting obervations about source interfaces. Usually source interfaces are
    // defined
    ...
    // multiple connection points (each CP handles exactly one source interface). COM objects that want
    // to
    // fire events are required to implement IConnectionPointContainer interface which is used by the
    // COM
    ...
    // generic enough to handle all late-bound event firings and invoke corresponding COM delegates
    // (through
    // reflection).
    //
    // When delegate is registered (using code:ComEventsHelper.Combine) we will verify we have
    // corresponding
    // event sink created and bound.
    //
    // But what happens when COM events are fired? code:ComEventsSink.Invoke implements
    ...
    // dispid of .NET delegate that has been registered for this object. Once this is found we do call
    // the
    // delegates using reflection (code:ComEventsMethod.Invoke).
    //
    // #ComEventsArgsMarshalling
    // Notice, that we may not have a delegate registered against every method on the source interface.
    // If we
    // were to marshal all the input parameters for methods that do not reach user code - we would end
    // up
    // generatic RCWs that are not reachable for user code (the inconvenience it might create is there
    // will
    // be RCWs that users can not call Marshal.ReleaseComObject on to explicitly manage the lifetime of
    // these
    // COM objects). The above behavior was one of the shortcoimings of legacy TLBIMP's implementation
    // of COM
    // event sinking. In our code we will not marshal any data if there is no delegate registered to
    // handle
    // the event. (code:ComEventsMethod.Invoke)
    //
    // #ComEventsFinalization:
    // Additional area of interest is when COM sink should be unadvised from the connection point.
    // Legacy
    // TLBIMP's implementation of COM event sinks will unadvises the sink when corresponding RCW is
    // GCed.
    // This is achieved by rooting the event sinks in a finalizable object stored in RCW's property bag
    // (using Marshal.SetComObjectData). Hence, once RCW is no longer reachable - the finalizer is
    // called and
    ...
    //
    // Classes of interest:
    // * code:ComEventsHelpers - defines public methods but there are also a number of internal classes
    // that
    //     implement the actual COM event sink:
    // * code:ComEventsInfo - represents a finalizable container for all event sinks for a particular
    // RCW.
    ...
    // * code:ComEventsMethod - represents a single method from the source interface which has .NET
    // delegates
    //     attached to it. Maintains an internal pointer to the next instance (in a singly linked list).
    // A
    ...
    // http://support.microsoft.com/kb/810228. Such design might represent a problem for us - e.g. what
    // is
    // the return value of a chain of delegates - is it the value of the last call in the chain or the
    // the
    // first one? As the above KB article indicates, in cases where OM has events returning values, it
    // is
    // suggested that people implement their event sink by explicitly implementing the source interface.
    // This
