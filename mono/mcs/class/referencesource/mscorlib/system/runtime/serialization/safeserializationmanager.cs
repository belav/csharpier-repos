...
namespace System.Runtime.Serialization
{
    ...
    // #SafeSerialization
    //
    // Types which are serializable via the ISerializable interface have a problem when it comes to
    // allowing
    ...
    // transparent exceptions with their own fields that need to be serialized.  (For instance, in
    // transparent
    ...
    // individual chunks of serializable data to be included in the serialized version of the derived
    // class.
    // These chunks are then deserialized back out of the serialized type and notified that they should
    // populate the fields of the deserialized object when serialization is complete.  This allows
    // partial
    // trust or transparent code to participate in serialization of an ISerializable type without having
    // to
    // override GetObjectData or implement the ISerializable constructor.
    //
    // On the serialization side, SafeSerializationManager has an event SerializeObjectState which it
    // will
    // fire in response to serialization in order to gather the units of serializable data that should
    // be
    // stored with the rest of the object during serialization.  Methods which respond to these events
    // create serializable objects which implement the ISafeSerializationData interface and add them to
    // the
    // collection of other serialized data by calling AddSerializedState on the
    // SafeSerializationEventArgs
    // passed into the event.
    //
    // By using an event rather than a virtual method on the base ISerializable object, we allow
    // multiple
    // potentially untrusted subclasses to participate in serialization, without each one having to
    // ensure
    // that it calls up to the base type in order for the whole system to work.  (For instance Exception
    // :
    // TrustedException : UntrustedException, in this scenario UntrustedException would be able to
    // override
    // the virtual method an prevent TrustedException from ever seeing the method call, either
    // accidentally
    // or maliciously).
    //
    ...
    //
    // At deserialization time, SafeSerializationManager performs the reverse operation.  It
    // deserializes the
    // chunks of serialized state, and then notifies them that the object they belong to is deserialized
    // by
    // calling their CompleteSerialization method. In repsonse to this call, the state objects populate
    // the
    // fields of the object being deserialized with the state that they held.
    //
    // From a security perspective, the chunks of serialized state can only contain data that the
    // specific
    // subclass itself had access to read (otherwise it wouldn't be able to populate the type with that
    // data), as opposed to having access to far more data in the SerializationInfo that GetObjectData
    // uses.
    // Similarly, at deserialization time, the serialized state can only modify fields that the type
    // itself
    // has access to (again, as opposed to the full SerializationInfo which could be modified).
    //
    // Individual types which wish to participate in safe serialization do so by containing an instance
    // of a
    // SafeSerializationManager and exposing its serialization event.  During GetObjectData, the
    // SafeSerializationManager is serialized just like any other field of the containing type.
    // However, at
    ...
    // SafeSerializationManager) with itself.  This allows it to gain more control over the
    // deserialization
    // process.  It also saves away an extra bit of state in the serialization info indicating the real
    // type
    ...
    //       m_serializedState       -> list of saved serialized states from subclasses responding to
    // the safe
    //                                  serialization event
    ...
    //
    //  That is, the serialized data claims to be of type SafeSerializationManager, however contains
    // only the
    //  data from the real object being serialized along with one bit of safe serialization metadata.
    //
    //  At deserialization time, since the serialized data claims to be of type
    // SafeSerializationManager, the
    //  root object being created is an instance of the SafeSerializationManager class.  However, it
    // detects
    //  that this isn't a real SafeSerializationManager (by looking for the real type field in the
    // metadata),
    //  and simply saves away the SerializationInfo and the real type being deserialized.
    //
    //  Since SafeSerializationManager implements IObjectReference, the next step of deserialization is
    // the
    //  GetRealObject callback.  This callback is the one responsible for getting the
    //  SafeSerializationManager out of the way and instead creating an instance of the actual type
    // which was
    //  serialized.
    //
    ...
    //  inheritance hierarchy (moving toward the most derived type) looking for the last full trust type
    // to
    //  implement the standard ISerializable constructor before any type does not implement the
    // constructor.
    //  It is this last type's deserialization constructor which is then invoked, passing in the saved
    //  SerializationInfo.  Once the constructors are run, we return this object as the real
    // deserialized
    //  object.
    //
    //  The reason that we do this walk is so that ISerializable types can protect themselves from
    // malicious
    ...
    //  participate in safe serialization and not be required to have access to the parent's
    // constructor.
    //
    //  It should be noted however, that this heuristic means that if a full trust type does derive from
    //  a transparent or partial trust type using this safe serialization mechanism, that full trust
    // type
    ...
    //  handler registered.
    //
    //  Another interesting note is that at this point there are now two SafeSerializationManagers alive
    // for
    //  this deserialization.  The first object is the one which is controlling the deserialization and
    // was
    //  created as the root object of the deserialization.  The second one is the object which contains
    // the
    //  serialized data chunks and is a data member of the real object being deserialized.  For this
    // reason,
    //  the data objects cannot be notified that the deserialization is complete during GetRealObject
    // since
    //  the ISafeSerializationData objects are not members of the active SafeSerializationManager
    // instance.
    //
    //  The next step is the OnDeserialized callback, which comes to SafeSerializableObject since it was
    //  pretending to be the root object of the deserialization.  It responds to this callback by
    // calling
    //  any existing OnDeserialized callback on the real type that was deserialized.
    //
    //  The real type needs to call its data member SafeSerializationData object's
    // CompleteDeserialization
    //  method in response to the OnDeserialized call.  This CompleteDeserialization call will then
    // iterate
    //  through the ISafeSerializationData objects calling each of their CompleteDeserialization methods
    // so
    ...
    //  IDeserializationCallback is that IDeserializationCallback will be called on the stored data
    // chunks
    ...
    //
    //    2. Add a protected SerializeObjectState event, which passes through to the
    // SafeSerializationManager:
    //
    ...
    //       }
    //
    //    3. Serialize the safe serialization object in GetObjectData, and call its
    // CompleteSerialization method:
    //
    //       [SecurityCritical]
    //       void ISerializable.GetObjectData(SerializationInfo info, StreamingContext context)
    //       {
    //           info.AddValue("m_safeSerializationManager", m_safeSerializationManager,
    // typeof(SafeSerializationManager));
    //           m_safeSerializationManager.CompleteSerialization(this, info, context);
    //       }
    //
    //    4. Add an OnDeserialized handler if one doesn't already exist, and call
    // CompleteDeserialization in it:
    //
    //       [OnDeserialized]
    ...
    //       {
    //           SerializeObjectState += delegate(object exception, SafeSerializationEventArgs
    // eventArgs)
    //           {
    ...
    // SafeSerializationEventArgs are provided to the delegates which do safe serialization.  Each
    // delegate
    // serializes its own state into an IDeserializationCallback instance which must, itself, be
    // serializable.
    // These indivdiual states are then added to the SafeSerializationEventArgs in order to be saved
    // away when
    ...
    internal sealed class SafeSerializationManager : IObjectReference, ISerializable
    {
        ...
        private SafeSerializationManager(SerializationInfo info, StreamingContext context)
        {
            ...
            // construction.
            RuntimeType realType =
                info.GetValueNoThrow(RealTypeSerializationName, typeof(RuntimeType)) as RuntimeType;

            if (realType == null)
            {
                m_serializedStates =
                    info.GetValue("m_serializedStates", typeof(List<object>)) as List<object>;
            }
            else
            {
                m_realType = realType;
        ...
        )
        {
            ...
            EventHandler<SafeSerializationEventArgs> serializeObjectStateEvent =
        ...
        void ISerializable.GetObjectData(SerializationInfo info, StreamingContext context)
        {
            info.AddValue(
                "m_serializedStates",
                m_serializedStates,
                typeof(List<IDeserializationCallback>)
            );
        }
        ...
        object IObjectReference.GetRealObject(StreamingContext context)
        {
            ...
            BCLDebug.Assert(
                previousType != null,
                "We should have at least one inheritance from the base type"
            );
