...
#pragma warning restore 0414
        ...
        )
        {
            ...
            if (
                ...
                && (
                    serializationSurrogate = surrogateSelector.GetSurrogate(
                        objectType,
                        context,
        ...
        private void DumpMemberInfo()
        {
            for (int i = 0; i < cache.memberInfos.Length; i++)
            {
                SerTrace.Log(
                    this,
                    objectInfoId,
                    " Constructor 1 memberInfos data ",
                ...
                if (!FormatterServices.UnsafeTypeForwardersIsEnabled())
                {
                    ...
                    )
                    {
                        ...
                        throw new SecurityException(
                            Environment.GetResourceString(
                                "Serialization_RequireFullTrust",
                                objectType
                            )
                        );
                    }
        ...
        )
        {
            ...
            if (isSi)
            {
                if (!isNamed)
                    throw new SerializationException(
                        Environment.GetResourceString("Serialization_ISerializableMemberInfo")
                    );
            }
        }
...
#pragma warning restore 0414
        ...
        internal Type GetType(String name)
        {
            ...
            if ((object)type == null)
                throw new SerializationException(
                    Environment.GetResourceString(
                        "Serialization_ISerializableTypes",
                        objectType + " " + name
                    )
                );
        ...
        internal void RecordFixup(long objectId, String name, long idRef)
        {
            if (isSi)
            {
                SerTrace.Log(
                    ...
                    " ",
                    objectType,
                    " RecordFixup  RecordDelayedFixup objectId ",
                    objectId,
                    " name ",
                    name,
                    " idRef ",
        ...
        private int Position(String name)
        {
            ...
            if (cache.memberNames.Length > 0 && cache.memberNames[lastPosition].Equals(name))
            {
                ...
                (++lastPosition < cache.memberNames.Length)
                && (cache.memberNames[lastPosition].Equals(name))
            )
            {
                return lastPosition;
            }
        ...
        internal Type[] GetMemberTypes(String[] inMemberNames, Type objectType)
        {
            if (isSi)
                throw new SerializationException(
                    Environment.GetResourceString("Serialization_ISerializableTypes", objectType)
                );
    ...
    internal sealed class TypeInformation
    {
        ...
        {
            get { return assemblyString; }
        }

        internal bool HasTypeForwardedFrom
        {
            get { return hasTypeForwardedFrom; }
        }
