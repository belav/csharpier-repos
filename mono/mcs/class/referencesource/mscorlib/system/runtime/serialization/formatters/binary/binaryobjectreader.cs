...
#pragma warning restore
...
#endif
#if MONO_FEATURE_CAS
        private static FileIOPermission sfileIOPermission = new FileIOPermission(
        ...
        )
        {
            ...
            SerTrace.Log(
                this,
                "Constructor ISurrogateSelector ",
                ((selector == null) ? "null selector " : "selector present")
            );
        ...
        )
        {
            if (serParser == null)
                throw new ArgumentNullException(
                    "serParser",
                    Environment.GetResourceString("ArgumentNull_WithParamName", serParser)
                );
            Contract.EndContractBlock();
...
#endif
            {
                if (TopObject == null)
                    throw new SerializationException(
                        Environment.GetResourceString("Serialization_TopObject")
                    );
            ...
            else if (bMethodReturn)
            {
                ...
                TopObject = binaryMethodReturn.ReadArray(
                    methodReturnArray,
                    methodCallMessage,
                    handlerObject
                );
        ...
        {
            Contract.Assert(
                (index < crossAppDomainArray.Length),
        ...
        internal void Parse(ParseRecord pr)
        {
            ...
            switch (pr.PRparseTypeEnum)
            {
                ...
                default:
                    throw new SerializationException(
                        Environment.GetResourceString("Serialization_XMLElement", pr.PRname)
                    );
            }
        }
        ...
        private void ParseError(ParseRecord processing, ParseRecord onStack)
        {
            ...
            throw new SerializationException(
                Environment.GetResourceString(
                    "Serialization_ParseError",
                    onStack.PRname
                        + " "
                        + ((Enum)onStack.PRparseTypeEnum)
                        + " "
                        + processing.PRname
                        + " "
                        + ((Enum)processing.PRparseTypeEnum)
                )
            );
        }
                ...
                if (IsRemoting)
                {
                    if (typeof(MarshalByRefObject).IsAssignableFrom(t))
                        throw new ArgumentException(
                            Environment.GetResourceString("Serialization_MBRAsMBV", t.FullName)
                        );
        ...
        private void ParseObject(ParseRecord pr)
        {
            ...
            if (pr.PRnewObj == null)
                throw new SerializationException(
                    Environment.GetResourceString("Serialization_TopObjectInstantiate", pr.PRdtType)
                );
            ...
            if (pr.PRobjectInfo == null)
                pr.PRobjectInfo = ReadObjectInfo.Create(
                    pr.PRdtType,
                    m_surrogates,
                    m_context,
                    m_objectManager,
        ...
        private void ParseObjectEnd(ParseRecord pr)
        {
            ...
            if (objectPr.PRobjectTypeEnum == InternalObjectTypeE.Array)
            {
                ...
                SerTrace.Log(
                    this,
                    "ParseArray  RegisterObject ",
                    objectPr.PRobjectId,
                    " ",
                    objectPr.PRnewObj.GetType()
                );
#endif
            ...
            if (objectPr.PRobjectPositionEnum == InternalObjectPositionE.Top)
            {
                ...
                SerTrace.Log(
                    this,
                    "ParseObjectEnd  Top Object Resolved ",
                    objectPr.PRnewObj.GetType()
                );
#endif
                TopObject = objectPr.PRnewObj;
        ...
        private void ParseArray(ParseRecord pr)
        {
            ...
            if (pr.PRarrayTypeEnum == InternalArrayTypeE.Base64)
            {
                ...
                (pr.PRnewObj != null) && Converter.IsWriteAsByteArray(pr.PRarrayElementTypeCode)
            )
            {
            ...
            )
            {
                ...
                SerTrace.Log(
                    this,
                    "ParseArray Before Jagged,Simple create ",
                    pr.PRarrayElementType,
                    " ",
                    pr.PRlengthA[0]
                );
                ...
                if ((pr.PRlowerBoundA == null) || (pr.PRlowerBoundA[0] == 0))
                {
                    ...
                    {
                        pr.PRnewObj = Array.UnsafeCreateInstance(
                            pr.PRarrayElementType,
                            pr.PRlengthA[0]
                        );
                    }
                ...
                if (pr.PRarrayTypeEnum == InternalArrayTypeE.Single)
                {
                    if (
                        !pr.PRisLowerBound
                        && (Converter.IsWriteAsByteArray(pr.PRarrayElementTypeCode))
                    )
                    {
                        pr.PRprimitiveArray = new PrimitiveArray(
                            pr.PRarrayElementTypeCode,
                            (Array)pr.PRnewObj
                        );
                    }
            ...
            else if (pr.PRarrayTypeEnum == InternalArrayTypeE.Rectangular)
            {
                ...
                throw new SerializationException(
                    Environment.GetResourceString(
                        "Serialization_ArrayType",
                        ((Enum)pr.PRarrayTypeEnum)
                    )
                );
        ...
        private void ParseArrayMember(ParseRecord pr)
        {
            ...
            if (objectPr.PRarrayTypeEnum == InternalArrayTypeE.Rectangular)
            {
                ...
                if (objectPr.PRisLowerBound)
                {
                    ...
                    {
                        objectPr.PRindexMap[i] =
                            objectPr.PRrectangularMap[i] + objectPr.PRlowerBoundA[i];
                    }
                }
            }
            else
            ...
            IndexTraceMessage(
                "ParseArrayMember isLowerBound " + objectPr.PRisLowerBound + " indexMap  ",
                objectPr.PRindexMap
            );
            ...
            if (pr.PRmemberValueEnum == InternalMemberValueE.Reference)
            {
                ...
                if (refObj == null)
                {
                    ...
                    // Array fixup manager
                    IndexTraceMessage(
                        "ParseArrayMember Record Fixup  " + objectPr.PRnewObj.GetType(),
                        objectPr.PRindexMap
                    );
                    ...
                    SerTrace.Log(
                        this,
                        "ParseArrayMember RecordArrayElementFixup objectId ",
                        objectPr.PRobjectId,
                        " idRef ",
                        pr.PRidRef
                    );
                    m_objectManager.RecordArrayElementFixup(
                        objectPr.PRobjectId,
                        fixupIndex,
                        pr.PRidRef
                    );
                ...
                {
                    IndexTraceMessage(
                        "ParseArrayMember SetValue ObjectReference "
                            + objectPr.PRnewObj.GetType()
                            + " "
                            + refObj,
                        objectPr.PRindexMap
            ...
            else if (pr.PRmemberValueEnum == InternalMemberValueE.Nested)
            {
                ...
                {
                    if (
                        (objectPr.PRarrayElementType.IsValueType)
                        && (pr.PRarrayElementTypeCode == InternalPrimitiveTypeE.Invalid)
                    )
                    {
#if _DEBUG
                        SerTrace.Log(
                            "ParseArrayMember ValueType ObjectPr ",
                            objectPr.PRnewObj,
                            " index ",
                        ...
                        ValueFixupStack.Push(
                            new ValueFixup((Array)objectPr.PRnewObj, objectPr.PRindexMap)
                        ); //valuefixup
                    }
                    else
                    {
#if _DEBUG
                        SerTrace.Log(
                            "ParseArrayMember SetValue Nested, memberIndex ",
                            objectPr.PRmemberIndex
                        );
                        IndexTraceMessage(
                            "ParseArrayMember SetValue Nested ContainerObject "
                                + objectPr.PRnewObj.GetType()
                                + " "
                                + objectPr.PRnewObj
                                + " item Object "
                        ...
                        stack.Dump();
                        SerTrace.Log(
                            "ParseArrayMember SetValue Nested ContainerObject objectPr ",
                            objectPr.Trace()
                        );
                        SerTrace.Log(
                            "ParseArrayMember SetValue Nested ContainerObject pr ",
                            pr.Trace()
                        );
#endif
                        if (objectPr.PRobjectA != null)
            ...
            {
                if (
                    (Object.ReferenceEquals(objectPr.PRarrayElementType, Converter.typeofString))
                    || (Object.ReferenceEquals(pr.PRdtType, Converter.typeofString))
                )
                {
                    // String in either a string array, or a string element of an object array
                    ParseString(pr, objectPr);
                    IndexTraceMessage(
                        "ParseArrayMember SetValue String "
                            + objectPr.PRnewObj.GetType()
                            + " "
                            + pr.PRvalue,
                        objectPr.PRindexMap
                    ...
                    if (objectPr.PRobjectA != null)
                        ...
                        ((Array)objectPr.PRnewObj).SetValue(
                            (Object)pr.PRvalue,
                            objectPr.PRindexMap
                        );
                }
                else if (objectPr.PRisArrayVariant)
                {
                    ...
                    if (pr.PRkeyDt == null)
                        throw new SerializationException(
                            Environment.GetResourceString("Serialization_ArrayTypeObject")
                        );
                    ...
                    if (Object.ReferenceEquals(pr.PRdtType, Converter.typeofString))
                    {
                        ...
                        Object.ReferenceEquals(pr.PRdtTypeCode, InternalPrimitiveTypeE.Invalid)
                    )
                    {
                        CheckSerializable(pr.PRdtType);
                    ...
                    IndexTraceMessage(
                        "ParseArrayMember SetValue variant or Object "
                            + objectPr.PRnewObj.GetType()
                            + " var "
                            + var
                            + " indexMap ",
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        SerTrace.Log(
                            this,
                            "ParseArrayMember SetValue Primitive pr.PRvalue " + var,
                            " elementTypeCode ",
                            ((Enum)objectPr.PRdtTypeCode)
                        );
                        IndexTraceMessage(
                            "ParseArrayMember SetValue Primitive "
                                + objectPr.PRnewObj.GetType()
                                + " var: "
                                + var
                                + " varType "
                        ...
                        if (objectPr.PRobjectA != null)
                        {
                            SerTrace.Log(
                                this,
                                "ParseArrayMember SetValue Primitive predefined array "
                                    + objectPr.PRobjectA.GetType()
                            );
                            objectPr.PRobjectA[objectPr.PRindexMap[0]] = var;
                        }
                        else
...
#if _DEBUG
            SerTrace.Log(
                "ParseArrayMember increment memberIndex ",
                objectPr.PRmemberIndex,
                " ",
                objectPr.Trace()
            );
        ...
        private void ParseMember(ParseRecord pr)
        {
            ...
            else if (pr.PRmemberValueEnum == InternalMemberValueE.Nested)
            {
                SerTrace.Log(
                    this,
                    "ParseMember Nested Type member: ",
                    pr.PRname,
                    " objectPr.PRnewObj ",
                    objectPr.PRnewObj
                );
                ...
                SerTrace.Log(
                    ...
                    pr.PRnewObj,
                    " is value type ",
                    pr.PRnewObj.GetType().IsValueType
                );

                if (
                    (pr.PRobjectInfo != null)
                    && ((object)pr.PRobjectInfo.objectType != null)
                    && (pr.PRobjectInfo.objectType.IsValueType)
                )
                {
                    SerTrace.Log(
                        "ParseMember ValueType ObjectPr ",
                        objectPr.PRnewObj,
                        " memberName  ",
                        pr.PRname,
                        " nested object ",
                    ...
                    pr.PRisValueTypeFixup = true; //Valuefixup
                    ValueFixupStack.Push(
                        new ValueFixup(objectPr.PRnewObj, pr.PRname, objectPr.PRobjectInfo)
                    ); //valuefixup
                }
                else
                {
            ...
            else if (pr.PRmemberValueEnum == InternalMemberValueE.Reference)
            {
                ...
                else
                {
                    SerTrace.Log(
                        this,
                        "ParseMember Referenced Object Known ",
                        pr.PRname,
                        " ",
                        refObj
                    );
            ...
            else if (pr.PRmemberValueEnum == InternalMemberValueE.InlineValue)
            {
                ...
                else if (pr.PRdtTypeCode == InternalPrimitiveTypeE.Invalid)
                {
                    ...
                    if (pr.PRarrayTypeEnum == InternalArrayTypeE.Base64)
                    {
                        ...
                        throw new SerializationException(
                            Environment.GetResourceString("Serialization_TypeMissing", pr.PRname)
                        );
                    else
                    {
                        SerTrace.Log(
                            this,
                            "Object Class with no memberInfo data  Member "
                                + pr.PRname
                                + " type "
                                + pr.PRdtType
                        );
        ...
        )
        {
            if (!pr.PRisRegistered)
            {
                ...
                if (bIsString)
                    m_objectManager.RegisterString(
                        (String)obj,
                        pr.PRobjectId,
                        si,
                        parentId,
                    ...
                    m_objectManager.RegisterObject(
                        obj,
                        pr.PRobjectId,
                        si,
                        parentId,
            ...
            if (entry == null || entry.assemblyName != assemblyName)
            {
                ...
                if (bSimpleAssembly)
                {
                    try
                    {
                        ...
                        {
#if FEATURE_FUSION
                            assm = ObjectReader.ResolveSimpleAssemblyName(
        ...
        private static Assembly ResolveSimpleAssemblyName(AssemblyName assemblyName)
        {
            ...
            Assembly assm = RuntimeAssembly.LoadWithPartialNameInternal(
                assemblyName,
                null,
                ref stackMark
            );
            if (assm == null && assemblyName != null)
                assm = RuntimeAssembly.LoadWithPartialNameInternal(
                    assemblyName.Name,
                    null,
                    ref stackMark
                );
        ...
        )
        {
            ...
            if ((object)type == null)
            {
                type = Type.GetType(
                    typeName,
                    ObjectReader.ResolveSimpleAssemblyName,
        ...
        internal Type GetType(BinaryAssemblyInfo assemblyInfo, String name)
        {
            ...
            if (
                (
                    (previousName != null)
                    && (previousName.Length == name.Length)
                    && (previousName.Equals(name))
                )
                && (
                    (previousAssemblyString != null)
                    && (previousAssemblyString.Length == assemblyInfo.assemblyString.Length)
                    && (previousAssemblyString.Equals(assemblyInfo.assemblyString))
                )
            )
            {
            ...
            else
            {
                ...
                if ((object)objectType == null)
                {
                    ...
                    if (bSimpleAssembly)
                    {
                        ObjectReader.GetSimplyNamedTypeFromAssembly(
                            sourceAssembly,
                            name,
                            ref objectType
                        );
