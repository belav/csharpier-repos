...
namespace System.Runtime.Remoting.Activation
{
    internal class RemotingXmlConfigFileData
    {
        ...
        internal class InteropXmlElementEntry
        {
            ...
            internal InteropXmlElementEntry(
        ...
        internal class RemoteAppEntry
        {
            ...
            internal void AddActivatedEntry(
                String typeName,
                String assemName,
        ...
        internal void AddInteropXmlElementEntry(
            String xmlElementName,
            String xmlElementNamespace,
            String urtTypeName,
            String urtAssemblyName
        ...
        internal void AddInteropXmlTypeEntry(
            String xmlTypeName,
            String xmlTypeNamespace,
            String urtTypeName,
            String urtAssemblyName
        ...
        {
            TryToLoadTypeIfApplicable(urtTypeName, urtAssemblyName);
            InteropXmlTypeEntry ixte = new InteropXmlTypeEntry(
                xmlTypeName,
                xmlTypeNamespace,
                urtTypeName,
                urtAssemblyName
        ...
        internal void AddServerActivatedEntry(
            String typeName,
            String assemName,
            ArrayList contextAttributes
        )
        ...
        internal ServerWellKnownEntry AddServerWellKnownEntry(
            String typeName,
            String assemName,
            ArrayList contextAttributes,
            String objURI,
        ...
        {
            TryToLoadTypeIfApplicable(typeName, assemName);
            ServerWellKnownEntry swke = new ServerWellKnownEntry(
                typeName,
                assemName,
                contextAttributes,
                objURI,
    ...
    internal static class RemotingXmlConfigFileParser
    {
        ...
        private static void ReportUniqueSectionError(
            ConfigNode parent,
            ConfigNode child,
        ...
        private static void ReportUnknownValueError(
            ConfigNode node,
            String value,
            RemotingXmlConfigFileData configData
        )
        ...
        private static void ReportMissingAttributeError(
            ConfigNode node,
        ...
        private static void ReportMissingTypeAttributeError(
            ConfigNode node,
            String attributeName,
            RemotingXmlConfigFileData configData
        )
        ...
        private static void ReportMissingXmlTypeAttributeError(
            ConfigNode node,
            String attributeName,
            RemotingXmlConfigFileData configData
        )
        ...
        private static void ReportInvalidTimeFormatError(
            String time,
            RemotingXmlConfigFileData configData
        )
        {
        ...
        private static void ReportNonTemplateIdAttributeError(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
            ...
            ReportError(
                String.Format(
                    ...
                    Environment.GetResourceString(
                        "Remoting_Config_TemplateCannotReferenceTemplate"
                    ),
                    node.Name
                ),
                configData
        ...
        private static void ProcessDebugNode(ConfigNode node, RemotingXmlConfigFileData configData)
        {
            foreach (DictionaryEntry entry in node.Attributes)
            {
                ...
                {
                    case "loadTypes":
                        RemotingXmlConfigFileData.LoadTypes = Convert.ToBoolean(
                            (String)entry.Value,
        ...
        private static void ProcessApplicationNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
            ...
            foreach (ConfigNode childNode in node.Children)
            {
                ...
                {
                    case "channels":
                        ProcessChannelsNode(childNode, configData);
                        break;
                    case "client":
                        ProcessClientNode(childNode, configData);
                        break;
                    case "lifetime":
                        ProcessLifetimeNode(node, childNode, configData);
                        break;
                    case "service":
                        ProcessServiceNode(childNode, configData);
                        break;
                    case "soapInterop":
                        ProcessSoapInteropNode(childNode, configData);
                        break;
        ...
        private static void ProcessCustomErrorsNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
            foreach (DictionaryEntry entry in node.Attributes)
            {
                ...
                if (key.Equals("mode"))
                {
                    ...
                    if (String.Compare(value, "on", StringComparison.OrdinalIgnoreCase) == 0)
                        ...
                        String.Compare(value, "remoteonly", StringComparison.OrdinalIgnoreCase) == 0
                    )
        ...
        private static void ProcessLifetimeNode(
            ...
            foreach (DictionaryEntry entry in node.Attributes)
            {
                ...
                switch (key)
                {
                    ...
                    case "sponsorshipTimeout":
                        configData.Lifetime.SponsorshipTimeout = ParseTime(
                            (String)entry.Value,
                            configData
                        );
                        break;
                    ...
                    case "renewOnCallTime":
                        configData.Lifetime.RenewOnCallTime = ParseTime(
                            (String)entry.Value,
                            configData
                        );
                        break;
                    ...
                    case "leaseManagerPollTime":
                        configData.Lifetime.LeaseManagerPollTime = ParseTime(
                            (String)entry.Value,
                            configData
                        );
                        break;
        ...
        private static void ProcessServiceNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
            foreach (ConfigNode childNode in node.Children)
            {
                ...
                {
                    case "wellknown":
                        ProcessServiceWellKnownNode(childNode, configData);
                        break;
                    case "activated":
                        ProcessServiceActivatedNode(childNode, configData);
                        break;
        ...
        private static void ProcessClientNode(ConfigNode node, RemotingXmlConfigFileData configData)
        {
            ...
            foreach (DictionaryEntry entry in node.Attributes)
            {
                ...
                {
                    case "url":
                        remoteAppUri = (String)entry.Value;
                        break;

                    case "displayName":
                        break; // displayName is ignored (used by config utility for labelling the application)
            ...
            RemotingXmlConfigFileData.RemoteAppEntry remoteApp = configData.AddRemoteAppEntry(
                remoteAppUri
            );
            ...
            foreach (ConfigNode childNode in node.Children)
            {
                ...
                {
                    case "wellknown":
                        ProcessClientWellKnownNode(childNode, configData, remoteApp);
                        break;
                    case "activated":
                        ProcessClientActivatedNode(childNode, configData, remoteApp);
                        break;
        ...
        // appears under "application"
        private static void ProcessSoapInteropNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
            ...
            foreach (DictionaryEntry entry in node.Attributes)
            {
                ...
                switch (key)
                {
                    case "urlObjRef":
                    {
                        configData.UrlObjRefMode = Convert.ToBoolean(
                            entry.Value,
                            CultureInfo.InvariantCulture
                        );
                        break;
            ...
            foreach (ConfigNode childNode in node.Children)
            {
                ...
                {
                    case "preLoad":
                        ProcessPreLoadNode(childNode, configData);
                        break;
                    case "interopXmlElement":
                        ProcessInteropXmlElementNode(childNode, configData);
                        break;
                    case "interopXmlType":
                        ProcessInteropXmlTypeNode(childNode, configData);
                        break;
        ...
        // appears under "application"
        private static void ProcessChannelsNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
        ...
        private static void ProcessServiceWellKnownNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
            ...
            foreach (DictionaryEntry entry in node.Attributes)
            {
                ...
                switch (key)
                {
                    ...
                    case "mode":
                    {
                        ...
                        else if (String.CompareOrdinal(value, "SingleCall") == 0)
                    ...
                    case "objectUri":
                        objectURI = (String)entry.Value;
                        break;

                    case "type":
                    {
                        RemotingConfigHandler.ParseType(
                            (String)entry.Value,
                            out typeName,
                            out assemName
                        );
            ...
            configData.AddServerWellKnownEntry(
                typeName,
                assemName,
                contextAttributes,
                objectURI,
        ...
        // appears under "application/service"
        private static void ProcessServiceActivatedNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
        ...
        private static void ProcessClientWellKnownNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData,
        ...
        // appears under "application/client"
        private static void ProcessClientActivatedNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData,
            RemotingXmlConfigFileData.RemoteAppEntry remoteApp
        )
        ...
        private static void ProcessInteropXmlElementNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
            ...
            configData.AddInteropXmlElementEntry(
                xmlElementName,
                xmlElementNamespace,
                urtTypeName,
                urtAssemName
        ...
        private static void ProcessInteropXmlTypeNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
            ...
            configData.AddInteropXmlTypeEntry(
                xmlTypeName,
                xmlTypeNamespace,
                urtTypeName,
                urtAssemName
        ...
        private static void ProcessPreLoadNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
        ...
        private static RemotingXmlConfigFileData.ContextAttributeEntry ProcessContextAttributeNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
        ...
        // appears under "application/client"
        private static RemotingXmlConfigFileData.ChannelEntry ProcessChannelsChannelNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData,
            bool isTemplate
        )
            ...
            foreach (DictionaryEntry entry in node.Attributes)
            {
                ...
                switch (keyStr)
                {
                    ...
                    case "ref":
                    {
                        ...
                        {
                            channelTemplate = (RemotingXmlConfigFileData.ChannelEntry)
                                _channelTemplates[entry.Value];
        ...
        private static void ProcessSinkProviderNodes(
            ConfigNode node,
            RemotingXmlConfigFileData.ChannelEntry channelEntry,
            RemotingXmlConfigFileData configData,
            bool isServer
        )
        {
            ...
            foreach (ConfigNode childNode in node.Children)
            {
                RemotingXmlConfigFileData.SinkProviderEntry entry = ProcessSinkProviderNode(
                    childNode,
                    configData,
                    false,
                    isServer
        ...
        private static RemotingXmlConfigFileData.SinkProviderEntry ProcessSinkProviderNode(
            ConfigNode node,
            RemotingXmlConfigFileData configData,
            bool isTemplate,
            bool isServer
        )
        {
            ...
            RemotingXmlConfigFileData.SinkProviderEntry sinkProviderEntry =
                new RemotingXmlConfigFileData.SinkProviderEntry(
                    typeName,
                    assemName,
                    properties,
                    isFormatter
            ...
            foreach (ConfigNode childNode in node.Children)
            {
                SinkProviderData providerData = ProcessSinkProviderData(childNode, configData);
        ...
        private static SinkProviderData ProcessSinkProviderData(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
        ...
        private static void ProcessChannelTemplates(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
            foreach (ConfigNode childNode in node.Children)
            {
                ...
                {
                    case "channel":
                        ProcessChannelsChannelNode(childNode, configData, true);
                        break;
        ...
        private static void ProcessChannelSinkProviderTemplates(
            ConfigNode node,
            RemotingXmlConfigFileData configData
        )
        {
            foreach (ConfigNode childNode in node.Children)
            {
                ...
                {
                    case "clientProviders":
                        ProcessChannelProviderTemplates(childNode, configData, false);
                        break;
                    case "serverProviders":
                        ProcessChannelProviderTemplates(childNode, configData, true);
                        break;
        ...
        private static void ProcessChannelProviderTemplates(
            ConfigNode node,
            RemotingXmlConfigFileData configData,
            bool isServer
        )
        ...
        private static TimeSpan ParseTime(String time, RemotingXmlConfigFileData configData)
        {
            ...
            try
            {
                ...
                int value = Int32.Parse(
                    time.Substring(0, time.Length - metricLength),
                    CultureInfo.InvariantCulture
                );
