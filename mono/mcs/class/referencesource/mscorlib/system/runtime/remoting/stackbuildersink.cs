    ...
    internal class StackBuilderSink : IMessageSink
    {
        ...
        public virtual IMessage SyncProcessMessage(IMessage msg)
        {
            ...
            try
            {
                ...
                // ************
                if (IsOKToStackBlt(mcMsg, server) && ((Message)mcMsg).Dispatch(server))
                {
                ...
                else
                {
                    ...
                    ret = PrivateProcessMessage(mb.MethodHandle, args, server, out outArgs);
        ...
        {
            if (
                reflectedType.FullName.Equals(sIRemoteDispatch)
                && reflectedType
                    .GetRuntimeAssembly()
                    .GetSimpleName()
                    .Equals(sIRemoteDispatchAssembly)
            )
            {
                throw new RemotingException(
        ...
        )
        {
            BCLDebug.Assert(
                ...
                "message should always have a call context"
            );
