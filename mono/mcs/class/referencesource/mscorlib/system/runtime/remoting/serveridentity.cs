...
#pragma warning restore 0414
        ...
        internal IMessageSink GetServerObjectChain(out MarshalByRefObject obj)
        {
            ...
            if (!this.IsSingleCall())
            {
                ...
                if (_serverObjectChain == null)
                {
                    ...
                    try
                    {
                        ...
                        if (_serverObjectChain == null)
                        {
                            MarshalByRefObject srvObj = (MarshalByRefObject)this.TPOrObject;

                            _serverObjectChain = _srvCtx.CreateServerObjectChain(srvObj);
                        }
                    }
                    finally
                    {
                ...
                BCLDebug.Assert(null != _serverObjectChain, "null != _serverObjectChain");
            ...
            else
            {
                ...
                if (
                    _tpOrObject != null
                    && _firstCallDispatched == 0
                    && Interlocked.CompareExchange(ref _firstCallDispatched, 1, 0) == 0
                )
                {
                ...
                else
                {
                    ...
                    // on each dispatch!
                    srvObj = (MarshalByRefObject)Activator.CreateInstance((Type)_srvType, true);
                    ...
                    if (tempUri != null)
                    {
                        throw new RemotingException(
                            String.Format(
                                ...
                                Environment.GetResourceString("Remoting_WellKnown_CtorCantMarshal"),
                                this.URI
                            )
                        );
                    }
                    ...
                    if (!RemotingServices.IsTransparentProxy(srvObj))
                    {
                        ...
                        BCLDebug.Assert(
                            idObj == MarshalByRefObject.GetIdentity(srvObj),
                            "Bad ID state!"
                        );
#endif
                    }
        ...
        internal override void AssertValid()
        {
            ...
            {
                BCLDebug.Assert(
                    MarshalByRefObject.GetIdentity((MarshalByRefObject)this.TPOrObject) == this,
                    "Server ID mismatch with Object"
                );
            }
        }
    }
