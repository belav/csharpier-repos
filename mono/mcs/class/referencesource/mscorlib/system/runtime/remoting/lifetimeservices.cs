    ...
    internal class LeaseLifeTimeServiceProperty : IContextProperty, IContributeObjectSink
    {
        ...
        public IMessageSink GetObjectSink(MarshalByRefObject obj, IMessageSink nextSink)
        {
            ...
            ServerIdentity identity = (ServerIdentity)
                MarshalByRefObject.GetIdentity(obj, out fServer);
            ...
            if (identity.IsSingleCall())
            {
                BCLDebug.Trace(
                    "REMOTE",
                    "LeaseLifeTimeServiceProperty.GetObjectSink, no lease SingleCall",
                    obj,
                    ", NextSink " + nextSink
                );
                return nextSink;
            ...
            BCLDebug.Trace(
                "REMOTE",
                "LeaseLifeTimeServiceProperty.GetObjectSink, return from InitializeLifetimeService obj ",
                obj,
                ", lease ",
                leaseObj
            );
            ...
            if (leaseObj == null)
            {
                BCLDebug.Trace(
                    "REMOTE",
                    "LeaseLifeTimeServiceProperty.GetObjectSink, no lease ",
                    obj,
                    ", NextSink " + nextSink
                );
                return nextSink;
            ...
            if (ilease.InitialLeaseTime.CompareTo(TimeSpan.Zero) <= 0)
            {
                // No lease
                {
                    BCLDebug.Trace(
                        "REMOTE",
                        "LeaseLifeTimeServiceProperty.GetObjectSink, no lease because InitialLeaseTime is Zero ",
                        obj
                    );
            ...
            lock (identity)
            {
                if (identity.Lease != null)
                {
                    ...
                    BCLDebug.Trace(
                        "REMOTE",
                        "LeaseLifeTimeServiceProperty.GetObjectSink, Lease already exists for object ",
                        obj
                    );
                ...
                else
                {
                    ...
                    if (!(ilease is System.Runtime.Remoting.Lifetime.Lease))
                    {
                        ...
                        BCLDebug.Trace(
                            "REMOTE",
                            "LeaseLifeTimeServiceProperty.GetObjectSink, New Lease, lease not of type Lease  ",
                            obj
                        );
                    ...
                    else
                    {
                        ...
                        BCLDebug.Trace(
                            "REMOTE",
                            "LeaseLifeTimeServiceProperty.GetObjectSink, New Lease, lease is type Lease  ",
                            obj
                        );
                        lease = (Lease)ilease;
                    }
            ...
            if (lease.RenewOnCallTime > TimeSpan.Zero)
            {
                ...
                BCLDebug.Trace(
                    "REMOTE",
                    "LeaseLifeTimeServiceProperty.GetObjectSink, lease created ",
                    obj
                );
            ...
            else
            {
                ...
                BCLDebug.Trace(
                    "REMOTE",
                    "LeaseLifeTimeServiceProperty.GetObjectSink, No RenewOnCall so no sink created ",
                    obj
                );
                return nextSink;
            }
