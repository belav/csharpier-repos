    ...
    public sealed class MemoryFailPoint : CriticalFinalizerObject, IDisposable
    {
        ...
        public MemoryFailPoint(int sizeInMegabytes)
        {
            ...
            for (int stage = 0; stage < 3; stage++)
            {
                ...
                BCLDebug.Trace(
                    "MEMORYFAILPOINT",
                    "MemoryFailPoint: Checking for {0} MB, for allocation size of {1} MB, stage {9}.  Need page file? {2}  Need Address Space? {3}  Need Contiguous address space? {4}  Avail page file: {5} MB  Total free VA space: {6} MB  Contiguous free address space (found): {7} MB  Space reserved via process's MemoryFailPoints: {8} MB",
                    segmentSize >> 20,
                    sizeInMegabytes,
                    needPageFile,
                    needAddressSpace,
                ...
                switch (stage)
                {
                    ...
                    case 1:
                        ...
                        finally
                        {
                            ...
                            unsafe
                            {
                                void* pMemory = Win32Native.VirtualAlloc(
                                    null,
                                    numBytes,
                                    Win32Native.MEM_COMMIT,
                                    Win32Native.PAGE_READWRITE
                                );
                                if (pMemory != null)
                                {
                                    bool r = Win32Native.VirtualFree(
                                        pMemory,
                                        UIntPtr.Zero,
                                        Win32Native.MEM_RELEASE
                                    );
                                    if (!r)
                                        __Error.WinIOError();
                                }
                    ...
                    case 2:
                        ...
                        if (needPageFile || needAddressSpace)
                        {
                            InsufficientMemoryException e = new InsufficientMemoryException(
                                Environment.GetResourceString("InsufficientMemory_MemFailPoint")
                            );
#if _DEBUG
        ...
        private static void CheckForAvailableMemory(
        ...
        private static unsafe bool CheckForFreeAddressSpace(ulong size, bool shouldThrow)
        {
            ...
            BCLDebug.Trace(
                "MEMORYFAILPOINT",
                "MemoryFailPoint: Checked for free VA space.  Found enough? {0}  Asked for: {1}  Found: {2}",
                (freeSpaceAfterGCHeap >= size),
                size,
                freeSpaceAfterGCHeap
            );
        ...
        private static unsafe ulong MemFreeAfterAddress(void* address, ulong size)
        {
            ...
            Win32Native.MEMORY_BASIC_INFORMATION memInfo =
        ...
        private static extern void GetMemorySettings(
            out ulong maxGCSegmentSize,
            out ulong topOfMemory
        );
