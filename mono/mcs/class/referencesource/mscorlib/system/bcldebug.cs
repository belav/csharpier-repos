...
namespace System
{
    ...
    internal static class BCLDebug
    {
        ...
        private static extern int GetRegistryLoggingValues(
            ...
            out int logLevel,
            out bool perfWarnings,
            out bool correctnessWarnings,
            out bool safeHandleStackTraces
        );
        ...
        private static void CheckRegistry()
        {
            ...
            int logLevel;
            int facilityValue;
            facilityValue = GetRegistryLoggingValues(
                out loggingEnabled,
                out logToConsole,
                out logLevel,
                out m_perfWarnings,
                out m_correctnessWarnings,
                out m_safeHandleStackTraces
            );
                ...
                try
                {
                    ...
                    if (facilityValue > 0)
                    {
                        for (int i = 0; i < switches.Length; i++)
                        {
                            ...
                            {
                                LogSwitch L = new LogSwitch(
                                    switches[i].name,
                                    switches[i].name,
                                    System.Diagnostics.Log.GlobalSwitch
                                );
