...
namespace System.Threading
{
    ...
    // cref="T:System.Threading.CancellationToken">CancellationToken</see>.
    /// </summary>
    /// <remarks>
            ...
            IDisposable
    {
        ...
        internal bool TryDeregister()
        {
            ...
            // It is also possible there is info in the array but it doesn't match our current registration's
            // callback info.
        ...
        public void Dispose()
        {
            ...
            //
            //    2. if deregistration failed, and we are on a different thread, then the callback may be
            ...
            if (callbackInfo != null)
            {
                ...
                if (
                    ...
                    !tokenSource.IsCancellationCompleted
                    && //running callbacks hasn't finished
                    !deregisterOccured
                    && //deregistration failed (ie the callback is missing from the list)
                    tokenSource.ThreadIDExecutingCallbacks != Thread.CurrentThread.ManagedThreadId
                ) //the executingThreadID is not this threadID.
                {
                    // Callback execution is in progress, the executing thread is different to us and has taken the
        ...
        // cref="T:System.Threading.CancellationTokenRegistration">CancellationTokenRegistration</see> instance
        // is equal to the
        ...
        {
            return (
                (obj is CancellationTokenRegistration) && Equals((CancellationTokenRegistration)obj)
            );
        }

        /// <summary>
        /// Determines whether the current <see
        // cref="T:System.Threading.CancellationToken">CancellationToken</see> instance is equal to the
        /// specified <see cref="T:System.Object"/>.
        /// </summary>
        /// <param name="other">The other <see
        // cref="T:System.Threading.CancellationTokenRegistration">CancellationTokenRegistration</see> to which
        // to compare this instance.</param>
        ...
        public override int GetHashCode()
        {
            if (m_registrationInfo.Source != null)
                return m_registrationInfo.Source.GetHashCode()
                    ^ m_registrationInfo.Index.GetHashCode();
