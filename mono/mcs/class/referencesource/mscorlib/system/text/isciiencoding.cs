...
namespace System.Text
{
    ...
    internal class ISCIIEncoding : EncodingNLS, ISerializable
    {
            ...
            : base(codePage)
        {
            ...
            Contract.Assert(
                defaultCodePage >= CodeDevanagari && defaultCodePage <= CodePunjabi,
            ...
            if (defaultCodePage < CodeDevanagari || defaultCodePage > CodePunjabi)
                throw new ArgumentException(
                    Environment.GetResourceString("Argument_CodepageNotSupported", codePage),
                    "codePage"
                );
        }
        ...
        )
        {
            ...
            while (buffer.MoreData)
            {
                ...
                if (indicScript != currentCodePage)
                {
                    ...
                    Contract.Assert(
                        currentCodePage >= CodeDevanagari && currentCodePage <= CodePunjabi,
                        "[ISCIIEncoding.GetBytes]Code page ("
                            + currentCodePage
                            + " shouldn't appear in ISCII from Unicode table!"
                    );
                }
                ...
                if (indicTwoBytes != 0)
                {
                    ...
                    Contract.Assert(
                        (indicTwoBytes >> 12) > 0 && (indicTwoBytes >> 12) <= 3,
                        "[ISCIIEncoding.GetBytes]Expected indicTwoBytes from 1-3, not "
                            + (indicTwoBytes >> 12)
                    );
        ...
        )
        {
            ...
            Contract.Assert(
                currentCodePage >= CodeDevanagari && currentCodePage <= CodePunjabi,
                "[ISCIIEncoding.GetChars]Decoder code page must be >= Devanagari and <= Punjabi, not "
                    + currentCodePage
            );
            ...
            while (buffer.MoreData)
            {
                ...
                if (bLastSpecial)
                {
                    ...
                    else
                    {
                        ...
                        Contract.Assert(
                            cLastCharForNextNukta > 0 && cLastCharForNoNextNukta > 0,
                ...
                Contract.Assert(
                    currentCodePage == CodeDevanagari && b == DevenagariExt,
                    String.Format(
                        CultureInfo.InvariantCulture,
        ...
        internal class ISCIIEncoder : EncoderNLS
        {
            ...
            internal override bool HasState
            {
                ...
                {
                    return (
                        this.charLeftOver != (char)0 || this.currentCodePage != this.defaultCodePage
                    );
                }
            }
        }
            ...
            internal override bool HasState
            {
                get
                {
                    return (
                        ...
                        || this.bLastATR
                        || this.bLastDevenagariStressAbbr
                    );
                }
            }
        }
