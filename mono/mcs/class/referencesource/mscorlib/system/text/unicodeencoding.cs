...
namespace System.Text
{
    ...
    public class UnicodeEncoding : Encoding
    {
        ...
        static readonly ulong highLowPatternMask = (
            (ulong)0xd800d800d800d800
            | (BitConverter.IsLittleEndian ? (ulong)0x0400000004000000 : (ulong)0x0000040000000400)
        );
                        ...
                        while (longChars < longEnd)
                        {
                            ...
                            if ((0x8000800080008000 & *longChars) != 0)
                            {
                                ...
                                )
                                {
                                    ...
                                    if (
                                        ((0xfc00fc00fc00fc00 & *longChars) ^ highLowPatternMask)
                                        != 0
                                    )
                                    {
...
#else
                        (unchecked((int)chars) & 3) != (unchecked((int)bytes) & 3)
                        && // Only do this if chars & bytes are out of line, otherwise faster loop'll be faster next time
#endif // WIN64
        ...
        )
        {
            ...
            if (decoder == null || decoder.MustFlush)
            {
                ...
                if (lastByte >= 0)
                {
                    ...
                    // No hanging odd bytes allowed if must flush
                    if (
                        !fallbackBuffer.InternalFallback(
                            new byte[] { unchecked((byte)lastByte) },
                            bytes,
                            ref chars
                        )
            ...
            {
                Contract.Assert(
                    (decoder.MustFlush == false) || ((lastChar == (char)0) && (lastByte == -1)),
        ...
        private class Decoder : System.Text.DecoderNLS, ISerializable
        {
            ...
            {
                get { return (this.lastByte != -1 || this.lastChar != '\0'); }
            }
        }
    }
}
