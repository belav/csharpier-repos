...
*/
                ...
                if (pSrc >= pEnd)
                {
                    ...
                    else
                    {
                        ...
                        if (fallbackBuffer != null && fallbackBuffer.bFallingBack)
                        {
                            Contract.Assert(
                                ch >= 0xD800 && ch <= 0xDBFF,
                                "[UTF8Encoding.GetBytes]expected high surrogate, not 0x"
        ...
        internal override unsafe int GetCharCount(byte* bytes, int count, DecoderNLS baseDecoder)
        {
            ...
            for (; ; )
            {
                ...
                {
                    Contract.Assert(
                        (ch & (SupplimentarySeq | ThreeByteSeq)) != 0,
                    ...
                    else
                    {
                        ...
                        ) // illegal individually encoded surrogate
                        {
                            goto InvalidByteSequence;
                        }
                    }
                ...
                if (ch > 0x7F)
                {
                    ...
                    if ((ch & 0x20) != 0)
                    {
                        if ((ch & 0x10) != 0)
                        {
                            ...
                            ch |=
                                (FinalByte >> 3 * 6)
                                | // Final byte is 3 more bytes from now
                                (1 << 30)
                                | // If it dies on next byte we'll need an extra char
                                (3 << (30 - 2 * 6))
                                | // If it dies on last byte we'll need to subtract a char
                                (SupplimentarySeq)
                                | (SupplimentarySeq >> 6)
                                | (SupplimentarySeq >> 2 * 6)
                                | (SupplimentarySeq >> 3 * 6);
            ...
            // (don't have to check m_throwOnOverflow for count)
            Contract.Assert(
                fallback == null || fallback.Remaining == 0,
        ...
        )
        {
            ...
            for (; ; )
            {
                ...
                if (pTarget >= pAllocatedBufferEnd)
                {
                    ...
                    if (ch > 0x7f)
                    {
                        ...
                        {
                            if (
                                ch >= CharUnicodeInfo.LOW_SURROGATE_START
            ...
            if (baseDecoder != null)
            {
                ...
                Contract.Assert(
                    !baseDecoder.MustFlush || ch == 0 || !baseDecoder.m_throwOnOverflow,
        ...
        internal class UTF8Encoder : EncoderNLS, ISerializable
        {
            ...
            internal UTF8Encoder(SerializationInfo info, StreamingContext context)
            {
                ...
                try
                {
                    this.m_fallback = (EncoderFallback)
                        info.GetValue("m_fallback", typeof(EncoderFallback));
                }
            ...
            {
                get { return (this.surrogateChar != 0); }
            }
        }
