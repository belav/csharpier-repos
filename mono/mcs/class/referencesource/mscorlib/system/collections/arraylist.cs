    ...
    public class ArrayList : IList, ICloneable
    {
        ...
        public ArrayList(int capacity)
        {
            if (capacity < 0)
                throw new ArgumentOutOfRangeException(
                    "capacity",
                    Environment.GetResourceString("ArgumentOutOfRange_MustBeNonNegNum", "capacity")
                );
        ...
        public ArrayList(ICollection c)
        {
            if (c == null)
                throw new ArgumentNullException(
                    "c",
                    Environment.GetResourceString("ArgumentNull_Collection")
                );
            Contract.EndContractBlock();
        ...
        private void EnsureCapacity(int min)
        {
            if (_items.Length < min)
            {
                ...
                if ((uint)newCapacity > Array_ReferenceSources.MaxArrayLength)
                    newCapacity = Array_ReferenceSources.MaxArrayLength;
                if (newCapacity < min)
                    newCapacity = min;
        ...
        public virtual void Insert(int index, Object value)
        {
            ...
            if (index < 0 || index > _size)
                throw new ArgumentOutOfRangeException(
                    "index",
                    Environment.GetResourceString("ArgumentOutOfRange_ArrayListInsert")
                );
        ...
        public virtual int LastIndexOf(Object value, int startIndex, int count)
        {
            ...
            if (startIndex >= _size || count > startIndex + 1)
                throw new ArgumentOutOfRangeException(
                    (startIndex >= _size ? "startIndex" : "count"),
                    Environment.GetResourceString("ArgumentOutOfRange_BiggerThanCollection")
                );
        ...
        public virtual void Remove(Object obj)
        {
            ...
            BCLDebug.Correctness(
                index >= 0 || !(obj is Int32),
                "You passed an Int32 to Remove that wasn't in the ArrayList."
                    + Environment.NewLine
                    + "Did you mean RemoveAt?  int: "
                    + obj
                    + "  Count: "
                    + Count
            );
        ...
        private class ReadOnlyList : IList
        {
            ...
            public virtual Object this[int index]
            {
                ...
                {
                    throw new NotSupportedException(
                        Environment.GetResourceString("NotSupported_ReadOnlyCollection")
                    );
                }
            }
        ...
        private sealed class ArrayListEnumerator : IEnumerator, ICloneable
        {
            ...
            public bool MoveNext()
            {
                if (version != list._version)
                    throw new InvalidOperationException(
                        Environment.GetResourceString(ResId.InvalidOperation_EnumFailedVersion)
                    );
                if (index < endIndex)
                {
        ...
        private class Range : ArrayList
        {
            ...
            private void InternalUpdateRange()
            {
                if (_baseVersion != _baseList._version)
                    throw new InvalidOperationException(
                        Environment.GetResourceString("InvalidOperation_UnderlyingArrayListChanged")
                    );
            }
            ...
            public override bool Contains(Object item)
            {
                ...
                else
                {
                    for (int i = 0; i < _baseSize; i++)
                        if (
                            _baseList[_baseIndex + i] != null
                            && _baseList[_baseIndex + i].Equals(item)
                        )
                            return true;
                    return false;
                }
