    ...
    public abstract class IsolatedStoragePermission : CodeAccessPermission, IUnrestrictedPermission
    {
        ...
        internal SecurityElement ToXml(String permName)
        {
            ...
            if (!IsUnrestricted())
            {
                esd.AddAttribute(
                    "Allowed",
                    Enum.GetName(typeof(IsolatedStorageContainment), m_allowed)
                );
                if (m_userQuota > 0)
                {
                    esd.AddAttribute(
                ...
                if (m_machineQuota > 0)
                {
                    esd.AddAttribute(
                        _strMachineQuota,
                        (m_machineQuota).ToString(CultureInfo.InvariantCulture)
                    );
                }
                if (m_expirationDays > 0)
                {
                    esd.AddAttribute(
                        _strExpiry,
                        (m_expirationDays).ToString(CultureInfo.InvariantCulture)
                    );
                }
                if (m_permanentData)
                {
        ...
        public override void FromXml(SecurityElement esd)
        {
            ...
            else
            {
                ...
                if (allowed != null)
                    m_allowed = (IsolatedStorageContainment)
                        Enum.Parse(typeof(IsolatedStorageContainment), allowed);
            }
            ...
            else
            {
                ...
                m_machineQuota =
                    param != null ? Int64.Parse(param, CultureInfo.InvariantCulture) : 0;
