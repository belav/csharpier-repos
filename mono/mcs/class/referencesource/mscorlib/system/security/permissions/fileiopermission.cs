...
namespace System.Security.Permissions
{
    ...
    using System.Security.AccessControl;using System.Security.Util;using System.IO;using System.Collections;using System.Globalization;using System.Runtime.Serialization;using System.Runtime.Versioning;using System.Diagnostics.Contracts;
            ...
            IBuiltInPermission
    {
        ...
        )
        {
            ...
            ArrayList pathArrayList = StringExpressionSet.CreateListFromExpressions(
                pathList,
                needFullPath
            );
        ...
        private static bool AccessIsSet(
            FileIOPermissionAccess access,
            FileIOPermissionAccess question
        )
        {
            ...
            if (operand.IsUnrestricted())
                ...
                return (
                    ...
                    && (
                        ...
                        || this.m_pathDiscovery.IsSubsetOf(operand.m_pathDiscovery)
                    )
        ...
        public override IPermission Intersect(IPermission target)
        {
            ...
            FileIOAccess intersectRead =
                this.m_read == null ? null : this.m_read.Intersect(operand.m_read);
            FileIOAccess intersectWrite =
                this.m_write == null ? null : this.m_write.Intersect(operand.m_write);
            FileIOAccess intersectAppend =
                this.m_append == null ? null : this.m_append.Intersect(operand.m_append);
            FileIOAccess intersectPathDiscovery =
                this.m_pathDiscovery == null
                    ? null
                    : this.m_pathDiscovery.Intersect(operand.m_pathDiscovery);
            FileIOAccess intersectViewAcl =
                this.m_viewAcl == null ? null : this.m_viewAcl.Intersect(operand.m_viewAcl);
            FileIOAccess intersectChangeAcl =
                this.m_changeAcl == null ? null : this.m_changeAcl.Intersect(operand.m_changeAcl);
        ...
        public override IPermission Union(IPermission other)
        {
            ...
            FileIOAccess unionRead =
                this.m_read == null ? operand.m_read : this.m_read.Union(operand.m_read);
            FileIOAccess unionWrite =
                this.m_write == null ? operand.m_write : this.m_write.Union(operand.m_write);
            FileIOAccess unionAppend =
                this.m_append == null ? operand.m_append : this.m_append.Union(operand.m_append);
            FileIOAccess unionPathDiscovery =
                this.m_pathDiscovery == null
                    ? operand.m_pathDiscovery
                    : this.m_pathDiscovery.Union(operand.m_pathDiscovery);
            FileIOAccess unionViewAcl =
                this.m_viewAcl == null
                    ? operand.m_viewAcl
                    : this.m_viewAcl.Union(operand.m_viewAcl);
            FileIOAccess unionChangeAcl =
                this.m_changeAcl == null
                    ? operand.m_changeAcl
                    : this.m_changeAcl.Union(operand.m_changeAcl);
        ...
        public override SecurityElement ToXml()
        {
            ...
            if (!IsUnrestricted())
            {
                ...
                if (this.m_pathDiscovery != null && !this.m_pathDiscovery.IsEmpty())
                {
                    esd.AddAttribute(
                        "PathDiscovery",
                        SecurityElement.Escape(m_pathDiscovery.ToString())
                    );
                }
    ...
    internal sealed class FileIOAccess
    {
        ...
        public FileIOAccess(String value)
        {
            if (value == null)
            {
                ...
                value.Length >= m_strAllFiles.Length
                && String.Compare(m_strAllFiles, value, StringComparison.Ordinal) == 0
            )
            {
                m_set = new StringExpressionSet(m_ignoreCase, true);
                m_allFiles = true;
                ...
                value.Length >= m_strAllLocalFiles.Length
                && String.Compare(
                    m_strAllLocalFiles,
                    0,
                    value,
                    0,
                    m_strAllLocalFiles.Length,
                    StringComparison.Ordinal
                ) == 0
            )
            {
                m_set = new StringExpressionSet(
                    m_ignoreCase,
                    value.Substring(m_strAllLocalFiles.Length),
                    true
                );
                m_allFiles = false;
        ...
        public FileIOAccess Union(FileIOAccess operand)
        {
            ...
            return new FileIOAccess(
                this.m_set.Union(operand.m_set),
                false,
            ...
            if (this.m_allLocalFiles)
            {
                ...
                if (expressions != null)
                {
                    for (int i = 0; i < expressions.Length; ++i)
                    {
                        ...
                        {
                            intersectionSet.AddExpressions(
                                new String[] { expressions[i] },
                                true,
                                false
                            );
                        }
        ...
        public bool IsSubsetOf(FileIOAccess operand)
        {
            ...
            if (
                !(
                    (m_pathDiscovery && this.m_set.IsSubsetOfPathDiscovery(operand.m_set))
                    || this.m_set.IsSubsetOf(operand.m_set)
                )
            )
            {
                if (operand.m_allLocalFiles)
        ...
        public override bool Equals(Object obj)
        {
            ...
            if (m_pathDiscovery)
            {
                ...
                if (
                    ...
                    && operand.m_set.IsSubsetOf(m_set)
                ) // Watch Out: This calls StringExpressionSet.IsSubsetOf, unlike below
                    return true;
                return false;
            }
            else
