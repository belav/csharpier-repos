...
namespace System.Security
{
    ...
    public sealed class SecurityElement : ISecurityElementFactory
    {
            ...
            if (!IsValidTag(tag))
                throw new ArgumentException(
                    String.Format(
                        ...
                        Environment.GetResourceString("Argument_InvalidElementTag"),
                        tag
                    )
                );
            Contract.EndContractBlock();
        ...
        public SecurityElement(String tag, String text)
        {
            ...
            if (text != null && !IsValidText(text))
                throw new ArgumentException(
                    String.Format(
                        CultureInfo.CurrentCulture,
                        Environment.GetResourceString("Argument_InvalidElementText"),
                        text
                    )
                );
            Contract.EndContractBlock();
            ...
            set
            {
                ...
                else
                {
                    ...
                    System.Collections.IDictionaryEnumerator enumerator =
                        (System.Collections.IDictionaryEnumerator)value.GetEnumerator();
                    ...
                    while (enumerator.MoveNext())
                    {
                        ...
                        if (!IsValidAttributeName(attrName))
                            throw new ArgumentException(
                                String.Format(
                                    CultureInfo.CurrentCulture,
                                    Environment.GetResourceString("Argument_InvalidElementName"),
                        ...
                        if (!IsValidAttributeValue(attrValue))
                            throw new ArgumentException(
                                String.Format(
                                    CultureInfo.CurrentCulture,
                                    Environment.GetResourceString("Argument_InvalidElementValue"),
                ...
                if (value != null)
                {
                    ...
                    while (enumerator.MoveNext())
                    {
                        if (enumerator.Current == null)
                            throw new ArgumentException(
                                Environment.GetResourceString("ArgumentNull_Child")
                            );
                    }
                }
            ...
            else
            {
                ...
                for (int i = 0; i < iMax; i += 2)
                {
                    ...
                    if (String.Equals(strAttrName, name))
                        throw new ArgumentException(
                            Environment.GetResourceString("Argument_AttributeNamesMustBeUnique")
                        );
                }
            }
        ...
        {
            IPermission ip = XMLUtil.CreatePermission(
                this,
        ...
        internal String SearchForTextOfLocalName(String strLocalName)
        {
            ...
            if (m_strTag == null)
                return null;
            if (
                m_strTag.Equals(strLocalName)
                || m_strTag.EndsWith(":" + strLocalName, StringComparison.Ordinal)
            )
