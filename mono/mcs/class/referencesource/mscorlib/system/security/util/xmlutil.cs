    ...
    internal static class XMLUtil
    {
        ...
        public static void AddClassAttribute(SecurityElement element, Type type, String typename)
        {
            ...
            Contract.Assert(
                ...
                "Incorrect class name passed! Was : " + typename + " Shoule be: " + type.FullName
            );
            element.AddAttribute(
                "class",
                typename + ", " + type.Module.Assembly.FullName.Replace('\"', '\'')
            );
        }
        ...
        private static bool ParseElementForObjectCreation(
            SecurityElement el,
        ...
        )
        {
            if (el == null || !(el.Tag.Equals("Permission") || el.Tag.Equals("IPermission")))
                throw new ArgumentException(
                    String.Format(
                        null,
                        Environment.GetResourceString("Argument_WrongElementType"),
                        "<Permission>"
                    )
                );
            ...
            if (!(typeof(IPermission).IsAssignableFrom(permClass)))
                throw new ArgumentException(
                    Environment.GetResourceString("Argument_NotAPermissionType")
                );
...
#pragma warning disable 618 // CodeGroups are obsolete
        ...
        public static CodeGroup CreateCodeGroup(SecurityElement el)
        {
            ...
            if (!(typeof(CodeGroup).IsAssignableFrom(groupClass)))
                throw new ArgumentException(
                    Environment.GetResourceString("Argument_NotACodeGroupType")
                );

            group = (CodeGroup)Activator.CreateInstance(groupClass, true);

            Contract.Assert(
                groupClass.Module.Assembly != Assembly.GetExecutingAssembly(),
...
#pragma warning restore 618
        ...
        internal static IMembershipCondition CreateMembershipCondition(SecurityElement el)
        {
            ...
            if (!(typeof(IMembershipCondition).IsAssignableFrom(condClass)))
                throw new ArgumentException(
                    Environment.GetResourceString("Argument_NotAMembershipCondition")
                );
            ...
            if (className == null)
            {
                if (ignoreTypeLoadFailures)
                    ...
                    throw new ArgumentException(
                        String.Format(
                            null,
                            Environment.GetResourceString("Argument_InvalidXMLMissingAttr"),
                            "class"
                        )
                    );
            }
        ...
        public static bool IsUnrestricted(SecurityElement el)
        {
            ...
            return sUnrestricted.Equals("true")
                || sUnrestricted.Equals("TRUE")
                || sUnrestricted.Equals("True");
        }
