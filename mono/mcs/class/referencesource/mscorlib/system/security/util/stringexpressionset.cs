...
namespace System.Security.Util
{
    ...
    internal class StringExpressionSet
    {
        ...
        public StringExpressionSet Intersect(StringExpressionSet ses)
        {
            ...
            for (int this_index = 0; this_index < this.m_list.Count; ++this_index)
            {
                for (int ses_index = 0; ses_index < ses.m_list.Count; ++ses_index)
                {
                    if (
                        StringSubsetString(
                            (String)this.m_list[this_index],
                            (String)ses.m_list[ses_index],
                            m_ignoreCase
                        )
                    )
                    {
                        ...
                        intersectSet.AddSingleExpressionNoDuplicates(
                            (String)this.m_list[this_index]
                        );
                    }
                    else if (
        ...
        [SecurityCritical]
        private bool StringSubsetStringExpression(
            String left,
            StringExpressionSet right,
            bool ignoreCase
        )
        {
        ...
        [SecurityCritical]
        private static bool StringSubsetStringExpressionPathDiscovery(
            String left,
            StringExpressionSet right,
            bool ignoreCase
        )
        ...
        protected static bool StringSubsetStringPathDiscovery(
            String left,
            String right,
            bool ignoreCase
        )
        ...
        protected void Reduce()
        {
            ...
            for (int i = 0; i < this.m_list.Count - 1; i++)
            {
                ...
                while (j < this.m_list.Count)
                {
                    if (
                        StringSubsetString(
                            (String)this.m_list[j],
                            (String)this.m_list[i],
                            m_ignoreCase
                        )
                    )
                    {
