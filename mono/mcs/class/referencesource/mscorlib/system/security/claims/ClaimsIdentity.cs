...
namespace System.Security.Claims
{
    ...
    public class ClaimsIdentity : IIdentity
    {
            ...
            if (claimsIdentity != null)
            {
                ...
                if (claimsIdentity.m_userSerializationData != null)
                {
                    m_userSerializationData =
                        claimsIdentity.m_userSerializationData.Clone() as byte[];
                }
            }
            else
            {
        ...
        public virtual void RemoveClaim(Claim claim)
        {
            ...
            {
                throw new InvalidOperationException(
                    Environment.GetResourceString("InvalidOperation_ClaimCannotBeRemoved", claim)
                );
            }
        }
        ...
        private void OnDeserializedMethod(StreamingContext context)
        {
            ...
            m_nameType = string.IsNullOrEmpty(m_serializedNameType)
                ? DefaultNameClaimType
                : m_serializedNameType;
            m_roleType = string.IsNullOrEmpty(m_serializedRoleType)
            ...
            if (!string.IsNullOrEmpty(serializedClaims))
            {
                using (
                    MemoryStream stream = new MemoryStream(
                        Convert.FromBase64String(serializedClaims)
                    )
                )
                {
                    m_instanceClaims =
                        (List<Claim>)(new BinaryFormatter()).Deserialize(stream, null, false);
                    for (int i = 0; i < m_instanceClaims.Count; i++)
                    {
            ...
            while (enumerator.MoveNext())
            {
                switch (enumerator.Name)
                {
                    ...
                    case ActorKey:
                        using (
                            MemoryStream stream = new MemoryStream(
                                Convert.FromBase64String(info.GetString(ActorKey))
                            )
                        )
                        {
                    ...
                    case BootstrapContextKey:
                        using (
                            MemoryStream ms = new MemoryStream(
                                Convert.FromBase64String(info.GetString(BootstrapContextKey))
                            )
                        )
                        {
