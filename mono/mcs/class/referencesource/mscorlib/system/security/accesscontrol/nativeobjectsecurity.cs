    ...
    public abstract class NativeObjectSecurity : CommonObjectSecurity
    {
                ...
                if (exception == null)
                {
                    ...
                    else if (error == Win32Native.ERROR_INVALID_OWNER)
                    {
                        exception = new InvalidOperationException(
                            Environment.GetResourceString("AccessControl_InvalidOwner")
                        );
                    }
                    else if (error == Win32Native.ERROR_INVALID_PRIMARY_GROUP)
                    {
                        exception = new InvalidOperationException(
                            Environment.GetResourceString("AccessControl_InvalidGroup")
                        );
                    }
                    else if (error == Win32Native.ERROR_INVALID_PARAMETER)
                    {
                        exception = new InvalidOperationException(
                            Environment.GetResourceString("AccessControl_UnexpectedError", error)
                        );
                    }
        ...
        )
        {
            ...
            try
            {
                ...
                if (
                    (includeSections & AccessControlSections.Owner) != 0
                    && _securityDescriptor.Owner != null
                )
                {
                ...
                if (
                    (includeSections & AccessControlSections.Group) != 0
                    && _securityDescriptor.Group != null
                )
                {
                ...
                if ((includeSections & AccessControlSections.Audit) != 0)
                {
                    ...
                    if (
                        _securityDescriptor.IsSystemAclPresent
                ...
                if (
                    ...
                    && _securityDescriptor.IsDiscretionaryAclPresent
                )
                {
