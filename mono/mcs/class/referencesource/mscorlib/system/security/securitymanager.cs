    ...
    public static class SecurityManager
    {
        ...
        public static PermissionSet GetStandardSandbox(Evidence evidence)
        {
            ...
            else if (zone.SecurityZone == SecurityZone.Intranet)
            {
                ...
                PolicyStatement webPolicy = new NetCodeGroup(new AllMembershipCondition()).Resolve(
                    evidence
                );
                PolicyStatement filePolicy = new FileCodeGroup(
                ...
                zone.SecurityZone == SecurityZone.Internet
        ...
        public static PolicyLevel LoadPolicyLevelFromFile(string path, PolicyLevelType type)
        {
            ...
            if (!File.InternalExists(path))
                throw new ArgumentException(
                    Environment.GetResourceString("Argument_PolicyFileDoesNotExist")
                );
            ...
            if (elRoot == null)
                throw new ArgumentException(
                    String.Format(
                        ...
                        Environment.GetResourceString("Policy_BadXml"),
                        "configuration"
                    )
                );
        ...
        private static PermissionSet ResolveCasPolicy(
            Evidence evidence,
            PermissionSet reqdPset,
            PermissionSet optPset,
            PermissionSet denyPset,
        ...
        )
        {
            ...
            PermissionSet granted = ResolvePolicy(
                evidence,
                reqdPset,
                optPset,
                denyPset,
        ...
        )
        {
            ...
            if (executionSecurityPermission == null)
                executionSecurityPermission = new SecurityPermission(
            ...
            {
                if (
                    !allowed.Contains(executionSecurityPermission)
            ...
            if (reqdPset != null && !reqdPset.IsSubsetOf(allowed))
            {
                BCLDebug.Assert(
                    ...
                    "Evaluating assembly level declarative security without legacy CAS policy enabled"
                );
                throw new PolicyException(
                    Environment.GetResourceString("Policy_NoRequiredPermission"),
        ...
        private static int[][] s_BuiltInPermissionIndexMap =
        {
            new int[]
            {
                BuiltInPermissionIndex.EnvironmentPermissionIndex,
                (int)PermissionType.EnvironmentPermission,
            },
            new int[]
            {
                BuiltInPermissionIndex.FileDialogPermissionIndex,
                (int)PermissionType.FileDialogPermission,
            },
            new int[]
            {
                BuiltInPermissionIndex.FileIOPermissionIndex,
                (int)PermissionType.FileIOPermission,
            },
            new int[]
            {
                BuiltInPermissionIndex.ReflectionPermissionIndex,
                (int)PermissionType.ReflectionPermission,
            },
            new int[]
            {
                BuiltInPermissionIndex.SecurityPermissionIndex,
                (int)PermissionType.SecurityPermission,
            },
            new int[]
            {
                BuiltInPermissionIndex.UIPermissionIndex,
                (int)PermissionType.UIPermission,
            },
        };
        ...
        {
            if (
                (grantSet != null && grantSet.IsUnrestricted())
...
#pragma warning restore 618
                ...
                ReflectionPermissionFlag reflectionPermissionFlags =
                ...
                if (grantSet != null)
                {
                    ...
                    {
                        securityPermission =
                            grantSet.GetPermission(BuiltInPermissionIndex.SecurityPermissionIndex)
                            as SecurityPermission;
                        if (securityPermission != null)
                            securityPermissionFlags = securityPermission.Flags;
                        reflectionPermission =
                            grantSet.GetPermission(BuiltInPermissionIndex.ReflectionPermissionIndex)
                            as ReflectionPermission;
                        if (reflectionPermission != null)
                            reflectionPermissionFlags = reflectionPermission.Flags;
                        for (int i = 0; i < specialPermissions.Length; i++)
                        {
                            specialPermissions[i] =
                                grantSet.GetPermission(s_BuiltInPermissionIndexMap[i][0])
                ...
                if (deniedSet != null)
                {
                    ...
                    {
                        securityPermission =
                            deniedSet.GetPermission(BuiltInPermissionIndex.SecurityPermissionIndex)
                            as SecurityPermission;
                        if (securityPermission != null)
                            securityPermissionFlags &= ~securityPermission.Flags;
                        reflectionPermission =
                            deniedSet.GetPermission(
                                BuiltInPermissionIndex.ReflectionPermissionIndex
                            ) as ReflectionPermission;
                        if (reflectionPermission != null)
                            reflectionPermissionFlags &= ~reflectionPermission.Flags;
                        for (int i = 0; i < s_BuiltInPermissionIndexMap.Length; i++)
                        {
                            CodeAccessPermission deniedSpecialPermission =
                                deniedSet.GetPermission(s_BuiltInPermissionIndexMap[i][0])
                                as CodeAccessPermission;
                            if (
                                deniedSpecialPermission != null
                                && !deniedSpecialPermission.IsSubsetOf(null)
                            )
                ...
                if (flags != -1)
                {
                    for (int i = 0; i < specialPermissions.Length; i++)
                    {
                        if (
                            specialPermissions[i] != null
                            && ((IUnrestrictedPermission)specialPermissions[i]).IsUnrestricted()
                        )
...
#pragma warning disable 618
        private static int MapToSpecialFlags(
            SecurityPermissionFlag securityPermissionFlags,
            ReflectionPermissionFlag reflectionPermissionFlags
        )
        {
#pragma warning restore 618
...
#pragma warning disable 618
            if (
                (securityPermissionFlags & SecurityPermissionFlag.UnmanagedCode)
                == SecurityPermissionFlag.UnmanagedCode
            )
#pragma warning restore 618
                flags |= (1 << (int)PermissionType.SecurityUnmngdCodeAccess);
            if (
                (securityPermissionFlags & SecurityPermissionFlag.SkipVerification)
                == SecurityPermissionFlag.SkipVerification
            )
                flags |= (1 << (int)PermissionType.SecuritySkipVerification);
            if (
                (securityPermissionFlags & SecurityPermissionFlag.Assertion)
                == SecurityPermissionFlag.Assertion
            )
                flags |= (1 << (int)PermissionType.SecurityAssert);
            if (
                (securityPermissionFlags & SecurityPermissionFlag.SerializationFormatter)
                == SecurityPermissionFlag.SerializationFormatter
            )
                flags |= (1 << (int)PermissionType.SecuritySerialization);
            if (
                (securityPermissionFlags & SecurityPermissionFlag.BindingRedirects)
                == SecurityPermissionFlag.BindingRedirects
            )
                flags |= (1 << (int)PermissionType.SecurityBindingRedirects);
            if (
                (securityPermissionFlags & SecurityPermissionFlag.ControlEvidence)
                == SecurityPermissionFlag.ControlEvidence
            )
                flags |= (1 << (int)PermissionType.SecurityControlEvidence);
            if (
                (securityPermissionFlags & SecurityPermissionFlag.ControlPrincipal)
                == SecurityPermissionFlag.ControlPrincipal
            )
                flags |= (1 << (int)PermissionType.SecurityControlPrincipal);

            if (
                (reflectionPermissionFlags & ReflectionPermissionFlag.RestrictedMemberAccess)
                == ReflectionPermissionFlag.RestrictedMemberAccess
            )
                flags |= (1 << (int)PermissionType.ReflectionRestrictedMemberAccess);
            if (
                (reflectionPermissionFlags & ReflectionPermissionFlag.MemberAccess)
        ...
        internal static extern void GetGrantedPermissions(
            ObjectHandleOnStack retGranted,
            ObjectHandleOnStack retDenied,
