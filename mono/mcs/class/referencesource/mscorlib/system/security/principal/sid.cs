...
namespace System.Security.Principal
{
    ...
    public sealed class SecurityIdentifier : IdentityReference, IComparable<SecurityIdentifier>
    {
        ...
        private void CreateFromParts(IdentifierAuthority identifierAuthority, int[] subAuthorities)
        {
            ...
            if (subAuthorities.Length > MaxSubAuthorities)
            {
                throw new ArgumentOutOfRangeException(
                    ...
                    Environment.GetResourceString(
                        "IdentityReference_InvalidNumberOfSubauthorities",
                        MaxSubAuthorities
                    )
                );
            }
            ...
            for (i = 0; i < 6; i++)
            {
                _BinaryForm[2 + i] = (byte)(
                    (((ulong)this._IdentifierAuthority) >> ((5 - i) * 8)) & 0xFF
                );
            }
            ...
            for (i = 0; i < this.SubAuthorityCount; i++)
            {
                ...
                {
                    _BinaryForm[8 + 4 * i + shift] = (byte)(
                        ((ulong)_SubAuthorities[i]) >> (shift * 8)
                    );
                }
            }
        }
        ...
        public SecurityIdentifier(WellKnownSidType sidType, SecurityIdentifier domainSid)
        {
            ...
            {
                throw new ArgumentException(
                    Environment.GetResourceString("IdentityReference_CannotCreateLogonIdsSid"),
                    "sidType"
                );
            }
            Contract.EndContractBlock();
            ...
            )
            {
                if (domainSid == null)
                {
                    throw new ArgumentNullException(
                        "domainSid",
                        Environment.GetResourceString(
                            "IdentityReference_DomainSidRequired",
                            sidType
                        )
                    );
                }
                ...
                else if (ErrorCode == Win32Native.ERROR_NON_ACCOUNT_SID)
                {
                    ...
                    throw new ArgumentException(
                        Environment.GetResourceString("IdentityReference_NotAWindowsDomain"),
                        "domainSid"
                    );
                }
        ...
        )
        {
            ...
            try
            {
                ...
                someFailed = false;
                uint ReturnCode;
                ReturnCode = Win32Native.LsaLookupSids(
                    LsaHandle,
                    sourceSids.Count,
                    SidArrayPtr,
                ...
                NamesPtr.Initialize(
                    ...
                    (uint)Marshal.SizeOf(typeof(Win32Native.LSA_TRANSLATED_NAME))
                );
                ...
                if (ReturnCode == 0 || ReturnCode == Win32Native.STATUS_SOME_NOT_MAPPED)
                {
                    ...
                    {
                        Win32Native.LSA_TRUST_INFORMATION ti = (Win32Native.LSA_TRUST_INFORMATION)
                    ...
                    Win32Native.LSA_TRANSLATED_NAME[] translatedNames =
