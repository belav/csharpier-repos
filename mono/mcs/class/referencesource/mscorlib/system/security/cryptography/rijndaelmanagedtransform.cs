...
namespace System.Security.Cryptography
{
    ...
    public sealed class RijndaelManagedTransform : ICryptoTransform
    {
        ...
        private unsafe int EncryptData(
            byte[] inputBuffer,
            int inputOffset,
            int inputCount,
                        ...
                        fixed (int* TF = s_TF)
                        {
                            ...
                            for (int blockNum = 0; blockNum < iNumBlocks; ++blockNum)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        int i0 =
                                            (index >= workBaseIndex + lonelyBytes)
                                                ? padBytes[padIndex++]
                                                : inputBuffer[index++];
                                        int i1 =
                                            (index >= workBaseIndex + lonelyBytes)
                                ...
                                if (m_cipherMode == CipherMode.CFB)
                                {
                                    ...
                                    if (blockNum != iNumBlocks - 1 || padSize == 0)
                                    {
                                        for (int i = 0; i < m_Nb; ++i)
                                        {
                                            if (index >= workBaseIndex + m_inputBlockSize)
                                                break;
                                            outputBuffer[transformCount++] = (byte)(
                                                (temp[i] & 0xFF) ^ inputBuffer[index++]
                                            );
                                            if (index >= workBaseIndex + m_inputBlockSize)
                                                break;
                                            outputBuffer[transformCount++] = (byte)(
                                                (temp[i] >> 8 & 0xFF) ^ inputBuffer[index++]
                                            );
                                            if (index >= workBaseIndex + m_inputBlockSize)
                                                break;
                                            outputBuffer[transformCount++] = (byte)(
                                                (temp[i] >> 16 & 0xFF) ^ inputBuffer[index++]
                                            );
                                            if (index >= workBaseIndex + m_inputBlockSize)
                                                break;
                                            outputBuffer[transformCount++] = (byte)(
                                                (temp[i] >> 24 & 0xFF) ^ inputBuffer[index++]
                                            );
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        for (int i = 0; i < m_Nb; ++i)
                                        {
                                            if (index >= m_inputBlockSize)
                                                break;
                                            outputBuffer[transformCount++] = (byte)(
                                                (temp[i] & 0xFF) ^ tmpInputBuffer[index++]
                                            );
                                            if (index >= m_inputBlockSize)
                                                break;
                                            outputBuffer[transformCount++] = (byte)(
                                                (temp[i] >> 8 & 0xFF) ^ tmpInputBuffer[index++]
                                            );
                                            if (index >= m_inputBlockSize)
                                                break;
                                            outputBuffer[transformCount++] = (byte)(
                                                (temp[i] >> 16 & 0xFF) ^ tmpInputBuffer[index++]
                                            );
                                            if (index >= m_inputBlockSize)
                                                break;
                                            outputBuffer[transformCount++] = (byte)(
                                                (temp[i] >> 24 & 0xFF) ^ tmpInputBuffer[index++]
                                            );
                                        }
                                    }
                                    ...
                                    {
                                        m_shiftRegister[index] = m_shiftRegister[
                                            index + m_inputBlockSize
                                        ];
                                        index++;
                                    }
                                    Buffer.InternalBlockCopy(
                                        outputBuffer,
                                        blockNum * m_inputBlockSize,
                                        m_shiftRegister,
                                        m_blockSizeBytes - m_inputBlockSize,
                                        m_inputBlockSize
                                    );
                                }
                                else
        ...
        private unsafe int DecryptData(
            byte[] inputBuffer,
            int inputOffset,
            int inputCount,
            ref byte[] outputBuffer,
                                        ...
                                        fixed (int* iTF = s_iTF)
                                        {
                                            ...
                                            )
                                            {
                                                ...
                                                if (m_cipherMode == CipherMode.CFB)
                                                {
                                                    ...
                                                    index = 0;
                                                    while (
                                                        index < m_blockSizeBytes - m_inputBlockSize
                                                    )
                                                    {
                                                        m_shiftRegister[index] = m_shiftRegister[
                                                            index + m_inputBlockSize
                                            ...
                                            if (inputCount == 0)
                                                throw new CryptographicException(
                                                    Environment.GetResourceString(
                                                        "Cryptography_PKCS7_InvalidPadding"
                                                    )
                                                );
                                            padSize = outputBuffer[inputCount - 1];
                                            if (
                                                padSize > outputBuffer.Length
                                                || padSize > InputBlockSize
                                                || padSize <= 0
                                            )
                                                throw new CryptographicException(
                                                    Environment.GetResourceString(
        ...
        private void GenerateKeyExpansion(byte[] rgbKey)
        {
            ...
            for (int i = m_Nb; i < m_Nb * m_Nr; ++i)
            {
                ...
                int mul09 = key ^ mul08;
                m_decryptKeyExpansion[i] =
                    mul02 ^ mul04 ^ mul08 ^ rot3(mul02 ^ mul09) ^ rot2(mul04 ^ mul09) ^ rot1(mul09);
            }
        }

        private static int rot1(int val)
        {
            return (val << 8 & unchecked((int)0xFFFFFF00))
                | (val >> 24 & unchecked((int)0x000000FF));
        }

        private static int rot2(int val)
        {
            return (val << 16 & unchecked((int)0xFFFF0000))
                | (val >> 16 & unchecked((int)0x0000FFFF));
        }

        private static int rot3(int val)
        {
            return (val << 24 & unchecked((int)0xFF000000))
                | (val >> 8 & unchecked((int)0x00FFFFFF));
        }
        ...
        private static int MulX(int x)
        {
            ...
            return ((x & unchecked((int)0x7f7f7f7f)) << 1)
                ^ ((u - (u >> 7 & 0x01FFFFFF)) & 0x1b1b1b1b);
        }
