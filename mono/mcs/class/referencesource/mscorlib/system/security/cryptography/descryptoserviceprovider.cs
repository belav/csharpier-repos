...
namespace System.Security.Cryptography
{
    ...
    public sealed class DESCryptoServiceProvider : DES
    {
        ...
        public override ICryptoTransform CreateEncryptor(byte[] rgbKey, byte[] rgbIV)
        {
            if (IsWeakKey(rgbKey))
                throw new CryptographicException(
                    Environment.GetResourceString("Cryptography_InvalidKey_Weak"),
                    "DES"
                );
            if (IsSemiWeakKey(rgbKey))
                throw new CryptographicException(
                    Environment.GetResourceString("Cryptography_InvalidKey_SemiWeak"),
                    "DES"
                );
#if MONO
            return new DESTransform(this, true, rgbKey, rgbIV);
#else
            return _NewEncryptor(
                rgbKey,
                ModeValue,
                rgbIV,
                FeedbackSizeValue,
        ...
        private ICryptoTransform _NewEncryptor(
            ...
            byte[] rgbIV,
            int feedbackSize,
            CryptoAPITransformMode encryptMode
        )
        {
            int cArgs = 0;
