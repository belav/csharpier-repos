...
namespace System.Security.Cryptography
{
    ...
    internal static class CapiNative
    {
        ...
        internal enum AlgorithmID
        {
            ...
            RsaKeyExchange =
                (AlgorithmClass.KeyExchange | AlgorithmType.Rsa | AlgorithmSubId.RsaAny), // CALG_RSA_KEYX
        ...
        {
            // MS_ENHANCED_PROV
            internal const string MicrosoftEnhanced =
        ...
        internal static class UnsafeNativeMethods
        {
            ...
            [DllImport("advapi32", SetLastError = true)]
            [return: MarshalAs(UnmanagedType.Bool)]
            internal static extern unsafe bool CryptGenRandom(
                SafeCspHandle hProv,
                int dwLen,
                byte* pbBuffer
            );
        ...
        )
        {
            ...
            if (
                (
                    (flags & CryptAcquireContextFlags.VerifyContext)
                    == CryptAcquireContextFlags.VerifyContext
                )
                && (
                    (flags & CryptAcquireContextFlags.MachineKeyset)
                    == CryptAcquireContextFlags.MachineKeyset
                )
            )
            {
                flags &= ~CryptAcquireContextFlags.MachineKeyset;
        ...
        /// </summary>
        [SecurityCritical]
        internal static SafeCspHashHandle CreateHashAlgorithm(
        ...
        )
        {
            ...
            Contract.Assert(
                ((AlgorithmClass)algorithm & AlgorithmClass.Hash) == AlgorithmClass.Hash,
                "Invalid hash algorithm"
            );

            SafeCspHashHandle hashHandle = null;
            if (
                !UnsafeNativeMethods.CryptCreateHash(
                    cspHandle,
                    algorithm,
                    IntPtr.Zero,
                    0,
        ...
        /// </summary>
        [SecurityCritical]
        internal static unsafe void GenerateRandomBytes(
            SafeCspHandle cspHandle,
            byte[] buffer,
            int offset,
            int count
        ...
        /// </summary>
        [SecurityCritical]
        internal static int GetHashPropertyInt32(
            SafeCspHashHandle hashHandle,
            HashProperty property
        )
        {
            byte[] rawProperty = GetHashProperty(hashHandle, property);
            Contract.Assert(
                rawProperty.Length == sizeof(int) || rawProperty.Length == 0,
        ...
        internal static byte[] GetHashProperty(SafeCspHashHandle hashHandle, HashProperty property)
        {
            ...
            if (
                !UnsafeNativeMethods.CryptGetHashParam(
                    hashHandle,
                    property,
                    buffer,
                    ref bufferSize,
        ...
        internal static byte[] GetKeyProperty(SafeCspKeyHandle keyHandle, KeyProperty property)
        {
            ...
            // Figure out how big of a buffer we need to hold the property
            if (
                !UnsafeNativeMethods.CryptGetKeyParam(
                    keyHandle,
                    property,
                    buffer,
                    ref bufferSize,
        ...
        /// </summary>
        [SecurityCritical]
        internal static void SetHashProperty(
            SafeCspHashHandle hashHandle,
            HashProperty property,
            byte[] value
        )
        ...
        )
        {
            ...
            Contract.Assert(
                ((AlgorithmClass)signatureAlgorithm & AlgorithmClass.Signature)
                    == AlgorithmClass.Signature,
                "Invalid signature algorithm"
            );
            Contract.Assert(
                ((AlgorithmClass)hashAlgorithm & AlgorithmClass.Hash) == AlgorithmClass.Hash,
                "Invalid hash algorithm"
            );
            ...
            using (SafeCspHashHandle hashHandle = CreateHashAlgorithm(cspHandle, hashAlgorithm))
            {
                ...
                if (
                    UnsafeNativeMethods.CryptVerifySignature(
                        hashHandle,
                        signatureValue,
