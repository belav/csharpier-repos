...
#endif // FEATURE_MACL
    using System.Security.Cryptography.X509Certificates;using System.Security.Permissions;
#if FEATURE_IMPERSONATION
    using System.Security.Principal;
#endif // FEATURE_IMPERSONATION
    using System.Text;using System.Threading;using System.Diagnostics.Contracts;using System.Runtime.Versioning;
    ...
    internal static class Utils
    {
            ...
            get
            {
                if (_safeProvHandle == null)
                {
                    lock (InternalSyncObject)
                    {
                        if (_safeProvHandle == null)
                        {
                            _safeProvHandle = AcquireProvHandle(
            ...
            get
            {
                if (_safeDssProvHandle == null)
                {
                    lock (InternalSyncObject)
                    {
                        if (_safeDssProvHandle == null)
                        {
                            _safeDssProvHandle = CreateProvHandle(
                                new CspParameters(Constants.PROV_DSS_DH),
                                true
                            );
                        }
                    }
        ...
        )
        {
            ...
            if (hr != Constants.S_OK)
            {
                ...
                if (
                    (parameters.Flags & CspProviderFlags.UseExistingKey) != 0
                    || (
                        hr != Constants.NTE_KEYSET_NOT_DEF
                        && hr != Constants.NTE_BAD_KEYSET
                        && hr != Constants.NTE_FILENOTFOUND
                    )
                )
        ...
        [System.Security.SecurityCritical] // auto-generated
        internal static byte[] ExportCspBlobHelper(
        ...
        )
        {
            ...
            Utils.ExportCspBlob(
                safeKeyHandle,
                includePrivateParameters ? Constants.PRIVATEKEYBLOB : Constants.PUBLICKEYBLOB,
        ...
        [ResourceExposure(ResourceScope.None)]
        [ResourceConsumption(ResourceScope.Machine, ResourceScope.Machine)]
        internal static void GetKeyPairHelper(
        ...
        {
            SafeProvHandle TempFetchedProvHandle = Utils.CreateProvHandle(
                parameters,
                randomKeyContainer
            );
            ...
            byte[] algid = (byte[])
                Utils._GetKeyParameter(TempFetchedKeyHandle, Constants.CLR_ALGID);
            int dwAlgId = (algid[0] | (algid[1] << 8) | (algid[2] << 16) | (algid[3] << 24));
            if (
                (
                    keyType == CspAlgorithmType.Rsa
                    && dwAlgId != Constants.CALG_RSA_KEYX
                    && dwAlgId != Constants.CALG_RSA_SIGN
        ...
        [System.Security.SecurityCritical] // auto-generated
        internal static void ImportCspBlobHelper(
            CspAlgorithmType keyType,
            byte[] keyBlob,
            bool publicOnly,
            ref CspParameters parameters,
            bool randomKeyContainer,
            ref SafeProvHandle safeProvHandle,
            ref SafeKeyHandle safeKeyHandle
        )
        {
            ...
            if (publicOnly)
            {
                parameters.KeyNumber = Utils._ImportCspBlob(
                    keyBlob,
                    keyType == CspAlgorithmType.Dss
                        ? Utils.StaticDssProvHandle
        ...
        )
        {
            ...
            if (parameters.KeyNumber == -1)
                parameters.KeyNumber =
                    keyType == CspAlgorithmType.Dss
                        ? Constants.AT_SIGNATURE
                        : Constants.AT_KEYEXCHANGE;
            else if (
                parameters.KeyNumber == Constants.CALG_DSS_SIGN
                || parameters.KeyNumber == Constants.CALG_RSA_SIGN
            )
        ...
        private static void ValidateCspFlags(CspProviderFlags flags)
        {
            ...
            if ((flags & CspProviderFlags.UseUserProtectedKey) != 0)
            {
                ...
                if (!System.Environment.UserInteractive)
                    throw new InvalidOperationException(
                        Environment.GetResourceString("Cryptography_NotInteractive")
                    );
        ...
        [RegistryPermissionAttribute(
            ...
            Read = "HKEY_LOCAL_MACHINE\\System\\CurrentControlSet\\Control\\Lsa"
        )]
            ...
            try
            {
                ...
                )
                {
                    ...
                    if (data == null)
                    {
                        ...
                        fipsAlgorithmPolicyKey.GetValueKind("FIPSAlgorithmPolicy")
        ...
        internal static HashAlgorithm ObjToHashAlgorithm(Object hashAlg)
        {
            ...
            if (hashAlg is String)
            {
                ...
                if (hash == null)
                {
                    string oidFriendlyName = X509Utils.GetFriendlyNameFromOid(
                        (string)hashAlg,
        ...
        internal static unsafe void DWORDFromLittleEndian(uint* x, int digits, byte* block)
        {
            ...
            for (i = 0, j = 0; i < digits; i++, j += 4)
                x[i] = (uint)(
                    block[j] | (block[j + 1] << 8) | (block[j + 2] << 16) | (block[j + 3] << 24)
                );
        }
        ...
        {
            return unchecked(
                new byte[] { (byte)(i >> 24), (byte)(i >> 16), (byte)(i >> 8), (byte)i }
            );
        }
        ...
        )
        {
            ...
            if ((data.Length + 2 + 2 * cbHash) > cb)
                throw new CryptographicException(
                    String.Format(
                        null,
                        Environment.GetResourceString("Cryptography_Padding_EncDataTooBig"),
                        cb - 2 - 2 * cbHash
                    )
                );
        ...
        private static bool DoesRsaKeyOverrideSlowPath(
            Type t,
            string methodName,
            Type[] parameterTypes
        )
        ...
        private static extern void SignValue(
            ...
            byte[] hash,
            int cbHash,
            ObjectHandleOnStack retSignature
        );
        ...
        )
        {
            ...
            SignValue(
                ...
                hash,
                hash.Length,
                JitHelpers.GetObjectHandleOnStack(ref signature)
            );
            return signature;
        }
        ...
        internal static extern void _CreateCSP(
            CspParameters param,
            bool randomKeyContainer,
            ref SafeProvHandle hProv
        );
        ...
        [ResourceExposure(ResourceScope.None)]
        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        internal static extern int _ImportCspBlob(
            byte[] keyBlob,
            SafeProvHandle hProv,
            CspProviderFlags flags,
            ref SafeKeyHandle hKey
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        internal static extern int _OpenCSP(
            CspParameters param,
            uint flags,
            ref SafeProvHandle hProv
        );
