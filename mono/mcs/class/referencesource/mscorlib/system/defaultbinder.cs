...
namespace System
{
    ...
    internal partial class DefaultBinder : Binder
    {
        ...
        )
        {
            ...
            if (CurIdx == 1)
            {
                ...
                if (names != null)
                {
                    state = new BinderState(
                        (int[])paramOrder[0].Clone(),
                        args.Length,
                        paramArrayTypes[0] != null
                    );
                ...
                else
                {
                    if ((candidates[0].CallingConvention & CallingConventions.VarArgs) == 0)
                    {
                        ...
                        objs[paramArrayPos] = Array.UnsafeCreateInstance(
                            paramArrayTypes[0],
                        ...
                        Array.Copy(
                            args,
                            paramArrayPos,
                            (System.Array)objs[paramArrayPos],
                            0,
                            args.Length - paramArrayPos
                        );
                        args = objs;
            ...
            {
                state = new BinderState(
                    (int[])paramOrder[currentMin].Clone(),
                    args.Length,
                    paramArrayTypes[currentMin] != null
                );
        ...
        )
        {
            ...
            for (i = 0; i < candidates.Length; i++)
            {
                ...
                for (j = 0; j < types.Length; j++)
                {
                    ...
                    {
                        if (
                            !(types[j].UnderlyingSystemType is RuntimeType)
                            || !CanConvertPrimitive(
                                (RuntimeType)types[j].UnderlyingSystemType,
                                (RuntimeType)pCls.UnderlyingSystemType
                            )
                        )
                            break;
                    }
            ...
            for (i = 0; i < candidates.Length; i++)
            {
                if (indexes != null)
                {
                    ...
                    for (j = 0; j < indexesLength; j++)
                    {
                        ...
                        {
                            if (
                                !(indexes[j].UnderlyingSystemType is RuntimeType)
                                || !CanConvertPrimitive(
                                    (RuntimeType)indexes[j].UnderlyingSystemType,
                                    (RuntimeType)pCls.UnderlyingSystemType
                                )
                            )
                                break;
                ...
                if (j == indexesLength)
                {
                    if (returnType != null)
                    {
                        ...
                        {
                            if (
                                !(returnType.UnderlyingSystemType is RuntimeType)
                                || !CanConvertPrimitive(
                                    (RuntimeType)returnType.UnderlyingSystemType,
                                    (RuntimeType)candidates[i].PropertyType.UnderlyingSystemType
                                )
                            )
                                continue;
                        }
        ...
        public override Object ChangeType(Object value, Type type, CultureInfo cultureInfo)
        {
            throw new NotSupportedException(
                Environment.GetResourceString("NotSupported_ChangeType")
            );
        }
        ...
        public static MethodBase ExactBinding(
            MethodBase[] match,
            Type[] types,
            ParameterModifier[] modifiers
        )
