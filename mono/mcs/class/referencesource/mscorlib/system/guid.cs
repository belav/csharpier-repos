...
namespace System
{
...
#endif
    {
        ...
        public Guid(byte[] b)
        {
            ...
            if (b.Length != 16)
                throw new ArgumentException(
                    Environment.GetResourceString("Arg_GuidArrayCtor", "16")
                );
        ...
        public Guid(int a, short b, short c, byte[] d)
        {
            ...
            if (d.Length != 8)
                throw new ArgumentException(
                    Environment.GetResourceString("Arg_GuidArrayCtor", "8")
                );
            Contract.EndContractBlock();
        ...
        private struct GuidResult
        {
            ...
            {
                Contract.Assert(
                    failure != ParseFailureKind.NativeException,
        ...
        private static bool TryParseGuid(String g, GuidStyles flags, ref GuidResult result)
        {
            ...
            {
                result.SetFailure(
                    ParseFailureKind.FormatWithInnerException,
        ...
        private static bool TryParseGuidWithHexPrefix(String guidString, ref GuidResult result)
        {
            ...
            // Check for '{'
            if (
                guidString.Length <= numStart + numLen + 1
        ...
        )
        {
            ...
            if (
                requiredLength != -1
                && parsePos != null
                && (*parsePos) - currStart != requiredLength
            )
            {
        ...
        private static bool IsHexPrefix(String str, int i)
        {
            if (
                str.Length > i + 1
                && str[i] == '0'
                && (Char.ToLower(str[i + 1], CultureInfo.InvariantCulture) == 'x')
            )
                return true;
            else
                return false;
