...
namespace System.Globalization
{
    ...
    public class StringInfo
    {
        ...
        internal static int GetCurrentTextElementLen(
            String str,
            int index,
            int len,
        ...
        )
        {
            Contract.Assert(
                index >= 0 && len >= 0,
                "StringInfo.GetCurrentTextElementLen() : index = " + index + ", len = " + len
            );
            Contract.Assert(
                index < len,
                "StringInfo.GetCurrentTextElementLen() : index = " + index + ", len = " + len
            ...
            if (CharUnicodeInfo.IsCombiningCategory(ucNext))
            {
                ...
                if (
                    CharUnicodeInfo.IsCombiningCategory(ucCurrent)
                    || (ucCurrent == UnicodeCategory.Format)
                    || (ucCurrent == UnicodeCategory.Control)
                    || (ucCurrent == UnicodeCategory.OtherNotAssigned)
                    || (ucCurrent == UnicodeCategory.Surrogate)
                ) // An unpair high surrogate or low surrogate
                {
                ...
                else
                {
                    ...
                    while (index < len)
                    {
                        ucNext = CharUnicodeInfo.InternalGetUnicodeCategory(
                            str,
                            index,
                            out nextCharCount
                        );
        ...
        public static String GetNextTextElement(String str, int index)
        {
            ...
            int charLen;
            UnicodeCategory uc = CharUnicodeInfo.InternalGetUnicodeCategory(
                str,
                index,
                out charLen
            );
            return (
                str.Substring(index, GetCurrentTextElementLen(str, index, len, ref uc, ref charLen))
            );
        }
...
*/
        ...
        public static int[] ParseCombiningCharacters(String str)
        {
            ...
            int i = 0;
            int currentCharLen;
            UnicodeCategory currentCategory = CharUnicodeInfo.InternalGetUnicodeCategory(
                str,
                0,
                out currentCharLen
            );
