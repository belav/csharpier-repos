    ...
    public sealed class IdnMapping
    {
        ...
        public String GetAscii(String unicode, int index, int count)
        {
            ...
            unicode = unicode.Normalize(
                (NormalizationForm)(
                    m_bAllowUnassigned
                        ? ExtendedNormalizationForms.FormIdna
                        : ExtendedNormalizationForms.FormIdnaDisallowUnassigned
                )
            );
            ...
            {
                throw new ArgumentException(
                    Environment.GetResourceString("Argument_IdnBadLabelSize"),
                    "unicode"
                );
            }
        ...
        private String GetAsciiUsingOS(String unicode)
        {
            ...
            uint flags = (uint)(
                (AllowUnassigned ? IDN_ALLOW_UNASSIGNED : 0)
                | (UseStd3AsciiRules ? IDN_USE_STD3_ASCII_RULES : 0)
            );
            ...
            if (length == 0)
            {
                ...
                {
                    throw new ArgumentException(
                        Environment.GetResourceString("Argument_IdnIllegalName"),
                        "unicode"
                    );
                }
        ...
        {
            return (this.m_bAllowUnassigned ? 100 : 200)
                + (this.m_bUseStd3AsciiRules ? 1000 : 2000);
        }
        ...
        static void ValidateStd3(char c, bool bNextToDot)
        {
            ...
            if (
                ...
                throw new ArgumentException(
                    Environment.GetResourceString("Argument_IdnBadStd3", c),
                    "Unicode"
                );
        }
            ...
            while (iNextDot < unicode.Length)
            {
                ...
                if (eBidi == BidiCategory.RightToLeft || eBidi == BidiCategory.RightToLeftArabic)
                {
                    ...
                    )
                    {
                        ...
                        throw new ArgumentException(
                            Environment.GetResourceString("Argument_IdnBadBidi"),
                            "unicode"
                        );
                    }
                }
                ...
                for (basicCount = iAfterLastDot; basicCount < iNextDot; basicCount++)
                {
                    ...
                    Contract.Assert(
                        Char.IsLowSurrogate(unicode, basicCount) == false,
                    ...
                    if (
                        !bRightToLeft
                        && (
                            testBidi == BidiCategory.RightToLeft
                ...
                else
                {
                    ...
                    if (
                        unicode.Length - iAfterLastDot >= M_strAcePrefix.Length
                        && unicode
                            .Substring(iAfterLastDot, M_strAcePrefix.Length)
                            .Equals(M_strAcePrefix, StringComparison.OrdinalIgnoreCase)
                    )
                        throw new ArgumentException(
                            Environment.GetResourceString("Argument_IdnBadPunycode"),
                            "unicode"
                    ...
                    while (numProcessed < (iNextDot - iAfterLastDot))
                    {
                        ...
                        for (j = iAfterLastDot; j < iNextDot; j += IsSupplementary(test) ? 2 : 1)
                        {
                            ...
                            if (test == n)
                            {
                                ...
                                bias = adapt(
                                    delta,
                                    (numProcessed - numSurrogatePairs) + 1,
                                    numProcessed == numBasicCodePoints
                                );
                                delta = 0;
                                numProcessed++;
        ...
        static String punycode_decode(String ascii)
        {
            ...
            if (ascii.Length > M_defaultNameLimit - (IsDot(ascii[ascii.Length - 1]) ? 0 : 1))
                throw new ArgumentException(
                    Environment.GetResourceString(
                        "Argument_IdnBadNameSize",
                        M_defaultNameLimit - (IsDot(ascii[ascii.Length - 1]) ? 0 : 1)
                    ),
                    "ascii"
                );
            ...
            while (iNextDot < ascii.Length)
            {
                ...
                if (
                    ascii.Length < M_strAcePrefix.Length + iAfterLastDot
                    || !ascii
                        .Substring(iAfterLastDot, M_strAcePrefix.Length)
                        .Equals(M_strAcePrefix, StringComparison.OrdinalIgnoreCase)
                )
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        )
                        {
                            ...
                            output.Append(
                                (char)(
                                    ascii[copyAscii] >= 'A' && ascii[copyAscii] <= 'Z'
                                        ? ascii[copyAscii] - 'A' + 'a'
                                        : ascii[copyAscii]
                                )
                            );
                    ...
                    int asciiIndex =
                        iAfterLastDot + (numBasicCodePoints > 0 ? numBasicCodePoints + 1 : 0);
                    ...
                    while (asciiIndex < iNextDot)
                    {
                        ...
                        bias = adapt(
                            i - oldi,
                            (output.Length - iOutputAfterLastDot - numSurrogatePairs) + 1,
                            oldi == 0
                        );
                    ...
                    // Check for RTL.  If right-to-left, then 1st & last chars must be RTL
                    BidiCategory eBidi = CharUnicodeInfo.GetBidiCategory(
                        output.ToString(),
                        iOutputAfterLastDot
                    );
                    if (
                    ...
                    for (int iTest = iOutputAfterLastDot; iTest < output.Length; iTest++)
                    {
                        ...
                        if (
                            (bRightToLeft && eBidi == BidiCategory.LeftToRight)
                            || (
                                !bRightToLeft
                                && (
                                    eBidi == BidiCategory.RightToLeft
            ...
            if (output.Length > M_defaultNameLimit - (IsDot(output[output.Length - 1]) ? 0 : 1))
                throw new ArgumentException(
                    Environment.GetResourceString(
                        "Argument_IdnBadNameSize",
                        M_defaultNameLimit - (IsDot(output[output.Length - 1]) ? 0 : 1)
                    ),
                    "ascii"
                );
        ...
        */
        ...
        private static extern int IdnToAscii(
            ...
            int cchUnicodeChar,
            [System.Runtime.InteropServices.OutAttribute()] char[] lpASCIICharStr,
            int cchASCIIChar
        );
        ...
        private static extern int IdnToUnicode(
            ...
            int cchASCIIChar,
            [System.Runtime.InteropServices.OutAttribute()] char[] lpUnicodeCharStr,
            int cchUnicodeChar
        );
#endif
    }
