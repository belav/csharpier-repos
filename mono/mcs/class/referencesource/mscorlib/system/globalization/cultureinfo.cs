...
#pragma warning restore 618
        ...
        public virtual CultureInfo Parent
        {
            ...
            get
            {
                ...
                if (null == m_parent)
                {
                    ...
                    {
                        culture = CreateCultureInfoNoThrow(
                            parentName,
                            m_cultureData.UseUserOverride
                        );
                        if (culture == null)
                        {
        ...
        {
            get { return (this.m_cultureData.ILANGUAGE); }
        }
#endif
        ...
        {
            get { return this.m_cultureData.IsNeutralCulture; }
        }
...
#pragma warning restore 618
                types |= m_cultureData.IsSupplementalCustomCulture
                    ? CultureTypes.UserCustomCulture
                    : 0;
                types |= m_cultureData.IsReplacementCulture
        ...
        **Exceptions:
        ...
        internal static CultureInfo GetCultureInfoHelper(int lcid, string name, string altName)
        {
            ...
            else
            {
                ...
                if (
                    (retval.LCID == LCID_ZH_CHS_HANS && newName == "zh-hans")
        ...
        public static CultureInfo GetCultureInfoByIetfLanguageTag(string name)
        {
            ...
            if (name == "zh-CHT" || name == "zh-CHS")
            {
                throw new CultureNotFoundException(
                    ...
                    String.Format(
                        CultureInfo.CurrentCulture,
                        Environment.GetResourceString("Argument_CultureIetfNotSupported"),
                        name
                    )
                );
            }
        ...
        {
            Contract.Assert(
                localeType == LOCALE_USER_DEFAULT || localeType == LOCALE_SYSTEM_DEFAULT,
                "[CultureInfo.GetDefaultLocaleName] localeType must be LOCALE_USER_DEFAULT or LOCALE_SYSTEM_DEFAULT"
            );

            string localeName = null;
            if (
                InternalGetDefaultLocaleName(
                    localeType,
                    JitHelpers.GetStringHandleOnStack(ref localeName)
                )
            )
            {
                return localeName;
        ...
        private static extern bool InternalGetDefaultLocaleName(
            int localetype,
        ...
        private static String GetUserDefaultUILanguage()
        {
            ...
            if (
                InternalGetUserDefaultUILanguage(
                    JitHelpers.GetStringHandleOnStack(ref userDefaultUiLanguage)
                )
            )
            {
        ...
        private static String GetSystemDefaultUILanguage()
        {
            ...
            if (
                InternalGetSystemDefaultUILanguage(
                    JitHelpers.GetStringHandleOnStack(ref systemDefaultUiLanguage)
                )
            )
            {
