...
namespace System.Globalization
{
    ...
    internal partial class CalendarData
    {
        ...
        static CalendarData()
        {
            ...
            invariant.saDayNames = new String[]
            {
                "Sunday",
                "Monday",
                "Tuesday",
            ...
            invariant.saAbbrevDayNames = new String[]
            {
                "Sun",
                "Mon",
                "Tue",
            ...
            invariant.saSuperShortDayNames = new String[]
            {
                "Su",
                "Mo",
                "Tu",
            ...
            invariant.saMonthNames = new String[]
            {
                "January",
                "February",
                "March",
            ...
            invariant.saAbbrevMonthNames = new String[]
            {
                "Jan",
                "Feb",
                "Mar",
        ...
        internal CalendarData(String localeName, int calendarId, bool bUseUserOverrides)
        {
            ...
            if (!nativeGetCalendarData(this, localeName, calendarId))
            {
                Contract.Assert(
                    false,
                    "[CalendarData] nativeGetCalendarData call isn't expected to fail for calendar "
                        + calendarId
                        + " locale "
                        + localeName
                );
            ...
            if (
                this.saMonthGenitiveNames == null
                || String.IsNullOrEmpty(this.saMonthGenitiveNames[0])
            )
                this.saMonthGenitiveNames = this.saMonthNames; // Genitive month names (same as month names for invariant)
            if (
                this.saAbbrevMonthGenitiveNames == null
                || String.IsNullOrEmpty(this.saAbbrevMonthGenitiveNames[0])
            )
            ...
            if (
                this.saLeapYearMonthNames == null
                || String.IsNullOrEmpty(this.saLeapYearMonthNames[0])
            )
            ...
            switch ((CalendarId)calendarId)
            {
                ...
                case CalendarId.UMALQURA:
                    ...
                    else
                    {
                        ...
                        {
                            "\x0628\x0639\x062F \x0627\x0644\x0647\x062C\x0631\x0629",
                        };
                    }
                    break;
        ...
        [ResourceExposure(ResourceScope.None)]
        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        private static extern bool nativeGetCalendarData(
            CalendarData data,
            String localeName,
            int calendar
        );
        ...
        [ResourceExposure(ResourceScope.None)]
        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        internal static extern int nativeGetCalendars(
            String localeName,
            bool useUserOverride,
            [In, Out] int[] calendars
        );
#endif
    }
}
