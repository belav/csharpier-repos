...
namespace Microsoft.Win32
{
...
#endif
    {
        ...
        internal static readonly IntPtr HKEY_PERFORMANCE_DATA = new IntPtr(
            unchecked((int)0x80000004)
        );
        internal static readonly IntPtr HKEY_CURRENT_CONFIG = new IntPtr(
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        public unsafe RegistryKey CreateSubKey(
            String subkey,
            RegistryKeyPermissionCheck permissionCheck,
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        private unsafe RegistryKey CreateSubKeyInternal(
            String subkey,
            RegistryKeyPermissionCheck permissionCheck,
        ...
        )
        {
            ...
            if (!remoteKey)
            {
                RegistryKey key = InternalOpenSubKey(
                    subkey,
                ...
                { // Key already exits
                    CheckPermission(
                        RegistryInternalCheck.CheckSubKeyWritePermission,
                        subkey,
                        false,
                        RegistryKeyPermissionCheck.Default
                    );
                    CheckPermission(
                        RegistryInternalCheck.CheckSubTreePermission,
                        subkey,
                        false,
                        permissionCheck
                    );
            ...
            CheckPermission(
                RegistryInternalCheck.CheckSubKeyCreatePermission,
                subkey,
                false,
                RegistryKeyPermissionCheck.Default
            );
        ...
        public void DeleteSubKeyTree(String subkey, Boolean throwOnMissingSubKey)
        {
            ...
            subkey = FixupName(subkey); // Fixup multiple slashes to a single slash
            CheckPermission(
                RegistryInternalCheck.CheckSubTreeWritePermission,
                subkey,
                false,
                RegistryKeyPermissionCheck.Default
            );
        ...
        public void DeleteValue(String name, bool throwOnMissingValue)
        {
            ...
            //
            if (
                errorCode == Win32Native.ERROR_FILE_NOT_FOUND
        ...
        )
        {
            ...
            int ret = Win32Native.RegConnectRegistry(
                machineName,
                new SafeRegistryHandle(new IntPtr((int)hKey), false),
                out foreignHKey
            );
            ...
            if (foreignHKey.IsInvalid)
                ...
                throw new ArgumentException(
                    Environment.GetResourceString("Arg_RegKeyNoRemoteConnect", machineName)
                );
            ...
            RegistryKey key = new RegistryKey(
                ...
                false,
                true,
                ((IntPtr)hKey) == HKEY_PERFORMANCE_DATA,
                view
            );
            key.checkMode = RegistryKeyPermissionCheck.Default;
            key.keyName = hkeyNames[index];
        ...
        public RegistryKey OpenSubKey(string name, bool writable)
        {
            ...
            CheckPermission(
                RegistryInternalCheck.CheckOpenSubKeyWithWritablePermission,
                name,
                writable,
                RegistryKeyPermissionCheck.Default
            );
            ...
            if (
                ret == Win32Native.ERROR_ACCESS_DENIED
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        private RegistryKey InternalOpenSubKey(
            String name,
            RegistryKeyPermissionCheck permissionCheck,
            int rights
        )
        {
            ...
            CheckPermission(
                RegistryInternalCheck.CheckOpenSubKeyPermission,
                name,
                false,
                permissionCheck
            );
        ...
        public int SubKeyCount
        {
            ...
            {
                CheckPermission(
                    RegistryInternalCheck.CheckKeyReadPermission,
                    null,
                    false,
                    RegistryKeyPermissionCheck.Default
                );
        ...
        {
            CheckPermission(
                RegistryInternalCheck.CheckValueReadPermission,
                name,
                false,
                RegistryKeyPermissionCheck.Default
            );
        ...
        )
        {
            ...
            if (ret != 0)
            {
                if (IsPerfDataKey())
                {
                    ...
                    while (
                        Win32Native.ERROR_MORE_DATA
                        == (
                            r = Win32Native.RegQueryValueEx(
                                hkey,
                                name,
                                null,
                                ref type,
                        ...
                        if (datasize % 2 == 1)
                        {
                            ...
                            catch (OverflowException e)
                            {
                                throw new IOException(
                                    Environment.GetResourceString("Arg_RegGetOverflowBug"),
                                    e
                                );
                            }
                        }
        ...
        public unsafe void SetValue(String name, Object value, RegistryValueKind valueKind)
        {
            ...
            if (!Enum.IsDefined(typeof(RegistryValueKind), valueKind))
                throw new ArgumentException(
                    Environment.GetResourceString("Arg_RegBadKeyKind"),
                    "valueKind"
                );
            ...
            { // Creating a new value
                CheckPermission(
                    RegistryInternalCheck.CheckValueCreatePermission,
                    name,
                    false,
                    RegistryKeyPermissionCheck.Default
                );
                        ...
                        fixed (byte* b = basePtr)
                        {
                            ...
                            for (int i = 0; i < dataStrings.Length; i++)
                            {
                                ...
                                String.InternalCopy(
                                    dataStrings[i],
                                    currentPtr,
                                    (checked(dataStrings[i].Length * 2))
                                );
                                currentPtr = new IntPtr(
        ...
        private RegistryValueKind CalculateValueKind(Object value)
        {
            ...
            else if (value is Array)
            {
                if (value is byte[])
                    ...
                    throw new ArgumentException(
                        Environment.GetResourceString("Arg_RegSetBadArrType", value.GetType().Name)
                    );
            }
            else
...
*/
                ...
                case Win32Native.ERROR_FILE_NOT_FOUND:
                    throw new IOException(
                        Environment.GetResourceString("Arg_RegKeyNotFound"),
                        errorCode
                    );
        ...
        //
        private void GetSubKeyReadPermission(
            string subkeyName,
            out RegistryPermissionAccess access,
            out string path
        )
        {
        ...
        private void GetSubKeyWritePermission(
            string subkeyName,
            out RegistryPermissionAccess access,
            out string path
        )
        ...
        private void GetSubKeyCreatePermission(
            string subkeyName,
            out RegistryPermissionAccess access,
            out string path
        )
        ...
        //
        private void GetSubTreeReadPermission(
            string subkeyName,
            out RegistryPermissionAccess access,
            out string path
        )
        ...
        private void GetSubTreeWritePermission(
            string subkeyName,
            out RegistryPermissionAccess access,
            out string path
        )
        ...
        private void GetSubTreeReadWritePermission(
            string subkeyName,
            out RegistryPermissionAccess access,
            out string path
        )
        ...
        //
        private void GetValueReadPermission(
            string valueName,
            out RegistryPermissionAccess access,
            out string path
        )
        ...
        private void GetValueWritePermission(
            string valueName,
            out RegistryPermissionAccess access,
            out string path
        )
        ...
        private void GetValueCreatePermission(
            string valueName,
            out RegistryPermissionAccess access,
            out string path
        )
        ...
        )
        {
            ...
            switch (check)
            {
                ...
                case RegistryInternalCheck.CheckSubKeyReadPermission:
                    ...
                    else
                    {
                        BCLDebug.Assert(
                            ...
                            "Should be called from a key opened under default mode only!"
                        );
                        BCLDebug.Assert(
                            subKeyWritable == false,
                            "subKeyWritable should be false (unused)"
                        );
                        BCLDebug.Assert(
                            subKeyCheck == RegistryKeyPermissionCheck.Default,
                            "subKeyCheck should be Default (unused)"
                        );
                        demand = true;
                ...
                case RegistryInternalCheck.CheckSubKeyWritePermission:
                    ...
                    {
                        BCLDebug.Assert(
                            checkMode != RegistryKeyPermissionCheck.ReadSubTree,
                            "We shouldn't allow creating sub key under read-only key!"
                        );
                        BCLDebug.Assert(
                            subKeyWritable == false,
                            "subKeyWritable should be false (unused)"
                ...
                case RegistryInternalCheck.CheckSubTreeWritePermission:
                    ...
                    else
                    {
                        BCLDebug.Assert(
                            checkMode != RegistryKeyPermissionCheck.ReadSubTree,
                            "We shouldn't allow writing value to read-only key!"
                        );
                        BCLDebug.Assert(
                            subKeyWritable == false,
                            "subKeyWritable should be false (unused)"
                ...
                case RegistryInternalCheck.CheckValueCreatePermission:
                    ...
                    else
                    {
                        BCLDebug.Assert(
                            checkMode != RegistryKeyPermissionCheck.ReadSubTree,
                            "We shouldn't allow creating value under read-only key!"
                        );
                        BCLDebug.Assert(
                            subKeyWritable == false,
                            "subKeyWritable should be false (unused)"
                ...
                case RegistryInternalCheck.CheckKeyReadPermission:
                    ...
                    if (checkMode == RegistryKeyPermissionCheck.Default)
                    {
                        BCLDebug.Assert(
                            item == null,
                            "CheckKeyReadPermission should never have a non-null item parameter!"
                        );
                        BCLDebug.Assert(
                            subKeyWritable == false,
                            "subKeyWritable should be false (unused)"
                ...
                default:
                    BCLDebug.Assert(
                        false,
                        "CheckPermission default switch case should never be hit!"
                    );
                    break;
            }
