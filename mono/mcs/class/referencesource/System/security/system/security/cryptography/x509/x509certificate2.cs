...
namespace System.Security.Cryptography.X509Certificates
{
    ...
    public sealed class PublicKey
    {
        ...
        private byte[] CspBlobData
        {
            ...
            get
            {
                if (m_cspBlobData == null)
                    DecodePublicKeyObject(
                        AlgorithmId,
                        m_encodedKeyValue.RawData,
                        m_encodedParameters.RawData,
                        out m_cspBlobData
                    );
                    ...
                    switch (AlgorithmId)
                    {
                        ...
                        default:
                            throw new NotSupportedException(
                                SR.GetString(SR.NotSupported_KeyAlgorithm)
                            );
                    }
                }
                return m_key;
        ...
        [SecuritySafeCritical]
#endif
        private static void DecodePublicKeyObject(
            uint aiPubKey,
        ...
        )
        {
            ...
            if ((uint)pszStructType == CAPI.RSA_CSP_PUBLICKEYBLOB)
            {
                ...
                Marshal.Copy(
                    decodedKeyValue.DangerousGetHandle(),
                    decodedData,
                    0,
                    decodedData.Length
                );
        ...
        [SecuritySafeCritical]
#endif
        private static byte[] ConstructDSSPubKeyCspBlob(
            SafeLocalAllocHandle decodedKeyValue,
        ...
        )
        {
            ...
            CAPI.CRYPTOAPI_BLOB pDssPubKey = (CAPI.CRYPTOAPI_BLOB)
                Marshal.PtrToStructure(
                    decodedKeyValue.DangerousGetHandle(),
                    typeof(CAPI.CRYPTOAPI_BLOB)
                );
            CAPI.CERT_DSS_PARAMETERS pDssParameters = (CAPI.CERT_DSS_PARAMETERS)
                Marshal.PtrToStructure(
                    decodedParameters.DangerousGetHandle(),
    ...
    public class X509Certificate2 : X509Certificate
    {
        ...
        public override string ToString(bool verbose)
        {
            ...
            sb.Append(newLinesp2);
            sb.Append(
                this.SignatureAlgorithm.FriendlyName + "(" + this.SignatureAlgorithm.Value + ")"
            );
        ...
        public bool Archived
        {
            ...
            get
            {
                ...
                uint cbData = 0;
                return CAPI.CertGetCertificateContextProperty(
                    m_safeCertContext,
        ...
        public string FriendlyName
        {
            ...
            get
            {
                ...
                uint cbData = 0;
                if (
                    !CAPI.CertGetCertificateContextProperty(
                        m_safeCertContext,
                ...
                if (m_issuerName == null)
                {
                    unsafe
                    {
                        CAPI.CERT_CONTEXT pCertContext = *(
                            (CAPI.CERT_CONTEXT*)m_safeCertContext.DangerousGetHandle()
                        );
                ...
                if (m_notAfter == DateTime.MinValue)
                {
                    unsafe
                    {
                        ...
                        CAPI.CERT_INFO pCertInfo = (CAPI.CERT_INFO)
                            Marshal.PtrToStructure(pCertContext.pCertInfo, typeof(CAPI.CERT_INFO));
                        long dt =
                            (((long)(uint)pCertInfo.NotAfter.dwHighDateTime) << 32)
                            | ((long)(uint)pCertInfo.NotAfter.dwLowDateTime);
                ...
                if (m_notBefore == DateTime.MinValue)
                {
                    unsafe
                    {
                        ...
                        CAPI.CERT_INFO pCertInfo = (CAPI.CERT_INFO)
                            Marshal.PtrToStructure(pCertContext.pCertInfo, typeof(CAPI.CERT_INFO));
                        long dt =
                            (((long)(uint)pCertInfo.NotBefore.dwHighDateTime) << 32)
                            | ((long)(uint)pCertInfo.NotBefore.dwLowDateTime);
        ...
        public AsymmetricAlgorithm PrivateKey
        {
            ...
            set
            {
                ...
                if (asymmetricAlgorithm != null)
                {
                    ...
                    ICspAsymmetricAlgorithm publicKey =
                        this.PublicKey.Key as ICspAsymmetricAlgorithm;
                    ...
                    if (
                        ...
                        || array1.Length <= s_publicKeyOffset
                    )
                        throw new CryptographicUnexpectedOperationException(
                            SR.GetString(SR.Cryptography_X509_KeyMismatch)
                        );
        ...
        public unsafe string GetNameInfo(X509NameType nameType, bool forIssuer)
        {
            ...
            pAltName[0] = CAPI.CertFindExtension(
                forIssuer ? CAPI.szOID_ISSUER_ALT_NAME : CAPI.szOID_SUBJECT_ALT_NAME,
                pCertInfo.cExtension,
                pCertInfo.rgExtension
            );
            pAltName[1] = CAPI.CertFindExtension(
                forIssuer ? CAPI.szOID_ISSUER_ALT_NAME2 : CAPI.szOID_SUBJECT_ALT_NAME2,
                pCertInfo.cExtension,
                pCertInfo.rgExtension
            );
            for (int i = 0; i < pAltName.Length; i++)
            {
                if (pAltName[i] != IntPtr.Zero)
                {
                    CAPI.CERT_EXTENSION extension = (CAPI.CERT_EXTENSION)
                        Marshal.PtrToStructure(pAltName[i], typeof(CAPI.CERT_EXTENSION));
                    ...
                    if (result)
                    {
                        CAPI.CERT_ALT_NAME_INFO altNameInfo = (CAPI.CERT_ALT_NAME_INFO)
                            Marshal.PtrToStructure(
                        ...
                        {
                            IntPtr pAltInfoPtr = new IntPtr(
                                (long)altNameInfo.rgAltEntry
                                    + index * Marshal.SizeOf(typeof(CAPI.CERT_ALT_NAME_ENTRY))
                            );
                            CAPI.CERT_ALT_NAME_ENTRY altNameEntry = (CAPI.CERT_ALT_NAME_ENTRY)
                                Marshal.PtrToStructure(
                                    pAltInfoPtr,
                            ...
                            switch (type)
                            {
                                case CAPI.CERT_NAME_UPN_TYPE:
                                    if (
                                        altNameEntry.dwAltNameChoice
                                        == CAPI.CERT_ALT_NAME_OTHER_NAME
                                    )
                                    {
                                        CAPI.CERT_OTHER_NAME otherName = (CAPI.CERT_OTHER_NAME)
                                            Marshal.PtrToStructure(
                                                altNameEntry.Value.pOtherName,
                                                typeof(CAPI.CERT_OTHER_NAME)
                                            );
                                        if (otherName.pszObjId == CAPI.szOID_NT_PRINCIPAL_NAME)
                                        {
                                            ...
                                            if (result)
                                            {
                                                CAPI.CERT_NAME_VALUE nameValue =
                                                    (CAPI.CERT_NAME_VALUE)
                                                        Marshal.PtrToStructure(
                                                            pUpnName.DangerousGetHandle(),
                                                            typeof(CAPI.CERT_NAME_VALUE)
                                                        );
                                                if (
                                                    X509Utils.IsCertRdnCharString(
                                                        nameValue.dwValueType
                                                    )
                                                )
                                                    name = Marshal.PtrToStringUni(
                                                        nameValue.Value.pbData
                                                    );
                                                pUpnName.Dispose();
                                            }
                                        }
                                ...
                                case CAPI.CERT_NAME_DNS_TYPE:
                                    if (altNameEntry.dwAltNameChoice == CAPI.CERT_ALT_NAME_DNS_NAME)
                                        name = Marshal.PtrToStringUni(
                                            altNameEntry.Value.pwszDNSName
                                        );
            ...
            if (nameType == X509NameType.DnsName)
            {
                ...
                if (name == null || name.Length == 0)
                    name = CAPI.GetCertNameInfo(
                        m_safeCertContext,
                        issuerFlag,
                        CAPI.CERT_NAME_ATTR_TYPE
                    );
            }
        ...
        public bool Verify()
        {
            ...
            int hr = X509Utils.VerifyCertificate(
                this.CertContext,
                null,
                null,
        ...
        )
        {
            ...
            CAPI.CRYPT_KEY_PROV_INFO pKeyProvInfo = (CAPI.CRYPT_KEY_PROV_INFO)
                Marshal.PtrToStructure(ptr.DangerousGetHandle(), typeof(CAPI.CRYPT_KEY_PROV_INFO));
        ...
        {
            CAPI.CERT_CONTEXT pCertContext = *(
                (CAPI.CERT_CONTEXT*)safeCertContextHandle.DangerousGetHandle()
            );
            CAPI.CERT_INFO pCertInfo = (CAPI.CERT_INFO)
                Marshal.PtrToStructure(pCertContext.pCertInfo, typeof(CAPI.CERT_INFO));
            return new Oid(
                pCertInfo.SignatureAlgorithm.pszObjId,
        ...
        private static unsafe uint QueryCertBlobType(byte[] rawData)
        {
            uint contentType = 0;
            if (
                !CAPI.CryptQueryObject(
                    CAPI.CERT_QUERY_OBJECT_BLOB,
                    rawData,
        ...
        [SecuritySafeCritical]
#endif
        private static unsafe void SetFriendlyNameExtendedProperty(
        ...
        [SecuritySafeCritical]
#endif
        private static unsafe void SetPrivateKeyProperty(
            SafeCertContextHandle safeCertContextHandle,
            ICspAsymmetricAlgorithm asymmetricAlgorithm
        )
        {
            SafeLocalAllocHandle ptr = SafeLocalAllocHandle.InvalidHandle;
            if (asymmetricAlgorithm != null)
