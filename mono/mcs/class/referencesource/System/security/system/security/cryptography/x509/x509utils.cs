...
namespace System.Security.Cryptography.X509Certificates
{
    ...
    internal class X509Utils
    {
        ...
        internal static uint MapKeyStorageFlags(X509KeyStorageFlags keyStorageFlags)
        {
            uint dwFlags = 0;
            if (
                (keyStorageFlags & X509KeyStorageFlags.UserKeySet) == X509KeyStorageFlags.UserKeySet
            )
            ...
            if (
                (keyStorageFlags & X509KeyStorageFlags.Exportable) == X509KeyStorageFlags.Exportable
            )
        ...
        )
        {
            ...
            safeCertStoreHandle = CAPI.CertOpenStore(
        ...
        internal static uint OidToAlgId(string value)
        {
            ...
            CAPI.CRYPT_OID_INFO pOIDInfo = CAPI.CryptFindOIDInfo(
                CAPI.CRYPT_OID_INFO_OID_KEY,
                pszOid,
                0
            );
        ...
        [SecuritySafeCritical]
#endif
        internal static string FindOidInfo(
            uint keyType,
            string keyValue,
            System.Security.Cryptography.OidGroup oidGroup
        )
        {
            if (keyValue == null)
        ...
        internal static string FindOidInfoWithFallback(
            uint key,
            string value,
        ...
        internal static SafeLocalAllocHandle CopyOidsToUnmanagedMemory(OidCollection oids)
        {
            ...
            checked
            {
                ...
                safeLocalAllocHandle = CAPI.LocalAlloc(
                    CAPI.LPTR,
            ...
            for (int index = 0; index < oidStrs.Count; index++)
            {
                Marshal.WriteIntPtr(
                    new IntPtr(
                        (long)safeLocalAllocHandle.DangerousGetHandle()
        ...
        )
        {
            ...
            IntPtr pEnumContext = CAPI.CertEnumCertificatesInStore(
                safeCertStoreHandle,
                IntPtr.Zero
            );
        ...
        internal static string GetSystemErrorString(int hr)
        {
            ...
            uint dwErrorCode = CAPI.FormatMessage(
                CAPI.FORMAT_MESSAGE_FROM_SYSTEM | CAPI.FORMAT_MESSAGE_IGNORE_INSERTS,
                IntPtr.Zero,
                (uint)hr,
                0,
                strMessage,
