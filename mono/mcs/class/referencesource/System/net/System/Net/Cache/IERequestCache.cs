...
namespace Microsoft.Win32
{
    ...
    internal class WinInetCache : RequestCache
    {
        ...
        internal override void Remove(string key)
        {
                ...
                if (Logging.On)
                    Logging.PrintError(
                        ...
                        SR.GetString(
                            ...
                            "WinInetCache.Remove()",
                            SR.GetString(SR.net_cache_access_denied, "Write")
                        )
                    );
                return;
            }
            ...
            if (
                _WinInetCache.Remove(entry) != _WinInetCache.Status.Success
            ...
            )
            {
                ...
                if (Logging.On)
                    Logging.PrintError(
                        Logging.RequestCache,
                        SR.GetString(
                            SR.net_log_cache_cannot_remove,
                            "WinInetCache.Remove()",
                            key,
                ...
                throw new IOException(
                    SR.GetString(SR.net_cache_retrieve_failure, win32Exception.Message),
                    win32Exception
                );
            }
            ...
            else if (!forceRemove)
            {
                if (Logging.On)
                    Logging.PrintWarning(
                        Logging.RequestCache,
                        SR.GetString(
                            SR.net_log_cache_key_remove_failed_status,
                            "WinInetCache.TryRemove()",
                            key,
                            entry.Error.ToString()
                        )
                ...
                while (entry.Info.UseCount != 0)
                {
                    ...
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_usecount_file,
                                "WinInetCache.TryRemove()",
                                entry.Info.UseCount,
                                entry.Filename
                            )
                        ...
                        unsafe
                        {
                            ...
                            fixed (char* ch = entry.MetaInfo)
                            {
                                ...
                                for (i = 0; i < length; ++i)
                                {
                                    ...
                                    if ((i + 1 == length) || (ch[i] == '\n'))
                                    {
                                        string value = entry.MetaInfo.Substring(
                                            start,
                    ...
                    if (entry.Error != _WinInetCache.Status.FileNotFound)
                    {
                        if (Logging.On)
                            Logging.PrintError(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_lookup_failed,
                                    "WinInetCache.Retrieve()",
        ...
        //
        //
        private string CombineMetaInfo(
            StringCollection entryMetadata,
            StringCollection systemMetadata
        )
        {
            if (
                (entryMetadata == null || entryMetadata.Count == 0)
                && (systemMetadata == null || systemMetadata.Count == 0)
            )
                return string.Empty;
        ...
        )
        {
            ...
            if (
                ...
                && lastModifiedUtc > s_MinDateTimeUtcForFileTimeUtc
            )
            {
                entry.Info.LastModifiedTime = new _WinInetCache.FILETIME(
            ...
            if (Logging.On)
            {
                ...
                Logging.PrintInfo(
                    Logging.RequestCache,
                    SR.GetString(
                        SR.net_log_cache_last_modified,
                        (
                            entry.Info.LastModifiedTime.IsNull
                                ? "0"
                                : DateTime
                                    .FromFileTimeUtc(entry.Info.LastModifiedTime.ToLong())
                                    .ToString("r")
                        )
                    )
                );
                Logging.PrintInfo(
                    Logging.RequestCache,
                    SR.GetString(
                        SR.net_log_cache_expires,
                        (
                            entry.Info.ExpireTime.IsNull
                                ? "0"
                                : DateTime
                                    .FromFileTimeUtc(entry.Info.ExpireTime.ToLong())
                                    .ToString("r")
                        )
                    )
                );
            ...
            if (entry.Error != _WinInetCache.Status.Success)
            {
                if (Logging.On)
                {
                    Logging.PrintWarning(
                        Logging.RequestCache,
                        SR.GetString(
                            SR.net_log_cache_create_failed,
                            new Win32Exception((int)entry.Error).Message
                        )
                    );
                    Logging.Exit(Logging.RequestCache, "WinInetCache.Store");
        ...
        )
        {
            ...
            )
            {
                ...
                if (Logging.On)
                    Logging.PrintInfo(
                        Logging.RequestCache,
                        SR.GetString(
                            SR.net_log_cache_set_last_modified,
            ...
            )
            {
                ...
                entry.Info.LastSyncTime = new _WinInetCache.FILETIME(
                ...
                if (Logging.On)
                    Logging.PrintInfo(
                        Logging.RequestCache,
                        SR.GetString(
                            SR.net_log_cache_set_last_synchronized,
                ...
                if (maxStale >= TimeSpan.Zero)
                {
                    ...
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_enable_max_stale,
            ...
            if (entry.Error != _WinInetCache.Status.Success)
            {
                if (Logging.On)
                {
                    Logging.PrintWarning(
                        Logging.RequestCache,
                        SR.GetString(
                            SR.net_log_cache_update_failed,
                            "WinInetCache.Update()",
                            entry.Key,
                            new Win32Exception((int)entry.Error).Message
                        )
                    ...
                    try
                    {
                        if (m_CallNesting != 0)
                            throw new NotSupportedException(
                                SR.GetString(SR.net_no_concurrent_io_allowed)
                            );
                        if (m_Aborted)
                ...
                lock (m_Handle)
                {
                    ...
                    finally
                    {
                        ...
                        if (m_Event != null)
                            ...
                            catch { } // the problem is he WaitHandle cannot tell if it is disposed or not
                    }
                }
            }
                ...
                if (Interlocked.Exchange(ref m_Disposed, 1) == 0 && m_Entry != null)
                {
                    ...
                    if (cacheCommitAction == TriState.False)
                    {
                        ...
                        catch (Exception exception)
                        {
                            ...
                            if (Logging.On)
                                Logging.PrintError(
                                    Logging.RequestCache,
                                    SR.GetString(
                                        SR.net_log_cache_error_deleting_filename,
                                        "WinInetWriteStream.Close()",
                                        m_Entry.Filename
                                    )
                                );
                        ...
                        finally
                        {
                            ...
                            if (
                                errorStatus != _WinInetCache.Status.Success
                            ...
                            )
                            {
                                if (Logging.On)
                                    Logging.PrintWarning(
                                        Logging.RequestCache,
                                        SR.GetString(
                                            SR.net_log_cache_delete_failed,
                                            "WinInetWriteStream.Close()",
                                            m_Entry.Key,
                                            new Win32Exception((int)m_Entry.Error).Message
                                        )
                                    );
                            }
                    ...
                    if (_WinInetCache.Commit(m_Entry) != _WinInetCache.Status.Success)
                    {
                        if (Logging.On)
                            Logging.PrintError(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_commit_failed,
                                    "WinInetWriteStream.Close()",
                                    m_Entry.Key,
                                    new Win32Exception((int)m_Entry.Error).Message
                                )
                    ...
                    if (Logging.On)
                    {
                        if (m_StreamSize > 0 || (m_StreamSize < 0 && m_Aborted))
                            Logging.PrintWarning(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_committed_as_partial,
                                    "WinInetWriteStream.Close()",
                                    m_Entry.Key,
                                    (
                                        m_StreamSize > 0
                                            ? m_StreamSize.ToString(CultureInfo.CurrentCulture)
                    ...
                    if (
                        (m_Entry.Info.EntryType & _WinInetCache.EntryType.StickyEntry)
                        == _WinInetCache.EntryType.StickyEntry
                    )
                    {
                        if (
                            _WinInetCache.Update(m_Entry, _WinInetCache.Entry_FC.ExemptDelta)
                        ...
                        )
                        {
                            if (Logging.On)
                                Logging.PrintError(
                                    ...
                                    SR.GetString(
                                        SR.net_log_cache_update_failed,
                                        "WinInetWriteStream.Close(), Key = " + m_Entry.Key,
                                        new Win32Exception((int)m_Entry.Error).Message
                                    )
                                );
                            if (m_IsThrow)
                        ...
                        if (Logging.On)
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_max_stale_and_update_status,
                                    "WinInetWriteFile.Close()",
                                    m_Entry.Info.U.ExemptDelta,
                                    m_Entry.Error.ToString()
                                )
                            );
                    }
