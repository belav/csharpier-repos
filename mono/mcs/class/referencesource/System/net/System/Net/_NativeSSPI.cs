...
namespace System.Net
{
    ...
    internal class SSPISecureChannelType : SSPIInterface
    {
        ...
        {
            get { return m_SecurityPackages; }
            set { m_SecurityPackages = value; }
        }
        ...
        )
        {
            ...
            return SafeFreeContextBufferChannelBinding.QueryContextChannelBinding(
                Library,
                phContext,
                attribute,
                &bindings,
            ...
            if (handleType != null)
            {
                ...
                else
                {
                    throw new ArgumentException(
                        SR.GetString(SR.SSPIInvalidHandleType, handleType.FullName),
                        "handleType"
                    );
                }
            }
        ...
        {
            return SafeFreeContextBuffer.SetContextAttributes(
                Library,
                phContext,
                attribute,
                buffer
    ...
    internal class SSPIAuthType : SSPIInterface
    {
        ...
        )
        {
            ...
            {
                GlobalLog.Assert(
                    "NativeNTSSPI.DecryptMessage",
                    "Expected qop = 0, returned value = "
                        + qop.ToString("x", CultureInfo.InvariantCulture)
                );
                throw new InvalidOperationException(
                    SR.GetString(SR.net_auth_message_not_encrypted)
                );
            }
        ...
        private static int GetSecurityContextToken(
            SafeDeleteContext phContext,
            out SafeCloseHandle safeHandle
        )
        {
            int status = (int)SecurityStatus.InvalidHandle;
            bool b = false;
