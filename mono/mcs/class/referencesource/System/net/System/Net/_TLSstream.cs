...
namespace System.Net
{
    ...
    internal class TlsStream : NetworkStream, IDisposable
    {
            ...
            : base(networkStream, true)
        {
            ...
            m_Worker = new SslState(
                networkStream,
                initiatingRequest is HttpWebRequest,
            ...
            RemoteCertValidationCallback certValidationCallback =
                servicePoint.SetupHandshakeDoneProcedure(this, initiatingRequest);
        ...
        {
            get { return m_Worker.DataAvailable || base.DataAvailable; }
        }
        ...
        public override int Read(byte[] buffer, int offset, int size)
        {
            GlobalLog.Print(
                "TlsStream#"
                    ...
                    + "::Read() SecureWorker#"
                    + ValidationHelper.HashString(m_Worker)
                    + " offset:"
                    + offset.ToString()
                    + " size:"
                    + size.ToString()
        ...
        )
        {
            ...
            if (!m_Worker.IsAuthenticated)
            {
                BufferAsyncResult result = new BufferAsyncResult(
                    this,
                    buffer,
                    offset,
                    size,
        ...
        //
        internal override IAsyncResult UnsafeBeginRead(
            byte[] buffer,
            int offset,
            int size,
            AsyncCallback asyncCallback,
        ...
        internal override IAsyncResult UnsafeBeginWrite(
            byte[] buffer,
            int offset,
            int size,
            AsyncCallback asyncCallback,
        ...
        )
        {
            GlobalLog.Print(
                "TlsStream#"
                    + ValidationHelper.HashString(this)
                    + "::BeginMultipleWrite() SecureWorker#"
                    + ValidationHelper.HashString(m_Worker)
                    + " buffers.Length:"
                    + buffers.Length.ToString()
            );
        ...
        {
            get { return m_Worker.InternalLocalCertificate; }
        }
        ...
        internal bool ProcessAuthentication(LazyAsyncResult result)
        {
            ...
            try
            {
                if (doHandshake)
                {
                    ...
                    try
                    {
                        m_Worker.ValidateCreateContext(
                            false,
                            m_DestinationHost,
                        ...
                        if (!isSyncCall)
                        {
                            ...
                            handshakeResult = new LazyAsyncResult(
                                m_Worker,
                                null,
                                new AsyncCallback(WakeupPendingIO)
                            );
#if DEBUG
                        ...
                        if (_ExecutionContext != null)
                        {
                            ExecutionContext.Run(
                                ...
                                new ContextCallback(CallProcessAuthentication),
                                handshakeResult
                            );
                        }
                        else
                            ...
                            lock (m_PendingIO)
                            {
                                if (m_PendingIO.Count > 1)
                                {
                                    ...
                                    ThreadPool.QueueUserWorkItem(
                                        new WaitCallback(StartWakeupPendingIO),
                                        null
                                    );
                                }
                                else
                ...
                else if (isSyncCall)
                {
                    GlobalLog.Assert(
                        result != null,
                        "TlsStream::ProcessAuthentication() this is a Sync call and it did not started the handshake hence null result must be wrapped into LazyAsyncResult"
                    );
        ...
        private void StartWakeupPendingIO(object nullState)
        {
            ...
            GlobalLog.Assert(
                nullState == null,
                "TlsStream::StartWakeupPendingIO|Expected null state but got {0}.",
                nullState == null ? "null" : (nullState.GetType().FullName)
            );
            ...
            lock (m_PendingIO)
            {
                while (m_PendingIO.Count != 0)
                {
                    ...
                    if (lazyResult is BufferAsyncResult)
                    {
                        ...
                        {
                            ThreadPool.QueueUserWorkItem(
                                new WaitCallback(ResumeIOWorker),
                                lazyResult
                            );
                        }
                    }
