...
namespace System.Net
{
    ...
    internal class LoggingObject : BaseLoggingObject
    {
        ...
        internal override unsafe void Dump(byte[] buffer, int offset, int length)
        {
            ...
            if (length > GlobalLog.s_MaxDumpSize)
            {
                PrintLine(
                    "(printing "
                        + GlobalLog.s_MaxDumpSize.ToString()
                        + " out of "
                        + length.ToString()
                        + ")"
                );
        ...
        unsafe void Dump(byte* buffer, int length)
        {
            do
            {
                ...
                string disp =
                    ((IntPtr)buffer).ToString("X8") + " : " + offset.ToString("X8") + " : ";
                byte current;
                for (int i = 0; i < n; ++i)
                {
                    current = *(buffer + i);
            ...
            lock (this)
            {
                if (!shouldClose)
                {
                    if (_Stream == null)
                    {
                        ...
                        _Stream.Write(
                            "# CurrentIdentity: " + WindowsIdentity.GetCurrent().Name + "\r\n"
                        );
        ...
        private void ValidatePop(string name)
        {
            if (GlobalLog.s_DebugCallNesting)
            {
                try
                {
                    ...
                    string[] parsedList = popedName.Split(
                        new char[] { '(', ')', ' ', '.', ':', ',', '#' }
                    );
    ...
    {
        public IntegerSwitch(string switchName, string switchDescription)
            : base(switchName, switchDescription) { }
    ...
    internal static class GlobalLog
    {
        ...
        private static BaseLoggingObject LoggingInitialize()
        {
            ...
            if (
                !GetSwitchValue(
                    "SystemNetLog_OverrideDefaults",
                    "System.Net log override default settings",
                    false
                )
            )
            {
            ...
            {
                s_ThreadIdSlot = Thread.AllocateDataSlot();
                s_UseThreadId = GetSwitchValue(
                    "SystemNetLog_UseThreadId",
                    "System.Net log display system thread id",
                    false
                );
                s_UseTimeSpan = GetSwitchValue(
                    "SystemNetLog_UseTimeSpan",
                    "System.Net log display ticks as TimeSpan",
                    false
                );
                s_DumpWebData = GetSwitchValue(
                    "SystemNetLog_DumpWebData",
                    "System.Net log display HTTP send/receive data",
                    false
                );
                s_MaxDumpSize = GetSwitchValue(
                    "SystemNetLog_MaxDumpSize",
                    "System.Net log max size of display data",
                    256
                );
                s_UsePerfCounter = GetSwitchValue(
                    "SystemNetLog_UsePerfCounter",
                    "System.Net log use QueryPerformanceCounter() to get ticks ",
                    false
                );
                s_DebugCallNesting = GetSwitchValue(
                    "SystemNetLog_DebugCallNesting",
                    "System.Net used to debug call nesting",
                    false
                );
                s_DumpToConsole = GetSwitchValue(
                    "SystemNetLog_DumpToConsole",
                    "System.Net log to console",
                    false
                );
                s_RootDirectory = GetSwitchValue(
                    "SystemNetLog_RootDirectory",
                    "System.Net root directory of log file",
                    string.Empty
                );
                return new LoggingObject();
            }
        ...
        internal static IDisposable SetThreadKind(ThreadKinds kind)
        {
            ...
            {
                Print(
                    "WARNING: Thread changed from User to System; user's thread shouldn't be hijacked."
                );
            }
            ...
            if ((threadKind & ThreadKinds.Async) != 0 && (kind & ThreadKinds.Sync) != 0)
            {
                ...
                (threadKind & (ThreadKinds.Other | ThreadKinds.CompletionPort)) == 0
                && (kind & ThreadKinds.Sync) != 0
            )
            {
                Print(
                    "WARNING: Thread from a limited resource changed to Sync, may deadlock or bottleneck."
                );
            }
#endif
            ...
            ThreadKindStack.Push(
                (((kind & ThreadKinds.OwnerMask) == 0 ? threadKind : kind) & ThreadKinds.OwnerMask)
                    | (
                        ((kind & ThreadKinds.SyncMask) == 0 ? threadKind : kind)
        ...
        internal static void SetThreadSource(ThreadKinds source)
        {
            ...
            {
                Print(
                    "WARNING: SetThreadSource must be called at the base of the stack, or the stack has been corrupted."
                );
            ...
            if (ThreadKindStack.Peek() != source)
            {
                ...
                Assert(
                    last == source || last == ThreadKinds.Other,
                    "Thread source changed.|Was:({0}) Now:({1})",
                    last,
                    source
                );
                ThreadKindStack.Push(source);
        ...
        [Conditional("DEBUG")]
        [ReliabilityContract(Consistency.MayCorruptAppDomain, Cer.None)]
        internal static void ThreadContract(
            ThreadKinds kind,
            ThreadKinds allowedSources,
            string errorMsg
        )
        {
            if (
                (kind & ThreadKinds.SourceMask) != ThreadKinds.Unknown
                || (allowedSources & ThreadKinds.SourceMask) != allowedSources
            )
            {
                throw new InternalException();
            }
            ...
            ThreadKinds threadKind = CurrentThreadKind;
            Assert(
                (threadKind & allowedSources) != 0,
                errorMsg,
                "Thread Contract Violation.|Expected source:({0}) Actual source:({1})",
                allowedSources,
                threadKind & ThreadKinds.SourceMask
            );
            Assert(
                (threadKind & kind) == kind,
                errorMsg,
                "Thread Contract Violation.|Expected kind:({0}) Actual kind:({1})",
                kind,
                threadKind & ~ThreadKinds.SourceMask
            );
        }
        ...
        public static void Assert(bool condition, string messageFormat, params object[] data)
        {
            if (!condition)
            {
                ...
                else
                {
                    ...
                    Assert(
                        fullMessage.Substring(0, pipeIndex),
                        detailLength > 0 ? fullMessage.Substring(pipeIndex + 1, detailLength) : null
                    );
                }
            }
        }
        ...
        public static void Assert(string message, string detailMessage)
        {
            try
            {
                Print(
                    "Assert: "
                        + message
                        + (!string.IsNullOrEmpty(detailMessage) ? ": " + detailMessage : "")
                );
                Print("*******");
