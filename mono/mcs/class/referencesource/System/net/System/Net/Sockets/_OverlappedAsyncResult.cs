...
namespace System.Net.Sockets
{
    ...
    internal class OverlappedAsyncResult : BaseOverlappedAsyncResult
    {
        ...
        {
            return Marshal.UnsafeAddrOfPinnedArrayElement(
                m_SocketAddress.m_Buffer,
        ...
        {
            get { return m_SocketAddress; }
        }

        //
        internal SocketAddress SocketAddressOriginal
        {
            get { return m_SocketAddressOriginal; }
            set { m_SocketAddressOriginal = value; }
        }
        ...
        internal void SetUnmanagedStructures(BufferOffsetSize[] buffers)
        {
            ...
            for (int i = 0; i < buffers.Length; i++)
            {
                ...
                m_WSABuffers[i].Pointer = Marshal.UnsafeAddrOfPinnedArrayElement(
                    buffers[i].Buffer,
                    buffers[i].Offset
                );
            }
        ...
        void LogBuffer(int size)
        {
            GlobalLog.Assert(
                Logging.On,
                "OverlappedAsyncResult#{0}::LogBuffer()|Logging is off!",
