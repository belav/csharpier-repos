...
namespace System.Net
{
    ...
    internal static class TimerThread
    {
        ...
        internal abstract class Queue
        {
            ...
            {
                get { return m_DurationMilliseconds; }
            }
        ...
        internal abstract class Timer : IDisposable
        {
            ...
            {
                get { return m_StartTimeMilliseconds; }
            }
            ...
            {
                get { return unchecked(m_StartTimeMilliseconds + m_DurationMilliseconds); }
            }
            ...
            internal int TimeRemaining
            {
                get
                {
                    ...
                    int remaining = IsTickBetween(StartTime, Expiration, now)
                        ? (int)(Math.Min((uint)unchecked(Expiration - now), (uint)Int32.MaxValue))
                        : 0;
        ...
        private class TimerQueue : Queue
        {
            ...
            internal override Timer CreateTimer(Callback callback, object context)
            {
                ...
                {
                    GlobalLog.Assert(
                        m_Timers.Prev.Next == m_Timers,
                        "TimerThread#{0}::CreateTimer()|m_Tail corruption.",
        ...
        private class TimerNode : Timer
        {
            ...
            */
            ...
            internal override bool Cancel()
            {
                if (m_TimerState == TimerState.Ready)
                {
                    lock (m_QueueLock)
                    {
                        if (m_TimerState == TimerState.Ready)
                        {
                            ...
                            GlobalLog.Print(
                                "TimerThreadTimer#" + StartTime.ToString() + "::Cancel() (success)"
                            );
                            return true;
                        }
                    }
                ...
                GlobalLog.Print(
                    "TimerThreadTimer#" + StartTime.ToString() + "::Cancel() (failure)"
                );
                return false;
            }
            ...
            internal bool Fire()
            {
                GlobalLog.Assert(
                    m_TimerState != TimerState.Sentinel,
                    "TimerThread#{0}::Fire()|TimerQueue tried to Fire a Sentinel.",
                    Thread.CurrentThread.ManagedThreadId.ToString()
                );
                    ...
                    catch (Exception exception)
                    {
                        ...
                        if (Logging.On)
                            Logging.PrintError(
                                Logging.Web,
                                "TimerThreadTimer#"
                                    + StartTime.ToString(NumberFormatInfo.InvariantInfo)
                                    + "::Fire() - "
                                    + SR.GetString(SR.net_log_exception_in_callback, exception)
                            );
                        GlobalLog.Print(
                            "TimerThreadTimer#"
                                + StartTime
                                + "::Fire() exception in callback: "
                                + exception
                        );
                            ...
                            while (true)
                            {
                                    ...
                                    ? (int)(
                                        IsTickBetween(now, nextTick, newNow)
                                            ? Math.Min(
                                                ...
                                                (uint)(Int32.MaxValue - c_TickCountResolution)
                                            ) + c_TickCountResolution
                                            : 0
                                    )
                                ...
                                if (waitResult == 0)
                                {
                                    GlobalLog.Print(
                                        "TimerThread#"
                                            + Thread.CurrentThread.ManagedThreadId.ToString()
                                            + "::ThreadProc() Awoke, cause: Shutdown"
                                    );
                                    running = false;
                                    break;
                                }
