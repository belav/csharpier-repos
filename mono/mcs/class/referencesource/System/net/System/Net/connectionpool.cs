...
namespace System.Net
{
    ...
    internal class ConnectionPool
    {
        ...
        private static TimerThread.Callback s_CancelErrorCallback = new TimerThread.Callback(
            CancelErrorCallbackWrapper
        );
        private static TimerThread.Queue s_CancelErrorQueue = TimerThread.GetOrCreateQueue(
            ErrorWait
        );
        ...
        {
            get { return (Mutex)m_WaitHandles[CreationHandleIndex]; }
        }

        private ManualResetEvent ErrorEvent
        {
            get { return (ManualResetEvent)m_WaitHandles[ErrorHandleIndex]; }
        }

        private Semaphore Semaphore
        {
            get { return (Semaphore)m_WaitHandles[SemaphoreHandleIndex]; }
        }
            ...
            : base()
        {
            ...
            if (idleTimeout > 0)
            {
                ...
                m_CleanupQueue = TimerThread.GetOrCreateQueue(
                    ...
                    try
                    {
                        while ((PooledStream == null) && continueLoop)
                        {
                            int result = WaitHandle.WaitAny(
                                localWaitHandles,
                                asyncState.CreationTimeout,
                                false
                            );
                            PooledStream = Get(
                                asyncState.OwningObject,
                                result,
                                ref continueLoop,
                                ref localWaitHandles
                            );
                        }
        ...
        internal void ForceCleanup()
        {
            ...
            while (Count > 0)
            {
                if (Semaphore.WaitOne(0, false))
                {
                    ...
                    Debug.Assert(
                        pooledStream != null,
                        "Acquired Semaphore with no connections in either stack"
                    );
        ...
        private PooledStream Create(CreateConnectionDelegate createConnectionCallback)
        {
            ...
            GlobalLog.Leave(
                "ConnectionPool#" + ValidationHelper.HashString(this) + "::Create",
                ValidationHelper.HashString(newObj)
            );
            return newObj;
        }
        ...
        private PooledStream GetFromPool(object owningObject)
        {
            ...
            GlobalLog.Enter(
                "ConnectionPool#" + ValidationHelper.HashString(this) + "::GetFromPool"
            );
            ...
            if (null != res)
            {
                ...
                GlobalLog.Print(
                    "GetFromGeneralPool pooledStream#" + ValidationHelper.HashString(res)
                );
            }

            GlobalLog.Leave(
                "ConnectionPool#" + ValidationHelper.HashString(this) + "::GetFromPool",
                ValidationHelper.HashString(res)
            );
            return (res);
        }
        ...
        /// </summary>
        private PooledStream Get(
            object owningObject,
            int result,
            ref bool continueLoop,
            ref WaitHandle[] waitHandles
        )
        {
            PooledStream pooledStream = null;
            GlobalLog.Enter(
                "ConnectionPool#" + ValidationHelper.HashString(this) + "::Get",
                result.ToString()
            );
            ...
            switch (result)
            {
                case WaitTimeout:
                    ...
                    GlobalLog.Leave(
                        "ConnectionPool#" + ValidationHelper.HashString(this) + "::Get",
                        "throw Timeout WebException"
                    );
                    throw new WebException(
                        NetRes.GetWebStatusString("net_timeout", WebExceptionStatus.ConnectFailure),
        ...
        )
        {
            ...
            GlobalLog.Enter(
                "ConnectionPool#" + ValidationHelper.HashString(this) + "::GetConnection"
            );
            ...
            if (async)
            {
                ...
                {
                    GlobalLog.Print(
                        "GetConnection:" + ValidationHelper.HashString(this) + " going async"
                    );
                    AsyncConnectionPoolRequest asyncState = new AsyncConnectionPoolRequest(
                        this,
                        owningObject,
                        asyncCallback,
                        creationTimeout
        ...
        {
            GlobalLog.Print(
                "ConnectionPool#" + ValidationHelper.HashString(this) + "::PutConnection"
            );
            if (pooledStream == null)
            {
            ...
            if (m_State != State.ShuttingDown)
            {
                ...
                else
                {
                    ...
                    { // Make sure to release the mutex even under error conditions.
        ...
        private bool ReclaimEmancipatedObjects()
        {
                        ...
                        if (null != pooledStream)
                        {
                            ...
                            try
                            {
                                ...
                                if (locked)
                                {
                                    ...
                                    {
                                        GlobalLog.Print(
                                            "EmancipatedObject pooledStream#"
                                                + ValidationHelper.HashString(pooledStream)
                                        );
            ...
            GlobalLog.Leave(
                "ConnectionPool#"
                    + ValidationHelper.HashString(this)
                    + "::ReclaimEmancipatedObjects",
                emancipatedObjectFound.ToString()
            );
        ...
        private PooledStream UserCreateRequest()
        {
            ...
            GlobalLog.Enter(
                "ConnectionPool#" + ValidationHelper.HashString(this) + "::UserCreateRequest"
            );
