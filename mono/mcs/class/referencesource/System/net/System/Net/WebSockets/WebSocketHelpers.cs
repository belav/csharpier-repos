    ...
    internal static class WebSocketHelpers
    {
        ...
        )
        {
            ...
            try
            {
                ...
                string secWebSocketAccept = WebSocketHelpers.GetSecWebSocketAcceptString(
                    secWebSocketKey
                );
                ...
                if (Logging.On)
                {
                    Logging.PrintInfo(
                        Logging.WebSockets,
                        string.Format("{0} = {1}", HttpKnownHeaderNames.Origin, origin)
                    );
                    Logging.PrintInfo(
                        Logging.WebSockets,
                        string.Format(
                ...
                WebSocket webSocket = WebSocket.CreateServerWebSocket(
                    webSocketStream,
                    subProtocol,
                    receiveBufferSize,
                    keepAliveInterval,
        ...
        internal static string GetSecWebSocketAcceptString(string secWebSocketKey)
        {
            ...
            {
                string acceptString = string.Concat(
                    secWebSocketKey,
                    WebSocketHelpers.SecWebSocketKeyGuid
                );
        ...
        internal static void ValidateSubprotocol(string subProtocol)
        {
            if (string.IsNullOrWhiteSpace(subProtocol))
            {
                throw new ArgumentException(
                    SR.GetString(SR.net_WebSockets_InvalidEmptySubProtocol),
                    "subProtocol"
                );
            }
        ...
        internal static void ValidateBufferSizes(int receiveBufferSize, int sendBufferSize)
        {
            if (receiveBufferSize < WebSocketBuffer.MinReceiveBufferSize)
            {
                throw new ArgumentOutOfRangeException(
                    ...
                    SR.GetString(
                        SR.net_WebSockets_ArgumentOutOfRange_TooSmall,
                        WebSocketBuffer.MinReceiveBufferSize
                    )
                );
            }
        ...
        internal static void ThrowPlatformNotSupportedException_WSPC()
        {
            throw new PlatformNotSupportedException(
                SR.GetString(SR.net_WebSockets_UnsupportedPlatform)
            );
        }
        ...
        internal static void ValidateArraySegment<T>(
