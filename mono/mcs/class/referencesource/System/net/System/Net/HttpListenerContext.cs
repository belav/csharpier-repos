...
namespace System.Net
{
    ...
    public sealed unsafe class HttpListenerContext /* BaseHttpContext, */
    {
        ...
        internal HttpListenerContext(HttpListener httpListener, RequestContextBase memoryBlob)
        {
            if (Logging.On)
                Logging.PrintInfo(
                    ...
                    ".ctor",
                    "httpListener#"
                        + ValidationHelper.HashString(httpListener)
                        + " requestBlob="
            ...
            GlobalLog.Print(
                "HttpListenerContext#"
                    ...
                    + "::.ctor() HttpListener#"
                    + ValidationHelper.HashString(m_Listener)
                    + " HttpListenerRequest#"
                    + ValidationHelper.HashString(m_Request)
            );
        }
        ...
        internal void SetIdentity(IPrincipal principal, string mutualAuthentication)
        {
            ...
            GlobalLog.Print(
                "HttpListenerContext#"
                    + ValidationHelper.HashString(this)
                    + "::SetIdentity() mutual:"
                    + (mutualAuthentication == null ? "<null>" : mutualAuthentication)
                    + " Principal#"
                    + ValidationHelper.HashString(principal)
            );
        }
            ...
            get
            {
                ...
                if (m_Response == null)
                {
                    ...
                    GlobalLog.Print(
                        "HttpListenerContext#"
                            + ValidationHelper.HashString(this)
                            + "::.Response_get() HttpListener#"
                            + ValidationHelper.HashString(m_Listener)
                            + " HttpListenerRequest#"
                            + ValidationHelper.HashString(m_Request)
                            + " HttpListenerResponse#"
                            + ValidationHelper.HashString(m_Response)
                    );
                }
                if (Logging.On)
        ...
        {
            get { return m_AuthenticationSchemes; }
            set { m_AuthenticationSchemes = value; }
        }
        ...
        {
            get { return m_ExtendedProtectionPolicy; }
            set { m_ExtendedProtectionPolicy = value; }
        }
        ...
        {
            get { return m_MutualAuthentication; }
        }
        ...
        {
            get { return m_Listener.RequestQueueHandle; }
        }
        ...
        {
            return WebSocketHelpers.AcceptWebSocketAsync(
                this,
                subProtocol,
                receiveBufferSize,
                keepAliveInterval,
        ...
        internal UnsafeNclNativeMethods.HttpApi.HTTP_VERB GetKnownMethod()
        {
            ...
            return UnsafeNclNativeMethods.HttpApi.GetKnownVerb(
                Request.RequestBuffer,
                Request.OriginalBlobAddress
            );
        }
        ...
        {
            uint statusCode = UnsafeNclNativeMethods.HttpApi.HttpCancelHttpRequest(
                requestQueueHandle,
                requestId,
                IntPtr.Zero
            );
