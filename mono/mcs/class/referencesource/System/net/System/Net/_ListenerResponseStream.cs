    ...
    unsafe class HttpResponseStream : Stream
    {
        ...
        internal UnsafeNclNativeMethods.HttpApi.HTTP_FLAGS ComputeLeftToWrite()
        {
            ...
            if (m_LeftToWrite == long.MinValue)
            {
                ...
                m_LeftToWrite =
                    method != UnsafeNclNativeMethods.HttpApi.HTTP_VERB.HttpVerbHEAD
                        ? m_HttpContext.Response.ContentLength64
                        : 0;
                GlobalLog.Print(
                    "HttpResponseStream#"
                        + ValidationHelper.HashString(this)
                        + "::ComputeLeftToWrite() computed m_LeftToWrite:"
                        + m_LeftToWrite
                );
            }
            return flags;
                    ...
                    fixed (byte* pDataBuffer = buffer)
                    {
                        ...
                        if (m_HttpContext.Response.BoundaryType == BoundaryType.Chunked)
                        {
                            ...
                            Marshal.Copy(
                                buffer,
                                offset,
                                IntPtrHelper.Add(pBufferAsIntPtr, chunkHeader.Length + 2),
                                size
                            );
                        ...
                        else
                        {
                            ...
                            GlobalLog.Print(
                                "HttpResponseStream#"
                                    + ValidationHelper.HashString(this)
                                    + "::Write() call to UnsafeNclNativeMethods.HttpApi.HttpSendResponseEntityBody returned:"
                                    + statusCode
                            );
        ...
        )
        {
            ...
            UpdateAfterWrite(
                (uint)((m_HttpContext.Response.BoundaryType == BoundaryType.Chunked) ? 0 : size)
            );
            ...
            try
            {
                ...
                else
                {
                    GlobalLog.Print(
                        "HttpResponseStream#"
                            + ValidationHelper.HashString(this)
                            + "::BeginWrite() calling UnsafeNclNativeMethods.HttpApi.HttpSendResponseEntityBody"
                    );
                    ...
                    GlobalLog.Print(
                        "HttpResponseStream#"
                            + ValidationHelper.HashString(this)
                            + "::BeginWrite() call to UnsafeNclNativeMethods.HttpApi.HttpSendResponseEntityBody returned:"
                            + statusCode
                    );
                }
            }
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            try
            {
                if (disposing)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            SR.GetString(SR.net_io_notenoughbyteswritten)
                        );
                    }
                    bool sentHeaders = m_HttpContext.Response.SentHeaders;
                    ...
                    if (
                        (
                            m_HttpContext.Response.BoundaryType == BoundaryType.Chunked
                            || m_HttpContext.Response.BoundaryType == BoundaryType.None
                        )
                        && (
                            String.Compare(
                                m_HttpContext.Request.HttpMethod,
                                "HEAD",
                        ...
                        fixed (void* pBuffer = NclConstants.ChunkTerminator)
                        {
                            ...
                            else
                            {
                                ...
                                GlobalLog.Print(
                                    "HttpResponseStream#"
                                        + ValidationHelper.HashString(this)
                                        + "::Close() call to UnsafeNclNativeMethods.HttpApi.HttpSendResponseEntityBody returned:"
                                        + statusCode
                                );
                                if (m_HttpContext.Listener.IgnoreWriteExceptions)
                                {
        ...
        {
            GlobalLog.Print(
                "HttpResponseStream#" + ValidationHelper.HashString(this) + "::SwitchToOpaqueMode()"
            );
            m_InOpaqueMode = true;
        ...
        [SuppressMessage(
            ...
            Justification = "It is safe to ignore the return value on a cancel operation because the connection is being closed"
        )]
    ...
    unsafe class HttpResponseStreamAsyncResult : LazyAsyncResult
    {
        ...
        internal UnsafeNclNativeMethods.HttpApi.HTTP_DATA_CHUNK* pDataChunks
        {
            get
            {
                ...
                {
                    return (UnsafeNclNativeMethods.HttpApi.HTTP_DATA_CHUNK*)(
                        Marshal.UnsafeAddrOfPinnedArrayElement(m_DataChunks, 0)
                    );
                }
            }
        }
            ...
            else
            {
                ...
                GlobalLog.Print(
                    "HttpResponseStreamAsyncResult#"
                        + ValidationHelper.HashString(this)
                        + "::.ctor() m_pOverlapped:0x"
                        + ((IntPtr)m_pOverlapped).ToString("x8")
                );
                ...
                if (chunked)
                {
                    m_DataChunks[0].pBuffer = (byte*)(
                        Marshal.UnsafeAddrOfPinnedArrayElement(chunkHeaderBuffer, chunkHeaderOffset)
                    );
                    m_DataChunks[1].pBuffer = (byte*)(
                        Marshal.UnsafeAddrOfPinnedArrayElement(buffer, offset)
                    );
                    m_DataChunks[2].pBuffer = (byte*)(
                        Marshal.UnsafeAddrOfPinnedArrayElement(NclConstants.CRLF, 0)
                    );
                }
                else
                {
        ...
        )
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        result =
                            asyncResult.m_DataChunks.Length == 1
                        ...
                        if (Logging.On)
                        {
                            for (int i = 0; i < asyncResult.m_DataChunks.Length; i++)
                            {
                                Logging.Dump(
                                    ...
                                    asyncResult,
                                    "Callback",
                                    (IntPtr)asyncResult.m_DataChunks[0].pBuffer,
                                    (int)asyncResult.m_DataChunks[0].BufferLength
                                );
                            }
                        }
                    }
        ...
        )
        {
            ...
            HttpResponseStreamAsyncResult asyncResult =
                callbackOverlapped.AsyncResult as HttpResponseStreamAsyncResult;
            GlobalLog.Print(
                "HttpResponseStreamAsyncResult#"
                    + ValidationHelper.HashString(asyncResult)
                    + "::Callback() errorCode:0x"
