...
namespace System.Net.Cache
{
    ...
    internal abstract class BaseWrapperStream : Stream, IRequestLifetimeTracker
    {
        ...
        public void TrackRequestLifetime(long requestStartTimestamp)
        {
            ...
            Debug.Assert(
                stream != null,
                "Wrapped stream must implement IRequestLifetimeTracker interface"
            );
    ...
    internal class CombinedReadStream : BaseWrapperStream, ICloseEx
    {
        ...
        {
            get { return WrappedStream.Length + (m_HeadEOF ? m_HeadLength : m_HeadStream.Length); }
        }
            ...
            try
            {
                if (Interlocked.Increment(ref m_ReadNesting) != 1)
                {
                    throw new NotSupportedException(
                        SR.GetString(SR.net_io_invalidnestedcall, "Read", "read")
                    );
                }
        ...
        {
            GlobalLog.Assert(
                transportResult.AsyncState is InnerAsyncResult,
                "InnerAsyncResult::ReadCallback|The state expected to be of type InnerAsyncResult, received {0}.",
            ...
            try
            {
                ...
                if (!m_HeadEOF && count == 0 && userResult.Count != 0)
                {
                    ...
                    m_HeadEOF = true;
                    m_HeadStream.Close();
                    IAsyncResult ar = WrappedStream.BeginRead(
                        userResult.Buffer,
                        userResult.Offset,
                        userResult.Count,
                        m_ReadCallback,
        ...
        )
        {
            try
            {
                ...
                {
                    throw new NotSupportedException(
                        SR.GetString(SR.net_io_invalidnestedcall, "BeginRead", "read")
                    );
                }
                ...
                {
                    InnerAsyncResult userResult = new InnerAsyncResult(
                        state,
                        callback,
                        buffer,
                        offset,
                    ...
                    IAsyncResult ar = m_HeadStream.BeginRead(
                        buffer,
                        offset,
                        count,
                        m_ReadCallback,
        ...
        public override int EndRead(IAsyncResult asyncResult)
        {
            ...
            if (myResult == null)
            {
                ...
                GlobalLog.Assert(
                    m_HeadEOF,
                    "CombinedReadStream::EndRead|m_HeadEOF is false and asyncResult is not of InnerAsyncResult type {0).",
                    asyncResult.GetType().FullName
                );
                return m_HeadEOF
                    ? WrappedStream.EndRead(asyncResult)
                    : m_HeadStream.EndRead(asyncResult);
            }
        ...
        {
            get { return WrappedStream.CanTimeout && m_HeadStream.CanTimeout; }
        }

        public override int ReadTimeout
        {
            get { return (m_HeadEOF) ? WrappedStream.ReadTimeout : m_HeadStream.ReadTimeout; }
            set { WrappedStream.ReadTimeout = m_HeadStream.ReadTimeout = value; }
        }

        public override int WriteTimeout
        {
            get { return (m_HeadEOF) ? WrappedStream.WriteTimeout : m_HeadStream.WriteTimeout; }
            set { WrappedStream.WriteTimeout = m_HeadStream.WriteTimeout = value; }
        }
    }
    ...
    internal class ForwardingReadStream : BaseWrapperStream, ICloseEx
    {
        ...
        internal ForwardingReadStream(
            Stream originalStream,
            Stream shadowStream,
            long bytesToSkip,
            bool throwOnWriteError
        )
            : base(originalStream)
        {
            if (!shadowStream.CanWrite)
            {
                throw new ArgumentException(
                    SR.GetString(SR.net_cache_shadowstream_not_writable),
                    "shadowStream"
                );
            }
        ...
        public override long Length
        {
            get { return WrappedStream.Length - m_BytesToSkip; }
        }

        public override long Position
        {
            get { return WrappedStream.Position - m_BytesToSkip; }
            set { throw new NotSupportedException(SR.GetString(SR.net_noseek)); }
        }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            try
            {
                if (m_BytesToSkip != 0L)
                {
                    ...
                    {
                        int bytes = WrappedStream.Read(
                            tempBuffer,
                            0,
                            (
                                m_BytesToSkip < (long)tempBuffer.Length
                                    ? (int)m_BytesToSkip
                                    : tempBuffer.Length
                            )
                        );
            ...
            catch (Exception e)
            {
                ...
                GlobalLog.Print(
                    "ShadowReadStream::Read() Got Exception, disabling the shadow stream, stack trace = "
                        + e.ToString()
                );
                if (!m_ShadowStreamIsDead)
                {
                    ...
                    try
                    {
                        if (m_ShadowStream is ICloseEx)
                            ((ICloseEx)m_ShadowStream).CloseEx(
                    ...
                    catch (Exception ee)
                    {
                        if (
                            ...
                            throw;
                        GlobalLog.Print(
                            "ShadowReadStream::Read() Got (ignoring) Exception, on shadow stream.Close, stack trace = "
                                + ee.ToString()
                        );
                    }
                }
        ...
        private void ReadComplete(IAsyncResult transportResult)
        {
            while (true)
            {
                ...
                try
                {
                    if (!userResult.IsWriteCompletion)
                    {
                        ...
                        if (!m_ShadowStreamIsDead)
                        {
                            ...
                            transportResult = m_ShadowStream.BeginWrite(
                                userResult.Buffer,
                                userResult.Offset,
                                userResult.Count,
                                m_ReadCallback,
                    ...
                    else
                    {
                        GlobalLog.Assert(
                            ...
                            "ForwardingReadStream::ReadComplete|ERROR: IsWriteCompletion && m_ShadowStreamIsDead"
                        );
                ...
                catch (Exception e)
                {
                    ...
                    {
                        GlobalLog.Print(
                            "ShadowReadStream::ReadComplete() Rethrowing Exception (end), userResult.IsCompleted, stack trace = "
                                + e.ToString()
                        );
                        throw;
                    }
                    ...
                    catch (Exception ee)
                    {
                        ...
                        GlobalLog.Print(
                            "ShadowReadStream::ReadComplete() Got (ignoring) Exception, on shadow stream.Close, stack trace = "
                                + ee.ToString()
                        );
                    }
                ...
                try
                {
                    if (m_BytesToSkip != 0L)
                    {
                        ...
                        userResult.Count =
                            m_BytesToSkip < (long)userResult.Buffer.Length
                                ? (int)m_BytesToSkip
                        ...
                        if (m_BytesToSkip == 0L)
                        {
                            ...
                            GlobalLog.Assert(
                                userResult != null,
                                "ForwardingReadStream::ReadComplete|ERROR: Inner IAResult is null after stream FastForwarding."
                            );
                        }
                        transportResult = WrappedStream.BeginRead(
                            userResult.Buffer,
                            userResult.Offset,
                            userResult.Count,
                            m_ReadCallback,
                ...
                catch (Exception e)
                {
                    ...
                    {
                        GlobalLog.Print(
                            "ShadowReadStream::ReadComplete() Rethrowing Exception (begin), userResult.IsCompleted, stack trace = "
                                + e.ToString()
                        );
                        throw;
                    }
                    ...
                    catch (Exception ee)
                    {
                        //ASYNC: Again try to ignore even serious exceptions
                        GlobalLog.Print(
                            "ShadowReadStream::ReadComplete() Got (ignoring) Exception, on shadow stream.Close (after begin), stack trace = "
                                + ee.ToString()
                        );
                    }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (m_BytesToSkip != 0L)
                    {
                        ...
                        userResult = new InnerAsyncResult(
                            temp,
                            null,
                            new byte[4096],
                            0,
                            m_BytesToSkip < (long)buffer.Length ? (int)m_BytesToSkip : buffer.Length
                        );
                    }
                    IAsyncResult result = WrappedStream.BeginRead(
                        userResult.Buffer,
                        userResult.Offset,
                        userResult.Count,
                        m_ReadCallback,
        ...
        public override int EndRead(IAsyncResult asyncResult)
        {
            ...
            if (myResult == null)
            {
                ...
                GlobalLog.Assert(
                    m_ShadowStreamIsDead,
                    "ForwardingReadStream::EndRead|m_ShadowStreamIsDead is false and asyncResult is not of InnerAsyncResult type {0}.",
                    asyncResult.GetType().FullName
                );
            ...
            if (Interlocked.Increment(ref _Disposed) == 1)
            {
                ...
                if (closeState == CloseExState.Silent)
                {
                    try
                    {
                        ...
                        int bytesRead;
                        while (
                            total < ConnectStream.s_DrainingBuffer.Length
                            && (
                                bytesRead = Read(
                                    ConnectStream.s_DrainingBuffer,
                                    0,
                                    ConnectStream.s_DrainingBuffer.Length
                                )
                            ) > 0
                        )
                        {
        ...
        public override bool CanTimeout
        {
            get { return WrappedStream.CanTimeout && m_ShadowStream.CanTimeout; }
        }

        public override int ReadTimeout
        {
            get { return WrappedStream.ReadTimeout; }
            set { WrappedStream.ReadTimeout = m_ShadowStream.ReadTimeout = value; }
        }

        public override int WriteTimeout
        {
            get { return m_ShadowStream.WriteTimeout; }
            set { WrappedStream.WriteTimeout = m_ShadowStream.WriteTimeout = value; }
        }
    }
    ...
    internal class RangeStream : BaseWrapperStream, ICloseEx
    {
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            {
                throw new NotSupportedException(
                    SR.GetString(SR.net_cache_unsupported_partial_stream)
                );
            }
            WrappedStream.Write(buffer, offset, count);
            m_Position += count;
