...
namespace System.Net
{
    ...
    internal class NTAuthentication
    {
        ...
        {
            get { return !(m_SecurityContext == null || m_SecurityContext.IsInvalid); }
        }
        ...
        {
            get { return (m_ContextFlags & ContextFlags.Confidentiality) != 0; }
        }
        ...
        internal bool IsIntegrityFlag
        {
            get
            {
                return (
                        m_ContextFlags
                        & (m_IsServer ? ContextFlags.AcceptIntegrity : ContextFlags.InitIntegrity)
                    ) != 0;
            }
        }
        ...
        {
            get { return (m_ContextFlags & ContextFlags.MutualAuth) != 0; }
        }

        internal bool IsDelegationFlag
        {
            get { return (m_ContextFlags & ContextFlags.Delegate) != 0; }
        }
        ...
        internal bool IsIdentifyFlag
        {
            get
            {
                return (
                        m_ContextFlags
                        & (m_IsServer ? ContextFlags.AcceptIdentify : ContextFlags.InitIdentify)
                    ) != 0;
            }
        }
        ...
        internal bool OSSupportsExtendedProtection
        {
            ...
            {
                GlobalLog.Assert(
                    IsCompleted && IsValidContext,
                    "NTAuthentication#{0}::OSSupportsExtendedProtection|The context is not completed or invalid.",
        ...
        internal SecSizes Sizes
        {
            get
            {
                GlobalLog.Assert(
                    IsCompleted && IsValidContext,
                    "NTAuthentication#{0}::MaxDataSize|The context is not completed or invalid.",
                    ValidationHelper.HashString(this)
                );
                if (m_Sizes == null)
                {
        ...
        internal NTAuthentication(
            ...
            string spn,
            ContextFlags requestedContextFlags,
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ExecutionContext.Run(
                                x,
                                s_InitializeCallback,
                                new InitializeCallbackContext(
                                    this,
                                    isServer,
                                    package,
                                    credential,
        ...
        )
        {
            GlobalLog.Print(
                "NTAuthentication#"
                    + ValidationHelper.HashString(this)
                    + "::.ctor() package:"
                    + ValidationHelper.ToString(package)
                    + " spn:"
                    + ValidationHelper.ToString(spn)
                    + " flags :"
                    + requestedContextFlags.ToString()
            );
            m_TokenSize = SSPIWrapper
                .GetVerifyPackageInfo(GlobalSSPI.SSPIAuth, package, true)
                .MaxToken;
            m_IsServer = isServer;
            m_Spn = spn;
            ...
            else
            {
                ...
                AuthIdentity authIdentity = new AuthIdentity(
                    username,
                    credential.InternalGetPassword(),
                    (object)package == (object)NegotiationInfoClass.WDigest
                ...
                GlobalLog.Print(
                    "NTAuthentication#"
                        + ValidationHelper.HashString(this)
                        + "::.ctor(): using authIdentity:"
                        + authIdentity.ToString()
                );
        ...
        internal SafeCloseHandle GetContextToken(out SecurityStatus status)
        {
            GlobalLog.Assert(
                IsCompleted && IsValidContext,
                "NTAuthentication#{0}::GetContextToken|Should be called only when completed with success, currently is not!",
                ValidationHelper.HashString(this)
            );
            GlobalLog.Assert(
                IsServer,
                "NTAuthentication#{0}::GetContextToken|The method must not be called by the client side!",
                ValidationHelper.HashString(this)
            );
        ...
        {
            GlobalLog.Enter(
                "NTAuthentication#" + ValidationHelper.HashString(this) + "::GetOutgoingBlob",
                incomingBlob
            );
            ...
            if ((IsValidContext || IsCompleted) && decodedIncomingBlob == null)
            {
                ...
                GlobalLog.Print(
                    "NTAuthentication#"
                        + ValidationHelper.HashString(this)
                        + "::GetOutgoingBlob() null blob AND m_SecurityContext#"
                        + ValidationHelper.HashString(m_SecurityContext)
                        + "::Handle:[0x"
                        + m_SecurityContext.ToString()
                        + "]"
                );
                m_IsCompleted = true;
            }
        ...
        internal byte[] GetOutgoingBlob(
            byte[] incomingBlob,
            bool throwOnError,
            out SecurityStatus statusCode
        )
        {
            GlobalLog.Enter(
                "NTAuthentication#" + ValidationHelper.HashString(this) + "::GetOutgoingBlob",
                (
                    (incomingBlob == null)
                        ? "0"
                        : incomingBlob.Length.ToString(NumberFormatInfo.InvariantInfo)
                ) + " bytes"
            );
            ...
            try
            {
                if (!m_IsServer)
                {
                    ...
                    GlobalLog.Print(
                        "NTAuthentication#"
                            + ValidationHelper.HashString(this)
                            + "::GetOutgoingBlob() SSPIWrapper.InitializeSecurityContext() returns statusCode:0x"
                    ...
                    if (statusCode == SecurityStatus.CompleteNeeded)
                    {
                        ...
                        GlobalLog.Print(
                            "NTAuthentication#"
                                + ValidationHelper.HashString(this)
                                + "::GetOutgoingDigestBlob() SSPIWrapper.CompleteAuthToken() returns statusCode:0x"
                                + ((int)statusCode).ToString("x8", NumberFormatInfo.InvariantInfo)
                                + " ("
                                + statusCode.ToString()
                                + ")"
                ...
                else
                {
                    ...
                    GlobalLog.Print(
                        "NTAuthentication#"
                            + ValidationHelper.HashString(this)
                            + "::GetOutgoingBlob() SSPIWrapper.AcceptSecurityContext() returns statusCode:0x"
                            + ((int)statusCode).ToString("x8", NumberFormatInfo.InvariantInfo)
                            + " ("
                            + statusCode.ToString()
                            + ")"
            ...
            if (statusCode == SecurityStatus.OK)
            {
                ...
                GlobalLog.Assert(
                    ...
                    statusCode,
                    ValidationHelper.HashString(m_SecurityContext),
                    ValidationHelper.ToString(m_SecurityContext)
                );
                m_IsCompleted = true;
            }
            else
            {
                ...
                GlobalLog.Print(
                    "NTAuthentication#"
                        + ValidationHelper.HashString(this)
                        + "::GetOutgoingBlob() need continue statusCode:[0x"
                        + ((int)statusCode).ToString("x8", NumberFormatInfo.InvariantInfo)
                        + "] ("
                        + statusCode.ToString()
                        + ") m_SecurityContext#"
        ...
        )
        {
            ...
            SecurityBuffer outSecurityBuffer = new SecurityBuffer(
                m_TokenSize,
                isClientPreAuth ? BufferType.Parameters : BufferType.Token
            );
            ...
            try
            {
                if (!m_IsServer)
                {
                    ...
                    if (!isClientPreAuth)
                    {
                        if (incomingBlob != null)
                        {
                            ...
                            list.Add(
                                new SecurityBuffer(
                                    WebHeaderCollection.HeaderEncoding.GetBytes(incomingBlob),
                                    BufferType.Token
                                )
                            );
                            list.Add(
                            ...
                            list.Add(
                                new SecurityBuffer(
                                    Encoding.Unicode.GetBytes(m_Spn),
                        ...
                        GlobalLog.Print(
                            "NTAuthentication#"
                                + ValidationHelper.HashString(this)
                                + "::GetOutgoingDigestBlob() SSPIWrapper.InitializeSecurityContext() returns statusCode:0x"
                                + ((int)statusCode).ToString("x8", NumberFormatInfo.InvariantInfo)
                                + " ("
                                + statusCode.ToString()
                                + ")"
                ...
                else
                {
                    ...
                    list.Add(
                        incomingBlob == null
                            ? new SecurityBuffer(0, BufferType.Token)
                            : new SecurityBuffer(
                                WebHeaderCollection.HeaderEncoding.GetBytes(incomingBlob),
                                BufferType.Token
                            )
                    ...
                    list.Add(
                        realm == null
                            ? new SecurityBuffer(0, BufferType.Parameters)
                            : new SecurityBuffer(
                                Encoding.Unicode.GetBytes(realm),
                                BufferType.Parameters
                            )
                    );
                    ...
                    GlobalLog.Print(
                        "NTAuthentication#"
                            + ValidationHelper.HashString(this)
                            + "::GetOutgoingDigestBlob() SSPIWrapper.AcceptSecurityContext() returns statusCode:0x"
                            + ((int)statusCode).ToString("x8", NumberFormatInfo.InvariantInfo)
                            + " ("
                            + statusCode.ToString()
                            + ")"
            ...
            else
            {
                ...
                GlobalLog.Print(
                    "NTAuthentication#"
                        + ValidationHelper.HashString(this)
                        + "::GetOutgoingDigestBlob() need continue statusCode:[0x"
                        + ((int)statusCode).ToString("x8", NumberFormatInfo.InvariantInfo)
                        + "] ("
                        + statusCode.ToString()
                        + ") m_SecurityContext#"
            ...
            GlobalLog.Print(
                "NTAuthentication#"
                    + ValidationHelper.HashString(this)
                    + "::GetOutgoingDigestBlob() IsCompleted:"
        ...
        )
        {
            ...
            try
            {
                int maxCount = checked(
                    Int32.MaxValue - 4 - sizes.BlockSize - sizes.SecurityTrailer
                );
                ...
                if (count > maxCount || count < 0)
                {
                    throw new ArgumentOutOfRangeException(
                        "count",
                        SR.GetString(SR.net_io_out_range, maxCount)
                    );
                }
            }
            catch (Exception e)
            {
                if (!NclUtilities.IsFatal(e))
                {
                    GlobalLog.Assert(
                        false,
                        "NTAuthentication#" + ValidationHelper.HashString(this) + "::Encrypt",
            ...
            securityBuffer[1] = new SecurityBuffer(
                output,
                4 + sizes.SecurityTrailer,
                count,
                BufferType.Data
            );
            securityBuffer[2] = new SecurityBuffer(
                output,
                4 + sizes.SecurityTrailer + count,
                sizes.BlockSize,
                BufferType.Padding
            );
            ...
            {
                errorCode = SSPIWrapper.EncryptMessage(
                    GlobalSSPI.SSPIAuth,
                    m_SecurityContext,
                    securityBuffer,
                    sequenceNumber
            ...
            else
            {
                ...
                errorCode = SSPIWrapper.MakeSignature(
                    GlobalSSPI.SSPIAuth,
                    m_SecurityContext,
                    securityBuffer,
                    0
            ...
            if (resultSize != sizes.SecurityTrailer)
            {
                ...
                Buffer.BlockCopy(
                    output,
                    securityBuffer[1].offset,
                    output,
                    4 + resultSize,
                    securityBuffer[1].size
                );
            }
            ...
            if (
                ...
                && (forceCopy || resultSize != (count + sizes.SecurityTrailer))
            )
                Buffer.BlockCopy(
                    output,
        ...
        )
        {
            if (offset < 0 || offset > (payload == null ? 0 : payload.Length))
            {
                GlobalLog.Assert(
                    false,
                    "NTAuthentication#" + ValidationHelper.HashString(this) + "::Decrypt",
            ...
            if (IsConfidentialityFlag)
            {
                errorCode = SSPIWrapper.DecryptMessage(
                    GlobalSSPI.SSPIAuth,
                    m_SecurityContext,
                    securityBuffer,
                    expectedSeqNumber
            ...
            {
                errorCode = SSPIWrapper.VerifySignature(
                    GlobalSSPI.SSPIAuth,
                    m_SecurityContext,
                    securityBuffer,
                    expectedSeqNumber
        ...
        {
            GlobalLog.Assert(
                IsValidContext && IsCompleted,
                "NTAuthentication: Trying to get the client SPN before handshaking is done!"
            );
        ...
        )
        {
            ...
            if (count < 16)
            {
                GlobalLog.Assert(
                    false,
                    "NTAuthentication#" + ValidationHelper.HashString(this) + "::DecryptNtlm",
                    "Argument 'count' out of range."
                );
                throw new ArgumentOutOfRangeException("count");
            }
        ...
        internal int VerifySignature(byte[] buffer, int offset, int count)
        {
            ...
            if (offset < 0 || offset > (buffer == null ? 0 : buffer.Length))
            {
                GlobalLog.Assert(
                    false,
                    "NTAuthentication#" + ValidationHelper.HashString(this) + "::VerifySignature",
                    "Argument 'offset' out of range."
                );
                throw new ArgumentOutOfRangeException("offset");
            }
            ...
            SecurityBuffer[] securityBuffer = new SecurityBuffer[2];
            securityBuffer[0] = new SecurityBuffer(buffer, offset, count, BufferType.Stream);
            securityBuffer[1] = new SecurityBuffer(0, BufferType.Data);
