                ...
                else
                {
                    ...
                    if (errorCode == UnsafeNclNativeMethods.ErrorCodes.ERROR_SUCCESS)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                result = httpWebListener.HandleAuthentication(
            ...
            while (true)
            {
                GlobalLog.Print(
                    "ListenerAsyncResult#"
                        ...
                        + "::QueueBeginGetContext() calling UnsafeNclNativeMethods.HttpApi.HttpReceiveHttpRequest RequestId:"
                        + m_RequestContext.RequestBlob->RequestId
                        + " Buffer:0x"
                        + ((IntPtr)m_RequestContext.RequestBlob).ToString("x")
                        + " Size:"
                        + m_RequestContext.Size.ToString()
                );
                ...
                GlobalLog.Print(
                    "ListenerAsyncResult#"
                        + ValidationHelper.HashString(this)
                        + "::QueueBeginGetContext() call to UnsafeNclNativeMethods.HttpApi.HttpReceiveHttpRequest returned:"
                        + statusCode
                );
                if (
