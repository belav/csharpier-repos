...
namespace System.Net.NetworkInformation
{
    ...
    internal class SystemIPv4InterfaceProperties : IPv4InterfaceProperties
    {
        ...
        {
            get { return autoConfigEnabled; }
        } // proto

        public override bool IsAutomaticPrivateAddressingActive
        {
            get { return autoConfigActive; }
        }
        ...
        private void GetPerAdapterInfo(uint index)
        {
            if (index != 0)
            {
                ...
                uint result = UnsafeNetInfoNativeMethods.GetPerAdapterInfo(
                    index,
                    SafeLocalFree.Zero,
                    ref size
                );
                while (result == IpHelperErrors.ErrorBufferOverflow)
                {
                    try
                    {
                        ...
                        result = UnsafeNetInfoNativeMethods.GetPerAdapterInfo(
                            index,
                            buffer,
                            ref size
                        );
                        ...
                        {
                            IpPerAdapterInfo ipPerAdapterInfo = (IpPerAdapterInfo)
