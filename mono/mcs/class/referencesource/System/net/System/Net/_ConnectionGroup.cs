...
namespace System.Net
{
    ...
    internal class ConnectionGroup
    {
        ...
        internal ConnectionGroup(ServicePoint servicePoint, string connName)
        {
            ...
            GlobalLog.Print(
                "ConnectionGroup::.ctor m_ConnectionLimit:" + m_ConnectionLimit.ToString()
            );
        ...
        internal int ConnectionLimit
        {
            ...
            set
            {
                ...
                GlobalLog.Print(
                    "ConnectionGroup::ConnectionLimit.set m_ConnectionLimit:"
                        + m_ConnectionLimit.ToString()
                );
            }
        }
        ...
        private Queue AuthenticationRequestQueue
        {
            ...
            set { m_AuthenticationRequestQueue = value; }
        }
        ...
        internal void DisableKeepAliveOnConnections()
        {
            ...
            lock (m_ConnectionList)
            {
                GlobalLog.Print(
                    "ConnectionGroup#"
                        + ValidationHelper.HashString(this)
                        + "::DisableKeepAliveOnConnections() Name = "
                        + m_Name
                        + ", Count:"
                        + m_ConnectionList.Count
                );
                foreach (Connection currentConnection in m_ConnectionList)
                {
                    ...
                    GlobalLog.Print(
                        "ConnectionGroup#"
                            + ValidationHelper.HashString(this)
                            + "::DisableKeepAliveOnConnections() setting KeepAlive to false Connection#"
                            + ValidationHelper.HashString(currentConnection)
                    );
        ...
        private Connection FindMatchingConnection(
            ...
            string connName,
            out Connection leastbusyConnection
        )
        {
            ...
            lock (m_ConnectionList)
            {
                ...
                {
                    GlobalLog.Print(
                        "ConnectionGroup::FindMatchingConnection currentConnection.BusyCount:"
                            + currentConnection.BusyCount.ToString()
                    );
                    ...
                    GlobalLog.Print(
                        "ConnectionGroup::FindMatchingConnection: lockedRequest# "
                            + (
                                (currentConnection.LockedRequest == null)
                                    ? "null"
                                    : currentConnection.LockedRequest.GetHashCode().ToString()
                            )
                    );
                    if (
                        !currentConnection.NonKeepAliveRequestPipelined
                        && currentConnection.BusyCount < minBusyCount
                        && currentConnection.LockedRequest == null
                    )
                    {
                        leastbusyConnection = currentConnection;
                ...
                if (!freeConnectionsAvail && CurrentConnections < ConnectionLimit)
                {
                    ...
                    GlobalLog.Print(
                        "ConnectionGroup::FindMatchingConnection [returning new Connection] CurrentConnections:"
                            + CurrentConnections.ToString()
                            + " ConnectionLimit:"
        ...
        )
        {
            ...
            GlobalLog.Print(
                "ConnectionGroup::FindConnectionAuthenticationGroup ["
                    + connName
                    + "] for request#"
                    + request.GetHashCode()
                    + ", m_ConnectionList.Count:"
                    + m_ConnectionList.Count.ToString()
            );
                    ...
                    if (leastBusyConnection != null)
                    {
                        ...
                        if (
                            ...
                            || (
                                m_NtlmNegGroup
                                && !request.Pipelined
                                && request.UnsafeOrProxyAuthenticatedConnectionSharing
                                && m_IISVersion >= 6
                            )
                        )
                        {
                            GlobalLog.Print(
                                "Assigning New Locked Request#" + request.GetHashCode().ToString()
                            );
        ...
        )
        {
            ...
            GlobalLog.Print(
                "ConnectionGroup::FindConnection ["
                    + connName
                    + "] m_ConnectionList.Count:"
                    + m_ConnectionList.Count.ToString()
            );
            ...
            lock (m_ConnectionList)
            {
                ...
                {
                    GlobalLog.Print(
                        "ConnectionGroup::FindConnection currentConnection.BusyCount:"
                            + currentConnection.BusyCount.ToString()
                    );
                    ...
                    if (useThisConnection)
                    {
                        ...
                        else
                        {
                            GlobalLog.Assert(
                                !currentConnection.NonKeepAliveRequestPipelined,
                                "Connection.NonKeepAliveRequestPipelined == false|Non keep-alive request has been pipelined on this connection."
                            );
                        }
                ...
                if (!freeConnectionsAvail && CurrentConnections < ConnectionLimit)
                {
                    ...
                    GlobalLog.Print(
                        "ConnectionGroup::FindConnection [returning new Connection] freeConnectionsAvail:"
                            + freeConnectionsAvail.ToString()
                            + " CurrentConnections:"
                            + CurrentConnections.ToString()
                            + " ConnectionLimit:"
                            + ConnectionLimit.ToString()
                ...
                else
                {
                    ...
                    GlobalLog.Print(
                        "ConnectionGroup::FindConnection [returning leastbusyConnection] freeConnectionsAvail:"
                            + freeConnectionsAvail.ToString()
                            + " CurrentConnections:"
                            + CurrentConnections.ToString()
                            + " ConnectionLimit:"
                    ...
                    GlobalLog.Assert(
                        leastbusyConnection != null,
                        "Connect.leastbusyConnection != null|All connections have BusyCount equal to Int32.MaxValue."
                    );
