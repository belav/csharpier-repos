...
namespace System.Net
{
    ...
    public class CredentialCache : ICredentials, ICredentialsByHost, IEnumerable
    {
        ...
        {
            get { return m_NumbDefaultCredInCache != 0; }
        }
        ...
        public void Add(Uri uriPrefix, string authType, NetworkCredential cred)
        {
            ...
            if (
                ...
                && !(
                    ...
                    || (
                        DigestClient.WDigestAvailable
                        && (
                            string.Compare(
                                authType,
                                DigestClient.AuthType,
                                StringComparison.OrdinalIgnoreCase
                            ) == 0
                        )
                    )
            ...
            )
            {
                throw new ArgumentException(
                    SR.GetString(SR.net_nodefaultcreds, authType),
                    "authType"
                );
            }
            ...
            GlobalLog.Print(
                "CredentialCache::Add() Adding key:["
                    + key.ToString()
                    + "], cred:["
                    + cred.Domain
                    + "],["
        ...
        )
        {
            ...
            {
                throw new ArgumentException(
                    SR.GetString(SR.net_nodefaultcreds, authenticationType),
                    "authenticationType"
                );
            }
        ...
        public NetworkCredential GetCredential(Uri uriPrefix, string authType)
        {
            ...
            GlobalLog.Print(
                "CredentialCache::GetCredential(uriPrefix=\""
                    + uriPrefix
                    + "\", authType=\""
                    + authType
                    + "\")"
            ...
            GlobalLog.Print(
                "CredentialCache::GetCredential returning "
                    + (
                        (mostSpecificMatch == null)
                            ? "null"
                            : "("
                                + mostSpecificMatch.UserName
                                + ":"
        ...
        public NetworkCredential GetCredential(string host, int port, string authenticationType)
        {
            ...
            GlobalLog.Print(
                "CredentialCache::GetCredential(host=\""
                    + host
                    + ":"
                    + port.ToString()
                    + "\", authenticationType=\""
            ...
            GlobalLog.Print(
                "CredentialCache::GetCredential returning "
                    + ((match == null) ? "null" : "(" + match.UserName + ":" + match.Domain + ")")
            );
            return match;
        }
    ...
    internal class CredentialHostKey
    {
        ...
        public override int GetHashCode()
        {
            if (!m_ComputedHashCode)
            {
                ...
                m_HashCode =
                    AuthenticationType.ToUpperInvariant().GetHashCode()
        ...
        public override bool Equals(object comparand)
        {
            ...
            GlobalLog.Print(
                "CredentialKey::Equals("
                    + ToString()
                    + ", "
                    + comparedCredentialKey.ToString()
                    + ") returns "
                    + equals.ToString()
            );
        ...
        {
            return "["
                + Host.Length.ToString(NumberFormatInfo.InvariantInfo)
                + "]:"
                + Host
                + ":"
    ...
    internal class CredentialKey
    {
        ...
        internal bool Match(Uri uri, string authenticationType)
        {
            ...
            GlobalLog.Print(
                "CredentialKey::Match(" + UriPrefix.ToString() + " & " + uri.ToString() + ")"
            );
        ...
        {
            return "["
                + UriPrefixLength.ToString(NumberFormatInfo.InvariantInfo)
                + "]:"
                + ValidationHelper.ToString(UriPrefix)
                + ":"
                + ValidationHelper.ToString(AuthenticationType);
        }
    } // class CredentialKey
