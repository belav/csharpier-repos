...
namespace System.Net
{
    ...
    internal class AuthenticationState
    {
        ...
        internal NTAuthentication GetSecurityContext(IAuthenticationModule module)
        {
            GlobalLog.Print(
                "AuthenticationState#"
                    ...
                    + ") returning NTAuthentication#"
                    + ValidationHelper.HashString(
        ...
        internal void SetSecurityContext(
            NTAuthentication securityContext,
        ...
        {
            Uri newUri = IsProxyAuth
                ? httpWebRequest.ServicePoint.InternalAddress
                : httpWebRequest.GetRemoteResourceUri();

            if ((object)ChallengedUri != (object)newUri)
            {
                if (
                    (object)ChallengedUri == null
                    || (object)ChallengedUri.Scheme != (object)newUri.Scheme
                    || ChallengedUri.Host != newUri.Host
                    || ChallengedUri.Port != newUri.Port
                )
                {
                    //
        ...
        internal SpnToken GetComputeSpn(HttpWebRequest httpWebRequest)
        {
            ...
            string spnKey = httpWebRequest.ChallengedUri
                .GetParts(
            ...
            if (spnToken == null || spnToken.Spn == null)
            {
                ...
                if (
                    !IsProxyAuth
                    && (
                        httpWebRequest.ServicePoint.InternalProxyServicePoint
                ...
                )
                {
                    ...
                    if (
                        httpWebRequest.ChallengedUri.HostNameType != UriHostNameType.IPv6
                    ...
                    )
                    {
                        ...
                        catch (Exception exception)
                        {
                            ...
                            GlobalLog.Print(
                                "AuthenticationState#"
                                    + ValidationHelper.HashString(this)
                                    + "::GetComputeSpn() GetHostByName(host) failed:"
        ...
        internal void PreAuthIfNeeded(HttpWebRequest httpWebRequest, ICredentials authInfo)
        {
            ...
            GlobalLog.Print(
                "AuthenticationState#"
                    + ValidationHelper.HashString(this)
                    + "::PreAuthIfNeeded() TriedPreAuth:"
                    + TriedPreAuth.ToString()
                    + " authInfo:"
                    + ValidationHelper.HashString(authInfo)
            );
            if (!TriedPreAuth)
            {
                TriedPreAuth = true;
                if (authInfo != null)
                {
                    ...
                    try
                    {
                        ...
                        GlobalLog.Print(
                            "AuthenticationState#"
                                + ValidationHelper.HashString(this)
                                + "::PreAuthIfNeeded() preauth:"
                                + ValidationHelper.HashString(preauth)
                        );
                        if (preauth != null && preauth.Message != null)
                        {
                            GlobalLog.Print(
                                "AuthenticationState#"
                                    + ValidationHelper.HashString(this)
                                    + "::PreAuthIfNeeded() setting TriedPreAuth to Complete:"
                                    + preauth.Complete.ToString()
                            );
                    ...
                    catch (Exception exception)
                    {
                        GlobalLog.Print(
                            "AuthenticationState#"
                                + ValidationHelper.HashString(this)
                                + "::PreAuthIfNeeded() PreAuthenticate() returned exception:"
                                + exception.Message
                        );
        ...
        internal bool AttemptAuthenticate(HttpWebRequest httpWebRequest, ICredentials authInfo)
        {
            ...
            if (Authorization != null && Authorization.Complete)
            {
                ...
                GlobalLog.Print(
                    "AuthenticationState#"
                        + ValidationHelper.HashString(this)
                        + "::AttemptAuthenticate() Authorization!=null Authorization.Complete:"
                        + Authorization.Complete.ToString()
                );
                if (IsProxyAuth)
                {
                    //
                    ...
                    GlobalLog.Print(
                        "AuthenticationState#"
                            + ValidationHelper.HashString(this)
                            + "::AttemptAuthenticate() ProxyAuth cleaning up auth status"
                    );
            ...
            if (authInfo == null)
            {
                GlobalLog.Print(
                    "AuthenticationState#"
                        + ValidationHelper.HashString(this)
                        + "::AttemptAuthenticate() authInfo==null Authorization#"
            ...
            if (challenge == null)
            {
                ...
                if (
                    !IsProxyAuth
                    && Authorization != null
                    && httpWebRequest.ProxyAuthenticationState.Authorization != null
                )
                {
                ...
                GlobalLog.Print(
                    "AuthenticationState#"
                        + ValidationHelper.HashString(this)
                        + "::AttemptAuthenticate() challenge==null Authorization#"
                        + ValidationHelper.HashString(Authorization)
                );
                return false;
            }
            ...
            try
            {
                Authorization = AuthenticationManager.Authenticate(
                    challenge,
                    httpWebRequest,
                    authInfo
                );
            ...
            if (Authorization == null)
            {
                GlobalLog.Print(
                    "AuthenticationState#"
                        + ValidationHelper.HashString(this)
                        + "::AttemptAuthenticate() Authorization==null"
                );
                return false;
            }
            if (Authorization.Message == null)
            {
                GlobalLog.Print(
                    "AuthenticationState#"
                        + ValidationHelper.HashString(this)
                        + "::AttemptAuthenticate() Authorization.Message==null"
                );
                Authorization = null;
                return false;
            }
            ...
            GlobalLog.Print(
                "AuthenticationState#"
                    + ValidationHelper.HashString(this)
                    + "::AttemptAuthenticate() AuthorizationHeader:"
                    + AuthorizationHeader
                    + " blob: "
                    + Authorization.Message.Length
                    + "bytes Complete:"
                    + Authorization.Complete.ToString()
            );
        ...
        internal void Update(HttpWebRequest httpWebRequest)
        {
            ...
            GlobalLog.Print(
                "AuthenticationState#"
                    + ValidationHelper.HashString(this)
                    + "::Update() httpWebRequest#"
                    + ValidationHelper.HashString(httpWebRequest)
                    + " Authorization#"
                    + ValidationHelper.HashString(Authorization)
                    + " ResponseStatusCode:"
                    + httpWebRequest.ResponseStatusCode.ToString()
            );
            ...
            if (Authorization != null)
            {
                ...
                if (myModule != null)
                {
                    ...
                    GlobalLog.Print(
                        "AuthenticationState#"
                            ...
                            + Authorization.Complete.ToString()
                            + " Module:"
                            + ValidationHelper.ToString(Module)
                            + " challenge:"
                    ...
                    )
                    {
                        ...
                        GlobalLog.Print(
                            "AuthenticationState#"
                                + ValidationHelper.HashString(this)
                                + "::Update() skipping call to "
                                + myModule.ToString()
                                + ".Update() since we need to reauthenticate with the proxy"
                        );
                    }
                    else
                    {
                        ...
                        catch (Exception exception)
                        {
                            ...
                            if (
                                (
                                    httpWebRequest.AuthenticationLevel
                                    == AuthenticationLevel.MutualAuthRequired
                                )
                                && (
                                    httpWebRequest.CurrentAuthenticationState == null
                ...
                )
                {
                    GlobalLog.Print(
                        "AuthenticationState#"
                            + ValidationHelper.HashString(this)
                            + "::Update() handshake is Complete calling BindModule()"
                    );
