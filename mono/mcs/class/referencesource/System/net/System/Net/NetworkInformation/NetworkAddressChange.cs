namespace System.Net.NetworkInformation
{
    ...
    public class NetworkChange
    {
        ...
        internal static class AvailabilityChangeListener
        {
            ...
            {
                ((NetworkAvailabilityChangedEventHandler)state)(
                    null,
                    new NetworkAvailabilityEventArgs(isAvailable)
                );
            }
            ...
            private static void ChangedAddress(object sender, EventArgs eventArgs)
            {
                lock (syncObject)
                {
                    ...
                    if (isAvailableNow != isAvailable)
                    {
                        ...
                        DictionaryEntry[] callerArray = new DictionaryEntry[
                            s_availabilityCallerArray.Count
                        ];
                        s_availabilityCallerArray.CopyTo(callerArray, 0);

                        for (int i = 0; i < callerArray.Length; i++)
                        {
                            NetworkAvailabilityChangedEventHandler handler =
                                (NetworkAvailabilityChangedEventHandler)callerArray[i].Key;
                ...
                lock (s_callerArray)
                {
                    ...
                    for (int i = 0; i < callerArray.Length; i++)
                    {
                        NetworkAddressChangedEventHandler handler =
                            (NetworkAddressChangedEventHandler)callerArray[i].Key;
                        ExecutionContext context = (ExecutionContext)callerArray[i].Value;
                        if (context == null)
                        {
            ...
            )
            {
                lock (s_callerArray)
                {
                    ...
                    if (s_ipv4Socket == null)
                    {
                        ...
                        {
                            blocking = -1;
                            s_ipv4Socket = SafeCloseSocketAndEvent.CreateWSASocketWithEvent(
                        ...
                        {
                            blocking = -1;
                            s_ipv6Socket = SafeCloseSocketAndEvent.CreateWSASocketWithEvent(
