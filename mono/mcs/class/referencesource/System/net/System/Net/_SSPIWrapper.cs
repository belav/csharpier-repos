...
namespace System.Net
{
    ...
    internal static class SSPIWrapper
    {
            ...
            if (SecModule.SecurityPackages == null)
            {
                lock (SecModule)
                {
                    if (SecModule.SecurityPackages == null)
                    {
                        ...
                        try
                        {
                            int errorCode = SecModule.EnumerateSecurityPackages(
                                out moduleCount,
                                out arrayBaseHandle
                            );
                            GlobalLog.Print(
                                "SSPIWrapper::arrayBase: "
                                    + (arrayBaseHandle.DangerousGetHandle().ToString("x"))
                            );
                            if (errorCode != 0)
                            {
                            ...
                            SecurityPackageInfoClass[] securityPackages =
                                new SecurityPackageInfoClass[moduleCount];
                            if (Logging.On)
                                Logging.PrintInfo(
                                    Logging.Web,
                            ...
                            {
                                securityPackages[i] = new SecurityPackageInfoClass(
                                    arrayBaseHandle,
                                    i
                                );
                                if (Logging.On)
...
#if TRAVE
                GlobalLog.Print(
                    "SSPIWrapper::AcquireDefaultCredential(): error "
                        + SecureChannel.MapSecurityStatus((uint)errorCode)
                );
#endif
                if (Logging.On)
                    Logging.PrintError(
                        ...
                        SR.GetString(
                            ...
                            "AcquireDefaultCredential()",
                            String.Format(CultureInfo.CurrentCulture, "0X{0:X}", errorCode)
                        )
                    );
                throw new Win32Exception(errorCode);
            }
...
#if TRAVE
                GlobalLog.Print(
                    "SSPIWrapper::AcquireCredentialsHandle#2(): error "
                        + SecureChannel.MapSecurityStatus((uint)errorCode)
                );
#endif
                if (Logging.On)
...
#if TRAVE
                GlobalLog.Print(
                    "SSPIWrapper::AcquireCredentialsHandle#3(): error "
                        + SecureChannel.MapSecurityStatus((uint)errorCode)
                );
#endif
                if (Logging.On)
...
#if TRAVE
            GlobalLog.Print(
                "SSPIWrapper::AcquireCredentialsHandle#3(): cred handle = "
        ...
        )
        {
            ...
            if (Logging.On)
                Logging.PrintInfo(
                    Logging.Web,
                    SR.GetString(
                        SR.net_log_sspi_security_context_input_buffer,
                        "InitializeSecurityContext",
        ...
        )
        {
            ...
            if (Logging.On)
                Logging.PrintInfo(
                    Logging.Web,
                    SR.GetString(
                        SR.net_log_sspi_security_context_input_buffers,
                        "InitializeSecurityContext",
        ...
        )
        {
            ...
            int errorCode = SecModule.AcceptSecurityContext(
                ref credential,
                ref context,
                inputBuffer,
                inFlags,
        ...
        )
        {
            ...
            if (Logging.On)
                Logging.PrintInfo(
                    Logging.Web,
                    SR.GetString(
                        SR.net_log_operation_returned_something,
                        "CompleteAuthToken()",
                        (SecurityStatus)errorCode
                    )
                );
                ...
                try
                {
                    ...
                    switch (op)
                    {
                        ...
                        case OP.VerifySignature:
                            errorCode = SecModule.VerifySignature(
                                context,
                                sdcInOut,
                                sequenceNumber
                            );
                    ...
                    for (int i = 0; i < input.Length; i++)
                    {
                        ...
                        if (iBuffer.size == 0)
                        {
                            ...
                            checked
                            {
                                ...
                                {
                                    GlobalLog.Assert(
                                        "SSPIWrapper::EncryptDecryptHelper",
                        ...
                        GlobalLog.Assert(
                            ...
                            "SSPIWrapper::EncryptDecryptHelper|'offset' out of range.  [{0}]",
                            iBuffer.offset
                        );
                        GlobalLog.Assert(
                            iBuffer.size >= 0
                            ...
                            "SSPIWrapper::EncryptDecryptHelper|'size' out of range.  [{0}]",
                            iBuffer.size
                        );
                    }
        ...
        )
        {
            ...
            switch (contextAttribute)
            {
                ...
                default:
                    throw new ArgumentException(
                        SR.GetString(SR.net_invalid_enum, "ContextAttribute"),
                        "contextAttribute"
                    );
            }
            ...
            try
            {
                ...
                errorCode = SecModule.QueryContextAttributes(
                    securityContext,
                    contextAttribute,
                    nativeBuffer,
                    handleType,
                        ...
                        unsafe
                        {
                            fixed (void* ptr = nativeBuffer)
                            {
                                attribute = new NegotiationInfoClass(
                                    ...
                                    Marshal.ReadInt32(
                                        new IntPtr(ptr),
                                        NegotiationInfo.NegotiationStateOffest
                                    )
                                );
                            }
                        }
    ...
    internal struct NegotiationInfo
    {
        ...
        internal static readonly int NegotiationStateOffest = (int)
    ...
    internal class NegotiationInfoClass
    {
        ...
        internal NegotiationInfoClass(SafeHandle safeHandle, int negotiationState)
        {
            ...
            {
                GlobalLog.Print(
                    "NegotiationInfoClass::.ctor() the handle is invalid:"
                        + (safeHandle.DangerousGetHandle()).ToString("x")
                );
                return;
            }
            IntPtr packageInfo = safeHandle.DangerousGetHandle();
            GlobalLog.Print(
                "NegotiationInfoClass::.ctor() packageInfo:"
                    + packageInfo.ToString("x8")
                    + " negotiationState:"
                    + negotiationState.ToString("x8")
            );
    ...
    internal class SecurityPackageInfoClass
    {
        ...
        internal SecurityPackageInfoClass(SafeHandle safeHandle, int index)
        {
            ...
            {
                GlobalLog.Print(
                    "SecurityPackageInfoClass::.ctor() the pointer is invalid: "
                        + (safeHandle.DangerousGetHandle()).ToString("x")
                );
                return;
            }
            ...
            GlobalLog.Print(
                "SecurityPackageInfoClass::.ctor() unmanagedPointer: "
                    + ((long)unmanagedAddress).ToString("x")
            );
