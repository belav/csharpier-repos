...
namespace System.Net.Sockets
{
    ...
    //
    internal unsafe class ReceiveMessageOverlappedAsyncResult : BaseOverlappedAsyncResult
    {
        //
        ...
        private static readonly int s_ControlDataSize = Marshal.SizeOf(
            typeof(UnsafeNclNativeMethods.OSSOCK.ControlData)
        );
        private static readonly int s_ControlDataIPv6Size = Marshal.SizeOf(
        ...
        private static readonly int s_WSAMsgSize = Marshal.SizeOf(
        ...
        //
        internal ReceiveMessageOverlappedAsyncResult(
            Socket socket,
            Object asyncState,
        ...
        )
        {
            ...
            bool ipv4 = (
                ((Socket)AsyncObject).AddressFamily == AddressFamily.InterNetwork
            ...
            //setup structure
            m_Message = (UnsafeNclNativeMethods.OSSOCK.WSAMsg*)
                Marshal.UnsafeAddrOfPinnedArrayElement(m_MessageBuffer, 0);
            m_Message->socketAddress = Marshal.UnsafeAddrOfPinnedArrayElement(
                m_SocketAddress.m_Buffer,
                0
            );
            ...
            if (m_ControlBuffer != null)
            {
                m_Message->controlBuffer.Pointer = Marshal.UnsafeAddrOfPinnedArrayElement(
                    m_ControlBuffer,
                    0
                );
                ...
                UnsafeNclNativeMethods.OSSOCK.ControlData controlData =
                    (UnsafeNclNativeMethods.OSSOCK.ControlData)
                        Marshal.PtrToStructure(
                            m_Message->controlBuffer.Pointer,
                            typeof(UnsafeNclNativeMethods.OSSOCK.ControlData)
                        );
        ...
        void LogBuffer(int size)
        {
            GlobalLog.Assert(
                Logging.On,
                "ReceiveMessageOverlappedAsyncResult#{0}::LogBuffer()|Logging is off!",
