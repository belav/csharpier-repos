...
namespace System.Net
{
    ...
    internal class NtlmClient : ISessionAuthenticationModule
    {
        ...
        {
            GlobalLog.Print(
                "NtlmClient::Authenticate() challenge:["
        ...
        {
            GlobalLog.Print(
                "NtlmClient::DoAuthenticate() challenge:["
                    + ValidationHelper.ToString(challenge)
                    + "] webRequest#"
                    + ValidationHelper.HashString(webRequest)
                    + " credentials#"
            ...
            GlobalLog.Assert(
                credentials != null,
                "NtlmClient::DoAuthenticate()|credentials == null"
            );
            if (credentials == null)
            {
                return null;
            ...
            GlobalLog.Assert(
                httpWebRequest != null,
                "NtlmClient::DoAuthenticate()|httpWebRequest == null"
            );
            GlobalLog.Assert(
                httpWebRequest.ChallengedUri != null,
                "NtlmClient::DoAuthenticate()|httpWebRequest.ChallengedUri == null"
            );
            ...
            if (authSession == null)
            {
                ...
                GlobalLog.Print(
                    "NtlmClient::DoAuthenticate() GetCredential() returns:"
                ...
                GlobalLog.Print(
                    "NtlmClient::DoAuthenticate() setting SecurityContext for:"
                        + ValidationHelper.HashString(httpWebRequest.CurrentAuthenticationState)
                        + " to authSession:"
                        + ValidationHelper.HashString(authSession)
                );
        ...
        {
            GlobalLog.Print(
                "NtlmClient::PreAuthenticate() webRequest#"
                    + ValidationHelper.HashString(webRequest)
                    + " credentials#"
                    + ValidationHelper.HashString(credentials)
                    + " calling DoAuthenticate()"
        ...
        public bool Update(string challenge, WebRequest webRequest)
        {
            ...
            GlobalLog.Assert(
                httpWebRequest.ChallengedUri != null,
                "NtlmClient::Update()|httpWebRequest.ChallengedUri == null"
            );
            ...
            GlobalLog.Print(
                "NtlmClient::Update() authSession.IsCompleted:" + authSession.IsCompleted.ToString()
            );
            ...
            {
                GlobalLog.Print(
                    "NtlmClient::Update() still handshaking (based on status code) returning false"
                );
                return false;
            }
            ...
            GlobalLog.Print(
                "NtlmClient::Update() session removed and ConnectionGorup released returning true"
            );
            return true;
        }
