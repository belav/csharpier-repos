...
namespace System.Net
{
    ...
    public static class Dns
    {
        ...
        private static DnsPermission s_DnsPermission = new DnsPermission(
        ...
        internal static IPHostEntry NativeToHostEntry(IntPtr nativePointer)
        {
            ...
            while (nativePointer != IntPtr.Zero)
            {
                ...
                GlobalLog.Print(
                    "currentArrayElement: "
                        + currentArrayElement.ToString()
                        + " nativePointer: "
                        + nativePointer.ToString()
                        + " IPAddressToAdd:"
                        + IPAddressToAdd.ToString()
                );
            ...
            while (nativePointer != IntPtr.Zero)
            {
                GlobalLog.Print(
                    "currentArrayElement: "
                        + ((long)currentArrayElement).ToString()
                        + "nativePointer: "
                        + ((long)nativePointer).ToString()
                );
        ...
        ******************************************************************************/
        ...
        internal static IPHostEntry InternalGetHostByName(string hostName, bool includeIPv6)
        {
            ...
            if (
                hostName.Length > MaxHostName // If 255 chars, the last one must be a dot.
            ...
            )
            {
                throw new ArgumentOutOfRangeException(
                    ...
                    SR.GetString(
                        SR.net_toolong,
                        "hostName",
                        MaxHostName.ToString(NumberFormatInfo.CurrentInfo)
                    )
                );
            }
        ...
        ******************************************************************************/
        ...
        internal static bool TryInternalResolve(string hostName, out IPHostEntry result)
        {
            ...
            {
                GlobalLog.Print(
                    "Dns::InternalResolveFast() returned address:" + address.ToString()
                );
            ...
            if (
                SocketError.Success
                != TryGetAddrInfo(hostName, AddressInfoHints.AI_CANONNAME, out canonicalResult)
            )
            {
                result = null;
                return false;
            ...
            if (
                SocketError.Success
                != TryGetAddrInfo(hostName, AddressInfoHints.AI_FQDN, out fqdnResult)
            )
            {
            ...
            if (IPAddress.TryParse(hostName, out address))
            {
                ...
                )
                {
                    throw new ArgumentException(
                        SR.GetString(SR.net_invalid_ip_addr),
                        "hostNameOrAddress"
                    );
                }

                asyncResult = new ResolveAsyncResult(
                    address,
                    null,
                    includeIPv6,
                    state,
        ...
        )
        {
            ...
            // Set up the context, possibly flow.
            ResolveAsyncResult asyncResult = new ResolveAsyncResult(
                address,
                null,
                includeIPv6,
                state,
        ...
        private static IPHostEntry HostResolutionEndHelper(IAsyncResult asyncResult)
        {
            ...
            if (castedResult.EndCalled)
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.net_io_invalidendcall, "EndResolve")
                );
            }
        ...
        )
        {
            ...
            IAsyncResult asyncResult = HostResolutionBeginHelper(
                hostName,
                true,
                true,
                false,
        ...
        internal static IAsyncResult UnsafeBeginGetHostAddresses(
            string hostName,
            AsyncCallback requestCallback,
            object state
        )
        ...
        )
        {
            ...
            try
            {
                ...
                while (pAddressInfo != null)
                {
                    ...
                    //
                    if (
                        (pAddressInfo->ai_family == AddressFamily.InterNetwork)
                        || // Never filter v4
                        (
                            pAddressInfo->ai_family == AddressFamily.InterNetworkV6
                    ...
                    )
                    {
                        ...
                        {
                            addresses.Add(
                                ((IPEndPoint)IPEndPoint.IPv6Any.Create(sockaddr)).Address
                            );
                        }
                    }
                    //
