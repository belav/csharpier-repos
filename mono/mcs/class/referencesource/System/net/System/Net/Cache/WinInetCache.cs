...
*/
        ...
        internal struct EntryBuffer
        {
            ...
            {
                [FieldOffset(0)]
                public int ExemptDelta; // Exemption delta from last access

                [FieldOffset(0)]
                public int Reserved; // To keep the unmanaged layout
            }
        ...
        internal class Entry
        {
            ...
            public Entry(string key, int maxHeadersSize)
            {
                ...
                if (
                    maxHeadersSize != Int32.MaxValue
                    && (Int32.MaxValue - (key.Length + EntryBuffer.MarshalSize + 1024) * 2)
                        > maxHeadersSize
                )
                {
                    //
            ...
            try
            {
                while (true)
                {
                    fixed (byte* entryPtr = buffer)
                    {
                        ...
                        entry.Error = SafeUnlockUrlCacheEntryFile.GetAndLockFile(
                            entry.Key,
                            entryPtr,
                            ref size,
                            out handle
        ...
        unsafe private static Status EntryFixup(Entry entry, EntryBuffer* bufferPtr, byte[] buffer)
        {
            unchecked
            {
                bufferPtr->_OffsetExtension =
                    bufferPtr->_OffsetExtension == IntPtr.Zero
                        ? IntPtr.Zero
                        : (IntPtr)((byte*)bufferPtr->_OffsetExtension - (byte*)bufferPtr);
                bufferPtr->_OffsetFileName =
                    bufferPtr->_OffsetFileName == IntPtr.Zero
                        ? IntPtr.Zero
                        : (IntPtr)((byte*)bufferPtr->_OffsetFileName - (byte*)bufferPtr);
                bufferPtr->_OffsetHeaderInfo =
                    bufferPtr->_OffsetHeaderInfo == IntPtr.Zero
                        ? IntPtr.Zero
                        : (IntPtr)((byte*)bufferPtr->_OffsetHeaderInfo - (byte*)bufferPtr);
                bufferPtr->_OffsetSourceUrlName =
                    bufferPtr->_OffsetSourceUrlName == IntPtr.Zero
                        ? IntPtr.Zero
                        : (IntPtr)((byte*)bufferPtr->_OffsetSourceUrlName - (byte*)bufferPtr);

                // Get a managed EntryBuffer copy out of byte[]
                entry.Info = *bufferPtr;
                entry.OriginalUrl = GetEntryBufferString(
                    bufferPtr,
        ...
        internal static Status Commit(Entry entry)
        {
            ...
            if (
                (
                    ...
                    + (entry.OriginalUrl == null ? 0 : entry.OriginalUrl.Length)
                )
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            if ((attributes & Entry_FC.Attribute) == 0)
                            {
                                ...
                                if (
                                    (oldEntry.Info.EntryType & EntryType.StickyEntry)
        ...
        */
        ...
        )
        {
            ...
            if (
                bufferPtr->_OffsetHeaderInfo == IntPtr.Zero
                || bufferPtr->HeaderInfoChars == 0
                || (bufferPtr->EntryType & EntryType.UrlHistory) != 0
            )
            {
                return Status.Success;
            }

            int bufferCharLength =
