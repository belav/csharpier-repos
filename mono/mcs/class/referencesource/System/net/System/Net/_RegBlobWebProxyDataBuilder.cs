...
namespace System.Net
{
...
#endif // !FEATURE_PAL
#endif
    using Microsoft.Win32;using System.Runtime.Versioning;using System.Diagnostics;
    ...
    internal class RegBlobWebProxyDataBuilder : WebProxyDataBuilder
    {
        ...
        internal const string PolicyKey =
            @"SOFTWARE\Policies\Microsoft\Windows\CurrentVersion\Internet Settings";
        internal const string ProxyKey =
        ...
        private bool ReadRegSettings()
        {
            ...
            try
            {
                ...
                if (isPerUser)
                {
                    if (m_Registry != null)
                    {
                        errorCode = m_Registry.RegOpenKeyEx(
                            ProxyKey,
                            0,
                ...
                if (key != null)
                {
                    ...
                    object data;
                    errorCode = key.QueryValue(
                        m_Connectoid != null ? m_Connectoid : DefaultConnectionSettings,
                        out data
                    );
                    if (errorCode == UnsafeNclNativeMethods.ErrorCodes.ERROR_SUCCESS)
                    {
        ...
        private static string ReadConfigString(string ConfigName)
        {
            ...
            bool rc = UnsafeNclNativeMethods.FetchConfigurationString(
                true,
                ConfigName,
                parameterValue,
        ...
        protected override void BuildInternal()
        {
            GlobalLog.Enter(
                "RegBlobWebProxyDataBuilder#"
                    ...
                    + "::BuildInternal() m_Connectoid:"
                    + ValidationHelper.ToString(m_Connectoid)
            );
            ...
            GlobalLog.Print(
                "RegBlobWebProxyDataBuilder::BuildInternal() proxyFlags:"
                    + ValidationHelper.ToString(proxyFlags)
            );

            string addressString = ReadString(); // proxy name
            string proxyBypassString = ReadString(); // proxy bypass
            GlobalLog.Print(
                "RegBlobWebProxyDataBuilder::BuildInternal() proxyAddressString:"
                    + ValidationHelper.ToString(addressString)
                    + " proxyBypassString:"
                    + ValidationHelper.ToString(proxyBypassString)
            );
            ...
            GlobalLog.Print(
                "RegBlobWebProxyDataBuilder::BuildInternal() scriptLocation:"
                    + ValidationHelper.ToString(addressString)
            );
            ...
            if (IsValidTimeForLkgScriptLocation(ftLastKnownDetectTime))
            {
                ...
                GlobalLog.Print(
                    "RegBlobWebProxyDataBuilder::BuildInternal() lkgScriptLocation:"
                        + ValidationHelper.ToString(addressString)
                );
        ...
        internal unsafe static bool IsValidTimeForLkgScriptLocation(FILETIME ftLastKnownDetectTime)
        {
            ...
            GlobalLog.Print(
                "RegBlobWebProxyDataBuilder::BuildInternal() Detect Time:"
                    + ValidationHelper.ToString(ftDetect)
            );
            GlobalLog.Print(
                "RegBlobWebProxyDataBuilder::BuildInternal() Current Time:"
                    + ValidationHelper.ToString(ftCurrent)
            );
            GlobalLog.Print(
                "RegBlobWebProxyDataBuilder::BuildInternal() 7 days:"
                    + ValidationHelper.ToString(s_lkgScriptValidTime)
            );
            GlobalLog.Print(
                "RegBlobWebProxyDataBuilder::BuildInternal() Delta Time:"
                    + ValidationHelper.ToString((UInt64)(ftCurrent - ftDetect))
            );
