...
namespace System.Net
{
    ...
    internal class DigestClient : ISessionAuthenticationModule
    {
        ...
        {
            _WDigestAvailable =
                SSPIWrapper.GetVerifyPackageInfo(GlobalSSPI.SSPIAuth, NegotiationInfoClass.WDigest)
                != null;
        }
        ...
        {
            GlobalLog.Print(
                "DigestClient::Authenticate() challenge:["
        ...
        {
            GlobalLog.Print(
                "DigestClient::DoAuthenticate() challenge:["
                    + ValidationHelper.ToString(challenge)
                    + "] webRequest#"
                    + ValidationHelper.HashString(webRequest)
                    + " credentials#"
            ...
            GlobalLog.Assert(
                credentials != null,
                "DigestClient::DoAuthenticate()|credentials == null"
            );
            if (credentials == null)
            {
                return null;
            ...
            GlobalLog.Assert(
                httpWebRequest != null,
                "DigestClient::DoAuthenticate()|httpWebRequest == null"
            );
            GlobalLog.Assert(
                httpWebRequest.ChallengedUri != null,
                "DigestClient::DoAuthenticate()|httpWebRequest.ChallengedUri == null"
            );
            ...
            GlobalLog.Print(
                "DigestClient::DoAuthenticate() GetCredential() returns:"
            ...
            {
                GlobalLog.Print(
                    "DigestClient::DoAuthenticate() looking up digestChallenge for prefix:"
                        + httpWebRequest.ChallengedUri.AbsoluteUri
                );
                digestChallenge =
                    challengeCache.Lookup(httpWebRequest.ChallengedUri.AbsoluteUri)
                    as HttpDigestChallenge;
            }
            ...
            if (!supported)
            {
                if (Logging.On)
                    Logging.PrintError(
                        Logging.Web,
                        SR.GetString(
                            SR.net_log_digest_qop_not_supported,
            ...
            {
                GlobalLog.Print(
                    "DigestClient::DoAuthenticate() retrieved digestChallenge#"
                        + ValidationHelper.HashString(digestChallenge)
                        + " digestChallenge for prefix:"
                        + httpWebRequest.ChallengedUri.AbsoluteUri
                );
            ...
            SpnToken spnToken = httpWebRequest.CurrentAuthenticationState.GetComputeSpn(
                httpWebRequest
            );
            ...
            Authorization digestResponse = HttpDigest.Authenticate(
                digestChallenge,
                NC,
                spnToken.Spn,
                binding
            ...
            if (!preAuthenticate && webRequest.PreAuthenticate && digestResponse != null)
            {
                ...
                {
                    GlobalLog.Print(
                        "DigestClient::DoAuthenticate() adding digestChallenge#"
                            + ValidationHelper.HashString(digestChallenge)
                            + " for prefix:"
                            + prefixes[i]
                    );
        ...
        {
            GlobalLog.Print(
                "DigestClient::PreAuthenticate() webRequest#"
                    + ValidationHelper.HashString(webRequest)
                    + " credentials#"
                    + ValidationHelper.HashString(credentials)
                    + " calling DoAuthenticate()"
        ...
        internal static bool CheckQOP(HttpDigestChallenge challenge)
        {
            ...
            if (challenge.QopPresent)
            {
                ...
                while (index >= 0)
                {
                    ...
                    index = challenge.QualityOfProtection.IndexOf(
                        HttpDigest.SupportedQuality,
                        index
                    );
                    if (index < 0)
                    {
                    ...
                    if (
                        (
                            index == 0
                            || HttpDigest.ValidSeparator.IndexOf(
                                challenge.QualityOfProtection[index - 1]
                            ) >= 0
                        )
                        && (
        ...
        public bool Update(string challenge, WebRequest webRequest)
        {
            ...
            GlobalLog.Assert(
                httpWebRequest != null,
                "DigestClient::Update()|httpWebRequest == null"
            );
            GlobalLog.Assert(
                httpWebRequest.ChallengedUri != null,
                "DigestClient::Update()|httpWebRequest.ChallengedUri == null"
            );
            ...
            HttpDigestChallenge digestChallenge = HttpDigest.Interpret(
                challenge,
                index,
                httpWebRequest
            );
            ...
            {
                GlobalLog.Print(
                    "DigestClient::Update(): not a valid digest challenge. returning true"
                );
                return true;
            }

            GlobalLog.Print(
                "DigestClient::Update(): returning digestChallenge.Stale:"
                    + digestChallenge.Stale.ToString()
            );
        ...
        {
            get { return WDigestAvailable; }
        }

        internal static bool WDigestAvailable
        {
            get { return _WDigestAvailable; }
        }
        ...
        public void ClearSession(WebRequest webRequest)
        {
            ...
            GlobalLog.Assert(
                httpWebRequest != null,
                "NtlmClient::ClearSession()|httpWebRequest == null"
            );
        ...
        private Authorization XPDoAuthenticate(
            string challenge,
        ...
        {
            GlobalLog.Print(
                "DigestClient::XPDoAuthenticate() challenge:["
                    + ValidationHelper.ToString(challenge)
                    + "] httpWebRequest#"
            ...
            if (!preAuthenticate)
            {
                ...
                GlobalLog.Print(
                    "DigestClient::XPDoAuthenticate() key:"
            ...
            else
            {
#if WDIGEST_PREAUTH
                GlobalLog.Print(
                    "DigestClient::XPDoAuthenticate() looking up digestChallenge for prefix:"
                        + httpWebRequest.ChallengedUri.AbsoluteUri
                );
                authSession =
                    challengeCache.Lookup(httpWebRequest.ChallengedUri.AbsoluteUri)
                ...
                GlobalLog.Print(
                    "DigestClient::XPDoAuthenticate() retrieved digestChallenge#"
                        + ValidationHelper.HashString(digestChallenge)
                        + " digestChallenge for prefix:"
                        + httpWebRequest.ChallengedUri.AbsoluteUri
                );
            ...
            GlobalLog.Print(
                "DigestClient::XPDoAuthenticate() rawUri:" + ValidationHelper.ToString(rawUri)
            );
            ...
            if (authSession == null)
            {
                ...
                GlobalLog.Print(
                    "DigestClient::XPDoAuthenticate() GetCredential() returns:"
                        + ValidationHelper.ToString(NC)
                );
                ...
                authSession = new NTAuthentication(
                ...
                GlobalLog.Print(
                    "DigestClient::XPDoAuthenticate() setting SecurityContext for:"
                        + ValidationHelper.HashString(httpWebRequest.CurrentAuthenticationState)
                        + " to authSession:"
                        + ValidationHelper.HashString(authSession)
                );
            ...
            GlobalLog.Print(
                "DigestClient::XPDoAuthenticate() incoming:" + ValidationHelper.ToString(incoming)
            );

#if WDIGEST_PREAUTH
            clientResponse = authSession.GetOutgoingDigestBlob(
                incoming,
                httpWebRequest.CurrentMethod.Name,
                rawUri,
                null,
                preAuthenticate,
                true,
            ...
            GlobalLog.Print(
                "DigestClient::XPDoAuthenticate() GetOutgoingDigestBlob("
                    + incoming
                    + ") returns:"
            ...
            if (!preAuthenticate && httpWebRequest.PreAuthenticate)
            {
                ...
                {
                    GlobalLog.Print(
                        "DigestClient::XPDoAuthenticate() adding authSession#"
                            + ValidationHelper.HashString(authSession)
                            + " for prefix:"
                            + prefixes[i]
                    );
        ...
        private bool XPUpdate(string challenge, HttpWebRequest httpWebRequest)
        {
            ...
            SecurityStatus statusCode;
            string clientResponse = authSession.GetOutgoingDigestBlob(
                incoming,
                httpWebRequest.CurrentMethod.Name,
                null,
                null,
            ...
            GlobalLog.Print(
                "DigestClient::XPUpdate() GetOutgoingDigestBlob("
                    + incoming
                    + ") returns:"
                    + ValidationHelper.ToString(clientResponse)
            );
            GlobalLog.Assert(
                authSession.IsCompleted,
                "DigestClient::XPUpdate()|IsCompleted == false"
            );
            GlobalLog.Print(
                "DigestClient::XPUpdate() GetOutgoingBlob() returns clientResponse:["
                    + ValidationHelper.ToString(clientResponse)
                    + "] IsCompleted:"
        ...
        private static string RefineDigestChallenge(string challenge, int index)
        {
            ...
            for (; ; )
            {
                ...
                GlobalLog.Print(
                    "DigestClient::XPDoAuthenticate() SplitNoQuotes() returning index:"
                        + index
                        + " offset:"
                        + offset
                );
                ...
                GlobalLog.Print(
                    "DigestClient::XPDoAuthenticate() defineAttribute("
                        + name
                        + ", "
                        + value
                        + ") returns "
            ...
            GlobalLog.Print(
                "DigestClient::XPDoAuthenticate() start:"
                    + start
                    + " offset:"
                    + offset
                    + " index:"
            ...
            else
            {
                ...
                this.Uri =
                    ...
                    + httpWebRequest
                        ...
                        .GetParts(UriComponents.Path, UriFormat.UriEscaped);
            }
        ...
        internal string ToBlob()
        {
            ...
            if (NonceCount > 0)
            {
                ...
                stringBuilder.Append(
                    HttpDigest.pair(
                        HttpDigest.DA_nc,
                        NonceCount.ToString("x8", NumberFormatInfo.InvariantInfo),
                        true
                    )
                );
            }
    ...
    internal static class HttpDigest
    {
        ...
        private const string suppressExtendedProtectionKey =
            @"System\CurrentControlSet\Control\Lsa";
        private const string suppressExtendedProtectionKeyPath =
            @"HKEY_LOCAL_MACHINE\" + suppressExtendedProtectionKey;
        ...
        private static void ReadSuppressExtendedProtectionRegistryValue()
        {
            ...
            {
                using (
                    RegistryKey lsaKey = Registry.LocalMachine.OpenSubKey(
                ...
                )
                {
                    try
                    {
                        ...
                        if (
                            lsaKey.GetValueKind(suppressExtendedProtectionValueName)
        ...
        //
        internal static HttpDigestChallenge Interpret(
            string challenge,
            int startingPoint,
            HttpWebRequest httpWebRequest
        )
        {
            ...
            for (; ; )
            {
                ...
                if (string.Compare(name, DA_charset, StringComparison.OrdinalIgnoreCase) == 0)
                {
                    ...
                    GlobalLog.Print(
                        "HttpDigest::Interpret() server provided a hint to use ["
                            + value
                            + "] encoding"
                    );
            ...
            for (; ; )
            {
                ...
                index = AuthenticationManager.SplitNoQuotes(challenge, ref offset);
                GlobalLog.Print(
                    "HttpDigest::Interpret() SplitNoQuotes() returning index:"
                        + index.ToString()
                        + " offset:"
                        + offset.ToString()
                );
                ...
                if (digestChallenge.UTF8Charset)
                {
                    ...
                    {
                        GlobalLog.Print(
                            "HttpDigest::Interpret() UTF8 decoding required value:[" + value + "]"
                        );
                        ...
                        GlobalLog.Print(
                            "HttpDigest::Interpret() UTF8 decoded value:[" + value + "]"
                        );
                    }
                    else
                    {
                ...
                valid = digestChallenge.defineAttribute(name, value);
                GlobalLog.Print(
                    "HttpDigest::Interpret() defineAttribute("
                        + name
                        + ", "
                        + value
                        + ") returns "
...
#if TRAVE
            GlobalLog.Print(
                "HttpDigest::CharsetEncode() encoding rawString:["
                    + rawString
                    + "] Chars(rawString):["
                    + Chars(rawString)
                    + "] charset:["
            ...
            if (charset == Charset.UTF8 || charset == Charset.ANSI)
            {
                byte[] bytes =
                    charset == Charset.UTF8
                        ...
                        : Encoding.Default.GetBytes(rawString);
...
#if TRAVE
            GlobalLog.Print(
                "HttpDigest::CharsetEncode() encoded rawString:["
                    + rawString
                    + "] Chars(rawString):["
                    + Chars(rawString)
                    + "] charset:["
        ...
        private static Charset DetectCharset(string rawString)
        {
            ...
            for (int i = 0; i < rawString.Length; i++)
            {
                ...
                {
                    GlobalLog.Print(
                        "HttpDigest::DetectCharset() found non ASCII character:["
                            + ((int)rawString[i]).ToString()
                            + "] at offset i:["
                            + i.ToString()
                            + "] charset:["
                            + charset.ToString()
            ...
            GlobalLog.Print(
                "HttpDigest::DetectCharset() rawString:["
                    + rawString
                    + "] has charset:["
                    + charset.ToString()
                    + "]"
        ...
        //
        internal static Authorization Authenticate(
            HttpDigestChallenge digestChallenge,
        ...
        )
        {
            ...
            if (upgraded)
            {
                ...
                digestChallenge.ChannelBinding = hashChannelBinding(
                    binding,
                    digestChallenge.MD5provider
                );
            }
            ...
            if (digestChallenge.QopPresent)
            {
                ...
                {
                    GlobalLog.Print(
                        "HttpDigest::Authenticate() QopPresent:True, need new nonce. digestChallenge.ClientNonce:"
                            + ValidationHelper.ToString(digestChallenge.ClientNonce)
                            + " digestChallenge.Stale:"
                            + digestChallenge.Stale.ToString()
                    );
                ...
                {
                    GlobalLog.Print(
                        "HttpDigest::Authenticate() QopPresent:True, reusing nonce. digestChallenge.NonceCount:"
                            + digestChallenge.NonceCount.ToString()
                    );
            ...
            {
                GlobalLog.Print(
                    "HttpDigest::Authenticate() can't authenticate with UNICODE username. failing auth."
                );
                return null;
            }
            ...
            {
                GlobalLog.Print(
                    "HttpDigest::Authenticate() can't authenticate with UNICODE password. failing auth."
                );
                return null;
            }
            if (digestChallenge.UTF8Charset)
            {
                ...
                {
                    authorization.Append(
                        pair(DA_username, CharsetEncode(username, Charset.UTF8), true)
                    );
                    authorization.Append(",");
            ...
            if (digestChallenge.QopPresent)
            {
                ...
                authorization.Append(
                    pair(
                        DA_nc,
                        digestChallenge.NonceCount.ToString("x8", NumberFormatInfo.InvariantInfo),
                        false
                    )
                );
                // RAID#47397
                ...
                if (upgraded)
                {
                    ...
                    authorization.Append(",");
                    authorization.Append(
                        pair(DA_channelbinding, digestChallenge.ChannelBinding, true)
                    );
                }
            }
            ...
            GlobalLog.Print(
                "HttpDigest::Authenticate() digestChallenge.Stale:"
                    + digestChallenge.Stale.ToString()
            );

            // completion is decided in Update()
            Authorization finalAuthorization = new Authorization(
                DigestClient.AuthType + " " + authorization.ToString(),
                false
            );
        ...
        {
            if (
                challenge.Algorithm == null
                || string.Compare(challenge.Algorithm, "md5", StringComparison.OrdinalIgnoreCase)
                    == 0
            )
            {
                ...
                string.Compare(challenge.Algorithm, "md5-sess", StringComparison.OrdinalIgnoreCase)
                == 0
            )
            {
                return hashString(
            ...
            if (Logging.On)
                Logging.PrintError(
                    Logging.Web,
                    SR.GetString(
                        SR.net_log_digest_hash_algorithm_not_supported,
                        challenge.Algorithm
                    )
                );
            return null;
        ...
        private static int InitiatorTypeOffset = (int)
            Marshal.OffsetOf(typeof(SecChannelBindings), "dwInitiatorAddrType");
        private static int InitiatorLengthOffset = (int)
            Marshal.OffsetOf(typeof(SecChannelBindings), "cbInitiatorLength");
        private static int InitiatorOffsetOffset = (int)
            Marshal.OffsetOf(typeof(SecChannelBindings), "dwInitiatorOffset");
        private static int AcceptorTypeOffset = (int)
            Marshal.OffsetOf(typeof(SecChannelBindings), "dwAcceptorAddrType");
        private static int AcceptorLengthOffset = (int)
            Marshal.OffsetOf(typeof(SecChannelBindings), "cbAcceptorLength");
        private static int AcceptorOffsetOffset = (int)
            Marshal.OffsetOf(typeof(SecChannelBindings), "dwAcceptorOffset");
        private static int ApplicationDataLengthOffset = (int)
            Marshal.OffsetOf(typeof(SecChannelBindings), "cbApplicationDataLength");
        private static int ApplicationDataOffsetOffset = (int)
            Marshal.OffsetOf(typeof(SecChannelBindings), "dwApplicationDataOffset");
        ...
        {
            int initiatorType = Marshal.ReadInt32(
                binding.DangerousGetHandle(),
                InitiatorTypeOffset
            );
            int initiatorLength = Marshal.ReadInt32(
                binding.DangerousGetHandle(),
                InitiatorLengthOffset
            );
            int acceptorType = Marshal.ReadInt32(binding.DangerousGetHandle(), AcceptorTypeOffset);
            int acceptorLength = Marshal.ReadInt32(
                binding.DangerousGetHandle(),
                AcceptorLengthOffset
            );
            int applicationDataLength = Marshal.ReadInt32(
                binding.DangerousGetHandle(),
                ApplicationDataLengthOffset
            );

            byte[] formattedData = new byte[
                MinimumFormattedBindingLength
                    + initiatorLength
                    + acceptorLength
            ...
            {
                int initiatorOffset = Marshal.ReadInt32(
                    binding.DangerousGetHandle(),
            ...
            if (acceptorLength > 0)
            {
                int acceptorOffset = Marshal.ReadInt32(
                    binding.DangerousGetHandle(),
                    AcceptorOffsetOffset
                );
                Marshal.Copy(
                    IntPtrHelper.Add(binding.DangerousGetHandle(), acceptorOffset),
                    formattedData,
                    offset,
                    acceptorLength
                );
            ...
            {
                int applicationDataOffset = Marshal.ReadInt32(
                    binding.DangerousGetHandle(),
                    ApplicationDataOffsetOffset
                );
                Marshal.Copy(
                    IntPtrHelper.Add(binding.DangerousGetHandle(), applicationDataOffset),
                    formattedData,
                    offset,
                    applicationDataLength
                );
        ...
        private static string hashChannelBinding(
            ChannelBinding binding,
        ...
        {
            GlobalLog.Enter(
                "HttpDigest::hashString",
                "[" + myString.Length.ToString() + ":" + myString + "]"
            );
            ...
            GlobalLog.Leave(
                "HttpDigest::hashString",
                "[" + hashString.Length.ToString() + ":" + hashString + "]"
            );
            return hashString;
        }
