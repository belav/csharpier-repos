...
namespace System.Net
{
    ...
    internal sealed class HttpListenerRequestUriBuilder
    {
        ...
        private void BuildRequestUriUsingRawPath()
        {
            ...
            if (!HttpSysSettings.EnableNonUtf8 || (rawPath == string.Empty))
            {
                ...
                if (path == string.Empty)
                {
                    path = "/";
                    Debug.Assert(
                        cookedUriQuery == string.Empty,
                        "Query is only allowed if there is a non-empty path. At least '/' path required."
                    );
                }

                isValid = Uri.TryCreate(
                    cookedUriScheme + Uri.SchemeDelimiter + cookedUriHost + path + cookedUriQuery,
                    UriKind.Absolute,
                    out requestUri
                );
            }
            else
            {
                ...
                ParsingResult result = BuildRequestUriUsingRawPath(
                    GetEncoding(EncodingType.Primary)
                );
        ...
        private static Encoding GetEncoding(EncodingType type)
        {
            ...
            if (
                ((type == EncodingType.Primary) && (!HttpSysSettings.FavorUtf8))
        ...
        private bool AppendUnicodeCodePointValuePercentEncoded(string codePoint)
        {
            ...
            if (!int.TryParse(codePoint, NumberStyles.HexNumber, null, out codePointValue))
            {
                LogWarning(
                    "AppendUnicodeCodePointValuePercentEncoded",
                    SR.net_log_listener_cant_convert_percent_value,
                    codePoint
                );
                return false;
            }
            ...
            try
            {
                ...
                else
                {
                    AppendOctetsPercentEncoded(
                        requestUriString,
                        utf8Encoding.GetBytes(decodedString)
                    );
                }
