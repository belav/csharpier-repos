...
namespace System.Net
{
    ...
    {
        private static readonly WaitCallback s_InvokeConnectionCallback = new WaitCallback(
        ...
        internal static void AddExceptionRange(
            ref ConnectionReturnResult returnResult,
        ...
        internal static void AddExceptionRange(
            ...
            HttpWebRequest[] requests,
            Exception exception,
            Exception firstRequestException
        )
        {
        ...
        internal static void SetResponses(ConnectionReturnResult returnResult)
        {
            ...
            GlobalLog.Print(
                "ConnectionReturnResult#"
                    + ValidationHelper.HashString(returnResult)
                    + "::SetResponses() count="
                    + returnResult.m_Context.Count.ToString()
            );
...
#if DEBUG
                    CoreResponseData coreResponseData =
                        returnResult.m_Context[i].CoreResponse as CoreResponseData;
                    if (coreResponseData == null)
    ...
    internal class Connection : PooledStream
    {
        ...
        private static PinnableBufferCache s_PinnableBufferCache = new PinnableBufferCache(
        ...
        private static readonly WaitCallback m_PostReceiveDelegate = new WaitCallback(
            PostReceiveWrapper
        );
        private static readonly AsyncCallback m_ReadCallback = new AsyncCallback(
            ReadCallbackWrapper
        );
        private static readonly AsyncCallback m_TunnelCallback = new AsyncCallback(
            TunnelThroughProxyWrapper
        );
            ...
            set
            {
                ...
                GlobalLog.Print(
                    "Connection#"
                        ...
                        + "::LockedRequest_set() old#"
                        + ((myLock != null) ? myLock.GetHashCode().ToString() : "null")
                        + " new#"
                ...
                if ((object)value == (object)myLock)
                {
                    if (
                        value != null
                        && (object)value.UnlockConnectionDelegate != (object)m_ConnectionUnlock
                    )
                    {
                        throw new InternalException();
                    }
                ...
                if (
                    myDelegate != null
                    && (value != null || (object)m_ConnectionUnlock != (object)myDelegate)
                )
                    throw new InternalException();
                ...
                if ((object)chkDelegate != null)
                {
                    ...
                    GlobalLog.Print(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::LockedRequest_set() Unlocking old request Connection"
                    );
                    chkDelegate();
                }
        ...
        private void UnlockRequest()
        {
            GlobalLog.Print(
                "Connection#"
                    + ValidationHelper.HashString(this)
                    + "::UnlockRequest() LockedRequest#"
                    + ValidationHelper.HashString(LockedRequest)
            );
            ...
            if (ConnectionGroup != null)
            {
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::UnlockRequest() - forcing call to ConnectionGoneIdle()"
                );
        ...
        */
        ...
        internal int BusyCount
        {
            get
            {
                return (m_ReadDone ? 0 : 1)
                    + 2 * (m_WaitList.Count + m_WriteList.Count)
                    + m_ReservedCount;
            }
        }
        ...
        [SuppressMessage(
            ...
            Justification = "Re-Baseline System violations from 3.5 SP1 due to added parameter"
        )]
        internal bool SubmitRequest(HttpWebRequest request, bool forcedsubmit)
        {
            GlobalLog.Enter(
                "Connection#" + ValidationHelper.HashString(this) + "::SubmitRequest",
            ...
            lock (this)
            {
                ...
                if (request.Aborted)
                {
                    ...
                    GlobalLog.Leave(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::SubmitRequest - (Request was aborted before being submitted)",
                        true
                    );
                ...
                if (!forcedsubmit && NonKeepAliveRequestPipelined)
                {
                    GlobalLog.Leave(
                        "Connection#" + ValidationHelper.HashString(this) + "::SubmitRequest",
                        "false - behind non keep-alive request"
                    );
                    UnlockIfNeeded(request);
                    return false;
                }
                ...
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::SubmitRequest WriteDone:"
                        + m_WriteDone.ToString()
                        + ", ReadDone:"
                        + m_ReadDone.ToString()
                        + ", m_WriteList.Count:"
                        + m_WriteList.Count.ToString()
                );
                ...
                else
                {
                    ...
                    GlobalLog.Print(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::SubmitRequest - Request added to WaitList#"
                    ...
                    if (q_Tunnelling)
                    {
                        GlobalLog.Print(
                            "Connection#"
                                + ValidationHelper.HashString(this)
                                + "::SubmitRequest() MyLocalPort:"
                                + MyLocalPort
                                + " ERROR: adding HttpWebRequest#"
                                + ValidationHelper.HashString(request)
                                + " to tunnelling WaitList"
                        );
                    }
                    else
                    {
                        GlobalLog.Print(
                            "Connection#"
                                ...
                                + "::SubmitRequest() MyLocalPort:"
                                + MyLocalPort
                                + " adding HttpWebRequest#"
                                + ValidationHelper.HashString(request)
                                + " to non-tunnelling WaitList m_WaitList.Count:"
                                + m_WaitList.Count
                        );
                    }
#endif
            ...
            if (expiredIdleConnection)
            {
                GlobalLog.Leave(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::SubmitRequest(), expired idle connection",
                    false
                );
            ...
            if (!request.Async)
            {
                ...
                else if (responseObject is Exception)
                {
                    ...
                    GlobalLog.Leave(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::SubmitRequest (SYNC) - Error waiting for a connection: "
                            + exception.Message,
                        "Status:"
                            + (
                                webException == null
                                    ...
                                    : (
                                        ...
                                        + " Internal Status: "
                                        + webException.InternalStatus.ToString()
                                    )
                            )
                    );
                    return true;
        ...
        {
            GlobalLog.Enter(
                "Connection#" + ValidationHelper.HashString(this) + "::StartRequest",
                "HttpWebRequest#"
                    + ValidationHelper.HashString(request)
                    + " WriteDone:"
                    + m_WriteDone
            ...
            if (m_WriteList.Count == 0)
            {
                ...
                if (
                    ...
                    && m_IdleSinceUtc + TimeSpan.FromMilliseconds(ServicePoint.MaxIdleTime)
                        < DateTime.UtcNow
                )
                {
                    ...
                    GlobalLog.Leave(
                        "Connection#"
                            ...
                            + (int)((DateTime.UtcNow - m_IdleSinceUtc).TotalSeconds)
            ...
            GlobalLog.Print(
                "Connection#"
                    + ValidationHelper.HashString(this)
                    + "::StartRequest() setting WriteDone:"
                    + m_WriteDone.ToString()
                    + " to false"
            );
            m_WriteDone = false;
            GlobalLog.Print(
                "Connection#"
                    + ValidationHelper.HashString(this)
                    + "::StartRequest() m_WriteList adding HttpWebRequest#"
                    + ValidationHelper.HashString(request)
            );
            ...
            if (request.IsTunnelRequest)
            {
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::StartRequest() MyLocalPort:"
                        + MyLocalPort
                        + " setting Tunnelling to true HttpWebRequest#"
                        + ValidationHelper.HashString(request)
                );
                q_Tunnelling = true;
            }
            else
            {
                GlobalLog.Assert(
                    !q_Tunnelling,
                    "Connection#{0}::StartRequest()|MyLocalPort:{1} ERROR: Already tunnelling during non-tunnel request HttpWebRequest#{2}.",
                    ValidationHelper.HashString(this),
                    MyLocalPort,
                    ValidationHelper.HashString(request)
                );
        ...
        )
        {
            ...
            if (needReConnect == TriState.True)
            {
                ...
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::CompleteStartRequest() Queue StartConnection Delegate "
                );
                try
                {
                    if (request.Async)
                ...
                catch (Exception exception)
                {
                    GlobalLog.Print(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::CompleteStartRequest(): exception: "
                            + exception.ToString()
                    );
                ...
                if (!request.Async)
                {
                    GlobalLog.Print(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::CompleteStartRequest() Invoking Async Result"
                    );
        ...
        private HttpWebRequest CheckNextRequest()
        {
            GlobalLog.ThreadContract(
                ThreadKinds.Unknown,
                "Connection#" + ValidationHelper.HashString(this) + "::CheckNextRequest"
            );
            ...
            if (
                !nextRequest.Pipelined
                || nextRequest.HasEntityBody
                || !m_CanPipeline
                || !m_Pipelining
                || m_IsPipelinePaused
            )
            ...
            if (nextRequest != null)
            {
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::CheckNextRequest() Removing request#"
                        + ValidationHelper.HashString(nextRequest)
                        + " from m_WaitList. New Count:"
        ...
        private void CompleteStartConnection(bool async, HttpWebRequest httpWebRequest)
        {
            GlobalLog.Enter(
                ...
                "async:"
                    + async.ToString()
                    + " httpWebRequest:"
                    + ValidationHelper.HashString(httpWebRequest)
            );
            GlobalLog.ThreadContract(
                ThreadKinds.Unknown,
                "Connection#" + ValidationHelper.HashString(this) + "::CompleteStartConnection"
            ...
            try
            {
                ...
                lock (this)
                {
                    ...
                    if (m_WriteList.Count != 0)
                    {
                        GlobalLog.Assert(
                            m_WriteList.Count == 1,
                            "Connection#{0}::CompleteStartConnection()|WriteList is not sized 1.",
                            ValidationHelper.HashString(this)
                        );
                        GlobalLog.Assert(
                            (m_WriteList[0] as HttpWebRequest) == httpWebRequest,
                            "Connection#{0}::CompleteStartConnection()|Last request on write list does not match.",
                            ValidationHelper.HashString(this)
                        );
                    }
                }
                ...
                if (
                    (
                        httpWebRequest.IsWebSocketRequest
                        || httpWebRequest.Address.Scheme == Uri.UriSchemeHttps
                ...
                else
                {
                    if (
                        !Activate(
                            httpWebRequest,
                            async,
                            new GeneralAsyncDelegate(CompleteConnectionWrapper)
                        )
                    )
                    {
                        return;
            ...
            if (!success)
            {
                ...
                GlobalLog.Leave(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::CompleteStartConnection Failed to connect."
                );
                return;
            }
        ...
        private void CompleteConnectionWrapper(object request, object state)
        {
...
#endif
            GlobalLog.Enter(
                "Connection#" + ValidationHelper.HashString(state) + "::CompleteConnectionWrapper",
                "request:" + ValidationHelper.HashString(request)
            );
            ...
            {
                GlobalLog.Print(
                    "CompleteConnectionWrapper() Request#"
                        + ValidationHelper.HashString(request)
            ...
            GlobalLog.Leave(
                "Connection#"
                    ...
                    + "::CompleteConnectionWrapper"
                    + (stateException == null ? string.Empty : " failed")
            );
#if DEBUG
            }
#endif
        ...
        private void CompleteConnection(bool async, HttpWebRequest request)
        {
            GlobalLog.Enter(
                ...
                "async:" + async.ToString() + " request:" + ValidationHelper.HashString(request)
            );
            GlobalLog.ThreadContract(
                ThreadKinds.Unknown,
                "Connection#" + ValidationHelper.HashString(this) + "::CompleteConnection"
        ...
        private void InternalWriteStartNextRequest(
            HttpWebRequest request,
            ref bool calledCloseConnection,
            ref TriState startRequestResult,
            ref HttpWebRequest nextRequest,
            ref ConnectionReturnResult returnResult
        )
        {
            GlobalLog.ThreadContract(
            ...
            lock (this)
            {
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::WriteStartNextRequest() setting WriteDone:"
                        + m_WriteDone.ToString()
                        + " to true"
                );
                m_WriteDone = true;
                ...
                if (!m_KeepAlive || m_Error != WebExceptionStatus.Success || !CanBePooled)
                {
                    GlobalLog.Print(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::WriteStartNextRequest() m_WriteList.Count:"
                            + m_WriteList.Count
                    );
                    if (m_ReadDone)
                    {
                    ...
                    else
                    {
                        if (m_Error != WebExceptionStatus.Success)
                        {
                            GlobalLog.Print(
                                "Connection#"
                                    + ValidationHelper.HashString(this)
                                    + "::WriteStartNextRequest() a Failure, m_Error = "
                                    + m_Error.ToString()
                            );
                        }
                    }
                ...
                else
                {
                    ...
                    GlobalLog.Print(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::WriteStartNextRequest() Non-Error m_WriteList.Count:"
                            + m_WriteList.Count
                            + " m_WaitList.Count:"
                            + m_WaitList.Count
                    );
                    ...
                    if (nextRequest != null)
                    {
                        ...
                        GlobalLog.Assert(
                            startRequestResult != TriState.Unspecified,
        ...
        )
        {
            ...
            InternalWriteStartNextRequest(
                request,
                ref calledCloseConnection,
                ref startRequestResult,
                ref nextRequest,
                ref returnResult
            );
            ...
            GlobalLog.Print(
                "Connection#"
                    + ValidationHelper.HashString(this)
                    + "::WriteStartNextRequest: Pipelining:"
                    + m_Pipelining
                    + " nextRequest#"
                    + ValidationHelper.HashString(nextRequest)
            );
            ...
            if (!calledCloseConnection && startRequestResult != TriState.Unspecified)
            {
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::WriteStartNextRequest calling CompleteStartRequest"
                );
        ...
        )
        {
            ...
            GlobalLog.Assert(
                currentCount >= 0,
                "m_ReservedCount must not be < 0 when decremented."
            );
                ...
                lock (this)
                {
                    if (m_WriteList.Count > 0 && (object)currentRequest == m_WriteList[0])
                    {
                        ...
                        GlobalLog.Print(
                            "Connection#"
                                + ValidationHelper.HashString(this)
                                + "::ReadStartNextRequest() Removed request#"
                                + ValidationHelper.HashString(currentRequest)
                                + " from m_WriteList. New m_WriteList.Count:"
                                + m_WriteList.Count.ToString()
                        );
                    }
                    else
                    {
                        GlobalLog.Print(
                            "Connection#"
                                + ValidationHelper.HashString(this)
                                + "::ReadStartNextRequest() The request#"
                                + ValidationHelper.HashString(currentRequest)
                                + " was disassociated so do nothing.  m_WriteList.Count:"
                                + m_WriteList.Count.ToString()
                        );
                        mustExit = true;
                        ;
                    ...
                    if (!mustExit)
                    {
                        ...
                        if (!m_KeepAlive || m_Error != WebExceptionStatus.Success || !CanBePooled)
                        {
                            GlobalLog.Print(
                                "Connection#"
                                    + ValidationHelper.HashString(this)
                                    + "::ReadStartNextRequest() KeepAlive:"
                                    + m_KeepAlive
                                    + " WriteDone:"
                                    + m_WriteDone
                            );
                    ...
                    else
                    {
                        if (m_BytesScanned < m_BytesRead)
                        {
                            GlobalLog.Print(
                                "Connection#"
                                    + ValidationHelper.HashString(this)
                                    + "::ReadStartNextRequest() Calling ReadComplete, bytes unparsed = "
                                    + (m_BytesRead - m_BytesScanned)
                            );
                        ...
                        else if (Thread.CurrentThread.IsThreadPoolThread)
                        {
                            GlobalLog.Print(
                                "Connection#"
                                    + ValidationHelper.HashString(this)
                                    + "::ReadStartNextRequest() Calling PostReceive()."
                            );
                            PostReceive();
                        }
                        else
                        {
                            ...
                            GlobalLog.Print(
                                "Connection#"
                                    + ValidationHelper.HashString(this)
                                    + "::ReadStartNextRequest() ThreadPool.UnsafeQueueUserWorkItem(m_PostReceiveDelegate, this)"
                            );
        ...
        internal void MarkAsReserved()
        {
            ...
            GlobalLog.Assert(
                currentCount > 0,
                "m_ReservedCount must not be less or equal zero after incrementing."
            );
        }
        ...
        internal void CheckStartReceive(HttpWebRequest request)
        {
            ...
            if (!request.Async)
            {
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::CheckStartReceive() SYNC request, calling ConnectionReaderAsyncResult.InvokeCallback()"
                );
            ...
            else if (m_BytesScanned < m_BytesRead)
            {
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::CheckStartReceive() Calling ReadComplete, bytes unparsed = "
                        + (m_BytesRead - m_BytesScanned)
                );
                ReadComplete(0, WebExceptionStatus.Success);
            }
            else if (Thread.CurrentThread.IsThreadPoolThread)
            {
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::CheckStartReceive() Calling PostReceive()."
                );
                PostReceive();
            }
            else
            {
                ...
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::CheckStartReceive() ThreadPool.UnsafeQueueUserWorkItem(m_PostReceiveDelegate, this)"
                );
                ThreadPool.UnsafeQueueUserWorkItem(m_PostReceiveDelegate, this);
            }
        }
        ...
        {
            GlobalLog.Enter(
                "Connection#" + ValidationHelper.HashString(this) + "::ParseStatusLine",
                statusLineLength.ToString(NumberFormatInfo.InvariantInfo)
                    + ", "
                    + bytesParsed.ToString(NumberFormatInfo.InvariantInfo)
                    + ", "
                    + statusState.ToString(NumberFormatInfo.InvariantInfo)
            );
            GlobalLog.ThreadContract(
                ThreadKinds.Unknown,
                "Connection#" + ValidationHelper.HashString(this) + "::ParseStatusLine"
            ...
            GlobalLog.Assert(
                ...
                "Connection#{0}::ParseStatusLine()|(statusLineLength - bytesParsed) < 0",
                ValidationHelper.HashString(this)
            );
            ...
            while (
                (bytesParsed < statusLineLength)
                && (statusLine[bytesParsed] != '\r')
                && (statusLine[bytesParsed] != '\n')
            )
            {
                ...
                ++bytesParsed;
                if (
                    m_MaximumResponseHeadersLength >= 0
                    && ++m_TotalResponseHeadersLength >= m_MaximumResponseHeadersLength
                )
                {
            ...
            GlobalLog.Print(
                "Connection#"
                    + ValidationHelper.HashString(this)
                    + "::ParseStatusLine() StatusCode:"
                    + statusLineInts[StatusCodeNumber]
                    + " MajorVersionNumber:"
                    + statusLineInts[MajorVersionNumber]
                    + " MinorVersionNumber:"
                    + statusLineInts[MinorVersionNumber]
                    + " StatusDescription:"
                    + ValidationHelper.ToString(statusDescription)
            );
            GlobalLog.Leave(
                "Connection#" + ValidationHelper.HashString(this) + "::ParseStatusLine",
                parseStatus.ToString()
        ...
        {
            GlobalLog.Enter(
                "Connection::ParseStatusLineStrict",
                statusLineLength.ToString(NumberFormatInfo.InvariantInfo)
                    + ", "
                    + bytesParsed.ToString(NumberFormatInfo.InvariantInfo)
                    + ", "
            ...
            GlobalLog.Assert(
                (statusLineLength - bytesParsed) >= 0,
                "Connection::ParseStatusLineStrict()|(statusLineLength - bytesParsed) < 0"
            );
            GlobalLog.Assert(
                maximumHeaderLength <= 0 || totalBytesParsed <= maximumHeaderLength,
                "Connection::ParseStatusLineStrict()|Headers already read exceeds limit."
            );
            ...
            int effectiveMax =
                maximumHeaderLength <= 0
                    ? int.MaxValue
                    : (maximumHeaderLength - totalBytesParsed + bytesParsed);
            ...
            fixed (byte* byteBuffer = statusLine)
            {
                ...
                switch (statusState)
                {
                    case BeforeVersionNumbers:
                        ...
                        while (
                            totalBytesParsed - initialBytesParsed + bytesParsed
                            < BeforeVersionNumberBytes.Length
                        )
                        {
                            if (
                                (byte)
                                    BeforeVersionNumberBytes[
                                        totalBytesParsed - initialBytesParsed + bytesParsed
                                    ] != byteBuffer[bytesParsed]
                            )
                            {
                                parseStatus = DataParseStatus.Invalid;
                                goto quit;
                    ...
                    case MajorVersionNumber:
                        while (byteBuffer[bytesParsed] != '.')
                        {
                            ...
                            statusLineValues.MajorVersion =
                                statusLineValues.MajorVersion * 10 + byteBuffer[bytesParsed] - '0';
                    ...
                    case MinorVersionNumber:
                        ...
                        while (byteBuffer[bytesParsed] != ' ')
                        {
                            ...
                            statusLineValues.MinorVersion =
                                statusLineValues.MinorVersion * 10 + byteBuffer[bytesParsed] - '0';
                    ...
                    case StatusCodeNumber:
                        ...
                        while (byteBuffer[bytesParsed] >= '0' && byteBuffer[bytesParsed] <= '9')
                        {
                            ...
                            statusLineValues.StatusCode =
                                statusLineValues.StatusCode * 10 + byteBuffer[bytesParsed] - '0';
                    ...
                    case AfterStatusCode:
                    {
                        ...
                        if (statusLineValues.StatusDescription == null)
                        {
                            ...
                            {
                                if (
                                    bytesParsed < effectiveMax - s.Length
                                    && byteBuffer[bytesParsed] == (byte)s[0]
                                )
                                {
                                    int i;
            ...
            GlobalLog.Print(
                "Connection::ParseStatusLineStrict() StatusCode:"
                    + statusLineValues.StatusCode
                    + " MajorVersionNumber:"
                    + statusLineValues.MajorVersion
                    + " MinorVersionNumber:"
                    + statusLineValues.MinorVersion
                    + " StatusDescription:"
                    + ValidationHelper.ToString(statusLineValues.StatusDescription)
            );
        ...
        private void SetStatusLineParsed()
        {
            ...
            m_ResponseData.m_IsVersionHttp11 =
                m_StatusLineValues.MajorVersion >= 1 && m_StatusLineValues.MinorVersion >= 1;
            if (
            ...
            )
            {
                ...
                ServicePoint.HttpBehaviour = m_ResponseData.m_IsVersionHttp11
        ...
        )
        {
            ...
            {
                transferEncodingString = transferEncodingString.ToLower(
                ...
                if (contentLengthString != null)
                {
                    ...
                    if (contentLength < 0)
                    {
                        GlobalLog.Leave(
                            "Connection#"
                                + ValidationHelper.HashString(this)
                                + "::ProcessHeaderData - ContentLength value in header: "
                                + contentLengthString
                                + ", HttpWebRequest#"
                                + ValidationHelper.HashString(m_CurrentRequest)
                        );
            ...
            GlobalLog.Print(
                "Connection#"
                    + ValidationHelper.HashString(this)
                    + "::ProcessHeaderData() Content-Length parsed:"
                    + contentLength.ToString(NumberFormatInfo.InvariantInfo)
            );

            dummyResponseStream =
                !request.CanGetResponseStream
                || m_ResponseData.m_StatusCode < HttpStatusCode.OK
                || m_ResponseData.m_StatusCode == HttpStatusCode.NoContent
                || (m_ResponseData.m_StatusCode == HttpStatusCode.NotModified && contentLength < 0);
            ...
            if (m_KeepAlive)
            {
                ...
                //-----------------------------------------------------------------------------------------------
                    ...
                    m_ResponseData.m_StatusCode == HttpStatusCode.Forbidden
                ...
                //     so we don't have a way of eliminating the 1st auth leg.
                    ...
                    && (
                        (
                            request.CurrentMethod == KnownHttpVerb.Post
                            || request.CurrentMethod == KnownHttpVerb.Put
                        )
                        && m_MaximumUnauthorizedUploadLength >= 0
                        && request.ContentLength > m_MaximumUnauthorizedUploadLength
                        && (
                            request.CurrentAuthenticationState == null
                            || request.CurrentAuthenticationState.Module == null
                        )
                    )
                )
                {
                ...
                else
                {
                    ...
                    if (
                        connection == null
                        && ((ServicePoint.InternalProxyServicePoint) || (request.IsTunnelRequest))
                    )
                    {
                    ...
                    if (
                        (haveClose && ServicePoint.HttpBehaviour == HttpBehaviour.HTTP11)
        ...
        {
            get { return m_NonKeepAliveRequestPipelined; }
        }
        ...
        private DataParseStatus ParseStreamData(ref ConnectionReturnResult returnResult)
        {
            ...
            if (m_CurrentRequest == null)
            {
                GlobalLog.Leave(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::ParseStreamData - Aborted Request, return DataParseStatus.Invalid"
                );
            ...
            long contentLength = ProcessHeaderData(
                ref fHaveChunked,
                m_CurrentRequest,
                out dummyResponseStream
            );

            GlobalLog.Assert(
                !fHaveChunked || contentLength == -1,
                "Connection#{0}::ParseStreamData()|fHaveChunked but contentLength != -1",
                ValidationHelper.HashString(this)
            );
            ...
            if (
                m_CurrentRequest.IsWebSocketRequest
                && m_ResponseData.m_StatusCode == HttpStatusCode.SwitchingProtocols
            )
            {
                m_ResponseData.m_ConnectStream = new ConnectStream(
                    this,
                    m_ReadBuffer,
                    m_BytesScanned,
                    bufferLeft,
            ...
            else if (bytesToCopy != -1 && bytesToCopy <= bufferLeft)
            {
                m_ResponseData.m_ConnectStream = new ConnectStream(
                    ...
                    m_BytesScanned,
                    bytesToCopy,
                    dummyResponseStream ? 0 : contentLength,
                    fHaveChunked,
                    m_CurrentRequest
                );
        ...
        private DataParseStatus ParseResponseData(
            ref ConnectionReturnResult returnResult,
            out bool requestDone,
            out CoreResponseData continueResponseData
        )
        {
            GlobalLog.Enter(
                "Connection#" + ValidationHelper.HashString(this) + "::ParseResponseData()"
            );
            ...
            switch (m_ReadState)
            {
                case ReadState.Start:
                    ...
                    if (m_CurrentRequest == null)
                    {
                        lock (this)
                        {
                            if (
                                m_WriteList.Count == 0
                                || ((m_CurrentRequest = m_WriteList[0] as HttpWebRequest) == null)
                            )
                            {
                                m_ParseError.Section = WebParseErrorSection.Generic;
                    ...
                    m_MaximumResponseHeadersLength =
                        m_CurrentRequest.MaximumResponseHeadersLength * 1024;
                    m_ResponseData = new CoreResponseData();
                ...
                case ReadState.StatusLine:
                    ...
                    if (parseSubStatus == DataParseStatus.Done)
                    {
                        if (Logging.On)
                            Logging.PrintInfo(
                                ...
                                this,
                                SR.GetString(
                                    SR.net_log_received_status_line,
                                    m_StatusLineValues.MajorVersion
                                        + "."
                                        + m_StatusLineValues.MinorVersion,
                                    m_StatusLineValues.StatusCode,
                    ...
                    else if (parseSubStatus != DataParseStatus.NeedMoreData)
                    {
                        ...
                        GlobalLog.Print(
                            "Connection#"
                                + ValidationHelper.HashString(this)
                                + "::ParseResponseData() ParseStatusLine() parseSubStatus:"
                                + parseSubStatus.ToString()
                        );
                ...
                case ReadState.Headers:
                    ...
                    if (
                        parseSubStatus == DataParseStatus.Invalid
                        || parseSubStatus == DataParseStatus.DataTooBig
                    )
                    {
                        //
                        // report error
                        ...
                        GlobalLog.Print(
                            "Connection#"
                                + ValidationHelper.HashString(this)
                                + "::ParseResponseData() ParseHeaders() parseSubStatus:"
                                + parseSubStatus.ToString()
                        );
                        parseStatus = parseSubStatus;
                        break;
                    ...
                    else if (parseSubStatus == DataParseStatus.Done)
                    {
                        if (Logging.On)
                            Logging.PrintInfo(
                                ...
                                this,
                                SR.GetString(
                                    SR.net_log_received_headers,
                                    m_ResponseData.m_ResponseHeaders.ToString(true)
                                )
                            );
                        ...
                        GlobalLog.Print(
                            "Connection#"
                                + ValidationHelper.HashString(this)
                                + "::ParseResponseData() DataParseStatus.Done StatusCode:"
                                + (int)m_ResponseData.m_StatusCode
                                + " m_BytesRead:"
                                + m_BytesRead
                                + " m_BytesScanned:"
                                + m_BytesScanned
                        ...
                        if (m_IISVersion == -1)
                        {
                            ...
                            if (
                                m_IISVersion == -1
                                && m_ResponseData.m_StatusCode != HttpStatusCode.Continue
                            )
                            {
                                m_IISVersion = 0;
                            }
                        ...
                        if (
                            m_ResponseData.m_StatusCode == HttpStatusCode.Continue
                            || m_ResponseData.m_StatusCode == HttpStatusCode.BadRequest
                        )
                        {
                            GlobalLog.Assert(
                                m_CurrentRequest != null,
                                "Connection#{0}::ParseResponseData()|m_CurrentRequest == null",
                                ValidationHelper.HashString(this)
                            );
                            GlobalLog.Print(
                                "Connection#"
                                    + ValidationHelper.HashString(this)
                                    + "::ParseResponseData() HttpWebRequest#"
                                    + ValidationHelper.HashString(m_CurrentRequest)
                            );
                            ...
                            if (m_ResponseData.m_StatusCode == HttpStatusCode.BadRequest)
                            {
                                ...
                                GlobalLog.Print(
                                    "Connection#"
                                        + ValidationHelper.HashString(this)
                                        + "::ParseResponseData() got a 400 StatusDescription:"
                                        + m_ResponseData.m_StatusDescription
                                );
                                if (
                                    ServicePoint.HttpBehaviour == HttpBehaviour.HTTP11
                                    ...
                                    && string.Compare(
                                        m_ResponseData.m_StatusDescription,
                                        "Bad Request ( The HTTP request includes a non-supported header. Contact the Server administrator.  )",
                                ...
                                )
                                {
                                    GlobalLog.Print(
                                        "Connection#"
                                            + ValidationHelper.HashString(this)
                                            + "::ParseResponseData() downgrading server to HTTP11PartiallyCompliant."
                                    );
        ...
        internal void CloseOnIdle()
        {
            ...
            GlobalLog.ThreadContract(
                ...
                "Connection#" + ValidationHelper.HashString(this) + "::CloseOnIdle"
            );
        ...
        {
            GlobalLog.Enter(
                "Connection#" + ValidationHelper.HashString(this) + "::AbortOrDisassociate",
                "request#" + ValidationHelper.HashString(request)
            );
            GlobalLog.ThreadContract(
                ThreadKinds.Unknown,
            ...
            lock (this)
            {
                ...
                if (idx == -1)
                {
                    ...
                    if (m_WaitList.Count > 0)
                    {
                        foundItem = m_WaitList.Find(o =>
                            object.ReferenceEquals(o.Request, request)
                        );
                    }
                ...
                else if (idx != 0)
                {
                    ...
                    GlobalLog.Leave(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::AbortOrDisassociate()",
                        "Request was Disassociated from the Write List, idx = " + idx
                    );
                    return true;
                }
        ...
        internal void AbortSocket(bool isAbortState)
        {
            ...
            GlobalLog.ThreadContract(
                ...
                "Connection#" + ValidationHelper.HashString(this) + "::AbortSocket"
            );
            GlobalLog.Enter(
                "Connection#" + ValidationHelper.HashString(this) + "::Abort",
                "isAbortState:" + isAbortState.ToString()
            );
        ...
        private void PrepareCloseConnectionSocket(ref ConnectionReturnResult returnResult)
        {
            ...
            if (m_WriteList.Count != 0 || m_WaitList.Count != 0)
            {
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::PrepareCloseConnectionSocket() m_WriteList.Count:"
                        + m_WriteList.Count
                );
                ...
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::PrepareCloseConnectionSocket() m_WaitList.Count:"
                        + m_WaitList.Count
                );
                ...
                if (lockedRequest != null)
                {
                    ...
                    GlobalLog.Print(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::PrepareCloseConnectionSocket() looking for HttpWebRequest#"
                            + ValidationHelper.HashString(lockedRequest)
                    );
                ...
                if (m_WaitList.Count != 0)
                {
                    ...
                    ConnectionReturnResult.AddExceptionRange(
                        ref returnResult,
                        requestArray,
                        ExceptionHelper.IsolatedException
                    );
                }
                ...
                if (m_WriteList.Count != 0)
                {
                    ...
                    if (!(theException is WebException) && !(theException is SecurityException))
                    {
                        if (m_Error == WebExceptionStatus.ServerProtocolViolation)
                        {
                            ...
                            if (m_ParseError.Code != WebParseErrorCode.Generic)
                            {
                                detailedInfo +=
                                    " Detail="
                                    + SR.GetString(
                                        "net_WebResponseParseError_" + m_ParseError.Code.ToString()
                                    );
                            }
                            if (detailedInfo.Length != 0)
                                errorString += "." + detailedInfo;

                            theException = new WebException(
                                errorString,
                                theException,
                                m_Error,
                                null,
                        ...
                        else if (m_Error == WebExceptionStatus.RequestCanceled)
                        {
                            ...
                            m_Error == WebExceptionStatus.MessageLengthLimitExceeded
                    ...
                    WebException pipelineException = new WebException(
                        NetRes.GetWebStatusString(
                            "net_connclosed",
                ...
                {
                    GlobalLog.Print(
                        "Request removed from WaitList#" + ValidationHelper.HashString(item.Request)
                    );
                }
                ...
                {
                    GlobalLog.Print(
                        "Request removed from m_WriteList#" + ValidationHelper.HashString(request)
                    );
                }
#endif
        ...
        )
        {
            lock (this)
            {
                ...
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::HandleError() current HttpWebRequest#"
                        + ValidationHelper.HashString(m_CurrentRequest)
                );
        ...
        private void ReadCallback(IAsyncResult asyncResult)
        {
            ...
            GlobalLog.Assert(
                asyncResult != null,
                "Connection#{0}::ReadCallback()|asyncResult == null",
                ValidationHelper.HashString(this)
            );
            GlobalLog.Assert(
                (asyncResult is OverlappedAsyncResult || asyncResult is LazyAsyncResult),
                "Connection#{0}::ReadCallback()|asyncResult is not OverlappedAsyncResult.",
                ValidationHelper.HashString(this)
            );
            ...
            catch (Exception exception)
            {
                ...
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::ReadCallback() EndRead() errorStatus:"
        ...
        internal void PollAndRead(HttpWebRequest request, bool userRetrievedStream)
        {
            ...
            GlobalLog.Print(
                "Connection#"
                    + ValidationHelper.HashString(this)
                    + "::PollAndRead() InternalPeekCompleted:"
                    + request.ConnectionReaderAsyncResult.InternalPeekCompleted.ToString()
                    + " Result:"
                    + ValidationHelper.ToString(request.ConnectionReaderAsyncResult.Result)
            );
            if (
                request.ConnectionReaderAsyncResult.InternalPeekCompleted
                && request.ConnectionReaderAsyncResult.Result == null
                && CanBePooled
            )
            {
        ...
        internal void SyncRead(HttpWebRequest request, bool userRetrievedStream, bool probeRead)
        {
            GlobalLog.Enter(
                "Connection#"
                    + ValidationHelper.HashString(this)
                    + "::SyncRead(byte[]) request#"
                    + ValidationHelper.HashString(request)
                    + (probeRead ? ", Probe read = TRUE" : string.Empty)
            );
            GlobalLog.ThreadContract(
                ThreadKinds.Sync,
                ...
                do
                {
                        ...
                        if (bytesRead != 0)
                        {
                            ...
                            if (pollSuccess)
                            {
                                ...
                                bytesRead = Read(
                                    m_ReadBuffer,
                                    m_BytesRead,
                                    m_ReadBuffer.Length - m_BytesRead
                                );
                                errorStatus = WebExceptionStatus.Success;
                                if (bytesRead == 0)
                                    bytesRead = -1; // 0 is reserved for re-entry on already buffered data
                    ...
                    catch (Exception exception)
                    {
                        ...
                        GlobalLog.Print(
                            "Connection#"
                                + ValidationHelper.HashString(this)
                                + "::SyncRead() Read() threw errorStatus:"
                                + errorStatus.ToString()
                                + " bytesRead:"
                                + bytesRead.ToString()
                        );
                    ...
                    !requestDone
                    && (userRetrievedStream || requestContinueCount == request.RequestContinueCount)
                );
            }
            finally
            {
            ...
            if (probeRead)
            {
                ...
                else
                {
                    GlobalLog.Print(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::SyncRead() Poll has timed out, calling SetRequestContinue()."
                    );
        ...
        private bool ReadComplete(int bytesRead, WebExceptionStatus errorStatus)
        {
            ...
            try
            {
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::ReadComplete() m_BytesRead:"
                        + m_BytesRead.ToString()
                        + " m_BytesScanned:"
                        + m_BytesScanned
                        + " (+= new bytesRead:"
                        + bytesRead.ToString()
                ...
                DataParseStatus parseStatus = ParseResponseData(
                    ref returnResult,
                    out requestDone,
                    out continueResponseData
                );
                ...
                else if (parseStatus == DataParseStatus.Done)
                {
                    GlobalLog.Print(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::ReadComplete() [The response stream is ready] parseStatus = DataParseStatus.Done"
                    );
                    goto done;
                }
                ...
                if (parseStatus == DataParseStatus.NeedMoreData)
                {
                    GlobalLog.Print(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::ReadComplete() OLD buffer. m_ReadBuffer.Length:"
                            + m_ReadBuffer.Length.ToString()
                            + " m_BytesRead:"
                            + m_BytesRead.ToString()
                            + " m_BytesScanned:"
                            + m_BytesScanned.ToString()
                    );
                    ...
                    if (unparsedDataSize != 0)
                    {
                        if (m_BytesScanned == 0 && m_BytesRead == m_ReadBuffer.Length)
                        {
                            ...
                            byte[] newReadBuffer = new byte[
                                m_ReadBuffer.Length * 2 /*+ ReadBufferSize*/
                            ];
                    ...
                    GlobalLog.Print(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::ReadComplete() NEW or shifted buffer. m_ReadBuffer.Length:"
                            + m_ReadBuffer.Length.ToString()
                            + " m_BytesRead:"
                            + m_BytesRead.ToString()
                            + " m_BytesScanned:"
                    ...
                    if (currentRequest != null)
                    {
                        ...
                        if (currentRequest.Async)
                        {
                            GlobalLog.Print(
                                "Connection#"
                                    + ValidationHelper.HashString(this)
                                    + "::ReadComplete() Reposting Async Read.  Buffer:"
                                    + ValidationHelper.HashString(m_ReadBuffer)
                                    + " BytesScanned:"
                                    + m_BytesScanned.ToString()
                            );
                            ...
                            if (Thread.CurrentThread.IsThreadPoolThread)
                            {
                                GlobalLog.Print(
                                    "Connection#"
                                        + ValidationHelper.HashString(this)
                                        + "::ReadComplete() Calling PostReceive()."
                                );
                                PostReceive();
                            }
                            else
                            {
                                ...
                                GlobalLog.Print(
                                    "Connection#"
                                        + ValidationHelper.HashString(this)
                                        + "::ReadComplete() ThreadPool.UnsafeQueueUserWorkItem(m_PostReceiveDelegate, this)"
                                );
                                ThreadPool.UnsafeQueueUserWorkItem(m_PostReceiveDelegate, this);
                            }
                        }
            ...
            try
            {
                ...
                if (
                    currentRequest != null
                    && currentRequest.HttpWriteMode != HttpWriteMode.None
                    && (
                        continueResponseData != null
                        // not a 100 continue, but we have buffering so we don't care what it was.
                        || (
                            returnResult != null
                            && returnResult.IsNotEmpty
                            && currentRequest.AllowWriteStreamBuffering
                        )
                    )
                )
                {
        ...
        internal void Write(ScatterGatherBuffers writeBuffer)
        {
            GlobalLog.Enter(
                "Connection#"
                    + ValidationHelper.HashString(this)
                    + "::Write(ScatterGatherBuffers) networkstream#"
                    + ValidationHelper.HashString(NetworkStream)
            );
            //
            // parameter validation
            //
            GlobalLog.Assert(
                writeBuffer != null,
                "Connection#{0}::Write(ScatterGatherBuffers)|writeBuffer == null",
                ValidationHelper.HashString(this)
            );
            //
            ...
            GlobalLog.Leave(
                "Connection#"
                    + ValidationHelper.HashString(this)
                    + "::Write(ScatterGatherBuffers) this:"
                    + ValidationHelper.HashString(this)
                    + " writeBuffer.Size:"
                    + writeBuffer.Length.ToString()
            );
        }
        ...
        private static void PostReceiveWrapper(object state)
        {
            ...
            GlobalLog.Enter(
                "Connection#"
                    + ValidationHelper.HashString(thisConnection)
                    + "::PostReceiveWrapper",
                "Cnt#" + ValidationHelper.HashString(thisConnection)
            );
            GlobalLog.Assert(
                thisConnection != null,
                "Connection#{0}::PostReceiveWrapper()|thisConnection == null",
                ValidationHelper.HashString(thisConnection)
            );
        ...
        private void PostReceive()
        {
            ...
            try
            {
                GlobalLog.Assert(
                    m_BytesScanned == 0,
                    "PostReceive()|A receive should not be posted when m_BytesScanned != 0 (the data should be moved to offset 0)."
                );
        ...
        private static void TunnelThroughProxyWrapper(IAsyncResult result)
        {
            ...
            HttpWebRequest originalReq = (HttpWebRequest)
            ...
            try
            {
                ...
                conn.NetworkStream = new NetworkStream(
                    connectStream.Connection.NetworkStream,
                    true
                );
            ...
            catch (Exception exception)
            {
                ...
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(conn)
                        + "::TunnelThroughProxyCallback() exception occurred: "
                        + exception
                );
            }
            if (!success)
        ...
        {
            GlobalLog.Enter(
                "Connection#" + ValidationHelper.HashString(this) + "::TunnelThroughProxy",
                "proxy="
                    + proxy
                    + ", async="
                    + async
                    + ", originalRequest #"
                    + ValidationHelper.HashString(originalRequest)
            );
            ...
            try
            {
                ...
                connectRequest.Credentials =
                    originalRequest.InternalProxy == null
                        ? null
                        : originalRequest.InternalProxy.Credentials;
                ...
                if (async)
                {
                    ...
                    if (!asyncResult.CompletedSynchronously)
                    {
                        GlobalLog.Leave(
                            "Connection#"
                                + ValidationHelper.HashString(this)
                                + "::TunnelThroughProxy completed asynchronously",
                            true
                        );
                        return true;
                    }
            ...
            catch (Exception exception)
            {
                ...
                GlobalLog.Print(
                    "Connection#"
                        + ValidationHelper.HashString(this)
                        + "::TunnelThroughProxy() exception occurred: "
                        + exception
                );
            }
        ...
        private void CheckIdle()
        {
            ...
            GlobalLog.ThreadContract(
                ThreadKinds.Unknown,
                ThreadKinds.SafeSources | ThreadKinds.Finalization | ThreadKinds.Timer,
                "Connection#" + ValidationHelper.HashString(this) + "::CheckIdle"
            );
            ...
            if (!m_Idle && this.BusyCount == 0)
            {
                ...
                if (ConnectionGroup != null)
                {
                    GlobalLog.Print(
                        "Connection#"
                            + ValidationHelper.HashString(this)
                            + "::CheckIdle() - calling ConnectionGoneIdle()"
                    );
        ...
        {
            GlobalLog.Print(
                "WaitList[" + currentPos.ToString() + "] Req: #" + ValidationHelper.HashString(req)
            );
        }
