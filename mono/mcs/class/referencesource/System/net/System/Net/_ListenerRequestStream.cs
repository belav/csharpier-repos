    ...
    unsafe class HttpRequestStream : Stream
    {
            ...
            if (m_DataChunkIndex == -1 && dataRead < size)
            {
                ...
                fixed (byte* pBuffer = buffer)
                {
                    ...
                    GlobalLog.Print(
                        "HttpRequestStream#"
                            + ValidationHelper.HashString(this)
                            + "::Read() calling UnsafeNclNativeMethods.HttpApi.HttpReceiveRequestEntityBody"
                    );
                    ...
                    GlobalLog.Print(
                        "HttpRequestStream#"
                            + ValidationHelper.HashString(this)
                            + "::Read() call to UnsafeNclNativeMethods.HttpApi.HttpReceiveRequestEntityBody returned:"
                            + statusCode
                            + " dataRead:"
                            + dataRead
                    );
                ...
                try
                {
                    fixed (byte* pBuffer = buffer)
                    {
                        ...
                        GlobalLog.Print(
                            "HttpRequestStream#"
                                + ValidationHelper.HashString(this)
                                + "::BeginRead() calling UnsafeNclNativeMethods.HttpApi.HttpReceiveRequestEntityBody"
                        );
                        ...
                        GlobalLog.Print(
                            "HttpRequestStream#"
                                + ValidationHelper.HashString(this)
                                + "::BeginRead() call to UnsafeNclNativeMethods.HttpApi.HttpReceiveRequestEntityBody returned:"
                                + statusCode
                                + " dataRead:"
                                + dataRead
                        );
        ...
        public override int EndRead(IAsyncResult asyncResult)
        {
            ...
            GlobalLog.Print(
                "HttpRequestStream#"
                    + ValidationHelper.HashString(this)
                    + "::EndRead() returning returnValue:"
                    + ValidationHelper.ToString(returnValue)
            );
            if (Logging.On)
        ...
        {
            GlobalLog.Print(
                "HttpRequestStream#" + ValidationHelper.HashString(this) + "::SwitchToOpaqueMode()"
            );
            m_InOpaqueMode = true;
        }
        ...
        unsafe class HttpRequestStreamAsyncResult : LazyAsyncResult
        {
                    ...
                    else
                    {
                        ...
                        if (Logging.On)
                            Logging.Dump(
                                ...
                                asyncResult,
                                "Callback",
                                (IntPtr)asyncResult.m_pPinnedBuffer,
                                (int)numBytes
                            );
                    }
                    GlobalLog.Print(
            ...
            )
            {
                ...
                HttpRequestStreamAsyncResult asyncResult =
                    callbackOverlapped.AsyncResult as HttpRequestStreamAsyncResult;
                GlobalLog.Print(
                    "HttpRequestStreamAsyncResult#"
                        + ValidationHelper.HashString(asyncResult)
                        + "::Callback() errorCode:0x"
