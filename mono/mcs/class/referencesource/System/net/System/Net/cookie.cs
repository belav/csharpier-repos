...
namespace System.Net
{
    ...
    public sealed class Cookie
    {
        ...
        {
            if (
                ValidationHelper.IsBlankString(value)
                || value[0] == '$'
        ...
        private string _Path
        {
            get
            {
                return (Plain || m_path_implicit || (m_path.Length == 0))
                    ? string.Empty
                    : (SpecialAttributeLiteral + PathAttributeName + EqualsLiteral + m_path);
            }
        }
        ...
        )
        {
            ...
            )
            {
                if (isThrow)
                    throw new CookieException(
                        SR.GetString(SR.net_cookie_attribute, CommentAttributeName, Comment)
                    );
                return false;
            }
            ...
            )
            {
                ...
                {
                    throw new CookieException(
                        SR.GetString(SR.net_cookie_attribute, PathAttributeName, Path)
                    );
                }
                return false;
            }
            ...
            else
            {
                if (!m_domain_implicit)
                {
                    ...
                    if (
                        isLocalDomain
                        && (
                            string.Compare(localDomain, domain, StringComparison.OrdinalIgnoreCase)
                            == 0
                        )
                    )
                    {
                ...
                if (!valid)
                {
                    ...
                    {
                        throw new CookieException(
                            SR.GetString(SR.net_cookie_attribute, DomainAttributeName, m_domain)
                        );
                    }
                    return false;
                }
            ...
            else
            {
                ...
                if (!path.StartsWith(CookieParser.CheckQuoted(m_path)))
                {
                    ...
                    {
                        throw new CookieException(
                            SR.GetString(SR.net_cookie_attribute, PathAttributeName, m_path)
                        );
                    }
                    return false;
                }
            ...
            if (m_port_implicit == false)
            {
                ...
                if (!valid)
                {
                    ...
                    {
                        throw new CookieException(
                            SR.GetString(SR.net_cookie_attribute, PortAttributeName, m_port)
                        );
                    }
                    return false;
                }
        ...
        public string Port
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    {
                        throw new CookieException(
                            SR.GetString(SR.net_cookie_attribute, PortAttributeName, value)
                        );
                    }
        ...
        internal CookieVariant Variant
        {
            ...
            set
            {
                ...
                GlobalLog.Assert(
                    value == CookieVariant.Rfc2965,
                    "Cookie#{0}::set_Variant()|value:{1}",
    ...
    internal class CookieTokenizer
    {
        ...
        {
            get { return m_eofCookie; }
            set { m_eofCookie = value; }
        }

        internal bool Eof
        {
            get { return m_index >= m_length; }
        }
                            ...
                            switch (m_tokenizer.Token)
                            {
                                ...
                                case CookieToken.MaxAge:
                                    if (!expiresSet)
                                    {
                                        ...
                                        {
                                            cookie.Expires = DateTime.Now.AddSeconds(
                                                (double)parsed
                                            );
                                        }
                                        else
