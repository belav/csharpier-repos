...
namespace System.Net
{
    ...
    internal class BasicClient : IAuthenticationModule
    {
        ...
        )
        {
            ...
            GlobalLog.Assert(
                credentials != null,
                "BasicClient::Authenticate()|credentials == null"
            );
#if !FEATURE_PAL
            ...
            GlobalLog.Assert(
                httpWebRequest != null,
                "BasicClient::Authenticate()|httpWebRequest == null"
            );
        ...
        {
            GlobalLog.Print(
                "BasicClient::Lookup(): ChallengedUri:" + httpWebRequest.ChallengedUri.ToString()
            );
            ...
            GlobalLog.Print(
                "BasicClient::Lookup() GetCredential() returns:" + ValidationHelper.ToString(NC)
            );
            ...
            string rawString =
                ((!ValidationHelper.IsBlankString(domain)) ? (domain + "\\") : "")
                + username
                + ":"
                + NC.InternalGetPassword();
        ...
        internal static byte[] EncodingRightGetBytes(string rawString)
        {
            GlobalLog.Enter(
                ...
                "[" + rawString.Length.ToString() + ":" + rawString + "]"
            );
            //
            ...
            bool canMapToCurrentCodePage =
                string.Compare(rawString, rawCopy, StringComparison.Ordinal) == 0;

            GlobalLog.Print(
                "BasicClient::EncodingRightGetBytes(): canMapToCurrentCodePage:"
                    + canMapToCurrentCodePage.ToString()
            );
            ...
            if (!canMapToCurrentCodePage)
            {
                ...
                GlobalLog.LeaveException(
                    "BasicClient::EncodingRightGetBytes",
                    ExceptionHelper.MethodNotSupportedException
                );
