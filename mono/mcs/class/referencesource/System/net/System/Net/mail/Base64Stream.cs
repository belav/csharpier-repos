...
namespace System.Net
{
    ...
    internal class Base64Stream : DelegatedStream, IEncodableStream
    {
        ...
        )
        {
            ...
            ReadAsyncResult result = new ReadAsyncResult(
                this,
                buffer,
                offset,
                count,
        ...
        public override void Close()
        {
            if (this.writeState != null && WriteState.Length > 0)
            {
                switch (WriteState.Padding)
                {
                    case 2:
                        WriteState.Append(
                            base64EncodeMap[WriteState.LastBits],
                            base64EncodeMap[64],
                            base64EncodeMap[64]
                        );
                        break;
        ...
        )
        {
            ...
            switch (WriteState.Padding)
            {
                case 2:
                    WriteState.Append(
                        base64EncodeMap[WriteState.LastBits | ((buffer[cur] & 0xf0) >> 4)]
                    );
                    if (count == 1)
                    {
                    ...
                    WriteState.Append(
                        base64EncodeMap[
                            ((buffer[cur] & 0x0f) << 2) | ((buffer[cur + 1] & 0xc0) >> 6)
                        ]
                    );
                ...
                case 1:
                    WriteState.Append(
                        base64EncodeMap[WriteState.LastBits | ((buffer[cur] & 0xc0) >> 6)]
                    );
            ...
            for (; cur < calcLength; cur += 3)
            {
                ...
                WriteState.Append(
                    base64EncodeMap[((buffer[cur] & 0x03) << 4) | ((buffer[cur + 1] & 0xf0) >> 4)]
                );
                WriteState.Append(
                    base64EncodeMap[
                        ((buffer[cur + 1] & 0x0f) << 2) | ((buffer[cur + 2] & 0xc0) >> 6)
                    ]
                );
            ...
            if (
                ...
                && (lineLength != -1)
                && (
                    WriteState.CurrentLineLength + sizeOfBase64EncodedChar + writeState.FooterLength
                    >= lineLength
                )
            )
            {
            ...
            switch (count % 3)
            {
                case 2: //One character padding needed
                    ...
                    WriteState.Append(
                        base64EncodeMap[
                            ((buffer[cur] & 0x03) << 4) | ((buffer[cur + 1] & 0xf0) >> 4)
                        ]
                    );
