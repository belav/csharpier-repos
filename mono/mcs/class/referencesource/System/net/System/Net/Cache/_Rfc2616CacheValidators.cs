    ...
    internal class Rfc2616
    {
        ...
        public static CacheValidationStatus OnValidateRequest(HttpRequestCacheValidator ctx)
        {
            ...
            */
            ...
            {
                if (
                    ctx.Policy.Level == HttpRequestCacheLevel.Reload
                    || ctx.Policy.Level == HttpRequestCacheLevel.NoCacheNoStore
                )
                {
                    //adjust request headers since retrieval validators will be suppressed upon return.
                    ctx.Request.Headers.AddInternal(HttpKnownHeaderNames.CacheControl, "no-cache");
        ...
        public static CacheFreshnessStatus OnValidateFreshness(HttpRequestCacheValidator ctx)
        {
            ...
            */
            ...
            {
                if (
                    ctx.CacheHeaders.Expires == null
                    && (
                        ctx.CacheEntry.IsPrivateEntry
                            ? ctx.CacheCacheControl.MaxAge == -1
                ...
                if (ctx.CacheHttpVersion.Major <= 1 && ctx.CacheHttpVersion.Minor < 1)
                {
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_uri_with_query_and_cached_resp_from_http_10
                            )
                        );
                    return CacheFreshnessStatus.Stale;
                }
        ...
        public static CacheValidationStatus OnValidateCache(HttpRequestCacheValidator ctx)
        {
            ...
            if (
                enoughFresh
                || ctx.Policy.Level == HttpRequestCacheLevel.CacheOnly
                || ctx.Policy.Level == HttpRequestCacheLevel.CacheIfAvailable
                || ctx.Policy.Level == HttpRequestCacheLevel.CacheOrNextCacheOnly
            )
            {
                ...
                if (Logging.On)
                    Logging.PrintInfo(
                        Logging.RequestCache,
                        SR.GetString(
                            SR.net_log_cache_valid_as_fresh_or_because_policy,
                            ctx.Policy.ToString()
                        )
                    );
        ...
        public static CacheValidationStatus OnValidateResponse(HttpRequestCacheValidator ctx)
        {
            ...
            if (ctx.ResponseCount > 1)
            {
                if (Logging.On)
                    Logging.PrintInfo(
                        Logging.RequestCache,
                        SR.GetString(
                            SR.net_log_cache_accept_based_on_retry_count,
                            ctx.ResponseCount
                        )
                    );
            ...
            )
            {
                if (Logging.On)
                    Logging.PrintError(
                        Logging.RequestCache,
                        SR.GetString(
                            SR.net_log_cache_server_didnt_satisfy_range,
                            ctx.Request.Headers[HttpKnownHeaderNames.Range]
                        )
                    );
                Common.ConstructUnconditionalRefreshRequest(ctx);
                return CacheValidationStatus.RetryResponseFromServer;
                ...
                else if (ctx.RequestRangeCache)
                {
                    ...
                    if (Logging.On)
                        Logging.PrintError(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_304_received_on_unconditional_request_expected_200_206
                            )
                        );
                    Common.ConstructUnconditionalRefreshRequest(ctx);
                    return CacheValidationStatus.RetryResponseFromServer;
            ...
            if (
                ctx.CacheHttpVersion.Major <= 1
                && resp.ProtocolVersion.Major <= 1
                && ctx.CacheHttpVersion.Minor < 1
                && resp.ProtocolVersion.Minor < 1
        ...
        public static CacheValidationStatus OnUpdateCache(HttpRequestCacheValidator ctx)
        {
            ...
            *********/
            ...
            CacheValidationStatus noUpdateResult =
                (
                    ctx.RequestMethod >= HttpMethod.Post && ctx.RequestMethod <= HttpMethod.Delete
                    || ctx.RequestMethod == HttpMethod.Other
                )
            ...
            else if (resp.StatusCode == HttpStatusCode.PartialContent)
            {
                ...
                if (
                    ctx.CacheEntry.StreamSize != ctx.ResponseRangeStart
                    && ctx.ResponseRangeStart != 0
                )
                {
                    if (Logging.On)
                        Logging.PrintWarning(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_partial_resp_not_combined_with_existing_entry,
        ...
        internal static class Common
        {
            ...
            public static CacheValidationStatus OnValidateRequest(HttpRequestCacheValidator ctx)
            {
                ...
                */
                ...
                if (
                    ctx.Request.Headers[HttpKnownHeaderNames.IfModifiedSince] != null
            ...
            public static CacheFreshnessStatus ComputeFreshness(HttpRequestCacheValidator ctx)
            {
                ...
                if (date != DateTime.MinValue)
                {
                    ...
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_age1_date_header,
                    ...
                    if (Logging.On)
                    {
                        if (ctx.CacheAge != TimeSpan.MinValue)
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_age1_last_synchronized_age_header,
                            ...
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_age1_last_synchronized,
                                    ((int)age.TotalSeconds).ToString(
                                        NumberFormatInfo.InvariantInfo
                                    ),
                                    ctx.CacheEntry.LastSynchronizedUtc.ToString(
                    ...
                    if (!ctx.CacheEntry.IsPrivateEntry && ctx.CacheCacheControl.SMaxAge != -1)
                    {
                        ...
                        if (Logging.On)
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_max_age_cache_s_max_age,
                    ...
                    if (ctx.CacheCacheControl.MaxAge != -1)
                    {
                        ...
                        if (Logging.On)
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_max_age_cache_max_age,
                                    ((int)ctx.CacheMaxAge.TotalSeconds).ToString(
                                        NumberFormatInfo.InvariantInfo
                                    )
                                )
                ...
                */
                ...
                if (
                    ...
                    && ctx.CacheAge != TimeSpan.MinValue
                )
                {
                    ...
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_max_age_expires_date,
                                ((int)((expiresDate - date).TotalSeconds)).ToString(
                                    NumberFormatInfo.InvariantInfo
                                ),
                                expiresDate.ToString("r", CultureInfo.InvariantCulture)
                            )
                        );
                    if (ctx.CacheAge < ctx.CacheMaxAge)
                    {
                    ...
                    if (Logging.On)
                        Logging.PrintInfo(
                            ...
                            SR.GetString(
                                ...
                                lastModifiedDate.ToString("r", CultureInfo.InvariantCulture)
                            )
                        );
                ...
                if (Logging.On)
                    Logging.PrintWarning(
                        ...
                        SR.GetString(
                            ...
                            ((int)(ctx.UnspecifiedMaxAge.TotalSeconds)).ToString(
                                NumberFormatInfo.InvariantInfo
                            )
                        )
                    );
            ...
            */
            ...
            )
            {
                    ...
                    if (!ctx.CacheEntry.IsPrivateEntry)
                    {
                        ...
                        if (Logging.On)
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_resp_cache_control_is_private_plus_headers
                                )
                            );
                ...
                */
                ...
                {
                    if (
                        ctx.ResponseLastModified == DateTime.MinValue
                        && ctx.Response.Headers.ETag == null
                    )
                    {
                        if (Logging.On)
                            Logging.PrintWarning(
                ...
                if (ctx.ResponseCacheControl.SMaxAge != -1 || ctx.ResponseCacheControl.MaxAge != -1)
                {
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_resp_allows_caching,
                ...
                if (ctx.RequestMethod == HttpMethod.Post && resp.Headers.Expires == null)
                {
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_post_resp_without_cache_control_or_expires
                            )
                        );
                    return TriState.Unknown;
                }
                ...
                )
                {
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_valid_based_on_status_code,
                                (int)resp.StatusCode
                            )
                        );
                    return TriState.Valid;
                ...
                if (result != TriState.Valid)
                {
                    ...
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_resp_no_cache_control,
                                (int)resp.StatusCode
                            )
                        );
                }
                ...
                if (ctx.Policy.Level == HttpRequestCacheLevel.Default)
                {
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_age,
                                (
                                    ctx.CacheAge != TimeSpan.MinValue
                                        ? ((int)ctx.CacheAge.TotalSeconds).ToString(
                                            NumberFormatInfo.InvariantInfo
                                        )
                                        : SR.GetString(SR.net_log_unknown)
                                ),
                                (
                                    ctx.CacheMaxAge != TimeSpan.MinValue
                                        ? ((int)ctx.CacheMaxAge.TotalSeconds).ToString(
                                            NumberFormatInfo.InvariantInfo
                                        )
                                        : SR.GetString(SR.net_log_unknown)
                    ...
                    if (ctx.Policy.MinFresh > TimeSpan.Zero)
                    {
                        if (Logging.On)
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_policy_min_fresh,
                                    ((int)ctx.Policy.MinFresh.TotalSeconds).ToString(
                                        NumberFormatInfo.InvariantInfo
                                    )
                                )
                            );
                    ...
                    if (ctx.Policy.MaxAge != TimeSpan.MaxValue)
                    {
                        if (Logging.On)
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_policy_max_age,
                                    ((int)ctx.Policy.MaxAge.TotalSeconds).ToString(
                                        NumberFormatInfo.InvariantInfo
                                    )
                                )
                            );
                    ...
                    if (ctx.Policy.InternalCacheSyncDateUtc != DateTime.MinValue)
                    {
                        ...
                        if (
                            ctx.CacheEntry.LastSynchronizedUtc < ctx.Policy.InternalCacheSyncDateUtc
                        )
                        {
                            return false;
                        }
                    ...
                    if (ctx.Policy.MaxStale > TimeSpan.Zero)
                    {
                        if (Logging.On)
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_policy_max_stale,
            ...
            */
            internal static TriState ValidateCacheBySpecialCases(HttpRequestCacheValidator ctx)
            {
                ...
                */
                if (
                    ctx.CacheCacheControl.MustRevalidate
                    ...
                    )
                    {
                        if (Logging.On)
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_cached_auth_header_no_control_directive
                                )
                            );
                        return TriState.Invalid;
                    }
            ...
            )
            {
                ...
                if (
                    (ctx.CacheStream == Stream.Null || (int)ctx.CacheStatusCode == 0)
                    && resp.StatusCode == HttpStatusCode.NotModified
                )
                {
                    if (Logging.On)
                ...
                if (ctx.RequestMethod == HttpMethod.Head)
                {
                    ...
                    */
                    ...
                    if (
                        ctx.ResponseEntityLength != -1
                        && ctx.ResponseEntityLength != ctx.CacheEntityLength
                    )
                    {
                        if (Logging.On)
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_head_resp_has_different_content_length
                                )
                            );
                        invalidate = true;
                    }
                    if (
                        resp.Headers[HttpKnownHeaderNames.ContentMD5]
                        != ctx.CacheHeaders[HttpKnownHeaderNames.ContentMD5]
                    )
                    {
                        if (Logging.On)
                            Logging.PrintInfo(
                    ...
                    )
                    {
                        if (Logging.On)
                            Logging.PrintWarning(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_304_head_resp_has_different_last_modified
                                )
                            );
                        invalidate = true;
                    }
                    ...
                    */
                    ...
                    if (
                        ...
                        || (
                            ...
                            && (
                                ctx.Response.Headers.LastModified != null
                                || ctx.Response.Headers.ETag == null
                            )
                        )
                    )
                    ...
                    if (ctx.CacheEntry.StreamSize != ctx.ResponseRangeStart)
                    {
                        if (Logging.On)
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_206_resp_starting_position_not_adjusted
                                )
                            );
                        return CacheValidationStatus.DoNotTakeFromCache;
                    }
                ...
                */
                ...
                if (resp.StatusCode == HttpStatusCode.NotModified)
                {
                    ...
                    if (
                        (ctx.CacheExpires != ctx.ResponseExpires)
                        || (ctx.CacheLastModified != ctx.ResponseLastModified)
                        || (ctx.CacheDate != ctx.ResponseDate)
                        || (ctx.ResponseCacheControl.IsNotEmpty)
                        || (
                            (location = cc[HttpKnownHeaderNames.ContentLocation]) != null
                            && location != ctx.CacheHeaders[HttpKnownHeaderNames.ContentLocation]
                        )
            ...
            */
            ...
            )
            {
                ...
                */
                ...
                if (
                    ctx.CacheEntityLength != ctx.CacheEntry.StreamSize
                ...
                if (
                    ctx.Policy.Level == HttpRequestCacheLevel.Default
                    || ctx.Policy.Level == HttpRequestCacheLevel.Revalidate
                )
                {
            ...
            */
            internal static TriState ValidateCacheByVaryHeader(HttpRequestCacheValidator ctx)
            {
                ...
                HttpRequestCacheValidator.ParseHeaderValues(
                    cacheVary,
                ...
                */
                ...
                for (int i = 0; i < varyValues.Count; ++i)
                {
                    ...
                    if (requestFields.Count != cacheFields.Count)
                    {
                        if (Logging.On)
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_vary_header_mismatched_count,
                                    (string)varyValues[i]
                                )
                            );
                        return TriState.Invalid;
                    ...
                    {
                        if (
                            !AsciiLettersNoCaseEqual(
                                (string)cacheFields[j],
                                (string)requestFields[j]
                            )
                        )
                        {
                            if (Logging.On)
                                Logging.PrintInfo(
                                    Logging.RequestCache,
                                    SR.GetString(
                                        SR.net_log_cache_vary_header_mismatched_field,
                                        (string)varyValues[i],
                                        (string)cacheFields[j],
                                        (string)requestFields[j]
                                    )
            ...
            public static CacheValidationStatus TryResponseFromCache(HttpRequestCacheValidator ctx)
            {
                ...
                if (isRange == TriState.Invalid)
                {
                    ...
                    if (
                        start >= ctx.CacheEntry.StreamSize
                    ...
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_range_in_cache,
                                ctx.CacheHeaders[HttpKnownHeaderNames.ContentRange]
                            )
                        );
                ...
                if (ctx.CacheEntry.StreamSize >= Int32.MaxValue)
                {
                    if (Logging.On)
                        Logging.PrintWarning(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_entry_size_too_big,
                                ctx.CacheEntry.StreamSize
                            )
                        );
                    return CacheValidationStatus.DoNotTakeFromCache;
                ...
                if ((ranges = ctx.Request.Headers[HttpKnownHeaderNames.Range]) != null)
                {
                    ...
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_range_request_range,
                                ctx.Request.Headers[HttpKnownHeaderNames.Range]
                            )
                        );
                    return TriState.Invalid;
                ...
                { //The cache may contain a partial response
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_could_be_partial,
                                ctx.CacheEntry.StreamSize,
                ...
                if (str != null)
                {
                    ...
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_condition_if_none_match,
                                ctx.Request.Headers[HttpKnownHeaderNames.IfNoneMatch]
                            )
                        );
                }
                ...
                if (ctx.CacheEntry.LastModifiedUtc != DateTime.MinValue)
                {
                    ...
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_condition_if_modified_since,
                                ctx.Request.Headers[HttpKnownHeaderNames.IfModifiedSince]
                            )
                        );
                }
            ...
            */
                ...
                if (str != null)
                {
                    ...
                    if (Logging.On)
                        Logging.PrintInfo(
                            Logging.RequestCache,
                            SR.GetString(
                                SR.net_log_cache_condition_if_range,
                                ctx.Request.Headers[HttpKnownHeaderNames.IfRange]
                            )
                        );
                    return true;
                }
                    ...
                    if (ctx.CacheHttpVersion.Major == 1 && ctx.CacheHttpVersion.Minor == 0)
                    {
                        ...
                        if (Logging.On)
                            Logging.PrintWarning(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_conditional_range_not_implemented_on_http_10
                                )
                            );
                        return false;
                        /*
            ...
            )
            {
                ...
                string ranges =
                    ...
                    + rangeStart
                    + '-'
                    + (rangeStart + ctx.CacheStreamLength - 1)
                    + '/'
                    + (
                        ctx.CacheEntityLength <= 0
                            ? "*"
               ...
               */
                    ...
                    if (varyValues.Count != 0 && ((string)(varyValues[0]))[0] != '*')
                    {
                        ...
                        if (Logging.On)
                            Logging.PrintInfo(
                                Logging.RequestCache,
                                SR.GetString(
                                    SR.net_log_cache_saving_request_headers,
                                    resp.Headers[HttpKnownHeaderNames.Vary]
                                )
                            );
                        if (ctx.SystemMeta == null)
                        {
                            ctx.SystemMeta = new NameValueCollection(
                                varyValues.Count + 1,
                ...
                */
                ...
                for (int i = 0; i < ctx.Response.Headers.Count; ++i)
                {
                    ...
                    if (
                        AsciiLettersNoCaseEqual(key, HttpKnownHeaderNames.Connection)
            ...
            //   false     = parsing error
            public static bool GetBytesRange(
                string ranges,
                ref long start,
                ref long end,
                ref long total,
                        ...
                        if (ranges[idx++] != ' ')
                        {
                            if (Logging.On)
                                Logging.PrintError(
                                    Logging.RequestCache,
                                    SR.GetString(
                                        SR.net_log_cache_multiple_complex_range_not_implemented
                                    )
                                );
                            return false;
                        }
