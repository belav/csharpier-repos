...
namespace System.Net
{
    ...
    internal class NegotiateClient : ISessionAuthenticationModule
    {
        ...
        {
            GlobalLog.Print(
                "NegotiateClient::Authenticate() challenge:["
        ...
        {
            GlobalLog.Print(
                "NegotiateClient::DoAuthenticate() challenge:["
                    + ValidationHelper.ToString(challenge)
                    + "] webRequest#"
                    + ValidationHelper.HashString(webRequest)
                    + " credentials#"
            ...
            GlobalLog.Assert(
                credentials != null,
                "NegotiateClient::DoAuthenticate()|credentials == null"
            );
            if (credentials == null)
            {
                return null;
            ...
            GlobalLog.Assert(
                httpWebRequest != null,
                "NegotiateClient::DoAuthenticate()|httpWebRequest == null"
            );
            GlobalLog.Assert(
                httpWebRequest.ChallengedUri != null,
                "NegotiateClient::DoAuthenticate()|httpWebRequest.ChallengedUri == null"
            );
            ...
            if (!preAuthenticate)
            {
                ...
                int blobBegin =
                    index + (useNego2 ? nego2Signature.Length : negotiateSignature.Length);
                ...
                GlobalLog.Print(
                    "NegotiateClient::DoAuthenticate() key:"
            ...
            if (authSession == null)
            {
                ...
                GlobalLog.Print(
                    "NegotiateClient::DoAuthenticate() GetCredential() returns:"
                ...
                if (
                    NC == null
                    || (
                        !(NC is SystemNetworkCredential)
                        && (username = NC.InternalGetUserName()).Length == 0
                    )
                )
                {
                    return null;
                ...
                GlobalLog.Print(
                    "NegotiateClient::DoAuthenticate() setting SecurityContext for:"
                        + ValidationHelper.HashString(httpWebRequest.CurrentAuthenticationState)
                        + " to authSession:"
                        + ValidationHelper.HashString(authSession)
                );
            ...
            httpWebRequest.NtlmKeepAlive =
                incoming == null && authSession.IsValidContext && !authSession.IsKerberos;
            ...
            return AuthenticationManager.GetGroupAuthorization(
                this,
                (useNego2 ? nego2Header : negotiateHeader) + " " + clientResponse,
        ...
        {
            GlobalLog.Print(
                "NegotiateClient::PreAuthenticate() webRequest#"
                    + ValidationHelper.HashString(webRequest)
                    + " credentials#"
                    + ValidationHelper.HashString(credentials)
                    + " calling DoAuthenticate()"
        ...
        public bool Update(string challenge, WebRequest webRequest)
        {
            ...
            GlobalLog.Assert(
                httpWebRequest != null,
                "NegotiateClient::Update()|httpWebRequest == null"
            );
            GlobalLog.Assert(
                httpWebRequest.ChallengedUri != null,
                "NegotiateClient::Update()|httpWebRequest.ChallengedUri == null"
            );
            ...
            GlobalLog.Print(
                "NegotiateClient::Update() authSession.IsCompleted:"
            ...
            {
                GlobalLog.Print(
                    "NegotiateClient::Update() still handshaking (based on status code) returning false"
                );
                return false;
            }
            ...
            if (index >= 0)
            {
                ...
                GlobalLog.Print(
                    "NegotiateClient::Update() this must be a final incoming blob:["
                ...
                GlobalLog.Print(
                    "NegotiateClient::Update() GetOutgoingBlob() returns clientResponse:["
        ...
        public void ClearSession(WebRequest webRequest)
        {
            ...
            GlobalLog.Assert(
                httpWebRequest != null,
                "NegotiateClient::ClearSession()|httpWebRequest == null"
            );
        ...
        private static int GetSignatureIndex(string challenge, out bool useNego2)
        {
            ...
            {
                useNego2 = false;
                index = AuthenticationManager.FindSubstringNotInQuotes(
                    challenge,
                    negotiateSignature
                );
            }
