...
namespace System.Net
{
    ...
    public class ServicePoint
    {
        ...
        {
            get { return m_IsTrustedHost; }
        }

        public BindIPEndPoint BindIPEndPointDelegate
        {
            get { return m_BindIPEndPointDelegate; }
            set
            {
#if MONO_FEATURE_CAS
        ...
        internal ServicePoint(
            Uri address,
            TimerThread.Queue defaultIdlingQueue,
        ...
        )
        {
            ...
            if (Logging.On)
                Logging.Enter(
                    ...
                    this,
                    "ServicePoint",
                    address.DnsSafeHost + ":" + address.Port
                );
            ...
            m_IdleConnectionGroupTimeoutDelegate = new TimerThread.Callback(
                IdleConnectionGroupTimeoutCallback
            );
        }
        ...
        private ConnectionGroup FindConnectionGroup(string connName, bool dontCreate)
        {
            ...
            GlobalLog.Print(
                "ServicePoint#"
                    + ValidationHelper.HashString(this)
                    + "::FindConnectionGroup() lookupStr:["
                    + ValidationHelper.ToString(connName)
                    + "]"
            );
            ...
            if (entry == null && !dontCreate)
            {
                ...
                GlobalLog.Print(
                    "ServicePoint#"
                        + ValidationHelper.HashString(this)
                        + "::FindConnectionGroup() adding ConnectionGroup lookupStr:["
                        + lookupStr
                        + "]"
                );
            ...
            else
            {
                GlobalLog.Print(
                    "ServicePoint#"
                        + ValidationHelper.HashString(this)
                        + "::FindConnectionGroup() using existing ConnectionGroup"
                );
            }
            GlobalLog.Print(
                "ServicePoint#"
                    + ValidationHelper.HashString(this)
                    + "::FindConnectionGroup() returning ConnectionGroup:"
                    + ValidationHelper.ToString(entry)
                    + (entry != null ? " ConnLimit:" + entry.ConnectionLimit.ToString() : "")
            );
            return entry;
        }
        ...
        internal Socket GetConnection(
            ...
            ref Socket abortSocket,
            ref Socket abortSocket6
        )
        {
            Socket socket = null;
            Socket socket6 = null;
            ...
            if (ws != WebExceptionStatus.Success)
            {
                throw new WebException(
                    ...
                    ws == WebExceptionStatus.ProxyNameResolutionFailure
                    || ws == WebExceptionStatus.NameResolutionFailure
                        ? Host
                        : null,
                    innerException,
                    ws,
        ...
        ///    </para>
        /// </devdoc>
        private void CompleteGetConnection(
            Socket socket,
            Socket socket6,
            Socket finalSocket,
            IPAddress address
        )
        {
            ...
            if (m_UseTcpKeepAlive)
            {
                ...
                // do WSAIoctl call
                finalSocket.IOControl(IOControlCode.KeepAliveValues, input, null);
            }
        ...
        internal void SubmitRequest(HttpWebRequest request, string connName)
        {
            ...
            lock (this)
            {
                GlobalLog.Print(
                    "ServicePoint#"
                        + ValidationHelper.HashString(this)
                        + "::SubmitRequest() Finding ConnectionGroup:["
                        + connName
                        + "]"
                );
                ...
                GlobalLog.Assert(
                    connGroup != null,
                    "ServicePoint#{0}::SubmitRequest()|connGroup == null",
                    ValidationHelper.HashString(this)
                );
            }
            ...
            do
            {
                ...
                GlobalLog.Print(
                    "ServicePoint#"
                        + ValidationHelper.HashString(this)
                        + "::SubmitRequest() Using Connection#"
                        + ValidationHelper.HashString(connToUse)
                );
        ...
        {
            get { return m_ConnectionLeaseTimeout; }
            set
            {
            ...
            get
            {
                if (m_HostMode)
                {
                    throw new NotSupportedException(
                        SR.GetString(SR.net_servicePointAddressNotSupportedInHostMode)
                    );
                }
        ...
        internal Uri InternalAddress
        {
            get
            {
                GlobalLog.Assert(
                    !m_HostMode,
                    "ServicePoint#{0}::InternalAddress|Can't be used in Host Mode.",
                    ValidationHelper.HashString(this)
                );
                return m_Address;
            }
        ...
        {
            get { return m_IdlingQueue.Duration; }
            set
            {
                if (!ValidationHelper.ValidateRange(value, Timeout.Infinite, Int32.MaxValue))
                {
                ...
                lock (this)
                {
                    ...
                    if (m_ExpiringTimer == null || m_ExpiringTimer.Cancel())
                    {
                        ...
                        if (m_ExpiringTimer != null)
                        {
                            ...
                            int elapsed =
                                elapsedDouble >= (double)Int32.MaxValue
                                    ? Int32.MaxValue
                                    : (int)elapsedDouble;
                            int timeLeft =
                                value == Timeout.Infinite ? Timeout.Infinite
                                : elapsed >= value ? 0
                                : value - elapsed;
                            m_ExpiringTimer = TimerThread
                                .CreateQueue(timeLeft)
                                .CreateTimer(
                                    ServicePointManager.IdleServicePointTimeoutDelegate,
                                    this
                                );
        ...
        {
            get { return m_UseNagleAlgorithm; }
            set { m_UseNagleAlgorithm = value; }
        }
        ...
        public virtual Version ProtocolVersion
        {
            ...
            {
                return (
                    m_HttpBehaviour > HttpBehaviour.HTTP10
                    || m_HttpBehaviour == HttpBehaviour.Unknown
                )
        ...
        */
        ...
        public bool CloseConnectionGroup(string connectionGroupName)
        {
            GlobalLog.Enter(
                "ServicePoint#"
                    + ValidationHelper.HashString(this)
                    + "::CloseConnectionGroup() lookupStr:["
                    + connectionGroupName
                    + "]"
            );
            if (
                ReleaseConnectionGroup(
                    HttpWebRequest
                        .GenerateConnectionGroup(connectionGroupName, false, false)
                        .ToString()
                )
                || ReleaseConnectionGroup(
                    HttpWebRequest
                        .GenerateConnectionGroup(connectionGroupName, true, false)
                        .ToString()
                )
            ...
            GlobalLog.Leave(
                "ServicePoint#" + ValidationHelper.HashString(this) + "::CloseConnectionGroup()",
                "false"
            );
            return false;
        }
        ...
        internal void CloseConnectionGroupInternal(string connectionGroupName)
        {
            ...
            // HTTP.
            string connectionGroupPrefixSafe = HttpWebRequest
                .GenerateConnectionGroup(connectionGroupName, false, true)
                .ToString();
            string connectionGroupPrefixUnsafe = HttpWebRequest
                .GenerateConnectionGroup(connectionGroupName, true, true)
                .ToString();
            ...
            lock (this)
            {
                ...
                foreach (var item in m_ConnectionGroupList.Keys)
                {
                    ...
                    if (
                        current.StartsWith(connectionGroupPrefixSafe, StringComparison.Ordinal)
        ...
        public int ConnectionLimit
        {
            get
            {
                ...
                )
                {
                    ...
                    lock (this)
                    {
                        ...
                        )
                        {
                            ...
                            {
                                m_HostLoopbackGuess = IsAddressListLoopback(
                                    new IPAddress[] { addr }
                                )
                                    ? TriState.True
                                    : TriState.False;
                            ...
                            {
                                m_HostLoopbackGuess = NclUtilities.GuessWhetherHostIsLoopback(
                                    m_Host
                                )
                                    ? TriState.True
                                    : TriState.False;
                ...
                return
                    ...
                    || (
                        m_IPAddressInfoList == null
                            ? m_HostLoopbackGuess != TriState.True
                            : !m_IPAddressesAreLoopback
                    )
                    ? m_ConnectionLimit
        ...
        {
            set { m_Understands100Continue = value; }
            get { return m_Understands100Continue; }
        }
        ...
        {
            get { return m_ProxyServicePoint; }
        }
        ...
        {
            GlobalLog.Enter(
                "ServicePoint#" + ValidationHelper.HashString(this) + "::IncrementConnection()",
                m_CurrentConnections.ToString()
            );
            ...
            lock (this)
            {
                ...
                if (m_CurrentConnections == 1)
                {
                    GlobalLog.Assert(
                        ...
                        "ServicePoint#{0}::IncrementConnection|First connection active, but ServicePoint wasn't idle.",
                        ValidationHelper.HashString(this)
                    );
        ...
        internal void DecrementConnection()
        {
            ...
            GlobalLog.ThreadContract(
                ...
                "ServicePoint#" + ValidationHelper.HashString(this) + "::DecrementConnection"
            );
            GlobalLog.Enter(
                "ServicePoint#" + ValidationHelper.HashString(this) + "::DecrementConnection()",
                m_CurrentConnections.ToString()
            ...
            lock (this)
            {
                ...
                if (m_CurrentConnections == 0)
                {
                    GlobalLog.Assert(
                        m_ExpiringTimer == null,
                        "ServicePoint#{0}::DecrementConnection|Expiring timer set on non-idle ServicePoint.",
                        ValidationHelper.HashString(this)
                    );
                    m_IdleSince = DateTime.Now;
                    m_ExpiringTimer = m_IdlingQueue.CreateTimer(
...
#if !FEATURE_PAL
        internal RemoteCertValidationCallback SetupHandshakeDoneProcedure(
        ...
        private class HandshakeDoneProcedure
        {
            ...
            {
                HandshakeDoneProcedure adapter = new HandshakeDoneProcedure(
                    serviePoint,
                    secureStream,
                    request
                );
            ...
            private bool CertValidationCallback(
                string hostName,
            ...
            )
            {
                ...
                if (
                    ServicePointManager.GetLegacyCertificatePolicy() != null
                    && (m_Request is WebRequest)
                )
                {
                    useDefault = false;
                    ...
                    {
                        if (
                            !ServicePointManager.CertPolicyValidationCallback.UsesDefault
        ...
        internal void ReleaseAllConnectionGroups()
        {
            ...
            GlobalLog.ThreadContract(
                ThreadKinds.Unknown,
                ThreadKinds.SafeSources | ThreadKinds.Timer,
                "ServicePoint#" + ValidationHelper.HashString(this) + "::ReleaseAllConnectionGroups"
            );
        ...
        {
            IPEndPoint clonedRemoteIPEndPoint = new IPEndPoint(
            ...
            {
                IPEndPoint localIPEndPoint = BindIPEndPointDelegate(
                    this,
                    clonedRemoteIPEndPoint,
                    retryCount
                );
        ...
        private void SetUnicastReusePortForSocket(Socket socket)
        {
            ...
            if (
                ServicePointManager.ReusePortSupported.HasValue
                && !ServicePointManager.ReusePortSupported.Value
            )
            {
                ...
                try
                {
                    ...
                    if (Logging.On)
                    {
                        Logging.PrintInfo(
                            ...
                            this,
                            SR.GetString(
                                SR.net_log_set_socketoption_reuseport,
                                "Socket",
                                socket.GetHashCode()
                            )
                        );
                ...
                catch (SocketException)
                {
                    ...
                    if (Logging.On)
                    {
                        Logging.PrintInfo(
                            ...
                            this,
                            SR.GetString(
                                SR.net_log_set_socketoption_reuseport_not_supported,
                                "Socket",
                                socket.GetHashCode()
                            )
                        );
                ...
                catch (Exception ex)
                {
                    ...
                    if (Logging.On)
                    {
                        Logging.PrintInfo(
                            Logging.Web,
                            this,
                            SR.GetString(SR.net_log_unexpected_exception, ex.Message)
                        );
                    }
                }
            }
        ...
        /// </summary>
        private WebExceptionStatus ConnectSocketInternal(
            bool connectFailure,
            Socket s4,
            Socket s6,
            ref Socket socket,
            ref IPAddress address,
            ConnectSocketState state,
            ...
            for (int unsuccessfulAttempts = 0; unsuccessfulAttempts < 2; unsuccessfulAttempts++)
            {
                ...
                else
                {
                    GlobalLog.Print(
                        "ServicePoint#"
                            + ValidationHelper.HashString(this)
                            + "::ConnectSocketInternal() resuming previous state"
                    );
                        ...
                        if (state != null)
                        {
                            ...
                            else
                            {
                                GlobalLog.Print(
                                    "ServicePoint#"
                                        + ValidationHelper.HashString(this)
                                        + "::ConnectSocketInternal() calling BeginConnect() to:"
                                ...
                                attemptSocket.UnsafeBeginConnect(
                                    remoteIPEndPoint,
                                    m_ConnectCallbackDelegate,
                                    state
                                );
        ...
        /// </summary>
        private WebExceptionStatus ConnectSocket(
            Socket s4,
            Socket s6,
            ref Socket socket,
            ref IPAddress address,
        ...
        private void UpdateCurrentIndex(IPAddress[] addresses, int currentIndex)
        {
            if (
                ...
                && (m_CurrentAddressInfoIndex != currentIndex || !m_ConnectedSinceDns)
            )
            {
                lock (this)
                {
        ...
        private bool HasTimedOut
        {
            get
            {
                ...
                return dnsRefreshTimeout != Timeout.Infinite
                    && (m_LastDnsResolve + new TimeSpan(0, 0, 0, 0, dnsRefreshTimeout))
                        < DateTime.UtcNow;
            }
        }
        ...
        private IPAddress[] GetIPAddressInfoList(out int currentIndex, IPAddress[] addresses)
        {
            ...
            if (needDnsResolution)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    GlobalLog.Print(
                        "IPHostInformation#"
                            + ValidationHelper.HashString(this)
                            + "::GetIPAddressInfoList() Dns.InternalResolveFast() failed with exception:\r\n"
                            + exception.ToString()
                    );
                }
            }
            ...
            lock (this)
            {
                if (needDnsResolution)
                {
                    ...
                    if (!dnsResolutionFailed)
                    {
                        ...
                        else
                        {
                            GlobalLog.Print(
                                "IPHostInformation#"
                                    + ValidationHelper.HashString(this)
                                    + "::GetIPAddressInfoList() Dns.InternalResolveFast() failed with null"
                            );
                        }
                    }
                    else
                    {
                        GlobalLog.Print(
                            "IPHostInformation#"
                                + ValidationHelper.HashString(this)
                                + "::GetIPAddressInfoList() Dns.InternalResolveFast() had thrown an exception"
                        );
                    }
                }
                ...
                if (m_IPAddressInfoList != null && m_IPAddressInfoList.Length > 0)
                {
                    GlobalLog.Print(
                        "IPHostInformation#"
                            + ValidationHelper.HashString(this)
                            + "::GetIPAddressInfoList() m_IPAddressInfoList = "
                            + m_IPAddressInfoList
                    );
            ...
            GlobalLog.Print(
                "IPHostInformation#"
                    + ValidationHelper.HashString(this)
                    + "::GetIPAddressInfoList() GetIPAddressInfoList returning null"
            );
            return null;
        }
