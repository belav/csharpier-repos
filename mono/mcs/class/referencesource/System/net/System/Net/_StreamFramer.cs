...
namespace System.Net
{
    ...
    internal class StreamFramer
    {
        ...
        private FrameHeader m_ReadVerifier = new FrameHeader(
...
*/
        ...
        public IAsyncResult BeginReadMessage(AsyncCallback asyncCallback, object stateObject)
        {
            ...
            workerResult = new WorkerAsyncResult(
                this,
                stateObject,
                asyncCallback,
                m_ReadHeaderBuffer,
            ...
            IAsyncResult result = Transport.BeginRead(
                m_ReadHeaderBuffer,
                0,
                m_ReadHeaderBuffer.Length,
                m_ReadFrameCallback,
        ...
        {
            GlobalLog.Assert(
                transportResult.AsyncState is WorkerAsyncResult,
                "StreamFramer::ReadFrameCallback|The state expected to be WorkerAsyncResult, received:{0}.",
                transportResult.GetType().FullName
            );
            ...
            catch (Exception e)
            {
                ...
                if (!(e is IOException))
                {
                    e = new System.IO.IOException(
                        SR.GetString(SR.net_io_readfailure, e.Message),
                        e
                    );
                }
        ...
        private void ReadFrameComplete(IAsyncResult transportResult)
        {
            do
            {
                GlobalLog.Assert(
                    transportResult.AsyncState is WorkerAsyncResult,
                    "StreamFramer::ReadFrameComplete|The state expected to be WorkerAsyncResult, received:{0}.",
                    transportResult.GetType().FullName
                );
                WorkerAsyncResult workerResult = (WorkerAsyncResult)transportResult.AsyncState;
                ...
                GlobalLog.Assert(
                    ...
                    "StreamFramer::ReadFrameCallback|WRONG: offset - end = {0}",
                    workerResult.Offset - workerResult.End
                );
                    ...
                    if (!workerResult.HeaderDone)
                    {
                        ...
                        if (payloadSize < 0)
                        {
                            ...
                            workerResult.InvokeCallback(
                                new System.IO.IOException(SR.GetString(SR.net_frame_read_size))
                            );
                        }
                        if (payloadSize == 0)
                        {
                        ...
                        if (payloadSize > m_CurReadHeader.MaxMessageSize)
                        {
                            throw new InvalidOperationException(
                                SR.GetString(
                                    SR.net_frame_size,
                                    m_CurReadHeader.MaxMessageSize.ToString(
                ...
                transportResult = Transport.BeginRead(
                    workerResult.Buffer,
                    workerResult.Offset,
                    workerResult.End - workerResult.Offset,
                    m_ReadFrameCallback,
                    workerResult
                );
        ...
        public byte[] EndReadMessage(IAsyncResult asyncResult)
        {
            ...
            if (workerResult == null)
            {
                throw new ArgumentException(
                    SR.GetString(SR.net_io_async_result, typeof(WorkerAsyncResult).FullName),
                    "asyncResult"
                );
            }
        ...
        public void WriteMessage(byte[] message)
        {
            ...
            if (m_NetworkStream != null && message.Length != 0)
            {
                ...
                buffers[0] = new BufferOffsetSize(
                    m_WriteHeaderBuffer,
                    0,
                    m_WriteHeaderBuffer.Length,
                    false
                );
        ...
        //
        //
        public IAsyncResult BeginWriteMessage(
            byte[] message,
        ...
        )
        {
            ...
            // Prepare the second
            WorkerAsyncResult workerResult = new WorkerAsyncResult(
                this,
                stateObject,
                asyncCallback,
                message,
            ...
            // Charge the first
            IAsyncResult result = Transport.BeginWrite(
                m_WriteHeaderBuffer,
                0,
                m_WriteHeaderBuffer.Length,
                m_BeginWriteCallback,
                workerResult
            );
            if (result.CompletedSynchronously)
            {
        ...
        private void BeginWriteCallback(IAsyncResult transportResult)
        {
            GlobalLog.Assert(
                transportResult.AsyncState is WorkerAsyncResult,
                "StreamFramer::BeginWriteCallback|The state expected to be WorkerAsyncResult, received:{0}.",
                transportResult.AsyncState.GetType().FullName
            );
            if (transportResult.CompletedSynchronously)
            {
                return;
        ...
        private void BeginWriteComplete(IAsyncResult transportResult)
        {
            do
            {
                ...
                transportResult = Transport.BeginWrite(
                    workerResult.Buffer,
                    0,
                    workerResult.End,
                    m_BeginWriteCallback,
...
*/
        ...
        public void CopyFrom(byte[] bytes, int start, FrameHeader verifier)
        {
            ...
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.net_io_header_id, "MessageId", MessageId, verifier.MessageId)
                );
            }
            ...
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.net_io_header_id, "MajorV", MajorV, verifier.MajorV)
                );
            }
            ...
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.net_io_header_id, "MinorV", MinorV, verifier.MinorV)
                );
            }
        }
    }
