    ...
    internal class Logging
    {
        ...
        private static int GetMaxDumpSizeSetting(TraceSource traceSource)
        {
            ...
            if (traceSource.Attributes.ContainsKey(AttributeNameMaxSize))
            {
                try
                {
                    maxDumpSize = Int32.Parse(
                        traceSource.Attributes[AttributeNameMaxSize],
                ...
                catch (Exception exception)
                {
                    ...
                    traceSource.Attributes[AttributeNameMaxSize] = maxDumpSize.ToString(
                        NumberFormatInfo.InvariantInfo
                    );
                }
            }
        ...
        private static void InitializeLogging()
        {
            ...
            lock (InternalSyncObject)
            {
                if (!s_LoggingInitialized)
                {
                    ...
                    {
                        loggingEnabled = (
                            s_WebTraceSource.Switch.ShouldTrace(TraceEventType.Critical)
        ...
        [SuppressMessage(
            ...
            Justification = "Logging functions must work in partial trust mode"
        )]
...
#if MONO_FEATURE_LOGGING
            string logHeader =
                "[" + GetThreadId().ToString("d4", CultureInfo.InvariantCulture) + "] ";
        ...
        )
        {
            ...
            {
                paramObjectValue =
                    GetObjectName(paramObject) + "#" + ValidationHelper.HashString(paramObject);
            }
        ...
        )
        {
            ...
            string infoLine = SR.GetString(
                SR.net_log_exception,
                GetObjectLogHash(obj),
                method,
                e.Message
            );
        ...
        internal static void PrintInfo(TraceSource traceSource, object obj, string msg)
        {
            ...
            PrintLine(
                ...
                TraceEventType.Information,
                0,
                GetObjectName(obj) + "#" + ValidationHelper.HashString(obj) + " - " + msg
            );
        }
        ...
        {
            if (
                !ValidateSettings(traceSource, TraceEventType.Verbose)
        ...
        )
        {
            ...
            if (GetUseProtocolTextSetting(traceSource))
            {
                string output =
                    "<<"
                    + WebHeaderCollection.HeaderEncoding.GetString(buffer, offset, length)
                    + ">>";
