...
namespace System.Net
{
    ...
    public class HttpWebResponse : WebResponse, ISerializable
    {
        ...
        {
            get { return m_ConnectStream; }
            set { m_ConnectStream = value; }
        }
        ...
        {
            set { m_IsMutuallyAuthenticated = value; }
        }
        ...
        internal bool KeepAlive
        {
            get
            {
                ...
                if (ProtocolVersion == HttpVersion.Version10)
                {
                    ...
                    return (
                        connectionHeader != null && connectionHeader.IndexOf("keep-alive") >= 0
                    );
                }
        ...
        public override Stream GetResponseStream()
        {
            ...
            if (m_IsWebSocketResponse && m_StatusCode == HttpStatusCode.SwitchingProtocols) // HTTP 101
            {
                if (this.m_WebSocketConnectionStream == null)
                {
                    ...
                    GlobalLog.Assert(
                        connectStream != null,
                        "HttpWebResponse.m_ConnectStream should always be a ConnectStream in WebSocket cases."
                    );
                    GlobalLog.Assert(
                        connectStream.Connection != null,
                        "HttpWebResponse.m_ConnectStream.Connection should never be null in WebSocket cases."
                    );
                    this.m_WebSocketConnectionStream = new WebSocketConnectionStream(
                        connectStream,
        ...
        public override void Close()
        {
            ...
            if (!m_disposed)
            {
                ...
                try
                {
                    ...
                    {
                        icloseEx.CloseEx(
                            CloseExState.Normal /* | CloseExState.Silent */
                        );
                    }
                    else if (stream != null)
                    {
        ...
        )
        {
            ...
            if (decompressionMethod != DecompressionMethods.None)
            {
                ...
                {
                    if (
                        ((decompressionMethod & DecompressionMethods.GZip) != 0)
                        && contentEncoding.IndexOf(
                            HttpWebRequest.GZipHeader,
                            StringComparison.CurrentCulture
                        ) != -1
                    )
                    {
                        m_ConnectStream = new GZipWrapperStream(
                            m_ConnectStream,
                        ...
                        ((decompressionMethod & DecompressionMethods.Deflate) != 0)
                        && contentEncoding.IndexOf(
                            HttpWebRequest.DeflateHeader,
                            StringComparison.CurrentCulture
                        ) != -1
                    )
                    {
                        m_ConnectStream = new DeflateWrapperStream(
                            m_ConnectStream,
                            CompressionMode.Decompress
                        );
                        m_ContentLength = -1; // unknown on compressed streams
        ...
        {
            m_HttpResponseHeaders = (WebHeaderCollection)
                serializationInfo.GetValue("m_HttpResponseHeaders", typeof(WebHeaderCollection));
            m_Uri = (Uri)serializationInfo.GetValue("m_Uri", typeof(Uri));
#if !FEATURE_PAL
            m_Certificate = (X509Certificate)
                serializationInfo.GetValue("m_Certificate", typeof(X509Certificate));
#endif // !FEATURE_PAL
