...
namespace System.Net.Mail
{
    ...
    public class MailMessage : IDisposable
    {
        ...
        {
            get { return message.ReplyToList; }
        }
        ...
        {
            get { return deliveryStatusNotification; }
            set
            {
        ...
        {
            get { return (message.Subject != null ? message.Subject : String.Empty); }
        ...
        {
            get { return message.HeadersEncoding; }
            set { message.HeadersEncoding = value; }
        }

        public string Body
        {
            get { return (body != null ? body : String.Empty); }
            set
            {
                body = value;
        ...
        {
            get { return bodyTransferEncoding; }
            set { bodyTransferEncoding = value; }
        }
            ...
            if (AlternateViews.Count == 0 && Attachments.Count == 0)
            {
                if (!string.IsNullOrEmpty(body))
                {
                    bodyView = AlternateView.CreateAlternateViewFromString(
                        body,
                        bodyEncoding,
                        (isBodyHtml ? MediaTypeNames.Text.Html : null)
                    );
        ...
        internal string BuildDeliveryStatusNotificationString()
        {
            if (deliveryStatusNotification != DeliveryNotificationOptions.None)
            {
                ...
                if (
                    (((int)deliveryStatusNotification) & (int)DeliveryNotificationOptions.OnSuccess)
                    > 0
                )
                {
                    s.Append("SUCCESS");
                ...
                if (
                    (((int)deliveryStatusNotification) & (int)DeliveryNotificationOptions.OnFailure)
                    > 0
                )
                {
                    if (oneSet)
                ...
                if (
                    (((int)deliveryStatusNotification) & (int)DeliveryNotificationOptions.Delay) > 0
                )
                {
                    if (oneSet)
                    {
