namespace System.Net.NetworkInformation
{
    ...
    internal class SystemIPGlobalProperties : IPGlobalProperties
    {
        ...
        private List<SystemTcpConnectionInformation> GetAllTcpConnections()
        {
            ...
            uint result = 0;
            SafeLocalFree buffer = null;
            List<SystemTcpConnectionInformation> tcpConnections =
                new List<SystemTcpConnectionInformation>();
                ...
                while (result == IpHelperErrors.ErrorInsufficientBuffer)
                {
                    try
                    {
                        ...
                        if (result == IpHelperErrors.Success)
                        {
                            ...
                            MibTcpTable tcpTableInfo = (MibTcpTable)
                                Marshal.PtrToStructure(newPtr, typeof(MibTcpTable));

                            if (tcpTableInfo.numberOfEntries > 0)
                            {
                                //we need to skip over the tableinfo to get the inline rows
                                newPtr = (IntPtr)(
                                    (long)newPtr + Marshal.SizeOf(tcpTableInfo.numberOfEntries)
                                );

                                for (int i = 0; i < tcpTableInfo.numberOfEntries; i++)
                                {
                                    MibTcpRow tcpRow = (MibTcpRow)
                                        Marshal.PtrToStructure(newPtr, typeof(MibTcpRow));
                ...
                while (result == IpHelperErrors.ErrorInsufficientBuffer)
                {
                    try
                    {
                        ...
                        if (result == IpHelperErrors.Success)
                        {
                            ...
                            MibTcp6TableOwnerPid tcpTable6OwnerPid = (MibTcp6TableOwnerPid)
                                Marshal.PtrToStructure(newPtr, typeof(MibTcp6TableOwnerPid));

                            if (tcpTable6OwnerPid.numberOfEntries > 0)
                            {
                                // We need to skip over the tableinfo to get the inline rows
                                newPtr = (IntPtr)(
                                    (long)newPtr + Marshal.SizeOf(tcpTable6OwnerPid.numberOfEntries)
                                );

                                for (int i = 0; i < tcpTable6OwnerPid.numberOfEntries; i++)
                                {
                                    MibTcp6RowOwnerPid tcp6RowOwnerPid = (MibTcp6RowOwnerPid)
                                        Marshal.PtrToStructure(newPtr, typeof(MibTcp6RowOwnerPid));
                                    tcpConnections.Add(
                                        new SystemTcpConnectionInformation(tcp6RowOwnerPid)
                                    );

                                    // We increment the pointer to the next row
                                    newPtr = (IntPtr)(
                                        (long)newPtr + Marshal.SizeOf(tcp6RowOwnerPid)
                                    );
                                }
                            }
                        }
                ...
                while (result == IpHelperErrors.ErrorInsufficientBuffer)
                {
                    try
                    {
                        ...
                        if (result == IpHelperErrors.Success)
                        {
                            //the table info just gives us the number of rows.
                            IntPtr newPtr = buffer.DangerousGetHandle();
                            MibUdpTable udpTableInfo = (MibUdpTable)
                                Marshal.PtrToStructure(newPtr, typeof(MibUdpTable));

                            if (udpTableInfo.numberOfEntries > 0)
                            {
                                //we need to skip over the tableinfo to get the inline rows
                                newPtr = (IntPtr)(
                                    (long)newPtr + Marshal.SizeOf(udpTableInfo.numberOfEntries)
                                );
                                for (int i = 0; i < udpTableInfo.numberOfEntries; i++)
                                {
                                    MibUdpRow udpRow = (MibUdpRow)
                                        Marshal.PtrToStructure(newPtr, typeof(MibUdpRow));
                                    int localPort = udpRow.localPort1 << 8 | udpRow.localPort2;

                                    udpListeners.Add(
                                        new IPEndPoint(udpRow.localAddr, (int)localPort)
                                    );
                ...
                while (result == IpHelperErrors.ErrorInsufficientBuffer)
                {
                    try
                    {
                        ...
                        if (result == IpHelperErrors.Success)
                        {
                            // The table info just gives us the number of rows.
                            IntPtr newPtr = buffer.DangerousGetHandle();
                            MibUdp6TableOwnerPid udp6TableOwnerPid = (MibUdp6TableOwnerPid)
                                Marshal.PtrToStructure(newPtr, typeof(MibUdp6TableOwnerPid));

                            if (udp6TableOwnerPid.numberOfEntries > 0)
                            {
                                // We need to skip over the tableinfo to get the inline rows
                                newPtr = (IntPtr)(
                                    (long)newPtr + Marshal.SizeOf(udp6TableOwnerPid.numberOfEntries)
                                );
                                for (int i = 0; i < udp6TableOwnerPid.numberOfEntries; i++)
                                {
                                    MibUdp6RowOwnerPid udp6RowOwnerPid = (MibUdp6RowOwnerPid)
                                        Marshal.PtrToStructure(newPtr, typeof(MibUdp6RowOwnerPid));
                                    int localPort =
                                    ...
                                    // We increment the pointer to the next row
                                    newPtr = (IntPtr)(
                                        (long)newPtr + Marshal.SizeOf(udp6RowOwnerPid)
                                    );
                                }
                            }
                        }
        ...
        public override UnicastIPAddressInformationCollection GetUnicastAddresses()
        {
            ...
            {
                if (
                    !TeredoHelper.UnsafeNotifyStableUnicastIpAddressTable(
                        StableUnicastAddressTableCallback,
                        stable
                    )
                )
                {
        ...
        )
        {
            ...
            if (
                result == null
                || result.AsyncObject == null
                || result.AsyncObject.GetType() != typeof(SystemIPGlobalProperties)
            )
            {
            ...
            if (result.EndCalled)
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.net_io_invalidendcall, "EndGetStableUnicastAddresses")
                );
            }
    ...
    internal struct FixedInfo
    {
        ...
        {
            get { return info.scopeId; }
        }

        internal bool EnableRouting
        {
            get { return info.enableRouting; }
        }

        internal bool EnableProxy
        {
            get { return info.enableProxy; }
        }

        internal bool EnableDns
        {
            get { return info.enableDns; }
        }
    }
}
