...
namespace System
{
    ...
    internal class DomainNameHelper
    {
        ...
        internal unsafe static string IdnEquivalent(
            ...
            int end,
            ref bool allAscii,
            ref bool atLeastOneValidIdn
        )
        {
            string bidiStrippedHost = null;
            string idnEquivalent = IdnEquivalent(
                hostname,
                start,
                end,
                ref allAscii,
                    ...
                    do
                    {
                        ...
                        if (foundAce)
                        {
                            ...
                            try
                            {
                                ...
                                map.GetUnicode(
                                    new string(strippedHostPtr, curPos, newPos - curPos)
                                );
        ...
        )
        {
            ...
            string unescapedHostname = Uri.StripBidiControlCharacter(
                hostname,
                start,
                (end - start)
            );
            ...
            do
            {
                ...
                while (newPos < length)
                {
                    ...
                    if (!checkedAce)
                    {
                        ...
                        if (
                            (newPos + 3 < length)
                            && (c == 'x')
                            && IsIdnAce(unescapedHostname, newPos)
                        )
                            foundAce = true;
                    }
                    if (asciiLabel && (c > '\x7F'))
                ...
                else
                {
                    ...
                    if (foundAce)
                    {
                        ...
                        try
                        {
                            unicodeEqvlHost += map.GetUnicode(
                                unescapedHostname.Substring(curPos, newPos - curPos)
                            );
                            if (foundDot)
                                unicodeEqvlHost += ".";
                            aceValid = true;
                    ...
                    if (!aceValid)
                    {
                        ...
                        unicodeEqvlHost += unescapedHostname
                            .Substring(curPos, newPos - curPos)
                            .ToLowerInvariant();
                        if (foundDot)
                            unicodeEqvlHost += ".";
                    }
