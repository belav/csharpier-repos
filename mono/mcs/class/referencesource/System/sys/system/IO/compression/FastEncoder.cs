namespace System.IO.Compression
{
    ...
    internal class FastEncoder
    {
        ...
        {
            Debug.Assert(
                matchLen >= FastEncoderWindow.MinMatch && matchLen <= FastEncoderWindow.MaxMatch,
                "Illegal currentMatch length!"
            );
            Debug.WriteLineIf(
                CompressionTracingSwitch.Verbose,
                String.Format(CultureInfo.InvariantCulture, "Match: {0}:{1}", matchLen, matchPos),
                "Compression"
            );

            // Get the code information for a match code
            uint codeInfo = FastEncoderStatics.FastEncoderLiteralCodeInfo[
            ...
            codeInfo = FastEncoderStatics.FastEncoderDistanceCodeInfo[
                FastEncoderStatics.GetSlot(matchPos)
            ];
        ...
        internal static void WriteChar(byte b, OutputBuffer output)
        {
            Debug.WriteLineIf(
                CompressionTracingSwitch.Verbose,
                String.Format(CultureInfo.InvariantCulture, "Literal: {0}", b),
                "Compression"
            );
        ...
        //  FastEncoderDistanceTreeLength
        //
        static void WriteDeflatePreamble(OutputBuffer output)
        {
            //Debug.Assert( bitCount == 0, "bitCount must be zero before writing tree bit!");

            output.WriteBytes(
                FastEncoderStatics.FastEncoderTreeStructureData,
                0,
                FastEncoderStatics.FastEncoderTreeStructureData.Length
            );
            output.WriteBits(
                FastEncoderStatics.FastEncoderPostTreeBitCount,
