...
namespace System.IO.Compression
{
    ...
    internal class DeflaterZLib : IDeflater
    {
        ...
        {
            Contract.Ensures(
                Contract.Result<int>() >= 0 && Contract.Result<int>() <= outputBuffer.Length
            );
        ...
        private ZErrorCode ReadDeflateOutput(
            byte[] outputBuffer,
            ...
            switch (errC)
            {
                ...
                case ZErrorCode.MemError:
                    throw new ZLibException(
                        SR.GetString(SR.ZLibErrorNotEnoughMemory),
                        "deflateInit2_",
                        (int)errC,
                ...
                case ZErrorCode.VersionError:
                    throw new ZLibException(
                        SR.GetString(SR.ZLibErrorVersionMismatch),
                        "deflateInit2_",
                        (int)errC,
                        _zlibStream.GetErrorMessage()
                    );
                ...
                case ZErrorCode.StreamError:
                    throw new ZLibException(
                        SR.GetString(SR.ZLibErrorIncorrectInitParameters),
                        "deflateInit2_",
                        (int)errC,
                        _zlibStream.GetErrorMessage()
                    );
                ...
                default:
                    throw new ZLibException(
                        SR.GetString(SR.ZLibErrorUnexpected),
                        "deflateInit2_",
                        (int)errC,
                        _zlibStream.GetErrorMessage()
                    );
