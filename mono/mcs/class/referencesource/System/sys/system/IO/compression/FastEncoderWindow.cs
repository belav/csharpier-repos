    ...
    internal class FastEncoderWindow
    {
        ...
        { // uncompressed bytes
            get
            {
                Debug.Assert(
                    bufEnd - bufPos >= 0,
                    "Ending pointer can't be in front of starting pointer!"
                );
        ...
        { // Free space in the window
            get { return 2 * FastEncoderWindowSize - bufEnd; }
        }
        ...
        public void MoveWindows()
        {
            int i;
            Debug.Assert(
                bufPos == 2 * FastEncoderWindowSize,
        ...
        {
            Debug.Assert(
                bufPos >= FastEncoderWindowSize && bufPos < (2 * FastEncoderWindowSize),
        ...
        {
            Debug.Assert(
                bufPos >= 0 && bufPos < 2 * FastEncoderWindowSize,
                "Invalid Buffer position!"
            );
            ...
            while (search > earliest)
            {
                ...
                Debug.Assert(
                    prev[search & FastEncoderWindowMask] < search,
                    "we should always go backwards!"
                );
            ...
            Debug.Assert(
                bestMatch < MinMatch || matchPos < FastEncoderWindowSize,
        ...
        uint RecalculateHash(int position)
        {
            return (uint)(
                (
                    (window[position] << (2 * FastEncoderHashShift))
