...
[assembly: SuppressMessage(
    ...
    Scope = "member",
    Target = "System.Configuration.ClientSettingsStore+QuotaEnforcedStream.Dispose(System.Boolean):System.Void"
)]
...
namespace System.Configuration
{
    ...
    internal sealed class ClientSettingsStore
    {
        ...
        internal void WriteSettings(string sectionName, bool isRoaming, IDictionary newSettings)
        {
            ...
            if (section != null)
            {
                ...
                catch (ConfigurationErrorsException ex)
                {
                    ...
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.SettingsSaveFailed, ex.Message),
                        ex
                    );
                }
            }
            ...
            {
                throw new ConfigurationErrorsException(
                    SR.GetString(SR.SettingsSaveFailedNoSection)
                );
            }
        }
        ...
        private sealed class ClientSettingsConfigurationHost : DelegatingConfigHost
        {
            private const string ClientConfigurationHostTypeName =
                "System.Configuration.ClientConfigurationHost," + AssemblyRef.SystemConfiguration;
            ...
            )
            {
                ...
                {
                    stream = Host.OpenStreamForWrite(
                        streamName,
                        templateStreamName,
                        ref writeContext
                    );
        ...
        private sealed class QuotaEnforcedStream : Stream
        {
                ...
                set
                {
                    if (value < 0)
                    {
                        throw new ArgumentOutOfRangeException(
                            "value",
                            SR.GetString(SR.PositionOutOfRange)
                        );
                    }
            ...
            public override void Write(byte[] buffer, int offset, int count)
            {
                ...
                long oldLen = _originalStream.Length;
                long newLen = _originalStream.CanSeek
                    ? _originalStream.Position + (long)count
            ...
            public override void WriteByte(byte value)
            {
                ...
                long oldLen = _originalStream.Length;
                long newLen = _originalStream.CanSeek
                    ? _originalStream.Position + 1
                    : _originalStream.Length + 1;
                EnsureQuota(Math.Max(oldLen, newLen));
            ...
            {
                return _originalStream.BeginRead(
                    buffer,
                    offset,
                    numBytes,
                    userCallback,
            ...
            )
            {
                ...
                long oldLen = _originalStream.Length;
                long newLen = _originalStream.CanSeek
                    ? _originalStream.Position + (long)numBytes
                    : _originalStream.Length + (long)numBytes;
                EnsureQuota(Math.Max(oldLen, newLen));
                return _originalStream.BeginWrite(
                    buffer,
                    offset,
                    numBytes,
                    userCallback,
            ...
            {
                IsolatedStoragePermission storagePerm = new IsolatedStorageFilePermission(
