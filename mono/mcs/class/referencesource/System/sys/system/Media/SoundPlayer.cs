namespace System.Media
{
    ...
    public class SoundPlayer : Component, ISerializable
    {
        ...
        public SoundPlayer()
        {
            loadAsyncOperationCompleted = new SendOrPostCallback(LoadAsyncOperationCompleted);
        }
            ...
            set
            {
                if (value < 0)
                {
                    throw new ArgumentOutOfRangeException(
                        "LoadTimeout",
                        value,
                        SR.GetString(SR.SoundAPILoadTimeout)
                    );
                }
        ...
        public void LoadAsync()
        {
            ...
            if (uri != null && uri.IsFile)
            {
                ...
                if (!fi.Exists)
                {
                    throw new FileNotFoundException(
                        SR.GetString(SR.SoundAPIFileDoesNotExist),
                        this.soundLocation
                    );
                }
            ...
            if (
                copyThread != null && copyThread.ThreadState == System.Threading.ThreadState.Running
            )
            {
                return;
            }
            ...
            if (uri != null && uri.IsFile)
            {
                ...
                try
                {
                    ...
                    UnsafeNativeMethods.PlaySound(
                        localPath,
                        IntPtr.Zero,
                        NativeMethods.SND_NODEFAULT | flags
                    );
                }
                finally
                {
            ...
            else
            {
                ...
                try
                {
                    UnsafeNativeMethods.PlaySound(
                        streamData,
                        IntPtr.Zero,
                        NativeMethods.SND_MEMORY | NativeMethods.SND_NODEFAULT | flags
                    );
                }
                finally
                {
        ...
        {
            add { Events.AddHandler(EventLoadCompleted, value); }
        ...
        {
            add { Events.AddHandler(EventSoundLocationChanged, value); }
            remove { Events.RemoveHandler(EventSoundLocationChanged, value); }
        }

        /// <include file='doc\SoundPlayer.uex' path='docs/doc[@for="SoundPlayer.StreamChanged"]/*' />
        public event EventHandler StreamChanged
        {
            add { Events.AddHandler(EventStreamChanged, value); }
            remove { Events.RemoveHandler(EventStreamChanged, value); }
        }
        ...
        ) // The set of reasons why WorkerThread should fail is not finite
        ]
        ...
        private unsafe void ValidateSoundFile(string fileName)
        {
            ...
            IntPtr hMIO = UnsafeNativeMethods.mmioOpen(
                fileName,
                IntPtr.Zero,
                NativeMethods.MMIO_READ | NativeMethods.MMIO_ALLOCBUF
            );
            ...
            try
            {
                ...
                if (
                    UnsafeNativeMethods.mmioDescend(hMIO, ckRIFF, null, NativeMethods.MMIO_FINDRIFF)
                    != 0
                )
                    throw new InvalidOperationException(
                        SR.GetString(SR.SoundAPIInvalidWaveFile, this.soundLocation)
                    );
                ...
                while (UnsafeNativeMethods.mmioDescend(hMIO, ck, ckRIFF, 0) == 0)
                {
                    if (ck.dwDataOffset + ck.cksize > ckRIFF.dwDataOffset + ckRIFF.cksize)
                        throw new InvalidOperationException(
                            SR.GetString(SR.SoundAPIInvalidWaveHeader)
                        );
                    ...
                    if (ck.ckID == mmioFOURCC('f', 'm', 't', ' '))
                    {
                        if (waveFormat == null)
                        {
                            ...
                            if (UnsafeNativeMethods.mmioRead(hMIO, data, dw) != dw)
                                throw new InvalidOperationException(
                                    SR.GetString(SR.SoundAPIReadError, this.soundLocation)
                                );
        ...
        private class UnsafeNativeMethods
        {
            ...
            internal static extern bool PlaySound(
                [MarshalAs(UnmanagedType.LPWStr)] string soundName,
                IntPtr hmod,
                int soundFlags
            );
            ...
            [DllImport(ExternDll.WinMM, CharSet = CharSet.Auto)]
            [ResourceExposure(ResourceScope.None)]
            internal static extern int mmioAscend(
                IntPtr hMIO,
