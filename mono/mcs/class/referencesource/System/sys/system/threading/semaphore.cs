    ...
    public sealed class Semaphore : WaitHandle
    {
        ...
        public Semaphore(int initialCount, int maximumCount, string name)
        {
            ...
            var myHandle = new SafeWaitHandle(
                CreateSemaphore_internal(initialCount, maximumCount, name, out errorCode),
                true
            );
#else
            SafeWaitHandle myHandle = SafeNativeMethods.CreateSemaphore(
                null,
                initialCount,
                maximumCount,
                name
            ...
            if (myHandle.IsInvalid)
            {
                ...
                if (
                    null != name
                    && 0 != name.Length
                    && NativeMethods.ERROR_INVALID_HANDLE == errorCode
                )
                    throw new WaitHandleCannotBeOpenedException(
                        SR.GetString(SR.WaitHandleCannotBeOpenedException_InvalidHandle, name)
                    );
...
#endif
        {
            ...
            if (semaphoreSecurity != null)
            {
                ...
                {
                    secAttrs.lpSecurityDescriptor = new SafeLocalMemHandle(
                        (IntPtr)pSecDescriptor,
                        false
                    );
        ...
        )
        {
            ...
            if (name.Length == 0)
            {
                throw new ArgumentException(
                    SR.GetString(SR.InvalidNullEmptyArgument, "name"),
                    "name"
                );
            }
            if (null != name && MAX_PATH < name.Length)
            ...
            int errorCode;
            var myHandle = new SafeWaitHandle(
                OpenSemaphore_internal(name, rights, out errorCode),
                true
            );
#else
            ...
            SafeWaitHandle myHandle = SafeNativeMethods.OpenSemaphore(
            ...
            if (myHandle.IsInvalid)
            {
                ...
                if (
                    ...
                    || NativeMethods.ERROR_INVALID_NAME == errorCode
                )
...
#else
            if (
                !SafeNativeMethods.ReleaseSemaphore(SafeWaitHandle, releaseCount, out previousCount)
            )
#endif
            {
...
#if MONO
        internal unsafe static IntPtr CreateSemaphore_internal(
            int initialCount,
            int maximumCount,
            string name,
            out int errorCode
        ...
        private static unsafe IntPtr OpenSemaphore_internal(
            string name,
            SemaphoreRights rights,
            out int errorCode
        )
        ...
        private static extern unsafe IntPtr CreateSemaphore_icall(
            int initialCount,
            int maximumCount,
            char* name,
            int name_length,
        ...
        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        private static extern unsafe IntPtr OpenSemaphore_icall(
            char* name,
            int name_length,
            SemaphoreRights rights,
            out int errorCode
