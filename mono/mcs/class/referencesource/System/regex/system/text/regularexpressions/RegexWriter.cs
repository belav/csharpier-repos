...
*/
        internal RegexCode RegexCodeFromRegexTree(RegexTree tree)
        {
            ...
            CultureInfo culture =
                (tree._options & RegexOptions.CultureInvariant) != 0
            ...
            switch (nodetype)
            {
                ...
                case RegexNode.Lazyloop | BeforeChild:
                    ...
                    if (node._n < Int32.MaxValue || node._m > 1)
                        Emit(
                            node._m == 0 ? RegexCode.Nullcount : RegexCode.Setcount,
                            node._m == 0 ? 0 : 1 - node._m
                        );
                    else
                ...
                case RegexNode.Onelazy:
                    if (node._m > 0)
                        Emit(
                            (
                                (node._type == RegexNode.Oneloop || node._type == RegexNode.Onelazy)
                                    ? RegexCode.Onerep
                                    : RegexCode.Notonerep
                            ) | bits,
                            (int)node._ch,
