...
namespace System.Diagnostics
{
    ...
    internal class EventLogInternal : IDisposable, ISupportInitialize
    {
        ...
        private readonly static Hashtable listenerInfos = new Hashtable(
        ...
        [SuppressMessage(
            ...
            Justification = "Microsoft: Safe, oldLog.machineName doesn't change"
        )]
        ...
        internal int EntryCount
        {
            get
            {
                ...
                int count;
                bool success = UnsafeNativeMethods.GetNumberOfEventLogRecords(
                    readHandle,
                    out count
                );
                if (!success)
        ...
        [SuppressMessage(
            "Microsoft.Security",
            "CA2103:ReviewImperativeSecurity",
            Justification = "Microsoft: Safe, machineName doesn't change"
        )]
        ...
        [SuppressMessage(
            ...
            Justification = "Microsoft: MaximumKilobytes is the name of this property."
        )]
            ...
            set
            {
                ...
                if (value < 64 || value > 0x3FFFC0 || value % 64 != 0)
                    throw new ArgumentOutOfRangeException(
                        "MaximumKilobytes",
                        SR.GetString(SR.MaximumKilobytesOutOfRange)
                    );
        ...
        )
        {
            lock (InternalSyncObject)
            {
                ...
                bool success = UnsafeNativeMethods.NotifyChangeEventLog(
                    info.handleOwner.ReadHandle,
                    info.waitHandle.SafeWaitHandle
                );
                if (!success)
                    throw new InvalidOperationException(
                        SR.GetString(SR.CantMonitorEventLog),
        ...
        public void BeginInit()
        {
            ...
            if (boolFlags[Flag_initializing])
                throw new InvalidOperationException(SR.GetString(SR.InitTwice));
        ...
        public void Clear()
        {
            ...
            bool success = UnsafeNativeMethods.ClearEventLog(
                readHandle,
        ...
        [SuppressMessage(
            "Microsoft.Security",
            "CA2103:ReviewImperativeSecurity",
            Justification = "Microsoft: Safe, currentMachineName doesn't change"
        )]
        ...
        private void CompletionCallback(object context)
        {
            ...
            Debug.WriteLineIf(
                ...
                "EventLog::CompletionStatusChanged: starting at "
                    + lastSeenCount.ToString(CultureInfo.InvariantCulture)
            );
            ...
            try
            {
                ...
                Debug.WriteLineIf(
                    CompModSwitches.EventLog.TraceVerbose,
                    "EventLog::CompletionStatusChanged: OldestEntryNumber is "
                        + OldestEntryNumber
                        + ", EntryCount is "
                        + EntryCount
                );
        ...
        )
        {
            ...
            foreach (string dllName in listDll)
            {
                ...
                if (IsOpen)
                {
                    ...
                    {
                        hModule = SafeLibraryHandle.LoadLibraryEx(
                            dllName,
                            IntPtr.Zero,
        ...
        internal EventLogEntry[] GetAllEntries()
        {
            ...
            while (idx < entries.Length)
            {
                ...
                bool success = UnsafeNativeMethods.ReadEventLog(
                    readHandle,
                ...
                if (!success)
                {
                    ...
                    if (
                        error == NativeMethods.ERROR_INSUFFICIENT_BUFFER
                        || error == NativeMethods.ERROR_EVENTLOG_FILE_CHANGED
                    )
                    {
#endif
            ...
            if (idx != entries.Length)
            {
                if (error != 0)
                    throw new InvalidOperationException(
                        SR.GetString(SR.CantRetrieveEntries),
                        SharedUtils.CreateSafeWin32Exception(error)
                    );
                else
        ...
        internal EventLogEntry GetEntryAt(int index)
        {
            ...
            if (entry == null)
                throw new ArgumentException(
                    SR.GetString(SR.IndexOutOfBounds, index.ToString(CultureInfo.CurrentCulture))
                );
            return entry;
        }
        ...
        private RegistryKey GetLogRegKey(string currentMachineName, bool writable)
        {
            ...
            try
            {
                ...
                if (eventkey == null)
                    throw new InvalidOperationException(
                        SR.GetString(SR.RegKeyMissingShort, EventLogKey, currentMachineName)
                    );

                logkey = eventkey.OpenSubKey(logname, writable);
                if (logkey == null)
                    throw new InvalidOperationException(
                        SR.GetString(SR.MissingLog, logname, currentMachineName)
                    );
            }
            finally
            {
        ...
        private void OpenForRead(string currentMachineName)
        {
            ...
            if (!EventLog.Exists(logname, currentMachineName)) // do not open non-existing Log [Microsoft]
                throw new InvalidOperationException(
                    SR.GetString(SR.LogDoesNotExists, logname, currentMachineName)
                );
            ...
            SafeEventLogReadHandle handle = SafeEventLogReadHandle.OpenEventLog(
                currentMachineName,
                logname
            );
            if (handle.IsInvalid)
            {
                ...
                throw new InvalidOperationException(
                    SR.GetString(SR.CantOpenLog, logname.ToString(), currentMachineName),
                    e
                );
            }
        ...
        private void OpenForWrite(string currentMachineName)
        {
            ...
            if (handle.IsInvalid)
            {
                ...
                throw new InvalidOperationException(
                    SR.GetString(SR.CantOpenLogAccess, sourceName),
                    e
                );
            }
        ...
        private static void StaticCompletionCallback(object context, bool wasSignaled)
        {
            ...
            lock (InternalSyncObject)
            {
                interestedComponents = (EventLogInternal[])
                    info.listeningComponents.ToArray(typeof(EventLogInternal));
            }

            Debug.WriteLineIf(
                CompModSwitches.EventLog.TraceVerbose,
                "EventLog::StaticCompletionCallback: notifying "
                    + interestedComponents.Length
                    + " components."
            );
        ...
        )
        {
            if (eventID < 0 || eventID > ushort.MaxValue)
                throw new ArgumentException(
                    SR.GetString(SR.EventID, eventID, 0, (int)ushort.MaxValue)
                );
        ...
        )
        {
            ...
            try
            {
                ...
                bool success = UnsafeNativeMethods.ReportEvent(
                    ...
                    rawData.Length,
                    new HandleRef(this, stringsRootHandle.AddrOfPinnedObject()),
                    rawData
                );
                if (!success)
                {
