...
namespace System.Diagnostics
{
    ...
    internal class MainWindowFinder
    {
        ...
        public IntPtr FindMainWindow(int processId)
        {
            ...
            NativeMethods.EnumThreadWindowsCallback callback =
                new NativeMethods.EnumThreadWindowsCallback(this.EnumWindowsCallback);
        ...
        {
            if (
                NativeMethods.GetWindow(new HandleRef(this, handle), NativeMethods.GW_OWNER)
                    != (IntPtr)0
            ...
            else
            {
                if (isRemoteMachine)
                    throw new PlatformNotSupportedException(
                        SR.GetString(SR.WinNTRequiredForRemote)
                    );
        ...
        public static bool IsRemoteMachine(string machineName)
        {
            ...
            if (machineName.Length == 0)
                throw new ArgumentException(
                    SR.GetString(SR.InvalidParameter, "machineName", machineName)
                );
            ...
            if (String.Compare(computerName, baseName, StringComparison.OrdinalIgnoreCase) == 0)
                return false;
            return true;
        }
    }
    ...
    internal static class WinProcessManager
    {
        ...
        public static ProcessInfo[] GetProcessInfos()
        {
            ...
            try
            {
                handle = NativeMethods.CreateToolhelp32Snapshot(
                    NativeMethods.TH32CS_SNAPPROCESS | NativeMethods.TH32CS_SNAPTHREAD,
                    0
                );
                if (handle == (IntPtr)(-1))
                ...
                if (NativeMethods.Process32First(handleRef, bufferPtr))
                {
                    do
                    {
                        ...
                        String name = Marshal.PtrToStringAnsi(
                            (IntPtr)((long)bufferPtr + entrySize)
                        );
            ...
            finally
            {
                if (bufferHandle.IsAllocated)
                    bufferHandle.Free();
                Debug.WriteLineIf(
                    Process.processTracing.TraceVerbose,
        ...
        public static ModuleInfo[] GetModuleInfos(int processId)
        {
            ...
            {
                handle = NativeMethods.CreateToolhelp32Snapshot(
                    NativeMethods.TH32CS_SNAPMODULE,
                    processId
                );
                if (handle == (IntPtr)(-1))
                    throw new Win32Exception();
                ...
                int bufferSize =
                    entrySize
                    + NativeMethods.WinModuleEntry.sizeofFileName
                    + NativeMethods.WinModuleEntry.sizeofModuleName;
                int[] buffer = new int[bufferSize / 4];
                bufferHandle = GCHandle.Alloc(buffer, GCHandleType.Pinned);
                IntPtr bufferPtr = bufferHandle.AddrOfPinnedObject();
                Marshal.WriteInt32(bufferPtr, bufferSize);
                ...
                if (NativeMethods.Module32First(handleRef, bufferPtr))
                {
                    do
                    {
                        ...
                        moduleInfo.baseName = Marshal.PtrToStringAnsi(
                            (IntPtr)((long)bufferPtr + entrySize)
                        );
                        moduleInfo.fileName = Marshal.PtrToStringAnsi(
                            (IntPtr)(
                                (long)bufferPtr
                                + entrySize
                                + NativeMethods.WinModuleEntry.sizeofModuleName
    ...
    internal static class NtProcessManager
    {
        ...
        private static ModuleInfo[] GetModuleInfos(int processId, bool firstModuleOnly)
        {
            ...
            try
            {
                ...
                for (; ; )
                {
                    ...
                    try
                    {
                        moduleHandlesArrayHandle = GCHandle.Alloc(
                            moduleHandles,
                            GCHandleType.Pinned
                        );
                        enumResult = NativeMethods.EnumProcessModules(
                            processHandle,
                            moduleHandlesArrayHandle.AddrOfPinnedObject(),
                            moduleHandles.Length * IntPtr.Size,
                            ref moduleCount
                        );
                        ...
                        if (!enumResult)
                        {
                            ...
                            if (!ProcessManager.IsOSOlderThanXP)
                            {
                                ...
                                try
                                {
                                    ...
                                    wow64Ret = SafeNativeMethods.IsWow64Process(
                                        hCurProcess,
                ...
                for (int i = 0; i < moduleCount; i++)
                {
                    try
                    {
                        ...
                        ret = NativeMethods.GetModuleBaseName(
                            processHandle,
                            new HandleRef(null, moduleHandle),
                            baseName,
                            baseName.Capacity * 2
                        ...
                        ret = NativeMethods.GetModuleFileNameEx(
                            processHandle,
                            new HandleRef(null, moduleHandle),
                            fileName,
                            fileName.Capacity * 2
                        ...
                        {
                            moduleInfo.fileName = Path.Combine(
                    ...
                    {
                        if (
                            e.NativeErrorCode == NativeMethods.ERROR_INVALID_HANDLE
        ...
        public static int GetProcessIdFromHandle(SafeProcessHandle processHandle)
        {
            ...
            int status = NativeMethods.NtQueryInformationProcess(
                processHandle,
                NativeMethods.NtQueryProcessBasicInfo,
                info,
                (int)Marshal.SizeOf(info),
                null
            );
            if (status != 0)
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.CantGetProcessId),
        ...
        public static ProcessInfo[] GetProcessInfos(string machineName, bool isRemoteMachine)
        {
            ...
            try
            {
                library = PerformanceCounterLib.GetPerformanceCounterLib(
                    machineName,
            ...
            catch (Exception e)
            {
                ...
                {
                    throw new InvalidOperationException(
                        SR.GetString(SR.CouldntConnectToRemoteMachine),
                        e
                    );
                }
                else
        ...
        )
        {
            ...
            try
            {
                ...
                NativeMethods.PERF_INSTANCE_DEFINITION instance =
                    new NativeMethods.PERF_INSTANCE_DEFINITION();
                NativeMethods.PERF_COUNTER_BLOCK counterBlock =
                    new NativeMethods.PERF_COUNTER_BLOCK();
                for (int i = 0; i < dataBlock.NumObjectTypes; i++)
                {
                    ...
                    {
                        NativeMethods.PERF_COUNTER_DEFINITION counter =
                            new NativeMethods.PERF_COUNTER_DEFINITION();
                    ...
                    NativeMethods.PERF_COUNTER_DEFINITION[] counters =
                        new NativeMethods.PERF_COUNTER_DEFINITION[counterList.Count];
                    ...
                    for (int j = 0; j < type.NumInstances; j++)
                    {
                        ...
                        if (type.ObjectNameTitleIndex == processIndex)
                        {
                            ProcessInfo processInfo = GetProcessInfo(
                                type,
                                (IntPtr)((long)instancePtr + instance.ByteLength),
                                counters
                            );
                            if (
                                ...
                                else
                                {
                                    ...
                                    if (processName.Length == 15)
                                    {
                                        if (instanceName.EndsWith(".", StringComparison.Ordinal))
                                            ...
                                            instanceName.EndsWith(".e", StringComparison.Ordinal)
                                        )
                                            processName = instanceName.Substring(0, 13);
                                        else if (
                                            instanceName.EndsWith(".ex", StringComparison.Ordinal)
                                        )
                        ...
                        instancePtr = (IntPtr)(
                            (long)instancePtr + instance.ByteLength + counterBlock.ByteLength
                        );
                    }
        ...
        static ThreadInfo GetThreadInfo(
            NativeMethods.PERF_OBJECT_TYPE type,
            IntPtr instancePtr,
            NativeMethods.PERF_COUNTER_DEFINITION[] counters
        )
        {
        ...
        public static ProcessInfo[] GetProcessInfos()
        {
            ...
            try
            {
                ...
                { // see definition of NT_SUCCESS(Status) in SDK
                    throw new InvalidOperationException(
                        SR.GetString(SR.CouldntGetProcessInfos),
                        new Win32Exception(status)
                    );
                }
            ...
            while (true)
            {
                ...
                else
                {
                    string processName = GetProcessShortName(
                        Marshal.PtrToStringUni(pi.NamePtr, pi.NameLength / sizeof(char))
                    );
                    //
                ...
                while (i < pi.NumberOfThreads)
                {
                    ...
                    threadInfo.threadWaitReason = NtProcessManager.GetThreadWaitReason(
                        (int)ti.WaitReason
                    );
