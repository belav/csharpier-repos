    ...
    public class DiagnosticsConfigurationHandler : IConfigurationSectionHandler
    {
            ...
            foreach (XmlNode child in section.ChildNodes)
            {
                ...
                switch (child.Name)
                {
                    case "switches":
                        if (foundSwitches)
                            throw new ConfigurationErrorsException(
                                SR.GetString(SR.ConfigSectionsUnique, "switches")
                            );
                        foundSwitches = true;
                    ...
                    case "assert":
                        if (foundAssert)
                            throw new ConfigurationErrorsException(
                                SR.GetString(SR.ConfigSectionsUnique, "assert")
                            );
                        foundAssert = true;
                    ...
                    case "trace":
                        if (foundTrace)
                            throw new ConfigurationErrorsException(
                                SR.GetString(SR.ConfigSectionsUnique, "trace")
                            );
                        foundTrace = true;
                    ...
                    case "performanceCounters":
                        if (foundCounters)
                            throw new ConfigurationErrorsException(
                                SR.GetString(SR.ConfigSectionsUnique, "performanceCounters")
                            );
                        foundCounters = true;
        ...
        private static void HandleSwitches(Hashtable config, XmlNode switchesNode, object context)
        {
            ...
            while (en.MoveNext())
            {
                ...
                {
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Value_must_be_numeric, en.Key)
                    );
                }
            }
        ...
        private static void HandleAssert(Hashtable config, XmlNode assertNode, object context)
        {
            ...
            if (
                HandlerBase.GetAndRemoveBooleanAttribute(
                    assertNode,
                    "assertuienabled",
                    ref assertuienabled
                ) != null
            ...
            if (
                HandlerBase.GetAndRemoveStringAttribute(assertNode, "logfilename", ref logfilename)
                != null
            )
        ...
        )
        {
            ...
            if (
                HandlerBase.GetAndRemoveIntegerAttribute(
                    countersNode,
                    "filemappingsize",
                    ref filemappingsize
                ) != null
        ...
        private static void HandleTrace(Hashtable config, XmlNode traceNode, object context)
        {
            ...
            if (
                HandlerBase.GetAndRemoveBooleanAttribute(traceNode, "autoflush", ref autoflush)
                != null
            )
            ...
            int indentsize = 0;
            if (
                HandlerBase.GetAndRemoveIntegerAttribute(traceNode, "indentsize", ref indentsize)
                != null
            )
            ...
            foreach (XmlNode traceChild in traceNode.ChildNodes)
            {
                ...
                if (traceChild.Name == "listeners")
                {
                    if (foundListeners)
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.ConfigSectionsUnique, "listeners")
                        );
                    foundListeners = true;
            ...
            foreach (XmlNode listenersChild in listenersNode.ChildNodes)
            {
                ...
                if (className != null)
                {
                    ...
                    if (!typeof(TraceListener).IsAssignableFrom(t))
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Type_isnt_tracelistener, className)
                        );
                    ...
                    if (initializeData == null)
                    {
                        ...
                        newListener = (TraceListener)(
                            SecurityUtils.ConstructorInfoInvoke(ctorInfo, new object[] { })
                        );
                    }
                    ...
                    else
                    {
                        ...
                        newListener = (TraceListener)(
                            SecurityUtils.ConstructorInfoInvoke(
                                ctorInfo,
                ...
                switch (op[0])
                {
                    case 'a':
                        if (newListener == null)
                            throw new ConfigurationErrorsException(
                                SR.GetString(SR.Could_not_create_listener, name)
                            );
                    ...
                    case 'r':
                        if (newListener == null)
                        {
                            ...
                            if (name == null)
                                throw new ConfigurationErrorsException(
                                    SR.GetString(SR.Cannot_remove_with_null)
                                );
