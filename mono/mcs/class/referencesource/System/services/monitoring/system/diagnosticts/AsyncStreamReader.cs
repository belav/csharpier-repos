...
#pragma warning restore
                        ...
                        if (asyncReadResult != null && !asyncReadResult.IsCompleted)
                        {
                            ...
                            if (stream is FileStream)
                            {
                                ...
                                while (!asyncReadResult.IsCompleted)
                                {
                                    MonoIOError error;
                                    if (
                                        !MonoIO.Cancel(tmpStreamHandle, out error)
                                        && error == MonoIOError.ERROR_NOT_SUPPORTED
                                    )
                                    {
...
#if MONO
                asyncReadResult = stream.BeginRead(
                    byteBuffer,
                    0,
                    byteBuffer.Length,
        ...
        private void ReadBuffer(IAsyncResult ar)
        {
            ...
            else
            {
                ...
                {
                    if (decoder == null)
                    { //we got disposed after the EndRead, retry as Diposed
                        byteLen = 0;
                        goto retry_dispose;
                    }
#endif
