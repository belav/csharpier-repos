...
namespace System.Diagnostics
{
    ...
    [
        ...
        InstallerType(
            "System.Diagnostics.EventLogInstaller, " + AssemblyRef.SystemConfigurationInstall
        ),
    ...
    public class EventLog : Component, ISupportInitialize
    {
        ...
        {
            get { return m_underlyingEventLog.Entries; }
        }
        ...
        public string LogDisplayName
        {
            ...
            get { return m_underlyingEventLog.LogDisplayName; }
        }
        ...
        [SuppressMessage(
            ...
            Justification = "Microsoft: By design, see justification above assert"
        )]
        public string Log
        {
            get { return m_underlyingEventLog.Log; }
            set
            {
                EventLogInternal newLog = new EventLogInternal(
                    value,
                    m_underlyingEventLog.MachineName,
                    m_underlyingEventLog.Source,
                    this
                );
        ...
        {
            get { return m_underlyingEventLog.MachineName; }
            set
            {
                EventLogInternal newLog = new EventLogInternal(
                    m_underlyingEventLog.logName,
                    value,
                    m_underlyingEventLog.sourceName,
                    this
                );
                EventLogInternal oldLog = m_underlyingEventLog;
        ...
        {
            get { return m_underlyingEventLog.MaximumKilobytes; }
            [ResourceExposure(ResourceScope.None)]
            [ResourceConsumption(ResourceScope.Machine, ResourceScope.Machine)]
            set { m_underlyingEventLog.MaximumKilobytes = value; }
        }
        ...
        {
            get { return m_underlyingEventLog.OverflowAction; }
        }
        ...
        {
            get { return m_underlyingEventLog.MinimumRetentionDays; }
        }
        ...
        {
            get { return m_underlyingEventLog.EnableRaisingEvents; }
            set { m_underlyingEventLog.EnableRaisingEvents = value; }
        }
        ...
        public ISynchronizeInvoke SynchronizingObject
        {
            ...
            get { return m_underlyingEventLog.SynchronizingObject; }
            set { m_underlyingEventLog.SynchronizingObject = value; }
        }
        ...
        {
            get { return m_underlyingEventLog.Source; }
            set
            {
                EventLogInternal newLog = new EventLogInternal(
                    m_underlyingEventLog.Log,
                    m_underlyingEventLog.MachineName,
        ...
        {
            add { m_underlyingEventLog.EntryWritten += value; }
            remove { m_underlyingEventLog.EntryWritten -= value; }
        }
        ...
        public static void CreateEventSource(EventSourceCreationData sourceData)
        {
            ...
            if (source.Length + EventLogKey.Length > 254)
                throw new ArgumentException(
                    SR.GetString(SR.ParameterTooLong, "source", 254 - EventLogKey.Length)
                );
            ...
            try
            {
                ...
                if (SourceExists(source, machineName, true))
                {
                    ...
                    if (".".Equals(machineName))
                        throw new ArgumentException(
                            SR.GetString(SR.LocalSourceAlreadyExists, source)
                        );
                    else
                        throw new ArgumentException(
                            SR.GetString(SR.SourceAlreadyExists, source, machineName)
                        );
                }
                ...
                try
                {
                    ...
                    if (logKey == null && logName.Length >= 8)
                    {
                        ...
                        if (
                            ...
                            throw new ArgumentException(
                                SR.GetString(SR.InvalidCustomerLogName, logName)
                            );

                        string sameLogName = FindSame8FirstCharsLog(eventKey, logName);
                        if (sameLogName != null)
                            throw new ArgumentException(
                                SR.GetString(SR.DuplicateLogName, logName, sameLogName)
                            );
                    }
                    ...
                    if (createLogKey)
                    {
                        if (SourceExists(logName, machineName, true))
                        {
                            ...
                            if (".".Equals(machineName))
                                throw new ArgumentException(
                                    SR.GetString(SR.LocalLogAlreadyExistsAsSource, logName)
                                );
                            else
                                throw new ArgumentException(
                                    SR.GetString(SR.LogAlreadyExistsAsSource, logName, machineName)
                                );
                        }
            ...
            try
            {
                ...
                try
                {
                    ...
                    using (RegistryKey logKey = eventlogkey.OpenSubKey(logName))
                    {
                        if (logKey == null)
                            throw new InvalidOperationException(
                                SR.GetString(SR.MissingLog, logName, machineName)
                            );
            ...
            try
            {
                ...
                using (key = FindSourceRegistration(source, machineName, true))
                {
                    if (key == null)
                    {
                        if (machineName == null)
                            throw new ArgumentException(
                                SR.GetString(SR.LocalSourceNotRegistered, source)
                            );
                        else
                            throw new ArgumentException(
                                SR.GetString(
                    ...
                    if (
                        ...
                        throw new InvalidOperationException(
                            SR.GetString(SR.CannotDeleteEqualSource, source)
                        );
                }
        ...
        private static string FindSame8FirstCharsLog(RegistryKey keyParent, string logName)
        {
            ...
            for (int i = 0; i < logNames.Length; i++)
            {
                ...
                if (
                    currentLogName.Length >= 8
                    && string.Compare(
                        currentLogName.Substring(0, 8),
                        logNameFirst8,
                        StringComparison.OrdinalIgnoreCase
                    ) == 0
                )
        ...
        public static EventLog[] GetEventLogs(string machineName)
        {
            ...
            try
            {
                ...
                if (eventkey == null)
                    ...
                    // or, more likely, we don't have the access to read the registry.
                    throw new InvalidOperationException(
                        SR.GetString(SR.RegKeyMissingShort, EventLogKey, machineName)
                    );
                // Most machines will return only { "Application", "System", "Security" },
                // but you can create your own if you want.
        ...
        private static void SetSpecialLogRegValues(RegistryKey logKey, string logName)
        {
            ...
            if (!SkipRegPatch)
            {
                ...
                if (logKey.GetValue("File") == null)
                {
                    ...
                    if (logName.Length > 8)
                        filename =
                            @"%SystemRoot%\System32\config\" + logName.Substring(0, 8) + ".evt";
                    else
