...
namespace System.Diagnostics
{
    ...
    public static class CounterSampleCalculator
    {
        ...
        public static float ComputeCounterValue(CounterSample oldSample, CounterSample newSample)
        {
            ...
            {
                if (
                    (newCounterType != NativeMethods.PERF_RAW_FRACTION)
            ...
            NativeMethods.PDH_FMT_COUNTERVALUE pdhFormattedValue =
                new NativeMethods.PDH_FMT_COUNTERVALUE();
            long timeBase = newSample.SystemFrequency;
            int result = SafeNativeMethods.FormatFromRawValue(
                (uint)newCounterType,
                NativeMethods.PDH_FMT_DOUBLE
                    | NativeMethods.PDH_FMT_NOSCALE
                    | NativeMethods.PDH_FMT_NOCAP100,
                ref timeBase,
                newPdhValue,
                oldPdhValue,
                pdhFormattedValue
            ...
            if (result != NativeMethods.ERROR_SUCCESS)
            {
                ...
                if (
                    result == NativeMethods.PDH_CALC_NEGATIVE_VALUE
                    || result == NativeMethods.PDH_CALC_NEGATIVE_DENOMINATOR
                    || result == NativeMethods.PDH_NO_DATA
                )
                    return 0;
                else
        ...
        private static void FillInValues(
            ...
            NativeMethods.PDH_RAW_COUNTER oldPdhValue,
            NativeMethods.PDH_RAW_COUNTER newPdhValue
        )
        {
            int newCounterType = (int)newSample.CounterType;
            ...
            switch (newCounterType)
            {
                ...
                case NativeMethods.PERF_COUNTER_MULTI_TIMER_INV:
                    ...
                    oldPdhValue.SecondValue = oldSample.TimeStamp;
                    if (
                        newCounterType == NativeMethods.PERF_COUNTER_MULTI_TIMER
                    ...
                    if (
                        (newCounterType & NativeMethods.PERF_MULTI_COUNTER)
                        == NativeMethods.PERF_MULTI_COUNTER
                    )
                    {
