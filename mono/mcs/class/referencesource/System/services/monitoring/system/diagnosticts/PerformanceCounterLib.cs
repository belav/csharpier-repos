namespace System.Diagnostics
{
    ...
    internal class PerformanceCounterLib
    {
        ...
        private const string PerflibPath =
                    ...
                    lock (this.CategoryTableLock)
                    {
                        if (this.categoryTable == null)
                        {
                            ...
                            fixed (byte* perfDataPtr = perfData)
                            {
                                ...
                                NativeMethods.PERF_DATA_BLOCK dataBlock =
                                    new NativeMethods.PERF_DATA_BLOCK();
                                ...
                                long endPerfData =
                                    (long)(new IntPtr((void*)perfDataPtr))
                                    + dataBlock.TotalByteLength;
                                Hashtable tempCategoryTable = new Hashtable(
                                    categoryNumber,
                                    StringComparer.OrdinalIgnoreCase
                                );
                                for (
                                    int index = 0;
                                    index < categoryNumber && ((long)dataRef < endPerfData);
                                    index++
                                )
                                {
                                    NativeMethods.PERF_OBJECT_TYPE perfObject =
                                        new NativeMethods.PERF_OBJECT_TYPE();

                                    Marshal.PtrToStructure(dataRef, perfObject);

                                    CategoryEntry newCategoryEntry = new CategoryEntry(perfObject);
                                    IntPtr nextRef = (IntPtr)(
                                        (long)dataRef + perfObject.TotalByteLength
                                    );
                                    ...
                                    {
                                        NativeMethods.PERF_COUNTER_DEFINITION perfCounter =
                                            new NativeMethods.PERF_COUNTER_DEFINITION();
                                        Marshal.PtrToStructure(dataRef, perfCounter);
                                        if (
                                            perfCounter.CounterNameTitleIndex
                                            != previousCounterIndex
                                        )
                                        {
                                            newCategoryEntry.CounterIndexes[index3] =
                                                perfCounter.CounterNameTitleIndex;
                                            newCategoryEntry.HelpIndexes[index3] =
                                                perfCounter.CounterHelpTitleIndex;
                                            previousCounterIndex =
                                                perfCounter.CounterNameTitleIndex;
                                            ++index3;
                                        }
                                    ...
                                    if (index3 < newCategoryEntry.CounterIndexes.Length)
                                    {
                                        ...
                                        Array.Copy(
                                            newCategoryEntry.CounterIndexes,
                                            adjustedCounterIndexes,
                                            index3
                                        );
                                        Array.Copy(
                                            newCategoryEntry.HelpIndexes,
                                            adjustedHelpIndexes,
                                            index3
                                        );
                                    ...
                                    string categoryName = (string)
                                        this.NameTable[newCategoryEntry.NameIndex];
                ...
                try
                {
                    ...
                    foreach (string languageId in languageIds)
                    {
                        ...
                        foreach (CounterCreationData counterData in creationData)
                        {
                            ...
                            Debug.Assert(
                                !String.IsNullOrEmpty(counterData.CounterHelp),
                                "CounterHelp should have been fixed up by the caller"
                            );
            ...
            try
            {
                ...
                try
                {
                    ...
                    for (int counterIndex = 1; counterIndex <= creationData.Count; ++counterIndex)
                    {
                        ...
                        symbolWriter.Write(" ");
                        symbolWriter.Write(
                            (counterIndex * 2).ToString(CultureInfo.InvariantCulture)
                        );
        ...
        )
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (key != null)
                    {
                        object systemDllName = key.GetValue(
                            "Library",
                            null,
        ...
        private CategorySample GetCategorySample(string category)
        {
            ...
            byte[] dataRef = GetPerformanceData(
                entry.NameIndex.ToString(CultureInfo.InvariantCulture)
            );
            if (dataRef == null)
        ...
        internal static string GetCounterHelp(string machine, string category, string counter)
        {
            ...
            if (!categoryExists)
                throw new InvalidOperationException(
                    SR.GetString(SR.MissingCategoryDetail, category)
                );
        ...
        private Hashtable GetStringTable(bool isHelp)
        {
            ...
            if (
                String.Compare(this.machineName, ComputerName, StringComparison.OrdinalIgnoreCase)
                == 0
            )
                    ...
                    for (int index = 0; index < (names.Length / 2); ++index)
                    {
                        ...
                        )
                        {
                            if (isHelp)
                            {
                                // Category Help Table
                                throw new InvalidOperationException(
                                    SR.GetString(SR.CategoryHelpCorrupt, names[index * 2])
                                );
                            }
                            else
                            {
                                // Counter Name Table
                                throw new InvalidOperationException(
                                    SR.GetString(SR.CounterNameCorrupt, names[index * 2])
                                );
                            }
                        }
        ...
        {
            return (
                type == NativeMethods.PERF_AVERAGE_BASE
        ...
        private static void RegisterFiles(string arg0, bool unregister)
        {
            ...
            if (res == NativeMethods.ERROR_ACCESS_DENIED)
            {
                throw new UnauthorizedAccessException(
                    SR.GetString(SR.CantChangeCategoryRegistration, arg0)
                );
            }
    ...
    internal class PerformanceMonitor
    {
        ...
        internal byte[] GetData(string item)
        {
            ...
            while (waitRetries > 0)
            {
                ...
                {
                    throw new InvalidOperationException(
                        SR.GetString(SR.CounterDataCorrupt, perfDataKey.ToString()),
                        e
                    );
                }
            }
    ...
    internal class CategorySample
    {
        ...
        internal unsafe CategorySample(
            byte[] data,
            CategoryEntry entry,
            PerformanceCounterLib library
        )
        {
            ...
            fixed (byte* dataPtr = data)
            {
                ...
                if (!foundCategory)
                    throw new InvalidOperationException(
                        SR.GetString(
                            SR.CantReadCategoryIndex,
                            categoryIndex.ToString(CultureInfo.CurrentCulture)
                        )
                    );
                ...
                for (int index = 0; index < samples.Length; ++index)
                {
                    ...
                    else
                    {
                        ...
                        Debug.Assert(
                            index > 0,
                            "Index > 0 because base counters should never be at index 0"
                        );
                        if (index > 0)
                ...
                else
                {
                    ...
                    this.InstanceNameTable = new Hashtable(
                        instanceNumber,
                        StringComparer.OrdinalIgnoreCase
                    );
                    for (int i = 0; i < instanceNumber; i++)
                    {
                        NativeMethods.PERF_INSTANCE_DEFINITION perfInstance =
                        ...
                        if (perfInstance.ParentObjectTitleIndex > 0 && parentInstanceNames == null)
                            parentInstanceNames = GetInstanceNamesFromIndex(
                                perfInstance.ParentObjectTitleIndex
                            );

                        string instanceName;
                        if (
                            parentInstanceNames != null
                            && perfInstance.ParentObjectInstance >= 0
                            && perfInstance.ParentObjectInstance < parentInstanceNames.Length - 1
                        )
                            instanceName =
                                parentInstanceNames[perfInstance.ParentObjectInstance]
                                + "/"
                                + Marshal.PtrToStringUni(
                                    (IntPtr)((long)dataRef + perfInstance.NameOffset)
                                );
                        else
                        ...
                        while (true)
                        {
                            ...
                            else
                            {
                                newInstanceName =
                                    instanceName
                                    + "#"
                                    + newInstanceNumber.ToString(CultureInfo.InvariantCulture);
                                ++newInstanceNumber;
                            }
                        }
        ...
        {
            byte[] data = library.GetPerformanceData(
            ...
            fixed (byte* dataPtr = data)
            {
                ...
                for (int i = 0; i < instanceNumber; i++)
                {
                    NativeMethods.PERF_INSTANCE_DEFINITION perfInstance =
                        new NativeMethods.PERF_INSTANCE_DEFINITION();
                    Marshal.PtrToStructure(dataRef, perfInstance);
                    instanceNames[i] = Marshal.PtrToStringUni(
                        (IntPtr)((long)dataRef + perfInstance.NameOffset)
                    );
                    dataRef = (IntPtr)((long)dataRef + perfInstance.ByteLength);
                    dataRef = (IntPtr)((long)dataRef + Marshal.ReadInt32(dataRef));
        ...
        internal CounterDefinitionSample GetCounterDefinitionSample(string counter)
        {
            for (int index = 0; index < this.entry.CounterIndexes.Length; ++index)
            {
                ...
                {
                    if (
                        String.Compare(counterName, counter, StringComparison.OrdinalIgnoreCase)
                        == 0
                    )
                    {
                        CounterDefinitionSample sample = (CounterDefinitionSample)
                            this.CounterTable[counterIndex];
                        if (sample == null)
                        {
                            //This is a base counter and has not been added to the table
                            foreach (
                                CounterDefinitionSample multiSample in this.CounterTable.Values
                            )
                            {
                                if (
                                    multiSample.BaseCounterDefinitionSample != null
                                    && multiSample.BaseCounterDefinitionSample.NameIndex
                                        == counterIndex
                                )
        ...
        internal CounterSample GetInstanceValue(string instanceName)
        {
            if (!categorySample.InstanceNameTable.ContainsKey(instanceName))
            {
                ...
                if (instanceName.Length > SharedPerformanceCounter.InstanceNameMaxLength)
                    instanceName = instanceName.Substring(
                        0,
                        SharedPerformanceCounter.InstanceNameMaxLength
                    );

                if (!categorySample.InstanceNameTable.ContainsKey(instanceName))
                    throw new InvalidOperationException(
                        SR.GetString(SR.CantReadInstance, instanceName)
                    );
            }
...
#pragma warning restore 618
            ...
            for (int index = 0; index < keys.Length; ++index)
            {
                ...
                CounterSample sample = new CounterSample(
                    this.instanceValues[indexes[index]],
                    baseValue,
                    categorySample.CounterFrequency,
                    categorySample.SystemFrequency,
                    categorySample.TimeStamp,
