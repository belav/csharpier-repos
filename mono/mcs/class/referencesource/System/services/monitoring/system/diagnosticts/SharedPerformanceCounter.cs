...
namespace System.Diagnostics
{
    ...
    internal sealed class SharedPerformanceCounter
    {
        ...
        private static readonly int ProcessLifetimeEntrySize = Marshal.SizeOf(
            ...
            get
            {
                if (procData == null)
                {
                    ...
                    try
                    {
                        ...
                        using (
                            SafeProcessHandle procHandle = SafeProcessHandle.OpenProcess(
        ...
        )
        {
            ...
            else
            {
                if (lifetime != PerformanceCounterInstanceLifetime.Global)
                    throw new InvalidOperationException(
                        SR.GetString(SR.ProcessLifetimeNotValidInGlobal)
                    );
            }
            ...
            if (counterName != null && instanceName != null)
            {
                if (!categoryData.CounterNames.Contains(counterName))
                    Debug.Assert(
                        false,
                        "Counter " + counterName + " does not exist in category " + catName
                    );
                else
                    this.counterEntryPointer = GetCounter(
                        counterName,
                        instanceName,
                        categoryData.EnableReuse,
                        lifetime
                    );
            }
        }
        ...
        private FileMapping FileView
        {
            ...
            get { return categoryData.FileMapping; }
        }
        ...
        private unsafe int CalculateAndAllocateMemory(int totalSize, out int alignmentAdjustment)
        {
            ...
            Debug.Assert(
                !categoryData.UseUniqueSharedMemory,
            ...
            do
            {
                ...
                SafeNativeMethods.InterlockedCompareExchange(
                    (IntPtr)baseAddress,
                    newOffset,
                    oldOffset
                ) != oldOffset
        ...
        )
        {
            ...
            if (categoryData.UseUniqueSharedMemory)
            {
                ...
                newCounterEntryPointer->LifetimeOffset = (int)(
                    (long)newLifetimeEntry - baseAddress
                );
            ...
            newCategoryEntryPointer->FirstInstanceOffset = (int)(
                (long)newInstanceEntryPointer - baseAddress
            );
            ...
            newInstanceEntryPointer->FirstCounterOffset = (int)(
                (long)newCounterEntryPointer - baseAddress
            );
            ...
            Debug.Assert(
                nextPtr - baseAddress == freeMemoryOffset + totalSize + alignmentAdjustment,
        ...
        )
        {
            int instanceNameLength;
            int totalSize =
                InstanceEntrySize + (CounterEntrySize * categoryData.CounterNames.Count);
            int alignmentAdjustment;
            int freeMemoryOffset;
            int newOffset = 0;
            ...
            Debug.Assert(
                nextPtr - baseAddress == freeMemoryOffset + totalSize,
                "We should have used all of the space we requested at this point"
            );
        ...
        )
        {
            ...
            Debug.Assert(
                nextPtr + counterNameLength - baseAddress == freeMemoryOffset + totalSize,
                "We should have used all of the space we requested at this point"
            );
        ...
        )
        {
                ...
                while (!FindCategory(&categoryPointer))
                {
                    ...
                    if (sectionEntered)
                    {
                        ...
                        categoryPointer = (CategoryEntry*)(
                            ResolveOffset(newCategoryOffset, CategoryEntrySize)
                        );
                        instancePointer = (InstanceEntry*)(
                            ResolveOffset(categoryPointer->FirstInstanceOffset, InstanceEntrySize)
                        );
                    ...
                    if (sectionEntered)
                    {
                        try
                        {
                            ...
                            if (!reused)
                            {
                                ...
                                instancePointer = (InstanceEntry*)(
                                    ResolveOffset(newInstanceOffset, InstanceEntrySize)
                                );
                    ...
                    )
                    {
                        ...
                        if (sectionEntered)
                        {
                            try
                            {
                                ...
                                return (CounterEntry*)(
                                    ResolveOffset(newCounterOffset, CounterEntrySize)
                                );
                            }
                            finally
                            {
            ...
            finally
            {
                ...
                try
                {
                    ...
                    {
                        this.thisInstanceOffset = ResolveAddress(
        ...
        private unsafe bool FindCategory(CategoryEntry** returnCategoryPointerReference)
        {
            CategoryEntry* firstCategoryPointer = (CategoryEntry*)(
                ResolveOffset(InitialOffset, CategoryEntrySize)
            );
            ...
            for (; ; )
            {
                ...
                if (currentCategoryPointer->NextCategoryOffset != 0)
                    currentCategoryPointer = (CategoryEntry*)(
                        ResolveOffset(currentCategoryPointer->NextCategoryOffset, CategoryEntrySize)
                    );
                else
                {
        ...
        )
        {
            CounterEntry* currentCounterPointer = (CounterEntry*)(
                ResolveOffset(instancePointer->FirstCounterOffset, CounterEntrySize)
            );
            ...
            for (; ; )
            {
                ...
                if (currentCounterPointer->NextCounterOffset != 0)
                    currentCounterPointer = (CounterEntry*)(
                        ResolveOffset(currentCounterPointer->NextCounterOffset, CounterEntrySize)
                    );
                else
                {
        ...
        )
        {
            ...
            if (currentInstancePointer->InstanceNameHashCode == SingleInstanceHashCode)
            {
                if (StringEquals(SingleInstanceName, currentInstancePointer->InstanceNameOffset))
                {
                    if (instanceName != SingleInstanceName)
                        throw new InvalidOperationException(
                            SR.GetString(SR.SingleInstanceOnly, categoryName)
                        );
                }
                else
                {
                    if (instanceName == SingleInstanceName)
                        throw new InvalidOperationException(
                            SR.GetString(SR.MultiInstanceOnly, categoryName)
                        );
                }
            }
            else
            ...
            if (activateUnusedInstances)
            {
                ...
                {
                    long tickDelta = (
                        DateTime.Now.Ticks - Volatile.Read(ref LastInstanceLifetimeSweepTick)
                    );
                            ...
                            *returnInstancePointerReference = currentInstancePointer;
                            ...
                            if (currentInstancePointer->RefCount != 0)
                            {
                                ...
                                else
                                {
                                    if (lifetime == PerformanceCounterInstanceLifetime.Process)
                                        throw new InvalidOperationException(
                                            SR.GetString(SR.CantConvertGlobalToProcess)
                                        );
                                }
                                return true;
                            }
        ...
        )
        {
            ...
            for (; ; )
            {
                ...
                if (currentInstancePointer->NextInstanceOffset != 0)
                    currentInstancePointer = (InstanceEntry*)(
                        ResolveOffset(currentInstancePointer->NextInstanceOffset, InstanceEntrySize)
                    );
                else
                {
                    *returnInstancePointerReference = previousInstancePointer;
            ...
            try
            {
                ...
                for (; ; )
                {
                    ...
                    if (instancePointer->NextInstanceOffset != 0)
                        instancePointer = (InstanceEntry*)(
                            ResolveOffset(instancePointer->NextInstanceOffset, InstanceEntrySize)
                        );
                    else
                    {
                        break;
            ...
            try
            {
                ...
                if (this.thisInstanceOffset != -1)
                {
                    try
                    {
                        ...
                        instancePointer = (InstanceEntry*)(
                            ResolveOffset(this.thisInstanceOffset, InstanceEntrySize)
                        );
        ...
        private class FileMapping
        {
            ...
            )
            {
                ...
                try
                {
                    ...
                    if (
                        !SafeLocalMemHandle.ConvertStringSecurityDescriptorToSecurityDescriptor(
                            sddlString,
                        ...
                        throw new InvalidOperationException(
                            SR.GetString(SR.SetSecurityDescriptorFailed)
                        );
                    ...
                    {
                        fileMappingHandle = NativeMethods.CreateFileMapping(
                            (IntPtr)(-1),
                            securityAttributes,
                        ...
                        else
                        {
                            ...
                            fileMappingHandle = NativeMethods.OpenFileMapping(
                                NativeMethods.FILE_MAP_WRITE,
                                false,
                                mappingName
                            );

                            if (
                                (Marshal.GetLastWin32Error() != NativeMethods.ERROR_FILE_NOT_FOUND)
                                || !fileMappingHandle.IsInvalid
                            )
                            {
                                created = true;
                    ...
                    fileViewAddress = SafeFileMapViewHandle.MapViewOfFile(
                        fileMappingHandle,
                        NativeMethods.FILE_MAP_WRITE,
                        0,
                        0,
                    ...
                    NativeMethods.MEMORY_BASIC_INFORMATION meminfo =
                        new NativeMethods.MEMORY_BASIC_INFORMATION();
                    if (
                        NativeMethods.VirtualQuery(
                            fileViewAddress,
                            ref meminfo,
                            (IntPtr)sizeof(NativeMethods.MEMORY_BASIC_INFORMATION)
                        ) == IntPtr.Zero
                    )
                ...
                SafeNativeMethods.InterlockedCompareExchange(
                    fileViewAddress.DangerousGetHandle(),
                    initialOffset,
                    0
                );
            }
