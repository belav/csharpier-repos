...
namespace Microsoft.CSharp
{
    ...
    internal class CSharpCodeGenerator : ICodeCompiler, ICodeGenerator
    {
        ...
        private void ProcessCompilerOutputLine(CompilerResults results, string line)
        {
            ...
            {
                outputRegWithFileAndLine = new Regex(
        ...
        private string CmdArgsFromParameters(CompilerParameters options)
        {
            ...
            if (String.IsNullOrWhiteSpace(options.CoreAssemblyFileName))
            {
                ...
                if (
                    CodeDomProvider.TryGetProbableCoreAssemblyFilePath(
                        options,
                        out probableCoreAssemblyFilePath
                    )
                )
                {
        ...
        private void GenerateStatement(CodeStatement e)
        {
            ...
            else
            {
                throw new ArgumentException(
                    SR.GetString(SR.InvalidElementType, e.GetType().FullName),
                    "e"
                );
            }
        ...
        private void GeneratePrimitiveExpressionBase(CodePrimitiveExpression e)
        {
            ...
            {
                throw new ArgumentException(
                    SR.GetString(SR.InvalidPrimitiveType, e.Value.GetType().ToString())
                );
            }
        }
        ...
        private void GenerateProperty(CodeMemberProperty e, CodeTypeDeclaration c)
        {
            ...
            if (
                ...
                && String.Compare(e.Name, "Item", StringComparison.OrdinalIgnoreCase) == 0
            )
            {
                Output.Write("this[");
                OutputParameters(e.Parameters);
        ...
        private void OutputParameters(CodeParameterDeclarationExpressionCollection parameters)
        {
            ...
            while (en.MoveNext())
            {
                CodeParameterDeclarationExpression current = (CodeParameterDeclarationExpression)
                    en.Current;
                if (first)
                {
                    first = false;
                    ...
                    for (int i = 0; i < baseType.Length; i++)
                    {
                        switch (baseType[i])
                        {
                            ...
                            case '`':
                                ...
                                int numTypeArgs = 0;
                                while (
                                    i < baseType.Length && baseType[i] >= '0' && baseType[i] <= '9'
                                )
                                {
...
#pragma warning restore 618
        ...
        /// <internalonly/>
        void ICodeGenerator.GenerateCodeFromNamespace(
            CodeNamespace e,
            TextWriter w,
            CodeGeneratorOptions o
        )
