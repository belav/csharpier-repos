...
namespace System.Diagnostics
{
    ...
    {
        private static readonly ConfigurationProperty _propFilter = new ConfigurationProperty(
            "filter",
        ...
        private static readonly ConfigurationProperty _propOutputOpts = new ConfigurationProperty(
            "traceOutputOptions",
            typeof(TraceOptions),
            TraceOptions.None,
            ConfigurationPropertyOptions.None
        ...
        {
            get { return (FilterElement)this[_propFilter]; }
        }
        ...
        public override bool Equals(object compareTo)
        {
            if (
                ...
                && this.TypeName.Equals(typeof(DefaultTraceListener).FullName)
            )
            {
        ...
        public TraceListener GetRuntimeObject()
        {
            ...
            try
            {
                ...
                if (String.IsNullOrEmpty(className))
                {
                    ...
                    if (
                        _attributes != null
                        || ElementInformation.Properties[_propFilter.Name].ValueOrigin
                            == PropertyValueOrigin.SetHere
                        || TraceOutputOptions != TraceOptions.None
                        || !String.IsNullOrEmpty(InitData)
                    )
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Reference_listener_cant_have_properties, Name)
                        );

                    if (DiagnosticsConfiguration.SharedListeners == null)
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Reference_to_nonexistent_listener, Name)
                        );
                ...
                else
                {
                    ...
                    if (
                        ...
                        && (Filter.TypeName.Length != 0)
                    )
                    {
                        newListener.Filter = Filter.GetRuntimeObject();
                        XmlWriterTraceListener listerAsXmlWriter =
                            newListener as XmlWriterTraceListener;
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (
                        ...
                        || InitData != listener.initializeData
                    )
                    {
