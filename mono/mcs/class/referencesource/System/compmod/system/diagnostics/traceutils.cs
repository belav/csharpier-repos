    ...
    internal static class TraceUtils
    {
        ...
        )
        {
            ...
            if (objectType == null)
            {
                throw new ConfigurationErrorsException(
                    SR.GetString(SR.Could_not_find_type, className)
                );
            }

            if (!baseType.IsAssignableFrom(objectType))
                throw new ConfigurationErrorsException(
                    SR.GetString(SR.Incorrect_base_type, className, baseType.FullName)
                );
            ...
            try
            {
                if (String.IsNullOrEmpty(initializeData))
                {
                    if (IsOwnedTL(objectType))
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.TL_InitializeData_NotSpecified)
                        );
                    ...
                    if (ctorInfo == null)
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Could_not_get_constructor, className)
                        );
                ...
                else
                {
                    ...
                    if (ctorInfo != null)
                    {
                        ...
                        newObject = SecurityUtils.ConstructorInfoInvoke(
                            ctorInfo,
                        ...
                        for (int i = 0; i < ctorInfos.Length; i++)
                        {
                            ...
                            if (ctorparams.Length == 1)
                            {
                                ...
                                try
                                {
                                    ...
                                    newObject = SecurityUtils.ConstructorInfoInvoke(
                                        ctorInfos[i],
                                        new object[] { convertedInitializeData }
                                    );
                                    break;
                                }
            ...
            if (newObject == null)
            {
                if (innerException != null)
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Could_not_create_type_instance, className),
                        innerException
                    );
                else
                    throw new ConfigurationErrorsException(
        ...
        )
        {
            ...
            foreach (string key in attributes.Keys)
            {
                ...
                if (!found)
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.AttributeNotSupported, key, parent.GetType().FullName)
                    );
            }
#endif
        }
