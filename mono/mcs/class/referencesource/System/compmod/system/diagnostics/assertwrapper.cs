...
namespace System.Diagnostics
{
    internal static class AssertWrapper
    {
        ...
        private static void ShowMessageBoxAssert(
            string stackTrace,
            string message,
            string detailMessage
        )
        {
...
#else
            int flags =
                0x00000002 /*AbortRetryIgnore*/
                | 0x00000200 /*DefaultButton3*/
                | 0x00000010 /*IconHand*/
                | 0x00040000 /* TopMost */
            ;
#endif
            ...
            if (!Environment.UserInteractive)
                flags =
                    flags
                    | 0x00200000 /*ServiceNotification */
                ;
#endif
            ...
            {
                rval = SafeNativeMethods.MessageBox(
                    IntPtr.Zero,
                    fullMessage,
        ...
        {
            get { return SR.GetString(SR.RTL) != "RTL_False"; }
        }

#if SILVERLIGHT && FEATURE_PAL
        [SecuritySafeCritical]
        private static void ShowCFUserNotificationDisplayAlertAssert(
            string stackTrace,
            string message,
            string detailMessage
        )
        {
            IntPtr cfsContinueText,
                cfsFullMessage,
                cfsTitle;
            ...
            cfsFullMessage = SafeNativeMethods.CFStringCreateWithCharacters(
                IntPtr.Zero,
                fullMessage,
                fullMessage.Length
            );
            cfsContinueText = SafeNativeMethods.CFStringCreateWithCharacters(
                IntPtr.Zero,
                SR.GetString(SR.ContinueButtonText),
                SR.GetString(SR.ContinueButtonText).Length
            );
            cfsTitle = SafeNativeMethods.CFStringCreateWithCharacters(
                IntPtr.Zero,
                SR.GetString(SR.DebugAssertTitleShort),
                SR.GetString(SR.DebugAssertTitleShort).Length
            );

            uint rval = 0;

            SafeNativeMethods.CFUserNotificationDisplayAlert(
                0,
                0,
                IntPtr.Zero,
                IntPtr.Zero,
        ...
        [SuppressMessage(
            ...
            MessageId = "Microsoft.Win32.UnsafeNativeMethods.ReleaseDC(System.IntPtr,System.IntPtr)",
            Justification = "Microsoft: If the DC is not released there's not much we can do."
        )]
