...
namespace System.Diagnostics
{
    ...
    public static class Debug
    {
        ...
        {
            TraceInternal.Assert(
                condition,
                message,
                String.Format(CultureInfo.InvariantCulture, detailMessageFormat, args)
            );
        }
        ...
        public static void Assert(bool condition, string message, string detailMessage)
        {
            if (!condition)
            {
                ...
                AssertWrapper.ShowAssert(
                    stackTrace,
                    stack.GetFrame(userStackFrameIndex),
                    message,
                    detailMessage
                );
            }
        ...
        private static void WriteAssert(string stackTrace, string message, string detailMessage)
        {
            string assertMessage =
                SR.GetString(SR.DebugAssertBanner)
                + Environment.NewLine
        ...
        public static void Write(string message)
        {
            ...
            lock (s_ForLock)
            {
                ...
                else
                {
                    ...
                    for (
                        offset = 0;
                        offset < message.Length - internalWriteSize;
                        offset += internalWriteSize
                    )
                    {
