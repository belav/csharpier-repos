...
namespace System.Diagnostics
{
    ...
    public class XmlWriterTraceListener : TextWriterTraceListener
    {
        ...
        public override void WriteLine(string message)
        {
            this.TraceEvent(
                null,
                SR.GetString(SR.TraceAsTraceSource),
        ...
        private void WriteData(object data)
        {
            ...
            else
            {
                if (strBldr == null)
                {
                    ...
                    xmlBlobWriter = new XmlTextWriter(
                        new StringWriter(strBldr, CultureInfo.CurrentCulture)
                    );
                }
                else
                    strBldr.Length = 0;
        ...
        {
            if (
                shouldRespectFilterOnTraceTransfer
                && (
                    Filter != null
                    && !Filter.ShouldTrace(eventCache, source, TraceEventType.Transfer, id, message)
                )
            )
                return;
        ...
        private void WriteEndHeader(TraceEventCache eventCache)
        {
            ...
            InternalWrite(
                ((uint)TraceEventCache.GetProcessId()).ToString(CultureInfo.InvariantCulture)
            );
