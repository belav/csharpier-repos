...
namespace System.Collections.Generic
{
    ...
    [SuppressMessage(
        ...
        Justification = "by design name choice"
    )]
    ...
    public class SortedSet<T> : ISet<T>, ICollection<T>, ICollection, IReadOnlyCollection<T>
    {
            ...
            : this(comparer)
        {
            ...
            if (
                baseSortedSet != null
                && baseTreeSubSet == null
                && AreComparersEqual(this, baseSortedSet)
            )
            {
                ...
                Stack<Node> theirStack = new Stack<SortedSet<T>.Node>(
                    2 * log2(baseSortedSet.Count) + 2
                );
                Stack<Node> myStack = new Stack<SortedSet<T>.Node>(
                    2 * log2(baseSortedSet.Count) + 2
                );
                Node theirCurrent = baseSortedSet.root;
                Node myCurrent = (
                    theirCurrent != null
                        ? new SortedSet<T>.Node(theirCurrent.Item, theirCurrent.IsRed)
                        : null
                );
                root = myCurrent;
                while (theirCurrent != null)
                {
                    ...
                    myCurrent.Left = (
                        theirCurrent.Left != null
                            ? new SortedSet<T>.Node(theirCurrent.Left.Item, theirCurrent.Left.IsRed)
                            : null
                    );
                ...
                while (theirStack.Count != 0)
                {
                    ...
                    while (theirRight != null)
                    {
                        ...
                        myRight.Left = (
                            theirRight.Left != null
                                ? new SortedSet<T>.Node(theirRight.Left.Item, theirRight.Left.IsRed)
                                : null
                        );
                        ...
                        else
                        {
                            ...
                            switch (rotation)
                            {
                                case TreeRotation.RightRotation:
                                    Debug.Assert(
                                        parent.Left == sibling,
                                        "sibling must be left child of parent!"
                                    );
                                    Debug.Assert(
                                        sibling.Left.IsRed,
        ...
        )
        {
            ...
            else
            {
                ...
                Debug.Assert(
                    (succesor.Right == null && succesor.IsRed)
                        || (succesor.Right.IsRed && !succesor.IsRed),
        ...
        )
        {
            ...
            else
            {
                ...
                if (size % 2 == 0)
                {
                    root.Right = ConstructRootFromSortedArray(
                        arr,
                        midpt + 2,
                        endIndex,
                        new Node(arr[midpt + 1], true)
                    );
                }
                else
                {
        ...
        public void ExceptWith(IEnumerable<T> other)
        {
            ...
            if (asSorted != null && AreComparersEqual(this, asSorted))
            {
                ...
                if (
                    !(
                        comparer.Compare(asSorted.Max, this.Min) < 0
                        || comparer.Compare(asSorted.Min, this.Max) > 0
                    )
                )
                {
                    T min = this.Min;
        ...
        public void SymmetricExceptWith(IEnumerable<T> other)
        {
            ...
            if (asSorted != null && AreComparersEqual(this, asSorted))
            {
                ...
                asHash != null
                && this.comparer.Equals(Comparer<T>.Default)
                && asHash.Comparer.Equals(EqualityComparer<T>.Default)
            )
            {
        ...
        internal sealed class TreeSubSet : SortedSet<T>
        {
            ...
            [SuppressMessage(
                ...
                Justification = "special case TreeSubSet serialization"
            )]
...
#if DEBUG
                Debug.Assert(
                    this.versionUpToDate() && this.root == this.underlying.FindRange(min, max)
                );
#endif
            ...
            internal override bool BreadthFirstTreeWalk(TreeWalkPredicate<T> action)
            {
                ...
                while (processQueue.Count != 0)
                {
                    ...
                    if (
                        current.Left != null
                        && (!lBoundActive || Comparer.Compare(min, current.Item) < 0)
                    )
                    {
                        processQueue.Add(current.Left);
                    }
                    if (
                        current.Right != null
                        && (!uBoundActive || Comparer.Compare(max, current.Item) > 0)
                    )
                    {
                        processQueue.Add(current.Right);
                    }
            ...
            private void OnDeserializationImpl(Object sender)
            {
                ...
                if (savedCount != 0)
                {
                    ...
                    {
                        ThrowHelper.ThrowSerializationException(
                            ExceptionResource.Serialization_MissingValues
                        );
                    }
                ...
                {
                    ThrowHelper.ThrowSerializationException(
                        ExceptionResource.Serialization_MismatchedCount
                    );
                }
                siInfo = null;
            }
        ...
        public struct Enumerator : IEnumerator<T>, IEnumerator
        {
            ...
            private void GetObjectData(SerializationInfo info, StreamingContext context)
            {
                ...
                info.AddValue(
                    NodeValueName,
                    (current == null ? dummyNode.Item : current.Item),
                    typeof(T)
                );
            }
