...
namespace System.Configuration
{
    ...
    internal sealed class RuntimeConfigurationRecord : BaseConfigurationRecord
    {
        ...
        [SuppressMessage(
            ...
            Justification = "This PermitOnly is meant to protect unassuming handlers from malicious callers by undoing any asserts we have put on the stack."
        )]
        ...
        [PermissionSet(SecurityAction.Assert, Unrestricted = true)]
        protected override string CallHostDecryptSection(
            string encryptedXml,
        ...
        private class RuntimeConfigurationFactory
        {
            ...
            private void Init(RuntimeConfigurationRecord configRecord, FactoryRecord factoryRecord)
            {
                ...
                {
                    _sectionCtor = TypeUtil.GetConstructorWithReflectionPermission(
                        type,
            ...
            // Ignore user code on the stack
            [PermissionSet(SecurityAction.Assert, Unrestricted = true)]
            private void InitWithFullTrust(
                RuntimeConfigurationRecord configRecord,
                FactoryRecord factoryRecord
            )
            {
            ...
            private void InitWithRestrictedPermissions(
                RuntimeConfigurationRecord configRecord,
                FactoryRecord factoryRecord
            )
            {
            ...
            private static void CheckForLockAttributes(string sectionName, XmlNode xmlNode)
            {
                ...
                if (attributes != null)
                {
                    foreach (XmlAttribute attribute in attributes)
                    {
                        ...
                        {
                            throw new ConfigurationErrorsException(
                                SR.GetString(SR.Config_element_locking_not_supported, sectionName),
                                attribute
                            );
                        }
                    }
            ...
            )
            {
                ...
                if (_sectionCtor != null)
                {
                    ...
                    if (
                        configRecord != null
                        && sectionInput != null
                        && sectionInput.ConfigBuilder != null
                    )
                    {
                        configSection = configRecord.CallHostProcessConfigurationSection(
                            configSection,
            ...
            )
            {
                if (inputIsTrusted)
                {
                    return CreateSectionWithFullTrust(
                        configRecord,
                        factoryRecord,
                        sectionRecord,
                        sectionInput,
                ...
                {
                    return CreateSectionWithRestrictedPermissions(
                        configRecord,
                        factoryRecord,
                        sectionRecord,
                        sectionInput,
