...
#pragma warning restore 0618
        ...
        internal ConfigurationErrorsException(string message, IConfigErrorInfo errorInfo)
            : this(
                message,
                null,
                GetUnsafeConfigErrorInfoFilename(errorInfo),
            ...
            if (count != 0)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    _errors[i] = (ConfigurationException)
                        info.GetValue(
                            numPrefix + SERIALIZATION_PARAM_ERROR_DATA,
        ...
        public override void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            if ((_errors != null) && (_errors.Length > 1))
            {
                ...
                for (int i = 0; i < _errors.Length; i++)
                {
                    ...
                    info.AddValue(numPrefix + SERIALIZATION_PARAM_ERROR_DATA, _errors[i]);
                    info.AddValue(numPrefix + SERIALIZATION_PARAM_ERROR_TYPE, _errors[i].GetType());
                }
            }
        ...
        public override string Message
        {
            get
            {
                ...
                else if (Line != 0)
                {
                    return BareMessage
                        + " (line "
                        + Line.ToString("G", CultureInfo.CurrentCulture)
                        + ")";
                }
                else
                {
        ...
        {
            get { return SafeFilename(_firstFilename); }
        }
        ...
        {
            get { return (ICollection<ConfigurationException>)this.Errors; }
        }
        ...
        [SuppressMessage(
            "Microsoft.Security",
            "CA2106:SecureAsserts",
            Justification = "This method simply extracts the filename, which isn't sensitive information."
        )]
