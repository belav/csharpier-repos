    ...
    internal sealed class ClientConfigurationHost : DelegatingConfigHost, IInternalConfigClientHost
    {
        ...
        internal static string MachineConfigFilePath
        {
            ...
            [SuppressMessage(
                ...
                Justification = "The callers do not expose this information without performing the appropriate demands themselves."
            )]
            get
            {
                if (s_machineConfigFilePath == null)
                {
                    string directory = System.Runtime
                        .InteropServices
                        .RuntimeEnvironment
                        .GetRuntimeDirectory();
        ...
        {
            get { return !IsFile(GetStreamName(ExeConfigPath)); }
        }
        ...
        )
        {
            if (!IsDefinitionAllowed(configPath, allowDefinition, allowExeDefinition))
            {
                switch (allowExeDefinition)
                {
                    case ConfigurationAllowExeDefinition.MachineOnly:
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Config_allow_exedefinition_error_machine),
                            errorInfo
                        );

                    case ConfigurationAllowExeDefinition.MachineToApplication:
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Config_allow_exedefinition_error_application),
                            errorInfo
                        );

                    case ConfigurationAllowExeDefinition.MachineToRoamingUser:
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Config_allow_exedefinition_error_roaminguser),
                            errorInfo
                        );
        ...
        [SecurityPermissionAttribute(
            SecurityAction.Assert,
            Flags = SecurityPermissionFlag.ControlPrincipal | SecurityPermissionFlag.UnmanagedCode
        )]
