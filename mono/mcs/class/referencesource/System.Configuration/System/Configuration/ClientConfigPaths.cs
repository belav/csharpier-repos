...
namespace System.Configuration
{
    ...
    class ClientConfigPaths
    {
        ...
        [FileIOPermission(
            ...
            AllFiles = FileIOPermissionAccess.PathDiscovery | FileIOPermissionAccess.Read
        )]
        ...
        private ClientConfigPaths(string exePath, bool includeUserConfig)
        {
            ...
            if (exePath == null)
            {
                ...
                else
                {
                    ...
                    while (
                        (
                            (
                                length = UnsafeNativeMethods.GetModuleFileName(
                    ...
                    )
                    {
                        ...
                        int capacity =
                            noOfTimes * MAX_PATH < MAX_UNICODESTRING_LEN
                                ? noOfTimes * MAX_PATH
            ...
            if (this.IsClickOnceDeployed(AppDomain.CurrentDomain))
            {
                ...
                if (Path.IsPathRooted(dataPath))
                {
                    ...
                    _localConfigFilename = CombineIfValid(
                        _localConfigDirectory,
                        UserConfigFilename
                    );
                }
            }
            else if (!isHttp)
            {
                ...
                string applicationUriLower = !String.IsNullOrEmpty(_applicationUri)
                    ? _applicationUri.ToLower(CultureInfo.InvariantCulture)
                    : null;
                string namePrefix = !String.IsNullOrEmpty(validAppDomainName)
                    ? validAppDomainName
                    : Validate(_productName, true);
                string hashSuffix = GetTypeAndHashSuffix(
                ...
                if (Path.IsPathRooted(roamingFolderPath))
                {
                    ...
                    _roamingConfigFilename = CombineIfValid(
                        _roamingConfigDirectory,
                        UserConfigFilename
                    );
                }
        ...
        {
            get { return GetPaths(null, true); }
        }

        internal bool HasEntryAssembly
        {
            get { return _hasEntryAssembly; }
        }

        internal string ApplicationUri
        {
            get { return _applicationUri; }
        }

        internal string ApplicationConfigUri
        {
            get { return _applicationConfigUri; }
        }

        internal string RoamingConfigFilename
        {
            get { return _roamingConfigFilename; }
        }

        internal string RoamingConfigDirectory
        {
            get { return _roamingConfigDirectory; }
        }
        ...
        {
            get { return _localConfigFilename; }
        }

        internal string LocalConfigDirectory
        {
            get { return _localConfigDirectory; }
        }
        ...
        private static SecurityPermission ControlEvidencePermission
        {
            get
            {
                ...
                {
                    s_controlEvidencePerm = new SecurityPermission(
        ...
        private static SecurityPermission SerializationFormatterPermission
        {
            get
            {
                ...
                {
                    s_serializationPerm = new SecurityPermission(
        ...
        private static object GetEvidenceInfo(
            AppDomain appDomain,
            string exePath,
            out string typeName
        )
        ...
        private void SetNamesAndVersion(
        ...
        )
        {
            ...
            if (
                ...
                && (
                    ...
                    || String.IsNullOrEmpty(_productVersion)
                )
            )
            {
                ...
                if (versionInfoFileName != null)
                {
                    System.Diagnostics.FileVersionInfo version =
                        System.Diagnostics.FileVersionInfo.GetVersionInfo(versionInfoFileName);
                    if (version != null)
                    {
        ...
        private string Validate(string str, bool limitSize)
        {
            ...
            if (!String.IsNullOrEmpty(validated))
            {
                ...
                {
                    validated =
                        (validated.Length > MAX_LENGTH_TO_USE)
                            ? validated.Substring(0, MAX_LENGTH_TO_USE)
                            : validated;
                }
            }
