    ...
    public abstract class ConfigurationElementCollection : ConfigurationElement, ICollection
    {
            ...
            set
            {
                ...
                if (BaseConfigurationRecord.IsReservedAttributeName(value))
                {
                    throw new ArgumentException(
                        SR.GetString(SR.Item_name_reserved, DefaultAddItemName, value)
                    );
                }
            }
        }
        ...
        protected internal string RemoveElementName
        {
            ...
            set
            {
                ...
                {
                    throw new ArgumentException(
                        SR.GetString(SR.Item_name_reserved, DefaultRemoveItemName, value)
                    );
                }
        ...
        protected internal string ClearElementName
        {
            ...
            set
            {
                ...
                {
                    throw new ArgumentException(
                        SR.GetString(SR.Item_name_reserved, DefaultClearItemsName, value)
                    );
                }
                _clearElement = value;
            }
                                    ...
                                    if (saveMode == ConfigurationSaveMode.Modified)
                                    {
                                        if (elem.IsModified())
                                        {
                                            Inheritance[entry.GetKey(this)] =
                                                InheritedType.inBothDiff;
                                        }
                                        else if (elem.ElementPresent)
                                        {
        ...
        public int Count
        {
            get { return _items.Count - _removedItemCount; }
        }

        public bool EmitClear
        {
            get { return bEmitClearTag; }
            set
            {
                if (IsReadOnly())
                {
        ...
        private void BaseAdd(ConfigurationElement element, bool throwIfExists, bool ignoreLocks)
        {
            ...
            if (LockItem == true && ignoreLocks == false)
            {
                throw new ConfigurationErrorsException(
                    SR.GetString(SR.Config_base_element_locked, _addElement)
                );
            }
            ...
            for (int index = 0; index < _items.Count; index++)
            {
                ...
                if (CompareKeys(key, entry.GetKey(this)))
                {
                    if (
                        entry._value != null
                        && entry._value.LockItem == true
                        && ignoreLocks == false
                    )
                    {
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Config_base_collection_item_locked)
                        );
                    }
                    if (entry._entryType != EntryType.Removed && throwIfExists)
                    {
                        ...
                        {
                            throw new ConfigurationErrorsException(
                                SR.GetString(SR.Config_base_collection_entry_already_exists, key),
        ...
        )
        {
            ...
            { // during reset we ignore locks so we can copy the elements
                ...
                )
                {
                    ...
                    {
                        throw new ArgumentException(
                            SR.GetString(SR.Basicmap_item_name_reserved, ElementName)
                        );
                    }
                    ...
                    )
                    {
                        throw (
                            new ConfigurationErrorsException(
                                SR.GetString(SR.Config_base_cannot_add_items_below_inherited_items)
                            )
                        );
                    }
                }
            ...
            )
            {
                throw (
                    new ConfigurationErrorsException(
                        SR.GetString(SR.Config_base_cannot_add_items_above_inherited_items)
                    )
                );
            }
                    ...
                    if (CompareKeys(key, entry.GetKey(this)))
                    {
                        if (entry._entryType != EntryType.Removed)
                        {
                            if (!element.Equals(entry._value))
                            {
                                throw new ConfigurationErrorsException(
                                    SR.GetString(
                                        SR.Config_base_collection_entry_already_exists,
                                        key
                                    ),
                                    element.PropertyFileName(""),
                                    element.PropertyLineNumber("")
            ...
            foreach (Entry entry in _items)
            {
                if (CompareKeys(key, entry.GetKey(this)))
                {
                    ...
                    if (entry._value == null) // A phoney delete is already present
                    {
                        ...
                        {
                            throw new ConfigurationErrorsException(
                                SR.GetString(SR.Config_base_collection_entry_not_found, key)
                            );
                        }
                        else
                        {
                    ...
                    {
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Config_base_attribute_locked, key)
                        );
                    }
                    ...
                    switch (entry._entryType)
                    {
                        ...
                        case EntryType.Removed:
                            ...
                            {
                                throw new ConfigurationErrorsException(
                                    SR.GetString(SR.Config_base_collection_entry_already_removed)
                                );
                            }
                            break;
                        default:
        ...
        protected internal void BaseClear()
        {
            ...
            else
            {
                ...
                for (int CheckIndex = 0; CheckIndex < _items.Count; CheckIndex++)
                {
                    ...
                    {
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Config_base_collection_item_locked_cannot_clear)
                        );
                    }
                }
        ...
        protected internal void BaseRemoveAt(int index)
        {
            ...
            else
            {
                ...
                {
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Config_base_attribute_locked, entry.GetKey(this))
                    );
                }
                ...
                switch (entry._entryType)
                {
                    ...
                    default:
                        ...
                        {
                            throw new ConfigurationErrorsException(
                                SR.GetString(SR.Config_base_collection_elements_may_not_be_removed)
                            );
                        }
        ...
        )
        {
            ...
            {
                if (
                    type == ConfigurationElementCollectionType.BasicMap
                ...
                else if (elementName == _clearElement)
                {
                    if (reader.AttributeCount > 0)
                    {
                        while (reader.MoveToNextAttribute())
                        {
                            ...
                            throw new ConfigurationErrorsException(
                                SR.GetString(SR.Config_base_unrecognized_attribute, propertyName),
                                reader
                            );
                        }
                    }
        ...
        internal Object GetElementKeyInternal(ConfigurationElement element)
        {
            ...
            if (key == null)
                throw new ConfigurationErrorsException(
                    SR.GetString(SR.Config_base_invalid_element_key)
                );
            return key;
        }
        ...
        private class Enumerator : IDictionaryEnumerator
        {
            ...
            bool IEnumerator.MoveNext()
            {
                while (_itemsEnumerator.MoveNext())
                {
                    ...
                    {
                        _current.Key =
                            (entry.GetKey(ThisCollection) != null)
                                ? entry.GetKey(ThisCollection)
                                : "key";
