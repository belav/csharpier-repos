...
namespace System.ServiceModel.Routing
{
            ...
            IDisposable
    {
        ...
        [SuppressMessage(
            ...
            Justification = "private setter does get called"
        )]
        internal RoutingChannelExtension ChannelExtension { get; private set; }
        ...
        internal RoutingConfiguration RoutingConfig { get; private set; }

        internal CommittableTransaction RetryTransaction { get; private set; }

        internal Exception SessionException { get; set; }
        ...
        internal IList<MessageRpc> SessionMessages { get; private set; }

        Transaction ReceiveTransaction { get; set; }
        ...
        internal void CreateNewTransactionIfNeeded(MessageRpc messageRpc)
        {
            if (messageRpc.Transaction != null && this.ChannelExtension.TransactedReceiveEnabled)
            {
                ...
                {
                    TD.RoutingServiceUsingExistingTransaction(
                        messageRpc.EventTraceActivity,
                        messageRpc.Transaction.TransactionInformation.LocalIdentifier
                    );
                }
                Fx.Assert(
                    this.ReceiveTransaction == null,
                    "Should only happen at the start of a session."
                );
            ...
            Fx.Assert(
                ...
                "Logic error, we shouldn't be calling CreateNewTransactionIfNeeded if we have a RC Transaction"
            );
            ...
            {
                TD.RoutingServiceCreatingTransaction(
                    messageRpc.EventTraceActivity,
                    this.RetryTransaction.TransactionInformation.LocalIdentifier
                );
            }
        }
        ...
        IAsyncResult BeginProcessMessage<TContract>(
            Message message,
            AsyncCallback callback,
            object state
        )
        {
            try
            {
                ...
                return new ProcessMessagesAsyncResult<TContract>(
                    message,
                    this,
                    this.ChannelExtension.OperationTimeout,
                    callback,
            ...
            catch (Exception exception)
            {
                ...
                {
                    TD.RoutingServiceProcessingFailure(
        ...
        IAsyncResult BeginProcessRequest<TContract>(
            Message message,
            AsyncCallback callback,
            object state
        )
