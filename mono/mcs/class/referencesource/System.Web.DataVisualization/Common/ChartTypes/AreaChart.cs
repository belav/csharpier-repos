...
#endif
{
    ...
    internal class AreaChart : SplineChart
    {
        ...
        )
        {
                    ...
                    using (
                        Brush shadowBrush = new SolidBrush(
                            (series.ShadowColor.A != 255)
                                ? series.ShadowColor
                                : Color.FromArgb(pointColor.A / 2, series.ShadowColor)
                        )
                    )
                    {
            ...
            {
                Pen pen = new Pen(
                    (pointBorderColor != Color.Empty) ? pointBorderColor : pointColor,
                    pointBorderWidth
                );
            ...
            if (common.ProcessModeRegions)
            {
                ...
                if (
                    pointBorderWidth > 1
                    && pointBorderDashStyle != ChartDashStyle.NotSet
        ...
        )
        {
            ...
            //**********************************************************************
            if (
                Math.Round(firstPoint.xPosition, 3) == Math.Round(secondPoint.xPosition, 3)
            ...
            if (
                ((decimal)firstPoint.yPosition) >= ((decimal)axisPosition)
            ...
            if (
                (
                    Math.Round((decimal)firstPoint.yPosition, 3) > (decimal)axisPosition + 0.001M
                    && Math.Round((decimal)secondPoint.yPosition, 3)
                        < (decimal)axisPosition - 0.001M
                )
                || (
                    Math.Round((decimal)firstPoint.yPosition, 3) < (decimal)axisPosition - 0.001M
                    && Math.Round((decimal)secondPoint.yPosition, 3)
            ...
            )
            {
                ...
                DataPoint3D intersectionPoint = GetAxisIntersection(
                    firstPoint,
                    secondPoint,
                    axisPosition
                );
            ...
            for (int elemLayer = 1; elemLayer <= 2; elemLayer++)
            {
                ...
                SurfaceNames[] surfacesOrder = new SurfaceNames[]
                {
                    SurfaceNames.Back,
                    SurfaceNames.Bottom,
                    SurfaceNames.Top,
                ...
                foreach (SurfaceNames currentSurface in surfacesOrder)
                {
                    ...
                    )
                    {
                        ...
                        {
                            if (
                                currentSurface == SurfaceNames.Back
                                || currentSurface != SurfaceNames.Front
                            )
                            {
                                if (elemLayer == 1)
                    ...
                    switch (currentSurface)
                    {
                        ...
                        case (SurfaceNames.Back):
                        {
                            ...
                            // Draw surface
                            surfacePath = graph.Draw3DPolygon(
                                area,
                                matrix,
                                currentSurface,
                                positionZ,
        ...
        internal DataPoint3D GetAxisIntersection(
            DataPoint3D firstPoint,
            DataPoint3D secondPoint,
            float axisPosition
        )
