    ...
    internal class RenkoChart : IChartType
    {
        ...
        internal static void PrepareData(Series series)
        {
            ...
            Series seriesOriginalData = new Series(
                "RENKO_ORIGINAL_DATA_" + series.Name,
        ...
        internal static bool UnPrepareData(Series series)
        {
            if (series.Name.StartsWith("RENKO_ORIGINAL_DATA_", StringComparison.Ordinal))
            {
                ...
                parseSucceed = int.TryParse(
                    renkoSeries["OldYValuesPerPoint"],
                    NumberStyles.Any,
                    CultureInfo.InvariantCulture,
                    out yValsPerPoint
                );
                ...
                if (renkoSeries.IsCustomPropertySet("OldAutomaticXAxisInterval"))
                {
                    ...
                    if (renkoSeries.ChartArea.Length > 0)
                    {
                        ...
                        Axis xAxis = area.GetAxis(
                            AxisName.X,
                            renkoSeries.XAxisType,
                            renkoSeries.XSubAxisName
                        );
            ...
            foreach (DataPoint dataPoint in originalData.Points)
            {
                if (!dataPoint.IsEmpty)
                {
                    ...
                    if (upBrickColorString != null)
                    {
                        ...
                        {
                            throw (
                                new InvalidOperationException(SR.ExceptionRenkoUpBrickColorInvalid)
                            );
                        }
                    }
                    ...
                    if (dataPoint.YValues[yValueIndex] >= (prevHigh + boxSize))
                    {
                        goingUp = true;
                        numberOfBricks = (int)
                            Math.Floor((dataPoint.YValues[yValueIndex] - prevHigh) / boxSize);
                    }
                    else if (dataPoint.YValues[yValueIndex] <= (prevLow - boxSize))
                    {
                        goingUp = false;
                        numberOfBricks = (int)
                            Math.Floor((prevLow - dataPoint.YValues[yValueIndex]) / boxSize);
                    }
