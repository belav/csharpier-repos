...
#endif
{
    ...
    internal class PieChart : IChartType
    {
        ...
        internal static void PrepareData(Series series)
        {
            ...
            if (series.IsCustomPropertySet(CustomPropertyName.CollectedThreshold))
            {
                ...
                bool parseSucceed = double.TryParse(
                    series[CustomPropertyName.CollectedThreshold],
                    NumberStyles.Any,
                ...
                else
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionDoughnutCollectedThresholdInvalidFormat
                        )
                    );
                }
            ...
            if (threshold > 0.0)
            {
                ...
                Series seriesOriginalData = new Series(
                    "PIE_ORIGINAL_DATA_" + series.Name,
                ...
                if (series.IsCustomPropertySet(CustomPropertyName.CollectedThresholdUsePercent))
                {
                    if (
                        string.Compare(
                            series[CustomPropertyName.CollectedThresholdUsePercent],
                            "True",
                            StringComparison.OrdinalIgnoreCase
                        ) == 0
                    )
                    ...
                    else
                    {
                        throw (
                            new InvalidOperationException(
                                SR.ExceptionDoughnutCollectedThresholdUsePercentInvalid
                            )
                        );
                    }
                }
                ...
                if (percent)
                {
                    if (threshold > 100.0)
                    {
                        throw (
                            new InvalidOperationException(
                                SR.ExceptionDoughnutCollectedThresholdInvalidRange
                            )
                        );
                    }
                ...
                for (int dataPointIndex = 0; dataPointIndex < series.Points.Count; dataPointIndex++)
                {
                    ...
                    dataPoint["OriginalPointIndex"] = originalDataPointIndex.ToString(
                        CultureInfo.InvariantCulture
                    );
                ...
                if (collectedCount > 1 && collectedPoint != null)
                {
                    ...
                    if (series.IsCustomPropertySet(CustomPropertyName.CollectedColor))
                    {
                        ...
                        try
                        {
                            collectedPoint.Color = (Color)
                                colorConverter.ConvertFromString(
                                    null,
                                    CultureInfo.InvariantCulture,
                                    series[CustomPropertyName.CollectedColor]
                                );
                        }
                        catch
                        {
                            throw (
                                new InvalidOperationException(
                                    SR.ExceptionDoughnutCollectedColorInvalidFormat
                                )
                            );
                        }
                    }
                ...
                if (series.IsVisible() && series.ChartArea == area.Name)
                {
                    ...
                    )
                    {
                        if (!common.ChartPicture.SuppressExceptions)
                        {
                            ...
                            throw (
                                new InvalidOperationException(
                                    SR.ExceptionChartCanNotCombine(this.Name)
                                )
                            );
                        }
                    }
        ...
        private double MinimumRelativePieSize(ChartArea area)
        {
            ...
            if (
                dataSeries[typeSeries[0]]
                    .IsCustomPropertySet(CustomPropertyName.MinimumRelativePieSize)
            )
            {
                minimumSize =
                    CommonElements.ParseFloat(
                        dataSeries[typeSeries[0]][CustomPropertyName.MinimumRelativePieSize]
                    ) / 100.0;
        ...
        )
        {
            ...
            if (typeSeries.Count > 0)
            {
                if (dataSeries[typeSeries[0]].IsCustomPropertySet(CustomPropertyName.PieStartAngle))
                {
                    ...
                    bool parseSucceed = float.TryParse(
                        dataSeries[typeSeries[0]][CustomPropertyName.PieStartAngle],
                        NumberStyles.Any,
                        CultureInfo.InvariantCulture,
                        out angle
                    );
                    ...
                    if (!parseSucceed || startAngle > 360f || startAngle < 0f)
                    {
                        throw (
                            new InvalidOperationException(
                                SR.ExceptionCustomAttributeAngleOutOfRange("PieStartAngle")
                            )
                        );
                    }
                }
            ...
            {
                doughnutRadius = CommonElements.ParseFloat(
                    dataSeries[typeSeries[0]][CustomPropertyName.DoughnutRadius]
                );
            ...
            foreach (DataPoint point in dataSeries[typeSeries[0]].Points)
            {
                ...
                if (area.InnerPlotPosition.Auto)
                {
                    rectangle = new RectangleF(
                        area.Position.ToRectangleF().X,
                        area.Position.ToRectangleF().Y,
                        area.Position.ToRectangleF().Width,
                ...
                {
                    rectangle = new RectangleF(
                        area.PlotAreaPosition.ToRectangleF().X,
                        area.PlotAreaPosition.ToRectangleF().Y,
                        area.PlotAreaPosition.ToRectangleF().Width,
                ...
                // Find smallest edge
                SizeF absoluteSize = graph.GetAbsoluteSize(
                    new SizeF(rectangle.Width, rectangle.Height)
                );
                float absRadius =
                    (absoluteSize.Width < absoluteSize.Height)
                        ? absoluteSize.Width
                        : absoluteSize.Height;
                ...
                rectangle = new RectangleF(
                    middlePoint.X - relativeSize.Width / 2,
                    middlePoint.Y - relativeSize.Height / 2,
                    relativeSize.Width,
                    relativeSize.Height
                );
                ...
                if (point.IsCustomPropertySet(CustomPropertyName.Exploded))
                {
                    ...
                    if (
                        String.Compare(explodedAttrib, "true", StringComparison.OrdinalIgnoreCase)
                        == 0
                    )
                        exploded = true;
                    else
                ...
                if (
                    point.IsCustomPropertySet(CustomPropertyName.PieLineColor)
                    || dataSeries[typeSeries[0]]
                ...
                )
                {
                    ...
                    try
                    {
                        pieLineColor = (Color)
                            colorConverter.ConvertFromString(
                                (point.IsCustomPropertySet(CustomPropertyName.PieLineColor))
                                    ? point[CustomPropertyName.PieLineColor]
                                    : dataSeries[typeSeries[0]][CustomPropertyName.PieLineColor]
                            );
                        failed = false;
                    }
                    ...
                    {
                        pieLineColor = (Color)
                            colorConverter.ConvertFromInvariantString(
                                (point.IsCustomPropertySet(CustomPropertyName.PieLineColor))
                                    ? point[CustomPropertyName.PieLineColor]
                                    : dataSeries[typeSeries[0]][CustomPropertyName.PieLineColor]
                            );
                ...
                if (common.ProcessModePaint)
                {
                    ...
                    if (shadow)
                    {
                        double offset = graph
                            .GetRelativeSize(
                                new SizeF(point.series.ShadowOffset, point.series.ShadowOffset)
                            )
                            .Width;
                        ...
                        if (!point.BackSecondaryColor.IsEmpty)
                        {
                            if (point.BackSecondaryColor.A != 255)
                                shGradientColor = Color.FromArgb(
                                    point.BackSecondaryColor.A / 2,
                ...
                point.positionRel.X =
                    (float)Math.Cos((midAngle) * Math.PI / 180) * rectangle.Width * expShift / 2
                    + middlePoint.X;
                point.positionRel.Y =
                    (float)Math.Sin((midAngle) * Math.PI / 180) * rectangle.Height * expShift / 2
                    + middlePoint.Y;
        ...
        )
        {
            ...
            if (series.IsCustomPropertySet(CustomPropertyName.LabelStyle))
            {
                ...
                // Labels Disabled
                if (
                    String.Compare(labelStyleAttrib, "disabled", StringComparison.OrdinalIgnoreCase)
                    == 0
                )
                    style = PieLabelStyle.Disabled;
                else if (
                    String.Compare(labelStyleAttrib, "outside", StringComparison.OrdinalIgnoreCase)
                    == 0
                )
            ...
            if (series.IsCustomPropertySet(CustomPropertyName.LabelsHorizontalLineSize))
            {
                ...
                labelsHorizontalLineSize = CommonElements.ParseFloat(
                    labelsHorizontalLineSizeAttrib
                );
            ...
            if (style == PieLabelStyle.Inside && !overlapTest)
            {
                ...
                if (point.IsCustomPropertySet("InsideLabelOffset"))
                {
                    ...
                    if (!parseSucceed || positionRatio < 0f || positionRatio > 100f)
                    {
                        throw (
                            new InvalidOperationException(
                                SR.ExceptionCustomAttributeIsNotInRange0to100("InsideLabelOffset")
                            )
                        );
                    }
                ...
                x =
                    (float)Math.Cos((startAngle + sweepAngle / 2) * Math.PI / 180) * width
                    + middlePoint.X;
                y =
                    (float)Math.Sin((startAngle + sweepAngle / 2) * Math.PI / 180) * height
                    + middlePoint.Y;
            ...
            else if (style == PieLabelStyle.Outside)
            {
                ...
                // Find first line position
                float x1 =
                    (float)Math.Cos((midAngle) * Math.PI / 180) * relativeSize.Width * expShift / 2
                    + middlePoint.X;
                float y1 =
                    (float)Math.Sin((midAngle) * Math.PI / 180) * relativeSize.Height * expShift / 2
                    + middlePoint.Y;

                float x2 =
                    (float)Math.Cos((midAngle) * Math.PI / 180)
                        * relativeSize.Width
                        * shift
                        * expShift
                    + middlePoint.X;
                float y2 =
                    (float)Math.Sin((midAngle) * Math.PI / 180)
                        * relativeSize.Height
                        * shift
                        * expShift
                    + middlePoint.Y;
                ...
                using (StringFormat format = new StringFormat())
                {
                    ...
                    if (midAngle > 90 && midAngle < 270)
                    {
                        ...
                        x3Overlap =
                            -relativeSize.Width * shift * expShift
                            + middlePoint.X
                        ...
                        labelRectOver = GetLabelRect(
                            new PointF(x3Overlap, y3),
                            area,
                            text,
                            format,
                            graph,
                    ...
                    if (!overlapTest)
                    {
                        if (this._labelsOverlap)
                        {
                            float calculatedY3 =
                                (
                                    ((RectangleF)this._labelsRectangles[pointIndex]).Top
                                    + ((RectangleF)this._labelsRectangles[pointIndex]).Bottom
                                ) / 2f;
                            graph.DrawLineRel(
                                pieLineColor,
                                point.BorderWidth,
                                ChartDashStyle.Solid,
                                new PointF(x2, y2),
                                new PointF(x3Overlap, calculatedY3)
                            );
                        }
                        else
                        {
                    ...
                    if (!overlapTest)
                    {
                        ...
                        // Get label background position
                        SizeF valueTextSize = graph.MeasureStringRel(
                            text.Replace("\\n", "\n"),
                            point.Font
                        );
        ...
        )
        {
            ...
            using (GraphicsPath path = new GraphicsPath())
            {
                ...
                doughnutRect.Y =
                    rectangle.Y + rectangle.Height * (1 - (100 - doughnutRadius) / 100) / 2;
        ...
        private void CheckPaleteColors(DataPointCollection points)
        {
            ...
            if (
                firstPoint.tempColorIsSet
                && lastPoint.tempColorIsSet
        ...
        private void SortIntervals(
            double[] startOfIntervals,
            double[] endOfIntervals,
            int[] positinIndex
        )
        {
            double firstCenter;
            ...
            for (int firstIndex = 0; firstIndex < startOfIntervals.Length; firstIndex++)
            {
                for (
                    ...
                    secondIndex < startOfIntervals.Length;
                    secondIndex++
                )
                {
                    firstCenter = (startOfIntervals[firstIndex] + endOfIntervals[firstIndex]) / 2.0;
                    secondCenter =
                        (startOfIntervals[secondIndex] + endOfIntervals[secondIndex]) / 2.0;
        ...
        private bool ArrangeOverlappingIntervals(
            double[] startOfIntervals,
            double[] endOfIntervals,
            double startArea,
            double endArea
        )
        {
            ...
            for (
                int intervalIndex = 0;
                intervalIndex < startOfIntervals.Length - 1;
                intervalIndex++
            )
            {
                // Check overlapping
            ...
            double outOfArea =
                (endOfIntervals[endOfIntervals.Length - 1] - endArea)
                + (startArea - startOfIntervals[0]);
            if (outOfArea <= 0)
            {
            ...
            ReduceEmptySpace(
                startOfIntervals,
                endOfIntervals,
                (emptySpace - outOfArea) / emptySpace
            );
        ...
        )
        {
            ...
            )
            {
                ...
                // Reduce space
                double shift =
                    (startOfIntervals[intervalIndex + 1] - endOfIntervals[intervalIndex])
                    - (startOfIntervals[intervalIndex + 1] - endOfIntervals[intervalIndex])
                        * reduction;
                for (
                    int reductionIndex = intervalIndex + 1;
                    reductionIndex < startOfIntervals.Length;
                    reductionIndex++
                )
                {
        ...
        )
        {
            ...
            {
                labelLineSize = CommonElements.ParseFloat(
                    dataSeries[typeSeries[0]][CustomPropertyName._3DLabelLineSize]
                );
            ...
            RectangleF plotingRectangle = new RectangleF(
                area.Position.ToRectangleF().X + 1,
                area.Position.ToRectangleF().Y + 1,
                area.Position.ToRectangleF().Width - 2,
                area.Position.ToRectangleF().Height - 2
            );
            ...
            for (int turn = 0; turn < 5; turn++)
            {
                ...
                foreach (DataPoint point in points)
                {
                    ...
                    if (exploded)
                    {
                        ...
                        midAngle = (2 * startAngle + sweepAngle) / 2;
                        double xComponent =
                            Math.Cos(midAngle * Math.PI / 180) * rectangle.Width / 10;
                        double yComponent =
                            Math.Sin(midAngle * Math.PI / 180) * rectangle.Height / 10;
        ...
        )
        {
            ...
            else if (turn == 2)
            {
                ...
                if (sameBackFront && sweepAngle > 180)
                {
                    ...
                    bool BackFrontDoughnut =
                        (
                            startAngle > -180 && startAngle < 0
                            || startAngle > 180 && startAngle < 360
                        ) && (endAngle > -180 && endAngle < 0 || endAngle > 180 && endAngle < 360);
            ...
            else if (turn == 3)
            {
                ...
                if (GetLabelStyle(point) == PieLabelStyle.Outside)
                {
                    ...
                    if (
                        ...
                        || (
                            point.series != null
                            && point.series.IsCustomPropertySet(CustomPropertyName.PieLineColor)
                        )
                    )
                    {
                        ColorConverter colorConverter = new ColorConverter();
                        ...
                        try
                        {
                            ...
                            )
                            {
                                pieLineColor = (Color)
                                    colorConverter.ConvertFromString(
                                        point.series[CustomPropertyName.PieLineColor]
                                    );
                            }
                        }
                        catch (ArgumentException)
        ...
        )
        {
            ...
            points[(int)PiePoints.Top180] = new Point3D(
                rectangle.X
                    + (float)Math.Cos(180 * Math.PI / 180) * rectangle.Width / 2F
                    + rectangle.Width / 2F,
                rectangle.Y
                    + (float)Math.Sin(180 * Math.PI / 180) * rectangle.Height / 2F
                    + rectangle.Height / 2F,
                pieWidth
            );
            ...
            points[(int)PiePoints.Top0] = new Point3D(
                rectangle.X
                    + (float)Math.Cos(0 * Math.PI / 180) * rectangle.Width / 2F
                    + rectangle.Width / 2F,
                rectangle.Y
                    + (float)Math.Sin(0 * Math.PI / 180) * rectangle.Height / 2F
                    + rectangle.Height / 2F,
                pieWidth
            );
            ...
            points[(int)PiePoints.TopStart] = new Point3D(
                rectangle.X
                    + (float)Math.Cos(startAngle * Math.PI / 180) * rectangle.Width / 2F
                    + rectangle.Width / 2F,
                rectangle.Y
                    + (float)Math.Sin(startAngle * Math.PI / 180) * rectangle.Height / 2F
                    + rectangle.Height / 2F,
                pieWidth
            );
            ...
            points[(int)PiePoints.TopEnd] = new Point3D(
                rectangle.X
                    + (float)Math.Cos((startAngle + sweepAngle) * Math.PI / 180)
                        * rectangle.Width
                        / 2F
                    + rectangle.Width / 2F,
                rectangle.Y
                    + (float)Math.Sin((startAngle + sweepAngle) * Math.PI / 180)
                        * rectangle.Height
                        / 2F
                    + rectangle.Height / 2F,
                pieWidth
            ...
            points[(int)PiePoints.TopLabelLine] = new Point3D(
                rectangle.X
                    + (float)Math.Cos((startAngle + sweepAngle / 2) * Math.PI / 180)
                        * rectangle.Width
                        / 2F
                    + rectangle.Width / 2F,
                rectangle.Y
                    + (float)Math.Sin((startAngle + sweepAngle / 2) * Math.PI / 180)
                        * rectangle.Height
                        / 2F
                    + rectangle.Height / 2F,
                pieWidth
            ...
            points[(int)PiePoints.TopRectTopLeftPoint] = new Point3D(
                rectangle.X,
                rectangle.Y,
                pieWidth
            );
            ...
            points[(int)PiePoints.TopRectBottomRightPoint] = new Point3D(
                rectangle.Right,
                rectangle.Bottom,
                pieWidth
            );
            ...
            points[(int)PiePoints.BottomRectTopLeftPoint] = new Point3D(
                rectangle.X,
                rectangle.Y,
                0
            );
            ...
            points[(int)PiePoints.BottomRectBottomRightPoint] = new Point3D(
                rectangle.Right,
                rectangle.Bottom,
                0
            );
            ...
            if (Doughnut)
            {
                ...
                points[(int)PiePoints.DoughnutTop180] = new Point3D(
                    rectangle.X
                        + (float)Math.Cos(180 * Math.PI / 180)
                            * rectangle.Width
                            * doughnutRadius
                            / 2F
                        + rectangle.Width / 2F,
                    rectangle.Y
                        + (float)Math.Sin(180 * Math.PI / 180)
                            * rectangle.Height
                            * doughnutRadius
                            / 2F
                        + rectangle.Height / 2F,
                ...
                points[(int)PiePoints.DoughnutTop0] = new Point3D(
                    rectangle.X
                        + (float)Math.Cos(0 * Math.PI / 180) * rectangle.Width * doughnutRadius / 2F
                        + rectangle.Width / 2F,
                    rectangle.Y
                        + (float)Math.Sin(0 * Math.PI / 180)
                            * rectangle.Height
                            * doughnutRadius
                            / 2F
                        + rectangle.Height / 2F,
                ...
                points[(int)PiePoints.DoughnutTopStart] = new Point3D(
                    rectangle.X
                        + (float)Math.Cos(startAngle * Math.PI / 180)
                            * rectangle.Width
                            * doughnutRadius
                            / 2F
                        + rectangle.Width / 2F,
                    rectangle.Y
                        + (float)Math.Sin(startAngle * Math.PI / 180)
                            * rectangle.Height
                            * doughnutRadius
                            / 2F
                        + rectangle.Height / 2F,
                ...
                rectangle.Inflate(
                    -rectangle.Width * (1 - doughnutRadius) / 2F,
                    -rectangle.Height * (1 - doughnutRadius) / 2F
                );

                // Doughnut Top Rectangle Top Left Point
                points[(int)PiePoints.DoughnutTopRectTopLeftPoint] = new Point3D(
                    rectangle.X,
                    rectangle.Y,
                    pieWidth
                );
                ...
                points[(int)PiePoints.DoughnutTopRectBottomRightPoint] = new Point3D(
                    rectangle.Right,
                    rectangle.Bottom,
                    pieWidth
                );
                ...
                points[(int)PiePoints.DoughnutBottomRectTopLeftPoint] = new Point3D(
                    rectangle.X,
                    rectangle.Y,
                    0
                );
                ...
                points[(int)PiePoints.DoughnutBottomRectBottomRightPoint] = new Point3D(
                    rectangle.Right,
                    rectangle.Bottom,
                    0
                );
            ...
            using (GraphicsPath path = new GraphicsPath())
            {
                ...
                else
                {
                    brush = graph.GetGradientBrush(
                        graph.GetAbsoluteRectangle(area.Position.ToRectangleF()),
                        Color.FromArgb(brushWithoutLight.Color.A, 0, 0, 0),
                        brushWithoutLight.Color,
        ...
        private DataPoint[] PointOrder(
            Series series,
            ChartArea area,
            out float[] newStartAngleList,
            out float[] newSweepAngleList,
            out int[] newPointIndexList,
            out bool sameBackFrontPoint
        )
        {
            double startAngle;
            double sweepAngle;
            ...
            foreach (DataPoint point in series.Points)
            {
                ...
                if (
                    ...
                    || startAngle <= 270 && endAngle > 270 && points[0] == null
                )
                {
                    /*if( points[0] != null )
                ...
                if (
                    ...
                    || startAngle <= 450
                        && endAngle > 450
                        && frontPoint == -1
                        && (points[points.Length - 1] == null || points.Length == 1)
                )
                {
                    /*
            ...
            if (frontPoint == backPoint)
            {
                ...
                if (rightSidePoints)
                {
                    SwitchPoints(
                        numOfPoints,
                        ref points,
                        ref newStartAngleList,
                        ref newSweepAngleList,
                        ref newPointIndexList,
        ...
        private void SwitchPoints(
            int numOfPoints,
            ref DataPoint[] points,
            ref float[] newStartAngleList,
            ref float[] newSweepAngleList,
            ref int[] newPointIndexList,
            bool sameBackFront
        )
        {
        ...
        internal class LabelColumn
        {
            ...
            internal PointF GetLabelPosition(DataPoint dataPoint)
            {
                ...
                angle = (float)
                    Math.Atan(
                        (position.Y - _innerPlotPosition.Top - _innerPlotPosition.Height / 2)
                            / (position.X - _innerPlotPosition.Left - _innerPlotPosition.Width / 2)
                    );
                ...
                {
                    correct = (float)(
                        _innerPlotPosition.Width * 0.4
                        - _innerPlotPosition.Width * 0.4 / Math.Cos(angle)
                    );
                }
        ...
        )
        {
            ...
            int maxNumOfRows = (int)(
                pieRectangle.Height / maxSizeVertical /*fontSize.Height*/
            );
        ...
        )
        {
            ...
            graph.DrawLine(
                pen,
                points[(int)PiePoints.TopLabelLine],
            ...
            using (StringFormat format = new StringFormat())
            {
                ...
                RectangleF chartAreaPosition = graph.GetAbsoluteRectangle(
                ...
                if (midAngle >= -90 && midAngle < 90 || midAngle >= 270 && midAngle < 450)
                {
                    ...
                    float labelVertSize = graph
                        .GetAbsoluteSize(new SizeF(0f, this.labelColumnRight.columnHeight))
                        .Height;
                ...
                else
                {
                    ...
                    float labelVertSize = graph
                        .GetAbsoluteSize(new SizeF(0f, this.labelColumnLeft.columnHeight))
                        .Height;
                    labelPoint = graph.GetAbsolutePoint(columnLabel.GetLabelPosition(point));
        ...
        private String GetPointLabel(DataPoint point)
        {
            ...
            if (point.Label.Length == 0)
            {
                ...
                if (
                    ...
                    && String.Equals(
                        point.series.GetCustomProperty(CustomPropertyName.PieAutoAxisLabels),
                        "false",
                        StringComparison.OrdinalIgnoreCase
                    )
                )
