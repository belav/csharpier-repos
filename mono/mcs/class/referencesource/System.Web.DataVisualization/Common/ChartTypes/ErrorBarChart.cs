...
#endif
{
    ...
    internal class ErrorBarChart : IChartType
    {
        ...
        )
        {
            ...
            foreach (Series ser in common.DataManager.Series)
            {
                ...
                if (ser.IsCustomPropertySet(CustomPropertyName.ErrorBarSeries))
                {
                    ...
                    List<string> typeLinkedSeries = linkedSeriesArea.GetSeriesFromChartType(
                ...
                if (
                    ...
                    && ser.IsCustomPropertySet(CustomPropertyName.DrawSideBySide)
                )
                {
                    ...
                    )
                    {
                        ...
                        sideBySideWidth =
                            (float)(ser.GetPointWidth(graph, hAxis, interval, 0.8))
                            / numberOfLinkedSeries;
                    }
                    else if (
                ...
                foreach (DataPoint point in ser.Points)
                {
                    ...
                    if (showSideBySide)
                    {
                        xPosition = (float)(
                            ...
                            - sideBySideWidth * ((double)numberOfLinkedSeries) / 2.0
                            + sideBySideWidth / 2
                            + indexOfLinkedSeries * sideBySideWidth
                        );
                    }
                    else
                    {
                    ...
                    if (
                        point.IsCustomPropertySet(CustomPropertyName.ErrorBarStyle)
                        || ser.IsCustomPropertySet(CustomPropertyName.ErrorBarStyle)
                    )
                    {
                            ...
                            throw (
                                new InvalidOperationException(
                                    SR.ExceptionCustomAttributeValueInvalid(
                                        point[CustomPropertyName.ErrorBarStyle],
                                        "ErrorBarStyle"
                                    )
                                )
                            );
        ...
        )
        {
            ...
            if (
                point.IsCustomPropertySet(CustomPropertyName.ErrorBarCenterMarkerStyle)
            ...
            )
            {
                ...
                markerStyle = markerStyle.ToUpper(
                ...
                if (ser.YValuesPerPoint < 3)
                {
                    throw (
                        new ArgumentException(
                            SR.ExceptionChartTypeRequiresYValues(
                                ChartTypeNames.ErrorBar,
                                ((int)(3)).ToString(CultureInfo.CurrentCulture)
                            )
                        )
                    );
                }
        ...
        internal static void CalculateErrorAmount(Series errorBarSeries)
        {
            ...
            if (
                !errorBarSeries.IsCustomPropertySet(CustomPropertyName.ErrorBarType)
            ...
            if (errorBarSeries.IsCustomPropertySet(CustomPropertyName.ErrorBarType))
            {
                ...
                else if (typeName.StartsWith("Percentage", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    typeName.StartsWith("StandardDeviation", StringComparison.OrdinalIgnoreCase)
                )
                {
                ...
                else
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionErrorBarTypeInvalid(
                ...
                if (typeName.Length > 0)
                {
                    ...
                    )
                    {
                        throw (
                            new InvalidOperationException(
                                SR.ExceptionErrorBarTypeFormatInvalid(
                                    errorBarSeries[CustomPropertyName.ErrorBarType]
                                )
                            )
                        );
            ...
            else if (errorBarType == ErrorBarType.StandardError)
            {
                ...
                if (pointNumber > 1)
                {
                    ...
                    errorAmount =
                        param * Math.Sqrt(errorAmount / (pointNumber * (pointNumber - 1))) / 2.0;
                }
                else
                {
                    errorAmount = 0;
        ...
        internal static void GetDataFromLinkedSeries(Series errorBarSeries)
        {
            ...
            if (control != null)
            {
                ...
                if (control.Series.IndexOf(linkedSeriesName) == -1)
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionDataSeriesNameNotFound(linkedSeriesName)
                        )
                    );
                }
