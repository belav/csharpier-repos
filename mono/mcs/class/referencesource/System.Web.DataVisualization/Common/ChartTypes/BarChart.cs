...
#endif
{
    ...
    internal class BarChart : IChartType
    {
        ...
        protected BarValueLabelDrawingStyle defLabelDrawingStyle =
            BarValueLabelDrawingStyle.Outside;
            ...
            foreach (Series ser in common.DataManager.Series)
            {
                ...
                foreach (DataPoint point in ser.Points)
                {
                    ...
                    double yValue = hAxis.GetLogValue(
                        GetYValue(common, area, ser, point, pointIndex, (useTwoValues) ? 1 : 0)
                    );
                    ...
                    if (useTwoValues)
                    {
                        ...
                        double yValueStart = hAxis.GetLogValue(
                    ...
                    if (indexedSeries)
                    {
                        ...
                        xPosition =
                            vAxis.GetPosition((double)pointIndex + 1)
                    ...
                    {
                        xPosition =
                            vAxis.GetPosition(point.XValue)
                            - width * ((double)numOfSeries) / 2.0
                            + width / 2
                            + seriesIndx * width;
                    }
                    ...
                    point.positionRel = new PointF(
                        (barStartPosition < barSize) ? rectSize.Right : rectSize.X,
                        (float)xPosition
                    );
                    ...
                    if (common.ProcessModePaint)
                    {
                        ...
                        {
                            if (
                                (rectSize.Y + rectSize.Height / 2f) >= area.PlotAreaPosition.Y
                                && (rectSize.Y + rectSize.Height / 2f)
                                    <= area.PlotAreaPosition.Bottom
                            )
                            {
                                if (labels)
                                {
                ...
                using (StringFormat format = new StringFormat())
                {
                    ...
                    while (!labelFit)
                    {
                        ...
                        if (barStartPosition <= barSize)
                        {
                            ...
                            if (rectLabel.Width < 0.001f && barStartPosition == barSize)
                            {
                        ...
                        if (drawingStyle == BarValueLabelDrawingStyle.Outside)
                        {
                            ...
                            {
                                rectLabel.Width -= (float)
                                    Math.Min(rectLabel.Width, markerSize.Width / 2F);
                                if (barStartPosition < barSize)
                                {
                                    rectLabel.X += (float)
                                        Math.Min(rectLabel.Width, markerSize.Width / 2F);
        ...
        )
        {
            ...
            ArrayList dataPointDrawingOrder = area.GetDataPointDrawingOrder(
                ...
                new BarPointsDrawingOrderComparer(
                    area,
                    selection,
                    COPCoordinates.X | COPCoordinates.Y
                ),
            ...
            foreach (object obj in dataPointDrawingOrder)
            {
                ...
                if (useTwoValues)
                {
                    // Point Y value (first) is used to determine the bar starting position
                    double yValueStart = hAxis.GetLogValue(
                        GetYValue(common, area, ser, pointEx.dataPoint, pointEx.index - 1, 0)
                    );
                ...
                // Make sure that points with small values are still visible
                if (barSize < barStartPosition && (barStartPosition - barSize) < pixelRelSize.Width)
                {
                    barSize = barStartPosition - pixelRelSize.Width;
                }
                if (barSize > barStartPosition && (barSize - barStartPosition) < pixelRelSize.Width)
                {
                    barSize = barStartPosition + pixelRelSize.Width;
                }
                ...
                if (!point.IsEmpty)
                {
                    ...
                    bool clipRegionSet = false;
                    if (
                        rectSize.Bottom <= area.PlotAreaPosition.Y
                        || rectSize.Y >= area.PlotAreaPosition.Bottom
                    )
                    {
                        continue;
                    }
            ...
            if (drawLabels)
            {
                foreach (object obj in dataPointDrawingOrder)
                {
                    ...
                    if (!point.IsEmpty)
                    {
                        ...
                        // Check if column is partialy in the data scaleView
                        if (
                            (decimal)rectSize.Y >= (decimal)area.PlotAreaPosition.Y
                            && (decimal)rectSize.Bottom <= (decimal)area.PlotAreaPosition.Bottom
                        )
                        {
                            // Draw 3D labels
                            DrawLabels3D(
            ...
            if (point.MarkerStyle != MarkerStyle.None || point.MarkerImage.Length > 0)
            {
                ...
                if ((pointEx.index % ser.MarkerStep) == 0)
                {
                    ...
                    marker3DPosition[0] = new Point3D(
                        markerPosition.X,
                        markerPosition.Y,
                        (float)(pointEx.zPosition + pointEx.depth / 2f)
                    );
        ...
        )
        {
            ...
            if (ser.IsValueShownAsLabel || point.IsValueShownAsLabel || point.Label.Length > 0)
            {
                ...
                using (StringFormat format = new StringFormat())
                {
                    ...
                    // Measure string size
                    SizeF size = graph.MeasureStringRel(
                        text,
                        point.Font,
                        new SizeF(rectLabel.Width, rectLabel.Height),
                        format
                    );
                    ...
                    Point3D[] rotationCenterProjection = new Point3D[]
                    {
                        new Point3D(
                            rotationCenter.X,
                            rotationCenter.Y,
                            pointEx.zPosition + pointEx.depth
                        ),
                        new Point3D(
                            rotationCenter.X - 20f,
                            rotationCenter.Y,
                            pointEx.zPosition + pointEx.depth
                        ),
                    };
                    ...
                    if (angle == 0 || angle == 180)
                    {
                        ...
                        rotationCenterProjection[0].PointF = graph.GetAbsolutePoint(
                            rotationCenterProjection[0].PointF
                        );
                        rotationCenterProjection[1].PointF = graph.GetAbsolutePoint(
                            rotationCenterProjection[1].PointF
                        );

                        // Calcuate axis angle
                        float angleXAxis = (float)
                            Math.Atan(
                                (rotationCenterProjection[1].Y - rotationCenterProjection[0].Y)
                                    / (
                                        rotationCenterProjection[1].X
                                        - rotationCenterProjection[0].X
                                    )
                            );
