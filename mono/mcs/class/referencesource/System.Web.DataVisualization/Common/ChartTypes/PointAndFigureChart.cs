    ...
    internal class PointAndFigureChart : RangeColumnChart
    {
        ...
        internal static bool UnPrepareData(Series series)
        {
            if (series.Name.StartsWith("POINTANDFIGURE_ORIGINAL_DATA_", StringComparison.Ordinal))
            {
                ...
                bool parseSucceed = bool.TryParse(
                    pointAndFigureSeries["OldXValueIndexed"],
                    out xValIndexed
                );
                ...
                parseSucceed = int.TryParse(
                    pointAndFigureSeries["OldYValuesPerPoint"],
                    NumberStyles.Any,
                    CultureInfo.InvariantCulture,
                    out yVals
                );
                ...
                if (pointAndFigureSeries.IsCustomPropertySet("OldAutomaticXAxisInterval"))
                {
                    ...
                    if (pointAndFigureSeries.ChartArea.Length > 0)
                    {
                        ...
                        Axis xAxis = area.GetAxis(
                            AxisName.X,
                            pointAndFigureSeries.XAxisType,
                            pointAndFigureSeries.XSubAxisName
                        );
                        ...
                        throw (
                            new InvalidOperationException(
                                SR.ExceptionChartTypeRequiresYValues(
                                    ChartTypeNames.PointAndFigure,
                                    ((int)(2)).ToString(CultureInfo.CurrentCulture)
                                )
                            )
                        );
                    }
        ...
        private static double GetBoxSize(Series series, double minPrice, double maxPrice)
        {
            ...
            if (percentOfPriceRange > 0.0)
            {
                ...
                if (roundBoxSize)
                {
                    ...
                    {
                        if (
                            boxSize > availableBoxSizes[index - 1]
        ...
        private static double GetReversalAmount(Series series)
        {
            ...
            if (series.IsCustomPropertySet(CustomPropertyName.ReversalAmount))
            {
                ...
                else
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionPointAndFigureReversalAmountInvalidFormat
                        )
                    );
                }
            }
        ...
        private static void FillPointAndFigureData(Series series, Series originalData)
        {
            ...
            if (series.IsCustomPropertySet(CustomPropertyName.UsedYValueHigh))
            {
                try
                {
                    yValueHighIndex = int.Parse(
                        series[CustomPropertyName.UsedYValueHigh],
                        CultureInfo.InvariantCulture
                    );
                }
                catch
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionPointAndFigureUsedYValueHighInvalidFormat
                        )
                    );
                }
                ...
                if (yValueHighIndex >= series.YValuesPerPoint)
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionPointAndFigureUsedYValueHighOutOfRange
                        )
                    );
                }
            }
            ...
            if (series.IsCustomPropertySet(CustomPropertyName.UsedYValueLow))
            {
                try
                {
                    yValueLowIndex = int.Parse(
                        series[CustomPropertyName.UsedYValueLow],
                        CultureInfo.InvariantCulture
                    );
                }
                catch
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionPointAndFigureUsedYValueLowInvalidFormat
                        )
                    );
                }
                ...
                if (yValueLowIndex >= series.YValuesPerPoint)
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionPointAndFigureUsedYValueLowOutOfrange
                        )
                    );
                }
            }
            ...
            if (upPriceColorString != null)
            {
                ...
                catch
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionPointAndFigureUpBrickColorInvalidFormat
                        )
                    );
                }
            }
                ...
                if (!dataPoint.IsEmpty)
                {
                    ...
                    if (prevDirection == 1 || prevDirection == 0)
                    {
                        if (dataPoint.YValues[yValueHighIndex] >= (prevHigh + boxSize))
                        {
                            ...
                            dataPoint.YValues[yValueLowIndex]
                            <= (prevHigh - boxSize * reversalAmount)
                        )
                        {
                            direction = -1;
                            numberOfBricks = (int)
                        ...
                        else if (dataPoint.YValues[yValueHighIndex] <= (prevLow - boxSize))
                        {
                            ...
                            if (series.Points.Count > 0)
                            {
                                series.Points[series.Points.Count - 1].YValues[0] -=
                                    numberOfBricks * boxSize;
                            }
                            prevLow -= numberOfBricks * boxSize;
                        }
                    }
                    if (direction == 0 && (prevDirection == -1 || prevDirection == 0))
                    {
                        if (dataPoint.YValues[yValueLowIndex] <= (prevLow - boxSize))
                        {
                            direction = -1;
                            numberOfBricks = (int)
                                Math.Floor((prevLow - dataPoint.YValues[yValueLowIndex]) / boxSize);
                        }
                        else if (
                            dataPoint.YValues[yValueHighIndex]
                            >= (prevLow + boxSize * reversalAmount)
                        )
                        {
                            direction = 1;
                            numberOfBricks = (int)
                        ...
                        else if (dataPoint.YValues[yValueLowIndex] >= (prevHigh + boxSize))
                        {
                            ...
                            if (series.Points.Count > 0)
                            {
                                series.Points[series.Points.Count - 1].YValues[1] +=
                                    numberOfBricks * boxSize;
                            }
                    ...
                    if (direction != 0 && !doNotUpdate)
                    {
                        ...
                        if (direction == prevDirection)
                        {
                            if (direction == 1)
                            {
                                series.Points[series.Points.Count - 1].YValues[1] +=
                                    numberOfBricks * boxSize;
                                prevHigh += numberOfBricks * boxSize;
                                series.Points[series.Points.Count - 1]["OriginalPointIndex"] =
                        ...
                        else
                        {
                            ...
                            if (direction == 1)
                            {
                                ...
                                newDataPoint.YValues[0] =
                                    prevLow + ((prevDirection != 0) ? boxSize : 0.0);
                                newDataPoint.YValues[1] =
                                    newDataPoint.YValues[0]
                                    + numberOfBricks * boxSize
                                    - ((prevDirection != 0) ? boxSize : 0.0);
                            }
                            else
                            {
                                newDataPoint.YValues[1] =
                                    prevHigh - ((prevDirection != 0) ? boxSize : 0.0);
                                newDataPoint.YValues[0] =
                                    newDataPoint.YValues[1] - numberOfBricks * boxSize;
                            }
        ...
        static private void OnCustomize(Object sender, EventArgs e)
        {
            ...
            foreach (Series series in chart.Series)
            {
                ...
                )
                {
                    ...
                    if (
                        attrValue != null
                        && String.Compare(attrValue, "True", StringComparison.OrdinalIgnoreCase)
                            != 0
                    )
                    {
                    ...
                    )
                    {
                        ...
                        Axis axisX = chartArea.GetAxis(
                            AxisName.X,
                            pointAndFigureSeries.XAxisType,
                            pointAndFigureSeries.XSubAxisName
                        );
                        Axis axisY = chartArea.GetAxis(
                            AxisName.Y,
                            pointAndFigureSeries.YAxisType,
                            pointAndFigureSeries.YSubAxisName
                        );
                        ...
                        if (!chartArea.Area3DStyle.Enable3D)
                        {
                            ...
                            double boxXSize = Math.Abs(
                                axisX.GetPosition(1.0) - axisX.GetPosition(0.0)
                            );
                            boxXSize *= 0.8;
                            ...
                            {
                                pointCount = (int)(
                                    pointAndFigureSeries.Points.Count
        ...
        )
        {
            ...
            boxSizeRel = Math.Abs(
                boxSizeRel - vAxis.GetLinearPosition(vAxis.GetLogValue(vAxis.ViewMinimum + boxSize))
            );

            // Draw a series of Xs or Os
            for (
                float positionY = rectSize.Y;
                positionY < rectSize.Bottom - (float)(boxSizeRel - boxSizeRel / 4.0);
                positionY += (float)boxSizeRel
            )
            {
