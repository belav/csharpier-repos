    ...
    internal class FastPointChart : IChartType
    {
        ...
        )
        {
            ...
            foreach (Series series in common.DataManager.Series)
            {
                ...
                {
                    float seriesDepth;
                    area.GetSeriesZPositionAndDepth(
                        series,
                        out seriesDepth,
                        out this.seriesZCoordinate
                    );
                ...
                double axesValuesPixelSizeY = Math.Abs(
                    vAxis.PositionToValue(axesMin.Height + pixelSize.Height, false)
                        - vAxis.PositionToValue(axesMin.Height, false)
                );

                // Create point marker brush
                SolidBrush markerBrush = new SolidBrush(
                    ((series.MarkerColor.IsEmpty) ? series.Color : series.MarkerColor)
                );
                SolidBrush emptyMarkerBrush = new SolidBrush(
                    (
                        (series.EmptyPointStyle.MarkerColor.IsEmpty)
                            ? series.EmptyPointStyle.Color
                            : series.EmptyPointStyle.MarkerColor
                    )
                );
                ...
                if (
                    !series.EmptyPointStyle.MarkerBorderColor.IsEmpty
                    && series.EmptyPointStyle.MarkerBorderWidth > 0
                )
                {
                    emptyBorderPen = new Pen(
                        series.EmptyPointStyle.MarkerBorderColor,
                        series.EmptyPointStyle.MarkerBorderWidth
                    );
                }
                ...
                if (graph != null && graph.Graphics != null)
                {
                    ...
                    markerSize = (int)
                        Math.Max(
                            ...
                            markerSize * graph.Graphics.DpiY / 96
                        );
                }
                ...
                foreach (DataPoint point in series.Points)
                {
                    ...
                    if (index > 0)
                    {
                        ...
                        if (
                            Math.Abs(xValue - xValuePrev) < axesValuesPixelSizeX
                    ...
                    MarkerStyle currentMarkerStyle =
                        (currentPointIsEmpty) ? emptyMarkerStyle : markerStyle;
        ...
        )
        {
            ...
            RectangleF markerBounds = new RectangleF(
                location.X - markerSize / 2f,
                location.Y - markerSize / 2f,
                markerSize,
                markerSize
            );
