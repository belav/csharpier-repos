...
#else
using System.Web.UI.DataVisualization.Charting;using System.Web.UI.DataVisualization.Charting.ChartTypes;using System.Web.UI.DataVisualization.Charting.Data;using System.Web.UI.DataVisualization.Charting.Utilities;
#endif
    ...
    internal class ThreeLineBreakChart : IChartType
    {
        ...
        internal static void PrepareData(Series series)
        {
            ...
            foreach (Series currentSeries in chart.Series)
            {
                if (
                    ...
                    && area == chart.ChartAreas[currentSeries.ChartArea]
                )
                {
            ...
            Series seriesOriginalData = new Series(
                "THREELINEBREAK_ORIGINAL_DATA_" + series.Name,
            ...
            if (series.ChartArea.Length > 0 && series.IsXValueDateTime())
            {
                ...
                if (xAxis.Interval == 0 && xAxis.IntervalType == DateTimeIntervalType.Auto)
                {
                    ...
                    if (nonZeroXValues)
                    {
                        ...
                        xAxis.interval = xAxis.CalcInterval(
                            minX,
                            maxX,
                            true,
                            out intervalType,
        ...
        internal static bool UnPrepareData(Series series)
        {
            if (series.Name.StartsWith("THREELINEBREAK_ORIGINAL_DATA_", StringComparison.Ordinal))
            {
                ...
                bool xValIndexed;
                bool parseSucceed = bool.TryParse(
                    threeLineBreakSeries["OldXValueIndexed"],
                    out xValIndexed
                );
                threeLineBreakSeries.IsXValueIndexed = parseSucceed && xValIndexed;

                int yValsPerPoint;
                parseSucceed = int.TryParse(
                    threeLineBreakSeries["OldYValuesPerPoint"],
                    NumberStyles.Any,
                    CultureInfo.InvariantCulture,
                    out yValsPerPoint
                );
                ...
                if (threeLineBreakSeries.IsCustomPropertySet("OldAutomaticXAxisInterval"))
                {
                    ...
                    if (threeLineBreakSeries.ChartArea.Length > 0)
                    {
                        ...
                        Axis xAxis = area.GetAxis(
                            AxisName.X,
                            threeLineBreakSeries.XAxisType,
                            threeLineBreakSeries.XSubAxisName
                        );
        ...
        private static void FillThreeLineBreakData(Series series, Series originalData)
        {
            ...
            if (series.IsCustomPropertySet(CustomPropertyName.UsedYValue))
            {
                try
                {
                    yValueIndex = int.Parse(
                        series[CustomPropertyName.UsedYValue],
                        CultureInfo.InvariantCulture
                    );
                }
                catch
                {
                    throw (
                        new InvalidOperationException(SR.ExceptionThreeLineBreakUsedYValueInvalid)
                    );
                }
                ...
                if (yValueIndex >= series.YValuesPerPoint)
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionThreeLineBreakUsedYValueOutOfRange
                        )
                    );
                }
            }
            ...
            if (series.IsCustomPropertySet(CustomPropertyName.NumberOfLinesInBreak))
            {
                try
                {
                    linesInBreak = int.Parse(
                        series[CustomPropertyName.NumberOfLinesInBreak],
                        CultureInfo.InvariantCulture
                    );
                }
                catch
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionThreeLineBreakNumberOfLinesInBreakFormatInvalid
                        )
                    );
                }
                ...
                if (linesInBreak <= 0)
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionThreeLineBreakNumberOfLinesInBreakValueInvalid
                        )
                    );
                }
            }
            ...
            foreach (DataPoint dataPoint in originalData.Points)
            {
                ...
                if (priceUpColorString != null)
                {
                    ...
                    catch
                    {
                        throw (
                            new InvalidOperationException(
                                SR.ExceptionThreeLineBreakUpBrickColorInvalid
                            )
                        );
                    }
                }
                ...
                if (direction != 0)
                {
                    ...
                    if (direction != 0)
                    {
                        ...
                        newDataPoint["OriginalPointIndex"] = pointIndex.ToString(
                            CultureInfo.InvariantCulture
                        );
                        ...
                        if (direction == 1)
                        {
                            ...
                            if (
                                (
                                    newDataPoint.BorderColor == Color.Empty
                                    || newDataPoint.BorderColor == Color.Transparent
                                )
                                && (
                                    newDataPoint.Color == Color.Empty
