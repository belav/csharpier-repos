    ...
    internal class TimeSeriesAndForecasting : IFormula
    {
        ...
        )
        {
            ...
            else if (String.Equals(parameterList[0], "Linear", StringComparison.OrdinalIgnoreCase))
            {
                ...
                String.Equals(parameterList[0], "IsLogarithmic", StringComparison.OrdinalIgnoreCase)
                || String.Equals(
                    parameterList[0],
                    "Logarithmic",
            ...
            if (degree > inputValues[0].Length)
                throw new InvalidOperationException(
                    SR.ExceptionForecastingNotEnoughDataPoints(
                        degree.ToString(System.Globalization.CultureInfo.InvariantCulture)
                    )
                );
            ...
            if (parameterList.Length < 4 || !bool.TryParse(parameterList[3], out forecastingError))
            {
            ...
            {
                error +=
                    (outputErrorEst[1][index] - inputValues[1][index])
                    * (outputErrorEst[1][index] - inputValues[1][index]);
            }
            ...
            {
                dev +=
                    (tempOut[1][index] - inputValues[1][index])
                    * (tempOut[1][index] - inputValues[1][index]);
            }
        ...
        )
        {
            if (regressionType == RegressionType.Exponential)
            {
                ...
                for (int index = 0; index < inputValues[1].Length; index++)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            SR.ExceptionForecastingExponentialRegressionHasZeroYValues
                        );
                    }
            ...
            else if (regressionType == RegressionType.Logarithmic)
            {
                ...
                interval =
                    Math.Abs(inputValues[0][0] - inputValues[0][inputValues[0].Length - 1])
            ...
            else if (regressionType == RegressionType.Power)
            {
                ...
                for (int index = 0; index < inputValues[1].Length; index++)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            SR.ExceptionForecastingPowerRegressionHasZeroYValues
                        );
                    }
                }
        ...
        )
        {
            ...
            for (int i = 0; i < polynomialDegree; i++)
            {
                ...
                for (int k = 0; k < polynomialDegree; k++)
                {
                    ...
                    for (int j = 0; j < inputValues[0].Length; j++)
                    {
                        coeffDeterminant[i][k] +=
                            (double)inputValues[1][j] * (double)Math.Pow(inputValues[0][j], k);
                    }
                }
            ...
            if (regressionType == RegressionType.Polynomial)
            {
                ...
                for (int i = 0; i < size + forecastingPeriod; i++)
                {
                    ...
                    {
                        outputValues[1][i] +=
                            (double)coefficients[j] * Math.Pow(outputValues[0][i], j);
                    }
                }
            }
            else if (regressionType == RegressionType.Exponential)
            {
                ...
                for (int i = 0; i < size + forecastingPeriod; i++)
                {
                    ...
                    outputValues[1][i] =
                        Math.Exp(coefficients[0]) * Math.Exp(coefficients[1] * outputValues[0][i]);
                }
            }
            else if (regressionType == RegressionType.Logarithmic)
            {
                ...
                for (int i = 0; i < size + forecastingPeriod; i++)
                {
                    ...
                    outputValues[1][i] =
                        coefficients[1] * Math.Log(outputValues[0][i]) + coefficients[0];
                }
            }
            else if (regressionType == RegressionType.Power)
            {
                ...
                for (int i = 0; i < size + forecastingPeriod; i++)
                {
                    ...
                    outputValues[1][i] =
                        Math.Exp(coefficients[0]) * Math.Pow(outputValues[0][i], coefficients[1]);
                }
            }
        ...
        private double Determinant(double[][] inputDeterminant)
        {
            ...
            {
                return inputDeterminant[0][0] * inputDeterminant[1][1]
                    - inputDeterminant[0][1] * inputDeterminant[1][0];
            }
