    ...
    internal class VolumeIndicators : PriceIndicators
    {
        ...
        )
        {
            ...
            catch (Exception e)
            {
                if (e.Message == SR.ExceptionObjectReferenceIsNull)
                    ...
                    throw new InvalidOperationException(
                        SR.ExceptionPriceIndicatorsPeriodMissing + e.Message
                    );
            }
            ...
            for (int index = 0; index < inputValues[1].Length; index++)
            {
                // Find Typical Price
                TypicalPrice[index] =
                    (inputValues[1][index] + inputValues[2][index] + inputValues[3][index]) / 3.0;
                // Find Money Flow
                MoneyFlow[index] =
                    (inputValues[1][index] + inputValues[2][index] + inputValues[3][index])
                    / 3.0
                    * inputValues[4][index];
            }
        ...
        private void PriceVolumeTrend(double[][] inputValues, out double[][] outputValues)
        {
            ...
            for (int index = 1; index < inputValues[1].Length; index++)
            {
                ...
                outputValues[1][index] =
                    (todayClose - yesterdayClose) / yesterdayClose * inputValues[2][index]
                    + outputValues[1][index - 1];
            }
        }
        ...
        private void NegativeVolumeIndex(
            double[][] inputValues,
            out double[][] outputValues,
            string[] parameterList
        )
        {
            ...
            for (int index = 1; index < inputValues[1].Length; index++)
            {
                ...
                if (inputValues[2][index] < inputValues[2][index - 1])
                {
                    ...
                    outputValues[1][index] =
                        (todayClose - yesterdayClose) / yesterdayClose * outputValues[1][index - 1]
                        + outputValues[1][index - 1];
                }
        ...
        /// <param name="outputValues">Arrays of doubles - Output values</param>
        /// <param name="parameterList">Array of strings - Parameters</param>
        private void PositiveVolumeIndex(
            double[][] inputValues,
            out double[][] outputValues,
            string[] parameterList
        )
        ...
        internal void AccumulationDistribution(double[][] inputValues, out double[][] outputValues)
        {
            ...
            for (int index = 0; index < inputValues[1].Length; index++)
            {
                ...
                distribution[index] =
                    (
                        (inputValues[3][index] - inputValues[2][index])
                        - (inputValues[1][index] - inputValues[3][index])
                    )
