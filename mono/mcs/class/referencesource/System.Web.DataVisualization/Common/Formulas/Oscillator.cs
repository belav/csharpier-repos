...
#endif
{
    ...
    internal class Oscillators : PriceIndicators
    {
        ...
        private void ChaikinOscillator(
            double[][] inputValues,
        ...
        )
        {
            ...
            ExponentialMovingAverage(
                outputDistribution[1],
                out ExpAvgDistribution,
                longPeriod,
                startFromFirst
            );
            ...
            for (int index = inputValues[1].Length - period; index < inputValues[1].Length; index++)
            {
                ...
                else if ((expIndex + longPeriod - shortPeriod) < ExpAvg.Length)
                {
                    ...
                    outputValues[1][expIndex] =
                        ExpAvg[expIndex + longPeriod - shortPeriod] - ExpAvgDistribution[expIndex];
                }
                else
                {
        ...
        /// <param name="outputValues">Arrays of doubles - Output values</param>
        /// <param name="parameterList">Array of strings - Parameters</param>
        private void DetrendedPriceOscillator(
            double[][] inputValues,
            out double[][] outputValues,
            string[] parameterList
        )
        {
            ...
            for (int index = 0; index < outputValues[1].Length; index++)
            {
                ...
                outputValues[1][index] =
                    inputValues[1][index + period + period / 2] - outputAverage[index];
            }
        }
        ...
        /// <param name="outputValues">Arrays of doubles - Output values</param>
        /// <param name="parameterList">Array of strings - Parameters</param>
        private void VolatilityChaikins(
            double[][] inputValues,
            out double[][] outputValues,
            string[] parameterList
        )
        {
            ...
            for (int index = 0; index < outputValues[1].Length; index++)
            {
                ...
                if (outputAverage[index] != 0.0)
                    outputValues[1][index] =
                        (outputAverage[index + period] - outputAverage[index])
                        / outputAverage[index]
                        * 100.0;
                else
                    // Div with zero error.
        ...
        )
        {
            ...
            // percentage
            bool percentage;
            if (parameterList.Length < 3 || !bool.TryParse(parameterList[2], out percentage))
            {
                percentage = true;
            }
            ...
            for (int index = 0; index < longAverage.Length; index++)
            {
                ...
                if (percentage)
                {
                    ...
                    if (longAverage[index] == 0.0)
                        ...
                        outputValues[1][index] =
                            outputValues[1][index] / shortAverage[index + shortPeriod] * 100;
                }
            }
        }
        ...
        /// <param name="outputValues">Arrays of doubles - Output values</param>
        /// <param name="parameterList">Array of strings - Parameters</param>
        internal void StochasticIndicator(
            double[][] inputValues,
            out double[][] outputValues,
            string[] parameterList
        )
        ...
        )
        {
            ...
            for (int index = period - 1; index < inputValues[0].Length; index++)
            {
                ...
                // Find R%
                outputValues[1][index - period + 1] =
                    (maxHi - inputValues[3][index]) / (maxHi - minLow) * (-100.0);
            }
        }
