...

{
    ...
    public class TickMark : Grid
    {
        ...
        internal void Paint(ChartGraphics graph, bool backElements)
        {
            ...
            if (!this.majorGridTick && (this.interval == 0 || double.IsNaN(this.interval)))
            {
                ...
                else
                {
                    ...
                    this.interval = Axis.CalcInterval(
                        ...
                        this.Axis.ViewMinimum
                            + (this.Axis.ViewMaximum - this.Axis.ViewMinimum)
                                / Grid.NumberOfDateTimeIntervals,
                        true,
            ...
            if (
                !Axis.ChartArea.chartAreaIsCurcular
                || Axis.axisType == AxisName.Y
                || Axis.axisType == AxisName.Y2
            )
            {
                current = ChartHelper.AlignIntervalStart(
                    current,
                    this.GetInterval(),
            ...
            if (
                GetIntervalOffset() != 0 && !double.IsNaN(GetIntervalOffset()) && axisSeries == null
            )
            {
                current += ChartHelper.GetIntervalSize(
                    current,
                    GetIntervalOffset(),
                    offsetType,
                    axisSeries,
            ...
            // Too many tick marks
            if (
                (Axis.ViewMaximum - Axis.ViewMinimum)
                ...
                if ((decimal)current >= (decimal)Axis.ViewMinimum)
                {
                    ...
                    if (Axis.Common.ProcessModeRegions)
                    {
                        if (this.Axis.ChartArea.chartAreaIsCurcular)
                        {
                            ...
                            !this.Axis.ChartArea.Area3DStyle.Enable3D
                            || this.Axis.ChartArea.chartAreaIsCurcular
                        )
                        {
                            RectangleF rect = new RectangleF(
        ...
        private double GetLogMinimum(double current, Series axisSeries)
        {
            ...
            {
                viewMinimum += ChartHelper.GetIntervalSize(
                    viewMinimum,
                    GetIntervalOffset(),
                    offsetType,
                    axisSeries,
        ...
        )
        {
            ...
            if (
                tickMarksOnEdge && this.Axis.MajorTickMark.TickMarkStyle == TickMarkStyle.AcrossAxis
            ...
            SurfaceNames surfaceName =
                (wallZPosition == 0f) ? SurfaceNames.Back : SurfaceNames.Front;
            ...
            )
            {
                ...
                Axis.AxisPosition == AxisPosition.Left
                && (!Axis.GetIsMarksNextToAxis() || axisOnEdge)
                ...
                if (Axis.AxisPosition == AxisPosition.Top)
                {
                    ...
                    else if (_style == TickMarkStyle.OutsideArea)
                    {
                        ...
                        point4 = new Point3D(
                            point1.X,
                            point1.Y,
                            -_size - area.areaSceneWallWidth.Width
                        );
                    }
                    else if (_style == TickMarkStyle.AcrossAxis)
                    {
                        ...
                        point4 = new Point3D(
                            point1.X,
                            point1.Y,
                            -_size / 2 - area.areaSceneWallWidth.Width
                        );
                    }
    ...
    public class Grid
    {
        ...
        internal void Paint(ChartGraphics graph)
        {
            ...
            if (!this.majorGridTick && (this.interval == 0 || double.IsNaN(this.interval)))
            {
                ...
                else
                {
                    ...
                    this.interval = _axis.CalcInterval(
                        this._axis.minimum,
                        this._axis.minimum
                            + (this._axis.maximum - this._axis.minimum)
                                / Grid.NumberOfDateTimeIntervals,
                        true,
                        out localIntervalType,
                        ChartValueType.DateTime
            ...
            // Adjust start position depending on the interval type
            if (
                !_axis.ChartArea.chartAreaIsCurcular
            ...
            // Too many gridlines
            if (
                (_axis.ViewMaximum - _axis.ViewMinimum)
                    / ChartHelper.GetIntervalSize(
                        current,
                        this.GetInterval(),
                        this.GetIntervalType(),
            ...
            while ((decimal)current <= viewMaximum)
            {
                ...
                if (this.majorGridTick || this._axis.IsLogarithmic == false)
                {
                    ...
                    gridInterval = ChartHelper.GetIntervalSize(
                        current,
                        autoInterval,
                        this.GetIntervalType(),
                        axisSeries,
                ...
                else
                {
                    ...
                    // Draw between min & max values only
                    if (
                        (decimal)current >= (decimal)_axis.ViewMinimum
                        && (decimal)current <= (decimal)_axis.ViewMaximum
                    )
                    {
                        DrawGrid(graph, current);
                    }
        ...
        private void DrawGrid(ChartGraphics graph, double current)
        {
            ...
            {
                if (
                    this._axis.ChartArea.Area3DStyle.Enable3D
                    && !this._axis.ChartArea.chartAreaIsCurcular
                )
                {
                    if (!common.ProcessModePaint) //if ProcessModePaint is true it will be called later
                        graph.Draw3DGridLine(
                            this._axis.ChartArea,
                            borderColor,
                            borderWidth,
                            borderDashStyle,
                            first,
                            second,
                            (
                                _axis.AxisPosition == AxisPosition.Left
            ...
            foreach (CustomLabel label in _axis.CustomLabels)
            {
                if ((label.GridTicks & GridTickTypes.Gridline) == GridTickTypes.Gridline)
                {
                    ...
                    if (position >= _axis.ViewMinimum && position <= _axis.ViewMaximum)
                    {
                        ...
                        // Vertical gridlines
                        if (
                            _axis.AxisPosition == AxisPosition.Top
