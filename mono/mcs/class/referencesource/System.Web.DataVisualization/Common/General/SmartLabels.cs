...
#endif
{
    ...
    public class SmartLabelStyle
    {
        ...
        public virtual LabelAlignmentStyles MovingDirection
        {
            ...
            set
            {
                ...
                {
                    throw (
                        new InvalidOperationException(SR.ExceptionSmartLabelsDirectionUndefined)
                    );
                }
                ...
                if (value < 0)
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionSmartLabelsMinMovingDistanceIsNegative
                        )
                    );
                }
        ...
        public virtual double MaxMovingDistance
        {
            ...
            set
            {
                if (value < 0)
                {
                    throw (
                        new InvalidOperationException(
                            SR.ExceptionSmartLabelsMaxMovingDistanceIsNegative
                        )
                    );
                }
    ...
    internal class SmartLabel
    {
        ...
        )
        {
            ...
            float movingStep = 2f;
            float minMove = (float)
                Math.Min(smartLabelStyle.MinMovingDistance, smartLabelStyle.MaxMovingDistance);
            float maxMove = (float)
                Math.Max(smartLabelStyle.MinMovingDistance, smartLabelStyle.MaxMovingDistance);
            for (
                labelMovement = minMove;
                !positionFound && labelMovement <= maxMove;
                labelMovement += movingStep
            )
            {
                ...
                for (positionIndex = 0; positionIndex < positions.Length; positionIndex++)
                {
                    ...
                    if (
                        positions[positionIndex] == LabelAlignmentStyles.Center
                        && labelMovement != minMove
                    )
                    {
                        continue;
                    ...
                    if (
                        (smartLabelStyle.MovingDirection & positions[positionIndex])
        ...
        /// <param name="labelAlignment">Label alignment.</param>
        /// <returns>Adjusted position of the label.</returns>
        internal virtual void DrawCallout(
            CommonElements common,
            ChartGraphics graph,
            ChartArea area,
            SmartLabelStyle smartLabelStyle,
        ...
        )
        {
            ...
            if (smartLabelStyle.CalloutLineAnchorCapStyle == LineAnchorCapStyle.Arrow)
            {
                ...
                calloutPen.CustomStartCap = new AdjustableArrowCap(
                    calloutPen.Width + 2,
                    calloutPen.Width + 3,
                    true
                );
        ...
        internal virtual bool IsSmartLabelCollide(
            CommonElements common,
            ChartGraphics graph,
            ChartArea area,
            SmartLabelStyle smartLabelStyle,
        ...
        )
        {
            ...
            if (
                labelPosition.X < 0f
                || labelPosition.Y < 0f
                || labelPosition.Bottom > 100f
                || labelPosition.Right > 100f
            )
            {
#if DEBUG
            ...
            if (!collisionDetected && area != null)
            {
                if (area.chartAreaIsCurcular)
                {
                    using (GraphicsPath areaPath = new GraphicsPath())
                    {
                        ...
                        if (
                            smartLabelStyle.AllowOutsidePlotArea
                        ...
                        {
                            if (
                                !areaPath.IsVisible(labelPosition.Location)
                                || !areaPath.IsVisible(
            ...
            bool allowOneCollision =
                (
                    ...
                    && !smartLabelStyle.IsMarkerOverlappingAllowed
                )
                    ? true
                    : false;
            ...
            if (!collisionDetected && this.smartLabelsPositions != null)
            {
                ...
                foreach (RectangleF pos in this.smartLabelsPositions)
                {
                    ...
                    if (!collision && checkCalloutLineOverlapping && index >= markersCount)
                    {
            ...
            if (this.smartLabelsPositions.Count == 0 && area != null)
            {
                ...
                foreach (Series series in common.DataManager.Series)
                {
                    ...
                    )
                    {
                        ...
                        chartType.AddSmartLabelMarkerPositions(
                            common,
                            area,
                            series,
                ...
                foreach (Axis currentAxis in area.Axes)
                {
                    ...
                    if (
                        currentAxis.ScaleBreakStyle.Spacing > 0.0
    ...
    internal class AnnotationSmartLabel : SmartLabel
    {
        ...
        /// <param name="labelAlignment">Label alignment.</param>
        /// <returns>Adjusted position of the label.</returns>
        internal override void DrawCallout(
            CommonElements common,
            ChartGraphics graph,
            ChartArea area,
            SmartLabelStyle smartLabelStyle,
