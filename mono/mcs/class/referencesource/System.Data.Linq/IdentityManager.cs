    ...
    internal abstract class IdentityManager
    {
        ...
        class StandardIdentityManager : IdentityManager
        {
            ...
            static KeyManager GetKeyManager(MetaType type)
            {
                ...
                KeyManager km = (KeyManager)
                    Activator.CreateInstance(
                        ...
                        null,
                        new object[] { mm.StorageAccessor, 0 },
                        null
                    );
                for (int i = 1; i < n; i++)
                {
                    ...
                    km = (KeyManager)
                        Activator.CreateInstance(
                            typeof(MultiKeyManager<,,>).MakeGenericType(
                                type.Type,
                                mm.Type,
                                km.KeyType
                            ),
                            BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic,
                            null,
                            new object[] { mm.StorageAccessor, i, km },
                            null
                        );
                }
            ...
            internal class MultiKeyManager<T, V1, V2> : KeyManager<T, MultiKey<V1, V2>>
            {
                ...
                internal override bool TryCreateKeyFromValues(
                    object[] values,
                ...
                internal override IEqualityComparer<MultiKey<V1, V2>> Comparer
                {
                    get
                    {
                        if (this.comparer == null)
                        {
                            this.comparer = new MultiKey<V1, V2>.Comparer(
                                EqualityComparer<V1>.Default,
                                next.Comparer
                            );
                        }
                        return this.comparer;
            ...
            internal struct MultiKey<T1, T2>
            {
                ...
                internal class Comparer : IEqualityComparer<MultiKey<T1, T2>>, IEqualityComparer
                {
                    ...
                    {
                        return this.comparer1.GetHashCode(x.value1)
