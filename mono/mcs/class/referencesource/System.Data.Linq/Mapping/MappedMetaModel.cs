...
namespace System.Data.Linq.Mapping
{
    internal class MappedMetaModel : MetaModel
    {
        ...
        )
        {
            ...
            {
                this.providerType = this.FindType(
        ...
        {
            if (
                string.Compare(rootMapping.Name, type.Name, StringComparison.Ordinal) == 0
    ...
    internal sealed class MappedRootType : MappedType
    {
            ...
            : base(model, table, typeMapping, type, null)
        {
            ...
            if (typeMapping.InheritanceCode != null || typeMapping.DerivedTypes.Count > 0)
            {
                ...
                if (!MappingSystem.IsSupportedDiscriminatorType(this.Discriminator.Type))
                {
                    throw Error.DiscriminatorClrTypeNotSupported(
                        this.Discriminator.DeclaringType.Name,
            ...
            if (
                this.inheritanceDefault == null
                && (
                    this.inheritanceCode != null
                    || this.inheritanceCodes != null && this.inheritanceCodes.Count > 0
                )
            )
        ...
        private MetaType InitInheritedType(TypeMapping typeMap, MappedType type)
        {
            ...
            { // Mapping with no inheritance code: For example, an unmapped intermediate class in a hierarchy.
                ...
                object keyValue = DBConvert.ChangeType(
                    typeMap.InheritanceCode,
                    this.Discriminator.Type
                );
                foreach (object d in inheritanceCodes.Keys)
                {
                    ...
                    if (
                        (
                            keyValue.GetType() == typeof(string)
                            && ((string)keyValue).Trim().Length == 0
    ...
    internal class MappedType : MetaType
    {
        ...
        private void InitMethods()
        {
            if (!this.hasMethods)
            {
                ...
                this.hasAnyLoadMethod =
                    (this.onLoadedMethod != null)
                    || (this.InheritanceBase != null && this.InheritanceBase.HasAnyLoadMethod);
                this.hasAnyValidateMethod =
                    (this.onValidateMethod != null)
                    || (this.InheritanceBase != null && this.InheritanceBase.HasAnyValidateMethod);
        ...
        private void InitDataMembers()
        {
            if (this.dataMembers == null)
            {
                ...
                foreach (var orphan in names)
                {
                    ...
                    {
                        foreach (
                            MemberInfo mi in aboveRoot.GetMembers(
                                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic
                            )
                        )
                        {
                            ...
                            {
                                throw Error.MappedMemberHadNoCorrespondingMemberInType(
                                    orphan.Value.MemberName,
                                    type.Name
                                );
                            }
        ...
        private void InitSpecialMember(MetaDataMember mm)
        {
            ...
            if (mm.IsDbGenerated && mm.IsPrimaryKey && string.IsNullOrEmpty(mm.Expression))
            {
                ...
                {
                    throw Error.TwoMembersMarkedAsPrimaryKeyAndDBGenerated(
                        mm.Member,
                        this.dbGeneratedIdentity.Member
                    );
                }
            ...
            if (mm.IsDiscriminator)
            {
                if (this.discriminator != null)
                {
                    ...
                    {
                        throw Error.TwoMembersMarkedAsInheritanceDiscriminator(
                            mm.Member,
                            this.discriminator.Member
                        );
                    }
                }
                else
        ...
        public override bool CanInstantiate
        {
            get
            {
                return !this.type.IsAbstract
                    && (this == this.InheritanceRoot || this.HasInheritanceCode);
            }
        }
        ...
        public override MetaDataMember GetDataMember(MemberInfo mi)
        {
            ...
            MetaDataMember mm;
            if (
                this.dataMemberMap.TryGetValue(InheritanceRules.DistinguishedMemberName(mi), out mm)
            )
            {
                return mm;
            }
            else
            {
                ...
                {
                    throw Error.MappingOfInterfacesMemberIsNotSupported(
    ...
    internal sealed class MappedDataMember : MetaDataMember
    {
        ...
        )
        {
            ...
            if (cmap != null)
            {
                ...
                this.isDBGenerated =
                    cmap.IsDbGenerated || !string.IsNullOrEmpty(cmap.Expression) || this.isVersion;
                this.isDiscriminator = cmap.IsDiscriminator;
                this.canBeNull =
                    cmap.CanBeNull == null
                        ? this.isNullableType || !this.type.IsValueType
                        : (bool)cmap.CanBeNull;
            ...
            if (!this.hasAccessors)
            {
                lock (this.locktarget)
                {
                    if (!this.hasAccessors)
                    {
                        ...
                        {
                            this.accPrivate = MakeMemberAccessor(
                            ...
                            if (this.isDeferred)
                            {
                                MakeDeferredAccessors(
                                    ...
                                    this.accPrivate,
                                    out this.accPrivate,
                                    out this.accDefValue,
                                    out this.accDefSource
                                );
                            }
                            this.accPublic = MakeMemberAccessor(
                                this.member.ReflectedType,
                                this.member,
                                this.accPrivate
                            );
                        ...
                        {
                            this.accPublic = this.accPrivate = MakeMemberAccessor(
                                this.member.ReflectedType,
                                this.member,
                                null
                            );
                ...
                if (this.IsAssociation)
                {
                    ...
                    if (this.assoc == null)
                    {
                        lock (this.locktarget)
                        {
                            if (this.assoc == null)
                            {
                                this.assoc = new MappedAssociation(
                                    this,
                                    (AssociationMapping)this.memberMap
                                );
                            }
                        }
                    }
        ...
        private static MetaAccessor MakeMemberAccessor(
            Type accessorType,
            MemberInfo mi,
            ...
            if (accessor.Type.IsGenericType)
            {
                ...
                if (gtype == typeof(Link<>))
                {
                    accessorValue = CreateAccessor(
                        typeof(LinkValueAccessor<,>).MakeGenericType(declaringType, itemType),
                        accessor
                    );
                    accessorDeferredValue = CreateAccessor(
                        typeof(LinkDefValueAccessor<,>).MakeGenericType(declaringType, itemType),
                        accessor
                    );
                    accessorDeferredSource = CreateAccessor(
                        typeof(LinkDefSourceAccessor<,>).MakeGenericType(declaringType, itemType),
                        accessor
                    );
                    return;
                }
                ...
                {
                    accessorValue = CreateAccessor(
                        typeof(EntityRefValueAccessor<,>).MakeGenericType(declaringType, itemType),
                        accessor
                    );
                    accessorDeferredValue = CreateAccessor(
                        typeof(EntityRefDefValueAccessor<,>).MakeGenericType(
                            declaringType,
                            itemType
                        ),
                        accessor
                    ...
                    accessorDeferredSource = CreateAccessor(
                        typeof(EntityRefDefSourceAccessor<,>).MakeGenericType(
                            declaringType,
                            itemType
                        ),
                        accessor
                ...
                {
                    accessorValue = CreateAccessor(
                        typeof(EntitySetValueAccessor<,>).MakeGenericType(declaringType, itemType),
                        accessor
                    );
                    accessorDeferredValue = CreateAccessor(
                        typeof(EntitySetDefValueAccessor<,>).MakeGenericType(
                            declaringType,
                            itemType
                        ),
                        accessor
                    ...
                    accessorDeferredSource = CreateAccessor(
                        typeof(EntitySetDefSourceAccessor<,>).MakeGenericType(
                            declaringType,
                            itemType
                        ),
                        accessor
    ...
    internal class MappedAssociation : MetaAssociationImpl
    {
        ...
        internal MappedAssociation(MappedDataMember mm, AssociationMapping assocMap)
        {
            ...
            if (
                this.thisKey.Count != this.otherKey.Count
        ...
        private void Init()
        {
            ...
            this.thisKeyIsPrimaryKey = AreEqual(
                this.thisKey,
                this.thisMember.DeclaringType.IdentityMembers
            );
            ...
            if (this.otherType == null)
            {
                ...
                foreach (MetaDataMember omm in this.otherType.DataMembers)
                {
                    if (
                        omm.IsAssociation
                        && omm != this.thisMember
                        && omm.MappedName == this.thisMember.MappedName
                    )
                    {
        ...
        {
            get { return this.assocMap.DeleteRule; }
        }
        public override bool DeleteOnNull
        {
            get { return this.assocMap.DeleteOnNull; }
        }
    }
    ...
    class MappedFunction : MetaFunction
    {
        ...
        static ReadOnlyCollection<MetaParameter> _emptyParameters = new List<MetaParameter>(
            0
        ).AsReadOnly();
        ...
        internal MappedFunction(MappedMetaModel model, FunctionMapping map, MethodInfo method)
        {
            ...
            {
                this.returnParameter = new MappedReturnParameter(
    ...
    internal abstract class MetaAssociationImpl : MetaAssociation
    {
        ...
        protected static bool AreEqual(
            IEnumerable<MetaDataMember> key1,
            IEnumerable<MetaDataMember> key2
        )
        {
    ...
    internal sealed class UnmappedType : MetaType
    {
        ...
        private static ReadOnlyCollection<MetaDataMember> _emptyDataMembers =
            new List<MetaDataMember>().AsReadOnly();
        private static ReadOnlyCollection<MetaAssociation> _emptyAssociations =
            new List<MetaAssociation>().AsReadOnly();
        ...
        public override MetaDataMember GetDataMember(MemberInfo mi)
        {
            ...
            if (this.dataMemberMap == null)
            {
                lock (this.locktarget)
                {
                    if (this.dataMemberMap == null)
                    {
                        Dictionary<object, MetaDataMember> map =
                            new Dictionary<object, MetaDataMember>();
