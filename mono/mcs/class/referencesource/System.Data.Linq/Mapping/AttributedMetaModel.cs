...
namespace System.Data.Linq.Mapping
{
    ...
    internal class AttributedMetaModel : MetaModel
    {
        ...
        internal AttributedMetaModel(MappingSource mappingSource, Type contextType)
        {
            ...
            // Provider type
            ProviderAttribute[] attrs = (ProviderAttribute[])
                this.contextType.GetCustomAttributes(typeof(ProviderAttribute), true);
            ...
            // Database name
            DatabaseAttribute[] das = (DatabaseAttribute[])
                this.contextType.GetCustomAttributes(typeof(DatabaseAttribute), false);
                ...
                try
                {
                    if (!this.initFunctions)
                    {
                        if (this.contextType != typeof(DataContext))
                        {
                            ...
                            {
                                foreach (
                                    MethodInfo mi in type.GetMethods(
                                        BindingFlags.Instance
                                            | BindingFlags.Public
                                            | BindingFlags.NonPublic
                                            | BindingFlags.DeclaredOnly
                                ...
                                )
                                {
                                    if (IsUserFunction(mi))
                                    {
                                        if (mi.IsGenericMethodDefinition)
                                        {
                                            ...
                                            // the two models.
                                            throw Error.InvalidUseOfGenericMethodAsMappedFunction(
                                                mi.Name
                                            );
                                        }
                                        ...
                                        {
                                            MetaFunction metaFunction = new AttributedMetaFunction(
                                                this,
                                                mi
                                            );
        ...
        internal MetaTable GetTableNoLocks(Type rowType)
        {
            ...
            if (!this.metaTables.TryGetValue(rowType, out table))
            {
                ...
                TableAttribute[] attrs = (TableAttribute[])
                    root.GetCustomAttributes(typeof(TableAttribute), true);
                if (attrs.Length == 0)
                {
        ...
        private static Type GetRoot(Type derivedType)
        {
            ...
            {
                TableAttribute[] attrs = (TableAttribute[])
                    derivedType.GetCustomAttributes(typeof(TableAttribute), false);
                if (attrs.Length > 0)
                    return derivedType;
    ...
    internal sealed class AttributedRootType : AttributedMetaType
    {
            ...
            : base(model, table, type, null)
        {
            ...
            InheritanceMappingAttribute[] inheritanceInfo = (InheritanceMappingAttribute[])
            ...
            if (inheritanceInfo.Length > 0)
            {
                ...
                foreach (InheritanceMappingAttribute attr in inheritanceInfo)
                {
                    ...
                    foreach (object d in codeMap.Keys)
                    {
                        ...
                        if (
                            (
                                codeValue.GetType() == typeof(string)
        ...
        private void Validate()
        {
            ...
            foreach (MetaType type in this.InheritanceTypes)
            {
                ...
                {
                    TableAttribute[] attrs = (TableAttribute[])
                        type.Type.GetCustomAttributes(typeof(TableAttribute), false);
                    if (attrs.Length > 0)
    ...
    internal sealed class AttributedMetaFunction : MetaFunction
    {
        ...
        public AttributedMetaFunction(AttributedMetaModel model, MethodInfo mi)
        {
            ...
            this.functionAttrib =
                Attribute.GetCustomAttribute(mi, typeof(FunctionAttribute), false)
                as FunctionAttribute;
            System.Diagnostics.Debug.Assert(functionAttrib != null);

            // Gather up all mapped results
            ResultTypeAttribute[] attrs = (ResultTypeAttribute[])
                Attribute.GetCustomAttributes(mi, typeof(ResultTypeAttribute));
    ...
    internal sealed class AttributedMetaParameter : MetaParameter
    {
        ...
        public AttributedMetaParameter(ParameterInfo parameterInfo)
        {
            ...
            this.paramAttrib =
                Attribute.GetCustomAttribute(parameterInfo, typeof(ParameterAttribute), false)
    ...
    internal sealed class AttributedMetaDataMember : MetaDataMember
    {
        ...
        internal AttributedMetaDataMember(AttributedMetaType metaType, MemberInfo mi, int ordinal)
        {
            ...
            this.attrColumn = (ColumnAttribute)
                Attribute.GetCustomAttribute(mi, typeof(ColumnAttribute));
            this.attrAssoc = (AssociationAttribute)
                Attribute.GetCustomAttribute(mi, typeof(AssociationAttribute));
            this.attr =
                (this.attrColumn != null)
                    ? (DataAttribute)this.attrColumn
            ...
            if (attrColumn != null && attrColumn.IsDbGenerated && attrColumn.IsPrimaryKey)
            {
                ...
                if (
                    (attrColumn.AutoSync != AutoSync.Default)
        ...
        public override bool IsDbGenerated
        {
            get
            {
                return this.attrColumn != null
                        && (
                            this.attrColumn.IsDbGenerated
                            || !string.IsNullOrEmpty(attrColumn.Expression)
                        )
                    || IsVersion;
            }
        }
            ...
            if (accessor.Type.IsGenericType)
            {
                ...
                if (gtype == typeof(Link<>))
                {
                    ...
                    accessorDeferredValue = CreateAccessor(
                        typeof(LinkDefValueAccessor<,>).MakeGenericType(
                            objectDeclaringType,
                            itemType
                        ),
                        accessor
                    ...
                    accessorDeferredSource = CreateAccessor(
                        typeof(LinkDefSourceAccessor<,>).MakeGenericType(
                            objectDeclaringType,
                            itemType
                        ),
                        accessor
                ...
                {
                    accessorValue = CreateAccessor(
                        typeof(EntityRefValueAccessor<,>).MakeGenericType(
                            objectDeclaringType,
                            itemType
                        ),
                        accessor
                ...
                {
                    accessorValue = CreateAccessor(
                        typeof(EntitySetValueAccessor<,>).MakeGenericType(
                            objectDeclaringType,
                            itemType
                        ),
                        accessor
    ...
    internal class AttributedMetaAssociation : MetaAssociationImpl
    {
        ...
        internal AttributedMetaAssociation(
            AttributedMetaDataMember member,
        ...
        )
        {
            ...
            this.thisKey =
                (attr.ThisKey != null)
                    ? MakeKeys(this.thisMember.DeclaringType, attr.ThisKey)
                    : this.thisMember.DeclaringType.IdentityMembers;
            this.otherKey =
                (attr.OtherKey != null)
                    ? MakeKeys(otherType, attr.OtherKey)
            ...
            {
                AssociationAttribute oattr = (AssociationAttribute)
