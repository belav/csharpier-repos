...
namespace System.Data.Linq.SqlClient
{
    ...
    {
        internal static string GetCreateDatabaseCommand(
            string catalog,
            string dataFilename,
            string logFilename
        )
        ...
        internal static string GetCreateSchemaForTableCommand(MetaTable table)
        {
            ...
            List<string> parts = new List<string>(
                SqlIdentifier.GetCompoundIdentifierParts(table.TableName)
            );
        ...
        internal static string GetCreateTableCommand(MetaTable table)
        {
            ...
            sb.AppendFormat(
                "CREATE TABLE {0}",
                SqlIdentifier.QuoteCompoundIdentifier(table.TableName)
            );
            sb.Append("(");
        ...
        private static int BuildFieldDeclarations(
            MetaType type,
            Dictionary<object, string> memberNameToMappedName,
            StringBuilder sb
        )
        {
            int n = 0;
                        ...
                        if (mm.IsPersistent)
                        {
                            ...
                            sb.Append(
                                string.Format(
                                    Globalization.CultureInfo.InvariantCulture,
                                    "  {0} ",
                                    SqlIdentifier.QuoteCompoundIdentifier(mm.MappedName)
                                )
                            );
