...
namespace System.Data.Linq.SqlClient
{
    internal abstract class SqlVisitor
    {
                ...
                switch (node.NodeType)
                {
                    ...
                    case SqlNodeType.TableValuedFunctionCall:
                        result = this.VisitTableValuedFunctionCall(
                            (SqlTableValuedFunctionCall)node
                        );
                        break;
        ...
        internal virtual SqlRowNumber VisitRowNumber(SqlRowNumber rowNumber)
        {
            ...
            {
                rowNumber.OrderBy[i].Expression = this.VisitExpression(
            ...
            try
            {
                ...
                if (exp != null)
                {
                    switch (exp.NodeType)
                    {
                        case SqlNodeType.Column:
                            return exp == col
                                || this.RefersToColumn(((SqlColumn)exp).Expression, col);
                        case SqlNodeType.ColumnRef:
                            SqlColumnRef cref = (SqlColumnRef)exp;
                            return cref.Column == col
                                || this.RefersToColumn(cref.Column.Expression, col);
                        case SqlNodeType.ExprSet:
