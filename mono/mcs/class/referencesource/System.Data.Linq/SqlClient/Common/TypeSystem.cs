...
namespace System.Data.Linq.SqlClient
{
    internal static class TypeSystem
    {
        ...
        {
            Dictionary<MetaPosition, PropertyInfo> seen =
                new Dictionary<MetaPosition, PropertyInfo>();
            Type currentType = type;
            do
            {
        ...
        internal static MethodInfo FindSequenceMethod(
            string name,
            Type[] args,
        ...
        )
        {
            ...
            {
                _sequenceMethods = typeof(Enumerable)
                    .GetMethods(BindingFlags.Static | BindingFlags.Public)
                    .ToLookup(m => m.Name);
            }
            MethodInfo mi = _sequenceMethods[name]
                .FirstOrDefault(m => ArgsMatchExact(m, args, typeArgs));
            if (mi == null)
                return null;
            if (typeArgs != null)
        ...
        internal static MethodInfo FindSequenceMethod(string name, IEnumerable sequence)
        {
            return FindSequenceMethod(
                name,
                new Type[] { sequence.GetType() },
                new Type[] { GetElementType(sequence.GetType()) }
            );
        }

        private static ILookup<string, MethodInfo> _queryMethods;

        internal static MethodInfo FindQueryableMethod(
            string name,
            Type[] args,
            params Type[] typeArgs
        )
        {
            ...
            {
                _queryMethods = typeof(Queryable)
                    .GetMethods(BindingFlags.Static | BindingFlags.Public)
                    .ToLookup(m => m.Name);
            }
        ...
        internal static MethodInfo FindStaticMethod(
            Type type,
            string name,
            Type[] args,
            params Type[] typeArgs
    ...
    internal struct MetaPosition : IEqualityComparer<MetaPosition>, IEqualityComparer
    {
        ...
        internal static bool AreSameMember(MemberInfo x, MemberInfo y)
        {
            if (
                ...
                || x.DeclaringType.Assembly != y.DeclaringType.Assembly
            )
            {
                return false;
            }
