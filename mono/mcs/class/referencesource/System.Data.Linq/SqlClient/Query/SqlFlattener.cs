    ...
    internal class SqlFlattener
    {
        ...
        class Visitor : SqlVisitor
        {
            [SuppressMessage(
                ...
                Justification = "Microsoft: part of our standard visitor pattern"
            )]
            SqlFactory sql;
            ...
            internal override SqlSelect VisitSelect(SqlSelect select)
            {
                ...
                if (!this.isTopLevel)
                {
                    select.Selection = new SqlNop(
                        select.Selection.ClrType,
                        select.Selection.SqlType,
            ...
            private SqlExpression FlattenSelection(
                SqlRow row,
                bool isInput,
            ...
            class SelectionFlattener : SqlVisitor
            {
                ...
                internal override SqlExpression VisitColumn(SqlColumn col)
                {
                    ...
                    if (
                        ...
                        && col.Expression != null
                        && !this.isInput
                        && (!this.isNew || (this.isNew && !col.Expression.IsConstantColumn))
                    )
                    {
                    ...
                    else if (c != col)
                    {
                        ...
                        if (
                            col.Expression.NodeType == SqlNodeType.ExprSet
                            && c.Expression.NodeType != SqlNodeType.ExprSet
                        )
                        {
                ...
                private SqlColumnRef MakeFlattenedColumn(SqlExpression expr, string name)
                {
                    SqlColumn c =
                        (!this.isInput)
                            ? this.FindColumnWithExpression(this.row.Columns, expr)
                            : null;
                    if (c == null)
                    {
                        c = new SqlColumn(
                ...
                private SqlColumn FindColumnWithExpression(
            ...
            private void FlattenOrderBy(List<SqlOrderExpression> exprs)
            {
                foreach (SqlOrderExpression obex in exprs)
                {
                    if (!obex.Expression.SqlType.IsOrderable)
                    {
                        ...
                        {
                            throw Error.InvalidOrderByExpression(
                                obex.Expression.SqlType.ToQueryString()
                            );
                        }
                        else
                        {
