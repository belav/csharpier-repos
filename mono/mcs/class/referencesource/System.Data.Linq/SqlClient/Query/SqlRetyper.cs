        ...
        class Visitor : SqlVisitor
        {
            ...
            private static bool CanDbConvert(Type from, Type to)
            {
                ...
                switch (tcTo)
                {
                    ...
                    case TypeCode.Int32:
                        return tcFrom == TypeCode.Byte
                            || tcFrom == TypeCode.SByte
                            || tcFrom == TypeCode.Int16
                            || tcFrom == TypeCode.UInt16;
                    case TypeCode.Int64:
                        return tcFrom == TypeCode.Byte
                            || tcFrom == TypeCode.SByte
                            || tcFrom == TypeCode.Int16
                            || tcFrom == TypeCode.UInt16
                            || tcFrom == TypeCode.Int32
                            || tcFrom == TypeCode.UInt32;
                    ...
                    case TypeCode.Double:
                        return tcFrom == TypeCode.Single;
                    case TypeCode.Decimal:
                        return tcFrom == TypeCode.Single || tcFrom == TypeCode.Double;
                }
                return false;
            }
            ...
            internal override SqlExpression VisitBinaryOperator(SqlBinary bo)
            {
                ...
                if (
                    bo.NodeType.IsComparisonOperator()
                    && bo.Left.ClrType != typeof(bool)
                ...
                )
                {
                    ...
                    if (bo.Left.NodeType == SqlNodeType.Convert)
                    {
                        ...
                        if (
                            CanDbConvert(conv.Operand.ClrType, bo.Right.ClrType)
                    ...
                    if (bo.Right.NodeType == SqlNodeType.Convert)
                    {
                        ...
                        if (
                            CanDbConvert(conv.Operand.ClrType, bo.Left.ClrType)
                ...
                if (bo.NodeType.IsComparisonOperator())
                {
                    ...
                    Func<SqlExpression, SqlExpression, bool> needsRetype = (expr, val) =>
                        (
                            val.NodeType == SqlNodeType.Value
                            || val.NodeType == SqlNodeType.ClientParameter
                        )
                        && !(
                            expr.NodeType == SqlNodeType.Value
                            || expr.NodeType == SqlNodeType.ClientParameter
                        )
                        && val.SqlType.IsUnicodeType
            ...
            internal override SqlExpression VisitIn(SqlIn sin)
            {
                ...
                var newValues = new System.Collections.Generic.List<SqlExpression>(
                    sin.Values.Count
                );
            ...
            internal override SqlExpression VisitLike(SqlLike like)
            {
                ...
                if (
                    !like.Expression.SqlType.IsUnicodeType
                    && like.Pattern.SqlType.IsUnicodeType
                    && (
            ...
            internal override SqlExpression VisitSearchedCase(SqlSearchedCase c)
            {
                ...
                {
                    c.Else = sql.UnaryConvert(
                        c.Else.ClrType,
                        type,
                        c.Else,
                        c.Else.SourceExpression
                    );
                }
            ...
            private void CoerceTypeFamily(SqlExpression arg1, SqlExpression arg2)
            {
                if (
                    (
                        arg1.SqlType.HasPrecisionAndScale
                        && arg2.SqlType.HasPrecisionAndScale
                        && arg1.SqlType != arg2.SqlType
                    )
                    || SqlFactory.IsSqlHighPrecisionDateTimeType(arg1)
                    || SqlFactory.IsSqlHighPrecisionDateTimeType(arg2)
                )
                {
            ...
            private SqlExpression CoerceValueForExpression(SqlValue value, SqlExpression expression)
            {
                ...
                ProviderType newSqlType = typeProvider.ChangeTypeFamilyTo(
                    value.SqlType,
                    expression.SqlType
                );
                return sql.Value(
