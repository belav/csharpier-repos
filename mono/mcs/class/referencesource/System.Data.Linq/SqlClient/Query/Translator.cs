...
namespace System.Data.Linq.SqlClient
{
    internal class Translator
    {
        ...
        )
        {
            ...
            SqlExpression projection = this.BuildProjection(
                tableAliasRef,
                table.RowType,
                allowDeferred,
                link,
        ...
        internal SqlExpression BuildProjection(
            SqlExpression item,
            MetaType rowType,
            bool allowDeferred,
            SqlLink link,
        ...
        )
        {
            if (!rowType.HasInheritance)
            {
                return this.BuildProjectionInternal(
                    item,
                    rowType,
                    (rowType.Table != null) ? rowType.PersistentDataMembers : rowType.DataMembers,
                    allowDeferred,
                    link,
                    source
                );
            ...
            else
            {
                ...
                foreach (MetaType type in mappedTypes)
                {
                    ...
                    {
                        SqlNew defaultProjection = this.BuildProjectionInternal(
                            item,
                            type,
                            type.PersistentDataMembers,
                            allowDeferred,
                            link,
                            source
                        );
                        ...
                        object code = InheritanceRules.InheritanceCodeForClientCompare(
                            type.InheritanceCode,
                            dm.SqlType
                        );
                        SqlExpression match = sql.Value(
                            dt,
                            sql.Default(discriminator),
                            code,
                            true,
                            source
                        );
        ...
        private SqlNew BuildProjectionInternal(
            SqlExpression item,
            MetaType rowType,
            IEnumerable<MetaDataMember> members,
            bool allowDeferred,
            SqlLink link,
            Expression source
        )
        {
            ...
            foreach (MetaDataMember mm in members)
            {
                if (allowDeferred && (mm.IsAssociation || mm.IsDeferred))
                {
                    ...
                    {
                        bindings.Add(
                            new SqlMemberAssign(mm.Member, this.BuildLink(item, mm, source))
                        );
                    }
                }
        ...
        /// </summary>
        internal static Expression TranslateAssociation(
            DataContext context,
        ...
        )
        {
            ...
            return WhereClauseFromSourceAndKeys(
                otherSource,
                association.OtherKey.ToArray(),
                keyValues
            );
        }

        internal static Expression WhereClauseFromSourceAndKeys(
            Expression source,
        ...
        )
        {
            ...
            for (int i = 0; i < keyMembers.Length; i++)
            {
                ...
                Expression parameterAsDeclaring =
                    elementType == metaMember.Member.DeclaringType
                        ? (Expression)p
                        : (Expression)Expression.Convert(p, metaMember.Member.DeclaringType);
                Expression memberExpression =
                    (metaMember.Member is FieldInfo)
                        ? Expression.Field(parameterAsDeclaring, (FieldInfo)metaMember.Member)
                        : Expression.Property(
                            parameterAsDeclaring,
                            (PropertyInfo)metaMember.Member
                        );
            ...
            Expression sequenceExpression = Expression.Call(
                typeof(Enumerable),
                "Where",
                new Type[] { p.Type },
                source,
                Expression.Lambda(whereExpression, p)
            );
        ...
        private class RelationComposer : ExpressionVisitor
        {
            ...
            private static Expression[] GetKeyValues(
                Expression expr,
            ...
            internal override Expression VisitMemberAccess(MemberExpression m)
            {
                ...
                {
                    Expression[] keyValues = GetKeyValues(
                        this.Visit(m.Expression),
                        this.association.ThisKey
                    );
                    return WhereClauseFromSourceAndKeys(
                        this.otherSouce,
                        this.association.OtherKey.ToArray(),
                        keyValues
                    );
                }
                ...
                {
                    if (
                        exp.Type != m.Expression.Type
                        && m.Member.Name == "Count"
                        && TypeSystem.IsSequenceType(exp.Type)
                    )
                    {
                        return Expression.Call(
                            typeof(Enumerable),
                            "Count",
                            new Type[] { TypeSystem.GetElementType(exp.Type) },
                            exp
                        );
                    }
        ...
        internal SqlNode TranslateLink(
            SqlLink link,
        ...
        )
        {
            ...
            if (mm.IsAssociation)
            {
                ...
                Expression source = new LinkedTableExpression(
                    link,
                    table,
                    typeof(IQueryable<>).MakeGenericType(otherType.Type)
                );
                ...
                Expression expr = TranslateAssociation(
        ...
        internal SqlExpression TranslateEquals(SqlBinary expr)
        {
            ...
            )
            {
                ...
                {
                    return this.TranslateEqualsOp(
                        expr.NodeType,
                        sql.DoNotVisitExpression(expr.Left),
                        sql.DoNotVisitExpression(expr.Right),
                        false
                    );
                }
                return expr;
            ...
            if ((mtLeft != mtRight) && (mtLeft.InheritanceRoot != mtRight.InheritanceRoot))
            {
                return sql.Binary(
                    SqlNodeType.EQ,
                    sql.ValueFromObject(0, expr.SourceExpression),
                    sql.ValueFromObject(1, expr.SourceExpression)
                );
            }
            ...
            {
                SqlExpression eq = this.TranslateEqualsOp(
                    eqKind,
                    exprs1[i],
                    exprs2[i],
                    !mtLeft.IsEntity
        ...
        )
        {
            switch (op)
            {
                ...
                case SqlNodeType.EQ2V:
                    if (
                        SqlExpressionNullability.CanBeNull(left) != false
                    ...
                    )
                    {
                        ...
                        return sql.Binary(
                            ...
                            sql.Binary(
                                ...
                                sql.Unary(
                                    SqlNodeType.IsNull,
                                    (SqlExpression)SqlDuplicator.Copy(left)
                                ),
                                sql.Unary(
                                    SqlNodeType.IsNull,
                                    (SqlExpression)SqlDuplicator.Copy(right)
                                )
                            ),
                            sql.Binary(
                                SqlNodeType.And,
        ...
        internal SqlExpression TranslateLinkIsNull(SqlUnary expr)
        {
            ...
            if (
                !(link != null && link.Member.IsAssociation && link.Member.Association.IsForeignKey)
            )
            {
                return expr;
            }
            ...
            SqlExpression exp = null;
            SqlNodeType combo =
                (expr.NodeType == SqlNodeType.IsNull) ? SqlNodeType.Or : SqlNodeType.And;
            for (int i = 0, n = exprs.Count; i < n; i++)
            {
                SqlExpression compare = sql.Unary(
                    expr.NodeType,
                    sql.DoNotVisitExpression(exprs[i]),
