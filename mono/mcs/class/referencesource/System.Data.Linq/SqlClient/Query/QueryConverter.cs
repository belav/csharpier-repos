...
namespace System.Data.Linq.SqlClient
{
    ...
    internal class QueryConverter
    {
        ...
        private SqlNode VisitInner(Expression node)
        {
            ...
            this.dominatingExpression = ChooseBestDominatingExpression(
        ...
        {
            if (
                sel.Selection.NodeType != SqlNodeType.AliasRef
                || sel.Where != null
                || sel.OrderBy.Count > 0
                || sel.GroupBy.Count > 0
                || sel.Having != null
        ...
        private SqlNode VisitInvocation(InvocationExpression invoke)
        {
            ...
            else
            {
                    ...
                    if (d != null)
                    {
                        ...
                        if (cq != null)
                        {
                            return this.VisitInvocation(
                                Expression.Invoke(cq.Expression, invoke.Arguments)
                            );
                        }
                ...
                var sca = new SqlClientArray(
                    typeof(object[]),
                    this.typeProvider.From(typeof(object[])),
                    args,
                    this.dominatingExpression
                );
                return sql.MethodCall(
                    invoke.Type,
                    typeof(Delegate).GetMethod("DynamicInvoke"),
                    expr,
                    new SqlExpression[] { sca },
                    this.dominatingExpression
                );
            }
        }
...
#pragma warning disable 618 // Disable the 'obsolete' warning
                        Expression.ArrayIndex(pa, Expression.Constant(i)), p.Type),
#pragma warning restore 618
                    pa
                );

                SqlClientParameter cp = new SqlClientParameter(
                    p.Type,
                    this.typeProvider.From(p.Type),
                    accessor,
                    this.dominatingExpression
                );
        ...
        private SqlSelect VisitSelect(Expression sequence, LambdaExpression selector)
        {
            ...
            if (pselect != null)
            {
                return new SqlSelect(
                    sql.SubSelect(SqlNodeType.Multiset, pselect, selector.Body.Type),
                    alias,
                    this.dominatingExpression
                );
            }
                ...
                (
                    project.NodeType == SqlNodeType.Element
                    || project.NodeType == SqlNodeType.ScalarSubSelect
                )
        ...
        private SqlNode VisitSequenceCast(Expression sequence, Type type)
        {
            ...
            return this.Visit(
                Expression.Call(
                    ...
                    sequence,
                    Expression.Lambda(Expression.Convert(p, type), new ParameterExpression[] { p })
                )
            );
        }
        ...
        private SqlNode VisitArrayIndex(BinaryExpression b)
        {
            ...
            )
            {
                ...
                return new SqlClientParameter(
                    b.Type,
                    sql.TypeProvider.From(b.Type),
                    Expression.Lambda(
#pragma warning disable 618 // Disable the 'obsolete' warning
                        Expression.ArrayIndex(
                            cpArray.Accessor.Body,
...
#pragma warning restore 618
        ...
        {
            SqlExpression datetime2 = new SqlVariable(
                expr.ClrType,
                expr.SqlType,
                "DATETIME2",
            ...
            return sql.FunctionCall(
                typeof(DateTime),
                "CONVERT",
                new SqlExpression[2] { datetime2, expr },
                expr.SourceExpression
            );
        }
        ...
        private SqlNode ChangeType(SqlExpression expr, Type type)
        {
            ...
            else if (expr.NodeType == SqlNodeType.ClientParameter)
            {
                ...
                return new SqlClientParameter(
                    type,
                    sql.TypeProvider.From(type),
                    Expression.Lambda(
                        Expression.Convert(cp.Accessor.Body, type),
                        cp.Accessor.Parameters.ToArray()
                    ),
                    cp.SourceExpression
                );
            }
        ...
        private ConversionMethod ChooseConversionMethod(Type fromType, Type toType)
        {
            ...
            if (
                nnFromType == nnToType // same non-nullable .NET types
        ...
        {
            if (
                TypeSystem.IsNullableType(qn.Type)
                && qn.Arguments.Count == 1
                && TypeSystem.GetNonNullableType(qn.Type) == qn.Arguments[0].Type
            )
            {
                return this.VisitCast(Expression.Convert(qn.Arguments[0], qn.Type))
                    as SqlExpression;
            }
            else if (qn.Type == typeof(decimal) && qn.Arguments.Count == 1)
            {
                return this.VisitCast(Expression.Convert(qn.Arguments[0], typeof(decimal)))
                    as SqlExpression;
            }
        ...
        private SqlSelect VisitTake(Expression sequence, Expression count)
        {
            ...
            if (takeExp.NodeType == SqlNodeType.Value)
            {
                ...
                if (
                    typeof(int).IsAssignableFrom(constTakeCount.Value.GetType())
                    && ((int)constTakeCount.Value) < 0
                )
                {
            ...
            )
            {
                ...
                if (skipExp.NodeType == SqlNodeType.Value)
                {
                    ...
                    if (
                        typeof(int).IsAssignableFrom(constSkipCount.Value.GetType())
                        && ((int)constSkipCount.Value) < 0
                    )
                    {
        ...
        private SqlSelect GenerateSkipTake(
            SqlSelect sequence,
            SqlExpression skipExp,
            SqlExpression takeExp
        )
        {
            ...
            if (this.UseConverterStrategy(ConverterStrategy.SkipWithRowNumber))
            {
                ...
                SqlColumn rowNumber = new SqlColumn(
                    "ROW_NUMBER",
                    sql.RowNumber(new List<SqlOrderExpression>(), this.dominatingExpression)
                );
        ...
        )
        {
            ...
            SqlTableValuedFunctionCall functionCall = sql.TableValuedFunctionCall(
        ...
        )
        {
            ...
            )
            {
                ...
                SqlUserRow rowExp = new SqlUserRow(
                    rowType,
        ...
        )
        {
            ...
            for (int i = 0, n = mce.Arguments.Count; i < n; i++)
            {
                ...
                {
                    SqlSimpleTypeExpression typeExpression =
                        newParamExpression as SqlSimpleTypeExpression;
        ...
        private SqlUserQuery VisitUserQuery(string query, Expression[] arguments, Type resultType)
        {
            ...
            if (resultType != typeof(void))
            {
                ...
                {
                    SqlUserColumn col = new SqlUserColumn(
                        elementType,
                        typeProvider.From(elementType),
                        suq,
                        "",
                        false,
                        this.dominatingExpression
        ...
        private SqlExpression MakeCoalesce(SqlExpression left, SqlExpression right, Type resultType)
        {
            ...
            else
            {
                List<SqlWhen> whens = new List<SqlWhen>(1);
                whens.Add(
                    new SqlWhen(sql.Unary(SqlNodeType.IsNull, left, left.SourceExpression), right)
                );
                SqlDuplicator dup = new SqlDuplicator(true);
                return sql.SearchedCase(
                    whens.ToArray(),
                    (SqlExpression)dup.Duplicate(left),
                    this.dominatingExpression
                );
            }
        }
        ...
        //
        private void CompensateForLowerPrecedenceOfDateType(
            ref SqlExpression left,
            ref SqlExpression right
        )
        {
        ...
        private SqlNode VisitIntersect(Expression source1, Expression source2)
        {
            ...
            SqlExpression any = this.GenerateQuantifier(
                alias2,
                sql.Binary(SqlNodeType.EQ2V, aref1, aref2),
                true
            );
        ...
        {
            if (t.IsGenericType && t.GetGenericTypeDefinition() == typeof(IGrouping<,>))
                return true;
            return false;
        }
        ...
        )
        {
            ...
            SqlSelect gsel = new SqlSelect(
                new SqlSharedExpressionRef(keySubExpr),
                seqAlias,
                this.dominatingExpression
            );
            ...
            if (resultSelector != null)
            {
                ...
                Type elementType = typeof(IGrouping<,>).MakeGenericType(
                    keyExpr.ClrType,
                    elemExpr.ClrType
                );

                SqlExpression keyGroup = new SqlGrouping(
                    elementType,
                    this.typeProvider.From(elementType),
                    keyExpr,
                    ss,
                    this.dominatingExpression
                );
                SqlSelect keyGroupSel = new SqlSelect(
                    keyGroup,
                    gselAlias,
                    this.dominatingExpression
                );
                ...
                this.map[resultSelector.Parameters[0]] = sql.Member(
                    kgAliasRef,
                    elementType.GetProperty("Key")
                );
            ...
            else
            {
                ...
                SqlExpression resultExpr = new SqlGrouping(
                    elementType,
                    this.typeProvider.From(elementType),
                    keyExpr,
                    ss,
        ...
        )
        {
            ...
            if (
                ...
                && (
                    lambda == null
                    || (lambda.Parameters.Count == 1 && lambda.Parameters[0] == lambda.Body)
                )
                && (mce != null)
                && IsSequenceOperatorCall(mce, "Select")
                && select.From is SqlAlias
            )
            {
                LambdaExpression selectionLambda = GetLambda(mce.Arguments[1]);

                lambda = Expression.Lambda(
                    selectionLambda.Type,
                    selectionLambda.Body,
            ...
            if (
                select.Selection.SqlType.IsRuntimeOnlyType
                && !IsGrouping(sequence.Type)
                && !isCount
                && lambda == null
            )
            {
            ...
            else
            {
                ...
                for (int i = 1, n = list.Count; i < n; i++)
                {
                    pred = sql.Binary(
                        SqlNodeType.Or,
                        pred,
                        sql.Binary(SqlNodeType.EQ, (SqlExpression)SqlDuplicator.Copy(expr), list[i])
                    );
                }
                return pred;
            }
        ...
        private SqlNode VisitMethodCall(MethodCallExpression mc)
        {
            ...
            if (mc.Method.IsStatic)
            {
                ...
                else if (declType == typeof(DBConvert) || declType == typeof(Convert))
                {
                    if (mc.Method.Name == "ChangeType")
                    {
                        ...
                        if (sn == null)
                        {
                            throw Error.MethodFormHasNoSupportConversionToSql(
                                mc.Method.Name,
                                mc.Method
                            );
                        }
            ...
            else if (typeof(DataContext).IsAssignableFrom(mc.Method.DeclaringType))
            {
                switch (mc.Method.Name)
                {
                    ...
                    case "ExecuteQuery":
                        return this.VisitUserQuery(
                            (string)GetValue(mc.Arguments[0], mc.Method.Name),
                            GetArray(mc.Arguments[1]),
                            mc.Type
                        );
                }
                ...
                && TypeSystem
                    .GetElementType(mc.Method.DeclaringType)
                    .IsAssignableFrom(mc.Arguments[0].Type)
            )
            {
            ...
            if (IsSequenceOperatorCall(mc))
            {
                switch (mc.Method.Name)
                {
                    ...
                    case "Join":
                        isSupportedSequenceOperator = true;
                        if (
                            mc.Arguments.Count == 5
                            && this.IsLambda(mc.Arguments[2])
                            && this.GetLambda(mc.Arguments[2]).Parameters.Count == 1
                            && this.IsLambda(mc.Arguments[3])
                            && this.GetLambda(mc.Arguments[3]).Parameters.Count == 1
                            && this.IsLambda(mc.Arguments[4])
                            && this.GetLambda(mc.Arguments[4]).Parameters.Count == 2
                        )
                        {
                            return this.VisitJoin(
                                mc.Arguments[0],
                                mc.Arguments[1],
                                this.GetLambda(mc.Arguments[2]),
                                this.GetLambda(mc.Arguments[3]),
                                this.GetLambda(mc.Arguments[4])
                            );
                        }
                        break;
                    case "GroupJoin":
        ...
        private SqlNode VisitFirst(Expression sequence, LambdaExpression lambda, bool isFirst)
        {
            ...
            SqlNodeType subType =
                (this.typeProvider.From(select.Selection.ClrType).CanBeColumn)
        ...
        private SqlStatement VisitInsert(Expression item, LambdaExpression resultSelector)
        {
            ...
            foreach (MetaDataMember mm in itemMetaType.PersistentDataMembers)
            {
                ...
                {
                    bindings.Add(
                        new SqlMemberAssign(mm.Member, sql.Member(sqlExprItem, mm.Member))
                    );
                }
            }
            ...
            else
            {
                ...
                if (id != null)
                {
                    ...
                    if (
                        id.Type == typeof(Guid)
                        && (this.converterStrategy & ConverterStrategy.CanOutputFromInsert) != 0
                    )
                    {
                        sin.OutputKey = new SqlColumn(
                            id.Type,
                            sql.Default(id),
                            id.Name,
                            id,
            ...
            else
            {
                ...
                if ((this.converterStrategy & ConverterStrategy.CanUseScopeIdentity) != 0)
                {
                    return new SqlVariable(
                        typeof(decimal),
                        typeProvider.From(typeof(decimal)),
                        "SCOPE_IDENTITY()",
                        this.dominatingExpression
                    );
                }
        ...
        )
        {
            ...
            try
            {
                ...
                if (check != null)
                {
                    findPredicate = Expression.Lambda(
                        Expression.And(
                            Expression.Invoke(findPredicate, p),
                            Expression.Invoke(check, p)
                        ),
                        p
                    );
                }
                ...
                foreach (ModifiedMemberInfo mmi in table.GetModifiedMembers(conItem.Value))
                {
                    ...
                    assignments.Add(
                        new SqlAssign(
                            ...
                            new SqlValue(
                                mdm.Type,
                                this.typeProvider.From(mdm.Type),
                                mmi.CurrentValue,
                                true,
                                source
                            ),
                ...
                seq = Expression.Call(
                    typeof(Enumerable),
                    "Select",
                    new Type[] { rowType, resultSelector.Body.Type },
                    seq,
                    resultSelector
                );
                ...
                select.Where = sql.AndAccumulate(
                    sql.Binary(
                        SqlNodeType.GT,
                        this.GetRowCountExpression(),
                        sql.ValueFromObject(0, false, this.dominatingExpression)
                    ),
                    select.Where
                );
        ...
        private SqlExpression VisitNewArrayInit(NewArrayExpression arr)
        {
            ...
            return new SqlClientArray(
                arr.Type,
                this.typeProvider.From(arr.Type),
                exprs,
                this.dominatingExpression
            );
        }
        ...
        private SqlExpression VisitListInit(ListInitExpression list)
        {
            ...
            return new SqlClientArray(
                list.Type,
                this.typeProvider.From(list.Type),
                exprs,
                this.dominatingExpression
            );
        }
