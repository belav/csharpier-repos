...
namespace System.Data.Linq.SqlClient
{
    ...
    internal class SqlMethodTransformer : SqlVisitor
    {
            ...
            if (result is SqlFunctionCall)
            {
                ...
                if (resultFunctionCall.Name == "LEN")
                {
                    ...
                    if (expr.SqlType.IsLargeType && !expr.SqlType.SupportsLength)
                    {
                        ...
                        {
                            result = sql.ConvertToInt(
                                sql.Divide(result, sql.ValueFromObject(2, expr.SourceExpression))
                            );
                        }
                    }
                }
                ...
                bool skipConversion = SqlMethodTransformer.SkipConversionForDateAdd(
                    resultFunctionCall.Name,
                ...
                switch (unary.NodeType)
                {
                    case SqlNodeType.ClrLength:
                        ...
                        if (expr.SqlType.IsUnicodeType)
                        {
                            result = sql.Divide(
                                result,
                                sql.ValueFromObject(2, expr.SourceExpression)
                            );
                        }
        ...
        // a less precise type.
        //
        private static bool SkipConversionForDateAdd(
            string functionName,
            Type expected,
            Type actual
        )
