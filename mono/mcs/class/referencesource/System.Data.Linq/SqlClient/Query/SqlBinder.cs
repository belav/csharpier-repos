        ...
        class Visitor : SqlVisitor
        {
            ...
            internal SqlExpression FetchExpression(SqlExpression expr)
            {
                return this.ConvertToExpression(
                    this.ConvertToFetchedExpression(this.ConvertLinks(this.VisitExpression(expr)))
                );
            }
            ...
            internal override SqlExpression VisitBinaryOperator(SqlBinary bo)
            {
                ...
                switch (bo.NodeType)
                {
                    ...
                    case SqlNodeType.EQ2V:
                        if (this.IsConstNull(bo.Left) && !TypeSystem.IsNullableType(bo.ClrType))
                        {
                            return this.VisitUnaryOperator(
                                this.sql.Unary(SqlNodeType.IsNull, bo.Right, bo.SourceExpression)
                            );
                        }
                        else if (
                            this.IsConstNull(bo.Right) && !TypeSystem.IsNullableType(bo.ClrType)
                        )
                        {
                            return this.VisitUnaryOperator(
                                this.sql.Unary(SqlNodeType.IsNull, bo.Left, bo.SourceExpression)
                            );
                        }
                        break;
                    case SqlNodeType.NE:
                    case SqlNodeType.NE2V:
                        ...
                        {
                            return this.VisitUnaryOperator(
                                this.sql.Unary(SqlNodeType.IsNotNull, bo.Right, bo.SourceExpression)
                            );
                        }
                        else if (
                            this.IsConstNull(bo.Right) && !TypeSystem.IsNullableType(bo.ClrType)
                        ...
                        {
                            return this.VisitUnaryOperator(
                                this.sql.Unary(SqlNodeType.IsNotNull, bo.Left, bo.SourceExpression)
                            );
                        }
                        break;
                }
                ...
                switch (bo.NodeType)
                {
                    ...
                    case SqlNodeType.NE2V:
                    {
                        ...
                        {
                            bool equal =
                                bo.NodeType != SqlNodeType.NE && bo.NodeType != SqlNodeType.NE2V;
                            bool isTwoValue =
                            ...
                            if (leftIsBool && !rightIsBool)
                            {
                                ...
                                if (value ^ equal)
                                {
                                    return VisitUnaryOperator(
                                        new SqlUnary(
                                            negator,
                                            bo.ClrType,
                                            bo.SqlType,
                                            sql.DoNotVisitExpression(bo.Right),
                                            bo.SourceExpression
                                        )
                                    );
                                }
                                if (bo.Right.ClrType == typeof(bool))
                                { // If the other side is nullable bool then this expression is already a reasonable way to handle three-values
                                    return bo.Right;
                                }
                            }
                                ...
                                if (value ^ equal)
                                {
                                    return VisitUnaryOperator(
                                        new SqlUnary(
                                            ...
                                            bo.ClrType,
                                            bo.SqlType,
                                            sql.DoNotVisitExpression(bo.Left),
                                            bo.SourceExpression
                                        )
                                    );
                                }
                ...
                switch (bo.NodeType)
                {
                    case SqlNodeType.And:
                    {
                        ...
                        {
                            return sql.ValueFromObject(
                                (bool)(vLeft.Value ?? false) && (bool)(vRight.Value ?? false),
                                false,
                                bo.SourceExpression
                            );
                        }
                    ...
                    case SqlNodeType.NE2V:
                        ...
                        if (typeof(Type).IsAssignableFrom(bo.Left.ClrType))
                        {
                            ...
                            if (leftPossibleTypes.Length == 1 && rightPossibleTypes.Length == 1)
                            {
                                ...
                                return this.VisitExpression(
                                    sql.ValueFromObject(
                                        (bo.NodeType == SqlNodeType.EQ)
                                            == (leftPossibleTypes[0] == rightPossibleTypes[0]),
                                        false,
                                        bo.SourceExpression
                                    )
                                );
                            ...
                            if (leftDt != null && rightDt != null)
                            {
                                return this.VisitExpression(
                                    sql.Binary(
                                        bo.NodeType,
                                        leftDt.Discriminator,
                                        rightDt.Discriminator
                                    )
                                );
                            }
                        }
            ...
            internal override SqlExpression VisitDiscriminatorOf(SqlDiscriminatorOf dof)
            {
                ...
                while (
                    obj.NodeType == SqlNodeType.OptionalValue
                ...
                if (obj.NodeType == SqlNodeType.TypeCase)
                {
                    ...
                    foreach (SqlTypeCaseWhen when in tc.Whens)
                    {
                        ...
                        if (when.Match == null)
                        {
                            SqlExpression @default = sql.Value(
                                discriminator.GetType(),
                                tc.Whens[0].Match.SqlType,
                                defaultType.InheritanceCode,
                                true,
                                tc.SourceExpression
                            );
                            values.Add(@default);
            ...
            {
                if (
                    (c.ClrType == typeof(bool) || c.ClrType == typeof(bool?))
                    && c.Whens.Count == 1
                    && c.Else != null
                )
                {
            ...
            private SqlExpression ApplyTreat(SqlExpression target, Type type)
            {
                switch (target.NodeType)
                {
                    ...
                    default:
                        ...
                        {
                            if (
                                !type.IsAssignableFrom(expr.ClrType)
                                && !expr.ClrType.IsAssignableFrom(type)
                            )
                            {
                                return sql.TypedLiteralNull(type, target.SourceExpression);
                            }
            ...
            internal override SqlExpression VisitUnaryOperator(SqlUnary uo)
            {
                ...
                // ------------------------------------------------------------
                if (
                    (uo.NodeType == SqlNodeType.Not || uo.NodeType == SqlNodeType.Not2V)
                ...
                )
                {
                    ...
                    return sql.Value(
                        ...
                        val.SqlType,
                        !(bool)val.Value,
                        val.IsClientSpecified,
                        val.SourceExpression
                    );
                }
                ...
                )
                {
                    ...
                    return sql.Value(
                        uo.ClrType,
                        uo.SqlType,
                        DBConvert.ChangeType(val.Value, uo.ClrType),
                        val.IsClientSpecified,
                        val.SourceExpression
                    );
                }
                else if (uo.NodeType == SqlNodeType.IsNull || uo.NodeType == SqlNodeType.IsNotNull)
                {
                    ...
                    {
                        return sql.ValueFromObject(
                            uo.NodeType == SqlNodeType.IsNotNull,
                            false,
                            uo.SourceExpression
                        );
                    }
                    ...
                    switch (exp.NodeType)
                    {
                        ...
                        case SqlNodeType.TypeCase:
                        {
                            ...
                            foreach (SqlTypeCaseWhen when in tc.Whens)
                            {
                                SqlUnary un = new SqlUnary(
                                    uo.NodeType,
                                    uo.ClrType,
                                    uo.SqlType,
                                    when.TypeBinding,
                                    when.TypeBinding.SourceExpression
                                );
                        ...
                        case SqlNodeType.Value:
                        {
                            ...
                            return sql.Value(
                                typeof(bool),
                                this.typeProvider.From(typeof(int)),
                                (val.Value == null) == (uo.NodeType == SqlNodeType.IsNull),
                                val.IsClientSpecified,
                                uo.SourceExpression
                            );
                        }
            ...
            internal override SqlExpression VisitNew(SqlNew sox)
            {
                ...
                for (int i = 0, n = sox.Members.Count; i < n; i++)
                {
                    ...
                    if (
                        mm.IsAssociation
                        && ma.Expression != null
                        && ma.Expression.NodeType != SqlNodeType.Link
                        && this.shape != null
                        && this.shape.IsPreloaded(mm.Member)
                        && mm.LoadMethod == null
                        && this.alreadyIncluded != null
                        && !this.alreadyIncluded.Contains(otherType)
                    )
                    {
            ...
            private SqlNode AccessMember(SqlMember m, SqlExpression expo)
            {
                ...
                switch (exp.NodeType)
                {
                    ...
                    case SqlNodeType.AliasRef:
                    {
                        ...
                        if (tab != null)
                        {
                            MetaDataMember mm = GetRequiredInheritanceDataMember(
                                tab.RowType,
                                m.Member
                            );
                        ...
                        SqlTableValuedFunctionCall fc =
                            aref.Alias.Node as SqlTableValuedFunctionCall;
                        if (fc != null)
                        {
                            MetaDataMember mm = GetRequiredInheritanceDataMember(
                                fc.RowType,
                    ...
                    case SqlNodeType.ScalarSubSelect:
                    {
                        ...
                        try
                        {
                            ...
                            if (rexp != null)
                            {
                                ...
                                if (
                                    rexp.NodeType == SqlNodeType.Member
                                    && !SqlColumnizer.CanBeColumn(rexp)
                                )
                                {
                                    ...
                                    if (
                                        this.canUseOuterApply
                                        && exp.NodeType == SqlNodeType.Element
                    ...
                    case SqlNodeType.ClientParameter:
                    {
                        ...
                        LambdaExpression accessor = Expression.Lambda(
            ...
            private SqlExpression FoldSubquery(SqlSubSelect ss)
            {
                ...
                {
                    if (
                        ss.NodeType == SqlNodeType.Element
                        && ss.Select.Selection.NodeType == SqlNodeType.Multiset
                    )
                    {
                        ...
                        ss.NodeType == SqlNodeType.Element
                        && ss.Select.Selection.NodeType == SqlNodeType.Element
                    )
                    {
                        SqlSubSelect msub = (SqlSubSelect)ss.Select.Selection;
                        SqlAlias alias = new SqlAlias(msub.Select);
            ...
            private SqlExpression PushDownExpression(SqlExpression expr)
            {
                ...
                SqlSelect simple = new SqlSelect(
                    expr,
                    this.currentSelect.From,
                    expr.SourceExpression
                );
            ...
            internal override SqlSelect VisitSelect(SqlSelect select)
            {
                ...
                try
                {
                    ...
                    if (
                        select.Where != null
                        && select.Where.NodeType == SqlNodeType.Value
                        && (bool)((SqlValue)select.Where).Value
                    )
                    {
                        select.Where = null;
                    }
                    ...
                    case SqlNodeType.ClientCase:
                    {
                        ...
                        if (allExprs)
                        {
                            ...
                            for (int i = 0, c = fetchedValues.Count; i < c; ++i)
                            {
                                ...
                                {
                                    throw Error.DidNotExpectTypeChange(
                                        cc.ClrType,
                    ...
                    case SqlNodeType.Link:
                        {
                            ...
                            )
                            {
                                ...
                                SqlJoinType joinType =
                                    (
                                        link.Member.Association.IsForeignKey
                                ...
                                if (
                                    ...
                                    && this.IsOuterDependent(this.currentSelect.From, sa, where)
                                )
                                {
            ...
            private bool IsOuterDependent(
                bool isOuterDependent,
                SqlSource location,
                HashSet<SqlAlias> consumed,
                out HashSet<SqlAlias> produced
            )
            {
                if (location.NodeType == SqlNodeType.Join)
                {
                    ...
                    bool rightIsOuterDependent =
                        join.JoinType == SqlJoinType.LeftOuter
                        || join.JoinType == SqlJoinType.OuterApply;
                    if (
                        this.IsOuterDependent(
                            rightIsOuterDependent,
                            join.Right,
                            consumed,
                            out rightProduced
                        )
            ...
            private SqlSelect SimulateCaseOfSequences(
                SqlClientCase clientCase,
                List<SqlNode> sequences
            )
            {
                /*
                ...
                else
                {
                    ...
                    for (int i = 0; i < sequences.Count - elseCount; ++i)
                    {
                        ...
                        SqlExpression discriminatorPredicate = sql.Binary(
                            SqlNodeType.EQ,
                            clientCase.Expression,
                            clientCase.Whens[i].Match
                        );
                        sel.Where = sql.AndAccumulate(sel.Where, discriminatorPredicate);
                        elseFilter = sql.AndAccumulate(
                            elseFilter,
                            sql.Binary(
                                SqlNodeType.NE,
                                clientCase.Expression,
