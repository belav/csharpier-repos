                ...
                switch (exprs[0].NodeType)
                {
                    case SqlNodeType.MethodCall:
                    {
                        ...
                        return factory.MethodCall(
                            mcs[0].Method,
                            mcs[0].Object,
                            expandedArgs.ToArray(),
                            mcs[0].SourceExpression
                        );
                    }
                    case SqlNodeType.ClientCase:
                    {
                        ...
                        for (int i = 0; i < scs[0].Whens.Count; ++i)
                        {
                            ...
                            whens.Add(
                                new SqlClientWhen(scs[0].Whens[i].Match, this.ExpandTogether(scos))
                            );
                        }

                        return new SqlClientCase(
                            scs[0].ClrType,
                            expression,
                            whens,
                    ...
                    case SqlNodeType.TypeCase:
                    {
                        ...
                        return factory.TypeCase(
                            tcs[0].ClrType,
                            tcs[0].RowType,
                            discriminator,
                            whens,
                            tcs[0].SourceExpression
                        );
                    }
                    case SqlNodeType.New:
                    {
                        ...
                        for (int m = 0, mn = bindings.Length; m < mn; m++)
                        {
                            ...
                            bindings[m] = new SqlMemberAssign(
                                cobs[0].Members[m].Member,
                                this.ExpandTogether(mexprs)
                            );
                            for (int i = 0, n = exprs.Count; i < n; i++)
                            {
                    ...
                    case SqlNodeType.Link:
                    {
                        ...
                        for (int i = 1, n = exprs.Count; i < n; i++)
                        {
                            ...
                            if (
                                links[i].KeyExpressions.Count != links[0].KeyExpressions.Count
                    ...
                    case SqlNodeType.OptionalValue:
                    {
                        ...
                        return new SqlOptionalValue(
                            this.ExpandTogether(hvals),
                            this.ExpandTogether(vals)
                        );
                    }
                    case SqlNodeType.OuterJoinedValue:
                    {
                        ...
                        for (int i = 0, n = exprs.Count; i < n; i++)
                        {
                            if (
                                exprs[i] == null
                                || exprs[i].NodeType != SqlNodeType.OuterJoinedValue
                            )
                            {
                                throw Error.UnionIncompatibleConstruction();
                            }
                    ...
                    case SqlNodeType.DiscriminatedType:
                    {
                        ...
                        return factory.DiscriminatedType(
                            this.ExpandTogether(foos),
