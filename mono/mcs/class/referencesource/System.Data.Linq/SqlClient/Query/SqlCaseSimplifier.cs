        ...
        class Visitor : SqlVisitor
        {
            ...
            internal override SqlExpression VisitBinaryOperator(SqlBinary bo)
            {
                switch (bo.NodeType)
                {
                    ...
                    case SqlNodeType.NE2V:
                        if (
                            bo.Left.NodeType == SqlNodeType.SimpleCase
                        ...
                        )
                        {
                            return this.DistributeOperatorIntoCase(
                                bo.NodeType,
                                (SqlSimpleCase)bo.Left,
                                bo.Right
                            );
                        }
                        else if (
                            bo.Right.NodeType == SqlNodeType.SimpleCase
            ...
            )
            {
                ...
                foreach (SqlWhen when in sc.Whens)
                {
                    ...
                    values.Add(
                        sql.ValueFromObject(
                            (nt == SqlNodeType.EQ || nt == SqlNodeType.EQ2V) == eq,
                            false,
                            sc.SourceExpression
                        )
                    );
            ...
            internal override SqlExpression VisitSimpleCase(SqlSimpleCase c)
            {
                ...
                for (int i = 0, n = c.Whens.Count; i < n; i++)
                {
                    if (compareWhen != i)
                    {
                        ...
                        allValuesLiteral =
                            allValuesLiteral && when.Value.NodeType == SqlNodeType.Value;
                    }
                }
                newWhens.Add(c.Whens[compareWhen]);

                // Did everything reduce to a single CASE?
                SqlExpression rewrite = TryToConsolidateAllValueExpressions(
                    newWhens.Count,
                ...
                rewrite = TryToWriteAsSimpleBooleanExpression(
                    c.ClrType,
                    c.Expression,
                    newWhens,
                    allValuesLiteral
                ...
                rewrite = TryToWriteAsReducedCase(
                    c.ClrType,
                    c.Expression,
                    newWhens,
            ...
            private SqlExpression TryToConsolidateAllValueExpressions(
                int valueCount,
                SqlExpression value
            )
            {
            ...
            /// </summary>
            private SqlExpression TryToWriteAsSimpleBooleanExpression(
                Type caseType,
                SqlExpression discriminator,
                List<SqlWhen> newWhens,
                bool allValuesLiteral
            )
            {
                ...
                if (caseType == typeof(bool) && allValuesLiteral)
                {
                    ...
                    for (int i = 0; i < newWhens.Count; ++i)
                    {
                        ...
                        { // Skip the ELSE
                            if (value)
                            {
                                rewrite = sql.OrAccumulate(
                                    rewrite,
                                    sql.Binary(SqlNodeType.EQ, discriminator, newWhens[i].Match)
                                );
                            }
                            else
                            {
                                rewrite = sql.AndAccumulate(
                                    rewrite,
                                    sql.Binary(SqlNodeType.NE, discriminator, newWhens[i].Match)
                                );
                            }
                        }
                        else
            ...
            private SqlExpression TryToWriteAsReducedCase(
                Type caseType,
                SqlExpression discriminator,
                List<SqlWhen> newWhens,
                SqlExpression elseCandidate,
                int originalWhenCount
            )
            {
