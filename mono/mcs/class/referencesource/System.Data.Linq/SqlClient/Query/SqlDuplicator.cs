...
namespace System.Data.Linq.SqlClient
{
    internal class SqlDuplicator
    {
        ...
        internal class DuplicatingVisitor : SqlVisitor
        {
                ...
                foreach (SqlOrderExpression expr in rowNumber.OrderBy)
                {
                    orderBy.Add(
                        new SqlOrderExpression(
                            expr.OrderType,
                            (SqlExpression)this.Visit(expr.Expression)
                        )
                    );
                }
            ...
            {
                return new SqlJoinedCollection(
                    jc.ClrType,
                    jc.SqlType,
                    this.VisitExpression(jc.Expression),
                    this.VisitExpression(jc.Count),
                    jc.SourceExpression
                );
            }
            ...
            internal override SqlExpression VisitTypeCase(SqlTypeCase tc)
            {
                ...
                foreach (SqlTypeCaseWhen when in tc.Whens)
                {
                    whens.Add(
                        new SqlTypeCaseWhen(
                            VisitExpression(when.Match),
                            VisitExpression(when.TypeBinding)
                        )
                    );
                }
                return new SqlTypeCase(
                    tc.ClrType,
                    tc.SqlType,
                    tc.RowType,
                    disc,
            ...
            {
                SqlInsert n = new SqlInsert(
                    si.Table,
                    this.VisitExpression(si.Expression),
                    si.SourceExpression
                );
            ...
            internal override SqlNode VisitMember(SqlMember m)
            {
                return new SqlMember(
                    m.ClrType,
                    m.SqlType,
                    (SqlExpression)this.Visit(m.Expression),
                    m.Member
                );
            }
            ...
            internal override SqlSelect VisitSelect(SqlSelect select)
            {
                ...
                if (select.OrderBy.Count > 0)
                {
                    ...
                    {
                        SqlOrderExpression nsox = new SqlOrderExpression(
                            sox.OrderType,
                            (SqlExpression)this.Visit(sox.Expression)
                        );
                        lex.Add(nsox);
                    }
                }
            ...
            internal override SqlUserQuery VisitUserQuery(SqlUserQuery suq)
            {
                ...
                SqlUserQuery n = new SqlUserQuery(
                    suq.QueryText,
                    projection,
                    args,
                    suq.SourceExpression
                );
                this.nodeMap[suq] = n;

                foreach (SqlUserColumn suc in suq.Columns)
                {
                    SqlUserColumn dupSuc = new SqlUserColumn(
                        suc.ClrType,
                        suc.SqlType,
                        suc.Query,
                        suc.Name,
                        suc.IsRequired,
                        suc.SourceExpression
                    );
            ...
            )
            {
                ...
                SqlStoredProcedureCall n = new SqlStoredProcedureCall(
                    spc.Function,
                    projection,
                    args,
            ...
            {
                return new SqlUserRow(
                    row.RowType,
                    row.SqlType,
                    (SqlUserQuery)this.Visit(row.Query),
            ...
            internal override SqlExpression VisitTreat(SqlUnary t)
            {
                return new SqlUnary(
                    ...
                    t.ClrType,
                    t.SqlType,
                    (SqlExpression)this.Visit(t.Operand),
                    t.SourceExpression
                );
            }
            ...
            internal override SqlExpression VisitUnaryOperator(SqlUnary uo)
            {
                return new SqlUnary(
                    ...
                    uo.ClrType,
                    uo.SqlType,
                    (SqlExpression)this.Visit(uo.Operand),
                    uo.Method,
                    uo.SourceExpression
                );
            }
            ...
            {
                return new SqlAssign(
                    this.VisitExpression(sa.LValue),
                    this.VisitExpression(sa.RValue),
                    sa.SourceExpression
                );
            }
            ...
            internal override SqlExpression VisitIn(SqlIn sin)
            {
                SqlIn nin = new SqlIn(
                    sin.ClrType,
                    sin.SqlType,
                    this.VisitExpression(sin.Expression),
                    sin.Values,
            ...
            )
            {
                ...
                SqlTableValuedFunctionCall nfc = new SqlTableValuedFunctionCall(
                    fc.RowType,
                    fc.ClrType,
                    fc.SqlType,
                    fc.Name,
            ...
            internal override SqlExpression VisitSharedExpressionRef(SqlSharedExpressionRef sref)
            {
                ...
                return new SqlSharedExpressionRef(
                    (SqlSharedExpression)this.Visit(sref.SharedExpression)
                );
            }

            internal override SqlExpression VisitSimpleExpression(SqlSimpleExpression simple)
            {
                SqlSimpleExpression n = new SqlSimpleExpression(
                    this.VisitExpression(simple.Expression)
                );
                return n;
            }
            ...
            {
                SqlGrouping n = new SqlGrouping(
                    g.ClrType,
                    g.SqlType,
                    this.VisitExpression(g.Key),
                    this.VisitExpression(g.Group),
                    g.SourceExpression
                );
                return n;
            }
            ...
            internal override SqlExpression VisitLift(SqlLift lift)
            {
                return new SqlLift(
                    lift.ClrType,
                    this.VisitExpression(lift.Expression),
                    lift.SourceExpression
                );
            }

            internal override SqlExpression VisitDiscriminatorOf(SqlDiscriminatorOf dof)
            {
                return new SqlDiscriminatorOf(
                    this.VisitExpression(dof.Object),
                    dof.ClrType,
                    dof.SqlType,
