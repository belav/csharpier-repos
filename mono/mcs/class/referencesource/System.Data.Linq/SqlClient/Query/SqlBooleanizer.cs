...
namespace System.Data.Linq.SqlClient
{
    ...
    internal class SqlBooleanizer
    {
        private class Booleanizer : SqlBooleanMismatchVisitor
        {
            ...
            internal override SqlSelect VisitSelect(SqlSelect select)
            {
                ...
                if (select.Where != null && select.Where.NodeType == SqlNodeType.Coalesce)
                {
                    ...
                    if (bin.Right.NodeType == SqlNodeType.Value)
                    {
                        ...
                        if (
                            value.Value != null
                            && value.Value.GetType() == typeof(bool)
            ...
            internal override SqlExpression ConvertValueToPredicate(SqlExpression valueExpression)
            {
                ...
                return new SqlBinary(
                    SqlNodeType.EQ,
                    valueExpression.ClrType,
                    sql.TypeProvider.From(typeof(bool)),
                    valueExpression,
                    sql.Value(
                        typeof(bool),
                        valueExpression.SqlType,
                        true,
                        false,
            ...
            internal override SqlExpression ConvertPredicateToValue(
                SqlExpression predicateExpression
            )
            {
                ...
                SqlExpression valueTrue = sql.ValueFromObject(
                    true,
                    false,
                    predicateExpression.SourceExpression
                );
                SqlExpression valueFalse = sql.ValueFromObject(
                    false,
                    false,
                    predicateExpression.SourceExpression
                );
                ...
                {
                    SqlExpression valueNull = sql.Value(
                        valueTrue.ClrType,
                        valueTrue.SqlType,
                        null,
                        false,
        ...
        /// </summary>
        internal static SqlNode Rationalize(
            SqlNode node,
