...
namespace System.Data.Linq.SqlClient
{
    ...
    internal static class PreBindDotNetConverter
    {
        ...
        {
            return call.Method.IsStatic
                && call.Method.DeclaringType.FullName
        ...
        private static bool IsVbIIF(SqlMethodCall mc)
        {
            return mc.Method.IsStatic
                && mc.Method.DeclaringType.FullName == "Microsoft.VisualBasic.Interaction"
                && mc.Method.Name == "IIf";
        }
        ...
        private class Visitor : SqlVisitor
        {
            ...
            internal override SqlExpression VisitBinaryOperator(SqlBinary bo)
            {
                ...
                else if (IsVbCompareStringEqualsValue(bo))
                {
                    ...
                    {
                        SqlValue emptyStr = new SqlValue(
                            strValue.ClrType,
                            strValue.SqlType,
                            String.Empty,
                            strValue.IsClientSpecified,
                            strValue.SourceExpression
                        );
                        bo =
                            this.MakeCompareTo(call.Arguments[0], emptyStr, bo.NodeType, iValue)
                            ?? bo;
                    }
                    else
                    {
            ...
            private SqlExpression CreateComparison(
                SqlExpression a,
                SqlExpression b,
                Expression source
            )
                ...
                else
                {
                    ...
                    else if (mc.Method.Name == "GetType" && mc.Arguments.Count == 0)
                    {
                        ...
                        if (mt.HasInheritance)
                        {
                            ...
                            SqlDiscriminatorOf discriminatorOf = new SqlDiscriminatorOf(
                                mc.Object,
                                discriminatorType,
                                this.sql.TypeProvider.From(discriminatorType),
                                mc.SourceExpression
                            );
            ...
            private SqlExpression TranslateVbIIF(SqlMethodCall mc)
            {
                ...
                {
                    throw Error.IifReturnTypesMustBeEqual(
                        mc.Arguments[1].ClrType.Name,
                        mc.Arguments[2].ClrType.Name
                    );
                }
            }
            ...
            internal override SqlExpression VisitTreat(SqlUnary t)
            {
                ...
                else if (treatType.IsAssignableFrom(originalType))
                {
                    ...
                    !treatType.IsAssignableFrom(originalType)
                    && !originalType.IsAssignableFrom(treatType)
                )
                {
