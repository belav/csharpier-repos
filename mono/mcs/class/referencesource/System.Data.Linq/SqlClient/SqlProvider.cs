    ...
    public class SqlProvider : IReaderProvider, IConnectionUser
    {
        ...
        void IProvider.Initialize(IDataServices dataServices, object connection)
        {
            ...
            )
            {
                ...
                if (
                    string.Compare(
                        (string)builder["MultipleActiveResultSets"],
                        "true",
                        StringComparison.OrdinalIgnoreCase
                    ) == 0
                )
            ...
            this.conManager = new SqlConnectionManager(
                ...
                fileOrServerOrConnectionString != null /*disposeConnection*/
            );
            if (tx != null)
            {
        ...
        private string GetConnectionString(string fileOrServerOrConnectionString)
        {
            ...
            else
            {
                DbConnectionStringBuilder builder = new DbConnectionStringBuilder();
                if (
                    fileOrServerOrConnectionString.EndsWith(
                        ".mdf",
                        StringComparison.OrdinalIgnoreCase
                    )
                )
        ...
        void IProvider.CreateDatabase()
        {
            ...
            if (this.conManager.Connection.State == ConnectionState.Closed)
            {
                if (this.mode == ProviderMode.SqlCE)
                {
                    ...
                    {
                        throw Error.CreateDatabaseFailedBecauseSqlCEDatabaseAlreadyExists(
                            this.dbName
                        );
                    }
                }
            ...
            try
            {
                if (this.services.Model.GetTables().FirstOrDefault() == null)
                {
                    ...
                    throw Error.CreateDatabaseFailedBecauseOfContextWithNoTables(
                        this.services.Model.DatabaseName
                    );
                }
                ...
                {
                    string createdb = SqlBuilder.GetCreateDatabaseCommand(
                        catalog,
                        filename,
                        Path.ChangeExtension(filename, ".ldf")
                    );
        ...
        void IProvider.DeleteDatabase()
        {
            ...
            else
            {
                ...
                finally
                {
                    this.conManager.ReleaseConnection(this);
                    if (
                        conManager.Connection.State == ConnectionState.Closed
                        && string.Compare(
                            conManager.Connection.ConnectionString,
                            holdConnStr,
        ...
        [SuppressMessage(
            ...
            Justification = "Microsoft: Code needs to return false regarless of exception."
        )]
        [ResourceExposure(ResourceScope.None)] // Exposure is via other methods that set dbName.
        [ResourceConsumption(ResourceScope.Machine, ResourceScope.Machine)] // File.Exists method call.
        ...
        private ICompiledSubQuery CompileSubQuery(
            SqlNode query,
            Type elementType,
        ...
        )
        {
            ...
            QueryInfo[] qis = this.BuildQuery(
                ResultShape.Sequence,
                TypeSystem.GetSequenceType(elementType),
                query,
                parameters,
                annotations
            );
        ...
        IExecuteResult IProvider.Execute(Expression query)
        {
...
#if PERFORMANCE_BUILD
            PerformanceCounter pcBuildQuery = null,
                bpcBuildQuery = null,
                pcExecQuery = null,
                bpcExecQuery = null,
                pcSession = null,
                bpcSession = null;
            PerfTimer timerAll = null,
                timer = null;
            ...
            if (collectQueryPerf)
            {
                ...
                bpcExecQuery = new PerformanceCounter(
                    "DLinq",
                    "ExecuteQueryElapsedTimeBase",
                    false
                );
                pcSession = new PerformanceCounter(
                    "DLinq",
                    "SessionExecuteQueryElapsedTime",
                    false
                );
                bpcSession = new PerformanceCounter(
                    "DLinq",
                    "SessionExecuteQueryElapsedTimeBase",
                    false
                );
        ...
        private void CheckSqlCompatibility(QueryInfo[] queries, SqlNodeAnnotations annotations)
        {
            if (this.Mode == ProviderMode.Sql2000 || this.Mode == ProviderMode.SqlCE)
            {
                ...
                {
                    SqlServerCompatibilityCheck.ThrowIfUnsupported(
                        queries[i].Query,
                        annotations,
                        this.Mode
                    );
            ...
            try
            {
                ...
                switch (queryInfo.ResultShape)
                {
                    ...
                    case ResultShape.Singleton:
                    {
                        ...
                        IObjectReader objReader = factory.Create(
                            reader,
                            true,
                            this,
                            parentArgs,
                            ...
                            )
                            {
                                switch (mce.Method.Name)
                                {
                                    ...
                                    case "SingleOrDefault":
                                        sequenceMethod = TypeSystem.FindSequenceMethod(
                                            mce.Method.Name,
                                            sequence
                                        );
                                        break;
                            ...
                            sequence = (IEnumerable)
                                Activator.CreateInstance(
                                    ...
                                    BindingFlags.Instance
                                        ...
                                        | BindingFlags.NonPublic,
                                    null,
                                    new object[] { sequence, result, this.services.Context },
                                    null
                                );
                        }
        ...
        private void LogCommand(TextWriter writer, DbCommand cmd)
        {
            if (writer != null)
            {
                ...
                writer.WriteLine(
                    ...
                    this.GetType().Name,
                    this.Mode,
                    this.services.Model.GetType().Name,
        ...
        private void AssignParameters(
            DbCommand cmd,
            ReadOnlyCollection<SqlParameterInfo> parms,
            object[] userArguments,
            object lastResult
        )
        {
            if (parms != null)
            {
                foreach (SqlParameterInfo pi in parms)
                {
                    ...
                    if (
                        pi.Parameter.Direction == ParameterDirection.Input
        ...
        private ResultShape GetResultShape(Expression query)
        {
            ...
            if (mce != null)
            {
                ...
                else if (mce.Method.DeclaringType.IsSubclassOf(typeof(DataContext)))
                {
                    ...
                    mce.Method.DeclaringType == typeof(DataManipulation)
                    && mce.Method.ReturnType == typeof(int)
                )
                {
                    return ResultShape.Return;
                }
        ...
        private QueryInfo[] BuildQuery(
            ...
            Type resultType,
            SqlNode node,
            ReadOnlyCollection<Me.SqlParameter> parentParameters,
        ...
        )
        {
            ...
            SqlBinder binder = new SqlBinder(
                ...
                this.sqlFactory,
                this.services.Model,
                this.services.Context.LoadOptions,
                columnizer,
                canUseOuterApply
            );
            binder.OptimizeLinkExpansions =
                (optimizationFlags & OptimizationFlags.OptimizeLinkExpansions) != 0;
            binder.SimplifyCaseStatements =
                (optimizationFlags & OptimizationFlags.SimplifyCaseStatements) != 0;
            ...
            SqlMultiplexer.Options options =
                (
                    ...
                    ? SqlMultiplexer.Options.EnableBigJoin
                    : SqlMultiplexer.Options.None;
            ...
            if (block != null && this.mode == ProviderMode.SqlCE)
            {
                ...
                ReadOnlyCollection<ReadOnlyCollection<SqlParameterInfo>> parameters =
                    parameterizer.ParameterizeBlock(block);
        ...
        private IObjectReaderFactory GetDefaultFactory(MetaType rowType)
        {
            ...
            if (TypeSystem.IsSimpleType(rowType.Type))
            {
                ...
                SqlUserColumn col = new SqlUserColumn(
                    rowType.Type,
                    typeProvider.From(rowType.Type),
                    suq,
                    "",
                    false,
        ...
        class ExecuteResult : IExecuteResult, IDisposable
        {
            ...
            [SuppressMessage(
                ...
                Justification = "Microsoft: used in an assert in ReturnValue.set"
            )]
            bool useReturnValue;
            bool isDisposed;
            ...
            [SuppressMessage(
                ...
                MessageId = "value",
                Justification = "FxCop Error -- False positive during code analysis"
            )]
            ...
            public object GetParameterValue(int parameterIndex)
            {
                if (
                    ...
                    || parameterIndex < 0
                    || parameterIndex > this.parameters.Count
                )
                {
        ...
        class MultipleResults : IMultipleResults, IDisposable
        {
            ...
            public IEnumerable<T> GetResult<T>()
            {
                ...
                return new SingleResult<T>(
                    new OneTimeEnumerable<T>((IEnumerator<T>)objReader),
                    this.executeResult,
                    this.provider.services.Context
                );
            }
