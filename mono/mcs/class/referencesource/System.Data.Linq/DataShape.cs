...
namespace System.Data.Linq
{
    public sealed class DataLoadOptions
    {
        ...
        Dictionary<MetaPosition, LambdaExpression> subqueries =
            new Dictionary<MetaPosition, LambdaExpression>();
        ...
        [SuppressMessage(
            ...
            Justification = "Microsoft: Need to provide static typing."
        )]
        ...
        private void AssociateWithInternal(LambdaExpression expression)
        {
            ...
            while (
                op.NodeType == ExpressionType.Convert
                || op.NodeType == ExpressionType.ConvertChecked
            )
            {
        ...
        {
            bool shapesAreSameOrEmpty =
                (ds1 == ds2) || ((ds1 == null || ds1.IsEmpty) && (ds2 == null || ds2.IsEmpty));
        ...
        private void Subquery(MemberInfo association, LambdaExpression subquery)
        {
            ...
            subquery = (LambdaExpression)
                System.Data.Linq.SqlClient.Funcletizer.Funcletize(subquery); // Layering violation.
            ...
            for (int loop = 0; loop < this.includes.Count; ++loop)
            {
                ...
                foreach (MemberInfo edge in edges)
                {
                    if (
                        edgeTargets
                            .Where(et =>
                                et.IsAssignableFrom(edge.DeclaringType)
                                || edge.DeclaringType.IsAssignableFrom(et)
                            )
                            .Any()
                    )
                    {
