    ...
    internal class ChangeProcessor
    {
        ...
        internal void SubmitChanges(ConflictMode failureMode)
        {
            ...
            if (conflicts.Count > 0)
            {
                ...
                {
                    Debug.Assert(
                        syncDependentItem.IsNew || syncDependentItem.IsPossiblyModified,
                        "SynchDependent data should only be rolled back for new and modified objects."
                    );
        ...
        private void PostProcessUpdates(
            List<TrackedObject> insertedItems,
            List<TrackedObject> deletedItems
        )
        {
            ...
            foreach (TrackedObject insertedItem in insertedItems)
            {
                ...
                if (lookup != insertedItem.Current)
                {
                    throw new DuplicateKeyException(
                        insertedItem.Current,
                        Strings.DatabaseGeneratedAlreadyExistingKey
                    );
                }
        ...
        private void ClearForeignKeyReferences(TrackedObject to)
        {
            ...
            foreach (MetaAssociation assoc in to.Type.Associations)
            {
                if (assoc.IsForeignKey)
                {
                    ...
                    {
                        Debug.Assert(
                            assoc.OtherMember.IsAssociation,
                            "OtherMember of the association is expected to also be an association."
                        );
                        ...
                        object[] keyValues = CommonDataServices.GetForeignKeyValues(
                            assoc,
                            to.Current
                        );
                        ...
                        if (cached != null)
                        {
                            if (assoc.OtherMember.Association.IsMany)
                            {
                                ...
                                System.Collections.IList collection =
                                    assoc.OtherMember.MemberAccessor.GetBoxedValue(cached)
        ...
        private static void ClearForeignKeysHelper(MetaAssociation assoc, object trackedInstance)
        {
            ...
            Debug.Assert(
                assoc.ThisMember.IsAssociation,
            ...
            if (
                thisMember.IsDeferred
                && !(
                    thisMember.StorageAccessor.HasAssignedValue(trackedInstance)
                    || thisMember.StorageAccessor.HasLoadedValue(trackedInstance)
                )
            )
            {
        ...
        internal ChangeSet GetChangeSet()
        {
            ...
            return new ChangeSet(
                newEntities.AsReadOnly(),
                deletedEntities.AsReadOnly(),
                changedEntities.AsReadOnly()
            );
        }
        ...
        /// </summary>
        private static ChangeConflictException CreateChangeConflictException(
        ...
        internal void TrackUntrackedObjects()
        {
            ...
            List<TrackedObject> items = new List<TrackedObject>(
                this.tracker.GetInterestingObjects()
            );
                    ...
                    if (!tracked.IsPendingGeneration(tracked.Type.IdentityMembers))
                    {
                        ...
                        if (cached != item)
                        {
                            ...
                            else if (!cachedTracked.IsDead)
                            {
                                throw new DuplicateKeyException(
                                    item,
                                    Strings.CantAddAlreadyExistingKey
                                );
                            }
                        }
                    }
        ...
        private TrackedObject GetOtherItem(MetaAssociation assoc, object instance)
        {
            ...
            if (
                assoc.ThisMember.StorageAccessor.HasAssignedValue(instance)
        ...
        private bool HasAssociationChanged(MetaAssociation assoc, TrackedObject item)
        {
            ...
            {
                if (
                    assoc.ThisMember.StorageAccessor.HasAssignedValue(item.Current)
                    || assoc.ThisMember.StorageAccessor.HasLoadedValue(item.Current)
                )
                {
                    return this.GetOtherItem(assoc, item.Current)
                        != this.GetOtherItem(assoc, item.Original);
                }
                else
                {
                    object[] currentFKs = CommonDataServices.GetForeignKeyValues(
                        assoc,
                        item.Current
                    );
                    object[] originaFKs = CommonDataServices.GetForeignKeyValues(
                        assoc,
                        item.Original
                    );
            ...
            foreach (TrackedObject item in list)
            {
                ...
                foreach (MetaAssociation assoc in mt.Associations)
                {
                    if (assoc.IsForeignKey)
                    {
                        ...
                        bool pointsToDeleted =
                            (otherItem != null && otherItem.IsDeleted)
                            || (dbOtherItem != null && dbOtherItem.IsDeleted);
                        ...
                        if (
                            ...
                            || pointsToDeleted
                            || pointsToNew
                            || this.HasAssociationChanged(assoc, item)
                        )
                        {
                            if (otherItem != null)
                            {
        ...
        private void BuildDependencyOrderedList(
            TrackedObject item,
            List<TrackedObject> list,
            Dictionary<TrackedObject, VisitState> visited
        )
        {
            VisitState state;
        ...
        class EdgeMap
        {
            ...
            {
                this.associations =
                    new Dictionary<MetaAssociation, Dictionary<TrackedObject, TrackedObject>>();
            }
