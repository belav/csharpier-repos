    ...
    internal class CommonDataServices : IDataServices
    {
                    ...
                    if (mm.Association.IsForeignKey == isForeignKey)
                    {
                        ...
                        if (value != null)
                        {
                            if (mm.Association.IsMany)
                            {
                                ...
                                {
                                    yield return new RelatedItem(
                                        otherType.GetInheritanceType(otherItem.GetType()),
                                        otherItem
                                    );
                                }
                            }
                            ...
                            {
                                yield return new RelatedItem(
                                    otherType.GetInheritanceType(value.GetType()),
                                    value
                                );
                            }
                        }
        ...
        internal Expression GetObjectQuery(MetaType type, Expression[] keyValues)
        {
            ...
            for (int i = 0, n = type.IdentityMembers.Count; i < n; i++)
            {
                ...
                Expression memberEqualityExpression = Expression.Equal(
                    memberExpression,
                    keyValues[i]
                );
                whereExpression =
        ...
        internal Expression GetDataMemberQuery(MetaDataMember member, Expression[] keyValues)
        {
            ...
            if (member.IsAssociation)
            {
                ...
                Expression thisInstance = Expression.Call(
                    typeof(Enumerable),
                    "FirstOrDefault",
                    new Type[] { association.ThisMember.DeclaringType.Type },
                ...
                {
                    otherSource = Expression.Call(
                        typeof(Enumerable),
                        "Cast",
                        new Type[] { association.OtherType.Type },
                        otherSource
                    );
                }
            ...
            else
            {
                ...
                return Expression.Call(
                    typeof(Queryable),
                    "Select",
                    new Type[] { elementType, selector.Body.Type },
                    query,
                    selector
                );
            }
        ...
        private static Expression[] BuildKeyExpressions(
            object[] keyValues,
            ReadOnlyCollection<MetaDataMember> keyMembers
        )
        {
        ...
        public IDeferredSourceFactory GetDeferredSourceFactory(MetaDataMember member)
        {
            ...
            Type elemType =
                member.IsAssociation && member.Association.IsMany
        ...
        class DeferredSourceFactory<T> : IDeferredSourceFactory
        {
            ...
            private bool TryGetCachedObject(object[] keyValues, out T cached)
            {
                ...
                if (this.refersToPrimaryKey)
                {
                    ...
                    MetaType mt = this.member.IsAssociation
                        ? this.member.Association.OtherType
...
#pragma warning restore 618
        ...
        {
            get { return this.hasCachedObjects; }
        }
        ...
        internal object[] GetKeyValues(MetaType type, LambdaExpression predicate)
        {
            ...
            if (
                this.GetKeysFromPredicate(type, keys, predicate.Body)
                && keys.Count == type.IdentityMembers.Count
            )
            {
                object[] values = keys.OrderBy(kv => kv.Key.Ordinal)
                    .Select(kv => kv.Value)
                    .ToArray();
                return values;
            }
        ...
        private bool GetKeysFromPredicate(
            MetaType type,
            Dictionary<MetaDataMember, object> keys,
            Expression expr
        )
        {
        ...
        )
        {
            ...
            if (
                memex == null
                || memex.Expression == null
                || memex.Expression.NodeType != ExpressionType.Parameter
                || memex.Expression.Type != type.Type
            )
            {
                return false;
            }
            if (
                !type.Type.IsAssignableFrom(memex.Member.ReflectedType)
                && !memex.Member.ReflectedType.IsAssignableFrom(type.Type)
            )
            {
                return false;
            }
