...
namespace System.Data.Linq
{
    ...
    internal abstract class ChangeTracker
    {
        ...
        internal static ChangeTracker CreateChangeTracker(
            CommonDataServices dataServices,
            bool asReadOnly
        )
        {
            if (asReadOnly)
        ...
        class StandardChangeTracker : ChangeTracker
        {
            ...
            {
                List<StandardTrackedObject> list = new List<StandardTrackedObject>(
                    (IEnumerable<StandardTrackedObject>)this.items.Values
                );
            ...
            class StandardTrackedObject : TrackedObject
            {
                ...
                internal StandardTrackedObject(
                    StandardChangeTracker tracker,
                    MetaType type,
                    object current,
                    object original
                )
                ...
                internal override bool IsPossiblyModified
                {
                    get
                    {
                        return this.state == State.Modified || this.state == State.PossiblyModified;
                    }
                }
                ...
                internal override void Refresh(RefreshMode mode, object freshInstance)
                {
                    ...
                    {
                        var memberMode = mm.IsDbGenerated
                        ...
                        if (memberMode != RefreshMode.KeepCurrentValues)
                        {
                            if (
                                ...
                                && (
                                    ...
                                    || mm.DeclaringType.Type.IsAssignableFrom(instanceType)
                                )
                            )
                            {
                ...
                )
                {
                    ...
                    if (!object.Equals(freshValue, currentValue))
                    {
                        ...
                        foreach (MetaDataMember am in this.GetAssociationsForKey(mm))
                        {
                            ...
                            {
                                IEnumerable ds = this.tracker.services
                                ...
                                if (am.StorageAccessor.HasValue(this.current))
                                {
                                    this.AssignMember(
                                        this.current,
                                        am,
                                        ds.Cast<Object>().SingleOrDefault()
                                    );
                                }
                            }
                        }
                ...
                internal override bool SynchDependentData()
                {
                            ...
                            if (hasAssigned || hasLoaded)
                            {
                                ...
                                else if (assoc.IsNullable)
                                {
                                    if (
                                        ...
                                        || (
                                            this.original != null
                                            && mm.MemberAccessor.GetBoxedValue(this.original)
                                                != null
                                        )
                                    )
                                    {
                    ...
                    if (this.type.HasInheritance)
                    {
                        if (this.original != null)
                        {
                            ...
                            {
                                throw Error.CannotChangeInheritanceType(
                                    dbDiscriminator,
                                    currentDiscriminator,
                                    original.GetType().Name,
                                    currentTypeFromDiscriminator
                                );
                            }
                        }
                        else
                    ...
                    foreach (MetaDataMember mm in this.type.PersistentDataMembers)
                    {
                        if (this.IsModifiedMember(mm))
                        {
                            ...
                            if (this.original != null && mm.StorageAccessor.HasValue(this.original))
                            {
                                ...
                                || (
                                    mm.IsDeferred
                                    && !mm.StorageAccessor.HasLoadedValue(this.current)
                                )
                            )
                            {
                ...
                internal override bool HasDeferredLoaders
                {
                    get
                    {
                        ...
                        IEnumerable<MetaDataMember> deferredMembers =
                            this.Type.PersistentDataMembers.Where(
                        ...
                        if (assoc.IsForeignKey)
                        {
                            ...
                            if (index > -1)
                            {
                                ...
                                if (otherItem != null)
                                {
                                    ...
                                    {
                                        StandardTrackedObject trackedOther = (StandardTrackedObject)
                                            this.tracker.GetTrackedObject(otherItem);
                                        if (trackedOther != null)
                                        {
                                            MetaDataMember otherMember = assoc.OtherKey[index];
                                            return trackedOther.IsMemberPendingGeneration(
                                                otherMember
                                            );
                                        }
                                    }
