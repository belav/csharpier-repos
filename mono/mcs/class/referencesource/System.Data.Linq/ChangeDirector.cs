    ...
    internal abstract class ChangeDirector
    {
        ...
        internal class StandardChangeDirector : ChangeDirector
        {
            ...
            [SuppressMessage(
                ...
                Justification = "Microsoft: FxCop bug Dev10:423110 -- List<KeyValuePair<object, object>> is not supposed to be flagged as a violation."
            )]
            ...
            internal override int DynamicInsert(TrackedObject item)
            {
                ...
                else
                {
                    IEnumerable<object> facts =
                        (IEnumerable<object>)this.context.Provider.Execute(cmd).ReturnValue;
                    ...
                    if (syncResults != null)
                    {
                        ...
                        AutoSyncMembers(
                            ...
                            item,
                            UpdateType.Insert,
                            AutoSyncBehavior.ApplyNewAutoSync
                        );
                        return 1;
                    }
                    else
                ...
                if (this.syncRollbackItems != null)
                {
                    foreach (
                        KeyValuePair<
                            TrackedObject,
                            object[]
                        > rollbackItemPair in this.SyncRollbackItems
                    )
                    {
            ...
            private Expression GetInsertCommand(TrackedObject item)
            {
                ...
                if (membersToSync.Count > 0)
                {
                    ...
                    return Expression.Call(
                        typeof(DataManipulation),
                        "Insert",
                        new Type[] { item.Type.InheritanceRoot.Type, resultSelector.Body.Type },
                ...
                else
                {
                    return Expression.Call(
                        typeof(DataManipulation),
                        "Insert",
                        new Type[] { item.Type.InheritanceRoot.Type },
                        Expression.Constant(item.Current)
                    );
                }
            }
            ...
            private Expression CreateAutoSync(List<MetaDataMember> membersToSync, Expression source)
            {
                ...
                {
                    initializers[i++] = Expression.Convert(
                        this.GetMemberExpression(source, mm.Member),
                        typeof(object)
                    );
                }
            ...
            )
            {
                ...
                foreach (
                    MetaDataMember metaMember in metaType.PersistentDataMembers.OrderBy(
                        m => m.Ordinal
                    )
                )
                {
                    ...
                    if (
                        (
                            ...
                            && metaMember.AutoSync == AutoSync.OnInsert
                        )
                        || (
                            updateType == UpdateType.Update
                            && metaMember.AutoSync == AutoSync.OnUpdate
                        )
                ...
                if (membersToSync.Count > 0)
                {
                    ...
                    if (check != null)
                    {
                        return Expression.Call(
                            typeof(DataManipulation),
                            "Update",
                            new Type[] { rowTypeRoot.Type, resultSelector.Body.Type },
            ...
            private Expression GetDeleteVerificationCommand(TrackedObject tracked)
            {
                ...
                Expression pred = Expression.Lambda(
                    Expression.Equal(p, Expression.Constant(tracked.Current)),
                    p
                );
                ...
                Expression select = Expression.Call(
                    typeof(Queryable),
                    "Select",
                    new Type[] { table.ElementType, typeof(int?) },
                    where,
                    selector
                );
                Expression singleOrDefault = Expression.Call(
                    typeof(Queryable),
                    "SingleOrDefault",
                    new Type[] { typeof(int?) },
                    select
