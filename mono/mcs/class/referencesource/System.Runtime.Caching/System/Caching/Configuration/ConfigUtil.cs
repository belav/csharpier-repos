...
namespace System.Runtime.Caching.Configuration
{
    internal static class ConfigUtil
    {
        ...
        )
        {
            ...
            int iValue;
            if (!Int32.TryParse(sValue, out iValue) || iValue < 0 || (!zeroAllowed && iValue == 0))
            {
                if (zeroAllowed)
                {
                    throw new ArgumentException(
                        RH.Format(R.Value_must_be_non_negative_integer, valueName, sValue),
                        "config"
                    );
                }

                throw new ArgumentException(
                    RH.Format(R.Value_must_be_positive_integer, valueName, sValue),
                    "config"
                );
            }
        ...
        )
        {
            ...
            {
                throw new ArgumentException(
                    RH.Format(R.TimeSpan_invalid_format, valueName, sValue),
                    "config"
                );
            }
            ...
            int iValue =
                (milliseconds < (double)Int32.MaxValue) ? (int)milliseconds : Int32.MaxValue;
            return iValue;
        }
        ...
        )
        {
            ...
            {
                throw new ArgumentException(
                    RH.Format(R.Value_must_be_boolean, valueName, sValue),
                    "config"
                );
            }
