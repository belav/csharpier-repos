...
namespace System.Runtime
{
    ...
    class IOThreadScheduler
    {
        ...
        bool ScheduleCallbackHelper(Action<object> callback, object state)
        {
            ...
            bool wrapped;
            bool queued = this.slots[slot >> Bits.HiShift & SlotMask].TryEnqueueWorkItem(
                callback,
                state,
                out wrapped
            );
        ...
        bool ScheduleCallbackLowPriHelper(Action<object> callback, object state)
        {
            ...
            bool wrapped;
            bool queued = this.slotsLowPri[
                slot >> Bits.HiShift & SlotMaskLowPri
            ...
            while (true)
            {
                ...
                if (wasEmpty)
                {
                    ...
                    while (Bits.CountNoIdle(slotLowPri) != 0)
                    {
                        ...
                        {
                            this.overlapped.Post(this);
                            this.slotsLowPri[slotLowPri & SlotMaskLowPri].DequeueWorkItem(
                                out callback,
                                out state
                            );
                            return;
