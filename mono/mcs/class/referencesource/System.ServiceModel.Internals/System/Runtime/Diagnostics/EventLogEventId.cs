...
{
    // When adding an EventLogEventId, an entry must also be added to
    ...
    //     Hex digit  1   ('a')    : Severity : a=0 for Success, a=4 for Informational, a=8 for Warning,
    // a=c for Error
    //     Hex digits 2-4 ('bbb')  : Facility : bbb=001 for Tracing, bbb=002 for ServiceModel, bbb=003
    // for TransactionBridge, bbb=004 for SMSvcHost, bbb=005 for Info_Cards, bbb=006 for Security_Audit
    //     Hex digits 5-8 ('cccc') : Code     : Each event within the same facility is assigned a unique
    // "code".
    // The EventId generated from EventLog.mc must match the EventId assigned here.
    // Order is important: The order here must match the order of strings in
    ...
    //         - Each subsequent event within this facility, regardless of severity, is assigned the
    // value of the previous event incremented by 1.
    //     Thus, if you add an EventLogEventId below, you must ensure that its code is equal to the code
    // of the previous EventLogEventId in its facility incremented by 1.
    // The Severity and Facility assigned in EventLog.mc must match those assigned via the EventId here.
    // If the EventId's do not match, the EventViewer will not be able to display the strings defined in
    // EventLog.mc correctly.  In this case, the following error message will be included in the logged
    // event:
    //     "The description for Event ID XX from source System.ServiceModel 4.0.0.0 cannot be found..."
    // To inspect the value assigend to the enum elements below, build
    // 'ndp\cdf\src\System.ServiceModel.Internals', and inspect
    // System.ServiceModel.Internals\System.ServiceModel.Internals.asmmeta
    // To inspect the EventId generated from EventLog.mc, build 'ndp\cdf\src\WCF\EventLog', and open
    // ServiceModelEvents.dll.mui with \\indigofs\PrivateLabDebugShare\sarada\RPFRecorder\RPFRecorder.exe
    // (convert EventId from decimal to hex).
    // You could also use any other method of viewing ServiceModelEvents.dll.mui which would allow you
    // to inspect the EventId
    enum EventLogEventId : uint
    {
        // EventIDs from shared Diagnostics and Reliability code
        // All EventId's beneath 'FailedToSetupTracing', until the next explicitly assigned one, inherit its
        // severity and facility, via the enum's auto-incrememt
        ...
        // All EventId's beneath 'WebHostUnhandledException', until the next explicitly assigned one,
        // inherit its severity and facility, via the enum's auto-incrememt
        ...
        // Assigning code 0x001c to this EventId because it is the 28th (0x001c) EventId with Facility =
        // ServiceModel.
        ...
        // TransactionBridge
        // All EventId's beneath 'UnhandledStateMachineExceptionRecordDescription', until the next
        // explicitly assigned one, inherit its severity and facility, via the enum's auto-incrememt
        ...
        // SMSvcHost
        // All EventId's beneath 'StartErrorPublish', until the next explicitly assigned one, inherit its
        // severity and facility, via the enum's auto-incrememt
