...
namespace System.Web
{
    ...
    internal class ImplicitAsyncPreloadModule
    {
        ...
        internal void GetEventHandlers(
            out BeginEventHandler beginHandler,
            out EndEventHandler endHandler
        )
        {
        ...
        private IAsyncResult OnEnter(Object sender, EventArgs e, AsyncCallback cb, Object state)
        {
            ...
            if (
                asyncPreloadMode == AsyncPreloadModeFlags.None
                ...
                || (
                    (
                        isForm = StringUtil.StringStartsWithIgnoreCase(
                            request.ContentType,
                ...
                || (
                    (
                        isFormMultiPart = StringUtil.StringStartsWithIgnoreCase(
                            request.ContentType,
                            "multipart/form-data"
                        )
                    )
                    && (asyncPreloadMode & AsyncPreloadModeFlags.FormMultiPart)
                        != AsyncPreloadModeFlags.FormMultiPart
                )
                || !isForm
                    && !isFormMultiPart
                    && (asyncPreloadMode & AsyncPreloadModeFlags.NonForm)
                        != AsyncPreloadModeFlags.NonForm
            )
            {
            ...
            try
            {
                ...
                do
                {
                    IAsyncResult readAsyncResult = _inputStream.BeginRead(
                        buffer,
                        0,
                        buffer.Length,
                        _callback,
        ...
        private void OnAsyncCompletion(IAsyncResult readAsyncResult)
        {
            ...
            try
            {
                ...
                while (bytesRead != 0)
                {
                    readAsyncResult = _inputStream.BeginRead(
                        buffer,
                        0,
                        buffer.Length,
                        _callback,
