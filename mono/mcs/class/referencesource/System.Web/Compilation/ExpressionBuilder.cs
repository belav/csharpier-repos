...
namespace System.Web.Compilation
{
    ...
    public abstract class ExpressionBuilder
    {
        ...
        internal virtual void BuildExpression(
            ...
            ref bool hasTempObject
        )
        {
            CodeExpression codeExpression = GetCodeExpression(
                bpe,
                bpe.ParsedExpressionData,
                new ExpressionBuilderContext(controlBuilder.VirtualPath)
            );

            CodeDomUtility.CreatePropertySetStatements(
                methodStatements,
                statements,
                controlReference,
                bpe.Name,
        ...
        internal static ExpressionBuilder GetExpressionBuilder(
        ...
        )
        {
            ...
            if (host != null)
            {
                ...
                {
                    config =
                        webapp.OpenWebConfiguration(true).GetSection("system.web/compilation")
            ...
            System.Web.Configuration.ExpressionBuilder builder = config.ExpressionBuilders[
                expressionPrefix
            ];
            if (builder == null)
            {
                throw new HttpParseException(
                    SR.GetString(SR.InvalidExpressionPrefix, expressionPrefix)
                );
            }
            ...
            if (host != null)
            {
                ...
                ITypeResolutionService ts = (ITypeResolutionService)
            ...
            {
                throw new HttpParseException(
                    SR.GetString(SR.ExpressionBuilder_InvalidType, expressionBuilderType.FullName)
                );
            }
            ExpressionBuilder expressionBuilder = (ExpressionBuilder)
                HttpRuntime.FastCreatePublicInstance(expressionBuilderType);
        ...
        public virtual object ParseExpression(
            string expression,
            Type propertyType,
