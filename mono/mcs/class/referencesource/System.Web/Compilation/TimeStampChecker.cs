...
namespace System.Web.Compilation
{
    ...
    internal class TimeStampChecker
    {
        ...
        private static TimeStampChecker Current
        {
            get
            {
                TimeStampChecker tsc =
                    (TimeStampChecker)
                        System.Runtime.Remoting.Messaging.CallContext.GetData(CallContextSlotName)
                    as TimeStampChecker;
        ...
        private void AddFileInternal(string virtualPath, string path)
        {
            ...
            if (_timeStamps.Contains(virtualPath))
            {
                ...
                if (storedValue == DateTime.MaxValue)
                {
                    Debug.Trace(
                        "TimeStampChecker",
                        "AddFileInternal: Same time stamp (" + path + ")"
                    );
                    return;
                }
                ...
                if (storedValue != lastWriteTimeUtc)
                {
                    ...
                    Debug.Trace(
                        "TimeStampChecker",
                        "AddFileInternal: Changed time stamp (" + path + ")"
                    );
                }
            }
            else
        ...
        private bool CheckFilesStillValidInternal(string key, ICollection virtualPaths)
        {
            ...
            foreach (string virtualPath in virtualPaths)
            {
                ...
                if (lastWriteTimeUtc != storedValue)
                {
                    Debug.Trace(
                        "TimeStampChecker",
                        "CheckFilesStillValidInternal: File (" + path + ") has changed!"
                    );
