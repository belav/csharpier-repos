...
namespace System.Web.Compilation
{
    ...
    public class AssemblyBuilder
    {
        ...
        {
            string generatedFilePath = GetTempFilePhysicalPathWithAssert(
                _codeProvider.FileExtension
            );
        ...
        public void AddCodeCompileUnit(BuildProvider buildProvider, CodeCompileUnit compileUnit)
        {
            ...
            using (new ProcessImpersonationContext())
            {
                ...
                try
                {
                    _codeProvider.GenerateCodeFromCompileUnit(
                        compileUnit,
                        writer,
        ...
        private void AddCompileWithBuildProvider(
            VirtualPath virtualPath,
            BuildProvider owningBuildProvider
        )
        {
            BuildProvider buildProvider = BuildManager.CreateBuildProvider(
                virtualPath,
                _compConfig,
                _initialReferencedAssemblies,
                true /*failIfUnknown*/
        ...
        private void AddSecurityRulesAttribute()
        {
            ...
            if (trustSection.LegacyCasModel)
            {
                ...
                declaration = new CodeAttributeDeclaration(
                    new CodeTypeReference(attrType),
                    new CodeAttributeArgument(field)
                );
                AddAssemblyAttribute(declaration);
            }
            else
        ...
        private void AddChecksumPragma(BuildProvider buildProvider, CodeCompileUnit compileUnit)
        {
            ...
            {
                pragma.FileName = ErrorFormatter.MakeHttpLinePragma(
                    buildProvider.VirtualPathObject.VirtualPathString
                );
            }
            else
            {
...
#pragma warning restore 618
        ...
        internal CompilerParameters GetCompilerParameters()
        {
            ...
            if (CultureName != null)
            {
                ...
                compilParams.OutputAssembly = Path.Combine(
                    dir,
                    OutputAssemblyName + ".resources.dll"
                );
            }
            else
            {
        ...
        [SuppressMessage(
            ...
            Justification = "Warnings was about use of CompilerParameters.CoreAssemblyFileName which was not set by user supplied string - so okay."
        )]
        ...
        )
        {
            ...
            )
            {
                ...
                AssemblyResolver.GetPathToReferenceAssembly(
                    typeof(string).Assembly,
                    out coreAssemblyFile
                );
            ...
            bool disableObsoleteWarnings =
                !BuildManagerHost.InClientBuildManager
                && compilationSection.DisableObsoleteWarnings;
            ...
            if (codeDomProviderType == typeof(Microsoft.CSharp.CSharpCodeProvider))
            {
                ...
                CodeDomUtility.PrependCompilerOption(
                    compilParams,
                    "/nowarn:" + String.Join(";", noWarnStrings)
                );
            }
        ...
        )
        {
            ...
            if (
                ...
                || !CompilationUtil.IsCompilerVersion35OrAbove(codeDomProviderType)
            )
                return;
        ...
        private static void ProcessBooleanProviderOption(
            string providerOptionName,
            string trueCompilerOption,
            string falseCompilerOption,
        ...
        )
        {
            ...
            if (!providerOptions.TryGetValue(providerOptionName, out providerOptionValue))
                return;
            if (string.IsNullOrEmpty(providerOptionValue))
                throw new System.Configuration.ConfigurationErrorsException(
                    SR.GetString(
                        SR.Property_NullOrEmpty,
                        CompilationUtil.CodeDomProviderOptionPath + providerOptionName
                    )
                );
        ...
        internal CompilerResults Compile()
        {
            ...
            if (context != null)
            {
                if (EtwTrace.IsTraceEnabled(EtwTraceLevel.Verbose, EtwTraceFlags.Infrastructure))
                    EtwTrace.Trace(EtwTraceType.ETW_TYPE_COMPILE_ENTER, context.WorkerRequest);
            }
            ...
            {
                using (
                    IAssemblyPostProcessor postProcessor = (IAssemblyPostProcessor)
        ...
        private void FixUpLinePragmas(CompilerResults results)
        {
            ...
            for (int i = results.Errors.Count - 1; i >= 0; i--)
            {
                ...
                if (File.Exists(physicalPath))
                {
                    ...
                    if (
                        error.Line == TemplateControlCodeDomTreeGenerator.badBaseClassLineMarker
                        || (
                            error.Line
                    ...
                    )
                    {
                        ...
                        error.Line > TemplateControlCodeDomTreeGenerator.badBaseClassLineMarker
                        && error.Line
                            < TemplateControlCodeDomTreeGenerator.badBaseClassLineMarker + 50
                    )
                    {
    ...
    internal class CbmCodeGeneratorBuildProviderHost : AssemblyBuilder
    {
            ...
            : base(compConfig, referencedAssemblies, compilerType, outputAssemblyName)
        {
            ...
            if (Directory.Exists(generatedFilesDir))
            {
                ...
                foreach (FileData fileData in FileEnumerator.Create(generatedFilesDir))
                {
                    ...
                    Debug.Trace(
                        "CbmCodeGeneratorBuildProviderHost",
                        "Deleting " + fileData.FullName
                    );
        ...
        internal override TextWriter CreateCodeFile(
        ...
        )
        {
            ...
            generatedCodeFile = FileUtil.TruncatePathIfNeeded(
                generatedCodeFile,
                10 /*length of extension */
            );

            generatedCodeFile = generatedCodeFile + "." + _codeProvider.FileExtension;
            filename = generatedCodeFile;

            BuildManager.GenerateFileTable[
