...
namespace System.Web.Compilation
{
    ...
    internal class CodeDirectoryCompiler
    {
        ...
        {
            return dirType == CodeDirectoryType.AppResources
                || dirType == CodeDirectoryType.LocalResources;
        }

        internal static Assembly GetCodeDirectoryAssembly(
        ...
        )
        {
            ...
            if (!isDirectoryAllowed)
            {
                ...
                if (Directory.Exists(physicalDir))
                {
                    throw new HttpException(
                        SR.GetString(SR.Bar_dir_in_precompiled_app, virtualDir)
                    );
                }
            }
            ...
            if (result != null)
            {
                ...
                if (result is BuildResultCompiledAssembly)
                {
                    ...
                    BuildResultResourceAssembly buildResultResAssembly =
                        (BuildResultResourceAssembly)result;

                    string newResourcesDependenciesHash = HashCodeCombiner.GetDirectoryHash(
                        virtualDir
                    );

                    // If the resources hash (which includes satellites) is up to date, we're done
                    if (
                        newResourcesDependenciesHash
                        == buildResultResAssembly.ResourcesDependenciesHash
                    )
                        return resultAssembly;
                }
            }
            ...
            {
                throw new HttpException(
                    SR.GetString(SR.Virtual_codedir, virtualDir.VirtualPathString)
                );
            }
            ...
            CodeDirectoryCompiler cdc = new CodeDirectoryCompiler(
                virtualDir,
                dirType,
            ...
            if (results != null)
            {
                ...
                for (; ; )
                {
                    ...
                    Debug.Trace(
                        "CodeDirectoryCompiler",
                        results.PathToAssembly + " is already loaded. Waiting a bit"
                    );
                    ...
                    if (DateTime.UtcNow > waitLimit)
                    {
                        Debug.Trace(
                            "CodeDirectoryCompiler",
                            "Timeout waiting for old assembly to unload: " + results.PathToAssembly
                        );
                        throw new HttpException(
                            SR.GetString(SR.Assembly_already_loaded, results.PathToAssembly)
                        );
                    }
                }
            ...
            {
                result.AddVirtualPathDependencies(
                    new SingleObjectCollection(virtualDir.AppRelativeVirtualPathString)
                );
            }
        ...
        private void AddFolderLevelBuildProviders(
            VirtualDirectory vdir,
            FolderLevelBuildProviderAppliesTo appliesTo
        )
        {
            BuildManager.AddFolderLevelBuildProviders(
                _buildProviders,
                vdir.VirtualPathObject,
                appliesTo,
                _bpc.CompConfig,
                _bpc.ReferencedAssemblies
            );
        }
        ...
        private void ProcessDirectoryRecursive(VirtualDirectory vdir, bool topLevel)
        {
            ...
            {
                AddFolderLevelBuildProviders(
                    vdir,
                    FolderLevelBuildProviderAppliesTo.GlobalResources
                );
            }
            else if (_dirType == CodeDirectoryType.LocalResources)
            {
                AddFolderLevelBuildProviders(
                    vdir,
                    FolderLevelBuildProviderAppliesTo.LocalResources
                );
            }
            else if (
                _dirType == CodeDirectoryType.MainCode || _dirType == CodeDirectoryType.SubCode
            )
            {
            ...
            foreach (VirtualFileBase child in vdir.Children)
            {
                ...
                {
                    if (
                        _onlyBuildLocalizedResources
                        && System.Web.UI.Util.GetCultureName(child.VirtualPath) == null
                    )
                    {
                        continue;
                    }
                ...
                BuildProvider buildProvider = BuildManager.CreateBuildProvider(
                    child.VirtualPathObject,
                    (IsResourceCodeDirectoryType(_dirType))
                        ? BuildProviderAppliesTo.Resources
                ...
                if (
                    _dirType == CodeDirectoryType.LocalResources
                    && buildProvider is BaseResourcesBuildProvider
                )
                {
