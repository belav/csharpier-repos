...
namespace System.Web.Compilation
{
    ...
    internal class ProfileBuildProvider : BuildProvider
    {
        ...
        internal static ProfileBuildProvider Create()
        {
            ...
            buildProvider.SetVirtualPath(
                HttpRuntime.AppDomainAppVirtualPathObject.SimpleCombine("Profile")
            );
        ...
        internal static bool HasCompilableProfile
        {
            get
            {
                ...
                if (
                    ProfileBase.GetPropertiesForCompilation().Count == 0
                    && !ProfileBase.InheritsFromCustomType
            ...
            else
            {
                ...
                if (config != null)
                {
                    ...
                    if (prop != null && prop.Source != null && prop.LineNumber > 0)
                        type.LinePragma = new CodeLinePragma(
                            HttpRuntime.GetSafePath(prop.Source),
                            prop.LineNumber
                        );
                }
            }
            // tell the assemblyBuilder to generate a fast factory for this type
            assemblyBuilder.GenerateTypeFactory( /*ns.Name + "." + */
                ProfileTypeName
            );
                ...
                else
                {
                    ...
                    if (!assemblyBuilder.CodeDomProvider.IsValidIdentifier(grpName))
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Profile_bad_group, grpName),
                            property.FileName,
                            property.LineNumber
                        );
        ...
        ////////////////////////////////////////////////////////////
        ////////////////////////////////////////////////////////////
        private void CreateCodeForProperty(
            AssemblyBuilder assemblyBuilder,
            CodeTypeDeclaration type,
            ProfileNameTypeStruct property
        )
        {
            ...
            if (!assemblyBuilder.CodeDomProvider.IsValidIdentifier(name))
                throw new ConfigurationErrorsException(
                    SR.GetString(SR.Profile_bad_name),
                    property.FileName,
                    property.LineNumber
                );
        ...
        )
        {
            ...
            foreach (string grpProp in grpProps)
            {
                ...
                //                       get { return (string) GetProperty("Color"); }
                //                       set { SetProperty("Color", value); } }
                CreateCodeForProperty(
                    assemblyBuilder,
                    grpType,
                    (ProfileNameTypeStruct)properties[grpProp]
                );
            }
        ...
        private void AddCodeForGetProfileForUser(CodeTypeDeclaration type)
        {
            ...
            CodeMethodReturnStatement returnSatement = new CodeMethodReturnStatement(
                new CodeCastExpression(method.ReturnType, cmie)
            );
            ProfileSection config = MTConfigUtil.GetProfileAppConfig();
            //if (config != null)
            //{
