...
namespace System.Web.Compilation
{
    ...
    internal class BuildProvidersCompiler
    {
        ...
        internal BuildProvidersCompiler(
            VirtualPath configPath,
            bool supportLocalization,
            ...
            foreach (BuildProvider buildProvider in _buildProviders)
            {
                ...
                else
                {
                    if (cultureName != null)
                    {
                        ...
                        AssemblyBuilder satelliteAssemblyBuilder = (AssemblyBuilder)
                            _satelliteAssemblyBuilders[cultureName];
        ...
        internal CompilerResults PerformBuild()
        {
            ...
            if (_satelliteAssemblyBuilders != null)
            {
                int maxConcurrent = Math.Min(
                    _satelliteAssemblyBuilders.Count,
                    CompilationUtil.MaxConcurrentCompilations
                );
                try
                {
                    Parallel.ForEach(
                        _satelliteAssemblyBuilders.Values.Cast<AssemblyBuilder>(),
    ...
    internal class WebDirectoryBatchCompiler
    {
        ...
        internal void Process()
        {
            ...
            if (_parserErrors != null && _parserErrors.Count > 0)
            {
                ...
                HttpParseException newException = new HttpParseException(
                    _firstException.Message,
                    _firstException,
                    _firstException.VirtualPath,
                    _firstException.Source,
        ...
        private void AddBuildProviders(bool retryIfDeletionHappens)
        {
            ...
            if (DiskBuildResultCache.InUseAssemblyWasDeleted)
            {
                ...
                {
                    Debug.Trace(
                        "WebDirectoryBatchCompiler",
                        "Rerunning AddBuildProviders for '"
                            + _vdir.VirtualPath
                            + "' because an assembly was out of date."
                    );
        ...
        private void CacheAssemblyResults(AssemblyBuilder assemblyBuilder, CompilerResults results)
        {
            foreach (BuildProvider buildProvider in assemblyBuilder.BuildProviders)
            {
                ...
                if (
                    !BuildManager.CacheVPathBuildResult(
                ...
                if (results != null)
                {
                    if (DelayLoadType.Enabled)
                    {
                        Debug.Trace(
                            "BuildManager",
                            buildProvider.VirtualPath + " Delay Load Assembly"
                        );
                    }
                    else
                    {
                        Debug.Trace(
                            "BuildManager",
                            buildProvider.VirtualPath + results.CompiledAssembly.EscapedCodeBase
                        );
                    }
                }
                else
        ...
        private void CacheCompileErrors(AssemblyBuilder assemblyBuilder, CompilerResults results)
        {
            ...
            foreach (CompilerError error in results.Errors)
            {
                ...
                BuildProvider buildProvider = assemblyBuilder.GetBuildProviderFromLinePragma(
                    error.FileName
                );
                if (buildProvider == null)
                    continue;
                ...
                CompilerResults newResults = new CompilerResults(
                    null /*tempFiles*/
                );
                ...
                BuildResult result = new BuildResultCompileError(
                    buildProvider.VirtualPathObject,
                    e
                );
        ...
        private void GetBuildResultDependencies()
        {
            ...
            {
                ICollection virtualPathDependencies =
                    buildProvider.GetBuildResultVirtualPathDependencies();
                if (virtualPathDependencies == null)
                    continue;

                foreach (string virtualPathDependency in virtualPathDependencies)
                {
                    BuildProvider dependentBuildProvider = (BuildProvider)
                        _buildProviders[virtualPathDependency];
        ...
        private void ProcessDependencies()
        {
                        ...
                        foreach (BuildProvider child in curnode.BuildProviderDependencies)
                        {
                            if (depth.ContainsKey(child))
                            {
                                if (maxdepth <= (int)depth[child])
                                    ...
                                    throw new HttpException(
                                        SR.GetString(SR.File_Circular_Reference, child.VirtualPath)
                                    );
                            }
                            else
                            {
            ...
            for (
                IDictionaryEnumerator en = (IDictionaryEnumerator)depth.GetEnumerator();
                en.MoveNext();
            ...
            foreach (ICollection buildProviders in _nonDependentBuckets)
            {
                ...
                Debug.Trace(
                    "BuildManager",
                    "Bucket " + i + " contains " + buildProviders.Count + " files"
                );
        ...
        private bool CompileNonDependentBuildProviders(ICollection buildProviders)
        {
            ...
            foreach (BuildProvider buildProvider in buildProviders)
            {
                ...
                if (compilerType == null)
                {
                    ...
                    if (
                        defaultAssemblyBuilder == null
                        || defaultAssemblyBuilder.IsBatchFull
                ...
                )
                {
                    ...
                    if (defaultAssemblyBuilder == null || defaultAssemblyBuilder == assemblyBuilder)
                    {
                ...
                foreach (BuildProvider languageFreeBuildProvider in languageFreeBuildProviders)
                {
                    ...
                    )
                    {
                        ...
                        defaultAssemblyBuilder = CompilerType.GetDefaultAssemblyBuilder(
                            _compConfig,
                            _referencedAssemblies,
                            _vdir.VirtualPathObject /*configPath*/
                            ,
                            null /*outputAssemblyName*/
                        );
        ...
        private void CompileAssemblyBuilderParallel(ICollection assemblyBuilders)
        {
            ...
            else
            {
                ...
                ConcurrentDictionary<AssemblyBuilder, CompilerResults> buildResults =
                    new ConcurrentDictionary<AssemblyBuilder, CompilerResults>();
                ConcurrentDictionary<AssemblyBuilder, CompilerResults> buildErrors =
                    new ConcurrentDictionary<AssemblyBuilder, CompilerResults>();

                try
                {
                    Parallel.ForEach(
                        assemblyBuilders.Cast<AssemblyBuilder>(),
                        new ParallelOptions { MaxDegreeOfParallelism = maxConcurrent },
                        builder =>
                        {
