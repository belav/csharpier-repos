...
namespace System.Web.Compilation
{
    ...
    internal class PreservationFileReader
    {
        ...
        internal BuildResult ReadBuildResultFromFile(
        ...
        )
        {
            ...
            {
                Debug.Trace(
                    "PreservationFileReader",
                    "Can't find preservation file " + Path.GetFileName(preservationFile)
                );
                return null;
            }
        ...
        [SuppressMessage(
            "Microsoft.Security",
            "MSEC1207:UseXmlReaderForLoad",
            Justification = "Xml file is created by us and only accessible to admins."
        )]
        [SuppressMessage(
        ...
        private BuildResult ReadFileInternal(
            VirtualPath virtualPath,
            string preservationFile,
            long hashCode,
            bool ensureIsUpToDate
        )
        {
            ...
            Debug.Assert(
                _root != null && _root.Name == "preserve",
                "_root != null && _root.Name == \"preserve\""
            );
            ...
            BuildResultTypeCode resultTypeCode = (BuildResultTypeCode)
                Int32.Parse(resultTypeCodeString, CultureInfo.InvariantCulture);
            ...
            if (!_precompilationMode)
            {
                ...
                if (!result.IsUpToDate(virtualPath, ensureIsUpToDate))
                {
                    Debug.Trace(
                        "PreservationFileReader",
                        Path.GetFileName(preservationFile) + " is out of date (IsUpToDate==false)"
                    );
                ...
                else
                {
                    ...
                    if (currentHash == 0 || currentHash != savedHash)
                    {
                        ...
                        Debug.Trace(
                            "PreservationFileReader",
                            Path.GetFileName(preservationFile) + " is out of date (ComputeHashCode)"
                        );
                    }
                }
            ...
            while (childEnumerator.MoveNext())
            {
                ...
                switch (dependenciesNode.Name)
                {
                    case PreservationFileWriter.fileDependenciesTagName:
                        ...
                        _sourceDependencies = ReadDependencies(
                            dependenciesNode,
