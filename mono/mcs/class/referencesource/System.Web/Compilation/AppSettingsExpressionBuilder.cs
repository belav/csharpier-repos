...
namespace System.Web.Compilation
{
    ...
    [ExpressionEditor(
        "System.Web.UI.Design.AppSettingsExpressionEditor, " + AssemblyRef.SystemDesign
    )]
    public class AppSettingsExpressionBuilder : ExpressionBuilder
    {
        ...
        public static object GetAppSetting(string key, Type targetType, string propertyName)
        {
            ...
            if (targetType != null)
            {
                PropertyDescriptor propDesc = TypeDescriptor.GetProperties(targetType)[
                    propertyName
                ];
                if (propDesc != null)
                {
                    if (propDesc.PropertyType != typeof(string))
                    {
                        ...
                        else
                        {
                            throw new InvalidOperationException(
                                SR.GetString(
                                    SR.AppSetting_not_convertible,
                                    value,
                                    propDesc.PropertyType.Name,
                                    propDesc.Name
                                )
                            );
                        }
