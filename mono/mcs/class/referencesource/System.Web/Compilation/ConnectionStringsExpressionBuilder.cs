...
namespace System.Web.Compilation
{
    ...
    [ExpressionEditor(
        "System.Web.UI.Design.ConnectionStringsExpressionEditor, " + AssemblyRef.SystemDesign
    )]
    public class ConnectionStringsExpressionBuilder : ExpressionBuilder
    {
        ...
        )
        {
            ...
            if (expression != null)
            {
                ...
                {
                    name = expression.Substring(
                        0,
                        expression.Length - connectionStringSuffix.Length
                    );
                }
                else
                {
                    if (expression.EndsWith(providerNameSuffix, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        name = expression.Substring(
                            0,
                            expression.Length - providerNameSuffix.Length
                        );
                    }
                    else
                    {
