...
#pragma warning restore 0649
        ...
        internal static BuildResult CreateBuildResultFromCode(
            BuildResultTypeCode code,
        ...
        internal void EnsureVirtualPathDependenciesHashComputed()
        {
            if (!DependenciesHashComputed)
            {
                ...
                _virtualPathDependenciesHash = ComputeSourceDependenciesHashCode(
                    null /*virtualPath*/
                );
        ...
        internal bool IsUpToDate(VirtualPath virtualPath, bool ensureIsUpToDate)
        {
            ...
            if (now < _nextUpToDateCheck && !BuildManagerHost.InClientBuildManager)
            {
                Debug.Trace(
                    "BuildResult",
                    "IsUpToDate: true since called less than 2 seconds ago. "
                        + _nextUpToDateCheck
                        + ","
                        + now
                );
            ...
            if (Interlocked.CompareExchange(ref _lock, 1, 0) != 0)
            {
                Debug.Trace(
                    "BuildResult",
                    "IsUpToDate returning true because it didn't get the lock"
                );
                return true;
            }
    ...
    internal class BuildResultCompileError : BuildResult
    {
        ...
        {
            get { return _compileException; }
        }

        internal BuildResultCompileError(
            VirtualPath virtualPath,
    ...
    internal abstract class BuildResultCompiledAssemblyBase : BuildResult
    {
        ...
        {
            get { return (ResultAssembly == null); }
        }
        ...
        {
            get { return ResultAssembly.GlobalAssemblyCache; }
        }
        ...
        internal static Assembly GetPreservedAssembly(PreservationFileReader pfr)
        {
            ...
            catch
            {
                Debug.Trace(
                    "BuildResult",
                    "GetPreservedAssembly: couldn't load assembly '"
                        + assemblyName
                ...
                if (tmpAppInitializeMethod != null)
                {
                    ...
                    if (_appInitializeMethod != null)
                    {
                        throw new HttpException(
                            SR.GetString(
                                SR.Duplicate_appinitialize,
                                _appInitializeMethod.ReflectedType.FullName,
                                t.FullName
                            )
                        );
                    }
    ...
    internal class BuildResultCompiledType : BuildResultCompiledAssemblyBase, ITypedWebObjectFactory
    {
        ...
        public object CreateInstance()
        {
            ...
            {
                _instObj = ObjectFactoryCodeDomTreeGenerator.GetFastObjectCreationDelegate(
                    ResultType
                );
    ...
    internal abstract class BuildResultNoCompileTemplateControl
        : BuildResult,
    ...
    internal class BuildResultNoCompilePage : BuildResultNoCompileTemplateControl
    {
            ...
            : base(baseType, parser)
        {
            ...
            if (pageParser.OutputCacheParameters != null)
            {
                ...
                if (
                    _outputCacheData.Duration == 0
                    || _outputCacheData.Location == OutputCacheLocation.None
                )
                {
    ...
    internal class BuildResultNoCompileMasterPage : BuildResultNoCompileUserControl
    {
        ...
        public override object CreateInstance()
        {
            ...
            {
                masterPage.ContentPlaceHolders.Add(
    ...
    */
    internal class BuildResultCodeCompileUnit : BuildResult
    {
        ...
        internal override void GetPreservedAttributes(PreservationFileReader pfr)
        {
            ...
            _ccuPreservationFileName = Path.Combine(
        ...
        internal override void SetPreservedAttributes(PreservationFileWriter pfw)
        {
            ...
            preservationFileName = Path.Combine(
                HttpRuntime.CodegenDirInternal,
        ...
        internal override void RemoveOutOfDateResources(PreservationFileReader pfr)
        {
            ...
            ccuPreservationFileName = Path.Combine(
                HttpRuntime.CodegenDirInternal,
