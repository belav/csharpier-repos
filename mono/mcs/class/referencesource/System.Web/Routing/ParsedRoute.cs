namespace System.Web.Routing
{
    ...
    internal sealed class ParsedRoute
    {
        ...
        )
        {
            ...
            ForEachParameter(
                PathSegments,
                delegate(ParameterSubsegment parameterSubsegment)
                {
            ...
            foreach (var currentValue in currentValues)
            {
                ...
                {
                    ParameterSubsegment parameterSubsegment = GetParameterSubsegment(
                        PathSegments,
                        parameterName
                    );
                        ...
                        foreach (PathSubsegment subsegment in contentPathSegment.Subsegments)
                        {
                            ...
                            else
                            {
                                ParameterSubsegment parameterSubsegment =
                                    subsegment as ParameterSubsegment;
                                if (parameterSubsegment != null)
                                {
                                    if (pendingPartsAreAllSafe)
                                    {
        ...
        )
        {
            ...
            bool continueProcessing = ForEachParameter(
                pathSegments,
                delegate(ParameterSubsegment parameterSubsegment)
                {
                    if (
        ...
        {
            IList<string> requestPathSegments = RouteParser.SplitUrlToPathSegmentStrings(
                virtualPath
            );
        ...
        private void MatchCatchAll(
            ...
            RouteValueDictionary defaultValues,
            RouteValueDictionary matchedValues
        )
        {
            string remainingRequest = String.Join(String.Empty, remainingRequestSegments.ToArray());

            ParameterSubsegment catchAllSegment =
                contentPathSegment.Subsegments[0] as ParameterSubsegment;
        ...
        )
        {
            if (String.IsNullOrEmpty(requestPathSegment))
            {
                ...
                ParameterSubsegment parameterSubsegment =
                    routeSegment.Subsegments[0] as ParameterSubsegment;
                if (parameterSubsegment == null)
                {
                    return false;
                }
            ...
            while (indexOfLastSegmentUsed >= 0)
            {
                ...
                ParameterSubsegment parameterSubsegment =
                    routeSegment.Subsegments[indexOfLastSegmentUsed] as ParameterSubsegment;
                if (parameterSubsegment != null)
                {
                ...
                else
                {
                    LiteralSubsegment literalSubsegment =
                        routeSegment.Subsegments[indexOfLastSegmentUsed] as LiteralSubsegment;
                    if (literalSubsegment != null)
                    {
        ...
        private static string UrlEncode(string str)
        {
            ...
            return Regex.Replace(
                escape,
                "([#;?:@&=+$,])",
                new MatchEvaluator(EscapeReservedCharacters)
            );
        }
    }
}
