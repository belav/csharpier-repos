    ...
    public class ProfileBase : SettingsBase
    {
                ...
                if (p != null)
                {
                    ...
                    if (!fAllowAnonymous)
                        throw new ProviderException(
                            SR.GetString(SR.Profile_anonoymous_not_allowed_to_set_property)
                        );
                }
            }
        ...
        public ProfileGroupBase GetProfileGroup(string groupName)
        {
            ...
            if (grp == null)
            {
                ...
                if (t == null)
                    throw new ProviderException(
                        SR.GetString(SR.Profile_group_not_found, groupName)
                    );
            ...
            get
            {
                ...
                if (!typeof(ProfileBase).IsAssignableFrom(t))
                {
                    ...
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Wrong_profile_base_type),
                        null,
                        config.ElementInformation.Properties["inherits"].Source,
                        config.ElementInformation.Properties["inherit"].LineNumber
                    );
                }
                return t;
            }
        ...
        {
            get { return s_SingletonInstance; }
        }
        ...
        internal static Hashtable GetPropertiesForCompilation()
        {
            ...
            lock (s_InitializeLock)
            {
                ...
                AddProfilePropertySettingsForCompilation(
                    ProfileManager.DynamicProfileProperties,
                    ht,
                    null
                );
        ...
        )
        {
            foreach (ProfilePropertySettings pps in propertyCollection)
            {
                ...
                if (t == null)
                    t = ResolvePropertyTypeForCommonTypes(
        ...
        static private void InitializeStatic()
        {
            ...
            lock (s_InitializeLock)
            {
                ...
                try
                {
                    ProfileSection config = MTConfigUtil.GetProfileAppConfig();
                    bool fAnonEnabled = (
                        HostingEnvironment.IsHosted ? AnonymousIdentificationModule.Enabled : true
                    );
                    ...
                    ProfileBase.AddPropertySettingsFromConfig(
                        baseType,
                        fAnonEnabled,
                        hasLowTrust,
                        ProfileManager.DynamicProfileProperties,
                    ...
                    if (baseType != typeof(ProfileBase))
                    {
                        ...
                        NameValueCollection baseProperties = new NameValueCollection(
                            baseProps.Length
                        );
                        ...
                        {
                            if (baseProperties[prop.Name] == null)
                            { //not in the base class
                                ...
                                SettingsAttributeDictionary settings =
                                ...
                                SettingsProperty sp = new SettingsProperty(
                                    prop.Name,
                                    prop.PropertyType,
                                    prov,
                                    readOnly,
        ...
        )
        {
            foreach (ProfilePropertySettings pps in settingsCollection)
            {
                ...
                if (baseType != typeof(ProfileBase) && s_Properties[name] != null)
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Profile_property_already_added),
                        null,
                        pps.ElementInformation.Properties["name"].Source,
                        pps.ElementInformation.Properties["name"].LineNumber
                    );
                ...
                {
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Profile_could_not_create_type, e.Message),
                        e,
                        pps.ElementInformation.Properties["type"].Source,
                        pps.ElementInformation.Properties["type"].LineNumber
                    );
                }
                if (!fAnonEnabled)
                {
                    ...
                    if (fAllowAnonymous)
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Annoymous_id_module_not_enabled, pps.Name),
                            pps.ElementInformation.Properties["allowAnonymous"].Source,
                            pps.ElementInformation.Properties["allowAnonymous"].LineNumber
                        );
                }
                if (hasLowTrust)
                {
                ...
                if (
                    requireSerializationCheck
                    && pps.SerializeAs == SerializationMode.Binary
                    && !pps.TypeInternal.IsSerializable
                )
                {
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Property_not_serializable, pps.Name),
                        pps.ElementInformation.Properties["serializeAs"].Source,
                        pps.ElementInformation.Properties["serializeAs"].LineNumber
                    );
                }
                ...
                SettingsProperty sp = new SettingsProperty(
                    ...
                    pps.ReadOnly,
                    pps.DefaultValue,
                    (SettingsSerializeAs)pps.SerializeAs,
                    settings,
                    false,
                    true
                );
        ...
        static private void SetProviderForProperty(ProfilePropertySettings pps)
        {
            ...
            if (pps.ProviderInternal == null)
                throw new ConfigurationErrorsException(
                    SR.GetString(SR.Profile_provider_not_found, pps.Provider),
                    pps.ElementInformation.Properties["provider"].Source,
                    pps.ElementInformation.Properties["provider"].LineNumber
                );
        }
