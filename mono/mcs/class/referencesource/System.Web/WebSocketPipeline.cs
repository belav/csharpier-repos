...
namespace System.Web
{
    ...
    internal sealed class WebSocketPipeline : IDisposable, ISyncContext
    {
        ...
        [SuppressMessage(
            ...
            MessageId = "System.Web.Hosting.UnsafeIISMethods.MgdPostCompletion(System.IntPtr,System.Web.RequestNotificationStatus)",
            Justification = @"This will never return an error HRESULT."
        )]
        public void ProcessRequest()
        {
            ...
            Task abortTask = webSocketTask
                .ContinueWith(
                    task =>
                        (task.Result != null)
                            ? ((AspNetWebSocket)task.Result).AbortAsync()
                            : (Task)null,
            ...
            abortTask.ContinueWith(
                _ =>
                    UnsafeIISMethods.MgdPostCompletion(
                        _root.WorkerRequest.RequestContext,
        ...
        private async Task<AspNetWebSocket> ProcessRequestImplAsync()
        {
            ...
            try
            {
                ...
                UnmanagedWebSocketContext unmanagedWebSocketContext =
                    _root.WorkerRequest.GetWebSocketContext();
                WebSocketPipe pipe =
                    (unmanagedWebSocketContext != null)
                        ? new WebSocketPipe(unmanagedWebSocketContext, PerfCounters.Instance)
                        : null;
                ...
                try
                {
                    ...
                    syncContext.Send(
                        ...
                        {
                            task = _userFunc(
                                new AspNetWebSocketContextImpl(
                                    new HttpContextWrapper(_httpContext),
                                    _root.WorkerRequest,
                                    webSocket
                                )
                            );
