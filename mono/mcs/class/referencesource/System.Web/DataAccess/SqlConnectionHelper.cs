...
namespace System.Web.DataAccess
{
    ...
    internal static class SqlConnectionHelper
    {
        ...
        internal static SqlConnectionHolder GetConnection(
        ...
        internal static string GetConnectionString(
            string specifiedConnectionString,
            bool lookupConnectionString,
            bool appLevel
        )
        {
            System
        ...
        private static void EnsureDBFile(string connectionString)
        {
            ...
            string[] splitedConnStr = connectionString.Split(
                new char[] { ';' },
                StringSplitOptions.RemoveEmptyEntries
            );
            bool lookingForUserInstance = !connectionString
                .ToUpperInvariant()
            ...
            foreach (string str in splitedConnStr)
            {
                ...
                if (lookingForDataDir && strUpper.Contains(s_strUpperDataDirWithToken))
                {
                    ...
                    int startPos =
                        strUpper.IndexOf(s_strUpperDataDirWithToken, StringComparison.Ordinal)
                    ...
                    lookingForDB
                    && (
                        strUpper.StartsWith("INITIAL CATALOG", StringComparison.Ordinal)
                ...
                )
                {
                    ...
                    lookingForTimeout
                    && strUpper.StartsWith("CONNECT TIMEOUT", StringComparison.Ordinal)
                )
                {
            ...
            if (fullFileName == null)
                throw new ProviderException(
                    SR.GetString(SR.SqlExpress_file_not_found_in_connection_string)
                );
            ...
            if (!HttpRuntime.HasAspNetHostingPermission(AspNetHostingPermissionLevel.High))
                throw new ProviderException(
                    SR.GetString(SR.Provider_can_not_create_file_in_this_trust_level)
                );
        ...
        [PermissionSet(SecurityAction.Assert, Unrestricted = true)]
        private static void CreateMdfFile(
            string fullFileName,
            string dataDir,
            string connectionString
        )
        {
            ...
            try
            {
                ...
                if (strippedFileName.Length > 30)
                    databaseName =
                        strippedFileName.Substring(0, 30)
                        + "_"
                ...
                tempFileName = Path.Combine(
                    ...
                    strippedFileName + "_TMP" + s_strSqlExprFileExt
                );
            ...
            catch (Exception e)
            {
                ...
                if (e is UnauthorizedAccessException)
                    httpExec.SetFormatter(
                        new SqlExpressConnectionErrorFormatter(
                            creatingDir
                                ? DataConnectionErrorEnum.CanNotCreateDataDir
                                : DataConnectionErrorEnum.CanNotWriteToDataDir
                        )
                    );
                else
        ...
        internal SqlConnectionHolder(string connectionString)
        {
            ...
            catch (ArgumentException e)
            {
                throw new ArgumentException(
                    SR.GetString(SR.SqlError_Connection_String),
                    "connectionString",
                    e
                );
            }
