    ...
    public sealed class HtmlHead : HtmlGenericControl
    {
        ...
        protected internal override void AddedControl(Control control, int index)
        {
            ...
            else if (control is HtmlMeta)
            {
                ...
                if (
                    _description == null
                    && string.Equals(meta.Name, "description", StringComparison.OrdinalIgnoreCase)
                )
                {
                    _description = meta;
                }
                    ...
                    _keywords == null
                    && string.Equals(meta.Name, "keywords", StringComparison.OrdinalIgnoreCase)
                )
                {
                    _keywords = meta;
                }
        ...
        internal static void RenderCssRule(
            CssTextWriter cssWriter,
            string selector,
            Style style,
        ...
        private sealed class StyleSheetInternal : IStyleSheet, IUrlResolutionService
        {
            ...
            {
                get { return _cssStyleString; }
                set { _cssStyleString = value; }
            }
            ...
            void IStyleSheet.CreateStyleRule(
                Style style,
                IUrlResolutionService urlResolver,
                string selector
            )
            ...
            void IStyleSheet.RegisterStyle(Style style, IUrlResolutionService urlResolver)
            {
                ...
                else if (style.RegisteredCssClass.Length != 0)
                {
                    ...
                    throw new InvalidOperationException(
                        SR.GetString(SR.HtmlHead_StyleAlreadyRegistered)
                    );
                }
