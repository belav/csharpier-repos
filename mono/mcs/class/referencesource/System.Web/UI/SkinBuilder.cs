    ...
    public sealed class SkinBuilder : ControlBuilder
    {
        ...
        private void ApplyTemplateProperties(Control control)
        {
            ...
            foreach (TemplatePropertyEntry entry in entries)
            {
                try
                {
                    object originalValue = FastPropertyAccessor.GetProperty(
                        control,
                        entry.Name,
                        InDesigner
                    );
            ...
            foreach (ComplexPropertyEntry entry in entries)
            {
                ...
                if (builder != null)
                {
                    ...
                    else
                    {
                        ...
                        PropertyDescriptor desc = PropertyMapper.GetMappedPropertyDescriptor(
                            control,
                            PropertyMapper.MapNameToPropertyName(propertyName),
                            out childObj,
                            out actualPropName,
                            InDesigner
                        );
                        if (desc != null)
                        {
                            ...
                            if (
                                value != null
                                && desc.Attributes[typeof(UrlPropertyAttribute)] != null
                ...
                try
                {
                    ...
                    if (desc != null)
                    {
                        DefaultValueAttribute defValAttr = (DefaultValueAttribute)
                            desc.Attributes[typeof(DefaultValueAttribute)];
...
#pragma warning restore 1058
    ...
    public sealed class ThemeProvider
    {
        ...
        )
        {
            ...
            ControlBuilder themeBuilder = DesignTimeTemplateParser.ParseTheme(
                host,
                themeDefinition,
                themePath
            );
