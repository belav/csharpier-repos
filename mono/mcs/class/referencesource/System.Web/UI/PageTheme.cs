    ...
    internal class FileLevelPageThemeBuilder : RootBuilder
    {
            ...
            if (s != null)
            {
                if (!Util.IsWhiteSpaceString(s))
                {
                    throw new HttpException(
                        SR.GetString(
                            SR.Literal_content_not_allowed,
                            SR.GetString(SR.Page_theme_skin_file),
                            s.Trim()
                        )
                    );
                }
        ...
        public override void AppendSubBuilder(ControlBuilder subBuilder)
        {
            ...
            if (!typeof(Control).IsAssignableFrom(ctrlType))
            {
                throw new HttpException(
                    SR.GetString(
                        SR.Page_theme_only_controls_allowed,
                        ctrlType == null ? String.Empty : ctrlType.ToString()
                    )
                );
            }
            ...
            if (InPageTheme && !ThemeableAttribute.IsTypeThemeable(subBuilder.ControlType))
            {
                throw new HttpParseException(
                    SR.GetString(SR.Type_theme_disabled, subBuilder.ControlType.FullName),
                    null,
                    subBuilder.VirtualPath,
                    null,
                    subBuilder.Line
                );
            }
    ...
    public abstract class PageTheme
    {
            ...
            if (LinkedStyleSheets != null && LinkedStyleSheets.Length > 0)
            {
                if (Page.Header == null)
                    throw new InvalidOperationException(
                        SR.GetString(SR.Page_theme_requires_page_header)
                    );
        ...
        private class SkinKey
        {
            ...
            public override bool Equals(object o)
            {
                ...
                return (_typeName == key._typeName) && (_skinID == key._skinID);
            }
        }
    }
}
