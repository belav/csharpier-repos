...
namespace System.Web.UI
{
    ...
    internal class LegacyPageAsyncTaskManager
    {
        ...
        private void WaitForAllStartedTasks(bool syncCaller, bool forceTimeout)
        {
            ...
            for (int i = 0; i < _tasks.Count; i++)
            {
                ...
                if (!forceTimeout && !TimeoutEndReached)
                {
                    ...
                    { // re-check not to wait negative time span
        ...
        private IAsyncResult BeginExecuteAsyncTasks(
            object sender,
            EventArgs e,
            AsyncCallback cb,
            object extraData
                ...
                finally
                {
                    ...
                    catch (SynchronizationLockException)
                    {
                        ...
                        throw new InvalidOperationException(
                            SR.GetString(SR.Async_tasks_wrong_thread)
                        );
                    }
                }
            }
        ...
        private void ResumeTasks(bool waitUntilDone, bool onCallerThread)
        {
            ...
            Debug.Trace(
                "Async",
                "TaskManager.ResumeTasks: onCallerThread="
                    + onCallerThread
                    + ", _tasksCompleted="
                    + _tasksCompleted
                    + ", _tasksStarted="
        ...
        internal void TaskCompleted(bool onCallerThread)
        {
            ...
            Debug.Trace(
                "Async",
                "TaskManager.TaskCompleted: onCallerThread="
                    + onCallerThread
                    + ", _tasksCompleted="
                    + newTasksCompleted
                    + ", _tasksStarted="
        ...
        private void StartTimerIfNeeeded()
        {
            ...
            _timeoutTimer = new Timer(
                new TimerCallback(this.TimeoutTimerCallback),
                null,
                (int)timerPeriod,
                -1
            );
        ...
        internal void DisposeTimer()
        {
            ...
            if (
                timer != null
                && Interlocked.CompareExchange(ref _timeoutTimer, null, timer) == timer
            )
            {
                timer.Dispose();
            }
