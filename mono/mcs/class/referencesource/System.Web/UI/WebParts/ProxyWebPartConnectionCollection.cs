...
namespace System.Web.UI.WebControls.WebParts
{
    ...
    public sealed class ProxyWebPartConnectionCollection : CollectionBase
    {
        ...
        private void CheckReadOnly()
        {
            if (IsReadOnly)
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.ProxyWebPartConnectionCollection_ReadOnly)
                );
            }
        }
        ...
        protected override void OnSet(int index, object oldValue, object newValue)
        {
            ...
            if (_webPartManager != null)
            {
                int webPartManagerIndex = _webPartManager.StaticConnections.IndexOf(
                    (WebPartConnection)oldValue
                );
                // It is a bug if the main WebPartManager does not contain the oldValue
                Debug.Assert(webPartManagerIndex >= 0);
                _webPartManager.StaticConnections[webPartManagerIndex] =
                    (WebPartConnection)newValue;
            }
