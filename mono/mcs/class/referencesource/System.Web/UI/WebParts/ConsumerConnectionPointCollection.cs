...
namespace System.Web.UI.WebControls.WebParts
{
    ...
    public sealed class ConsumerConnectionPointCollection : ReadOnlyCollectionBase
    {
            ...
            foreach (object obj in connectionPoints)
            {
                if (obj == null)
                {
                    throw new ArgumentException(
                        SR.GetString(SR.Collection_CantAddNull),
                        "connectionPoints"
                    );
                }
                ...
                {
                    throw new ArgumentException(
                        SR.GetString(SR.Collection_InvalidType, "ConsumerConnectionPoint"),
                        "connectionPoints"
                    );
                }
                string id = point.ID;
                ...
                else
                {
                    throw new ArgumentException(
                        SR.GetString(
                            SR.WebPart_Collection_DuplicateID,
        ...
        {
            get { return this[ConnectionPoint.DefaultID]; }
        }

        public ConsumerConnectionPoint this[int index]
        {
            get { return (ConsumerConnectionPoint)InnerList[index]; }
        }

        public ConsumerConnectionPoint this[string id]
        {
            get { return ((_ids != null) ? (ConsumerConnectionPoint)_ids[id] : null); }
        }
