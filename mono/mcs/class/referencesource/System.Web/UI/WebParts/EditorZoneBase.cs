...
namespace System.Web.UI.WebControls.WebParts
{
    ...
    public abstract class EditorZoneBase : ToolZone
    {
        ...
        {
            get { return (base.Display && WebPartToEdit != null); }
        }
                        ...
                        foreach (EditorPart editorPart in editorParts)
                        {
                            if (String.IsNullOrEmpty(editorPart.ID))
                            {
                                throw new InvalidOperationException(
                                    SR.GetString(SR.EditorZoneBase_NoEditorPartID)
                                );
                            }
                        }
                    }
        ...
        {
            if (
                String.Equals(eventArgument, applyEventArgument, StringComparison.OrdinalIgnoreCase)
            )
            {
            ...
            )
            {
                ...
                String.Equals(eventArgument, okEventArgument, StringComparison.OrdinalIgnoreCase)
            )
            {
        ...
        protected override object SaveViewState()
        {
            ...
            myState[applyVerbIndex] =
                (_applyVerb != null) ? ((IStateManager)_applyVerb).SaveViewState() : null;
            myState[cancelVerbIndex] =
                (_cancelVerb != null) ? ((IStateManager)_cancelVerb).SaveViewState() : null;
            myState[okVerbIndex] =
                (_okVerb != null) ? ((IStateManager)_okVerb).SaveViewState() : null;
