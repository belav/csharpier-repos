...
namespace System.Web.UI.WebControls.WebParts
{
    ...
    public sealed class RowToParametersTransformer : WebPartTransformer, IWebPartParameters
    {
        ...
        public string[] ConsumerFieldNames
        {
            get
            {
                return (_consumerFieldNames != null)
                    ? (string[])_consumerFieldNames.Clone()
                    : new string[0];
            }
            set { _consumerFieldNames = (value != null) ? (string[])value.Clone() : null; }
        }

        private PropertyDescriptorCollection ConsumerSchema
        {
            get { return _consumerSchema; }
        }
        ...
        public string[] ProviderFieldNames
        {
            get
            {
                return (_providerFieldNames != null)
                    ? (string[])_providerFieldNames.Clone()
                    : new string[0];
            }
            set { _providerFieldNames = (value != null) ? (string[])value.Clone() : null; }
        }

        private PropertyDescriptorCollection ProviderSchema
        {
            get { return (_provider != null) ? _provider.Schema : null; }
        }
        ...
        private PropertyDescriptorCollection SelectedProviderSchema
        {
            get
            {
                ...
                if (
                    providerSchema != null
                    && _providerFieldNames != null
                    && _providerFieldNames.Length > 0
                )
                {
                    foreach (string fieldName in _providerFieldNames)
                    {
                        PropertyDescriptor prop = providerSchema.Find(
        ...
        {
            int consumerFieldNamesLength =
                (_consumerFieldNames != null) ? _consumerFieldNames.Length : 0;
            int providerFieldNamesLength =
                (_providerFieldNames != null) ? _providerFieldNames.Length : 0;

            if (consumerFieldNamesLength != providerFieldNamesLength)
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.RowToParametersTransformer_DifferentFieldNamesLength)
                );
            }
        }
        ...
        PropertyDescriptorCollection IWebPartParameters.Schema
        {
            get
            {
                ...
                if (
                    _consumerSchema != null
                    && _consumerFieldNames != null
                    && _consumerFieldNames.Length > 0
                )
                {
        ...
        private sealed class RowToParametersConfigurationWizard : TransformerConfigurationWizardBase
        {
            ...
            {
                get { return _owner.ConsumerSchema; }
            }

            protected override PropertyDescriptorCollection ProviderSchema
            {
                get { return _owner.ProviderSchema; }
            }
            ...
            protected override void CreateWizardSteps()
            {
                ...
                if (oldProviderNamesLength > 0)
                {
                    ...
                    for (int i = 0; i < oldProviderNamesLength / 2; i++)
                    {
                        ...
                        if (consumerItems != null)
                        {
                            ...
                            if (providerFieldNames != null)
                            {
                                for (int j = 0; j < providerFieldNames.Length; j++)
                                {
                                    ...
                                    if (
                                        String.Equals(
                                            providerFieldNames[j],
                                            providerFieldValue,
                                            StringComparison.OrdinalIgnoreCase
                                        )
                                        && consumerFieldNames != null
                                        && consumerFieldNames.Length > j
                                    )
                                    {
                        ...
                        consumerFieldNameLabel.Text = SR.GetString(
                            SR.RowToParametersTransformer_ConsumerFieldName
                        );
