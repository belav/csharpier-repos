...
namespace System.Web.UI.WebControls.WebParts
{
    ...
    public sealed class PropertyGridEditorPart : EditorPart
    {
        ...
        private static readonly UrlPropertyAttribute urlPropertyAttribute =
        ...
        public override bool ApplyChanges()
        {
            ...
            for (int i = 0; i < properties.Count; i++)
            {
                ...
                try
                {
                    ...
                    if (
                        pd.Attributes.Matches(urlPropertyAttribute)
                ...
                catch
                {
                    ...
                    {
                        _errorMessages[i] = SR.GetString(
                            SR.EditorPart_ErrorConvertingPropertyWithType,
                            pd.PropertyType.FullName
                        );
                    }
                }
            }
        ...
        {
            WebDescriptionAttribute attribute = (WebDescriptionAttribute)
                pd.Attributes[typeof(WebDescriptionAttribute)];
            if (attribute != null)
            {
        ...
        {
            WebDisplayNameAttribute attribute = (WebDisplayNameAttribute)
                pd.Attributes[typeof(WebDisplayNameAttribute)];
        ...
        private PropertyDescriptorCollection GetEditableProperties(object editableObject, bool sort)
        {
            ...
            PropertyDescriptorCollection propDescs = TypeDescriptor.GetProperties(
                editableObject,
                FilterAttributes
            );
            if (sort)
        ...
        protected internal override void RenderContents(HtmlTextWriter writer)
        {
            ...
            if (editableObject != null)
            {
                PropertyDescriptorCollection propDescs = GetEditableProperties(
                    editableObject,
                    true
                );
            ...
            if (propertyDisplayNames != null)
            {
                WebControl[] editorControls = (WebControl[])
                    EditorControls.ToArray(typeof(WebControl));
                Debug.Assert(
                    propertyDisplayNames.Length == editorControls.Length
                        && propertyDisplayNames.Length == _errorMessages.Length
                );
        ...
        private sealed class DesignModeWebPart : WebPart
        {
            ...
            {
                get { return SampleEnum.EnumValue; }
                set { }
            }
            ...
            private sealed class WebSysWebDisplayNameAttribute : WebDisplayNameAttribute
            {
                ...
                {
                    get { return typeof(WebDisplayNameAttribute); }
                }
            }
        }
    }
