    ...
    public class SqlPersonalizationProvider : PersonalizationProvider
    {
            ...
            set
            {
                if (value != null && value.Length > maxStringLength)
                {
                    throw new ProviderException(
                        SR.GetString(
                            SR.PersonalizationProvider_ApplicationNameExceedMaxLength,
        ...
        )
        {
            ...
            try
            {
                try
                {
                    ...
                    SqlParameter parameter = parameters.Add(
                        new SqlParameter("AllUsersScope", SqlDbType.Bit)
                    );
                    ...
                    if (reader != null)
                    {
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                ...
                                sharedStateInfoCollection.Add(
                                    new SharedPersonalizationStateInfo(
                                        returnedPath,
                                        lastUpdatedDate,
                                        size,
                                        userDataSize,
        ...
        )
        {
            ...
            if (scope == PersonalizationScope.Shared)
            {
                ...
                if (query != null)
                {
                    pathToMatch = StringUtil.CheckAndTrimString(
                        query.PathToMatch,
                        "query.PathToMatch",
                        false,
                        maxStringLength
            ...
            else
            {
                ...
                if (query != null)
                {
                    ...
                    usernameToMatch = StringUtil.CheckAndTrimString(
                        query.UsernameToMatch,
                        "query.UsernameToMatch",
                        false,
                        maxStringLength
                    );
        ...
        )
        {
            ...
            try
            {
                try
                {
                    ...
                    reader = command.ExecuteReader(CommandBehavior.SequentialAccess);
                    PersonalizationStateInfoCollection stateInfoCollection =
                        new PersonalizationStateInfoCollection();
                    ...
                    if (reader != null)
                    {
                        if (reader.HasRows)
                        {
                            ...
                            {
                                string returnedPath = reader.GetString(0);
                                DateTime lastUpdatedDate = DateTime.SpecifyKind(
                                    reader.GetDateTime(1),
                                    DateTimeKind.Utc
                                );
                                int size = reader.GetInt32(2);
                                string returnedUsername = reader.GetString(3);
                                DateTime lastActivityDate = DateTime.SpecifyKind(
                                    reader.GetDateTime(4),
                                    DateTimeKind.Utc
                                );
                                stateInfoCollection.Add(
                                    new UserPersonalizationStateInfo(
                                        returnedPath,
                                        lastUpdatedDate,
                                        size,
                                        returnedUsername,
        ...
        {
            SqlConnection connection = null;
            SqlConnectionHolder connectionHolder = SqlConnectionHelper.GetConnection(
                _connectionString,
                true
            );
            ...
            {
                throw new ProviderException(
                    SR.GetString(SR.PersonalizationProvider_CantAccess, Name)
                );
            }
        ...
        public override void Initialize(string name, NameValueCollection configSettings)
        {
            ...
            if (string.IsNullOrEmpty(configSettings["description"]))
            {
                ...
                configSettings.Add(
                    "description",
                    SR.GetString(SR.SqlPersonalizationProvider_Description)
                );
            }
            ...
            string connectionString = SqlConnectionHelper.GetConnectionString(
            ...
            {
                throw new ProviderException(
                    SR.GetString(SR.PersonalizationProvider_BadConnection, connectionStringName)
                );
            }
            ...
            if (configSettings.Count > 0)
            {
                ...
                throw new ProviderException(
                    SR.GetString(SR.PersonalizationProvider_UnknownProp, invalidAttributeName, name)
                );
            }
        }
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        foreach (string path in paths)
                        {
                            ...
                            if (
                                countParam != null
                                && countParam.Value != null
                                && countParam.Value is Int32
                            )
                            {
        ...
        public override int ResetUserState(string path, DateTime userInactiveSinceDate)
        {
            ...
            return ResetUserState(
                ResetUserStateMode.PerInactiveDate,
        ...
        {
            PersonalizationProviderHelper.CheckPersonalizationScope(scope);
            paths = PersonalizationProviderHelper.CheckAndTrimNonEmptyStringEntries(
                paths,
                "paths",
                false,
                false,
            ...
            usernames = PersonalizationProviderHelper.CheckAndTrimNonEmptyStringEntries(
                usernames,
                "usernames",
                false,
                true,
        ...
        private int ResetUserState(string[] paths, string[] usernames)
        {
            ...
            {
                count = ResetUserState(
                    ResetUserStateMode.PerPaths,
                    PersonalizationAdministration.DefaultInactiveSinceDate,
                    paths,
                    usernames
                );
        ...
        private int ResetUserState(
            ResetUserStateMode mode,
            DateTime userInactiveSinceDate,
            string[] paths,
            string[] usernames
        )
        ...
        )
        {
            ...
            try
            {
                ...
                catch (SqlException sqlEx)
                {
                    ...
                    if (
                        userName != null
                        && (sqlEx.Number == 2627 || sqlEx.Number == 2601 || sqlEx.Number == 2512)
                    )
                    {
