...
namespace System.Web.UI.WebControls.WebParts
{
    ...
    {
        internal static string[] CheckAndTrimNonEmptyStringEntries(
            string[] array,
            string paramName,
            bool throwIfArrayIsNull,
            bool checkCommas,
            int lengthToCheck
        )
        {
            ...
            if (array.Length == 0)
            {
                throw new ArgumentException(
                    SR.GetString(SR.PersonalizationProviderHelper_Empty_Collection, paramName)
                );
            }
            ...
            for (int i = 0; i < array.Length; i++)
            {
                ...
                if (String.IsNullOrEmpty(trimmedStr))
                {
                    throw new ArgumentException(
                        SR.GetString(
                            SR.PersonalizationProviderHelper_Null_Or_Empty_String_Entries,
                            paramName
                        )
                    );
                }
                if (checkCommas && trimmedStr.IndexOf(',') != -1)
                {
                    throw new ArgumentException(
                        SR.GetString(
                            SR.PersonalizationProviderHelper_CannotHaveCommaInString,
                            paramName,
                            str
                        )
                    );
        ...
        internal static void CheckNullEntries(ICollection array, string paramName)
        {
            ...
            foreach (object item in array)
            {
                ...
                {
                    throw new ArgumentException(
                        SR.GetString(SR.PersonalizationProviderHelper_Null_Entries, paramName)
                    );
                }
            }
        }
