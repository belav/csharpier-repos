    ...
    internal sealed class WebPartMenu
    {
        ...
        private static string DefaultCheckImageUrl
        {
            get
            {
                ...
                {
                    _defaultCheckImageUrl = AssemblyResourceLoader.GetWebResourceUrl(
                        typeof(WebPartMenu),
        ...
        private void RegisterStartupScript(string clientID)
        {
            ...
            {
                menuItemHoverStyleCss = itemHoverStyle
                    .GetStyleAttributes(_menuUser.UrlResolver)
                    .Value;
            }
...
menu"
                + clientID
                + ".itemStyle = '"
                + Util.QuoteJScriptString(menuItemStyleCss)
                + @"';
menu"
                + clientID
                + ".itemHoverStyle = '"
                + Util.QuoteJScriptString(menuItemHoverStyleCss)
                + @"';
menu"
                + clientID
                + ".labelHoverColor = '"
            ...
            foreach (WebPartVerb verb in verbs)
            {
                ...
                if (verb is WebPartHelpVerb)
                {
                    ...
                    string resolvedHelpUrl = (
                        (IUrlResolutionService)associatedWebPart
                    ).ResolveClientUrl(associatedWebPart.HelpUrl);
                    ...
                    if (isEnabled)
                    {
                        writer.AddAttribute(
                            ...
                            "document.body.__wpm.ShowHelp('"
                                ...
                                + "', "
                                + ((int)associatedWebPart.HelpMode).ToString(
                ...
                else
                {
                    ...
                    if (isEnabled)
                    {
                        ...
                        if (verb is WebPartCloseVerb)
                        {
                            ...
                            {
                                onclick =
                                    "if(document.body.__wpmCloseProviderWarning.length == 0 || "
                                    + "confirm(document.body.__wpmCloseProviderWarning)){"
                                    + onclick
                                    + "}";
                            }
                        }
                        ...
                        {
                            onclick =
                                "if(document.body.__wpmDeleteWarning.length == 0 || "
                                + "confirm(document.body.__wpmDeleteWarning)){"
                                + onclick
                                + "}";
                        }
