...
namespace System.Web.UI
{
    ...
    internal sealed class EventValidationStore
    {
        ...
        private readonly HashSet<byte[]> _hashes = new HashSet<byte[]>(
            HashEqualityComparer.Instance
        );

        public int Count
        {
            get { return _hashes.Count; }
        }
        ...
        private static byte[] Hash(string target, string argument)
        {
            ...
            Debug.Assert(
                currentOffset == bufferToBeHashed.Length,
                "Should have populated the entire buffer."
            );
        ...
        {
            public DeserializingBinaryReader(Stream input)
                : base(input) { }
        ...
        {
            public SerializingBinaryWriter(Stream input)
                : base(input) { }
