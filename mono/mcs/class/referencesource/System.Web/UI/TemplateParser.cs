...
#pragma warning restore 0649
        ...
        {
            get { return TypeMapper.UserControlRegisterEntries; }
        }
        internal List<TagNamespaceRegisterEntry> TagRegisterEntries
        {
            get { return TypeMapper.TagRegisterEntries; }
        }
        ...
        {
            get { return flags[hasCodeBehind]; }
        }
        ...
        internal IDesignerHost DesignerHost
        {
            get
            {
                Debug.Assert(
                    FInDesigner,
                    "DesignerHost should be accessed only when FInDesigner == true"
                );
            ...
            set
            {
                ...
                {
                    _typeResolutionService = (ITypeResolutionService)
                        _designerHost.GetService(typeof(ITypeResolutionService));
        ...
        {
            get { return _typeHashCode.CombinedHash32; }
        }
        ...
        {
            get { return _compilerType.CompilerParameters; }
        }

        internal bool FExplicit
        {
            get { return flags[useExplicit]; }
        }

        internal bool FLinePragmas
        {
            get { return !flags[noLinePragmas]; }
        }
        ...
        {
            get { return _codeFileVirtualPath; }
        }
        ...
        internal ControlBuilderInterceptor ControlBuilderInterceptor
        {
            ...
            {
                if (
                    _controlBuilderInterceptor == null
                    && CompConfig != null
                    && CompConfig.ControlBuilderInterceptorTypeInternal != null
                )
                {
        ...
        /// textual content.
        /// </devdoc>
        internal static Control ParseControl(
            string content,
        ...
        private static ITemplate ParseTemplate(
            string content,
            VirtualPath virtualPath,
            bool ignoreFilter
        )
        ...
        private ITemplate ParseTemplateInternal(
            string content,
            VirtualPath virtualPath,
            bool ignoreFilter
        )
            ...
            else
            {
                ...
                else
                {
                    ...
                    _rootBuilder = (RootBuilder)
                        HttpRuntime.CreateNonPublicInstance(fileLevelBuilderType);
                }
            }
        ...
        private Type GetFileLevelControlBuilderType()
        {
            ...
            object[] attrs = BaseType.GetCustomAttributes(
                typeof(FileLevelControlBuilderAttribute), /*inherit*/
                true
            );
        ...
        internal void ParseString(string text, VirtualPath virtualPath, Encoding fileEncoding)
        {
            ...
            try
            {
                ...
                if (HasParserErrors)
                {
                    ...
                    HttpParseException ex = new HttpParseException(
                        firstError.ErrorText,
                        originalException,
                        firstError.VirtualPath,
                        Text,
                        firstError.Line
                    );
        ...
        private void ParseStringInternal(string text, Encoding fileEncoding)
        {
            ...
            for (; ; )
            {
...
#endif
                {
                    ...
                    string directiveName = ProcessAttributes(
                        text,
                        match,
                        out directive,
                        true,
                        out duplicateAttribute
                    );
                    ...
                    {
                        CreateModifiedMainDirectiveFileIfNeeded(
                            text,
                            match,
                            directive,
                            fileEncoding
...
#else
                    (
                        match = aspExprRegex.Match(text, textPos)
                    ).Success
                )
#endif
                {
                ...
                // Check to see if it's an encoded expression code block (i.e. <%: ... %> block)
                    ...
                    !flags[inScriptTag]
                    && (match = aspEncodedExprRegex.Match(text, textPos)).Success
                )
                {
                ...
                // attributes of server tags.
                    ...
                    !flags[inScriptTag] && (match = databindExprRegex.Match(text, textPos)).Success
                )
                {
...
#else
                    (
                        match = aspCodeRegex.Match(text, textPos)
                    ).Success
                )
#endif
                {
                    ...
                    if (code.StartsWith("$", StringComparison.Ordinal))
                    {
                        ProcessError(
                            SR.GetString(
                                SR.ExpressionBuilder_LiteralExpressionsNotAllowed,
                                match.ToString(),
                                code
                            )
                        );
        ...
        void CreateModifiedMainDirectiveFileIfNeeded(
            string text,
            Match match,
            ParsedAttributeCollection mainDirective,
            Encoding fileEncoding
        )
        {
            TextWriter precompTargetWriter = BuildManager.GetUpdatableDeploymentTargetWriter(
                CurrentVirtualPath,
                fileEncoding
            );
        ...
        internal virtual void HandlePostParse()
        {
            ...
            if (compilParams.IncludeDebugInformation)
                HttpRuntime.CheckAspNetHostingPermission(
                    ...
                    SR.Debugging_not_supported_in_low_trust
                );
        ...
        private void ProcessServerScript()
        {
            ...
            if (!flags[ignoreScriptTag])
            {
                ...
                if (
                    !PageParserFilterProcessedCodeBlock(
                        CodeConstructType.ScriptTag,
                        script,
                        _currentScript.Line
                    )
                )
        ...
        internal virtual void CheckObjectTagScope(ref ObjectTagScope scope)
        {
            ...
            {
                throw new HttpException(SR.GetString(SR.App_session_only_valid_in_global_asax));
            }
        }
        ...
        private bool ProcessBeginTag(Match match, string inputText)
        {
            ...
            if (BuilderStack.Count > 1)
            {
                ...
                subBuilder = parentBuilder.CreateChildBuilder(
                    filter,
                    realTagName,
                    attribs,
                    this,
            ...
            {
                subBuilder = RootBuilder.CreateChildBuilder(
                    filter,
                    realTagName,
                    attribs,
                    this,
            ...
            else if (flags[isServerTag])
            {
                ...
                if (!(subBuilder.Parser is PageThemeParser) && cacheAttrib != null)
                {
                    ...
                    subBuilder.PreprocessAttribute(
                        String.Empty,
                        "id",
                        _id,
            ...
            if (!fSelfClosed && subBuilder.HasBody())
            {
                ...
                if (
                    subBuilder is TemplateBuilder
                    && ((TemplateBuilder)subBuilder).AllowMultipleInstances
                )
                {
                ...
                _builderStack.Push(
                    new BuilderStackEntry(
                        subBuilder,
                        tagName,
                        CurrentVirtualPathString,
                        _lineNumber,
                        inputText,
            ...
            foreach (DictionaryEntry entry in mainDirective)
            {
                ...
                try
                {
                    ...
                    )
                    {
                        ...
                        unknownAttributes.AddFilteredAttribute(
                            deviceName,
                            attribName,
                            (string)entry.Value
                        );
        ...
        internal virtual bool ProcessMainDirectiveAttribute(
            string deviceName,
            string name,
            string value,
        ...
        )
        {
            switch (name)
            {
                ...
                case "debug":
                    ...
                    {
                        throw new HttpException(
                            SR.GetString(SR.Insufficient_trust_for_attribute, "debug")
                        );
                    }
                ...
                case "classname":
                    ...
                    _generatedClassName = Util.GetNonEmptyFullClassNameAttribute(
                        name,
                        value,
        ...
        internal virtual void ProcessUnknownMainDirectiveAttribute(
            string filter,
            string attribName,
            string value
        )
        {
            ...
            ProcessError(
                SR.GetString(SR.Attr_not_supported_in_directive, attribName, DefaultDirectiveName)
            );
        }
        ...
        private void ProcessInheritsAttribute(
            string baseTypeName,
            string codeFileBaseTypeName,
            string src,
            Assembly assembly
        )
        {
            ...
            if (_codeFileVirtualPath != null)
            {
                _baseTypeName = Util.GetNonEmptyFullClassNameAttribute(
                    "inherits",
                    baseTypeName,
            ...
            {
                ProcessError(
                    SR.GetString(SR.Invalid_type_to_inherit_from, baseTypeName, _baseType.FullName)
                );
        ...
        private Assembly ImportSourceFile(VirtualPath virtualPath)
        {
            ...
            BuildResultCompiledAssembly result =
                BuildManager.GetVPathBuildResult(virtualPath) as BuildResultCompiledAssembly;
            if (result == null)
            {
        ...
        internal Type GetType(string typeName, bool ignoreCase, bool throwOnError)
        {
            ...
            if (commaIndex > 0)
            {
                ...
                try
                {
                    a = LoadAssembly(
                        assemblyName,
                        !FInDesigner /*throwOnFail*/
                    );
                }
                catch
                {
                    throw new HttpException(SR.GetString(SR.Assembly_not_compiled, assemblyName));
                }
            }
            ...
            if (throwOnError)
            {
                throw new HttpException(SR.GetString(SR.Invalid_type, typeName));
            }
        ...
        private void ProcessServerInclude(Match match)
        {
            if (flags[inScriptTag])
            {
                throw new HttpException(SR.GetString(SR.Include_not_allowed_in_server_script_tag));
            }
            ...
            if (StringUtil.EqualsIgnoreCase(pathType, "file"))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        newPhysicalPath = Path.GetFullPath(
                            Path.Combine(currentPhysicalDir, filename.Replace('/', '\\'))
                        );
                    }
                }
            }
            ...
            if (
                _pageParserFilter != null
                && !_pageParserFilter.AllowServerSideInclude(newVirtualPath.VirtualPathString)
            )
            {
        ...
        private void ProcessCodeBlock(Match match, CodeBlockType blockType, string text)
        {
            ...
            if (
                !PageParserFilterProcessedCodeBlock(
                    CodeConstructTypeFromCodeBlockType(blockType),
                    code,
                    lineNumber
                )
            )
            {
                ...
                subBuilder = new CodeBlockBuilder(
                    blockType,
                    code,
                    lineNumber,
                    column,
        ...
        internal bool PageParserFilterProcessedDataBindingAttribute(
            string controlId,
        ...
        internal bool PageParserFilterProcessedEventHookupAttribute(
            string controlId,
            string eventName,
            string handlerName
        )
        ...
        internal void AddControl(Type type, IDictionary attributes)
        {
            ...
            ControlBuilder subBuilder = ControlBuilder.CreateBuilderFromType(
                this,
                parentBuilder,
                type,
                null,
        ...
        private string ProcessAttributes(
            string text,
            Match match,
            out ParsedAttributeCollection attribs,
            bool fDirective,
            out string duplicateAttribute
        )
        {
            ...
            for (int i = 0; i < attrnames.Count; i++)
            {
                ...
                else if (StringUtil.EqualsIgnoreCase(realAttributeName, "runat"))
                {
                    ...
                    FInDesigner
                    && StringUtil.EqualsIgnoreCase(realAttributeName, "ignoreParentFrozen")
                )
                {
                ...
                if (attribName != null)
                {
                    ...
                    try
                    {
                        ...
                        {
                            ProcessError(
                                SR.GetString(SR.Device_unsupported_in_directive, directiveName)
                            );
                        ...
                        if (BuildManagerHost.InClientBuildManager)
                        {
                            ...
                            int column =
                                attrValue.Index
                                - text.LastIndexOfAny(s_newlineChars, attrValue.Index - 1);
                            attribs.AddAttributeValuePositionInformation(
                                realAttributeName,
                                lineNumber,
                                column
                            );
        ...
        private bool MaybeTerminateControl(string tagName, Match match)
        {
            ...
            if (
                stackEntry._tagName == null
                || !StringUtil.EqualsIgnoreCase(stackEntry._tagName, tagName)
            )
            {
                return false;
            }
        ...
        private void AddBaseTypeDependencies(Type type)
        {
            ...
            if (
                a == typeof(string).Assembly
                || a == typeof(Page).Assembly
                || a == typeof(Uri).Assembly
            )
                return;
        ...
        {
            return AddAssemblyDependency(
                assemblyName,
                false /*addDependentAssemblies*/
            );
        }
        ...
        internal IImplicitResourceProvider GetImplicitResourceProvider()
        {
            ...
            IResourceProvider resourceProvider = ResourceExpressionBuilder.GetLocalResourceProvider(
