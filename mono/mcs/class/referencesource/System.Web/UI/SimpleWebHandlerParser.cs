...
namespace System.Web.UI
{
    ...
    public abstract class SimpleWebHandlerParser : IAssemblyDependencyParser
    {
        ...
        protected Type GetCompiledTypeFromCache()
        {
            ...
            BuildResultCompiledType result = (BuildResultCompiledType)
                BuildManager.GetVPathBuildResult(_virtualPath);
        ...
        internal CodeCompileUnit GetCodeModel()
        {
            ...
            snippetCompileUnit.LinePragma = BaseCodeDomTreeGenerator.CreateCodeLinePragmaHelper(
                _virtualPath.VirtualPathString,
                _lineNumber
            );
        ...
        private void ParseString(string text)
        {
            ...
            {
                throw new HttpException(SR.GetString(SR.Missing_directive, DefaultDirectiveName));
            }
            ...
            for (int i = 0; i < attrnames.Count; i++)
            {
                ...
                if (attribName != null)
                {
                    ...
                    catch (ArgumentException)
                    {
                        ...
                        throw new HttpException(SR.GetString(SR.Duplicate_attr_in_tag, attribName));
                    }
                }
            }
        ...
        private static void ProcessCompilationParams(
        ...
        )
        {
            ...
            int warningLevel = 0;
            if (
                Util.GetAndRemoveNonNegativeIntegerAttribute(
                    directive,
                    "warninglevel",
                    ref warningLevel
                )
        ...
        internal virtual void ProcessDirective(string directiveName, IDictionary directive)
        {
            ...
            if (IsMainDirective(directiveName))
            {
                ...
                if (language != null)
                {
                    _compilerType = _buildProvider.GetDefaultCompilerTypeForLanguageInternal(
                        language
                    );
                }
                else
        ...
        private void ImportSourceFile(VirtualPath virtualPath)
        {
            ...
            BuildResultCompiledAssembly result = (BuildResultCompiledAssembly)
                BuildManager.GetVPathBuildResult(fullVirtualPath);
    ...
    public class WebServiceParser : SimpleWebHandlerParser
    {
        ...
        public static Type GetCompiledType(string inputFile, HttpContext context)
        {
            ...
            BuildResultCompiledType result = (BuildResultCompiledType)
                BuildManager.GetVPathBuildResult(context, VirtualPath.Create(inputFile));
