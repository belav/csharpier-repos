...
namespace System.Web.UI.WebControls
{
    ...
    public class MultiView : Control
    {
        ...
        /// </devdoc>
        [DefaultValue(-1), WebCategory("Behavior"), WebSysDescription(SR.MultiView_ActiveView)]
            ...
            set
            {
                ...
                if (value >= Views.Count)
                {
                    throw new ArgumentOutOfRangeException(
                        "value",
                        SR.GetString(
                            SR.MultiView_ActiveViewIndex_equal_or_greater_than_count,
                            value,
                            Views.Count
                        )
                    );
        ...
        /// </devdoc>
        [WebCategory("Action"), WebSysDescription(SR.MultiView_ActiveViewChanged)]
        public event EventHandler ActiveViewChanged
        {
            add { Events.AddHandler(_eventActiveViewChanged, value); }
            remove { Events.RemoveHandler(_eventActiveViewChanged, value); }
        }
        ...
        protected override void AddParsedSubObject(object obj)
        {
            if (obj is View)
            {
                ...
                throw new HttpException(
                    SR.GetString(SR.MultiView_cannot_have_children_of_type, obj.GetType().Name)
                );
        }
                ...
                else if (cn == SwitchViewByIndexCommandName)
                {
                    ...
                    catch (FormatException)
                    {
                        throw new FormatException(
                            SR.GetString(
                                SR.MultiView_invalid_view_index_format,
                                (string)ce.CommandArgument,
