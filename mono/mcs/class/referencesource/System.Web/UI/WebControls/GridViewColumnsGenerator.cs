...
namespace System.Web.UI.WebControls
{
    public class GridViewColumnsGenerator : AutoFieldsGenerator
    {
        ...
        )
        {
            if (!(control is GridView))
            {
                throw new ArgumentException(
                    SR.GetString(
                        SR.InvalidDefaultAutoFieldGenerator,
                        GetType().FullName,
            ...
            if (propDescs == null)
            {
                ...
                else if (sampleItemType != null)
                {
                    ...
                    {
                        AutoGeneratedFieldProperties fieldProps =
                            new AutoGeneratedFieldProperties();
                        ...
                        AutoGeneratedField field = CreateAutoGeneratedFieldFromFieldProperties(
                            fieldProps
                        );
                        if (field != null)
                        {
            ...
            if ((propDescs != null) && (propDescs.Count != 0))
            {
                ...
                foreach (PropertyDescriptor pd in propDescs)
                {
                    ...
                    {
                        string name = pd.Name;
                        bool isKey = ((IList)dataKeyNamesCaseInsensitive).Contains(
            ...
            if ((generatedFields.Count == 0) && throwException)
            {
                ...
                throw new InvalidOperationException(
                    SR.GetString(SR.GridView_NoAutoGenFields, gridView.ID)
                );
            }
        ...
        {
            if (
                gridView.RenderingCompatibility < VersionUtil.Framework45
            ...
            else
            {
                ...
                return DataBoundControlHelper.IsBindableType(
                    propertyType,
                    enableEnums: AutoGenerateEnumFields ?? true
                );
            }
        }
    }
