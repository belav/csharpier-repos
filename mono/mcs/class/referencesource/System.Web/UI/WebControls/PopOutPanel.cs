...
namespace System.Web.UI.WebControls
{
    ...
    internal sealed class PopOutPanel : Panel
    {
        ...
        {
            get { return _scrollerClass; }
            set { _scrollerClass = value; }
        }

        internal Style ScrollerStyle
        {
            get { return _scrollerStyle; }
            set { _scrollerStyle = value; }
        }
            ...
            if (!_owner.DesignMode)
            {
                ...
                if (scrollUpImageUrl.Length != 0)
                {
                    writer.AddAttribute(
                        ...
                        _owner.ResolveClientUrl(scrollUpImageUrl)
                    );
                }
                else
                {
                    writer.AddAttribute(
                        HtmlTextWriterAttribute.Src,
                        _owner.GetImageUrl(Menu.ScrollUpImageIndex)
                    );
                }
                ...
                if (scrollDownImageUrl.Length != 0)
                {
                    writer.AddAttribute(
                        HtmlTextWriterAttribute.Src,
                        _owner.ResolveClientUrl(scrollDownImageUrl)
                    );
                }
                else
                {
                    writer.AddAttribute(
                        HtmlTextWriterAttribute.Src,
                        _owner.GetImageUrl(Menu.ScrollDownImageIndex)
                    );
                }
            ...
            if ((Page != null) && Page.SupportsStyleSheets)
            {
                if (!String.IsNullOrEmpty(ScrollerClass))
                {
                    writer.AddAttribute(
                        ...
                        ScrollerClass + ' ' + GetEmptyPopOutPanelStyle().RegisteredCssClass
                    );
                }
                else
                {
