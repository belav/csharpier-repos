...
namespace System.Web.UI.WebControls
{
    ...
    public class ImageField : DataControlField
    {
        ...
        ///       in this field.</para>
        /// </devdoc>
        [WebCategory("Behavior"), DefaultValue(false), WebSysDescription(SR.ImageField_ReadOnly)]
        ...
        )
        {
            ...
            { // this should happen only in design mode
                Debug.Assert(
                    DesignMode,
                    "Unless you're in designmode, there should be a control in the cell."
                );
                return;
            }
        ...
        protected virtual string GetFormattedAlternateText(Control controlContainer)
        {
            ...
            if (altTextField.Length > 0)
            {
                ...
                {
                    formattedAlternateText = String.Format(
                        CultureInfo.CurrentCulture,
                        altTextFormatString,
                        value
                    );
        ...
        protected virtual object GetValue(
            ...
            string fieldName,
            ref PropertyDescriptor cachedDescriptor
        )
        {
            Debug.Assert(
                DataImageUrlField.Length != 0,
                "Shouldn't be DataBinding without an DataImageUrlField"
            );
        ...
        protected virtual void OnDataBindField(object sender, EventArgs e)
        {
            ...
            if (DesignMode && (boundControl is TableCell))
            {
                if (boundControl.Controls.Count == 0 || !(boundControl.Controls[0] is Image))
                {
                    throw new HttpException(
                        SR.GetString(SR.ImageField_WrongControlType, DataImageUrlField)
                    );
                }
            ...
            { // read-only
                ...
                if (
                    cell.Controls.Count < 2
                    || !(cell.Controls[0] is Image)
