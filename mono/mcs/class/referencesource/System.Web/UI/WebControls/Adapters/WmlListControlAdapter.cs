    ...
    public class WmlListControlAdapter : ListControlAdapter, IPostBackDataHandler
    {
        ...
        protected internal override void OnPreRender(EventArgs e)
        {
            ...
            if (firstSelectedIndex < count)
            {
                ...
                if (IsMultiSelect)
                {
                    int i = 0;
                    for (
                        i = realCounter = firstSelectedIndex + 1;
                        realCounter < count;
                        realCounter++
                    )
                    {
        ...
        protected virtual string GetInputElementText(ListItem item)
        {
            return item.Selected
                ? CheckBoxAdapter.AltSelectedText
                : CheckBoxAdapter.AltUnselectedText;
        }
        ...
        protected virtual void RenderDisabledItem(HtmlTextWriter writer, ListItem item)
        {
            ...
            bool leftTextAlign = (
                Control is CheckBoxList && ((CheckBoxList)Control).TextAlign == TextAlign.Left
            );
                    ...
                    if (item.Selected)
                    {
                        if (selected && !IsMultiSelect)
                        {
                            throw new HttpException(
                                SR.GetString(SR.Cant_Multiselect_In_Single_Mode)
                            );
                        }
                        selected = true;
                    }
        ...
        protected virtual bool LoadPostData(String key, NameValueCollection data)
        {
            ...
            // first element.
            if (!IsMultiSelect && originalSelection.Count == 0 && Control.Items.Count > 0)
            {
