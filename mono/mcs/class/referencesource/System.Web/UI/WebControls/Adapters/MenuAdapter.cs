...
namespace System.Web.UI.WebControls.Adapters
{
    ...
    public class MenuAdapter : WebControlAdapter, IPostBackEventHandler
    {
        ...
        private void RenderContentsRecursive(
        ...
        private void RenderExpand(HtmlTextWriter writer, MenuItem item, Menu owner)
        {
            ...
            else
            {
                writer.Write(' ');
                if (
                    item.Depth < owner.StaticDisplayLevels
                ...
                {
                    writer.Write(
                        HttpUtility.HtmlEncode(SR.GetString(SR.MenuAdapter_Expand, item.Text))
                    );
                }
            }
        }
        ...
        )
        {
            ...
            if (enabled && (clickOpensThisNode || selectable))
            {
                ...
                string itemAccessKey =
                    ((position == 0 || (position == 1 && depth == 0)) && accessKey.Length != 0)
                        ? accessKey
                        : null;
                        ...
                        if (newPath[i] == TreeView.InternalPathSeparator)
                        {
                            if (++matches >= Control.MaximumDepth)
                            {
                                throw new InvalidOperationException(
                                    SR.GetString(SR.Menu_InvalidDepth)
                                );
                            }
                        }
                    }
