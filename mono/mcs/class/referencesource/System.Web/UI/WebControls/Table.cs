...
namespace System.Web.UI.WebControls
{
    ...
    public class Table : WebControl, IPostBackEventHandler
    {
        ...
        /// </devdoc>
        [WebCategory("Appearance"), DefaultValue(-1), WebSysDescription(SR.Table_CellPadding)]
        ...
        /// </devdoc>
        [WebCategory("Appearance"), DefaultValue(-1), WebSysDescription(SR.Table_CellSpacing)]
        ...
        {
            get { return _hasRowSections; }
            set { _hasRowSections = value; }
        }
        ...
        protected override void AddAttributesToRender(HtmlTextWriter writer)
        {
            ...
            if (ControlStyleCreated)
            {
                if (EnableLegacyRendering || writer is Html32TextWriter)
                {
                    ...
                    {
                        writer.AddAttribute(
                            HtmlTextWriterAttribute.Bordercolor,
                    ...
                    foreach (TableRow row in rows)
                    {
                        if (row.TableSection < currentSection)
                        {
                            ...
                            throw new HttpException(
                                SR.GetString(SR.Table_SectionsMustBeInOrder, ID)
                            );
                        }
                        if (
                            currentSection < row.TableSection
                            || (row.TableSection == TableRowSection.TableHeader && !openedTag)
                        )
                        {
                            if (openedTag)
                            {
