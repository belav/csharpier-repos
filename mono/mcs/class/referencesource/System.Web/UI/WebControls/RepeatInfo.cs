...
namespace System.Web.UI.WebControls
{
    ...
    public sealed class RepeatInfo
    {
        ...
        {
            get { return (caption == null) ? String.Empty : caption; }
            set { caption = value; }
        }
        ...
        {
            get { return enableLegacyRendering; }
            set { enableLegacyRendering = value; }
        }
        ...
        private bool IsListLayout
        {
            get
            {
                return (RepeatLayout == RepeatLayout.UnorderedList)
        ...
        {
            get { return outerTableImplied; }
            set { outerTableImplied = value; }
        }
        ...
        )
        {
            Debug.Assert(
                outerTableImplied == false,
                "Cannot use outer implied table with Horizontal layout"
            );
            ...
            if (IsListLayout)
            {
                if (user.HasFooter || user.HasHeader || user.HasSeparators)
                {
                    throw new InvalidOperationException(
                        SR.GetString(SR.RepeatInfo_ListLayoutDoesNotSupportHeaderFooterSeparator)
                    );
                }
                if (RepeatDirection != RepeatDirection.Vertical)
                {
                    throw new InvalidOperationException(
                        SR.GetString(SR.RepeatInfo_ListLayoutOnlySupportsVerticalLayout)
                    );
                }
                if ((RepeatColumns != 0) && (RepeatColumns != 1))
                {
                    throw new InvalidOperationException(
                        SR.GetString(SR.RepeatInfo_ListLayoutDoesNotSupportMultipleColumn)
                    );
                }
                if (OuterTableImplied)
                {
                    throw new InvalidOperationException(
                        SR.GetString(SR.RepeatInfo_ListLayoutDoesNotSupportImpliedOuterTable)
                    );
                }
            }
        ...
        /// <devdoc>
        /// </devdoc>
        private void RenderVerticalRepeater(
            HtmlTextWriter writer,
            IRepeatInfoUser user,
            Style controlStyle,
            WebControl baseControl
        )
        {
            ...
            for (int currentRow = 0; currentRow < totalRows; currentRow++)
            {
                ...
                {
                    if (
                        ((currentRow != totalRows - 1) || user.HasFooter)
                        && (!outerTableImplied)
                        && (!IsListLayout)
                    )
                    {
