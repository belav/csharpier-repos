namespace System.Web.UI.WebControls
{
    ...
    internal sealed class MethodParametersDictionary
        : IDictionary<string, MethodParameterValue>,
            IStateManager
    {
            ...
            if (savedState != null)
            {
                ...
                for (int i = 0; i < names.Length; i++)
                {
                    ...
                    Debug.Assert(
                        ...
                        "The collection was not empty when loading the viewstate"
                    );
        ...
        bool ICollection<KeyValuePair<string, MethodParameterValue>>.IsReadOnly
        {
            ...
            {
                return (
                    (ICollection<KeyValuePair<string, MethodParameterValue>>)InnerDictionary
                ).IsReadOnly;
            }
        }

        void ICollection<KeyValuePair<string, MethodParameterValue>>.Add(
        ...
        bool ICollection<KeyValuePair<string, MethodParameterValue>>.Contains(
            KeyValuePair<string, MethodParameterValue> item
        )
        {
            return (
        ...
        void ICollection<KeyValuePair<string, MethodParameterValue>>.CopyTo(
            KeyValuePair<string, MethodParameterValue>[] array,
            int arrayIndex
        )
        {
            ((ICollection<KeyValuePair<string, MethodParameterValue>>)InnerDictionary).CopyTo(
                array,
                arrayIndex
            );
        }

        bool ICollection<KeyValuePair<string, MethodParameterValue>>.Remove(
            KeyValuePair<string, MethodParameterValue> item
        )
        {
            return (
