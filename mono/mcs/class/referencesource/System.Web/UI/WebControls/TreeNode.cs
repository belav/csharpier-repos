...
namespace System.Web.UI.WebControls
{
    ...
    public class TreeNode : IStateManager, ICloneable
    {
        ...
        {
            get { return (ViewState["Checked"] != null); }
        }
        ...
        internal bool PreserveChecked
        {
            ...
            set { ViewState["PreserveChecked"] = value; }
        }
        ...
        public string ValuePath
        {
            get
            {
                ...
                else if ((Owner != null) && !String.IsNullOrEmpty(InternalValuePath))
                {
                    ...
                    string[] splitValuePath = InternalValuePath.Split(
                        TreeView.InternalPathSeparator
                    );
        ...
        /// </devdoc>
        private string GetPopulateNodeAttribute(
        ...
        )
        {
            ...
            if (_owner.IsNotIE)
            {
                populateNodeAttributeValue =
                    ...
                    + "'),"
                    + (
                        (selectImageId.Length == 0)
                            ? "null"
                            : ("document.getElementById('" + selectImageId + "')")
                    )
                    + ","
                    + "'"
                    + lineType
        ...
        private string GetToggleNodeAttributeValue(string myId, string lineType)
        {
            Debug.Assert(
                ChildNodes.Count > 0,
                "No nodes for expansion, why are we rendering an expander?"
            );
            if (_toggleNodeAttributeValue == null)
            {
                if (_owner.IsNotIE)
                {
                    _toggleNodeAttributeValue =
        ...
        internal void Render(HtmlTextWriter writer, int position, bool[] isLast, bool enabled)
        {
            ...
            if (
                (mergedStyle != null)
                && !mergedStyle.NodeSpacing.IsEmpty
                && ((depth != 0) || (position != 0))
            )
            {
                ...
                for (int i = 0; i < depth; i++)
                {
                    if (writer is Html32TextWriter)
                    {
                        ...
                        writer.AddStyleAttribute(
                            ...
                            _owner.NodeIndent.ToString(CultureInfo.InvariantCulture) + "px"
                        );
                        writer.RenderBeginTag(HtmlTextWriterTag.Table);
                        writer.RenderBeginTag(HtmlTextWriterTag.Tr);
                        writer.RenderBeginTag(HtmlTextWriterTag.Td);
                        if (_owner.ShowLines && !isLast[i])
                        {
                            writer.AddAttribute(
                                ...
                                _owner.GetImageUrl(TreeView.IImageIndex)
                            );
                    ...
                    else
                    {
                        ...
                        writer.Write(
                            "<div style=\"width:"
                                + _owner.NodeIndent.ToString(CultureInfo.InvariantCulture)
                                + "px;height:1px\">"
                        );
                        if (_owner.ShowLines && !isLast[i])
                        {
            ...
            bool canExpand =
                (PopulateOnDemand || (ChildNodes.Count > 0)) && _owner.ShowExpandCollapse;
            ...
            {
                if (
                    (depth < _owner.LevelStyles.Count)
                    && (_owner.LevelStyles[depth] != null)
                    && mergedStyle.ImageUrl.Length > 0
                )
                {
                ...
                if (canExpand)
                {
                    ...
                    if (_owner.RenderClientScript && !_owner.CustomExpandCollapseHandlerExists)
                    {
                            ...
                            if (_owner.PopulateNodesFromClient)
                            {
                                if (ChildNodes.Count != 0)
                                {
                                    throw new InvalidOperationException(
                                        SR.GetString(SR.TreeView_PopulateOnlyEmptyNodes, _owner.ID)
                                    );
                                }
                                writer.AddAttribute(
                        ...
                        else
                        {
                            writer.AddAttribute(
                                HtmlTextWriterAttribute.Href,
                                GetToggleNodeAttributeValue(myId, lineType)
                            );
                        }
                    }
                    else
            ...
            if (NavigateUrl.Length > 0)
            {
                ...
                if (target.Length > 0)
                {
                    ...
                    if (_owner.RenderClientScript)
                    {
                        ...
                        if (
                            _owner.Page != null
                                && _owner.Page.SupportsStyleSheets
            ...
            if (_owner.EnableHover && (SelectAction != TreeNodeSelectAction.None))
            {
                writer.AddAttribute(
                    "onmouseover",
                    "TreeView_HoverNode(" + _owner.ClientDataObjectID + ", this)"
                );
            ...
            if (GetEffectiveShowCheckBox(type))
            {
                ...
                if (enabled == false)
                {
                    ...
                    if (
                        !_owner.Enabled
                        && (_owner.RenderingCompatibility >= VersionUtil.Framework40)
        ...
        /// </devdoc>
        internal void RenderChildNodes(
            HtmlTextWriter writer,
            int depth,
            bool[] isLast,
            bool enabled
        )
        {
            ...
            {
                writer.AddAttribute(
                    HtmlTextWriterAttribute.Height,
                    mergedStyle.ChildNodesPadding.ToString(CultureInfo.InvariantCulture)
                );
                writer.RenderBeginTag(HtmlTextWriterTag.Table);
                writer.RenderBeginTag(HtmlTextWriterTag.Tr);
                writer.RenderBeginTag(HtmlTextWriterTag.Td);
