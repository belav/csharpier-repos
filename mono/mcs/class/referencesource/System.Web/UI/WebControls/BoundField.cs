...
namespace System.Web.UI.WebControls
{
    ...
    public class BoundField : DataControlField
    {
        ...
        ///       field.</para>
        /// </devdoc>
        [WebCategory("Data"), DefaultValue(""), WebSysDescription(SR.BoundField_DataFormatString)]
        ...
        ///       in this field.</para>
        /// </devdoc>
        [WebCategory("Behavior"), DefaultValue(false), WebSysDescription(SR.BoundField_ReadOnly)]
        ...
        )
        {
            ...
            else
            {
                if (includeReadOnly == true)
                {
                    ...
                    if (cellText == "&nbsp;")
                    { // nothing HtmlEncodes to &nbsp;, so we know that this means it was empty.
                        value = String.Empty;
                    }
                    else
                    {
            ...
            if (value != null)
            {
                ...
                if (
                    value is string
                    && (string)value == nullDisplayText
                    && nullDisplayText.Length > 0
                )
                {
                    value = null;
                }
            ...
            else
            {
                if (!(boundControl is TextBox))
                {
                    throw new HttpException(
                        SR.GetString(SR.BoundField_WrongControlType, DataField)
                    );
                }
