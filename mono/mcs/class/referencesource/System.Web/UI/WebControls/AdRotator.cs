...
namespace System.Web.UI.WebControls
{
    ...
    public class AdRotator : DataBoundControl
    {
        ...
        {
            get { return ((_advertisementFile == null) ? String.Empty : _advertisementFile); }
            set { _advertisementFile = value; }
        }
        ...
        public String AlternateTextField
        {
            ...
            set { ViewState["AlternateTextField"] = value; }
        }
        ...
        {
            get { return (ViewState["Target"] != null); }
        }

        internal bool IsPostCacheAdHelper
        {
            get { return _isPostCacheAdHelper; }
            set { _isPostCacheAdHelper = value; }
        }
        ...
        {
            get { return _adCreatedEventArgs; }
            set { _adCreatedEventArgs = value; }
        }
        ...
        /// </devdoc>
        [WebCategory("Action"), WebSysDescription(SR.AdRotator_OnAdCreated)]
        ...
        private AdCreatedEventArgs GetAdCreatedEventArgs()
        {
            ...
            AdCreatedEventArgs adArgs = new AdCreatedEventArgs(
                adInfo,
                ImageUrlField,
                NavigateUrlField,
                AlternateTextField
        ...
        private AdRec[] GetFileData(string fileName)
        {
            ...
            if (adRecs == null)
            {
                ...
                catch (Exception e)
                {
                    if (
                        !String.IsNullOrEmpty(physicalPath)
                        && HttpRuntime.HasPathDiscoveryPermission(physicalPath)
                    )
                    {
                        // We want to catch the error message, but not propage the inner exception. Otherwise we can throw up
                        // logon prompts through IE;
                        throw new HttpException(
                            SR.GetString(SR.AdRotator_cant_open_file, ID, e.Message)
                        );
                    }
                    else
                    {
                        throw new HttpException(
                            SR.GetString(SR.AdRotator_cant_open_file_no_permission, ID)
                        );
                    }
                }
        ...
        private AdRec[] LoadStream(Stream stream)
        {
            ...
            {
                throw new HttpException(SR.GetString(SR.AdRotator_parse_error, ID, e.Message), e);
            }
        ...
        protected internal override void PerformDataBinding(IEnumerable data)
        {
            if (data != null)
            {
                ...
                { // DataSourceID case, we know that only one source is available
        ...
        {
            if (!DesignMode && !IsPostCacheAdHelper && DoPostCacheSubstitutionAsNeeded(writer))
            {
                return;
            }
        ...
        private void RenderLink(HtmlTextWriter writer, AdCreatedEventArgs adArgs)
        {
            ...
            else
            {
                ...
                string altTextKey =
                    (AlternateTextField.Length != 0)
                        ? AlternateTextField
                        : AdCreatedEventArgs.AlternateTextElement;
        ...
        private struct AdRec
        {
            ...
            public void Initialize(IDictionary adProperties)
            {
                ...
                if (
                    String.IsNullOrEmpty(impressionsString)
                    || !int.TryParse(
                        impressionsString,
