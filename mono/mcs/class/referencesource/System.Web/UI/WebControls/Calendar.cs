...
namespace System.Web.UI.WebControls
{
    ...
    [
        ...
        DataBindingHandler(
            "System.Web.UI.Design.WebControls.CalendarDataBindingHandler, "
    ...
    public class Calendar : WebControl, IPostBackEventHandler
    {
        ...
        /// </devdoc>
        [WebCategory("Layout"), DefaultValue(2), WebSysDescription(SR.Calendar_CellPadding)]
        ...
        /// </devdoc>
        [WebCategory("Layout"), DefaultValue(0), WebSysDescription(SR.Calendar_CellSpacing)]
        ...
        /// </devdoc>
        [WebCategory("Appearance"), DefaultValue(true), WebSysDescription(SR.Calendar_ShowTitle)]
        ...
        /// </devdoc>
        [WebCategory("Action"), WebSysDescription(SR.Calendar_OnDayRender)]
        public event DayRenderEventHandler DayRender
        {
            add { Events.AddHandler(EventDayRender, value); }
            remove { Events.RemoveHandler(EventDayRender, value); }
        }
        ...
        /// </devdoc>
        [WebCategory("Action"), WebSysDescription(SR.Calendar_OnSelectionChanged)]
        ...
        /// </devdoc>
        [WebCategory("Action"), WebSysDescription(SR.Calendar_OnVisibleMonthChanged)]
        ...
        private DateTime EffectiveVisibleDate()
        {
            ...
            {
                return threadCalendar.AddDays(
                    visDate,
                    -(threadCalendar.GetDayOfMonth(visDate) - 1)
                );
            }
        }
        ...
        private DateTime FirstCalendarDay(DateTime visibleDate)
        {
            ...
            int daysFromLastMonth =
                ((int)threadCalendar.GetDayOfWeek(firstDayOfMonth)) - NumericFirstDayOfWeek();
        ...
        /// <devdoc>
        /// </devdoc>
        private string GetCalendarButtonText(
        ...
        )
        {
            if (showLink)
            {
                ...
                sb.Append(
                    foreColor.IsEmpty ? defaultButtonColorText : ColorTranslator.ToHtml(foreColor)
                );
        ...
        {
            return (
                selectionMode == CalendarSelectionMode.DayWeek
...
#endif
            return (
                threadCalendar.GetEra(date1) == threadCalendar.GetEra(date2)
        ...
        {
            MonthChangedEventHandler handler = (MonthChangedEventHandler)
                Events[EventVisibleMonthChanged];
            if (handler != null)
            {
        ...
        protected virtual void RaisePostBackEvent(string eventArgument)
        {
            ...
            else
            {
                ...
                )
                {
                    ...
                    int newDateDiff = Int32.Parse(
                        eventArgument.Substring(NAVIGATE_MONTH_COMMAND.Length),
                ...
                )
                {
                    ...
                    int rangeValue = Int32.Parse(
                        eventArgument.Substring(SELECT_RANGE_COMMAND.Length),
                        CultureInfo.InvariantCulture
                    );
        ...
        private void RenderCalendarHeaderCell(
            HtmlTextWriter writer,
            TableItemStyle style,
            string text,
            string abbrText
        ...
        )
        {
            ...
            if (HasWeekSelectors(selectionMode))
            {
                ...
                if (selectionMode == CalendarSelectionMode.DayWeekMonth)
                {
                    ...
                    int monthLength = threadCalendar.GetDaysInMonth(
                        threadCalendar.GetYear(visibleDate),
                        threadCalendar.GetMonth(visibleDate),
                        threadCalendar.GetEra(visibleDate)
                    );
                    ...
                    string monthSelectKey =
                        SELECT_RANGE_COMMAND
                        + ((startOffset * 100) + monthLength).ToString(
                            CultureInfo.InvariantCulture
                        );
        ...
        )
        {
            ...
            bool hasRenderEvent = (
                this.GetType() != typeof(Calendar) || Events[EventDayRender] != null
            );
            ...
            bool inDesignSelectionMode = (
                DesignMode && SelectionMode != CalendarSelectionMode.None
            );
            ...
            if (IsMinSupportedYearMonth(visibleDate))
            {
                numOfFirstDaysToSkip =
                    (int)threadCalendar.GetDayOfWeek(firstDay) - NumericFirstDayOfWeek();
            ...
            for (int iRow = 0; iRow < 6; iRow++)
            {
                ...
                if (hasWeekSelectors)
                {
                    ...
                    string weekSelectKey =
                        SELECT_RANGE_COMMAND
                        + dayDiffParameter.ToString(CultureInfo.InvariantCulture);

                    string selectWeekTitle = null;
                    if (useAccessibleHeader)
                    {
                        int weekOfMonth = iRow + 1;
                        selectWeekTitle = SR.GetString(
                            SR.Calendar_SelectWeekTitle,
                            weekOfMonth.ToString(CultureInfo.InvariantCulture)
                        );
                    }
                    RenderCalendarCell(
                        writer,
                ...
                for (int iDay = 0; iDay < 7; iDay++)
                {
                    ...
                    if (hasRenderEvent)
                    {
                        ...
                        dayContent.Text = GetCalendarButtonText(
                    ...
                    if (
                        lastOrSecondLastMonth
                        && d.Month == maxSupportedDate.Month
            ...
            if (ShowNextPrevMonth)
            {
                ...
                else
                {
                    ...
                    if (
                        ...
                        || nextPrevFormat == NextPrevFormat.FullMonth
                    )
                    {
                        int monthNo = threadCalendar.GetMonth(
                            threadCalendar.AddMonths(visibleDate, -1)
                        );
                        prevMonthText = GetMonthName(
                            monthNo,
                            (nextPrevFormat == NextPrevFormat.FullMonth)
                        );
                    }
        ...
        protected override object SaveViewState()
        {
            ...
            myState[2] =
                (nextPrevStyle != null) ? ((IStateManager)nextPrevStyle).SaveViewState() : null;
            myState[3] = (dayStyle != null) ? ((IStateManager)dayStyle).SaveViewState() : null;
            myState[4] =
                (dayHeaderStyle != null) ? ((IStateManager)dayHeaderStyle).SaveViewState() : null;
            myState[5] =
                (todayDayStyle != null) ? ((IStateManager)todayDayStyle).SaveViewState() : null;
            myState[6] =
                (weekendDayStyle != null) ? ((IStateManager)weekendDayStyle).SaveViewState() : null;
            myState[7] =
                (otherMonthDayStyle != null)
                    ? ((IStateManager)otherMonthDayStyle).SaveViewState()
                    : null;
            myState[8] =
                (selectedDayStyle != null)
                    ? ((IStateManager)selectedDayStyle).SaveViewState()
                    : null;
            myState[9] =
                (selectorStyle != null) ? ((IStateManager)selectorStyle).SaveViewState() : null;
