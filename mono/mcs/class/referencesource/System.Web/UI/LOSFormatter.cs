...
namespace System.Web.UI
{
    ...
    public sealed class LosFormatter
    {
        ...
        public LosFormatter(bool enableMac, string macKeyModifier)
            : this(enableMac, GetBytes(macKeyModifier)) { }
    ...
    public sealed class LosFormatter : IStateFormatter
    {
        ...
        public object Deserialize(string input)
        {
            ...
            if (EnableViewStateMac)
            {
                try
                {
                    dataBytes = MachineKeySection.GetDecodedData(
                        dataBytes,
                        _macKey,
                        0,
                        dataBytes.Length,
        ...
        private object DeserializeValueInternal()
        {
            ...
            if (
                _current >= _deserializationData.Length
                || _deserializationData[_current] != leftAngleBracketChar
            )
            {
        ...
        private object ConsumeTypeConverterValue(string token)
        {
            ...
            else
            {
                ...
                _deserializedConverterTable[
                    (_deserializedTypeTable.Count + 50).ToString(NumberFormat)
                ] = tc;
                    ...
                    else
                    {
                        ...
                        catch (SerializationException)
                        {
                            throw new HttpException(
                                SR.GetString(SR.NonSerializableType, value.GetType().FullName)
                            );
                        }
