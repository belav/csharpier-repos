    ...
    */
    ...
    public abstract class BaseTemplateParser : TemplateParser
    {
            ...
            if (host != null)
            {
                IUserControlTypeResolutionService ucTypeResService =
                    (IUserControlTypeResolutionService)
                        host.GetService(typeof(IUserControlTypeResolutionService));
        ...
        */
        ...
        {
            Type t = GetReferencedType(
                virtualPath,
                false /*allowNoCompile*/
            );
        ...
        */
        ...
        internal Type GetReferencedType(VirtualPath virtualPath, bool allowNoCompile)
        {
            ...
            if (
                _pageParserFilter != null
                && !_pageParserFilter.AllowVirtualReference(CompConfig, virtualPath)
            )
            {
            ...
            BuildResultNoCompileTemplateControl noCompileResult =
                result as BuildResultNoCompileTemplateControl;
        ...
        internal override void ProcessDirective(string directiveName, IDictionary directive)
        {
            if (StringUtil.EqualsIgnoreCase(directiveName, "register"))
            {
                ...
                string tagPrefix = Util.GetAndRemoveNonEmptyIdentifierAttribute(
                    directive,
                    "tagprefix",
                    true /*required*/
                );

                string tagName = Util.GetAndRemoveNonEmptyIdentifierAttribute(
                    directive,
                    _tagnameString,
                    false /*required*/
                );

                VirtualPath src = Util.GetAndRemoveVirtualPathAttribute(
                    directive,
                    _sourceString,
                    false /*required*/
                );

                string ns = Util.GetAndRemoveNonEmptyNoSpaceAttribute(
                    directive,
                    _namespaceString,
                    false /*required*/
                );
                ...
                string assemblyName = Util.GetAndRemoveNonEmptyAttribute(
                    directive,
                    "assembly",
                    false /*required*/
                );
        ...
        internal string Key
        {
            get
            {
                return TagPrefix
                    ...
                    + _ns
                    + ":"
                    + (_assemblyName == null ? String.Empty : _assemblyName);
            }
        }
#endif
    }
