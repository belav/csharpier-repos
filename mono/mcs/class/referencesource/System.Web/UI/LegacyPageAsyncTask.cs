...
namespace System.Web.UI
{
    ...
    internal sealed class LegacyPageAsyncTask
    {
        ...
        private void OnAsyncTaskCompletion(IAsyncResult ar)
        {
            Debug.Trace(
                "Async",
                "Task completed, CompletedSynchronously=" + ar.CompletedSynchronously
            );
        ...
        private void CompleteTask(bool timedOut, bool syncTimeoutCaller)
        {
            ...
            catch (ThreadAbortException e)
            {
                ...
                HttpApplication.CancelModuleException exceptionState =
                    e.ExceptionState as HttpApplication.CancelModuleException;
            ...
            _completed = true;
            _taskManager.TaskCompleted(
                _completedSynchronously /*onCallerThread*/
            ); // notify TaskManager

            // Wait for pending AsyncTasks (DDB 140655)
            if (responseEnded)
            {
                _taskManager.CompleteAllTasksNow(
                    false /*syncCaller*/
                );
            }
        }
