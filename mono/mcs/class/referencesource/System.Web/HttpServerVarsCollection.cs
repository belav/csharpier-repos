...
namespace System.Web
{
    ...
    internal class HttpServerVarsCollection : HttpValueCollection
    {
                ...
                switch (name[0])
                {
                    ...
                    case 'a':
                        if (StringUtil.EqualsIgnoreCase(name, "AUTH_TYPE"))
                            return _request.CalcDynamicServerVariable(
                                DynamicServerVariable.AUTH_TYPE
                            );
                        else if (StringUtil.EqualsIgnoreCase(name, "AUTH_USER"))
                            return _request.CalcDynamicServerVariable(
                                DynamicServerVariable.AUTH_USER
                            );
                        break;
                    case 'H':
                    case 'h':
        ...
        internal void SynchronizeServerVariable(
            String name,
            String value,
            bool ensurePopulated = true
        )
        {
            if (name == null)
            {
        ...
        internal void RemoveNoDemand(String name)
        {
            ...
            _iis7workerRequest.SetServerVariable(
                name,
                null /*value*/
            );
