            ...
            get
            {
                ...
                if (s_Provider == null)
                {
                    throw new InvalidOperationException(
                        SR.GetString(SR.Def_membership_provider_not_found)
                    );
                }
                return s_Provider;
            }
...
*/
        ...
        public static string GeneratePassword(int length, int numberOfNonAlphanumericCharacters)
        {
            ...
            if (numberOfNonAlphanumericCharacters > length || numberOfNonAlphanumericCharacters < 0)
            {
                throw new ArgumentException(
                    SR.GetString(
                        SR.Membership_min_required_non_alphanumeric_characters_incorrect,
                ...
                if (settings != null && settings.Providers != null)
                {
                    foreach (ProviderSettings ps in settings.Providers)
                    {
                        if (ps != null && ps.Parameters != null)
                        {
                            ...
                            if (
                                StringUtil.EqualsIgnoreCase(passwordFormat, "Clear")
                                || StringUtil.EqualsIgnoreCase(passwordFormat, "Encrypted")
                            )
                            {
                                string providerName = ps.Name ?? string.Empty;
                                WebBaseEvent.RaiseRuntimeError(
                                    new ConfigurationErrorsException(
                                        SR.GetString(
                                            SR.MembershipPasswordFormat_Obsoleted,
                                            providerName,
                                            passwordFormat
                                        )
                                    ),
        ...
        private static bool InitializeSettings(
            bool initializeGeneralSettings,
            RuntimeConfig appConfig,
            MembershipSection settings
        )
        {
        ...
        public static MembershipUserCollection FindUsersByName(string usernameToMatch)
        {
            ...
            int totalRecords = 0;
            return Provider.FindUsersByName(usernameToMatch, 0, Int32.MaxValue, out totalRecords);
        }
