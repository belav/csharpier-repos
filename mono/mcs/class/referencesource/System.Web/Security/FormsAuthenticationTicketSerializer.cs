...
*/
    ...
    internal static class FormsAuthenticationTicketSerializer
    {
        ...
        public static FormsAuthenticationTicket Deserialize(
            byte[] serializedTicket,
            int serializedTicketLength
        )
        {
            try
            {
                ...
                {
                    using (
                        SerializingBinaryReader ticketReader = new SerializingBinaryReader(
                            ticketBlobStream
                        )
                    )
                    {
                        ...
                        DateTime ticketIssueDateUtc = new DateTime(
                            ticketIssueDateUtcTicks,
                            DateTimeKind.Utc
                        );
                        ...
                        DateTime ticketExpirationDateUtc = new DateTime(
                            ticketExpirationDateUtcTicks,
                            DateTimeKind.Utc
                        );
        ...
        public static byte[] Serialize(FormsAuthenticationTicket ticket)
        {
            ...
            {
                using (
                    SerializingBinaryWriter ticketWriter = new SerializingBinaryWriter(
                        ticketBlobStream
                    )
                )
                {
                    ...
                    // LENGTH: 1 byte
                    Debug.Assert(
                        ticketBlobStream.Position == 10,
                        "Critical that we be at position 10 in the stream at this point."
                    );
