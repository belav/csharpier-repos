    ...
    public sealed class PassportAuthenticationModule : IHttpModule
    {
        ...
        void OnEnter(Object source, EventArgs eventArgs)
        {
            ...
            context.SetSkipAuthorizationNoDemand(
                AuthenticationConfig.AccessingLoginPage(context, _LoginUrl),
                false /*managedOnly*/
            );

            if (!context.SkipAuthorization)
            {
                context.SkipAuthorization = AssemblyResourceLoader.IsValidWebResourceRequest(
                    context
                );
            }
        }
        ...
        void OnLeave(Object source, EventArgs eventArgs)
        {
            ...
            if (
                ...
                || !(context.User.Identity is PassportIdentity)
            )
                return;
            ...
            if (
                _LoginUrl == null
                || _LoginUrl.Length < 1
                || String.Compare(_LoginUrl, "internal", StringComparison.Ordinal) == 0
            )
            {
