...
namespace System.Web.Security
{
    ...
    public sealed class FormsAuthenticationModule : IHttpModule
    {
        ...
        private void OnAuthenticate(FormsAuthenticationEventArgs e)
        {
            ...
            FormsAuthenticationTicket ticket = ExtractTicketFromCookie(
                e.Context,
            ...
            if (ticket2 != ticket)
            {
                ...
                {
                    FormsAuthenticationTicket tempTicket = FormsAuthenticationTicket.FromUtc(
                        ticket2.Version,
                        ticket2.Name,
                        ticket2.IssueDateUtc,
                        ticket2.ExpirationUtc,
                        ticket2.IsPersistent,
                        ticket2.UserData,
                        "/"
                    );
                    ticket2 = tempTicket;
        ...
        private void OnLeave(Object source, EventArgs eventArgs)
        {
            ...
            if (!String.IsNullOrEmpty(FormsAuthentication.LoginUrl))
                loginUrl = AuthenticationConfig.GetCompleteLoginUrl(
                    context,
            ...
            {
                loginUrl = FormsAuthentication.RemoveQueryStringVariableFromUrl(
                    loginUrl,
                    FormsAuthentication.ReturnUrlVar
                );
                strRedirect =
                    ...
                    + FormsAuthentication.ReturnUrlVar
                    + "="
                    + HttpUtility.UrlEncode(strUrl, context.Request.ContentEncoding);
            }
            else
            {
                strRedirect =
        ...
        private static FormsAuthenticationTicket ExtractTicketFromCookie(
            HttpContext context,
            String name,
            out bool cookielessTicket
        )
        {
            ...
            try
            {
                try
                {
                    ...
                    cookielessTicket = CookielessHelperClass.UseCookieless(
                        context,
                        false,
                        FormsAuthentication.CookieMode
                    );
                    ...
                    if (encValue != null && encValue.Length > 1)
                    {
                        ...
                        if (ticket != null && !ticket.Expired)
                        {
                            ...
                            ) // Make sure it is NOT a secure cookie over an in-secure connection
                    ...
                    if (FormsAuthentication.EnableCrossAppRedirects)
                    {
                        ...
                        if (encValue != null && encValue.Length > 1)
                        {
                            if (
                                !cookielessTicket
                                && FormsAuthentication.CookieMode == HttpCookieMode.AutoDetect
                            )
                                cookielessTicket = CookielessHelperClass.UseCookieless(
                                    context,
                                    true,
                    ...
                    if (cookielessTicket)
                    {
                        if (ticket.CookiePath != "/")
                        {
                            FormsAuthenticationTicket tempTicket =
                                FormsAuthenticationTicket.FromUtc(
                                    ticket.Version,
                                    ticket.Name,
                                    ticket.IssueDateUtc,
                        ...
                        string strUrl = FormsAuthentication.RemoveQueryStringVariableFromUrl(
