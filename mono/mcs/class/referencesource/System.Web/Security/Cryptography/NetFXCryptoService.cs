    ...
    ******************************************************************/
    ...
    internal sealed class NetFXCryptoService : ICryptoService
    {
        ...
        public NetFXCryptoService(
            ...
            CryptographicKey validationKey,
            bool predictableIV = false
        )
        {
            ...
            checked
            {
                ...
                )
                {
                    ...
                    if (_predictableIV)
                    {
                        ...
                        encryptionAlgorithm.IV = CryptoUtil.CreatePredictableIV(
                            clearData,
                            encryptionAlgorithm.BlockSize
                        );
                    }
                    else
                    {
                    ...
                    using (MemoryStream memStream = new MemoryStream())
                    {
                        ...
                        using (ICryptoTransform encryptor = encryptionAlgorithm.CreateEncryptor())
                        {
                            ...
                            )
                            {
                                ...
                                using (
                                    KeyedHashAlgorithm signingAlgorithm =
                                ...
                                )
                                {
                                    ...
                                    byte[] signature = signingAlgorithm.ComputeHash(
                                        memStream.GetBuffer(),
                                        0,
            ...
            checked
            {
                ...
                )
                {
                    ...
                    )
                    {
                        ...
                        int encryptedPayloadByteCount =
                            protectedData.Length - ivByteCount - signatureByteCount;
                        ...
                        byte[] computedSignature = validationAlgorithm.ComputeHash(
                            protectedData,
                            0,
                        ...
                        if (
                            !CryptoUtil.BuffersAreEqual(
                                buffer1: protectedData,
                                buffer1Offset: ivByteCount + encryptedPayloadByteCount,
                                buffer1Count: signatureByteCount,
                                buffer2: computedSignature,
                                buffer2Offset: 0,
                                buffer2Count: computedSignature.Length
                            )
                        )
                        {
                        ...
                        {
                            using (
                                ICryptoTransform decryptor = decryptionAlgorithm.CreateDecryptor()
                            )
                            {
                                using (
                                    CryptoStream cryptoStream = new CryptoStream(
