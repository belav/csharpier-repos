    ...
    public sealed class PassportIdentity : IIdentity, IDisposable
    {
        ...
        public PassportIdentity()
        {
            ...
            if (_iPassportVer < 3)
            {
                ...
                String strMSPAuthCookie = (
                    (cookieAuth != null && cookieAuth.Value != null)
                        ? cookieAuth.Value
                        : String.Empty
                );
                String strMSPProfCookie = (
                    (cookieProf != null && cookieProf.Value != null)
                        ? cookieProf.Value
                        : String.Empty
                );
                String strMSPProfCCookie = (
                    (cookieProfC != null && cookieProfC.Value != null)
                        ? cookieProfC.Value
                        : String.Empty
                );
                ...
                int iRet = UnsafeNativeMethods.PassportCreate(
                    strTVariable,
                    strPVariable,
                    strMSPAuthCookie,
                    strMSPProfCookie,
                ...
                if (_iPassport == IntPtr.Zero)
                    throw new COMException(
                        SR.GetString(SR.Could_not_create_passport_identity),
                        iRet
                    );
            ...
            else
            {
                ...
                int iRet = UnsafeNativeMethods.PassportCreateHttpRaw(
                    strRequestLine,
        ...
        public String Name
        {
            get
            {
                if (_Name == null)
                {
                    if (_iPassportVer >= 3)
                        _Name = HexPUID;
                    else if (HasProfile("core"))
                    {
                        _Name =
                            Int32
                                .Parse(this["MemberIDHigh"], CultureInfo.InvariantCulture)
                                .ToString("X8", CultureInfo.InvariantCulture)
                            + Int32
                                .Parse(this["MemberIDLow"], CultureInfo.InvariantCulture)
                                .ToString("X8", CultureInfo.InvariantCulture);
                    }
                    else
                    {
        ...
        {
            int iRet = UnsafeNativeMethods.PassportIsAuthenticated(
                _iPassport,
                iTimeWindow,
                iForceLogin,
                iCheckSecure
        ...
        public int Error
        {
            get { return UnsafeNativeMethods.PassportGetError(_iPassport); }
        }
        ...
        public String GetDomainFromMemberName(String strMemberName)
        {
            ...
            int iRet = UnsafeNativeMethods.PassportDomainFromMemberName(
                _iPassport,
                strMemberName,
                str,
                1024
        ...
        {
            int iRet = UnsafeNativeMethods.PassportHasConsent(
                _iPassport,
                bNeedFullConsent ? 1 : 0,
                bNeedBirthdate ? 1 : 0
            );
            if (iRet < 0)
                throw new COMException(SR.GetString(SR.Passport_method_failed), iRet);
        ...
        )
        {
            ...
            int iRet = UnsafeNativeMethods.PassportLogoutURL(
                _iPassport,
                szReturnURL,
                szCOBrandArgs,
                iLangID,
        ...
        )
        {
            ...
            int iRet = UnsafeNativeMethods.PassportLogoTag(
                _iPassport,
                strReturnUrl,
                iTimeWindow,
                iForceLogin,
        ...
        {
            StringBuilder str = new StringBuilder(4092);
            int iRet = UnsafeNativeMethods.PassportLogoTag2(
                _iPassport,
                strReturnUrl,
                iTimeWindow,
                iForceLogin,
        ...
        {
            StringBuilder str = new StringBuilder(4092);
            int iRet = UnsafeNativeMethods.PassportAuthURL(
                _iPassport,
                strReturnUrl,
                iTimeWindow,
                (fForceLogin ? 1 : 0),
        ...
        {
            StringBuilder str = new StringBuilder(4092);
            int iRet = UnsafeNativeMethods.PassportAuthURL2(
                _iPassport,
                strReturnUrl,
                iTimeWindow,
                (fForceLogin ? 1 : 0),
        ...
        )
        {
            ...
            if (
                str != null
                && str.Length > 0
                && str.IndexOf("Passport", StringComparison.Ordinal) >= 0
            )
            {
        ...
        {
            StringBuilder str = new StringBuilder(1028);
            int iRet = UnsafeNativeMethods.PassportGetDomainAttribute(
                _iPassport,
                strAttribute,
                iLCID,
                strDomain,
        ...
        {
            Object oOut = new Object();
            int iRet = UnsafeNativeMethods.PassportGetCurrentConfig(
                _iPassport,
                strAttribute,
                out oOut
            );
        ...
        private static String CallPassportCryptFunction(int iFunctionID, String strData)
        {
            ...
            do
            {
                ...
                if (iRet != HResults.E_INSUFFICIENT_BUFFER && iRet < 0)
                    throw new COMException(SR.GetString(SR.Passport_method_failed), iRet);
            } while (iRet == HResults.E_INSUFFICIENT_BUFFER && iSize < 10 * 1024 * 1024); // Less than 10MB
