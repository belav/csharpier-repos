    ...
    public class WindowsTokenRoleProvider : RoleProvider
    {
        ...
        public override string[] GetRolesForUser(string username)
        {
            ...
            if (status < 0)
            {
                ...
                status = UnsafeNativeMethods.GetGroupsForUser(
                    token,
                    allRoles,
                    -status,
                    error,
            ...
            if (status <= 0)
                throw new ProviderException(
                    SR.GetString(SR.API_failed_due_to_error, error.ToString())
                );
            ...
            for (int iter = 0; iter < roles.Length; iter++)
            {
                ...
                if (
                    roles[iter]
                        ...
                        .StartsWith(computerName, StringComparison.Ordinal)
                ) // Is it a local group?
        ...
        private WindowsIdentity GetCurrentWindowsIdentityAndCheckName(string userName)
        {
            if (HostingEnvironment.IsHosted)
            {
                ...
                if (context == null || context.User == null)
                    throw new ProviderException(
                        SR.GetString(SR.API_supported_for_current_user_only)
                    );
