...
#pragma warning restore 0649
        ...
        //
        internal static CachedPathData GetVirtualPathData(
            VirtualPath virtualPath,
            ...
            if (s_appConfigPathLength == 0)
            {
                ...
                s_appConfigPathLength =
                    (HostingEnvironment.IsHosted)
                        ? HostingEnvironment.AppConfigPath.Length
                        : WebConfigurationHost.RootWebConfigPath.Length;
            }
        ...
        private static CachedPathData GetConfigPathData(string configPath)
        {
            ...
            if (isRemovable && DoNotCacheUrlMetadata)
            {
                ...
                CachedPathData pathData = new CachedPathData(
                    configPath,
                    virtualFilePath,
                    physicalFilePath,
                    exists
            ...
            CacheItemPriority priority = cacheEntryIsNotRemovable
                ? CacheItemPriority.NotRemovable
                : CacheItemPriority.Normal;
            TimeSpan slidingExpiration = cacheEntryIsNotRemovable
            ...
            finally
            {
                ...
                if (isDataCreator)
                {
                    ...
                    if (
                        !initCompleted
                        || (dataAdd.ConfigRecord != null && dataAdd.ConfigRecord.HasInitErrors)
                    )
                    {
                        //
        ...
        void Init(CachedPathData parentData)
        {
            ...
            {
                IInternalConfigRecord configRecord = HttpConfigurationSystem.GetUniqueConfigRecord(
                    _configPath
                );
        ...
        internal void ValidatePath(String physicalPath)
        {
            ...
            if (!String.IsNullOrEmpty(_physicalPath) && !String.IsNullOrEmpty(physicalPath))
            {
                ...
                else if (_physicalPath.Length - physicalPath.Length == 1)
                {
                    ...
                    if (
                        _physicalPath[_physicalPath.Length - 1]
                ...
                else if (physicalPath.Length - _physicalPath.Length == 1)
                {
                    // if they differ by a trailing slash, we shouldn't call CheckSuspiciousPhysicalPath again
                    if (
                        physicalPath[physicalPath.Length - 1]
                            == System.IO.Path.DirectorySeparatorChar
                        && (
                            0
                            == String.Compare(
            ...
            Debug.Assert(
                false,
                "ValidatePath optimization failed: Request.PhysicalPath="
                    + physicalPath
                    + "; _physicalPath="
                    + _physicalPath
            );
        ...
        internal bool CompletedFirstRequest
        {
            ...
            set { _flags[FCompletedFirstRequest] = value; }
        }
        ...
        {
            get { return s_urlMetadataSlidingExpiration; }
        }
