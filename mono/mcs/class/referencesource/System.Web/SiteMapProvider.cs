    ...
    public abstract class SiteMapProvider : ProviderBase
    {
                            ...
                            while (candidate.ParentProvider != null)
                            {
                                if (providers.Contains(candidate.ParentProvider))
                                    throw new ProviderException(
                                        SR.GetString(SR.SiteMapProvider_Circular_Provider)
                                    );
        ...
        public override void Initialize(string name, NameValueCollection attributes)
        {
            if (attributes != null)
            {
                ...
                ProviderUtil.GetAndRemoveBooleanAttribute(
                    attributes,
                    _securityTrimmingEnabledAttrName,
                    Name,
                    ref _securityTrimmingEnabled
                );
            }
        ...
        public virtual bool IsAccessibleToUser(HttpContext context, SiteMapNode node)
        {
            ...
            if (node.Roles != null)
            {
                foreach (string role in node.Roles)
                {
                    ...
                    if (role == _allRoles || context.User != null && context.User.IsInRole(role))
                    {
                        return true;
                    }
                }
        ...
        protected SiteMapNode ResolveSiteMapNode(HttpContext context)
        {
            ...
            if (!context.Items.Contains(_resolutionTicket))
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < len; i++)
                    {
                        SiteMapNode ret = ((SiteMapResolveEventHandler)ds[i])(
                            this,
                            new SiteMapResolveEventArgs(context, this)
                        );
                        if (ret != null)
                        {
                            return ret;
