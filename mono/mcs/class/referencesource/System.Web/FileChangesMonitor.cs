...
namespace System.Web
{
    ...
    sealed class FileSecurity
    {
        ...
        internal static byte[] GetDacl(string filename)
        {
            ...
            int fOK = UnsafeNativeMethods.GetFileSecurity(
                filename,
                DACL_INFORMATION,
                dacl,
                dacl.Length,
            ...
            else
            {
                ...
                if (hr != HResults.E_INSUFFICIENT_BUFFER)
                {
                    Debug.Trace(
                        "GetDacl",
                        "Error in first call to GetFileSecurity: 0x"
                            + hr.ToString("x", NumberFormatInfo.InvariantInfo)
                    );
                    return null;
                }
                ...
                fOK = UnsafeNativeMethods.GetFileSecurity(
                    filename,
                    DACL_INFORMATION,
                    dacl,
                    dacl.Length,
                ...
                if (fOK == 0)
                {
                    ...
                    Debug.Trace(
                        "GetDacl",
                        "Error in second to GetFileSecurity: 0x"
                            + hr.ToString("x", NumberFormatInfo.InvariantInfo)
                    );
#endif
    ...
    sealed class FileMonitor
    {
        ...
        internal int RemoveTarget(object callbackTarget)
        {
            ...
            if (FileChangesMonitor.s_enableRemoveTargetAssert)
            {
                Debug.Assert(
                    target != null,
                    "removing file monitor target that was never added or already been removed"
                );
            }
#endif
        ...
        internal string DebugDescription(string indent)
        {
            ...
            sb.Append("\n");
            sb.Append(i2 + "LastAction=");
            sb.Append(_lastAction);
            sb.Append("; LastCompletion=");
            sb.Append(Debug.FormatUtcDate(_utcLastCompletion));
            sb.Append("\n");
            ...
            foreach (DictionaryEntry d in delegateEntries)
            {
                sb.Append(
                    i3
                        ...
                        + d.Key.GetType()
                        + "(HC="
                        + d.Key.GetHashCode().ToString("x", NumberFormatInfo.InvariantInfo)
                        + ")\n"
                );
            }
    ...
    delegate void NativeFileChangeNotification(
        FileAction action,
    ...
    sealed class DirMonCompletion : IDisposable
    {
        ...
        {
            get { return _activeDirMonCompletions; }
        }

        internal DirMonCompletion(
            DirectoryMonitor dirMon,
            string dir,
            bool watchSubtree,
            uint notifyFilter
        )
        {
            Debug.Trace(
                "FileChangesMonitor",
                "DirMonCompletion::ctor "
                    + dir
                    + " "
                    + watchSubtree.ToString()
                    + " "
            ...
            finally
            {
                ...
                lock (_ndirMonCompletionHandleLock)
                {
                    ...
                    hr = UnsafeNativeMethods.DirMonOpen(
                        dir,
            ...
            if (Interlocked.Exchange(ref _disposed, 1) == 0)
            {
                ...
                lock (_ndirMonCompletionHandleLock)
                {
                    ...
                    if (ndirMonCompletionHandle.Handle != IntPtr.Zero)
                    {
                        ...
                        UnsafeNativeMethods.DirMonClose(
                            ndirMonCompletionHandle,
                            fNeedToSendFileActionDispose
                        );
                    }
                }
            }
...
#if DBG
            Debug.Trace(
                "FileChangesMonitorOnFileChange",
                "Action="
                    + action
                    + "; Dir="
                    + _dirMon.Directory
                    + "; fileName="
                    + Debug.ToStringMaybeNull(fileName)
                    + "; completion="
                    + Debug.FormatUtcDate(utcCompletion)
                    + ";_ndirMonCompletionPtr=0x"
                    + _ndirMonCompletionPtr.ToString("x")
            );
#endif
        ...
        internal string DebugDescription(string indent)
        {
            ...
            string description =
                ...
                + _ndirMonCompletionPtr.ToString("x")
                + "; callback=0x"
                + hc.ToString("x", NumberFormatInfo.InvariantInfo)
                + "\n";
            return description;
        }
#endif
    ...
    sealed class NotificationQueueItem
    {
        ...
        internal NotificationQueueItem(
    ...
    sealed class DirectoryMonitor : IDisposable
    {
        ...
        internal DirectoryMonitor(string appPathInternal, int fcnMode)
            : this(
                appPathInternal,
                true,
                UnsafeNativeMethods.RDCW_FILTER_FILE_AND_DIR_CHANGES,
                fcnMode
            )
        {
        ...
        internal DirectoryMonitor(string dir, bool watchSubtree, uint notifyFilter, int fcnMode)
        ...
        void StartMonitoring()
        {
            if (_dirMonCompletion == null)
            {
                _dirMonCompletion = new DirMonCompletion(
                    this,
                    Directory,
                    _watchSubtree,
                    _notifyFilter
        ...
        //
        void UpdateFileNameShort(
            FileMonitor fileMon,
            string oldFileNameShort,
            string newFileNameShort
        )
        {
        ...
        internal FileMonitor StartMonitoringFileWithAssert(
            string file,
            FileChangeEventHandler callback,
            string alias
        )
        ...
        //
        bool IsChangeAfterStartMonitoring(
            FileAttributesData fad,
        ...
        )
        {
...
#if DBG
                Debug.Trace(
                    "FileChangesMonitorIsChangeAfterStart",
                    "LastAccessTime is more than 60 seconds before monitoring started."
                );
#endif
                return true;
            }
            ...
            if (utcCompletion > target.UtcStartMonitoring)
            {
                ...
                Debug.Trace(
                    "FileChangesMonitorIsChangeAfterStart",
                    "Notification came after we started monitoring."
                );
#endif
                return true;
            }
            ...
            if (fad.UtcLastAccessTime < fad.UtcLastWriteTime)
            {
                ...
                Debug.Trace(
                    "FileChangesMonitorIsChangeAfterStart",
                    "UtcLastWriteTime is greater then UtcLastAccessTime."
                );
#endif
                return true;
            }
            ...
            if (fad.UtcLastAccessTime.TimeOfDay == TimeSpan.Zero)
            {
                ...
                Debug.Trace(
                    "FileChangesMonitorIsChangeAfterStart",
                    "UtcLastAccessTime is midnight -- FAT32 likely."
                );
#endif
                return true;
            }
            ...
            if (fad.UtcLastAccessTime >= target.UtcStartMonitoring)
            {
                ...
                Debug.Trace(
                    "FileChangesMonitorIsChangeAfterStart",
                    "UtcLastAccessTime is greater than UtcStartMonitoring."
                );
#endif
                return true;
            }
            ...
            Debug.Trace(
                "FileChangesMonitorIsChangeAfterStart",
                "Change is before start of monitoring.  Data:\n FileAttributesData: \nUtcCreationTime: "
                    + fad.UtcCreationTime
                    + " UtcLastAccessTime: "
                    + fad.UtcLastAccessTime
                    + " UtcLastWriteTime: "
                    + fad.UtcLastWriteTime
                    + "\n FileMonitorTarget:\n UtcStartMonitoring: "
            ...
            for (int i = 0; i < FileChangesMonitor.s_dirsToMonitor.Length; i++)
            {
                if (
                    StringUtil.StringStartsWithIgnoreCase(
                        fileName,
                        FileChangesMonitor.s_dirsToMonitor[i]
                    )
                )
                {
        ...
        internal void OnFileChange(FileAction action, string fileName, DateTime utcCompletion)
        {
            ...
            try
            {
                    ...
                    if (_fileMons.Count > 0)
                    {
                        if (action == FileAction.Error || action == FileAction.Overwhelming)
                        {
                            ...
                            if (action == FileAction.Overwhelming)
                            {
                                ...
                                if (
                                    Interlocked.Increment(ref s_notificationBufferSizeIncreased)
                                    == 1
                                )
                                {
                                    UnsafeNativeMethods.GrowFileNotificationBuffer(
                                        HttpRuntime.AppDomainAppId,
                                        _watchSubtree
                                    );
                                }
                        ...
                        else
                        {
                            ...
                            {
                                isSpecialDirectoryChange = GetFileMonitorForSpecialDirectory(
                                    fileName,
                                    ref fileMon
                                );
                            }
                ...
                if (targets != null && !ignoreThisChangeNotification)
                {
...
#if DBG
                                reasonFire =
                                    "(action != FileAction.Added && action != FileAction.Modified) || fadNew == null";
#endif
                            }
                            else if (action == FileAction.Added)
                            {
                                // Added actions are significant if they occur after we started monitoring.
                                isSignificantChange = IsChangeAfterStartMonitoring(
                                    fadNew,
                                    target,
                                    utcCompletion
                                );
                            ...
                            if (isSignificantChange)
                            {
                                ...
                                Debug.Trace(
                                    ...
                                    "Firing change event, reason="
                                        ...
                                        + lastAction
                                        + "; LastCompletion="
                                        + Debug.FormatUtcDate(utcLastCompletion)
                                        + "\nfadOld="
                                        + (
                                            (fadOld != null)
                                                ? fadOld.DebugDescription("\t")
                                                : "<null>"
                                        )
                                        + "\nfileMon="
                                        + (
                                            (fileMon != null)
                                                ? fileMon.DebugDescription("\t")
                                                : "<null>"
                                        )
                                        + "\n"
                                ...
                                s_notificationQueue.Enqueue(
                                    new NotificationQueueItem(target.Callback, action, target.Alias)
                                );
                            }
#if DBG
                            else
                    ...
                    if (
                        s_notificationQueue.Count > 0
                        && s_inNotificationThread == 0
                        && Interlocked.Exchange(ref s_inNotificationThread, 1) == 0
                    )
                    {
            ...
            try
            {
                ...
                for (; ; )
                {
                    ...
                    for (; ; )
                    {
                        ...
                        {
                            Debug.Trace(
                                "FileChangesMonitorFireNotification",
                                "Firing change event"
                                    + "\n\tArgs: Action="
                                    + nqi.Action
                                    + "; fileName="
        ...
        internal string DebugDescription(string indent)
        {
            StringBuilder sb = new StringBuilder(200);
            string i2 = indent + "    ";
            DictionaryEntryCaseInsensitiveComparer decomparer =
                new DictionaryEntryCaseInsensitiveComparer();
            ...
            lock (this)
            {
                ...
                {
                    sb.Append(
                        i2 + "_dirMonCompletion " + _dirMonCompletion.DebugDescription(String.Empty)
                    );
                }
                else
                {
...
#pragma warning restore 0649
            ...
            foreach (string specialDirName in s_dirsToMonitor)
            {
                if (StringUtil.StringStartsWithIgnoreCase(dirName, specialDirName))
                {
                    ...
                    // a subfolder?
                        ...
                        dirName.Length > specialDirName.Length
        ...
        // file.
        //
        internal DateTime StartMonitoringPath(
            string alias,
            FileChangeEventHandler callback,
            out FileAttributesData fad
        )
        {
            Debug.Trace(
                "FileChangesMonitor",
            ...
            if (alias == null)
            {
                throw new HttpException(
                    SR.GetString(SR.Invalid_file_name_for_monitoring, String.Empty)
                );
            }
                ...
                try
                {
                    ...
                    do
                    {
                        ...
                        if (alias.Length == 0 || !UrlPath.IsAbsolutePhysicalPath(alias))
                        {
                            throw new HttpException(
                                SR.GetString(
                                    SR.Invalid_file_name_for_monitoring,
                        ...
                        if (!String.IsNullOrEmpty(file))
                        {
                            ...
                            if (dirMon != null)
                            {
                                ...
                                try
                                {
                                    fileMon = dirMon.StartMonitoringFileWithAssert(
                                        file,
                                        callback,
                                        alias
                                    );
        ...
        internal void StartMonitoringDirectoryRenamesAndBinDirectory(
            string dir,
            FileChangeEventHandler callback
        )
        {
            ...
            Debug.Assert(
                _dirs.Count == 0,
                "This function must be called before monitoring other directories, otherwise monitoring of UNC directories will be unreliable on Windows2000 Server."
            );
#endif
            using (new ApplicationImpersonationContext())
            {
                ...
                try
                {
                    ...
                    _dirMonSubdirs = new DirectoryMonitor(
                        dirRoot,
                        true,
                        UnsafeNativeMethods.RDCW_FILTER_DIR_RENAMES,
                        true,
                        _FCNMode
                    );
                    try
                    {
                        _dirMonSubdirs.StartMonitoringFileWithAssert(
                            null,
                            new FileChangeEventHandler(this.OnSubdirChange),
                            dirRoot
                        );
                    }
                    catch
                    ...
                    {
                        _dirMonSpecialDirs.Add(
                            ListenToSubdirectoryChanges(dirRoot, s_dirsToMonitor[i])
                        );
                    }
                }
                finally
        ...
        DirectoryMonitor ListenToSubdirectoryChanges(string dirRoot, string dirToListenTo)
        {
            ...
            if (IsBeneathAppPathInternal(dirRootSubDir))
            {
                ...
                dirMonSubDir.StartMonitoringFileWithAssert(
                    dirToListenTo,
                    new FileChangeEventHandler(this.OnCriticaldirChange),
                    dirRootSubDir
                );
            }
            else if (Directory.Exists(dirRootSubDir))
            {
                dirMonSubDir = new DirectoryMonitor(
                    dirRootSubDir,
                    true,
                    UnsafeNativeMethods.RDCW_FILTER_FILE_CHANGES,
                    _FCNMode
                );
                try
                {
        ...
        void OnSubdirChange(Object sender, FileChangeEvent e)
        {
            try
            {
                ...
                if (
                    handler != null
                    && (
                        e.Action == FileAction.Error
                        || e.Action == FileAction.Overwhelming
