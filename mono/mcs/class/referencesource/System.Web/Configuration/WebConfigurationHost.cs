...
namespace System.Web.Configuration
{
    ...
    {
        const string InternalHostTypeName =
            "System.Configuration.Internal.InternalConfigHost, " + AssemblyRef.SystemConfiguration;
        const string InternalConfigConfigurationFactoryTypeName =
        ...
        private static readonly string RootWebConfigPathAndPathSeparator =
            RootWebConfigPath + PathSeparator;
        private static readonly string RootWebConfigPathAndDefaultSiteID =
            RootWebConfigPathAndPathSeparator + DefaultSiteID;
        ...
        internal static void GetConfigPaths(
            IConfigMapPath configMapPath,
            WebLevel webLevel,
            VirtualPath virtualPath,
            string site,
            string locationSubPath,
            out VirtualPath appPath,
            out string appSiteName,
            out string appSiteID,
        ...
        )
        {
            ...
            else
            {
                ...
                if (!String.IsNullOrEmpty(site))
                {
                    ...
                    if (String.IsNullOrEmpty(appSiteID))
                    {
                        throw new InvalidOperationException(
                            SR.GetString(SR.Config_failed_to_resolve_site_id, site)
                        );
                    }
                }
                else
                {
                    ...
                    Debug.Assert(
                        !String.IsNullOrEmpty(appSiteID),
                        "No appSiteID found when site argument is null"
                    );
                }
            ...
            {
                locationConfigPath = GetConfigPathFromLocationSubPathBasic(
                    configPath,
                    locationSubPath
                );
                GetSiteIDAndVPathFromConfigPath(
                    locationConfigPath,
                    out locationSite,
                    out locationVPath
                );
                ...
                if (String.IsNullOrEmpty(appSiteID) && !String.IsNullOrEmpty(locationSite))
                {
                    ...
                    if (!String.IsNullOrEmpty(appSiteID))
                    {
                        ...
                        locationConfigPath = GetConfigPathFromSiteIDAndVPath(
                            appSiteID,
                            locationVPath
                        );
                    }
            ...
            {
                appPathString = configMapPath.GetAppPathForPath(
                    appSiteID,
                    locationVPath.VirtualPathString
                );
            }
        ...
        void ChooseAndInitConfigMapPath(
            bool useConfigMapPath,
            IConfigMapPath configMapPath,
        ...
        public override void Init(IInternalConfigRoot configRoot, params object[] hostInitParams)
        {
            ...
            if (hostInitParams.Length > 6)
            {
                ...
                {
                    _rootWebConfigFile = Path.Combine(
                        Path.GetDirectoryName(_machineConfigFile),
                        "web.config"
                    );
                }
            }
            ...
            Debug.Assert(
                configMapPath == null || useConfigMapPath,
                "non-null configMapPath without useConfigMapPath == true"
            );
        ...
        )
        {
            ...
            VirtualPath path = VirtualPath.CreateAbsoluteAllowNull(
        ...
        void IInternalConfigWebHost.GetSiteIDAndVPathFromConfigPath(
            string configPath,
            out string siteID,
            out string vpath
        )
        ...
        string IInternalConfigWebHost.GetConfigPathFromSiteIDAndVPath(string siteID, string vpath)
        {
            return WebConfigurationHost.GetConfigPathFromSiteIDAndVPath(
                siteID,
                VirtualPath.CreateAbsoluteAllowNull(vpath)
            );
        }
        ...
        internal static string GetConfigPathFromSiteIDAndVPath(string siteID, VirtualPath vpath)
        {
            ...
            Debug.Assert(
                vpath == null
                    || vpath.VirtualPathStringIfAvailable != null
                    || vpath.AppRelativeVirtualPathStringIfAvailable == null,
            ...
            string configPath =
                (siteID == DefaultSiteID)
                    ...
                    : RootWebConfigPathAndPathSeparator + siteID;
        ...
        public override string GetStreamName(string configPath)
        {
            ...
            {
                return !string.IsNullOrEmpty(_machineConfigFile)
                    ? _machineConfigFile
                    : _configMapPath.GetMachineConfigFilename();
            }
            if (IsRootWebConfigPath(configPath))
            {
                return !string.IsNullOrEmpty(_rootWebConfigFile)
                    ? _rootWebConfigFile
                    : _configMapPath.GetRootWebConfigFilename();
            }
            else
            {
                string siteID;
                ...
                if (null != _configMapPath2)
                {
                    _configMapPath2.GetPathConfigFilename(
                        siteID,
                        path,
                        out directory,
                        out baseName
                ...
                {
                    _configMapPath.GetPathConfigFilename(
                        siteID,
        ...
        public override object StartMonitoringStreamForChanges(
            string streamName,
        ...
        public override void StopMonitoringStreamForChanges(
            string streamName,
            StreamChangeCallback callback
        )
        {
            ...
            switch (allowDefinition)
            {
                ...
                default:
                    ...
                    throw ExceptionUtil.UnexpectedError(
                        "WebConfigurationHost::IsDefinitionAllowed"
                    );
            }
        }
        ...
        )
        {
            if (!IsDefinitionAllowed(configPath, allowDefinition, allowExeDefinition))
            {
                switch (allowDefinition)
                {
                    ...
                    case ConfigurationAllowDefinition.MachineToWebRoot:
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Config_allow_definition_error_webroot),
                            errorInfo.Filename,
                            errorInfo.LineNumber
                        );
                    ...
                    default:
                        ...
                        // make sure to update this switch accordingly
                        throw ExceptionUtil.UnexpectedError(
                            "WebConfigurationHost::VerifyDefinitionAllowed"
                        );
                }
            }
        }
        ...
        public override string GetConfigPathFromLocationSubPath(
            string configPath,
            string locationSubPath
        )
        {
            ...
            else
            {
                ...
                if (
                    StringUtil.EqualsIgnoreCase(site, _appSiteID)
                    || StringUtil.EqualsIgnoreCase(site, _appSiteName)
                )
                {
                    siteID = _appSiteID;
                }
        ...
        internal static void StaticGetRestrictedPermissions(
        ...
        public override bool IsFullTrustSectionWithoutAptcaAllowed(
            IInternalConfigRecord configRecord
        )
        {
            if (HostingEnvironment.IsHosted)
        ...
        {
            if (
                StringUtil.StringStartsWith(sectionGroupName, SysWebName)
                && (
                    sectionGroupName.Length == SysWebName.Length
                    || sectionGroupName[SysWebName.Length] == '/'
                )
            )
            {
                return true;
        ...
        private bool IsApplication(string configPath)
        {
            ...
            {
                appPath = VirtualPath.CreateAllowNull(
                    _configMapPath.GetAppPathForPath(siteID, vpath.VirtualPathString)
                );
            }
        ...
        internal static Configuration OpenConfiguration(
            WebLevel webLevel,
            ConfigurationFileMap fileMap,
            VirtualPath path,
            string site,
        ...
        )
        {
            ...
            if (isRemote)
            {
                configuration = ConfigurationFactory.Create(
                    typeof(RemoteWebConfigurationHost),
                    webLevel,
                    null,
        ...
        private static string GetMachineConfigPathFromTargetFrameworkMoniker(string moniker)
        {
            ...
            string machineConfig = ToolLocationHelper.GetPathToDotNetFrameworkFile(
        ...
        private static TargetDotNetFrameworkVersion GetTargetFrameworkVersionEnumFromMoniker(
            string moniker
        )
        {
            //
