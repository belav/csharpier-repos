...
namespace System.Web.Configuration
{
    ...
    internal class HealthMonitoringSectionHelper
    {
        ...
        HealthMonitoringSectionHelper()
        {
            ...
            _cachedMatchedRules = new ArrayList[
        ...
        void BasicSanityCheck()
        {
            ...
            foreach (EventMappingSettings eventMappingSettings in _section.EventMappings)
            {
                ...
                if (!(eventMappingSettings.StartEventCode <= eventMappingSettings.EndEventCode))
                {
                    ...
                    if (
                        eventMappingSettings.ElementInformation.Properties[attribute].LineNumber
                        == 0
                    )
                    {
                    ...
                    throw new ConfigurationErrorsException(
                        ...
                        eventMappingSettings.ElementInformation.Properties[attribute].Source,
                        eventMappingSettings.ElementInformation.Properties[attribute].LineNumber
                    );
                }
                ...
                HandlerBase.CheckAssignableType(
                    ...
                    typeof(System.Web.Management.WebBaseEvent),
                    type
                );
            ...
            foreach (RuleSettings rule in _section.Rules)
            {
                ...
                if (_section.EventMappings[rule.EventName] == null)
                {
                    throw new ConfigurationErrorsException(
                        ...
                        rule.ElementInformation.Properties["eventName"].Source,
                        rule.ElementInformation.Properties["eventName"].LineNumber
                    );
                }
            }
        }
        ...
        void DisplayRuleInfo(RuleInfo ruleInfo)
        {
            ...
            Debug.Trace(
                ...
                "referenced provider:"
                    + (
                        ruleInfo._referencedProvider == null
                            ? String.Empty
                            : ruleInfo._referencedProvider.GetType().Name
                    )
            );
            Debug.Trace(
                "BuildRuleInfos",
        ...
        void MergeValuesWithProfile(RuleInfo ruleInfo)
        {
            ...
            if (
                ruleInfo._ruleSettings.ElementInformation.Properties["profile"].ValueOrigin
            ...
            if (
                ...
                && ruleInfo._ruleSettings.ElementInformation.Properties["minInstances"].ValueOrigin
                    == PropertyValueOrigin.Default
            )
            {
            ...
            if (
                profileSettings != null
                && ruleInfo._ruleSettings.ElementInformation.Properties["maxLimit"].ValueOrigin
                    == PropertyValueOrigin.Default
            )
            {
            ...
            if (
                profileSettings != null
                && ruleInfo._ruleSettings.ElementInformation.Properties["minInterval"].ValueOrigin
                    == PropertyValueOrigin.Default
            )
            {
            ...
            if (
                profileSettings != null
                && ruleInfo._ruleSettings.ElementInformation.Properties["custom"].ValueOrigin
                    == PropertyValueOrigin.Default
            )
            {
        ...
        void InitCustomEvaluator(RuleInfo ruleInfo)
        {
            ...
            ruleInfo._customEvaluatorType = ConfigUtil.GetType(
            ...
            // Make sure the type support WebBaseEvent
            HandlerBase.CheckAssignableType(
                ruleInfo._customEvaluatorConfig.ElementInformation.Properties["custom"].Source,
                ruleInfo._customEvaluatorConfig.ElementInformation.Properties["custom"].LineNumber,
                typeof(System.Web.Management.IWebEventCustomEvaluator),
                ruleInfo._customEvaluatorType
            );

            // Create a public instance of the custom evaluator
            if (_customEvaluatorInstances[ruleInfo._customEvaluatorType] == null)
            {
                _customEvaluatorInstances[ruleInfo._customEvaluatorType] =
                    HttpRuntime.CreatePublicInstance(ruleInfo._customEvaluatorType);
            }
        }
        ...
        internal ArrayList FindFiringRuleInfos(Type eventType, int eventCode)
        {
            ...
            else
            {
                ...
                foundFiringRuleInfos = (ArrayList)
                    _cachedMatchedRulesForCustomEvents[customWebEventKey];
            }
            ...
            lock (lockObject)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (info1._ruleInfo._referencedProvider != null)
                    {
                        for (int j = i + 1; j < count; j++)
                        {
                            ...
                            if (
                                info2._ruleInfo._referencedProvider != null
                                && // ignore null-provider
                                info2._indexOfFirstRuleInfoWithSameProvider == -1
                                && // ignore rules that were marked already
                                info1._ruleInfo._referencedProvider
                                    == info2._ruleInfo._referencedProvider
                            )
                            { // they are pointing to the same provider
                ...
                Debug.Trace(
                    "FindRuleInfos",
                    "Find ruleInfos for event with type="
                        + eventType.ToString()
                        + ", EventCode="
                        + eventCode
                );
                ...
                foreach (FiringRuleInfo info in matchedRules)
                {
                    Debug.Trace(
                        "FindRuleInfos",
                        "Provider="
                            + info._ruleInfo._ruleSettings.Provider
                            + "; eventNameType="
                            + info._ruleInfo._eventMappingSettings.RealType.ToString()
                            + "; _indexOfFirstRuleInfoWithSameProvider="
        ...
        internal class RuleInfo
        {
            ...
            internal bool Match(Type eventType, int eventCode)
            {
                ...
                if (
                    !(
                        eventType.Equals(_eventMappingSettings.RealType)
                ...
                if (
                    !(
                        _eventMappingSettings.StartEventCode <= eventCode
        ...
        internal class ProviderInstances
        {
            ...
            internal ProviderInstances(HealthMonitoringSection section)
            {
                ...
                _instances = CollectionsUtil.CreateCaseInsensitiveHashtable(
            ...
            {
                get { return GetProviderInstance(name); }
            }
