...
namespace System.Web.Configuration
{
    ...
    //
    internal sealed class ExpressServerConfig
        : IServerConfig,
            IServerConfig2,
            IConfigMapPath,
            IConfigMapPath2,
            IDisposable
    {
        ...
        string CurrentAppSiteName
        {
            get
            {
                ...
                if (name == null)
                {
                    ...
                    if (name == null)
                    {
                        name = _nativeConfig.GetSiteNameFromId(
        ...
        )
        {
            ...
            try
            {
                int result = _nativeConfig.MgdGetVrPathCreds(
                    appHost.GetSiteName(),
        ...
        )
        {
            if (
                ...
                || StringUtil.EqualsIgnoreCase(
                    siteArgument,
                    _nativeConfig.GetSiteNameFromId(ProcessHostConfigUtils.DEFAULT_SITE_ID_UINT)
                )
            )
            {
                siteName = _nativeConfig.GetSiteNameFromId(
                    ProcessHostConfigUtils.DEFAULT_SITE_ID_UINT
                );
                siteID = ProcessHostConfigUtils.DEFAULT_SITE_ID_STRING;
            }
        ...
        VirtualPath GetAppPathForPathWorker(string siteID, VirtualPath path)
        {
            ...
            try
            {
                int result = _nativeConfig.MgdGetAppPathForPath(
                    siteValue,
                    path.VirtualPathString,
                    out pBstr,
                    out cBstr
        ...
        private string MapPathCaching(string siteID, VirtualPath path)
        {
            ...
            string physicalPath = _nativeConfig.MapPathDirect(
                ((IServerConfig)this).GetSiteNameFromSiteID(siteID),
                path
            );
