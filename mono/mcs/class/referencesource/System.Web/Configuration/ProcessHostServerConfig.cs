...
namespace System.Web.Configuration
{
    ...
    internal sealed class ProcessHostServerConfig : IServerConfig, IServerConfig2
    {
        ...
        [SuppressMessage(
            ...
            Justification = "Private constructor used to make a singleton instance."
        )]
        ProcessHostServerConfig()
        {
            ...
            {
                _siteNameForCurrentApplication = ProcessHostConfigUtils.GetSiteNameFromId(
                    ProcessHostConfigUtils.DEFAULT_SITE_ID_UINT
                );
            }
        }
        ...
        {
            string siteName =
                (appHost == null) ? _siteNameForCurrentApplication : appHost.GetSiteName();
        ...
        string[] IServerConfig.GetVirtualSubdirs(VirtualPath path, bool inApp)
        {
            ...
            {
                int count = 0;
                int result = UnsafeIISMethods.MgdGetAppCollection(
                    IntPtr.Zero,
                ...
                {
                    throw new InvalidOperationException(
                        SR.GetString(SR.Cant_Enumerate_NativeDirs, result)
                    );
                }
                ...
                string appRootRelativePath =
                    (lenNoTrailingSlash > lenAppRoot)
                        ? vpath.Substring(lenAppRoot, lenNoTrailingSlash - lenAppRoot)
                        : String.Empty;

                for (uint index = 0; index < count; index++)
                {
                    result = UnsafeIISMethods.MgdGetNextVPath(
                        pAppCollection,
                        index,
                        out pBstr,
                        out cBstr
                    ...
                    string subVdir =
                        (cBstr > 1) ? StringUtil.StringFromWCharPtr(pBstr, cBstr) : null;
                    Marshal.FreeBSTR(pBstr);
                    pBstr = IntPtr.Zero;
                    cBstr = 0;
                    ...
                    if (subVdir != null && subVdir.Length > appRootRelativePath.Length)
                    {
                        if (appRootRelativePath.Length == 0)
                        {
                            ...
                            StringUtil.EqualsIgnoreCase(
                                ...
                                subVdir,
                                0,
                                appRootRelativePath.Length
                            )
                        )
                        {
                            int nextSlashIndex = subVdir.IndexOf(
                                '/',
                                1 + appRootRelativePath.Length
                            );
                            if (nextSlashIndex > -1)
                            {
                                dirList[dirListCount++] = subVdir.Substring(
                                    appRootRelativePath.Length + 1,
                                    nextSlashIndex - appRootRelativePath.Length
                                );
                            }
                            else
                            {
        ...
        )
        {
            ...
            try
            {
                int result = UnsafeIISMethods.MgdGetVrPathCreds(
                    IntPtr.Zero,
                    appHost.GetSiteName(),
                ...
                if (result == 0)
                {
                    username =
                        (cBstrUserName > 0)
                            ? StringUtil.StringFromWCharPtr(pBstrUserName, cBstrUserName)
                            : null;
                    password =
                        (cBstrPassword > 0)
                            ? StringUtil.StringFromWCharPtr(pBstrPassword, cBstrPassword)
                            : null;
                    foundCreds = (
