...
namespace System.Web.Configuration
{
    ...
    {
        private const string InternalConfigSettingsFactoryTypeString =
            "System.Configuration.Internal.InternalConfigSettingsFactory, "
            + AssemblyRef.SystemConfiguration;
        internal const string ConfigSystemTypeString =
            "System.Configuration.Internal.ConfigSystem, " + AssemblyRef.SystemConfiguration;
        ...
        )
        {
                ...
                lock (s_initLock)
                {
                    if (!s_inited)
                    {
                        ...
                        s_configSystem = (IConfigSystem)
                            Activator.CreateInstance(typeConfigSystem, true);
                        s_configSystem.Init(
                        ...
                        {
                            s_configRoot.ConfigChanged += new InternalConfigEventHandler(
                                configSystem.OnConfigurationChanged
                            );
                        }
                        ...
                        Type typeFactory = Type.GetType(
                            InternalConfigSettingsFactoryTypeString,
                            true
                        );
                        s_configSettingsFactory = (IInternalConfigSettingsFactory)
                            Activator.CreateInstance(typeFactory, true);
            ...
            Debug.Assert(
                s_httpConfigSystem != null,
                "s_httpConfigSystem != null - The appdomain is using the client configuration system."
            );
        }
            ...
            if (UseHttpConfigurationSystem)
            {
                if (s_fileChangeEventHandler == null)
                {
                    s_fileChangeEventHandler = new FileChangeEventHandler(
                        s_httpConfigSystem.OnConfigFileChanged
                    );
                }
        ...
        internal static string MachineConfigurationDirectory
        {
            get
            {
                ...
                {
#if !FEATURE_PAL
                    s_MachineConfigurationDirectory = Path.Combine(
                        MsCorLibDirectory,
                        MachineConfigSubdirectory
                    );
#else // !FEATURE_PAL
        ...
        internal static string MachineConfigurationFilePath
        {
            get
            {
                ...
                {
                    s_MachineConfigurationFilePath = Path.Combine(
        ...
        internal static string RootWebConfigurationFilePath
        {
            get
            {
                ...
                {
                    s_RootWebConfigurationFilePath = Path.Combine(
                        MachineConfigurationDirectory,
