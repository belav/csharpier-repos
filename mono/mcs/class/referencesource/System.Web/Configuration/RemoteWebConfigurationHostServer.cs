...
#endif // !FEATURE_PAL
    using System.Security.Permissions;using System.Diagnostics.CodeAnalysis;
    ...
    public class RemoteWebConfigurationHostServer : IRemoteWebConfigurationHostServer
    {
        ...
        )
        {
            ...
            if (fileExists)
            {
                ...
                if (
                    ((int)(fileAttributes & (FileAttributes.ReadOnly | FileAttributes.Hidden))) != 0
                )
            ...
            if (
                !UnsafeNativeMethods.MoveFileEx(
                    tempFile,
                    fileName,
        ...
        )
        {
            ...
            string configPath,
                locationConfigPath;
            WebConfigurationHost.GetConfigPaths(
                configMapPath,
                webLevel,
                VirtualPath.CreateNonRelativeAllowNull(path),
                site,
                locationSubPath,
                out appPath,
                out appSiteName,
            ...
            if (webLevel != WebLevel.Machine)
            {
                ...
                for (
                    ...
                    currentVirtualPath = currentVirtualPath.Parent
                )
                {
                    string currentConfigPath = WebConfigurationHost.GetConfigPathFromSiteIDAndVPath(
                        appSiteID,
                        currentVirtualPath
                    );
                    string currentFilePath = configMapPath.MapPath(
                        appSiteID,
        ...
        [SuppressMessage(
            ...
            Justification = "Developer-controlled xml contents are implicitly trusted by ASP.Net."
        )]
        public string DoEncryptOrDecrypt(
            bool doEncrypt,
            string xmlString,
            string protectionProviderName,
            string protectionProviderType,
            string[] paramKeys,
            string[] paramValues
        )
        {
        ...
        private static string GetRandomFileExt()
        {
            ...
            return buf[1].ToString("X", CultureInfo.InvariantCulture)
                + buf[0].ToString("X", CultureInfo.InvariantCulture);
        }
