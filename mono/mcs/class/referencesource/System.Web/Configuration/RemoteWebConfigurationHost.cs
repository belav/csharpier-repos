...
namespace System.Web.Configuration
{
    ...
    internal sealed class RemoteWebConfigurationHost : DelegatingConfigHost
    {
        ...
        )
        {
            ...
            try
            {
                WindowsImpersonationContext wiContext =
                    (_Identity != null) ? _Identity.Impersonate() : null;
                try
                {
                    IRemoteWebConfigurationHostServer remoteSrv =
                        RemoteWebConfigurationHost.CreateRemoteObject(
                            ...
                            _Domain,
                            password
                        ); //(IRemoteWebConfigurationHostServer) Activator.CreateInstance(type);
                    try
                    {
                        filePaths = remoteSrv.GetFilePaths(
                            (int)webLevel,
                            path,
                            site,
                            locationSubPath
            ...
            if (filePaths == null)
            {
                throw ExceptionUtil.UnexpectedError(
                    "RemoteWebConfigurationHost::InitForConfiguration"
                );
            }
            ...
            //
            string[] parts = filePaths.Split(
                RemoteWebConfigurationHostServer.FilePathsSeparatorParams
            );
            ...
            for (int i = 5; i < parts.Length; i += 2)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        WebConfigurationHost.GetSiteIDAndVPathFromConfigPath(
                            configPathTemp,
                            out dummy,
                            out vPath
                        );
            ...
            webConfigurationHost.Init(
                root,
                true,
                new UserMapPath(
                    configFileMap, /*pathsAreLocal*/
                    false
                ),
                null,
                appPath,
        ...
        public override object GetStreamVersion(string streamName)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    IRemoteWebConfigurationHostServer remoteSrv = CreateRemoteObject(
                        _Server,
                        _Username,
                        _Domain,
                        _Password
            ...
            return new FileDetails(
                exists,
                size,
                DateTime.FromFileTimeUtc(createDate),
                DateTime.FromFileTimeUtc(lastWriteDate)
            );
#endif // FEATURE_PAL
        }
        ...
        {
            RemoteWebConfigurationHostStream rcs = new RemoteWebConfigurationHostStream(
                false,
                _Server,
                streamName,
                null,
        ...
        public override void WriteCompleted(string streamName, bool success, object writeContext)
        {
            if (success)
            {
                RemoteWebConfigurationHostStream rcs =
                    (RemoteWebConfigurationHostStream)writeContext;
        ...
        /////////////////////////////////////////////////////////////////////////////////
        /////////////////////////////////////////////////////////////////////////////////
        public override void GetRestrictedPermissions(
            IInternalConfigRecord configRecord,
            out PermissionSet permissionSet,
            out bool isHostReady
        )
        {
            WebConfigurationHost.StaticGetRestrictedPermissions(
                configRecord,
                out permissionSet,
                out isHostReady
            );
        ...
        public override string EncryptSection(
            string clearTextXmlString,
            ProtectedConfigurationProvider protectionProvider,
            ProtectedConfigurationSection protectedConfigSection
        )
        {
        ...
        private string CallEncryptOrDecrypt(
            bool doEncrypt,
            string xmlString,
            ProtectedConfigurationProvider protectionProvider,
            ProtectedConfigurationSection protectedConfigSection
            ...
            try
            {
                try
                {
                    ...
                    try
                    {
                        //////////////////////////////////////////////////////////////////
                        // Step 4: Call the API
                        returnString = remoteSrv.DoEncryptOrDecrypt(
                            doEncrypt,
                            xmlString,
...
#if OLD_WAY
        private static WindowsIdentity CreateWindowsIdentity(
            string userName,
            string password,
            IntPtr tokenHandle
        )
        ...
        internal static IRemoteWebConfigurationHostServer CreateRemoteObject(
            string server,
            string username,
            string domain,
            string password
        )
        {
            ...
            catch (COMException ex)
            {
                if ((uint)ex.ErrorCode == 0x80040154)
                    throw new Exception(
                        SR.GetString(SR.Make_sure_remote_server_is_enabled_for_config_access)
                    );
                throw;
            }
        }

        private static IRemoteWebConfigurationHostServer CreateRemoteObjectUsingGetTypeFromCLSID(
            string server
        )
        {
            Type type = Type.GetTypeFromCLSID(
                typeof(RemoteWebConfigurationHostServer).GUID,
                server,
                true
            );
        ...
        private static IRemoteWebConfigurationHostServer CreateRemoteObjectOn32BitPlatform(
            string server,
            string username,
            string domain,
            string password
        )
        {
            ...
            try
            {
                ...
                Marshal.StructureToPtr(
                    typeof(IRemoteWebConfigurationHostServer).GUID,
                    guidbuf,
                    false
                );
                ...
                ca = new COAUTHINFO(
                    RpcAuthent.WinNT,
                    RpcAuthor.None,
                    null, /*RpcLevel.Connect*/
                    RpcLevel.Default,
                    RpcImpers.Impersonate,
                    ciptr
                );
                ...
                hr = UnsafeNativeMethods.CoCreateInstanceEx(
                    ref clsid,
                    IntPtr.Zero,
                    (int)ClsCtx.RemoteServer,
                    cs,
                    1,
                    amqi
                );
                ...
                hr = UnsafeNativeMethods.CoSetProxyBlanket(
                    amqi[0].pItf,
                    RpcAuthent.WinNT,
                    RpcAuthor.None,
                    null, /*RpcLevel.Connect*/
                ...
                return (IRemoteWebConfigurationHostServer)
                    Marshal.GetObjectForIUnknown(amqi[0].pItf);
            }
            finally
            {
        ...
        private static IRemoteWebConfigurationHostServer CreateRemoteObjectOn64BitPlatform(
            string server,
            string username,
            string domain,
            string password
