...
*/
    public sealed class IdentitySection : ConfigurationSection
    {
        ...
        private static readonly ConfigurationProperty _propImpersonate = new ConfigurationProperty(
            "impersonate",
            typeof(bool),
            false,
            ConfigurationPropertyOptions.None
        );
        private static readonly ConfigurationProperty _propUserName = new ConfigurationProperty(
            "userName",
            typeof(string),
            String.Empty,
            ConfigurationPropertyOptions.None
        ...
        {
            get { return (string)base[_propUserName]; }
            set { base[_propUserName] = value; }
        }
        ...
        )
        {
            ...
            if (Impersonate != source.Impersonate)
            { // this will not be copied by unmerge if it is the same as parent
            ...
            {
                if (
                    source.ElementInformation.Properties[_propUserName.Name].IsModified
        ...
        internal static IntPtr CreateUserToken(String name, String password, out String error)
        {
            ...
            if (VersionInfo.ExeName == "aspnet_wp")
            {
                ...
                if (
                    UnsafeNativeMethods.PMCallISAPI(
                        IntPtr.Zero,
                ...
                )
                {
                    ...
                    Debug.Trace(
                        "Token",
                        "Token " + token + " for (" + name + "," + password + ") obtained via ISAPI"
                    );
                }
            }
            ...
            if (token == IntPtr.Zero)
            {
                ...
                if (token != IntPtr.Zero)
                {
                    Debug.Trace(
                        "Token",
                        "Token " + token + " for (" + name + "," + password + ") obtained directly"
                    );
                }
            }
            else
