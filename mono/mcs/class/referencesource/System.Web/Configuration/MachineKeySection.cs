    ...
    */
    ...
    public sealed class MachineKeySection : ConfigurationSection
    {
        private const string OBSOLETE_CRYPTO_API_MESSAGE =
            "This API exists only for backward compatibility; new framework features that require cryptographic services MUST NOT call it. New features should use the AspNetCryptoServiceProvider class instead.";
        ...
        private static readonly ConfigurationProperty _propDecryption = new ConfigurationProperty(
            "decryption",
            typeof(string),
            "Auto",
            StdValidatorsAndConverters.WhiteSpaceTrimStringConverter,
        ...
        private static readonly ConfigurationProperty _propValidation = new ConfigurationProperty(
            "validation",
            typeof(string),
            DefaultValidationAlgorithm,
            StdValidatorsAndConverters.WhiteSpaceTrimStringConverter,
            StdValidatorsAndConverters.NonEmptyStringValidator,
            ConfigurationPropertyOptions.None
        );
        ...
        private static readonly ConfigurationProperty _propCompatibilityMode =
            new ConfigurationProperty(
                "compatibilityMode",
                typeof(MachineKeyCompatibilityMode),
        ...
        internal byte[] ValidationKeyInternal
        {
            ...
            {
                RuntimeDataInitialize();
                return (byte[])_ValidationKey.Clone();
            }
        }
        internal byte[] DecryptionKeyInternal
        {
            ...
            {
                RuntimeDataInitialize();
                return (byte[])_DecryptionKey.Clone();
            }
        }
        ...
        {
            get { return GetApplicationConfig().CompatibilityMode; }
        }
        ...
        {
            get { return (string)base[_propValidationKey]; }
            set { base[_propValidationKey] = value; }
        }
        ...
        {
            get { return (string)base[_propDecryptionKey]; }
            set { base[_propDecryptionKey] = value; }
        }
        ...
        public string Decryption
        {
            get
            {
                ...
                if (
                    ...
                    && s != "3DES"
                    && s != "DES"
                    && !s.StartsWith(ALGO_PREFIX, StringComparison.Ordinal)
                )
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Wrong_decryption_enum),
            ...
            set
            {
                if (
                    ...
                    && value != "Auto"
                    && value != "DES"
                    && !value.StartsWith(ALGO_PREFIX, StringComparison.Ordinal)
                )
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Wrong_decryption_enum),
                        ElementInformation.Properties["decryption"].Source,
        ...
        {
            get { return (string)base[_propDataProtectorType]; }
            set { base[_propDataProtectorType] = value; }
        }
        ...
        {
            get { return (string)base[_propApplicationName]; }
            set { base[_propApplicationName] = value; }
        }
        ...
        [ConfigurationProperty(
            "compatibilityMode",
            DefaultValue = MachineKeyCompatibilityMode.Framework20SP1
        )]
        ...
        private void RuntimeDataInitialize()
        {
            if (DataInitialized == false)
            {
                ...
                if (appName == null)
                {
                    ...
                    if (
                        ValidationKey.Contains("AutoGenerate")
                        || DecryptionKey.Contains("AutoGenerate")
                    )
                    {
                        fNonHttpApp = true;

                        bKeysRandom = new byte[
                            _AutoGenValidationKeySize + _AutoGenDecryptionKeySize
                        ];
                ...
                else
                {
                    if (strKey.Length < 40 || (strKey.Length & 0x1) == 1)
                        throw new ConfigurationErrorsException(
                            SR.GetString(
                                SR.Unable_to_get_cookie_authentication_validation_key,
                                strKey.Length.ToString(CultureInfo.InvariantCulture)
                            ),
                            ElementInformation.Properties["validationKey"].Source,
                            ElementInformation.Properties["validationKey"].LineNumber
                        );
...
#pragma warning restore 618
                    if (_ValidationKey == null)
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Invalid_validation_key),
                            ElementInformation.Properties["validationKey"].Source,
                            ElementInformation.Properties["validationKey"].LineNumber
                        );
                }
                ...
                else
                {
                    ...
                    if ((strKey.Length & 1) != 0)
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Invalid_decryption_key),
                            ElementInformation.Properties["decryptionKey"].Source,
                            ElementInformation.Properties["decryptionKey"].LineNumber
                        );
...
#pragma warning restore 618
        ...
        internal static byte[] EncryptOrDecryptData(
            bool fEncrypt,
            byte[] buf,
            byte[] modifier,
            int start,
        ...
        )
        {
            ...
            return EncryptOrDecryptData(
                ...
                useLegacyMode,
                ivType,
                !AppSettings.UseLegacyEncryption
            );
        }
        ...
        )
        {
            ...
            try
            {
                ...
                ICryptoTransform cryptoTransform = GetCryptoTransform(
                    fEncrypt,
                    useValidationSymAlgo,
                    useLegacyMode
                );
                ...
                {
                    int ivLength = (
                        useValidationSymAlgo ? _IVLengthValidation : _IVLengthDecryption
                    );
                    byte[] iv = null;
        ...
        private static byte[] GetIVHash(byte[] buf, int ivLength)
        {
            ...
            while (bytesWritten < ivLength)
            {
                ...
                int hr = UnsafeNativeMethods.GetSHA1Hash(
                    hash,
                    hash.Length,
                    newHash,
                    newHash.Length
        ...
        private static void SetInnerOuterKeys(
            byte[] validationKey,
            ref byte[] inner,
            ref byte[] outer
        )
        ...
        private static byte[] GetHMACSHA1Hash(byte[] buf, byte[] modifier, int start, int length)
        {
            ...
            int hr = UnsafeNativeMethods.GetHMACSHA1Hash(
                buf,
                start,
                length,
                modifier,
                (modifier == null) ? 0 : modifier.Length,
                s_inner,
                s_inner.Length,
                s_outer,
                s_outer.Length,
        ...
        // buffers.
        [Obsolete(OBSOLETE_CRYPTO_API_MESSAGE)]
        internal static byte[] GetEncodedData(
            byte[] buf,
            byte[] modifier,
            int start,
            ref int length
        )
        {
            ...
            if (
                s_config.Validation == MachineKeyValidation.TripleDES
                || s_config.Validation == MachineKeyValidation.AES
            )
            {
                returnBuffer = EncryptOrDecryptData(
                    true,
                    returnBuffer,
                    modifier,
                    start,
        ...
        [Obsolete(OBSOLETE_CRYPTO_API_MESSAGE)]
        internal static byte[] GetDecodedData(
            byte[] buf,
            byte[] modifier,
            int start,
            int length,
        ...
        internal static byte[] HashData(byte[] buf, byte[] modifier, int start, int length)
        {
            ...
            if (_CustomValidationTypeIsKeyed)
            {
                return HashDataUsingKeyedAlgorithm(
                    KeyedHashAlgorithm.Create(_CustomValidationName),
                    buf,
                    modifier,
                    start,
                    length,
            ...
            {
                return HashDataUsingNonKeyedAlgorithm(
                    HashAlgorithm.Create(_CustomValidationName),
                    buf,
                    modifier,
                    start,
                    length,
...
#pragma warning restore 618
        ...
        private void SetKeyOnSymAlgorithm(SymmetricAlgorithm symAlgo, byte[] dKey)
        {
            ...
            {
                throw new ConfigurationErrorsException(
                    SR.GetString(SR.Bad_machine_key, e.Message),
                    ElementInformation.Properties["decryptionKey"].Source,
                    ElementInformation.Properties["decryptionKey"].LineNumber
                );
            }
        ...
        private static void ReturnCryptoTransform(
            bool fEncrypt,
            ICryptoTransform ct,
            bool useValidationSymAlgo,
            bool legacyMode
        ...
        private void InitValidationAndEncyptionSizes()
        {
            ...
            switch (ValidationAlgorithm)
            {
                ...
                default:
                    ...
                    {
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Wrong_validation_enum),
                    ...
                    try
                    {
                        ...
                        if (_AutoGenValidationKeySize < 1)
                            _AutoGenValidationKeySize = RoundupNumBitsToNumBytes(
                                alg.InputBlockSize
                            );
                        if (_AutoGenValidationKeySize < 1)
                            _AutoGenValidationKeySize = RoundupNumBitsToNumBytes(
            ...
            switch (Decryption)
            {
                ...
                default:
                    ...
                    try
                    {
                        s_oSymAlgoDecryption = SymmetricAlgorithm.Create(
        ...
        {
            int totalLength =
                length + validationKey.Length + ((modifier != null) ? modifier.Length : 0);
