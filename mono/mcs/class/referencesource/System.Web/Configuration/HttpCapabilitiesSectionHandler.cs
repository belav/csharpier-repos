...
namespace System.Web.Configuration
{
    ...
    public class HttpCapabilitiesSectionHandler : IConfigurationSectionHandler
    {
        ...
        public object Create(object parent, object configurationContext, XmlNode section)
        {
            ...
            parseState.Evaluator = new HttpCapabilitiesDefaultProvider(
            ...
            HandlerBase.GetAndRemoveNonEmptyStringAttribute(
                section,
                "provider",
                ref browserCapabilitiesProviderType
            );
        ...
        static CapabilitiesRule RuleFromElement(ParseState parseState, XmlNode element)
        {
            ...
            if (matchpat == null)
            {
                if (testpat != null)
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Cannot_specify_test_without_match),
                        element
                    );
                regex = null;
                pat = null;
        ...
        static void ProcessFile(ArrayList fileList, XmlNode node)
        {
            ...
            XmlNode attr = HandlerBase.GetAndRemoveRequiredStringAttribute(
                node,
                "src",
                ref filename
            );
        ...
        static void ProcessResult(
            HttpCapabilitiesDefaultProvider capabilitiesEvaluator,
            XmlNode node
        )
        {
            bool inherit = true;
            ...
            if (attribute != null)
            {
                if (resultType.Equals(capabilitiesEvaluator._resultType) == false)
                {
                    ...
                    HandlerBase.CheckAssignableType(
                        attribute,
                        capabilitiesEvaluator._resultType,
                        resultType
                    );
            ...
            int cacheTime = 0;
            attribute = HandlerBase.GetAndRemovePositiveIntegerAttribute(
                node,
                "cacheTime",
                ref cacheTime
            );
        ...
        static void ResolveFiles(ParseState parseState, object configurationContext)
        {
            ...
            HttpConfigurationContext httpConfigurationContext =
                (HttpConfigurationContext)configurationContext;
            ...
            try
            {
                ...
                configurationDirectory = Path.GetDirectoryName(
                    ConfigurationErrorsException.GetFilename(srcAttribute)
                );
            }
            finally
            {
            ...
            foreach (Pair pair in parseState.FileList)
            {
                ...
                try
                {
                    ...
                    {
                        throw new ConfigurationErrorsException(
                            SR.GetString(SR.Error_loading_XML_file, fullFilename, e.Message),
                            e,
                            (XmlNode)pair.Second
                        );
                    }
                ...
                {
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Capability_file_root_element, parseState.SectionName),
                        section
                    );
                }
