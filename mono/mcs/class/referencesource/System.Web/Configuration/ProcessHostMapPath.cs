...
namespace System.Web.Configuration
{
    ...
    internal sealed class ProcessHostMapPath : IConfigMapPath, IConfigMapPath2
    {
        ...
        [SuppressMessage(
            ...
            Justification = "We carefully control this method's caller."
        )]
        ...
        string IConfigMapPath.GetRootWebConfigFilename()
        {
            ...
            Debug.Trace("MapPath", "ProcHostMP.GetRootWebConfigFilename = " + rootWeb);
        ...
        private void GetPathConfigFilenameWorker(
            string siteID,
            VirtualPath path,
        ...
        )
        {
            ...
            Debug.Trace(
                "MapPath",
                "ProcHostMP.GetPathConfigFilename("
                    + siteID
                    + ", "
                    + path
                    + ")\n"
        ...
        void IConfigMapPath.GetDefaultSiteNameAndID(out string siteName, out string siteID)
        {
            ...
            siteName = ProcessHostConfigUtils.GetSiteNameFromId(
                ProcessHostConfigUtils.DEFAULT_SITE_ID_UINT
            );
        }

        void IConfigMapPath.ResolveSiteArgument(
            string siteArgument,
            out string siteName,
            out string siteID
        )
        {
            ...
            if (
                String.IsNullOrEmpty(siteArgument)
                || StringUtil.EqualsIgnoreCase(
                    siteArgument,
                    ProcessHostConfigUtils.DEFAULT_SITE_ID_STRING
                )
                || StringUtil.EqualsIgnoreCase(
                    siteArgument,
        ...
        VirtualPath GetAppPathForPathWorker(string siteID, VirtualPath path)
        {
            Debug.Trace(
                "MapPath",
                "ProcHostMP.GetAppPathForPath(" + siteID + ", " + path.VirtualPathString + ")\n"
            );
            ...
            try
            {
                int result = UnsafeIISMethods.MgdGetAppPathForPath(
                    IntPtr.Zero,
                    siteValue,
                ...
                appPath =
                    (result == 0 && cBstr > 0) ? StringUtil.StringFromWCharPtr(pBstr, cBstr) : null;
            }
            finally
            {
                ...
                lock (cacheInfo)
                {
                    if (!cacheInfo.Evaluated && HttpRuntime.IsMapPathRelaxed)
                    {
                        ...
                        if (path.VirtualPathString.Length > 1)
                        {
                            ...
                            if (vParent != null)
                            {
                                ...
                                if (
                                    parentPath.Length > 1
                                    && StringUtil.StringEndsWith(parentPath, '/')
                                )
                                { // Trim the extra trailing / if there is one
                                    vParent = VirtualPath.Create(
                                ...
                                {
                                    cacheInfo.MapPathResult = HttpRuntime.GetRelaxedMapPathResult(
                                        null
                                    );
                                    cacheInfo.Evaluated = true;
                                }
                    ...
                    if (!cacheInfo.Evaluated)
                    {
                        try
                        {
                            ...
                            {
                                string siteName = ProcessHostConfigUtils.GetSiteNameFromId(
                                    siteIDValue
                                );
