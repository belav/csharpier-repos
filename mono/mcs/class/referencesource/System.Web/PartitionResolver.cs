    ...
    class PartitionManager : IDisposable
    {
        ...
        internal object GetPartition(IPartitionResolver partitionResolver, string id)
        {
            if (EtwTrace.IsTraceEnabled(EtwTraceLevel.Verbose, EtwTraceFlags.Infrastructure))
                EtwTrace.Trace(
                    EtwTraceType.ETW_TYPE_SESSIONSTATE_PARTITION_START,
            ...
            try
            {
                try
                {
                    ...
                    if (partitionString == null)
                    {
                        throw new HttpException(
                            SR.GetString(
                                SR.Bad_partition_resolver_connection_string,
                                partitionResolver.GetType().FullName
                            )
                        );
                    }
                ...
                try
                {
                    ...
                    if (partitionInfo != null)
                    {
                        Debug.Trace(
                            "PartitionManager",
                            "id=" + id + "; partitionString=" + partitionString
                        );
                ...
                try
                {
                    ...
                    if (partitionInfo == null)
                    {
                        ...
                        Debug.Trace(
                            "PartitionManager",
                            "Add a new partition; id=" + id + "; partitionString=" + partitionString
                        );
            ...
            finally
            {
                if (EtwTrace.IsTraceEnabled(EtwTraceLevel.Verbose, EtwTraceFlags.Infrastructure))
                {
                    ...
                    EtwTrace.Trace(
                        EtwTraceType.ETW_TYPE_SESSIONSTATE_PARTITION_END,
                        HttpContext.Current.WorkerRequest,
                        msg
                    );
                }
