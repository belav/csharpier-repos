...
namespace System.Web
{
    ...
    */
            ...
            get
            {
                if (isEnabled == -1)
                {
                    ...
                    Hashtable h = (Hashtable)
                        WebConfigurationManager.GetWebApplicationSection("httpDebug");
                    if (h != null)
                    {
        ...
        internal static void PrintTickDelta(string s)
        {
            ...
            if (initialTick == 0)
            {
                ...
                SafeNativeMethods.OutputDebugString(
                    s + " : " + initialTick + " (HttpDebugHandler + 0)\n"
                );
                if (writer != null)
                {
            ...
            else
            {
                ...
                SafeNativeMethods.OutputDebugString(
                    s + " : " + curTick + " (HttpDebugHandler + " + (curTick - initialTick) + ")\n"
                );
                if (writer != null)
                {
                    writer.WriteLine(
    ...
    {
        private static string[] validClsIds =
        {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                Debug.Trace(
                    "AutoAttach",
                    "Authentication type string: " + ((authType != null) ? authType : "NULL")
                );
                Debug.Trace(
                    "AutoAttach",
                    "Logon user string: " + ((logonUser != null) ? logonUser : "NULL")
                );
                ...
                Debug.Trace(
                    "AutoAttach",
                    "DebugSessionID: " + ((sessId != null) ? sessId : "NULL")
                );
                ...
                if (isClsIdOk == false)
                {
                    ...
                    Debug.Trace(
                        "AutoAttach",
                        "Debug attach not attempted because of invalid CLSID."
                    );
                    return;
                }
                ...
                int rc = UnsafeNativeMethods.AttachDebugger(
                    clsId,
                    sessId,
