...
namespace System.Web
{
    ...
    internal class HttpApplicationFactory
    {
        ...
        {
            _sessionOnEndEventHandlerAspCompatHelper = new EventHandler(
                this.SessionOnEndEventHandlerAspCompatHelper
            );
        }
        ...
        internal static void ThrowIfApplicationOnStartCalled()
        {
            if (_theApplicationFactory._appOnStartCalled)
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.MethodCannotBeCalledAfterAppStart)
                );
            }
        }
        ...
        internal static void EnsureAppStartCalledForIntegratedMode(
            HttpContext context,
            HttpApplication app
        )
        {
                    ...
                    if (!_theApplicationFactory._appOnStartCalled)
                    {
                        using (new DisposableHttpContextWrapper(context))
                        {
                            ...
                            WebBaseEvent.RaiseSystemEvent(
                                ...
                                WebEventCodes.ApplicationStart
                            );
        ...
        private void GetAppStateByParsingGlobalAsax()
        {
            using (new ApplicationImpersonationContext())
            {
                ...
                if (FileUtil.FileExists(_appFilename))
                {
                    ...
                    // Create app state
                    _state = new HttpApplicationState(
                        parser.ApplicationObjects,
                        parser.SessionObjects
                    );
                }
            }
        }
        ...
        private bool ReflectOnMethodInfoIfItLooksLikeEventHandler(MethodInfo m)
        {
            ...
            if (
                StringUtil.EqualsIgnoreCase(name, "Application_OnStart")
            ...
            )
            {
                ...
                StringUtil.EqualsIgnoreCase(name, "Application_OnEnd")
            ...
            )
            {
                ...
                StringUtil.EqualsIgnoreCase(name, "Session_OnEnd")
        ...
        private void ReflectOnApplicationType()
        {
            ...
            methods = _theApplicationType.GetMethods(
        ...
        private void OnAppFileChange(Object sender, FileChangeEvent e)
        {
            ...
            Debug.Trace(
                "AppDomainFactory",
                "Shutting down appdomain because of application file change"
            );
        ...
        internal static HttpApplication GetPipelineApplicationInstance(
            IntPtr appContext,
            HttpContext context
        )
        {
        ...
        class AspCompatSessionOnEndHelper
        {
            ...
            internal AspCompatSessionOnEndHelper(
                HttpApplication app,
        ...
        private void SessionOnEndEventHandlerAspCompatHelper(
            Object eventSource,
            EventArgs eventArgs
        )
        {
        ...
        )
        {
            if (_sessionOnEndMethod != null)
            {
                ...
                if (
                    AspCompatApplicationStep.AnyStaObjectsInSessionState(session)
                    || HttpRuntime.ApartmentThreading
                )
                {
                    AspCompatSessionOnEndHelper helper = new AspCompatSessionOnEndHelper(
                        app,
                        session,
                        eventSource,
                        eventArgs
        ...
        private static void DisposeHttpApplicationInstances(
            Stack freeList,
            ref int numFreeInstances
        )
        {
            ...
            if (_theApplicationFactory != null)
            {
                if (removeAll)
                {
                    ...
                    DisposeHttpApplicationInstances(
                        _theApplicationFactory._freeList,
                        ref _theApplicationFactory._numFreeAppInstances
                    );
                }
                else
                {
