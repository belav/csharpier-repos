...
namespace System.Web.Util
{
    ...
    internal class FileUtil
    {
        ...
        {
            if (
                filePath.Length > appRoot.Length + 1
        ...
        internal static String TruncatePathIfNeeded(string path, int reservedLength)
        {
            ...
            if (path.Length > maxPathLength)
            {
                ...
                path =
                    path.Substring(0, maxPathLength - 13)
        ...
        internal static void PhysicalPathStatus(
            string physicalPath,
            bool directoryExistsOnError,
            bool fileExistsOnError,
            out bool exists,
        ...
        )
        {
            ...
            using (new ApplicationImpersonationContext())
            {
                ...
                bool ok = UnsafeNativeMethods.GetFileAttributesEx(
                    physicalPath,
                    UnsafeNativeMethods.GetFileExInfoStandard,
                    out data
                );
                if (ok)
                {
                    exists = true;
                    isDirectory = (
                        (data.fileAttributes & (int)FileAttributes.Directory)
        ...
        internal static bool DirectoryExists(string filename, bool trueOnError)
        {
            ...
            if (ok)
            {
                ...
                return (data.fileAttributes & (int)FileAttributes.Directory)
                    == (int)FileAttributes.Directory;
            }
            else
            {
    ...
    sealed class FindFileData
    {
        ...
        static internal int FindFile(string path, out FindFileData data)
        {
            ...
            file = file.TrimEnd(' ', '.');
            Debug.Assert(
                StringUtil.EqualsIgnoreCase(file, wfd.cFileName)
        ...
        internal static int FindFile(
            string fullPath,
            string rootDirectoryPath,
            out FindFileData data
        )
        {
            ...
            Debug.Assert(
                rootDirectoryPath.Length < 4
                    || rootDirectoryPath[rootDirectoryPath.Length - 1] != '\\',
                "Trailing slash unexpected: " + rootDirectoryPath
            );
#endif
...
#if DBG
            Debug.Assert(
                fullPath.IndexOf(rootDirectoryPath, StringComparison.OrdinalIgnoreCase) == 0,
                "fullPath ("
                    + fullPath
                    + ") is not within rootDirectoryPath ("
                    + rootDirectoryPath
                    + ")"
            );
#endif
            ...
            while (
                ...
                && currentParentDir.IndexOf(rootDirectoryPath, StringComparison.OrdinalIgnoreCase)
                    == 0
            )
            {
...
#if DBG
                Debug.Assert(
                    !String.IsNullOrEmpty(fd.cFileName),
                    "!String.IsNullOrEmpty(fd.cFileName)"
                );
#endif
                ...
                {
                    relativePathShort =
                        fd.cAlternateFileName + Path.DirectorySeparatorChar + relativePathShort;
                }
                else
                {
                    relativePathShort =
                        fd.cFileName + Path.DirectorySeparatorChar + relativePathShort;
                }
            ...
            Debug.Trace(
                "FindFile",
                "fullPath=" + fullPath + ", rootDirectoryPath=" + rootDirectoryPath
            );
            Debug.Trace(
                "FindFile",
                "relativePathLong=" + relativePathLong + ", relativePathShort=" + relativePathShort
            );
            string fileNameShort = data.FileNameShort == null ? "<null>" : data.FileNameShort;
            Debug.Trace(
                "FindFile",
                "FileNameLong=" + data.FileNameLong + ", FileNameShrot=" + fileNameShort
            );
#endif
            return hr;
        }
    ...
    sealed class FileAttributesData
    {
        ...
        internal static int GetFileAttributes(string path, out FileAttributesData fad)
        {
            ...
            UnsafeNativeMethods.WIN32_FILE_ATTRIBUTE_DATA data;
            if (
                !UnsafeNativeMethods.GetFileAttributesEx(
                    path,
                    UnsafeNativeMethods.GetFileExInfoStandard,
                    out data
                )
        ...
        FileAttributesData(ref UnsafeNativeMethods.WIN32_FILE_ATTRIBUTE_DATA data)
        {
            ...
            UtcCreationTime = DateTimeUtil.FromFileTimeToUtc(
                ((long)data.ftCreationTimeHigh) << 32 | (long)data.ftCreationTimeLow
            );
            UtcLastAccessTime = DateTimeUtil.FromFileTimeToUtc(
                ((long)data.ftLastAccessTimeHigh) << 32 | (long)data.ftLastAccessTimeLow
            );
            UtcLastWriteTime = DateTimeUtil.FromFileTimeToUtc(
                ((long)data.ftLastWriteTimeHigh) << 32 | (long)data.ftLastWriteTimeLow
            );
        ...
        internal FileAttributesData(ref UnsafeNativeMethods.WIN32_FIND_DATA wfd)
        {
            ...
            UtcCreationTime = DateTimeUtil.FromFileTimeToUtc(
                ((long)wfd.ftCreationTime_dwHighDateTime) << 32
                    | (long)wfd.ftCreationTime_dwLowDateTime
            );
            UtcLastAccessTime = DateTimeUtil.FromFileTimeToUtc(
                ((long)wfd.ftLastAccessTime_dwHighDateTime) << 32
                    | (long)wfd.ftLastAccessTime_dwLowDateTime
            );
            UtcLastWriteTime = DateTimeUtil.FromFileTimeToUtc(
                ((long)wfd.ftLastWriteTime_dwHighDateTime) << 32
                    | (long)wfd.ftLastWriteTime_dwLowDateTime
            );
        ...
        internal string DebugDescription(string indent)
        {
            ...
            sb.Append(
                i2
                    + "      FileSize: "
                    + FileSize.ToString("n0", NumberFormatInfo.InvariantInfo)
                    + "\n"
            );
