...
namespace System.Web.Util
{
    ...
    internal static class StringUtil
    {
        ...
        internal static string CheckAndTrimString(
            string paramValue,
            string paramName,
            bool throwIfNull
        )
        ...
        )
        {
            ...
            if (trimmedValue.Length == 0)
            {
                throw new ArgumentException(
                    SR.GetString(SR.PersonalizationProviderHelper_TrimmedEmptyString, paramName)
                );
            }
            if (lengthToCheck > -1 && trimmedValue.Length > lengthToCheck)
            {
                throw new ArgumentException(
                    SR.GetString(
                        SR.StringUtil_Trimmed_String_Exceed_Maximum_Length,
                        paramValue,
                        paramName,
        ...
        )
        {
            ...
            if ((s1 == null ? 0 : s1.Length) - offset1 < length)
                throw new ArgumentOutOfRangeException(
                    SR.GetString(SR.InvalidOffsetOrCount, "offset1", "length")
                );
            if ((s2 == null ? 0 : s2.Length) - offset2 < length)
                throw new ArgumentOutOfRangeException(
                    SR.GetString(SR.InvalidOffsetOrCount, "offset2", "length")
                );
            if (String.IsNullOrEmpty(s1) || String.IsNullOrEmpty(s2))
                return true;
        ...
        internal static bool StringEndsWithIgnoreCase(string s1, string s2)
        {
            ...
            return 0
                == string.Compare(s1, offset, s2, 0, s2.Length, StringComparison.OrdinalIgnoreCase);
        }
        ...
        [SuppressMessage(
            ...
            Justification = "We carefully control the callers."
        )]
        internal static int GetNonRandomizedStringComparerHashCode(string s)
        {
            ...
            if (appManager != null)
            {
                ...
                int hashCode = 0;
                ExecutionContextUtil.RunInNullExecutionContext(
                    delegate
                    {
                        hashCode = appManager.GetNonRandomizedStringComparerHashCode(
                            s,
                            ignoreCase: true
                        );
                    }
