...
namespace System.Web.Util
{
    internal class RegexUtil
    {
        ...
        )
        {
            ...
            if (timeout > 0 || timeoutInMillsec.HasValue)
            {
                return Regex.IsMatch(
                    ...
                    pattern,
                    regOption,
                    TimeSpan.FromMilliseconds((double)timeout)
                );
            }
            else
            {
        ...
        private static bool IsRegexTimeoutSetInAppDomain
        {
            get
            {
                if (!_isRegexTimeoutSetInAppDomain.HasValue)
                {
                    ...
                    try
                    {
                        timeoutSetInAppDomain =
                            AppDomain.CurrentDomain.GetData("REGEX_DEFAULT_MATCH_TIMEOUT") != null;
                    }
                    catch { }
        ...
        private static int GetRegexTimeout(int? timeoutInMillsec)
        {
            ...
            {
                if (
                    !IsRegexTimeoutSetInAppDomain
                    && BinaryCompatibility.Current.TargetsAtLeastFramework461
                )
                {
                    timeout = 2000;
                }
