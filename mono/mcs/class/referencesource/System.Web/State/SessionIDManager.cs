...
namespace System.Web.SessionState
{
    ...
    public class SessionIDManager : ISessionIDManager
    {
        ...
        internal const String ASP_SESSIONID_MANAGER_INITIALIZEREQUEST_CALLED_KEY =
...
#pragma warning restore 0649
        ...
        internal void GetCookielessSessionID(
            HttpContext context,
            bool allowRedirect,
            out bool cookieless
        )
        {
            ...
            cookieless = CookielessHelperClass.UseCookieless(
                context,
                allowRedirect,
                Config.Cookieless
            );
            ...
            if (cookieless)
            {
                ...
                Debug.Trace(
                    "SessionIDManager",
                    "CookielessSessionModule found SessionId="
                        + id
            ...
            if (id.Length > SESSION_ID_LENGTH_LIMIT)
            {
                if (throwOnFail)
                {
                    throw new HttpException(
                        SR.GetString(
                            SR.Session_id_too_long,
                            SESSION_ID_LENGTH_LIMIT.ToString(CultureInfo.InvariantCulture),
                            id
                        )
                    );
                }
        ...
        void CheckInitializeRequestCalled(HttpContext context)
        {
            ...
            {
                throw new HttpException(
                    SR.GetString(SR.SessionIDManager_InitializeRequest_not_called)
                );
            }
        }
        ...
        )
        {
            ...
            else
            {
                ...
                Debug.Trace(
                    "SessionIDManager",
                    "Redirecting to create cookieless session, path="
                        + path
                        + ", idEncoded="
                        + idEncoded
