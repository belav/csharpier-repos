...
namespace System.Web.SessionState
{
    ...
    internal sealed class InProcSessionStateStore : SessionStateStoreProviderBase
    {
        ...
        public void OnCacheItemRemoved(String key, Object value, CacheItemRemovedReason reason)
        {
            ...
            if (
                (state._flags & (int)SessionStateItemFlags.IgnoreCacheItemRemoved) != 0
            ...
            if (_expireCallback != null)
            {
                ...
                _expireCallback(
                    id,
                    SessionStateUtility.CreateLegitStoreData(
                        null,
                        state._sessionItems,
        ...
        )
        {
            ...
            // with SQL provider
            SessionIDManager.CheckIdLength(
                id,
                true /* throwOnFail */
            );
            ...
            if (state != null)
            {
                ...
                if ((initialFlags & (int)SessionStateItemFlags.Uninitialized) != 0)
                {
                    ...
                    Debug.Trace(
                        "SessionStateClientSet",
                        "Removing the Uninit flag for item; key = " + key
                    );
                    if (
                        initialFlags
                ...
                {
                    return SessionStateUtility.CreateLegitStoreData(
                        context,
                        state._sessionItems,
                        state._staticObjects,
                        state._timeout
        ...
        )
        {
            ...
            if (!newItem)
            {
                ...
                Debug.Assert(
                    (stateCurrent._flags & (int)SessionStateItemFlags.Uninitialized) == 0,
                    "Should never set an unitialized item; key = " + key
                );
                ...
                try
                {
                    ...
                    if (!stateCurrent._locked || stateCurrent._lockCookie != lockCookie)
                    {
                        Debug.Trace(
                            "SessionStateClientSet",
                            "Leave because we're not the owner; key = " + key
                        );
                        return;
                    }
                    ...
                    if (stateCurrent._timeout == item.Timeout)
                    {
                        ...
                        Debug.Trace(
                            "SessionStateClientSet",
                            "Changing state inplace; key = " + key
                        );
                    }
                    else
                    {
        ...
        public override void CreateUninitializedItem(HttpContext context, String id, int timeout)
        {
            ...
            Debug.Assert(
                timeout <= SessionStateModule.MAX_CACHE_BASED_TIMEOUT_MINUTES,
                "item.Timeout <= SessionStateModule.MAX_CACHE_BASED_TIMEOUT_MINUTES"
            );
            ...
            Debug.Trace(
                "SessionStateClientSet",
                "Inserting an uninitialized item into Cache; key = " + key
            );
        ...
        internal static void TraceSessionStats()
        {
            ...
            Debug.Trace(
                "SessionState",
                "sessionsTotal="
                    + PerfCounters.GetCounter(AppPerfCounter.SESSIONS_TOTAL)
                    + ", sessionsActive="
                    + PerfCounters.GetCounter(AppPerfCounter.SESSIONS_ACTIVE)
                    + ", sessionsAbandoned="
                    + PerfCounters.GetCounter(AppPerfCounter.SESSIONS_ABANDONED)
                    + ", sessionsTimedout="
                    + PerfCounters.GetCounter(AppPerfCounter.SESSIONS_TIMED_OUT)
            );
#endif
        }
    }
