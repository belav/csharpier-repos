...
namespace System.Web.SessionState
{
    ...
    class SessionOnEndTargetWorkItem
    {
        ...
        internal SessionOnEndTargetWorkItem(
            SessionOnEndTarget target,
    ...
    class SessionOnEndTarget
    {
        ...
        internal void RaiseSessionOnEnd(String id, SessionStateStoreData item)
        {
            ...
            else
            {
                ...
                SessionOnEndTargetWorkItem workItem = new SessionOnEndTargetWorkItem(
                    this,
                    sessionState
                );
    ...
    {
        internal const string SQL_CONNECTION_STRING_DEFAULT =
        ...
        static readonly TimeSpan LOCKED_ITEM_POLLING_DELTA = new TimeSpan(
            250 * TimeSpan.TicksPerMillisecond
        );

        static readonly TimeSpan DEFAULT_DBG_EXECUTION_TIMEOUT = new TimeSpan(
            0,
            0,
            System.Web.Compilation.PageCodeDomTreeGenerator.DebugScriptTimeout
        );
...
#pragma warning restore 0649
        ...
        {
            return (SessionStateStoreProviderBase)
        ...
        IPartitionResolver InitPartitionResolver(SessionStateSection config)
        {
            ...
            if (
                config.Mode != SessionStateMode.StateServer
                && config.Mode != SessionStateMode.SQLServer
            )
            {
                throw new ConfigurationErrorsException(
                    SR.GetString(SR.Cant_use_partition_resolve),
                    config.ElementInformation.Properties["partitionResolverType"].Source,
                    config.ElementInformation.Properties["partitionResolverType"].LineNumber
                );
            }

            resolverType = ConfigUtil.GetType(
                partitionResolverType,
                "partitionResolverType",
                config
            );
            ConfigUtil.CheckAssignableType(
                typeof(IPartitionResolver),
                resolverType,
                config,
                "partitionResolverType"
            );
        ...
        ISessionIDManager InitSessionIDManager(SessionStateSection config)
        {
            ...
            else
            {
                ...
                managerType = ConfigUtil.GetType(
        ...
        void InitModuleFromConfig(HttpApplication app, SessionStateSection config)
        {
            ...
            if (
                (
                    config.Mode == SessionStateMode.InProc
                    || config.Mode == SessionStateMode.StateServer
            ...
            if (!s_oneTimeInit)
            {
                ...
                try
                {
                    if (!s_oneTimeInit)
                    {
                        ...
                        // for details.
                        if (config.Mode == SessionStateMode.InProc && _usingAspnetSessionIdManager)
                        {
        ...
        )
        {
            ...
            Debug.Trace(
                ...
                "Beginning SessionStateModule::OnAcquireState"
            );
            ...
            try
            {
                if (EtwTrace.IsTraceEnabled(EtwTraceLevel.Information, EtwTraceFlags.AppSvc))
                    EtwTrace.Trace(
                        EtwTraceType.ETW_TYPE_SESSION_DATA_BEGIN,
                ...
                if (_idManager.InitializeRequest(_rqContext, false, out _rqSupportSessionIdReissue))
                {
                    ...
                    if (EtwTrace.IsTraceEnabled(EtwTraceLevel.Information, EtwTraceFlags.AppSvc))
                        EtwTrace.Trace(
                            EtwTraceType.ETW_TYPE_SESSION_DATA_END,
                            _rqContext.WorkerRequest
                        );
                    return _rqAr;
                }
                ...
                if (
                    ...
                    && (
                        !requiresState
                        || // Case 1
                        !((SessionIDManager)_idManager).UseCookieless(_rqContext)
                    )
                )
                { // Case 2
                    skipReadingId = true;
                    ...
                    if (!requiresState)
                    {
                        ...
                        Debug.Trace(
                            "SessionStateModuleOnAcquireState",
                            "Skip reading id because page has disabled session state"
                        );
                    }
                    else
                    {
                        ...
                        Debug.Trace(
                            "SessionStateModuleOnAcquireState",
                            "Delay reading id because we're using InProc optimization, and we are not using cookieless"
                        );
                    }
#endif
                }
                ...
                if (!requiresState)
                {
                    ...
                    else
                    {
                        Debug.Trace(
                            "SessionStateModuleOnAcquireState",
                            "Handler does not require state, "
                                + "resetting timeout for SessionId="
                                + _rqId
                                + "\nReturning from SessionStateModule::OnAcquireState"
                        );
        ...
        internal bool CreateSessionId()
        {
            ...
            Debug.Assert(
                _rqId == null
                    || // Session id isn't found in the request, OR
                    (
                        _rqSessionStateNotFound
                        && // The session state isn't found, AND
                        s_configRegenerateExpiredSessionId
                        && // We are regenerating expired session id, AND
                        _rqSupportSessionIdReissue
                        && // This request supports session id re-issue, AND
                        !_rqIdNew
        ...
        void CompleteAcquireState()
        {
            Debug.Trace(
                "SessionStateModuleOnAcquireState",
                "Item retrieved=" + (_rqItem != null).ToString(CultureInfo.InvariantCulture)
            );
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (s_allowDelayedStateStoreItemCreation)
                    {
                        Debug.Trace(
                            "SessionStateModuleOnAcquireState",
                            "Delay creating new session state"
                        );
                    ...
                    if (
                        ...
                        s_configRegenerateExpiredSessionId
                        && // And we're asked to regenerate expired session
                        _rqSupportSessionIdReissue
                    )
                    { // And this request support session id reissue
                        ...
                        Debug.Trace(
                            "SessionStateModuleOnAcquireState",
                            "Complete re-creating new id; redirected=" + redirected
                        );
            ...
            if (_rqIsNewSession)
            {
                if (_rqId == null)
                {
                    ...
                    Debug.Trace(
                        "SessionStateModuleOnAcquireState",
                        "New session: session id reading is delayed"
                            + "\nReturning from SessionStateModule::OnAcquireState"
                    );
                }
                else
            ...
            else
            {
                Debug.Trace(
                    "SessionStateModuleOnAcquireState",
                    "Retrieved old session, SessionId= "
                        + _rqId
                        + "\nReturning from SessionStateModule::OnAcquireState"
                );
            }
        ...
        {
            Debug.Assert(
                _rqId != null || s_allowInProcOptimization,
                "_rqId != null || s_allowInProcOptimization"
            );
            ...
            try
            {
                ...
                if (_rqSessionItems == null)
                {
                    throw new HttpException(
                        SR.GetString(SR.Null_value_for_SessionStateItemCollection)
                    );
                }
        ...
        internal string DelayedGetSessionId()
        {
            Debug.Assert(
                s_allowInProcOptimization,
                "Shouldn't be called if we don't allow InProc optimization"
            );
        ...
        bool GetSessionStateItem()
        {
            ...
            else
            {
                ...
                if (_rqItem == null && locked == false && _rqId != null)
                {
                    if (
                        !(
                            s_configCookieless == HttpCookieMode.UseUri
                            && s_configRegenerateExpiredSessionId == true
                        )
                    )
                    {
                        CreateUninitializedSessionState();
        ...
        private static bool IsRequestQueueEnabled
        {
            ...
            {
                return (
                    AppSettings.RequestQueueLimitPerSession
                    != AppSettings.UnlimitedRequestsPerSession
                );
            }
        }
        ...
        void ChangeImpersonation(HttpContext context, bool timerThread)
        {
            ...
            // and session id read/write.
            if (
                s_configMode == SessionStateMode.SQLServer
            ...
            else
            {
                if (timerThread)
                {
                    ...
                    _rqTimerThreadImpersonationIctx = new ClientImpersonationContext(
                        context,
                        false
                    );
                }
        ...
        void RestoreImpersonation()
        {
            ...
            {
                Debug.Assert(
                    !(_rqIctx != null && _rqTimerThreadImpersonationIctx != null),
                    "Should not have mixed mode of impersonation"
                );
        ...
        void OnReleaseState(Object source, EventArgs eventArgs)
        {
            ...
            Debug.Trace(
                "SessionStateOnReleaseState",
                "Beginning SessionStateModule::OnReleaseState"
            );

            Debug.Assert(
                !(_rqAddedCookie && !_rqIsNewSession),
                ...
                if (_rqSessionState != null)
                {
                    ...
                    if (
                        ...
                        && (
                            delayedSessionState
                            || _rqStaticObjects == null
                            || _rqStaticObjects.NeverAccessed
                        )
                    )
                    {
                        Debug.Trace(
                            "SessionStateOnReleaseState",
                            "Not storing unused new session."
                        );
                    }
                    else if (_rqSessionState.IsAbandoned)
                    {
                        Debug.Trace(
                            "SessionStateOnReleaseState",
                            "Removing session due to abandonment, SessionId=" + _rqId
                        );
                        ...
                        else
                        {
                            Debug.Assert(
                                _rqItem != null,
                                "_rqItem cannot null if it's not a new session"
                            );

                            // Remove it from the store because the session is abandoned.
                            _store.RemoveItem(
                                _rqContext,
                                ReleaseStateGetSessionID(),
                                _rqLockId,
                                _rqItem
                            );
                        }
                    ...
                    )
                    {
                        ...
                        )
                        {
                            if (delayedSessionState)
                            {
                                Debug.Assert(
                                    _rqIsNewSession,
                                    "Saving a session and delayedSessionState is true: _rqIsNewSession must be true"
                                );
                                Debug.Assert(
                                    s_allowDelayedStateStoreItemCreation,
                                ...
                                Debug.Assert(
                                    _rqItem == null,
                                    "Saving a session and delayedSessionState is true: _rqItem == null"
                                );
                            ...
                            if (_rqSessionItems.Dirty)
                            {
                                Debug.Trace(
                                    "SessionStateOnReleaseState",
                                    "Setting new session due to dirty SessionItems, SessionId="
                                        + _rqId
                                );
                            }
                            else if (_rqStaticObjects != null && !_rqStaticObjects.NeverAccessed)
                            {
                                Debug.Trace(
                                    "SessionStateOnReleaseState",
                                    "Setting new session due to accessed Static Objects, SessionId="
                                        + _rqId
                                );
                            }
                            else if (_rqSessionStateNotFound)
                            {
                                Debug.Trace(
                                    "SessionStateOnReleaseState",
                                    "Setting new session because it's not found, SessionId=" + _rqId
                                );
                            }
                            else
                            {
                                Debug.Trace(
                                    "SessionStateOnReleaseState",
                                    "Setting new session due to options change, SessionId="
                                        + _rqId
                                        + "\n\t_rq.timeout="
                        ...
                        else
                        {
                            ...
                            Debug.Trace(
                                "SessionStateOnReleaseState",
                                "Release exclusive lock on session, SessionId=" + _rqId
                            );
                    ...
                    else
                    {
                        Debug.Trace(
                            "SessionStateOnReleaseState",
                            "Session is read-only, ignoring SessionId=" + _rqId
                        );
                    }
#endif
                    ...
                    Debug.Trace(
                        "SessionStateOnReleaseState",
                        "Returning from SessionStateModule::OnReleaseState"
                    );
                }
            ...
            try
            {
                if (!_releaseCalled)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Debug.Trace(
                                "SessionStateOnEndRequest",
                                "Resetting timeout for SessionId=" + id
                            );
