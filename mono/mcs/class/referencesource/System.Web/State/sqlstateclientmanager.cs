...
#pragma warning restore 0649
        ...
        void OneTimeInit()
        {
            ...
            {
                s_usePartition = true;
                s_partitionManager = new PartitionManager(
                    new CreatePartitionInfo(CreatePartitionInfo)
                );
            }
            ...
            catch (Exception e)
            {
                if (s_usePartition)
                {
                    HttpException outerException = new HttpException(
                        SR.GetString(
                            SR.Error_parsing_sql_partition_resolver_string,
                ...
                else
                {
                    throw new ConfigurationErrorsException(
                        ...
                        e,
                        s_configSqlConnectionFileName,
                        s_configSqlConnectionLineNumber
                    );
                }
            }
                ...
                if (!s_configAllowCustomSqlDatabase)
                {
                    if (s_usePartition)
                    {
                        throw new HttpException(
                            SR.GetString(
                                SR.No_database_allowed_in_sql_partition_resolver_string,
                                s_configPartitionResolverType,
                                dummyConnection.DataSource,
                                database
                            )
                        );
                    }
        ...
        public bool KnowForSureNotUsingIntegratedSecurity
        {
            get
            {
                if (_partitionInfo == null)
                {
                    Debug.Assert(
                        s_usePartition,
                        "_partitionInfo can be null only if we're using paritioning and we haven't called GetConnection yet."
                    );
        ...
        SqlStateConnection GetConnection(string id, ref bool usePooling)
        {
            ...
            if (_partitionInfo == null)
            {
                ...
                _partitionInfo = (SqlPartitionInfo)
                    s_partitionManager.GetPartition(_partitionResolver, id);
            }

            Debug.Trace(
                "SessionStatePooling",
                "Calling GetConnection under " + WindowsIdentity.GetCurrent().Name
            );
#if DBG
            Debug.Assert(
                _module._rqChangeImpersonationRefCount != 0,
        ...
        internal static void ThrowSqlConnectionException(SqlConnection conn, Exception e)
        {
            if (s_usePartition)
            {
                throw new HttpException(
                    SR.GetString(
                        SR.Cant_connect_sql_session_database_partition_resolver,
                        s_configPartitionResolverType,
                        conn.DataSource,
                        conn.Database
                    ),
            ...
            {
                throw new HttpException(SR.GetString(SR.Cant_connect_sql_session_database), e);
            }
        }
        ...
        )
        {
            ...
            Debug.Assert(
                id.Length <= SessionIDManager.SESSION_ID_LENGTH_LIMIT,
                "id.Length <= SessionIDManager.SESSION_ID_LENGTH_LIMIT"
            );
            Debug.Assert(context != null, "context != null");
            ...
            Debug.Assert(
                _partitionInfo.SupportFlags != SupportFlags.Uninitialized,
                "_partitionInfo.SupportFlags != SupportFlags.Uninitialized"
            );
            ...
            try
            {
                ...
                Debug.Assert(
                    !Convert.IsDBNull(cmd.Parameters[3].Value),
                    "!Convert.IsDBNull(cmd.Parameters[3].Value)"
                );
                Debug.Assert(
                    !Convert.IsDBNull(cmd.Parameters[4].Value),
                    "!Convert.IsDBNull(cmd.Parameters[4].Value)"
                );
                ...
                if (locked)
                {
                    ...
                    Debug.Assert(
                        ((int)cmd.Parameters[5].Value & (int)SessionStateActions.InitializeItem)
                            == 0,
                ...
                if (buf == null)
                {
                    /* Get short item */
                    Debug.Assert(
                        !Convert.IsDBNull(cmd.Parameters[1].Value),
                        "!Convert.IsDBNull(cmd.Parameters[1].Value)"
                    );
                ...
                {
                    item = SessionStateUtility.DeserializeStoreData(
                        context,
                        stream,
                        s_configCompressionEnabled
                    );
        ...
        )
        {
            ...
            try
            {
                ...
                try
                {
                    SessionStateUtility.SerializeStoreData(
                        item,
                        ITEM_SHORT_LENGTH,
                        out buf,
                        out length,
        ...
        public override void CreateUninitializedItem(HttpContext context, String id, int timeout)
        {
            ...
            try
            {
                ...
                // Store an empty data
                SessionStateUtility.SerializeStoreData(
                    CreateNewStoreData(context, timeout),
                    ITEM_SHORT_LENGTH,
                    out buf,
                    out length,
                    s_configCompressionEnabled
        ...
        static bool IsInsertPKException(SqlException ex, bool ignoreInsertPKException, string id)
        {
            ...
            )
            {
                Debug.Trace(
                    ...
                    "Insert failed because of primary key violation; just leave gracefully; id="
                        + id
                );
        ...
        static bool CanRetry(
            SqlException ex,
            SqlConnection conn,
            ref bool isFirstAttempt,
            ref DateTime endRetryTime
        )
        {
            ...
            if (isFirstAttempt)
            {
                ...
                if (0 == Interlocked.CompareExchange(ref s_isClearPoolInProgress, 1, 0))
                {
                    Debug.Trace(
                        ...
                        "CanRetry: Call ClearPool to destroy the corrupted connections in the pool"
                    );
        ...
        internal class SqlPartitionInfo : PartitionInfo
        {
                ...
                : base(rpool)
            {
                ...
                Debug.Trace(
                    "PartitionInfo",
                    "Created a new info, sqlConnectionString=" + sqlConnectionString
                );
            }
            ...
            void GetServerSupportOptions(SqlConnection sqlConnection)
            {
                ...
                if (!v2)
                {
                    if (s_usePartition)
                    {
                        throw new HttpException(
                            SR.GetString(
                                SR.Need_v2_SQL_Server_partition_resolver,
                                s_configPartitionResolverType,
                    ...
                    {
                        throw new HttpException(SR.GetString(SR.Need_v2_SQL_Server));
                    }
                }
            ...
            internal void InitSqlInfo(SqlConnection sqlConnection)
            {
                ...
                lock (_lock)
                {
                    ...
                    p = cmdTempGetAppId.Parameters.Add(
                        new SqlParameter("@appName", SqlDbType.VarChar, APPID_MAX)
                    );
        ...
        */
        class SqlStateConnection : IDisposable
        {
            ...
            {
                Debug.Trace(
                    "SessionStateConnectionIdentity",
                    "Connecting under " + WindowsIdentity.GetCurrent().Name
                );
                ...
                while (true)
                {
                    ...
                    catch (SqlException e)
                    {
                        ...
                        )
                        {
                            ...
                            HttpException outerException = new HttpException(
                                SR.GetString(SR.Login_failed_sql_session_database, user),
                                e
                            );

                            outerException.SetFormatter(
            ...
            internal SqlCommand TempGet
            {
                get
                {
                    if (_cmdTempGet == null)
                    {
                        ...
                        if ((_partitionInfo.SupportFlags & SupportFlags.GetLockAge) != 0)
                        {
                            _cmdTempGet.Parameters.Add(
                                new SqlParameter("@id", SqlDbType.NVarChar, ID_LENGTH)
                            );
                            p = _cmdTempGet.Parameters.Add(
                                new SqlParameter(
                                    "@itemShort",
                                    SqlDbType.VarBinary,
                                    ITEM_SHORT_LENGTH
                            ...
                            p.Direction = ParameterDirection.Output;
                            p = _cmdTempGet.Parameters.Add(
                                new SqlParameter("@lockAge", SqlDbType.Int)
                            );
                            p.Direction = ParameterDirection.Output;
                            p = _cmdTempGet.Parameters.Add(
                                new SqlParameter("@lockCookie", SqlDbType.Int)
                            );
                            p.Direction = ParameterDirection.Output;
                            p = _cmdTempGet.Parameters.Add(
                                new SqlParameter("@actionFlags", SqlDbType.Int)
                            );
                            p.Direction = ParameterDirection.Output;
                        }
                        else
            ...
            internal SqlCommand TempGetExclusive
            {
                get
                {
                    if (_cmdTempGetExclusive == null)
                    {
                        ...
                        _cmdTempGetExclusive = new SqlCommand(
                        ...
                        if ((_partitionInfo.SupportFlags & SupportFlags.GetLockAge) != 0)
                        {
                            _cmdTempGetExclusive.Parameters.Add(
                                new SqlParameter("@id", SqlDbType.NVarChar, ID_LENGTH)
                            );
                            p = _cmdTempGetExclusive.Parameters.Add(
                                new SqlParameter(
                                    "@itemShort",
                                    SqlDbType.VarBinary,
                                    ITEM_SHORT_LENGTH
            ...
            internal SqlCommand TempReleaseExclusive
            {
                get
                {
                    ...
                    {
                        /* ReleaseExlusive */
                        _cmdTempReleaseExclusive = new SqlCommand(
                        ...
                        _cmdTempReleaseExclusive.Parameters.Add(
                            new SqlParameter("@id", SqlDbType.NVarChar, ID_LENGTH)
                        );
                        _cmdTempReleaseExclusive.Parameters.Add(
                            new SqlParameter("@lockCookie", SqlDbType.Int)
            ...
            internal SqlCommand TempInsertLong
            {
                get
                {
                    ...
                    {
                        _cmdTempInsertLong = new SqlCommand(
                        ...
                        _cmdTempInsertLong.Parameters.Add(
                            new SqlParameter("@id", SqlDbType.NVarChar, ID_LENGTH)
                        );
                        _cmdTempInsertLong.Parameters.Add(
                            new SqlParameter("@itemLong", SqlDbType.Image, 8000)
                        );
                        _cmdTempInsertLong.Parameters.Add(
            ...
            internal SqlCommand TempInsertShort
            {
                get
                {
                    ...
                    {
                        _cmdTempInsertShort = new SqlCommand(
                        ...
                        _cmdTempInsertShort.Parameters.Add(
                            new SqlParameter("@id", SqlDbType.NVarChar, ID_LENGTH)
                        );
                        _cmdTempInsertShort.Parameters.Add(
                            new SqlParameter("@itemShort", SqlDbType.VarBinary, ITEM_SHORT_LENGTH)
                        );
                        _cmdTempInsertShort.Parameters.Add(
                            new SqlParameter("@timeout", SqlDbType.Int)
                        );
            ...
            internal SqlCommand TempUpdateLong
            {
                get
                {
                    ...
                    {
                        _cmdTempUpdateLong = new SqlCommand(
                        ...
                        _cmdTempUpdateLong.Parameters.Add(
                            new SqlParameter("@id", SqlDbType.NVarChar, ID_LENGTH)
                        );
                        _cmdTempUpdateLong.Parameters.Add(
                            new SqlParameter("@itemLong", SqlDbType.Image, 8000)
            ...
            internal SqlCommand TempUpdateShort
            {
                get
                {
                    ...
                    {
                        _cmdTempUpdateShort = new SqlCommand(
                        ...
                        _cmdTempUpdateShort.Parameters.Add(
                            new SqlParameter("@id", SqlDbType.NVarChar, ID_LENGTH)
                        );
                        _cmdTempUpdateShort.Parameters.Add(
                            new SqlParameter("@itemShort", SqlDbType.VarBinary, ITEM_SHORT_LENGTH)
            ...
            internal SqlCommand TempUpdateShortNullLong
            {
                get
                {
                    ...
                    {
                        _cmdTempUpdateShortNullLong = new SqlCommand(
                        ...
                        _cmdTempUpdateShortNullLong.Parameters.Add(
                            new SqlParameter("@id", SqlDbType.NVarChar, ID_LENGTH)
                        );
                        _cmdTempUpdateShortNullLong.Parameters.Add(
                            new SqlParameter("@itemShort", SqlDbType.VarBinary, ITEM_SHORT_LENGTH)
            ...
            internal SqlCommand TempUpdateLongNullShort
            {
                get
                {
                    ...
                    {
                        _cmdTempUpdateLongNullShort = new SqlCommand(
                        ...
                        _cmdTempUpdateLongNullShort.Parameters.Add(
                            new SqlParameter("@id", SqlDbType.NVarChar, ID_LENGTH)
                        );
                        _cmdTempUpdateLongNullShort.Parameters.Add(
                            new SqlParameter("@itemLong", SqlDbType.Image, 8000)
            ...
            internal SqlCommand TempInsertUninitializedItem
            {
                get
                {
                    ...
                    {
                        _cmdTempInsertUninitializedItem = new SqlCommand(
                        ...
                        _cmdTempInsertUninitializedItem.Parameters.Add(
                            new SqlParameter("@id", SqlDbType.NVarChar, ID_LENGTH)
                        );
                        _cmdTempInsertUninitializedItem.Parameters.Add(
                            new SqlParameter("@itemShort", SqlDbType.VarBinary, ITEM_SHORT_LENGTH)
            ...
            internal SqlCommand TempResetTimeout
            {
                get
                {
                    ...
                    {
                        /* ResetTimeout */
                        _cmdTempResetTimeout = new SqlCommand(
                        ...
                        _cmdTempResetTimeout.Parameters.Add(
                            new SqlParameter("@id", SqlDbType.NVarChar, ID_LENGTH)
                        );
                    }
