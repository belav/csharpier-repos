...
#pragma warning restore 0649
        ...
        void OneTimeInit()
        {
            ...
            s_configStateConnectionStringFileName = config.ElementInformation.Properties[
                "stateConnectionString"
            ].Source;
            s_configStateConnectionStringLineNumber = config.ElementInformation.Properties[
                "stateConnectionString"
            ].LineNumber;
            ...
            if (_partitionResolver == null)
            {
                ...
                SessionStateModule.ReadConnectionString(
                    config,
                    ref stateConnectionString,
                    "stateConnectionString"
                );

                s_singlePartitionInfo = (StateServerPartitionInfo)CreatePartitionInfo(
                    stateConnectionString
                );
            }
            else
            ...
            string idHash = Convert.ToBase64String(
                CryptoUtil.ComputeSHA256Hash(Encoding.UTF8.GetBytes(appId))
            );
        ...
        internal IPartitionInfo CreatePartitionInfo(string stateConnectionString)
        {
            ...
            try
            {
                ParseStateConnectionString(
                    stateConnectionString,
                    out server,
                    out serverIsIpv6NumericAddress,
                    out port
                );
            ...
            catch
            {
                if (s_usePartition)
                {
                    throw new HttpException(
                        SR.GetString(
                            SR.Error_parsing_state_server_partition_resolver_string,
                ...
                else
                {
                    throw new ConfigurationErrorsException(
                        SR.GetString(
                            SR.Invalid_value_for_sessionstate_stateConnectionString,
                            s_configStateConnectionString
                        ),
                        s_configStateConnectionStringFileName,
        ...
        private static Regex _ipv6ConnectionStringFormat = new Regex(
            @"^\[(?<ipv6Address>.*)\]:(?<port>\d*)$"
        );
        ...
        [SuppressMessage(
            ...
            Justification = @"The exception is never bubbled up to the user."
        )]
        internal static void ParseStateConnectionString(
            string stateConnectionString,
            out string server,
            out bool serverIsIPv6NumericAddress,
            out int port
        )
        {
            /*
            ...
            */
            ...
            if (ipv6RegexMatch != null && ipv6RegexMatch.Success)
            {
                ...
                port = UInt16.Parse(
                    ipv6RegexMatch.Groups["port"].Value,
        ...
        internal static HttpException CreateConnectionException(string server, int port, int hr)
        {
            if (s_usePartition)
            {
                return new HttpException(
                    SR.GetString(
                        SR.Cant_make_session_request_partition_resolver,
                        s_configPartitionResolverType,
                        server,
            ...
            {
                return new HttpException(SR.GetString(SR.Cant_make_session_request), hr);
            }
        }
        ...
        )
        {
            ...
            try
            {
                ...
                Debug.Trace(
                    "OutOfProcSessionStateStoreMakeRequest",
                    "Calling MakeRequest, "
                        + "socket="
                        + (IntPtr)socketHandle.Handle
                        + "verb="
                        + verb
                        + " id="
                        + id
                ...
                hr = UnsafeNativeMethods.SessionNDMakeRequest(
                    socketHandle,
                    _partitionInfo.Server,
                    _partitionInfo.Port,
                    _partitionInfo.ServerIsIPv6NumericAddress /* forceIPv6 */
                    ,
                    networkTimeout,
                    verb,
                    uri,
                ...
                Debug.Trace(
                    "OutOfProcSessionStateStoreMakeRequest",
                    "MakeRequest returned: "
                        + "hr="
                        + hr
                        + " socket="
                        + (IntPtr)results.socket
                        + " httpstatus="
                        + results.httpStatus
                        + " timeout="
                        + results.timeout
                        ...
                        + " uri="
                        + (IntPtr)results.content
                        + " lockCookie="
                        + results.lockCookie
                        + " lockDate="
                        + string.Format("{0:x}", results.lockDate)
                        + " lockAge="
                        + results.lockAge
                        + " stateServerMajVer="
                        + results.stateServerMajVer
                        + " actionFlags="
                        + results.actionFlags
                );
            ...
            if (hr != 0)
            {
                HttpException e = CreateConnectionException(
                    _partitionInfo.Server,
                    _partitionInfo.Port,
                    hr
                );
                ...
                WebBaseEvent.RaiseSystemEvent(
                    ...
                    WebEventCodes.StateServerConnectionError,
                    e
                );
            ...
            if (results.httpStatus == 400)
            {
                if (s_usePartition)
                {
                    throw new HttpException(
                        SR.GetString(
                            SR.Bad_state_server_request_partition_resolver,
                            s_configPartitionResolverType,
                            _partitionInfo.Server,
                            _partitionInfo.Port.ToString(CultureInfo.InvariantCulture)
                        )
                    );
                }
                else
                {
                    throw new HttpException(SR.GetString(SR.Bad_state_server_request));
                }
            }
            ...
            if (checkVersion)
            {
                ...
                if (_partitionInfo.StateServerVersion < WHIDBEY_MAJOR_VERSION)
                {
                    ...
                    if (s_usePartition)
                    {
                        throw new HttpException(
                            SR.GetString(
                                SR.Need_v2_State_Server_partition_resolver,
                                s_configPartitionResolverType,
                                _partitionInfo.Server,
                                _partitionInfo.Port.ToString(CultureInfo.InvariantCulture)
                            )
                    ...
                    {
                        throw new HttpException(SR.GetString(SR.Need_v2_State_Server));
                    }
                }
            }
        }
        ...
        internal SessionStateStoreData DoGet(
            HttpContext context,
            String id,
            UnsafeNativeMethods.StateProtocolExclusive exclusiveAccess,
            out bool locked,
        ...
        )
        {
            ...
            {
                MakeRequest(
                    UnsafeNativeMethods.StateProtocolVerb.GET,
                    id,
                    exclusiveAccess,
                    0,
                    0,
                ...
                switch (results.httpStatus)
                {
                    ...
                    case 423:
                        ...
                        Debug.Assert(
                            (results.actionFlags & (int)SessionStateActions.InitializeItem) == 0,
        ...
        )
        {
            ...
            Debug.Trace(
                ...
                "Calling Set, id=" + id + " sessionItems=" + item.Items + " timeout=" + item.Timeout
            );
            ...
            MakeRequest(
                UnsafeNativeMethods.StateProtocolVerb.PUT,
                id,
                UnsafeNativeMethods.StateProtocolExclusive.NONE,
                0,
                item.Timeout,
        ...
        )
        {
            ...
            MakeRequest(
                UnsafeNativeMethods.StateProtocolVerb.DELETE,
                id,
                UnsafeNativeMethods.StateProtocolExclusive.NONE,
                0,
                0,
        ...
        public override void ResetItemTimeout(HttpContext context, String id)
        {
            ...
            MakeRequest(
                UnsafeNativeMethods.StateProtocolVerb.HEAD,
                id,
                UnsafeNativeMethods.StateProtocolExclusive.NONE,
                0,
                0,
        ...
        public override void CreateUninitializedItem(HttpContext context, String id, int timeout)
        {
            ...
            Debug.Trace(
                "OutOfProcSessionStateStore",
                "Calling CreateUninitializedItem, id=" + id + " timeout=" + timeout
            );
        ...
        class StateServerPartitionInfo : PartitionInfo
        {
                ...
                : base(rpool)
            {
                ...
                Debug.Trace(
                    "PartitionInfo",
                    "Created a new info, server=" + server + ", port=" + port
                );
            }
