...
namespace System.Web.Hosting
{
    ...
    internal class MapPathBasedVirtualPathProvider : VirtualPathProvider
    {
        ...
        private bool CacheLookupOrInsert(string virtualPath, bool isFile)
        {
            ...
            string existingDir =
                (exists) ? physicalPath : FileUtil.GetFirstExistingDirectory(AppRoot, physicalPath);
            if (existingDir != null)
            {
                ...
                HttpRuntime.Cache.InternalCache.Insert(
                    cacheKey,
                    exists,
                    new CacheInsertOptions() { Dependencies = dep, SlidingExpiration = slidingExp }
                );
            }
        ...
        public override IEnumerable Directories
        {
            get
            {
                return new MapPathBasedVirtualPathCollection(
                    System.Web.VirtualPath.CreateNonRelative(VirtualPath),
                    RequestedEntryType.Directories
                );
            }
        }
    ...
    internal class MapPathBasedVirtualPathCollection : MarshalByRefObject, IEnumerable
    {
        ...
        internal MapPathBasedVirtualPathCollection(
    ...
    internal class MapPathBasedVirtualPathEnumerator : MarshalByRefObject, IEnumerator, IDisposable
    {
        ...
        internal MapPathBasedVirtualPathEnumerator(
            VirtualPath virtualPath,
            RequestedEntryType requestedEntryType
        )
        {
            if (virtualPath.IsRelative)
            {
                throw new ArgumentException(
                    SR.GetString(SR.Invalid_app_VirtualPath),
                    "virtualPath"
                );
            }
            ...
            else
            {
                ...
                if (_requestedEntryType != RequestedEntryType.Files)
                {
                    ...
                    string[] virtualSubdirsInApp = serverConfig.GetVirtualSubdirs(
                        _virtualPath,
                        true
                    );
                    if (virtualSubdirsInApp != null)
                    {
                        ...
                        foreach (string subdir in virtualSubdirsInApp)
                        {
                            ...
                            {
                                _virtualPaths[subdir] = new MapPathBasedVirtualDirectory(
                                    subpath.VirtualPathString
                                );
                            }
                        }
            ...
            if (!more)
            {
                ...
                for (; ; )
                {
                    ...
                    if (fileData.IsDirectory)
                    {
                        ...
                        if (
                            _serverConfig2 != null
                            && !_serverConfig2.IsWithinApp(
                                UrlPath.SimpleCombine(_virtualPath.VirtualPathString, name)
                            )
                        )
                        {
                            continue;
                ...
                if (_useFileEnumerator)
                {
                    ...
                    else
                    {
                        ...
                        return new MapPathBasedVirtualFile(
                            childVirtualPath.VirtualPathString,
                            fileData.FullName,
                            ffd
                        );
                    }
