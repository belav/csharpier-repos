...
namespace System.Web.Hosting
{
    ...
    internal class RecyclableCharBuffer
    {
        ...
        static RecyclableCharBuffer()
        {
            ...
            s_ByteBufferAllocator = new UbyteBufferAllocator(
                Encoding.UTF8.GetMaxByteCount(BUFFER_SIZE),
                MAX_FREE_BUFFERS
            );
        }
        ...
        {
            return (_charBuffer != null && _freePos > 0)
                ? new String(_charBuffer, 0, _freePos)
                : null;
        }
    }
    ...
    internal class RecyclableByteBuffer
    {
        ...
        internal String[] GetDecodedTabSeparatedStrings(
            Encoding encoding,
            int numStrings,
            int numSkipStrings
        )
    ...
    internal class MemoryBytes
    {
        ...
        private void CloseHandle()
        {
            if (
                _fileHandle != IntPtr.Zero
                && _fileHandle != UnsafeNativeMethods.INVALID_HANDLE_VALUE
            )
            {
    ...
    internal delegate void AsyncCompletionCallback(
        int bytesCompleted,
        int hresult,
    ...
    internal abstract class ISAPIWorkerRequest : HttpWorkerRequest
    {
        ...
        private void FlushCachedResponse(bool isFinal)
        {
            ...
            try
            {
                ...
                int finalStatus = isFinal
                    ? (
                        (_cachedResponseKeepConnected != 0)
        ...
        public override byte[] GetPreloadedEntityBody()
        {
            if (!_preloadedContentRead)
            {
                if (_contentAvailLength > 0)
                {
                    ...
                    int r = GetPreloadedPostedContentCore(
                        _preloadedContent,
                        0,
                        _contentAvailLength
                    );
        ...
        internal virtual MemoryBytes PackageFile(
            String filename,
            long offset64,
            long length64,
        ...
        internal abstract int GetPreloadedPostedContentCore(
            byte[] bytes,
            int offset,
            int numBytesToRead
        );
        internal abstract int GetAdditionalPostedContentCore(
            byte[] bytes,
            int offset,
            int bufferSize
        );
        ...
        // ISAPIWorkerRequest
        internal abstract void FlushCore(
            byte[] status,
            byte[] header,
            int keepConnected,
            int totalBodySize,
    ...
    internal class ISAPIWorkerRequestInProc : ISAPIWorkerRequest
    {
            ...
            : base(ecb)
        {
            if (
                ecb == IntPtr.Zero
                || UnsafeNativeMethods.EcbGetTraceContextId(ecb, out _traceId) != 1
            )
            {
                _traceId = Guid.Empty;
            }
        ...
        internal override int GetPreloadedPostedContentCore(
            byte[] bytes,
            int offset,
            int numBytesToRead
        )
        {
            ...
            int rc = UnsafeNativeMethods.EcbGetPreloadedPostedContent(
                _ecb,
                bytes,
                offset,
                numBytesToRead
        ...
        internal override int GetAdditionalPostedContentCore(
            byte[] bytes,
            int offset,
            int bufferSize
        )
        {
            ...
            try
            {
                ...
                rc = UnsafeNativeMethods.EcbGetAdditionalPostedContent(
                    _ecb,
                    bytes,
                    offset,
                    bufferSize
        ...
        internal override int GetClientCertificateCore(byte[] buffer, int[] pInts, long[] pDates)
        {
            ...
            return UnsafeNativeMethods.EcbGetClientCertificate(
                _ecb,
                buffer,
                buffer.Length,
                pInts,
        ...
        internal override void FlushCore(
            byte[] status,
            byte[] header,
            int keepConnected,
            int totalBodySize,
        ...
        protected virtual String GetServerVariableCore(String name)
        {
            ...
            int retVal = UnsafeNativeMethods.EcbGetServerVariable(
                _ecb,
                name,
                buf.Buffer,
                buf.Buffer.Length
            ...
            while (retVal < 0)
            {
                ...
                retVal = UnsafeNativeMethods.EcbGetServerVariable(
                    _ecb,
                    name,
                    buf.Buffer,
                    buf.Buffer.Length
        ...
        protected virtual void GetAdditionalServerVariables()
        {
            ...
            for (int i = 0; i < _additionalServerVars.Length; i++)
            {
                ...
                int retVal = UnsafeNativeMethods.EcbGetServerVariableByIndex(
                    _ecb,
                    nameIndex,
                    buf.Buffer,
                    buf.Buffer.Length
                ...
                {
                    buf.Resize(-retVal); // buffer not big enough
                    retVal = UnsafeNativeMethods.EcbGetServerVariableByIndex(
                        _ecb,
                        nameIndex,
                        buf.Buffer,
                        buf.Buffer.Length
        ...
        )
        {
            ...
            return UnsafeNativeMethods.EcbCallISAPI(
                _ecb,
                iFunction,
                bufIn,
                bufIn.Length,
        ...
        internal ChannelBinding HttpChannelBindingToken
        {
            get
            {
                if (_channelBindingToken == null)
                {
                    ...
                    hr = UnsafeNativeMethods.EcbGetChannelBindingToken(
                        _ecb,
                        out token,
                        out tokenSize
                    );
        ...
        internal override void RaiseTraceEvent(WebBaseEvent webEvent)
        {
            if (IntPtr.Zero != _ecb)
            {
                ...
                )
                {
                    ...
                    UnsafeNativeMethods.EcbEmitWebEventTrace(
                        _ecb,
                        webEventType,
                        fieldCount,
                        fieldNames,
    ...
    internal class ISAPIWorkerRequestInProcForIIS6 : ISAPIWorkerRequestInProc
    {
        ...
        private void GetBasicServerVariables()
        {
            ...
            try
            {
                ...
                int r = 0;
                int hresult = UnsafeNativeMethods.EcbGetUnicodeServerVariables(
                    _ecb,
                    buffer.PinnedAddress,
                    buffer.Length,
                    serverVarLengths,
                    serverVarLengths.Length,
                    0,
                    ref r
                );
                if (r > buffer.Length)
                {
                    buffer.Resize(r);
                    hresult = UnsafeNativeMethods.EcbGetUnicodeServerVariables(
                        _ecb,
                        buffer.PinnedAddress,
                        buffer.Length,
                        serverVarLengths,
        ...
        private String GetUnicodeServerVariable(String name, ServerVarCharBuffer buffer)
        {
            if (_ecb == IntPtr.Zero)
                return null;
            int r = UnsafeNativeMethods.EcbGetUnicodeServerVariable(
                _ecb,
                name,
                buffer.PinnedAddress,
                buffer.Length
            ...
            {
                buffer.Resize(-r);
                r = UnsafeNativeMethods.EcbGetUnicodeServerVariable(
                    _ecb,
                    name,
                    buffer.PinnedAddress,
                    buffer.Length
        ...
        private String GetUnicodeServerVariable(int nameIndex, ServerVarCharBuffer buffer)
        {
            if (_ecb == IntPtr.Zero)
                return null;
            int r = UnsafeNativeMethods.EcbGetUnicodeServerVariableByIndex(
                _ecb,
                nameIndex,
                buffer.PinnedAddress,
                buffer.Length
            ...
            {
                buffer.Resize(-r);
                r = UnsafeNativeMethods.EcbGetUnicodeServerVariableByIndex(
                    _ecb,
                    nameIndex,
                    buffer.PinnedAddress,
                    buffer.Length
        ...
        internal override MemoryBytes PackageFile(
            string filename,
            long offset,
            long size,
            bool isImpersonating
        ...
        )
        {
            ...
            if (
                doneWithSession != 0
                && !HttpRuntime.ShutdownInProgress
                && (_ignoreMinAsyncSize || (totalBodySize >= MIN_ASYNC_SIZE))
            )
            {
                if (_requiresAsyncFlushCallback)
                {
                    _asyncFlushCompletionCallback = new ISAPIAsyncCompletionCallback(
                        OnAsyncFlushCompletion
                    );
            ...
            int flags = _trySkipIisCustomErrors
                ? finalStatus | TRY_SKIP_IIS_CUSTOM_ERRORS
                : finalStatus;
            ...
            else if (rc != 0 && !async && doneWithSession == 0 && !_serverSupportFunctionError)
            {
                ...
                throw new HttpException(
                    SR.GetString(message, rc.ToString("X8", CultureInfo.InvariantCulture)),
                    rc
                );
            }
        }
        ...
        )
        {
            ...
            {
                ar.MarkCallToBeginMethodStarted();
                hresult = UnsafeNativeMethods.EcbReadClientAsync(
                    _ecb,
                    count,
        ...
        )
        {
            if (
                _ecb == IntPtr.Zero
                || // after done with session
                _asyncResultOfExecuteUrl != null
                || // another ExecuteUrl in progress
                (sendHeaders && HeadersSent())
            ) // asked to send headers, but already sent them
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.Cannot_execute_url_in_this_context)
                );
            }

            if (entity != null && entity.Length > 0)
            {
                int ret = UnsafeNativeMethods.EcbGetExecUrlEntityInfo(
                    entity.Length,
                    entity,
                    out _entity
                );
            ...
            Debug.Trace(
                "ExecuteUrl",
                "ISAPIWorkerRequestInProcForIIS6.BeginExecuteUrl: url=\"" + url + "\"."
            );

            HttpAsyncResult ar = new HttpAsyncResult(cb, state);
            _asyncResultOfExecuteUrl = ar;

            _executeUrlCompletionCallback = new ISAPIAsyncCompletionCallback(
            ...
            if (rc != 1)
            {
                ...
                Debug.Trace(
                    "ExecuteUrl",
                    "ISAPIWorkerRequestInProcForIIS6.BeginExecuteUrl: failed!"
                );
    ...
    internal class ISAPIWorkerRequestOutOfProc : ISAPIWorkerRequest
    {
        ...
        private static String[] _serverVarNames = new String[_numServerVars]
        {
        ...
        private void GetAllServerVars()
        {
            ...
            int r = UnsafeNativeMethods.PMGetAllServerVariables(
                _ecb,
                buf.Buffer,
                buf.Buffer.Length
            );
            ...
            {
                buf.Resize(-r); // buffer not big enough
                r = UnsafeNativeMethods.PMGetAllServerVariables(
                    _ecb,
                    buf.Buffer,
                    buf.Buffer.Length
                );
            ...
            String[] ss = buf.GetDecodedTabSeparatedStrings(
                Encoding.Default,
                _numServerVars - 1,
                1
            );
            ...
            for (int i = 1; i < _numServerVars; i++)
            { // starting with 1 to skip APPL_MD_PATH
        ...
        )
        {
            ...
            byte[] buffer = new byte[
                4 + offsetBytes.Length + lengthBytes.Length + nameBytes.Length + 2
            ];
            ...
            Buffer.BlockCopy(
                ...
                0,
                buffer,
                4 + offsetBytes.Length + lengthBytes.Length,
                nameBytes.Length
            );
        ...
        internal override int GetClientCertificateCore(byte[] buffer, int[] pInts, long[] pDates)
        {
            ...
            return UnsafeNativeMethods.PMGetClientCertificate(
                _ecb,
                buffer,
                buffer.Length,
                pInts,
        ...
        )
        {
            ...
            return UnsafeNativeMethods.PMCallISAPI(
                _ecb,
                iFunction,
                bufIn,
                bufIn.Length,
