    ...
    internal class CacheManager : IDisposable
    {
        ...
        internal CacheManager(ApplicationManager appManager, long privateBytesLimit)
        {
            ...
            SafeNativeMethods.OutputDebugString(
                String.Format(
                    "Creating CacheManager with PrivateBytesLimit = {0:N}\n",
                    privateBytesLimit
                )
            );
#endif
            ...
            _timer = new Timer(
                new TimerCallback(this.PBytesMonitorThread),
                null,
        ...
        void AdjustMaxDeltaAndPressureMarks(long delta)
        {
            ...
            if (_maxDelta != newMaxDelta)
            {
                ...
                _highPressureMark = Math.Max(
                    _limit * 9 / 10,
                    _limit - (_maxDelta * 2 * HIGH_FREQ_INTERVAL_S)
                );
                _lowPressureMark = Math.Max(
                    _limit * 6 / 10,
                    _limit - (_maxDelta * 2 * LOW_FREQ_INTERVAL_S)
                );
                _mediumPressureMark = Math.Max(
                    (_highPressureMark + _lowPressureMark) / 2,
                    _limit - (_maxDelta * 2 * MEDIUM_FREQ_INTERVAL_S)
                );
                ...
                SafeNativeMethods.OutputDebugString(
                    String.Format(
                        "CacheManager.AdjustMaxDeltaAndPressureMarks:  _highPressureMark={0:N}, _mediumPressureMark={1:N}, _lowPressureMark={2:N}, _maxDelta={3:N}\n",
                        _highPressureMark,
                        _mediumPressureMark,
                        _lowPressureMark,
                        _maxDelta
                ...
                Debug.Trace(
                    "CacheMemory",
                    "AdjustMaxDeltaAndPressureMarks "
                        + "delta="
                        + delta
                        + ", _maxDelta="
                        + _maxDelta
        ...
        [SuppressMessage(
            ...
            Justification = "Need to call GC.Collect."
        )]
        private void CollectInfrequently(long privateBytes)
        {
            ...
            if (infrequent || _lastTrimPercent < 50)
            {
                ...
                _inducedGCMinInterval = Math.Max(
                    _inducedGCDurationTicks * 1000 / 33,
                ...
                Debug.Trace(
                    ...
                    "GC.COLLECT STATS "
                        ...
                        + ")"
                        + ", trimDurationSeconds="
                        + (_trimDurationTicks / TimeSpan.TicksPerSecond)
                        + ", trimmedOrExpired="
                        + trimmedOrExpired
                        + ", #secondsSinceInducedGC="
                        + (timeSinceInducedGC / TimeSpan.TicksPerSecond)
                        + ", InducedGCCount="
                        + _inducedGCCount
                        + ", gcDurationSeconds="
                        + (_inducedGCDurationTicks / TimeSpan.TicksPerSecond)
                        + ", PrePrivateBytes="
                        + privateBytes
                        + ", PostPrivateBytes="
                        + _inducedGCPostPrivateBytes
                        + ", PrivateBytesChange="
                        + _inducedGCPrivateBytesChange
                        + ", gcMinIntervalSeconds="
                        + (_inducedGCMinInterval / TimeSpan.TicksPerSecond)
                );
#endif
...
#if PERF
                SafeNativeMethods.OutputDebugString(
                    "CacheManager.GetUpdatedTotalCacheSize:"
                        + " _totalCacheSize= "
                        + totalSize
                        + ", sizeUpdate="
                        + sizeUpdate
        ...
        private long NextSample()
        {
            ...
            if (_useGetProcessMemoryInfo)
            {
                ...
                UnsafeNativeMethods.GetPrivateBytesIIS6(
                    out privatePageCount,
                    true /*nocache*/
                );
            ...
            else
            {
                ...
                UnsafeNativeMethods.GetProcessMemoryInformation(
                    _pid,
                    out privatePageCount,
                    out dummy,
                    true /*nocache*/
            ...
            SafeNativeMethods.OutputDebugString(
                String.Format(
                    "CacheManager.NextSample:  privateBytes={0:N}, _highPresureMark={1:N}\n",
                    privateBytes,
                    _highPressureMark
                )
            );
