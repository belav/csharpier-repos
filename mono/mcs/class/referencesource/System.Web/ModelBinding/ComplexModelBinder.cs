    ...
    public sealed class ComplexModelBinder : IModelBinder
    {
        ...
        )
        {
            ...
            foreach (ModelMetadata propertyMetadata in complexModel.PropertyMetadata)
            {
                ...
                IModelBinder propertyBinder = bindingContext.ModelBinderProviders.GetBinder(
                ...
                {
                    if (
                        propertyBinder.BindModel(
                            modelBindingExecutionContext,
                            propertyBindingContext
                        )
                    )
                    {
                        complexModel.Results[propertyMetadata] = new ComplexModelResult(
                            propertyBindingContext.Model,
