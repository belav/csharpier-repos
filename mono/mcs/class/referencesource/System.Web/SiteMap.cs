    ...
    public static class SiteMap
    {
            ...
            get
            {
                ...
                if (rootNode == null)
                {
                    ...
                    throw new InvalidOperationException(
                        SR.GetString(SR.SiteMapProvider_Invalid_RootNode, name)
                    );
                }
        ...
        private static void Initialize()
        {
            ...
            lock (_lockObject)
            {
                ...
                if (!config.Enabled)
                    throw new InvalidOperationException(
                        SR.GetString(SR.SiteMap_feature_disabled, SiteMap.SectionName)
                    );
        ...
        public override void Add(ProviderBase provider)
        {
            ...
            if (!(provider is SiteMapProvider))
                throw new ArgumentException(
                    SR.GetString(
                        SR.Provider_must_implement_the_interface,
        ...
        public void AddArray(SiteMapProvider[] providerArray)
        {
            ...
            foreach (SiteMapProvider provider in providerArray)
            {
                if (this[provider.Name] != null)
                    throw new ArgumentException(
                        SR.GetString(
                            SR.SiteMapProvider_Multiple_Providers_With_Identical_Name,
                            provider.Name
                        )
                    );
