...
namespace System.Web
{
    ...
    [SuppressMessage(
        ...
        Justification = "The service provider implementation is only for specific types which are not com interop types."
    )]
    public sealed class HttpContext : IServiceProvider, IPrincipalContainer
    {
                ...
                switch (GetWebSocketInitStatus())
                {
                    ...
                    case WebSocketInitStatus.CannotCallFromBeginRequest:
                        throw new InvalidOperationException(
                            SR.GetString(SR.WebSockets_CannotBeCalledDuringBeginRequest)
                        );
        ...
        public IList<string> WebSocketRequestedProtocols
        {
            get
            {
                if (IsWebSocketRequest)
                {
                    ...
                    {
                        string rawHeaderValue = _wr.GetUnknownRequestHeader(
                            "Sec-WebSocket-Protocol"
                        );
                        IList<string> requestedProtocols = SubProtocolUtil.ParseHeader(
                            rawHeaderValue
                        ); // checks for invalid values
                        _webSocketRequestedProtocols = new ReadOnlyCollection<string>(
                            requestedProtocols ?? new string[0]
                        );
                    }
        ...
        [SuppressMessage(
            ...
            Justification = "This is a safe critical method."
        )]
        public void AcceptWebSocketRequest(
        ...
        )
        {
            ...
            if (IsWebSocketRequestUpgrading)
            {
                ...
                throw new InvalidOperationException(
                    SR.GetString(SR.WebSockets_AcceptWebSocketRequestCanOnlyBeCalledOnce)
                );
            }
            ...
            switch (GetWebSocketInitStatus())
            {
                ...
                case WebSocketInitStatus.NativeModuleNotEnabled:
                    throw new PlatformNotSupportedException(
                        SR.GetString(SR.WebSockets_WebSocketModuleNotEnabled)
                    );

                case WebSocketInitStatus.NotAWebSocketRequest:
                    throw new HttpException(
                        (int)HttpStatusCode.BadRequest,
                        SR.GetString(SR.WebSockets_NotAWebSocketRequest)
                    );

                case WebSocketInitStatus.CurrentRequestIsChildRequest:
                    throw new InvalidOperationException(
                        SR.GetString(SR.WebSockets_CannotBeCalledDuringChildExecute)
                    );
            ...
            if (CurrentNotification > RequestNotification.ExecuteRequestHandler)
            {
                ...
                throw new InvalidOperationException(
                    SR.GetString(SR.WebSockets_CannotBeCalledAfterHandlerExecute)
                );
            }
            ...
            RootedObjects.WebSocketPipeline = new WebSocketPipeline(
                RootedObjects,
                this,
                userFunc,
                subprotocol
        ...
        internal void TransitionToWebSocketState(WebSocketTransitionState newState)
        {
            ...
            Debug.Assert(
                WebSocketTransitionState == expectedOldState,
                String.Format(
                    CultureInfo.InvariantCulture,
                    "Expected WebSocketTransitionState to be '{0}', but it was '{1}'.",
                    expectedOldState,
                    WebSocketTransitionState
                )
            );
        ...
        internal void EnsureHasNotTransitionedToWebSocket()
        {
            if (HasWebSocketRequestTransitionCompleted)
            {
                throw new NotSupportedException(
                    SR.GetString(SR.WebSockets_MethodNotAvailableDuringWebSocketProcessing)
                );
            }
        }
        ...
        private string GetEurl()
        {
            ...
            if (
                !(_wr is ISAPIWorkerRequestInProcForIIS6)
            ...
            if (eurl == null && !s_eurlSet)
            {
                try
                {
                    ...
                    {
                        eurl = StringUtil.StringFromWCharPtr(
                            pBuffer,
                            UnsafeNativeMethods.lstrlenW(pBuffer)
                        );
                    }
                }
        ...
        {
            get { return (CallContext.GetData("__ContextAssert") != null); }
        }
#endif
        ...
        [SuppressMessage(
            ...
            Justification = @"The normal event pattern doesn't work between HttpContext and HttpContextBase since the signatures differ."
        )]
        ...
        {
            get { return SyncContext.AllowAsyncDuringSyncStages; }
            set { SyncContext.AllowAsyncDuringSyncStages = value; }
        }
        ...
        public HttpApplication ApplicationInstance
        {
            ...
            set
            {
                ...
                {
                    throw new InvalidOperationException(
                        SR.GetString(SR.Application_instance_cannot_be_changed)
                    );
                }
                else
                {
                    ...
                    if (_isIntegratedPipeline)
                    {
                        ...
                        IAllocatorProvider allocator =
                            _appInstance != null ? _appInstance.AllocatorProvider : null;
        ...
        public void RemapHandler(IHttpHandler handler)
        {
            ...
            if (wr != null)
            {
                ...
                )
                {
                    throw new InvalidOperationException(
                        SR.GetString(
                            SR.Invoke_before_pipeline_event,
        ...
        internal void EnsureSessionStateIfNecessary()
        {
            ...
            { // Ensure the session Id is valid - it will force to create new if didn't exist
        ...
        internal void ReportRuntimeErrorIfExists(ref RequestNotificationStatus status)
        {
            ...
            if (_notificationContext != null && CurrentModuleIndex == -1)
            {
                try
                {
                    IIS7WorkerRequest wr = _wr as IIS7WorkerRequest;
                    if (
                        Request.QueryString["aspxerrorpath"] != null
                        && wr != null
            ...
            if (HttpRuntime.AppOfflineMessage != null)
            {
                try
                {
                    ...
                    HttpRuntime.ReportAppOfflineErrorMessage(
                        Response,
                        HttpRuntime.AppOfflineMessage
                    );
                }
                catch { }
            }
            ...
            get
            {
                if (_Profile == null && _ProfileDelayLoad)
                    _Profile = ProfileBase.Create(
                        Request.IsAuthenticated ? User.Identity.Name : Request.AnonymousID,
        ...
        public bool SkipAuthorization
        {
            ...
            [SecurityPermission(SecurityAction.Demand, ControlPrincipal = true)]
            set { SetSkipAuthorizationNoDemand(value, false); }
        }

        internal void SetSkipAuthorizationNoDemand(bool value, bool managedOnly)
        {
            if (HttpRuntime.UseIntegratedPipeline && !managedOnly && value != _skipAuthorization)
            {
        ...
        {
            get { return CustomErrorsSection.GetSettings(this).CustomErrorsEnabled(_request); }
        }
        ...
        {
            RewritePath(
                VirtualPath.CreateAllowNull(filePath),
                VirtualPath.CreateAllowNull(pathInfo),
                queryString,
        ...
        internal TimeSpan Timeout
        {
            ...
            set { Interlocked.Exchange(ref _timeoutTicks, value.Ticks); }
        }
        ...
        internal CancellationToken TimedOutToken
        {
            get
            {
                ...
                CancellationTokenHelper helper = LazyInitializer.EnsureInitialized(
                    ref _timeoutCancellationTokenHelper,
        ...
        private void DisposeTimedOutToken()
        {
            ...
            CancellationTokenHelper helper = LazyInitializer.EnsureInitialized(
                ref _timeoutCancellationTokenHelper,
                () => CancellationTokenHelper.StaticDisposed
            );
            helper.Dispose();
        }
        ...
        */
        ...
        internal bool HasTimeoutExpired
        {
            get
            {
                ...
                long expirationUtcTicks =
                    Volatile.Read(ref _timeoutStartTimeUtcTicks) + Timeout.Ticks; // don't care about overflow
        ...
        internal int CallISAPI(
            UnsafeNativeMethods.CallISAPIFunc iFunction,
            byte[] bufIn,
            byte[] bufOut
        )
        {
        ...
        {
            get { return _sqlDependencyCookie; }
            set
            {
        ...
        internal bool IsChangeInServerVars
        {
            ...
            {
                return (
                        _notificationContext.CurrentNotificationFlags
        ...
        internal bool IsChangeInRequestHeaders
        {
            get
            {
                return (
                        _notificationContext.CurrentNotificationFlags
                        & FLAG_CHANGE_IN_REQUEST_HEADERS
                    ) == FLAG_CHANGE_IN_REQUEST_HEADERS;
            }
        }
        ...
        internal bool IsChangeInResponseHeaders
        {
            get
            {
                return (
                        _notificationContext.CurrentNotificationFlags
                        & FLAG_CHANGE_IN_RESPONSE_HEADERS
                    ) == FLAG_CHANGE_IN_RESPONSE_HEADERS;
            }
        }
        ...
        internal bool IsChangeInResponseStatus
        {
            get
            {
                return (
                        _notificationContext.CurrentNotificationFlags
                        & FLAG_CHANGE_IN_RESPONSE_STATUS
                    ) == FLAG_CHANGE_IN_RESPONSE_STATUS;
            }
        }
        ...
        internal bool IsChangeInUserPrincipal
        {
            get
            {
                return (_notificationContext.CurrentNotificationFlags & FLAG_CHANGE_IN_USER_OBJECT)
        ...
        internal bool IsSendResponseHeaders
        {
            get
            {
                return (_notificationContext.CurrentNotificationFlags & FLAG_SEND_RESPONSE_HEADERS)
        ...
        internal bool AreResponseHeadersSent
        {
            get
            {
                return (_notificationContext.CurrentNotificationFlags & FLAG_RESPONSE_HEADERS_SENT)
                    == FLAG_RESPONSE_HEADERS_SENT;
            }
        }
        ...
        internal int CurrentNotificationFlags
        {
            ...
            set { _notificationContext.CurrentNotificationFlags = value; }
        }
        ...
        {
            get { return _notificationContext.CurrentModuleIndex; }
            set { _notificationContext.CurrentModuleIndex = value; }
        }
        ...
        {
            get { return _notificationContext.CurrentModuleEventIndex; }
            set { _notificationContext.CurrentModuleEventIndex = value; }
        }
        ...
        internal IntPtr ImpersonationToken
        {
            get
            {
                ...
                if (c != null)
                {
                    ...
                    {
                        token =
                            (c.ImpersonateToken != IntPtr.Zero)
                                ? c.ImpersonateToken
                                : ClientIdentityToken;
                    }
                    else
        ...
        internal void RestoreSavedAspNetSynchronizationContext(
        ...
        internal CultureInfo CultureFromConfig(string configString, bool requireSpecific)
        {
            ...
            else if (StringUtil.StringStartsWithIgnoreCase(configString, "auto:"))
            {
                ...
                if (userLanguages != null)
                {
                    ...
                    {
                        return CultureUtil.CreateReadOnlyCulture(
                            configString.Substring(
                                5 /* "auto:".Length */
                            ),
                            requireSpecific
                        );
