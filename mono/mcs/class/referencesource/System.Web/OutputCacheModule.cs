...
namespace System.Web.Caching
{
    ...
    sealed class OutputCacheModule : IHttpModule
    {
        ...
        )
        {
            ...
            if (verb == HttpVerb.POST)
            {
                sb = new StringBuilder(
                    OUTPUTCACHE_KEYPREFIX_POST,
                    path.Length + OUTPUTCACHE_KEYPREFIX_POST.Length
                );
            }
            else
            {
                sb = new StringBuilder(
                    OUTPUTCACHE_KEYPREFIX_GET,
                    path.Length + OUTPUTCACHE_KEYPREFIX_GET.Length
                );
            }
                ...
                for (j = 0; j <= 2; j++)
                {
                    ...
                    switch (j)
                    {
                        ...
                        case 1:
                            ...
                            if (
                                request.HasQueryString && (a != null || cachedVary._varyByAllParams)
                            )
                            {
            ...
            if (tokenEnd == -1)
            {
                ...
                if (tokenEnd > -1)
                {
                    ...
                    if (ParseWeight(acceptEncoding, tokenEnd) == 0)
                    {
                        ...
                        bool identityIsAcceptable =
                            acceptEncodingWithoutWeight != IDENTITY
        ...
        static double GetAcceptableEncodingHelper(string coding, string acceptEncoding)
        {
            ...
            {
                int indexStart = acceptEncoding.IndexOf(
                    coding,
                    startSearchIndex,
        ...
        static double ParseWeight(string acceptEncoding, int startIndex)
        {
            ...
            if (qIndex > -1 && qIndex < tokenEnd)
            {
                ...
                {
                    string s = acceptEncoding.Substring(
                        equalsIndex + 1,
                        tokenEnd - (equalsIndex + 1)
                    );
                    double d;
                    if (
                        Double.TryParse(
        ...
        static bool IsIdentityAcceptable(string acceptEncoding)
        {
            ...
            if (
                ...
                || (
                    identityWeight <= 0
                    && GetAcceptableEncodingHelper(ASTERISK, acceptEncoding) == 0
                )
            )
            {
                result = false;
        ...
        internal void OnEnter(Object source, EventArgs eventArgs)
        {
            ...
            switch (request.HttpVerb)
            {
                ...
                default:
                    Debug.Trace(
                        "OutputCacheModuleEnter",
                        "Miss, Http method not GET, POST, or HEAD"
                            + "\nReturning from OutputCacheModule::Enter"
                    );
            ...
            if (cachedVary != null)
            {
                ...
                if (key == null)
                {
                    Debug.Trace(
                        "OutputCacheModuleEnter",
                        "Miss, key could not be created for vary-by item."
                            + "\nReturning from OutputCacheModule::Enter"
                    );
                ...
                else
                {
                    ...
                    if (acceptEncoding != null)
                    {
                        ...
                        while (!done)
                        {
                            ...
                            int index = GetAcceptableEncoding(
                                contentEncodings,
                                startIndex,
                                acceptEncoding
                            );
                            if (index > -1)
                            {
                                ...
                                Debug.Trace(
                                    "OutputCacheModuleEnter",
                                    "VaryByContentEncoding key=" + key + contentEncodings[index]
                                );
#endif
                ...
                if (
                    item == null
                    || ((CachedRawResponse)item)._cachedVaryId != cachedVary.CachedVaryId
                )
                {
#if DBG
                    if (item == null)
                    {
                        Debug.Trace(
                            "OutputCacheModuleEnter",
                            "Miss, cVary found, cRawResponse not found.\n\t\tkey="
                                + key
                                + "\nReturning from OutputCacheModule::Enter"
                        );
                    }
                    else
                    {
                        string msg =
                            "Miss, _cachedVaryId="
                            + ((CachedRawResponse)item)._cachedVaryId.ToString()
                            + ", cVary.CachedVaryId="
                            + cachedVary.CachedVaryId.ToString();
                        Debug.Trace(
                            "OutputCacheModuleEnter",
                            msg + key + "\nReturning from OutputCacheModule::Enter"
                        );
            ...
            if (cachedVary == null && !settings.IgnoreParams)
            {
                ...
                if (request.HttpVerb == HttpVerb.POST)
                {
                    Debug.Trace(
                        "OutputCacheModuleEnter",
                        "Output cache item found but method is POST and no VaryByParam specified."
                            + "\n\tkey="
                            + key
                            + "\nReturning from OutputCacheModule::Enter"
                    );
                ...
                if (request.HasQueryString)
                {
                    Debug.Trace(
                        "OutputCacheModuleEnter",
                        "Output cache item found but contains a querystring and no VaryByParam specified."
                            + "\n\tkey="
                            + key
                            + "\nReturning from OutputCacheModule::Enter"
                    );
            ...
            if (settings.IgnoreRangeRequests)
            {
                ...
                if (StringUtil.StringStartsWithIgnoreCase(rangeHeader, "bytes"))
                {
                    Debug.Trace(
                        "OutputCacheModuleEnter",
                        "Output cache item found but this is a Range request and IgnoreRangeRequests is true."
                            + "\n\tkey="
                            + key
                            + "\nReturning from OutputCacheModule::Enter"
                    );
                    ...
                    for (i = 0; i < cacheDirectives.Length; i++)
                    {
                        ...
                        if (StringUtil.StringStartsWith(directive, "max-age="))
                        {
                            ...
                            if (maxage >= 0)
                            {
                                age = (int)(
                                    (
                                        context.UtcTimestamp.Ticks
                                        - settings.UtcTimestampCreated.Ticks
                        ...
                        else if (StringUtil.StringStartsWith(directive, "min-fresh="))
                        {
                            try
                            {
                                minfresh = Convert.ToInt32(
                                    directive.Substring(10),
                                    CultureInfo.InvariantCulture
                                );
                            }
                            catch
                            ...
                            {
                                fresh = (int)(
                                    (settings.UtcExpires.Ticks - context.UtcTimestamp.Ticks)
                                    / TimeSpan.TicksPerSecond
                                );
                                if (fresh < minfresh)
                                {
            ...
            else if (settings.ValidationCallbackInfo != null)
            {
                ...
                for (i = 0, n = settings.ValidationCallbackInfo.Length; i < n; i++)
                {
                    ...
                    switch (validationStatus)
                    {
                        case HttpValidationStatus.Invalid:
                            Debug.Trace(
                                "OutputCacheModuleEnter",
                                "Output cache item found but callback invalidated it."
                                    + "\n\tkey="
                                    + key
                                    + "\nReturning from OutputCacheModule::Enter"
                            );
                        ...
                        default:
                            Debug.Trace(
                                "OutputCacheModuleEnter",
                                "Invalid validation status, ignoring it, status="
                                    + validationStatus
                                    + "\n\tkey="
                                    + key
                            );
                ...
                if (validationStatusFinal == HttpValidationStatus.IgnoreThisRequest)
                {
                    Debug.Trace(
                        "OutputCacheModuleEnter",
                        "Output cache item found but callback status is IgnoreThisRequest."
                            + "\n\tkey="
                            + key
                            + "\nReturning from OutputCacheModule::Enter"
                    );
            ...
            if (!rawResponse.HasSubstBlocks)
            {
                ...
                if (ifModifiedSinceHeader != null)
                {
                    ...
                    catch
                    {
                        Debug.Trace(
                            "OutputCacheModuleEnter",
                            "Ignore If-Modified-Since header, invalid format: "
            ...
            else
            {
                ...
                else
                {
                    Debug.Trace(
                        "OutputCacheModuleEnter",
                        "Hit, but conditional request not satisfied.\n\tkey="
                            + key
                            + "\nReturning from OutputCacheModule::Enter"
                    );
                }
        ...
        void OnLeave(Object source, EventArgs eventArgs)
        {
            ...
            do
            {
...
#if DBG
                    reason =
                        "CachePolicy.Cacheability is not Public, ServerAndPrivate, or ServerAndNoCache.";
#endif
                    break;
                }
                ...
                if (
                    !cache.VaryByParams.AcceptsParams()
                    && (request.HttpVerb == HttpVerb.POST || request.HasQueryString)
                )
                {
#if DBG
                    reason =
                        "the cache cannot cache responses to POSTs or GETs with query strings unless Cache.VaryByParams is modified.";
#endif
                    break;
                }
...
#if DBG
                    reason =
                        "the cache cannot cache encoded responses that are not listed in the VaryByContentEncodings collection.";
#endif
                    break;
                }
            ...
            if (!cacheable)
            {
                ...
                Debug.Trace(
                    "OutputCacheModuleLeave",
                    "Item is not output cacheable because "
                        + reason
                        + "\n\tUrl="
                        + request.Path
            ...
            else
            {
                ...
                if (keyRawResponse == null)
                {
                    Debug.Trace(
                        "OutputCacheModuleLeave",
                        "Couldn't add non-cacheable post.\n\tkey=" + _key
                    );
                    return;
                }
            ...
            else if (settings.IsMaxAgeSet)
            {
                DateTime utcTimestamp =
                    (settings.UtcTimestampCreated != DateTime.MinValue)
