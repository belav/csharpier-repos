...
namespace System.Web.Caching
{
    ...
    internal class CacheKey
    {
        ...
        internal CacheKey(String key, bool isPublic)
        {
            ...
            {
                Debug.Assert(
                    CacheInternal.PrefixFIRST[0] <= key[0] && key[0] <= CacheInternal.PrefixLAST[0],
                    "CacheInternal.PrefixFIRST[0] <= key[0] && key[0] <= CacheInternal.PrefixLAST[0], key="
                        + key
                );
            }
#endif
...
*/
        ...
        internal void MonitorDependencyChanges()
        {
            ...
            if (dependency != null && State == EntryState.AddedToCache)
            {
                ...
                dependency.SetCacheDependencyChanged(
...
*/
        internal void Close(CacheItemRemovedReason reason)
        {
            ...
            if (onRemovedTargets != null)
            {
                ...
                {
                    CallCacheItemRemovedCallback(
                        (CacheItemRemovedCallback)onRemovedTargets,
                        reason
                    );
                }
                else
        ...
        string DebugDescription(string indent)
        {
            ...
            sb.Append(nlindent);
            sb.Append("_value=");
            sb.Append(Debug.GetDescription(_value, indent));
            sb.Append(nlindent);
            sb.Append("_utcExpires=");
            sb.Append(Debug.FormatUtcDate(_utcExpires));
            sb.Append(nlindent);
            sb.Append("_bits=0x");
            sb.Append(((int)_bits).ToString("x", CultureInfo.InvariantCulture));
            sb.Append("\n");
