...
namespace System.Web.Caching
{
    ...
    sealed class CacheMemoryTotalMemoryPressure : CacheMemoryPressure
    {
        ...
        internal CacheMemoryTotalMemoryPressure()
        {
            ...
            */
            ...
            PerfCounters.SetCounter(
                AppPerfCounter.CACHE_PERCENT_MACH_MEM_LIMIT_USED_BASE,
                _pressureHigh
            );
        }
        ...
        internal override void ReadConfig(CacheSection cacheSection)
        {
            ...
            Debug.Trace(
                "CacheMemory",
                "CacheMemoryTotalMemoryPressure.ReadConfig: _pressureHigh="
                    + _pressureHigh
                    + ", _pressureMiddle="
                    + _pressureMiddle
                    + ", _pressureLow="
        ...
        protected override int GetCurrentPressure()
        {
            ...
            if (_pressureHigh != 0)
            {
                ...
                //    = total physical memory used / total physical memory used limit
                PerfCounters.SetCounter(
                    AppPerfCounter.CACHE_PERCENT_MACH_MEM_LIMIT_USED,
                    memoryLoad
                );
            }
        ...
        internal override int GetPercentToTrim(DateTime lastTrimTime, int lastTrimPercent)
        {
            ...
            if (IsAboveHighPressure())
            {
                ...
                SafeNativeMethods.OutputDebugString(
                    String.Format(
                        "CacheMemoryTotalMemoryPressure.GetPercentToTrim: percent={0:N}, lastTrimPercent={1:N}, secondsSinceTrim={2:N}\n",
                        percent,
                        lastTrimPercent,
    ...
    sealed class CacheMemorySizePressure : CacheMemoryPressure
    {
            ...
            get
            {
                ...
                if (memoryLimit == -1)
                {
                    ...
                    if (totalPhysical != 0)
                    {
                        ...
                        long usableMemory = HostingEnvironment.IsHosted
                            ? totalPhysical * 3 / 5
                            : totalPhysical;
            ...
            get
            {
                ...
                if (memoryLimit == -1)
                {
                    ...
                    if (UnsafeNativeMethods.GetModuleHandle(ModName.WP_FULL_NAME) != IntPtr.Zero)
                    {
                        ...
                        UnsafeNativeMethods.GetModuleHandle(ModName.W3WP_FULL_NAME) != IntPtr.Zero
                    )
                    {
        ...
        internal override void ReadConfig(CacheSection cacheSection)
        {
            ...
            Debug.Trace(
                "CacheMemory",
                "CacheMemorySizePressure.ReadConfig: _memoryLimit="
            ...
            s_pollInterval = (int)
                Math.Min(
                    cacheSection.PrivateBytesPollTime.TotalMilliseconds,
            ...
            PerfCounters.SetCounter(
                AppPerfCounter.CACHE_PERCENT_PROC_MEM_LIMIT_USED_BASE,
                (int)(_memoryLimit >> KILOBYTE_SHIFT)
            );

            Debug.Trace(
                "CacheMemory",
                "CacheMemorySizePressure.ReadConfig: _pressureHigh="
                    + _pressureHigh
                    + ", _pressureMiddle="
                    + _pressureMiddle
                    + ", _pressureLow="
        ...
        protected override int GetCurrentPressure()
        {
            ...
            //
            PerfCounters.SetCounter(
                AppPerfCounter.CACHE_PERCENT_PROC_MEM_LIMIT_USED,
                (int)(cacheSize >> KILOBYTE_SHIFT)
            );
        ...
        internal override int GetPercentToTrim(DateTime lastTrimTime, int lastTrimPercent)
        {
            ...
            if (IsAboveHighPressure())
            {
                ...
                SafeNativeMethods.OutputDebugString(
                    String.Format(
                        "CacheMemorySizePressure.GetPercentToTrim: percent={0:N}, lastTrimPercent={1:N}\n",
                        percent,
                        lastTrimPercent
                    )
                );
    ...
    class CacheMemoryStats
    {
        ...
        internal bool IsAboveHighPressure()
        {
            return _pressureTotalMemory.IsAboveHighPressure()
                || _pressureCacheSize.IsAboveHighPressure();
        }

        internal bool IsAboveMediumPressure()
        {
            return _pressureTotalMemory.IsAboveMediumPressure()
                || _pressureCacheSize.IsAboveMediumPressure();
        }
        ...
        internal int GetPercentToTrim()
        {
            ...
            {
                return Math.Max(
                    _pressureTotalMemory.GetPercentToTrim(_lastTrimTime, _lastTrimPercent),
        ...
        internal void SetTrimStats(
            long trimDurationTicks,
            long totalCountBeforeTrim,
            long trimCount
        )
        {
