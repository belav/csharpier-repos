...
namespace System.Web.Caching
{
    ...
    public sealed class Cache : IEnumerable
    {
        ...
        static CacheItemRemovedCallback s_sentinelRemovedCallback = new CacheItemRemovedCallback(
            SentinelEntry.OnCacheItemRemovedCallback
        );
        ...
        public int Count
        {
            get { return Convert.ToInt32(ObjectCache.ItemCount); }
        }
                ...
                lock (this)
                {
                    if (_internalCache == null)
                    {
                        NameValueCollection cacheProviderSettings =
                            HostingEnvironment.CacheStoreProviderSettings;
                        ...
                        if (cacheProviderSettings != null)
                        {
                            ...
                            _internalCache = (CacheStoreProvider)
                        ...
                        else
                        {
                            ...
                            {
                                _internalCache = new AspNetCache(
                                    (AspNetCache)_objectCache,
                                    isPublic: false
                                );
                            }
                            else
                ...
                lock (this)
                {
                    if (_objectCache == null)
                    {
                        ...
                        if (cacheProviderSettings != null)
                        {
                            ...
                            _objectCache = (CacheStoreProvider)
                                ProvidersHelper.InstantiateProvider(
                                    cacheProviderSettings,
                                    typeof(CacheStoreProvider)
                                );
                        ...
                        else
                        {
                            ...
                            {
                                _objectCache = new AspNetCache(
                                    (AspNetCache)_internalCache,
                                    isPublic: true
                                );
                            }
                            else
        ...
        private class SentinelEntry
        {
            ...
            public static void OnCacheItemRemovedCallback(
                string key,
                object value,
                CacheItemRemovedReason reason
            )
            {
                ...
                switch (reason)
                {
                    ...
                    case CacheItemRemovedReason.Underused:
                        Debug.Fail(
                            "Reason should never be CacheItemRemovedReason.Underused since the entry was inserted as NotRemovable."
                        );
                        return;
                    default:
        ...
        public void Insert(string key, object value, CacheDependency dependencies)
        {
            ObjectCache.Insert(
                key,
                value,
                new CacheInsertOptions() { Dependencies = dependencies }
            );
        }
        ...
        {
            DateTime utcAbsoluteExpiration = DateTimeUtil.ConvertToUniversalTime(
                absoluteExpiration
            );
            ObjectCache.Insert(
                key,
        ...
        {
            get { return AspNetMemoryMonitor.ProcessPrivateBytesLimit; }
        }

        public long EffectivePercentagePhysicalMemoryLimit
        {
            get { return AspNetMemoryMonitor.PhysicalMemoryPercentageLimit; }
        }
    }
}
