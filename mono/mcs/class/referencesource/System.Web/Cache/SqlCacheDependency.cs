...
namespace System.Web.Caching
{
    ...
    public sealed class SqlCacheDependency : CacheDependency
    {
        ...
        public SqlCacheDependency(SqlCommand sqlCmd)
        {
            ...
            if (
                context != null
                && context.SqlDependencyCookie != null
                && // That means We have already setup SQL9 dependency for output cache
                sqlCmd.NotificationAutoEnlist
            )
        ...
        internal static void ValidateOutputCacheDependencyString(string depString, bool page)
        {
            ...
            if (StringUtil.EqualsIgnoreCase(depString, SQL9_CACHE_DEPENDENCY_DIRECTIVE))
            {
                if (!page)
                {
                    ...
                    throw new HttpException(SR.GetString(SR.Attrib_Sql9_not_allowed));
                }
            }
            else
            {
        ...
        public static CacheDependency CreateOutputCacheDependency(string dependency)
        {
            if (dependency == null)
            {
                throw new HttpException(
                    SR.GetString(SR.Invalid_sqlDependency_argument, dependency)
                );
            }
            ...
            else
            {
                ...
                Debug.Trace(
                    "SqlCacheDependency",
                    "Creating SqlCacheDependency for SQL8 output cache"
                );

                if (sqlDependencies.Count == 1)
                {
                    info = (Sql7DependencyInfo)sqlDependencies[0];
                    return CreateSql7SqlCacheDependencyForOutputCache(
                        info._database,
                        info._table,
                        dependency
                    );
        ...
        static SqlCacheDependency CreateSql7SqlCacheDependencyForOutputCache(
            string database,
            string table,
            string depString
        )
        {
            ...
            catch (HttpException e)
            {
                HttpException outerException = new HttpException(
                    SR.GetString(SR.Invalid_sqlDependency_argument2, depString, e.Message),
                    e
                );
        ...
        internal static ArrayList ParseSql7OutputCacheDependency(string outputCacheString)
        {
            ...
            try
            {
                for (int i = 0; i < outputCacheString.Length + 1; i++)
                {
                    ...
                    if (i == outputCacheString.Length || outputCacheString[i] == ';')
                    {
                        ...
                        info._table = VerifyAndRemoveEscapeCharacters(
                            outputCacheString.Substring(iTableStart, len)
                        );
            ...
            {
                throw new ArgumentException(
                    SR.GetString(SR.Invalid_sqlDependency_argument, outputCacheString)
                );
            }
        }
    }
    ...
    public sealed class DatabaseNotEnabledForNotificationException : SystemException
    {
        ...
        internal DatabaseNotEnabledForNotificationException(
    ...
    public sealed class TableNotEnabledForNotificationException : SystemException
    {
        ...
        internal TableNotEnabledForNotificationException(
            SerializationInfo info,
            StreamingContext context
        )
            : base(info, context) { }
    ...
    internal class DatabaseNotifState : IDisposable
    {
            ...
            else
            {
                ...
                try
                {
                    ...
                    sqlCmd = new SqlCommand(
                        SqlCacheDependencyManager.SQL_POLLING_SP_DBO,
                        holder.Connection
                    );
        ...
        internal void ReleaseConnection(
            ref SqlConnection sqlConn,
            ref SqlCommand sqlCmd,
            bool error
        )
        {
            // !!! Please note that GetConnection and ReleaseConnection does NOT support
    ...
    internal static class SqlCacheDependencyManager
    {
        ...
        internal static void InitPolling(string database)
        {
            ...
            if (!config.Enabled)
            {
                throw new ConfigurationErrorsException(
                    ...
                    config.ElementInformation.Properties["enabled"].Source,
                    config.ElementInformation.Properties["enabled"].LineNumber
                );
            }
            ...
            if (sqlDepDB.PollTime == 0)
            {
                throw new ConfigurationErrorsException(
                    ...
                    sqlDepDB.ElementInformation.Properties["pollTime"].Source,
                    sqlDepDB.ElementInformation.Properties["pollTime"].LineNumber
                );
            }
            ...
            if (connectionString == null || connectionString.Length < 1)
            {
                throw new ConfigurationErrorsException(
                    ...
                    sqlDepDB.ElementInformation.Properties["connectionStringName"].Source,
                    sqlDepDB.ElementInformation.Properties["connectionStringName"].LineNumber
                );
            }
            ...
            lock (s_DatabaseNotifStates)
            {
                ...
                state._timer = new Timer(
                    s_timerCallback,
                    state,
                    0 /* dueTime */
                    ,
                    sqlDepDB.PollTime /* period */
                );
        ...
        internal static void PollDatabaseForChanges(DatabaseNotifState dbState, bool fromTimer)
        {
            ...
            Debug.Trace(
                "SqlCacheDependencyManagerPolling",
                ...
                if (!fromTimer)
                {
                    ...
                    for (; ; )
                    {
                        ...
                        if (!System.Diagnostics.Debugger.IsAttached && DateTime.UtcNow > waitLimit)
                        {
                            ...
                            throw new HttpException(
                                SR.GetString(
                                    SR.Cant_connect_sql_cache_dep_database_polling,
                                    dbState._database
                                )
                            );
                        }
            ...
            try
            {
                try
                {
                    ...
                    while (sqlReader.Read())
                    {
                        ...
                        {
                            Debug.Assert(
                                !dbState._tables.ContainsKey(tableName),
                ...
                catch (Exception e)
                {
                    ...
                    if (sqlExpt != null)
                    {
                        Debug.Trace(
                            "SqlCacheDependencyManagerPolling",
                            "Error reading rows.  SqlException:"
                                + "\nMessage="
                                + sqlExpt.Message
                                + "\nNumber="
                                + sqlExpt.Number
                    ...
                    else
                    {
                        ...
                        Debug.Trace(
                            "SqlCacheDependencyManagerPolling",
                            "Error reading rows.  Exception:" + pollExpt
                        );
                    }
                }
                finally
                {
                    ...
                    lock (dbState)
                    {
                        ...
                        if (
                            dbState._notifEnabled
                            && !notifEnabled
                            && pollExpt != null
                            && dbState._pollSqlError == SQL_EXCEPTION_SP_NOT_FOUND
                        )
                        {
                            foreach (object key in dbState._tables.Keys)
                            {
                                try
                                {
                                    cacheInternal.Remove(
                                        GetMoniterKey(dbState._database, (string)key)
                                    );
                                }
                                catch { }
                                ...
                                Debug.Trace(
                                    "SqlCacheDependencyManagerPolling",
                                    "Changed to disabled.  Remove Database="
                                        + dbState._database
                                        + "; key="
                                        + key
                                );
                        ...
                        Debug.Trace(
                            ...
                            "dbState:_pollExpt="
                                + dbState._pollExpt
                                + "; _pollSqlError="
                                + dbState._pollSqlError
                                + "; _notifEnabled="
                                + dbState._notifEnabled
                                + "; __utcTablesUpdated="
                                + dbState._utcTablesUpdated
                        );
                    }
            ...
            {
                throw;
            } // Prevent Exception Filter Security Issue (ASURT 122835)
        }
        ...
        static internal void EnsureTableIsRegisteredAndPolled(string database, string table)
        {
            ...
            Debug.Trace(
                "SqlCacheDependencyManagerCheck",
            ...
            if (!dbState._init)
            {
                ...
                Debug.Trace(
                    "SqlCacheDependencyManagerCheck",
                    "Waiting for intialization: timeout=" + timeout + "s"
                );
            ...
            while (true)
            {
                ...
                if (
                    pollException == null
                    && // No exception from polling
                    dbRegistered
                    && // The database is registered
                    dbState._tables.ContainsKey(table)
                )
                { // The table is also registered
                    Debug.Trace(
                        "SqlCacheDependencyManagerCheck",
                        "The table is registered too.  Exit now!"
                    );
                    return;
                }
                ...
                if (!doubleChecked && DateTime.UtcNow - utcTablesLastUpdated >= OneSec)
                {
                ...
                if (pollSqlError == SQL_EXCEPTION_SP_NOT_FOUND)
                {
                    ...
                    Debug.Assert(
                        dbRegistered == false,
                        "When this error happened, we shouldn't be able to poll the database"
                    );
                    pollException = null;
                }
                ...
                if (pollException != null)
                {
                    ...
                    if (
                        pollSqlError == SQL_EXCEPTION_PERMISSION_DENIED_ON_OBJECT
        ...
        internal static void UpdateDatabaseNotifState(string database)
        {
            using (new ApplicationImpersonationContext())
            {
                Debug.Trace(
                    "SqlCacheDependencyManager",
                    "UpdateDatabaseNotifState called for database "
                        + database
                        + "; running as "
                ...
                InitPolling(database);
                Debug.Assert(
                    s_DatabaseNotifStates[database] != null,
                    "s_DatabaseNotifStates[database] != null"
                );

                PollDatabaseForChanges(
                    (DatabaseNotifState)s_DatabaseNotifStates[database],
                    false /*fromTimer*/
                );
            }
        }
        ...
        internal static DatabaseNotifState AddRef(string database)
        {
            ...
            Debug.Trace(
                "SqlCacheDependencyManager",
                "AddRef called for " + database + "; res=" + res
            );
        ...
        internal static void Release(DatabaseNotifState dbState)
        {
            ...
            Debug.Trace(
                "SqlCacheDependencyManager",
                "Release called for " + dbState._database + "; res=" + res
            );
    ...
    public static class SqlCacheDependencyAdmin
    {
        ...
        internal const string SQL_CREATE_ENABLE_DATABASE_SP =
            ...
            + "         BEGIN \n"
            + "             RAISERROR (''"
            + SqlCacheDependencyManager.SQL_CUSTOM_ERROR_TABLE_NOT_FOUND
            + "'', 16, 1) \n"
            + "             RETURN \n"
            + "         END \n"
            + "\n"
        ...
        internal static void SetupNotifications(int flags, string table, string connectionString)
        {
            ...
            if (tableOp)
            {
                ...
                if (table == null)
                {
                    ...
                    {
                        throw new ArgumentException(
                            SR.GetString(SR.Cache_null_table_in_tables),
                            "tables"
                        );
                    }
                    else
                ...
                else if (table.Length == 0)
                {
                    ...
                    {
                        throw new ArgumentException(SR.GetString(SR.Cache_null_table), "table");
                    }
                }
            }
            ...
            catch (Exception e)
            {
                    ...
                    if (sqlExpt.Number == SqlCacheDependencyManager.SQL_EXCEPTION_SP_NOT_FOUND)
                    {
                        if (!disable)
                        {
                            if (table != null)
                            {
                                throw new DatabaseNotEnabledForNotificationException(
                                    SR.GetString(
                                        SR.Database_not_enabled_for_notification,
                        ...
                        sqlExpt.Number
                            == SqlCacheDependencyManager.SQL_EXCEPTION_PERMISSION_DENIED_ON_OBJECT
                        || sqlExpt.Number
                            == SqlCacheDependencyManager.SQL_EXCEPTION_PERMISSION_DENIED_ON_DATABASE
                        || sqlExpt.Number
                            == SqlCacheDependencyManager.SQL_EXCEPTION_PERMISSION_DENIED_ON_USER
                        || sqlExpt.Number
                            == SqlCacheDependencyManager.SQL_EXCEPTION_NO_GRANT_PERMISSION
                    )
                    {
                        string error;
                        ...
                        if (table != null)
                        {
                            throw new HttpException(SR.GetString(error, table));
                        }
                        else
                        {
                            throw new HttpException(SR.GetString(error));
                        }
                    }
                    else if (
                        sqlExpt.Number == SqlCacheDependencyManager.SQL_EXCEPTION_ADHOC
                        && sqlExpt.Message
                            == SqlCacheDependencyManager.SQL_CUSTOM_ERROR_TABLE_NOT_FOUND
                    )
                    {
                ...
                {
                    errString = SR.GetString(
                        SR.Cant_connect_sql_cache_dep_database_admin_cmdtxt,
                        sqlCmd.CommandText
                    );
                }
                else
        ...
        static string[] GetEnabledTables(string connectionString)
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    throw new DatabaseNotEnabledForNotificationException(
                        SR.GetString(SR.Database_not_enabled_for_notification, sqlConn.Database)
                    );
                }
                else
                {
                    throw new HttpException(
                        SR.GetString(SR.Cant_get_enabled_tables_sql_cache_dep),
                        e
                    );
                }
            }
