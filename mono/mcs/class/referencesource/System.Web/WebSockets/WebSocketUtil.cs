...
namespace System.Web.WebSockets
{
    ...
    internal static class WebSocketUtil
    {
        ...
        public static bool IsSameOriginRequest(HttpWorkerRequest workerRequest)
        {
            ...
            bool urisCreatedSuccessfully =
                Uri.TryCreate(
                    workerRequest.GetProtocol() + "://" + hostHeader.Trim(),
                    UriKind.Absolute,
                    out hostHeaderUri
                ) // RFC 2616 (Sec. 14.23): "Host" header doesn't contain the scheme, so we need to prepend
                && Uri.TryCreate(
                    secWebSocketOriginHeader.Trim(),
                    UriKind.Absolute,
                    out originHeaderUri
                );
