...
namespace System.Web.Handlers
{
    ...
    public sealed class AssemblyResourceLoader : IHttpHandler
    {
        ...
        ///     a unique cache key.
        /// </devdoc>
        private static int CreateWebResourceUrlCacheKey(
            Assembly assembly,
            string resourceName,
            bool htmlEncoded,
        ...
        private static void EnsureHandlerExistenceChecked()
        {
            ...
            if (!_handlerExistenceChecked)
            {
                ...
                IIS7WorkerRequest iis7WorkerRequest =
                    (context != null) ? context.WorkerRequest as IIS7WorkerRequest : null;
                string webResourcePath = UrlPath.Combine(
                ...
                if (iis7WorkerRequest != null)
                {
                    ...
                    string handlerTypeString = iis7WorkerRequest.MapHandlerAndGetHandlerTypeString(
                        method: "GET",
                        path: UrlPath.Combine(
                            HttpRuntime.AppDomainAppVirtualPathString,
                            _webResourceUrl
                        ),
                        convertNativeStaticFileModule: false,
                        ignoreWildcardMappings: true
                    );
        ...
        /// </devdoc>
        private static string FormatWebResourceUrl(
            string assemblyName,
            string resourceName,
            long assemblyDate,
            bool htmlEncoded
        ...
        {
            string encryptedData = Page.EncryptString(
        ...
        internal static string GetWebResourceUrl(
            Type type,
            string resourceName,
            bool htmlEncoded,
        ...
        )
        {
            ...
            if (assembly == typeof(AssemblyResourceLoader).Assembly)
            {
                if (String.Equals(resourceName, "WebForms.js", StringComparison.Ordinal))
                {
                    ...
                    String.Equals(resourceName, "WebUIValidation.js", StringComparison.Ordinal)
                )
                {
        ...
        private static WebResourceAttribute FindWebResourceAttribute(
            Assembly assembly,
            string resourceName
        )
        {
            ...
            for (int i = 0; i < attrs.Length; i++)
            {
                ...
                if (
                    (wra != null)
                    && String.Equals(wra.WebResource, resourceName, StringComparison.Ordinal)
                )
                {
                    return wra;
                }
        ...
        internal static string GetWebResourceUrlInternal(
            Assembly assembly,
            string resourceName,
            bool htmlEncoded,
            bool forSubstitution,
        ...
        )
        {
            ...
            if (!_handlerExists)
            {
                throw new InvalidOperationException(
                    SR.GetString(SR.AssemblyResourceLoader_HandlerNotRegistered)
                );
            }
            ...
            int urlCacheKey = CreateWebResourceUrlCacheKey(
                assembly,
                resourceName,
                htmlEncoded,
                forSubstitution,
            ...
            if (url == null)
            {
                ...
                if (definition != null)
                {
                    if (enableCdn)
                    {
                        ...
                        if (debuggingEnabled)
                        {
                            path = secureConnection
                                ? definition.CdnDebugPathSecureConnection
                            ...
                            if (String.IsNullOrEmpty(path))
                            {
                                ...
                                if (String.IsNullOrEmpty(path))
                                {
                                    ...
                                    if (
                                        !secureConnection
                                        || String.IsNullOrEmpty(definition.CdnDebugPath)
                                    )
                                    {
                                        path = GetCdnPath(
                        ...
                        {
                            path = secureConnection
                                ? definition.CdnPathSecureConnection
                                : definition.CdnPath;
                            if (String.IsNullOrEmpty(path))
                            {
                                // Get CDN Path from the redirected resource name/assembly, not the original one
                ...
                else
                {
                    ...
                    if (effectiveAssembly.GlobalAssemblyCache)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                builder.Append(
                                    token[i].ToString("x2", CultureInfo.InvariantCulture)
                                );
                            }
        ...
        internal static bool IsValidWebResourceRequest(HttpContext context)
        {
            ...
            string webResourceHandlerUrl = UrlPath.Combine(
                HttpRuntime.AppDomainAppVirtualPathString,
                _webResourceUrl
            );
        ...
        internal static void LogWebResourceFailure(string decryptedData, Exception exception)
        {
            ...
            {
                errorMessage = SR.GetString(
                    SR.Webevent_msg_RuntimeErrorWebResourceFailure_ResourceMissing,
                    decryptedData
                );
            }
            else
            {
                errorMessage = SR.GetString(
                    SR.Webevent_msg_RuntimeErrorWebResourceFailure_DecryptionError
                );
            }
            ...
            try
            {
                ...
                if (String.IsNullOrEmpty(encryptedData))
                {
                    throw new HttpException(
                        404,
                        SR.GetString(SR.AssemblyResourceLoader_InvalidRequest)
                    );
                }
                resourceIdentifierPresent = true;

                decryptedData = Page.DecryptString(
                    encryptedData,
                    Purpose.AssemblyResourceLoader_WebResourceUrl
                );
                ...
                if (String.IsNullOrEmpty(assemblyName))
                {
                    throw new HttpException(
                        404,
                        SR.GetString(SR.AssemblyResourceLoader_AssemblyNotFound, assemblyName)
                    );
                }
                ...
                if (String.IsNullOrEmpty(resourceName))
                {
                    throw new HttpException(
                        404,
                        SR.GetString(SR.AssemblyResourceLoader_ResourceNotFound, resourceName)
                    );
                }
                    ...
                    try
                    {
                        ...
                        if (resourceStream != null)
                        {
                            ...
                            if (performSubstitution)
                            {
                                ...
                                {
                                    newContent.Append(
                                        content.Substring(startIndex, match.Index - startIndex)
                                    );
                                        ...
                                        if (embeddedResourceName.Length > 0)
                                        {
                                            ...
                                            )
                                            {
                                                throw new HttpException(
                                                    404,
                                                    SR.GetString(
                                                        SR.AssemblyResourceLoader_NoCircularReferences,
                                                        resourceName
                                                    )
                                                );
                                            }
                                ...
                                newContent.Append(
                                    content.Substring(startIndex, content.Length - startIndex)
                                );
