...
namespace System.Web.Handlers
{
    ...
    public class TraceHandler : IHttpHandler
    {
        ...
        {
            get { return ((IHttpHandler)this).IsReusable; }
        }
        ...
        void IHttpHandler.ProcessRequest(HttpContext context)
        {
            ...
            if (
                DeploymentSection.RetailInternal
        ...
        protected void ShowDetails(DataSet data)
        {
            ...
            table = CreateTable(
                data.Tables[SR.Trace_Session_State],
                true /*encodeSpaces*/
            );
            if (table != null)
                table.RenderControl(_writer);
            ...
            table = CreateTable(
                data.Tables[SR.Trace_Application_State],
                true /*encodeSpaces*/
            );
            if (table != null)
                table.RenderControl(_writer);
            ...
            table = CreateTable(
                data.Tables[SR.Trace_Request_Cookies_Collection],
                true /*encodeSpaces*/
            );
            if (table != null)
                table.RenderControl(_writer);
            ...
            table = CreateTable(
                data.Tables[SR.Trace_Response_Cookies_Collection],
                true /*encodeSpaces*/
            );
            if (table != null)
                table.RenderControl(_writer);
            ...
            table = CreateTable(
                data.Tables[SR.Trace_Headers_Collection],
                true /*encodeSpaces*/
            );
            if (table != null)
                table.RenderControl(_writer);
            ...
            table = CreateTable(
                data.Tables[SR.Trace_Server_Variables],
                true /*encodeSpaces*/
            );
            if (table != null)
                table.RenderControl(_writer);
        ...
        protected void ShowRequests(IList data)
        {
            ...
            AddCell(
                trow,
                "[ <a href=\"Trace.axd?clear=1\" class=\"link\">"
                    + SR.GetString(SR.Trace_Clear_Current)
                    + "</a> ]"
            );
            ...
            if (HttpRuntime.HasAppPathDiscoveryPermission())
                physicalPath =
                    SR.GetString(SR.Trace_Physical_Directory) + _request.PhysicalApplicationPath;
            trow = AddRow(table);
            ...
            tcell = AddHeaderCell(
                trow,
                "<h3><b>" + SR.GetString(SR.Trace_Requests_This) + "</b></h3>"
            );
            tcell.ColumnSpan = 5;
            tcell.CssClass = "alt";
            ...
            tcell = AddHeaderCell(
                trow,
                SR.GetString(SR.Trace_Remaining)
                    + " "
                    + HttpRuntime.Profile.RequestsRemaining.ToString(NumberFormatInfo.InvariantInfo)
            );
            tcell.CssClass = "alt";
            ...
            for (int i = 0; i < data.Count; i++)
            {
                ...
                AddCell(
                    trow,
                    (string)current.Tables[SR.Trace_Request].Rows[0][SR.Trace_Time_of_Request]
                );
                AddCell(
                    trow,
                    HttpUtility
                        .HtmlEncode((string)current.Tables[SR.Trace_Request].Rows[0][SR.Trace_Url])
                        .Substring(vrootLen)
                );
                AddCell(
                    trow,
                    current.Tables[SR.Trace_Request].Rows[0][SR.Trace_Status_Code].ToString()
                );
                AddCell(
                    trow,
                    (string)current.Tables[SR.Trace_Request].Rows[0][SR.Trace_Request_Type]
                );
        ...
        internal static Table CreateControlTable(DataTable datatable)
        {
            ...
            tcell = AddHeaderCell(
                trow,
                "<h3><b>" + SR.GetString(datatable.TableName) + "</b></h3>"
            );
            tcell.CssClass = "alt";
            tcell.ColumnSpan = 5;
            tcell.HorizontalAlign = HorizontalAlign.Left;
            ...
            while (en.MoveNext())
            {
                ...
                control = HttpUtility.HtmlEncode(
                    (string)((DataRow)en.Current)[SR.Trace_Control_Id]
                );
        ...
        internal static Table CreateTraceTable(DataTable datatable)
        {
            ...
            while (en.MoveNext())
            {
                ...
                tcell = AddCell(
                    trow,
                    HttpUtility.FormatPlainTextAsHtml((string)datarow[SR.Trace_Category])
                );
                if (isErr)
                    tcell.CssClass = "err";

                StringBuilder message = new StringBuilder(
                    HttpUtility.FormatPlainTextAsHtml((string)datarow[SR.Trace_Message])
                );
                ...
                if (!(errormessage is System.DBNull))
                    message.Append(
                        "<br>" + HttpUtility.FormatPlainTextAsHtml((string)errormessage)
                    );
        ...
        private static string FormatPotentialDouble(object o)
        {
            ...
            return (o is double)
                ? ((double)o).ToString("F6", CultureInfo.CurrentCulture)
                : o.ToString();
        }
        ...
        internal static Table CreateTable(DataTable datatable, bool encodeSpaces)
        {
            ...
            while (en.MoveNext())
            {
                ...
                for (int i = 0; i < cells.Length; i++)
                {
                    string temp;
                    if (encodeSpaces)
                        temp = HttpUtility.FormatPlainTextSpacesAsHtml(
                            HttpUtility.HtmlEncode(cells[i].ToString())
                        );
                    else
        ...
        internal static Table CreateDetailsTable(DataTable datatable)
        {
            ...
            TableRow trow = AddRow(table);
            TableCell tcell = AddHeaderCell(
                trow,
                "<h3><b>" + SR.GetString(SR.Trace_Request_Details) + "</b></h3>"
            );
            tcell.ColumnSpan = 10;
            tcell.CssClass = "alt";
            tcell.HorizontalAlign = HorizontalAlign.Left;
            ...
            AddCell(
                trow,
                HttpUtility.HtmlEncode(datatable.Rows[0][SR.Trace_Session_Id].ToString())
            );
