    ...
    public class SqlWebEventProvider : BufferedWebEventProvider, IInternalWebEventProvider
    {
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (!String.IsNullOrEmpty(temp))
            {
                if (!String.IsNullOrEmpty(_sqlConnectionString))
                {
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Only_one_connection_string_allowed)
                    );
                }
                ...
                {
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Connection_string_not_found, temp)
                    );
                }
            }
            else
            {
                ...
                {
                    throw new ConfigurationErrorsException(
                        SR.GetString(SR.Cannot_use_integrated_security)
                    );
                }
            }
            ...
            {
                throw new ConfigurationErrorsException(
                    SR.GetString(SR.Must_specify_connection_string_or_name, temp)
                );
            }
            ...
            else if (_maxEventDetailsLength > SQL_MAX_NTEXT_SIZE)
            {
                throw new ConfigurationErrorsException(
                    SR.GetString(
                        SR.Invalid_max_event_details_length,
                        name,
                        _maxEventDetailsLength.ToString(CultureInfo.CurrentCulture)
                    )
                );
            }

            ProviderUtil.GetAndRemovePositiveAttribute(
                config,
                "commandTimeout",
                name,
                ref _commandTimeout
        ...
        void PrepareParams(SqlCommand sqlCommand)
        {
            ...
            sqlCommand.Parameters.Add(
                new SqlParameter("@ApplicationPath", SqlDbType.NVarChar, 256)
            );
            sqlCommand.Parameters.Add(
                new SqlParameter("@ApplicationVirtualPath", SqlDbType.NVarChar, 256)
            );
        ...
        void FillParams(SqlCommand sqlCommand, WebBaseEvent eventRaised)
        {
            ...
            sqlCommand.Parameters[n++].Value = eventRaised.EventID.ToString(
                "N",
            ...
            sqlCommand.Parameters[n++].Value =
                (reqInfo != null) ? reqInfo.RequestUrl : Convert.DBNull;
            ...
            sqlCommand.Parameters[n++].Value =
                (exception != null) ? exception.GetType().ToString() : Convert.DBNull;

            // @Details
            details = eventRaised.ToString();
            if (_maxEventDetailsLength != NO_LIMIT && details.Length > _maxEventDetailsLength)
            {
                ...
                try
                {
                    ...
                    if (eventsDiscardedByBuffer != 0)
                    {
                        WebBaseEvent infoEvent = new WebBaseEvent(
                            SR.GetString(
                                SR.Sql_webevent_provider_events_dropped,
        ...
        public override void ProcessEvent(WebBaseEvent eventRaised)
        {
            ...
            else
            {
                Debug.Trace(
                    "SqlWebEventProvider",
                    "Writing event to SQL: event=" + eventRaised.GetType().Name
                );
