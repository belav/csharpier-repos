...
namespace System.Web.Management
{
    ...
    public sealed class EventLogWebEventProvider : WebEventProvider, IInternalWebEventProvider
    {
        ...
        void AddWebProcessInformationDataFields(
        ...
        void AddExceptionDataFields(ArrayList dataFields, Exception exception)
        {
            ...
            else
            {
                ...
                for (
                    int totalLength = 0;
                    totalLength < MAX_CHARS_IN_EXCEPTION_MSG && exception != null;
        ...
        void AddWebThreadInformationDataFields(
            ArrayList dataFields,
        ...
        public override void ProcessEvent(WebBaseEvent eventRaised)
        {
            Debug.Trace(
                ...
                "ProcessEvent: event=" + eventRaised.GetType().Name
            );
            ...
            {
                AddWebProcessInformationDataFields(
                    dataFields,
                    ((WebManagementEvent)eventRaised).ProcessInformation
                );
            }

            if (eventRaised is WebHeartbeatEvent)
            {
                AddWebProcessStatisticsDataFields(
                    dataFields,
                    ((WebHeartbeatEvent)eventRaised).ProcessStatistics
                );
            }

            if (eventRaised is WebRequestEvent)
            {
                AddWebRequestInformationDataFields(
                    dataFields,
            ...
            if (eventRaised is WebRequestErrorEvent)
            {
                AddWebRequestInformationDataFields(
                    dataFields,
                    ((WebRequestErrorEvent)eventRaised).RequestInformation
                );
                AddWebThreadInformationDataFields(
                    dataFields,
                    ((WebRequestErrorEvent)eventRaised).ThreadInformation
                );
            }
            ...
            if (eventRaised is WebErrorEvent)
            {
                AddWebRequestInformationDataFields(
                    dataFields,
                    ((WebErrorEvent)eventRaised).RequestInformation
                );
                AddWebThreadInformationDataFields(
                    dataFields,
                    ((WebErrorEvent)eventRaised).ThreadInformation
                );
            }
            ...
            {
                dataFields.Add(
                    ((WebAuthenticationSuccessAuditEvent)eventRaised).NameToAuthenticate
                );
            }
            ...
            {
                dataFields.Add(
                    ((WebAuthenticationFailureAuditEvent)eventRaised).NameToAuthenticate
                );
            }

            if (eventRaised is WebViewStateFailureAuditEvent)
            {
                AddViewStateExceptionDataFields(
                    dataFields,
                    ((WebViewStateFailureAuditEvent)eventRaised).ViewStateException
                );
            }
            ...
            for (int i = 0; i < dataFields.Count; i++)
            {
                ...
                if (len > EventLogParameterMaxLength)
                {
                    ...
                    dataFields[i] =
                        ((string)field).Substring(0, _maxTruncatedParamLen) + _truncateWarning;
                }
            }

#if !FEATURE_PAL // FEATURE_PAL does not enable IIS-based hosting features
            hr = UnsafeNativeMethods.RaiseEventlogEvent(
                (int)eventType,
                (string[])dataFields.ToArray(typeof(string)),
                dataFields.Count
            );
            if (hr != 0)
            {
