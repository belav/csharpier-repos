    ...
    */
    ...
    public abstract class MailWebEventProvider : BufferedWebEventProvider
    {
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (
                String.IsNullOrEmpty(_to) && String.IsNullOrEmpty(_cc) && String.IsNullOrEmpty(_bcc)
            )
            {
                throw new ConfigurationErrorsException(
                    SR.GetString(
                        SR.MailWebEventProvider_no_recipient_error,
            ...
            ProviderUtil.GetAndRemoveNonZeroPositiveOrInfiniteAttribute(
                ...
                name,
                ref _maxMessagesPerNotification
            );
        ...
        internal string GenerateSubject(
        ...
        )
        {
            ...
            if (count == 1)
            {
                return HttpUtility.HtmlEncode(
                    SR.GetString(
                        SR.WebEvent_event_email_subject,
                        new string[]
                        {
            ...
            else
            {
                return HttpUtility.HtmlEncode(
                    SR.GetString(
                        SR.WebEvent_event_group_email_subject,
                        new string[]
                        {
                            notificationSequence.ToString(CultureInfo.InstalledUICulture),
                            messageSequence.ToString(CultureInfo.InstalledUICulture),
        ...
        internal void SendMail(MailMessage msg)
        {
            ...
            {
                throw new HttpException(SR.GetString(SR.MailWebEventProvider_cannot_send_mail), e);
            }
        }
        ...
        public override void ProcessEventFlush(WebEventBufferFlushInfo flushInfo)
        {
            ...
            if (eventsInNotification > MaxEventsPerMessage)
            {
                ...
                {
                    eventsToBeDiscarded =
                        eventsInNotification - MaxMessagesPerNotification * MaxEventsPerMessage;
            ...
            for (int eventsSent = 0; eventsSent < eventsInNotification; messageSequence++)
            {
                if (split)
                {
                    int chunkSize = Math.Min(
                        MaxEventsPerMessage,
                        eventsInNotification - eventsSent
                    );
                ...
                Debug.Trace(
                    "MailWebEventProvider",
                    "Calling SendMessageInternal; # of events: " + eventsToSend.Count
                );
                ...
                if (fatalError)
                {
                    Debug.Trace(
                        "MailWebEventProvider",
                        "Stop sending because we hit a fatal error"
                    );
                    break;
                }
