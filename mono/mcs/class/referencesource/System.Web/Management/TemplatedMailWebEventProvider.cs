...
namespace System.Web.Management
{
    ...
    public sealed class TemplatedMailWebEventProvider
            ...
            IInternalWebEventProvider
    {
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (_templateUrl == null)
            {
                throw new ConfigurationErrorsException(
                    SR.GetString(SR.Provider_missing_attribute, "template", name)
                );
            }
            ...
            {
                throw new ConfigurationErrorsException(
                    SR.GetString(SR.Invalid_provider_attribute, "template", name, _templateUrl)
                );
            }
            ...
            if (!UrlPath.IsRelativeUrl(_templateUrl))
            {
                throw new ConfigurationErrorsException(
                    SR.GetString(
                        SR.Invalid_mail_template_provider_attribute,
                        "template",
                        name,
                        _templateUrl
                    )
        ...
        )
        {
            ...
            try
            {
                TemplatedMailErrorFormatterGenerator gen = new TemplatedMailErrorFormatterGenerator(
                    events.Count + eventsRemaining,
                    _detailedTemplateErrors
                );
                ...
                if (fatalError)
                {
                    msg.Subject = HttpUtility.HtmlEncode(
                        SR.GetString(
                            SR.WebEvent_event_email_subject_template_error,
        ...
        {
            get { return (MailEventNotificationInfo)CallContext.GetData(CurrentEventsName); }
        }
    }
    ...
    internal class TemplatedMailCompileErrorFormatter : DynamicCompileErrorFormatter
    {
        ...
        protected override string ErrorTitle
        {
            ...
            {
                return SR.GetString(
                    SR.MailWebEventProvider_template_compile_error,
    ...
    internal class TemplatedMailRuntimeErrorFormatter : UnhandledErrorFormatter
    {
        ...
        internal TemplatedMailRuntimeErrorFormatter(
            Exception e,
            int eventsRemaining,
            bool showDetails
        )
        ...
        protected override string ErrorTitle
        {
            get
            {
                ...
                {
                    return SR.GetString(
                        SR.MailWebEventProvider_template_file_not_found_error,
                        _eventsRemaining.ToString(CultureInfo.InstalledUICulture)
                    );
                }
                else
                {
                    return SR.GetString(
                        SR.MailWebEventProvider_template_runtime_error,
                        _eventsRemaining.ToString(CultureInfo.InstalledUICulture)
                    );
                }
            }
