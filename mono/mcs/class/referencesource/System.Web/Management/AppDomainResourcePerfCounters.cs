...
namespace System.Web.Management
{
    ...
    internal class AppDomainResourcePerfCounters
    {
        ...
        internal static void Init()
        {
            ...
            lock (_InitLock)
            {
                ...
                if (AppDomain.MonitoringIsEnabled)
                {
                    ...
                    _Timer = new Timer(
                        (new AppDomainResourcePerfCounters()).TimerCallback,
                        null,
                        NUM_SECONDS_TO_POLL * 1000,
                        NUM_SECONDS_TO_POLL * 1000
                    );
                }
                _fInit = true;
        ...
        private void TimerCallback(Object state)
        {
            if (
                _StopRequested
                || // Stop has been called -- exit immediately
                !AppDomain.MonitoringIsEnabled
                || // Monitoring APIs will throw NotSupportedException if not-enabled
                Interlocked.Exchange(ref _inProgressLock, 1) != 0
            ) // Is some thread currently executing the callback
            {
                return;
            }
            ...
            { // don't bubble up exceptions, since we are on a timer thread
            }
            finally
            {
