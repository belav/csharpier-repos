...
namespace System.Web.Management
{
    ...
    public sealed class RegiisUtility : IRegiisUtility
    {
            ...
            try
            {
                ...
                if (doReg != 0)
                {
                    if (!rs.RegisterAssembly(webAssembly, AssemblyRegistrationFlags.None))
                        exception = Marshal.StringToBSTR(
                            (
                                new Exception(
                                    SR.GetString(
                                        SR.Unable_To_Register_Assembly,
                    ...
                    if (!rs.UnregisterAssembly(webAssembly))
                        exception = Marshal.StringToBSTR(
                            (
                                new Exception(
                                    SR.GetString(
                                        SR.Unable_To_UnRegister_Assembly,
                                        webAssembly.FullName
                                    )
                                )
                            ).ToString()
        ...
        public void RegisterAsnetMmcAssembly(
            int doReg,
            string typeName,
        ...
        )
        {
            ...
            try
            {
                ...
                if (doReg != 0)
                {
                    ...
                    IRegisterCreateITypeLib typeLib = (IRegisterCreateITypeLib)
                        converter.ConvertAssemblyToTypeLib(
                            webAssembly,
                            System.IO.Path.Combine(binaryDirectory, "AspNetMMCExt.tlb"),
                            0,
                            eventHandler
                        );
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////
        ///////////////////////////////////////////////////////////////////////////////////////////////
        public void ProtectedConfigAction(
            long options,
        ...
        private void DoProtectSection(
            string configSection,
            string providerName,
            string appPath,
            string site,
            string location,
            bool useMachineConfig
        )
        {
            ...
            if (section == null) // Throw an error that the section was not found.
                throw new Exception(
                    SR.GetString(SR.Configuration_Section_not_found, configSection)
                );
            section.SectionInformation.ProtectSection(providerName);
            config.Save();
        }

        private void DoUnprotectSection(
            string configSection,
            string appPath,
            string site,
            string location,
        ...
        private ConfigurationSection GetConfigSectionFile(
            string configSection,
            string dirName,
        ...
        )
        {
            ...
            else
            {
                ...
                if (!Directory.Exists(dirName))
                    throw new Exception(
                        SR.GetString(SR.Configuration_for_physical_path_not_found, dirName)
                    );
            }
        ...
        )
        {
            ...
            {
                Debug.Assert(
                    appPath.StartsWith("/", StringComparison.Ordinal),
                    "This check is done in main.cxx in regiis"
                );
            }
            ...
            catch (Exception e)
            {
                ...
                {
                    throw new Exception(
                        SR.GetString(SR.Configuration_for_machine_config_not_found),
                        e
                    );
                }
                else
                {
                    throw new Exception(
                        SR.GetString(
                            SR.Configuration_for_path_not_found,
                            appPath,
                            String.IsNullOrEmpty(site) ? SR.GetString(SR.DefaultSiteName) : site
                        ),
                        e
                    );
                }
        ...
        private void DoKeyCreate(string containerName, string csp, long options, int keySize)
        {
            ...
            uint returnHR = (uint)
                UnsafeNativeMethods.DoesKeyContainerExist(
                    containerName,
                    csp,
                    ((options & DO_RSA_PKU) == 0) ? 1 : 0
                );
            switch (returnHR)
            {
                case 0:
                ...
                case 0x80090016: // Not found -- create it
                    RsaProtectedConfigurationProvider rsaProv = CreateRSAProvider(
                        containerName,
                        csp,
                        options
                    );
        ...
        private void DoKeyAclChange(string containerName, string account, string csp, long options)
        {
            ...
            if ((options & DO_RSA_FULL_ACCESS) != 0)
                flags |= 0x4;
            int returnHR = UnsafeNativeMethods.ChangeAccessToKeyContainer(
                containerName,
                account,
                csp,
                flags
        ...
        private RsaProtectedConfigurationProvider CreateRSAProvider(
        ...
        private static void MakeSureContainerExists(
            string containerName,
            string csp,
            bool machineContainer
        )
        {
            uint returnHR = (uint)
                UnsafeNativeMethods.DoesKeyContainerExist(
