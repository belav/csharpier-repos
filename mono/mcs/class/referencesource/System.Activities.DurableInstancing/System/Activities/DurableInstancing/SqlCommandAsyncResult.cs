...
namespace System.Activities.DurableInstancing
{
    ...
    sealed class SqlCommandAsyncResult : TransactedAsyncResult
    {
        ...
        static readonly RetryErrorCode[] retryErrorCodes =
        {
            new RetryErrorCode(
                -2,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), // SqlError: Timeout
            new RetryErrorCode(
                20,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), // SQL Azure error - a connection failed early in the login process. to SQL Server.
            new RetryErrorCode(
                53,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ...
            new RetryErrorCode(
                64,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), //A transport-level error has occurred when receiving results from the server (TCP Provider, error: 0 - The specified network name is no longer available).
            new RetryErrorCode(121, RetryErrorOptions.RetryBeginOrEnd), //  A transport-level error has occurred
            new RetryErrorCode(
                233,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), // Severed shared memory/named pipe connection drawn from the pool
            ...
            new RetryErrorCode(
                3910,
                RetryErrorOptions.RetryOnBegin | RetryErrorOptions.RetryWhenTransaction
            ...
            new RetryErrorCode(
                8645,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), // A timeout occurred while waiting for memory resources
            new RetryErrorCode(
                8641,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), // Could not perform the operation because the requested memory grant was not available
            new RetryErrorCode(
                10053,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ...
            new RetryErrorCode(
                10054,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), // Severed tcp connection drawn from the pool
            new RetryErrorCode(
                10060,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ...
            new RetryErrorCode(
                10928,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), // SQL Azure error - The limit for the database resource has been reached.
            new RetryErrorCode(
                10929,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), // SQL Azure error - The server is currently too busy to support requests up to the maximum limit.
            new RetryErrorCode(
                40143,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), // SQL Azure error - server encountered error processing the request.
            new RetryErrorCode(
                40197,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), // SQL Azure error - server encountered error processing the request.
            new RetryErrorCode(
                40501,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), // SQL Azure error - server is currently busy.
            new RetryErrorCode(
                40549,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), // SQL Azure error - transaction blocking system calls
            new RetryErrorCode(
                40553,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ), // SQL Azure error - excessive memory usage
            new RetryErrorCode(
                40613,
                RetryErrorOptions.RetryBeginOrEnd | RetryErrorOptions.RetryWhenTransaction
            ) // SQL Azure error - database on server is not available.
        };
        ...
        public SqlCommandAsyncResult(
            ...
            : base(callback, state)
        {
            long openTimeout = Math.Min(
                timeout.Ticks,
                SqlCommandAsyncResult.MaximumOpenTimeout.Ticks
            );
        ...
        {
            SqlCommandAsyncResult SqlCommandAsyncResult = AsyncResult.End<SqlCommandAsyncResult>(
                result
            );
        ...
        static bool ShouldRetryForSqlError(int error, RetryErrorOptions retryErrorOptions)
        {
            ...
            return SqlCommandAsyncResult.retryErrorCodes.Any(
                x =>
                    x.ErrorCode == error
        ...
        bool CompleteExecuteReader(IAsyncResult result)
        {
            ...
            catch (SqlException exception)
            {
                ...
                {
                    if (
                        this.CheckRetryCountAndTimer()
                        && ShouldRetryForSqlError(exception.Number, RetryErrorOptions.RetryOnEnd)
                    )
                    {
                        if (this.EnqueueRetry())
                        {
                            ...
                            {
                                TD.RetryingSqlCommandDueToSqlError(
                                    this.eventTraceActivity,
                                    exception.Number.ToString(CultureInfo.InvariantCulture)
                                );
                            }
                ...
                {
                    if (
                        this.retryCount == maximumRetries
                        && TD.MaximumRetriesExceededForSqlCommandIsEnabled()
                    )
                    {
        ...
        bool EnqueueRetry()
        {
            ...
            if (this.timeoutHelper.RemainingTime().TotalMilliseconds > delay)
            {
                ...
                IOThreadTimer iott = new IOThreadTimer(
                    StartCommandCallback,
                    new SqlCommandAsyncResult(
                        CloneSqlCommand(this.sqlCommand),
                        ...
                        this.retryCount,
                        this.maximumRetries,
                        this.PrepareAsyncCompletion(onRetryCommandCallback),
                        this
                    ),
                    false
                );
        ...
        void StartCommandInternal(bool synchronous)
        {
            if (!this.HasOperationTimedOut())
            {
                try
                {
                    ...
                    {
                        AsyncCallback wrappedCallback = this.PrepareAsyncCompletion(
                            onExecuteReaderCallback
                        );
                        this.sqlCommand.Connection = StoreUtilities.CreateConnection(
                            this.connectionString
                        );
                        if (!this.HasOperationTimedOut())
                        {
            ...
            if (this.HasOperationTimedOut())
            {
                ...
                {
                    TD.TimeoutOpeningSqlConnection(
                        this.eventTraceActivity,
                        this.timeoutHelper.OriginalTimeout.ToString()
                    );
                }
            ...
            this.Complete(
                synchronous,
                new TimeoutException(
                    SR.TimeoutOnSqlOperation(this.timeoutHelper.OriginalTimeout.ToString())
                )
            );
        }
        ...
        class RetryErrorCode
        {
            ...
            public RetryErrorOptions RetryErrorOptions { get; private set; }
        }
    }
}
