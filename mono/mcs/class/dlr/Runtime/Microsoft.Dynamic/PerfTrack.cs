...
namespace Microsoft.Scripting
{
    ...
    public static class PerfTrack
    {
        ...
        private static readonly Dictionary<Categories, Dictionary<string, int>> _events =
            MakeEventsDictionary();
        private static readonly Dictionary<Categories, int> summaryStats =
            new Dictionary<Categories, int>();

        private static Dictionary<Categories, Dictionary<string, int>> MakeEventsDictionary()
        {
            Dictionary<Categories, Dictionary<string, int>> result =
                new Dictionary<Categories, Dictionary<string, int>>();
        ...
        )
        {
            foreach (var entry in addend)
            {
                int value;
                result[entry.Key] =
                    entry.Value + (result.TryGetValue(entry.Key, out value) ? value : 0);
            }
        }
        ...
        public static void NoteEvent(Categories category, object key)
        {
            ...
            lock (categoryEvents)
            {
                ...
                int v;
                if (!categoryEvents.TryGetValue(name, out v))
                    categoryEvents[name] = 1;
                else
                    categoryEvents[name] = v + 1;

                if (!summaryStats.TryGetValue(category, out v))
                    summaryStats[category] = 1;
                else
                    summaryStats[category] = v + 1;
            }
        }
    }
}
