...
namespace ICSharpCode.SharpZipLib.Zip
{
    ...
    public class ZipOutputStream : DeflaterOutputStream
    {
        ...
        public void PutNextEntry(ZipEntry entry)
        {
            ...
            // write ZIP version
            WriteLeShort(
                method == CompressionMethod.Stored ? ZIP_STORED_VERSION : ZIP_DEFLATED_VERSION
            );
            if ((flags & 8) == 0)
            {
                WriteLeShort(flags);
...
/* Now write the data descriptor entry if needed. */
        ...
        public override void Finish()
        {
            ...
            foreach (ZipEntry entry in entries)
            {
                ...
                byte[] comment =
                    strComment != null ? ZipConstants.ConvertToArray(strComment) : new byte[0];
                ...
                { // -jr- 17-12-2003 mark entry as directory (from nikolam.AT.perfectinfo.com)
                    WriteLeInt(16);
                }
                else
                {
