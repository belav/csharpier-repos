...
namespace System.Data.OracleClient
{
    ...
    public sealed class OracleParameterCollection : DbParameterCollection
    {
        ...
        {
            get { return (OracleParameter)GetParameter(parameterName); }
        ...
        {
            get { return (OracleParameter)GetParameter(index); }
        ...
        static void AssertParameterValid(object value)
        {
            ...
            string msg = string.Format(
                ...
                "Only non-null {0} instances are valid for " + "the {1}, not {2} instances.",
        ...
        static Exception CreateParameterNullException()
        {
            string msg = string.Format(
                CultureInfo.InvariantCulture,
                "Only non-null {0} instances are valid for " + "{1}.",
                typeof(OracleParameter).Name,
                typeof(OracleParameterCollection).Name
            );
        ...
        static Exception ParameterAlreadyOwnedException()
        {
            string msg = string.Format(
                CultureInfo.InvariantCulture,
                "The specified {0} is already owned by this " + "or another {1}.",
                typeof(OracleParameter).Name,
                typeof(OracleParameterCollection).Name
            );
        ...
        Exception ParameterNotOwnedException()
        {
            throw new ArgumentException(
                string.Format(
                    CultureInfo.InvariantCulture,
                    "An {0} instance that is not contained " + "by this {1} cannot be removed.",
                    typeof(OracleParameter).Name,
                    this.GetType().Name
                )
            );
        ...
        Exception ParameterNotOwnedException(string name)
        {
            throw new IndexOutOfRangeException(
                string.Format(
                    CultureInfo.InvariantCulture,
                    "{0} parameter '{1}' is not contained by " + "this {2}.",
                    typeof(OracleParameter).Name,
                    name,
                    this.GetType().Name
                )
