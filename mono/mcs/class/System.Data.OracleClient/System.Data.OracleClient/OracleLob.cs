...
namespace System.Data.OracleClient
{
    public sealed class OracleLob : Stream, ICloneable, IDisposable, INullable
    {
        ...
        void AssertAmountIsEven(long amount, string argName)
        {
            if (amount % 2 == 1)
                throw new ArgumentOutOfRangeException(
                    "CLOB and NCLOB parameters require even number of bytes for this argument."
                );
        }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            position = offset;
            bytesRead = locator.Read(
                output,
                (uint)Position + 1,
                (uint)count,
                LobType == OracleType.Blob
            );
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            if (offset + count > buffer.Length)
                throw new ArgumentOutOfRangeException(
                    "The offset and count values specified exceed the buffer provided."
                );
            AssertAmountIsValid(offset, "offset");
            AssertAmountIsValid(count, "count");
            AssertTransactionExists();
