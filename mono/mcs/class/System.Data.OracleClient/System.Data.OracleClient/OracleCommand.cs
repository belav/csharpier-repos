...
namespace System.Data.OracleClient
{
    ...
    [Designer(
        "Microsoft.VSDesigner.Data.VS.OracleCommandDesigner, " + Consts.AssemblyMicrosoft_VSDesigner
    )]
    [ToolboxItem(true)]
    public sealed class OracleCommand : DbCommand, ICloneable
    {
        ...
        [Editor(
            "Microsoft.VSDesigner.Data.Oracle.Design.OracleCommandTextEditor, "
            ...
            set
            {
                if (value == CommandType.TableDirect)
                    throw new ArgumentException(
                        "OracleClient provider does not support TableDirect CommandType."
                    );
                commandType = value;
            }
        }
        ...
        private void AssertCommandTextIsSet()
        {
            if (CommandText.Length == 0)
                throw new InvalidOperationException(
                    "The command text for this Command has not been set."
                );
        }
        ...
        private void AssertConnectionIsOpen()
        {
            if (Connection == null || Connection.State == ConnectionState.Closed)
                throw new InvalidOperationException(
                    "An open Connection object is required to continue."
                );
        }
        ...
        internal void DeriveParameters()
        {
            if (commandType != CommandType.StoredProcedure)
                throw new InvalidOperationException(
                    String.Format(
                        "OracleCommandBuilder DeriveParameters only supports CommandType.StoredProcedure, not CommandType.{0}",
                        commandType
                    )
                );
                    ...
                    if (statement.Fetch())
                    {
                        ...
                        if (!defineHandle.IsNull)
                        {
                            switch (defineHandle.DataType)
                            {
                                case OciDataType.Blob:
                                case OciDataType.Clob:
                                    OracleLob lob = (OracleLob)
                                        defineHandle.GetValue(
                                            Connection.SessionFormatProvider,
                                            Connection
                                        );
        ...
        private OciStatementHandle GetStatementHandle()
        {
            ...
            OciStatementHandle h = (OciStatementHandle)
                Connection.Environment.Allocate(OciHandleType.Statement);
        ...
        {
            if (preparedStatement != null)
                OciCalls.OCIHandleFree(preparedStatement, OciHandleType.Statement);
            if (disposing)
                if (Parameters.Count > 0)
                    foreach (OracleParameter parm in Parameters)
                        parm.FreeHandle();
