    ...
    internal abstract class OciHandle : IDisposable
    {
        ...
        internal OciHandle Allocate(OciHandleType type)
        {
            ...
            if (type < OciHandleType.LobLocator)
                status = OciCalls.OCIHandleAlloc(this, out newHandle, type, 0, IntPtr.Zero);
            else
                status = OciCalls.OCIDescriptorAlloc(this, out newHandle, type, 0, IntPtr.Zero);

            if (status != 0 && status != 1)
                throw new Exception(
                    String.Format("Could not allocate new OCI Handle of type {0}", type)
                );
            ...
            if (type < OciHandleType.LobLocator)
            {
                switch (type)
                {
                    ...
                    case OciHandleType.Environment:
                        ...
                        break;
                    default:
                        if (handle != IntPtr.Zero && parent != null && parent.Handle != IntPtr.Zero)
                        {
                            OciCalls.OCIHandleFree(handle, type);
                        }
                        break;
        ...
        internal sbyte GetAttributeSByte(OciAttributeType attrType, OciErrorHandle errorHandle)
        {
            ...
            status = OciCalls.OCIAttrGetSByte(
                Handle,
                HandleType,
                out output,
                IntPtr.Zero,
        ...
        internal byte GetAttributeByte(OciAttributeType attrType, OciErrorHandle errorHandle)
        {
            ...
            status = OciCalls.OCIAttrGetByte(
                Handle,
                HandleType,
                out output,
                IntPtr.Zero,
        ...
        internal ushort GetAttributeUInt16(OciAttributeType attrType, OciErrorHandle errorHandle)
        {
            ...
            status = OciCalls.OCIAttrGetUInt16(
                Handle,
                HandleType,
                out output,
                IntPtr.Zero,
        ...
        internal int GetAttributeInt32(OciAttributeType attrType, OciErrorHandle errorHandle)
        {
            ...
            status = OciCalls.OCIAttrGetInt32(
                Handle,
                HandleType,
                out output,
                IntPtr.Zero,
        ...
        internal static extern int OCIAttrGetRowIdDesc(
            IntPtr trgthndlp,
        ...
        internal OciRowIdDescriptor GetAttributeRowIdDescriptor(
        ...
        internal IntPtr GetAttributeIntPtr(OciAttributeType attrType, OciErrorHandle errorHandle)
        {
            ...
            status = OciCalls.OCIAttrGetIntPtr(
                Handle,
                HandleType,
                out output,
                IntPtr.Zero,
        ...
        internal void SetAttributeString(
            string attribute,
