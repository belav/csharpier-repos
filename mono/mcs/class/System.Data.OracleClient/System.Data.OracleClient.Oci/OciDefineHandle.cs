...
namespace System.Data.OracleClient.Oci
{
    internal sealed class OciDefineHandle : OciHandle, IDisposable
    {
        ...
        void DefineTimeStamp(int position, OracleConnection connection)
        {
            ...
            dateTimeDesc = (OciDateTimeDescriptor)
                connection.Environment.Allocate(OciHandleType.TimeStamp);
            ...
            status = OciCalls.OCIDefineByPosPtr(
                Parent,
                out handle,
                ErrorHandle,
                position + 1,
        ...
        void DefineDate(int position, OracleConnection connection)
        {
            ...
            status = OciCalls.OCIDefineByPos(
                Parent,
                out handle,
                ErrorHandle,
                position + 1,
        ...
        void DefineInterval(int position, OciDataType type, OracleConnection connection)
        {
            ...
            switch (type)
            {
                case OciDataType.IntervalDayToSecond:
                    definedSize = 11;
                    intervalDesc = (OciIntervalDescriptor)
                        connection.Environment.Allocate(OciHandleType.IntervalDayToSecond);
                    break;
                case OciDataType.IntervalYearToMonth:
                    intervalDesc = (OciIntervalDescriptor)
                        connection.Environment.Allocate(OciHandleType.IntervalYearToMonth);
                    definedSize = 5;
                    break;
            }
            ...
            switch (DataType)
            {
                ...
                case OciDataType.Long:
                    ...
                    if (BitConverter.IsLittleEndian)
                        longSize = BitConverter.ToInt32(
                            new byte[] { buffer[0], buffer[1], buffer[2], buffer[3] },
                            0
                        );
                    else
                        longSize = BitConverter.ToInt32(
                            new byte[] { buffer[3], buffer[2], buffer[1], buffer[0] },
                            0
                        );
                ...
                case OciDataType.LongVarRaw:
                    ...
                    int longrawSize = 0;
                    if (BitConverter.IsLittleEndian)
                        longrawSize = BitConverter.ToInt32(
                            new byte[] { buffer[0], buffer[1], buffer[2], buffer[3] },
                            0
                        );
                    else
                ...
                case OciDataType.Clob:
                    return GetOracleLob();
                case OciDataType.IntervalDayToSecond:
                    return new OracleTimeSpan(
                        intervalDesc.GetDayToSecond(conn.Environment, intervalDesc.ErrorHandle)
                    );
                case OciDataType.IntervalYearToMonth:
                    return new OracleMonthSpan(
                        intervalDesc.GetYearToMonth(conn.Environment, intervalDesc.ErrorHandle)
                    );
                default:
