...
namespace System.Data.OracleClient.Oci
{
    internal sealed class OciLobLocator : OciDescriptorHandle, IDisposable
    {
        ...
        {
            int status = 0;
            status = OciCalls.OCILobOpen(Service, ErrorHandle, Handle, (byte)mode);
        ...
        public uint Erase(uint offset, uint amount)
        {
            int status = 0;
            uint output = amount;
            status = OciCalls.OCILobErase(Service, ErrorHandle, this, ref output, (uint)offset);
        ...
        public int GetChunkSize()
        {
            int status = 0;
            uint output;
            status = OciCalls.OCILobGetChunkSize(Service, ErrorHandle, this, out output);
        ...
        public long GetLength(bool binary)
        {
            int status = 0;
            uint output;
            status = OciCalls.OCILobGetLength(Service, ErrorHandle, this, out output);
            if (status != 0)
            {
                OciErrorInfo info = ErrorHandle.HandleError();
                throw new OracleException(info.ErrorCode, info.ErrorMessage);
        ...
        public int Read(byte[] buffer, uint offset, uint count, bool binary)
        {
            ...
            {
                amount /= 2;
                status = OciCalls.OCILobCharSetForm(environment, ErrorHandle, this, out csfrm);
                if (status != 0)
                {
                    OciErrorInfo info = ErrorHandle.HandleError();
                    throw new OracleException(info.ErrorCode, info.ErrorMessage);
        ...
        {
            int status = 0;
            status = OciCalls.OCILobTrim(Service, ErrorHandle, this, newlen);
