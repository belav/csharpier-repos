...
namespace Mono.AppleTls
{
    static class AppleCertificateHelper
    {
        ...
        public static SafeSecIdentityHandle GetIdentity(
            X509Certificate certificate,
            out SafeSecCertificateHandle[] intermediateCerts
        )
        {
            ...
            try
            {
                for (int i = 0; i < intermediateCerts.Length; i++)
                    intermediateCerts[i] = MonoCertificatePal.FromOtherCertificate(
                        impl2.IntermediateCertificates[i]
                    );
        ...
        public static bool InvokeSystemCertificateValidator(
            MNS.ChainValidationHelper validator,
            string targetHost,
            bool serverMode,
        ...
        )
        {
            ...
            using (var trust = new SecTrust(certificates, policy))
            {
                ...
                {
                    var status = trust.SetVerifyDate(
                        validator.Settings.CertificateValidationTime.Value
                    );
                    if (status != SecStatusCode.Success)
                        throw new InvalidOperationException(status.ToString());
                }
