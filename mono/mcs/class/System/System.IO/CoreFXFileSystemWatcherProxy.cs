...
namespace System.IO
{
    internal class CoreFXFileSystemWatcherProxy : IFileWatcher
    {
        ...
        protected void Operation(
            Action<
                IDictionary<object, C>,
                ConditionalWeakTable<object, M>,
                IDictionary<object, object>,
                Handle
            > map_op = null,
            Action<C, M> object_op = null,
            object handle = null,
            Action<C, M> cancel_op = null
        )
        {
            C internal_fsw = null;
            M fsw = null;
            ...
            if (cancel_op != null)
            { // highest priority and must not lock
                havelock = Monitor.TryEnter(instance, INTERRUPT_MS);
                live = (
                    handle != null
                    && (
                        internal_map.TryGetValue(handle, out internal_fsw)
                        || external_map.TryGetValue(handle, out fsw)
                    )
                );
                if (live && havelock)
                    try
        ...
        protected void ProxyDispatch(object sender, FileAction action, FileSystemEventArgs args)
        {
            ...
            Operation(
                map_op: (in_map, out_map, event_map, h) => event_map.TryGetValue(sender, out handle)
            );
        ...
        protected void ProxyDispatchError(object sender, ErrorEventArgs args)
        {
            ...
            Operation(
                map_op: (in_map, out_map, event_map, _) => event_map.TryGetValue(sender, out handle)
            );

            Operation(object_op: (_, fsw) => fsw.DispatchErrorEvents(args), handle: handle);
        }
        ...
        public object NewWatcher(M fsw)
        {
            ...
            Operation(
                map_op: (in_map, out_map, event_map, _) =>
                {
