...
namespace System
{
    static class MonoToolsLocator
    {
        ...
        {
            var gac = typeof(Environment).GetProperty(
                "GacPath",
            ...
            if (Path.DirectorySeparatorChar == '\\')
            {
                ...
                if (!File.Exists(Mono))
                    Mono = Path.Combine(
                        Path.GetDirectoryName(Path.GetDirectoryName(GacPath)),
                        "bin\\mono.exe"
                    );
                ...
                if (!File.Exists(McsCSharpCompiler))
                {
                    ...
                    McsCSharpCompiler = Path.Combine(
            ...
            else
            {
                ...
                McsCSharpCompiler = Path.GetFullPath(
                    Path.Combine(mscorlibPath, "..", "..", "..", "..", "bin", "mcs")
                );
                if (!File.Exists(McsCSharpCompiler))
                    McsCSharpCompiler = "mcs";

                VBCompiler = Path.GetFullPath(
                    Path.Combine(mscorlibPath, "..", "..", "..", "..", "bin", "vbnc")
                );
                if (!File.Exists(VBCompiler))
                    VBCompiler = "vbnc";

                AssemblyLinker = Path.GetFullPath(
                    Path.Combine(mscorlibPath, "..", "..", "..", "..", "bin", "al")
                );
                if (!File.Exists(AssemblyLinker))
                    AssemblyLinker = "al";
            }
