    ...
    public class CodeGeneratorFromExpressionTest
    {
        ...
        public void TypeReferenceExpressionTest()
        {
            ...
            using (StringWriter sw = new StringWriter(sb))
            {
                Assert.AreEqual(
                    "bool",
                    Generate(new CodeTypeReferenceExpression(typeof(bool)), sw),
                    "#1"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "char",
                    Generate(new CodeTypeReferenceExpression(typeof(char)), sw),
                    "#2"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    typeof(DateTime).FullName,
                    Generate(new CodeTypeReferenceExpression(typeof(DateTime)), sw),
                    "#3"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "short",
                    Generate(new CodeTypeReferenceExpression(typeof(short)), sw),
                    "#4"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "int",
                    Generate(new CodeTypeReferenceExpression(typeof(int)), sw),
                    "#5"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "long",
                    Generate(new CodeTypeReferenceExpression(typeof(long)), sw),
                    "#6"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "object",
                    Generate(new CodeTypeReferenceExpression(typeof(object)), sw),
                    "#7"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "void",
                    Generate(new CodeTypeReferenceExpression(typeof(void)), sw),
                    "#8"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "void",
                    Generate(new CodeTypeReferenceExpression((string)null), sw),
                    "#9"
                );
                sb.Length = 0;
                ...
                Assert.AreEqual(
                    "byte",
                    Generate(new CodeTypeReferenceExpression(typeof(byte)), sw),
                    "#11"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "decimal",
                    Generate(new CodeTypeReferenceExpression(typeof(decimal)), sw),
                    "#12"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "double",
                    Generate(new CodeTypeReferenceExpression(typeof(double)), sw),
                    "#13"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "sbyte",
                    Generate(new CodeTypeReferenceExpression(typeof(sbyte)), sw),
                    "#14"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "ushort",
                    Generate(new CodeTypeReferenceExpression(typeof(ushort)), sw),
                    "#15"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "uint",
                    Generate(new CodeTypeReferenceExpression(typeof(uint)), sw),
                    "#16"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "ulong",
                    Generate(new CodeTypeReferenceExpression(typeof(ulong)), sw),
                    "#17"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "float",
                    Generate(new CodeTypeReferenceExpression(typeof(float)), sw),
                    "#18"
                );
                sb.Length = 0;
                sw.Close();
        ...
        public void PrimitiveExpressionTest()
        {
            ...
            using (StringWriter sw = new StringWriter(sb))
            {
                ...
                Assert.AreEqual(
                    "\"AB\\\"C\"",
                    Generate(new CodePrimitiveExpression("AB\"C"), sw),
                    "#2"
                );
                sb.Length = 0;
                ...
                Assert.AreEqual(
                    "434343",
                    Generate(new CodePrimitiveExpression((long)434343), sw),
                    "#7"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "6.445F",
                    Generate(new CodePrimitiveExpression((float)6.445), sw),
                    "#8"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "5.76D",
                    Generate(new CodePrimitiveExpression((double)5.76), sw),
                    "#9"
                );
                sb.Length = 0;
                Assert.AreEqual(
                    "7.667m",
                    Generate(new CodePrimitiveExpression((decimal)7.667), sw),
                    "#10"
                );
                sb.Length = 0;
        ...
        public void PrimitiveExpressionTest_Char()
        {
            ...
            using (StringWriter sw = new StringWriter(sb))
            {
                ...
                Assert.AreEqual(
                    "'\\u2028'",
                    Generate(new CodePrimitiveExpression('\u2028'), sw),
                    "#40"
                );
                sb.Length = 0;
                Assert.AreEqual(
        ...
        public void ParameterDeclarationExpressionTest()
        {
            ...
            using (StringWriter sw = new StringWriter(sb))
            {
                ...
                cpde = new CodeParameterDeclarationExpression((string)null, (string)null);
        ...
        public void ArrayCreateExpressionTest()
        {
                ...
                Assert.AreEqual(
                    ...
                    Generate(
                        new CodeArrayCreateExpression(
                            typeof(int),
                            new CodeExpression[] { new CodePrimitiveExpression(5) }
                        ),
                        sw
                    ),
                    "#1"
            ...
            using (StringWriter sw = new StringWriter(sb))
            {
                Assert.AreEqual(
                    "new int[5]",
                    Generate(
                        new CodeArrayCreateExpression(typeof(int), new CodePrimitiveExpression(5)),
                        sw
                    ),
                    "#2"
                );
        ...
        public void EventReferenceTest()
        {
            ...
            {
                code = Generate(
                    new CodeEventReferenceExpression(new CodeThisReferenceExpression(), null),
                    sw
                );
            ...
            {
                code = Generate(
                    new CodeEventReferenceExpression(new CodeThisReferenceExpression(), "abc"),
                    sw
                );
        ...
        public void DefaultValueExpressionTest()
        {
            ...
            using (StringWriter sw = new StringWriter(sb))
            {
                Assert.AreEqual(
                    "default(string)",
                    Generate(
                        new CodeDefaultValueExpression(new CodeTypeReference(typeof(string))),
                        sw
                    ),
                    "#0"
                );
        ...
        public void DelegateInvokeTest()
        {
            ...
            {
                code = Generate(
                    new CodeDelegateInvokeExpression(null, new CodePrimitiveExpression("abc")),
                    sw
                );
            ...
            using (StringWriter sw = new StringWriter(sb))
            {
                code = Generate(
                    new CodeDelegateInvokeExpression(
                        ...
                        new CodePrimitiveExpression("abc")
                    ),
                    sw
                );
