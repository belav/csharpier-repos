...
namespace MonoTests.System.Text.RegularExpressions
{
    ...
    public class RegexBugs
    {
        ...
        public void BugXamarin12612()
        {
            ...
            const string r_scalar =
                r_open //[
                + r_alphaNumericSpaceString
                + r_close; //]

            const string r_array =
                r_open //[
                + @"("
                + r_scalar
                + @")+"
            ...
            const string r_permissable_Identifier =
                r_Identifier + @"(?<!" + r_non_letter + r_uncased_Header + @")";

            const string r_captured_Identifier = @"(?<Id>" + r_permissable_Identifier + @")";

            const string r_param =
                @"(" + @"(?<p>" + r_permissable_Identifier + r_data + @")" + @")";

            const string r_paramList = r_param + @"*";

            const string r_Encoder_string =
                r_begin
                + r_uncased_Header
                + r_open //[
        ...
        {
            Regex rx = new Regex("([^a-zA-Z_0-9])+", RegexOptions.Compiled);
        ...
        public void bug443841()
        {
            ...
            string doubleString = string.Format(
                @" *[+-]? *{0}(\.{0})?([eE][+-]?{0})? *",
                numberString
            );
            string vector1To3String = string.Format(@"{0}(,{0}(,{0})?)?", doubleString);
            Regex r;
            ...
            r = new Regex(string.Format("^{0}$", vector1To3String), RegexOptions.Compiled);
        ...
        static RegexTrial[] bug80554_trials =
        {
            ...
            new RegexTrial(
                ...
                RegexOptions.None,
                "hydrostatic",
                "Pass. Group[0]=(0,11) Group[1]=(0,5) Group[2]=(5,6)"
            ),
            new RegexTrial(
                bug80554_s,
                RegexOptions.None,
        ...
        static RegexTrial[] trials =
        {
            new RegexTrial(
                ...
                RegexOptions.None,
                "MD 9.18",
                "Pass. Group[0]=(0,7) Group[1]=(3,1) Group[2]=(5,2)"
            ),
            new RegexTrial(
                @"(.*:|.*)(DirName)",
