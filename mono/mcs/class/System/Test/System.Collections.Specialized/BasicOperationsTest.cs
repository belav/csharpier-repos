...
namespace MonoTests.System.Collections.Specialized
{
    ...
    public class BasicOperationsTest
    {
        ...
        public void AddRemoveClearSetGet()
        {
            ...
            Assert.IsTrue(
                (nvc.Get(0).Equals("this")) && (nvc.Get("k1").Equals("this")),
                "Get operation returns wrong result.\n" + FormatForPrinting(nvc)
            );
            ...
            Assert.AreEqual(
                "project,project",
                nvc["k2"],
                "Wrong effect of add(samekey,samevalue)\n" + FormatForPrinting(nvc)
            );
            ...
            Assert.AreEqual(
                4,
                nvc.Count,
                FormatForPrinting(nvc) + "Count is wrong after Add(nvc1)"
            );
            Assert.IsTrue(
                (nvc["k1"].Equals("this,these")) && (nvc["k5"].Equals("!")),
                "Values are wrong after Add(nvc1)"
            );
        ...
        public void UnderHeavyLoad()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                String err =
                    "nvc[\"" + key + "\"]=\"" + val + "\", expected \"" + cache[i + max] + "\"";
