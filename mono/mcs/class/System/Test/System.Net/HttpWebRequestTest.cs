...
namespace MonoTests.System.Net
{
    ...
    public class HttpWebRequestTest
    {
            ...
            foreach (DictionaryEntry de in methods)
            {
                ...
                )
                {
                    ...
                    using (HttpWebResponse resp = (HttpWebResponse)req.GetResponse())
                    {
                        ...
                        sr.Close();
                        Assert.AreEqual(((string)de.Value) + " /test/ HTTP/1.1", line, req.Method);
                        resp.Close();
                    }
                }
            }
        ...
        {
            string[] methods = new string[] { "GET", "HEAD", "CONNECT", "get", "HeAd", "ConNect" };
        ...
        {
            using (
                SocketResponder responder = new SocketResponder(out var ep, s => VerbEchoHandler(s))
            )
            {
        ...
        void TestTimeOut(string url, WebExceptionStatus expectedExceptionStatus)
        {
            ...
            if (!String.IsNullOrEmpty(timeoutWorker.Body))
            {
                ...
                {
                    Assert.Fail(
                        "Should not be reached, timeout exception was not thrown and webrequest managed to retrieve proper body"
                    );
                }
                Assert.Fail(
                    "Should not be reached, timeout exception was not thrown and webrequest managed to retrieve an incorrect body: "
                        + timeoutWorker.Body
                );
            }
            ...
            Assert.IsNotNull(
                webEx,
                "Exception thrown should be WebException, but was: "
            ...
            Assert.AreEqual(
                ...
                webEx.Status,
                "WebException was thrown, but with a wrong status (should be "
                    + expectedExceptionStatus
                    + "): "
                    + webEx.Status
            );
            ...
            Assert.IsFalse(
                timeoutWorker.End
                    > (
                        timeoutWorker.Start
                        + TimeSpan.FromMilliseconds(three_seconds_in_milliseconds + 500)
                    ),
        ...
        internal static byte[] EchoRequestHandler(Socket socket)
        {
            ...
            sw.WriteLine(
                "Content-Length: " + request.Length.ToString(CultureInfo.InvariantCulture)
            );
            sw.WriteLine();
            sw.Write(request);
            sw.Flush();
        ...
        static byte[] NotModifiedSinceHandler(Socket socket)
        {
            ...
            sw.WriteLine(
                "Server: Apache/2.2.6 (Debian) PHP/5.2.6-2+b1 with Suhosin-Patch mod_ssl/2.2.6 OpenSSL/0.9.8g"
            );
        ...
        **/
        ...
        void DoRequest(
            Action<HttpWebRequest, EventWaitHandle> request,
        ...
        class ListenerScope : IDisposable
        {
            ...
            )
            {
                ...
                this.listener = NetworkHelpers.CreateAndStartHttpListener(
                    "http://localhost:",
                    out port,
                    "/",
        ...
        internal static byte[] CookieRequestHandler(Socket socket)
        {
            ...
            sw.WriteLine(
                "Content-Length: " + cookies.Length.ToString(CultureInfo.InvariantCulture)
            );
            sw.WriteLine();
            sw.Write(cookies);
            sw.Flush();
    ...
    static class ExceptionAssert
    {
        ...
        /// ]]></example>
        public static T Throws<T>(Action f)
            where T : Exception
        {
            ...
            if (actualException == null)
                throw new AssertionException(
                    string.Format("No exception thrown. Expected '{0}'", typeof(T).FullName)
                );
