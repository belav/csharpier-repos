...
namespace MonoTests.System.Net.WebSockets
{
    ...
    {
        const string EchoServerUrl =
            "ws://corefx-net-http11.azurewebsites.net/WebSocket/EchoWebSocket.ashx";
        ...
        HttpListener listener
        {
            get
            {
                ...
                return NetworkHelpers.CreateAndStartHttpListener(
                    "http://localhost:",
                    out Port,
                    "/"
                );
...
#pragma warning restore 4014
            try
            {
                Assert.IsTrue(
                    socket
                        .ConnectAsync(new Uri("ws://localhost:" + Port), CancellationToken.None)
                        .Wait(5000)
                );
            }
...
#pragma warning restore 4014
        ...
        {
            Assert.IsTrue(
                socket.ConnectAsync(new Uri(EchoServerUrl), CancellationToken.None).Wait(5000)
            );
            Assert.AreEqual(WebSocketState.Open, socket.State);
            ...
            var resp = socket
                .ReceiveAsync(new ArraySegment<byte>(new byte[0]), CancellationToken.None)
                .Result;
        ...
        public void SendAsyncEndOfMessageTest()
        {
            ...
            SendAsyncEndOfMessageTest(false, WebSocketMessageType.Text, cancellationToken)
                .Wait(5000);
            SendAsyncEndOfMessageTest(true, WebSocketMessageType.Text, cancellationToken)
                .Wait(5000);
            SendAsyncEndOfMessageTest(false, WebSocketMessageType.Binary, cancellationToken)
                .Wait(5000);
            SendAsyncEndOfMessageTest(true, WebSocketMessageType.Binary, cancellationToken)
                .Wait(5000);
        }

        public async Task SendAsyncEndOfMessageTest(
            bool expectedEndOfMessage,
        ...
        )
        {
            using (var client = new ClientWebSocket())
            {
                ...
                var serverReceive = HandleHttpWebSocketRequestAsync<WebSocketReceiveResult>(
                    async socket =>
                ...
                Assert.AreEqual(
                    expectedEndOfMessage,
                    result.EndOfMessage,
                    "EndOfMessage should be " + expectedEndOfMessage
                );
            }
        }

        async Task<T> HandleHttpWebSocketRequestAsync<T>(
            Func<WebSocket, Task<T>> action,
            CancellationToken cancellationToken
        )
        {
        ...
        void ForceSetHeader(WebHeaderCollection headers, string name, string value)
        {
            if (headerSetMethod == null)
                headerSetMethod = typeof(WebHeaderCollection).GetMethod(
                    "AddValue",
