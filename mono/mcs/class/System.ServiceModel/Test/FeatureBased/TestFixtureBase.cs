...
namespace MonoTests.Features
{
    public class Configuration
    {
        static Configuration()
        {
            ...
            endpointBase =
                ConfigurationManager.AppSettings["endpointBase"] ?? $"http://localhost:{port}/";
        ...
        {
            get { return !onlyServers && !onlyClients; }
        }
    }
        ...
        where TServer : new()
    {
        ...
        public void CheckWsdlImpl()
        {
            ...
            try
            {
                ...
                StreamReader _stream = new StreamReader(
                    _assembly.GetManifestResourceStream(
                        "MonoTests.System.ServiceModel.Test.FeatureBased.Features.Contracts."
                            + typeof(TServer).Name
                            + ".xml"
                    )
                );
            ...
            {
                Console.WriteLine(
                    "Couldn't test WSDL of server "
                        + typeof(TServer).Name
                        + " because gold wsdl is not embedded in test !"
                );
                return;
            }
            string currentWsdl = "";
            ...
            Assert.IsTrue(
                comparer.AreEqual(goldWsdl, currentWsdl),
                "Service WSDL does not match gold WSDL"
            );
        }
        ...
        protected virtual TClient InitializeClient()
        {
            ...
            object[] parameters = new object[]
            {
                ...
                new EndpointAddress(getEndpoint()),
            };
        ...
        {
            if (
                !Configuration.onlyClients
                && !Configuration.onlyServers
