    ...
    public class MessageFaultTest
    {
        ...
        public void CreateFault()
        {
            var msg = Message.CreateMessage(
                XmlReader.Create(
                    new StreamReader(
                        TestResourceHelper.GetFullPathOfResource("Test/Resources/soap-fault.xml")
                    )
                ),
                0x10000,
        ...
        {
            var msgVersion = MessageVersion.CreateVersion(
                ...
                XmlReader.Create(
                    new StreamReader(
                        TestResourceHelper.GetFullPathOfResource(
                            "Test/Resources/soap-fault-number.xml"
                        )
                    )
                ),
                0x10000,
        ...
        public void CreateFaultMessageVersionNone()
        {
            var msg = Message.CreateMessage(
                MessageVersion.None,
                new FaultCode("DestinationUnreachable"),
                "typical error",
                null
            );
        ...
        public void CreateFaultIncomplete()
        {
            var msg = Message.CreateMessage(
                XmlReader.Create(
                    new StreamReader(
                        TestResourceHelper.GetFullPathOfResource(
                            "Test/Resources/soap-fault-incomplete.xml"
                        )
                    )
                ),
                0x10000,
        ...
        public void CreateFaultIncomplete4()
        {
            var msg = Message.CreateMessage(
                XmlReader.Create(
                    new StreamReader(
                        TestResourceHelper.GetFullPathOfResource(
                            "Test/Resources/soap-fault-incomplete4.xml"
                        )
                    )
                ),
                0x10000,
