...
namespace System.ServiceModel.Dispatcher
{
    internal class OperationInvokerHandler : BaseRequestProcessorHandler
    {
        ...
        protected override bool ProcessRequest(MessageProcessingContext mrc)
        {
            ...
            {
                mrc.ReplyMessage = BuildExceptionMessage(
                    mrc,
                    ex.InnerException,
        ...
        )
        {
            ...
            FaultCode fc = new FaultCode("InternalServiceFault", req.Version.Addressing.Namespace);
        ...
        void EnsureValid(DispatchOperation operation)
        {
            if (operation.Invoker == null)
                throw new InvalidOperationException(
                    String.Format(
                        "DispatchOperation '{0}' for contract '{1}' requires Invoker.",
                        operation.Name,
                        operation.Parent.EndpointDispatcher.ContractName
                    )
                );
            if (
                (operation.DeserializeRequest || operation.SerializeReply)
                && operation.Formatter == null
            )
                throw new InvalidOperationException(
