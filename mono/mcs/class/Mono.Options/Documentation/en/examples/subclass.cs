...
class DemoOptionSet : OptionSet
{
    ...
    protected override bool Parse(string option, OptionContext c)
    {
        ...
        if (c.Option != null)
        {
            ...
            if (c.Option != null && haveParts)
            {
                if (nextOption == null)
                {
                    ...
                    throw new OptionException(
                        string.Format(
                            "Found option `{0}' as value for option `{1}'.",
                            option,
                            c.OptionName
                        ),
                        c.OptionName
            ...
            {
                if (!option.EndsWith("\\") && c.Option.MaxValueCount == c.OptionValues.Count)
                {
                    c.Option.Invoke(c);
                }
                return true;
        ...
        if (nextOption.OptionValueType != OptionValueType.None && v.EndsWith("\\"))
        {
            c.Option = nextOption;
