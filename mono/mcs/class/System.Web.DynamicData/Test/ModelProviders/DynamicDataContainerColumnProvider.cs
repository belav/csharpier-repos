...
namespace MonoTests.ModelProviders
{
    public class DynamicDataContainerColumnProvider<T> : ColumnProvider
    {
        ...
        public override AssociationProvider Association
        {
            ...
            protected set { base.Association = value; }
        }

        public DynamicDataContainerColumnProvider(
            DynamicDataContainerTableProvider<T> owner,
            DynamicDataColumn column
        )
            : base(owner)
        {
            ...
            if (columnType == null)
                throw new InvalidOperationException(
                    "column.TContext must not be null for column '" + column.Name + "'"
                );

            Name = column.Name;
            ColumnType = columnType;
            Nullable =
                columnType.IsGenericType
                && typeof(Nullable<>).IsAssignableFrom(columnType.GetGenericTypeDefinition());
        ...
        public void ResolveAssociations()
        {
            ...
            if (names.Length != 2)
                throw new ApplicationException(
                    "Only associations of type Table.Column are supported"
                );
            ...
            Association = new DynamicDataAssociationProvider(
                column.AssociationDirection,
                this,
                toColumn
            );
        }
        ...
        PropertyInfo GetPropertyInfo(Type type, string name)
        {
            try
            {
                PropertyInfo ret = type.GetProperties(
