...
public class Driver
{
    ...
    public static void Main(string[] args)
    {
        ...
        if (foundErrors)
            Console.WriteLine(
                "Please check error log at " + Path.Combine(GetErrorPath(), "error.log")
            );
    }
    ...
    static void DownloadWsdls(string host)
    {
        ...
        foreach (ServiceData sd in services.services)
        {
            if (host != null && host != new Uri(sd.Wsdl).Host)
                continue;
    ...
    public static void RetrieveServiceData(ServiceData sd, DiscoveryClientProtocol contract)
    {
        ...
        foreach (object doc in contract.Documents.Values)
            if (doc is ServiceDescription)
                col.Add((ServiceDescription)doc);
    ...
    static ArrayList GetBindingTypes(ServiceDescriptionCollection col)
    {
        ...
        foreach (Service s in doc.Services)
        {
            foreach (Port p in s.Ports)
            {
                ...
                if (
                    bin.Extensions.Find(typeof(System.Web.Services.Description.SoapBinding)) != null
                )
    ...
    static string GetLocation(ServiceDescription doc)
    {
        foreach (Service s in doc.Services)
        {
            foreach (Port p in s.Ports)
            {
                SoapAddressBinding loc = (SoapAddressBinding)
                    p.Extensions.Find(typeof(System.Web.Services.Description.SoapAddressBinding));
                if (loc != null)
                    return loc.Location;
            }
        }
    ...
    static string Normalize(string s)
    {
        ...
        for (int n = 0; n < s.Length; n++)
            if (Char.IsLetterOrDigit(s[n]) || s[n] == '_')
                sb.Append(s[n]);
    ...
    */
    ...
    static void ShowStatus()
    {
        ...
        foreach (ServiceData sd in services.services)
        {
            if (sd.Protocols != null)
            {
                if (Array.IndexOf(sd.Protocols, "Soap") != -1)
                    soap++;
                if (Array.IndexOf(sd.Protocols, "HttpPost") != -1)
                    post++;
                if (Array.IndexOf(sd.Protocols, "HttpGet") != -1)
                    get++;
            }
    ...
    static void BuildProxies(string host)
    {
        ...
        {
            if (host != null && host != new Uri(fd.Wsdl).Host)
                continue;
    ...
    static void BuildProxy(ServiceData fd, bool rebuild, ArrayList proxies, XmlElement errdoc)
    {
        ...
        foreach (string prot in fd.Protocols)
        {
            ...
            proc.StartInfo.Arguments =
                "/out:" + pfile + " /nologo /namespace:" + ns + " /protocol:" + prot + " " + wsdl;
            proc.Start();
            ...
            if (!proc.WaitForExit(30000))
            {
                ...
                WriteError(
                    errdoc,
                    ns,
                    "Errors found while generating " + prot + " proxy for WSDL: " + wsdl,
                    "wsdl.exe timeout"
                );
            }
    ...
    public static void CheckDiff(string diffFile)
    {
            ...
            foreach (string prot in sd.Protocols)
            {
                ...
                XmlElement elem =
                    doc.SelectSingleNode(
                        "assemblies/assembly/namespaces/namespace[@name='" + ns + "']"
                    ) as XmlElement;
                if (elem == null)
                {
                    ign++;
                ...
                if (
                    (
                        elem.GetAttribute("error") != ""
                        || elem.GetAttribute("missing") != ""
                        || elem.GetAttribute("extra") != ""
                    ) && !extrans
                )
                {
                    XmlElement errelem =
                        errdoc.SelectSingleNode("/errors/error[@ns='" + ns + "']") as XmlElement;
                    if (errelem != null)
                    {
                ...
                {
                    if (extrans)
                        Console.WriteLine("BONUS CLASS: " + ns);
                    suc++;
        ...
        Console.WriteLine(
            "Compared:" + tot + " Ignored:" + ign + " Sucess:" + suc + " Fail:" + (tot - suc)
        );
    }
    ...
    static void MsFix(string fileName)
    {
        ...
        while ((line = sr.ReadLine()) != null)
        {
            ...
            XmlElement elem =
                doc.SelectSingleNode(
                    "assemblies/assembly/namespaces/namespace[@name='"
                        + ss[0]
                        + "']/classes/class[@name='"
                        + ss[1]
                        + "']"
    ...
    static void GenerateFiles(string templateFile)
    {
        ...
        XmlElement gvarset = (XmlElement)
            doc.SelectSingleNode("fileGeneration/vars[@name='_global']");

        foreach (XmlElement node in targets)
        {
            string tid = node.GetAttribute("template");
            string file = node.GetAttribute("file");
            XmlElement xtemp = (XmlElement)
                doc.SelectSingleNode("fileGeneration/templates/template[@id='" + tid + "']");
            if (xtemp == null)
                throw new Exception("Template " + tid + " not found");
