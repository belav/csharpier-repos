...
class C
{
    ...
    int TestInstance()
    {
        ...
        if (
            e.Body.ToString()
            != "Convert(Void M().CreateDelegate(EmptyDelegate, value(C)), EmptyDelegate)"
        )
            return 1;
        ...
        if (
            e2.Body.ToString()
            != "Convert(Void M().CreateDelegate(EmptyDelegate, l), EmptyDelegate)"
        )
            return 2;
    ...
    public static int Main()
    {
        ...
        if (
            e.Body.ToString()
            != "Convert(Void Test().CreateDelegate(EmptyDelegate, null), EmptyDelegate)"
        )
            return 1;
        ...
        unsafe
        {
            ...
            if (
                e3.Body.ToString()
                != "Convert(Int32* Foo().CreateDelegate(UnsafeDelegate, null), UnsafeDelegate)"
            )
                return 5;
