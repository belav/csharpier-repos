...
class AssertDynamicObject : DynamicMetaObject
{
    ...
    DynamicMetaObject GetFakeMetaObject(object value)
    {
        ...
        return new DynamicMetaObject(
            Expression.Convert(et.Body, t),
            BindingRestrictions.GetExpressionRestriction(restr)
        );
    }
...
class Tester : DynamicObjectMock
{
    ...
    static void Assert<T>(IList<T> expected, IList<T> value, string name)
    {
        ...
        if (expected.Count != value.Count)
            throw new ApplicationException(
                name + ": Array length does not match " + expected.Count + " != " + value.Count
            );
        ...
        for (int i = 0; i < expected.Count; ++i)
        {
            if (!EqualityComparer<T>.Default.Equals(expected[i], value[i]))
                throw new ApplicationException(
                    name + ": Index " + i + ": " + expected[i] + " != " + value[i]
                );
        }
    }

    static PropertyInfo flags = typeof(CSharpArgumentInfo).GetProperty(
        "Flags",
    ...
    {
        var ai = obj.GetType()
            .GetField("_argumentInfo", BindingFlags.NonPublic | BindingFlags.Instance);
        if (ai == null)
            throw new ApplicationException(
                "Could not find 'argumentInfo' private field on " + obj.GetType()
            );

        IList<CSharpArgumentInfo> values = (IList<CSharpArgumentInfo>)ai.GetValue(obj);
        if (values.Count != expected.Length)
            throw new ApplicationException(
                name + ": Array length does not match " + values.Count + " != " + expected.Length
            );
