...
struct S
{
    ...
    {
        return new S() { Value = a.Value + b.Value };
    }
}
...
class Base
{
    ...
    {
        get { return field_this; }
        set { field_this += value * arg; }
    }
    ...
    {
        return new Base() { field_int = 100 };
    }

    public static Base operator |(Base a, Base b)
    {
        return new Base() { field_int = 200 };
    }
...
class Tester : Base
{
    ...
    async Task<bool> ArrayAccessTest_5()
    {
        ...
        int index = 1;
        CallRefParams(
            ref a[await Task.Factory.StartNew(() => index++)][0],
    ...
    {
        int a = await Task.Factory.StartNew(() => 1),
            b = await Task.Factory.StartNew(() => 2);
        if (a != 1)
            return 1;
        if (b != 2)
            return b;
    ...
    {
        var r =
            await Task.Factory.StartNew<bool?>(() => true)
            & await Task.Factory.StartNew<bool?>(() => null);
        if (r != null)
            return 1;
    ...
    {
        var r1 =
            await Task.Factory.StartNew<short?>(() => 2)
            * await Task.Factory.StartNew<byte?>(() => null);
        if (r1 != null)
            return 1;

        var r2 =
            await Task.Factory.StartNew<decimal?>(() => 100)
            / await Task.Factory.StartNew<decimal?>(() => null);
        if (r2 != null)
            return 2;
    ...
    {
        int value = 9;
        return CallNamed(b: await Task.Factory.StartNew(() => value++), a: value) - 1;
    }
    ...
    {
        var r =
            await Task.Factory.StartNew<string>(() => null)
    ...
    {
        var r =
            await Task.Factory.StartNew<short?>(() => null)
            ?? await Task.Factory.StartNew<byte>(() => 2);
        return r == 2;
    }
    ...
    async Task<int> ConditionalTest_3()
    {
        int v = 5;
        return v
            * (await Task.Factory.StartNew(() => true) ? 0 : await Task.Factory.StartNew(() => 1));
    }
    ...
    {
        var r =
            await Task.Factory.StartNew(() => new Base())
            && await Task.Factory.StartNew(() => new Base());
    ...
    {
        var s = new S(
            await Task.Factory.StartNew(() => 77),
    ...
    async Task<int> NewInitTest_2()
    {
        ...
        s = new S() { Value = await Task.Factory.StartNew(() => 13) };
    ...
    {
        int a = 2,
            b = await Task.Factory.StartNew(() => 1),
            c = await Task.Factory.StartNew(() => 1);
        return a == (b + c);
    }
    ...
    {
        var tests =
            from test in typeof(Tester).GetMethods(
                BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.DeclaredOnly
            )
            where
                test.GetParameters().Length == 0
                && !test.IsDefined(typeof(CompilerGeneratedAttribute), false)
            orderby test.Name
            select RunTest(test);
