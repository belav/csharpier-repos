...
class Tester
{
    async Task<T> NewInitTestGen<T>()
        where T : struct, IFoo
    {
        var s = new T() { Value = await Task.Factory.StartNew(() => 13).ConfigureAwait(false) };
    ...
    {
        var s = new S2 { await Task.FromResult(1), await Task.Factory.StartNew(() => 2) };
