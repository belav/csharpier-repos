...
using Microsoft.JSInterop.Infrastructure;using static Microsoft.AspNetCore.Internal.LinkerFlags;
...
public abstract partial class JSRuntime : IJSRuntime, IDisposable
{
    ...
    > _cancellationRegistrations = new();
    ...
    [UnconditionalSuppressMessage(
        "Trimming",
        "IL2026",
        Justification = "We expect application code is configured to ensure JS interop arguments are linker friendly."
    )]
    ...
    protected virtual void BeginInvokeJS(
        long taskId,
        string identifier,
    ...
    protected internal virtual void SendByteArray(int id, byte[] data)
    {
        throw new NotSupportedException(
            "JSRuntime subclasses are responsible for implementing byte array transfer to JS."
        );
    }
    ...
    protected internal virtual void ReceiveByteArray(int id, byte[] data)
    {
        ...
        if (id != ByteArraysToBeRevived.Count)
        {
            throw new ArgumentOutOfRangeException(
                $"Element id '{id}' cannot be added to the byte arrays to be revived with length '{ByteArraysToBeRevived.Count}'.",
                innerException: null
            );
        }
    ...
    protected internal virtual Task TransmitStreamAsync(
        long streamId,
