...
public interface IJSObjectReference : IAsyncDisposable
{
    ...
    // <see cref="JSRuntime.DefaultAsyncTimeout"/>. To dispatch a call with a different, or no timeout,
    ...
    // value.</returns>
    ValueTask<TValue> InvokeAsync<[DynamicallyAccessedMembers(JsonSerialized)] TValue>(
        string identifier,
        object?[]? args
    );
