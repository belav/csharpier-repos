...
internal class NullParser<TRequestHandler> : IHttpParser<TRequestHandler>
    where TRequestHandler : struct, IHttpHeadersHandler, IHttpRequestLineHandler
{
    ...
    private readonly byte[] _acceptHeaderValue = Encoding.ASCII.GetBytes(
    ...
    public static readonly NullParser<Http1ParsingHandler> Instance =
        new NullParser<Http1ParsingHandler>();
    ...
    public bool ParseHeaders(TRequestHandler handler, ref SequenceReader<byte> reader)
    {
        ...
        handler.OnHeader(
            new Span<byte>(_connectionHeaderName),
            new Span<byte>(_connectionHeaderValue)
        );
