...
namespace Microsoft.AspNetCore.Server.Kestrel.Core.Tests
{
    public class Http2TimeoutTests : Http2TestBase
    {
        ...
        public async Task Preamble_NotReceivedInitially_WithinKeepAliveTimeout_ClosesConnection()
        {
            ...
            _connectionTask = _connection.ProcessRequestsAsync(
                new DummyApplication(_noopApplication)
            );
        ...
        public async Task HEADERS_NotReceivedAfterFirstRequest_WithinKeepAliveTimeout_ClosesConnection()
        {
            ...
            _mockTimeoutControl.Verify(
                c => c.SetTimeout(It.IsAny<long>(), TimeoutReason.KeepAlive),
                Times.Once
            );
            _mockTimeoutHandler.Verify(h => h.OnTimeout(It.IsAny<TimeoutReason>()), Times.Never);
            ...
            var setTimeoutTcs = new TaskCompletionSource(
                TaskCreationOptions.RunContinuationsAsynchronously
            );
            _mockTimeoutControl.Setup(c => c.SetTimeout(It.IsAny<long>(), TimeoutReason.KeepAlive))
                .Callback<long, TimeoutReason>(
                    (t, r) =>
                    {
            ...
            _mockTimeoutControl.Verify(
                c => c.SetTimeout(It.IsAny<long>(), TimeoutReason.RequestHeaders),
                Times.Once
            );
        ...
        public async Task PING_WithinKeepAliveTimeout_ResetKeepAliveTimeout()
        {
            ...
            _mockTimeoutControl.Verify(
                c => c.ResetTimeout(It.IsAny<long>(), TimeoutReason.KeepAlive),
                Times.Never
            );
        ...
        public async Task ResponseDrain_SlowerThanMinimumDataRate_AbortsConnection()
        {
            ...
            _mockTimeoutHandler.Verify(h => h.OnTimeout(It.IsAny<TimeoutReason>()), Times.Never);
            _mockConnectionContext.Verify(
                c => c.Abort(It.IsAny<ConnectionAbortedException>()),
                Times.Never
            );
            ...
            async Task AdvanceClockAndSendFrames()
            {
                ...
                while (!closed)
                {
                    ...
                    mockSystemClock.UtcNow +=
                        Constants.RequestBodyDrainTimeout + TimeSpan.FromTicks(1);
                    ...
                    switch (finalFrameType)
                    {
                        ...
                        case Http2FrameType.CONTINUATION:
                            await SendContinuationAsync(
                                1,
                                Http2ContinuationFrameFlags.NONE,
                                new byte[0]
                            );
                            break;
        ...
        public async Task DATA_Sent_TooSlowlyDueToSocketBackPressureOnSmallWrite_AbortsConnectionAfterGracePeriod()
        {
            ...
            AdvanceClock(
                TimeSpan.FromSeconds(
                    (_bytesReceived + _helloWorldBytes.Length)
            ...
            Assert.True(
                (await _pair.Application.Input.ReadAsync().AsTask().DefaultTimeout()).IsCompleted
            );
        ...
        public async Task DATA_Sent_TooSlowlyDueToSocketBackPressureOnLargeWrite_AbortsConnectionAfterRateTimeout()
        {
            ...
            var timeToWriteMaxData =
                TimeSpan.FromSeconds(
                    (_bytesReceived + _maxData.Length) / limits.MinResponseDataRate.BytesPerSecond
                )
                + limits.MinResponseDataRate.GracePeriod
                + Heartbeat.Interval
                - TimeSpan.FromSeconds(.5);
