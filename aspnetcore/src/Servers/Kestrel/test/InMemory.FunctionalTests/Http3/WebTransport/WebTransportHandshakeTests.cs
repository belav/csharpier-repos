...
public class WebTransportHandshakeTests : Http3TestBase
{
    ...
    public async Task WebTransportHandshake_ClientToServerPasses()
    {
        ...
        var appCompletedTcs = new TaskCompletionSource<bool>(
...
#pragma warning disable CA2252 // WebTransport is a preview feature
            ...
            try
            {
                var session = await webTransportFeature
                    ...
                    .DefaultTimeout(); // todo session is null here
...
#pragma warning restore CA2252
        ...
        Assert.Equal(
            ...
            Convert.ToInt32(response2[InternalHeaderNames.Status], null)
        );
    ...
    [InlineData(
        ...
        nameof(InternalHeaderNames.Method),
        "GET", // incorrect method (verifies that webtransport doesn't break regular Http/3 get)
        nameof(InternalHeaderNames.Protocol),
        "webtransport",
        nameof(InternalHeaderNames.Scheme),
        "http",
        nameof(InternalHeaderNames.Path),
        "/",
        nameof(InternalHeaderNames.Authority),
        "server.example.com",
    ...
    public async Task WebTransportHandshake_IncorrectHeadersRejects(
        long error,
    ...
    )
    {
        ...
        {
            headersConnectFrame.Add(
                new KeyValuePair<string, string>(GetHeaderFromName(headers[i]), headers[i + 1])
            );
        }
        ...
        await requestStream.WaitForStreamErrorAsync(
            ...
            GetCoreStringFromName(targetErrorMessage)
        );
    }
    ...
    private static string GetCoreStringFromName(string headerName)
    {
        ...
        {
            nameof(CoreStrings.Http3MissingAuthorityOrPathPseudoHeaders) =>
                CoreStrings.Http3MissingAuthorityOrPathPseudoHeaders,
            nameof(CoreStrings.Http3MethodMustBeConnectWhenUsingProtocolPseudoHeader) =>
                CoreStrings.Http3MethodMustBeConnectWhenUsingProtocolPseudoHeader,
