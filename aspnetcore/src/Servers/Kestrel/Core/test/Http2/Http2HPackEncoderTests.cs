...
public class Http2HPackEncoderTests
{
    ...
    public void BeginEncodeHeaders_MaxHeaderTableSizeExceeded_EvictionsToFit()
    {
        ...
        enumerator.Initialize(headers);
        Assert.True(
            HPackHeaderWriter.BeginEncodeHeaders(307, hpackEncoder, enumerator, buffer, out length)
        );
        ...
        enumerator.Initialize(headers);
        Assert.True(
            HPackHeaderWriter.BeginEncodeHeaders(200, hpackEncoder, enumerator, buffer, out length)
        );
    ...
    public static TheoryData<KeyValuePair<string, string>[], byte[], int?> SinglePayloadData
    {
        get
        {
            ...
            data.Add(
                new[] { new KeyValuePair<string, string>("CustomHeader", "CustomValue") },
                new byte[]
                {
                    //      12     c     u     s     t     o     m
                    0x40,
    ...
    )
    {
        ...
        {
            Assert.True(
                expectedPayload[i] == payload[i],
                $"{expectedPayload[i]} != {payload[i]} at {i} (len {length})"
            );
        }
    ...
    public void EncodesHeadersInMultiplePayloadsWhenSpaceNotAvailable(bool exactSize)
    {
        ...
        var expectedStatusCodePayload = new byte[] { 0x88 };
        ...
        var sliceLength =
            expectedStatusCodePayload.Length
            + (exactSize ? 0 : expectedDateHeaderPayload.Length - 1);
        Assert.False(
            HPackHeaderWriter.BeginEncodeHeaders(
                statusCode,
                hpackEncoder,
                headerEnumerator,
                payload.Slice(offset, sliceLength),
                out var length
            )
        );
        ...
        sliceLength =
            expectedDateHeaderPayload.Length
            + (exactSize ? 0 : expectedContentTypeHeaderPayload.Length - 1);
        Assert.False(
            HPackHeaderWriter.ContinueEncodeHeaders(
                hpackEncoder,
                headerEnumerator,
                payload.Slice(offset, sliceLength),
                out length
        ...
        sliceLength =
            expectedContentTypeHeaderPayload.Length
            + (exactSize ? 0 : expectedServerHeaderPayload.Length - 1);
        Assert.False(
            HPackHeaderWriter.ContinueEncodeHeaders(
                hpackEncoder,
                headerEnumerator,
    ...
    public void BeginEncodeHeaders_MaxHeaderTableSizeUpdated_SizeUpdateInHeaders()
    {
        ...
        Assert.True(
            HPackHeaderWriter.BeginEncodeHeaders(hpackEncoder, enumerator, buffer, out var length)
        );
        ...
        Assert.False(
            integerDecoder.BeginTryDecode(
                (byte)(buffer[0] & ~DynamicTableSizeUpdateMask),
                prefixLength: 5,
                out _
            )
        );
        ...
        enumerator.Initialize(new Dictionary<string, StringValues>());
        Assert.True(
            HPackHeaderWriter.BeginEncodeHeaders(hpackEncoder, enumerator, buffer, out length)
        );
