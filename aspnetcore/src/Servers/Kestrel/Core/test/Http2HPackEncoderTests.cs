...
namespace Microsoft.AspNetCore.Server.Kestrel.Core.Tests
{
    public class Http2HPackEncoderTests
    {
        ...
        )
        {
            ...
            Assert.True(
                HPackHeaderWriter.BeginEncodeHeaders(hpackEncoder, enumerator, buffer, out _)
            );
        ...
        public void BeginEncodeHeaders_HeaderExceedHeaderTableSize_NoIndexAndNoHeaderEntry()
        {
            ...
            headers.Append(
                "x-Custom",
                new string('!', (int)Http2PeerSettings.DefaultHeaderTableSize)
            );
        ...
        )
        {
            var groupedHeaders = headers.GroupBy(k => k.Key)
                .ToDictionary(
                    g => g.Key,
                    g => new StringValues(g.Select(gg => gg.Value).ToArray())
                );
