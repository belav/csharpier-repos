...
public class KestrelServerTests
{
    ...
    private static (
        IEnumerable<TestApplicationErrorLogger.LogMessage> warnings,
        IEnumerable<TestApplicationErrorLogger.LogMessage> infos
    ) GetNoTlsLogging(HttpProtocols? protocols)
    {
        var testLogger = new TestApplicationErrorLogger();
        var kestrelOptions = new KestrelServerOptions();
        ...
        using (var server = CreateServer(kestrelOptions, testLogger))
        {
            StartDummyApplication(server);
            return (
                testLogger.Messages.Where(log => log.EventId == 64),
                testLogger.Messages.Where(log => log.EventId == 65)
            );
        }
    }
    ...
    public async Task StartWithMultipleTransportFactories_UseSupported()
    {
        ...
        using var server = CreateKestrelServer(
            ...
            {
                transportFactory,
                new NonBindableTransportFactory(),
            },
            Array.Empty<IMultiplexedConnectionListenerFactory>()
        );
    ...
    public async Task StartWithNoValidTransportFactoryThrows_Http3()
    {
        ...
        using var server = CreateKestrelServer(
            ...
            {
                new NonBindableMultiplexedTransportFactory(),
            }
        );
    ...
    public async Task ListenWithCustomEndpoint_DoesNotThrow()
    {
        ...
        var multiplexedTransportEndPoint = Assert.Single(
            mockMultiplexedTransportFactory.BoundEndPoints
        );
    ...
    public async Task StopAsyncCallsCompleteWhenFirstCallCompletes()
    {
        var options = new KestrelServerOptions
        {
            CodeBackedListenOptions = { new ListenOptions(new IPEndPoint(IPAddress.Loopback, 0)) },
        };
    ...
    public async Task StopAsyncDispatchesSubsequentStopAsyncContinuations()
    {
        ...
        var unbindTcs = new TaskCompletionSource(
    ...
    public void StartingServerInitializesHeartbeat()
    {
        ...
        using (
            var server = new KestrelServerImpl(
                new[] { new MockTransportFactory() },
                Array.Empty<IMultiplexedConnectionListenerFactory>(),
                new HttpsConfigurationService(),
                testContext
            )
        )
        {
            ...
            Assert.Equal(
                HeaderUtilities.FormatDate(testContext.FakeTimeProvider.GetUtcNow()),
    ...
    public async Task ReloadsOnConfigurationChangeWhenOptedIn()
    {
        ...
        mockChangeToken
            .Setup(t => t.RegisterChangeCallback(It.IsAny<Action<object>>(), It.IsAny<object>()))
            .Returns<Action<object>, object>(
                (callback, state) =>
                {
                    ...
                    changeCallback = () =>
                    {
                        changeCallbackRegisteredTcs = new TaskCompletionSource(
                            TaskCreationOptions.RunContinuationsAsynchronously
                        );
                        callback(state);
        ...
        mockConfig
            ...
            .Returns<string>(name => currentConfig.GetSection(name));
        ...
        mockTransportFactory.Verify(
            f =>
                f.BindAsync(new IPEndPoint(IPAddress.IPv6Any, 5000), It.IsAny<CancellationToken>()),
            Times.Once
        );
        mockTransportFactory.Verify(
            f =>
                f.BindAsync(new IPEndPoint(IPAddress.IPv6Any, 5001), It.IsAny<CancellationToken>()),
            Times.Once
        );
        mockTransportFactory.Verify(
            f =>
                f.BindAsync(new IPEndPoint(IPAddress.IPv6Any, 5002), It.IsAny<CancellationToken>()),
            Times.Never
        );
        mockTransportFactory.Verify(
            f =>
                f.BindAsync(new IPEndPoint(IPAddress.IPv6Any, 5003), It.IsAny<CancellationToken>()),
            Times.Never
        );
        ...
        foreach (var mockTransport in mockTransports)
        {
            ...
            {
                mockTransport.Verify(
                    t => t.UnbindAsync(It.IsAny<CancellationToken>()),
                    Times.Never
                );
            }
        }
    ...
    )
    {
        return new KestrelServer(
            Options.Create(options),
            new MockTransportFactory(),
            new LoggerFactory(new[] { new KestrelTestLoggerProvider(throwOnCriticalErrors) })
        );
    }
    ...
    {
        server
            .StartAsync(new DummyApplication(context => Task.CompletedTask), CancellationToken.None)
            .GetAwaiter()
            .GetResult();
    }
    ...
    private class MockTransportFactory : IConnectionListenerFactory
    {
        ...
        )
        {
            ...
            if (resolvedEndPoint is IPEndPoint ipEndPoint)
            {
                ...
                resolvedEndPoint = new IPEndPoint(
                    new IPAddress(ipEndPoint.Address.GetAddressBytes()),
                    port
                );
            }
    ...
    private class NonBindableTransportFactory
    ...
    private class NonBindableMultiplexedTransportFactory
