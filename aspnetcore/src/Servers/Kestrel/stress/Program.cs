...
public class Program
{
    ...
    )
    {
        ...
        string contentSource = string
            .Concat(Enumerable.Repeat("1234567890", maxContentLength / 10));
        ...
        void ValidateResponse(HttpResponseMessage m, Version expectedVersion)
        {
            ...
            {
                throw new Exception(
                    $"Expected response version {expectedVersion}, got {m.Version}"
                );
            }
        }
        ...
        void ValidateContent(string expectedContent, string actualContent)
        {
            ...
            {
                throw new Exception(
                    $"Expected response content \"{expectedContent}\", got \"{actualContent}\""
                );
            }
        }
        ...
        var clientOperations = new (string, Func<ClientContext, Task>)[]
        {
            ...
            (
                ...
                async ctx =>
                {
                    ...
                    {
                        using (
                            var req = new HttpRequestMessage(HttpMethod.Get, serverUri + "/abort")
                            {
                                Version = httpVersion
                            }
                        )
        ...
        Console.WriteLine(
            "     .NET Core: "
                + Path.GetFileName(Path.GetDirectoryName(typeof(object).Assembly.Location))
        );
        Console.WriteLine(
            "  ASP.NET Core: "
                + Path.GetFileName(Path.GetDirectoryName(typeof(WebHost).Assembly.Location))
        );
        Console.WriteLine(
            "       Tracing: "
        ...
        Console.WriteLine(
            "      Lifetime: "
                + (connectionLifetime.HasValue ? $"{connectionLifetime}ms" : "(infinite)")
        );
        Console.WriteLine(
        ...
        {
            PooledConnectionLifetime = connectionLifetime.HasValue
                ? TimeSpan.FromMilliseconds(connectionLifetime.Value)
        ...
        using (var client = new HttpClient(handler))
        {
            ...
            long total = 0;
            long[] success = new long[clientOperations.Length],
                fail = new long[clientOperations.Length];
            ...
            Task.WaitAll(Enumerable.Range(0, concurrentRequests).Select(
                    taskNum =>
                        Task.Run(
                            async () =>
                            {
            ...
            for (var i = 0; i < fail.Length; i++)
            {
                ...
                {
                    throw new Exception(
                        "There was a failure in the stress run. See logs for exact time of failure"
                    );
                }
            }
        }
    ...
    private sealed class CancelableContent : HttpContent
    {
        ...
        public CancelableContent(CancellationToken cancellationToken) =>
            ...
            lock (Console.Out)
            {
                if (_writer != null)
                {
                    ...
                    for (int i = 0; i < eventData.Payload.Count; i++)
                    {
                        if (i > 0)
                            sb.Append(", ");
                        sb.Append(eventData.PayloadNames[i])
                            .Append(": ")
                            .Append(eventData.Payload[i]);
                    }
