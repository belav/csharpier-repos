...
internal sealed class Response
{
    ...
    // Trailers are supported on this OS, it's HTTP/2, and the app added a Trailer response header to
    // announce trailers were intended.
    ...
    /// <summary>
    /// Enable kernel caching for the response with the given timeout. Http.Sys determines if the
    // response
    /// can be cached.
    /// </summary>
...
The following flags can be used on calls to HttpSendHttpResponse() and HttpSendResponseEntityBody()
API calls:
...
*/
    ...
    // TODO: Consider using HTTP_SEND_RESPONSE_RAW_HEADER with HttpSendResponseEntityBody instead of
    // calling HttpSendHttpResponse.
    // This will give us more control of the bytes that hit the wire, including encodings, HTTP 1.0,
    // etc..
    ...
    internal uint ComputeHeaders(long writeCount, bool endOfRequest = false)
    {
        ...
        // Http.Sys does not allow us to specify the response protocol version, assume this is a HTTP/1.1
        // response when making decisions.
        ...
        // Determine the body format. If the user asks to do something, let them, otherwise choose a good
        // default for the scenario.
        if (responseContentLength.HasValue)
        {
            ...
            {
                // A single write with the whole content-length. Http.Sys will set the content-length for us in this
                // scenario.
