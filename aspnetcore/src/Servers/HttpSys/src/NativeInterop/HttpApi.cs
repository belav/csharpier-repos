...
internal static unsafe class HttpApi
{
    ...
    internal static extern uint HttpSendHttpResponse(
        ...
        ulong requestId,
        uint flags,
        HTTP_RESPONSE_V2* pHttpResponse,
    ...
    internal static extern uint HttpWaitForDisconnectEx(
        SafeHandle requestQueueHandle,
        ulong connectionId,
        uint reserved,
    ...
    internal static extern uint HttpFindUrlGroupId(
    ...
    internal static extern unsafe uint HttpCreateRequestQueue(
        HTTPAPI_VERSION version,
        string? pName,
        UnsafeNclNativeMethods.SECURITY_ATTRIBUTES? pSecurityAttributes,
        HTTP_CREATE_REQUEST_QUEUE_FLAG flags,
    ...
    internal static extern unsafe uint HttpDelegateRequestEx(
        SafeHandle pReqQueueHandle,
        SafeHandle pDelegateQueueHandle,
        ulong requestId,
        ulong delegateUrlGroupId,
        ulong propertyInfoSetSize,
        HTTP_DELEGATE_REQUEST_PROPERTY_INFO* pRequestPropertyBuffer
    );

    internal delegate uint HttpSetRequestPropertyInvoker(
        SafeHandle requestQueueHandle,
        ulong requestId,
    ...
    private static void InitHttpApi(ushort majorVersion, ushort minorVersion)
    {
        ...
        var statusCode = HttpInitialize(
            version,
            (uint)(HTTP_FLAGS.HTTP_INITIALIZE_SERVER | HTTP_FLAGS.HTTP_INITIALIZE_CONFIG),
            null
        );
