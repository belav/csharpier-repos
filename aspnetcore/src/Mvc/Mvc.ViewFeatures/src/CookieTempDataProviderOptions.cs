...
public class CookieTempDataProviderOptions
{
    private CookieBuilder _cookieBuilder = new CookieBuilder
    {
        ...
        // This cookie has been marked as non-essential because a user could use the
    ...
    // cref="SameSiteMode.Lax"/>. Setting this to
    /// <see cref="SameSiteMode.Strict"/> may cause browsers to not send back the cookie to the server
    // in an
    ...
    /// <item><description><see cref="CookieBuilder.IsEssential"/> defaults to <c>false</c>. This
    // property is only considered when a
    /// user opts into the CookiePolicyMiddleware. If you are using the CookiePolicyMiddleware
    // middleware together with
