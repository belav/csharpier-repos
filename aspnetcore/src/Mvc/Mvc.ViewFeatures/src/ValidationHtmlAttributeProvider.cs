...
{
    /// <summary>
    /// Adds validation-related HTML attributes to the <paramref name="attributes" /> if client
    // validation is
    /// enabled.
    /// </summary>
    ...
    /// The <see cref="Dictionary{TKey, TValue}"/> to receive the validation attributes. Maps the
    // validation
    /// attribute names to their <see cref="string"/> values. Values must be HTML encoded before they
    // are written
    ...
    /// Adds validation-related HTML attributes to the <paramref name="attributes" /> if client
    // validation is
    /// enabled and validation attributes have not yet been added for this <paramref name="expression"/>
    // in the
    /// current &lt;form&gt;.
    /// </summary>
    /// <param name="viewContext">A <see cref="ViewContext"/> instance for the current scope.</param>
    ...
    /// </param>
    /// <remarks>
    /// Tracks the <paramref name="expression"/> in the current <see cref="FormContext"/> to avoid
    // generating
    /// duplicate validation attributes. That is, validation attributes are added only if no previous
    // call has
