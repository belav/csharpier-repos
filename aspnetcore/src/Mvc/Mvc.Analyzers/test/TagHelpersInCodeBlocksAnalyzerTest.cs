...
namespace Microsoft.AspNetCore.Mvc.Analyzers
{
    public class TagHelpersInCodeBlocksAnalyzerTest
    {
        ...
        private MvcDiagnosticAnalyzerRunner Executor { get; } =
            new MvcDiagnosticAnalyzerRunner(new TagHelpersInCodeBlocksAnalyzer());
        ...
        private async Task VerifyDefault(TestSource testSource)
        {
            ...
            var filteredDiagnostics = result.Where(
                diagnostic => diagnostic.Id.StartsWith("MVC", StringComparison.Ordinal)
            );
            Assert.Collection(
                filteredDiagnostics,
                diagnostic =>
