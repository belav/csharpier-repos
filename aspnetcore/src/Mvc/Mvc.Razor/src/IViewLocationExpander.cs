...
/// <summary>
/// Specifies the contracts for a view location expander that is used by <see
// cref="RazorViewEngine"/> instances to
...
/// The populated values are used to determine a cache key - if all values are identical to the last
// time
...
/// <see cref="ExpandViewLocations(ViewLocationExpanderContext, IEnumerable{string})"/> is invoked
// to determine
...
{
    /// <summary>
    /// Invoked by a <see cref="RazorViewEngine"/> to determine the values that would be consumed by
    // this instance
    /// of <see cref="IViewLocationExpander"/>. The calculated values are used to determine if the view
    // location
    /// has changed since the last time it was located.
    /// </summary>
    /// <param name="context">The <see cref="ViewLocationExpanderContext"/> for the current view
    // location
