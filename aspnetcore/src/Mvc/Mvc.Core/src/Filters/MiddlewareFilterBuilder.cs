...
{
    // 'GetOrAdd' call on the dictionary is not thread safe and we might end up creating the pipeline
    // more than
    // once. To prevent this Lazy<> is used. In the worst case multiple Lazy<> objects are created for
    // multiple
    ...
    {
        // Build the pipeline only once. This is similar to how middleware registered in Startup are
        // constructed.
    ...
    private RequestDelegate BuildPipeline(Type middlewarePipelineProviderType)
    {
        ...
        // The middleware resource filter, after receiving the request executes the user configured
        // middleware
        // pipeline. Since we want execution of the request to continue to later MVC layers (resource
        // filters
        // or model binding), add a middleware at the end of the user provided pipeline which make sure to
        // continue
        // this flow.
        // Example:
        // middleware filter -> user-middleware1 -> user-middleware2 -> end-middleware -> resource filters
        // or model binding
        nestedAppBuilder.Run(
            async (httpContext) =>
            {
                ...
                // Ideally we want the experience of a middleware pipeline to behave the same as if it was
                // registered
                // in Startup. In this scenario, an Exception thrown in a middleware later in the pipeline gets
                // propagated back to earlier middleware. So, check if a later resource filter threw an Exception
                // and
