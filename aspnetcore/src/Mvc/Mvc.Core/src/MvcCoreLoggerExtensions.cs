...
internal static class MvcCoreLoggerExtensions
{
    ...
    static MvcCoreLoggerExtensions()
    {
        ...
        _selectingOutputFormatterUsingAcceptHeaderAndExplicitContentTypes = LoggerMessage.Define<
            IEnumerable<MediaTypeSegmentWithQuality>,
            MediaTypeCollection
        >(
            LogLevel.Debug,
    ...
    public static void ExecutingAction(this ILogger logger, ActionDescriptor action)
    {
        if (logger.IsEnabled(LogLevel.Information))
        {
            ...
            for (var i = 0; i < routeValues.Length; i++)
            {
                ...
                {
                    stringBuilder.Append(
                        FormattableString.Invariant($"{routeKeys[i]} = \"{routeValues[i]}\"")
                    );
                }
                else
                {
    ...
    public static void ExceptionFilterShortCircuited(this ILogger logger, IFilterMetadata filter)
    {
