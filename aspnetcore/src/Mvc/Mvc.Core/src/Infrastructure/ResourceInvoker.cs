...
namespace Microsoft.AspNetCore.Mvc.Infrastructure
{
    internal abstract class ResourceInvoker
    {
        ...
        )
        {
            ...
            _actionContext =
                actionContext ?? throw new ArgumentNullException(nameof(actionContext));
            ...
            switch (next)
            {
                ...
                case State.AuthorizationNext:
                {
                    ...
                    if (current.FilterAsync != null)
                    {
                        ...
                        {
                            _authorizationContext = new AuthorizationFilterContextSealed(
                                _actionContext,
                                _filters
                            );
                        }
...
#pragma warning restore CS1998
        ...
        private Task InvokeAlwaysRunResultFilters()
        {
            try
            {
                ...
                while (!isCompleted)
                {
                    var lastTask = ResultNext<IAlwaysRunResultFilter, IAsyncAlwaysRunResultFilter>(
                        ref next,
                        ref scope,
                        ref state,
                        ref isCompleted
            ...
            )
            {
                ...
                while (!isCompleted)
                {
                    await invoker.ResultNext<IAlwaysRunResultFilter, IAsyncAlwaysRunResultFilter>(
                        ref next,
                        ref scope,
                        ref state,
                        ref isCompleted
        ...
        private Task InvokeResultFilters()
        {
            try
            {
                ...
                while (!isCompleted)
                {
                    var lastTask = ResultNext<IResultFilter, IAsyncResultFilter>(
                        ref next,
                        ref scope,
                        ref state,
                        ref isCompleted
            ...
            switch (next)
            {
                ...
                case State.ResultNext:
                {
                    ...
                    if (current.FilterAsync != null)
                    {
                        ...
                        {
                            _resultExecutingContext = new ResultExecutingContextSealed(
                                _actionContext,
                                _filters,
                                _result!,
                                _instance!
            ...
            where TFilterAsync : class, IAsyncResultFilter
        {
            ...
            )
            {
                ...
                {
                    invoker._resultExecutedContext = new ResultExecutedContextSealed(
                        invoker._actionContext,
                        invoker._filters,
                        invoker._result!,
                        invoker._instance!
        ...
        private Task<ResultExecutedContext> InvokeNextResultFilterAwaitedAsync<
            TFilter,
            TFilterAsync
        >()
            where TFilter : class, IResultFilter
