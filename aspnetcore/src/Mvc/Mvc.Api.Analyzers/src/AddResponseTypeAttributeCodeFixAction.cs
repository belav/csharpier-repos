...
/// <summary>
/// A <see cref="CodeAction"/> that adds one or more <c>ProducesResponseType</c> attributes on the
// action.
/// 1) It get status codes from ProducesResponseType, ProducesDefaultResponseType, and conventions
// applied to the action to get the declared metadata.
...
///     c) the return type wasn't the error type (specified by ProducesErrorResponseType or implicit
// ProblemDetails)
...
internal sealed class AddResponseTypeAttributeCodeFixAction : CodeAction
{
    ...
    )
    {
        ...
        return SyntaxFactory.Attribute(
            SyntaxFactory
                .ParseName(ApiSymbolNames.ProducesResponseTypeAttribute)
    ...
    )
    {
        if (context.StatusCodeConstants.TryGetValue(statusCode, out var constantName))
        {
            ...
            return SyntaxFactory.MemberAccessExpression(
                ...
                SyntaxFactory
                    .ParseTypeName(ApiSymbolNames.HttpStatusCodes)
                    .WithAdditionalAnnotations(Simplifier.Annotation),
    ...
    private static AttributeSyntax CreateProducesDefaultResponseTypeAttribute()
    {
        return SyntaxFactory.Attribute(
            SyntaxFactory
                .ParseName(ApiSymbolNames.ProducesDefaultResponseTypeAttribute)
                .WithAdditionalAnnotations(Simplifier.Annotation)
        );
    }
