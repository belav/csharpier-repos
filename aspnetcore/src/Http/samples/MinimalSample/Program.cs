...
inner.AddEndpointFilterFactory(
    (routeContext, next) =>
    {
...
app.MapGet(
    "/todo/{id}",
    Results<Ok<Todo>, NotFound, BadRequest> (int id) =>
        id switch
        {
...
app.MapGet(
    "/todos",
    () => new Todo[] { new Todo(1, "Walk the dog"), new Todo(2, "Come back home") }
);
...
public class TodoBindable : IBindableFromHttpContext<TodoBindable>
{
    ...
    {
        return ValueTask.FromResult<TodoBindable?>(
            new TodoBindable
            {
                Id = 1,
