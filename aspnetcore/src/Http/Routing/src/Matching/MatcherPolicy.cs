...
public abstract class MatcherPolicy
{
    ...
    /// <remarks>
    /// <para>
    /// The presence of <see cref="IDynamicEndpointMetadata"/> signifies that an endpoint that may be
    // replaced
    /// during processing by an <see cref="IEndpointSelectorPolicy"/>.
    /// </para>
    /// <para>
    /// An implementation of <see cref="INodeBuilderPolicy"/> should also implement <see
    // cref="IEndpointSelectorPolicy"/>
    /// and use its <see cref="IEndpointSelectorPolicy"/> implementation when a node contains a dynamic
    // endpoint.
    /// <see cref="INodeBuilderPolicy"/> implementations rely on caching of data based on a static set
    // of endpoints. This
