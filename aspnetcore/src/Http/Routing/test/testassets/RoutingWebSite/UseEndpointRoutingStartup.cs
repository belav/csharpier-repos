...
public class UseEndpointRoutingStartup
{
    ...
    public void Configure(IApplicationBuilder app)
    {
        ...
        app.UseEndpoints(endpoints =>
        {
            ...
            endpoints.MapGet("/plaintext", (httpContext) =>
            {
                var response = httpContext.Response;
            ...
            endpoints.MapGet("/withconstraints/{id:endsWith(_001)}", (httpContext) =>
            {
                var response = httpContext.Response;
                response.StatusCode = 200;
                response.ContentType = "text/plain";
            ...
            endpoints.MapGet("/withoptionalconstraints/{id:endsWith(_001)?}", (httpContext) =>
            {
                var response = httpContext.Response;
                response.StatusCode = 200;
                response.ContentType = "text/plain";
            ...
            endpoints
                .MapGet("/WithSingleAsteriskCatchAll/{*path}", (httpContext) =>
                {
                    var linkGenerator =
            ...
            endpoints
                .MapGet("/WithDoubleAsteriskCatchAll/{**path}", (httpContext) =>
                {
                    var linkGenerator =
                        httpContext.RequestServices.GetRequiredService<LinkGenerator>();
    ...
    )
    {
        ...
        var conventionBuilder = endpoints.MapGet("api/DomainWildcard", httpContext =>
        {
            var response = httpContext.Response;
            response.StatusCode = 200;
            response.ContentType = "text/plain";
    ...
    private void SetupBranch(IApplicationBuilder app, string name)
    {
        ...
        app.UseEndpoints(endpoints =>
        {
            endpoints.MapGet("api/get/{id}", (context) =>
                context.Response.WriteAsync($"{name} - API Get {context.Request.RouteValues["id"]}")
            );
        });
    }
}
