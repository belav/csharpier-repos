...
public abstract partial class RequestDelegateCreationTests
{
    ...
    public async Task MapAction_SingleParsable_StringReturn(
        string typeName,
...
"""
        ...
        if (queryStringInput != null)
        {
            httpContext.Request.QueryString = new QueryString(
                $"?p={UrlEncoder.Default.Encode(queryStringInput)}"
            );
        }
...
""";
        ...
        httpContext.Request.QueryString = new QueryString(
            $"?time={UrlEncoder.Default.Encode(queryStringInput)}"
        );
    ...
    public async Task MapAction_DateTimeOffset_StringReturn(
        string queryStringInput,
        string expectedBody
    )
    {
...
"""
    ...
    [Theory]
    [InlineData(
        "void TestAction(HttpContext httpContext, [FromRoute] MyBindAsyncRecord myBindAsyncRecord) { }"
    )]
    [InlineData(
        "void TestAction(HttpContext httpContext, [FromQuery] MyBindAsyncRecord myBindAsyncRecord) { }"
    )]
