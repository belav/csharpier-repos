...
internal class EndpointParameter
{
    public EndpointParameter(
        ...
        : this(endpoint, parameter.Type, parameter.Name, wellKnownTypes)
    {
        Ordinal = parameter.Ordinal;
        IsOptional = parameter.IsOptional();
        HasDefaultValue = parameter.HasExplicitDefaultValue;
        DefaultValue = parameter.GetDefaultValueString();
        ProcessEndpointParameterSource(
            endpoint,
            parameter,
    ...
    private EndpointParameter(
        ...
        : this(endpoint, property.Type, property.Name, wellKnownTypes)
    {
        ...
        var propertyInfo =
            $"typeof({property.ContainingType.ToDisplayString()})!.GetProperty({SymbolDisplay.FormatLiteral(property.Name, true)})!";
    ...
    )
    {
        ...
        IsEndpointParameterMetadataProvider = ImplementsIEndpointParameterMetadataProvider(
            typeSymbol,
            wellKnownTypes
        );
        endpoint.EmitterContext.HasEndpointParameterMetadataProvider |=
            IsEndpointParameterMetadataProvider;
    ...
    )
    {
        ...
        else if (TryGetExplicitFromJsonBody(symbol, attributes, wellKnownTypes, out var isOptional))
        {
            ...
            attributes.HasAttributeImplementingInterface(
                wellKnownTypes.Get(
        ...
        )
        {
            ...
            attributes.HasAttribute(
                wellKnownTypes.Get(WellKnownType.Microsoft_AspNetCore_Http_AsParametersAttribute)
            )
        )
        {
            ...
            if (
                ...
                || !TryGetAsParametersConstructor(
                    endpoint,
                    namedTypeSymbol,
                    out var isDefaultConstructor,
                    out var matchedProperties
                )
            )
            {
            ...
            if (isDefaultConstructor == true)
            {
                var parameterList = string.Join(
                    ", ",
                    EndpointParameters.Select(p => $"{p.SymbolName} = {p.EmitHandlerArgument()}")
                );
                AssigningCode =
                    $"new {namedTypeSymbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)} {{ {parameterList} }}";
            }
            else
            {
                var parameterList = string.Join(
                    ", ",
                    EndpointParameters.Select(p => p.EmitHandlerArgument())
                );
                AssigningCode =
                    $"new {namedTypeSymbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}({parameterList})";
            }
            ...
            TryGetSpecialTypeAssigningCode(Type, wellKnownTypes, out var specialTypeAssigningCode)
        )
        {
            Source = EndpointParameterSource.SpecialType;
        ...
        else if (HasBindAsync(Type, wellKnownTypes, out var bindMethod, out var bindMethodSymbol))
        {
            ...
            endpoint.EmitterContext.RequiresPropertyAsParameterInfo =
                IsProperty
                && bindMethod
                    is BindabilityMethod.BindAsyncWithParameter
                        or BindabilityMethod.IBindableFromHttpContext;
    ...
    private static bool ImplementsIEndpointParameterMetadataProvider(
        ITypeSymbol type,
        WellKnownTypes wellKnownTypes
    ) =>
        type.Implements(
            wellKnownTypes.Get(
                WellKnownType.Microsoft_AspNetCore_Http_Metadata_IEndpointParameterMetadataProvider
            )
        );
    ...
    private static bool HasBindAsync(
        ITypeSymbol typeSymbol,
        WellKnownTypes wellKnownTypes,
        [NotNullWhen(true)] out BindabilityMethod? bindMethod,
        [NotNullWhen(true)] out IMethodSymbol? bindMethodSymbol
    )
    {
        var parameterType = typeSymbol.UnwrapTypeSymbol(unwrapArray: true, unwrapNullable: true);
        return ParsabilityHelper.GetBindability(
                parameterType,
                wellKnownTypes,
                out bindMethod,
                out bindMethodSymbol
    ...
    private bool TryGetParsability(
        ITypeSymbol typeSymbol,
        WellKnownTypes wellKnownTypes,
        [NotNullWhen(true)] out Action<CodeWriter, string, string>? parsingBlockEmitter
    )
    {
        var parameterType = typeSymbol.UnwrapTypeSymbol(unwrapArray: true, unwrapNullable: true);
        ...
        {
            preferredTryParseInvocation = (string inputArgument, string outputArgument) =>
                $$"""{{parameterType.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}}.TryParse({{inputArgument}}!, CultureInfo.InvariantCulture, DateTimeStyles.AllowWhiteSpaces, out var {{outputArgument}})""";
        }
        ...
        if (IsOptional)
        {
            ...
            {
                writer.WriteLine(
                    $"""{typeSymbol.ToDisplayString(EmitterConstants.DisplayFormat)} {outputArgument} = default;"""
                );
                writer.WriteLine(
        ...
        else
        {
            parsingBlockEmitter = (writer, inputArgument, outputArgument) =>
            {
                if (IsArray && ElementType.NullableAnnotation == NullableAnnotation.Annotated)
                {
                    ...
                    writer.WriteLine("wasParamCheckFailure = true;");
                    writer.WriteLine(
                        $@"logOrThrowExceptionHelper.RequiredParameterNotProvided({SymbolDisplay.FormatLiteral(Type.ToDisplayString(SymbolDisplayFormat.CSharpShortErrorMessageFormat), true)}, {SymbolDisplay.FormatLiteral(SymbolName, true)}, {SymbolDisplay.FormatLiteral(this.ToMessageString(), true)});"
                    );
                    writer.EndBlock();
                    writer.EndBlock();
                }
                else
                {
                    ...
                    writer.StartBlock();
                    writer.WriteLine(
                        $@"logOrThrowExceptionHelper.ParameterBindingFailed({SymbolDisplay.FormatLiteral(Type.ToDisplayString(SymbolDisplayFormat.CSharpShortErrorMessageFormat), true)}, {SymbolDisplay.FormatLiteral(SymbolName, true)}, {inputArgument});"
                    );
                    writer.WriteLine("wasParamCheckFailure = true;");
                    writer.EndBlock();
                    writer.EndBlock();
    ...
    private static bool TryGetSpecialTypeAssigningCode(
        ITypeSymbol type,
        WellKnownTypes wellKnownTypes,
        [NotNullWhen(true)] out string? callingCode
    )
    {
        callingCode = null;
        if (
    ...
    private static bool TryGetExplicitFromJsonBody(
        ISymbol typeSymbol,
        ImmutableArray<AttributeData> attributes,
        WellKnownTypes wellKnownTypes,
        out bool isOptional
    )
    {
        ...
        isOptional |=
            fromBodyAttribute.TryGetNamedArgumentValue<int>(
                "EmptyBodyBehavior",
                out var emptyBodyBehaviorValue
            )
            && emptyBodyBehaviorValue == 1;
        isOptional |=
            fromBodyAttribute.TryGetNamedArgumentValue<bool>("AllowEmpty", out var allowEmptyValue)
            && allowEmptyValue;
    ...
    private static bool TryGetAsParametersConstructor(
        ...
        out bool? isDefaultConstructor,
        [NotNullWhen(true)] out IEnumerable<ConstructorParameter>? matchedProperties
    )
    {
        isDefaultConstructor = null;
        matchedProperties = null;
        var parameterTypeString = type.ToDisplayString(
