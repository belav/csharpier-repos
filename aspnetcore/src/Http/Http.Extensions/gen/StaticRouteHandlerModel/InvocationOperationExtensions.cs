...
internal static class InvocationOperationExtensions
{
    ...
    public static bool TryGetRouteHandlerMethod(
    ...
    public static bool TryGetRouteHandlerArgument(
        this IInvocationOperation invocation,
    ...
    private static IMethodSymbol? ResolveMethodFromOperation(
        ...
        operation switch
        {
            ...
            IFieldReferenceOperation { Field.IsReadOnly: true } f
                when ResolveDeclarationOperation(f.Field, semanticModel) is IOperation op =>
    ...
    )
    {
        foreach (var syntaxReference in symbol.DeclaringSyntaxReferences)
        {
            ...
            if (syn is VariableDeclaratorSyntax { Initializer: { Value: var expr } })
            {
                ...
                var targetSemanticModel = semanticModel?.Compilation
