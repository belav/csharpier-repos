...
{
    // Note that the ETag header does not allow a * but we're not that strict: We allow both '*' and
    // ETag values in a single value.
    ...
    // Note that if multiple ETag values are allowed (e.g. 'If-Match', 'If-None-Match'), according to
    // the RFC
    // the value must either be '*' or a list of ETag values. It's not allowed to have both '*' and a
    // list of
    // ETag values. We're not that strict: We allow both '*' and ETag values in a list. If the server
    // sends such
    ...
    /// </summary>
    /// <param name="tag">A <see cref="StringSegment"/> that contains an <see
    // cref="EntityTagHeaderValue"/>.</param>
    ...
    /// <param name="tag">A <see cref="StringSegment"/> that contains an <see
    // cref="EntityTagHeaderValue"/>.</param>
    /// <param name="isWeak">A value that indicates if this entity-tag header is a weak
    // validator.</param>
    ...
    /// This equality check should not be used to determine if two values match under the RFC
    // specifications (<see href="https://tools.ietf.org/html/rfc7232#section-2.3.2"/>).
    /// </summary>
    ...
    /// <c>false</c> if the other value is null, is not an <see cref="EntityTagHeaderValue"/>, or if
    // there is a mismatch of strength or tag between the two values.
    /// </returns>
    ...
    /// <summary>
    /// Compares against another <see cref="EntityTagHeaderValue"/> to see if they match under the RFC
    // specifications (<see href="https://tools.ietf.org/html/rfc7232#section-2.3.2"/>).
    /// </summary>
    /// <param name="other">The other <see cref="EntityTagHeaderValue"/> to compare against.</param>
    /// <param name="useStrongComparison"><c>true</c> to use a strong comparison, <c>false</c> to use a
    // weak comparison</param>
    /// <returns>
    ...
    // cref="EntityTagHeaderValue"/>.
    /// </summary>
    /// <param name="input">The value to parse.</param>
    /// <param name="parsedValue">The parsed value.</param>
    /// <returns><see langword="true"/> if input is a valid <see cref="EntityTagHeaderValue"/>,
    ...
    /// <summary>
    /// Parses a sequence of inputs as a sequence of <see cref="EntityTagHeaderValue"/> values using
    ...
    /// <returns><see langword="true"/> if all inputs are valid <see cref="EntityTagHeaderValue"/>,
    // otherwise <see langword="false"/>.</returns>
