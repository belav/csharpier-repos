...
public class Program
{
    ...
    static void GenerateClassFile(string classFilePath, int typeArgCount, bool interactive = true)
    {
        ...
        for (int i = 2; i <= typeArgCount; i++)
        {
            ...
            for (int j = 1; j <= i; j++)
            {
                writer.WriteIndentedLine(
                    2,
                    $"ResultsOfTHelper.PopulateMetadataIfTargetIsIEndpointMetadataProvider<TResult{j}>(context);"
                );
            }
            writer.WriteIndentedLine("}");
    ...
    )
    {
        ...
        for (int j = 1; j <= typeArgNumber; j++)
        {
            writer.WriteIndentedLine(
                2,
                $"Assert.Contains(context.EndpointMetadata, m => m is ResultTypeProvidedMetadata {{ SourceTypeName: nameof(ProvidesMetadataResult{j}) }});"
            );
        }
    ...
    static void GenerateTest_PopulateMetadata_Throws_ArgumentNullException_WhenContextIsNull(
        StreamWriter writer,
        int typeArgNumber
    )
    {
        ...
        writer.WriteIndent(
            2,
            "Assert.Throws<ArgumentNullException>(\"context\", () => PopulateMetadata<Results<"
        );
        for (int j = 1; j <= typeArgNumber; j++)
        {
            writer.Write($"ProvidesMetadataResult{j}");
    ...
    static void Generate_ProvidesMetadataResultClass(StreamWriter writer, int typeArgNumber)
    {
        ...
        writer.WriteIndentedLine(
            3,
            $"context.EndpointMetadata.Add(new ResultTypeProvidedMetadata {{ SourceTypeName = nameof(ProvidesMetadataResult{typeArgNumber}) }});"
        );
        writer.WriteIndentedLine(2, "}");
        writer.WriteIndentedLine(1, "}");
    }
