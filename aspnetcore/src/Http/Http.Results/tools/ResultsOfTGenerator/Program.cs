...
public class Program
{
        ...
        if (args.Length > 0)
        {
            if (args.Length != 2)
            {
                throw new ArgumentException(
                    "Invalid number of args specified. Must specify both class file path and test file path if args are passed."
                );
            }
    ...
    static void GenerateClassFile(string classFilePath, int typeArgCount, bool interactive = true)
    {
        ...
        for (int i = 2; i <= typeArgCount; i++)
        {
            ...
            writer.WriteLine(
                $"/// An <see cref=\"IResult\"/> that could be one of {i.ToWords()} different <see cref=\"IResult\"/> types. On execution will"
            );
            writer.WriteLine(
            ...
            {
                writer.WriteLine(
                    @$"/// <typeparam name=""TResult{j}"">The {j.ToOrdinalWords()} result type.</typeparam>"
                );
            }
            ...
            {
                writer.Write(
                    $"[DynamicallyAccessedMembers(ResultsOfTHelper.RequireMethods)] TResult{j}"
                );
                if (j != i)
                {
                    writer.Write(", ");
            ...
            for (int j = 1; j <= i; j++)
            {
                writer.WriteIndentedLine("/// <summary>");
                writer.WriteIndentedLine(
                    $"/// Converts the <typeparamref name=\"TResult{j}\"/> to a <see cref=\"Results{{{typeArgsList}}}\" />."
                );
                writer.WriteIndentedLine("/// </summary>");
                writer.WriteIndentedLine("/// <param name=\"result\">The result.</param>");
                writer.WriteIndentedLine(
                    $"public static implicit operator Results<{typeArgsList}>(TResult{j} result) => new(result);"
                );
            ...
            for (int j = 1; j <= i; j++)
            {
                writer.WriteIndentedLine(
                    2,
                    $"ResultsOfTHelper.PopulateMetadataIfTargetIsIEndpointMetadataProvider<TResult{j}>(method, builder);"
                );
            }
            writer.WriteIndentedLine("}");
    ...
    static void GenerateTestFiles(string testFilePath, int typeArgCount, bool interactive = true)
    {
        ...
        for (int i = 1; i <= typeArgCount; i++)
        {
            ...
            GenerateTest_PopulateMetadata_PopulatesMetadataFromTypeArgsThatImplementIEndpointMetadataProvider(
                writer,
                i
            );
            GenerateTest_PopulateMetadata_Throws_ArgumentNullException_WhenMethodOrBuilderIsNull(
                writer,
                i
            );
        }
    ...
    static void GenerateTest_ExecuteResult_ExecutesAssignedResult(
        StreamWriter writer,
        int typeArgNumber
    )
    {
    ...
    static void GenerateTest_Throws_ArgumentNullException_WhenHttpContextIsNull(
        StreamWriter writer,
        int typeArgNumber
    )
    {
    ...
    static void GenerateTest_Throws_InvalidOperationException_WhenResultIsNull(
        StreamWriter writer,
        int typeArgNumber
    )
    {
    ...
    static void GenerateTest_AcceptsIResult_AsNthTypeArg(
        StreamWriter writer,
        int typeArgCount,
        int typeArgNumber
    )
    {
        ...
        writer.WriteLine(
            $"_AcceptsIResult_As{typeArgNumber.ToOrdinalWords().TitleCase()}TypeArg(int input, Type expectedResultType)"
        );
        writer.WriteIndentedLine("{");
    ...
    static void GenerateTest_AcceptsNestedResultsOfT_AsAnyTypeArg(
        StreamWriter writer,
        int typeArgCount
    )
    {
    ...
    static void GenerateTest_AcceptsNestedResultsOfT_AsNthTypeArg(
        StreamWriter writer,
        int typeArgCount,
        int typeArgNumber
    )
    {
        ...
        writer.WriteLine(
            $"_AcceptsNestedResultsOfT_As{typeArgNumber.ToOrdinalWords().TitleCase()}TypeArg(int input, Type expectedResultType)"
        );
        writer.WriteIndentedLine("{");
        ...
        writer.WriteIndent(2, "Results<");
        writer.WriteLine(
            $"{nestedResultTypeName}, ChecksumResult{typeArgCount + 1}> MyApi(int id)"
        );
        writer.WriteIndentedLine(2, "{");
        writer.WriteIndentedLine(3, "return id switch");
        writer.WriteIndentedLine(3, "{");
        for (int j = 1; j <= typeArgCount; j++)
        {
            writer.WriteIndentedLine(
                4,
                $"{j} => ({nestedResultTypeName})new ChecksumResult{j}({j}),"
            );
        }
        writer.WriteIndentedLine(
            4,
            $"_ => new ChecksumResult{typeArgCount + 1}({typeArgCount + 1})"
        );
        writer.WriteIndentedLine(3, "};");
        writer.WriteIndentedLine(2, "}");
        writer.WriteIndentedLine(2, "var httpContext = GetHttpContext();");
    ...
    static void GenerateTest_PopulateMetadata_PopulatesMetadataFromTypeArgsThatImplementIEndpointMetadataProvider(
        StreamWriter writer,
        int typeArgNumber
    )
    {
        ...
        writer.WriteLine(
            "_PopulateMetadata_PopulatesMetadataFromTypeArgsThatImplementIEndpointMetadataProvider()"
        );
        writer.WriteIndentedLine("{");
        ...
        for (int j = 1; j <= typeArgNumber; j++)
        {
            writer.WriteIndentedLine(
                2,
                $"Assert.Contains(builder.Metadata, m => m is ResultTypeProvidedMetadata {{ SourceTypeName: nameof(ProvidesMetadataResult{j}) }});"
            );
        }
    ...
    static void GenerateTest_PopulateMetadata_Throws_ArgumentNullException_WhenMethodOrBuilderIsNull(
        StreamWriter writer,
        int typeArgNumber
    )
    {
        ...
        writer.WriteLine(
            "_PopulateMetadata_Throws_ArgumentNullException_WhenMethodOrBuilderAreNull()"
        );
        writer.WriteIndentedLine("{");
        ...
        writer.WriteIndent(
            2,
            "Assert.Throws<ArgumentNullException>(\"method\", () => PopulateMetadata<Results<"
        );
        for (int j = 1; j <= typeArgNumber; j++)
        {
            writer.Write($"ProvidesMetadataResult{j}");
        ...
        writer.WriteIndent(
            2,
            "Assert.Throws<ArgumentNullException>(\"builder\", () => PopulateMetadata<Results<"
        );
        for (int j = 1; j <= typeArgNumber; j++)
        {
            writer.Write($"ProvidesMetadataResult{j}");
    ...
    static void Generate_ChecksumResultClass(StreamWriter writer, int typeArgNumber = -1)
    {
        ...
        else
        {
            ...
            writer.WriteIndentedLine(
                2,
                $"public ChecksumResult{typeArgNumber}(int checksum = 0) : base(checksum) {{ }}"
            );
            writer.WriteIndentedLine(1, "}");
        }
    }
    ...
    static void Generate_ProvidesMetadataResultClass(StreamWriter writer, int typeArgNumber)
    {
        ...
        writer.WriteIndentedLine(
            1,
            $"class ProvidesMetadataResult{typeArgNumber} : IResult, IEndpointMetadataProvider"
        );
        writer.WriteIndentedLine(1, "{");
        writer.WriteIndentedLine(
            2,
        ...
        writer.WriteIndentedLine(
            3,
            $"builder.Metadata.Add(new ResultTypeProvidedMetadata {{ SourceTypeName = nameof(ProvidesMetadataResult{typeArgNumber}) }});"
        );
        writer.WriteIndentedLine(2, "}");
        writer.WriteIndentedLine(1, "}");
    }
