...
{
    public const DynamicallyAccessedMemberTypes RequireMethods =
    ...
    public static void PopulateMetadataIfTargetIsIEndpointMetadataProvider<
        [DynamicallyAccessedMembers(RequireMethods)] TTarget
    >(MethodInfo method, EndpointBuilder builder)
    {
        if (typeof(IEndpointMetadataProvider).IsAssignableFrom(typeof(TTarget)))
        {
            ...
            else
            {
                ...
                {
                    populateMetadataMethod = typeof(TTarget).GetMethod(
                        "PopulateMetadata",
        ...
        [UnconditionalSuppressMessage(
            "AOT",
            "IL3050",
            Justification = "Validated with IsDynamicCodeSupported check."
        )]
        [UnconditionalSuppressMessage(
            "ReflectionAnalysis",
