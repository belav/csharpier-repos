...
namespace Microsoft.AspNetCore.Identity.Test
{
    public class SignInManagerTest
    {
        ...
        public async Task CanPasswordSignIn(bool isPersistent)
        {
            ...
            manager.Setup(m => m.CheckPasswordAsync(user, "[PLACEHOLDER]-1a"))
                .ReturnsAsync(true)
                .Verifiable();
        ...
        public async Task PasswordSignInWorksWithNonTwoFactorStore()
        {
            ...
            manager.Setup(m => m.CheckPasswordAsync(user, "[PLACEHOLDER]-1a"))
                .ReturnsAsync(true)
                .Verifiable();
            manager.Setup(m => m.ResetAccessFailedCountAsync(user))
        ...
        )
        {
            ...
            if (tfaEnabled)
            {
                manager.Setup(m => m.GetTwoFactorEnabledAsync(user))
                    .ReturnsAsync(true)
                    .Verifiable();
                manager.Setup(m => m.GetValidTwoFactorProvidersAsync(user))
        ...
        public async Task ExternalSignInRequiresVerificationIfNotBypassed(bool bypass)
        {
            ...
            manager.Setup(m => m.FindByLoginAsync(loginProvider, providerKey))
                .ReturnsAsync(user)
                .Verifiable();
            if (!bypass)
            {
        ...
        public async Task CanTwoFactorRecoveryCodeSignIn(bool supportsLockout, bool externalLogin)
        {
            ...
            var manager = SetupUserManager(user);
            manager.Setup(m => m.SupportsUserLockout).Returns(supportsLockout).Verifiable();
            manager.Setup(m => m.RedeemTwoFactorRecoveryCodeAsync(user, bypassCode))
                .ReturnsAsync(IdentityResult.Success)
                .Verifiable();
            if (supportsLockout)
            {
...
#pragma warning restore xUnit1026
        ...
        )
        {
            ...
            manager.Setup(m => m.VerifyTwoFactorTokenAsync(user, provider, code))
                .ReturnsAsync(true)
                .Verifiable();
            var context = new DefaultHttpContext();
            var auth = MockAuth(context);
        ...
        public async Task PasswordSignInFailsWithWrongPassword()
        {
            ...
            manager.Setup(m => m.SupportsUserLockout).Returns(true).Verifiable();
            manager.Setup(m => m.IsLockedOutAsync(user)).ReturnsAsync(false).Verifiable();
            manager.Setup(m => m.CheckPasswordAsync(user, "[PLACEHOLDER]-bogus1"))
                .ReturnsAsync(false)
                .Verifiable();
            var context = new Mock<HttpContext>();
            var logger = new TestLogger<SignInManager<PocoUser>>();
        ...
        public async Task PasswordSignInFailsWithUnknownUser()
        {
            // Setup
            var manager = MockHelpers.MockUserManager<PocoUser>();
            manager.Setup(m => m.FindByNameAsync("unknown-username"))
        ...
        public async Task PasswordSignInFailsWithWrongPasswordCanAccessFailedAndLockout()
        {
            ...
            var manager = SetupUserManager(user);
            var lockedout = false;
            manager.Setup(m => m.AccessFailedAsync(user))
                .Returns(
                    () =>
                    {
                        lockedout = true;
        ...
        public async Task CanRequireConfirmedPhoneNumberForPasswordSignIn(bool confirmed)
        {
            ...
            var user = new PocoUser { UserName = "Foo" };
            var manager = SetupUserManager(user);
            manager.Setup(m => m.IsPhoneNumberConfirmedAsync(user))
