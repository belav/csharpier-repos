...
namespace Microsoft.AspNetCore.Identity.Test
{
    public class SecurityStampTest
    {
        ...
        )
        {
            ...
            signInManager.Setup(s => s.ValidateSecurityStampAsync(It.IsAny<ClaimsPrincipal>()))
            ...
            if (shouldStampValidate)
            {
                ...
                id.AddClaim(new Claim(ClaimTypes.NameIdentifier, user.Id));
                var principal = new ClaimsPrincipal(id);
                signInManager.Setup(s => s.CreateUserPrincipalAsync(user))
        ...
        public async Task OnValidateIdentityAcceptsWhenStoreDoesNotSupportSecurityStamp()
        {
            ...
            userManager.Setup(u => u.GetUserAsync(It.IsAny<ClaimsPrincipal>()))
                .ReturnsAsync(user)
                .Verifiable();
            claimsManager.Setup(c => c.CreateAsync(user))
        ...
        public async Task OnValidateIdentityDoesNotRejectsWhenNotExpired()
        {
            ...
            signInManager.Setup(s => s.SignInAsync(user, false, null))
                .Throws(new Exception("Shouldn't be called"));
            var services = new ServiceCollection();
            services.AddSingleton(options.Object);
            services.AddSingleton(signInManager.Object);
