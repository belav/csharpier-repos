    ...
    public class UserManagerTest
    {
        ...
        public class CustomUserManager : UserManager<PocoUser>
        {
            public CustomUserManager()
                : base(
                    new Mock<IUserStore<PocoUser>>().Object,
                    null,
                    null,
                    null,
                    null,
        ...
        public async Task CreateCallsStore()
        {
            ...
            store
                .Setup(s => s.CreateAsync(user, CancellationToken.None))
                .ReturnsAsync(IdentityResult.Success)
                .Verifiable();
            store
                .Setup(s => s.GetUserNameAsync(user, CancellationToken.None))
        ...
        public async Task CreateUpdatesSecurityStampStore()
        {
            ...
            store
                ...
                .Verifiable();
            store
                .Setup(s => s.GetSecurityStampAsync(user, CancellationToken.None))
        ...
        public async Task CreateCallsUpdateEmailStore()
        {
            ...
            store
                ...
                .Verifiable();
            store
                .Setup(s => s.GetEmailAsync(user, CancellationToken.None))
        ...
        public async Task DeleteCallsStore()
        {
            ...
            var user = new PocoUser { UserName = "Foo" };
            store
                .Setup(s => s.DeleteAsync(user, CancellationToken.None))
                .ReturnsAsync(IdentityResult.Success)
                .Verifiable();
        ...
        public async Task UpdateCallsStore()
        {
            ...
            store
                ...
                .Verifiable();
            store
                .Setup(s => s.UpdateAsync(user, CancellationToken.None))
                .ReturnsAsync(IdentityResult.Success)
                .Verifiable();
            var userManager = MockHelpers.TestUserManager(store.Object);
        ...
        public async Task SetUserNameCallsStore()
        {
            ...
            store
                .Setup(s => s.SetUserNameAsync(user, "foo", CancellationToken.None))
                .Returns(Task.FromResult(0))
                .Verifiable();
            store
                .Setup(s => s.GetUserNameAsync(user, CancellationToken.None))
                ...
                .Verifiable();
            store
                .Setup(s => s.SetNormalizedUserNameAsync(user, "FOO", CancellationToken.None))
                .Returns(Task.FromResult(0))
                .Verifiable();
            store
                .Setup(s => s.UpdateAsync(user, CancellationToken.None))
        ...
        public async Task FindByIdCallsStore()
        {
            ...
            var user = new PocoUser { UserName = "Foo" };
            store
                .Setup(s => s.FindByIdAsync(user.Id, CancellationToken.None))
        ...
        public async Task CanFindByNameCallsStoreWithoutNormalizedName()
        {
            ...
            var user = new PocoUser { UserName = "Foo" };
            store
                .Setup(s => s.FindByNameAsync(user.UserName, CancellationToken.None))
                .Returns(Task.FromResult(user))
                .Verifiable();
            var userManager = MockHelpers.TestUserManager(store.Object);
        ...
        public async Task CanFindByEmailCallsStoreWithoutNormalizedEmail()
        {
            ...
            var user = new PocoUser { Email = "Foo" };
            store
                .Setup(s => s.FindByEmailAsync(user.Email, CancellationToken.None))
                .Returns(Task.FromResult(user))
                .Verifiable();
            var userManager = MockHelpers.TestUserManager(store.Object);
            userManager.KeyNormalizer = null;
        ...
        public async Task FindByEmailCallsStoreWithCustomNormalizedEmail()
        {
            ...
            var user = new PocoUser { Email = "Foo" };
            store
                .Setup(s => s.FindByEmailAsync("@Foo", CancellationToken.None))
                .Returns(Task.FromResult(user))
                .Verifiable();
            var userManager = MockHelpers.TestUserManager(store.Object);
        ...
        public async Task FindByNameCallsStoreWithCustomNormalizedName()
        {
            ...
            store
                .Setup(s => s.FindByNameAsync("#Foo", CancellationToken.None))
                .Returns(Task.FromResult(user))
                .Verifiable();
            var userManager = MockHelpers.TestUserManager(store.Object);
            userManager.KeyNormalizer = new CustomNormalizer();
        ...
        public async Task AddToRolesCallsStore()
        {
            ...
            store
                .Setup(s => s.AddToRoleAsync(user, "A", CancellationToken.None))
                .Returns(Task.FromResult(0))
                .Verifiable();
            store
                .Setup(s => s.AddToRoleAsync(user, "B", CancellationToken.None))
                .Returns(Task.FromResult(0))
                .Verifiable();
            store
                .Setup(s => s.AddToRoleAsync(user, "C", CancellationToken.None))
                .Returns(Task.FromResult(0))
                .Verifiable();
            ...
            store
                ...
                .Verifiable();
            store
                .Setup(s => s.IsInRoleAsync(user, "A", CancellationToken.None))
                .Returns(Task.FromResult(false))
                .Verifiable();
            store
                .Setup(s => s.IsInRoleAsync(user, "B", CancellationToken.None))
                .Returns(Task.FromResult(false))
                .Verifiable();
            store
                .Setup(s => s.IsInRoleAsync(user, "C", CancellationToken.None))
                .Returns(Task.FromResult(false))
                .Verifiable();
            var userManager = MockHelpers.TestUserManager<PocoUser>(store.Object);
        ...
        public async Task AddToRolesFailsIfUserInRole()
        {
            ...
            IdentityResultAssert.IsFailure(
                result,
                new IdentityErrorDescriber().UserAlreadyInRole("B")
            );
            store.VerifyAll();
        }
        ...
        public async Task RemoveFromRolesCallsStore()
        {
            ...
            var roles = new[] { "A", "B", "C" };
            store
                .Setup(s => s.RemoveFromRoleAsync(user, "A", CancellationToken.None))
                .Returns(Task.FromResult(0))
                .Verifiable();
            store
                .Setup(s => s.RemoveFromRoleAsync(user, "B", CancellationToken.None))
                .Returns(Task.FromResult(0))
                .Verifiable();
            store
                .Setup(s => s.RemoveFromRoleAsync(user, "C", CancellationToken.None))
                .Returns(Task.FromResult(0))
                .Verifiable();
            store
                .Setup(s => s.UpdateAsync(user, CancellationToken.None))
        ...
        public async Task AddClaimsCallsStore()
        {
            ...
            store
                .Setup(s => s.AddClaimsAsync(user, claims, CancellationToken.None))
                .Returns(Task.FromResult(0))
                .Verifiable();
            store
                .Setup(s => s.UpdateAsync(user, CancellationToken.None))
        ...
        public async Task CheckPasswordWillRehashPasswordWhenNeeded()
        {
            ...
            store
                .Setup(s => s.GetPasswordHashAsync(user, CancellationToken.None))
                .ReturnsAsync(hashed)
                .Verifiable();
            store
                .Setup(
                ...
                .Verifiable();
            store
                .Setup(x => x.UpdateAsync(It.IsAny<PocoUser>(), It.IsAny<CancellationToken>()))
                .Returns(Task.FromResult(IdentityResult.Success));

            hasher
                .Setup(s => s.VerifyHashedPassword(user, hashed, pwd))
        ...
        public async Task RemoveClaimsCallsStore()
        {
            ...
            store
                .Setup(s => s.RemoveClaimsAsync(user, claims, CancellationToken.None))
                .Returns(Task.FromResult(0))
                .Verifiable();
            store
                .Setup(s => s.UpdateAsync(user, CancellationToken.None))
        ...
        public async Task UsersEmailMethodsFailWhenStoreNotImplemented()
        {
            ...
            await Assert.ThrowsAsync<NotSupportedException>(
                () => manager.SetEmailAsync(null, null)
            );
        ...
        public async Task PasswordMethodsFailWhenStoreNotImplemented()
        {
            ...
            await Assert.ThrowsAsync<NotSupportedException>(
                () => manager.RemovePasswordAsync(null)
            );
            await Assert.ThrowsAsync<NotSupportedException>(
        ...
        public async Task SecurityStampMethodsFailWhenStoreNotImplemented()
        {
            var store = new Mock<IUserStore<PocoUser>>();
            store
                .Setup(x => x.GetUserIdAsync(It.IsAny<PocoUser>(), It.IsAny<CancellationToken>()))
        ...
        public async Task AuthTokenMethodsFailWhenStoreNotImplemented()
        {
            ...
            await VerifyException<NotSupportedException>(
                async () => await manager.GetAuthenticationTokenAsync(null, null, null),
                error
            );
            await VerifyException<NotSupportedException>(
                async () => await manager.SetAuthenticationTokenAsync(null, null, null, null),
                error
            );
            await VerifyException<NotSupportedException>(
                async () => await manager.RemoveAuthenticationTokenAsync(null, null, null),
                error
            );
        }
        ...
        public async Task AuthenticatorMethodsFailWhenStoreNotImplemented()
        {
            ...
            await VerifyException<NotSupportedException>(
                async () => await manager.GetAuthenticatorKeyAsync(null),
                error
            );
            await VerifyException<NotSupportedException>(
                async () => await manager.ResetAuthenticatorKeyAsync(null),
                error
            );
        }
        ...
        public async Task RecoveryMethodsFailWhenStoreNotImplemented()
        {
            ...
            await VerifyException<NotSupportedException>(
                async () => await manager.RedeemTwoFactorRecoveryCodeAsync(null, null),
                error
            );
            await VerifyException<NotSupportedException>(
                async () => await manager.GenerateNewTwoFactorRecoveryCodesAsync(null, 10),
                error
            );
        }

        private async Task VerifyException<TException>(Func<Task> code, string expectedMessage)
        ...
        public async Task PasswordValidatorBlocksCreate()
        {
            ...
            IdentityResultAssert.IsFailure(
                await manager.CreateAsync(new PocoUser(), "password"),
        ...
        public async Task MethodsThrowWhenDisposedTest()
        {
            ...
            await Assert.ThrowsAsync<ObjectDisposedException>(
                () => manager.AddClaimAsync(null, null)
            );
            await Assert.ThrowsAsync<ObjectDisposedException>(
                () => manager.AddClaimsAsync(null, null)
            );
            await Assert.ThrowsAsync<ObjectDisposedException>(
                () => manager.AddLoginAsync(null, null)
            );
            await Assert.ThrowsAsync<ObjectDisposedException>(
                () => manager.AddPasswordAsync(null, null)
            );
            await Assert.ThrowsAsync<ObjectDisposedException>(
                () => manager.AddToRoleAsync(null, null)
            );
            await Assert.ThrowsAsync<ObjectDisposedException>(
            ...
            await Assert.ThrowsAsync<ObjectDisposedException>(
                () => manager.IsInRoleAsync(null, null)
            );
            await Assert.ThrowsAsync<ObjectDisposedException>(
              ...
              IUserSecurityStampStore<PocoUser>
        {
            ...
            public Task<PocoUser> FindByLoginAsync(
                string loginProvider,
                string providerKey,
                CancellationToken cancellationToken = default(CancellationToken)
            )
        ...
        public async Task CanCustomizeUserValidatorErrors()
        {
            ...
            store
                .Setup(s => s.FindByEmailAsync("DUPE@EMAIL.COM", CancellationToken.None))
                .Returns(Task.FromResult(user2))
                .Verifiable();
            store
                .Setup(s => s.GetUserIdAsync(user2, CancellationToken.None))
            ...
            IdentityResultAssert.IsFailure(
                ...
                describer.DuplicateEmail(user.Email)
            );
