...
public class HtmlRendererTest
{
            ...
            collection.AddSingleton(
                new RenderFragment(rtb =>
                {
                    ...
                    rtb.AddMultipleAttributes(
                        3,
                        new Dictionary<string, object>() { { "Class", "test2" } }
                    );
    ...
    public async Task RenderComponentAsync_CanPassParameters()
    {
        ...
        var serviceProvider = GetServiceProvider(collection =>
            collection.AddSingleton(new Func<ParameterView, RenderFragment>(Content))
        );
    ...
    public async Task CanRender_AsyncComponent()
    {
        ...
        var serviceProvider = GetServiceProvider(collection =>
            collection.AddSingleton<AsyncComponent>()
        );
    ...
    public async Task BeginRenderingComponent_CanObserveStateBeforeAndAfterQuiescence()
    {
        ...
        var services = GetServiceProvider(collection =>
            collection.AddSingleton(
                new AsyncLoadingComponentCompletion { Task = completionTcs.Task }
            )
        );
    ...
    public async Task RenderComponentAsync_ThrowsAsync()
    {
        ...
        var completionTcs = new TaskCompletionSource();
        var services = GetServiceProvider(collection =>
            collection.AddSingleton(new AsyncLoadingComponentCompletion { Task = Task.Delay(0) })
        );
...
And now with HTML encoding: Person with special chars like &#x27; &quot; &lt;/script&gt;".Replace(
    ...
    [InlineData(
        "https://example.com/",
        "https://example.com/a/b/c?q=1&p=hello%20there",
        "/a/b/c?q=1&p=hello%20there"
    )]
    ...
    [InlineData(
        "http://user:pass@xyz.example.com:1234/a/b/",
        "http://user:pass@xyz.example.com:1234/a/b/c&q=1&p=2",
        "/a/b/c&q=1&p=2"
    )]
    ...
    )
    {
        ...
        await htmlRenderer.Dispatcher.InvokeAsync(async () =>
        {
            ...
            // Assert
            Assert.Equal(
                $"<form action=\"{HttpUtility.HtmlAttributeEncode(expectedAction)}\"></form>",
                result.ToHtmlString()
            );
        });
    }
    ...
    public async Task RenderComponentAsync_DoesNotAddActionAttributeWithCurrentUrl_WhenActionIsExplicitlySpecified()
    {
        ...
        await htmlRenderer.Dispatcher.InvokeAsync(async () =>
        {
            ...
            // Assert
            Assert.Equal(
                "<form action=\"https://example.com/explicit\"></form>",
                result.ToHtmlString()
            );
        });
    }
