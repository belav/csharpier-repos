...
internal partial class EndpointHtmlRenderer
{
    private readonly Dictionary<
        (int ComponentId, int FrameIndex),
        string
    > _namedSubmitEventsByLocation = new();
    private readonly Dictionary<
        string,
        HashSet<(int ComponentId, int FrameIndex)>
    > _namedSubmitEventsByScopeQualifiedName = new(StringComparer.Ordinal);
    ...
    internal Task DispatchSubmitEventAsync(string? handlerName, out bool isBadRequest)
    {
        ...
        if (
            !_namedSubmitEventsByScopeQualifiedName.TryGetValue(
                handlerName,
                out var locationsForName
            )
            || locationsForName.Count == 0
        )
        {
        ...
        if (locationsForName.Count > 1)
        {
            ...
            throw new InvalidOperationException(
                CreateMessageForAmbiguousNamedSubmitEvent(handlerName, locationsForName)
            );
        }

        isBadRequest = false;
        var frameLocation = locationsForName.Single();
        var eventHandlerId = FindEventHandlerIdForNamedEvent(
            "onsubmit",
            frameLocation.ComponentId,
    ...
    private string CreateMessageForAmbiguousNamedSubmitEvent(
        string scopeQualifiedName,
        IEnumerable<(int ComponentId, int FrameIndex)> locations
    )
    {
        var sb = new StringBuilder(
            $"There is more than one named submit event with the name '{scopeQualifiedName}'. Ensure named submit events have unique names, or are in scopes with distinct names. The following components use this name:"
        );
    ...
    private void ProcessNamedSubmitEventRemovals(ArrayRange<NamedEventChange> changes)
    {
        ...
        for (var i = 0; i < changesCount; i++)
        {
            ...
            if (
                change.ChangeType == NamedEventChangeType.Removed
            ...
            )
            {
                ...
                {
                    var locationsForName = _namedSubmitEventsByScopeQualifiedName[
                        scopeQualifiedName
                    ];
    ...
    private void ProcessNamedSubmitEventAdditions(ArrayRange<NamedEventChange> changes)
    {
        ...
        for (var i = 0; i < changesCount; i++)
        {
            ref var change = ref changesArray[i];
            if (
                change.ChangeType == NamedEventChangeType.Added
                && string.Equals(change.EventType, "onsubmit", StringComparison.Ordinal)
            )
            {
                if (
                ...
                {
                    var locationsForName = GetOrAddNewToDictionary(
                        _namedSubmitEventsByScopeQualifiedName,
                        scopeQualifiedName
                    );
                    var location = (change.ComponentId, change.FrameIndex);
                    if (!locationsForName.Add(location))
                    {
                        ...
                        throw new InvalidOperationException(
                            $"A single named submit event is tracked more than once at the same location."
                        );
                    }
    ...
    private static TVal GetOrAddNewToDictionary<TKey, TVal>(
    ...
    private ulong? FindEventHandlerIdForNamedEvent(
        string eventType,
        int componentId,
        int frameIndex
    )
    ...
    private string GenerateComponentPath(int componentId)
    {
        ...
        for (
            var current = GetComponentState(componentId);
            current != null;
            current = current.ParentComponentState
        )
        {
