...
internal sealed class ComplexTypeExpressionConverterFactory<T>(FormDataMetadataFactory factory)
    : ComplexTypeExpressionConverterFactory
{
    [RequiresDynamicCode(FormMappingHelpers.RequiresDynamicCodeMessage)]
    [RequiresUnreferencedCode(FormMappingHelpers.RequiresUnreferencedCodeMessage)]
    internal override CompiledComplexTypeConverter<T> CreateConverter(
        Type type,
    ...
    [RequiresDynamicCode(FormMappingHelpers.RequiresDynamicCodeMessage)]
    [RequiresUnreferencedCode(FormMappingHelpers.RequiresUnreferencedCodeMessage)]
    private CompiledComplexTypeConverter<T>.ConverterDelegate CreateConverterBody(
        Type type,
        FormDataMapperOptions options
    )
    {
        ...
        var (readerParam, typeParam, optionsParam, resultParam, foundValueParam) =
            CreateFormDataConverterParameters();
        ...
        {
            body.Add(
                CreatePrefixCheckForRecursiveTypes(readerParam, foundValueParam, succeeded, end)
            );
        }
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException(
                $"Type '{model.Type}' does not have a constructor. "
        ...
        var checks = ReportMissingValues(
            context,
            constructorParameterValueLocals,
            constructorParameters,
            propsLocals,
        ...
        var clause = model.Type.IsValueType
            ? failedAndHasHandler
            : Expression.And(
                failedAndHasHandler,
        ...
        if (!model.Type.IsValueType)
        {
            ...
            for (var i = 0; i < props.Count; i++)
            {
                assignments.Add(
                    Expression.Assign(
                        Expression.Property(resultParam, props[i].Property),
                        GetValueLocalVariableValueExpression(propsLocals[i])
                    )
                );
            }
            ...
            if (metadata.Required)
            {
                ...
                yield return Expression.IfThen(
                    ...
                    Expression.Block(
                        ...
                        AddMappingError(
                            readerParam,
                            "Missing required value for constructor parameter '{0}'.",
                            metadata.Name
                        ),
            ...
            if (metadata.Required)
            {
                ...
                yield return Expression.IfThen(
                    ...
                    Expression.Block(
                        ...
                        AddMappingError(
                            readerParam,
                            "Missing required value for property '{0}'.",
                            metadata.Name
                        ),
                        PopPrefix(readerParam, metadata.Name),
                        Expression.Assign(succeeded, Expression.Constant(false, typeof(bool)))
    ...
    )
    {
        ...
        for (var i = 0; i < properties.Count; i++)
        {
            ...
            var propertyConverterVar = Expression.Variable(
            ...
            body.Add(
                Expression.Assign(
                    GetValueLocalVariableFoundExpression(propertyVar),
                    propertyFoundValue
                )
            );
    ...
    )
    {
        ...
        for (var i = 0; i < constructorParameters.Count; i++)
        {
            ...
            var constructorParameterConverterType = typeof(FormDataConverter<>).MakeGenericType(
                constructorParameter.Type
            );
            var constructorParameterConverterVar = Expression.Variable(
                constructorParameterConverterType,
                $"{constructorParameter.Name}Converter"
            );
            ...
            body.Add(
                Expression.Assign(
                    GetValueLocalVariableFoundExpression(constructorParameterVar),
                    propertyFoundValue
                )
            );
            body.Add(Expression.OrAssign(localFoundValueVar, propertyFoundValue));
    ...
    private static Expression GetValueLocalVariableFoundExpression(
        ParameterExpression constructorParameterVar
    )
    {
        return Expression.PropertyOrField(
            constructorParameterVar,
            nameof(ValueTuple<bool, object>.Item1)
        );
    }
    ...
    private static ParameterExpression CreateValueLocalVariable(IFormDataValue constructorParameter)
    {
        return Expression.Variable(
            typeof(ValueTuple<,>).MakeGenericType(typeof(bool), constructorParameter.Type),
    ...
    )
    {
        return Expression.IfThen(
            Expression.Not(
                Expression.Call(
                    readerParam,
                    nameof(FormDataReader.CurrentPrefixExists),
                    Array.Empty<Type>()
                )
            ),
            Expression.Block(
