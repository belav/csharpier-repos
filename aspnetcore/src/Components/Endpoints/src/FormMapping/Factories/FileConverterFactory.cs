...
internal sealed class FileConverterFactory : IFormDataConverterFactory
{
    ...
    public bool CanConvert(Type type, FormDataMapperOptions options) =>
        CanConvertCommon(type)
        || type == typeof(IBrowserFile)
        || type == typeof(IReadOnlyList<IBrowserFile>);
#else
    ...
    {
        return Activator.CreateInstance(typeof(FileConverter<>).MakeGenericType(type))
