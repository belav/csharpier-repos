    ...
    : IFormDataConverterFactory
{
    ...
    [UnconditionalSuppressMessage(
        "Trimming",
        "IL2046",
        Justification = "This derived implementation doesn't require unreferenced code like other implementations of the interface."
    )]
    [UnconditionalSuppressMessage(
        "AOT",
        "IL3051",
        Justification = "This derived implementation doesn't use dynamic code like other implementations of the interface."
    )]
    ...
    public FormDataConverter CreateConverter(Type _, FormDataMapperOptions options)
    {
        ...
        var customFactory =
            Activator.CreateInstance(
                typeof(CustomCollectionConverterFactory<>).MakeGenericType(
                    typeof(TCollection),
                    typeof(TElement),
                    typeof(TCollection)
                )
            ) as CustomCollectionConverterFactory;

        if (customFactory == null)
        {
            throw new InvalidOperationException(
                $"Unable to create converter for type '{typeof(TCollection).FullName}'."
            );
        }
    ...
    {
        public abstract FormDataConverter CreateConverter(
    ...
    {
        public override FormDataConverter CreateConverter(
            FormDataConverter<TElement> elementConverter
        )
        {
