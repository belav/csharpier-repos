...
namespace Microsoft.AspNetCore.Components.E2ETest.ServerExecutionTests
{
        ...
        : ServerTestBase<BasicTestAppServerSiteFixture<MultipleComponents>>
    {
        ...
        public void CanRenderMultipleRootComponents()
        {
            ...
            var greets = Browser.FindElements(By.CssSelector(".greet-wrapper .greet"))
                .Select(e => e.Text)
                .ToArray();
            ...
            Browser.Exists(By.CssSelector("h3.interactive"));
            var updatedGreets = Browser.FindElements(By.CssSelector(".greet-wrapper .greet"))
                .Select(e => e.Text)
                .ToArray();
        ...
        private (ServerComponentMarker, ServerComponentMarker)[] ReadMarkers(string content)
        {
            ...
            var markers = matches.Select(
                s =>
                    JsonSerializer.Deserialize<ServerComponentMarker>(
                        s.Groups[1].Value,
                        ServerComponentSerializationSettings.JsonSerializationOptions
                    )
            );

            var prerenderMarkers = markers.Where(m => m.PrerenderId != null)
                .GroupBy(p => p.PrerenderId)
                .Select(g => (g.First(), g.Skip(1).First()))
                .ToArray();
            var nonPrerenderMarkers = markers.Where(m => m.PrerenderId == null)
                .Select(g => (g, (ServerComponentMarker)default))
                .ToArray();

            return prerenderMarkers.Concat(nonPrerenderMarkers)
