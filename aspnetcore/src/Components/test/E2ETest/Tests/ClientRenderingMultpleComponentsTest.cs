...
{
    public class ClientRenderingMultpleComponentsTest
        : E2ETest.Infrastructure.ServerTestBase<BasicTestAppServerSiteFixture<MultipleComponents>>
    {
        ...
        )
        {
            ...
            var markers = matches.Select(
                s =>
                    JsonSerializer.Deserialize<WebAssemblyComponentMarker>(
                        s.Groups[1].Value,
            ...
            var nonPrerenderMarkers = markers.Where(m => m.PrerenderId == null)
                .Select(g => (g, (WebAssemblyComponentMarker)default))
                .ToArray();
