...
public class JSRootComponentsTest : ServerTestBase<ToggleExecutionModeServerFixture<Program>>
{
    ...
    public void CanAddAndDisposeRootComponents(bool intoBlazorUi, bool attachShadowRoot)
    {
        ...
        Browser.Equal(
            $"Disposed component in {(attachShadowRoot ? "ShadowDOM" : containerId)}",
            () => message.Text
        );
    ...
    public void CanSupplyCatchAllParameters()
    {
        ...
        Assert.Collection(
            ...
            param =>
            {
                Assert.Equal(
                    "stringVal",
                    param.FindElement(By.ClassName("unmatched-value-name")).Text
                );
                Assert.Equal(
                    "String",
                    param.FindElement(By.ClassName("unmatched-value-type")).Text
                );
                Assert.Equal(
                    "Hello",
                    param.FindElement(By.ClassName("unmatched-value-value")).Text
                );
            },
            param =>
            {
    ...
    public void CanSupplyAndInvokeFunctionParameters()
    {
        ...
        Browser.Equal(
            $"JavaScript button callback received mouse event args (id=0, buttons=0)",
            () => app.FindElement(By.Id("message")).Text
        );
    ...
    public void CallsJavaScriptInitializers()
    {
        ...
        var expectedCallLog =
            "[{\"name\":\"component-with-many-parameters\",\"parameters\":["
    ...
    {
        var result = ((IJavaScriptExecutor)Browser).ExecuteScript(
