...
namespace Microsoft.AspNetCore.Components.E2ETest.Tests
{
    public class KeyTest : ServerTestBase<ToggleExecutionModeServerFixture<Program>>
    {
        ...
        public async Task CanRetainFocusWhileMovingTextBox()
        {
            ...
            Func<IWebElement> textboxFinder = () =>
                appElem.FindElement(By.CssSelector(".incomplete-items .item-1 input[type=text]"));
            ...
            appElem.FindElement(By.CssSelector(".incomplete-items .item-1 input[type=checkbox]"))
                .Click();
            Browser.Equal(
                expectedTextTyped,
                () =>
                    appElem.FindElement(By.CssSelector(".complete-items .item-1 input[type=text]"))
                        .GetAttribute("value")
            );
        }
        ...
        {
            var appElem = Browser.MountTestComponent<ReorderingFocusComponent>();
            Func<IWebElement> checkboxFinder = () =>
                appElem.FindElement(By.CssSelector(".item-2 input[type=checkbox]"));
            Func<IEnumerable<bool>> incompleteItemStates = () =>
                appElem.FindElements(By.CssSelector(".incomplete-items input[type=checkbox]"))
                    .Select(elem => elem.Selected);
            Func<IEnumerable<bool>> completeItemStates = () =>
                appElem.FindElements(By.CssSelector(".complete-items input[type=checkbox]"))
                    .Select(elem => elem.Selected);
        ...
        private void PerformTest(Node[] before, Node[] after)
        {
            ...
            var jsonBefore = JsonSerializer.Serialize(
                rootBefore,
                TestJsonSerializerOptionsProvider.Options
            );
            var jsonAfter = JsonSerializer.Serialize(
                rootAfter,
                TestJsonSerializerOptionsProvider.Options
            );
        ...
        private static void ValidateRenderedOutput(
            IWebElement appElem,
        ...
        private static Node ReadNodeFromDOM(IWebElement nodeElem)
        {
            ...
            var childNodes = nodeElem.FindElements(
                By.XPath("*[@class='children']/*[@class='node']")
            );
