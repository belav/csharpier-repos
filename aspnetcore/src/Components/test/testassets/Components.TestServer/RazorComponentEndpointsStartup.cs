...
public class RazorComponentEndpointsStartup<TRootComponent>
{
                ...
                app.UseEndpoints(endpoints =>
                {
                    endpoints
                        ...
                        .AddInteractiveWebAssemblyRenderMode(
                            options => options.PathPrefix = "/WasmMinimal"
                        );
            ...
            (HttpContext context, Func<Task> next) =>
            {
                ...
                if (!string.IsNullOrEmpty(username))
                {
                    ...
                    context.User = new ClaimsPrincipal(
                        new ClaimsIdentity(claims, "FakeAuthenticationType")
                    );
                }
    ...
    private static void MapEnhancedNavigationEndpoints(IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.Map(
            "/nav/give-404-with-content",
            async (HttpResponse response) =>
            {
                response.StatusCode = 404;
                response.ContentType = "text/html";
                await response.WriteAsync(
                    "<h1>404</h1><p>Sorry, there's nothing here! This is a custom server-generated 404 message.</p>"
                );
            }
        );
        ...
        endpoints.Map(
            ...
            async (HttpRequest request, HttpResponse response) =>
            {
                ...
                {
                    await response.WriteAsync(
                        $"<li>{HttpUtility.HtmlEncode(header.Key)}: {HttpUtility.HtmlEncode(header.Value)}</li>"
                    );
                }
        ...
        endpoints.Map(
            "/nav/non-blazor-html-response",
            async (HttpResponse response) =>
            {
                response.ContentType = "text/html";
                await response.WriteAsync(
                    "<html><body><h1>This is a non-Blazor endpoint</h1><p>That's all</p></body></html>"
                );
            }
        );

        endpoints.MapPost(
            "api/antiforgery-form",
            (
            ...
            ) =>
            {
                ...
                if (
                    string.IsNullOrEmpty(inFormCsrfToken) && string.IsNullOrEmpty(inHeaderCsrfToken)
                )
                {
