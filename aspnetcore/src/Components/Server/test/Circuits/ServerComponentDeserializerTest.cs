...
public class ServerComponentDeserializerTest
{
    ...
    public void CanParseSingleMarker()
    {
        ...
        // Act & assert
        Assert.True(
            serverComponentDeserializer.TryDeserializeComponentDescriptorCollection(
                markers,
                out var descriptors
            )
        );
    ...
    public void DoesNotParseOutOfOrderMarkers()
    {
        // Arrange
        var markers = SerializeMarkers(
            CreateMarkers(typeof(TestComponent), typeof(TestComponent)).Reverse().ToArray()
        );
        var serverComponentDeserializer = CreateServerComponentDeserializer();
    ...
    public void DoesNotParseMarkersWhoseSequenceDoesNotStartAtZero()
    {
        // Arrange
        var markers = SerializeMarkers(
            CreateMarkers(typeof(TestComponent), typeof(TestComponent)).Skip(1).ToArray()
        );
        var serverComponentDeserializer = CreateServerComponentDeserializer();
    ...
    public void TryDeserializeSingleComponentDescriptor_CanParseSingleMarker()
    {
        ...
        // Act & assert
        Assert.True(
            serverComponentDeserializer.TryDeserializeSingleComponentDescriptor(
                markers[0],
                out var descriptor
            )
        );
    ...
    public void TryDeserializeSingleComponentDescriptor_AllowsParsingMarkersOutOfOrder()
    {
        ...
        // Act & assert
        Assert.True(
            serverComponentDeserializer.TryDeserializeSingleComponentDescriptor(markers[1], out _)
        );
        Assert.True(
            serverComponentDeserializer.TryDeserializeSingleComponentDescriptor(markers[0], out _)
        );
    }
    ...
    public void TryDeserializeSingleComponentDescriptor_AllowsParsingMarkersFromMultipleInvocations()
    {
        ...
        Assert.True(
            serverComponentDeserializer.TryDeserializeSingleComponentDescriptor(
                firstInvocationMarkers[0],
                out _
            )
        );
        Assert.True(
            serverComponentDeserializer.TryDeserializeSingleComponentDescriptor(
                secondInvocationMarkers[0],
                out _
            )
        );
    }
    ...
    public void UpdateRootComponents_TryDeserializeRootComponentOperationsReturnsFalse_WhenAddOperationIsMissingSelectorId()
    {
        ...
        var operation = new RootComponentOperation
        {
            ...
            SelectorId = 1,
            Marker = new ComponentMarker() { Descriptor = "some random text", },
        };
        ...
        // Act
        var result = deserializer.TryDeserializeRootComponentOperations(
            operationsJson,
            out var parsed
        );
    ...
    public void UpdateRootComponents_TryDeserializeRootComponentOperationsReturnsFalse_WhenComponentIdIsMissing()
    {
        ...
        var operation = new RootComponentOperation
        {
            ...
            Marker = CreateMarker(
                ...
                new() { ["Message"] = "Some other message", }
            ),
        };
        var operationsJson = JsonSerializer.Serialize(
            new[] { operation },
    ...
    private ComponentMarker[] CreateMarkers(
