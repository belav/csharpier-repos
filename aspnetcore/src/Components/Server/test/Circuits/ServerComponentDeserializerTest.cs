...
public class ServerComponentDeserializerTest
{
    ...
    public void TryDeserializeWebRootComponentDescriptor_CanParseSingleMarker()
    {
        ...
        // Act & assert
        Assert.True(
            serverComponentDeserializer.TryDeserializeWebRootComponentDescriptor(
                markers[0],
                out var descriptor
            )
        );
    ...
    public void TryDeserializeWebRootComponentDescriptor_AllowsParsingMarkersOutOfOrder()
    {
        ...
        // Act & assert
        Assert.True(
            serverComponentDeserializer.TryDeserializeWebRootComponentDescriptor(markers[1], out _)
        );
        Assert.True(
            serverComponentDeserializer.TryDeserializeWebRootComponentDescriptor(markers[0], out _)
        );
    }
    ...
    public void UpdateRootComponents_TryDeserializeRootComponentOperationsReturnsFalse_WhenSsrComponentIdIsRepeated()
    {
        ...
        var batchJson = SerializeRootComponentOperationBatch(
            new() { Operations = [operation, other] }
        );
