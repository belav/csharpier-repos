...
// Each comment is in the form <!-- Blazor:<<Json>>--> for example { "type": "server", "sequence":
// 0, descriptor: "base64(dataprotected(<<ServerComponent>>))" }
...
// 'descriptor' a data-protected payload that allows the server to validate the legitimacy of the
// rendered component.
// 'prerenderId' a unique identifier that uniquely identifies the marker to match start and end
// markers.
//
// descriptor holds the information to validate a component render request. It prevents an infinite
...
// 'parameterDefinitions' a JSON serialized array that contains the definitions for the parameters
// including their names and types and assemblies.
// 'parameterValues' a JSON serialized array containing the parameter values.
// 'invocationId' a random string that matches all components rendered by as part of a single HTTP
// response.
// For example: base64(dataprotection({ "sequence": 1, "assemblyName":
// "Microsoft.AspNetCore.Components", "type":"Microsoft.AspNetCore.Components.Routing.Router",
// "invocationId": "<<guid>>"}))
// With parameters
// For example: base64(dataprotection({ "sequence": 1, "assemblyName":
// "Microsoft.AspNetCore.Components", "type":"Microsoft.AspNetCore.Components.Routing.Router",
// "invocationId": "<<guid>>", parameterDefinitions: "[{ \"name\":\"Parameter\",
// \"typeName\":\"string\", \"assembly\":\"System.Private.CoreLib\"}], parameterValues:
// [<<string-value>>]}))

// Serialization:
// For a given response, MVC renders one or more markers in sequence, including a descriptor for
// each rendered
// component containing the information described above.

// Deserialization:
// To prevent a client from rendering an infinite amount of components, we require clients to send
// all component
...
// * We compare the invocationId for the previous descriptor against the invocationId for the
// current descriptor to make sure they match.
...
// * We ensure that we do the minimal amount of work in the case of an invalid sequence of
// descriptors.
//
// For example:
// A client can't just send 100 component markers and force us to process them if the server didn't
// generate those 100 markers.
//  * If a marker is out of sequence we will fail early, so we process at most n-1 markers.
//  * If a marker has the right sequence but the invocation ID is different we will fail at that
// point. We know for sure that the
//    component wasn't render as part of the same response.
//  * If a marker can't be unprotected we will fail early. We know that the marker was tampered with
...
internal sealed partial class ServerComponentDeserializer : IServerComponentDeserializer
{
    ...
    {
        var markers = JsonSerializer.Deserialize<IEnumerable<ComponentMarker>>(
    ...
    public bool TryDeserializeWebRootComponentDescriptor(
        ComponentMarker record,
        [NotNullWhen(true)] out WebRootComponentDescriptor? result
    )
    {
        result = default;
        ...
        // [1] A new invocation has started, in which case we should stop accepting components from the
        // previous one.
        // [2] The client is attempting to use an already-expired invocation ID. We should reject the
        // component in
        ...
        if (
            !TryDeserializeComponentTypeAndParameters(
                serverComponent,
    ...
    {
        parameters = default;
        componentType = _rootComponentTypeCache.GetRootComponent(
    ...
    private static partial class Log
    {
        ...
        [LoggerMessage(
            9,
            LogLevel.Debug,
            "The descriptor invocationId '{invocationId}' has expired.",
            EventName = "ExpiredInvocationId"
        )]
        public static partial void ExpiredInvocationId(
            ILogger<ServerComponentDeserializer> logger,
            string invocationId
        );
        ...
        [LoggerMessage(
            10,
            LogLevel.Debug,
            "The descriptor with sequence '{sequence}' was already used for the current invocationId '{invocationId}'.",
            EventName = "ReusedDescriptorSequence"
        )]
        public static partial void ReusedDescriptorSequence(
            ILogger<ServerComponentDeserializer> logger,
            int sequence,
            string invocationId
        );
        ...
        [LoggerMessage(
            11,
            LogLevel.Debug,
            "The root component operation of type '{OperationType}' was invalid: {Message}",
            EventName = "InvalidRootComponentOperation"
        )]
        public static partial void InvalidRootComponentOperation(
            ILogger logger,
        ...
        [LoggerMessage(
            12,
            LogLevel.Debug,
            "Failed to parse root component operations",
            EventName = nameof(FailedToProcessRootComponentOperations)
        )]
        ...
        [LoggerMessage(
            13,
            LogLevel.Debug,
            "The provided root component key was not valid.",
            EventName = nameof(InvalidRootComponentKey)
        )]
