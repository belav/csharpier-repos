...
/// <summary>
/// A <see cref="CircuitHandler"/> allows running code during specific lifetime events of a <see
// cref="Circuit"/>.
/// <list type="bullet">
/// <item><description>
/// <see cref="OnCircuitOpenedAsync(Circuit, CancellationToken)"/> is invoked after an initial
// circuit to the client
/// has been established.
/// </description></item>
/// <item><description>
/// <see cref="OnConnectionUpAsync(Circuit, CancellationToken)"/> is invoked immediately after the
// completion of
/// <see cref="OnCircuitOpenedAsync(Circuit, CancellationToken)"/>. In addition, the method is
// invoked each time a connection is re-established
/// with a client after it's been dropped. <see cref="OnConnectionDownAsync(Circuit,
// CancellationToken)"/> is invoked each time a connection
/// is dropped.
/// </description></item>
/// <item><description>
/// <see cref="OnCircuitClosedAsync(Circuit, CancellationToken)"/> is invoked prior to the server
// evicting the circuit to the client.
...
public abstract class CircuitHandler
{
    ...
    /// <param name="cancellationToken">A <see cref="CancellationToken"/> that notifies when the client
    // connection is aborted.</param>
    ...
    /// <para>
    /// This method is executed once initially after <see cref="OnCircuitOpenedAsync(Circuit,
    // CancellationToken)"/>
    ...
    /// <summary>
    /// Creates a handler that gets invoked when inbound activity on the circuit causes an asynchronous
    // task to be dispatched on the server.
    /// </summary>
    /// <param name="next">The next handler to invoke.</param>
    /// <returns>A handler function that returns a <see cref="Task"/> that completes when the activity
    // has finished.</returns>
    public virtual Func<CircuitInboundActivityContext, Task> CreateInboundActivityHandler(
