...
public sealed class CircuitOptions
{
    ...
    /// interval. This allows the client to re-establish a connection to the existing circuit on the
    // server
    ...
    /// </para>
    /// <para>
    /// This value determines the maximum duration circuit state is retained by the server before being
    // evicted.
    ...
    /// <summary>
    /// Gets or sets a value that determines whether or not to send detailed exception messages to
    // JavaScript when an unhandled exception
    /// happens on the circuit or when a .NET method invocation through JS interop results in an
    // exception.
    /// </summary>
    /// <remarks>
    /// This value should only be turned on in development scenarios as turning it on in production
    // might result in the leak of
    ...
    /// <summary>
    /// Gets or sets a value that indicates how long the server will wait before timing out an
    // asynchronous JavaScript function invocation.
    /// </summary>
    /// <value>
    ...
    /// <summary>
    /// Gets or sets the maximum number of render batches that a circuit will buffer until an
    // acknowledgement for the batch is
    /// received.
    /// </summary>
    /// <remarks>
    /// When the limit of buffered render batches is reached components will stop rendering and will
