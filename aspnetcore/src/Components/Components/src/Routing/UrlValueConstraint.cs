...
internal abstract class UrlValueConstraint
{
    ...
    public static bool TryGetByTargetType(
        Type targetType,
        [MaybeNullWhen(false)] out UrlValueConstraint result
    )
    {
    ...
    public abstract Array ParseMultiple(
        StringSegmentAccumulator values,
        string destinationNameForMessage
    );
    ...
    private class TypedUrlValueConstraint<T> : UrlValueConstraint
    {
        ...
        public override object? Parse(ReadOnlySpan<char> value, string destinationNameForMessage)
        {
            if (!_parser(value, out var parsedValue))
            {
                throw new InvalidOperationException(
                    $"Cannot parse the value '{value.ToString()}' as type '{typeof(T)}' for '{destinationNameForMessage}'."
                );
            }
        ...
        public override Array ParseMultiple(
            StringSegmentAccumulator values,
            string destinationNameForMessage
        )
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Cannot parse the value '{values[i]}' as type '{typeof(T)}' for '{destinationNameForMessage}'."
                    );
                }
            }
    ...
    private sealed class NullableTypedUrlValueConstraint<T> : TypedUrlValueConstraint<T?>
        where T : struct
    {
