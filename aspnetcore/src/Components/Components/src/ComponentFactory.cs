...
using Microsoft.AspNetCore.Components.Reflection;using static Microsoft.AspNetCore.Internal.LinkerFlags;
...
internal sealed class ComponentFactory
{
    ...
    private static readonly ConcurrentDictionary<
        Type,
        Action<IServiceProvider, IComponent>
    > _cachedInitializers = new();
    ...
    private static Action<IServiceProvider, IComponent> CreateInitializer(
    ...
    )
    {
        ...
        )
        {
            ...
            injectables ??= new();
            injectables.Add(
                (property.Name, property.PropertyType, new PropertySetter(type, property))
            );
        }
