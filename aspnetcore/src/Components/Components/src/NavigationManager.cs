...
public abstract class NavigationManager
{
    ...
    public string ToBaseRelativePath(string uri)
    {
        ...
        if (
            _baseUri.OriginalString.EndsWith('/')
            && uriPathOnly.Equals(
                _baseUri.OriginalString.AsSpan(0, _baseUri.OriginalString.Length - 1),
    ...
    internal ReadOnlySpan<char> ToBaseRelativePath(ReadOnlySpan<char> uri)
    {
        if (
            MemoryExtensions.StartsWith(
                uri,
                _baseUri!.OriginalString.AsSpan(),
                StringComparison.Ordinal
            )
        )
        {
    ...
    )
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    ArrayPool<Func<LocationChangingContext, ValueTask>>.Shared
    ...
    private static bool TryGetLengthOfBaseUriPrefix(Uri baseUri, string uri, out int length)
    {
        ...
        if (
            ...
            && uriPathOnly.Equals(
                baseUri.OriginalString.AsSpan(0, baseUri.OriginalString.Length - 1),
                StringComparison.Ordinal
            )
        )
        {
