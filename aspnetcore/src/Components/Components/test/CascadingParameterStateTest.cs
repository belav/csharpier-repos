...
public class CascadingParameterStateTest
{
    ...
    public void FindCascadingParameters_IfHasPartialMatchesInAncestors_ReturnsMatches()
    {
        ...
        Assert.Collection(result, match =>
        {
            Assert.Equal(
                ...
                match.ParameterInfo.PropertyName
            );
    ...
    public void FindCascadingParameters_IfHasMultipleMatchesInAncestors_ReturnsMatches()
    {
        ...
        // Assert
        Assert.Collection(
            result.OrderBy(x => x.ParameterInfo.PropertyName),
            match =>
            {
                Assert.Equal(
    ...
    public void FindCascadingParameters_CanOverrideNonNullValueWithNull()
    {
        ...
        // Assert
        Assert.Collection(result.OrderBy(x => x.ParameterInfo.PropertyName), match =>
        {
            Assert.Equal(
                nameof(ComponentWithCascadingParams.CascadingParam1),
                match.ParameterInfo.PropertyName
    ...
    public void FindCascadingParameters_WithoutSingleDelivery()
    {
        ...
        // Act
        _ = CascadingParameterState.FindCascadingParameters(
            states.Last(),
            out var hasSingleDeliveryParameters
        );
    ...
    class TestNavigationManager : NavigationManager
    {
        ...
        {
            Initialize(
                "https://localhost:85/subdir/",
                "https://localhost:85/subdir/path?query=value#hash"
            );
        }
    }
}
