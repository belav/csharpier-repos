...
namespace Microsoft.AspNetCore.Components.Test
{
    public class LayoutViewTest
    {
        ...
        public void GivenLayoutButNoContent_RendersLayoutWithEmptyBody()
        {
            ...
            var layoutViewFrames = _renderer.GetCurrentRenderTreeFrames(_layoutViewComponentId)
                .AsEnumerable();
            Assert.Collection(
                layoutViewFrames,
            ...
            var rootLayoutFrames = _renderer.GetCurrentRenderTreeFrames(rootLayoutComponentId)
                .AsEnumerable();
            Assert.Collection(
                rootLayoutFrames,
        ...
        public void RendersContentInsideNestedLayout()
        {
            ...
            Assert.Collection(
                ...
                frame =>
                    AssertFrame.Attribute(frame, nameof(LayoutComponentBase.Body), sequence: 1),
                frame => AssertFrame.Text(frame, "RootLayout ends here", sequence: 2)
            );

            var nestedLayoutComponentId =
                batch.GetComponentFrames<NestedLayout>().Single().ComponentId;
            var nestedLayoutFrames = _renderer.GetCurrentRenderTreeFrames(nestedLayoutComponentId)
                .AsEnumerable();
            Assert.Collection(
                nestedLayoutFrames,
