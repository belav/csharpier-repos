...
public class RoutePatternParserTests
{
    ...
    [InlineData(
        "{p*}",
        "The route parameter name 'p*' is invalid. Route parameter names must be non-empty and cannot contain these characters: '{', '}', '/'. The '?' character marks a parameter as optional, and can occur only at the end of the parameter. The '*' character marks a parameter as catch-all, and can occur only at the start of the parameter."
    )]
    [InlineData(
        "{{}",
        "There is an incomplete parameter in the route template. Check that each '{' character has a matching '}' character."
    ...
    [InlineData(
        "{=}",
        "The route parameter name '=' is invalid. Route parameter names must be non-empty and cannot contain these characters: '{', '}', '/'. The '?' character marks a parameter as optional, and can occur only at the end of the parameter. The '*' character marks a parameter as catch-all, and can occur only at the start of the parameter.",
        Skip = "{=} is allowed"
    )]
    [InlineData(
        "{.}",
    ...
    [InlineData("/test/{a}/{*b*c}", "b*c")]
    public void InvalidTemplate_CatchAllParamWithIncorrectPlacedAsterisks(
        string template,
    ...
    )
    {
        ...
        var expectedMessage =
            $"The route parameter name '{lastParameter}' is invalid. Route parameter names must be non-empty and cannot contain these characters: '{{', '}}', '/'. The '?' character marks a parameter as optional, and can occur only at the end of the parameter. The '*' character marks a parameter as catch-all, and can occur only at the start of the parameter.";
    ...
    private class ExpectedTemplateBuilder
    {
        ...
        public IList<RoutePatternPathSegment> Segments { get; set; } =
            new List<RoutePatternPathSegment>();
        ...
        public ExpectedTemplateBuilder Literal(string value)
        {
            ...
            Segments.Add(
                new RoutePatternPathSegment(
                    new List<RoutePatternPart> { new RoutePatternLiteralPart(value) }
                )
            );
            return this;
        }
        ...
        public static implicit operator RoutePattern(ExpectedTemplateBuilder builder) =>
            builder.Build();
    }
