...
public class QueryParameterValueSupplierTest
{
    ...
    {
        var query =
            $"?{nameof(IgnorableProperties.Invalid1)}=a&{nameof(IgnorableProperties.Invalid2)}=b&{nameof(IgnorableProperties.Valid)}=c";
        Assert.Collection(
            GetSuppliedParameters<IgnorableProperties>(query),
    ...
    public void SupportsExpectedValueTypes()
    {
        ...
        Assert.Collection(
            GetSuppliedParameters<ValidTypes>(query),
            AssertKeyValuePair(nameof(ValidTypes.BoolVal), true),
            AssertKeyValuePair(
                nameof(ValidTypes.DateTimeVal),
            ...
            AssertKeyValuePair(
                nameof(ValidTypes.GuidVal),
                new Guid("9e7257ad-03aa-42c7-9819-be08b177fef9")
            ),
    ...
    public void SupportsExpectedArrayTypes()
    {
        ...
        Assert.Collection(
            GetSuppliedParameters<ValidArrayTypes>(query),
            AssertKeyValuePair(nameof(ValidArrayTypes.BoolVals), new[] { true }),
            AssertKeyValuePair(
                nameof(ValidArrayTypes.DateTimeVals),
            ...
            AssertKeyValuePair(
                nameof(ValidArrayTypes.GuidVals),
    ...
    public void CanOverrideParameterName()
    {
        ...
        Assert.Collection(
            GetSuppliedParameters<OverrideParameterName>(query),
    ...
    public void AcceptsBlankValuesWhenNullable()
    {
        ...
        Assert.Collection(
            GetSuppliedParameters<ValidTypes>(query)
                .Where(pair => pair.key.StartsWith("Nullable", StringComparison.Ordinal)),
            AssertKeyValuePair(nameof(ValidTypes.NullableBoolVal), (object)null),
            AssertKeyValuePair(nameof(ValidTypes.NullableDateTimeVal), (object)null),
            AssertKeyValuePair(nameof(ValidTypes.NullableDecimalVal), (object)null),
            AssertKeyValuePair(nameof(ValidTypes.NullableDoubleVal), (object)null),
    ...
    public void EmptyStringValuesAreSuppliedAsEmptyString(string queryPart)
    {
        ...
        var suppliedParameters = GetSuppliedParameters<ValidTypes>(query)
    ...
    {
        var query =
            $"?{nameof(ValidArrayTypes.StringVals)}=a&"
            + $"{nameof(ValidArrayTypes.StringVals)}&"
            + $"{nameof(ValidArrayTypes.StringVals)}=&"
            + $"{nameof(ValidArrayTypes.StringVals)}=b";
        var suppliedParameters = GetSuppliedParameters<ValidArrayTypes>(query)
            .ToDictionary(x => x.key, x => x.value);
        Assert.Equal(
            new[] { "a", string.Empty, string.Empty, "b" },
            suppliedParameters[nameof(ValidArrayTypes.StringVals)]
        );
    }
    ...
    public void DecodesKeysAndValues()
    {
        ...
        Assert.Collection(
            GetSuppliedParameters<SpecialQueryParameterName>(query),
    ...
    public void MatchesKeysCaseInsensitively()
    {
        ...
        Assert.Collection(
            GetSuppliedParameters<KeyCaseMatching>(query),
    ...
    {
        var query =
            $"?{nameof(KeysWithNonAsciiChars.Имя_моей_собственности)}=first&خاصية_أخرى=second";
    ...
    public void ForNonArrayValuesOnlyOneValueIsSupplied()
    {
        ...
        Assert.Collection(
            GetSuppliedParameters<SingleValueOverwriting>(
                $"?age=123&age=456&age=789&id=1&id&name=Bobbins&name"
            ),
    ...
    private Action<(string key, object value)> AssertKeyValuePair<T>(
        string expectedKey,
        T expectedValue
    )
    {
