...
public class PullFromJSDataStreamTest
{
    ...
    class TestJSRuntime : IJSRuntime
    {
        ...
        public virtual ValueTask<TValue> InvokeAsync<
        ...
        >(string identifier, CancellationToken cancellationToken, object[] args)
        {
            ...
            if (typeof(TValue) != typeof(byte[]))
            {
                throw new ArgumentException(
                    $"Unexpected call to InvokeAsync, expected byte[] got {typeof(TValue)}"
                );
            }
            var offset = (long)args[1];
            var bytesToRead = (int)args[2];
            return ValueTask.FromResult(
                (TValue)(object)_data.Skip((int)offset).Take(bytesToRead).ToArray()
            );
        }
    ...
    {
        public TestJSRuntime_ProvidesInsufficientData(byte[] data)
            : base(data) { }
        ...
        >(string identifier, CancellationToken cancellationToken, object[] args)
        {
            ...
            var bytesToRead = (int)args[2];
            return ValueTask.FromResult(
                (TValue)(object)_data.Skip((int)offset).Take(bytesToRead - 1).ToArray()
            );
        }
    }
    ...
    {
        public TestJSRuntime_ProvidesExcessData(byte[] data)
            : base(data) { }
