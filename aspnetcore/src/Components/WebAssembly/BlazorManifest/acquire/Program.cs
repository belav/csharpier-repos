    ...
    class Program
    {
        ...
        static string GetDotnetPath()
        {
            ...
            return Path.GetFullPath(
                Path.Combine(
                    ...
                    "..",
                    "..",
                    "dotnet" + (OperatingSystem.IsWindows() ? ".exe" : "")
                )
            );
        }
        ...
        static int Main(string[] args)
        {
            ...
            try
            {
                ...
                var sdkVersionProc = Process.Start(
        ...
        static List<(string Id, string Version)> GetPacks(string sdkDirectory)
        {
            ...
            if (!Directory.Exists(manifestDirectory))
            {
                throw new DirectoryNotFoundException(
                    $"Cound not find directory {manifestDirectory}. A 6.0-preview3 SDK or newer is required for this tool to function."
                );
            }

            var manifestPath = Path.Combine(manifestDirectory, "WorkloadManifest.json");
            var manifest = JsonSerializer.Deserialize<PackInformation>(
        ...
        )
        {
            ...
            File.WriteAllText(
                Path.Combine(restoreProjectDirectory.FullName, "Directory.Build.props"),
                "<Project />"
            );
            File.WriteAllText(
                Path.Combine(restoreProjectDirectory.FullName, "Directory.Build.targets"),
                "<Project />"
            );
...
";
            ...
            var process = Process.Start(
                new ProcessStartInfo
                {
                    ...
                    RedirectStandardOutput = true,
#endif
                    Environment = { ["NUGET_PACKAGES"] = restoreDirectory, },
                }
            );
            process.WaitForExit();
            return 0;
