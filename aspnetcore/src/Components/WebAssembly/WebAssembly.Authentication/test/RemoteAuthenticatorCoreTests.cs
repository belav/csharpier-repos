...
public class RemoteAuthenticatorCoreTests
{
    private const string _action = nameof(
        RemoteAuthenticatorViewCore<RemoteAuthenticationState>.Action
    );
    private const string _onLogInSucceded = nameof(
        RemoteAuthenticatorViewCore<RemoteAuthenticationState>.OnLogInSucceeded
    );
    private const string _onLogOutSucceeded = nameof(
        RemoteAuthenticatorViewCore<RemoteAuthenticationState>.OnLogOutSucceeded
    );
    ...
    public async Task AuthenticationManager_Throws_ForInvalidAction()
    {
        ...
        var parameters = ParameterView.FromDictionary(
            new Dictionary<string, object> { [_action] = "" }
        );
    ...
    public async Task AuthenticationManager_Login_NavigatesToReturnUrlOnSuccess()
    {
        ...
        authServiceMock.SignInCallback = _ =>
            Task.FromResult(
        ...
        var parameters = ParameterView.FromDictionary(
            new Dictionary<string, object> { [_action] = RemoteAuthenticationActions.LogIn }
        );

        // Act
        await renderer.Dispatcher
            .InvokeAsync<object>(() => remoteAuthenticator.SetParametersAsync(parameters));
    ...
    public async Task AuthenticationManager_Login_DoesNothingOnRedirect()
    {
        ...
        authServiceMock.SignInCallback = s =>
            Task.FromResult(
                new RemoteAuthenticationResult<RemoteAuthenticationState>()
                {
    ...
    public async Task AuthenticationManager_Login_NavigatesToLoginFailureOnError()
    {
        ...
        Assert.Equal(
            ...
            remoteAuthenticator.Navigation.Uri.ToString()
        );
        Assert.Equal(
            "There was an error trying to log in.",
            remoteAuthenticator.Navigation.HistoryEntryState
        );
    }
    ...
    public async Task AuthenticationManager_LoginCallback_ThrowsOnRedirectResult()
    {
        ...
        authServiceMock.CompleteSignInCallback = s =>
            Task.FromResult(
                new RemoteAuthenticationResult<RemoteAuthenticationState>()
                {
                    Status = RemoteAuthenticationStatus.Redirect,
    ...
    public async Task AuthenticationManager_Callbacks_OnlyExecuteOncePerAction()
    {
        ...
        authServiceMock.CompleteSignOutCallback = s =>
            Task.FromResult(
                new RemoteAuthenticationResult<RemoteAuthenticationState>()
                {
                    Status = RemoteAuthenticationStatus.Success,
        ...
        // Act
        await renderer.Dispatcher
            .InvokeAsync<object>(() => remoteAuthenticator.SetParametersAsync(initialParameters));
        await renderer.Dispatcher
            .InvokeAsync<object>(() => remoteAuthenticator.SetParametersAsync(initialParameters));

        await renderer.Dispatcher
            .InvokeAsync<object>(() => remoteAuthenticator.SetParametersAsync(finalParameters));
        await renderer.Dispatcher
            .InvokeAsync<object>(() => remoteAuthenticator.SetParametersAsync(finalParameters));
    ...
    public async Task AuthenticationManager_Logout_NavigatesToReturnUrlOnSuccess()
    {
        ...
        authServiceMock.GetAuthenticatedUserCallback = () =>
            new ValueTask<ClaimsPrincipal>(new ClaimsPrincipal(new ClaimsIdentity("Test")));

        authServiceMock.SignOutCallback = s =>
            Task.FromResult(
                new RemoteAuthenticationResult<RemoteAuthenticationState>()
                {
                    Status = RemoteAuthenticationStatus.Success,
        ...
        var parameters = ParameterView.FromDictionary(
            new Dictionary<string, object> { [_action] = RemoteAuthenticationActions.LogOut }
        );
    ...
    public async Task AuthenticationManager_Logout_NavigatesToDefaultReturnUrlWhenNoReturnUrlIsPresent()
    {
        ...
        // Assert
        Assert.Equal(
            "https://www.example.com/base/authentication/logged-out",
            remoteAuthenticator.Navigation.Uri
        );
    }
    ...
    public static TheoryData<UIValidator> DisplaysRightUIData { get; } =
        new TheoryData<UIValidator>
        {
            {
                new UIValidator
                {
    ...
    public async Task AuthenticationManager_DisplaysRightUI_ForEachStateAsync(UIValidator validator)
    {
        ...
        var logger = new TestLoggerFactory(new TestSink(), false).CreateLogger<
            RemoteAuthenticatorViewCore<RemoteAuthenticationState>
        >();
        ...
        var parameters = ParameterView.FromDictionary(
            new Dictionary<string, object> { [_action] = validator.Action }
        );

        // Act
        await renderer.Dispatcher
            .InvokeAsync<object>(() => authenticator.SetParametersAsync(parameters));
    ...
    )
    {
        ...
        // Act
        Task result = await renderer.Dispatcher
            .InvokeAsync<Task>(() => authenticator.SetParametersAsync(parameters));

        // Assert
        Assert.Null(result.Exception);
    }

    public static TheoryData<UIValidator, string> DisplaysRightUIWhenPathsAreMissingData { get; } =
        new TheoryData<UIValidator, string>
        {
            // Profile fragment overrides
            {
                new UIValidator
    ...
    public class UIValidator
    {
        ...
        public Action<
            UIValidator,
            RemoteAuthenticatorViewCore<RemoteAuthenticationState>
        > SetupFakeRenderAction { get; set; }
        public Action<
            UIValidator,
            RemoteAuthenticatorViewCore<RemoteAuthenticationState>
        > RetrieveOriginalRenderAction { get; set; }
    ...
    )
    {
        ...
        var navigationManager = new TestNavigationManager(baseUri, currentState, currentUri);
...
#pragma warning restore CS0618 // Type or member is obsolete, we keep it for now for backwards compatibility
    ...
    public class TestRemoteAuthenticatorView
        : RemoteAuthenticatorViewCore<RemoteAuthenticationState>
    {
        ...
        >
    {
        ...
        > SignInCallback { get; set; }
        public Func<
            RemoteAuthenticationContext<RemoteAuthenticationState>,
            Task<RemoteAuthenticationResult<RemoteAuthenticationState>>
        > CompleteSignInCallback { get; set; }
        public Func<
            RemoteAuthenticationContext<RemoteAuthenticationState>,
            Task<RemoteAuthenticationResult<RemoteAuthenticationState>>
        > SignOutCallback { get; set; }
        public Func<
            RemoteAuthenticationContext<RemoteAuthenticationState>,
            Task<RemoteAuthenticationResult<RemoteAuthenticationState>>
        > CompleteSignOutCallback { get; set; }
        public Func<ValueTask<ClaimsPrincipal>> GetAuthenticatedUserCallback { get; set; }

        public override async Task<AuthenticationState> GetAuthenticationStateAsync() =>
            new AuthenticationState(await GetAuthenticatedUserCallback());

        public override Task<
            RemoteAuthenticationResult<RemoteAuthenticationState>
        > CompleteSignInAsync(RemoteAuthenticationContext<RemoteAuthenticationState> context) =>
            CompleteSignInCallback(context);

        protected internal override ValueTask<ClaimsPrincipal> GetAuthenticatedUser() =>
            GetAuthenticatedUserCallback();

        public override Task<
            RemoteAuthenticationResult<RemoteAuthenticationState>
        > CompleteSignOutAsync(RemoteAuthenticationContext<RemoteAuthenticationState> context) =>
            CompleteSignOutCallback(context);

        public override Task<RemoteAuthenticationResult<RemoteAuthenticationState>> SignInAsync(
            RemoteAuthenticationContext<RemoteAuthenticationState> context
        ) => SignInCallback(context);

        public override Task<RemoteAuthenticationResult<RemoteAuthenticationState>> SignOutAsync(
            RemoteAuthenticationContext<RemoteAuthenticationState> context
