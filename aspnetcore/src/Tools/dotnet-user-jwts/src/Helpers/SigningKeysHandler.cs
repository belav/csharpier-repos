...
internal static class SigningKeysHandler
{
    public static byte[] GetSigningKeyMaterial(string userSecretsId, string scheme, string issuer)
    {
        ...
        if (
            !string.IsNullOrEmpty(signingKey?.Value)
    ...
    public static byte[] GetOrCreateSigningKeyMaterial(
        string userSecretsId,
        string scheme,
        string issuer
    ) =>
        GetSigningKeyMaterial(userSecretsId, scheme, issuer)
        ?? CreateSigningKeyMaterial(userSecretsId, scheme, issuer);

    public static byte[] CreateSigningKeyMaterial(
        string userSecretsId,
        string scheme,
        string issuer,
    ...
    )
    {
        ...
        var newKeyMaterial = System.Security.Cryptography.RandomNumberGenerator.GetBytes(
            signingKeyLength
        );
        ...
        {
            using var secretsFileStream = new FileStream(
                secretsFilePath,
                FileMode.Open,
                FileAccess.Read
            );
        ...
        var key = new SigningKey(
            shortId,
            issuer,
            Convert.ToBase64String(newKeyMaterial),
            signingKeyLength
        );
        ...
        if (secrets.ContainsKey(signkingKeysPropertyName))
        {
            ...
            if (reset)
            {
                var toRemove = signingKeys.SingleOrDefault(key =>
                    key["Issuer"].GetValue<string>() == issuer
                );
        ...
        using var secretsWriteStream = new FileStream(
            secretsFilePath,
            FileMode.Create,
            FileAccess.Write
        );
