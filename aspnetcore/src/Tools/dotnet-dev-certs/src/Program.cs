...
namespace Microsoft.AspNetCore.DeveloperCertificates.Tools
{
    internal class Program
    {
        ...
        private const string InvalidUsageErrorMessage =
            @"Incompatible set of flags. Sample usages
...
'dotnet dev-certs https -ep ./certificate.crt -p password --trust --key-format Pem'";
        ...
        private static int CheckHttpsCertificate(CommandOption trust, IReporter reporter)
        {
            ...
            else
            {
                foreach (var certificate in certificates)
                {
                    ...
                    var status = certificateManager.CheckCertificateState(
                        certificate,
                        interactive: false
                    );
                    if (!status.Success)
            ...
            if (trust != null && trust.HasValue())
            {
                ...
                {
                    var store = RuntimeInformation.IsOSPlatform(OSPlatform.OSX)
                        ? StoreName.My
                        : StoreName.Root;
                    var trustedCertificates = certificateManager.ListCertificates(
                        store,
                        StoreLocation.CurrentUser,
                        isValid: true
                    );
    ...
    {string.Join(Environment.NewLine, certificates.Select(c => $"{c.Subject} - {c.Thumbprint}"))}"
                ...
                {
                    reporter.Warn(
                        "Checking the HTTPS development certificate trust status was requested. Checking whether the certificate is trusted or not is not supported on Linux distributions."
        ...
        private static int EnsureHttpsCertificate(
            ...
            if (trust?.HasValue() == true)
            {
                if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
                {
                    reporter.Warn(
                        "Trusting the HTTPS development certificate was requested. If the certificate is not "
                            + "already trusted we will run the following command:"
                            + Environment.NewLine
                            + "'sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain <<certificate>>'"
                            + Environment.NewLine
                            + "This command might prompt you for your password to install the certificate "
                ...
                {
                    reporter.Warn(
                        "Trusting the HTTPS development certificate was requested. Trusting the certificate on Linux distributions automatically is not supported. "
            ...
            switch (result)
            {
                case EnsureCertificateResult.Succeeded:
                    ...
                    {
                        reporter.Verbose(
                            $"The certificate was exported to {Path.GetFullPath(exportPath.Value())}"
                        );
                    }
                    return Success;
