...
public class CertificateManagerTests : IClassFixture<CertFixture>
{
    ...
    public void EnsureCreateHttpsCertificate_CreatesACertificate_WhenThereAreNoHttpsCertificates()
    {
        try
        {
            ...
            var httpsCertificates = _manager.ListCertificates(
                StoreName.My,
            ...
            Assert.Equal(
                ...
                exportedCertificate.GetCertHashString()
            );
        }
        catch (Exception e)
        {
    ...
    public void EnsureCreateHttpsCertificate_DoesNotCreateACertificate_WhenThereIsAnExistingHttpsCertificates()
    {
        ...
        var creation = _manager.EnsureAspNetCoreHttpsDevelopmentCertificate(
            now,
            now.AddYears(1),
            path: null,
            trust: false,
        ...
        var exportedCertificate = new X509Certificate2(
            File.ReadAllBytes(CertificateName),
            certificatePassword
        );
        Assert.NotNull(exportedCertificate);
    ...
    public void EnsureCreateHttpsCertificate_CanExportTheCertInPemFormat()
    {
        ...
        var result = _manager.EnsureAspNetCoreHttpsDevelopmentCertificate(
            ...
            includePrivateKey: true,
            password: certificatePassword,
            keyExportFormat: CertificateKeyExportFormat.Pem,
            isInteractive: false
        );
        ...
        var exportedCertificate = X509Certificate2.CreateFromEncryptedPemFile(
            CertificateName,
            certificatePassword,
            Path.ChangeExtension(CertificateName, "key")
        );
        Assert.NotNull(exportedCertificate);
        Assert.True(exportedCertificate.HasPrivateKey);
    ...
    public void EnsureCreateHttpsCertificate_CanImport_ExportedPfx()
    {
        ...
        var importedCertificate = Assert.Single(
    ...
    public void EnsureCreateHttpsCertificate_CanExportTheCertInPemFormat_WithoutPassword()
    {
        ...
        var exportedCertificate = X509Certificate2.CreateFromPemFile(
            CertificateName,
            Path.ChangeExtension(CertificateName, "key")
        );
        Assert.NotNull(exportedCertificate);
    ...
    public void EnsureCreateHttpsCertificate_ReturnsExpiredCertificateIfVersionIsIncorrect()
    {
        ...
        var httpsCertificateList = _manager.ListCertificates(
            StoreName.My,
            StoreLocation.CurrentUser,
            isValid: true
        );
    ...
    public void ListCertificates_AlwaysReturnsTheCertificate_WithHighestVersion()
    {
        ...
        _manager.AspNetHttpsCertificateVersion = 2;
        creation = _manager.EnsureAspNetCoreHttpsDevelopmentCertificate(
            now,
            now.AddYears(1),
            path: null,
            trust: false,
    ...
    [OSSkipCondition(
        ...
        SkipReason = "https://github.com/dotnet/aspnetcore/issues/6720"
    )]
    public void EnsureCreateHttpsCertificate_CreatesFilesWithUserOnlyUnixFileMode()
    {
        ...
        const string CertificateName =
            nameof(EnsureCreateHttpsCertificate_CreatesFilesWithUserOnlyUnixFileMode) + ".pem";
        const string KeyName =
            nameof(EnsureCreateHttpsCertificate_CreatesFilesWithUserOnlyUnixFileMode) + ".key";
        ...
        Assert.Equal(
            ...
            File.GetUnixFileMode(CertificateName)
        );
