...
public class BlazorServerTemplateTest : BlazorTemplateTest
{
    ...
    [Fact]
    public Task BlazorServerTemplate_NoHttps_Works_NoAuth() =>
    ...
    [Fact]
    public Task BlazorServerTemplate_NoHttps_Works_ProgamMainNoAuth() =>
        CreateBuildPublishAsync(args: new[] { ArgConstants.UseProgramMain, ArgConstants.NoHttps });
    ...
    [SkipOnHelix("https://github.com/dotnet/aspnetcore/issues/30825", Queues = "All.OSX")]
    public Task BlazorServerTemplateWorks_IndividualAuth_NoHttps(string auth, string[] args) =>
        CreateBuildPublishAsync(auth, args: args);
    ...
    [InlineData(
        "Individual",
        new[] { ArgConstants.UseProgramMain, ArgConstants.UseLocalDb, ArgConstants.NoHttps }
    )]
    [OSSkipCondition(
        OperatingSystems.Linux | OperatingSystems.MacOSX,
        SkipReason = "No LocalDb on non-Windows"
    )]
    public Task BlazorServerTemplateWorks_IndividualAuth_NoHttps_LocalDb(
        string auth,
        string[] args
    ) => CreateBuildPublishAsync(auth, args: args);
    ...
    public Task BlazorServerTemplate_IdentityWeb_BuildAndPublish_IndividualB2C_NoHttps(
        string auth,
        string[] args
    ) => CreateBuildPublishAsync(auth, args);
    ...
    public Task BlazorServerTemplate_IdentityWeb_BuildAndPublish_SingleOrg_NoHttps(
        string auth,
        string[] args
    ) => CreateBuildPublishAsync(auth, args);
}
