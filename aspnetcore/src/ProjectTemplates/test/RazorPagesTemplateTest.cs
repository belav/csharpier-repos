    ...
    public class RazorPagesTemplateTest : LoggedTest
    {
        ...
        public async Task RazorPagesTemplate_NoAuth()
        {
            ...
            Assert.True(
                ...
                ErrorMessages.GetFailedProcessMessage("razor", project, createResult)
            );
            ...
            Assert.True(
                ...
                ErrorMessages.GetFailedProcessMessage("publish", project, createResult)
            );
            ...
            Assert.True(
                ...
                ErrorMessages.GetFailedProcessMessage("build", project, createResult)
            );
        ...
        public async Task RazorPagesTemplate_IndividualAuth(bool useLocalDB)
        {
            var project = await ProjectFactory.GetOrCreateProject(
                "razorpagesindividual" + (useLocalDB ? "uld" : ""),
                Output
            );
        ...
        {
            var project = await BuildAndPublishRazorPagesTemplate(
                auth: null,
                new[] { "--razor-runtime-compilation" }
            );

            Assert.False(
                Directory.Exists(Path.Combine(project.TemplatePublishDir, "refs")),
                "The refs directory should not be published."
            );

            // Verify ref assemblies isn't published
            var refsDirectory = Path.Combine(project.TemplatePublishDir, "refs");
            Assert.False(
                Directory.Exists(refsDirectory),
                $"{refsDirectory} should not be in the publish output."
            );
        }
        ...
        public Task RazorPagesTemplate_IdentityWeb_BuildsAndPublishes(string auth, string[] args) =>
            BuildAndPublishRazorPagesTemplate(auth: auth, args: args);

        [ConditionalTheory]
        [InlineData("SingleOrg", new string[] { "--calls-graph" })]
        [QuarantinedTest("https://github.com/dotnet/aspnetcore/issues/31729")]
        public Task RazorPagesTemplate_IdentityWeb_BuildsAndPublishes_WithSingleOrg(
            string auth,
            string[] args
        ) => BuildAndPublishRazorPagesTemplate(auth: auth, args: args);
        ...
        {
            var project = await ProjectFactory.GetOrCreateProject(
                "razorpages" + Guid.NewGuid().ToString().Substring(0, 10).ToLowerInvariant(),
                Output
            );
