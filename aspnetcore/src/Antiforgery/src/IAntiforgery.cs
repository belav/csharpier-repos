...
public interface IAntiforgery
{
    ...
    /// in the response. This operation also sets the "Cache-control" and "Pragma" headers to "no-cache"
    // and
    ...
    /// <summary>
    /// Asynchronously returns a value indicating whether the request passes antiforgery validation. If
    // the
    /// request uses a safe HTTP method (GET, HEAD, OPTIONS, TRACE), the antiforgery token is not
    // validated.
    /// </summary>
    /// <param name="httpContext">The <see cref="HttpContext"/> associated with the current
    // request.</param>
    /// <returns>
    /// A <see cref="Task{Boolean}"/> that, when completed, returns <c>true</c> if the request uses a
    // safe HTTP
