...
public abstract class ScaleoutHubLifetimeManagerTests<TBackplane> : HubLifetimeManagerTestsBase<Hub>
{
    ...
    public async Task InvokeGroupAsyncOnServerWithoutConnectionWritesOutputToGroupConnection()
    {
        ...
        using (var client = new TestClient())
        {
            ...
            await manager2
                .SendGroupAsync("name", "Hello", new object[] { "World" })
                .DefaultTimeout();
    ...
    public async Task WritingToRemoteConnectionThatFailsDoesNotThrow()
    {
        ...
        using (var client = new TestClient())
        {
            ...
            await manager1
                .SendConnectionAsync(connectionMock.ConnectionId, "Hello", new object[] { "World" })
                .DefaultTimeout();
        }
    }
    ...
    public async Task InvokeUserSendsToAllConnectionsForUser()
    {
        ...
        using (var client3 = new TestClient())
        {
            ...
            await manager
                .SendUserAsync("userA", "Hello", new object[] { "World" })
                .DefaultTimeout();
            await AssertMessageAsync(client1);
            await AssertMessageAsync(client2);
        }
    ...
    public async Task InvocationsFromDifferentServersUseUniqueIDs()
    {
        ...
        using (var client = new TestClient())
        {
            ...
            var invoke2 = manager2.InvokeConnectionAsync<int>(
                connection.ConnectionId,
                "Result",
                new object[] { "test" },
                cancellationToken: default
            ...
            await manager1
                .SetConnectionResultAsync(
                    connection.ConnectionId,
                    CompletionMessage.WithResult(invocation2.InvocationId, 2)
                )
                .DefaultTimeout();
            await manager2
                .SetConnectionResultAsync(
                    connection.ConnectionId,
                    CompletionMessage.WithResult(invocation1.InvocationId, 5)
                )
                .DefaultTimeout();
    ...
    public async Task ClientReturnResultAcrossServersWithWrongReturnedTypeErrors()
    {
        ...
        using (var client1 = new TestClient())
        {
            ...
            await manager1
                .SetConnectionResultAsync(
                    connection1.ConnectionId,
                    CompletionMessage.WithResult(invocation.InvocationId, "wrong type")
                )
                .DefaultTimeout();
