...
public partial class HubConnectionHandlerTests
{
    ...
    public async Task CanReturnClientResultErrorToHub()
    {
        using (StartVerifiableLog(write => write.EventId.Name == "FailedInvokingHubMethod"))
        {
            ...
            using (var client = new TestClient())
            {
                ...
                Assert.Equal(
                    "An unexpected error occurred invoking 'GetClientResult' on the server. HubException: Client error",
                    completion.Error
                );
                Assert.Equal(invocationId, completion.InvocationId);
            }
    ...
    public async Task ThrowsWhenUsedInOnConnectedAsync()
    {
        using (StartVerifiableLog(write => write.EventId.Name == "ErrorDispatchingHubEvent"))
        {
            ...
            var connectionHandler = serviceProvider.GetService<
                HubConnectionHandler<OnConnectedClientResultHub>
            >();
            ...
            using (var client = new TestClient())
            {
                ...
                // Hub asks client for a result, this is an invocation message with an ID
                var closeMessage = Assert.IsType<CloseMessage>(
                    await client.ReadAsync().DefaultTimeout()
                );
                Assert.Equal(
                    "Connection closed with an error. InvalidOperationException: Client results inside OnConnectedAsync Hub methods are not allowed.",
                    closeMessage.Error
                );
            }
        }
        ...
        Assert.Single(
            TestSink.Writes.Where(write => write.EventId.Name == "ErrorDispatchingHubEvent")
        );
    }
    ...
    public async Task ThrowsWhenUsedInOnDisconnectedAsync()
    {
        using (StartVerifiableLog(write => write.EventId.Name == "ErrorDispatchingHubEvent"))
        {
            ...
            var connectionHandler = serviceProvider.GetService<
                HubConnectionHandler<OnDisconnectedClientResultHub>
            >();
            ...
            using (var client = new TestClient())
            {
                ...
                var ex = await Assert
                    .ThrowsAsync<IOException>(() => connectionHandlerTask)
                    .DefaultTimeout();
                Assert.Equal(
                    $"Connection '{client.Connection.ConnectionId}' disconnected.",
                    ex.Message
                );
            }
        }
    ...
    public async Task CanUseClientResultsWithIHubContext()
    {
        using (StartVerifiableLog())
        {
            ...
            var resultTask = context
                .Clients.Client(client.Connection.ConnectionId)
    ...
    public async Task CanReturnClientResultToTypedHubTwoWays()
    {
        using (StartVerifiableLog())
        {
            ...
            using var client = new TestClient(
                invocationBinder: new GetClientResultTwoWaysInvocationBinder()
            );
            ...
            using (var client = new TestClient())
            {
                ...
                _ = await client
                    .SendHubMessageAsync(
                        new InvocationMessage(
                            "1",
                            nameof(MethodHub.BlockingMethod),
                            Array.Empty<object>()
                        )
                    )
                    .DefaultTimeout();
                ...
                var invocationId = await client
                    ...
                    .DefaultTimeout();
                var invocationId2 = await client
                    .SendHubMessageAsync(
                        new InvocationMessage(
                            "3",
                            nameof(MethodHub.GetClientResult),
                    ...
                    .DefaultTimeout();
                var invocationId3 = await client
                    .SendHubMessageAsync(
                        new InvocationMessage(
                            "4",
                            nameof(MethodHub.GetClientResult),
                ...
                var invocationMessage2 = Assert.IsType<InvocationMessage>(
                    await client.ReadAsync().DefaultTimeout()
                );
                var invocationMessage3 = Assert.IsType<InvocationMessage>(
                    await client.ReadAsync().DefaultTimeout()
                );
                ...
                await client
                    .SendHubMessageAsync(
                        CompletionMessage.WithResult(invocationMessage2.InvocationId, res)
                    )
                    .DefaultTimeout();
                completion = Assert.IsType<CompletionMessage>(
                    await client.ReadAsync().DefaultTimeout()
                );
                ...
                await client
                    .SendHubMessageAsync(
                        CompletionMessage.WithResult(invocationMessage3.InvocationId, res)
                    )
                    .DefaultTimeout();
                completion = Assert.IsType<CompletionMessage>(
                    await client.ReadAsync().DefaultTimeout()
    ...
    public async Task ClientResultFromBackgroundThreadInHubMethodWorks()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var completionMessage = await client
                    .InvokeAsync(nameof(MethodHub.BackgroundClientResult))
                    .DefaultTimeout();
                ...
                completionMessage = await client
                    .InvokeAsync(nameof(MethodHub.ValueMethod))
                    .DefaultTimeout();
            ...
            using (var client = new TestClient())
            {
                ...
                var invocationId = await client
                    .BeginUploadStreamAsync(
                        "1",
                        nameof(MethodHub.GetClientResultWithStream),
                        new[] { streamId },
                        Array.Empty<object>()
                    )
                    .DefaultTimeout();
    ...
    public async Task ClientResultInStreamingMethodWorks()
    {
        using (StartVerifiableLog())
        {
            ...
            invocationBinder
                ...
                .Returns(typeof(int));
            invocationBinder
                .Setup(b => b.GetParameterTypes(It.IsAny<string>()))
            ...
            using (var client = new TestClient(invocationBinder: invocationBinder.Object))
            {
                ...
                var invocationId = await client
                    .SendStreamInvocationAsync(nameof(MethodHub.StreamWithClientResult))
                    .DefaultTimeout();
                ...
                var streamItem = Assert.IsType<StreamItemMessage>(
                    await client.ReadAsync().DefaultTimeout()
                );
