...
partial public class HubConnectionHandlerTests : VerifiableLoggedTest
{
    [Fact]
        ...
        using (StartVerifiableLog())
        {
            ...
            var serviceProvider = HubConnectionHandlerTestUtils.CreateServiceProvider(
                services =>
                    services.Configure<HubOptions>(
                        options =>
                            options.KeepAliveInterval = TimeSpan.FromMilliseconds(intervalInMS)
                    ),
                LoggerFactory
            );
            var connectionHandler = serviceProvider.GetService<HubConnectionHandler<MethodHub>>();
        ...
        using (StartVerifiableLog())
        {
            ...
            var serviceProvider = HubConnectionHandlerTestUtils.CreateServiceProvider(
                services =>
                    services.Configure<HubOptions>(
                        options =>
                            options.ClientTimeoutInterval = TimeSpan.FromMilliseconds(timeoutInMS)
                    ),
                LoggerFactory
            );
            var connectionHandler = serviceProvider.GetService<HubConnectionHandler<MethodHub>>();
