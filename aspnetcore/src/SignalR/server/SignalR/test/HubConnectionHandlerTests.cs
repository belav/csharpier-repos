...
public partial class HubConnectionHandlerTests : VerifiableLoggedTest
{
    ...
    public async Task HubsAreDisposed()
    {
        using (StartVerifiableLog())
        {
            ...
            var serviceProvider = HubConnectionHandlerTestUtils.CreateServiceProvider(
                s => s.AddSingleton(trackDispose),
                LoggerFactory
            );
            var connectionHandler = serviceProvider.GetService<
                HubConnectionHandler<DisposeTrackingHub>
            >();
    ...
    public async Task AsyncDisposablesInHubsAreSupported()
    {
        using (StartVerifiableLog())
        {
            ...
            var connectionHandler = serviceProvider.GetService<
                HubConnectionHandler<HubWithAsyncDisposable>
            >();
            ...
            using (var client = new TestClient())
            {
                ...
                var result = (
                    await client.InvokeAsync(nameof(HubWithAsyncDisposable.Test)).DefaultTimeout()
                );
    ...
    public async Task ConnectionAbortedTokenTriggers()
    {
        using (StartVerifiableLog())
        {
            ...
            var serviceProvider = HubConnectionHandlerTestUtils.CreateServiceProvider(
                s => s.AddSingleton(state),
                LoggerFactory
            );
            var connectionHandler = serviceProvider.GetService<
                HubConnectionHandler<ConnectionLifetimeHub>
            >();
    ...
    public async Task OnDisconnectedAsyncTriggersWhenAbortedTokenCallbackThrows()
    {
        using (StartVerifiableLog())
        {
            ...
            var connectionHandler = serviceProvider.GetService<
                HubConnectionHandler<ErrorInAbortedTokenHub>
            >();
    ...
    public async Task HandshakeFailureFromUnknownProtocolSendsResponseWithError()
    {
        using (StartVerifiableLog())
        {
            ...
            var connectionHandler = HubConnectionHandlerTestUtils.GetHubConnectionHandler(
                typeof(HubT),
            ...
            using (var client = new TestClient(protocol: hubProtocolMock.Object))
            {
                ...
                Assert.NotNull(client.HandshakeResponseMessage);
                Assert.Equal(
                    "The protocol 'CustomProtocol' is not supported.",
                    client.HandshakeResponseMessage.Error
                );
    ...
    public async Task HandshakeFailureFromUnsupportedFormatSendsResponseWithError()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient(protocol: new MessagePackHubProtocol()))
            {
                ...
                Assert.NotNull(client.HandshakeResponseMessage);
                Assert.Equal(
                    "Cannot use the 'messagepack' protocol on the current transport. The transport does not support 'Binary' transfer format.",
                    client.HandshakeResponseMessage.Error
                );
    ...
    public async Task HubMessageOverTheMaxMessageSizeThrows()
    {
        ...
        using (StartVerifiableLog())
        {
            var connectionHandler = HubConnectionHandlerTestUtils.GetHubConnectionHandler(
                ...
                services =>
                    services
                        .AddSignalR()
                        .AddHubOptions<HubT>(o => o.MaximumReceiveMessageSize = maximumMessageSize)
            );
        ...
        Assert.Equal(
            ...
            $"The maximum message size of {maximumMessageSize}B was exceeded. The message size can be configured in AddHubOptions."
        );
    }
        ...
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var completionMessage =
                    await client.ReadAsync().DefaultTimeout() as CompletionMessage;
                Assert.NotNull(completionMessage);
                Assert.Equal("1", completionMessage.InvocationId);
                ...
                CloseMessage closeMessage =
                    await client.ReadAsync().DefaultTimeout() as CloseMessage;
    ...
    public async Task HandshakeFailureFromIncompatibleProtocolVersionSendsResponseWithError()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient(protocol: hubProtocolMock.Object))
            {
                ...
                Assert.NotNull(client.HandshakeResponseMessage);
                Assert.Equal(
                    "The server does not support version 9001 of the 'json' protocol.",
                    client.HandshakeResponseMessage.Error
                );
        ...
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                mockLifetimeManager.Verify(
                    m => m.OnConnectedAsync(It.IsAny<HubConnectionContext>()),
                    Times.Once
                );
                mockLifetimeManager.Verify(
                    m => m.OnDisconnectedAsync(It.IsAny<HubConnectionContext>()),
                    Times.Once
                );
    ...
    public async Task LifetimeManagerOnDisconnectedAsyncCalledIfHubOnDisconnectedAsyncThrows()
    {
        ...
        using (StartVerifiableLog(ExpectedErrors))
        {
            ...
            var connectionHandler = serviceProvider.GetService<
                HubConnectionHandler<OnDisconnectedThrowsHub>
            >();
    ...
    public async Task HubMethodCanThrowOrYieldFailedTask(string methodName, bool detailedErrors)
    {
        ...
        using (StartVerifiableLog(ExpectedErrors))
        {
            ...
            using (var client = new TestClient())
            {
                ...
                {
                    Assert.Equal(
                        $"An unexpected error occurred invoking '{methodName}' on the server. InvalidOperationException: BOOM!",
                        message.Error
                    );
                }
                else
    ...
    public async Task DetailedExceptionEvenWhenNotExplicitlySet()
    {
        ...
        using (StartVerifiableLog(ExpectedErrors))
        {
            ...
            using (var client = new TestClient())
            {
                ...
                Assert.Equal(
                    $"An unexpected error occurred invoking '{methodName}' on the server. HubException: This is a hub exception",
                    message.Error
                );
    ...
    public async Task HubMethodDoesNotSendResultWhenInvocationIsNonBlocking()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                await client
                    .SendInvocationAsync(nameof(MethodHub.ValueMethod), nonBlocking: true)
                    .DefaultTimeout();
    ...
    public async Task NonBlockingInvocationDoesNotSendCompletion(string methodName)
    {
        bool ExpectedErrors(WriteContext writeContext)
        {
            return methodName == nameof(MethodHub.MethodThatThrows)
                && writeContext.LoggerName
                    == "Microsoft.AspNetCore.SignalR.Internal.DefaultHubDispatcher"
                && writeContext.EventId.Name == "FailedInvokingHubMethod";
        }
        ...
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var result = await client
                    .InvokeAsync(nameof(MethodHub.OnDisconnectedAsync))
                    .DefaultTimeout();

                Assert.Equal(
                    "Failed to invoke 'OnDisconnectedAsync' due to an error on the server. HubException: Method does not exist.",
                    result.Error
                );
    ...
    public void HubsCannotHaveOverloadedMethods()
    {
        using (StartVerifiableLog())
        {
            ...
            {
                var connectionHandler = serviceProvider.GetService<
                    HubConnectionHandler<InvalidHub>
                >();
                Assert.True(false);
            }
    ...
    public async Task CannotCallStaticHubMethods()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var result = await client
                    .InvokeAsync(nameof(MethodHub.StaticMethod))
                    .DefaultTimeout();

                Assert.Equal(
                    "Failed to invoke 'StaticMethod' due to an error on the server. HubException: Method does not exist.",
                    result.Error
                );
    ...
    public async Task CannotCallObjectMethodsOnHub()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                Assert.Equal(
                    "Failed to invoke 'ToString' due to an error on the server. HubException: Method does not exist.",
                    result.Error
                );

                result = await client.InvokeAsync(nameof(MethodHub.GetHashCode)).DefaultTimeout();
                Assert.Equal(
                    "Failed to invoke 'GetHashCode' due to an error on the server. HubException: Method does not exist.",
                    result.Error
                );

                result = await client.InvokeAsync(nameof(MethodHub.Equals)).DefaultTimeout();
                Assert.Equal(
                    "Failed to invoke 'Equals' due to an error on the server. HubException: Method does not exist.",
                    result.Error
                );

                result = await client
                    .InvokeAsync(nameof(MethodHub.ReferenceEquals))
                    .DefaultTimeout();
                Assert.Equal(
                    "Failed to invoke 'ReferenceEquals' due to an error on the server. HubException: Method does not exist.",
                    result.Error
                );
    ...
    public async Task CannotCallDisposeMethodOnHub()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                Assert.Equal(
                    "Failed to invoke 'Dispose' due to an error on the server. HubException: Method does not exist.",
                    result.Error
                );
    ...
    public async Task BroadcastHubMethodSendsToAllClients(Type hubType)
    {
        using (StartVerifiableLog())
        {
            ...
            using (var secondClient = new TestClient())
            {
                ...
                await firstClient
                    .SendInvocationAsync(nameof(MethodHub.BroadcastMethod), "test")
                    .DefaultTimeout();

                foreach (
                    var result in await Task.WhenAll(
                ...
                await Task.WhenAll(firstConnectionHandlerTask, secondConnectionHandlerTask)
                    .DefaultTimeout();
            }
        }
    }
        ...
        using (StartVerifiableLog())
        {
            ...
            using (var secondClient = new TestClient())
            {
                ...
                )
                {
                    ...
                    Assert.Single(invocation.Arguments);
                    var values = ((JArray)invocation.Arguments[0])
    ...
    public async Task SendToOthers(Type hubType)
    {
        using (StartVerifiableLog())
        {
            ...
            using (var secondClient = new TestClient())
            {
                ...
                await secondClient
                    .SendInvocationAsync("BroadcastMethod", "To everyone")
                    .DefaultTimeout();
    ...
    public async Task FailsToInitializeInvalidTypedHub()
    {
        ...
        {
            var connectionHandler = HubConnectionHandlerTestUtils.GetHubConnectionHandler(
                typeof(SimpleVoidReturningTypedHub),
                loggerFactory: LoggerFactory
            );
    ...
    public async Task SendToAllExcept(Type hubType)
    {
        using (StartVerifiableLog())
        {
            ...
            using (var thirdClient = new TestClient())
            {
                ...
                await firstClient
                    .SendInvocationAsync("SendToAllExcept", "To second", excludeThirdClientId)
                    .DefaultTimeout();
                await firstClient
                    .SendInvocationAsync("SendToAllExcept", "To third", excludeSecondClientId)
                    .DefaultTimeout();
                ...
                await Task.WhenAll(
                        firstConnectionHandlerTask,
                        secondConnectionHandlerTask,
                        thirdConnectionHandlerTask
                    )
                    .DefaultTimeout();
            }
        }
    ...
    public async Task SendToMultipleUsers(Type hubType)
    {
        using (StartVerifiableLog())
        {
            ...
            using (var thirdClient = new TestClient(userIdentifier: "userC"))
            {
                ...
                await firstClient
                    .SendInvocationAsync(
                        nameof(MethodHub.SendToMultipleUsers),
                        new[] { "userB", "userC" },
                        "Second and Third"
                    )
                    .DefaultTimeout();
    ...
    public async Task SendToOthersInGroup(Type hubType)
    {
        using (StartVerifiableLog())
        {
            ...
            using (var secondClient = new TestClient())
            {
                ...
                await firstClient
                    .SendInvocationAsync("SendToOthersInGroup", "testGroup", "test")
                    .DefaultTimeout();
    ...
    public async Task InvokeMultipleGroups(Type hubType)
    {
        using (StartVerifiableLog())
        {
            ...
            using (var secondClient = new TestClient())
            {
                ...
                await secondClient
                    .InvokeAsync(nameof(MethodHub.GroupAddMethod), "GroupA")
                    .DefaultTimeout();
                await firstClient
                    .InvokeAsync(nameof(MethodHub.GroupAddMethod), "GroupB")
                    .DefaultTimeout();

                var groupNames = new List<string> { "GroupA", "GroupB" };
                await firstClient
                    .SendInvocationAsync(nameof(MethodHub.SendToMultipleGroups), "test", groupNames)
                    .DefaultTimeout();
    ...
    public async Task RemoveFromGroupWhenNotInGroupDoesNotFail()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                await client
                    .SendInvocationAsync(nameof(MethodHub.GroupRemoveMethod), "testGroup")
                    .DefaultTimeout();
    ...
    public async Task HubsCanSendToUser(Type hubType)
    {
        using (StartVerifiableLog())
        {
            ...
            using (var secondClient = new TestClient(userIdentifier: "userB"))
            {
                ...
                await firstClient
                    .SendInvocationAsync("ClientSendMethod", "userB", "test")
                    .DefaultTimeout();
        ...
        using (StartVerifiableLog())
        {
            ...
            )
            {
                ...
                HubConnectionHandlerTestUtils.AssertHubMessage(
                    new StreamItemMessage(string.Empty, "0"),
                    messages[0]
                );
                HubConnectionHandlerTestUtils.AssertHubMessage(
                    new StreamItemMessage(string.Empty, "1"),
                    messages[1]
                );
                HubConnectionHandlerTestUtils.AssertHubMessage(
                    new StreamItemMessage(string.Empty, "2"),
                    messages[2]
                );
                HubConnectionHandlerTestUtils.AssertHubMessage(
                    new StreamItemMessage(string.Empty, "3"),
                    messages[3]
                );
                HubConnectionHandlerTestUtils.AssertHubMessage(
                    CompletionMessage.Empty(string.Empty),
                    messages[4]
                );
            ...
            using (var client = new TestClient())
            {
                ...
                await client.SendHubMessageAsync(
                    new StreamInvocationMessage(
                        invocationId,
                        nameof(StreamingHub.BlockingStream),
                        Array.Empty<object>()
                    )
                );
                ...
                await client
                    .SendHubMessageAsync(new CancelInvocationMessage(invocationId))
                    .DefaultTimeout();

                var hubMessage = Assert.IsType<CompletionMessage>(
                    await client.ReadAsync().DefaultTimeout()
                );
    ...
    [Theory]
    [InlineData(
        nameof(StreamingHub.ExceptionAsyncEnumerable),
        "Exception: Exception from async enumerable"
    )]
    ...
    [InlineData(
        nameof(StreamingHub.ChannelClosedExceptionStream),
        "ChannelClosedException: ChannelClosedException from channel"
    )]
    [InlineData(nameof(StreamingHub.ChannelClosedExceptionStream), null)]
    [InlineData(
        nameof(StreamingHub.ChannelClosedExceptionInnerExceptionStream),
        "Exception: ChannelClosedException from channel"
    )]
    ...
    )
    {
        ...
        using (StartVerifiableLog(ExpectedErrors))
        {
            ...
            using (var client = new TestClient())
            {
                ...
                {
                    Assert.Equal(
                        $"An error occurred on the server while streaming results. {detailedError}",
                        completion.Error
                    );
                }
                else
        ...
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var message = await client
                    .InvokeAsync(nameof(MethodHub.AuthMethod))
                    .DefaultTimeout();
    ...
    private class TestAuthHandler : IAuthorizationHandler
    {
        public Task HandleAsync(AuthorizationHandlerContext context)
        {
            ...
            Assert.Equal(
                typeof(MethodHub).GetMethod(nameof(MethodHub.MultiParamAuthMethod)),
                resource.HubMethod
            );
    ...
    public async Task HubMethodWithAuthorizationProvidesResourceToAuthHandlers()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var message = await client
                    .InvokeAsync(nameof(MethodHub.MultiParamAuthMethod), "Hello", "World!")
                    .DefaultTimeout();
    ...
    public async Task HubOptionsCanUseCustomMessagePackSettings()
    {
        using (StartVerifiableLog())
        {
            ...
            var msgPackOptions = serviceProvider.GetRequiredService<
                IOptions<MessagePackHubProtocolOptions>
            >();
            using (
                var client = new TestClient(protocol: new MessagePackHubProtocol(msgPackOptions))
            )
            {
    ...
    public async Task CanGetHttpContextFromHubConnectionContext()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var feature = new TestHttpContextFeature { HttpContext = httpContext };
    ...
    public async Task WritesPingMessageIfNothingWrittenWhenKeepAliveIntervalElapses()
    {
        using (StartVerifiableLog())
        {
            ...
            var serviceProvider = HubConnectionHandlerTestUtils.CreateServiceProvider(
                services =>
                    services.Configure<HubOptions>(options => options.KeepAliveInterval = interval),
                LoggerFactory
            );
            var connectionHandler = serviceProvider.GetService<HubConnectionHandler<MethodHub>>();
    ...
    public async Task ConnectionNotTimedOutIfClientNeverPings()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient(new NewtonsoftJsonHubProtocol()))
            {
                ...
                var id = await client
                    .SendInvocationAsync(nameof(MethodHub.ValueMethod))
                    .DefaultTimeout();
    ...
    internal class PipeReaderWrapper : PipeReader
    {
        ...
        public PipeReaderWrapper(PipeReader pipeReader)
        {
            ...
            _waitForRead = new TaskCompletionSource(
        ...
        public override void CancelPendingRead() => _originalPipeReader.CancelPendingRead();
    ...
    public async Task ErrorInHubOnConnectSendsCloseMessageWithError(bool detailedErrors)
    {
        ...
        using (StartVerifiableLog(ExpectedErrors))
        {
            ...
            using (var client = new TestClient(new NewtonsoftJsonHubProtocol()))
            {
                ...
                {
                    Assert.Equal(
                        "Connection closed with an error. InvalidOperationException: Hub OnConnected failed.",
                        closeMessage.Error
                    );
                }
                else
    ...
    public async Task StreamingInvocationsDoNotBlockOtherInvocations()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient(new NewtonsoftJsonHubProtocol()))
            {
                ...
                var completion = await client
                    .InvokeAsync(nameof(StreamingHub.NonStream))
                    .DefaultTimeout();
    ...
    public async Task StreamMethodThatThrowsWillCleanup()
    {
        ...
        using (StartVerifiableLog(ExpectedErrors))
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var hubActivator =
                    serviceProvider.GetService<IHubActivator<StreamingHub>>()
                    as CustomHubActivator<StreamingHub>;
    ...
    public async Task InvocationsRunInOrderWithNoParallelism()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                await client
                    .SendInvocationAsync(
                        nameof(LongRunningHub.LongRunningMethod),
                        nonBlocking: false
                    )
                    .DefaultTimeout();
                ...
                await client
                    .SendInvocationAsync(nameof(LongRunningHub.SimpleMethod), nonBlocking: false)
                    .DefaultTimeout();
        ...
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                for (var i = 0; i < 5; i++)
                {
                    ...
                    await client
                        .SendInvocationAsync(
                            nameof(LongRunningHub.SimpleMethod),
                            nonBlocking: false
                        )
                        .DefaultTimeout();
    ...
    public async Task StreamInvocationsDoNotBlockOtherInvocations()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var hubActivator =
                    serviceProvider.GetService<IHubActivator<LongRunningHub>>()
                    as CustomHubActivator<LongRunningHub>;

                await client
                    .SendHubMessageAsync(new CancelInvocationMessage(streamInvocationId))
                    .DefaultTimeout();
    ...
    public async Task ServerSendsCloseWithErrorWhenConnectionClosedWithPartialMessage()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                Assert.Equal(
                    "Connection closed with an error. InvalidDataException: Connection terminated while reading a message.",
                    closeMessage.Error
                );
        ...
        using (var client = new TestClient())
        {
            ...
            await client
                .BeginUploadStreamAsync(
                    "invocationId",
                    nameof(MethodHub.StreamDontRead),
                    new[] { "id" },
                    Array.Empty<object>()
                )
                .DefaultTimeout();
            ...
            {
                await client
                    .SendHubMessageAsync(new StreamItemMessage("id", letter))
                    .DefaultTimeout();
            }
    ...
    public async Task UploadStringsToConcat()
    {
        ...
        using (var client = new TestClient())
        {
            var connectionHandlerTask = await client
                .ConnectAsync(connectionHandler)
                .DefaultTimeout();
            await client.BeginUploadStreamAsync(
                "invocation",
                nameof(MethodHub.StreamingConcat),
                new[] { "id" },
                Array.Empty<object>()
            );
    ...
    public async Task UploadStreamedObjects()
    {
        ...
        using (var client = new TestClient())
        {
            ...
            {
                await client
                    .SendHubMessageAsync(new StreamItemMessage("id", thing))
                    .DefaultTimeout();
            }
    ...
    public async Task UploadManyStreams()
    {
        ...
        using (var client = new TestClient())
        {
            ...
            foreach (var spot in order)
            {
                await client
                    .SendHubMessageAsync(
                        new StreamItemMessage(
                            spot.ToString(CultureInfo.InvariantCulture),
                            words[spot][pos[spot]]
                        )
                    )
                    .DefaultTimeout();
            ...
            foreach (string id in new[] { "0", "2", "1" })
            {
                ...
                Assert.Equal(
                    words[int.Parse(id, CultureInfo.InvariantCulture)],
                    ((CompletionMessage)response).Result
                );
            }
        }
    }
    ...
    public async Task UploadStreamStreamItemsSentAsSoonAsPossible()
    {
        ...
        using (var client = new TestClient())
        {
            ...
            await client.BeginUploadStreamAsync(
                "invocation",
                nameof(MethodHub.UploadArrayAuth),
                new[] { "id" },
                Array.Empty<object>()
            );
            await tcsService.StartedMethod.Task.DefaultTimeout();
    ...
    public async Task SerializationExceptionsSendSelfArePassedToOnDisconnectedAsync()
    {
        using (StartVerifiableLog(write => write.EventId.Name == "FailedWritingMessage"))
        {
            ...
            using (var client = new TestClient())
            {
                ...
                await client
                    .SendInvocationAsync(nameof(ConnectionLifetimeHub.ProtocolErrorSelf))
                    .DefaultTimeout();
    ...
    public async Task SerializationExceptionsSendAllArePassedToOnDisconnectedAsync()
    {
        using (StartVerifiableLog(write => write.EventId.Name == "FailedWritingMessage"))
        {
            ...
            using (var client = new TestClient())
            {
                ...
                await client
                    .SendInvocationAsync(nameof(ConnectionLifetimeHub.ProtocolErrorAll))
                    .DefaultTimeout();
        ...
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                await client
                    .BeginUploadStreamAsync(
                        "invocationId",
                        nameof(MethodHub.TestTypeCastingErrors),
                        new[] { "channelId" },
                        Array.Empty<object>()
                    )
                    .DefaultTimeout();
                ...
                await client
                    .SendHubMessageAsync(new StreamItemMessage("channelId", "not a number"))
                    .DefaultTimeout();
                var response = await client.ReadAsync().DefaultTimeout();

                Assert.Equal(typeof(CompletionMessage), response.GetType());
                Assert.Equal(
                    "error identified and caught",
                    (string)((CompletionMessage)response).Result
                );
            }
        }
    }
        ...
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                var connectionHandlerTask = await client
                    ...
                    .DefaultTimeout();
                await client
                    .SendHubMessageAsync(new StreamItemMessage("fake_id", "not a number"))
                    .DefaultTimeout();
        ...
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                var connectionHandlerTask = await client
                    ...
                    .DefaultTimeout();
                await client
                    .SendHubMessageAsync(CompletionMessage.Empty("fake_id"))
                    .DefaultTimeout();
        ...
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                await client
                    .BeginUploadStreamAsync(
                        "invocation",
                        nameof(MethodHub.TestCustomErrorPassing),
                        streamIds: new[] { "id" },
                        args: Array.Empty<object>()
                    )
                    .DefaultTimeout();
                await client
                    .SendHubMessageAsync(CompletionMessage.WithError("id", CustomErrorMessage))
                    .DefaultTimeout();
    ...
    public async Task UploadStreamWithTooManyStreamsFails()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var response = (CompletionMessage)await client.ReadAsync().DefaultTimeout();
                Assert.Equal(
                    "An unexpected error occurred invoking 'StreamingConcat' on the server. HubException: Client sent 2 stream(s), Hub method expects 1.",
                    response.Error
                );
            }
        }
    ...
    public async Task UploadStreamWithTooFewStreamsFails()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var response = (CompletionMessage)await client.ReadAsync().DefaultTimeout();
                Assert.Equal(
                    "An unexpected error occurred invoking 'StreamingConcat' on the server. HubException: Client sent 0 stream(s), Hub method expects 1.",
                    response.Error
                );
            }
        }
    ...
    public async Task UploadStreamReleasesHubActivatorOnceComplete()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                await client
                    .SendHubMessageAsync(new StreamItemMessage("id", "hello"))
                    .DefaultTimeout();
                await client
                    .SendHubMessageAsync(new StreamItemMessage("id", " world"))
                    .DefaultTimeout();
                await client.SendHubMessageAsync(CompletionMessage.Empty("id")).DefaultTimeout();
                var result = await client.ReadAsync().DefaultTimeout();
                ...
                var hubActivator =
                    serviceProvider.GetService<IHubActivator<MethodHub>>()
                    as CustomHubActivator<MethodHub>;
        ...
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                await client
                    .BeginUploadStreamAsync(
                        "invocation",
                        nameof(MethodHub.UploadIgnoreItems),
                        streamIds: new[] { "id" },
                        args: Array.Empty<object>()
                    )
                    .DefaultTimeout();
                ...
                await client
                    .SendHubMessageAsync(new StreamItemMessage("id", "ignored"))
                    .DefaultTimeout();
                var result = await client.ReadAsync().DefaultTimeout();
            ...
            using (var client = new TestClient())
            {
                ...
                await client
                    .SendStreamInvocationAsync(
                        nameof(MethodHub.StreamAndUploadIgnoreItems),
                        streamIds: new[] { "id" },
                        args: Array.Empty<object>()
                    )
                    .DefaultTimeout();
    ...
    )
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var streamInvocationId = await client
                    .SendStreamInvocationAsync(methodName, args)
                    .DefaultTimeout();
        ...
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient(invocationBinder: invocationBinder.Object))
            {
                ...
                var invocationId = await client
                    .SendStreamInvocationAsync(methodName, count)
                    .DefaultTimeout();
    ...
    public async Task StreamHubMethodCanAcceptCancellationTokenAsArgumentAndBeTriggeredOnConnectionAborted()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var streamInvocationId = await client
                    .SendStreamInvocationAsync(
                        nameof(LongRunningHub.CancelableStreamSingleParameter)
                    )
                    .DefaultTimeout();
                // Wait for the stream method to start
                await tcsService.StartedMethod.Task.DefaultTimeout();
    ...
    public async Task StreamHubMethodCanAcceptNullableParameter()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var streamInvocationId = await client
                    .SendStreamInvocationAsync(
                        nameof(LongRunningHub.StreamNullableParameter),
                        5,
                        null
                    )
                    .DefaultTimeout();
    ...
    public async Task StreamHubMethodCanAcceptNullableParameterWithCancellationToken()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var streamInvocationId = await client
                    .SendStreamInvocationAsync(
                        nameof(LongRunningHub.CancelableStreamNullableParameter),
                        5,
                        null
                    )
                    .DefaultTimeout();
    ...
    public async Task InvokeHubMethodCannotAcceptCancellationTokenAsArgument()
    {
        using (StartVerifiableLog())
        {
            ...
            using (var client = new TestClient())
            {
                ...
                var invocationId = await client
                    .SendInvocationAsync(nameof(MethodHub.InvalidArgument))
                    .DefaultTimeout();
                ...
                Assert.Equal(
                    "Failed to invoke 'InvalidArgument' due to an error on the server.",
                    completion.Error
                );
        ...
        using (StartVerifiableLog())
        {
            ...
            using (TestClient client = new TestClient(invocationBinder: invocationBinder.Object))
            {
                ...
                var messagePromise = client
                    .StreamAsync(
                        nameof(StreamingHub.StreamEcho),
                        new[] { streamId },
                        Array.Empty<object>()
                    )
                    .DefaultTimeout();
    ...
    public async Task CanPassDerivedParameterToStreamHubMethod(string method)
    {
        using (StartVerifiableLog())
        {
            ...
            )
            {
                ...
                HubConnectionHandlerTestUtils.AssertHubMessage(
                    new StreamItemMessage(string.Empty, argument.Value),
                    messages[0]
                );
                HubConnectionHandlerTestUtils.AssertHubMessage(
                    CompletionMessage.Empty(string.Empty),
    ...
    public async Task ConnectionCloseCleansUploadStreams()
    {
        ...
        using (StartVerifiableLog())
        {
            ...
            await client
                .BeginUploadStreamAsync(
                    "invocation",
                    nameof(MethodHub.UploadDoesWorkOnComplete),
                    streamIds: new[] { "id" },
                    args: Array.Empty<object>()
                )
                .DefaultTimeout();
            ...
            var task =
                (Task<int>)client.Connection.Items[nameof(MethodHub.UploadDoesWorkOnComplete)];

            var exception = await Assert
                .ThrowsAsync<OperationCanceledException>(() => task)
                .DefaultTimeout();
    ...
    public async Task CanSendThroughIHubContextBaseHub()
    {
        using (StartVerifiableLog())
        {
            ...
            IHubContext<TestHub> context = serviceProvider.GetRequiredService<
    ...
    public async Task HubMethodFailsIfServiceNotFound()
    {
        ...
        using (var client = new TestClient())
        {
            ...
            Assert.Equal(
                "An unexpected error occurred invoking 'SingleService' on the server. InvalidOperationException: No service for type 'Microsoft.AspNetCore.SignalR.Tests.Service1' has been registered.",
                res.Error
            );
        }
    }
    ...
    public async Task HubMethodCanInjectMultipleServices()
    {
        ...
        using (var client = new TestClient())
        {
            ...
            var res = await client
                .InvokeAsync(nameof(ServicesHub.MultipleServices))
                .DefaultTimeout();
            Assert.True(Assert.IsType<bool>(res.Result));
        }
    }
    ...
    public async Task HubMethodCanInjectServicesWithOtherParameters()
    {
        ...
        using (var client = new TestClient())
        {
            ...
            await client
                .BeginUploadStreamAsync(
                    "0",
                    nameof(ServicesHub.ServicesAndParams),
                    new string[] { "1" },
                    10,
                    true
                )
            ...
            var response = Assert.IsType<CompletionMessage>(
                await client.ReadAsync().DefaultTimeout()
            );
    ...
    public async Task StreamFromServiceDoesNotWork()
    {
        ...
        using (var client = new TestClient())
        {
            ...
            Assert.Equal(
                "An unexpected error occurred invoking 'Stream' on the server. HubException: Client sent 0 stream(s), Hub method expects 1.",
                res.Error
            );
        }
    }
    ...
    public async Task ServiceNotResolvedWithoutAttribute_WithSettingDisabledGlobally()
    {
        ...
        using (var client = new TestClient())
        {
            var connectionHandlerTask = await client
                ...
                .DefaultTimeout();
            var res = await client
                .InvokeAsync(nameof(ServicesHub.ServiceWithoutAttribute))
                .DefaultTimeout();
            Assert.Equal(
                "Failed to invoke 'ServiceWithoutAttribute' due to an error on the server. InvalidDataException: Invocation provides 0 argument(s) but target expects 1.",
                res.Error
            );
        }
    }
    ...
    public async Task ServiceResolvedForIEnumerableParameter()
    {
        ...
        using (var client = new TestClient())
        {
            var connectionHandlerTask = await client
                ...
                .DefaultTimeout();
            var res = await client
                .InvokeAsync(nameof(ServicesHub.IEnumerableOfServiceWithoutAttribute))
                .DefaultTimeout();
            Assert.Equal(1L, res.Result);
        }
    }
    ...
    public async Task ServiceNotResolvedWithAndWithoutAttribute_WithOptionDisabled()
    {
        ...
        using (var client = new TestClient())
        {
            var connectionHandlerTask = await client
                ...
                .DefaultTimeout();
            var res = await client
                .InvokeAsync(nameof(ServicesHub.ServiceWithAndWithoutAttribute))
                .DefaultTimeout();
            Assert.Equal(
                "Failed to invoke 'ServiceWithAndWithoutAttribute' due to an error on the server. InvalidDataException: Invocation provides 0 argument(s) but target expects 1.",
                res.Error
            );
        }
    }
    ...
    public async Task ServiceNotResolvedForIEnumerableParameterIfNotInDI()
    {
        ...
        using (var client = new TestClient())
        {
            ...
            var res = await client
                ...
                .DefaultTimeout();
            Assert.Equal(
                "Failed to invoke 'IEnumerableOfServiceWithoutAttribute' due to an error on the server. InvalidDataException: Invocation provides 0 argument(s) but target expects 1.",
                res.Error
            );
        }
    }
    ...
    public async Task KeyedServiceResolvedIfInDI()
    {
        ...
        using (var client = new TestClient())
        {
            var connectionHandlerTask = await client
                ...
                .DefaultTimeout();
            var res = await client
                .InvokeAsync(nameof(KeyedServicesHub.KeyedService))
                .DefaultTimeout();
    ...
    public async Task HubMethodCanInjectKeyedServiceWithOtherParameters()
    {
        ...
        using (var client = new TestClient())
        {
            var connectionHandlerTask = await client
                ...
                .DefaultTimeout();
            var res = await client
                .InvokeAsync(nameof(KeyedServicesHub.KeyedServiceWithParam), 91)
                .DefaultTimeout();
    ...
    public async Task HubMethodCanInjectKeyedServiceWithNonKeyedService()
    {
        ...
        using (var client = new TestClient())
        {
            var connectionHandlerTask = await client
                ...
                .DefaultTimeout();
            var res = await client
                .InvokeAsync(nameof(KeyedServicesHub.KeyedServiceNonKeyedService))
                .DefaultTimeout();
    ...
    public async Task MultipleKeyedServicesResolved()
    {
        ...
        using (var client = new TestClient())
        {
            var connectionHandlerTask = await client
                ...
                .DefaultTimeout();
            var res = await client
                .InvokeAsync(nameof(KeyedServicesHub.MultipleKeyedServices))
                .DefaultTimeout();
    ...
    public async Task MultipleKeyedServicesWithSameNameResolved()
    {
        ...
        using (var client = new TestClient())
        {
            var connectionHandlerTask = await client
                ...
                .DefaultTimeout();
            var res = await client
                .InvokeAsync(nameof(KeyedServicesHub.MultipleSameKeyedServices))
                .DefaultTimeout();
    ...
    public async Task SendToAnotherClientFromOnConnectedAsync()
    {
        ...
        var connectionHandler = serviceProvider.GetService<
            HubConnectionHandler<OnConnectedSendToClientHub>
        >();
        ...
        using (var client2 = new TestClient())
        {
            ...
            httpContext.Request.QueryString = new QueryString(
                $"?client={client1.Connection.ConnectionId}"
            );
            var feature = new TestHttpContextFeature { HttpContext = httpContext };
            client2.Connection.Features.Set<IHttpContextFeature>(feature);

            var connectionHandlerTask = await client1
                .ConnectAsync(connectionHandler)
                .DefaultTimeout();
...
#pragma warning restore CA2252 // This API requires opting into preview features
            ...
            var seqMessage = Assert.IsType<SequenceMessage>(
                await client.ReadAsync().DefaultTimeout()
            );
...
#pragma warning restore CA2252 // This API requires opting into preview features
    {
        private TaskCompletionSource _reconnectDisabled = new TaskCompletionSource(
            TaskCreationOptions.RunContinuationsAsynchronously
        );
