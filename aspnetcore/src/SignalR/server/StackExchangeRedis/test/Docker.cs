...
namespace Microsoft.AspNetCore.SignalR.StackExchangeRedis.Tests
{
    public class Docker
    {
        ...
        private static readonly string _dockerMonitorContainerName =
            _dockerContainerName + "Monitor";
        ...
        private void StartRedis(ILogger logger)
        {
            ...
            void Run()
            {
                ...
                RunProcessAndThrowIfFailed(
                    _path,
                    $"run --rm -p 6379:6379 --name {_dockerContainerName} -d redis",
                    "redis",
                    logger,
        ...
        public void Start(ILogger logger)
        {
            ...
            RunProcessAndWait(
                _path,
                $"stop {_dockerMonitorContainerName}",
                "docker stop",
                logger,
                TimeSpan.FromSeconds(15),
                out var _
            ...
            var (monitorProcess, monitorOutput) = RunProcess(
                _path,
                $"run -i --name {_dockerMonitorContainerName} --link {_dockerContainerName}:redis --rm redis redis-cli -h redis -p 6379",
                "redis monitor",
                logger
            );
        ...
        private static void RunProcessAndThrowIfFailed(
            string fileName,
            string arguments,
            string prefix,
            ILogger logger,
            ...
            {
                throw new Exception(
                    $"Command '{fileName} {arguments}' failed with exit code '{exitCode}'. Output:{Environment.NewLine}{output}"
                );
            }
        }
        ...
        private static int RunProcessAndWait(
            ...
            using (process)
            {
                if (!process.WaitForExit((int)timeout.TotalMilliseconds))
                {
                    process.Close();
                    logger.LogError(
                        "Closing process '{processName}' because it is running longer than the configured timeout.",
                        fileName
                    );
        ...
        private static (Process, ConcurrentQueue<string>) RunProcess(
            string fileName,
            string arguments,
            string prefix,
            ILogger logger
        ...
        private static void LogIfNotNull(
