    ...
    where THub : Hub
{
    ...
    )
    {
        ...
        if (!connection.ShouldProcessMessage(hubMessage))
        {
            Log.DroppingMessage(
                _logger,
                hubMessage.GetType().Name,
                (hubMessage as HubInvocationMessage)?.InvocationId ?? "(null)"
            );
    ...
    private void InitializeHub(THub hub, HubConnectionContext connection, bool invokeAllowed = true)
    {
        hub.Clients = new HubCallerClients(
            _hubContext.Clients,
            connection.ConnectionId,
            connection.ActiveInvocationLimit
        )
        {
    ...
    )
    {
        ...
        for (var parameterPointer = 0; parameterPointer < arguments.Length; parameterPointer++)
        {
            ...
            else
            {
                ...
                {
                    arguments[parameterPointer] = descriptor.GetService(
                        scope.ServiceProvider,
                        parameterPointer,
