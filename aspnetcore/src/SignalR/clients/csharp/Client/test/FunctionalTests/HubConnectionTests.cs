...
namespace Microsoft.AspNetCore.SignalR.Client.FunctionalTests
{
    ...
    public class HubConnectionTests : FunctionalTestBase
    {
        ...
        private Func<EndPoint, ValueTask<ConnectionContext>> GetHttpConnectionFactory(
            string url,
                    ...
                    connection.On<string>(
                        ...
                        async (message) =>
                        {
                            ...
                            helloWorldTcs.SetResult(
                                await connection.InvokeAsync<string>(nameof(TestHub.HelloWorld))
                                    .DefaultTimeout()
                            );
                        }
                    );
        ...
        public async Task CanStreamToHubWithIAsyncEnumerableMethodArg(string protocolName)
        {
            ...
            await using (var server = await StartServer<Startup>())
            {
                ...
                try
                {
                    ...
                    await connection.StartAsync().DefaultTimeout();
                    var expectedValue = 0;
                    var asyncEnumerable = connection.StreamAsync<int>(
                        "StreamIAsyncConsumer",
                        stream
                    );
                    await foreach (var streamValue in asyncEnumerable)
                    {
            ...
            await using (var server = await StartServer<Startup>())
            {
                ...
                try
                {
                    ...
                    await Assert.ThrowsAnyAsync<OperationCanceledException>(
                        () =>
                            connection.StreamAsChannelAsync<int>("Stream", 5, cts.Token)
                                .DefaultTimeout()
                    );
                }
                catch (Exception ex)
        ...
        public async Task RandomGenericIsNotTreatedAsStream()
        {
            ...
            await using (var server = await StartServer<Startup>())
            {
                ...
                var hubException = await Assert.ThrowsAsync<HubException>(
                    () =>
                        connection.InvokeAsync<int>("StreamEcho", new List<string> { "1", "2" })
                            .DefaultTimeout()
                );
                Assert.Equal(
