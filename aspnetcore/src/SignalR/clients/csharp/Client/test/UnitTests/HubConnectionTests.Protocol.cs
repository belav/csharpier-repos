        ...
        public class Protocol : VerifiableLoggedTest
        {
            ...
            public async Task ReceiveCloseMessageWithErrorWillCloseHubConnection()
            {
                ...
                try
                {
                    ...
                    await connection.ReceiveJsonMessage(new { type = 7, error = "Error!" })
                        .DefaultTimeout();
            ...
            public async Task StreamSendsAnInvocationMessage()
            {
                ...
                try
                {
                    ...
                    await connection.ReceiveJsonMessage(new { invocationId = "1", type = 3 })
                        .DefaultTimeout();
            ...
            public async Task InvokeYieldsResultWhenCompletionMessageReceived()
            {
                ...
                try
                {
                    ...
                    await connection.ReceiveJsonMessage(
                        new { invocationId = "1", type = 3, result = 42 }
                    )
                        .DefaultTimeout();
            ...
            public async Task StreamFailsIfCompletionMessageHasPayload()
            {
                ...
                try
                {
                    ...
                    await connection.ReceiveJsonMessage(
                        new { invocationId = "1", type = 3, result = "Oops" }
                    )
                        .DefaultTimeout();
            ...
            public async Task InvokeFailsWithErrorWhenStreamingItemReceived()
            {
                ...
                try
                {
                    ...
                    await connection.ReceiveJsonMessage(
                        new { invocationId = "1", type = 2, item = 42 }
                    )
                        .DefaultTimeout();
            ...
            public async Task StreamYieldsItemsAsTheyArrive()
            {
                ...
                try
                {
                    ...
                    await connection.ReceiveJsonMessage(
                        new { invocationId = "1", type = 2, item = "1" }
                    )
                        .DefaultTimeout();
                    await connection.ReceiveJsonMessage(
                        new { invocationId = "1", type = 2, item = "2" }
                    )
                        .DefaultTimeout();
                    await connection.ReceiveJsonMessage(
                        new { invocationId = "1", type = 2, item = "3" }
                    )
                        .DefaultTimeout();
                    await connection.ReceiveJsonMessage(new { invocationId = "1", type = 3 })
                        .DefaultTimeout();
            ...
            public async Task ClientWithInherentKeepAliveDoesNotPing()
            {
                ...
                try
                {
                    ...
                    Assert.Equal(
                        0,
                        (
                            await connection.ReadAllSentMessagesAsync(ignorePings: false)
                                .DefaultTimeout()
                        ).Count
                    );
                }
