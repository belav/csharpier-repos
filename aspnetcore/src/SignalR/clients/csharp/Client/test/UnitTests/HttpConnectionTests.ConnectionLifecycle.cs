                    ...
                    await WithConnectionAsync(
                        CreateConnection(
                            ...
                            transport: new TestTransport(
                                onTransportStart: SyncPoint.Create(out var syncPoint)
                            )
                        ),
                        async (connection) =>
                        {
            ...
            public async Task StartThrowsAfterAllTransportsFail()
            {
                ...
                using (StartVerifiableLog(expectedErrorsFilter: ExpectedErrors))
                {
                    ...
                    await WithConnectionAsync(
                        ...
                        {
                            var ex = await Assert
                                .ThrowsAsync<AggregateException>(() => connection.StartAsync())
                                .DefaultTimeout();
                            Assert.Equal(
                ...
                using (StartVerifiableLog())
                {
                    ...
                    await WithConnectionAsync(
                        ...
                        async (connection) =>
                        {
                            ...
                            await Assert
                                .ThrowsAsync<OperationCanceledException>(() => startTask)
                                .DefaultTimeout();
                        }
                    );
                }
