...
namespace Microsoft.AspNetCore.SignalR.Common.Tests.Internal.Protocol
{
    ...
    public class MessagePackHubProtocolTests : MessagePackHubProtocolTestBase
    {
        ...
        {
            var result = Write(
                CompletionMessage.WithResult("0", new List<int> { 42 }.AsReadOnly())
            );
            AssertMessages(
                new byte[]
                {
        ...
        public void WriteAndParseDateTimeConvertsToUTC(DateTimeKind dateTimeKind)
        {
            ...
            try
            {
                HubProtocol.WriteMessage(
                    CompletionMessage.WithResult("xyz", originalDateTime),
                    writer
                );
        ...
        public void WriteAndParseDateTimeOffset()
        {
            ...
            {
                HubProtocol.WriteMessage(
                    CompletionMessage.WithResult("xyz", dateTimeOffset),
                    writer
                );
                var bytes = new ReadOnlySequence<byte>(writer.ToArray());
                HubProtocol.TryParseMessage(
