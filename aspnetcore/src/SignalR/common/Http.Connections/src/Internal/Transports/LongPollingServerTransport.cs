...
internal class LongPollingServerTransport : IHttpTransport
{
    ...
    {
        private static readonly Action<ILogger, Exception?> _longPolling204 = LoggerMessage.Define(
            LogLevel.Debug,
            new EventId(1, "LongPolling204"),
            "Terminating Long Polling connection by sending 204 response."
        );

        private static readonly Action<ILogger, Exception?> _pollTimedOut = LoggerMessage.Define(
            LogLevel.Debug,
