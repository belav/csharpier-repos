...
internal sealed partial class HttpConnectionManager
{
    ...
    )
    {
        ...
        if (_connections.TryRemove(id, out var connection))
        {
            ...
            // You can't stop something that hasn't started so only log connection stop events if there is a
            // transport.
            if (connection.TransportType != HttpTransportType.None)
            {
                var currentTimestamp =
                    (connection.StartTimestamp > 0) ? Stopwatch.GetTimestamp() : default;

                HttpConnectionsEventSource.Log.ConnectionStop(
                    id,
                    connection.StartTimestamp,
                    currentTimestamp
                );
        ...
        foreach (var c in _connections)
        {
            ...
            tasks.Add(
                DisposeAndRemoveAsync(
                    ...
                    HttpConnectionStopStatus.AppShutdown
                )
            );
        }
