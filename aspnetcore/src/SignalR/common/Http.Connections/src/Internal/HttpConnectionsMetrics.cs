...
internal sealed class HttpConnectionsMetrics : IDisposable
{
    ...
    public void ConnectionStop(
        ...
        HttpConnectionStopStatus status,
        long startTimestamp,
    ...
    )
    {
        if (metricsContext.ConnectionDurationEnabled)
        {
            ...
            _connectionDuration.Record(
                duration.TotalSeconds,
                new KeyValuePair<string, object?>(
                    "signalr.connection.status",
                    ResolveStopStatus(status)
                ),
                new KeyValuePair<string, object?>(
                    "signalr.transport",
                    ResolveTransportType(transportType)
                )
            );
        }
    }

    public void ConnectionTransportStart(
        in MetricsContext metricsContext,
        HttpTransportType transportType
    )
    {
        ...
        {
            _currentConnectionsCounter.Add(
                1,
                new KeyValuePair<string, object?>(
                    "signalr.transport",
                    ResolveTransportType(transportType)
