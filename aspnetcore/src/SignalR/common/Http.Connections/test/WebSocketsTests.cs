        ...
        public async Task ReceivedFramesAreWrittenToChannel(string webSocketMessageType)
        {
            using (StartVerifiableLog())
            {
                ...
                var connection = new HttpConnectionContext(
                    ...
                    LoggerFactory.CreateLogger("HttpConnectionContext1"),
                    pair.Transport,
                    pair.Application
                );
        ...
        public async Task WebSocketTransportSetsMessageTypeBasedOnTransferFormatFeature(
            ...
            using (StartVerifiableLog())
            {
                ...
                using (var feature = new TestWebSocketConnectionFeature())
                {
                    ...
                    Assert.Equal(
                        "Hello",
                        Encoding.UTF8.GetString(clientSummary.Received[0].Buffer)
                    );
                }
            }
        }
        ...
        public async Task ClientReceivesInternalServerErrorWhenTheApplicationFails()
        {
            using (StartVerifiableLog())
            {
                ...
                var connection = new HttpConnectionContext(
                    "foo",
                    connectionToken: null,
                    LoggerFactory.CreateLogger(nameof(HttpConnectionContext)),
                    pair.Transport,
                    pair.Application
                );
