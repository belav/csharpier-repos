...
internal static class JsonRequestHelpers
{
    ...
    )
    {
        ...
        var e =
            GetStatusDetails(trailers)
            ?? new Google.Rpc.Status { Message = status.Detail, Code = (int)status.StatusCode };
        ...
        static Google.Rpc.Status? GetStatusDetails(Metadata trailers)
        {
            ...
            if (statusDetails?.IsBinary == true)
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Error when parsing the '{StatusDetailsTrailerName}' trailer.",
                        ex
                    );
                }
            }
        ...
        try
        {
            ...
            if (serverCallContext.DescriptorInfo.BodyDescriptor != null)
            {
                    ...
                    try
                    {
                        if (serverCallContext.DescriptorInfo.BodyDescriptorRepeated)
                        {
                            ...
                            type = JsonConverterHelper.GetFieldType(
                                serverCallContext.DescriptorInfo.BodyFieldDescriptor
                            );
                ...
                if (serverCallContext.DescriptorInfo.BodyFieldDescriptor != null)
                {
                    ...
                    ServiceDescriptorHelpers.SetValue(
                        requestMessage,
                        serverCallContext.DescriptorInfo.BodyFieldDescriptor,
                        bodyContent
                    );
            ...
            )
            {
                ...
                if (routeValue != null)
                {
                    ServiceDescriptorHelpers.RecursiveSetValue(
                        requestMessage,
                        parameterDescriptor.Value.DescriptorsPath,
                        routeValue
                    );
                }
            }
    ...
    )
    {
        if (serverCallContext.DescriptorInfo.BodyDescriptor != null)
        {
            ...
            if (
                bodyFieldName.Length + 1 < variable.Length
