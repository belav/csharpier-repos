    ...
    where TMessage : IMessage, new()
{
    ...
    public AnyConverter(JsonContext context)
        : base(context) { }
    ...
    )
    {
        ...
        if (ServiceDescriptorHelpers.IsWellKnownType(descriptor))
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Expected '{AnyWellKnownTypeValueField}' property for well-known type Any body."
                );
            }
        ...
        {
            data = (IMessage)
                JsonSerializer.Deserialize(d.RootElement, descriptor.ClrType, options)!;
        }

        var message = new TMessage();
        message.Descriptor.Fields[Any.TypeUrlFieldNumber].Accessor.SetValue(message, typeUrl);
        message
            .Descriptor.Fields[Any.ValueFieldNumber]
            .Accessor.SetValue(message, data.ToByteString());
        ...
        if (ServiceDescriptorHelpers.IsWellKnownType(descriptor))
        {
            ...
            if (ServiceDescriptorHelpers.IsWrapperType(descriptor))
            {
                var wrappedValue = valueMessage
                    .Descriptor.Fields[JsonConverterHelper.WrapperValueFieldNumber]
    ...
    )
    {
        ...
        foreach (var field in fields.InFieldNumberOrder())
        {
            ...
            if (
                !JsonConverterHelper.ShouldFormatFieldValue(
                    message,
                    field,
                    value,
                    !settings.IgnoreDefaultValues
                )
            )
            {
                continue;
