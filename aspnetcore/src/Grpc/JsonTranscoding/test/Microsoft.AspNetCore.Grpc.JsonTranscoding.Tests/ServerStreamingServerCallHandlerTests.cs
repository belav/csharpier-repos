...
public class ServerStreamingServerCallHandlerTests : LoggedTest
{
    ...
    public async Task HandleCallAsync_WriteMultipleMessages_Returned()
    {
        ...
        var descriptorPath = new List<FieldDescriptor>(
            new[] { HelloRequest.Descriptor.FindFieldByNumber(HelloRequest.NameFieldNumber) }
        );
        ...
        Assert.Equal(
            "Hello TestName! 1",
            responseJson1.RootElement.GetProperty("message").GetString()
        );
        ...
        Assert.Equal(
            "Hello TestName! 2",
            responseJson2.RootElement.GetProperty("message").GetString()
        );
    ...
    public async Task HandleCallAsync_MessageThenError_MessageThenErrorReturned()
    {
        ...
        var exceptionWrite = TestSink.Writes
    ...
    public async Task HandleCallAsync_MessageThenRpcException_MessageThenErrorReturned()
    {
        ...
        Assert.Equal(
            (int)StatusCode.Aborted,
            responseJson2.RootElement.GetProperty("code").GetInt32()
        );

        var exceptionWrite = TestSink.Writes.Single(w => w.EventId.Name == "RpcConnectionError");
        Assert.Equal(
            "Error status code 'Aborted' with detail 'Detail!' raised.",
            exceptionWrite.Message
        );
    ...
    public async Task HandleCallAsync_ErrorWithDetailedErrors_DetailedErrorResponse()
    {
        ...
        Assert.Equal(
            "Exception was thrown by handler. Exception: Exception!",
    ...
    public async Task HandleCallAsync_HttpBody_WriteMultipleMessages_Returned()
    {
        ...
        ServerStreamingServerMethod<JsonTranscodingGreeterService, HelloRequest, HttpBody> invoker =
            async (s, r, w, c) =>
            {
                await w.WriteAsync(
                    new HttpBody
                    {
                        ...
                        Data = ByteString.CopyFrom(
                            Encoding.UTF8.GetBytes($"<message>Hello {r.Name} 1</message>")
                        ),
                    }
                );
                await syncPoint.WaitToContinue();
                await w.WriteAsync(
                    new HttpBody
                    {
                        ContentType = "application/xml",
                        Data = ByteString.CopyFrom(
                            Encoding.UTF8.GetBytes($"<message>Hello {r.Name} 2</message>")
                        ),
                    }
                );
            };
    ...
    private ServerStreamingServerCallHandler<
        JsonTranscodingGreeterService,
        HelloRequest,
        HelloReply
    > CreateCallHandler(
        ...
        where TResponse : class, IMessage<TResponse>
    {
        ...
        var callInvoker = new ServerStreamingServerMethodInvoker<
            JsonTranscodingGreeterService,
            TRequest,
            TResponse
        >(
        ...
        var jsonSettings =
            jsonTranscodingOptions?.JsonSettings
        ...
        return new ServerStreamingServerCallHandler<
            JsonTranscodingGreeterService,
            TRequest,
            TResponse
        >(
