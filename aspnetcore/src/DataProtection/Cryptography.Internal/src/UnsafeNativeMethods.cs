...
internal static unsafe class UnsafeNativeMethods
{
    ...
    private static readonly Lazy<SafeLibraryHandle> _lazyCrypt32LibHandle = GetLazyLibraryHandle(
        CRYPT32_LIB
    );
    ...
    private static Lazy<SafeLibraryHandle> GetLazyLibraryHandle(string libraryName)
    {
        ...
        return new Lazy<SafeLibraryHandle>(
            () => SafeLibraryHandle.Open(libraryName),
