...
public class TimeLimitedDataProtectorTests
{
    ...
    public void Protect_LifetimeSpecified()
    {
        ...
        var protectedPayload = subProtector.Protect(
    ...
    public void Unprotect_WithinPayloadValidityPeriod_Success()
    {
        ...
        // Act
        var retVal = timeLimitedProtector.UnprotectCore(
            new byte[] { 0x10, 0x11 },
            now,
            out var actualExpiration
        );
    ...
    public void Unprotect_PayloadHasExpired_Fails()
    {
        ...
        // Assert
        Assert.Equal(
            ExtResources.FormatTimeLimitedDataProtector_PayloadExpired(expectedExpiration),
            ex.Message
        );
    }
    ...
    public void RoundTrip_ProtectedData()
    {
        ...
        // Act
        byte[] ephemeralProtectedPayload = ephemeralProtector.Protect(
            new byte[] { 0x01, 0x02, 0x03, 0x04 }
        );
        byte[] timeLimitedProtectedPayload = timeLimitedProtector.Protect(
        ...
        Assert.Equal(
            ...
            timeLimitedProtector.UnprotectCore(
                timeLimitedProtectedPayload,
                StringToDateTime("2010-01-01 00:00:00Z"),
                out var actualExpiration
            )
        );
        Assert.Equal(expectedExpiration, actualExpiration);
