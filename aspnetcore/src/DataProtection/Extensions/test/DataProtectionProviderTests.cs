    ...
    public class DataProtectionProviderTests
    {
                ...
                WithUniqueTempDirectory(
                    directory =>
                    {
                        ...
                        try
                        {
                            ...
                            // Step 2: instantiate the system and round-trip a payload
                            var protector = DataProtectionProvider.Create(directory, certificate)
                                .CreateProtector("purpose");
        ...
        public void System_UsesProvidedCertificateNotFromStore()
        {
            using (var store = new X509Store(StoreName.My, StoreLocation.CurrentUser))
            {
                store.Open(OpenFlags.ReadWrite);
                var certWithoutKey = new X509Certificate2(
                    Path.Combine(GetTestFilesPath(), "TestCert3WithoutPrivateKey.pfx"),
                    "password3",
                    X509KeyStorageFlags.Exportable
                );
                ...
                directory =>
                {
                    ...
                    )
                    {
                        ...
                        try
                        {
                            ...
                            var protector = DataProtectionProvider.Create(directory, certWithKey)
                                .CreateProtector("purpose");
                            var data = protector.Protect("payload");

                            var keylessUnprotector = DataProtectionProvider.Create(directory)
                                .CreateProtector("purpose");
        ...
        {
            string uniqueTempPath = Path.Combine(
