...
public class Pbkdf2Tests
{
    ...
    [InlineData(
        ...
        5,
        256 / 8 - 1,
        "JRNz8bPKS02EG1vf7eWjA64IeeI+TI8gBEwb1oVvRA=="
    )]
    [InlineData(
        "my-password",
        KeyDerivationPrf.HMACSHA256,
        5,
        256 / 8 + 0,
        "JRNz8bPKS02EG1vf7eWjA64IeeI+TI8gBEwb1oVvRLo="
    )]
    [InlineData(
        "my-password",
        KeyDerivationPrf.HMACSHA256,
        5,
        256 / 8 + 1,
        "JRNz8bPKS02EG1vf7eWjA64IeeI+TI8gBEwb1oVvRLpk"
    )]
    [InlineData(
        "my-password",
        ...
        5,
        512 / 8 - 1,
        "ZTallQJrFn0279xIzaiA1XqatVTGei+ZjKngA7bIMtKMDUw6YJeGUQpFG8iGTgN+ri3LNDktNbzwfcSyZmm9"
    )]
    [InlineData(
        "my-password",
        KeyDerivationPrf.HMACSHA512,
        5,
        512 / 8 + 0,
        "ZTallQJrFn0279xIzaiA1XqatVTGei+ZjKngA7bIMtKMDUw6YJeGUQpFG8iGTgN+ri3LNDktNbzwfcSyZmm90Q=="
    )]
    [InlineData(
        "my-password",
        KeyDerivationPrf.HMACSHA512,
        5,
        512 / 8 + 1,
        "ZTallQJrFn0279xIzaiA1XqatVTGei+ZjKngA7bIMtKMDUw6YJeGUQpFG8iGTgN+ri3LNDktNbzwfcSyZmm90Wk="
    )]
    public void RunTest_Normal_NetCore(
        string password,
    ...
    )
    {
        ...
        // Act & assert
#if NETFRAMEWORK
        TestProvider<ManagedPbkdf2Provider>(
            password,
            salt,
            prf,
            iterationCount,
            numBytesRequested,
            expectedValueAsBase64
        );
#elif NETCOREAPP
        TestProvider<NetCorePbkdf2Provider>(
            password,
            salt,
            prf,
            iterationCount,
...
#error Update target frameworks
    ...
    public void RunTest_WithLongPassword_NetCore_FallbackToManaged()
    {
        ...
        const string expectedDerivedKeyBase64 =
            "Sc+V/c3fiZq5Z5qH3iavAiojTsW97FAp2eBNmCQAwCNzA8hfhFFYyQLIMK65qPnBFHOHXQPwAxNQNhaEAH9hzfiaNBSRJpF9V4rpl02d5ZpI6cZbsQFF7TJW7XJzQVpYoPDgJlg0xVmYLhn1E9qMtUVUuXsBjOOdd7K1M+ZI00c=";
...
#error Update target frameworks
    ...
    public void RunTest_WithLongPassword_NetCore()
    {
        ...
        RunTest_WithLongPassword_Impl<ManagedPbkdf2Provider>(
            salt,
            "NGJtFzYUaaSxu+3ZsMeZO5d/qPJDUYW4caLkFlaY0cLSYdh1PN4+nHUVp4pUUubJWu3UeXNMnHKNDfnn8GMfnDVrAGTv1lldszsvUJ0JQ6p4+daQEYBc//Tj/ejuB3luwW0IinyE7U/ViOQKbfi5pCZFMQ0FFx9I+eXRlyT+I74="
        );
#elif NETCOREAPP
        RunTest_WithLongPassword_Impl<NetCorePbkdf2Provider>(
            salt,
            "NGJtFzYUaaSxu+3ZsMeZO5d/qPJDUYW4caLkFlaY0cLSYdh1PN4+nHUVp4pUUubJWu3UeXNMnHKNDfnn8GMfnDVrAGTv1lldszsvUJ0JQ6p4+daQEYBc//Tj/ejuB3luwW0IinyE7U/ViOQKbfi5pCZFMQ0FFx9I+eXRlyT+I74="
        );
#else
#error Update target frameworks
    ...
    public void RunTest_Normal_Managed(
        string password,
        KeyDerivationPrf prf,
        int iterationCount,
        int numBytesRequested,
    ...
    public void RunTest_Normal_Win7(
        string password,
        KeyDerivationPrf prf,
        int iterationCount,
        int numBytesRequested,
    ...
    )
    {
        ...
        // Act & assert
        TestProvider<Win7Pbkdf2Provider>(
            password,
            salt,
            prf,
            iterationCount,
    ...
    public void RunTest_Normal_Win8(
        string password,
        KeyDerivationPrf prf,
        int iterationCount,
        int numBytesRequested,
    ...
    )
    {
        ...
        // Act & assert
        TestProvider<Win8Pbkdf2Provider>(
            password,
            salt,
            prf,
            iterationCount,
        ...
        where TProvider : IPbkdf2Provider, new()
    {
        ...
        TestProvider<TProvider>(
            ...
            iterationCount,
            numBytesRequested,
            expectedDerivedKeyBase64
        );
    }

    private static void TestProvider<TProvider>(
        string password,
        byte[] salt,
        KeyDerivationPrf prf,
        int iterationCount,
