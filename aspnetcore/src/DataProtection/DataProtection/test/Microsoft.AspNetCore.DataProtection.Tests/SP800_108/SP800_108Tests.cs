...
public unsafe class SP800_108Tests
{
    ...
    )
    {
        ...
        TestCngKeyDerivation(
            (pbKdk, cbKdk) => new Win7SP800_108_CTR_HMACSHA512Provider(pbKdk, cbKdk),
            kdk,
            label,
            contextHeader,
            context,
    ...
    )
    {
        ...
        // Act & assert
        TestCngKeyDerivation(
            (pbKdk, cbKdk) => new Win8SP800_108_CTR_HMACSHA512Provider(pbKdk, cbKdk),
            kdk,
            label,
            contextHeader,
            context,
    ...
    private static void TestCngKeyDerivation(
    ...
    )
    {
        ...
        Buffer.BlockCopy(
            context,
            0,
            contextSegment.Array,
            contextSegment.Offset,
            contextSegment.Count
        );
        var derivedSubkeySegment = new ArraySegment<byte>(
            new byte[numDerivedBytes + 10],
            4,
            numDerivedBytes
        );
        ...
        Assert.Equal(
            expectedDerivedSubkeyAsBase64,
            Convert.ToBase64String(derivedSubkeySegment.AsStandaloneArray())
        );
    }
}
