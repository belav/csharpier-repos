...
public class AuthenticatedEncryptorDescriptorTests
{
    ...
    )
    {
        ...
        var control = new CbcAuthenticatedEncryptor(
            ...
            hmacAlgorithmHandle: BCryptAlgorithmHandle.OpenAlgorithmHandle(
                hashAlgorithm,
                hmac: true
            )
        );
        var test = CreateEncryptorInstanceFromDescriptor(
            CreateDescriptor(encryptionAlgorithm, validationAlgorithm, masterKey)
        );
    ...
    )
    {
        ...
        var test = CreateEncryptorInstanceFromDescriptor(
            CreateDescriptor(
                encryptionAlgorithm,
                ValidationAlgorithm.HMACSHA256 /* unused */
                ,
                masterKey
            )
        );
