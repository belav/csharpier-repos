...
internal static unsafe class SP800_108_CTR_HMACSHA512Extensions
{
    public static void DeriveKeyWithContextHeader(
        this ISP800_108_CTR_HMACSHA512Provider provider,
        byte* pbLabel,
        uint cbLabel,
        byte[] contextHeader,
        byte* pbContext,
        ...
        fixed (byte* pbHeapAllocatedCombinedContext = heapAllocatedCombinedContext)
        {
            ...
            fixed (byte* pbContextHeader = contextHeader)
            {
                UnsafeBufferUtil.BlockCopy(
                    from: pbContextHeader,
                    to: pbCombinedContext,
                    byteCount: contextHeader.Length
                );
            }
            UnsafeBufferUtil.BlockCopy(
                from: pbContext,
                to: &pbCombinedContext[contextHeader.Length],
                byteCount: cbContext
            );
