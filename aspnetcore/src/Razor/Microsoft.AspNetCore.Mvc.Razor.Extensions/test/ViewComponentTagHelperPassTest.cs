...
<p foo=""17"">"
        ...
        var pass = new ViewComponentTagHelperPass() { Engine = projectEngine.Engine, };
...
<tagcloud foo=""17"">"
        ...
        Assert.Equal(
            vcthFullName,
            Assert.IsType<DefaultTagHelperCreateIntermediateNode>(tagHelper.Children[1]).TypeName
        );
        Assert.Equal(
            "Foo",
            Assert
...
<p foo=""17""><tagcloud foo=""17""></p>"
        ...
        {
            TagHelperDescriptorBuilder
                .Create("PTestTagHelper", "TestAssembly")
                .TypeName("PTestTagHelper")
                .BoundAttributeDescriptor(
                    attribute => attribute.PropertyName("Foo").Name("Foo").TypeName("System.Int32")
                )
                .TagMatchingRuleDescriptor(rule => rule.RequireTagName("p"))
                .Build(),
            TagHelperDescriptorBuilder
        ...
        Assert.Equal(
            "Foo",
            Assert.IsType<DefaultTagHelperPropertyIntermediateNode>(vcth.Children[2]).PropertyName
        );
