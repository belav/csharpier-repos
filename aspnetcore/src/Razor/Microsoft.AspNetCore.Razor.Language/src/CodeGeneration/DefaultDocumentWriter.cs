...
internal class DefaultDocumentWriter : DocumentWriter
{
    ...
    )
    {
        ...
        var allOrderedDiagnostics = context
            .Diagnostics
    ...
    private class Visitor : IntermediateNodeVisitor
    {
        ...
        public override void VisitDocument(DocumentIntermediateNode node)
        {
            if (!Context.Options.SuppressChecksum)
            {
                ...
                )
                {
                    ...
                    string.Equals(algorithm, HashAlgorithmName.SHA1.Name, StringComparison.Ordinal)
                    ||
                ...
                else
                {
                    var supportedAlgorithms = string.Join(
                        " ",
                        new string[] { HashAlgorithmName.SHA1.Name, HashAlgorithmName.SHA256.Name }
                    );
        ...
        public override void VisitFieldDeclaration(FieldDeclarationIntermediateNode node)
        {
            Context
                .CodeWriter
                .WriteField(node.SuppressWarnings, node.Modifiers, node.FieldType, node.FieldName);
        }
        ...
        public override void VisitPropertyDeclaration(PropertyDeclarationIntermediateNode node)
        {
            Context
                .CodeWriter
                .WriteAutoPropertyDeclaration(node.Modifiers, node.PropertyType, node.PropertyName);
        }
