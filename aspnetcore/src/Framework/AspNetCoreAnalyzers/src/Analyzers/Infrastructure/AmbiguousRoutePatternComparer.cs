...
internal sealed class AmbiguousRoutePatternComparer : IEqualityComparer<RoutePatternTree>
{
    ...
    public bool Equals(RoutePatternTree x, RoutePatternTree y)
    {
        ...
        for (var i = 0; i < x.Root.Parts.Length; i++)
        {
            ...
            var equal = xPart switch
            {
                ...
                RoutePatternSegmentNode xSegment
                    => Equals(xSegment, yPart as RoutePatternSegmentNode),
    ...
    private static bool Equals(RoutePatternSegmentNode x, RoutePatternSegmentNode? y)
    {
        ...
        for (var i = 0; i < x.Children.Length; i++)
        {
            ...
            var equal = xChild switch
            {
                ...
                RoutePatternReplacementNode xReplacement
                    => yChild is RoutePatternReplacementNode yReplacement
                        && IgnoreCaseEquals(
                            xReplacement.TextToken.Value,
                            yReplacement.TextToken.Value
                        ),
                RoutePatternLiteralNode xLiteral
                    => yChild is RoutePatternLiteralNode yLiteral
                        && IgnoreCaseEquals(
                            xLiteral.LiteralToken.Value,
                            yLiteral.LiteralToken.Value
                        ),
                RoutePatternParameterNode xParameter
                    => Equals(xParameter, yChild as RoutePatternParameterNode),
                _
    ...
    )
    {
        ...
        for (var i = 0; i < x.PolicyFragments.Length; i++)
        {
            ...
            var equal = xPart switch
            {
                RoutePatternPolicyFragment xFragment
                    => yPart is RoutePatternPolicyFragment yFragment
                        && Equals(xFragment.ArgumentToken.Value, yFragment.ArgumentToken.Value),
                RoutePatternPolicyFragmentEscapedNode xFragmentEscaped
                    => yPart is RoutePatternPolicyFragmentEscapedNode yFragmentEscaped
                        && Equals(
