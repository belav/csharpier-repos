...
public class RoutePatternCompletionProvider : CompletionProvider
{
    ...
    private static readonly CompletionItemRules s_rules = CompletionItemRules.Create(
        ...
        filterCharacterRules: ImmutableArray.Create(
    ...
    public ImmutableHashSet<char> TriggerCharacters { get; } =
    ...
    )
    {
        ...
        return Task.FromResult(
            CompletionChange.Create(
                new TextChange(
                    new TextSpan(
                        int.Parse(startString, CultureInfo.InvariantCulture),
                        int.Parse(lengthString, CultureInfo.InvariantCulture)
                    ),
                    newText
                ),
                newPositionString == null
                    ? null
                    : int.Parse(newPositionString, CultureInfo.InvariantCulture)
            )
        );
    }
    ...
    public override async Task ProvideCompletionsAsync(CompletionContext context)
    {
        ...
        var (success, stringToken, semanticModel) = await RouteStringSyntaxDetectorDocument
            .TryGetStringSyntaxTokenAtPositionAsync(
                context.Document,
                position,
        ...
        if (!success || position <= stringToken.SpanStart || position >= stringToken.Span.End)
        {
            return;
        }
        ...
        var (methodSymbol, isMinimal, isMvcAttribute) = RoutePatternUsageDetector.BuildContext(
            stringToken,
            semanticModel,
            wellKnownTypes,
            context.CancellationToken
        ...
        var tree = RoutePatternParser.TryParse(
            virtualChars,
            supportTokenReplacement: isMvcAttribute
        );
        if (tree == null)
        {
            return;
        ...
        foreach (var embeddedItem in routePatternCompletionContext.Items)
        {
            ...
            properties.Add(
                LengthKey,
                textChange.Span.Length.ToString(CultureInfo.InvariantCulture)
            );
    ...
    private (RoutePatternNode Parent, RoutePatternToken Token)? GetCurrentToken(
    ...
    private static void ProvideParameterCompletions(
        EmbeddedCompletionContext context,
        RoutePatternNode? parentOpt
    )
    {
    ...
    private static void ProvidePolicyNameCompletions(
        EmbeddedCompletionContext context,
        RoutePatternNode? parentOpt
    )
    {
        ...
        context.AddIfMissing(
            "datetime",
            suffix: null,
            "Matches a valid DateTime value in the invariant culture.",
            WellKnownTags.Keyword,
            parentOpt
        );
        context.AddIfMissing(
            "decimal",
            suffix: null,
            "Matches a valid decimal value in the invariant culture.",
            WellKnownTags.Keyword,
            parentOpt
        );
        context.AddIfMissing(
            "double",
            suffix: null,
            "Matches a valid double value in the invariant culture.",
            WellKnownTags.Keyword,
            parentOpt
        );
        context.AddIfMissing(
            "float",
            suffix: null,
            "Matches a valid float value in the invariant culture.",
            WellKnownTags.Keyword,
            parentOpt
        );
        context.AddIfMissing(
        ...
        context.AddIfMissing(
            "minlength",
            suffix: null,
            "Matches a string with a length greater than, or equal to, the constraint argument.",
            WellKnownTags.Keyword,
            parentOpt
        );
        context.AddIfMissing(
            "maxlength",
            suffix: null,
            "Matches a string with a length less than, or equal to, the constraint argument.",
            WellKnownTags.Keyword,
            parentOpt
        );
        context.AddIfMissing(
            "length",
            suffix: null,
            @"The string length constraint supports one or two constraint arguments.
...
If there are two arguments then the string length must be greater than, or equal to, the first argument and less than, or equal to, the second argument. For example, length(8,16) matches a string at least 8 and no more than 16 characters long.",
        ...
        context.AddIfMissing(
            "min",
            suffix: null,
            "Matches an integer with a value greater than, or equal to, the constraint argument.",
            WellKnownTags.Keyword,
            parentOpt
        );
        context.AddIfMissing(
            "max",
            suffix: null,
            "Matches an integer with a value less than, or equal to, the constraint argument.",
            WellKnownTags.Keyword,
            parentOpt
        );
        context.AddIfMissing(
            "range",
            suffix: null,
            "Matches an integer with a value greater than, or equal to, the first constraint argument and less than, or equal to, the second constraint argument.",
            WellKnownTags.Keyword,
            parentOpt
        );
        context.AddIfMissing(
        ...
        context.AddIfMissing(
            "regex",
            suffix: null,
            "Matches a string to the regular expression constraint argument.",
            WellKnownTags.Keyword,
            parentOpt
        );
        context.AddIfMissing(
    ...
    private (RoutePatternNode Parent, RoutePatternToken Token)? FindToken(
        RoutePatternNode parent,
        VirtualChar ch
    )
    {
    ...
    private readonly struct EmbeddedCompletionContext
    {
        ...
        )
        {
            ...
            var replacementEnd = parentOpt != null ? parentOpt.GetSpan().End : Position;
