...
public class RoutePatternCompletionProvider : CompletionProvider
{
    ...
    public override async Task ProvideCompletionsAsync(CompletionContext context)
    {
        ...
        if (context.Position <= stringToken.SpanStart || context.Position >= stringToken.Span.End)
        {
            return;
        }
        ...
        foreach (var embeddedItem in routePatternCompletionContext.Items)
        {
            ...
            if (change.NewPosition != null)
            {
                properties.Add(
                    NewPositionKey,
                    change.NewPosition.Value.ToString(CultureInfo.InvariantCulture)
                );
            }
    ...
    private static (RoutePatternNode Parent, RoutePatternToken Token)? GetCurrentToken(
...
If there are two arguments then the string length must be greater than, or equal to, the first argument and less than, or equal to, the second argument. For example, length(8,16) matches a string at least 8 and no more than 16 characters long.",
    ...
    private static (RoutePatternNode Parent, RoutePatternToken Token)? FindToken(
