...
public sealed class AddAuthorizationBuilderAnalyzer : DiagnosticAnalyzer
{
    public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics =>
        ImmutableArray.Create(DiagnosticDescriptors.UseAddAuthorizationBuilder);
    ...
    private static void OnCompilationStart(CompilationStartAnalysisContext context)
    {
        ...
        var policyServiceCollectionExtensions = wellKnownTypes.Get(
        ...
        var addAuthorizationMethod = policyServiceCollectionExtensions
            .GetMembers()
    ...
    private static bool IsCompatibleWithAuthorizationBuilder(
        IInvocationOperation invocation,
        AuthorizationOptionsTypes authorizationOptionsTypes
    )
    {
        if (
            TryGetConfigureArgumentOperation(invocation, out var configureArgumentOperation)
            && TryGetConfigureDelegateCreationOperation(
                configureArgumentOperation,
                out var configureDelegateCreationOperation
            )
            && TryGetConfigureAnonymousFunctionOperation(
                configureDelegateCreationOperation,
                out var configureAnonymousFunctionOperation
            )
            && TryGetConfigureBlockOperation(
                configureAnonymousFunctionOperation,
                out var configureBlockOperation
            )
        )
        {
            // Ensure that the child operations of the configuration action passed to AddAuthorization are all
            // related to AuthorizationOptions.
            ...
            return allOperationsInvolveAuthorizationOptions
                // Ensure that the configuration action passed to AddAuthorization does not use any
                // AuthorizationOptions-specific APIs.
                && IsConfigureActionCompatibleWithAuthorizationBuilder(
                    configureBlockOperation,
    ...
    private static bool TryGetConfigureArgumentOperation(
        IInvocationOperation invocation,
        [NotNullWhen(true)] out IArgumentOperation? configureArgumentOperation
    )
    {
    ...
    private static bool TryGetConfigureDelegateCreationOperation(
        IArgumentOperation configureArgumentOperation,
        [NotNullWhen(true)] out IDelegateCreationOperation? configureDelegateCreationOperation
    )
    {
        configureDelegateCreationOperation = null;

        if (
            configureArgumentOperation is { ChildOperations: { Count: 1 } argumentChildOperations }
            && argumentChildOperations.First()
                is IDelegateCreationOperation delegateCreationOperation
        )
        {
    ...
    private static bool TryGetConfigureAnonymousFunctionOperation(
    ...
    )
    {
        ...
        if (
            configureDelegateCreationOperation
                is { ChildOperations: { Count: 1 } delegateCreationChildOperations }
            && delegateCreationChildOperations.First()
                is IAnonymousFunctionOperation anonymousFunctionOperation
        )
        {
    ...
    private static bool TryGetConfigureBlockOperation(
        IAnonymousFunctionOperation configureAnonymousFunctionOperation,
        [NotNullWhen(true)] out IBlockOperation? configureBlockOperation
    )
    {
    ...
    private static bool DoesOperationInvolveAuthorizationOptions(
    ...
    )
    {
        if (
            ...
            {
                Operation: { } expressionStatementOperation
            }
        )
        {
            if (
    ...
    private static bool IsConfigureActionCompatibleWithAuthorizationBuilder(
        IBlockOperation configureAction,
        AuthorizationOptionsTypes authorizationOptionsTypes
    )
    {
        var usesAuthorizationOptionsSpecificAPIs = configureAction
            .Descendants()
            .Any(operation =>
    ...
    private static bool UsesAuthorizationOptionsSpecificGetters(
        IOperation operation,
        AuthorizationOptionsTypes authorizationOptionsTypes
    )
    {
        if (operation is IPropertyReferenceOperation propertyReferenceOperation)
        {
            ...
            if (
                propertyReferenceOperation.Parent
                    is IAssignmentOperation { Target: IPropertyReferenceOperation targetProperty }
            ...
            )
            {
                ...
                if (
                    propertyReferenceOperation.Parent
                        is IAssignmentOperation { Value: IPropertyReferenceOperation valueProperty }
            ...
            if (
                SymbolEqualityComparer.Default.Equals(
                    property,
                    authorizationOptionsTypes.DefaultPolicy
                )
                || SymbolEqualityComparer.Default.Equals(
                    property,
                    authorizationOptionsTypes.FallbackPolicy
                )
                || SymbolEqualityComparer.Default.Equals(
                    property,
                    authorizationOptionsTypes.InvokeHandlersAfterFailure
                )
            )
            {
                return true;
    ...
    private static bool UsesAuthorizationOptionsGetPolicy(
        IOperation operation,
        AuthorizationOptionsTypes authorizationOptionsTypes
    )
    {
        if (
            ...
            && SymbolEqualityComparer.Default.Equals(
                methodReferenceOperation.Member,
                authorizationOptionsTypes.GetPolicy
            )
            && SymbolEqualityComparer.Default.Equals(
    ...
    )
    {
        context.ReportDiagnostic(
            Diagnostic.Create(DiagnosticDescriptors.UseAddAuthorizationBuilder, location)
        );
    }
}
