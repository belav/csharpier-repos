...
public class AssemblyTestLogTests : LoggedTest
{
    ...
    public Task ForAssemblyWritesToAssemblyBaseDirectory() =>
        RunTestLogFunctionalTest(
            (tempDir) =>
            {
                ...
                var assembly = TestableAssembly.Create(
                    typeof(AssemblyTestLog),
                    logDirectory: tempDir
                );
                var assemblyName = assembly.GetName().Name;
                var testName =
                    $"{TestableAssembly.TestClassName}.{TestableAssembly.TestMethodName}";
                ...
                Assert.True(
                    File.Exists(globalLogPath),
                    $"Expected global log file {globalLogPath} to exist."
                );
    ...
    public void TestLogWritesToITestOutputHelper()
    {
        ...
        using var assemblyLog = AssemblyTestLog.Create(
    ...
    public Task TestLogWritesToGlobalLogFile() =>
        RunTestLogFunctionalTest(
            (tempDir) =>
            {
                ...
                using (
                    var testAssemblyLog = AssemblyTestLog.Create(
                        TestableAssembly.ThisAssembly,
                        baseDirectory: tempDir
                    )
                )
        ...
        RunTestLogFunctionalTest(
            (tempDir) =>
            {
                ...
                )
                {
                    ...
                    Assert.True(
                        File.Exists(testLog),
                        $"Expected test log file {testLog} to exist."
                    );
                }
                ...
                Assert.True(
                    !File.Exists(globalLogPath),
                    $"Expected no global log file {globalLogPath} to exist."
                );
                Assert.True(
                    !File.Exists(testLog),
                    $"Expected no test log file {testLog} to exist."
                );
            }
        );
    ...
    public Task TestLogTruncatesTestNameToAvoidLongPaths() =>
        RunTestLogFunctionalTest(
            (tempDir) =>
            {
                ...
                Assert.Equal(
                    longTestName.Substring(
                        longTestName.Length - testFileName.Length / 2,
                        testFileName.Length / 2
                    ),
                    testFileName.Substring(
                        testFileName.Length - testFileName.Length / 2,
                        testFileName.Length / 2
                    )
                );
            }
    ...
    private static string MakeConsistent(string input)
    {
        return string.Join(
            ...
            input
                ...
                .Select(line =>
                {
                    var strippedPrefix = line.Contains('[')
