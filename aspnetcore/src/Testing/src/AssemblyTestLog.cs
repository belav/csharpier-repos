...
namespace Microsoft.AspNetCore.Testing
{
    ...
    {
        private static readonly string MaxPathLengthEnvironmentVariableName =
        ...
        public IServiceProvider CreateLoggerServices(
            ...
            if (!string.IsNullOrEmpty(_baseDirectory))
            {
                ...
                if (
                    ...
                    testName =
                        testName.Substring(0, testNameLength / 2)
                        + testName.Substring(
                            testName.Length - testNameLength / 2,
                            testNameLength / 2
                        );
                ...
                var testOutputFile = Path.Combine(
                    logOutputDirectory,
                    $"{testName}{LogFileExtension}"
                );
        ...
        public static AssemblyTestLog Create(Assembly assembly, string baseDirectory)
        {
            ...
            {
                baseDirectory = TestFileOutputContext.GetAssemblyBaseDirectory(
                    assembly,
                    baseDirectory
                );
