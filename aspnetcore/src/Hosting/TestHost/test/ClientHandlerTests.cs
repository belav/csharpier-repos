...
public class ClientHandlerTests
{
    ...
    public async Task SlashUrlEncodedDoesNotGetDecoded()
    {
        var handler = new ClientHandler(
            new PathString(),
            new InspectingApplication(features =>
            {
                Assert.True(
        ...
        var handler = new ClientHandler(
            ...
            new InspectingApplication(features =>
            {
                Assert.Equal(
                    ...
                    features.Get<IHttpConnectionFeature>().RemoteIpAddress
                );
            }),
            context =>
            {
    ...
    {
        var hasStartedTcs = new TaskCompletionSource(
            TaskCreationOptions.RunContinuationsAsynchronously
        );
        var hasAssertedResponseTcs = new TaskCompletionSource(
            TaskCreationOptions.RunContinuationsAsynchronously
        );
