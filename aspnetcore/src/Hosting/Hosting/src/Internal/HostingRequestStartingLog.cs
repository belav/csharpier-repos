...
namespace Microsoft.AspNetCore.Hosting
{
    internal class HostingRequestStartingLog : IReadOnlyList<KeyValuePair<string, object?>>
    {
        ...
        internal static readonly Func<object, Exception?, string> Callback = (state, exception) =>
            ((HostingRequestStartingLog)state).ToString();
        ...
        public override string ToString()
        {
            if (_cachedToString == null)
            {
                ...
                _cachedToString =
                    $"{LogPreamble}{request.Protocol} {request.Method} {request.Scheme}://{request.Host.Value}{request.PathBase.Value}{request.Path.Value}{request.QueryString.Value} {EscapedValueOrEmptyMarker(request.ContentType)} {ValueOrEmptyMarker(request.ContentLength)}";
                ;
            }
        ...
        internal string ToStringWithoutPreamble() => ToString().Substring(LogPreamble.Length);
        ...
        internal static string ValueOrEmptyMarker<T>(T? potentialValue)
