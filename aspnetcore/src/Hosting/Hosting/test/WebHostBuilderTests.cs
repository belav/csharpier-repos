...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    public async Task UseStartupImplementingIStartupWorks()
    {
        ...
        using (
            var host = builder
                .UseServer(server)
                .UseStartup(context => new DelegatingStartupWithIStartup(
                    ConfigureServices,
                    Configure
                ))
                .Build()
...
#pragma warning disable CS0618 // Type or member is obsolete
                    var heDescriptor = services.SingleOrDefault(s =>
                        s.ServiceType == typeof(IHostingEnvironment)
                    );
                    Assert.NotNull(heDescriptor);
                    Assert.NotNull(heDescriptor.ImplementationInstance);
#pragma warning restore CS0618 // Type or member is obsolete
                    var wheDescriptor = services.SingleOrDefault(s =>
                        s.ServiceType == typeof(IWebHostEnvironment)
                    );
