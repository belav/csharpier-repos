...
public class HostingApplicationDiagnosticsTests
{
    ...
    public async Task EventCountersAndMetricsValues()
    {
        ...
        var eventListener = new TestCounterListener(
            new[] { "requests-per-second", "total-requests", "current-requests", "failed-requests" }
        );
        ...
        var hostingApplication1 = CreateApplication(
            out var features1,
            eventSource: hostingEventSource,
            meterFactory: testMeterFactory1
        );
        var hostingApplication2 = CreateApplication(
            out var features2,
            eventSource: hostingEventSource,
            meterFactory: testMeterFactory2
        );

        using var activeRequestsCollector1 = new MetricCollector<long>(
            testMeterFactory1,
            HostingMetrics.MeterName,
            "http.server.active_requests"
        );
        using var activeRequestsCollector2 = new MetricCollector<long>(
            testMeterFactory2,
            HostingMetrics.MeterName,
            "http.server.active_requests"
        );
        using var requestDurationCollector1 = new MetricCollector<double>(
            testMeterFactory1,
            HostingMetrics.MeterName,
            "http.server.request.duration"
        );
        using var requestDurationCollector2 = new MetricCollector<double>(
            testMeterFactory2,
            HostingMetrics.MeterName,
            "http.server.request.duration"
        );
        ...
        Assert.Collection(
            activeRequestsCollector1.GetMeasurementSnapshot(),
            m => Assert.Equal(1, m.Value),
            m => Assert.Equal(-1, m.Value)
        );
        Assert.Collection(
            activeRequestsCollector2.GetMeasurementSnapshot(),
            m => Assert.Equal(1, m.Value),
            m => Assert.Equal(-1, m.Value)
        );
        Assert.Collection(
    ...
    public void MetricsEnabled()
    {
        ...
        var testMeterFactory = new TestMeterFactory();
        using var activeRequestsCollector = new MetricCollector<long>(
            testMeterFactory,
            HostingMetrics.MeterName,
            "http.server.active_requests"
        );
        using var requestDurationCollector = new MetricCollector<double>(
            testMeterFactory,
            HostingMetrics.MeterName,
            "http.server.request.duration"
        );
    ...
    public void SamplersReceiveCorrectParentAndTraceIds()
    {
        ...
        using var listener = new ActivityListener
        {
            ...
            Sample = (ref ActivityCreationOptions<ActivityContext> options) =>
                ComputeActivitySamplingResult(ref options),
        ...
        static ActivitySamplingResult ComputeActivitySamplingResult(
    ...
    public void RequestLogs()
    {
        ...
        Assert.Equal(
            "Request starting 1.1 POST http://localhost/hello - text/plain 1024",
            startLog.Message
        );
        Assert.Equal(
            "Request reached the end of the middleware pipeline without being handled by application code. Request path: POST http://localhost/hello, Response status code: 404",
            unhandedLog.Message
        );
    ...
    private static HostingApplication CreateApplication(
        ...
        HostingEventSource eventSource = null,
