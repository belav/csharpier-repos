...
{
    /// <summary>
    /// Returns the normalized form of the requested range if the Range Header in the <see
    // cref="HttpContext.Request"/> is valid.
    /// </summary>
    /// <param name="context">The <see cref="HttpContext"/> associated with the request.</param>
    /// <param name="requestHeaders">The <see cref="RequestHeaders"/> associated with the given
    // <paramref name="context"/>.</param>
    /// <param name="length">The total length of the file representation requested.</param>
    /// <param name="logger">The <see cref="ILogger"/>.</param>
    /// <returns>A boolean value which represents if the <paramref name="requestHeaders"/> contain a
    // single valid
    /// range request. A <see cref="RangeItemHeaderValue"/> which represents the normalized form of the
    /// range parsed from the <paramref name="requestHeaders"/> or <c>null</c> if it cannot be
    // normalized.</returns>
    /// <remark>If the Range header exists but cannot be parsed correctly, or if the provided length is
    // 0, then the range request cannot be satisfied (status 416).
    ...
    )
    {
        ...
        if (rawRangeHeader.Count > 1 || (rawRangeHeader[0] ?? string.Empty).Contains(','))
        {
            ...
            // The spec allows for multiple ranges but we choose not to support them because the client may
            // request
