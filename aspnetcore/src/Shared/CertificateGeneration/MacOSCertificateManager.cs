...
internal sealed class MacOSCertificateManager : CertificateManager
{
    ...
    private static readonly string MacOSUserHttpsCertificateLocation = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.UserProfile),
        ".aspnet",
        "dev-certs",
        "https"
    ...
    private const string MacOSVerifyCertificateCommandLineArgumentsFormat =
        "verify-cert -c \"{0}\" -p basic -p ssl";
    ...
    private static readonly string MacOSTrustCertificateCommandLineArguments =
        $"add-trusted-cert -p basic -p ssl -k \"{MacOSUserKeychain}\" ";
    ...
    private const string MacOSUntrustLegacyCertificateCommandLineArguments =
    ...
    public override bool IsTrusted(X509Certificate2 certificate)
    {
        ...
        try
        {
            ExportCertificate(
                ...
                includePrivateKey: false,
                password: null,
                CertificateKeyExportFormat.Pem
            );
...
{output}"
    ...
    private static bool IsCertOnKeychain(string keychain, X509Certificate2 certificate)
    {
        ...
        using var findCertificateProcess = Process.Start(
            new ProcessStartInfo(
                MacOSFindCertificateOnKeychainCommandLine,
                string.Format(
                    CultureInfo.InvariantCulture,
                    MacOSFindCertificateOnKeychainCommandLineArgumentsFormat,
                    subject,
                    keychain
                )
            )
    ...
    )
    {
        ...
        try
        {
            ...
            if (Log.IsEnabled())
            {
                Log.MacOSAddCertificateToUserProfileDirStart(
                    MacOSUserKeychain,
    ...
    private static void SaveCertificateToUserKeychain(X509Certificate2 certificate)
    {
        ...
        using (var process = Process.Start(processInfo))
        {
            ...
            if (process.ExitCode != 0)
            {
                ...
                throw new InvalidOperationException(
                    "Failed to add the certificate to the keychain. Are you running in a non-interactive session perhaps?"
                );
            }
        }
    ...
    private static string GetCertificateFilePath(X509Certificate2 certificate) =>
        Path.Combine(
            MacOSUserHttpsCertificateLocation,
    ...
    {
        if (
            store.Name! == StoreName.My.ToString()
            && store.Location == StoreLocation.CurrentUser
            && Directory.Exists(MacOSUserHttpsCertificateLocation)
        )
        {
            ...
            // from the first set.
            var onDiskAndKeychain = requireExportable
                ? certsFromStore.Intersect(certsFromDisk, ThumbprintComparer.Instance)
    ...
    {
        public static readonly IEqualityComparer<X509Certificate2> Instance =
            new ThumbprintComparer();
...
#pragma warning restore CS8769 // Nullability of reference types in type of parameter doesn't match implemented member (possibly because of nullability attributes).
    ...
    private static ICollection<X509Certificate2> GetCertsFromDisk()
    {
        ...
        {
            var certificateFiles = Directory.EnumerateFiles(
                MacOSUserHttpsCertificateLocation,
