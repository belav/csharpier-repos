    ...
    : JsonConverter<HttpValidationProblemDetails>
{
    ...
    [RequiresUnreferencedCode(
        "JSON serialization and deserialization ProblemDetails.Extensions might require types that cannot be statically analyzed. "
    )]
    ...
    )
    {
        ...
        [UnconditionalSuppressMessage(
            "Trimmer",
            "IL2026",
            Justification = "We ensure Dictionary<string, string[]> is preserved."
        )]
        [DynamicDependency(
        ...
        static Dictionary<string, string[]>? DeserializeErrors(
            ref Utf8JsonReader reader,
            JsonSerializerOptions options
    ...
    )
    {
        ...
        [UnconditionalSuppressMessage(
            "Trimmer",
            "IL2026",
            Justification = "We ensure IDictionary<string, string[]> is preserved."
        )]
        [DynamicDependency(
            DynamicallyAccessedMemberTypes.PublicProperties,
            typeof(IDictionary<string, string[]>)
        )]
        static void SerializeErrors(
            Utf8JsonWriter writer,
