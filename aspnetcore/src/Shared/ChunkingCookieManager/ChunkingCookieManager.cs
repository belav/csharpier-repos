...
#pragma warning restore CA1822 // Mark members as static
    {
        ...
        if (domainHasValue)
        {
            rejectPredicate = value =>
                predicate(value)
                && value.Contains("domain=" + options.Domain, StringComparison.OrdinalIgnoreCase);
        }
        else if (pathHasValue)
        {
            rejectPredicate = value =>
                predicate(value)
                && value.Contains("path=" + options.Path, StringComparison.OrdinalIgnoreCase);
        }
        else
        {
