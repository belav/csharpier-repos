...
internal static class ParsabilityHelper
{
    private static readonly BoundedCacheWithFactory<
        ITypeSymbol,
        (BindabilityMethod?, IMethodSymbol?)
    > BindabilityCache = new();
    private static readonly BoundedCacheWithFactory<
        ITypeSymbol,
        (Parsability, ParsabilityMethod?)
    > ParsabilityCache = new();

    private static bool IsTypeAlwaysParsable(
        ITypeSymbol typeSymbol,
        WellKnownTypes wellKnownTypes,
        [NotNullWhen(true)] out ParsabilityMethod? parsabilityMethod
    )
    {
        // Any enum is valid.
    ...
    internal static Parsability GetParsability(
        ITypeSymbol typeSymbol,
        WellKnownTypes wellKnownTypes
    )
    {
    ...
    internal static Parsability GetParsability(
        ITypeSymbol typeSymbol,
        WellKnownTypes wellKnownTypes,
        [NotNullWhen(false)] out ParsabilityMethod? parsabilityMethod
    )
    {
        var parsability = Parsability.NotParsable;
        parsabilityMethod = null;

        (parsability, parsabilityMethod) = ParsabilityCache.GetOrCreateValue(
            typeSymbol,
            (typeSymbol) =>
            {
                ...
                var tryParseMethods = typeSymbol
                    .GetThisAndBaseTypes()
    ...
    private static bool IsTryParseWithFormat(
    ...
    )
    {
        ...
        var implementsIParsable = typeSymbol.AllInterfaces.Any(i =>
            SymbolEqualityComparer.Default.Equals(i.ConstructedFrom, iParsableTypeSymbol)
        );
        return implementsIParsable;
    }

    private static bool IsBindableViaIBindableFromHttpContext(
        ITypeSymbol typeSymbol,
        WellKnownTypes wellKnownTypes
    )
    {
        ...
        var constructedTypeSymbol = typeSymbol.AllInterfaces.FirstOrDefault(i =>
            SymbolEqualityComparer.Default.Equals(
                i.ConstructedFrom,
    ...
    private static bool IsBindAsync(
        IMethodSymbol methodSymbol,
        ITypeSymbol typeSymbol,
        WellKnownTypes wellKnownTypes
    )
    ...
    private static bool IsBindAsyncWithParameter(
        IMethodSymbol methodSymbol,
        ITypeSymbol typeSymbol,
        WellKnownTypes wellKnownTypes
    )
    ...
    private static bool IsReturningValueTaskOfTOrNullableT(
    ...
    internal static Bindability GetBindability(
        ITypeSymbol typeSymbol,
        WellKnownTypes wellKnownTypes,
        out BindabilityMethod? bindabilityMethod,
        out IMethodSymbol? bindMethodSymbol
    )
    {
        ...
        (bindabilityMethod, bindMethodSymbol) = BindabilityCache.GetOrCreateValue(
            typeSymbol,
            (typeSymbol) =>
            {
                ...
                var searchCandidates = typeSymbol
                    .GetThisAndBaseTypes()
        ...
        {
            if (
                bindAsyncMethod.ReturnType is INamedTypeSymbol returnType
                && !IsReturningValueTaskOfTOrNullableT(returnType, typeSymbol, wellKnownTypes)
            )
            {
