...
[UnconditionalSuppressMessage(
    "Trimmer",
    "IL2060",
    Justification = "Trimmer warnings are presented in RequestDelegateFactory."
)]
[UnconditionalSuppressMessage(
    "Trimmer",
    "IL2065",
...
internal sealed class ParameterBindingMethodCache
{
    ...
    private static readonly MethodInfo BindAsyncMethod =
        typeof(ParameterBindingMethodCache).GetMethod(
            nameof(BindAsync),
            BindingFlags.NonPublic | BindingFlags.Static
        )!;
    private static readonly MethodInfo UriTryCreateMethod = typeof(Uri).GetMethod(
        nameof(Uri.TryCreate),
    ...
    internal static readonly ParameterExpression HttpContextExpr = Expression.Parameter(
        typeof(HttpContext),
        "httpContext"
    );
    ...
    private readonly ConcurrentDictionary<
        Type,
        (Func<ParameterInfo, Expression>?, int)
    > _bindAsyncMethodCallCache = new();
    private readonly ConcurrentDictionary<
        Type,
    ...
    public ParameterBindingMethodCache(
        bool preferNonGenericEnumParseOverload,
        bool throwOnInvalidMethod = true
    )
    {
    ...
    [RequiresUnreferencedCode(
        "Performs reflection on type hierarchy. This cannot be statically analyzed."
    )]
    ...
    public Func<ParameterExpression, Expression, Expression>? FindTryParseMethod(Type type)
    {
        ...
        Func<ParameterExpression, Expression, Expression>? Finder(Type type)
        {
                    ...
                    return Expression.Block(
                        ...
                        Expression.Assign(
                            ...
                            Expression.Condition(
                                success,
                                Expression.Convert(enumAsObject, type),
            ...
            {
                return (expression, formatProvider) =>
                    Expression.Call(methodInfo, TempSourceStringExpr, formatProvider, expression);
            }
            ...
            if (
                _throwOnInvalidMethod
                && GetAnyMethodFromHierarchy(type, "TryParse") is MethodInfo invalidMethod
            )
            {
    ...
    public (Expression? Expression, int ParamCount) FindBindAsyncMethod(ParameterInfo parameter)
    {
        (Func<ParameterInfo, Expression>?, int) Finder(Type nonNullableParameterType)
        {
            ...
            if (methodInfo is null)
            {
                ...
                methodInfo = GetStaticMethodFromHierarchy(
                    ...
                    "BindAsync",
                    new[] { typeof(HttpContext), typeof(ParameterInfo) },
                    ValidateReturnType
                );
            ...
            if (methodInfo is not null)
            {
                ...
                if (valueTaskResultType == nonNullableParameterType)
                {
                    return (
                        (parameter) =>
                        {
                            ...
                            return Expression.Call(
                                GetGenericConvertValueTask(nonNullableParameterType),
                                typedCall
                            );
                        },
                    ...
                    [UnconditionalSuppressMessage(
                        "Trimmer",
                        "IL2060",
                        Justification = "Linker workaround. The type is annotated with RequiresUnreferencedCode"
                    )]
                    static MethodInfo GetGenericConvertValueTask(Type nonNullableParameterType) =>
                        ConvertValueTaskMethod.MakeGenericMethod(nonNullableParameterType);
                }
                // ValueTask<Nullable<{type}>>?
                else if (
                    valueTaskResultType.IsGenericType
                ...
                )
                {
                    return (
                        (parameter) =>
                        {
                            ...
                            return Expression.Call(
                                GetGenericConvertValueTaskOfNullableResult(
                                    nonNullableParameterType
                                ),
                                typedCall
                            );
                    ...
                    static MethodInfo GetGenericConvertValueTaskOfNullableResult(
                        Type nonNullableParameterType
                    ) =>
                        ConvertValueTaskOfNullableResultMethod.MakeGenericMethod(
                            nonNullableParameterType
                        );
                }
            }
            ...
            if (
                _throwOnInvalidMethod
                && GetAnyMethodFromHierarchy(nonNullableParameterType, "BindAsync")
                    is MethodInfo invalidBindMethod
            )
            {
                var stringBuilder = new StringBuilder();
                stringBuilder.AppendLine(
                    CultureInfo.InvariantCulture,
                    $"BindAsync method found on {TypeNameHelper.GetTypeDisplayName(nonNullableParameterType, fullName: false)} with incorrect format. Must be a static method with format"
                );
                stringBuilder.AppendLine(
                    CultureInfo.InvariantCulture,
                    $"ValueTask<{TypeNameHelper.GetTypeDisplayName(nonNullableParameterType, fullName: false)}> BindAsync(HttpContext context, ParameterInfo parameter)"
                );
                stringBuilder.AppendLine(
                    CultureInfo.InvariantCulture,
                    $"ValueTask<{TypeNameHelper.GetTypeDisplayName(nonNullableParameterType, fullName: false)}> BindAsync(HttpContext context)"
                );
                stringBuilder.AppendLine(
                    CultureInfo.InvariantCulture,
                    $"ValueTask<{TypeNameHelper.GetTypeDisplayName(nonNullableParameterType, fullName: false)}?> BindAsync(HttpContext context, ParameterInfo parameter)"
        ...
        var (method, paramCount) = _bindAsyncMethodCallCache.GetOrAdd(
            nonNullableParameterType,
            Finder
        );
    ...
    public (ConstructorInfo?, ConstructorParameter[]) FindConstructor(Type type)
    {
        static (ConstructorInfo? constructor, ConstructorParameter[] parameters) Finder(Type type)
        {
            ...
            {
                lookupTable.Add(
                    new ParameterLookupKey(properties[i].Name, properties[i].PropertyType),
                    properties[i]
                );
            }
    ...
    private static ConstructorInfo? GetConstructor(Type type)
    {
        ...
        var parameterlessConstructor = constructors.SingleOrDefault(
    ...
    private static MethodInfo? GetIBindableFromHttpContextMethod(Type type)
    {
        ...
        foreach (var i in type.GetInterfaces())
        {
            if (
                i.IsGenericType
                && i.GetGenericTypeDefinition() == typeof(IBindableFromHttpContext<>)
    ...
    private MethodInfo? GetStaticMethodFromHierarchy(
        Type type,
        string name,
        Type[] parameterTypes,
    ...
    )
    {
        bool IsMatch(MethodInfo? method) =>
            method is not null && !method.IsAbstract && validateReturnType(method);
        ...
        {
            var interfaceMethod = implementedInterface.GetMethod(
                name,
                BindingFlags.Public | BindingFlags.Static,
                parameterTypes
            );
            ...
            if (IsMatch(interfaceMethod))
            {
                if (candidateInterfaceMethodInfo is not null)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            $"{TypeNameHelper.GetTypeDisplayName(type, fullName: false)} implements multiple interfaces defining a static {interfaceMethod} method causing ambiguity."
                        );
                    }
    ...
    private static bool TryGetDateTimeTryParseMethod(
        Type type,
    ...
    private static bool TryGetNumberStylesTryGetMethod(
        Type type,
        [NotNullWhen(true)] out MethodInfo? method,
        [NotNullWhen(true)] out NumberStyles? numberStyles
    )
    {
        method = null;
    ...
    private static ValueTask<object?> ConvertValueTask<T>(ValueTask<T> typedValueTask)
    {
        ...
        static async ValueTask<object?> ConvertAwaited(ValueTask<T> typedValueTask) =>
            await typedValueTask;
    ...
    ) where T : struct
    {
        ...
        static async ValueTask<object?> ConvertAwaited(ValueTask<Nullable<T>> typedValueTask) =>
            await typedValueTask;
        return ConvertAwaited(typedValueTask);
    }
