...
internal static class HttpCharacters
{
    ...
    private static readonly SearchValues<byte> _allowedAuthorityBytes = SearchValues.Create(
    ...
    private static readonly SearchValues<char> _allowedTokenChars = SearchValues.Create(
        "!#$%&'*+-.^_`|~" + AlphaNumeric
    );
    private static readonly SearchValues<byte> _allowedTokenBytes = SearchValues.Create(
    ...
    // HTAB, [VCHAR, SP]
    private static readonly SearchValues<char> _allowedFieldChars = SearchValues.Create(
        "\t !\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~" + AlphaNumeric
    );
