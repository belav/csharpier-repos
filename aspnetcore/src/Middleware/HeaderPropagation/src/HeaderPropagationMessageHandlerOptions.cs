...
public class HeaderPropagationMessageHandlerOptions
{
    ...
    /// </summary>
    /// <remarks>
    /// If <see cref="Headers"/> is empty, all the headers captured by the <see
    // cref="HeaderPropagationMiddleware"/> are propagated.
    /// Entries in <see cref="Headers"/> are processed in order while adding headers inside
    /// <see cref="HeaderPropagationMessageHandler"/>. This can cause an earlier entry to take
    // precedence
    /// over a later entry if they have the same <see
    // cref="HeaderPropagationMessageHandlerEntry.OutboundHeaderName"/>.
    /// </remarks>
