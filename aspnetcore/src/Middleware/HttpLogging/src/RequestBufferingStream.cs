...
internal sealed class RequestBufferingStream : BufferingStream
{
    ...
    )
    {
        ...
        // Zero-byte reads (where the passed in buffer has 0 length) can occur when using PipeReader, we
        // don't want to accidentally complete the RequestBody logging in this case
