...
public static class HttpLoggingEndpointConventionBuilderExtensions
{
    ...
    public static TBuilder WithHttpLogging<TBuilder>(
        this TBuilder builder,
        HttpLoggingFields loggingFields,
        int? requestBodyLogLimit = null,
        int? responseBodyLogLimit = null
    )
        where TBuilder : IEndpointConventionBuilder
    {
        ...
        {
            ArgumentOutOfRangeException.ThrowIfLessThan(
                responseBodyLogLimit.Value,
                0,
                nameof(responseBodyLogLimit)
            );
