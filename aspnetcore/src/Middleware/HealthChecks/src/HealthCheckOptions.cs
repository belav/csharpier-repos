...
public class HealthCheckOptions
{
    ...
    private IDictionary<HealthStatus, int> _resultStatusCodes = new Dictionary<HealthStatus, int>(
    ...
    /// to the response. This property can be used to configure the status codes returned for each
    // status.
    /// </summary>
    /// <remarks>
    /// Setting this property to <c>null</c> resets the mapping to its default value which maps
    /// <see cref="HealthStatus.Healthy"/> to 200 (OK), <see cref="HealthStatus.Degraded"/> to 200 (OK)
    // and
    /// <see cref="HealthStatus.Unhealthy"/> to 503 (Service Unavailable).
    /// </remarks>
    /// <exception cref="InvalidOperationException">Thrown if at least one <see cref="HealthStatus"/> is
    // missing when setting this property.</exception>
    ...
    /// </summary>
    /// <remarks>
    /// The default value is a delegate that will write a minimal <c>text/plain</c> response with the
    // value
    ...
    /// <summary>
    /// Gets or sets a value that controls whether responses from the health check middleware can be
    // cached.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The health check middleware does not perform caching of any kind. This setting configures
    // whether
    ...
    /// <c>Cache-Control</c>, <c>Expires</c>, and <c>Pragma</c> headers to prevent response caching. If
    // the value
