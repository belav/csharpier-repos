...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    public async Task Error_page_displayed_no_migrations()
    {
        using (var database = SqlTestStore.CreateScratch())
        {
            ...
            Assert.Contains(
                StringsHelpers
                    .GetResourceString("DatabaseErrorPage_AddMigrationCommandPMC")
                    .Replace(">", "&gt;"),
                content
            );
            Assert.Contains(
                StringsHelpers
                    .GetResourceString("DatabaseErrorPage_ApplyMigrationsCommandPMC")
                    .Replace(">", "&gt;"),
                content
            );
        }
    ...
    public async Task No_exception_on_diagnostic_event_received_when_null_state()
    {
        ...
        {
            using (
                var server = await SetupServer<BloggingContext, NoMigrationsMiddleware>(database)
            )
            {
    ...
    public async Task Error_page_displayed_pending_model_changes()
    {
        using (var database = SqlTestStore.CreateScratch())
        {
            ...
            Assert.Contains(
                StringsHelpers
                    .GetResourceString("DatabaseErrorPage_AddMigrationCommandCLI")
                    .Replace(">", "&gt;"),
                content
            );
            Assert.Contains(
            ...
            Assert.Contains(
                StringsHelpers
                    .GetResourceString("DatabaseErrorPage_ApplyMigrationsCommandCLI")
                    .Replace(">", "&gt;"),
                content
            );
            Assert.Contains(
    ...
    public async Task Error_page_then_apply_migrations()
    {
        using (var database = SqlTestStore.CreateScratch())
        {
            ...
            Assert.Contains(
                "req.open(\"POST\", \""
                    + JavaScriptEncode(expectedMigrationsEndpoint)
                    + "\", true);",
                content
            );
            Assert.Contains(
                "data-assemblyname=\"" + JavaScriptEncode(expectedContextType) + "\"",
                content
            );
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            var content = await response.Content.ReadAsStringAsync();
            Assert.Contains(
                "req.open(\"POST\", \"" + JavaScriptEncode(migrationsEndpoint) + "\", true);",
                content
            );
        }
    }
