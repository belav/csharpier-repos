    ...
    public class CorsMiddlewareTests
    {
        ...
        public async Task CorsRequest_MatchPolicy_SetsResponseHeaders()
        {
            ...
            using (var server = host.GetTestServer())
            {
                ...
                Assert.Equal(
                    "AllowedHeader",
                    response.Headers.GetValues(CorsConstants.AccessControlExposeHeaders)
                        .FirstOrDefault()
                );
            }
        }
        ...
        {
            // Arrange
            var policy = new CorsPolicyBuilder(OriginUrl).AllowAnyHeader()
                .AllowAnyMethod()
                .AllowCredentials()
                .Build();
                                    ...
                                    services.AddCors(
                                        options =>
                                        {
                                            options.AddDefaultPolicy(
                                                policyBuilder =>
                                                {
                                                    policyBuilder.WithOrigins(OriginUrl)
                                                        .WithMethods("PUT")
                                                        .WithHeaders("Header1")
                                                        .WithExposedHeaders("AllowedHeader")
                                                        .Build();
        ...
        public async Task Invoke_WithCustomPolicyProviderThatReturnsAsynchronously_Works()
        {
            ...
            var corsService = new CorsService(
                Options.Create(new CorsOptions()),
                NullLoggerFactory.Instance
            );
            var mockProvider = new Mock<ICorsPolicyProvider>();
            var loggerFactory = NullLoggerFactory.Instance;
            var policy = new CorsPolicyBuilder().WithOrigins(OriginUrl)
        ...
        public async Task Invoke_HasEndpointWithEnableMetadata_MiddlewareHasPolicyName_RunsCorsWithPolicyName()
        {
            ...
            httpContext.SetEndpoint(
                new Endpoint(
                    c => Task.CompletedTask,
                    new EndpointMetadataCollection(new EnableCorsAttribute("MetadataPolicyName")),
                    "Test endpoint"
                )
            );
            httpContext.Request.Headers.Add(CorsConstants.Origin, new[] { "http://example.com" });
        ...
        public async Task Invoke_HasEndpointWithEnableMetadata_HasSignificantDisableCors_ExecutesNextMiddleware()
        {
            ...
            Mock.Get(policyProvider)
                .Verify(
                    v => v.GetPolicyAsync(It.IsAny<HttpContext>(), It.IsAny<string>()),
                    Times.Never()
                );
            Mock.Get(corsService)
                .Verify(
                    v => v.EvaluatePolicy(It.IsAny<HttpContext>(), It.IsAny<CorsPolicy>()),
                    Times.Never()
                );
        }
        ...
        public async Task Invoke_HasEndpointRequireCorsMetadata_MiddlewareHasPolicy_RunsCorsWithPolicyName()
        {
            ...
            mockProvider.Setup(o => o.GetPolicyAsync(It.IsAny<HttpContext>(), It.IsAny<string>()))
                .Returns(Task.FromResult<CorsPolicy>(null))
                .Verifiable();
            mockCorsService.Setup(
                    o => o.EvaluatePolicy(It.IsAny<HttpContext>(), It.IsAny<CorsPolicy>())
                )
            ...
            httpContext.SetEndpoint(
                new Endpoint(
                    c => Task.CompletedTask,
                    new EndpointMetadataCollection(new CorsPolicyMetadata(metadataPolicy)),
                    "Test endpoint"
                )
            );
            httpContext.Request.Headers.Add(CorsConstants.Origin, new[] { "http://example.com" });
        ...
        public async Task Invoke_HasEndpointWithEnableMetadataWithNoName_RunsCorsWithStaticPolicy()
        {
            ...
            httpContext.SetEndpoint(
                new Endpoint(
                    c => Task.CompletedTask,
                    new EndpointMetadataCollection(new EnableCorsAttribute()),
                    "Test endpoint"
                )
            );
            httpContext.Request.Headers.Add(CorsConstants.Origin, new[] { "http://example.com" });
        ...
        public async Task Invoke_HasEndpointWithDisableMetadata_SkipCors()
        {
            ...
            httpContext.SetEndpoint(
                new Endpoint(
                    c => Task.CompletedTask,
                    new EndpointMetadataCollection(new DisableCorsAttribute()),
                    "Test endpoint"
                )
            );
            httpContext.Request.Headers.Add(CorsConstants.Origin, new[] { "http://example.com" });
        ...
        public async Task Invoke_HasEndpointWithMutlipleMetadata_SkipCorsBecauseOfMetadataOrder()
        {
            ...
            httpContext.SetEndpoint(
                new Endpoint(
                    c => Task.CompletedTask,
                    new EndpointMetadataCollection(
                        new EnableCorsAttribute("MetadataPolicyName"),
                        new DisableCorsAttribute()
                    ),
                    "Test endpoint"
                )
