        ...
        while (true)
        {
            ...
            else if (value.StartsWith(WebSocketDeflateConstants.ClientMaxWindowBits))
            {
                ...
                if (
                    !ParseWindowBits(
                        value,
                        WebSocketDeflateConstants.ClientMaxWindowBits,
                        out var clientMaxWindowBits
                    )
                )
                {
                    return false;
            ...
            else if (value.StartsWith(WebSocketDeflateConstants.ServerMaxWindowBits))
            {
                ...
                if (
                    !ParseWindowBits(
                        value,
                        WebSocketDeflateConstants.ServerMaxWindowBits,
                        out var parsedServerMaxWindowBits
                    )
                )
                {
                    return false;
