...
public class AutobahnTests : LoggedTest
{
    ...
    public async Task AutobahnTestSuite()
    {
        ...
        Assert.True(
            Wstest.Default != null,
            $"The 'wstest' executable (Autobahn WebSockets Test Suite) could not be found at '{Wstest.DefaultLocation}'. Run the Build Agent setup scripts to install it or see https://github.com/crossbario/autobahn-testsuite for instructions on manual installation."
        );
        ...
        using (StartLog(out var loggerFactory))
        {
            ...
            )
            {
                ...
                {
                    await tester.DeployTestAndAddToSpec(
                        ServerType.Kestrel,
                        ssl: false,
                        environment: "ManagedSockets",
    ...
    private bool IsIISExpress10Installed()
    {
        ...
        return File.Exists(iisExpressExe)
            && FileVersionInfo.GetVersionInfo(iisExpressExe).FileMajorPart >= 10;
    }
}
