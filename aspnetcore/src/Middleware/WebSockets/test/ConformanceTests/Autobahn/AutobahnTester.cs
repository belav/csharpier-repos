...
namespace Microsoft.AspNetCore.WebSockets.ConformanceTest.Autobahn
{
    public class AutobahnTester : IDisposable
    {
        ...
        private readonly List<AutobahnExpectations> _expectations =
            new List<AutobahnExpectations>();
        ...
        public async Task<AutobahnResult> Run(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var pinger = new Timer(
                    state => Pinger((CancellationToken)state),
                    pingCts.Token,
                ...
                _logger.LogInformation(
                    "Using 'wstest' from: {WsTestPath}",
                    Wstest.Default.Location
                );
                _logger.LogInformation(
                ...
                while (!token.IsCancellationRequested)
                {
                    try
                    {
                        foreach (var deployment in _deployments)
                        {
                            ...
                            {
                                _logger.LogWarning(
                                    "Non-successful response when pinging {url}: {statusCode} {reasonPhrase}",
        ...
        public void Verify(AutobahnResult result)
        {
            ...
            {
                var serverExpectation = _expectations.FirstOrDefault(
                    e => e.Server == serverResult.Server && e.Ssl == serverResult.Ssl
                );
                if (serverExpectation == null)
                {
                    failures.AppendLine(
                        $"Expected no results for server: {serverResult.Name} but found results!"
                    );
                }
                else
                {
        ...
        public async Task DeployTestAndAddToSpec(
            ServerType server,
            bool ssl,
            string environment,
        ...
        )
        {
            ...
            var logger = _loggerFactory.CreateLogger(
                $"AutobahnTestApp:{server}:{sslNamePart}:{environment}"
            );
            ...
            var parameters = new DeploymentParameters(
                ...
                ServerConfigTemplateContent =
                    (server == ServerType.IISExpress) ? File.ReadAllText(configPath) : null,
            };
