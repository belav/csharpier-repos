            ...
            var server = KestrelWebSocketHelpers.CreateServer(
                ...
                async context =>
                {
                    ...
                    var result = await webSocket.ReceiveAsync(
                        new ArraySegment<byte>(serverBuffer),
                        CancellationToken.None
                    );
        ...
        )
        {
            using (var client = new ClientWebSocket())
            {
                ...
                await client.SendAsync(
                    new ArraySegment<byte>(originalData),
    ...
    public async Task SendFragmentedData_Success()
    {
                ...
                async context =>
                {
                    ...
                    result = await webSocket.ReceiveAsync(
                        new ArraySegment<byte>(
                            serverBuffer,
                            totalReceived,
                            serverBuffer.Length - totalReceived
                        ),
                        CancellationToken.None
                    );
                    Assert.False(result.EndOfMessage);
        ...
        )
        {
            using (var client = new ClientWebSocket())
            {
                ...
                await client.SendAsync(
                    new ArraySegment<byte>(originalData, 0, 2),
                    WebSocketMessageType.Binary,
                    false,
                    CancellationToken.None
                );
                ...
                await client.SendAsync(
                    new ArraySegment<byte>(originalData, 2, 2),
                    WebSocketMessageType.Binary,
                    false,
                    CancellationToken.None
                );
                ...
                tcs = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
                await client.SendAsync(
                    new ArraySegment<byte>(originalData, 4, 7),
                    WebSocketMessageType.Binary,
                    true,
                    CancellationToken.None
                );
        ...
        )
        {
            using (var client = new ClientWebSocket())
            {
                ...
                var result = await client.ReceiveAsync(
                    new ArraySegment<byte>(clientBuffer),
                    CancellationToken.None
                );
                Assert.True(result.EndOfMessage);
                Assert.Equal(originalData.Length, result.Count);
                ...
                do
                {
                    result = await client.ReceiveAsync(
                        new ArraySegment<byte>(
                            clientBuffer,
                            receivedCount,
                            clientBuffer.Length - receivedCount
                        ),
                        CancellationToken.None
                    );
            ...
            using (var client = new ClientWebSocket())
            {
                ...
                result = await client.ReceiveAsync(
                    new ArraySegment<byte>(
                        clientBuffer,
                        totalReceived,
                        clientBuffer.Length - totalReceived
                    ),
                    CancellationToken.None
                );
                Assert.False(result.EndOfMessage);
    ...
    public async Task WebSocket_Abort_Interrupts_Pending_ReceiveAsync()
    {
        ...
        await using (
            var server = KestrelWebSocketHelpers.CreateServer(
                ...
                async context =>
                {
                    ...
                    {
                        while (
                            serverSocket.State is WebSocketState.Open or WebSocketState.CloseSent
                        )
                        {
                            if (firstReceiveOccured.IsSet)
                            {
                                var pendingResponse = serverSocket.ReceiveAsync(
                            ...
                            {
                                var response = await serverSocket.ReceiveAsync(
                                    new ArraySegment<byte>(new byte[1024]),
                                    default
                                );
        ...
        )
        {
            ...
            using (var client = new ClientWebSocket())
            {
                ...
                Assert.True(
                    socketWasAcceptedDidNotTimeout,
                    "Socket was not accepted within the allotted time."
                );
                ...
                Assert.True(
                    firstReceiveOccuredDidNotTimeout,
                    "First receive did not occur within the allotted time."
                );

                var secondReceiveInitiatedDidNotTimeout = secondReceiveInitiated.Wait(10000);
                Assert.True(
                    secondReceiveInitiatedDidNotTimeout,
                    "Second receive was not initiated within the allotted time."
                );
                ...
                Assert.True(
                    socketWasAbortedDidNotTimeout,
                    "Abort did not occur within the allotted time."
                );
            }
        }
    }
    ...
    public async Task WebSocket_AllowsCancelling_Pending_ReceiveAsync_When_CancellationTokenProvided()
    {
        ...
        )
        {
            ...
            using (var client = new ClientWebSocket())
            {
                ...
                Assert.True(
                    operationWasCancelledDidNotTimeout,
                    "Cancel did not occur within the allotted time."
                );
            }
        }
    }
            ...
            var server = KestrelWebSocketHelpers.CreateServer(
                ...
                async context =>
                {
                    ...
                    Assert.Same(
                        ...
                        context.Request.Headers.SecWebSocketVersion.ToString()
                    );
                }
            )
        )
