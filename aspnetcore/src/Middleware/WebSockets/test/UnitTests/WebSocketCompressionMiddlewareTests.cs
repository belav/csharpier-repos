...
public class WebSocketCompressionMiddlewareTests : LoggedTest
{
    ...
    [Theory]
    [InlineData(
        "permessage-deflate; server_max_window_bits=14, permessage-deflate; server_max_window_bits=13",
        "permessage-deflate; server_max_window_bits=13"
    )]
    [InlineData(
        "permessage-deflate; client_max_window_bits=8, permessage-deflate; client_max_window_bits=13",
        "permessage-deflate; client_max_window_bits=13; server_max_window_bits=13"
    )]
    public async Task CompressionNegotiationCanChooseExtension(
        string clientHeader,
                ...
                async context =>
                {
                    ...
                    while (true)
                    {
                        ...
                        await webSocket.SendAsync(
                            serverBuffer.AsMemory(0, result.Count),
                            result.MessageType,
                            result.EndOfMessage,
                            default
                        );
        ...
        )
        {
            using (var client = new ClientWebSocket())
            {
                ...
                await client.SendAsync(
                    clientBuf.AsMemory(0, sendCount),
                ...
                while (totalRecv < sendCount)
                {
                    ...
                    if (result.EndOfMessage)
                    {
                        ...
                        {
                            Assert.True(
                                clientBuf[i] == receiveBuf[i],
                                $"offset {i} not equal: {clientBuf[i]} == {receiveBuf[i]}"
                            );
                        }
                    }
                }
