...
namespace EchoApp
{
    public class Startup
    {
            ...
            while (result.MessageType != WebSocketMessageType.Close)
            {
                ...
                if (result.MessageType == WebSocketMessageType.Text)
                {
                    ...
                    if (content.Equals("ServerClose"))
                    {
                        ...
                        logger.LogDebug(
                            $"Sent Frame Close: {WebSocketCloseStatus.NormalClosure} Closing from Server"
                        );
                        return;
                    }
                ...
                logger.LogDebug(
                    $"Sent Frame {result.MessageType}: Len={result.Count}, Fin={result.EndOfMessage}: {content}"
                );
        ...
        private void LogFrame(
            ...
            {
                message =
                    $"Close: {webSocket.CloseStatus.Value} {webSocket.CloseStatusDescription}";
            }
            else
            {
                string content = "<<binary>>";
                ...
                message =
                    $"{frame.MessageType}: Len={frame.Count}, Fin={frame.EndOfMessage}: {content}";
            }
