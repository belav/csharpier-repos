...
public class RequestLocalizationOptions
{
    ...
    /// <summary>
    /// Gets or sets the default culture to use for requests when a supported culture could not be
    // determined by
    /// one of the configured <see cref="IRequestCultureProvider"/>s.
    /// Defaults to <see cref="CultureInfo.CurrentCulture"/> and <see
    ...
    /// <summary>
    /// Gets or sets a value indicating whether to set a request culture to an parent culture in the
    // case the
    ...
    /// </remarks>
    /// <example>
    /// If this property is <c>true</c> and the application is configured to support the culture "fr",
    // but not the
    /// culture "fr-FR", and a configured <see cref="IRequestCultureProvider"/> determines a request's
    // culture is
    /// "fr-FR", then the request's culture will be set to the culture "fr", as it is a parent of
    // "fr-FR".
    /// </example>
    public bool FallBackToParentCultures { get; set; } = true;

    /// <summary>
    /// Gets or sets a value indicating whether to set a request UI culture to a parent culture in the
    // case the
    ...
    /// </remarks>
    /// <example>
    /// If this property is <c>true</c> and the application is configured to support the UI culture
    // "fr", but not
    /// the UI culture "fr-FR", and a configured <see cref="IRequestCultureProvider"/> determines a
    // request's UI
    /// culture is "fr-FR", then the request's UI culture will be set to the culture "fr", as it is a
    // parent of
    /// "fr-FR".
    /// </example>
    ...
    /// <summary>
    /// Gets or sets a value that determines if <see cref="CultureInfo.CurrentUICulture" /> is applied
    // to the response <c>Content-Language</c> header.
    /// </summary>
    public bool ApplyCurrentCultureToResponseHeaders { get; set; }

    /// <summary>
    /// The cultures supported by the application. The <see cref="RequestLocalizationMiddleware"/> will
    // only set
    ...
    /// <summary>
    /// The UI cultures supported by the application. The <see cref="RequestLocalizationMiddleware"/>
    // will only set
    /// the current request culture to an entry in this list.
    ...
    /// <summary>
    /// An ordered list of providers used to determine a request's culture information. The first
    // provider that
    ...
    // cref="AcceptLanguageHeaderRequestCultureProvider"/></description></item>
    /// </list>
    /// </summary>
    ...
    public RequestLocalizationOptions AddSupportedCultures(params string[] cultures)
    {
        ...
        {
            supportedCultures.Add(
                new CultureInfo(culture, useUserOverride: CultureInfoUseUserOverride)
            );
        }
    ...
    /// <summary>
    /// Set the default culture which is used by the application when a supported culture could not be
    // determined by
    /// one of the configured <see cref="IRequestCultureProvider"/>s.
    /// </summary>
