    ...
    public class MicrosoftAccountTests : RemoteAuthenticationTests<MicrosoftAccountOptions>
    {
        ...
        public async Task ChallengeWillIncludeScopeAsOverwrittenWithBaseAuthenticationProperties()
        {
            ...
            var transaction = await server.SendAsync(
                "http://example.com/challengeWithOtherScopeWithBaseAuthenticationProperties"
            );
            var res = transaction.Response;
            Assert.Equal(HttpStatusCode.Redirect, res.StatusCode);
            Assert.Contains("scope=baz%20qux", res.Headers.Location.Query);
        ...
        public async Task AuthenticatedEventCanGetRefreshToken()
        {
            ...
            using var server = host.GetTestServer();
            var transaction = await server.SendAsync(
                "https://example.com/signin-microsoft?code=TestCode&state="
            ...
            Assert.Contains(
                $".AspNetCore.Correlation.{correlationValue}",
                    ...
                    o.BackchannelHttpHandler = new TestHttpMessageHandler
                    {
                        Sender = req =>
                        {
                            ...
                            )
                            {
                                ...
                                Assert.Equal(
                                    "https://example.com/signin-microsoft",
...
#pragma warning restore CS0618 // Type or member is obsolete
                                            ...
                                            req.Path == new PathString("/challengeWithOtherScope")
                                        )
                                        {
