...
namespace Microsoft.AspNetCore.Authentication.Cookies
{
    public class CookieTests : SharedAuthenticationTests<CookieAuthenticationOptions>
    {
        ...
        {
            using var host = await CreateHostWithServices(s => s.AddAuthentication().AddCookie(
                        o =>
                        {
        ...
        public async Task CustomAuthSchemeEncodesCookieName()
        {
            ...
            using var host = await CreateHostWithServices(
                s => s.AddAuthentication(schemeName).AddCookie(
                        schemeName,
                        o =>
                        {
                            o.LoginPath = new PathString("/login");
            ...
            using var host = await CreateHost(
                o =>
                {
                    ...
                    o.Events = new CookieAuthenticationEvents()
                    {
                        OnSigningIn = context =>
                        {
                            context.Properties.ExpiresUtc = _clock.UtcNow
        ...
        private static Task DescribeAsync(HttpResponse res, AuthenticateResult result)
        {
            ...
            {
                xml.Add(
                    result.Ticket.Properties.Items
                        .Select(
                            extra =>
                                new XElement(
                                    "extra",
