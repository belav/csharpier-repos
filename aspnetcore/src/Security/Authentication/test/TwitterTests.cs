...
public class TwitterTests : RemoteAuthenticationTests<TwitterOptions>
{
    ...
    public async Task HandleRequestAsync_RedirectsToAccessDeniedPathWhenExplicitlySet()
    {
        ...
        Assert.Contains("https://api.twitter.com/oauth/authenticate?oauth_token=", location);
        Assert.True(
            transaction.Response.Headers.TryGetValues(HeaderNames.SetCookie, out var setCookie)
        );
        ...
        Assert.Equal(
            "http://localhost/access-denied?ReturnUrl=%2Fchallenge",
    ...
    {
        var stateFormat = new SecureDataFormat<RequestToken>(
        ...
        Assert.Equal(
            "123456",
            transaction.FindClaimValue(ClaimTypes.NameIdentifier, expectedIssuer)
        );
        Assert.Equal("123456", transaction.FindClaimValue("urn:twitter:userid", expectedIssuer));
        Assert.Equal(
            "@dotnet",
            transaction.FindClaimValue("urn:twitter:screenname", expectedIssuer)
        );
        ...
        if (req.RequestUri.AbsoluteUri == "https://api.twitter.com/oauth/request_token")
        {
            return new HttpResponseMessage(HttpStatusCode.OK)
            {
                Content = new StringContent(
                    "oauth_callback_confirmed=true&oauth_token=test_oauth_token&oauth_token_secret=test_oauth_token_secret",
                    Encoding.UTF8,
    ...
    )
    {
        if (req.RequestUri.AbsoluteUri == "https://api.twitter.com/oauth/request_token")
        {
            return new HttpResponseMessage(HttpStatusCode.Forbidden)
            {
                Content = new StringContent("example response text", Encoding.UTF8, "text/html"),
            };
        }
        throw new NotImplementedException(req.RequestUri.AbsoluteUri);
    }
