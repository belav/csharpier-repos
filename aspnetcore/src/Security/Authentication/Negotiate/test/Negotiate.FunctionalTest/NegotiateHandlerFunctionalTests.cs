...
namespace Microsoft.AspNetCore.Authentication.Negotiate
{
    ...
    public class NegotiateHandlerFunctionalTests : LoggedTest
    {
        ...
        [MinimumOSVersion(
            ...
            SkipReason = "Windows only supports ALPN and required ciphers on 10 and later."
        )]
        ...
        {
            using var host = await CreateHostAsync(
                options => options.PersistNtlmCredentials = true
            );
            // https://github.com/dotnet/corefx/issues/35195 SocketHttpHandler won't downgrade HTTP/2. WinHttpHandler does.
            using var client = CreateWinHttpClient(host);
            ...
            result = await client.SendAsync(
                new HttpRequestMessage(HttpMethod.Get, "/AlreadyAuthenticated") { Version = second }
            );
            Assert.Equal(HttpStatusCode.OK, result.StatusCode);
            Assert.Equal(Http11Version, result.Version);
        }
        ...
        )
        {
            ...
            // Re-uses the 1.1 connection.
            result = await client.SendAsync(
                new HttpRequestMessage(HttpMethod.Get, "/Authenticate") { Version = second }
            );
            Assert.Equal(HttpStatusCode.OK, result.StatusCode);
            Assert.Equal(Http11Version, result.Version);
        }
        ...
        [MinimumOSVersion(
            OperatingSystems.Windows,
            WindowsVersions.Win10,
            SkipReason = "The client only supports HTTP/2 on Win10."
        )]
