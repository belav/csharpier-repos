...
/// <summary>
/// Provides programmatic configuration used by <see cref="IAuthorizationService"/> and <see
// cref="IAuthorizationPolicyProvider"/>.
/// </summary>
public class AuthorizationOptions
{
    ...
    /// <summary>
    /// Determines whether authorization handlers should be invoked after <see
    // cref="AuthorizationHandlerContext.HasFailed"/>.
    /// Defaults to true.
    /// </summary>
    ...
    /// </summary>
    /// <remarks>
    /// The default policy used when evaluating <see cref="IAuthorizeData"/> with no policy name
    // specified.
    /// </remarks>
    ...
    /// <summary>
    /// Gets or sets the fallback authorization policy used by <see
    // cref="AuthorizationPolicy.CombineAsync(IAuthorizationPolicyProvider, IEnumerable{IAuthorizeData})"/>
    /// when no IAuthorizeData have been provided. As a result, the AuthorizationMiddleware uses the
    // fallback policy
    /// if there are no <see cref="IAuthorizeData"/> instances for a resource. If a resource has any
    // <see cref="IAuthorizeData"/>
    /// then they are evaluated instead of the fallback policy. By default the fallback policy is null,
    // and usually will have no
    /// effect unless you have the AuthorizationMiddleware in your pipeline. It is not used in any way
    // by the
    ...
    /// <returns>The policy for the specified name, or null if a policy with the name does not
    // exist.</returns>
