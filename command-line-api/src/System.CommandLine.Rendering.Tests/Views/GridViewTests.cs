...
namespace System.CommandLine.Rendering.Tests.Views
{
    public class GridViewTests
    {
        ...
        public void Column_width_definition_size_calculation_according_priority_fixed_then_size_to_content_then_star(
        ...
        [InlineData(OutputMode.Ansi)]
        [InlineData(OutputMode.NonAnsi)]
        public void Row_height_definition_size_calculation_according_priority_fixed_then_size_to_content_then_star(
            OutputMode outputMode
        )
        {
            var grid = new GridView();
        ...
        [InlineData(OutputMode.Ansi)]
        [InlineData(OutputMode.NonAnsi)]
        public void Column_width_definition_is_preserved_even_defintion_is_mixed_for_subsequent_columns2(
            OutputMode outputMode
        )
        {
            var grid = new GridView();
            grid.SetColumns(
                ColumnDefinition.Fixed(9),
                ColumnDefinition.Star(1),
                ColumnDefinition.SizeToContent(),
                ColumnDefinition.Fixed(15)
            );
            grid.SetChild(new ContentView("The quick"), 0, 0);
            grid.SetChild(new ContentView("brown fox"), 1, 0);
            grid.SetChild(new ContentView("jumped"), 2, 0);
        ...
        [InlineData(OutputMode.Ansi)]
        [InlineData(OutputMode.NonAnsi)]
        public void Column_width_definition_is_preserved_even_defintion_is_mixed_for_subsequent_columns(
            OutputMode outputMode
        )
        {
            var grid = new GridView();
