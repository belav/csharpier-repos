...
namespace System.CommandLine.Tests
{
    public class SuggestDirectiveTests
    {
        ...
        public SuggestDirectiveTests()
        {
            ...
            _vegetableOption.CompletionSources.Add(_ =>
                new[] { "asparagus", "broccoli", "carrot" }
            );
        ...
        {
            CliRootCommand rootCommand = new() { _eatCommand, new SuggestDirective() };
            ...
            config.Output.ToString().Should().Be($"apple{NewLine}banana{NewLine}cherry{NewLine}");
        }
        ...
        {
            CliRootCommand rootCommand = new() { _vegetableOption, _fruitOption };
            CliConfiguration config = new(rootCommand) { Output = new StringWriter() };
            ...
            config
                ...
                .Should()
                .Be(
                    $"--fruit{NewLine}--help{NewLine}--vegetable{NewLine}--version{NewLine}-?{NewLine}-h{NewLine}/?{NewLine}/h{NewLine}"
                );
        }
        ...
        public async Task It_writes_suggestions_for_subcommand_aliases_under_root_command()
        {
            ...
            config
                ...
                .Should()
                .Be(
                    $"--help{NewLine}--version{NewLine}-?{NewLine}-h{NewLine}/?{NewLine}/h{NewLine}eat{NewLine}"
                );
        }
        ...
        public async Task It_writes_suggestions_for_partial_option_aliases_under_root_command()
        {
            ...
            config.Output.ToString().Should().Be($"--fruit{NewLine}");
        }
        ...
        public async Task It_writes_suggestions_for_partial_subcommand_aliases_under_root_command()
        {
            ...
            config.Output.ToString().Should().Be($"wash-dishes{NewLine}");
        }
        ...
        public async Task It_writes_suggestions_for_partial_option_and_subcommand_aliases_under_root_command()
        {
            ...
            config.Output.ToString().Should().Be($"--version{NewLine}");
        }
        ...
        public async Task It_writes_suggestions_for_partial_option_and_subcommand_aliases_under_root_command_with_an_argument()
        {
            ...
            config.Output.ToString().Should().Be($"--option1{NewLine}--option2{NewLine}");
        }

        [Fact]
        public async Task It_does_not_repeat_suggestion_for_already_specified_bool_option()
        {
            var command = new CliRootCommand { new CliOption<bool>("--bool-option") };
            CliConfiguration config = new(command) { Output = new StringWriter() };

            var commandLine = "--bool-option false";

            await command
                .Parse($"[suggest:{commandLine.Length + 1}] \"{commandLine}\"", config)
                .InvokeAsync();

            config.Output.ToString().Should().NotContain("--bool-option");
        }
    }
}
