    ...
    public class GridView : LayoutView<View>
    {
        ...
        public override void Add(View child) =>
            throw new InvalidOperationException(
                "Must call SetChild(View child, int column, int row) instead"
            );
        ...
        public void SetColumns(params ColumnDefinition[] columns)
        {
            ...
            {
                throw new InvalidOperationException(
                    "Cannot change columns once children are added"
                );
            }
        ...
        public override void Render(ConsoleRenderer renderer, Region region)
        {
            ...
            for (int row = 0; row < _rows.Count; row++)
            {
                ...
                for (int column = 0; column < _columns.Count; column++)
                {
                    ...
                    if (
                        ChildLocations[column, row] is not { } child
                    ...
                    var contentSize = new Size(
                        sizes[column, row].Width - paddingWidth,
                        sizes[column, row].Height
                    );
                    ...
                    if (paddingWidth > 0)
                    {
                        ...
                        renderer.RenderToRegion(
                            TextSpan.Empty(),
                            new Region(left + contentSize.Width, top, paddingSize)
                        );
                    }
        ...
        {
            int countStarSizedColumnsWithPadding = _columns
                .AsEnumerable()
                .Reverse()
                .Skip(1)
                .Count(x => x.SizeMode == SizeMode.Star);
            ...
            foreach (
                var (column, columnIndex) in _columns
                    .Select((definition, columnIndex) => (definition, columnIndex))
                    .OrderBy(t => GetProcessOrder(t.definition.SizeMode))
            )
            {
                ...
                foreach (
                    var (row, rowIndex) in _rows
                        .Select((definition, rowIndex) => (definition, rowIndex))
                        .OrderBy(t => GetProcessOrder(t.definition.SizeMode))
                )
                {
                    if (measuredRows[rowIndex] == null)
                    {
                        switch (row.SizeMode)
                        {
                            ...
                            default:
                                throw new InvalidOperationException(
                                    $"Unknown row size mode {row.SizeMode}"
                                );
                        }
                    }
                    ...
                    switch (column.SizeMode)
                    {
                        case SizeMode.Fixed:
                        {
                            ...
                            {
                                measuredColumns[columnIndex] = Math.Min(
                                    (int)column.Value + paddingWidth,
                                    availableWidth
                                );
                            }
                            break;
                        ...
                        case SizeMode.SizeToContent:
                            {
                                ...
                                measuredColumns[columnIndex] = Math.Min(
                                    Math.Max(measuredColumns[columnIndex] ?? 0, width),
                                    availableWidth
                                );
                            }
                            break;
                        ...
                        default:
                            throw new InvalidOperationException(
                                $"Unknown column size mode {column.SizeMode}"
                            );
                    }
                    ...
                    if (row.SizeMode == SizeMode.SizeToContent)
                    {
                        ...
                        measuredRows[rowIndex] = Math.Min(
                            Math.Max(childSize?.Height ?? 0, childSize?.Height ?? 0),
                            availableHeight
                        );
                    }
            ...
            for (int rowIndex = 0; rowIndex < _rows.Count; rowIndex++)
            {
                ...
                {
                    rv[columnIndex, rowIndex] = new Size(
                        measuredColumns[columnIndex].Value,
                        measuredRows[rowIndex].Value
                    );
                }
            }
            return rv;
        ...
        private int GetColumnPaddingWidth(int columnIndex) =>
            columnIndex < _columns.Count - 1 ? _columnPaddingRight : 0;
