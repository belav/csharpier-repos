...
public partial class ModelBindingCommandHandlerTests
{
    public class BindingByName
    {
        ...
        )
        {
            ...
            CliConfiguration configuration = new(command) { Output = new StringWriter() };
        ...
        )
        {
            ...
            var command = new CliCommand("the-command") { ArgumentBuilder.CreateArgument(type) };
            command.Action = handler;
            CliConfiguration configuration = new(command) { Output = new StringWriter() };
        ...
        public void When_argument_type_is_more_specific_than_parameter_type_then_parameter_is_bound_correctly()
        {
            ...
            var root = new CliRootCommand { new CliOption<DirectoryInfo>("-f") };
